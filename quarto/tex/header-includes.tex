...
\renewcommand{\sidenote}[1]{%
  \oldsidenote{%
    ...
    \footnotesize #1 % Sidenote text in smaller font size (removed braces to preserve formatting)
  }%
}
...
\renewcommand{\cftchapfont}{\bfseries\color{crimson}} % Makes TOC chapters bold and crimson
\renewcommand{\cftchappresnum}{\color{crimson}Chapter~} % Adds "Chapter" prefix (crimson)

% Custom formatting for division entries (using chapter level but styled like parts)
\newcommand{\divisionchapter}[1]{%
  \addvspace{12pt}% Space before division
  \noindent\hfil\bfseries\color{crimson}#1\hfil\par% Centered division title
  \addvspace{6pt}% Space after division
}

% Part TOC formatting is now handled by Lua filter for consistency
...
% Margin collision detection system
\newcounter{marginposition}
\newcounter{lastmarginheight}
\setcounter{marginposition}{0}
\setcounter{lastmarginheight}{0}

% Command to calculate safe margin spacing
\newcommand{\safemarginspace}[1]{%
  \stepcounter{marginposition}%
  \ifnum\value{marginposition}>1%
    \ifnum\value{lastmarginheight}>0%
      \vspace{\thelastmarginheight mm}%
    \fi%
  \fi%
  \setcounter{lastmarginheight}{#1}%
}
