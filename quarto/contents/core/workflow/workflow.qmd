...
**Deployment and Integration**: Once validated, the trained model is integrated into production systems and workflows. This stage requires addressing practical challenges such as system compatibility, scalability, and operational constraints across different deployment contexts ranging from cloud to edge environments, as explored in [Chapter @sec-ml-systems].
...
Data is the foundation of machine learning systems, yet collecting and preparing data for ML applications introduces challenges that extend far beyond gathering enough training examples[^fn-data-challenges]. These fundamental data challenges form the core focus of [Chapter @sec-data-engineering], which provides comprehensive coverage of data collection, preprocessing, and quality assurance. Modern ML systems often need to handle terabytes of data, which range from raw, unstructured inputs to carefully annotated datasets, while maintaining quality, diversity, and relevance for model training. For medical systems like DR screening, data preparation must meet the highest standards to ensure diagnostic accuracy.
...
These operational realities shaped the system architecture in significant ways. The volume and size of high-resolution images necessitated local storage and preprocessing capabilities at clinics, as centralizing all data collection was impractical due to unreliable internet access[^fn-edge-computing]. Furthermore, patient privacy regulations required secure data handling at every stage, from image capture to model training[^fn-medical-privacy]. Coordinating expert annotations also introduced logistical challenges, necessitating systems that could bridge the physical distance between clinics and ophthalmologists while maintaining workflow efficiency.

[^fn-edge-computing]: **Edge Computing in Healthcare**: Healthcare edge computing deployments grew 67% annually from 2018-2023, driven by data sovereignty requirements and bandwidth constraints. A single high-resolution retinal scan (4096x4096 pixels) generates 50-200MB files; rural clinics processing 50+ patients daily would require 2.5-10GB daily uploads—impossible with typical rural bandwidth of 1-5 Mbps. Edge deployment reduces bandwidth needs by 95% but increases local compute requirements 10x, creating a fundamental trade-off between connectivity and computational resources that doesn't exist in traditional cloud applications.
