...
- Understand the ML lifecycle's structured, iterative framework and how its six stages—problem definition, data collection, model development, evaluation, deployment, and maintenance—interconnect through continuous feedback loops.

- Identify the fundamental distinctions between traditional software and machine learning lifecycles, including their approaches to problem definition, testing validation, deployment behavior, and maintenance requirements.

- Examine system-level considerations including resource constraints, infrastructure limitations, deployment environments, and how these operational realities shape architectural decisions throughout the development process.

- Recognize how feedback loops operate across multiple timescales—from minute-level operational adjustments to quarterly strategic evaluations—enabling continuous improvement and adaptation in production ML systems.

- Apply systems thinking patterns including constraint propagation, multi-scale feedback, emergent complexity, and resource optimization to understand how decisions in one lifecycle stage create cascading effects throughout the entire system development process.

:::

## Overview {#sec-ai-workflow-overview-97fb}

Building upon the foundational principles established in Part I—system characteristics, deployment environments, mathematical frameworks, and architectural patterns—this chapter advances the discourse from component-level analysis to system-level engineering. The transition from theoretical understanding to operational implementation requires a systematic framework that governs the development of production machine learning systems.
...
## The ML Lifecycle Framework {#sec-ai-workflow-ml-lifecycle-framework-dd7f}
...
Understanding this lifecycle requires a systems thinking approach that recognizes four fundamental patterns: constraint propagation (how decisions in one stage influence all others), multi-scale feedback loops (how systems adapt across different timescales), emergent complexity (how system-wide behaviors differ from component behaviors), and resource optimization (how trade-offs create interdependencies). These patterns, which we explore throughout our diabetic retinopathy case study, provide the analytical framework for understanding why ML systems demand integrated engineering approaches rather than sequential component optimization.
...
This workflow framework serves as scaffolding for the technical chapters ahead. Data engineering principles detailed in @sec-data-engineering ensure the data pipeline's quality and lifecycle management throughout ML development. The iterative training methodologies that @sec-ai-training explores enable efficient model development at scale, supported by software frameworks from @sec-ai-frameworks that enable this iterative development process. Production deployment and ongoing operations through @sec-ml-operations address how systems maintain performance across diverse environments. This chapter establishes how these pieces interconnect before we explore each in depth—understanding the complete system makes the specialized components meaningful.
...
These six dimensions reveal a fundamental pattern: machine learning systems replace deterministic specifications with probabilistic optimization, static behavior with dynamic adaptation, and isolated development with continuous feedback. This shift explains why traditional project management approaches often fail when applied to ML projects without modification.

It is crucial to understand that experimentation in machine learning differs fundamentally from testing in traditional software. In ML, experimentation is not just about finding bugs in code; it is the core development process itself. It involves systematically testing hypotheses about which data sources, feature engineering approaches, model architectures, or hyperparameters will yield the best performance for a given task. This represents a scientific process of discovery, not merely a quality assurance step. Where traditional software testing verifies that code behaves according to predetermined specifications, ML experimentation explores an uncertain space to discover which combination of choices produces the best empirical results.

The differences emphasize the need for a robust ML lifecycle framework that can accommodate iterative development, dynamic behavior, and data-driven decision-making. Understanding these distinctions prepares us to examine how ML projects unfold through their lifecycle stages, each presenting unique challenges that traditional software methodologies cannot adequately address.
...
### AI Lifecycle in Practice: The DR Screening System {#sec-ai-workflow-ai-lifecycle-practice-dr-screening-system-7412}
...
#### The Clinical Challenge {#sec-ai-workflow-clinical-challenge-6504}
...
#### The Broader Significance {#sec-ai-workflow-broader-significance-e50f}
...
### Scaling Problem Definition {#sec-ai-workflow-scaling-problem-definition-5e34}
...
Each high-resolution retinal scan generates files ranging from 45-120 megabytes, creating substantial infrastructure challenges. A typical clinic processing 50 patients daily generates 10-15 GB of imaging data per week, quickly exceeding the capacity of rural internet connections (often limited to 2-10 Mbps upload speeds). This data volume constraint forces architectural decisions toward edge-computing solutions rather than cloud-based processing.
...
### Laboratory-to-Clinic Data Gaps {#sec-ai-workflow-laboratorytoclinic-data-gaps-a614}
...
### Infrastructure Design Principles {#sec-ai-workflow-infrastructure-design-principles-61c7}
...
### Scaling Data Collection {#sec-ai-workflow-scaling-data-collection-45fa}
...
These constraints drive architectural innovations including model optimization techniques for size reduction, inference acceleration, and efficient deployment scenarios—balancing deep convolutional network demands (explored in @sec-dnn-architectures) against the edge device constraints that @sec-ml-systems addresses in deployment scenarios.

Following the iterative development framework we've established, the model development process requires continuous iteration between accuracy optimization and efficiency optimization. Each architectural decision—from convolutional layer counts to activation function choices that @sec-dl-primer covers, through to the overall network depth that @sec-dnn-architectures explores—must be validated against test set metrics and the infrastructure constraints identified during data collection. This multi-objective optimization approach exemplifies the interdependence principle where deployment constraints shape development decisions.

### Performance-Deployment Trade-offs {#sec-ai-workflow-performancedeployment-tradeoffs-63ff}
...
Optimizing for clinical performance alone proves insufficient. Edge deployment constraints from the data collection phase impose additional requirements: the model must run efficiently on resource-limited hardware while maintaining real-time inference speeds compatible with clinical workflows. This creates a multi-objective optimization problem where improvements in one dimension often come at the cost of others—a fundamental tension between model capacity from @sec-dnn-architectures and the deployment feasibility that @sec-ml-systems discusses. Teams may discover that an original 2GB model with 95.2% accuracy can be optimized to 96MB with 94.8% accuracy through systematic application of quantization, pruning, and knowledge distillation techniques, achieving deployment requirements while maintaining clinical utility.
...
### Scaling Model Development {#sec-ai-workflow-scaling-model-development-2544}
...
### Scaling Deployment Across Diverse Environments {#sec-ai-workflow-scaling-deployment-across-diverse-environments-e550}
...
### Scaling Monitoring and Maintenance {#sec-ai-workflow-scaling-monitoring-maintenance-b8bc}
...
### Multi-Scale Feedback Loops {#sec-ai-workflow-multiscale-feedback-loops-7174}
...
### Managing Emergent Complexity {#sec-ai-workflow-managing-emergent-complexity-9e60}
...
### The Path Forward: Integrated System Development {#sec-ai-workflow-path-forward-integrated-system-development-95a9}
