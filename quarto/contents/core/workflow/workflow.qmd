...
The machine learning (ML) lifecycle is a structured, iterative process that guides the development, evaluation, and continual improvement of machine learning systems. Integrating ML into broader software engineering practices introduces unique challenges that necessitate systematic approaches to experimentation, evaluation, and adaptation over time [@amershi2019software]. This systematic approach builds upon decades of structured development methodologies [@chapman2000crisp] that have evolved to address the unique challenges of data-driven systems, fundamentally different from traditional software engineering principles explored in @sec-introduction.
...
**Data Collection and Preparation**: This stage includes gathering relevant data, cleaning it, and preparing it for model training. This process often involves curating diverse datasets, ensuring high-quality labeling, and developing preprocessing pipelines to address variations in the data. The complexities of this stage are thoroughly explored in @sec-data-engineering.

**Model Development and Training**: In this stage, researchers select appropriate algorithms, design model architectures, and train models using the prepared data. Success depends on choosing techniques suited to the problem and iterating on the model design for optimal performance. The training methodologies and infrastructure requirements are detailed in @sec-ai-training, while the underlying architectures are covered in @sec-dnn-architectures.
...
The development of machine learning systems begins with a critical challenge that significantly differs from traditional software development: defining not just what the system should do, but how it should learn to do it. Unlike conventional software, where requirements directly translate into implementation rules, ML systems require teams to consider how the system will learn from data while operating within real-world constraints[^fn-problem-definition]. This stage lays the foundation for all subsequent phases in the ML lifecycle.
...
Data is the foundation of machine learning systems, yet collecting and preparing data for ML applications introduces challenges that extend far beyond gathering enough training examples[^fn-data-challenges]. These fundamental data challenges form the core focus of @sec-data-engineering, which provides comprehensive coverage of data collection, preprocessing, and quality assurance. Modern ML systems often need to handle terabytes of data, which range from raw, unstructured inputs to carefully annotated datasets, while maintaining quality, diversity, and relevance for model training. For medical systems like DR screening, data preparation must meet the highest standards to ensure diagnostic accuracy.
...
In the DR project, data collection involved a development dataset of 128,000 retinal fundus photographs evaluated by a panel of 54 ophthalmologists, with each image reviewed by 3-7 experts[^fn-medical-annotation]. This collaborative effort ensured high-quality labels that captured clinically relevant features like microaneurysms, hemorrhages, and hard exudates. The project's infrastructure requirements became immediately apparent: each high-resolution retinal scan generated 50-200MB files, requiring specialized storage architecture with object storage for archives, distributed filesystems for processing, and in-memory stores for real-time serving. The team implemented a multi-tier storage design with hot tier access times under 100ms for active training data and warm tier storage for historical datasets. Additionally, clinical validation datasets comprising 12,000 images provided an independent benchmark to test the model's robustness against real-world variability, with the validation infrastructure requiring 64GB RAM per GPU and 10Gb/s network bandwidth for efficient data loading during distributed training across 8-GPU clusters. The scale and complexity of this effort highlight how domain expertise and interdisciplinary collaboration are critical to building datasets for high-stakes ML systems.
...
These operational realities shaped the system architecture in significant ways. Rural clinics processing 50 patients daily generated 2.5-10GB of imaging data, far exceeding the 1-5 Mbps bandwidth typical in these environments. The team's solution involved edge computing infrastructure at each clinic: local preprocessing reduced bandwidth requirements by 95% while requiring 10x local computational resources. The distributed system architecture comprised NVIDIA Jetson edge devices (4GB RAM, 128 CUDA cores) for local inference, clinic aggregation servers (8-core CPUs, 32GB RAM) for data management, and cloud training infrastructure using 32-GPU clusters for weekly model updates. This distributed orchestration achieved under 100ms inference latency with 94% uptime across 200+ clinic deployments, but required careful coordination protocols for managing experiment tracking and model versioning across the distributed infrastructure. Furthermore, patient privacy regulations required secure data handling at every stage, with federated learning architecture enabling model training without centralizing sensitive patient data. Coordinating expert annotations also introduced logistical challenges, necessitating systems that could bridge the physical distance between clinics and ophthalmologists while maintaining workflow efficiency.
...
At each step, the system must balance local needs with centralized aggregation requirements. Clinics with reliable high-speed internet could transmit data in real-time, but many rural locations relied on store-and-forward systems, where data was queued locally and transmitted in bulk when connectivity permitted. The infrastructure design followed a rule of thumb: budget 10x raw data size for intermediate processing files, 3x for data versioning, and 5x for backup and redundancy. This translated to a multi-tier storage architecture where hot tier SSD storage provided sub-100ms access times for active training data, warm tier HDD storage served historical data with under 1-second access, and cold tier object storage archived older datasets with acceptable 5-second retrieval times. Intelligent caching based on access patterns enabled the system to prefetch frequently used data, optimizing the workflow for the 80/20 distribution where 20% of datasets accounted for 80% of access requests. These differences necessitated flexible infrastructure that could adapt to varying conditions while maintaining data consistency and integrity across the lifecycle. This adaptability ensured that the system could function reliably despite the diverse operational environments of the clinics.
...
Model development and training form the core of machine learning systems, yet this stage presents unique challenges that extend far beyond selecting algorithms and tuning hyperparameters[^fn-hyperparameter-tuning]. The comprehensive training methodologies, infrastructure requirements, and distributed training strategies are thoroughly covered in @sec-ai-training. It involves designing architectures suited to the problem, optimizing for computational efficiency, and iterating on models to balance performance with deployability. In high-stakes domains like healthcare, the stakes are particularly high, as every design decision impacts clinical outcomes.
...
As the project advanced to model design and training, computational demands escalated. Training deep learning models on high-resolution images required extensive GPU resources and sophisticated infrastructure. The team discovered that production ML workflows create multiplicative computational costs: 10 model variants × 5 hyperparameter sweeps × 3 data preprocessing approaches = 150 training runs. At $1000 per run for large models, iteration costs reached $150K per experiment cycle. This drove the implementation of scalable training systems with intelligent resource sharing through job scheduling, caching strategies for intermediate results, and early stopping techniques for resource optimization. The distributed training infrastructure required coordination across 8-1000 GPUs for hours to days, with automatic retry and failure recovery mechanisms built into Kubernetes-based ML platforms. These systems also supported experiment comparison, enabling rapid evaluation of different architectures, hyperparameters, and preprocessing pipelines while tracking over 100+ experiments per model variant and maintaining terabytes of training artifacts.

Model development was inherently iterative, with each cycle involving adjustments to DNN architectures, refinements of hyperparameters, or incorporations of new data. Unlike traditional software with predictable behavior, learning systems exhibit emergent properties that arise from data-model interactions, making development inherently experimental and uncertain. The team applied scientific methodology principles to their workflow: controlled variables through fixed random seeds and environment versions, confounding factor analysis to separate architectural changes from optimization changes, systematic ablation studies to isolate individual component contributions, and statistical significance testing with multiple runs and confidence intervals. This rigorous approach was essential because ML experiments can fail to produce reproducible results without careful experimental design—the team learned to distinguish correlation from causation in performance improvements and implemented statistical process control to detect significant performance shifts across their iterative development cycles. Each cycle produced extensive metadata, including checkpoints, validation results, and performance metrics. Managing this information across the team required robust tools for experiment tracking and version control to ensure that progress remained organized and reproducible.
...
Large-scale training infrastructure became essential to meet these demands. While it significantly reduced training time, it introduced complexities in data synchronization, gradient aggregation, and fault tolerance. The distributed system architecture required careful orchestration: multi-node distributed training with fault tolerance mechanisms, parallel validation across multiple test environments, coordinated rollout across edge devices and data centers, and centralized observability across distributed components. The team relied on advanced frameworks like Kubeflow Pipelines for native Kubernetes orchestration of ML workloads, component-based pipeline construction with reusable stages, and automatic resource scaling based on workload requirements. These frameworks included automated failure recovery mechanisms and sophisticated monitoring systems that processed 1,000+ model quality checks daily, which helped maintain progress even in the event of hardware interruptions.

The need for continuous experimentation and improvement compounded these challenges. The team discovered that systematic experiment management became critical as they generated terabytes of artifacts including checkpoints, logs, and metrics. Without structured tracking, teams typically lose 40-60% of institutional knowledge from their experiments. The solution involved implementing unique experiment IDs with hierarchical organization, automated artifact versioning with cleanup policies, comprehensive metadata collection including hyperparameters and system resources, and sophisticated search and filtering capabilities to query experiments by performance, configuration, and date. This growth required scalable systems that budgeted 20-30% of compute resources for experiment management overhead while maintaining the ability to analyze results and extract insights that drove continuous improvement across the distributed development teams.
...
Feedback loops are inherent to this stage, but they revealed deeper questions about causal relationships in model performance. When the DR model's accuracy improved after architecture changes, the team needed to understand whether improvements came from increased model capacity, better optimization, or simply random variation. They implemented causal inference techniques to distinguish correlation from causation, using controlled experiments to isolate single variables across feedback cycles and root cause analysis frameworks to map performance changes back to specific workflow modifications. Understanding these cycles became critical for iterative improvement and long-term success, as the team discovered that daily feedback loops enabled 1-2% performance improvement per week, while weekly feedback loops slowed improvement by 3-5x, and monthly loops by 10x or more. This quantitative understanding of feedback timing fundamentally shaped their development velocity and resource allocation decisions.
...
These requirements influenced deployment strategies significantly. A cloud-based deployment, while technically simpler, was not feasible due to unreliable connectivity in many clinics. Instead, the team opted for edge deployment, where models ran locally on clinic hardware. This approach required aggressive model optimization to meet specific hardware constraints: target metrics included under 100MB model size, sub-50ms inference latency, and under 500MB RAM usage on edge devices. Achieving these targets required systematic application of optimization techniques—quantization delivered 4-8x size reduction with typically 1-3% accuracy cost, pruning provided 2-5x speedup but introduced 2-5% accuracy degradation, and knowledge distillation achieved 10-50x size reduction for simpler deployment scenarios. The team discovered that their original 2GB model with 95.2% accuracy could be optimized to 500MB with 94.8% accuracy, meeting deployment constraints while maintaining clinical utility through careful trade-off analysis and systematic evaluation of accuracy versus efficiency.
...
Despite achieving high performance metrics during development, the DR system faced unexpected challenges in real-world deployment that illustrated the fundamental difference between laboratory and production environments. For example, in rural clinics, variations in imaging equipment and operator expertise led to inconsistencies in image quality that the model struggled to handle. Initial prototypes exceeded memory limits on edge devices, prompting emergency model optimization that demonstrated how deployment constraints can propagate backwards through workflow design, affecting data preprocessing strategies, model architecture choices, and validation approaches. These issues underscored the gap between laboratory success and operational reliability, with the team discovering that infrastructure decisions like edge versus cloud deployment created constraint propagation effects—edge deployment minimized latency to under 10ms but limited model complexity, while cloud deployment enabled elastic scaling but introduced 50-200ms latency that was unacceptable for real-time clinical workflows. Feedback from clinicians further revealed that initial system interfaces were not intuitive enough for widespread adoption, leading to additional redesigns that balanced technical sophistication with clinical usability.
...
Under pressure to deliver quickly, teams often bypass thorough validation, testing, and documentation stages. This approach treats validation as overhead rather than essential engineering discipline. Inadequate validation leads to models with hidden biases, poor generalization, or unexpected failure modes that only manifest in production. The cost of fixing these issues after deployment far exceeds the time investment required for systematic validation. Robust workflows embed validation throughout the development process rather than treating it as a final checkpoint, incorporating the benchmarking and evaluation principles detailed in @sec-benchmarking-ai.
