...
Having established the foundational building blocks of machine learning systems—the deployment contexts from @sec-ml-systems, the neural network fundamentals from @sec-dl-primer, and the specialized architectural patterns from @sec-dnn-architectures—we now shift perspective from individual components to integrated systems. This chapter serves as the bridge from understanding ML system components (Part I: Systems Foundations) to engineering with them systematically (Part II: Design Principles).

The architectural choices explored in previous chapters, whether Multi-Layer Perceptrons for tabular data, Convolutional Neural Networks for spatial patterns, Recurrent Neural Networks for sequences, or Transformers for dynamic attention, do not exist in isolation. Each architecture represents an engineering decision within a larger development lifecycle, where constraints from data collection, deployment environments, and operational requirements determine which architectures are viable and how they must be adapted. Understanding these architectures provides the technical vocabulary; understanding workflows provides the engineering discipline to deploy them effectively.
...
The complete system architecture reveals these critical interconnections between stages.
