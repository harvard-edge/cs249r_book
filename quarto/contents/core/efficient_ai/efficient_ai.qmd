...
[^fn-efficient-gpt3]: **GPT-3**: OpenAI's 175-billion parameter language model released in 2020, as detailed in our opening example. Its training data included 45TB of text from the internet, books, and other sources.
...
However, scaling laws reveal critical constraints. As detailed in our analysis above, computational demands increase exponentially with model size, raising fundamental questions about sustainability and efficiency. Understanding these relationships provides the necessary context for our subsequent examination of algorithmic, compute, and data efficiency.
...
Following our detailed scaling law analysis, which revealed critical breakdown points and sustainability challenges, we now examine how to move beyond brute-force scaling. The fundamental constraint identified—that continued progress requires systematic efficiency optimization—motivates a strategic shift from asking "how much more can we scale" to "how can we achieve more with less?"

This shift leads us to the three complementary efficiency dimensions introduced in @sec-efficient-ai-defining-system-efficiency-8e59. Each dimension addresses a specific limitation exposed by scaling laws: algorithmic efficiency tackles computational intensity, compute efficiency addresses hardware utilization gaps, and data efficiency solves the data saturation problem.
...
Building on our three efficiency dimensions framework (@sec-efficient-ai-defining-system-efficiency-8e59), these pillars provide a systematic approach for navigating interdependent trade-offs. Modern techniques achieve dramatic improvements: 10-100x gains in algorithmic efficiency through optimized architectures, 5-50x improvements in hardware utilization through specialized processors, and 10-1000x reductions in data requirements through advanced learning methods.
...
As established in @sec-efficient-ai-defining-system-efficiency-8e59, machine learning system efficiency spans three interconnected dimensions. Understanding how optimizations in one dimension affect others is necessary for designing systems that are not only performant but also scalable, adaptable, and sustainable [@patterson2021carbon].
...
The specific priorities and trade-offs vary across deployment environments. As detailed in our opening examples, these range from cloud systems with abundant resources to edge devices with severe memory and power constraints. @tbl-deployment-efficiency-priorities maps how these constraints translate into efficiency optimization priorities across deployment contexts.
...
These trade-offs become even more critical in resource-constrained environments, exemplifying the deployment constraints introduced in our overview. TinyML systems like IoT environmental monitors face the extreme end of these efficiency challenges, requiring holistic optimization across all three dimensions simultaneously.
...
In contrast, edge and mobile systems operate under the strict constraints detailed in our efficiency framework. These scenarios, from smartphone applications to remote IoT sensors, demand solutions that prioritize efficiency over raw performance. @sec-agi-systems explores how these efficiency principles scale to future AI systems, where resource optimization becomes even more critical as system complexity increases.
