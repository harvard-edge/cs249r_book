...
[^fn-carbon-emissions]: **Carbon Emissions**: Training GPT-3 generated ~552 tons of CO₂ equivalent, similar to 123 gasoline-powered cars driven for one year. The ML community increasingly tracks carbon footprint, with tools like CodeCarbon measuring energy usage during model training.

These efficiency challenges are not merely engineering problems but essential constraints that shape what AI applications are possible. Understanding and managing these trade-offs determines whether machine learning systems can achieve their potential impact across diverse deployment contexts. This chapter builds upon foundational AI workflow concepts and deep learning principles to explore how efficiency considerations permeate every stage of system development, from initial architecture decisions to production monitoring.
...
The key insight driving this progress is the scaling hypothesis: larger models possess increased capacity to capture intricate data patterns, facilitating improved accuracy and generalization. However, this scaling comes with critical constraints that make efficiency paramount. Training GPT-3 required approximately 314 sextillion floating-point operations (314 followed by 21 zeros), equivalent to running a modern gaming PC continuously for over 350 years. The financial cost reached $4.6 million, and the energy consumption generated 552 tons of CO₂ equivalent [@strubell2019energy].
...
Finally, scaling laws provide insight into efficiency limits. By quantifying the trade-offs between scale and performance, they highlight when brute-force scaling becomes inefficient and signal the need for alternative approaches. This includes methods such as knowledge distillation, transfer learning, sparsity, and hardware-aware model design—all of which aim to extract more value from existing resources without requiring further increases in raw scale. @sec-generative-ai demonstrates how these principles apply to modern foundation models, where efficiency becomes critical for practical deployment.
...
Three major approaches dominate modern algorithmic efficiency: pruning, quantization, and knowledge distillation. Each targets different aspects of model inefficiency and works synergistically with the other efficiency dimensions.

**Pruning** removes redundant connections and neurons, exploiting the inherent sparsity in neural networks. The theoretical foundation lies in the lottery ticket hypothesis, which reveals that networks contain sparse subnetworks—typically just 10-20% of original parameters—that achieve comparable accuracy when trained in isolation [@frankle2019lottery]. Modern structured pruning techniques remove entire channels or blocks, achieving hardware-friendly speedups. Magnitude-based pruning eliminates weights below threshold values, while gradual pruning during training maintains accuracy. Pruning to 20% sparsity typically reduces inference time by 3-5x while maintaining 95%+ of original accuracy.

[^fn-efficient-model-parallelism]: **Model Parallelism**: Distributing model components across multiple processors, contrasting with data parallelism. Modern transformer models like GPT-3 require model parallelism due to their 175B parameters exceeding single GPU memory (~24GB for A100).
...
**Quantization** reduces numerical precision from 32-bit floating-point to 8-bit or even 4-bit integers. This technique leverages neural networks' inherent robustness to precision loss, exploiting redundancy in floating-point representations where many bits contribute minimally to final predictions. Networks naturally develop compressed internal representations that capture essential features while discarding noise, making them inherently compressible without performance degradation.

#### Hardware-Algorithm Co-optimization {#sec-efficient-ai-deep-learning-era-4647}

The practical benefits of algorithmic efficiency vary dramatically across hardware platforms, demonstrating the critical interconnection with compute efficiency. INT8 quantization achieves 2.3x speedup on NVIDIA V100 GPUs with typically 1.2% accuracy loss for vision models, while specialized neural processing units deliver 4.1x speedup on ARM Cortex-A78 processors with 2.1% accuracy degradation [@gholami2021survey]. Emerging INT4 quantization on dedicated AI accelerators provides 8x speedup but requires careful calibration to limit accuracy loss to 3-5%.

Energy efficiency improvements are even more dramatic: 8-bit quantization consumes 16x less energy than 32-bit floating-point operations, while pruning to 20% sparsity linearly reduces energy consumption proportional to the sparsity ratio. These hardware-specific efficiency gains demonstrate why algorithmic and compute efficiency must be optimized together rather than in isolation.

**Knowledge Distillation** transfers capabilities from large "teacher" models to efficient "student" models through specialized training procedures. The student learns to match the teacher's output distributions rather than just final predictions, capturing richer information about decision boundaries and uncertainty. This approach enables 10-100x model compression while preserving much of the original model's capability, directly addressing data efficiency by requiring fewer training examples for the student model.

#### Architectural Innovation for Efficiency {#sec-efficient-ai-architectural-innovation}

Beyond compression techniques, modern efficiency requires architectures designed from the ground up for resource constraints. Models like MobileNet[^fn-mobilenet], EfficientNet[^fn-efficientnet], and SqueezeNet[^fn-squeezenet] demonstrate that compact designs can deliver high performance through architectural innovations rather than just scaling up existing designs. These architectures use techniques like depthwise separable convolutions, inverted residuals, and neural architecture search to optimize the efficiency-accuracy trade-off.
...
[^fn-tinyml]: **TinyML**: Machine learning on microcontrollers[^fn-microcontrollers] and edge devices with <1KB-1MB memory and <1mW power consumption. Enables AI in IoT devices, wearables, and sensors where traditional ML deployment is impossible due to resource constraints.

[^fn-microcontrollers]: **Microcontrollers**: Integrated circuits containing a processor, memory, and input/output peripherals on a single chip. Arduino Uno has 32KB memory vs. modern smartphones with 8GB+ RAM—a 250,000× difference, requiring specialized ML algorithms for these ultra-constrained environments.
