...
Building on the data engineering foundations established in @sec-data-engineering, we examined how data is collected, preprocessed, and transformed into features suitable for model training and inference. Within the context of MLOps, these tasks are formalized and scaled into systematic, repeatable processes that ensure data reliability, traceability, and operational efficiency. Data management, in this setting, extends beyond initial preparation to encompass the continuous handling of data artifacts throughout the lifecycle of a machine learning system.
...
Understanding why boundary erosion occurs so frequently in ML systems requires examining the fundamental differences between traditional software engineering principles and the realities of machine learning workflows. Boundary erosion in ML systems violates established software engineering principles, particularly the Law of Demeter and the principle of least knowledge. Traditional software achieves modularity through explicit interfaces and information hiding, as formalized by Parnas in his seminal work on modular programming. However, ML systems undermine these principles because data flows create implicit couplings that bypass explicit interfaces.

The CACHE phenomenon represents a breakdown of the Liskov Substitution Principle, where component modifications violate behavioral contracts expected by dependent components. In traditional software, type systems and interface contracts prevent such violations at compile time, but ML systems operate with statistical rather than logical guarantees. This fundamental difference between deterministic and probabilistic behavior suggests that classical software engineering principles may need to be extended or reconsidered when applied to ML systems.
