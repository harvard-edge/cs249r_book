...
- Analyze how AI systems address critical societal challenges including healthcare access, food security, disaster response, and environmental conservation

- Master design patterns for ML systems in resource-constrained environments: Hierarchical Processing, Progressive Enhancement, Distributed Knowledge, and Adaptive Resource Management

- Select appropriate design patterns based on infrastructure constraints, resource availability, and deployment requirements

- Integrate Cloud ML, Edge ML, Mobile ML, and TinyML deployment paradigms into social impact applications

- Evaluate trade-offs between model accuracy, resource consumption, and system reliability in constrained environments

- Apply engineering principles from the UN Sustainable Development Goals framework to guide AI system design decisions

- Synthesize knowledge from previous chapters to build trustworthy, resilient ML systems under extreme constraints
...
Although these issues vary in scope and scale, they share several important characteristics. They disproportionately affect vulnerable populations, exacerbating existing inequalities. Resource constraints in affected regions pose significant barriers to implementing solutions. Addressing these challenges requires navigating trade-offs between competing priorities and limited resources, often under conditions of great uncertainty.
...
A common pitfall in this domain is the technology-first approach, where engineers build solutions without deeply understanding community needs. This often leads to technically impressive systems that go unused because they fail to address real priorities or operate effectively under local constraints. Instead of starting with available AI techniques and searching for applications, successful deployments emerge from thorough needs assessment and co-design processes that prioritize community-identified problems over technological capabilities.
...
The healthcare sector presents similar opportunities for transformation through machine learning. For millions in underserved communities, access to healthcare often means long waits and travel to distant clinics. TinyML is changing that by enabling diagnostics to occur at the patient's side. For example, a low-cost wearable developed by [Respira x Colabs](https://www.samayhealth.com/) uses embedded machine learning to analyze cough patterns and detect pneumonia[^fn-cough-detection]. Designed for remote areas, the device operates independently of internet connectivity and is powered by a simple microcontroller, making life-saving diagnostics accessible to those who need it most.
...
Complementing these point-of-care innovations, Cloud ML is advancing healthcare research and diagnostics on a broader scale. Platforms like [Google Genomics](https://health.google/health-research/genomics/)[^fn-genomics-cloud] analyze vast datasets to identify disease markers, accelerating breakthroughs in personalized medicine. These examples show how AI technologies, ranging from the portability of TinyML to the computational power of Cloud ML, are converging to democratize healthcare access and improve outcomes worldwide.
...
### Quantitative Optimization Techniques {#sec-ai-good-quantitative-optimization-techniques-fe05}
...
### System Resilience and Failure Recovery {#sec-ai-good-system-resilience-failure-recovery-5e05}
...
## Design Pattern Selection Framework {#sec-ai-good-design-pattern-selection-framework-1ce7}
...
Rather than address these constraints ad-hoc, systematic design patterns provide principled architectural approaches. It is a fallacy to assume that resource-constrained deployments simply require "scaled-down" versions of cloud systems. As the design patterns show, they require fundamentally different architectures optimized for specific constraint combinations rather than reduced functionality.

Four patterns emerge from analysis of successful social good deployments, each targeting specific constraint combinations:

### Pattern Selection Dimensions {#sec-ai-good-pattern-selection-dimensions-5e6c}
...
### Pattern Overview {#sec-ai-good-pattern-overview-c5ad}
...
### Pattern Comparison Framework {#sec-ai-good-pattern-comparison-framework-5d2b}
...
\end{tikzpicture}
```
This pattern employs a distributed knowledge architecture where autonomous nodes collaboratively process data and share insights via communication networks, enabling scalable and adaptable AI systems through decentralized knowledge aggregation and analysis. The architecture decouples data processing from centralized control, fostering resilience and allowing systems to respond effectively to dynamic environments and distributed data sources.
:::
...
Looking at the systems we discussed earlier, it is clear that these systems could benefit from adaptive resource allocation in their operations. In the case of Google's flood forecasting system, the Hierarchical Processing Pattern approach ensures that data is processed at the appropriate level, from edge sensors to cloud-based analysis. However, dynamic resource management would allow this system to adjust its operations depending on available infrastructure. In areas with limited connectivity, the system could rely more heavily on edge processing to reduce bandwidth requirements, while in regions with better infrastructure, it could scale up to leverage cloud-based processing power.

Similarly, PlantVillage Nuru could integrate adaptive resource allocation into its progressive enhancement approach. The app is designed to work in diverse settings, from low-resource rural areas to more developed regions. Dynamic resource management would help the system adjust processing complexity based on available device and network resources, ensuring useful insights without overwhelming the system.

In the case of Wildlife Insights, adaptive resource management would complement the Distributed Knowledge Pattern. The camera traps in the field process data locally, but when network conditions improve, the system could scale up to transmit more data to central systems for deeper analysis. By using adaptive techniques, the system ensures that camera traps can continue functioning with limited power and connectivity while providing valuable insights when resources allow for greater computational effort.

These systems could integrate adaptive resource management to dynamically adjust based on available resources, improving efficiency and ensuring continuous operation under varying conditions. By incorporating dynamic resource allocation into their design, these systems can remain responsive and scalable, even as resource availability fluctuates. This pattern acts as an enabler, supporting system operations and helping them adapt to the demands of real-time environments.

#### Structure {#sec-ai-good-structure-7568}

This pattern revolves around dynamically allocating resources in response to changing environmental conditions, such as network bandwidth, computational power, or storage. It requires systems to monitor available resources continuously and adjust operations accordingly to ensure optimal performance and efficiency.
...
## Resource-Constrained Learning Theory {#sec-ai-good-resourceconstrained-learning-theory-b4f7}
...
### Sample Complexity in Low-Resource Environments {#sec-ai-good-sample-complexity-lowresource-environments-3f27}
...
#### Few-Shot Learning Requirements {#sec-ai-good-fewshot-learning-requirements-2179}
...
#### Information-Theoretic Bounds {#sec-ai-good-informationtheoretic-bounds-0108}
...
### Self-Supervised Learning Foundations {#sec-ai-good-selfsupervised-learning-foundations-3193}
...
#### Contrastive Learning Theory {#sec-ai-good-contrastive-learning-theory-a8d8}
...
#### Mutual Information Bounds {#sec-ai-good-mutual-information-bounds-7b7c}
...
### Resource-Constrained Optimization Theory {#sec-ai-good-resourceconstrained-optimization-theory-774b}
...
#### Communication-Constrained Learning {#sec-ai-good-communicationconstrained-learning-7474}
...
#### Energy-Aware Learning Theory {#sec-ai-good-energyaware-learning-theory-0ecb}
...
## Fallacies and Pitfalls {#sec-ai-good-fallacies-pitfalls-2678}
...
### Technical Superiority Fallacy {#sec-ai-good-technical-superiority-fallacy-ea7e}
...
### Infrastructure Assumption Pitfall {#sec-ai-good-infrastructure-assumption-pitfall-bd90}
...
### Community Engagement Oversimplification {#sec-ai-good-community-engagement-oversimplification-4c63}

Technical teams frequently underestimate the complexity of community engagement, treating it as a implementation detail rather than a fundamental design constraint that shapes system architecture and deployment strategy. This oversimplification leads to systems that may function technically but fail to integrate meaningfully into community practices and decision-making processes.
...
### Data Colonialism and Extractive Practices {#sec-ai-good-data-colonialism-extractive-practices-0ab5}
...
### Sustainability Myopia {#sec-ai-good-sustainability-myopia-af98}
