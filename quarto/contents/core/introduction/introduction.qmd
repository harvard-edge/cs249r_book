...
Artificial intelligence's transformative impact across society raises a central question: How can we create these intelligent capabilities? The relationship between AI and ML provides the theoretical and practical framework to address this question.

Artificial Intelligence represents the systematic pursuit of understanding and replicating intelligent behavior, specifically the capacity to learn, reason, and adapt to new situations. As the theoretical framework, AI encompasses core questions about the nature of intelligence itself: How do we recognize patterns? How do we learn from experience? How do we adapt our behavior based on new information? AI explores these questions by drawing insights from cognitive science, psychology, neuroscience, and computer science, establishing the conceptual foundations for what it means to be intelligent.
...
AlexNet, shown in @fig-alexnet, achieved a breakthrough in the 2012 ImageNet[^fn-intro-imagenet] competition that transformed machine learning. AlexNet required two NVIDIA GTX 580 GPUs with 3GB memory each, achieving 2.3 GFLOPS peak performance (per GPU). The 60 million parameters demanded 240MB storage, while training on 1.2 million images required careful memory management across GPU boundaries. Training took approximately 6 days, achieving 15.3% top-5 error rate compared to 26.2% for second place, a 42% relative improvement that represented the step-function change driving technology adoption. This 10-100x speedup over CPU implementations made training feasible in days rather than months [@krizhevsky2012imagenet].
...
This historical progression from symbolic AI through statistical learning to deep learning reveals a consistent underlying pattern that helps explain why systems engineering has become so crucial to AI success. Each major breakthrough we've examined, from expert systems to neural networks to deep learning, succeeded not by better encoding human knowledge, but by finding ways to leverage greater computational resources more effectively.
...
The bitter lesson reflects hardware exponentials: compute increased 10^12x since 1960 while human expert systems stayed constant. Moore's Law provided 10^6x transistor growth, enabling the data-centric approaches that ultimately succeeded. This extends to systems engineering: simple, scalable algorithms beat complex, optimized ones (MapReduce vs hand-tuned parallel algorithms); general-purpose hardware with scale beats specialized optimization (GPUs for AI vs custom ASICs for specific algorithms).

## ML Systems Engineering {#sec-introduction-ml-systems-engineering-e9d8}

This key insight, that AI progress depends on computational scaling rather than knowledge engineering, explains why an entirely new engineering discipline has emerged over the past decade. The bitter lesson reveals that our success in AI is primarily limited not by our understanding of intelligence, but by our ability to build systems that can effectively harness computational resources at unprecedented scale.

Consider what this means in practice. Creating a modern language model requires coordinating thousands of GPUs across multiple data centers, processing petabytes of text data, and serving the resulting system to millions of users with millisecond latency requirements. This challenge extends far beyond algorithmic innovation—it demands expertise in distributed systems, data engineering, hardware optimization, and operational practices that simply didn't exist when AI research focused on encoding human knowledge into expert systems.

This realization has shifted how we approach AI development, from focusing primarily on algorithmic innovations to recognizing that the engineering of scalable systems often determines what's actually achievable in practice.
...
Understanding these interdependencies helps explain why creating effective ML systems requires simultaneous consideration of data, algorithms, and infrastructure rather than treating them as separate engineering problems.
...
Perhaps the most insidious challenge in ML systems is their tendency to fail silently. Unlike traditional software that crashes visibly when broken, ML systems continue operating while their performance quietly degrades. A recommendation system might gradually decline from 85% accuracy to 60% over months without triggering any alerts, silently eroding user experience and business metrics. This degradation often manifests as subtle shifts in prediction quality that remain undetected until significant damage has occurred.

This silent failure mode helps explain why 90% of ML models never reach production successfully. Many promising models that perform well in development fail not due to algorithmic limitations, but because these gradual degradation patterns go unnoticed until business impact becomes severe. Production ML systems therefore require comprehensive monitoring of model performance, input data distribution, and prediction quality to detect these invisible failures before they compound into critical issues.
