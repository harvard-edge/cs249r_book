...
[^fn-electrical-grids]: **Smart Electrical Grids**: Modern power grids use AI to manage over 7,300 power plants and 160,000 miles of transmission lines in the US alone, optimizing energy distribution in real-time to prevent blackouts and integrate renewable sources like solar and wind that fluctuate throughout the day.

[^fn-particle-accelerators]: **Particle Accelerator Data**: The Large Hadron Collider at CERN generates over 50 petabytes of data annually from 600 million particle collisions per second—requiring AI algorithms to filter and identify the rare, scientifically significant events from this massive data stream in real-time.
...
Training these models effectively is itself a significant challenge. Unlike traditional programming where we write explicit instructions, ML models learn from examples[^fn-transfer]. This learning process involves many choices: How should we structure the model? How long should we train it? How can we tell if it's learning the right things? Making these decisions often requires both technical expertise and considerable trial and error. The systematic approaches to these training challenges are explored in detail in @sec-training.
...
Consider a company building a speech recognition system. They need infrastructure to collect and store audio data, systems to train models on this data, and then separate systems to actually process users' speech in real-time. Each part of this pipeline needs to work reliably and efficiently, and all the parts need to work together seamlessly. The engineering principles for building such robust data pipelines are covered comprehensively in @sec-data-engineering.
