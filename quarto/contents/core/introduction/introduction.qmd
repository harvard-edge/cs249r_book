...
This shift mirrors computer science and engineering evolution in the late 1960s and early 1970s. As computing systems grew more complex, Computer Engineering[^fn-computer-engineering] emerged as a new discipline to address the growing complexity of integrating hardware and software systems. This field bridged the gap between Electrical Engineering's hardware expertise and Computer Science's focus on algorithms and software. Computer Engineering arose because the challenges of designing and building complex computing systems required an integrated approach that neither discipline could fully address on its own.
...
This field lacks universal definition but can be broadly characterized as (with comprehensive coverage of engineering practices provided in [Chapter @sec-ml-systems]):
...
Understanding the interplay between algorithms and engineering is fundamental for modern AI practitioners. Researchers advance algorithmic possibilities while engineers address the complex challenge of reliable, efficient real-world implementation. This comprehensive engineering approach to building production ML systems is detailed in [Chapter @sec-ai-workflow], which covers the end-to-end development lifecycle. This raises a fundamental question: what constitutes a machine learning system, and how does it differ from traditional software systems?
...
- **Data**: Processes and infrastructure for collecting, storing, processing, managing, and serving data for both training and inference (comprehensive data engineering practices are covered in [Chapter @sec-data-engineering]).

- **Computing**: Hardware and software infrastructure that enables efficient training, serving, and operation of models at scale (with specialized acceleration techniques detailed in [Chapter @sec-ai-acceleration]).
...
As ML systems grow, they often need to handle increasingly large amounts of data. A video streaming service like Netflix, for example, needs to process billions of viewer interactions to power its recommendation system. This scale introduces new challenges in how to store, process, and manage such large datasets efficiently. Comprehensive approaches to managing data at scale are covered in [Chapter @sec-data-engineering].
...
Consider a company building a speech recognition system. They need infrastructure to collect and store audio data, systems to train models on this data, and then separate systems to actually process users' speech in real-time. Each part of this pipeline needs to work reliably and efficiently, and all the parts need to work together seamlessly. The operational frameworks and best practices for managing such complex ML pipelines are detailed in [Chapter @sec-ml-operations].
...
Another important consideration is transparency. Many modern ML models, particularly deep learning models, work as "black boxes"[^fn-black-box]---while they can make predictions, it's often difficult to understand how they arrived at their decisions. The principles and practices for building transparent, fair, and accountable AI systems are comprehensively addressed in [Chapter @sec-responsible-ai].
...
As concerns about computational costs and environmental impact grow, there's an increasing focus on making ML systems more efficient. Researchers are developing new techniques for training models with less data and computing power. Innovation in specialized hardware, from improved GPUs to custom AI chips, is making ML systems faster and more energy-efficient. Comprehensive coverage of efficiency techniques is provided in [Chapter @sec-efficient-ai] and [Chapter @sec-model-optimizations], while the environmental and societal implications are explored in [Chapter @sec-sustainable-ai].
...
- **Training**: Exploring the methodologies for AI training, focusing on efficiency, optimization, and acceleration techniques to enhance model performance (covered in [Chapter @sec-ai-training], [Chapter @sec-model-optimizations], and [Chapter @sec-ai-acceleration]).

- **Deployment**: Encompassing benchmarks, on-device learning strategies, and machine learning operations to ensure effective model application (detailed in [Chapter @sec-benchmarking-ai], [Chapter @sec-ondevice-learning], and [Chapter @sec-ml-operations]).

- **Operations**: Highlighting the maintenance challenges unique to machine learning systems, which require specialized approaches distinct from traditional engineering systems (explored in [Chapter @sec-ml-operations] and [Chapter @sec-ai-workflow]).

- **Ethics & Governance**: Addressing concerns such as security, privacy, responsible AI practices, and the broader societal implications of AI technologies (covered in [Chapter @sec-responsible-ai], [Chapter @sec-security-privacy], [Chapter @sec-sustainable-ai], and [Chapter @sec-ai-good]).
