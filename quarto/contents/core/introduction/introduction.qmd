...
This field lacks universal definition but can be broadly characterized as (with comprehensive coverage of engineering practices provided in @sec-ml-systems):
...
Understanding the interplay between algorithms and engineering is fundamental for modern AI practitioners. Researchers advance algorithmic possibilities while engineers address the complex challenge of reliable, efficient real-world implementation. This comprehensive engineering approach to building production ML systems is detailed in @sec-ai-workflow, which covers the end-to-end development lifecycle. This raises a fundamental question: what constitutes a machine learning system, and how does it differ from traditional software systems?
...
- **Data**: Processes and infrastructure for collecting, storing, processing, managing, and serving data for both training and inference (comprehensive data engineering practices are covered in @sec-data-engineering).

- **Computing**: Hardware and software infrastructure that enables efficient training, serving, and operation of models at scale (with specialized acceleration techniques detailed in @sec-ai-acceleration).
...
As ML systems grow, they often need to handle increasingly large amounts of data. A video streaming service like Netflix, for example, needs to process billions of viewer interactions to power its recommendation system. This scale introduces new challenges in how to store, process, and manage such large datasets efficiently. Comprehensive approaches to managing data at scale are covered in @sec-data-engineering.
...
Consider a company building a speech recognition system. They need infrastructure to collect and store audio data, systems to train models on this data, and then separate systems to actually process users' speech in real-time. Each part of this pipeline needs to work reliably and efficiently, and all the parts need to work together seamlessly. The operational frameworks and best practices for managing such complex ML pipelines are detailed in @sec-ml-operations.
...
- **Training**: Exploring the methodologies for AI training, focusing on efficiency, optimization, and acceleration techniques to enhance model performance (covered in @sec-ai-training, @sec-model-optimizations, and @sec-ai-acceleration).

- **Deployment**: Encompassing benchmarks, on-device learning strategies, and machine learning operations to ensure effective model application (detailed in @sec-benchmarking-ai, @sec-ondevice-learning, and @sec-ml-operations).

- **Operations**: Highlighting the maintenance challenges unique to machine learning systems, which require specialized approaches distinct from traditional engineering systems (explored in @sec-ml-operations and @sec-ai-workflow).

- **Ethics & Governance**: Addressing concerns such as security, privacy, responsible AI practices, and the broader societal implications of AI technologies (covered in @sec-responsible-ai, @sec-security-privacy, @sec-sustainable-ai, and @sec-ai-good).
