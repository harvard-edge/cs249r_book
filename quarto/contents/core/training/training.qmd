...
[^fn-training-model-parallelism]: **Model Parallelism Memory Scaling**: Enables training models too large for single GPUs. GPT-3 (175B parameters) needs 350GB just for weights in FP16, far exceeding any single GPU's 80GB maximum. However, model parallelism often achieves only 20-60% compute efficiency due to sequential dependencies between model partitions and communication overhead between devices.
