...
We use **training GPT-2 (1.5 billion parameters)** as our consistent reference point to ground abstract concepts in concrete reality. GPT-2 represents an ideal teaching example because it:
...
Beginning with the foundational activation functions that shaped early neural network development, the sigmoid function represents one of the earliest approaches to introducing non-linearity in neural networks. It maps input values to the range $(0, 1)$ through the following mathematical expression:
$$
...
Despite these computational challenges, sigmoid maintains its relevance in specialized contexts where probabilistic interpretation is essential. It remains the standard choice for binary classification output layers, where its bounded output naturally represents class probabilities.

##### Tanh {#sec-ai-training-tanh-a81b}

Addressing the key limitations identified with sigmoid, particularly the lack of zero-centered outputs, the hyperbolic tangent (tanh) function emerged as an improved alternative for neural network activation. It maps input values through a nonlinear transformation into the range $(-1, 1)$. The mathematical definition of the tanh function is:
$$
...
This transition from mathematical foundations to pipeline architecture represents a fundamental perspective shift from "what to compute" to "how to orchestrate computation." While the preceding sections examined the mathematical properties of matrix operations, gradient calculations, and optimization algorithms, we now explore how training systems compose these operations into complete training workflows that manage data flow, resource allocation, and execution timing. The pipeline architecture perspective addresses operational questions the mathematical view cannot: When should data loading occur relative to computation? How do we checkpoint training state for fault tolerance? How do we monitor training progress without impacting throughput? The mathematical operations remain the same, but the systems perspective reveals how to execute them efficiently in real computational environments.

## Pipeline Architecture {#sec-ai-training-pipeline-architecture-622a}

Building directly upon the mathematical foundations established in the previous section, a training pipeline provides the organizational framework that coordinates these mathematical operations with data movement, system resources, and operational monitoring. This pipeline transforms the abstract mathematical operations we have studied—matrix multiplications, gradient computations, and parameter updates—into concrete computational workflows that can reliably train models on real systems with finite memory, bounded computational throughput, and operational constraints.
