...
Machine learning systems design principles culminate in the training phase, where theoretical constructs undergo empirical realization through computational optimization. Building upon system design methodologies from @sec-ml-systems, data pipeline architectures from @sec-data-engineering, and computational frameworks from @sec-ai-frameworks, this chapter investigates how algorithmic theory, data processing, and hardware architecture converge in the iterative refinement of intelligent systems.

Training represents the most computationally demanding phase in machine learning systems, requiring sophisticated coordination of mathematical optimization with distributed systems engineering. Modern training workloads impose computational requirements that exceed conventional computing capabilities: models with billions of parameters demand terabytes of memory, training corpora span petabyte-scale storage, and gradient-based optimization requires synchronized computation across thousands of processing units. These computational scales create core systems engineering challenges in memory hierarchy management, inter-node communication efficiency, and resource allocation that distinguish training infrastructure from general-purpose computational architectures.
...
GPT-2 employs a transformer architecture (detailed in @sec-dnn-architectures) that processes text through self-attention mechanisms. Understanding these key computational patterns provides essential context for the training examples throughout this chapter:
...
GPT-2's architecture defines the computational challenges addressed throughout this chapter. The model contains 1.5 billion parameters (1,558,214,656 exact count) trained on OpenWebText (~40GB text, ~9B tokens). Training typically uses an effective batch size of 512 distributed across 8-32 GPUs, with a memory footprint of approximately 3GB for parameters (in FP16 format) and 18GB for activations (batch_size=32). Complete training requires roughly 2 weeks on 32 V100 GPUs.
...
Each GPT-2 layer performs attention computations that exemplify dense matrix multiplication demands. For a single attention head with batch_size=32, sequence_length=1024, hidden_dim=1280, the query, key, and value projections require three separate matrix multiplications:
...
The attention score computation (Q × K^T) adds additional computational load:
$$
...
This yields approximately 204 billion FLOPS for one attention layer's forward pass. With 48 layers in GPT-2, each training step requires roughly 9.8 trillion FLOPS, and the complete 50K training steps demand approximately 490 petaFLOPS total computation.

The system implications are substantial: a V100 GPU (125 TFLOPS peak FP16) would require 79 seconds just for the attention computations per step at 100% utilization. Since actual training steps complete in 180-220ms, achieving this throughput requires 8-32 GPUs working in parallel.
...
GELU involves system performance trade-offs: computational cost runs approximately 3-4x higher than ReLU due to erf function evaluation, though memory usage remains identical since both are element-wise operations. For GPT-2's 48 layers, GELU adds roughly 5-8% to total forward pass time. However, the improved model quality (lower perplexity) typically offsets this computational overhead.

Modern frameworks (PyTorch, TensorFlow) implement GELU with optimized approximations:
```python
...
###### Stochastic Gradient Descent {#sec-ai-training-stochastic-gradient-descent-3517}
...
#### Use Cases {#sec-ai-training-use-cases-55c1}
...
#### Challenges and Trade-offs {#sec-ai-training-challenges-tradeoffs-94e3}
...
#### Scaling Beyond Single-Machine {#sec-ai-training-scaling-beyond-singlemachine-fad1}
...
### Efficiency Metrics and Scaling Characteristics {#sec-ai-training-efficiency-metrics-scaling-characteristics-53df}
