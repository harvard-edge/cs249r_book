...
```{.tikz}
\begin{tikzpicture}[
    box/.style={draw, rounded corners=3pt, minimum width=2.5cm, minimum height=1cm, align=center},
    pe/.style={box, fill=blue!20},
    mem/.style={box, fill=green!20},
    compute/.style={box, fill=yellow!20, minimum width=1.5cm, minimum height=0.6cm},
    control/.style={box, fill=orange!20},
    interface/.style={box, fill=red!20}
]

% Memory hierarchy at top
\node[mem] (hbm) at (0,6) {High-Bandwidth\\Memory\\(HBM)};
\node[mem] (l2) at (4,5) {L2 Cache\\(Shared)};
\node[mem] (l1) at (8,4) {L1 Cache\\(Local)};

% Processing element cluster
\node[pe] (pe1) at (2,2) {PE 0};
\node[pe] (pe2) at (4,2) {PE 1};
\node[pe] (pe3) at (6,2) {PE 2};
\node[pe] (pe4) at (8,2) {PE 3};

% Compute units inside PEs (showing detail for PE1)
\node[compute] (tensor1) at (1.2,1.2) {Tensor\\Core};
\node[compute] (vector1) at (2.8,1.2) {Vector\\Unit};
\node[compute] (sfu1) at (2,0.4) {SFU};
...
% Host interface
\node[interface] (pcie) at (10,4) {Host Interface\\(PCIe/NVLink)};
\draw[thick, <->] (l1) -- (pcie);

% Control unit
\node[control] (ctrl) at (6,0) {Control Unit\\& Scheduler};
\draw[thick, ->] (ctrl) -- (pe2);
\draw[thick, ->] (ctrl) -- (pe3);
...
\node[anchor=west, font=\footnotesize] at (0,2.9) {through caches to PEs};
...
\node[anchor=west, font=\footnotesize] at (9,0.6) {compute units};
...
## Heterogeneous SoC AI Acceleration {#sec-ai-acceleration-heterogeneous-soc-ai-acceleration-b1bb-ai-acceleration-b1bb}
...
### Dynamic Workload Distribution Strategies {#sec-ai-acceleration-dynamic-workload-distribution-strategies-7e00-strategies-7e00}
...
### Automotive Heterogeneous AI Systems {#sec-ai-acceleration-automotive-heterogeneous-ai-systems-deda-ai-systems-deda}
