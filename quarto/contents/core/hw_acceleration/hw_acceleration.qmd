...
**Understanding Systolic Array Data Flow**: Think of a systolic array like a "computational heartbeat" where data pulses rhythmically through a grid of processing elements. In @fig-systolic-array, input matrix **A** flows horizontally (left to right) while input matrix **B** flows vertically (top to bottom). Each processing element (PE) performs a multiply-accumulate operation: it multiplies the values flowing through it and adds the result to a partial sum that accumulates the final matrix **C**. The key insight is that each data element gets reused multiple times as it flows through neighboring PEs, dramatically reducing memory bandwidth requirements compared to repeatedly fetching the same data from external memory.
