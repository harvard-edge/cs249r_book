...
[^fn-opencl]: **OpenCL (Open Computing Language)**: Cross-platform framework for parallel computing across CPUs, GPUs, FPGAs, and other processors. Developed by Apple and standardized by Khronos Group in 2009, OpenCL enables portable code but often sacrifices performance compared to vendor-specific solutions like CUDA or ROCm. While theoretically hardware-agnostic, achieving optimal performance requires hardware-specific tuning, limiting its practical portability advantages.

[^fn-cuda]: **CUDA (Compute Unified Device Architecture)**: NVIDIA's proprietary parallel computing platform and programming model, introduced in 2006. CUDA provides C/C++ extensions and libraries (cuBLAS, cuDNN, cuFFT) that enable efficient GPU programming. Despite being proprietary, CUDA dominates AI development due to mature ecosystem and optimization tools. NVIDIA's $47 billion data center revenue (2024) largely stems from CUDA's software moat that locks developers into NVIDIA hardware.
...
[^fn-heterogeneous]: **Heterogeneous Computing**: Computing systems that combine different types of processors (CPUs, GPUs, TPUs, FPGAs) to optimize performance for diverse workloads. Modern data centers mix x86 CPUs for control tasks, GPUs for training, and TPUs for inference. Programming heterogeneous systems requires frameworks like OpenCL[^fn-opencl] or CUDA[^fn-cuda] that can coordinate execution across different architectures, but offers 10-100$\times$ efficiency gains by matching each task to optimal hardware.
...
patches = im2col(input)[^fn-im2col]
...
__global__ void matrix_multiply(float* C, float* A, float*
    ...
    float sum = 0.0f;
    for (int k = 0; k < N; k++) {
        // Threads in a warp[^fn-warp] execute in parallel
