...
\node[anchor=west] at (0,2) {\textbf{2D Tensor (2×3):}};
\foreach \row in {0,1} {
  \foreach \col in {0,1,2} {
    \pgfmathsetmacro{\val}{\row * 3 + \col + 1}
    \node[draw, minimum width=15mm, minimum height=10mm,
          fill=col1!50] at (\col*1.6, 1-\row*1.1) {\val};
  }
}

% Row-major memory layout (RIGHT SIDE)
\node[anchor=west] at (6,1.3) {\textbf{Row-Major Layout:}};
\foreach \i in {0,1,2,3,4,5} {
  \pgfmathsetmacro{\val}{\i + 1}
  \node[draw, minimum width=10mm, minimum height=8mm,
        fill=col2!50] at (6+\i*1.1, 0.5) {\val};
  \node[below, font=\tiny] at (6+\i*1.1, 0.05) {[\i]};
}

% Column-major memory layout (RIGHT SIDE)
\node[anchor=west] at (6,-0.5) {\textbf{Column-Major Layout:}};
\foreach \i in {0,1,2,3,4,5} {
  \pgfmathsetmacro{\val}{int(mod(\i,2)*3 + int(\i/2) + 1)}
  \node[draw, minimum width=10mm, minimum height=8mm,
        fill=col3!50] at (6+\i*1.1, -1.3) {\val.0};
  \node[below, font=\tiny] at (6+\i*1.1, -1.75) {[\i]};
}

% Strides explanation (BOTTOM)
\node[anchor=west] at (0,-1.5) {\textbf{Stride Calculation:}};
\node[anchor=west] at (0,-2.0) {Row-major strides: [3, 1]};
\node[anchor=west] at (0,-2.5) {Column-major strides: [1, 2]};
\node[anchor=west] at (0,-3.0) {Element [i,j] offset = i×stride[0] + j×stride[1]};
\end{tikzpicture}
```
