...
- Basic computer architecture concepts (CPU, GPU, memory hierarchy)
- ML training and inference workflows from previous chapters
- Performance metrics like latency and throughput
- The concept of trade-offs in system design
:::
...
The evolution of benchmarks in computing illustrates how systematic performance measurement has shaped technological progress. During the 1960s and 1970s, when mainframe computers dominated the computing landscape, performance benchmarks focused primarily on fundamental computational tasks. The [Whetstone benchmark](https://en.wikipedia.org/wiki/Whetstone_(benchmark)), introduced in 1964 to measure floating-point arithmetic performance, became a definitive standard that demonstrated how systematic testing could drive improvements in computer architecture [@curnow1976synthetic].
...
::: {.callout-important title="Decision Framework: Choosing Benchmarks"}
**When to use which benchmark:**
- **MLPerf Training**: Evaluating hardware for model development
- **MLPerf Inference**: Assessing deployment performance
- **MLPerf Mobile/Tiny**: Testing edge device capabilities
- **Custom benchmarks**: When your use case differs significantly from standard scenarios

**Key Questions to Ask:**
1. What is my deployment target? (cloud, edge, mobile)
2. What metrics matter most? (latency, throughput, energy)
3. How similar is my workload to benchmark tasks?
:::
...
::: {.callout-tip title="Concept Simplification: Hardware Lottery"}
**Simple Explanation**: Imagine two equally talented athletes competing - one trained for swimming, one for running. If the competition is a swimming race, the swimmer wins not because they're a better athlete, but because the contest favors their specific training. Similarly, some ML models succeed not because they're inherently better, but because current hardware (like GPUs) happens to match their computational patterns perfectly.

**Why This Matters**: This creates a bias where we might choose models based on what runs well on today's hardware rather than what's actually the best solution for the problem.
:::
...
::: {.callout-warning title="Common Pitfall: Gaming the System"}
Benchmark engineering is like "teaching to the test" in education - optimizing specifically for what's measured rather than genuine capability. Watch out for:
- Models that excel on benchmarks but fail in production
- Hyperparameter tuning focused solely on benchmark datasets
- Architectural choices driven by benchmark requirements rather than real needs
:::
...
::: {.callout-note title="Chapter Summary: 5-Minute Recap"}
**Key Concepts Covered:**
1. **Evolution of Benchmarks**: From simple FLOPS to complex ML-specific evaluations
2. **Hardware Lottery**: Why some models win due to hardware compatibility, not merit
3. **MLPerf Framework**: Industry standard for fair ML system comparison
4. **Beyond Accuracy**: Why we need fairness, robustness, and efficiency metrics
5. **Benchmark Limitations**: Gaming, overfitting, and the Sim2Real gap

**Essential Takeaways:**
- No single benchmark tells the whole story
- Always consider your specific deployment context
- Be skeptical of benchmark-only performance claims
