...
Modern hardware also introduces variability through dynamic power management techniques designed to optimize energy efficiency. Dynamic voltage and frequency scaling (DVFS) is a widely used power management technique that adjusts processor voltage and clock frequency based on workload demands, allowing systems to reduce power consumption during periods of lower computational intensity [@barroso2022datacenter]. These DVFS features can cause power consumption to vary by 30-50% for the same ML model, depending on system load and concurrent activity.
