...
**From single-dimension to multi-objective evaluation** emerged as deployment contexts diversified beyond raw computational speed. Graphics benchmarks like 3DMark (1998) measured rendering quality alongside frame rates, while mobile computing benchmarks like MobileMark evaluated battery life under realistic usage patterns rather than peak performance alone. This shift recognized that different deployment contexts impose distinct constraints requiring simultaneous optimization across competing objectives. The multi-objective optimization challenges from @sec-efficient-ai—balancing algorithmic complexity, computational resources, and energy efficiency—manifest directly in this benchmarking evolution. Modern ML evaluation must similarly assess accuracy, latency, throughput, memory footprint, and energy consumption simultaneously, as no single metric captures deployment viability. Our three-dimensional framework extends this multi-objective principle to ML's unique evaluation requirements.
...
The multidimensional nature of training performance evaluation demands systematic measurement approaches that ensure meaningful comparisons. Following our established evaluation methodology principles, ML system performance exhibits inherent variability from multiple sources: algorithmic randomness, hardware thermal states, system load variations, and environmental factors.
