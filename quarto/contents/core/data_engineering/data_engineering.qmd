...
Building on the pipeline architecture established in @sec-data-engineering-pipeline-basics-31ba, data processing implementation follows the architectural patterns (ETL or ELT) selected during pipeline design[^fn-etl-vs-elt]. The processing approach directly impacts when and where transformations occur: ETL systems front-load transformations before storage, while ELT systems defer transformations until query time. This architectural choice influences resource allocation, system flexibility, and operational complexity.
