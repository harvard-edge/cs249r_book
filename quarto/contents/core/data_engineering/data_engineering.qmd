...
While previous chapters established the computational environments where ML systems operate (@sec-ml-systems), the mathematical foundations of neural networks (@sec-dl-primer), and specialized architectures for different data types (@sec-dnn-architectures), this chapter examines the infrastructure that supplies these systems with their most essential resource: high-quality, reliable data. The relationship between data quality and system performance is not merely correlative but causal and often amplifying. Small data quality issues cascade through ML pipelines, degrading model accuracy, increasing computational costs, and undermining system reliability. A single mislabeled training example might seem insignificant, but systematic labeling inconsistencies can corrupt model behavior across entire feature spaces. Data drift in production silently degrades performance until complete retraining becomes necessary. These cascading failures distinguish ML systems from traditional software, where input validation provides clear boundaries between acceptable and problematic inputs.

This chapter presents data engineering as a systematic discipline organized around four foundational pillars that work together to ensure ML systems can operate reliably in production: **Quality** ensures data accuracy and fitness for ML tasks; **Reliability** provides consistent, predictable data processing that handles failures gracefully; **Scalability** enables systems to handle growing data volumes without architectural redesigns; and **Governance** establishes the legal, ethical, and operational frameworks within which data systems operate. Understanding how these pillars interact and sometimes create tensions is essential for building production ML systems.

The organization of this chapter reflects how these pillars manifest across the complete data lifecycle, from initial acquisition through production deployment. Rather than treating each technical topic in isolation, we examine how engineering decisions in one area affect others, revealing the systems thinking required for effective data engineering.
