...
Efficiency principles guide us toward systematic optimization of machine learning models, transforming theoretical architectures into deployable systems. Building on the strategic foundations from @sec-efficient-ai, this chapter shifts from conceptual understanding to practical implementation. Where efficiency principles establish the "what" and "why" of optimization, this chapter provides the "how" through concrete algorithms and engineering approaches.

Modern machine learning models face a fundamental engineering challenge: the computational requirements for state-of-the-art performance often exceed real-world deployment constraints. GPT-3 training consumed approximately 1,287 MWh and cost an estimated $4.6 million in compute resources [@patterson2021carbon], while requiring 350 GB of memory just to store model weights in FP32 precision. These requirements create a substantial performance gap between research environments (125-275 TFLOPS on specialized hardware) and production systems, where mobile SoCs deliver 1-5 TOPS and embedded devices provide 0.1-1 TOPS. Memory bandwidth varies even more dramatically, from 3,352 GB/s on H100 GPUs to 25-50 GB/s on mobile devices, fundamentally changing optimization priorities.

This gap necessitates systematic optimization engineering that balances multiple competing objectives: accuracy preservation, computational efficiency, memory constraints, latency requirements, and energy consumption. Model optimization addresses these challenges through principled techniques that transform models while maintaining their essential capabilities.

::: {.callout-definition title="Definition of Model Optimization"}

**Model optimization** transforms machine learning models to run efficiently in real-world systems while preserving their _accuracy_ and _effectiveness_. This process involves balancing _trade-offs_ between accuracy, computational cost, memory usage, latency, and energy efficiency to ensure models can operate within _real-world constraints_. Model optimization is driven by core principles such as _reducing redundancy_ (removing unnecessary parameters), _improving numerical representation_ (using lower precision), and _structuring computations more efficiently_ (optimizing execution patterns). These principles guide the adaptation of models across _different deployment environments_, from _cloud-scale infrastructure_ to _resource-constrained edge devices_, enabling _scalable, practical, and high-performance_ machine learning systems.

:::

The optimization process operates through three interconnected layers that bridge software algorithms and hardware execution. At the algorithmic level, techniques like pruning[^fn-opt-pruning] eliminate redundant parameters, knowledge distillation[^fn-knowledge-distill] transfers capabilities to smaller models, and neural architecture search discovers efficient designs. These software optimizations create opportunities for hardware acceleration by restructuring computations to match processor capabilities.
