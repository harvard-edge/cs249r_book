...
Machine learning research prioritizes accuracy above all other considerations, producing models that achieve remarkable performance while consuming enormous computational and memory resources that make real-world deployment economically and technically infeasible. The most capable models often cannot be deployed where they are needed most—whether in resource-constrained mobile devices, cost-sensitive cloud environments, or latency-critical edge applications. Model optimization represents the essential bridge between theoretical capability and practical deployment, transforming computationally intensive research models into efficient systems that preserve performance while meeting stringent constraints on memory, energy, latency, and cost. Without systematic optimization techniques, advanced AI capabilities remain trapped in research laboratories. Understanding optimization principles enables engineers to democratize AI capabilities by making sophisticated models accessible across diverse deployment contexts, from billion-parameter language models running on mobile devices to computer vision systems operating in embedded sensors.
...
Building on the efficiency principles established in @sec-efficient-ai, this chapter shifts from conceptual understanding to practical implementation. Where the Efficient AI chapter introduced **what** optimization techniques exist (pruning, quantization, distillation) and **why** they matter for system efficiency, this chapter focuses on **how** to implement these techniques algorithmically, **when** to apply them in production systems, and **how to combine** them effectively. Think of the previous chapter as establishing the efficiency landscape; this chapter provides the detailed maps and navigation tools for traversing that landscape.

The distinction is important: @sec-efficient-ai answered "What is quantization?" with "reducing numerical precision to improve efficiency." This chapter answers "How do I quantize a BERT model from FP32 to INT8 while losing less than 1% accuracy?" with calibration algorithms, quantization-aware training procedures, and hardware-specific implementation strategies. Similarly, while Efficient AI introduced pruning as a concept, this chapter provides magnitude-based algorithms, structured pruning techniques, and the Lottery Ticket Hypothesis with concrete implementation guidance.

We organize model optimization techniques along three core dimensions that directly address the system constraints introduced in @sec-efficient-ai:
