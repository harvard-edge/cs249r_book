...
[^fn-pruning]: **Pruning**: The optimal brain damage algorithm by Yann LeCun (1990) pioneered removing unnecessary neural network connections, inspiring modern magnitude-based and structured pruning techniques that can reduce model sizes by 90% with minimal accuracy loss.
...
[^fn-activation-checkpointing]: **Activation Checkpointing**: Enables training 4x larger models on same hardware by trading computation for memory. BERT-Large training memory drops from 16GB to 4GB with 20% computational overhead, while GPT-3 175B uses checkpointing to fit on 8x A100 GPUs instead of 32x.
