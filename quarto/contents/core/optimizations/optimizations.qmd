...
Neural network foundations established in @sec-dnn-architectures enable understanding how machine learning models evolve in complexity while becoming increasingly ubiquitous, requiring focus to shift from solely enhancing accuracy to ensuring that models are practical, scalable, and efficient. Building on the deep learning primer's mathematical foundations @sec-dl-primer, these optimization techniques transform theoretical network architectures into deployable systems. The computational requirements for training and deploying state-of-the-art models often exceed the limitations of real-world environments. Modern large language models require enormous computational resources: GPT-3 training consumed approximately 1,287 MWh and cost an estimated $4.6 million in compute resources [@patterson2021carbon]. State-of-the-art models can require 175 billion parameters (GPT-3) to 540 billion parameters (PaLM), demanding 350-1,080 GB of memory just to store model weights in FP32 precision. These requirements far exceed typical deployment environments, whether in data centers with abundant compute and memory or on resource-constrained mobile devices with limited power, memory, and processing capacity.
...
Given these dramatic resource disparities, model optimization becomes essential for reconciling advanced modeling techniques with practical system limitations, ensuring that enhanced performance is achieved without compromising operational viability. This optimization framework directly supports the data engineering requirements @sec-data-engineering by enabling efficient processing of large datasets through reduced computational overhead.
...
Building on this definition, model optimization arises from the limitations of modern computational systems. Machine learning models function within an ecosystem of hardware capabilities (@sec-ai-acceleration), software frameworks (@sec-ai-frameworks), and different deployment scenarios. A model that excels in controlled research environments may prove unsuitable for practical applications due to prohibitive computational costs or large memory requirements. Optimization techniques are important for aligning high-performing models with the practical constraints of real-world systems.
...
### Quantization vs. Model Representation {#sec-model-optimizations-quantization-vs-model-representation-af44}
