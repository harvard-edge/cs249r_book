...
Neural networks designed for optimal accuracy often prove impractical for real-world deployment due to excessive computational and memory requirements. A significant gap exists between theoretical model designs and practical deployment constraints, particularly on resource-limited devices. Model optimization techniques bridge this gap by systematically reducing computational requirements, memory footprint, and inference latency while preserving accuracy. Quantization reduces numerical precision, pruning removes unnecessary parameters, distillation transfers knowledge to smaller models, and compression techniques minimize storage requirements. These optimization methods enable sophisticated models to operate within practical constraints, reducing operational costs and improving system responsiveness. Understanding optimization techniques enables engineers to bridge the deployment gap across diverse environments.
...
Neural network foundations established in [Chapter @sec-dnn-architectures] enable understanding how machine learning models evolve in complexity while becoming increasingly ubiquitous, requiring focus to shift from solely enhancing accuracy to ensuring that models are practical, scalable, and efficient. The substantial computational requirements for training and deploying state-of-the-art models frequently surpass the limitations imposed by real-world environments, whether in expansive data centers (with abundant compute and memory) or on resource-constrained mobile devices (with limited power, memory, and processing capacity). Considerations such as memory constraints, energy consumption, and inference latency critically influence the effective deployment of these models. Model optimization, therefore, serves as the framework that reconciles advanced modeling techniques with practical system limitations, ensuring that enhanced performance is achieved without compromising operational viability.
...
[^fn-opt-pruning]: **Pruning**: The optimal brain damage algorithm by Yann LeCun (1990) pioneered removing unnecessary neural network connections, inspiring modern magnitude-based and structured pruning techniques that can reduce model sizes by 90% with minimal accuracy loss.

[^fn-knowledge-distill]: **Knowledge Distillation**: Geoffrey Hinton et al. (2015) introduced this technique where a smaller "student" network learns from a larger "teacher" network's soft outputs, enabling compact models that retain much of the original's performance while being orders of magnitude more efficient.
...
Machine learning models must balance accuracy, efficiency, and feasibility to operate effectively in real-world systems, applying systems foundations from [Chapter @sec-ml-systems]. As discussed in the previous section, optimization is necessary to address key system constraints such as computational cost, memory limitations, energy efficiency, and latency requirements. However, model optimization is not a single technique but a structured process that can be categorized into three fundamental dimensions: model representation optimization, numerical precision optimization, and architectural efficiency optimization.
...
Understanding these three dimensions provides a structured framework for systematically improving model efficiency that incorporates workflow principles from [Chapter @sec-ai-workflow]. Rather than applying ad hoc techniques, machine learning practitioners must carefully select optimizations based on their impact across these dimensions, considering trade-offs between accuracy, efficiency, and deployment constraints.
...
2. **Increased Computational Cost**: More parameters lead to higher inference latency and energy consumption, which is particularly problematic for real-time applications such as autonomous systems, speech recognition, and mobile AI where optimization ([Chapter @sec-ondevice-learning]) becomes crucial. Running unoptimized models on hardware accelerators like GPUs and TPUs ([Chapter @sec-ai-acceleration]) requires additional compute cycles, increasing operational costs.

3. **Scalability Limitations**: Training and deploying large models at scale is resource-intensive in terms of compute, memory, and power. Large-scale distributed training demands high-bandwidth communication and storage, while inference in production environments becomes costly without optimizations, requiring operational strategies ([Chapter @sec-ml-operations]).
...
**Implementation Note**: Modern frameworks provide built-in APIs that make these optimization techniques readily accessible. PyTorch offers `torch.nn.utils.prune` for pruning operations, while TensorFlow provides the Model Optimization Toolkit with functions like `tfmot.sparsity.keras.prune_low_magnitude()`. These tools transform complex research algorithms into practical function calls, making optimization achievable for practitioners at all levels.
...
```
**Energy Costs**: Lower precision reduces computational energy[^fn-energy-efficiency-metrics], illustrating trade-offs in model accuracy. Machine learning systems can optimize efficiency by reducing floating-point operations from 32-bit to 16-bit or even lower for significant savings. Source: IEEE spectrum.
...
Recall from [Chapter @sec-efficient-ai] how we discussed model compression techniques as a key efficiency principle. Quantization is the specific algorithmic technique that enables the "4x memory bandwidth reduction" we mentioned when discussing the memory wall. While the Efficient AI chapter established why quantization matters for system efficiency, this section shows you exactly how to implement it.
...
Beyond architecture design, AutoML also focuses on hyperparameter optimization, which plays a crucial role in determining a model's performance. Parameters such as learning rate, batch size[^fn-batch-size-effects], weight decay, and activation functions must be carefully tuned for stability and efficiency.
