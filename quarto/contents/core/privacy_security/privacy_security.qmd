...
Machine learning systems require unprecedented access to personal data, institutional knowledge, and behavioral patterns to function effectively, creating a tension between utility and protection that determines their acceptance in society. Unlike traditional software that processes data transiently, ML systems learn from sensitive information and embed these patterns into persistent models that can inadvertently reveal private details. This capability creates systemic risks that extend beyond individual privacy violations to threaten institutional trust, competitive advantages, and democratic governance. The success of machine learning deployment across critical domains—healthcare, finance, education, and public services—depends entirely on establishing robust security and privacy foundations that enable beneficial use while preventing harmful exposure. Without these protections, even the most capable ML systems remain unused due to legal, ethical, and practical concerns. Understanding privacy and security principles enables engineers to design systems that achieve both technical excellence and societal acceptance, unlocking machine learning's transformative potential while maintaining the trust essential for widespread adoption.
...
**ML Lesson**: Training pipelines and model repositories face persistent supply chain risks. Compromised dependencies, malicious training data, or backdoored model weights can propagate through MLOps workflows, mirroring how Stuxnet (2010) infiltrated air-gapped[^fn-air-gapped] industrial systems via infected USB drives to sabotage Iran's Natanz centrifuges [@farwell2011stuxnet]. The worm exploited four zero-day vulnerabilities to bridge digital and physical worlds, demonstrating how trusted infrastructure becomes an attack vector. Modern ML systems using open-source libraries (PyTorch, TensorFlow), third-party datasets (HuggingFace, Kaggle), and multi-vendor hardware (NVIDIA, AMD, specialized accelerators) inherit similar vulnerability patterns. Defense requires provenance tracking for datasets and models, cryptographic verification of dependencies, and secure build pipelines that validate artifact integrity before deployment. @fig-stuxnet illustrates the Stuxnet operation, showing how malware targeted programmable logic controllers through compromised software supply chains.

[^fn-air-gapped]: **Air-Gapped Systems**: Networks physically isolated from external connections, originally developed for military systems in the 1960s. Despite seeming impenetrable, studies show 90% of air-gapped systems can be breached through supply chain compromise, infected removable media, or covert channels (acoustic, electromagnetic, thermal).
...
**ML Lesson**: Connected ML systems require strict isolation between external interfaces and safety-critical components. The 2015 Jeep Cherokee hack [@miller2015remote; @miller2019lessons] demonstrated how researchers remotely compromised a vehicle's Uconnect entertainment system via cellular network, then escalated access to control engine, transmission, and braking systems without physical access. This architectural flaw—allowing external interfaces to reach safety-critical functions—directly threatens modern ML deployments. Autonomous vehicles integrate perception models (object detection, lane keeping) with actuator control (steering, braking), while smart home systems connect voice assistants (wake-word detection, NLP models) to physical devices (door locks, thermostats). Without proper isolation, attackers can exploit ML inference APIs to manipulate downstream actuators. Defense requires compartmentalized architectures with authentication boundaries, privilege separation between inference and control planes, and fail-safe mechanisms that revert to safe states when anomalies are detected. The incident triggered 1.4 million vehicle recalls[^fn-automotive-recalls] and established cybersecurity-by-design mandates for connected systems.

[^fn-automotive-recalls]: **Automotive Cybersecurity Recalls**: The Jeep hack triggered the first cybersecurity recall in 2015. Since then, over 15 million vehicles have been recalled for cybersecurity issues, costing $2.4 billion and spurring regulations mandating isolation between externally accessible systems and safety-critical functions.
...
**ML Lesson**: Edge-deployed ML devices with weak authentication become attack infrastructure at scale. The 2016 Mirai botnet [@antonakakis2017understanding] compromised 600,000+ IoT devices (cameras, DVRs, routers) using default credentials (admin/admin, root/12345), orchestrating 623 Gbps DDoS attacks that disabled Twitter, Netflix, and Reddit for hours[^fn-mirai-scale]. Modern ML edge devices—smart cameras running object detection, voice assistants performing wake-word detection, IoT sensors with anomaly detection models—inherit identical vulnerabilities when shipped with weak credentials, lack secure boot, or omit encrypted firmware updates. Compromised ML endpoints enable three attack vectors: (1) exfiltrating inference data (camera feeds, voice recordings) containing sensitive information, (2) participating in coordinated DDoS attacks against ML infrastructure, and (3) serving as pivot points to compromise connected networks. Defense requires secure provisioning (unique per-device credentials, certificate-based authentication), over-the-air update mechanisms with cryptographic verification, network segmentation isolating edge devices from critical infrastructure, and anomaly detection monitoring for compromised device behavior. The incident cost businesses $2.3 million per attack on average, demonstrating that security cannot be deprioritized in resource-constrained edge deployments.

[^fn-mirai-scale]: **Mirai Botnet Scale**: At its peak, Mirai controlled 600,000+ infected IoT devices generating 623 Gbps attacks. The malware's source code was released publicly, spawning variants that continue threatening edge infrastructures. Modern botnets exceed 2.3 Tbps capacity, emphasizing the compounding risk as ML-enabled devices proliferate without security-by-design.
...
The historical incidents reveal fundamental security principles applicable to machine learning systems: supply chain vulnerabilities (Stuxnet) enable persistent compromise of ML training pipelines and model repositories; insufficient isolation (Jeep Cherokee) allows attackers to escalate from external interfaces to safety-critical ML components; weaponized endpoints (Mirai) demonstrate how compromised ML edge devices can exfiltrate inference data or participate in coordinated attacks.

However, machine learning introduces attack vectors absent from traditional computing. Rather than exploiting buffer overflows or SQL injection vulnerabilities, adversaries can manipulate training data to embed backdoors, craft adversarial inputs that exploit learned decision boundaries, or extract proprietary models through systematic API queries. These ML-specific threats require defenses that account for the data-driven nature of learning systems, not merely hardened infrastructure.

The following sections examine threats unique to machine learning, organized by the stage of the ML lifecycle they target: model confidentiality (theft), training integrity (poisoning), and inference robustness (adversarial attacks). Understanding these attack surfaces enables the design of layered defenses spanning data privacy, model security, runtime monitoring, and hardware trust anchors.

## Threats to ML Models {#sec-security-privacy-threats-ml-models-fbb8}

We now turn to threats that specifically target machine learning models. The architectural vulnerabilities exposed in these historical incidents, network connectivity, supply chain compromise, and physical access, directly affect ML systems with additional complexity from model-specific attack vectors. These threats span the entire ML lifecycle, ranging from training-time manipulations to inference-time evasion, and fall into three broad categories: threats to model confidentiality (e.g., model theft), threats to training integrity (e.g., data poisoning[^fn-data-poisoning]), and threats to inference robustness (e.g., adversarial examples[^fn-adversarial-examples]). Each category targets different vulnerabilities and requires distinct defensive strategies.
...
This vulnerability manifests in the model's sensitivity to small, targeted perturbations. A single image, for instance, can be subtly altered, by altering only a few pixel values, such that a classifier misidentifies a stop sign as a speed limit sign. In natural language processing, specially crafted input sequences may trigger toxic or misleading outputs in a generative model, even when the prompt appears benign to a human reader [@ramesh2021zero; @rombach2022highresolution].

These attacks create significant real-world risks. Adversarial attacks pose important safety and security risks in domains such as autonomous driving, biometric authentication, and content moderation. Unlike data poisoning, which corrupts the model during training, adversarial attacks manipulate the model's behavior at test time, often without requiring any access to the training data or model internals. The attack surface thus shifts from upstream data pipelines to real-time interaction, demanding robust defense mechanisms capable of detecting or mitigating malicious inputs at the point of inference.
...
\|\delta\| \leq \epsilon
$$
where $f(\cdot)$ is the target model, $\mathcal{L}$ is the loss function, and $\epsilon$ defines the maximum allowed perturbation magnitude. The constraint $\|\delta\| \leq \epsilon$ ensures that the perturbation remains small and often imperceptible to humans, while still leading the model to produce an incorrect output.
...
$$
where $\ell$ is a loss function measuring the discrepancy between the surrogate's predictions and the target model's outputs. By minimizing this loss across the query dataset, the attacker builds a surrogate model that closely approximates the target's behavior. Once trained, this surrogate can be used to generate adversarial examples using white-box techniques. These examples often transfer to the original target model, even without internal access, making such attacks effective in black-box settings. This phenomenon, known as adversarial transferability, presents a significant challenge for defense.
...
To address these complex challenges systematically, this section presents a structured framework for defensive strategies, progressing from data-centric protections to infrastructure-level enforcement. These strategies span differential privacy and federated learning, robust model architectures, secure deployment pipelines, runtime validation and monitoring, and hardware-based trust anchors such as secure boot and TEEs. The framework builds upon data protection practices in @sec-data-engineering, where data provenance tracking, validation pipelines, versioning, and quality monitoring establish trust in training data sources—defenses that prevent data poisoning and backdoor injection. It connects forward to the operational security measures detailed in @sec-ml-operations, including continuous integration and deployment security, model versioning, rollback capabilities, and incident response protocols. By integrating safeguards across layers, organizations can build ML systems that not only perform reliably but also withstand adversarial pressure in production environments.

This layered approach is visualized in @fig-defense-stack, which shows a layered defense stack for machine learning systems. The stack progresses from foundational hardware-based security mechanisms to runtime system protections, model-level controls, and privacy-preserving techniques at the data level. Each layer builds on the trust guarantees of the layer below it, forming an end-to-end strategy for deploying ML systems securely. We will progressively explore each of these layers, highlighting their roles in securing machine learning systems against a range of threats.
...
[^fn-sha256-security]: **SHA-256**: Cryptographic hash function producing 256-bit digests, part of the SHA-2 family designed by the NSA in 2001. Despite processing trillions of hashes daily across Bitcoin mining and digital signatures, no practical collision attacks exist after 20+ years, making it the gold standard for file integrity verification. This process ensures that the model has not been altered during transit or storage. For example, a PyTorch .pt or TensorFlow .pb model artifact stored in object storage (e.g., S3) might be verified using a signed hash from a deployment registry before loading into a production container. If the verification fails, the system can block inference, alert an operator, or revert to a trusted model version.
...
Completing our layered defense strategy, we arrive at the foundational hardware level. Machine learning systems deployed in edge devices, embedded systems, and untrusted cloud infrastructure increasingly rely on hardware-based security features to provide additional layers of protection. The hardware acceleration platforms discussed in @sec-ai-acceleration—including GPUs, TPUs, and specialized ML accelerators—often incorporate these security features (secure enclaves, trusted execution environments, hardware cryptographic units), while edge deployment scenarios from @sec-ondevice-learning present unique security challenges.
