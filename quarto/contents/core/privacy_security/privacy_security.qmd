...
These solutions must address not only technical threats, but also regulatory, ethical, and operational requirements across cloud, edge, and embedded deployments. The ethical dimensions of these requirements are examined comprehensively as covered in @sec-responsible-ai, while operational concerns in production environments are addressed as discussed in @sec-ml-operations.
...
Model theft can target two distinct objectives: extracting exact model properties, such as architecture and parameters, or replicating approximate model behavior to produce similar outputs without direct access to internal representations. Understanding the neural network architectures as covered in @sec-dnn-architectures helps recognize which architectural patterns are most vulnerable to extraction attacks. Both forms of theft undermine the security and value of machine learning systems, as explored in the following subsections.
...
This optimization view underlies common adversarial strategies used in both white-box and black-box settings. A full taxonomy of attack algorithms, including gradient-based, optimization-based, and transfer-based techniques, is provided as detailed in @sec-robust-ai, which explores comprehensive approaches to building adversarially robust systems.
...
While ML models themselves present important attack surfaces, they ultimately run on hardware that can introduce vulnerabilities beyond the model's control. The specialized hardware accelerators as discussed in @sec-ai-acceleration introduce unique attack vectors beyond traditional CPUs. In the next section, we examine how adversaries can target the physical infrastructure that executes machine learning workloads—through hardware bugs, physical tampering, side channels, and supply chain risks.
...
This section presents a structured framework for defensive strategies, progressing from data-centric protections to infrastructure-level enforcement. These strategies span differential privacy and federated learning, robust model architectures, secure deployment pipelines, runtime validation and monitoring, and hardware-based trust anchors such as secure boot and TEEs. The framework builds upon data protection practices as covered in @sec-data-engineering and connects forward to the operational security measures detailed as discussed in @sec-ml-operations. By integrating safeguards across layers, organizations can build ML systems that not only perform reliably but also withstand adversarial pressure in production environments.
...
Complementary to DP, federated learning (FL) reduces privacy risks by restructuring the learning process itself. Rather than aggregating raw data at a central location, FL distributes the training across a set of client devices, each holding local data [@mcmahan2017communicationefficient]. This distributed training paradigm, as explored further in @sec-ai-training, requires careful coordination of security measures across multiple participants and infrastructure providers. Clients compute model updates locally and share only parameter deltas with a central server for aggregation:
$$
...
To address scenarios requiring computation on encrypted data, homomorphic encryption (HE) and secure multiparty computation (SMPC) allow models to perform inference or training over encrypted inputs. The computational overhead of homomorphic operations often requires the efficiency optimization techniques as covered in @sec-efficient-ai to maintain practical performance. : **Homomorphic Encryption Breakthrough**: Considered the "holy grail" of cryptography since the 1970s, fully homomorphic encryption remained theoretical until Craig Gentry's 2009 PhD thesis. His breakthrough was realizing that "noisy" ciphertexts could support unlimited operations if periodically "refreshed," solving a decades-old puzzle that allows computation on encrypted data.
...
This section addresses security mechanisms across three key stages: model design, secure packaging and serialization, and deployment and access control. These practices complement the model optimization techniques as discussed in @sec-model-optimizations, where performance improvements must not compromise security properties.
...
When applied together, these practices protect against a range of threats—from model theft and unauthorized inference access to tampering during deployment and output manipulation at runtime. No single mechanism suffices in isolation, but a layered strategy, beginning at the design phase and extending through deployment, provides a strong foundation for securing machine learning systems under real-world conditions. The integration of these security measures into standard development workflows is covered as discussed in @sec-ai-workflow. The operational aspects of maintaining these security measures in production environments are covered as explored in @sec-ml-operations, while distributed training security patterns are addressed as detailed in @sec-ai-training. Framework-specific security implementations are detailed as covered in @sec-ai-frameworks, and the closely related topic of adversarial robustness is covered as discussed in @sec-robust-ai.

### System-level Monitoring {#sec-security-privacy-systemlevel-monitoring-21ae}

Even with robust design and deployment safeguards, machine learning systems remain vulnerable to runtime threats. Attackers may craft inputs that bypass validation, exploit model behavior, or target system-level infrastructure. As ML systems enter production, particularly in cloud, edge, or embedded deployments as covered in @sec-ml-systems, defensive strategies must extend beyond static protection to include real-time monitoring, threat detection, and incident response. The operational aspects of these defenses are central to the MLOps practices as discussed in @sec-ml-operations. This section outlines operational defenses that maintain system trust under adversarial conditions.
...
Machine learning systems deployed in edge devices, embedded systems, and untrusted cloud infrastructure increasingly rely on hardware-based security features to provide additional layers of protection. The hardware acceleration platforms discussed in @sec-ai-acceleration often incorporate these security features, while edge deployment scenarios from @sec-ondevice-learning present unique security challenges. These features can help ensure the integrity of model execution, protect sensitive data, and prevent unauthorized access to system resources. This section discusses several key hardware-based security mechanisms that can enhance the security posture of machine learning systems.
