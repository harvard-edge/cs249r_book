...
Cloud ML's defining characteristic is its centralized infrastructure that operates at unprecedented scale. @fig-cloudml-example illustrates this concept with an example from Google's Cloud TPU[^fn-mlsys-tpu] data center. A single cloud deployment like the NVIDIA DGX A100 provides over 1000 TFLOPS of compute power with 1TB system RAM and 15TB storage, representing a 100-1000x computational advantage over mobile devices (1-10 TOPS). Google's TPU v4 Pod scales even further with 4,096 chips consuming 1-2 megawatts of power. Cloud service providers offer virtual platforms with >100GB/s memory bandwidth housed in globally distributed data centers[^fn-hyperscale]. These centralized facilities operate at kilowatt to megawatt power scales, enabling computational workloads impossible on resource-constrained devices. However, this centralization introduces critical trade-offs: network round-trip latency of 100-500ms eliminates real-time applications, while operational costs scale linearly with usage, with systems like the DGX A100 costing $200,000+ upfront or cloud services charging per inference request.
...
Edge ML operates under intermediate resource constraints that require careful architectural optimization. The NVIDIA Jetson AGX Orin exemplifies edge capabilities with 32GB LPDDR5 RAM and 1-100 TOPS compute power at 15-60W, while the Intel NUC 12 Pro provides 32GB DDR4 and 28W operation at $750-899 price points. These specifications place edge servers in the middle ground between cloud systems (1TB+ RAM, 1000+ TFLOPS, 6.5kW) and mobile devices (8GB RAM, 1-10 TOPS, 3-5W). Memory bandwidth at 25-100 GB/s enables models requiring 100MB-1GB parameters but blocks larger models that dominate cloud applications. These constraints drive specific algorithmic choices: edge inference systems typically use model optimization techniques (explored in @sec-model-optimizations) to reduce memory footprint by 4x and achieve 2-4x speedup compared to cloud models. Local processing eliminates network round-trip latency of 100-500ms, enabling real-time applications with <100ms response requirements. The bandwidth savings become significant at scale: processing 1000 camera feeds locally at 1Mbps each avoids 1Gbps uplink costs and reduces cloud processing expenses by $10,000-100,000 annually.
