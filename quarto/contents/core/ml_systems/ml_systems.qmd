...
Chapter @sec-introduction established that machine learning systems integrate three fundamental components: data, algorithms, and infrastructure working as an integrated system—the three-component framework. But a critical question remains: **Where do these systems actually run?** This question of deployment location proves far more consequential than it might initially appear. The same recommendation model trained on identical data behaves fundamentally differently depending on where it operates. Deployed in a cloud data center, it accesses virtually unlimited computational resources but faces inevitable network latency. That same model on a smartphone eliminates latency and preserves privacy but must operate within severe memory and battery constraints. On an embedded microcontroller, constraints become even more extreme—kilobytes of memory, milliwatts of power—yet the system gains years of autonomous operation in remote locations.

These deployment contexts don't merely constrain implementation details; they fundamentally reshape how the three-component framework manifests in practice. This chapter examines how deployment environments[^fn-deployment-environments] shape ML system architecture across the full spectrum of computing platforms, from massive data centers to coin-cell powered sensors. The three components manifest differently across the deployment spectrum: cloud systems prioritize algorithmic sophistication with abundant infrastructure, while embedded systems[^fn-embedded-systems] must optimize algorithms for minimal infrastructure, and edge systems balance both concerns while managing distributed data challenges.
...
Modern machine learning systems span this deployment spectrum through four primary paradigms, each addressing specific combinations of these trade-offs. Cloud ML uses massive centralized computing resources in data centers[^fn-data-centers]—systems like the NVIDIA DGX A100 with 1TB RAM and >1000 TFLOPS compute consuming 6.5kW—when computational power outweighs latency concerns. Edge ML brings computation closer to data sources using intermediate platforms like the NVIDIA Jetson AGX Orin (32GB RAM, 15-60W) when low latency[^fn-latency] and privacy matter more than unlimited resources. Mobile ML extends capabilities to personal devices like the iPhone 15 Pro (8GB RAM, 3-5W power) when user proximity and offline operation become priorities. Tiny ML enables widespread intelligence on severely constrained devices like the Arduino Nano (256KB RAM, 0.02-0.04W power, $35 cost) when power efficiency and cost matter more than computational complexity.
...
**A critical insight**: These deployment paradigms function as complementary building blocks rather than mutually exclusive choices. While understanding each paradigm individually proves essential for engineering decisions, production ML systems increasingly combine multiple paradigms into hybrid architectures. A voice assistant might use Tiny ML for wake-word detection on a microcontroller, Mobile ML for local speech recognition on a smartphone, Edge ML for contextual processing on a local server, and Cloud ML for complex natural language understanding—all within a single user interaction. This hybrid integration represents the practical reality of modern ML systems, where pure single-paradigm deployments often prove too limiting for real-world requirements.
