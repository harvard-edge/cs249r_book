...
[^fn-jetson-ecosystem]: **NVIDIA Jetson Ecosystem**: Family of embedded computing boards designed for AI at the edge, from the $59 Jetson Nano (5W) to the $1,999 AGX Orin (60W). Used in over 1 million deployed robots, drones, and autonomous vehicles worldwide since launch in 2014.
...
[^fn-transfer-learning]: **Transfer Learning for TinyML**: Reuses knowledge from large pre-trained models by adapting only final layers for specific tasks. A TinyML keyword spotting model might start from a 100MB speech recognition model but deploy as a 20KB model, achieving 95% accuracy on new keywords with <100 training examples.

[^fn-federated-learning]: **Federated Learning**: Distributed training where models learn from decentralized data without sharing raw information [@mcmahan2017federated]. Google's Gboard keyboard improves prediction using federated learning across millions of devices while keeping all typed text private—processing 1 billion+ training examples daily.
