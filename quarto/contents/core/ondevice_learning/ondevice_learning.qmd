...
On-device learning emerges as a response to the limitations of centralized machine learning workflows. The architectural transformation from centralized to decentralized learning creates three distinct operational phases, each with different characteristics and challenges.
...
@fig-centralized-vs-decentralized illustrates these three phases and their operational characteristics. Understanding this progression helps frame the fundamental challenges that on-device learning systems must address.
...
This transition from theory to practice requires systematic engineering approaches that balance competing constraints while maintaining system reliability. Successful on-device learning deployments depend not on individual algorithmic improvements but on holistic system designs that orchestrate multiple techniques within operational constraints. The following sections examine how production systems address these integration challenges through principled design patterns, operational practices, and monitoring strategies that enable scalable, reliable on-device learning deployment.

### Operational Integration with MLOps {#sec-ondevice-learning-operational-integration}

The practical deployment of on-device learning systems requires fundamentally extending the MLOps workflows established in @sec-ml-operations. Traditional deployment pipelines—continuous integration, automated testing, model versioning, monitoring dashboards, and rollback mechanisms—were designed for centralized cloud deployments where models remain static post-deployment. On-device learning challenges every component of these established practices.

Traditional MLOps assumes centralized control over the training process, uniform deployment targets, and coordinated model versioning. On-device learning distributes training across potentially millions of heterogeneous devices with varying capabilities, connectivity patterns, and operational states. This transformation requires rethinking fundamental MLOps practices while building upon established operational principles.
