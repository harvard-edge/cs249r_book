...
Latency and availability constraints provide additional justification for local learning. In edge computing scenarios, connectivity to centralized infrastructure may be unreliable, delayed, or intentionally limited to preserve bandwidth or reduce energy usage. On-device learning enables autonomous improvement of models even in fully offline or delay-sensitive contexts, where round-trip updates to the cloud are architecturally infeasible.
...
Infrastructure efficiency considerations provide economic motivation for distributed learning approaches. Centralized training pipelines require substantial backend infrastructure to collect, store, and process user data from potentially millions of devices. By shifting learning to the edge, systems can reduce communication costs and distribute training workloads across the deployment fleet, relieving pressure on centralized resources while improving system scalability.
...
Each domain reveals a common pattern: deployment environments introduce variation and context-specific requirements that cannot be fully anticipated during centralized training. This pattern illuminates the fundamental design requirement shaping all subsequent technical decisions: learning must be performed efficiently, privately, and reliably under significant resource constraints.

### Training Paradigms {#sec-ondevice-learning-training-paradigms-170f}

These applications demonstrate the practical value of on-device learning across diverse domains. Building on this foundation, we now examine how on-device learning fundamentally differs from traditional ML architectures, revealing a complete reimagining of the training lifecycle that goes far beyond simple deployment choices.
...
These amplifications reveal why simply applying Part III optimization techniques to training workloads proves insufficient. Each constraint category fundamentally shapes on-device learning system design, requiring approaches that build on but extend far beyond the inference-focused methods from earlier chapters.
...
Building on the constraint framework established earlier, data scarcity and quality represent the second major challenge for on-device learning systems. Given the data efficiency constraints outlined previously—limited samples, lack of supervision, irregular arrival patterns, and distribution mismatch—on-device systems must leverage specialized sample-efficient learning approaches that can extract maximal signal from minimal data.
...
The model adaptation and data efficiency techniques examined thus far enable individual devices to learn effectively within resource constraints. Bias-only updates reduce memory footprint by 100x, few-shot learning extracts signal from minimal examples, and experience replay maximizes learning from limited data. However, these local solutions, while creating sophisticated adaptive systems, also reveal a fundamental tension.
