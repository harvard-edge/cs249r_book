...
To address these risks, researchers and engineers must develop advanced techniques for fault detection, isolation, and recovery, ensuring the reliable operation of future ML systems through systematic approaches ([Chapter @sec-security-privacy]).
...
ML systems depend on complex hardware architectures, following hardware acceleration principles ([Chapter @sec-ai-acceleration]), and large-scale computations to train and deploy models that learn from data and make intelligent predictions. As a result, hardware faults can disrupt the MLOps pipeline ([Chapter @sec-ml-operations]), introducing errors that compromise model accuracy, robustness, and reliability [@li2017understanding]. Understanding the types of hardware faults, their mechanisms, and their impact on system behavior is essential for developing strategies to detect, mitigate, and recover from these issues.
...
Understanding this fault taxonomy and its relevance to both traditional computing and ML systems ([Chapter @sec-ml-systems]) provides a foundation for making informed decisions when designing, implementing, and deploying fault-tolerant solutions. This knowledge is crucial for improving the reliability and trustworthiness of computing systems and ML applications.
...
[^fn-single-event-upsets]: **Single Event Upsets (SEUs)**: Radiation-induced bit flips in memory or logic caused by cosmic rays or alpha particles. Occur roughly once per gigabit per month at sea level [@baumann2005soft], increasing exponentially with altitude—commercial aircraft experience 300x higher rates.
...
Power and signal integrity issues constitute another major category of transient fault causes, affecting hardware systems ([Chapter @sec-ai-acceleration]). Voltage fluctuations due to power supply noise or instability [@reddi2013resilient] can cause logic circuits to operate outside their specified voltage ranges, leading to incorrect computations. Ground bounce, triggered by simultaneous switching of multiple outputs, creates temporary voltage variations in the ground reference that can affect signal integrity. Crosstalk, caused by unintended signal coupling between adjacent conductors, can induce noise that temporarily corrupts data or control signals, impacting training processes ([Chapter @sec-ai-training]).
...
Runtime monitoring involves continuously observing the behavior of the system and its components during execution [@francalanza2017foundation], extending the operational monitoring practices from [Chapter @sec-ml-operations]. It helps detect anomalies, errors, or unexpected behavior that may indicate the presence of faults. For example, consider an ML-based image classification system deployed in a self-driving car. Runtime monitoring can be implemented to track the classification model's performance and behavior [@mahmoud2021issre].
...
One prominent category of adversarial attacks is gradient-based attacks[^fn-gradient-based-attacks]. These attacks leverage the gradients of the ML model's loss function to craft adversarial examples. The [Fast Gradient Sign Method](https://www.tensorflow.org/tutorials/generative/adversarial_fgsm) (FGSM) [@goodfellow2014explaining][^fn-fgsm] is a well-known technique in this category. FGSM perturbs the input data by adding small noise in the direction of the gradient of the loss with respect to the input. The goal is to maximize the model's prediction error with minimal distortion to the original input.
...
The mathematical formulation exploits the model's gradient information to construct adversarial perturbations, leveraging the optimization principles detailed in [Chapter @sec-model-optimizations]. The gradient $\nabla_x J(\theta, x, y)$ quantifies how the loss function changes with respect to each input feature, indicating which input modifications would most effectively increase the model's prediction error. The $\text{sign}(\cdot)$ function extracts the direction of steepest ascent, while the perturbation magnitude $\epsilon$ controls the strength of the modification applied to each input dimension.

Where:
- $x$ is the original input (e.g., an image of a cat)
- $y$ is the true label (e.g., "cat")
- $\theta$ represents the model parameters (the trained weights)
- $J(\theta, x, y)$ is the loss function measuring how wrong the model is
- $\epsilon$ is a small scalar controlling perturbation magnitude (typically 0.01-0.3)

These attacks exploit the high-dimensional nature of neural network input spaces, where small perturbations can shift inputs across learned decision boundaries. The effectiveness stems from the complex, non-linear decision surfaces that neural networks learn, which can be sensitive to carefully crafted modifications that remain imperceptible to human observers.
...
Production deployment patterns, MLOps pipeline integration, and monitoring strategies for robust ML systems are covered in detail in [Chapter @sec-ml-operations], while distributed robustness coordination and fault tolerance at scale are addressed in distributed training contexts within [Chapter @sec-ai-training].
...
Addressing software faults in ML systems requires an integrated strategy that spans development, testing, deployment, and monitoring, building upon the operational best practices from [Chapter @sec-ml-operations]. A comprehensive mitigation framework should combine proactive detection methods with robust design patterns and operational safeguards.
