...
Malicious manipulation of ML models remains a critical concern as ML systems face various threats to their integrity. Adversarial attacks, data poisoning[^fn-data-poisoning-concern] attempts, and distribution shifts can cause models to misclassify inputs, exhibit distorted behavior patterns, or produce unreliable outputs. [^fn-data-poisoning-concern]: **Data Poisoning Emergence**: The threat became prominent with Microsoft's Tay chatbot (2016), which was "corrupted" by coordinated user inputs within 24 hours. This incident revealed how bad actors could systematically manipulate training data or model updates to create backdoors or degrade performance—a vulnerability unique to learning systems.
...
Watchdog timers[^fn-watchdog-origin] are hardware components that monitor the execution of critical tasks or processes [@pont2002using]. [^fn-watchdog-origin]: **Watchdog Timer Origins**: The term comes from actual watchdogs used by guards—if the guard didn't regularly check in, something was wrong. The first electronic watchdog timers appeared in missile guidance systems in the 1960s, where an unresponsive computer could mean mission failure. The concept proved so reliable that it's now standard in every microcontroller.
...
One prominent category of adversarial attacks is gradient-based attacks. These attacks leverage the gradients of the ML model's loss function to craft adversarial examples. The [Fast Gradient Sign Method](https://www.tensorflow.org/tutorials/generative/adversarial_fgsm) (FGSM)[^fn-fgsm-breakthrough] is a well-known technique in this category. [^fn-fgsm-breakthrough]: **FGSM Breakthrough**: Ian Goodfellow invented FGSM in 2014 while working at Google, creating the first practical algorithm for generating adversarial examples. His insight was elegantly simple: "If you want to fool a neural network, just push the input in the direction that increases the loss most rapidly." This single-step attack became the foundation for an entire field of adversarial research.
