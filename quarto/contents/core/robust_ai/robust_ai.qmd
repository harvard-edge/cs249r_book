...
This imperative for fault tolerance forms the foundation of what we define as Robust AI:
...
Despite these contextual differences, the essential characteristics of a robust ML system include fault tolerance, error resilience, and sustained performance. By understanding and addressing these multifaceted challenges, it is possible to develop reliable ML systems capable of operating effectively in real-world environments.
...
These principles extend beyond fault recovery to encompass comprehensive performance adaptation strategies. Dynamic resource allocation and auto-scaling approaches are detailed in @sec-ml-operations, while adaptive computation strategies for distributed training are covered in @sec-ai-training. Model complexity adaptation techniques, including dynamic quantization and pruning under resource pressure, are examined in @sec-model-optimizations, and adaptive inference systems for resource-constrained environments are explored in @sec-efficient-ai and @sec-ondevice-learning.
...
The consequences of embedded system failures extend beyond space exploration to commercial aviation. In 2015, a Boeing 787 Dreamliner experienced a complete electrical shutdown mid-flight due to a software bug in its generator control units. The failure stemmed from a scenario in which powering up all four generator control units simultaneously, following 248 days of uninterrupted operation, caused them to enter failsafe mode, disabling all AC electrical power.
...
Crucially, rather than treating robustness as an afterthought, these tools integrate robustness considerations throughout the AI development lifecycle. This early introduction provides the foundation for understanding how the detection and mitigation techniques described throughout this chapter are implemented and validated in practice.
...
Building on these theoretical foundations, **Hardware-based fault injection** methods like FPGA-based injection and radiation testing provide the highest accuracy by directly introducing faults into physical systems. These approaches are essential for validating software models but face limitations in cost, scalability, and flexibility.
...
To address these limitations, **Bridging tools** like Fidelity address the abstraction gap between hardware and software fault modeling by studying fault propagation through the compute stack and identifying equivalence relationships between fault classes.
...
This tooling foundation enables examination of specific fault types in the following sections. The exploration of hardware errors, adversarial attacks, and software vulnerabilities demonstrates how these tools enable both the study of threats and the development of defenses. Each major section will demonstrate how theoretical understanding translates into practical tools and mitigation strategies.
...
The first category, **Transient Faults**, comprises temporary and non-recurring faults. External factors such as cosmic rays, electromagnetic interference, or power fluctuations typically cause them. A common example of a transient fault is a bit flip, where a single bit in a memory location or register changes its value unexpectedly. Transient faults can lead to incorrect computations or data corruption, but they do not cause permanent damage to the hardware.

In contrast, **Permanent Faults**, also called hard errors, are irreversible and persist over time. Physical defects or wear-out of hardware components typically cause them. Examples of permanent faults include stuck-at faults, where a bit or signal is permanently set to a specific value (e.g., always 0 or always 1), and device failures, such as a malfunctioning processor or a damaged memory module. Permanent faults can result in complete system failure or significant performance degradation.

A third category, **Intermittent Faults**, represents recurring faults that appear and disappear intermittently. Unstable hardware conditions, such as loose connections, aging components, or manufacturing defects, typically cause them. Intermittent faults can be challenging to diagnose and reproduce because they may occur sporadically and under specific conditions. Examples include intermittent short circuits or contact resistance issues. These faults can lead to unpredictable system behavior and sporadic errors.
...
These theoretical fault rates translate into practical reliability metrics. **AI Accelerator MTBF Analysis** reveals that typical AI accelerators demonstrate Mean Time Between Failures (MTBF)[^fn-mtbf] values that vary significantly with deployment environment and workload characteristics:
...
**Memory Bandwidth Impact**: Fault tolerance mechanisms significantly affect memory bandwidth utilization, a critical constraint in ML workloads. ECC memory[^fn-ecc-memory] reduces effective bandwidth by 12.5% due to additional storage requirements (8 ECC bits per 64 data bits). Memory scrubbing operations for error detection consume additional 5-15% of available bandwidth depending on scrubbing frequency and memory configuration.
...
This complexity stems from the diverse characteristics of memory technologies. **Memory Technology Fault Characteristics** show that different memory technologies exhibit distinct fault patterns and protection requirements. @tbl-memory-bandwidth-protection shows how ECC protection affects memory bandwidth across different technologies:
...
[^fn-gpu-fault-rates]: **GPU Fault Characteristics**: Graphics processors experience dramatically higher error rates than CPUs due to thousands of simpler cores operating at higher frequencies with aggressive power optimization. NVIDIA's V100 contains 5,120 CUDA cores versus 24-48 cores in server CPUs, creating 100x more potential failure points. Additionally, GPU memory (HBM) operates at 1.6 TB/s bandwidth with minimal error correction, making AI training particularly vulnerable to silent data corruption.
...
[^fn-ks-test]: **Kolmogorov-Smirnov Test**: Non-parametric statistical test developed by Andrey Kolmogorov (1933) and Nikolai Smirnov (1939) to compare probability distributions. In adversarial detection, K-S tests compare input feature distributions against training data, with p-values <0.05 indicating potential adversarial manipulation. Computationally efficient (O(n log n)) but limited to univariate distributions, requiring dimension reduction for high-dimensional inputs like images. A related approach, [kernel density estimation (KDE)](https://mathisonian.github.io/kde/), estimates the probability density function of benign examples in the input space. Since adversarial examples often lie in low-density regions, inputs with estimated density below a threshold can be flagged as potentially adversarial.
