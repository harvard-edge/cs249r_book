...
[^fn-agi-compute-requirements]: **AGI Compute Requirements**: Estimates based on brain computation (10¹⁵-10¹⁶ operations/second for 20 years of learning) suggest approximately 10²⁶-10²⁸ FLOPs total. Alternative estimates from scaling laws project similar requirements. Current frontier models (GPT-4: ~10²³ FLOPs) remain 3-5 orders of magnitude below these thresholds.
...
[^fn-agi-infrastructure-scale]: **AGI Infrastructure Scale**: Training requiring 10²⁷ FLOPs at 50% efficiency would need ~500,000 H100 GPUs running for 1 year, consuming 350 MW continuous power, costing $50+ billion in hardware alone. Post-Moore's Law architectures could reduce these requirements by 10-100x through efficiency improvements.
...
Compound AI systems require sophisticated data engineering to feed their specialized components. Machine learning faces a data availability crisis. GPT-3 consumed 300 billion tokens, GPT-4 likely used over 10 trillion tokens, and current estimates suggest 4.6-17 trillion high-quality tokens exist on the entire internet. At current consumption rates, traditional web-scraped text data may be exhausted by 2026, though synthetic data generation provides alternative scaling paths. This limitation challenges the scaling hypothesis that model capabilities improve predictably with more data and compute.
...
Compound systems require memory architectures that scale beyond individual model constraints. Transformers face quadratic memory scaling with sequence length: a 100K token context requires approximately 247GB just for attention matrices in typical transformer configurations (100K × 100K × 96 heads × 128 dimensions × 2 bytes). This limits knowledge access during inference and prevents long-context reasoning across system components.
...
Reinforcement Learning from Human Feedback (RLHF) addresses alignment through multi-stage training that compounds naturally to system-level alignment. Rather than training solely on text prediction, RLHF creates specialized components within the training pipeline itself.
...
@fig-compound-ai-system illustrates the engineering complexity: the central orchestrator routes user queries to appropriate specialized modules, manages bidirectional communication between components, coordinates iterative refinement processes, and maintains conversation state across the entire interaction. Each component represents distinct engineering challenges requiring different optimization strategies, hardware configurations, and operational practices.
...
**Systems Challenges for Energy-Based Models**: While theoretically compelling, EBMs present significant systems engineering challenges. Inference requires solving optimization problems that can be computationally expensive, particularly for high-dimensional spaces. Training EBMs often involves contrastive learning methods that require generating negative examples through MCMC sampling or other computationally intensive procedures. The optimization landscapes can be complex with many local minima, requiring sophisticated inference algorithms.
...
Five barriers separate current ML systems from artificial general intelligence. Each represents not just an algorithmic challenge but a systems engineering problem requiring innovation across the entire stack, though compound systems approaches may address some through intelligent component orchestration.
...
Building AGI memory systems requires innovations from @sec-data-engineering: hierarchical indexing supporting multi-scale retrieval, attention mechanisms that selectively forget irrelevant information, and experience consolidation that transfers short-term interactions into long-term knowledge. Compound systems may address this through specialized memory components with different temporal scales and retrieval mechanisms.
...
GPT-4 training is estimated to have consumed 50-100 GWh of electricity (unofficial estimates), enough to power 50,000 homes for a year[^fn-gpt4-energy]. Extrapolating to AGI suggests energy requirements exceeding small nations' output, creating both economic and environmental challenges.
...
[^fn-brain-efficiency]: **Biological Efficiency**: Human brain: 20W, 10¹⁵ synaptic operations/second = 5×10¹³ operations/watt. NVIDIA H100: 700W, 10¹⁵ operations/second = 1.4×10¹² operations/watt. Brain appears 35x more efficient despite using chemical signaling, though this comparison should be interpreted carefully as biological and digital computation operate on fundamentally different principles.
...
**Communication protocols** become the bottleneck. Unlike traditional distributed systems that exchange simple state updates, AGI agents must communicate rich semantic information including partial world models, reasoning chains, uncertainty estimates, and intent representations[^fn-agi-communication]. The protocols must compress complex cognitive states into network packets while preserving semantic fidelity across heterogeneous agent architectures. Current internet protocols lack semantic understanding; future AGI networks might require content-aware routing that understands reasoning context.
...
- **Memory bandwidth limitations**: Current systems move 10-100TB/s between processors and memory, but AGI training requires 1-10PB/s. Solutions exist through HBM stacking, processing-in-memory, and optical interconnects
- **Energy efficiency gaps**: Training consumes 50-100 GWh while human learning uses equivalent of 20W-years. Post-Moore's Law architectures (neuromorphic, quantum-hybrid) provide clear paths to 100-1000x efficiency improvements
- **Communication latency**: Distributed training faces 1-10ms interconnect delays limiting coordination. Solutions include optical networks, hierarchical topologies, and intelligent caching
...
Artificial intelligence stands at an inflection point where the building blocks mastered throughout this textbook assemble into systems of extraordinary capability. Large language models demonstrate that engineered scale unlocks emergent intelligence: GPT-3 exhibits 150+ emergent abilities absent in GPT-2, including arithmetic, translation, and code generation, appearing discontinuously at parameter thresholds rather than gradually.
