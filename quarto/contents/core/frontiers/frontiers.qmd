...
However, MoE introduces systems challenges covered in [Chapter @sec-ml-operations]: load balancing across experts, preventing expert collapse where all routing converges to few experts, and managing irregular memory access patterns that complicate hardware optimization[@fedus2022switch].
...
The approach leverages the optimization techniques from [Chapter @sec-model-optimizations]: the model essentially distills its own knowledge through principled self-refinement (@fig-constitutional-ai), similar to knowledge distillation but guided by constitutional objectives rather than teacher models.
...
These training innovations (alignment through human feedback, principled self-improvement, and continual adaptation) transform the training paradigms from [Chapter @sec-ai-training] into dynamic learning systems that improve through deployment rather than remaining static after training.

### Optimization: From Compression to Capability {#sec-agi-systems-optimization-compression-capability-ae5a}

The optimization techniques from [Chapter @sec-model-optimizations] take on new significance as we move toward AGI, evolving from static compression to dynamic intelligence allocation.
...
Each subsystem leverages distinct engineering principles covered throughout this textbook. Language models employ the architectures and optimization techniques from model design chapters. Code interpreters require secure execution environments following principles from [Chapter @sec-security-privacy]. Web search demands efficient data pipelines engineered according to [Chapter @sec-data-engineering]. Memory systems necessitate the storage and retrieval engineering covered in workflow chapters. Safety filters implement responsible AI principles from [Chapter @sec-responsible-ai]. The routing system (perhaps most critically) applies the orchestration techniques from [Chapter @sec-ai-workflow] to coordinate these heterogeneous components seamlessly.
...
These five barriers form an interconnected web of challenges. Progress on any single barrier remains insufficient—AGI requires coordinated breakthroughs across all dimensions, as illustrated in @fig-technical-barriers. The engineering principles developed throughout this textbook—from data engineering ([Chapter @sec-data-engineering]) through distributed training ([Chapter @sec-ai-training]) to robust deployment ([Chapter @sec-ml-operations])—provide foundations for addressing each barrier, though the complete solutions remain unknown.
...
This multi-agent approach requires orchestration ([Chapter @sec-ai-workflow]), robust communication infrastructure, and attention to failure modes where agent interactions could lead to unexpected behaviors.
...
Engineering principles transcend specific architectures or algorithms, providing foundations for future system construction regardless of technological trajectory. The systematic approaches to distributed systems ([Chapter @sec-ai-training]), efficient deployment ([Chapter @sec-ml-operations]), and robust operation ([Chapter @sec-robust-ai]) remain essential regardless of whether AGI emerges from scaled transformers or entirely new architectures.
...
This textbook's progression prepares readers for contribution to this challenge. Understanding encompasses data flow through systems ([Chapter @sec-data-engineering]), model optimization and deployment ([Chapter @sec-model-optimizations]), hardware acceleration of computation ([Chapter @sec-ai-acceleration]), and reliable ML system operation at scale ([Chapter @sec-ml-operations]). These capabilities extend beyond technical skills: they constitute requirements for next-generation intelligent system construction.
