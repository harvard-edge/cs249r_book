...
def find_qmd_files(directory):
    """Finds all .qmd files in a given directory, ignoring 'images' directories."""
    qmd_files = []
...
def process_file(qmd_file, dry_run=False, images_only=False):
    try:
    ...
    # This regex is designed to be more robust. It handles nested brackets
    # in the caption and ensures that it only matches the main image URL.
    pattern = r'(?<!\])\[((?:[^\[\]]|\[[^\]]*\])*)\]\((https?://[^\)]+)\)'
    ...
    for match in matches:
        ...
        # Filter by file extension if the images_only flag is set
        if images_only:
            parsed_url = urlparse(url)
            path = parsed_url.path.lower()
            if not any(path.endswith(ext) for ext in ['.png', '.jpg', '.jpeg', '.gif', '.svg']):
                continue
...
def main():
    ...
    parser.add_argument("--images-only", action="store_true", help="Only download files with common image extensions")
    ...
    for qmd_file in qmd_files:
        process_file(qmd_file, args.dry_run, args.images_only)
