...
- [ ] Macro subsection-flow pass: after the edited-paragraph pass, inspect every changed chapter at the `##` and `###` level so the audit does not overfit to local paragraph repairs. Use prose-line counts excluding code, TikZ, math-heavy examples, and listing/table material as heuristics for spotting sections that may be too fragmented, too compressed, or still stitched together.
  ...
  - [ ] Wave 2 triage queue:
    ...
    - [x] `hw_acceleration.qmd`: combinatorial-complexity hierarchy is unbalanced, runtime-support bridge needs a serving anchor, and multi-chip scaling should preserve package/node/cluster/wafer sequence. Resolved by promoting the mapping search-space components to sibling H4s, anchoring runtime support against the later serving chapter, and clarifying the package/node/cluster/wafer boundary sequence.
    - [x] `model_serving.qmd`: Little's Law scope drift, multi-server stitched prose, tail-tolerant technique taxonomy over-compressed, runtime ecosystem/configuration orphaning, profiling diagnostic compression, and generic node-to-factory bridge. Resolved by narrowing Little's Law to in-flight work before GPU provisioning, distinguishing replica scale-out from distributed inference, organizing tail-tolerant techniques by failure mode, tying runtime configuration back to runtime selection, rechecking the restored profiling loop, and making the node-to-factory bridge name the concrete ML operations machinery.
    - [x] `model_compression.qmd`: repeated quantization-energy thread, PTQ/QAT label-only H5s, dynamic schemes catalog drift, sparsity utilization scope drift, late selection/comparison imbalance, and diagnostics misplaced under hardware-specific libraries. Resolved by softening the quantization speedup claim, collapsing the energy repeat into the precision-to-strategy handoff, removing the thin PTQ functionality label while preserving worked calibration/math structure, organizing dynamic schemes by control variable, refocusing sparsity utilization on executable sparse representations, moving the comparison table into Technique Selection, and giving diagnostic visualization its own subsection.
    - [x] `benchmarking.qmd`: training evaluation promised reproducibility but under-delivers, inference evaluation setup/sequence mismatch, MLPerf synthesis under narrow heading, and LLM table handoff weakened. Resolved by making reproducibility a full-run envelope, moving inference motivation before deployment-diversity detail, giving MLPerf synthesis its own benchmark-gaming bridge, and tying LLM responsiveness metrics to the wall-clock timing calculation.
    - [x] `responsible_engr.qmd`: fairness-accuracy bridge arrives too early, regulatory H4s remain law catalog, and ethical deployment checkpoint/final bridge is scoped too narrowly under US sectoral regulation. Resolved by making the success examples mechanism-driven, tying the fairness frontier to subgroup measurement before Gender Shades, compressing the fairness-tool inventory into a metric-selection principle, collapsing law-specific H4s into one regulatory capability flow, and broadening the ethical-deployment checkpoint bridge beyond US sectoral rules.
- [x] Whole-narrative flow pass: after local edits, reread each changed chapter as a chapter-level argument, not as a list of paragraphs. Resolved the current wave by smoothing Oura Ring lifecycle flow, responsible-engineering test-family flow, multi-chip scaling constraints, Volume 2 conclusion synthesis, graceful-degradation strategy flow, inference routing metrics, ops-scale organizational practices, sustainability resource placement, and several appendix/roadmap paragraphs while preserving stepwise examples where they teach better than prose.
- [x] Cross-reference capitalization pass: scanned changed QMD files for sentence-position cross-reference casing across sections, figures, tables, listings, equations, examples, algorithms, theorems, definitions, checkpoints, notebooks, and lighthouse references. Capitalized forms such as `@Sec-*`, `@Fig-*`, `@Tbl-*`, and `@Lst-*` are used at sentence starts, including structural lead-ins after a colon; lowercase forms such as `@sec-*`, `@fig-*`, `@tbl-*`, and `@lst-*` are used after commas, semicolons, or inside sentences.
- [x] Explicit chapter/part-reference pass: scanned changed QMD files for bare course-catalog phrases such as "the Distributed ML chapters", "the Fleet chapters", "the chapter on X", "earlier chapters", and "later chapters". Replaced vague references with narrative layer language or concrete `@sec-*` references where the cross-reference carries pedagogical value.
- [x] Engineering-chapter identity audit: launched parallel read-only agents across Volume 1 and Volume 2 with a standard rubric asking whether each chapter reads like an ML systems engineering chapter rather than a policy/category survey. Agents rated all chapters `yes` or `mostly`; no high-severity findings were reported.
  ...
  - Resolved medium findings in `vol2/responsible_ai.qmd` by implementing the equalized-odds gap in the production fairness monitor and adding an autonomous-system safety-case table.
  - Added small engineering anchors for low findings in `vol1/hw_acceleration.qmd`, `vol1/ml_ops.qmd`, `vol1/conclusion.qmd`, `vol2/introduction.qmd`, and `vol2/distributed_training.qmd`; deferred remaining low suggestions that would add scope without clear payoff.
  - Placement: Completed before the ordered progressive-disclosure audit and before the final Tokenland litmus, because engineering-anchor repairs may change what later chapters can assume.
- [x] Changed-code/LEGO-standard and notation pass: reviewed branch-wide diffs across all commits in this worktree, not just the latest engineering-identity commit, for notation conventions, added/modified code and listing blocks, LEGO conventions where applicable, output-name rules, and whether the updated production fairness monitor should remain a static listing rather than become a LEGO cell.
  - [x] Notation cleanup: changed the compound-AI tool-branch terms to `T_{\text{tool},1}` / `T_{\text{tool},2}` and removed the ad hoc lowercase `$b$` byte-width symbol from the gradient-payload sentence.
  - [x] Verification passed: `./book/binder check notation --scope consistency`, `./book/binder check code --scope lego-units`, `./book/binder check registry --scope sources`, `python3 book/tools/audit/audit_mlsysim_drift.py`, `./book/binder check math --scope canonical`, `./book/binder check refs --scope inline`, `./book/binder check code --scope lego-prose-literals`, and `git diff --check`.
...
- [x] Ordered concept-map audit: read Volume 1 and Volume 2 as separate books in chapter order. Volume 2 may assume Volume 1 knowledge and the appendices, but should not assume concepts introduced later in Volume 2.
  - [x] Use the concept maps as the starting ledger for prior knowledge in any follow-up progressive-disclosure audit, but audit the maps against the chapter prose rather than treating them as authoritative.
  - [x] Preserve the technical register for junior/senior undergraduate and master's-level systems students. Progressive disclosure should add bridges or local reminders when a concept arrives too early; it should not water down concrete systems anchors such as NVIDIA A100, H100, DGX H100, NVLink, InfiniBand, MFU, FLOP/s, bandwidth, or latency when those specifics teach scale or constraints.
- [x] Cross-chapter progressive-flow pass: read Volume 1 and Volume 2 independently in book order, treating each chapter as allowed to assume only prior chapters in that volume. For later chapters, verify that language naturally builds on already disclosed concepts rather than importing future vocabulary or unearned abstractions.
- [x] Parallel-agent flow audit: used independent part-level agents to evaluate paragraph-to-paragraph flow, chapter argument, and progressive disclosure. Integrated only local bridge findings: SGD/HBM/MFCC/metric glosses in early Volume 1, queueing/SHAP/feedback-loop bridges in deployment chapters, a stale tensor-vs-expert parallelism pointer, an MLPerf scenario gloss, two dense neural-computation footnotes, two fault-tolerance future-dependency bridges, and a decode-equation symbol key in Volume 2. Responsible Fleet returned clean.
- [x] Final litmus test: reread the Systems Approach "Textbooks in Tokenland" post and check the revised chapters against that lens.
  - [x] `vol1/frameworks.qmd`: reframed late framework comparison material as execution/differentiation/abstraction trade-offs rather than a product/runtime catalog.
  - [x] `vol2/inference.qmd`: replaced API-snippet texture in quantized serving with a decision model and runtime contract while preserving the quantized-serving capacity calculation.
  - [x] `vol2/ops_scale.qmd`: reframed fleet economics, deployment APIs, runbooks, and post-incident reviews around operating invariants instead of procurement, YAML, or template checklists.
  - [x] `vol2/security_privacy.qmd`: replaced the calendar-style implementation roadmap with a threat-model-driven maturity model.
  - [x] `vol2/fault_tolerance.qmd`: folded the short training-fault-tolerance umbrella into the checkpointing section and removed the orphaned bridge quiz entry.
