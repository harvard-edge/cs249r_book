...
MLOps, or Machine Learning Operations, is a systematic approach that combines machine learning, data science, and software engineering to automate the end-to-end machine learning lifecycle. This includes everything from data preparation and model training to deployment and maintenance. MLOps ensures that machine learning models are developed, deployed, and maintained efficiently and effectively.
...
With MLOps, there are protocols and tools in place to ensure that the model developed by the data science team can be seamlessly deployed and integrated into the production environment. In essence, MLOps removes friction during the development, deployment, and maintenance of machine learning systems. It improves collaboration between teams through defined workflows and interfaces. MLOps also accelerates iteration speed by enabling continuous delivery for machine learning models.
...
[MLOps](https://cloud.google.com/solutions/machine-learning/mlops-continuous-delivery-and-automation-pipelines-in-machine-learning), on the other hand, stands for Machine Learning Operations, and it extends the principles of DevOps to the machine learning lifecycle. MLOps aims to automate and streamline the end-to-end machine learning lifecycle, from data preparation and model development to deployment and monitoring. The main focus of MLOps is to facilitate collaboration between data scientists, data engineers, and IT operations, and to automate the deployment, monitoring, and management of machine learning models.
...
* Infrastructure misconfigured for machine learning
...
Continuous integration and continuous delivery (CI/CD) pipelines actively automate the progression of machine learning models from initial development into production deployment. Adapted for machine learning systems, CI/CD principles empower teams to rapidly and robustly deliver new models with minimized manual errors.

CI/CD pipelines orchestrate key steps, including checking out new code changes, transforming data, training and registering new models, validation testing, containerization, deploying to environments like staging clusters, and promoting to production. Teams leverage popular CI/CD solutions like [Jenkins](https://www.jenkins.io/), [CircleCI](https://circleci.com/) and [GitHub Actions](https://github.com/features/actions) to execute these MLOps pipelines, while [Prefect](https://www.prefect.io/), [Metaflow](https://metaflow.org/) and [Kubeflow](https://www.kubeflow.org/) offer machine learning-focused options.
...
Weights & Biases provides collaborative tools tailored to machine learning teams. Data scientists log experiments, visualize metrics like loss curves, and share experimentation insights with colleagues. Comparison dashboards highlight model differences. Teams discuss progress and next steps.
...
Technical debt is an increasingly pressing issue for machine learning systems. This metaphor, originally proposed in the 1990s, likens the long-term costs of quick software development to financial debt. Just as some financial debt powers beneficial growth, carefully managed technical debt enables rapid iteration. However, left unchecked, accumulating technical debt can outweigh any gains.

![](images/ai_ops/hidden_debt.png)
*Components of a production ML system with "ML code" highlighted as a small portion of system*
...
Unlike traditional software, machine learning systems can change their own behavior over time, making it difficult to analyze pre-deployment. This debt manifests in feedback loops, both direct and hidden.
...
Machine learning workflows often lack standardized interfaces between components. This leads teams to incrementally "glue" together pipelines with custom code. What emerges are "pipeline jungles" – tangled preprocessing steps that are brittle and resist change. Avoiding modifications to these messy pipelines causes teams to experiment through alternate prototypes. Soon, multiple ways of doing everything proliferate. The lack of abstractions and interfaces then impedes sharing, reuse, and efficiency.
...
### Configuration Debt 
Machine learning systems involve extensive configuration of hyperparameters, architectures, and other tuning parameters. However, configuration is often an afterthought, lacking rigor and testing. Ad hoc configurations proliferate, amplified by the many knobs available for tuning complex ML models.
...
### The Changing World
Machine learning systems operate in dynamic real-world environments. Thresholds and decisions that are initially effective become outdated as the world evolves. But legacy constraints make it difficult to adapt systems to reflect changing populations, usage patterns, and other shifting contextual factors.
...
MLOps provides the framework and practices for coordinating the efforts of various roles involved in developing, deploying and running machine learning systems. Bridging traditional silos between data, engineering and operations teams is key to MLOps success. Enabling seamless collaboration through the machine learning lifecycle accelerates benefit realization while ensuring long-term reliability and performance of ML models.
...
* Establishing data versioning, backup and archival processes for machine learning datasets and features. Enforcing data governance policies.
...
The job of the data scientists is to focus on the research, experimentation, development and continuous improvement of machine learning models. They leverage their expertise in statistics, modeling and algorithms to create high-performing models. Their main responsibilities include:
...
Machine learning engineers enable models data scientists develop to be productized and deployed at scale. Their expertise makes models reliably serve predictions in applications and business processes. Their main responsibilities include:
...
DevOps engineers enable MLOps by building and managing the underlying infrastructure for developing, deploying, and monitoring machine learning models. They provide the cloud architecture and automation pipelines. Their main responsibilities include:
...
As I said in class, I am not really a fan of these folks but they are essential to the job. 🙂 Project managers play a vital role in MLOps by coordinating the activities between the different teams involved in delivering ML projects. They help drive alignment, accountability, and accelerated results. Their main responsibilities include:
...
Furthermore, embedded MLOps must consider the unique challenges posed by integrating machine learning models with other components of the embedded system. For example, the model must be compatible with the system's software and hardware and must be able to interface seamlessly with other components, such as sensors or actuators. This requires a deep understanding of both machine learning and embedded systems, as well as close collaboration between data scientists, engineers, and other stakeholders.

So, while traditional MLOps and embedded MLOps share the common goal of deploying and maintaining machine learning models in production environments, the unique challenges posed by embedded systems require a specialized approach. Embedded MLOps must carefully balance the need for model accuracy and performance with the constraints of the hardware on which the model is deployed. This requires a deep understanding of both machine learning and embedded systems, as well as close collaboration between various stakeholders to ensure the successful integration of machine learning models into embedded systems.

This time we will group the subtopics under broader categories to streamline the structure of our thought process on MLOps. This structure will help you understand how different aspects of MLOps are interconnected and why each is important for the efficient operation of machine learning systems as we discuss the challenges in the context of embedded systems.
...
* Development and Operations Integration 
    * CI/CD Pipelines: Integrating machine learning models into continuous integration and continuous deployment pipelines.
...
![Transfer learning](images/ai_ops/transfer_learning.png)

To mitigate data scarcity and irregularity, strategies like transfer learning become essential. Models can pre-train on large public datasets, then fine-tune the training on limited domain-specific edge data. Even incremental on-device learning to customize models helps overcome the decentralized nature of embedded data. The lack of broad labeled data also motivates semi-supervised techniques.
...
Despite the proliferation of new ML Ops tools in response to the increase in demand, the challenges described earlier have constrained the availability of such tools in embedded systems environments. More recently, new tools such as Edge Impulse have made the development process somewhat easier, as we’ll describe below.

#### Edge Impulse

[Edge Impulse](https://edgeimpulse.com/) is a comprehensive SaaS platform streamlining the process of embedded ML development. The team behind the platform designed it with seven key attributes in mind: accessible, end-to-end, data-centric, iterative, extensible, team-oriented, and community-supported. The UI is very intuitive, such that developers of all levels of experience can be guided through the process of uploading data, choosing a model, training it, and deploying it on all relevant hardware systems. It is important to note, of course, that like all tools Edge Impulse is meant to facilitate the process of embedded ML development, but not answer more fundamental questions such as whether ML is the right solution to pursue, or what domain knowledge is needed to build for a certain environment.

![](images/ai_ops/edge_impulse_dashboard.png)
*Screenshot of Edge Impulse user interface for building workflows from input data to output features*

What makes Edge Impulse so notable is its comprehensive yet intuitive end-to-end workflow. Developers start by uploading their data, either through file upload or command line interface (CLI) tools, after which they can examine raw samples and visualize the distribution of data in the training and test splits. Next, users can pick from a variety of preprocessing “blocks” to facilitate digital signal processing (DSP). While default parameter values are provided, users have the option to customize the parameters as needed, with considerations around memory and latency displayed. Next, users can easily choose their neural network architecture - without any code needed. Thanks to the platform’s visual editor, users can customize the components of the architecture and the specific parameters, all while ensuring that the model is still trainable. Users can also leverage unsupervised learning algorithms, such as K-means clustering and Gaussian mixture models (GMM).

To meet the resource constraints of TinyML, Edge Impulse provides a confusion matrix outlining the key metrics described above such as per-class accuracy and F1 scores. Additionally, the platform clearly articulates the tradeoff between model performance and size and latency, leveraging Renode and device-specific benchmarking. For data streaming use cases, users can also use the performance calibration tool’s genetic algorithm to determine the ideal post-processing configuration by trading off false acceptance rate (FAR) and false rejection rate (FRR). To optimize the model, users can leverage techniques such as quantization, code optimization, or device-specific optimization. When it comes to compilation, users can download their model in the appropriate format for the relevant edge device they are building for, and can even leverage the native firmware SDK for direct data collection on the device.

In addition to streamlining ML development, Edge Impulse is very powerful since it scales the process as well. One of the most powerful features is the EON Tuner, an automated ML (AutoML) interface to assist users of all experience levels in the process of tuning hyperparameters according to system constraints. The Tuner runs a random search algorithm to quickly generate different parameter configurations in the DSP and training steps, then displays the resulting models for the user to choose from based on the relevant metrics across performance, memory, and latency. To improve the data inputs, Edge Impulse facilitates active learning in embedded sensors to train a model on a small labeled subset of the data then manually or automatically label new samples based on their proximity to existing classes. The platform allows developers to embed functionality into their existing workflows, with solutions such as REST APIs, block creation via Docker images, an inferencing SDK library, and a Python SDK.  Developers can even create their own blocks via Docker images. The platform also leverages AWS Elastic Kubernetes Service to dynamically scale compute resources based on workload requirements.

Beyond the accessibility of the platform itself, Edge Impulse team have expanded the knowledge base of the embedded ML ecosystem at large. The platform naturally lends itself to an academic environment, as it has been used in both online embedded ML courses as well as on-site workshops in developing countries across Africa, Asia, and Latin America. The team has also been able to document a series of case studies featuring industry and research use cases - most notably [Oura Ring](https://ouraring.com/), the wearable that leverages ML to identify sleep patterns. Lastly, the team has made a concerted effort to make repositories open source on GitHub, facilitating the growth of a community around the platform. Additionally, users have the opportunity to make their projects on the platform public to share their techniques, and can also download their preprocessing and model inference libraries to share via open source libraries such as Apache. With organization-level access, developers can also collaborate on workflows within a project.

Overall, Edge Impulse stands out as a platform that is uniquely comprehensive and easily integrated into developer workflows. Larger tech platforms such as Google and Microsoft have built their own ML Ops platforms, but they still index more heavily on cloud-scale environments rather than embedded systems. TinyMLOps frameworks such as Neuton AI and Latent AI offer some functionality, but not the same breadth as Edge Impulse’s end-to-end workflow. TFLM is the standard inference engine because it is flexible, opensource, and associated with the TensorFlow ecosystem, yet still uses more memory and storage compared to Edge Impulse’s EON Compiler. Other platforms either are no longer maintained, limited to academic settings rather than production, or simply are not as versatile for adoption.


## Case Studies
### Oura Ring

The Oura Ring is a wearable that, when placed on the user’s finger, can measure activity, sleep, and recovery. Using sensors to track physiological metrics, the device uses embedded machine learning to predict the stages of sleep. To establish a baseline of legitimacy in the industry, Oura conducted a correlation experiment to evaluate the success of the device in predicting sleep stages against a baseline study, resulting in a solid 62% correlation compared to the baseline of 82-83%. Thus, the team set out to determine how they could improve their performance even further.
...
Wearable machine learning enabled sensors enable continuous physiological and activity monitoring outside of clinics, opening up possibilities for timely, data-driven adjustments of therapies. For example, wearable insulin biosensors and wrist-worn ECG sensors for glucose monitoring can automate insulin dosing for diabetes, wrist-worn ECG and PPG sensors can adjust blood thinners based on atrial fibrillation patterns, and accelerometers tracking gait can trigger preventative care for declining mobility in the elderly. The variety of signals that can now be captured passively and continuously allows therapy titration and optimization tailored to each patient’s changing needs. By closing the loop between physiological sensing and therapeutic response with TinyML and ondevice learning, wearables are poised to transform many areas of personalized medicine.
...
This case study analyzes “ClinAIOps” as a model for embedded ML operations in complex clinical environments. We provide an overview of the framework and why it's needed, walk through an application example, and discuss key implementation challenges related to model monitoring, workflow integration, and stakeholder incentives. Analyzing real-world examples like ClinAIOps illuminates crucial principles and best practices needed for reliable and effective AI Ops across many domains. 
...
* MLOps focuses on the machine learning model lifecycle - training, deployment, monitoring. But healthcare involves coordinating multiple human stakeholders - patients, clinicians - not just models.
...
#### Data Collection
The data collected would include continuous blood pressure monitoring using a wrist-worn device equipped with photoplethysmography (PPG) and electrocardiography (ECG) sensors to estimate blood pressure. The wearable would also track the patient's physical activity via embedded accelerometers. The patient would log any antihypertensive medications they take, along with the time and dose. Additionally, the patient's demographic details and medical history from their electronic health record (EHR) would be incorporated. This multimodal real-world data provides valuable context for the AI model to analyze the patient's blood pressure patterns, activity levels, medication adherence, and responses to therapy.

#### AI Model
The on-device AI model would analyze the patient's continuous blood pressure trends, circadian patterns, physical activity levels, medication adherence behaviors, and other context. It would use machine learning to predict optimal antihypertensive medication doses and timing to control the individual's blood pressure. The model would send dosage change recommendations directly to the patient for minor adjustments, or to the reviewing clinician for approval for more significant modifications. By observing clinician feedback on its recommendations, as well as evaluating the resulting blood pressure outcomes in patients, the AI model could be continually retrained and improved to enhance performance. The goal is fully personalized blood pressure management optimized for each patient's needs and responses.
...
Here is a table comparing them. The point of this table is to highlight how when MLOps is put into practice, we need to think about more than just machine learning models.
