...
* **Data Diversity and Quality**: Traditional machine learning algorithms often work well with structured data (input to model is a set of features, ideally independent of each other), but can require a lot of preprocessing effort (feature engineering). On the other hand, deep learning takes the apprach of doing automatic feature engineering as part of the model architecture. The deep learning approach enables bulding end-to-end models that can map directly from unstructured input data (e.g., text, audio, images) to the output you want (instead of using simplistic heuristics that don't take you far); however this makes the model bigger (which requires more data and computational resources to run). In situations with noisy data, the need for larger datasets (when using Deep Learning) accentuates.
