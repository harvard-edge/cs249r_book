...
![The components of microcontrollers. Credit: MIKROE.](https://electronicsforu.com/wp-contents/uploads/2016/05/8c2acbb3d9c4ab2807cec1275225fec7.jpg){#fig-microcontroller width=70%}
...
@fig-micro-comparison illustrates the scale differences between microprocessors and microcontrollers across several parameters: compute, memory, storage, and power needs. As microprocessors are used in data-intensive applications, they require higher clock speeds (1GHz-4GHz) in order to accelerate data processing. The large amounts of memory (512MB-64Gb) and storage (64GB-1TB) enable it to manage multiple programs simultaneously and handle databases. On the other hand, microcontrollers are embedded within other devices and are designed to handle speecific control tasks - such as controling thermostat temperature and kitchen appliances. These applications do not process a lot of data, so their compute needs are very low (1MHz-400MHz) and require little memory (2KB-2MB). And their embedded nature imposes power efficiency constraints (with some requiring as little as 150 microWatts).

![Microcontrollers vs Microprocessors Comparison. Credit: HarvardX.](images/png/microprocessor_vs_microcontroller.png){#fig-micro-comparison}
...
![NVIDIA's Tegra 2 combines two ARM Cortex-A9 cores with an ARM7 for SoC management tasks. Credit: NVIDIA.](https://www.bdti.com/sites/default/files/insidedsp/articlepix/201110/Tegra2.jpg){#fig-soc width=70%}
...
Digital Input/Output (I/O) interfaces are fundamental to embedded systems, enabling interaction with other devices and components. For instance, a digital I/O pin may be used to read a binary signal (0 or 1) from sensors or to control actuators. In embedded systems, these I/O ports often operate under strict timing constraints, a requirement less common in general-purpose computing systems. Moreover, these systems are usually programmed for specific, optimized operations on digital signals, sometimes needing to function in real-time or near-real-time settings.
...
If you examine @fig-nicla-io closely, you will notice indications of I/O pinouts for analog (left-side pins 2,7, and 8), digital, and communication layouts.

![Nicla Vision pinout. Credit: Arduino.](https://content.arduino.cc/assets/ABX00051-pinout.png){#fig-nicla-io}
...
![Nicla Vision battery. Credit: Arduino.](https://content.arduino.cc/assets/nicla_vision_battery.png){#fig-battery}
