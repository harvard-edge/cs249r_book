...
## Overview {#sec-security-privacy-overview-257f}
...
## Definitions and Distinctions {#sec-security-privacy-definitions-distinctions-dad9}
...
### Security Defined {#sec-security-privacy-security-defined-d80c}
...
### Privacy Defined {#sec-security-privacy-privacy-defined-e582}
...
### Security versus Privacy {#sec-security-privacy-security-versus-privacy-8b2f}
...
### Interactions and Trade-offs {#sec-security-privacy-interactions-tradeoffs-8482}
...
::: {.callout-quiz-question #quiz-question-sec-security-privacy-definitions-distinctions-dad9}

1. Which of the following best describes the primary goal of privacy in machine learning systems?
   a) Prevent unauthorized access or disruption
   b) Limit exposure of sensitive information
   c) Enhance system performance
   d) Ensure data integrity

2. What is a typical concern for security in machine learning systems?
   a) Data leakage
   b) Model memorization
   c) Model theft
   d) Re-identification

3. Explain how differential privacy can impact both privacy and model utility in machine learning systems.

4. In what way can encryption improve security but complicate privacy compliance in machine learning systems?

See Answer \ref{quiz-answer-sec-security-privacy-definitions-distinctions-dad9}.

:::

## Historical Incidents {#sec-security-privacy-historical-incidents-c7f3}
...
### Stuxnet {#sec-security-privacy-stuxnet-8d42}
...
![Stuxnet explained. Source: IEEE Spectrum](images/png/stuxnet.png){#fig-stuxnet width=90%}

### Jeep Cherokee Hack {#sec-security-privacy-jeep-cherokee-hack-0793}
...
### Mirai Botnet {#sec-security-privacy-mirai-botnet-9cb6}
...
::: {.callout-quiz-question #quiz-question-sec-security-privacy-historical-incidents-c7f3}

1. Explain why the Jeep Cherokee hack is relevant to modern machine learning systems deployed in connected environments.

2. Which of the following best illustrates a lesson from the Mirai botnet incident that is applicable to ML systems?
   a) Implementing complex encryption algorithms
   b) Ensuring secure credential management and software updates
   c) Focusing solely on model accuracy
   d) Deploying ML models only in centralized data centers

3. The Stuxnet incident primarily targeted ________ to cause physical damage in industrial control systems.

See Answer \ref{quiz-answer-sec-security-privacy-historical-incidents-c7f3}.

:::

## Secure Design Priorities {#sec-security-privacy-secure-design-priorities-7a07}
...
### Device-Level Security {#sec-security-privacy-devicelevel-security-1f2a}
...
### System-Level Isolation {#sec-security-privacy-systemlevel-isolation-08d0}
...
### Large-Scale Network Exploitation {#sec-security-privacy-largescale-network-exploitation-58eb}
...
### Toward Secure Design {#sec-security-privacy-toward-secure-design-80e7}
...
::: {.callout-quiz-question #quiz-question-sec-security-privacy-secure-design-priorities-7a07}

1. Which of the following is a primary concern for device-level security in edge ML deployments?
   a) High computational power
   b) Secure boot processes
   c) Large storage capacity
   d) User-friendly interfaces

2. True or False: System-level isolation is only necessary for automotive applications of machine learning.

3. Explain how the lessons from the Jeep Cherokee hack can be applied to improve security in machine learning systems.

See Answer \ref{quiz-answer-sec-security-privacy-secure-design-priorities-7a07}.

:::

## Threats to ML Models {#sec-security-privacy-threats-ml-models-9ab5}
...
\node[Box2,left=of B2](LB2){Backdoors};
\node[Box2,right=of B2](RB2){Label\\ Manipulation};
\node[Box2,left=of B3](LB3){Model Theft};
\node[Box2,right=of B3](RB3){Model Inversion};
\node[Box2,left=of B4](LB4){Adversarial\\ Examples};
\node[Box2,right=of B4](RB4){Membership\\ Inference};
\node[Box3,above left=1.5 of B1](PL){Privacy Leakage};
\node[Box3,above right=1.5 of B1](DP){Data Poisoning};
%
\scoped[on background layer]
\node[draw=BackLine,inner xsep=10mm,inner ysep=5mm,
...
### Model Theft {#sec-security-privacy-model-theft-e5f7}
...
\scoped[on background layer]
\node[draw=BackLine,inner xsep=10mm,minimum height=92mm,
...
%%%
\node[Box3,right=4.5 of B1](RB1){Access to model file or deployment artifact};
\node[Box3,right=4.5 of B5](RB4){Use or resell\\ proprietary IP};
\node[Box3](RB2)at($(RB1)!0.35!(RB4)$){Extract parameters, architectire, hyperparameters};
\node[Box3](RB3)at($(RB1)!0.7!(RB4)$){Reconstruct original\\ model};
\foreach \i in{1,2,3}{
\pgfmathtruncatemacro{\newI}{\i + 1}
\draw[Line,-latex](RB\i)--(RB\newI);
}
\scoped[on background layer]
\node[draw=GreenD,inner xsep=10mm,minimum height=92mm,,
...
#### Exact Model Theft {#sec-security-privacy-exact-model-theft-f76d}
...
#### Approximate Model Theft {#sec-security-privacy-approximate-model-theft-15ea}
...
#### Case Study: Tesla IP Theft {#sec-security-privacy-case-study-tesla-ip-theft-0de5}
...
### Data Poisoning {#sec-security-privacy-data-poisoning-628f}
...
### Adversarial Attacks {#sec-security-privacy-adversarial-attacks-0972}
...
### Case Study: Traffic Sign Detection Model Trickery {#sec-security-privacy-case-study-traffic-sign-detection-model-trickery-ddff}
...
::: {#fig-threat-mitigation-flow fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[line join=round,font=\small\usefont{T1}{phv}{m}{n}]
\tikzset{%
Line/.style={line width=1.0pt,black!50},
Box/.style={inner xsep=2pt,inner ysep=6pt,
    ...
    align=flush center,
    text width=33mm,
    minimum width=33mm, minimum height=10.5mm
  },
...
%
\node[Box2,right=1.5of B1](SB1){Data Poisoning};
...
%
\node[Box3,right=1.5of SB1](RB1){Adversarial Examples};
...
::: {.callout-quiz-question #quiz-question-sec-security-privacy-threats-ml-models-9ab5}

1. Which of the following best describes the primary threat posed by model theft?
   a) Loss of model accuracy
   b) Economic loss and exposure of proprietary information
   c) Increased computational costs
   d) Reduced model interpretability

2. Explain why data poisoning poses a significant threat to the integrity of machine learning models.

3. Adversarial attacks exploit a model's sensitivity to ________ to cause incorrect predictions.

4. Order the following steps involved in approximate model theft: Record responses, Train surrogate model, Access to public API, Send crafted queries.

5. Describe a defensive strategy that could mitigate the risk of model theft in a public API deployment.

See Answer \ref{quiz-answer-sec-security-privacy-threats-ml-models-9ab5}.

:::

## Threats to ML Hardware {#sec-security-privacy-threats-ml-hardware-a12d}
...
### Hardware Bugs {#sec-security-privacy-hardware-bugs-ad65}
...
Further research has revealed that these were not isolated incidents. Variants such as Foreshadow[^fn-foreshadow], ZombieLoad[^fn-zombieload], and RIDL[^fn-ridl] target different microarchitectural elements, ranging from secure enclaves to CPU internal buffers, demonstrating that speculative execution flaws are a systemic hardware risk.
...
### Physical Attacks {#sec-security-privacy-physical-attacks-51dc}
...
### Fault Injection Attacks {#sec-security-privacy-fault-injection-attacks-8273}
...
![Laser fault injection attack on a microcontroller. Source: @breier2018deeplaser.](images/png/laser_bitflip.png){#fig-laser-bitflip fig-pos=t!}
...
![Fault-injection demonstrated with assembly code. Source: @breier2018deeplaser.](images/png/Fault-injection_demonstrated_with_assembly_code.png){#fig-injection width=75% fig-pos=b!}
...
### Leaky Interfaces {#sec-security-privacy-leaky-interfaces-3cfc}
...
### Counterfeit Hardware {#sec-security-privacy-counterfeit-hardware-940c}
...
### Supply Chain Risks {#sec-security-privacy-supply-chain-risks-3bd0}
...
### Case Study: The Supermicro Hardware Security Controversy {#sec-security-privacy-case-study-supermicro-hardware-security-controversy-8a36}
...
::: {.callout-quiz-question #quiz-question-sec-security-privacy-threats-ml-hardware-a12d}

1. Which of the following best describes a side-channel attack in ML hardware security?
   a) Direct manipulation of hardware components to alter system behavior
   b) Exploitation of leaked information from hardware operation to extract sensitive data
   c) Introduction of counterfeit components into the hardware supply chain
   d) Insertion of malicious code through software vulnerabilities

2. Explain how hardware bugs can pose a threat to machine learning systems and give an example.

3. True or False: Physical attacks on ML hardware are only a concern for systems deployed in remote or isolated environments.

4. Fault injection attacks can induce errors in hardware operation by applying ________ disturbances, leading to incorrect outputs or data leaks.

5. What are some strategies to mitigate supply chain risks in ML hardware, and why are they important?

See Answer \ref{quiz-answer-sec-security-privacy-threats-ml-hardware-a12d}.

:::

## Defensive Strategies {#sec-security-privacy-defensive-strategies-0542}
...
### Data Privacy Techniques {#sec-security-privacy-data-privacy-techniques-2eba}
...
#### Differential Privacy {#sec-security-privacy-differential-privacy-0c93}
...
#### Federated Learning {#sec-security-privacy-federated-learning-8810}
...
#### Synthetic Data Generation {#sec-security-privacy-synthetic-data-generation-8855}
...
#### Comparative Properties {#sec-security-privacy-comparative-properties-6731}
...
### Secure Model Design {#sec-security-privacy-secure-model-design-b930}
...
### Secure Model Deployment {#sec-security-privacy-secure-model-deployment-8b00}
...
::: {.content-visible when-format="html"}
::: {#lst-openai-api .callout-important title="Example of securely loading an API key for OpenAI's GPT-4 model. The API key is retrieved from an environment variable to avoid hardcoding sensitive information in the source code."}
```{.python}
import openai
import os
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-openai-api
#| lst-cap: Example of securely loading an API key for OpenAI's GPT-4 model. The API key is retrieved from an environment variable to avoid hardcoding sensitive information in the source code.
#| eval: false
import openai
import os
...
### System-level Monitoring {#sec-security-privacy-systemlevel-monitoring-9282}
...
#### Input Validation {#sec-security-privacy-input-validation-ba59}
...
#### Output Monitoring {#sec-security-privacy-output-monitoring-c0e1}
...
#### Integrity Checks {#sec-security-privacy-integrity-checks-aa20}
...
#### Response and Rollback {#sec-security-privacy-response-rollback-fe7b}
...
### Hardware-based Security {#sec-security-privacy-hardwarebased-security-e032}
...
#### Trusted Execution Environments {#sec-security-privacy-trusted-execution-environments-9a6b}
...
\end{tikzpicture}
```
System-on-chip secure enclave. Source: Apple.
:::
...
#### Secure Boot {#sec-security-privacy-secure-boot-c502}
...
\begin{tikzpicture}[line join=round,font=\small\usefont{T1}{phv}{m}{n}]
\tikzset{%
Line/.style={black!50,-{Triangle[width = 6pt, length = 6pt]}, line width = 1.75pt,text=black},
LineA/.style={line width=1.0pt,black!50,latex-latex},
Box/.style={inner xsep=2pt,inner ysep=6pt,
    node distance=0.5,
    draw=BlueLine,
...
diamond, minimum width=45mm, minimum height=20mm, text centered,inner sep=0pt},
Box4/.style={Box,anchor=west,minimum width=42mm, minimum height=48mm},
Box5/.style={Box,draw=BlueLine,fill=BlueL!50},
Box6/.style={Box,draw=OrangeLine,fill=OrangeL!50},
}
...
\draw[Line](B7)--node[right]{No}(B8);
\draw[Line](B7.east)--node[above]{Yes}++(1.8,0)--++(0,10.5)-|(B9);
\draw[Line](B11.west)--node[above]{No}++(-1.8,0)|-(B8);
\draw[Line](B11)--node[right]{Yes}(B12);
\end{tikzpicture}
```
Secure Boot flow. Source: @Rashmi2018Secure.
:::

A well-known real-world implementation of Secure Boot appears in Apple's Face ID system, which leverages advanced machine learning for facial recognition. For Face ID to operate securely, the entire device stack, from the initial power-on to the execution of the model, must be verifiably trusted.
...
#### Hardware Security Modules {#sec-security-privacy-hardware-security-modules-39fa}
...
#### Physical Unclonable Functions {#sec-security-privacy-physical-unclonable-functions-2c7a}
...
#### Mechanisms Comparison {#sec-security-privacy-mechanisms-comparison-d58e}
...
### Toward Trustworthy Systems {#sec-security-privacy-toward-trustworthy-systems-3b77}
...
\tikzset{%
LineA/.style={line width=1.0pt,black!50,latex-latex},
Line/.style={BrownLine!60, -{Triangle[width = 6pt, length = 6pt]}, line width = 1.7pt},
Box/.style={inner xsep=2pt,inner ysep=6pt,
    node distance=0.5,
    draw=RedLine,
    line width=0.75pt,
...
\node[Box,right=of B1](B2){Model Layer: Robustness, Watermarking, Secure Deployment};
...
\node[Box,right=of B3](B4){Hardware Layer: TEEs, Secure Boot, PUFs};
 %
\node[Box2,above=0.7 of B2](B22){Select Defenses Across the Stack};
\node[Box3,above=0.7 of B22](B23){Assess Threat Model \& Deployment Context};
%
\node[Box6,below=0.7 of B4](B44){Plan for Runtime Adaptation and Recovery};
...
::: {.callout-quiz-question #quiz-question-sec-security-privacy-defensive-strategies-0542}

1. Which of the following best describes the purpose of a layered defense strategy in machine learning systems?
   a) To focus solely on data encryption
   b) To provide a single point of security
   c) To integrate multiple security mechanisms across different layers
   d) To rely only on hardware-based security

2. Explain how Trusted Execution Environments (TEEs) contribute to the security of machine learning systems.

3. In a machine learning system, ________ is used to ensure that only cryptographically verified software components are executed during the boot process.

4. True or False: Differential Privacy can be applied at both the training and inference stages of a machine learning system to enhance privacy.

5. Order the following layers of the defense stack from foundational to top-level: System-Level Security, Data Privacy & Governance, Model-Level Security, Hardware-Level Security.

See Answer \ref{quiz-answer-sec-security-privacy-defensive-strategies-0542}.

:::

## Offensive Capabilities {#sec-security-privacy-offensive-capabilities-0239}
...
### Case Study: Deep Learning for SCA {#sec-security-privacy-case-study-deep-learning-sca-b9ae}
...
![Power traces from an AES S-box operation. Source: @scaaml_2019.](images/png/side-effects-binary.png){#fig-side-channel-curves width=80%}
...
::: {.callout-quiz-question #quiz-question-sec-security-privacy-offensive-capabilities-0239}

1. Which of the following is an example of an offensive use of machine learning?
   a) Improving model accuracy through hyperparameter tuning
   b) Generating adversarial examples to evade detection systems
   c) Reducing model training time with optimized algorithms
   d) Enhancing data privacy using differential privacy

2. Explain how the dual-use nature of machine learning models impacts system-level threat models.

3. In offensive machine learning, ________ models can be used for reconnaissance and fingerprinting to automate profiling of system behavior.

4. True or False: Offensive machine learning applications are purely speculative and have not been integrated into real-world attack scenarios.

5. Discuss the challenges posed by the scaling behavior of attack-oriented machine learning models.

See Answer \ref{quiz-answer-sec-security-privacy-offensive-capabilities-0239}.

:::

## Conclusion {#sec-security-privacy-conclusion-7e97}
...
::: {.callout-quiz-question #quiz-question-sec-security-privacy-conclusion-7e97}

1. Explain why security in machine learning systems is described as an evolving process rather than a static checklist.

2. Which of the following best describes the importance of a defense-in-depth approach in securing ML systems?
   a) It focuses solely on hardware-level security.
   b) It provides multiple layers of security to address various vulnerabilities.
   c) It ensures that only one security layer is active at any time.
   d) It relies on a single security method that is universally applicable.

3. True or False: The same security measures that protect a publicly exposed API will be sufficient for an embedded medical device.

See Answer \ref{quiz-answer-sec-security-privacy-conclusion-7e97}.

:::

## Resources {#sec-security-privacy-resources-c05d}
...
:::{.callout-quiz-answer #quiz-answer-sec-security-privacy-definitions-distinctions-dad9}
...
1. **Which of the following best describes the primary goal of privacy in machine learning systems?**
   ...
   The correct answer is B. Privacy in machine learning systems aims to limit the exposure of sensitive information, ensuring that personal or proprietary data is not disclosed or misused, even when the system operates correctly.

   *Learning Objective*: Understand the primary goal of privacy in ML systems.
...
2. **What is a typical concern for security in machine learning systems?**
   ...
   The correct answer is C. Security in machine learning systems is concerned with protecting models from theft, ensuring that adversaries cannot access or manipulate the model for malicious purposes.

   *Learning Objective*: Identify typical security concerns in ML systems.

3. **Explain how differential privacy can impact both privacy and model utility in machine learning systems.**

   Differential privacy enhances privacy by adding noise to data, reducing the risk of exposing individual data points. However, this can also lower model utility by affecting the accuracy of the model's predictions, as the added noise can obscure useful patterns in the data.

   *Learning Objective*: Analyze the trade-offs between privacy and model utility in differential privacy.

4. **In what way can encryption improve security but complicate privacy compliance in machine learning systems?**

   Encryption improves security by protecting data from unauthorized access. However, it can complicate privacy compliance by obscuring data transparency and auditability, making it difficult to verify that privacy regulations are being followed.

   *Learning Objective*: Understand the interaction between encryption, security, and privacy compliance.

:::

:::{.callout-quiz-answer #quiz-answer-sec-security-privacy-historical-incidents-c7f3}

1. **Explain why the Jeep Cherokee hack is relevant to modern machine learning systems deployed in connected environments.**

   The Jeep Cherokee hack is relevant because it exposed vulnerabilities in connected systems that can be exploited remotely. As ML systems are increasingly integrated into connected environments, such as autonomous vehicles, the need for secure interfaces, defense-in-depth strategies, and robust incident response protocols becomes critical to ensure safety and reliability.

   *Learning Objective*: Analyze the implications of connected system vulnerabilities for ML deployments.
...
2. **Which of the following best illustrates a lesson from the Mirai botnet incident that is applicable to ML systems?**
   ...
   The correct answer is B. Ensuring secure credential management and software updates is important to prevent devices from being compromised and used in large-scale attacks, as demonstrated by the Mirai botnet incident.

   *Learning Objective*: Identify security practices critical for protecting ML-enabled systems from large-scale exploitation.

3. **The Stuxnet incident primarily targeted ________ to cause physical damage in industrial control systems.**

   The answer is "programmable logic controllers (PLCs)". Stuxnet specifically targeted PLCs to manipulate electromechanical processes, demonstrating how software vulnerabilities can lead to physical disruptions.

   *Learning Objective*: Recall the specific components targeted by Stuxnet and understand their role in industrial systems.

:::

:::{.callout-quiz-answer #quiz-answer-sec-security-privacy-secure-design-priorities-7a07}
...
1. **Which of the following is a primary concern for device-level security in edge ML deployments?**
   ...
   The correct answer is B. Secure boot processes are important for device-level security in edge ML deployments to ensure that only trusted software can run on the device, protecting it from unauthorized access and manipulation.

   *Learning Objective*: Understand the importance of secure boot processes in protecting edge ML devices.

2. **True or False: System-level isolation is only necessary for automotive applications of machine learning.**

   False. System-level isolation is necessary for any ML application where safety-critical functions are involved, including healthcare, industrial automation, and infrastructure systems, to prevent external threats from compromising critical system pathways.

   *Learning Objective*: Recognize the importance of system-level isolation across various ML applications.

3. **Explain how the lessons from the Jeep Cherokee hack can be applied to improve security in machine learning systems.**

   The Jeep Cherokee hack highlights the need for strong isolation between connected services and safety-critical functions. In ML systems, this means ensuring that external interfaces, like those used for updates or diagnostics, cannot access or manipulate safety-critical ML components. Implementing architectural compartmentalization and authenticated communication channels can help prevent unauthorized access.

   *Learning Objective*: Apply lessons from past security incidents to enhance ML system security.

:::

:::{.callout-quiz-answer #quiz-answer-sec-security-privacy-threats-ml-models-9ab5}
...
1. **Which of the following best describes the primary threat posed by model theft?**
   ...
   The correct answer is B. Model theft primarily leads to economic loss and exposure of proprietary information, as adversaries can replicate or misuse the stolen models, undermining their economic value and confidentiality.

   *Learning Objective*: Understand the implications of model theft on economic value and confidentiality in ML systems.

2. **Explain why data poisoning poses a significant threat to the integrity of machine learning models.**

   Data poisoning is a significant threat because it involves injecting malicious data into the training set, which can corrupt the learning process and lead to incorrect or biased model behavior. This manipulation can degrade model performance, introduce vulnerabilities, and cause systemic failures, especially in applications relying on external data sources.

   *Learning Objective*: Analyze the impact of data poisoning on the integrity and reliability of ML models.

3. **Adversarial attacks exploit a model's sensitivity to ________ to cause incorrect predictions.**

   The answer is "small perturbations". Adversarial attacks use small, often imperceptible changes to input data to manipulate model predictions, highlighting vulnerabilities in the model's decision boundary.

   *Learning Objective*: Identify the mechanism by which adversarial attacks manipulate ML model predictions.

4. **Order the following steps involved in approximate model theft: Record responses, Train surrogate model, Access to public API, Send crafted queries.**

   1. Access to public API, 2. Send crafted queries, 3. Record responses, 4. Train surrogate model. This sequence outlines the process of approximate model theft, where an attacker uses observed input-output behavior to create a surrogate model.

   *Learning Objective*: Understand the process of approximate model theft and its implications for ML systems.

5. **Describe a defensive strategy that could mitigate the risk of model theft in a public API deployment.**

   One defensive strategy is to implement rate limiting and anomaly detection on API queries to identify and block suspicious patterns indicative of model extraction attempts. Additionally, using techniques like output watermarking can help trace unauthorized model replication.

   *Learning Objective*: Apply defensive strategies to protect against model theft in ML systems.

:::

:::{.callout-quiz-answer #quiz-answer-sec-security-privacy-threats-ml-hardware-a12d}
...
1. **Which of the following best describes a side-channel attack in ML hardware security?**
   ...
   The correct answer is B. Side-channel attacks exploit leaked information from hardware operation, such as power consumption or electromagnetic emissions, to extract sensitive data. This is particularly relevant in ML systems where hardware characteristics can inadvertently reveal information about the data being processed.

   *Learning Objective*: Understand the concept of side-channel attacks and their relevance to ML hardware security.

2. **Explain how hardware bugs can pose a threat to machine learning systems and give an example.**

   Hardware bugs are intrinsic flaws in hardware designs that can be exploited to access, manipulate, or extract sensitive data, compromising system integrity. An example is the Meltdown and Spectre vulnerabilities, which exploit speculative execution in CPUs to bypass memory isolation and access sensitive data.

   *Learning Objective*: Analyze the impact of hardware bugs on the security of ML systems and understand specific examples of such vulnerabilities.

3. **True or False: Physical attacks on ML hardware are only a concern for systems deployed in remote or isolated environments.**

   False. Physical attacks can occur in any environment where adversaries have physical access to the hardware, including edge devices, data centers, and even consumer electronics. These attacks can bypass software defenses and directly manipulate hardware components.

   *Learning Objective*: Recognize the broad applicability of physical attack threats to ML hardware across various deployment environments.

4. **Fault injection attacks can induce errors in hardware operation by applying ________ disturbances, leading to incorrect outputs or data leaks.**

   The answer is "physical or electrical". Fault injection attacks use disturbances like voltage manipulation or electromagnetic pulses to disrupt hardware operations, potentially leading to incorrect outputs or data leaks.

   *Learning Objective*: Understand the mechanisms and implications of fault injection attacks on ML hardware.

5. **What are some strategies to mitigate supply chain risks in ML hardware, and why are they important?**

   Mitigation strategies include screening suppliers, validating component provenance, implementing tamper-evident protections, and continuously monitoring system behavior for signs of compromise. These strategies are important because they help ensure the integrity and security of the hardware components used in ML systems, which is critical for maintaining trustworthiness and preventing unauthorized access or manipulation.

   *Learning Objective*: Evaluate strategies for mitigating supply chain risks in ML hardware and understand their importance for system security.

:::

:::{.callout-quiz-answer #quiz-answer-sec-security-privacy-defensive-strategies-0542}
...
1. **Which of the following best describes the purpose of a layered defense strategy in machine learning systems?**
   ...
   The correct answer is C. A layered defense strategy integrates multiple security mechanisms across different layers, providing comprehensive protection against various threats in machine learning systems.

   *Learning Objective*: Understand the concept and purpose of layered defense strategies in ML systems.
...
2. **Explain how Trusted Execution Environments (TEEs) contribute to the security of machine learning systems.**
   ...
   *Learning Objective*: Understand the role of TEEs in enhancing the security of ML systems.

3. **In a machine learning system, ________ is used to ensure that only cryptographically verified software components are executed during the boot process.**

   The answer is "Secure Boot". Secure Boot ensures that only cryptographically verified software components are executed during the boot process, preventing unauthorized or malicious code from running.

   *Learning Objective*: Recall the function of Secure Boot in ML systems.

4. **True or False: Differential Privacy can be applied at both the training and inference stages of a machine learning system to enhance privacy.**

   True. Differential Privacy can be applied during training to protect sensitive data and at inference to ensure privacy when returning aggregate statistics, balancing privacy with performance goals.

   *Learning Objective*: Recognize the applications of Differential Privacy in ML systems.

5. **Order the following layers of the defense stack from foundational to top-level: System-Level Security, Data Privacy & Governance, Model-Level Security, Hardware-Level Security.**

   1. Hardware-Level Security, 2. System-Level Security, 3. Model-Level Security, 4. Data Privacy & Governance. This order reflects the progression from foundational hardware security to data privacy measures at the top level.

   *Learning Objective*: Understand the hierarchical structure of the defense stack in ML systems.

:::

:::{.callout-quiz-answer #quiz-answer-sec-security-privacy-offensive-capabilities-0239}
...
1. **Which of the following is an example of an offensive use of machine learning?**
   ...
   The correct answer is B. Generating adversarial examples to evade detection systems is an offensive use of ML, as it involves crafting inputs to bypass security measures.

   *Learning Objective*: Understand specific offensive applications of machine learning models.

2. **Explain how the dual-use nature of machine learning models impacts system-level threat models.**

   The dual-use nature of ML models means they can be used both to enhance security and to launch attacks. This impacts threat models by requiring them to consider not only the vulnerabilities of ML systems but also how these systems can be used offensively to compromise other components.

   *Learning Objective*: Analyze the implications of ML's dual-use nature on system security.

3. **In offensive machine learning, ________ models can be used for reconnaissance and fingerprinting to automate profiling of system behavior.**

   The answer is "supervised classifiers". Supervised classifiers can learn patterns in system behavior, enabling automated reconnaissance and fingerprinting.

   *Learning Objective*: Recall specific ML model types used in offensive applications.

4. **True or False: Offensive machine learning applications are purely speculative and have not been integrated into real-world attack scenarios.**

   False. Offensive ML applications are not speculative; they are already being integrated into real-world attack scenarios, such as spam filtering evasion and malware generation.

   *Learning Objective*: Correct misconceptions about the real-world application of offensive ML capabilities.

5. **Discuss the challenges posed by the scaling behavior of attack-oriented machine learning models.**

   The scaling behavior of attack-oriented ML models poses challenges as they can improve in performance with more data and compute resources, similar to beneficial AI. This creates an asymmetry where attackers can scale their capabilities with minimal cost, while defenders face constraints such as latency and regulatory requirements.

   *Learning Objective*: Evaluate the challenges and asymmetries introduced by scalable offensive ML models.

:::

:::{.callout-quiz-answer #quiz-answer-sec-security-privacy-conclusion-7e97}
...
1. **Explain why security in machine learning systems is described as an evolving process rather than a static checklist.**
   ...
   *Learning Objective*: Understand the dynamic nature of security in ML systems and why continuous adaptation is necessary.
...
2. **Which of the following best describes the importance of a defense-in-depth approach in securing ML systems?**
   ...
   The correct answer is B. It provides multiple layers of security to address various vulnerabilities. A defense-in-depth approach integrates multiple security measures across different layers of the system, ensuring comprehensive protection against a wide range of threats by addressing vulnerabilities at each level.

   *Learning Objective*: Recognize the importance and structure of a defense-in-depth strategy in ML system security.

3. **True or False: The same security measures that protect a publicly exposed API will be sufficient for an embedded medical device.**

   False. The security measures required for a publicly exposed API may not be sufficient for an embedded medical device due to differing deployment contexts, risk profiles, and regulatory requirements. Each system requires a tailored security approach that considers its specific vulnerabilities and operational environment.

   *Learning Objective*: Understand the need for context-specific security strategies in different ML deployment scenarios.
