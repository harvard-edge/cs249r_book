...
The historical breaches described in this section reveal how weaknesses in system design—whether at the hardware, software, or network level—can lead to widespread and often physical consequences. While these incidents did not directly involve machine learning systems, they provide critical insights into the architectural patterns and deployment practices that are increasingly common in modern ML deployments. The lessons can be grouped into three key areas of concern: device-level security, system-level isolation and control, and large-scale network exploitation.

### Device-Level Security

The Mirai botnet highlights the destructive potential of large-scale exploitation of devices with poor security hygiene. The attack was made possible by the widespread use of default usernames and passwords, lack of secure firmware update mechanisms, and unencrypted communications. These weaknesses are not limited to consumer IoT devices. Many machine learning systems, particularly those deployed at the edge, rely on lightweight embedded hardware such as ARM-based processors running minimal operating systems like Linux.

Edge ML devices—ranging from smart sensors to industrial controllers—are increasingly tasked with collecting and processing data locally before transmitting results to the cloud. Without proper device-level security, these systems are vulnerable to being remotely compromised and weaponized, as demonstrated by the Mirai botnet. An attacker could build a botnet of compromised ML devices, leveraging their computational power for distributed attacks, data exfiltration, or system disruption.

### System-Level Isolation

The Jeep Cherokee hack exposed the dangers of poor isolation between externally connected services and safety-critical control systems. By exploiting a vulnerability in the Uconnect entertainment system, researchers were able to manipulate core vehicle functions such as steering and braking. This incident forced the automotive industry to confront the reality that networked software services must be properly isolated from critical control pathways.

Modern vehicles increasingly integrate machine learning models for driver-assistance features, autonomous navigation, and sensor fusion, all of which operate alongside connected software services. This expands the attack surface if systems are not designed with defense-in-depth strategies. The automotive industry’s response—ranging from recalls and software patches to new cybersecurity standards such as those developed by Auto-ISAC and the National Highway Traffic Safety Administration (NHTSA)—provides a blueprint for how other industries might respond to emerging ML system vulnerabilities.

These lessons are equally applicable to ML systems deployed in other safety-critical environments, including medical devices, industrial automation, and critical infrastructure. Securing these systems requires compartmentalizing subsystems, authenticating communications, and validating software updates to limit the ability of external actors to manipulate core system functions.

### Large-Scale Network Exploitation

Stuxnet demonstrated how targeted attacks can bridge the digital and physical worlds, causing real-world damage by exploiting software vulnerabilities in industrial control systems. While the attack did not involve machine learning, it revealed the strategic risks posed by supply chain vulnerabilities, zero-day exploits, and air-gap bypass techniques.

As machine learning systems become embedded in cyber-physical systems such as manufacturing, energy, and robotics, they inherit similar risks. Attacks that compromise ML-based controllers or decision-making algorithms could result in physical consequences, ranging from production downtime to equipment damage or safety failures. Stuxnet’s sophistication serves as a reminder that state-sponsored or highly resourced attackers may target ML systems for strategic advantage, especially as these systems take on more critical decision-making roles.

### Toward Secure Design

Collectively, these incidents illustrate that security must be designed into machine learning systems from the outset. This includes:

* **Model-level protections** to defend against attacks such as model theft, adversarial manipulation, and data leakage.
* **Data pipeline security** to ensure the confidentiality, integrity, and governance of training and inference data across cloud, edge, and embedded environments.
* **System-level isolation and access control** to prevent external interfaces from compromising ML model execution or manipulating safety-critical outputs.
* **Secure deployment and update mechanisms** to protect ML models, data pipelines, and runtime environments from tampering or exploitation.
* **Monitoring, incident response, and recovery processes** to detect and mitigate breaches in distributed and dynamic ML system deployments.

These historical incidents frame three key priorities for securing machine learning systems: protecting device-level resources, isolating critical system functions, and defending against large-scale exploitation. The rest of this chapter builds on these priorities, beginning with a closer examination of threats specific to machine learning models and data. We then expand the discussion to include hardware-level vulnerabilities and the special considerations of embedded ML systems. Finally, we explore defensive strategies, including privacy-preserving techniques, secure hardware mechanisms, and system-level design principles. Together, these elements form a foundation for building secure and privacy-preserving ML systems that can withstand both known and emerging threats.

## ML Model Security

Building on the lessons from historical security incidents, we now turn to the threats that are specific to machine learning models themselves. While device-level vulnerabilities and system-level weaknesses expose the broader ML system, the models at the heart of these systems present their own unique attack surfaces. If left unprotected, these models can be exploited in ways that compromise their integrity, performance, and trustworthiness.
...
Machine learning models can become targets of theft when attackers gain unauthorized access to their internal representations, trained parameters, or the sensitive data they encode [@ateniese2015hacking]. This threat extends across various deployment scenarios, including public APIs, cloud-hosted services, on-device inference engines, and shared model repositories. System-level factors such as exposed interfaces, insecure model serialization, and insufficient access controls increase the risk of model theft.
...
Data poisoning attacks occur when an adversary manipulates the training data used to develop a machine learning model, resulting in compromised model behavior [@biggio2012poisoning]. By injecting misleading or malicious data points into the training set, attackers can influence the model to make incorrect predictions, degrade its overall performance, or introduce hidden vulnerabilities that can be exploited after deployment.

Poisoning is particularly concerning in applications where models retrain on data collected from external sources, such as user interactions, crowdsourced annotations, or online scraping. In these cases, attackers can inject poisoned data without needing direct access to the training infrastructure. Even in more controlled environments, poisoning may occur through compromised data storage, insider manipulation, or insecure data transfer processes.
...
Researchers have identified several types of data poisoning attacks, each with different goals and impact scopes. Availability attacks broadly degrade model performance, making the model unreliable across all tasks. Targeted attacks focus on manipulating a specific class or input without affecting overall accuracy. Backdoor attacks introduce hidden triggers that activate malicious behavior only when specific patterns appear in the input. Subpopulation attacks target a particular subset of data with shared characteristics, making the attack harder to detect.

One notable real-world demonstration targeted Perspective, an online toxicity classification model [@hosseini2017deceiving]. Researchers showed that by injecting synthetically generated toxic comments with slight misspellings and grammatical errors into the model’s training data, they could degrade its ability to detect harmful content. After retraining on the poisoned data, the model’s false negative rate increased significantly, allowing offensive comments to bypass detection. This case highlights the risks of data poisoning in content moderation systems, where attackers can exploit feedback loops or user-generated data to weaken model effectiveness over time.

Defending against data poisoning requires securing the entire data pipeline, from data collection and storage to labeling and training. This includes implementing data validation checks, monitoring for unusual patterns, and applying robust learning techniques that reduce sensitivity to mislabeled or manipulated examples. While no defense is perfect, proactive data governance and continuous monitoring are essential for reducing the risk of poisoning attacks in real-world machine learning deployments.

Adversarial attacks exploit the way machine learning models generalize from data by introducing carefully crafted inputs designed to mislead the model during inference [@parrish2023adversarial]. These inputs, known as adversarial examples, often involve small, imperceptible perturbations to legitimate data. While such perturbations may not affect human perception, they can cause models to produce incorrect or even harmful outputs.

These attacks pose significant risks in safety-critical systems. For example, researchers have demonstrated that modifying a few pixels in an image can cause facial recognition systems to misidentify individuals or allow unauthorized access. In generative AI systems, carefully crafted text prompts can induce harmful or unintended outputs, including unsafe or misleading content generation [@ramesh2021zero; @rombach2022highresolution].
...
This formulation helps explain why small, carefully crafted changes can mislead even well-performing models. It also provides the basis for many practical attack methods, such as the Fast Gradient Sign Method (FGSM) and Projected Gradient Descent (PGD), which use gradients of the loss function to efficiently compute such perturbations.
...
Adversarial attacks highlight the need for robust defenses that go beyond improving model accuracy. Defending against these attacks requires a combination of adversarial training, input validation, and runtime monitoring. However, these defenses often involve trade-offs in computational cost, model performance, and deployment complexity, which must be carefully balanced in production systems.
...
A real-world scenario where this could be devastating would be in the healthcare industry. ML systems routinely process highly sensitive patient data to help diagnose, plan treatment, and forecast outcomes. A bug in the system's hardware could lead to the unauthorized disclosure of personal health information, violating patient privacy and contravening strict regulatory standards like the [Health Insurance Portability and Accountability Act (HIPAA)](https://www.cdc.gov/phlp/php/resources/health-insurance-portability-and-accountability-act-of-1996-hipaa.html) in the United States.
...
@fig-encryption3 shows another example but where the password is entirely incorrect (`0x30, 0x30, 0x30, 0x30, 0x30`), unlike the previous example with the first three bytes correct. Here, the device identifies the mismatch immediately after processing the first byte and halts further processing. This is reflected in the power consumption profile, where the blue line exhibits a sharp jump following the first byte, indicating the device's early termination of authentication.

![Power consumption profile of the device when an entirely incorrect password (0x30, 0x30, 0x30, 0x30, 0x30) is entered. The blue line shows a sharp jump after processing the first byte, indicating that the device has halted the authentication process. Source: [Colin O'Flynn.](https://www.youtube.com/watch?v=2iDLfuEBcs8)](images/png/Power_analysis_of_an_encryption_device_with_a_wrong_password.png){#fig-encryption3}

The example above demonstrates how information about the encryption process and the secret key can be inferred by analyzing different inputs and brute-force testing variations of each password byte, effectively 'eavesdropping' on the device's operations. For a more detailed explanation, watch @vid-powerattack below.
...
* **Pacemakers**: Interface vulnerabilities were discovered in some [pacemakers](https://www.moph.gov.lb/userfiles/files/Medical%20Devices/Medical%20Devices%20Recalls%202021/17-05-2021/AssurityandEndurity.pdf) that could allow attackers to manipulate cardiac functions if exploited. This presents a potentially life-threatening scenario.

* **Smart Lightbulbs**: A researcher found he could access unencrypted data from smart lightbulbs via a debug interface, including WiFi credentials, allowing him to gain access to the connected network [@greengard2021internet].
...
### Case Study: Hardware Security Wake-Up Call
...
Handling personal and sensitive data securely and ethically is critical as machine learning permeates devices like smartphones, wearables, and smart home appliances. For medical hardware, handling data securely and ethically is further required by law through [HIPPA](https://www.hhs.gov/hipaa/for-professionals/privacy/laws-regulations/index.html). These embedded ML systems pose unique privacy risks, given their intimate proximity to users' lives.
...
* [CCPA](https://oag.ca.gov/privacy/ccpa), which applies in California, protects consumer data privacy through provisions like required disclosures and opt-out rights—ioT gadgets like smart speakers and fitness trackers Californians use likely to fall under its scope.
...
:::{#exr-dptf .callout-caution collapse="true" title="Differential Privacy - TensorFlow Privacy"}
...
:::{#exr-he .callout-caution collapse="true" title="Homomorphic Encryption"}
...
+-------------------------------+--------------------------------------------------------------------------+----------------------------------------------------------------------------+
| Machine Unlearning            | - Enables selective removal of data influence from models                | - May degrade model performance on related tasks                           |
|                               | - Useful for compliance with privacy regulations                         | - Implementation complexity in large-scale models                          |
|                               | - Prevents unintended retention of adversarial or outdated data          | - Risk of incomplete or ineffective unlearning                             |
+-------------------------------+--------------------------------------------------------------------------+----------------------------------------------------------------------------+
