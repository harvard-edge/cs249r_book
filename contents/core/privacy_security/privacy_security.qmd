...
![**Stuxnet**: Targets plcs by exploiting windows and siemens software vulnerabilities, highlighting the intersection of digital malware and physical infrastructure security. @fig-stuxnet](images/png/stuxnet.png){#fig-stuxnet width=90%}
...
: **Threat Landscape**: Machine learning systems face diverse threats throughout their lifecycle, ranging from data manipulation during training to model theft post-deployment. The table categorizes these threats by lifecycle stage and attack vector, clarifying how vulnerabilities manifest and enabling targeted mitigation strategies. {#tbl-threats-models-summary .striped .hover}
...
: **Hardware Threat Landscape**: Machine learning systems face diverse hardware threats ranging from intrinsic design flaws to physical attacks and supply chain vulnerabilities. Understanding these threats—and their relevance to ML hardware—is critical for building secure and trustworthy AI deployments. {#tbl-threat_types .striped .hover}
...
::: {.content-visible when-format="html"}
::: {#lst-openai-api .callout-important title="**Secure API Key Loading**: Retrieves the OpenAI API key from an environment variable to prevent hardcoding sensitive information in source code, enhancing security during model inference. *Source: IEEE Spectrum*'s GPT-4 model. The API key is retrieved from an environment variable to avoid hardcoding sensitive information in the source code."}
```{.python}
import openai
import os
...
#| lst-cap: "**Secure API Key Loading**: Retrieves the OpenAI API key from an environment variable to prevent hardcoding sensitive information in source code, enhancing security during model inference. *Source: IEEE Spectrum*"
#| eval: false
import openai
import os
...
![**Power Traces**: Cryptographic computations reveal subtle, data-dependent variations in power consumption that reflect internal states during specific operations.](images/png/side-effects-binary.png){#fig-side-channel-curves width=80%}
...
## Summary {#sec-security-privacy-summary-120c}
