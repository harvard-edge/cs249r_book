...
Box/.style={inner xsep=2pt,inner ysep=6pt,
    ...
    align=flush center,
    text width=33mm,
    minimum width=33mm, minimum height=11mm
  },
...
%
\node[Box4,above=0.8of SB1](B0){Start: Identify Threat Type};
\foreach \x in{,S,R}{
  \foreach \i in{1,2,3}{
\pgfmathtruncatemacro{\newI}{\i + 1}
...
![Fault-injection demonstrated with assembly code. Source: @breier2018deeplaser.](images/png/Fault-injection_demonstrated_with_assembly_code.png){#fig-injection width=75%}
...
Box2/.style={Box,  node distance=3.2,draw=BrownLine,fill=BrownL},
Box3/.style={Box,  draw=VioletLine,fill=VioletL2},
Box4/.style={Box,node distance=0.9,  draw=BlueLine,fill=BlueL}
}

\node[Box](B1){Trusted Execution Environments};
\node[Box,below=of B1](B2){Secure Boot};
\node[Box,below=4of B2](B3){Hardware Security Modules};
\node[Box,below=of B3](B4){Physical Unclonable Functions};
%
%%%
\node[Box3,right=1.85of B1](2B1){System Integrity Checks};
 \node[Box3,below=of 2B1](2B2){Runtime Input Validation};
\node[Box3,below=of 2B2](2B3){Runtime Output Monitoring};
\node[Box3,below=of 2B3](2B4){Incident Response \& Recovery};
%
\node[Box2,right=1.85 of 2B1](3B1){Model Encryption \& Serialization};
 \node[Box2,below=of 3B1](3B2){Secure Model Design};
\path[](3B1)|-coordinate(S)(B4);
\node[Box2](3B3)at(S){Secure Deployment \& Access};
%
\node[Box4,above right=0.6 and 1.3of 3B2](4B1){Differential Privacy};
\node[Box4,below right=0.6 and 1.3of 3B2](4B2){Federated Learning};
\node[Box4,above=of 4B1](5B1){Homomorphic Encryption};
\node[Box4,below=of 4B2](5B2){Synthetic Data Generation};
%
\draw[Line,-latex](B1)--(2B1);
\draw[Line,-latex](B2)--++(2.6,0)|-(2B1);
\draw[Line,-latex](B3)--++(2.6,0)|-(3B3);
\draw[Line,-latex](B4)--(3B3);
%
\draw[Line,-latex](2B1)--(3B1);
\draw[Line,-latex](2B2)--++(2.6,0)|-(3B2);
\draw[Line,-latex](2B3)--++(2.6,0)|-(3B2);
%
\draw[Line,-latex](3B2)|-(4B1);
\draw[Line,-latex](3B2)|-(4B2);
\draw[Line,-latex](4B1)--(5B1);
\draw[Line,-latex](4B2)--(5B2);
%
\scoped[on background layer]
\node[draw=BackLine,inner xsep=6mm,inner ysep=8mm,
...
%
\scoped[on background layer]
\node[draw=BlueD,inner xsep=6mm,inner ysep=8mm,
yshift=2.5mm,fill=cyan!5,fit=(2B1)(2B4),line width=0.75pt](BB2){};
\node[below=6pt of  BB2.north,inner sep=0pt,
anchor=north]{\textbf{System-Level Security}};
%
\scoped[on background layer]
\node[draw=GreenD,inner xsep=4mm,inner ysep=8mm,
yshift=2.5mm,fill=green!5,fit=(3B1)(3B3),line width=0.75pt](BB3){};
\node[below=6pt of  BB3.north,inner sep=0pt,
anchor=north]{\textbf{Model-Level Security}};
%
\scoped[on background layer]
\node[draw=RedLine,inner xsep=6mm,inner ysep=14.2mm,
yshift=1.9mm,fill=magenta!4,fit=(5B1)(5B2),line width=0.75pt](BB4){};
...
\node[Box3,below=of B6](B7){Success};
\node[Box,below=of B7](B8){Boot abort};
%
...
Box/.style={inner xsep=2pt,inner ysep=6pt,
    ...
    align=flush center,
    text width=38mm,
    minimum width=38mm, minimum height=15.5mm
  },
Box2/.style={Box,draw=RedLine,fill=RedL},
Box3/.style={Box,  draw=VioletLine,fill=VioletL2, text width=42mm,
    minimum width=42mm, minimum height=8mm},
...
\node[Box,right=of B3](B4){Hardware Layer: TEEs, Secure Boot, PUFs};
 %
\node[Box,above=0.7 of B2](B22){Select Defenses Across the Stack};
\node[Box,above=0.7 of B22](B23){Assess Threat Model \& Deployment Context};
%
\node[Box,below=0.7 of B4](B44){Plan for Runtime Adaptation and Recovery};
\node[Box,below=0.7  of B44](B45){Rollback, Isolation, Incident Response};
\node[Box,left=of B45](B46){Monitoring, Logging, Alerting};
\node[Box,right=of B45](B47){Design Feedback Loop};
\draw[Line](B23)--(B22);
\draw[Line](B22)--(B2);
\draw[Line](B22)-|(B1);
\draw[Line](B22)-|(B3);
