...
![**Benchmark Workflow**: Illustrates a typical AI benchmark’s structured evaluation pipeline, showing how task definition, dataset selection, model selection, and evaluation interconnect to ensure systematic and reproducible performance assessment. this workflow enables fair comparison of different AI models and systems by standardizing the evaluation process.](images/png/benchmarking_components.png){#fig-benchmark-components }
...
![**Benchmarking Layers**: The hierarchical granularity of ML system benchmarking, ranging from end-to-end application performance to individual model efficiency, enabling targeted performance analysis and optimization. this layered approach allows practitioners to pinpoint bottlenecks and improve overall system effectiveness by focusing on specific components.](images/png/end2end.png){#fig-granularity}
...
**Benchmarking Granularity Levels**: The distinct scopes of micro-, macro-, and end-to-end benchmarks, revealing how each approach targets different stages of ML system development and identifies unique performance bottlenecks—from individual operations to full pipeline efficiency. this table clarifies that selecting the appropriate benchmarking granularity is crucial for effectively diagnosing and optimizing ML systems throughout their lifecycle. {#tbl-benchmark-comparison .striped .hover}
...
![**MLPerf Training Progress**: Performance gains on standardized machine learning training benchmarks have consistently exceeded moore’s law, demonstrating the impact of systems-level optimizations on ML computing. these trends highlight how focused measurement and iterative improvement drive rapid advancements in ML training efficiency. *source: @tschand2024mlperf.*](images/png/mlperf_training_06-12-2024.png){#fig-mlperf-training-improve}
...
**Inference Evaluation Metrics**: Key performance indicators—latency, throughput, and resource usage—used to quantify the efficiency and scalability of deployed machine learning systems and guide optimization efforts. understanding these metrics and their interdependencies is crucial for selecting appropriate hardware and software configurations for specific application requirements. {#tbl-inference-metrics .striped .hover}
...
**Power Consumption Spectrum**: The wide range of power demands across different machine learning deployment categories, from microwatt-scale TinyML devices to milliwatt-scale microcontrollers, highlighting the challenges in establishing standardized energy efficiency benchmarks. {#tbl-power .striped .hover}
...
![**MLPerf Energy Efficiency Trends**: Consistent improvements in energy efficiency (samples/watt) across different mlperf inference system scales (datacenter, edge, tiny) with each successive benchmark version, demonstrating ongoing progress in hardware and software optimization for AI. this figure highlights how standardized measurement protocols enable meaningful comparisons of energy efficiency gains across diverse AI systems and deployment scenarios. *source: @tschand2024mlperf.*](images/png/power_trends.png){#fig-power-trends}
...
![**Hardware-Dependent Performance**: The variability in model accuracy across different hardware platforms (CPU, GPU, edgetpu, DSP), demonstrating how a model’s efficiency is not solely determined by its architecture but also by its compatibility with underlying hardware. this highlights the “hardware lottery” effect, where models achieving high accuracy on one platform may underperform on others, influencing benchmarking results and model selection. *source: @chu2021discovering.*](images/png/hardware_lottery.png){#fig-hw-lottery}
...
![**Performance Requirements**: The vast range of data rates and computation time demands across emerging scientific applications, highlighting the need for specialized benchmarks beyond traditional accuracy metrics. *source: [@duarte2022fastmlsciencebenchmarksaccelerating].*](images/png/sciml_graph.png){#fig-sciml-graph width="65%"}
...
```
**AI Development Paradigms**: Highlights the contrasting approaches of model-centric and data-centric AI, demonstrating how performance gains can result from either refining model architectures or systematically improving dataset quality and representativeness. understanding this distinction is crucial because modern AI systems increasingly show that data quality often presents a greater opportunity for performance improvement than model complexity alone.
:::
...
![**Dataset Saturation**: How AI systems have surpassed human performance across diverse tasks, indicating that benchmark datasets may no longer accurately reflect genuine progress in AI capabilities. this trend highlights the need for more challenging and representative datasets to drive continued advancement and avoid misleading evaluations. *source: @kiela2021dynabench*](images/png/dynabench.png){#fig-dataset-saturation}
