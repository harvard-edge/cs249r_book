...
The evolution of benchmarks in computing illustrates how systematic performance measurement has shaped technological progress. During the 1960s and 1970s, when mainframe computers dominated the computing landscape, performance benchmarks focused primarily on fundamental computational tasks. The [Whetstone benchmark](https://en.wikipedia.org/wiki/Whetstone_(benchmark))[^fn-whetstone], introduced in 1972 to measure floating-point arithmetic performance, became a definitive standard that demonstrated how systematic testing could drive improvements in computer architecture.  

[^fn-whetstone]: Introduced in 1972, the Whetstone benchmark was one of the first synthetic benchmarks designed to measure floating-point arithmetic performance, influencing early computer architecture improvements.  
...
Historically, AI progress has been model-centric, focusing on architectural improvements and optimization techniques. However, modern AI performance is increasingly constrained by data quality rather than just model design. Data benchmarking is emerging as a crucial field, ensuring that AI models are trained on high-quality, diverse, and unbiased datasets rather than simply larger ones.  

The shift toward data-centric AI acknowledges that better data—not just better models—leads to more robust and trustworthy AI. Recent efforts such as [DataPerf](https://www.dataperf.org/) and [DataComp](https://www.datacomp.ai/) evaluate how dataset improvements impact model performance. For instance, DataComp [@gadre2024datacomp] found that models trained on a carefully filtered 30% subset of data outperformed models trained on the full dataset, suggesting that curation matters more than sheer volume [@northcutt2021pervasive].  

One major challenge in data benchmarking is dataset saturation. Some benchmarks, such as ImageNet, have seen models achieve near-perfect accuracy, raising the question: *Are we really improving AI, or are we just overfitting to existing test sets?* Similarly, models trained on static datasets may struggle with distribution shifts—real-world changes that aren't reflected in outdated training data. This has led to new benchmarking approaches, such as [Dynabench](https://dynabench.org/) [@kiela2021dynabench], which continuously updates test data based on model performance to prevent overfitting.  
