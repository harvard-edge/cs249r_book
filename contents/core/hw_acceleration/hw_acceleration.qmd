...
Tiling addresses this problem by ensuring that smaller portions of matrices are loaded into fast memory, reused efficiently, and only written back to main memory when necessary. This technique is especially crucial in AI accelerators, where memory accesses dominate execution time. Figure @fig-tiling-diagram shows the intuition behind such an approach for matrix multiplication.
...
\node[below left=2 of AM.north east]{A matrix};
 \end{tikzpicture}
```
Example of how matrix multiplication can be tiled for better paralellization and memory accesses.
:::
