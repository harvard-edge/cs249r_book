...
The sheer number of possible mapping configurations makes exhaustive search infeasible. Even a single layer in a model can have thousands of ways to order computation loops, hundreds of possible parallelization strategies, and exponentially growing memory placement choices across different levels of the hierarchy. The result is a combinatorial explosion, where small variations in mapping choices can lead to substantial differences in execution efficiency, memory traffic, and energy consumption.

Given the impracticality of brute-force exploration, structured optimization approaches are necessary to efficiently navigate this space. Instead of blindly evaluating every possibility, modern AI accelerators rely on a combination of heuristic, analytical, and machine learning-based techniques to guide the search toward high-performance configurations. These strategies aim to balance computational throughput, minimize memory bottlenecks, and maximize resource utilization while adhering to the constraints imposed by hardware architectures.

Different AI accelerators demand different mapping approaches based on their execution models. GPUs, for example, rely on SIMT (Single Instruction, Multiple Threads) execution, requiring mappings that maximize warp-level parallelism while minimizing memory divergence. TPUs, in contrast, employ systolic arrays, favoring structured mappings that optimize data movement for matrix operations. Custom ASICs introduce even more specialized constraints, often requiring tailored dataflow architectures that align with specific workload characteristics.

To address these challenges, optimization techniques take different forms. Some rely on heuristics, where simple yet effective rules—such as loop transformations and greedy allocation methods—help find reasonable mappings without exhaustive search. Others leverage analytical models to estimate the trade-offs between computation, memory access, and data movement, allowing for informed decision-making when selecting mapping strategies. More recently, machine learning-driven approaches, including reinforcement learning and cost-model-based search, have emerged as powerful tools for optimizing mapping decisions dynamically based on workload characteristics and hardware constraints.

### Core Mapping Optimizations
