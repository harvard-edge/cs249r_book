...
## Overview {#sec-ai-acceleration-overview-a9d1}
...
## Hardware Evolution {#sec-ai-acceleration-hardware-evolution-080b}
...
### Specialized Computing Expansion {#sec-ai-acceleration-specialized-computing-expansion-7d19}
...
### ML in Computational Domains {#sec-ai-acceleration-ml-computational-domains-c641}

Machine learning has emerged as one of the most computationally demanding fields, demonstrating the need for dedicated hardware that targets its unique characteristics. Domain-specific architectures, previously developed for video codecs or other specialized tasks, have now expanded to meet the challenges posed by ML workloads. These specialized designs optimize the execution of dense matrix operations and manage data movement efficiently, a necessity given the inherent memory bandwidth[^fn-memory_bandwidth] limitations.

[^fn-memory_bandwidth]: **Memory Bandwidth**: The rate at which data can be read from or written to memory by a processor, influencing performance in data-intensive operations.
...
### Application-Specific Accelerators {#sec-ai-acceleration-applicationspecific-accelerators-cdaf}
...
::: {.callout-quiz-question #quiz-question-sec-ai-acceleration-hardware-evolution-080b}

1. What is a primary trade-off when designing specialized hardware accelerators for machine learning?
   a) Increased flexibility in handling diverse workloads
   b) Higher power consumption compared to general-purpose processors
   c) Reduced silicon area utilization
   d) Complexity in programming and integration

2. Explain how the historical evolution of hardware specialization has influenced the design of modern ML accelerators.

3. The integration of specialized hardware accelerators into ML systems requires alignment with ______ frameworks like TensorFlow and PyTorch.

4. True or False: General-purpose processors are more efficient than specialized hardware for executing machine learning workloads.

See Answer \ref{quiz-answer-sec-ai-acceleration-hardware-evolution-080b}.

:::

## AI Compute Primitives {#sec-ai-acceleration-ai-compute-primitives-6854}
...
::: {#lst-dense_layer_def .callout-important title="Declarative creation of dense layer"}
```{.python}
...
```{python}
#| lst-label: lst-dense_layer_def
#| lst-cap: Declarative creation of dense layer
#| eval: false
...
::: {.content-visible when-format="html"}
::: {#lst-dense_expansion .callout-important title="Breaking down layer computation into primitives"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-dense_expansion
#| lst-cap: Breaking down layer computation into primitives
#| eval: false
...
::: {.content-visible when-format="html"}
::: {#lst-loop_level_dense .callout-important title="Processor-level nested loop computation"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-loop_level_dense
#| lst-cap: Processor-level nested loop computation
#| eval: false
...
### Vector Operations {#sec-ai-acceleration-vector-operations-5558}
...
#### Framework-Hardware Execution {#sec-ai-acceleration-frameworkhardware-execution-ba91}
...
::: {.content-visible when-format="html"}
::: {#lst-linear_layer_highlevel .callout-important title="Framework Level: What ML developers write"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-linear_layer_highlevel
#| lst-cap: "Framework Level: What ML developers write"
#| eval: false
...
::: {.content-visible when-format="html"}
::: {#lst-linear_math_internal .callout-important title="Internal mathematical representation of a linear layer"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-linear_math_internal
#| lst-cap: Internal mathematical representation of a linear layer
#| eval: false
...
::: {.content-visible when-format="html"}
::: {#lst-loop_linear_layer .callout-important title="Loop-based execution of a linear layer"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-loop_linear_layer
#| lst-cap: Loop-based execution of a linear layer
#| eval: false
...
#### Sequential Scalar Execution {#sec-ai-acceleration-sequential-scalar-execution-886f}
...
#### Parallel Vector Execution {#sec-ai-acceleration-parallel-vector-execution-d05a}

Vector processing units transform this execution pattern by operating on multiple data elements simultaneously. As shown in @lst-riscv_vector_mac, the RISC-V assembly code demonstrates modern vector processing.

::: {.content-visible when-format="html"}
::: {#lst-riscv_vector_mac .callout-important title="RISC-V vectorized multiply-accumulate loop"}
```{.c}
...
::: {.content-visible when-format="pdf"}
```{c}
#| lst-label: lst-riscv_vector_mac
#| lst-cap: RISC-V vectorized multiply-accumulate loop
#| eval: false
...
To clarify how vector instructions map to common deep learning patterns, @tbl-vector introduces key vector operations and their typical applications in neural network computation. These operations, such as reduction, gather, scatter, and masked operations, are frequently encountered in layers like pooling, embedding lookups, and attention mechanisms. Understanding this terminology is essential for interpreting how low-level vector hardware accelerates high-level machine learning workloads.
...
#### Vector Processing History {#sec-ai-acceleration-vector-processing-history-429e}
...
### Matrix Operations {#sec-ai-acceleration-matrix-operations-9bc1}
...
#### Matrix Operations in NNs {#sec-ai-acceleration-matrix-operations-nns-388d}
...
::: {.content-visible when-format="html"}
::: {#lst-linear_matrix_hierarchy .callout-important title="Framework Level: What ML developers write"}
```{.python}
layer = nn.Linear(256, 512)  # Layer transforms 256 inputs to
                             # 512 outputs
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-linear_matrix_hierarchy
#| lst-cap: "Framework Level: What ML developers write"
#| eval: false
...
#### Matrix Computation Types in NNs {#sec-ai-acceleration-matrix-computation-types-nns-5343}

Matrix operations appear consistently across modern neural architectures, as illustrated in @lst-matrix_patterns.

::: {.content-visible when-format="html"}
::: {#lst-matrix_patterns .callout-important title="Linear Layers – Direct matrix multiply"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-matrix_patterns
#| lst-cap: Linear Layers – Direct matrix multiply
#| eval: false
...
#### Matrix Operations Hardware Acceleration {#sec-ai-acceleration-matrix-operations-hardware-acceleration-f868}
...
::: {.content-visible when-format="html"}
::: {#lst-matrix_unit .callout-important title="Matrix unit operation for block-wise computation in hardware"}
```{.c}
...
::: {.content-visible when-format="pdf"}
```{c}
#| lst-label: lst-matrix_unit
#| lst-cap: Matrix unit operation for block-wise computation in hardware
#| eval: false
...
### Special Function Units {#sec-ai-acceleration-special-function-units-f296}
...
#### Non-Linear Functions {#sec-ai-acceleration-nonlinear-functions-5dc0}

Non-linear functions play a fundamental role in machine learning by enabling neural networks to model complex relationships [@Goodfellow2016]. @lst-nonlinear_layer illustrates a typical neural network layer sequence.

::: {.content-visible when-format="html"}
::: {#lst-nonlinear_layer .callout-important title="Typical layer sequence with non-linear operations"}
```{.python}
layer = nn.Sequential(
    nn.Linear(256, 512),
    nn.ReLU(),
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-nonlinear_layer
#| lst-cap: Typical layer sequence with non-linear operations
#| eval: false
...
::: {.content-visible when-format="html"}
::: {#lst-nonlinear_math .callout-important title="Mathematical operations from non-linear layer sequence"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-nonlinear_math
#| lst-cap: Mathematical operations from non-linear layer sequence
#| eval: false
...
#### Non-Linear Functions Implementation {#sec-ai-acceleration-nonlinear-functions-implementation-e79e}
...
::: {.content-visible when-format="html"}
::: {#lst-traditional_overhead .callout-important title="Traditional implementation overhead for ReLU and BatchNorm"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-traditional_overhead
#| lst-cap: Traditional implementation overhead for ReLU and BatchNorm
#| eval: false
...
#### Hardware Acceleration {#sec-ai-acceleration-hardware-acceleration-acbd}
...
::: {.content-visible when-format="html"}
::: {#lst-sfu_vector_ops .callout-important title="Hardware-accelerated non-linear vector operations"}
```{.c}
...
::: {.content-visible when-format="pdf"}
```{c}
#| lst-label: lst-sfu_vector_ops
#| lst-cap: Hardware-accelerated non-linear vector operations
#| eval: false
...
#### SFUs History {#sec-ai-acceleration-sfus-history-1674}
...
### Compute Units and Execution Models {#sec-ai-acceleration-compute-units-execution-models-998a}
...
#### Primitive-Execution Unit Mapping {#sec-ai-acceleration-primitiveexecution-unit-mapping-99f4}
...
#### SIMD to SIMT Transition {#sec-ai-acceleration-simd-simt-transition-f3fc}
...
::: {.content-visible when-format="html"}
::: {#lst-arm_sve_vector .callout-important title="Vector operation implementation using ARM SVE"}
```{.c}
...
::: {.content-visible when-format="pdf"}
```{c}
#| lst-label: lst-arm_sve_vector
#| lst-cap: Vector operation implementation using ARM SVE
#| eval: false
...
::: {.content-visible when-format="html"}
::: {#lst-cuda_simt .callout-important title="CUDA kernel for SIMT execution"}
```{.c}
#| eval: false
...
::: {.content-visible when-format="pdf"}
```{c}
#| lst-label: lst-cuda_simt
#| lst-cap: CUDA kernel for SIMT execution
#| eval: false
...
#### Tensor Cores {#sec-ai-acceleration-tensor-cores-3e5e}
...
Tensor cores, implemented in architectures such as NVIDIA's Ampere GPUs, provide an example of this approach. They expose matrix computation capabilities through specialized instructions, such as the tensor core operation shown in @lst-tensor_core_op on the NVIDIA A100 GPU.

::: {.content-visible when-format="html"}
::: {#lst-tensor_core_op .callout-important title="Tensor Core operation on NVIDIA A100 GPU"}
```{.c}
...
::: {.content-visible when-format="pdf"}
```{c}
#| lst-label: lst-tensor_core_op
#| lst-cap: Tensor Core operation on NVIDIA A100 GPU
#| eval: false
...
#### Processing Elements {#sec-ai-acceleration-processing-elements-364d}
...
#### Systolic Arrays {#sec-ai-acceleration-systolic-arrays-125b}
...
\end{tikzpicture}}
```
Data flow movement in a systolic array.
:::
...
#### Numerics in AI Acceleration {#sec-ai-acceleration-numerics-ai-acceleration-6bec}
...
##### Precision Trade-offs {#sec-ai-acceleration-precision-tradeoffs-ae42}
...
##### Mixed-Precision Computing {#sec-ai-acceleration-mixedprecision-computing-3bfc}
...
#### Architectural Integration {#sec-ai-acceleration-architectural-integration-a52f}
...
::: {.callout-quiz-question #quiz-question-sec-ai-acceleration-ai-compute-primitives-6854}

1. Which of the following is NOT a characteristic of AI compute primitives?
   a) They are optimized for high-throughput execution.
   b) They require irregular control flow for efficiency.
   c) They emphasize data-level parallelism.
   d) They support predictable data reuse.

2. Explain why vector operations are essential for efficient neural network execution.

3. True or False: Systolic arrays are optimized for irregular data flow patterns in matrix operations.

See Answer \ref{quiz-answer-sec-ai-acceleration-ai-compute-primitives-6854}.

:::

## AI Memory Systems {#sec-ai-acceleration-ai-memory-systems-4d27}
...
### AI Memory Wall {#sec-ai-acceleration-ai-memory-wall-7ac6}
...
#### Compute-Memory Imbalance {#sec-ai-acceleration-computememory-imbalance-fc28}
...
#### Memory-Intensive ML Workloads {#sec-ai-acceleration-memoryintensive-ml-workloads-05c3}
...
#### Irregular Memory Access {#sec-ai-acceleration-irregular-memory-access-d7aa}
...
#| fig-cap: "Model growth (in parameters) versus memory bandwidth (in GB/s)."
#| echo: false
...
### Memory Hierarchy {#sec-ai-acceleration-memory-hierarchy-7271}
...
#### On-Chip Memory {#sec-ai-acceleration-onchip-memory-ac4c}
...
#### Off-Chip Memory {#sec-ai-acceleration-offchip-memory-c615}
...
### Host-Accelerator Communication {#sec-ai-acceleration-hostaccelerator-communication-5f6d}
...
```
Host-accelerator memory access interactions.
:::
...
#### Data Transfer Patterns {#sec-ai-acceleration-data-transfer-patterns-e4bd}
...
#### Data Transfer Mechanisms {#sec-ai-acceleration-data-transfer-mechanisms-bdb2}
...
##### PCIe Interface {#sec-ai-acceleration-pcie-interface-ceb9}
...
##### NVLink Interface {#sec-ai-acceleration-nvlink-interface-ef15}
...
##### DMA for Data Transfers {#sec-ai-acceleration-dma-data-transfers-eaa5}
...
##### Unified Memory {#sec-ai-acceleration-unified-memory-905c}
...
#### Data Transfer Overheads {#sec-ai-acceleration-data-transfer-overheads-99ab}
...
### Model Memory Pressure {#sec-ai-acceleration-model-memory-pressure-173a}
...
#### Multilayer Perceptrons {#sec-ai-acceleration-multilayer-perceptrons-6c5e}
...
#### Convolutional Neural Networks {#sec-ai-acceleration-convolutional-neural-networks-23e2}
...
#### Transformer Networks {#sec-ai-acceleration-transformer-networks-69ee}
...
### ML Accelerators Implications {#sec-ai-acceleration-ml-accelerators-implications-4381}
...
::: {.callout-quiz-question #quiz-question-sec-ai-acceleration-ai-memory-systems-4d27}

1. Which of the following best describes the 'AI Memory Wall' in machine learning accelerators?
   a) The limitation of AI models due to insufficient computational power.
   b) The gap between computational capabilities and memory bandwidth.
   c) The challenge of integrating AI models with traditional CPUs.
   d) The inefficiency of AI models caused by outdated software frameworks.

2. Explain why memory bandwidth is a critical factor in AI acceleration, despite advancements in computational power.

3. In AI systems, ______ memory is used to store frequently accessed data to minimize latency and improve performance.

4. Order the steps involved in host-accelerator data movement: Copy processing data, Instruct the processing, Execute parallel in each core, Store results.

See Answer \ref{quiz-answer-sec-ai-acceleration-ai-memory-systems-4d27}.

:::

## Neural Networks Mapping {#sec-ai-acceleration-neural-networks-mapping-445f}
...
### Computation Placement {#sec-ai-acceleration-computation-placement-3632}
...
#### Computation Placement Definition {#sec-ai-acceleration-computation-placement-definition-10e2}
...
#### Computation Placement Importance {#sec-ai-acceleration-computation-placement-importance-742b}
...
#### Effective Computation Placement {#sec-ai-acceleration-effective-computation-placement-043d}
...
### Memory Allocation {#sec-ai-acceleration-memory-allocation-be51}
...
#### Memory Allocation Definition {#sec-ai-acceleration-memory-allocation-definition-1885}
...
#### Memory Allocation Importance {#sec-ai-acceleration-memory-allocation-importance-2693}
...
#### Effective Memory Allocation {#sec-ai-acceleration-effective-memory-allocation-de72}
...
### Combinatorial Complexity {#sec-ai-acceleration-combinatorial-complexity-ab45}
...
#### Configuration Space Mapping {#sec-ai-acceleration-configuration-space-mapping-3b7e}
...
#### Computation and Execution Ordering {#sec-ai-acceleration-computation-execution-ordering-cb0c}
...
#### Processing Elements Parallelization {#sec-ai-acceleration-processing-elements-parallelization-d446}
...
#### Memory Placement and Data Movement {#sec-ai-acceleration-memory-placement-data-movement-1422}
...
#### Mapping Search Space {#sec-ai-acceleration-mapping-search-space-6002}
...
::: {.callout-quiz-question #quiz-question-sec-ai-acceleration-neural-networks-mapping-445f}

1. Which aspect of mapping in AI acceleration focuses on distributing computations across processing elements to maximize parallelism?
   a) Memory Allocation
   b) Computation Placement
   c) Data Movement
   d) Execution Scheduling

2. Explain why poor computation placement can lead to increased execution time and reduced efficiency in AI accelerators.

3. In AI acceleration, ______ refers to the assignment of machine learning computations to hardware processing units to optimize execution efficiency.

4. True or False: Effective memory allocation in AI accelerators primarily aims to minimize latency by keeping frequently accessed data close to processing elements.

5. Order the following steps involved in mapping neural network computations to AI accelerators: Data Placement, Computation Scheduling, Data Movement Timing.

See Answer \ref{quiz-answer-sec-ai-acceleration-neural-networks-mapping-445f}.

:::

## Optimization Strategies {#sec-ai-acceleration-optimization-strategies-b57c}
...
### Mapping Strategies Building Blocks {#sec-ai-acceleration-mapping-strategies-building-blocks-0315}
...
#### Data Movement Patterns {#sec-ai-acceleration-data-movement-patterns-0b92}
...
::: {.content-visible when-format="html"}
::: {#lst-matmul_data_movement .callout-important title="Matrix multiplication illustrating data movement bottlenecks"}
```{.python}

::: {.callout-quiz-question #quiz-question-sec-ai-acceleration-optimization-strategies-b57c}

1. Which mapping strategy is most effective for convolutional neural networks (CNNs) to maximize weight reuse?
   a) Output Stationary
   b) Weight Stationary
   c) Input Stationary
   d) Activation Stationary

2. Explain why temporal tiling is particularly beneficial for transformer architectures.

3. In AI accelerators, ______ strategies determine which data remains fixed in memory and which data is streamed dynamically to maximize reuse and reduce memory fetches.

4. True or False: Kernel fusion primarily benefits operations with complex data dependencies, such as matrix multiplications and convolutions.

5. Order the following mapping strategies by their suitability for CNNs, Transformers, and MLPs: Activation Stationary, Weight Stationary, Output Stationary.

See Answer \ref{quiz-answer-sec-ai-acceleration-optimization-strategies-b57c}.
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-matmul_data_movement
#| lst-cap: Matrix multiplication illustrating data movement bottlenecks
#| eval: false
...
##### Weight Stationary {#sec-ai-acceleration-weight-stationary-1ac2}
...
::: {.content-visible when-format="html"}
::: {#lst-weight_stationary .callout-important title="Weight Stationary implementation for matrix multiplication"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-weight_stationary
#| lst-cap: Weight Stationary implementation for matrix multiplication
#| eval: false
...
##### Output Stationary {#sec-ai-acceleration-output-stationary-cba7}
...
::: {.content-visible when-format="html"}
::: {#lst-output_stationary .callout-important title="Output Stationary implementation for matrix multiplication"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-output_stationary
#| lst-cap: Output Stationary implementation for matrix multiplication
#| eval: false
## - Partial sums remain in local memory
## - Weights and input activations stream through dynamically
## - Final outputs are written only once
...
##### Input Stationary {#sec-ai-acceleration-input-stationary-108c}
...
::: {.content-visible when-format="html"}
::: {#lst-input_stationary .callout-important title="Input Stationary implementation for matrix multiplication"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-input_stationary
#| lst-cap: Input Stationary implementation for matrix multiplication
#| eval: false
...
#### Memory-Aware Tensor Layouts {#sec-ai-acceleration-memoryaware-tensor-layouts-960e}
...
##### Row-Major Layout {#sec-ai-acceleration-rowmajor-layout-6d3f}
...
##### Channel-Major Layout {#sec-ai-acceleration-channelmajor-layout-dd17}
...
##### Row-Major vs Channel-Major Layouts {#sec-ai-acceleration-rowmajor-vs-channelmajor-layouts-7969}
...
#### Kernel Fusion {#sec-ai-acceleration-kernel-fusion-145d}

##### Intermediate Memory Write {#sec-ai-acceleration-intermediate-memory-write-9656}
...
::: {.content-visible when-format="html"}
::: {#lst-naive_execution .callout-important title="Naïve kernel-by-kernel execution"}
```{.python}
import torch
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-naive_execution
#| lst-cap: Naïve kernel-by-kernel execution
#| eval: false
...
##### Kernel Fusion for Memory Efficiency {#sec-ai-acceleration-kernel-fusion-memory-efficiency-fda4}
...
##### Performance Benefits and Constraints {#sec-ai-acceleration-performance-benefits-constraints-c880}
...
#### Tiling for Memory Efficiency {#sec-ai-acceleration-tiling-memory-efficiency-3894}
...
::: {.content-visible when-format="html"}
::: {#lst-naive_matmul .callout-important title="Naïve matrix multiplication without tiling"}
```{.python}
for i in range(N):
    for j in range(N):
        for k in range(N):
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-naive_matmul
#| lst-cap: Naïve matrix multiplication without tiling
#| eval: false
...
\node[below left=2 of AM.north east]{A matrix};
 \end{tikzpicture}
```
Example of how matrix multiplication can be tiled for better parallelization and memory accesses.
:::

##### Tiling Fundamentals {#sec-ai-acceleration-tiling-fundamentals-43e7}
...
::: {.content-visible when-format="html"}
::: {#lst-naive_matmul_repeat .callout-important title="Naïve matrix multiplication"}
```{.python}
for i in range(N):
    for j in range(N):
        for k in range(N):
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-naive_matmul_repeat
#| lst-cap: Naïve matrix multiplication
#| eval: false
...
##### Tiling Performance Improvement {#sec-ai-acceleration-tiling-performance-improvement-9cb8}
...
::: {.content-visible when-format="html"}
::: {#lst-tiled_matmul .callout-important title="Tiled matrix multiplication"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-tiled_matmul
#| lst-cap: Tiled matrix multiplication
#| eval: false
...
##### Tiling Methods {#sec-ai-acceleration-tiling-methods-51b3}
...
###### Spatial Tiling {#sec-ai-acceleration-spatial-tiling-9202}
...
::: {.content-visible when-format="html"}
::: {#lst-tiled_spatial .callout-important title="Tiled matrix multiplication with spatial tiling"}
```{.python}
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-tiled_spatial
#| lst-cap: Tiled matrix multiplication with spatial tiling
#| eval: false
...
###### Temporal Tiling {#sec-ai-acceleration-temporal-tiling-2248}
...
::: {.content-visible when-format="html"}
::: {#lst-loop_blocking .callout-important title="Matrix Multiplication with Temporal Tiling (Loop Blocking)"}
```{.python}
for i in range(0, N, TILE_SIZE):
    for j in range(0, N, TILE_SIZE):
        for k in range(0, N, TILE_SIZE):
...
::: {.content-visible when-format="pdf"}
```{python}
#| lst-label: lst-loop_blocking
#| lst-cap: Matrix Multiplication with Temporal Tiling (Loop Blocking)
#| eval: false
...
##### Tiling Challenges and Trade-offs {#sec-ai-acceleration-tiling-challenges-tradeoffs-35ef}
...
### Mapping Strategies Application {#sec-ai-acceleration-mapping-strategies-application-f788}
...
#### Convolutional Neural Networks {#sec-ai-acceleration-convolutional-neural-networks-5b0d}
...
#### Transformer Architectures {#sec-ai-acceleration-transformer-architectures-bab0}
...
#### Multi-Layer Perceptrons {#sec-ai-acceleration-multilayer-perceptrons-3e49}
...
### Hybrid Mapping Strategies {#sec-ai-acceleration-hybrid-mapping-strategies-371f}
...
#### Layer-Specific Mapping {#sec-ai-acceleration-layerspecific-mapping-05ed}
...
### Hybrid Strategies Hardware Implementations {#sec-ai-acceleration-hybrid-strategies-hardware-implementations-1d82}
...
## Compiler Support {#sec-ai-acceleration-compiler-support-1834}
...
### ML vs Traditional Compilers {#sec-ai-acceleration-ml-vs-traditional-compilers-e404}
...
### ML Compilation Pipeline {#sec-ai-acceleration-ml-compilation-pipeline-fdab}
...
### Graph Optimization {#sec-ai-acceleration-graph-optimization-8481}
...
#### Computation Graph Optimization {#sec-ai-acceleration-computation-graph-optimization-7b47}
...
#### AI Compilers Implementation {#sec-ai-acceleration-ai-compilers-implementation-4a86}
...
#### Graph Optimization Importance {#sec-ai-acceleration-graph-optimization-importance-96a8}
...
### Kernel Selection {#sec-ai-acceleration-kernel-selection-6b33}
...
#### Kernel Selection in AI Compilers {#sec-ai-acceleration-kernel-selection-ai-compilers-8dc2}
...
#### Kernel Selection Importance {#sec-ai-acceleration-kernel-selection-importance-bf27}
...
### Memory Planning {#sec-ai-acceleration-memory-planning-2880}
...
#### Memory Planning in AI Compilers {#sec-ai-acceleration-memory-planning-ai-compilers-e7e2}
...
#### Memory Planning Importance {#sec-ai-acceleration-memory-planning-importance-c7ad}
...
### Computation Scheduling {#sec-ai-acceleration-computation-scheduling-7c47}
...
#### Computation Scheduling in AI Compilers {#sec-ai-acceleration-computation-scheduling-ai-compilers-c456}
...
#### Computation Scheduling Importance {#sec-ai-acceleration-computation-scheduling-importance-b4f4}
...
#### Code Generation {#sec-ai-acceleration-code-generation-2ed5}
...
### Compilation-Runtime Support {#sec-ai-acceleration-compilationruntime-support-856e}
...
::: {.callout-quiz-question #quiz-question-sec-ai-acceleration-compiler-support-1834}

1. Which aspect is unique to machine learning compilers compared to traditional compilers?
   a) Instruction scheduling
   b) Graph transformations
   c) Register allocation
   d) Loop unrolling

2. Order the stages of the machine learning compilation pipeline: Graph Optimization, Kernel Selection, Memory Planning, Computation Scheduling, Code Generation.

3. Explain why graph optimization is crucial in the machine learning compilation process.

4. True or False: Kernel selection in AI compilers is primarily concerned with ensuring numerical accuracy rather than maximizing hardware utilization.

See Answer \ref{quiz-answer-sec-ai-acceleration-compiler-support-1834}.

:::

## Runtime Support {#sec-ai-acceleration-runtime-support-349c}
...
### ML vs Traditional Runtimes {#sec-ai-acceleration-ml-vs-traditional-runtimes-776e}
...
### Dynamic Kernel Execution {#sec-ai-acceleration-dynamic-kernel-execution-7835}
...
### Runtime Kernel Selection {#sec-ai-acceleration-runtime-kernel-selection-4fe6}
...
### Kernel Scheduling and Utilization {#sec-ai-acceleration-kernel-scheduling-utilization-8ca1}
...
::: {.callout-quiz-question #quiz-question-sec-ai-acceleration-runtime-support-349c}

1. Which of the following best describes the role of AI runtimes in managing kernel execution?
   a) Executing a fixed sequence of instructions
   b) Dynamically selecting and dispatching kernels based on system state
   c) Managing CPU thread execution
   d) Statically allocating memory for tensor operations

2. True or False: AI runtimes primarily focus on managing sequential execution of instructions, similar to traditional software runtimes.

3. Explain how dynamic kernel execution helps prevent performance bottlenecks in AI workloads.

4. In AI runtimes, ______ execution ensures that computations proceed without waiting for memory transfers to complete.

5. Order the steps involved in dynamic kernel scheduling for AI runtimes: Select kernel based on system state, Schedule kernel across processing units, Preload intermediate data into cache, Execute kernel.

See Answer \ref{quiz-answer-sec-ai-acceleration-runtime-support-349c}.

:::

## Multi-Chip AI Acceleration {#sec-ai-acceleration-multichip-ai-acceleration-ab50}
...
#### Chiplet-Based Architectures {#sec-ai-acceleration-chipletbased-architectures-fddf}
...
![AMD's chiplet-based architecture.](images/png/chiplets.png){#fig-AMD_chiplet_based}
...
#### Multi-GPU Systems {#sec-ai-acceleration-multigpu-systems-9c3d}
...
```
Multi-GPU architecture with NVSwitch interconnects.
:::

#### TPU Pods {#sec-ai-acceleration-tpu-pods-d8df}
...
#### Wafer-Scale AI {#sec-ai-acceleration-waferscale-ai-cb0e}
...
#### AI Systems Scaling Trajectory {#sec-ai-acceleration-ai-systems-scaling-trajectory-328b}
...
### Computation and Memory Scaling Changes {#sec-ai-acceleration-computation-memory-scaling-changes-f413}
...
#### Multi-chip Execution Mapping {#sec-ai-acceleration-multichip-execution-mapping-92ab}
...
#### Distributed Access Memory Allocation {#sec-ai-acceleration-distributed-access-memory-allocation-dff6}
...
#### Data Movement Constraints {#sec-ai-acceleration-data-movement-constraints-04e2}
...
#### Compilers and Runtimes Adaptation {#sec-ai-acceleration-compilers-runtimes-adaptation-3cd7}
...
### Execution Models Adaptation {#sec-ai-acceleration-execution-models-adaptation-41f2}
...
#### Cross-Accelerator Scheduling {#sec-ai-acceleration-crossaccelerator-scheduling-e644}
...
#### Cross-Accelerator Coordination {#sec-ai-acceleration-crossaccelerator-coordination-50c5}
...
#### Cross-Accelerator Execution Management {#sec-ai-acceleration-crossaccelerator-execution-management-2aef}
...
#### Computation Placement Adaptation {#sec-ai-acceleration-computation-placement-adaptation-359c}
...
### Navigating Multi-Chip AI Complexities {#sec-ai-acceleration-navigating-multichip-ai-complexities-dbd4}
...
::: {.callout-quiz-question #quiz-question-sec-ai-acceleration-multichip-ai-acceleration-ab50}

1. Which of the following is a primary challenge when scaling AI systems using chiplet-based architectures?
   a) Thermal dissipation
   b) Inter-chiplet communication latency
   c) Synchronization overhead
   d) Fault tolerance

2. Explain why multi-GPU systems face communication bottlenecks and how NVLink helps mitigate this issue.

3. In TPU Pods, the ______ interconnect topology enables efficient data exchange between accelerators, minimizing communication bottlenecks.

4. True or False: Wafer-scale AI processors eliminate the need for inter-chip communication, thus completely removing synchronization overhead.

5. Order the following AI scaling strategies by their complexity and challenges: Multi-GPU Systems, Chiplet-Based Architectures, Wafer-Scale AI, TPU Pods.

See Answer \ref{quiz-answer-sec-ai-acceleration-multichip-ai-acceleration-ab50}.

:::

## Conclusion {#sec-ai-acceleration-conclusion-e981}
...
## Resources {#sec-ai-acceleration-resources-fad5}
...
:::{.callout-quiz-answer #quiz-answer-sec-ai-acceleration-hardware-evolution-080b}
...
1. **What is a primary trade-off when designing specialized hardware accelerators for machine learning?**
   ...
   The correct answer is D. Complexity in programming and integration. While specialized hardware accelerators offer significant performance gains, they often come with increased complexity in programming and integration due to their tailored designs for specific tasks.

   *Learning Objective*: Understand the trade-offs involved in designing specialized hardware for ML systems.

2. **Explain how the historical evolution of hardware specialization has influenced the design of modern ML accelerators.**

   The evolution of hardware specialization has led to the development of ML accelerators by applying principles such as identifying computational bottlenecks and designing specialized circuits. This historical context informs current designs, which focus on optimizing matrix operations and memory hierarchies for ML tasks.

   *Learning Objective*: Analyze how historical trends in hardware specialization inform modern ML accelerator designs.

3. **The integration of specialized hardware accelerators into ML systems requires alignment with ______ frameworks like TensorFlow and PyTorch.**

   The answer is "software". The integration of specialized hardware accelerators into ML systems requires alignment with software frameworks like TensorFlow and PyTorch to ensure seamless deployment and operation.

   *Learning Objective*: Understand the importance of software integration in deploying specialized hardware accelerators.

4. **True or False: General-purpose processors are more efficient than specialized hardware for executing machine learning workloads.**

   False. Specialized hardware is more efficient than general-purpose processors for executing machine learning workloads because it is optimized for specific computational tasks, leading to better performance and energy efficiency.

   *Learning Objective*: Evaluate the efficiency of specialized hardware compared to general-purpose processors for ML workloads.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-acceleration-ai-compute-primitives-6854}
...
1. **Which of the following is NOT a characteristic of AI compute primitives?**
   ...
   The correct answer is B. AI compute primitives do not require irregular control flow; rather, they benefit from structured and predictable execution patterns, which allow for architectural simplifications and optimizations.

   *Learning Objective*: Understand the characteristics of AI compute primitives and their role in optimizing neural network execution.

2. **Explain why vector operations are essential for efficient neural network execution.**

   Vector operations are essential because they enable parallel processing of multiple data elements simultaneously, maximizing computational efficiency and memory bandwidth utilization. This parallelism is crucial for handling the large-scale, data-parallel computations typical in neural networks.

   *Learning Objective*: Analyze the importance of vector operations in the context of neural network execution efficiency.

3. **True or False: Systolic arrays are optimized for irregular data flow patterns in matrix operations.**

   False. Systolic arrays are optimized for continuous data flow and operand reuse, which involve structured and rhythmic data movement patterns, not irregular ones.

   *Learning Objective*: Understand the data flow characteristics of systolic arrays and their optimization for matrix operations.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-acceleration-ai-memory-systems-4d27}
...
1. **Which of the following best describes the 'AI Memory Wall' in machine learning accelerators?**
   ...
   The correct answer is B. The 'AI Memory Wall' refers to the growing disparity between computational advancements and slower memory performance, which creates bottlenecks in data-intensive applications.

   *Learning Objective*: Understand the concept of the AI Memory Wall and its implications for machine learning accelerators.

2. **Explain why memory bandwidth is a critical factor in AI acceleration, despite advancements in computational power.**

   Memory bandwidth is critical because it determines the rate at which data can be supplied to processing units. Despite high computational power, accelerators can become idle if data cannot be delivered quickly enough, leading to performance bottlenecks.

   *Learning Objective*: Analyze the role of memory bandwidth in sustaining computational efficiency in AI accelerators.

3. **In AI systems, ______ memory is used to store frequently accessed data to minimize latency and improve performance.**

   The answer is "cache". Cache memory stores frequently accessed data to minimize latency and improve performance by reducing the need for slower off-chip memory accesses.

   *Learning Objective*: Recall the role of cache memory in AI systems and its impact on performance.

4. **Order the steps involved in host-accelerator data movement: Copy processing data, Instruct the processing, Execute parallel in each core, Store results.**

   1. Copy processing data, 2. Instruct the processing, 3. Execute parallel in each core, 4. Store results. These steps outline the sequence of data movement and execution in host-accelerator interactions.

   *Learning Objective*: Understand the sequence of data movement and execution in host-accelerator interactions.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-acceleration-neural-networks-mapping-445f}
...
1. **Which aspect of mapping in AI acceleration focuses on distributing computations across processing elements to maximize parallelism?**
   ...
   The correct answer is B. Computation Placement focuses on distributing computations across processing elements to maximize parallelism and reduce idle time.

   *Learning Objective*: Understand the role of computation placement in mapping strategies for AI accelerators.

2. **Explain why poor computation placement can lead to increased execution time and reduced efficiency in AI accelerators.**

   Poor computation placement can cause workload imbalance, where some processing elements are idle while others are overloaded, leading to inefficient resource utilization. It can also increase memory traffic and latency due to excessive data movement, ultimately reducing execution efficiency.

   *Learning Objective*: Analyze the impact of computation placement on execution efficiency and resource utilization in AI accelerators.

3. **In AI acceleration, ______ refers to the assignment of machine learning computations to hardware processing units to optimize execution efficiency.**

   The answer is "mapping". Mapping in AI acceleration involves assigning computations to processing units to optimize execution efficiency, including spatial allocation, temporal scheduling, and memory-aware execution.

   *Learning Objective*: Recall the definition and components of mapping in AI acceleration.

4. **True or False: Effective memory allocation in AI accelerators primarily aims to minimize latency by keeping frequently accessed data close to processing elements.**

   True. Effective memory allocation minimizes latency and power consumption by ensuring frequently accessed data is stored in fast-access memory locations, optimizing execution efficiency.

   *Learning Objective*: Understand the importance of memory allocation in optimizing execution efficiency in AI accelerators.

5. **Order the following steps involved in mapping neural network computations to AI accelerators: Data Placement, Computation Scheduling, Data Movement Timing.**

   1. Data Placement: Allocate data across memory hierarchies to minimize latency and energy consumption. 2. Computation Scheduling: Determine the order of operations to optimize compute efficiency and memory access patterns. 3. Data Movement Timing: Manage data transfers to reduce latency and energy costs.

   *Learning Objective*: Understand the sequential process of mapping neural network computations to AI accelerators and its impact on performance.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-acceleration-optimization-strategies-b57c}
...
1. **Which mapping strategy is most effective for convolutional neural networks (CNNs) to maximize weight reuse?**
   ...
   The correct answer is B. Weight Stationary execution is most effective for CNNs because it keeps filter weights in fast memory while streaming activations, maximizing weight reuse and reducing memory bandwidth demands.

   *Learning Objective*: Understand the mapping strategy that optimizes weight reuse in CNNs.

2. **Explain why temporal tiling is particularly beneficial for transformer architectures.**

   Temporal tiling is beneficial for transformers because it structures computations to process sequence blocks efficiently, improving memory locality and reducing excessive memory transfers, which is crucial for handling long-sequence tasks.

   *Learning Objective*: Analyze the benefits of temporal tiling in transformer architectures.

3. **In AI accelerators, ______ strategies determine which data remains fixed in memory and which data is streamed dynamically to maximize reuse and reduce memory fetches.**

   The answer is "dataflow". Dataflow strategies optimize data movement by determining which data remains fixed in memory and which is streamed dynamically, maximizing reuse and minimizing redundant memory fetches.

   *Learning Objective*: Recall the role of dataflow strategies in optimizing data movement.

4. **True or False: Kernel fusion primarily benefits operations with complex data dependencies, such as matrix multiplications and convolutions.**

   False. Kernel fusion primarily benefits element-wise operations, such as ReLU and batch normalization, because these computations depend only on single elements from the input tensor, allowing them to be executed as a single fused kernel.

   *Learning Objective*: Understand the types of operations that benefit from kernel fusion.

5. **Order the following mapping strategies by their suitability for CNNs, Transformers, and MLPs: Activation Stationary, Weight Stationary, Output Stationary.**

   1. Weight Stationary (CNNs), 2. Activation Stationary (Transformers), 3. Output Stationary (MLPs). Weight Stationary is best for CNNs due to structured weight reuse, Activation Stationary suits Transformers for efficient KV-cache management, and Output Stationary optimizes accumulation in MLPs.

   *Learning Objective*: Apply mapping strategies to different AI workloads based on their computational characteristics.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-acceleration-compiler-support-1834}
...
1. **Which aspect is unique to machine learning compilers compared to traditional compilers?**
   ...
   The correct answer is B. Machine learning compilers focus on graph transformations to optimize computation graphs for tensor operations, which is not a focus for traditional compilers.

   *Learning Objective*: Understand the unique aspects of machine learning compilers compared to traditional compilers.

2. **Order the stages of the machine learning compilation pipeline: Graph Optimization, Kernel Selection, Memory Planning, Computation Scheduling, Code Generation.**

   1. Graph Optimization, 2. Kernel Selection, 3. Memory Planning, 4. Computation Scheduling, 5. Code Generation. This sequence reflects the logical progression from high-level graph restructuring to low-level code generation in the ML compilation pipeline.

   *Learning Objective*: Identify and sequence the stages of the machine learning compilation pipeline.

3. **Explain why graph optimization is crucial in the machine learning compilation process.**

   Graph optimization is crucial because it restructures computation graphs to eliminate inefficiencies, reduce redundant operations, and improve data locality. This ensures that AI accelerators operate at peak efficiency, minimizing memory stalls and maximizing parallel execution.

   *Learning Objective*: Analyze the importance of graph optimization in the ML compilation process.

4. **True or False: Kernel selection in AI compilers is primarily concerned with ensuring numerical accuracy rather than maximizing hardware utilization.**

   False. Kernel selection in AI compilers aims to maximize hardware utilization by choosing the most efficient implementation for each operation, while also considering numerical accuracy.

   *Learning Objective*: Understand the goals of kernel selection in AI compilers.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-acceleration-runtime-support-349c}
...
1. **Which of the following best describes the role of AI runtimes in managing kernel execution?**
   ...
   The correct answer is B. AI runtimes dynamically select and dispatch computation kernels based on the current system state, ensuring minimal latency and optimal resource utilization.

   *Learning Objective*: Understand the dynamic role of AI runtimes in managing kernel execution.

2. **True or False: AI runtimes primarily focus on managing sequential execution of instructions, similar to traditional software runtimes.**

   False. AI runtimes focus on massively parallel tensor execution and dynamic memory management, unlike traditional runtimes that manage sequential or multi-threaded execution.

   *Learning Objective*: Differentiate between AI and traditional runtimes in terms of execution focus.

3. **Explain how dynamic kernel execution helps prevent performance bottlenecks in AI workloads.**

   Dynamic kernel execution allows AI runtimes to adjust execution strategies in real-time, such as modifying tiling strategies or kernel selection based on current memory availability and system load. This adaptability prevents performance bottlenecks like cache thrashing or excessive memory transfers.

   *Learning Objective*: Analyze how dynamic kernel execution mitigates performance bottlenecks.

4. **In AI runtimes, ______ execution ensures that computations proceed without waiting for memory transfers to complete.**

   The answer is "asynchronous". Asynchronous execution allows computations to overlap with memory transfers, maintaining a steady flow of data and avoiding pipeline stalls.

   *Learning Objective*: Recall the concept of asynchronous execution in AI runtimes.

5. **Order the steps involved in dynamic kernel scheduling for AI runtimes: Select kernel based on system state, Schedule kernel across processing units, Preload intermediate data into cache, Execute kernel.**

   1. Select kernel based on system state, 2. Preload intermediate data into cache, 3. Schedule kernel across processing units, 4. Execute kernel. This sequence ensures that the most efficient kernel is chosen, data is ready for processing, and execution is optimized for parallelism.

   *Learning Objective*: Understand the process of dynamic kernel scheduling in AI runtimes.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-acceleration-multichip-ai-acceleration-ab50}
...
1. **Which of the following is a primary challenge when scaling AI systems using chiplet-based architectures?**
   ...
   The correct answer is B. Inter-chiplet communication latency is a primary challenge in chiplet-based architectures as it affects the efficiency of data transfer between chiplets, impacting system performance.

   *Learning Objective*: Understand the challenges associated with chiplet-based AI architectures.

2. **Explain why multi-GPU systems face communication bottlenecks and how NVLink helps mitigate this issue.**

   Multi-GPU systems face communication bottlenecks due to limited bandwidth in data exchange between GPUs. NVLink mitigates this by providing high-speed interconnects, reducing latency and improving data transfer rates, thus enhancing parallel processing efficiency.

   *Learning Objective*: Analyze the communication challenges in multi-GPU systems and the role of NVLink in addressing them.

3. **In TPU Pods, the ______ interconnect topology enables efficient data exchange between accelerators, minimizing communication bottlenecks.**

   The answer is "2D torus". The 2D torus interconnect topology in TPU Pods facilitates efficient data exchange between accelerators, minimizing communication bottlenecks and supporting large-scale parallel processing.

   *Learning Objective*: Recall the interconnect topology used in TPU Pods and its significance for scaling.

4. **True or False: Wafer-scale AI processors eliminate the need for inter-chip communication, thus completely removing synchronization overhead.**

   False. While wafer-scale AI processors reduce inter-chip communication by integrating a large number of transistors on a single wafer, they still face challenges like thermal management and fault tolerance, which can introduce synchronization overhead.

   *Learning Objective*: Understand the limitations and challenges of wafer-scale AI processors despite reduced inter-chip communication.

5. **Order the following AI scaling strategies by their complexity and challenges: Multi-GPU Systems, Chiplet-Based Architectures, Wafer-Scale AI, TPU Pods.**

   1. Chiplet-Based Architectures, 2. Multi-GPU Systems, 3. TPU Pods, 4. Wafer-Scale AI. Chiplet-based architectures start with modular scaling, multi-GPU systems introduce external interconnects, TPU Pods scale across clusters, and wafer-scale AI integrates an entire wafer, each increasing in complexity and challenges.

   *Learning Objective*: Sequence AI scaling strategies by complexity and associated challenges.
