...
## Self-Check Answers {#sec-ai-acceleration-selfcheck-answers-85b3}
...
3. **The integration of specialized hardware accelerators into ML systems requires alignment with ______ frameworks like TensorFlow and PyTorch.**
   ...
   *Answer*: The answer is "software". The integration of specialized hardware accelerators into ML systems requires alignment with software frameworks like TensorFlow and PyTorch to ensure seamless deployment and operation.
...
3. **In AI systems, ______ memory is used to store frequently accessed data to minimize latency and improve performance.**
   ...
   *Answer*: The answer is "cache". Cache memory stores frequently accessed data to minimize latency and improve performance by reducing the need for slower off-chip memory accesses.

   *Learning Objective*: Recall the role of cache memory in AI systems and its impact on performance.

4. **Order the steps involved in host-accelerator data movement: Copy processing data, Instruct the processing, Execute parallel in each core, Store results.**

   *Answer*: The order is as follows: 1. Copy processing data, 2. Instruct the processing, 3. Execute parallel in each core, 4. Store results. These steps outline the sequence of data movement and execution in host-accelerator interactions.
...
3. **In AI acceleration, ______ refers to the assignment of machine learning computations to hardware processing units to optimize execution efficiency.**
   ...
   *Answer*: The answer is "mapping". Mapping in AI acceleration involves assigning computations to processing units to optimize execution efficiency, including spatial allocation, temporal scheduling, and memory-aware execution.
...
5. **Order the following steps involved in mapping neural network computations to AI accelerators: Data Placement, Computation Scheduling, Data Movement Timing.**
   ...
   *Answer*: The order is as follows: 1. Data Placement: Allocate data across memory hierarchies to minimize latency and energy consumption. 2. Computation Scheduling: Determine the order of operations to optimize compute efficiency and memory access patterns. 3. Data Movement Timing: Manage data transfers to reduce latency and energy costs.
...
3. **In AI accelerators, ______ strategies determine which data remains fixed in memory and which data is streamed dynamically to maximize reuse and reduce memory fetches.**
   ...
   *Answer*: The answer is "dataflow". Dataflow strategies optimize data movement by determining which data remains fixed in memory and which is streamed dynamically, maximizing reuse and minimizing redundant memory fetches.
...
5. **Order the following mapping strategies by their suitability for CNNs, Transformers, and MLPs: Activation Stationary, Weight Stationary, Output Stationary.**
   ...
   *Answer*: The order is as follows: 1. Weight Stationary (CNNs), 2. Activation Stationary (Transformers), 3. Output Stationary (MLPs). Weight Stationary is best for CNNs due to structured weight reuse, Activation Stationary suits Transformers for efficient KV-cache management, and Output Stationary optimizes accumulation in MLPs.
...
2. **Order the stages of the machine learning compilation pipeline: Graph Optimization, Kernel Selection, Memory Planning, Computation Scheduling, Code Generation.**
   ...
   *Answer*: The order is as follows: 1. Graph Optimization, 2. Kernel Selection, 3. Memory Planning, 4. Computation Scheduling, 5. Code Generation. This sequence reflects the logical progression from high-level graph restructuring to low-level code generation in the ML compilation pipeline.
...
4. **In AI runtimes, ______ execution ensures that computations proceed without waiting for memory transfers to complete.**
   ...
   *Answer*: The answer is "asynchronous". Asynchronous execution allows computations to overlap with memory transfers, maintaining a steady flow of data and avoiding pipeline stalls.

   *Learning Objective*: Recall the concept of asynchronous execution in AI runtimes.

5. **Order the steps involved in dynamic kernel scheduling for AI runtimes: Select kernel based on system state, Schedule kernel across processing units, Preload intermediate data into cache, Execute kernel.**

   *Answer*: The order is as follows: 1. Select kernel based on system state, 2. Preload intermediate data into cache, 3. Schedule kernel across processing units, 4. Execute kernel. This sequence ensures that the most efficient kernel is chosen, data is ready for processing, and execution is optimized for parallelism.
...
3. **In TPU Pods, the ______ interconnect topology enables efficient data exchange between accelerators, minimizing communication bottlenecks.**
   ...
   *Answer*: The answer is "2D torus". The 2D torus interconnect topology in TPU Pods facilitates efficient data exchange between accelerators, minimizing communication bottlenecks and supporting large-scale parallel processing.
