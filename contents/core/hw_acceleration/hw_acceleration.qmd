...
## Compiler Support for Mapping and Optimization
...
The following table highlights the key differences between traditional and machine learning compilers.  
...
: Traditional vs. Machine Learning Compilers  
...
## AI Runtimes for Execution Management
...
Traditional software runtimes are designed to manage  general-purpose program execution , primarily handling  sequential and multi-threaded workloads  on CPUs. These runtimes allocate memory, schedule tasks, and optimize execution  at the level of individual function calls and instructions . In contrast,  AI runtimes  are tailored for  machine learning workloads , which demand  massively parallel computation, large-scale tensor operations, and dynamic memory management .  

A key distinction between these two runtime models lies in  execution flow . Traditional software runtimes operate on a predictable, structured execution model where function calls and CPU threads follow a predefined control path.  AI runtimes , however, execute  computational graphs , requiring complex scheduling decisions that account for  dependencies between tensor operations , parallel kernel execution, and  efficient memory access .  

Another significant difference is  memory management . Traditional software runtimes handle  small, frequent memory allocations , optimizing for  cache efficiency and low-latency access . On the other hand,  AI runtimes  are responsible for  dynamically allocating, reusing, and optimizing large tensors , ensuring that memory access patterns align with  accelerator-friendly execution . Poor memory management in AI workloads can lead to  performance bottlenecks , especially due to excessive off-chip memory transfers and inefficient cache usage.  

Moreover,  AI runtimes are inherently designed for adaptability . While traditional runtimes often follow a  mostly static execution plan ,  AI workloads  typically operate in  highly variable execution environments , such as  cloud-based accelerators  or  multi-tenant hardware . Consequently, AI runtimes must continuously  adjust batch sizes, reallocate compute resources, and manage real-time scheduling decisions  to maintain high throughput and minimize delays.  

The following table highlights the key differences between  traditional and machine learning runtimes :  
...
As we can see, AI acceleration has evolved from single-chip processors to chiplet-based architectures, then to multi-GPU systems, further into distributed TPU Pods, and finally to wafer-scale AI. Each step introduces increasing complexity in terms of data movement, memory access, interconnect efficiency, and workload distribution.  
...
Each of these approaches represents a different trade-off between scalability, efficiency, and architectural complexity. The next step in our discussion will explore how AI workloads are mapped to these architectures, examining how computational graphs, memory management, and execution scheduling must adapt as AI systems scale.
...
The following table summarizes these adaptations, illustrating how compilers and runtimes extend fundamental acceleration techniques to manage the challenges of multi-chip execution:  
...
As AI systems scale beyond a single chip, computation placement must extend beyond local processing elements (PEs) to consider inter-chip workload distribution. In a single-chip accelerator, compilers optimize placement by mapping workloads to tensor cores, vector units, and PEs to maximize parallel execution and minimize data movement. However, in multi-chip systems, computation placement must now consider interconnect bandwidth, synchronization latency, and hierarchical workload distribution across multiple accelerators.

Compilers handle this increased complexity by implementing interconnect-aware workload partitioning. Instead of focusing solely on intra-chip execution efficiency, they must now assign computations to accelerators in a way that reduces costly cross-chip communication. For example, in multi-GPU training, compilers strategically place operations to minimize NVLink or PCIe traffic, while in TPU Pods, workload partitioning aligns with the torus interconnect topology to ensure efficient data exchanges.

Runtimes also play a critical role in computation placement, particularly in dynamic execution environments. Unlike static compilation, which assumes a fixed hardware topology, runtimes continuously monitor execution workloads and adjust computation placement dynamically to balance loads across accelerators. This ensures that no single accelerator becomes a bottleneck due to uneven workload distribution. Additionally, adaptive load balancing techniques can migrate tasks across accelerators in real-time to account for changing system conditions.

The following table summarizes how computation placement strategies extend from single-chip to multi-chip execution:
