...
The sigmoid function is differentiable and has a well-defined gradient, which makes it suitable for use with gradient-based optimization methods. Its bounded output ensures numerical stability, preventing excessively large activations that might destabilize the training process. However, for inputs with very high magnitudes (positive or negative), the gradient becomes negligible, which can lead to the vanishing gradient problem.[^explode-vanish] This issue is particularly detrimental in deep networks, where gradients must propagate through many layers during training [@hochreiter1998vanishing].

[^explode-vanish]: Vanishing gradients prevent learning in deep layers as parameter updates become negligible. Conversely, exploding gradients cause rapid weight updates, leading to unstable training. Both issues can hinder convergence and degrade model performance.

Additionally, sigmoid outputs are not zero-centered, meaning that the function produces only positive values. This lack of symmetry can cause optimization algorithms like stochastic gradient descent (SGD)[^sto-gd] to exhibit inefficient updates, as gradients may introduce biases that slow convergence. To mitigate these issues, techniques such as batch normalization[^fn-batchnorm] or careful initialization may be employed.[^pop-frameworks]

[^pop-frameworks]: Popular deep learning frameworks like PyTorch and TensorFlow implement robust initialization schemes based on theoretical principles and empirical research. These defaults help prevent vanishing/exploding gradients and ensure stable training.

[^sto-gd]: **Stochastic Gradient Descent (SGD):** Unlike full gradient descent which computes gradients over the entire dataset, SGD estimates gradients using small batches of data. This reduces memory requirements and enables frequent parameter updates, though it introduces variance in the optimization process. This variance can help escape local minima but results in less precise convergence compared to full gradient descent.
...
\begin{axis}[
   ...
   axis line style={thick,-latex},
    ...
    bar width=30pt,
    symbolic x coords={Sigmoid,Tanh,ReLU,Softmax},
    xtick=data]
    \addplot[ybar,line width=1.0pt,draw=GreenLine,fill=GreenL] coordinates {
        (Sigmoid,1.1)
        (Tanh,0.61)
        (ReLU,0.78)
        (Softmax,0.91)
...
These algorithms are responsible for navigating the complex, high-dimensional landscape of the loss function, identifying regions where the function achieves its lowest values. This task is challenging because the loss function surface is rarely smooth or simple, often characterized by local minima, saddle points, and sharp gradients. Effective optimization algorithms are designed to overcome these challenges, ensuring convergence to a solution that generalizes well to unseen data.[^unseen]

[^unseen]: When training machine learning models, a portion of data should remain completely isolated from both training and validation to provide an unbiased assessment of final model performance. This held-out test set helps evaluate how well the model generalizes to truly unseen examples.
...
Advanced optimization algorithms introduce mechanisms like momentum and adaptive learning rates[^learn-rate] to improve convergence. These methods have been instrumental in addressing the inefficiencies of classical approaches [@kingma2014adam].

[^learn-rate]: **Learning rate:** A parameter that controls the size of parameter updates during training. A rate that is too high can cause training to diverge, while one that is too low leads to slow convergence. Finding the optimal learning rate is critical for efficient model training.
...
\begin{scope}[node distance=0.2,shift={(SC1.center)},shift={(4.5,0.9))},local bounding box = SC2]
\node[mycycle] (C1) {};
...
\begin{scope}[node distance=0.2,shift={(SC1.center)},shift={(14,0.9))},local bounding box = SC3]
\node[mycycle] (3C1) {};
...
\begin{scope}[node distance=0.2,shift={(SC1.center)},shift={(20.0,0.9))},local bounding box = SC4]
\node[mycycle] (4C1) {};
...
\node[below=3pt of  BB1.north,anchor=north,helvetica]{Storage Zone};
%
\scoped[on background layer]
\node[draw=BackLine,inner xsep=11,inner ysep=43,yshift=0mm,
           fill=BackColor,fit=(B2)(B3)(B4),line width=0.75pt](BB1){};
\node[below=3pt of  BB1.north,anchor=north,helvetica]{CPU Preprocessing Zone};
%
\scoped[on background layer]
\node[draw=BackLine,inner xsep=11,inner ysep=20,yshift=2mm,
           fill=BackColor,fit=(B5)(B6)(B7),line width=0.75pt](BB1){};
\node[below=3pt of  BB1.north,anchor=north,helvetica]{GPU Training Zone};
...
```{r #fig-galore-llm-memory-breakdown}
#| echo: false
#| fig-cap: "Example memory footprint breakdown for the Llama-7B model under different optimized training schemes. Note that in the unoptimized bfloat16 case, how optimizer state and weight gradients combined can take up more than double the footprint of the model weights."
...
# Create data
data <- data.frame(
  Method = factor(c("BF16", "Adafactor", "8-bit Adam", "8-bit GaLore"),
                 levels = c("8-bit GaLore", "8-bit Adam", "Adafactor", "BF16")),  # Reverse order
  Weight = c(20, 15, 10, 8),
  Activation = c(25, 20, 15, 12),
  Optimization = c(15, 10, 8, 6),
  WeightGradient = c(10, 8, 6, 4),
  Others = c(5, 3, 2, 1)
)

# Reshape the data
data_long <- melt(data, id.vars = "Method",
                  variable.name = "Component",
                  value.name = "MemoryCost")

# Custom color palette inspired by viridis but with better contrast
custom_colors <- c(
  "Weight" = "#440154FF",        # Deep purple
  "Activation" = "#21908CFF",    # Teal
  "Optimization" = "#FDE725FF",  # Yellow
  "WeightGradient" = "#35B779FF", # Green
  "Others" = "#31688EFF"         # Blue
)

# Create the enhanced plot
ggplot(data_long, aes(x = MemoryCost, y = Method, fill = Component)) +
  geom_bar(stat = "identity", position = position_stack(vjust = 1), width = 0.7) +
  ...
  geom_vline(xintercept = 30, color = "#666666", linetype = "longdash", linewidth = 0.5, alpha = 0.7) +
  annotate(
    "text",
    x = 32,
    y = 3.5,
    label = "RTX 4090 Memory Limit",
    color = "#666666",
    size = 3.5,
    hjust = 0,
    fontface = "italic"
...
\node[Box,right=of B1](B2){Gradients FP16};
\node[Box,right=of B2](B3){Gradients FP32};
\node[Box,right=of B3](B4){Weights FP32};
\node[Box,right=of B4](B5){Weights FP16};
%
\scoped[on background layer]
...
fit=(B2)(B4)](BB){};
\node[below=1pt of BB.north,anchor=north]{Optimizer};
%
\foreach \x in {1,2,3,4} {
    \pgfmathtruncatemacro{\newX}{\x + 1}  %
    \draw[-latex,Line] (B\x) -- (B\newX);
}
\draw[Line,-latex](B1.south)--++(270:1)-|(B5);
\end{tikzpicture}
```
...
\draw[-latex,Line] (L\x) --
            node[above,black]{$\frac{\partial L_\x}{\partial x}$} (D\x);
}
...
\begin{scope}[node distance=0.2,shift={(SC1.center)},shift={(4.5,5))},local bounding box = SC2]
\node[mycycle] (C1) {};
\node[mycycle,below=of C1] (C2) {};
\node[mycycle,below=of C2] (C3) {};
\node[mycycle,below=of C3] (C4) {};
...
\begin{scope}[node distance=0.2,shift={(SC1.center)},shift={(13,5))},local bounding box = SC3]
\node[mycycle] (3C1) {};
\node[mycycle,below=of 3C1] (3C2) {};
\node[mycycle,below=of 3C2] (3C3) {};
\node[mycycle,below=of 3C3] (3C4) {};
...
\begin{scope}[node distance=0.2,shift={(SC1.center)},shift={(21.0,5))},local bounding box = SC4]
\node[mycycle] (4C1) {};
\node[mycycle,below=of 4C1] (4C2) {};
\node[mycycle,below=of 4C2] (4C3) {};
\node[mycycle,below=of 4C3] (4C4) {};
...
%down
%%%%%%%%%%%%%%%%%%%%%%%
\begin{scope}[node distance=0.2,shift={(SC1.center)},shift={(4.5,-3))},local bounding box = DSC2]
\node[mycycle] (DC1) {};
...
\begin{scope}[node distance=0.2,shift={(SC1.center)},shift={(13,-3))},local bounding box = DSC3]
\node[mycycle] (D3C1) {};
...
\begin{scope}[node distance=0.2,shift={(SC1.center)},shift={(21.0,-3))},local bounding box = DSC4]
\node[mycycle] (D4C1) {};
...
\node[Box,node distance=0.9](B1){GPU 1\\Forward \& Backward Pass};
\node[Box,node distance=0.9,right=of B1](B2){GPU 2\\Forward \& Backward Pass};
\node[Box,node distance=0.9,right=of B2](B3){GPU 3\\Forward \& Backward Pass};
\node[Box,node distance=0.9,right=of B3](B4){GPU 4\\Forward \& Backward Pass};
%
...
-|node[Text,pos=0.8,text=black]{Split into\\Non-Overlapping Subsets}(GB1);
\draw[Line,-latex](GGB1)
-|node[Text,pos=0.8,text=black]{Split into\\Non-Overlapping Subsets}(GB2);
\draw[Line,-latex](GGB1)
-|node[Text,pos=0.8,text=black]{Split into\\Non-Overlapping Subsets}(GB3);
\draw[Line,-latex](GGB1)
-|node[Text,pos=0.8,text=black]{Split into\\Non-Overlapping Subsets}(GB4);
%%
\draw[Line,-latex](GB1)--node[Text,pos=0.5,text=black]{Assigned \\to GPU 1}(B1);
\draw[Line,-latex](GB2)--node[Text,pos=0.5,text=black]{Assigned \\to GPU 2}(B2);
\draw[Line,-latex](GB3)--node[Text,pos=0.5,text=black]{Assigned \\to GPU 3}(B3);
\draw[Line,-latex](GB4)--node[Text,pos=0.5,text=black]{Assigned \\to GPU 4}(B4);
%
\draw[Line,-latex](B1)
...
|-node[Text,pos=0.2,text=black]{Compute\\ Gradients}(DB1);
%
\draw[Line,-latex](DB1)--node[Text,pos=0.5,text=black]{Synchronize Gradients}(DB2);
\draw[Line,-latex](DB2)--node[Text,pos=0.5,text=black]{Aggregate Gradients\\ and Update Parameters}(DB3);
%
...
-|node[Text,pos=0.25,text=black]{Forward\\ Pass}(B2.120);
\draw[Line,latex-](B1)--++(270:12mm)
-|node[Text,pos=0.25,text=black]{Gradient\\ Updates}(B2.240);
%
\draw[Line,-latex](B2)--++(90:12mm)
-|node[Text,pos=0.25,text=black]{Intermediate\\ Data}(B3.120);
\draw[Line,latex-](B2)--++(270:12mm)
-|node[Text,pos=0.25,text=black]{Gradient\\ Updates}(B3.240);
%
\draw[Line,-latex](B3)--++(90:12mm)
-|node[Text,pos=0.25,text=black]{Intermediate\\ Data}(B4.120);
\draw[Line,latex-](B3)--++(270:12mm)
-|node[Text,pos=0.25,text=black]{Gradient\\ Updates}(B4.240);
%
\draw[Line,-latex](B4)--++(90:12mm)
-|node[Text,pos=0.25,text=black]{Output}(B5.120);
\draw[Line,latex-](B4)--++(270:12mm)
-|node[Text,pos=0.25,text=black]{Backward\\ Pass}(B5.240);
\end{tikzpicture}
```
...
\draw[Line,-latex](G4B3)--node[Text,text=black,pos=0.45]{No}(G3B3);
\draw[Line,-latex](G4B3)-|node[Text,pos=0.1,text=black]{Yes}(G1B4);
\draw[Line,-latex](G3B3)--node[Text,pos=0.45,text=black]{No}(G2B3);
\draw[Line,-latex](G2B3)--node[Text,pos=0.45,text=black]{No}(G1B3);
\draw[Line,-latex](G1B4)--node[Text,pos=0.45,text=black]{No}(B4);
%
\draw[Line,-latex](G3B3)-|node[Text,pos=0.22,text=black]{Yes}(B2.150);
\draw[Line,-latex](G2B3)-|node[Text,pos=0.32,text=black]{Yes}(B1);
\draw[Line,-latex](G1B3.south)--++(270:7mm)-|node[Text,pos=0.22,text=black]{Scaling\\ Model}(B2);
\draw[Line,-latex](G1B3.south)--++(270:7mm)-|node[Text,pos=0.22,text=black]{Scaling\\ Model}(B3);
