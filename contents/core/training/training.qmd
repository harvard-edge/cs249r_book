...
_What essential principles guide the training of machine learning systems, and how do computational environments shape training strategies?_
...
To avoid overfitting, regularization techniques can help constrain the model. One regularization method is adding a penalty term to the loss function that discourages complexity, like the L2 norm of the weights. This penalizes large parameter values. Another technique is dropout, where a percentage of neurons is randomly set to zero during training. This reduces **neuron co-adaptation**[^neuron-coad].

[^neuron-coad]: Neurons can overly rely on each other during the learning process and can fail to learn generalizable features independently. They can depend on specific configurations of inputs from other neurons to make decisions which may lead to overfitting.
...
The core of a neural network can be viewed as a sequence of alternating linear and nonlinear operations, as shown in @fig-neural-net-diagram. 

![Neural network diagram. Source: astroML.](images/png/aitrainingnn.png){#fig-neural-net-diagram}

Neural networks are structured with layers of neurons connected by weights (representing linear operations) and activation functions (representing nonlinear operations). By examining the figure, we see how information flows through the network, starting from the input layer, passing through one or more hidden layers, and finally reaching the output layer. Each connection between neurons represents a weight, while each neuron typically applies a nonlinear **activation function**[^act-fun] to its inputs.

[^act-fun]: Activation functions determine if a neuron's input to the network is important or not for achieving accurate predictions. Nonlinear activation functions introduce complexity that would not be achieved with just linear transformations. Common examples include sigmoid, tanh, and ReLU functions. More information about activation function is included in section 7.8.
...
:::{.callout-note}
In practice, the gradient is computed over a minibatch of data points to improve computational efficiency. This is called **stochastic gradient descent**[^sto-gd] or batch gradient descent.
:::

[^sto-gd]: Full gradient descent computes the true gradient over the entire dataset giving the exact direction of steepest descent in the loss landscape. In contrast, stochastic gradient descent estimates the gradient by using a minibatch of data. While this requires a lower memory footprint and allows for more frequent parameter updates, it introduces variance. This variance can help escape poor local minima, but it means stochastic gradient descent typically oscillates around the true minimum instead of fully converging to it.

Where $\lambda$ is the stepsize or **learning rate**[^learn-rate] of our tweaks, in training our neural network, we repeatedly perform the step above until convergence, or when the loss no longer decreases. @fig-gradient-descent illustrates this process: we want to reach the minimum point, which's done by following the gradient (as illustrated with the blue arrows in the figure). This prior approach is known as full gradient descent since we are computing the derivative concerning the entire training data and only then taking a single gradient step; a more efficient approach is to calculate the gradient concerning just a random batch of data points and then taking a step, a process known as batch gradient descent or stochastic gradient descent [@robbins1951stochastic], which is more efficient since now we are taking many more steps per pass of the entire training data. Next, we will cover the mathematics behind computing the gradient of the loss function concerning the $W_i$s, a process known as backpropagation.

[^learn-rate]: It is crucial to find the optimal learning rate to ensure efficient and effective training. If the learning rate is too large, training may diverge and the model will bounce between parameters that minimize the loss. If the learning rate is too small, the step size will be very small for each iterative and training may be too slow to converge to the minimum. More information on tuning hyperparamters like the learning rate is included in section 7.6.
...
The validation set evaluates the model during training, usually after each epoch. Typically, 20% of the data is allocated for the validation set. The model does not learn or update its parameters based on the validation data. It is used to tune **hyperparameters**[^hyperp] and make other tweaks to improve training. Monitoring metrics like loss and accuracy on the validation set prevents overfitting on just the training data.

[^hyperp]: Parameters (likes weights and biases) are learned during training through gradient descent. In contrast, hyperparameters like the learning rate, batch sizes, and number of layers are chosen beforehand. While parameters directly optimize the loss function, hyperparameters determine how that optimization happens.
...
The relative proportions of the training, validation, and test sets can vary based on data size and application. However, following the general guidelines for a 60/20/20 split is a good starting point. Careful data splitting ensures models are properly trained, tuned, and evaluated to achieve the best performance.[^unseen]

[^unseen]: Some portion of the data should be set aside for final evaluation of the completed model. This is separate from the test and validation sets. A more detailed description of this unseen data for final evaluation is included in section 7.4.2.
...
For example, if you train an image classification model to recognize handwritten digits, providing only 10 or 20 images per digit class would be completely inadequate. The model would need more examples to capture the wide variances in writing styles, rotations, stroke widths, and other variations.[^millions]

[^millions]: ML models can use millions or billions of parameters to capture the complex patterns in the data. It is standard practice to have a training set size that is at least 10 times the number of features/parameters.
...
A good rule of thumb is that the validation set size should be at least several hundred samples and up to 10-20% of the training set, while still leaving sufficient samples for training. The splits should also be stratified, meaning that the class proportions in the validation set should match those in the full dataset, especially if working with **imbalanced datasets**[^imbal]. A larger validation set representing the original data characteristics is essential for proper model selection and tuning.

[^imbal]: Data is imbalance when one class is outnumbers by others such as having 95% healthy and 5% diseased samples in a medical context. Models will achieve high accuracy by predicting the majority class which gives misleading results about the success of the model. There are several techniques to handle data imbalances such as adjusted metric (F1-score), stratified splits, and sampling techniques (SMOTE).
...
* **Neural networks:** **Learning rate**[^nn-rate], batch size, number of hidden units, activation functions
* **Support vector machines:** Regularization strength, kernel type and parameters
* **Random forests:** Number of trees, tree depth
* **K-means:** Number of clusters

[^nn-rate]: The learning rate is often the most important hyperparameter to tune.
...
* **Random Search:** Instead of defining a grid, you randomly select values for each hyperparameter from a predefined range or set. This method is more efficient at exploring a vast hyperparameter space because it doesn't require an exhaustive search. However, it may still miss optimal parameters since it doesn't systematically explore all possible combinations.[^grid-random]

[^grid-random]: While grid search tests all combinations of hyperparameters, random search can find good solutions faster by considering that not all hyperparameters are equally important. Think of tuning hyperparameters as looking for a needle in a haystack. It can be more efficient to randomly sample different locations than to check every point methodically.
...
[^bayes-optim]: Unlike grid search or random search, Bayesian optimization considers that some hyperparameter configurations are more likely to be optimal than others. It builds a model that maps the hyperparameter values to the objective. By the Bayesian approach, the model evaluates promising configurations and updates the models based on these results.
...
Several commercial auto-tuning platforms are available to address this problem. One solution is Google's **Vertex AI Cloud**[^vertex-ai], which has extensive integrated support for state-of-the-art tuning techniques.

[^vertex-ai]: Vertex AI Cloud allows for parallel processing of multiple hyperparameter configurations to speed up the optimization process.

One of the most salient capabilities of Google's Vertex AI-managed machine learning platform is efficient, integrated hyperparameter tuning for model development. Successfully training performant ML models requires identifying optimal configurations for a set of external hyperparameters that dictate model behavior, posing a challenging high-dimensional search problem. Vertex AI simplifies this through **Automated Machine Learning**[^auto-ml] (AutoML) tooling.

[^auto-ml]: AutoML tools help democratize the ML pipeline by simplifying the complexities of model selection, tuning, and feature engineering. This will allow people with limited data science experience to build effective models.
...
The exponentiation transform allows the function to smoothly transition from near 0 towards near 1 as the input moves from very negative to very positive. The monotonic rise covers the full (0,1) range.[^monotonic]

[^monotonic]: Sigmoid activation function are very common in the final layer for binary classification where you need a probability output between 0 and 1.
...
It leaves all positive inputs unchanged while clipping all negative values to 0.[^thres-brain] This sparse activation and cheap computation make ReLU widely favored over sigmoid/tanh.

[^thres-brain]: Similarly, neurons in biological context are only activated if they pass a certain threshold.
...
The softmax activation function is generally used as the last layer for classification tasks to normalize the activation value vector so that its elements sum to 1. This is useful for classification tasks where we want to learn to predict class-specific probabilities of a particular input, in which case the cumulative probability across classes is equal to 1.[^soft-expense] The softmax activation function is defined as

[^soft-expense]: Softmax is computationally expensive due to the exponential terms, however this is typically not a significant issue since it is often only used in the final layer.
...
The underlying neural network architecture also affects whether matrix multiplication poses more of a computational or memory bottleneck during execution. **Transformers**[^transformers] and **Multi-Layer Perceptron**[^mlps] (MLPs) are much more compute-bound than CNN convolutional neural networks. This stems from the types of matrix multiplication operations involved in each model. Transformers rely on self-attention, multiplying large activation matrices by massive parameter matrices to relate elements. MLPs stack fully connected layers, also requiring large matrix multiplies.

[^transformers]: Transformers are neural networks that use the multi-head attention mechanism to process sequential data. Attention is an ML method that determines and considers the relative importance of different elements in a sequence. In the context of large language models, each word in a passage can 'attend' to other words simultaneously.
[^mlps]: MLPs are feedforward artificial neural networks consisting of fully connected neurons organized into layers.
...
4. **Gradient Aggregation:** After processing their respective batches, the gradients from each processing unit are aggregated. Common aggregation methods include summation or averaging of the gradients.[^sum-avg]

[^sum-avg]: The choice between summing or averaging over gradients is based on the specific goals and constraints of training the model. When summing the gradients, the learning rate may be scaled in proportion to the number of workers to ensure the magnitude of parameter updates remains consistent. Averaging the gradients often creates a more stable direction and reduced variance of the updated parameter, however it requires a central node to communication between workers. This may cause a bottleneck as the number of workers increases.
