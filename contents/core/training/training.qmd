...
Understanding the distinct nature of machine learning training systems requires tracing the evolution of prior computing paradigms. High-performance computing (HPC) systems emerged in the 1960s with the CDC 6600 [@thornton1965cdc] and evolved through the 1990s with massively parallel processors like the CM-5 [@thinking_machines_cm5]. These systems excelled at scientific simulations and numerical computations, relying on predictable memory access patterns and high arithmetic intensity.

In the early 2000s, warehouse-scale computing (WSC) emerged as companies like Google and Amazon developed large-scale data centers to handle distributed workloads. Unlike HPC systems, WSC architectures focused on throughput, reliability, and scalability to support independent tasks such as web search and video streaming [@barroso2003web].

Machine learning training systems build upon principles from both paradigms but introduce fundamentally different challenges. Since the advent of deep learning in 2012 with AlexNet [@krizhevsky2012imagenet], training workloads have required irregular memory access patterns, frequent parameter updates, and tightly coordinated computations across devices. These workloads demand synchronization of shared parameter updates, creating bottlenecks that traditional HPC and WSC architectures were not designed to address.

Performance differences between these paradigms highlight the uniqueness of training workloads. While HPC systems often achieve 50-80% of peak hardware performance [@dongarra2003highperf], training workloads are constrained by memory bottlenecks and synchronization overheads, necessitating specialized designs for efficient execution.

### Role in ML Systems

Machine learning training systems are the engines that enable the creation of sophisticated predictive models. They provide the computational infrastructure required to transform raw data into actionable insights through iterative optimization of model parameters. This process is foundational to machine learning, powering advancements across domains such as computer vision, natural language processing, and robotics.

Training systems do not operate in isolation. They are deeply integrated into the machine learning lifecycle, connecting upstream processes such as data preprocessing and feature engineering with downstream tasks like deployment and inference. The efficiency and scalability of training systems directly influence the pace at which models can be developed, validated, and deployed.

The advent of deep learning and large-scale models has significantly increased the demands on training systems. These systems must manage vast datasets, coordinate computations across distributed hardware, and address memory and communication bottlenecks. As training workloads grow in size and complexity, the design and optimization of training systems play a critical role in determining the feasibility and accessibility of machine learning technologies.

Beyond their computational role, training systems impact the sustainability and scalability of machine learning. Energy efficiency, compatibility with emerging hardware, and the ability to adapt to larger datasets and models are critical considerations in the design of modern training infrastructure.

### Systems Thinking

The practical execution of training models is deeply tied to system design. Training is not merely a mathematical optimization problem; it is a system-driven process that requires careful orchestration of hardware, memory, and data movement.

Training workflows consist of interdependent stages: data preprocessing, forward and backward passes, and parameter updates. Each stage imposes specific demands on system resources. For instance, data preprocessing relies on storage and I/O subsystems to provide accelerators with continuous input. Similarly, gradient computation and parameter updates depend on high-performance hardware like GPUs and TPUs. The efficiency of these stages is contingent on how well the system handles bottlenecks such as memory bandwidth and communication latency.
...
For example, training large Transformer models requires partitioning data and model parameters across multiple devices [@vaswani2017attention]. This introduces synchronization challenges, particularly during gradient updates. Communication libraries such as NCCL enable efficient gradient sharing, while techniques like gradient accumulation and checkpointing manage memory constraints. These examples illustrate how system-level considerations influence the feasibility and efficiency of modern training workflows.

## Mathematical Foundations

### Neural Network Computation

We have previously introduced the basic operations involved in training a neural network, such as forward propagation and the use of loss functions to evaluate performance. Here, we build on those foundational concepts to explore how these operations are executed at the system level. Key mathematical operations such as matrix multiplications and activation functions underpin the system requirements for training neural networks. Foundational works by @rumelhart1986learning via the introduction of backpropagation and the development of efficient matrix computation libraries, e.g., BLAS [@dongarra1988extended], laid the groundwork for modern training architectures.

#### Core Operations

At the heart of a neural network is the process of forward propagation, which involves two primary operations: matrix multiplication and the application of an activation function. Matrix multiplication forms the basis of the linear transformation in each layer of the network. At layer $l$, the computation can be described as:
...
Matrix multiplication forms the computational backbone of neural networks, often accounting for 60-90% of the total computation time in typical architectures [@he2016residual]. These operations are computationally intensive due to their cubic complexity with respect to input size and layer dimensions. The implications of this complexity are profound: as data passes through each layer, activations must be stored for backpropagation, significantly increasing memory usage. Optimizing memory management, such as employing techniques like mixed-precision training, has become critical for scaling deep networks.

The evolution of matrix multiplication algorithms has closely followed advancements in numerical linear algebra. From Strassen's algorithm, which reduced the naive $O(n^3)$ complexity to approximately $O(n^{2.81})$ [@strassen1969gauss], to contemporary hardware-accelerated libraries like cuBLAS, these innovations have continually pushed the limits of computational efficiency in neural network training.

Matrix multiplications not only shape compute requirements but also inform hardware design. Deep learning accelerators, such as Google's TPU [@jouppi2017tpu], incorporate matrix units optimized for these operations, enabling high-throughput, low-latency computation essential for training large-scale models like GPT-3 [@brown2020language].

#### Activation Functions

Activation functions are central to neural network operation. They introduce the essential non-linearity that allows neural networks to approximate complex mappings between inputs and outputs. Without activation functions, neural networks, regardless of depth, would collapse into linear systems, severely limiting their representational power [@goodfellow2016deep].
...
Modern neural network training relies on variations of gradient descent for parameter optimization. These fundamental approaches differ in how they process training data, leading to distinct system-level implications.
...
Training frameworks must efficiently implement these optimization algorithms while managing system resources. Key implementation strategies include:

1. **Fused Operations:** Modern frameworks combine multiple optimization steps into single operations. For example, the Adam update can be fused into a single kernel launch, reducing memory bandwidth requirements:

$$ \text{Bandwidth}_{\text{separate}} = 5 \times \text{Size}_{\text{params}} $$
$$ \text{Bandwidth}_{\text{fused}} = 2 \times \text{Size}_{\text{params}} $$

2. **Memory Access Patterns:** Optimizer implementations must consider cache efficiency. Sequential access to parameter and optimizer state vectors allows better utilization of cache hierarchies and memory bandwidth.

3. **Numerical Precision:** While 32-bit floating-point provides high numerical precision, optimization state often tolerates reduced precision. Training systems frequently store optimizer states in 16-bit formats, halving memory requirements:

$$ \text{Memory}_{\text{Adam-FP16}} = \frac{3}{2} \times \text{Size}_{\text{params}} $$

These implementation choices significantly impact training performance. A well-implemented optimizer minimizes memory bandwidth requirements while maintaining numerical stability and convergence properties.
...
The mathematical principles underlying neural network computations---matrix multiplications and activation functions---are more than theoretical constructs. They form the backbone of practical systems that implement and optimize these operations. These fundamental computations not only dictate the mechanics of forward and backward propagation but also have profound impacts on critical system-level factors such as memory management, computational throughput, and hardware utilization.
...
These foundational operations also set the stage for addressing the challenges of training pipeline architecture. From designing workflows for data preprocessing to employing advanced techniques like mixed-precision training, gradient accumulation, and checkpointing, their implications are far-reaching.

Moreover, they provide the groundwork for scaling neural networks across distributed systems. As we delve into distributed training strategies, these operations will remain central to understanding synchronization challenges, communication overheads, and resource management across multiple GPUs or nodes. Ultimately, the interplay between these computations and system-level design is essential to achieving efficient, scalable AI training.
...
{{ Figure Recommendation: A diagram illustrating the data pipeline, showing raw data entering a preprocessing module and being output as batches ready for training. }}
...
{{ Figure Recommendation: A flowchart showing the steps in the training loop: forward pass → loss calculation → backward pass → parameter updates. }}
...
{{ Figure Recommendation: A diagram showing the evaluation pipeline, illustrating data flow from the validation set to metric computation. }}
...
{{ Figure Recommendation: A high-level diagram showing how the components—data pipeline, training loop, and evaluation pipeline—connect and interact. }}
...
The data pipeline transforms raw data stored on disk into processed batches ready for model training. Consider an image recognition model: the pipeline reads image files from storage, converts them to the correct format, applies preprocessing operations like resizing, and delivers them to GPUs for computation. Each stage in this process must work efficiently to maintain smooth data flow during training.
...
Modern GPUs execute neural network computations through thousands of parallel processing cores. These cores excel at matrix multiplications, which form the backbone of gradient computation. However, the backward pass introduces irregular computation patterns. 

When calculating activation function derivatives, the GPU alternates between reading values, computing derivatives, and performing multiplication operations. This mixing of operation types prevents the GPU from maintaining optimal throughput.

GPU hardware processes data in fixed-width blocks, aligned with physical processor arrangements. NVIDIA GPUs, for example, group 32 threads into processing units called warps. These warps execute instructions in lockstep---all threads must complete their current operation before the group advances. Neural network layers rarely align perfectly with these hardware constraints. When a layer requires 100 feature computations, it leaves 4 threads in each 32-thread warp idle, creating a 12.5% efficiency loss. These partial-warp executions accumulate across the network, reducing overall training speed.[^fn-calc-eff]

[^fn-calc-eff]: Efficiency calculation: 
  - Warp size: 32 threads
  - Features: 100
  - Unused threads per warp: 32 - (100 % 32) = 4
  - Efficiency loss: 4/32 = 12.5%
...
Gradient descent, the most basic optimization algorithm, illustrates the fundamental memory and computation patterns in parameter updates. The mathematical update rule for a parameter is:
...
Where:
- $\alpha$ is the learning rate, which determines how much to adjust the parameters
- $\nabla_\theta L(\theta_t)$ represents the gradient of the loss with respect to the parameter $\theta_t$

From a systems perspective, this equation translates into a sequence of operations. For each parameter update, the system must:

1. Read the current parameter value $\theta_t$ from memory
2. Access the computed gradient $\nabla_\theta L(\theta_t)$ from memory
3. Perform the multiplication and subtraction operations
4. Write the new parameter value $\theta_{t+1}$ back to memory

Because gradient descent only requires memory for storing parameters and gradients, it has relatively low memory overhead compared to more complex optimizers.

However, more advanced optimizers introduce additional memory requirements and computational complexity. For example, Adam maintains two extra vectors for each parameter: one for the first moment (the moving average of gradients) and one for the second moment (the moving average of squared gradients). This triples the memory usage but can lead to faster convergence.

For a model with $N$ parameters, the total memory required for different optimizers can be compared:

$$
\text{Memory for Gradient Descent} = N \times \text{(Size of Gradient)}
$$

$$
\text{Memory for Adam} = 3 \times N \times \text{(Size of Gradient)}
$$

For instance, if $N = 100,000$ parameters and each gradient requires 4 bytes (32 bits):
...
The choice of batch size interacts with other aspects of the optimization process. For instance:

1. It affects the frequency of parameter updates: larger batches mean less frequent but potentially more impactful updates.
2. It influences the behavior of adaptive optimization algorithms, which may need to be tuned differently for various batch sizes.
3. In distributed training, batch size often determines the degree of data parallelism, impacting how gradient computations and parameter updates are distributed across devices.
...
This section explores three widely adopted optimization strategies that address key performance bottlenecks in training pipelines:
...
Training data passes through three primary stages:

1. **Data Fetching**: Raw data retrieval from storage (local disk, network storage, or cloud infrastructure)
2. **Preprocessing**: Data transformation into training-ready format through resizing, normalization, and augmentation
3. **Computation**: Model forward and backward passes using the processed data

An unoptimized pipeline executes these stages sequentially. The GPU remains idle during data fetching and preprocessing, waiting for data preparation to complete. This sequential execution creates significant inefficiencies in the training process.
...
Thus far, we have focused on ML training pipelines from a single-system perspective. But in practice, training machine learning models often requires scaling beyond a single machine. As models grow in complexity and datasets increase in size, the demand for computational power, memory, and storage can exceed the capacity of individual devices. This challenge is particularly evident in areas such as natural language processing, computer vision, and scientific computing, where models and datasets can reach substantial sizes.
...
At each training step, the devices perform computations independently. After calculating gradients based on their respective data subsets, the devices share these gradients with each other through a process called gradient synchronization. The gradients are aggregated—typically through averaging—and then used to update the model parameters. This synchronization step occurs after each mini-batch, ensuring that all devices maintain consistent copies of the model throughout training.
...
The process of data parallelism can be broken into a series of distinct steps, each with its role in ensuring the system operates efficiently. These steps are as follows:
...
The computation flows sequentially: When processing a sequence of tokens, device 1 computes the first six transformer block outputs, then transfers the resulting hidden states to device 2. Each hidden state tensor contains the intermediate representations needed for subsequent computation. The dimensionality of these transferred tensors matches the model's hidden size—for instance, a tensor of shape [batch_size, sequence_length, hidden_dimension] moves between devices.
...
Pipeline parallelism processes multiple data batches concurrently across devices. Each device executes a segment of the model while maintaining a continuous flow of computation. When device A completes processing batch 1, it immediately begins batch 2 while device B processes batch 1's intermediate results. This overlapping computation maximizes device utilization and throughput.

Consider a transformer model divided across four devices: While device 1 processes batch N+1's input embeddings, device 2 computes batch N's self-attention, device 3 handles batch N-1's feed-forward operations, and device 4 generates batch N-2's output probabilities. This orchestration maintains full hardware utilization while preserving the model's mathematical properties.
...
One prominent example of FPGA adoption is Microsoft’s Project Brainwave, which leverages FPGAs to accelerate machine learning workloads in the Azure cloud. Microsoft chose FPGAs for their ability to provide low-latency inference while maintaining high throughput. This approach is especially beneficial in scenarios where real-time predictions are critical, such as search engine queries or language translation services. By integrating FPGAs directly into their data center network, Microsoft has achieved significant performance gains while minimizing power consumption.

From a training perspective, FPGAs offer unique advantages in optimizing training pipelines. Their reconfigurability allows them to implement custom dataflow architectures tailored to specific model requirements. For instance, data preprocessing and augmentation steps, which can often become bottlenecks in GPU-based systems, can be offloaded to FPGAs, freeing up GPUs for core training tasks. Additionally, FPGAs can be programmed to perform operations such as sparse matrix multiplications, which are common in recommendation systems and graph-based models but are less efficient on traditional accelerators.
...
### ASICs in Training: The Cerebras Wafer-Scale Engine

Application-Specific Integrated Circuits (ASICs) represent a class of hardware designed for specific tasks, offering unparalleled efficiency and performance by eschewing the general-purpose flexibility of GPUs or FPGAs. Among the most innovative examples of ASICs for machine learning training is the Cerebras Wafer-Scale Engine (WSE), which stands apart for its unique approach to addressing the computational and memory challenges of training massive machine learning models.

The Cerebras WSE is unlike traditional chips in that it is a single wafer-scale processor, spanning the entire silicon wafer rather than being cut into smaller chips. This architecture enables Cerebras to pack 2.6 trillion transistors and 850,000 cores onto a single device. These cores are connected via a high-bandwidth, low-latency interconnect, allowing data to move across the chip without the bottlenecks associated with external communication between discrete GPUs or TPUs.
