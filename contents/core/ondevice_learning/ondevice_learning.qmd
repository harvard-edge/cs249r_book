...
Federated learning allows edge devices to collaboratively train a shared model by performing local computation and periodically aggregating updates at a central server or aggregator. Critically, raw data remains on-device throughout the process, preserving user privacy and minimizing network transmission. This decentralized approach introduces a new set of system-level challenges, including variability in device availability, communication bottlenecks, and the need for personalization.
...
Thank you for the clarification. A dedicated **summary for the Challenges and Limitations** section is absolutely appropriate and pedagogically helpful. It should reinforce the key takeaways while drawing attention to the cross-cutting implications for ML system design.
