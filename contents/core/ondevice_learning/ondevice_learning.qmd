...
## Deployment Drivers {#sec-ondevice-learning-deployment-drivers-1919}
...
### On-Device Learning Benefits {#sec-ondevice-learning-ondevice-learning-benefits-c991}
...
### Application Domains {#sec-ondevice-learning-application-domains-b718}
...
::: {.callout-quiz-question #quiz-question-sec-ondevice-learning-deployment-drivers-1919}

1. Which of the following is NOT a primary motivation for on-device learning?
   a) Personalization
   b) Latency and availability
   c) Centralized data aggregation
   d) Privacy

2. Explain how on-device learning can improve privacy compared to centralized learning.

3. On-device learning allows models to adapt using ____ data, which is often unavailable to centralized infrastructure.

4. True or False: On-device learning can operate effectively without any connectivity to centralized infrastructure.

5. 
   a) Model deployment to client devices,
   b) Local data collection and model adaptation,
   c) Centralized training on aggregated data.

See Answer \ref{quiz-answer-sec-ondevice-learning-deployment-drivers-1919}.
...
1. Which of the following is a primary challenge of on-device learning compared to cloud-based learning?
   a) Access to large, curated datasets
   b) Availability of extensive compute resources
   c) Limited memory and energy resources
   d) High-speed internet connectivity

2. Explain why non-IID data is a concern for on-device learning systems.

3. On-device learning systems often use techniques such as ____ to reduce memory and compute requirements while maintaining performance.

4. True or False: On-device learning systems can rely on traditional deep learning libraries without modification.

5. 
   a) Quantization,
   b) Meta-training with generic data,
   c) Ranking and selecting layers for adaptation.

See Answer \ref{quiz-answer-sec-ondevice-learning-design-constraints-51d3}.

:::

## Model Adaptation {#sec-ondevice-learning-model-adaptation-661a}
...
### Weight Freezing {#sec-ondevice-learning-weight-freezing-ef6c}
...
### Residual and Low-Rank Updates {#sec-ondevice-learning-residual-lowrank-updates-a096}
...
#### Adapter-Based Adaptation {#sec-ondevice-learning-adapterbased-adaptation-91d6}
...
#### Low-Rank Techniques {#sec-ondevice-learning-lowrank-techniques-2954}
...
### Sparse Updates {#sec-ondevice-learning-sparse-updates-0cf7}
...
#### Sparse Update Design {#sec-ondevice-learning-sparse-update-design-b2b7}
...
#### Layer Selection {#sec-ondevice-learning-layer-selection-6539}
...
::: {.callout-quiz-question #quiz-question-sec-ondevice-learning-model-adaptation-661a}

1. Which of the following adaptation strategies is most suitable for devices with extreme memory and compute limitations?
   a) Bias-only updates
   b) Residual adapters
   c) Sparse layer updates
   d) Full model fine-tuning

2. Explain why residual adapters might be preferred over bias-only updates in a mobile device scenario.

3. Task-adaptive sparse updates require a mechanism for ____ to determine which layers or parameters to update.

4. True or False: Low-rank updates increase the number of trainable parameters compared to full model fine-tuning.

5. Discuss the tradeoffs involved in using task-adaptive sparse updates for on-device learning.

See Answer \ref{quiz-answer-sec-ondevice-learning-model-adaptation-661a}.
...
#### Adaptation Strategy Comparison {#sec-ondevice-learning-adaptation-strategy-comparison-4e76}
...
## Data Efficiency {#sec-ondevice-learning-data-efficiency-7b59}
...
### Few-Shot and Streaming {#sec-ondevice-learning-fewshot-streaming-01d1}
...
### Experience Replay {#sec-ondevice-learning-experience-replay-d2ff}
...
### Data Compression {#sec-ondevice-learning-data-compression-773a}
...
### Tradeoffs Summary {#sec-ondevice-learning-tradeoffs-summary-ec4b}
...
::: {.callout-quiz-question #quiz-question-sec-ondevice-learning-data-efficiency-7b59}

1. Which technique is most suitable for adapting models on devices with extremely limited memory and compute resources?
   a) Full model finetuning
   b) Few-shot adaptation
   c) Experience replay
   d) Batch training with large datasets

2. Explain why experience replay is beneficial for on-device learning in non-stationary environments.

3. True or False: Compressed data representations can help reduce the memory footprint of on-device learning systems.

4. In on-device learning, ____ adaptation allows models to update continuously as new data arrives, without requiring access to prior data.

5. A device uses a replay buffer with a capacity of 100 samples. If it stores compressed feature vectors of 256 bytes each, what is the total memory usage of the buffer in kilobytes?

See Answer \ref{quiz-answer-sec-ondevice-learning-data-efficiency-7b59}.
...
### Learning Protocols {#sec-ondevice-learning-learning-protocols-4207}
...
#### Protocols Overview {#sec-ondevice-learning-protocols-overview-e723}
...
#### Efficient Communication {#sec-ondevice-learning-efficient-communication-d9c5}
...
#### Federated Personalization {#sec-ondevice-learning-federated-personalization-6852}
...
1. Which of the following is a primary advantage of federated learning over traditional centralized learning?
   a) Reduced model complexity
   b) Improved data privacy
   c) Increased training speed
   d) Simplified deployment

2. Explain how client scheduling impacts the performance and fairness of federated learning systems.

3. In federated learning, the process of aggregating model updates from multiple clients is known as ____. This process is crucial for creating a global model that benefits from diverse data without compromising individual privacy.

4. True or False: Federated learning completely eliminates the risk of data leakage.

5. A federated learning system involves 100 devices, each with a local dataset of 500 samples. If each device performs 10 local training steps per round and the average model update size is 1MB, calculate the total data transmitted to the server in one round in GB.
...
1. Which of the following strategies is most suitable for minimizing the adaptation footprint in on-device learning systems?
   a) Full-model fine-tuning
   b) Bias-only optimization
   c) Centralized training
   d) Heavy adaptation with large heads

2. True or False: Opportunistic scheduling in on-device learning systems helps in reducing the impact on device performance and user experience.

3. Explain why lightweight encryption is important for on-device learning systems.

4. In on-device learning, retaining trusted model checkpoints is crucial for enabling ____ in case of adaptation failures.

See Answer \ref{quiz-answer-sec-ondevice-learning-practical-system-design-2a97}.

:::

## Challenges {#sec-ondevice-learning-challenges-cf40}
...
#### Challenges Summary {#sec-ondevice-learning-challenges-summary-0c18}
...
::: {.callout-quiz-question #quiz-question-sec-ondevice-learning-challenges-cf40}

1. Which of the following best describes a challenge posed by hardware heterogeneity in on-device learning systems?
   a) Uniform training environments across all devices
   b) Standardized software stacks and runtime libraries
   c) Diverse hardware capabilities affecting model deployment
   d) Consistent network connectivity and power availability

2. Explain how data fragmentation challenges the generalization of on-device learning models.

3. In on-device learning, the absence of centralized validation data complicates the process of ____ and monitoring model updates.

4. True or False: Resource contention in on-device learning systems is primarily due to the need for additional storage space.

5. Discuss how deployment risks in on-device learning systems can affect regulatory compliance.

See Answer \ref{quiz-answer-sec-ondevice-learning-challenges-cf40}.
...
:::{.callout-quiz-answer #quiz-answer-sec-ondevice-learning-deployment-drivers-1919}
...
1. **Which of the following is NOT a primary motivation for on-device learning?**
   ...
   The correct answer is C. Centralized data aggregation is not a motivation for on-device learning; instead, it is a characteristic of traditional centralized learning systems.

   *Learning Objective*: Understand the key motivations driving the shift to on-device learning.

2. **Explain how on-device learning can improve privacy compared to centralized learning.**

   On-device learning improves privacy by keeping raw data on the device, reducing the need to transmit sensitive information to the cloud. This approach aligns with privacy regulations such as GDPR and HIPAA, as it minimizes data exposure and potential compliance issues.

   *Learning Objective*: Analyze the privacy benefits of on-device learning in comparison to centralized learning.

3. **On-device learning allows models to adapt using ____ data, which is often unavailable to centralized infrastructure.**

   The answer is "local". On-device learning leverages local data for model adaptation, enabling personalization and reducing reliance on cloud-based data aggregation.

   *Learning Objective*: Recall the type of data used in on-device learning and its significance.

4. **True or False: On-device learning can operate effectively without any connectivity to centralized infrastructure.**

   True. On-device learning is designed to function autonomously, allowing models to adapt locally even in fully offline or delay-sensitive environments, which is crucial for scenarios with unreliable connectivity.

   *Learning Objective*: Understand the operational independence of on-device learning from centralized infrastructure.

5. **Order the following steps in the transition from centralized to on-device learning: A) Model deployment to client devices, B) Local data collection and model adaptation, C) Centralized training on aggregated data.**

   C, A, B. Centralized training on aggregated data occurs first, followed by model deployment to client devices, and finally, local data collection and model adaptation occur on-device.

   *Learning Objective*: Understand the sequence of processes in transitioning from centralized to on-device learning.
...
1. **Which of the following is a primary challenge of on-device learning compared to cloud-based learning?**
   ...
   The correct answer is C. On-device learning is challenged by limited memory and energy resources, which are not typically constraints in cloud-based environments.

   *Learning Objective*: Understand the primary resource constraints that differentiate on-device learning from cloud-based learning.

2. **Explain why non-IID data is a concern for on-device learning systems.**

   Non-IID data is a concern because it can lead to models that do not generalize well across different devices or users. The data collected on-device is often specific to individual users or environments, which may not represent the overall distribution used during initial training, complicating model adaptation and convergence.

   *Learning Objective*: Analyze the impact of non-IID data on the performance and generalizability of on-device learning systems.

3. **On-device learning systems often use techniques such as ____ to reduce memory and compute requirements while maintaining performance.**

   The answer is "quantization". Quantization reduces the precision of model parameters, allowing models to fit within the limited memory and compute resources available on-device, while still maintaining acceptable performance.

   *Learning Objective*: Recall techniques used to optimize models for on-device learning environments.

4. **True or False: On-device learning systems can rely on traditional deep learning libraries without modification.**

   False. On-device learning systems often require modifications to traditional deep learning libraries to accommodate resource constraints, such as limited memory and the absence of floating-point operations.

   *Learning Objective*: Understand the necessity of adapting deep learning libraries for resource-constrained environments.

5. **Order the following steps in adapting a model for on-device learning: A) Quantization, B) Meta-training with generic data, C) Ranking and selecting layers for adaptation.**

   B, A, C. First, the model undergoes meta-training with generic data. Then, quantization is applied to reduce the model's resource requirements. Finally, layers are ranked and selected for adaptation based on device-specific constraints.

   *Learning Objective*: Understand the process of adapting a model for on-device learning, considering resource constraints.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ondevice-learning-model-adaptation-661a}
...
1. **Which of the following adaptation strategies is most suitable for devices with extreme memory and compute limitations?**
   ...
   The correct answer is A. Bias-only updates are most suitable for devices with extreme memory and compute limitations because they only update scalar offsets, significantly reducing memory and computational requirements.

   *Learning Objective*: Understand which adaptation strategies are suitable for specific hardware constraints.

2. **Explain why residual adapters might be preferred over bias-only updates in a mobile device scenario.**

   Residual adapters might be preferred because they offer greater flexibility and expressivity while maintaining control over adaptation cost. They allow for user-specific tuning, which is beneficial in mobile scenarios where moderate adaptation capacity is needed.

   *Learning Objective*: Analyze the benefits of using residual adapters over bias-only updates in specific scenarios.

3. **Task-adaptive sparse updates require a mechanism for ____ to determine which layers or parameters to update.**

   The answer is "layer selection". Task-adaptive sparse updates require a mechanism for layer selection to determine which layers or parameters to update based on their contribution to performance.

   *Learning Objective*: Understand the requirements for implementing task-adaptive sparse updates.

4. **True or False: Low-rank updates increase the number of trainable parameters compared to full model fine-tuning.**

   False. Low-rank updates reduce the number of trainable parameters by approximating updates with low-rank matrices, thereby decreasing the computational load compared to full model fine-tuning.

   *Learning Objective*: Understand the impact of low-rank updates on trainable parameters and computational load.

5. **Discuss the tradeoffs involved in using task-adaptive sparse updates for on-device learning.**

   Task-adaptive sparse updates offer high expressivity and allow for task-specific fine-tuning, but they require a complex mechanism for layer selection and profiling. They can adapt to large domain shifts but introduce overhead in contribution analysis and require careful validation to ensure sufficient adaptation capacity.

   *Learning Objective*: Evaluate the tradeoffs of using task-adaptive sparse updates in on-device learning scenarios.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ondevice-learning-data-efficiency-7b59}
...
1. **Which technique is most suitable for adapting models on devices with extremely limited memory and compute resources?**
   ...
   The correct answer is B. Few-shot adaptation is designed to work with a small number of labeled examples and minimal compute resources, making it suitable for devices with limited capacity.

   *Learning Objective*: Understand which data-efficient techniques are suitable for resource-constrained environments.

2. **Explain why experience replay is beneficial for on-device learning in non-stationary environments.**

   Experience replay helps mitigate catastrophic forgetting by allowing models to retrain on past examples, thus improving stability in non-stationary environments where data distribution may change over time.

   *Learning Objective*: Analyze the benefits of experience replay in maintaining model performance over time.

3. **True or False: Compressed data representations can help reduce the memory footprint of on-device learning systems.**

   True. Compressed data representations transform raw inputs into lower-dimensional embeddings, reducing memory usage and enabling efficient adaptation.

   *Learning Objective*: Understand the role of compressed data representations in managing memory constraints.

4. **In on-device learning, ____ adaptation allows models to update continuously as new data arrives, without requiring access to prior data.**

   The answer is "streaming". Streaming adaptation enables models to learn incrementally from new data, adapting in real-time without needing to store large datasets.

   *Learning Objective*: Recall the concept of streaming adaptation and its application in real-time learning scenarios.

5. **A device uses a replay buffer with a capacity of 100 samples. If it stores compressed feature vectors of 256 bytes each, what is the total memory usage of the buffer in kilobytes?**

   The total memory usage is 25.6 KB. This is calculated by multiplying the number of samples (100) by the size of each compressed feature vector (256 bytes), then converting bytes to kilobytes (100 * 256 / 1024 = 25.6 KB). This calculation helps in understanding memory constraints in on-device learning systems.

   *Learning Objective*: Calculate memory requirements for replay buffers in on-device learning systems.
...
1. **Which of the following is a primary advantage of federated learning over traditional centralized learning?**
   ...
   The correct answer is B. Federated learning improves data privacy by keeping raw data on the device and only sharing model updates, unlike centralized learning which requires data aggregation at a central location.

   *Learning Objective*: Understand the privacy benefits of federated learning compared to centralized learning.

2. **Explain how client scheduling impacts the performance and fairness of federated learning systems.**

   Client scheduling affects which devices participate in training, impacting model convergence and fairness. Poor scheduling can lead to participation bias, where only devices with favorable conditions contribute, skewing the model towards certain user groups. Effective scheduling ensures diverse and representative client participation, balancing system performance and fairness.

   *Learning Objective*: Analyze the impact of client scheduling on federated learning system performance and fairness.

3. **In federated learning, the process of aggregating model updates from multiple clients is known as ____. This process is crucial for creating a global model that benefits from diverse data without compromising individual privacy.**

   The answer is "Federated Averaging". This process is crucial for creating a global model that benefits from diverse data without compromising individual privacy.

   *Learning Objective*: Recall the key process used in federated learning for aggregating model updates.

4. **True or False: Federated learning completely eliminates the risk of data leakage.**

   False. While federated learning reduces data leakage by keeping raw data on devices, model updates can still leak information about local data. Techniques like secure aggregation and differential privacy are needed to mitigate these risks.

   *Learning Objective*: Recognize the limitations of federated learning in terms of data privacy and security.

5. **A federated learning system involves 100 devices, each with a local dataset of 500 samples. If each device performs 10 local training steps per round and the average model update size is 1MB, calculate the total data transmitted to the server in one round in GB.**

   The total data transmitted is 100 devices * 1MB = 100MB. In GB, this is 0.1GB. This calculation highlights the communication cost in federated learning and the importance of efficient update mechanisms.

   *Learning Objective*: Calculate and understand the communication overhead in federated learning systems.
...
1. **Which of the following strategies is most suitable for minimizing the adaptation footprint in on-device learning systems?**
   ...
   The correct answer is B. Bias-only optimization is suitable for minimizing the adaptation footprint as it involves updating only a small part of the model, reducing resource usage while still allowing for personalization.

   *Learning Objective*: Understand the importance of minimizing adaptation footprint in on-device learning systems.

2. **True or False: Opportunistic scheduling in on-device learning systems helps in reducing the impact on device performance and user experience.**

   True. Opportunistic scheduling defers local updates to periods when the device is idle or connected to power, minimizing the impact on latency, battery consumption, and thermal performance.

   *Learning Objective*: Recognize the role of opportunistic scheduling in maintaining device performance and user experience.

3. **Explain why lightweight encryption is important for on-device learning systems.**

   Lightweight encryption is important for on-device learning systems to protect sensitive data such as replay buffers and adaptation logs from unauthorized access while imposing minimal resource overhead. This ensures data security without significantly impacting device performance.

   *Learning Objective*: Understand the importance of security measures in protecting data in on-device learning systems.

4. **In on-device learning, retaining trusted model checkpoints is crucial for enabling ____ in case of adaptation failures.**

   The answer is "rollback mechanisms". Retaining trusted model checkpoints allows systems to revert to a known-good state if local adaptations lead to unacceptable behavior, ensuring robustness and reliability.

   *Learning Objective*: Understand the role of model checkpoints in ensuring system robustness and reliability.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ondevice-learning-challenges-cf40}
...
1. **Which of the following best describes a challenge posed by hardware heterogeneity in on-device learning systems?**
   ...
   The correct answer is C. Diverse hardware capabilities affecting model deployment. Hardware heterogeneity introduces complexity in deploying models across devices with varying memory, processing power, and AI accelerators, impacting the feasibility and efficiency of on-device learning.

   *Learning Objective*: Understand the implications of hardware heterogeneity on the deployment of on-device learning systems.

2. **Explain how data fragmentation challenges the generalization of on-device learning models.**

   Data fragmentation results in non-IID data distributions, where each device collects unique user-specific data. This can lead to models overfitting to local idiosyncrasies, reducing their ability to generalize across different devices and contexts.

   *Learning Objective*: Analyze the impact of data fragmentation on model generalization in on-device learning systems.

3. **In on-device learning, the absence of centralized validation data complicates the process of ____ and monitoring model updates.**

   The answer is "validation". Without centralized validation data, it is challenging to ensure that model updates are beneficial, as models adapt based on local inputs without systematic evaluation.

   *Learning Objective*: Recognize the challenges in validating and monitoring model updates in decentralized learning environments.

4. **True or False: Resource contention in on-device learning systems is primarily due to the need for additional storage space.**

   False. Resource contention arises from competing demands for compute cycles, memory bandwidth, and energy, as training workloads require more resources than inference-only deployments.

   *Learning Objective*: Understand the nature of resource contention in on-device learning systems and its impact on system performance.

5. **Discuss how deployment risks in on-device learning systems can affect regulatory compliance.**

   Deployment risks include the loss of centralized oversight, which complicates validation and rollback. This can lead to models drifting outside of compliance boundaries, making it difficult to demonstrate consistent behavior required for regulatory approval.

   *Learning Objective*: Evaluate the impact of deployment risks on regulatory compliance in on-device learning systems.
