...
4. **In on-device learning, the shift from centralized to decentralized learning is primarily driven by the need for ____.**
   ...
   The answer is "local adaptation". Local adaptation allows models to refine their behavior based on user-specific data and dynamic conditions, which is not feasible with static, centralized models.
...
3. **In on-device learning, the lack of support for ________ operations in embedded systems necessitates the use of integer arithmetic and minimal memory allocation.**
   ...
   The answer is "floating-point". Embedded systems often lack hardware support for floating-point operations, requiring models to be designed for integer arithmetic to fit within memory and power constraints.
...
4. **In on-device learning, ________ adaptation allows models to learn incrementally as new data arrives, without access to prior data.**
   ...
   The answer is "streaming". Streaming adaptation enables models to update continuously with new data, important for adapting to changing environments in real-time.
...
4. **In federated learning, the process of ________ involves devices performing local training and sending model updates to a central server for aggregation.**
   ...
   The answer is "model aggregation". In federated learning, model aggregation involves collecting and combining model updates from multiple devices to refine a global model, enabling decentralized training while preserving data privacy.
...
4. **In on-device learning, retaining trusted model checkpoints is important for ________ in safety-critical domains.**
   ...
   The answer is "failure recovery". Retaining trusted model checkpoints allows for rapid restoration of a known-good baseline version of the model if adaptation leads to unacceptable behavior, which is important for failure recovery in safety-critical domains.
...
4. **In on-device learning, the absence of centralized monitoring makes it difficult to ensure ________ and detect performance regressions.**
   ...
   The answer is "update validation". Without centralized monitoring, it's challenging to validate updates or detect performance regressions, as models adapt locally and may drift without external oversight.
