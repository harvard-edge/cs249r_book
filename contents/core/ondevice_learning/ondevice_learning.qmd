...
Moreover, availability-driven selection introduces participation bias: clients with favorable conditions—such as frequent charging, high-end hardware, or consistent connectivity—are more likely to participate repeatedly, while others are systematically underrepresented. This can skew the resulting model toward behaviors and preferences of a privileged subset of the population, raising both fairness and generalization concerns. Systems must therefore balance scheduling efficiency with statistical representativeness, often using stratified or quota-based sampling techniques. To avoid the challenges of exact synchonous participation, FedBuff [@fedbuff] incorporates an asynchronous buffer which participating clients can push model updates into: once the buffer is filled, a global model update is computed, therby not requiring participants to provide an update every single round. Followup works such as FedStale [@fedstale] and FedStaleWeight [@fedstaleweight] examine how to incorporate staleness and fairness into this asynchronous buffer, as more active participants may bias central training towards their data.
