...
NAS addresses these limitations by automating model design. Instead of manually tuning configurations, NAS systematically explores a large space of architectures to identify those that best balance accuracy, computational cost, memory efficiency, and inference latency. By framing model selection as a structured search problem, NAS reduces reliance on trial and error, allowing architectures to be discovered programmatically rather than heuristically.
...
Quantization plays a crucial role in optimizing machine learning models for low-precision hardware deployment by reducing both model size and computational demands. The decision between PTQ and QAT hinges on balancing accuracy, computational cost, and deployment constraints.

PTQ is typically preferred when retraining is impractical or unnecessary, given its low computational overhead stemming from its post-training application. It is effective for models that can tolerate some degree of quantization error, exemplified by many CNN-based image classifiers. However, for models—especially transformers in NLP—where numerical precision is paramount, PTQ may degrade accuracy significantly.

In contrast, QAT is essential when preserving high accuracy is critical. Integrating quantization into training enables the model to adjust its weight distributions to better cope with low-precision arithmetic, substantially mitigating quantization errors. Despite the increased training cost and complexity, QAT is particularly valuable for accuracy-sensitive applications. A hybrid approach, initiating with PTQ and then selectively applying QAT for critical components, can often achieve an optimal balance between efficiency and performance.

### Binarization and Ternarization

Extreme precision reduction techniques such as binarization and ternarization aim to reduce the bit-width of weights and activations drastically. Representing values with only one or two bits (binary or ternary representations, respectively) achieves significant memory savings and reduced computational requirements, making these techniques especially attractive for deployment in hardware-constrained environments.

Binarization restricts weights and activations to just two values—typically -1 and +1, or 0 and 1—depending on the specific approach. Its primary advantage lies in the substantial reduction of model size, allowing models to fit into very small memory footprints. This reduction can markedly accelerate inference, particularly on specialized hardware designed for binary neural networks (BNNs). However, binarization poses significant challenges in maintaining model accuracy, as limiting values to only two options greatly diminishes the model’s expressiveness. Non-differentiable operations introduced during binarization further complicate optimization, though techniques such as the STE can approximate gradients to allow effective training.
...
Another critical aspect of AutoML is model compression. Techniques such as pruning and quantization help reduce the memory footprint and computational requirements of a model, making it more suitable for deployment on resource-constrained hardware. AutoML frameworks automate the selection of pruning thresholds, sparsity patterns, and quantization levels, optimizing models for both speed and energy efficiency. This is particularly important for edge AI applications, where models need to operate with minimal latency and power consumption.

AutoML also extends optimization beyond the model itself, incorporating data processing and augmentation strategies. High-quality training data is essential for achieving good generalization, and AutoML frameworks can automatically select data augmentation techniques that enhance robustness while maintaining efficiency. By systematically evaluating different augmentation policies, these frameworks help improve performance without increasing model complexity.
