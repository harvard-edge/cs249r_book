...
This case illustrates how silent data corruption can propagate across multiple layers of the application stack, resulting in data loss and application failures in large-scale distributed systems. Left unaddressed, such errors can degrade ML system performance. For example, corrupted training data or inconsistencies in data pipelines due to SDC may compromise model accuracy and reliability. Similar challenges have been reported by other major companies. As shown in @fig-sdc-jeffdean, [Jeff Dean[^fn-jeffdean]](https://en.wikipedia.org/wiki/Jeff_Dean), Chief Scientist at Google DeepMind and Google Research, highlighted these issues in AI hypercomputers during a keynote at [MLSys 2024](https://mlsys.org/).

[^fn-jeffdean]: **Jeff Dean:** A computer scientist known for pioneering work in distributed systems, machine learning, and large-scale computing infrastructure. As a key figure at Google since 1999, he has shaped fundamental technologies in AI and distributed computing that have influenced industry practices worldwide.
...
The causes of intermittent faults are diverse, ranging from physical degradation to environmental influences. One common cause is the aging and wear-out of electronic components. As hardware endures prolonged operation, degradation mechanisms such as electromigration[^fn-electromigration], oxide breakdown[^fn-oxide-breakdown], or solder joint fatigue[^fn-solder-joint-fatigue] can create marginal electrical paths that fail under certain conditions.

[^fn-electromigration]: **Electromigration:** Gradual movement of atoms in a conductor caused by the momentum transfer between conducting electrons and diffusing metal atoms.
[^fn-oxide-breakdown]: **Oxide breakdown:** Failure of insulating properties of metal oxides used in electronic devices due to high electric field.
[^fn-solder-joint-fatigue]: **Solder joint fatigue:** Gradual mechanical failure of a solder joint, which commonly involves crack formation and growth due to fluctuating strains and stresses.
...
![The curly brackets enclose the distribution shift between the environments. Here, z stands for the spurious feature, and y stands for label class. Source: [@xin2023connection]](./images/png/distribution_shift.png){#fig-distribution-shift}
...
![Concept drift refers to a change in data patterns and relationships over time. Source: [@evidently2023drift]](./images/png/drift_over_time.png){#fig-drift-over-time}
...
![Temporal evolution. Source: [@bialek2023nannyml]](./images/png/temporal_evoltion.png){#fig-temporal-evoltion}
