...
Robust AI refers to a system’s ability to maintain performance and reliability in the presence of errors. A robust machine learning system is designed to be fault-tolerant and error-resilient, capable of functioning effectively even under adverse conditions.
...
ML systems are expected to play critical roles in autonomous vehicles, smart cities, healthcare, and industrial automation. In these domains, the consequences of hardware or software faults can be severe, potentially resulting in loss of life, economic disruption, or environmental harm.
...
This chapter focuses specifically on three primary categories of faults and errors that can impact the robustness of ML systems: hardware faults, software faults, and human errors.

* **Hardware Faults:** Transient, permanent, and intermittent faults can affect hardware components, leading to corrupted computations and degraded system performance.

* **Model Robustness:** ML models may be vulnerable to adversarial attacks, data poisoning, and distribution shifts, which can cause misclassifications, distort learned behavior, or undermine the system’s integrity and reliability.

* **Software Faults:** Bugs, design flaws, and implementation errors in software components—such as algorithms, libraries, and frameworks—can propagate faults and introduce systemic vulnerabilities.

The specific challenges and mitigation strategies for robustness vary depending on the scale and constraints of the ML system. Large-scale cloud computing or data center environments may emphasize fault tolerance through redundancy, distributed processing, and sophisticated error detection and correction mechanisms. In contrast, resource-constrained edge devices or embedded systems must address robustness within limited computational, memory, and energy budgets.
...
In May 2016, a fatal accident occurred when a Tesla Model S operating on Autopilot crashed into a white semi-trailer truck crossing the highway. The Autopilot system, which relied on computer vision and machine learning algorithms, failed to recognize the white trailer against a bright sky background. The driver, who was reportedly watching a movie when the crash, did not intervene in time, and the vehicle collided with the trailer at full speed as shown in [@fig-tesla-example]. This incident raised concerns about the limitations of AI-based perception systems and the need for robust failsafe mechanisms in autonomous vehicles. Similarly in March 2018, an Uber self-driving test [vehicle struck](https://money.cnn.com/2018/03/19/technology/uber-autonomous-car-fatal-crash/index.html?iid=EL) and killed a pedestrian crossing the street in Tempe, Arizona. The incident was caused by a software flaw in the vehicle's object recognition system, which failed to identify the pedestrians appropriately to avoid them as obstacles.
...
:::{#exr-aa .callout-caution collapse="true" title="Adversarial Attacks"}
...
:::{#exr-pa .callout-caution collapse="true" title="Poisoning Attacks"}
