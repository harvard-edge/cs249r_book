...
3. **In the context of Keyword Spotting (KWS), achieving a specific accuracy rate, ensuring low latency, and minimizing power consumption are examples of setting clear ________.**
   ...
   objectives. Setting clear objectives helps define measurable outcomes and performance benchmarks for the ML system.
...
3. **In an ML data pipeline, the process of converting raw data into a format suitable for model training is known as ____. **
   ...
   transformation. Transformation involves converting raw data into a structured format that can be used effectively in training ML models.
...
3. **When using pre-existing datasets, ML practitioners must be cautious of ________, which can lead to overfitting and misalignment with real-world conditions.**
   ...
   biases. Biases in pre-existing datasets can cause models to perform well on benchmark tests but poorly in real-world applications due to overfitting to specific dataset characteristics.
...
3. **In a data ingestion pipeline, ________ queues are used to store data that fails processing for later analysis and reprocessing.**
   ...
   dead letter. Dead letter queues hold unprocessed data for analysis, helping identify and address issues in data ingestion pipelines.
...
4. **In a labeling pipeline, ________ is used to identify which data points are most valuable for human annotation, optimizing the use of resources.**
   ...
   active learning. Active learning prioritizes data points based on model uncertainty, ensuring that human annotation efforts focus on the most informative examples to improve model performance efficiently.
...
3. **In ML workflows, ________ are used to absorb large, bursty I/O operations during training, smoothing performance fluctuations.**
   ...
   burst buffers. Burst buffers are high-speed storage layers that help manage large, temporary I/O demands in ML workflows, particularly during data-intensive operations like training.
...
4. **In ML systems, ________ are used to document data lineage, characteristics, and potential biases, promoting transparency and responsible use.**
   ...
   Data Cards. Data Cards provide structured documentation of datasets, detailing their characteristics, limitations, and potential biases, which helps in evaluating datasets and promoting responsible use.
