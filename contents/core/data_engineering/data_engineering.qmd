...
### Keyword Spotting (KWS) Case Study
...
The KWS example demonstrates how carefully defining the problem upfront shapes all subsequent data engineering decisions. By systematically working through requirements—from core functionality to deployment constraints—teams establish a robust foundation for their ML systems. This methodical approach to problem definition provides a framework that translates across different ML applications, whether building medical imaging systems, recommendation engines, or language models.

The following sections continue using KWS as a running example to illustrate how these foundational concepts manifest in practice. The examples examine how choices in data collection, preprocessing, labeling, and storage all trace back to initial requirements. This leads to data pipelines—the infrastructure that transforms raw data into ML-ready formats while maintaining quality and reliability. These pipelines implement defined requirements in production systems, managing everything from initial ingestion through feature generation.
...
[^data-bias]: Biases can infiltrate all stages of the ML workflow from data collection and feature engineering through model training and deployment. Each stage presents opportunities for bias to be introduced or amplified. The impacts of bias and approaches for mitigation are covered in depth in later chapters.
...
While data engineering encompasses many aspects of preparing data for machine learning systems, data labeling represents a particularly complex systems challenge. As training datasets grow to millions or billions of examples,[^millions] the infrastructure supporting labeling operations becomes increasingly critical to system performance.

[^millions]: Modern ML models often contain millions or billions of parameters to capture complex data patterns. A general rule of thumb in ML is to have a training dataset that is at least 10 times larger than the model's parameter count to ensure robust learning and avoid overfitting.
