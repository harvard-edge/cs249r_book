...
Using this KWS as an example, we can break each of the steps out as follows:
...
The KWS example illustrates the broader principles of problem definition in ML systems, showing how initial decisions about data requirements ripple throughout a project's lifecycle. By carefully considering each aspect - from core problem identification through performance benchmarks to deployment constraints - teams can build a strong foundation for their ML systems. This methodical approach helps prevent the data cascades described by @sambasivan2021everyone, where early oversights in data requirements compound into serious issues during model development and deployment.

When teams invest time in thorough problem definition, they're better equipped to anticipate data needs before they become bottlenecks and align stakeholder expectations around both data quality and system performance. This upfront investment creates a clear roadmap for data collection and processing, establishing concrete metrics to gauge progress and success. The structured approach demonstrated through the KWS example applies equally well to other ML domains, from computer vision systems processing medical images to recommendation engines handling millions of user interactions.

With a clear understanding of our data requirements established through careful problem definition, we can turn our attention to one of the most fundamental challenges in building ML systems: data sourcing. While our problem definition tells us what data we need, finding or creating that data brings its own set of challenges and considerations. From pre-existing datasets to web scraping, from crowdsourcing to synthetic data generation, each sourcing method offers distinct advantages and limitations that must be carefully weighed against our defined requirements.

## Data Sourcing

Now that we have clearly defined our ML system's requirements through careful problem definition, the next challenge lies in sourcing appropriate data to meet these needs. The quality and diversity of our training data will fundamentally determine our ML system's capabilities and limitations. As we saw with @sambasivan2021everyone's data cascades, poor data sourcing decisions early in development can lead to compounding issues throughout a system's lifecycle.

ML systems can obtain their training data through several different approaches, each with their own advantages and challenges. Let's examine each of these approaches in detail.
...
However, ML practitioners must carefully consider the quality assurance aspects of pre-existing datasets. For instance, [the ImageNet dataset was found to have over 6.4% errors](https://arxiv.org/abs/2103.14749). While popular datasets benefit from community scrutiny that helps identify and correct errors and biases, these issues can significantly impact system performance if not properly addressed. 
...
A critical consideration for ML systems is how well pre-existing datasets reflect real-world deployment conditions. As illustrated in @fig-misalignment, relying on standard datasets can create a concerning disconnect between training and production environments. This misalignment becomes particularly problematic when multiple ML systems are trained on the same datasets, potentially propagating biases and limitations throughout an entire ecosystem of deployed models.
