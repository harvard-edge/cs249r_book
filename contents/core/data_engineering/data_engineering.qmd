...
\end{tikzpicture}}
```
**Data Science Time Allocation**: The disproportionate effort data scientists spend on data cleaning and preparation—up to 60%—underscoring the critical importance of data engineering for successful machine learning systems. this allocation emphasizes that robust data pipelines and quality control are often more impactful than complex model architectures.
:::
...
\end{tikzpicture}
```
**Data Cascade Effects**: How initial data quality issues compound across ML workflow stages—from collection to deployment—resulting in increased costs, flawed predictions, and potential harm. understanding these cascades motivates proactive investment in data engineering and quality control to mitigate risks and ensure reliable system performance. *source: @sambasivan2021everyone.*
:::
...
\end{tikzpicture}}
```
**Data Pipeline Stages**: The modular architecture of a typical machine learning data pipeline, progressing from raw data sources through ingestion, storage, and preparation for model training and validation. this layered design enables independent scaling, improved data quality control, and facilitates the reliable delivery of data to ML models.
:::
...
\end{tikzpicture}
```
**Model Convergence** : Training multiple models on a shared dataset can mask underlying limitations and propagate biases, potentially leading to overoptimistic performance evaluations and reduced generalization to novel data. This figure illustrates how reliance on common datasets may create a false sense of progress across an ecosystem of ML systems, hindering robust deployment.
:::
...
![**Historical Data Artifacts**: Web scraping can return unexpected or irrelevant data—like images of early traffic lights in a modern image search—highlighting the need for careful data validation and cleaning in ML pipelines. *source: vox.*](images/jpg/1914_traffic.jpeg){#fig-traffic-light}
...
```
**Data Augmentation With Synthesis**: Combining historical data with algorithmically generated synthetic data expands training set size and diversity, addressing limitations in real-world data availability and improving model robustness. *source: [anylogic](HTTPS://www.anylogic.com/features/artificial-intelligence/synthetic-data/).*
:::
...
\end{tikzpicture}
```
**Data Pipeline Architectures**: Compares ETL and ELT approaches to data ingestion, highlighting how transformations are positioned relative to data loading to impact system flexibility and resource utilization.
:::
...
![**Audio Feature Extraction**: How raw audio waveforms are transformed into spectrograms and then into mel-frequency cepstral coefficients (mfccs), creating a compact, perceptually relevant representation for keyword spotting and other audio tasks. this process reduces noise and variability while preserving essential speech characteristics for improved model performance.](images/png/kws_spectrogram.png){#fig-spectrogram-example}
...
![**Label Types for Machine Learning**: How data annotation granularity impacts model training—from broad bounding boxes to detailed pixel-level segmentation—and highlights the trade-offs between annotation cost and model accuracy.](images/png/CS249r_Labels_new.png){#fig-labels width=90%}
...
\end{tikzpicture}
```
**Weak Supervision Strategies**: How AI-powered techniques, including programmatic labeling, distant supervision, and active learning, can augment limited labeled data to scale machine learning systems beyond manual annotation efforts. these strategies trade off potential labeling errors for gains in throughput, requiring careful system design to balance labeling speed, cost, and model quality. *source: stanford AI lab.*
:::
...
![**Multilingual Data Pipeline**: Paired audio-text data undergoes forced alignment and segmentation to generate a large-scale, multilingual dataset for keyword spotting model training. This automated process efficiently creates labeled examples from readily available speech resources, enabling scalable development of robust KWS systems.](images/png/data_engineering_kws2.png){#fig-mswc}
...
: **Storage System Trade-Offs**: Compares conventional databases, data warehouses, and data lakes based on their purpose and data type, highlighting how each system suits different stages of a machine learning workflow—from operational transactions to analytical reporting and raw data storage. {#tbl-storage .striped .hover}
...
\end{tikzpicture}
```
**Feature Store Architecture**: How a feature store centralizes feature engineering and serving, enabling consistent feature access for both model training and real-time inference, and promoting feature reuse across teams and projects. this architecture supports time-travel capabilities, ensuring point-in-time correctness for historical analysis and consistent model behavior.
:::
...
\end{tikzpicture}}
```
**Data Governance Pillars**: The core practices (privacy, fairness, transparency, and accountability) necessary to build and deploy reliable, ethical machine learning systems throughout the data lifecycle. These pillars address the unique challenges of ML workflows by ensuring data is used responsibly and decisions are explainable and auditable.
:::
...
\end{tikzpicture}
```
**Data Governance Documentation**: A data card, which is a standardized method for documenting critical information about a dataset, is useful in promoting transparency, accountability, and responsible AI practices, as required by regulations like GDPR and HIPAA. Data cards facilitate data governance by providing a structured overview of dataset characteristics, intended uses, and potential risks.
:::
