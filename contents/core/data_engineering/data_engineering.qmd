...
## Overview {#sec-data-engineering-overview-9090}
...
\end{tikzpicture}}
```
Data scientists spend most of their time on cleaning and organizing data.
:::
...
## Problem Definition {#sec-data-engineering-problem-definition-2348}
...
\end{tikzpicture}
```
Data cascades: compounded costs. Source: @sambasivan2021everyone.
:::
...
### Keyword Spotting Example {#sec-data-engineering-keyword-spotting-example-a8f7}
...
::: {.callout-quiz-question #quiz-question-sec-data-engineering-problem-definition-2348}

1. Why is it critical to define the problem clearly before collecting data in ML systems?
   a) To ensure data collection is aligned with the system's objectives
   b) To minimize the amount of data needed
   c) To reduce the computational complexity of the model
   d) To avoid any need for stakeholder engagement

2. Explain how data quality issues can lead to 'Data Cascades' in ML systems.

3. In the context of Keyword Spotting systems, ensuring low ____ is crucial for maintaining user satisfaction.

See Answer \ref{quiz-answer-sec-data-engineering-problem-definition-2348}.

:::

## Pipeline Basics {#sec-data-engineering-pipeline-basics-4f5b}
...
::: {.callout-quiz-question #quiz-question-sec-data-engineering-pipeline-basics-4f5b}

1. Which component of a data pipeline is primarily responsible for converting raw data into a format suitable for machine learning training?
   a) Data Ingestion
   b) Processing Layer
   c) Storage Layer
   d) Data Sources

2. Explain why the interaction between data ingestion and storage layers is crucial in a data pipeline.

3. True or False: In a data pipeline, the storage layer is only responsible for storing raw data.

4. In a data pipeline, the ____ layer is responsible for tasks such as feature creation and data transformation.

5. Order the following stages of a data pipeline from first to last: Data Ingestion, ML Training, Data Sources, Processing Layer.

See Answer \ref{quiz-answer-sec-data-engineering-pipeline-basics-4f5b}.

:::

## Data Sources {#sec-data-engineering-data-sources-3ee0}
...
### Existing Datasets {#sec-data-engineering-existing-datasets-0b0f}
...
### Web Scraping {#sec-data-engineering-web-scraping-51d6}
...
### Crowdsourcing {#sec-data-engineering-crowdsourcing-8b38}
...
### Anonymization Techniques {#sec-data-engineering-anonymization-techniques-9edd}
...
### Synthetic Data Creation {#sec-data-engineering-synthetic-data-creation-ec90}
...
### Continuing the KWS Example {#sec-data-engineering-continuing-kws-example-e7a9}
...
::: {.callout-quiz-question #quiz-question-sec-data-engineering-data-sources-3ee0}

1. Which of the following is a primary advantage of using pre-existing datasets like ImageNet for training ML models?
   a) They are always up-to-date and reflect current real-world conditions.
   b) They provide immediate access to large, cleaned datasets with established benchmarks.
   c) They eliminate the need for any further data preprocessing.
   d) They guarantee unbiased data for all ML applications.

2. Explain why web scraping might introduce challenges in maintaining data quality for ML systems.

3. True or False: Crowdsourcing always ensures high-quality data annotations due to the diversity of contributors.

4. In ML systems, synthetic data is particularly valuable for addressing ____ when real-world data is scarce or difficult to obtain.

See Answer \ref{quiz-answer-sec-data-engineering-data-sources-3ee0}.

:::

## Data Ingestion {#sec-data-engineering-data-ingestion-998e}
...
### Ingestion Patterns {#sec-data-engineering-ingestion-patterns-c88b}
...
### ETL and ELT Comparison {#sec-data-engineering-etl-elt-comparison-1c56}
...
\end{tikzpicture}
```
Key differences between Extract, Transform, Load (ETL) versus Extract, Load, Transform (ELT).
:::
...
### Data Source Integration {#sec-data-engineering-data-source-integration-5ac0}
...
### Validation Techniques {#sec-data-engineering-validation-techniques-1cda}
...
### Error Management {#sec-data-engineering-error-management-b465}
...
### Continuing the KWS Example {#sec-data-engineering-continuing-kws-example-5bb6}
...
::: {.callout-quiz-question #quiz-question-sec-data-engineering-data-ingestion-998e}

1. Which data ingestion pattern is most suitable for applications requiring immediate processing of incoming data?
   a) Batch ingestion
   b) Stream ingestion
   c) Hybrid ingestion
   d) Delayed ingestion

2. Explain the tradeoffs between using ETL and ELT approaches in ML data pipelines.

3. True or False: ELT is more suitable than ETL for environments with frequently changing data requirements.

4. In a data ingestion pipeline, implementing ____ degradation allows systems to function with reduced capabilities during partial data loss.

5. Order the following steps in a typical ETL process: Transform, Load, Extract.

See Answer \ref{quiz-answer-sec-data-engineering-data-ingestion-998e}.

:::

## Data Processing {#sec-data-engineering-data-processing-651a}
...
### Cleaning Techniques {#sec-data-engineering-cleaning-techniques-c304}
...
### Data Quality Assessment {#sec-data-engineering-data-quality-assessment-8427}
...
### Transformation Techniques {#sec-data-engineering-transformation-techniques-4cff}
...
### Feature Engineering {#sec-data-engineering-feature-engineering-d501}
...
### Processing Pipeline Design {#sec-data-engineering-processing-pipeline-design-079b}
...
\end{tikzpicture}
```
Example breakdown of the entire data to model processing pipeline for TensorFlow Extended.
:::

### Scalability Considerations {#sec-data-engineering-scalability-considerations-1937}
...
### Continuing the KWS Example {#sec-data-engineering-continuing-kws-example-61ad}
...
::: {.callout-quiz-question #quiz-question-sec-data-engineering-data-processing-651a}

1. Which data processing approach is typically more suitable for dealing with unstructured or semi-structured data?
   a) ETL (Extract, Transform, Load)
   b) ELT (Extract, Load, Transform)
   c) Both ETL and ELT are equally suitable
   d) Neither ETL nor ELT is suitable

2. Explain why data cleaning is a critical step in the data processing pipeline for machine learning systems.

3. True or False: In an ETL-based system, data transformations are typically performed after loading the data into the target system.

4. In a KWS system, converting audio signals into ____ is a common transformation step to standardize representation across different recording conditions.

See Answer \ref{quiz-answer-sec-data-engineering-data-processing-651a}.

:::

## Data Labeling {#sec-data-engineering-data-labeling-21da}
...
### Types of Labels {#sec-data-engineering-types-labels-74fb}
...
### Annotation Techniques {#sec-data-engineering-annotation-techniques-556c}
...
### Label Quality Assessment {#sec-data-engineering-label-quality-assessment-f6ca}
...
![Some examples of hard labeling cases. Source: @northcutt2021pervasive](images/png/label-errors-examples.png){#fig-hard-labels}
...
### AI in Annotation {#sec-data-engineering-ai-annotation-5adc}
...
### Labeling Challenges {#sec-data-engineering-labeling-challenges-54b5}
...
### Continuing the KWS Example {#sec-data-engineering-continuing-kws-example-44b7}
...
::: {.callout-quiz-question #quiz-question-sec-data-engineering-data-labeling-21da}

1. Which label type requires the most storage and processing resources in a machine learning system designed for traffic monitoring?
   a) Classification labels
   b) Bounding boxes
   c) Segmentation maps
   d) Metadata labels

2. Explain why a hybrid annotation approach might be beneficial in large-scale ML systems.

3. True or False: AI-assisted labeling completely eliminates the need for human oversight in data labeling workflows.

4. In a labeling system, the use of ____ learning helps prioritize which examples need human attention by analyzing model uncertainty.

5. A machine learning system is designed to process 500,000 video frames per day, each requiring segmentation maps. If each segmentation map requires 2MB of storage, calculate the total storage required per day and discuss the implications for system design.

See Answer \ref{quiz-answer-sec-data-engineering-data-labeling-21da}.

:::

## Data Storage {#sec-data-engineering-data-storage-f54d}
...
### Storage System Types {#sec-data-engineering-storage-system-types-5462}
...
### Storage Considerations {#sec-data-engineering-storage-considerations-8a44}
...
### Performance Factors {#sec-data-engineering-performance-factors-a586}
...
### Storage in ML Lifecycle {#sec-data-engineering-storage-ml-lifecycle-9382}
...
#### Development Phase {#sec-data-engineering-development-phase-901f}
...
#### Training Phase {#sec-data-engineering-training-phase-b6d8}
...
#### Deployment Phase {#sec-data-engineering-deployment-phase-2d2a}
...
#### Maintenance Phase {#sec-data-engineering-maintenance-phase-e4f6}
...
### Feature Storage {#sec-data-engineering-feature-storage-56c5}
...
\tikzset{%
 LinePE/.style={line width=\Linewidth,draw=\drawchannelcolor,fill=\channelcolor},
 ellipsePE/.style={line width=\Linewidth,draw=\drawchannelcolor,fill=\channelcolor,fill=\channelcolor,ellipse,minimum width = 2.5mm, inner sep=2pt,minimum width=29,
 minimum height=40},
 pics/person/.style = {
        code = {
        \pgfkeys{/channel/.cd, #1}
\begin{scope}[local bounding box=PERSON,scale=\scalefac, every node/.append style={transform shape}]
\node[ellipsePE](\picname-EL1)at(0,0.44){};
...
\pic[shift={(0,0)}] at  (0,0){person={scalefac=0.8,picname=1,drawchannelcolor=BrownLine,
channelcolor=none, Linewidth=1.5pt}};
 \end{scope}
...
\end{tikzpicture}
```
High-level overview of feature stores interact with data, users and model training and deployment.
:::
...
### Caching Techniques {#sec-data-engineering-caching-techniques-884d}
...
### Data Access Patterns {#sec-data-engineering-data-access-patterns-4f5b}
...
### Continuing the KWS Example {#sec-data-engineering-continuing-kws-example-5897}
...
::: {.callout-quiz-question #quiz-question-sec-data-engineering-data-storage-f54d}

1. Which storage system is most suitable for handling large volumes of unstructured data in ML workflows?
   a) Conventional Database
   b) Data Warehouse
   c) Data Lake
   d) In-Memory Database

2. Explain why data lakes require careful cataloging and metadata management in ML systems.

3. True or False: Data warehouses are optimized for storing and processing unstructured data in ML systems.

4. In ML workflows, the tension between sequential read performance for training and ____ access for inference is a key consideration.

See Answer \ref{quiz-answer-sec-data-engineering-data-storage-f54d}.

:::

## Data Governance {#sec-data-engineering-data-governance-9768}
...
\end{tikzpicture}
```
The many pillars of data governance.
:::
...
::: {.callout-quiz-question #quiz-question-sec-data-engineering-data-governance-9768}

1. Which of the following practices is essential for maintaining transparency and accountability in ML systems?
   a) Data anonymization
   b) Audit trails
   c) Feature engineering
   d) Model hyperparameter tuning

2. True or False: Differential privacy ensures that individual data points are unidentifiable while preserving the statistical patterns necessary for model training.

3. Explain why regulatory compliance is crucial for ML systems handling sensitive data.

4. In ML systems, implementing ____ controls is critical to ensure that only authorized personnel can access sensitive data.

See Answer \ref{quiz-answer-sec-data-engineering-data-governance-9768}.

:::

## Conclusion {#sec-data-engineering-conclusion-6c20}
...
## Resources {#sec-data-engineering-resources-2746}
...
:::{.callout-quiz-answer #quiz-answer-sec-data-engineering-problem-definition-2348}
...
1. **Why is it critical to define the problem clearly before collecting data in ML systems?**
   ...
   The correct answer is A. Defining the problem clearly ensures that data collection is aligned with the system's objectives, which helps in building a model that effectively addresses the intended use case.

   *Learning Objective*: Understand the importance of clear problem definition before data collection in ML systems.

2. **Explain how data quality issues can lead to 'Data Cascades' in ML systems.**

   Data quality issues can lead to 'Data Cascades' where errors in data collection compound over time, affecting model evaluation and deployment, potentially leading to flawed predictions and costly project failures. This highlights the importance of early error detection and mitigation.

   *Learning Objective*: Analyze the impact of data quality issues on the ML system lifecycle.

3. **In the context of Keyword Spotting systems, ensuring low ____ is crucial for maintaining user satisfaction.**

   The answer is "latency". Ensuring low latency is crucial because it affects how quickly the system can respond to user commands, which is vital for maintaining user satisfaction in real-time applications.

   *Learning Objective*: Recognize the importance of latency in real-time ML applications like Keyword Spotting.

:::

:::{.callout-quiz-answer #quiz-answer-sec-data-engineering-pipeline-basics-4f5b}
...
1. **Which component of a data pipeline is primarily responsible for converting raw data into a format suitable for machine learning training?**
   ...
   The correct answer is B. The Processing Layer is responsible for transforming raw data into a format suitable for machine learning training, including tasks like data validation, transformation, and feature engineering.

   *Learning Objective*: Understand the role of the processing layer in data pipelines.

2. **Explain why the interaction between data ingestion and storage layers is crucial in a data pipeline.**

   The interaction between data ingestion and storage layers is crucial because it ensures that data is efficiently and reliably captured and stored for subsequent processing. This interaction impacts the pipeline's ability to handle data volume, maintain data integrity, and ensure timely availability for downstream tasks, affecting overall system performance.

   *Learning Objective*: Analyze the importance of interactions between pipeline components.

3. **True or False: In a data pipeline, the storage layer is only responsible for storing raw data.**

   False. The storage layer in a data pipeline is responsible not only for storing raw data but also for managing processed data and ensuring it is accessible for further processing and ML training. It plays a critical role in maintaining data integrity and availability.

   *Learning Objective*: Correct misconceptions about the role of the storage layer in data pipelines.

4. **In a data pipeline, the ____ layer is responsible for tasks such as feature creation and data transformation.**

   The answer is "processing". The processing layer handles tasks like feature creation and data transformation, which are essential for preparing data for machine learning models.

   *Learning Objective*: Recall the specific responsibilities of the processing layer in data pipelines.

5. **Order the following stages of a data pipeline from first to last: Data Ingestion, ML Training, Data Sources, Processing Layer.**

   1. Data Sources, 2. Data Ingestion, 3. Processing Layer, 4. ML Training. This order reflects the typical flow of data through a pipeline, starting from acquisition to preparation and finally use in training models.

   *Learning Objective*: Understand the sequential flow of data through a pipeline.

:::

:::{.callout-quiz-answer #quiz-answer-sec-data-engineering-data-sources-3ee0}
...
1. **Which of the following is a primary advantage of using pre-existing datasets like ImageNet for training ML models?**
   ...
   The correct answer is B. Pre-existing datasets like ImageNet provide immediate access to large, cleaned datasets with established benchmarks, which can save time and resources in the initial stages of model development.

   *Learning Objective*: Understand the advantages of using pre-existing datasets in ML systems.

2. **Explain why web scraping might introduce challenges in maintaining data quality for ML systems.**

   Web scraping can introduce challenges in maintaining data quality due to variations in website structure, potential legal and ethical issues, and the dynamic nature of web content, which can lead to inconsistencies and require thorough validation and cleaning processes.

   *Learning Objective*: Analyze the challenges and operational considerations of using web scraping for data collection in ML systems.

3. **True or False: Crowdsourcing always ensures high-quality data annotations due to the diversity of contributors.**

   False. While crowdsourcing can introduce diverse perspectives, it also presents challenges in quality control due to variability in contributors' expertise and attention, requiring careful management and validation techniques.

   *Learning Objective*: Understand the tradeoffs and quality control challenges associated with crowdsourcing data for ML systems.

4. **In ML systems, synthetic data is particularly valuable for addressing ____ when real-world data is scarce or difficult to obtain.**

   The answer is "data scarcity". Synthetic data generation allows for the creation of artificial datasets to supplement or replace real-world data, expanding training possibilities when data is scarce or ethically challenging to obtain.

   *Learning Objective*: Recognize the role of synthetic data in addressing data scarcity in ML systems.

:::

:::{.callout-quiz-answer #quiz-answer-sec-data-engineering-data-ingestion-998e}
...
1. **Which data ingestion pattern is most suitable for applications requiring immediate processing of incoming data?**
   ...
   The correct answer is B. Stream ingestion is most suitable for applications requiring immediate processing of incoming data, such as real-time fraud detection, because it processes data as it arrives.

   *Learning Objective*: Understand the use cases and applications of different data ingestion patterns.

2. **Explain the tradeoffs between using ETL and ELT approaches in ML data pipelines.**

   ETL transforms data before loading, resulting in ready-to-query datasets, but lacks flexibility for schema changes. ELT loads raw data first, allowing flexible transformations later, but demands more from storage and query systems. The choice affects pipeline efficiency and adaptability.

   *Learning Objective*: Analyze the tradeoffs between ETL and ELT approaches in data ingestion for ML systems.

3. **True or False: ELT is more suitable than ETL for environments with frequently changing data requirements.**

   True. ELT is more suitable for environments with frequently changing data requirements because it allows transformations to be applied after loading, providing greater flexibility to adapt to changes.

   *Learning Objective*: Evaluate the suitability of ELT in dynamic ML environments.

4. **In a data ingestion pipeline, implementing ____ degradation allows systems to function with reduced capabilities during partial data loss.**

   The answer is "graceful". Implementing graceful degradation allows systems to continue functioning with reduced capabilities during partial data loss, ensuring resilience in ML systems.

   *Learning Objective*: Understand the concept of graceful degradation in error management for data ingestion.

5. **Order the following steps in a typical ETL process: Transform, Load, Extract.**

   Extract, Transform, Load. In the ETL process, data is first extracted from the source, then transformed to fit the target schema, and finally loaded into the data warehouse or repository.

   *Learning Objective*: Reinforce the sequence of steps in the ETL data ingestion process.

:::

:::{.callout-quiz-answer #quiz-answer-sec-data-engineering-data-processing-651a}
...
1. **Which data processing approach is typically more suitable for dealing with unstructured or semi-structured data?**
   ...
   The correct answer is B. ELT (Extract, Load, Transform) is more suitable for unstructured or semi-structured data because it allows for flexibility in applying transformations after data is loaded into the target system, which is beneficial when the exact transformations needed are not known in advance.

   *Learning Objective*: Understand the suitability of ELT for unstructured data and its operational implications.

2. **Explain why data cleaning is a critical step in the data processing pipeline for machine learning systems.**

   Data cleaning is critical because it addresses errors, inconsistencies, and inaccuracies in datasets that can significantly impact model performance. By removing duplicates, handling missing values, and correcting formatting issues, data cleaning ensures that the data used for training is reliable and accurate, leading to more effective and robust ML models.

   *Learning Objective*: Analyze the importance of data cleaning in ensuring reliable model performance.

3. **True or False: In an ETL-based system, data transformations are typically performed after loading the data into the target system.**

   False. In an ETL-based system, data transformations are performed before loading the data into the target system. This approach ensures that data is in a ready-to-use state when it reaches the data warehouse or ML pipeline.

   *Learning Objective*: Distinguish between the order of transformations in ETL and ELT systems.

4. **In a KWS system, converting audio signals into ____ is a common transformation step to standardize representation across different recording conditions.**

   The answer is "spectrograms". Spectrograms provide a visual representation of the spectrum of frequencies in a signal, which helps standardize audio data for analysis in ML models.

   *Learning Objective*: Understand the role of spectrograms in transforming audio data for ML applications.

:::

:::{.callout-quiz-answer #quiz-answer-sec-data-engineering-data-labeling-21da}
...
1. **Which label type requires the most storage and processing resources in a machine learning system designed for traffic monitoring?**
   ...
   The correct answer is C. Segmentation maps provide pixel-level classification, significantly increasing storage and processing requirements compared to simpler label types.

   *Learning Objective*: Understand the resource implications of different label types in ML systems.

2. **Explain why a hybrid annotation approach might be beneficial in large-scale ML systems.**

   A hybrid approach balances speed, cost, and quality by combining programmatic labeling for initial coverage, crowdsourced verification, and expert review for uncertain cases. This allows efficient data processing while maintaining high label quality, crucial for large-scale systems.

   *Learning Objective*: Analyze the benefits and tradeoffs of using a hybrid annotation approach in ML systems.

3. **True or False: AI-assisted labeling completely eliminates the need for human oversight in data labeling workflows.**

   False. While AI-assisted labeling can significantly reduce manual effort, human oversight is essential to correct AI-generated errors, manage biases, and ensure label quality, especially in safety-critical domains.

   *Learning Objective*: Evaluate the role of human oversight in AI-assisted labeling workflows.

4. **In a labeling system, the use of ____ learning helps prioritize which examples need human attention by analyzing model uncertainty.**

   The answer is "active". Active learning identifies data points that are most informative for model improvement, optimizing human labeling efforts.

   *Learning Objective*: Understand how active learning is used to optimize labeling processes in ML systems.

5. **A machine learning system is designed to process 500,000 video frames per day, each requiring segmentation maps. If each segmentation map requires 2MB of storage, calculate the total storage required per day and discuss the implications for system design.**

   The total storage required per day is 500,000 frames * 2MB/frame = 1,000,000MB or approximately 1TB. This highlights the need for robust storage infrastructure and efficient data retrieval mechanisms to handle large-scale data labeling requirements in ML systems.

   *Learning Objective*: Calculate storage requirements for large-scale data labeling and understand its implications for system design.

:::

:::{.callout-quiz-answer #quiz-answer-sec-data-engineering-data-storage-f54d}
...
1. **Which storage system is most suitable for handling large volumes of unstructured data in ML workflows?**
   ...
   The correct answer is C. Data Lake. Data lakes are designed to store large volumes of structured, semi-structured, and unstructured data, making them ideal for handling diverse data types common in ML workflows.

   *Learning Objective*: Understand the suitability of different storage systems for handling various data types in ML workflows.

2. **Explain why data lakes require careful cataloging and metadata management in ML systems.**

   Data lakes require careful cataloging and metadata management to prevent them from becoming unsearchable and disorganized. Without proper management, the vast amount of diverse data can lead to inefficiencies and difficulties in retrieving relevant data for ML tasks.

   *Learning Objective*: Analyze the operational challenges associated with managing data lakes in ML systems.

3. **True or False: Data warehouses are optimized for storing and processing unstructured data in ML systems.**

   False. Data warehouses are optimized for analytical queries on structured data and may not efficiently handle unstructured data, which is better suited for data lakes.

   *Learning Objective*: Identify the limitations of data warehouses in handling unstructured data for ML systems.

4. **In ML workflows, the tension between sequential read performance for training and ____ access for inference is a key consideration.**

   The answer is "random". This access pattern requires storage systems to balance between high-throughput sequential reads for training and low-latency random access for inference.

   *Learning Objective*: Understand the access pattern tradeoffs in ML storage systems.

:::

:::{.callout-quiz-answer #quiz-answer-sec-data-engineering-data-governance-9768}
...
1. **Which of the following practices is essential for maintaining transparency and accountability in ML systems?**
   ...
   The correct answer is B. Audit trails are essential for maintaining transparency and accountability in ML systems by tracking data access and usage.

   *Learning Objective*: Understand the role of audit trails in ensuring transparency and accountability in ML systems.

2. **True or False: Differential privacy ensures that individual data points are unidentifiable while preserving the statistical patterns necessary for model training.**

   True. Differential privacy adds noise to data outputs to protect individual identities, ensuring privacy while maintaining data utility for model training.

   *Learning Objective*: Recognize the importance of differential privacy in protecting individual identities during model training.
...
3. **Explain why regulatory compliance is crucial for ML systems handling sensitive data.**
   ...
   *Learning Objective*: Understand the importance of regulatory compliance in ML systems and its implications for data handling.

4. **In ML systems, implementing ____ controls is critical to ensure that only authorized personnel can access sensitive data.**

   The answer is "access". Implementing access controls is critical in ML systems to protect sensitive data from unauthorized access and ensure data security.

   *Learning Objective*: Identify the importance of access controls in securing sensitive data in ML systems.
