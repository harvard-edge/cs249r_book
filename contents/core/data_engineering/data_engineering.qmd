...
## Self-Check Answers {#sec-data-engineering-selfcheck-answers-b263}
...
3. **In the context of Keyword Spotting systems, ensuring low ____ is crucial for maintaining user satisfaction.**
   ...
   *Answer*: The answer is "latency". Ensuring low latency is crucial because it affects how quickly the system can respond to user commands, which is vital for maintaining user satisfaction in real-time applications.
...
4. **In a data pipeline, the ____ layer is responsible for tasks such as feature creation and data transformation.**
   ...
   *Answer*: The answer is "processing". The processing layer handles tasks like feature creation and data transformation, which are essential for preparing data for machine learning models.

   *Learning Objective*: Recall the specific responsibilities of the processing layer in data pipelines.

5. **Order the following stages of a data pipeline from first to last: Data Ingestion, ML Training, Data Sources, Processing Layer.**

   *Answer*: The order is as follows: 1. Data Sources, 2. Data Ingestion, 3. Processing Layer, 4. ML Training. This order reflects the typical flow of data through a pipeline, starting from acquisition to preparation and finally use in training models.
...
4. **In ML systems, synthetic data is particularly valuable for addressing ____ when real-world data is scarce or difficult to obtain.**
   ...
   *Answer*: The answer is "data scarcity". Synthetic data generation allows for the creation of artificial datasets to supplement or replace real-world data, expanding training possibilities when data is scarce or ethically challenging to obtain.
...
4. **In a data ingestion pipeline, implementing ____ degradation allows systems to function with reduced capabilities during partial data loss.**
   ...
   *Answer*: The answer is "graceful". Implementing graceful degradation allows systems to continue functioning with reduced capabilities during partial data loss, ensuring resilience in ML systems.

   *Learning Objective*: Understand the concept of graceful degradation in error management for data ingestion.

5. **Order the following steps in a typical ETL process: Transform, Load, Extract.**

   *Answer*: The order is as follows: Extract, Transform, Load. In the ETL process, data is first extracted from the source, then transformed to fit the target schema, and finally loaded into the data warehouse or repository.
...
4. **In a KWS system, converting audio signals into ____ is a common transformation step to standardize representation across different recording conditions.**
   ...
   *Answer*: The answer is "spectrograms". Spectrograms provide a visual representation of the spectrum of frequencies in a signal, which helps standardize audio data for analysis in ML models.
...
4. **In a labeling system, the use of ____ learning helps prioritize which examples need human attention by analyzing model uncertainty.**
   ...
   *Answer*: The answer is "active". Active learning identifies data points that are most informative for model improvement, optimizing human labeling efforts.
...
4. **In ML workflows, the tension between sequential read performance for training and ____ access for inference is a key consideration.**
   ...
   *Answer*: The answer is "random". This access pattern requires storage systems to balance between high-throughput sequential reads for training and low-latency random access for inference.
...
4. **In ML systems, implementing ____ controls is critical to ensure that only authorized personnel can access sensitive data.**
   ...
   *Answer*: The answer is "access". Implementing access controls is critical in ML systems to protect sensitive data from unauthorized access and ensure data security.
