...
## Self-Check Answers {#sec-ml-systems-selfcheck-answers-0914}
...
4. **One of the challenges of Cloud ML is the risk of ____ due to reliance on specific cloud service providers.**
   ...
   *Answer*: The answer is "vendor lock-in". Vendor lock-in occurs when organizations become dependent on a specific cloud provider's tools and services, complicating future transitions or migrations.
...
4. **One of the challenges in deploying Edge ML is managing the limited ____ resources available on edge devices.**
   ...
   *Answer*: The answer is "computational". Edge devices often have restricted computational resources compared to cloud servers, limiting the complexity of machine learning models that can be deployed.
...
5. **Order the following steps in deploying an Edge ML system: A) Update security protocols, B) Deploy ML model on edge device, C) Train model in a centralized environment, D) Monitor system performance.**
   ...
   *Answer*: The order is as follows: C) Train model in a centralized environment, B) Deploy ML model on edge device, A) Update security protocols, D) Monitor system performance. The model is first trained centrally, then deployed to the edge, followed by ensuring security and monitoring performance.
...
3. **Mobile ML applications often use techniques like pruning and quantization to reduce ____ and ensure efficient performance on resource-constrained devices.**
   ...
   *Answer*: The answer is "model size". Techniques like pruning and quantization help reduce the model size, allowing for efficient deployment on devices with limited resources like smartphones.
...
4. **One of the challenges in developing Tiny ML models is optimizing them to fit within ____ constraints.**
   ...
   *Answer*: The answer is "resource". Tiny ML models must be optimized to fit within the limited memory and computational power constraints of ultra-constrained devices.
...
5. **Order the following steps in deploying a Tiny ML system: A) Optimize model for resource constraints, B) Deploy model on device, C) Test model performance, D) Train model on larger system.**
   ...
   *Answer*: The order is as follows: D) Train model on larger system, A) Optimize model for resource constraints, B) Deploy model on device, C) Test model performance. This sequence reflects the typical workflow for deploying Tiny ML systems, starting with model training and ending with performance testing.
...
3. **In a Hybrid ML system, the ____ pattern involves distributing model training across edge devices to maintain privacy while benefiting from collective learning.**
   ...
   *Answer*: The answer is "federated learning". This pattern allows devices to learn from local data and share model updates with a central server without exposing raw data, enhancing privacy.
...
5. **Order the following steps in a model deployment scenario for Hybrid ML: A) Deploy model to edge servers, B) Train model in the cloud, C) Optimize model for mobile devices, D) Test model on tiny sensors.**
   ...
   *Answer*: The order is as follows: B) Train model in the cloud, A) Deploy model to edge servers, C) Optimize model for mobile devices, D) Test model on tiny sensors. This sequence reflects the progression from training to deployment across different computational tiers.
...
4. **In ML systems, ensuring ____ is crucial for maintaining security, privacy, and reliability across implementations.**
   ...
   *Answer*: The answer is "trustworthiness". Ensuring trustworthiness is crucial for maintaining security, privacy, and reliability across all ML system implementations, regardless of their scale.
...
3. **One of the key advantages of Mobile ML is its ability to provide personalized intelligence while maintaining ____ costs.**
   ...
   *Answer*: The answer is "low". Mobile ML leverages existing consumer devices, minimizing additional hardware costs and allowing for cost-effective deployment of personalized intelligence.
...
5. **Order the following ML system paradigms from highest to lowest in terms of energy consumption: A) Mobile ML, B) Cloud ML, C) Tiny ML, D) Edge ML.**
   ...
   *Answer*: The order is as follows: B) Cloud ML, D) Edge ML, A) Mobile ML, C) Tiny ML. Cloud ML consumes the most energy due to its data center infrastructure, followed by Edge ML with its powerful edge devices. Mobile ML has moderate energy consumption, while Tiny ML is designed for ultra-low-power operation.
...
3. **In the deployment decision framework, the ____ layer assesses network stability and its impact on deployment feasibility.**
   ...
   *Answer*: The answer is "Reliability". This layer evaluates network stability, which is crucial for determining if a deployment can handle interruptions or requires stable connectivity.
