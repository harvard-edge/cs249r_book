...
The introduction of deep learning in the early 2010s marked a turning point for algorithmic efficiency. Neural networks, which had previously been constrained by hardware limitations, now benefited from advancements in computational power, particularly the adoption of GPUs. This capability allowed researchers to train larger, more complex models, leading to breakthroughs in tasks such as image recognition, natural language processing, and speech synthesis.
...
Notably, as @fig-algo-efficiency shows, by 9, the computational resources needed to train a neural network to achieve AlexNet-level performance on ImageNet classification had decreased by 44x compared to 2012. This improvement---halving every 16 months---outpaced the hardware efficiency gains of Moore's Law[^fn-moores-law]. Such rapid progress demonstrates the role of algorithmic advancements in driving efficiency alongside hardware innovations.
...
![From AlexNet to AlphaGo Zero, there has been a 300,000x increase in demand for computing power over seven years. Source: [@AIandcom90:online].](images/png/comp_efficiency.png){#fig-comp_efficiency}
...
@fig-virtuous-cycle below illustrates the virtuous cycle of machine learning systems, highlighting how efficiency drives scalability, scalability fosters sustainability, and sustainability reinforces efficiency.

<!-- @zh: there is a lot of whitespace around dot diagrams for HTML rendering, not sure how PDF looks. not sure why. we need to fix it somehow.  -->
...
digraph G {
    ...
    node [shape=circle, style=filled, width=1.0, height=1.0, fixedsize=true, fontsize=10];

    node0 [label="Efficiency", fillcolor="#FFA07A80"]; // Light Salmon, 50% transparent
    node1 [label="Scalability", fillcolor="#FFD70080"]; // Gold, 50% transparent
    node2 [label="Sustainability", fillcolor="#FF69B480"]; // Hot Pink, 50% transparent
...
<!-- @zh: this is the kind of thing i feel we need to use something else to draw the figure, cause this is ugly and could look really nice -->
