...
Fairness refers to the expectation that machine learning systems do not discriminate against individuals or groups on the basis of protected attributes such as race, gender, or socioeconomic status. Fairness encompasses both statistical definitions—such as demographic parity[^fn-demographic-parity] or equalized odds[^fn-equalized-odds]—and normative concerns about equity, justice, and structural bias.

[^fn-demographic-parity]: **Demographic Parity:** A fairness measure ensuring equal outcomes across different groups.

[^fn-equalized-odds]: **Equalized Odds:** A fairness criterion that equal outcomes should be maintained for all groups at all thresholds.

Explainability concerns the ability of stakeholders to interpret how a model produces its outputs. This may involve post hoc explanations[^fn-post-hoc-explanations] for individual decisions or global understanding of a models behavior. Explainability is essential for error analysis, regulatory compliance, and building user trust.

[^fn-post-hoc-explanations]: **Post hoc explanations:** Clarifications provided after a model decision, detailing how the decision was reached.
...
Responsible machine learning begins with a set of foundational principles—fairness, transparency, accountability, privacy, and safety—that define what it means for an AI system to behave ethically and predictably. These principles are not abstract ideals or afterthoughts; they must be translated into concrete constraints that guide how models are trained, evaluated, deployed, and maintained.
...
This ensures that qualified individuals—those with $Y = 1$—are treated equally by the model regardless of group membership.
...
Practical implementation is often shaped by limitations in data access and system instrumentation. In many real-world environments—particularly mobile, federated, or embedded systems—sensitive attributes such as gender, age, or race may not be available at inference time, making it difficult to track or audit model performance across demographic groups. In such contexts, fairness interventions must occur upstream during data curation or training, as post-deployment recalibration may not be feasible. Even when data is available, continuous retraining pipelines that incorporate user feedback can reinforce existing disparities unless explicitly monitored for fairness degradation. For example, an on-device recommendation model that adapts to user behavior may amplify prior biases if it lacks the infrastructure to detect demographic imbalances in user interactions or outputs.

@fig-fairness-example illustrates how fairness constraints can introduce tension with deployment choices. In a binary loan approval system, two subgroups—A (blue) and B (red)—require different decision thresholds to achieve equal true positive rates. Using a single threshold across groups leads to disparate outcomes, potentially disadvantaging Subgroup B. Addressing this imbalance by adjusting thresholds per group may improve fairness, but doing so requires support for conditional logic in the model serving stack, access to sensitive attributes at inference time, and a governance framework for explaining and justifying differential treatment across groups.
...
Post-processing methods—such as applying group-specific thresholds or adjusting scores to equalize outcomes—require inference systems that can condition on sensitive attributes or reference external policy rules. This demands coordination between model serving infrastructure, access control policies, and logging pipelines to ensure that differential treatment is both auditable and legally defensible. Moreover, any post-processing strategy must be carefully validated to ensure that it does not compromise user experience, model stability, or compliance with jurisdictional regulations on attribute use.
