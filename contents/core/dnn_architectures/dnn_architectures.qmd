...
Modern deep learning architectures vary significantly in their structure, from Multi-Layer Perceptrons (MLPs) to Convolutional Neural Networks (CNNs), Recurrent Neural Networks (RNNs), and Transformer architectures. Despite their differences, each architecture is fundamentally designed to efficiently process specific types of patterns: dense connections for arbitrary relationships, convolutional structures for spatial patterns, recurrent networks for temporal sequences, and attention mechanisms for global dependencies.

This chapter approaches these architectures from a systems perspective, emphasizing their computational building blocks and system-level implications. Understanding these relationships is essential for designing and optimizing machine learning systems. Dense networks, for instance, require substantial memory bandwidth to support all-to-all connectivity, while convolutional architectures leverage data locality and weight sharing. Recurrent networks must efficiently manage temporal states, and attention mechanisms face the challenge of quadratic scaling in memory and computation requirements. Each architectural pattern creates distinct computational and memory demands that directly influence system design decisions. Consequently, these system-level characteristics drive important decisions and considerations such as memory hierarchy organization, processing unit design, parallelization opportunities, and trade-offs between architectural sophistication and resource requirements. Recognizing these trade-offs is needed for engineers when selecting models for deployment, optimizing system performance, or designing hardware accelerators to improve efficiency.

Through systematic examination of major architectural patterns and real-world case studies, we develop an understanding of how deep learning model architectures map to systems. It highlights the trade-offs and implementation strategies required to build efficient, scalable AI systems capable of addressing complex problems while meeting practical constraints in computation, memory, and energy efficiency. The principles established here will inform deeper discussions of training, optimization, and acceleration in subsequent chapters.
