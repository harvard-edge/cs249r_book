...
![**Sustainable Development Goals**: A globally-accepted framework for addressing interconnected social, economic, and environmental challenges, providing context for ML system applications aimed at positive societal impact. *source: united nations.*](./images/png/un_sdg.png){#fig-sdg}
...
**Deployment Resource Trade-Offs**: Highlights the stark differences in computational resources available across typical machine learning deployment contexts—from resource-constrained rural settings using microcontrollers, to urban deployments leveraging server-grade systems, and the aggressive model compression techniques required for scaling to large numbers of devices. this comparison illustrates the engineering challenges in balancing model accuracy, resource consumption, and deployment feasibility in social impact applications. {#tbl-social_challenges .striped .hover}
...
\end{tikzpicture}
```
**Hierarchical Data Flow**: The tiered processing approach common in distributed ML systems, where data progressively refines through edge, regional, and cloud layers to enable scalable and efficient analysis with continuous model improvement via feedback loops.
:::
...
**Progressive Enhancement Layers**: Illustrates how resource constraints—computational power, network connectivity, and storage—define the capabilities available in each layer of the progressive enhancement pattern, enabling adaptable system design. this table highlights the trade-offs between resource availability and functionality, demonstrating how systems can gracefully degrade or enhance performance based on environmental conditions. {#tbl-enhancement-layers .striped .hover}
...
\end{tikzpicture}}
```
**Progressive Enhancement Layers**: How a machine learning system can adapt to varying resource availability by implementing functionality in tiered layers, ensuring core features remain operational even with limited connectivity or compute. each layer builds upon the previous, adding complexity and capability as resources allow, demonstrating a robust design pattern for deployment in diverse environments.
:::
...
\end{tikzpicture}
```
**Distributed Knowledge Flow**: The core components of the distributed knowledge pattern—autonomous nodes, communication networks, and aggregation—and how they collaborate to process and share insights across a system. this pattern enables decentralized data processing and knowledge discovery by combining local computations with centralized analysis, facilitating scalable and adaptable AI solutions.
:::
...
\end{tikzpicture}
```
**Resource-Adaptive Operations**: How a machine learning system dynamically adjusts its functionality based on available resources, maintaining performance across varying conditions—from full capabilities with high resources to reduced functionality with low resources. this pattern ensures system resilience and efficiency by prioritizing core operations when resources are constrained.
:::
...
\end{tikzpicture}}
```
**Design Space Mapping**: How AI system design patterns align with varying resource constraints and adaptability requirements, guiding pattern selection for AI for social good projects. understanding this mapping helps prioritize patterns suitable for specific deployment contexts, from resource-limited settings to robust, scalable infrastructures.
:::
...
**Design Pattern Trade-Offs**: Key characteristics of common machine learning system design patterns, highlighting the strengths and challenges associated with each to guide implementation choices based on specific requirements. {#tbl-patterns .striped .hover}
...
## Summary {#sec-ai-good-summary-79b5}
