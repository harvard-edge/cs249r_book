...
```{mermaid}
%%| label: fig-pattern-heirarchical
%%| fig-width: "100%"
%%| fig-cap: |
%%|   Sequence diagram illustrating the Hierarchical Processing Pattern.

sequenceDiagram
    participant Sensor as Edge (Sensor/Device) 📡
    participant Regional as Regional Tier 🌍
    participant Cloud as Cloud Tier ☁️
    participant User as End User 🙋‍♂️

    Sensor->>Sensor: Collect Data
    Sensor->>Regional: Send Preprocessed Data
    Regional->>Regional: Aggregate Data
    Regional->>Cloud: Transmit Aggregated Data
    Cloud->>Cloud: Train/Update Model
    Cloud->>Regional: Send Updated Model
    Regional->>Sensor: Push Optimized Model
    Sensor->>User: Perform Real-Time Inference
...
+:==============+:=================================================+:====================================+:================================+
| Computational | Microcontroller-class (100-200MHz CPU, <1MB RAM) | Entry-level smartphones (1-2 GB RAM)| Cloud/edge servers (8 GB+ RAM)  |
+---------------+--------------------------------------------------+-------------------------------------+---------------------------------+
...
\usetikzlibrary{calc,fit,backgrounds,positioning,arrows.meta}
...
\node[Box,right=of B1](B2){Memory Management};
\node[Box,right=of B2](B3){Resource Optimization};
%
...
fit=(B1)(B3)](BB1){};
\node[below=1pt of BB1.north,anchor=north]{System-Level Operations};
\end{scope}

\begin{scope}[shift={(0,-2.75)}]
\node[Box](B1){GEMM\\ Operations};
\node[Box,right=of B1](B2){BLAS\\ Operations};
\node[Box,right=of B2](B3){Element-wise Operations};
%
\scoped[on background layer]
\node[draw=col5,inner xsep=8mm,
line width=0.75pt,
...
fill=col2!10,yshift=2mm,
fit=(B1)(B3)](BB2){};
\node[below=1pt of BB2.north,anchor=north]{Basic Numerical Operations};
\end{scope}

\begin{scope}[shift={(0,-5.5)}]
\node[Box](B1){Compute Kernel Management};
\node[Box,right=of B1](B2){Memory Abstraction};
\node[Box,right=of B2](B3){Execution\\ Control};
%
\scoped[on background layer]
\node[draw=col5,inner xsep=8mm,
line width=0.75pt,
...
fill=col2!10,yshift=2mm,
fit=(B1)(B3)](BB3){};
\node[below=1pt of BB3.north,anchor=north]{Hardware Operations};
\end{scope}

\draw[-{Triangle[width=10pt,length=6pt]}, line width=5pt,col5!90](BB1)--(BB2); 
\draw[-{Triangle[width=10pt,length=6pt]}, line width=5pt,col5!90](BB2)--(BB3); 
\end{tikzpicture}}
```
...
\definecolor{colorFill1}{RGB}{219,253,166}
...
%%| fig-width: 2.8
quadrantChart
    title AI for Social Good: Design Pattern Selection
    x-axis Low Resource Availability --> High Resource Availability
    y-axis Low Scalability/Adaptability --> High Scalability/Adaptability
    quadrant-1 Progressive Enhancement
    quadrant-2 Hierarchical Processing
    quadrant-3 Distributed Knowledge
    quadrant-4 Adaptive Resource
    PlantVillage Nuru: [0.2, 0.8]
    Medic Mobile: [0.3, 0.7]
    Google's Flood Forecasting: [0.8, 0.9]
    Global Fishing Watch: [0.9, 0.8]
    Wildlife Insights: [0.3, 0.3]
    WildEyes AI: [0.4, 0.2]
    AI for Disaster Response: [0.8, 0.4]
    Famine Action Mechanism: [0.9, 0.3]
```
