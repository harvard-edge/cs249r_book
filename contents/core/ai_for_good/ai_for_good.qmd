...
## Overview {#sec-ai-good-overview-a2ec}
...
## Global Challenges {#sec-ai-good-global-challenges-a1f6}
...
::: {.callout-quiz-question #quiz-question-sec-ai-good-global-challenges-a1f6}

1. Explain how systemic barriers in healthcare and agriculture can exacerbate global challenges. Provide examples from the text.

2. What is a significant challenge in addressing global environmental issues as highlighted in the text?
   a) Availability of advanced technology
   b) Lack of monitoring and protection in vulnerable regions
   c) Overabundance of natural resources
   d) Excessive funding for environmental projects

3. True or False: Machine learning systems can help address global challenges by processing large amounts of data to uncover patterns and generate insights.

4. Resource constraints and systemic barriers often result in the perpetuation of ____ and undermine resilience in affected regions.

See Answer \ref{quiz-answer-sec-ai-good-global-challenges-a1f6}.

:::

## Key AI Applications {#sec-ai-good-key-ai-applications-d79f}

AI technologies, such as Cloud ML, Mobile ML, Edge ML, and Tiny ML, are unlocking transformative solutions to some of the world's most pressing challenges. By adapting to diverse constraints and leveraging unique strengths, these technologies are driving innovation in agriculture, healthcare, disaster response, and environmental conservation. This section explores how these paradigms bring social good to life through real-world applications.
...
### Healthcare {#sec-ai-good-healthcare-f2bb}

For millions in underserved communities, access to healthcare often means long waits and travel to distant clinics. Tiny ML is changing that by enabling diagnostics to occur at the patient's side. For example, a low-cost wearable developed by [Respira x Colabs](https://www.samayhealth.com/) uses embedded machine learning to analyze cough patterns and detect pneumonia. Designed for remote areas, the device operates independently of internet connectivity and is powered by a simple microcontroller, making life-saving diagnostics accessible to those who need it most.

Tiny ML's potential extends to tackling global health issues like vector-borne diseases[^fn-vector-borne] that are spread by mosquitoes. Researchers have developed low-cost devices that use machine learning to identify mosquito species by their wingbeat frequencies [@altayeb2022classifying]. This technology enables real-time monitoring of malaria-carrying mosquitoes. It offers a scalable solution for malaria control in high-risk regions.

[^fn-vector-borne]: **Vector-borne diseases**: Illnesses caused by pathogens transmitted by vectors like mosquitoes, ticks, or fleas.

In parallel, Cloud ML is advancing healthcare research and diagnostics on a broader scale. Platforms like [Google Genomics](https://health.google/health-research/genomics/) analyze vast datasets to identify disease markers, accelerating breakthroughs in personalized medicine. These examples show how AI technologies, ranging from the portability of Tiny ML to the computational power of Cloud ML, are converging to democratize healthcare access and improve outcomes worldwide.
...
### Environmental Conservation {#sec-ai-good-environmental-conservation-d41f}
...
### AI's Holistic Impact {#sec-ai-good-ais-holistic-impact-de43}
...
::: {.callout-quiz-question #quiz-question-sec-ai-good-key-ai-applications-d79f}

1. Which AI technology is used to optimize water usage in rice paddies in Indonesia?
   a) Cloud ML
   b) Edge ML
   c) Mobile ML
   d) Tiny ML

2. Explain how Tiny ML is used in disaster response to enhance safety and accelerate efforts.

3. EdgeML-powered collars are used to track animal behavior by processing data on the collar itself, minimizing ____ consumption.

4. True or False: Cloud ML is primarily responsible for enabling real-time monitoring of illegal fishing activities.

See Answer \ref{quiz-answer-sec-ai-good-key-ai-applications-d79f}.

:::

## Global Development Perspective {#sec-ai-good-global-development-perspective-8e14}

The sheer scale and complexity of these problems demand a systematic approach to ensure that efforts are targeted, coordinated, and sustainable. This is where global frameworks such as the United Nations Sustainable Development Goals (SDGs) and guidance from institutions like the World Health Organization (WHO)[^fn-who] play a pivotal role. These frameworks provide a structured lens for thinking about and addressing the world's most pressing challenges. They offer a roadmap to align efforts, set priorities, and foster international collaboration to create impactful and lasting change [@un_desa_2018].

[^fn-who]: **World Health Organization (WHO)**: A specialized agency of the United Nations responsible for international public health.

The SDGs shown in @fig-sdg are a global agenda adopted in 2015. These 17 interconnected goals form a blueprint for addressing the world's most pressing challenges by 2030. They range from eliminating poverty and hunger to ensuring quality education, from promoting gender equality to taking climate action.

![United Nations Sustainable Development Goals (SDG). Source: [United Nations](https://sdgs.un.org/goals).](./images/png/un_sdg.png){#fig-sdg}
...
::: {.callout-quiz-question #quiz-question-sec-ai-good-global-development-perspective-8e14}

1. Explain how the United Nations Sustainable Development Goals (SDGs) provide a framework for deploying machine learning systems to address global challenges.

2. What is a significant challenge when deploying ML systems in regions that could benefit most from them?
   a) Lack of skilled data scientists
   b) Insufficient computational power
   c) Unreliable electricity and internet infrastructure
   d) High cost of machine learning models

3. Machine learning systems must be designed to operate within local resource constraints while respecting ____ contexts and existing infrastructure limitations.

See Answer \ref{quiz-answer-sec-ai-good-global-development-perspective-8e14}.

:::

## Engineering Challenges {#sec-ai-good-engineering-challenges-944a}
...
|                         | (ESP32: 240 MHz, 520 KB RAM)       | (100-200 W, 32-64 GB RAM)          | (e.g., 50 MB to 500 KB)                 |
+-------------------------+------------------------------------+------------------------------------+-----------------------------------------+
...
### Resource Paradox {#sec-ai-good-resource-paradox-ab65}
...
This paradox becomes evident in the computational and power requirements of machine learning systems, as shown in @tbl-social_challenges. A typical cloud deployment might utilize servers consuming 100-200 W of power with multiple CPU cores and 32-64 GB of RAM. However, rural deployments must often operate on single-board computers drawing 5 W or microcontrollers consuming mere milliwatts, with RAM measured in kilobytes rather than gigabytes.

Network infrastructure limitations further constrain system design. Urban environments offer high-bandwidth options like fiber (100+ Mbps) and 5G networks (1-10 Gbps). Rural deployments must instead rely on low-power wide-area network technologies such as LoRa or NB-IoT[^fn-NB-IoT], which achieve kilometer-range coverage with minimal power consumption.

[^fn-NB-IoT]: **NB-IoT (Narrowband Internet of Things)**: NB-IoT is a low-power, wide-area wireless communication technology optimized for connecting IoT devices with minimal energy usage, often in resource-constrained environments.
...
### Data Dilemma {#sec-ai-good-data-dilemma-e5da}
...
Network limitations further constrain data collection and processing. Agricultural sensor networks, operating on limited power budgets, might transmit only 100-200 bytes per reading. With LoRa[^fn-LoRa] bandwidth constraints of 50 kbps, these systems often limit transmission frequency to once per hour. A network of 1000 sensors thus generates only 4-5 MB of data per day, requiring models to learn from sparse temporal data. For perspective, streaming a single minute of video on Netflix can consume several megabytes, highlighting the disparity in data volumes between industrial IoT networks and everyday internet usage.

[^fn-LoRa]: **LoRa (Long Range)**: LoRA is a low-power wireless communication protocol designed for transmitting small data packets over long distances with minimal energy consumption.
...
### Scale Challenge {#sec-ai-good-scale-challenge-40a8}
...
### Sustainability Challenge {#sec-ai-good-sustainability-challenge-ad6d}
...
::: {.callout-quiz-question #quiz-question-sec-ai-good-engineering-challenges-944a}

1. What is a major challenge when scaling ML systems from development to production in resource-constrained environments?
   a) Increasing computational resources
   b) Maintaining consistent network performance
   c) Reducing model complexity
   d) Enhancing data availability

2. Explain why power infrastructure is a critical consideration in rural ML deployments.

3. In resource-constrained environments, models must often be redesigned to operate within strict ____ limits.

4. True or False: High-bandwidth network options are readily available in rural ML deployments.

5. Discuss the importance of community capacity building in the sustainability of ML systems in social impact contexts.

See Answer \ref{quiz-answer-sec-ai-good-engineering-challenges-944a}.

:::

## Design Patterns {#sec-ai-good-design-patterns-236e}

The challenges of deploying machine learning systems in resource-constrained environments reflect fundamental constraints that have shaped system architecture for decades. Computing systems across domains have developed robust solutions to operate within limited computational resources, unreliable networks, and power restrictions. These solutions, formalized as "design patterns[^fn-design-patterns]," represent reusable architectural approaches to common deployment challenges.

[^fn-design-patterns]: Reusable architectural approaches to address common challenges in system design and deployment.
...
### Hierarchical Processing {#sec-ai-good-hierarchical-processing-dc65}
...
[^fn-regional-nodes]: **Regional nodes**: Intermediate computing layers that consolidate and preprocess data between edge and cloud tiers.
...
#### Structure {#sec-ai-good-structure-15c5}
...
#### Modern Adaptations {#sec-ai-good-modern-adaptations-f3c1}
...
#### System Implications {#sec-ai-good-system-implications-9f3d}
...
#### Limitations {#sec-ai-good-limitations-1e31}
...
### Progressive Enhancement {#sec-ai-good-progressive-enhancement-0c58}
...
#### PlantVillage Nuru {#sec-ai-good-plantvillage-nuru-6085}
...
#### Structure {#sec-ai-good-structure-9c96}
...
#### Modern Adaptations {#sec-ai-good-modern-adaptations-e467}
...
#### System Implications {#sec-ai-good-system-implications-b40a}
...
#### Limitations {#sec-ai-good-limitations-d6c9}
...
### Distributed Knowledge {#sec-ai-good-distributed-knowledge-f92f}
...
#### Wildlife Insights {#sec-ai-good-wildlife-insights-9f50}
...
#### Structure {#sec-ai-good-structure-94fa}
...
#### Modern Adaptations {#sec-ai-good-modern-adaptations-6c04}
...
#### System Implications {#sec-ai-good-system-implications-7bf6}
...
#### Limitations {#sec-ai-good-limitations-66a5}
...
### Adaptive Resource {#sec-ai-good-adaptive-resource-ce22}
...
#### Case Studies {#sec-ai-good-case-studies-e40d}
...
#### Structure {#sec-ai-good-structure-876b}
...
#### Modern Adaptations {#sec-ai-good-modern-adaptations-2085}
...
#### System Implications {#sec-ai-good-system-implications-7a06}
...
#### Limitations {#sec-ai-good-limitations-ab20}
...
::: {.callout-quiz-question #quiz-question-sec-ai-good-design-patterns-236e}

1. What is a key advantage of the Hierarchical Processing Pattern in machine learning systems?
   a) It centralizes all data processing to the cloud.
   b) It allows for real-time processing at the edge tier.
   c) It eliminates the need for regional nodes.
   d) It ensures that all tiers operate independently without communication.

2. Explain how the Distributed Knowledge Pattern can enhance system resilience in resource-constrained environments.

3. Which of the following is a challenge associated with the Hierarchical Processing Pattern in ML systems?
   a) Ensuring all data is processed at the edge tier.
   b) Managing dynamic model updates across tiers.
   c) Eliminating the need for cloud infrastructure.
   d) Reducing the number of tiers in the system.

4. Discuss the role of edge computing in the Distributed Knowledge Pattern.

See Answer \ref{quiz-answer-sec-ai-good-design-patterns-236e}.

:::

## Selection Framework {#sec-ai-good-selection-framework-c35d}
...
### Selection Dimensions {#sec-ai-good-selection-dimensions-a152}
...
### Implementation Guidance {#sec-ai-good-implementation-guidance-a8b8}
...
### Comparison Analysis {#sec-ai-good-comparison-analysis-2ae6}
...
::: {.callout-quiz-question #quiz-question-sec-ai-good-selection-framework-c35d}

1. Which design pattern is most suitable for a low-resource environment with high adaptability needs?
...
2. Explain why the Hierarchical Processing Pattern is suitable for environments with stable infrastructure.

3. True or False: The Distributed Knowledge Pattern is primarily focused on temporal adaptation to changing conditions.

4. The ____ Pattern is ideal for systems requiring dynamic resource allocation in fluctuating environments.

5. Discuss the trade-offs between using the Progressive Enhancement Pattern and the Hierarchical Processing Pattern in a resource-constrained environment.

See Answer \ref{quiz-answer-sec-ai-good-selection-framework-c35d}.

:::

## Conclusion {#sec-ai-good-conclusion-2648}

The potential of AI for addressing societal challenges is undeniable. However, the path to successful deployment is anything but straightforward. ML systems for social good are not "plug-and-play" solutions, as they are complex engineering endeavors.
...
These patterns will evolve as technological capabilities advance and deployment contexts transform. Developments in edge computing, federated learning, and adaptive ML architectures will expand the potential applications of these patterns, particularly in resource-constrained environments. However, the core principles, such as accessibility, reliability, and scalability, remain fundamental to developing ML systems that generate meaningful social impact.
...
::: {.callout-quiz-question #quiz-question-sec-ai-good-conclusion-2648}

1. Explain why ML systems for social good are not 'plug-and-play' solutions and how this affects their deployment.

2. The implementation of design patterns in ML systems for social good depends fundamentally on a comprehensive understanding of both the operational environment and ____ requirements.

3. True or False: The core principles of accessibility, reliability, and scalability are likely to change as technological capabilities advance.

4. Which of the following is a critical consideration when selecting a design pattern for ML systems in resource-constrained environments?
   a) Network reliability
   b) Historical data availability
   c) User interface design
   d) Marketing strategy

See Answer \ref{quiz-answer-sec-ai-good-conclusion-2648}.

:::

## Resources {#sec-ai-good-resources-aba5}
...
## Self-Check Answers {#sec-ai-good-selfcheck-answers-fc01}

:::{.callout-quiz-answer #quiz-answer-sec-ai-good-global-challenges-a1f6}

1. **Explain how systemic barriers in healthcare and agriculture can exacerbate global challenges. Provide examples from the text.**

   *Answer*: Systemic barriers in healthcare, such as lack of access to medical expertise and diagnostic tools, can lead to preventable health crises. In agriculture, limited information for smallholder farmers can result in reduced yields due to erratic weather and pest outbreaks. These barriers perpetuate inequities and undermine resilience, as seen in remote communities facing health crises and farmers struggling with food security.

   *Learning Objective*: Understand how systemic barriers in healthcare and agriculture contribute to global challenges.
...
2. **What is a significant challenge in addressing global environmental issues as highlighted in the text?**
   ...
   *Answer*: The correct answer is B. Lack of monitoring and protection in vulnerable regions leaves ecosystems susceptible to illegal activities like poaching and logging, exacerbating environmental degradation.

   *Learning Objective*: Identify challenges related to environmental degradation and resource constraints.

3. **True or False: Machine learning systems can help address global challenges by processing large amounts of data to uncover patterns and generate insights.**

   *Answer*: True. Machine learning systems have the potential to transformatively address global challenges by analyzing vast data sets to inform decision-making and improve efficiency, especially in resource-constrained environments.

   *Learning Objective*: Recognize the potential of machine learning systems in addressing global challenges.

4. **Resource constraints and systemic barriers often result in the perpetuation of ____ and undermine resilience in affected regions.**

   *Answer*: The answer is "inequities". Resource constraints and systemic barriers exacerbate existing inequalities, making it difficult for affected regions to implement effective solutions and build resilience.

   *Learning Objective*: Understand the impact of resource constraints and systemic barriers on global challenges.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-good-key-ai-applications-d79f}
...
1. **Which AI technology is used to optimize water usage in rice paddies in Indonesia?**
   ...
   *Answer*: The correct answer is D. Tiny ML sensors are used to monitor microclimates and optimize water usage locally in Indonesian rice paddies, enabling precision irrigation.

   *Learning Objective*: Understand the application of Tiny ML in agriculture for optimizing resource usage.

2. **Explain how Tiny ML is used in disaster response to enhance safety and accelerate efforts.**

   *Answer*: Tiny ML is used in autonomous drones to navigate disaster zones, analyzing thermal imaging and acoustic signals locally to identify survivors and hazards without cloud connectivity, thus enhancing safety and accelerating response efforts.

   *Learning Objective*: Analyze the role of Tiny ML in improving disaster response capabilities.

3. **EdgeML-powered collars are used to track animal behavior by processing data on the collar itself, minimizing ____ consumption.**

   *Answer*: The answer is "power". By processing data on the collar, these devices minimize power consumption, reducing the need for frequent battery changes.

   *Learning Objective*: Recall the operational advantage of EdgeML in conservation efforts.

4. **True or False: Cloud ML is primarily responsible for enabling real-time monitoring of illegal fishing activities.**

   *Answer*: True. Cloud ML analyzes satellite data to detect anomalies in fishing activities, helping enforce regulations and protect marine ecosystems.

   *Learning Objective*: Evaluate the role of Cloud ML in environmental conservation.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-good-global-development-perspective-8e14}

1. **Explain how the United Nations Sustainable Development Goals (SDGs) provide a framework for deploying machine learning systems to address global challenges.**

   *Answer*: The SDGs provide a structured lens to align efforts, prioritize actions, and foster international collaboration, ensuring that ML systems target pressing global challenges effectively. This alignment helps create impactful and lasting change by integrating technological solutions with broader development objectives.

   *Learning Objective*: Understand the role of global frameworks in guiding ML system deployment for sustainable development.
...
2. **What is a significant challenge when deploying ML systems in regions that could benefit most from them?**
   ...
   *Answer*: The correct answer is C. Unreliable electricity and internet infrastructure are significant challenges in deploying ML systems in regions that need them most, necessitating innovative design solutions that consider local resource constraints.

   *Learning Objective*: Identify key infrastructural challenges in deploying ML systems for social impact.

3. **Machine learning systems must be designed to operate within local resource constraints while respecting ____ contexts and existing infrastructure limitations.**

   *Answer*: The answer is "cultural". ML systems must respect cultural contexts to ensure their sustainable integration and acceptance within communities.

   *Learning Objective*: Recognize the importance of cultural considerations in the design of ML systems for global development.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-good-engineering-challenges-944a}
...
1. **What is a major challenge when scaling ML systems from development to production in resource-constrained environments?**
   ...
   *Answer*: The correct answer is B. Maintaining consistent network performance is a major challenge as different deployment contexts require different communication protocols, affecting bandwidth and latency conditions.

   *Learning Objective*: Understand the challenges of scaling ML systems in resource-constrained environments.

2. **Explain why power infrastructure is a critical consideration in rural ML deployments.**

   *Answer*: Power infrastructure is critical in rural ML deployments because these systems often rely on solar and battery power, which require careful power budgeting due to limited generation and storage capacity. This affects every aspect of system operation, from sensor sampling rates to model inference frequency.

   *Learning Objective*: Analyze the impact of power infrastructure on ML system design in rural settings.

3. **In resource-constrained environments, models must often be redesigned to operate within strict ____ limits.**

   *Answer*: The answer is "power". Models must be optimized to function within the limited power availability of rural deployments, which often rely on solar and battery systems.

   *Learning Objective*: Recall the constraints that influence ML model design in resource-constrained environments.

4. **True or False: High-bandwidth network options are readily available in rural ML deployments.**

   *Answer*: False. Rural ML deployments typically rely on low-power wide-area network technologies like LoRa or NB-IoT, which offer lower bandwidth compared to urban environments.

   *Learning Objective*: Understand the network constraints in rural ML system deployments.

5. **Discuss the importance of community capacity building in the sustainability of ML systems in social impact contexts.**

   *Answer*: Community capacity building is crucial for the sustainability of ML systems because it ensures that local technicians can maintain and adapt systems independently. This involves providing accessible documentation and training programs to bridge knowledge gaps, enabling communities to manage and sustain the systems over time.

   *Learning Objective*: Evaluate the role of community engagement in sustaining ML systems in resource-constrained environments.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-good-design-patterns-236e}
...
1. **What is a key advantage of the Hierarchical Processing Pattern in machine learning systems?**
   ...
   *Answer*: The correct answer is B. The Hierarchical Processing Pattern allows for real-time processing at the edge tier, enabling critical functions to continue even during network disruptions, which is essential for maintaining system resilience in varying infrastructure conditions.

   *Learning Objective*: Understand the operational benefits of the Hierarchical Processing Pattern in ML systems.

2. **Explain how the Distributed Knowledge Pattern can enhance system resilience in resource-constrained environments.**

   *Answer*: The Distributed Knowledge Pattern enhances system resilience by enabling nodes to operate independently and share insights through peer-to-peer communication. This decentralization allows systems to maintain functionality even with intermittent connectivity, as nodes can process data locally and share processed insights when network conditions permit.

   *Learning Objective*: Analyze how distributed knowledge sharing contributes to system resilience in challenging environments.
...
3. **Which of the following is a challenge associated with the Hierarchical Processing Pattern in ML systems?**
   ...
   *Answer*: The correct answer is B. Managing dynamic model updates across tiers is a challenge because ML models require regular updates to adapt to new data, and synchronization across tiers must be carefully managed to prevent outdated models from affecting system performance.

   *Learning Objective*: Identify challenges in implementing the Hierarchical Processing Pattern in ML systems.

4. **Discuss the role of edge computing in the Distributed Knowledge Pattern.**

   *Answer*: Edge computing plays a crucial role in the Distributed Knowledge Pattern by enabling local data processing and inference, reducing the dependency on centralized systems. This allows for real-time decision-making and efficient use of limited resources, particularly important in environments with intermittent connectivity or low bandwidth.

   *Learning Objective*: Evaluate the impact of edge computing on distributed knowledge systems.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-good-selection-framework-c35d}
...
1. **Which design pattern is most suitable for a low-resource environment with high adaptability needs?**
   ...
   *Answer*: The correct answer is B. The Progressive Enhancement Pattern is designed for environments with low resources and high adaptability needs, allowing systems to scale capabilities smoothly as resources become available.

   *Learning Objective*: Identify the appropriate design pattern for specific resource and adaptability constraints.

2. **Explain why the Hierarchical Processing Pattern is suitable for environments with stable infrastructure.**

   *Answer*: The Hierarchical Processing Pattern is suitable for stable infrastructure environments because it organizes operations into clear tiers, allowing for efficient data flow and model updates. This structure supports scalability and resilience, which are crucial for maintaining system responsiveness in social impact applications.

   *Learning Objective*: Analyze the suitability of design patterns based on infrastructure stability.

3. **True or False: The Distributed Knowledge Pattern is primarily focused on temporal adaptation to changing conditions.**

   *Answer*: False. The Distributed Knowledge Pattern focuses on spatial distribution and peer-to-peer collaboration, rather than temporal adaptation, which is the focus of the Adaptive Resource Pattern.

   *Learning Objective*: Differentiate between design patterns based on their focus areas.

4. **The ____ Pattern is ideal for systems requiring dynamic resource allocation in fluctuating environments.**

   *Answer*: The answer is "Adaptive Resource". The Adaptive Resource Pattern is designed for environments with fluctuating resource availability, allowing systems to adjust operations dynamically to maintain functionality.

   *Learning Objective*: Recall the design pattern suited for dynamic resource management.

5. **Discuss the trade-offs between using the Progressive Enhancement Pattern and the Hierarchical Processing Pattern in a resource-constrained environment.**

   *Answer*: The Progressive Enhancement Pattern offers adaptability to resource variability, making it suitable for environments with unpredictable resource availability. However, it can increase complexity in layered design. The Hierarchical Processing Pattern provides scalability and resilience but requires stable infrastructure and can face synchronization issues. In resource-constrained environments, the choice between these patterns depends on whether adaptability or structured scalability is prioritized.

   *Learning Objective*: Evaluate trade-offs between different design patterns in resource-constrained environments.

:::

:::{.callout-quiz-answer #quiz-answer-sec-ai-good-conclusion-2648}

1. **Explain why ML systems for social good are not 'plug-and-play' solutions and how this affects their deployment.**

   *Answer*: ML systems for social good are not 'plug-and-play' because they must be tailored to operate under severe constraints such as limited power, unreliable connectivity, and sparse data. This complexity requires reimagining every component from data collection to model deployment, demanding rigorous engineering to meet the needs of underserved communities.

   *Learning Objective*: Understand the complexity and customization required for deploying ML systems in social impact contexts.

2. **The implementation of design patterns in ML systems for social good depends fundamentally on a comprehensive understanding of both the operational environment and ____ requirements.**

   *Answer*: The answer is "system". The implementation of design patterns must consider system requirements and operational constraints to ensure effectiveness and sustainability.

   *Learning Objective*: Recall the factors influencing the implementation of design patterns in ML systems.

3. **True or False: The core principles of accessibility, reliability, and scalability are likely to change as technological capabilities advance.**

   *Answer*: False. While technological capabilities will advance and expand potential applications, the core principles of accessibility, reliability, and scalability remain fundamental to developing ML systems that generate meaningful social impact.

   *Learning Objective*: Recognize the enduring principles essential for ML systems in social impact applications.
...
4. **Which of the following is a critical consideration when selecting a design pattern for ML systems in resource-constrained environments?**
   ...
   *Answer*: The correct answer is A. Network reliability is a critical consideration because it affects the feasibility and performance of ML systems in resource-constrained environments.

   *Learning Objective*: Identify key considerations in selecting design patterns for ML systems in constrained environments.
