...
1. **Which of the following is a common characteristic of global challenges such as healthcare access, agricultural productivity, and educational inequity?**
   ...
   The correct answer is C. These challenges disproportionately affect vulnerable populations, exacerbating existing inequalities and highlighting the need for targeted solutions.

   *Learning Objective*: Understand the common characteristics of global challenges and their impact on vulnerable populations.

2. **Explain how machine learning systems can help address the systemic barriers faced by smallholder farmers in agriculture.**

   Machine learning systems can process vast amounts of data to provide smallholder farmers with insights on weather patterns, pest outbreaks, and soil conditions, enabling better decision-making and improving yields. This helps overcome information constraints and enhances food security.

   *Learning Objective*: Analyze the role of ML systems in overcoming systemic barriers in agriculture.

3. **True or False: Addressing global challenges with technology requires no trade-offs between competing priorities.**

   False. Addressing global challenges with technology often requires navigating trade-offs between competing priorities and limited resources, especially in resource-constrained environments.

   *Learning Objective*: Recognize the trade-offs involved in deploying technology to address global challenges.

4. **In the context of global challenges, machine learning systems can help uncover patterns and generate insights to inform action, even in ______ environments.**

   The answer is "resource-constrained". Machine learning systems can process large data sets and provide actionable insights, which is particularly valuable in environments with limited resources.
...
1. **Which AI technology is primarily used for processing data locally to optimize water usage in agriculture?**
   ...
   The correct answer is D. Tiny ML is used for processing data locally to optimize water usage in agriculture, as it allows for low-power, localized data processing, which is crucial in areas with minimal infrastructure.

   *Learning Objective*: Understand the specific applications and advantages of Tiny ML in agriculture.

2. **True or False: Cloud ML is essential for enabling real-time monitoring of mosquito species in remote areas.**

   False. Tiny ML is essential for enabling real-time monitoring of mosquito species in remote areas, as it allows for low-cost, localized processing without the need for internet connectivity.

   *Learning Objective*: Differentiate between the roles of Tiny ML and Cloud ML in healthcare and environmental monitoring.

3. **Explain how AI technologies can enhance disaster response efforts in remote areas.**

   AI technologies enhance disaster response by using Tiny ML for autonomous drones to navigate and detect signs of life locally, and Cloud ML for processing satellite imagery to predict flood zones. These technologies provide real-time insights and enable faster, more efficient resource allocation.

   *Learning Objective*: Analyze how AI technologies improve operational efficiency and effectiveness in disaster response.

4. **In environmental conservation, Edge ML-powered collars are used to track animal behavior by processing data ______.**

   The answer is "locally". Edge ML-powered collars process data locally, minimizing power consumption and reducing the need for frequent battery changes, which is crucial for long-term wildlife monitoring.
...
1. **Explain why it is necessary to consider local resource constraints when designing ML systems for advancing the SDGs.**
   ...
   Considering local resource constraints is essential because many regions that could benefit from ML applications lack reliable electricity and internet infrastructure. Designing systems that operate within these constraints ensures they are sustainable and effective in advancing the SDGs without creating new dependencies or inequalities.
...
2. **True or False: Deploying ML systems in regions lacking reliable infrastructure can deepen existing inequalities if not carefully managed.**
   ...
   True. If ML systems are deployed without considering local infrastructure limitations, they can exacerbate inequalities by creating dependencies on technologies that are not accessible to all, thereby widening the gap between those with and without access.
...
1. **Which of the following best describes the primary challenge when deploying ML systems in rural environments?**
   ...
   The correct answer is B. Limited computational and power resources. Rural environments often lack the infrastructure needed for traditional ML deployments, requiring systems to operate on minimal resources.

   *Learning Objective*: Understand the primary resource constraints faced in rural ML system deployments.

2. **Explain how scaling machine learning systems from development to production in resource-constrained environments impacts model architecture.**

   Scaling ML systems requires significant changes in model architecture due to reduced computational and power resources. Models must be quantized and pruned to fit within the constraints of microcontrollers, often requiring a reduction from 50 MB to 500 KB while maintaining performance.
...
3. **True or False: In resource-constrained environments, maintaining ML systems is solely about optimizing computational efficiency.**
   ...
   False. While computational efficiency is crucial, maintaining ML systems also involves addressing environmental sustainability, community capacity building, and financial viability.
...
1. **Which of the following best describes the primary advantage of using the Hierarchical Processing Pattern in machine learning systems?**
   ...
   The correct answer is B. The Hierarchical Processing Pattern allows for efficient resource allocation across different tiers, optimizing resource usage by distributing tasks based on the capabilities of edge, regional, and cloud tiers.
...
3. **In the Hierarchical Processing Pattern, the ______ tier is responsible for advanced analytics and model training.**
   ...
   The answer is "cloud". The cloud tier provides the computational power needed for comprehensive analytics and model retraining, supporting the overall hierarchical structure.

   *Learning Objective*: Recall the specific responsibilities of each tier in the Hierarchical Processing Pattern.

4. **True or False: The Hierarchical Processing Pattern eliminates the need for model updates at the edge tier.**

   False. The Hierarchical Processing Pattern includes bidirectional feedback loops where model updates flow back to the edge tier, ensuring continuous improvement and adaptation to new data.
...
1. **Which design pattern is most suitable for environments with low resource availability and high adaptability needs?**
   ...
   The correct answer is B. The Progressive Enhancement Pattern is designed to provide baseline functionality in resource-constrained environments while allowing for dynamic scaling as resources become available.

   *Learning Objective*: Understand which design patterns align with specific resource and adaptability requirements.

2. **Explain how the quadrant analysis framework assists in selecting a design pattern for ML systems in social impact contexts.**

   The quadrant analysis framework helps by mapping design patterns based on resource availability and scalability/adaptability needs, allowing system architects to visualize and select patterns that best fit the operational constraints and requirements of their deployment scenarios.

   *Learning Objective*: Analyze how the quadrant analysis framework aids in making informed design pattern selections.

3. **In environments with high resource availability and significant scalability demands, the ______ pattern is commonly used.**

   The answer is "Hierarchical Processing Pattern". This pattern is effective in environments where robust infrastructure supports scalability and resilience, such as Google's Flood Forecasting Initiative.

   *Learning Objective*: Recall the design pattern best suited for high-resource, high-scalability environments.

4. **True or False: The Distributed Knowledge Pattern is ideal for environments where centralized coordination is essential.**

   False. The Distributed Knowledge Pattern is suited for environments where centralized coordination is not feasible, emphasizing peer-to-peer operations and decentralized data processing.
...
1. **Explain why the implementation of design patterns in ML systems for social good requires a comprehensive understanding of the operational environment.**
   ...
   Implementing design patterns in ML systems for social good requires understanding the operational environment to tailor solutions to constraints like power, connectivity, and data availability. This ensures the systems are effective and sustainable, addressing local needs while maintaining performance.
...
2. **True or False: The core principles of accessibility, reliability, and scalability in ML systems for social impact will remain constant despite advancements in technology.**
   ...
   True. While technological advancements will expand the potential applications of design patterns, the core principles of accessibility, reliability, and scalability remain fundamental to developing ML systems that generate meaningful social impact.
...
3. **Which of the following factors is most critical when selecting a design pattern for ML systems in resource-constrained environments?**
   ...
   The correct answer is B. Resource availability and adaptability requirements are critical for selecting a design pattern, as they dictate how the system can be effectively implemented and sustained in resource-constrained environments.
