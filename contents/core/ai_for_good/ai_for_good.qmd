...
For millions in underserved communities, access to healthcare often means long waits and travel to distant clinics. TinyML is changing that by enabling diagnostics to occur at the patient's side. For example, a low-cost wearable developed by [Respira xColabs](https://www.samayhealth.com/) uses embedded machine learning to analyze cough patterns and detect pneumonia. Designed for remote areas, the device operates independently of internet connectivity and is powered by a simple microcontroller, making life-saving diagnostics accessible to those who need it most.

TinyML's potential extends to tackling global health issues like vector-borne diseases. Researchers have developed low-cost devices that use machine learning to identify mosquito species by their wingbeat frequencies [@altayeb2022classifying]. This technology enables real-time monitoring of malaria-carrying mosquitoes, offering a scalable solution for vector control in high-risk regions.

In parallel, Cloud ML is advancing healthcare research and diagnostics on a broader scale. Platforms like Google Genomics analyze vast datasets to identify disease markers, accelerating breakthroughs in personalized medicine. These examples show how AI technologies—from TinyML's portability to Cloud ML's computational power—are converging to democratize healthcare access and improve outcomes worldwide.

### Disaster Response

In disaster zones, where every second counts, AI technologies are providing tools to accelerate response efforts and enhance safety. Tiny, autonomous drones equipped with Edge ML algorithms are making their way into collapsed buildings, navigating obstacles to detect signs of life. By analyzing thermal imaging and acoustic signals locally, these drones can identify survivors and hazards without relying on cloud connectivity [@duisterhof2021sniffy].
...
Machine learning systems for social impact present unique challenges that require thoughtful application of design patterns. The patterns discussed in this chapter—Hierarchical Processing, Progressive Enhancement, Distributed Knowledge, and Adaptive Resource—provide frameworks for addressing these challenges while ensuring systems remain effective and sustainable across diverse deployment contexts.

The selection and implementation of these patterns depends critically on understanding both the operational environment and system requirements. Resource availability and adaptability needs often drive initial pattern selection, while specific implementation decisions must consider factors such as network reliability, computational constraints, and scalability requirements. The success of social impact applications relies not just on selecting the appropriate pattern, but on implementing it in a way that addresses local constraints while maintaining system effectiveness.

Looking forward, these patterns will continue to evolve as new technologies emerge and deployment contexts change. Advances in edge computing, federated learning, and adaptive ML architectures will likely expand the capabilities of these patterns, particularly in resource-constrained environments. However, the fundamental principles—ensuring accessibility, maintaining reliability, and enabling scalability—will remain essential for developing ML systems that create meaningful social impact.

The thoughtful application of these design patterns, guided by careful consideration of deployment contexts and constraints, enables the development of ML systems that can operate effectively across the computing spectrum while delivering sustainable social impact.
