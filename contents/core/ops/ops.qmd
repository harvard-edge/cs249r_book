...
1. **Which of the following best describes the role of a feature store in an MLOps framework?**
   ...
   The correct answer is C. A centralized repository for storing and retrieving engineered features. Feature stores ensure consistent access to features across training and inference, reducing training-serving skew and improving model reliability.

   *Learning Objective*: Understand the purpose and function of feature stores in MLOps.

2. **Explain why dataset versioning is critical in MLOps.**

   Dataset versioning is critical in MLOps because it ensures reproducibility and traceability of machine learning experiments. By maintaining a clear mapping between data versions and model iterations, teams can audit decisions, reproduce results, and manage model updates effectively.
...
3. **True or False: In MLOps, model governance is only concerned with the technical performance of models.**
   ...
   False. Model governance in MLOps encompasses transparency, fairness, and compliance, ensuring that models are not only technically performant but also ethically sound and aligned with regulatory requirements.

   *Learning Objective*: Understand the broader scope of model governance beyond technical performance.

4. **In MLOps, ________ pipelines automate the process of transitioning models from development to production, ensuring reproducibility and scalability.**

   The answer is "CI/CD". CI/CD pipelines automate the integration and deployment processes, supporting reproducibility and scalability in transitioning models from development to production.
...
1. **Explain why technical debt in machine learning systems is often more hidden than in traditional software systems.**
   ...
   Technical debt in ML systems is often more hidden due to the complexity and interdependencies of data, models, and infrastructure. Unlike traditional software, where debt is primarily in code, ML debt arises from data dependencies, feedback loops, and evolving operational environments, which are less visible and harder to quantify.
...
1. **Which role is primarily responsible for ensuring data integrity and managing data pipelines in an MLOps framework?**
   ...
   The correct answer is B. Data Engineers are responsible for building and maintaining data pipelines, ensuring data quality, structure, and lineage, which are crucial for reliable ML operations.

   *Learning Objective*: Understand the primary responsibilities of Data Engineers in MLOps.

2. **Explain how the collaboration between Data Scientists and ML Engineers is crucial for transitioning models from development to production.**

   Data Scientists develop models which ML Engineers then translate into production-ready systems. This collaboration ensures that models are optimized for performance and integrated into the application ecosystem, addressing latency, throughput, and resource constraints.
...
3. **In MLOps, the ________ Engineer is responsible for provisioning infrastructure and managing CI/CD pipelines to support model deployment and monitoring.**
   ...
   The answer is "DevOps". DevOps Engineers ensure that the infrastructure is scalable and reliable, and that deployment processes are automated and repeatable, which is essential for maintaining operational efficiency in ML systems.

   *Learning Objective*: Recall the responsibilities of DevOps Engineers in the MLOps lifecycle.

4. **True or False: The Responsible AI Lead is only concerned with the technical performance of machine learning models.**

   False. The Responsible AI Lead focuses on ensuring that ML systems operate in ways that are transparent, fair, and compliant with ethical and regulatory standards, beyond just technical performance.
...
1. **Which of the following best describes the impact of high operational maturity on ML system architecture?**
   ...
   The correct answer is B. High operational maturity enhances modularity, automation, and observability, allowing for scalable and maintainable ML systems.

   *Learning Objective*: Understand how high operational maturity influences the design and architecture of ML systems.

2. **Explain how operational maturity affects the organizational structure of teams involved in ML system development.**

   Operational maturity affects organizational structure by promoting clear ownership, cross-functional collaboration, and interface discipline. Mature environments often have centralized MLOps teams or federated models to ensure consistency and reduce duplicated efforts, enabling efficient scaling and integration.
...
3. **True or False: Operational maturity in ML systems is solely determined by the adoption of advanced tools like CI/CD pipelines and model registries.**
   ...
   False. Operational maturity is not solely determined by tool adoption but by how well these tools are integrated into cohesive systems that support reliability, reproducibility, and responsiveness.
...
4. **In high-maturity ML environments, ________ is used to manage infrastructure, enabling reproducibility and scalability.**
   ...
   The answer is "infrastructure-as-code". This approach allows infrastructure to be managed programmatically, ensuring reproducibility and scalability in ML systems.
...
1. **What was a key challenge in deploying the Oura Ring's ML models in a resource-constrained environment?**
   ...
   The correct answer is A. Ensuring high predictive accuracy with limited computational resources was a key challenge because the Oura Ring operates on edge devices with constraints on memory and compute power.

   *Learning Objective*: Understand the operational challenges of deploying ML models in resource-constrained environments.

2. **Explain how the ClinAIOps framework addresses the limitations of traditional MLOps in clinical settings.**

   ClinAIOps addresses limitations by integrating structured feedback loops that involve patients, clinicians, and AI systems, ensuring human oversight, ethical accountability, and alignment with clinical workflows. This framework supports personalized care and continuous adaptation, which traditional MLOps lacks.

   *Learning Objective*: Analyze how ClinAIOps adapts MLOps principles for clinical environments.

3. **True or False: The success of the Oura Ring's ML system was solely due to advanced model architectures.**

   False. The success of the Oura Ring's ML system was not solely due to advanced model architectures but also relied on robust data acquisition, preprocessing, and systematic MLOps practices.

   *Learning Objective*: Recognize the importance of comprehensive MLOps practices beyond model architecture in system success.

4. **In the ClinAIOps framework, the ________ loop ensures that AI-generated recommendations are reviewed and vetted by healthcare professionals.**

   The answer is "Clinician-AI". The Clinician-AI loop ensures that AI-generated recommendations are reviewed and vetted by healthcare professionals, maintaining clinical oversight and accountability.

   *Learning Objective*: Identify the role of the Clinician-AI loop in maintaining oversight in AI-driven healthcare systems.

5. **Order the following steps in the Oura Ring's ML deployment process: Model Compression, Data Preprocessing, Model Evaluation, OTA Deployment.**

   1. Data Preprocessing: Automated pipelines manage data ingestion and cleaning. 2. Model Evaluation: Models are validated against clinical standards. 3. Model Compression: Techniques like quantization reduce model size. 4. OTA Deployment: Models are deployed using over-the-air updates.
