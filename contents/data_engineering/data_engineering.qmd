...
Let us explore Wake Vision, a large dataset for TinyML Person detection. The dataset is "bootstrapped" from a larger, general-purpose dataset, Open Images [@kuznetsova2020open], and repurposed for the use case we care about (binary person detection). This process leverages the exisiting labels and bounding boxes in Open Images by filtering and relabeling the data based with an automated pipeline. This approach not only saves time and resources but also ensures that the dataset is tailored to the specific needs of the TinyML application. Furthermore, we can create additional metadata that allows us to benchmark the fairness and robustness of a model in challenging scenarios.
Let's get started!  
