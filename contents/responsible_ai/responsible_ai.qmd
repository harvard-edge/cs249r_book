...
To illustrate, imagine a model predicting loan repayment where the plusses (+'s) represent repayment and the circles (O's) represent default in @fig-fairness-example. The optimal accuracy would be correctly classifying all of Group A while misclassifying some of Group B's creditworthy applicants as defaults. If positive classifications allow access loans, Group A would receive many more loans---which would naturally result in a biased outcome.

![Various fairness metrics and accuracy may result in different optimal classifiers.](images/fairness_cartoon.png){#fig-fairness-example}
...
![Diffusion models may memorize samples from their training data. Source: [@carlini2023extracting].](images/diffusion_memorization.png){#fig-diffusion-model-example}
...
![Example of a small perturbation changing a model prediction.](images/adversarial_robustness.png){#fig-adversarial-example}

*Figure @fig-adversarial-example: Example of a small meaningless perturbation changing a model prediction. Courtesy of [Microsoft](https://www.microsoft.com/en-us/research/blog/adversarial-robustness-as-a-prior-for-better-transfer-learning/).*
...
![How various groups impact human-centered AI.](images/human_centered_ai.png){#fig-human-centered-ai}
