...
- $x_{i}$ is the individual input feature, where $i$ ranges from $1$ to $N$.
- $w_{ij}$ are the weights connecting neuron $i$ in one layer to neuron $j$ in the next layer, which are adjusted during training.
- $f(\theta)$ is the non-linear activation function applied at each layer (e.g., ReLU, softmax, etc.).
- $A_{j}$ is the output of the neural network at each layer $j$, where $j$ denotes the layer number.
...
- $M$ is the number of hidden neurons in the final layer before the output.
- $w_{jk}$ is the weight between hidden neuron $a_j$ and output neuron $y_k$.
- $g(\theta)$ is the activation function applied to the weighted sum of the hidden layer outputs.

Our neural network, as defined, performs a sequence of linear and nonlinear operations on the input data ($x_{i}$) to obtain predictions ($y_{i}$), which hopefully is a good answer to what we want the neural network to do on the input (i.e., classify if the input image is a cat or not). Our neural network may then be represented succinctly as a function $N$ which takes in an input $x \in \mathbb{R}^{d_0}$ parameterized by $W_1, ..., W_n$, and produces the final output $y_k$:

$$
N(x; W_1, ..., W_n) = y_k \quad \text{where } A_0 = x
$$

This equation indicates that the network starts with the input $A_0 = x$ and iteratively computes $A_j$ at each layer using the parameters $W_j$ until it produces the final output $y_k$ at the output layer.
...
The validation set should also be manageable, leaving sufficient samples for training. Overall, the validation set is a critical piece of the data-splitting process, and care should be taken to avoid the pitfalls of small, inadequate samples that negatively impact model development.
