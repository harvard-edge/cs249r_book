...
Consider a situation where we decide to prune 1/3 of the weights. In @fig-iterative-pruning, we have 6 total weights, so we want to remove 2 of them. In the first iteration, edge `A-X` has the lowest weight with a value of 0.009, so we prune it and fine-tune on the remaining edges. In our case, the fine-tuning has changed some of the weights in a way that changed their ranking/importance. Now, the lowest-weight edge is `C-Y`, with a value of 0.011, so we prune it on the second iteration. 


![In iterative pruning on edges, we gradually remove a few edges and fine-tune on the new structure, which can change the weights and ranking of the remaining edges.](images/jpg/modeloptimization_iterative_pruning.jpeg){#fig-iterative-pruning}
...
Now consider the same network we had in the iterative pruning example. In one-shot pruning, we would have to prune the lowest two weights in one iteration: `A-X` and `B-X`. As you can see in @fig-oneshot-pruning, the final structure is different from that in @fig-iterative-pruning. Since we didn't fine-tune on the new structure, the network didn't have the optimally update the weights, and we ended up with a sub-optimal pruned structure. We pruned `B-X` prematurely, when we should have pruned `C-Y`. But we had no way of knowing that without iteratively pruning with fine-tuning.


![In one-shot pruning on edges, we remove a certain number of edges with the lowest weights. This might lead to sub-optimal pruning since the network doesn't have the chance to update the weights as it does in iterative pruning.](images/jpg/modeloptimization_oneshot_pruning.jpeg){#fig-oneshot-pruning}
