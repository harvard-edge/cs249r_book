...
![In structured iterative pruning, we gradually remove a small set of structures, in this case channels. Running fine-tuning on the new structure on every iteration allows the model to quickly adapt to the changes. The gradual and controlled nature of iterative pruning ensures that the accuracy drop on each iteration is minimal. Subsequently, the network typically manages to restore its accuracy to levels comparable to its original, unpruned state.](images/jpg/modeloptimization_iterative_pruning.jpeg){#fig-iterative-pruning}
