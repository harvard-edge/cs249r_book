...
ML serving systems employ various techniques to optimize performance and resource utilization, such as the following:

1. **Request scheduling and batching:** The serving system efficiently manages the incoming ML inference requests, optimizing performance and resource utilization through smart queuing and grouping strategies.
2. **Model instance selection and routing:** Intelligent routing algorithms inside the serving system direct each request to the most appropriate model version or instance, ensuring optimal handling of diverse prediction tasks.
3. **Load balancing:** The workloads are distributed evenly across multiple serving instances, preventing bottlenecks and maintaining consistent performance under varying loads.
4. **Model instance autoscaling:** The serving infrastructure dynamically adjusts its capacity based on demand, efficiently handling workload fluctuations while optimizing resource use.
5. **Model orchestration:** Sophisticated coordination systems manage model execution, enabling parallel processing, partitioning of large models, and strategic placement across available hardware resources.

Several works have contributed to these advancements in ML serving systems. @crankshaw2017clipper for instance introduce [Clipper](https://rise.cs.berkeley.edu/projects/clipper/), which is a low-latency online prediction serving system designed to meet the demands of real-time, accurate, and robust predictions under heavy query load. It introduces a modular architecture to simplify model deployment across frameworks and applications, and incorporates caching, batching, and adaptive model selection techniques to reduce prediction latency and improve throughput, accuracy, and robustness without modifying underlying machine learning frameworks. In Clockwork [@gujarati2020serving], the system focuses on delivering high-performance model serving by predicting execution times of individual inferences and making efficient use of costly hardware accelerators.

@romero2021infaas further explored model instance selection and autoscaling in their [INFaaS framework](https://github.com/stanford-mast/INFaaS). INFaaS generates model-variants from already trained models and efficiently navigates the trade-off space of model-variants to select the appropriate model, hardware architecture, and optimizations for each query. Developers can simply specify the performance and accuracy requirements for their applications, and INFaaS handles the selection process. @zhang2019mark show how to do load balancing and autoscaling in their MArk (Model
Ark) system. @li2023alpaserve demonstrated advanced model orchestration techniques in their AlpaServe framework, enabling efficient handling of large models and complex serving scenarios.

One of the key challenges for model serving systems is operating under performance requirements defined by Service Level Agreements (SLAs) and Service Level Objectives (SLOs). SLAs are formal contracts specifying expected service levels. These service levels rely on metrics such as response time, availability, and throughput. SLOs are internal goals teams set to meet or exceed their SLAs. For ML model serving, these agreements and objectives directly impact user experience, system reliability, and business outcomes. For instance, a recommendation system for an e-commerce platform might have an SLA guaranteeing response times under 100 milliseconds for 99.9% of requests. Meeting such requirements in production environments under varying load and complex models presents significant challenges for serving. Therefore, teams carefully tune their serving platform and many decision-making algorithms, including some of the techniques we discussed above.
