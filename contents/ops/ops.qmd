...
The MLOps lifecycle starts from data management (@sec-ops-datamgnt) and CI/CD pipelines for model development (@sec-ops-model-dev). Developed models go through model training (@sec-ops-model-train) and evaluation (@sec-ops-model-eval). Once trained to convergence, model deployment (@sec-ops-model-deploy) brings models up to production and ready to serve. After deployment, model serving (Sec. 13.3.6) reacts to workload changes and meets SLAs cost-effectively when serving millions of end users or AI applications. Infrastructure management (@sec-ops-infra) ensures the necessary resources are available and optimized throughout the lifecycle. Continuous monitoring (@sec-ops-monitor), Governance (@sec-ops-govern), and communication and collaboration (@sec-ops-comm) are the remaining pieces of MLOps to ensure seamless development and operations of ML models.
...
![MLOps CI/CD diagram. Source: HarvardX.](images/png/cicd_pipelines.png){#fig-ci-cd}
...
After model deployment, serving becomes a critical component in the MLOps lifecycle. It bridges the gap between developed models and ML applications or end-users, ensuring that deployed models are accessible, performant, and scalable in production environments.
...
3. **Near-Online (semi-synchronous) Serving:** Balances between online and offline, offering relatively quick responses for less time-sensitive applications.

To support these serving types and meet diverse application needs, model serving systems incorporate several key functionalities that work in concert to ensure efficient, scalable, and reliable model inference in production environments:

1. **Request scheduling and batching [@crankshaw2017clipper; @gujarati2020serving]:** The serving system efficiently manages the incoming ML inference requests, optimizing performance and resource utilization through smart queuing and grouping strategies.
2. **Model instance selection and routing [@crankshaw2017clipper; @romero2021infaas]:** Intelligent routing algorithms inside the serving system direct each request to the most appropriate model version or instance, ensuring optimal handling of diverse prediction tasks.
3. **Load balancing [@zhang2019mark]:** The workloads are distributed evenly across multiple serving instances, preventing bottlenecks and maintaining consistent performance under varying loads.
4. **Model instance autoscaling [@zhang2019mark; @romero2021infaas]:** The serving infrastructure dynamically adjusts its capacity based on demand, efficiently handling workload fluctuations while optimizing resource use.
5. **Model orchestration [@li2023alpaserve]:** Sophisticated coordination systems manage model execution, enabling parallel processing, partitioning of large models, and strategic placement across available hardware resources.

One of the key challenges for model serving systems is operating under performance requirements defined by Service Level Agreements (SLAs) and Service Level Objectives (SLOs). SLAs are formal contracts specifying expected service levels. These service levels rely on metrics such as response time, availability, and throughput. SLOs are internal goals teams set to meet or exceed their SLAs. For ML model serving, these agreements and objectives directly impact user experience, system reliability, and business outcomes. For instance, a recommendation system for an e-commerce platform might have an SLA guaranteeing response times under 100 milliseconds for 99.9% of requests. Meeting such requirements in production environments under varying load and complex models presents significant challenges for serving. Therefore, teams carefully tune their serving platform and many decision-making algorithms.

The tuning process encompasses a range of techniques to enhance system performance:

1. Optimizing model inference speed through techniques like quantization or pruning, (see [Optimizations](../optimizations/optimizations.qmd) chapter).
2. Implementing caching [@crankshaw2017clipper] mechanisms to reduce redundant computations.
3. Utilizing hardware accelerators [@romero2021infaas] like GPUs or TPUs for faster processing.
4. Employing adaptive batching [@crankshaw2017clipper] to balance between latency and throughput.
5. Implementing circuit breakers and rate limiting [@gujarati2020serving] to prevent system overload.
6. Dynamic autoscaling and load balancing [@zhang2019mark] to smoothly address workload fluctuations.
7. Continuous online monitoring and alerting to quickly identify and address performance or availability issues.

By focusing on these aspects of model serving, organizations can ensure their ML models deliver consistent, high-quality predictions at scale, meeting the demands of modern, data-driven applications.
