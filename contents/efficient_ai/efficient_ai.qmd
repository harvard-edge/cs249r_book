...
![Pruning applies different criteria that determine which nodes and/or weights can be removed without having significant impact on the model's performance.](images/pruning.jpeg){#fig-pruning}
...
![One method of quantization involves rounding to the nearest representable number. Quantization helps save on memory while minimizing performance loss.](images/quantization.jpeg){#fig-quantization}
...
![Many applications require very high-performance inference, which can be achieved with on device accelerators such as Edge TPUs. Source: [TensorFlow Blog](https://blog.tensorflow.org/2019/03/build-ai-that-works-offline-with-coral.html)](images/tflite_edge_tpu_perf.png){#fig-edge-tpu-perf}
