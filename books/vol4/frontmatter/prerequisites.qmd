...
* **Foundational Physics & Mathematics**: Readers should be comfortable with basic algebra, vectors and matrices, and single-variable calculus (rates of change). Physical AI relies directly on freshman Newtonian mechanics:
  ...
  - Kinetic energy and stopping distance napkin math ($E_k = \frac{1}{2} m v^2$, $d_{\text{stop}} \approx v \cdot t + \frac{v^2}{2a}$).
  - Basic electrical power and heating ($P = I_{\text{phase}}^2 R_{\text{phase}}$).
  Advanced differential geometry, Lie group continuous control theory, and formal reachability analysis are deliberately not required; their mathematical treatments are quarantined in the volume appendices.
...
Every number in this book belongs to one of four categories, and its category decides what it may be used for.
...
* **Illustrative**: Chosen to show arithmetic or scaling. It supports no margin, timeout, or release decision.

A budget or requirement, such as a lease duration or a clearance margin, is none of these. It is a design decision, labeled **chosen**, and it means something only when it is checked against the measured or derived number it must fit, as a chosen lease is checked against the derived ceiling on pre-brake delay.
