...
One machine runs through the book: a warehouse mobile manipulator, a wheeled base that must stop before a person stepping out from a rack and an arm that must open a spring-latched door and take a mug from a moving conveyor within a contact force. Each chapter adds what it owns to that machine's stopping budget, so the value one chapter computes is the value the next chapter uses. Each part closes by testing its law on a humanoid, where all three classes meet on one chassis.
