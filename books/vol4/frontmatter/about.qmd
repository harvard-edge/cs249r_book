...
One machine runs through the book: a warehouse mobile manipulator, a wheeled base that must stop before a person stepping out from a rack, and an arm that must open a spring-latched door, take a mug from a moving conveyor, and hand it to a coworker, each within a contact force. Each chapter adds what it owns to that machine's stopping budget, so the value one chapter computes is the value the next chapter uses. Each part closes by testing its law on a humanoid, which couples all three classes on one chassis.
...
* **Explicit Foundational Context**: Whenever a chapter draws upon a core concept established earlier (such as the proposal boundary, the four-stream action tap tuple, or the three machine classes), the text provides an immediate inline orientation gloss and a precise section cross-reference.
