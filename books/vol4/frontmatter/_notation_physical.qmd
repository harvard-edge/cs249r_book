...
Below, we formalize the cross-discipline disambiguation contract, establish the governing cyber-physical laws of embodied systems, and tabulate the canonical symbols used throughout this book.
...
| **$\mathbf{a}$ vs. $\hat{\mathbf{u}}$ vs. $\mathbf{u}^*$ vs. $\boldsymbol{\tau}$** | Four-tier causal authority chain ($\mathbf{a}_t \to \hat{\mathbf{u}}_t \to \mathbf{u}^*_t \to \boldsymbol{\tau}_t$) | Generic "action" symbol colliding with acceleration $a_{\max}$ or barrier normal $\mathbf{a}(\mathbf{x})$                                                                                         | **Disambiguate execution authority**: $\mathbf{a}_t \in \mathcal{A}$ is the unprivileged policy action (dimensionless/normalized); $\hat{\mathbf{u}}_t \in \mathcal{U}$ is the proposed plant control setpoint; $\mathbf{u}^*_t \in \mathcal{U}$ is the certified, CBF-filtered control command; $\boldsymbol{\tau}_t \in \mathbb{R}^n$ is realized physical actuator torque. Linear deceleration is $a_{\max}$ ($\text{m/s}^2$); barrier constraints use $\mathbf{A}_{\text{cbf}}(\mathbf{x})\mathbf{u} \le \mathbf{b}_{\text{cbf}}(\mathbf{x})$. |
...
| **$D$**                                                                            | Dataset size ($D$ in tokens or trajectories)                                                                        | Data volume ($D_{\text{vol}}$), clearance distance ($d_{\text{clear}}$), duty cycle ($D_{\text{cycle}}$), TSDF field $D_t(\mathbf{v})$, or damping matrix $\mathbf{D}_{\text{virt}}$              | Training dataset size is $D$; bytes moved across buses is $D_{\text{vol}}$; physical stopping/clearance distances use lowercase $d$ ($d_{\text{stop}}, d_{\text{clear}}, d_{\text{gap}}$); actuator duty cycle uses subscripted $D_{\text{cycle}} \in [0, 1]$; voxel TSDF fields use $D_t(\mathbf{v})$; virtual damping matrices use bold $\mathbf{D}_{\text{virt}}$.                                                                                                                                                                              |
...
## Governing Cyber-Physical Systems Laws {#sec-notation-governing-laws .unlisted}

Embodied intelligence operates subject to strict thermodynamic, mechanical, kinematic, and communications constraints. The governing laws below formalize these physical boundaries across the volume:

### 1. The embodied iron law: Sense-to-torque latency budget {#sec-notation-embodied-iron-law .unlisted}

Computational latency converts directly into unguided physical displacement and tracking error. Total closed-loop sense-to-torque loop time $T_{\text{loop}}$ must satisfy the hard real-time cadence bound:
...
| **$f_{\text{ctrl}}$**  | **Control Execution Frequency**    | $\text{Hz}$ | Target closed-loop execution rate ($1000\text{ Hz}$ for joint reflex loops; $20\text{--}50\text{ Hz}$ for cognitive chunks). |

:::

### 2. The action-chunking streaming cadence law {#sec-notation-chunking-cadence .unlisted}

Cognitive neural policies cannot execute at kilohertz frequencies due to memory bandwidth limits. Action chunking predicts an action trajectory of horizon $H$ at a lower cognitive frequency $f_{\text{policy}}$, while real-time reflexes execute at $f_{\text{exec}}$. The memory streaming latency must remain bounded by the chunk execution window:
...
| **Symbol**                   | **Definition**                 | **Unit**    | **Notes**                                                                                                   |
|:-----------------------------|:-------------------------------|:------------|:------------------------------------------------------------------------------------------------------------|
| **$H$**                      | **Action Chunk Horizon**       | Time steps  | Number of contiguous future action steps generated in a single policy forward pass ($H=16$ or $H=32$).      |
...
Forward-invariance of safe set $\mathcal{C}$ dictates that the machine must maintain sufficient braking distance to arrest all motion before violating environmental clearance margin $d_{\text{gap}}$:

$$d_{\text{stop}}(v) = \underbrace{v \cdot t_{\text{react}}}_{\text{Reaction Drift}} + \underbrace{\frac{v^2}{2 a_{\max}}}_{\text{Deceleration Envelope}} + \underbrace{\frac{v^3}{6 j_{\max}}}_{\text{Jerk Ramp Distance}} + \delta_{\text{loc}} + \delta_{\text{margin}} \le d_{\text{gap}}$$
...
| **$a_{\max}$**               | **Maximum Deceleration**       | $\text{m/s}^2$ | Peak actuator or braking deceleration capability within plant thermal and traction limits.             |
| **$j_{\max}$**               | **Jerk Limit**                 | $\text{m/s}^3$ | Maximum rate of change of deceleration, bounded to prevent gearbox tooth shock and drivetrain ringing. |
| **$\delta_{\text{loc}}$**    | **Localization Uncertainty**   | $\text{m}$     | Spatial state estimation bound from Kalman/perception filtering ($3\sigma$ confidence ellipsoid).      |
| **$\delta_{\text{margin}}$** | **Physical Clearance Cushion** | $\text{m}$     | Mandatory nonzero structural buffer between the machine envelope and environment.                      |

:::

### 4. The digital sampling zero-order hold (ZOH) phase margin law {#sec-notation-zoh-law .unlisted}
...
| **$\tau_{\text{age}}$**                                         | **Telemetry Spatial Age**           | $\text{ms}$                    | Total elapsed duration since physical transduction: $\tau_{\text{age}} = \frac{1}{2} t_{\text{exp}} + t_{\text{read}} + t_{\text{xfer}} + t_{\text{proc}}$.     |
...
| **$h_{\text{kinetic}}(\mathbf{x})$**                     | **Kinetic Inset Barrier Function** | Scalar                                  | Relative-degree-1 barrier: $h_{\text{kinetic}}(\mathbf{x}) = h(\mathbf{x}) - v^2 / (2 a_{\max})$ incorporating stopping distance.                                       |
...
| **$H$**                                                              | **Chunk Prediction Horizon**         | Time steps    | Number of future action steps emitted per inference cycle ($H=16, 32, 64$).                                                                                                         |
...
| **$T_{\text{lease}}$**                                               | **Intent Lease Expiration Horizon**  | $\text{ms}$   | Maximum allowable duration of autonomous execution authority before mandatory re-evaluation ($\le 100\text{ ms}$).                                                                  |
...
| **$\delta_{\text{skew}}$**            | **Clock Synchronization Skew**       | $\text{ppm}, \mu\text{s/s}$   | Hardware crystal drift between perception host and real-time microcontroller clocks.                                                                        |
