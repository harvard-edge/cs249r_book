...
Consider an autonomous mobile robot (AMR) in a warehouse with mass $m = 250\text{ kg}$ cruising at $v_0 = 1.8\text{ m/s}$ ($4.0\text{ mph}$). The perception camera runs at $30\text{ Hz}$ ($33.3\text{ ms}$ exposure and readout). The vision-language-action (VLA) navigation policy runs on an edge accelerator with an inference latency of $120\text{ ms}$. Motor controller CAN bus communication consumes $15\text{ ms}$. The maximum allowable braking deceleration without tipping cargo is $a_{\max} = 2.0\text{ m/s}^2$.

* **Lag displacement:** Total sense-to-actuation lag is $t_{\text{lag}} = 33.3 + 120 + 15 = 168.3\text{ ms} \approx 0.168\text{ s}$. During this computational window, the unguided robot coasts:
  $$d_{\text{lag}} = v_0 \cdot t_{\text{lag}} = (1.8\text{ m/s})(0.1683\text{ s}) \approx 0.303\text{ m} \ (30.3\text{ cm})$$
* **Braking distance:** Bringing the physical mass to a complete halt requires:
  $$d_{\text{brake}} = \frac{v_0^2}{2a_{\max}} = \frac{1.8^2}{2(2.0)} = \frac{3.24}{4.0} = 0.810\text{ m} \ (81.0\text{ cm})$$
* **Total required stopping clearance:** Total stopping distance is $d_{\text{stop}} = d_{\text{lag}} + d_{\text{brake}} = 0.303 + 0.810 = 1.113\text{ m}$.
* **The cost of compute scale:** If an engineer upgrades to a larger foundation model that increases inference latency from $120\text{ ms}$ to $320\text{ ms}$, total lag expands to $t_{\text{lag}}' = 368.3\text{ ms}$, driving unguided displacement to $d_{\text{lag}}' = 0.663\text{ m}$ and total stopping distance to $d_{\text{stop}}' = 1.473\text{ m}$ ($+36.0\text{ cm}$, a $+32.3\%$ expansion). In narrow warehouse aisles, that 200 ms computational delay directly consumes the entire safety margin.
:::
...
A 6-axis articulated robotic arm is assembling automotive transmission components. The end-effector approaches a rigid steel fixture ($k = 2.5 \times 10^5\text{ N/m}$) at $v = 0.20\text{ m/s}$. The robot's cycloidal transmission has a maximum allowable shock yield force of $F_{\text{yield}} = 1500\text{ N}$.

* **Stall displacement:** If an unhandled operating system scheduling hiccup or memory lock pauses the control loop for $\Delta t = 75\text{ ms}$, unguided penetration into the fixture accumulates:
  $$\Delta x = v \cdot \Delta t = (0.20\text{ m/s})(0.075\text{ s}) = 0.015\text{ m} = 15.0\text{ mm}$$
* **Generated contact force:** By Hooke's law, the resulting mechanical contact force is:
  $$F = k \cdot \Delta x = (2.5 \times 10^5\text{ N/m})(0.015\text{ m}) = 3750\text{ N}$$
  This generates a contact force $2.5\times$ greater than the gearbox shear limit ($3750\text{ N} / 1500\text{ N} = 2.5$), causing catastrophic mechanical tooth shear.
* **The reflex deadline:** The maximum allowable penetration before yield is $\Delta x_{\max} = \frac{1500\text{ N}}{2.5 \times 10^5\text{ N/m}} = 6.0\text{ mm}$. The hard deadline for the bare-metal current loop to detect the collision and clamp motor torque is:
  $$t_{\text{deadline}} = \frac{\Delta x_{\max}}{v} = \frac{0.006\text{ m}}{0.20\text{ m/s}} = 0.030\text{ s} = 30\text{ ms}$$
* **Systems lesson:** Structural destruction occurs within 30 ms. A high-level 20 Hz neural policy (50 ms cycle) cannot protect the mechanical plant; invariant enforcement must belong to a sub-millisecond microcontroller reflex loop.
:::
...
A bipedal humanoid powers its onboard edge compute ($12.0\text{ V}$ regulated rail, $10.8\text{ V}$ brownout reset threshold) and 12 joint actuators from a common $48.0\text{ V}$ battery pack. The wiring harness connecting the battery to the power distribution board has parasitic inductance $L_{\text{bus}} = 25.0\,\mu\text{H}$. During a dynamic jump landing, the leg actuators simultaneously command a current surge from $10\text{ A}$ to $110\text{ A}$ in $\Delta t = 0.2\text{ ms}$ ($dI_{\text{bus}}/dt = 5.0 \times 10^5\text{ A/s}$).

* **Current slew rate:** The sudden phase current demand generates a current slew rate:
  $$\frac{dI_{\text{bus}}}{dt} = \frac{110\text{ A} - 10\text{ A}}{0.2 \times 10^{-3}\text{ s}} = 5.0 \times 10^5\text{ A/s} = 500\text{ A/ms}$$
* **Transient inductive droop:** By Faraday's law of induction, the transient voltage droop across the bus is:
  $$\Delta V_{\text{droop}} = L_{\text{bus}} \cdot \frac{dI_{\text{bus}}}{dt} = (25.0 \times 10^{-6}\text{ H})(5.0 \times 10^5\text{ A/s}) = 12.5\text{ V}$$
* **Bus voltage collapse:** The instantaneous bus voltage plummets:
  $$V_{\text{bus, min}} = 48.0\text{ V} - 12.5\text{ V} = 35.5\text{ V}$$
  If the DC-DC step-down converter feeding the $12\text{ V}$ compute rail drops out when input voltage falls below $38.0\text{ V}$, this landing spike instantly triggers an unrecoverable processor reboot and bipedal collapse.
* **Systems lesson:** Physical AI demands physical power domain isolation. High-capacity neural computers cannot share unbuffered, inductive power rails with high-torque motor drives.
:::
