...
\begin{marginfigure}
\vspace{1em}
\resizebox{\linewidth}{!}{\mlphysicalstackfull{20}{20}{20}{100}{20}}
\end{marginfigure}
:::
...
The chapter follows a command from computation to physical work, compares the budgets of four machine archetypes, orders the machine into five levels, and uses the Tempe collision to test the boundary between learned proposals and actuation permission. The final sections turn that failure analysis into four laws and the architecture of the book.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: The blind-travel paragraph after @psp-boundary-latency-in-millimeters.
...
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import fmt, fmt_frequency, check

class MachineLevelRates:
    """Pinned level rates of the five-level machine (chosen values, not measurements)."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    intent_lo = 1 * hertz  # category: chosen (intent model, lower bound)
    intent_hi = 5 * hertz  # category: chosen (intent model, upper bound)
    chunk_lo = 10 * hertz  # category: chosen (chunk policy, lower bound)
    chunk_hi = 50 * hertz  # category: chosen (chunk policy, upper bound)
    permission_example = (
        Embodied.MobileManipulator.WarehouseMobileManipulator.permission_rate
    )  # category: chosen (running machine, one implementation)
    current_lo = 10 * ureg.kilohertz  # category: chosen (current loop, lower bound)
    current_hi = 25 * ureg.kilohertz  # category: chosen (current loop, upper bound)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    intent_to_str = f"{fmt(intent_lo.magnitude, precision=0)} to {fmt_frequency(intent_hi, unit=hertz, precision=0)}"
    chunk_to_str = f"{fmt(chunk_lo.magnitude, precision=0)} to {fmt_frequency(chunk_hi, unit=hertz, precision=0)}"
    current_to_str = f"{fmt(current_lo.magnitude, precision=0)} to {fmt_frequency(current_hi, unit=ureg.kilohertz, precision=0)}"
    intent_range_str = f"{fmt(intent_lo.magnitude, precision=0)}–{fmt_frequency(intent_hi, unit=hertz, precision=0)}"
    chunk_range_str = f"{fmt(chunk_lo.magnitude, precision=0)}–{fmt_frequency(chunk_hi, unit=hertz, precision=0)}"
    current_range_str = f"{fmt(current_lo.magnitude, precision=0)}–{fmt_frequency(current_hi, unit=ureg.kilohertz, precision=0)}"
...
| **Execution Cadence**     | Throughput-optimized batching; soft or unbounded latency budgets                                           | Hard real-time single-rate loops on microcontrollers (100–1000 Hz)               | Multi-rate: an intent model (`{python} MachineLevelRates.intent_range_str`) and a chunk policy (`{python} MachineLevelRates.chunk_range_str`) propose; a permission loop, `{python} MachineLevelRates.permission_str` in one implementation, admits |
...
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import fmt_length, fmt_energy, fmt_qty, fmt_velocity, fmt_acceleration, fmt_percent, check
...
To see the consequences of this scaling, return to the warehouse base cruising at its `{python} MachineMomentum.v_str` drive limit toward a rack end where a person has stepped out. Loaded with its tote rack, its mass is at most `{python} MachineMomentum.m_loaded_str`, and it carries `{python} MachineMomentum.e_kinetic_str` of kinetic energy, which braking must shed through the grip of its wheels on the floor. At a credible deceleration of `{python} MachineMomentum.a_brake_str`, braking alone takes `{python} MachineMomentum.d_brake_str` of floor. The `{python} MachineMomentum.blind_str` of blind travel during the pre-brake delay comes before any of that braking, adding `{python} MachineMomentum.blind_share_str` to the distance. The two terms answer to different design choices. A larger model that lengthens the delay lengthens the blind travel in proportion, whereas doubling the cruising speed would quadruple both the energy and the braking distance. The machine therefore does not let a learned model's latency set its stopping distance. An independent check bounds how long the base may keep acting on a proposal that has not been renewed, and @sec-nervous sizes that bound from the stopping budget.
...
- [ ] **Sense-to-actuation lag**: Can you explain why the delay $\tau_{\text{delay}}$ from frame capture to braking onset becomes blind travel $v\,\tau_{\text{delay}}$ that no braking can recover?
...
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import fmt, fmt_length, fmt_latency, fmt_velocity, fmt_qty, fmt_sci_qty, check
...
class LatchContact:
    ...
    f_limit_str = fmt_qty(f_limit, newton, precision=0)
    t_stall_str = fmt_latency(t_stall, unit=millisecond, precision=0)
    dx_str = fmt_length(dx, unit=millimeter, precision=1)
    force_str = fmt_qty(force, newton, precision=0, commas=True)
    force_ratio_str = fmt(force_ratio, precision=0)
```

Contact stiffness can make a held motion command expensive before the high-level planner returns. Consider the arm of the same warehouse mobile manipulator driving its gripper toward the strike plate of a spring-latched door at `{python} LatchContact.v_str`, and suppose its low-level controller keeps executing that command while the high-level process stalls for `{python} LatchContact.t_stall_str`. The plate acts as a spring of stiffness `{python} LatchContact.k_str`, the latch tolerates at most `{python} LatchContact.f_limit_str` of contact force, and the arm is assumed to have enough torque to keep moving through contact. Under these assumptions the plate deflects by $\Delta x = v \cdot \Delta t =$ `{python} LatchContact.dx_str`, and the spring model gives $F = k \cdot \Delta x =$ `{python} LatchContact.force_str`, `{python} LatchContact.force_ratio_str` times the limit. Motor saturation, compliance elsewhere, or a controller that stops on stale commands would change that result. Against a plate this stiff, the time available to detect contact and bring commanded motion within the limit is a matter of milliseconds, far shorter than any planner stall, and @sec-training derives that budget for this plate.
...
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import fmt, fmt_qty, fmt_power, fmt_latency, fmt_frequency, check
...
class ProcessBudgets:
    ...
    c_thermal_str = fmt_qty(c_thermal, kilojoule / kelvin, precision=0)
    warming_rate_str = fmt_qty(warming_rate, kelvin / second, precision=1)
    margin_str = fmt_qty(margin, kelvin, precision=0)
...
This book follows one machine from its first chapter to its last. The warehouse mobile manipulator is a wheeled base that carries a seven-joint arm, a navigation camera and lidar, a wrist camera, and two computers. An application processor runs its learned models, and a separate safety microcontroller decides, on a fixed short period, whether a proposed motion may reach the motors. Its chunk policy is a whole-body action-chunking model that proposes base velocity and arm joint targets together, as mobile manipulation systems of this class do [@fu2024mobile]. The base is a Class 1 machine that must stop before it reaches a person who steps out from the end of a rack. The arm is a Class 2 machine that must open a spring-latched door and lift an unfamiliar mug from a moving conveyor without exceeding a contact force. Each chapter adds what it owns to a single stopping budget for this machine, and the value one chapter computes is the value the next chapter uses. A Class 3 process plant appears once in each part as a contrast.

Belonging to one of these three physical archetypes does not by itself mean that a machine requires the full systems discipline of Physical AI. A classical hydraulic press or a deterministic PID drone autopilot manipulates mass and contact force, but operates entirely within classical control theory. To determine when a machine crosses into Physical AI, we must establish a rigorous diagnostic scope test.
...
Before examining the five-level machine, verify your diagnostic ability to categorize ambiguous autonomous machines:
...
These two failures, one before impact and one after, motivate four laws.
...
- [ ] **Dual-failure poles across the boundary**: Can you contrast the pre-impact failure mode (suppressing braking due to classification thrashing) with the post-impact failure mode (a fallback that moved the vehicle while a pedestrian was pinned beneath it)?

*If any check reveals uncertainty, review the forensic autopsy in this section before proceeding to the four laws.*
:::

## The Four Bedrock Laws {#sec-boundary-bedrock-laws}

Four laws hold for every machine whose learned proposals can reach an actuator. Each one leads a part of this book, and together they turn the causal boundary into requirements.
...
For computer systems engineers, the third law resembles hardware-enforced protection rings: the learned model proposes from an unprivileged domain, while a more trusted controller can refuse an inadmissible torque command before actuation. The analogy is limited because refusing a command also requires a controlled physical fallback; a page fault alone does not stop moving mass. The permission path therefore checks freshness, limits, and fallback feasibility before any command reaches an actuator. One implementation exchanges proposals through a nonblocking mailbox (@sec-nervous-cadences).
...
The book follows one question through four parts: can a proposed action cross the causal boundary before the physical margin runs out, and on what evidence? Each part is led by one of the four laws, and every chapter of Parts I to IV except @sec-brain closes by producing a record that a later chapter consumes (@fig-01-spine). Read in order, the records carry the argument forward: what the plant can do, what the learned proposer has shown it can do, what it proposes now, what the permission path admits, and what the evidence licenses. The two middle strands meet at the permission decision, which is the third law in the form of an interface.
...
# │ Show: Stall duration and coasting distance at the drive limit. The stall's
# │       cost to the stopping budget is @sec-intent's.
...
The machine's chunk policy stalls for `{python} StallCoast.t_stall_str` when its memory subsystem throttles. The computation pauses, but physical momentum ensures the base keeps moving. A base that kept executing its last proposal at `{python} StallCoast.v_str` would coast blind for `{python} StallCoast.coast_str` before the next command arrived, and that displacement can consume the clearance it needs to stop if a person steps out. The machine does not allow the coast. A lease bounds how long the base may act on the last proposal, and when the lease expires the permission path brakes whether or not a new proposal has arrived; @sec-intent accounts for what such a stall costs the base. A hard real-time reflex loop must maintain the stopping envelope and respond to missed deadlines while the deliberative policy is unavailable.
...
* **Proposal Boundary:** Learned models may only propose; an independent permission path with bounded latency checks freshness, limits, and fallback feasibility before a command reaches an actuator. A dedicated real-time microcontroller is one implementation.
* **Five-Level Machine:** The machine is ordered from its learned proposers down to the physics they act on (Brain, Nervous System, Body, Boundary, World), divided by the proposal boundary between the Brain and the Nervous System, and surrounded by governance as a lifecycle envelope rather than a sixth level.
* **Four Bedrock Laws:** Irreversibility, endogenous data, proposal is not permission, and evidence bounds authority each lead one part of the book, and the third law is the one the other three serve.
