...
| Contact-dominated | Manipulators, grippers, presses       | Actuator current and gearbox stress          | Gear tooth shear; workpiece damage  | Torque-derivative clamp and $C^2$ profiler  |
...
At Tempe, the one second of suppressed braking was spent before any response began, and nothing the software did afterward could return that distance. Four laws hold for every machine whose learned proposals can reach an actuator, and Tempe's lost second is the first of them at work. Each one leads a part of this book, and together they turn the causal boundary into requirements.
...
Each misconception below imports an assumption that holds for software or for classical control into a machine whose learned proposals can reach an actuator. The first three misplace a system relative to the scope test, and the last three assume that a human can intervene instantly, that latency only degrades performance, or that physical work can be undone.
