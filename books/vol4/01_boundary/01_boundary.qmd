...
This asymmetry underlies **Moravec's paradox**\index{Moravec's paradox}[^fn-ai-moravec-paradox] [@moravec1988mind]: tasks that require intense human conscious effort, such as playing chess or proving theorems, require modest computation, whereas sensorimotor skills executed effortlessly—such as walking across uneven gravel or grasping an egg without crushing it—demand vast computational and control resources. As high-capacity neural networks are deployed to master these sensorimotor tasks, engineering teams encounter the *exposure barrier*: offline benchmark accuracy can neither predict nor guarantee closed-loop stability in the open world. The steep rise in commercial autonomous fleet mileage in @fig-boundary-autonomous-miles shows that physical deployment shifts machine learning from controlled demonstrations to massive cumulative exposure, where safety arguments must withstand millions of hours of non-stationary interaction.
...
Advisory software operates within an informational sandbox where computational errors carry no physical momentum and incur no thermodynamic cost. When a recommender model serves an irrelevant product, a classifier mislabels an image, or a language model emits an inaccurate token, the computational pipeline terminates in a display buffer, a network socket, or a sandboxed database table. Failures remain bounded by the digital medium. A user dismisses the recommendation, a human analyst closes the alert, or a database transaction coordinator issues an abort that restores memory pages from an undo log. In computer systems engineering, this property is known as *idempotency*: the state of the surrounding physical universe remains completely decoupled from the intermediate outputs of the software pipeline.[^fn-math-idempotency-physical] *Advisory software is immune to its own errors because of where it stops, not because of how it was built.* Pipeline safety stems not from statistical guarantees or rigorous validation, but from the physical detachment of the interface. Between the output of the neural network and any change in the physical world, an air gap or a human operator intercepts the signal.
...
The boundary conditions of Physical AI are life-critical systems requirements, not mere theoretical classifications. Examining what happens when an embodied machine violates them reveals their importance. Granting unverified motor authority over physical mass to a high-capacity learned model converts software failure into kinetic destruction due to loss of idempotency. A physical crash in an autonomous machine rarely results from an inaccurate prediction alone, but from the lack of an architectural mechanism to refuse it.
...
Despite continuous sensor contact for over five seconds, the vehicle did not brake. Between $T - 4.4\text{ s}$ and $T - 1.2\text{ s}$, the stochastic neural perception classifier repeatedly toggled its semantic classification label between an *unknown vehicle*, a *bicycle*, and an *unclassified static obstacle*.
...
The software tracking architecture coupled its Kalman filter state estimators[^fn-ctrl-kalman-tracking] to discrete semantic classification labels, causing each toggle to discard the existing kinematic track and re-initialize a new filter with an assumed velocity vector of zero. This classification instability prevented the downstream trajectory planner from accumulating evidence of an imminent collision course.
...
# 'TEMPE' is a framework for evaluating kinematic constraints in robotic systems. A 'kinematic stopping budget' quantifies the maximum distance a robot can travel before stopping, based on its speed and acceleration. This concept ensures robotic systems operate safely and effectively in their environments.
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-boundary-tempe-stopping-budget: This notation denotes constraints on resource allocation in AI systems, including computational power, memory, and time. For instance, computational tasks often have budgets that limit the CPU cycles or memory bandwidth available, directly impacting their performance and energy consumption. Understanding these constraints is crucial for designing efficient algorithms that operate within defined physical limits. This concept is vital for designing efficient algorithms within defined limits.
# │ Goal: Quantify 'safety stopping distance,' defined as the minimum distance required for a system to come to a complete stop under controlled conditions, in relation to 'software suppression lag,' which refers to the delay in system response due to software processing times. These concepts are essential for assessing the safety and effectiveness of control algorithms in physical AI systems.
# │ Show:    In this context, 'd_lag' is the distance for the system to process sensor data: 23.0 m. 'd_brake' is the distance for the vehicle to stop: 23.0 m; 'd_stop' is the total stopping distance: 46.1 m. The remaining distance of 3.8 m underscores the urgency, necessitating timely decision-making in control algorithms.
# │ How:     The kinematic delay model describes how an object's motion is affected by its initial velocity (v0), the time during which deceleration occurs (t_suppress), and the maximum deceleration (a_max). Specifically, the equation v0 * t_suppress + v0^2 / (2 * a_max) illustrates the relationship between these variables in determining the distance an object travels while decelerating. This model is essential for designing physical AI systems, as it reveals how motion dynamics influence control strategies and system performance.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
::: {#fig-01-tempe-vehicle fig-env="figure" fig-pos="t" fig-cap="**Autonomous Vehicle Sensor Suite and Failure Points**: The developmental testbed involved in the 2018 Tempe collision [@ntsb2019uber], annotated across four critical physical and cybernetic subsystems: (1) roof-mounted 64-beam LiDAR providing direct 3D range from 82 meters, (2) forward vision and 77 GHz radar where classification oscillation reset kinematic tracks, (3) factory Autonomous Emergency Braking (AEB) intentionally suppressed to avoid bus contention, eliminating the hardware-isolated safety referee, and (4) trunk ADS compute rack where a 1.2-second software suppression delay consumed 23.0 meters of unguided stopping clearance before impact. (Source: NTSB, Public Domain)." fig-alt="Annotated photographic plate of the developmental autonomous test vehicle showing four crisp rectangular callout cards: (1) Roof 64-Beam LiDAR Puck, (2) Forward Vision and 77 GHz Radar, (3) Suppressed Factory AEB highlighting the missing safety referee, and (4) Trunk ADS Compute Rack detailing the fatal 1.2-second software suppression delay."}
![](images/jpg/fig01_real_ntsb_tempe_vehicle_annotated.jpg){width="95%"}
:::
...
The second bedrock law governs how embodied policies learn and fail. In traditional computer vision or natural language processing, data is exogenous: a model's correct identification of an image does not affect subsequent image distributions. In standard supervised learning with per-step error probability $\epsilon$, expected cumulative errors across an episode of horizon $T$ grow linearly: $\mathbb{E}[\text{Errors}] \le T\epsilon$, because samples are drawn independently from a stationary distribution $p_{\text{data}}(x)$.
...
Engineering dependable physical AI systems requires mastering the composition of statistical learning, physical dynamics, and real-time computer systems across all machine tiers. To build this discipline systematically from first principles, this textbook is organized as an unbroken intellectual journey across four foundational movements (@tbl-01-book-roadmap):

**Part I: The Machine Anatomy (@sec-boundary through @sec-nervous).** Before writing a single line of policy code or launching a neural network training cluster, an engineer must first step onto the hardware floor. We begin by characterizing what the physical world will not negotiate—the unyielding laws of momentum, reflected gear inertia ($J_{\text{ref}} = N^2 J_{\text{rotor}}$), thermal dissipation, and microsecond clock jitter that bound the body, the nervous system, and the cognitive brain. Treating the physical plant as an immutable container establishes the non-negotiable temporal and dynamic budgets that every upstream algorithm must satisfy.
...
* **Proposal–Permission Contract (Dual-Brain Architecture):** High-capacity neural policies on application processors operate as unprivileged proposal engines, while isolated real-time microcontrollers retain exclusive authority to evaluate safety barrier certificates before committing PWM gate pulses to inverter switches.
...
The loss of idempotency across the causal boundary shows that computation cannot be decoupled from physical consequence: battery energy drawn cannot be reclaimed, momentum cannot be halted instantly, and unmodeled mechanical stress will damage hardware regardless of offline training convergence.
