...
While a single-class machine (such as a wheeled AMR or a fixed industrial arm) isolates its physical budget to a single dominant conservation law, a humanoid robot forces the systems architect to manage **cross-class coupled contention**\index{Cross-Class Coupled Contention}, in which no one budget can be sized without the other two (@sec-body-power-integrity quantifies the bus droop).
...
- [ ] **Blind travel and braking**: Can you explain why the pre-brake delay $\tau_{\text{delay}}$ becomes blind travel $v\,\tau_{\text{delay}}$ that no braking can recover, and why doubling speed quadruples the braking term but only doubles the delay term?
