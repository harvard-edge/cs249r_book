...
[^fn-ctrl-cbf-barrier]: **Control Barrier Functions (CBFs)**\index{Control Barrier Functions!safety filter}: Intuitively, a Control Barrier Function defines a mathematical 'safety cushion' around forbidden physical states (such as impending collisions or joint limits) [@ames2019control]. When solved as a Quadratic Program (CBF-QP), the safety filter computes the minimal necessary alteration to the neural network's candidate proposal to guarantee the system remains within its safe operating envelope. The formal optimization equations, Lie derivatives, and discrete-time margin bounds are deferred to @sec-appendix-control-cbf.
