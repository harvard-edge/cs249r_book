...
To ensure auditable tracking of unmonitored invariants across software deployments and physical revisions, safety-critical systems represent open residual claims through a formal, structured data contract (@tbl-17-residual-claims-schema). The schema defines the mandatory fields, physical SI units, and structural invariants required for every unresolved physical assumption.
...
| **Predeclared Closure**     | `sample_trials`          | count                  | Case-specific independent exposure (`{python} OilFilm.n_closure_str` zero misses for a `{python} OilFilm.confidence_sym_str` upper miss rate $\le$ `{python} OilFilm.p_miss_target_sym_str`) |
| **Predeclared Closure**     | `confidence_level`       | —                      | Declared confidence level (`{python} OilFilm.confidence_sym_str` in the worked detector test)                                                                                                |
...
The Tempe collision of @sec-boundary-when-fails and the Deepwater Horizon blowout fail in opposite ways. At Tempe the hazard was observable. The sensors had detected the pedestrian seconds before impact, and the architecture withheld braking while the available evidence already warranted it, so a permission path acting on that state had grounds to brake. In the Deepwater Horizon blowout the protective device acted on a state that no sensor reported. The blind shear rams closed on a drill pipe that buckling had pushed off center, a condition outside the geometry for which the rams were qualified. A faster trigger or broader authority would not have changed the outcome, because no measurement separated the qualified state from the actual one. Architecture repairs the first failure. Only qualification and sensing extended to every state the protection may meet can repair the second, and that second kind is the detectorless failure this section describes.
