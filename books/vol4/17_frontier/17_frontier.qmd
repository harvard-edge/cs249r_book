...
Yet every engineering method has a boundary where its guarantees expire. As captured by the Embodied Latency Scaling Law (\ref{dfn-frontier-embodied-latency-scaling-law}), simply scaling the parameter count of foundation models cannot circumvent physical mechanics: computational delay directly consumes the physical reaction time constant of the plant. Furthermore, as governed by Ashby's Law of Requisite Variety [@ashby1956introduction], a controller cannot regulate disturbances that its sensory architecture cannot perceive.
