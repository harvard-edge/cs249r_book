...
The mobile manipulator's release claim for the aisle is not one assertion but a chain of links, and which links it needs depends on the hazards and the protection chosen. The argument must establish the operating domain, measured actuator and stopping bounds, component contracts, and independent permission for learned proposals (@sec-boundary-machine-model). Hazards controlled by runtime detection also need measured detector coverage and timely monitoring. Hazards controlled by passive limits or operating restrictions instead need evidence that those measures hold over the declared load, wear, and exposure envelope. Generating extensive evidence for one link cannot compensate for an unsupported premise in another.[^fn-hist-popper-falsification]
...
Every link in a safety case requires an explicit **epistemic classification**\index{Epistemic classification}, identifying each premise as measured directly through physical instrumentation, derived analytically from measured premises, assumed on physical or operational grounds, decided by accountable judgment, left unsupported by existing data, or explicitly declared outside the threat model. For the running machine's base and arm, and for the process-line contrast, these classifications reveal the exact boundary where evidence ends. For the base, the stopping-distance claim relies on an assumed friction coefficient between wheel and floor that cannot be measured before braking begins. For the arm, the contact argument depends on an assumed damage threshold for the latch and the objects it handles, which is an engineering judgment about structural yield rather than a measured property of the controller. For the flow regulator, the pressure claim assumes a transducer whose zero point has not drifted, a premise that its own reading cannot check. Naming these premises explicitly does not weaken the release record. It establishes the physical envelope within which the release verdict is justified, and outside of which the verdict ceases to apply.
...
proposal_events_hypothetical = 0  # category: illustrative (separate counterfactual-oracle proposal log)
proposal_exposure_hours_hypothetical = 216000.0  # category: illustrative (proposal-log exposure)
...
Operational fleet data may also violate the model behind the bound. Fleet-wide updates, shared layouts, and correlated weather can create common-cause events; the observation process and exposure model must account for them. Human interventions should remain in the log as visited states and intercepted proposals, with authority flags and counterfactual adjudication where possible. A simulation contributes evidence only for the dynamics and disturbances it actually represents.
...
The relation requires a valid proposal oracle, timely detection, actuator authority, and a justified treatment of correlated failures. It gives no credit for an oil film or brittle material state that the shield cannot observe. The 90-day proposal record and $c_h=0.9999$ in the notebook would still miss the $10^{-9}\text{ h}^{-1}$ target. No release follows from the arithmetic until each premise is supported.[^fn-hw-fault-registers]

[^fn-hw-fault-registers]: **Physical Trip Path**\index{Hardware fault register!trip}: Some configured comparator and gate-driver paths can remove commanded torque without waiting for the host scheduler. What such a path contributes to a stopping claim depends on the pin response, drive behavior, and mechanical stopping time of the particular plant, and torque removal alone may permit coasting or gravity-driven motion.
...
Both aisle entries close only through a detector, a floor-film detector for the first and a qualified presence sensor for the second. Each closure test therefore has to show that the detector, once qualified, reports early enough for the permission path to act, and that requirement has a physical limit of its own.
...
At the $18\text{ ms}$ first possible command change, the unmitigated rigid-jaw model has reached $18\text{ N}$, already past the $15\text{ N}$ threshold. Had closure continued unchecked to $42\text{ ms}$, one winding time constant after that first command, the force would reach $42\text{ N}$. This is a detectorless-for-this-path failure: the observation and actuation chain cannot alter force before the workpiece's deadline. A different sensor or a validated passive force path is required.[^fn-math-contact-lipschitz]
...
The probe, the passive limit, and the second transducer answer the same deficit in different ways, and @tbl-17-detectorless-modes sets four detectorless modes beside the response each still permits.[^fn-hw-motor-thermal]
...
[^fn-math-hamming-bounds]: **Structural Fault Bounds**\index{Fault tolerance!structural bounds}: In classical fault-tolerant architectures, system dependability does not require eliminating every internal component error, but ensuring that fault propagation is bounded by structural invariants. When runtime detectors cannot resolve latent physical degradation, passive mechanical limiters or firmware parameter clamps prevent localized faults from escalating into a hazard. Such isolation supports containment only for the faults, load paths, and actuator states included in its validation.
...
Certain gaps cannot be closed by any known sensor, sampling regime, or mathematical framework. When physical laws, computational limits, or economic constraints preclude the necessary measurement or proof, the safety case cannot remain suspended in anticipation of future discovery. Narrowing the actuator's physical authority, restricting the operational design domain\index{Operational design domain}, or refusing to operate the machine is not an admission of research defeat. It is the engineering resolution. As established in classical fault-tolerant systems engineering, reliable cyber-physical systems cannot eliminate unmodeled open-world disturbances through unconstrained scale; rather, dependable operation requires explicit hardware self-monitoring thresholds\index{Self-Monitoring} and structural isolation\index{Structural Isolation} mechanisms that contain faults. A machine whose actuator authority is physically bounded below destructive force levels does not require an ultrasonic sensor to survive an undetected void. The residual-claims register records the boundary of verifiable knowledge, and the permission path and the machine's passive limits enforce the constraints that bound the consequence of what remains unknown.

### Research against an open claim

A new sensing or modeling method matters here when it supplies evidence for a named open entry. An event camera can report local brightness changes with low latency under a specified sensor mode; it does not measure oil friction or hidden ceramic voids. Compute-in-memory may reduce data movement on a target workload, but it cannot replace the independent permission path. Neither closes a register entry by itself.
...
Research therefore moves a machine's authority only by closing a named register entry. The register, the practical-detector test, and the four gap classes have so far been applied to machines whose records already existed. The method must also hold for a machine with no records yet, where the engineer has to decide which premises to measure, which to restrict, and which to leave open.
...
Because the tray's momentum cannot be recalled, the permission path must act before the carriage consumes its clearance. Consider, with illustrative values throughout, a linear carriage of mass $m_{\text{carriage}} = 12.0\text{ kg}$ transporting a loaded tray at a nominal velocity of $v_{\text{nom}} = 1.50\text{ m/s}$ toward an optical inspection station whose mechanical gate provides a physical clearance margin of $d_{\text{clear}} = 60.0\text{ mm}$ before contacting a rigid frame stop. An overhead vision system detects a closing inspection gate. The controller commands an electromagnetic track brake with a credible deceleration of $a_{\text{brake}} = 15.0\text{ m/s}^2$. Determining whether an onboard vision system can halt the carriage prior to an impact requires computing the end-to-end sensorimotor information age\index{Information age!end-to-end} across the full observation-to-enforcement path: optical exposure and rolling-shutter integration ($t_{\text{exp}} = 16.0\text{ ms}$), direct memory access (DMA) transfer and neural detection inference ($t_{\text{inf}} = 36.0\text{ ms}$), CAN-FD safety arbitration bus latency ($t_{\text{bus}} = 2.0\text{ ms}$), and brake solenoid flux buildup rise time ($t_{\text{sol}} = 10.0\text{ ms}$), summing to a total delay of $\tau_{\text{info}} = 16.0 + 36.0 + 2.0 + 10.0 = 64.0\text{ ms} = 0.064\text{ s}$. Over this sensing and actuation lag, the carriage travels unbraked for a lag distance $d_{\text{lag}} = v_{\text{nom}} \tau_{\text{info}} = 1.50\text{ m/s} \times 0.064\text{ s} = 96.0\text{ mm}$. Once the solenoid builds flux and mechanical pads engage, deceleration consumes an additional distance $d_{\text{brake}} = \frac{v_{\text{nom}}^2}{2 a_{\text{brake}}} = \frac{(1.50\text{ m/s})^2}{2 \times 15.0\text{ m/s}^2} = 75.0\text{ mm}$. The stopping distance of @sec-body-kinetic-momentum is the sum of the two, delay travel that grows linearly with speed and braking distance that grows with its square, so $d_{\text{stop}} = d_{\text{lag}} + d_{\text{brake}} = 96.0\text{ mm} + 75.0\text{ mm} = 171.0\text{ mm}$. Because $171.0\text{ mm}$ exceeds the $60.0\text{ mm}$ physical clearance by a deficit of $\Delta d = d_{\text{stop}} - d_{\text{clear}} = 111.0\text{ mm}$, the machine cannot stop before the modeled gate at this velocity by relying on that vision path. Because the unbraked lag distance alone ($d_{\text{lag}} = 96.0\text{ mm}$) exceeds the available clearance ($d_{\text{clear}} = 60.0\text{ mm}$), the physical deadline is missed before braking begins.
...
[^fn-std-sotif-ul4600]: **Standards Scope**\index{ISO 21448!SOTIF and UL 4600}: ISO 26262 and ISO 21448 concern road vehicles; UL 4600 provides an autonomous-product safety-case framework. Apply the standard relevant to the machine and its jurisdiction, then state this case's actual requirements.
...
**Fallacy**: *Empirical fleet testing can statistically validate ultra-rare failure rates in open-world environments.*

A delivery-vehicle developer presents $500{,}000\text{ hours}$ without accidents to support a claimed accident rate of $p \le 10^{-9}/\text{hour}$ at 95 percent confidence. Zero events bound a rate only at about three divided by the exposure hours (@sec-evaluation-trial-limits), so this record supports a rate thousands of times higher than the one claimed. Correlated fleet conditions and configuration changes require an argument about which observations are applicable. The record cannot support the proposed rate merely because no accident occurred. Release must narrow the claim or provide additional architectural evidence for bounded physical authority, with explicit assumptions and validation of the protective mechanism.
...
In the ceramic-gripper case, the rigid force ramp reaches $15\text{ N}$ at $15\text{ ms}$, while filter, bus, and MCU delay permit the first new command only at $18\text{ ms}$. The $24\text{ ms}$ winding time constant then describes current decay. Improving classification accuracy\index{Classification!latency tradeoff} without shortening the physical response cannot prevent that modeled fracture. The proposed software safeguard fails its physical timing requirement even if it identifies every event perfectly. The system design must instead modify the physical exposure or protection mechanism—for example, through series elastic actuators\index{Actuator!series elastic} or passive compliant mechanisms that bound contact forces below a case-specific, anatomically relevant contact threshold within the available actuation interval. Documenting a hazard supplies a software detection objective, not a compute pipeline and actuator dynamics\index{Actuator Dynamics} fast enough to act on it.
:::
...
The oil film shows what the fourth law asks of a machine once its records run out. Evidence bounds authority for as long as the machine operates, not only on the day a release is signed. The same case stress-tests the first law. The arithmetic of the stopping budget never changed, but one of its terms had rested all along on a floor that nothing measured, and irreversibility made that unmeasured term as binding as any measured one.
