...
class LatchTransformStaleness:
    ...
    omega_str = fmt_angular_velocity(omega, precision=2)
...
The choice of metric representation defines an empirical Pareto trade-off between volumetric memory footprint and spatial collision query latency (@fig-09-memory-3d-representation-pareto). Continuous trajectory optimization and real-time reflex controllers running in 1 kHz control loops ($1{,}000\ \mu\text{s}$ periods) require spatial clearance and gradient queries to execute in under $10\ \mu\text{s}$ across dozens of robot body articulation points. While implicit neural representations such as coordinate MLPs (NeRF, iMAP) achieve minimal memory density ($\sim 0.08\ \text{MB/m}^3$), querying signed distance or occupancy requires evaluating deep neural network forward passes, imposing massive computational latency ($>650\ \mu\text{s}$ per query point) that confines them to deliberative offline planners. Hierarchical octrees (OctoMap [@hornung2013octomap]) compress unobserved free space down to $0.18\text{--}1.25\ \text{MB/m}^3$, but pointer indirection during recursive tree traversals incurs CPU cache misses that limit query speeds to $24\text{--}38\ \mu\text{s}$. Conversely, dynamically hashed truncated signed distance fields (Voxblox TSDF [@oleynikova2017voxblox]) allocate memory only around observed surface bands ($4.1\ \text{MB/m}^3$), enabling $1.8\ \mu\text{s}$ ray queries, while propagating Euclidean signed distance fields (ESDF) increases memory overhead to $28.5\ \text{MB/m}^3$ to deliver sub-microsecond ($0.85\ \mu\text{s}$) distance evaluations with analytic continuous gradients. Meanwhile, 3D Gaussian splatting primitives [@kerbl20233d] occupy a middle regime ($1.6\text{--}5.2\ \text{MB/m}^3$, $5.8\text{--}8.2\ \mu\text{s}$), enabling photographic radiance rendering alongside bounding-volume-hierarchy accelerated collision checks.

::: {#fig-09-memory-3d-representation-pareto fig-env="figure" fig-pos="htb" fig-cap="**3D scene representation frontier: memory footprint versus spatial query latency**: Empirical Pareto trade-off across implicit neural fields, hierarchical octrees, 3D Gaussian splatting, and hashed voxel grids. While implicit coordinate MLPs compress rooms into megabytes of weights, multi-layer forward passes exceed real-time collision query limits ($>100\\ \\mu\\text{s}$). Hashed voxel grids (Voxblox TSDF/ESDF [@oleynikova2017voxblox]) consume $4.1\\text{--}28.5\\ \\text{MB/m}^3$ but provide the sub-microsecond query latencies ($<2\\ \\mu\\text{s}$) necessary to service 1 kHz reactive control loops." fig-alt="Log-log scatter plot of volumetric memory footprint in megabytes per cubic meter versus spatial point distance and collision query latency in microseconds. The empirical Pareto frontier descends from iMAP NeRF (0.08 MB/m3, 650 us) through OctoMap 5cm (0.18 MB/m3, 24 us), 3DGS Compressed Scaffold (1.6 MB/m3, 8.2 us), Voxblox TSDF (4.1 MB/m3, 1.8 us), Voxblox ESDF (28.5 MB/m3, 0.85 us), to Dense Occupancy Array (125 MB/m3, 0.45 us). Shaded green region highlights the real-time 1 kHz control budget below 10 us, and shaded red region indicates the deliberative zone above 100 us."}

```{python}
#| label: fig-09-memory-3d-representation-pareto
#| echo: false
#| message: false
#| warning: false
...
cat_styles = {
    ...
    "octree": {"label": "Hierarchical Octree (OctoMap)", "color": "#004B87", "marker": "s", "size": 110},
    "3dgs": {"label": "3D Gaussian Splatting (3DGS)", "color": "#A51C30", "marker": "*", "size": 190},
    "voxblox": {"label": "Hashed Voxel TSDF / ESDF (Voxblox)", "color": "#1E793C", "marker": "^", "size": 125},
    "dense": {"label": "Dense Uniform Array", "color": "#475569", "marker": "o", "size": 95},
}

for name, mem, lat, cat, ha, va, (dx, dy) in data:
    style = cat_styles[cat]
    ax.scatter(mem, lat, color=style["color"], marker=style["marker"], s=style["size"], edgecolors="#FFFFFF", linewidth=1.2, zorder=5)
    ax.annotate(
        name, (mem, lat), xytext=(dx, dy), textcoords="offset points", fontsize=8.8, fontweight="bold", color="#1E293B", ha=ha, va=va, zorder=6
    )

frontier_x = np.array([0.08, 0.18, 1.60, 4.10, 28.5, 125.0])
frontier_y = np.array([650.0, 24.0, 8.2, 1.8, 0.85, 0.45])

ax.plot(frontier_x, frontier_y, linestyle="--", color="#0284C7", linewidth=1.8, alpha=0.85, zorder=4, label="Empirical Pareto Frontier")
...
ax.set_xlabel("Volumetric Memory Footprint (MB / m³ at operational resolution)", fontsize=11.5, fontweight="bold", color="#1E293B", labelpad=10)
ax.set_ylabel("Spatial Point Distance / Collision Query Latency (microseconds, µs)", fontsize=11.5, fontweight="bold", color="#1E293B", labelpad=10)
ax.set_title("3D Scene Representation Frontier: Memory Density vs. Spatial Query Latency", fontsize=14.5, fontweight="bold", color="#1E293B", pad=24)

def add_clean_badge(ax, text, xy, xytext, color="#1E293B", fontsize=8.5, arrow_color="#94A3B8"):
...
legend_elements = [
    plt.Line2D([0], [0], linestyle="--", color="#0284C7", lw=1.8, label="Empirical Pareto Frontier"),
    plt.Line2D([0], [0], marker="D", color="w", markerfacecolor="#7C3AED", markersize=9, label="Implicit Neural Field (NeRF / NGP)"),
    plt.Line2D([0], [0], marker="s", color="w", markerfacecolor="#004B87", markersize=9, label="Hierarchical Octree (OctoMap)"),
    plt.Line2D([0], [0], marker="*", color="w", markerfacecolor="#A51C30", markersize=12, label="3D Gaussian Splatting (3DGS)"),
    plt.Line2D([0], [0], marker="^", color="w", markerfacecolor="#1E793C", markersize=9, label="Hashed Voxel TSDF / ESDF (Voxblox)"),
    plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="#475569", markersize=8, label="Dense Uniform Array"),
]

legend = ax.legend(
    handles=legend_elements, loc="upper right", frameon=True, facecolor="#FFFFFF", edgecolor="#CBD5E1", framealpha=0.96, fontsize=8.5, ncol=1
)
...
from mlsysim.fmt import fmt, fmt_force, fmt_int, check
...
class MemoryBiasDrift:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────────
    y_str = fmt_force(y_read, precision=0)
    zero_str = fmt_force(0 * newton, precision=0)
    sigma_str = fmt_force(sigma_claim, precision=2)
    rate_str = fmt(drift_rate.magnitude, precision=1)
    span_str = fmt_int(span.magnitude)
    shift_str = fmt_force(shift, precision=0)
    x_true_str = fmt_force(x_true, precision=0)
    pct_str = fmt_int(pct)
