...
: **Belief-expiry roots on the machine**: Each root solves $E(t)=E_{\max}$ for that belief's error model and task limit; the latch-slot bounds and the coworker's initial error and tolerance are illustrative, while the coworker's speed is the same walking-speed bound as the rack end, so both person rows grow linearly. The last column names the response the root would trigger. {#tbl-09-belief-decay-horizons tbl-colwidths="[20,28,14,12,26]"}
...
Before turning to the fallacies that arise when valid records are trusted too far, verify your understanding of belief expiry, observability limits, and the belief record:
