...
# │          the walking-person ceiling), @sec-memory-rate-belief-decay (worked
...
# │          ceiling as the positive root of the finished budget (C2 stop,
# │          all three overheads) plus v_h (tau_delay + T_stop).
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
class RackEndBelief:
    ...
    v_unit = 1.0 * ureg.meter / ureg.second
    stop_unit = calc_c2_stop_suffix(v_unit, a_brake)
    qa = stop_unit["distance"] / v_unit**2
    qb = tau_delay + v_h * stop_unit["duration"] / v_unit
    ...
    d_walk = (v_walk * tau_delay + stop_walk["distance"] + v_h * (tau_delay + stop_walk["duration"]) + overhead).to(ureg.meter)
    ...
    check(
        abs((d_walk - d_clear).to(ureg.millimeter).magnitude) < 1e-6, "At the walking-person ceiling the budget must fill the clear distance exactly."
    )
...
A person who keeps walking toward the machine during the whole stop is a different obstacle. A safety-distance calculation counts that approach as $v_h(\tau_{\text{delay}} + T_{\text{stop}})$, the walking speed applied over the entire time from detection to standstill. With that term added to the finished budget of @sec-enforcement-stopping-envelopes, the machine could not exceed `{python} RackEndBelief.v_walk_str` at this rack end. The site handles the approaching person with a crossing rule instead, a floor-marked crossing plus a rack-end presence sensor (both chosen), and @sec-frontier-residual-claims-register records the premise the rule rests on. Whatever the speed, the permission path must refuse any proposal whose clearance relies on remembered emptiness past the belief's deadline.
...
Consider the mobile manipulator's arm guiding a gripper finger into the latch-release slot of the spring-latched cage door. In the final approach the finger hides the slot from the wrist camera, and the finger must stay within $E_{\max} = 1.0\text{ mm}$ of the slot's centerline (@fig-uncertainty-growth). Assume illustrative bounds within the arm's validated operating region: initial position error at most $E_0 = 0.1\text{ mm}$, unobserved velocity error at most $v_{\text{drift}} = 12.0\text{ mm/s}$, and acceleration disturbance at most $a_{\text{dist}} = 50\text{ mm/s}^2$. Position error then follows the growth law of @eq-mem-occlusion-expansion.

Evaluating this growth bound with the stated parameters yields $E(\Delta t) = 0.1 + 12.0 \Delta t + 25.0 (\Delta t)^2$, with $\Delta t$ in seconds and $E(\Delta t)$ in millimeters. Setting $E(\Delta t) = E_{\max} = 1.0\text{ mm}$ gives the quadratic equation $25.0 (\Delta t)^2 + 12.0 \Delta t - 0.9 = 0$. The single positive real root gives $\Delta t = 0.066\text{ s}$ ($66\text{ ms}$), so a proposal depending on this belief must expire by $66\text{ ms}$. If the slot were machined to a clearance of $E_{\max} = 0.30\text{ mm}$, the allowable unobserved duration shrinks to $\Delta t \approx 16.1\text{ ms}$ ($25.0 (\Delta t)^2 + 12.0 \Delta t - 0.20 = 0$). Tightening mechanical tolerance by $3.3\times$ shrinks the allowable unobserved duration by $4.1\times$, requiring sensor re-acquisition or compliant impedance holding within that shorter window. In contrast, if an estimator naively assumes constant velocity ($a_{\text{dist}} = 0$), it predicts a valid horizon of $\Delta t_{\text{linear}} = 0.90 / 12.0 = 75\text{ ms}$. At $\Delta t = 75\text{ ms}$, the modeled bound reaches $E(0.075) = 1.141\text{ mm}$, exceeding the clearance by 14.1 percent. A constant-velocity-only admission rule would keep accepting the estimate for 9 ms beyond the bounded-model deadline; whether contact occurs depends on the path and geometry.

::: {#fig-uncertainty-growth fig-env="figure" fig-pos="t" fig-cap="**Belief uncertainty and invalidation horizon**: Spatial error bound $E(\\Delta t)$ of a gripper finger entering the cage door's latch-release slot, expanding quadratically under unobserved forward propagation until intersecting the slot clearance $E_{\\max} = 1.0\\text{ mm}$ at invalidation horizon $t_{\\text{exp}} = 66\\text{ ms}$. A lower lifecycle timeline marks transitions from valid through degraded to expired operational regimes. Belief has a shelf life governed by physical disturbance dynamics; sensor re-acquisition resets the evidence clock, whereas republishing cached matrices conceals physical drift." fig-alt="Belief uncertainty growth and invalidation horizon plot. Spatial error bound E(Δt) plotted against elapsed time Δt since last measurement. The quadratic curve expands from E0 = 0.1 mm to intersect task clearance E_max at invalidation horizon t_exp = 66 ms. Below the time axis, an operational regime bar indicates valid, degraded, and expired intervals."}
...
# ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────────────
E0 = 0.1  # mm, residual error at the last observation  # category: illustrative
V_DIST = 12.0  # mm/s, bounded disturbance velocity  # category: illustrative
A_DIST = 25.0  # mm/s^2, half the bounded disturbance acceleration (a_dist / 2)  # category: illustrative
E_MAX = 1.0  # mm, task clearance  # category: illustrative
...
| Initial error                      | $E_0=0.00005\text{ m}$                                                                                                                 | Check the initial bound against calibration                                              |
| Growth law                         | $E(t)=E_0+(0.001\text{ m/s})t+\tfrac12(0.005\text{ m/s}^2)t^2$                                                                         | Compare at evidence age with the task's lateral tolerance                                |
| Validity region                    | Arm acceleration at most `2.0 m/s²`; contact mode unchanged; residual drift and acceleration bounds above validated within this region | Expire immediately on a mode or residual-bound violation                                 |
| Expiry                             | $t_0+t_{\mathrm{exp}}$, the evidence epoch plus the time at which the growth law reaches the task's lateral tolerance                  | Refuse dependent proposals once expiry passes; a later estimate epoch does not extend it |
