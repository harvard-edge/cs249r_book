...
- In **Manipulators (Class 2)**, an articulated robot arm executes high-dimensional diffusion grasping policies ($20\text{--}50\text{ Hz}$) alongside high-gain $SE(3)$ kinematics and joint torque loops ($1000\text{ Hz}$). If an accelerator tensor burst evicts safety state tables from the shared last-level cache, the resulting microsecond bus jitter degrades feedback phase margins, exciting gearbox compliance, inducing motor torque ripple, or breaching ISO 10218 safety limits.
...
The mailbox rejection rules of @tbl-13-boundary-fallbacks keep a failed producer from forcing the enforcer to wait. When they leave the valve controller with no admissible proposal, the MCU uses a separately validated hold or stop response matched to sensed pressure and actuator capability; a fixed valve ramp or ten-millisecond hold is only a candidate design until tested against the plant. That fallback must still complete within a bounded time, and on shared silicon that bounded time is what the placement test measures under load.
