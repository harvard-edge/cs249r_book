...
The mitigations in the table's last column are hardware mechanisms, not scheduler policies. @Algo-hardware-crossbar-qos specifies two of them as the interconnect fabric and the Power Management Controller (PMC) execute them cycle by cycle. The crossbar arbiter routes tagged safety transactions through their own queue and channel while metering best-effort bursts with bandwidth credits, and the thermal governor halves the accelerator's clock at the trip threshold while holding the safety domain's clock at nominal.
...
Arbitration decides when a transaction moves, not what it carries. What crosses the physical boundary between the two paths must be defined by explicit guarantees on production time, validity, and failure response.
...
* **Averages cannot bound a deadline:** In the constructed FIFO case, a safety read queued behind weight-tile, camera, and telemetry transactions pushes read plus compute to `{python} PlacementBus.total_us_str` $\mu\text{s}$, past its subdeadline. Transaction size, queue policy, and bypass behavior decide the wait.
...
* **The placement record scopes what was shown:** It takes its deadline from the enforcement record and adds loaded observed maxima, mitigations, and untested conditions. On the mobile manipulator the loaded field is still empty, so the independence claim reaches release review without evidence.
:::

The permission path's independence is a property of one placement under one declared load, established by a test that could have refuted it, not a property of the block diagram. The third law, proposal is not permission, demands a permission path that learned workloads cannot delay, a requirement the earlier chapters of Part III assumed and this chapter turns into a falsifiable threshold with a stress test and a record behind it.
