...
***Heterogeneous silicon partitioning***\index{Heterogeneous silicon partitioning!definition}\index{Silicon partitioning!definition} is the architectural allocation of stochastic, high-capacity cognitive workloads (perception, learned policy evaluation) and deterministic reflex loops (safety invariant checks, admission and fallback) across specialized execution domains isolated in hardware.
...
With placement, Part III's stopping budget is complete for a machine whose fallback brakes it to rest, as the mobile manipulator's does. A walking humanoid has no rest state to brake into mid-stride, so its fallback is a recovery step, and each row of the budget has to be rechecked against that maneuver.
