...
When a real-time safety reflex reads a sensor buffer, its requests can queue behind direct memory access traffic. In the worked controller of @sec-placement-resource-contention, which is not the machine, the enforcer misses a $120\,\mu\text{s}$ sensor-read-plus-compute subdeadline (illustrative; see the Reader Guide) because the proposal workload occupies the shared memory controller. Logical privilege does nothing to prevent that miss, because it governs which addresses a task may touch, not where its requests wait in the memory queue.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import microsecond, millisecond, hertz
...
For the machine's permission loop, compare three *constructed* placements. Shared application cores give $P_{99.9}=2.8\text{ ms}$, which misses not only the `{python} PlacementMachine.deadline_str` deadline but the `{python} PlacementMachine.tick_str` tick itself. A real-time cluster on the application SoC keeps its own core but shares the die's memory controller, rails, and thermal path; @sec-placement-thermal-timing-coupling shows the clock throttle that rejects it. The discrete safety microcontroller that the machine uses pays for its separation in transport instead. Each proposal must now cross a board-level link from the application processor to the microcontroller, a hop that neither shared-die option needs. The `{python} PlacementMachine.t_bus_str` EtherCAT cycle to the drives is not that price, because every placement pays it; it is the $T_{\text{bus}}$ term that @sec-nervous-cadences already counted into $\tau_{\text{delay}}$. The choice among the three rests on each option's measured sensor-to-drive response under interference, not on how many packages it uses.

## Contention for Shared Resources {#sec-placement-resource-contention}

Every shared physical resource has multiple claimants; a utilization metric that ignores concurrent competition cannot bound a deadline. Had the mobile manipulator placed its permission path on the application SoC, physical wires would not respect the functional boundaries drawn on its software architecture diagram. The proposal path, the enforcement path, and the system infrastructure would share the same silicon die, competing directly for the same memory controllers, crossbar switches, and power distribution rails. Digital claimants on the main memory bus include camera and lidar DMA streams depositing frames into DRAM, the neural accelerator fetching model weight matrices and intermediate activation tensors, the real-time core reading its constraint tables and sensor history, actuator write buffers for drive commands, cache controllers executing dirty line writebacks, and diagnostic logging processes writing state records to nonvolatile storage.
...
In @eq-placement-dram-queuing, $\mathcal{Q}$ contains the transactions ahead of the safety read, $B_k$ is each transaction's byte count, and $\text{BW}_{\text{peak}}$ is an optimistic service-rate assumption. The equation models this FIFO scenario only, so arbitration overhead, refresh, and further arrivals add to it rather than being covered by it.
...
class PlacementBus:
    result = calc_axi_fifo_contention(
        ...
        safety_read_bytes=64 * 1024,  # category: illustrative (diagnostic sensor history)
        ...
        compute_time=Q_("95 us"),  # category: illustrative
    )
    queue_us_str = fmt(result["queue_fifo"].to("us").magnitude, precision=3)
    read_us_str = fmt((result["queue_fifo"] + result["own_read"]).to("us").magnitude, precision=3)
    total_us_str = fmt(result["total_fifo"].to("us").magnitude, precision=3)
    bypass_us_str = fmt(result["total_priority_bypass"].to("us").magnitude, precision=2)
```

Consider a separate $65{,}536$-byte diagnostic sensor-history transfer for a $1000\text{ Hz}$ controller with a $120\,\mu\text{s}$ read-plus-compute subdeadline. The per-tick $4{,}096$-byte state read used in the preceding *average-bandwidth* calculation is a different workload. In this diagnostic case, compute takes $95\,\mu\text{s}$ and an unloaded $65{,}536$-byte read takes at least $65{,}536/(12.8\times10^9)=5.120\,\mu\text{s}$ at ideal peak bandwidth, leaving $19.880\,\mu\text{s}$ of the subdeadline. Immediately before the read arrives, the controller has admitted 88 earlier transactions, and its FIFO holds 64 aligned $4{,}096$-byte tile transactions, 16 camera transactions ($65{,}536\text{ bytes}$), and eight telemetry transactions ($32{,}768\text{ bytes}$): 88 transactions or $360{,}448\text{ bytes}$ total. Their ideal transfer time is $28.160\,\mu\text{s}$; three bank switches add $0.084\,\mu\text{s}$. Thus $t_{\text{queue}}=$ `{python} PlacementBus.queue_us_str` $\mu\text{s}$, $t_{\text{sensor,loaded}}=$ `{python} PlacementBus.read_us_str` $\mu\text{s}$, and $t_{\text{exec,loaded}}=$ `{python} PlacementBus.total_us_str` $\mu\text{s}$, exceeding the $120\,\mu\text{s}$ subdeadline by $8.364\,\mu\text{s}$. If priority service bypasses every queued transaction after the single in-flight $4{,}096$-byte transaction, the same idealized model gives `{python} PlacementBus.bypass_us_str` $\mu\text{s}$ total, which is why the arbiter's actual bypass behavior is the first thing to establish. A dedicated DMA route into a coherent MCU snapshot and a $131{,}072$-byte Tightly Coupled Memory (TCM) allocation for the $65{,}536$-byte buffer plus $8{,}192$-byte state table remove this DRAM read from the critical path only if transfer completion and coherence are validated. TCM access latency is per access, not per buffer. At 800 MHz and eight bytes per cycle, 8192 accesses take at least $10.24\,\mu\text{s}$, giving $105.24\,\mu\text{s}$ total and $14.76\,\mu\text{s}$ slack; at 200 MHz the $40.96\,\mu\text{s}$ read floor exceeds the subdeadline after $95\,\mu\text{s}$ compute.
...
On a nominal $0.85\text{ V}$ core supply, a large enough droop crosses the minimum operating voltage of a co-located core, a threshold that the device specification sets. The off-chip Voltage Regulator Module (VRM) operates with a control loop bandwidth several orders of magnitude too slow to counteract a nanosecond transient, so it cannot push more current into the chip fast enough to catch the falling voltage. Therefore, first-stage charge delivery relies entirely on integrated on-die deep-trench decoupling capacitors (DTCs), which supply localized charge directly adjacent to switching logic blocks. If local capacitance is depleted before the current transient settles, supply voltage collapses.
...
To ground this physical vulnerability on silicon, consider an analytical heterogeneous SoC powering a systolic array neural accelerator and a real-time safety processor from a shared $V_{\text{core}} =$ `{python} InductivePDNVoltageDroop.v_core_str` $\text{V}$ power distribution network. The PDN package loop exhibits parasitic inductance $L =$ `{python} InductivePDNVoltageDroop.l_pkg_str` $\text{pH}$ and equivalent series resistance $R =$ `{python} InductivePDNVoltageDroop.r_pkg_str` $\text{m}\Omega$. When the neural accelerator starts a matrix multiply, its current demand surges by $\Delta I =$ `{python} InductivePDNVoltageDroop.delta_i_str` $\text{A}$ with rise time $t_r =$ `{python} InductivePDNVoltageDroop.t_rise_str` $\text{ns}$ ($dI/dt = 3.0\text{ A/ns}$). Evaluating the electrical collapse via @eq-placement-voltage-droop yields inductive drop $\Delta V_{\text{ind}} = (30 \times 10^{-12}\text{ H}) \times (3.0 \times 10^9\text{ A/s}) = 0.090\text{ V} =$ `{python} InductivePDNVoltageDroop.v_ind_mv_str` $\text{mV}$ and resistive drop $\Delta V_{\text{res}} = 6\text{ A} \times 0.008\,\Omega =$ `{python} InductivePDNVoltageDroop.v_res_mv_str` $\text{mV}$, producing a total rail droop of $\Delta V_{\text{droop}} =$ `{python} InductivePDNVoltageDroop.v_droop_mv_str` $\text{mV} =$ `{python} InductivePDNVoltageDroop.v_droop_v_str` $\text{V}$. The drooped core voltage falls from `{python} InductivePDNVoltageDroop.v_core_3dec_str` $\text{V}$ to `{python} InductivePDNVoltageDroop.v_drooped_str` $\text{V}$—an instantaneous `{python} InductivePDNVoltageDroop.droop_pct_str` percent voltage drop.

Under the stated alpha-power *gate* model ($V_{\text{th}}=0.42\text{ V}$, $\alpha=1.3$), the `{python} InductivePDNVoltageDroop.droop_pct_str` percent rail droop predicts a `{python} InductivePDNVoltageDroop.delay_expansion_pct_str` percent propagation-delay increase.[^fn-hw-alpha-power] That is a gate delay, not a thread runtime. Whether the stretch consumes a loop's slack, trips a watchdog, or delays a drive command depends on the device threshold, circuit path, and clock policy, which a rail and end-to-end timing measurement settles. A $22\text{ A}$ step in $5\text{ ns}$ with the same $L$ and $R$ would instead give $132+176=308\text{ mV}$, so the droop bound holds only for the declared current step.
...
When thousands of multiply-accumulate units in a neural accelerator turn on together, they demand a sudden surge in current. Even when the power supply is large enough overall, the package wires and bumps connecting the silicon to the supply carry small but finite resistance and inductance.

Because of this physical reality, the sudden rush of current causes the voltage supplied to the entire chip to briefly drop, a phenomenon known as **voltage droop**\index{Voltage droop}. If the voltage sags below the minimum operating level of the adjacent real-time core, the transistors inside that core cannot switch fast enough to meet their clock timings.

The derivation of inductive rail droop and of the alpha-power law for gate delay stretch is in @sec-appendix-systems-droop.
...
::: {#fig-13-soc-contention-droop fig-env="figure" fig-pos="t" fig-cap="**Heterogeneous SoC interference**: (a) In the constructed FIFO example, 88 queued $4{,}096$-byte transactions delay a $65{,}536$-byte safety read and push read plus compute to $128.364\,\mu\text{s}$ against a $120\,\mu\text{s}$ subdeadline. (b) A separate $6\text{ A}$ in $2\text{ ns}$ current step gives $138\text{ mV}$ PDN droop. The drawn $V_{\min}$ crossing is a conditional hazard, not established by that droop calculation." fig-alt="Two panels. Panel a shows a FIFO transaction queue delaying a safety read, with read plus compute overrunning its subdeadline by 8.364 microseconds. Panel b shows a current step producing a 138-millivolt rail droop, drawn against a device minimum-voltage line."}
...
The mailbox rejection rules of @tbl-04-boundary-fallbacks keep a failed producer from forcing the enforcer to wait. When they leave the permission path with no admissible proposal, the MCU executes the stop rung of @sec-enforcement-fallback-ladder, the resident stop that does not wait for the proposer. That fallback must still complete within a bounded time, and on shared silicon that bounded time is what the placement test measures under load.
...
Configure $T_{\text{lower}}$ and $T_{\text{upper}}$ around that *measured or justified* range, while keeping the late-trip response within the plant's separately derived fault budget. A one-tick execution time alone does not set a watchdog service interval.
...
Thermal management controllers protect the die from structural degradation through automated clock throttling, and this is the mechanism that rejects the shared-die placement for the mobile manipulator. Suppose its permission path ran on a real-time cluster of the application SoC instead of on the discrete microcontroller. When the internal temperature sensor registers a junction temperature crossing a hardware trip threshold $T_{\text{trip}} = 95.0\,^{\circ}\text{C}$, the clock distribution network forcibly reduces operating frequencies, and that cluster shifts from a nominal $f_{\text{nom}} =$ `{python} RejectedPlacementThrottle.f_nom_str` to a throttled $f_{\text{throt}} =$ `{python} RejectedPlacementThrottle.f_throt_str`. Let that cluster be sized to meet the permission path's declared worst-case execution time, `{python} RejectedPlacementThrottle.wcet_str`, at its nominal clock, so that each tick of reading the drive feedback and the IMU, verifying the proposal's sequence and lease, and evaluating the stopping constraints costs $N_{\text{cycles}} =$ `{python} RejectedPlacementThrottle.n_cycles_str` cycles. That is several times the `{python} PlacementMachine.unloaded_cycles_str` cycles the discrete microcontroller needs unloaded, because a cluster without tightly coupled memory pays cache-refill and coherence cycles on every snapshot and table read, the shared-DRAM exposure that @sec-placement-fallacies-pitfalls examines. Nominal execution takes $t_{\text{exec}} =$ `{python} RejectedPlacementThrottle.t_exec_str`, leaving `{python} RejectedPlacementThrottle.slack_str` before the `{python} RejectedPlacementThrottle.deadline_str` deadline. Because those cycles all run in the cluster's clock domain, execution time under the throttled clock scales inversely with frequency, expanding to $t_{\text{exec,throt}} =$ `{python} RejectedPlacementThrottle.t_exec_throt_str`. The modeled tick misses its deadline by `{python} RejectedPlacementThrottle.overrun_str`, a `{python} RejectedPlacementThrottle.overrun_pct_str` percent overrun, and it exceeds the declared bound on which every margin above the enforcer was sized. The discrete microcontroller runs on its own clock and rail, so the application processor's thermal governor cannot reach it.

Thermal throttling introduces **state-dependent hysteresis**\index{Hysteresis!state-dependent} that extends deadline violations long after the triggering load subsides. Hardware thermal managers do not restore nominal clock frequencies immediately when temperature drops below $T_{\text{trip}}$. To prevent the system from toggling rapidly between fast and slow clocks as the temperature hovers near the trip line (thermal oscillation), the controller enforces a cooler clearance threshold, such as $T_{\text{clear}} = 85.0\,^{\circ}\text{C}$. This requires the junction to cool by a full $10.0\text{ K}$ before restoring the `{python} RejectedPlacementThrottle.f_nom_str` clock. After power returns to baseline, Newton's law of cooling governs substrate temperature recovery:
...
At the machine's `{python} RejectedPlacementThrottle.tick_str` tick, the modeled `{python} RejectedPlacementThrottle.t_recov_str` cooldown spans `{python} RejectedPlacementThrottle.ticks_exposed_str` scheduled ticks exposed to the throttled clock. None of them may be missed, so a separately timed fallback or workload limit must be active once the `{python} RejectedPlacementThrottle.t_exec_throt_str` path cannot meet its `{python} RejectedPlacementThrottle.deadline_str` deadline.
...
# │       a constructed 10^7-tick trace that refutes a candidate placement.
# │ How: calc_safe_stopping_distance with a_eff = a_brake / 1.5 (the C² stop)
# │      and the aisle scenario's overheads; trace percentiles are constructed.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
class PlacementSeparationBudget:
    ...
    # Constructed trace for a candidate placement (not a measurement)
...
Consider a *constructed* $10^7$-tick stress trace for a candidate placement of the permission path. At one tick per `{python} PlacementSeparationBudget.tick_str`, the trace spans `{python} PlacementSeparationBudget.trace_s_str`, or `{python} PlacementSeparationBudget.trace_h_str` h `{python} PlacementSeparationBudget.trace_min_str` min `{python} PlacementSeparationBudget.trace_sec_str` s, before reset or setup time. The cold trace has $P_{50}=$ `{python} PlacementSeparationBudget.cold_p50_str`, $P_{99}=$ `{python} PlacementSeparationBudget.cold_p99_str`, and observed maximum `{python} PlacementSeparationBudget.cold_max_str`; the warm, loaded trace has $P_{50}=$ `{python} PlacementSeparationBudget.warm_p50_str`, $P_{99}=$ `{python} PlacementSeparationBudget.warm_p99_str`, $P_{99.9}=$ `{python} PlacementSeparationBudget.warm_p999_str`, and observed maximum `{python} PlacementSeparationBudget.warm_max_str`. The loaded $P_{99}$ already exceeds the `{python} PlacementSeparationBudget.wcet_str` declared bound, and the maximum crosses the `{python} PlacementSeparationBudget.deadline_str` deadline, so the trace refutes the candidate.

A passing trace supports much less than a failing one refutes. Millions of test cycles without a timing violation show only that observed interference stayed below the threshold in the tested states, and each percentile describes that trace, not a confidence level or a bound. A report gives the latency distribution, observed maximum, sample count, duration, and unexercised conditions, including supply degradation, electromagnetic interference, and aging.[^fn-hw-silicon-aging]
...
Apollo 11’s Executive alarms and the Mars Pathfinder priority inversion (examined in @sec-nervous-deterministic-fieldbuses) illustrate two distinct forms of real-time resource contention. In Apollo 11 a peripheral consumed execution capacity; in Pathfinder a low-priority task held a resource that a high-priority task needed. Neither was a memory-bus queue of the kind this chapter measures. Ariane 501 marks what contention testing cannot catch. Its unprotected floating-point conversion was a specification and exception-handling fault that no timing measurement would have exposed [@lions1996ariane5].

On modern safety silicon, placing high-throughput cognitive policies and hard real-time safety reflexes on a single die exposes the physical machine to the hardware analog of both contention pathologies. On the rejected shared-die placement, the dangerous combination is specific. A person steps out at a rack end, and the permission path takes its refusal branch toward the stop rung, a cold path through uncached code. At the same instant the application processor, having flagged the same anomaly, starts a diagnostic memory dump over the shared direct memory access fabric. A test suite that exercises refusal and diagnostic logging separately passes both and never measures the pair, because the two subsystems contend for one memory controller only when they run together. That is why the factorial matrix above crosses refusal paths with full logging load.
...
**Static reservation**\index{Static reservation} preserves the physical placement of workloads on the shared chip while allocating a guaranteed fraction of a shared resource to the critical path. Reservation assigns dedicated execution slots or fixed memory bus bandwidth to the nominal tracking loop. The cost of reservation is idle capacity. If the tracking loop requires $2.0\text{ ms}$ of memory arbiter access during every $10\text{ ms}$ control window to meet its deadline during a stop, that 20 percent slice of memory bandwidth remains unavailable to the background model during steady-state operation, even when the base is parked and no stop is under way.
...
- [ ] **Physical migration penalties**: Can you explain why the machine's fieldbus cycle is not a cost of relocation, trace where the proposal link's delay is charged instead, and state what moving a camera interface to the safety microcontroller would cost the learned models?
...
Every placement choice trades off communication latency and resource contention; engineering teams must avoid unwritten assumptions about boundary conditions. The **placement record**\index{Placement record!definition} consumes the enforcement record of @sec-enforcement-telemetry, from which it takes the permission path's deadline, and adds the evidence that this deadline holds on shared silicon: execution domain, privilege, period, unloaded and loaded observed maxima, arbitration settings, droop and thermal trips, mitigations, and a digest of the test ledger. An independence claim stands only while every loaded observed maximum stays below the permission path's deadline under the declared load; a sample maximum is evidence, not a WCET.
...
For the mobile manipulator, the header records the `{python} PlacementMachine.tick_str` tick, the `{python} PlacementMachine.deadline_str` deadline, and the `{python} PlacementMachine.unloaded_str` unloaded maximum; example domain IDs are MPU, NPU, GPU, MCU, and lockstep controller. Its loaded field stays empty, because no loaded test ledger exists for this machine yet, and an empty loaded field leaves the permission path's independence without evidence when @sec-release weighs it.

The external ledger inventories memory channels, cache partitions, DMA engines, power rails, and their competing claimants. For each it records the arbitration configuration and measured interference. A constructed $45\rightarrow310\text{ ns}$ LPDDR read inflation is the kind of entry that motivates testing a local TCM snapshot; the deadline itself is judged on the whole tick.
...
The mobile manipulator's fallback brakes it to rest. A walking humanoid has no rest state to brake into mid-stride, so its fallback is a recovery step, and each row of the budget has to be rechecked against that maneuver.
...
# │       own enforcer budget.
# │ Show: halving the 400 MHz clock doubles the 250 µs WCET to 500 µs, past the
# │       400 µs deadline; a 320 µs read tail leaves 80 µs, less than the 95 µs
# │       that constraint evaluation and the drive-frame write need.
# │ How: Reuses PlacementMachine (registry fields and chapter-local sub-costs);
# │      the throttle ratio and read tail are scenario inputs.
# └─────────────────────────────────────────────────────────────────────────────
...
class PlacementFallacyMargins:
    # ┌── 1. LOAD ───────────────────────────────────────────────────────────
    mcu_clock = PlacementMachine.mcu_clock  # category: illustrative (registry)
    wcet = PlacementMachine.wcet  # category: chosen (registry)
    ...
    throttle_ratio = 2.0  # scenario assumption; category: illustrative (governor halves the clock)
    read_tail = 320 * microsecond  # scenario assumption; category: illustrative (shared-DRAM read tail)

    # ┌── 2. EXECUTE ────────────────────────────────────────────────────────
    throttled_clock = mcu_clock / throttle_ratio
    throttled_wcet = (wcet * throttle_ratio).to(microsecond)
    after_read = (deadline - read_tail).to(microsecond)

    # ┌── 3. GUARD ──────────────────────────────────────────────────────────
    check(throttled_wcet > deadline, "A halved clock pushes the declared WCET past the deadline.")
    check(abs(throttled_wcet.magnitude - 500.0) < 1e-9, "250 µs doubled is 500 µs.")
    check(after_read < compute_after_sense, "The read tail leaves less than the 95 µs of constraint and write work.")

    # ┌── 4. OUTPUT ─────────────────────────────────────────────────────────
    mcu_clock_str = fmt_frequency(mcu_clock, precision=0)
    throttled_clock_str = fmt_frequency(throttled_clock, precision=0)
    wcet_str = fmt_latency(wcet, unit=microsecond, precision=0)
    throttled_wcet_str = fmt_latency(throttled_wcet, unit=microsecond, precision=0)
    deadline_str = fmt_latency(deadline, unit=microsecond, precision=0)
...
Suppose the mobile manipulator placed a neural accelerator and its lockstep permission core on one SoC. An accelerator current burst ($di/dt$) depresses their shared power rail, and the power-management circuitry halves the permission core's clock from `{python} PlacementFallacyMargins.mcu_clock_str` to `{python} PlacementFallacyMargins.throttled_clock_str` to prevent a complete brownout. For a compute-bound tick, the declared `{python} PlacementFallacyMargins.wcet_str` worst case becomes `{python} PlacementFallacyMargins.throttled_wcet_str`, past the `{python} PlacementFallacyMargins.deadline_str` deadline, despite the task's highest-priority software privileges. Access control prevents unauthorized memory writes, but it cannot preserve execution timing through the physical electrical transient. Placement qualification must test shared-rail interference and establish adequate capacitance decoupling, $di/dt$ slew limits, or physical supply separation so accelerator activity does not consume the enforcer's timing margin.
