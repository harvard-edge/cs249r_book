...
The historical evolution of physical AI architectures is fundamentally rooted in the physical limits of semiconductor scaling (@fig-placement-fifty-year-microprocessor-trend). Over fifty years of microprocessor progress (1975–2025), computing systems transitioned across two major architectural boundaries. During the classical scaling era (1975–2004), Moore's Law and Dennard scaling allowed clock frequencies to surge exponentially from $2\,\text{MHz}$ to $3.8\,\text{GHz}$ without increasing thermal power density. Autonomous robotics rode this wave directly, transitioning from offboard minicomputer tethers (Shakey) to onboard VMEbus racks (CMU Navlab 1) and embedded x86 motherboards (Stanford Stanley).
...
::: {#fig-placement-fifty-year-microprocessor-trend fig-env="figure" fig-pos="htb" fig-cap="**Fifty years of microprocessor scaling and physical AI compute (1975–2025)**: Top: Transistor count (Moore's Law), clock frequency, and peak server TDP against the mobile robot payload thermal budget ($15\text{--}60\,\text{W}$), showing the 2004 Dennard scaling breakdown at $3.8\,\text{GHz}$. Bottom: Onboard compute density at $\le 60\,\text{W}$ payload (GFLOPS and TOPS-equivalent, expanding $10^9\times$) versus closed-loop perception-to-action deliberation cadence (Hz), contrasting the $1\,\text{kHz}$ deterministic fieldbus servo baseline, the $1\text{--}3\,\text{Hz}$ autoregressive VLA deliberation wall, and $30\text{--}50\,\text{Hz}$ flow-matching action chunking. (Milestones adapted from Karl Rupp and Stanford CPU database)." fig-alt="Two-panel longitudinal plot spanning 1975 to 2025. Top panel shows exponential transistor growth alongside clock frequency saturation at 3.8 GHz in 2004 and server power rising above the 15 to 60 Watt mobile robot thermal budget. Bottom panel illustrates a nine-order-of-magnitude leap in onboard edge compute density and tracks cognitive perception-to-action cadences against a 1,000 Hertz fieldbus servo line."}

```{python}
#| label: fig-placement-fifty-year-microprocessor-trend
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ FIFTY YEARS OF MICROPROCESSOR SCALING & PHYSICAL AI COMPUTE (1975–2025)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Section 13.1, @fig-placement-fifty-year-microprocessor-trend.
# │ Goal:    Show 50-year evolution of microprocessor compute adapted for robotics:
# │          1. Dennard scaling breakdown at 2004 (frequency capped at ~3.8 GHz).
# │          2. Mobile robot thermal payload ceiling (15-60W) prohibiting server TDPs.
# │          3. Emergence of Edge NPUs expanding onboard TOPS by 10^9x.
# │          4. Autoregressive deliberation wall (1-3 Hz) vs flow matching (50 Hz).
# │ Sources: Adapted from Karl Rupp microprocessor trend data, Stanford CPU DB,
# │          and robotics autonomous milestones (Shakey, Navlab, Stanley, Orin, Thor).
# └─────────────────────────────────────────────────────────────────────────────
...
fig, (ax1, ax2) = plt.subplots(
    2, 1, figsize=(14, 11), sharex=True, dpi=200, facecolor="#FFFFFF", gridspec_kw={"height_ratios": [1.15, 1.0], "hspace": 0.14}
)
...
ax1.text(2008, 1.2e12, "MULTICORE GAP\n(Thermal wall)", ha="center", va="top", fontsize=9, fontweight="bold", color="#1E40AF", bbox=era_style)
ax1.text(2016, 1.2e12, "EMBEDDED GPU\n(Deep learning)", ha="center", va="top", fontsize=9, fontweight="bold", color="#065F46", bbox=era_style)
ax1.text(
    2023, 1.2e12, "PHYSICAL AI NPUs\n(Heterogeneous SoCs)", ha="center", va="top", fontsize=9, fontweight="bold", color="#92400E", bbox=era_style
)

years = [1975, 1978, 1982, 1985, 1989, 1993, 1997, 2000, 2004, 2006, 2008, 2010, 2012, 2015, 2018, 2020, 2022, 2024]
transistors = [5e3, 2.9e4, 1.34e5, 2.75e5, 1.2e6, 3.1e6, 7.5e6, 4.2e7, 1.25e8, 2.9e8, 7.3e8, 1.17e9, 2.6e9, 5.5e9, 1.2e10, 2.8e10, 5.4e10, 1.5e11]
freq_years = [1975, 1978, 1982, 1986, 1989, 1993, 1997, 2000, 2004, 2006, 2010, 2015, 2020, 2024]
frequency_mhz = [2, 5, 8, 16, 25, 66, 200, 1500, 3800, 2930, 3330, 4000, 4200, 4500]
tdp_years = [1975, 1982, 1989, 1993, 1997, 2000, 2004, 2008, 2012, 2016, 2020, 2024]
server_tdp = [1.5, 3, 5, 15, 35, 75, 130, 125, 150, 250, 400, 700]

ax1.plot(years, transistors, color="#004B87", linewidth=2.4, marker="o", markersize=5, label="Transistors per Die (Moore's Law)")
ax1.plot(
    freq_years,
    frequency_mhz,
    color="#A51C30",
...
ax1.set_title("Fifty Years of Microprocessor Scaling & Physical AI Compute (1975–2025)", fontsize=14, fontweight="bold", color="#0F172A", pad=14)
...
ax2.text(1975.5, 1500, "1,000 Hz Deterministic Fieldbus Motor Servo Loop (CAN / EtherCAT)", fontsize=9, fontweight="bold", color="#475569")
...
# │       fact recorded here; decided in Ch2, sized in Ch12). The 5 ms
# │       renewal-jitter allowance tolerates at most a 12.5 percent stretch of
# │       the 40 ms chunk inference (Ch8's HostContentionAllowance).
...
class PlacementMachine:
    ...
    renewal_jitter = 5 * millisecond  # category: chosen (renewal-jitter allowance of the lease rule, @sec-nervous-coupled-budget)
    ...
    contention_stretch = (renewal_jitter / chunk_inference).to("dimensionless").magnitude
    ...
    check(chunk_period + renewal_jitter < lease, "Lease rule: chunk period plus the 5 ms renewal-jitter allowance fits inside the lease.")
    check(
        abs(contention_stretch - 0.125) < 1e-9,
        "Matches Ch8 HostContentionAllowance: 5 ms of jitter is a 12.5 percent stretch of the 40 ms inference.",
    )
...
On a shared die, **crossbar bus and DRAM bank contention**\index{Crossbar bus and DRAM bank contention!definition} is queuing delay when memory and DMA transactions share an interconnect and controller, and its bound depends on transaction size, queue policy, and bank timing.[^fn-place-dram-bandwidth]
...
On a shared $V_{\text{core}} =$ `{python} InductivePDNVoltageDroop.v_core_str` $\text{V}$ supply with package inductance $L =$ `{python} InductivePDNVoltageDroop.l_pkg_str` $\text{pH}$ and resistance $R =$ `{python} InductivePDNVoltageDroop.r_pkg_str` $\text{m}\Omega$, an accelerator current step of $\Delta I =$ `{python} InductivePDNVoltageDroop.delta_i_str` $\text{A}$ in `{python} InductivePDNVoltageDroop.t_rise_str` $\text{ns}$ produces `{python} InductivePDNVoltageDroop.v_ind_mv_str` $\text{mV}$ of inductive and `{python} InductivePDNVoltageDroop.v_res_mv_str` $\text{mV}$ of resistive drop, `{python} InductivePDNVoltageDroop.v_droop_mv_str` $\text{mV}$ in all, a `{python} InductivePDNVoltageDroop.droop_pct_str` percent sag to `{python} InductivePDNVoltageDroop.v_drooped_str` $\text{V}$. The alpha-power gate model[^fn-hw-alpha-power] turns that sag into a `{python} InductivePDNVoltageDroop.delay_expansion_pct_str` percent increase in propagation delay (@sec-appendix-systems-droop derives both steps). That is a gate delay, not a tick. Whether it consumes the permission path's slack, trips a brownout reset, or does nothing depends on the device's minimum operating voltage and clock policy, and only rail and timing measurements under the declared current step can settle it. Both interference channels—interconnect FIFO queueing and power rail voltage droop—are structured in @fig-13-soc-contention-droop. A shared die leaves those questions open for every accelerator burst; the discrete microcontroller, on its own bus and rail, does not inherit them.

::: {#fig-13-soc-contention-droop fig-env="figure" fig-pos="t" fig-cap="**Heterogeneous SoC interference channels**: (a) In the memory-bus FIFO queue, queued background transactions delay a safety read and push read plus compute past its subdeadline; priority bypass changes the wait. (b) On the power distribution network, a sudden matrix-accelerator current step ($\Delta I = 6.0\text{ A}$ in $2.0\text{ ns}$) produces $138\text{ mV}$ of inductive and resistive droop, stretching gate propagation delay non-linearly under the alpha-power law. Crossing the minimum operating threshold $V_{\min}$ causes timing violations or brownout." fig-alt="Two conditional interference paths: a FIFO transaction queue causes an illustrative read-plus-compute overrun; a separate modeled current step causes 138-millivolt rail droop and gate delay expansion, with risk of crossing minimum device voltage."}
...
Software memory boundaries and lock-free seqlocks protect against torn reads, but cannot guarantee that the safety enforcer code is alive and progressing through its intended operations. Liveness of the enforcer belongs to the self-watchdog of @sec-nervous-cadences, which is fed only after a completed tick. A countdown watchdog serviced from a timer interrupt stays blind to a dead task, the failure argued in *Bookout v. Toyota*. On shared silicon, the enforcer can also be starved by interconnect contention or run too fast after clock corruption, so placement needs a stronger form of that self-watchdog, one that constrains the feed from both sides.

High-integrity physical AI architectures therefore enforce **windowed watchdog timers**\index{Windowed watchdog timer!hardware mechanics}\index{Watchdog timer!windowed}. A windowed watchdog rejects any refresh that occurs either too early or too late. The timer hardware defines a service window parameterized by a lower bound (the closed window, $T_{\text{lower}}$) and an upper bound (the open window, $T_{\text{upper}}$):
...
from mlsysim.fmt import fmt, fmt_int, fmt_latency, fmt_frequency, fmt_percent, check
...
class RejectedPlacementThrottle:
    ...
    p_load = 26.0  # W, total with continuous proposal inference; scenario assumption; category: illustrative
    ...
    t_rise_load = p_load * theta_ja  # K, steady-state rise under continuous inference
    t_steady_load = t_ambient + t_rise_load  # °C, loaded steady state
    ...
    check(abs(t_steady_load - 97.0) < 1e-9 and t_steady_load > t_start, "26 W x 2.0 K/W = 52 K over 45 °C is 97 °C, above the 95 °C trip.")
    ...
    t_ambient_str = fmt(t_ambient, precision=0)
    theta_ja_str = fmt(theta_ja, precision=0)
    tau_th_str = fmt(tau_th.to(second).magnitude, precision=0)
    p_base_str = fmt(p_base, precision=0)
    p_load_str = fmt(p_load, precision=0)
    t_target_str = fmt(t_target, precision=0)
    t_rise_load_str = fmt(t_rise_load, precision=0)
    t_steady_load_str = fmt(t_steady_load, precision=0)
    t_start_str = fmt(t_start, precision=0)
    t_clear_str = fmt(t_clear, precision=0)
```

The electronics bay in the machine's base sits at `{python} RejectedPlacementThrottle.t_ambient_str`$\,^{\circ}\text{C}$ ambient. @Fig-13-jetson-orin shows the developer kit of the module class the machine uses as its application processor; its enclosure reveals neither the heat path nor the clock domains on which the following estimate depends. Suppose the shared SoC were conduction-cooled with a **junction-to-ambient thermal resistance**\index{Thermal resistance!junction-to-ambient} of $\theta_{\text{JA}} =$ `{python} RejectedPlacementThrottle.theta_ja_str` $\text{K/W}$ and a **thermal time constant**\index{Thermal time constant} of $\tau_{\text{th}} =$ `{python} RejectedPlacementThrottle.tau_th_str` $\text{s}$. The permission loop with idle and memory power draws `{python} RejectedPlacementThrottle.p_base_str` $\text{W}$ and holds the junction at `{python} RejectedPlacementThrottle.t_target_str`$\,^{\circ}\text{C}$. Continuous proposal inference adds accelerator, memory-traffic, and regulator losses, and a total of `{python} RejectedPlacementThrottle.p_load_str` $\text{W}$ raises the steady state by $P\,\theta_{\text{JA}} =$ `{python} RejectedPlacementThrottle.t_rise_load_str` $\text{K}$ to $T_j =$ `{python} RejectedPlacementThrottle.t_steady_load_str`$\,^{\circ}\text{C}$.
...
- [ ] **Cooling recovery sizing**: Can you derive the clearance time $t_{\text{recov}}$ required for an SoC with thermal time constant $\tau_{\text{th}} =$ `{python} RejectedPlacementThrottle.tau_th_str` $\text{s}$ to cool from $T_{\text{trip}} =$ `{python} RejectedPlacementThrottle.t_start_str`$\,^{\circ}\text{C}$ to $T_{\text{clear}} =$ `{python} RejectedPlacementThrottle.t_clear_str`$\,^{\circ}\text{C}$ under a `{python} RejectedPlacementThrottle.t_target_str`$\,^{\circ}\text{C}$ baseline equilibrium?
...
::: {.column-margin .margin-pointer}
↳ **Downstream:** Adversarial microarchitectural contention tests inform the hardware fault injection suite in @sec-verification-hardware-fault-injection.
:::
...
A refuted independence claim leaves four remedies, each priced in a different currency (@tbl-13-replacement-decisions): static reservation, spatial partitioning, workload reduction, and physical relocation to separate silicon.
...
: **Workload re-placement and isolation decision matrix**: Remediation pathways for when a test falsifies independence on shared silicon. Each remedy trades distinct engineering currencies (throughput, capacity, latency, board footprint) to eliminate contention channels before escalating to physically segregated silicon. {#tbl-13-replacement-decisions tbl-colwidths="[16,16,18,18,16,16]"}
...
**Workload reduction**\index{Workload reduction} resolves contention by shrinking the computational or memory footprint of the non-critical workload until its peak demand falls within the uncontended margins of the shared system. An engineer might downsample telemetry logging from $1\text{ kHz}$ to $100\text{ Hz}$, quantize policy weights from sixteen-bit floating point to eight-bit integers to cut memory bus traffic in half, or lower the chunk policy's rate. The cost of reduction is degraded functional authority. On the mobile manipulator the lease bounds the last lever. A chunk period longer than the `{python} PlacementMachine.lease_str` lease of @sec-nervous-cadences lets proposals expire between renewals, so the policy's rate must stay above `{python} PlacementMachine.min_chunk_rate_str` even before renewal jitter is counted, which leaves little room below its `{python} PlacementMachine.chunk_rate_str`. Weight quantization can degrade the policy's accuracy near the grasp tolerance of the conveyor pick.
...
Hardware selection in physical AI cannot be decoupled from the mechanical dynamics of the embodiment. While conventional benchmark suites evaluate edge accelerators on isolated metrics such as theoretical peak TOPS or raw batch inference throughput, embodied systems operate under closed-loop energy constraints where compute latency and physical motion are tightly coupled (@fig-placement-robotics-aware-dse). Evaluating edge accelerator configurations across a continuum—from ultra-low-power edge TPUs to multi-hundred-watt workstation GPUs—reveals an inverted-U Pareto efficiency frontier.
...
::: {#fig-placement-robotics-aware-dse fig-env="figure" fig-pos="htb" fig-cap="**Robotics-aware design space exploration (DSE) across edge accelerator tiers**: Top: System energy efficiency (closed-loop tasks solved per megajoule) exhibiting an inverted-U Pareto frontier across seven edge hardware configurations. Sluggish inference on under-provisioned chips causes static chassis drain, whereas over-provisioned accelerators hit kinematic velocity saturation ($v_{\\text{max}} = 0.8\\,\\text{m/s}$) and suffer excessive dynamic thermal dissipation. Bottom: Energy breakdown per task decomposing static chassis and motor standby energy ($P_{\\text{static}} \\times T_{\\text{mission}}$) versus dynamic accelerator dissipation ($P_{\\text{chip}} \\times T_{\\text{mission}}$), identifying an optimal minimum at $4.40\\,\\text{kJ}$ per task on balanced edge SoCs." fig-alt="Two-panel visualization of robotics-aware design space exploration. The top panel shows an inverted-U curve of system energy efficiency peaking at 227.3 tasks per megajoule for Jetson AGX Orin between under-provisioned and over-provisioned zones. The bottom panel shows stacked bar charts of static chassis standby energy and dynamic accelerator energy per task, with a dashed line showing a U-shaped minimum at 4.4 kilojoules."}

```{python}
#| label: fig-placement-robotics-aware-dse
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ ROBOTICS-AWARE DESIGN SPACE EXPLORATION (DSE): ENERGY & EFFICIENCY FRONTIER
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Section 13.5, @fig-placement-robotics-aware-dse.
# │ Goal:    Demonstrate inverted-U Pareto frontier in physical AI accelerator choice:
# │          1. Under-provisioned zone: Sluggish inference leads to chassis standby drain.
# │          2. Over-provisioned zone: Kinematic velocity saturation and thermal explosion.
# │          3. Optimal co-design point: Minimum total energy per closed-loop task.
# └─────────────────────────────────────────────────────────────────────────────
import matplotlib as mpl
...
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14.0, 9.8), sharex=True, dpi=200, facecolor="#FFFFFF")
ax1.set_facecolor("#FFFFFF")
ax2.set_facecolor("#FFFFFF")
...
def add_clean_badge(ax, text, xy, xytext, color="#1E293B", fontsize=8.3, arrow_color="#94A3B8", ha="center", va="center"):
    bbox = dict(boxstyle="round,pad=0.45", facecolor="#FFFFFF", edgecolor="#CBD5E1", alpha=0.96, linewidth=0.8)
    ap = dict(arrowstyle="->", color=arrow_color, linewidth=1.1, shrinkA=3, shrinkB=4)
    ax.annotate(text, xy=xy, xytext=xytext, fontsize=fontsize, fontweight="medium", color=color, bbox=bbox, arrowprops=ap, ha=ha, va=va, zorder=10)
...
# Top Panel
ax1.plot(x_fine, eff_fine, color="#059669", linewidth=2.8, alpha=0.88, zorder=4, label="Robotics-Aware System Efficiency Frontier")
for p in platforms:
    if p["cat"] == "optimal_peak":
        ax1.scatter(p["x"], p["eff"], color="#047857", marker="x", s=280, linewidths=3.8, zorder=7, label="Pareto Optimal Operating Point (AGX Orin)")
        ax1.scatter(p["x"], p["eff"], facecolors="none", edgecolors="#047857", s=320, linewidths=1.8, linestyle="--", zorder=6)
    else:
        ax1.scatter(p["x"], p["eff"], color="#004B87", marker="o", s=70, edgecolors="#FFFFFF", linewidth=1.2, zorder=5)
...
ax1.set_ylim(0, 315)
ax1.set_ylabel("System Energy Efficiency\n(Closed-Loop Tasks Solved / MJ)", fontsize=10.5, fontweight="bold", color="#1E293B", labelpad=8)
ax1.set_title(
    "Robotics-Aware Design Space Exploration (DSE): Chip Configuration vs. System Efficiency",
    fontsize=13.0,
    fontweight="bold",
    color="#1E293B",
    pad=14,
...
ax1.text(
    1.5,
    296,
    "Under-Provisioned Zone\n(Sluggish VLA Execution -> Static Drain)",
    ha="center",
    va="top",
    fontsize=8.2,
    fontweight="bold",
...
b1 = ax2.bar(
    ...
    color="#475569",
    alpha=0.85,
    label=r"Static Chassis & Motor Standby Energy ($P_{\mathrm{static}} \times T_{\mathrm{mission}}$)",
    zorder=3,
)
b2 = ax2.bar(
    xs,
    ...
    color="#A51C30",
    alpha=0.88,
    label=r"Dynamic Accelerator Energy ($P_{\mathrm{chip}} \times T_{\mathrm{mission}}$)",
    zorder=3,
)
ax2.plot(
    xs, e_totals, linestyle="--", color="#004B87", linewidth=2.0, marker="D", markersize=6, label="Closed-Loop Total Mission Energy (kJ)", zorder=4
)

for i, p in enumerate(platforms):
    tot = p["e_total"]
    weight_txt = "bold"
    color_txt = "#047857" if p["cat"] == "optimal_peak" else "#1E293B"
    lbl_text = f"{tot:.1f} kJ\n(Min)" if p["cat"] == "optimal_peak" else f"{tot:.1f} kJ"
    ax2.text(p["x"], tot + 0.6, lbl_text, ha="center", va="bottom", fontsize=8.3, fontweight=weight_txt, color=color_txt, zorder=5)

ax2.set_ylabel("Closed-Loop Energy per Task (kJ)", fontsize=10.5, fontweight="bold", color="#1E293B", labelpad=8)
ax2.set_ylim(0, 32)
ax2.set_title(
    "Energy Breakdown: Static Chassis Standby Burden vs. Dynamic Accelerator Dissipation", fontsize=12.5, fontweight="bold", color="#1E293B", pad=14
)

add_clean_badge(
    ax2,
    "Minimum Energy: 4.40 kJ / Task\nCo-design equilibrium:\nAmortizes inference to 155 ms\nwithout wasteful GPU TDP.",
    xy=(5.0, 4.40),
    xytext=(3.7, 16.0),
    color="#047857",
    fontsize=8.2,
...
x_tick_labels = [f"{p['name']}\n{p['tops']} TOPS | {p['tdp']:.0f}W\n(T = {p['t_mission']:.1f}s)" for p in platforms]
ax2.set_xticks(xs)
ax2.set_xticklabels(x_tick_labels, fontsize=8.8, fontweight="bold", color="#1E293B")
ax2.set_xlabel(
    r"Edge Accelerator Platform & Closed-Loop Mission Duration ($T_{\mathrm{mission}}$)",
    fontsize=11.0,
    fontweight="bold",
    color="#1E293B",
    labelpad=10,
...
# │       own enforcer budget.
# │ Show: the droop worked example's accelerator step stretches every gate
# │       delay on the shared rail by about 39 percent, whatever the
# │       enforcer's privilege; a 320 µs read tail leaves 80 µs, less than
...
The chunk policy crashes halfway through publishing a chunk of `{python} PlacementMachine.chunk_horizon_str` setpoints. An enforcer that had to acquire a lock held by that producer would freeze with it, which is why the boundary contract of @sec-placement-boundary-contract admits no blocking synchronization. Its seqlock\index{Sequence lock (seqlock)} lets the enforcer reject a torn record without waiting and proceed with its fallback, but only if the retry behavior itself has a bound. The communication contract must specify timing behavior for interrupted publications as rigorously as it specifies the payload format.

:::

## Summary {#sec-placement-summary}

Physical AI platforms place learned proposers on application processors and accelerators and the permission path on a bare-metal microcontroller\index{Microcontroller!bare-metal}, but that allocation alone does not separate them, because privilege boundaries govern none of the physical resources the two paths still share. Partitioning is therefore a design hypothesis until a loaded test fails to refute it.
...
* **Coupling runs on three timescales:** Rail droop acts in nanoseconds, queueing in microseconds, and thermal throttling persists for seconds after the load stops. In the rejected shared-die placement, a halved clock stretches the enforcer's `{python} RejectedPlacementThrottle.wcet_str` cycle to `{python} RejectedPlacementThrottle.t_exec_throt_str` for `{python} RejectedPlacementThrottle.ticks_exposed_str` ticks.
