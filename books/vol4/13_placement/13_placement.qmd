...
# │          @tbl-13-placement-record-schema, @sec-placement-fallacies-pitfalls,
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import microsecond, millisecond, nanosecond, hertz
...
class PlacementMachine:
    ...
    unloaded_ns_str = fmt_latency(unloaded, unit=nanosecond, precision=0, commas=True)
    ...
    deadline_ns_str = fmt_latency(deadline, unit=nanosecond, precision=0, commas=True)
```
...
: **Heterogeneous compute allocation and execution domain characteristics**: Illustrative placement options; clock, power, jitter, and fault-containment values require a named device and measured operating conditions. {#tbl-13-compute-allocation tbl-colwidths="[16,16,18,18,16,16]"}
...
::: {.column-margin}
![](images/svg/margin_execution_determinism_ladder.svg){width="100%" fig-alt="Illustrative timing ladder; actual jitter depends on the device, workload, and measured interference."}
...
::: {#chk-placement-lockfree-boundary-contract .callout-checkpoint title="Boundary contract and watchdog liveness under co-location"}

Before implementing shared-memory communication across the proposal boundary, verify your understanding of the boundary contract and watchdog liveness:

- [ ] **Snapshot attempt bound**: Can you state what evidence bounds the enforcer's single snapshot attempt when the proposal accelerator is streaming weights over the same interconnect, rather than quoting its latency on an idle bus?
- [ ] **Windowed watchdog mechanics**: Can you explain how the open window $[T_{\text{lower}}, T_{\text{upper}}]$ catches both starved and runaway execution?
...
At the machine's `{python} RejectedPlacementThrottle.tick_str` tick, the modeled `{python} RejectedPlacementThrottle.t_recov_str` cooldown spans `{python} RejectedPlacementThrottle.ticks_exposed_str` scheduled ticks exposed to the throttled clock. It does not authorize `{python} RejectedPlacementThrottle.ticks_exposed_str` missed updates: a separately timed fallback or workload limit must be active once the `{python} RejectedPlacementThrottle.t_exec_throt_str` path cannot meet its `{python} RejectedPlacementThrottle.deadline_str` deadline.
...
Consider a *constructed* $10^7$-tick stress trace for a candidate placement of the permission path. At one tick per `{python} PlacementSeparationBudget.tick_str`, the trace spans `{python} PlacementSeparationBudget.trace_s_str`, or `{python} PlacementSeparationBudget.trace_h_str` h `{python} PlacementSeparationBudget.trace_min_str` min `{python} PlacementSeparationBudget.trace_sec_str` s, before reset or setup time. The illustrative cold trace has $P_{50}=$ `{python} PlacementSeparationBudget.cold_p50_str`, $P_{99}=$ `{python} PlacementSeparationBudget.cold_p99_str`, and observed maximum `{python} PlacementSeparationBudget.cold_max_str`; the warm, loaded trace has $P_{50}=$ `{python} PlacementSeparationBudget.warm_p50_str`, $P_{99}=$ `{python} PlacementSeparationBudget.warm_p99_str`, $P_{99.9}=$ `{python} PlacementSeparationBudget.warm_p999_str`, and observed maximum `{python} PlacementSeparationBudget.warm_max_str`. The loaded $P_{99}$ already exceeds the `{python} PlacementSeparationBudget.wcet_str` declared bound, and the maximum crosses the `{python} PlacementSeparationBudget.deadline_str` deadline, so this constructed test refutes the candidate. A percentile describes this trace, not a confidence level or an always-valid bound; a real qualification must measure the complete endpoint from lease renewal to brake onset.
...
Apollo 11’s Executive alarms and the Mars Pathfinder priority inversion (examined in @sec-nervous-deterministic-fieldbuses) illustrate two distinct forms of real-time resource contention. Apollo 11 recovered by restarting and selectively resuming priority work; the episode does not identify a modern DRAM queue or prove an actuator deadline miss. Ariane 501 marks what contention testing cannot catch. Its unprotected floating-point conversion was a specification and exception-handling fault that no timing measurement would have exposed [@lions1996ariane5].
...
**Static reservation**\index{Static reservation} preserves the physical placement of workloads on the shared chip while allocating a guaranteed fraction of a shared resource to the critical path. Reservation assigns dedicated execution slots or fixed memory bus bandwidth to the nominal tracking loop. The cost of reservation is idle capacity. If the tracking loop requires $2.0\text{ ms}$ of memory arbiter access during every $10\text{ ms}$ control window to guarantee deadline compliance during a stop, that 20 percent slice of memory bandwidth remains unavailable to the background model during steady-state operation, even when the base is parked and no stop is under way.
...
| `0x10`: `nominal_period_ns`          | `uint64_t` (8B)     | $\text{ns}$                    | $T_s \in [10^6, 5 \times 10^7]\text{ ns}$ (`{python} PlacementMachine.tick_str` enforcer tick)                                      | Free-running monotonic timer      |
| `0x18`: `hard_deadline_ns`           | `uint64_t` (8B)     | $\text{ns}$                    | `{python} PlacementMachine.deadline_ns_str`, the chosen deadline inside the tick                                                    | Monotonic timer and output probe  |
| `0x20`: `unloaded_observed_max_ns`   | `uint64_t` (8B)     | $\text{ns}$                    | Illustrative unloaded maximum `{python} PlacementMachine.unloaded_ns_str`                                                           | Timestamped test ledger           |
| `0x28`: `loaded_observed_max_ns`     | `uint64_t` (8B)     | $\text{ns}$                    | Empty until a loaded test runs; must not exceed the declared `{python} PlacementMachine.wcet_str` WCET, the falsification threshold | Timestamped test ledger           |
...
A placement record applies to its declared workload, temperature, supply, fault, and arbitration conditions. New operating conditions require additional evidence.
