...
[^fn-math-cbf-enforcer]: **Control Barrier Functions (CBFs)**\index{Control barrier function}: Under their stated dynamics and feasibility assumptions, Control Barrier Functions can establish forward invariance of a specified set $\mathcal{C}$ by evaluating the affine inequality $\dot{h}(\mathbf{x}, \mathbf{u}) + \alpha(h(\mathbf{x})) \ge 0$. Under a valid dynamics model, state estimate, input bounds, and feasible control authority, a candidate that violates this condition can be rejected or adjusted before actuation. For the formal mathematical proofs and relaxation formulations, see @sec-appendix-control.
