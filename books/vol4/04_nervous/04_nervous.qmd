...
Before trusting multi-rate communication or safety barriers, the machine must resolve where these opposing workloads execute. We begin in @sec-nervous-silicon-partitioning with the physical silicon boundary isolating the stochastic neural engine from the cycle-deterministic reflexes of the physical body.
...
The separation of compute domains reflects a constraint where no single execution frequency satisfies both the body's physical dynamics and the computational complexity of perception and inference. A physical AI nervous system operates across four distinct rate tiers (@fig-04-multirate-cadence-buffer), each determined by a different physical process:
...
This kinematic balance establishes the physical origin of the `{python} AMRWatchdogLease.t_lease_str` lease ceiling: a longer lease allows a frozen machine to impact the obstacle before deceleration engages. The trajectory payload exchanged through this lock-free mailbox incorporates this physical lease into the strict schema defined in @tbl-04-schema-action-chunk, establishing explicit data types, SI units, and structural invariants.
...
Aliasing occurs when high-frequency sensor noise folds into low-rate estimators, which the system prevents through analog anti-aliasing filters and digital decimation[^fn-anti-moving-average] within the high-rate microcontroller domain. Classical stability guarantees [@slotine1991applied] assume inner-loop execution jitter remains strictly bounded and setpoints arrive before the trajectory buffer drains. The nervous system enforces these conditions via cycle-deterministic microcontroller interrupts and buffer depth tracking. If the planner stalls and the buffer drains to its final valid waypoint, the operational assumption no longer holds. At that moment, the nervous system rejects further planner inputs and executes an autonomous, deterministic stopping trajectory computed locally at $1000\text{ Hz}$, bringing all joints safely to rest.
...
::: {#fig-industrial-ethercat-controller fig-env="figure" fig-pos="t" fig-cap="**Industrial Fieldbus Controller and Deterministic Execution Floor**: An industrial Beckhoff DIN-rail EtherCAT controller annotated across four real-time subsystems: (1) embedded PC module running the TwinCAT 3 real-time kernel on an isolated core holding cyclic jitter $J_{\text{cycle}} < 1.0\,\mu\text{s}$, (2) modular E-bus I/O and motor drive slices providing $2.5\text{ kV}$ galvanic isolation against ground loops, (3) shielded RJ-45 EtherCAT master port utilizing hardware Distributed Clocks (DC) for sub-$100\text{ ns}$ multi-axis synchronization, and (4) fieldbus watchdog module enforcing a failsafe Safe Torque Off (STO) state machine within $1.0\text{ ms}$ upon cyclic packet loss. (Credit: Wikimedia Commons, CC BY-SA 4.0)." fig-alt="Annotated photographic plate of an industrial Beckhoff EtherCAT DIN-rail controller showing four crisp rectangular callout cards: (1) Embedded PC Module with TwinCAT Kernel, (2) Modular E-Bus I/O and Drive Slices, (3) EtherCAT Master and Distributed Clocks port, and (4) Fieldbus Watchdog and Fail-Safe STO state machine."}
![](images/jpg/fig04_real_industrial_ethercat_controller_annotated.jpg){width="85%"}
:::
...
ax.set_xlabel("Payload Bandwidth (Mbps) - The amount of data transmitted over a communication channel, impacting system performance.")
ax.set_ylabel("8-Axis Control Cycle Time (ms) - The time to complete a control cycle across eight axes; lower values indicate faster control.")
ax.set_title("Fieldbus performance trade-off: bandwidth vs. cycle time - Higher bandwidth can enhance data transmission but may introduce latency.")
...
With established physical boundaries, timing contracts, and arbitration hardware, the machine no longer risks hardware destruction during experimental software execution. The real-time nervous system stands as an immutable hardware referee: it guarantees that compute delays, kernel preemptions, or unverified model outputs can never damage the mechanical plant.
...
When control software delegates safety directly to statistical neural policies, any operating system preemption or memory stall instantly degrades into uncommanded kinetic drift. Safety in physical systems is not a property of the model; it is an invariant property enforced by the bare-metal architecture that contains it.
