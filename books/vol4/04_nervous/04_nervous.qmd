...
::: {#fig-04-multirate-cadence-buffer fig-env="figure" fig-pos="t" fig-cap="**Multi-Rate Cadence and Memory Exchange**: Heterogeneous execution timeline across deliberative planning at $50\text{ Hz}$, perception at $50\text{--}100\text{ Hz}$, and deterministic reflexes at $1\text{ kHz}$ (Panel A), coupled through a single-producer seqlock triple buffer (Panel B). The tiers operate asynchronously without shared deadlines: the reflex loop latches the newest committed buffer and extrapolates via splines if a policy tick overruns." fig-alt="Two-panel diagram showing multi-rate execution timelines across planning, perception, and reflexes, alongside a lock-free single-producer triple-buffer memory exchange."}
