...
_How does a deterministic nervous system maintain absolute safety when commanding non-deterministic neural proposals across shared silicon?_
...
This chapter systematically constructs the multi-rate bridge across eight architectural stages:

We begin in @sec-nervous-4-2 by establishing silicon partitioning, physically separating high-throughput Linux application processors from cycle-deterministic real-time microcontrollers on isolated power rails and memory domains. In @sec-nervous-4-3, we design multi-rate cadences across four execution tiers, using lock-free, single-producer triple buffers and seqlocks to exchange data asynchronously without blocking. In @sec-nervous-4-4, we deploy deterministic industrial fieldbuses (EtherCAT, CAN FD, and TSN) to hold communication jitter strictly below microsecond bounds. In @sec-nervous-4-5, we operationalize actuation authority, formalizing the hardware gatekeeper that arbitrates between unprivileged neural proposals and physical motor registers.

In @sec-nervous-4-6, we harden this gatekeeper through physical hardware isolation, using dedicated reset lines, independent power domains, and physical watchdog leases. In @sec-nervous-4-7, we implement real-time invariant checking, deploying control barrier functions and kinematic filters that mathematically guarantee safety envelope defense. In @sec-nervous-4-8, we incorporate supervisory teleoperation, managing network latency and authority handovers for remote human intervention. Finally, in @sec-nervous-4-9, we synthesize these mechanisms into a comprehensive handoffs matrix that codifies the system-wide execution contracts across every software and silicon interface.
...
::: {#fig-04-multirate-cadence-buffer fig-env="figure" fig-pos="t" fig-cap="**Multi-Rate Cadence and Lock-Free Seqlock Memory Exchange**: Heterogeneous execution timeline across deliberative planning at $50\text{ Hz}$, perception at $50\text{--}100\text{ Hz}$, and deterministic reflexes at $1\text{ kHz}$ (Panel A), coupled through a single-producer seqlock triple buffer with ARMv8 store-release and load-acquire memory barriers (Panel B). The tiers operate asynchronously without shared deadlines: the reflex loop latches the newest committed buffer in $<1.5\,\mu\text{s}$ and extrapolates via splines if a policy tick overruns (synthesized from Slotine & Li [-@slotine1991applied] and ARM Architecture Reference Manual)." fig-alt="Two-panel schematic showing multi-rate execution timelines across planning, perception, and reflexes with a 1 ms servo slot breakdown, alongside a lock-free single-producer triple-buffer memory exchange with ARMv8 barriers and monotonic sequence counter."}
...
\Function{LatchLatestActionChunk}{$B_{\text{local}}, t_{\text{now}}$}
    \State $c_{\text{start}} \gets \Call{AtomicLoadExplicit}{c_{\text{seq}}, \text{memory\_order\_acquire}}$ \Comment{load initial counter (ARMv8 LDAR acquire)}
    \If{$c_{\text{start}}$ is odd}
        \State \Return $\text{STATUS\_WRITER\_ACTIVE}$ \Comment{writer active; retain prior spline}
    \EndIf
    \State $\Call{HardwareMemoryBarrier}{\text{DMB\_ISHLD}}$ \Comment{explicit barrier: prevent payload reads hoisting before counter}
    \State $\Call{CopyMemory}{B_{\text{local}}, \text{Slot}_{\text{shared}}.\text{payload}, \text{sizeof}(\text{Payload})}$ \Comment{latch to local SRAM}
    \State $\Call{HardwareMemoryBarrier}{\text{DMB\_ISHLD}}$ \Comment{LDAR cannot prevent prior reads sinking; DMB forces payload reads before $c_{\text{end}}$}
...
- Robot mass and cruise velocity: $m = 300\text{ kg}$, $v_0 = 1.5\text{ m/s}$.
- Braking capability: Mechanical brakes certified for worst-case deceleration $a_{\max} = 2.0\text{ m/s}^2$.
- Optical safety clearance: LIDAR safety field reserves $D_{\text{clear}} = 0.6939\text{ m}$ ahead of dynamic obstacles.

*2. Stopping Kinematics*:

- Once brakes engage, physical braking distance evaluates as:
  $$d_{\text{brake}} = \frac{v_0^2}{2 a_{\max}} = \frac{1.5^2}{2(2.0)} = \frac{2.25}{4.0} = 0.5625\text{ m}$$
- Allowable blind drift distance before braking must begin:
  $$\Delta d_{\text{drift}} = D_{\text{clear}} - d_{\text{brake}} = 0.6939\text{ m} - 0.5625\text{ m} = 0.1314\text{ m}$$
...
- The nervous system must detect the absence of fresh proposals and initiate emergency braking before drifting across $\Delta d_{\text{drift}}$:
  $$T_{\text{lease}} \le \frac{\Delta d_{\text{drift}}}{v_0} = \frac{0.1314\text{ m}}{1.5\text{ m/s}} = 0.0876\text{ s} = 87.6\text{ ms}$$

**Systems insight**: Watchdog lease durations are not software timeouts chosen for convenience—they are derived from the physical braking kinematics and sensor clearance of the plant. A lease longer than $87.6\text{ ms}$ permits a frozen machine to impact obstacles before mechanical deceleration can physically arrest its momentum.
:::

This kinematic balance establishes the exact physical origin of the $87.6\text{ ms}$ lease ceiling: any longer lease permits a frozen machine to impact the obstacle before deceleration can engage. The trajectory payload exchanged through this lock-free mailbox incorporates this physical lease into the strict schema defined in @tbl-04-schema-action-chunk, establishing explicit data types, SI units, and structural invariants.
...
*1. CAN-FD Serialization and Arbitration Latency (8 Axes, 64-byte payload per axis):*
...
- Transmission time per axis: $t_{\text{tx}} \approx 140.4\,\mu\text{s}$.
- For 8 axes polling cyclically: $8 \times 140.4\,\mu\text{s} = 1123.2\,\mu\text{s} = 1.123\text{ ms}$.
- Result: Bus utilization exceeds 100 percent ($\rho = 1.123 > 1.0$), causing frame drops, buffer bloat, and missed $1.0\text{ ms}$ control deadlines. Even if payload is truncated to 32 bytes ($t_{\text{tx}} \approx 78.8\,\mu\text{s}$), total bus time is $630.4\,\mu\text{s}$ (63.0 percent utilization). Any transient retransmission or asynchronous chassis frame pushes utilization above the 70 percent critical saturation threshold, causing priority inversion and high-jitter delays for lower-priority axes.

*2. EtherCAT On-The-Fly Processing (8 Axes, Daisy-Chain Ring):*
- Bitrate: $100\text{ Mbps}$ full-duplex Fast Ethernet ($10\text{ ns/bit}$).
- A single standard Ethernet frame ($64\text{ bytes}$ header/FCS + $8 \times 32\text{ bytes}$ PDO data $= 320\text{ bytes} = 2560\text{ bits}$) carries all 8 joint commands and sensor returns in one pass.
- Frame serialization time: $2560\text{ bits} \times 10\text{ ns} = 25.6\,\mu\text{s}$.
- Node forwarding delay: 8 slave application-specific integrated circuit (ASIC) controller hops at $0.6\,\mu\text{s}$ per hop $= 4.8\,\mu\text{s}$.
- Total cycle time across all 8 axes: $T_{\text{cycle}} = 25.6\,\mu\text{s} + 4.8\,\mu\text{s} = 30.4\,\mu\text{s}$.
- Result: Bus utilization is just 3.04 percent ($\rho = 0.0304$), leaving $969.6\,\mu\text{s}$ of idle margin in every $1.0\text{ ms}$ loop. Hardware Distributed Clocks (DC) synchronize sampling across all axes to within $< 50\text{ ns}$.
...
::: {#fig-04-privilege-enforcer-registers fig-env="figure" fig-pos="t" fig-cap="**Hardware Privilege Boundary**: Panel A isolates the real-time microcontroller with exclusive write privilege over motor drive registers while the policy processor proposes trajectories across a shared-memory link. Panel B collapses this boundary. In the decoupled architecture, unverified proposals must clear safety barrier checks before any actuator register can be written." fig-alt="Comparison of a hardware privilege boundary architecture where an isolated microcontroller gates actuator registers against a collapsed architecture where policies write registers directly."}
...
The duration of the watchdog heartbeat lease is not an arbitrary software tuning parameter, but a bound dictated by the physical dynamics of the uncontrolled plant. In the event of a silent brain stall, the actuators mindlessly continue applying their last commanded torque---acting much like a stuck accelerator pedal---until the enforcer detects the timeout and commands emergency braking. Enforcing this dynamic limit at the causal boundary (@sec-boundary-1-2) yields a strict upper bound on the watchdog timeout—as demonstrated by the $87.6\text{ ms}$ AMR stopping calculation in @sec-nervous-4-3.[^fn-math-watchdog-lease] Setting the lease any higher permits the physical plant to breach its dynamic safety envelope before the nervous system is permitted to intervene.
...
A manipulator evaluates a control barrier function [@ames2019control] using commanded joint positions instead of load-side encoder measurements to reduce bus traffic. When a gear fails and the link moves away from its commanded trajectory, the interpolator continues producing valid setpoints. The enforcer still calculates that all is safe ($h(\mathbf{q}_{\text{cmd}}) > 0$), even as the physical link violently enters a forbidden region. Computing the wrong input faster cannot correct it. Under the causal boundary formulation of @sec-boundary-1-2, safety enforcement must evaluate physical state measurements through a dependable sensory channel and account for their limitations; inspecting the command stream merely establishes what the planner intended, not where the heavy machinery actually is.
:::
...
[^fn-nervous-bus-contention]: **Interconnect Crossbar Contention**\index{Interconnect Crossbar Contention}: When an accelerator streams multi-gigabyte foundation model weights across shared AXI or PCIe crossbars, memory transactions saturate DRAM banks. Real-time CPU cores attempting to read motor encoder registers face unpredictable hardware wait states that software priorities cannot preempt.

[^fn-nervous-watchdog-envelope]: **Watchdog Leases and Stopping Envelopes**\index{Watchdog Leases}: A hardware watchdog lease enforces a hard temporal deadline: if the deliberative policy fails to supply a fresh, certified action chunk before the lease expires ($\tau_{\text{lease}}$), the real-time microcontroller autonomously takes custody and commands a safe dynamic stop.
