...
Once authority resides in $\text{Human-Manual}$, the machine must verify the engagement signal required by its delegated-control design. A teleoperation console might sense grip, gaze, or force, and the designer chooses among them by their measured coverage and false-loss rate. A manual lease $T_{\text{lease,man}}$ bounds loss of engagement. It is a lease in the sense of @sec-nervous-cadences, here on the operator's input, set at a chosen `{python} InterventionTimeoutBudget.manual_lease_str`. When it lapses without renewal, authority transfers to a validated, state-matched fallback. The state machine then blocks a return to $\text{Autonomous}$ or $\text{Human-Manual}$ until its specified reset conditions are met. Those conditions may require a full stop, fault review, and a new handshake.
...
One writer makes authority unambiguous at each control tick, but an unblended switch may still request an abrupt command change. If the policy's last admitted drive torque is $15.0\text{ N}\cdot\text{m}$ and the operator target is zero, directly swapping those setpoints requests a $15.0\text{ N}\cdot\text{m}$ step. The committed enforcer instead uses the latched endpoints to generate a checked transition profile with a reserved stopping margin (@sec-planning-continuous-trajectories).
...
Eliminating this step discontinuity requires smoothly mixing the old and new commands across a finite time window $\tau_{\text{blend}}$, a technique known as **bumpless transfer**\index{Bumpless transfer}. A linear blend changes slope at its endpoints. For this fixed-endpoint acceleration-command example, a **$\mathcal{C}^2$ bumpless transfer ramp**\index{C2 bumpless transfer ramp} eases in and out, removing endpoint jumps in the command and its first two derivatives. A standard curve used for this is the quintic smoothstep.
...
For the specified fixed acceleration endpoints, $\tau_{\text{blend}} \ge 1.875|\Delta \mathbf u|/j_{\text{allowable}}$ enforces the chosen command-jerk ceiling. Clearance must include this delay. Faster transfer is possible only with a different validated profile, actuator response, or limit.
:::
...
**Execution protocol ($1\text{ kHz}$ enforcer):**
...
On the mobile manipulator at its `{python} InterventionAuthBudget.v_str` aisle speed, @eq-intervention-crypto-budget allows `{python} InterventionAuthBudget.t_auth_max_str`, the finished stopping budget's `{python} InterventionAuthBudget.spare_str` spare spent as time. Assume the following illustrative verification times on a microcontroller:

- **Algorithm A (Asymmetric RSA-2048)**: Verification takes $t_{\text{auth,A}} =$ `{python} InterventionAuthBudget.t_auth_a_str`, more than the deadline. The machine travels `{python} InterventionAuthBudget.travel_a_str` while the signature is checked, `{python} InterventionAuthBudget.overrun_a_str` more than the spare, so its stop ends that far past the rack-end clear distance.
- **Algorithm B (Elliptic-Curve Ed25519)**: Verification takes $t_{\text{auth,B}} =$ `{python} InterventionAuthBudget.t_auth_b_str`, well inside the deadline. The machine travels `{python} InterventionAuthBudget.travel_b_str` during the check and keeps `{python} InterventionAuthBudget.left_b_str` of the spare.
...
When an intervention occurs while an actuator applies force, manual input and the enforcer can oppose each other. For example, a coworker hand-guiding the arm through the wrist force-torque sensor may push against a joint that the enforcer is holding at its position limit. The torque the coworker applies at the wrist and the drive current holding that joint then act against each other on the same link. If the human-device loop has insufficient phase margin, repeated corrections may oscillate at a frequency set by the gains, delay, grip, and joint mechanics. The enforcer must retain the physical limit while the interface makes the opposing force and active mode legible to the person guiding the arm.
...
The authority record states the command-contention rule, and the fault-injection campaign shows whether the arbiter follows it. Neither shows that the base and arm stay stable through the switch under real floor friction, worn actuators, and fast operator corrections. A handover that executes flawlessly at the bus protocol level can still slide the base or shift the tote rack if the machine and the human operator are pushed beyond their combined operational limits.
...
Transferring control from an active policy takes time, because replacing an admitted policy setpoint with a different human target in one $1\text{ ms}$ cycle requests a torque step. The $C^2$ bumpless authority transfer ramp\index{C² bumpless authority transfer ramp} instead varies an authority parameter $\alpha(t)$ smoothly from zero to one between latched command endpoints. Its sampled implementation must respect command-rate limits and the stopping reserve developed in @sec-planning-kinodynamic-feasibility.
...
Intervention telemetry informs incident analysis and model retraining. A logged commit marks the recorded boundary where autonomous policy authority was revoked, subject to the logger's clock and sampling limits. Hash chaining and protected keys can reveal later modification. Command and authority records support reconstruction when paired with independent sensor, current, and contact evidence and their timing uncertainty.
