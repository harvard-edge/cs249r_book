...
The enforcer's $1\text{ kHz}$ loop gathers the four-phase handshake, integrator pre-biasing, and the $\mathcal{C}^2$ quintic blend into one protocol on the permission processor.
...
When the remote assistant holds a manual lease on the arm and drives it through a force-reflecting master, the machine must decide whether it responds to the operator's motion or to the operator's force. In physical AI and haptic teleoperation [@hogan1985impedance], this distinction separates **impedance control** (responding to motion by rendering a force) from **admittance control** (responding to force by yielding in motion):
...
The remote assistant's `veto_path` packet and a forged drive-torque packet can reach the mobile manipulator over the same wireless link in the same control cycle. The permission path must keep the forger from seizing physical authority while still letting an authorized operator halt the machine before collision. These requirements conflict, so intervention operations are classified by the mechanical work they induce. Removing power or asserting a mechanical brake is physically distinct from injecting active motion commands, and the two actions do not warrant identical verification mechanisms. A protective stop may need controlled braking and load support before torque inhibition; removing motor torque alone can leave a moving plant coasting or a suspended load falling. In contrast, injecting base velocity, wheel torque, or joint velocity setpoints introduces fresh mechanical power into the system. An unauthorized or corrupted motion command can accelerate a moving mass into an obstacle, whereas a trusted out-of-band protective input requests the plant-specific minimum-risk stop.
...
The arbiter of @sec-intervention-authority-transitions exists to prevent these failures, because the control architecture\index{Control architecture} cannot rely on implicit handoffs, human attentiveness, or ad hoc torque overrides. Showing afterward that it did requires the machine to record, at every tick, who held each actuator channel and what was proposed, admitted, and delivered on it. Deciding afterward whether a stop answered a genuine boundary violation, an invalid token, or noise on a safety line requires that per-tick record to survive power cycles and scheduling stalls.
...
* **Authority becomes evidence:** Each tick records who held authority and what was proposed, admitted, and delivered, and a missing entry counts as a gap. The manifest's four authority rules become the fault cases verification injects, and their passing records, with the authority record, are the evidence for release claim C6.
:::

When the proposer is a person entitled to take over, the clearance left by the person's delay, not the person's rank, decides how long the machine may wait. A human command outranks the learned proposer, yet it reaches the actuators only through the enforcer's check, and the permission path still owns the fallback when the person is late. Proposal is not permission for the person either; entitlement changes who proposes and leaves unchanged what the permission path admits.

::: {.callout-chapter-connection title="From authority revocation to physical verification"}
If weeks of handovers pass without incident, has the machine shown that its authority rules hold? It has not, because none of those handovers exercised the duplicate writer, the replayed packet, or the unacknowledged request that the rules exist to contain. @Sec-verification turns A1–A4 into fault cases beside the rack-end faults, injects each with the loaded base approaching the rack end, and times each response with instruments independent of the machine's own log, so that a handover fallback counts from its start at the drive rather than from the arbiter's record of it.
:::
