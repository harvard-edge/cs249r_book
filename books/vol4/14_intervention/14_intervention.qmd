...
↰ **Prerequisite:** The dual-brain proposal-permission boundary establishes the baseline for supervisory preemption in @sec-boundary-bedrock-laws.
:::
...
During this reaction interval the machine keeps moving under the command it was last admitted, before the intervention signal reaches the arbiter. Substituting the human reaction floor $T_\Sigma$ into $\tau_{\text{delay}}$ in @eq-body-stopping-distance gives the distance a handover consumes.

The total distance for a human operator to detect an obstacle, actuate an override, and bring the plant to rest is dominated by neurological, muscular, and communication delays before the brakes engage. Because reaction travel grows linearly with the delay $T_{\Sigma}$ and with speed, while braking distance grows with the square of speed, human intervention becomes physically incapable of averting close-range collisions as operational speed increases.

::: {.column-margin .margin-pointer}
↰ **Prerequisite:** Actuator torque limits and back-EMF dynamics governing safe handovers originate in @sec-body-actuator-limits.
:::

To see the direct physical consequences of @eq-intervention-reaction-floor and @eq-body-stopping-distance, consider a mobile robot traveling at $v = 5.0\text{ m/s}$ ($18.0\text{ km/h}$) toward an obstacle at clear distance $D_{\text{clear}} = 10.0\text{ m}$, with chassis braking that delivers $a_{\text{brake}} = 2.5\text{ m/s}^2$ and with $\delta_{\text{loc}} = \delta_{\text{margin}} = 0$. Once mechanical braking engages, the vehicle requires $d_{\text{brake}} = \frac{v^2}{2 a_{\text{brake}}} = \frac{(5.0\text{ m/s})^2}{2 \times 2.5\text{ m/s}^2} = 5.00\text{ m}$ of deceleration runway regardless of intervention mode. However, the total stopping envelope diverges dramatically based on supervisory latency:

1. **Mode 1: Direct Bilateral Teleoperation**: Under continuous haptic teleoperation with an active proprioceptive loop, total closed-loop reaction and bus transit delay is bounded to $T_{\Sigma,1} = 150.0\text{ ms}$ ($0.15\text{ s}$). Free-running reaction drift is $d_{\text{drift},1} = v \cdot T_{\Sigma,1} = (5.0\text{ m/s})(0.15\text{ s}) = 0.75\text{ m}$. Applying @eq-body-stopping-distance with $\tau_{\text{delay}} = T_{\Sigma,1}$ yields a total stopping distance of $d_{\text{stop},1} = 0.75\text{ m} + 5.00\text{ m} = 5.75\text{ m}$, safely halting the machine with a comfortable $4.25\text{ m}$ buffer remaining.
2. **Mode 2: Passive Out-of-the-Loop Supervisory Takeover**: When an operator monitors remote telemetry passively, sensory recognition consumes $T_{\text{sensory}} = 300.0\text{ ms}$, cognitive situational assessment requires $T_{\text{cognitive}} = 1200.0\text{ ms}$, motor switch actuation takes $T_{\text{motor}} = 300.0\text{ ms}$, and network transport and handshake protocol overhead adds $T_{\text{comm}} = 200.0\text{ ms}$. By @eq-intervention-reaction-floor, the cumulative reaction latency expands to $T_{\Sigma,2} = 2000.0\text{ ms} = 2.0\text{ s}$. During this cognitive re-engagement window, the unmonitored chassis covers $d_{\text{drift},2} = (5.0\text{ m/s})(2.00\text{ s}) = 10.00\text{ m}$. Total stopping distance under @eq-body-stopping-distance expands to $d_{\text{stop},2} = 10.00\text{ m} + 5.00\text{ m} = 15.00\text{ m}$—a spatial expansion ratio of $\frac{d_{\text{stop},2}}{d_{\text{stop},1}} = \frac{15.00\text{ m}}{5.75\text{ m}} \approx 2.61$ ($2.61\times$), while reaction drift alone expands by $\frac{10.00}{0.75} \approx 13.33\times$.

In Mode 2, the vehicle breaches the $10.0\text{ m}$ clearance boundary by $5.00\text{ m}$, striking the obstacle at the full initial velocity of $v_{\text{impact}} = 5.0\text{ m/s}$ because impact occurs before mechanical braking ever begins. For an out-of-the-loop supervisor in Mode 2 to avert collision within that same $10.0\text{ m}$ clearance given the $2.0\text{ s}$ reaction time, solving the quadratic kinematic constraint $D_{\text{clear}} = v_{\max} T_{\Sigma} + \frac{v_{\max}^2}{2 a_{\text{brake}}}$ bounds the maximum allowable velocity to:
...
class InterventionBlendBudget:
    ...
    j_allowable = 10.0 * meter / second**3
    tick = 1.0 * millisecond
    # Steering-column bias removal (tbl-14-authority-timing-contract)
    steer_bias = 25.0 * newton * meter
    steer_rate = 500.0 * newton * meter / second

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    # Peak smoothstep slope 1.875 for fixed endpoints (@sec-appendix-control-splines).
    T = (1.875 * abs(a_target) / j_allowable).to(second)
    steer_blend = calc_quintic_blend_duration(steer_bias, steer_rate)
    # The enforcer blends in whole controller ticks, so the window rounds up.
    steer_blend_ticks = -int(-(steer_blend / tick).to('').magnitude // 1)
    steer_blend_budget = steer_blend_ticks * tick
    step_jerk = (abs(a_target) / tick).to(meter / second**3)
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(T.magnitude - 0.75) < 1e-9, "smoothstep blend window")
    check(abs(steer_blend.to(millisecond).magnitude - 93.75) < 1e-9, "steering blend window")
    check(abs(steer_blend_budget.to(millisecond).magnitude - 94.0) < 1e-9, "steering blend in whole ticks")
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    v0_str = fmt_velocity(v0, unit=meter / second, precision=0)
    delta_a_str = fmt_acceleration(abs(a_target), unit=meter / second**2, precision=0)
    j_allowable_str = fmt_jerk(j_allowable, unit=meter / second**3, precision=0)
    T_str = fmt_latency(T, unit=millisecond, precision=0)
    ...
    d_blend_str = fmt_length(d_blend, unit=meter, precision=2)
    d_after_str = fmt_length(d_after, unit=meter, precision=2)
    d_total_str = fmt_length(d_total, unit=meter, precision=2)
    d_ideal_str = fmt_length(d_ideal, unit=meter, precision=1)
    d_penalty_str = fmt_length(d_penalty, unit=meter, precision=2)
    steer_bias_str = fmt_torque(steer_bias, unit=newton * meter, precision=0)
    steer_rate_str = fmt_torque_rate(steer_rate, precision=0)
    steer_blend_str = fmt_latency(steer_blend, unit=millisecond, precision=2)
    steer_blend_budget_str = fmt_latency(steer_blend_budget, unit=millisecond, precision=0)
```
...
| Pending, Blend, or Manual → Fallback | Timeout, stale lease, or invalid state                                                                  | MCU executes the validated HOLD or STOP rung and records it                                                               | Re-arm only after validated recovery procedure  |
| Any state → Protective               | Independent protective input or qualified critical fault                                                | Safety circuit/controller selects plant-specific protective stop                                                          | STO alone may coast; brake/hold where validated |
...
Because human attention is volatile, a handover request initiated by the autonomous policy cannot remain open indefinitely. If a machine encounters an operational design domain boundary at speed and requests intervention, an operator who is distracted, asleep, or incapacitated will fail to complete the handshake. The state machine enforces the earlier of a configured protocol ceiling and a clearance-derived latest fallback time. For forward speed $v>0$, verified deceleration $a_{\text{eff}}>0$, clear distance $D_{\text{clear}}$, fixed overheads $\delta_{\text{loc}}+\delta_{\text{margin}}$, and the delay $\tau_{\text{delay}}$ from fallback dispatch to retarding force, @eq-body-stopping-distance gives the latest time after the request at which fallback may be dispatched:
$$T_{\text{latest}} = \frac{D_{\text{clear}}-\delta_{\text{loc}}-\delta_{\text{margin}}-v^2/(2a_{\text{eff}})}{v}-\tau_{\text{delay}},\qquad T_{\text{timeout}}=\max\!\left(0,\min(T_{\text{config}},T_{\text{latest}})\right).$$
...
The enforcer blends with the quintic smoothstep, whose coefficients follow from requiring zero first and second derivatives at both ends of the window (@sec-appendix-control-splines). It is the same $C^2$ property the planner relies on at its seam, where no acceleration step appears at the switch (@sec-planning-behavior-seam); here the switch is between proposers rather than between trajectory segments. The appendix also derives the shortest window that keeps a fixed change in commanded acceleration within a chosen command-jerk limit $j_{\text{allowable}}$. The same bound applies to a torque command, so removing a `{python} InterventionBlendBudget.steer_bias_str` steering bias under a `{python} InterventionBlendBudget.steer_rate_str` rate ceiling takes at least `{python} InterventionBlendBudget.steer_blend_str`. The bound holds only for fixed endpoints and a validated command limit, and it does not by itself prove physical traction or stopping safety.

This latency-jerk trade-off applies to the stated acceleration-command profile. Increasing $\tau_{\text{blend}}$ reduces the profile's peak commanded jerk, but delays the full human target and can lengthen the stopping trajectory. Consider an autonomous vehicle traveling at $v =$ `{python} InterventionBlendBudget.v0_str` when an operator takes over. The human asserts full emergency braking while the autonomous policy was commanding steady cruising, a fixed acceleration step of $|\Delta a| =$ `{python} InterventionBlendBudget.delta_a_str`. If chassis dynamics and tire traction limit commanded jerk to $j_{\text{allowable}} =$ `{python} InterventionBlendBudget.j_allowable_str`, the smoothstep bound sets a minimum blending window of `{python} InterventionBlendBudget.T_str`. The smoothstep weight averages one half over the window, so the vehicle leaves the blend at `{python} InterventionBlendBudget.v1_str` after traveling `{python} InterventionBlendBudget.d_blend_str`; braking from there at the full target takes another `{python} InterventionBlendBudget.d_after_str`, for `{python} InterventionBlendBudget.d_total_str` to rest.
...
An instantaneous step takeover would stop in `{python} InterventionBlendBudget.d_ideal_str`, so bumpless blending imposes a spatial penalty of `{python} InterventionBlendBudget.d_penalty_str`.[^fn-math-trajectory-blend] However, attempting to bypass this blend window by applying a step change in acceleration over a single $1.0\text{ ms}$ control cycle would request a command jerk of about `{python} InterventionBlendBudget.step_jerk_str` over that tick. Whether the actuator can follow it, traction is lost, or stopping distance increases depends on the measured brake and tire dynamics; the idealized step is only a comparison baseline.

[^fn-math-trajectory-blend]: **Bumpless Transfer Braking Penalty**\index{Bumpless transfer braking penalty}: This difference belongs to the specified acceleration profile and its fixed endpoints. It is not a universal cubic jerk formula. A real vehicle also needs measured tire, brake, and actuator response to compare this idealized blend against a fast brake request.
...
Motion commands introduce active kinetic hazards; thus, the verification mechanism must meet both latency and freshness bounds from the kinematics of the maneuver. If an obstacle is detected at clear distance $D_{\text{clear}}$ while the platform travels at speed $v$, and the brakes apply deceleration $a_{\text{brake}}$ after actuator onset delay $T_{\text{act}}$ and bus delay $T_{\text{bus}}$, both of which enter $\tau_{\text{delay}}$ in @eq-body-stopping-distance, the maximum permissible latency for cryptographic signature verification and replay checking is bounded by:
$$T_{\text{auth,max}} = \frac{D_{\text{clear}} - \frac{v^2}{2 a_{\text{brake}}}}{v} - T_{\text{act}} - T_{\text{bus}}$$ {#eq-intervention-crypto-budget}

To see the physical consequences of @eq-intervention-crypto-budget, consider an autonomous transport robot ($m = 250\text{ kg}$) traveling at $v = 2.0\text{ m/s}$ toward an obstruction detected at clear distance $D_{\text{clear}} = 1.20\text{ m}$. When an emergency override command is broadcast, mechanical brakes apply constant deceleration $a_{\text{brake}} = 4.0\text{ m/s}^2$ once actuated, requiring mechanical braking distance $d_{\text{brake}} = \frac{v^2}{2 a_{\text{brake}}} = \frac{(2.0\text{ m/s})^2}{2 \times 4.0\text{ m/s}^2} = 0.50\text{ m}$. The displacement available before braking must commence is $d_{\text{avail}} = D_{\text{clear}} - d_{\text{brake}} = 1.20\text{ m} - 0.50\text{ m} = 0.70\text{ m}$. At $v = 2.0\text{ m/s}$, total elapsed time before braking must engage is $T_{\text{max}} = \frac{0.70\text{ m}}{2.0\text{ m/s}} = 350.0\text{ ms}$. If mechanical actuator lag and hydraulic buildup consume $T_{\text{act}} = 160.0\text{ ms}$, and CAN-FD bus serialization consumes $T_{\text{bus}} = 40.0\text{ ms}$, @eq-intervention-crypto-budget yields a cryptographic verification deadline of:
...
- **Algorithm A (Asymmetric RSA-2048)**: Assumed verification takes $t_{\text{auth,A}} = 180.0\text{ ms}$, exceeding the $150.0\text{ ms}$ budget. Total delay before braking commences expands to $T_{\text{total,A}} = 180.0 + 160.0 + 40.0 = 380.0\text{ ms}$ ($0.380\text{ s}$). The robot traverses $d_{\text{travel}} = (2.0\text{ m/s})(0.380\text{ s}) = 0.76\text{ m}$ before brakes actuate. The remaining runway to the obstacle is only $d_{\text{rem}} = 1.20\text{ m} - 0.76\text{ m} = 0.44\text{ m} < 0.50\text{ m}$. The machine collides with the obstacle at impact velocity:
  $$v_{\text{impact}} = \sqrt{v^2 - 2 a_{\text{brake}} d_{\text{rem}}} = \sqrt{(2.0)^2 - 2(4.0)(0.44)} = \sqrt{4.0 - 3.52} = \sqrt{0.48} \approx 0.693\text{ m/s}\quad (2.5\text{ km/h})$$
...
Uncoordinated handovers also corrupt the data drawn from fleet logs. If intervention segments enter behavioral cloning\index{Behavioral cloning} or diffusion training without intervention masking\index{Intervention masking}, the policy learns emergency recovery as nominal behavior, and validation loss on trajectory chunks does not reveal the corruption. @Sec-data-intervention-recovery develops the remedy, corrective aggregation that labels the learner's own states; it depends on an intervention record that keeps the trace from before the takeover, separated from the recovery by the logged commit instant.

The Tempe collision, which @sec-boundary-when-fails analyzes, shows vigilance decay meeting a disabled stop path\index{Tempe fatal collision!operator monitoring}. The developmental ADS recognized the need for emergency braking 1.2 seconds before impact and then suppressed it for one second, the factory forward-collision warning and automatic emergency braking were disabled during testing, and the safety operator did not intervene in time [@ntsb2019uber]. The failure asks whether an independent stop path and an operator-monitoring contract would have been effective under the measured remaining clearance. The Cruise incident of @sec-enforcement-fallback-ladder, in which a vehicle that had stopped after a pedestrian was thrown into its path began a pull-over and dragged the pedestrian [@quinnemanuel2024cruise], raises the related question of whether contact uncertainty should revoke motion authority until a validated hold is established.
...
|:-------------------------|-------------:|:---------------------------------------------------------------------------------------------------------|
| `t_mcu_ns`               |            8 | Event time on the permission processor's clock; the PTP mapping and its error bound are in the manifest. |
| `actuator_channel_mask`  |            2 | Channels to which this decision applies; bus/DMA permissions enforce write scope.                        |
| `authority_state`        |            1 | `AUTO`, `PEND`, `BLEND`, `MAN`, `FALLBACK` (with its ladder rung), or `PROTECTIVE`.                      |
...
This static manifest and event schema form test inputs and evidence for verification. The manifest's `authority_claims` field hands @sec-verification-deriving-fault-list four authority rules by identifier, each of which becomes a hardware-in-the-loop fault case:

- **A1.** One enforcer path writes each actuator channel, even when a second bus master attempts a write.
- **A2.** No source bypasses an active higher-tier source or the independent permission check.
- **A3.** An unacknowledged handover falls back by $T_{\text{timeout}}$, the earlier of the configured ceiling and the clearance-derived deadline.
- **A4.** Invalid authentication locks out the affected network channel and leaves authority unchanged.

A duplicate channel writer, a frozen operator input, dropped acknowledgments, and replayed override packets test the response to those injected conditions. Broader physical protection still requires measured actuator, timing, and plant evidence under the stated operating envelope.
