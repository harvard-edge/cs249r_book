...
During this reaction interval the machine keeps moving under the command it was last admitted, before the intervention signal reaches the arbiter, so adding the human reaction floor $T_\Sigma$ to $\tau_{\text{delay}}$ in @eq-body-stopping-distance gives the distance a handover consumes. Because reaction travel grows linearly with the delay $T_{\Sigma}$ and with speed, while braking distance grows with the square of speed, human intervention becomes physically incapable of averting close-range collisions as operational speed increases.
...
On the mobile manipulator at its `{python} InterventionAuthBudget.v_str` aisle speed, that bound is `{python} InterventionAuthBudget.t_auth_max_str`. Assume the following illustrative verification times on a microcontroller:
...
When assembling the safety case for operational certification, @sec-release evaluates the authority record as the evidence for claim C6, the claim that authority transfers as these four rules specify. A safety assessment examines the stated authority constraints, implementation evidence, fault coverage, and tested operating envelope. The certification package pairs the static authority ledger with the plant measurements that @sec-intervention-bumpless-transfer requires of every transfer and with execution traces from the fault-injection campaign that show any gaps in the audit trail. The measured preemption latency in those traces is a percentile of the tested distribution rather than a bound (@sec-nervous-deterministic-fieldbuses). The plant measurements are needed because a handover that is correct at the bus can still slide the base or shift the tote rack.
