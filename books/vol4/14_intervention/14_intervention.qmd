...
# │          (veto_path for the remote assistant, accept_item for the coworker).
# │
# │ Goal:    Show that the chosen handover speed sits under the force-limited
# │          ceiling for contact with the coworker's hand.
# │ Show:    Chosen TCP handover speed, the ceiling F_h / sqrt(k_h m_eff), the
# │          arm's effective contact mass, the contact criterion.
...
from mlsysim.fmt import fmt_velocity, fmt_qty, check
...
class InterventionCoworkerHandover:
    ...
    f_criterion_str = fmt_qty(f_criterion, newton, precision=0)
```

For example, the remote assistant who supervises the mobile manipulator may hold a signed **trajectory veto** capability for the base's planner channel: role `remote_assistant`, operation `veto_path`, channel `planner_proposal`, priority above the learned planner but below the local enforcer, authenticated session and nonce, and a clearance-derived acknowledgment deadline. The enforcer responds to a valid veto with a checked local hold or stop. A drive-torque packet from that same credential is denied because its capability grants no actuator channel. The coworker at the packing station holds a different entry, `accept_item`, which lets the coworker take the mug from the gripper. That entry grants no motion authority either; the enforcer caps the arm's approach to the coworker's hand at a chosen `{python} InterventionCoworkerHandover.v_handover_str`, under the `{python} InterventionCoworkerHandover.v_ceiling_str` at which an unplanned contact of the arm's `{python} InterventionCoworkerHandover.m_eff_str` effective mass with the hand would reach the `{python} InterventionCoworkerHandover.f_criterion_str` contact criterion. The permission record binds actor, operation, channel, deadline, authentication, rank, and fallback; a valid signature alone grants no motion authority.

When an operator attempts an intervention, human biology imposes a hard real-time latency budget before any physical corrective action can occur. Human response time is not a single point event, but a cascaded pipeline of physiological and mechanical stages. In one illustrative, additive budget for an operator monitoring a machine's telemetry stream, the sensory recognition stage requires $150\text{ ms}$ to $300\text{ ms}$ for light to strike the retina, travel through the visual pathway, and register as an optical anomaly. The situational assessment stage, in which the operator recognizes that the learned policy is drifting toward a hazard and selects an appropriate counter-action, demands $500\text{ ms}$ to $1500\text{ ms}$. Finally, the motor actuation stage, covering the transmission of motor signals down the spinal cord, muscle recruitment in the hand, and the physical displacement of an override switch, consumes $200\text{ ms}$ to $400\text{ ms}$. Summing these sequential budgets establishes the physiological **takeover reaction floor**\index{Takeover reaction floor}. Accounting for network transport and communication handshake latency $T_{\text{comm}}$, total supervisory takeover latency $T_{\Sigma}$ decomposes into:
$$T_{\Sigma} = T_{\text{sensory}} + T_{\text{cognitive}} + T_{\text{motor}} + T_{\text{comm}}$$ {#eq-intervention-reaction-floor}

During this reaction interval the machine keeps moving under the command it was last admitted, before the intervention signal reaches the arbiter. Adding the human reaction floor $T_\Sigma$ to $\tau_{\text{delay}}$ in @eq-body-stopping-distance gives the distance a handover consumes.
...
1. **Machine reaction travel**: The distance covered during the machine's own pre-brake delay ($v\,\tau_{\text{delay}}$), from @tbl-enforcement-stopping-budget.
...
Handover is an allocation of physical distance, not digital clock cycles. If the spatial travel during the takeover delay $v\,T_{\Sigma}$ plus the machine's own stopping distance exceeds the clear distance, no software handshake or arbitration algorithm can avert collision. The handover deadline $T_{\text{timeout}}$ must be dynamically bounded by physical stopping distance, not fixed in configuration files. Both terms grow with speed, the braking term faster, so a takeover that fits at a crawl overruns the rack end at aisle speed.
:::
...
# │          constructed joint-torque change under a torque-rate ceiling.
...
class InterventionBlendBudget:
    """Fixed-endpoint smoothstep blends on the machine's base and arm, not a plant guarantee."""
    ...
    # Scenario assumptions (this chapter only): a constructed arm-joint command change
...
2. In `AUTO`, a manual request or policy handover request enters `PENDING`; the policy may propose only while its existing permission remains valid. Set $T_{\text{timeout}}$ to the smaller of the protocol ceiling and clearance-derived deadline. No later than the last permission tick before $T_{\text{timeout}}$, transfer to the MCU-owned fallback. An invalid network packet is rejected and logged, not interpreted as a stop request.
...
# │          deadline, the two constructed algorithms, and the freshness window.
# │
...
class InterventionAuthBudget:
    ...
    # Scenario assumptions (this chapter only): constructed verifier timings
    t_auth_a = 180 * millisecond  # category: illustrative (asymmetric RSA-2048)
    t_auth_b = 15 * millisecond  # category: illustrative (elliptic-curve Ed25519)
...
Motion commands introduce active kinetic hazards; thus, the verification mechanism must meet both latency and freshness bounds from the kinematics of the maneuver. Verifying the signature and checking the replay window of a network motion or override command is one more delay before braking can begin. It adds to the actuator onset $T_{\text{act}}$, the bus delay $T_{\text{bus}}$, and the other terms of $\tau_{\text{delay}}$ (@eq-body-stopping-distance), so by @eq-enforce-stopping-envelope, for a static obstacle at clear distance $D_{\text{clear}}$ and forward speed $v$ the maximum permissible verification latency is the spare margin spent as time:
$$T_{\text{auth,max}} = \frac{D_{\text{clear}}-\delta_{\text{loc}}-\delta_{\text{margin}}-\epsilon_{\text{track}}-v^2/(2a_{\text{eff}})}{v}-\tau_{\text{delay}} = T_{\text{latest}}$$ {#eq-intervention-crypto-budget}

On the mobile manipulator at its `{python} InterventionAuthBudget.v_str` aisle speed, @eq-intervention-crypto-budget allows `{python} InterventionAuthBudget.t_auth_max_str`, the finished stopping budget's `{python} InterventionAuthBudget.spare_str` spare spent as time. For a **constructed** microcontroller timing comparison, assume the following verification times; they are not measured or portable benchmarks:

- **Algorithm A (Asymmetric RSA-2048)**: Assumed verification takes $t_{\text{auth,A}} =$ `{python} InterventionAuthBudget.t_auth_a_str`, more than the deadline. The machine travels `{python} InterventionAuthBudget.travel_a_str` while the signature is checked, `{python} InterventionAuthBudget.overrun_a_str` more than the spare, so its stop ends that far past the rack-end clear distance.
- **Algorithm B (Elliptic-Curve Ed25519)**: Assumed verification takes $t_{\text{auth,B}} =$ `{python} InterventionAuthBudget.t_auth_b_str`, well inside the deadline. The machine travels `{python} InterventionAuthBudget.travel_b_str` during the check and keeps `{python} InterventionAuthBudget.left_b_str` of the spare.
...
Mapping these entries to the mobile manipulator requires bounding four timing parameters against its limits: the **operator reaction budget**\index{Operator reaction budget} $T_{\text{react}}$, the **handshake protocol latency**\index{Handshake protocol latency} $T_{\text{handshake}}$, the **manual lease**\index{Manual lease} $T_{\text{lease,man}}$, and the minimum **blending window**\index{Blending window} $\tau_{\text{blend}}$. @Tbl-14-authority-timing-contract states them for the machine's two command channels, the base and the arm, which one remote-assistant session commands together.

| Machine channel           | Command change and chosen rate ceiling                                                                                                                                                                                | Smoothstep window $\tau_{\text{blend}}\ge1.875\lvert\Delta u\rvert/r$, whole ticks | Assumed human / protocol time                                                                         | Fallback contract                                                                                                                                          |
|:--------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------:|:------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------|
...
: **Illustrative blending windows on the mobile manipulator**: One remote-assistant session commands both channels, and each window is rounded up to whole `{python} InterventionBlendBudget.tick_str` permission ticks. The rate ceiling $r$ bounds the derivative of each *command*, not measured base jerk or joint torque without tracking evidence. The base row is sized at the out-of-loop crawl, where its blend fits the stopping reserve; at the in-loop takeover speed no blended base command fits, and the base receives the resident stop. The arm's command change and rate ceiling are constructed, and the human and protocol times are scenario assumptions. Every lease and pending deadline must fit the state-dependent stopping reserve. {#tbl-14-authority-timing-contract tbl-colwidths="[18,26,15,18,23]"}
...
Consider the remote assistant's in-loop takeover of the mobile manipulator, requested after the machine has slowed to `{python} InterventionAssistantSession.v_in_str`. The operator reaction budget $T_{\text{react}}$ sets the maximum duration the nervous system permits between issuing a takeover demand and detecting the assistant's first command; here it is the `{python} InterventionAssistantSession.t_react_str` in-loop takeover of @sec-intervention-human-authority, which already includes transport. The handshake latency $T_{\text{handshake}}$ defines the round-trip deadline for mutual authentication and state acknowledgment over the bus, budgeted at `{python} InterventionAssistantSession.t_handshake_str` per the four-phase protocol in @tbl-14-handshake-protocol; it is part of $T_{\text{react}}$, not an addition to it. The manual lease $T_{\text{lease,man}}$ of @sec-intervention-authority-transitions sets the longest silence tolerated from the assistant before the nervous system declares a link failure and reverts to the fallback trajectory, here `{python} InterventionAssistantSession.manual_lease_str`. Finally, the blending window $\tau_{\text{blend}}$ governs the rate of authority transfer to prevent mechanical shock. The arm's constructed joint-torque change in @tbl-14-authority-timing-contract takes `{python} InterventionBlendBudget.arm_budget_str` in whole `{python} InterventionBlendBudget.tick_str` ticks. The base's window depends on its speed, because the resident stop's peak jerk, which caps the blend, rises as speed falls. At the `{python} InterventionBlendBudget.v_crawl_str` crawl a step to braking takes at least `{python} InterventionBlendBudget.T_crawl_str` under the smoothstep bound of @sec-intervention-bumpless-transfer, which the enforcer allots as `{python} InterventionBlendBudget.T_crawl_budget_str`.

These timing parameters determine how much of the clearance remains. At `{python} InterventionAssistantSession.v_in_str` the finished stopping budget leaves `{python} InterventionAssistantSession.spare_in_str` of the rack-end clear distance. The assistant's `{python} InterventionAssistantSession.t_react_str` takeover spends `{python} InterventionAssistantSession.travel_react_str` of it, which leaves `{python} InterventionAssistantSession.remainder_str`, or `{python} InterventionAssistantSession.remainder_time_str` of further delay at this speed. That remainder bounds the transfer itself. Any further delay before a stop could begin while authority is changing hands must fit inside the remainder, and the enforcer checks the resident stop against it at every tick of the transfer. No base blend passes that check at this speed. Blending the step to braking from `{python} InterventionAssistantSession.v_in_str` grows the travel plus the resident stop from the blended state by up to `{python} InterventionBlendBudget.excess_in_str`, far past the remainder, so in this session the base receives the resident stop and only the arm blends. After a `{python} InterventionTimeoutBudget.t_ool_str` out-of-loop takeover at the crawl, `{python} InterventionAssistantSession.remainder_crawl_str` remains, and the crawl's blend uses at most `{python} InterventionBlendBudget.excess_crawl_str` of it. Once the assistant holds `MAN`, the takeover delay is spent and the enforcer re-evaluates the budget each tick with the manual lease in the place of the `{python} InterventionAssistantSession.chunk_lease_str` chunk lease in $\tau_{\text{delay}}$. The `{python} InterventionAssistantSession.manual_lease_str` lease adds `{python} InterventionAssistantSession.lease_travel_str` of travel at this speed, which the `{python} InterventionAssistantSession.spare_in_str` spare covers.

This static manifest and event schema form test inputs and evidence for verification. The manifest's `authority_claims` field hands @sec-verification-deriving-fault-list four authority rules by identifier, each of which becomes a hardware-in-the-loop fault case on the machine:

- **A1.** One enforcer path writes each actuator channel, even when a second bus master writes a base-drive channel while the policy and the remote assistant both command.
...
- **A3.** An unacknowledged handover falls back by $T_{\text{timeout}}$ of @sec-intervention-authority-transitions, the earlier of the configured ceiling and the clearance-derived deadline, `{python} InterventionAssistantSession.t_latest_str` at the aisle speed.
- **A4.** A replayed `veto_path` or `accept_item` packet, or one with a bad authentication tag, locks out the affected network channel, leaves authority unchanged, and records the event.
...
# │ Show:    Spare margin and T_latest at the aisle speed; the chosen
# │          slow-down and crawl speeds; the out-of-loop ceiling.
