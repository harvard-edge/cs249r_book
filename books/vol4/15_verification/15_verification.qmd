...
An incident-free operating log answers only the conditions it encountered. Three quiet weeks in a warehouse do not exercise a frozen camera frame, a brownout, or a bus occupied by DMA traffic. Whole-system verification therefore turns each safety claim into a deliberate fault and an observable physical outcome. The sampling cost of waiting for a rare event also grows as the event becomes rarer. At a constant event rate, a tenfold rarer event takes ten times as many miles to observe on average (@fig-15-av-disengagement-trends). Injected faults can expose the untested mechanism directly, with a test oracle and limits declared before the run.
...
A mobile manipulator that runs its aisle for three shifts without incident has shown that its nominal loop worked under the conditions it met. To test its stopping claim, the team must also stall its proposer, age its observations, drop its fieldbus frames, and sag its control rail at the rack end, and measure whether brake onset follows the last valid renewal within `{python} RackEndVerification.onset_bound_str` and the base stops within `{python} RackEndVerification.budget_str`. Each test links a design claim, injection, detector, response, and measured plant outcome. Passing one condition narrows uncertainty; it does not prove every combination of faults and aging states safe.
