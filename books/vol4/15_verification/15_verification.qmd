...
3. **Deterministic In-Situ Fault Injection Trigger (Stage 3):**
   ...
   - *Control-Rail Droop (F4a):* Programmable electronic load pulls the shared control rail from `{python} RackEndVerification.rail_low_str` down to `{python} RackEndVerification.rail_min_str` for $\Delta t_{\text{duration}} = 5.0\text{ ms}$ while an independent digitizer records the permission rail and the permission path's heartbeat.
   - *Permission-Rail Feed Loss (F4b):* Solid-state switch opens the permission rail's feed as the resident stop from the inspected-floor ceiling begins.
