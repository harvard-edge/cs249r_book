...
: **Representative hardware and cyber-physical fault classes**: Physical and logical failure modes mapped to injection points, detector, response, and observable pass criteria; the table does not establish exhaustive coverage for a specific machine. {#tbl-15-fault-classification tbl-colwidths="[16,18,18,16,16,16]"}
...
[^fn-inc-apollo-1202]: **Apollo 11 AGC Executive Priority**\index{Apollo 11!AGC 1202 alarm}: The [NASA mission report](https://www.nasa.gov/wp-content/uploads/static/apollo50th/pdf/A11_MissionReport.pdf) attributes excess computer load to rendezvous-radar-related counter interrupts, consuming up to about 15 percent of AGC time. The overloaded Executive issued 1201/1202 restart alarms; the [NASA engineer account](https://history.nasa.gov/wp-content/uploads/static/history/alsj/a11/a11.1201-pa.html) describes selective resumption of essential steering and display jobs. These sources do not establish a missed throttle deadline or a simple memory-bus theft mechanism.

[^fn-inc-toyota-ua]: **Toyota watchdog hypothesis**\index{Unintended Acceleration!watchdog refresh}: In *Bookout v. Toyota*, plaintiff expert Michael Barr proposed that task death could leave an unwanted throttle command while a timer-tick interrupt continued servicing the watchdog. This is an expert failure hypothesis, not an established cause of all unintended-acceleration events; the [NHTSA/NASA investigation](https://www.nhtsa.gov/sites/nhtsa.dot.gov/files/nhtsa-ua_report.pdf) did not find an electronic cause of unintended acceleration. The test-design lesson is to inject loss of each critical task and observe the physical response rather than infer coverage from a running CPU.
...
**Context**: The Toyota ETCS-i litigation raised questions about whether a throttle-control task could fail while a timer-tick interrupt continued to service the watchdog [@barr2013toyota].

**Mechanism to test**: Plaintiff expert Michael Barr proposed a task-death path associated with memory corruption. The test vector kills the control task while leaving the timer interrupt active, then independently measures throttle and watchdog outputs. The [NHTSA/NASA investigation](https://www.nhtsa.gov/sites/nhtsa.dot.gov/files/nhtsa-ua_report.pdf) did not establish an electronic cause of unintended acceleration, so this is an attributed hypothesis rather than a universal incident explanation.

**Oracle**: A running watchdog is insufficient evidence if the safety claim requires timely task-specific control. Record whether the throttle returns to a validated state and whether the brake response meets the vehicle's tested limit. The result depends on the implemented monitor and plant; initial brake application can overcome full engine torque in the NHTSA tests.
...
The **fault record**\index{Fault record!definition} links a small fixed header to a retained evidence bundle by digest, following @sec-nervous-handoffs-matrix. Together they identify the claim, operating point, injection, hardware and software builds, raw trace, detector, response timing, physical outcome, and anomalies. For flow-regulator claim C-04, a record could specify a $24\text{ V}$ valve-driver override for $120\text{ ms}$ at $350\text{ kPa}$ and $45^\circ\text{C}$, with firmware 3.4.1 on board B. Its bundle would name the pressure-gradient detector, measured $1.8\text{ ms}$ response, peak pressure below $380\text{ kPa}$, and observed thermal, breaker, and bus status. @Tbl-15-verification-manifest-schema defines the fixed portion; the variable trace stays in the bundle.

The verdicts must remain distinct. A measured containment violation is `FAIL`. A breaker that prevents the requested injection makes the trial `INVALID`; lost truth frames make it `UNOBSERVABLE`, with the partial trace retained. Neither may be counted as a pass or silently dropped from coverage.
...
| `measured_t_det`        |            4 | Microseconds from fault trigger to detector assertion.                                                                                                                           |
| `measured_t_takeover`   |            4 | Microseconds from fault trigger to measured gate takeover; subtract detection time for the post-detection interval.                                                              |
...
When $n$ independent Bernoulli trials sampled from the declared fault distribution have zero missed mitigations, the result sets a one-sided upper confidence limit on the miss probability for that distribution.[^fn-math-clopper-exact] To place that upper limit at most $p_\star=1-C_\star$ with confidence $1-\alpha$, the exact count and useful small-$p_\star$ approximation are:
$$N_{\text{trials}}=\left\lceil\frac{\ln\alpha}{\ln(1-p_\star)}\right\rceil\approx\frac{-\ln\alpha}{p_\star}. $$ {#eq-verification-zero-failure-bound}
Applying @eq-verification-zero-failure-bound at 95 percent confidence, $p_\star=0.01$ requires 299 zero-miss trials; $p_\star=0.001$ requires 2,995. These are limits on the sampled fault population, not proof that every fault mode is covered. To infer a FIT-weighted hardware coverage, injections must represent the raw-fault weights $\lambda_i$ or use a justified stratified weighting and per-stratum analysis.[^fn-math-zero-failure]
...
[^fn-math-zero-failure]: **Trial sampling and aging**\index{Zero-failure testing!independence}: The binomial count assumes independent, identically distributed Bernoulli outcomes under the declared injection distribution; it does not require Poisson event arrivals. Repeated heating, wear, or contact changes can break independence and require a state-stratified test design. A separate Poisson or constant-rate model is used below only to illustrate zero-event operating-hour exposure.
...
::: {#nbk-verification-hardware-fault-injection-coverage-mean-time .callout-notebook title="Fault coverage and exposure under declared assumptions"}
...
For a fault population sampled in proportion to its $\lambda_i$ weights, zero missed mitigations in 299 independent injections puts the one-sided 95 percent miss-probability limit at 1 percent; 2,995 are needed for 0.1 percent. Uniform injections across unequal-rate components estimate a different distribution unless outcomes are weighted and each stratum has adequate evidence. The counting result does not itself verify fault rates, common-cause failures, detector latency, or physical containment. A test must record which mode was injected, whether it was detected, and whether the plant actually reached its allowed state.
:::

For operating exposure, make a separate constant-rate assumption: if incidents occur as a Poisson process with rate $\lambda$, zero observed events over $T$ independent, representative operating hours gives the 95 percent upper limit $\lambda_U=-\ln(0.05)/T$. To put that limit at a selected $10^{-9}\text{ h}^{-1}$ target requires about $3.0\times10^9$ event-free aggregate hours. With 100 units operating continuously, that is roughly 3,420 years before maintenance or distribution shifts are considered. This is impractical for the illustrated program, not a mathematical impossibility. As @butler1993infeasibility explain, such exposure demands call for an argument built from more than black-box operating hours. DO-178C is software development assurance, while aircraft catastrophic-condition probability objectives are separate system safety targets; ISO 26262 applies to road-vehicle electrical and electronic hazards.

::: {.content-visible when-format="pdf"}
::: {.column-margin}
![](images/svg/margin_poisson_exposure_wall_scale_anchor_knee.svg){width="100%" fig-alt="Conceptual rising curve from a green low-exposure point to a red high-exposure point; the adjacent table gives the quantitative constant-rate zero-event exposure calculation."}

*The zero-event exposure requirement depends on the selected rate model and test population.*
:::
:::

| Illustrative target rate | 95% zero-event hours | Duration for 100 continuous units | Interpretation                                                     |
|:-------------------------|---------------------:|----------------------------------:|:-------------------------------------------------------------------|
| $10^{-3}\text{ h}^{-1}$  |      $3.0\times10^3$ |                     $30\text{ h}$ | Representative exposure may be practical for this selected target. |
| $10^{-6}\text{ h}^{-1}$  |      $3.0\times10^6$ |                $3.4\text{ years}$ | Distribution and aging assumptions become consequential.           |
| $10^{-8}\text{ h}^{-1}$  |      $3.0\times10^8$ |                $342\text{ years}$ | Exposure alone is impractical in the assumed fleet.                |
| $10^{-9}\text{ h}^{-1}$  |      $3.0\times10^9$ |              $3,420\text{ years}$ | Other analysis and targeted evidence must carry the argument.      |

: **Illustrative zero-event exposure under a constant-rate model**: $T=-\ln(0.05)/\lambda$ gives a one-sided 95 percent event-rate upper limit after zero observed events in representative independent exposure. Rows are chosen hazard-rate targets, not SIL, ASIL, or DAL assignments. Fleet durations assume 100 units operate continuously and share the same target population; they do not account for aging, maintenance, or correlated failures. {#tbl-15-statistical-testing-bounds tbl-colwidths="[24,20,23,33]"}

The exposure targets in @tbl-15-statistical-testing-bounds are assumptions, not assurance levels. A defensible safety case requires that untested operational envelopes, unobservable test results, and unsupported probability claims be presented with the exact same visual and structural prominence as successful verifications. Hiding an untested envelope boundary or an unverified hardware permutation in an appendix manufactures a false impression of completeness. The fault record must explicitly list every parameter combination where fault injection was omitted due to test bench limitations, sensor bandwidth constraints, or destructive hardware limits. Documenting what was never tested prevents unverified operational zones from silently masquerading as verified safety margins when the record is transferred to the safety case in @sec-release. Furthermore, every isolated entry in the record documents the response of the machine to a single, localized disturbance. In physical operation, however, disturbances rarely occur in strict isolation, and verifying how a machine responds to an individual fault leaves open the question of how it behaves when multiple coupled physical disturbances (e.g., stator delays\index{Stator delay}, gearbox compliance\index{Gearbox compliance}, and bus jitter\index{Bus jitter}) strike the system at the same time.
...
Before using empirical test campaigns to claim rare-event reliability, verify your understanding of fault manifests, statistical power, and exposure requirements:
...
- [ ] **Zero-miss confidence limit**: Why do 299 representative independent injections with no misses give a 95 percent upper miss-probability limit of 1 percent for that injection population, without proving universal diagnostic coverage?
- [ ] **Exposure model**: Under what constant-rate and representative-exposure assumptions does a $10^{-9}\text{ h}^{-1}$ target need about $3.0\times10^9$ zero-event hours for a 95 percent upper limit?
:::
