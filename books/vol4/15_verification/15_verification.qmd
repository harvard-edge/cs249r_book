...
class RackEndVerification:
    ...
    g = (1 * ureg.standard_gravity).to(meter / second**2)  # physical constant
...
::: {#chk-verification-rack-end-oracle .callout-checkpoint title="The rack-end oracle"}
Consider the warehouse mobile manipulator at the rack end, and the process plant beside it:

- [ ] **Onset bound**: Can you assemble the brake-onset bound from the lease, the permission-loop tick, the bus cycle, and brake onset, and explain why a stalled proposer cannot lengthen it?
- [ ] **Distance bound**: Can you state the stopping distance the oracle allows at the aisle speed, and convert the reserve left inside the clear distance into time?
- [ ] **Volume versus distance**: Can you explain why a delay that exhausts the machine's reserve barely moves the tank, and how that difference changes each plant's fault list?
:::
...
The one-sigma scenario in @eq-verification-clock-drift is not a peak or hard bound: a separate justified jitter envelope and clock-conversion test are needed for a deterministic deadline. A fifth missed period gives $100\,\mu\text{s}$ of drift alone, before jitter. If the base's two drive axes must apply a stop within $100\,\mu\text{s}$ of each other, the timebase could consume most of that command-timing budget in this scenario. Whether command skew becomes actuation skew, asymmetric braking, or yaw requires measured drive response, track width, braking forces, and the base's yaw inertia. Clock phase error by itself does not establish a heading error.
...
| `envelope_point`        |           16 | Four declared `float32` values for the mobile manipulator: speed (m/s), payload (kg), floor friction coefficient, ambient temperature (K); another machine needs a versioned field definition. |
