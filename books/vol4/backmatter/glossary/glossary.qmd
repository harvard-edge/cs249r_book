...
**Causal Boundary**
: The physical, energetic, and architectural threshold in an embodied system where reversible digital computations terminate and irreversible physical work is performed on the external environment ($dW = \mathbf{F} \cdot d\mathbf{x}$). Beyond this boundary, computational errors cannot be rolled back by software exception handlers or abort routines, requiring that safety invariants be verified before actuator register writes occur. It defines the formal placement of safety enforcement filters between policy proposal engines and physical drive electronics. (@Sec-boundary, @sec-body)
...
: A deterministic, prioritized escalation hierarchy of defensive interventions executed by the real-time safety supervisor when nominal policy execution degrades, solver infeasibility occurs, or hardware tripwires fire. It stages responses across ordered degradation levels—from minimal-intervention optimization, to active Cartesian position hold, to controlled dynamic deceleration, to mechanical brake engagement or drive torque inhibit. Each rung corresponds to a validated safe state that relaxes task performance to defend physical survivability. (@Sec-enforcement, @sec-intervention)

**Dynamic Stopping Distance ($d_{\text{stop}}$)**
: The certified physical clearance required to bring a moving embodied system to a complete standstill from its current velocity under worst-case plant, sensing, and execution delays: $d_{\text{stop}} = v \tau_{\text{latency}} + \frac{v^2}{2 a_{\text{floor}}} + \epsilon_{\text{margin}}$. It comprises the unguided reaction displacement traversed during end-to-end sensing-to-actuation latency, the kinematic braking distance governed by physical deceleration limits, and an additive clearance margin for localization error. Any candidate motion chunk that violates this clearance breaches forward invariance and admits an unavoidable collision. (@Sec-boundary, @sec-body, @sec-enforcement)

## E {#sec-glossary-vol4-e .unlisted}

**Embodied Latency Scaling Law**
: A physical scaling principle for embodied foundation models stating that expanding neural policy parameter count yields diminishing safety and task returns unless offset by proportional reductions in end-to-end inference latency. Because physical stopping clearance scales linearly with computational delay ($d_{\text{react}} \propto v \cdot t_{\text{inference}}$), a larger but slower model enlarges the machine's required braking envelope, counteracting the benefits of increased cognitive capacity. High-speed physical interaction thus imposes a hard latency ceiling that constrains model scale in embodied deployments. (@Sec-frontier)
...
: The strictly bounded set of environmental, geographic, dynamic, and operational conditions under which an autonomous cyber-physical system is certified to function safely. Operating outside this domain invalidates design premises, model calibration, and safety case warrants, legally and architecturally mandating that the machine refuse tracking authority or initiate a fallback transition. It defines the external boundary conditions within which the system's safety enforcement mechanisms are guaranteed to hold. (@Sec-release)
...
**Proposal–Permission Architecture**
: An architectural safety pattern for embodied systems that strictly decouples unverified, high-capacity neural deliberation from deterministic physical actuation by routing all candidate commands through an independent, verified safety supervisor. High-level learned policies propose candidate action chunks, while a cycle-deterministic execution engine running on an isolated hardware substrate validates physical invariants before granting actuation permission. If a proposed action threatens physical safety boundaries, the permission mechanism modifies or vetoes the command before it reaches the plant. (@Sec-boundary, @sec-brain, @sec-enforcement)
...
: An asymmetric cyber-physical computing paradigm that separates asynchronous, high-latency cognitive deliberation ($1\text{--}50\,\text{Hz}$) from synchronous, hard real-time safety enforcement ($100\text{--}1000\,\text{Hz}$) across isolated execution substrates. The slow cognitive path processes multimodal foundation models and semantic intent on high-throughput processors, while the fast reactive path continuously tracks kinematics, enforces invariant barrier certificates, and executes emergency stopping maneuvers on certified microcontrollers. This temporal decoupling guarantees continuous dynamic stability even when high-level neural inference experiences multi-millisecond tail jitter. (@Sec-boundary, @sec-brain, @sec-placement)
