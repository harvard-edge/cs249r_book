...
When a proposal conflicts with physical constraints, the enforcer chooses between filtering and veto. Filtering applies when the proposal is structurally sound but marginally exceeds a kinematic or dynamic boundary, as when an end-effector velocity would drive one arm joint slightly past its rated speed. The enforcer then projects the command onto the boundary of the safe set, the least-change correction that @sec-enforcement-minimal-intervention computes as a quadratic program\index{Quadratic programming} constrained by a Control Barrier Function\index{Control barrier function}.

Filtering must respect momentum as well as position, because the actuators must still be able to stop the machine before clearance vanishes (@sec-enforcement-safe-sets-conditional-permission). Projection is permissible only when the deviation is local, continuous, and well clear of a hazard. A structurally invalid proposal, such as a discontinuous jump in commanded position, an acceleration beyond the available torque, or a trajectory aimed into an immovable obstacle, indicates that perception or inference has entered an unmodeled failure mode, and projecting it onto the nearest safe boundary would execute a motion unrelated to any task intent. The enforcer vetoes such a proposal, and the output selector drops authority to a local fallback.
...
# │          and price a proposer stall that the drive does not cut off.
# │ Show:    chunk rate, lease, heartbeat rate and timeout, self-watchdog,
# │          tick, bus cycle, brake onset, renewal-to-onset delay; the base
# │          stall at the aisle speed (distance added, overrun past D_clear).
# │ How:     the same RM/AISLE fields that size the lease in @sec-nervous-cadences;
# │          calc_safe_stopping_distance with the C2 suffix's a_eff for the stall.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.physics.robotics import calc_c2_stop_suffix, calc_safe_stopping_distance
from mlsysim.fmt import fmt, fmt_latency, fmt_length, fmt_velocity, check
...
class EnforcementLeaseStop:
    ...
    c2_stop_aisle = calc_c2_stop_suffix(v_aisle, a_brake)
    d_const_decel_aisle = (v_aisle**2 / (2 * a_brake)).to(meter)
    c2_distance_factor = (c2_stop_aisle["distance"] / d_const_decel_aisle).to("dimensionless").magnitude
    a_eff = a_brake / c2_distance_factor
    budget_aisle = calc_safe_stopping_distance(v_aisle, tau_delay, a_eff, AISLE.delta_loc + AISLE.eps_track, AISLE.delta_margin).to(millimeter)
    stall_extra_delay = (delta_t_stall_base - lease).to(millisecond)
    budget_stalled = calc_safe_stopping_distance(
        v_aisle, tau_delay + stall_extra_delay, a_eff, AISLE.delta_loc + AISLE.eps_track, AISLE.delta_margin
    ).to(millimeter)
    stall_added = (budget_stalled - budget_aisle).to(millimeter)
    stall_breach = (budget_stalled - d_clear).to(millimeter)
    ...
    check(abs(budget_aisle.magnitude - 997.43) < 5e-3, "Finished budget at 1.3 m/s is 997.43 mm.")
...
# │          the walking-person ceiling (moved from @sec-memory-belief-through-occlusion).
...
Later chapters spend the remainder or restrict the premise beneath it. At the aisle speed the unspent distance lasts `{python} EnforcementStoppingBudget.t_latest_str` (@sec-intervention-authority-transitions); one missed permission tick costs `{python} EnforcementStoppingBudget.missed_tick_str` against a declared WCET of `{python} EnforcementStoppingBudget.wcet_str` inside a `{python} EnforcementStoppingBudget.deadline_str` deadline (@sec-placement-testing-separation); and the credible deceleration binds only on floors with friction of at least `{python} EnforcementStoppingBudget.mu_min_str`, since $a=\min(a_{\text{brake}},\mu g)$, so on a floor inspected to `{python} EnforcementStoppingBudget.mu_inspected_str` the ceiling falls to `{python} EnforcementStoppingBudget.ceiling_inspected_str` (@sec-frontier-residual-claims-register).
...
The fallback must be selected before the admissible set becomes empty. Once it is empty, escalation can contain a fault or limit harm but cannot promise to avoid a boundary crossing. Nor can the response be a single switch. Cutting bus power whenever a proposal brushes a barrier margin produces uncoordinated deceleration and continuous mission aborts, while relying on software optimization during an inverter shoot-through (@sec-body-actuator-limits) burns the gate drivers before an interrupt handler can run. The permission path therefore escalates deterministically, from feasible projection to plant-specific controlled stopping or torque removal.
...
3.  **Common pitfall**: Assuming higher ladder rungs (such as Safe Torque Off, STO) universally provide faster or safer arrest. Each rung has its own entry conditions, and the terminal rung removes torque without arresting anything by itself.
...
Every rung of the fallback ladder assumes that the hardware executing it stays independent of the fault that triggered the escalation. A secondary processor beside the primary compute module looks independent, yet a shared regulator, crystal, or ground plane\index{Hardware Architecture!common-mode coupling} can reset, halt, or corrupt both chips in the same instant, as when a host-side short drags the rail below the microcontroller's threshold or motor-driver return currents bounce the ground\index{Hardware Architecture!ground bounce} beneath its buses. Software on either chip cannot see these faults, so the permission path runs on its own held-up rail (@sec-enforcement-fallback-ladder), and @sec-nervous-hardware-isolation separates its supply, clock, and ground.
...
Contention on the shared system-on-chip (SoC) interconnect can defeat an enforcer that meets its own deadline, because a camera direct memory access (DMA) burst or bulk flash write that holds the bus matrix after the solve adds directly to the command's age at the drive while the inverter holds its last setpoint. Whether the permission path survives such bursts under declared load is measured in @sec-placement-resource-contention.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: the record list, the routing paragraph, and the door-approach
...
Filled in for the machine at the rack end and the cage door, the entries that fix the record's stops and validity regions mostly repeat values derived earlier in the chapter. Only the arm's validity region and the base's restricted configuration are set here for the first time.
...
No enforcement guarantee holds when the machine operates outside its designed physical environment, so the record bounds the plant dynamics within an explicit validity region. The arm's region is the target ODD of @sec-evaluation-logs, which admits only the nominal plate, the door's own load, a stopped base, and the `{python} EnforcementRecordMachine.v_latch_str` guarded approach. The chunk policy's proposal to approach the strike plate at `{python} EnforcementRecordMachine.v_latch_high_str` falls outside it and does not pass as proposed. The approach-speed channel of the evaluation record's `runtime_monitor_specs` enters the check as a `{python} EnforcementRecordMachine.v_latch_str` ceiling inside the door zone, so the enforcer projects the proposal to `{python} EnforcementRecordMachine.v_latch_str` and logs the channel that bound it. The arithmetic alone would have admitted the faster approach, whose `{python} EnforcementRecordMachine.t_yield_high_str` contact deadline still exceeds the `{python} EnforcementRecordMachine.t_contact_str` contact loop, but neither the demonstrations nor the evaluation trials behind the policy include that speed (@sec-data-schema), so the projection rests on evidence rather than on the contact model.
