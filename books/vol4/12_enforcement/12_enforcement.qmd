...
When a proposal conflicts with physical constraints, the enforcer chooses between filtering and veto. Filtering applies when the proposal is structurally sound and a nearby admissible command still serves its intent. The chunk policy's proposal to approach the strike plate of the cage door at `{python} EnforcementLatchFilter.v_latch_high_str` is well formed, for instance, but the door zone admits only the `{python} EnforcementLatchFilter.v_latch_str` guarded approach of the target ODD (@sec-evaluation-logs), the speed its demonstrations and trials cover. The enforcer therefore keeps the approach's direction and target and projects only its speed onto the boundary of the safe set at `{python} EnforcementLatchFilter.v_latch_str`. That least-change correction is the one @sec-enforcement-minimal-intervention computes as a quadratic program\index{Quadratic programming} constrained by a Control Barrier Function\index{Control barrier function}.
...
# │          tick, bus cycle, brake onset, renewal-to-onset delay. The base
# │          stall is priced in calc-enforcement-authority-loss, after the budget.
...
Downstream of the feedback controller, near the power electronics, a command checker inspects phase currents, duty cycles, and joint torque references immediately before they are written to hardware registers. Running at the machine's $1\text{ kHz}$ inner-loop rate, it prevents overcurrent, over-torque, and thermal runaway by clipping or zeroing signals beyond the plant's ratings, but it surrenders foresight. A torque well inside its continuous rating looks safe to a scalar clamp even while it accelerates a descending arm joint toward the fixture faster than the remaining travel can absorb, and the clamp registers no fault until no allowable reverse torque can prevent the collision.
...
# │          ceiling (moved from @sec-memory-belief-through-occlusion).
...
# │          delay and a proposer stall on the base each overrun the spare.
# │ Show:    arm stop with and without bias; unbraked windup travel vs spare;
# │          the base stall at the aisle speed (distance added, overrun past
# │          D_clear), moved here from the lease paragraph.
...
class EnforcementAuthorityLoss:
    ...
    d_clear = AISLE.d_clear  # category: illustrative
    lease = AISLE.t_lease  # category: chosen
    delta_t_stall_base = 200.0 * millisecond  # category: illustrative (proposer stall before any brake)
    ...
    stall_added = (v_aisle * (delta_t_stall_base - lease)).to(millimeter)
    stall_breach = (budget + stall_added - d_clear).to(millimeter)
    ...
    spare_str = fmt_length(spare, unit=millimeter, precision=1)
    t_stall_str = fmt_latency(delta_t_stall_base, unit=millisecond, precision=0)
...
A stalled proposer spends the same spare from upstream of the drive. Were the chunk lease of @sec-enforcement-authority-refuse left to the host to honor, a last proposal that kept its authority for `{python} EnforcementAuthorityLoss.t_stall_str` would add `{python} EnforcementAuthorityLoss.stall_added_str` to the stop and carry the base `{python} EnforcementAuthorityLoss.stall_breach_str` past the clear distance at the rack end. Enforcing the lease at the drive, rather than trusting the host to withdraw its own proposal, is what bounds that delay inside the budget.
...
The MCU evaluates this barrier on sampled cycles, so the admission margin must cover intersample travel and measured sensing-to-torque delay. A barrier on the arm's distance to the fixture alone would register the danger only once momentum had already made contact inevitable,[^fn-math-cbf-degree] so the barrier must carry the kinetic stopping angle ($\omega^2 / 2\alpha$ for approach rate $\omega$ and braking bound $\alpha$) as well as position, a requirement that follows from **relative degree**\index{Relative degree}. Actuators command current and torque, torque produces acceleration, and position therefore lies two integrals from the input:
...
Carry the descending-arm scenario of @sec-enforcement-exhausted-authority into this barrier: $J=$ `{python} EnforcementAuthorityLoss.inertia_str`, $\tau_{\text{bias}}=$ `{python} EnforcementAuthorityLoss.bias_str` toward the station, $|\tau|\le$ `{python} EnforcementAuthorityLoss.torque_limit_str`, $\alpha=(\tau_{\max}-\tau_{\text{bias}})/J=$ `{python} KineticBarrierArm.alpha_str` $\text{rad/s}^2$, $\ell=$ `{python} EnforcementAuthorityLoss.link_length_str`, $\omega=$ `{python} EnforcementAuthorityLoss.joint_speed_str` $\text{rad/s}$, and $\gamma=$ `{python} KineticBarrierArm.gamma_str` $\text{s}^{-1}$. These are stated joint-level scenario inputs; the bias is not inferred from the payload mass alone. At `{python} KineticBarrierArm.clear_far_str` of tool clearance, $D=$ `{python} KineticBarrierArm.angle_far_str` $\text{rad}$ and $h=$ `{python} KineticBarrierArm.h_far_str` $\text{rad}$, so @eq-enforce-kinetic-torque requires the joint to brake with at least `{python} KineticBarrierArm.brake_far_str`. At `{python} KineticBarrierArm.clear_zero_str`, $h=0$ and the requirement reaches `{python} KineticBarrierArm.brake_zero_str`, the joint's full peak torque. At `{python} KineticBarrierArm.clear_near_str`, it demands `{python} KineticBarrierArm.brake_near_str`, more than the joint can produce, so this state is already outside the modeled viable set. The enforcer must prevent entry into it; a fallback started there cannot restore the lost clearance.
...
Filled in for the machine, the record fixes the base's stops at the rack end and the arm's validity region at the door and the packing station:
...
- The validity region for the arm covers the door and the packing station. At the door it is the target ODD of @sec-evaluation-logs, which admits only the nominal plate, the door's own load, a stopped base, and the guarded approach, and the approach-speed channel of the evaluation record's `runtime_monitor_specs` enters the check as a `{python} EnforcementRecordMachine.v_latch_str` ceiling on the tool point inside the door zone. At the packing station it admits the mug pick from the conveyor under the `{python} EnforcementRecordMachine.tcp_limit_str` free-space TCP limit and a live intent lease (@sec-intent-lease), and the handover that follows. While the coworker holds `accept_item` (@sec-intervention-human-authority), a handover channel caps the tool point's approach to the coworker's hand at `{python} EnforcementRecordMachine.v_handover_str`, a ceiling the enforcer applies as it applies the door's.
