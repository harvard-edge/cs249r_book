...
1. **Expectation versus Pointwise Guarantees:** Constrained RL enforces constraints *in expectation* over the stationary policy distribution ($\mathbb{E}[C] \le d$). An expectation allows the robot to collide with an obstacle in 1 percent of trials, provided it behaves with extreme conservatism in the remaining 99 percent to keep the statistical average below $d$. An expected cost limit does not bound the consequence of an individual high-momentum collision. The permission path therefore checks each candidate at its sampled actuation boundary.
2. **The Reward Shaping Dilemma and the Freezing Robot Trap:** Balancing task reward against constraint penalties is brittle. If the Lagrange multiplier $\lambda$ is initialized too low, exploratory policy gradients prioritize task completion, causing destructive physical breaches. Increasing $\lambda$ may reduce sampled violations but can also suppress useful motion or make optimization unstable near a constraint boundary. Training penalties alone provide no per-tick containment guarantee.
3. **Destructive Exploratory Rollouts:** In reinforcement learning, the policy learns constraint boundaries by experiencing violations and backpropagating the resulting penalties. In a simulated environment, a crash is resolved by an instantaneous software reset. On physical hardware, exploratory collisions destroy transmissions, strip gearboxes, and trigger hardware E-stops, rendering trial-and-error constraint discovery physically infeasible.
...
When the policy's proposal is safe, the shield is completely transparent ($\mathbf{u}^* = \mathbf{u}_{\text{nom}}$), leaving nominal task execution unhindered. When a candidate violates the current input half-space, the shield projects it onto that constraint, and the operating conditions of @sec-enforcement-safe-sets-conditional-permission decide whether the projected command keeps the machine viable.
...
A barrier result holds only while the permission path meets its deadline on real silicon. In @sec-placement, we turn from mathematical safety filtering to hardware isolation and heterogeneous SoC placement, partitioning the learned proposers and the permission path across compute fabrics and testing whether memory starvation, clock throttling, or bus contention can delay permission or fallback.
:::
