...
Filtering and veto both happen at runtime, and training the policy cannot replace either. As @sec-boundary-bedrock-laws argues, penalty terms and barrier losses shape the distribution a policy learns but do not bound what the deployed policy emits, and constrained reinforcement learning\index{Constrained reinforcement learning} bounds only the expected cost of violations, which permits a rare high-momentum collision provided the average stays below its limit. The permission path therefore checks each candidate at its actuation boundary whatever objective trained the proposer, and when the proposal is safe, the check is transparent and the learned competence passes to the drive unchanged.
...
Downstream of the feedback controller, near the power electronics, a command checker inspects phase currents, duty cycles, and joint torque references immediately before they are written to hardware registers. Running at a plant-derived inner-loop rate, illustrated here by $1000\text{ Hz}$, it prevents overcurrent, over-torque, and thermal runaway by clipping or zeroing signals beyond the plant's ratings, but it surrenders foresight. A torque well inside its continuous rating looks safe to a scalar clamp even while it accelerates a descending arm joint toward the fixture faster than the remaining travel can absorb, and the clamp registers no fault until no allowable reverse torque can prevent the collision.
...
Integral action beneath the enforcer adds a latency penalty during saturation. While the actuator is clipped and tracking error persists, the integrator keeps accumulating a command the plant cannot execute, so when the enforcer commands full braking the output stays pinned at the forward limit until that integral unwinds. In an analytical example where error accumulates over `{python} EnforcementAuthorityLoss.windup_span_str`, unwinding delays net braking force by $\tau_{\text{unwind}} =$ `{python} EnforcementAuthorityLoss.unwind_str`. On the mobile manipulator's base at the `{python} EnforcementAuthorityLoss.v_aisle_str` aisle speed, this delay adds $v\,\tau_{\text{unwind}} =$ `{python} EnforcementAuthorityLoss.windup_travel_str` of unbraked travel, almost twice the `{python} EnforcementAuthorityLoss.spare_str` that the finished stopping budget of @sec-enforcement-stopping-envelopes leaves spare. Anti-windup clamping, back-calculation, and conditional integration address the loop [@astrom2006advanced; @franklin1998digital]; the enforcement requirement is to measure the unwinding delay that remains and budget its unbraked distance inside the stopping horizon.
...
The specification contract grounds its guarantees in quantified parameters. For the machine at the rack end and the cage door, the entries that fix its stops and its validity regions read as follows. Most repeat values the chapter has already derived; the arm's validity region and the base's restricted configuration are set here for the first time.

- The tracking bound $\epsilon_{\text{track}}$ is `{python} EnforcementRecordMachine.eps_track_str` (illustrative), the inset of row 7 in @tbl-enforcement-stopping-budget.
...
No enforcement guarantee holds when the machine operates outside its designed physical environment, so the record bounds the plant dynamics within an explicit validity region. For the arm, the operating region the record bounds is the target ODD of @sec-evaluation-logs, and the first proposal to meet it is the approach to the cage door. The chunk policy proposes the arm's approach to the strike plate at `{python} EnforcementRecordMachine.v_latch_high_str`. The target ODD admits only the nominal plate, the door's own load, a stopped base, and the `{python} EnforcementRecordMachine.v_latch_str` guarded approach, and the approach-speed channel of the evaluation record's `runtime_monitor_specs` enters the check as a `{python} EnforcementRecordMachine.v_latch_str` ceiling inside the door zone. The enforcer therefore projects the proposal to `{python} EnforcementRecordMachine.v_latch_str` and logs the channel that bound it. The arithmetic alone would have admitted the faster approach, whose `{python} EnforcementRecordMachine.t_yield_high_str` contact deadline still exceeds the `{python} EnforcementRecordMachine.t_contact_str` contact loop, but neither the demonstrations nor the evaluation trials behind the policy include that speed (@sec-data-schema), so the projection rests on evidence rather than on the contact model. A live state outside the region that the machine can observe, such as a payload the manifest never declared, is refused like any other failed check, and the supervisory monitors that detect it revoke policy dispatch and select the state-matched stop. A floor below the base's friction premise is not such a state, because no sensor on the machine observes the friction under its wheels. The normal configuration therefore rests on that friction as a premise, and the restricted configuration rests on inspection of the floor (@sec-frontier-residual-claims-register).
