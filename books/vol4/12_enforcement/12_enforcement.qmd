...
#| echo: false
# ┌── LOAD: timing contract and a constructed lease-expiry stop ──────────
from mlsysim import Embodied
from mlsysim.core.units import meter, second, millisecond, millimeter, hertz
...
class EnforcementLeaseStop:
    # Timing contract: the same scenario fields that size the lease in @sec-nervous-cadences
    scenario = Embodied.Scenario.WarehouseAisle
    chunk_rate = Embodied.MobileManipulator.WarehouseMobileManipulator.chunk_rate  # category: chosen
    lease = scenario.t_lease  # category: chosen
    heartbeat_rate = scenario.heartbeat_rate  # category: chosen
    heartbeat_timeout = scenario.heartbeat_timeout  # category: chosen
    self_watchdog = scenario.mcu_self_watchdog  # category: chosen
    renewal_jitter = 5 * millisecond  # scenario assumption; category: chosen
    # Constructed AMR stop (a scenario, not the running machine)
    speed = 1.2 * meter / second  # category: illustrative
    brake_response = 50 * millisecond  # category: illustrative
    braking = 2.5 * meter / second**2  # category: illustrative
    tracking = 40 * millimeter  # category: illustrative

    # ┌── EXECUTE ───────────────────────────────────────────────────────────
    chunk_period = (1 / chunk_rate).to(millisecond)
    renewal_bound = (chunk_period + renewal_jitter).to(millisecond)
    delay = (lease + brake_response).to(millisecond)
    clearance = calc_safe_stopping_distance(speed, delay, braking, physical_margin=tracking)

    # ┌── GUARD ─────────────────────────────────────────────────────────────
    check(renewal_bound < lease, "The chunk period plus renewal jitter must fit inside the lease.")
    check(heartbeat_timeout < lease, "A total host stall must trip the heartbeat before the lease expires.")
    check(abs(clearance.to(millimeter).magnitude - 460.0) < 1e-6, "Constructed stop: 1.2 m/s, 110 ms, 2.5 m/s^2, 40 mm gives 460 mm.")

    # ┌── OUTPUT ────────────────────────────────────────────────────────────
    chunk_rate_hz_str = fmt(chunk_rate.to(hertz).magnitude, precision=0)
    chunk_period_ms_str = fmt_latency(chunk_period, unit=millisecond, precision=0)
    renewal_jitter_ms_str = fmt_latency(renewal_jitter, unit=millisecond, precision=0)
    renewal_bound_ms_str = fmt_latency(renewal_bound, unit=millisecond, precision=0)
    lease_ms_str = fmt_latency(lease, unit=millisecond, precision=0)
    lease_seconds_value_str = fmt(lease.to(second).magnitude, precision=3)
...
Refusal is therefore a steady-state decision, not only an emergency exception. On each illustrated millisecond control tick, the enforcer checks the proposal's freshness against its lease and evidence deadline, the separate host heartbeat, the actuator and kinetic limits that apply in the current state, tracking error against its bound, and that a fallback remains feasible from the current state. If a check fails, it revokes the proposal and selects the resident response appropriate to the current state and available clearance. That permission path must remain able to act when the upstream proposal source fails.
...
The pre-brake delay $\tau_{\text{delay}}$ in the stopping distance of @sec-body-kinetic-momentum already includes the enforcer's own permission-loop tick, which @sec-nervous-cadences budgets with the lease path. The term this chapter adds to the stopping budget is the tracking inset.
...
Consider an analytical example of an autonomous mobile robot (AMR) navigating an industrial facility with an onboard safety LiDAR sensor offering a clear detection horizon $d_{\text{sense}} = 2.50\text{ m}$. The nervous system exhibits a total command transport and electrical rise delay $\tau_{\text{delay}} = 50\text{ ms} = 0.050\text{ s}$ (accounting for sensor ingress, fieldbus packet exchange, safety filter solve, and motor inverter rise time). Within the declared load and surface envelope, the chassis braking system provides a credible deceleration $a_{\text{brake}} = 2.50\text{ m/s}^2$, and the low-level tracking controller provides a validated transient error bound $\epsilon_{\text{track}} = 40\text{ mm} = 0.040\text{ m}$. This idealized worked calculation takes the localization bound and the protective clearance as zero ($\delta_{\text{loc}} = \delta_{\text{margin}} = 0$); nonzero values subtract directly from available clearance. The deceleration is a lower bound on achievable braking in this scenario, not a motor nameplate maximum.

At a nominal cruising speed $v_1 = 1.20\text{ m/s}$, the reaction drift distance during transport lag is $d_{\text{react}} = v_1 \tau_{\text{delay}} = (1.20\text{ m/s})(0.050\text{ s}) = 0.060\text{ m} = 60\text{ mm}$. The active braking distance is $d_{\text{brake}} = v_1^2 / (2 a_{\text{brake}}) = (1.20\text{ m/s})^2 / (2 \times 2.50\text{ m/s}^2) = 0.288\text{ m} = 288\text{ mm}$. Summing these with the tracking margin yields a total required stopping clearance of $d_{\text{stop}}(1.20) + \epsilon_{\text{track}} = 0.060\text{ m} + 0.288\text{ m} + 0.040\text{ m} = 0.388\text{ m} = 388\text{ mm}$.

If an optimization policy doubles vehicle velocity to $v_2 = 2.40\text{ m/s}$ to increase warehouse throughput, the reaction drift expands linearly to $d_{\text{react}} = (2.40)(0.050) = 0.120\text{ m} = 120\text{ mm}$, while the active braking distance quadruples to $d_{\text{brake}} = (2.40)^2 / (2 \times 2.50) = 1.152\text{ m} = 1152\text{ mm}$. Under the same zero-overhead assumption, the total required stopping clearance surges to $d_{\text{stop}}(2.40) + \epsilon_{\text{track}} = 0.120\text{ m} + 1.152\text{ m} + 0.040\text{ m} = 1.312\text{ m} = 1312\text{ mm}$. While velocity increased by 100 percent ($2.0\times$), the required stopping clearance expanded by 238 percent ($3.38\times$), demonstrating how the quadratic scaling of kinetic braking energy ($E_k \propto v^2$) dominates spatial clearance demands.
...
Furthermore, if the robot approaches a blind warehouse rack aisle where structural shelving limits the unobstructed sightline to $D_{\text{clear}} = 1.20\text{ m}$, the stopping envelope of @eq-enforce-stopping-envelope, with $a_{\text{eff}} = a_{\text{brake}}$ and zero overheads, requires:
$$\frac{1}{2 a_{\text{brake}}} v^2 + \tau_{\text{delay}} v + (\epsilon_{\text{track}} - D_{\text{clear}}) \le 0$$
...
- [ ] **Datasheet limits vs. dynamic headroom**: Can you explain why evaluating safety barriers using nominal datasheet torque ratings creates a false sense of security during vertical or loaded maneuvers?
...
class EnforcementCycleBudget:
    ...
    acquisition = 15 * microsecond  # category: chosen (sub-budget)
    proposal_lease = 10 * microsecond  # category: chosen (sub-budget)
    ...
    reserve = 50 * microsecond  # category: chosen (sub-budget)
...
**Systems Analysis**: In this constructed schedule, an unloaded cycle of acquisition, validation, and a typical active-set solve completes in about `{python} EnforcementCycleBudget.unloaded_us_str` $\mu\text{s}$. That is a nominal observation, not a full-cycle bound.

The qualified full path instead declares a worst-case execution time of `{python} EnforcementCycleBudget.wcet_us_str` $\mu\text{s}$ for the whole cycle, of which the solver's share is `{python} EnforcementCycleBudget.qp_us_str` $\mu\text{s}$, inside a `{python} EnforcementCycleBudget.deadline_us_str` $\mu\text{s}$ deadline within the $1.0\text{ ms}$ tick; a timeout at the deadline transfers authority before an overrun. For a clock-cycle-accurate worst-case execution time (WCET) analysis, see @sec-appendix-systems-timers.
:::

In this illustrative heterogeneous design, the shield executes inside a $1000\text{ Hz}$ ($1000\,\mu\text{s}$) timer interrupt routine on the real-time microcontroller core. The declared worst-case budget divides `{python} EnforcementCycleBudget.wcet_us_str` $\mu\text{s}$ among the stages below, and the whole sequence must finish inside the `{python} EnforcementCycleBudget.deadline_us_str` $\mu\text{s}$ deadline:

1. **Interrupt Entry & State Acquisition (`{python} EnforcementCycleBudget.acquisition_us_str` $\mu\text{s}$):** Hardware timer triggers ISR; firmware reads raw optical encoder registers and ADC current shunts.
2. **Proposal & Lease Verification (`{python} EnforcementCycleBudget.proposal_lease_us_str` $\mu\text{s}$):** The MCU inspects the shared SRAM ring buffer populated by the Linux MPU over RPMSG or shared memory. It validates the checksum, confirms sequence order, and checks lease expiry. The proposal's age on the MCU clock may not exceed the `{python} EnforcementLeaseStop.lease_ms_str` chunk lease. This lease limits command age under the stated clock-conversion and delivery bounds. A separate `{python} EnforcementLeaseStop.heartbeat_rate_hz_str` $\text{Hz}$ host heartbeat times out after `{python} EnforcementLeaseStop.heartbeat_timeout_ms_str` of silence. The MCU feeds its `{python} EnforcementLeaseStop.self_watchdog_ms_str` self-watchdog only after successful control cycles; an external trip covers MCU loss of execution.
...
5. **Output Selection & Register Write (`{python} EnforcementCycleBudget.write_us_str` $\mu\text{s}$):** If the QP is feasible and constraints hold, $u^*$ is written directly to the PWM timer compare registers. If the chunk lease has expired, tracking error exceeds $\epsilon_{\text{track}}$, or actuator saturation renders the feasible control set empty ($\mathcal{U}_{\text{safe}} = \emptyset$), the output selector revokes the policy and selects the state-matched fallback. If the state is already outside the viable set, that fallback can limit harm but cannot restore a forward-invariance claim.
...
- [ ] **Torque-space change vs. path**: What does the minimum-norm projection ($\mathbf{u}^* = \mathbf{u}_{\text{nom}} - \lambda^* \mathbf{a}$) minimize, and why does it not by itself preserve the Cartesian path?
...
Echoing the layered competence hierarchies of behavior-based robotics and the subsumption architecture\index{Subsumption architecture} [@brooks1986subsumption], the **four-tier fallback ladder**\index{Four-tier fallback ladder} is a graded, deterministic escalation hierarchy of defensive actions (project: minimal-intervention QP projection; hold: active position hold; stop: controlled dynamic stop; inhibit: drive torque inhibit) that transitions across independent execution substrates as control authority, solver feasibility, or hardware invariants degrade.

The first rung of the fallback ladder is least-squares projection\index{Least-squares projection}, executed on every control cycle while the feasible control set remains non-empty ($\mathcal{U}_{\text{safe}} \neq \emptyset$). When a nominal proposal $u_{\text{nom}}$ points outside the admissible half-spaces, the enforcer computes the minimal-norm modification $u^* = \arg\min_u \|u - u_{\text{nom}}\|^2$ subject to the active barrier inequalities. In the constructed $1000\text{ Hz}$ loop, a nominal solve finishes within its `{python} EnforcementCycleBudget.qp_us_str` $\mu\text{s}$ share of the declared full-cycle budget. Projection supports forward invariance only from a viable state under the validated model, measurement, actuation, and deadline bounds. The projection succeeds only while the intersection of barrier constraints and achievable actuator torques contains a solution and the system remains in the validated viable region. The admission path must preserve viability before saturation makes $\mathcal{U}_{\text{safe}}=\emptyset$. If infeasibility occurs anyway, the MCU revokes the proposal and invokes the best available state-matched response without claiming restored invariance.
...
1.  **Significance**: Physical AI systems cannot treat all faults identically; cutting actuator power on dynamic, underactuated, or high-momentum plants (e.g., legged robots at speed, multirotors in flight) can precipitate catastrophic tumbling, requiring state-matched deceleration before terminal torque removal.
...
The Therac-25 radiotherapy accelerator shows the concurrency class with nothing independent behind it. Its designers removed the hardware interlocks of earlier models and left enforcement to software on the same computer that ran the treatment, where a race in operator data entry and a one-byte counter that rolled over to zero each let the machine fire a high-current beam without its target in place [@leveson1993investigating]. Between 1985 and 1987 six patients received massive overdoses, and several died.
...
: **Illustrative enforcement state contract**: The `{python} EnforcementLeaseStop.chunk_rate_hz_str` $\text{Hz}$ proposal, `{python} EnforcementLeaseStop.lease_ms_str` chunk lease, `{python} EnforcementLeaseStop.heartbeat_rate_hz_str` $\text{Hz}$ host heartbeat, and $1\text{ kHz}$ MCU loop have distinct renewal and failure paths. Timing and physical arrest limits require configuration-specific validation. {#tbl-12-fallback-hierarchy tbl-colwidths="[18,29,22,31]"}
