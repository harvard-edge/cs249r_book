...
Authority depletion must also reach the planner. A `{python} EnforcementLeaseStop.chunk_rate_hz_str` $\text{Hz}$ policy that keeps commanding accelerations to a motor saturated at $1000\text{ Hz}$ plans against a body that cannot execute them, so the permission path reports saturation as an active constraint violation, revokes the active trajectory, and requires a re-plan from the machine's authority-constrained state.
...
An enforcer hosted as a real-time operating system task rather than a bare-metal interrupt adds priority inversion\index{Priority inversion} at the operating-system layer, where a low-priority telemetry thread holds a mutex the $1000\text{ Hz}$ enforcer needs and a medium-priority thread keeps the holder from running, so the check waits without bound. The machine's timer interrupt and lock-free mailbox exclude that case by design. On a task-hosted enforcer, priority inheritance (`PTHREAD_PRIO_INHERIT`) or ceiling locking bounds the wait, and @sec-nervous-deterministic-fieldbuses examines the failure in flight hardware.
