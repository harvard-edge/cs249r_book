...
# ┌── 1. LOAD ─────────────────────────────────────────────────────────────────
v_drive = RM.base.max_velocity
v_aisle = AISLE.v_aisle
a_brake = AISLE.a_brake
lease_path = AISLE.t_lease + AISLE.t_tick + AISLE.t_bus
d_clear_mm = AISLE.d_clear.to(millimeter).magnitude
...
# ┌── 1. LOAD ─────────────────────────────────────────────────────────────────
A_BRAKE = AISLE.a_brake.to(meter / second**2).magnitude
TAU_DELAY = AISLE.tau_delay.to(second).magnitude
OVERHEAD = AISLE.fixed_overhead.to(meter).magnitude
D_CLEAR = AISLE.d_clear.to(meter).magnitude
V_AISLE = AISLE.v_aisle.to(meter / second).magnitude
_v_ref = RM.base.max_velocity
C2_COEFF = (calc_c2_stop_suffix(_v_ref, AISLE.a_brake)["distance"] * AISLE.a_brake / _v_ref**2).to("dimensionless").magnitude
...
- [ ] **The kinematic roofline principle**: Can you describe how finite sensor detection horizons and occluded sightlines (such as a rack end in a warehouse aisle) impose an unyielding ceiling on safe operating speed, regardless of how fast neural inference can propose trajectories?
:::
...
class EnforcementAuthorityLoss:
    ...
    bias_fraction = 0.75  # scenario assumption; share of peak torque holding the load on descent
...
class KineticBarrierArm:
    ...
    speed = EnforcementAuthorityLoss.joint_speed  # rad/s; scenario assumption
    radius = EnforcementAuthorityLoss.link_length  # scenario assumption
    gamma = 10.0 / second  # scenario assumption; category: chosen (barrier gain)
    clear_far = 110 * millimeter  # scenario assumption
    clear_near = 70 * millimeter  # scenario assumption
...
Carry the descending-arm scenario of @sec-enforcement-exhausted-authority into this barrier: $J=$ `{python} EnforcementAuthorityLoss.inertia_str`, $\tau_{\text{bias}}=$ `{python} EnforcementAuthorityLoss.bias_str` toward the fixture, $|\tau|\le$ `{python} EnforcementAuthorityLoss.torque_limit_str`, $\alpha=(\tau_{\max}-\tau_{\text{bias}})/J=$ `{python} KineticBarrierArm.alpha_str` $\text{rad/s}^2$, $\ell=$ `{python} EnforcementAuthorityLoss.link_length_str`, $\omega=$ `{python} EnforcementAuthorityLoss.joint_speed_str` $\text{rad/s}$, and $\gamma=$ `{python} KineticBarrierArm.gamma_str` $\text{s}^{-1}$. These are stated joint-level scenario inputs; the bias is not inferred from the payload mass alone. At `{python} KineticBarrierArm.clear_far_str` of tool clearance, $D=$ `{python} KineticBarrierArm.angle_far_str` $\text{rad}$ and $h=$ `{python} KineticBarrierArm.h_far_str` $\text{rad}$, so @eq-enforce-kinetic-torque requires the joint to brake with at least `{python} KineticBarrierArm.brake_far_str`. At `{python} KineticBarrierArm.clear_zero_str`, $h=0$ and the requirement reaches `{python} KineticBarrierArm.brake_zero_str`, the joint's full peak torque. At `{python} KineticBarrierArm.clear_near_str`, it demands `{python} KineticBarrierArm.brake_near_str`, more than the joint can produce: this state is already outside the modeled viable set. The enforcer must prevent entry into it; a fallback started there cannot restore the lost clearance. These results assume immediate torque realization, constant bias, and exact state. A deployed checker must inset the boundary for bounded response delay, measurement error, and torque-tracking deviation.
...
In this illustrative heterogeneous design, the shield executes inside a `{python} EnforcementCycleBudget.rate_str` timer interrupt routine on the real-time microcontroller core. The declared worst-case budget divides `{python} EnforcementCycleBudget.wcet_us_str` $\mu\text{s}$ among the stages below, and the whole sequence must finish inside the `{python} EnforcementCycleBudget.deadline_us_str` $\mu\text{s}$ deadline:
