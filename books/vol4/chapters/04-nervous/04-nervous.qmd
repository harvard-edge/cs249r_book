...
::: {#fig-04-fieldbus-evolution fig-env="figure" fig-pos="htb" fig-cap="**Fieldbus Performance Trade-Off**: Evolution of industrial communication protocols plotted as a trade-off between payload bandwidth and 8-axis control cycle time. As the demand for distributed multi-axis control has grown, real-time protocols have moved down and to the right, delivering higher throughput and stricter cycle determinism."}
...
data = pd.read_csv("data/fieldbus_evolution.csv")
...
for i, row in data.iterrows():
    ax.annotate(f'{row["protocol"]} ({row["year"]})',
...
The master 5x5 synthesis grid unites the physical body (@sec-body), the deliberative brain (@sec-brain), and the real-time nervous system (@sec-nervous). With the complete physical and computational container established, Part I has assembled the machine without yet saying where its competence comes from. A foundation model does not arrive knowing how to grasp a deformable object or recover from a slipped footfall; that behavior must be taught through physical interaction. @Sec-data opens Part II by asking what physical demonstration data actually is, how it is collected from real hardware, and what makes a recorded trajectory fit to train on.
