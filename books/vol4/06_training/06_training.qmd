...
class LatchContactDeadline:
    ...
    f_lim_str = fmt_force(f_lim, precision=0)
...
::: {#dfn-training-compounding-rollout-error .callout-definition title="Compounding rollout error"}

***Compounding rollout error***\index{Compounding rollout error!definition}\index{Behavioral cloning!error accumulation} is the quadratic accumulation of closed-loop tracking drift incurred when an open-loop supervised policy executes sequentially on a physical dynamical plant, bounded in expectation by:
$$J(\pi) - J(\pi^*) \le \mathcal{O}(T^2 \epsilon)$$
where $T$ is the task execution horizon and $\epsilon = \mathbb{E}_{\mathbf{s} \sim d_{\pi^*}} [\mathbb{I}(\pi(\mathbf{s}) \ne \pi^*(\mathbf{s}))]$ is the single-step policy error rate on the demonstration distribution $d_{\pi^*}$.

1.  **Significance**: Proves that minimizing single-step prediction error on offline demonstrations is fundamentally insufficient for closed-loop stability; any nonzero error rate $\epsilon > 0$ eventually drives physical state outside demonstrated support.
2.  **Distinction**: Unlike static supervised learning where validation error scales linearly with sample count, compounding rollout error couples policy mistakes with environmental physics, turning small initial trajectory deviations into irreversible state divergence.
3.  **Common pitfall**: Assuming low offline validation loss on held-out demonstration splits translates to high closed-loop task success, ignoring that offline datasets contain zero examples of returning to nominal support once perturbed.
...
None of these mechanisms changes the behavioral-cloning support bound $\mathcal{O}(T^2 \epsilon)$. As demonstrated theoretically and empirically in @fig-06-compounding-error, single-step behavior cloning experiences a steep drop in task survival as rollout horizon $T$ grows, falling into the catastrophic intervention envelope ($<18\text{ percent}$ survival) within 250 control steps at $50\text{ Hz}$. While temporal architectures like BC-RNN delay the onset of divergence, their quadratic error compounding remains asymptotically identical. In contrast, on-policy interactive aggregation (DAgger [@ross2011reduction]) bounds error growth linearly to $\mathcal{O}(T \epsilon)$ by collecting expert feedback on learner-visited states. Without interactive supervisors, modern generative policies counteract compounding drift through action chunking ($K=16$), which contracts the effective execution horizon to $T_{\text{eff}} = T/K$ (ACT [@zhao2023learning]), and continuous receding-horizon diffusion replanning [@chi2024diffusionpolicy], maintaining over $80\text{ percent}$ task survival across extended 500-step manipulation horizons.

::: {#fig-06-compounding-error fig-env="figure" fig-pos="htb" fig-cap="**Compounding covariate shift across imitation learning paradigms**: Task survival and in-envelope rate versus closed-loop rollout horizon $T$ at $50\text{ Hz}$ ($10\text{ seconds} = 500\text{ steps}$). Single-step behavioral cloning suffers from quadratic error compounding $\\mathcal{O}(T^2 \\epsilon)$ [@ross2011reduction], dropping into the catastrophic failure envelope within 250 steps. DAgger bounds error accumulation linearly $\\mathcal{O}(T \\epsilon)$ via interactive on-policy relabeling. Modern action chunking (ACT [@zhao2023learning]) contracts the effective compounding horizon by $K=16$, while receding-horizon Diffusion Policy [@chi2024diffusionpolicy] achieves over 80 percent survival across 500 execution steps." fig-alt="Plot comparing task survival rate in percent against rollout horizon T from 0 to 500 control steps at 50 Hz. A red curve for pure single-step behavioral cloning displays a steep quadratic cliff falling below 18 percent survival by step 250 and reaching near zero by step 450. An orange dashed curve for BC-RNN delays the cliff but follows the same quadratic decay. An amber dash-dotted curve for DAgger displays linear compounding, reaching approximately 40 percent survival at step 500. A purple dotted curve for Action Chunking (ACT, K=16) and a green solid curve for Diffusion Policy maintain over 70 percent and 80 percent survival respectively at step 500. Annotated empirical benchmark points from Robomimic Can, Push-T, and Peg Insertion validate the trajectory curves."}
...
# Horizon steps T (0 to 500 steps at 50 Hz = 0 to 10 seconds)
...
ax.plot(
    ...
    linewidth=2.4,
    zorder=3,
    label=r"Pure BC (Single-Step): $\mathcal{O}(T^2 \epsilon)$ Quadratic Compounding Cliff",
)
ax.plot(
    T,
    surv_bcrnn,
    ...
    linewidth=1.8,
    zorder=3,
    label=r"BC-RNN: Delayed Cliff, Same $\mathcal{O}(T^2 \epsilon)$ Quadratic Bound",
)
ax.plot(
    T,
    surv_dagger,
    ...
    linewidth=2.2,
    zorder=3,
    label=r"DAgger: Linear Compounding $\mathcal{O}(T \epsilon)$ (Interactive Expert)",
)
ax.plot(
    T,
    surv_act,
    ...
    linewidth=2.4,
    zorder=3,
    label=r"Action Chunking (ACT, $K=16$): Horizon Contraction $T_{\mathrm{eff}} = T/K$",
)
ax.plot(
    T, surv_diff, color="#1E793C", linestyle="-", linewidth=3.0, zorder=4, label=r"Diffusion Policy ($K=16$, Receding): Multimodal & Smooth Chunking"
)

emp_points = [
    {"x": 200, "y": 26, "color": "#DC2626"},
    {"x": 200, "y": 78, "color": "#7C3AED"},
    {"x": 200, "y": 92, "color": "#1E793C"},
    {"x": 300, "y": 18, "color": "#EA580C"},
    {"x": 300, "y": 55, "color": "#D97706"},
    {"x": 300, "y": 88, "color": "#1E793C"},
    {"x": 400, "y": 4, "color": "#DC2626"},
    {"x": 400, "y": 80, "color": "#7C3AED"},
    {"x": 400, "y": 86, "color": "#1E793C"},
]
...
add_badge(
    ax,
    r"Ross-Bagnell $\mathcal{O}(T^2 \epsilon)$ Cliff"
    + "\n"
...
ax.set_xlabel(r"Rollout Horizon $T$ (Control Steps / Timesteps at 50 Hz)", fontsize=11, fontweight="bold", labelpad=8)
ax.set_ylabel("Task Survival / In-Envelope Rate (%)", fontsize=11, fontweight="bold", labelpad=8)
ax.set_xticks(np.arange(0, 501, 50))
ax.get_xaxis().set_major_formatter(ticker.FuncFormatter(lambda x, loc: f"T={int(x)}"))
ax.set_yticks(np.arange(0, 101, 10))
ax.get_yaxis().set_major_formatter(ticker.FuncFormatter(lambda y, loc: f"{int(y)}%"))

fig.suptitle(
    "Compounding Covariate Shift: Rollout Horizon vs. Task Survival Rate",
    fontsize=13.5,
    fontweight="bold",
    color="#1E293B",
    x=0.08,
...
ax.set_title(
    "Theoretical error compounding vs. empirical survival curves across imitation learning paradigms",
    fontsize=8.8,
    color="#475569",
    loc="left",
    pad=14,
...
legend = ax.legend(
    loc="lower left", bbox_to_anchor=(0.02, 0.22), frameon=True, facecolor="#FFFFFF", edgecolor="#CBD5E1", fontsize=7.8, framealpha=0.95
)
...
As detailed in @fig-diffusion-policy-architecture (@sec-appendix-ml-diffusion), the policy conditions on an observation history of images and joint states to iteratively denoise an action chunk $\mathbf{A}^0 \in \mathbb{R}^{K \times D_a}$ from pure Gaussian noise. Rather than executing the entire prediction horizon, the machine dispatches an execution horizon to the real-time microcontroller before receding-horizon re-planning occurs. The formal dual-rate pipeline—coupling asynchronous reverse-diffusion on the host with deterministic spline interpolation, rate-limiting, and safety clamping on the microcontroller—is developed in @algo-diffusion-policy-trajectory (@sec-appendix-ml-diffusion).
...
class LatchContactExploit:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────
    m_eff_str = fmt_mass(m_eff, unit=kg, precision=0)
    k_plate_str = fmt_sci_qty(k_plate, newton / meter, precision=1)
    delta_pen_str = fmt_length(delta_pen, unit=millimeter, precision=1)
    f_sim_str = fmt_force(f_sim, precision=0)
    f_real_str = fmt_force(f_real, precision=1)
    f_lim_str = fmt_force(f_lim, precision=0)
...
class LinearDriveGap:
    ...
    f_target_str = fmt_force(f_target, precision=0)
    rate_sim_str = fmt_force_rate(rate_sim, unit=newton / second, precision=0)
    rate_real_str = fmt_force_rate(rate_real, unit=newton / second, precision=0)
