...
* **Optimization Inversion:** Offline loss minimization and simulated return evaluate numerical compliance rather than physical stability. Policies that score highest on sanitized benchmarks can fail their first contact trials because of unmodeled contact compliance or omitted rotor inertia (building on the actuator limits established in @sec-body-actuator-limits).
* **Covariate Shift Compounding:** Supervised behavioral cloning penalizes deviations only along demonstrated trajectories. Minor tracking errors physically push the machine into unvisited states where the policy, having never seen this perspective, emits commands with no training support behind them, requiring recovery demonstrations or verified support monitoring to address the drift; chunking alone does not change the support bound.
...
* **Domain Randomization Limits:** Training across randomized parameter distributions forces policy invariance against bounded dynamic variations, but does not cover physical phenomena omitted entirely from the simulator's equations of motion.
* **Forgetting During Hardware Fine-Tuning:** Gradient steps on narrow physical rollout batches can resolve localized contact mismatches while degrading recovery behavior elsewhere in the state space and inflating tracking error.
