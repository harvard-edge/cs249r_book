...
Reconciling open-world reasoning with deterministic execution requires a systematic progression through the cognitive hierarchy:
...
To escape this combinatorial fragility, embodied systems must discover smooth, generalizable structure directly from multimodal sensory data rather than enumerating every physical contingency by hand.
...
- [ ] **Engineering bottleneck of explicit state machines**: Can you contrast the maintenance scalability of hand-authored Boolean state machines against continuous learned representations when scaling to long-horizon, un-modeled tasks?
- [ ] **Industrial caging versus open-world stability**: Can you explain why Lyapunov stability certificates and sub-millisecond caging guarantees (ISO 10218) cannot be directly transferred from factory workcells to unstructured search-and-rescue or domestic environments?
...
Physical AI dismantles the classical wall by replacing rigid pre-programmed models with deep neural representations fitted to multimodal sensory data. In the embodied AI and robotics community, this transition was driven by four foundational pillars:
...
The second pillar is continuous sensory robustness. Classical algorithms suffer from Boolean brittleness—minor shifts in illumination or surface texture can abruptly flip classification branches—while deep neural networks project high-dimensional sensory inputs onto smooth, continuous latent manifolds. Physically, variations in illumination, clutter, and perspective produce gradual, bounded displacements in the internal representation rather than catastrophic jumps in the robot's behavior.

The third pillar is the ability to handle complex multi-contact and deformable dynamics. Many physical manipulation tasks—such as folding textiles, routing flexible wire harnesses through automotive door assemblies, or scooping granular media—involve non-smooth contact mechanics, variable friction, and infinite-dimensional deformations that cannot be modeled in closed-form differential equations. Neural policies learn from physical teleoperation demonstrations to acquire compliance-aware sensorimotor strategies that modulate contact forces and yield to unexpected mechanical resistance instead of driving into stiff obstacles.
...
3. **Specialized Action Decoders (The Outputs):**
   ...
   * *Discrete Categorical Tokenization (RT-1, RT-2, OpenVLA):* Early VLAs adapt autoregressive language modeling directly to robotics by discretizing continuous joint or Cartesian offsets into $B = 256$ uniform bins per dimension [@brohan2023rt1; @brohan2023rt2; @kim2024openvla]:
     ...
     Each bin is assigned a discrete token (`<act_000>` to `<act_255>`) in the language vocabulary. While this natively leverages web-scale vision-language pretraining, it introduces a severe systems bottleneck: predicting a 7-DoF robot action requires 7 sequential autoregressive forward passes per control step. Streaming all multi-billion parameters from external DRAM across the memory bus 7 times induces a forward latency exceeding $600\text{ ms}$ on edge system-on-chip (SoC) accelerators, violating real-time deadlines. Furthermore, scalar binning introduces quantization chatter ($\Delta a = (a_{\max} - a_{\min}) / 256$), preventing smooth force compliance during delicate contact tasks.
...
- [ ] **Discrete binning versus continuous chunking**: Can you explain why autoregressive scalar token binning ($B=256$) saturates memory bandwidth with sequential DRAM weight sweeps, whereas continuous action chunking amortizes memory transfers across multiple control timesteps?
...
The ingestion stage translates heterogeneous, high-bandwidth bitstreams—ranging from analog voltage spikes to digital pixel grids—into normalized numerical embeddings, a uniform vocabulary for neural networks. For the mathematical formulation of patch embeddings and 3D metric lifting, see @sec-appendix-ml.
Because monocular 2D camera images suffer from fundamental scale-depth projective ambiguity—where a small near object and an identically shaped, proportionally larger distant object cast identical pixel projections—the ingestion stage must lift flat pixel arrays into 3D metric space. This grounds semantic concepts in the robot's workspace, creating a unified multimodal state for downstream reasoning.[^fn-std-curriculum-deepdives]
...
Perception extracts metric structure, semantic identity, and spatial affordances from ingested tokens. Rather than treating sensory data as an unstructured bag of visual features, perception grounds representations in the Special Euclidean group $SE(3)$, which formally defines rigid-body spatial configurations across translation and rotation coordinates.[^fn-math-se3-pose]
...
This cadence creates a freshness gap. If a large vision-language foundation model requires $400\text{ ms}$ to generate an intent lease, the physical world has moved during that inference window. If the target mug was bumped or a coworker walked between the robot and the sink, the intent lease is partially stale before it reaches the motion planner. Systems engineers must bound intent lease lifetimes ($\tau_{\text{valid}}$) and enforce strict timeout rejection gates at execution time.
...
- [ ] **3D metric lifting from projective cameras**: Can you explain why monocular 2D vision requires learned scale-depth disambiguation to lift pixel grids into the Special Euclidean group $SE(3)$, and how extrinsic camera calibration drift degrades physical grasping accuracy?
- [ ] **Spatial affordance cross-attention**: Can you describe how cross-attention mechanisms between natural language instruction tokens and visual patch embeddings dynamically generate affordance heatmaps over physical patch coordinates?
...
Token-by-token execution of neural policies forces the processor to sweep gigabytes of weight matrices across the memory bus for each millisecond control step (see @fig-patterson-roofline) [@williams2009roofline]. This memory wall bottleneck[^fn-hw-dram-bandwidth] traps inference cadences at single-digit frequencies ($1\text{--}5\text{ Hz}$). If an embodied manipulator attempted to control moving mass at $5\text{ Hz}$, the actuators would stutter across discrete steps, inducing severe contact instability and mechanical chatter.
...
Instead of fetching billions of weights from DRAM to predict a single scalar action for the immediate millisecond, the planning policy (such as Action Chunking with Transformers [ACT], Diffusion Policy, or Flow Matching) generates an entire temporal horizon of continuous actions $\mathbf{A}_{t:t+H} = [\mathbf{a}_t, \mathbf{a}_{t+1}, \dots, \mathbf{a}_{t+H}] \in \mathbb{R}^{H \times d_a}$ (typically $H = 16\text{--}64$ steps spanning $320\text{--}1280\text{ ms}$) in a single forward pass or fixed denoising schedule ($K = 10\text{--}16$ diffusion steps).[^fn-math-chunk-horizon]

[^fn-math-chunk-horizon]: **Action Chunk Horizon Bounds**\index{Action chunking!temporal horizon bounds}\index{Latency!open-loop chunk horizon}: Sizing prediction horizon $H$ balances compute amortization against open-loop drift. Restricting $H \in [16, 64]$ steps ($320\text{--}1280\text{ ms}$) confines kinematic error growth within mechanical compliance tolerances before sensory feedback corrects the trajectory.

[^fn-hw-dram-bandwidth]: **Autoregressive DRAM Bandwidth Saturation**\index{GPU!memory bandwidth wall}: Generating action tokens sequentially requires reloading billions of model weights from external LPDDR5 or high-bandwidth memory (HBM) per step, capping single-batch throughput at memory bus bandwidth ($\text{FLOPs} \ll \text{Bandwidth} \times \text{Operational Intensity}$). This memory wall forces physical architectures to employ action chunking rather than querying the network at every millisecond.
...
# │ Show:    The single-step processing rate of 10.2 Hz increases to an amortized rate of 327.7 Hz with H=32, where H denotes the number of parallel processing units. This amortization distributes the computational load across these units, enhancing system performance. This relationship impacts task scheduling efficiency and the hardware's physical limitations.
# │ How:     Before presenting the equation, we define the terms: 'M_weights' is the total number of weights in the machine learning model, 'B_sustained' is the sustained bandwidth, measured in bits per second, and 'H' is the number of processing steps. This relationship is expressed as: t_stream = M_weights / B_sustained, indicating the time to stream data based on model size and bandwidth. We derive tau_step = t_stream / H, showing how streaming time influences processing time per step, crucial for understanding system performance in real-time applications.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
class ActionChunkingLPDDR5:
    ...
    f_effective_str = fmt_frequency(f_effective, unit=ureg.hertz, precision=1)
...
Learned foundation models dismantle the classical wall of generalizability but introduce a profound liability: the loss of formal mathematical verifiability.[^fn-math-symbolic-connectionist]
...
A 'semantic hallucination' occurs when a machine learning model's latent representation produces a plausible prediction that lacks physical reality. For instance, when an autonomous mobile manipulator approaches a workstation in an industrial cleanroom, it may misinterpret sunlight reflecting off a polished transparent acrylic safety shield as open traversable space due to the processing capabilities of a visual transformer, which analyzes visual input to inform navigation. Here, a 'policy output' denotes the model's decisions on movement commands. A high-confidence output signals strong belief in the predicted action, like a trajectory command instructing the robotic arm to move through an obstacle, potentially leading to unsafe operations if incorrect. The output tensor passes all numerical range checks, but its underlying physical premise is entirely false.
...
When an agent operates in ambiguous sensory environments, Discrete classification heuristics can oscillate rapidly across consecutive time steps. For example, in a sensory environment where a vehicle must identify objects like bicycles or pedestrians, these heuristics can lead to rapid changes in classification due to sensory ambiguity, impacting the system's response time. In physical manipulation, perceptual chattering injects high-frequency direction reversals into trajectory planners, causing torque oscillations that damage gearboxes when mechanical actuator inertia cannot track rapid software state transitions.[^fn-inc-tempe-chatter]

[^fn-inc-tempe-chatter]: **Perceptual Chattering in Fatal Collisions**\index{Incidents!Uber Tempe collision}\index{Perceptual chattering!state reset hazard}: NTSB investigation (HAR-19/03) into the 2018 Tempe crash revealed that semantic classification toggling between vehicle, bicycle, and other repeatedly purged tracking histories. This chattering prevented the downstream planner from fitting a stable velocity vector, delaying emergency braking until kinetic collision was physically unavoidable.

The Tempe collision analysis (@sec-boundary-when-fails) shows that physical bodies cannot tolerate discrete cognitive chattering. Robust sensorimotor architectures must use continuous latent representations and multi-step action chunking to maintain kinodynamic trajectory smoothness across ambiguous perceptual states.
...
To prevent deliberative latency and stale proposals from causing catastrophic falls, the autonomy architecture must separate proposal generation from execution permission.[^fn-inc-drc-2015]
...
With the cognitive brain and its deliberation bounds now specified, we confront the core architectural tension of physical AI. On one hand sits the unyielding physical plant of the Body (@sec-body) with strict conservation laws, thermal dissipation limits, and irreversible momentum. On the other sits the open-world deliberative Brain analyzed in this chapter, with its stochastic tokens, multi-rate inference latency, and ungrounded proposals. In @sec-nervous, we assemble the heroic architectural bridge caught in the tension between them: **The Nervous System**. We examine how deterministic fieldbuses (EtherCAT, CAN-FD, and TSN), lock-free multi-rate seqlocks, and bare-metal safety shields arbitrate authority and guarantee survival across the causal boundary before unverified neural proposals can reach motor inverters.
:::
