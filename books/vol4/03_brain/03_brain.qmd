...
[^fn-hw-dram-bandwidth]: **Autoregressive DRAM Bandwidth Saturation**\index{GPU!memory bandwidth wall}: Generating action tokens sequentially requires reloading billions of model weights from external LPDDR5 or HBM per step, capping single-batch throughput at memory bus bandwidth ($\text{FLOPs} \ll \text{Bandwidth} \times \text{Operational Intensity}$). This memory wall forces physical architectures to employ action chunking rather than querying the network at every millisecond.
