...
[^fn-math-lipschitz-calibration]: **Uncontrolled Lipschitz Bounds in Neural Networks**\index{Uncontrolled Lipschitz Bounds in Neural Networks}: While neural networks with continuous activations are mathematically Lipschitz continuous on bounded domains, their Lipschitz constant is uncertified and often astronomically large ($L \gg 10^3$). In physical terms, a tiny change in a camera image could cause the network to output a wildly different and dangerous motor command without any pointwise safety guarantees. For the rigorous math behind Lipschitz continuity in networks, see @sec-appendix-ml.
