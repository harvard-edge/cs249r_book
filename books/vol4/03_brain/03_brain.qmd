...
Because deep neural policies require billions of parameters, executing them token-by-token (autoregressively) forces the system to sweep gigabytes of data across the physical memory bus for every single control step (see @fig-patterson-roofline) [@williams2009roofline]. This creates an unyielding physical bottleneck: memory bus latency traps the system at single-digit execution frequencies ($1\text{--}5\text{ Hz}$). If an embodied manipulator attempted to control moving mass at $5\text{ Hz}$, the actuators would stutter across discrete steps, causing dynamic drift and severe contact instability.
...
To guarantee smooth physical execution, systems implement **temporal ensembling**\index{Temporal Ensembling}, blending overlapping trajectory predictions via exponentially weighted averaging:
