...
::: {#fig-cadence-gap fig-env="figure" fig-pos="htb" fig-cap="**Cadence regimes**: Parameter counts and example update rates for unlike control, vision, and embodied workloads." fig-alt="Log-log scatter of parameter counts and example update rates for different workload classes, with shaded control and deliberation rate bands."}
...
The plotted points are workload regimes (illustrative; see the Reader Guide), not measurements on a common device or input, and parameter count alone cannot determine update rate. Reconciling open-world reasoning with deterministic execution requires a systematic progression through the cognitive hierarchy:
...
Learned representations approach these three breakdowns from data rather than from hand-authored models.
...
4. **The Proposal Boundary (Isolation from Actuators):**
   Whether the model is a VLN mobility planner or a VLA manipulation policy, its output is strictly an **unprivileged candidate proposal**\index{Unprivileged proposal!proposal boundary} ($p_t$). Under the machine model of @sec-boundary-machine-model, the foundation model operates in unprivileged user space on the application processor and possesses **zero direct hardware authority** to write pulse-width modulation (PWM) registers or physically switch current into the motor coils.

   The design rule for this deliberative boundary is that cognitive models operating at low inference cadences ($1\text{--}10\text{ Hz}$) propose kinematic setpoints ($\mathbf{q}, \dot{\mathbf{q}}, \ddot{\mathbf{q}}$ or Cartesian poses), **never unbuffered, open-loop motor torques ($\boldsymbol{\tau}$)** directly to inverter switches. Motor torque is directly coupled to continuous plant dynamics:
...
:::
   ...
   Proposals cross lock-free shared memory to the permission path, which in the two-processor implementation runs on an isolated microcontroller (MCU). At $1000\text{ Hz}$ in that implementation, it checks stopping distance, torque limits, and the actuator thermal limits detailed in @sec-body-thermal-duty-cycles before granting execution permission, so a hallucinated proposal can be refused when independent measurements, modeled limits, and a feasible fallback support that decision.[^fn-math-unprivileged-control]

[^fn-math-unprivileged-control]: **Proposal Boundary**\index{Proposal boundary!unprivileged proposals}\index{Control barrier function!safety invariance}: Treating the neural policy as an unprivileged proposal generator isolates stochastic inference from actuator registers. Candidate trajectory setpoints $\mathbf{q}_{\text{ref}}(t)$ are continuously evaluated against forward-invariant Control Barrier Functions (CBFs) $h(\mathbf{x}) \ge 0$ on dedicated safety silicon, so proposals that violate modeled actuator limits or keep-out regions can be rejected. Mathematical formulations of CBFs appear in @sec-appendix-control.
...
The ingestion stage translates heterogeneous, high-bandwidth bitstreams—ranging from analog voltage spikes to digital pixel grids—into normalized numerical embeddings, a uniform vocabulary for neural networks.
...
where $\mathbf{a}_{t \mid t-i}$ represents the action predicted for time step $t$ by the action chunk initiated at time $t-i$, and $m > 0$ is a tunable temporal discount factor. The resulting smoothed waypoints are interpolated via quintic ($C^2$) splines below the proposal boundary, at $1000\text{ Hz}$ in one implementation, ensuring continuous positions, velocities, and accelerations across all actuator joints while preserving valid $SE(3)$ transformations via Lie algebra $\mathfrak{se}(3)$ exponential maps or Spherical Linear Interpolation to maintain orthonormal rotation matrices.
...
3. **The Proposal Boundary:** The Brain proposes candidate intent and trajectories in unconstrained software coordinates; the permission path admits or refuses each one against limits stated in verified SI units.
...
Embodied machines encountering out-of-distribution (OOD) observations—such as novel optical textures, deep shadows, or lens water droplets—can assign confidence scores exceeding $0.95$ to erroneous classifications.
...
**Context**: The 2015 DARPA Robotics Challenge Finals tested robots, most of them humanoid in layout, on disaster-response tasks under intentionally degraded communications [@krotkov2017darpa].\index{Incidents!DARPA Robotics Challenge}
...
To limit the effect of stale plans during balance disturbances, the architecture separates proposal generation from local execution permission.\index{Proposal boundary!DRC dynamic balance}
...
Before the fallacies, verify your understanding of runtime failure modes and how the permission path refuses a proposal:
...
The mobile manipulator's chunk policy proposes an arm trajectory toward the mug on the conveyor. Host scheduling delays delivery past the proposal's expiry, while residual belt slip has carried the mug toward the edge of the grasp tolerance. The trajectory remains smooth and numerically valid, but the observation that justified it no longer describes where the mug is. Executing it could close the gripper on the rim or knock the mug from the belt. The permission path must reject the expired proposal and leave control with the validated local response (the expiry that every intent lease carries, @sec-intent-lease). Smoothness describes the proposed command; temporal validity determines whether the machine may still act on it.

:::

## Summary {#sec-brain-summary}

In the machine model of @sec-boundary-machine-model, high-capacity neural representations act as unprivileged proposal engines parsing open-world multimodal complexity, while the permission path evaluates dynamic and kinematic invariants (e.g., momentum limits and collision boundaries) with exclusive authority over physical actuators.
...
* **The Proposal Boundary:** The Brain operates strictly as an unprivileged proposal engine ($p_t$), possessing capability to suggest actions, but zero authority to command actuators without deterministic validation.
* **Softmax is Not Probability:** Exponentiated logits sum to one by mathematical definition, not physical truth; models can be overconfident under out-of-distribution shift, requiring empirical calibration (ECE) and selective abstention.
:::

A neural foundation model is an expressive inductive engine, but nothing in its weights encodes the physical laws governing its machine. When those weights generate a smooth trajectory that demands infeasible motor acceleration or violates contact friction limits, the permission path, not gradient-descent convergence, must prevent structural failure.
