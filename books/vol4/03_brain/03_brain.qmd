...
A robot among people must interpret changing surroundings and propose actions beyond fixed programmed movements, and learned models supply that capability by connecting what the robot sees with patterns acquired in training. The capability is paid for in memory traffic. Every fresh evaluation of a multi-billion-parameter model streams its weights across a bus of fixed bandwidth, so the model that understands the scene best is also the one that sees it least often. Emitting many future setpoints from each evaluation keeps the motors supplied, but those setpoints age with the observation that produced them, and a proposal that arrives on time can still be wrong. Neither speed nor confidence therefore entitles a learned model to command the machine. The Brain supplies candidate behaviors, and the permission path beneath the proposal boundary decides, before any energy crosses the causal boundary, which of them the machine may still act on.
...
The plotted points are workload regimes (illustrative; see the Reader Guide), not measurements on a common device or input, and parameter count alone cannot determine update rate.
...
Rather than an opaque monolith, an embodied foundation model is a four-stage neural pipeline (@fig-vla-taxonomy, @fig-vla-architecture) that transforms sensory inputs into candidate motion proposals that still need kinematic checks.
...
Each stage of this pipeline is paid for in memory traffic. Every patch token must be attended over and held in the key-value cache, and every evaluation of the backbone streams its weights from DRAM, so the capabilities this section described arrive at a rate the application processor sets rather than one the task chooses. On the mobile manipulator, that rate for its largest model falls well below the rate at which its arm consumes setpoints.
...
Four stages turn camera frames into those proposals (@fig-cognitive-pipeline), and Part III develops each one. Here they matter only for what each hands downstream and how quickly that goes stale. Perception begins by tokenizing the sensor streams, the ingestion box of the figure, and grounds what it recognizes in metric coordinates relative to the base, so that the mug becomes a pose and a graspable handle rather than a label;[^fn-brain-metric-grounding] its output ages from the moment of capture, and @sec-perception turns that age into a contract. Memory carries those estimates through occlusion and must let each one expire when the evidence behind it does (@sec-memory). Intent decomposes a task such as "take the ceramic mug off the conveyor and put it in the tote" into subgoals, and because the mug moves while the intent model deliberates, it hands the planner a target that expires, an intent lease\index{Intent lease!temporal contract} (@sec-intent-lease). Planning converts that target into candidate setpoints for the base and the arm, the proposals the permission path will judge (@sec-planning). All four stages run on the application processor, and all four draw on the same memory bus, which is where the machine's cadence is decided.
...
**Systems lesson**: A fall can develop on the scale of a few hundred milliseconds for a standing biped. High-level planning cannot be the sole balance response; a local controller needs current state, sufficient torque and contact authority, and a feasible recovery action when a proposal stalls.\index{Proposal boundary!DRC dynamic balance}
:::
...
* **A proposal is true only for a while**: Intent and chunks expire with the evidence behind them, and a smooth, in-range, on-time proposal can still be stale or wrong. That is why the permission path, never the proposer, decides whether the machine acts.
:::

Every limit this chapter derived is a limit of supply or of truth, never of authority. Weight traffic fixes how often the intent model can look, chunking fixes how long the arm stays supplied between looks, and drift, calibration, and expiry fix how long what the Brain proposes remains true. None of these numbers decides whether a proposal may reach the motors. They are the proposer limits that the permission path takes as inputs when it makes that decision.

::: {.callout-chapter-connection title="From learned proposals to the permission path"}
How long may one proposal keep driving the machine if its successor never arrives? The mobile manipulator's chunk policy renews every `{python} ConveyorChunkSizing.chunk_period_str` and its intent model far less often, and either can stall, arrive late, or be wrong about the mug. @Sec-nervous derives from the stopping budget of @sec-body how often the permission path must check each proposal and how long one may drive the actuators before the machine falls back, and it records both in the handoff record alongside the proposer limits established here.
:::
