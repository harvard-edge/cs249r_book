...
_csv = (
    "vol4/03_brain/data/brain_frequency_vs_parameters.csv"
    if os.path.exists("vol4/03_brain/data/brain_frequency_vs_parameters.csv")
    else "data/brain_frequency_vs_parameters.csv"
)
...
fig, ax = plt.subplots(figsize=(14, 8.5), dpi=200, facecolor="#FFFFFF")
...
# 2. Deadlines
deadlines = [
    (50.0, "50 Hz: Real-Time Dynamic Manipulation & Tactile Contact Deadline", "#1E793C", "--"),
    (10.0, "10 Hz: Coarse Visual Servoing & Quasistatic Pick-and-Place Limit", "#D97706", ":"),
    (2.0, "2 Hz: Cloud VLM Autoregressive Latency Floor (0.5–2 s)", "#A51C30", "-."),
]
...
ax.plot(ar_params, ar_freq, color="#A51C30", lw=2.2, ls="--", zorder=4, alpha=0.8)
ax.text(
    1.2e9,
    2.4,
    "Autoregressive Memory Wall: $f \\propto 1 / M_{\\mathrm{weights}}$",
    fontsize=8.5,
    fontweight="bold",
    color="#A51C30",
    rotation=-6,
    ha="center",
    bbox=dict(boxstyle="round,pad=0.2", facecolor="#FFFFFF", edgecolor="#FECACA", alpha=0.9),
)

fx = np.array([8e7, 5e8, 2e9, 3.3e9, 5e9, 1.5e10])
fy = np.array([50.0, 50.0, 50.0, 52.0, 80.0, 100.0])
ax.plot(fx, fy, color="#1E793C", lw=2.5, ls="-", zorder=4, alpha=0.85)
ax.text(
    1.8e8, 56.0, "Action Chunking Frontier ($H \\geq 16$, Flow Matching)", fontsize=8.5, fontweight="bold", color="#1E793C", ha="left", va="bottom"
)

def add_badge(ax, text, xy, xytext, color="#1E293B", fontsize=7.8, arrow=True, arrow_color="#94A3B8"):
...
for m in models:
    ...
    ax.scatter(m["plot_params"], m["freq"], color=m["color"], s=s_size, marker=m["marker"], edgecolor="#FFFFFF", linewidth=1.5, zorder=10)
    add_badge(
        ax, m["text"], xy=(m["plot_params"], m["freq"]), xytext=m["badge_xytext"], color="#1E293B", fontsize=7.8, arrow=True, arrow_color=m["color"]
    )
...
ax.get_xaxis().set_major_formatter(ticker.FuncFormatter(lambda x, p: f"{int(x/1e6)}M" if x < 1e9 else f"{int(x/1e9)}B"))

ax.set_yticks([0.5, 1, 2, 5, 10, 20, 50, 100, 200])
ax.get_yaxis().set_major_formatter(ticker.FuncFormatter(lambda y, p: f"{y:g} Hz"))
...
fig.suptitle(
    "The Embodied Brain Frontier: Parameter Scaling vs Physical Action Frequency (2020–2026)",
    fontsize=14,
    fontweight="bold",
    x=0.08,
    y=0.98,
...
ax.set_title(
    "Overcoming the deliberation wall: how continuous action chunking and flow matching break the memory wall ($1\\,\\mathrm{Hz} \\rightarrow 100\\,\\mathrm{Hz}$)",
    fontsize=9.5,
    color="#475569",
    loc="left",
    pad=14,
...
legend_elements = [
    Line2D([0], [0], marker='o', color='w', markerfacecolor='#A51C30', markersize=8, label='Discrete Autoregressive Tokenization (Single-Step)'),
    Line2D([0], [0], marker='^', color='w', markerfacecolor='#008080', markersize=8, label='Diffusion Action Chunking (ACT / DDPM)'),
    Line2D([0], [0], marker='*', color='w', markerfacecolor='#1E793C', markersize=12, label='Continuous Flow Matching / Dual-System VLA (π₀, GR00T)'),
    Line2D([0], [0], color='#A51C30', lw=2, ls='--', label='Autoregressive Memory Wall Trend ($f \\propto 1/M$)'),
    Line2D([0], [0], color='#1E793C', lw=2, ls='-', label='Action Chunking Scaling Frontier'),
]
legend = ax.legend(
    ...
    edgecolor="#CBD5E1",
    fontsize=7.8,
    title="Embodied Policy Architectures",
    title_fontsize=8.2,
)
