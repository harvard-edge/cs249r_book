...
- Realistic sustained DRAM bandwidth (accounting for $\sim 70\%$ bus scheduling efficiency and row-buffer turnaround delays, the physical time required to close one memory bank page and open another): $B_{\text{sustained}} \approx$ `{python} ActionChunkingLPDDR5.b_sustained_str`.
...
::: {#chk-brain-weights-streaming-deadline .callout-checkpoint title="Edge SoC memory wall and VLA weights streaming deadline"}
Consider an autonomous mobile manipulator running an open-source 7-billion parameter Vision-Language-Action (VLA) policy on an embedded edge System-on-Chip (SoC). The SoC features a 256-bit LPDDR5 memory interface with theoretical peak bandwidth $B_{\text{peak}} = 204.8\text{ GB/s}$ and sustained efficiency $\eta = 70\%$ ($B_{\text{sustained}} \approx 143.4\text{ GB/s}$). The robot must execute closed-loop trajectory updates at $f_{\text{target}} = 10\text{ Hz}$ ($\Delta t = 100\text{ ms}$).

* **FP16 weight footprint and streaming time:** Storing the 7B model in standard FP16 ($2\text{ bytes/parameter}$) requires:
  $$M_{\text{weights, FP16}} = 7.0 \times 10^9 \times 2\text{ bytes} = 14.0\text{ GB}$$
  $$t_{\text{stream, FP16}} = \frac{M_{\text{weights}}}{B_{\text{sustained}}} = \frac{14.0\text{ GB}}{143.36\text{ GB/s}} \approx 97.7\text{ ms}$$
* **Single-token deadline feasibility:** If visual patch encoding consumes $t_{\text{vision}} = 15\text{ ms}$ and transformer matrix arithmetic consumes $t_{\text{compute}} = 10\text{ ms}$:
  $$t_{\text{total, FP16}} = 15\text{ ms} + 97.7\text{ ms} + 10\text{ ms} = 122.7\text{ ms}$$
  $$f_{\text{actual}} = \frac{1}{0.1227\text{ s}} \approx 8.15\text{ Hz} < 10.0\text{ Hz} \quad (\mathbf{Deadline\ Missed!})$$
  *(If the action space requires 7 discrete scalar tokens autoregressively, 7 sequential sweeps consume $>680\text{ ms}$, dropping control to $1.46\text{ Hz}$ and starving physical actuators.)*
* **INT4 quantization speedup:** Quantizing weights to INT4 ($0.5\text{ bytes/parameter}$) contracts the footprint to $M_{\text{weights, INT4}} = 3.5\text{ GB}$:
  $$t_{\text{stream, INT4}} = \frac{3.5\text{ GB}}{143.36\text{ GB/s}} \approx 24.4\text{ ms} \implies t_{\text{total, INT4}} = 49.4\text{ ms} \implies f_{\text{actual}} \approx 20.2\text{ Hz} \quad (\mathbf{Target\ Met!})$$
* **FP16 Action Chunking ($H = 32$):** Instead of quantizing, evaluating the 7B backbone once every $300\text{ ms}$ while a lightweight 50M-parameter diffusion head ($100\text{ MB}$, $t_{\text{head, stream}} \approx 0.7\text{ ms}$) denoises $H = 32$ steps in $15\text{ ms}$ on-chip yields:
  $$\tau_{\text{step, effective}} = \frac{122.7\text{ ms} + 15\text{ ms}}{32} \approx 4.30\text{ ms/step} \implies f_{\text{effective}} \approx 232\text{ Hz}$$
* **Systems takeaway:** Action chunking breaks the memory wall without sacrificing parameter precision, transforming an 8 Hz single-step bottleneck into an effective $>200\text{ Hz}$ feedforward stream.
:::

::: {#chk-brain-kvcache-explosion .callout-checkpoint title="Multi-camera visual ingestion and KV-cache growth on shared DRAM"}
A bimanual manipulation robot features 3 synchronized RGB cameras (1 overhead workspace camera, 2 wrist cameras) streaming at $30\text{ fps}$. Each image ($224 \times 224$) is processed by a Vision Transformer with patch size $14 \times 14$. A natural language instruction contributes 32 tokens, and proprioception adds 16 tokens. The policy uses a 32-layer transformer backbone with hidden dimension $d = 4096$, $n_{\text{heads}} = 32$, and head dimension $d_{\text{head}} = 128$ in FP16 ($2\text{ bytes}$). The edge SoC features $32\text{ GB}$ of unified DRAM shared across OS, GPU, and real-time drivers ($B_{\text{sustained}} = 143.4\text{ GB/s}$).

* **Input tokens per observation step:** Each camera generates $(224/14) \times (224/14) = 256$ visual tokens. Total prompt size per step is:
  $$S_{\text{step}} = (3 \times 256) + 32 + 16 = 768 + 48 = 816\text{ tokens/step}$$
* **KV-cache footprint per step:** Storing the Key and Value matrices across $L = 32$ layers requires:
  $$\text{Size}_{\text{token}} = 2 \ (\text{K and V}) \times 32 \times 4096 \times 2\text{ bytes} = 524{,}288\text{ bytes} \approx 0.50\text{ MB/token}$$
  $$M_{\text{KV, step}} = 816\text{ tokens} \times 0.50\text{ MB/token} \approx 408.0\text{ MB/step}$$
* **Rolling 5-second context growth:** Over a 5-second window at $10\text{ Hz}$ (50 observation steps):
  $$M_{\text{KV, 5s}} = 50 \times 408.0\text{ MB} = 20{,}400\text{ MB} = 20.4\text{ GB}$$
* **DRAM contention and memory exhaustion:** Combining model weights ($14.0\text{ GB}$) with unpruned rolling visual KV-cache ($20.4\text{ GB}$) yields $34.4\text{ GB} > 32.0\text{ GB}$ physical memory, triggering an immediate out-of-memory kernel panic. Even on a 64 GB system, streaming $20.4\text{ GB}$ of KV cache during attention consumes $t_{\text{KV}} = 20.4 / 143.4 \approx 142.3\text{ ms}$, dropping execution frequency below $4.2\text{ Hz}$ and locking out real-time nervous system telemetry.
* **Systems takeaway:** Embodied foundation models must avoid unpruned autoregressive visual history accumulation; policies must compress visual features via spatial pooling, attend via cross-attention without temporal KV retention, or rely on fixed-horizon action chunking.
:::

::: {#chk-brain-chunk-horizon-drift .callout-checkpoint title="Action chunk horizon sizing, execution overlap, and dynamic drift"}
A 7-DoF robotic manipulator picks electronic components from a moving conveyor ($v_{\text{conveyor}} = 0.30\text{ m/s}$). The trajectory setpoint sampling period is $\Delta t = 20\text{ ms}$ ($50\text{ Hz}$). The vision-language policy runs on an edge accelerator with inference latency $t_{\text{infer}} = 160\text{ ms}$. Under external disturbance (conveyor belt slip), the physical target drifts relative to the open-loop trajectory with acceleration $a_{\text{drift}} = 0.50\text{ m/s}^2$, accumulating spatial error $\Delta x_{\text{drift}} \approx \frac{1}{2} a_{\text{drift}} t^2$. The mechanical gripper has a maximum allowable alignment tolerance of $\Delta x_{\max} = 15\text{ mm}$ ($0.015\text{ m}$).

* **Buffer starvation threshold ($H_{\min}$):** To prevent the actuator buffer from emptying before the next neural prediction completes:
  $$H_{\min} = \frac{t_{\text{infer}}}{\Delta t} = \frac{160\text{ ms}}{20\text{ ms}} = 8\text{ steps}$$
  If $H < 8$, the arm halts mid-motion every cycle while waiting on inference.
* **Spatial drift under open-loop execution ($H = 64$):** Over $H = 64$ steps ($T_{\text{chunk}} = 64 \times 0.02\text{ s} = 1.28\text{ s}$):
  $$\Delta x_{\text{drift}} = \frac{1}{2} (0.50\text{ m/s}^2)(1.28\text{ s})^2 \approx 0.410\text{ m} = 410\text{ mm}$$
  Drift exceeds the $15\text{ mm}$ tolerance by $27.3\times$, causing a violent collision and grasp failure.
* **Optimal receded-horizon execution:** The maximum allowable execution duration before drift breaches $15\text{ mm}$ is:
  $$T_{\max} = \sqrt{\frac{2 \Delta x_{\max}}{a_{\text{drift}}}} = \sqrt{\frac{2(0.015\text{ m})}{0.50\text{ m/s}^2}} \approx 0.245\text{ s} = 245\text{ ms} \implies H_{\max} = \frac{245\text{ ms}}{20\text{ ms}} = 12.25\text{ steps}$$
  To prevent starvation while bounding drift, the execution window $H_{\text{exec}}$ must satisfy $8 \le H_{\text{exec}} \le 12$ steps. Setting $H_{\text{exec}} = 10$ steps ($200\text{ ms}$, replanning frequency $f_{\text{replan}} = 5\text{ Hz}$) leaves a $40\text{ ms}$ inference safety margin while capping maximum drift at $\Delta x_{\text{drift}} = \frac{1}{2}(0.50)(0.20)^2 = 10.0\text{ mm} < 15\text{ mm}$.
* **Systems takeaway:** Long chunk horizons ($H=32\text{--}64$) stabilize feedforward motion, but policies must never execute them to completion; overlapping receded-horizon execution with temporal ensembling balances closed-loop freshness against actuator continuity.
:::
