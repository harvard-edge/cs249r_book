...
::: {#fig-02-latency-waterfall fig-env="figure" fig-pos="t" fig-cap="**Sense-to-Actuation Latency Waterfall and Silicon Dataflow Pipeline**: The eight-stage causal chain from photonic exposure to stator current rise (Panel A), evaluated against a $40.0\text{ ms}$ freshness deadline (Panel B). Nominal execution lands at $P_{50} = 35.0\text{ ms}$ with $4.2\text{ cm}$ unguided drift at $1.2\text{ m/s}$, while memory contention and kernel preemption blow out tail latency to $P_{99.9} = 68.4\text{ ms}$, expanding drift to $8.2\text{ cm}$ ($+4.0\text{ cm}$ blowout) and breaching safety clearance envelopes into mechanical collision (adapted from Brooks 1986 [@brooks1986robust])." fig-alt="Sense-to-actuation latency waterfall and silicon dataflow pipeline. Panel A illustrates the eight-stage causal hardware pipeline across CMOS photodiode exposure, MIPI CSI-2 readout, PCIe DMA transfer, kernel V4L2 queues, edge NPU policy inference, IPC shared SRAM mailbox, EtherCAT fieldbus, and gate driver stator current rise. Panel B charts the sense-to-actuation timelines comparing nominal P50 execution of 35.0 ms against a tail contention spike P99.9 of 68.4 ms relative to a 40.0 ms freshness deadline, showing how unmanaged tail latency expands unguided coasting drift from 4.2 cm to 8.2 cm at 1.2 m/s."}
...
For the physical AI systems engineer, this quadratic formulation transforms speed limits from static configuration constants into dynamic runtime safety invariants. Rather than assuming a fixed operational velocity in high-level software, the engineer must program the deterministic nervous system to continuously solve the clearance quadratic, dynamically clamping candidate policy proposals to $v_{\text{max}}(D_{\text{clear}}, \Delta t_{\text{delay}})$ on every control cycle. In general multi-degree-of-freedom robotic systems, runtime safety certification is formalized through Control Barrier Functions (CBF-QP) [@ames2019control], which guarantee set invariance assuming bounded compute latency and deterministic solver execution. For 1D longitudinal stopping on an autonomous mobile robot, this barrier condition reduces to the exact closed-form kinematic quadratic root derived above. When sensory range degrades, when compute latency spikes, or when environmental clearance shrinks, the nervous system automatically contracts the robot's velocity ceiling, guaranteeing that moving mass never enters a physical state where its momentum requires more stopping distance than the available space.
...
- [ ] **Zero-failure statistical confidence**: Can you explain why completing 3,000 test cycles without a single brownout only guarantees a failure rate bounded below roughly 1 in 1,000 at 95 percent confidence?
