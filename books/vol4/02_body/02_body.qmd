...
***Dynamic stopping distance***\index{Dynamic stopping distance!definition}\index{Stopping distance!definition} ($d_{\text{stop}}$) is the minimum physical clearance required to bring a moving mass to a complete halt from initial velocity $\mathbf{v}_0$ under total system reaction delay $\tau_{\text{react}}$ and bounded braking deceleration $a_{\text{floor}}$:
$$d_{\text{stop}} = \|\mathbf{v}_0\| \tau_{\text{react}} + \frac{\|\mathbf{v}_0\|^2}{2 a_{\text{floor}}} + \delta_{\text{overhead}}$$
where $\tau_{\text{react}} = \tau_{\text{trans}} + \tau_{\text{compute}} + \tau_{\text{comm}} + \tau_{\text{brake}}$ encompasses sensor transduction, neural inference, fieldbus transport, and mechanical brake onset, and $\delta_{\text{overhead}} = \delta_{\text{loc}} + \delta_{\text{margin}}$ represents spatial localization uncertainty and protective clearance margin.
...
2.  **Distinction**: Unlike static geometric obstacle clearance in classical kinematics, dynamic stopping distance scales quadratically with speed ($\|\mathbf{v}_0\|^2$), meaning a doubling of velocity quadruples required clearance regardless of controller loop rate.
...
from mlsysim.fmt import fmt_mass, fmt_length, fmt_energy, fmt_time, fmt_velocity, fmt_acceleration, fmt_display_math, check
...
- [ ] **Linear vs. quadratic scaling**: Can you explain why doubling compute latency expands the stopping envelope linearly while doubling travel speed expands it quadratically?
