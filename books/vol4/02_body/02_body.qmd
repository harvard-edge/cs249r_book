...
We begin in @sec-body-measurement-freshness by analyzing measurement freshness, tracing the causal delay from photon absorption on sensor silicon to motor current rise and proving why sensor data decays the instant physical transduction occurs.[^fn-body-transduction-clock] In @sec-body-kinetic-momentum, we derive kinetic momentum and stopping distance envelopes, showing why stopping distance scales quadratically with velocity and establishing dynamic speed limits. In @sec-body-actuator-limits, we investigate actuator bandwidth and transmission mechanics, demonstrating how gearboxes magnify rotor inertia quadratically ($N^2 J$) and choke mechanical acceleration. In @sec-body-thermal-duty-cycles, we model thermal duty cycles and Joule dissipation, showing how continuous motion generates heat that must be budgeted across cyclic operations. In @sec-body-power-integrity, we examine electrical power integrity, addressing transient voltage collapse and regenerative energy surges on shared direct-current buses. In @sec-body-measuring-limits, we replace bench datasheets with empirical testing protocols on fully assembled machines at the 99.9th percentile tail ($P_{99.9}$). Finally, in @sec-body-archetype-matrix, we synthesize these five budgets into a cross-archetype design matrix across mobile systems, manipulators, process plants, and humanoids.
