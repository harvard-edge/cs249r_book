...
::: {#fig-02-latency-waterfall fig-env="figure" fig-pos="t" fig-cap="**Sense-to-actuation latency waterfall and silicon dataflow pipeline**: The eight-stage causal chain from exposure to motor current rise (panel a) and an illustrative latency profile (panel b). The parcel sorter's $24\text{ mm}$ tolerance at $1.2\text{ m/s}$ sets a $20\text{ ms}$ age deadline, because unguided travel converts elapsed time into displacement at the carriage speed. Both the synthetic $35.0\text{ ms}$ median and $68.4\text{ ms}$ tail exceed it, corresponding to $4.2\text{ cm}$ and $8.2\text{ cm}$ of motion, respectively. These are scenario values, not measured hardware results." fig-alt="Two-panel diagram. Panel (a) shows eight stages from sensor exposure through transport, inference, command transfer, and motor current rise. Panel (b) shows illustrative 35.0 and 68.4 millisecond timelines, both crossing a 20 millisecond freshness deadline; at 1.2 meters per second they correspond to 4.2 and 8.2 centimeters of parcel motion."}
...
To illustrate how stage timing differs from end-to-end age, consider a synthetic timing profile for the camera-to-joint path under continuous inference load, judged against the same $20\text{ ms}$ deadline. The values in @tbl-02-latency-breakdown are constructed for this example; they are not recorded cycles or a characterized machine.
...
class BodyStoppingBudget:
    ...
    v_fast = 2.0 * meter / second  # scenario assumption: speed-scaling case above the drive limit
    dt_extra = 100 * millisecond  # scenario assumption: added pre-brake delay step
...
2.  **Distinction**: Unlike static geometric obstacle clearance in classical kinematics, dynamic stopping distance scales quadratically with speed ($v^2$), meaning a doubling of velocity quadruples required clearance regardless of controller loop rate.
...
S = BodyStoppingBudget
MM = ureg.millimeter
A_BRAKE = S.a_brake
OVERHEAD = S.overhead.to(MM).magnitude
D_CLEAR = S.d_clear.to(MM).magnitude
CASES = [
...
- $\tau_{\text{delay}} \in \mathbb{R}_{> 0}$: Conservatively bounded sense-to-braking delay, including actuator response $[\text{s}]$; a measured $P_{99.9}$ alone is insufficient.
...
1. Modeled stopping distance: $d_{\text{stop}}(v_{\text{safe}}) = v_{\text{safe}} \tau_{\text{delay}} + \frac{v_{\text{safe}}^2}{2 a_{\text{brake}}} + \delta_{\text{loc}} + \delta_{\text{margin}} \le D_{\text{clear}}$ under bounded delay, available braking, a stationary obstacle, and valid clearance estimates.
...
class DriveBusIntegrity:
    ...
    i_compute = 4.0 * ureg.ampere  # scenario assumption: application processor's share of the step
    l_bus = 2.0 * ureg.microhenry  # scenario assumption: harness inductance
    c_bus = 22000.0 * ureg.microfarad  # scenario assumption: bulk link capacitance
    v_breakdown = 40.0 * volt  # scenario assumption: inverter MOSFET rating
    dt_step = 0.5 * millisecond  # scenario assumption: step rise time
    regen_fraction = 0.75  # scenario assumption: share of E_k returned to the rail
    v_clamp = 30.0 * volt  # chosen: chopper clamp threshold
    r_candidate = 1.0 * ureg.ohm  # chosen: candidate chopper resistor
    p_candidate = 1.0 * ureg.kilowatt  # chosen: candidate resistor pulse rating
    ...
    v_final_str = fmt_voltage(v_final, unit=volt, precision=1)
...
3. **Regenerative surge**:
   ...
   - *Failure mechanism*: `{python} DriveBusIntegrity.v_final_str` is a hypothetical no-protection endpoint, not a predicted spike or time trace. The bus would cross the `{python} DriveBusIntegrity.v_breakdown_str` switch rating far earlier; protection or component failure changes the actual trajectory.
4. **Sizing brake chopper**:
   - Under constant deceleration the regenerative power falls linearly from its peak to zero over the stop, which lasts $t_{\text{stop}} = v / a_{\text{brake}} =$ `{python} DriveBusIntegrity.t_stop_str`. The area under that triangle is $E_{\text{regen}}$, so a $V_{\text{clamp}} =$ `{python} DriveBusIntegrity.v_clamp_str` design faces the illustrative peak:
...
**Systems lesson**: A physical body cannot transcend the saturation ranges of its sensory transducers. When sensor physical limits (such as gyroscope rate ceilings or accelerometer dynamic ranges) are exceeded, state estimators that naively integrate saturated measurements inject unbounded mathematical drift into physical coordinates. In Physical AI systems, software layers must treat sensor readings not as pure mathematical variables, but as physical measurements bounded by hardware saturation ranges, asserting state plausibility before commanding irreversible actuator actions.
:::
...
class MeltPoolContrast:
    ...
    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    ded = Embodied.ContinuousProcess.DED_MeltPool
    p_laser = ded.nominal_power  # category: illustrative
    f_control = ded.control_frequency  # category: illustrative
    t_stall = 100 * millisecond  # scenario assumption: controller stall with the laser on
    e_k_machine = MachineMomentum.e_k  # derived (running machine at its drive limit)
...
class ThrottledPerception:
    ...
    clock_ratio = 2 / 3  # scenario assumption: DVFS clock as a fraction of full clock
