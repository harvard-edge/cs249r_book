...
A moving machine cannot pause for software. During inference, its position changes and observations age. Stopping requires time to issue a command and distance to dissipate kinetic energy. The motor and transmission also limit how quickly a command can change motion, while electrical losses heat the windings during operation. Policies ignoring these constraints request impossible forces or unsustainable movements. Faster inference helps only within this physical path; it does not eliminate braking distance, mechanical response time, or thermal limits. These limits must be measured empirically under operational conditions. In physical AI systems architecture, the body sets the budgets that sensing, computation, and control must share, connecting each software decision to the motion and energy it produces.
...
@Sec-body-measurement-freshness traces the causal delay from photon absorption to motor current rise, showing why sensor data decays upon transduction.[^fn-body-transduction-clock] @Sec-body-kinetic-momentum derives stopping distance envelopes and dynamic speed limits. @Sec-body-actuator-limits demonstrates how gearboxes magnify rotor inertia ($N^2 J$) to choke mechanical acceleration. @Sec-body-thermal-duty-cycles models continuous Joule heating across cyclic operations. @Sec-body-power-integrity addresses transient voltage collapse and regenerative energy surges on shared DC buses. In @sec-body-measuring-limits, we replace bench datasheets with empirical testing protocols on fully assembled machines at the 99.9th percentile tail ($P_{99.9}$). Finally, in @sec-body-archetype-matrix, we synthesize these five budgets into a cross-archetype design matrix across mobile systems, manipulators, process plants, and humanoids.
...
Executing an actuation command from expired sensor data injects severe phase lag into the feedback loop, degrading phase margin and exciting resonant mechanical chatter—a limit-cycle oscillation where corrective torques reinforce rather than damp physical tracking errors.[^fn-math-bode-delay] High-frequency inference does not guarantee real-time responsiveness. The system must tag every observation with a hardware timestamp at physical transduction (using **IEEE 1588 gPTP**\index{IEEE 1588 gPTP!hardware timestamping}[^fn-hw-gptp-timestamp] or hardware strobe capture at the Ethernet PHY / **MIPI CSI-2**\index{MIPI CSI-2!sensor readout interface}[^fn-hw-mipi-capture] interface, rather than uncalibrated clocks like `time.now()`) and carry it through the pipeline. Immediately before committing an action setpoint to the motor registers, the execution runtime must compute the elapsed age $t_{\text{now}} - t_{\text{transduction}}$ and compare it against the physical freshness deadline $\Delta t_{\text{fresh}}$. When latency jitter pushes measurement age past this threshold, the nervous system must refuse the stale policy command and transition to a deterministic holding reflex [@brooks1986robust], protecting the mechanical plant from the destructive phase lag of outdated beliefs.
...
For the physical AI systems engineer, these electromechanical realities dictate two foundational design rules. First, raw neural network outputs must never be mapped directly to joint motor inverters without smooth continuous motion profiling; the real-time microcontroller must interpolate discrete action chunks through $C^2$-continuous splines (such as quintic polynomials or S-curve limiters) to bound commanded jerk and prevent current saturation against reflected rotor inertia. Second, closed-loop safety verification cannot rely solely on motor-side encoders; To ensure system integrity, the engineer must place absolute encoders and torque sensors directly on the output load side of the transmission. This placement allows for the immediate detection of silent mechanical failures. Silent mechanical failures refer to issues that occur without obvious signs, such as flexspline tooth fractures (where the teeth of the spline can break under stress), shaft shear (where the shaft can break due to excessive torque), or transmission backlash (the slight movement that occurs when the direction of the drive is reversed). Monitoring these parameters is critical to intercepting failures before they can damage the workcell.
