...
::: {.content-visible when-format="pdf"}
\begin{marginfigure}
\resizebox{0.92\linewidth}{!}{\mlphysicalstackfull{20}{20}{100}{20}{20}}
\caption{The five-level machine stack with the Body level highlighted.}
\end{marginfigure}
:::
...
from mlsysim.fmt import fmt_mass, fmt_velocity, fmt_energy, check
...
The distance from sensing a hazard to zero velocity comprises two phases: blind travel and braking travel. During the pre-brake delay, the machine runs the pipeline of @sec-body-measurement-freshness. Throughout this delay $\tau_{\text{delay}}$, which runs from the exposure of the frame that shows the hazard until retarding force begins, the actuators apply no retarding force, and the vehicle continues moving along its existing trajectory at its speed $v$ when that frame is exposed. Assuming velocity remains constant across this window, the blind travel evaluates to
...
The differing exponents of the blind-travel and braking terms make velocity the dominant multiplier of spatial risk (@fig-02-stopping-distance), a scaling that \ref{nbk-body-stopping-distance-budgeting-high-tail-compute} works numerically.
...
for i, (name, sub, react, brake, stop) in enumerate(rows):
    ...
    for width, color, label in (
        (react, COLORS["BlueL"], "Blind travel"),
        (brake, COLORS["BlueLine"], "Braking"),
        (OVERHEAD, COLORS["grid"], "Overhead"),
    ):
...
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import fmt_int, fmt_mass, fmt_length, check
...
class ReflectedInertia:
    ...
    m_rotor_str = fmt_mass(m_rotor, unit=ureg.gram, precision=0)
    m_fly_str = fmt_mass(m_fly, unit=kilogram, precision=1)
...
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import fmt_force, fmt_force_rate, fmt_sci_qty, fmt_velocity, check
...
class LatchContactRate:
    ...
    rate_guarded_str = fmt_force_rate(rate_guarded, unit=ureg.newton / millisecond, precision=0)
    rate_high_str = fmt_force_rate(rate_high, unit=ureg.newton / millisecond, precision=0)
...
This architectural divide across transmission ratios dictates the mechanical transparency and specific torque envelope of the embodiment (@fig-02-actuator-torque-density-vs-gear-ratio). In direct-drive joints ($N = 1:1$), such as the seminal CMU Direct Drive Arm [@asada1983design], reflected inertia is negligible ($N^2 = 1$), eliminating backlash and enabling perfect mechanical backdrivability. However, because electric motors produce torque in proportion to stator copper volume and gap radius, direct-drive actuators rarely exceed $5\,\text{N}\cdot\text{m/kg}$ of continuous specific torque density, burdening manipulators with heavy, bulky linkages. Conversely, classical industrial arms (such as the KUKA LBR iiwa or Universal Robots UR5) and high-load industrial machinery employ high-reduction strain wave or cycloidal gearboxes ($N = 100{:}1\text{ to }160{:}1$). While high gearing boosts specific continuous torque density to $22\text{--}45\,\text{N}\cdot\text{m/kg}$, it inflicts an extreme reflected rotor inertia penalty ($N^2 \ge 2{,}500\text{ to }25{,}600$), rendering joints virtually non-backdrivable. Under unexpected contact or obstacle collision, the kinetic energy of the rapidly spinning rotor cannot backdrive the motor; instead, stress concentrates at the gear root fillet, risking catastrophic tooth shear before digital torque loops can intervene.

Modern dynamic physical AI platforms resolve this impasse through **quasi-direct drive (QDD)** actuation [@seok2015design; @wensing2017proprioceptive; @katz2019mini]. By pairing high-torque-density frameless outrunner brushless motors with low single-stage planetary reductions ($N \le 10{:}1$), QDD architectures cap reflected inertia multiplier below $N^2 \le 100$. This preserves mechanical transparency, allowing motor phase currents measured at the inverter to serve as accurate, low-latency proxies for contact force—a property termed **proprioceptive sensing**. As illustrated by quadruped and humanoid milestones including the MIT Cheetah 3, MIT Mini Cheetah, Unitree Go1/H1, and Boston Dynamics Electric Atlas, QDD actuators achieve peak intermittent torque densities up to $70\text{--}95\,\text{N}\cdot\text{m/kg}$ while safely absorbing impact shocks during explosive dynamic maneuvers.

::: {#fig-02-actuator-torque-density-vs-gear-ratio fig-env="figure" fig-pos="htb" fig-cap="**Actuator dynamics: specific torque density versus transmission gear ratio in robotic bodies**: Trade-off between delivered mechanical torque density (continuous thermal $S_1$ rating vs. peak intermittent burst) and reflected rotor inertia multiplier ($J_{\\text{refl}} / J_{\\text{rotor}} = N^2$, right axis, log scale) across transmission regimes. Regime I: Direct Drive ($N=1{:}1$, 100 percent backdrivable, low torque density $<5\\,\\text{N}\\cdot\\text{m/kg}$, e.g. CMU Direct Drive Arm 1983, Maxon EC90). Regime II: Quasi-Direct Drive (QDD) / Proprioceptive sweet spot ($N \\le 10{:}1$, $N^2 \\le 100$, continuous $15\\text{--}35\\,\\text{N}\\cdot\\text{m/kg}$, peak up to $95\\,\\text{N}\\cdot\\text{m/kg}$, transparent contact sensing via motor current, e.g. MIT Cheetah 3, Mini Cheetah, Unitree Go1/H1, Boston Dynamics Electric Atlas). Regimes III & IV: High-reduction Stiff Industrial (Harmonic/Cycloidal) and Series Elastic Actuators ($N \\ge 50{:}1$, $N^2 \\ge 2{,}500\\text{--}25{,}600$, non-backdrivable, impact shock fragile, e.g. UR5, Franka Emika Panda, KUKA LBR iiwa, ANYdrive, Nabtesco RV)." fig-alt="Log-log plot of actuator specific torque density from 0 to 135 N·m/kg against transmission gear ratio from 1:1 to 250:1. Three shaded regimes highlight Direct Drive, Quasi-Direct Drive (N<=10), and High-Reduction Industrial (N>=50). Data points display continuous and peak torque envelopes for historical and modern robotic actuators alongside an exponential reflected inertia curve scaling as N squared on the right axis."}

```{python}
#| label: fig-02-actuator-torque-density-vs-gear-ratio
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ ACTUATOR DYNAMICS: TORQUE DENSITY VS GEAR RATIO (FIGURE)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Section 2.5, @fig-02-actuator-torque-density-vs-gear-ratio.
# │ Goal:    Show the physical trade-off between delivered specific torque density
# │          and reflected rotor inertia (N^2), contrasting stiff industrial
# │          reducers with backdrivable Quasi-Direct Drive (QDD) actuators.
# └─────────────────────────────────────────────────────────────────────────────
...
ax.text(
    1.30,
    regime_y,
    "REGIME I: DIRECT DRIVE\nBackdrivable · Zero Backlash\nLow Torque (<5 Nm/kg)",
    ha="center",
    va="center",
    fontsize=8.0,
    fontweight="bold",
...
def add_badge(ax, text, xy, xytext, color="#1E293B", fontsize=7.6, arrow=True, arrow_color="#94A3B8"):
...
actuators = [
    ...
    },
    {
        ...
        "color": "#A51C30",
        "marker": "^",
        "badge_xytext": (125.0, 88.0),
        "text": "Nabtesco RV Cycloidal\nN = 161:1 · Cont: 45 Nm/kg (Peak: 118)\nPrecision robotics reducer; stiff & heavy",
    },
]

for act in actuators:
    ax.plot([act["plot_N"], act["plot_N"]], [act["cont"], act["peak"]], color=act["color"], linewidth=2.0, alpha=0.6, zorder=5)
    ax.scatter(act["plot_N"], act["cont"], color=act["color"], s=80, marker=act["marker"], edgecolor="#FFFFFF", linewidth=1.2, zorder=6)
    ax.scatter(act["plot_N"], act["peak"], color="#FFFFFF", s=90, marker=act["marker"], edgecolor=act["color"], linewidth=2.0, zorder=6)
    ax.scatter(act["plot_N"], act["peak"], color=act["color"], s=18, marker="o", zorder=7)
    add_badge(
        ax,
        act["text"],
        xy=(act["plot_N"], (act["cont"] + act["peak"]) / 2.0),
        xytext=act["badge_xytext"],
        color="#1E293B",
        fontsize=7.6,
        arrow=True,
        arrow_color=act["color"],
    )
...
ax2.get_yaxis().set_major_formatter(ticker.FuncFormatter(lambda y, p: f"{int(y):,}×"))
ax2.set_ylabel(
    "Reflected Rotor Inertia Multiplier $J_{\\mathrm{refl}} / J_{\\mathrm{rotor}} = N^2$ (log scale)",
    fontsize=10,
    fontweight="bold",
    color="#64748B",
    labelpad=10,
...
ax.get_xaxis().set_major_formatter(ticker.FuncFormatter(lambda x, p: f"{int(x)}:1"))
...
fig.suptitle(
    "Actuator Dynamics: Torque Density vs Gear Ratio in Robotic Bodies", fontsize=14, fontweight="bold", x=0.08, y=0.98, ha="left", color="#1E293B"
)
ax.set_title(
    "Mechanical transparency trade-off: stiff industrial reducers ($N^2 \\geq 2,500$) vs backdrivable Quasi-Direct Drive (QDD) actuators ($N^2 \\leq 100$)",
    fontsize=9.5,
    color="#475569",
    loc="left",
    pad=14,
...
legend_elements = [
    ...
    Line2D(
        ...
        markeredgewidth=1.8,
        markersize=9,
        label='Peak Intermittent Torque Density (Transient / Jump)',
    ),
...
legend = ax.legend(
    ...
    edgecolor="#CBD5E1",
    fontsize=8.0,
    title="Actuator Mechanical Metrics",
    title_fontsize=8.5,
)
...
class DriveBusIntegrity:
    ...
    l_bus_str = fmt_inductance(l_bus, unit=ureg.microhenry, precision=0)
    c_bus_str = fmt_capacitance(c_bus, unit=ureg.microfarad, precision=0, commas=True)
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: the zero-failure paragraph in @sec-body-measuring-limits.
# │ Goal:    Bound the per-stop failure rate that n failure-free loaded stops
# │          support at 95 percent confidence.
# │ Show:    n = 3,000; -ln(0.05) = 2.996 (the rule of three); upper limit
# │          9.99e-4, about one stop in 1,000.
...
The absence of a failure during testing bounds the failure rate statistically; it does not establish safety. If the mobile manipulator completes $n =$ `{python} ZeroFailureBound.n_str` loaded stops without a brownout or an overrun, the data support only a per-stop failure rate below roughly $3/n$ (@sec-evaluation-closed-loop-survival), about one stop in `{python} ZeroFailureBound.one_in_str` at `{python} ZeroFailureBound.conf_pct_str` confidence, and only if the trials are independent and stationary. Windings that heat across successive trials and gears that wear make consecutive trials correlated and non-stationary, so the same stops run on a cool morning say nothing about the afternoon, when a hotter winding has higher resistance and less peak torque.
...
class LimitRecords:
    ...
    c_store_str = fmt_capacitance(c_store, unit=ureg.farad, precision=3)
...
: **Cross-class physical constraint matrix**: The five budgets, the law that governs each, the assumption a software-trained policy makes about it, the class it binds first, and the runtime invariant that answers it. {#tbl-02-cross-archetype-matrix tbl-colwidths="[18,20,24,18,20]"}
...
An engineer uses a motor's continuous torque rating to set the planner's limits. The rating carries the vendor's free-air test conditions (@sec-body-thermal-duty-cycles), not those of the sealed joint beside the compute unit, so it overstates what the installed motor can sustain. Because the planner treats that rating as its torque limit, every trajectory it later generates inherits the error, and the fault surfaces only after the enclosure has heat-soaked, far from the design decision that caused it. Planning limits must derive from measurements on the fully assembled machine under expected environmental and cooling conditions.
