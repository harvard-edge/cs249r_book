...
from mlsysim.fmt import fmt_latency, fmt_length, fmt_force, fmt_force_rate, fmt_qty, check
...
class IntentLeaseArchetypes:
    ...
    f_at_brake_onset_n_str = fmt_force(f_at_brake_onset, unit=newton, precision=0)
    delta_x_unbounded_mm_str = fmt_length(delta_x_unbounded, unit=millimeter, precision=0)
    f_unbounded_n_str = fmt_force(f_unbounded, unit=newton, precision=0, commas=True)
...
***Expiring intent lease***\index{Expiring intent lease!definition}\index{Intent lease!definition} ($\mathcal{L}_{\text{intent}}$) is a time-bounded execution authorization issued by the independent permission tier that binds an admitted target pose and effort envelope to a strict monotonic hardware deadline ($\tau_{\text{lease}}$):
