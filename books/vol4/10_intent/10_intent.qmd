...
Memory can supply a spatial belief with a validity window (@sec-memory); it cannot decide what action that belief authorizes. **Intent**\index{Intent} closes that gap by connecting high-level task semantics to a bounded physical goal. It specifies an admissible, time-bounded target before the trajectory planner of @sec-planning constructs a continuous motion profile to reach it.
...
If a validated disturbance acceleration bound $a_{\text{dist}}$ is also needed, the growth law of @sec-memory-belief-through-occlusion becomes $e_0+v_{\text{drift}}t+\frac12a_{\text{dist}}t^2$ in this chapter's notation, and its positive root at $r_{\text{tol}}$ shortens the evidence horizon.
