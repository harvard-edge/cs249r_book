...
# │ Local assumptions; outputs feed the stall contrast and the notebook below.  │
# └─────────────────────────────────────────────────────────────────────────────┘
...
# Mobile manipulator stall contrast: protected pre-brake delay vs. a stall.
v_base_mob = 1.2 * meter / second  # scenario assumption
a_brake_mob = 3.0 * meter / second**2  # scenario assumption
tau_delay_protected_mob = 150.0 * millisecond  # scenario assumption
tau_delay_stall_mob = 400.0 * millisecond  # scenario assumption
...
d_stop_protected_mob = calc_safe_stopping_distance(v_base_mob, tau_delay_protected_mob, a_brake_mob).to(millimeter)
d_stop_stall_mob = calc_safe_stopping_distance(v_base_mob, tau_delay_stall_mob, a_brake_mob).to(millimeter)
...
check(abs(d_stop_protected_mob.magnitude - 420.0) < 0.5, "Protected 150 ms stop")
check(abs(d_stop_stall_mob.magnitude - 720.0) < 0.5, "400 ms stall stop")
...
class IntentLeaseArchetypes:
    d_stop_protected_mm_str = fmt_length(d_stop_protected_mob, unit=millimeter, precision=0)
    d_stop_stall_mm_str = fmt_length(d_stop_stall_mob, unit=millimeter, precision=0)
...
Omitting any of these three bounds leaves a machine exposed during communication dropouts or perception stalls. Stopping distance grows linearly with the delay before braking and quadratically with speed (@eq-body-stopping-distance). The intent lease is one term inside that pre-brake delay, so an admitted lease must be shorter than the delay budget by the detection, communication, and actuator delays that follow its expiry. For an illustrative mobile manipulator at $1.2\text{ m/s}$ with $3.0\text{ m/s}^2$ of available deceleration, a $150\text{ ms}$ total pre-brake delay stops the base within `{python} IntentLeaseArchetypes.d_stop_protected_mm_str`, while a $400\text{ ms}$ unserviced stall at constant speed stretches the stop to `{python} IntentLeaseArchetypes.d_stop_stall_mm_str`. Clearance holds only if an independently checked obstacle boundary lies beyond the protected distance, with model and sensing margins added.
