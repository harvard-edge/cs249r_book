...
Continuous tracking requires *fresh physical evidence*, not merely a new packet from the same stale frame. A renewal based on a new capture must arrive before the current evidence and admitted authority expire, with capture, processing, transfer, clock uncertainty, and permission time included in its budget. The simple condition $T_{\text{period}}+t_{\text{latency}}<\tau_{\text{evidence}}$ is only a necessary scheduling check. When a fresh proposal arrives, the planner may compute a jerk-limited splice\index{Trajectory Generation!jerk-limited splice} from the measured state. It admits that splice only if the swept path, new evidence, and stopping reserve remain feasible; an abrupt target change can instead require refusal or a checked stop.
...
The intent lease takes its lineage from memory. Its parent is the belief record of @sec-memory-spatial-state-schema, named by digest, and that record's evidence epoch, converted to the safety MCU's clock, becomes the lease's evidence time. The lease itself carries a goal region, an error envelope and task tolerance, a requested wrench, a requested terminal mode, an evidence horizon, and an expiry. It is a lease in the sense of @sec-nervous-cadences, here on a target, and the proposal that requests it travels under the proposal header with an intent payload in place of a chunk. An admitted intent grants a target and effort ceilings, never a path.

::: {.column-margin .margin-pointer}
↳ **Byte layout:** The intent payload's fields, types, and offsets are given in @sec-appendix-systems-wire-intent.
:::

On the running machine the proposal travels from the application processor to the safety MCU as a fixed-size, statically parsed message in a bounded shared mailbox,[^fn-intent-intercore-channel] so its parse time can be measured against the control deadline. The message only transports a request; the independent permission controller owns the configured force ceilings, deadline ceilings, and state-dependent fallback.
...
$$t_{\text{exp}} = \min(t_{\text{evidence}} + \tau_{\text{evidence}}, \; t_{\text{issue}} + \tau_{\text{lease}}, \; t_{\text{stop\_deadline}})$$
...
2.  **Distinction**: Unlike a network keepalive or software ping, an intent lease couples the horizon of physical sensor evidence ($\tau_{\text{evidence}}$) with dynamic braking feasibility, and its expiry falls no later than the stop deadline at which a validated fallback still fits the remaining clearance.
...
- [ ] **Clock domain**: Why must the safety MCU convert the belief record's evidence epoch into its own monotonic clock, with a bounded conversion error, before checking age and the requested expiry?
- [ ] **Fallback selection**: Can you explain why the safety MCU must select a feasible approved hold, deceleration, or torque-disable response from the measured load state rather than obey the proposal's requested terminal mode?
