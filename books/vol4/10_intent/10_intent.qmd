...
The pick admitted in @sec-intent-lease takes the first course, resting on the belief record of @sec-memory-spatial-state-schema, whose closer wrist-camera capture on the approach bounds the initial error at `{python} IntentLeaseArchetypes.e0_str`. A 95 percent region also leaves a residual tail and cannot by itself certify clearance, so the intent proposal carries conservative base-frame error bounds rather than claiming to encode this covariance.
