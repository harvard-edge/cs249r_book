...
Each mistake in this section trusts an upstream message to arrive, or to mean what it says. Local checks\index{Local checks!authority withdrawal} must withdraw tracking authority when the goal's temporal validity window expires or its physical justification dissolves, even when no status updates or abort packets arrive from the application processor.
