...
| **`complete_outcome_ledger`** | Full-denominator raw outcome counts: $n_{\text{attempted}}$, $n_{\text{success}}$, $n_{\text{estop}}$, $n_{\text{timeout}}$, $n_{\text{supervisor}}$ (permission-path overrides), and $T_{\text{TTI}}$ time-series | Monotonic microsecond hardware timestamps, uninterrupted ring-buffer telemetry          | Preventing censored trial fallacy; calculating uncorrupted empirical reliability                               |
