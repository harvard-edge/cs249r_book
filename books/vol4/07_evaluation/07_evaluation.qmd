...
# │       values the chapter quotes: approach speed, tripwire, latch limit, rack
...
from mlsysim.fmt import fmt_length, fmt_velocity, fmt_latency, fmt_force, fmt_mass, fmt_frequency, check, MarkdownStr
...
class EvalMachine:
    ...
    f_latch_limit = AISLE.f_latch_limit  # category: illustrative; latch damage limit
    f_trip = AISLE.f_latch_tripwire  # category: illustrative; contact tripwire that ends an attempt (@sec-data)
    ...
    check(abs(f_trip.to(newton).magnitude - 15.0) < 1e-9, "Contact tripwire is not 15 N")
    check(f_trip < f_latch_limit, "The tripwire must end an attempt well below the damage limit")
    ...
    f_latch_limit_str = fmt_force(f_latch_limit, precision=0)
    f_trip_str = fmt_force(f_trip, precision=0)
    rack_capacity_str = fmt_mass(rack_capacity, unit=kilogram, precision=0)
...
class ClopperPearsonZeroFailure:
    ...
    n_looks = 5  # category: illustrative; independent 5 percent looks in the optional-stopping example
    ...
    p_any_false_positive = 1.0 - (1.0 - alpha) ** n_looks  # chance of at least one false positive across independent looks
    ...
    check(abs(p_any_false_positive - 0.2262) < 0.001, "Five independent 5 percent looks should give about 22.6 percent")
    ...
    _words = {5: "five", 8: "eight"}
    n_one_in_str = MarkdownStr(_words[n_one_in])
    ...
    n_looks_str = MarkdownStr(_words[n_looks].capitalize())
    p_any_false_positive_pct_str = fmt_percent(p_any_false_positive, precision=1)
```
...
**Problem**: The mobile manipulator completes $n =$ `{python} ClopperPearsonZeroFailure.n_runs_str` physical door-opening runs with zero contact-tripwire crossings or human interventions ($k =$ `{python} ClopperPearsonZeroFailure.n_runs_str`). What is the exact one-sided `{python} ClopperPearsonZeroFailure.confidence_pct_str` percent lower confidence bound on its true operational success probability $p$?
...
The true success probability under this evaluation protocol is the expected value of the binary task outcome averaged over the joint distribution of trial conditions, $p(\mathcal{D}) = \mathbb{E}_{c \sim \mathcal{D}}[\phi(\text{Traj}(c))]$ (@sec-appendix-ml-trajectory-distribution gives the formal integral).
Here $\text{Traj}(c)$ is the continuous physical trajectory produced when the policy $\pi_\theta$ executes within the physical environment and hardware state defined by $c$. This dependency demonstrates why success probability cannot be treated as a measurement of $\theta$ alone. The model weights $\theta$ constitute only one parameter inside the condition vector $c$. If the distribution over floor friction or the distribution over initial battery charge changes, the expected success rate changes while $\theta$ remains identical. The sampling protocol is not an external apparatus measuring an isolated model; it is part of the mathematical quantity being estimated.
...
3.  **Common pitfall**: Reporting a single success rate percentage without predeclaring the support, density, and environmental bounds of the test distribution $\mathcal{D}$. Any physical policy can be made arbitrarily successful by testing exclusively in benign, unperturbed states.
...
3. *Full-Denominator Accounting:* Every initiated run that aborts due to hardware faults, communication timeouts, or safety-rated emergency stops (e.g., exceeding the collaborative-robot power and force limits of ISO 10218\index{ISO 10218} / ISO/TS 15066\index{ISO/TS 15066}) must be recorded as a failure ($Y = 0$). Dropping aborted trials substitutes a measure of conditional execution on surviving hardware for the true operational reliability of the machine.
...
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import fmt, fmt_length, fmt_velocity, fmt_time, fmt_angle, fmt_math, sci_latex, check
...
class AisleDrift:
    ...
    eps_rad_str = fmt_angle(eps_q, precision=5)
...
Offline scores can also mislead in the pessimistic direction when the task admits several valid actions. If the demonstrator passed an obstacle on the left, an open-loop score penalizes a candidate that passes on the right (the two-mode case of @sec-training-synthesis-regimes), although in closed loop both reach the goal. An action-level error measures divergence from one recording, not satisfaction of the task.
...
<!-- fig-alt below hand-types values because fig-alt cannot hold inline python. Keep them in sync with these cell outputs:
     ...
     unicycle paragraph, and the takeaways; the guard pins it.
...
|:-----------------------------------|:-----------------------------------------------------------------|:---------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------|
| **Action mean squared error**      | Small residuals give small path deviations                       | A persistent bias integrates into drift and carries the machine off the demonstrated states        | The base spends its rack clearance (stop or strike)                    | Peak deviation from the reference path; time to intervention\index{Time-to-Intervention!metric} ($T_{\text{TTI}}$) |
...
: **Evaluation evidence regimes compared**: Trade-offs between causal feedback coupling, environmental realism, supported empirical claims, and experimental boundary distortions across the four testing regimes. No single environment evaluates open deployment; rigorous certification requires combining complementary regimes rather than assuming a linear fidelity ladder. {#tbl-07-evidence-regimes tbl-colwidths="[20,20,20,20,20]"}

The quantitative relationship between simulated proxy benchmarks and real-world robot performance is governed by the **sim-to-real proxy transfer law** (@fig-evaluation-sim-to-real-proxy-transfer). While offline open-loop metrics (such as action Mean Squared Error) fail to predict closed-loop outcomes ($r = 0.308$, suffering severe ranking inversions as detailed in @tbl-07-offline-closed-loop-disconnect), closed-loop simulation benchmarks such as SIMPLER [@li2024simpler] demonstrate a strong linear correspondence with physical manipulation success across embodiments ($R^2 = 0.836$, Pearson $r = 0.914$, $p < 0.0001$; Spearman rank correlation $\rho = 0.942$).
...
1. **Visual domain gap**: Specular lighting miscalibrations or rendering discrepancies can disrupt vision transformer tokenizers. For example, untuned visual textures in Octo-Base collapsed simulated solve rate to $0.0\%$ despite achieving $29.3\%$ real-world task success.
...
3. **Tactile and force gap**: Fine-grained manipulation (such as sub-millimeter peg insertion) yields high visual solve rates in simulation ($82.0\%$), but collapses in reality ($14.0\%$) where vision alone cannot detect contact force wedging.
...
::: {#fig-evaluation-sim-to-real-proxy-transfer fig-env="figure" fig-pos="htb" fig-cap="**The sim-to-real proxy transfer law: simulation benchmark solve rate versus physical robot success rate**: Paired empirical evaluations across Google Robot (Everyday Robots) and WidowX embodiments from the SIMPLER benchmark [@li2024simpler]. Nominal policies demonstrate a tight linear transfer ($R^2 = 0.836$, Pearson $r = 0.914$, Spearman rank $\\rho = 0.942$, MMRV $= 0.027$), in sharp contrast to offline supervised action MSE ($r = 0.308$). Three marked diagnostic outliers illustrate the reality gap failure modes: visual rendering miscalibration (square), contact friction jamming (triangle), and tactile force absence (diamond)." fig-alt="Scatter plot comparing simulation benchmark solve rate from 0 to 100 percent on the horizontal axis against physical real-world robot success rate from 0 to 100 percent on the vertical axis. A blue linear regression fit with a shaded 95% confidence interval shows strong correlation for nominal models, alongside three highlighted diagnostic outliers representing visual domain gap, contact mechanics gap, and tactile force gap."}

```{python}
#| label: fig-evaluation-sim-to-real-proxy-transfer
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ SIM-TO-REAL PROXY TRANSFER LAW: BENCHMARK VS. PHYSICAL ROBOT PERFORMANCE
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Section 7.2, @fig-evaluation-sim-to-real-proxy-transfer.
# │ Goal:    Show empirical relationship between simulation and real-world success:
# │          1. Strong linear transfer (R^2 = 0.836, r = 0.914, rho = 0.942).
# │          2. Superiority over offline action MSE (r = 0.308).
# │          3. Three critical physical gaps: Visual, Contact Mechanics, Tactile.
# │ Sources: SIMPLER benchmark (Li et al., CoRL 2024 / arXiv:2405.05941).
# └─────────────────────────────────────────────────────────────────────────────
...
fig, ax = plt.subplots(figsize=(14.0, 8.2), dpi=200, facecolor="#FFFFFF")
...
models = [
    {"name": "RT-2-X (Google Robot)", "sim": 78.7, "real": 90.7, "cat": "nominal", "color": "#004B87", "badge_pos": (64, 102)},
    {"name": "RT-1 (Converged)", "sim": 85.7, "real": 85.3, "cat": "nominal", "color": "#004B87", "badge_pos": (93, 74)},
    {"name": "RT-1 (15% Checkpoint)", "sim": 71.0, "real": 92.0, "cat": "nominal", "color": "#004B87", "badge_pos": (50, 92)},
    {"name": "RT-1-X (Google Robot)", "sim": 56.7, "real": 76.0, "cat": "nominal", "color": "#004B87", "badge_pos": (38, 80)},
    {"name": "RT-1 (Single-Task Pick Can)", "sim": 40.3, "real": 68.0, "cat": "nominal", "color": "#004B87", "badge_pos": (24, 70)},
    {"name": "Octo-Small (Bridge Suite Avg)", "sim": 58.1, "real": 47.9, "cat": "nominal", "color": "#008080", "badge_pos": (68, 56)},
    {"name": "Octo-Base (Bridge Suite Avg)", "sim": 39.1, "real": 34.4, "cat": "nominal", "color": "#008080", "badge_pos": (44, 23)},
    {"name": "RT-1-X (Bridge Suite Avg)", "sim": 12.0, "real": 8.3, "cat": "nominal", "color": "#475569", "badge_pos": (14, 2)},
    {"name": "RT-1 (Begin / Untrained)", "sim": 2.7, "real": 13.3, "cat": "nominal", "color": "#475569", "badge_pos": (8, 5)},
    {"name": "Octo-Base (Untuned Arm Visuals)", "sim": 0.0, "real": 29.3, "cat": "visual_gap", "color": "#A51C30"},
    {"name": "RT-1-X (Top Drawer Jamming)", "sim": 89.1, "real": 40.7, "cat": "contact_gap", "color": "#D97706"},
    {"name": "Vision Policy (Sub-mm Peg Insertion)", "sim": 82.0, "real": 14.0, "cat": "tactile_gap", "color": "#6B21A8"},
]
...
ax.fill_between(x_fit, y_fit - ci, y_fit + ci, color="#0284C7", alpha=0.10, zorder=1, label="95% Linear Regression CI")
ax.plot([0, 100], [0, 100], color="#94A3B8", linestyle=":", linewidth=1.5, zorder=2, label="Ideal Zero Gap ($y = x$)")
ax.plot(x_fit, y_fit, color="#0284C7", linestyle="-", linewidth=2.5, zorder=3, label=f"Sim-to-Real Law ($y = {slope:.2f}x + {intercept:.1f}\\%$)")
...
for m in models:
    ...
    ax.scatter(m["sim"], m["real"], s=size, marker=marker, color=m["color"], edgecolors="#FFFFFF", linewidth=edge_width, zorder=6)
...
def add_badge(ax, text, xy, xytext, color="#1E293B", fontsize=8.0, arrow=True):
    ...
    ap = dict(arrowstyle="->", color=color, linewidth=1.0, shrinkA=2, shrinkB=3) if arrow else None
    ax.annotate(
...
add_badge(ax, "RT-2-X (Google Robot)\nSim: 78.7% | Real: 90.7%", (78.7, 90.7), (64, 102), color="#004B87")
add_badge(ax, "RT-1 (Converged)\nSim: 85.7% | Real: 85.3%", (85.7, 85.3), (93, 74), color="#004B87")
add_badge(ax, "RT-1 (15% Checkpoint)\nSim: 71.0% | Real: 92.0%", (71.0, 92.0), (50, 92), color="#004B87")
add_badge(ax, "RT-1-X (Google Robot)\nSim: 56.7% | Real: 76.0%", (56.7, 76.0), (38, 80), color="#004B87")
add_badge(ax, "Octo-Small (Bridge)\nSim: 58.1% | Real: 47.9%", (58.1, 47.9), (68, 56), color="#008080")
add_badge(ax, "Octo-Base (Bridge)\nSim: 39.1% | Real: 34.4%", (39.1, 34.4), (44, 23), color="#008080")
add_badge(ax, "RT-1 (Begin)\nSim: 2.7% | Real: 13.3%", (2.7, 13.3), (8, 5), color="#475569")
...
fig.text(
    0.08,
    0.958,
    "The Sim-to-Real Proxy Transfer Law: Simulation vs. Physical Robot Performance",
    fontsize=13.5,
    fontweight="bold",
    color="#1E293B",
    va="top",
...
custom_handles = [
    ...
    handles[0],
    handles[1],
    Line2D([0], [0], marker="o", color="w", markerfacecolor="#004B87", markersize=7.5, label="Nominal (Google Robot / RT)"),
    Line2D([0], [0], marker="o", color="w", markerfacecolor="#008080", markersize=7.5, label="Nominal (WidowX / Octo)"),
    Line2D([0], [0], marker="s", color="w", markerfacecolor="#A51C30", markersize=7.5, label="Visual Gap Outlier"),
    Line2D([0], [0], marker="^", color="w", markerfacecolor="#D97706", markersize=7.5, label="Contact Gap Outlier"),
    Line2D([0], [0], marker="D", color="w", markerfacecolor="#6B21A8", markersize=7.5, label="Tactile Gap Outlier"),
]
legend = ax.legend(
...
A zero-failure campaign is sized by inverting the exact bound of @sec-evaluation-closed-loop-survival for the trial count, so the claim, not the test budget, fixes how many runs the test cell must deliver (@fig-07-confidence-bounds and @tbl-07-confidence-bounds; derivations in @sec-appendix-ml-exposure).

::: {#nbk-evaluation-statistical-scaling-high-reliability-assurance .callout-notebook title="Statistical scaling of assurance"}
**Problem**: Suppose an evaluation target calls for a one-sided success lower limit of $p_{\text{target}} = 0.999$ at 95 percent confidence ($\alpha = 0.05$). If each door trial requires $T_{\text{trial}} =$ `{python} DoorCycle.t_cycle_str`, how many zero-failure runs and how much wall-clock testing time are required?
...
Butler and Finelli drew the design conclusion from this arithmetic [@butler1993infeasibility] (\ref{psp-evaluation-untestable-reliability}). A fleet does not rescue the test, because `{python} ExposureWall.fleet_rigs_str` rigs running continuously would still need about `{python} ExposureWall.yr_e9_fleet_str` calendar years at $10^{-9}$ per hour. Road-vehicle mileage meets the same wall (@fig-07-av-miles-scaling), and simulated miles add no physical exposure to it. A high-assurance claim therefore rests on independently validated runtime constraints and structured fault injection (@sec-verification-deriving-fault-list).[^fn-std-iso-asil]
...
_csv = (
    "vol4/07_evaluation/data/autonomous_miles_scaling.csv"
    if os.path.exists("vol4/07_evaluation/data/autonomous_miles_scaling.csv")
    else "data/autonomous_miles_scaling.csv"
)
data = pd.read_csv(_csv, comment="#")
...
class MtbfCampaign:
    ...
    l10_math = fmt_math(sci_latex(l10_rev, precision=1))
    p_fail_mission_math = fmt_math(sci_latex(p_fail_mission, precision=1))
```

The exposure wall binds even at a site requirement far looser than those rates. Suppose the warehouse mobile manipulator must demonstrate a critical failure rate $\lambda \le 1.0\times 10^{-4}\text{ failures/hour}$ (Mean Time Between Critical Failures $\text{MTBF} \ge 10{,}000\text{ hours}$) at 95 percent statistical confidence ($\alpha = 0.05$). Each retrieval mission takes $t_{\text{mission}} =$ `{python} MtbfCampaign.t_mission_str` (`{python} MtbfCampaign.t_mission_h_str`) and covers `{python} MtbfCampaign.d_mission_str` at the `{python} MtbfCampaign.v_str` drive limit. Automated dock recharge and telemetry reset add $t_{\text{reset}} =$ `{python} MtbfCampaign.t_reset_str`, for `{python} MtbfCampaign.t_cycle_str` per trial cycle. The drive-wheel diameter is $D_{\text{wheel}} =$ `{python} MtbfCampaign.d_wheel_str`, and the drive gearhead carries an illustrative $L_{10}$ bearing fatigue rating of `{python} MtbfCampaign.l10_math` revolutions.

By the exposure wall, this target needs `{python} MtbfCampaign.t_continuous_str` failure-free hours (`{python} MtbfCampaign.yr_continuous_str` years) of continuous exposure. Discretizing this requirement into missions with per-trial failure budget $p_{\text{fail}} \le \lambda \cdot t_{\text{mission}} =$ `{python} MtbfCampaign.p_fail_mission_math` requires $N_{\text{req}} = \ln(\alpha)/\ln(1-p_{\text{fail}})$, or `{python} MtbfCampaign.n_req_str` consecutive zero-failure trials. This inverts the zero-failure bound of @sec-evaluation-closed-loop-survival at a target far beyond practical budgets of tens to thousands of runs. On a single physical rig, executing these trials at `{python} MtbfCampaign.t_cycle_str` per cycle consumes `{python} MtbfCampaign.t_calendar_str` hours (`{python} MtbfCampaign.yr_calendar_str` years) and accumulates $N_{\text{rev}} = (N_{\text{req}} \cdot v \cdot t_{\text{mission}}) / (\pi D_{\text{wheel}}) \approx$ `{python} MtbfCampaign.n_rev_math` wheel revolutions.

The wheel-rotation total is `{python} MtbfCampaign.rev_ratio_str` times the gearhead's $L_{10}$ rating, but the comparison holds only after the drivetrain ratio maps wheel revolutions to bearing revolutions, and $L_{10}$ is a population-life statistic at specified load and speed, not a destruction deadline. A campaign of this length must therefore plan maintenance and component replacement, and record each one, before it may pool trials.
...
```{python}
#| label: calc-evaluation-confidence-table
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ ZERO-FAILURE CONFIDENCE LIMITS FOR THE BOUNDS TABLE (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @tbl-07-confidence-bounds (statistics columns).
# │ Show: one-sided success lower limits at 90, 95, and 99 percent confidence
# │       and the 95 percent failure upper limit for each table row.
# │ How: p_lower = (1 - C)^(1/n) via calc_clopper_pearson_zero_failure_bound;
# │      rows are DoorCycle.table_n so the time column and these stay aligned.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.fmt import fmt_percent, check

class ConfidenceBoundsTable:
    """Clopper-Pearson zero-failure limits for every row of the bounds table."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    table_n = DoorCycle.table_n  # category: chosen; rows of @tbl-07-confidence-bounds

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    lo90 = [calc_clopper_pearson_zero_failure_bound(n, confidence=0.90) for n in table_n]
    lo95 = [calc_clopper_pearson_zero_failure_bound(n, confidence=0.95) for n in table_n]
    lo99 = [calc_clopper_pearson_zero_failure_bound(n, confidence=0.99) for n in table_n]
    up95 = [1.0 - p for p in lo95]

    # ┌── 3. GUARD (Assertions) ────────────────────────────────────────────
    check(abs(lo95[1] - ClopperPearsonZeroFailure.p_lower) < 1e-12, "20-run row must match the opening campaign's bound")
    check(abs(lo95[-1] - 0.999) < 1e-4, "Last row must meet the 99.9 percent target")
    check(all(a < b < c for a, b, c in zip(lo99, lo95, lo90)), "Higher confidence must give a lower success limit")

    # ┌── 4. OUTPUT (Format & Export) ────────────────────────────────────────
    lo90_str = [fmt_percent(p, precision=2, style="symbol") for p in lo90]
    lo95_str = [fmt_percent(p, precision=2, style="symbol") for p in lo95]
    lo99_str = [fmt_percent(p, precision=2, style="symbol") for p in lo99]
    up95_str = [fmt_percent(p, precision=2, style="symbol") for p in up95]
```
...
|:------------------------|---------------------------------------------:|---------------------------------------------:|---------------------------------------------:|---------------------------------------------:|--------------------------------:|:---------------------------------------------------|
| 10                      | `{python} ConfidenceBoundsTable.lo90_str[0]` | `{python} ConfidenceBoundsTable.lo95_str[0]` | `{python} ConfidenceBoundsTable.lo99_str[0]` | `{python} ConfidenceBoundsTable.up95_str[0]` |   `{python} DoorCycle.h_10_str` | Wide uncertainty; no observed failures             |
| 20                      | `{python} ConfidenceBoundsTable.lo90_str[1]` | `{python} ConfidenceBoundsTable.lo95_str[1]` | `{python} ConfidenceBoundsTable.lo99_str[1]` | `{python} ConfidenceBoundsTable.up95_str[1]` |   `{python} DoorCycle.h_20_str` | Bound applies only to the sampled distribution     |
| 50                      | `{python} ConfidenceBoundsTable.lo90_str[2]` | `{python} ConfidenceBoundsTable.lo95_str[2]` | `{python} ConfidenceBoundsTable.lo99_str[2]` | `{python} ConfidenceBoundsTable.up95_str[2]` |   `{python} DoorCycle.h_50_str` | More evidence; untested conditions remain          |
| 100                     | `{python} ConfidenceBoundsTable.lo90_str[3]` | `{python} ConfidenceBoundsTable.lo95_str[3]` | `{python} ConfidenceBoundsTable.lo99_str[3]` | `{python} ConfidenceBoundsTable.up95_str[3]` |  `{python} DoorCycle.h_100_str` | No deployment permission follows from the interval |
| 250                     | `{python} ConfidenceBoundsTable.lo90_str[4]` | `{python} ConfidenceBoundsTable.lo95_str[4]` | `{python} ConfidenceBoundsTable.lo99_str[4]` | `{python} ConfidenceBoundsTable.up95_str[4]` |  `{python} DoorCycle.h_250_str` | Check hardware and sampling stationarity           |
| 500                     | `{python} ConfidenceBoundsTable.lo90_str[5]` | `{python} ConfidenceBoundsTable.lo95_str[5]` | `{python} ConfidenceBoundsTable.lo99_str[5]` | `{python} ConfidenceBoundsTable.up95_str[5]` |  `{python} DoorCycle.h_500_str` | Bound remains tied to declared conditions          |
| 1,000                   | `{python} ConfidenceBoundsTable.lo90_str[6]` | `{python} ConfidenceBoundsTable.lo95_str[6]` | `{python} ConfidenceBoundsTable.lo99_str[6]` | `{python} ConfidenceBoundsTable.up95_str[6]` | `{python} DoorCycle.h_1000_str` | Below a 99.9% lower-limit target                   |
| 2,995                   | `{python} ConfidenceBoundsTable.lo90_str[7]` | `{python} ConfidenceBoundsTable.lo95_str[7]` | `{python} ConfidenceBoundsTable.lo99_str[7]` | `{python} ConfidenceBoundsTable.up95_str[7]` | `{python} DoorCycle.h_2995_str` | Meets that statistical target if assumptions hold  |
...
- [ ] **Exposure wall**: Under a stationary Poisson failure model, why does a one-sided 95 percent upper rate limit corresponding to a $10{,}000\text{-hour}$ MTBF target require `{python} MtbfCampaign.t_continuous_str` hours of zero-failure physical exposure?
...
2. **Execution Latency Monitor:**\index{Runtime monitoring!execution latency} Measures each inference age against the declared control deadline (for example, the latch policy's `{python} EvalMachine.chunk_period_str` chunk period at `{python} EvalMachine.chunk_rate_str`), records misses, and checks that lease expiry invokes the validated fallback. A $P_{99}$ summary alone is not a hard deadline bound.
...
4. **Intervention Rate Monitor:**\index{Runtime monitoring!intervention rate} Calculates the frequency of permission-path overrides, detecting when policy instability causes excessive corrections.
...
| **`complete_outcome_ledger`** | Full-denominator raw outcome counts: $n_{\text{attempted}}$, $n_{\text{success}}$, $n_{\text{estop}}$, $n_{\text{timeout}}$, $n_{\text{override}}$ (permission-path overrides), and $T_{\text{TTI}}$ time-series | Monotonic microsecond hardware timestamps, uninterrupted ring-buffer telemetry          | Preventing censored trial fallacy; calculating uncorrupted empirical reliability                               |
...
The door campaign repeats one condition twenty times: the nominal strike plate, the guarded approach, the base stopped, the rack empty. Under the binomial model\index{Evaluation!Binomial Model}, those runs give the `{python} ClopperPearsonZeroFailure.p_lower_pct_str` percent lower bound of \ref{nbk-evaluation-derivation-20-run-lower-bound} for that condition\index{Evaluation!Confidence Intervals}, and nothing for the other cells of the ledger or a loaded tote rack that moves the arm's torque margins. Repeating the condition sharpens the nominal estimate while the deployment-critical envelopes\index{Safety!Operational Envelope} stay untested. A readiness claim must name its target ODD, allocate trials across the physical variations inside it, and report uncertainty within each tested cell.
...
An evaluation report that cites close agreement with expert actions\index{Evaluation!Action Agreement} on recorded runs as release evidence has measured imitation on those records and nothing more. The aisle policy of @sec-evaluation-closed-loop-dynamics matched its recordings almost exactly and still spent its side clearance to covariate shift\index{Evaluation!Covariate Shift}. Agreement can screen candidates before physical trials, but a claim of closed-loop stability\index{Control!Closed-Loop Stability} rests only on closed-loop outcomes counted against the full denominator, the metric-drift guard of @sec-evaluation-failure-modes.
