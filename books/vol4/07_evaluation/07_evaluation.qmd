...
**Calculations**: Using the Clopper-Pearson bound formula, meeting this statistical target after zero failures requires $n = \lceil\ln(0.05)/\ln(0.999)\rceil = 2{,}995$ consecutive zero-failure trials. At `{python} DoorCycle.t_cycle_str` per trial, this demands $T_{\text{total}} =$ `{python} ClopperPearsonZeroFailure.n_req_999_str` $\times$ `{python} DoorCycle.t_cycle_str` $\approx$ `{python} DoorCycle.t_target_str` (`{python} DoorCycle.t_target_days_str`) of continuous testing. With a `{python} DoorCycle.n_operators_str`-operator safety protocol, this consumes `{python} DoorCycle.t_target_operator_str` of operator time, or over `{python} DoorCycle.shifts_floor_str` eight-hour shifts.

**Systems insight**: Establishing a 99.9 percent one-sided success lower confidence limit by this method requires nearly $3{,}000$ iid zero-failure trials. Even on a cycle as short as the door's, that is `{python} DoorCycle.t_target_days_str` of continuous test-cell occupancy, and a task with a longer cycle scales the occupancy in proportion. The campaign must track wear and recalibration throughout; a component replacement or changing dynamics may require a new population definition rather than pooling incompatible runs.
:::
