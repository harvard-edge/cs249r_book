...
%
% Stack (top to bottom), ordered from the learned models down to the physics:
%   Brain            1-50 Hz                      learned proposers: perception, memory, intent, planning
%   ---- proposal boundary: above may only propose, below may command ----
%   Nervous System   1 kHz (one implementation)   permission path, lease, fallback, placement
%   Body             10-25 kHz current loop       drives, transmission, windings, rails
%   Boundary         event-timed                  register write and sensor capture
%   Physical World   continuous                   contact, friction, payload, people
%   Governance is the lifecycle envelope, not a tier: chapters 14-17 set every tier inactive.
%
...
\newcommand{\mlphysicalstack}[5]{%
  \begin{tikzpicture}
    ...
    \physrow{1.80}{Nervous System}{1 kHz}{#2}
    \physrow{0.70}{Body}{10--25 kHz}{#3}
    \physrow{-0.40}{Boundary}{event-timed}{#4}
...
\newcommand{\mlphysicalstackfull}[5]{%
  \begin{tikzpicture}
    \physrowfull{4.80}{Brain}{1--50 Hz}%
      {perception, memory, intent, planning}{#1}{\faBrain}
    ...
    \node[anchor=west,text=physctxink,
          ...
          at (0,4.36) {above: propose; below: command};
    \physrowfull{2.80}{Nervous System}{1 kHz (one implementation)}%
      {permission path, lease, fallback}{#2}{\faNetworkWired}
    \physrowfull{1.20}{Body}{10--25 kHz current loop}%
      {drives, transmission, windings}{#3}{\faMicrochip}
    \physrowfull{-0.40}{Boundary}{event-timed}%
      {register write, sensor capture}{#4}{\faRobot}
    \physrowfull{-2.00}{Physical World}{continuous}%
      {contact, friction, payload, people}{#5}{\faGlobe}
  \end{tikzpicture}%
}
