...
```{python}
#| label: calc-data-aisle-takeover-drift
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ AISLE TAKEOVER DRIFT (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-data-intervention-recovery (selection-trap paragraph)
# │ Goal:    Place the takeover example on the running machine in its aisle.
# │ Show:    Lateral drift that trips the side-clearance monitor, at the
# │          in-loop takeover speed the machine slows to before asking.
# │ How:     Clearance and takeover speed from AISLE; the monitor threshold is
# │          a chapter-local fraction of the clearance.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
...
class AisleTakeoverDrift:
    """Remote-assistant takeover when the lateral monitor trips inside the side clearance."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    side_clearance = AISLE.side_clearance  # category: illustrative
    v_takeover = AISLE.v_takeover_in_loop  # category: chosen
    monitor_fraction = 2 / 3  # category: chosen (chapter-local monitor threshold)

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    drift = side_clearance * monitor_fraction
    margin_left = side_clearance - drift

    # ┌── 3. GUARD (Assertions) ────────────────────────────────────────────
    check(abs(side_clearance.to(ureg.meter).magnitude - 0.15) < 1e-9, "Side clearance should be 0.15 m")
    check(drift < side_clearance, "Monitor must trip before the base reaches the rack face")
    check(abs(drift.to(ureg.meter).magnitude - 0.10) < 1e-9, "Monitor threshold should be 0.10 m")
    check(abs(v_takeover.to(ureg.meter / ureg.second).magnitude - 1.2) < 1e-9, "In-loop takeover speed should be 1.2 m/s")

    # ┌── 4. OUTPUT (Format & Export) ────────────────────────────────────────
    drift_str = fmt_length(drift, unit=ureg.meter, precision=2)
    clearance_str = fmt_length(side_clearance, unit=ureg.meter, precision=2)
    margin_left_str = fmt_length(margin_left, unit=ureg.meter, precision=2)
    v_takeover_str = fmt_velocity(v_takeover, precision=1)
```

Human takeovers and automated disengagements introduce a selection trap into fleet data. An intervention occurs precisely when the autonomous policy has entered a region of state space that it handles poorly, causing tracking error or model uncertainty to exceed an operational threshold. Because disengagements are triggered by failing rollouts, the collected intervention data is not drawn from the nominal state distribution of the task. Suppose the mobile manipulator drifts `{python} AisleTakeoverDrift.drift_str` toward a rack inside its `{python} AisleTakeoverDrift.clearance_str` side clearance. The lateral monitor trips, and because the machine never requests in-loop help at aisle speed (@sec-intervention), it first slows to `{python} AisleTakeoverDrift.v_takeover_str` and then asks a remote assistant, who takes over with `{python} AisleTakeoverDrift.margin_left_str` of clearance left. The trajectory recorded after the takeover captures an expert recovering from an edge case disturbance. The distribution of states visited during these rescues, denoted $d^{\text{takeover}}$, is conditioned on policy failure rather than standard task execution. Treating intervention logs as if they were drawn from the nominal distribution $d^{\pi^*}$ distorts the training distribution toward states that only exist because the prior policy failed.
