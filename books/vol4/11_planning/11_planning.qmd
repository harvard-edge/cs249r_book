...
The first decision is what the planner must hand to the tracking controller and the permission path while the arm is still moving.
...
[^fn-math-bellman-dp]: **Dynamic Programming Lineage**\index{Dynamic programming!provenance}: The principle of optimality and discrete dynamic programming algorithms were formalized by @bellman1957dynamic for sequential decision processes. While DP finds the globally optimal trajectory over a discretized state lattice, computational complexity scales exponentially as $O(K^d)$ where $d$ is state dimension and $K$ is grid resolution. In high-dimensional physical systems, this exponential curse of dimensionality forces trajectory planners to use continuous trajectory optimization or randomized sampling.
...
Budget replacement admission before $t_{\mathrm{commit}}$ and the complete stopping continuation after it. A tail percentile allocates the risk of a late replacement; the resident stop is what the machine does when that risk comes due.
:::
