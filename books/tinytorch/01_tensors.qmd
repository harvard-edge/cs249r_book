...
![](../../packages/tinytorch/quarto/assets/images/diagrams/01_tensor-diag-1.svg)

**Figure 1.1: Logical Multidimensional Tensor vs. Physical Linear Storage.** A $2 \times 3$ matrix is represented as a contiguous six-element linear memory buffer. Stride tuples dictate how coordinate indices $(i, j)$ translate into memory offsets.
:::
...
To locate element $A[i, j]$, the runtime does not search multidimensional pointers. Instead, it evaluates a closed-form linear mapping equation:

$$\text{Offset}(i, j) = i \times 3 + j \times 1$$

The multipliers $(3, 1)$ are the **strides** of the tensor. Stride arithmetic is the engine that powers every tensor library.
...
```python
# Listing 1.1: Foundational Tensor Data Structure in TinyTorch
import numpy as np
...
Without broadcasting, an engine would need to physically replicate the bias vector $B$ times in memory, wasting precious memory bandwidth. **Broadcasting** allows element-wise operations to occur across mismatched shapes by logically repeating dimensions of length 1 without memory duplication.

::: {#fig-broadcasting-rules}
![](../../packages/tinytorch/quarto/assets/images/diagrams/01_tensor-diag-2.svg)

**Figure 1.2: Broadcasting Mechanics.** When adding a $(2, 3)$ tensor to a $(1, 3)$ tensor, the single row is virtually expanded along dimension 0 by setting its stride to 0, avoiding physical data copying.
:::
...
```python
# Listing 1.2: Operator Overloading and Element-Wise Arithmetic
...
Element-wise operations scale linearly with the number of elements ($O(N)$). Matrix multiplication, in contrast, is the computational heart of neural networks and exhibits cubic complexity ($O(N^3)$ for standard implementations).
...
```python
# Listing 1.3: Matrix Multiplication with Shape Validation
...
::: {.callout-checkpoint}
## Check Your Understanding: Arithmetic Intensity
...
Because matrix multiplication exhibits high arithmetic intensity, modern accelerators deploy specialized systolic array hardware (such as Tensor Cores or TPUs) to execute these operations at peak throughput.
:::
...
::: {#fig-view-vs-copy}
![](../../packages/tinytorch/quarto/assets/images/diagrams/01_tensor-diag-3.svg)

**Figure 1.3: Views vs. Copies.** A `transpose` operation produces a view by swapping stride values $(3, 1) \to (1, 3)$ without moving a single byte in memory. Calling `contiguous()` creates a new buffer where elements are physically rearranged in linear order.
:::

```python
# Listing 1.4: Shape Manipulation and Transposition
...
When a tensor is transposed, its logical indexing changes instantly, but its elements remain in their original physical order in memory. While element access works correctly, downstream operations (such as vector dot products) can no longer read contiguous memory blocks, degrading hardware cache line prefetching. Production frameworks provide `.contiguous()` to materialize a fresh, sequential buffer when stride discontinuity degrades performance.
:::
...
```python
# Listing 1.5: Reduction Operations
...
In production frameworks like PyTorch, the Python `torch.Tensor` is merely a thin CPython binding wrapper around native C++ objects defined in the **ATen** (A Tensors) library.
...
1. **`c10::Storage`**: Represents the raw memory buffer allocated on either the CPU or a CUDA device. It manages a reference count so that multiple tensors (such as slices or transposed views) can reference the same byte array without duplicating memory.
2. **`c10::TensorImpl`**: Stores metadata including `sizes`, `strides`, `storage_offset`, and data type (`ScalarType`).
3. **`at::Tensor`**: A lightweight smart pointer that user code passes by value across C++ functions.

Our TinyTorch `Tensor` class mirrors this exact relationship: the NumPy array acts as the `c10::Storage` buffer, while the Python class manages the shape and arithmetic bindings.
...
- **Broadcasting Efficiency**: Arithmetic across mismatched shapes uses virtual stride repetition (setting strides to 0) to avoid memory allocation overhead.
