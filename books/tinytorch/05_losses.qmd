...
## 5.1 The Framework Role of Loss Functions
...
## 5.2 Mean Squared Error (MSELoss) for Continuous Regression
...
```python
# Listing 5.1: Mean Squared Error (MSELoss) Implementation
...
## 5.3 Binary Cross-Entropy (BCELoss) and Epsilon Clamping
...
```python
# Listing 5.2: Binary Cross-Entropy (BCELoss) with Epsilon Safeguard
EPSILON = 1e-7
...
## 5.4 Categorical Cross-Entropy and the Log-Sum-Exp Invariant
...
```python
# Listing 5.3: Numerically Stable Log-Softmax Implementation
...
**Figure 5.1: Naive Softmax vs. Fused Log-Sum-Exp Pipeline.** Computing probabilities in linear space risks exponent overflow and log underflow. Fusing the operations into the Log-Sum-Exp invariant operates directly on shifted logits, guaranteeing stable IEEE 754 precision.
:::
...
```python
# Listing 5.4: CrossEntropyLoss Implementation in TinyTorch
...
## 5.5 Forward Error Evaluation: Connecting Networks to Loss
...
```python
# Listing 5.5: End-to-End Forward Evaluation Pipeline
