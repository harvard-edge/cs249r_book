...
[^sparse-tc-footnote]: NVIDIA 2:4 Structured Sparsity (introduced in Ampere GA100) compresses each 4-element FP16/INT8 vector into 2 non-zero elements plus a 2-bit index metadata per pair. Dedicated Sparse Tensor Core hardware decompresses these pairs on-the-fly inside register banks before feeding the systolic multiplier array, delivering an exact $2\times$ theoretical math speedup.
