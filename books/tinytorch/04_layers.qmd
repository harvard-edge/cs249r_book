...
## 4.1 The Physics of Weight Initialization
...
```python
# Listing 4.1: The Layer Base Class
...
```python
# Listing 4.2: The Stateful Linear Layer Implementation
import numpy as np
...
## 4.3 Regularization via Inverted Dropout
...
**Figure 4.1: Standard vs. Inverted Dropout Execution.** During training, a random binary mask zeroes activations with probability $p$. Inverted Dropout scales surviving units by $\frac{1}{1-p}$, ensuring test-time evaluation requires zero arithmetic scaling.
:::
...
```python
# Listing 4.3: Inverted Dropout Implementation with State Modes
class Dropout(Layer):
    """
...
## 4.4 Composition: The Sequential Container
...
```python
# Listing 4.4: The Sequential Model Container
...
```python
# Listing 4.5: Constructing and Executing a Multi-Layer Network
...
## 4.5 Systems Reality: Parameter Counting and Storage Footprint
...
Consider the 3-layer MLP constructed in Listing 4.5:
