...
Deep learning accelerators and multi-core CPUs are designed to process dense matrix arithmetic at massive throughput. If training data is fed to the model as individual single samples, the runtime is trapped in low-intensity memory stalls, starving the arithmetic registers while waiting for isolated memory fetches. To achieve high arithmetic throughput, frameworks must decouple physical storage from model execution through a two-tiered software pipeline: the **Dataset** abstraction (which defines sample-level retrieval) and the **DataLoader** engine (which orchestrates mini-batch collation, index shuffling without memory copying, and lazy batch generation). In a framework runtime, the data pipeline is the physical I/O valve that bridges uncompressed storage on disk with high-bandwidth memory in compute registers.
...
- **Analyze** the arithmetic intensity shift when moving from single-sample processing ($B = 1$) to mini-batch GEMM matrix operations ($B \ge 32$).
- **Build** a lazy Python generator pipeline yielding batched `Tensor` tuples on-demand.
...
## 6.1 The Systems Rationale for Mini-Batching

Consider evaluating a linear transformation $\mathbf{W} \in \mathbb{R}^{D_{\text{in}} \times D_{\text{out}}}$ on $N$ input samples.

If we process samples individually with batch size $B = 1$:
- For each sample vector $\mathbf{x} \in \mathbb{R}^{1 \times D_{\text{in}}}$, the hardware must load the entire weight matrix $\mathbf{W}$ ($D_{\text{in}} \times D_{\text{out}} \times 4\text{ bytes}$) from memory into cache registers.
- Across $N$ samples, the weight matrix is loaded from memory $N$ separate times.
- The operation is a **Vector-Matrix Multiplication (GEMV)** with very low arithmetic intensity ($\approx 1\text{ FLOP/Byte}$), heavily bottlenecked by memory bandwidth.

If we group samples into mini-batches of size $B = 64$:
- The input becomes a 2D matrix $\mathbf{X} \in \mathbb{R}^{B \times D_{\text{in}}}$.
- The weight matrix $\mathbf{W}$ is loaded from memory **only once** for all 64 samples simultaneously.
- The operation becomes a **Matrix-Matrix Multiplication (GEMM)** where every loaded weight element is reused across 64 independent inputs, increasing arithmetic intensity by a factor of 64× and saturating hardware ALUs.
...
│  • Arithmetic Intensity: ~64 FLOPs / Byte (Compute-Bound)   │
└─────────────────────────────────────────────────────────────┘
```

Furthermore, mini-batching bounds peak RAM usage: instead of loading an entire 100,000-sample dataset into working memory at once, the runtime maintains only the active mini-batch ($B \times D$ elements) in memory.

---

## 6.2 Decoupling Storage: The Dataset Abstraction
...
```python
# Listing 6.1: The Dataset Abstract Base Class
...
```python
# Listing 6.2: TensorDataset Implementation
...
## 6.3 The DataLoader Engine: Shuffling without Memory Movement
...
![](../../packages/tinytorch/quarto/assets/images/diagrams/05_dataloader-diag-1.svg)

**Figure 6.1: The DataLoader Shuffling and Batching Pipeline.** Shuffling generates a lightweight integer index permutation. The engine slices index chunks of size $B$, gathers sample tensors, and collates them into unified batch tensors.
:::

---

## 6.4 Building the Iterable DataLoader

The `DataLoader` combines a `Dataset` with a batching engine, implementing the Python iterator protocol (`__iter__` and `__next__`):

```python
# Listing 6.3: The Complete DataLoader Implementation in TinyTorch
import numpy as np
...
## 6.5 Forward Pipeline Integration: Full Dataset Evaluation

We can now assemble the complete forward pipeline constructed across Chapters 2 through 6:
1. Wrap synthetic dataset samples in `TensorDataset`.
2. Construct a `DataLoader` to iterate in batches of $B = 32$.
3. Pass each batch through our Chapter 4 `Sequential` MLP.
4. Measure batch prediction error with Chapter 5 `CrossEntropyLoss`.
5. Aggregate total epoch loss and compute classification accuracy.

```python
# Listing 6.4: Full Dataset Forward Evaluation Loop
...
## 6.6 Systems Reality: Multi-Process I/O and Prefetching

In production deep learning frameworks like PyTorch, reading gigabytes of training data from disk or network storage cannot occur synchronously on the main training thread. If the model must wait for disk reads between every batch, GPU utilization drops to near zero (an I/O bottleneck).
...
Production data loaders solve this by launching background worker processes (`num_workers > 0`) using POSIX shared memory (`/dev/shm`). While the accelerator computes the forward pass of Batch $k$, background workers load and collate Batch $k+1$ in parallel, prefetching data into pinned memory buffers so arithmetic compute is never starved.

---

## Summary

In this chapter, we constructed the fifth foundational component of TinyTorch: data loading infrastructure. We examined the physical systems rationale for mini-batching, demonstrating how grouping samples into batches shifts computation from memory-bound vector operations into high-intensity GEMM matrix operations that saturate hardware ALUs.
...
- **Arithmetic Intensity Boost**: Mini-batching ($B \ge 32$) transforms memory-bound vector-matrix operations into compute-bound matrix multiplications (GEMM), reusing loaded weights across multiple inputs.
...
- **Zero-Copy Shuffling**: Shuffling lightweight integer index arrays $[0\dots N-1]$ achieves full stochastic randomization without copying or rearranging heavy data tensors in memory.
...
- **Asynchronous Prefetching**: Production runtimes deploy multi-process background workers to load upcoming batches in parallel, preventing I/O stalls from starving compute engines.
:::

::: {.callout-chapter-connection}
## Chapter Connection: Computing Gradients Automatically

We now have data flowing through multi-layer networks and measuring prediction error, but how does the framework know how to adjust weights to decrease loss? In **Chapter 7 (Automatic Differentiation and Graph Tracing)**, we build the crown jewel of TinyTorch: the dynamic reverse-mode autograd engine, computational DAG tracing, and Vector-Jacobian Products.
:::
