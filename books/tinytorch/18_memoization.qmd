...
## 18.1 The Computational Redundancy of Naive Decoding

Consider generating 100 new tokens from a 500-token prompt:
...
![](../../packages/tinytorch/quarto/assets/images/diagrams/18_memoization-diag-1.svg)
...
## 18.2 The Key-Value Cache Formulation
...
## 18.3 Implementing KVCache in TinyTorch
...
## 18.4 Cache-Aware Multi-Head Attention
...
## 18.5 Memory Scaling of the KV Cache
...
::: {.callout-note}
## Production Systems Bridge: vLLM and PagedAttention

In modern high-concurrency LLM inference servers, **vLLM** implements **PagedAttention** (Kwon et al., 2023), inspired by operating system virtual memory paging.

Instead of allocating contiguous GPU memory for each request's maximum sequence length (which wastes up to $80\%$ of VRAM to internal fragmentation), PagedAttention partitions the KV cache into fixed-size virtual memory blocks ($16$ tokens per block). Non-contiguous blocks are dynamically mapped via a page table, increasing GPU serving throughput by **2× to 4×**.[^vllm-paper]
:::

[^vllm-paper]: Kwon et al. (2023), *Efficient Memory Management for Large Language Model Serving with PagedAttention*, SOSP.

## 18.6 Architectural Mitigations: MQA, GQA, and PagedAttention
