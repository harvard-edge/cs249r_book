...
## 13.1 Pre-LN vs. Post-LN and Residual Highways
...
Post-LN causes gradients to vanish near input layers, requiring delicate learning rate warmups. Modern architectures (GPT-2, GPT-3, LLaMA) standardized on **Pre-LayerNorm (Pre-LN)**, applying normalization *before* each sub-layer:
...
Pre-LN guarantees that the clean identity gradient $\mathbf{I}$ flows directly from the loss to the input embeddings through the residual additions without being attenuated.

---

## 13.2 Layer Normalization: Feature-Wise Control
...
## 13.3 The Transformer Feed-Forward Network (FFN)
...
## 13.4 The Complete TransformerBlock
...
## 13.5 Building the Autoregressive GPT Model
...
![](../../packages/tinytorch/quarto/assets/images/diagrams/13_transformers-diag-1.svg)
...
## 13.6 Autoregressive Text Generation
...
::: {.callout-note}
## PyTorch Systems Bridge: `torch.nn.LayerNorm` vs. `RMSNorm`

In modern transformer architectures (such as LLaMA, Mistral, and Gemma), standard `LayerNorm` is frequently replaced by **RMSNorm (Root Mean Square Normalization)**:

$$\text{RMSNorm}(\mathbf{x}) = \frac{\mathbf{x}}{\sqrt{\frac{1}{D} \sum_{i=1}^D x_i^2 + \epsilon}} \odot \mathbf{\gamma}$$

RMSNorm eliminates the mean calculation ($\mu = 0$), saving approximately $10\%$ to $15\%$ of normalization compute time on GPUs without harming downstream language modeling perplexity.[^rmsnorm-paper]
:::

[^rmsnorm-paper]: Zhang & Sennrich (2019), *Root Mean Square Layer Normalization*, NeurIPS.

## 13.7 Systems Reality: Parameter Counting and Scaling Laws

How many parameters are in a decoder-only transformer? Let us break down the mathematical accounting for a model with $L$ layers, hidden dimension $D$, vocabulary size $V$, and $4D$ FFN expansion:

1. **Embedding Tables**: Token Embeddings ($V \times D$) + Positional Embeddings ($S_{\text{max}} \times D$).
2. **Per Transformer Block**:
   - Multi-Head Attention: $\mathbf{W}_Q, \mathbf{W}_K, \mathbf{W}_V, \mathbf{W}_O \implies 4 \times D^2$ weights.
   - Feed-Forward Network: $\mathbf{W}_1 (D \times 4D) + \mathbf{W}_2 (4D \times D) \implies 8 \times D^2$ weights.
   - Total per Block: $12 D^2$ weights!
3. **Total Model Weights**:

$$N_{\text{weights}} \approx 12 \times L \times D^2 + V \times D$$

::: {.callout-principle}
## The 12 $L D^2$ Rule of Thumb

In any decoder-only transformer, the core transformer stack contains exactly **$12 L D^2$ parameter weights**. 
- GPT-2 Small ($L=12, D=768$): $12 \times 12 \times 768^2 = 85\text{M params} + 39\text{M vocab} \approx 124\text{M params}$.
- GPT-3 ($L=96, D=12288$): $12 \times 96 \times 12288^2 \approx 174\text{ Billion parameters}$.
:::
