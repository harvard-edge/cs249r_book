...
Furthermore, runtime diagnostic systems must adhere to Edsger Dijkstra's empirical verification principle: testing demonstrates the presence of bugs, but never their absence. An unprivileged neural network cannot verify its own competence. Prompts that instruct a model to "reflect on your previous answer" or "evaluate your code for correctness" merely initiate another unprivileged inference query subject to the identical statistical epistemic gap. The model cannot grade its own work. Verification closure is achieved only when candidate actions and resulting environmental transitions are validated by deterministic external software oracles: compilers that enforce type soundness, linters that verify syntactic invariants, security monitors that audit syscalls, and sandboxed test suites that evaluate operational state deltas, as formalized in @fig-vol3-failure-diagnostic-tree.

![**The Diagnostic Decision Tree for Agent Execution Failures**: Systematic classification of trajectory anomalies across runtime substrates, context windows, and tool schemas before escalating up the Systems Intervention Ladder.](images/svg/fig-vol3-failure-diagnostic-tree.svg){#fig-vol3-failure-diagnostic-tree width="100%"}

::: {#exmp-12-quantitative-trade-offs-in-the-systems .callout-example title="Quantitative Trade-offs in the Systems Intervention Ladder"}
...
::: {#exmp-12-storage-io-and-reset-latency .callout-example title="Storage I/O and Reset Latency in a 512-Worker Rollout Cluster"}
...
High-throughput trajectory harvesting platforms generate rollouts at rates that quickly overwhelm downstream compute budgets. When hundreds of parallel worker processes sample thousands of candidate trajectories per hour, submitting every raw proposal to an un-staged, comprehensive validation pipeline saturates container runtimes, locks compilation daemons, and inflates cluster operational costs. Worse still, an agent navigating an unconstrained environment frequently produces trajectories that appear successful under superficial observation—such as returning an exit code of zero—while harboring malformed syntax, dangling database locks, or deliberate tampering with the test harness itself. Submitting every raw trace to end-to-end dynamic testing squanders cluster resources on programs that could have been rejected by an abstract syntax tree parser in microseconds, as illustrated by the progressive filtering cascade in @fig-vol3-verifier-cascade.

![**The Five-Stage Trajectory Acceptance Funnel**: Raw proposals traverse an escalating cascade where early mechanical stages discard malformed attempts at microsecond latencies, protecting expensive dynamic sandboxes and semantic triage layers from compute exhaustion.](images/svg/fig-vol3-verifier-cascade.svg){#fig-vol3-verifier-cascade width="95%"}
...
![**The Tripartite Trajectory Typology**: Structural contrast between pristine forward paths, perturbed recovery cycles with diagnostic inspection, and terminal hard negatives annotated at the point of irrecoverable divergence.](images/svg/vol3-trajectory-typology.svg){#fig-vol3-trajectory-typology width="90%"}
...
::: {#exmp-12-sizing-and-token-budget-allocation .callout-example title="Sizing and Token Budget Allocation for a Mixed Curation Pipeline"}
...
![**Distributed Trajectory Harvesting Pipeline Architecture**: Decoupled task dispatch, rollout generation, microVM snapshot pools, staged verification, and columnar sink ingestion governed by closed-loop queue backpressure.](images/svg/vol3-collection-pipeline.svg){#fig-vol3-collection-pipeline width="100%"}

### Distributed Pipeline Topology and Execution Stages

To achieve high hardware utilization across heterogeneous compute resources, the collection infrastructure decomposes trajectory harvesting into five loosely coupled stages connected via durable, asynchronous message channels (@fig-vol3-collection-pipeline). Each stage scales independently, isolates its specific failure domains, and enforces strict operational contracts at its ingress and egress boundaries, as detailed in @tbl-vol3-pipeline-stages.
...
::: {#exmp-12-sizing-intermediate-staging-buffers-and .callout-example title="Sizing Intermediate Staging Buffers and Calculating Backpressure Throttling"}
...
where $K_{\text{pipe}}$ represents the private key of the ingestion pipeline, and $H_{\text{trajectory}}$ is the Merkle root computed across the ordered sequence of interaction turns $\langle (s_0, a_0), (s_1, a_1), \dots, (s_T, a_T) \rangle$. As illustrated in @fig-vol3-provenance-envelope, storing trajectories indexed by $H_{\text{lineage}}$ guarantees content-addressable storage: downstream training jobs can selectively filter training data based on exact environment configurations, ensuring that policies are never trained on traces generated under incompatible tool definitions or unverified runtime environments.

![**The Trajectory Lineage Envelope and Sanitization Flow**: Binding the multi-layer execution stack into an immutable cryptographic digest while directing raw traces through a three-pass sanitization pipeline before persistent storage.](images/svg/vol3-provenance-envelope.svg){#fig-vol3-provenance-envelope width="95%"}

::: {#exmp-12-storage-overhead-and-ingestion-latency .callout-example title="Storage Overhead and Ingestion Latency of Lineage Envelopes"}
...
#### Pass 1: Deterministic Pattern Scanning and Shannon Entropy Analysis {.unnumbered}
...
![**Downstream Task Success vs. Trajectory Corpus Composition**: Downstream task success versus trajectory corpus composition under a fixed training token budget. Policies trained on 100% pristine demonstrations achieve rapid initial success on easy tasks but degrade on complex rollouts. Incorporating 30% recovery demonstrations maximizes robust generalization across unseen repository fixtures.](images/svg/vol3-data-composition-ablation.svg){#fig-vol3-data-composition-ablation width="90%"}
...
#### Stage 2: Staged Mechanical and Semantic Verification Cascade {.unnumbered}
...
#### Stage 3: Behavioral Role Labeling and Perturbation Annotation {.unnumbered}
...
#### Stage 4: Multi-Pass Redaction and Structural Normalization {.unnumbered}
...
::: {#exmp-12-sample-efficiency-and-compute-budgets .callout-example title="Sample Efficiency and Compute Budgets in Trajectory Distillation"}
...
::: {.callout-takeaways title="Core Systems Principles of Trajectory Data and Feedback"}
