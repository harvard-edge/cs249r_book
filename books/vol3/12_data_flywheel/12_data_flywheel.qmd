# Trajectory Harvesting {#sec-vol3-data-flywheel}
...
*Exhausting deterministic runtime tiers resolves 90 percent of failures before escalating to model retraining across four orders of magnitude in latency.*
:::
**When an autonomous database migration agent fails repeatedly on schema alteration tasks across production clusters, the engineering team faces a fundamental systems dilemma.** In telemetry logs, the agent emits invalid SQL statements during `ALTER TABLE` operations, triggering database transaction rollbacks under syntax error exceptions. The immediate operational impulse across many machine learning engineering teams is to classify this failure as a fundamental model deficiency and immediately schedule an expensive supervised fine-tuning run on thousands of curated database migration scripts. Yet a rigorous forensic trace decomposition reveals that the tool schema exposed to the model, `execute_sql(query: string)`, omitted the target database engine dialect and version. Lacking this environmental constraint in its staged context buffer, the model defaulted to valid PostgreSQL syntax while executing against an active MySQL 8.0 target cluster. Updating the JSON schema specification to include an explicit `engine_dialect` enum and injecting the target engine version into the context window resolved 100 percent of the failures instantly without modifying a single model weight.
...
A running agent fleet leaves a continuous record of attempts, and most of that record is not training signal. A trace can fail because the context lacked a fact, because a tool schema was ambiguous, because a sandbox timed out, or because the policy could not do the task, and only the last cause is repaired by changing weights. A trace can also succeed for the wrong reason, by luck or by editing its own test. This chapter treats harvesting as the admission stage of the policy compiler. It asks which failures earn a place in a training corpus, and what the runtime must build before any trace does, namely reproducible fixtures, an ordered cascade of verifiers, a channel for recovery traces, and evaluation splits that cannot leak.

Dumping raw, uncurated production telemetry directly into training datasets creates a catastrophic, self-reinforcing failure loop. Unfiltered execution traces are dominated by transient infrastructure failures, retry thrashing, inefficient search paths, and plausible-sounding hallucinations. If an engineering team naively trains a model on its own unverified operational traces, the fine-tuned policy simply memorizes its historical inefficiencies and learns to mimic its past failure modes with higher confidence. Trajectory harvesting therefore cannot function as a passive logging sink. It must triage every observed anomaly before any trace is admitted into an offline learning pipeline.

::: {.column-margin}
**Gate rejections in telemetry:** Because the model holds zero ambient authority, every tool call it emits is a proposal the runtime may refuse. A recorded failure in an operational trace often reflects a rejection by the runtime's verification gate rather than an environmental failure, and telemetry must classify the two separately.
:::

Harvesting inherits two facts from @sec-vol3-intro-fail-plausible and @sec-vol3-intro-invariant-closure. A failed trajectory rarely announces its cause, because a fail-plausible model emits well-formed calls built on false assumptions about host state, and a model's claim of success carries no evidential weight. A failure therefore does not by itself show which layer must change.
...
Only when telemetry proves that the context buffer contained all necessary evidence, the tool schemas were mathematically unambiguous, and the runtime executed deterministically without defect, can a failure be classified as a true **Policy Incapability**. This residual class encompasses procedural reasoning breakdowns, algorithmic hallucinations, inability to decompose multi-step tasks, and catastrophic failure to recover from valid error returns. Only this class represents genuine training signal for offline policy learning, which is why verified trajectory post-training (principle \ref{pri-vol3-trajectory-post-training}) requires this triage before any weight changes. Every other class is repaired faster, and more reversibly, below the weights.
...
This progression directly instantiates the seminal **End-to-End Argument in System Design** formulated by Saltzer, Reed, and Clark [@saltzer1984endtoend]: a system function should not be implemented at a lower, heavier layer of an architecture if it can be implemented completely and correctly with lightweight mechanisms at the application boundary. Attempting to "teach" an agent an API signature by fine-tuning model weights when the underlying tool schema lacks explicit type constraints directly violates this principle. Fine-tuning models to compensate for upstream interface defects produces fragile memorization: the moment the downstream API changes, the trained weights become obsolete. A typed action contract (principle \ref{pri-vol3-strict-action-abi}) and a complete context instead establish invariants that hold for any model behind them.

The triage itself cannot be delegated to the model. Self-report sits at the bottom of the closure evidence levels (@sec-vol3-intro-closure-evidence), and a prompt that asks the model to "reflect on your previous answer" only issues another call subject to the same limits. Each triage verdict therefore rests on checks outside the model, such as compilers that enforce type soundness, linters that verify syntactic invariants, security monitors that audit syscalls, and sandboxed test suites that evaluate state deltas.
...
::: {.column-margin}
**Completion evidence:**
A model's declaration that it finished ("I have completed the task") carries no evidential weight (@sec-vol3-intro-closure-evidence). The oracle inspects the state the trajectory left behind.
:::

The *mechanical verification oracle* ($\mathcal{O}_{\text{verify}}$) provides the objective ground truth that classifies a harvested trajectory as a success or a failure. The invariant closure principle (\ref{pri-invariant-closure}) requires that evidence to come from outside the model's authority, so the verification oracle is completely decoupled from the model's self-assessment. It consists of automated test suites, Abstract Syntax Tree (AST) linters, binary execution checks, or database state assertions executed within a separate, privileged supervisor process. The oracle evaluates the mutated environment against sealed acceptance invariants, emitting a deterministic binary score $y \in \{0, 1\}$ or a structured diagnostic vector. If an oracle relies on an unpinned LLM-as-a-judge to grade trajectory completion, it re-introduces stochastic drift into the evaluation boundary, compromising the integrity of the data flywheel.
...
```{python}
#| label: lego-12-storage-io-and-reset-latency
#| echo: false
# │ Show: CoW OverlayFS cuts write bandwidth by 121.3x and recovers 1,117 worker-hours.
# │ How: Model cold re-cloning vs ephemeral upper layer mutation throughput.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_bandwidth, fmt_int, fmt_multiple

class RolloutClusterStorageResetLatency:
    """Storage I/O bandwidth and reset latency in rollout harvesting clusters."""
    ...
    t_cold = 12 * second
    delta_mut = 18 * MB
    t_cow = 65 * millisecond

    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    # Cold
    tau_cold = t_exec + t_cold
    r_cold = (w_workers / tau_cold.to(second).magnitude) * (1 / second)
    b_cold = (r_cold.magnitude * s_repo.to(GB).magnitude) * (GB / second)
    idle_cold = t_cold.to(second).magnitude / tau_cold.to(second).magnitude
    wasted_hours = w_workers * idle_cold * 24

    # CoW
    tau_cow = t_exec + t_cow
    r_cow = (w_workers / tau_cow.to(second).magnitude) * (1 / second)
    b_cow = (r_cow.magnitude * delta_mut.to(MB).magnitude) * (MB / second)
    idle_cow = t_cow.to(second).magnitude / tau_cow.to(second).magnitude

    bandwidth_ratio = (b_cold.to(MB / second) / b_cow.to(MB / second)).magnitude

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(abs(tau_cold.to(second).magnitude - 132.0) < 1e-4, "tau_cold")
    check(abs(r_cold.magnitude - 3.87878) < 0.001, "r_cold")
    check(abs(b_cold.to(GB / second).magnitude - 9.309) < 0.01, "b_cold")
    check(abs(idle_cold - 0.0909) < 0.001, "idle_cold")
    check(abs(wasted_hours - 1117.09) < 1.0, "wasted_hours")

    check(abs(tau_cow.to(second).magnitude - 120.065) < 1e-4, "tau_cow")
    check(abs(r_cow.magnitude - 4.264) < 0.001, "r_cow")
    check(abs(b_cow.to(MB / second).magnitude - 76.758) < 0.01, "b_cow")
    check(abs(idle_cow - 0.000541) < 0.0001, "idle_cow")
    check(abs(bandwidth_ratio - 121.27) < 0.2, "bandwidth_ratio")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    b_cold_gb_s_str = fmt_bandwidth(b_cold, unit=GB / second, precision=2)
    wasted_hours_str = fmt_int(round(wasted_hours))
    bandwidth_ratio_mult_str = fmt_multiple(bandwidth_ratio, precision=1)
```
...
A sustained write throughput of `{python} RolloutClusterStorageResetLatency.b_cold_gb_s_str` exhausts the PCIe bus and write endurance of standard enterprise NVMe arrays, inducing extreme I/O wait queues. Furthermore, the worker idle time spent waiting on disk resets is:
$$\text{Idle Fraction}_{\text{cold}} = \frac{t_{\text{cold}}}{\tau_{\text{cold}}} = \frac{12}{132} \approx 9.09\%$$
Over a 24-hour run, the cluster wastes `{python} RolloutClusterStorageResetLatency.wasted_hours_str` worker-hours purely writing duplicate bytes to disk.
...
**Systems Takeaway:** The CoW architecture reduces storage write bandwidth by a factor of `{python} RolloutClusterStorageResetLatency.bandwidth_ratio_mult_str`, eliminates NVMe drive burnout, and recovers over $1,100$ worker-hours per day of idle compute capacity. Sub-second reset harnesses represent a mandatory mechanical requirement for large-scale trajectory harvesting.
:::
...
*Training moves no exposure, so the harvest must sample every exposure the agent meets.*
:::

To systematically populate the training distribution, runtime engineers organize fixtures into a *stratified task matrix*. This matrix organizes environment instances along the three H·S·A exposures of @sec-vol3-intro-hsa:

- *Horizon ($H$):* Stratified into shallow single-turn commands ($H = 1$), localized multi-turn workflows ($H \in [2, 5]$), and deep exploratory problem-solving ($H \in [10, 50]$).
- *State ($S$):* Every fixture carries an $S_1$ context and $S_2$ working files, so the matrix stratifies state by the staged context length $M$, partitioned into compact contexts ($2\text{k}$ tokens, focusing on localized AST refactoring), intermediate contexts ($8\text{k}\text{--}16\text{k}$ tokens, typical of multi-file module navigation), and massive repository-scale contexts ($32\text{k}\text{--}128\text{k}$ tokens, requiring hierarchical search and external memory retrieval).
- *Authority ($A$):* Categorized from read-only inspection at $A_0$ (using `grep`, `find`, or file viewing), to local sandboxed mutation at $A_1$ (such as editing source code and executing unit tests), up to coordinated multi-resource orchestration, the class of task that holds $A_2$ in production (database migrations, payment gateway calls, or inter-service RPCs). The fixture issues these calls against its own mocks, so the rollout itself stays at $A_1$ while the policy learns $A_2$ behavior.

Training moves none of these exposures, so the matrix exists to make the corpus cover each one the deployed agent will face.
...
- **Stage 5 (Semantic Triage)**: Applies advisory model scoring and human audit ($c_5 = 15.0\,\text{s}$, $p_5 = 0.70$) to suppress tautological solutions, with every rejection at this stage a human audit decision, culminating in a Golden Training Corpus with a final yield of $Y = 3.87\% \approx 3.9\%$ ($3{,}867$ of $100{,}000$ rollouts) at an amortized evaluation cost of $C_{\text{attempt}} = 3.15\,\text{s}$ per candidate.

Raw execution traces cannot be admitted into training corpora on self-reported completion or a single evaluation pass. Candidate trajectories instead traverse an escalating cascade of independent verifiers, from microsecond syntactic validation to multi-second dynamic isolation, and each stage filters invalid proposals at minimal computational cost. The stages climb the closure evidence levels of @sec-vol3-intro-closure-evidence. Stages 1 and 2 are static checks, Stage 3 runs tests, and the out-of-tree test mounts of @sec-vol3-verifier-gaming make those tests sealed rather than visible to the agent. Acceptance through this pipeline never establishes metaphysical correctness; it proves only that a trajectory has survived a finite sequence of rigorous empirical falsification checks.
...
Defending against harness subversion extends containment beneath the model (principle \ref{pri-vol3-zero-trust-sandboxing}) to the oracle itself, which must sit outside anything the agent's envelope can write. The agent runtime must enforce three defensive invariants across the execution environment:
...
***Test-tampering defense***\index{Test-tampering defense!definition}\index{Data flywheel!test tampering defense definition} is an execution isolation invariant $\text{PathDiff}(\mathcal{W}_{\text{src}}', \text{HEAD}) \cap \mathcal{W}_{\text{test}} = \emptyset$ requiring that verification test fixtures reside on immutable, read-only mounts physically segregated from the agent's mutable workspace during trajectory harvesting.

1.  **Significance**: Prevents specification gaming and reward hacking during automated trajectory collection, ensuring that policies internalize genuine code repair capabilities rather than learning to delete assertions, skip suites, or suppress error alarms.
2.  **Distinction**: Unlike naive acceptance filters (which trust guest process exit codes like `pytest` returning 0), test-tampering defense combines physical volume isolation with cryptographic diff validation to enforce that zero test assertions were altered.
3.  **Common pitfall**: Granting agents ambient write permissions to the repository root during data collection runs, allowing failing models to manufacture spurious successes by modifying test files or mocking out assertions.

:::

::: {#psp-12-bohrbugs-vs-heisenbugs-in-verification .callout-perspective title="Bohrbugs vs. Heisenbugs in automated trajectory verification"}
In his classic 1986 treatise on computer system reliability, Jim Gray established the fundamental distinction between Bohrbugs—deterministic software faults that manifest consistently under identical execution conditions—and Heisenbugs—transient, non-reproducible bugs whose symptoms disappear or mutate whenever one attempts to isolate them. In the context of autonomous agent trajectory harvesting, this distinction defines the boundary between sound curriculum generation and silent data contamination.

When an automated harvesting cluster evaluates thousands of candidate rollouts per day, environmental flakiness—such as network socket timeouts, DNS query jitter, unpinned dependency cache updates, and multi-threaded race conditions in the test suite—manifests as synthetic Heisenbugs. If the verification harness naively treats every test pass or failure as an authoritative signal, two catastrophic failure modes emerge. First, false negatives discard valid, innovative trajectories, wasting the GPU FLOPs invested in exploratory search. Second, and far more insidiously, false positives admit trajectories where tests passed due to timing flukes or skipped assertions, poisoning the supervised fine-tuning corpus with corrupted reasoning patterns.

An unprivileged foundation model trained on contaminated trajectories rapidly internalizes the environmental noise of the harvesting harness. Rather than learning to resolve underlying software faults, the model learns to emit timing-dependent hacks, exploit test suite timeouts, or retry operations until a transient network glitch clears.

**Systems insight**: Flaky tests are not minor testing nuisances; in autonomous data flywheels, they act as an adversarial entropy vector that corrupts downstream policy weights. Sound harvesting pipelines must isolate Heisenbugs through hermetic sandbox isolation, local dependency mirrors, and multi-pass re-execution before admitting any trajectory into the training distribution.
:::
...
*Progressive filtering sheds 93.5 percent of invalid proposals before expensive microVM test execution, cutting cluster compute by 68.6 percent.*
:::
...
Similarly, the cumulative harvesting yield $Y$, representing the fraction of raw candidate rollouts that successfully survive all $K$ validation stages, is given by the product of all conditional stage pass rates in @eq-verifier-cumulative-yield:
...
```{python}
#| label: lego-12-verification-cascade-economics-and-throughput
#| echo: false
# │ Show: Staged cascade cuts verifier compute by 68.6%, saving 191 core-hours/day.
# │ How: Microsecond AST checks and static filters shed 74% before microVM sandbox.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
...
class VerificationCascadeEconomicsThroughput:
    """Verification cascade economics and compute throughput optimization."""
    ...
    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    y_yield = p_1 * p_2 * p_3 * p_4 * p_5
    n_accepted = 3867

    c_naive = c_3 + (p_3 * c_2) + (p_3 * p_2 * c_5)
    t_naive = n_rollouts * c_naive
    t_naive_hours = t_naive.to(hour).magnitude

    c_cascade = c_1 + (p_1 * c_2) + (p_1 * p_2 * c_3) + (p_1 * p_2 * p_3 * c_4) + (p_1 * p_2 * p_3 * p_4 * c_5)
    t_cascade = n_rollouts * c_cascade
    t_cascade_hours = t_cascade.to(hour).magnitude

    compute_savings = (t_naive.magnitude - t_cascade.magnitude) / t_naive.magnitude
    core_hours_saved = t_naive_hours - t_cascade_hours

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(abs(y_yield - 0.038675) < 1e-6, "y_yield")
    check(abs(c_naive.to(second).magnitude - 10.03) < 1e-4, "c_naive")
    check(abs(t_naive_hours - 278.61) < 0.1, "t_naive_hours")
    check(abs(c_cascade.to(second).magnitude - 3.14605) < 1e-4, "c_cascade")
    check(abs(t_cascade_hours - 87.39) < 0.1, "t_cascade_hours")
    check(abs(compute_savings - 0.6863) < 0.001, "compute_savings")
    check(abs(core_hours_saved - 191.22) < 0.5, "core_hours_saved")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_accepted_str = fmt_int(n_accepted)
    compute_savings_pct_str = fmt_percent(compute_savings, precision=1, style="prose")
    core_hours_saved_str = fmt_int(round(core_hours_saved))
```

::: {#nbk-12-verification-cascade-economics-and-throughput .callout-notebook title="Verification cascade economics and throughput"}
**Problem Statement:**
...
Out of $100{,}000$ attempts, exactly `{python} VerificationCascadeEconomicsThroughput.n_accepted_str` trajectories are promoted to the training corpus.
...
The staged cascade reduces cluster compute consumption by `{python} VerificationCascadeEconomicsThroughput.compute_savings_pct_str`, saving over `{python} VerificationCascadeEconomicsThroughput.core_hours_saved_str` core-hours per day while accepting the exact same set of `{python} VerificationCascadeEconomicsThroughput.n_accepted_str` validated trajectories.
:::
...
::: {#chk-12-harvesting-cascade .callout-checkpoint title="Evaluating verifier cascades and harness defenses"}
Before analyzing recovery demonstration curation and corpus balancing, verify your understanding of staged verifier economics:

- [ ] **Cascade ordering**: Why must low-cost syntactic checks and static linters precede expensive dynamic microVM sandbox execution?
- [ ] **Test tampering**: How does mounting the test suite as a read-only volume (`mount -o ro`) prevent agents from deleting assertions to manufacture false successes?
- [ ] **State delta whitelisting**: Why must the verifier intersect the workspace git diff against an immutable path manifest before accepting a task as completed?
- [ ] **Economic yield**: How does progressive filtering optimize the total compute cost $C_{\text{attempt}}$ across a fleet harvesting thousands of daily trajectories?
:::
...
*Without recovery demonstrations, unforced errors trigger quadratic compounding that crosses a 50 percent failure knee at step 14.*
:::
...
*Multi-turn diagnostic loops cause recovery traces to consume 45 percent of the token budget despite comprising only 25 percent of trajectories.*
:::
...
```{python}
#| label: lego-12-sizing-and-token-budget-allocation
#| echo: false
# │ Show: Recovery traces generate 44.97% of supervised tokens despite 25% trace count.
# │ How: Model pristine, recovery, and hard negative sequence length distribution.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_memory, fmt_percent

class MixedCurationTokenBudgetAllocation:
    """Mixed curation token budget allocation and training sequence economics."""
    ...
    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    n_pristine = int(n_total * alpha)
    n_recover = int(n_total * beta)
    n_negative = int(n_total * gamma)

    v_pristine = (n_pristine * l_p.magnitude) * token
    v_recover = (n_recover * l_r.magnitude) * token
    v_negative = (n_negative * l_n.magnitude) * token
    v_total = v_pristine + v_recover + v_negative

    share_recover = v_recover.magnitude / v_total.magnitude

    s_packed = (v_total.magnitude * 2) * byte
    s_packed_gb = s_packed.to(GB)

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(n_pristine == 32_500, "n_pristine")
    check(n_recover == 12_500, "n_recover")
    check(n_negative == 5_000, "n_negative")
    check(v_pristine.magnitude == 234_000_000, "v_pristine")
    check(v_recover.magnitude == 231_250_000, "v_recover")
    check(v_negative.magnitude == 49_000_000, "v_negative")
    check(v_total.magnitude == 514_250_000, "v_total")
    check(abs(share_recover - 0.4497) < 0.001, "share_recover")
    check(abs(s_packed_gb.magnitude - 1.0285) < 0.001, "s_packed_gb")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_total_str = fmt_int(n_total)
    n_recover_str = fmt_int(n_recover)
    share_recover_pct_str = fmt_percent(share_recover, precision=2, style="prose")
    s_packed_gb_str = fmt_memory(s_packed_gb, unit=GB, precision=3)
```

::: {#nbk-12-sizing-and-token-budget-allocation .callout-notebook title="Mixed curation token budget allocation"}
...
Although recovery traces constitute only $25\%$ of the raw trajectory count (`{python} MixedCurationTokenBudgetAllocation.n_recover_str` out of `{python} MixedCurationTokenBudgetAllocation.n_total_str`), their diagnostic verbosity and extended turn count cause them to generate `{python} MixedCurationTokenBudgetAllocation.share_recover_pct_str` of all supervised training tokens.

At a standard float16 representation (2 bytes per parameter/token gradient footprint in data loaders), the dataset occupies over `{python} MixedCurationTokenBudgetAllocation.s_packed_gb_str` of packed token storage. More critically, the variance in sequence length ($\bar{L}_{\text{r}} = 18{,}500$ vs. $\bar{L}_{\text{p}} = 7{,}200$) imposes substantial memory-packing challenges during distributed training. If batches are padded naively to the longest sequence in the micro-batch, batches containing recovery traces will incur severe tail-padding overhead. The training pipeline must therefore implement dynamic sequence packing (e.g., concatenated multi-trajectory packing with cross-attention masking) to maintain high tensor-core utilization during gradient updates.
:::
...
- **Stage 2 (Rollout Fleet)** hosts unprivileged agent supervisors interfacing with vLLM continuous-batching GPU decoders ($P_{50} = 8\,\text{s}$). The supervisor holds the model to zero ambient authority, delegating all shell and compiler executions across mediated RPC boundaries.
...
The **Verification Worker Pool** dequeues raw trajectories and subjects them to the staged cascades established in @sec-vol3-flywheel-verification:
...
3. Stage 3 (if applicable) runs semantic checks ($V_3$) with isolated compiler toolchains. A secondary discriminator model may also score the trajectory to order human review, but, as @sec-vol3-verifier-funnel requires, it never admits or rejects a trajectory on its own.
...
```{python}
#| label: lego-12-sizing-intermediate-staging-buffers-and
#| echo: false
# │ Show: Staging buffer caps queue at 3,855 traces with high watermark at 2,891.
# │ How: Model rollout rate, nominal vs degraded verification, and RAM growth.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int

class StagingBuffersBackpressureSizing:
    """Staging buffer memory exhaustion and backpressure queue sizing."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_roll = 1024
    t_roll = 45 * second
    lambda_rollout = n_roll / t_roll.to(second).magnitude
    s_traj = 8.5 * MB
    n_ver = 512
    p_fast = 0.75
    t_fast = 2.5 * second
    ...
    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    t_verify_mean = (p_fast * t_fast.to(second).magnitude) + (p_slow * t_slow.to(second).magnitude)
    mu_verify = n_ver / t_verify_mean
    rho = lambda_rollout / mu_verify

    # Degraded
    t_timeout = 120.0 * second
    mu_degraded = n_ver / t_timeout.to(second).magnitude
    rho_degraded = lambda_rollout / mu_degraded
    delta_lambda = lambda_rollout - mu_degraded
    dm_dt = delta_lambda * s_traj.to(MB).magnitude

    m_max = 32 * 1024 * MB
    t_exhaust_s = m_max.to(MB).magnitude / dm_dt
    t_exhaust_min = t_exhaust_s / 60.0
    q_max = m_max.to(MB).magnitude / s_traj.to(MB).magnitude
    q_high = round(0.75 * q_max)

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(abs(lambda_rollout - 22.755) < 0.01, "lambda_rollout")
    check(abs(t_verify_mean - 16.875) < 1e-4, "t_verify_mean")
    check(abs(mu_verify - 30.34) < 0.01, "mu_verify")
    check(abs(rho - 0.750) < 0.005, "rho")
    check(abs(mu_degraded - 4.267) < 0.01, "mu_degraded")
    check(abs(rho_degraded - 5.333) < 0.01, "rho_degraded")
    check(abs(delta_lambda - 18.488) < 0.01, "delta_lambda")
    check(abs(dm_dt - 157.15) < 0.2, "dm_dt")
    check(abs(t_exhaust_s - 208.5) < 0.5, "t_exhaust_s")
    check(abs(t_exhaust_min - 3.48) < 0.05, "t_exhaust_min")
    check(round(q_max) == 3855, "q_max")
    check(q_high == 2891, "q_high")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    q_high_str = fmt_int(q_high)
```

::: {#nbk-12-sizing-intermediate-staging-buffers-and .callout-notebook title="Staging buffers and backpressure sizing"}
...
Setting a high-water mark at $Q_{\text{high}} = 0.75 \times Q_{\max} \approx$ `{python} StagingBuffersBackpressureSizing.q_high_str` trajectories allows the backpressure controller to immediately halt task leasing at the Task Dispatcher, forcing rollout workers to pause execution before the broker exhausts physical memory.
:::
...
```{python}
#| label: lego-12-storage-overhead-and-ingestion-latency
#| echo: false
# │ Show: Lineage envelope adds 2.32% storage overhead with 65.2 us SHA-256 latency.
# │ How: Merkle tree intermediate node digests, environment hashes, and streaming.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_memory, fmt_percent, fmt_time, MarkdownStr

class LineageEnvelopeStorageLatency:
    """Cryptographic lineage envelope storage overhead and hashing throughput."""
    ...
    s_gen = 2048 * byte
    s_obs = 1536 * byte
    s_raw = (t_steps * (s_gen.magnitude + s_obs.magnitude)) * byte
    s_raw_kb = s_raw.to(KB)

    s_envelope = (160 + 96 + 256 + 2016 + 128) * byte
    s_envelope_kb = s_envelope.to(KB)
    overhead_ratio = s_envelope.magnitude / s_raw.magnitude

    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    total_raw_gb_day = (n_traj * s_raw.to(GB).magnitude) * (GB / day)
    total_env_gb_day = (n_traj * s_envelope.to(GB).magnitude) * (GB / day)

    s_combined = s_raw + s_envelope
    b_hash = 1.8 * 1e9 * (byte / second)
    t_hash = (s_combined.magnitude / b_hash.magnitude) * second
    t_hash_us = t_hash.to(microsecond)
    throughput = 1.0 / t_hash.to(second).magnitude

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(s_raw.magnitude == 114_688, "s_raw")
    check(s_envelope.magnitude == 2656, "s_envelope")
    check(abs(overhead_ratio - 0.023158) < 0.0001, "overhead_ratio")
    check(abs(total_raw_gb_day.magnitude - 11.4688) < 0.01, "total_raw_gb_day")
    check(abs(total_env_gb_day.magnitude - 0.2656) < 0.01, "total_env_gb_day")
    check(abs(t_hash_us.magnitude - 65.19) < 0.1, "t_hash_us")
    check(throughput > 15_000, "throughput")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    total_raw_gb_day_str = MarkdownStr(f"{total_raw_gb_day.magnitude:.2f} GB/day")
    total_env_gb_day_str = MarkdownStr(f"{total_env_gb_day.magnitude:.2f} GB/day")
    throughput_str = fmt_int(round(throughput))
```

::: {#nbk-12-storage-overhead-and-ingestion-latency .callout-notebook title="Lineage envelope storage and latency"}
...
Across $100{,}000$ daily trajectories, the total raw payload is `{python} LineageEnvelopeStorageLatency.total_raw_gb_day_str`, and the cryptographic lineage envelope accounts for merely `{python} LineageEnvelopeStorageLatency.total_env_gb_day_str`.
...
A single dedicated validation thread can cryptographically seal over `{python} LineageEnvelopeStorageLatency.throughput_str` trajectories per second, introducing negligible latency into the ingestion pipeline.
:::
...
::: {#chk-12-exposure-bias .callout-checkpoint title="Evaluating recovery curation and pipeline hygiene"}
Before examining split partitioning and environment contamination vectors, verify your understanding of trajectory curation:

- [ ] **Covariate shift**: Why does fine-tuning exclusively on error-free golden trajectories cause policies to collapse when encountering runtime tool errors?
- [ ] **Curriculum balance**: What proportional balance between linear successes, recovered trajectories, and mined hard negatives maximizes policy resilience?
- [ ] **Backpressure governance**: How do watermarked buffer queues prevent high-latency container sandboxes from starving upstream generation workers?
- [ ] **Lineage envelopes**: Why must harvested trajectories preserve cryptographic parent hashes, tool schemas, and environment fingerprints?
:::
...
Two trajectories solving the same bug in identical repositories may exhibit less than 40 percent lexical token overlap due to divergent chain-of-thought scratchpads, variable tool argument ordering, and non-deterministic process IDs. Yet structurally, both rely on identical environmental invariants. Classical MinHash LSH marks them as distinct, hiding the underlying contamination.
:::
...
3. *Recovery-Augmented Mixture:* A curated balance consisting of 70 percent pristine trajectories and 30 percent structured recovery demonstrations (synthesized via the divergence and checkpoint rollback techniques established in @sec-vol3-flywheel-curation).
...
We formalize the economic cost per usable trajectory, $C_{\text{usable}}$, in @eq-cost-usable:
...
If an organization runs a sloppy collection pipeline with a 30 percent leakage rate—meaning 30 percent of its collected tasks inadvertently duplicate repositories, schemas, or fixtures reserved for evaluation—the denominator shrinks, dramatically inflating the amortized unit cost of every valid training token.

```{python}
#| label: lego-12-the-economic-cost-of-contaminated
#| echo: false
# │ Show: 25% evaluation leakage inflates cost by 33.3% and wastes $22,917 on 50k traces.
# │ How: Amortize generation, verification, and sanitization across usable yield.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_usd

class ContaminatedYieldEconomics:
    """Economic cost of contaminated evaluation leakage in trajectory harvesting."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    price_token = 0.75 / 1_000_000
    n_tokens_rollout = 40_000
    k_attempts = 5

    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    c_gen_attempt = n_tokens_rollout * price_token
    c_generation = k_attempts * c_gen_attempt

    c_ver_attempt = 0.020
    c_verification = k_attempts * c_ver_attempt

    c_san_attempt = 0.005
    c_sanitization = k_attempts * c_san_attempt

    c_task = c_generation + c_verification + c_sanitization
    n_accepted_per_100 = 20
    c_nominal = (100 * c_task) / n_accepted_per_100

    leakage_a = 0.25
    c_usable_a = (100 * c_task) / (n_accepted_per_100 * (1 - leakage_a))

    leakage_b = 0.00
    c_usable_b = (100 * c_task) / (n_accepted_per_100 * (1 - leakage_b))

    cost_increase = (c_usable_a - c_usable_b) / c_usable_b
    waste_50k = 50_000 * (c_usable_a - c_usable_b)

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(abs(c_generation - 0.150) < 1e-4, "c_generation")
    check(abs(c_verification - 0.100) < 1e-4, "c_verification")
    check(abs(c_sanitization - 0.025) < 1e-4, "c_sanitization")
    check(abs(c_task - 0.275) < 1e-4, "c_task")
    check(abs(c_nominal - 1.375) < 1e-4, "c_nominal")
    check(abs(c_usable_a - 1.8333) < 0.001, "c_usable_a")
    check(abs(c_usable_b - 1.375) < 1e-4, "c_usable_b")
    check(abs(cost_increase - 0.3333) < 0.001, "cost_increase")
    check(abs(waste_50k - 22916.6) < 100.0, "waste_50k")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    waste_50k_str = fmt_usd(round(waste_50k), precision=0)
```

::: {#nbk-12-the-economic-cost-of-contaminated .callout-notebook title="The economic cost of contaminated yield"}
...
**Conclusion:**
Incurring a 25 percent leakage rate increases the effective unit cost of training data by:
$$\frac{\$1.833 - \$1.375}{\$1.375} = +33.3\%$$
For a production run targeting 50,000 usable trajectories, Regime A wastes `{python} ContaminatedYieldEconomics.waste_50k_str` in lost inference and compute cycles solely due to downstream split hygiene purges. Rigorous upfront partitioning is an economic necessity, not merely an academic formality.
:::
...
The harvesting pipeline originates at the agent runtime supervisor. As established in the host architecture, an unprivileged agent model executes under zero ambient authority, interacting with its environment exclusively through mediated Remote Procedure Calls (RPCs). Every prompt assembly, decoded output token, tool call invocation, standard output stream, and sandbox filesystem modification is appended sequentially to an append-only Write-Ahead Log backed by an Agent Control Block (ACB).
...
When $R_{\text{flake}} > 0.02$ (greater than 2 percent), the pipeline halts automatic ingestion, as non-deterministic tests allow invalid trajectories to pollute the training corpus while discarding valid recovery paths.
...
```{python}
#| label: lego-12-sizing-an-industrial-trajectory-harvester
#| echo: false
# │ Show: 122 CPU workers verify 50k traces for $0.0165/trace vs $0.2952 generation.
# │ How: Model V1-V3 cascade work, 14.11% yield, and 24-hour throughput limits.
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim import *
from mlsysim.core.units import *
...
class IndustrialTrajectoryHarvesterSizing:
    """Industrial trajectory harvester compute sizing and verification economics."""
    ...
    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    eta_yield = p_1 * p_2 * p_3 * p_4
    n_accepted = round(n_generated * eta_yield)

    w_1 = n_generated * 0.001
    w_2 = (n_generated * p_1) * 0.05
    w_3 = (n_generated * p_1 * p_2) * 0.15
    w_san = (n_generated * p_1 * p_2 * p_3) * 0.002
    w_total = w_1 + w_2 + w_3 + w_san

    workers_required = math.ceil(w_total / 24.0)

    rate_cpu = 0.04
    cost_verify = w_total * rate_cpu
    cost_verify_per_accepted = cost_verify / n_accepted

    w_gpu = n_generated * (1.0 / 60.0)
    rate_gpu = 2.50
    cost_generation = w_gpu * rate_gpu
    cost_gen_per_accepted = cost_generation / n_accepted

    cost_total_per_accepted = cost_gen_per_accepted + cost_verify_per_accepted
    verify_fraction = cost_verify_per_accepted / cost_total_per_accepted
    defective_eliminated = 1.0 - eta_yield

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(abs(eta_yield - 0.14112) < 1e-5, "eta_yield")
    check(n_accepted == 7056, "n_accepted")
    check(abs(w_total - 2914.4) < 0.1, "w_total")
    check(workers_required == 122, "workers_required")
    check(abs(cost_verify - 116.576) < 0.01, "cost_verify")
    check(abs(cost_verify_per_accepted - 0.01652) < 0.0001, "cost_verify_per_accepted")
    check(abs(cost_generation - 2083.33) < 0.1, "cost_generation")
    check(abs(cost_gen_per_accepted - 0.2952) < 0.001, "cost_gen_per_accepted")
    check(abs(cost_total_per_accepted - 0.3117) < 0.001, "cost_total_per_accepted")
    check(abs(verify_fraction - 0.053) < 0.001, "verify_fraction")
    check(abs(defective_eliminated - 0.85888) < 0.001, "defective_eliminated")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    verify_fraction_pct_str = fmt_percent(verify_fraction, precision=1, style="prose")
    defective_eliminated_pct_str = fmt_percent(defective_eliminated, precision=1, style="prose")
```

::: {#nbk-12-sizing-an-industrial-trajectory-harvester .callout-notebook title="Sizing an industrial trajectory harvester"}
...
Notice that verification represents only `{python} IndustrialTrajectoryHarvesterSizing.verify_fraction_pct_str` of total pipeline expenditure, yet it eliminates `{python} IndustrialTrajectoryHarvesterSizing.defective_eliminated_pct_str` of defective, misleading, or toxic training data.
:::
...
## Fallacies and Pitfalls {#sec-vol3-data-flywheel-fallacies}
...
```{python}
#| label: lego-12-sample-efficiency-and-compute-budgets
#| echo: false
# │ Show: Staged verification cuts training compute by 29.5x with 43.6% zero-shot pass.
# │ How: Compare 100k raw traces at 6.5k tokens vs 10k curated traces at 2.2k tokens.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_multiple, fmt_percent

class TrajectoryDistillationComputeBudgets:
    """Training compute footprint and sample efficiency in trajectory distillation."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    p_params = 70 * 10**9
    flops_per_token = 6 * p_params

    n_a = 100_000
    l_a = 6500 * token
    tokens_a = n_a * l_a.magnitude
    compute_a = flops_per_token * tokens_a

    n_b = 10_000
    l_b = 2200 * token
    tokens_b = n_b * l_b.magnitude
    compute_b = flops_per_token * tokens_b

    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    compute_ratio = compute_a / compute_b
    fraction_b = compute_b / compute_a

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(abs(compute_a - 2.73e20) < 1e18, "compute_a")
    check(abs(compute_b - 9.24e18) < 1e16, "compute_b")
    check(abs(compute_ratio - 29.545) < 0.1, "compute_ratio")
    check(abs(fraction_b - 0.03385) < 0.001, "fraction_b")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    fraction_b_upper_pct_str = fmt_percent(0.04, precision=0, style="prose")
```

::: {#nbk-12-sample-efficiency-and-compute-budgets .callout-notebook title="Trajectory distillation compute budgets"}
...
Trajectory harvesting turned verified trajectory post-training (principle \ref{pri-vol3-trajectory-post-training}) from a rule into a procedure. The intervention ladder puts weights last, so harvesting targets only the failures that survive triage, the verifier cascade admits a trajectory only on evidence produced outside the model, and the recovery channel supplies the one kind of trace a clean corpus lacks. The tamper defense then showed that the invariant closure principle (\ref{pri-invariant-closure}) governs training data as strictly as it governs live actions, because a test the agent can edit admits exactly the trajectories a policy should never learn from.
