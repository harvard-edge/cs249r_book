...
To diagnose execution failures with architectural precision, systems engineers must anchor their analysis in the physical reality of the model invocation boundary. As established across Parts I and III, an unprivileged foundation model operates under Zero Ambient Authority ($A = 0$).[^fn-fail-plausible-harvest] When an agent trajectory fails, the system does not crash with a deterministic hardware exception; foundation models are fundamentally *fail-plausible*. The model smoothly emits syntactically valid JSON payloads or shell invocations that reference invalid paths or make false assumptions about host state.

Blaming the base model for every operational fault conflates visible symptoms with systems root causes. An execution failure does not automatically indicate that model weights must be fine-tuned. The failure may stem from a missing context file, an ambiguous tool schema, or a microVM timeout. Fine-tuning a 70-billion-parameter model to compensate for an ephemeral container timeout or an unhandled socket disconnect is a catastrophic misuse of engineering resources.

[^fn-fail-plausible-harvest]: **Fail-Plausible Triage**: First formalized in @sec-vol3-intro-fail-plausible and @sec-vol3-processor; this chapter develops the forensic pipeline required to classify whether trajectory failures originate in working context, tool schemas, sandbox runtimes, or true policy incapability.
...
A curated execution trajectory containing intermediate environment faults, tool errors, or incorrect hypotheses paired with successful diagnostic localization and compensating actions, explicitly included in training data to counter covariate shift and teach policy self-healing.
:::
