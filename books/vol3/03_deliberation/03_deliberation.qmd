...
*In unverified execution, compounding errors drive trajectory survival below 15 percent after 40 steps.*
:::
...
$$y_t \sim P(y_t \mid x, y_{<t}; \Theta) = \text{softmax}\left(\frac{\mathbf{z}_t}{\tau}\right)$$
...
This separation of concerns operationalizes Dijkstra's [-@dijkstra1970structured] testing principle (introduced in @sec-vol3-intro-operational-incident):[^fn-dijkstra-testing-deliberation] empirical test execution demonstrates the presence of bugs but never their absence. In an agentic deliberation process, an empirical test execution provides an indispensable systems property: *discriminatory power*. An external execution probe can definitively falsify an invalid hypothesis. When a concurrency test runner emits a nonzero exit code or an assertion failure, that empirical observation injects ground-truth evidence directly into the runtime's state.

[^fn-dijkstra-testing-deliberation]: **Dijkstra Testing Principle**: As formalized in @sec-vol3-intro-operational-incident, deterministic test execution establishes empirical falsification across the deliberation tree, separating valid reasoning branches from plausible hallucinations.
...
To safeguard the environment, the runtime must enforce that exploratory search operates exclusively within isolated scratchpads or sandboxes (as formalized in @sec-vol3-virtualization). Physical environment commits are deferred until the search algorithm terminates, the stopping criteria are satisfied, and a candidate trajectory is formally selected and certified.
