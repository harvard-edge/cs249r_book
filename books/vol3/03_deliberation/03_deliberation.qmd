...
@Algo-mcts-radix-cache integrates the four PUCT search phases with copy-on-write key-value cache page tables. Rather than recomputing prompt prefixes across candidate branches or duplicating context in device memory, the expansion phase forks the parent node's physical cache handle, pinning shared prompt pages in high-bandwidth memory (HBM) and allocating new pages only for divergent continuation tokens.
...
2. **Zero-Copy Expansion via `ForkPrefixHandle` (Lines 8–13):** This is the core architectural innovation. In a naive implementation, expanding $K$ candidate reasoning branches would require either:
...
@Algo-watchdog-circuit-breaker formalizes this out-of-band monitoring loop across step, token, error, and physical progress dimensions. On each agent control step, the watchdog evaluates four orthogonal health invariants: total step count against task ceiling $K_{\max}$, cumulative token consumption against budget $T_{\max}$, consecutive execution errors against burst limit $E_{\max}$, and the physical environment progress delta $\Delta \mathcal{S}_{\text{env}}$ established in @tbl-vol3-deliberation-cycle-detection.
...
4. **The Half-Open Canary Budget (Lines 13–14 & 20–23):** Rather than terminating execution abruptly on transient environment friction, the breaker transitions from `CLOSED` to `HALF-OPEN`. In this degraded state, the runtime modifies the agent's operating context:
