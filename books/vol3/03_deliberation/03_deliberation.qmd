...
By coupling tree expansion directly to reference-counted page tables, the memory footprint of deliberation scales linearly with the active search frontier rather than combinatorially with the total number of sampled reasoning tokens.
...
Operating the watchdog out-of-band ensures that even when an agent policy enters an unconstrained livelock or hallucinatory loop, the supervisor arrests execution before runaway token expenditure exhausts cluster quotas or corrupts downstream state.
