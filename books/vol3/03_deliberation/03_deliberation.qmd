...
class MCTSNode:
    """Deliberation tree node backed by a copy-on-write KV-cache page handle."""

    def __init__(self, parent: Optional["MCTSNode"] = None, prior: float = 1.0, cache_handle=None):
        self.parent, self.prior = parent, prior
        self.cache_handle = cache_handle  # Reference-counted physical page in Radix cache
    ...
    def puct(self, c: float = 1.414) -> float:
        u = c * self.prior * math.sqrt(self.parent.visits) / (1 + self.visits)
        return self.q_value + u


def search_step(root: MCTSNode, expand_fn, eval_fn) -> None:
    """Executes a 4-phase MCTS iteration with CoW prefix cache fork & backpropagation."""
    # 1. Select: traverse path maximizing PUCT
    curr, path = root, [root]
    while curr.children:
        curr = max(curr.children.values(), key=lambda n: n.puct())
        path.append(curr)

    # 2. Expand: fork candidate children sharing parent's KV prefix handle
    if not curr.children:
        for action, prior in expand_fn(curr).items():
            child_handle = curr.cache_handle.fork() if curr.cache_handle else None
            curr.children[action] = MCTSNode(parent=curr, prior=prior, cache_handle=child_handle)

    # 3. Evaluate & 4. Backpropagate: update visit counts and Q-values along search path
    val = eval_fn(curr)
...
def compute_optimal_cadence(t_step: float, t_ver: float, reliability_p: float) -> int:
    """Evaluates Young-Daly verification cadence: m* = sqrt(2 * T_ver / ((1 - p) * T_step))."""
    ratio = (2.0 * t_ver) / (max(1.0 - reliability_p, 1e-4) * t_step)
...
class WatchdogCircuitBreaker:
    ...
    def __init__(self, max_steps: int = 30, max_tokens: int = 50_000, error_thresh: int = 3):
    ...
    def step(self, tokens_used: int, error: bool = False, delta_env: float = 1.0) -> CircuitState:
        """Evaluates trajectory health invariants on each agent control step."""
        self.steps += 1
        self.tokens += tokens_used
        self.consecutive_errors = (self.consecutive_errors + 1) if error else 0

        # Hard invariant violations trip immediately to OPEN
        if (
            self.steps > self.max_steps
            or self.tokens > self.max_tokens
            or self.consecutive_errors >= self.error_thresh
        ):
            self.state = CircuitState.OPEN
        elif delta_env <= 0.0:  # Stagnation trips to HALF_OPEN degraded remediation
            self.state = CircuitState.HALF_OPEN if self.state == CircuitState.CLOSED else CircuitState.OPEN
        elif self.state == CircuitState.HALF_OPEN and not error:
            self.state = CircuitState.CLOSED  # Remediated progress restores normal operation
