...
```{python}
#| label: lego-03-memory-bus-latency-tax
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ SIZING THE MEMORY BUS AND LATENCY TAX OF AUTOREGRESSIVE COMMITMENT (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-03-memory-bus-latency-tax
# │ Goal: Dimension memory bandwidth consumption and waste under premature hypothesis commitment.
# │ Show: 1,024-token pass moves 71.68 TB; committing to a false premise at step 16 wastes 21.06 s (98.4%).
# │ How: Model footprint divided by HBM bandwidth gives per-token latency; accumulate over token count.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_memory

class MemoryBusLatencyTax:
    """Memory bus and latency tax of autoregressive commitment."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    p_params = 70e9
    bytes_per_weight = 1 * byte
    bw_mem = 3.35 * (TB / second)
    l_tokens = 1_024
    t_commit = 16
    t_probe = 40 * millisecond

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    weight_footprint = (p_params * bytes_per_weight).to(GB)
    t_decode = (weight_footprint.to(byte) / bw_mem).to(second)
    t_total = (l_tokens * t_decode).to(second)
    v_mem_total = (l_tokens * weight_footprint).to(TB)
    tokens_wasted = l_tokens - t_commit
    t_wasted = (tokens_wasted * t_decode).to(second)
    pct_wasted = (t_wasted / t_total).magnitude
    v_mem_wasted = (tokens_wasted * weight_footprint).to(TB)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_decode.to(millisecond).magnitude - 20.90) < 0.05, "t_decode")
    check(abs(t_total.magnitude - 21.40) < 0.05, "t_total")
    check(abs(v_mem_total.magnitude - 71.68) < 0.05, "v_mem_total")
    check(tokens_wasted == 1008, "tokens_wasted")
    check(abs(t_wasted.magnitude - 21.07) < 0.05, "t_wasted")
    check(abs(pct_wasted - 0.984) < 0.005, "pct_wasted")
    check(abs(v_mem_wasted.magnitude - 70.56) < 0.05, "v_mem_wasted")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    l_tokens_str = fmt_int(l_tokens)
    t_total_s_str = fmt_time(t_total, unit=second, precision=2)
    v_mem_total_tb_str = fmt_memory(v_mem_total, unit=TB, precision=2)
    t_commit_str = fmt_int(t_commit)
    tokens_wasted_str = fmt_int(tokens_wasted)
    t_wasted_s_str = fmt_time(t_wasted, unit=second, precision=2)
    pct_wasted_pct_str = fmt_percent(pct_wasted, precision=1, style="prose")
    v_mem_wasted_tb_str = fmt_memory(v_mem_wasted, unit=TB, precision=2)
...
Applying the single-token memory bandwidth floor derived in @sec-vol3-processor-cost, per-token decode latency is strictly memory-bandwidth bound:
...
Suppose the model generates an open-loop diagnostic rationale and patch consisting of $L =$ `{python} MemoryBusLatencyTax.l_tokens_str` output tokens:

- Total decode wall-clock time: $T_{\text{total}} = 1{,}024 \times 20.90\text{ ms} =$ `{python} MemoryBusLatencyTax.t_total_s_str`.
- Total memory volume shuttled across the HBM bus: $V_{\text{mem}} = 1{,}024 \times 70\text{ GB} =$ `{python} MemoryBusLatencyTax.v_mem_total_tb_str`.

**The Cost of Premise Poisoning:**
Suppose the prompt contains ambiguous telemetry where the model's pretraining prior assigns probability $P(H_1) = 0.75$ to a heartbeat timeout and $P(H_2) = 0.25$ to a lease race. At token position $t =$ `{python} MemoryBusLatencyTax.t_commit_str`, the decode loop samples token $y_{16} = \texttt{"heartbeat"}$, committing the KV cache to $H_1$.

- Tokens $y_{17}$ through $y_{1024}$ (`{python} MemoryBusLatencyTax.tokens_wasted_str` tokens) are spent elaborating and implementing the false hypothesis.
- Wasted wall-clock latency: $1{,}008 \times 20.90\text{ ms} = 21.07\text{ seconds}$ (`{python} MemoryBusLatencyTax.pct_wasted_pct_str` of total generation time).
- Wasted memory bus traffic: $1{,}008 \times 70\text{ GB} =$ `{python} MemoryBusLatencyTax.v_mem_wasted_tb_str`.
...
Now consider an agent runtime that pauses generation at $t = 16$ and executes an external diagnostic probe (a lightweight concurrency fuzzer script) taking $T_{\text{probe}} =$ `{python} MemoryBusLatencyTax.t_probe_ms_str` of CPU time and consuming zero GPU memory bandwidth. The probe emits 256 bytes of stdout confirming that the lease lock was dropped prematurely. This observation collapses the epistemic entropy of the hypothesis space:

$$H(\mathcal{H}) = -\sum_{i=1}^2 P(H_i)\log_2 P(H_i) = -(0.75 \log_2 0.75 + 0.25 \log_2 0.25) \approx 0.811\text{ bits} \longrightarrow 0\text{ bits}$$

Spending `{python} MemoryBusLatencyTax.t_probe_ms_str` on an external execution probe saves `{python} MemoryBusLatencyTax.t_wasted_s_str` of wasted GPU decode latency, conserves `{python} MemoryBusLatencyTax.v_mem_wasted_tb_str` of memory bus traffic, and prevents downstream operational failure.
:::
...
where $t_{\text{step}}$ is the latency of a single decode step. At small batch sizes $t_{\text{step}}$ is set by memory bandwidth rather than compute (@sec-vol3-processor-cost), so depth pays the full weight read for every token it adds.
...
```{python}
#| label: lego-03-hardware-trade-offs-across-compute-allocation
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ HARDWARE TRADE-OFFS ACROSS COMPUTE ALLOCATION AXES (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-03-hardware-trade-offs-across-compute-allocation
# │ Goal: Compare compute throughput, latency, and goodput across depth, breadth, and feedback axes.
# │ Show: Feedback (Config C) uses 2.37x FLOPs but yields 100% goodput; depth and breadth yield 0%.
# │ How: Model prefill and decode latencies against empirical test and execution costs.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time, fmt_multiple

class ComputeAllocationAxes:
    """Hardware trade-offs across depth, breadth, and feedback axes."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_gpus = 8
    n_params = 70e9
    bytes_per_elem = 2 * byte
    bw_mem = 26.8 * (TB / second)
    r_peak = 15_800e12 / second
    p_prompt = 4_096
    k_ext = 4_096
    n_breadth = 4
    k_breadth = 1_024
    r_feedback = 3
    k_feedback = 1_024
    t_oracle = 1.50 * second
    o_diag = 340

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    t_prefill_a = ((2 * n_params * p_prompt) / r_peak.magnitude) * second
    t_decode_per_token_b1 = ((n_params * bytes_per_elem) / bw_mem).to(second)
    t_decode_a = (k_ext * t_decode_per_token_b1).to(second)
    t_wall_a = (0.036 + 21.384) * second
    t_decode_b = (k_breadth * 5.35 * millisecond).to(second)
    t_wall_b = t_prefill_a + t_decode_b
    flops_feedback = 2.723e15
    flops_base = 2 * n_params * (p_prompt + k_ext)
    mult_flops = flops_feedback / flops_base
    t_wall_c = 20.70 * second
    t_oracle_total = r_feedback * t_oracle

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_wall_a.magnitude - 21.42) < 0.05, "t_wall_a")
    check(abs(t_wall_b.magnitude - 5.51) < 0.1, "t_wall_b")
    check(abs(mult_flops - 2.37) < 0.05, "mult_flops")
    check(abs(t_wall_c.magnitude - 20.70) < 0.05, "t_wall_c")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_gpus_str = fmt_int(n_gpus)
    p_prompt_str = fmt_int(p_prompt)
    k_ext_str = fmt_int(k_ext)
    n_breadth_str = fmt_int(n_breadth)
    k_breadth_str = fmt_int(k_breadth)
    r_feedback_str = fmt_int(r_feedback)
    k_feedback_str = fmt_int(k_feedback)
    t_oracle_s_str = fmt_time(t_oracle, unit=second, precision=2)
    o_diag_str = fmt_int(o_diag)
    mult_flops_mult_str = fmt_multiple(mult_flops, precision=2)
    t_wall_c_s_str = fmt_time(t_wall_c, unit=second, precision=2)
    t_wall_a_s_str = fmt_time(t_wall_a, unit=second, precision=2)
```
...
Consider an autonomous agent runtime deployed on an `{python} ComputeAllocationAxes.n_gpus_str`-GPU node running an unquantized 70-billion parameter model ($|\Theta| = 70 \times 10^9$) in BF16 precision ($b_{\text{bytes}} = 2$ bytes). The node possesses an aggregate memory bandwidth $B_{\text{mem}} = 26.8\text{ TB/s}$ ($3.35\text{ TB/s}$ per GPU) and sustained BF16 matrix compute throughput of $R_{\text{peak}} = 15{,}800\text{ TFLOPS}$.

The agent is tasked with synthesizing a lockless ring buffer. The base specification prompt contains $P =$ `{python} ComputeAllocationAxes.p_prompt_str` tokens. We analyze three concrete configurations designed to consume comparable total generation budgets of approximately $4{,}096$ emitted tokens:

1. **Configuration A (Depth):** A single serial rollout of $K_{\text{ext}} =$ `{python} ComputeAllocationAxes.k_ext_str` intermediate reasoning and implementation tokens.
2. **Configuration B (Breadth):** An $N =$ `{python} ComputeAllocationAxes.n_breadth_str` parallel sampling sweep, where each branch emits $K =$ `{python} ComputeAllocationAxes.k_breadth_str` tokens concurrently.
3. **Configuration C (Feedback):** An $R =$ `{python} ComputeAllocationAxes.r_feedback_str` round iterative repair loop. Each round emits $K =$ `{python} ComputeAllocationAxes.k_feedback_str` tokens, followed by an external compilation and ThreadSanitizer test pass taking $t_{\text{oracle}} =$ `{python} ComputeAllocationAxes.t_oracle_s_str` that yields $|O_r| =$ `{python} ComputeAllocationAxes.o_diag_str` tokens of diagnostic traces.
...
- **Outcome and Trajectory Goodput:** Configuration C consumes `{python} ComputeAllocationAxes.mult_flops_mult_str` more total FLOPs than Configurations A and B due to the quadratic accumulation of context in the prefill phases. Its wall-clock latency (`{python} ComputeAllocationAxes.t_wall_c_s_str`) is nearly identical to single-shot depth (`{python} ComputeAllocationAxes.t_wall_a_s_str`). Yet evaluated through the systems lens of Trajectory Goodput ($\mathcal{G}$), Configuration A and Configuration B yielded $\mathcal{G} = 0\%$ (100 percent badput: $\$0.178$ of GPU compute wasted producing non-functional code), whereas Configuration C yielded $\mathcal{G} = 100\%$ goodput: in Round 2, ThreadSanitizer flagged the missing memory barrier, providing the exact line and register offset required for Round 3 to emit a verified, race-free barrier. Feedback is the only architecture that converts raw FLOPs into verified systems deliverables.
:::
...
```{python}
#| label: lego-03-candidate-sampling-and-verifier-throughput
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ CANDIDATE SAMPLING AND VERIFIER THROUGHPUT (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-03-candidate-sampling-and-verifier-throughput
# │ Goal: Dimension memory bus vs compute-bound overhead of candidate generation and PRM scoring.
# │ Show: 16 candidates take 21.4 s decode + 5.73 s PRM scoring (27.13 s total); 75% filter drops PRM to 1.43 s.
# │ How: Weight memory reads against GEMM throughput for 16,384 candidate tokens.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_bandwidth

class CandidateSamplingVerifierThroughput:
    """Candidate sampling and verifier throughput under fixed GPU memory bandwidth."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    bw_hbm = 3_350.0 * (GB / second)
    n_candidates = 16
    l_tokens = 1_024
    p_params = 70e9
    throughput_prm = 400e12 / second
    filter_drop = 0.75

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    m_weights = (p_params * 1 * byte).to(GB)
    t_read_step = (m_weights / bw_hbm).to(second)
    t_decode = (l_tokens * t_read_step).to(second)
    tokens_total = n_candidates * l_tokens
    flops_prm = 2 * p_params * tokens_total
    t_prm = (flops_prm / throughput_prm.magnitude) * second
    t_total_deliberation = t_decode + t_prm
    t_prm_filtered = t_prm * (1.0 - filter_drop)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_read_step.to(millisecond).magnitude - 20.9) < 0.05, "t_read_step")
    check(abs(t_decode.magnitude - 21.4) < 0.05, "t_decode")
    check(tokens_total == 16384, "tokens_total")
    check(abs(t_prm.magnitude - 5.73) < 0.05, "t_prm")
    check(abs(t_total_deliberation.magnitude - 27.13) < 0.05, "t_total_deliberation")
    check(abs(t_prm_filtered.magnitude - 1.43) < 0.05, "t_prm_filtered")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    bw_hbm_gb_per_s_str = fmt_bandwidth(bw_hbm, unit=GB / second, precision=0)
    n_candidates_str = fmt_int(n_candidates)
    l_tokens_str = fmt_int(l_tokens)
    t_decode_s_str = fmt_time(t_decode, unit=second, precision=1)
    tokens_total_str = fmt_int(tokens_total)
    t_prm_s_str = fmt_time(t_prm, unit=second, precision=2)
    t_total_deliberation_s_str = fmt_time(t_total_deliberation, unit=second, precision=2)
    filter_drop_pct_str = fmt_percent(filter_drop, precision=0, style="prose")
    t_prm_filtered_s_str = fmt_time(t_prm_filtered, unit=second, precision=2)
```
...
A systems engineer deploys an agentic code generation supervisor running on an NVIDIA H100 SXM5 GPU (`{python} CandidateSamplingVerifierThroughput.bw_hbm_gb_per_s_str` HBM3 memory bandwidth). The system generates $N =$ `{python} CandidateSamplingVerifierThroughput.n_candidates_str` candidate solutions in parallel, each generating an average of $L =$ `{python} CandidateSamplingVerifierThroughput.l_tokens_str` decode tokens. The base generator is a 70-billion parameter dense model stored in FP8 ($P = 70 \times 10^9$ bytes).

Decode is bound by memory bandwidth (@sec-vol3-processor-cost), so each decode step reads the full 70 GB of weights from HBM. Evaluating candidates using a learned Process Reward Model (PRM) of identical size requires an independent forward prefill pass over the generated tokens.
...
For $L =$ `{python} CandidateSamplingVerifierThroughput.l_tokens_str` autoregressive steps, the minimum execution time spent sweeping model weights across the bus is:
$$T_{\text{decode}} = 1,024 \times 20.9 \text{ ms} = 21.4 \text{ seconds}$$

**2. Candidate Verification Phase (PRM Prefill):**
The `{python} CandidateSamplingVerifierThroughput.n_candidates_str` generated candidates collectively total $16 \times 1,024 =$ `{python} CandidateSamplingVerifierThroughput.tokens_total_str` tokens. Unlike decode, the PRM evaluates these tokens in the compute-bound prefill phase (GEMM). Assuming the H100 achieves an effective throughput of $400 \times 10^{12}$ FP8 FLOPs/sec on prefill, and the model requires $2P$ FLOPs per token:
...
**Systems Implication:**
Generating the candidates consumes `{python} CandidateSamplingVerifierThroughput.t_decode_s_str` on the memory bus, while learned verification adds `{python} CandidateSamplingVerifierThroughput.t_prm_s_str` of compute time, bringing total deliberation latency to `{python} CandidateSamplingVerifierThroughput.t_total_deliberation_s_str`. If the runtime deployed a deterministic static filter that dropped `{python} CandidateSamplingVerifierThroughput.filter_drop_pct_str` of candidates prior to PRM scoring, the verification phase latency would drop from `{python} CandidateSamplingVerifierThroughput.t_prm_s_str` to `{python} CandidateSamplingVerifierThroughput.t_prm_filtered_s_str`, saving over four seconds of critical-path execution time.
:::
...
```{python}
#| label: lego-03-compute-overhead-and-yield-under
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ COMPUTE OVERHEAD AND YIELD UNDER ASYMMETRIC VERIFIER ERROR (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-03-compute-overhead-and-yield-under
# │ Goal: Dimension posterior candidate precision and risk-weighted incident cost under asymmetric verifier error.
# │ Show: 70.31% posterior precision; $608 risk-weighted incident damage outstrips $0.178 compute cost by 3,400x.
# │ How: Compound false negative and false positive rates over 64 parallel candidate samples.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_usd

class AsymmetricVerifierYield:
    """Compute overhead and yield under asymmetric verifier error."""
    ...
    alpha_far = 0.02
    cost_gpu_hr = 2.50
    cost_incident = 500.00

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    n_valid = n_candidates * p_success
    n_invalid = n_candidates * (1.0 - p_success)
    valid_accepted = n_valid * (1.0 - beta_frr)
    valid_rejected = n_valid * beta_frr
    invalid_rejected = n_invalid * (1.0 - alpha_far)
    invalid_accepted = n_invalid * alpha_far
    n_accepted = valid_accepted + invalid_accepted
    precision_accepted = valid_accepted / n_accepted
    t_wall = (l_tokens / r_decode.magnitude) * second
    t_gpu_s = (n_gpus * t_wall).to(second)
    t_gpu_hr = t_gpu_s.to(hour)
    cost_compute = t_gpu_hr.magnitude * cost_gpu_hr
    wasted_valid_tokens = valid_rejected * l_tokens
    cost_fa_risk = invalid_accepted * cost_incident
    damage_ratio = round(cost_fa_risk / cost_compute, -2)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(n_valid - 3.2) < 0.01, "n_valid")
    check(abs(n_invalid - 60.8) < 0.01, "n_invalid")
    check(abs(valid_accepted - 2.88) < 0.01, "valid_accepted")
    check(abs(valid_rejected - 0.32) < 0.01, "valid_rejected")
    check(abs(invalid_accepted - 1.216) < 0.01, "invalid_accepted")
    check(abs(n_accepted - 4.096) < 0.01, "n_accepted")
    check(abs(precision_accepted - 0.7031) < 0.001, "precision_accepted")
    check(abs(t_wall.magnitude - 32.0) < 0.01, "t_wall")
    check(abs(t_gpu_s.magnitude - 256.0) < 0.1, "t_gpu_s")
    check(abs(cost_compute - 0.178) < 0.005, "cost_compute")
    check(abs(cost_fa_risk - 608.0) < 0.1, "cost_fa_risk")
    check(abs(damage_ratio - 3400) < 50, "damage_ratio")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_gpus_str = fmt_int(n_gpus)
    n_candidates_str = fmt_int(n_candidates)
    l_tokens_str = fmt_int(l_tokens)
    p_success_pct_str = fmt_percent(p_success, precision=0, style="prose")
    beta_frr_pct_str = fmt_percent(beta_frr, precision=0, style="prose")
    alpha_far_pct_str = fmt_percent(alpha_far, precision=0, style="prose")
    cost_gpu_hr_str = fmt_usd(cost_gpu_hr, precision=2)
    cost_compute_str = fmt_usd(cost_compute, precision=3)
    cost_incident_str = fmt_usd(cost_incident)
    damage_ratio_str = fmt_int(damage_ratio)
```
...
A distributed agent runtime orchestrates candidate generation on an `{python} AsymmetricVerifierYield.n_gpus_str`-GPU node running an inference serving engine. The engine generates $N =$ `{python} AsymmetricVerifierYield.n_candidates_str` parallel candidate trajectories for a complex concurrency bug fix. Each candidate consumes $L =$ `{python} AsymmetricVerifierYield.l_tokens_str` decode tokens. The serving cluster achieves an autoregressive decode throughput of $R = 32\text{ tokens/second}$ per GPU worker thread, allocating one GPU worker per candidate.

The generation parameters and verifier performance metrics are:

- Candidate length: $L =$ `{python} AsymmetricVerifierYield.l_tokens_str` tokens
- Raw generator base success rate: $p = 0.05$ (`{python} AsymmetricVerifierYield.p_success_pct_str` of generated patches are truly correct)
- Candidate sampling count: $N =$ `{python} AsymmetricVerifierYield.n_candidates_str`
- Verifier False Rejection Rate: $\beta = 0.10$ (`{python} AsymmetricVerifierYield.beta_frr_pct_str` of valid patches wrongly discarded)
- Verifier False Acceptance Rate: $\alpha = 0.02$ (`{python} AsymmetricVerifierYield.alpha_far_pct_str` of invalid patches mistakenly approved)
- Execution cost per GPU-hour: `{python} AsymmetricVerifierYield.cost_gpu_hr_str`

**Problem 1: Calculate the Expected Candidate Yield and Posterior Precision.**
Across $N =$ `{python} AsymmetricVerifierYield.n_candidates_str` sampled candidates:
...
The critical-path wall-clock time required to generate the $N=$ `{python} AsymmetricVerifierYield.n_candidates_str` candidates across the `{python} AsymmetricVerifierYield.n_gpus_str`-GPU node (with 8 concurrent candidate streams per GPU) is:
...
If the system downstream accepts one of the $1.216$ false acceptances and deploys it, triggering a production incident requiring a 15-minute automated rollback and engineering triage valued at `{python} AsymmetricVerifierYield.cost_incident_str`, the expected risk-weighted cost of false acceptance is:
$$\mathbb{E}[\text{Cost}_{\text{FA}}] = 1.216 \times \$500.00 = \$608.00$$

The risk-weighted damage of the verifier's false acceptance ($1.216$ instances) outstrips the raw GPU generation cost (`{python} AsymmetricVerifierYield.cost_compute_str`) by more than a factor of `{python} AsymmetricVerifierYield.damage_ratio_str`. In systems design, verifier optimization must prioritize driving $\alpha \to 0$ over minimizing $\beta$.
:::
...
The runtime mediates every interaction between the model and the execution environment, and the model can neither execute an operation nor declare that a precondition holds. Precondition checks exist because of the epistemic gap of @sec-vol3-processor-continuations, here the divergence between the model's parametric assumptions about the environment and the ground-truth state $\mathcal{S}$.
...
```{python}
#| label: lego-03-compute-and-token-savings-of
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ COMPUTE AND TOKEN SAVINGS OF SUBGRAPH RESYNTHESIS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-03-compute-and-token-savings-of
# │ Goal: Dimension FLOP savings and critical-path latency reduction from localized subgraph resynthesis.
# │ Show: Subgraph repair cuts compute by 82.4% (0.231 vs 1.315 PFLOPs) and delivers 4x decode speedup (11.25 vs 45 s).
# │ How: Contrast full plan re-generation prompt and decode FLOPs against scoped 3-node replacement.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_multiple

class SubgraphResynthesisSavings:
    """Compute and token savings of subgraph resynthesis vs global regeneration."""
    ...
    n_scoped = 1_200
    n_resynth_nodes = 3
    throughput_decode = 40 / second

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    n_prompt_a = n_base + n_exec_prefix
    flops_prefill_a = 2 * p_params * n_prompt_a
    n_gen_a = n_plan_nodes * tokens_per_node
    flops_decode_a = 2 * p_params * n_gen_a
    flops_total_a = (flops_prefill_a + flops_decode_a) / 1e15
    n_prompt_b = n_scoped
    flops_prefill_b = 2 * p_params * n_prompt_b
    n_gen_b = n_resynth_nodes * tokens_per_node
    flops_decode_b = 2 * p_params * n_gen_b
    flops_total_b = (flops_prefill_b + flops_decode_b) / 1e15
    compute_reduction = (flops_total_a - flops_total_b) / flops_total_a
    t_decode_a = (n_gen_a / throughput_decode.magnitude) * second
    t_decode_b = (n_gen_b / throughput_decode.magnitude) * second
    speedup_resynth = (t_decode_a / t_decode_b).magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(n_prompt_a == 7596, "n_prompt_a")
    check(n_gen_a == 1800, "n_gen_a")
    check(abs(flops_total_a - 1.315) < 0.01, "flops_total_a")
    check(n_gen_b == 450, "n_gen_b")
    check(abs(flops_total_b - 0.231) < 0.01, "flops_total_b")
    check(abs(compute_reduction - 0.824) < 0.005, "compute_reduction")
    check(abs(t_decode_a.magnitude - 45.0) < 0.05, "t_decode_a")
    check(abs(t_decode_b.magnitude - 11.25) < 0.05, "t_decode_b")
    check(abs(speedup_resynth - 4.0) < 0.01, "speedup_resynth")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_plan_nodes_str = fmt_int(n_plan_nodes)
    tokens_per_node_str = fmt_int(tokens_per_node)
    n_base_str = fmt_int(n_base)
    n_scoped_str = fmt_int(n_scoped)
    n_resynth_nodes_str = fmt_int(n_resynth_nodes)
    speedup_resynth_mult_str = fmt_multiple(speedup_resynth, precision=0)
```
...
The agent is executing a `{python} SubgraphResynthesisSavings.n_plan_nodes_str`-node plan graph. Each plan node requires an average of `{python} SubgraphResynthesisSavings.tokens_per_node_str` tokens to specify. The initial system prompt, environment description, and tool definitions consume $N_{\text{base}} =$ `{python} SubgraphResynthesisSavings.n_base_str` tokens.
...
- **Policy B (Subgraph Resynthesis):** The runtime retains nodes $v_1$ through $v_5$ in memory escrow, prunes the failed branch $\{v_6, v_7, v_8\}$, and provides a scoped prompt containing only the base system instructions ($N_{\text{base}}$), the state delta produced by $v_5$, and the specific linker error. This scoped prompt totals $N_{\text{scoped}} =$ `{python} SubgraphResynthesisSavings.n_scoped_str` tokens. The model generates a `{python} SubgraphResynthesisSavings.n_resynth_nodes_str`-node replacement subgraph ($3 \times 150 = 450$ tokens) that builds the missing dependency from source.
...
Subgraph resynthesis delivers a **`{python} SubgraphResynthesisSavings.speedup_resynth_mult_str` speedup** in critical-path replanning latency while avoiding the risk of re-executing or corrupting the verified state deltas produced by steps $v_1$ through $v_5$.
:::
...
The generation cost $C_{\text{gen}}$ scales with the aggregate number of tokens generated across all explored branches. Its latency follows the decode floor of @sec-vol3-processor-cost, and its computational cost for a model with $|\Theta|$ active parameters is quantified in floating-point operations:
...
```{python}
#| label: lego-03-compute-allocation-and-latency-trade-offs
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ COMPUTE ALLOCATION AND LATENCY TRADE-OFFS IN BOUNDED SEARCH (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-03-compute-allocation-and-latency-trade-offs
# │ Goal: Compare wall-clock latency, token volume, and FLOP cost between Best-of-N and beam search.
# │ Show: Beam search with PRM matches latency (11.26 vs 11.70 s) while cutting tokens by 43.75% and FLOPs by 37.3%.
# │ How: Contrast flat parallel decoding against step-level synchronous PRM evaluation and pruning.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_bandwidth

class BoundedSearchTradeoffs:
    """Compute allocation and latency trade-offs in bounded search."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    bw_hbm = 3.35 * (TB / second)
    n_params = 70e9
    l_step = 128
    n_traj_p1 = 16
    n_steps_p1 = 4
    t_sandbox = 500 * millisecond
    concurrency_sandbox = 8
    beam_width = 4
    m_expand = 3
    n_depth_p2 = 4
    n_params_prm = 8e9
    t_prm_step = 15 * millisecond

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    t_token = ((n_params * 1 * byte) / bw_hbm).to(second)
    l_total_p1 = n_steps_p1 * l_step
    k_total_p1 = n_traj_p1 * l_total_p1
    t_gen_p1 = (l_total_p1 * t_token).to(second)
    rounds_sandbox_p1 = n_traj_p1 / concurrency_sandbox
    t_verif_p1 = (rounds_sandbox_p1 * t_sandbox).to(second)
    t_wall_p1 = t_gen_p1 + t_verif_p1
    c_flop_p1 = 2 * n_params * k_total_p1
    n_segments_p2 = 3 + 9 + 12 + 12
    k_total_p2 = n_segments_p2 * l_step
    reduction_tokens = (k_total_p1 - k_total_p2) / k_total_p1
    t_gen_p2 = (n_depth_p2 * l_step * t_token).to(second)
    t_prm_total = (n_depth_p2 * t_prm_step).to(second)
    t_sandbox_p2 = (1 * t_sandbox).to(second)
    t_verif_p2 = t_prm_total + t_sandbox_p2
    t_wall_p2 = t_gen_p2 + t_verif_p2
    c_flop_gen_p2 = 2 * n_params * k_total_p2
    c_flop_prm_p2 = 2 * n_params_prm * k_total_p2
    c_flop_total_p2 = c_flop_gen_p2 + c_flop_prm_p2
    flop_savings = (c_flop_p1 - c_flop_total_p2) / c_flop_p1

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_token.to(millisecond).magnitude - 20.90) < 0.05, "t_token")
    check(k_total_p1 == 8192, "k_total_p1")
    check(abs(t_gen_p1.magnitude - 10.70) < 0.05, "t_gen_p1")
    check(abs(t_verif_p1.magnitude - 1.00) < 0.01, "t_verif_p1")
    check(abs(t_wall_p1.magnitude - 11.70) < 0.05, "t_wall_p1")
    check(n_segments_p2 == 36, "n_segments_p2")
    check(k_total_p2 == 4608, "k_total_p2")
    check(abs(reduction_tokens - 0.4375) < 0.001, "reduction_tokens")
    check(abs(t_gen_p2.magnitude - 10.70) < 0.05, "t_gen_p2")
    check(abs(t_prm_total.magnitude - 0.06) < 0.01, "t_prm_total")
    check(abs(t_sandbox_p2.magnitude - 0.50) < 0.01, "t_sandbox_p2")
    check(abs(t_verif_p2.magnitude - 0.56) < 0.01, "t_verif_p2")
    check(abs(t_wall_p2.magnitude - 11.26) < 0.05, "t_wall_p2")
    check(abs(flop_savings - 0.373) < 0.005, "flop_savings")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    l_step_str = fmt_int(l_step)
    n_traj_p1_str = fmt_int(n_traj_p1)
    n_steps_p1_str = fmt_int(n_steps_p1)
    l_total_p1_str = fmt_int(l_total_p1)
    t_sandbox_ms_str = fmt_time(t_sandbox, unit=millisecond, precision=0)
    beam_width_str = fmt_int(beam_width)
    m_expand_str = fmt_int(m_expand)
    n_depth_p2_str = fmt_int(n_depth_p2)
    t_prm_step_ms_str = fmt_time(t_prm_step, unit=millisecond, precision=0)
    reduction_tokens_pct_str = fmt_percent(reduction_tokens, precision=2, style="prose")
    t_wall_p2_s_str = fmt_time(t_wall_p2, unit=second, precision=2)
    t_wall_p1_s_str = fmt_time(t_wall_p1, unit=second, precision=2)
    flop_savings_pct_str = fmt_percent(flop_savings, precision=1, style="prose")
```
...
1. **Policy 1 (Best-of-16 Flat Sampling):** Generate $N =$ `{python} BoundedSearchTradeoffs.n_traj_p1_str` complete trajectories of `{python} BoundedSearchTradeoffs.n_steps_p1_str` steps each ($L_{\text{total}} =$ `{python} BoundedSearchTradeoffs.l_total_p1_str` tokens). Concurrently evaluate all completed patches using an automated sandbox unit test suite ($t_{\text{sandbox}} =$ `{python} BoundedSearchTradeoffs.t_sandbox_ms_str` per test execution).
2. **Policy 2 (Beam Search with PRM):** Run beam search with beam width $B =$ `{python} BoundedSearchTradeoffs.beam_width_str`, expanding $M =$ `{python} BoundedSearchTradeoffs.m_expand_str` candidate steps at each of the `{python} BoundedSearchTradeoffs.n_depth_p2_str` depth levels. Each candidate step is scored by an 8B parameter PRM ($|\Theta_{\text{PRM}}| = 8 \times 10^9$ bytes) executing a forward pass of `{python} BoundedSearchTradeoffs.l_step_str` tokens. Only the final surviving `{python} BoundedSearchTradeoffs.beam_width_str` trajectories are verified in the sandbox.
...
For the 8B PRM, the prefill/evaluation time for a `{python} BoundedSearchTradeoffs.l_step_str`-token step is compute-bound at $t_{\text{PRM}} \approx$ `{python} BoundedSearchTradeoffs.t_prm_step_ms_str`.
...
- **Tokens Generated:**
  ...
  *(A `{python} BoundedSearchTradeoffs.reduction_tokens_pct_str` reduction in generated tokens compared to Best-of-16).*
...
- **Verification Latency:**
  At each of the `{python} BoundedSearchTradeoffs.n_depth_p2_str` steps, PRM scoring must run synchronously before the runtime can prune branches. With PRM scoring taking `{python} BoundedSearchTradeoffs.t_prm_step_ms_str` per step:
  $$T_{\text{PRM}} = 4 \times 15\text{ ms} = 60\text{ ms} = 0.06\text{ seconds}$$
  At the leaf level, only the surviving `{python} BoundedSearchTradeoffs.beam_width_str` candidates execute sandbox verification in parallel:
...
Beam search with process verification achieves roughly equivalent wall-clock time (`{python} BoundedSearchTradeoffs.t_wall_p2_s_str` vs. `{python} BoundedSearchTradeoffs.t_wall_p1_s_str`) while cutting total FLOP expenditure and token consumption by **`{python} BoundedSearchTradeoffs.flop_savings_pct_str`**. The compute saved by early step pruning directly compensates for the overhead of the PRM model and synchronous pipeline stalls.
:::
...
To safeguard the environment, the runtime must enforce that exploratory search operates exclusively within isolated scratchpads or sandboxes the runtime can discard, which hold its actions at $A_1$ (as formalized in @sec-vol3-virtualization). Physical environment commits are deferred until the search algorithm terminates, the stopping criteria are satisfied, and a candidate trajectory is formally selected and certified.
...
- *Depth-Heavy Topologies (Sequential Deliberation):* Sequential chain-of-thought generation, iterative critique, and single-branch backtracking serialize execution. The wall-clock latency is strictly bounded by the sum of sequential forward passes:
  ...
  Because decode is bound by memory bandwidth (@sec-vol3-processor-cost), the critical path cannot be compressed by throwing parallel compute at a single instance. This creates high median latency ($p50$) and exposes the execution to severe variance if the model enters an expansive generation loop.
