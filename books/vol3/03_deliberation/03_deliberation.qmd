...
where $\bar{p} = \frac{1}{T}\sum_{t=1}^T p_t$ is the arithmetic mean step reliability.

```
1.0 ┼────────────────────────────────────────────────────────────────────────
    │ ╲  p = 0.999 (High Token Precision)
0.8 ┼   ╲
    │    ╲
0.6 ┼     ╲
    │      ╲
0.4 ┼       ╲   p = 0.990 (Coarse Token Precision)
    │        ╲───────────────
0.2 ┼                        ╲╲  p = 0.900 (Action Step Reliability)
    │                          ╲╲────────────────────────────────────────────
0.0 ┼────────────────────────────┴───────────────────────────────────────────
    0           200          400          600          800         1000
                                Sequence Length (Tokens / Steps)
```
...
This failure illustrates the classical **Credit Assignment Problem** in sequential decision making: when an outcome is evaluated only at the terminal leaf of an extended trajectory, how does the runtime determine which intermediate actions contributed to success and which introduced fatal errors?
...
If verification collapses search spaces, why not verify after every emitted token or micro-action? The answer lies in the asymmetric wall-clock latency profile of verification oracles.
...
Apply the mathematical derivations and architectural trade-offs established throughout this chapter to solve the following quantitative problems.

### Problem 1: Combinatorial branching and breakeven verification compute

An autonomous data science agent generates SQL queries using MCTS over a search horizon of $d = 6$ steps with a branching factor of $b = 5$. Each candidate action step generates $T_{\text{step}} = 400$ tokens on a 70B parameter model ($P = 70 \times 10^9$, requiring $C_{\text{gen}} \approx 2 \cdot P \cdot T_{\text{step}} = 5.6 \times 10^{13} \text{ FLOPs} = 56\text{ TFLOPs}$ per action node).

1. Calculate the total number of candidate nodes generated under an unverified exhaustive breadth-first search to depth $d = 6$, and compute the total generation compute in PFLOPs ($10^{15}$ FLOPs).
2. The engineering team introduces an intermediate SQL typechecker and schema validator that rejects invalid queries, achieving an effective survival fraction of $\alpha = 0.25$. Calculate the effective branching factor $b_{\text{eff}}$, the total number of candidate nodes generated across all 6 plies under step-verified search, and the percentage reduction in node expansions.
3. Derive the maximum allowable verification compute budget ($C_{\text{ver}}^{\max}$) per candidate node—in both TFLOPs and token equivalents—before step-verified search becomes more computationally expensive than unverified exhaustive search.

---

### Problem 2: Optimal verification cadence under asymmetric latencies

An autonomous site reliability engineering agent executes an automated incident remediation trajectory of $N = 60$ steps. Profiling reveals the following execution parameters:

- Step generation latency: $T_{\text{step}} = 1.2\text{ seconds}$ per action.
- Single-step semantic correctness: $p = 0.94$.
- Verification oracle: An isolated containerized integration test suite with latency $T_{\text{ver}} = 18.0\text{ seconds}$.
1. Using the verification latency formulation $T(m) = \frac{N}{m} \cdot \frac{m \cdot T_{\text{step}} + T_{\text{ver}}}{p^m}$, calculate the expected total wall-clock execution time $T(m)$ for verification intervals $m = 1$, $m = 5$, $m = 10$, and $m = 60$ steps.
2. Identify the optimal cadence $m^*$ among these values that minimizes expected wall-clock latency, and calculate the speedup factor achieved by $m^*$ compared to continuous verification ($m = 1$) and terminal-only verification ($m = 60$).
3. If the team replaces the containerized integration test with an in-process static analyzer taking $T_{\text{ver}} = 50\text{ ms}$ ($0.05\text{ s}$), calculate the new expected execution latency $T(1)$ at $m = 1$. Explain the architectural principle governing this latency shift.

---

### Problem 3: RadixAttention HBM sizing for tree-of-thoughts deliberation

An inference cluster serves an agent executing Tree-of-Thoughts deliberation. The search tree operates with active beam width $B = 8$, search depth $d = 4$, and candidate branching factor $k = 3$.

The model is a 70-billion-parameter transformer with:

- Number of layers: $L = 80$
- Number of attention heads: $H = 64$
- Head dimension: $d_h = 128$
- Data type: FP16 ($b_{\text{elem}} = 2\text{ bytes}$)
- Shared prompt prefix: $S_{\text{prompt}} = 8{,}192\text{ tokens}$
- Per-step action length: $S_{\text{step}} = 256\text{ tokens}$
1. Calculate the Key-Value cache memory footprint per token ($m_{\text{token}}$) in megabytes.
2. At the maximum search depth $d = 4$, calculate the peak KV cache memory footprint (in gigabytes) if the serving system naively duplicates the prefix and ancestral history across all $B \times k = 24$ active leaf paths.
3. Calculate the peak KV cache memory footprint (in gigabytes) if the runtime employs RadixAttention with copy-on-write page tables, storing each unique tree node once. Calculate the memory savings ratio achieved by radix prefix caching.
