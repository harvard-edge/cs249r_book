...
![**The Diagnostic Tree and Discriminatory Verification**: Under an ambiguous telemetry observation, open-loop autoregressive generation (left) prematurely commits to hypothesis $H_1$, poisoning downstream KV cache context and producing a flawed patch that leaves the root defect intact. Deliberative search (right) withholds actuation until a targeted discriminatory probe falsifies $H_1$, isolating $H_2$ and enabling a verified state resolution.](images/svg/diagnostic_tree.svg){#fig-vol3-diagnostic-tree width=\"95%\"}
...
Test-time computation is not an undifferentiated scalar currency. Its architectural utility is governed entirely by its execution topology across three orthogonal axes: depth (sequential token extension), breadth (parallel candidate exploration), and feedback (observation-conditioned revision). Each axis establishes a fundamentally different dependency graph across model invocations, exhibits contrasting critical-path latency and accelerator memory profiles, and interacts distinctively with the model's epistemic boundaries (@fig-vol3-allocation-axes).

![**Three Canonical Deliberation Topologies**: Sequential depth, parallel breadth, and closed-loop feedback. Arrows denote data dependencies; cards summarize critical-path latency equations, accelerator memory footprints ($M_{\text{KV}}$), and fundamental systems trade-offs.](images/svg/three_compute_allocation_axes.svg){#fig-vol3-allocation-axes width="95%"}
...
:::
Once a diverse candidate set $\mathcal{C} = \{\mathbf{y}^{(1)}, \mathbf{y}^{(2)}, \dots, \mathbf{y}^{(N)}\}$ is staged in memory, the host supervisor must select the trajectory $\mathbf{y}^* \in \mathcal{C}$ that optimizes task success. The supervisor operates under Zero Ambient Authority ($A=0$); it cannot assume candidate self-reports are truthful, nor can it allow candidate code to interact directly with host OS resources. To select a candidate securely, the runtime employs three primary architectural strategies, organized as a hierarchical selection cascade (@fig-candidate-selection-cascade).

![**The Three-Tier Candidate Selection Cascade**: A defense-in-depth architecture filtering candidate proposals before execution. Tier 1 executes zero-FLOP deterministic checks (AST syntax, typing, static lints) to drop malformed proposals. Tier 2 clusters syntactically distinct candidates into semantic equivalence classes via majority voting and canonical hashing. Tier 3 evaluates reasoning steps with learned process reward models (PRMs) and verifies invariant closure before advancing to physical execution.](images/svg/candidate_selection_cascade.svg){#fig-candidate-selection-cascade width="95%"}
...
::: {.column-margin}
![](images/svg/vol3_deliberation_margin_003.svg){width="100%" fig-alt="Scatter and calibration curve comparing process reward model step scores against sparse terminal outcome labels."}

*Step-level process reward models localize reasoning faults before terminal execution failure occurs.*
:::
...
![**Goodhart Divergence in Search**: Divergence of proxy verifier score and true system utility under scaling search intensity $N$. Beyond an optimal threshold $N^*$, high-intensity search selects adversarial exploits.](images/svg/goodhart_divergence.svg){#fig-vol3-goodhart-divergence width="95%"}
...
![**The Multi-Layered Verification Pipeline**: Defense-in-depth architecture filtering candidate proposals before state mutation. Deterministic AST filters and static linters eliminate syntactically invalid candidates (&lt;2 ms); property-based fuzzers and isolated sandbox test suites evaluate functional regressions (100–2,000 ms); policy and invariant checks confirm safety boundaries before state commit.](images/svg/verification_pipeline_gates.svg){#fig-vol3-verification-gates width="95%"}
...
![**Plan Node Execution Lifecycle Finite State Machine**: Nodes remain in `Pending` until all causal ancestors reach `Verified`. The scheduler evaluates precondition predicate $\mathcal{P}_i(\mathcal{S})$ before moving to `Executing`. Postcondition satisfaction commits the node to `Verified`; mismatches transition the node to `Failed` and cascade invalidations across all downstream descendants $\text{Desc}(v_i)$.](images/svg/plan_node_lifecycle_fsm.svg){#fig-vol3-plan-node-lifecycle width="95%"}
...
![**Deliberative Plan Revision**: Deliberative plan revision under environmental precondition failure. (*Left*) An initial plan DAG executes until step $v_2$ encounters an unresolvable dependency mismatch, which immediately invalidates the preconditions of downstream dependent nodes $v_3$ and $v_4$. (*Right*) The runtime performs branch pruning: verified ancestor nodes ($v_0, v_1$) and the independent concurrent branch ($v_5$) are preserved in memory escrow, while targeted subgraph resynthesis replaces $\{v_2, v_3\}$ with a compensated path $\{v_2', v_3'\}$, reconnecting cleanly to $v_4$.](images/svg/fig-vol3-plan-revision.svg){#fig-vol3-plan-revision width=\"95%\"}
...
![**Three-Tier Hierarchical Plan Revision Mechanics**: Modular error containment handles environmental failures at the lowest viable tier. Tier 1 executes local node retrials when external postcondition contracts are preserved. Tier 2 prunes the invalidated subgraph $\text{Desc}(v_{\text{fail}})$ and resynthesizes replacement branches while anchoring verified ancestor state in memory escrow. Tier 3 halts execution and escalates to operators when root task invariants are falsified.](images/svg/hierarchical_revision_mechanics.svg){#fig-vol3-hierarchical-revision width="95%"}
...
![**The Replanning Thrashing Cycle and Damping Invariants**: Left: Over-sensitive invalidation predicates trap the agent in an infinite livelock where non-fatal environment warnings trigger continuous plan mutation without advancing physical state ($\chi > 2.0$). Right: Enforcing observation partitioning, replenishment budgets ($R_{\text{budget}}$), and quarantine ledgers damps oscillatory resynthesis.](images/svg/replanning_thrashing_cycle.svg){#fig-vol3-replanning-thrashing width="95%"}
...
![**Search Topologies: Parallel Sampling, Beam Search, MCTS, and Graph of Thoughts**: Architectural trade-offs across four test-time search structures. Best-of-$N$ maximizes GPU batch parallelism but suffers from redundant prefix evaluation; Beam Search prunes at step boundaries with linear memory; MCTS balances exploration and exploitation with value backpropagation; Graph of Thoughts allows arbitrary DAG branching and thought merging.](images/svg/search_topologies_comparison.svg){#fig-vol3-search-topologies width="95%"}
...
![**Test-Time Search Stopping Boundaries and Budget Allocation**: Multicriteria stopping hierarchy balancing hard resource ceilings, opportunistic threshold satisfaction, and marginal utility economics. Reaching token ($K_{\max}$), latency ($T_{\max}$), or depth ($D_{\max}$) ceilings unconditionally halts search; certified deterministic verification ($V(\tau)=1$) commits immediately; sub-epsilon marginal gain ($\Delta P / \Delta C < \epsilon_{\text{stop}}$) terminates exploration when returns diminish.](images/svg/search_stopping_boundaries.svg){#fig-vol3-stopping-boundaries width="95%"}
...
![**Search Budget Allocation and Marginal Benefit**: Search budget allocation and marginal benefit curves across search topologies under fixed compute ceilings.](images/svg/search_budget_marginal_benefit.svg){#fig-vol3-search-budget width=\"95%\"}
...
![**The Deliberation Pareto Frontier and Regimes of Failure**: Empirical task success probability $P(\text{success})$ plotted against inference compute (generated and evaluated tokens). While Greedy Baseline and Self-Refine scale early, Best-of-8 breadth sampling risks adversarial false acceptance ($9\%$ FAR). Grounding deliberation in execution feedback (ReAct + Tool Feedback) defines the true operational Pareto frontier, achieving optimal task completion without ungrounded neural hallucinations.](images/svg/deliberation_pareto_frontier.svg){#fig-vol3-deliberation-pareto width="95%"}
