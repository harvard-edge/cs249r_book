...
Nearly 88 percent of unguided greedy trajectories collapse into unrecoverable states.
...
On an NVIDIA H100 SXM5 GPU capable of $989\text{ TFLOP/s}$ of dense 16-bit tensor compute (rising to $1{,}979\text{ TFLOP/s}$ with 2:4 structured sparsity or FP8 precision) and $3.35\text{ TB/s}$ of High-Bandwidth Memory (HBM3) bandwidth, saturating the dense tensor cores requires an arithmetic intensity of:

$$
\mathcal{I}_{\text{knee}} = \frac{989 \times 10^{12} \text{ FLOPs/s}}{3.35 \times 10^{12} \text{ Bytes/s}} \approx 295 \text{ FLOPs/Byte}
$$

(rising to $\approx 590\text{ FLOPs/Byte}$ under sparse or FP8 execution). Greedy decoding operates nearly $300\times$ below the accelerator's dense roofline knee (and nearly $600\times$ below its sparse ceiling). The compute engine is stalled, waiting for model weights to stream across the memory bus.
...
The topology of $\mathcal{G}$ determines the degree of freedom the agent possesses to explore, backtrack, and synthesize solutions, as contrasted in @fig-vol3-deliberation-execution-topologies across linear ReAct chains, hierarchical DAGs, and branching deliberate search.
...
The execution graph is a degenerate, unbranching chain, as popularized by Chain-of-Thought prompting [@wei2022cot] and ReAct interleaving of reasoning traces with action calls [@yao2023react]:
$$
...
PRMs transform test-time search by enabling **Early Error Localization**. Rather than generating hundreds or thousands of speculative rollout tokens, evaluating a candidate step with a PRM requires only a single forward inference pass through the reward head ($2 \cdot |a_t| \cdot P_{\text{PRM}}$ FLOPs). The runtime detects the precise step index $t^*$ where the trajectory derailed:
...
In naive serving, the prefix is recomputed or duplicated across all 64 branches ($16{,}500$ tokens per branch, or $1{,}056{,}000$ total tokens). The memory consumed by the KV cache alone is:

$$
M_{\text{naive}} = 64 \times (16{,}000 + 500) \times 2 \times 80 \times 8 \times 128 \times 2 \text{ bytes} \approx \mathbf{346.0\text{ GB}}
$$

Storing $346.0\text{ GB}$ of KV cache alone requires partitioning across at least five 80 GB GPUs before accounting for model weights or activation memory.

Under RadixAttention prefix sharing, the 16,000-token prompt is stored **once**:

$$
M_{\text{shared}} = \left[16{,}000 + (64 \times 500)\right] \times 2 \times 80 \times 8 \times 128 \times 2 \text{ bytes} = 48{,}000 \times 327{,}680 \text{ bytes} \approx \mathbf{15.73\text{ GB}}
$$

Prefix sharing achieves an immediate **$22.0\times$ memory reduction**, allowing the KV cache for all 64 parallel branches to fit easily within a single accelerator alongside model weights. Failing to integrate radix prefix caching into search runtimes turns multi-branch deliberation into an operational non-starter.
