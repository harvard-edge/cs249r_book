...
Deterministic recovery mandates that fail-plausible semantic errors are handled not by prompting the failing model to introspect, but by **external verifier oracles**\index{Verifier Oracles} (linters, type checkers, unit test runners, AST validators) that intercept execution, halt forward progress, prune poisoned tokens from the KV cache, and trigger deterministic rollback protocols, as structured in @fig-vol3-checkpoint-verification-pipeline.
