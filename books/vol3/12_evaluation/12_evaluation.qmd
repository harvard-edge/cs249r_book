...
- Formulate the 4-layer evaluation contract (syntactic schema validity, execution invariants, environment state deltas, and operational constraint compliance)
- Design hermetic task environments with ephemeral sandboxes, deterministic mock services, and sub-second reset harnesses
- Compare outcome verifiers, process grading and step-level metrics, and calibrated LLM judges as graders of a trajectory
- Calculate confidence intervals, required sample sizes, and pass$^k$ reliability from repeated trials at matched budgets
- Instrument a trajectory as an OpenTelemetry span graph and select a tail-sampling and secret-redaction policy that retains every failure
- Conduct forensic incident post-mortems using deterministic trace replay and counterfactual gym ablations
- Evaluate a candidate release against a composite gate through offline, shadow, and sequential canary stages
...
## The multi-layer evaluation contract {#sec-vol3-evaluation-contract}
...
*Observability watches all three exposures, verifying state, bounding horizon, and auditing authority.*
:::
...
where trajectory steps $N$ are bounded by $N \le N_{\max}$ to terminate infinite reasoning loops. Concurrently, security checks verify that execution remained within the designated sandbox namespace, respected capability tokens, and never attempted privilege escalation. Together the four layers observe all three exposures of @sec-vol3-intro-hsa, since state delta verification checks what the trajectory did to state, the step and wall-clock budgets bound its horizon, and the security checks confirm it never acted beyond its authority.
...
```{python}
#| label: lego-16-evaluating-the-four-layer-contract-on
#| echo: false
# │ Show: Multi-layer task contract exposes lock contention and partial state write.
# │ How: Layer 1 JSON syntax, Layer 2 execution invariants, Layer 3 state deltas, Layer 4 budgets.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_percent, fmt_time

class EvaluatingFourLayerTaskContract:
    t_limit = 120.0 * second
    t_task = 142.6 * second
    overrun_s = (t_task - t_limit).to(second).magnitude
    overrun_ratio = overrun_s / t_limit.to(second).magnitude

    check(abs(overrun_s - 22.6) < 1e-4, "overrun_s")
    check(abs(overrun_ratio - 0.18833) < 0.001, "overrun_ratio")

    overrun_s_str = fmt_time(overrun_s * second, unit=second, precision=1)
    overrun_pct_str = fmt_percent(overrun_ratio, precision=1, style="prose")
```
...
The trajectory breached the maximum latency threshold by {python} EvaluatingFourLayerTaskContract.overrun_s_str ({python} EvaluatingFourLayerTaskContract.overrun_pct_str overrun) due to the table lock contention.
...
```{python}
#| label: lego-16-storage-io-and-reset-amortization
#| echo: false
# │ Show: Copy-on-write snapshotting delivers 102x reset speedup and 150x less write I/O.
# │ How: Transient overlay mounts vs sequential 12 GB directory duplication across 64 workers.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_multiple, fmt_time

class StorageIoResetAmortization:
    t_reset_naive = 4.8 * second
    t_reset_cow = 0.047 * second
    speedup = t_reset_naive.to(second).magnitude / t_reset_cow.to(second).magnitude

    check(abs(speedup - 102.127) < 0.01, "speedup")

    t_reset_naive_s_str = fmt_time(t_reset_naive, unit=second, precision=1)
    t_reset_cow_s_str = fmt_time(t_reset_cow, unit=second, precision=3)
    speedup_mult_str = fmt_multiple(round(speedup))
```
...
During the task, only the modified blocks are written to the upper layer ($S_{\Delta} = 80\text{ MB}$). At the end of the task, tearing down the environment requires an `umount` syscall and a recursive removal of the $80\text{ MB}$ upper directory:
...
Reset latency drops from {python} StorageIoResetAmortization.t_reset_naive_s_str to {python} StorageIoResetAmortization.t_reset_cow_s_str, yielding a {python} StorageIoResetAmortization.speedup_mult_str speedup and eliminating storage bus saturation across the 64-worker evaluation fleet.
:::
...
### Resampling hierarchies {#sec-vol3-evaluation-resampling}
...
### Finite-sample uncertainty {#sec-vol3-evaluation-wilson}
...
::: {#nbk-vol3-evaluation-wilson-superiority .callout-example title="Wilson score bounds on agent benchmarks"}
...
### Latent potential decoupling {#sec-vol3-evaluation-pass-k}
...
```{python}
#| label: lego-16-critical-path-latency-and-bottleneck
#| echo: false
# │ Show: Critical path decomposition shows tool execution and inference dominate latency.
# │ How: Summing causal span durations across retrieval, generation, sandbox, and fan-out.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_percent

class CriticalPathLatencyDecomposition:
    t_wall = 18.25
    t_crit = 17.26
    t_orchestration = t_wall - t_crit
    pct_orchestration = t_orchestration / t_wall

    pct_tool = 6.20 / t_crit
    pct_model = (3.39 + 3.74) / t_crit
    pct_barrier = 3.85 / t_crit
    pct_retrieval = 0.08 / t_crit

    check(abs(pct_tool - 0.3592) < 0.001, "pct_tool")
    check(abs(pct_model - 0.4131) < 0.001, "pct_model")
    check(abs(pct_barrier - 0.2230) < 0.001, "pct_barrier")
    check(abs(pct_retrieval - 0.0046) < 0.001, "pct_retrieval")
    check(abs(pct_orchestration - 0.0542) < 0.001, "pct_orchestration")

    pct_tool_str = fmt_percent(round(pct_tool, 3), precision=1, style="prose")
    pct_model_str = fmt_percent(round(pct_model, 3), precision=1, style="prose")
    pct_barrier_str = fmt_percent(round(pct_barrier, 3), precision=1, style="prose")
    pct_retrieval_str = fmt_percent(round(pct_retrieval, 3), precision=1, style="prose")
    pct_orchestration_str = fmt_percent(round(pct_orchestration, 3), precision=1, style="prose")
```
...
The remaining latency ($T_{\text{wall}} - T_{\text{crit}} = 18.25\text{ s} - 17.26\text{ s} = 0.99\text{ s}$, or {python} CriticalPathLatencyDecomposition.pct_orchestration_str) represents host runtime orchestration overhead (JSON serialization, process context switching, and RPC queue wait times).

Decomposing the critical path reveals the governing operational bottleneck:

- Sandboxed tool execution accounts for {python} CriticalPathLatencyDecomposition.pct_tool_str ($6.20\text{ s}$) of critical path time.
- Neural model inference accounts for {python} CriticalPathLatencyDecomposition.pct_model_str ($3.39\text{ s} + 3.74\text{ s} = 7.13\text{ s}$).
- Multi-agent coordination barrier accounts for {python} CriticalPathLatencyDecomposition.pct_barrier_str ($3.85\text{ s}$).
- Vector retrieval accounts for only {python} CriticalPathLatencyDecomposition.pct_retrieval_str ($0.08\text{ s}$).
...
## Tail-based sampling budgets {#sec-vol3-evaluation-budgets}
...
```{python}
#| label: lego-16-telemetry-ingestion-inverted-economics
#| echo: false
# │ Show: Tail-based sampling slashes agent telemetry ingestion by 93.1% (14.5x reduction).
# │ How: In-memory escrow retention of anomalies vs blind head-based sampling discard rates.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
...
class TelemetryIngestionInvertedEconomics:
    v_full = 336.0
    v_tail = 23.2
    data_reduction = 1.0 - (v_tail / v_full)
    reduction_ratio = v_full / v_tail

    check(abs(data_reduction - 0.93095) < 0.001, "data_reduction")
    check(abs(reduction_ratio - 14.4827) < 0.01, "reduction_ratio")

    reduction_mult_str = fmt_multiple(round(reduction_ratio, 1), precision=1)
```
...
By retaining telemetry based on terminal execution properties rather than initial randomization, the tail-based architecture achieves a {python} TelemetryIngestionInvertedEconomics.reduction_mult_str storage reduction while capturing $100\%$ of all operational failures and performance anomalies.
:::
...
## Forensic incident post-mortems {#sec-vol3-evaluation-postmortems}
...
## Staged canary deployments {#sec-vol3-evaluation-releases}
...
Finally, *Mean Time to Recovery* (MTTR) in agentic systems accounts for both traffic draining and state restoration. If a canary failure corrupts external systems before the circuit breaker trips, recovery requires executing Saga compensating transactions (@sec-vol3-failure-recovery) to reverse partial modifications. Monitoring the full MTTR ensures that automated release systems account for both the speed of the software rollback and the latency of physical state reconciliation.

```{python}
#| label: lego-16-sizing-canary-traffic-and-circuit
#| echo: false
# │ Show: Managed canary deployment limits blast radius, preventing 95% of excess failures.
# │ How: Wald SPRT sample size and detection latency under 5% traffic routing vs full rollout.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_percent
...
class SizingCanaryTrafficCircuitBreaker:
    excess_reduction = 80.45 / 84.69
    check(abs(excess_reduction - 0.9499) < 0.001, "excess_reduction")

    reduction_pct_str = fmt_percent(0.95, precision=0, style="prose")
```
...
The staged canary deployment reduces excess customer-impacting failures by {python} SizingCanaryTrafficCircuitBreaker.reduction_pct_str:
...
## Empirical observability harness synthesis {#sec-vol3-evaluation-synthesis}
...
### The unified control-plane architecture {#sec-vol3-evaluation-synthesis-architecture}
...
### Evaluation readiness contracts {#sec-vol3-evaluation-synthesis-readiness}
...
### Mathematical synthesis of the automated release gate {#sec-vol3-evaluation-synthesis-math}
...
```{python}
#| label: lego-16-automated-canary-verification-and-release
#| echo: false
# │ Show: Candidate runtime satisfies non-inferiority accuracy gate and throughput bounds.
# │ How: Wilson score lower bounds and empirical throughput degradation vs production baseline.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_percent

class CanaryVerificationReleaseGates:
    deg_rounded = (0.03347 - 0.03071) / 0.03347
    check(abs(deg_rounded - 0.08246) < 0.001, "deg_rounded")

    throughput_deg_pct_str = fmt_percent(0.0824, precision=2, style="prose")
```
...
Because $0.03071 \ge 0.03012$, the candidate satisfies the throughput criterion (exhibiting an {python} CanaryVerificationReleaseGates.throughput_deg_pct_str degradation, safely within the $10\%$ tolerance budget).
