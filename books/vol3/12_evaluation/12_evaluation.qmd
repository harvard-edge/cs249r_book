...
Layer 4 turns the budgets of @sec-vol3-controlplane-accounting into acceptance criteria. Using the duration accounting of @sec-vol3-intro-duration-accounting over the trajectory's $N$ turns, a trajectory is within budget only if

$$T_{\text{task}} = \sum_{t=1}^N \left( T_{\text{model}, t} + T_{\text{tool}, t} + T_{\text{wait}, t} + T_{\text{runtime}, t} \right) \le T_{\max}, \qquad C_{\text{task}} \le C_{\max}, \qquad N \le N_{\max}$$
...
Setting the sampling temperature to zero does not make an agent repeatable (@sec-vol3-foundation-model-autoregressive). The serving system can compute slightly different logits for the same prompt when batch composition changes, and when the two most likely tokens are nearly tied, that difference changes the chosen token. One changed token early in a fifty-turn trajectory changes every later turn. The environment adds its own variation. Concurrent tool calls can return in different orders, so two runs build different prompts from the same observations, and a simulated user samples its replies. Replay divergence in the durable log is the same phenomenon seen after the fact (@sec-vol3-persistence-non-determinism).
...
The first is pass@$k$, the probability that at least one of $k$ runs succeeds. @Sec-vol3-test-time-compute-candidate-diversity used it as the ceiling on what selection among $k$ candidates can deliver. Chen et al. [-@chen2021evaluating] give the unbiased estimator from $n \ge k$ runs:

$$\text{pass}@k = \mathbb{E}_{\text{tasks}} \left[ 1 - \frac{\binom{n-c}{k}}{\binom{n}{k}} \right]$$ {#eq-pass-at-k}

Pass@$k$, as estimated by @eq-pass-at-k, measures potential. It is achievable only if something picks the successful run, and in production that something is a verifier the runtime can run before committing. With a sealed test suite in the loop, pass@$k$ is a fair target. Without one, users receive pass@1.
...
A trace turns a latency complaint into arithmetic. Spans that run one after another add, spans that run in parallel contribute only their longest member, and the sum along the longest chain is the critical path. The economics of shortening that path belong to @sec-vol3-agent-economics-criticalpath. Here the trace supplies the measured durations.

::: {#nbk-16-critical-path-latency-and-bottleneck .callout-notebook title="Which span to optimize"}

**Problem**: A coding agent's trace records five steps with a total wall-clock time of $T_{\text{wall}} = 18.25\text{ s}$: retrieval ($t_1 = 0.08\text{ s}$), a model call that writes a patch ($t_2 = 3.39\text{ s}$), a sandboxed test run ($t_3 = 6.20\text{ s}$), two checks run in parallel, a linter ($t_{4a} = 2.10\text{ s}$) and a reviewer model call ($t_{4b} = 3.85\text{ s}$), and a final model call ($t_5 = 3.74\text{ s}$). Which span should the team shorten first?

**Math**: The parallel checks contribute their maximum, so the critical path is

$$T_{\text{crit}} = t_1 + t_2 + t_3 + \max(t_{4a}, t_{4b}) + t_5 = 0.08 + 3.39 + 6.20 + 3.85 + 3.74 = 17.26\text{ s}$$

The remaining $18.25 - 17.26 = 0.99\text{ s}$ is runtime overhead between spans. Along the critical path, the two model calls take $7.13\text{ s}$ (41 percent), the test run $6.20\text{ s}$ (36 percent), the reviewer call $3.85\text{ s}$ (22 percent), and retrieval $0.08\text{ s}$ (under 1 percent).

**Systems insight**: The model calls and the test run together hold more than three quarters of the critical path, and the reviewer call, not the linter, sets the parallel stage. A faster retrieval index would save less than a tenth of a second.
...
1. *Playback* re-drives the harness from the recorded model outputs and tool results, without calling the model or any tool. It checks the runtime's own logic, such as budget accounting, schema validation, and state transitions. If the harness mishandled a recorded event, playback reproduces the fault at no model cost.
2. *Model replay* rebuilds the context the model saw at the failing turn, calls the model again, and serves every tool call from the recorded results instead of executing it. It asks whether the model, given exactly that context, tends to make the same proposal.
...
- [ ] **Replay modes**: Which question does playback answer, and which does model replay answer?
...
It is the operational form of the accepted task, and it is the quantity trajectory goodput\index{Trajectory goodput} counts (@sec-vol3-intro-macro-vs-micro-efficiency). Only accepted tasks deliver value, so every resource spent on the rest is charged to them (principle \ref{pri-vol3-trajectory-goodput}).
