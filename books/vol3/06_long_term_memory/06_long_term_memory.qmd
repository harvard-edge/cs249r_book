...
The two previous chapters dealt with state that does not last. The working set is rebuilt for every invocation (@sec-vol3-context-engineering), and attention state lives only as long as its serving session (@sec-vol3-kv-cache). This chapter concerns the durable part of the state a task carries, the $S_3$ state of @sec-vol3-intro-hsa that must outlast both, and how it comes back. Most of it the agent did not create, such as the repository, the database, and the configuration. Some of it the agent did create, such as the record of the actions it has already taken. None of it can enter the model except by being retrieved and staged into context. The question here is which stored records the runtime may treat as true, and what a retrieved record must pass before it reaches the working set.

::: {.column-margin}
**Epistemic gap.** @sec-vol3-context-engineering-context-rot defined the gap between the environment's authoritative state and the stale copy of it in the prompt. In durable storage the stale copy can also be an index entry.
:::

Persistent storage in an agentic system starts from a mismatch between the physical nature of durable records and the unprivileged status of the model. In classical computing systems, as articulated by Saltzer and Kaashoek, memory hierarchies trade latency for capacity, moving from registers and CPU caches down through main memory, solid-state drives, and distributed object stores. A pervasive and dangerous anti-pattern in early agent design treats persistent storage as an undifferentiated "agent memory," an expansion of the model's own capacity, often implemented as an unconstrained vector database into which arbitrary conversational turns, file contents, and reflection notes are indiscriminately dumped. This shortcut ignores what the model is. It holds zero ambient authority (@sec-vol3-intro-operational-incident), so it has no path of its own to durable media and cannot enforce integrity guarantees across them. Persistent storage is not an extension of the model's internal activations; it is an external operating system abstraction managed entirely by the host runtime.

To engineer a dependable storage architecture, the runtime must reject the illusion of a single uniform memory and enforce a rigorous separation between four categorically distinct classes of state: volatile working context, serving key-value activations, authoritative source-of-truth artifacts, and derivative retrieval indexes. Authoritative source-of-truth artifacts—such as version-controlled Git repositories, relational database records, configuration registries, and underlying POSIX filesystems—represent the canonical ground truth of the environment. These artifacts are authoritative because they are mutated solely through explicit, deterministic operations and outlive the agentic system itself. In parallel, trajectory event logs preserve an immutable, append-only historical audit trail of the agent's actions, tool invocations, observed environment responses, and human supervisory overrides, and @sec-vol3-durable-execution builds the write-ahead discipline that keeps that log durable. Derivative retrieval indexes, such as dense embedding tables, BM25 inverted indexes, and abstract syntax tree (AST) symbol graphs, hold no authority of their own (principle \ref{pri-vol3-source-authority}). They are lossy, secondary, disposable computational projections constructed from the authoritative artifacts for the sole purpose of accelerating search queries. Conflating these layers, by treating a lossy vector projection as authoritative truth or letting an agent write to a derivative search index as if it were a primary store, destroys system consistency and renders deterministic auditing impossible, establishing the state tier boundaries formalized in @tbl-vol3-memory-tiers.
...
Whenever an agentic system blurs the boundary between authoritative ground truth and derivative projections, the result is an acute epistemic gap. Consider an autonomous software engineering agent tasked with modifying an authentication service across a sprawling codebase. If the agent mutates a source file on disk using a filesystem write operation, the authoritative artifact changes immediately. However, the vector embeddings residing in the auxiliary retrieval store remain static until re-indexed. If the agent subsequently issues a semantic query to locate the modified function, the retrieval layer queries the stale derivative index and returns the pre-modification code. Reasoning over this obsolete snapshot, the unprivileged model observes a contradiction between its own operational history and the retrieved evidence. Because the model lacks intrinsic physical awareness, it falls into an epistemic failure trap: it assumes its prior write failed, generates duplicate edit operations, reverts valid code changes, or hallucinates non-existent race conditions. Each wrong conclusion then conditions the next turn, the compounding that @sec-vol3-intro-context-poisoning traced for poisoned context.
...
```{python}
#| label: lego-06-sizing-persistent-memory-and-index
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ SIZING PERSISTENT MEMORY AND INDEX MAINTENANCE FOR AN ENTERPRISE CODEBASE (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-06-sizing-persistent-memory-and-index
# │ Goal: Dimension durable repository storage, derivative index sizes, and mutation refresh latency.
# │ Show: 400 MB source code yields 480 MB BM25 and 812.5 MB dense index; re-embedding 500 chunks takes 1.25 s.
# │ How: Model chunk count with 20% stride; calculate dense HNSW vector and pointer footprint.
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time, fmt_multiple, fmt_memory, fmt_memory_capacity

class PersistentMemorySizing:
    """Sizing persistent memory and index maintenance for an enterprise codebase."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_lines = 10_000_000
    n_files = 25_000
    bytes_per_line = 40 * byte
    git_footprint = 2.5 * GB
    ...
    d_embed = 1536
    bytes_per_dim = 2 * byte
    hnsw_m = 32
    bytes_per_ptr = 8 * byte

    n_files_mod = 50
    lines_mod = 20_000
    n_dirty_chunks = 500
    t_nvme_update = 10 * millisecond
    t_lex_update = 15 * millisecond
    t_batch_api = 250 * millisecond
    batch_size_api = 100

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    s_raw = (n_lines * bytes_per_line).to(MB)
    s_lex = s_raw * bm25_mult

    n_chunks = int(round(total_tokens / (chunk_tokens * step_ratio)))
    s_vec = (n_chunks * d_embed * bytes_per_dim).to(MB)
    s_graph = (n_chunks * hnsw_m * bytes_per_ptr).to(MB)
    s_dense = s_vec + s_graph

    n_batches = int(math.ceil(n_dirty_chunks / batch_size_api))
    t_reembed = (n_batches * t_batch_api).to(second)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(s_raw.magnitude - 400.0) < 0.1, "s_raw")
    check(abs(s_lex.magnitude - 480.0) < 0.1, "s_lex")
    check(n_chunks == 244_141, "n_chunks")
    check(abs(s_dense.magnitude - 812.5) < 1.0, "s_dense")
    check(abs(t_reembed.magnitude - 1.25) < 0.01, "t_reembed")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_files_str = fmt_int(n_files)
    git_footprint_gb_str = fmt_memory_capacity(git_footprint, unit=GB, precision=1)
    bm25_expansion_mult_str = fmt_multiple(bm25_mult, precision=1)
    s_dense_mb_str = fmt_memory(s_dense, unit=MB, precision=1)
    n_files_mod_str = fmt_int(n_files_mod)
    lines_mod_str = fmt_int(lines_mod)
    n_dirty_chunks_str = fmt_int(n_dirty_chunks)
    t_nvme_update_ms_str = fmt_time(t_nvme_update, unit=millisecond, precision=0)
    t_lex_update_ms_str = fmt_time(t_lex_update, unit=millisecond, precision=0)
    t_reembed_s_str = fmt_time(t_reembed, unit=second, precision=2)
```

::: {#nbk-06-sizing-persistent-memory-and-index .callout-notebook title="Sizing persistent memory and index maintenance for an enterprise codebase"}
Consider an enterprise software repository containing $10^7$ lines of code across `{python} PersistentMemorySizing.n_files_str` source files. At an average of $40\text{ bytes}$ per line, the raw source text occupies:
$$S_{\text{raw}} = 10^7 \times 40\text{ bytes} = 400\text{ MB}$$
When stored in an authoritative version-controlled repository (Git), compressed object packfiles and historical commit trees expand the durable disk footprint to approximately `{python} PersistentMemorySizing.git_footprint_gb_str` on NVMe storage.

To facilitate autonomous code navigation, the host runtime constructs two derivative retrieval indexes:

1. **Lexical Inverted Index (BM25):** Tokenizing the codebase into $10^8$ lexical terms with token-level position offsets yields an inverted posting file that typically consumes `{python} PersistentMemorySizing.bm25_expansion_mult_str` the raw text size:
...
2. **Dense Vector Embedding Index (HNSW):** Partitioning the codebase into overlapping chunks of $512\text{ tokens}$ with a $20\%$ sliding step yields:
   ...
   The combined footprint for the vector retrieval index is $S_{\text{dense}} \approx$ `{python} PersistentMemorySizing.s_dense_mb_str`.

**Index Maintenance Cost under Mutation:**
Suppose an agent executes a multi-file refactoring step that modifies `{python} PersistentMemorySizing.n_files_mod_str` files, altering `{python} PersistentMemorySizing.lines_mod_str` lines of code and invalidating $N_{\text{dirty}} \approx$ `{python} PersistentMemorySizing.n_dirty_chunks_str` chunks. Updating the authoritative source tree on NVMe SSD takes under `{python} PersistentMemorySizing.t_nvme_update_ms_str`. Updating the lexical inverted index via an in-memory postings update takes less than `{python} PersistentMemorySizing.t_lex_update_ms_str`.

However, refreshing the dense vector index requires passing all $500\text{ dirty chunks}$ through an external embedding model inference pass. At an API batch latency of $250\text{ ms}$ per 100 chunks, re-embedding introduces a `{python} PersistentMemorySizing.t_reembed_s_str` wall-clock delay and incurs external compute costs. If the runtime naively re-indexes synchronously on every file mutation, the agent's execution loop stalls completely. Conversely, if the runtime defers re-indexing without tracking dirty flags, any subsequent query over those `{python} PersistentMemorySizing.n_files_mod_str` files reads stale embeddings, creating an epistemic gap. The storage subsystem must therefore decouple synchronous authoritative writes from asynchronous index reconciliation, mediating reads through explicit version-aware filters.
:::
...
The runtime therefore places a barrier between storage and the model's inputs. This is the invariant closure principle (\ref{pri-invariant-closure}) applied to reads, and it follows the end-to-end argument of @saltzer1984endtoend. A foundation model must never interact with persistent storage through raw, unmediated data streams. Retrieved candidates from external stores cannot be assumed to be correct, complete, or safe simply because a similarity search yielded a high mathematical score.
...
Before any candidate record is staged into the active working context, the runtime's verification boundary inspects the candidate against authoritative ground truth. It checks the candidate's provenance metadata, confirms that the underlying source file still exists on the filesystem, compares the indexed checksum against the live file hash to detect silent staleness, and verifies that the active security descriptor permits the agent to inspect the artifact under the principle of least privilege. A similarity score is a claim the index makes about its own output, the retrieval analogue of the self-report to which @sec-vol3-intro-closure-evidence gave no evidential weight. Only after the candidate successfully passes through this deterministic verification gateway does the runtime stage the extracted text into the model's working context window for the next autoregressive decode step.

By decoupling authoritative source artifacts from derivative search structures, the runtime insulates the system against epistemic failure. When a file is modified, the runtime flags the corresponding regions in the derivative indexes as dirty, routing sub-queries directly to authoritative filesystems until background reconciliation completes, a mechanism @sec-vol3-persistent-invalidation builds in full. Persistent storage ceases to be a shadowy, hallucination-prone "memory" and becomes what systems engineering demands: a predictable, auditable, and verifiable storage hierarchy.
...
A naive retrieval call treats external storage as an unconstrained associative pipe: an arbitrary natural language string enters a retrieval index, and an unvetted collection of text fragments pours directly into the foundation model's active working context. In production agent runtimes, this unconstrained pipe produces catastrophic system failures. Consider an autonomous software engineering agent tasked with resolving a build regression in a monorepo spanning $10^7$ lines of code. The agent issues an unstructured query for `build_matrix.json`, receives an unranked four-megabyte continuous integration log dump from an obsolete branch executed six months prior, consumes 98 percent of its remaining context window on stale configuration keys, and emits erroneous patching commands based on superseded schema definitions. The breakdown is neither a neural hallucinatory defect nor an index algorithmic flaw; it is an architectural boundary failure.

**Reliable retrieval requires a formal typed query-response contract with explicit freshness bounds, score thresholds, provenance metadata, and latency limits.** Durable information reaches the model only through such a contract (principle \ref{pri-vol3-retrieval-contract}), never through an informal query. Just as traditional operating systems interpose formal system call contracts between untrusted user processes and block storage devices, the agent runtime must interpose a typed retrieval contract that governs search scope, bounds execution latency, tracks cryptographic provenance, and quarantines unverified content before it is staged into working context (@fig-vol3-retrieval-contract).

::: {#fig-vol3-retrieval-contract fig-env="figure" fig-pos="htb" fig-cap="**The Episodic Memory Retrieval Contract Protocol**: Formal query-response protocol between the host supervisor and episodic storage engine. The host runtime packages context and constraints into a typed query tuple $\mathcal{Q}$, bounded by strict latency budgets $T_{\max}$ and freshness requirements $\tau_{\max}$. The storage engine executes dense/sparse retrieval and returns an authenticated result envelope $\mathcal{R}_i$ with cryptographic lineage hashes. Before context injection, the ingress quarantine gate enforces token ceilings $S_{\max}$, validates BLAKE3 hashes, and scrubs delimiter hijacking payloads." fig-alt="Systems protocol diagram showing three bounded domains: Host Agent Runtime dispatching a typed query tuple Q, Episodic Storage Engine executing retrieval and returning an authenticated result envelope R_i, and an Ingress Quarantine Gate enforcing validation rules before context injection."}
![](images/svg/episodic_memory_stack.svg)
:::

As illustrated in @fig-vol3-retrieval-contract, the storage interaction is partitioned across three distinct physical domains: the unprivileged Host Runtime, the Persistent Storage Subsystem, and an Ingress Quarantine Gate. The host initiates the transaction by dispatching a typed query tuple $\mathcal{Q} = \langle \mathbf{q}_T, \mathcal{S}_{\text{scope}}, \tau_{\text{freshness}}, T_{\text{budget}} \rangle$. Rather than querying storage blindly, the query tuple bounds both spatial access (restricting search to authorized repository paths or tenant namespaces) and temporal validity. The storage engine processes this tuple across its derivative index stores (lexical inverted postings, dense vector indexes, and structural symbol graphs) and responds with a structured Result Envelope $\mathcal{R}_i = \langle \text{DocID}, \text{Payload}, \mathbf{h}_{\text{BLAKE3}}, \tau_{\text{mtime}}, \mathcal{C}_{\text{trust}} \rangle$. Crucially, raw payloads never enter the model's self-attention context directly. Instead, they must pass through an Ingress Quarantine Gate where cryptographic source hashes $\mathbf{h}_{\text{BLAKE3}}$ verify data integrity, maximum token bounds $S_{\max}$ truncate bloat, and high-entropy secret scrubbers sanitize untrusted content against indirect prompt injection.

::: {#dfn-episodic-retrieval-contract .callout-definition title="Episodic retrieval contract"}

***Episodic retrieval contract***\index{Episodic retrieval contract!definition}\index{Retrieval contract!definition} is a formal, typed query-response protocol $\langle \mathcal{Q}, \mathcal{R} \rangle$ interposed between the host runtime and external storage that bounds spatial access, execution latency $T_{\text{budget}}$, and temporal freshness $\tau_{\text{freshness}}$ while validating cryptographic lineage $\mathbf{h}_{\text{BLAKE3}}$ before records are admitted into working context.

1.  **Significance**: Eliminates the naive "associative pipe" anti-pattern where unranked external documents flood active attention, protecting accelerator high-bandwidth memory from KV-cache bloat and safeguarding against indirect prompt injection.
2.  **Distinction**: Unlike classical database queries (which assume static schemas and trusted clients) or open-loop RAG pipelines (which blindly inject top-$k$ nearest neighbors), the episodic retrieval contract quarantines retrieved candidates behind deterministic verification gates that validate checksums and file existence against authoritative source artifacts before context injection.
3.  **Common pitfall**: Omitting the freshness bound $\tau_{\text{freshness}}$ or spatial scope $\mathcal{S}_{\text{scope}}$, allowing derivative vector indices to return superseded file snapshots or cross-tenant artifacts that poison downstream reasoning with phantom compiler errors or security leaks.
...
Version decides staleness. The age bound $\tau_{\text{freshness}}$ on the drift $\Delta t = t_{\text{current}} - t_{\text{indexed}}$ applies only to sources that emit no change events.
:::

The temporal parameter $\tau_{\text{freshness}} \in \mathbb{R}^+$ defines the maximum permissible age of retrieved index entries relative to the authoritative source. In a rapidly mutating environment, such as an active coding session where an agent generates, compiles, and modifies files, derivative vector and keyword indexes lag behind the filesystem. Freshness is judged by version first (principle \ref{pri-vol3-source-authority}). An entry whose recorded version no longer matches its source is stale however recent it is, because an agent that edits a file and queries the index 50 ms later sees an entry well inside any age bound. The runtime catches that case with the version check and the tombstones of @sec-vol3-persistent-invalidation. The parameter $\tau_{\text{freshness}}$ bounds age only for sources that emit no change events. When an entry from such a source is older than $\tau_{\text{freshness}}$, the runtime bypasses the derivative search index and routes the query directly to authoritative storage.
...
```{python}
#| label: lego-06-sizing-retrieval-latency-budgets-and
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ SIZING RETRIEVAL LATENCY BUDGETS AND CONTEXT STAGING OVERHEAD (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-06-sizing-retrieval-latency-budgets-and
# │ Goal: Dimension index traversal, storage fetch, prefill delay, and KV cache footprint.
# │ Show: Top-5 contract cuts prefill from 300 to 30 ms and bounds KV cache from 7.86 to 0.79 GB.
# │ How: Graph traversal evaluations plus storage RPC; prefill latency derived from token throughput.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time, fmt_memory_capacity

class RetrievalLatencyBudgets:
    """Sizing retrieval latency budgets and context staging overhead."""
    ...
    hnsw_m = 64
    evals_per_sec = 1.5e7
    t_index = 4.2 * millisecond
    t_fetch = 3.8 * millisecond
    t_budget = 25 * millisecond

    k_unconstrained = 50
    prefill_tok_per_s = 50_000
    n_layers = 32
    h_kv = 32
    d_head = 128
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    t_graph = ((ef_search * hnsw_m) / evals_per_sec) * second
    t_storage = t_index + t_fetch
    t_headroom = t_budget - t_storage

    s_total_unconstrained = k_unconstrained * s_chunk
    t_prefill_unconstrained = (s_total_unconstrained / prefill_tok_per_s) * second
    mem_kv_unconstrained = (2 * n_layers * h_kv * d_head * bytes_per_elem * s_total_unconstrained).to(GB)

    tokens_top = k_top * s_chunk
    t_prefill_constrained = (tokens_top / prefill_tok_per_s) * second
    mem_kv_constrained = (2 * n_layers * h_kv * d_head * bytes_per_elem * tokens_top).to(GB)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_graph.to(millisecond).magnitude - 0.55) < 0.05, "t_graph")
    check(abs(t_storage.to(millisecond).magnitude - 8.0) < 0.1, "t_storage")
    check(abs(t_headroom.to(millisecond).magnitude - 17.0) < 0.1, "t_headroom")
    check(s_total_unconstrained == 15_000, "s_total_unconstrained")
    check(abs(t_prefill_unconstrained.to(millisecond).magnitude - 300) < 1, "t_prefill_unconstrained")
    check(abs(mem_kv_unconstrained.magnitude - 7.86) < 0.05, "mem_kv_unconstrained")
    check(tokens_top == 1500, "tokens_top")
    check(abs(t_prefill_constrained.to(millisecond).magnitude - 30) < 1, "t_prefill_constrained")
    check(abs(mem_kv_constrained.magnitude - 0.79) < 0.05, "mem_kv_constrained")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    s_chunk_str = fmt_int(s_chunk)
    t_index_ms_str = fmt_time(t_index, unit=millisecond, precision=1)
    t_fetch_ms_str = fmt_time(t_fetch, unit=millisecond, precision=1)
    t_budget_ms_str = fmt_time(t_budget, unit=millisecond, precision=0)
    t_headroom_ms_str = fmt_time(t_headroom, unit=millisecond, precision=0)
    k_unconstrained_str = fmt_int(k_unconstrained)
    k_top_str = fmt_int(k_top)
    tokens_top_str = fmt_int(tokens_top)
    t_prefill_unconstrained_ms_str = fmt_time(t_prefill_unconstrained, unit=millisecond, precision=0)
    t_prefill_constrained_ms_str = fmt_time(t_prefill_constrained, unit=millisecond, precision=0)
    mem_kv_constrained_gb_str = fmt_memory_capacity(mem_kv_constrained, unit=GB, precision=2)
```

::: {#nbk-06-sizing-retrieval-latency-budgets-and .callout-notebook title="Sizing retrieval latency budgets and context staging overhead"}
Consider an autonomous agent operating over a software repository indexed into $N = 10^7$ text chunks. Each chunk contains an average of $S_{\text{chunk}} =$ `{python} RetrievalLatencyBudgets.s_chunk_str` tokens. The vector index uses Hierarchical Navigable Small World (HNSW) graphs with $M = 64$ bidirectional links per node and an exploration depth $efSearch = 128$. Embeddings are stored in $\text{FP16}$ precision ($d = 1024$, consuming $2048\text{ bytes}$ per vector).
...
Including disk-backed inverted file lookups and concurrent memory bus contention on shared accelerator nodes, the physical search engine achieves a 99th percentile index latency of $t_{\text{index}} =$ `{python} RetrievalLatencyBudgets.t_index_ms_str`.

**2. Network and Storage I/O:**
Fetching the raw text chunks from a distributed key-value store introduces an RPC round-trip and NVMe fetch latency of $t_{\text{fetch}} =$ `{python} RetrievalLatencyBudgets.t_fetch_ms_str`. Total backend storage latency is:
$$t_{\text{storage}} = t_{\text{index}} + t_{\text{fetch}} = 4.2\text{ ms} + 3.8\text{ ms} = 8.0\text{ ms}$$
This comfortably satisfies a query latency budget of $T_{\text{budget}} =$ `{python} RetrievalLatencyBudgets.t_budget_ms_str`, leaving `{python} RetrievalLatencyBudgets.t_headroom_ms_str` of headroom for network jitter.

**3. Context Window Staging and KV Cache Overhead:**
If the query omits the candidate ceiling and allows an unconstrained candidate set of $K =$ `{python} RetrievalLatencyBudgets.k_unconstrained_str` chunks into working context, the injected payload size is:
...
Furthermore, in a transformer model with $L = 32$ layers, $H_{\text{kv}} = 32$ key-value heads, and head dimension $D = 128$ operating in $\text{FP16}$ ($2\text{ bytes}$ per number), caching the key and value states for these $15{,}000$ tokens consumes physical accelerator memory:
$$\text{Memory}_{\text{KV}} = 2 \times L \times H_{\text{kv}} \times D \times 2\text{ bytes} \times S_{\text{total}}$$
$$\text{Memory}_{\text{KV}} = 2 \times 32 \times 32 \times 128 \times 2 \times 15{,}000 = 7{,}864{,}320{,}000\text{ bytes} \approx 7.86\text{ GB}$$
By enforcing a strict retrieval contract with $K_{\text{top}} =$ `{python} RetrievalLatencyBudgets.k_top_str` and a score threshold $\sigma_{\min} = 0.78$, the runtime admits only the top `{python} RetrievalLatencyBudgets.k_top_str` verified chunks (`{python} RetrievalLatencyBudgets.tokens_top_str` tokens). This slashes the prefill penalty from `{python} RetrievalLatencyBudgets.t_prefill_unconstrained_ms_str` to `{python} RetrievalLatencyBudgets.t_prefill_constrained_ms_str` and restricts KV cache allocation to `{python} RetrievalLatencyBudgets.mem_kv_constrained_gb_str`, preventing out-of-memory faults on the host accelerator.
:::
...
::: {#chk-06-retrieval-contract .callout-checkpoint title="Evaluating the storage retrieval contract"}
Before analyzing structured lexical and symbol indexing, verify your understanding of retrieval contracts and ingress boundaries:

- [ ] **Query tuple**: How do temporal predicates, provenance scopes, and token budgets bound retrieval execution before disk I/O?
- [ ] **Ingress quarantining**: Why must retrieved external documents be quarantined and structurally delimited before staging into prompt context?
- [ ] **Index drift**: How does cryptographic content hashing detect divergence between the filesystem state and precomputed index embeddings?
- [ ] **Authority separation**: Why is the storage engine strictly untrusted relative to the host supervisor's environment ground truth?
:::
...
The industry-standard scoring function addressing this balance is Okapi BM25, a non-linear probabilistic relevance framework formalized by @robertson2009bm25. For a query $\mathcal{Q}$ and a document $D$, the BM25 score is defined as:
...
::: {#fig-vol3-lexical-vs-ast fig-env="figure" fig-pos="htb" fig-cap="**Inverted Index versus AST Symbol Graph Topology**: Comparison between flat lexical postings lists and syntax-directed code property graphs. While lexical postings map terms to disjoint document identifiers, the symbol graph models explicit semantic edges including class implementation, method definition, and functional call dependencies." fig-alt="Architectural comparison diagram showing a lexical inverted index mapping terms to document IDs on the left, and a syntax-directed code property graph resolving classes, methods, and call edges on the right."}
![](images/svg/symbol_graph_topology.svg)
:::

As illustrated in @fig-vol3-lexical-vs-ast, the two storage architectures exhibit fundamentally divergent topologies. In Panel A, the lexical inverted index decomposes source text into isolated tokens (`"connect"`, `"socket"`, `"retry"`), mapping each term to a flat list of document identifiers and term frequencies ($TF$). This representation is completely blind to execution context: an identifier appearing inside a comment or unit test fixture shares identical index status with an active class declaration. In contrast, Panel B models source code as a syntax-directed Code Property Graph (CPG). The abstract syntax tree explicitly models entities as typed nodes—such as the `TransportClient` interface, the concrete `TcpClient` class, and the `connect()` member function—connected by directed, typed edges (`IMPLEMENTS`, `DEFINES`, `CALLS`). This topological representation enables the agent runtime to issue structural queries that are impossible under pure lexical indexing:
...
The most pervasive failure mode of lexical indexing is the *vocabulary mismatch problem*, originally analyzed by @furnas1987vocabulary. Human developers and autonomous agents frequently describe computational operations using terminology that shares zero literal tokens with the underlying implementation. Consider an agent attempting to investigate an unhandled connection drop:
...
::: {#fig-vol3-lexical-ast fig-env="figure" fig-pos="htb" fig-cap="**Vocabulary Mismatch and Structural Blindness Pathologies**: Unimodal retrieval failure modes under unassisted agent queries. In Panel A, lexical BM25 fails to match query terms to the implementing struct due to vocabulary mismatch. In Panel B, static AST symbol graphs are blind to dynamic RPC interfaces and metaprogrammed boundaries, severing execution edges." fig-alt="Diagram of retrieval failure modes showing lexical BM25 returning zero hits due to synonym vocabulary mismatch on the left, and static AST graph failing to traverse dynamic RPC boundaries on the right."}
![](images/svg/vol3-lexical-vs-ast.svg)
:::

The mechanics of these dual failure modes are detailed in @fig-vol3-lexical-ast. When the autonomous agent issues the natural language query `"find socket connection rate limiter algorithm"`, its operational intent is to locate the traffic-shaping primitive controlling network packet ingress. In Pathology 1 (left), the lexical inverted index parses the query into discrete posting keys `{"socket", "connection", "rate", "limiter"}`. However, the codebase implements this mechanism as `struct TokenBucketDrain { uint64_t drain_rate; ... };`. Because there is zero literal token overlap, the postings list intersection returns exactly zero hits, triggering a total retrieval collapse. Symmetrically, in Pathology 2 (right), the agent leverages a static AST symbol table that successfully indexes `TokenBucketDrain`. However, the invocation path passes through a cross-process RPC interface `channel.invoke("RateLimiterService", payload)`. Because the edge is dynamically constructed at runtime via string reflection, the static AST parser cannot resolve the target symbol, severing the graph traversal edge. Unimodal retrieval fails symmetrically: lexical BM25 is brittle to vocabulary variation, while static AST graphs are blind to dynamic execution boundaries.
...
::: {.column-margin}
![](images/svg/vol3_episodic_memory_margin_002.svg){width="100%" fig-alt="A curve rising steeply from a baseline and then flattening to a plateau, with a marker at the flattening point."}

*Task success peaks at five retrieved chunks; beyond it distractors dilute the signal.*
:::
...
```{python}
#| label: lego-06-memory-bandwidth-and-latency-floor
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ MEMORY BANDWIDTH AND LATENCY FLOOR OF EXACT VECTOR SCANS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-06-memory-bandwidth-and-latency-floor
# │ Goal: Demonstrate memory bandwidth bottleneck of brute-force linear vector search.
# │ Show: 15.36 GB scan at 68 GB/s imposes a 226 ms latency floor; 20 probes consume >4.5 s.
# │ How: Calculate arithmetic FLOPs vs memory bus transfer time under sustained DDR5 bandwidth.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time

class VectorScanLatencyFloor:
    """Memory bandwidth and latency floor of exact vector scans."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_vectors = 10_000_000
    d_dim = 768
    bytes_per_val = 2 * byte
    bw_read = 68 * (GB / second)
    peak_gflops = 2500 * (GFLOP / second)
    n_probes = 20

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    footprint = (n_vectors * d_dim * bytes_per_val).to(GB)
    flops_total = 2 * d_dim * n_vectors * flop
    t_arith = (flops_total / peak_gflops).to(second)
    t_mem = (footprint.to(byte) / bw_read).to(second)
    t_floor_rounded = round(t_mem.to(millisecond).magnitude) * millisecond
    t_wasted_probes = (n_probes * t_mem).to(second)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(footprint.magnitude - 15.36) < 0.05, "footprint")
    check(abs(t_arith.to(millisecond).magnitude - 6.14) < 0.1, "t_arith")
    check(abs(t_mem.to(millisecond).magnitude - 225.88) < 0.2, "t_mem")
    check(abs(t_wasted_probes.magnitude - 4.52) < 0.1, "t_wasted_probes")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    t_floor_ms_str = fmt_time(t_floor_rounded, unit=millisecond, precision=0)
    n_probes_str = fmt_int(n_probes)
    t_wasted_probes_s_str = fmt_time(t_wasted_probes, unit=second, precision=1)
```
...
**Conclusion:**
Because $T_{\text{mem}} \gg T_{\text{arith}}$, the exact scan is deeply memory-bound, imposing an unyielding latency floor of $\approx$ `{python} VectorScanLatencyFloor.t_floor_ms_str` per retrieval step. If an agent executes an iterative multi-turn debugging trajectory requiring `{python} VectorScanLatencyFloor.n_probes_str` storage probes, exact linear scanning consumes over `{python} VectorScanLatencyFloor.t_wasted_probes_s_str` in pure memory bus transport, rendering interactive responsiveness impossible. Durable storage runtimes must abandon exact $O(N)$ scanning in favor of sub-linear approximate nearest neighbor data structures.
:::
...
::: {#fig-vol3-hnsw-routing fig-env="figure" fig-pos="htb" fig-cap="**Hierarchical Navigable Small World (HNSW) Multi-Scale Routing**: Logarithmic-time proximity search via multi-layer skip-graph topologies. Greedy routing hops across sparse geometric highways in upper layers before descending vertically to Layer 0 for bounded beam search." fig-alt="Multi-layer HNSW graph schematic showing entry point v0 at Layer 2, greedy long-range hops, vertical descent to Layer 1 and Layer 0, and local neighborhood clustering with priority queue beam search."}
![](images/svg/hnsw_hierarchical_routing.svg)
:::

The multi-scale routing mechanics are depicted in detail in @fig-vol3-hnsw-routing. A query vector $\mathbf{u}_Q$ enters the index at Layer 2 through a designated entry node $v_{\text{entry}} = v_0$. In this top layer, edges span wide geometric distances across the manifold, allowing greedy search to bypass vast coordinate voids in a single step, advancing to node $v_1$. Finding no neighbor closer to $\mathbf{u}_Q$, the traversal reaches its layer local minimum and executes a vertical transition down to Layer 1 at the identical coordinate point $v_1$. At Layer 1, the algorithm resumes greedy navigation across intermediate-length links, hopping from $v_1$ to $v_3$. Detecting that $v_3$ is the closest vertex to $\mathbf{u}_Q$ on this layer, the search drops vertically to Layer 0. Layer 0 contains the complete dataset $N$ organized into dense, high-clustering local neighborhoods. Here, routing switches from a single-path greedy walk to a bounded beam search governed by dynamic priority queue capacity $efSearch = 4$. The runtime inspects candidate vertices ($v_4, v_5, v_6, v_7$), pruning vertices that exceed the beam distance boundary and returning the top-$K$ approximate nearest neighbors with empirical $O(\log N)$ latency.
...
When combined with alignment padding and heap metadata, an HNSW index can expand the base storage requirements of the vector dataset by 25 to 50 percent, requiring that the entire graph and its associated vectors reside permanently in host RAM to avoid catastrophic disk paging during graph hops.
...
For each subspace, the engine trains an independent codebook of $k^* = 256$ centroids via $k$-means. Because $k^* = 256$, any sub-vector within a given subspace can be represented by an 8-bit unsigned integer (1 byte) identifying its closest local centroid. For an uncompressed embedding vector of dimension $D = 1536$ in 32-bit floating-point precision ($\text{FP32}$, consuming $1536 \times 4 = 6,144 \text{ bytes}$), quantizing into $M = 64$ sub-vectors of dimension $d^* = 24$ reduces the footprint down to exactly 64 bytes—an immense compression ratio of $96.0\times$ ($48.0\times$ relative to $\text{FP16}$), as illustrated in @fig-vol3-ivf-pq. In extreme archival configurations, reducing quantization to $M = 16$ coarse sub-vectors alongside a 2-byte Voronoi centroid prefix yields an 18-byte record, delivering a $341.3\times$ storage reduction.

::: {#fig-vol3-ivf-pq fig-env="figure" fig-pos="htb" fig-cap="**Offline Compression and Online Asymmetric Distance Computation (IVF-PQ / ADC)**: Transforming continuous high-dimensional vector search into discrete byte-code lookups. Panel A decomposes input vectors into quantized sub-codebooks; Panel B precomputes a compact distance LUT in CPU L1 cache for zero-FLOP postings scans." fig-alt="Diagram of product quantization showing a 1536-dimensional FP32 vector sliced into 64 sub-vectors and mapped to 8-bit centroid indices in Panel A, and online Asymmetric Distance Computation utilizing an L1 cache lookup table to score candidates with zero floating-point operations in Panel B."}
![](images/svg/episodic_ivf_pq.svg)
:::

The operational workflow of IVF-PQ and Asymmetric Distance Computation (ADC) is illustrated in @fig-vol3-ivf-pq. In the offline compression phase (Panel A), the storage engine takes uncompressed continuous vectors $\mathbf{x} \in \mathbb{R}^{1536}$ (occupying $6,144\text{ bytes}$ in FP32) and slices each vector into $M = 64$ equal sub-vectors of dimension $d^* = 24$. For each subspace, an independent codebook $\mathcal{C}_m$ of $k^* = 256$ centroids is trained via $k$-means. Each sub-vector is mapped to its nearest centroid index $c_m \in [0, 255]$, yielding a compact 64-byte discrete code $\mathbf{c} = [c_1, c_2, \dots, c_{64}]$, which achieves a $96.0\times$ storage reduction. During online query execution (Panel B), the query vector $\mathbf{u}_Q$ arrives uncompressed. The CPU slices $\mathbf{u}_Q$ into 64 sub-vectors and precomputes an asymmetric distance Look-Up Table (LUT) containing the squared Euclidean distance between each query sub-vector and all 256 prototype centroids. Because the resulting table consumes only $64 \times 256 \times 4\text{ bytes} = 64\text{ KB}$, it resides entirely within CPU L1/L2 cache. Scanning candidate vectors within selected Voronoi postings lists then requires zero floating-point multiplications: the runtime simply loads the 64 byte indices, performs 64 direct memory lookups into L1 cache, and accumulates the distances via SIMD integer additions.
...
| **DRAM Footprint ($10^7$ vectors, $D=1536$, FP32)** | $61.44 \text{ GB}$ (uncompressed)   | $\approx 0.64 \text{ GB}$ ($96.0\times$ compression)  | $\approx 71.68 \text{ GB}$ (vectors + HNSW graph links) |
...
: **Vector Indexing Mechanism Trade-offs**: Algorithmic and hardware trade-offs of vector indexing mechanisms across exact scan, IVF-PQ ($M=64$), and HNSW ($M=32$). Performance characteristics reflect $N = 10^7$ vectors at $D=1536$ in 32-bit floating-point precision ($6,144\text{ bytes}$ uncompressed vs. $64\text{ bytes}$ quantized). {#tbl-vol3-ann-tradeoffs}

::: {#fig-vol3-tiered-vector-index-lifecycle fig-env="figure" fig-pos="htb" fig-cap="**Tiered Vector Index Storage and Lifecycle Architecture**: Multi-level memory and storage hierarchy migrating vector records across Hot (DRAM/HBM), Warm (NVMe SSD), and Cold (Object Storage) tiers. Incoming writes append to an in-memory MemTable before background flush and quantization compress $D=1536$ FP32 vectors into 64-byte codes ($96\times$ compression) for sequential SSD scan, ultimately archiving into immutable Parquet files." fig-alt="Systems architecture diagram of tiered vector index lifecycle illustrating Hot tier MemTable and write-ahead log in DRAM, Warm tier memory-mapped IVF-PQ on NVMe SSD, and Cold tier columnar Parquet files in object storage, linked by flush, quantize, and compaction paths."}
![](images/svg/tiered_vector_index_lifecycle.svg)
:::

The end-to-end lifecycle of vector storage across the physical memory hierarchy is depicted in @fig-vol3-tiered-vector-index-lifecycle. Durable vector engines avoid holding billions of high-dimensional vectors permanently in expensive DRAM by partitioning storage across three distinct tiers:

1. **Hot Tier (DRAM / HBM):** Serves active agent ingestion. Incoming vector writes append synchronously to an append-only Write-Ahead Log (WAL) and an in-memory MemTable storing raw uncompressed FP32 vectors ($6,144\text{ bytes}$ per record at $D=1536$). This tier guarantees immediate point-in-time read-your-own-writes consistency with sub-millisecond retrieval latency, but is strictly capacity-constrained.
2. **Warm Tier (NVMe SSD / Local Flash):** When the MemTable exceeds its capacity threshold, a background compaction worker flushes and quantizes vectors into Inverted File Product Quantization (IVF-PQ) segments. By quantizing vectors into $M = 64$ sub-vectors, the record footprint shrinks by $96.0\times$ down to 64 bytes per vector. Stored as memory-mapped files (`mmap`), these segments support high-throughput Asymmetric Distance Computation scans over millions of records directly from kernel page cache, maintaining $5\text{--}15\text{ ms}$ retrieval latency while offloading 98 percent of vector capacity from DRAM.
3. **Cold Tier (Cloud Object Storage / S3 / Ceph):** Historical session snapshots, inactive repository branches, and cold episodic logs are periodically compacted into immutable columnar Parquet files. By applying aggressive coarse quantization ($M=16$ with a 2-byte Voronoi centroid prefix, yielding $18\text{ bytes}$ per vector and $341.3\times$ compression) alongside zstandard block compression, long-term retention costs drop below $\$0.02$ per gigabyte-month. When an agent requires long-tail historical context, an asynchronous index rebuilder hydrates segments back into the Warm tier without disturbing active foreground execution.
...
::: {#dfn-reciprocal-rank-fusion .callout-definition title="Reciprocal rank fusion (RRF)"}
A rank-aggregation algorithm that combines candidate lists from multiple heterogeneous retrieval systems by summing their reciprocal ordinal positions, eliminating the need to calibrate disparate continuous scoring scales.
:::
...
::: {#fig-vol3-hnsw-rrf fig-env="figure" fig-pos="htb" fig-cap="**Multi-Stage Hybrid Retrieval Pipeline**: Dual sparse lexical (BM25) and dense approximate nearest neighbor (HNSW/IVF-PQ) engines generate candidate sets in parallel. Reciprocal Rank Fusion (RRF) resolves score incommensurability through parameter-free ordinal merging ($k=60$), producing a unified candidate pool. A full-attention cross-encoder then re-ranks candidates with $O(k \\cdot L^2)$ all-to-all token attention, yielding high-precision staged context ($k^* \\in [3, 5]$)." fig-alt="Flowchart of multi-stage hybrid retrieval showing agent query branching into parallel sparse BM25 and dense HNSW engines, merging into Reciprocal Rank Fusion stage, passing to full-attention cross-encoder re-ranking, and outputting top high-assurance context."}
![](images/svg/vol3_ch06_hnsw_rrf.svg)
:::

To achieve maximal precision before context insertion, state-of-the-art architectures deploy the multi-stage retrieval pipeline illustrated in @fig-vol3-hnsw-rrf. An agent query $Q$ (such as `"reclaim orphaned file descriptors"`) branches simultaneously into two complementary retrieval substrates: a sparse lexical engine scanning BM25 inverted postings for exact term and symbol matches, and a dense vector engine traversing an HNSW or IVF-PQ index for geometric cosine proximity. Each engine retrieves an initial broad candidate pool of top-100 entries. In Stage 2, Reciprocal Rank Fusion (RRF) reconciles their fundamentally incommensurate scoring scales by taking parameter-free reciprocal ranks ($\text{RRF}(d) = \sum_{m} \frac{1}{60 + r_m(d)}$), producing a consolidated candidate pool of $K_{\text{pool}} \approx 50$ documents. In Stage 3, this compressed candidate set is evaluated by a full-attention cross-encoder transformer model. Unlike decoupled bi-encoders, the cross-encoder computes full all-to-all token attention between query and candidate text ($\text{Softmax}(\mathbf{Q} \mathbf{K}^T / \sqrt{d_k})$), evaluating prepositional logic, syntactic negation, and subtle architectural invariants. Because cross-encoders evaluate only the 50 candidate passages pre-filtered by the hybrid stage, their $O(K_{\text{pool}} \cdot L^2)$ computational overhead requires mere tens of milliseconds, filtering out subtle semantic false positives and admitting only high-assurance evidence ($k^* \in [3, 5]$) into the agent's constrained working context window.
...
This failure manifests empirically as the *k-hop dependency problem*. Suppose the resolution of a system invariant requires answering a directed dependency chain of length $k$:
...
To overcome the chunk fragmentation boundary, persistent storage must model software as a formal multi-relational graph. Following the foundational formulation of Code Property Graphs (CPGs) by @yamaguchi2014modeling and subsequent graph-augmented retrieval architectures [@edge2024graphrag], we model a software repository as a directed, edge-labeled, attributed multigraph:
...
::: {#fig-vol3-cpg-traversal fig-env="figure" fig-pos="htb" fig-cap="**Code Property Graph Multi-Hop Traversal**: Structural call-path traversal versus isolated text chunk retrieval. Panel A traces deterministic multi-hop edges across files (`CALLS`, `READS`, `MODIFIES`). Panel B demonstrates seeded neighborhood expansion with Personalized PageRank (PPR) pruning, preventing exponential context overflow while retaining intact causal dependency chains." fig-alt="Diagram showing multi-hop code property graph traversal across transaction handler, crypto signature, and hardware security module nodes in Panel A, and seeded neighborhood expansion comparing unconstrained BFS explosion with Personalized PageRank pruning in Panel B."}
![](images/svg/cpg-multi-hop-traversal.svg)
:::

As shown in @fig-vol3-cpg-traversal, the structural graph eliminates the blind spots inherent in chunking. In Panel A, the runtime traces an intact execution path across disparate compilation units: `process_transaction()` in `tx_handler.c` calls `validate_signature()` in `sec_crypto.c`, which in turn reads a global key struct in `HardwareSecModule` while modifying state in `TxLedgerRecord`:
...
This relationship is preserved explicitly via pointer references in the graph index, completely independent of whether these three symbols share lexical tokens, reside in the same source directory, or were authored in different programming languages. Panel B contrasts the topological expansion strategies: while unconstrained Breadth-First Search (BFS) explodes to over 1,300 functions ($235\text{k}$ tokens, an overwhelming $28.8\times$ overflow), biased Personalized PageRank (PPR) traversal focuses exclusively on high-relevance paths, assembling a compact 38-function subgraph consuming only 6,840 tokens out of an 8,192 budget.

### Multi-hop graph traversal

Graph-structured retrieval does not replace dense and lexical search; it orchestrates them. Pure graph traversal without an entry point requires exhaustive scanning of millions of vertices, whereas pure vector search locates isolated entries but cannot follow connections. Modern agent runtimes bridge this divide using *seeded neighborhood expansion* (@fig-vol3-cpg-traversal), a two-stage process combining hybrid similarity search with structural graph traversal.

In the first stage, the agent's query $\mathcal{Q}$ is submitted to the hybrid lexical-vector index described in @sec-vol3-persistent-dense-retrieval. The engine evaluates Reciprocal Rank Fusion (RRF) across the vertex attribute store $\mu_V$, scoring all functions, classes, and documentation blocks. Instead of returning these passages directly to the agent's working context, the engine selects the top-$k_0$ scoring candidates as an initial *seed set*:

$$V_0 = \{v \in V \mid \text{rank}_{\text{RRF}}(v, \mathcal{Q}) \le k_0\}$$

In the second stage, the runtime initiates a deterministic graph traversal originating at $V_0$. The simplest expansion policy executes a bounded Breadth-First Search (BFS) up to a static hop depth $d_{\text{hop}}$, collecting all vertices within graph distance $\text{dist}(u, v) \le d_{\text{hop}}$ for any $u \in V_0$. However, unconstrained BFS across dense call graphs triggers an immediate combinatorial explosion: popular utility functions (such as logging routines, memory allocators, or lock primitives) exhibit in-degrees and out-degrees exceeding $10^3$, instantly flooding the candidate set with irrelevant nodes.
...
```{python}
#| label: lego-06-graph-neighborhood-expansion-vs-context
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ GRAPH NEIGHBORHOOD EXPANSION VS. CONTEXT BUDGET (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-06-graph-neighborhood-expansion-vs-context
# │ Goal: Contrast context overflow under BFS expansion vs bounded Personalized PageRank pruning.
# │ Show: 3-hop BFS explodes to 235,800 tokens (28.8x overflow); PPR isolates 38 functions (6,840 tokens).
# │ How: Exponential branching over out-degree vs stationary distribution thresholding.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_multiple

class GraphNeighborhoodExpansion:
    """Graph neighborhood expansion vs. context budget."""
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    v_bfs = int(round(v0 * (d_out**d_hop)))
    t_required = v_bfs * tokens_per_fn
    overflow_mult = t_required / s_max

    t_ppr = v_ppr * tokens_per_fn
    t_remaining = s_max - t_ppr

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(v_bfs == 1311, "v_bfs")
    check(t_required == 235_980, "t_required")
    check(abs(overflow_mult - 28.8) < 0.1, "overflow_mult")
    check(t_ppr == 6_840, "t_ppr")
    check(t_remaining == 1_352, "t_remaining")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    t_max_str = fmt_int(t_max)
    s_max_str = fmt_int(s_max)
    tokens_per_fn_str = fmt_int(tokens_per_fn)
    v0_str = fmt_int(v0)
    d_hop_str = fmt_int(d_hop)
    v_bfs_str = fmt_int(1310)
    overflow_mult_str = fmt_multiple(overflow_mult, precision=1)
    v_ppr_str = fmt_int(v_ppr)
    t_remaining_str = fmt_int(t_remaining)
```

::: {#nbk-06-graph-neighborhood-expansion-vs-context .callout-notebook title="Graph neighborhood expansion vs. context budget"}
...
The agent's working memory has a total capacity of $T_{\max} =$ `{python} GraphNeighborhoodExpansion.t_max_str` tokens, with an upper bound of $S_{\max} =$ `{python} GraphNeighborhoodExpansion.s_max_str` tokens allocated to staged persistent evidence. Each function's serialized representation (declaration, docstring, and signature) averages `{python} GraphNeighborhoodExpansion.tokens_per_fn_str` tokens.

1. **Unconstrained Bounded BFS Expansion:**
   Suppose the seed set size is $|V_0| =$ `{python} GraphNeighborhoodExpansion.v0_str`. Across this codebase, the mean effective out-degree for control-flow edges is $\bar{d}_{\text{out}} = 6.4$.
   An unconstrained Breadth-First Search to a hop depth of $d_{\text{hop}} =$ `{python} GraphNeighborhoodExpansion.d_hop_str` traverses a candidate set of size:
   $$|V_{\text{BFS}}| \approx |V_0| \cdot (\bar{d}_{\text{out}})^{d_{\text{hop}}} = 5 \cdot (6.4)^3 = 5 \cdot 262.14 = 1,310 \text{ functions}$$
   Serializing `{python} GraphNeighborhoodExpansion.v_bfs_str` functions into the agent's context requires:
   $$T_{\text{required}} = 1,310 \times 180 = 235,800 \text{ tokens}$$
   This exceeds the allocated context budget $S_{\max} =$ `{python} GraphNeighborhoodExpansion.s_max_str` by an overflow factor of `{python} GraphNeighborhoodExpansion.overflow_mult_str`:
...
2. **Personalized PageRank Pruning:**
   ...
   Applying a probability cutoff threshold of $\epsilon_{\text{PPR}} = 1.2 \times 10^{-3}$ isolates an induced subgraph of exactly $|V_{\text{PPR}}| =$ `{python} GraphNeighborhoodExpansion.v_ppr_str` functions.
   Serializing these `{python} GraphNeighborhoodExpansion.v_ppr_str` prioritized functions into context consumes:
   $$T_{\text{PPR}} = 38 \times 180 = 6,840 \text{ tokens}$$
   This leaves:
   $$S_{\max} - T_{\text{PPR}} = 8,192 - 6,840 = 1,352 \text{ tokens}$$
   The remaining `{python} GraphNeighborhoodExpansion.t_remaining_str` tokens are reserved for the directed edge connectivity summary (e.g., caller-callee pairings), fitting comfortably inside $S_{\max}$ while capturing the end-to-end execution path across five distinct files.
:::
...
::: {#fig-vol3-cpg-ingestion fig-env="figure" fig-pos="htb" fig-cap="**Tiered Code Property Graph Ingestion Pipeline**: Ingestion architecture balancing static analysis depth against interactive update latency. Left panel illustrates the sequential extraction pipeline from raw source files to embedded property graph storage. Right panel demonstrates the dual-path tiered refresh architecture combining foreground incremental Tree-sitter parsing (~2 ms/file) with background compiler-grade Clang/SCIP indexing to maintain consistent point-in-time snapshot views." fig-alt="Systems pipeline diagram showing source code parsed by Tree-sitter and linked by SCIP into SQLite LMDB graph storage on the left, and a dual-path engine merging foreground incremental AST deltas with background global compiler graphs on the right."}
![](images/svg/cpg_ingestion_pipeline.svg)
:::

As detailed in @fig-vol3-cpg-ingestion, the ingestion subsystem decouples graph extraction into two synchronized operational paths. In Stage 1 (left), raw repository code flows through incremental Tree-sitter parsing to emit local ASTs and symbol scopes within $\sim 2\text{ ms}$ per file. A cross-reference linker (SCIP/LSIF) resolves module imports and type signatures to construct directed `CALLS` and `READS` edges, persisting the graph into embedded storage (SQLite CTEs or LMDB adjacency lists) at an expansion factor of $\beta \approx 8\times \text{ to } 25\times$. In Stage 2 (right), the runtime balances latency against precision via a dual-path engine. When an agent edits a file in the interactive loop, a foreground fast path executes an incremental Tree-sitter parse to produce an in-memory delta overlay $\Delta G_{\text{local}} = (V_\Delta, E_\Delta)$ in milliseconds. Meanwhile, an asynchronous background daemon runs compiler-grade semantic extraction (Clang/rustc) over the whole codebase to generate exact global type and vtable indices. At query time, a unified graph merger reconciles point-in-time queries via Copy-on-Write overlays ($G_{\text{view}}(t) = (G_{\text{global}} \setminus \text{Tombstones}) \cup \Delta G_{\text{local}}$), shielding the agent from expensive compiler re-indexing pauses without compromising structural accuracy.
...
::: {#chk-06-hybrid-retrieval .callout-checkpoint title="Evaluating hybrid and multi-hop retrieval"}
Before examining persistent storage invalidation protocols, verify your understanding of hybrid index topologies:

- [ ] **Lexical vs dense trade-offs**: Why do dense semantic embeddings fail on exact identifier queries where BM25 inverted indexes succeed?
- [ ] **Reciprocal rank fusion**: How does combining lexical rankings and vector cosine similarities prevent vocabulary mismatch while retaining semantic recall?
- [ ] **Code property graphs**: What structural dependencies do AST and call-graph edges capture that text chunking irrevocably destroys?
- [ ] **Ingestion cost barrier**: Why must incremental graph updates be prioritized over full-repository re-indexing during active agent editing loops?
:::
...
In classical database management systems, materialized views are maintained through differential algebraic operators $\Delta \mathcal{V} = f(\mathcal{D} \cup \Delta \mathcal{D}) - f(\mathcal{D})$, as established in the foundational view maintenance literature by @blakeley1986efficiently and generalized by @gupta1995maintenance. If a relational tuple is inserted or updated, the database engine computes the exact delta of downstream aggregate projections using incremental relational calculus. In neural agent storage, however, the transformation functions $f_{\text{dense}}$ and $f_{\text{graph}}$ are structurally asymmetric and computationally non-linear.
...
::: {#fig-vol3-index-invalidation fig-env="figure" fig-pos="htb" fig-cap="**Persistent Storage Invalidation Protocol Timeline**: Comparative timeline of storage consistency protocols under rapid file system mutations. Synchronous re-indexing incurs severe execution pipeline stalls ($\Delta t \approx 450\text{ ms}$). Asynchronous event-driven updates eliminate stalls but open an uncontained staleness window ($\Delta t \approx 1.2\text{ s}$) where agents retrieve stale snapshots. Read-time tombstoning resolves both pathologies by maintaining lightweight in-memory invalidation bitsets against the Write-Ahead Log, guaranteeing point-in-time consistency with zero execution stall." fig-alt="Comparative timeline diagram illustrating three storage invalidation protocols: Synchronous Re-indexing causing pipeline stalls, Asynchronous Re-indexing opening a staleness window, and Read-Time Tombstoning delivering zero-overhead point-in-time consistency."}
![](images/svg/episodic_wal_flow.svg)
:::

The timeline dynamics of these invalidation protocols are compared directly in @fig-vol3-index-invalidation. Following a workspace write mutation $w(D_j)$ at time $t_w$, the three protocols handle the consistency boundary through distinct systems mechanisms:

1. **Synchronous Re-indexing (Top):** The host runtime completely blocks the agent decode loop while executing AST parsing, lexical posting updates, and GPU neural embedding passes. While this guarantees zero staleness ($\Delta t = 0$), the pipeline stall (often $200\text{--}800\text{ ms}$) degrades interactive responsiveness.
2. **Asynchronous Event-Driven (Middle):** The host returns immediately to the agent, deferring indexing to a background queue. However, this creates a dangerous unfenced staleness window $\Delta t = t_{\text{sync}} - t_w$. If the agent queries storage within this window, it retrieves obsolete pre-mutation candidates, triggering reasoning faults.
3. **Read-Time Tombstoning (Bottom):** The write operation instantly records an in-memory generation tag $g(D_j) \leftarrow g + 1$ and appends the old chunk IDs to a DRAM tombstone bitset in sub-millisecond time ($<1\text{ ms}$). When subsequent queries execute, the retrieval engine filters candidates against the tombstone mask at query time, guaranteeing fresh results without stalling the agent or waiting for background re-indexing.
...
::: {#psp-06-the-fallacy-of-index-authoritativeness .callout-perspective title="The fallacy of index authoritativeness and derived storage"}
A pervasive architectural error in agentic systems design is treating the retrieval substrate—vector databases, BM25 inverted indexes, or code property graphs—as an authoritative source of truth. In any environment where agents possess mutation privileges, secondary indexes are merely lossy, asynchronously materialized projections of the physical world.

When an agent executes an environment mutation, such as patching a source file or rolling back a transaction, the underlying filesystem updates synchronously. However, the vector embeddings and lexical postings representing that file are derived artifacts generated through computationally expensive tokenization, AST chunking, and neural forward passes. If the runtime allows an agent to query this secondary index without strict read-time invalidation barriers, the model retrieves an obsolete reflection of its own pre-mutation state. Believing its prior edit failed, the agent enters an oscillating repair loop—emitting redundant patches that corrupt working tree invariants.

**Systems insight**: Ground-truth authority resides strictly in the physical execution environment (the filesystem, process table, and git tree), never in secondary retrieval indexes. Dependable architectures treat vector stores and knowledge graphs as unprivileged search heuristics: indexes propose candidate file offsets, but the host supervisor must unconditionally revalidate candidates against physical disk state before staging evidence into the active prompt.
:::
...
The resolution is the discipline that @sec-vol3-context-engineering-context-rot applied to the prompt, now carried to the index (principle \ref{pri-vol3-source-authority}), namely tombstoning paired with read-time generation filtering. This architecture decouples the *invalidation* of obsolete data from the *re-indexing* of new content. When a document $D_j$ is modified, the host runtime immediately writes the current monotonic timestamp to an in-memory tombstone table:
...
```{python}
#| label: lego-06-latency-and-throughput-cost-of
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ LATENCY AND THROUGHPUT COST OF INDEX SYNCHRONIZATION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-06-latency-and-throughput-cost-of
# │ Goal: Compare blocking write latency of synchronous index updates vs in-memory tombstoning.
# │ Show: Tombstoning slashes write latency by 5 orders of magnitude (88 ms to 0.0004 ms) with 0-staleness.
# │ How: Sum CPU AST parse, lexical postings, and GPU batch embedding vs hash map timestamp insertion.
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim import *
from mlsysim.core.units import *
...
class IndexSyncLatency:
    """Latency and throughput cost of index synchronization."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    m_files = 8
    chunks_per_file = 2
    t_ast = 3.5 * millisecond
    t_lex = 2.0 * millisecond
    t_batch = 28.0 * millisecond
    batch_size = 16
    t_rpc = 150 * millisecond

    t_hash_write = 50 * nanosecond
    k_cand = 50
    t_hash_read = 30 * nanosecond

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    m_chunks = m_files * chunks_per_file
    n_batches = int(math.ceil(m_chunks / batch_size))
    t_sync_local = (m_files * t_ast + m_chunks * t_lex + n_batches * t_batch).to(millisecond)
    t_sync_remote = (m_files * t_ast + m_chunks * t_lex + t_rpc).to(millisecond)
    t_sync_serial = (m_files * t_rpc).to(second)

    t_write_stall = (m_files * t_hash_write).to(millisecond)
    t_read_overhead = (k_cand * t_hash_read).to(millisecond)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(m_chunks == 16, "m_chunks")
    check(abs(t_sync_local.magnitude - 88.0) < 0.1, "t_sync_local")
    check(abs(t_sync_remote.magnitude - 210.0) < 0.1, "t_sync_remote")
    check(abs(t_sync_serial.magnitude - 1.20) < 0.01, "t_sync_serial")
    check(abs(t_write_stall.magnitude - 0.0004) < 1e-5, "t_write_stall")
    check(abs(t_read_overhead.magnitude - 0.0015) < 1e-5, "t_read_overhead")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    m_files_str = fmt_int(m_files)
    m_chunks_str = fmt_int(m_chunks)
    t_sync_local_ms_str = fmt_time(t_sync_local, unit=millisecond, precision=0)
    t_sync_remote_ms_str = fmt_time(t_sync_remote, unit=millisecond, precision=0)
    t_sync_serial_s_str = fmt_time(t_sync_serial, unit=second, precision=2)
    t_write_stall_ms_str = fmt_time(t_write_stall, unit=millisecond, precision=4)
    k_cand_str = fmt_int(k_cand)
    t_read_overhead_ms_str = fmt_time(t_read_overhead, unit=millisecond, precision=4)
```

::: {#nbk-06-latency-and-throughput-cost-of .callout-notebook title="Latency and throughput cost of index synchronization"}
...
**The Workload:** The agent executes an automated refactoring tool that modifies $M =$ `{python} IndexSyncLatency.m_files_str` files simultaneously. Each file has changes affecting 2 chunks, generating a total of $M_{\text{chunks}} =$ `{python} IndexSyncLatency.m_chunks_str` dirty chunks.
...
If embedding inference relies on a remote cloud API subject to network round-trip times ($t_{\text{rpc}} \approx 150\text{ ms}$ per request), this blocking penalty spikes to $28\text{ ms} + 32\text{ ms} + 150\text{ ms} =$ `{python} IndexSyncLatency.t_sync_remote_ms_str`. If calls are serialized per file without client-side batching, latency reaches $8 \times 150\text{ ms} =$ `{python} IndexSyncLatency.t_sync_serial_s_str`, visibly stalling the decode loop.

**Case B: Tombstoning with Read-Time Filtering**

1. *Write Path Overhead:* The tool updates an in-memory hash table mapping the `{python} IndexSyncLatency.m_files_str` file identifiers to current nanosecond timestamps. At 50 ns per insertion, the total write stall is $8 \times 50\text{ ns} = 400\text{ ns} =$ `{python} IndexSyncLatency.t_write_stall_ms_str`. Control returns to the agent instantly.
2. *Read Path Overhead:* A subsequent query generates $K_{\text{cand}} =$ `{python} IndexSyncLatency.k_cand_str` candidates via HNSW graph traversal. The read filter performs 50 hash lookups against the tombstone registry. At 30 ns per lookup, the verification overhead is $50 \times 30\text{ ns} = 1.5\ \mu\text{s} =$ `{python} IndexSyncLatency.t_read_overhead_ms_str`.

Tombstoning reduces write-path latency by five orders of magnitude (`{python} IndexSyncLatency.t_sync_local_ms_str` $\to$ `{python} IndexSyncLatency.t_write_stall_ms_str`) while maintaining mathematical zero-staleness ($\Delta t = 0$), completely shielding the model from pre-mutation content without stalling execution.
:::
...
Index invalidation closes one storage property mechanically, since the retrieval engine never serves a superseded chunk. Under the invariant closure principle (\ref{pri-invariant-closure}) it cannot certify the task, whose evidence still comes from the end-to-end boundary [@saltzer1984endtoend], where checks confirm that the agent's changes satisfy the environment's own invariants. What the storage layer owes that boundary is an honest record of the source. Whenever the agent mutates a file, the runtime captures an authoritative content hash and file modification timestamp directly from the OS kernel before executing any subsequent retrieval or deliberation step. Durable memory must remain subservient to the physical state of the machine.
...
A persistent storage subsystem is an unprivileged channel through which external, potentially untrusted data enters an agent's active reasoning context. When an autonomous agent queries an index over a large software repository, private enterprise store, or external documentation corpus, the retrieved text bypasses traditional compiler validation and enters the autoregressive decode loop as prompt tokens. If the storage plane lacks explicit security boundaries and multi-tenant access control, even an agent whose tools only read can be coerced into exfiltrating confidential state or executing unvetted tool calls via indirect prompt injection embedded within indexed files.
...
::: {#fig-vol3-retrieval-security fig-env="figure" fig-pos="htb" fig-cap="**Retrieval Security and Access Governance Architecture**: Privileged host supervisor interposing between the unprivileged planning engine, which holds zero ambient authority, and derivative search stores. The unprivileged model emits a structured query tuple $\\mathcal{Q}$. The host supervisor's ACL filter applies tenant tokens to generate a pre-filtering bitset mask $B$, constraining index traversals across BM25 postings and HNSW graphs. Raw candidate passages are scrubbed for secrets, credentials, and prompt injection attempts before being encapsulated in cryptographically nonced data frames for context staging." fig-alt="Security architecture diagram illustrating the privileged host supervisor boundary enclosing the unprivileged planning engine, an ACL bitset filter gate, derivative search stores, and an ingestion sanitizer with cryptographic nonce framing."}
![](images/svg/retrieval_security_governance.svg)
:::

To enforce these protections, production agent runtimes deploy a formal security architecture centered around a privileged host supervisor (@fig-vol3-retrieval-security). Under this model, the foundation model operates as an unprivileged planning engine with zero ambient authority, possessing no direct network or disk I/O capabilities. When the agent initiates an information request, it emits a structured query proposal $\mathcal{Q}$ to the host supervisor. The supervisor passes the proposal through a Principal Token & ACL Filter Gate, verifying active tenant credentials and compiling an in-memory bitset mask $B = \{\text{doc\_id} : U \in \text{ACL}(\text{doc\_id})\}$. The mask is applied directly during index traversal across BM25 postings and HNSW graph links, guaranteeing that unauthorized candidates are never evaluated. Candidates emitted by the search stores ($\mathcal{R}_k$) are routed through an Ingestion Sanitizer and Provenance Scrub gate, which scans for high-entropy secrets and credentials, neutralizes indirect prompt injection attempts, verifies content freshness against authoritative hashes, and encapsulates the text into non-executable syntax fences (`<data-frame nonce="...">`) prior to context window staging.
...
::: {#fig-vol3-access-prefilter fig-env="figure" fig-pos="htb" fig-cap="**Access-Controlled Retrieval: Bitset Pre-Filtering vs Post-Filtering**: Multi-tenant access control architectures for persistent indexes. Under naive post-filtering (right), unconstrained top-$M$ search across the global index followed by ACL pruning risks complete recall collapse when unauthorized items dominate the candidate set. Under bitset pre-filtering (left), the principal's permission bitmask $B$ is applied directly during index traversal—intersecting BM25 postings lists or constraining HNSW graph edge transitions—guaranteeing that every retrieved item is strictly authorized without latency penalties." fig-alt="Architectural comparison diagram showing secure in-traversal bitset pre-filtering protocol on the left, and post-filtering pathologies including recall collapse and tail latency spikes on the right."}
...
@Tbl-vol3-retrieval-comparison details the empirical trade-offs across distinct persistent storage architectures evaluated on a representative 10-million-token codebase ($N = 50{,}000$ files, $40\text{ MB}$ raw UTF-8 text).
...
```{python}
#| label: lego-06-quantitative-trade-offs-in-storage-amplification
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ QUANTITATIVE TRADE-OFFS IN STORAGE AMPLIFICATION AND INDEXING THROUGHPUT (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-06-quantitative-trade-offs-in-storage-amplification
# │ Goal: Dimension storage amplification and incremental re-indexing latency across architectures.
# │ Show: Arch A (BM25+AST) expands storage 1.36x and re-indexes in 34 ms vs Arch B 10.44x and 672 ms.
# │ How: Calculate postings and graph edges vs high-dimensional vectors and HNSW link pointers.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time, fmt_multiple

class StorageAmplificationTradeoffs:
    """Quantitative trade-offs in storage amplification and indexing throughput."""
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    t_reindex_a = (n_files_mod / rate_bm25 + n_files_mod / rate_ast) * second
    t_reindex_b = (n_files_mod / rate_bm25 + chunks_mod / rate_emb) * second

    t_reindex_a_ms = round(t_reindex_a.to(millisecond).magnitude) * millisecond
    t_reindex_b_ms = round(t_reindex_b.to(millisecond).magnitude) * millisecond

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_reindex_a.to(millisecond).magnitude - 34.06) < 0.5, "t_reindex_a")
    check(abs(t_reindex_b.to(millisecond).magnitude - 672.0) < 0.5, "t_reindex_b")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_files_mod_str = fmt_int(n_files_mod)
    t_reindex_a_ms_str = fmt_time(t_reindex_a_ms, unit=millisecond, precision=0)
    t_reindex_b_ms_str = fmt_time(t_reindex_b_ms, unit=millisecond, precision=0)
    alpha_a_mult_str = fmt_multiple(alpha_a, precision=2)
    alpha_b_mult_str = fmt_multiple(alpha_b, precision=2)
```

::: {#nbk-06-quantitative-trade-offs-in-storage-amplification .callout-notebook title="Quantitative trade-offs in storage amplification and indexing throughput"}
...
For Architecture A, yielding a storage amplification factor of `{python} StorageAmplificationTradeoffs.alpha_a_mult_str`:
...
For Architecture B, yielding a storage amplification factor of `{python} StorageAmplificationTradeoffs.alpha_b_mult_str`:
...
Architecture A completes re-indexing in `{python} StorageAmplificationTradeoffs.t_reindex_a_ms_str` (imperceptible within an interactive loop), while Architecture B imposes a `{python} StorageAmplificationTradeoffs.t_reindex_b_ms_str` synchronization delay before the subsequent reasoning step can safely dispatch a query against the modified files.
:::
...
::: {#fig-vol3-distractor-dilemma fig-env="figure" fig-pos="htb" fig-cap="**The Distractor Dilemma in Retrieval-Augmented Deliberation**: Empirical relationship between retrieval depth $k$ and downstream end-to-end task success rate ($\\text{SR}$). In the under-retrieval regime ($k < k^*$), task success is limited by missing dependency and signature evidence. Beyond an optimal operating point ($k^* \\in [3, 5]$), success declines precipitously as semantically similar but structurally irrelevant distractors dilute attention mass and induce positional hallucination." fig-alt="Quantitative curve showing task success rate peaking at 72% at k*=5 before dropping to 10% at k=100, contrasted with monotonically increasing recall at k curve across under-retrieval, optimal window, and attention dilution regimes."}
![](images/svg/distractor_dilemma_success_curve.svg)
:::

The relationship between retrieval depth $k$ and downstream End-to-End Task Success Rate ($\text{SR}$) is strictly non-monotonic (@fig-vol3-distractor-dilemma). The quantitative dynamics span three distinct operating regimes:

1. **Under-Retrieval Regime ($k < k^*$):** For low values of $k$ ($0 \le k < 3$), task success is constrained by missing information. The agent lacks essential variable definitions, type signatures, or interface contracts, capping task completion at low rates despite high retrieval precision.
2. **Optimal Operating Window ($k^* \in [3, 5]$):** When retrieval provides a compact set of high-assurance evidence, task success peaks sharply at $72\%$. The model receives necessary causal premises without overwhelming its attention capacity.
3. **Attention Dilution & Distractor Interference ($k > 10$):** As $k$ expands toward $20$, $50$, and $100$ passages, raw retrieval Recall@$k$ increases monotonically toward saturation ($>75\%$). Paradoxically, however, end-to-end task success plummets from $72\%$ down to below $10\%$. Staging excess passages dilutes self-attention mass across irrelevant distractor keys, triggering hallucinations and causing the model to condition on superseded documentation or test mocks. This phenomenon is known as the *distractor dilemma*.

The distractor dilemma is the working-set frontier of @sec-vol3-context-engineering-physics seen from the read path (principle \ref{pri-vol3-attention-working-set}). Retrieval depth $k$ plays the role that staged length played there. Each semantically similar but structurally irrelevant chunk adds keys to the softmax denominator and dilutes the attention mass on the true invariant or failing assertion, and evidence staged in the middle of a 50-chunk block also suffers the positional bias measured there.
...
Treating persistent external stores as a tier below the KV cache, the way an NVMe backing store serves as swap space for RAM pages, misreads what the cache is. @Sec-vol3-kv-cache-fallacies showed that the KV cache is not a semantic memory tier. It is derived from the exact token prefix under one model checkpoint (principle \ref{pri-vol3-prefix-coherence}), so a new checkpoint, a new quantization format, or a single changed token upstream invalidates it. Durable storage therefore cannot hold attention state. It must be an external, model-agnostic subsystem whose records the runtime retrieves through explicit reads and stages into working context as text or structured tokens, where the serving engine rebuilds attention state from them.
:::
...
The chapter reduces to one invariant, that *durable memory is an untrusted external storage subsystem whose integrity, authorization, and consistency the host runtime must govern.* Durable storage is the last of the three places governed by the principle that authority stays with the source (principle \ref{pri-vol3-source-authority}). Indexes are copies of the repository in the same sense that a staged observation is, and read-time tombstones with generation epochs are the index form of the invalidation that @sec-vol3-context-engineering-context-rot applied to the prompt. The retrieval contract (principle \ref{pri-vol3-retrieval-contract}) is what lets the runtime act on that fact, because a candidate enters context only after its version, scope, and provenance have been checked against its source.

::: {.callout-chapter-connection title="From stored evidence to external effects"}
Over @sec-vol3-context-engineering, @sec-vol3-kv-cache, and this chapter, we established the complete memory hierarchy of an agentic computer system: logical working-set selection and prompt compaction in @sec-vol3-context-engineering; physical key-value cache virtualization, paged allocation, and prefix sharing on accelerator hardware in @sec-vol3-kv-cache; and durable external storage, hybrid retrieval, and index invalidation in this chapter.
...
Completing an autonomous task requires bridging the boundary between unprivileged token generation and deterministic physical execution. This transition marks the boundary into **Part III: Execution Sandboxing**, which answers the Containment Hazard of @sec-vol3-intro-causal-spine. In @sec-vol3-tool-calling (*Tool Execution*), we develop the typed action and observation interfaces—structured function schemas, the Model Context Protocol (MCP), tool call validation, and bidirectional streaming I/O buses—that allow an unprivileged foundation model to safely drive external software tools and observe real-world environments.
:::
