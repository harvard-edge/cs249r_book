...
::: {.column-margin}
**Epistemic gap.** @sec-vol3-context-engineering-context-rot defined the gap between the environment's authoritative state and the stale copy of it in the prompt. In durable storage the stale copy can also be an index entry or a note.
:::
...
::: {.column-margin}
**Boundary framing.** Delimiters such as `<retrieved_evidence>` use per-session nonces, and any occurrence of the delimiter inside the payload is escaped, so retrieved text cannot close its own frame.
:::

Framing wraps each chunk in explicit delimiters that carry its provenance, so the model sees where the text came from and where it ends:
...
Framing lowers the probability that the model follows an embedded instruction; it does not prevent it. The runtime's guarantees are the mechanical ones: the scope of the query, the version check, the token ceilings, and, most important, the fact that retrieved text carries no authority. Whatever an injected instruction persuades the model to propose still passes the authorization checks of Part III before it can act, and @sec-vol3-sandboxes-threat-model develops the threat model and the defenses below the model.
...
### Generation epochs and compaction
...
::: {.fallacy-pitfall}
**Fallacy:** *Persistent memory is a lower tier of the model's KV cache.*

Treating durable storage as a tier below the KV cache, the way swap space backs main memory, misreads what the cache is. @Sec-vol3-kv-cache-fallacies showed that attention state is derived from an exact token prefix under one model version (principle \ref{pri-vol3-prefix-coherence}), so a new model, a new numeric format, or a single changed token upstream invalidates it. Durable storage therefore cannot hold attention state. It holds records that the runtime retrieves and stages as tokens, from which the serving engine rebuilds attention state on every call.
:::
