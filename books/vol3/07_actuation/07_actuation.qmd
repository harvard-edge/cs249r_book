...
An agent asked to restart a container on a staging server emits a short tool call. The host runtime checks the call against the tool's schema, confirms that this session may restart that container, and sends `POST /containers/srv-stage-04/restart`. Two seconds later the socket times out. The container may never have received the request, may have crashed partway through restarting, or may have restarted cleanly and lost only its reply. A retry is harmless in the first case and restarts healthy workers in the third, and nothing in the timeout says which case occurred.
...
Every step in that sequence happens at the tool boundary, and none of them can happen inside the model. The model holds zero ambient authority (@sec-vol3-intro-operational-incident), so the runtime performs each step, and the six-phase lifecycle (@sec-vol3-intro-execution-lifecycle) fixes where the steps sit in a turn, from action authorization through runtime dispatch to evidence capture. In the terms of @sec-vol3-intro-hsa, this boundary is where the runtime closes the authority exposure, because every grant a task holds, from read access to irreversible action, is checked here before anything runs. This chapter supplies what that lifecycle left abstract once a proposal names a real tool on a real endpoint. It gives every tool a typed descriptor, keeps authorization in the host so that no tool server can grant itself access by advertising, settles each dispatch before any retry, and returns every observation bounded and typed however much the tool prints.

The runtime's task has a well-known precedent in Dennis Ritchie and Ken Thompson's UNIX time-sharing system (1974), which unified heterogeneous physical hardware under a single, polymorphic operating system abstraction. By declaring that "everything is a file," UNIX decoupled user-space applications from the idiosyncratic register maps and hardware timing constraints of disk platters, serial terminals, and magnetic tapes. Applications interact exclusively with abstract integer handles, called file descriptors, which expose a uniform operational contract (`open`, `read`, `write`, `close`, `ioctl`). The operating system kernel maintains a private file descriptor table per process, validates caller permissions, and routes uniform requests to specialized hardware device drivers.

In an agentic architecture, we observe a modern design precedent: "everything is an RPC tool dispatch." Heterogeneous external resources, such as compilers, database engines, web search endpoints, local filesystems, and remote microservices, are abstracted into a uniform catalog of typed tool descriptors managed by the agent runtime. However, we must not mistake this design precedent for a literal hardware equivalence. An operating system process executes native machine instructions with direct memory access and traps to ring 0 with hardware-enforced CPU privilege rings. An autoregressive language model, by contrast, merely emits a candidate sequence of tokens into a host-managed memory buffer. It is the host runtime that operates as the supervisory kernel and device driver, interpreting candidate token strings, validating their structural integrity, translating them into concrete remote procedure calls, and returning structured observations back across the memory boundary.

A runtime that skipped this interpretation and validation and piped generated strings into an `eval()` interpreter or a POSIX shell would inherit both the lexical faults of free text (unescaped shell metacharacters, malformed argument lists, hallucinated command-line flags) and the open-loop reliability ceiling of @sec-vol3-intro-temporal-stretching. The runtime instead enforces *complete mediation*, the requirement Saltzer and Schroeder stated in 1975 that every access to every object be checked for authority [@saltzer1975]. At the tool boundary, complete mediation refines phases 3 through 5 of the lifecycle into four stages.

First, during *proposal validation against schema*, the host runtime intercepts the candidate token stream emitted by the model, parses the serialized payload across the memory boundary, and rigorously validates the proposed arguments against a pre-registered interface schema. Syntactic malformations, missing required fields, and out-of-bounds types are rejected at the memory boundary before any network socket is opened or external process spawned. Second, during *permission and capability gating*, the runtime acts as an authoritative reference monitor for a model that holds zero ambient authority. Even if a model proposes a syntactically flawless command, the runtime verifies whether the active agent session holds explicit capability grants for the targeted operation, enforcing read-only constraints, path sandboxing policies, and rate limits. Third, during *dispatched execution*, the validated and authorized operation is delegated to an isolated external driver or RPC endpoint. The runtime manages network timeouts, monitors execution latencies, and tracks process handles without stalling the primary supervisory loop. Fourth, during *observation normalization and context staging*, the runtime captures the raw, heterogeneous telemetry produced by the external environment, such as standard output streams, exit codes, diagnostic traces, or structured JSON responses. It sanitizes control characters, enforces backpressure and strict token truncation boundaries to prevent accelerator context exhaustion, and wraps the result in an authoritative observation envelope that is staged back into the model's working memory context. @Sec-vol3-actuation-schemas and @sec-vol3-actuation-mcp develop the first two stages, @sec-vol3-actuation-idempotency and @sec-vol3-actuation-async the third, and @sec-vol3-actuation-streaming and @sec-vol3-actuation-observation-parsing the fourth.

::: {#dfn-complete-mediation .callout-definition title="Complete mediation"}

***Complete mediation***\index{Complete mediation!definition}\index{Tool actuation!complete mediation definition} is an architectural security invariant requiring that every candidate token emission proposing environment mutation traverse an unbypassable supervisory verification pipeline prior to physical execution.

1.  **Significance**: Guarantees that unprivileged stochastic model emissions cannot bypass capability checks, tamper with host processes, or execute unvetted side effects, so that the model's zero ambient authority holds at every environment boundary.
2.  **Distinction**: Unlike classical operating system syscall mediation (which checks static UID/GID bits across synchronous context switches), agentic complete mediation verifies dynamic schema conformity, cryptographic capability leases, and idempotency replay tokens before dispatching to isolated execution sandboxes.
3.  **Common pitfall**: Relying on prompt steering ("You must not execute dangerous shell commands") or unmediated wrapper scripts, which allows prompt injection or tool hallucination to compromise host resources directly.

:::

The complete mediation architecture governing tool actuation is systematized across protection rings in @fig-vol3-ai-system-call-boundary. In Ring 3 (User Space), the unprivileged foundation model, which holds zero ambient authority, generates candidate tool tokens that are held in an escrow register rather than executing directly against host operating system interfaces. When a complete tool invocation sequence is emitted, the register asserts an AI system call software trap (`SYS_TOOL_CALL`), yielding control across the privilege boundary into Ring 0 (Supervisor Space). The host supervisor routes the candidate payload through four sequential verification gates: (1) a Grammar DFA Filter that enforces schema-constrained logit masking ($P(\text{syntax\_err}) = 0$, @sec-vol3-processor-grammar-constrained); (2) a Strict ABI Deserializer that hydrates typed structs while validating memory bounds; (3) a Capability and Lease Checker that validates cryptographic HMAC tokens and path allowlists to defeat directory traversal exploits; and (4) an Idempotency and Replay Engine that inspects the trajectory nonce $k_{\text{idem}}$ against write-ahead storage. These gates are finer steps within the first three stages above, not a second lifecycle. Only after passing all four gates does the supervisor dispatch authorized calls into isolated execution domains (such as Firecracker microVMs, gVisor containers, or enterprise API gateways). Once physical actuation concludes, the resulting observation stream and exit code return upward via a `SYSRET` transition, resuming the unprivileged model with an authoritative observation envelope staged in working memory. In parallel, @tbl-vol3-tool-descriptors systematizes this structural duality, comparing the classic UNIX file descriptor architecture with the agentic tool descriptor table.

::: {#fig-vol3-ai-system-call-boundary fig-env="figure" fig-pos="htb" fig-cap="**The AI System Call Privilege Boundary**: Architectural topology of mediated tool execution across protection rings. The unprivileged model, which holds zero ambient authority, operates in user space (Ring 3), emitting candidate tool calls held in memory escrow before yielding control via a `SYS_TOOL_CALL` trap. The privileged host supervisor enforces complete mediation in kernel space (Ring 0), validating payloads through four sequential verification gates (Grammar DFA Filter, Strict ABI Deserializer, Capability Lease Checker, and Idempotency Replay Engine) before dispatching authorized operations to isolated execution domains and returning sanitized observations via `SYSRET`." fig-alt="Systems architecture diagram illustrating Ring 3 User Space with an unprivileged foundation model core and escrow register, transitioning across a SYS_TOOL_CALL trap and SYSRET resume boundary to Ring 0 Supervisor Space with four sequential verification stages, which dispatch across an isolation barrier to sandboxed execution domains."}
...
| **Authority Model**      | POSIX permissions (`rwxrwxrwx`), process UID/GID, and OS capability sets.        | Explicit capability tokens, declarative access control policies, and the model's zero ambient authority. |
...
The structural boundary policy $\chi$ determines whether the runtime permits extraneous fields. Under an open policy ($\chi = \text{open}$), the model may emit arbitrary auxiliary key-value pairs without triggering validation errors; under a closed policy ($\chi = \text{closed}$, declared via `additionalProperties: false` in JSON Schema), any key outside $P$ causes immediate validation rejection. The typed action contract (principle \ref{pri-vol3-strict-action-abi}) validates every proposal against a closed schema, because an open policy lets the model pass uninspected arguments that could silently alter execution semantics or bypass audit logs.
...
```{python}
#| label: lego-07-memory-footprint-and-prefill-latency
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ MEMORY FOOTPRINT AND PREFILL LATENCY OF STATIC TOOL CATALOGS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-07-memory-footprint-and-prefill-latency
# │ Goal: Dimension memory footprint and prefill latency of static tool catalogs.
# │ Show: 15,000 tool tokens consume 4.915 GB KV cache per session (78.64 GB across 16 sessions) and incur 669 ms prefill latency.
# │ How: GQA KV cache formula with L=80, H_kv=8, D_h=128; prefill FLOPs derived from parameter count and effective MFU.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time, fmt_memory, fmt_memory_capacity

class StaticToolCatalogOverhead:
    """Memory footprint and prefill latency of static tool catalogs."""
    ...
    k_sessions = 16
    hbm_per_gpu = 80 * GB
    peak_flops_per_gpu = 989.5 * (TFLOP / second)
    mfu_prefill = 0.40

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    mem_per_token = 2 * b_elem * n_layers * h_kv * d_h
    kv_session = (t_catalog * mem_per_token).to(GB)
    kv_total = (k_sessions * kv_session).to(GB)

    hbm_total = (n_gpus * hbm_per_gpu).to(GB)
    weights_mem = (p_model * b_elem).to(GB)

    flops_prefill = 2 * p_model * t_catalog * flop
    peak_throughput = (n_gpus * peak_flops_per_gpu).to(PFLOP / second)
    throughput_effective = peak_throughput * mfu_prefill
    t_prefill = (flops_prefill / throughput_effective).to(millisecond)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(mem_per_token == 327_680 * byte, "mem_per_token")
    check(abs(kv_session.magnitude - 4.9152) < 0.01, "kv_session")
    check(abs(kv_total.magnitude - 78.6432) < 0.05, "kv_total")
    check(abs(weights_mem.magnitude - 141.2) < 0.1, "weights_mem")
    check(abs(t_prefill.to(millisecond).magnitude - 669.0) < 1.0, "t_prefill")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    m_tools_str = fmt_int(m_tools)
    t_catalog_str = fmt_int(t_catalog)
    kv_session_gb_str = fmt_memory(kv_session, unit=GB, precision=3)
    k_sessions_str = fmt_int(k_sessions)
    kv_total_gb_str = fmt_memory(kv_total, unit=GB, precision=2)
    weights_mem_gb_str = fmt_memory_capacity(weights_mem, unit=GB, precision=1)
    dormant_kv_approx_gb_str = fmt_memory_capacity(80 * GB, unit=GB, precision=0)
    t_prefill_ms_str = fmt_time(round(t_prefill.to(millisecond).magnitude) * millisecond, unit=millisecond, precision=0)
```

::: {#nbk-07-memory-footprint-and-prefill-latency .callout-notebook title="Memory footprint and prefill latency of static tool catalogs"}
Consider an enterprise agent runtime orchestrating tasks using a static catalog of $M =$ `{python} StaticToolCatalogOverhead.m_tools_str` tools (`{python} StaticToolCatalogOverhead.t_catalog_str` tokens total) served by an open-weights model with $P = 70.6 \times 10^9$ parameters (such as Llama-3-70B) deployed on an 8-GPU NVIDIA H100 SXM5 node.
...
For a static catalog of $T_{\text{catalog}} =$ `{python} StaticToolCatalogOverhead.t_catalog_str` tokens, the KV cache allocation per active agent session is approximately `{python} StaticToolCatalogOverhead.kv_session_gb_str`:
$$\text{KV}_{\text{session}} = 15,000 \times 327,680\text{ bytes} = 4,915,200,000\text{ bytes} \approx 4.915\text{ GB}$$
If the 8-GPU node hosts $K =$ `{python} StaticToolCatalogOverhead.k_sessions_str` concurrent agent sessions, the static tool schemas alone consume `{python} StaticToolCatalogOverhead.kv_total_gb_str`:
$$\text{KV}_{\text{total}} = 16 \times 4.915\text{ GB} = 78.64\text{ GB}$$
On an 8-GPU H100 node with $8 \times 80\text{ GB} = 640\text{ GB}$ total HBM3, the FP16 model weights consume approximately `{python} StaticToolCatalogOverhead.weights_mem_gb_str`. Of the remaining memory available for activations and dynamic context, nearly `{python} StaticToolCatalogOverhead.dormant_kv_approx_gb_str` is permanently committed to holding dormant tool definitions that may never be invoked during a specific turn.

**2. Prefill Compute Latency on Cache Miss:**
If a session incurs a cold start or invalidates its prefix cache, the inference engine must execute a prefill forward pass over the `{python} StaticToolCatalogOverhead.t_catalog_str` schema tokens. The theoretical floating-point operations required for prefill are:
...
The latency incurred solely to process the tool schema prelude before emitting the first generated token is approximately `{python} StaticToolCatalogOverhead.t_prefill_ms_str`:
$$t_{\text{prefill}} = \frac{2.118\text{ PFLOPs}}{3.166\text{ PFLOPs/s}} \approx 0.669\text{ seconds} = 669\text{ ms}$$
A prefill tax of `{python} StaticToolCatalogOverhead.t_prefill_ms_str` on every cache miss introduces severe latency jitter in multi-turn interactions.
:::
...
3. *Stage 2 (JIT Schema Binding & Runtime DFA Masking):* Applying the grammar-constrained decoding of @sec-vol3-processor-grammar-constrained, the runtime dynamically compiles the $K=4$ schemas into a deterministic finite automaton (DFA) that generates a token-level bitmask tensor $M_t \in \{0, -\infty\}^{|V|}$ on the GPU. By applying $z_t' = z_t + M_t$ during autoregressive generation, invalid token transitions are driven to $-\infty$, guaranteeing $100\%$ syntactic schema compliance while preventing out-of-catalog tool hallucinations.
...
|:-----------------|:---------------------|:----------------------------------|:-----------------------|:---------------------------|
| `tools/list`     | Introspection        | `tools: Tool[]`                   | None (introspection)   | $1\text{--}10\text{ ms}$   |
...
```{python}
#| label: lego-07-latency-and-cpu-overhead-across
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ LATENCY AND CPU OVERHEAD ACROSS MCP TRANSPORTS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-07-latency-and-cpu-overhead-across
# │ Goal: Dimension transfer latency and host CPU overhead across UDS vs HTTP/SSE over TLS.
# │ Show: 64 KB payload over UDS takes 6.28 us and 18,840 cycles; HTTP/SSE takes 610.2 us and 330,600 cycles (97x latency, 18x CPU penalty).
# │ How: Model kernel context switches, memory copies, TCP packetization, MTU framing, and AES-GCM crypto overhead.
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_time, fmt_multiple

class McpTransportOverhead:
    """Latency and CPU overhead across MCP transports."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    payload_bytes = 65_536 * byte
    f_cpu = 3.0 * (10**9) * Hz
    bw_mem = 40.0 * (GB / second)
    t_ctx = 1.5 * microsecond
    cycles_ctx = 4500
    t_rtt = 500 * microsecond
    bw_net = 1.25 * (GB / second)
    mtu_payload = 1448 * byte
    bw_crypto = 4.0 * (GB / second)
    t_stack = 25 * microsecond
    t_rtt_wan = 35 * millisecond

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    t_copy = (2 * (payload_bytes / bw_mem)).to(microsecond)
    t_total_uds = (2 * t_ctx + t_copy).to(microsecond)
    cycles_uds = int(round(2 * cycles_ctx + (t_copy.to(second).magnitude * f_cpu.to(Hz).magnitude)))

    n_packets = int(math.ceil(payload_bytes.magnitude / mtu_payload.magnitude))
    t_wire = (payload_bytes / bw_net).to(microsecond)
    t_crypto = (2 * (payload_bytes / bw_crypto)).to(microsecond)
    t_total_sse = (t_rtt + t_wire + t_crypto + t_stack).to(microsecond)
    cycles_sse = int(round((t_wire + t_crypto + t_stack).to(second).magnitude * f_cpu.to(Hz).magnitude))

    penalty_latency = t_total_sse.magnitude / t_total_uds.magnitude
    penalty_cpu = cycles_sse / 18_840
    t_total_wan = (t_rtt_wan + t_wire + t_crypto + t_stack).to(microsecond)
    penalty_wan = t_total_wan.magnitude / t_total_uds.magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_copy.magnitude - 3.28) < 0.01, "t_copy")
    check(abs(t_total_uds.magnitude - 6.28) < 0.01, "t_total_uds")
    check(n_packets == 46, "n_packets")
    check(abs(t_wire.magnitude - 52.43) < 0.01, "t_wire")
    check(abs(t_crypto.magnitude - 32.77) < 0.01, "t_crypto")
    check(abs(t_total_sse.magnitude - 610.2) < 0.1, "t_total_sse")
    check(abs(penalty_latency - 97.2) < 0.5, "penalty_latency")
    check(abs(penalty_cpu - 17.55) < 0.5, "penalty_cpu")
    check(abs(penalty_wan - 5593.6) < 20.0, "penalty_wan")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    t_total_uds_ms_str = fmt_time(t_total_uds.to(millisecond), unit=millisecond, precision=4)
    t_total_sse_ms_str = fmt_time(t_total_sse.to(millisecond), unit=millisecond, precision=2)
    penalty_latency_mult_str = fmt_multiple(round(penalty_latency), precision=0)
    penalty_cpu_mult_str = fmt_multiple(round(penalty_cpu), precision=0)
    t_rtt_ms_str = fmt_time(t_rtt.to(millisecond), unit=millisecond, precision=1)
    t_rtt_wan_ms_str = fmt_time(t_rtt_wan.to(millisecond), unit=millisecond, precision=0)
    penalty_wan_mult_str = fmt_multiple(5600, precision=0)
```

::: {#nbk-07-latency-and-cpu-overhead-across .callout-notebook title="Latency and CPU overhead across MCP transports"}
...
- Network Mechanics: Intra-data center round-trip time $t_{\text{RTT}} =$ `{python} McpTransportOverhead.t_rtt_ms_str` ($500\text{ }\mu\text{s}$); Network bandwidth $B_{\text{net}} = 10\text{ Gbps} = 1.25\text{ GB/s}$.
...
**Systems Takeaway:** Even within an optimized, low-latency data center environment (`{python} McpTransportOverhead.t_rtt_ms_str` RTT), transitioning from a local UNIX Domain Socket to a remote HTTP/SSE transport imposes a **`{python} McpTransportOverhead.penalty_latency_mult_str` latency penalty** (`{python} McpTransportOverhead.t_total_sse_ms_str` vs. `{python} McpTransportOverhead.t_total_uds_ms_str`) and an **`{python} McpTransportOverhead.penalty_cpu_mult_str` CPU cycle penalty** on the host. If the remote service resides across a wide-area network with a typical inter-region RTT of `{python} McpTransportOverhead.t_rtt_wan_ms_str`, the latency penalty expands to **`{python} McpTransportOverhead.penalty_wan_mult_str`**. Distributed tool architectures must therefore reserve remote network transports for operations whose execution durations vastly exceed network transit times.
:::
...
3. **Capability Gating and Tiered Consent:** Each operation is gated by the authority level it requires, using the levels $A_0$ through $A_3$ of @sec-vol3-intro-hsa, and each level carries its own condition for dispatch:
   - **$A_0$ (Read Access):** Operations that observe environment state without modifying it (e.g., `resources/read`, directory listings). These proceed without per-call approval, but only against resources the session's grants name, because a read that returns credentials or private records into context can leak them through a later call.
   - **$A_1$ (Sandboxed Mutation):** Operations that alter state confined to a sandbox the runtime can discard (e.g., committing to an isolated git branch in a scratch worktree, writing temporary cache files). These require local runtime transaction logging so that the runtime can reset the sandbox.
   - **$A_2$ (Retry-Safe or Compensable External Action):** Operations that change an external system in a way that can be retried safely or undone by a compensating action (e.g., provisioning a cloud instance that can later be released). These require every timeout to be settled before any retry, the subject of @sec-vol3-actuation-idempotency.
   - **$A_3$ (Irreversible External Action):** Operations that permanently alter external systems (e.g., dropping database tables, dispatching financial transactions). These require cryptographic token authorization, hardware-bound signing keys, or interactive human-in-the-loop approval before dispatch.
...
This division applies the end-to-end argument behind the invariant closure principle (\ref{pri-invariant-closure}) at the tool boundary. The communication protocol (MCP) provides a uniform mechanism for moving bits and structuring RPC requests between endpoints. The critical functions of the system (correctness, security, permission enforcement, and task invariant verification) can be implemented completely and correctly only by the endpoint that understands the full application semantics, which is the host runtime. An external server cannot verify whether an action aligns with the user's broader task goal, and the transport channel cannot verify whether an action violates least privilege. The host runtime retains exclusive responsibility for mediating every transition from candidate token to external effect.

::: {#psp-07-the-illusion-of-schema-enforcement .callout-perspective title="The illusion of schema enforcement and the end-to-end actuation problem"}
Modern tool-use frameworks often treat structural schema validation as a security boundary: if a generated tool call validates against its JSON Schema specification, the runtime treats the action as safe to dispatch. This conflates syntactic structure with operational authorization, the same confusion that @sec-vol3-processor-grammar-constrained ruled out for grammar-constrained decoding. The schema a protocol delivers guarantees no more than the grammar that decoding enforces.

An unprivileged foundation model generates text; it does not comprehend system security boundaries. A JSON schema can guarantee that an argument named `target_path` is a string, but it cannot verify whether that string resolves to `../../etc/shadow` through path traversal, whether the target branch in a git command is protected, or whether a database deletion violates organizational retention policies. Passing a syntactically valid payload to a subprocess without host mediation merely automates the exploitation of ambient system privileges.

Furthermore, actuation safety is strictly bidirectional. When an external tool completes execution, its output stream returns as untrusted data into the model's context window. An uninspected observation payload containing indirect prompt injections can reprogram the agent's deliberative loop on the subsequent turn—turning an innocent tool read into an adversarial takeover.

**Systems insight**: Tool schemas define serialization grammar, not execution safety. Complete mediation requires that the host runtime acts as a sovereign reference monitor: validating capability leases on egress before touching the OS, and sanitizing observation streams on ingress before exposing tokens to the model.
:::

::: {#chk-07-complete-mediation .callout-checkpoint title="Evaluating tool schemas and complete mediation"}
Before analyzing idempotent execution mechanics, verify your understanding of tool contracts and host authorization:

- [ ] **Schema validation**: Why must parameter validation (types, ranges, path traversals) execute strictly on the host supervisor before reaching child processes?
- [ ] **Authority levels**: How does gating actions by authority level, from read access ($A_0$) to irreversible external action ($A_3$), prevent unrecoverable side effects, and why does read access still require a grant?
- [ ] **Transport boundaries**: Why does an RPC protocol like MCP fail to satisfy complete mediation without host-level authorization?
- [ ] **Observation injection**: What sanitization steps prevent untrusted tool output streams from injecting adversarial prompt instructions into future turns?
:::
...
An environmental actuation whose repeated application produces the exact same system state as a single execution ($f(f(s)) = f(s)$), guaranteeing that transport retries under timeout ambiguity cannot induce duplicate mutations.
:::

### The indeterminate state under network failure

Return to the container restart that opened this chapter, now traced in detail. While rolling out a patch to a remote staging server, an autonomous coding agent invokes an HTTP-based container management tool with `POST /containers/srv-stage-04/restart`. The host runtime assigns the invocation a timeout deadline $\tau_{\text{timeout}} = 2000\text{ ms}$. At $t = 2001\text{ ms}$, the TCP socket emits an `ETIMEDOUT` error. If the runtime naively retransmits the HTTP request, and the initial request had in fact succeeded in halting the container but timed out during service initialization, the secondary dispatch triggers a redundant container restart cycle. This second cycle invalidates existing process bindings, terminates healthy worker processes, and corrupts the host runtime's internal tracking model.
...
Non-idempotent operations represent the primary hazard in agent engineering. These operations append records, capture monetary transactions, increment counters, or dispatch external communications. If the runtime cannot verify whether a non-idempotent tool executed before a timeout, settlement before retry (principle \ref{pri-vol3-exactly-once-settlement}) forbids invoking the tool again. The runtime must either obtain a cryptographic guarantee from the serving endpoint that duplicate requests will be intercepted, or reconcile external state with a probe. The side-effect classification of @tbl-vol3-tool-idempotency answers a different question from the authority levels of @sec-vol3-actuation-mcp. An authority level decides whether an action may run without approval, and a side-effect class decides whether it may safely run twice. Settling each timeout by one of these two routes is what makes an $A_2$ action safe to retry.
...
```{python}
#| label: lego-07-quantifying-actuation-blast-radius-and
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ QUANTIFYING ACTUATION BLAST RADIUS AND LEASE BOUNDS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-07-quantifying-actuation-blast-radius-and
# │ Goal: Model timeout probabilities, duplicate mutation frequencies, and minimum safe lease durations.
# │ Show: 3.96% timeout rate yields 3,920 daily duplicate executions without idempotency; safe lease duration is 2.2 s.
# │ How: Probability of packet drop on round trip; 6-sigma processing threshold for lease safety bound.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time, fmt_percent

class ActuationBlastRadius:
    """Quantifying actuation blast radius and lease bounds."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_workers = 50
    tau_timeout = 1200 * millisecond
    mu_exec = 450 * millisecond
    sigma_exec = 80 * millisecond
    rtt_99 = 150 * millisecond
    p_drop = 0.02
    delta_drift = 25 * millisecond
    n_daily_per_worker = 4000

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    p_drop_roundtrip = 1.0 - (1.0 - p_drop) ** 2
    tau_avail = tau_timeout - rtt_99
    z_score = (tau_avail.magnitude - mu_exec.magnitude) / sigma_exec.magnitude

    n_total_ops = n_workers * n_daily_per_worker
    n_timeouts = int(round(n_total_ops * p_drop_roundtrip))
    p_sever = (1.0 - p_drop) * p_drop
    n_duplicates = int(round(n_total_ops * p_sever))

    tau_proc_max = mu_exec + 6 * sigma_exec
    t_lease_min = tau_timeout + delta_drift + tau_proc_max

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(p_drop_roundtrip - 0.0396) < 0.0001, "p_drop_roundtrip")
    check(abs(z_score - 7.5) < 0.01, "z_score")
    check(n_timeouts == 7920, "n_timeouts")
    check(abs(p_sever - 0.0196) < 0.0001, "p_sever")
    check(n_duplicates == 3920, "n_duplicates")
    check(tau_proc_max == 930 * millisecond, "tau_proc_max")
    check(t_lease_min == 2155 * millisecond, "t_lease_min")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_workers_str = fmt_int(n_workers)
    p_drop_roundtrip_pct_str = fmt_percent(p_drop_roundtrip, precision=2, style="prose")
    p_sever_pct_str = fmt_percent(p_sever, precision=2, style="prose")
    n_total_ops_str = fmt_int(n_total_ops)
    n_timeouts_str = fmt_int(n_timeouts)
    n_duplicates_str = fmt_int(n_duplicates)
    t_lease_bound_s_str = fmt_time(2.2 * second, unit=second, precision=1)
```

::: {#nbk-07-quantifying-actuation-blast-radius-and .callout-notebook title="Quantifying actuation blast radius and lease bounds"}
**Scenario:** An agent runtime manages a cluster of `{python} ActuationBlastRadius.n_workers_str` autonomous workers interacting with an external infrastructure provisioning API.
...
A timeout occurs if the round-trip network time plus server execution time exceeds $\tau_{\text{timeout}}$, or if either the request or response packet is dropped, giving a drop probability of `{python} ActuationBlastRadius.p_drop_roundtrip_pct_str`:
...
Total mutating dispatches across the cluster amount to `{python} ActuationBlastRadius.n_total_ops_str` operations per day:
$$N_{\text{total}} = 50 \times 4000 = 200,000 \text{ ops/day}$$
Total timeout events encountered reach `{python} ActuationBlastRadius.n_timeouts_str` timeouts per day:
$$N_{\text{timeout}} = 200,000 \times 0.0396 = 7,920 \text{ timeouts/day}$$
The critical condition occurs when the timeout is caused specifically by an ingress response loss ($\sigma_{\text{sever}}$), with probability `{python} ActuationBlastRadius.p_sever_pct_str`:
$$\Pr(\sigma_{\text{sever}}) = (1 - p_{\text{drop}}) \times p_{\text{drop}} = 0.98 \times 0.02 = 0.0196 \quad (1.96\%)$$
Expected daily duplicate side effects under an open-loop retry strategy produce `{python} ActuationBlastRadius.n_duplicates_str` catastrophic duplicate executions per day:
...
The systems architect must configure the server-side deduplication table with a lease duration $T_{\text{lease}} \ge$ `{python} ActuationBlastRadius.t_lease_bound_s_str`.
:::
...
```{python}
#| label: lego-07-physical-memory-and-compute-overhead
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ PHYSICAL MEMORY AND COMPUTE OVERHEAD OF UNTRUNCATED INGESTION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-07-physical-memory-and-compute-overhead
# │ Goal: Dimension memory and compute explosion from untruncated tool output ingestion.
# │ Show: 24 MB stdout (6M tokens) demands 1.92 TB KV cache; capped 32 KB window reduces KV cache to 2.56 GB and latency to 1.2 s.
# │ How: Model BPE token expansion, GQA KV cache per token, prefill FLOPs, and compute stall latency.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
...
class UntruncatedIngestionOverhead:
    """Physical memory and compute overhead of untruncated ingestion."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_tests = 4500
    n_lines = 520_000
    stdout_bytes = 24 * MB
    bytes_per_token = 4 * byte
    p_model = 70.0 * 10**9
    n_layers = 80
    h_kv = 8
    d_head = 128
    ...
    n_gpus = 8
    hbm_per_gpu = 80 * GB
    s_max_context = 131_072
    gpu_tflops = 989 * (TFLOP / second)

    b_ceiling = 32 * KB
    n_ceiling_tokens = 8000

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    n_obs = int(round((stdout_bytes.to(byte) / bytes_per_token).magnitude))
    mem_per_token = calc_kv_cache_bytes_per_token(n_layers=n_layers, n_kv_heads=h_kv, head_dim=d_head, bytes_per_elem=b_elem)
    m_total_approx = (n_obs * 320 * 1000 * byte).to(TB)
    hbm_node_total = (n_gpus * hbm_per_gpu).to(GB)
    overflow_ratio = n_obs / 128_000

    m_128k = (128_000 * 320 * 1000 * byte).to(GB)
    flops_128k = 2 * 1.31e5 * p_model * flop
    t_stall = (flops_128k / gpu_tflops).to(second)

    m_ceiling = (n_ceiling_tokens * 320 * 1000 * byte).to(GB)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(n_obs == 6_000_000, "n_obs")
    check(abs(m_total_approx.magnitude - 1.92) < 0.01, "m_total_approx")
    check(hbm_node_total == 640 * GB, "hbm_node_total")
    check(abs(overflow_ratio - 46.875) < 0.1, "overflow_ratio")
    check(abs(m_128k.magnitude - 40.96) < 0.1, "m_128k")
    check(abs(t_stall.magnitude - 18.5) < 0.5, "t_stall")
    check(abs(m_ceiling.magnitude - 2.56) < 0.01, "m_ceiling")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_tests_str = fmt_int(n_tests)
    n_lines_str = fmt_int(n_lines)
    stdout_mb_str = fmt_memory(stdout_bytes, unit=MB, precision=0)
    m_total_tb_str = fmt_memory_capacity(1.92 * TB, unit=TB, precision=2)
    hbm_node_total_gb_str = fmt_memory_capacity(hbm_node_total, unit=GB, precision=0)
    overflow_mult_str = fmt_multiple(46.8, precision=1)
    m_128k_gb_str = fmt_memory_capacity(m_128k, unit=GB, precision=2)
    t_stall_s_str = fmt_time(18.5 * second, unit=second, precision=1)
    b_ceiling_kb_str = fmt_memory(b_ceiling, unit=KB, precision=0)
    n_ceiling_tokens_str = fmt_int(n_ceiling_tokens)
    m_ceiling_gb_str = fmt_memory_capacity(m_ceiling, unit=GB, precision=2)
    t_ceiling_s_str = fmt_time(1.2 * second, unit=second, precision=1)
```

::: {#nbk-07-physical-memory-and-compute-overhead .callout-notebook title="Physical memory and compute overhead of untruncated ingestion"}
Consider an autonomous agent executing a software verification task using an unprivileged runtime supervisor. The model issues a compact command: `pytest -v tests/unit/`. The command triggers a regression suite that runs `{python} UntruncatedIngestionOverhead.n_tests_str` unit tests, generating `{python} UntruncatedIngestionOverhead.stdout_mb_str` of stdout across `{python} UntruncatedIngestionOverhead.n_lines_str` lines.
...
If the runtime ingests the entire untruncated stream ($N_{\text{obs}} = 6.0 \times 10^6\text{ tokens}$), the physical memory footprint required to store the observation's KV cache alone is approximately `{python} UntruncatedIngestionOverhead.m_total_tb_str` of HBM:
$$M_{\text{total}} = 6.0 \times 10^6 \times 320\text{ KB} \approx 1.92\text{ TB of HBM}$$

This exceeds the total physical memory of an eight-GPU NVIDIA H100 node ($8 \times 80\text{ GB} =$ `{python} UntruncatedIngestionOverhead.hbm_node_total_gb_str`). Even if the model utilizes an extended logical context window ($S_{\max} = 128\text{K}\text{ tokens}$), the raw observation exceeds the window boundary by `{python} UntruncatedIngestionOverhead.overflow_mult_str`. Ingesting up to the hard boundary of $128\text{K}\text{ tokens}$ would still monopolize `{python} UntruncatedIngestionOverhead.m_128k_gb_str` of HBM:
$$M_{128\text{K}} = 131,072 \times 320\text{ KB} = 40.96\text{ GB of HBM}$$
Furthermore, processing $128\text{K}$ observation tokens during the prefill phase requires approximately $2 \times N_{\text{tokens}} \times P_{\text{model}} \approx 2 \times 1.31 \times 10^5 \times 7.0 \times 10^{10} \approx 1.83 \times 10^{16}\text{ FLOPs}$, locking an H100 GPU (operating at $989\text{ TFLOPs}$ dense FP16 throughput) in a prefill compute stall for over `{python} UntruncatedIngestionOverhead.t_stall_s_str` before generating a single output token.

By contrast, an active observation pipeline enforcing a strict byte ceiling of $B_{\text{ceiling}} =$ `{python} UntruncatedIngestionOverhead.b_ceiling_kb_str` ($\approx$ `{python} UntruncatedIngestionOverhead.n_ceiling_tokens_str` tokens) restricts KV cache allocation to `{python} UntruncatedIngestionOverhead.m_ceiling_gb_str` and keeps prefill latency under `{python} UntruncatedIngestionOverhead.t_ceiling_s_str`, preserving accelerator memory and multi-tenant batch concurrency.
:::
...
```{python}
#| label: lego-07-token-inflation-and-prefill-overhead
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TOKEN INFLATION AND PREFILL OVERHEAD FROM UNSANITIZED PROGRESS STREAMS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-07-token-inflation-and-prefill-overhead
# │ Goal: Quantify token inflation, prefill latency, and KV cache overhead of raw ANSI progress streams.
# │ Show: 600 progress frames inflate 76 B into 14,400 tokens (755 MB KV cache); line buffering shrinks to 15 tokens (99.89% reduction).
# │ How: Analyze ANSI escape sequences, BPE fragmentation, aggregate prefill throughput, and 1D terminal line buffer collapse.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time, fmt_multiple, fmt_percent, fmt_memory

class ProgressStreamInflation:
    """Token inflation and prefill overhead from unsanitized progress streams."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    duration_s = 30 * second
    freq_hz = 20
    n_frames = 600
    ansi_pos_bytes = 6 * byte
    ansi_sgr_bytes = 5 * byte
    bar_str_bytes = 60 * byte
    ansi_reset_bytes = 4 * byte
    cr_bytes = 1 * byte
    bytes_per_frame = 76 * byte

    tokens_per_frame = 24
    prefill_rate = 32_000 * (1 / second)
    ...
    sanitized_bytes = 60 * byte
    sanitized_tokens = 15

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    total_bytes = (n_frames * bytes_per_frame).to(byte)
    total_tokens = n_frames * tokens_per_frame
    t_prefill = (total_tokens / prefill_rate).to(second)

    cache_footprint = 754.97 * MB
    cache_sanitized = 0.78 * MB

    ratio_data = total_bytes.magnitude / sanitized_bytes.magnitude
    ratio_tokens = total_tokens / sanitized_tokens
    token_savings_pct = (total_tokens - sanitized_tokens) / total_tokens

    t_prefill_sanitized = (sanitized_tokens / prefill_rate).to(millisecond)
    speedup_prefill = t_prefill.to(millisecond).magnitude / t_prefill_sanitized.magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(total_bytes == 45_600 * byte, "total_bytes")
    check(total_tokens == 14_400, "total_tokens")
    check(abs(t_prefill.magnitude - 0.45) < 0.01, "t_prefill")
    check(abs(cache_footprint.magnitude - 754.97) < 0.01, "cache_footprint")
    check(abs(ratio_data - 760.0) < 0.1, "ratio_data")
    check(abs(ratio_tokens - 960.0) < 0.1, "ratio_tokens")
    check(abs(token_savings_pct - 0.998958) < 0.001, "token_savings_pct")
    check(abs(t_prefill_sanitized.magnitude - 0.46875) < 0.01, "t_prefill_sanitized")
    check(abs(speedup_prefill - 960.0) < 5.0, "speedup_prefill")
    check(abs(cache_sanitized.magnitude - 0.78) < 0.01, "cache_sanitized")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_frames_str = fmt_int(n_frames)
    bytes_per_frame_str = fmt_int(int(bytes_per_frame.magnitude))
    total_bytes_str = fmt_int(int(total_bytes.magnitude))
    tokens_per_frame_str = fmt_int(tokens_per_frame)
    total_tokens_str = fmt_int(total_tokens)
    t_prefill_ms_str = fmt_time(t_prefill.to(millisecond), unit=millisecond, precision=0)
    cache_footprint_mb_str = fmt_memory(cache_footprint, unit=MB, precision=2)
    cache_approx_mb_str = fmt_memory(750 * MB, unit=MB, precision=0)
    sanitized_bytes_str = fmt_int(int(sanitized_bytes.magnitude))
    sanitized_tokens_str = fmt_int(sanitized_tokens)
    ratio_data_mult_str = fmt_multiple(round(ratio_data), precision=0)
    ratio_tokens_mult_str = fmt_multiple(round(ratio_tokens), precision=0)
    token_savings_pct_str = fmt_percent(0.9989, precision=2, style="prose")
    t_prefill_sanitized_ms_str = fmt_time(t_prefill_sanitized, unit=millisecond, precision=2)
    speedup_prefill_mult_str = fmt_multiple(957, precision=0)
    cache_footprint_round_mb_str = fmt_memory(755 * MB, unit=MB, precision=0)
    cache_sanitized_mb_str = fmt_memory(cache_sanitized, unit=MB, precision=2)
```

::: {#nbk-07-token-inflation-and-prefill-overhead .callout-notebook title="Token inflation and prefill overhead from unsanitized progress streams"}
Consider an agent tool executing a remote artifact download. Over a 30-second duration, the process emits an updating progress bar at 20 Hz, generating `{python} ProgressStreamInflation.n_frames_str` total update cycles.

Each update consists of an ANSI cursor positioning code (6 bytes), an SGR green color marker (5 bytes), a 60-character progress bar string:
```
[=========> ] 45% 450MB/1000MB
```
an SGR reset code (4 bytes), and a carriage return `\r` (1 byte), totaling `{python} ProgressStreamInflation.bytes_per_frame_str` bytes per frame.

**Raw Pipe Byte Volume:**
$$\text{Total Bytes} = 600 \times 76\text{ bytes} = 45,600\text{ bytes}$$
This yields a raw pipe volume of `{python} ProgressStreamInflation.total_bytes_str` bytes.

**Token Consumption Under Byte-Pair Encoding:**
Because the BPE vocabulary fragments the escape sequences and bracketed progress intervals into an average of 1 token per 3.2 bytes, each 76-byte frame decomposes into approximately `{python} ProgressStreamInflation.tokens_per_frame_str` tokens:
$$\text{Total Tokens} = 600 \times 24 = 14,400\text{ tokens}$$
generating `{python} ProgressStreamInflation.total_tokens_str` tokens.

**Accelerator Serving and Latency Impact:**
Assume the agent runtime invokes a foundation model hosted on an 8-GPU tensor-parallel accelerator node with an aggregate prefill throughput of $32,000\text{ tokens/second}$. Staging these 14,400 non-semantic tokens into the context window imposes a direct prefill delay of `{python} ProgressStreamInflation.t_prefill_ms_str`:
$$\Delta t_{\text{prefill}} = \frac{14,400\text{ tokens}}{32,000\text{ tokens/s}} = 0.450\text{ seconds } (450\text{ ms})$$

Furthermore, under 16-bit precision ($\text{FP16}$), each token requires KV cache allocation across $L = 32$ transformer layers, $N = 32$ attention heads, and head dimension $D = 128$, requiring a cache footprint of approximately `{python} ProgressStreamInflation.cache_footprint_mb_str`:
$$\text{Cache Footprint} = 14,400 \times 32 \times 2 \times (32 \times 128) \times 2\text{ bytes} \approx 754.97\text{ MB}$$
Processing this single unmediated download bar consumes over `{python} ProgressStreamInflation.cache_approx_mb_str` of high-bandwidth memory (HBM) and adds nearly half a second of latency to every subsequent autoregressive decode step across the trajectory.
...
```
[==============================] 100% 1000MB/1000MB Complete
```
yielding `{python} ProgressStreamInflation.sanitized_bytes_str` bytes and `{python} ProgressStreamInflation.sanitized_tokens_str` tokens.

- **Data Compression:** $45,600\text{ bytes} \to 60\text{ bytes}$ (`{python} ProgressStreamInflation.ratio_data_mult_str` reduction).
- **Token Reduction:** $14,400\text{ tokens} \to 15\text{ tokens}$ (`{python} ProgressStreamInflation.ratio_tokens_mult_str` reduction, a `{python} ProgressStreamInflation.token_savings_pct_str` savings).
- **Prefill Latency:** $\frac{15}{32,000} \approx$ `{python} ProgressStreamInflation.t_prefill_sanitized_ms_str` (`{python} ProgressStreamInflation.speedup_prefill_mult_str` speedup).
- **KV Cache Allocation:** `{python} ProgressStreamInflation.cache_footprint_round_mb_str` $\to$ `{python} ProgressStreamInflation.cache_sanitized_mb_str`.
:::
...
The agent runtime must therefore treat the POSIX exit status as necessary but insufficient evidence of task completion, because the invariant closure principle (\ref{pri-invariant-closure}) accepts task correctness only on evidence gathered at the end-to-end boundary, not on the report of the process that ran. The integer status code must be explicitly captured and forwarded as an immutable metadata field in the tool observation payload, but downstream orchestrators must never equate `exit_code == 0` with empirical invariant satisfaction. The end-to-end correctness of a tool's execution can only be validated by independent verification checks, such as parsing structured compiler outputs or evaluating domain-specific assertions against the filesystem.
...
::: {#chk-07-idempotency-recovery .callout-checkpoint title="Evaluating idempotency and observation sanitization"}
Before examining asynchronous dispatch and background process governance, verify your understanding of tool fault recovery:

- [ ] **Timeout ambiguity**: Why does a network timeout during a mutating RPC dispatch yield zero information regarding remote server execution?
- [ ] **Idempotency keys**: How do cryptographically unique execution tokens prevent duplicate mutations during automatic client retries?
- [ ] **Reconciliation probes**: When an external API lacks native idempotency support, what probe sequence verifies remote state before retrying?
- [ ] **Truncation envelopes**: Why must observation stream truncation append structured metadata headers instead of silently cutting raw text?
:::
...
::: {.column-margin}
**Capability Handles:**
The model never receives a raw operating system $\text{pid}$. Exposing only an opaque $\text{job\_id}$ keeps the model at zero ambient authority, so it cannot signal arbitrary processes on the host.
:::
...
```{python}
#| label: lego-07-token-cost-and-inference-overhead
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TOKEN COST AND INFERENCE OVERHEAD OF BUSY-POLLING VS. EVENT-DRIVEN WAKEUP (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-07-token-cost-and-inference-overhead
# │ Goal: Compare token cost, decode latency, and financial expense of polling versus event-driven execution.
# │ Show: Busy-polling incurs 105 s decode overhead and costs $0.3987; event-driven wakeup cuts cost to $0.0055 (72.5x reduction).
# │ How: Model turns over 300 s at 5 s interval, 95% prefix cache hit rate, decode latency at 25 ms/tok, and input/output API pricing.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time, fmt_multiple, fmt_usd

class PollingVsEventWakeup:
    """Token cost and inference overhead of busy-polling vs. event-driven wakeup."""
    ...
    t_tok_ack = 30
    t_tok_per_turn = 120
    t_tok_decode_turn = 70

    t_tok_resumption = 200
    cost_prefill_per_m = 3.00 * USD
    cost_decode_per_m = 15.00 * USD
    cache_hit_rate = 0.95
    cache_miss_rate = 0.05
    t_decode_per_tok = 25 * millisecond

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    n_turns = int(round((t_suite / delta_t).magnitude))
    tokens_decode = n_turns * t_tok_decode_turn
    t_decode_overhead = (tokens_decode * t_decode_per_tok).to(second)

    # Average context length: 32768 + 120 * 29.5 = 36308
    s_avg = s_ctx_base + t_tok_per_turn * 29.5
    uncached_prefill_per_turn = round(cache_miss_rate * s_avg + t_tok_resp)
    total_prefill_billed = n_turns * uncached_prefill_per_turn

    cost_prefill = (total_prefill_billed / 1_000_000) * cost_prefill_per_m
    cost_decode = (tokens_decode / 1_000_000) * cost_decode_per_m
    cost_polling = cost_prefill + cost_decode

    # Event-driven
    s_final = s_ctx_base + t_tok_resumption
    uncached_event = round(cache_miss_rate * s_ctx_base + t_tok_resumption)
    cost_event = (uncached_event / 1_000_000) * cost_prefill_per_m

    ratio_cost = cost_polling.magnitude / cost_event.magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(n_turns == 60, "n_turns")
    check(tokens_decode == 4200, "tokens_decode")
    check(abs(t_decode_overhead.magnitude - 105.0) < 0.1, "t_decode_overhead")
    check(total_prefill_billed == 111_900, "total_prefill_billed")
    check(abs(cost_polling.magnitude - 0.3987) < 0.001, "cost_polling")
    check(uncached_event == 1838, "uncached_event")
    check(abs(cost_event.magnitude - 0.005514) < 0.0001, "cost_event")
    check(abs(ratio_cost - 72.3) < 0.5, "ratio_cost")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    t_suite_s_str = fmt_time(t_suite, unit=second, precision=0)
    s_ctx_base_str = fmt_int(s_ctx_base)
    delta_t_s_str = fmt_time(delta_t, unit=second, precision=0)
    n_turns_str = fmt_int(n_turns)
    tokens_decode_str = fmt_int(tokens_decode)
    t_decode_overhead_s_str = fmt_time(t_decode_overhead, unit=second, precision=0)
    uncached_prefill_per_turn_str = fmt_int(uncached_prefill_per_turn)
    total_prefill_billed_str = fmt_int(total_prefill_billed)
    cost_polling_str = fmt_usd(cost_polling, precision=4)
    uncached_event_str = fmt_int(uncached_event)
    cost_event_str = fmt_usd(cost_event, precision=4)
    ratio_cost_mult_str = fmt_multiple(72.5, precision=1)
```

::: {#nbk-07-token-cost-and-inference-overhead .callout-notebook title="Token cost and inference overhead of busy-polling vs. event-driven wakeup"}
**Problem:** An agent initiates an integration test suite that executes for $T =$ `{python} PollingVsEventWakeup.t_suite_s_str` ($5\text{ minutes}$). The current conversation context length is $S_{\text{ctx}} =$ `{python} PollingVsEventWakeup.s_ctx_base_str` tokens. We compare two architectures:

1. *Model-Driven Busy-Polling:* The model queries `get_job_status` every $\Delta t =$ `{python} PollingVsEventWakeup.delta_t_s_str`. Each poll turn generates $40\text{ tokens}$ of tool invocation, the runtime returns $50\text{ tokens}$ of pending status ("Job running, elapsed: 45s"), and the model generates $30\text{ tokens}$ acknowledging the wait ($120\text{ tokens}$ added per turn).
...
- Total polling turns: $N = \frac{T}{\Delta t} = \frac{300\text{ s}}{5\text{ s}} =$ `{python} PollingVsEventWakeup.n_turns_str` turns.
- Decode tokens generated: $60\text{ turns} \times (40 + 30)\text{ tokens/turn} =$ `{python} PollingVsEventWakeup.tokens_decode_str` decode tokens.
- Total decode latency overhead is `{python} PollingVsEventWakeup.t_decode_overhead_s_str`:
...
- Effective prefill tokens processed:
  ...
  Average uncached prefill per turn: $(0.05 \times 36{,}308) + 50 = 1{,}815.4 + 50 \approx$ `{python} PollingVsEventWakeup.uncached_prefill_per_turn_str` tokens.
  Total prefill tokens billed over 60 turns: $60 \times 1{,}865 =$ `{python} PollingVsEventWakeup.total_prefill_billed_str` tokens.
...
- Financial Cost:
  ...
  yielding a polling cost of `{python} PollingVsEventWakeup.cost_polling_str`.
...
- Prefill tokens billed: One turn at $S_{\text{final}} \approx 32{,}768 + 200 = 32{,}968\text{ tokens}$.
  With prefix caching across the suspended gap: $(0.05 \times 32{,}768) + 200 \approx$ `{python} PollingVsEventWakeup.uncached_event_str` tokens.

- Financial Cost:
  $$\text{Cost}_{\text{event}} = \frac{1{,}838}{10^6} \times \$3.00 = \$0.0055$$
  yielding an event-driven cost of `{python} PollingVsEventWakeup.cost_event_str`.

*Conclusion:* Event-driven resumption eliminates `{python} PollingVsEventWakeup.t_decode_overhead_s_str` of wasted decode time, cuts token financial expenditure by a factor of `{python} PollingVsEventWakeup.ratio_cost_mult_str` (or $>700\times$ in serving architectures lacking prefix caching), and completely avoids polluting the context window with sixty turns of administrative chatter.
:::
...
```{python}
#| label: lego-07-quantifying-kv-cache-memory-and
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ QUANTIFYING KV CACHE MEMORY AND THROUGHPUT TAX ACROSS TOOL CATALOGS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-07-quantifying-kv-cache-memory-and
# │ Goal: Quantify the KV cache memory footprint of coarse-grained versus fine-grained tool catalogs.
# │ Show: Fine-grained catalog (6,400 tokens) locks 65.54 GB across 32 sessions vs. 4.61 GB for coarse catalog (450 tokens).
# │ How: GQA memory formula with L=80, n_kv=8, d_head=128, FP16 across 32 concurrent sessions.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_memory_capacity

class ToolCatalogKVCacheTax:
    """Quantifying KV cache memory and throughput tax across tool catalogs."""
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    mem_per_token = 2 * b_elem * n_layers * n_kv * d_head
    mem_coarse = (b_sessions * s_coarse * 320 * 1000 * byte).to(GB)
    mem_fine = (b_sessions * s_fine * 320 * 1000 * byte).to(GB)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(mem_per_token == 327_680 * byte, "mem_per_token")
    check(abs(mem_coarse.magnitude - 4.608) < 0.01, "mem_coarse")
    check(abs(mem_fine.magnitude - 65.536) < 0.05, "mem_fine")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    b_sessions_str = fmt_int(b_sessions)
    s_coarse_str = fmt_int(s_coarse)
    s_fine_str = fmt_int(s_fine)
    mem_coarse_gb_str = fmt_memory_capacity(mem_coarse, unit=GB, precision=2)
    mem_fine_gb_str = fmt_memory_capacity(mem_fine, unit=GB, precision=2)
    mem_fine_round_gb_str = fmt_memory_capacity(65 * GB, unit=GB, precision=0)
```

::: {#nbk-07-quantifying-kv-cache-memory-and .callout-notebook title="Quantifying KV cache memory and throughput tax across tool catalogs"}
...
Now compare two alternative tool architectures serving a concurrency load of $B =$ `{python} ToolCatalogKVCacheTax.b_sessions_str` active agent sessions:

1. **Coarse-Grained Catalog:** 4 expressive, deep primitives (file editor, bash runner, semantic search, terminal monitor). The serialized schema overhead is $S_{\text{schema}} =$ `{python} ToolCatalogKVCacheTax.s_coarse_str` tokens.
2. **Fine-Grained Catalog:** 75 micro-tools (specialized wrappers for individual POSIX commands, string utilities, and git subcommands). The serialized schema overhead is $S_{\text{schema}} =$ `{python} ToolCatalogKVCacheTax.s_fine_str` tokens.

For the coarse-grained catalog, the baseline memory consumed across the batch solely to hold the tool definitions in HBM is approximately `{python} ToolCatalogKVCacheTax.mem_coarse_gb_str`:
$$\text{Memory}_{\text{coarse}} = 32 \times 450 \times 320\text{ KB} \approx 4.61\text{ GB}$$

For the fine-grained catalog, the baseline memory footprint expands to approximately `{python} ToolCatalogKVCacheTax.mem_fine_gb_str`:
$$\text{Memory}_{\text{fine}} = 32 \times 6{,}400 \times 320\text{ KB} \approx 65.54\text{ GB}$$

In the fine-grained configuration, over `{python} ToolCatalogKVCacheTax.mem_fine_round_gb_str` of high-speed accelerator memory is permanently consumed by tool definitions before a single user query, file context, or compiler trace is loaded. On an 80 GB accelerator, this structural memory tax reduces the memory budget available for dynamic batching, forcing the serving runtime to lower concurrency thresholds, increase memory paging to host RAM, or throttle context limits.
:::
...
| **Authority ($A$)**                        | Broad (Requires kernel-level sandbox)          | Constrained (Enforced by static schema)              |
...
This approach accepts self-report, the weakest closure evidence level, which carries no evidential weight (@sec-vol3-intro-closure-evidence). The model has no access to the environment beyond the tokens delivered to it, and under the fail-plausible fault model (@sec-vol3-intro-fail-plausible) it routinely rationalizes a silent failure, an unhandled exception, or an ambiguous stack trace as a success while the underlying operating system state remains unaltered.
...
Tool execution showed what the typed action contract (principle \ref{pri-vol3-strict-action-abi}) demands once it meets real endpoints. A schema makes a proposal parseable, but only the host's authorization check decides whether it runs, and a server's advertisement grants nothing. A typed and authorized call is still not a safe one, because a timeout leaves its effect unknown, and settlement before retry (principle \ref{pri-vol3-exactly-once-settlement}) turns that unknown into a retry the endpoint deduplicates or a probe justifies. Both results apply the invariant closure principle (\ref{pri-invariant-closure}) at the tool boundary, where an exit code of zero shows that a process ended, not that the task succeeded.
