...
1.  **Significance**: Guarantees that unprivileged stochastic model emissions cannot bypass capability checks, tamper with host processes, or execute unvetted side effects, enforcing Zero Ambient Authority ($A=0$) across physical environment boundaries.
...
Modern tool-use frameworks often treat structural schema validation as a security boundary: if a generated tool call validates against its JSON Schema specification, the runtime treats the action as safe to dispatch. This conflates syntactic structure with operational authorization.
...
- [ ] **Capability tiers**: How does partitioning actions into read-only ($C_0$), reversible ($C_1$), and irreversible ($C_2$) tiers prevent unrecoverable side effects?
