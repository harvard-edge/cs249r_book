...
::: {#dfn-complete-mediation .callout-definition title="Complete mediation in tool actuation"}
**Complete Mediation** in an agentic runtime dictates that every unprivileged model emission proposing environment mutation must traverse an unbypassable four-stage supervisory gate prior to physical actuation:

1. **Schema Validation:** Syntactic verification against registered tool signatures via grammar-constrained automata or strict schema deserializers.
2. **Capability and Lease Gating:** Verification that active agent credentials hold explicit cryptographic leases for the requested resource under Zero Ambient Authority ($A=0$).
3. **Dispatched Execution:** Controlled delegation to isolated sandboxes (microVMs or network-isolated containers) with bounded timeouts and preemption lines.
4. **Observation Normalization:** Packaging raw external telemetry into typed execution envelopes staged back into the model's working memory context.
:::
