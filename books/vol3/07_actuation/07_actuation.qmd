...
Every step in that sequence happens at the tool boundary, and none of them can happen inside the model. The model holds zero ambient authority (@sec-vol3-intro-operational-incident), so the runtime performs each step, and the six-phase lifecycle (@sec-vol3-intro-execution-lifecycle) fixes where the steps sit in a turn, from action authorization through runtime dispatch to evidence capture. This chapter supplies what that lifecycle left abstract once a proposal names a real tool on a real endpoint. It gives every tool a typed descriptor, keeps authorization in the host so that no tool server can grant itself access by advertising, settles each dispatch before any retry, and returns every observation bounded and typed however much the tool prints.
