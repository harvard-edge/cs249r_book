...
This four-stage mediation architecture is illustrated in @fig-vol3-ai-system-call-boundary, contrasting the unprivileged stochastic core with the privileged host runtime supervisor. In parallel, @tbl-vol3-tool-descriptors systematizes this structural duality, comparing the classic UNIX file descriptor architecture with the agentic tool descriptor table.

![**The AI System Call Privilege Boundary**: Architectural topology of mediated tool execution. The unprivileged stochastic processor operates under Zero Ambient Authority ($A=0$) in user space, emitting candidate system call traps held in memory escrow. The host supervisor enforces complete mediation in privileged space, validating arguments against registered schemas and capabilities before dispatching to external drivers.](images/svg/ai_system_call_privilege_boundary.svg){#fig-vol3-ai-system-call-boundary width="95%"}
...
![**Two-Stage Tool Retrieval and Just-in-Time Schema Binding**: Architecture of Just-in-Time (JIT) tool schema injection. An offline catalog of $M > 3{,}000$ tools is indexed in dense embedding space. At turn $t$, coarse MIPS retrieval prunes the catalog to active subset $K \ll M$ ($K \approx 4$), followed by runtime logit masking. This reduces schema context tax by over $98\%$ and preserves accelerator KV cache budgets for dynamic execution state.](images/svg/two_stage_tool_retrieval.svg){#fig-vol3-two-stage-tool-retrieval width="95%"}

As illustrated in @fig-vol3-two-stage-tool-retrieval, JIT injection decouples the global catalog from the active context window through a multi-stage retrieval pipeline:
...
![**Idempotency Nonce Execution Flow and Deduplication Gateway**: Architectural protocol for distributed tool actuation idempotency. (1) The client supervisor synthesizes a deterministic 128-bit idempotency key (UUIDv7) binding agent ID, trajectory turn, and canonical argument hash. (2) The remote gateway inspects its transactional deduplication store; if previously committed, it returns the cached observation without re-executing side effects; otherwise, it acquires an atomic lease lock. (3) The isolated sandbox executes the single physical mutation and commits the result to write-ahead storage before releasing the lock.](images/svg/idempotency_nonce_execution_flow.svg){#fig-vol3-idempotency-flow width="95%"}

When the tool server receives a request tagged with $k_{\text{idem}}$, it executes an atomic state machine transition within its local deduplication store (@fig-vol3-idempotency-flow):
...
Beyond preventing duplicate mutations, content-addressed caching of deterministic tool observations unlocks a powerful cross-subsystem synergy: by returning byte-for-byte identical observation strings, the actuation gateway preserves prompt prefix alignment within the inference engine's Radix tree cache, as illustrated in @fig-vol3-tool-result-prefix-cache-synergy.

![**Content-Addressed Tool Result Memoization and Prefix Cache Synergy**: End-to-end integration between the Actuation Gateway and the GPU Inference Serving Engine. (1) The host intercepts mutating or expensive tool dispatches (`ToolCall("compile", ...)`), computing a deterministic SHA-256 hash over canonical parameters. (2) Content-addressed cache hit returns stored observation bytes in $0.8\text{ ms}$, bypassing a $14.5\text{ s}$ container compilation sandbox. (3) Deterministic observation bytes appended to prompt match Node 3 in the GPU Radix-tree prefix cache, achieving $100\%$ prefix cache hit, zero prefill FLOP recomputation, and a $250\times$ acceleration in Time to First Token (TTFT).](images/svg/tool_result_prefix_cache_synergy.svg){#fig-vol3-tool-result-prefix-cache-synergy width="95%"}
...
When an uncooperative tool dispatch encounters an ambiguous timeout $\tau_{\text{timeout}}$, the runtime must shift from transport-level key deduplication to closed-loop *state reconciliation probes* (@fig-vol3-state-reconciliation-flow). Instead of issuing an immediate retransmission, the supervisor initiates a read-only inquiry probe to inspect the physical environment and determine whether the target mutation completed.

![**State Reconciliation Probe Architecture for Uncooperative Endpoints**: Closed-loop state reconciliation protocol under ambiguous RPC timeouts. If a mutating tool dispatch triggers a timeout, the supervisor suppresses automatic retransmission and issues a lightweight nullipotent inquiry probe ($\text{probe}_{\text{post}}$). If the postcondition holds ($\sigma_{\text{sever}}$), the mutation succeeded and a synthetic observation is constructed. If the environment reflects pre-mutation state $s_0$ ($\sigma_{\text{drop}}$), re-dispatch is provably safe. If state has drifted or corrupted ($\sigma_{\text{crash}}$), a terminal fault is raised.](images/svg/state_reconciliation_probe_flow.svg){#fig-vol3-state-reconciliation-flow width="95%"}
...
::: {.column-margin}
![](images/svg/vol3_actuation_margin_002.svg){width="100%" fig-alt="Segmented stream buffer diagram contrasting a 4.2 MB unconstrained tool output against a 4 KB sandwich-truncated window preserving head and tail lines."}

*Sandwich truncation discards intermediate execution noise while pinning critical head schemas and tail stack traces.*
:::
...
**The runtime must treat every external observation stream as an untrusted, unbounded data producer.** Between the raw operating system transport and the context staging buffer, the runtime must interpose an active observation pipeline (@fig-vol3-stream-ingestion). This pipeline decouples external process emission from neural context staging through three cooperating systems primitives: kernel-level backpressure that constrains producer throughput at the operating system boundary, fixed-size sliding-window ring buffers that extract high-entropy diagnostic signals from stream boundaries, and structured pagination contracts that convert unbounded data dumps into demand-driven traversals.

![**Stream Ingestion Pipeline**: The observation stream ingestion pipeline decouples unprivileged tool execution from context staging. Data flows from the child process across a kernel pipe buffer into a fixed-capacity circular ring buffer before undergoing sandwich truncation and token budget validation.](images/svg/vol3-stream-ingestion.svg){#fig-vol3-stream-ingestion width="95%"}
...
The generation and lifecycle of continuation tokens introduce a concrete systems trade-off between host memory utilization and query consistency. The runtime can implement pagination state through either server-side session leases or client-side stateless encrypted tokens.
...
As illustrated in @fig-vol3-stream-ingestion, the transfer of observation data between the isolated child process and the host runtime supervisor is mediated by an inter-process communication (IPC) channel: the standard UNIX unidirectional data channel created via [`pipe(2)`](file:///usr/include/unistd.h).
...
![**Terminal Stream Normalization and Escape Resolution Pipeline**: Architecture of deterministic terminal stream normalization. Raw bytes from child processes are parsed through an ECMA-48 finite-state machine to strip display and color escape sequences (Phase 1), followed by a 1D virtual line buffer that resolves in-place carriage returns (`\r`) to collapse repetitive progress bars into a single terminal status (Phase 2). Sanitized streams are then enclosed in structured observation frames with exit codes and truncation metadata (Phase 3).](images/svg/terminal_stream_normalization_pipeline.svg){#fig-vol3-terminal-normalization width="95%"}
...
This asynchronous execution model is contrasted with synchronous blocking dispatch in @fig-vol3-async-tool-dispatch.

![**Asynchronous Job Escrow Dispatch**: Comparison between synchronous blocking dispatch and asynchronous job escrow dispatch, illustrating how the runtime decouples deliberative thread scheduling from physical child process execution.](images/svg/fig-vol3-async-tool-dispatch.svg){#fig-vol3-async-tool-dispatch width="95%"}
...
Connecting an agent to external tools gives it the power to act on the world, but it also exposes the host environment to catastrophic failure. When a model dispatches arbitrary shell commands, compiles unfamiliar source code, or executes dynamically generated scripts, syntactic schema validation alone cannot prevent malicious exploits, destructive side effects, or adversarial prompt injection attacks from compromising the underlying operating system. Establishing a typed RPC interface is only half of the protection equation; the code invoked by that interface must execute in an environment where fault domains are strictly contained. In Chapter 08 (*Environmental Isolation*, @sec-vol3-virtualization), we shift from the communication interface to the containment substrate, analyzing how hardware-isolated microVMs, WebAssembly runtimes, Linux namespaces, and capability-based security policies isolate untrusted execution without forfeiting the low-latency guarantees required by interactive agent loops.
:::
