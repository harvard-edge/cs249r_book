...
The first result compounds over a trajectory. Let $L_0$ be the tokens present on the first turn and $\Delta$ the tokens a turn appends, a tool call plus its result. An append-only transcript carries $L_t = L_0 + (t-1)\Delta$ tokens on turn $t$, so the input a trajectory of $T$ turns sends in total is

$$C_{\text{in}}(T) = \sum_{t=1}^{T} L_t = T L_0 + \frac{T(T-1)}{2}\,\Delta$$

The second term grows with the square of the horizon. Doubling the number of turns roughly quadruples the input a long trajectory sends, even though each turn adds the same amount. The quadratic term is why input tokens dominate the cost of a trajectory although each output token is priced higher, and why any policy that bounds $L_t$ changes the growth from quadratic to linear in the number of turns. Prefix caching lowers the price of the re-sent part, but only when the re-sent tokens are identical from call to call, which is a layout question taken up in @sec-vol3-context-engineering-staging.
...
# │          @sec-vol3-context-engineering-staging, and the volatile-header
# │          pitfall in @sec-vol3-context-engineering-fallacies.
# │
...
- [ ] **Trajectory input**: Why does the total input of an append-only trajectory grow with the square of its horizon?
...
A summary the model writes about its own progress is a claim, not evidence, so the invariant closure principle (\ref{pri-invariant-closure}) applies to compaction directly. Any candidate record $\mathcal{R}'$ the model proposes is untrusted until the runtime checks it. The stakes are specific. If the runtime accepts a record claiming the tests passed and then discards the raw turns that show they failed, the context no longer contains any evidence of the failure.
...
Context policies fail most often through intuitions carried over from long-document question answering or from memory hierarchies, neither of which has an environment that changes under the reader. The items below are the ones that recur.

::: {.fallacy-pitfall}
**Fallacy:** *A larger context window removes the need to choose what the model sees.*

A larger window raises the limit, not the usable context. Accuracy on evidence placed in the interior of a long input drops well before the window is full [@liu2024lost], every staged token is re-sent and re-paid on every turn, and total input grows with the square of the horizon (@sec-vol3-context-engineering-physics). A large window also holds more stale and poisoned content, not less. Selection remains necessary. A larger window only moves the point at which its absence becomes impossible to ignore.
:::

::: {.fallacy-pitfall}
**Fallacy:** *The context window and the KV cache are two levels of one cache.*

They are different objects with different owners. The context is the sequence of tokens the harness chooses for a call. The KV cache is attention state the serving system derives from that sequence, valid only for an exact prefix (principle \ref{pri-vol3-prefix-coherence}). Nothing moves between them like a cache line between levels. Removing or editing a token in the middle of the context does not leave the later attention state reusable; it invalidates everything after the edit. The harness controls the cache only indirectly, through layout, and @sec-vol3-kv-cache covers how the serving system holds and shares that state.
:::
