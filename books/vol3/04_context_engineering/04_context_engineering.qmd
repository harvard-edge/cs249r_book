...
### Placing volatile metadata in the system message

A timestamp, turn counter, or request identifier in the system message changes the first tokens of every call, so no call ever reuses the previous call's work. In the trajectory priced in \ref{nbk-04-quantitative-impact-of-prefix-layout}, that one placement makes the run `{python} ContextLayoutPrice.cost_mult_str` more expensive and prefills `{python} ContextLayoutPrice.prefill_mult_str` more tokens. The fix is syntactic: keep per-call fields out of the root and place them in the leaf, or omit them altogether.
...
* **Layout sets the price**: Prefill cache hits make the first tokens much cheaper than the last. Placing volatile fields in the root forfeits that discount for the entire context, turning an engineering oversight into a factor of `{python} ContextLayoutPrice.cost_mult_str` on cost.
