...
## The Working-Set Decision {#sec-vol3-context-engineering-decision}

Consider an autonomous software engineering agent tasked with locating and repairing a subtle race condition in a multi-threaded storage engine. During its first twenty iterations, the agent reads configuration manifests, invokes a build system that emits thousands of lines of compiler warnings, executes an integration test harness, and inspects dozens of source files across several directories. By iteration twenty-four, an agent runtime that naively appends every exchange into a single, ever-growing transcript has assembled a prompt exceeding one hundred and eighty thousand tokens. At this juncture, the underlying foundation model begins to exhibit erratic behavior: it hallucinates the signatures of standard library interfaces, re-reads source files it inspected only three turns earlier, and reverts a synchronization fix it had already verified against the unit test suite. When profiled at the serving infrastructure, more than 85 percent of the accelerator compute and memory bandwidth during the prefill phase is consumed processing stale diagnostic logs that possess zero predictive value for the next action, while the model's empirical recall of the core race condition constraints defined in the original specification degrades precipitously.
...
*This chapter closes State by keeping each invocation's context bounded, ordered, and current.*
:::

The State Explosion of @sec-vol3-intro-causal-spine named the problem this chapter solves. A trajectory's history grows with every turn, and the model keeps none of it between invocations, so the runtime must rebuild what the model knows before every call. In the terms of @sec-vol3-intro-hsa, this is the State exposure, and the horizon is what makes it grow. The rebuild raises a question a single model call never faces. When the record of a trajectory is longer than the model can use, the runtime must choose which part of it the next invocation sees, in what order, and for how long each staged fact stays true. Three constraints shape every answer. The model uses far less context than it nominally accepts, so a larger window does not remove the choice. The serving engine rewards a prompt whose opening tokens stay fixed, so layout has a price. The environment keeps changing under the agent, so a staged observation can become false while no token in the prompt changes. The chapter takes these constraints in turn and ends with a way to measure whether a context policy works.

**The Physical Boundary: Decoupling Logical Context from Physical KV Memory.** The choice is made on one side of a boundary between the *host agent runtime*, executing in host memory, and the *neural inference service*, executing on accelerator hardware. The host runtime is a deterministic software supervisor with direct access to operating system resources, local storage, and external network endpoints. The model holds zero ambient authority and retains no state across invocations (@sec-vol3-intro-operational-incident). Each invocation is a discrete function evaluation, in which the accelerator runs forward-pass matrix operations over the input tokens, the static model parameters, and any precomputed key-value tensors, and returns a probability distribution over the vocabulary for the next token.
...
**The Open-Loop Failure Wall.** A runtime that appends every exchange to one transcript meets two failures at once. The first is attentional. A transformer can attend to any token in its window, but it attends unevenly, favoring the head and the tail of the prompt, so the tool responses, constraint updates, and hypotheses that pile up in the middle lose weight as the transcript grows (@sec-vol3-context-engineering-physics derives why). The second is the context poisoning of @sec-vol3-intro-context-poisoning. A flawed hypothesis or a hallucinated variable name from turn four stays in the transcript on every later turn, and the model conditions on it as though the environment had confirmed it. Without runtime filtering, both failures worsen with every turn the trajectory runs.

The systems contrast between passive transcript accumulation and active context governance is illustrated in @fig-vol3-working-set-selection. In the unmanaged trajectory (left panel), staging raw tool dumps and compiler stderr traces causes token volume $L$ to scale exponentially toward the hardware ceiling $S_{\max}$. This trajectory growth collides with three physical barriers: prefill execution latency scales quadratically ($O(L^2)$), pushing Time to First Token (TTFT) from hundreds of milliseconds into multi-second freezes; autoregressive decode saturates accelerator High-Bandwidth Memory (HBM) by streaming gigabytes of KV tensors for every generated token; and self-attention suffers from lost-in-the-middle dispersion that submerges critical instructions beneath background chatter.

To avert this collapse, the host runtime implements active logical working set selection (right panel). The runtime treats the prompt not as a persistent storage medium, but as an actively curated execution frame $\mathcal{W}(t)$ strictly bounded by token budget $M^*$. The working set isolates three functional tiers: the immutable Pinned Root ($[0, L_{\text{root}}]$) containing supervisory policies and tool contracts that guarantee continuous Radix cache hits; the dynamic working window holding immediate turn observations, unified diff hunks, and an ephemeral scratchpad governed by a two-phase commit lease; and an explicit Eviction and Compaction Gate. Rather than discarding historical context, the gate commits superseded turns into an off-context host archival store $\mathcal{H}_{1:t-1}$ (such as a local SQLite database or structured audit log), preserving complete forensic traceability while maintaining bounded inference latency and sharp attention recall on the accelerator.

::: {#fig-vol3-working-set-selection fig-env="figure" fig-pos="htb" fig-cap="**Logical Working Set Selection**: Comparison between unmanaged append-only trajectory growth and active logical working set selection. The left panel shows how raw tool dumps push context length toward the hardware cap $S_{\max}$, causing quadratic prefill latency, memory bandwidth saturation during decode, and attentional dispersion. The right panel shows how the host supervisor enforces a bounded working set $\mathcal{W}(t) \le S_{\max}$ partitioned into a pinned root, active turn observations, ephemeral scratchpads, and an eviction gate to off-context archival storage." fig-alt="Systems diagram contrasting an unmanaged append-only trajectory exhibiting quadratic growth against a bounded logical working set partitioned into pinned root directives, dynamic execution turns, and host archival storage."}
...
The runtime cannot compute $\mathcal{W}(t)$ from the model's own account of its progress, because a self-report carries no evidential weight (@sec-vol3-intro-epistemic-boundary). The host supervisor instead enforces invariant closure (principle \ref{pri-invariant-closure}) from outside the model, with deterministic software checks. The runtime directly captures process exit codes, runs sandboxed regression suites, computes cryptographic hashes over filesystem trees, and parses structured AST diffs. The logical working set $\mathcal{W}(t)$ is subsequently assembled through active transformation: pinning immutable root instructions, refreshing the verified environmental state, distilling verbose compiler outputs down to specific file-line diagnostics, preserving failing assertions as negative constraints, and evicting transient intermediate chatter that has been superseded by concrete environmental feedback.
...
## Working Set Capacity {#sec-vol3-context-engineering-physics}
...
The *usable working context* $M_{\text{eff}}$, by contrast, denotes the maximum sequence length over which the model can reliably attend to, retrieve, and synthesize disparate pieces of evidence to satisfy task invariants. While $S_{\max}$ is a static design constant reported on a specification sheet, $M_{\text{eff}}$ is an empirical, workload-dependent variable that is invariably smaller than $S_{\max}$. The divergence between these two metrics stems from the fundamental difference between single-needle retrieval and the complex, multi-hop dependency reasoning demanded by agentic tasks.

::: {#dfn-usable-working-context .callout-definition title="Usable working context"}
The maximum input sequence length over which an autoregressive model reliably retrieves, integrates, and reasons across multi-hop operational evidence, which is strictly smaller than the model's architectural nominal context capacity ($M_{\text{eff}} \ll S_{\max}$).
:::
...
::: {.column-margin}
![](images/svg/vol3_working_sets_margin_002.svg){width="100%" fig-alt="Two-endpoint sparkline showing target attention probability plummeting from 97 percent in a 100-token prompt down to 3 percent in a 100,000-token prompt due to softmax denominator dilution."}

*Accumulating 100k background tokens dilutes the softmax denominator, slashing target attention from 97 percent to 3 percent.*
:::

The inability of a transformer to utilize its full nominal context uniformly is not a transient training quirk; it is an intrinsic structural consequence of how unprivileged self-attention routes information across long sequences. In an extensive empirical study of long-context language model behavior, @liu2024lost demonstrated that evidence retrieval performance is heavily dependent on the physical location of relevant tokens within the prompt.

The physical and mathematical foundations of this position-dependent retrieval failure are detailed in @fig-vol3-lost-in-middle across three complementary mechanisms. In Panel A, the softmax denominator normalizer enforces strict conservation of attention mass across the sequence. In a compact 100-token prompt with a target logit of $z=6$ and background noise logits of $z=-2$, the target token commands $96.8\%$ of the total attention distribution. However, when an unmanaged execution transcript reaches 100,000 tokens, the cumulative exponential sum of weakly correlated background distractors surges to $13{,}903.4$, diluting target attention mass to just $2.9\%$. This 33-fold signal attenuation prevents downstream Feed-Forward Network (FFN) layers from activating the appropriate tool dispatch pathways.

Panel B demonstrates the compounding effect of Rotary Position Embeddings (RoPE). Because RoPE rotates key and query representations across orthogonal 2D sub-planes at exponentially increasing frequencies, relative token offset $\Delta = |m - n|$ governs attentional coherence. Over local horizons ($\Delta < 1{,}024$ tokens), inner products exhibit constructive alignment; at long sequence distances, high-frequency rotational components interfere destructively, causing expected attention weights $\mathbb{E}[\mathbf{q}^T R(0, \Delta) \mathbf{k}]$ to decay along an inverse polynomial envelope $\Delta^{-\gamma}$.

Panel C illustrates the resulting empirical retrieval curve across prompt depth. While models exhibit near-perfect retrieval fidelity in the primacy zone ($0\% - 20\%$ depth) due to initial attention sinks, and in the recency zone ($80\% - 100\%$ depth) due to local RoPE coherence, the intermediate $20\% - 80\%$ sequence range forms a pronounced 'attenuation trough' where fact retrieval recall collapses to approximately $30\%$. For an autonomous agent, staging uncompacted build logs, raw source code, or historical tool outputs within this middle zone creates a critical vulnerability: the unprivileged model routinely overlooks operational constraints staged directly in its prompt.

::: {#fig-vol3-lost-in-middle fig-env="figure" fig-pos="htb" fig-cap="**Lost-in-the-Middle Attention Degradation**: Mathematical mechanisms and empirical evidence retrieval accuracy across relative prompt depth. The left panel illustrates softmax denominator dilution where 100,000 background tokens reduce target attention mass from 96.8 percent to 2.9 percent. The center panel shows RoPE harmonic phase cancellation decaying as an inverse polynomial $\Delta^{-\gamma}$ across relative token distance $\Delta$. The right panel depicts the resulting empirical U-shaped recall curve, highlighting the severe attenuation trough spanning 20 to 80 percent prompt depth." fig-alt="Three-panel diagram showing softmax denominator dilution calculations, RoPE harmonic phase decay curves, and the empirical U-shaped fact retrieval accuracy curve across context depth."}
![](images/svg/lost_in_the_middle_rope_decay.svg)
:::

When critical task evidence is located at the very beginning of the context (the primacy zone) or at the very end of the sequence immediately preceding the generation boundary (the recency zone), models retrieve and utilize that evidence with high fidelity. When that identical evidence is placed in the interior of the prompt—typically spanning the 20 percent to 80 percent relative context depth—retrieval accuracy drops precipitously, frequently plummeting by 20 to 50 percentage points on complex reasoning benchmarks.
...
Second, the recency effect is governed by the locality of causal attention and positional encodings. Tokens situated near the trailing boundary of the prompt have had fewer intermediate transformation steps between their representations and the final output logits. To formalize the competition for attention mass across expanding sequences, consider a query token $\mathbf{q}$ attempting to attend to an authoritative target key token $\mathbf{k}_{\text{target}}$ (such as `worker_db_port = 5432`) embedded amidst $K$ semantically correlated distractor key tokens $\{\mathbf{k}_1, \dots, \mathbf{k}_K\}$ (such as configuration ports for metric collectors, legacy endpoints, or cache proxies). Under the softmax normalizer, the attention weight allocated to the target token is governed by:
...
As the number of staged distractor tokens $K$ scales into the tens of thousands, the background summation $\sum_{j=1}^K \exp\left(\frac{\mathbf{q} \cdot \mathbf{k}_j}{\sqrt{d_k}}\right)$ dominates the denominator, systematically diluting $\alpha_{\text{target}}$ toward zero even when the individual dot-product alignment with each distractor is modest.
...
::: {.column-margin}
![](images/svg/vol3_working_sets_margin_001.svg){width="100%" fig-alt="A curve rising quadratically to cross a linear projection baseline at 90k tokens, with the region beyond 90k shaded red to mark the quadratic prefill compute explosion."}

*Above 90k tokens, quadratic self-attention overtakes linear projections, dominating prefill latency.*
:::
...
When the sequence length $M$ is small relative to the hidden dimension $d$, linear projections dominate total execution time. But as an agent runtime dumps larger working sets into the prompt, a critical crossover occurs. For a model with hidden dimension $d = 4096$, when the sequence length exceeds $M = 6 d = 24{,}576$ tokens, the quadratic self-attention operations consume more floating-point operations than all feed-forward networks and linear projections combined. At $M = 65{,}536$ tokens, attention FLOPs represent more than 72 percent of the entire prefill workload.
...
The systems impact of massive prefill sequences extends beyond the single agent's TTFT. In production inference clusters hosting multiple concurrent agents, monolithic long-context prefills introduce severe *head-of-line blocking* and *inter-token latency (ITL) bubbles*.

When an agent runtime dispatches an uncompacted 65,536-token prompt, the inference engine launches dense GEMM kernels that monopolize the GPU's Tensor Cores for seconds (over $6.26\text{ s}$ as quantified in \ref{nbk-04-prefill-compute-scaling-and-latency}). During this prolonged execution, any concurrent agent request running in the memory-bandwidth-bound autoregressive decode phase is completely preempted or delayed, causing its token generation latency to spike by orders of magnitude (from $20\text{ ms}$ up to several seconds per token).

To mitigate this multi-tenant interference, modern serving runtimes (such as Sarathi-Serve and vLLM) implement **Chunked Prefill**:

- The serving engine partitions the $M$-token prompt into discrete chunks of fixed token budget $C_{\text{chunk}} \in [512, 2048]$ tokens.
- Each prefill chunk is co-scheduled in a batch alongside active decode tokens, co-locating compute-bound GEMMs alongside memory-bound GEMVs to achieve high Tensor Core utilization without starvation.
- While chunked prefill successfully bounds ITL spikes for decoding streams, it stretches the wall-clock completion of the agent's prefill across $\lceil M / C_{\text{chunk}} \rceil$ serving iterations, reinforcing why active context compaction at the host runtime level is an indispensable prerequisite for cluster goodput.

::: {#nbk-04-prefill-compute-scaling-and-latency .callout-notebook title="Prefill compute scaling and latency on accelerator hardware"}
...
**Systems Takeaway:**
Expanding the context window by a factor of 8 ($8\text{k} \to 64\text{k}$) increases total prefill compute by a factor of 22 ($140.7\text{ TFLOPs} \to 3{,}096.2\text{ TFLOPs}$). The accelerator shifts from a linear-projection-dominated workload (where attention accounts for only 25 percent of FLOPs) into an attention-dominated quadratic regime (where attention accounts for nearly 73 percent of FLOPs). For an agent executing a 30-turn interactive debugging loop, Policy B introduces over three minutes ($30 \times 6.26\text{ s} \approx 188\text{ s}$) of pure prefill pipeline latency, compared to under nine seconds for Policy A, while consuming 22 times more energy on the accelerator cluster.
:::

To break this quadratic scaling wall and compress the memory footprint of long context sequences, frontier model architectures move beyond Grouped-Query Attention to low-rank latent attention projections, as illustrated in @fig-vol3-multi-head-latent-attention.

As detailed in Panel A of @fig-vol3-multi-head-latent-attention, conventional Multi-Head Attention (MHA) allocates independent key and value heads for each of its 64 attention heads, consuming 2,560 bytes per token and requiring 43 GB of device High-Bandwidth Memory (HBM) to maintain a single 128k context stream on a 70B model. While Grouped-Query Attention (GQA) reduces this footprint to 320 bytes per token by sharing 8 KV heads, Multi-Head Latent Attention (MLA) delivers a 38-fold compression over MHA down to 67.5 bytes per token. At 128k context, this reduces the per-stream cache allocation to 9.06 GB, eliminating out-of-memory faults on 80 GB accelerator nodes and multiplying serving concurrency by nearly fivefold.

The architectural mechanism enabling this compression is shown in Panel B. Rather than caching full key and value tensors, MLA down-projects the input activation $\mathbf{x}_t \in \mathbb{R}^{8192}$ into a compact latent vector $\mathbf{c}_t^{KV} \in \mathbb{R}^{512}$ via down-projection matrix $W_{DKV}$. Because applying Rotary Position Embeddings (RoPE) directly to a compressed latent vector shatters rotational commutativity with the downstream up-projection matrix ($R(\theta, m) W_{UK} \neq W_{UK} R(\theta, m)$), MLA isolates positional encoding into a decoupled, low-dimensional key $\mathbf{k}_t^R \in \mathbb{R}^{64}$. Accelerator HBM stores exclusively the compressed 512-dimensional latent and the 64-dimensional positional key.

Panel C illustrates the matrix associativity trick that accelerates autoregressive decode. Under standard attention, dot products require expanding latents back into high-dimensional keys $\mathbf{k}_i = \mathbf{c}_i^{KV} W_{UK}$. MLA exploits associativity by absorbing the up-projection matrix directly into the active query token once within on-chip SRAM ($\mathbf{q}_{\text{latent}} = \mathbf{q}_t W_{UK}^T \in \mathbb{R}^{512}$). During decode, the processor memory bus streams only 512-dimensional latents from HBM, and tensor cores evaluate dot products directly against compressed representations in SRAM. This reordering slashes HBM bandwidth traffic by $4.74\times$ while completely avoiding the materialization of high-dimensional key-value activations in device memory.

::: {#fig-vol3-multi-head-latent-attention fig-env="figure" fig-pos="htb" fig-cap="**Multi-Head Latent Attention (MLA) Architecture and Cache Compression**: Hardware footprint and algorithmic mechanics of low-rank latent KV cache compression. The left panel compares per-token KV cache memory footprints between Multi-Head Attention (2,560 B/tok), Grouped-Query Attention (320 B/tok), and MLA (67.5 B/tok), illustrating serving density gains at 128k context. The center panel outlines the low-rank down-projection into a 512-dimensional latent vector and decoupled 64-dimensional RoPE key. The right panel details the matrix associativity trick in autoregressive decode, where queries absorb projection weights in SRAM to compute attention without expanding high-dimensional keys and values in HBM." fig-alt="Three-panel systems schematic illustrating KV cache memory footprints across attention variants, latent down-projection architectures with decoupled RoPE, and the matrix associativity decoding pipeline."}
![](images/svg/multi_head_latent_attention_mla.svg)
:::

### Workload-driven sizing

::: {.column-margin}
![](images/svg/vol3_working_sets_margin_004.svg){width="100%" fig-alt="Four horizontal bars on a logarithmic scale, longest to shortest, labelled MHA, GQA, MLA and MQA."}

*Latent attention cuts per-token KV from 2,560 B to 67.5 B, a $38\times$ reduction.*
:::
...
This operational dynamic is formalized in @fig-vol3-working-set-frontier, which maps empirical task completion (Pass@1) against nominal context sequence length $M$. Rather than exhibiting monotonic improvements with expanded context windows, autonomous agent performance partitions into three distinct systems regimes:

::: {#fig-vol3-working-set-frontier fig-env="figure" fig-pos="htb" fig-cap="**The Effective Working-Set Frontier and Three Operational Regimes**: Empirical task success (Pass@1) across context sequence length $M$. In Phase I ($M < M_{\min}$), missing interfaces trigger severe hallucination. In Phase II ($M_{\min} \le M \le M^*$), the working set supplies sufficient evidence while preserving sub-second prefill latency, achieving peak task completion at frontier $M^*$. In Phase III ($M > M^*$), distractor interference, softmax dilution, and quadratic prefill compute ($\mathcal{O}(M^2)$) induce negative marginal returns and execution freezes." fig-alt="Quantitative curve showing empirical task success rising through Phase I information deficit to peak at the optimal frontier M-star in Phase II, before declining through Phase III attentional saturation."}
![](images/svg/working_set_frontier_phases.svg)
:::

As visually demarcated by the three shaded operating envelopes in @fig-vol3-working-set-frontier, the systems behavior changes fundamentally across sequence boundaries:
...
The primary duty of the host supervisor's memory management subsystem is to enforce policies that keep the logical working set strictly bounded within Phase II. The runtime must not rely on the unprivileged model to self-filter irrelevant text, nor can it assume that a larger context window will automatically improve agentic autonomy. Instead, the runtime measures the trade-off curve on its own workload and holds the working set at $M^*$, as the principle that context is a selected working set requires (principle \ref{pri-vol3-attention-working-set}).
...
## Staging the Next Invocation {#sec-vol3-context-engineering-staging}
...
Because transformers process instructions and data within the same unified attention mechanism, control-plane signals and data-plane payloads share an in-band channel. Without structural framing enforced by the supervisor, data tokens can mimic control tokens and act as instructions.
:::

### The three-zone context hierarchy

To prevent authority collapse and maintain layout stability, the host runtime partitions the logical working set into three functional zones ordered by their mutability and authority: the *Root*, the *Trunk*, and the *Leaf*. This layout, illustrated schematically in @fig-vol3-context-zones, separates permanent supervisory policies from slowly mutating project artifacts and rapidly turning execution observations, while aligning boundary transitions with physical prefix-caching hardware boundaries. The zones carry authority tiers, which rank how far the runtime trusts a zone's tokens as instructions to the model. They are distinct from the authority levels $A_0$ to $A_3$ of @sec-vol3-intro-hsa, which rank what a task may do to the world.

::: {#fig-vol3-context-zones fig-env="figure" fig-pos="htb" fig-cap="**The Three-Zone Logical Context Hierarchy**: Memory layout and attention salience across prompt sequence boundaries. The upper bar partitions the working set into the immutable Root Zone $[0, L_{\text{root}}]$ (Tier 0 supervisor invariants anchored at the Radix cache boundary), the Trunk Zone $[L_{\text{root}}, L_{\text{leaf}}]$ (Tier 1 verified workspace state and compaction target), and the Leaf Zone $[L_{\text{leaf}}, M^*]$ (Tier 2 ephemeral observations and scratchpad tokens). The lower curve illustrates the corresponding attention logit distribution, mapping attention sinks to the root, the attenuation trough to the trunk, and recency bias to the generation frontier." fig-alt="Two-panel systems diagram illustrating a three-zone logical context memory layout aligned with sequence indices 0, L-root, L-leaf, and M-star, matched with an attention logit distribution curve."}
![](images/svg/context_staging_layout.svg)
:::

As mapped across sequence boundaries in @fig-vol3-context-zones, each zone is allocated an explicit index range that aligns directly with the causal attention salience curve shown in the lower panel:

The *Root* zone establishes the immutable task contract. It occupies the absolute beginning of the sequence, spanning token indices $0$ through $L_{\text{root}}-1$. The Root contains system policies, safety invariants, operational constraints, and the canonical definitions of all available tool interfaces. Because the model holds zero ambient authority, these tokens are what define the boundary of its sandbox, and they carry the highest authority tier (Tier 0). The Root is strictly read-only throughout the lifetime of the agent's task trajectory; its contents do not mutate between invocations. By enforcing an identical token prefix across every turn, the runtime aligns the Root with the inference engine's prompt cache boundary, ensuring that attention activations for $[0, L_{\text{root}}]$ are computed once and reused across subsequent turns via Radix tree lookups. Furthermore, anchoring these tokens at the sequence head leverages initial attention sinks to prevent supervisory policy amnesia.

The *Trunk* zone anchors the active workspace state and environmental artifacts, spanning token indices $L_{\text{root}}$ through $L_{\text{leaf}}-1$. The Trunk contains the validated plan directed acyclic graph (DAG), the list of verified subgoals, and the primary files currently under inspection or modification. Tokens within the Trunk carry intermediate authority (Tier 1) because they are copies of verified application state, read deterministically from the file system or confirmed by the user. Like any staged copy, they go stale when their source is written (@sec-vol3-context-engineering-context-rot). The Trunk mutates infrequently—typically only when a subgoal transitions to completed status, when a file is rewritten, or when a new dependency is introduced into the workspace. Because the Trunk occupies the interior sequence range where attention experiences severe lost-in-the-middle attenuation, the runtime targets this zone for structured AST skeletonization and lossless diff extraction.

The *Leaf* zone captures dynamic execution observations and the ephemeral scratchpad, occupying the remainder of the prompt from $L_{\text{leaf}}$ to the working set boundary $M^*$. The Leaf contains the latest command outputs, process exit codes, compiler diagnostics, and candidate reasoning traces generated during the immediately preceding turn. Because these tokens reflect raw outputs from the external environment—which may be noisy, malformed, or adversarial—they occupy the lowest authority tier (Tier 2). The Leaf exhibits continuous turnover: its contents are appended, truncated, or replaced on every single invocation cycle. Situated at the prompt tail immediately adjacent to the generation frontier $q_T$, the Leaf exploits strong recency bias and local RoPE coherence to ground next-step action generation in immediate environmental feedback (@tbl-vol3-context-zone-stratification).
...
: **Working-Memory Context Zones and Authority Stratification**: Authority tiers, mutation frequencies, token allocations, and invalidation triggers across context zones. {#tbl-vol3-context-zone-stratification}
...
2. *The Artifact Channel:* Staged copies of verified file system contents, structural syntax trees, and completed plan nodes. Emitted exclusively in the Trunk zone.
...
Because cached attention state is reusable only up to the first token that differs (principle \ref{pri-vol3-prefix-coherence}), the runtime stages tokens in order of *ascending volatility*. The most static tokens must occupy the lowest index positions, while the most volatile tokens must be relegated to the end of the sequence.
...
::: {#nbk-04-quantitative-impact-of-prefix-layout .callout-notebook title="Quantitative impact of prefix layout on prefill amortization"}
...
Because token index $0$ mutates on every invocation, the prefix match length is $k = 0$ for all $20$ turns. In accordance with the foundation engine Roofline model established in @sec-vol3-foundation-model-cost, the prefill engine processes all $L = 32{,}768$ tokens on every turn without cache reuse:
$$\text{FLOPs per turn} = 2 \cdot P \cdot L = 2 \cdot (70 \times 10^9) \cdot 32{,}768 \approx 4.587 \times 10^{15}\text{ FLOPs}$$
$$\text{Total Trajectory Prefill FLOPs} = 20 \cdot 4.587 \times 10^{15} \approx 9.175 \times 10^{16}\text{ FLOPs}$$

On an accelerator serving system delivering an effective sustained prefill throughput of $\Pi_{\text{prefill}} = 150\text{ TFLOPs/s}$ ($1.5 \times 10^{14}\text{ FLOPs/s}$), the prefill duration per turn evaluates to:
...
The benefits of layout stability extend directly to the commercial financial accounting of cloud-hosted agent fleets. Leading frontier API providers (such as Anthropic, OpenAI, and DeepSeek) charge tiered pricing based on prefix cache hits:

- **Cache-Hit Pricing Invariant:** Cached prompt tokens are billed at a $75\%$ to $90\%$ discount relative to uncached prefill tokens (e.g., $\$0.30$ per million tokens for cached reads versus $\$3.00$ per million tokens for base prefill).
- **Trajectory Financial Waste:** In an interactive 20-turn agent session averaging 30,000 tokens of context, an unstable layout that busts the cache on every turn costs $\$1.80$ in input token expenditure. Enforcing prefix stability reduces the same session cost to under $\$0.25$, yielding an immediate $7.2\times$ cost reduction. Across a fleet executing 100,000 tasks per month, prefix stability prevents over $\$155,000$ in unnecessary API spending.

However, realizing these financial and latency savings requires overcoming the **Token-Boundary Alignment Pitfall**:

Modern Byte-Pair Encoding (BPE) tokenizers operate greedily across whitespace and punctuation boundaries. When the host runtime constructs a prompt by dynamically concatenating static string blocks with dynamic state, subtle boundary variations mutate token IDs. For instance:

- Staging a static prefix ending with `"\nSupervisor:"` and a dynamic block beginning with `" Execute"` produces token IDs `[..., 1845, 3422]`.
- But if the runtime omits the leading space and concatenates `"\nSupervisor:"` with `"Execute"`, the tokenizer merges the colon and the initial letter into a completely different token ID `[..., 1042]`.

Because physical serving engines (such as vLLM or SGLang) index Radix tree KV cache nodes strictly by exact sequences of *token IDs*, a single mutated boundary token ID at position $k$ invalidates the cache match for that token and all downstream tokens. The engine fails to match the prefix, forcing a full, expensive prefill recomputation. Robust agent runtimes prevent this by tokenizing static prompt segments independently or enforcing strict, verified token-boundary separators.

::: {#chk-04-context-hierarchy .callout-checkpoint title="Evaluating working context staging and prefix stability"}
Before analyzing context compaction algorithms, verify your understanding of context zones and cache alignment:

- [ ] **Three-zone layout**: How does partitioning prompt memory into static system instructions, immutable history, and ephemeral scratchpad prevent attention dilution?
- [ ] **Prefix stability**: Why does mutating a single whitespace character or dynamic timestamp in the system prefix invalidate KV cache reuse across subsequent trajectory turns?
- [ ] **Token-boundary alignment**: How do BPE merge rules across segment boundaries bust Radix tree cache matches in serving engines like vLLM and SGLang?
- [ ] **Working-set sizing**: Why does the usable context capacity $M^*$ diverge from the model's nominal architectural window $M_{\max}$ under multi-turn retrieval?
:::

## Context Compaction {#sec-vol3-context-engineering-compaction}

::: {.column-margin}
![](images/svg/vol3_working_sets_margin_003.svg){width="100%" fig-alt="A budget comparison bar diagram showing 57.6 gigabytes of unpruned reasoning scratchpad memory overflowing the 10 gigabyte GPU limit, compared to 0.96 gigabytes under the Two-Phase Commit protocol."}

*Two-Phase Commit purges ephemeral scratchpads post-action, slashing 30-turn reasoning memory from 57.6 GB to 0.96 GB.*
:::
...
When lossless filtering cannot reclaim sufficient headroom, the runtime transitions to *structured extraction*. Unprivileged language models should never be tasked with parsing multi-megabyte build logs when deterministic host parsers can isolate errors with zero variance and zero token cost. Compilers and test harnesses—such as GCC, Clang, Rustc, Pytest, and Cargo—emit outputs where more than 95 percent of the log volume consists of informational progress updates, timing metrics, and passing test cases.

The mechanical operation of this filtering cascade is illustrated in @fig-vol3-compaction-pipeline across four disciplined transformation stages:

1. *Stage 1 (Raw Trajectory Ingestion):* Unfiltered execution traces accumulate raw stdout/stderr stack traces, redundant multi-thousand-line source file reads, and conversational chatter. Over extended trajectories, this unmanaged buffer exhausts context capacity (128,000 tokens in the example shown), diluting the usable signal below 2 percent and inducing quadratic prefill stalls.
2. *Stage 2 (Deterministic Lossless Pruning):* The supervisor routes raw logs through deterministic host preprocessors. A head/tail log slicer retains the initial 25 initialization lines, matches failure markers via regular expressions (`FAIL|ERROR|Panic`), and preserves the trailing 25 exit lines, eliding thousands of passing lines. Simultaneously, a unified diff extractor replaces full file reads with compact unified patches (`git diff -U3`), reducing a 2,000-line source payload to a 12-line hunk. This deterministic pass reclaims 81 percent of the buffer (down to 24,000 tokens) in under 5 milliseconds of host CPU time, without consuming a single GPU FLOP or discarding actionable error information.
3. *Stage 3 (Structured AST Skeletonization):* For source code that must be retained in context, the runtime executes grammar-aware symbol pruning using a Tree-sitter AST parser. The parser strips function and method implementation bodies while retaining module docstrings, class hierarchies, and typed public API signatures (`def acquire(...) -> Conn:`). This drops the token footprint by 95 percent (down to 6,000 tokens) while preserving complete structural type integrity, allowing the model to reason about system interfaces without drowning in implementation details.
4. *Stage 4 (State Ledger & Invariant Enclosure):* In the final stage, the runtime synthesizes operational progress into a structured key-value state ledger recording the active epoch, confirmed hypotheses, modified files, and passing assertion ratios. Crucially, the supervisor encapsulates immutable task invariants ($I_{\text{spec}}$) in an isolated enclosure protected from drift. The resulting working set footprint stabilizes at 1,200 tokens—a 106.7-fold reduction from the raw transcript—delivering complete immunity against stale-state accumulation.

::: {#fig-vol3-compaction-pipeline fig-env="figure" fig-pos="htb" fig-cap="**Multi-Stage Context Compaction Pipeline**: Cascading four-stage context transformation from raw trajectory telemetry to verified state ledgers. Stage 1 ingests verbose diagnostic logs and full file reads totaling 128,000 tokens. Stage 2 applies deterministic lossless regex and unified diff extraction, slashing footprint by 81 percent (down to 24,000 tokens) with zero GPU overhead. Stage 3 performs Tree-sitter AST symbol skeletonization, pruning implementation bodies to achieve a 95 percent reduction (down to 6,000 tokens). Stage 4 commits verified subgoals and immutable invariant enclosures into a compact 1,200-token state ledger, delivering a 106.7-fold global compaction." fig-alt="Four-column systems pipeline diagram showing the progression from raw trajectory ingestion through deterministic pruning, AST skeletonization, and state ledger enclosure, with token counts at each stage."}
...
Beyond compiler diagnostics, structured extraction applies syntax-aware code compaction to repository files staged in the Trunk zone. When an agent queries a large source module to inspect a single class or method, staging the entire raw file inflates the working set and invites distractor interference. Rather than relying on naive text clipping (which severs syntax trees and induces hallucinated interface completions), modern runtime supervisors deploy **Tree-sitter** or **Language Server Protocol (LSP)** parsers:

- **AST Outline Folding:** The parser builds a concrete syntax tree, preserving module docstrings, type definitions, class structures, and exported function signatures, while folding the implementation blocks of non-target routines (`def handle_handshake(sock: Socket) -> Session: /* folded 64 lines */`).
- **Syntactic Validity Invariant:** Because the compacted representation is generated from the grammar's AST, the prompt retains valid language syntax and exact type signatures, allowing the foundation model to reason over system interfaces without consuming thousands of tokens on irrelevant implementation details.

@Tbl-compaction-spectrum details the performance characteristics, typical reduction ratios, and information-preservation guarantees across the three compaction tiers.
...
***Context compaction***\index{Context compaction!definition}\index{Working sets!compaction definition} is an entropy-reduction transformation $\mathcal{C}: \mathcal{S}_{\text{ctx}} \to \mathcal{S}_{\text{ctx}}'$ executed across the context hierarchy to bound active token memory under a length ceiling $L_{\max}$ while preserving the invariant verbatim set $\mathcal{V}(O_t) \subseteq \mathcal{C}(O_t)$.

1.  **Significance**: Prevents KV-cache memory exhaustion and quadratic attention latency degradation across long-horizon trajectories while preserving the critical causal chain required for task completion.
2.  **Distinction**: Unlike naive rolling-window eviction (FIFO truncation, which indiscriminately drops foundational system prompts or early architectural decisions), structured compaction orders transformations by escalating semantic risk from lossless filtering to structured AST extraction and lossy semantic summarization.
3.  **Common pitfall**: Summarizing away negative execution evidence (such as compiler stack traces or failed test assertions), causing the agent to lose track of falsified hypotheses and fall into repetitive, looping action proposals.
...
> **Naive Lossy Summary (Secondary Model — Violation of Invariant):**
...
By preserving the exact file path (`services/auth_token.py`), the line number (`142`), the failing expression, and the local variable assignments verbatim, the compacted frame uses fewer than 70 tokens while preserving 100 percent of the actionable signal required for the model to synthesize the repair on its next forward pass.
...
::: {#nbk-04-token-budget-compaction-and-roofline .callout-notebook title="Token budget compaction and Roofline prefill savings"}
...
2. The reduction in prefill computational FLOPs and estimated prefill latency, assuming an effective model FLOPs utilization (MFU) of 50 percent ($494.5\text{ TFLOPS}$ sustained).
...
## Context Checkpointing {#sec-vol3-context-engineering-summarization}
...
By structuring the checkpoint into distinct, typed partitions, the runtime supervisor enforces a strict separation between unalterable constraints and speculative working hypotheses. When token budget exhaustion forces the eviction of the preceding conversation history, the structured record provides a dense, verified kernel that maintains operational continuity without carrying forward thousands of tokens of ephemeral tool chatter.

### Pre-eviction verification

The invariant closure principle (\ref{pri-invariant-closure}) applies to compaction in its end-to-end form. A summary the model writes about its own progress is a claim, not evidence of correctness, so any checkpoint candidate $\mathcal{S}'_{\text{checkpoint}}$ emitted by the model during context compaction is an untrusted proposal. If the host supervisor naively accepts a hallucinated summary asserting that a test suite passed when the raw event trace contains a fatal assertion failure, deallocating the raw context destroys the only evidence of that failure, permanently corrupting the agent's state trajectory.
...
::: {#nbk-04-quantitative-checkpoint-auditing-and-prefill .callout-notebook title="Quantitative checkpoint auditing and prefill reclaim"}
...
::: {#chk-04-context-compaction .callout-checkpoint title="Evaluating context compaction and checkpointing"}
Before examining environmental dependency tracking and cache invalidation, verify your understanding of compaction invariants:

- [ ] **Information preservation**: Which environmental state components must be retained as structured milestone frames rather than lossy narrative prose?
- [ ] **Invariant erosion**: How does memory anchoring prevent negative constraints from decaying during repeated recursive summarization passes?
- [ ] **Pre-eviction verification**: What validation checks must succeed before purging raw tool observations from the active working set?
- [ ] **Scratchpad lifecycle**: Why does purging ephemeral chain-of-thought tokens post-action slash trajectory memory overhead without impairing subsequent steps?
:::

## Context Invalidation {#sec-vol3-context-engineering-context-rot}
...
This dependency structure is formalized in the bipartite tracking architecture depicted in @fig-vol3-invalidation-graph. The architecture separates authoritative environmental ground truth $\Omega_{\text{env}}$ (left partition) from the unprivileged model's receptive field $S$ (right partition), mediated by the host supervisor's invalidation engine (center column):

1. *Authoritative Environmental Entities ($\mathcal{E} \subset \Omega_{\text{env}}$):* Each tracked physical resource is represented as an authoritative node tagged with unique URI locators and cryptographic version tuples. Entity $\mathcal{E}_1$ models a local source file (`file:///repo/src/network/buffer.c`) with its filesystem inode, nanosecond modification timestamp (`mtime`), and BLAKE3 content hash. Entity $\mathcal{E}_2$ models a build target depending on $\mathcal{E}_1$, while $\mathcal{E}_3$ and $\mathcal{E}_4$ track active OS processes and directory hierarchies.
2. *Staged Logical Working Set ($\mathcal{O} \subset S$):* When observations are staged in the prompt, they occupy explicit token spans $[L_i, L_j]$. Rather than treating these spans as static text, the runtime registers directed edges connecting environmental entities to their corresponding staged observations via the binding function $\lambda(o_i) = \langle \text{URI}, \text{EntityKind}, \mathbf{v}_{\text{env}}, \tau_{\text{read}}, H_{\text{content}} \rangle$. Observation $o_1$ maps to source file $\mathcal{E}_1$, while observation $o_2$ represents a derived diagnostic with incoming edges from both $\mathcal{E}_1$ and $\mathcal{E}_2$.
3. *Mediated Write-After-Read (WAR) Invalidation:* When the foundation model issues a mutating tool call—such as executing a patch against `buffer.c`—the host supervisor intercepts the operation. Upon applying the edit to disk, the runtime observes that $H_{\text{content}}'(\mathcal{E}_1) \neq H(o_1)$. The invalidation engine traverses the bipartite edges originating from $\mathcal{E}_1$: observation $o_1$ is immediately flagged as `STALE`, replacing the obsolete code span with a semantic tombstone or triggering a just-in-time refresh before the next prefill pass. Simultaneously, downstream diagnostic $o_2$ is marked `DIRTY` due to the upstream dependency mutation. Independent elements, such as test runner status $o_3$ and directory listing $o_4$, remain valid as long as their cryptographic hashes and time-to-live ($\tau_{\text{ttl}}$) constraints are satisfied.

::: {#fig-vol3-invalidation-graph fig-env="figure" fig-pos="htb" fig-cap="**Environmental Dependency Tracking Graph**: Directed bipartite graph mapping authoritative environmental entities to staged prompt observation elements. The left partition models physical ground truth ($\Omega_{\text{env}}$), including source files, build targets, processes, and directory trees. The center column shows the host runtime mediator intercepting Write-After-Read (WAR) tool actions, cryptographic hash mutations, and TTL timers. The right partition tracks staged prompt spans ($\mathcal{O} \subset S$), illustrating how modifying a source file invalidates bound code spans into stale tombstones and marks derived build diagnostics as dirty." fig-alt="Bipartite graph diagram showing directed dependency edges from environmental entities on the left through a runtime mediator in the center to staged prompt spans on the right, highlighting invalidation cascading."}
...
Once the delta exceeds $\tau_{\text{ttl}}$, the runtime marks the observation stale, stripping it of execution authority regardless of whether an explicit mutation event was intercepted. Age is the fallback for transient sources like these, which expose no version the runtime can compare. Wherever a source does expose one, the version decides staleness, and an observation whose recorded version no longer matches its source is stale however recent it is.
...
Once an observation $o_k$ is flagged as stale, the host runtime must resolve the invalidation before passing the prompt sequence to the tokenizer. Simply leaving the stale text in place with an appended natural-language warning (e.g., *"Note: the file above was modified"*) is notoriously unreliable; deep self-attention layers routinely attend to the detailed, highly specific tokens of the stale code rather than the abstract warning footer. The runtime must execute a structural remediation policy, arbitrating between the trade-offs summarized in @tbl-vol3-invalidation-policies.

| Resolution Strategy       | Operational Mechanism                                                       | Prefix Cache Impact                                                        | Attention Distraction Risk                                               | Token Overhead                                          | Primary Failure Mode                                          |
|:--------------------------|:----------------------------------------------------------------------------|:---------------------------------------------------------------------------|:-------------------------------------------------------------------------|:--------------------------------------------------------|:--------------------------------------------------------------|
| **Hard Eviction (Purge)** | Excises historical observation tokens completely from prior turns           | Catastrophic invalidation (busts downstream KV cache from eviction index)  | Zero (stale tokens completely removed from prompt)                       | Negative ($\Delta \text{Tokens} < 0$, reclaims context) | Massive prefill recomputation latency on accelerator cluster  |
| **Semantic Tombstoning**  | Replaces payload with compact structural marker referencing mutating action | High hit rate when appended at prompt tail; busts cache if edited in-place | Low-to-Moderate (model must attend to tombstone to suppress prior state) | Minimal ($10 - 25$ tokens per tombstone record)         | Attentional leakage if model ignores tombstone directive      |
| **Just-In-Time Refresh**  | Transparently invokes fresh tool read to rehydrate entity before prefill    | Preserves prefix cache if refreshed observation matches prior length/slot  | Zero (active context matches physical filesystem reality)                | Neutral (replaces old payload with fresh payload)       | Tool latency tax; potential cascading dependency invalidation |

: **Context Invalidation Resolution Policies**: Architectural trade-offs across eviction, tombstoning, and refresh strategies. {#tbl-vol3-invalidation-policies}

The runtime selects among these three invalidation resolution strategies based on token cost, prefix-cache stability, and the semantic necessity of the underlying data:

1. **Hard Context Eviction (Purge):** The runtime excises the stale observation $o_k$ entirely from the message history, removing both the tool invocation and its returned payload. Hard eviction is clean, but it alters the token sequence of earlier turns, and cached attention state survives only up to the first changed token (principle \ref{pri-vol3-prefix-coherence}). As established in @sec-vol3-context-engineering-staging, modifying historical tokens destroys prefix-cache sharing in runtimes utilizing RadixAttention or PagedAttention. If the runtime deletes 500 tokens from Turn 2 of an 18-turn trajectory, the inference engine cannot reuse the cached Key-Value (KV) tensors for Turns 3 through 18, forcing an expensive, full-context prefill phase.
...
::: {#psp-04-the-cache-coherence-dilemma .callout-perspective title="The cache coherence dilemma in neural working sets"}
In symmetric multiprocessing hardware, cache coherence is enforced transparently by silicon snooping protocols (MESI/MOESI) across sub-nanosecond interconnects. When a CPU core mutates a physical cache line, invalidation signals propagate instantaneously, guaranteeing that no stale value is ever read by an execution unit.

In an agentic machine learning system, the staged prompt history holds copies of external state, such as snapshots of files, git branches, process tables, and network sockets. Yet this neural cache is fundamentally decoupled from the environment it mirrors. Autoregressive sequence models are append-only state machines; mutating an earlier turn in-place to reflect an external change invalidates the entire downstream Key-Value cache in modern serving engines (such as vLLM or SGLang). This busts Radix tree prefix matches, transforming a sub-100-millisecond cached prefill into a multi-second recomputation across tens of thousands of tokens.

Conversely, preserving the KV cache by leaving historical turns untouched forces the unprivileged model to attend to obsolete code offsets, stale diagnostics, and superseded variables. The resulting failure mode, epistemic state fracture, causes the agent to act on historical memories that are mathematically present in the prompt but physically false in the host operating system.

**Systems insight**: Context management in agentic architectures is not a prompt styling exercise; it is an unyielding trade-off between physical accelerator cache reuse and epistemic coherence. Robust runtimes bridge this gap through out-of-band tombstoning and dependency-aware just-in-time rehydration, never trusting the neural engine to intuit when the world beneath it has shifted.
:::

::: {#nbk-04-latency-and-compute-roofline-of .callout-notebook title="Latency and compute Roofline of context invalidation strategies"}
...
At an achieved hardware compute efficiency of 60 percent of the theoretical FP8 peak ($0.60 \times 3\text{,}350 \text{ TFLOPS} = 2\text{,}010 \text{ TFLOPS}$):
...
However, under an append-only tombstone policy, the runtime leaves Turn 3 structurally untouched in the base prefix and appends an explicit invalidation record at the prompt tail ($L = 28\text{,}672$). The prefix cache hits across 100 percent of the existing $28\text{,}672$ tokens.
...
## Working-Memory Evaluation {#sec-vol3-context-engineering-evaluation}
...
Evaluating a working-memory architecture requires choosing an appropriate workload topology. In machine learning systems engineering, context evaluation topologies occupy a spectrum between isolated synthetic probes and end-to-end operational environments, as contrasted in @tbl-vol3-eval-spectrum. Each topology measures a distinct boundary in the host supervisor's memory hierarchy.

| Evaluation Topology                  | Canonical Benchmarks      | Primary Evaluated Capability      | State Mutation Dynamics                  | Distractor Semantic Entropy                   | Feedback Channel                    | Systemic Failure Mode Exposed                              |
|:-------------------------------------|:--------------------------|:----------------------------------|:-----------------------------------------|:----------------------------------------------|:------------------------------------|:-----------------------------------------------------------|
| **Synthetic Needle Microbenchmark**  | Single-Needle NIAH, RULER | Passive in-context addressability | Static (zero environment mutation)       | Low (uniform random background text)          | Zero (open-loop, single query)      | Positional decay, RoPE inner product attenuation           |
| **Multi-Needle Associative Probe**   | Multi-hop QA, BAMBOO      | Cross-turn key-value association  | Static (read-only reference corpus)      | Moderate (topically related prose)            | Zero (open-loop, multi-hop query)   | Softmax attention dilution across intermediate rungs       |
| **Closed-Loop Code Repair**          | SWE-bench, RepoQA         | Dependency-aware context staging  | Dynamic (file patches, compiler cycles)  | High (dense codebases, stale diffs)           | Authoritative test suite exit codes | Stale-state reuse, constraint evaporation under compaction |
| **Interactive System Orchestration** | InterCode, OSWorld        | Full trajectory lifecycle memory  | Continuous (filesystem, network sockets) | Extreme (heterogeneous logs, terminal output) | Dynamic environmental telemetry     | Compaction thrashing, blind-horizon amnesia, tool looping  |

: **Working Memory Evaluation Topologies**: Comparative taxonomy of evaluation paradigms, from passive synthetic probes to interactive closed-loop trajectories. {#tbl-vol3-eval-spectrum}
...
::: {.column-margin}
**Passive Addressability vs. Dynamic Utility:** A model that scores 100 percent on synthetic needle retrieval can still fail a 10-turn debugging task if the runtime fails to prune an outdated variable definition from turn 2 that conflicts with a patch applied at turn 8.
:::
...
1. **Multi-Turn Task Completion Rate ($\text{TCR}_H$):** The fraction of tasks successfully solved within a horizon of $H$ turns (@sec-vol3-intro-hsa), verified by an authoritative external oracle (such as an automated test harness or a sealed integration suite):
...
4. **Compression Ratio ($C_r$) and Token Amortization:** The degree of history reduction achieved by the context policy relative to an uncompressed, append-only log:
   ...
   where $|\cdot|$ denotes length in tokens. A policy with $C_r = 1.0$ maintains the full, raw transaction log. A policy achieving $C_r = 4.0$ compresses the historical footprint by 75 percent, admitting longer operational horizons within an identical hardware footprint.
...
::: {#nbk-04-quantifying-working-set-efficiency-and-compaction .callout-notebook title="Quantifying working-set efficiency and compaction trade-offs"}
...
The central trade-off in agent working-memory design is the *Compaction-Fidelity Dilemma*: as the runtime increases the compression ratio $C_r = N_{\text{raw}} / N_{\text{staged}}$ to conserve token budget, it risks stripping out low-salience tokens that contain critical operational constraints.

The empirical dynamics governing this trade-off are illustrated along the Pareto frontier in @fig-vol3-compaction-pareto, which evaluates task completion fidelity across four representative compaction policies:

::: {#fig-vol3-compaction-pareto fig-env="figure" fig-pos="htb" fig-cap="**The Compaction-Fidelity Pareto Frontier in Context Management**: Empirical trade-off between token compression ratio ($C_r$) and task success fidelity. Operating points trace the progression from uncompressed full logs ($1.0\times$), through the Pareto-optimal structured checkpointing frontier ($3.0\times$, $\sim 95\%$ fidelity), to lossy narrative summarization ($6.0\times$) and naive sliding windows ($10.0\times$). Exceeding $5\times$ compression triggers severe task regressions as narrative summarizers shed precise CLI flags, AST coordinates, and negative constraints below the minimum viable fidelity threshold." fig-alt="Pareto frontier curve plotting task success fidelity against token compression ratio, showing peak fidelity at 3x structured checkpoints and severe fidelity collapse beyond 5x compression."}
![](images/svg/compaction_fidelity_pareto.svg)
:::

1. *Full Log ($C_r = 1.0\times$, Baseline):* Staging uncompressed execution traces guarantees that no historical detail is omitted. However, task fidelity languishes at approximately $65\%$. As demonstrated in @fig-vol3-working-set-selection and @fig-vol3-lost-in-middle, retaining massive unpruned transcripts triggers softmax denominator dilution, quadratic prefill latency stalls, and stale-state poisoning, where the model attends to superseded file contents.
2. *Structured Checkpointing ($C_r = 3.0\times$, Pareto Optimal):* By deploying deterministic filtering and grammar-aware AST skeletonization, structured checkpointing achieves peak empirical fidelity ($\sim 95\%$). This policy prunes conversational bloat, repetitive progress lines, and full file dumps while preserving exact AST node signatures, unified diff hunks, exit codes, and error flags verbatim.
3. *Lossy Narrative Summarization ($C_r = 6.0\times$, Constraint Shedding):* When runtimes attempt to push compression past $5\times$ by delegating history reduction to a secondary language model, fidelity collapses to $\sim 45\%$, falling far below the minimum viable task threshold ($0.70$). As shown on the right of @fig-vol3-compaction-pareto, lossy summarization models optimize for fluent prose narratives while discarding low-frequency syntactic tokens—such as non-standard port numbers, compiler flags, and explicit negative constraints. Stripped of deterministic coordinates, the agent descends into unrecoverable stochastic guessing loops.
4. *Naive Sliding Window ($C_r = 10.0\times$, Truncation Amnesia):* Discarding turns beyond a fixed recency window achieves aggressive token reduction but produces catastrophic failure ($\sim 20\%$ fidelity). Evicting the prompt prefix obliterates initial task directives, security envelopes, and baseline failing test conditions, leaving the agent blind to its overarching goal.
...
Consider a long-horizon systems debugging agent undergoing $k=5$ successive compaction cycles. Under lossy narrative summarization with an empirical constraint attenuation rate of $\lambda = 0.20$ (a 20 percent chance per cycle of dropping a seemingly minor parameter flag or negative boundary condition), the probability of retaining all critical constraints decays exponentially:

$$S_{\text{CR}}(5) \approx (1 - 0.20)^5 = (0.80)^5 \approx 0.328 \quad (32.8\%)$$

By turn 30, more than two-thirds of the initial negative constraints have evaporated from the working memory, directly causing the agent to repeat forbidden actions or violate security boundaries. Conversely, under structured checkpointing where the invariant constraint set $\Omega$ is maintained as an immutable schema partition ($\lambda = 0$), constraint retention remains invariant at $S_{\text{CR}}(k) = 1.00$ ($100\%$) across indefinitely extended horizons.
...
Because the model has no channel to the environment except its staged observations (@sec-vol3-intro-operational-incident), its ability to repair a defect or navigate a complex codebase depends entirely on the epistemic precision of the empirical observations ($o_t$) staged in its working set. A lossy summary that excises literal identifier names (such as `sig_verify_ecdsa_secp256k1_v2`), failing file offsets, or exact error flags (`ENOENT` versus `EACCES`) deprives the model of the exact coordinates needed to formulate targeted interventions.
...
Third, context expansion accelerates the context poisoning of @sec-vol3-intro-context-poisoning. Massive context dumps inevitably ingest conflicting, obsolete, or tangential statements—such as superseded documentation versions or stale variable declarations—that directly contradict the active state of the environment. The conditional policy $\pi_{\text{ctx}}(a_t \mid S_t)$ becomes polluted by irrelevant distractor tokens, drastically elevating the probability of state hallucination.
...
::: {.callout-takeaways title="Attention is finite; context must be engineered"}
...
Usable context stops well short of the nominal window, and this chapter showed how a runtime finds its own frontier $M^*$ by measuring task completion on its workload. That measurement turns the principle that context is a selected working set (principle \ref{pri-vol3-attention-working-set}) from a claim into a budget the runtime enforces. The chapter also applied the principle that authority stays with the source (principle \ref{pri-vol3-source-authority}) to the prompt itself. A staged observation, a compacted summary, and a checkpoint are all copies of a source, so each is tracked against the source it read and retired when that source is written.
...
@Sec-vol3-kv-cache (*Paged Attention Memory*) transitions from logical context composition to physical memory virtualization. It investigates how modern inference runtimes such as vLLM and SGLang manage physical KV cache allocations on accelerator hardware, partitioning contiguous logical token sequences into non-contiguous physical pages via PagedAttention, sharing prompt prefixes across branching rollouts through Radix trees, and balancing memory-bound GEMV decode loops against compute-bound GEMM prefill phases under tight multi-tenant memory constraints.
:::
