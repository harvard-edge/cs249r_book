...
\begin{marginfigure}
\mlagentstack{10}{10}{90}{10}{10}{20}
\caption{The Agentic Machine Learning Systems Stack.}
\end{marginfigure}
:::

_Why does generating more candidate reasoning paths degrade into worse decisions?_
...
- Explain test-time deliberation as dynamic search over semantic state-action spaces ($\mathcal{V}, \mathcal{E}$) within the stochastic processor paradigm.
- Compare System 1 reflexive proposal generation with System 2 deliberate search (Best-of-$N$, Beam Search, MCTS, Graph of Thoughts).
- Evaluate the spectrum of control topologies from static prompt chains to autonomous loops, enforcing the Anti-Swarm Principle and the Minimum Autonomy Invariant.
- Design multi-tier semantic cycle detection and out-of-band circuit breaker state machines to protect infrastructure against semantic livelocks.
- Calculate the Verification Tax ($C_{\text{ver}}$) and derive the optimal verification cadence ($m^*$) that minimizes expected wall-clock execution latency.
- Diagnose failure modes of learned Process Reward Models (PRMs) under Goodhart over-optimization versus deterministic environmental oracles.
...
Consider an agent tasked with diagnosing a sudden latency spike in a microservice architecture. If the agent attempts to resolve the issue by guessing the root cause and deploying a patch in a single, blind forward pass, it will almost certainly fail. The foundation model operates as a stochastic arithmetic logic unit (ALU), calculating probabilistic next-token transitions over volatile memory. We can formalize this limitation as the **\1**\index{Reliability Wall} ($P \le p^N$). If an agent attempts to execute a long-horizon trajectory through a single, greedy sequence of actions, compounding error rapidly drives the probability of task completion to zero.
...
When a human engineer encounters a subtle concurrency bug, they do not instantaneously emit a perfect patch from memory. They read the stack trace, hypothesize a race condition, add print statements, and mentally simulate execution paths before writing code. To understand the systems architecture of deliberation, we must formalize this cognitive and algorithmic shift from single-pass inference to deliberate search.
...
Trading compute for accuracy through search is conceptually powerful, but search algorithms cannot execute in an architectural vacuum. We must first define the structural boundaries that prevent an autonomous model from spiraling into infinite, catastrophic loops.

## Control Topologies {#sec-vol3-deliberation-topologies}

If an autonomous coding agent is permitted to arbitrarily branch and loop without structural boundaries, a simple syntax error can trap it in a retry cycle that burns thousands of API credits in minutes without making forward progress. While test-time search provides the algorithmic machinery for deliberation, search algorithms cannot execute in an architectural vacuum. Allowing an autonomous model to branch and loop unconstrained is an operational hazard.
...
: Table 2.1: **Architectural Archetypes of Agentic Control**: Comparison of control topologies across graph structure, autonomy level, latency complexity, token variance, failure blast radius, and primary verification mechanisms. {#tbl-vol3-deliberation-archetypes tbl-colwidths="[15,15,15,15,15,15,10]"}
...
: Table 2.2: **Multi-Tier Semantic Cycle Detection Architecture**: Algorithmic primitives, computational complexities, trigger conditions, detection scopes, and operational risks across the four-tier cycle detection hierarchy. {#tbl-vol3-deliberation-cycle-detection tbl-colwidths="[20,20,15,15,15,15]"}
...
Control topologies provide the safety boundaries and termination guarantees required to operate autonomous loops in production. With these structural safeguards in place, the system can safely deploy the algorithmic mechanisms that navigate semantic token spaces.

## Tree Search Algorithms {#sec-vol3-deliberation-tree-search}

An agent deciding between two API endpoints cannot always predict which will succeed based solely on documentation. It must tentatively invoke one, observe the payload, and backtrack if the endpoint is deprecated. Rather than generating a single linear trajectory $\tau = (s_0, a_0, o_0, \dots, s_T)$, deliberate inference expands a directed search tree $\mathcal{G} = (\mathcal{V}, \mathcal{E})$. Vertices $v \in \mathcal{V}$ represent intermediate agent belief states and environment snapshots; edges $e = (u, v) \in \mathcal{E}$ represent candidate actions.
...
#### Phase 2: Expansion
When selection reaches a non-terminal leaf node $s_L$, the agent expands the node. The foundation model $\pi_\theta$ generates $K$ candidate tool actions or reasoning steps, instantiating new child vertices in the tree. To build intuition, imagine an agent tasked with optimizing a slow SQL query. At node $s_L$, the agent realizes the current query plan uses a nested loop join. The policy network generates three candidate actions: adding a composite index, rewriting the join as a hash join, or partitioning the table. These three actions become newly expanded child vertices waiting for evaluation.

#### Phase 3: Simulation (Evaluation)
To evaluate newly expanded node $s'$, the system estimates its future expected value $V(s')$. In classical board games, this was computed via random rollouts: the system would play completely random moves from $s'$ until the game ended, averaging the wins and losses to guess the node's strength. In agentic systems, random rollouts are computationally prohibitive and statistically meaningless—a random sequence of bash commands will almost never successfully configure a web server. Instead, production runtimes use two alternative evaluators to predict the branch's viability without executing it to completion:

*   **Value Network Head:** A learned value network $V_\phi(s')$ computes a scalar expected success probability in a single forward pass, essentially pattern-matching the current state against known successful trajectories.
*   **Truncated Fast Rollout:** A compact draft model executes a shallow, tightly bounded rollout (e.g., 2 steps) evaluated by a deterministic compiler or linter oracle, providing a cheap structural check before committing heavy compute to the branch.
...
: Table 2.3: **Comparative Systems Architecture of Test-Time Search Topologies**: Structural mechanisms, memory complexities, pruning granularity, rollback support, and serving execution profiles across test-time search algorithms. {#tbl-vol3-deliberation-search-topologies tbl-colwidths="[15,15,15,15,15,10,15]"}
...
The choice of search topology—whether Best-of-$N$, Beam Search, or MCTS—dictates how the agent navigates its action space. However, every candidate node expansion requires evaluation, and those evaluations consume wall-clock time and compute. This introduces the economic limit of search: the verification tax.

## Inference Scaling Laws & The Verification Tax {#sec-vol3-deliberation-scaling-verification}

Evaluating ten thousand candidate proofs for a theorem mathematically guarantees finding the solution if one exists, but generating those ten thousand proofs might consume more GPU hours than training the model itself. Search algorithms allow agents to trade inference compute for task accuracy, but search is not free. The economic thermodynamics of test-time compute govern when, where, and how frequently to verify intermediate states.
...
Every invocation of an evaluation oracle consumes computational resources, token budget, and wall-clock latency. This is not just a theoretical inefficiency; it is a hard physical constraint on execution speed. If evaluating a node requires spinning up a Docker container to run a 15-second test suite, the search algorithm is physically blocked from expanding the tree until that verification completes. We formalize this inescapable overhead as the **Verification Tax**\index{Verification Tax}[^fn-verification-tax] ($C_{\text{ver}}$), representing the total compute and latency invested in grading candidate states:
$$
...
*   **Failure Modes:**
    *   **False Positives (Reward Hacking):** Assigning high scores to confident, plausible-sounding, but mathematically false statements. Under high search budgets ($N \gg 100$), search algorithms ruthlessly exploit these blind spots. This represents a catastrophic manifestation of Goodhart's Law: when a learned metric becomes the target of a high-capacity search algorithm, the metric ceases to be a good measure. Because the PRM is a neural network mapping high-dimensional text to a scalar, its decision boundary contains adversarial pockets. As the search algorithm samples thousands of candidates, it inevitably discovers and selects these adversarial pockets—such as injecting academic jargon or formatting a fundamentally broken equation so beautifully that the PRM assigns it a perfect score.
...
: Table 2.5: **Architectural Taxonomy of Deliberation Verifiers**: Comparison of verifier classes across output signal domain, evaluation latency, credit assignment precision, reward hacking vulnerability, state reversibility constraints, and runtime infrastructure footprints. {#tbl-vol3-deliberation-verifier-taxonomy tbl-colwidths="[15,15,10,20,15,10,15]"}
...
The verification tax establishes the physical limits of test-time compute scaling: intermediate oracles collapse exponential branching, but only when their cadence is tuned to minimize both generation waste and verification overhead. When these economic constraints are ignored, architectural traps follow.

## Fallacies and Pitfalls {#sec-vol3-deliberation-fallacies}

Engineering test-time deliberation systems requires navigating these economic constraints while avoiding critical architectural fallacies and operational traps. The following fallacies and pitfalls highlight common missteps in scaling search.
...
**Fallacy**: *A foundation model can be trusted to self-terminate its own execution loop.*
...
**Pitfall**: *Unconstrained peer-to-peer agent swarms.*
...
By internalizing these limits and rejecting unconstrained architectural patterns, teams can design deliberate reasoning systems that safely execute complex workflows.
...
::: {.callout-takeaways title="The Thermodynamics of Deliberation"}
...
: Table 2.6: **Architectural Synthesis of Test-Time Deliberation Systems**: Subsystems, governing abstractions, primary systems bottlenecks, dominant failure modes, and production systems mitigations across the complete deliberate reasoning architecture. {#tbl-vol3-deliberation-summary-synthesis tbl-colwidths="[15,20,20,20,25]"}
...
::: {.callout-chapter-connection title="From Search Trees to Working Sets"}
...
## Quantitative Problems {#sec-vol3-deliberation-problems}

1. **The Cost of Combinatorial Branching**
An autonomous data science agent generates SQL queries using MCTS over a search horizon of $d = 6$ steps with a branching factor of $b = 4$. Each generated step consumes $1,500$ generation tokens on a 14B parameter model ($C_{\text{gen}} \approx 8.4$ TFLOPs). 
Calculate the total generation FLOPs required to exhaustively expand this tree without verification. If a deterministic semantic SQL linter (costing $0.05$ TFLOPs and $150$ ms of wall-clock time per node) can reject invalid queries and collapse the effective branching factor to $b_{\text{eff}} = 1.3$, calculate the net percentage savings in total test-time FLOPs.

2. **Verification Cadence and Error Rollback**
A code-refactoring agent executes a $N=60$ step trajectory. Each step requires $T_{\text{step}} = 1.2$ seconds of inference latency and has a single-step reliability of $p = 0.92$. The team is deciding how frequently to invoke an isolated unit test suite that consumes $T_{\text{ver}} = 18$ seconds.
Calculate the expected wall-clock execution time $T(m)$ for cadences $m = 1$, $m = 10$, and $m = 60$. Which cadence represents the optimal trade-off between verifier overhead and trajectory rollback waste?

3. **Prefix Duplication and Memory Exhaustion**
An orchestrator serves Best-of-$N$ sampling for $N=64$ parallel branches. The shared system context requires $12,000$ tokens, and each branch generates $800$ tokens. The model employs FP16 Key-Value cache costing $1$ MB per $1,000$ tokens.
Calculate the peak HBM required if the serving engine naively duplicates the prefix for every branch. Next, calculate the peak HBM required if the runtime utilizes RadixAttention to share the prefix tree. What is the memory savings ratio?

4. **Reward Hacking and Verification Economics**
A PRM assigns scores $r \in [-1, 1]$ to intermediate mathematical derivations. The generator proposes branches, and the search algorithm selects the top $10\%$ of branches by score. When $N$ candidate branches are expanded, the probability that the search algorithm encounters an adversarial artifact (a formally incorrect derivation graded $r > 0.95$ by the PRM) scales as $P(\text{hack}) = 1 - e^{-\lambda N}$ where $\lambda = 0.005$.
At what branch scale $N$ does the probability of selecting an adversarial hallucination exceed $50\%$? At this break-even point, what is the mandatory structural remedy to prevent catastrophic policy commitment?
