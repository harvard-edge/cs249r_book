...
- Explain why static machine learning benchmarks collapse under compounding trajectory errors and pretraining contamination.
- Calculate Trajectory Goodput and fleet economics across varying step error rates and hardware architectural footprints.
- Architect hermetic evaluation gyms that enforce strict network air-gapping and copy-on-write isolation.
- Design hierarchical distributed tracing payloads utilizing OpenTelemetry to mitigate span buffer overflow.
- Evaluate deterministic time-travel replay engines using boundary event sourcing at the stochastic-deterministic perimeter.
- Diagnose multi-step autonomy failures by decomposing trajectory latency according to Amdahl's Law.
:::


## Introduction {#sec-vol3-observability-intro}

When an autonomous coding agent degrades into an unrecoverable loop at Step 32, traditional request-response tracing provides zero diagnostic power. The engineer attempting to debug the failure faces a fundamental problem: rerunning the agent with the same initial prompt will not reproduce the error. Non-determinism at the hardware level (floating-point reduction order across GPU thread blocks), the algorithmic level (temperature sampling $T > 0$), and the environment level (network jitter, dynamic filesystem state) guarantees that the agent will take a different path.

Deploying autonomous agents into production creates an immediate dual crisis:

1. **The Observability Crisis:** In microservices, requests traverse linear, directed acyclic graphs of short-lived remote procedure calls. In agentic systems, execution is an unrolled, multi-turn feedback loop traversing foundation models, vector databases, compiler sandboxes, and external APIs.
2. **The Evaluation Crisis:** For decades, machine learning relied on static benchmarks: ImageNet [@deng2009imagenet], GLUE [@wang2018glue], and GSM8K [@cobbe2021gsm8k]. These evaluate feedforward, single-turn pattern matching: a fixed input $x$ is processed by model $f_\theta(x)$ and scored against a static label $y$. In agentic systems, static benchmarks reward prompt-engineering shortcuts, suffer pretraining contamination, and ignore closed-loop environmental feedback. A model with 95 percent single-step accuracy on a static benchmark can collapse to less than 1 percent success over a 100-step autonomous trajectory.

Observability is the architectural backplane that makes autonomous systems engineering possible. Without structured telemetry, developers cannot diagnose why an agent diverged. Without hermetic evaluation gyms, developers cannot distinguish genuine algorithmic progress from evaluation noise. And without time-travel replay engines, developers cannot construct deterministic regression suites.

Resolving this crisis requires abandoning the tools of the previous era. The static datasets that drove the first decade of deep learning progress are structurally incapable of measuring interactive, multi-step agency. The failure of these static benchmarks reveals why the entire evaluation paradigm must be replaced.

## The Collapse of Static Benchmarks {#sec-vol3-observability-collapse-static}

Consider an engineer reviewing a flawless 2,000-line code patch. They did not produce it in a single continuous keystroke sequence; they ran compilers, inspected stack traces, and executed unit tests. Yet a static benchmark treats a neural network exactly like that impossible single-shot typist. A prompt is submitted, an answer is emitted, and a scalar metric (accuracy, BLEU, F1 score) is recorded. In an agentic systems regime, this static paradigm collapses under two fatal structural flaws: pretraining contamination and the complete absence of environmental feedback.

### Goodhart's Law and Pretraining Contamination

In 1975, economist Charles Goodhart formulated the principle that "when a measure becomes a target, it ceases to be a good measure" (@goodhart1984problems). In foundation model evaluation, Goodhart's Law manifests through **pretraining contamination**\index{pretraining contamination}.

Modern foundation models are trained on internet-scale web crawls containing trillions of tokens (Common Crawl, GitHub, Wikipedia, Reddit). Because static benchmark datasets (such as GSM8K [@cobbe2021gsm8k], HumanEval [@chen2021codex], and MMLU [@hendrycks2021mmlu]) are publicly hosted on GitHub and discussed in blog posts, their questions, solutions, and unit tests are ingested into the pretraining corpus.
...
The fundamental mathematical failure of static benchmarks is their inability to model **trajectory error propagation**\index{trajectory error propagation}.

If an autonomous task requires sequential interaction steps without intermediate verification, the joint probability of trajectory success decays exponentially with the average per-step error rate. This compounding degradation highlights why open-loop execution is fundamentally unsustainable for long-horizon agentic tasks.

Mathematically, this manifests as a geometric decay curve. Even if a model has a 99 percent success rate per step, a 100-step trajectory will succeed only about 36 percent of the time. This non-linear decay means that relying solely on model improvements—pushing single-step accuracy from 99 to 99.5 percent—yields severely diminishing returns over long horizons compared to structural interventions like intermediate verification.
...
::: {.callout-note title="Systems Intuition: Trajectory Collapse"}
An agent executing a multi-step task without intermediate verification is a system without a feedback loop. Even if the per-step accuracy is extremely high, errors compound multiplicatively over long horizons. A model that looks flawless on a single-step benchmark will quickly collapse into a state of unrecoverable drift when required to execute a hundred sequential actions, because a single uncorrected mistake permanently corrupts all future observations.

Without independent state verification, an agent enters a self-delusion loop: it perceives the corrupted state caused by its own error as ground truth, generating future actions based on an invalid worldview. This divergence grows monotonically until the agent hits a fatal exception or a timeout limit.
:::
...
| **Evaluation Dimension** | **Static Evaluation (MMLU [@hendrycks2021mmlu], GSM8K [@cobbe2021gsm8k], HumanEval [@chen2021codex])**                                          | **Closed-Loop Agent Gyms (SWE-bench, WebArena, OSWorld)**                                                               |
|:-------------------------|:----------------------------------------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------|
...
| **Systems Substrate**    | Standard batch inference engines (vLLM [@kwon2023vllm], TensorRT-LLM [@nvidia2023tensorrtllm]) without host sandbox isolation    | Hardware-assisted microVMs (Firecracker [@agache2020firecracker]), CoW storage (OverlayFS), and strict network air-gapping                       |
...
Building these environments requires moving beyond static CSV files and into the realm of distributed systems engineering. To evaluate agents accurately, we must construct execution sandboxes that are fully interactive, perfectly reproducible, and isolated from external noise.
:::

## Hermetic Evaluation Gyms {#sec-vol3-observability-hermetic-gyms}

Evaluating an autonomous web agent on a live internet server guarantees that tomorrow's benchmark run will fail if the server's UI changes, or succeed trivially if the agent discovers a cached answer on a forum. To replace static datasets with reproducible dynamic environments, the machine learning systems community developed **Agent Gyms**—standardized, interactive execution environments that simulate real-world digital ecosystems.[^fn-hermetic-evaluation-gym]
...
To evaluate algorithmic capability rigorously without running thousands of redundant trials, runtimes sample a small pool of independent candidate trajectories per task. They then calculate the probability that at least one of the top $k$ attempts resolves the goal using an unbiased Pass@$k$ estimator. 

Because naive calculation of combinatorial subsets causes floating-point overflow and extreme variance, systems runtimes implement iterative, mathematically stable estimators. The core systems insight is that evaluating multi-step agency requires decoupling the inherent stochasticity of the model's token sampling from the deterministic success of the task.

Instead of directly counting successes over massive samples, the Pass@$k$ estimator mathematically interpolates the probability of success based on a smaller sample size $n \ge k$. By calculating the expected number of combinations that contain at least one correct trajectory, it provides a stable capability metric that is robust to individual run variance without requiring exhaustively evaluating all $k$ possibilities in production.
...
To evaluate real-world feasibility, we formulate **Trajectory Goodput**\index{Trajectory Goodput} ($G$)[^fn-trajectory-goodput]:

Fundamentally, Goodput penalizes agents that require excessive backtracking or generate massive, uninformative token streams. A highly capable but verbose agent might achieve a 95 percent success rate but exhibit lower Goodput than a concise, 90 percent accurate agent if the former consumes ten times more compute per goal. Economically, minimizing the Cost-per-Goal is often achieved by routing easy tasks to smaller, cheaper models and reserving heavy models exclusively for critical failure recovery.
...
| **Architectural Dimension**                           | **Dense Architecture (e.g., Llama-3-70B [@dubey2024llama3])**                                                              | **Sparse MoE Architecture (e.g., Mixtral-8x7B [@jiang2024mixtral] / DeepSeek-V3 [@deepseek2024v3])**                                                                                |
|:------------------------------------------------------|:--------------------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------|
...
While hermetic gyms provide the sterile environments necessary to evaluate agents objectively, evaluation alone does not fix broken systems. When a benchmark reveals that an agent's success rate has degraded, developers need systems to inspect the exact historical sequence of decisions that led to the failure. This requires pausing, rewinding, and editing the unrolled execution loops.
...
Rather than flattening agent execution into raw unstructured logs, modern runtimes map the agent's interaction loop into a strict hierarchical causal graph. The hierarchy typically cascades from a global trajectory root down to individual reasoning steps, and finally to physical leaf operations (model inference, tool execution, or database retrieval). 

At a systems level, structured observability payloads are crucial because unstructured text logs lack causal linkages. By organizing telemetry into this parent-child taxonomy, the runtime can attribute multi-model inference latency, token counts, tool I/O execution, and dollar costs to specific logical steps across distributed boundaries.

This hierarchical tracing transforms black-box non-determinism into analyzable performance data. By attributing every API call and filesystem operation to a specific node in the reasoning tree, systems engineers can pinpoint exactly which branch of a search algorithm introduced a massive latency spike or which tool caused a token-limit exhaustion.

### Context Propagation Across Asynchronous Queues and Search Trees

In distributed production deployments, agents dispatch sub-tasks across message queues (Kafka [@kreps2011kafka], Celery, Temporal) and spawn worker agents in remote microVMs. To maintain causal lineage across network boundaries, runtimes utilize the W3C TraceContext specification (@w3c2021tracecontext), injecting two standardized HTTP/RPC headers:
...
Production runtimes solve this via **Payload Offloading**\index{Payload Offloading}:
...
Furthermore, runtimes reject traditional **Head-Based Sampling**\index{Head-Based Sampling} (randomly discarding 99 percent of traces at ingress). Because agent trajectories are low-throughput but extremely high-value, dropping 99 percent of traces guarantees that engineers will have zero data when a catastrophic 0.5 percent failure occurs. Instead, systems employ **Tail-Based Sampling**\index{Tail-Based Sampling}: buffering all spans in memory or fast local NVMe caches during trajectory execution, and retroactively retaining 100 percent of traces that fail, trigger safety violations, or exceed cost thresholds, while aggressively downsampling routine successes.

### Event Sourcing at the Stochastic Boundary

To achieve bit-exact offline replay, the runtime implements the classical systems principle of **Boundary Event Sourcing**\index{Boundary Event Sourcing} (@geels2007friday; @king2005debugging)[^fn-boundary-event-sourcing]. The runtime establishes a strict boundary separating the internal deterministic state machine from external non-deterministic inputs.

The core insight of boundary event sourcing is that an agent is simply a deterministic state machine driven by stochastic inputs. If the runtime can perfectly record all external randomness—the exact tokens generated by the LLM and the exact bytes returned by external tools—it can replay the agent's internal logic with perfect fidelity, entirely bypassing the need to invoke the expensive LLM again.
...
To execute a **State Rewind**\index{State Rewind} to step $t^*$:
...
Once rewound, the engineer can perform **Counterfactual Branch Forking**\index{Counterfactual Branch Forking}:
...
In production fleets, trajectory latency is as critical as task accuracy. Total trajectory latency across an execution is formally decomposed into additive systems components: time-to-first-token (TTFT), autoregressive decode, external tool execution, hypervisor virtualization overhead, and verification delays.

This decomposition reveals that an agent's wall-clock latency is increasingly dominated not by the neural network's forward pass, but by the operating system overhead of booting secure sandboxes, installing dependencies, and running compilers.

When optimizing an agent runtime, systems engineers often default to optimizing model inference (deploying FP8 quantization or custom CUDA kernels). However, by **Amdahl's Law**\index{Amdahl's Law} (@amdahl1967; @patterson2017hardware), overall speedup is strictly bounded by the sequential fraction of the workload that is accelerated.

Amdahl's Law states that the theoretical maximum speedup of a system is limited by the portion of the execution time that cannot be parallelized or optimized. In the context of agentic runtimes, even if you could instantly evaluate the model (infinite tokens per second), the end-to-end trajectory time would still be bottlenecked by the sequential execution of bash scripts, HTTP requests, and disk I/O.
...
By Amdahl's Law, if an engineering team spends months accelerating model inference to achieve an extraordinary speedup, the end-to-end gain is severely bounded because inference typically accounts for a minority of total execution time. Conversely, accelerating the systems runtime—replacing full VM reboots with instant CoW snapshot rollbacks and caching compiler build artifacts—targets the dominant operational bottleneck, often yielding significantly higher end-to-end speedups than optimizing foundation model inference alone.
...
| **Autoregressive Decode ($T_{\text{decode}}$)**                  |     $20\text{--}25\%$      | Accelerator HBM memory bandwidth (GEMV bound)        | FP8/INT4 quantization, speculative decoding, FlashDecoding [@dao2023flashdecoding]       |         $2.0\times$         | $1.11\text{--}1.14\times$ |
...
Optimizing these individual subsystems—from bounding memory bandwidth to isolating sandbox latency—is the tactical implementation of a larger, unavoidable physical reality. Every architectural decision in an agentic runtime ultimately answers to a set of absolute constraints that dictate whether a long-horizon loop can succeed at all.

## Epilogue: The Sovereign Law of Agency {#sec-vol3-observability-sovereign-law}

An agent deployed to manage cloud infrastructure is mathematically guaranteed to fail if it executes hundreds of actions without intermediate verification. As this textbook concludes, we synthesize the architectural journey into the fundamental physical constraints governing all autonomous computational systems: **The Sovereign Law of Agency**.
...
Unverified open-loop autonomy decays exponentially. To make a long-horizon enterprise trajectory commercially viable through raw model intelligence alone, the foundation model must maintain an impossibly low error rate. Because foundation models are probabilistic estimators trained over human linguistic distributions, achieving such ultra-low error rates across complex, out-of-distribution real-world environments is mathematically intractable without systems-level verification.

The probability of a successful trajectory is the product of the success probabilities of its constituent steps. As the horizon $H$ increases, this product asymptotically approaches zero. Therefore, attempting to solve long-horizon autonomy purely by scaling parameter counts to marginally improve per-step accuracy is a structural dead end.

### Verification as a Mandatory Physical Tax

The only systems mechanism that counteracts exponential trajectory decay is **Intermediate Verification**\index{Intermediate Verification} (@cobbe2021training; @deepseekai2025deepseekr1).

If the systems runtime couples execution steps with external, deterministic verification oracles (compilers, test runners, formal linters) that detect invalid state transitions and trigger immediate backtracking, the compounded success probability is restored. Verification is not an optional application-layer feature; it is the mathematical precondition for long-horizon autonomous agency.

By verifying intermediate states, the runtime resets the exponential decay curve. An error at step $t$ is caught and corrected before it can corrupt step $t+1$, effectively transforming a fragile long-horizon trajectory into a sequence of robust, short-horizon transitions. This allows the system's reliability to scale independently of the model's inherent error rate.

::: {.callout-important title="The Sovereign Law of Agency"}
**Invariant**: In an unverified autonomous system, task success probability decays exponentially with step count. Reliable long-horizon execution is achievable if and only if intermediate verification density scales with horizon length.

The architecture must guarantee that for any unbounded sequence of actions, the distance between deterministic checkpoints remains bounded.
...
To balance operational velocity against catastrophic blast radius, runtimes enforce the **Progressive Autonomy Spectrum**\index{Progressive Autonomy Spectrum} shown in @fig-vol3-autonomy-spectrum, which calibrates delegated authority against verification rigor.
...
Simultaneously, an agent's context history rapidly accumulates with every interaction step, forcing the runtime to manage unbounded memory growth. 

Context accumulation in agentic systems is fundamentally superlinear. As an agent executes more steps, it not only appends new actions but also accumulates the expanding state of the environment, verbose error logs, and intermediate reasoning scratchpads. This causes the total token count to explode, severely degrading both memory capacity and inference latency.

Because standard Transformer self-attention compute scales quadratically $\mathcal{O}(L_t^2)$ and memory allocation for the Key-Value (KV) cache grows linearly $\mathcal{O}(L_t)$ per active trajectory, unmanaged context growth inevitably exhausts GPU accelerator HBM. Furthermore, feeding uncurated historical tokens into the context window causes **attention dilution** and context working set pollution, accelerating policy failure. Systems runtimes must enforce strict working set eviction, semantic summarization, and Radix prefix sharing to maintain bounded execution contexts.

### Engineering Hierarchy for Agent Systems

When an autonomous system fails in production, engineering teams must follow a disciplined debugging hierarchy. @Fig-vol3-improvement-ladder formalizes **The Autonomous Improvement Ladder**\index{Autonomous Improvement Ladder}, defining the priority order between systems scaffolding and neural weight modification.
...
These bedrock principles provide the foundation for building reliable systems, but they do not automatically protect developers from drawing false conclusions during the engineering process. Recognizing the limits of evaluation metrics and the hazards of misconfigured environments is just as critical as the architecture itself.

## Fallacies and Pitfalls {#sec-vol3-observability-fallacies}

Deploying a 95 percent accurate model and assuming it will safely complete a 100-step workflow is a statistical illusion. Even with robust architecture, engineers routinely fall into predictable traps when evaluating autonomous systems.
...
**Fallacy**: *Passing unit tests in a sandbox environment guarantees real-world task correctness.*
...
**Fallacy**: *Logging raw text strings into Elasticsearch constitutes production observability.*
...
**Pitfall**: *Benchmark contamination via unrestricted outbound internet access.*
...
Avoiding these traps requires a disciplined adherence to the systems principles that govern stateful autonomy. The following summary consolidates these principles.
...
::: {.callout-takeaways title="The Trajectory Observability Contract"}
...
## Quantitative Problems {#sec-vol3-observability-problems}

1. **Trajectory Decay and Verification:** An autonomous system executes a 50-step trajectory with a baseline per-step success rate of 98\%. Calculate the probability of end-to-end success in an open-loop execution. If the runtime introduces intermediate verification checkpoints every 5 steps that perfectly catch and correct errors within the preceding chunk (but each checkpoint costs 2 seconds of additional latency), what is the new probability of success? What is the total latency penalty incurred by the verification mechanism?
2. **Telemetry Storage Overhead:** A fleet of 100,000 daily autonomous agents operates at an average horizon of 40 steps per task. Each step generates an average of 4 KB of structured trace metadata and 150 KB of raw unstructured payload (prompt text, logs, and tool stdout). Calculate the daily and monthly (30-day) storage requirements if the engineering team uses head-based sampling (10\% retention) vs. tail-based sampling (buffering everything but only retaining the 5\% of trajectories that fail or encounter anomalies). Which strategy consumes less storage, and by what margin?
3. **Amdahl's Law Profiling:** An agent runtime's latency profile is 20\% prompt prefill, 30\% autoregressive decode, 35\% tool execution, and 15\% sandbox lifecycle overhead. The engineering team proposes deploying a custom FlashDecoding kernel that accelerates autoregressive decode by $2.5\times$, at a cost of $\$50,000$ in developer time. Alternatively, migrating to Firecracker microVMs will accelerate the sandbox lifecycle overhead by $8\times$, at a cost of $\$20,000$. Using Amdahl's Law, calculate the overall trajectory speedup for each intervention. Which option provides the highest return on investment (ROI)?
4. **Trajectory Goodput Optimization:** Compare a dense 70B model against an $8\times7$B MoE model for an agent task. The dense model requires $\$1.50$ per $1\text{M}$ tokens and achieves a Pass@$1$ rate of 82\%. The sparse MoE model costs $\$0.40$ per $1\text{M}$ tokens and achieves a Pass@$1$ rate of 78\%. If each trajectory consumes an average of $60,000$ tokens, calculate the Cost-per-Goal for each architecture. How many additional runs ($k$) would the MoE model need to match the dense model's 82\% pass rate, and would it still be more economical?


[^fn-hermetic-evaluation-gym]: **Hermetic Evaluation Gym**\index{Hermetic Evaluation Gym}: An air-gapped, statefully isolated execution sandbox that provides deterministic environment reset, copy-on-write storage isolation, and independent verification oracles for evaluating autonomous agents. **Significance**: Without strict hermeticity, agent evaluation is confounded by external network drift, transient API outages, shared filesystem contamination, and public benchmark data leakage. **Distinction**: Unlike passive unit testing suites or static benchmark harnesses, a hermetic gym acts as an interactive, stateful operating environment that executes untrusted agent tool calls within sub-50 ms disposable virtualization boundaries. **Common pitfall**: Permitting outbound internet access or DNS resolution during evaluation, which allows agents to search the web, retrieve canonical pull requests, and memorize committer patches rather than solving the underlying engineering problem.

[^fn-trajectory-goodput]: **Trajectory Goodput**\index{Trajectory Goodput}: ($G$) is the rate of verified successful goal completions delivered per unit of total economic or computational expenditure. Trajectory Goodput incorporates verification by dividing verified completions by total token and infrastructure cost. Cost-per-Goal measures the average expenditure required to deliver a single verified outcome, scaling non-linearly with reliability. A small improvement in single-pass reliability can drastically reduce the cost of verified delivery.

[^fn-boundary-event-sourcing]: **Boundary Event Sourcing**\index{Boundary Event Sourcing}: An observability pattern that captures all non-deterministic inputs and outputs crossing the agent's interaction perimeter—including pseudo-random seeds, wall-clock timestamps, model token streams, and tool observations—into an immutable append-only event log. **Significance**: Foundation models are non-deterministic under floating-point concurrency and positive temperature sampling; boundary event sourcing enables bit-exact offline replay and time-travel debugging at zero marginal token cost. **Distinction**: Unlike full-system checkpointing, which captures the entire memory space of the model and hypervisor, boundary event sourcing logs only the transitions at the stochastic-deterministic perimeter, keeping telemetry logs under hundreds of kilobytes per step. **Common pitfall**: Capturing internal chain-of-thought activations while omitting external environment timestamps or pseudorandom seeds, causing replay execution to diverge after the first tool invocation.

::: {.callout-chapter-connection title="From Observability to System Tokenomics"}
