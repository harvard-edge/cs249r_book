# Agentic Machine Learning Systems: Master Curriculum Outline (V4)
...
- **Curricular Role in Volume III:** *"Whole-Book Opening Bookend & Foundational Reference Architecture."* Chapter 01 establishes the conceptual foundation of the entire book: the agentic system. A statistical foundation model evaluates context and emits candidate token sequences, but completing an operational task requires advancing from delegated intent to an accepted deliverable backed by verifiable evidence. This chapter introduces the four functional subsystems (Foundation Model Engine, Memory Hierarchy, Tool Interfaces, and Agent Runtime), formalizes the 5-part task contract, poses the 4 Engineering Questions (Duration, State, Authority, Evidence), and establishes closed-loop execution as the governing systems paradigm.

#### The Curricular Compass (Where We Are in the 18 Chapters)

```
[SYSTEMS STATE AT CHAPTER 01]:
- Subsystem Under Construction: Introduction, Chapter 01 (Foundations of Agentic Systems - Whole-Book Opening Bookend).
- Computational Scope: The whole-trajectory execution contract (Goal, Environment, Permitted Actions, Available Observations, Completion Criteria), the four functional subsystems (Foundation Model Engine, Memory Hierarchy, Tool Execution, Agent Runtime), the 4 Engineering Questions (Duration, State, Authority, Evidence), closed-loop empirical verification, and trajectory latency/cost accounting.
...
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME AS PRE-BUILT IN CHAPTER 01):
  * Part I: Chapters 02–03 (The Foundation Model Engine and Test-Time Deliberation).
  * Part II: Chapters 04–06 (Working Memory, KV-Cache Hierarchy, Persistent Storage).
  * Part III: Chapters 07–08 (Tool Execution, Environmental Isolation).
  * Part IV: Chapters 09–11 (Runtime Orchestration, Persistence/WAL, Fault Recovery).
  * Part V: Chapters 12–14 (Trajectory Harvesting, Supervised Adaptation, RLVR).
  * Part VI: Chapters 15–17 (Multi-Agent Coordination, Observability, Serving Economics).
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  ...
  - *Possible focus (The Foundational Thesis of the Volume):* **Agency is a property of the whole computer system, not the neural model.** The foundation model is simply the foundation model engine core; reliability, safety, fault tolerance, and resource efficiency must be engineered into the runtime orchestration, memory, execution sandbox, and verification layers surrounding it.
- **What NOT to Cover (Negative Scope & Forward Deferrals):**
  - 🛑 **DO NOT** derive Byte-Pair Encoding (BPE), logit sampling, or autoregressive attention math (Deferred exclusively to Chapter 02: The Foundation Model Engine).
  - 🛑 **DO NOT** detail container, cgroup, or hypervisor virtualization architectures (Deferred exclusively to Chapter 08: Environmental Isolation).
  - 🛑 **DO NOT** discuss multi-agent delegation or swarms (Deferred exclusively to Chapter 15: Multi-Agent Coordination).
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  ...
  - *The Temporal Stretching of Execution Units:*
    - Machine instructions ($10^{-9}\text{ s}$) $\to$ OS threads/processes ($10^{-6}\text{ s}$) $\to$ RPC/REST requests ($10^{-3}\text{ s}$) $\to$ Stateless LLM inference ($10^{-1}\text{ s}$) $\to$ Autonomous trajectories ($10^1\text{ to }10^4\text{ s}$), spanning thirteen orders of temporal magnitude.
...
#### Section 1.3: Software Paradigm Evolution [core]
- **Heading & Anchor:** `## Software Paradigm Evolution {#sec-vol3-intro-the-tripartite-systems-comparison}`
...
- **Curricular Placement:** Failure taxonomy establishing the core reliability challenge of autonomous agent systems.
- **What to Cover (Positive Scope & Systems Mechanics):**
...
#### Section 1.9: The Systems Architecture Blueprint [core]
- **Heading & Anchor:** `## The Systems Architecture Blueprint {#sec-vol3-intro-stochastic-computer}`
- **The Single Key Point:** Foundations of Agentic Systems is a software-level functional architecture: learned computation, state, controlled interaction, and supervision form the live loop; training and fleet operations improve and operate it across tasks.
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *The Live Functional Architecture:* The Foundation Model Engine (unprivileged learned compute), Context Memory & KV Hierarchy (working set and physical cache), Execution Sandboxing (mediated tool dispatch), Agent Runtime (control plane, WAL, Sagas).
  - *Across-Task Lifecycle Infrastructure:* Trajectory Harvesting (Ch 12), Policy Optimization (Ch 13–14), Distributed Fleet Operations (Ch 15–17).
  - *The Software Systems Architecture:* Establishing the agentic system as a software systems architecture running on host operating systems and matrix accelerators; delineating the boundary between accelerator compute and host runtime governance.
- **What NOT to Cover (Negative Scope & Forward Deferrals):**
...
- **Visuals & Tables:**
  - Table: responsibility, owner, input, output, and verification boundary for each live component.
  - Figure: The Functional Architecture of the Agentic System [insert link here: books/vol3/01_introduction/images/svg/agentic_systems_architecture.svg], illustrating the closed-loop execution flow across inference, memory, sandboxing, and runtime orchestration.
- **Seminal Literature:**
  - John von Neumann (1945, *First Draft of a Report on the EDVAC*).
- **Causal Bridge to 1.10:** How does this book guide the reader through the construction and mastery of the agentic system?
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *The Seven Architectural Parts:*
    ...
    2. *Part I: The Computational Engine:* Chapters 02–03 (Invocation contracts, syntax masking, deliberation).
    3. *Part II: The Memory Hierarchy:* Chapters 04–06 (logical context selection, physical KV state, explicit durable retrieval).
    4. *Part III: Execution Sandboxing:* Chapters 07–08 (Typed tools, MCP, sandboxing).
    5. *Part IV: Runtime Orchestration:* Chapters 09–11 (Control plane, WAL event sourcing, Sagas).
    6. *Part V: Policy Optimization:* Chapters 12–14 (Trajectory mining, SFT, verifiable RL).
    7. *Part VI: Distributed Systems:* Chapters 15–17 (Multi-agent topologies, tracing, cost engineering).
    8. *Part VII: System Synthesis:* Chapter 18 (Capstone architecture and safety case).
...
- **Authoritative Synthesis:** Capturing the core mission of the agentic system.
...
- `::: {.callout-chapter-connection title="From Architectural Foundations to Stochastic Silicon"}`
  - Handoff forward to Part I (*The Foundation Model Engine*) and Chapter 02 (*The Foundation Model Engine*).
- Part transition marker: ````{=latex}\\part{key:vol3_processor}````

---

## Part I: The Computational Engine
...
- **Curricular Role in Volume III:** *"Here is the Inference Engine."* Introduces the Large Language Model through the dual lens of Computer Architecture (accelerator memory bandwidth, tensor algebra, embedding tables, KV cache memory footprint) and Operating Systems (unprivileged inference engine, zero ambient authority, reference monitors, status envelopes, deterministic verification).
...
- Subsystem Under Construction: Part I, Chapter 02 (The Foundation Model Engine).
...
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 02):
  * Chapter 03: Test-Time Deliberation (Search trees, Best-of-N, MCTS, PRMs).
  * Chapter 04: Working Context (Working set compaction, lost-in-the-middle).
  * Chapter 05: Paged Attention Memory (PagedAttention virtual block tables, DRAM swapping).
  * Chapter 06: Persistent Storage (Git index traversal, vector databases, freshness).
  * Chapter 07: Tool Execution (Subprocess dispatch, stdout/stderr streaming).
  * Chapter 08: Environmental Isolation (MicroVMs, Firecracker, OverlayFS, cgroups, seccomp).
  * Chapters 09-11: The Agent Runtime (ACB lifecycle, WAL event sourcing, Sagas).
  * Chapters 12-14: Policy Optimization (Trajectory harvesting, SFT distillation, RLVR).
  * Chapters 15-18: Distributed Fleets, Observability, Cost Engineering, Capstone Synthesis.
```

#### Purpose {.unnumbered .unlisted}

**The Core Question:** *How do software engineers treat a statistical, non-deterministic neural network as an unprivileged, dependable inference engine within an autonomous software system?*
...
- **The Single Key Point:** The LLM is an unprivileged neural inference engine that emits candidate token distributions; the host agent runtime is the authoritative supervisor that owns context staging, execution limits, and action verification.
- **Pedagogical Arc:**
  ...
  5. *Three-Tier Architecture (@fig-inference-engine-architecture):* Host Agent Runtime (supervisor), Inference Service Daemon (driver/controller), Neural Core (accelerator silicon).
...
- **Causal Bridge to 2.2:** Before data can cross into this inference engine, how must it be formatted and encoded?

#### Section 2.2: Discrete Token Representation [core]
- **Heading & Anchor:** `## Discrete Token Representation {#sec-vol3-processor-tokenization}`
...
#### Section 2.3: Autoregressive Generation [core]
- **Heading & Anchor:** `## Autoregressive Generation {#sec-vol3-processor-autoregressive}`
...
#### Section 2.4: Candidate Sequence Verification [core]
- **Heading & Anchor:** `## Candidate Sequence Verification {#sec-vol3-processor-continuations}`
...
#### Section 2.6: Grammar-Guided Decoding [core]
- **Heading & Anchor:** `## Grammar-Guided Decoding {#sec-vol3-processor-grammar-constrained}`
...
#### Section 2.7: Accelerator Serving Latency [core]
- **Heading & Anchor:** `## Accelerator Serving Latency {#sec-vol3-processor-cost}`
...
#### Section 2.8: Interface Benchmarking [synthesis]
- **Heading & Anchor:** `## Interface Benchmarking {#sec-vol3-processor-interface-design}`
...
- Subsystem Under Construction: Part I, Chapter 03 (Test-Time Deliberation).
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 03):
  * Chapter 04: Working Context (Working-set selection, compaction, lost-in-the-middle).
  * Chapter 05: Paged Attention Memory (PagedAttention virtual block tables, DRAM swapping).
  * Chapter 06: Persistent Storage (Git index traversal, vector databases, freshness).
  * Chapter 07: Tool Execution (Subprocess dispatch, stdout/stderr streaming).
  * Chapter 08: Environmental Isolation (MicroVMs, Firecracker, OverlayFS, cgroups, seccomp).
...
- **What NOT to Cover (Negative Scope & Forward Deferrals):**
  ...
  - 🛑 **DO NOT** discuss full Agent Runtime process scheduling or preemption (Deferred to Chapter 09).
...
#### Section 3.3: Candidate Selection [core]
- **Heading & Anchor:** `## Candidate Selection {#sec-vol3-deliberation-candidate-diversity}`
...
#### Section 3.4: Process Verification [core]
- **Heading & Anchor:** `## Process Verification {#sec-vol3-deliberation-verification}`
...
#### Section 3.6: Search Stopping Criteria [core]
- **Heading & Anchor:** `## Search Stopping Criteria {#sec-vol3-deliberation-bounded-search}`
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 04):
  * Chapter 05: Paged Attention Memory (PagedAttention virtual block tables, DRAM swapping, chunked prefill).
  * Chapter 06: Persistent Storage (Vector databases, HNSW, persistent Git repository indexing).
  * Chapter 07: Tool Execution (Subprocess dispatch, stdout/stderr streaming, MCP protocol).
  * Chapter 08: Environmental Isolation (MicroVMs, Firecracker, OverlayFS, cgroups, seccomp).
  * Chapters 09-11: The Agent Runtime (ACB lifecycle, WAL event sourcing, Sagas).
  * Chapters 12-14: Policy Optimization (Trajectory harvesting, SFT distillation, RLVR).
  * Chapters 15-18: Distributed Fleets, Observability, Cost Engineering, Capstone Synthesis.
...
- **Curricular Placement:** Establishes the boundary between logical prompt staging (Host Agent Runtime) and physical KV serving memory (Inference Service).
- **What to Cover (Positive Scope & Systems Mechanics):**
  ...
  - *Possible focus (The Systems Confrontation):* Decoupling Logical Context from Physical KV Cache. Distinguishing what the agent runtime selects to stage (logical working set in host DRAM) from how the GPU serving daemon physically represents and computes attention (physical KV pages in HBM).
...
#### Section 4.2: Working Set Capacity [core]
- **Heading & Anchor:** `## Working Set Capacity {#sec-vol3-working-sets-physics}`
...
#### Section 4.4: Context Compaction [core]
- **Heading & Anchor:** `## Context Compaction {#sec-vol3-working-sets-compaction}`
...
#### Section 4.5: Context Checkpointing [core]
- **Heading & Anchor:** `## Context Checkpointing {#sec-vol3-working-sets-summarization}`
...
#### Section 4.6: Context Invalidation [core]
- **Heading & Anchor:** `## Context Invalidation {#sec-vol3-working-sets-context-rot}`
...
- Subsystem Under Construction: Part II, Chapter 05 (Paged Attention Memory).
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 05):
  * Chapter 06: Persistent Storage (Vector databases, HNSW, persistent Git indices).
  * Chapter 07: Tool Execution (Subprocess dispatch, stdout/stderr streaming, MCP protocol).
  * Chapter 08: Environmental Isolation (MicroVMs, Firecracker, OverlayFS, cgroups, seccomp).
  * Chapters 09-11: The Agent Runtime (ACB lifecycle, WAL event sourcing, Sagas).
  * Chapters 12-14: Policy Optimization (Trajectory harvesting, SFT distillation, RLVR).
...
#### Section 5.2: Memory Fragmentation [core]
- **Heading & Anchor:** `## Memory Fragmentation {#sec-vol3-kvcache-fragmentation}`
...
#### Section 5.4: Prefix Caching [core]
- **Heading & Anchor:** `## Prefix Caching {#sec-vol3-kvcache-radix-tree}`
...
#### Section 5.5: Chunked Prefill Scheduling [core]
...
#### Section 5.7: Cache Capacity Provisioning [core]
- **Heading & Anchor:** `## Cache Capacity Provisioning {#sec-vol3-kvcache-capacity-planning}`
...
- Subsystem Under Construction: Part II, Chapter 06 (Persistent Storage).
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context), Chapter 05 (Paged Attention Memory).
...
#### Section 6.3: Structured Retrieval [core]
- **Heading & Anchor:** `## Structured Retrieval {#sec-vol3-persistent-bm25}`
...
#### Section 6.4: Hybrid Retrieval [core]
- **Heading & Anchor:** `## Hybrid Retrieval {#sec-vol3-persistent-dense-retrieval}`
...
#### Section 6.5: Multi-Hop Retrieval [core]
- **Heading & Anchor:** `## Multi-Hop Retrieval {#sec-vol3-persistent-graphrag}`
...
#### Section 6.6: Storage Invalidation [core]
- **Heading & Anchor:** `## Storage Invalidation {#sec-vol3-persistent-invalidation}`
...
#### Section 6.7: Retrieval Evaluation [core]
- **Heading & Anchor:** `## Retrieval Evaluation {#sec-vol3-persistent-governance-eval}`
...
- `::: {.callout-chapter-connection title="From Stored Evidence to External Effects"}`
  - Handoff forward: A system that can compute and retrieve still cannot complete a task until it can observe and change its environment. In Part III (Execution Sandboxing), Chapter 7 develops typed action and observation interfaces.

---

## Part III: Execution Sandboxing
...
- **Curricular Role in Volume III:** *"Bridging Candidate Tokens to External Execution."* Chapters 02–06 established the computational engine, reasoning loops, working memory, attention caches, and persistent stores of the agentic system. This chapter marks the fundamental transition from internal token generation to external real-world actuation. It examines how candidate token sequences are translated across a trust boundary into typed RPC dispatches, standardizes interoperability via the Model Context Protocol (MCP), enforces execution idempotency under network failure, bounds and sanitizes massive streaming outputs, handles asynchronous execution latencies, and balances tool catalog granularity against token costs.

#### The Curricular Compass (Where We Are in the 18 Chapters)

```
[SYSTEMS STATE AT CHAPTER 07]:
- Subsystem Under Construction: Part III, Chapter 07 (Tool Execution).
- Computational Scope: Tool subsystem architecture, typed tool RPC interface contracts (OpenAPI/JSON Schema), interoperable tool protocols (Model Context Protocol / MCP), idempotent execution and retry safety, observation stream truncation and backpressure, terminal sanitization and exit code preservation, asynchronous non-blocking tool dispatch, and tool library granularity partitioning.
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context), Chapter 05 (Paged Attention Memory), Chapter 06 (Persistent Storage).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 07):
  * Chapter 08: Environmental Isolation (MicroVMs, Firecracker, gVisor, Linux cgroups, seccomp-bpf, OverlayFS copy-on-write isolation, network egress filtering). *Chapter 07 handles the typed interface, parameter serialization, and observation capture; Chapter 08 provides the isolation boundary.*
  * Chapters 09–11: The Agent Runtime (Agent Control Blocks / ACB, multi-turn scheduling loops, Write-Ahead Log event sourcing, distributed saga compensation).
  * Chapters 12–14: Policy Optimization (Trajectory data harvesting, SFT action-loss distillation, RLVR verifiable rewards).
  * Chapters 15–18: Multi-Agent Coordination, Observability & Evaluation, Performance Economics, Capstone Synthesis.
```
...
#### Section 7.1: Tool Subsystem Architecture [stage-setter]
- **Heading & Anchor:** `## Tool Subsystem Architecture {#sec-vol3-actuation-unix-analogy}`
...
- **Curricular Placement:** Establishes the external execution boundary of the agentic system where stochastic model output transitions into deterministic external side effects.
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *Possible focus (Architectural Stage-Setting):* The External Execution Boundary. Moving from the internal computational core (Ch 2), reasoning loops (Ch 3), working memory (Ch 4), attention caches (Ch 5), and durable storage (Ch 6) to external actuation. A candidate token sequence has zero external effect until interpreted, validated, authorized, and dispatched by the host runtime.
  - *Possible focus (The Systems Problem & Operational Reality):* The Classical I/O Abstraction (Ritchie & Thompson 1974: "everything is a file" via `open`, `read`, `write`, `close`, `ioctl`) versus the Agent Tool Interface: "everything is an RPC tool dispatch" (tool definition schema, parameter binding, runtime authorization gate, execution handle, structured observation envelope).
  ...
  - *Possible focus (Computational Boundary & Analytical Handoff):* The 4-Stage Tool Execution Lifecycle: (1) Proposal validation against schema, (2) Permission and capability gating, (3) Dispatched execution, (4) Observation normalization and context staging. Concludes with handoff to Section 7.2.
- **What NOT to Cover (Negative Scope & Forward Deferrals):**
  - 🛑 **DO NOT** explain hypervisor sandboxing, Firecracker microVMs, or cgroups (Deferred exclusively to Chapter 08: Environmental Isolation).
...
- **Curricular Placement:** The syntactic and semantic contract for tool definition.
- **What to Cover (Positive Scope & Systems Mechanics):**
...
- **Curricular Placement:** Cross-process and distributed tool integration protocol.
- **What to Cover (Positive Scope & Systems Mechanics):**
...
- **Curricular Placement:** Ingestion and buffering pipeline between tool processes and context staging.
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *The Output Volume Hazard:* The asymmetry between tiny action commands (`find / -name "*.py"`, `pytest -v`) and massive tool execution output streams orders of magnitude larger than the model's total context capacity.
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *The Timescale Mismatch:* Token decode latency ($10\text{--}40\text{ ms}$) vs. tool execution (hundreds of milliseconds to hours).
...
- **What NOT to Cover (Negative Scope & Forward Deferrals):**
  ...
  - 🛑 **DO NOT** implement full Supervisory runtime scheduler and ACB queues (Deferred to Chapter 09).
- **Visuals & Tables:**
...
- **Authoritative Synthesis:** Synthesizing tool interfaces and execution sandboxing.
- `::: {.callout-takeaways title="Core Systems Principles of Tool Actuation"}`
  1. *Tools are external execution endpoints: typed schemas define their interface contract.*
  2. *The Model Context Protocol (MCP) standardizes external tool and resource connectivity across clients and servers.*
  3. *Idempotency keys are mandatory for all non-idempotent mutating tool calls.*
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context), Chapter 05 (Paged Attention Memory), Chapter 06 (Persistent Storage), Chapter 07 (Tool Execution).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 08):
  * Chapters 09–11: The Agent Runtime (Agent Control Blocks / ACB, multi-turn scheduling loops, Write-Ahead Log event sourcing, distributed saga compensation). *Chapter 08 provides the isolated execution environment; Chapter 09 provides the supervisor and scheduling control plane that allocates tasks to these sandboxes.*
  * Chapters 12–14: Policy Optimization (Trajectory data harvesting, SFT action-loss distillation, RLVR verifiable rewards).
  * Chapters 15–18: Multi-Agent Coordination, Observability & Evaluation, Performance Economics, Capstone Synthesis.
```
...
- **Curricular Placement:** Establishes the security and containment boundary of the agentic system.
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *Possible focus (Architectural Stage-Setting):* The Isolation Boundary. Moving from tool contracts (Ch 7) to containment. Tool execution gives models the power to mutate disk, network, and compute state; the host runtime must assume the model is untrusted or compromised.
...
- **What NOT to Cover (Negative Scope & Forward Deferrals):**
  - 🛑 **DO NOT** implement full Agent Runtime process scheduling (Deferred to Chapter 09).
...
- **Authoritative Synthesis:** Synthesizing virtualization and execution isolation for the agentic system.
...
- `::: {.callout-chapter-connection title="From Isolated Tools to Runtime Orchestration"}`
  - Handoff forward: We now have an isolated computational core, hierarchical memory, and sandboxed tool interfaces. But who coordinates long-running execution trajectories across hours or days, manages process lifecycles, traps user interrupts, and ensures crash recovery? In Part IV (*Runtime Orchestration*), Chapter 09 (*Supervisory Control Planes*), we begin analyzing the agent runtime that governs autonomous processes.

---

## Part IV: Runtime Orchestration
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context), Chapter 05 (Paged Attention Memory), Chapter 06 (Persistent Storage), Chapter 07 (Tool Execution), Chapter 08 (Environmental Isolation).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 09):
  * Chapter 10: Trajectory Persistence (Write-Ahead Logging / WAL, deterministic event sourcing, state checkpointing schemas, trajectory replay mechanics). *Chapter 09 defines the in-memory ACB and supervisor state machine; Chapter 10 makes that state durable and crash-resilient.*
  * Chapter 11: Fault Recovery (Distributed saga pattern, compensating transactions, semantic watchdog reconcilers).
  * Chapters 12–14: Policy Optimization (Trajectory harvesting, SFT distillation, RLVR verifiable rewards).
  * Chapters 15–18: Multi-Agent Coordination, Observability & Evaluation, Performance Economics, Capstone Synthesis.
```
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  ...
  - *Possible focus (The Systems Confrontation):* The Classical OS Analogy (Saltzer & Kaashoek 2009). How classical operating systems enforce isolation, arbitrate shared resources, virtualize hardware, and govern process lifecycles. Adapting these timeless principles to the agentic system: the runtime as a deterministic supervisor governing stochastic model invocations.
...
- **Visuals & Tables:**
  - Architecture comparison diagram (@fig-vol3-controlplane-arch): Ad-hoc while-loop architecture vs. Agent Runtime supervisory control plane.
- **Seminal Literature:**
...
- **The Single Key Point:** The fundamental unit of execution in an agent runtime is the trajectory process, formally tracked via an Agent Control Block (ACB) analogous to an OS Process Control Block (PCB).
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *The Formal Lifecycle States:*
    ...
    - `WAITING_IO`: Suspended awaiting asynchronous tool completion (Ch 7).
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *The Human-in-the-Loop Asynchronous Gateway Architecture:* Treating human operators as high-latency, asynchronous external decision endpoints; suspending the ACB to `WAITING_ESCROW` and releasing compute resources.
...
- **Authoritative Synthesis:** Synthesizing the Runtime Orchestration.
- `::: {.callout-takeaways title="Core Systems Principles of the Runtime Orchestration"}`
  ...
  4. *Cooperative yielding during tool waits decouples compute allocation from tool execution latency.*
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context), Chapter 05 (Paged Attention Memory), Chapter 06 (Persistent Storage), Chapter 07 (Tool Execution), Chapter 08 (Environmental Isolation), Chapter 09 (Runtime Orchestration).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 10):
  * Chapter 11: Fault Recovery (Distributed saga pattern, compensating transactions, semantic watchdog reconcilers). *Chapter 10 provides the durable event log and checkpoint storage; Chapter 11 provides the transactional failure compensation and recovery logic.*
  * Chapters 12–14: Policy Optimization (Trajectory harvesting, SFT distillation, RLVR verifiable rewards).
  * Chapters 15–18: Multi-Agent Coordination, Observability & Evaluation, Performance Economics, Capstone Synthesis.
```
...
- **The Single Key Point:** Runtimes must strictly enforce Write-Ahead Logging (WAL): never dispatch a mutating tool action before the corresponding event record is durably flushed to non-volatile storage via `fsync()`.
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  ...
  - *The Agent Write-Ahead Logging Invariant:* An agent runtime must never dispatch an external mutating tool call ($a_{\text{perm}}$) until the `ActionAuthorized` event is safely written and synced to disk via `fsync()`.
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *The Mechanics of Trajectory Reconstruction:* Re-hydrating the ACB from the nearest checkpoint, then sequentially applying recorded model completions and tool observation records; bypassing the model inference engine and external tool dispatch entirely.
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *Taxonomy of Trajectory Entropy Sources:*
    ...
    4. *External Environment Mutation:* External API payloads, web search results, and database records changing between live execution and replay.
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  ...
  - *The Tripartite State Migration Protocol:*
    ...
    3. *Tool Channel Re-binding:* Revoking existing sandbox socket handles and re-opening network connections and subagent IPC channels on the destination host.
...
- **Curricular Role in Volume III:** *"Resilience Beyond Database ACID."* Chapter 10 provided the durable storage substrate for event logs and snapshots. However, storing records of what happened does not solve the problem of partial failure: when an agent executing a 10-step mutation fails on Step 6, classical Two-Phase Commit ($2\text{PC}$) is impossible across real-world APIs. This chapter establishes the fault tolerance architecture of the Agent Runtime: the Fail-Plausible fault model, distributed Saga transactions, forward self-healing versus backward compensation, pivot actions and irreversibility boundaries, semantic watchdog progress timers, tool circuit breakers, blast radius quarantine, and end-to-end fault-tolerant system synthesis.

#### The Curricular Compass (Where We Are in the 18 Chapters)

```
[SYSTEMS STATE AT CHAPTER 11]:
- Subsystem Under Construction: Part IV, Chapter 11 (Fault Recovery).
- Computational Scope: Fail-Plausible fault model, distributed Saga transactions, forward self-healing vs. backward semantic compensation, pivot actions and irreversibility boundaries, semantic watchdog progress monitors, tool circuit breakers and bulkheads, blast radius quarantine protocols, and end-to-end fault-tolerant system synthesis.
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context), Chapter 05 (Paged Attention Memory), Chapter 06 (Persistent Storage), Chapter 07 (Tool Execution), Chapter 08 (Environmental Isolation), Chapter 09 (Runtime Orchestration), Chapter 10 (Trajectory Persistence).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 11):
  * Chapters 12–14: Policy Optimization (Trajectory data harvesting, SFT action-loss distillation, RLVR verifiable rewards). *Chapter 11 recovers from runtime errors online; Chapters 12–14 compile recurring execution traces offline into updated neural weights.*
  * Chapters 15–18: Multi-Agent Coordination, Observability & Evaluation, Performance Economics, Capstone Synthesis.
```
...
#### Section 11.1: The Transactional Boundary Collapse [stage-setter]
- **Heading & Anchor:** `## The Transactional Boundary Collapse {#sec-vol3-sagas-acid-boundary}`
...
#### Section 11.6: Tool Circuit Breakers [core]
- **Heading & Anchor:** `## Tool Circuit Breakers {#sec-vol3-sagas-containment}`
...
- **Curricular Placement:** Tool fault isolation, cascading failure prevention, and traffic shaping.
- **What to Cover (Positive Scope & Systems Mechanics):**
...
- **Curricular Placement:** Culminating system synthesis and empirical evaluation of Part IV (Runtime Orchestration).
- **What to Cover (Positive Scope & Systems Mechanics):**
...
- `::: {.callout-chapter-connection title="From Runtime Systems to Policy Compilers"}`
  - Handoff forward: The runtime now reliably records both accepted work and recurring failures. Some failures call for better tools or state handling; others reveal fundamental gaps in the model's policy. In Part V (*Policy Optimization*), Chapter 12 (*Trajectory Harvesting*), we transition from runtime systems to offline data curation, harvesting execution traces to compile recurring errors into fine-tuning and reinforcement learning signal.

---

## Part V: Policy Optimization
...
- **Curricular Role in Volume III:** *"From Runtime State to Policy Learning."* In Parts I–IV, we constructed the complete online execution machinery of the agentic system: the inference engine, memory hierarchy, sandboxed tool interfaces, and agent runtime. However, a durable runtime records both accepted work and recurring failures. Chapter 12 initiates Part V (*Policy Optimization*) by answering the foundational data engineering question: before initiating expensive policy training, how do systems engineers diagnose whether a failure is a true model defect or a runtime flaw? This chapter establishes the pipeline for task fixture design, staged verifier cascades, recovery demonstration curation, distributed collection infrastructure, provenance and secret redaction, split hygiene, and end-to-end data harvesting synthesis.

#### The Curricular Compass (Where We Are in the 18 Chapters)

```
[SYSTEMS STATE AT CHAPTER 12]:
- Subsystem Under Construction: Part V, Chapter 12 (Trajectory Harvesting).
...
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 12):
  * Chapter 13: Supervised Adaptation (SFT loss masking, sequence packing, LoRA PEFT memory, gradient backpropagation). *Chapter 12 harvests and curates trajectory datasets; Chapter 13 executes the supervised gradient updates.*
  * Chapter 14: Verifiable Reinforcement Learning (RLVR, GRPO, policy gradient updates, exploratory rollouts).
  * Chapters 15–18: Multi-Agent Coordination, Observability & Evaluation, Performance Economics, Capstone Synthesis.
```
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *Possible focus (Architectural Stage-Setting):* Locate trajectory harvesting at the output of the Agent Runtime (Chapters 09–11); explain why runtime logs are chaotic raw artifacts requiring forensic classification before entering training pipelines.
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *The 5-Component Anatomy of a Task Fixture:*
    ...
    2. *Tool Interface Manifest:* Strict, versioned OpenAPI/JSON schemas with mock endpoints where necessary.
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *The Complete Data Harvesting Architecture:* Step-by-step trace walkthrough of a raw execution trace progressing through the pipeline:
    1. Raw Execution Capture from Agent Runtime (WAL/ACB records).
...
- Subsystem Under Construction: Part V, Chapter 13 (Supervised Adaptation (SFT & Distillation)).
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context), Chapter 05 (Paged Attention Memory), Chapter 06 (Persistent Storage), Chapter 07 (Tool Execution), Chapter 08 (Environmental Isolation), Chapter 09 (Runtime Orchestration), Chapter 10 (Trajectory Persistence), Chapter 11 (Fault Recovery), Chapter 12 (Trajectory Harvesting).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 13):
  * Chapter 14: Verifiable Reinforcement Learning (RLVR, GRPO advantage estimation, reward oracles, exploratory rollouts). *Chapter 13 adapts weights from demonstrated historical traces; Chapter 14 learns from online trial-and-error exploration against reward oracles.*
  * Chapters 15–18: Multi-Agent Coordination, Observability & Evaluation, Performance Economics, Capstone Synthesis.
```
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context), Chapter 05 (Paged Attention Memory), Chapter 06 (Persistent Storage), Chapter 07 (Tool Execution), Chapter 08 (Environmental Isolation), Chapter 09 (Runtime Orchestration), Chapter 10 (Trajectory Persistence), Chapter 11 (Fault Recovery), Chapter 12 (Trajectory Harvesting), Chapter 13 (Supervised Adaptation).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 14):
  * Chapters 15–18: Multi-Agent Coordination, Observability & Evaluation, Performance Economics, Capstone Synthesis. *Chapter 14 trains a single agent's policy via environmental RL; Chapter 15 coordinates distributed fleets of collaborating agents.*
```
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *Possible focus (Architectural Stage-Setting):* Position RLVR at the apex of policy optimization (Part V); contrast the imitation ceiling of SFT with closed-loop policy improvement through environment interaction.
...
- `::: {.callout-chapter-connection title="From Single-Agent Learning to Multi-Agent Distributed Fleets"}`
  - Handoff forward: We have now explored the complete lifecycle of the single-agent Agentic System: its inference engine (Ch 2–3), memory hierarchy (Ch 4–6), sandboxed tool interfaces (Ch 7–8), agent runtime (Ch 9–11), and policy optimization pipeline (Ch 12–14). However, enterprise production problems frequently exceed the latency, context, and specialization boundaries of any single agent. In Part VI (*Distributed Systems*), Chapter 15 (*Multi-Agent Coordination*), we transition from single-agent runtimes to distributed fleets of collaborating, stateful agent processes.
...
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 15):
  * Chapter 16: System Observability (OpenTelemetry spans, SWE-bench evaluation harness, statistical confidence intervals). *Chapter 15 coordinates fleet execution; Chapter 16 instruments and benchmarks fleet telemetry.*
  * Chapter 17: Serving Economics (Roofline models, GPU cluster sizing, speculative decoding, tokenomics governance).
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *Possible focus (Architectural Stage-Setting):* Position multi-agent systems within the broader architecture of the agentic system; define delegation as spawning concurrent foundation model engine cores across the Agent Runtime.
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  ...
  - *Amdahl's Speedup Formulation for Multi-Agent Coordination:*
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context), Chapter 05 (Paged Attention Memory), Chapter 06 (Persistent Storage), Chapter 07 (Tool Execution), Chapter 08 (Environmental Isolation), Chapter 09 (Runtime Orchestration), Chapter 10 (Trajectory Persistence), Chapter 11 (Fault Recovery), Chapter 12 (Trajectory Harvesting), Chapter 13 (Supervised Adaptation), Chapter 14 (Verifiable Reinforcement Learning), Chapter 15 (Multi-Agent Coordination).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 16):
  * Chapter 17: Serving Economics (Critical-path Amdahl latency modeling, speculative decoding, multi-level feedback queue cluster sizing, tokenomics governance). *Chapter 16 measures what occurred and whether it succeeded; Chapter 17 optimizes latency, throughput, and financial cost.*
  * Chapter 18: System Synthesis (Capstone Reference Architecture).
```
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *Possible focus (Architectural Stage-Setting):* Define evaluation in the agentic system as verifying the final state delta against the formal task contract. Contrast with classical Software 1.0 (unit tests with deterministic assertion paths).
...
- Subsystems Active: Chapter 01 (Trajectory Architecture), Chapter 02 (The Foundation Model Engine), Chapter 03 (Test-Time Deliberation), Chapter 04 (Working Context), Chapter 05 (Paged Attention Memory), Chapter 06 (Persistent Storage), Chapter 07 (Tool Execution), Chapter 08 (Environmental Isolation), Chapter 09 (Runtime Orchestration), Chapter 10 (Trajectory Persistence), Chapter 11 (Fault Recovery), Chapter 12 (Trajectory Harvesting), Chapter 13 (Supervised Adaptation), Chapter 14 (Verifiable Reinforcement Learning), Chapter 15 (Multi-Agent Coordination), Chapter 16 (System Observability).
- Subsystems NOT YET BUILT (STRICTLY FORBIDDEN TO ASSUME OR EXPLAIN IN CHAPTER 17):
  * Chapter 18: System Synthesis (Capstone Reference Architecture). *Chapter 17 optimizes performance and costs; Chapter 18 synthesizes the entire Agentic System across all 18 chapters.*
```
...
#### Section 17.8: Serving Economics Synthesis [synthesis]
- **Heading & Anchor:** `## Serving Economics Synthesis {#sec-vol3-tokenomics-synthesis}`
...
- `::: {.callout-chapter-connection title="From Fleet Operations to the Capstone System Synthesis"}`
  - Handoff forward: We have now traversed the entire curriculum of Agentic Machine Learning Systems: the Foundation Model Engine (Part I), the Memory Hierarchy (Part II), Execution Sandboxing (Part III), the Agent Runtime (Part IV), policy optimization (Part V), and Distributed Fleet Operations (Part VI). In Part VII (*System Synthesis*), Chapter 18 (*Architectural Synthesis*), we bring every single invariant, interface, and trade-off together into the capstone reference architecture, synthesizing the agentic system into a cohesive, enduring engineering discipline.
...
- **Core Takeaway:** *The agentic system is an accountable execution loop whose model invocation, state, action boundary, supervisor, learning pipeline, and fleet operations can all be traced to a task contract and tested against accepted outcomes.*
...
- **Curricular Role in Volume III:** *"Capstone Synthesis of Agentic Systems."* Across seventeen chapters, we have constructed the complete anatomy of Agentic Machine Learning Systems: The Computational Engine (Part I), The Memory Hierarchy (Part II), Execution Sandboxing (Part III), Runtime Orchestration (Part IV), Policy Optimization (Part V), and Distributed Systems (Part VI). Chapter 18 serves as the culminating capstone synthesis of the entire volume. It unites every invariant, interface, and operational trade-off into an end-to-end production reference architecture. Rather than treating the agentic system as an abstract analogy or a collection of isolated libraries, this chapter walks through a complete reference trajectory from task delegation to verified completion. It establishes the 5-part workload contract and operating envelope, synthesizes the memory and execution hierarchies, formalizes the evidence-based systems intervention ladder, constructs multi-tier empirical safety cases, deconstructs Brooksian essential complexity in the era of Software 3.0, and delineates the permanent boundary separating digital agent execution from physical embodied agency.

#### The Curricular Compass (Where We Are in the 18 Chapters)

```
[SYSTEMS STATE AT CHAPTER 18]:
- Subsystem Under Construction: Part VII, Chapter 18 (System Synthesis: Architectural Synthesis).
...
**The Core Question:** *How do all eighteen subsystems—computational engine, memory hierarchy, execution sandboxes, supervisory runtimes, policy optimization, and distributed fleets—integrate into an accountable, end-to-end production agentic system?*

**Why It Matters:** *Building an agentic system is not an exercise in gluing together disparate LLM wrapper libraries. Reliability requires an unbroken chain of architectural contracts: unprivileged inference engines bounded by typed envelopes, working sets pruned by deterministic eviction, tool mutations contained within microVM sandboxes, multi-step actions coordinated by distributed Sagas, and policy adaptations compiled from mechanically verified traces. This capstone chapter traces a complete, real-world task trajectory through the entire architecture, establishing formal safety cases, evaluating architectural trade-offs, and confronting the boundary between software agency and physical embodied systems.*
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *Possible focus (Architectural Stage-Setting):* Revisit the central thesis of Chapter 01: the agentic system is an accountable execution loop, not an anthropomorphic intelligence. Synthesize the core model: a non-deterministic token prediction engine governed by deterministic systems invariants.
  - *Possible focus (The Reference Trajectory Walkthrough):* Trace one end-to-end enterprise incident remediation task from initial alert trigger to closed pull request, explicitly mapping every action to its subsystem owner (Computational Engine, Memory Hierarchy, Execution Sandboxes, Supervisory Runtime, Sagas, Telemetry).
  - *Possible focus (Live Owners vs. Operational Infrastructure):* Explicitly delineate live runtime owners (Inference Engine, Working Memory, KV Cache, Tool Sandbox, OS Supervisor) from across-task lifecycle infrastructure (Trajectory Data Engine, Policy SFT/RLVR Compiler, OpenTelemetry Tracing, SRE Canary Gates).
...
- **Visuals & Tables:**
  - Comprehensive Architecture Blueprint (@fig-vol3-capstone-reference-architecture): End-to-end trajectory flow across Inference Engine, Context Memory, KV Cache, Tool Sandbox, Supervisory Runtime, WAL Storage, Sagas, and Canary Gates.
- **Seminal Literature:**
...
- **What to Cover (Positive Scope & Systems Mechanics):**
  - *The 6-Level Systems Intervention Ladder:*
    ...
    - *Level 4: Supervised Adaptation (SFT):* Compiling established procedures, format discipline, and recovery demonstrations into weights.
...
- **Causal Bridge to 18.7:** As we reflect on the capabilities of the agentic system, what timeless software engineering principles govern what AI can and cannot solve?
...
- `::: {.callout-closing title="The Architect's Responsibility: From Wilkes to Software 3.0"}`
  ...
  - The foundation model supplies learned computation, while the surrounding system supplies state selection, controlled interaction, supervision, recovery, and evidence of completion. Treating those responsibilities as one accountable trajectory is the systems discipline of the agentic system.
