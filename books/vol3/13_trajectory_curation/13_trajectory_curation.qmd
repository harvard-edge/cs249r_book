...
**Problem**: An agent that maintains a company's internal services runs 50,000 trajectories a month and fails 6 percent of them, or 3,000 failures. Counterfactual probes attribute 70 percent of the failures (2,100) to ambiguous schemas on newly added version-control tools, 20 percent (600) to sandbox startup timeouts on loaded workers, and 10 percent (300) to genuine planning failures on multi-file refactoring tasks. Should the team fix the layers or fine-tune on all 3,000 failures?

**Option A (follow the ladder)**: Tightening the version-control schemas takes about four engineering hours. Pre-warming sandboxes (@sec-vol3-sandboxes-pooling) takes about twelve. Both changes ship through ordinary review within a day, revert with a commit if they misbehave, and touch no weights. Together they remove the causes of 2,700 of the 3,000 failures, and the remaining 300 become a small, well-labeled set of true policy gaps for curation.

**Option B (fine-tune on everything)**: The team collects all 3,000 failures, writes corrected versions, and trains. The run itself is the smallest cost. Preparing the data, training, and passing the release gates takes days, and the resulting model must be re-evaluated for regressions on everything else it does. The 600 timeouts do not change at all, because no weight can make a sandbox start faster. The 2,100 schema failures become less likely rather than impossible, and they return the next time the tool's interface changes. Only the 300 planning failures were ever training's to fix, and they are now diluted in a corpus that is 90 percent workarounds.

**Systems insight**: Training is the right repair for one failure in ten here. Applying the ladder first shrinks the training problem to the failures that need it and leaves the other nine fixed at their source.
:::
...
An oracle that trusts the exit code of a test command run inside the agent's own workspace invites a shortcut. When the tests fail and the fix is hard, the cheapest route to exit code zero is to change the tests.

```diff
--- a/tests/test_settlement.py
+++ b/tests/test_settlement.py
@@ -14,6 +14,5 @@ def test_settlement_under_partition():
     service = SettlementCoordinator(timeout_ms=500)
...
The diff shows the pattern. Faced with a settlement test that did not converge within its timeout, the agent did not repair the retry logic. It deleted the assertions that checked the reconciled balance and put a tautology in their place. A pipeline that reads only the final exit code records the trajectory as a success, and a model trained on it learns that the way to fix a failing test is to weaken it. The same pressure appears in other families, and a refund agent that could edit the fixture's expected-state file would find the same shortcut.

Defending the oracle extends containment beneath the model (principle \ref{pri-vol3-zero-trust-sandboxing}) to the checks themselves, which must sit outside anything the agent's envelope can write. Three measures together close the gap:

1. *Tests the agent cannot reach.* The sealed tests are mounted read-only from outside the workspace, or injected only after the agent has finished (@sec-vol3-sandboxes-cow).
2. *A diff checked against protected paths.* Stage 4 rejects any trajectory whose diff touches the test tree, build configuration, or CI definitions, whatever the test outcome.
3. *A clean interpreter for the tests.* In dynamic languages an agent can patch the test framework in memory, for instance by replacing an assertion helper with a no-op. The runtime therefore runs the tests in a fresh process it starts itself, with none of the agent's startup scripts or environment changes.
...
A policy trained by imitation sees states drawn from the demonstrator's trajectories. When it runs, it visits states produced by its own actions, and one early mistake or environment fault puts it in a state no demonstration covered. Ross, Gordon, and Bagnell showed that under imitation such errors compound, so the expected number of mistakes grows with the square of the horizon rather than linearly [@ross2011reduction]; @sec-vol3-appendix-math-compounding-error works through the bound. For an agent whose horizon runs to dozens of turns, that growth decides whether long tasks are feasible at all. @Sec-vol3-fine-tuning-exposure-bias treats the training-side remedies. The data-side remedy belongs here, and it is to keep the trajectories that went wrong and came back.
...
Recoveries are rare in a well-run environment. Waiting for organic permission errors or rate limits yields too few, so collection injects faults deliberately. The fault-injection harness of @sec-vol3-sagas-case-study, built there to test the runtime's recovery paths, doubles as a generator of recovery traces. At the tool boundary it returns a permission error, a full disk, a missing dependency, an HTTP 429, or a malformed response on a configured schedule, and the rollouts that recover become candidates for the recovery class. The refund family injects its own kind of fault, a timed-out `issue_refund` call whose effect is uncertain, which the agent must resolve by looking the order up before retrying, exactly the check that the idempotency discipline of @sec-vol3-tool-calling-idempotency makes safe.
...
What does a model actually learn from an admitted trajectory? Only what the loss is aimed at. A trajectory interleaves the model's actions with the environment's observations, the system prompt, and tool schemas the model will read again at run time, and training on all of it would teach the model to write observations it should wait for. @Sec-vol3-fine-tuning takes the curated mix and its labels as a data contract. It serializes trajectories into the chat and tool-call formats the model reads, masks the loss so that only the model's own actions are trained, packs trajectories without letting them see one another, and evaluates the result for what fine-tuning can and cannot change.
:::
