# Agent System Tokenomics {#sec-vol3-tokenomics}
...
To appreciate the physical reality behind this memory stall, consider the time required to emit a single token on an NVIDIA H100 SXM5 ($B_{\text{mem}} = 3.35\text{ TB/s} = 3,350\text{ GB/s}$). Generating one token from a 70B parameter model in FP16 ($140\text{ GB}$ of weights) requires streaming the entire $140\text{ GB}$ across the memory bus:

$$T_{\text{token}}(B=1) = \frac{140\text{ GB}}{3,350\text{ GB/s}} \approx 0.0418\text{ seconds} = 41.8\text{ ms}$$

This imposes an insurmountable physical speed ceiling for an unbatched stream:

$$\text{Speed}_{\max} = \frac{1 \text{ token}}{0.0418 \text{ s}} \approx 23.9 \text{ tokens/second}$$

Even if the GPU possessed infinite ALUs operating at zero picoseconds of compute latency, single-stream generation could not exceed 24 tokens per second. The execution speed is governed not by compute capacity, but by the physical pin bandwidth of the silicon interposer.
...
To ground this trade-off in physical numbers, consider hosting Llama-3-70B on an 8-way H100 SXM5 node rented at $\$32.00/\text{hr}$ ($\$4.00/\text{GPU-hr}$). Each GPU provides 80 GB of HBM3, yielding an effective memory holding cost:

$$C_{\text{HBM}} = \frac{\$4.00}{3,600\text{ s} \times 80\text{ GB}} \approx \$1.39 \times 10^{-5} \text{ per GB}\cdot\text{second}$$

For an agent holding a $32,000$-token prefix, the KV cache footprint from @eq-vol3-tokenomics-kv-cache-footprint is $32,000 \times 320\text{ KB} \approx 10.24\text{ GB}$. Storing this state in HBM costs:

$$C_{\text{rent}} = 10.24\text{ GB} \times (\$1.39 \times 10^{-5}/\text{GB}\cdot\text{s}) \approx \$1.42 \times 10^{-4} \text{ per second} \quad (\approx \$0.51/\text{hr})$$

Meanwhile, re-prefilling those $32,000$ tokens on an H100 running at $50\%$ MFU ($495\text{ TFLOP/s}$) requires $2 \times 70\times 10^9 \times 32,000 \approx 4.48\text{ PFLOPs}$, taking $9.0\text{ ms}$ of compute time and costing:

$$C_{\text{recompute}} = \frac{\$4.00}{3,600\text{ s}} \times 0.009\text{ s} \approx \$1.00 \times 10^{-5}$$

Dividing the recomputation cost by the holding cost per second yields the break-even idle duration:

$$T_{\text{break-even}} = \frac{\$1.00 \times 10^{-5}}{\$1.42 \times 10^{-4}/\text{s}} \approx 0.07 \text{ seconds}$$
...
- [ ] **Break-Even Threshold ($T_{\text{break-even}}$)**: Why is the holding cost of 10 GB of HBM on an H100 GPU so high that an idle wait of just 0.07 seconds exceeds the cost of recomputing the prefill?
- [ ] **Tiered KV Swapping**: How does paging idle KV cache blocks over PCIe Gen5 x16 simplex ($64\text{ GB/s}$ / $\approx 53\text{ GB/s}$ DMA) to host system DRAM prevent both memory holding penalties and redundant prefill compute?
- [ ] **Prefix Caching ROI**: In a 30-step trajectory with 90 percent prefix cache hit rates, how does caching transform quadratic context prefill accumulation from an economic blocker into a linear cost?
...
Comparing @eq-vol3-tokenomics-cumulative-cached-cost against stateless execution demonstrates that a $75\%\text{--}90\%$ caching discount slashes the multi-turn prefill bill by an order of magnitude, transforming extended agent trajectories from financial anomalies into viable production workloads.
...
2. **The Utilization Cliff $\left(\frac{1}{1 - \rho}\right)$:** Dictated by cluster load. As utilization approaches $100\%$ ($\rho \to 1.0$), queue waiting time diverges asymptotically. Pushing utilization from $70\%$ ($\frac{1}{1-0.7} = 3.33$) to $90\%$ ($\frac{1}{1-0.9} = 10.0$) triples queue waiting delays.
...
- [ ] **The Utilization Cliff**: According to Kingman's formula, why does increasing cluster GPU utilization from 70 percent to 90 percent triple the average queue waiting time?
- [ ] **Heavy-Tailed Variance ($C_s \ge 3.0$)**: Why do multi-turn agent workloads exhibit significantly higher variance than stateless chatbots, and how does this affect tail latency?
- [ ] **The Headroom Reserve Law**: Why is reserving 30 to 35 percent idle accelerator capacity mathematically mandatory rather than wasteful over-provisioning?
...
* **The quadratic prefill turn tax and compression ROI**: In multi-turn agent loops, context accumulation causes total input token processing to explode quadratically ($\mathcal{O}(N^2 \bar{\Delta})$), causing prefill to dominate $80\%–90\%$ of total invoice dollars. Radix prefix caching collapses prefill complexity to linear ($\mathcal{O}(N \bar{\Delta})$), while prompt compression and observation pruning deliver up to $40\times+$ ROI by eliminating multi-turn compounding token taxes.
