...
*Sub-50% task acceptance triggers a hyperbolic cost cliff that overwhelms nominal per-token discounts.*
:::
...
::: {#exmp-17-the-token-price-fallacy-in-automated .callout-example title="The Token-Price Fallacy in Automated Bug Repair"}
...
![**Waterfall Timeline Decomposition of an Autonomous Agent Turn**: Execution alternates between compute-bound prompt prefill, memory-bandwidth-bound autoregressive decoding, host supervisor mediation, isolated sandbox actuation, and external network RPCs.](images/svg/fig-vol3-critical-path-waterfall.svg){#fig-vol3-critical-path-waterfall width="95%"}
...
::: {#exmp-17-amdahls-law-on-an-automated .callout-example title="Amdahl's Law on an Automated Regression Repair Fleet"}
...
*Tiered model cascading resolves 88% of tasks on lightweight models, cutting expected cost by 86% against monolithic frontier routing.*
:::
...
![**Tiered Model Escalation Cascade with Deterministic Verification Checkpoints**: Low-cost engines attempt the task first; execution only escalates to heavier, more expensive models upon explicit verification failure.](images/svg/multi_agent_token_cascade_routing_v2.svg){#fig-vol3-model-cascade-flowchart width="95%"}
...
::: {#exmp-17-cost-and-latency-trade-offs-in .callout-example title="Cost and Latency Trade-offs in a Three-Tier Cascade"}
...
::: {#exmp-17-roofline-bound-decode .callout-example title="Roofline Bound on Decode Phase Throughput"}
...
::: {#exmp-17-sizing-queue-capacity-and-tail .callout-example title="Sizing Queue Capacity and Tail Latency under Heavy-Tailed Agent Traffic"}
...
![**Multi-Level Feedback Queue (MLFQ) Serving Architecture for Agent Fleets**: Interactive probes, intermediate tool loops, and background batch trajectories isolated into priority queues with dynamic quantum allocation and KV-cache tiering.](images/svg/vol3-mlfq-serving-architecture.svg){#fig-vol3-mlfq-serving-architecture width="95%"}
...
::: {#exmp-17-financial-break-even-utilization-for-dedicated .callout-example title="Financial Break-Even Utilization for Dedicated Accelerator Fleets"}
...
![**The Hierarchical Budget Reservation Ledger protocol**: A root agent allocates non-overlapping budget reservations into child escrow accounts. In-flight operations draw strictly from local escrow; upon child termination, unexpended capital is refunded upward to the parent's free balance.](images/svg/hierarchical-budget-ledger.svg){#fig-vol3-hierarchical-budget-ledger width="95%"}
...
::: {#exmp-17-hierarchical-multi-agent-budget-allocation-and .callout-example title="Hierarchical Multi-Agent Budget Allocation and Recovery"}
...
![**Dimensional Trade-Offs Across the Architectural Autonomy Spectrum**: Moving from Software 1.0 to Autonomous Fleets monotonically increases flexibility and error surface while shifting latency, cost, and verification complexity by orders of magnitude.](images/svg/fig-vol3-architectural-radar-chart.svg){#fig-vol3-architectural-radar-chart width="95%"}
...
::: {#pri-17-end-to-end-verification .callout-principle title="The End-to-End Verification Principle in Agent Runtimes"}
...
::: {#exmp-17-architectural-frontier-migration-under-cost .callout-example title="Architectural Frontier Migration Under Cost and Latency Scaling"}
...
To prevent these failure modes, a production agent platform must structure its control flow as a five-stage pipelined feedback engine (@fig-vol3-fleet-economics-architecture). Each stage enforces a specific architectural boundary, transforming an unprivileged user intent into verified external state modifications while actively minimizing resource consumption.

![**The Fleet Economics Control Loop Architecture**: Five-stage pipelined closed-loop engine integrating admission workload profiling, MLFQ scheduling, tiered cascade routing, speculative verification, execution escrows, and monotonic budget ledgers.](images/svg/fig-vol3-fleet-economics-architecture.svg){#fig-vol3-fleet-economics-architecture width="95%"}
...
::: {#exmp-17-quantitative-evaluation-of-fleet-goodput .callout-example title="Quantitative Evaluation of Fleet Goodput per Dollar"}
...
::: {.callout-takeaways title="Core Systems Principles of Serving Economics and Performance"}
