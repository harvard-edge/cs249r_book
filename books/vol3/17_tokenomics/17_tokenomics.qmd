...
This reveals a startling systems reality: **HBM holding rent is so precious that whenever an agent waits more than a fraction of a second for an external tool (e.g., executing a 5-second bash command or database query), holding the KV cache in active GPU memory is economically worse than recomputing it!** To avoid either penalty, state-of-the-art serving runtimes asynchronously page idle KV blocks out to host system DRAM (via PCIe Gen5 x16 simplex at $64\text{ GB/s}$ theoretical / $\approx 53\text{ GB/s}$ achievable DMA throughput, transferring $10.24\text{ GB}$ in $\approx 195\text{ ms}$) or fast local NVMe SSDs, keeping the HBM free for active decoders while avoiding redundant prefill FLOPs.
