...
For a multi-server cluster with $k$ parallel GPU serving instances ($M/G/k$), queue waiting time can be modeled using **Kingman's Heavy-Traffic Approximation**:
...
Apply the tokenomic Roofline models, capacity sizing equations, and multi-tier cascade formulations established in this chapter to solve the following quantitative problems.

### Problem 1: Roofline analysis, batch amortization, and speculative decoding break-even

An enterprise serving cluster deploys a 70-billion-parameter dense model ($P_{\text{params}} = 70 \times 10^9$) in FP16 precision (2 bytes per parameter) on an NVIDIA H100 SXM5 GPU node ($P_{\text{peak}} = 989.4\text{ TFLOP/s}$, $B_{\text{mem}} = 3.35\text{ TB/s}$). Neglect KV cache read traffic for parts (a) through (c).

1. **Calculate the single-stream arithmetic intensity:** Compute the operational arithmetic intensity $I_{\text{decode}}(B=1)$ and determine the attained compute throughput $P_{\text{attained}}$ in TFLOP/s. What percentage of the H100's peak Tensor Core capability is utilized?
2. **Compute the critical batch size:** Calculate the exact hardware balance knee $I^*$ for the H100 GPU. What is the minimum concurrent batch size $B^*$ required to reach the compute-bound plateau?
3. **Speculative Decoding Break-Even Analysis:** To accelerate decode, systems engineers deploy a 7-billion-parameter draft model ($P_{\text{draft}} = 7 \times 10^9$). In each speculative iteration, the draft model generates $K = 5$ speculative tokens sequentially, and the target model verifies all 5 tokens concurrently in a single batched verification pass.
   - The draft model executes sequential decode steps with per-token latency $t_{\text{draft}} = \frac{2 \cdot P_{\text{draft}}}{B_{\text{mem}}}$.
   - The target model executes a single verification pass for $K$ tokens with latency $t_{\text{verify}} = \frac{2 \cdot P_{\text{params}}}{B_{\text{mem}}} + \frac{2 \cdot K \cdot P_{\text{params}}}{P_{\text{peak}}}$.
   - Let $\alpha \in [0, 1]$ denote the average token acceptance rate, such that the expected number of accepted tokens per speculative iteration is $\mathbb{E}[M] = 1 + \alpha K$.
   - Formulate the effective latency per generated token $T_{\text{token}}(\alpha)$ under speculative decoding.
   - Calculate the critical acceptance rate $\alpha^*$ required for speculative decoding to achieve a lower latency per token than standard autoregressive target decoding ($t_{\text{target}} = \frac{2 \cdot P_{\text{params}}}{B_{\text{mem}}}$).

### Problem 2: Heavy-tailed queueing and multi-server accelerator provisioning

An autonomous code-review agent fleet receives incoming pull request review tasks at a mean arrival rate of $\lambda = 12$ tasks per minute ($\lambda = 0.20$ tasks/second) according to a Poisson arrival process. Empirical measurement reveals that task execution duration follows a heavy-tailed log-normal distribution with mean $\mathbb{E}[S] = 45$ seconds and standard deviation $\sigma = 135$ seconds.

1. **Calculate service distribution parameters:** Compute the service rate $\mu$ per instance, the coefficient of variation $C_s = \sigma / \mathbb{E}[S]$, and the variance penalty factor $\frac{1 + C_s^2}{2}$ under the Pollaczek-Khinchine formulation.
2. **Single-server queue wait:** If the organization attempts to process all traffic on a single consolidated high-throughput serving cluster with aggregate mean service time $\mathbb{E}[S_{\text{agg}}] = 4.0\text{ seconds}$ operating at utilization $\rho = 0.80$, calculate the mean queue waiting time $W_q$. Compare this waiting time to an idealized workload with deterministic service time ($C_s = 0$).
3. **Multi-Server Fleet Capacity Sizing:** Model the fleet as an $M/G/k$ queue using Kingman's multi-server approximation (@eq-vol3-tokenomics-kingman-multiserver) with arrival coefficient of variation $C_a = 1.0$. Each server instance handles individual tasks with mean service duration $\mathbb{E}[S] = 45\text{ seconds}$.
   - Calculate the minimum server count $k_{\text{min}}$ required strictly for system stability ($\rho < 1.0$).
   - Compute the traffic intensity $a = \lambda / \mu$ in Erlangs.
   - Calculate the mean queue waiting time $W_q$ when the cluster is provisioned with $k = 12$ servers ($\rho = 0.75$), $k = 15$ servers ($\rho = 0.60$), and $k = 20$ servers ($\rho = 0.45$).
   - If the corporate SLA mandates that mean queue waiting time must not exceed $W_q \le 5.0\text{ seconds}$, what is the minimum number of accelerator servers $k^*$ that must be provisioned?

### Problem 3: Multi-tier cascade optimization and CPVG minimization

An enterprise software engineering agent workload evaluates three candidate models for a 25-step code refactoring benchmark:

- **Model A (Small Distilled 8B):** Input price $c_{\text{in}}^{(A)} = \$0.15$ per 1M tokens; Output price $c_{\text{out}}^{(A)} = \$0.60$ per 1M tokens. Single-step correctness probability $p_A = 0.82$. Average trajectory token footprint: $S_{\text{in}} = 20,000$ input tokens (with prefix caching enabled across turns) and $S_{\text{out}} = 1,200$ output tokens.
- **Model B (Domain MoE 8x7B):** Input price $c_{\text{in}}^{(B)} = \$0.80$ per 1M tokens; Output price $c_{\text{out}}^{(B)} = \$2.40$ per 1M tokens. Single-step correctness probability $p_B = 0.92$. Average trajectory token footprint: $S_{\text{in}} = 22,000$ input tokens and $S_{\text{out}} = 1,500$ output tokens.
- **Model C (Frontier Reasoning 400B+):** Input price $c_{\text{in}}^{(C)} = \$5.00$ per 1M tokens; Output price $c_{\text{out}}^{(C)} = \$15.00$ per 1M tokens. Single-step correctness probability $p_C = 0.985$. Average trajectory token footprint: $S_{\text{in}} = 25,000$ input tokens and $S_{\text{out}} = 2,000$ output tokens.

Each trajectory attempt is evaluated by an automated compiler and sandbox test oracle costing $c_{\text{oracle}} = \$0.01$ per execution.

1. **Standalone Model Evaluation:**
   - For each model operating in isolation, compute the single-trajectory token cost $C_{\text{traj}} = C_{\text{in}} + C_{\text{out}} + c_{\text{oracle}}$.
   - Calculate the end-to-end 25-step trajectory success probability $P_{\text{success}} = p^{25}$ for each model under open-loop execution.
   - Calculate the expected Cost-per-Verified-Goal ($\text{CPVG} = C_{\text{traj}} / P_{\text{success}}$) for Model A, Model B, and Model C. Which model is economically optimal in isolation?
2. **Two-Tier Sequential Cascade Design:**
   Architect a cascade where tasks are first attempted by Model A. If Model A passes the verification oracle ($P_{\text{success}}^{(A)}$), the task terminates. If Model A fails, the task escalates to Model C. Assume that when Model C is invoked following a Model A failure, its conditional 25-step success probability is $P_{C \mid \bar{A}} = 0.70$.
   - Calculate the expected cost $\mathbb{E}[C_{\text{cascade}}]$ of this two-tier system per task.
   - Calculate the overall success probability $P_{\text{cascade}}$ of the two-tier cascade.
   - Compute the Cost-per-Verified-Goal ($\text{CPVG}_{\text{cascade}}$) of the two-tier cascade.
   - What percentage cost savings does the cascade achieve compared to executing Model C alone?
