...
::: {#psp-17-the-token-price-fallacy .callout-perspective title="The token-price fallacy in autonomous systems"}
When designing software platforms atop foundation models, engineering teams routinely treat model token tariffs—quoted in dollars per million tokens—as the primary cost driver of their architectures. Selecting a lightweight commodity model with a tenfold cheaper nominal token price is commonly assumed to yield a tenfold reduction in operating costs. In agentic systems operating inside closed-loop environments, this assumption represents an architectural trap known as **the token-price fallacy**.

In single-turn conversational chatbots, operational expenditure scales linearly with token volume: twice the tokens equals twice the bill. In autonomous agent architectures, however, the fundamental unit of accounting is not the token, but the *accepted task*—a verified system state transition that satisfies all test suites and compiler assertions. Because an agent acts under Zero Ambient Authority ($A=0$), it operates across long-horizon causal chains where single-step errors compound exponentially ($P_{\text{success}} \le p^K$).

When an unprivileged model suffers from lower reasoning fidelity or degraded tool-call accuracy, the failure-to-success odds ratio $\frac{1-\alpha}{\alpha}$ surges hyperbolically. A model that drops task acceptance from 80 percent to 20 percent does not merely require more retries; it incurs quadratic prompt prefill growth as context windows balloon with error diagnostics, exhausts container sandbox lifetimes, consumes billable vector search queries, and squanders engineer hours on manual post-mortems. Crucially, non-inference infrastructure expenses—ephemeral microVM hosting, hermetic test runner execution, and human escalation triage—often constitute over 50 percent of total trajectory costs. By triggering repeated execution failures, a "cheap" model rapidly costs several times more per verified deliverable than a frontier model with an order-of-magnitude higher nominal token tariff. Sound agent economics requires optimizing for *goodput per dollar*, never nominal cost per token.
:::
...
In enterprise software engineering, Michael T. Nygard formalized the *Circuit Breaker* pattern to isolate failing network dependencies and prevent cascading distributed collapse [@nygard2007releaseit]. In agentic machine learning systems, the circuit breaker pattern must be repurposed: rather than monitoring network socket timeouts or HTTP 500 error ratios, the agent runtime monitors financial burn velocity and *epistemic utility decay*.
...
Can the host supervisor deterministically verify whether the model's output satisfies the problem requirements? This question represents the primary governing constraint of agentic reliability, deriving directly from the seminal End-to-End Argument of @saltzer1984endtoend.

::: {#pri-17-end-to-end-verification .callout-principle title="End-to-end verification in serving economics"}
Applying the End-to-End Argument of @saltzer1984endtoend (introduced in @sec-vol3-intro-invariant-closure): the foundation model represents an unprivileged, lossy communication channel between user intent and physical software execution. Attempting to enforce semantic correctness inside the neural core itself—via prompt steering, chain-of-thought reflection, or probabilistic confidence metrics—violates the end-to-end principle. The model lacks internal ground truth to verify its own semantic soundness. Correctness can be certified completely and reliably *only* at the application endpoints: the host supervisor executing external, deterministic verifiers (compilers, linters, unit test suites, and cryptographic diff assertions). Tasks lacking deterministic endpoint verifiers must never receive unbounded loop budgets ($K \to \infty$), which result in non-convergent, infinite token expenditure.
:::
