...
Consider a real-world case study: an engineering team spent three months optimizing GPU inference kernels, successfully doubling large language model (LLM) generation speed from 30 tokens per second to 60 tokens per second. When deployed to production coding agents, overall task completion latency decreased by only 4.8 percent. A distributed trace analysis (@sec-vol3-observability-tracing) revealed why: 85 percent of the agent's wall-clock time was spent waiting for remote CI/CD compilation jobs and Docker container spin-up latencies. Doubling model generation speed had targeted a non-critical-path component.
...
![](images/svg/multi_agent_token_cascade_routing_v2.svg){width="100%"}
:::
...
In standard autoregressive generation, generating each token requires loading all tens of billions of model parameters from off-chip HBM into on-chip registers. For a single agent stream (batch size $B = 1$), compute units sit idle waiting for memory controllers, achieving less than 10 percent of peak GPU floating-point capacity (@sec-vol3-processor-analogy).

The physical hardware dynamics governing this memory-bandwidth bottleneck are captured by the operational roofline model shown in @fig-vol3-tokenomics-roofline. On modern accelerator hardware (such as an NVIDIA H100 SXM5), peak compute throughput reaches $\Pi = 989\text{ TFLOP/s}$ while high-bandwidth memory (HBM3) provides $\beta = 3.35\text{ TB/s}$, defining a critical arithmetic intensity ridge at $I^* = \Pi / \beta \approx 295.2\text{ FLOP/Byte}$. Single-stream autoregressive decoding ($B=1$) operates as a memory-bound matrix-vector multiply (GEMV) with an intensity of barely $1\text{ FLOP/Byte}$, utilizing less than $1\%$ of tensor core ALU capacity. Speculative verification ($K=4$) amortizes weight fetches across multiple draft tokens, shifting execution along the memory-bandwidth slope toward the compute ceiling and accelerating latency without degrading the target model distribution.

::: {#fig-vol3-tokenomics-roofline fig-env="figure" fig-pos="htb" fig-cap="**Operational Roofline and Execution Regime Accounting on H100 SXM5**: (a) Roofline analysis contrasting single-stream autoregressive decode ($B=1, \\text{GEMV}$), speculative decoding ($K=4$), and batched decoding against parallel prefill ($B \\ge 64, \\text{GEMM}$). The inflection ridge occurs at $I^* = 295.2\\text{ FLOP/Byte}$. (b) Hardware dataflow and arithmetic intensity accounting demonstrating why single-stream generation idles GPU tensor cores and inflates fleet-wide cost per token." fig-alt="Operational roofline model and dataflow accounting table illustrating arithmetic intensity differences between memory-bound decode GEMV and compute-bound prefill GEMM."}
...
2. **Parallel Verification Phase:** The large target model evaluates all $\gamma$ candidate tokens simultaneously in a single forward pass. Because the tokens are already known, the target model processes them as a parallel general matrix multiply (GEMM), running compute-bound at high arithmetic intensity.
...
![](images/svg/tokenomics_memory_hierarchy_caching_v2.svg){width="100%"}
:::
...
- **Priority Queue 0 (Interactive Service Level Agreement (SLA)):** Dedicated GPU capacity reserved strictly for single-turn interactive queries and user-facing chat ($< 5\text{ s}$). Preemption-safe; zero long-running tasks permitted.
