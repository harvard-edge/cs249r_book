...
::: {#fig-vol3-tokenomics-memory-hierarchy fig-env="figure" fig-pos="htb" fig-cap="**The Tokenomics Memory Hierarchy and Queueing Headroom Reserve**: Left: Tiered storage economics across accelerator HBM3, host system DRAM, and persistent local NVMe storage. Because HBM holding rent is orders of magnitude more expensive than host memory ($T_{\\text{break-even}} \\approx 0.07\\text{ s}$), serving runtimes asynchronously page idle KV-cache blocks across PCIe Gen5 ($128\\text{ GB/s}$) to preserve precious GPU memory for active decoders. Right: Kingman's multiserver queueing relation demonstrates the asymptotic delay cliff as GPU utilization approaches saturation; maintaining a mandatory 30 to 35 percent headroom reserve under steady state is required to absorb heavy-tailed agent trajectory bursts ($C_s \\ge 3.0$)." fig-alt="Two-panel architectural and queueing diagram showing three-tier memory hierarchy with HBM holding rent on the left and Kingman queueing cliff with 30 to 35 percent headroom reserve on the right."}
...
- [ ] **Tiered KV Swapping**: How does paging idle KV cache blocks over PCIe Gen5 ($128\text{ GB/s}$) to host system DRAM prevent both memory holding penalties and redundant prefill compute?
