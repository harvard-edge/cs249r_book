...
Notice the non-linear relationship revealed by @tbl-vol3-cpvg-comparison: While the 8B model is $62.5\times$ cheaper per trajectory than the Frontier model ($\$0.12$ vs. $\$7.50$), its CPVG ($\$1.54$) is virtually identical to the 8x7B mixture-of-experts (MoE) model ($\$1.55$), because its low per-step reliability requires nearly 13 repeated attempts to deliver a single verified solution. Sizing fleets based solely on nominal token pricing is an economic trap; operational economics are dictated by the product of token cost and trajectory reliability.
