...
For the first decade of modern deep learning, machine learning evaluation was dominated by static, feedforward datasets. Benchmarks such as ImageNet, GLUE, SQuAD, MMLU, and GSM8K evaluated models on isolated input-output pairs: a fixed context $x$ was fed into model $f_\theta(x)$, and the resulting prediction $\hat{y}$ was scored against a static ground-truth label $y$. In an agentic systems regime, this static paradigm collapses entirely. Two measures of the same models disagree about whether progress is happening, and @fig-vol3-static-vs-agentic shows why: the static benchmark flattened near its ceiling while the execution benchmark was still in its steep region, so a saturated score reports the limits of the instrument rather than the limits of the model.

::: {#fig-vol3-static-vs-agentic fig-env="figure" fig-pos="htb" fig-cap="**The Divergence of Static and Agentic Benchmarks (2023--2024)**: MMLU success rate against SWE-bench Lite over the same period. MMLU sits flat near 90 percent while the agentic benchmark climbs from near zero to about 50 percent, which is the signature of a saturated measure sitting beside an unsaturated one." fig-alt="Two-line chart from 2023 to late 2024 comparing static MMLU success rate, which stays flat near 90 percent, against agentic SWE-bench Lite success rate, which climbs from near zero to about 50 percent."}
...
data = pd.read_csv("vol3/telemetry_evaluation/data/static_vs_agentic.csv")
data['Date'] = pd.to_datetime(data['Date'])

fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 5))

# Plot lines
ax.plot(data['Date'], data['MMLU'], marker='o', color='#2c3e50', linewidth=2, label='MMLU (Static QA)')
ax.plot(data['Date'], data['SWE_bench_Lite'], marker='s', color='#e74c3c', linewidth=2, label='SWE-bench Lite (Agentic Gym)')

# Annotate points
for idx, row in data.iterrows():
    ax.annotate(row['Model'], (row['Date'], row['SWE_bench_Lite']), textcoords="offset points", xytext=(0, 10), ha='center', fontsize=9)

ax.set_ylim(0, 100)
ax.set_ylabel('Success Rate (%)')
ax.set_title('Evaluation saturation: Static vs Agentic capabilities', pad=15)
...
::: {#fig-vol3-goodput-pareto fig-env="figure" fig-pos="htb" fig-cap="**Trajectory Goodput Pareto Frontier**: Accuracy vs. inference compute economics. The chart plots task success rate (Pass@$k$) against Cost-per-Goal across dense baselines, sparse MoE cascades, and test-time search." fig-alt="Plot showing the Trajectory Goodput Pareto Frontier. Success rate rises as Cost-per-Goal increases. Sparse MoE cascade achieves maximum Goodput at Point B, while unconstrained swarms enter the diminishing returns zone at Point F."}
![](images/svg/trajectory_goodput_pareto.svg)
:::

@Fig-vol3-goodput-pareto places peak Trajectory Goodput at Point B rather than at maximum raw accuracy (Point E or F), where a dynamic cascade routes simple steps to small sparse MoE models and reserves heavy frontier models for error-recovery branches. Pushing past Point E into unconstrained agentic swarms (Point F) incurs a catastrophic economic penalty, requiring $4\times$ more capital for a negligible $+2.8\%$ gain in resolved tasks.
...
# | How:     calc_two_proportion_sample_size in its pooled form is the source
# |          of truth, matching the equation displayed in
# |          @sec-vol3-appendix-math-bootstrap. The delta a fixed suite can
# |          resolve is recovered by bisecting that same function.
# +-----------------------------------------------------------------------------
import math

from mlsysim import *

def _detectable(baseline, suite):
    """Delta whose pooled sample size is exactly `suite`.

    Bisects the registry formula rather than restating its constants, so the
    inverse cannot drift from the forward direction.
    """
    low, high = 1e-4, 1.0 - baseline - 1e-6
    for _ in range(200):
        mid = (low + high) / 2
        if calc_two_proportion_sample_size(baseline, mid, pooled=True) > suite:
            low = mid
        else:
            high = mid
    return (low + high) / 2
...
class EvaluationPower:
    ...
    required = math.ceil(calc_two_proportion_sample_size(baseline, delta, pooled=True))
    small_delta = _detectable(baseline, small_suite)
    typical_delta = _detectable(baseline, typical_suite)
    ...
    check(
        abs(calc_two_proportion_sample_size(baseline, typical_delta, pooled=True) - typical_suite) < 1e-3,
...
To capture agent execution semantics cleanly, tracing runtimes structure telemetry into the strict three-tier span hierarchy @fig-vol3-otel-trace lays out:
...
::: {#fig-vol3-otel-trace fig-env="figure" fig-pos="htb" fig-cap="**Hierarchical OpenTelemetry GenAI Distributed Trace Graph**: Structured span hierarchy attributing multi-model inference latency, token counts, tool I/O execution, and W3C context propagation. The nesting is what makes the trace diagnostic rather than descriptive. A flat log records that a trajectory was slow, while the hierarchy attributes the time to the step that spent it and to the model or tool underneath that step." fig-alt="Diagram of an OpenTelemetry GenAI trace hierarchy showing root span agent.trajectory and child step spans with nested gen_ai.inference and tool.execution subspans."}
...
Observability transforms an agent from an uninspectable black box into an accountable production service. When evaluation environments isolate state and distributed tracing captures causal lineage across multi-step execution, engineering teams can measure real trajectory progress, identify silent regressions, and optimize serving economics under strict operational bounds.

::: {.callout-chapter-connection title="From telemetry benchmarks to the agentic synthesis"}
Rigorous telemetry and interactive benchmarks complete the operational discipline required to run autonomous systems in production. With all five architectural parts established, the curriculum turns to the book's grand synthesis: @sec-vol3-conclusion unifies the foundational laws governing autonomous agency—from the inference-time bitter lesson to the reversibility boundary—and examines the frontier where software trajectories meet embodied physical intelligence.
:::
