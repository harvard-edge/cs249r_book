...
::: {#dfn-instruction-data-cohabitation .callout-definition title="The instruction-data co-inhabitation dilemma"}
The **Instruction-Data Co-Inhabitation Dilemma** is the architectural property of transformer-based inference where privileged supervisory instructions, system directives, and untrusted external environment observations occupy an undifferentiated sequence within the Key-Value cache:
$$\mathbf{X} = [\mathbf{x}_{\text{system}} \,\|\, \mathbf{x}_{\text{task}} \,\|\, \mathbf{x}_{\text{observation}}]$$
Because self-attention projects all tokens into shared Key-Value manifolds without hardware-enforced privilege boundaries ($W \oplus X$), untrusted observation tokens exert identical mathematical leverage over next-token generation as trusted supervisor directives. Consequently, prompt-level guardrails cannot mathematically guarantee safety, mandating external operating-system virtualization and complete mediation.
:::
