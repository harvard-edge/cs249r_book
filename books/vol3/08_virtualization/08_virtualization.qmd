...
The physical root of this vulnerability lies in the *instruction-data co-inhabitation dilemma* of the transformer architecture. As established in @sec-vol3-processor, multi-head attention operates over an undifferentiated token sequence: system directives, tool schemas, and external observations share identical Key and Value projection weights within the residual stream.[^fn-instruction-data-cohabitation] There is no physical capability bit, MMU permission flag, or hardware register distinguishing a trusted system instruction from an untrusted web payload. Once untrusted bytes enter the Key-Value cache, they exercise identical mathematical leverage over next-token probabilities.
...
::: {#dfn-copy-up-resolution .callout-definition title="OverlayFS VFS inode lookup and copy-up resolution protocol"}
