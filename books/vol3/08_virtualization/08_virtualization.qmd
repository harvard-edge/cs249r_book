...
An agent runtime has no such bit to check. The model reads system directives, tool schemas, and a web page it fetched a moment ago as one sequence of tokens, and nothing in attention marks which of those tokens may issue commands. Whatever an attacker wrote into that page, or whatever the model misread, reaches the host as a well-formed tool call. @Sec-vol3-actuation made that call typed, authorized, and settled. It did not limit what the code behind the call can touch once it runs. This chapter builds that limit beneath the model, from the threat it must stop to the start-up latency and memory it costs.
