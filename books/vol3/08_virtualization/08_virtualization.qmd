...
The physical root of this vulnerability lies in the *instruction-data co-inhabitation dilemma* of the transformer architecture. As established in @sec-vol3-processor, multi-head attention operates over an undifferentiated token sequence: system directives, tool schemas, and external observations share identical Key and Value projection weights within the residual stream.[^fn-instruction-data-cohabitation]
...
Operating under Zero Ambient Authority ($A = 0$), candidate tool commands emitted by the model remain in host memory escrow. The vulnerability arises the moment the host runtime decodes those candidate tokens into operating system commands. If the runtime attempts to execute commands directly on the host, a hijacked trajectory can compromise host file trees, exfiltrate environment secrets, or spawn rogue daemons. Because software alignment and prompt guardrails cannot guarantee behavioral invariants, isolation must be enforced by the operating system substrate beneath the model.

[^fn-instruction-data-cohabitation]: **Instruction-Data Co-Inhabitation**: See @sec-vol3-processor for the architectural analysis of attention projections over flat context buffers. This chapter develops the OS kernel containment mechanisms (cgroups, seccomp, microVMs) required to safely execute these untrusted proposals.
...
Confusing language-level variable scoping with hardware-enforced protection domains violates the principle of *Complete Mediation* [@saltzer1975]. An in-process barrier can never mediate access to the memory space it inhabits.
:::
...
True zero ambient authority requires terminating the untrusted execution boundary in silicon. Hardware-assisted virtualization (MicroVMs) ensures that guest instructions and guest kernel memory faults are trapped by CPU virtualization extensions (Intel VT-x, AMD-V) and mediated through Second Level Address Translation (SLAT/EPT). Even if an agent executing inside a MicroVM achieves root privilege over the guest kernel, it remains trapped in hardware register state—unable to mutate host memory or inspect sibling workloads.

**Systems insight**: Namespaces isolate cooperative processes from accidental interference; only hardware virtualization isolates unprivileged agents from deliberate or accidental host compromise. Systems runtimes that execute arbitrary agent-generated code must never share the host kernel.
:::
