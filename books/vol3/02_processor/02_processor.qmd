...
Chapter 1 established that an autonomous agent operates as an extended closed-loop machine, coordinating memory, tools, and execution runtimes to carry an operational task from intent to verifiable completion. Yet beneath all multi-turn planning, memory paging, and peripheral execution lies a single atomic event: **one invocation of a foundation model**.

Whenever an agent takes a step—whether diagnosing an anomaly, selecting a debugging tool, interpreting a compiler trace, or proposing a code modification—the host runtime freezes a slice of active state and dispatches it to the model core. Understanding the mechanics, guarantees, and physical costs of this single invocation is the foundation of agent systems engineering. Part I of this book focuses on this learned engine: the **Stochastic Processor**.

### The Anatomy of an Invocation

In classical computer systems, an instruction decoded by a microprocessor is an instruction executed. The hardware guarantees cycle-accurate state transitions, isolates address spaces through page tables, and traps illegal memory accesses directly at the silicon boundary.

A foundation model invocation possesses none of these properties. The model core is not an autonomous central processing unit with ambient system authority; it is a statistical sequence-continuation engine. It has no physical clock, holds no open operating system file descriptors, and produces no direct side effects in the host environment. Its sole operation is to evaluate a conditioned probability distribution over a discrete vocabulary of tokens and emit a candidate sequence.

Every model invocation follows a four-stage lifecycle across the boundary separating the host runtime from the model serving core:

1. **Context Staging:** The host runtime assembles task instructions, relevant environment observations, tool schemas, and recent conversation history into a formatted prompt buffer. The caller defines explicit operational constraints: model snapshot version, decoding temperature, maximum output tokens ($K_{\max}$), stop delimiters, and socket deadlines ($T_{\max}$).
2. **Epistemic Evaluation:** The serving core ingests the input tokens, evaluates causal self-attention across the sequence, and autoregressively samples continuation tokens in complete isolation. The core has no ambient knowledge of external state, real-world time, or concurrent filesystem mutations outside the staged buffer.
3. **Envelope Packaging:** When generation terminates—via an end-of-sequence token, an explicit stop delimiter, or token exhaustion—the serving infrastructure packages the output into an invocation response envelope. This envelope returns the emitted candidate tokens alongside caller-visible status flags (`Completed`, `Incomplete`, `Refusal`, `Transport Failure`) and resource telemetry (prompt tokens, decode tokens, and latency breakdowns).
4. **Runtime Mediation:** The host runtime unmarshals the response envelope, parses its structure against expected schemas, validates access permissions, and decides whether the candidate proposal should be permitted to execute against sandboxed tools.

### The Unprivileged Proposal: A Concrete Systems Hook

To see why this lifecycle boundary is mandatory, consider the distributed cache debugging scenario introduced in Chapter 1. Under heavy concurrency, read operations intermittently return stale data following network partition events. An autonomous repair agent is dispatched to inspect the service repository and resolve the failing regression test.

The host runtime stages the repository context—the failing test trace, the cache lease manager, and the configuration parser—along with a schema for file editing tools. The model's forward pass evaluates the context and samples the following candidate continuation:

```python
edit_file(path="parser.py", search="int(seconds) * 1000", replace="int(seconds * 1000)")
```

The stochastic processor core did not edit `parser.py`. It did not allocate a file descriptor, acquire an operating system lock, or modify a single byte on disk. It emitted an unprivileged string of token identifiers into a network socket buffer. 

The emitted payload is an algorithmic hypothesis. Whether `parser.py` exists, whether the agent possesses write authorization within the container, whether the proposed replacement introduces a syntax error, and whether this edit resolves the regression are questions the processor core cannot answer. The model proposes; the surrounding runtime harness interprets, authorizes, and mediates.

### Complete Mediation and Tripartite Execution {#sec-tripartite-separation}

Because candidate token sequences are sampled probabilistically from open distributions, unconstrained outputs frequently contain hallucinated parameters, syntactically malformed commands, or destructive side effects (such as proposing `rm -rf /` under high sampling entropy). Treating a model proposal as an executed instruction is the fundamental architectural flaw of naive agent designs.

To ensure system reliability, the Stochastic Computer operationalizes the classical security principle of **complete mediation**, formulated by Saltzer and Schroeder [-@saltzer1975]: *every access to every resource must be validated against an independent authorization mechanism, and no unvalidated assertion from an untrusted principal may bypass inspection.* In our architectural model, the foundation model core is an untrusted principal.

The runtime formalizes complete mediation through a strict tripartite execution separation:
...
1. **Model Proposal ($a_{\text{prop}}$):** The raw, unverified token sequence emitted by the neural core during decode. Whether structured as natural language or JSON tool calls, $a_{\text{prop}}$ carries zero authority to mutate host state.
2. **Permitted Action ($a_{\text{perm}}$):** The validated, strongly typed execution payload produced after $a_{\text{prop}}$ passes through deterministic verification gates: schema conformance checks, capability validation, and security sandbox policies.
3. **Committed Effect ($e_{\text{commit}}$):** The observable state mutation realized in the external environment (such as a filesystem write or network packet dispatch) following sandboxed execution of $a_{\text{perm}}$.
...
Operating this execution boundary reliably requires opening the model call and examining its mechanics from the symbolic interface down to the physical silicon:

* **Section 2.2 (Tokens as the Interface):** We examine the discrete currency crossing the boundary—subword tokens—and analyze why character counts and AST syntax trees diverge from token economics.
...
* **Section 2.4 (Candidate Sequences vs. Valid Conclusions):** We confront the epistemic reality of sampling, proving why statistical likelihood does not equal truth and how causal attention induces context poisoning.
...
With this operational roadmap established, we begin at the physical boundary: the subword tokens through which the caller and the core communicate.
