...
To a systems architect, this atomic step represents a direct neural analog of the classic CPU **Instruction Fetch-Decode-Execute-Writeback cycle**, augmented with stochastic branching:

1. **Instruction Fetch (IF)**: The runtime retrieves the prompt history and fetches the previously emitted token $y_{t-1}$ from the active context token buffer.
2. **Neural Decode (ID)**: The activation vector propagates through the $L$-layer transformer stack. The attention and feed-forward sublayers decode the context history into a dense representation $\mathbf{h}_L$, projecting the unnormalized logit score vector $\mathbf{z}_t \in \mathbb{R}^{|\mathcal{V}|}$ over the discrete action space.
3. **Execute and Stochastic Branch (EX)**: Rather than following a deterministic opcode, the execution unit applies sampling operators (temperature scaling, top-$p$, or grammar-derived logit masks) to evaluate the probability simplex and stochastically select the next token $y_t$.
4. **Writeback and Commit (WB)**: The newly sampled token $y_t$ is appended to the context buffer, its key and value projections are committed to the key-value (KV) cache in high-bandwidth memory (HBM), and the program counter in the Agent Control Block advances ($t \leftarrow t + 1$).

Under the hood, this forward pass decomposes into three primary computational stages: token embedding and positional encoding at the input, $L$ repeating transformer layers (each executing multi-head attention and a position-wise feed-forward network), and the final vocabulary projection. Each stage exhibits a distinct arithmetic intensity and, consequently, a distinct hardware bottleneck.
...
The transition table $T$ has memory footprint $|Q_F| \times |\mathcal{T}| \times 4$ bytes. For a JSON schema with $|Q_F| = 512$ states and $|\mathcal{T}| = 128{,}000$ tokens, this amounts to $512 \times 128{,}000 \times 4 = 262$ MB—well within host DRAM but too large for GPU L2 cache. Implementations therefore maintain the table in pinned host memory and transfer only the active row per decoding step, keeping device memory uncontested for KV cache allocation. For a 70B-parameter model at BF16 precision across $N_{\text{layers}} = 80$ transformer layers with sequence length $S = 4{,}096$ and $n_{\text{kv}} = 8$ grouped-query key-value heads of dimension $d_{\text{head}} = 128$, the KV cache requires the allocation quantified in @eq-kv-cache:

$$
M_{\text{KV}} = 2 \cdot N_{\text{layers}} \cdot n_{\text{kv}} \cdot d_{\text{head}} \cdot S \cdot b_{\text{elem}} = 2 \times 80 \times 8 \times 128 \times 4096 \times 2 \approx 1.34 \; \text{GB}
$$ {#eq-kv-cache}

amounting to $16.8\text{ MB}$ per layer. The 16 KB mask transfer per step is five orders of magnitude smaller than the active KV cache footprint and does not compete for HBM bandwidth, which on the H100 peaks at 3.35 TB/s.
...
::: {#chk-vol3-02-speculative-decoding .callout-checkpoint title="Speculative decoding economics and capability attenuation"}

Before exploring operational fallacies and proceeding to test-time deliberation, verify your understanding of speculative decoding speedups and capability attenuation:
...
- [ ] **Monotonic attenuation**: explain why capability attenuation in multi-agent delegation must be mathematically monotonic ($\mathcal{C}_{\text{child}} \subset \mathcal{C}_{\text{parent}}$) rather than relying on prompt-based instructions to child agents.
...
Beyond memory capacity, the memory wall imposes an escalating throughput penalty as context lengthens. During single-request decode, the accelerator must stream both the static model weights and the active KV cache from HBM for every generated token, as quantified by @eq-fallacy-attn-bytes:

$$
B_{\text{total}} = 2 \cdot N_{\text{params}} + 4 \cdot L \cdot H \cdot d_h \cdot S \;\text{bytes}
$$ {#eq-fallacy-attn-bytes}

For our 70B model at $S = 128{,}000$ tokens, the system must stream $140\text{ GB}$ of weights plus $420\text{ GB}$ of KV cache—a total of $B_{\text{total}} \approx 560\text{ GB}$ of data transferred to compute a single token. Meanwhile, total arithmetic operations scale as $F_{\text{total}} = 2 N_{\text{params}} + 4 L H d_h S \approx 140 \times 10^9 + 335 \times 10^9 = 475\text{ GFLOPs}$. The resulting aggregate arithmetic intensity remains trapped below unit intensity, as formulated in @eq-fallacy-attn-intensity:

$$
\mathcal{I}_{\text{decode}} = \frac{F_{\text{total}}}{B_{\text{total}}} = \frac{475 \;\text{GFLOPs}}{560 \;\text{GB}} \approx 0.85 \;\text{FLOPs/byte}
$$ {#eq-fallacy-attn-intensity}

which is more than $150\times$ below the H100 tensor core saturation threshold ($\approx 150$–$200$ FLOPs/byte). At an HBM3 bandwidth of $3.35\text{ TB/s}$, streaming $560\text{ GB}$ imposes an irreducible per-token latency of $560 / 3350 \approx 167\text{ ms}$, capping single-stream decode throughput to fewer than 6 tokens per second. Even when partitioned across 8 GPUs via tensor parallelism (where each GPU streams $17.5\text{ GB}$ of weights and $52.5\text{ GB}$ of KV cache), per-token latency is bounded by $70 / 3350 \approx 20.9\text{ ms}$ ($\approx 48$ tokens/second per user)—and the $420\text{ GB}$ KV cache footprint completely exhausts node HBM, eliminating the memory headroom required for concurrent continuous batching.
