...
In @eq-qkv-projections, $\mathbf{W}_Q^{(\ell)} \in \mathbb{R}^{d_{\text{model}} \times d_{\text{head}} \cdot H_Q}$, while $\mathbf{W}_K^{(\ell)}, \mathbf{W}_V^{(\ell)} \in \mathbb{R}^{d_{\text{model}} \times d_{\text{head}} \cdot H_{\text{KV}}}$, where $H_Q$ denotes query attention heads and $H_{\text{KV}}$ denotes key-value attention heads ($H_{\text{KV}} = H_Q$ under legacy Multi-Head Attention; $H_{\text{KV}} \ll H_Q$ under Grouped-Query Attention). The attention output is computed against all prior keys and values, which are materialized as the KV cache $\mathcal{K}^{(\ell)} \in \mathbb{R}^{t \times 2 \times H_{\text{KV}} \times d_{\text{head}}}$ accumulated over context length $t$. The memory footprint of this cache across all $L$ layers is
...
The runtime execution context is not merely bookkeeping metadata; it is the live execution state of the system. Its memory footprint scales linearly with context length through the KV cache term, and its integrity is a precondition for correct resumption after preemption, migration, or partial generation restart. Systems that treat runtime context as an afterthought — serializing it lazily or evicting it under memory pressure without a coherent replacement policy — exhibit the class of collapse behaviors described in the opening trace of this section: generation proceeds, latency degrades silently, and no surface-level metric captures the root cause until throughput collapses entirely. The execution lifecycle of an agent context is governed by the deterministic state transitions diagrammed in @fig-vol3-processor-acb-fsm.
...
The KV cache memory footprint for a transformer with $L$ layers, $H_{\text{KV}}$ key-value attention heads, head dimension $d_h$, and context length $S$ tokens, stored in BF16 ($b_{\text{elem}} = 2$ bytes per element), is given by @eq-kvcache:

$$
M_{\text{KV}} = 2 \cdot L \cdot H_{\text{KV}} \cdot d_h \cdot S \cdot b_{\text{elem}}
$$ {#eq-kvcache}

where the leading factor of $2$ accounts for both key and value tensors. For a 70B-parameter model ($L = 80, d_h = 128$) at a long context of $S = 128{,}000$ tokens, the architectural difference between legacy Multi-Head Attention and modern Grouped-Query Attention is dramatic:

- **Legacy Multi-Head Attention ($H_{\text{KV}} = H_Q = 64$):**
  $$
  M_{\text{KV}}^{\text{MHA}} = 2 \times 80 \times 64 \times 128 \times 128{,}000 \times 2 \approx 335.5 \;\text{GB}
  $$
  This exhausts the combined 320 GB HBM3 capacity of four H100 SXM5 GPUs ($4 \times 80\;\text{GB}$) before loading a single byte of the 140 GB model weights. Serving an uncompressed 128K context under MHA required at minimum six H100s for KV cache alone.
- **Modern Grouped-Query Attention ($H_{\text{KV}} = 8$, as in Llama 3 70B):**
  $$
  M_{\text{KV}}^{\text{GQA}} = 2 \times 80 \times 8 \times 128 \times 128{,}000 \times 2 \approx 41.9 \;\text{GB}
  $$
  GQA reduces KV cache storage by an exact factor of $8\times$ (from $2.62\text{ MB/token}$ down to $328\text{ KB/token}$ across all layers), allowing the cache to fit within GPU node memory.
