...
@Fig-stochastic-processor-core formalizes this four-stage microarchitectural pipeline. Notice that while a classical CPU pipeline operates on bit-exact discrete registers, the neural processor couples a memory-bound linear algebra pipeline (IF and ID) to a probabilistic sampling and logit-masking engine (EX and WB).
...
While swapping dormant KV cache blocks out of HBM resolves the Tool-Wait Memory Tax, it introduces a physical latency trade-off governed by PCIe interconnect bandwidth. For a 70B model with GQA ($H_{\text{KV}} = 8$) and an active context of $32{,}768$ tokens, the KV cache occupies $10.74\text{ GB}$. Over a PCIe Gen5 $\times 16$ interconnect operating at $64\text{ GB/s}$ unidirectional bandwidth ($128\text{ GB/s}$ bidirectional aggregate), asynchronous DMA eviction requires $10.74\text{ GB} / 64\text{ GB/s} \approx 168\text{ ms}$. Restoring this context upon tool completion requires a symmetric $168\text{ ms}$ prefetch, establishing a minimum round-trip swapping penalty of:
...
To understand how grammar-constrained decoding operates concretely inside an inference engine, @lst-vol3-processor-grammar-masking implements a functional grammar decoder. The listing illustrates the two core systems components: a byte-level `TokenTrie` that resolves subword vocabulary boundaries to guarantee prefix closure, and a deterministic state transition machine that computes the boolean logit mask $\mathbf{m}_k$ and applies $-\infty$ masking prior to the softmax operator.

::: {#lst-vol3-processor-grammar-masking lst-cap="**Grammar-Constrained Logit Masking Engine**: Python implementation of a schema-driven FSM decoder featuring byte-level prefix-closure matching via a vocabulary trie and dynamic pre-softmax logit masking."}
```python
...
class TrieNode:
    """Trie node for token-to-byte prefix matching."""
    def __init__(self):
...
class TokenTrie:
    """Prefix trie mapping raw byte sequences to subword vocabulary token IDs."""
...
class GrammarConstrainedDecoder:
    """Compiles structured schemas into FSMs and applies dynamic logit masks."""

    def __init__(self, vocab: Dict[int, bytes]):
        self.vocab = vocab
        self.vocab_size = len(vocab)
        self.trie = TokenTrie(vocab)

    def transition(self, state: JsonState, char: str) -> JsonState:
        """Deterministic FSM state transitions for a minimal JSON schema."""
        if state == JsonState.START and char == "{":
            return JsonState.OBJECT_OPEN
        if state == JsonState.OBJECT_OPEN and char == '"':
            return JsonState.KEY_START
        if state == JsonState.KEY_START and char.isalpha():
            return JsonState.IN_KEY
        if state == JsonState.IN_KEY and char.isalpha():
            return JsonState.IN_KEY
        if state == JsonState.IN_KEY and char == '"':
            return JsonState.KEY_END
        if state == JsonState.KEY_END and char == ":":
            return JsonState.COLON
        if state == JsonState.COLON and char == '"':
            return JsonState.VALUE_STRING
        if state == JsonState.COLON and char.isdigit():
            return JsonState.VALUE_NUMBER
        if state == JsonState.VALUE_STRING and char == '"':
            return JsonState.COMMA
        if state == JsonState.VALUE_STRING:
            return JsonState.VALUE_STRING
        if state == JsonState.VALUE_NUMBER and char.isdigit():
            return JsonState.VALUE_NUMBER
        if state == JsonState.VALUE_NUMBER and char == ",":
            return JsonState.OBJECT_OPEN
        if state == JsonState.VALUE_NUMBER and char == "}":
            return JsonState.ACCEPT
        if state == JsonState.COMMA and char == ",":
            return JsonState.OBJECT_OPEN
        if state == JsonState.COMMA and char == "}":
            return JsonState.ACCEPT
        return JsonState.ERROR

    def compute_valid_continuations(self, current_state: JsonState) -> Set[int]:
        """Computes prefix-closed valid tokens via recursive TokenTrie traversal."""
        valid_tokens: Set[int] = set()

        def dfs(node: TrieNode, state: JsonState):
            # Tokens whose complete byte sequence terminates at a valid state
            if state != JsonState.ERROR:
                valid_tokens.update(node.token_ids)
            # Prune entire sub-trie if the next byte transition is inadmissible
            for byte_val, child in node.children.items():
                next_state = self.transition(state, chr(byte_val))
                if next_state != JsonState.ERROR:
                    dfs(child, next_state)

        dfs(self.trie.root, current_state)
        return valid_tokens

    def step(self, current_state: JsonState, token_id: int) -> JsonState:
        """Advances the FSM state by consuming the token's byte sequence."""
        state = current_state
        for b in self.vocab.get(token_id, b""):
            state = self.transition(state, chr(b))
            if state == JsonState.ERROR:
                break
        return state

    def apply_logit_mask(
        self, logits: np.ndarray, current_state: JsonState
    ) -> np.ndarray:
        """Transforms raw logits into a strictly grammar-compliant probability distribution."""
        valid_tokens = self.compute_valid_continuations(current_state)
        masked_logits = np.full_like(logits, fill_value=-np.inf)
        if valid_tokens:
            indices = list(valid_tokens)
            masked_logits[indices] = logits[indices]
        return masked_logits
```
:::

By compiling formal schemas into deterministic automata and applying pre-softmax bitmasks, the runtime achieves an absolute systems guarantee: every emitted token conforms to the target grammar by construction, eliminating deserialization exceptions and collapsing the error-retry tax to unity.
...
To demonstrate how the draft-then-verify protocol is implemented in production systems (such as vLLM's speculative decoding backend), @lst-vol3-processor-speculative-decoding implements the complete speculation loop with residual distribution sampling and physical KV-cache truncation.

::: {#lst-vol3-processor-speculative-decoding lst-cap="**Speculative Decoding with Rejection Sampling and KV-Cache Rollback**: Python implementation of the two-tier draft-then-verify execution harness, featuring per-token acceptance testing, residual correction sampling, and KV cache length rollback."}
```python
...
class SpeculativeRoundResult:
  ...
  residual_sampled: bool
  kv_cache_length: int


class SpeculativeDecoder:
  """Two-tier draft-then-verify speculative decoding engine with KV-cache rollback."""

  def __init__(
      self,
      draft_policy: Callable[[List[int]], np.ndarray],
      target_policy: Callable[[List[int], List[int]], np.ndarray],
      speculation_window: int = 4,
  ):
    self.draft_policy = draft_policy  # M_q: returns (vocab_size,)
    self.target_policy = target_policy  # M_p: takes (context, draft_tokens), returns (K+1, vocab_size)
    self.K = speculation_window

  def decode_round(
      self,
      context_tokens: List[int],
      kv_cache: Optional[np.ndarray] = None,
  ) -> Tuple[SpeculativeRoundResult, Optional[np.ndarray]]:
    """Executes a single draft-then-verify speculation round with physical KV cache rollback."""
    draft_tokens: List[int] = []
    draft_probs: List[np.ndarray] = []
    curr_context = list(context_tokens)

    # Phase 1: Draft generation - K sequential forward passes on small model M_q
    for _ in range(self.K):
      q_logits = self.draft_policy(curr_context)
      q_dist = self._softmax(q_logits)
      sampled_token = int(np.random.choice(len(q_dist), p=q_dist))
      draft_tokens.append(sampled_token)
      draft_probs.append(q_dist)
      curr_context.append(sampled_token)

    # Phase 2: Target verification - single parallel forward pass on M_p
    # Evaluates context + all K draft tokens simultaneously in batched GEMM
    target_logits_matrix = self.target_policy(context_tokens, draft_tokens)

    # Phase 3: Token-level rejection sampling loop
    accepted_tokens: List[int] = []
    residual_sampled = False

    for k in range(self.K):
      token_id = draft_tokens[k]
      p_dist = self._softmax(target_logits_matrix[k])
      q_dist = draft_probs[k]

      p_k = p_dist[token_id]
      q_k = q_dist[token_id]

      # Modified rejection sampling condition: alpha = min(1, p_k / q_k)
      alpha = min(1.0, p_k / max(q_k, 1e-12))
      u = np.random.uniform(0.0, 1.0)

      if u <= alpha:
        # Token accepted: commit token and advance
        accepted_tokens.append(token_id)
      else:
        # Token rejected: sample replacement from residual distribution
        # p_res(v) = norm(max(0, p(v) - q(v)))
        residual_dist = np.maximum(0.0, p_dist - q_dist)
        sum_res = np.sum(residual_dist)
        if sum_res > 1e-12:
          residual_dist /= sum_res
        else:
          residual_dist = p_dist
        correction_token = int(
            np.random.choice(len(residual_dist), p=residual_dist)
        )
        accepted_tokens.append(correction_token)
        residual_sampled = True
        break  # Halts speculation round on first rejection

    # If all K tokens accepted, sample the (K+1)-th bonus token from target model
    if len(accepted_tokens) == self.K and not residual_sampled:
      bonus_dist = self._softmax(target_logits_matrix[self.K])
      bonus_token = int(np.random.choice(len(bonus_dist), p=bonus_dist))
      accepted_tokens.append(bonus_token)

    # Phase 4: KV-Cache Truncation & Physical Block Table Rollback
    # In PagedAttention runtimes (e.g., vLLM BlockManager), speculative tail slots
    # allocated in physical block tables beyond accepted length are unmapped and returned to free pool.
    committed_length = len(context_tokens) + len(accepted_tokens)
    committed_kv_cache = None
    if kv_cache is not None:
      # Physically roll back cache tensor to truncate unverified speculative entries
      committed_kv_cache = kv_cache[:, :, :committed_length, :]

    result = SpeculativeRoundResult(
        accepted_tokens=accepted_tokens,
        acceptance_count=len(accepted_tokens),
        speculation_window=self.K,
        residual_sampled=residual_sampled,
        kv_cache_length=committed_length,
    )
    return result, committed_kv_cache
