...
***Zero ambient authority***\index{Zero ambient authority!definition}\index{Privilege separation!zero ambient authority definition} ($A = 0$) is the foundational systems security invariant wherein an unprivileged execution unit—specifically a foundation model core—possesses zero native rights to inspect, mutate, or allocate external system resources: $\text{Privileges}(\pi_\theta) = \emptyset$.
