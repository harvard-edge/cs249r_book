...
1. **Arithmetic Intensity of Generation vs Context Prefill:** Consider an LLM processor with 80 GB of HBM capacity operating at 2.0 TB/s bandwidth. The model has 70 billion parameters (FP16, 2 bytes/param). Calculate the arithmetic intensity (FLOPs/byte) required to saturate the memory bandwidth during the autoregressive decode phase versus the prefill phase (assuming a context length of $L=8192$ and $d_{\text{model}}=4096$). At what batch size does the decoding phase become compute-bound rather than memory-bound?
2. **Latency Under Stochastic Retries:** An autonomous coding agent executes a trajectory of 15 dependent steps. Each step has a baseline success probability of $p=0.85$. If a step fails, the agent Control Block detects the error and retries the step, but each retry takes 50 percent longer due to an accumulating context window. Calculate the expected end-to-end latency if the base step time is 10 seconds. How does the expected latency change if the runtime employs an optimistic rollback mechanism that limits retries to a maximum of 3 per step?
