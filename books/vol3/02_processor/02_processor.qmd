...
::: {#fig-stochastic-processor-core fig-env="figure" fig-pos="htb" fig-cap="**The Stochastic Processor: Four-Stage Invocation Pipeline and Caller Contract**: Microarchitectural decomposition of the autoregressive forward pass, advancing from Context Staging (Stage 1) through Neural Compute (Stage 2) and Constrained Sampling (Stage 3) to the Tripartite Caller Contract (Stage 4). The runtime strictly separates untrusted neural proposals from capability-gated committed effects, arbitrating execution status across COMPLETED, TRUNCATED, REFUSAL, and TRANSPORT_ERROR." fig-alt="Four-stage architectural pipeline diagram illustrating Context Staging, Neural Core forward pass, Constrained Sampling with grammar masking, and the Tripartite Caller Contract with capability isolation."}
