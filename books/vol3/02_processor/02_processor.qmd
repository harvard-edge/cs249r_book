...
::: {#chk-02-stochastic-processor-boundary .callout-checkpoint title="Evaluating stochastic processor boundaries"}
...
By contrast, a deterministic external validator (such as a compiler, linter, or formal schema parser) enforces soundness over checked invariants ($\alpha_{\text{ext}} = 0.00$), driving $P(\text{Defective} \mid \text{PASS})$ to strictly $0\%$.

**Systems insight**: Invariant closure is a property of the environment boundary, not the generation engine. Autoregressive self-review can refine surface natural language explanations, but only deterministic external arbiters possessing ground-truth execution authority can enforce correctness.
:::
