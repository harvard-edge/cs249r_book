...
\noindent
![](images/png/cover_processor_blueprint_labeled_print.png){fig-alt="Isometric blueprint showing the stochastic processor contract: draft model execution core, speculative candidate token bus, memory-bandwidth streaming, target model verification engine, speculative rollback diversion gate, and parallel acceptance retirement."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_processor_blueprint_labeled.webp){fig-alt="Isometric blueprint showing the stochastic processor contract: draft model execution core, speculative candidate token bus, memory-bandwidth streaming, target model verification engine, speculative rollback diversion gate, and parallel acceptance retirement."}
:::
...
_What does a foundation model call actually compute on accelerator silicon, and what contract must the host runtime enforce to govern its execution safely?_

A model call is the unit every later chapter spends, so its price and its interface come first. On accelerator silicon the call splits into two regimes. Prefill processes the whole prompt in parallel and keeps the matrix units busy. Decode emits one token per pass, and each pass streams the full weight tensor from high-bandwidth memory, so an output token costs far more than a prompt token. That price shapes the contract the runtime writes for every call. The runtime caps how many tokens the model may emit, streams them so it can cancel early, masks illegal tokens during decode, and prefers interfaces that keep outputs short. Because the model holds zero ambient authority, whatever the call returns is still only a proposal, and the contract must also say what the runtime does with an incomplete, refused, or malformed one. A single call adds none of the H·S·A exposures, which makes it the origin from which every later part measures them. Its physics and its contract are one design problem, and this chapter treats them together.
...
- Trace a model call across the host runtime, the serving daemon, and the accelerator, identifying which component owns each decision and where the call's output stays until the runtime admits it.
...
```{python}
#| label: lego-logit-interconnect-bottleneck
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ THE LOGIT INTERCONNECT BOTTLENECK (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-logit-interconnect-bottleneck
# │ Goal: Quantify continuous interconnect bandwidth for host-side vs accelerator sampling.
# │ Show: 64,000x bandwidth reduction when transmitting token IDs instead of dense logit vectors.
# │ How: Multiply batch concurrency by per-stream decode rate and payload bytes across PCIe.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_memory, fmt_bandwidth, fmt_multiple, fmt_token_rate

class LogitInterconnectBottleneck:
    """Bandwidth comparison between host-side logit transfer and on-device sampling."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    v_vocab = 128_000
    batch_size = 64
    r_decode = 40.0 / second
    bytes_per_logit = 2 * byte
    bytes_per_token_id = 4 * byte
    pcie_bw_peak = 31.5 * (GB / second)
    v_vocab_stress = 256_000
    batch_size_stress = 256

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    payload_logits = v_vocab * bytes_per_logit
    aggregate_token_rate = batch_size * r_decode
    bw_logits = (aggregate_token_rate * payload_logits).to(MB / second)
    bw_tokens = (aggregate_token_rate * bytes_per_token_id).to(KB / second)
    reduction_factor = (payload_logits / bytes_per_token_id).to_base_units().magnitude
    bw_stress = ((batch_size_stress * r_decode) * (v_vocab_stress * bytes_per_logit)).to(GB / second)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(payload_logits.to(KB).magnitude == 256.0, f"Unexpected payload: {payload_logits}")
    check(aggregate_token_rate.magnitude == 2560.0, f"Unexpected rate: {aggregate_token_rate}")
    check(abs(bw_logits.magnitude - 655.36) < 0.01, f"Unexpected bw_logits: {bw_logits}")
    check(abs(bw_tokens.magnitude - 10.24) < 0.01, f"Unexpected bw_tokens: {bw_tokens}")
    check(reduction_factor == 64000.0, f"Unexpected reduction: {reduction_factor}")
    check(abs(bw_stress.magnitude - 5.24288) < 0.01, f"Unexpected bw_stress: {bw_stress}")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    v_vocab_str = fmt_int(v_vocab)
    batch_size_str = fmt_int(batch_size)
    r_decode_tokens_per_s_str = fmt_token_rate(r_decode.magnitude)
    bytes_per_token_id_str = fmt_int(bytes_per_token_id.to(byte).magnitude)
    payload_kb_str = fmt_memory(payload_logits, unit=KB, precision=0)
    aggregate_rate_count_str = fmt_int(aggregate_token_rate.magnitude)
    pcie_bw_peak_gb_per_s_str = fmt_bandwidth(pcie_bw_peak, unit=GB / second, precision=1)
    v_vocab_stress_str = fmt_int(v_vocab_stress)
    batch_size_stress_str = fmt_int(batch_size_stress)
    bw_stress_gb_per_s_str = fmt_bandwidth(bw_stress, unit=GB / second, precision=2)
    bw_tokens_kb_per_s_str = fmt_bandwidth(bw_tokens, unit=KB / second, precision=2)
    reduction_mult_str = fmt_multiple(reduction_factor, precision=0, commas=True)
```

::: {#nbk-logit-interconnect-bottleneck .callout-notebook title="The logit interconnect bottleneck"}
**Problem:** Consider an inference service serving an open-weight foundation model with a modern vocabulary size of $|V| =$ `{python} LogitInterconnectBottleneck.v_vocab_str` subword tokens. The inference service hosts an active concurrent batch of $B =$ `{python} LogitInterconnectBottleneck.batch_size_str` streams running on a single accelerator board connected via a standard PCIe 4.0 $\times 16$ interconnect. Each generation stream produces output at an average decode rate of $R =$ `{python} LogitInterconnectBottleneck.r_decode_tokens_per_s_str`.

Calculate the continuous interconnect bandwidth required if the accelerator transmits raw 16-bit floating-point logits (`fp16`, 2 bytes per logit) back to the host CPU for sampling, versus transmitting discrete 32-bit integer token identifiers (`uint32`, `{python} LogitInterconnectBottleneck.bytes_per_token_id_str` bytes per token). Evaluate the systems feasibility of host-side sampling under fine-grained PCIe transfer overheads.
...
For an aggregate batch generation throughput across all `{python} LogitInterconnectBottleneck.batch_size_str` streams:
...
While a peak theoretical PCIe 4.0 $\times 16$ bus provides approximately `{python} LogitInterconnectBottleneck.pcie_bw_peak_gb_per_s_str` of unidirectional bandwidth, this throughput assumes large, contiguous Direct Memory Access (DMA) transactions. Disagreeing with this assumption, an autoregressive serving loop dispatches these `{python} LogitInterconnectBottleneck.payload_kb_str` buffers in tiny, asynchronous bursts `{python} LogitInterconnectBottleneck.aggregate_rate_count_str` times every second. At this transfer granularity, PCIe transaction packetization overhead, host interrupt handling, DMA completion synchronization stalls, and host memory bus contention degrade performance, introducing significant jitter into the decode loop. If the vocabulary expands to $|V| =$ `{python} LogitInterconnectBottleneck.v_vocab_stress_str` or concurrency scales to $B =$ `{python} LogitInterconnectBottleneck.batch_size_stress_str`, bandwidth demand surges past `{python} LogitInterconnectBottleneck.bw_stress_gb_per_s_str`, severely saturating the system's interconnect queues.

*Case 2: Performing Logit Reduction on Accelerator Silicon.*
If top-$p$, top-$k$, temperature scaling, and multinomial categorical sampling execute directly within an accelerator kernel, the accelerator reduces the `{python} LogitInterconnectBottleneck.payload_kb_str` logit vector to a single scalar token ID:
...
**Conclusion:** Performing logit sampling on accelerator silicon reduces interconnect bandwidth pressure by `{python} LogitInterconnectBottleneck.reduction_mult_str`:
$$\frac{256{,}000\text{ bytes}}{4\text{ bytes}} = 64{,}000\times$$
This reduction transforms an interconnect traffic jam into a negligible stream of `{python} LogitInterconnectBottleneck.bw_tokens_kb_per_s_str`, freeing the host-accelerator interconnect for large model weight updates and input context ingestion. The discrete token identifier is the mandatory, bandwidth-optimal interface currency of the model invocation boundary.
:::
...
```{python}
#| label: lego-kv-cache-memory-explosion
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ KV CACHE MEMORY EXPLOSION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-02-kv-cache-memory-explosion
# │ Goal: Quantify KV cache scaling and memory capacity saturation for long contexts.
# │ Show: 128k context demands 41.94 GB (FP16) or 20.97 GB (FP8), exceeding 80 GB HBM.
# │ How: 2 * layers * kv_heads * head_dim * bytes_per_elem * context_length + model_footprint.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
...
class KvCacheMemoryExplosion:
    """Memory scaling of the autoregressive key-value cache under long contexts."""
    ...
    bytes_per_elem_fp16 = 2 * byte
    bytes_per_elem_fp8 = 1 * byte
    s_context = 128_000
    weights_footprint = 70.0 * GB
    hbm_capacity = 80.0 * GB

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    mem_kv_token_fp16 = calc_kv_cache_bytes_per_token(n_layers=n_layers, n_kv_heads=n_kv_heads, head_dim=d_head, bytes_per_elem=bytes_per_elem_fp16)
    mem_kv_token_fp8 = calc_kv_cache_bytes_per_token(n_layers=n_layers, n_kv_heads=n_kv_heads, head_dim=d_head, bytes_per_elem=bytes_per_elem_fp8)
    mem_kv_total_fp16 = s_context * mem_kv_token_fp16
    mem_kv_total_fp8 = s_context * mem_kv_token_fp8
    mem_total_fp16 = weights_footprint + mem_kv_total_fp16
    mem_total_fp8 = weights_footprint + mem_kv_total_fp8
    tp_kv_heads = n_kv_heads // 2
    tp_mem_kv_fp8 = mem_kv_total_fp8 / 2
    tp_weights = weights_footprint / 2

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(mem_kv_token_fp16.to(byte).magnitude == 327680.0, "Unexpected per-token FP16 bytes")
    check(mem_kv_token_fp16.to(KiB).magnitude == 320.0, "Unexpected per-token FP16 KiB")
    check(abs(mem_kv_total_fp16.to(GB).magnitude - 41.94) < 0.01, "Unexpected 128k FP16 KV size")
    check(abs(mem_total_fp16.to(GB).magnitude - 111.94) < 0.01, "Unexpected total FP16 footprint")
    check(mem_kv_token_fp8.to(byte).magnitude == 163840.0, "Unexpected per-token FP8 bytes")
    check(abs(mem_kv_total_fp8.to(GB).magnitude - 20.97) < 0.01, "Unexpected 128k FP8 KV size")
    check(abs(mem_total_fp8.to(GB).magnitude - 90.97) < 0.01, "Unexpected total FP8 footprint")
    check(tp_kv_heads == 4, "Unexpected TP KV heads")
    check(abs(tp_mem_kv_fp8.to(GB).magnitude - 10.49) < 0.01, "Unexpected TP FP8 KV size")
    check(tp_weights.to(GB).magnitude == 35.0, "Unexpected TP weights")
    ...
    s_context_str = fmt_int(s_context)
    hbm_capacity_gb_str = fmt_memory(hbm_capacity, unit=GB, precision=0)
    weights_footprint_gb_str = fmt_memory(weights_footprint, unit=GB, precision=0)
    mem_total_fp16_gb_str = fmt_memory(mem_total_fp16, unit=GB, precision=2)
    mem_kv_token_fp8_bytes_str = fmt_int(mem_kv_token_fp8.to(byte).magnitude)
    mem_kv_total_fp8_gb_str = fmt_memory(mem_kv_total_fp8, unit=GB, precision=2)
    mem_total_fp8_gb_str = fmt_memory(mem_total_fp8, unit=GB, precision=2)
    tp_kv_heads_str = fmt_int(tp_kv_heads)
    tp_mem_kv_fp8_gb_str = fmt_memory(tp_mem_kv_fp8, unit=GB, precision=2)
    tp_weights_gb_str = fmt_memory(tp_weights, unit=GB, precision=0)
```
...
- Layer depth: $L =$ `{python} KvCacheMemoryExplosion.n_layers_str`
- Query heads: $n_{\text{q}} =$ `{python} KvCacheMemoryExplosion.n_q_heads_str`
- Key-Value heads (GQA): $n_{\text{kv}} =$ `{python} KvCacheMemoryExplosion.n_kv_heads_str`
- Head dimension: $d_{\text{head}} =$ `{python} KvCacheMemoryExplosion.d_head_str` (hidden dimension $d_{\text{model}} = 64 \times 128 =$ `{python} KvCacheMemoryExplosion.d_model_str`)
...
Now, consider an agent performing complex codebase analysis across a large repository context, staging an active context window of $S =$ `{python} KvCacheMemoryExplosion.s_context_str` tokens. The memory demanded exclusively by the KV cache is:

$$\text{Mem}_{\text{KV}}(128\text{k}) = 128{,}000 \times 327{,}680\text{ bytes} = 41{,}943{,}040{,}000\text{ bytes} \approx 41.94\text{ GB}$$

Suppose the serving node is provisioned with an industry-standard NVIDIA A100-SXM4-80GB or H100-SXM5-80GB accelerator containing `{python} KvCacheMemoryExplosion.hbm_capacity_gb_str` of High-Bandwidth Memory (HBM). To run this 70B parameter model, the runtime uses 8-bit quantized weights and allocates static CUDA workspaces, communication scratchpads, and execution buffers, consuming a baseline resident footprint of `{python} KvCacheMemoryExplosion.weights_footprint_gb_str`:
...
Because `{python} KvCacheMemoryExplosion.mem_total_fp16_gb_str` $>$ `{python} KvCacheMemoryExplosion.hbm_capacity_gb_str`, the allocation breaches physical capacity, crashing the serving runtime with a CUDA Out-of-Memory (OOM) fault.

To mitigate this bottleneck, the systems engineer might attempt to quantize the KV cache to 8-bit precision (FP8, where $b = 1\text{ byte}$). Halving the storage precision cuts the per-token memory footprint to `{python} KvCacheMemoryExplosion.mem_kv_token_fp8_bytes_str` bytes/token, yielding:
...
Even with an 8-bit quantized KV cache, `{python} KvCacheMemoryExplosion.mem_total_fp8_gb_str` $>$ `{python} KvCacheMemoryExplosion.hbm_capacity_gb_str`. The serving engine still crashes the `{python} KvCacheMemoryExplosion.hbm_capacity_gb_str` device.

In production environments, resolving this failure mode requires one of two structural systems interventions:

1. **Tensor Parallelism ($TP \ge 2$):** The system shards the model weights and the `{python} KvCacheMemoryExplosion.n_kv_heads_str` KV heads across two or more interconnected GPUs. With $TP = 2$, each GPU holds only `{python} KvCacheMemoryExplosion.tp_kv_heads_str` KV heads (`{python} KvCacheMemoryExplosion.tp_mem_kv_fp8_gb_str` in FP8 or `{python} KvCacheMemoryExplosion.mem_kv_total_fp8_gb_str` in FP16) and half the model weights (`{python} KvCacheMemoryExplosion.tp_weights_gb_str`), bringing per-GPU memory well within the `{python} KvCacheMemoryExplosion.hbm_capacity_gb_str` threshold.
...
```{python}
#| label: lego-logit-scaling-truncation
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ LOGIT SCALING AND TRUNCATION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-02-logit-scaling-and-truncation-on
# │ Goal: Demonstrate numerical dynamics of softmax temperature scaling and nucleus truncation.
# │ Show: Low-temperature sharpening concentrates 95% mass; nucleus cuts low-rank tail risk.
# │ How: Apply z / tau, normalize exponentials, cumsum sorted probs, and renormalize subset.
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim import *
from mlsysim.fmt import check, fmt, fmt_percent

class LogitScalingTruncation:
    """Logit transformation and truncation dynamics on the vocabulary simplex."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    logits = [12.0, 10.5, 9.0, 6.0, 3.0]
    tau_base = 1.0
    tau_sharp = 0.5
    p_nucleus = 0.90
    ...
    exp_tau1 = []
    for z in logits:
        exp_tau1.append(math.exp(z / tau_base))
    sum_exp_tau1 = sum(exp_tau1)
    probs_tau1 = []
    for e in exp_tau1:
        probs_tau1.append(e / sum_exp_tau1)

    exp_tau05 = []
    for z in logits:
        exp_tau05.append(math.exp(z / tau_sharp))
    sum_exp_tau05 = sum(exp_tau05)
    probs_tau05 = []
    for e in exp_tau05:
        probs_tau05.append(e / sum_exp_tau05)

    sum_nuc = probs_tau1[0] + probs_tau1[1]
    p_nuc_v1 = probs_tau1[0] / sum_nuc
    p_nuc_v2 = probs_tau1[1] / sum_nuc

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(probs_tau1[0] - 0.7840) < 0.001, "Unexpected tau=1.0 top prob")
    check(abs(probs_tau05[0] - 0.9502) < 0.001, "Unexpected tau=0.5 top prob")
    check(abs(sum_nuc - 0.9589) < 0.001, "Unexpected nucleus denominator")
    check(abs(p_nuc_v1 - 0.8176) < 0.001, "Unexpected nucleus v1 prob")
    check(abs(p_nuc_v2 - 0.1824) < 0.001, "Unexpected nucleus v2 prob")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    sum_exp_tau1_str = fmt(sum_exp_tau1, precision=1, commas=False)
    top_mass_pct_str = fmt_percent(0.95, precision=0, style="prose")
    p_nucleus_str = fmt(p_nucleus, precision=2, commas=False)
```
...
Summing the terms yields $\sum_i \exp(z_i) =$ `{python} LogitScalingTruncation.sum_exp_tau1_str`. Dividing each term by the denominator gives:
...
Lowering the temperature concentrates `{python} LogitScalingTruncation.top_mass_pct_str` of the total probability mass onto the top token $v_1$, drastically reducing the entropy of the generation step.

**Step 3: Nucleus Truncation ($p =$ `{python} LogitScalingTruncation.p_nucleus_str`, $\tau = 1.0$)**
...
The cumulative threshold `{python} LogitScalingTruncation.p_nucleus_str` is exceeded by including $v_2$. Therefore, the nucleus set is $\mathcal{V}_p = \{v_1, v_2\}$.
...
**Sequence likelihood measures statistical typicality within a training corpus, not operational correctness, semantic truth, or system invariant preservation.** A foundation model produces hypotheses, not validated operational commands. Because the model holds zero ambient authority (@sec-vol3-processor-role), the command above could not run on its own, and whether it runs is the runtime's decision. The host agent runtime must stage every emitted candidate in memory escrow and subject it to a multi-tiered, deterministic verification perimeter before any state transition is authorized.
...
```{python}
#| label: lego-stream-cancellation-reclamation
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ STREAM CANCELLATION RECLAMATION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-02-memory-and-bandwidth-reclamation-via
# │ Goal: Calculate memory and bandwidth saved by early stream cancellation at step t=96.
# │ Show: Frees 1.22 GiB of KV cache and eliminates 560 TB of weight shuttles across bus.
# │ How: (k_max - t_cancel) * per_token_kv_bytes and (k_max - t_cancel) * weight_bytes.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_memory

class StreamCancellationReclamation:
    """Resource reclamation on accelerator silicon via reactive stream cancellation."""
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    mem_kv_token = calc_kv_cache_bytes_per_token(n_layers=n_layers, n_kv_heads=n_kv_heads, head_dim=d_head, bytes_per_elem=bytes_per_elem)
    steps_saved = k_max - t_cancel
    mem_saved = steps_saved * mem_kv_token
    weight_footprint = (p_params * bytes_per_elem).to(GB)
    bw_saved = (steps_saved * weight_footprint).to(TB)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(mem_kv_token.to(byte).magnitude == 327680.0, "Unexpected per-token bytes")
    check(mem_kv_token.to(KiB).magnitude == 320.0, "Unexpected per-token KiB")
    check(steps_saved == 4000, "Unexpected steps saved")
    check(abs(mem_saved.to(GiB).magnitude - 1.22) < 0.01, "Unexpected mem saved GiB")
    check(weight_footprint.to(GB).magnitude == 140.0, "Unexpected weight footprint")
    check(bw_saved.to(TB).magnitude == 560.0, "Unexpected bandwidth saved TB")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_layers_str = fmt_int(n_layers)
    d_head_str = fmt_int(d_head)
    n_kv_heads_str = fmt_int(n_kv_heads)
    k_max_str = fmt_int(k_max)
    t_cancel_str = fmt_int(t_cancel)
    steps_saved_str = fmt_int(steps_saved)
```
...
Consider a host agent supervising an invocation of a 70-billion parameter model utilizing Grouped-Query Attention (GQA). The model architecture features $L =$ `{python} StreamCancellationReclamation.n_layers_str` transformer layers, an internal head dimension of $d =$ `{python} StreamCancellationReclamation.d_head_str`, and $H_{KV} =$ `{python} StreamCancellationReclamation.n_kv_heads_str` key-value heads. Each weight and cache element is stored in 16-bit precision (2 bytes per value).
...
Assume the host specifies a generation budget of $K_{\max} =$ `{python} StreamCancellationReclamation.k_max_str` tokens for a complex code refactoring task. At decode token $t =$ `{python} StreamCancellationReclamation.t_cancel_str`, the model hallucinates an invalid command prefix that violates the agent runtime's tool schema.

If the host waits passively for completion, the inference engine executes the remaining $4{,}096 - 96 =$ `{python} StreamCancellationReclamation.steps_saved_str` decode steps. The physical KV cache memory locked by this single sequence over those steps is:
...
$$\text{Weight Footprint} = 70 \times 10^9 \times 2\text{ bytes} = 140\text{ Gigabytes}$$

By issuing an immediate `RST_STREAM` at token `{python} StreamCancellationReclamation.t_cancel_str`, the host supervisor prevents the accelerator from performing `{python} StreamCancellationReclamation.steps_saved_str` memory-bound decode iterations. This eliminates the need to stream:
...
```{python}
#| label: lego-grammar-bitmask-footprint
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ GRAMMAR BITMASK STORAGE AND DISPATCH (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-02-memory-footprint-and-transfer-latency
# │ Goal: Evaluate storage footprint and latency of on-chip vs host-mediated grammar masks.
# │ Show: 8 MiB table fits L2 cache (1.33 ns fetch); host round-trip adds 17.26 µs stall per token.
# │ How: q_states * (v_vocab / 8) bytes, L2 bandwidth fetch vs PCIe D2H + CPU + DMA + relaunch.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_memory_capacity, fmt_bandwidth, fmt_time

class GrammarBitmaskFootprint:
    """Memory footprint and dispatch latency of grammar bitmask decoding."""
    ...
    l2_cache_size = 50.0 * MiB
    bw_pcie = 64.0 * (GB / second)
    bw_l2 = 12.0 * (TB / second)
    t_d2h_token = 1.5 * microsecond
    t_host_cpu = 8.0 * microsecond
    t_packet = 2.5 * microsecond
    t_gpu_relaunch = 5.0 * microsecond
    seq_tokens = 500

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    bytes_per_state = (v_vocab // 8) * byte
    storage_table = q_states * bytes_per_state
    t_fetch_l2 = ((16 * KB) / bw_l2).to(nanosecond)
    t_dma_transfer = (bytes_per_state / bw_pcie).to(microsecond)
    t_dma = t_dma_transfer + t_packet
    t_host_overhead = t_d2h_token + t_host_cpu + t_dma + t_gpu_relaunch
    t_idle_total = (seq_tokens * t_host_overhead).to(millisecond)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(bytes_per_state.to(KiB).magnitude == 16.0, "Unexpected bytes per state KiB")
    check(storage_table.to(MiB).magnitude == 8.0, "Unexpected storage table MiB")
    check(abs(t_fetch_l2.to(nanosecond).magnitude - 1.33) < 0.05, "Unexpected L2 fetch ns")
    check(abs(t_dma.to(microsecond).magnitude - 2.76) < 0.05, "Unexpected DMA time us")
    check(abs(t_host_overhead.to(microsecond).magnitude - 17.26) < 0.05, "Unexpected host overhead us")
    check(abs(t_idle_total.to(millisecond).magnitude - 8.63) < 0.05, "Unexpected idle total ms")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    v_vocab_str = fmt_int(v_vocab)
    bytes_per_state_kb_str = fmt_memory_capacity(bytes_per_state, unit=KiB, precision=0)
    q_states_str = fmt_int(q_states)
    storage_table_mb_str = fmt_memory_capacity(storage_table, unit=MiB, precision=0)
    hbm_capacity_gb_str = fmt_memory_capacity(hbm_capacity, unit=GiB, precision=0)
    l2_cache_size_mb_str = fmt_memory_capacity(l2_cache_size, unit=MiB, precision=0)
    bw_l2_tb_per_s_str = fmt_bandwidth(bw_l2, unit=TB / second, precision=0)
    t_d2h_token_us_str = fmt_time(t_d2h_token, unit=microsecond, precision=1)
    t_host_cpu_us_str = fmt_time(t_host_cpu, unit=microsecond, precision=0)
    t_gpu_relaunch_us_str = fmt_time(t_gpu_relaunch, unit=microsecond, precision=0)
    seq_tokens_str = fmt_int(seq_tokens)
    t_idle_total_ms_str = fmt_time(t_idle_total, unit=millisecond, precision=2)
```
...
Consider serving a 70-billion parameter foundation model on an NVIDIA H100 SXM5 accelerator ($80\text{ GiB}$ HBM3 with $3.35\text{ TB/s}$ peak bandwidth, $50\text{ MiB}$ of on-chip L2 cache, and a PCIe Gen5 $\times 16$ interconnect providing $64\text{ GB/s}$ unidirectional bandwidth). The vocabulary contains $|\mathcal{V}| =$ `{python} GrammarBitmaskFootprint.v_vocab_str` tokens (`{python} GrammarBitmaskFootprint.bytes_per_state_kb_str` per state bitmask). An agent runtime enforces a structured JSON response schema that compiles into a DFA with $|Q| =$ `{python} GrammarBitmaskFootprint.q_states_str` states.
...
An `{python} GrammarBitmaskFootprint.storage_table_mb_str` footprint represents less than 0.01 percent of total device HBM (`{python} GrammarBitmaskFootprint.hbm_capacity_gb_str`) and fits entirely inside the accelerator's `{python} GrammarBitmaskFootprint.l2_cache_size_mb_str` on-chip L2 cache. Staging the table into L2 cache allows a custom GPU masking kernel to read the active `{python} GrammarBitmaskFootprint.bytes_per_state_kb_str` bitmask at L2 cache bandwidth ($\approx$ `{python} GrammarBitmaskFootprint.bw_l2_tb_per_s_str`), completing the memory fetch in:
...
- Device-to-host transfer of sampled token ID ($4\text{ bytes}$): $\approx$ `{python} GrammarBitmaskFootprint.t_d2h_token_us_str` (interconnect traversal and driver overhead).
- Host CPU context switch, thread scheduling, and DFA transition: $\approx$ `{python} GrammarBitmaskFootprint.t_host_cpu_us_str`.
- Host-to-device DMA transfer of the `{python} GrammarBitmaskFootprint.bytes_per_state_kb_str` bitmask:

$$t_{\text{DMA}} = \frac{16{,}384\text{ bytes}}{64\times 10^9\text{ bytes/s}} + t_{\text{packet}} \approx 0.26\ \mu\text{s} + 2.5\ \mu\text{s} = 2.76\ \mu\text{s}$$

- GPU kernel relaunch and stream synchronization: $\approx$ `{python} GrammarBitmaskFootprint.t_gpu_relaunch_us_str`.

Total host-synchronization latency per token:

$$t_{\text{host\_overhead}} \approx 1.5 + 8.0 + 2.76 + 5.0 = 17.26\ \mu\text{s}$$

Across an output sequence of $S =$ `{python} GrammarBitmaskFootprint.seq_tokens_str` tokens, host-driven masking introduces `{python} GrammarBitmaskFootprint.t_idle_total_ms_str` of pure idle stall time under ideal conditions. Under realistic host system load with multi-tenant scheduling contention, this penalty frequently expands to $50\text{--}150\ \mu\text{s}$ per token ($25\text{--}75\text{ ms}$ total), degrading token generation throughput by $15\%$ to $35\%$.
:::
...
```{python}
#| label: lego-roofline-single-token
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ ROOFLINE MODEL FOR SINGLE-TOKEN GENERATION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-02-roofline-bound
# │ Goal: Bound single-token decode latency and compute utilization at batch size B=1.
# │ Show: Memory bus limits decode to 20.90 ms (47.8 tokens/s); matrix units operate at 0.68% utilization.
# │ How: max(T_comp, T_mem) where T_mem = model_bytes / bw_mem and T_comp = 2*P / peak_compute.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt, fmt_time, fmt_percent, fmt_token_rate

class RooflineSingleToken:
    """Roofline execution bounds for unbatched autoregressive token generation."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    p_params = 70e9
    bytes_per_param = 1 * byte
    bw_mem = 3.35 * (TB / second)
    peak_compute = 989e12

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    model_bytes = p_params * bytes_per_param
    t_mem = (model_bytes / bw_mem).to(second)
    flops_step = 2 * p_params
    t_comp = ((flops_step / peak_compute) * second).to(second)
    t_step = max(t_mem, t_comp)
    utilization = (t_comp / t_mem).to_base_units().magnitude
    idle_pct = 1.0 - utilization
    throughput = 1.0 / 0.02090

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_mem.to(millisecond).magnitude - 20.90) < 0.05, "Unexpected t_mem ms")
    check(abs(t_comp.to(millisecond).magnitude - 0.14) < 0.02, "Unexpected t_comp ms")
    check(abs(t_step.to(millisecond).magnitude - 20.90) < 0.05, "Unexpected t_step ms")
    check(abs(utilization - 0.00678) < 0.0005, "Unexpected arithmetic utilization")
    check(abs(throughput - 47.8) < 0.2, "Unexpected decode throughput")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    idle_pct_str = fmt_percent(idle_pct, precision=1, style="prose")
```
...
More than `{python} RooflineSingleToken.idle_pct_str` of the accelerator's matrix math units sit completely idle during the step. The maximum attainable decoding speed for this trajectory is:
...
```{python}
#| label: lego-quantitative-interface-evaluation
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ QUANTITATIVE INTERFACE EVALUATION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-02-quantitative-interface-evaluation-under-an
# │ Goal: Compare total execution time and task completion under an invariant latency budget.
# │ Show: Interface 2 cuts decode tokens by 95.7%, yields 14x latency reduction, and closes SLA.
# │ How: Sum compute-bound prefill and bandwidth-bound decode; evaluate against T_max ceiling.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt, fmt_int, fmt_time, fmt_percent, fmt_multiple

class QuantitativeInterfaceEvaluation:
    """End-to-end latency and task completion comparison across invocation interfaces."""
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    m1 = m_base + delta_m1
    t_prefill1 = ((2 * p_params * m1 / peak_compute) * second).to(second)
    m2 = m_base + delta_m2
    t_prefill2 = ((2 * p_params * m2 / peak_compute) * second).to(second)
    t_token = round((p_params * (1 * byte) / bw_mem).to(second).magnitude, 4) * second
    t_decode1 = 38.67 * second
    t_total1 = t_prefill1 + t_decode1
    t_decode2 = 1.67 * second
    t_total2 = t_prefill2 + t_decode2
    overage1 = ((t_total1 - t_max) / t_max).to_base_units().magnitude
    pass2 = r_syntax2 * acc_semantic
    speedup = (t_total1 / t_total2).to_base_units().magnitude
    token_reduction = (k1 - k2) / k1

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(m1 == 8542, "Unexpected m1")
    check(abs(t_prefill1.magnitude - 1.21) < 0.02, "Unexpected t_prefill1")
    check(m2 == 8312, "Unexpected m2")
    check(abs(t_prefill2.magnitude - 1.18) < 0.02, "Unexpected t_prefill2")
    check(abs(t_token.to(millisecond).magnitude - 20.9) < 0.1, "Unexpected t_token ms")
    check(abs(t_decode1.magnitude - 38.67) < 0.01, "Unexpected t_decode1")
    check(abs(t_total1.magnitude - 39.88) < 0.01, "Unexpected t_total1")
    check(abs(t_decode2.magnitude - 1.67) < 0.01, "Unexpected t_decode2")
    check(abs(t_total2.magnitude - 2.85) < 0.01, "Unexpected t_total2")
    check(abs(pass2 - 0.57) < 0.01, "Unexpected pass2")
    check(abs(token_reduction - 0.9567) < 0.01, "Unexpected token reduction")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    m_base_str = fmt_int(m_base)
    t_max_s_str = fmt_time(t_max, unit=second, precision=0)
    delta_m1_str = fmt_int(delta_m1)
    k1_str = fmt_int(k1)
    r_syntax1_str = fmt(r_syntax1, precision=2, commas=False)
    delta_m2_str = fmt_int(delta_m2)
    k2_str = fmt_int(k2)
    r_syntax2_str = fmt(r_syntax2, precision=2, commas=False)
    acc_semantic_pct_str = fmt_percent(acc_semantic, precision=0, style="prose")
    overage1_pct_str = fmt_percent(round(overage1 * 100) / 100, precision=0, style="prose", max_ratio=3.0)
    speedup_mult_str = fmt_multiple(round(speedup), precision=0)
    token_reduction_pct_str = fmt_percent(token_reduction, precision=1, style="prose")
```
...
- Invariant Workload Context: Baseline prompt $M_{\text{base}} =$ `{python} QuantitativeInterfaceEvaluation.m_base_str` tokens.
- Invariant Latency Ceiling: $T_{\max} =$ `{python} QuantitativeInterfaceEvaluation.t_max_s_str`. Unbatched serving ($b=1$). Neglect $T_{\text{queue}}$ and $T_{\text{prep}}$.
...
- **Interface 1 (Monolithic JSON Rewrite):** The prompt requires the model to output the entire modified file inside a strict JSON envelope: `{"path": "...", "content": "..."}`.
  - Prompt Overhead: $\Delta M_1 =$ `{python} QuantitativeInterfaceEvaluation.delta_m1_str` tokens (JSON schema + few-shot instructions).
  - Generation Volume: $K_1 =$ `{python} QuantitativeInterfaceEvaluation.k1_str` tokens (JSON syntax, escaping, full file content).
  - Parsing Reliability: $R_{\text{syntax}} =$ `{python} QuantitativeInterfaceEvaluation.r_syntax1_str`.
- **Interface 2 (Anchored Search/Replace Diff):** The prompt requires the model to output localized diff markers: `<<<<<<< SEARCH ... ======= ... >>>>>>>`.
  - Prompt Overhead: $\Delta M_2 =$ `{python} QuantitativeInterfaceEvaluation.delta_m2_str` tokens (diff syntax definition).
  - Generation Volume: $K_2 =$ `{python} QuantitativeInterfaceEvaluation.k2_str` tokens (search context lines, replacement lines, delimiters).
  - Parsing Reliability: $R_{\text{syntax}} =$ `{python} QuantitativeInterfaceEvaluation.r_syntax2_str` (occasional anchor mismatch).
...
For Interface 1 ($K_1 =$ `{python} QuantitativeInterfaceEvaluation.k1_str` tokens):
...
For Interface 2 ($K_2 =$ `{python} QuantitativeInterfaceEvaluation.k2_str` tokens):
...
*Step 3: Systems Evaluation against $T_{\max} =$ `{python} QuantitativeInterfaceEvaluation.t_max_s_str`.*
Interface 1 yields $T_{\text{total}, 1} = 39.88 \text{ s}$, exceeding the latency ceiling by `{python} QuantitativeInterfaceEvaluation.overage1_pct_str`. The host runtime flags the invocation status envelope as `TRANSPORT_FAILURE` due to deadline expiry and aborts the RPC connection, resulting in an effective task completion rate of $\text{PASS}_1 = 0.0$ under the SLA, despite its high parsing reliability ($R_{\text{syntax}} =$ `{python} QuantitativeInterfaceEvaluation.r_syntax1_str`).

Interface 2 completes in $T_{\text{total}, 2} = 2.85 \text{ s}$, well beneath the `{python} QuantitativeInterfaceEvaluation.t_max_s_str` ceiling, returning a status envelope of `COMPLETED`. Even accounting for its $5\%$ anchor parsing failure rate ($R_{\text{syntax}} =$ `{python} QuantitativeInterfaceEvaluation.r_syntax2_str`), the remaining $95\%$ of candidates are delivered to the verification harness. If the model's semantic bug-fixing accuracy on valid diffs is `{python} QuantitativeInterfaceEvaluation.acc_semantic_pct_str`, Interface 2 achieves an end-to-end verified closure rate:
$$\text{PASS}_2 = 0.95 \times 0.60 = 0.57 \quad (57\%)$$

By reducing decode tokens by `{python} QuantitativeInterfaceEvaluation.token_reduction_pct_str`, Interface 2 eliminates memory bus saturation, achieves a `{python} QuantitativeInterfaceEvaluation.speedup_mult_str` latency reduction, and converts a non-viable operational interface into a high-performing systems contract.
:::
