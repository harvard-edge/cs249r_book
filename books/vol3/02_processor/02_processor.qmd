...
class GrammarConstrainedDecoder:
    """Pre-softmax logit masking engine enforcing prefix-closed grammar constraints."""

    def __init__(self, vocab: Dict[int, bytes], transition_fn, initial_state: int):
        self.vocab = vocab
        self.transition = transition_fn  # delta(state, byte) -> state or -1 (reject)
        self.state = initial_state

    def compute_valid_tokens(self, state: int) -> Set[int]:
        """Filters vocabulary tokens whose byte sequence preserves prefix closure."""
        valid = set()
        for token_id, byte_seq in self.vocab.items():
            curr = state
            for b in byte_seq:
                curr = self.transition(curr, b)
                if curr == -1:
                    break
            if curr != -1:
                valid.add(token_id)
    ...
    def mask_logits(self, logits: np.ndarray) -> np.ndarray:
        """Applies -inf logit mask to all illegal tokens prior to softmax kernel."""
        valid_tokens = self.compute_valid_tokens(self.state)
        mask = np.full_like(logits, fill_value=-np.inf)
        if valid_tokens:
            indices = list(valid_tokens)
            mask[indices] = logits[indices]
        return mask  # Transferred to GPU HBM prior to sampling kernel

    def step(self, sampled_token_id: int) -> None:
        """Consumes accepted token bytes to advance the internal FSM state."""
        for b in self.vocab[sampled_token_id]:
            self.state = self.transition(self.state, b)
```
:::
...
def speculative_decode_round(
    draft_fn, target_fn, context: List[int], K: int = 4
) -> Tuple[List[int], int]:
  """Executes draft-then-verify speculation with rejection sampling and KV rollback."""
  # 1. Draft phase: K sequential steps on small model M_q
  draft_tokens, q_probs, cur = [], [], list(context)
  for _ in range(K):
    q = draft_fn(cur)
    tok = int(np.random.choice(len(q), p=q))
    draft_tokens.append(tok)
    q_probs.append(q)
    cur.append(tok)

  # 2. Verify phase: parallel forward pass on target model M_p for all K candidates
  p_probs_matrix = target_fn(context, draft_tokens)  # Shape: (K + 1, vocab_size)

  # 3. Acceptance loop with residual sampling upon rejection
  accepted = []
  for k, (tok, q) in enumerate(zip(draft_tokens, q_probs)):
    p = p_probs_matrix[k]
    alpha = min(1.0, p[tok] / max(q[tok], 1e-12))
    if np.random.uniform(0, 1) <= alpha:
      accepted.append(tok)
    else:
      # Rejection: sample replacement from residual max(0, p - q) and stop
      res = np.maximum(0.0, p - q)
      res /= np.sum(res)
      accepted.append(int(np.random.choice(len(res), p=res)))
      break

  # 4. Bonus token if all K accepted; truncate speculative KV cache slots
  if len(accepted) == K:
    bonus = p_probs_matrix[K]
    accepted.append(int(np.random.choice(len(bonus), p=bonus)))

  kv_commit_len = len(context) + len(accepted)  # Unmapped speculative tail freed
  return accepted, kv_commit_len
```
:::
