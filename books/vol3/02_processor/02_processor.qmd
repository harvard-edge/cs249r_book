...
**Zero Ambient Authority** ($A = 0$) is the foundational systems security invariant wherein an unprivileged execution unit—specifically a foundation model core—possesses zero native rights to inspect, mutate, or allocate external system resources:

$$\text{Privileges}(\pi_\theta) = \emptyset$$

Candidate token emissions (such as shell commands or RPC invocations) reside strictly in host memory escrow. Every privileged operation requires explicit supervisor interception, capability verification, schema validation, and sandboxed dispatch before physical host state can be altered.
:::

The defining systems reality of foundation model execution is **Zero Ambient Authority** ($A = 0$). When a model autoregressively generates the text string `rm -rf /` or emits a structured JSON object requesting the deletion of a database table, exactly zero bytes on the host operating system are altered. The generated string is not an executing process; it is merely an array of integer token identifiers resident in high-bandwidth memory (HBM) on an accelerator board. The token array is transferred across an interconnect bus, decoded into characters by an inference daemon, and deposited into a user-space memory buffer managed by the host agent supervisor. The candidate output resides entirely in memory escrow. It has no ambient authority to modify host memory, invoke operating system primitives via `sys_call_table`, allocate network sockets, or manipulate peripheral devices.
...
**Problem:**
An agent runtime stages an emitted configuration payload $\mathbf{y}$ consisting of $K = 128$ tokens. Due to imperfect training distribution coverage, the model has an operational error rate of $\epsilon = 0.18$ (18 percent of generated payloads violate runtime invariants).
To catch errors before execution, an engineer considers two verification strategies:

1. *Autoregressive Self-Verification:* The runtime prompts the same model to review the payload. Because the evaluator shares parameter weights $\Theta$ with the generator, it exhibits correlated bias: while its true positive rate on valid payloads is $0.85$, its false positive rate on invalid payloads is $\alpha_{\text{model}} = 0.65$ (it mistakenly validates 65 percent of its own defective outputs).
...
**Grammar-Constrained Decoding** is a synchronous logit-filtering protocol wherein an unprivileged model's next-token distribution is mathematically restricted to a formal grammar $G$ via state-dependent logit masking:

$$M_i(q_t) = \begin{cases} 0 & \text{if } i \in \mathcal{V}_{\text{valid}}(q_t) \\ -\infty & \text{if } i \notin \mathcal{V}_{\text{valid}}(q_t) \end{cases}$$

where $q_t \in Q$ represents the active state of an execution automaton $\mathcal{M}_G$, and $\mathcal{V}_{\text{valid}}(q_t)$ denotes the subset of vocabulary tokens whose byte encodings produce valid transitions: $\delta^*(q_t, \mathbf{b}^{(i)}) \neq \emptyset$. Normalized probabilities over masked logits $\mathbf{z}_t' = \mathbf{z}_t + \mathbf{M}(q_t)$ guarantee that sampled tokens satisfy the target formal grammar with probability $1.0$ at the logit surface.
:::
...
Because the prompt for step $t+1$ depends strictly on the empirical side effects of the tool invoked at step $t$, the agent cannot pre-generate future tokens or parallelize across successive steps of its own trajectory. Along its own execution path, the agent operates at an irreducible batch size of $B = 1$, whose phased execution profile is summarized in @tbl-trajectory-lifecycle-b1.
