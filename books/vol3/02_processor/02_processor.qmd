...
```{mermaid}
%%| fig-cap: "The four-stage lifecycle of a single foundation-model invocation. The agent runtime stages context and budgets; the inference service evaluates the learned model across prefill and decode; the service returns a candidate payload with status and resource usage; and the runtime interprets the proposal to decide the next action."
sequenceDiagram
    autonumber
    participant Runtime as Agent Runtime (Caller)
    participant Service as Inference Service (Serving)
    participant Model as Learned Model (Core)

    Note over Runtime: 1. Context Staging & Budgeting
    Runtime->>Service: Dispatch Request(Context, K_max, T_max, Model_ID)

    activate Service
    Note over Service,Model: 2. Scoring & Candidate Generation
    Service->>Model: Forward Pass(Prefill Staged Tokens)
    Model-->>Service: Initial Next-Token Logits
    loop Autoregressive Decode Loop (until stop condition)
        Service->>Service: Sample Token y_t & Update KV Cache
        Service->>Model: Forward Step(y_t)
        Model-->>Service: Logits z_{t+1}
    end

    Note over Service: 3. Response Packaging
    Service-->>Runtime: Response Envelope(Candidate, Status, Token Usage, Latency)
    deactivate Service

    Note over Runtime: 4. Interpretation & Action Dispatch
    Runtime->>Runtime: Validate Schema & Permissions
    Runtime->>Runtime: Decide Next Step (Execute Tool, Reprompt, or Terminate)
```
:::
