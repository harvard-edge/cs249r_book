...
@Algo-grammar-logit-masking details how an inference engine applies deterministic grammar constraints before token emission. By maintaining the current state $q \in \mathcal{Q}$ of a pushdown automaton compiled from the target schema, the engine tests each vocabulary token's byte sequence against the transition function $\delta$, builds a boolean mask $\mathbf{m}$ over the vocabulary $\mathcal{V}$, and suppresses illegal continuations with $-\infty$ prior to the softmax kernel.
...
3. **Sampling and State Advancement (Lines 19–20):** The sampling kernel selects token $v^*$ from the masked distribution, and the pushdown automaton advances its state to $q'$ along the decoded byte path $\delta^*(q, \textsc{DecodeBytes}(v^*))$.
...
@Algo-speculative-decoding-rollback specifies the draft-then-verify execution harness with memory reclamation. After the lightweight draft model sequentially proposes $K$ speculative tokens, the target model evaluates all candidates concurrently in a single batched prefill pass. Rejection sampling either accepts the candidate or halts the speculative sequence, sampling a replacement from the positive residual distribution and releasing uncommitted speculative pages from accelerator memory.
...
1. **Phase 1: Lightweight Sequential Drafting (Lines 3–7):** The small draft model $q_\phi$ (typically $1\text{--}3\text{B}$ parameters) generates $K$ candidate tokens sequentially. Because the draft model has $20\text{--}70\times$ fewer parameters than the target model, its autoregressive decode steps stream significantly fewer weight bytes from memory, executing at upwards of $150\text{--}250\text{ tokens/s}$.
2. **Phase 2: Batched Parallel Verification (Line 8):** The primary target model $p_\theta$ evaluates all $K$ draft tokens concurrently within a single prefill forward pass. This is the pivotal roofline transition: evaluating $K$ tokens simultaneously transforms the target model's execution from a memory-bandwidth-bound matrix-vector product ($M \times K$ with $M=1$) into a compute-bound matrix-matrix multiplication ($M=K$). The memory controller streams the large target model weights (e.g., $140\text{ GB}$ in FP16) from HBM across the on-chip SRAM caches exactly once, amortizing the weight transfer cost across all $K$ candidate tokens.
3. **Phase 3: Rejection Sampling with Zero Distribution Shift (Lines 10–24):** The algorithm sequentially evaluates acceptance probability $\alpha_i = \min(1, \mathbf{P}[i, \hat{y}_i] / q_\phi(\hat{y}_i))$. The mathematical beauty of modified rejection sampling is that it guarantees zero output degradation:
