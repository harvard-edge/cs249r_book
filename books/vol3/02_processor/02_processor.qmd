...
None of these approaches achieves the unconditional, hardware-enforced separation that MMU-based $W \oplus X$ provides for general-purpose processors. The fundamental difficulty is that the transformer attention mechanism is a dense bilinear operator over the entire context; any architectural mechanism that restricts its expressivity to enforce privilege separation necessarily reduces its modeling capacity. Where hardware-enforced token privilege rings remain unattainable within the attention kernel, systems security must be enforced structurally at the agent orchestration boundary via monotonic capability attenuation across delegation trees, as diagrammed in @fig-vol3-processor-capability-attenuation.
...
Whether the accuracy-security trade-off can be made acceptable for production agentic systems is an open research problem, and one whose resolution has direct implications for the system contracts examined throughout this chapter.
