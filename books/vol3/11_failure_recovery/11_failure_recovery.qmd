...
These are per-step saga states, kept in the trajectory record beside the trajectory's own state from @sec-vol3-controlplane-statemachine, not replacements for it. While a forward step or a compensator runs, the trajectory cycles through `WAITING_TOOL` as it does for any tool call. When a saga reaches `TERMINATED` after a clean unwind, the trajectory leaves through `FINALIZING` and ends `FAILED`. A step that enters `PANIC` drives the trajectory into terminal escalation, which pauses it for an operator, and the step is recorded as `ESCALATED` until that person resolves it.
...
def unwind_saga(record: TrajectoryRecord) -> None:
    ...
    saga.set_state(SagaState.TERMINATED)
```
...
Escalation is a fixed procedure. The harness first settles whatever is in flight. It cancels an open model call, withdraws a held approval, and cancels each outstanding tool call or, when cancellation cannot be confirmed, marks it in doubt so that settlement by its idempotency key can resolve it later. With nothing outstanding, the trajectory is back at a turn boundary in `READY`, and the harness pauses it through the guarded transition of @sec-vol3-controlplane-statemachine, the only path into `PAUSED` that state machine allows. No model call or dispatch happens while paused, and the record, workspace, and log stay available to the operator. The harness then revokes the trajectory's grants and brokered credentials (@sec-vol3-sandboxes-credential-brokering) so that no pending callback can issue further effects while a person investigates. It classifies the residue by where the failure sits relative to the pivot, either as a failed compensation before the pivot, with a list of resources that remain uncompensated, or as a stalled finalization after it, with the committed pivot and the steps still owed. Finally, it pages an operator with an incident record that points to the exact log position, so the person starts from the same facts the runtime had.

```python
def escalate(record: TrajectoryRecord, err: ToolError) -> None:
    record.settle_in_flight(err)                 # cancel, or mark in doubt; ends in READY
    transition(record, "PAUSED", err)            # guarded READY -> PAUSED; no model call, no dispatch
...
* Prices are the illustrative ones of the repair notebook above, with no prefix caching.

**Math**:

* Input tokens over $n$ turns follow the same arithmetic series as the budget example of @sec-vol3-controlplane-accounting: $n \cdot s_0 + \Delta \cdot n(n-1)/2$.
...
**Systems insight**: The cost of a loop grows with the square of its length, because every turn re-sends a context that the loop keeps enlarging. Prefix caching lowers the price per re-sent token, as the budget example of @sec-vol3-controlplane-accounting showed, but not the turns or the minutes. A ceiling bounds the worst case, and only a progress signal keeps the typical loop short. What that does to the cost of each resolved task across a fleet is the goodput arithmetic of \ref{nbk-01-trajectory-goodput}. The detector's contribution is to move the early stop from an arbitrary turn to the first repeated state.
...
Detecting a loop is half the job, and the harness also needs a defined response, because silently dropping a tool call leaves the trajectory in an undefined state. @Tbl-vol3-watchdog-escalation-protocol gives three tiers of increasing cost, the progress counterpart of the budget ladder of @sec-vol3-controlplane-accounting.
