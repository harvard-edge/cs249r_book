...
- **Architect** Copy-on-Write (CoW) page sharing for speculative search trees (such as Tree-of-Thoughts [@yao2023tree] and MCTS [@browne2012survey]), calculating the memory capacity expansion achieved by aliasing parent reasoning paths.
...
Consider an autonomous agent that has just spent twenty minutes navigating a repository, issuing grep commands, and reading logs. When it invokes a tool to run tests, it receives a 5,000-line traceback. To decide on the next step, the agent must process this new information alongside its entire history. In the classic Von Neumann computer architecture, the central processing unit interacts with a hierarchical memory subsystem: fast, expensive registers and SRAM caches sit adjacent to the arithmetic logic units, backed by larger, slower DRAM, which is in turn paged to non-volatile secondary storage. Operating systems bridge these tiers through **virtual memory**\index{virtual memory}[^fn-virtual-memory] [@kilburn1962one; @denning1970virtual], presenting software with an illusion of vast, contiguous, isolated address spaces while dynamically mapping logical pages onto fragmented physical frames in silicon.

An autonomous LLM agent operates under a strikingly similar paradigm. The transformer foundation model [@vaswani2017attention] functions as the central reasoning engine. Its registers and primary working memory, however, do not consist of standard byte-addressable DRAM. Instead, the active working memory of an agent consists of token sequences materialized as high-dimensional **Key-Value (KV) activation tensors**\index{Key-Value (KV) activation tensors}[^fn-kv-activation-tensors] resident within accelerator High-Bandwidth Memory (HBM).
...
1. **The Allocation Pathology (Memory Fragmentation):** Standard deep learning frameworks allocate memory in contiguous chunks. Because the runtime cannot predict how many tokens an agent will generate or how many interaction turns a task will require, it must either pre-allocate memory for the theoretical maximum context window (e.g., $128\text{k}$ tokens) or dynamically re-allocate and copy tensors as the sequence grows. Pre-allocation leads to massive **internal fragmentation**\index{internal fragmentation}[^fn-internal-fragmentation], where up to $80\%$ of accelerator HBM sits idle, reserved for tokens that are never generated. Dynamic reallocation causes severe **external fragmentation**\index{external fragmentation}[^fn-external-fragmentation], where memory becomes partitioned into unusable gaps, precipitating premature Out-of-Memory (OOM) crashes even when total free memory is abundant [@kwon2023efficient].
2. **The Compute Pathology (Redundant Prefill):** Consecutive turns of an agent trajectory share between $85\%$ and $98\%$ of their prompt tokens. In test-time search algorithms (such as Tree-of-Thoughts [@yao2023tree] and Monte Carlo Tree Search [@browne2012survey], detailed in @sec-vol3-deliberation), dozens of speculative branches share identical parent reasoning chains. If the serving runtime treats each turn or branch as an isolated request, it recomputes the prompt prefill from scratch, burning tens of billions of floating-point operations (FLOPs) re-evaluating invariant system instructions, tool schemas, and historical steps.

Resolving these pathologies requires virtualizing the agent's context memory. Just as Ferranti Atlas engineers resolved core-memory scarcity in 1962 by introducing virtual memory paging [@kilburn1962one], modern agent serving systems introduce **Virtual Context Memory**\index{Virtual Context Memory}. By partitioning continuous Key-Value tensors into uniform physical memory blocks and indexing those blocks through dynamic Radix prefix trees, the inference engine decouples logical sequence representations from physical silicon addresses, unlocking zero-copy prefix sharing and demand-paged execution.
...
These virtual memory architectures map directly to the high-dimensional tensors computed by the foundation model. Understanding how to manage this memory requires first dissecting the physical geometry and arithmetic footprint of the Key-Value cache itself.
...
When an agent generates a multi-file pull request within a 128,000-token context, the underlying transformer must attend to the entire history for every single generated token. To avoid catastrophic recomputation, the system caches intermediate activation vectors in accelerator memory. During autoregressive decoding, each transformer layer attends to all previous tokens in the sequence. To avoid recomputing Key and Value vectors for historical tokens at every generation step, the engine caches these intermediate activations in accelerator HBM.
...
To build physical intuition, consider how this footprint accumulates during an agent's execution. Every time the agent generates a single token, it must cache a Key vector and a Value vector across every single transformer layer. For a standard Multi-Head Attention model, this requires storing $2 \times b \times d_{\text{model}}$ bytes per layer. When multiplied across dozens of layers, caching just one token can consume over a megabyte of physical silicon. As the agent's context expands to tens of thousands of tokens, this volatile memory footprint rapidly eclipses the static model weights, making efficient allocation the absolute bottleneck for agent concurrency.
...
1. **Multi-Query Attention (MQA)**\index{Multi-Query Attention (MQA)}[^fn-mqa] [@shazeer2019fast]: Collapses all Key and Value heads into a single head shared across all Query heads ($n_{\text{kv\_heads}} = 1$). The footprint reduces by a factor of $n_{\text{heads}}$:
   $$S_{\text{token, MQA}} = 2 \times b \times n_{\text{layers}} \times d_{\text{head}}$$
2. **Grouped-Query Attention (GQA)**\index{Grouped-Query Attention (GQA)}[^fn-gqa] [@ainslie2023gqa]: Partitions Query heads into $G$ groups, assigning one Key and one Value head to each group ($n_{\text{kv\_heads}} = G$). The memory footprint scales as:
   $$S_{\text{token, GQA}} = 2 \times b \times n_{\text{layers}} \times (G \cdot d_{\text{head}}) = \frac{G}{n_{\text{heads}}} S_{\text{token, MHA}}$$
3. **Multi-Head Latent Attention (MLA)**\index{Multi-Head Latent Attention (MLA)}[^fn-mla] [@deepseek2024v2]: Rather than caching uncompressed Key and Value heads, MLA projects Key and Value vectors into a low-dimensional latent space of dimension $d_c \ll n_{\text{heads}} \times d_{\text{head}}$, alongside a decoupled rotary key vector $d_R$. During generation, only the compressed latent vector $\mathbf{c}_t^{\text{KV}}$ and the rotary key $\mathbf{k}_t^R$ are stored:
...
|:------------------------|:----------------|:--------------------|:-------------------|:---------------|:------------------------------------------|:---------------------------------------------|:--------------------------------|:---------------------------------|
| **LLaMA-1 / LLaMA-2** [@touvron2023llama; @touvron2023llama2] | 70B             | 80                  | 8192               | MHA            | 64 / 64                                   | $1\text{,}310\text{,}720$ ($1.31\text{ MB}$) | $41.94\text{ GB}$               | $167.77\text{ GB}$               |
| **Llama-3 / Llama-3.1** [@dubey2024llama3] | 70B             | 80                  | 8192               | GQA ($8:1$)    | 64 / 8                                    | $163\text{,}840$ ($160\text{ KB}$)           | $5.24\text{ GB}$                | $20.97\text{ GB}$                |
| **Qwen-2.5** [@qwen2024qwen25] | 72B             | 80                  | 8192               | GQA ($8:1$)    | 64 / 8                                    | $163\text{,}840$ ($160\text{ KB}$)           | $5.24\text{ GB}$                | $20.97\text{ GB}$                |
| **Command R+** [@cohere2024command] | 104B            | 64                  | 12288              | GQA ($8:1$)    | 96 / 8                                    | $131\text{,}072$ ($128\text{ KB}$)           | $4.19\text{ GB}$                | $16.78\text{ GB}$                |
| **DeepSeek-V2 / V3** [@deepseek2024v2; @deepseek2024v3] | 236B / 671B     | 60                  | 5120               | MLA            | Latent ($512 + 64$)                       | $69\text{,}120$ ($67.5\text{ KB}$)           | $2.21\text{ GB}$                | $8.85\text{ GB}$                 |
...
This fragmentation crisis is compounded by the way agents interact with their environments. Because agents iteratively append actions and observations to a shared context, their memory access patterns exhibit extreme prefix overlap. The consequence is a dual challenge: the runtime must manage fragmented physical memory while simultaneously exploiting this prefix overlap to prevent redundant computation.
...
When an agent executes a bash command and observes the terminal output, its next prompt to the model is nearly identical to the previous one—only the new action and observation are appended. Autonomous agent workloads exhibit fundamentally different memory access and lifespan characteristics than standard conversational chatbots. A chatbot interaction typically consists of short, independent queries with localized dialogue history. In contrast, an autonomous agent interacts through structured, long-horizon operational environments.
...
In his foundational 1970 paper, Peter Denning established that programs exhibit **temporal locality**\index{temporal locality}[^fn-temporal-locality] (memory locations referenced recently are likely to be referenced again soon) and **spatial locality**\index{spatial locality}[^fn-spatial-locality] (storage locations adjacent to recently referenced memory are likely to be accessed together) [@denning1970virtual].

In autonomous agent systems, context access patterns exhibit an extreme variant: **Trajectory Prefix Locality**\index{Trajectory Prefix Locality}[^fn-trajectory-prefix-locality].
...
Consider serving Llama-3-70B [@dubey2024llama3] ($P \approx 7 \times 10^{10}$) on a $32\text{,}768$-token agent prompt. Computing the prefill pass requires:
...
If an agent executes a 20-turn trajectory on a stateless serving infrastructure, the cluster spends over **$180$ seconds** of pure GPU time re-evaluating identical tokens. This redundant computation destroys interactive response times (inflating **Time-to-First-Token (TTFT)**\index{Time-to-First-Token (TTFT)}[^fn-ttft]), wastes enormous electrical power, and creates severe queueing backpressure.
...
Recognizing the computational necessity of retaining the Key-Value cache reveals a new systems constraint. The runtime must physically store these growing, unpredictable trajectories without triggering the catastrophic fragmentation observed in static allocators. Resolving this constraint requires a shift from contiguous tensor storage to dynamic, block-level paging.
...
If an operating system required every application to occupy contiguous physical RAM, modern multitasking would collapse the moment memory became fragmented. Early LLM serving engines suffered exactly this collapse, stranding up to 80% of accelerator memory because variable-length agent trajectories required contiguous tensor allocations. To solve the dual challenges of physical memory fragmentation and zero-copy prefix sharing, Kwon et al. introduced **PagedAttention** [@kwon2023efficient]. Inspired by classical operating system virtual memory paging, PagedAttention breaks the requirement that an agent's KV cache must occupy contiguous memory in accelerator HBM.
...
2.  **Distinction**: Traditional attention kernels (such as FlashAttention [@dao2022flashattention]) require continuous memory strides to compute $\text{softmax}(QK^T/\sqrt{d})V$. PagedAttention executes attention directly across scattered physical memory blocks through an integrated address lookup table inside GPU SRAM.
...
2. **Bounding Internal Fragmentation:** Internal fragmentation is restricted strictly to the final block of an active sequence. Imagine an agent generates a trajectory of 35 tokens, and the system uses a block size of $B = 16$. The first 32 tokens perfectly fill two physical blocks (16 tokens each) with zero wasted space. The remaining 3 tokens spill into a third physical block. This third block reserves space for 16 tokens but only holds 3, resulting in 13 empty token slots of internal fragmentation. Because this waste only ever occurs in the single, final, partially-filled block of a sequence, it is mathematically bounded. For block size $B$, the expected internal waste per agent sequence is:
...
The primary capability unlocked by virtual block paging is **zero-copy branching** via Copy-on-Write (CoW).
...
1. **Shared Block Aliasing:** When Branch $A$, $B$, and $C$ are initialized, their page tables simply copy the physical block pointers of the parent context. No activation data is copied. The physical blocks enter a shared state, incrementing their internal **reference counter**\index{reference counter}[^fn-reference-counter] ($\text{ref\_count} = 3$).
...
![Copy-on-Write (CoW) block sharing across speculative agent rollouts. Logical blocks 0 and 1 are shared between Branch A and Branch B, while divergent generation allocates private physical blocks.](/assets/images/diagrams/vol3/fig-vol3-vm-pagedattention.png){width="100%"}{#fig-vol3-vm-pagedattention}
...
PagedAttention successfully decouples logical tokens from physical silicon blocks, allowing multiple branches to share memory efficiently. However, it requires the serving runtime to explicitly know which requests share a prefix. To handle asynchronous multi-agent swarms where sharing is implicit, the system needs a dynamic indexing structure to discover these overlapping contexts on the fly.
...
Imagine ten identical software engineering subagents spun up in parallel to test different patches for the same repository. They all share the exact same 24,000-token system prompt and repository map. Without an indexing mechanism to recognize this overlap, the cluster would allocate ten identical copies of the context, wasting gigabytes of memory. While PagedAttention provides the physical paging mechanism to store non-contiguous blocks and share identical physical pages across known forks, an autonomous serving system requires a higher-level data structure to **automatically discover and index** reusable prefixes across arbitrary agent requests.
...
3. **Tree Insertion and Edge Splitting:** If the matched path terminates in the middle of an existing edge, the Radix tree dynamically splits that edge to accommodate the divergence. The original node is split at the exact token where the new sequence diverges from the cached sequence. The shared prefix becomes a parent node, pointing to physical blocks that are now shared via reference counting. Two new child edges are then created: one preserving the remainder of the original sequence, and one for the newly generated suffix. The newly allocated physical blocks for the suffix are attached to the new leaf node. This edge-splitting mechanism ensures that any future agents sharing either the parent prefix or the divergent branches can match exactly the tokens they need, enabling zero-overhead prefix sharing across complex branching search trees without manual cache management.

### Reference Counting and LRU Eviction

Because accelerator HBM is finite, the Radix tree cannot retain all historical trajectories indefinitely. Memory management is governed by a **Reference-Counted LRU Eviction Policy**\index{Reference-Counted LRU Eviction Policy}[^fn-lru-eviction]:
...
In test-time search topologies (such as Tree-of-Thoughts [@yao2023tree] and Monte Carlo Tree Search [@browne2012survey], @sec-vol3-deliberation), an agent evaluates $K$ alternative actions branching from a root state $s_0$.
...
**Context**: In March 2023, OpenAI experienced a high-profile data disclosure incident in ChatGPT [@openai2023chatgptoutage]. ChatGPT used a shared Redis cache in front of its persistent databases to cache user profile and session metadata, accessed via an asynchronous Python client (`redis-py`) running under `asyncio`.
...
While dynamic prefix indexing and block-level paging resolve the fundamental capacity and compute bottlenecks of agent serving, their implementation introduces subtle edge cases. These mechanisms routinely trap designers who assume neural virtual memory behaves identically to persistent storage or semantic caches.
...
Assuming that because an inference engine deploys Radix tree prefix caching, developers can format prompt strings without regard for whitespace, JSON structure, or dynamic variable placement. The reality is Radix trees match on exact token IDs, not semantic strings. Subword tokenizers (such as BPE [@sennrich2015neural]) are notoriously sensitive to whitespace and leading characters. Formatting differences drop the cache hit rate from 98% to 0%, forcing a complete prefill. Production prompt compilers must enforce canonical serialization.

**Fallacy: Adopting PagedAttention completely eliminates memory capacity limits in test-time search.**
Believing that zero-copy Copy-on-Write branching allows an agent to expand speculative search trees (Tree-of-Thoughts [@yao2023tree] or MCTS [@browne2012survey]) to arbitrary breadth and depth. While PagedAttention shares parent nodes with zero replication overhead, the leaves of the search tree expand exponentially. Without strict admission control, depth pruning, and token-level budgeting, speculative rollouts trigger memory thrashing.

**Pitfall: Silent cache poisoning across tool-environment mutations (The Semantic Invalidation Dilemma).**
In an autonomous SWE agent, an agent executes a bash command that edits a file. If the prompt included the output of a prior file state, the Radix tree caches this trajectory. If an optimization system rewrites history to insert the updated file state in-place, it invalidates the prefix. More dangerously, cache entries must be leased with cryptographic content hashes to avoid semantic invalidation across independent agent runs.

**Pitfall: Placing dynamic nonces, timestamps, or session IDs at the root of prompt templates.**
Prefix matching proceeds strictly from left to right. By placing a millisecond timestamp or random UUID at the root of the prompt string, every incoming request presents a completely unique initial 20 tokens. The Radix tree lookup fails immediately at the root node, causing the entire cluster to operate at a 0% prefix cache hit rate. Dynamic variables must always be placed at the absolute tail of the prompt.

## Summary {#sec-vol3-vm-summary}

* **Decoupling Logical State from Physical Silicon:** Autonomous agent trajectories must not be allocated as contiguous physical tensors. Block-level paging (PagedAttention) eliminates external fragmentation and bounds internal waste to the final block of a sequence.
* **Exploiting Trajectory Prefix Locality:** Agent perception-action loops exhibit monotonic prefix expansion. Recomputing prompt prefills on stateless infrastructure wastes over 90% of cluster FLOPs and inflates Time-to-First-Token by orders of magnitude.
* **Automating Sharing with Dynamic Prefix Trees:** Radix tree indexing (RadixAttention) automatically identifies common prefix paths across multi-turn trajectories, parallel swarms, and speculative search branches, executing chunked prefills only for divergent suffixes.
* **Speculative Branching via Copy-on-Write:** Evaluating alternative reasoning branches requires aliasing parent page tables rather than copying activation tensors, widening speculative search capacity by over 30x.
* **Enforcing Strict Prompt Hygiene:** Prefix caching operates on token sequence identity. Non-deterministic JSON serialization, BPE whitespace shifts, and dynamic root nonces drop cache hit rates to zero, inducing cluster-wide queueing thrash.
* **Bounding Speculative Expansion:** Zero-copy branching does not eliminate memory limits; the leaves of a search tree still expand exponentially, requiring strict admission control and token-level budgeting to prevent out-of-memory crashes.

[^fn-virtual-memory]: **virtual memory**\index{virtual memory}: A memory management technique that provides an "idealized abstraction of the storage resources that are actually available on a given machine" which "creates the illusion to users of a very large (main) memory."
[^fn-kv-activation-tensors]: **Key-Value (KV) activation tensors**\index{Key-Value (KV) activation tensors}: High-dimensional memory states maintained by a transformer's attention mechanism to cache the representations of previously seen tokens, preventing redundant recalculations during autoregressive decoding.
[^fn-internal-fragmentation]: **internal fragmentation**\index{internal fragmentation}: Wasted memory space that occurs when a system allocates memory in fixed-size blocks or continuous chunks that are larger than what the requested data actually requires.
[^fn-external-fragmentation]: **external fragmentation**\index{external fragmentation}: The scattering of free memory into small, non-contiguous holes over time, which prevents the allocation of large continuous blocks even when the total aggregate free memory is sufficient.
[^fn-mqa]: **Multi-Query Attention (MQA)**\index{Multi-Query Attention (MQA)}: An attention architecture variant that collapses all Key and Value heads into a single shared head across all Query heads to drastically reduce the KV cache footprint.
[^fn-gqa]: **Grouped-Query Attention (GQA)**\index{Grouped-Query Attention (GQA)}: An intermediate attention architecture that partitions Query heads into groups and assigns one Key and one Value head to each group, balancing the memory efficiency of MQA with the performance of standard Multi-Head Attention.
[^fn-mla]: **Multi-Head Latent Attention (MLA)**\index{Multi-Head Latent Attention (MLA)}: An advanced attention mechanism that projects Key and Value vectors into a compressed, low-dimensional latent space to minimize memory consumption during generation.
[^fn-temporal-locality]: **temporal locality**\index{temporal locality}: The principle that memory locations referenced recently are highly likely to be accessed again in the near future.
[^fn-spatial-locality]: **spatial locality**\index{spatial locality}: The principle that memory locations physically adjacent to recently referenced memory are highly likely to be accessed soon.
[^fn-trajectory-prefix-locality]: **Trajectory Prefix Locality**\index{Trajectory Prefix Locality}: The extreme variant of temporal and spatial locality seen in autonomous agents where iterative perception-action loops ensure that the vast majority of tokens in a sequence form an invariant prefix shared with subsequent steps.
[^fn-cow]: **Copy-on-Write (CoW)**\index{Copy-on-Write (CoW)}: A resource-management technique where multiple callers given access to the same resource share a single pointer until one of them modifies the resource, at which point a private copy is created.
[^fn-reference-counter]: **reference counter**\index{reference counter}: An integer maintained by the memory allocator indicating the number of active, in-flight agent requests currently mapping a given physical memory block.
[^fn-radix-tree]: **Radix tree**\index{Radix tree}: A space-optimized, compressed prefix tree (Trie) used by the serving engine to automatically discover and index sequence prefixes across multi-turn trajectories and shared prompts.
[^fn-lru-eviction]: **Reference-Counted LRU Eviction Policy**\index{Reference-Counted LRU Eviction Policy}: A memory scheduling strategy that retains prefix blocks in accelerator memory until pressure requires eviction, prioritizing the reclaiming of unpinned physical blocks that were accessed least recently.

[^fn-ttft]: **Time-to-First-Token (TTFT)**\index{Time-to-First-Token (TTFT)}: The latency measured from when an inference engine receives a prompt to when it emits the first generated token, largely dominated by the prefill computation.
