...
\begin{marginfigure}
\mlagentstack{10}{95}{20}{10}{20}{20}
...
_Why does pretraining scaling hit an insurmountable ceiling that only execution oracles can breach?_

Treating the foundation model as a static, frozen stochastic processor requires designing elaborate external operating systems around it: context working sets to conserve high-bandwidth memory, virtual memory paging for multi-tenant KV caches, episodic vector storage for non-volatile recall, execution schedulers to manage heavy-tailed workloads, system calls to enforce capability boundaries, and microVM sandboxes to isolate untrusted execution. While this runtime scaffolding is indispensable for system stability and security, relying exclusively on inference-time prompting imposes severe economic and physical penalties: quadratic attention prefill overhead, KV-cache saturation, token recall degradation across extended horizons, and brittle adherence to complex API schemas. To overcome these constraints, agent systems engineering must transition from pure runtime containment to policy adaptation, which begins with the synthetic data flywheel. Human trajectory demonstrations are too slow, prohibitively expensive, and fundamentally unscalable for the combinatorial state spaces of multi-turn tool interaction. The synthetic data flywheel bypasses human annotation through automated task generation, stochastic teacher exploration, execution-based rejection sampling against deterministic oracles, and search distillation from high-compute deliberation trees into compact, single-pass policy weights. In agentic systems, the runtime must co-design the neural policy proposal engine with deterministic execution boundaries.
...
::: {.callout-takeaways title="The Synthetic Data Flywheel Invariant"}
...
::: {.callout-chapter-connection title="From Data Flywheels to Trajectory Fine-Tuning"}
...
## Quantitative Problems {#sec-vol3-data-flywheel-problems}

1. **Execution-Based Rejection Sampling Yield and Cost**
   An engineering team generates synthetic trajectories using an 8B parameter model deployed on H100 GPUs. Generating a single trajectory candidate takes $t_{\text{gen}} = 3$ seconds and costs $\$0.002$ in GPU inference compute. The deterministic oracle (a Python unit test sandbox) takes $t_{\text{eval}} = 8$ seconds and costs $\$0.0005$ in CPU compute. If the team uses a generation temperature $T = 0.8$, empirical data shows that $12\%$ of generated trajectories pass the oracle. 
   Calculate the expected cost and total compute time (assuming sequential execution on a single worker node) to produce one successfully verified trajectory. What is the break-even acceptance rate if human demonstration costs $\$15.00$ per verified trajectory?

2. **Compute Cluster Utilization and Disaggregation**
   A synthetic data generation cluster consists of $N_{\text{GPU}} = 64$ inference nodes and $N_{\text{CPU}} = X$ verification nodes. A GPU node can generate $40$ trajectories per minute. A CPU verification node running Firecracker microVMs can verify $15$ trajectories per minute.
   a) To achieve $100\%$ utilization of the GPU nodes (preventing oracle-bound starvation), what is the minimum number of CPU nodes ($X$) required?
   b) If the orchestration messaging queue overhead adds a constant $200$ ms delay to every trajectory evaluated by the CPU nodes, recalculate the effective throughput of a CPU node and the new required $N_{\text{CPU}}$ to keep the GPUs fully saturated.

3. **Student Entropy vs. Diversity Degradation**
   During search distillation, an MCTS agent explores a state graph with a branching factor $b = 5$ for a trajectory of depth $H = 10$. The search discovers exactly $3$ unique successful paths out of the entire tree. A student policy is trained via behavioral cloning on only the shortest of these $3$ paths.
   If the test environment injects a single node failure along the shortest path during production inference (forcing a branch alteration), what is the probability the student policy successfully recovers, assuming the student's action distribution collapsed entirely (Dirac delta) to the single training path? How does this change if the student was trained on all $3$ paths with uniform weighting?
