...
- In @sec-vol3-observability-hermetic-gyms, we formalize the systems architecture of **Hermetic Evaluation Gyms**\index{Hermetic Evaluation Gyms}, defining the five invariants of reproducible sandboxing, the unbiased Pass@$k$ estimator, and the economics of Trajectory Goodput ($G$).
- In @sec-vol3-observability-time-travel-replay, we develop **Time-Travel Replay Engines**\index{Time-Travel Replay Engines}, detailing hierarchical OpenTelemetry GenAI tracing, deterministic boundary event sourcing, state rewind via Copy-on-Write (CoW) filesystems, counterfactual branch forking, and Amdahl's Law latency profiling.
...
To replace static datasets with dynamic environments, the machine learning systems community developed **Agent Gyms**—standardized, interactive execution environments that simulate real-world digital ecosystems.[^fn-hermetic-evaluation-gym]
