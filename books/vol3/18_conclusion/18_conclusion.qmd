...
![**Host Agent Supervisor Operating Envelope and Escalation Dispatcher**: The supervisor enforces multi-dimensional operating constraints including token/time budgets, error ceilings ($K \le 3$), sandbox containment ($A=0$), and durable Write-Ahead Logging. Any invariant breach triggers deterministic escalation policies to halt cascading errors or quarantine execution.](images/svg/fig-vol3-operating-envelope-escalation.svg){#fig-vol3-operating-envelope-escalation width="95%"}
...
![**Synthesized Four-Tier Memory Hierarchy**: The synthesized four-tier memory hierarchy of an agentic computer, detailing the data flow from authoritative external artifacts down to accelerator HBM, along with explicit write-invalidation and tombstoning feedback loops.](images/svg/vol3-memory-hierarchy-synthesis.svg){#fig-vol3-memory-hierarchy-synthesis width="95%"}
...
![**Systems Intervention Decision Flowchart**: The Systems Intervention Decision Flowchart. When an agent fails its workload contract, engineers evaluate interventions starting at Level 1, escalating to higher rungs only when empirical evidence demonstrates that lower-tier modifications cannot resolve the failure mode.](images/svg/fig-vol3-intervention-ladder-flowchart.svg){#fig-vol3-intervention-ladder-flowchart width="95%"}
...
![**Digital Trajectory Assumptions versus Physical Embodied Constraints**: In digital systems, state mutations are buffered in speculative CoW sandboxes and validated by mechanical oracles before commitment. In physical systems, actuation directly couples to continuous, irreversible thermodynamic environments.](images/svg/vol3-digital-vs-physical-boundary.svg){#fig-vol3-digital-vs-physical-boundary width="95%"}

These four invariants transform agent design into a search problem across discrete directed acyclic graphs (@fig-vol3-digital-vs-physical-boundary). If a candidate action $a_t$ yields an execution error or fails an invariant check, the runtime discards the execution fork, prunes the context prefix, and samples an alternative trajectory. The cost of failure is bounded by the computational expenditure of inference tokens and ephemeral container recycling.
...
![**Hierarchical Safety Shield Architecture for Embodied Systems**: The systems boundary decomposes execution into tiered layers: an unprivileged high-level planner ($A=0$), a deterministic safety shield enforcing Control Barrier Functions ($< 1$ ms), and a hard real-time RTOS controller managing physical actuators.](images/svg/fig-vol3-embodied-safety-shield.svg){#fig-vol3-embodied-safety-shield width="95%"}
