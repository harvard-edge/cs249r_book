...
| \ref{pri-vol3-memory-bandwidth-decoding} | Memory-bandwidth-bounded decoding                      | I            | None (prices the single call)             | Output tokens cost far more than input tokens, and latency grows with output length                      |
...
Budgets and progress detection meet at the same seam. A budget bounds what a failing trajectory can spend, but a loop that reissues the same failing query will spend all of it. Progress detection and the circuit breaker on tool dispatch (@sec-vol3-failure-recovery) stop such a loop early, and they sit outside the model because the model cannot be relied on to notice its own repetition (principle \ref{pri-vol3-preemptive-interrupts}).
...
**Given** (illustrative inputs):

- Baseline: 12,000 input tokens and 800 output tokens per step, 5 steps per task, at \$2.50 per million input tokens and \$10.00 per million output tokens.
- Adapted model: 1,500 input tokens and 800 output tokens per step, 4 steps per task, at \$1.00 per million input tokens and \$4.00 per million output tokens.
- One-time investment: \$4,000 for data, \$500 for training compute, \$8,000 for evaluation and review, so $C_{\text{invest}} = \$12{,}500$.
- Expected volume: 20,000 pull requests per month.

**Per-task cost**:

$$C_{\text{task, base}} = 5 \times \left(12{,}000 \times \$2.50 \times 10^{-6} + 800 \times \$10.00 \times 10^{-6}\right) = 5 \times \$0.038 = \$0.190$$

$$C_{\text{task, adapt}} = 4 \times \left(1{,}500 \times \$1.00 \times 10^{-6} + 800 \times \$4.00 \times 10^{-6}\right) = 4 \times \$0.0047 = \$0.0188$$

**Break-even**:

$$N^* = \frac{\$12{,}500}{\$0.190 - \$0.0188} = \frac{\$12{,}500}{\$0.1712} \approx 73{,}000 \text{ tasks}$$

At 20,000 tasks per month, the investment pays back in about 3.7 months, just outside a quarter. The answer turns on inputs the team controls. Doubling the volume halves the payback, while a change to the tool definitions that forces retraining resets it.
:::
...
When a trajectory of several hundred steps fails at its final check, finding the step that caused the failure is an inverse problem with an exponential search space. Replay and counterfactual branching (@sec-vol3-durable-execution, @sec-vol3-evaluation-postmortems) localize failures after the fact, and intermediate checks every few steps shrink the search from the whole trajectory to one window. Assigning credit per turn during multi-turn reinforcement learning (@sec-vol3-rlvr) is harder, because the signal must be available at training scale rather than one postmortem at a time.
