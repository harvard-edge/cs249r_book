...
@Fig-vol3-capstone-reference-architecture places each subsystem where the chapter that built it left it. The runtime encapsulates the model call (@sec-vol3-foundation-model) and its test-time deliberation (@sec-vol3-test-time-compute), pages the KV cache and shares prefixes (@sec-vol3-kv-cache), retrieves durable state (@sec-vol3-long-term-memory), dispatches typed tool calls (@sec-vol3-tool-calling) into isolated microVM sandboxes (@sec-vol3-sandboxes), commits each step to an append-only log (@sec-vol3-durable-execution), compensates failed external steps through Sagas (@sec-vol3-failure-recovery), harvests verified traces (@sec-vol3-trajectory-curation) to adapt the policy (@sec-vol3-fine-tuning; @sec-vol3-rlvr), coordinates agent fleets (@sec-vol3-multi-agent), and reconstructs causal traces after the fact (@sec-vol3-evaluation).
...
### Dynamic trajectory orchestration and ownership {#sec-vol3-conclusion-trajectory-owners}
...
Because containment must be enforced beneath the code it contains (principle \ref{pri-vol3-zero-trust-sandboxing}), the execution harness runs the command inside an ephemeral microVM or a user-space kernel sandbox (@sec-vol3-sandboxes). The sandbox enforces defense-in-depth isolation:
...
The quantitative engineering trade-offs across these six tiers are detailed in @tbl-vol3-intervention-ladder-tradeoffs.
...
: **Intervention Ladder Trade-Offs**: The Systems Intervention Ladder Trade-Off Matrix. As organizations ascend the ladder, upfront non-recurring engineering and compute investments escalate by orders of magnitude, accompanied by increased system surface area and operational rigidity. {#tbl-vol3-intervention-ladder-tradeoffs}
...
The design discipline embodied by the intervention ladder applies the end-to-end argument as @sec-vol3-intro-end-to-end-boundary split it, into mechanical closure below the model and evidence above it, and as @sec-vol3-agent-economics-selection priced it. In an agentic architecture, functions placed at lower layers of the stack may be redundant or ineffective compared to providing them at the supervisory endpoints.
...
::: {.column-margin}
**Statistical Release Gates:** As established in @sec-vol3-evaluation, point estimates of agent success rates are statistically meaningless without confidence bounds. Release criteria must be formulated over the lower bound of the two-sided Wilson score interval at confidence level $1 - \alpha$.
:::
...
Furthermore, state modifications spanning multiple microservices cannot be executed as blind open-loop operations. The runtime enforces the Saga pattern (detailed in @sec-vol3-failure-recovery): every forward mutating action $a_t$ registered in the Write-Ahead Log must be paired with an executable compensating transaction $a_t^{-1}$. If an operational invariant trips during step $t+k$, the supervisor halts execution, suppresses further model decoding, and traverses the log backwards, executing compensations to unwind the environment to a known-consistent baseline state $s_0$.
