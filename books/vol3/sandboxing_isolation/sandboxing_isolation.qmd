...
::: {#fig-isolation-spectrum fig-env="figure" fig-pos="htb" fig-cap="**The Isolation Spectrum**: Comparing process containers, userspace kernels, WebAssembly, and hardware microVMs across latency, overhead, and containment boundaries. Containment strength and startup cost do not trade cleanly against each other here, since the microVM sits below the container on both, which is why the strongest boundary is also the practical default." fig-alt="Diagram comparing five isolation technologies: Linux containers, gVisor, WebAssembly, AWS Firecracker microVMs, and full legacy VMs across cold-boot latency, memory footprint, host kernel attack surface, and agent security suitability."}
...
```{python}
#| label: vol3-sandboxing-boot-budget
#| echo: false
# +-----------------------------------------------------------------------------
# | ISOLATION PROFILES AND THE PER-CALL BOOT BUDGET
# +-----------------------------------------------------------------------------
# | Context: @tbl-isolation-comparison, notebook
# |          \ref{nbk-sandboxing-boot-budget}, and the container, gVisor, and
# |          Firecracker paragraphs above the table.
# |
# | Goal:    Hold one set of isolation profiles for the whole section, then
# |          charge each of them against the wall clock of one trajectory.
# | Show:    Cold-boot and memory ranges per isolation primitive, the per-call
# |          boot budget an interactive agent can afford, and what each
# |          technology actually costs against it.
# | How:     Isolation's share of wall clock is T/(t_productive + T), so the
# |          budget solves to T = share * t_productive / (1 - share). The call
# |          count cancels, which is the result the notebook turns on.
# |
# | Note:    The profile ranges are the chapter's operating figures. Firecracker
# |          traces to @agache2020firecracker; the other four are engineering
# |          estimates and are declared here rather than given false registry
# |          provenance. They are a registry candidate once measured.
# +-----------------------------------------------------------------------------
from mlsysim import *

def _p(value):
    """Drop a decimal place when the value lands on a whole number."""
    return 0 if abs(value - round(value)) < 1e-9 else 1

class SandboxBudget:
    # +-- 1. LOAD (Constants) --------------------
    tool_calls = 40  # scenario assumption: tool calls in one trajectory
    think_time = 2.0 * second  # scenario assumption: model inference per turn
    tool_time = 1.5 * second  # scenario assumption: tool execution per turn
    budget_share = 0.05  # scenario assumption: wall clock isolation may consume
    qemu_boot = 30 * second  # scenario assumption: mid-range full-emulation boot
    ...
    boot_min = (150 * millisecond, 120 * millisecond, 1 * millisecond, 5 * millisecond, 15 * second)
    boot_max = (400 * millisecond, 250 * millisecond, 5 * millisecond, 45 * millisecond, 45 * second)
    mem_min = (15 * MB, 25 * MB, 1 * MB, 5 * MB, 512 * MB)
    mem_max = (35 * MB, 45 * MB, 4 * MB, 5 * MB, 2_000 * MB)
    io_penalty_min = (0.0, 0.15, 0.0, 0.0, 0.0)
    io_penalty_max = (0.0, 0.35, 0.0, 0.0, 0.0)

    # +-- 2. EXECUTE (The Compute) ---------------
    productive_per_call = think_time + tool_time
    trajectory_productive = tool_calls * productive_per_call
    boot_budget = (budget_share * productive_per_call / (1 - budget_share)).to(second)
    crossover = productive_per_call

    per_call, charges, shares, overruns = [], [], [], []
    for i in range(len(names)):
        boot = qemu_boot if i == 4 else boot_max[i]
        call_charge = boot + tool_time * io_penalty_max[i]
        wall = trajectory_productive + tool_calls * call_charge
        per_call.append(call_charge)
        charges.append((tool_calls * call_charge).to(second))
        shares.append(((tool_calls * call_charge) / wall).to("").magnitude)
        overruns.append((call_charge / boot_budget).to("").magnitude)
    gvisor_boot_charge = (tool_calls * boot_max[1]).to(second)
    gvisor_io_charge = (tool_calls * tool_time * io_penalty_max[1]).to(second)
    qemu_vs_productive = (charges[4] / trajectory_productive).to("").magnitude
    fc_margin = (boot_budget / boot_max[3]).to("").magnitude

    # +-- 3. GUARD (Invariants) ------------------
    check(abs(boot_budget.to(millisecond).magnitude - 184) < 1, f"The per-call budget should land near 184 ms, got {boot_budget.to(millisecond)}")
    check(overruns[3] < 1 < overruns[0] < overruns[1] < overruns[4], "Only the microVM should fit the budget, and QEMU should overrun it worst")
    check(gvisor_io_charge > gvisor_boot_charge, "gVisor's steady-state I/O tax should exceed its cold-start cost, which is the notebook's point")
    check(shares[4] > 0.85, f"Full emulation should consume most of the wall clock, got {shares[4]}")

    # +-- 4. OUTPUT (Formatting) -----------------
    boot_cells, mem_cells = [], []
    for i in range(len(names)):
        boot_cells.append(fmt_time_range(boot_min[i], boot_max[i], second if i == 4 else millisecond, precision=0))
        if mem_min[i] == mem_max[i]:
            mem_cells.append(fmt_qty(mem_max[i], MB, precision=0))
        else:
            mem_cells.append(fmt_qty_range(mem_min[i], mem_max[i], MB, precision=0))
    docker_boot_str, gvisor_boot_str, wasi_boot_str, fc_boot_str, qemu_boot_range_str = boot_cells
    docker_mem_str, gvisor_mem_str, wasi_mem_str, fc_mem_str, qemu_mem_str = mem_cells

    # The notebook charges the four POSIX-capable options; WASI is out of scope
    # there because it does not run the agent's bash toolchain.
    charge_cells, share_cells, overrun_cells = [], [], []
    for i in (0, 1, 3, 4):
        charge_cells.append(fmt_time(charges[i], second, precision=_p(charges[i].to(second).magnitude)))
        share_cells.append(fmt_percent(shares[i], precision=_p(shares[i] * 100), style="prose"))
        overrun_cells.append(fmt_multiple(overruns[i], precision=_p(overruns[i])))
    # gVisor's total is split into boot and I/O in the notebook, so only the
    # other three totals are exported.
    docker_charge_str, _gvisor_charge, fc_charge_str, qemu_charge_str = charge_cells
    docker_share_str, gvisor_share_str, fc_share_str, qemu_share_str = share_cells
    # Firecracker is reported as margin inside the budget rather than overrun.
    docker_overrun_mult_str, gvisor_overrun_mult_str, _fc_overrun, qemu_overrun_mult_str = overrun_cells

    tool_calls_str = fmt_int(tool_calls)
    think_time_str = fmt_time(think_time, second, precision=_p(think_time.to(second).magnitude))
    tool_time_str = fmt_time(tool_time, second, precision=_p(tool_time.to(second).magnitude))
    budget_share_str = fmt_percent(budget_share, precision=0, style="prose")
    productive_per_call_str = fmt_time(productive_per_call, second, precision=_p(productive_per_call.to(second).magnitude))
    trajectory_productive_str = fmt_time(trajectory_productive, second, precision=_p(trajectory_productive.to(second).magnitude))
    boot_budget_str = fmt_time(int(round(boot_budget.to(millisecond).magnitude)) * millisecond, millisecond, precision=0)
    crossover_str = fmt_time(crossover, second, precision=_p(crossover.to(second).magnitude))
    qemu_boot_str = fmt_time(qemu_boot, second, precision=0)
    fc_boot_max_str = fmt_time(boot_max[3], millisecond, precision=0)
    fc_mem_base_str = fmt_qty(mem_max[3], MB, precision=0)
    gvisor_boot_max_str = fmt_time(boot_max[1], millisecond, precision=0)
    gvisor_io_range_str = fmt_percent_range(io_penalty_min[1], io_penalty_max[1], precision=0, style="prose")
    gvisor_io_max_str = fmt_percent(io_penalty_max[1], precision=0, style="prose")
    gvisor_boot_charge_str = fmt_time(gvisor_boot_charge, second, precision=0)
    gvisor_io_charge_str = fmt_time(gvisor_io_charge, second, precision=0)
    qemu_vs_productive_mult_str = fmt_multiple(qemu_vs_productive, precision=_p(qemu_vs_productive))
    fc_margin_mult_str = fmt_multiple(fc_margin, precision=_p(fc_margin))
```

While containers boot relatively quickly (`{python} SandboxBudget.docker_boot_str`), they represent an **unacceptable risk for untrusted agent code execution** because they violate the principle of **Least Common Mechanism** [@saltzer1975]. By sharing the monolithic host Linux kernel across all tenants, containers introduce a massive shared attack surface. Over 450 system calls are exposed to the containerized process. Any local privilege escalation vulnerability in the host Linux kernel (such as Dirty COW [CVE-2016-5195] or Dirty Pipe [CVE-2022-0847]) grants root access to the physical host. Furthermore, misconfigured `/proc` and `/sys` mounts frequently leak host metadata, while shared loop devices can corrupt host block storage.
...
* **Trade-offs:** Emulating complex Linux subsystems introduces significant I/O and context-switch overhead. File system operations, heavy git branch switches, and compilation workloads suffer a `{python} SandboxBudget.gvisor_io_range_str` performance degradation, and startup latency runs to `{python} SandboxBudget.gvisor_boot_max_str`.
...
MicroVMs combine the isolation strength of hardware-assisted virtualization with the agility of lightweight containers. By pruning all legacy PC hardware emulation, AWS Firecracker [@agache2020firecracker] achieves boot times of `{python} SandboxBudget.fc_boot_str` and a `{python} SandboxBudget.fc_mem_base_str` baseline memory footprint while running an independent, fully functional guest Linux kernel atop the Linux Kernel-based Virtual Machine (KVM) subsystem.

| Isolation Primitive           | Boundary Mechanism       | Cold Boot ($T_{\text{boot}}$)                | Memory Base                             | Host Kernel Surface       | Untrusted Bash Safe?        |
|:------------------------------|:-------------------------|:---------------------------------------------|:----------------------------------------|:--------------------------|:----------------------------|
| **Linux Containers (Docker)** | Namespaces & cgroups     | `{python} SandboxBudget.docker_boot_str`     | `{python} SandboxBudget.docker_mem_str` | Shared (450+ syscalls)    | [Unsafe] Escape prone       |
| **gVisor (Sentry)**           | Userspace Syscall Filter | `{python} SandboxBudget.gvisor_boot_str`     | `{python} SandboxBudget.gvisor_mem_str` | Intercepted in Go         | [Moderate] I/O penalty      |
| **WebAssembly (WASI)**        | Linear Memory Bounds     | `{python} SandboxBudget.wasi_boot_str`       | `{python} SandboxBudget.wasi_mem_str`   | Zero (Capability imports) | [Safe] Bytecode only        |
| **MicroVM (Firecracker)**     | Hardware KVM (VT-x)      | `{python} SandboxBudget.fc_boot_str`         | `{python} SandboxBudget.fc_mem_str`     | Hardware VMCS barrier     | [Safe] Golden standard      |
| **Legacy VM (QEMU)**          | Full PC Emulation        | `{python} SandboxBudget.qemu_boot_range_str` | `{python} SandboxBudget.qemu_mem_str`   | Full Hardware Hypervisor  | [Safe] Impractical at scale |
...
As summarized in @tbl-isolation-comparison, **MicroVMs represent the golden architectural standard for autonomous coding agents**. They provide full POSIX compatibility (running bash, pytest, git, and python unmodified) while guaranteeing hardware-enforced hypervisor protection. The sub-$100\text{ ms}$ startup bar stated at the opening of this section, and the "impractical at scale" verdict in the table's last row, both follow from a single accounting question: how often is a sandbox torn down and rebuilt inside one agent trajectory, and what does each rebuild cost against the work it contains. Isolation became affordable per tool call rather than per session only recently. @Fig-boot-latency-trend places each primitive against the sub-hundred-millisecond threshold, and the microVM generation is the first to sit below it while still running an unmodified POSIX toolchain.

::: {#fig-boot-latency-trend fig-env="figure" fig-pos="htb" fig-cap="**The Evolution of Virtualization Boot Latency**: Cold-boot time by year of introduction, falling roughly five orders of magnitude from full virtual machines in 2001 to WebAssembly runtimes in 2022. Only the newest primitives clear the sub-hundred-millisecond band where per-call isolation costs less than the tool call it protects." fig-alt="Log-scale scatter of cold-boot latency against year of introduction for isolation primitives from full VMs in 2001 to WebAssembly runtimes in 2022, falling about five orders of magnitude. A shaded band marks the sub-hundred-millisecond agentic isolation target that only the newest primitives reach."}
...
data = pd.read_csv("vol3/sandboxing_isolation/data/virtualization_boot_latency.csv")

fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 5))

ax.plot(data["Year"], data["Boot_Latency_ms"], color="gray", alpha=0.7, linestyle="--")

_groups = list(dict.fromkeys(data["Type"]))
_cmap = plt.get_cmap('viridis', max(len(_groups), 2))
for _i, _g in enumerate(_groups):
    _sub = data[data["Type"] == _g]
    ax.scatter(_sub["Year"], _sub["Boot_Latency_ms"], color=_cmap(_i), label=str(_g), s=120)

ax.set_yscale("log")
ax.set_ylabel("Cold-Boot Latency (ms)", fontsize=11, fontweight="bold")
ax.set_xlabel("Year of Introduction", fontsize=11, fontweight="bold")
ax.set_title("Evolution of sandbox startup latency (2001–2022)", fontsize=12, fontweight="bold", pad=15)

for i in range(len(data)):
    x = data["Year"].iloc[i]
    y = data["Boot_Latency_ms"].iloc[i]
    text = data["Technology"].iloc[i]
    ...
    ax.annotate(text, (x, y), xytext=(x, y_offset), ha="center", fontsize=9, color="#34495e")

ax.yaxis.set_major_locator(LogLocator(base=10.0, numticks=10))
ax.yaxis.set_minor_formatter(NullFormatter())

ax.axhspan(0.1, 100, color="green", alpha=0.1, zorder=0)
ax.text(2001.5, 20, "Agentic Isolation Target (< 100 ms)", color="green", fontweight="bold", fontsize=9)

ax.legend(title="Isolation Primitive", title_fontsize="9", fontsize="8", loc="upper right")
...
**Variables**: A trajectory of `{python} SandboxBudget.tool_calls_str` tool calls, each turn spending `{python} SandboxBudget.think_time_str` on model inference and `{python} SandboxBudget.tool_time_str` on tool execution, and an engineering budget that allows isolation to consume at most `{python} SandboxBudget.budget_share_str` of wall clock. Those four are scenario assumptions. The boot latencies are the slow end of each range in @tbl-isolation-comparison, with gVisor also charged its `{python} SandboxBudget.gvisor_io_max_str` I/O degradation, and full emulation charged `{python} SandboxBudget.qemu_boot_str`.

**Math**: Productive work per call is `{python} SandboxBudget.productive_per_call_str`, so one trajectory contains `{python} SandboxBudget.trajectory_productive_str` of it. Writing $T$ for the isolation charge per call, isolation's share of wall clock is $T/(t_{\text{prod}} + T)$. Setting that equal to the budget share and solving:

$$T = \frac{s \cdot t_{\text{prod}}}{1 - s}$$

That gives `{python} SandboxBudget.boot_budget_str`. The call count cancels, so the budget does not move with trajectory length. Charging each row of the table against the same trajectory:

- Firecracker: `{python} SandboxBudget.fc_charge_str` of boot, so `{python} SandboxBudget.fc_share_str` of wall clock.
- gVisor: `{python} SandboxBudget.gvisor_boot_charge_str` of boot plus `{python} SandboxBudget.gvisor_io_charge_str` of I/O tax, so `{python} SandboxBudget.gvisor_share_str`.
- Docker: `{python} SandboxBudget.docker_charge_str` of boot, so `{python} SandboxBudget.docker_share_str`.
- QEMU: `{python} SandboxBudget.qemu_charge_str` of boot, so `{python} SandboxBudget.qemu_share_str`, which is `{python} SandboxBudget.qemu_vs_productive_mult_str` the productive work it wraps.

**Result**: The budget is `{python} SandboxBudget.boot_budget_str` per sandbox. Firecracker at `{python} SandboxBudget.fc_boot_max_str` sits `{python} SandboxBudget.fc_margin_mult_str` inside it, Docker overruns it by `{python} SandboxBudget.docker_overrun_mult_str`, gVisor by `{python} SandboxBudget.gvisor_overrun_mult_str` once its I/O tax is charged, and QEMU by `{python} SandboxBudget.qemu_overrun_mult_str`. Isolation reaches half of wall clock at `{python} SandboxBudget.crossover_str`, the crossover where building a sandbox costs as much as using it, and QEMU sits an order of magnitude past that line. This is what "impractical at scale" means arithmetically.

**Systems insight**: The threshold is fixed by the productive time of one tool call, not by how long the trajectory runs, so an ephemeral-per-call design has exactly one tunable term and it is the per-call cost of the isolation boundary. That term is not the boot column alone, as gVisor shows: its steady-state I/O degradation costs roughly twice what its cold start does, so a technology has to be scored on the full per-call charge, and a runtime that cannot meet the budget is pushed toward reusing sandboxes across calls, which trades the boot cost back for cross-turn contamination risk.
...
::: {#fig-firecracker-architecture fig-env="figure" fig-pos="htb" fig-cap="**AWS Firecracker Architecture**: Jailer rootless containment, KVM hardware virtualization barrier, and ephemeral copy-on-write OverlayFS storage. Every layer exists to remove something rather than add it, and the absence of legacy device emulation, a persistent root filesystem, and host privilege is what buys the boot time the design depends on." fig-alt="Architectural diagram of AWS Firecracker showing the host agent orchestrator, Jailer security boundary with chroot and seccomp-bpf, Firecracker Rust VMM process with minimal virtio devices, KVM hardware hypervisor boundary, guest Linux kernel, and ephemeral copy-on-write OverlayFS storage layers."}
...
1. **LowerDir (Immutable Base Layer):** Contains a pristine, read-only snapshot of the operating system root filesystem (e.g., Ubuntu 24.04), pre-installed developer toolchains (Python, Node.js, Rust, Git, LLVM), and a read-only mirror of the project repository. Because LowerDir is read-only, the host Linux kernel caches its pages once and every sandbox on that host reads the same physical copy. **A 10 GB base image therefore costs 10 GB of host RAM whether one sandbox is running or several hundred are.** Density is then set by the writable layer rather than the image: at a few hundred megabytes of working set per sandbox, a host with a terabyte of RAM carries sandboxes in the hundreds, and the base image has stopped being the constraint.
...
Each of these layers decides whether a destination is permitted. Permitted and safe are separable properties once an allowlisted host will itself fetch an arbitrary URL on request, and a critical advisory Microsoft published in 2025 records an exfiltration chain that ends in that gap (\ref{ws-sandboxing-echoleak-2025}).

::: {#ws-sandboxing-echoleak-2025 .callout-war-story title="EchoLeak in Microsoft 365 Copilot (2025)"}

**Context**: Microsoft 365 Copilot answers employee questions by retrieving from the Microsoft Graph across the mailbox, OneDrive storage, internal SharePoint sites, and Teams chat history, so an email delivered to an employee becomes candidate retrieval context whether or not anyone opens it. Aim Labs disclosed a chain against that design to Microsoft, which published it on June 11, 2025 as CVE-2025-32711 [@aimlabs2025echoleak; @msrc2025echoleak].

**Mechanism**: Four deployed controls were bypassed in sequence. The attacker's email was phrased as instructions to a human recipient and never named an assistant, so Microsoft's XPIA classifier for cross-prompt injection attacks did not flag it. Copilot's output filter stripped inline markdown links but did not recognize reference-style markdown, so a crafted target survived into the rendered answer. Reference-style image syntax then made the client fetch that target automatically, with no click. The final control was the `img-src` content security policy, an allowlist of the domains the page may load images from, and the attack did not defeat it. The domain `*.teams.microsoft.com` was on the list, and a Teams preview endpoint accepts a URL as a query parameter and retrieves it server side, so a Microsoft service fetched the attacker's address with the stolen text carried in it.

**Impact**: Everything in the Copilot context was reachable, including the chat history, resources already retrieved from the Microsoft Graph, and data preloaded into the conversation such as user and organization names. Microsoft classified the flaw as information disclosure over the network requiring no privileges and no user interaction, scored it 9.3, and rated it critical. Aim Labs reported no customers known to have been affected, and the advisory records no observed exploitation.

**Fix**: Microsoft mitigated the vulnerability inside its own service and stated that no action was required from users of it. The tenant-side controls that remained available, which exclude external mail and sensitivity-labeled documents from Copilot processing, narrow what the assistant is allowed to read rather than where it is allowed to reach.

**Systems lesson**: A domain allowlist authorizes destinations, not flows. Any allowlisted host that will fetch a URL on request is a relay straight through the boundary, which makes the allowlist an attack surface in its own right, to be audited for redirectors, preview services, and link unfurlers rather than extended for convenience. The order of failure matters as much as the endpoint. The classifier, the output filter, and the renderer were each defeated by content, which left the network layer as the only control with a structural guarantee still to give, and it had been configured to ask where a request was going instead of what was leaving.
...
T + 03:15  Cloud billing alarm triggers ($8,000 accrued); security team revokes IAM role.
```
...
## Summary {#sec-vol3-sandboxing-summary}

Because autoregressive foundation models lack hardware separation between executable instructions and untrusted data ($W \oplus X$), prompt injection is an architectural inevitability. Securing autonomous systems therefore requires enforcing strict isolation around the execution boundary—leveraging hardware-virtualized microVMs, Copy-on-Write filesystems, and Information Flow Control to contain untrusted reasoning.

::: {.callout-takeaways title="MicroVM sandboxing and epistemic taint isolation"}
...
Sandboxing accepts that the model cannot be secured from within. Because language models process instructions and data within an identical self-attention tensor, the host environment must enforce isolation entirely through external operating system primitives—ensuring that a compromised agent burns only ephemeral resources within a throwaway boundary.

::: {.callout-chapter-connection title="From process isolation to verification and recovery"}
Containing side effects within ephemeral sandboxes protects host infrastructure from harm, but isolation alone does not ensure that an agent accomplishes its task correctly or survives operational mistakes. @Sec-vol3-verification-recovery examines the mechanics of runtime verification: deterministic test oracles, the reversibility boundary, and distributed compensating Sagas that enable autonomous systems to detect semantic drift and recover from execution failure.
:::
