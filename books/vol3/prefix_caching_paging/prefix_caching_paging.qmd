...
## Prefix Locality in Trajectories {#sec-vol3-caching-prefix-locality-in-multi-turn}

Stateless large language model serving benchmarks traditionally assume independent, identically distributed requests where each prompt arrives with little or no shared history. In contrast, real-world autonomous agents—such as software engineering agents executing on SWE-bench, autonomous research assistants, and tool-augmented workflow orchestrators—generate highly stateful, clustered request traces characterized by extreme prefix locality. Agent sessions sit at the far right of this trend rather than the middle of it. @Fig-vol3-caching-context-scaling shows supported context growing three orders of magnitude in four years, and a cache partitioned for the left edge of that curve will thrash on the right.

::: {#fig-vol3-caching-context-scaling fig-env="figure" fig-pos="htb" fig-cap="**The Context Window Explosion**: Maximum supported context for frontier models from 2020 to 2024, colored by organization, rising from two thousand to two million tokens. KV-cache footprint scales with this axis, which is what makes a statically partitioned cache untenable for multi-turn agents." fig-alt="Log-scale scatter of frontier model context length against release date from 2020 to 2024, colored by organization, rising from two thousand tokens to two million."}
...
data = pd.read_csv("vol3/prefix_caching_paging/data/context_length_scaling.csv")
data['release_date'] = pd.to_datetime(data['release_date'])

fig, ax, COLORS, plt = viz.setup_plot(figsize=(9, 5))

# Create a scatter plot
_groups = list(dict.fromkeys(data['org']))
_cmap = plt.get_cmap('viridis', max(len(_groups), 2))
for _i, _g in enumerate(_groups):
    _sub = data[data['org'] == _g]
    ax.scatter(_sub['release_date'], _sub['context_length'], color=_cmap(_i), label=str(_g), linewidth=0.5, s=100, alpha=0.8, edgecolor='black')

# Customize the axes
plt.yscale('log', base=2)
plt.ylabel("Context Length (Tokens)", fontsize=10)
plt.xlabel("Release Date", fontsize=10)
plt.title("Frontier model context length scaling (2020-2024)", fontsize=12, weight='bold')

# Formatting y-axis ticks to show actual numbers instead of scientific notation
def y_fmt(x, pos):
    if x >= 1000000:
...
key_models = ["GPT-3", "GPT-4", "Claude 1", "Gemini 1.5 Pro", "Llama 3.1", "Gemini 1.5 Pro 2M", "GPT-4 Turbo", "PaLM", "Claude 3 Opus"]
for i, row in data.iterrows():
    if row['model'] in key_models:
        plt.annotate(row['model'], 
                     (mdates.date2num(row['release_date']), row['context_length']),
                     textcoords="offset points", 
                     xytext=(6, -2) if row['model'] == 'PaLM' else (6, 3), 
                     ha='left',
                     fontsize=8)

plt.legend(title="Organization", bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=8)
...
## KV-Cache Anatomy and Layouts {#sec-vol3-caching-kv-cache-anatomy-tensor-layouts}
...
3. **Grouped-Query Attention (GQA):** Query heads are partitioned into $G$ equal groups, where all Query heads within a group share a single Key head and a single Value head ($n_{\text{kv\_heads}} = n_{\text{q\_heads}} / G$) [@ainslie2023]. GQA achieves the high task quality of MHA while delivering up to an $8\times$ reduction in KV cache memory consumption.

```{python}
#| label: vol3-caching-attention-footprint
#| echo: false
# +-----------------------------------------------------------------------------
# | KV FOOTPRINT BY ATTENTION SCHEME
# +-----------------------------------------------------------------------------
# | Context: @tbl-vol3-caching-model-kv-footprint and the paragraph that reads it.
# |
# | Goal:    Price one cached token under multi-head, grouped-query, and latent
# |          attention, using real models rather than a stylized shape.
# | Show:    Layer depth, head ratio, head dimension, per-token bytes, and the
# |          resident cache at two agent-scale contexts.
# | How:     Every shape comes from the MLSysIM model registry; sizing goes
# |          through get_kv_cache_size, which takes the latent path for a model
# |          that declares an MLA rank and the standard path otherwise.
# +-----------------------------------------------------------------------------
from mlsysim import *

class AttentionFootprint:
    # +-- 1. LOAD (Constants) --------------------
    models = (
        Models.Language.GPT3,
        Models.Language.Llama3_8B,
        Models.Language.Llama3_70B,
        Models.Language.DeepSeek_V3,
    )
    short_ctx = 32_768  # scenario: a single agent turn carrying recent history
    long_ctx = 65_536  # scenario: a long multi-turn trajectory

    # +-- 2. EXECUTE (The Compute) ---------------
    per_token, short_kv, long_kv = [], [], []
    schemes, head_specs, dims = [], [], []
    for m in models:
        per_token.append(m.get_kv_cache_size(seq_len=1, batch_size=1, precision=BYTES_FP16))
        short_kv.append(m.get_kv_cache_size(seq_len=short_ctx, batch_size=1, precision=BYTES_FP16))
        long_kv.append(m.get_kv_cache_size(seq_len=long_ctx, batch_size=1, precision=BYTES_FP16))
        if m.kv_lora_rank is not None:
            schemes.append("MLA")
            head_specs.append("%d / latent" % m.heads)
            dims.append("%d (latent)" % (m.kv_lora_rank + m.qk_rope_head_dim))
        else:
            kv = m.kv_heads or m.heads
            schemes.append("MHA" if kv == m.heads else "GQA")
            head_specs.append("%d / %d" % (m.heads, kv))
            dims.append("%d" % (m.hidden_dim // m.heads))
    mha_over_gqa = (per_token[0] / per_token[2]).to("").magnitude
    gqa_over_mla = (per_token[2] / per_token[3]).to("").magnitude

    # +-- 3. GUARD (Invariants) ------------------
    check(schemes == ["MHA", "GQA", "GQA", "MLA"], f"The roster must span all three attention schemes, got {schemes}")
    check(per_token[0] > per_token[1] and per_token[2] > per_token[3], "Per-token cost must fall as the scheme sheds cached heads")
    check(10 < mha_over_gqa < 20, f"Multi-head should cost roughly an order of magnitude more per token, got {mha_over_gqa}")
    check(
        models[0].parameters > models[2].parameters and per_token[0] > per_token[2],
        "The multi-head model is both larger and more expensive per token, which is the point to check",
    )

    # +-- 4. OUTPUT (Formatting) -----------------
    names, params_cells, scheme_cells, layers_cells, heads_cells, dim_cells = [], [], [], [], [], []
    token_cells, short_cells, long_cells = [], [], []
    for i in range(len(models)):
        names.append(fmt_text(models[i].name))
        params_cells.append(fmt_params(models[i].parameters))
        scheme_cells.append(fmt_text(schemes[i]))
        layers_cells.append(fmt_int(models[i].layers))
        heads_cells.append(fmt_text(head_specs[i]))
        dim_cells.append(fmt_text(dims[i]))
        token_cells.append(fmt_memory(per_token[i]))
        short_cells.append(fmt_memory(short_kv[i], unit=GB, precision=1))
        long_cells.append(fmt_memory(long_kv[i], unit=GB, precision=1))
    gpt3_name_str, l8b_name_str, l70b_name_str, dsv3_name_str = names
    gpt3_params_str, l8b_params_str, l70b_params_str, dsv3_params_str = params_cells
    gpt3_scheme_str, l8b_scheme_str, l70b_scheme_str, dsv3_scheme_str = scheme_cells
    gpt3_layers_str, l8b_layers_str, l70b_layers_str, dsv3_layers_str = layers_cells
    gpt3_heads_str, l8b_heads_str, l70b_heads_str, dsv3_heads_str = heads_cells
    gpt3_dim_str, l8b_dim_str, l70b_dim_str, dsv3_dim_str = dim_cells
    gpt3_token_str, l8b_token_str, l70b_token_str, dsv3_token_str = token_cells
    gpt3_short_str, l8b_short_str, l70b_short_str, dsv3_short_str = short_cells
    gpt3_long_str, l8b_long_str, l70b_long_str, dsv3_long_str = long_cells
    mha_over_gqa_mult_str = fmt_multiple(mha_over_gqa, precision=1)
    gqa_over_mla_mult_str = fmt_multiple(gqa_over_mla, precision=1)
```

| Model Architecture                          |                   Parameters                  |                 Attention Type                |              $n_{\text{layers}}$              | $n_{\text{q\_heads}} / n_{\text{kv\_heads}}$ |             $d_{\text{head}}$              |              Bytes/Token (FP16)              |                 32k KV Cache                 |                 64k KV Cache                |
|:--------------------------------------------|:---------------------------------------------:|:---------------------------------------------:|:---------------------------------------------:|:--------------------------------------------:|:------------------------------------------:|:--------------------------------------------:|:--------------------------------------------:|:-------------------------------------------:|
| `{python} AttentionFootprint.gpt3_name_str` | `{python} AttentionFootprint.gpt3_params_str` | `{python} AttentionFootprint.gpt3_scheme_str` | `{python} AttentionFootprint.gpt3_layers_str` | `{python} AttentionFootprint.gpt3_heads_str` | `{python} AttentionFootprint.gpt3_dim_str` | `{python} AttentionFootprint.gpt3_token_str` | `{python} AttentionFootprint.gpt3_short_str` | `{python} AttentionFootprint.gpt3_long_str` |
| `{python} AttentionFootprint.l8b_name_str`  |  `{python} AttentionFootprint.l8b_params_str` |  `{python} AttentionFootprint.l8b_scheme_str` |  `{python} AttentionFootprint.l8b_layers_str` | `{python} AttentionFootprint.l8b_heads_str`  | `{python} AttentionFootprint.l8b_dim_str`  | `{python} AttentionFootprint.l8b_token_str`  | `{python} AttentionFootprint.l8b_short_str`  |  `{python} AttentionFootprint.l8b_long_str` |
| `{python} AttentionFootprint.l70b_name_str` | `{python} AttentionFootprint.l70b_params_str` | `{python} AttentionFootprint.l70b_scheme_str` | `{python} AttentionFootprint.l70b_layers_str` | `{python} AttentionFootprint.l70b_heads_str` | `{python} AttentionFootprint.l70b_dim_str` | `{python} AttentionFootprint.l70b_token_str` | `{python} AttentionFootprint.l70b_short_str` | `{python} AttentionFootprint.l70b_long_str` |
| `{python} AttentionFootprint.dsv3_name_str` | `{python} AttentionFootprint.dsv3_params_str` | `{python} AttentionFootprint.dsv3_scheme_str` | `{python} AttentionFootprint.dsv3_layers_str` | `{python} AttentionFootprint.dsv3_heads_str` | `{python} AttentionFootprint.dsv3_dim_str` | `{python} AttentionFootprint.dsv3_token_str` | `{python} AttentionFootprint.dsv3_short_str` | `{python} AttentionFootprint.dsv3_long_str` |

: **KV-Cache Footprint by Attention Architecture**: Per-token KV bytes and resident cache at two agent-scale contexts, at 16-bit precision. Per-token cost is set by layer depth, key-value head count, and head dimension, not by parameter count, which is why the largest model in the table is also the most expensive per token by a wide margin. {#tbl-vol3-caching-model-kv-footprint tbl-colwidths="[16,9,10,9,12,10,12,11,11]"}

As demonstrated in @tbl-vol3-caching-model-kv-footprint, the cost of a cached token tracks the attention scheme rather than the parameter count: multi-head attention charges `{python} AttentionFootprint.mha_over_gqa_mult_str` what grouped-query attention charges on a model less than half its size, and latent attention takes another `{python} AttentionFootprint.gqa_over_mla_mult_str` off that. Even so, a grouped-query 70B model holding a multi-turn agent context still commits `{python} AttentionFootprint.l70b_short_str` to `{python} AttentionFootprint.l70b_long_str` of accelerator memory per active trajectory. When dozens of autonomous agents execute concurrently across a GPU cluster, memory capacity—rather than compute throughput—becomes the primary bottleneck.
...
- **Internal Fragmentation:** Memory reserved for tokens up to $L_{\max}$ remains completely unused if the agent generates only a brief response. In typical serving deployments, internal fragmentation wastes between 60 and 80 percent of accelerator memory [@kwon2023].
...
## PagedAttention and Page Tables {#sec-vol3-caching-pagedattention-virtual-memory-page}

As briefly introduced in @sec-vol3-context-working-sets, to overcome the fatal fragmentation of static contiguous allocation, @kwon2023 introduced PagedAttention, an architecture inspired by classic operating system virtual memory paging [@denning1970].
...
## Radix Tree Prefix Indexing {#sec-vol3-caching-radix-tree-indexing-dynamic}
...
Because accelerator HBM has strictly finite capacity, the Radix tree cannot retain all historical prefixes indefinitely. Exploiting the system principle of **temporal locality**—the empirical observation that recently accessed trajectories are the most likely to receive new generation steps in the near future—the runtime implements reference-counted Least-Recently-Used (LRU) eviction:
...
This design ensures that if the agent issues its next turn before memory pressure forces eviction, the system achieves a 100 percent cache hit on the existing prefix, replaying a tree path whose logical nodes still resolve to the scattered physical blocks in @fig-vol3-caching-radix-tree.

::: {#fig-vol3-caching-radix-tree fig-env="figure" fig-pos="htb" fig-cap="**Radix Tree Prefix Cache**: A logical Radix tree indexes agent token sequences, and each matched node maps onto non-contiguous physical PagedAttention blocks in accelerator HBM. Reference counting is what keeps the two halves consistent: internal nodes stay pinned while active children exist, so only zero-count leaves reach the LRU eviction queue and a returning turn replays its prefix with no prefill compute." fig-alt="A two-panel diagram. The left panel shows a Radix tree rooted at a system prompt node that branches into two first-turn user goal nodes and then into tool-call and search leaves, each labeled with a token range and a reference count; one branch is marked as a full cache hit and a zero-count leaf is queued for eviction. The right panel shows a logical block table mapping logical block numbers to scattered physical HBM block numbers, alongside a grid of physical blocks with the unmapped ones shown as a free pool."}

![](images/svg/radix_prefix_tree_architecture.svg)

:::

Reference counting decides when a cached block may be released, but nothing in the tree records which request produced it, because the match that hands a block to a reader is performed on token identity alone. That is the structural risk every shared cache in a serving path carries, and the clearest published account of it concerns a different cache in the same kind of stack (\ref{ws-caching-openai-redis-2023}).

::: {#ws-caching-openai-redis-2023 .callout-war-story title="The ChatGPT cache disclosure (2023)"}

**Context**: ChatGPT placed a Redis cache in front of its database so that user information did not have to be re-read on every request, used Redis Cluster to spread that load across several Redis instances, and reached the cluster from Python servers running under Asyncio through the redis-py client [@openai2023chatgptoutage]. The cache held account records rather than attention state, but it occupied the position a prefix cache occupies in this chapter, a shared high-hit-rate layer that answers one request with state materialized for another.

**Mechanism**: The redis-py client maintains a shared pool of connections and recycles each connection once a request finishes with it. Under Asyncio, a request and its response behave as two queues, so the caller pushes a request onto the incoming queue, pops a response from the outgoing queue, and returns the connection to the pool. A request canceled after being pushed but before its response was popped left the connection corrupted, holding a response nobody had read, and the next unrelated request to borrow that connection dequeued the data left behind in it. In most cases the leftover value did not match the type the caller expected and the server returned an unrecoverable error, but when the types happened to agree, what came back from the cache looked valid even though it belonged to another user. At 1 a.m. Pacific time on March 20, 2023, a server change caused a spike in Redis request cancellations, which raised the probability of bad data on every connection in the pool at once.

**Impact**: Some users saw titles from another active user's chat history, and the first message of a newly created conversation could appear in a different user's history when both users were active at about the same time. OpenAI took ChatGPT offline and later restored the service and its chat history feature, with the exception of a few hours of history. The same bug may have made payment-related information visible for 1.2 percent of the ChatGPT Plus subscribers who were active during a nine-hour window, covering first and last name, email address, payment address, credit card type, the last four digits of a credit card number, and the card expiration date. Full credit card numbers were not exposed at any time. Some subscription confirmation emails generated between 1 a.m. and 10 a.m. Pacific time on March 20 were also sent to the wrong users.

**Fix**: OpenAI sent the Redis maintainers a patch for the underlying bug, which appeared only in the Asyncio redis-py client for Redis Cluster. Alongside the upstream fix, OpenAI added redundant checks that the data returned by the Redis cache matches the requesting user, examined its logs programmatically to confirm that messages were reaching only their correct owner, improved logging so that a recurrence would be visible, and improved the robustness and scale of the Redis cluster to reduce connection errors at extreme load.

**Systems lesson**: The lookup logic was sound and the cached values were the values that had been stored. What failed was the unstated assumption that the connection delivering an answer belonged to the request that asked for it. A radix prefix cache concentrates the same assumption in a different place, since a successful match hands a request physical blocks that some earlier request populated, and the match is decided on token identity with no reference to who is asking. The durable practice is the one OpenAI adopted after the fact, which is to bind a cached entry to the identity it was produced for and to verify that binding where the entry is consumed, rather than trusting the path that delivered it. A shared cache sits inside the correctness boundary of the serving system, not outside it as a pure performance optimization.

:::

## Cache Invalidation Protocols {#sec-vol3-caching-cache-invalidation-protocols-mutating}
...
3. **Environment Mutation Interception:** The agent runtime intercepts all state-modifying tool operations (e.g., file writes, deletions). When a file modification occurs, the runtime immediately invalidates the corresponding lease in the serving engine.
...
- **Append-Only Observation Logs:** Never mutate historical conversation turns in-place. If an environment state changes, record the change as an explicit new delta observation at the end of the prompt:
  ...
  Appending deltas preserves the entire historical prefix in the Radix tree, allowing a 100 percent cache hit on the prompt history while adding only the minimal delta tokens to the prefill phase.

## Multi-Tier KV Memory Hierarchies {#sec-vol3-caching-multi-tier-memory-hierarchies-kv-swapping}
...
### Working set constraints and the pinned memory crisis

In classical operating systems, a process's *working set* is the minimum amount of physical memory required to make forward progress without thrashing. For an active agent trajectory, the working set is strictly the entire uncompressed KV cache of its context history.

If an agent's $64\text{k}$-token KV cache ($21\text{ GB}$ on Llama-3-70B) remains pinned in accelerator HBM while the agent is blocked waiting for an external tool, that GPU memory is completely unavailable to concurrent requests. On an 8-GPU H100 node ($640\text{ GB}$ total HBM), the model's own FP16 weights claim $140\text{ GB}$ before a single request arrives, so roughly 20 idle agent sessions exhaust what remains, causing out-of-memory (OOM) crashes and stalling active generation.
...
2. **Tier 2: Host System DRAM (CPU Memory):** Moderate bandwidth ($300\text{ to } 500\text{ GB/s}$ across multi-channel DDR5 or CXL 3.0 memory expanders), low latency ($10\text{ to } 50\,\mu\text{s}$), large capacity ($512\text{ GB to } 2.0\text{ TB}$ per node). Connected to accelerators via PCIe Gen5 x16 duplex buses that deliver $64\text{ GB/s}$ theoretically and $\approx 54\text{ GB/s}$ of effective direct memory access (DMA) bandwidth. Serves as the primary paging store for suspended agent sessions during tool execution.
...
Capacity grows by roughly an order of magnitude at each step down the hierarchy, and the cost of carrying a context across each boundary rises with it, a coupling visible in the bandwidth and swap-time columns of @tbl-vol3-caching-tier-latency-tradeoffs.
...
: **Memory Tier Characteristics and Swap Costs**: Storage technology, effective transfer bandwidth, access latency, and 32k-token KV swap time for each tier of the paging hierarchy. Swap time grows by roughly an order of magnitude per tier, which sets the tool-wait duration at which offloading to that tier becomes profitable. {#tbl-vol3-caching-tier-latency-tradeoffs}
...
swapping out the KV cache is immediately profitable because the physical HBM frames can be yielded to concurrent active requests during the tool execution window. A $40.9\text{ ms}$ threshold puts essentially every real tool call above the line, which makes the descent in @fig-vol3-caching-hierarchical-paging the ordinary path for an idle trajectory rather than an emergency measure taken under memory pressure. As long as the runtime prefetches the KV blocks from Host DRAM back to HBM asynchronously as the tool completes, the agent resumes decoding with zero perceived transfer latency.

::: {#fig-vol3-caching-hierarchical-paging fig-env="figure" fig-pos="htb" fig-cap="**Hierarchical KV-Cache Paging**: An idle agent context migrates down three memory tiers, from accelerator HBM to host DRAM over PCIe Gen5 or CXL, and on to NVMe flash. The expected wait selects the tier: second-scale tool calls page to DRAM, while human-review pauses hibernate to flash, so a trajectory retains its logical context without holding scarce device memory." fig-alt="An upper panel stacks three memory tiers, each annotated with capacity, bandwidth, and the session state it holds: accelerator HBM at the top, host DDR5 or CXL memory in the middle reached over a PCIe Gen5 link, and NVMe flash with a pooled remote store at the bottom. A lower panel traces the tool-wait lifecycle as numbered steps, from active token generation through a non-blocking swap of key-value blocks from HBM to DRAM that frees device page frames."}

![](images/svg/hierarchical_kv_paging_topology.svg)

:::

## Prefill-Decode Disaggregation {#sec-vol3-caching-prefill-decode-disaggregation-splitwise-and}
...
- **Prefill Phase:** Computes across all input prompt tokens simultaneously using large general matrix multiplication (GEMM) kernels. Operating in the compute-bound regime of the hardware Roofline model, it achieves high Arithmetic Intensity (exceeding $100\text{ FLOPs/byte}$) and saturates Tensor Cores at peak utilization.
- **Decode Phase:** Generates one token at a time autoregressively using matrix-vector multiplications (GEMV). Operating in the memory-bound regime of the Roofline model, it suffers from severe Arithmetic Intensity starvation (near $1\text{ FLOP/byte}$) and is bottlenecked entirely by HBM read bandwidth.
...
As established in @sec-vol3-context-working-sets, to eliminate phase interference, disaggregated serving architectures—pioneered by Splitwise and DistServe—physically separate the prefill compute pool from the decode compute pool [@patel2024; @zhong2024distserve]:
...
Because $42\text{ ms}$ is orders of magnitude smaller than the seconds required for prefill compute or decode generation, network transfer adds negligible overhead to the end-to-end trajectory. Disaggregation isolates latency SLOs, eliminating ITL jitter and increasing cluster-wide serving goodput by $2.5\times$ to $4.2\times$ under rigorous production SLO targets [@zhong2024distserve]. The split also lets each pool be provisioned for its own bottleneck, tensor-core density on one side of the fabric in @fig-vol3-caching-disaggregation and HBM bandwidth on the other, a choice a co-located deployment has to compromise on because both phases share the same silicon.

::: {#fig-vol3-caching-disaggregation fig-env="figure" fig-pos="htb" fig-cap="**Prefill-Decode Disaggregation**: Compute-bound prompt prefill and bandwidth-bound token decode run on separate node pools joined by an RDMA fabric, which removes the head-of-line blocking a long agent prefill inflicts on co-located decode iterations. The split pays for itself because streaming the KV slice across the network costs far less time than the prefill that produced it, so inter-token latency is isolated without measurable end-to-end delay." fig-alt="Three columns. The left column describes a compute-bound prefill node pool with high arithmetic intensity, time to first token as its metric, tensor-core hardware, and a warning about head-of-line blocking when prefill and decode are co-located. The center column shows an RDMA transfer fabric over RoCEv2 or InfiniBand carrying zero-copy writes between accelerator memories, with the invariant that transfer time stays below prefill time. The right column describes a bandwidth-bound decode node pool with low arithmetic intensity, inter-token latency as its metric, and paged attention block management."}

![](images/svg/prefill_decode_disaggregation.svg)

:::

## KV-Centric Datacenter Architectures {#sec-vol3-caching-kvcache-centric-datacenter-architectures-rdma}
...
To overcome node-level isolation, @qin2024mooncake introduced Mooncake, the KVCache-centric disaggregated architecture previewed in @sec-vol3-context-working-sets, which elevates the KV cache to a first-class distributed storage system across the entire data center.
...
By decoupling prefix caching from the physical placement of compute, KVCache-centric data center architectures achieve global cache hit rates exceeding 90 percent across thousands of distributed agent workflows, transforming multi-turn agent serving economics.

### Immutability and the absence of cache coherence

A foundational principle of distributed systems is that caching mutable state requires complex, latency-inducing cache coherence protocols (e.g., MESI, directory-based invalidation) to prevent nodes from reading stale data. However, autoregressive LLM KV caches possess a profound systems property: **token sequences and their corresponding KV activations are strictly immutable**. The mathematical Key and Value tensors for the sequence `[A, B, C]` will never change, regardless of how many agents access them.

Because KV caches are append-only and immutable, KVCache-centric architectures completely bypass distributed cache coherence overhead. When Node C replicates a prefix from Node A, it requires no lease, no lock, and no invalidation callback. The global metadata service only tracks placement, not mutation. This structural immutability is the key system property that enables KV disaggregation to scale gracefully to tens of thousands of nodes without distributed coordination bottlenecks.

## Branching Speculation {#sec-vol3-caching-branching-speculation-prefix-cache}
...
For a $24\text{k}$-token prompt expanding a 3-step tree with branching factor $K = 8$, naive serving prefills **$12.3\text{ million}$ tokens at the leaf level alone**, and roughly $14\text{ million}$ across the whole tree. At a sustained $500\text{ TFLOP/s}$ that is about a GPU-hour of prefill to answer one query, which is what makes naive test-time search economically impossible.
...
instead of duplicating $L_{\text{root}}$ across all $N_{\text{branches}}$ leaves. By eliminating root duplication, Radix caching moves the limit on search width from the size of the shared prompt to the size of the divergent suffixes, and the distance between those two regimes on one 8-GPU node is what decides whether real-time MCTS rollouts are affordable at all [@snell2024scaling].

::: {#nbk-caching-branch-capacity .callout-notebook title="How many speculative branches fit on one node"}

**Problem**: On a single 8-GPU H100 node serving Llama-3-70B, how many parallel branches from a shared $24{,}000$-token root fit in accelerator memory when each branch duplicates the root, and how many fit once the root is shared?

**Variables**:

* Node capacity $8 \times 80\text{ GB} = 640\text{ GB}$ of HBM.
* Model weights at $70 \times 10^9$ parameters and $2\text{ bytes}$ per parameter in FP16.
* A free-block reserve of $10\%$ of node HBM held back by admission control. Scenario assumption.
* Llama-3-70B attention geometry from @tbl-vol3-caching-model-kv-footprint, with $n_{\text{layers}} = 80$, $n_{\text{kv\_heads}} = 8$, $d_{\text{head}} = 128$, and $b = 2$.
* $L_{\text{root}} = 24{,}000$ tokens for the search prompt and $L_{\text{branch}} = 2{,}000$ generated tokens per rollout. Both are scenario assumptions.

**Math**: The per-token footprint follows the derivation in @sec-vol3-caching-kv-cache-anatomy-tensor-layouts:

$$S_{\text{token}} = 2 \times 80 \times 8 \times 128 \times 2 = 327{,}680\text{ bytes}$$

Weights consume $70 \times 10^9 \times 2 = 140\text{ GB}$ and the reserve holds back $0.10 \times 640 = 64\text{ GB}$, leaving a KV budget of

$$640 - 140 - 64 = 436\text{ GB}$$

The root prefix and one rollout suffix cost

$$S_{\text{root}} = 24{,}000 \times 327{,}680 = 7.86\text{ GB}, \qquad S_{\text{branch}} = 2{,}000 \times 327{,}680 = 0.655\text{ GB}$$

Under contiguous allocation each branch carries a private copy of the root, so a branch costs $7.86 + 0.655 = 8.52\text{ GB}$ and the node holds

$$N_{\text{naive}} = \left\lfloor \frac{436}{8.52} \right\rfloor = 51$$

Under Copy-on-Write page sharing the root is paid once and only the suffixes scale:

$$N_{\text{shared}} = \left\lfloor \frac{436 - 7.86}{0.655} \right\rfloor = 653$$

**Result**: 51 concurrent branches with root duplication against 653 with prefix sharing, a $12.8\times$ widening of the search from the same $640\text{ GB}$ of HBM.

**Systems insight**: The duplicated prefix, not the generated text, is what caps search width. At these lengths the root is twelve times the suffix, so roughly $92\%$ of the naive footprint is 51 identical copies of the same tokens. Once the root is shared, capacity is governed by rollout length rather than prompt length, which inverts the tuning knob. Doubling the reasoning budget per branch halves the achievable width, while doubling the task prompt costs about $2\%$ of it.

:::

## Chunked Prefill and Batching {#sec-vol3-caching-chunked-prefill-and-continuous}
...
Building on the chunked prefill concepts introduced in @sec-vol3-context-working-sets, to maintain stable decode latencies while serving bursty multi-turn agent traffic, modern runtimes implement **Chunked Prefill** and **Piggybacking**, pioneered by SARATHI [@agrawal2023sarathi]:
...
Because decode tokens require loading the entire model parameter weight matrix $\mathbf{W}$ from HBM to compute only a single token per request, "piggybacking" a chunked prefill forwards those same weights through arithmetic tensor cores with zero additional memory read overhead. Operational intensity surges from $I \approx 1$ FLOP/byte up toward the accelerator saturation point, eliminating decode idle bubbles and ensuring that time-to-first-token (TTFT) and inter-token latency (ITL) remain completely stable even under bursty agent loads.

## Radix Cache Thrashing, Traced End to End {#sec-vol3-caching-production-postmortem-radix-cache}
...
### Scenario and architectural root cause

Consider a fleet of autonomous software debugging agents on a 64-GPU vLLM cluster. The walkthrough below is a constructed scenario rather than a documented incident, and its figures are illustrative; the causal chain is what the chapter is teaching. Cluster monitoring alerts on a massive drop in serving throughput accompanied by cascading out-of-memory (OOM) worker crashes.

Prior to the incident, the cluster operated at a steady-state prefix cache hit rate of **91.4 percent**, maintaining an average Time-to-First-Token (TTFT) of $220\text{ ms}$. Within six minutes of a new agent software release, the prefix cache hit rate plummeted to **0.0 percent**.
...
2. **Compute Explosion:** The cluster was instantly forced to execute full $35\text{k}$-token prefill computations for every single turn of every active agent. Effective cluster compute demand surged by $11.6\times$, the reciprocal of the $8.6$ percent of prefill the cluster had still been paying for.
...
4. **Queueing Backpressure and Pinned Block Exhaustion:** Dictated by Little's Law ($L = \lambda W$), as prefill latency ($W$) exploded from $220\text{ ms}$ to over $11\text{ seconds}$ under constant arrival rate ($\lambda$), the number of concurrent requests actively trapped in the system ($L$) skyrocketed. Requests that had already begun decoding could not complete because the GPU Tensor Cores were saturated with prefill compute. As partially decoded sequences accumulated, their pinned physical blocks completely exhausted the global free list.
...
This chapter gives principle \ref{pri-vol3-prefix-coherence} both halves of its claim. Prefix sharing is what makes multi-turn serving affordable, and coherence is what keeps the saving honest, because a cached prefix is valid only while the tokens and the model that produced it are unchanged.

## Summary {#sec-vol3-caching-summary}

Prefix caching and virtual memory paging establish the physical serving substrate for multi-turn agentic systems. By treating Key-Value activation tensors as demand-paged memory blocks organized in Radix prefix trees, runtimes reclaim GPU memory across branching reasoning trees and eliminate redundant prefill computation.

::: {.callout-takeaways title="Radix prefix caching and paged KV memory"}
* **Prefix Locality in Trajectories:** Real-world agent workloads exhibit 85 to 95 percent prompt token overlap between consecutive turns. Recomputing prefixes from scratch wastes massive GPU compute and degrades response times.
...
* **Hierarchical KV Paging:** Decoupling logical context retention from physical accelerator HBM residency allows runtimes to offload idle agent KV blocks over PCIe Gen5/CXL to Host DRAM during long tool-execution pauses, achieving swap breakevens under $100\text{ ms}$.
...
Paged memory management proves that foundation model serving is an operating systems problem. By structuring memory allocation around the physical reality of attention tensors and token prefix locality, modern serving engines turn what would otherwise be an unmanageable memory footprint into an amortized, demand-paged infrastructure resource.

::: {.callout-chapter-connection title="From KV paging to trajectory scheduling"}
Memory virtualization and Radix caching optimize the physical storage of attention activations on individual nodes, but deploying agents at production scale introduces severe scheduling challenges across accelerator clusters. @Sec-vol3-trajectory-scheduling addresses the cluster coordination problem: dispatching and scheduling stateful trajectories under heavy-tailed Pareto execution times, preemption under memory pressure, and multi-tier swapping.
:::
