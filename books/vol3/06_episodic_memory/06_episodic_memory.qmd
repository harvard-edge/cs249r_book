# Persistent Storage {#sec-vol3-episodic-memory}
...
The two previous chapters dealt with state that does not last. The working set is rebuilt for every invocation (@sec-vol3-working-sets), and attention state lives only as long as its serving session (@sec-vol3-virtual-memory). This chapter concerns the durable part of the state a task carries, the $S_3$ state of @sec-vol3-intro-hsa that must outlast both, and how it comes back. Most of it the agent did not create, such as the repository, the database, and the configuration. Some of it the agent did create, such as the record of the actions it has already taken. None of it can enter the model except by being retrieved and staged into context. The question here is which stored records the runtime may treat as true, and what a retrieved record must pass before it reaches the working set.

::: {.column-margin}
**Epistemic gap.** @sec-vol3-working-sets-context-rot defined the gap between the environment's authoritative state and the stale copy of it in the prompt. In durable storage the stale copy can also be an index entry.
:::
...
## Fallacies and Pitfalls {#sec-vol3-episodic-memory-fallacies}
...
## Summary {#sec-vol3-episodic-memory-summary}
...
Completing an autonomous task requires bridging the boundary between unprivileged token generation and deterministic physical execution. This transition marks the boundary into **Part III: Execution Sandboxing**, which answers the Containment Hazard of @sec-vol3-intro-causal-spine. In @sec-vol3-actuation (*Tool Execution*), we develop the typed action and observation interfaces—structured function schemas, the Model Context Protocol (MCP), tool call validation, and bidirectional streaming I/O buses—that allow an unprivileged foundation model to safely drive external software tools and observe real-world environments.
:::
