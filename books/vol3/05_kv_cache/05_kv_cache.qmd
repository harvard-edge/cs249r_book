...
The intersection of these two trends reveals why physical memory capacity has replaced arithmetic throughput as the fundamental governor of long-horizon agent systems. As demonstrated in the right panel of @fig-vol3-kv-cache-memory-wall, scaling context length to one million tokens requires 320 GB of key-value cache memory per 70B sequence—surpassing the 80 GB capacity of an NVIDIA H100 GPU by a factor of four. Even when scaling across an eight-accelerator node with a 480 GB dynamic pool, serving just two concurrent agents at this horizon ($B = 2$, 640 GB) completely exhausts cluster memory. This bottleneck has driven a two-pronged architectural response across machine learning systems engineering: runtime virtual memory paging via PagedAttention [@kwon2023vllm] to eliminate internal and external physical fragmentation, and algorithmic innovations such as Multi-Head Latent Attention (MLA) [@deepseek2024v3] that compress key-value projections into low-rank latent vectors, reducing memory footprint by up to 5.7-fold. Without these systems abstractions, supporting production agent workflows over multi-turn context windows would remain physically and economically intractable.
...
```{python}
#| label: lego-05-physical-block-sizing
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ SIZING PHYSICAL BLOCKS FOR 70B GQA MODEL (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-05-sizing-physical-blocks-for-a
# │ Goal: Sizing physical PagedAttention blocks and quantifying internal tail fragmentation.
# │ Show: 40 KiB/token on TP=8; B=16 incurs 0.12% tail waste; B=64 incurs 0.51% tail waste.
# │ How: Divide unmapped pool capacity by block footprint and calculate expected block tail waste.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
class PhysicalBlockSizing:
    """Block footprint and tail fragmentation in paged attention systems."""
    ...
    block_size_16 = 16
    block_size_64 = 64
    pool_per_gpu = 60.0 * GiB
    n_seq_batch = 256

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    n_kv_heads_gpu = max(1, n_kv_heads // tp_degree)
    m_token_gpu = calc_kv_cache_bytes_per_token(n_layers=n_layers, n_kv_heads=n_kv_heads_gpu, head_dim=d_head, bytes_per_elem=bytes_per_elem)
    m_token_node = m_token_gpu * tp_degree
    m_block_16 = (block_size_16 * m_token_gpu).to(KiB)
    m_block_64 = (block_size_64 * m_token_gpu).to(KiB)
    n_blocks_16 = int(pool_per_gpu.to(KiB) / m_block_16)
    token_cap_16 = n_blocks_16 * block_size_16
    waste_tokens_16 = (block_size_16 - 1) / 2
    node_waste_16 = (n_seq_batch * waste_tokens_16 * m_token_gpu).to(MiB)
    frag_ratio_16 = (node_waste_16 / pool_per_gpu.to(MiB)).to_base_units().magnitude
    waste_tokens_64 = (block_size_64 - 1) / 2
    node_waste_64 = (n_seq_batch * waste_tokens_64 * m_token_gpu).to(MiB)
    frag_ratio_64 = (node_waste_64 / pool_per_gpu.to(MiB)).to_base_units().magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(m_token_gpu.to(KiB).magnitude == 40.0, "m_token_gpu")
    check(m_token_node.to(KiB).magnitude == 320.0, "m_token_node")
    check(m_block_16.magnitude == 640.0, "m_block_16")
    check(m_block_64.magnitude == 2560.0, "m_block_64")
    check(n_blocks_16 == 98304, "n_blocks_16")
    check(token_cap_16 == 1572864, "token_cap_16")
    check(node_waste_16.magnitude == 75.0, "node_waste_16")
    check(abs(frag_ratio_16 - 0.0012) < 0.0001, "frag_ratio_16")
    check(node_waste_64.magnitude == 315.0, "node_waste_64")
    check(abs(frag_ratio_64 - 0.0051) < 0.0001, "frag_ratio_64")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_layers_str = fmt_int(n_layers)
    n_q_heads_str = fmt_int(n_q_heads)
    n_kv_heads_str = fmt_int(n_kv_heads)
    d_head_str = fmt_int(d_head)
    bytes_per_elem_str = fmt_int(bytes_per_elem.to(byte).magnitude)
    tp_degree_str = fmt_int(tp_degree)
    n_kv_heads_gpu_str = fmt_int(n_kv_heads_gpu)
    block_size_16_str = fmt_int(block_size_16)
    block_size_64_str = fmt_int(block_size_64)
    n_seq_batch_str = fmt_int(n_seq_batch)
    waste_tokens_16_str = fmt(waste_tokens_16, precision=1, commas=False)
    waste_tokens_64_str = fmt(waste_tokens_64, precision=1, commas=False)
```

::: {#nbk-05-sizing-physical-blocks-for-a .callout-notebook title="Sizing physical blocks for a 70B parameter model"}
Consider serving an open-weights 70B parameter model using Grouped-Query Attention (GQA). The architecture features $L =$ `{python} PhysicalBlockSizing.n_layers_str` layers, $H_Q =$ `{python} PhysicalBlockSizing.n_q_heads_str` query heads, $H_{\text{kv}} =$ `{python} PhysicalBlockSizing.n_kv_heads_str` key-value heads, head dimension $d_{\text{head}} =$ `{python} PhysicalBlockSizing.d_head_str`, and runs in BF16 precision ($b_{\text{elem}} =$ `{python} PhysicalBlockSizing.bytes_per_elem_str` bytes). The serving engine distributes the model across an 8-GPU node using tensor parallelism degree $TP =$ `{python} PhysicalBlockSizing.tp_degree_str`.

First, calculate the memory footprint per token on a single GPU. With $TP =$ `{python} PhysicalBlockSizing.tp_degree_str`, the $H_{\text{kv}} =$ `{python} PhysicalBlockSizing.n_kv_heads_str` key-value heads are partitioned evenly across the `{python} PhysicalBlockSizing.tp_degree_str` GPUs, resulting in $H_{\text{kv, GPU}} =$ `{python} PhysicalBlockSizing.n_kv_heads_gpu_str` key-value head per layer per GPU:
...
Now evaluate physical block parameters on a single GPU for block sizes $B =$ `{python} PhysicalBlockSizing.block_size_16_str` and $B =$ `{python} PhysicalBlockSizing.block_size_64_str`:

1. **Physical Block Footprint ($B =$ `{python} PhysicalBlockSizing.block_size_16_str`):**
   $$M_{\text{block, GPU}} = 16 \cdot 40\text{ KiB} = 640\text{ KiB}$$

2. **Physical Block Footprint ($B =$ `{python} PhysicalBlockSizing.block_size_64_str`):**
...
- **Pool Capacity at $B =$ `{python} PhysicalBlockSizing.block_size_16_str`:**
...
- **Tail Fragmentation under a Batch of $N_{\text{seq}} =$ `{python} PhysicalBlockSizing.n_seq_batch_str` Concurrent Sequences:**
  - For $B =$ `{python} PhysicalBlockSizing.block_size_16_str`: Expected waste is $(16 - 1)/2 =$ `{python} PhysicalBlockSizing.waste_tokens_16_str` tokens per sequence.
  ...
  - For $B =$ `{python} PhysicalBlockSizing.block_size_64_str`: Expected waste is $(64 - 1)/2 =$ `{python} PhysicalBlockSizing.waste_tokens_64_str` tokens per sequence.
