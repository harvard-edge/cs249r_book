...
_How can an inference serving system allocate and reuse the attention state of concurrent trajectories under finite accelerator memory?_

The fundamental bottleneck limiting concurrent agent execution on modern accelerator clusters is not arithmetic throughput, but high-bandwidth memory capacity consumed by key-value attention caches. For long-horizon agent trajectories, the KV cache generated across tens of thousands of historical tokens rapidly exceeds the physical memory footprint of the model weights themselves. Traditional serving engines allocate contiguous physical memory blocks based on worst-case sequence lengths, squandering over half of available GPU memory to internal and external fragmentation while preempting active tasks when allocations fail. Furthermore, agent workflows generate highly branchy execution trees: evaluating parallel tool proposals, hedging exploratory paths, and sharing identical system prompts across fleet workers. Treating every request as an isolated memory allocation forces redundant prefill computations that exhaust memory buses and inflate tail latencies. Systems engineers overcome these limits through virtualized attention memory: partitioning KV caches into fixed-size physical pages managed via dynamic page tables, sharing common prefix blocks across request trees through radix indexing, and orchestrating asynchronous swapping between accelerator HBM and host system memory. Scalable agent serving requires co-designing physical page allocation, prefix tree caching, and chunked prefill scheduling to eliminate memory fragmentation and maximize active trajectory density.
...
Working context, as @sec-vol3-context-engineering built it, decides which tokens each invocation sees. This chapter follows those tokens onto the accelerator, where each one leaves attention state that must stay in accelerator memory for as long as the trajectory might need it. For an agent, that state, not the model's parameters, comes to dominate accelerator memory. As an agent inspects files, runs build scripts, parses compiler diagnostics, and iterates across multi-step execution plans, the sequence length expands from hundreds of tokens to tens or hundreds of thousands. At these lengths, the attention state retained across forward passes to avoid recomputation swells to tens of gigabytes per active trajectory. If the serving runtime treats this state as an opaque, contiguous buffer, accelerator memory runs out within a handful of concurrent trajectories, and the engine faults or stops admitting work.

The architectural anatomy of this expanding context and its physical realization in accelerator high-bandwidth memory (HBM) is detailed in @fig-vol3-context-window-anatomy. The agent's token sequence is structurally partitioned into four functional regions: immutable system prompts ($S_{\text{sys}}$) defining agent behavioral constraints, static tool definitions ($S_{\text{tools}}$) declaring JSON function schemas, dynamic multi-turn execution scratchpads ($S_{\text{hist}}$) accumulating tool observations and command output, and the active generation head ($S_{\text{gen}}$) advancing step-by-step. To service autoregressive attention without recomputing past activations, the accelerator datapath passes each token's hidden state vector $x_t \in \mathbb{R}^{d_{\text{model}}}$ through linear projection matrices $W_K$ and $W_V$, yielding key and value vectors across $L$ transformer layers and $H_{\text{kv}}$ heads. These projections are appended to persistent physical tensor buffers in HBM, whose cumulative storage requirements scale linearly with sequence length $S$.

::: {#fig-vol3-context-window-anatomy fig-env="figure" fig-pos="htb" fig-cap="**Prompt and Context Window Datapath Anatomy**: Structural partitioning of an agent context window into static instructions, tool schemas, dynamic scratchpad execution history, and active generation heads, alongside the accelerator datapath projecting token hidden states into multi-layer key-value attention tensors in high-bandwidth memory." fig-alt="Schematic diagram illustrating four context window zones (system prompt, tool schemas, scratchpad history, generation head) feeding through linear key and value projections into multi-head key-value tensor buffers in accelerator high-bandwidth memory."}
![](images/svg/prompt_context_window_anatomy.svg)
:::

The **key-value cache** is the retained physical attention state of a transformer sequence; its byte footprint is dictated strictly by model geometry and active token count, establishing the fundamental concurrency ceiling of the serving runtime. Understanding the physical mechanics of this cache is the prerequisite for designing agent serving systems. A serving runtime cannot virtualize or schedule what it cannot measure. To build dependable agent runtimes, systems engineers must derive the exact memory footprint of attention tensors, characterize how model architectural variants alter state volume, and analyze the physical trade-off between retaining activations in high-bandwidth memory and recomputing them on demand.

::: {.column-margin}
![](images/svg/hsa_locator_s.svg){width="100%" fig-alt="H·S·A locator with the State axis highlighted."}

*This chapter closes State, holding attention state in pages it can share and reclaim.*
:::

Three agent behaviors make that state hard to manage. Trajectories stop at lengths no one can predict, so no fixed reservation fits them. They fork, so several branches want the same prefix. They pause for tools, so state sits idle while other trajectories wait for memory. In H·S·A terms (@sec-vol3-intro-hsa), this is where a long horizon becomes carried state that the accelerator must physically hold. The question for this chapter is how a serving system allocates, shares, and reclaims attention state so that a node's capacity is set by what trajectories use rather than by what they might use.

In @sec-vol3-foundation-model-tokenization, we established the tensor geometry of the Key-Value (KV) cache, demonstrating that avoiding quadratic $O(S^2)$ recomputation requires preserving intermediate projection vectors in accelerator High-Bandwidth Memory (HBM).[^fn-kv-cache-geometry] For an active context containing $S$ tokens across a concurrent batch of $B$ agent trajectories, aggregate attention state scales linearly as $\text{Mem}_{\text{batch}} = B \cdot S \cdot \text{Mem}_{\text{token}}$, where $\text{Mem}_{\text{token}} = 2 \cdot L \cdot H_{\text{kv}} \cdot d_{\text{head}} \cdot P$.

[^fn-kv-cache-geometry]: **KV Cache Geometry**: See @sec-vol3-foundation-model-tokenization for the mathematical derivation of Key-Value tensor projections across transformer layers and attention heads. This chapter addresses the physical virtualization of these blocks on accelerator silicon.
...
In @sec-vol3-foundation-model, this cache was analyzed under an idealized systems assumption: that the runtime allocates a contiguous physical memory buffer for each sequence. On physical accelerator hardware, however, this assumption fails catastrophically. Because an autonomous agent's execution horizon is non-deterministic—terminating after three turns or continuing for fifty tool interactions—contiguous memory allocators must pre-allocate physical buffers sized for the worst-case context length ($S_{\max}$). When actual trajectories terminate early or grow dynamically, this contiguous strategy induces severe internal and external memory fragmentation.

::: {#nbk-05-sizing-the-attention-state-for .callout-notebook title="Sizing the attention state for a 70-billion parameter model"}
Consider an inference cluster serving an open-weights 70-billion parameter model structured with the canonical architectural parameters from @sec-vol3-foundation-model-tokenization ($L=80$ layers, $H_{\text{q}}=64$ query heads, $H_{\text{kv}}=8$ GQA key-value heads, head dimension $d_{\text{head}}=128$, and 16-bit `bfloat16` precision with $P=2\text{ bytes}$).

Applying the token footprint formulated in @sec-vol3-foundation-model-tokenization:
$$\text{Mem}_{\text{token}} = 2 \cdot 80 \cdot 8 \cdot 128 \cdot 2\text{ B} = 320\text{ KiB/token}$$

Now evaluate the physical memory footprint across three realistic agent context regimes:

1. **Standard Turn ($S = 8,192$ tokens):**
   $$\text{Mem}_{\text{seq}} = 8{,}192 \cdot 320\text{ KiB} \approx 2.50\text{ GiB}$$

2. **Deep Diagnostic Trajectory ($S = 32,768$ tokens):**
   $$\text{Mem}_{\text{seq}} = 32{,}768 \cdot 320\text{ KiB} \approx 10.00\text{ GiB}$$

3. **Repository-Scale Context ($S = 131,072$ tokens):**
   $$\text{Mem}_{\text{seq}} = 131{,}072 \cdot 320\text{ KiB} \approx 40.00\text{ GiB}$$

Had this model been designed using Multi-Head Attention ($H_{\text{kv}} = 64$), the memory consumption would scale eightfold: a single 131,072-token sequence would demand $320.00 \text{ GiB}$ of physical memory purely for its key-value projections—an allocation that exceeds the entire onboard memory capacity of four 80 GB NVIDIA H100 GPUs combined.
:::
...
- [ ] **Internal fragmentation**: Why does reserving worst-case sequence capacity $S_{\max}$ upfront leave up to 80 percent of accelerator High-Bandwidth Memory unutilized?
...
The foundational premise of prefix caching is that token sequence identity implies key-value state identity. This equivalence holds strictly because autoregressive transformer architectures enforce a lower-triangular causal attention mask (formulated in @sec-vol3-foundation-model).[^fn-causal-mask-invalidation]

[^fn-causal-mask-invalidation]: **Causal Mask Invariant**: See @sec-vol3-foundation-model for the mathematical definition of causal attention masking $\text{softmax}(\mathbf{Q}\mathbf{K}^T/\sqrt{d_{\text{head}}} + \mathbf{M})\mathbf{V}$. Here, this causal dependency establishes that token modifications at index $k$ invalidate all downstream cached blocks $t \ge k$.
...
**Prefix caching is therefore all-or-nothing from left to right** (principle \ref{pri-vol3-prefix-coherence}). Any divergence in the token prefix instantly terminates tree traversal, invalidating the entire downstream branch of the Radix tree for the incoming request.

@Sec-vol3-context-engineering-staging turned this rule into the ascending-volatility layout, together with the timestamp and tokenization hazards that break it. The serving engine does not enforce that layout. It only rewards it, with a hit that covers every full block before the first volatile token.
...
::: {#nbk-05-sizing-the-break-even-frontier-for .callout-notebook title="Sizing the break-even frontier for a 70-billion parameter trajectory"}
