...
\begin{marginfigure}
\mlagentstack{10}{10}{95}{20}{20}{20}
\end{marginfigure}
:::

_Why does an agent waiting on an API call starve interactive users of GPU compute?_

When an agent enters a prolonged wait state for an external tool or human authorization, its execution trajectory blocks. If the runtime naively holds the active context in the physical GPU High-Bandwidth Memory during these extreme bimodal latency spikes, scarce accelerator resources are stranded, collapsing system-wide throughput. In agentic systems, the runtime must co-design the neural policy proposal engine with deterministic execution boundaries.
...
- Explain the queueing dynamics of heavy-tailed agent workloads and the failure of First-In-First-Out disciplines.
- Compare classical process control blocks with the Agent Control Block architecture.
- Calculate the analytical breakeven threshold for asynchronous memory swapping during external tool waits.
- Design bimodal execution mechanics to decouple compute-bound prompt evaluation from memory-bound generation.
- Diagnose head-of-line blocking and formulate Multi-Level Feedback Queue priority layers for continuous agent runtimes.
- Architect chunked prefill disaggregation protocols to prevent phase interference and Coffman deadlocks.
:::
...
Consider an autonomous agent tasked with bisecting an upstream repository regression. It checks out multiple commits, compiles native binaries, runs full integration test suites, and evaluates compiler errors. When this agent's execution state is active on hardware, it is not a static record on disk. It is an active, mutating, stochastic process that must contend with hundreds of peer trajectories for physical compute, memory bandwidth, and network I/O.

In classical machine learning systems, the serving runtime operates under the **Request-Response Paradigm**\index{Request-Response Paradigm}. An incoming client request carries a prompt, the serving engine batches it with concurrent queries via continuous iteration batching (e.g., vLLM [@kwon2023vllm], TensorRT-LLM), executes prompt prefill, autoregressively decodes tokens until an end-of-sequence delimiter is reached, and returns the generated payload. The transaction is atomic, isolated, and brief. Once the final token leaves the GPU, all execution state is destroyed or relegated to passive prefix caches.

Agentic systems shatter this operational paradigm by introducing **trajectory-level statefulness**\index{Trajectory-level statefulness}:

1. **Unpredictable Execution Horizons:**\index{Unpredictable Execution Horizons} Unlike single-turn queries whose completion lengths are tightly constrained, an autonomous agent tasked with resolving an open-ended software issue may terminate in two steps (detecting a syntax typo) or expand into hundreds of steps across multiple subagents (bisecting an upstream regression, compiling native binaries, running full integration test suites).
2. **Extreme Bimodal Latency:**\index{Extreme Bimodal Latency} Neural token generation operates on millisecond timescales ($20\text{ to }50\,\text{ms}$ per token). Conversely, executing tools within deterministic sandboxes—running compilers, web scrapers, or database migrations—consumes seconds to minutes, while waiting on human authorization gates consumes hours to days. An agent spends up to $90\%$ of its wall-clock lifetime waiting on external systems. Holding scarce GPU High-Bandwidth Memory (HBM) during these idle periods collapses serving utilization.
3. **Multi-Resource Coupling:**\index{Multi-Resource Coupling} Traditional schedulers optimize for a single scalar resource—typically GPU compute utilization or token throughput. Agentic trajectories contend simultaneously for coupled, multi-dimensional resources: Tensor Core floating-point throughput, physical HBM allocations for Key-Value (KV) activations, host DRAM, PCIe bus bandwidth, and finite external tool leases (e.g., container sandboxes, rate-limited third-party APIs).
4. **Phase Interference and Head-of-Line Blocking:**\index{Phase Interference}\index{Head-of-Line Blocking} A single agent submitting an expanded $64\text{k}$-token context prompt initiates a massive, compute-bound prefill phase that monopolizes accelerator streaming multiprocessors, freezing active decoding iterations for dozens of concurrent interactive agents and inducing severe jitter in Inter-Token Latency (ITL).
...
Consider a rogue script executing within an agent's sandbox that unexpectedly attempts to open hundreds of network connections or consume an infinite loop of tokens. If the runtime treats an agent simply as an open conversational prompt buffer, it lacks the mechanisms to enforce security isolation, track financial expenditures, preempt the execution, or recover from divergent reasoning paths.

A computer operating system does not execute raw machine code in a vacuum; it schedules *processes*. In classical systems theory, as established by Ritchie and Thompson in the design of UNIX [@ritchie1974], a process is defined by its authoritative execution descriptor: the **Process Control Block (PCB)**\index{Process Control Block}. The PCB encapsulates the processor registers, memory page table pointers, open file descriptors, signal handlers, and security credentials of an executing program.

The fundamental unit of execution in an agentic runtime is the **Agent Control Block (ACB)**\index{Agent Control Block}.
...
An **Agent Control Block**\index{Agent Control Block} is formally defined as a 9-tuple:
...
| **Memory Isolation**        | MMU page tables (CR3 register, virtual 4 KB pages)        | PagedAttention [@kwon2023vllm] virtual block tables, Radix tree prefix handles |
...
| **Failure Recovery**        | Core dump, signal handler (`SIGSEGV`), process restart    | Sagas [@garcia1987sagas] compensating transactions, reflection rollbacks       |
| **Scheduling Granularity**  | Instruction cycle / time slice ($1\text{--}10\text{ ms}$) | Token iteration / execution step ($20\text{--}50\text{ ms}$ / step)            |

: Table 7.1: **Architectural Comparison between POSIX PCB and Agent Control Block**: Structural mapping of operating system primitives, memory virtualization handles, execution privileges, and recovery mechanisms between classical processes and autonomous agents. {#tbl-pcb-acb-comparison tbl-colwidths="[20,35,45]"}

### The ACB Lifecycle State Machine

An agent trajectory does not run as an unbroken stream of execution. It transitions through a formal **Finite State Machine (FSM)**\index{Finite State Machine} governed by invariant checks, preemption triggers, and external I/O completions (@fig-vol3-scheduling-acb-lifecycle).
...
3. **`ACTIVE_DECODE`:** The foundation model autoregressively generates reasoning thought tokens and tool-call schemas. This phase is memory-bandwidth bound, reading the entire model weight matrix for every generated token.
4. **`SUSPENDED_TOOL_WAIT`:** The foundation model emits an actionable tool invocation. The runtime traps the call, validates arguments against capability lease schemas, and dispatches the request to an isolated sandbox (see @sec-vol3-virtualization). The agent relinquishes its active decode slot on the accelerator.
...
: Table 7.2: **Agent Control Block Lifecycle States and Execution Invariants**: Hardware resource residency, operational phases, transition triggers, and kernel management actions across the ACB finite state machine. {#tbl-vol3-scheduling-acb-states tbl-colwidths="[20,20,20,20,20]"}

### Resource Quotas and Token Burn Accounting

In classical operating systems, rogue processes are constrained by CPU quotas (e.g., CFS bandwidth limits) and memory limits (e.g., cgroup OOM killers). In agentic systems, runaway execution manifests as **Token Burn**\index{Token Burn}—the rapid, exponential consumption of inference compute, API credit, and context capacity driven by degenerative reasoning loops.
...
Static budget caps protect against catastrophic financial loss, but they fail to prevent an agent from wasting $95\%$ of its budget inside a tight, productive-looking failure loop. To catch semantic livelocks early, the scheduler monitors **Token Burn Velocity**\index{Token Burn Velocity} ($v_{\text{burn}}$) and **Action Repetition Entropy**\index{Action Repetition Entropy}:
...
To detect this phenomenon algorithmically, the ACB tracks the **Action Repetition Jaccard Distance**\index{Action Repetition Jaccard Distance} over a sliding window of the last $k$ tool invocations:
...
If an agent incorporates an observation that logically depended on an action that has not yet completed, it suffers from **Causal Inversion**\index{Causal Inversion}—generating reasoning steps based on an impossible timeline.

To guarantee rigorous causal ordering, each ACB maintains a **Vector Clock**\index{Vector Clock} [@lamport1978time; @mattern1989]:
...
With the Agent Control Block establishing the fundamental unit of execution and state, the kernel has the mechanisms to isolate and manage individual agent trajectories. However, tracking individual processes is only half of the scheduling problem. The runtime must determine the order in which these trajectories are dispatched to the physical hardware, a challenge complicated by the statistical properties of agent execution times.

## Heavy-Tailed Workloads {#sec-vol3-scheduling-heavy-tails}

Imagine a cluster handling hundreds of concurrent agent trajectories. One agent is tasked with correcting a single syntax typo, completing in two steps. Another is bisecting a multi-repository regression, expanding into hundreds of steps and monopolizing an accelerator for tens of minutes. This extreme variability destroys traditional scheduling assumptions.
...
|:-------------------------|:------------------------------------------|:--------------------------:|:---------:|:--------------------------:|:-----------------------:|:---------------------:|:------------------------:|
| **Stateless Serving**    | LMSYS Chatbot Arena [@chiang2024chatbot]  |            1.0             |    1.0    |            0.0             |         $0.0\%$         |        0 tokens       |          0.8 GB          |
| **Multi-Turn Chat**      | MT-Bench [@zheng2023judging]              |            2.0             |    2.0    |            0.0             |         $0.0\%$         |       450 tokens      |          1.4 GB          |
| **Web Navigation**       | WebArena [@zhou2023webarena] / VisualWeb  |            7.4             |    35.0   |            48.2            |         $72.4\%$        |      1,850 tokens     |         14.2 GB          |
| **Software Engineering** | SWE-bench Verified [@jimenez2023swebench] |            24.8            |   142.0   |           812.6            |         $84.1\%$        |       820 tokens      |         22.4 GB          |
| **Deep Research**        | GAIA [@mialon2023gaia] / Research Fleets  |            38.2            |   260.0   |          1,440.0           |         $88.6\%$        |      1,400 tokens     |         34.8 GB          |

: Table 7.3: **Empirical Workload Characteristics Across Serving Regimes**: Measurement of trajectory step counts, variance, external tool wait fractions, context growth, and resident KV-cache memory footprints across conversational, interactive, and autonomous agent benchmarks. {#tbl-vol3-scheduling-workload-comparison tbl-colwidths="[15,15,10,10,10,10,15,15]"}
...
In a naive First-In-First-Out (FIFO) queue, heavy-tailed agent workloads induce catastrophic **Head-of-Line (HoL) Blocking**\index{Head-of-Line Blocking}. Because trajectory lengths exhibit unbounded variance in these regimes, classical queueing models break down. A single massive 250-step task can monopolize accelerator HBM and compute slots for tens of minutes. Hundreds of subsequent quick, 2-step tasks are forced to wait in the queue behind the outlier, driving cluster tail latency to unacceptable extremes and stranding aggregate throughput.

### Clairvoyant vs. Non-Clairvoyant Scheduling

To minimize mean response time across jobs with arbitrary service times, classical scheduling theory points to **Shortest Remaining Processing Time (SRPT)**\index{Shortest Remaining Processing Time} [@harchol2001srpt; @harchol-balter2013].
...
However, SRPT is a **clairvoyant**\index{Clairvoyant scheduling} algorithm: it requires an exact oracle that knows the future processing requirement of every admitted job before execution finishes. In autonomous agent systems, clairvoyance is fundamentally impossible. An agent is a non-deterministic, open-ended reasoner operating over an unconstrained environment. Predicting whether an agent will finish in 3 steps or 200 steps with $100\%$ precision reduces to solving the Halting Problem over neural policy executions.

### Least Attained Service (LAS) Scheduling

When service durations are unknown a priori, the optimal non-clairvoyant scheduling discipline for heavy-tailed workloads is **Least Attained Service (LAS)**\index{Least Attained Service}—historically termed Feedback (FB) or Foreground-Background (FB) scheduling [@kleinrock1975queueing; @harchol-balter2013].
...
The superiority of LAS on heavy-tailed workloads stems from the Decreasing Hazard Rate property of power-law distributions. This leads to a profound systems intuition: **the longer an agent trajectory has been executing, the longer its expected remaining lifetime becomes**.

A brand-new agent has the highest instantaneous probability of completing immediately, whereas an agent that has already accumulated 100 steps is statistically likely to run for hundreds of additional steps.

LAS exploits this property optimally. By preemptively favoring tasks with the least attained service, it guarantees that quick, short-horizon agents are whisked through the system at maximum priority, eliminating Head-of-Line blocking without requiring oracle predictors.

### Multi-Level Feedback Queues (MLFQ)

In production GPU clusters, switching active execution contexts at every single token or forward pass induces intolerable PCIe and HBM swapping overhead. To implement LAS practically, schedulers discretize attained service using a **Multi-Level Feedback Queue (MLFQ)**\index{Multi-Level Feedback Queue} [@harchol-balter2013], as modeled in the flowchart below:
...
: Table 7.4: **Comparison of Scheduling Disciplines for Agent Workloads**: Theoretical properties, information requirements, tail-latency vulnerabilities, and implementation trade-offs across classical and agent-centric queueing disciplines. {#tbl-vol3-scheduling-discipline-comparison tbl-colwidths="[15,15,15,20,10,10,15]"}

While Least Attained Service scheduling mitigates head-of-line blocking by aggressively preempting long-running tasks, it assumes that execution itself is continuously compute-bound. In practice, agent workloads spend the vast majority of their lifetimes waiting for external events, necessitating a fundamentally different approach to physical memory management during these prolonged stalls.

## Bimodal Execution {#sec-vol3-scheduling-bimodal}

Consider an agent executing a web scraping task. The GPU evaluates the prompt and generates a tool call to fetch a URL in $45\text{ ms}$. The agent then waits $15\text{ s}$ for the remote server to respond and the headless browser to render the Document Object Model (DOM).

Stateless inference workloads are compute-and-memory bound within the accelerator. In contrast, autonomous agent trajectories are characterized by **extreme bimodal execution dynamics**\index{Bimodal execution dynamics}: they alternate between lightning-fast GPU forward passes and protracted, blocking external I/O operations.
...
1. **Neural Inference Phase ($20\text{ to }500\,\text{ms}$):** The foundation model consumes context tokens and autoregressively generates thought sequences and tool-call arguments.
...
: Table 7.5: **Multi-Tier Memory Hierarchy for Agent Context Swapping**: Physical media, interconnect bandwidths, capacities, and amortized operational costs across accelerator, host, and distributed storage tiers. {#tbl-vol3-scheduling-memory-hierarchy tbl-colwidths="[20,15,15,15,15,10,10]"}
...
Swapping a multi-gigabyte KV cache over the PCIe bus consumes memory bus bandwidth and incurs prefetch latency when the agent resumes. Therefore, memory swapping should not occur unconditionally. The runtime must only swap an agent if the anticipated suspension duration exceeds an analytical **Breakeven Threshold ($T_{\text{breakeven}}$)**\index{Breakeven Threshold}.

<!-- TODO: Moved to Appendix [ML/Systems/Hardware] -->

Because GPU HBM is significantly more expensive and scarce than Host DRAM, the breakeven duration is dominated by the physical time required to transfer the KV cache across the PCIe bus. If the expected wait time is longer than the round-trip DMA transfer time, it is more efficient to evict the context and reclaim the HBM for other active trajectories.

#### Numerical Case Study: Llama-3-70B [@dubey2024llama3]

Consider a 70B parameter model with a 64k context window. The KV cache footprint is roughly 21.5 GB. Over a PCIe Gen5 x16 interconnect operating at 64 GB/s, the one-way swap-out latency is approximately 336 ms. Consequently, the breakeven threshold is around 672 ms.
...
|:----------------------------------|:----------------------------------------------------|:-------------------:|:---------------------------------:|:------------------------------:|:------------------------------------:|:-------------------------------------------:|:-------------------------------------|
| **Llama-3-8B**                    | GQA [@ainslie2023gqa] (32 layers, 8 heads, $d=128$) | FP16 ($2\text{ B}$) |           16,384 tokens           |        $1.07\text{ GB}$        |           $16.8\text{ ms}$           |           $\approx 33.5\text{ ms}$          | Fast grep / AST query $\to$ Evict    |
| **Llama-3-8B**                    | GQA [@ainslie2023gqa] (32 layers, 8 heads, $d=128$) | FP16 ($2\text{ B}$) |           65,536 tokens           |        $4.29\text{ GB}$        |           $67.1\text{ ms}$           |          $\approx 134.2\text{ ms}$          | Web request / linter $\to$ Evict     |
| **Llama-3-70B**                   | GQA [@ainslie2023gqa] (80 layers, 8 heads, $d=128$) | FP16 ($2\text{ B}$) |           16,384 tokens           |        $5.37\text{ GB}$        |           $83.9\text{ ms}$           |          $\approx 167.8\text{ ms}$          | Sandbox bash command $\to$ Evict     |
| **Llama-3-70B**                   | GQA [@ainslie2023gqa] (80 layers, 8 heads, $d=128$) | FP16 ($2\text{ B}$) |           65,536 tokens           |       $21.47\text{ GB}$        |          $335.5\text{ ms}$           |          $\approx 671.1\text{ ms}$          | Compiler build / pytest $\to$ Evict  |
| **Llama-3-70B**                   | GQA [@ainslie2023gqa] (80 layers, 8 heads, $d=128$) | FP16 ($2\text{ B}$) |           131,072 tokens          |       $42.95\text{ GB}$        |          $671.1\text{ ms}$           |         $\approx 1,342.2\text{ ms}$         | Integration test run $\to$ Evict     |
| **Qwen-2.5-72B** [@qwen2024]      | GQA [@ainslie2023gqa] (80 layers, 8 heads, $d=128$) |  FP8 ($1\text{ B}$) |           65,536 tokens           |       $10.74\text{ GB}$        |          $167.8\text{ ms}$           |          $\approx 335.5\text{ ms}$          | Web browsing / DOM parse $\to$ Evict |
| **DeepSeek-V3** [@deepseek2024v3] | MLA [@deepseek2024v3] (61 layers, $d_c=512$)        |  FP8 ($1\text{ B}$) |           65,536 tokens           |        $2.05\text{ GB}$        |           $32.0\text{ ms}$           |           $\approx 64.0\text{ ms}$          | Micro-tool invocation $\to$ Evict    |
| **DeepSeek-V3** [@deepseek2024v3] | MLA [@deepseek2024v3] (61 layers, $d_c=512$)        |  FP8 ($1\text{ B}$) |           131,072 tokens          |        $4.10\text{ GB}$        |           $64.0\text{ ms}$           |          $\approx 128.1\text{ ms}$          | Multi-step bash tool $\to$ Evict     |

: Table 7.6: **KV-Cache Footprints, DMA Transfer Times, and Breakeven Suspension Thresholds**: Analytical evaluation of state sizes, bus migration latencies over PCIe Gen5 x16, and economic breakeven times across diverse model sizes, attention geometries, and context windows. {#tbl-vol3-scheduling-breakeven-tradeoffs tbl-colwidths="[15,20,10,15,15,10,15,20]"}
...
1. **Phase 1: Yield and Mark Inactive:** The foundation model emits the closing token of a tool-call schema. The parser traps the token, transitions the ACB to `SUSPENDED_TOOL_WAIT`, removes its active slot from the continuous batching iteration, and dispatches the execution payload to the sandbox daemon.
...
- **Tree-Search Deliberation Agents (MCTS) [@silver2016mastering]:** Saturated by GPU compute ($75\%$ compute share) running parallel rollouts; minimal memory footprint due to Radix tree Copy-on-Write prefix sharing; zero external tool calls.
...
To achieve Pareto-optimal cluster allocation across heterogeneous agents, the scheduler extends **Dominant Resource Fairness (DRF)**\index{Dominant Resource Fairness} [@ghodsi2011; @grandl2014].
...
Tenant $i$'s **dominant resource**\index{Dominant resource} is the resource type for which the tenant has acquired the largest fractional share of total cluster capacity:
...
The scalar $s_i$ is tenant $i$'s **dominant share**\index{Dominant share}.
...
To govern preemption safely, the scheduler categorizes agent execution into four distinct zones along the **Reversibility Spectrum**\index{Reversibility Spectrum}, detailed in Table 7.7 (@tbl-vol3-scheduling-policy-matrix) and illustrated in @fig-vol3-scheduling-preemption-matrix:
...
: Table 7.7: **Preemption Policy Across the Four Execution Zones**: Action classifications, state residency, reversibility invariants, and scheduler preemption mechanisms along the reversibility boundary. {#tbl-vol3-scheduling-policy-matrix tbl-colwidths="[15,15,20,15,15,15,25]"}
...
- **Zone 4 (Irreversible Actions):** Preemption is strictly prohibited. The scheduler must elevate the agent to **Pinned Real-Time Status**\index{Pinned Real-Time Status}, allocating emergency memory headroom until the physical action reaches terminal completion.

Bimodal context swapping efficiently reclaims accelerator memory during long external stalls, but it does not address the interference that occurs when agents are actively computing. When massive prompt evaluations collide with interactive token generation on the same accelerator, the runtime must decouple the architectural phases entirely.

## Chunked Prefill Disaggregation {#sec-vol3-scheduling-disaggregation}

An autonomous agent working on a large codebase submits a $64\text{k}$-token prompt containing a massive repository dump. If the cluster attempts to process this massive prefill on the same GPU that is actively decoding tokens for twenty other interactive agents, the decoding iterations freeze, inducing severe latency jitter.
...
- **The Prefill Phase:** Evaluates all input tokens concurrently using dense General Matrix Multiplication (GEMM) [@dao2022flashattention]. Operating squarely in the compute-bound regime of the hardware Roofline, it achieves high operational intensity ($I > 100\text{ FLOPs/byte}$) and saturates Tensor Cores at peak utilization.
...
To resolve phase interference without requiring separate physical hardware clusters, modern single-node inference engines implement **Chunked Prefill**\index{Chunked Prefill} and **Decode Piggybacking**\index{Decode Piggybacking} [@agrawal2024sarathi].
...
While chunked prefill alleviates phase collision on a single node, scaling enterprise agent fleets demands complete physical separation: **Prefill-Decode (P/D) Disaggregation**\index{Prefill-Decode Disaggregation} [@patel2024; @zhong2024distserve].
...
For prefill-decode disaggregation to maintain high throughput, network transfer latency must remain strictly lower than prefill compute latency. With modern 400 Gbps RDMA interfaces, streaming a multi-gigabyte KV cache slice between nodes takes only tens of milliseconds. Because this network latency is an order of magnitude smaller than the multiple seconds required to compute the prefill or decode a response, RDMA KV streaming introduces zero measurable delay. Disaggregating the phases increases cluster serving goodput by **$2.5\times$ to $4.2\times$** while guaranteeing strict tail ITL SLOs [@zhong2024distserve].
...
: Table 7.8: **Architectural Comparison of Serving Paradigm Disaggregation Strategies**: Systems characteristics, network interconnect demands, phase interference isolation, memory efficiency, and throughput scaling across serving architectures. {#tbl-vol3-scheduling-disaggregation-comparison tbl-colwidths="[15,15,15,15,15,15,10]"}
...
To overcome this, systems such as Mooncake [@qin2024mooncake] establish **KVCache-Centric Datacenter Architectures**\index{KVCache-Centric Datacenter Architectures}, elevating the KV cache to a globally shared distributed memory tier:
...
While KV pooling unlocks massive elasticity, memory-constrained agent serving introduces a dangerous distributed failure mode: **Coffman Memory Deadlocks**\index{Coffman Memory Deadlocks} [@coffman1971].
...
To prevent Coffman deadlocks, the scheduler must enforce strict **Capacity-Aware Admission Invariants**\index{Capacity-Aware Admission Invariants} based on Dijkstra's Banker's Algorithm [@dijkstra1965]:
...
By structurally isolating prefill from decoding and enforcing rigorous capacity invariants, the runtime prevents architectural phase collisions and distributed deadlocks. Yet, even with these robust mechanisms in place, reasoning about agent scheduling remains prone to classical intuition errors.
...
**Pitfall**: *Thundering herd during cluster-wide KV cache rehydration.*
...
- **Agent Workloads are Heavy-Tailed:** Trajectories expand unpredictably over Pareto horizons, causing First-In-First-Out disciplines to fail catastrophically.
- **Least Attained Service Optimality:** Schedulers must dynamically prioritize agents with the least attained service (LAS) to mitigate Head-of-Line blocking without requiring oracle predictors.
- **Bimodal Latency Demands Swapping:** External tool waits span seconds to minutes. Contexts must be asynchronously swapped to DDR5/NVMe to reclaim HBM.
- **Disaggregate Prefill from Decode:** Compute-bound prefill must be isolated from memory-bound decode to prevent jitter and maximize cluster goodput.
- **Enforce Delegation Quotas:** Bound recursion depth and token consumption to prevent cyclic Coffman deadlocks and financial runaway.
:::

*   **The Agent Control Block (ACB) as Execution Identity:** Abstracting raw token generation into an OS-level Agent Control Block provides the foundational data structure to track multi-dimensional resource quotas, cryptographically verify capability leases, and enforce finite-state lifecycle management over stochastic workflows.
*   **Embracing Non-Clairvoyant Queuing with LAS:** Agent trajectories exhibit unbounded heavy-tailed Pareto distributions, fundamentally breaking FIFO and SRPT queueing models. Schedulers must employ non-clairvoyant Least Attained Service (LAS) or Multi-Level Feedback Queues to aggressively cycle out long-running tasks, thereby preventing catastrophic Head-of-Line blocking.
*   **Context Swapping and the Breakeven Threshold:** Bridging the multi-order magnitude gap between microsecond token generation and minute-long tool waits requires preemptively evicting the KV-cache to Host DRAM. Eviction should occur asynchronously whenever an I/O stall is expected to exceed the analytical breakeven point ($T_{\text{breakeven}} \approx \frac{2 S_{\text{KV}}}{B_{\text{PCIe}}}$).
*   **Pareto-Optimal Allocation via Dominant Resource Fairness:** Enterprise agent fleets contend for conflicting resources—speculative models saturate compute, whereas software agents consume extreme HBM and research bots monopolize external sandbox leases. Dominant Resource Fairness (DRF) equalizes dominant consumption vectors, boosting aggregate cluster throughput by $40\text{--}65\%$ over static GPU partitioning.
*   **Safe Preemption through Reversibility Boundaries:** Agents acting upon the external world cannot be arbitrarily swapped or terminated. Schedulers evaluate an operation's external side effects on a strict four-zone Reversibility Spectrum, employing compensating Saga transactions (Zone 3) or elevating critical operations to real-time pinned status (Zone 4) during memory stress.
*   **Phase Decoupling and KV Fabric Disaggregation:** The compute-dense Roofline of prompt prefill aggressively interferes with memory-bound autoregressive decoding. True physical Disaggregated Serving (P/D) moves monolithic batching to specialized Pre-Fill and Decode node clusters connected via zero-copy RDMA fabrics, stabilizing tail Inter-Token Latency and resolving phase contention.
*   **Coffman Deadlock Prevention:** Extending disaggregated systems to KVCache-Centric data centers enables vast scalability but exposes the system to mutual-exclusion cyclic stalls. Schedulers must employ capacity-aware admission invariants via Banker's Algorithm [@dijkstra1965] to guarantee forward progress and preempt node-wide lockups.
...
::: {.callout-chapter-connection title="From Scheduling to Actuation Boundaries"}
While this chapter focused on the temporal orchestration of agent computation and memory state across hardware tiers, execution inevitably reaches the physical limits of the cluster. When an agent emits a verified tool invocation, the control plane must transition from scheduling internal GPU resources to brokering access to external environments. In **Chapter 8**, we explore the architectural design of these Actuation Boundaries—sandboxed isolation environments, privileged capability leases, and synchronous integration fabrics that safely translate stochastic neural outputs into deterministic side-effects.
:::

## Quantitative Problems {#sec-vol3-scheduling-problems}

**1. Least Attained Service Delay Under Heavy Tails**
Consider an inference cluster processing an agentic workload with service times drawn from a Pareto distribution (shape parameter $\alpha=1.2$, minimum service time $X_m = 2$ steps).
a) Calculate the expected waiting time for a new task under a pure First-In-First-Out (FIFO) discipline if the server utilization is $\rho=0.8$.
b) Formulate the Multi-Level Feedback Queue (MLFQ) priority thresholds to approximate Least Attained Service (LAS), and calculate the revised waiting time for a task that completes in 3 steps.

**2. Memory Evacuation Overhead**
An autonomous software engineering agent is operating on a 70B parameter model using Grouped Query Attention (GQA). The agent submits a $64\text{k}$ token prompt, which must be swapped out of HBM3e during a 45-second Python test suite execution.
a) Given a PCIe Gen5 x16 interface operating at $64\text{ GB/s}$, calculate the unidirectional DMA swap-out latency.
b) If the context grows to $128\text{k}$ tokens by step 10, what is the new breakeven suspension threshold ($T_{\text{breakeven}}$)? Assume Host DRAM allocation latency is $5\text{ ms}$.

**3. Chunked Prefill Compute-Memory Balance**
An agentic system co-locates prefill and decode on an H100 GPU (Peak FP16 TFLOPS = $989$, Memory Bandwidth = $3.35\text{ TB/s}$). A batch of 32 decoding agents requires a memory-bound GEMV every $25\text{ ms}$.
a) If a new agent arrives with a $32\text{k}$ token prefill request, calculate the prefill GEMM latency and the resulting Inter-Token Latency (ITL) jitter if executed monolithically.
b) Design a chunked prefill configuration (chunk size $K$) that limits ITL jitter to a maximum of $5\text{ ms}$, while maintaining roofline compute utilization.
