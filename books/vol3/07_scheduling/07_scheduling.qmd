...
Consider an inference cluster processing an agentic workload with service times drawn from a Pareto distribution (shape parameter $lpha=1.2$, minimum service time $X_m = 2$ steps). 
a) Calculate the expected waiting time for a new task under a pure First-In-First-Out (FIFO) discipline if the server utilization is $ho=0.8$.
