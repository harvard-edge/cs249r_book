...
### Formal loss objective and gradient dynamics

Formally, the supervised fine-tuning objective on an agent trajectory of length $T$ is governed by masked cross-entropy:
$$
\mathcal{L}_{\text{SFT}}(\theta) = -\frac{1}{\sum_{t=1}^T m_t} \sum_{t=1}^T m_t \log p_\theta(x_t \mid x_{<t})
$$
where $m_t \in \{0, 1\}$ is a binary token mask designating whether token $x_t$ is policed by gradient backpropagation:
$$
...
If observations are not masked, the gradient decomposes into:
$$
\nabla_\theta \mathcal{L}_{\text{unmasked}} = \nabla_\theta \mathcal{L}_{\text{action}} + \nabla_\theta \mathcal{L}_{\text{obs}}
$$
Because observations contain stochastic environment outputs (such as non-deterministic file timestamps, ephemeral process IDs, and dynamic network payloads), $\nabla_\theta \mathcal{L}_{\text{obs}}$ injects high-variance noise that penalizes the model for failing to memorize random environmental entropy rather than optimizing the decision policy $\pi_\theta(a \mid s)$. The trade-offs across masking strata are detailed in @tbl-vol3-adaptation-masking-tradeoffs.
...
### Loss normalization dynamics across variable-length trajectories

When trajectories are packed into training batches, the choice of loss normalization significantly impacts gradient variance. Under token-level normalization:
$$
\mathcal{L}_{\text{token}} = \frac{\sum_{i=1}^B \sum_{t=1}^{T_i} m_{i,t} \ell_{i,t}}{\sum_{i=1}^B \sum_{t=1}^{T_i} m_{i,t}}
$$
where $\ell_{i,t} = -\log p_\theta(x_{i,t} \mid x_{i,<t})$. Under sequence-level normalization:
$$
\mathcal{L}_{\text{seq}} = \frac{1}{B} \sum_{i=1}^B \frac{1}{\sum_{t=1}^{T_i} m_{i,t}} \sum_{t=1}^{T_i} m_{i,t} \ell_{i,t}
$$
In token-level normalization, a 100-turn trajectory containing 16,384 tokens contributes 32 times more gradient mass than a 2-turn trajectory containing 512 tokens, causing updates to over-index on multi-turn failure recoveries at the expense of concise, single-turn tool calls. Sequence-level normalization restores equal policy gradient weight across tasks.

### Block-diagonal attention isolation mechanics

Packing multiple trajectories into a single sequence buffer eliminates zero-padding waste, but requires strict attention isolation. Rather than computing full $O(L^2)$ attention across the packed buffer, modern kernel implementations (such as FlashAttention-2 and FlashAttention-3) accept a cumulative sequence length vector $\mathtt{cu\_seqlens} = [0, T_1, T_1 + T_2, \dots, \sum T_i]$. The block-diagonal attention mask enforces:
$$
A_{i,j} = \begin{cases} \frac{Q_i K_j^T}{\sqrt{d}} & \text{if } \text{traj}(i) = \text{traj}(j) \land j \le i \\ -\infty & \text{otherwise} \end{cases}
$$
This guarantees that query token $i$ can attend only to preceding tokens within its own trajectory, preventing cross-trajectory state pollution.
...
### Compounding regret derivation in behavioral cloning

Under standard behavioral cloning (BC) with teacher forcing, training assumes that the policy observes demonstration states drawn from an expert distribution $d_{\pi^*}$. If the policy has an expected single-step error probability bounded by $\epsilon$:
$$
\mathbb{E}_{s \sim d_{\pi^*}}[\mathbb{I}(\pi_\theta(s) \neq \pi^*(s))] \le \epsilon
$$
in an autoregressive rollout of horizon $T$, the first mistake diverts the policy to an unmodeled state $s \notin \text{supp}(d_{\pi^*})$. Because the policy was never trained on its own error states, subsequent predictions have error probability approaching 1. The expected total trajectory regret compounds quadratically:
$$
\mathbb{E}[\text{Regret}_{\text{BC}}(T)] \le \sum_{t=1}^T t \cdot \epsilon = \epsilon \cdot \frac{T(T+1)}{2} = \mathcal{O}(\epsilon T^2)
$$

Under interactive dataset aggregation (DAgger), rollouts are collected on-policy from the agent's induced state distribution $d_{\pi_\theta}$, and an expert labels the correction action. The expected regret bounds linearly with the trajectory horizon:
$$
\mathbb{E}[\text{Regret}_{\text{DAgger}}(T)] \le \mathcal{O}(\epsilon T)
$$

The real-world failure mode of unmitigated exposure bias is demonstrated in @lst-vol3-exposure-bias-trace, where a fine-tuned coding agent becomes trapped in an infinite error-retry loop.

::: {#lst-vol3-exposure-bias-trace lst-cap="**Failure Trace: Unmitigated Exposure Bias in Autonomous Trajectories**: An agent fine-tuned solely on pristine demonstrations encounters an unfamiliar Git CLI syntax error, entering an unrecoverable infinite loop because its training distribution contained zero recovery paths."}
```bash
# Policy loops on unmodeled state: git commit --force-resolve (Exit Code 128)
[Step 14] ACTION: git commit -m "fix bug" --force-resolve
[Step 14] OBS:    error: unknown option `force-resolve' (exit code 128)
[Step 15] THOUGHT: Retrying command to resolve conflicts...
[Step 15] ACTION: git commit -m "fix bug" --force-resolve
[Step 15] OBS:    error: unknown option `force-resolve' (exit code 128)
[Step 16] THOUGHT: Retrying command to resolve conflicts...
[Step 16] ACTION: git commit -m "fix bug" --force-resolve
[Step 16] OBS:    error: unknown option `force-resolve' (exit code 128)
```
:::
...
Fine-tuning agent models on extended multi-turn trajectories ($L = 32{,}768$ to $128{,}000$ tokens) encounters the *activation memory wall*. While model weights remain fixed, activation memory during backward passes scales linearly with sequence length $L$ and batch size $B$. For a transformer layer with hidden dimension $d_{\text{model}}$, attention heads $a$, and selective activation checkpointing, layer activation memory satisfies:
$$
M_{\text{act}} = B \cdot L \cdot d_{\text{model}} \cdot \left(34 + 5 \cdot \frac{a \cdot L}{d_{\text{model}}}\right) \text{ bytes per layer}
$$
As quantified in @tbl-vol3-sft-memory-footprints and @tbl-vol3-sft-memory-breakdown, activation memory rapidly dwarfs static adapter parameter weights, necessitating sequence parallelism, activation recomputation, and FlashAttention kernel optimization.
...
### Multi-tenant adapter dispatch and SRAM swapping

In production agent platforms, multiple specialized adapters (such as coding specialists, schema translation adapters, and database query writers) share a single frozen base foundation model. Instead of maintaining dedicated GPU memory allocations for each fine-tuned variant, runtimes utilize Low-Rank Adaptation (LoRA) where weight updates are factored into low-rank matrices:
$$
W = W_0 + \frac{\alpha}{r} B A, \quad B \in \mathbb{R}^{d \times r}, A \in \mathbb{R}^{r \times k}, \quad r \ll \min(d, k)
$$

The runtime dynamically hot-swaps adapter matrices into GPU SRAM and high-bandwidth memory (HBM) on a per-request basis, diagrammed in @fig-vol3-lora-adapter-swapping.
...
The latency, memory bandwidth, and capacity trade-offs across adapter serving tiers are detailed in @tbl-vol3-adapter-dispatch-arch.
...
### Perplexity divergence and multi-dimensional evaluation

Evaluating fine-tuned agent policies using language modeling perplexity or token-level validation loss is notoriously unreliable. A policy can exhibit lower validation loss while simultaneously suffering catastrophic operational collapse—such as emitting invalid tool JSON or entering infinite retry loops upon receiving unexpected command line errors. Robust evaluation requires an empirical evaluation scorecard measuring task completion rates, syntactic validity, step efficiency, and catastrophic forgetting, as diagrammed in @fig-vol3-sft-evaluation-radar.
...
The four operational evaluation dimensions are evaluated systematically in @tbl-vol3-sft-synthesis-scorecard.
...
### Controlled evaluation protocols and harness components

Benchmarking autonomous agents requires hermetic evaluation environments to prevent data leakage and non-deterministic network noise. Key architectural harness components are detailed in @tbl-vol3-evaluation-harness-components.
...
The end-to-end lifecycle of trajectory collection, validation filtering, schema regularization, parameter-efficient fine-tuning, and empirical scorecard evaluation forms an automated policy compilation pipeline, diagrammed in @fig-vol3-sft-synthesis-flow.
...
Fine-tuning adapts model priors to favor valid tool schemas, concise reasoning tokens, and robust error-recovery trajectories. However, no amount of parameter optimization can transform a stochastic neural network into an unbreachable security barrier. As formalized in the invariant closure principle (\ref{pri-invariant-closure}), runtime capabilities, filesystem isolation, network egress firewalls, and credential brokering must remain enforced mechanically beneath the model.
