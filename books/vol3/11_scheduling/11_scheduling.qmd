# Trajectory Scheduling & Cluster Resource Management {#sec-vol3-scheduling}
...
The hardware platform's **machine balance** or **ridge point** $\mathcal{I}_{\text{knee}}$ defines the critical threshold separating memory-bandwidth-bound operations from compute-bound operations in @eq-vol3-scheduling-ridge-point:

$$
\mathcal{I}_{\text{knee}} = \frac{P_{\text{peak}}}{B_{\text{mem}}} \quad \left[ \frac{\text{FLOP}}{\text{byte}} \right]
$$ {#eq-vol3-scheduling-ridge-point}

Consider an NVIDIA H100 SXM5 accelerator operating on 16-bit half-precision floating-point numbers (FP16/BF16), where dense peak Tensor Core throughput reaches $P_{\text{peak}} = 989\text{ TFLOPS}$ ($989 \times 10^{12}\text{ FLOP/s}$) and HBM3 memory bandwidth provides $B_{\text{mem}} = 3.35\text{ TB/s}$ ($3.35 \times 10^{12}\text{ B/s}$). Evaluating @eq-vol3-scheduling-ridge-point yields a machine ridge point of $\mathcal{I}_{\text{knee}} \approx 295.2\text{ FLOP/byte}$. Any kernel whose operational intensity satisfies $\mathcal{I} < 295.2\text{ FLOP/byte}$ is strictly bounded by HBM streaming bandwidth; its execution time is dominated by reading data across the memory bus, leaving the vast majority of Tensor Cores powered down or stalled. Conversely, any kernel with $\mathcal{I} \ge 295.2\text{ FLOP/byte}$ is compute-bound, fully utilizing the Tensor Cores.
...
We can derive the operational intensity from first-principles napkin math: in 16-bit half-precision (FP16/BF16), every model parameter occupies 2 bytes of memory and participates in 2 floating-point operations per token (one multiply-accumulate operation: $1 \text{ MAC} = 2 \text{ FLOPs}$). Loading the full model weight tensor requires reading $2 N_{\text{params}}$ bytes from HBM across the memory bus. Because those weights are loaded once and reused across all $S$ prompt tokens in parallel, the linear layers execute $2 N_{\text{params}} S$ FLOPs. The operational intensity is simply the ratio of compute to memory traffic, yielding @eq-vol3-scheduling-prefill-intensity:

$$
\mathcal{I}_{\text{prefill}}(S) \approx \frac{2 N_{\text{params}} S}{2 N_{\text{params}}} = S \quad \left[ \frac{\text{FLOP}}{\text{byte}} \right]
$$ {#eq-vol3-scheduling-prefill-intensity}
...
Every single decode step requires streaming the entire model's $2 N_{\text{params}}$ bytes of weights from HBM, alongside the historical Key-Value (KV) cache memory $M_{\text{KV}}$ for all $B_{\text{dec}}$ sequences across all $L$ layers (@eq-vol3-scheduling-decode-intensity):

$$
\mathcal{I}_{\text{decode}}(B_{\text{dec}}) = \frac{2 N_{\text{params}} B_{\text{dec}}}{2 N_{\text{params}} + M_{\text{KV}}} \quad \left[ \frac{\text{FLOP}}{\text{byte}} \right]
$$ {#eq-vol3-scheduling-decode-intensity}

where $M_{\text{KV}} = 4 L H_{\text{KV}} d_h \sum_{i=1}^{B_{\text{dec}}} S_i$ bytes.[^fn-scheduling-intensity-divergence]

Even if the historical KV cache were zero ($M_{\text{KV}} = 0$), the decode operational intensity is fundamentally capped by the batch size:

$$
\mathcal{I}_{\text{decode}} \le \frac{2 N_{\text{params}} B_{\text{dec}}}{2 N_{\text{params}}} = B_{\text{dec}} \quad \left[ \frac{\text{FLOP}}{\text{byte}} \right]
$$
...
Notice the duality in @eq-chunk-iter-time: reading model weights requires a baseline memory transfer time $t_{\text{weights}} = 2 N_{\text{params}} / B_{\text{mem}}$ regardless of whether $C=0$ or $C>0$. For a 70B parameter model on an NVIDIA H100 ($B_{\text{mem}} = 3.35\text{ TB/s}$), streaming weights across the memory bus takes:

$$
t_{\text{weights}} = \frac{2 \times 70 \times 10^9\text{ bytes}}{3.35 \times 10^{12}\text{ bytes/s}} \approx 41.8\text{ ms}
$$

During this $41.8\text{ ms}$ memory-read interval, the Tensor Cores are largely idle. Their theoretical arithmetic capacity during this time window is $P_{\text{peak}} \times t_{\text{weights}} = 2 N_{\text{params}} \mathcal{I}_{\text{knee}}$ FLOPs. A decode batch of $B_{\text{dec}}$ sequences only consumes $2 N_{\text{params}} B_{\text{dec}}$ FLOPs. The stranded arithmetic capacity available for prefill computation is therefore:

$$
\text{Stranded FLOPs} = 2 N_{\text{params}} \left( \mathcal{I}_{\text{knee}} - B_{\text{dec}} \right)
$$

Because each prefill token executes $2 N_{\text{params}}$ FLOPs in the linear projection and feed-forward layers, dividing stranded FLOPs by $2 N_{\text{params}}$ yields a remarkably clean **first-principles napkin rule**:

$$
C^* \approx \mathcal{I}_{\text{knee}} - B_{\text{dec}}
$$

On an NVIDIA H100 with $\mathcal{I}_{\text{knee}} \approx 295\text{ FLOP/byte}$, if the engine is actively decoding a batch of $B_{\text{dec}} = 32$ streams, the scheduler can piggyback:

$$
C^* \approx 295 - 32 = 263\text{ prefill tokens}
$$

into the iteration for **zero marginal latency cost**. The prefill compute executes concurrently within the memory fetch window of the decode weights.

When factoring in the secondary self-attention computation ($4 L d_{\text{model}} C S_{\text{avg}}$ FLOPs across historical context length $S_{\text{avg}}$), the exact critical chunk threshold $C^*$ at which the iteration transitions from memory-bound to compute-bound evaluates to @eq-vol3-scheduling-optimal-chunk:

$$
C^* = \frac{2 N_{\text{params}} \left( \mathcal{I}_{\text{knee}} - B_{\text{dec}} \right)}{2 N_{\text{params}} + 4 L d_{\text{model}} S_{\text{avg}}}
$$ {#eq-vol3-scheduling-optimal-chunk}

where $\mathcal{I}_{\text{knee}} = P_{\text{peak}} / B_{\text{mem}}$ is the machine balance.[^fn-scheduling-chunk-derivation] For typical sequence lengths, this denominator reduces $C^*$ by only 5 percent to 15 percent. This derivation demonstrates why chunked prefill is so effective: rather than slowing down active users, it repurposes arithmetic cycles that would otherwise be permanently lost to memory bus stalls.
...
To quantify this challenge, consider the physical footprint of the KV cache for a context sequence of length $S$. In a transformer with $L$ layers, $H_{\text{KV}}$ key-value heads, per-head dimension $d_h$, and numerical precision of $b_{\text{elem}}$ bytes per element, total resident memory footprint is governed by @eq-vol3-scheduling-kv-footprint:

$$
S_{\text{KV}}(S) = 2 L H_{\text{KV}} d_h S \cdot b_{\text{elem}} \quad \text{bytes}
$$ {#eq-vol3-scheduling-kv-footprint}

For the standard Llama-3-70B architecture ($L = 80, d_{\text{model}} = 8192, H_{\text{KV}} = 8, d_h = 128, b_{\text{elem}} = 2$) [@dubey2024llama], the KV cache memory density evaluates to $\sigma_{\text{KV}} = 2 \times 80 \times 8 \times 128 \times 2 = 327{,}680\text{ bytes/token} \approx 320\text{ KB/token}$. For an agent operating with an extended horizon of $S = 64\text{k} = 65{,}536$ tokens, resident context memory footprint reaches $S_{\text{KV}}(64\text{k}) \approx 21.47\text{ GB}$.

Transferring $21.47\text{ GB}$ of raw activation memory across a network interface introduces physical communication delay. Total transfer latency $t_{\text{transfer}}$ across an effective network bandwidth $B_{\text{net}}$ is governed by @eq-vol3-scheduling-kv-transfer:

$$
t_{\text{transfer}} = \frac{S_{\text{KV}}}{B_{\text{net}}} + t_{\text{setup}}
$$ {#eq-vol3-scheduling-kv-transfer}

where $t_{\text{setup}}$ represents RDMA queue pair connection and memory registration overhead ($\approx 0.5\text{--}2\text{ ms}$).[^fn-scheduling-network-fabrics]
...
then all intermediate network transfer latency is completely hidden behind forward-pass compute. The effective transfer penalty observed by the end-user collapses to the transmission duration of the final layer $L$ ($\Delta t_{\text{visible}} = \tau_{\text{transfer}}(L)$), governed by @eq-vol3-scheduling-visible-transfer:

$$
\Delta t_{\text{visible}} = \frac{S_{\text{KV}}}{L \cdot B_{\text{RDMA}}}
$$ {#eq-vol3-scheduling-visible-transfer}
:::

For Llama-3-70B with an extended context of $S = 64\text{k}$ across a 400 Gbps RDMA interconnect ($B_{\text{RDMA}} = 50\text{ GB/s}$), @eq-vol3-scheduling-visible-transfer evaluates to $\Delta t_{\text{visible}} = 21.47\text{ GB} / (80 \times 50\text{ GB/s}) \approx 5.37\text{ ms}$.[^fn-scheduling-streaming-overlap]
...
Consider an inference serving node modeled as an $M/G/1$ queue with Poisson arrival rate $\lambda$, arbitrary service time distribution $X$, and server utilization $\rho = \lambda \mathbb{E}[X] < 1$. By the classical Pollaczek-Khinchine formula [@kleinrock1975queueing; see @sec-vol3-appendix-math-queueing], the expected waiting time in the queue is governed by @eq-pollaczek-khinchine:

$$
\mathbb{E}[W_{\text{FIFO}}] = \frac{\lambda \mathbb{E}[X^2]}{2(1 - \rho)}
$$ {#eq-pollaczek-khinchine}

Notice the critical vulnerability in @eq-pollaczek-khinchine: expected waiting time scales not with the average task duration $\mathbb{E}[X]$, but with the second raw moment of service time $\mathbb{E}[X^2]$. For empirical agent workloads where $\alpha \in [1.1, 1.4]$, the second moment diverges to infinity ($\mathbb{E}[X^2] \to \infty$; see proof in @sec-vol3-appendix-math-pareto).[^fn-scheduling-pareto-variance]
...
The hazard rate $h(x) = f(x) / [1 - F(x)]$ represents the conditional probability density that a task will finish in the next infinitesimal interval, given that it has already executed for duration $x$. For a Pareto distribution with tail index $\alpha > 0$, the hazard rate evaluates to $h(x) = \alpha / x$, yielding a strictly negative derivative in @eq-decreasing-hazard:

$$
\frac{d}{dx} h(x) = -\frac{\alpha}{x^2} < 0 \quad \forall x > 0
$$ {#eq-decreasing-hazard}

where $h(x)$ decreases monotonically with attained service $x$.[^fn-scheduling-decreasing-hazard]

The physical implication of @eq-decreasing-hazard directly reflects real-world engineering experience: **the longer an agent has been running, the less likely it is to complete in the next step**. Think of an agent debugging a software repository:
...
- **Node A** holds $12\text{k}$ cached prefix tokens ($\text{MatchTokens} / S = 0.75$) and operates at $70\%$ memory load ($L_A / L_{\text{max}} = 0.70$):
  $$\text{Score}_A = 0.70 \times 0.75 - 0.30 \times (0.70)^2 = 0.525 - 0.147 = +0.378$$
- **Node B** is completely cold ($\text{MatchTokens} = 0$) but lightly loaded ($L_B / L_{\text{max}} = 0.20$):
  $$\text{Score}_B = 0.70 \times 0.00 - 0.30 \times (0.20)^2 = 0.000 - 0.012 = -0.012$$

Node A easily wins the request, sparing the cluster from recomputing $12\text{k}$ tokens ($3.8\text{ GB}$ of KV cache on a 70B model). However, if Node A's memory fills to $95\%$ load ($L_A / L_{\text{max}} = 0.95$), its score drops to $0.525 - 0.30 \times (0.95)^2 = +0.254$.

The quadratic exponent $\gamma = 2$ acts like an elastic barrier: below $70\%$ capacity, the load penalty is gentle, allowing prefix affinity to steer traffic freely; but as memory approaches $90\text{--}95\%$ saturation, the penalty escalates steeply, deflecting new requests to prevent GPU out-of-memory (OOM) evictions and cache thrashing.
...
Dominant Resource Fairness equalizes dominant shares across all active tenants via the max-min allocation in @eq-vol3-scheduling-drf-allocation:

$$
\max \min_{i \in \mathcal{A}} s_i^* \quad \text{subject to} \quad \sum_{i=1}^n N_i \mathbf{c}_i \le \mathbf{R}
$$ {#eq-vol3-scheduling-drf-allocation}

where $N_i$ denotes the allocated concurrency for tenant $i$.
:::
