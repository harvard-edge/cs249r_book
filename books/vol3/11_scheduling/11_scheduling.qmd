...
::: {.column-margin}
![](images/svg/vol3_scheduling_margin_001.svg){width="100%" fig-alt="Horizontal bar chart comparing forward execution latency of 5,200 ms against compensating saga unwind latency of 295 ms."}

*Saga compensation unwinds external state in 295 ms (17.6× faster than forward execution) without blocking transaction locks.*
:::
...
::: {#exmp-11-latency-cost-and-storage-overheads .callout-example title="Latency, Cost, and Storage Overheads of Saga Log Unwinding"}
...
![**Runtime Recovery Decision Architecture**: The runtime evaluates error taxonomy, resource headroom, and unwind costs to dynamically arbitrate between forward self-healing, backward compensation, and operator escalation.](images/svg/recovery-decision-tree.svg){#fig-vol3-recovery-decision-tree width="85%"}
...
::: {#exmp-11-quantitative-trade-off-between-forward-patching .callout-example title="Quantitative Trade-Off Between Forward Patching and Saga Rollback"}
...
![**Semantic Watchdog Oscillation Detection**: State hash oscillation detection over a sliding history window. The supervisor maintains rolling hashes to detect circular orbits and terminate non-advancing execution loops.](images/svg/semantic-watchdog-oscillation.svg){#fig-vol3-semantic-watchdog width="85%"}
...
::: {#exmp-11-the-microeconomic-and-hardware-cost .callout-example title="The Microeconomic and Hardware Cost of a Silent Semantic Loop"}
...
To protect upstream dependencies and conserve host compute, the runtime implements the circuit breaker pattern formulated by Michael Nygard. The tool execution gateway wraps every external RPC interface in a stateful software proxy that monitors outcome telemetry over a sliding execution window. As illustrated in @fig-vol3-circuit-breaker, the proxy operates as a finite-state machine with three distinct operational states: Closed, Open, and Half-Open.

![**Tri-State Tool Circuit Breaker State Machine**: The tri-state tool circuit breaker lifecycle. Transitions between Closed, Open, and Half-Open states are governed by a sliding-window failure rate threshold $\tau_{\text{fail}}$, an inactivity cooldown timer $T_{\text{cooldown}}$, and single-flight canary probes.](images/svg/circuit_breaker_state_machine.svg){#fig-vol3-circuit-breaker width="90%"}
...
::: {#exmp-11-sizing-bulkhead-capacity-and-jitter .callout-example title="Sizing Bulkhead Capacity and Jitter Under Degraded Tool RPCs"}
**Scenario:**
