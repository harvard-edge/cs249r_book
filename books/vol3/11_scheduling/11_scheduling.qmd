# Trajectory Scheduling {#sec-vol3-scheduling}
...
\noindent
![](images/png/cover_scheduling_blueprint_labeled_print.png){fig-alt="Isometric blueprint showing trajectory scheduling and cluster resource management: prompt activation tensor stream, compute-bound prefill blade bank, optical CXL interconnect fabric, streaming autoregressive tokens, memory-bound decode blade bank, and phase-interference isolation."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_scheduling_blueprint_labeled.webp){fig-alt="Isometric blueprint showing trajectory scheduling and cluster resource management: prompt activation tensor stream, compute-bound prefill blade bank, optical CXL interconnect fabric, streaming autoregressive tokens, memory-bound decode blade bank, and phase-interference isolation."}
:::
...
## Architectural Overview: Trajectory Scheduling and Cluster Resource Management {#sec-vol3-scheduling-intro}

Preemptive interrupts, watchdog timers, and asynchronous signal delivery (@sec-vol3-interrupts) establish execution control and safety boundaries for individual agent trajectories. In enterprise production environments, however, agents do not run in isolation on dedicated hardware. Instead, hundreds or thousands of heterogeneous agent workflows—spanning interactive code assistants, autonomous debugging loops, multi-agent swarms, and batch background processors—compete for shared accelerator compute and memory resources across distributed GPU clusters.

Managing this multi-tenant execution substrate introduces systems challenges that fundamentally diverge from classical stateless inference serving. In traditional web and conversational serving, requests are short, arrive with bounded variance, and complete in a single forward turn. In agentic systems, by contrast, trajectories are stateful, multi-turn loops that interleave GPU-intensive neural policy evaluations with prolonged, heavy-tailed pauses waiting on sandboxed code execution, compiler toolchains, database migrations, or human approvals. If the cluster scheduler relies on conventional First-In-First-Out (FIFO) queues or monolithic iteration batching, long-running reasoning tasks monopolize accelerator streaming multiprocessors, while blocked agents strand gigabytes of High-Bandwidth Memory (HBM), triggering cluster-wide head-of-line blocking and catastrophic out-of-memory faults.

This chapter develops the architectural foundation for multi-tenant trajectory scheduling across accelerator fleets. We first analyze the arithmetic intensity divergence between compute-bound prompt prefill and memory-bandwidth-bound token decode under the Roofline model, demonstrating how continuous chunked batching and piggybacking eliminate phase contention. We then examine physically disaggregated prefill-decode architectures that stream Key-Value (KV) activations across high-speed remote direct memory access (RDMA) fabrics. Next, we address the mathematical collapse of FIFO queues under heavy-tailed Pareto execution horizons, deriving non-clairvoyant Least Attained Service (LAS) and Multi-Level Feedback Queue (MLFQ) schedulers. Finally, we implement cache-aware prefix routing over global Radix tries and Banker's Algorithm memory admission controls to prevent Coffman deadlocks in shared KV memory pools.

## Continuous Chunked Batching {#sec-vol3-scheduling-continuous-batching}

Consider a production environment where an accelerator cluster serves 32 concurrent interactive developer agents alongside automated background maintenance tasks. Each interactive agent generates code edits and receives real-time syntax suggestions at an average Inter-Token Latency (ITL) of $25\text{ ms}$. When a background triage agent arrives with an expansive $64\text{k}$-token repository diff to bisect a compiler regression, a conventional serving engine running monolithic continuous iteration batching injects this entire $64\text{k}$-token prompt into the immediate next execution batch. The Tensor Cores on an NVIDIA H100 GPU are monopolized for $420\text{ ms}$ executing the prompt prefill General Matrix Multiply (GEMM) kernels. For nearly half a second, the 32 interactive agents cannot execute a single decode iteration; their Inter-Token Latency spikes by $16.8\times$ from $25\text{ ms}$ to $445\text{ ms}$. Interactive code completions stutter, token streaming buffers drain, and human-facing service level agreements (SLAs) are comprehensively breached.
...
While continuous batching dramatically improves accelerator throughput for short, homogeneous conversational queries in stateless serving runtimes, it encounters a physical breakdown when applied to agentic workloads characterized by expansive contexts, heavy-tailed Pareto service distributions, and intermittent tool executions. This breakdown stems from the fundamental divergence in operational intensity dictated by the Roofline model.

### The arithmetic intensity divergence and phase contention

The hardware efficiency of any workload on a modern accelerator is governed by the Williams-Waterman-Patterson Roofline model [@williams2009]. Let $P_{\text{peak}}$ denote the peak floating-point throughput of the processor (in $\text{FLOP/s}$), and let $B_{\text{mem}}$ denote the memory bandwidth between HBM and the compute cores (in $\text{bytes/s}$). The **arithmetic intensity**\index{Arithmetic intensity} (or operational intensity) $\mathcal{I}$ of an algorithm is defined as the ratio of total floating-point operations executed to total bytes transferred from memory: $\mathcal{I} = \text{FLOPs} / \text{Bytes}$.

The hardware platform's **machine balance** or **ridge point** $\mathcal{I}_{\text{knee}} = P_{\text{peak}} / B_{\text{mem}}$ defines the critical threshold separating memory-bandwidth-bound operations from compute-bound operations. Consider an NVIDIA H100 SXM5 accelerator operating on 16-bit half-precision floating-point numbers (FP16/BF16), where dense peak Tensor Core throughput reaches $P_{\text{peak}} = 989\text{ TFLOPS}$ ($989 \times 10^{12}\text{ FLOP/s}$) and HBM3 memory bandwidth provides $B_{\text{mem}} = 3.35\text{ TB/s}$ ($3.35 \times 10^{12}\text{ B/s}$), yielding a machine ridge point of $\mathcal{I}_{\text{knee}} \approx 295.2\text{ FLOP/byte}$. Any kernel whose operational intensity satisfies $\mathcal{I} < 295.2\text{ FLOP/byte}$ is strictly bounded by HBM streaming bandwidth; its execution time is dominated by reading data across the memory bus, leaving the vast majority of Tensor Cores powered down or stalled. Conversely, any kernel with $\mathcal{I} \ge 295.2\text{ FLOP/byte}$ is compute-bound, fully utilizing the Tensor Cores.
...
In the prefill phase, the model ingests an entire prompt sequence of length $S$ tokens in a single forward pass. Because all $S$ tokens are known simultaneously, the matrix-vector products across linear projections ($W_Q, W_K, W_V, W_O$ and the feed-forward blocks $W_{\text{gate}}, W_{\text{up}}, W_{\text{down}}$) are batched across all $S$ positions into General Matrix Multiplies (GEMMs).

We can derive the operational intensity from first-principles napkin math: in 16-bit half-precision (FP16/BF16), every model parameter occupies 2 bytes of memory and participates in 2 floating-point operations per token (one multiply-accumulate operation: $1 \text{ MAC} = 2 \text{ FLOPs}$). Loading the full model weight tensor requires reading $2 N_{\text{params}}$ bytes from HBM across the memory bus. Because those weights are loaded once and reused across all $S$ prompt tokens in parallel, the linear layers execute $2 N_{\text{params}} S$ FLOPs. The operational intensity is simply the ratio of compute to memory traffic, yielding $\mathcal{I}_{\text{prefill}}(S) \approx \frac{2 N_{\text{params}} S}{2 N_{\text{params}}} = S\text{ FLOP/byte}$.

When an agent prompt contains $S \ge 512$ tokens, its operational intensity $\mathcal{I}_{\text{prefill}} \ge 512\text{ FLOP/byte} > \mathcal{I}_{\text{knee}} = 295.2\text{ FLOP/byte}$. The prefill phase operates squarely in the **compute-bound regime**, saturating accelerator Tensor Cores.

**The decode phase (memory-bandwidth bound):**
In the autoregressive decode phase, the model generates exactly one new token at position $S + t$ for each active sequence in the batch. Let $B_{\text{dec}}$ denote the number of concurrent decoding sequences. For each sequence, the model must evaluate all $N_{\text{params}}$ weights against a single activation vector of length $1$. Operations degenerate from compute-dense GEMM to memory-bound GEMV (matrix-vector multiplication).

Every single decode step requires streaming the entire model's $2 N_{\text{params}}$ bytes of weights from HBM, alongside the historical Key-Value (KV) cache memory $M_{\text{KV}}$ for all $B_{\text{dec}}$ sequences across all $L$ layers, yielding operational intensity $\mathcal{I}_{\text{decode}}(B_{\text{dec}}) = \frac{2 N_{\text{params}} B_{\text{dec}}}{2 N_{\text{params}} + M_{\text{KV}}}\text{ FLOP/byte}$, where $M_{\text{KV}} = 4 L H_{\text{KV}} d_h \sum_{i=1}^{B_{\text{dec}}} S_i\text{ bytes}$.[^fn-scheduling-intensity-divergence] Even if the historical KV cache were zero ($M_{\text{KV}} = 0$), the decode operational intensity is fundamentally capped by the batch size: $\mathcal{I}_{\text{decode}} \le \frac{2 N_{\text{params}} B_{\text{dec}}}{2 N_{\text{params}}} = B_{\text{dec}}\text{ FLOP/byte}$.

For typical serving batch sizes ($B_{\text{dec}} \in [1, 64]$), decode operational intensity satisfies $\mathcal{I}_{\text{decode}} \le 64\text{ FLOP/byte} \ll 295.2\text{ FLOP/byte}$. Autoregressive decoding is severely **memory-bandwidth bound**: accelerator Tensor Cores sit starved of data for over 75 percent to 90 percent of each clock cycle while the memory bus streams weights from HBM.
...
> **The Systems Rule of Thumb (The Arithmetic Intensity Divergence):** Prefill and decode occupy diametrically opposed hardware regimes: prompt prefill is compute-bound ($\mathcal{I}_{\text{prefill}} \approx S \ge 512\text{ FLOP/byte}$), while autoregressive decode is memory-bus bound ($\mathcal{I}_{\text{decode}} \le B_{\text{dec}} \le 64\text{ FLOP/byte} \ll \mathcal{I}_{\text{knee}}$). Co-scheduling an unchunked prefill alongside active decode streams causes severe phase contention: the compute-dense GEMM monopolizes accelerator streaming multiprocessors, serializing memory-bound decode iterations and inducing tail latency spikes of 150 to 200 milliseconds.

::: {#chk-vol3-11-roofline-phase-contention .callout-checkpoint title="The arithmetic intensity divergence and phase contention"}

Before examining chunked prefill and physical cluster disaggregation, verify your understanding of serving physics:

- [ ] **Machine ridge point**: calculate the ridge point $\mathcal{I}_{\text{knee}} = P_{\text{peak}} / B_{\text{mem}}$ for an accelerator with $989\text{ TFLOP/s}$ compute and $3.35\text{ TB/s}$ HBM bandwidth.
- [ ] **Operational intensity divergence**: explain why prompt prefill is compute-bound ($\mathcal{I}_{\text{prefill}} \approx S$) while autoregressive decode is memory-bandwidth bound ($\mathcal{I}_{\text{decode}} \le B_{\text{dec}}$).
- [ ] **Phase contention**: describe how co-locating an unchunked 64k prefill with active decodes on the same GPU destroys Inter-Token Latency (ITL) service-level objectives.

:::

[^fn-scheduling-intensity-divergence]: Total prefill operations equal $2 N_{\text{params}} S + 4 L d_{\text{model}} S^2$ FLOPs while streaming $2 N_{\text{params}}$ bytes of weights in half precision; for $S \ll N_{\text{params}} / (2 L d_{\text{model}})$, this simplifies to $\mathcal{I}_{\text{prefill}} \approx S$. Decode evaluates $2 N_{\text{params}} B_{\text{dec}}$ FLOPs while streaming all weights and historical Key-Value caches from HBM, capping operational intensity at $B_{\text{dec}}$.
...
Instead of executing a long prompt of length $S$ as a single, monolithic, atomic GEMM, the scheduler slices the prompt into a sequence of smaller, uniform token chunks of size $C$ (e.g., $C = 512$ or $C = 1024$ tokens). The total number of chunk iterations required to ingest the prompt is $K = \lceil S / C \rceil$.
...
The total token budget per iteration is constrained by a strict ceiling $T_{\text{budget}}$, satisfying $N_{\text{iter}}(t) = B_{\text{dec}}(t) + C_{\text{eff}}(t) \le T_{\text{budget}}$. By fixing $T_{\text{budget}}$ such that the total execution time of the hybrid batch matches the target Service-Level Objective for Inter-Token Latency ($\Delta t_{\text{iter}} \le SLO_{\text{ITL}}$), the runtime guarantees bounded latency jitter for active decode streams while steadily advancing prefill computation (@tbl-vol3-scheduling-chunked-piggybacking).
...
What is the optimal token budget $T_{\text{budget}}^*$ that maximizes accelerator throughput without violating an operator's SLA? Let $\tau_{\text{target}}$ denote the target inter-token latency bound (e.g., $\tau_{\text{target}} = 25\text{ ms}$). The execution duration of an iteration processing $B_{\text{dec}}$ decode tokens and $C$ prefill tokens is governed by the two-term Roofline formulation in @eq-chunk-iter-time:

$$
\Delta t_{\text{iter}}(B_{\text{dec}}, C) = \max \left( \frac{2 N_{\text{params}} + \text{Bytes}_{\text{KV}}(B_{\text{dec}})}{B_{\text{mem}}}, \; \frac{2 N_{\text{params}} (B_{\text{dec}} + C) + 4 L d_{\text{model}} C S_{\text{avg}}}{P_{\text{peak}}} \right) + \delta_{\text{launch}}
$$ {#eq-chunk-iter-time}

where $\delta_{\text{launch}}$ represents kernel launch overhead and synchronization latency (typically $0.1\text{--}0.3\text{ ms}$).

Notice the duality in @eq-chunk-iter-time: reading model weights requires a baseline memory transfer time $t_{\text{weights}} = 2 N_{\text{params}} / B_{\text{mem}}$ regardless of whether $C=0$ or $C>0$. For a 70B parameter model on an NVIDIA H100 ($B_{\text{mem}} = 3.35\text{ TB/s}$), streaming weights across the memory bus takes $t_{\text{weights}} = \frac{2 \times 70 \times 10^9\text{ bytes}}{3.35 \times 10^{12}\text{ bytes/s}} \approx 41.8\text{ ms}$.

During this $41.8\text{ ms}$ memory-read interval, the Tensor Cores are largely idle. Their theoretical arithmetic capacity during this time window is $P_{\text{peak}} \times t_{\text{weights}} = 2 N_{\text{params}} \mathcal{I}_{\text{knee}}$ FLOPs. A decode batch of $B_{\text{dec}}$ sequences only consumes $2 N_{\text{params}} B_{\text{dec}}$ FLOPs. The stranded arithmetic capacity available for prefill computation is therefore $\text{Stranded FLOPs} = 2 N_{\text{params}} ( \mathcal{I}_{\text{knee}} - B_{\text{dec}} )$.

Because each prefill token executes $2 N_{\text{params}}$ FLOPs in the linear projection and feed-forward layers, dividing stranded FLOPs by $2 N_{\text{params}}$ yields a remarkably clean **first-principles napkin rule**: $C^* \approx \mathcal{I}_{\text{knee}} - B_{\text{dec}}$. On an NVIDIA H100 with $\mathcal{I}_{\text{knee}} \approx 295\text{ FLOP/byte}$, if the engine is actively decoding a batch of $B_{\text{dec}} = 32$ streams, the scheduler can piggyback $C^* \approx 295 - 32 = 263\text{ prefill tokens}$ into the iteration for **zero marginal latency cost**. The prefill compute executes concurrently within the memory fetch window of the decode weights.

When factoring in the secondary self-attention computation ($4 L d_{\text{model}} C S_{\text{avg}}$ FLOPs across historical context length $S_{\text{avg}}$), the critical chunk threshold $C^*$ at which the iteration transitions from memory-bound to compute-bound evaluates to $C^* = \frac{2 N_{\text{params}} ( \mathcal{I}_{\text{knee}} - B_{\text{dec}} )}{2 N_{\text{params}} + 4 L d_{\text{model}} S_{\text{avg}}}$, where $\mathcal{I}_{\text{knee}} = P_{\text{peak}} / B_{\text{mem}}$ is the machine balance.[^fn-scheduling-chunk-derivation] For typical sequence lengths, this denominator reduces $C^*$ by only 5 percent to 15 percent. This derivation demonstrates why chunked prefill is so effective: rather than slowing down active users, it repurposes arithmetic cycles that would otherwise be permanently lost to memory bus stalls.
...
> **The Systems Rule of Thumb (The Piggybacking Invariant):** Because single-token autoregressive decoding is strictly memory-bus bound, accelerator tensor cores sit idle for tens of milliseconds per step awaiting parameter streaming ($t_{\text{weights}} \approx 41.8\text{ ms}$ for 70B on H100). Iteration piggybacking allows schedulers to inject up to $C^*$ prefill tokens per step without increasing inter-token latency by a single microsecond, turning stranded compute capacity into free prompt ingestion.

[^fn-scheduling-chunk-derivation]: Equating the memory streaming floor $2 N_{\text{params}} / B_{\text{mem}}$ with execution compute time $[2 N_{\text{params}} (B_{\text{dec}} + C^*) + 4 L d_{\text{model}} C^* S_{\text{avg}}] / P_{\text{peak}}$ and solving for $C^*$ yields the critical chunk threshold that fully saturates arithmetic units without delaying decode step completion.
...
While chunked prefill successfully bounds Inter-Token Latency jitter within a single GPU or tensor-parallel group, co-locating prefill and decode phases on the exact same physical silicon remains fundamentally suboptimal. Slicing prefills forces compute-dense GEMMs to interleave with memory-bound GEMVs across the same Streaming Multiprocessors, generating severe intra-core contention for L2 cache capacity and shared memory bandwidth. Furthermore, co-location forces both operational phases into a single, coupled Tensor Parallelism degree ($TP$), creating an unresolvable architectural deadlock: prompt prefill requires high tensor parallelism ($TP = 8$) to minimize Time-To-First-Token, while autoregressive decode throughput favors low tensor parallelism ($TP = 1$ or $TP = 2$) to eliminate latency-dominated cross-GPU all-reduce synchronization. To break this hardware gridlock and achieve true hardware specialization, the runtime must physically disaggregate prefill from decode across high-speed interconnects.

## Disaggregated Prefill-Decode Serving {#sec-vol3-scheduling-disaggregated}

Even with chunked prefill, a homogeneous accelerator cluster exhibits deep operational inefficiencies when serving multi-agent workflows. Consider a 64-GPU cluster deployed to serve an enterprise agentic coding platform. During the morning hours, hundreds of agents ingest large codebases, documentation indices, and compiler traces—a workload dominated by massive prefill requests demanding tens of petaflops of raw FP16/BF16 compute. In the afternoon, those same agents enter autonomous debugging loops, performing multi-step tool calls, emitting thought tokens, and evaluating small intermediate results—a decode-dominated workload demanding terabytes per second of aggregate memory bandwidth while leaving 85 percent of the Tensor Cores unutilized.
...
This fundamental mismatch led to the architecture of **Disaggregated Serving**\index{Disaggregated Serving} (pioneered in research by DistServe [@zhong2024distserve] and Splitwise [@patel2024], and validated at industrial scale by **Mooncake** [@qin2024mooncake] in the Kimi / Moonshot AI infrastructure). In Mooncake, the disaggregated runtime centers on a distributed **KVCache Store** that pools underutilized host DRAM, local NVMe SSDs, and remote GPU HBM across the cluster into a unified memory hierarchy, coordinating zero-copy RDMA transfers so that prefill and decode instances scale independently.
...
![](images/svg/prefill_decode_disaggregation_v2.svg){width="100%"}
:::
...
To quantify this challenge, consider the physical footprint of the KV cache for a context sequence of length $S$. In a transformer with $L$ layers, $H_{\text{KV}}$ key-value heads, per-head dimension $d_h$, and numerical precision of $b_{\text{elem}}$ bytes per element, total resident memory footprint is governed by $S_{\text{KV}}(S) = 2 L H_{\text{KV}} d_h S \cdot b_{\text{elem}}\text{ bytes}$. For the standard Llama-3-70B architecture ($L = 80, d_{\text{model}} = 8192, H_{\text{KV}} = 8, d_h = 128, b_{\text{elem}} = 2$) [@dubey2024llama], the KV cache memory density evaluates to $\sigma_{\text{KV}} = 2 \times 80 \times 8 \times 128 \times 2 = 327{,}680\text{ bytes/token} \approx 320\text{ KB/token}$. For an agent operating with an extended horizon of $S = 64\text{k} = 65{,}536$ tokens, resident context memory footprint reaches $S_{\text{KV}}(64\text{k}) \approx 21.47\text{ GB}$.

Transferring $21.47\text{ GB}$ of raw activation memory across a network interface introduces physical communication delay $t_{\text{transfer}} = \frac{S_{\text{KV}}}{B_{\text{net}}} + t_{\text{setup}}$, where $t_{\text{setup}}$ represents RDMA queue pair connection and memory registration overhead ($\approx 0.5\text{--}2\text{ ms}$).[^fn-scheduling-network-fabrics]

@Tbl-vol3-scheduling-network-fabrics evaluates transfer latency across four generations of data center network fabrics.

| Network Fabric Generation     | Physical Link Rate | Effective Bandwidth ($B_{\text{net}}$) | 64k KV Transfer Time ($21.47\text{ GB}$) | Time-to-First-Token Consequence                |
|:------------------------------|:------------------:|:--------------------------------------:|:----------------------------------------:|:-----------------------------------------------|
| **Standard Ethernet**         |      100 Gbps      |           $12.5\text{ GB/s}$           |  $1{,}718\text{ ms}$ ($1.72\text{ s}$)   | Catastrophic (completely wipes prefill gains)  |
| **RoCEv2 / InfiniBand (NDR)** |      400 Gbps      |            $50\text{ GB/s}$            |            $429.4\text{ ms}$             | Substantial delay (exceeds interactive target) |
| **InfiniBand (XDR / 2x NDR)** |      800 Gbps      |           $100\text{ GB/s}$            |            $214.7\text{ ms}$             | Bounded overhead                               |
| **Scale-Up NVLink Network**   |      3.6 Tbps      |           $450\text{ GB/s}$            |             $47.7\text{ ms}$             | Negligible (well within TTFT budget)           |

: **Interconnect Fabric Bandwidth and 64k KV Transfer Latency**: Physical transfer time and operational consequences of migrating a $21.47\text{ GB}$ context memory buffer across network generations. {#tbl-vol3-scheduling-network-fabrics tbl-colwidths="[25,18,18,20,19]"}

On standard 100 Gbps Ethernet, a $1.72\text{-second}$ transfer latency erases the speedup of running prefill on dedicated accelerator nodes and breaches interactive TTFT service-level objectives. Even on 400 Gbps RoCEv2 fabrics, raw bulk transfer adds over $400\text{ ms}$ of unmasked delay unless the runtime actively overlaps transmission with execution.

> **The Systems Rule of Thumb (The KV Network Invariant):** Disaggregated serving only preserves Time-to-First-Token advantages when network interconnect throughput exceeds the KV state footprint: $t_{\text{transfer}} \ll t_{\text{prefill}}$. Transferring a $64\text{k}$ KV cache ($21.47\text{ GB}$ on 70B models) across commodity 100 Gbps Ethernet introduces an intolerable $1.72\text{-second}$ delay that wipes out prefill speedups. Production disaggregation requires high-performance RDMA fabrics ($\ge 400\text{ Gbps}$) or pipelined layer streaming to keep transfer latency bounded below 50 milliseconds.

[^fn-scheduling-network-fabrics]: Effective bandwidth assumes unconstrained line-rate transfers without queue congestion or packet drops. In multi-tenant enterprise clusters, incast congestion and switch buffer oversubscription can inflate tail transfer times by $2\times$ to $5\times$.
...
Rather than waiting for the entire forward pass across all $L$ layers to conclude before opening network sockets, the $P$-node initiates an asynchronous RDMA write into the $D$-node's pre-allocated HBM buffer as soon as layer $l$'s Key and Value tensors are produced.

Let $\tau_{\text{compute}}(l) \approx (2 N_{\text{params, layer}} S + 4 H d_h S^2) / P_{\text{eff}}$ denote the computation duration of layer $l$, and let $\tau_{\text{transfer}}(l) = (2 H_{\text{KV}} d_h S \cdot b_{\text{elem}}) / B_{\text{RDMA}}$ denote the network transmission duration of layer $l$'s Key-Value activations.

::: {.callout-note title="Lemma 11.1: The KV streaming invariant"}
...
$$
\tau_{\text{transfer}}(l) \le \tau_{\text{compute}}(l+1) \quad \forall l \in \{1, \dots, L-1\}
$$ {#eq-kv-streaming-invariant}

then all intermediate network transfer latency is completely hidden behind forward-pass compute. The effective transfer penalty observed by the end-user collapses to the transmission duration of the final layer $L$: $\Delta t_{\text{visible}} = \tau_{\text{transfer}}(L) = \frac{S_{\text{KV}}}{L \cdot B_{\text{RDMA}}}$.
:::

For Llama-3-70B with an extended context of $S = 64\text{k}$ across a 400 Gbps RDMA interconnect ($B_{\text{RDMA}} = 50\text{ GB/s}$), this evaluates to $\Delta t_{\text{visible}} = 21.47\text{ GB} / (80 \times 50\text{ GB/s}) \approx 5.37\text{ ms}$.[^fn-scheduling-streaming-overlap]
...
> **The Systems Rule of Thumb (The Pipelined Streaming Invariant):** Asynchronous layer-by-layer KV streaming hides network latency behind forward-pass compute whenever layer transmission time is less than subsequent layer compute time ($\tau_{\text{transfer}}(l) \le \tau_{\text{compute}}(l+1)$). When satisfied, visible transfer latency collapses from the entire context footprint down to a single layer ($S_{\text{KV}} / (L \cdot B_{\text{RDMA}}) \approx 5.4\text{ ms}$ for 64k context on 400 Gbps RDMA), completely neutralizing the disaggregation network tax.

[^fn-scheduling-streaming-overlap]: While layer $l+1$ executes forward-pass matrix multiplications, layer $l$'s Key-Value activations are transmitted concurrently over direct memory access (DMA) channels. Because network transmission finishes before subsequent layer computation concludes, only the terminal layer's transfer remains unoverlapped.

Physical disaggregation specializes hardware nodes and neutralizes the KV transfer tax, but physical specialization alone cannot protect cluster stability against workload heterogeneity. When incoming requests originate from diverse agent classes—ranging from low-latency interactive developer completions to multi-hour autonomous debugging loops—a naive cluster control plane suffers from catastrophic queueing interference. Even on physically decoupled pools, admitting long-running, unpredictable reasoning trajectories into unweighted FIFO queues paralyzes the prefill and decode dispatch pipelines. To preserve Quality of Service across the cluster, the runtime must formalize decoupled Service-Level Objectives and deploy priority-aware queueing disciplines grounded in the mathematical realities of heavy-tailed workloads.

## Swarm Priority Scheduling {#sec-vol3-scheduling-priority}
...
In traditional web microservices, performance is summarized by a single scalar metric: total end-to-end request response latency. In agentic large language model (LLM) serving, single-scalar latency is structurally inadequate because inference is an incremental, autoregressive process. The serving runtime must enforce two decoupled **Service-Level Objectives (SLOs)**\index{Service-Level Objectives (SLOs)}:

1. **Time-To-First-Token ($SLO_{\text{TTFT}}$):** The elapsed wall-clock time from when the agent submits a prompt $\mathbf{x}$ to when the serving system emits the initial generated token $y_1$ ($TTFT = t_{\text{first\_token}} - t_{\text{arrival}} \le SLO_{\text{TTFT}}$). TTFT is governed by queue waiting time, prompt prefill latency, and (in disaggregated systems) KV transfer time.
2. **Inter-Token Latency ($SLO_{\text{ITL}}$):** The time elapsed between the emission of consecutive generated tokens ($ITL_k = t_{k} - t_{k-1} \le SLO_{\text{ITL}}$ for all $k \in \{2, \dots, N_{\text{out}}\}$). ITL is governed by the iteration forward-pass duration of the decode engine.

To capture whether an agent's execution was actually useful, systems engineers define **Goodput**: $\mathcal{G} = \frac{1}{T_{\text{total}}} \sum_{i=1}^{N_{\text{req}}} \mathbf{1}( TTFT_i \le SLO_{\text{TTFT}} \land \max_k ITL_{i,k} \le SLO_{\text{ITL}} )\text{ requests/s}$.
...
Let $X$ denote the service time (measured in total generated tokens or forward-pass execution steps) of an agent trajectory. While conversational chatbot queries exhibit bounded variance, autonomous agent execution steps follow a heavy-tailed Pareto distribution: $P(X > x) = (x_m / x)^\alpha$ for $x \ge x_m$, where $x_m > 0$ is the minimum task horizon (e.g., $x_m = 2$ steps) and $\alpha > 0$ is the Pareto tail index.

For $\alpha > 1$, the first moment (mean service time) converges to $\mathbb{E}[X] = \alpha x_m / (\alpha - 1)$. However, when $\alpha \le 2$, the second raw moment $\mathbb{E}[X^2]$ diverges to infinity.[^fn-scheduling-pareto-variance] Empirical measurements across production agent fleets (such as SWE-bench execution traces and autonomous web-browsing agents) reveal tail indices strictly within the heavy-tailed band $\alpha \in [1.1, 1.4]$. The workload exhibits **infinite theoretical variance**.

**The mathematical collapse of FIFO queueing:**
In an inference serving engine, batch slots and KV cache page frames are finite physical resources. When requests are scheduled in strict First-In-First-Out (FIFO) order, an incoming task cannot begin execution until preceding tasks yield their slots. If service times have low variance (such as standard chatbot queries requiring 200–400 tokens), FIFO performs stably. But when service times follow a heavy-tailed distribution, catastrophic Head-of-Line (HoL) blocking emerges.

Consider an inference serving node modeled as an $M/G/1$ queue with Poisson arrival rate $\lambda$, arbitrary service time distribution $X$, and server utilization $\rho = \lambda \mathbb{E}[X] < 1$. By the classical Pollaczek-Khinchine formula [@kleinrock1975queueing; see @sec-vol3-appendix-math-queueing], the expected waiting time in the queue is $\mathbb{E}[W_{\text{FIFO}}] = \frac{\lambda \mathbb{E}[X^2]}{2(1 - \rho)}$.

Notice the critical vulnerability in the Pollaczek-Khinchine formula: expected waiting time scales not with the average task duration $\mathbb{E}[X]$, but with the second raw moment of service time $\mathbb{E}[X^2]$. For empirical agent workloads where $\alpha \in [1.1, 1.4]$, the second moment diverges to infinity ($\mathbb{E}[X^2] \to \infty$; see proof in @sec-vol3-appendix-math-pareto).[^fn-scheduling-pareto-variance]

This divergence is the mathematical root cause of real-world cluster freezes. In an actual GPU cluster, when a long-running 300-step agent enters the server, it seizes an active decode slot for minutes. Because the tail is heavy, dozens of quick 2-step verification requests back up in the external queue behind it. Even when average GPU utilization looks modest ($\rho \approx 0.6$), average queue latency explodes toward infinity as long-horizon outliers monopolize the serving slots. FIFO is structurally broken for multi-turn agent systems.

> **The Systems Rule of Thumb (The Heavy-Tailed Queueing Catastrophe):** Because autonomous agent trajectories exhibit heavy-tailed Pareto execution horizons with shape parameter $\alpha \in [1.1, 1.4]$, workload service times have divergent variance ($\mathbb{E}[X^2] \to \infty$). Under standard First-In-First-Out (FIFO) queueing, the Pollaczek-Khinchine formula guarantees infinite average waiting times ($\mathbb{E}[W_{\text{FIFO}}] \to \infty$) as long-running reasoning trajectories block the queue. Serving runtimes must never schedule heterogeneous agent workloads via FIFO; non-clairvoyant prioritization or preemptive multi-level feedback queues are strictly required.

[^fn-scheduling-pareto-variance]: For the complete mathematical derivation of divergent second moments under Pareto distributions and its queueing implications, see @sec-vol3-appendix-math-pareto and @sec-vol3-appendix-math-queueing.

### Least attained service (LAS) scheduling

In classical operating systems, when task durations are known in advance, **shortest remaining processing time** (SRPT) is provably optimal for minimizing mean response time. In autonomous agent serving, however, remaining processing time is fundamentally unknown. An agent is a stochastic, closed-loop process; whether it will resolve its task on the next turn or require 50 more turns depends on the unpredictable return values of external tools. Clairvoyant SRPT cannot be implemented.
...
The hazard rate $h(x) = f(x) / [1 - F(x)]$ represents the conditional probability density that a task will finish in the next infinitesimal interval, given that it has already executed for duration $x$. For a Pareto distribution with tail index $\alpha > 0$, the hazard rate evaluates to $h(x) = \alpha / x$, yielding a strictly negative derivative: $\frac{d}{dx} h(x) = -\frac{\alpha}{x^2} < 0$ for all $x > 0$, where $h(x)$ decreases monotonically with attained service $x$.[^fn-scheduling-decreasing-hazard]

The physical implication of this decreasing hazard rate directly reflects real-world engineering experience: **the longer an agent has been running, the less likely it is to complete in the next step**. Think of an agent debugging a software repository:

- On turn 1, it may simply answer a factual query or fix a syntax typo; its instantaneous probability of terminating on the next turn is high ($h(1) = \alpha$).
- On turn 100, that same agent has encountered repeated compiler errors, test failures, or web timeouts; it has branched into a deep troubleshooting loop. Its instantaneous probability of terminating on the next step has plummeted by a factor of 100 ($h(100) = \alpha / 100$).

Because remaining execution time stochastically increases with attained service, non-clairvoyant schedulers must always give priority to younger tasks over older ones. To minimize mean response time under decreasing hazard rates, the mathematically optimal non-clairvoyant discipline is **Least Attained Service (LAS)**\index{Least Attained Service} (also known as Feedback or Foreground-Background scheduling).
...
> **The Systems Rule of Thumb (The Decreasing Hazard Principle):** For heavy-tailed agent workloads, the longer an agent has executed, the lower its instantaneous probability of completion ($d h(x) / dx = -\alpha / x^2 < 0$). Least Attained Service (LAS) scheduling minimizes mean cluster completion time by continuously giving priority to newly spawned tasks over aging, compute-heavy trajectories, preventing outlier agents from starving interactive workloads.

[^fn-scheduling-decreasing-hazard]: For Pareto density $f(x) = \alpha x_m^\alpha x^{-(\alpha + 1)}$ and survival function $1 - F(x) = (x_m / x)^\alpha$, the quotient simplifies directly to $h(x) = \alpha / x$. The strictly negative derivative confirms that remaining lifetime stochastically increases with attained service.
...
The MLFQ architecture partitions pending and active agent contexts across $K$ priority queues $\mathcal{Q} = \{Q_0, Q_1, \dots, Q_{K-1}\}$, governed by an increasing sequence of service quantum thresholds $0 = T_0 < T_1 < T_2 < \dots < T_K = \infty$.
...
> **The Systems Rule of Thumb (The MLFQ Demotion Invariant):** Multi-Level Feedback Queues discretize Least Attained Service without incurring continuous preemption overhead. By admitting all new agent tasks into highest priority ($Q_0$) and demoting them across service quantum thresholds ($s \ge T_{i+1}$), interactive developer requests complete with near-zero queueing latency while long-running reasoning loops are gracefully isolated into lower-priority queues without starving the cluster.

::: {#chk-vol3-11-pareto-mlfq-scheduling .callout-checkpoint title="Heavy-tailed Pareto scheduling and MLFQ"}

Before examining cache-aware prefix routing and distributed load balancing, verify your understanding of multi-turn scheduling:

- [ ] **Decreasing hazard rate**: explain why agent trajectories governed by a Pareto distribution exhibit a decreasing hazard rate ($d h(x) / dx < 0$), and why this makes FIFO scheduling vulnerable to queue collapse.
- [ ] **MLFQ priority demotion**: how does a Multi-Level Feedback Queue isolate long-running background reasoning loops without starving interactive developer suggestions?
- [ ] **Decoupled SLOs and Goodput**: explain why evaluating agent serving with a single end-to-end latency metric hides critical failures, and formulate how the goodput metric captures both TTFT and ITL compliance.

:::

While Multi-Level Feedback Queues govern the temporal prioritization of compute across heavy-tailed agent horizons, scheduling compute across time resolves only half the cluster optimization challenge. When dozens of agents in an autonomous swarm collaborate on a shared code repository or tool ecosystem, their prompts contain vast, overlapping token sequences. How should the cluster orchestrator route requests spatially across distributed nodes to eliminate redundant prefill computation without causing memory hot-spotting? This requires cache-aware load balancing and prefix routing.

## Cache-Aware Prefix Routing {#sec-vol3-scheduling-prefix-routing}
...
Out of each agent's $40\text{k}$-token prompt, $36\text{k}$ tokens (90 percent) are completely identical across all 40 agents!

If an uncoordinated cluster router (such as a standard Round-Robin, Random, or Least-Connections proxy) distributes these 40 incoming requests across 8 independent GPU serving nodes, every single node independently evaluates and stores the exact same $36\text{k}$-token prefix, wasting $8 \times 36{,}000 = 288{,}000$ prefill tokens across the cluster. Translating this into hardware waste on an 8-node H100 cluster running Llama-3-70B:
...
To avoid transmitting full multi-thousand-token prompts to the router for string comparison, prompts are fingerprinted using rolling chunk hashes. The token sequence $\mathbf{x} = (x_1, \dots, x_S)$ is partitioned into chunks of $B_{\text{chunk}} = 16$ tokens ($c_k = (x_{(k-1)B_{\text{chunk}} + 1}, \dots, x_{k B_{\text{chunk}}})$), with recursive Merkle chunk hashes computed via $h_k = \text{Hash}(h_{k-1} \parallel c_k)$ from initial vector $h_0 = \text{IV}$.
...
$$ {#eq-vol3-scheduling-cache-routing}

where $\mu \in [0, 1]$ is the cache affinity coefficient ($\mu = 1$ is pure cache affinity, $\mu = 0$ is pure load-balancing) and $\gamma \ge 1$ is the load penalty exponent.[^fn-scheduling-affinity-routing]

To see how this equation behaves in practice, consider an incoming prompt with $S = 16\text{k}$ tokens in a cluster configured with $\mu = 0.7$ and $\gamma = 2$:

- **Node A** holds $12\text{k}$ cached prefix tokens ($\text{MatchTokens} / S = 0.75$) and operates at 70 percent memory load ($L_A / L_{\text{max}} = 0.70$), yielding $\text{Score}_A = 0.70 \times 0.75 - 0.30 \times (0.70)^2 = 0.525 - 0.147 = +0.378$.
- **Node B** is completely cold ($\text{MatchTokens} = 0$) but lightly loaded ($L_B / L_{\text{max}} = 0.20$), yielding $\text{Score}_B = 0.70 \times 0.00 - 0.30 \times (0.20)^2 = 0.000 - 0.012 = -0.012$.

Node A easily wins the request, sparing the cluster from recomputing $12\text{k}$ tokens ($3.8\text{ GB}$ of KV cache on a 70B model). However, if Node A's memory fills to 95 percent load ($L_A / L_{\text{max}} = 0.95$), its score drops to $0.525 - 0.30 \times (0.95)^2 = +0.254$.

The quadratic exponent $\gamma = 2$ acts like an elastic barrier: below 70 percent capacity, the load penalty is gentle, allowing prefix affinity to steer traffic freely; but as memory approaches 90 to 95 percent saturation, the penalty escalates steeply, deflecting new requests to prevent GPU out-of-memory (OOM) evictions and cache thrashing.

> **The Systems Rule of Thumb (The Affinity-Load Balance Principle):** Routing multi-agent traffic purely on prefix cache hits ($\mu = 1$) triggers devastating queue hot-spotting, while naive round-robin routing ($\mu = 0$) wastes terabytes of GPU memory recomputing identical system prefixes. Schedulers must evaluate a joint objective balancing matched prefix tokens against memory utilization with non-linear saturation penalties ($\gamma \ge 2$), ensuring high KV cache hit rates without stranding peer accelerators.

[^fn-scheduling-affinity-routing]: The non-linear load penalty exponent ($\gamma \in [2, 3]$) creates steep deflection away from worker nodes operating above 85 percent memory capacity, preventing queue spillover and avoiding expensive GPU out-of-memory preemption.
...
To enforce load bounds, let $\bar{L} = \frac{1}{M} \sum_{j=1}^M L_j$ denote the average cluster load, and define an allowed load ceiling $L_{\text{cap}} = (1 + \epsilon) \bar{L}$, where $\epsilon > 0$ is a tolerance parameter (typically $\epsilon \in [0.15, 0.30]$).
...
Prefix-aware routing and bounded consistent hashing maximize spatial KV cache reuse, eliminating terabytes of redundant prefill computation across the cluster. Yet, spatial routing alone cannot mitigate the temporal holding cost of agent execution. When an autonomous agent transitions from autoregressive token generation to external tool execution—waiting seconds for unit tests, minutes for container builds, or hours for human authorization—its accumulated multi-gigabyte KV cache sits completely dormant in physical accelerator memory. If hundreds of concurrent agents pin their context frames during these prolonged latency stalls, accelerator HBM is completely exhausted, and cluster throughput collapses to near zero. Resolving this crisis requires transforming the runtime into a true distributed operating system: formalizing trajectory state via the Agent Control Block and managing physical memory through analytical breakeven swapping and multi-resource fairness.
...
While this bash command executes in a sandboxed container, what happens to the agent's KV cache on the GPU accelerator? The agent has accumulated an active context of $48\text{k}$ tokens. On a 70B parameter model with Grouped-Query Attention, that $48\text{k}$-token context consumes $S_{\text{KV}} = 48{,}000 \times 327{,}680\text{ bytes} \approx 15.73\text{ GB}$ of HBM.

If the serving runtime naively holds this context resident in physical GPU memory during the 45-second tool wait, that $15.73\text{ GB}$ of high-bandwidth memory sits completely dormant. Multiply this by 20 concurrent agents running similar workflows, and over $314\text{ GB}$ of HBM—representing four entire NVIDIA H100 GPUs—is locked up in total idleness. Active decoding agents cannot allocate memory blocks; queue lengths explode; and cluster compute utilization collapses from 85 percent to below 5 percent.
...
Just as the Linux kernel encapsulates an executing process using `struct task_struct` to manage CPU registers, virtual memory page tables, file descriptors, and scheduling state [@ritchie1974], an autonomous agent serving runtime requires a persistent, strongly typed kernel data structure to govern multi-turn execution: the **Agent Control Block (ACB)**\index{Agent Control Block}.

::: {#dfn-acb .callout-definition title="Definition 11.1: Agent Control Block (ACB)"}
...
The structural divergence between classical operating system descriptors and the Agent Control Block is contrasted in @fig-vol3-scheduling-pcb-vs-acb.

::: {#fig-vol3-scheduling-pcb-vs-acb fig-env="figure" fig-pos="htb" fig-cap="**Architectural Duality between POSIX Process Control Block and Agent Control Block**: Structural comparison of execution descriptors across classical operating systems and autonomous machine learning runtimes. The POSIX PCB (left, slate) encapsulates deterministic CPU execution state (registers, MMU page tables, file descriptors) enabling microsecond context switching. In contrast, the Agent Control Block (right, purple) encapsulates stochastic trajectory state across distributed accelerator clusters: tracking prompt token working sets, PagedAttention block tables, attenuating cryptographic capability leases, and causal vector clocks required for asynchronous tool-wait suspension and state hydration." fig-alt="Side-by-side comparison cards illustrating the POSIX Process Control Block on the left and the Agent Control Block on the right, contrasting registers and page tables with capability leases and PagedAttention memory handles."}
![](images/svg/pcb_vs_acb.svg)
:::
...
- **Strategy B (Preemptive Swapping):** Evict the KV cache across the PCIe/Compute Express Link (CXL) bus into host DDR5 DRAM, and swap it back into HBM when the tool completes.
...
Let $S_{\text{KV}}$ denote the size of the agent's KV cache (in bytes), and let $B_{\text{PCIe}}$ denote the effective unidirectional DMA bandwidth over the host-to-device interconnect (e.g., PCIe Gen5 x16: theoretical $64\text{ GB/s}$, effective DMA $\approx 55\text{ GB/s}$). Round-trip context migration duration comprises swap-out to host DRAM, swap-in back to HBM, and DMA queue setup latencies $\delta_{\text{setup}}$: $T_{\text{swap\_overhead}} = 2 S_{\text{KV}} / B_{\text{PCIe}} + 2\delta_{\text{setup}}$.

Holding memory pinned in HBM during tool waits of duration $T_{\text{wait}}$ strands accelerator capital and memory capacity ($\text{Cost}_{\text{pin}} = S_{\text{KV}} \times T_{\text{wait}} \times C_{\text{HBM}}$). Swapping yields net positive cluster throughput whenever tool wait duration satisfies $T_{\text{wait}} \ge T_{\text{breakeven}}$, governed by @eq-breakeven-formula:

$$
T_{\text{breakeven}} = \frac{2 S_{\text{KV}}}{B_{\text{PCIe}}} + 2\delta_{\text{setup}} + \Delta t_{\text{slack}}
$$ {#eq-breakeven-formula}

where $\Delta t_{\text{slack}}$ represents safety scheduling margin (typically $100\text{ ms}$).[^fn-scheduling-breakeven-derivation]

Now compare swapping against recomputation (Strategy C). Recomputing the prefill from scratch requires time $T_{\text{recompute}} \approx (2 N_{\text{params}} S + 4 L d_{\text{model}} S^2) / P_{\text{eff}}$. Swapping to host DRAM is strictly faster than recomputing whenever $\sigma_{\text{KV}} / B_{\text{PCIe}} < N_{\text{params}} / P_{\text{eff}}$.

Consider an autonomous software engineering agent running Llama-3-70B on an NVIDIA H100 GPU over PCIe Gen5:

- Context length: $S = 48\text{k} = 49{,}152$ tokens
- KV cache size: $S_{\text{KV}} = 49{,}152 \times 327{,}680\text{ bytes} \approx 16.1\text{ GB}$
- Effective PCIe Gen5 DMA bandwidth: $B_{\text{PCIe}} = 55\text{ GB/s}$, with setup latency $\delta_{\text{setup}} \approx 3\text{ ms}$
- Recomputation throughput ($P_{\text{eff}} \approx 400\text{ TFLOPS}$): $T_{\text{recompute}} \approx (2 \times 70 \times 10^9 \times 49{,}152) / (400 \times 10^{12}) \approx 17.2\text{ seconds}$
- Round-trip swap overhead: $T_{\text{swap\_overhead}} = (2 \times 16.1\text{ GB}) / (55\text{ GB/s}) + 0.006\text{ s} \approx 591\text{ ms}$

With scheduling slack $\Delta t_{\text{slack}} = 100\text{ ms}$, @eq-breakeven-formula yields $T_{\text{breakeven}} = 591\text{ ms} + 100\text{ ms} \approx 0.7\text{ seconds}$.

This derivation reveals a critical operational threshold: **whenever an external tool call is anticipated to block for longer than $0.7\text{ seconds}$, the runtime must immediately evict the KV cache to host DRAM** (@fig-vol3-scheduling-swapping). Because software compilations, web requests, and database queries routinely take 2 to 60 seconds ($T_{\text{wait}} \gg 0.7\text{ s}$), preemptive swapping frees $16.1\text{ GB}$ of HBM for 95 percent of the agent's lifetime, boosting cluster concurrency by up to $8\times$. Furthermore, swapping back in takes only $295\text{ ms}$—which is $58\times$ faster than recomputing the prefill from scratch ($17.2\text{ seconds}$).

> **The Systems Rule of Thumb (The Breakeven Swapping Threshold):** Accelerator memory managers must decouple tool-wait suspension from physical memory residency. Whenever an external tool call or human interaction is anticipated to block for longer than $T_{\text{breakeven}} = 2 S_{\text{KV}} / B_{\text{PCIe}} + 2\delta_{\text{setup}} + \Delta t_{\text{slack}} \approx 0.7\text{ seconds}$ (for $48\text{k}$ context on PCIe Gen5), runtimes must preemptively DMA-swap the KV cache to host DRAM, reclaiming gigabytes of high-bandwidth memory at a $58\times$ latency advantage over recomputation.

[^fn-scheduling-breakeven-derivation]: Migration overhead accounts for unidirectional DMA push to host DDR5 DRAM, subsequent pull back to HBM, and driver registration delays. Holding pinned HBM throughout prolonged tool waits wastes $S_{\text{KV}} \cdot T_{\text{wait}} \cdot C_{\text{HBM}}$ byte-seconds of high-bandwidth memory capacity.
...
Static resource partitioning or single-scalar fair sharing (such as token-based fair share) leads to resource starvation and cluster inefficiency. Consider what happens if an operator enforces naive 50/50 fair sharing based on GPU compute alone across two tenant classes:

1. **Coding Agents (Class A):** If allocated 50 percent of cluster compute ($4000\text{ TFLOPS}$), each instance needs only $40\text{ TF}$, permitting up to $100$ instances. But 100 instances require $100 \times 32\text{ GB} = 3200\text{ GB}$ of Host DRAM—over $3\times$ the cluster's entire $1024\text{ GB}$ memory capacity! The cluster crashes with out-of-memory errors while Tensor Cores sit underutilized.
2. **Reasoning Agents (Class B):** Conversely, if partitioned 50/50 by Host DRAM alone ($512\text{ GB}$ each), Class B could launch $512 / 8 = 64$ instances, demanding $64 \times 250\text{ TF} = 16{,}000\text{ TFLOPS}$—double the physical compute capacity of the entire GPU fleet!

Single-resource fairness fails because different agent workloads bottleneck on fundamentally different physical hardware tiers. Coding agents bottleneck on memory (HBM and host DRAM for deep context histories), whereas reasoning agents bottleneck on arithmetic compute (Tensor Cores for tree search and verification).

To resolve multi-resource contention without stranding hardware capacity, the agent runtime must implement **Dominant Resource Fairness (DRF)** [@ghodsi2011].

::: {#dfn-drf .callout-definition title="Definition 11.2: Dominant resource fairness (DRF)"}
Let $\mathcal{R} = \{1, \dots, m\}$ denote the set of $m$ shared physical resources with total cluster capacities $\mathbf{R} = [R_1, \dots, R_m]^T$. Let $\mathcal{A} = \{1, \dots, n\}$ denote $n$ active agent tenants. Tenant $i$ requires a resource demand vector $\mathbf{c}_i = [c_{i,1}, \dots, c_{i,m}]^T$ to execute one unit trajectory, yielding normalized resource share $s_{i,r} = c_{i,r} / R_r$.

Tenant $i$'s **dominant resource** is the resource for which it claims the largest share of cluster capacity ($r_i^* = \arg\max_{r \in \mathcal{R}} s_{i,r}$), with dominant share $s_i^* = \max_{r \in \mathcal{R}} s_{i,r}$.

Dominant Resource Fairness equalizes dominant shares across all active tenants via the max-min allocation: $\max \min_{i \in \mathcal{A}} s_i^*$ subject to $\sum_{i=1}^n N_i \mathbf{c}_i \le \mathbf{R}$, where $N_i$ denotes the allocated concurrency for tenant $i$.
:::

DRF guarantees four essential systems properties: **Sharing Incentive** (no tenant is worse off than under uniform partitioning $1/n$), **Strategy-Proofness** (no tenant can game the system by misrepresenting its demands), **Envy-Freeness** (no tenant prefers another's allocation), and **Pareto Efficiency** (it is impossible to allocate more resources to an agent without reducing another's allocation).[^fn-scheduling-drf-fairness]
...
![](images/svg/drf_multi_resource_agent_allocation_v2.svg)
:::

> **The Systems Rule of Thumb (The Multi-Resource Dominant Share Law):** Single-scalar token accounting fails in agentic clusters because workloads simultaneously bottleneck on disparate physical tiers: memory-dense agents saturate HBM while compute-dense agents saturate tensor cores. Dominant Resource Fairness equalizes each tenant's maximum normalized resource share ($\max \min_i s_i^*$), preventing high-memory agents from starving high-compute workers and maximizing aggregate cluster concurrency.

[^fn-scheduling-drf-fairness]: Dominant share equalization ($s_1^* = s_2^* = \dots = s_n^*$) ensures Pareto efficiency, sharing incentive, strategy-proofness, and envy-freeness across multi-resource clusters.
...
To guarantee forward progress, the runtime kernel enforces **Banker's Algorithm** [@dijkstra1965] on context memory allocation: an allocation is admitted if and only if $\text{Free Blocks} \ge \sum_{i \in \text{Active}} \text{MaxRemainingTokens}(i) + \Delta_{\text{safety}}$. If admitting a child subagent would violate this safe-state condition, admission is gated, and the requesting parent trajectory is forced to swap its context to host DRAM before spawning its child.

By enforcing Banker's Algorithm on shared memory pools, establishing analytical breakeven boundaries for PCIe DMA swapping, and equalizing dominant shares across heterogeneous hardware tiers, the cluster runtime maintains mathematical guarantees of forward progress and memory safety. Yet, in production deployments, engineers frequently bypass these operating system primitives in favor of intuitive, naive heuristics—such as pinning memory during tool waits, assuming FIFO fairness under heavy-tailed workloads, or confusing local GPU capacity with distributed routing topology. In the next section, we identify and dismantle the pervasive architectural fallacies and operational pitfalls that undermine cluster stability in agentic serving infrastructure.
...
As derived in @sec-vol3-scheduling-priority, when $\alpha \le 2$, the theoretical variance of task service times diverges to infinity ($\mathbb{E}[X^2] \to \infty$). Under the Pollaczek-Khinchine formula, the expected queue waiting time under FIFO is $\mathbb{E}[W_{\text{FIFO}}] = \frac{\lambda \mathbb{E}[X^2]}{2(1 - \rho)} \to \infty$.
...
1. **Intra-Core Memory Bus Contention:** Slicing a prefill into chunks still executes compute-dense GEMMs alongside memory-bound GEMVs on the same Streaming Multiprocessors. The prefill kernels flood the L2 cache and shared memory controllers, reducing effective memory bandwidth for concurrent decode streams by 25 to 40 percent.
...
The entire cluster livelocks. Runtimes must enforce strict monotonic delegation limits ($D_{\text{depth}} \le 3$), global memory headroom reserves via Banker's Algorithm, and mandatory context swapping before subagent delegation.
...
- As demonstrated in @sec-vol3-scheduling-prefix-routing, a cluster of 8 nodes serving 40 agents with an identical $36\text{k}$-token prefix wastes $92\text{ GB}$ of HBM storing redundant state, while each node achieves an effective cache hit rate below 25 percent.
- Implementing cache-aware prefix routing with consistent hashing concentrates identical prefixes on designated affinity nodes, boosting cluster cache hit rates to over 85 percent on existing hardware without purchasing an additional gigabyte of accelerator memory.
...
::: {#ws-scheduling-cluster-freeze .callout-war-story title="The 1.2 terabyte cluster memory freeze"}

**Context:** In late 2024, an enterprise software company deployed an autonomous coding fleet of 60 agents to automatically generate unit tests across a legacy microservices repository, served by an inference engine with continuous batching across 12 NVIDIA H100 GPUs (960 GB HBM).

**Mechanism:** An upstream network partition severed connectivity to an internal npm registry while all 60 agents concurrently executed `bash(npm test)`. The commands blocked indefinitely awaiting 15-minute TCP socket timeouts. Because the serving runtime lacked bimodal context swapping and Agent Control Block (ACB) descriptors, each blocked agent held its active $48\text{k}$-token context window pinned in physical HBM ($15.7\text{ GB}$ per instance), locking up over $940\text{ GB}$ of GPU memory.

**Impact:** Incoming high-priority developer requests found zero free PagedAttention blocks. Memory allocation threads spun at 100 percent CPU in a catastrophic Coffman livelock, Kubernetes node health checks failed, and the orchestrator initiated a cluster-wide rolling reboot storm that destroyed all in-flight trajectory states.

**Response:** Schedulers implemented asynchronous DMA context swapping to host DRAM triggered whenever an agent enters an out-of-band tool wait exceeding $T_{\text{breakeven}}$ (700 ms), releasing physical GPU page frames back to the allocator's free pool.

**Systems Lesson:** Never permit a stochastic agent process to hold physical accelerator frames across an unsandboxed external boundary. The absence of bimodal context swapping transforms an ordinary network latency hiccup into a total cluster infrastructure collapse.

:::

## Summary {#sec-vol3-scheduling-summary}

Trajectory scheduling and disaggregated serving transform machine learning infrastructure from stateless request-response engines into distributed operating systems optimized for stateful, asynchronous, multi-agent processes. By parameterizing the fundamental divergence between compute-bound prefill and memory-bandwidth-bound decode on the Roofline model, systems architects can eliminate phase interference through chunked prefill and physical cluster disaggregation. Abstracting trajectory identity into the Agent Control Block enables non-clairvoyant priority scheduling via Multi-Level Feedback Queues, overcoming the mathematical collapse of FIFO under heavy-tailed Pareto workloads. Finally, cache-aware prefix routing and analytical breakeven context swapping bridge the temporal chasm between microsecond token generation and minute-long tool waits, ensuring that scarce accelerator High-Bandwidth Memory is continuously utilized by active compute streams.
...
::: {.callout-chapter-connection title="From cluster scheduling to trajectory harvesting"}
This chapter completes Part III: The Actuation Boundary, Isolation & Control by establishing how accelerator compute, context memory, and hardware interconnects are multiplexed, scheduled, and disaggregated across autonomous agent fleets. Across Chapters 08 through 11, we have constructed the complete execution substrate: formalizing typed system calls and capability leases (@sec-vol3-actuation), isolating untrusted execution inside ephemeral microVM sandboxes (@sec-vol3-virtualization), handling asynchronous interrupts and human approval gates (@sec-vol3-interrupts), and orchestrating multi-agent compute under heavy-tailed queueing dynamics (this chapter).

Yet, even the most sophisticated cluster scheduler cannot compensate for a fragile, unadapted neural policy. Managing multi-turn execution trajectories through complex runtime scaffolding—paged attention memories, preemptive swapping engines, and extensive prompt engineering—is an expensive palliative for a foundation model that was never trained to act as an autonomous agent. When an un-adapted model encounters an unexpected tool error or an ambiguous environment observation, it rapidly enters pathological retry loops, burning cluster token budgets and driving up operational costs. To achieve true agentic reliability, systems architects must compile operational capabilities directly into the model's neural parameters.

In @sec-vol3-data-flywheel, we transition across the architectural boundary into Part IV: The Policy Compiler. We examine how the execution traces, sandbox verification signals, and scheduler telemetry captured across production clusters are harvested to build closed-loop synthetic data flywheels—generating, filtering, and curating massive corpora of verified trajectories through self-play, execution-based rejection sampling, and negative trace mining to train the next generation of autonomous policies.
:::
