...
The event log of @sec-vol3-interrupts made every step of a trajectory durable. Because the runtime derives its record from an append-only write-ahead log (WAL) and periodic snapshots, it survives host crashes and network disconnections and knows exactly which steps of a failed plan committed. Knowing which steps committed repairs nothing. Once the loop is closed (@sec-vol3-intro-operational-incident), a trajectory provisions cloud infrastructure, mutates production tables, issues payments, and dispatches notifications, and each of those effects lands in a system the runtime does not own. Over a long horizon, the authority a trajectory exercises becomes a recovery problem (@sec-vol3-intro-hsa), because every step spends some of it before the task is known to succeed. When a ten-step deployment fails at step seven, six external systems already hold its effects. None of them shares the runtime's log, none exposes a prepare phase, and none will roll back on request. Recovery here is operational rather than archival, and this chapter asks what contract the runtime owes the environment when a trajectory leaves it half mutated. The answer has three parts. The runtime compensates what can be compensated, repairs forward when repair costs less than unwinding, and orders the one action that cannot be undone after every step that can be.

For more than four decades, classical computer systems resolved the dilemma of partial failure through the transaction concept formalized by Jim Gray in 1981. Under classical database semantics, the twin guarantees of Atomicity and Isolation govern state mutation: either an entire batch of modifications commits successfully, or the runtime executes an abort, relying on private undo logs, shadow pages, or write locks to erase every intermediate write and render the aborted attempt completely invisible to concurrent observers. When transactions span multiple independent storage nodes, distributed systems deploy Two-Phase Commit ($2\text{PC}$) protocols, wherein a central coordinator establishes a prepare phase to force participants into a pre-committed, locked state before issuing a definitive, globally synchronized commit command. In an autonomous agent system, however, this classical transactional boundary completely collapses. Once the host supervisor validates an action proposal and dispatches it over an external network boundary (a code forge API that opens a pull request, a payment API that captures a charge, a cloud API that launches a compute cluster, or an internal webhook that alerts an engineering team), the agent runtime relinquishes administrative control. These remote endpoints do not share a common write-ahead log, do not expose internal database undo mechanisms, and refuse to participate in distributed Two-Phase Commit protocols. Every dispatched HTTP request commits immediately upon arrival at the remote server, instantly entering the durable state of the external service and becoming globally visible to third parties. No subsequent database `ROLLBACK` command can un-send a message broadcast to a client channel, un-provision a virtual machine that has begun accumulating billing charges, or un-merge a commit that has already triggered a production deployment pipeline.

Even if external service providers could be coerced into exposing distributed lock managers, classical transaction semantics would remain physically impossible due to the mathematical behavior of Long-Lived Transactions (LLTs). In conventional Online Transaction Processing (OLTP) engines, transactions acquire exclusive write locks ($X$-locks) and shared read locks ($S$-locks) over data rows for durations bounded by microseconds or milliseconds. Under strict two-phase locking (2PL), serializability is preserved because the duration of lock holding is limited by the physical access latencies of DRAM and solid-state drives. In sharp contrast, an autonomous agent trajectory is fundamentally high-latency, bursty, and open-ended. An agent executing an end-to-end task must interleave network tool calls with autoregressive decode loops and may suspend execution entirely while awaiting asynchronous human approval gates, unit test execution, or continuous integration passes. If the runtime attempted to maintain exclusive distributed locks across external resources for the lifespan of an agent trajectory, the probability of lock starvation and catastrophic distributed deadlock approaches certainty. As Gray showed, the probability of deadlock in a system with $N$ concurrent transactions grows as $O(N^2 \cdot L^4)$, where $L$ represents the number of actions executed per transaction. Multiplying transaction holding times from five milliseconds to ten minutes increases the deadlock frequency by orders of magnitude, causing systemic lock starvation that paralyzes the entire hosting infrastructure. Errors also compound over a long horizon (@sec-vol3-intro-temporal-stretching), so a model that misreads a network timeout at step three can issue invalid mutations at step four that further corrupt external state. As contrasted in @tbl-vol3-acid-vs-saga, classical database transactions rely on locks and undo logs, whereas agent systems require distributed saga semantics. Without strict transactional isolation, these corrupt intermediate states leak immediately into the surrounding environment, polluting concurrent workflows and triggering cascading failures across the enterprise.

| Dimension                  | Classical ACID Transactions ($2\text{PC}$)                              | Distributed Agent Saga Contracts                                |
|:---------------------------|:------------------------------------------------------------------------|:----------------------------------------------------------------|
| **Transactional Boundary** | Homogeneous, tightly coupled databases sharing lock managers            | Heterogeneous, autonomous external APIs and services            |
...
| **Authority Boundary**     | Trusted database kernel with complete ambient storage access            | Privileged runtime mediating untrusted model proposals          |
...
*Compensating unwind costs $17.6\times$ less than the forward path it reverses.*
:::
...
A long-lived, fault-tolerant sequence of discrete sub-transactions executed by an agent runtime where each forward action commits immediately and registers a corresponding compensating action on a LIFO recovery stack, ensuring semantic consistency upon failure without holding distributed locks.
:::
...
***Semantic compensation***\index{Semantic compensation!definition}\index{Scheduling!semantic compensation definition} is a forward-mutating recovery operation $C_i$ that transitions external environment state to a new state $\mathbf{S}_{i-1}'$ that is semantically equivalent to $\mathbf{S}_{i-1}$ with respect to core system invariants, rather than attempting physical rollback.

1.  **Significance**: Enables transactional fault recovery across non-invertible, distributed tool interactions (such as cloud provisioning, message publishing, or billing APIs) where classical physical rollbacks are physically impossible.
2.  **Distinction**: Unlike database rollback (which restores exact bit-level disk pages from an undo log), semantic compensation executes an active forward compensating transaction (such as deleting a provisioned pod or issuing an account credit) that restores invariant equilibrium despite leaving historical audit artifacts.
3.  **Common pitfall**: Authoring non-idempotent compensating operations that fail or produce orphaned duplicate side effects when retried across transient network disconnects.
...
The execution of a Trajectory Saga is managed by the *Saga Orchestrator*, an authoritative control plane component operating within the host runtime supervisor. The orchestrator maintains two primary data structures inside the Agent Control Block (ACB): the *Forward Execution Log* ($\mathcal{L}_{\text{fwd}}$) and the *LIFO Compensation Stack* ($\mathcal{S}_{\text{comp}}$), as mapped in @tbl-vol3-saga-orchestrator-state.
...
```{python}
#| label: lego-11-latency-cost-and-storage-overheads
#| echo: false
# │ Show: Saga log unwinding takes 295 ms vs 5,200 ms forward execution.
# │ How: Model fsync overhead, compensation steps, and sunk token costs.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_usd

class SagaLogUnwindingOverheads:
    """Saga log unwinding overheads and forward dead-loss accounting."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    t_1 = 120 * millisecond
    t_2 = 350 * millisecond
    t_3 = 4200 * millisecond
    t_4 = 80 * millisecond
    t_5 = 450 * millisecond

    t_fsync = 1.2 * millisecond
    t_fw = 10 * millisecond
    t_clean = 15 * millisecond
    t_release = 220 * millisecond
    t_teardown = 45 * millisecond

    n_input_tokens = 18_400 * token
    n_output_tokens = 1_250 * token
    price_input = 3.00 / 1_000_000
    price_output = 15.00 / 1_000_000

    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    t_fwd_exec = t_1 + t_2 + t_3 + t_4 + t_5
    n_fsync_fwd = 5 * 2
    t_io_fwd = n_fsync_fwd * t_fsync

    n_fsync_rev = 4 * 1
    t_io_rev = n_fsync_rev * t_fsync
    t_io_total = t_io_fwd + t_io_rev

    t_unwind = t_fw + t_clean + t_release + t_teardown + t_io_rev

    cost_tokens = (n_input_tokens.magnitude * price_input) + (n_output_tokens.magnitude * price_output)
    io_overhead_ratio = (t_io_total.to(second) / t_fwd_exec.to(second)).magnitude

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(abs(t_fwd_exec.to(millisecond).magnitude - 5200.0) < 1e-4, "t_fwd_exec")
    check(abs(t_io_fwd.to(millisecond).magnitude - 12.0) < 1e-4, "t_io_fwd")
    check(abs(t_io_rev.to(millisecond).magnitude - 4.8) < 1e-4, "t_io_rev")
    check(abs(t_io_total.to(millisecond).magnitude - 16.8) < 1e-4, "t_io_total")
    check(abs(t_unwind.to(millisecond).magnitude - 294.8) < 1e-4, "t_unwind")
    check(abs(cost_tokens - 0.07395) < 1e-5, "cost_tokens")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    t_fsync_ms_str = fmt_time(t_fsync, unit=millisecond, precision=1)
    t_fw_ms_str = fmt_time(t_fw, unit=millisecond, precision=0)
    t_clean_ms_str = fmt_time(t_clean, unit=millisecond, precision=0)
    t_release_ms_str = fmt_time(t_release, unit=millisecond, precision=0)
    t_teardown_ms_str = fmt_time(t_teardown, unit=millisecond, precision=0)

    n_fsync_fwd_str = fmt_int(n_fsync_fwd)
    t_io_fwd_ms_str = fmt_time(t_io_fwd, unit=millisecond, precision=0)
    n_fsync_rev_str = fmt_int(n_fsync_rev)
    t_io_rev_ms_str = fmt_time(t_io_rev, unit=millisecond, precision=1)
    t_io_total_ms_str = fmt_time(t_io_total, unit=millisecond, precision=1)
    io_overhead_pct_str = fmt_percent(0.003, precision=1, style="prose")

    t_unwind_ms_str = fmt_time(t_unwind, unit=millisecond, precision=1)
    t_unwind_round_ms_str = fmt_time(round(t_unwind.to(millisecond).magnitude) * millisecond, unit=millisecond, precision=0)
    cost_tokens_str = fmt_usd(cost_tokens, precision=3)
    t_fwd_exec_ms_str = fmt_time(t_fwd_exec, unit=millisecond, precision=0)
```

::: {#nbk-11-latency-cost-and-storage-overheads .callout-notebook title="Saga log unwinding overheads"}
...
1. *Durable I/O Overhead:* Each sub-transaction incurs two fsync operations (transition to `COMMITTED`, and recording $C_i$ on stack). The forward phase incurred:
   ...
   Total log persistence overhead is `{python} SagaLogUnwindingOverheads.t_io_total_ms_str`, representing less than `{python} SagaLogUnwindingOverheads.io_overhead_pct_str` of total trajectory wall-clock time.
...
3. *Financial Waste Accounting:*
   ...
   Because the Saga Orchestrator executed $C_2$ (releasing the static IPv4 reservation within `{python} SagaLogUnwindingOverheads.t_unwind_round_ms_str` of failure), cloud egress holding fees were truncated before incurring hourly quantization charges.

**Key Architectural Takeaway:** The physical time to unwind the failed trajectory (`{python} SagaLogUnwindingOverheads.t_unwind_ms_str`) is an order of magnitude smaller than the forward execution time (`{python} SagaLogUnwindingOverheads.t_fwd_exec_ms_str`). The Write-Ahead Logging overhead (`{python} SagaLogUnwindingOverheads.t_io_total_ms_str`) is negligible compared to network and compilation latencies. However, the `{python} SagaLogUnwindingOverheads.cost_tokens_str` of burned inference tokens and the forward wall-clock execution time represent an unrecoverable dead loss.
:::
...
::: {#chk-11-sagas-compensation .callout-checkpoint title="Evaluating trajectory sagas and compensating actions"}
Before analyzing forward self-healing policies and anti-spin invariants, verify your understanding of saga transactions:

- [ ] **ACID boundary collapse**: Why do distributed agent tool mutations across independent external systems lack native multi-phase commit or isolation?
- [ ] **Compensating semantics**: How does a backward compensator $C_i$ differ from a database rollback when intermediate side effects have already been observed externally?
- [ ] **Compensation stack**: Why must compensating actions be logged to durable storage in reverse topological order as forward steps complete?
- [ ] **Sunk cost trade-off**: Under what failure conditions is unwinding an entire saga stack economically and computationally wasteful compared to localized forward repair?
:::
...
```{python}
#| label: lego-11-quantitative-trade-off-between-forward-patching
#| echo: false
# │ Show: Forward repair cuts recovery latency by 91.5% and cost by 92.5%.
# │ How: Compare multi-step saga rollback against single-turn prompt repair.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_percent, fmt_time, fmt_usd

class ForwardPatchingVsSagaRollback:
    """Forward self-healing versus backward saga rollback trade-offs."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    t_prov = (4.2 + 18.5 + 1.1 + 9.8 + 12.4) * second
    t_rollback = (14.1 + 6.2 + 0.8 + 16.0 + 3.5) * second
    cost_sunk = 0.142

    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    t_re_exec = t_rollback + t_prov
    cost_re_exec = cost_sunk + cost_sunk

    # Scenario B
    n_prompt_tokens = (1200 + 850 + 340) * token
    n_gen_tokens = 180 * token
    ttft = 0.090 * second
    r_gen = 50 * (token / second)
    t_tool_repair = 0.4 * second
    p_success = 0.92

    t_model_repair = ttft + (n_gen_tokens.magnitude / r_gen.magnitude) * second
    cost_repair = (n_prompt_tokens.magnitude * (3.0 / 1_000_000)) + (n_gen_tokens.magnitude * (15.0 / 1_000_000))

    t_expected_forward = t_model_repair + t_tool_repair + (1 - p_success) * t_rollback
    cost_expected_forward = cost_repair + (1 - p_success) * cost_sunk

    latency_reduction = (t_re_exec.magnitude - t_expected_forward.magnitude) / t_re_exec.magnitude
    cost_reduction = (cost_re_exec - cost_expected_forward) / cost_re_exec

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(abs(t_rollback.magnitude - 40.6) < 1e-4, "t_rollback")
    check(abs(t_prov.magnitude - 46.0) < 1e-4, "t_prov")
    check(abs(t_re_exec.magnitude - 86.6) < 1e-4, "t_re_exec")
    check(abs(cost_re_exec - 0.284) < 1e-4, "cost_re_exec")
    check(abs(t_model_repair.magnitude - 3.69) < 1e-4, "t_model_repair")
    check(abs(cost_repair - 0.00987) < 1e-5, "cost_repair")
    check(abs(t_expected_forward.magnitude - 7.338) < 0.01, "t_expected_forward")
    check(abs(cost_expected_forward - 0.02123) < 1e-4, "cost_expected_forward")
    check(abs(latency_reduction - 0.9152) < 0.005, "latency_reduction")
    check(abs(cost_reduction - 0.9252) < 0.005, "cost_reduction")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    t_rollback_s_str = fmt_time(t_rollback, unit=second, precision=1)
    cost_sunk_str = fmt_usd(cost_sunk, precision=3)
    t_re_exec_s_str = fmt_time(t_re_exec, unit=second, precision=1)
    cost_re_exec_str = fmt_usd(cost_re_exec, precision=3)

    t_model_repair_s_str = fmt_time(t_model_repair, unit=second, precision=2)
    t_tool_repair_s_str = fmt_time(t_tool_repair, unit=second, precision=1)
    cost_repair_str = fmt_usd(cost_repair, precision=5)

    t_expected_forward_s_str = fmt_time(t_expected_forward, unit=second, precision=2)
    cost_expected_forward_str = fmt_usd(cost_expected_forward, precision=3)

    latency_reduction_pct_str = fmt_percent(latency_reduction, precision=1, style="prose")
    cost_reduction_pct_str = fmt_percent(cost_reduction, precision=1, style="prose")
```

::: {#nbk-11-quantitative-trade-off-between-forward-patching .callout-notebook title="Forward patching vs. saga rollback"}
...
- Sunk Model Tokens: 38,500 prompt tokens and 3,100 generation tokens across steps $T_1$ through $T_5$, totaling `{python} ForwardPatchingVsSagaRollback.cost_sunk_str` in inference fees.
- Re-execution Cost: Repeating the entire workflow from step one to step six requires identical token generation and environment setup, consuming another $46.0\text{ s}$ of physical provisioning and `{python} ForwardPatchingVsSagaRollback.cost_sunk_str` of inference.
- Total Resource Penalty: `{python} ForwardPatchingVsSagaRollback.t_re_exec_s_str` of wall-clock delay, `{python} ForwardPatchingVsSagaRollback.cost_re_exec_str` in redundant inference fees, and non-refundable cloud provisioning minimum charges.
...
- Tool Re-Execution: The proxy daemon validates the patched configuration in `{python} ForwardPatchingVsSagaRollback.t_tool_repair_s_str`.
...
Forward recovery reduces end-to-end recovery latency by **{python} ForwardPatchingVsSagaRollback.latency_reduction_pct_str** (`{python} ForwardPatchingVsSagaRollback.t_expected_forward_s_str` vs. `{python} ForwardPatchingVsSagaRollback.t_re_exec_s_str`) and financial expenditure by **{python} ForwardPatchingVsSagaRollback.cost_reduction_pct_str** (`{python} ForwardPatchingVsSagaRollback.cost_expected_forward_str` vs. `{python} ForwardPatchingVsSagaRollback.cost_re_exec_str`). The runtime preserves the completed infrastructure work while resolving the failure via localized state transformation.
:::
...
Some effects cannot be rewound by any runtime. In a local relational database, an uncommitted transaction executes in synthetic isolation; if an invariant fails, an `ABORT` command leverages the write-ahead log to overwrite dirty cache pages and restore disk blocks to their exact prior bit-state. In autonomous agent architectures, however, tools routinely act across sovereign administrative boundaries, dispatch network packets across public wires, or trigger non-refundable financial clearinghouses. For these interactions, the foundational assumption of classical backward recovery, that for every forward action $T_i$ there exists an inverse operation $C_i$ such that $C_i(T_i(S)) = S$, collapses entirely.
...
To govern side effects without risking irrecoverable corruption, the agent runtime must maintain an explicit contract with its tool execution broker. Every registered tool endpoint declares its reversibility tier in its schema, extending the typed action contract (principle \ref{pri-vol3-strict-action-abi}) from what a tool accepts to what it can undo. The runtime classifies tool actions into three distinct categories based on their semantic and physical invariants. The categories refine the authority levels of @sec-vol3-intro-hsa, placing an invertible action confined to the sandbox at $A_1$, a compensable external action at $A_2$, and a pivot action at $A_3$.
...
Once executed, a pivot action commits an unalterable mutation to an external state space. Examples include dispatching an email to an external mailing list, publishing a compiled software binary to a public registry, dropping a primary production database table whose physical backups were simultaneously pruned, or settling a wire transfer to an external bank. Once the RPC for a pivot action completes, the world has permanently advanced. The agent runtime can neither roll back the action nor apply an automated counter-action to neutralize its effects.
...
| **Class III: Pivot / Irreversible** | $C_i = \emptyset$                                                       | External email delivery, production DNS flip, wire transfer       | Permanent effect observed outside the runtime     | Monotonic forward progress, deterministic retry, or operator escalation |
...
To prevent this state explosion, the runtime enforces the *Pivot Action Boundary Invariant*, the ordering rule at the center of the pivot boundary and compensating Sagas (principle \ref{pri-vol3-reversibility-sagas}). In any valid executable trajectory $\mathcal{T} = (T_1, T_2, \dots, T_n)$, there can exist at most one pivot transaction, denoted $T_{\text{pivot}} = T_k$. This single operation bifurcates the entire execution lifecycle into two structurally distinct operational regimes:
...
2. *The Pivot Step ($i = k$):* The pivot transaction represents the execution's atomic threshold. Before dispatching $T_{\text{pivot}}$, the runtime must verify that all pre-conditions are met, all temporary leases are active, and all post-pivot operations have validated execution paths. Where policy requires a human signature, the pivot also passes through the escrow protocol of @sec-vol3-controlplane-hitl. The dispatch of $T_{\text{pivot}}$ requires a synchronous write to the Write-Ahead Log (WAL), transitioning the trajectory state from `STATUS_PREPARING` to `STATUS_COMMITTED`.
...
::: {#psp-11-the-pivot-point-and-irreversible-effects .callout-perspective title="The pivot point and the asymmetry of irreversible actuation"}
In classical database architectures, transaction management rests upon the assumption that physical mutations can be rolled back cleanly via write-ahead logging or multi-version concurrency control. In an agentic machine learning system interacting with external environments, this symmetry between forward execution and backward recovery breaks down completely.

Many external operations are strictly irreversible. Once an agent dispatches an email to customers, flips a production DNS record, transfers cryptographic tokens, or broadcasts a public API mutation, no compensating transaction can un-send the bytes or reverse external observation. Garcia-Molina's Saga pattern recognizes this asymmetry through the concept of the *pivot transaction*, the single atomic boundary where an execution trajectory transitions from compensable preparation to irreversible commitment.

Yet stochastic foundation models possess no innate awareness of this physical boundary. Conditioned purely on sequence generation, a model will happily interleave irreversible mutations throughout an exploratory planning chain—issuing external API calls before validating local preconditions. If step four fails after steps one and three committed external effects, the runtime is trapped in an impossible state: unable to roll backward, yet blocked from moving forward.

**Systems insight**: Reversibility is a property of the environment, not the prompt. High-reliability agent runtimes must enforce the Pivot Action Boundary through structural supervisor compilation: topologically sorting proposed action DAGs so that all invertible and compensable operations execute first, isolating the single irreversible pivot step to the absolute point of terminal commitment.
:::
...
This architectural assumption collapses for a looping model. An inference engine running a forward pass does not stall at the instruction pointer level. Each token generation step is a regular matrix-vector multiplication ($\text{GEMV}$) that streams weights from high-bandwidth memory at a steady cadence, so a looping model looks exactly as busy as a productive one. As long as the inference server (such as vLLM or SGLang) and the host agent supervisor exchange well-formed Remote Procedure Call (RPC) messages, every standard transport-layer watchdog is satisfied. This is the fail-plausible fault model of @sec-vol3-intro-fail-plausible seen from the supervisor, a process that is healthy by every liveness measure and wrong by every task measure.
...
| **Semantic Embedding Drift**     | Cosine distance $1 - \frac{e(a_t) \cdot e(a_{t-k})}{\lVert e(a_t) \rVert \lVert e(a_{t-k}) \rVert}$ | Paraphrased reasoning loops and reworded prompts                               | Small matrix-vector dot product ($d \le 1536$)                       | Set conservative similarity thresholds ($\cos \theta \ge 0.96$)  |
: **Semantic Watchdog Anti-Spin Invariants**: Invariant types, detection mechanisms, detection targets, and overhead of runtime watchdog probes. {#tbl-vol3-watchdog-invariants}
...
```{python}
#| label: lego-11-the-microeconomic-and-hardware-cost
#| echo: false
# │ Show: Watchdog bounds loop at 40 s and 0.062 kWh vs 500 s and 0.778 kWh.
# │ How: 8x H100 TDP power, token accumulation, and sliding window cycle detection.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_energy, fmt_percent, fmt_time, fmt_usd

class SemanticLoopHardwareCost:
    """Hardware and microeconomic cost of silent semantic loops vs watchdog."""
    ...
    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    t_total_unmonitored = h_max * t_iter
    t_total_unmonitored_min = t_total_unmonitored.to(minute)

    e_cluster_j = (n_gpus * p_gpu_tdp.to(watt).magnitude * t_total_unmonitored.to(second).magnitude) * joule
    e_cluster_kwh = e_cluster_j.magnitude / 3_600_000.0

    tokens_step = 300 + 800
    total_tokens_context = h_max * tokens_step

    cost_prompt = 1.45 * 3.00
    cost_completion = 0.015 * 15.00
    cost_total_unmonitored = cost_prompt + cost_completion

    # Mitigated
    h_mitigated = 4
    t_total_mitigated = h_mitigated * t_iter
    e_cluster_mitigated_j = (n_gpus * p_gpu_tdp.to(watt).magnitude * t_total_mitigated.to(second).magnitude) * joule
    e_cluster_mitigated_kwh = e_cluster_mitigated_j.magnitude / 3_600_000.0
    cost_mitigated = 0.08

    latency_savings = (t_total_unmonitored.magnitude - t_total_mitigated.magnitude) / t_total_unmonitored.magnitude
    power_savings = (e_cluster_j.magnitude - e_cluster_mitigated_j.magnitude) / e_cluster_j.magnitude
    cost_savings = (cost_total_unmonitored - cost_mitigated) / cost_total_unmonitored

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(abs(t_total_unmonitored.magnitude - 500.0) < 1e-4, "t_total_unmonitored")
    check(abs(e_cluster_kwh - 0.77777) < 0.001, "e_cluster_kwh")
    check(total_tokens_context == 55_000, "total_tokens_context")
    check(abs(cost_total_unmonitored - 4.575) < 0.01, "cost_total_unmonitored")
    check(abs(t_total_mitigated.magnitude - 40.0) < 1e-4, "t_total_mitigated")
    check(abs(e_cluster_mitigated_kwh - 0.06222) < 0.001, "e_cluster_mitigated_kwh")
    check(abs(latency_savings - 0.92) < 1e-4, "latency_savings")
    check(abs(power_savings - 0.92) < 1e-4, "power_savings")
    check(abs(cost_savings - 0.9825) < 0.005, "cost_savings")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    t_total_unmonitored_s_str = fmt_time(t_total_unmonitored, unit=second, precision=0)
    t_total_unmonitored_min_str = fmt_time(t_total_unmonitored, unit=minute, precision=2)
    e_cluster_kwh_str = fmt_energy(e_cluster_kwh * kWh, unit=kWh, precision=3)
    cost_total_unmonitored_str = fmt_usd(cost_total_unmonitored, precision=2)

    t_total_mitigated_s_str = fmt_time(t_total_mitigated, unit=second, precision=0)
    latency_savings_pct_str = fmt_percent(latency_savings, precision=0, style="prose")
    e_cluster_mitigated_kwh_str = fmt_energy(e_cluster_mitigated_kwh * kWh, unit=kWh, precision=3)
    power_savings_pct_str = fmt_percent(power_savings, precision=0, style="prose")
    cost_mitigated_str = fmt_usd(cost_mitigated, precision=2)
    cost_savings_pct_str = fmt_percent(cost_savings, precision=1, style="prose")
```

::: {#nbk-11-the-microeconomic-and-hardware-cost .callout-notebook title="Hardware cost of silent semantic loops"}
...
Without an invariant monitor, the agent continues until hitting an arbitrary maximum step limit $H_{\max} = 50$.
...
- Total time elapsed: $4 \times 10.0 \text{ s} = 40.0 \text{ seconds}$ (a `{python} SemanticLoopHardwareCost.latency_savings_pct_str` reduction in wall-clock latency).
- Cluster energy expended: $8 \times 700 \text{ W} \times 40 \text{ s} = 0.224 \times 10^6 \text{ J} = 0.062 \text{ kWh}$ (a `{python} SemanticLoopHardwareCost.power_savings_pct_str` reduction in thermal and power load).
- Financial expenditure: Prompt tokens processed drop to approximately $21,000$, resulting in an API charge of `{python} SemanticLoopHardwareCost.cost_mitigated_str` (a `{python} SemanticLoopHardwareCost.cost_savings_pct_str` cost reduction).
:::

### Watchdog intervention protocols

Detecting an invariant violation represents only half of the architectural requirement; the runtime must also define deterministic state-machine semantics for intervention. Silently dropping an agent's tool call leaves the autoregressive decode loop in an undefined state. The runtime implements a three-tier intervention protocol designed to trade off recovery cost against trajectory termination, as detailed in @tbl-vol3-watchdog-escalation-protocol.
...
| **Tier 3: Terminal Abort**       | Unrecoverable deadlock; cyclic regression exceeds budget threshold       | Halts execution, raises `ERR_SEMANTIC_DEADLOCK`, strips capabilities; compensates before the pivot, freezes after it.    | Complete trajectory freeze; emits audit alert                     | Human-in-the-loop escalation / post-mortem queue    |
...
3. **Tier 3: Terminal Abort and Saga Escalation (`ERR_SEMANTIC_DEADLOCK`):** If the agent exhausts its allotted backtracking budget, the watchdog issues a non-maskable trajectory termination. The runtime raises `ERR_SEMANTIC_DEADLOCK`, revokes the agent's transient capability tokens, releases all acquired POSIX file and database locks, and dispatches the trajectory's Write-Ahead Log to human operators. What happens to the external side effects depends on where the trajectory stands relative to its pivot. Before the pivot, the runtime executes the compensators registered in the compensating log ($\mathcal{L}_{\text{comp}}$) in reverse order, returning the broader distributed system to a consistent, quiesced state. After the pivot, that log has been retired and compensation is no longer available, so the runtime takes the terminal escalation path of @sec-vol3-sagas-pivot, freezing the Agent Control Block and handing the committed state to an operator.
...
```{python}
#| label: lego-11-sizing-bulkhead-capacity-and-jitter
#| echo: false
# │ Show: Full jitter dampens peak retry pressure by 1000x across 128 agents.
# │ How: Worker thread exhaustion timing, deterministic burst vs uniform spread.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_multiple, fmt_time

class BulkheadCapacityJitter:
    """Bulkhead thread isolation and full jitter retry dampening."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_agents = 256
    m_pool = 300
    t_timeout = 5000 * millisecond

    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    m_free = m_pool - n_agents
    t_frozen = t_timeout.to(millisecond).magnitude - 200.0

    # Retries
    n_failing_agents = 128
    dt_dispatch = 0.002 * second
    lambda_peak_deterministic = n_failing_agents / dt_dispatch.magnitude

    window_jitter = 2.0 * second
    lambda_expected_jitter = n_failing_agents / window_jitter.magnitude

    dampening_factor = lambda_peak_deterministic / lambda_expected_jitter

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(m_free == 44, "m_free")
    check(t_frozen == 4800.0, "t_frozen")
    check(lambda_peak_deterministic == 64_000.0, "lambda_peak_deterministic")
    check(lambda_expected_jitter == 64.0, "lambda_expected_jitter")
    check(dampening_factor == 1000.0, "dampening_factor")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_agents_str = fmt_int(n_agents)
    m_pool_str = fmt_int(m_pool)
    m_free_str = fmt_int(m_free)
    t_frozen_ms_str = fmt_time(t_frozen * millisecond, unit=millisecond, precision=0)

    n_failing_agents_str = fmt_int(n_failing_agents)
    lambda_peak_deterministic_str = fmt_int(int(lambda_peak_deterministic))
    lambda_expected_jitter_str = fmt_int(int(lambda_expected_jitter))
    dampening_factor_mult_str = fmt_multiple(round(dampening_factor))
```
...
At $t \approx 100\text{ ms}$, `{python} BulkheadCapacityJitter.n_agents_str` of the `{python} BulkheadCapacityJitter.m_pool_str` available worker threads are fully occupied waiting on the frozen network socket. This leaves only:

$$M_{\text{free}} = 300 - 256 = 44\text{ threads}$$

for all other agent tasks, including local file writes and local memory compaction. If any of the remaining active agents spawn a sub-task or secondary tool call, the remaining `{python} BulkheadCapacityJitter.m_free_str` threads are consumed instantly. Total thread exhaustion occurs at:

$$t_{\text{exhaust}} < 200\text{ ms}$$

For the remaining `{python} BulkheadCapacityJitter.t_frozen_ms_str` until the socket timeout fires, the entire agent runtime freezes. It cannot execute trivial, non-failing local tasks because the shared thread pool is completely starved by blocked network sockets.
...
All `{python} BulkheadCapacityJitter.n_failing_agents_str` agents wake up simultaneously at exactly $t = t_0 + 5000\text{ ms} + 2000\text{ ms} = t_0 + 7000\text{ ms}$. Assuming an OS thread scheduling dispatch jitter of $\Delta t_{\text{dispatch}} \approx 2\text{ ms}$, the peak instantaneous arrival rate at the external service is:
...
If the host runtime dispatches all tool calls into a single global worker pool of capacity $C_{\text{pool}}$, an outage in a single external web-scraping tool will consume all worker threads as calls block waiting for TCP connection timeouts. If external endpoints experience transient delays of duration $T_{\text{timeout}} \gg T_{\text{local}}$, arriving external requests saturate all worker slots:
...
::: {#chk-11-pool-exhaustion .callout-checkpoint title="Evaluating bulkhead partitioning and resource exhaustion"}
Before evaluating multi-pool scheduling and circuit breaker thresholds, verify your understanding of worker pool contention:

- [ ] **Bulkhead isolation**: Why does partitioning worker threads into isolated pools prevent a failing external HTTP tool from blocking local filesystem reads?
- [ ] **Queue saturation**: How does unpartitioned head-of-line blocking cause fast deterministic tools to inherit the timeout latency of slow external APIs?
- [ ] **Fast-fail semantics**: Under what conditions should a saturated pool reject incoming tool invocations immediately (`ERR_BULKHEAD_FULL`) rather than unbounded queuing?
- [ ] **Capacity sizing**: How do timeout bounds and maximum expected concurrency dictate the semaphore limits for internal versus external execution pools?
:::

To enforce the end-to-end argument and maintain system availability, the runtime must implement bulkhead isolation by partitioning execution threads, rate limits, and memory queues into strictly isolated resource domains, as summarized in @tbl-vol3-bulkhead-architecture.
...
: **Bulkhead Fault Isolation and Resource Quotas**: Resource boundaries, failure impacts, and fallback policies across local, internal, and SaaS tool tiers. {#tbl-vol3-bulkhead-fault-isolation}
...
If a trajectory's failure stems from prompt injection, such as instructions embedded in a retrieved documentation page or an email body, the runtime must treat the model as compromised. Containment beneath the model (principle \ref{pri-vol3-zero-trust-sandboxing}) already assumes as much, and in this state model-directed repair or compensation becomes a hazard rather than a remedy. If the runtime asks a compromised model to synthesize compensating actions, the model may emit further destructive tool calls under the guise of cleanup, deleting database tables or forwarding sensitive credentials to an attacker's endpoint. Similarly, an agent caught in a degenerative token generation loop may consume its entire rate-limit quota or overwhelm downstream services with malformed mutations.
...
To enforce containment without inducing host-level instability, the supervisor executes a deterministic four-stage quarantine protocol (@tbl-vol3-quarantine-protocol). This pipeline bypasses the agent's inference loop entirely, executing strictly within the host runtime's privileged control plane.
...
The second stage is *Ephemeral Credential Revocation*. Because the model holds zero ambient authority, an agent never holds permanent infrastructure credentials. The supervisor instead issues short-lived, narrowly scoped capability leases and records them in the trajectory's Agent Control Block (ACB). Upon triggering quarantine, the supervisor dispatches a synchronous revocation event to its internal credential broker and external Identity Providers (such as AWS STS, Google Cloud IAM, or OAuth authorization servers). The leases are canceled, invalidating session cookies, database connection handles, and API bearer tokens. Even if an in-flight tool call escaped the process sandbox before suspension, downstream services reject the request upon token introspection.
...
::: {#dfn-taint-propagation .callout-definition title="Dynamic taint propagation"}

***Dynamic taint propagation***\index{Dynamic taint propagation!definition}\index{Taint tracking!definition} is an authoritative lineage-tracking protocol wherein the host supervisor flags all intermediate artifacts and state modifications emitted by a failing or compromised agent trajectory as tainted, intercepting downstream peer accesses to quarantine cascading failures.

1.  **Significance**: Prevents Byzantine or hallucinated outputs generated during compromised trajectory epochs from silently poisoning downstream peer agents, shared filesystems, or persistent data stores.
2.  **Distinction**: Unlike static dataflow analysis (which inspects code ASTs ahead of time), dynamic taint propagation maintains a live directed provenance graph during runtime execution, binding cryptographic artifact hashes to trajectory lifecycle states.
3.  **Common pitfall**: Clearing taint tags based purely on model self-verification ("I have checked the output and it is clean") rather than independent, out-of-band verification harnesses or human operator clearance.
...
The architectural synthesis of runtime orchestration brings together the three pillars of this part, the control plane of @sec-vol3-checkpointing, the event log of @sec-vol3-interrupts, and the Saga engine of this chapter:

$$\text{Agent Runtime} = \text{Control Plane} + \text{WAL Event Storage} + \text{Saga Recovery Engine}$$

Under this composition, the control plane scheduler acts as the unprivileged model's supervisor, managing queue priority, token generation budgets, and context window allocation. It does not, however, execute state transitions in isolation. Every state alteration, whether an internal belief update $\mathbf{s}_t \to \mathbf{s}_{t+1}$ or an external tool dispatch $a_t$, must pass through the Write-Ahead Logging (WAL) storage substrate before execution, and under intent before effect (principle \ref{pri-vol3-intent-before-effect}) no external effect leaves the host before its authorizing record is durable. The WAL guarantees durability by enforcing synchronous flushes to non-volatile storage at critical boundary points. In turn, the Saga recovery engine wraps these logged operations within a transactional lifecycle, maintaining the compensating ledger $\mathcal{L}_{\text{comp}}$ and dynamically arbitrating between backward compensation, forward self-healing, and irreversible pivot enforcement.
...
```{python}
#| label: lego-11-quantitative-recovery-overhead-and-rto
#| echo: false
# │ Show: Dual-tier checkpointing restores crashed agent in 0.57 s with RPO=0.
# │ How: Snapshot I/O, WAL deserialization, context prefill delta, and probe round-trip.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_memory, fmt_time

class QuantitativeRecoveryRto:
    """Quantitative recovery overhead and RTO bounds in dual-tier persistence."""
    ...
    d_snap = 48 * MB
    d_wal = 0.128 * MB
    b_disk = 3500 * (MB / second)
    t_latency = 0.025 * millisecond
    p_prefill = 8000 * (token / second)
    l_net = 45 * millisecond

    # ┌── 2. CALCULATE (Architecture & Simulation) ─────────────────────────
    t_snap = (d_snap.to(MB) / b_disk.to(MB / second)).magnitude * second
    t_wal = (d_wal.to(MB) / b_disk.to(MB / second)).magnitude * second + t_latency.to(second)
    delta_l = (l_10.magnitude - l_8.magnitude) * token
    t_prefill = (delta_l.magnitude / p_prefill.magnitude) * second
    t_probe = l_net

    rto = t_snap + t_wal + t_prefill + t_probe
    rto_s = rto.to(second)

    t_full_eval = (l_10.magnitude / p_prefill.magnitude) * second

    # ┌── 3. ASSERT (Validation & Boundaries) ──────────────────────────────
    check(abs(t_snap.to(millisecond).magnitude - 13.714) < 0.01, "t_snap")
    check(abs(t_wal.to(millisecond).magnitude - 0.06157) < 0.005, "t_wal")
    check(delta_l.magnitude == 4096, "delta_l")
    check(abs(t_prefill.to(millisecond).magnitude - 512.0) < 1e-4, "t_prefill")
    check(abs(rto.to(millisecond).magnitude - 570.776) < 0.1, "rto")
    check(abs(t_full_eval.to(second).magnitude - 2.048) < 1e-4, "t_full_eval")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    d_snap_mb_str = fmt_memory(d_snap, unit=MB, precision=0)
    t_snap_ms_str = fmt_time(t_snap, unit=millisecond, precision=1)
    t_wal_ms_str = fmt_time(t_wal, unit=millisecond, precision=2)
    delta_l_str = fmt_int(int(delta_l.magnitude))
    t_prefill_ms_str = fmt_time(t_prefill, unit=millisecond, precision=0)
    t_probe_ms_str = fmt_time(t_probe, unit=millisecond, precision=0)
    rto_ms_str = fmt_time(rto, unit=millisecond, precision=1)
    rto_s_str = fmt_time(rto_s, unit=second, precision=2)
    t_full_eval_s_str = fmt_time(t_full_eval, unit=second, precision=2)
```

::: {#nbk-11-quantitative-recovery-overhead-and-rto .callout-notebook title="Quantitative recovery overhead and RTO bounds"}
...
The synthesized harness recovers the crashed execution in approximately `{python} QuantitativeRecoveryRto.rto_s_str`, comfortably below the $5.0\text{ second}$ production threshold, while completely eliminating the need to re-evaluate the full $16\text{k}$ token context from scratch (which would have imposed an additional `{python} QuantitativeRecoveryRto.t_full_eval_s_str` of prefill latency). Furthermore, because all events through Turn 10 were durable on disk, $\text{RPO} = 0$.
:::
...
Dependability after a partial failure is a property of the supervisory harness, not of the model. This chapter turned the pivot boundary and compensating Sagas (principle \ref{pri-vol3-reversibility-sagas}) into an ordering rule the runtime can enforce. Compensable steps come first, each with its compensator registered before dispatch. At most one pivot follows, behind a deterministic gate, and after it the runtime can only retry idempotently or escalate. The semantic watchdog extends out-of-band preemption and progress watchdogs (principle \ref{pri-vol3-preemptive-interrupts}) from resource budgets to progress itself, stopping a trajectory that satisfies every budget and heartbeat while its environment state repeats.

::: {.callout-chapter-connection title="From runtime systems to policy compilers"}
The runtime architecture established across Part IV provides a dependable operational foundation: the supervisor governs each trajectory through its Agent Control Block (@sec-vol3-checkpointing), persists immutable execution histories to durable write-ahead logs (@sec-vol3-interrupts), and isolates partial failures through distributed Sagas and semantic watchdogs (this chapter). Every accepted state mutation and every compensated failure trajectory is now immutably captured.

Yet runtime resilience, however robust, is an operational mitigation rather than a cure. Continually invoking compensating transactions, tripping circuit breakers, and burning inference tokens on bounded self-healing attempts extracts a heavy toll in latency, GPU memory bandwidth, and financial compute. While some execution failures stem from environmental transients or fragile external APIs, failure cascades that recur after fixes to context, tool interfaces, and the runtime itself point to deficiencies in the foundation model's decision policy ($\pi_\theta$), such as an inability to interpret complex tool schemas, anticipate edge-case exceptions, or reason over long causal chains.

This observation marks the natural boundary between runtime orchestration and learning systems. Rather than discarding failed trajectories or leaving them dormant in cold storage, we can treat the runtime's Write-Ahead Log as a high-density supervisory signal. In Part V (*The Policy Compiler*), @sec-vol3-data-flywheel (*Trajectory Harvesting*), we pivot from dynamic runtime supervision to offline data curation. We examine how to harvest raw execution traces, filter unrecoverable anomalies, annotate failure points, and transform runtime fault records into high-quality training corpora that compile operational experience directly back into the neural weights.
:::
