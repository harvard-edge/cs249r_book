...
\begin{marginfigure}
\mlagentstack{95}{20}{20}{10}{20}{10}
\caption{The Agentic Machine Learning Systems Stack.}
\end{marginfigure}
:::

_Why does deploying ten agents in a shared room take longer and make more errors than a single agent working alone?_
...
- Explain the quadratic communication tax and correlated hallucination risks of unconstrained conversational swarms.
- Diagnose context pollution and KV-cache thrashing caused by shared prompt buffers.
- Design Hewitt-Agha actor models with zero shared context and private mailboxes.
- Calculate optimal multi-agent concurrency limits using Multi-Agent Amdahl's Law.
- Evaluate vector clocks and causal ordering protocols for deterministic execution.
- Compare optimistic concurrency control, TTL leases, and git worktree isolation for physical workspace management.
- Architect deterministic software oracles to override correlated neural voting consensus.
:::
...
: **Multi-Agent Coordination Topologies**: Architectural characteristics, communication complexity, state isolation boundaries, failure blast radii, and operational failure signatures across multi-agent coordination archetypes. {#tbl-vol3-multi-agent-topologies tbl-colwidths="[15,20,20,15,10,10,10]"}
...
: **Empirical Concurrency Regimes under Multi-Agent Amdahl's Law**: Quantitative speedup, parallel efficiency, aggregate token multipliers, and dominant operational bottlenecks across fleet sizes for representative enterprise software engineering parameters ($s = 0.15, \alpha = 0.004, \beta = 0.03$). Concurrency past $M^* = 4$ degrades wall-clock speedup while dramatically inflating financial token expenditures. {#tbl-vol3-multi-agent-amdahl-regimes tbl-colwidths="[10,15,15,15,15,15,15]"}
...
: **Workspace Concurrency Control and Isolation Primitives**: Architectural mechanisms, governed resource scopes, operational latency taxes, storage footprints, and recovery protocols for concurrent agent execution environments. {#tbl-vol3-multi-agent-concurrency-primitives tbl-colwidths="[15,20,15,15,15,20]"}
...
: **Distributed Stochastic Actors Architectural Synthesis**: Summary of core architectural subsystems, governing theoretical formalisms, primary failure modes, systems mitigations, and operational invariants in production multi-agent systems. {#tbl-vol3-multi-agent-synthesis tbl-colwidths="[15,20,25,20,20]"}

::: {.callout-takeaways title="The Multi-Agent Coordination Invariant"}
...
::: {.callout-chapter-connection title="From Coordination to Trajectory Observability"}
...
## Quantitative Problems {#sec-vol3-multi-agent-problems}

1. **Multi-Agent Amdahl's Law and Serial Bottlenecks:**
   A code translation task requires $W=1000$ logical steps. The sequential planning and final integration fraction is $s = 0.10$. The coordination synchronization penalty is $\alpha = 0.005$, and context provisioning overhead is $\beta = 0.02$.
   a. Calculate the theoretical maximum parallel speedup $S_{\text{classical}}(M)$ ignoring coordination tax as $M \to \infty$.
   b. Derive the optimal number of agents $M^*$ that maximizes effective speedup $S(M)$.
   c. Calculate the wall-clock speedup at $M^*$.
   d. At what fleet size $M_{\text{cross}}$ does the system enter the Negative Return Collapse Zone ($S(M) < 1.0$)?

2. **Mailbox Message Token Inflation under Quadratic Peer Topologies:**
   Consider a swarm of $M=10$ agents in a fully connected peer-to-peer topology. Each agent emits an average of 150 tokens per conversational turn.
   a. If each agent takes 5 conversational turns, calculate the total number of tokens processed by the inference cluster across all agents if using a shared context buffer without KV-cache prefix sharing.
   b. Recalculate the token consumption if the topology is migrated to a Hierarchical Supervisor Tree with one root planner and 9 leaf workers, where workers only communicate with the planner.

3. **Byzantine Agreement Consensus Failure Probability:**
   An engineering team implements a $2f+1$ neural consensus voting mechanism with $N=5$ agents. The probability of a single agent hallucinating a bug is $p = 0.15$.
   a. Assuming statistically independent failures (classical Byzantine assumption), calculate the probability that a majority ($\ge 3$) of agents simultaneously hallucinate the exact same bug.
   b. Now assume heavily correlated failures due to shared pretraining. The conditional probability that Agent B hallucinates given Agent A hallucinates is $P(B \mid A) = 0.90$. Calculate the probability of a unanimous 5-0 hallucinated consensus.
   c. Explain why deterministic software oracles must dominate neural voting in production architectures based on these results.
