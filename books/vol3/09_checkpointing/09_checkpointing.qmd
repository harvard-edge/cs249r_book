...
::: {#exmp-09-control-plane-descriptor-footprint-vs .callout-example title="Control Plane Descriptor Footprint vs. Tensor State Overhead"}
...
![**Scheduler Run Queue and Priority Scoring Engine**: The supervisory control plane inspects candidate Agent Control Blocks in host memory, filters for runnable trajectories, calculates fair-share priorities across multi-attribute execution vectors, and enforces hard resource caps prior to dispatching workloads to inference worker pipelines.](images/svg/fig-vol3-scheduler-dispatch.svg){#fig-vol3-scheduler-dispatch}
...
::: {#exmp-09-supervisory-state-transition-latency-vs .callout-example title="Supervisory State Transition Latency vs. Inference Overhead"}
...
::: {#exmp-09-quantifying-worker-scalability-and-memory .callout-example title="Quantifying Worker Scalability and Memory Efficiency under Cooperative Yielding"}
...
::: {.column-margin}
![](images/svg/vol3_checkpointing_margin_003.svg){width="100%" fig-alt="Paired bar comparison showing 9.06 TiB of memory consumed by 36,000 synchronous thread stacks versus 141 MiB consumed by asynchronous Agent Control Block stubs during human approval pauses."}

*Asynchronous escrow stubs reduce dormant memory footprint by 67,584x during multi-hour human-in-the-loop pauses.*
:::
...
The supervisor addresses this by compiling the candidate action into a canonical, tamper-evident document termed an *escrow manifest*, as depicted in @fig-vol3-human-escrow. The manifest functions as an explicit contract between the agent runtime, the proposing model state, and the human supervisor.

![**Cryptographic Human Escrow Protocol and Manifest Schema**: The supervisor suspends the Agent Control Block upon detecting high-consequence operations, formats a canonical escrow manifest bound to the causal context hash, and admits execution only upon receipt of valid cryptographic signatures prior to lease expiration.](images/svg/fig-vol3-human-escrow.svg){#fig-vol3-human-escrow width="95%"}
...
::: {#exmp-09-memory-seconds-and-sizing-of-escrow .callout-example title="Memory-Seconds and Sizing of Escrow Suspension Pools"}
...
![**Weighted Deficit Round-Robin Node Scheduling**: Arbitration of heterogeneous agent trajectories across host compute, inference token quotas, and execution sandboxes. The arbiter credits ready ACB deficit counters by priority weight, admitting turns whose compute and context cost does not exceed accumulated deficit, thereby preventing token bucket starvation while maintaining weighted proportional fairness.](images/svg/fig-vol3-node-scheduler.svg){#fig-vol3-node-scheduler width="95%"}
...
::: {#exmp-09-wdrr-quantum-allocation-and-inference .callout-example title="WDRR Quantum Allocation and Inference Rate Saturation"}
...
![**The Divergent Epistemic Attractor Cycle**: When environmental faults inject error tokens into the context buffer, conditional probability mass shifts away from terminal stop tokens toward speculative recovery, trapping unconstrained agents in non-halting loops that must be broken by out-of-band supervisory resource ceilings.](images/svg/fig-vol3-epistemic-attractor-cycle.svg){#fig-vol3-epistemic-attractor-cycle}
...
::: {#exmp-09-multi-dimensional-accounting-and-prefix-cache .callout-example title="Multi-Dimensional Accounting and Prefix Cache Pricing Dynamics"}
