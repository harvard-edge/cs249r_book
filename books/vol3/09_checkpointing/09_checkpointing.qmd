...
::: {#dfn-agent-control-block .callout-definition title="The Agent Control Block (ACB)"}
An **Agent Control Block (ACB)** is the canonical, host-managed kernel data structure that reifies an agent trajectory within the supervisory control plane:
$$\text{ACB} = \langle \text{id}_{\tau}, \mathcal{S}_{\text{life}}, \mathbf{\Theta}_{\text{gen}}, \mathbf{P}_{\text{mem}}, \mathbf{u}_k, \mathcal{M}_{\text{cap}} \rangle$$
decoupling supervisory governance (process identity, lifecycle state, generation envelope, memory handles, multi-dimensional resource accounting, and capability tokens) from the underlying inference engine's volatile tensor weights and KV-cache allocations. Neither the stochastic model core nor unprivileged tool sandboxes may directly access or mutate the ACB; all state transitions are mediated exclusively by the host supervisor.
:::
...
::: {#psp-observation-crossing-resolution .callout-perspective title="Observation crossing resolution protocol"}
...
::: {#psp-bounding-polytope .callout-perspective title="Multi-dimensional resource bounding polytope"}
