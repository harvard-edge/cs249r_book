...
::: {#psp-09-the-while-loop-fallacy .callout-perspective title="The while-loop fallacy and the need for supervisory control"}
In prototype engineering, an autonomous agent is almost universally implemented as an iterative Python loop: while not done, generate a continuation, parse a tool invocation, execute the function, and append the result. This architectural pattern commits the while-loop fallacy: confusing an unmanaged user-space thread with an operating system process.

In classical systems programming, an unmanaged thread cannot preempt itself, cannot meter its own memory consumption, cannot safely trap asynchronous cancellation signals, and cannot survive a host power failure. When an agent implemented as a raw loop executes a blocking network call or triggers an out-of-memory kernel panic, the entire execution context in volatile memory vanishes. There is no supervisor to capture partial progress, no capability mask to restrict privilege escalation, and no mechanism to prevent orphaned child processes from lingering indefinitely in the operating system.

Transforming stochastic models into dependable systems requires separating the unprivileged inference worker from the supervisory control plane. By reifying execution state into an explicit Agent Control Block (ACB) and Write-Ahead Log (WAL), the host runtime decouples task progression from volatile worker threads. The supervisor meters resource consumption across hard multi-dimensional ceilings, intercepts out-of-band signals at turn boundaries, and persists verified milestones—ensuring that agent execution can be paused, inspected, migrated, and recovered with the guarantees of an authentic operating system kernel.

**Systems insight**: A foundation model generates tokens, but only an external supervisor can guarantee process termination, fault isolation, and resource accounting. Building production agentic platforms requires treating the agent loop as an unprivileged process governed by an authoritative kernel.
:::
