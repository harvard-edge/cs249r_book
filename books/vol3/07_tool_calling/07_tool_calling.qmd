...
## Tool Subsystem Architecture {#sec-vol3-tool-calling-unix-analogy}
...
## Tool Interface Schemas {#sec-vol3-tool-calling-schemas}
...
3. *Stage 2 (JIT Schema Binding & Runtime DFA Masking):* Applying the grammar-constrained decoding of @sec-vol3-foundation-model-grammar-constrained, the runtime dynamically compiles the $K=4$ schemas into a deterministic finite automaton (DFA) that generates a token-level bitmask tensor $M_t \in \{0, -\infty\}^{|V|}$ on the GPU. By applying $z_t' = z_t + M_t$ during autoregressive generation, invalid token transitions are driven to $-\infty$, guaranteeing $100\%$ syntactic schema compliance while preventing out-of-catalog tool hallucinations.
...
## Interoperable Tool Discovery {#sec-vol3-tool-calling-mcp}
...
3. **Capability Gating and Tiered Consent:** Each operation is gated by the authority level it requires, using the levels $A_0$ through $A_3$ of @sec-vol3-intro-hsa, and each level carries its own condition for dispatch:
   ...
   - **$A_2$ (Retry-Safe or Compensable External Action):** Operations that change an external system in a way that can be retried safely or undone by a compensating action (e.g., provisioning a cloud instance that can later be released). These require every timeout to be settled before any retry, the subject of @sec-vol3-tool-calling-idempotency.
...
Modern tool-use frameworks often treat structural schema validation as a security boundary: if a generated tool call validates against its JSON Schema specification, the runtime treats the action as safe to dispatch. This conflates syntactic structure with operational authorization, the same confusion that @sec-vol3-foundation-model-grammar-constrained ruled out for grammar-constrained decoding. The schema a protocol delivers guarantees no more than the grammar that decoding enforces.
...
## Idempotent Action Execution {#sec-vol3-tool-calling-idempotency}
...
# │       produces at fleet volume, and size the endpoint's in-flight lock expiry.
# │ Show: Daily timeouts, daily duplicates, and the minimum lock expiry.
# │ How:  Independent loss on request and reply; duplicates arise only when
# │       the reply alone is lost; expiry bound from the displayed inequality.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
class SettlementBudget:
    ...
    lock_min_ms = timeout_ms + clock_skew_ms + exec_max_ms

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(duplicates_day < timeouts_day, "Only the lost-reply share of timeouts duplicates an effect")
    check(lock_min_ms > timeout_ms, "A lock expiry must outlast the timeout it protects")
...
**Lock expiry.** Bounding execution at `{python} SettlementBudget.exec_sigmas_str` standard deviations gives $\tau_{\text{proc\_max}} =$ `{python} SettlementBudget.exec_max_ms_str`, so the lock must not expire before `{python} SettlementBudget.lock_min_ms_str`.
...
## Observation Stream Truncation {#sec-vol3-tool-calling-streaming}
...
class ObservationBudget:
    ...
    bytes_per_token = 3  # scenario assumption: log text (paths, hashes, escapes) tokenizes denser than prose's ~4
...
## Terminal Output Sanitization {#sec-vol3-tool-calling-observation-parsing}
...
## Asynchronous Tool Dispatch {#sec-vol3-tool-calling-async}
...
# │ How:  Each poll re-reads the context, cached tokens billed at a fraction of
# │       the uncached price, plus the new status tokens; context grows by one
# │       poll exchange per poll.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
class PollingBudget:
    ...
    model = Models.Language.Llama3_70B  # same serving scenario as @sec-vol3-foundation-model-cost
    accel = Hardware.Cloud.H100
    weight_bytes = 1  # 8-bit weights, bytes per parameter (same as the call-price notebook)

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    params = model.parameters.to(ureg.count).magnitude
    bw = accel.memory.bandwidth.to(ureg.byte / ureg.second).magnitude
    decode_s_per_token = params * weight_bytes / bw  # single-stream decode floor, seconds per token
    ...
    cached_tokens = polls * cache_hit * mean_context
    billed_input = prefill_tokens + cached_fraction_price * cached_tokens
    cost_polling = billed_input / 1e6 * input_price_per_mtok + decode_tokens / 1e6 * output_price_per_mtok
    event_prefill_tokens = (1 - cache_hit) * context_tokens + final_result_tokens
    event_billed_input = event_prefill_tokens + cached_fraction_price * cache_hit * context_tokens
    cost_event = event_billed_input / 1e6 * input_price_per_mtok
    ...
    decode_ms_per_token_str = fmt_latency(Q_(decode_s_per_token * 1000, 'ms'), unit='ms', precision=1, commas=True)
    polls_str = fmt_int(polls)
    decode_tokens_str = fmt_int(decode_tokens)
    decode_s_str = fmt_time(Q_(round(decode_s), 's'), unit='s', precision=0)  # rounded: prose says "about"
    uncached_per_poll_str = fmt_int(uncached_per_poll)
    prefill_tokens_str = fmt_int(prefill_tokens)
    cached_tokens_str = fmt_int(cached_tokens)
...
**Problem**: An agent starts an integration suite that runs for `{python} PollingBudget.job_s_str` with `{python} PollingBudget.context_tokens_str` tokens of context. Under polling, the model checks every `{python} PollingBudget.poll_interval_s_str`, emitting a `{python} PollingBudget.poll_call_tokens_str`-token status call and a `{python} PollingBudget.poll_ack_tokens_str`-token acknowledgment around a `{python} PollingBudget.poll_result_tokens_str`-token status result. Prior context hits the prefix cache `{python} PollingBudget.cache_hit_str` of the time. Input costs `{python} PollingBudget.input_price_str` and output `{python} PollingBudget.output_price_str` per million tokens, cached input is billed at `{python} PollingBudget.cached_fraction_str` of the input price, and decoding runs at the single-stream floor of @sec-vol3-foundation-model-cost, about `{python} PollingBudget.decode_ms_per_token_str` per token. How do polling and a single event-driven resumption compare?

**Polling.** The job spans `{python} PollingBudget.polls_str` polls. They decode `{python} PollingBudget.decode_tokens_str` tokens, about `{python} PollingBudget.decode_s_str` of generation spent on status checks. Each poll re-reads the growing context, about `{python} PollingBudget.uncached_per_poll_str` uncached input tokens per poll, or `{python} PollingBudget.prefill_tokens_str` in all, plus `{python} PollingBudget.cached_tokens_str` cached tokens at the discounted rate. The total is `{python} PollingBudget.cost_polling_str`.

**Event-driven.** The runtime is notified when the job exits and makes one model call with a `{python} PollingBudget.final_result_tokens_str`-token result, billing `{python} PollingBudget.event_prefill_tokens_str` uncached input tokens plus one cached read of the context, and costing `{python} PollingBudget.cost_event_str`.
...
### Background process governance {#sec-vol3-tool-calling-async-governance}
...
## Toolkit Granularity Partitioning {#sec-vol3-tool-calling-toolkit-design}
...
Connecting an agent to external tools gives it the power to act on the world, but it also exposes the host environment to catastrophic failure. When a model dispatches arbitrary shell commands, compiles unfamiliar source code, or executes dynamically generated scripts, syntactic schema validation alone cannot prevent malicious exploits, destructive side effects, or adversarial prompt injection attacks from compromising the underlying operating system. Establishing a typed RPC interface is only half of the protection equation; the code invoked by that interface must execute in an environment where fault domains are strictly contained. In @sec-vol3-sandboxes (*Environmental Isolation*), we shift from the communication interface to the containment substrate, analyzing how hardware-isolated microVMs, WebAssembly runtimes, Linux namespaces, and capability-based security policies isolate untrusted execution without forfeiting the low-latency guarantees required by interactive agent loops.
:::
