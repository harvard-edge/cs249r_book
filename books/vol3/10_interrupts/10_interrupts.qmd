...
Apply the interrupt scheduling equations, memory holding cost models, and watchdog timeout bounds derived in this chapter to solve the following quantitative problems.

### Problem 1: KV-cache holding cost and swapping breakeven analysis

An enterprise deploys a fleet of autonomous code-review agents running a frontier 70B parameter model. The model architecture uses Grouped-Query Attention (GQA) with $L = 80$ transformer layers, $H_{\text{q}} = 64$ query heads, $H_{\text{kv}} = 8$ key/value heads, and head dimension $d_h = 128$. Activations are represented in 16-bit bfloat16 ($b_{\text{elem}} = 2\text{ bytes}$). Each agent operates over an active context window of $S = 65{,}536$ tokens ($64\text{k}$).

1. **Memory Footprint Calculation:** Calculate the exact memory footprint $M_{\text{KV}}$ of an agent's KV cache in gigabytes ($1\text{ GB} = 10^9\text{ bytes}$) and gibibytes ($1\text{GB} = 2^{30}\text{ bytes}$).
2. **PCIe Swap Latency:** The GPU is an 80 GB NVIDIA H100 SXM5 connected to host memory via a PCIe Gen5 x16 bus. The bus achieves an effective bidirectional DMA transfer bandwidth of $\mathcal{B}_{\text{PCIe}} = 54\text{ GB/s}$ with a kernel launch and DMA descriptor initialization overhead of $\delta_{\text{kernel}} = 35\text{ ms}$. Calculate the total round-trip swap latency $T_{\text{swap}}$ required to evacuate the KV cache to host DRAM and rehydrate it upon task resumption.
3. **NVLink Alternative:** Suppose the GPU is installed in an 8-way NVLink node providing an inter-device bandwidth of $\mathcal{B}_{\text{NVLink}} = 450\text{ GB/s}$ and $\delta_{\text{kernel}} = 10\text{ ms}$ for memory swapping to a dedicated staging GPU buffer. Calculate $T_{\text{swap}}$ across the NVLink interconnect.
4. **Economic Holding Cost Analysis:** An enterprise operates a dedicated 8-GPU H100 SXM5 node at an amortized operational cost of $\$36.00$ per node-hour (or $\$4.50$ per GPU-hour). Human code review latency follows an exponential distribution with a mean of $\mu = 1{,}800\text{ seconds}$ ($30\text{ minutes}$).
   - If the runtime synchronously blocks and pins the KV cache in GPU HBM during human review, calculate the direct dollar cost of wasted idle silicon capacity per human review.
   - What is the analytical swapping breakeven threshold $T_{\text{breakeven}}$ over PCIe Gen5 x16? What percentage of total human review time does this breakeven latency represent?

### Problem 2: Preemption latency and worst-case execution time (WCET)

An agentic runtime kernel implements a three-tier interrupt priority schedule:

- **Priority 0 (Non-Maskable Interrupt - NMI):** Administrative emergency kill switch or catastrophic safety tripwire.
- **Priority 1 (`SYS_ESCALATE` Trap):** Synchronous capability escalation crossing the Reversibility Boundary.
- **Priority 2 (External Asynchronous IRQ):** User steering token injection or external CI/CD webhook.

The foundation model generates tokens at a steady-state decode rate of $T_{\text{decode}} = 25\text{ ms}$ per token. Writing the Agent Control Block (ACB) snapshot to durable NVMe storage takes $T_{\text{save}} = 12\text{ ms}$. The DMA transfer engine moves memory across the system bus at $\mathcal{B}_{\text{bus}} = 32\text{ GB/s}$.

1. **Maskable IRQ Latency:** Suppose an asynchronous Priority 2 IRQ arrives during an active token decode step. The agent's current KV cache working set is $8.0\text{ GB}$. Assuming the step must drain cooperatively to maintain causal consistency before memory evacuation, calculate the worst-case interrupt latency $T_{\text{int}}$ until the agent safely enters `SUSPENDED_IO`.
2. **Hard Real-Time NMI Preemption:** A Priority 0 NMI is asserted to stop an agent attempting to execute an adversarial payload. The system contract mandates a hard real-time preemption bound of $T_{\text{NMI\_bound}} \le 8\text{ ms}$.
   - Prove why the runtime cannot wait for token step draining ($T_{\text{drain}}$) or full memory evacuation ($M_{\text{KV}} / \mathcal{B}_{\text{bus}}$).
   - Architect the preemption sequence required to satisfy this bound, calculating the total time required to sever the inference TCP socket ($1.2\text{ ms}$), issue a driver-level CUDA stream abort ($0.8\text{ ms}$), and dispatch `SIGKILL` to the tool microVM ($2.5\text{ ms}$).
3. **Speculative Branch Teardown:** A speculative execution branch is executing read-only analysis inside an ephemeral microVM with a Copy-on-Write tmpfs overlay. The speculative thread has modified $384\text{ MB}$ of memory pages. If the human supervisor rejects the pending escalation gate, calculate the time required to unmount the tmpfs overlay and reclaim the memory frames given a page deallocation throughput of $150\text{ GB/s}$.

### Problem 3: Multi-agent coffman deadlock and watchdog budget optimization

A multi-agent automated continuous integration system executes three autonomous agents concurrently:

- **Agent $A_1$ (Database Migration Agent):** Modifies relational schemas and backfills customer records.
- **Agent $A_2$ (API Gateway Agent):** Updates routing tables and rate-limiting rules.
- **Agent $A_3$ (End-to-End Integration Agent):** Executes live end-to-end regression test suites.

The agents share access to three exclusive system resources:

- $R_{\text{db}}$: Exclusive write lock on the staging relational database.
- $R_{\text{route}}$: Exclusive lock on the edge API routing configuration.
- $R_{\text{human}}$: The enterprise human supervisory approval queue (single ticket capacity).

At time $t_0$, the system state is:

- $A_1$ holds an exclusive lock on $R_{\text{db}}$ and requests $R_{\text{human}}$ to approve a non-invertible table drop.
- $A_2$ holds an exclusive lock on $R_{\text{route}}$ and requests $R_{\text{db}}$ to inspect updated foreign key constraints.
- $A_3$ holds $R_{\text{human}}$ (currently awaiting approval from a human engineer for an API credential rotation) and requests $R_{\text{route}}$ to execute an end-to-end validation run.
1. **Resource Allocation Graph & Coffman Analysis:**
   - Draw the directed Resource Allocation Graph (RAG) representing agents and resources.
   - Formally prove that the system is deadlocked by demonstrating the presence of all four Coffman conditions.
2. **Watchdog Preemption and Sagas Rollback:** The runtime enforces an authoritative watchdog timer of $\tau_{\text{watchdog}} = 900\text{ seconds}$ on all pending human gates. Backward-compensating Saga transactions require $T_{\text{comp}} = 3.8\text{ seconds}$ to execute.
   - If the human supervisor for $A_3$ fails to respond within $\tau_{\text{watchdog}}$, detail the exact sequence of kernel actions, lease revocations, and compensating transactions that break the deadlock.
   - Calculate the total latency from watchdog expiration until the remaining agents can resume forward execution.
3. **Global Partial Ordering:** Define a strict global partial order $\prec$ over $\{R_{\text{db}}, R_{\text{route}}, R_{\text{human}}\}$ that mathematically guarantees circular wait is impossible. Demonstrate how the acquisition protocol must be restructured so that deadlock can never occur regardless of human deliberation latency.
