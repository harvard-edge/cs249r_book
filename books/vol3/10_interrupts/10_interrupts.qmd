...
- **`SUSPENDED_IO`:** The agent is waiting for an asynchronous, low-risk tool execution (e.g., compiling a large software binary or waiting for a read-only database query). The KV cache remains resident in GPU high-bandwidth memory (HBM) or host DRAM.
...
In modern inference architectures, execution is scheduled across microsecond and millisecond intervals. A forward-pass general matrix multiplication (GEMM) kernel executes on GPU Tensor Cores in tens of microseconds; an autoregressive token decode step completes in $15\text{--}40\text{ ms}$; a full inference turn streams back in under a second. When the system blocks to wait for a human supervisor, the temporal scale fractures across six orders of magnitude:
