...
To eliminate ambient authority ($A=0$), the multi-agent runtime models all tool invocations, filesystem access, and network egress as protected operations gated by unforgeable capability descriptors. Following the foundational security principles formulated by @saltzer1975, an agent cannot access any resource simply by virtue of its running process context. Instead, it must explicitly present a valid capability token with every remote procedure call (RPC) dispatched to the host runtime gateway.
...
|:----------------------------|:--------------------------|:--------------------------------------------------------------------|:-----------------------------------------------------------------------------|
| **1. Destructive Attempt**  | Worker Agent ($A_1$)      | Proposes destructive operation (e.g. `git push --force`)            | Unprivileged worker operates under $A=0$; lacks ambient push rights.         |
...
| **4. Envelope Escalation**  | Escrow Mediation Bus      | Marshals formal escalation envelope to Root Orchestrator ($A_0$)    | Provides structured diff, context rationale, and risk scorecard.             |
| **5. Human Confirmation**   | Root Orchestrator ($A_0$) | Reviews synthesized context; solicits authenticated human signature | Operations committed only upon cryptographically verified operator approval. |
...
:::
A software engineering team deploys an ensemble of five specialized agents—a supervisor, three parallel code generators, and a test synthesizer—to resolve complex bug tickets, observing a modest 4 percent improvement in task completion over their initial prototype. When instrumented under rigorous systems profiling, however, the cluster reveals an alarming pathology: the multi-agent system consumes fourteen times more tokens, incurs a $3.8\times$ critical-path wall-clock latency penalty due to serialization and context-synchronization barriers, and repeatedly deadlocks when reconciling merge conflicts in shared repositories. More damning still, when the original single agent is granted identical test-time computational resources—permitting it to run an iterative critique-and-refine loop with tool execution as formulated in @sec-vol3-deliberation—the single agent matches the ensemble's pass rate while cutting token consumption by 68 percent and finishing in half the wall-clock time.
...
To determine whether multi-agent delegation provides true architectural value, the baseline must be an *optimized single-agent runtime*. As established in @sec-vol3-deliberation and @sec-vol3-actuation, an unprivileged neural inference engine can be organized into an iterative execution loop governed by host supervision. A properly configured single-agent baseline must be granted:
...
**Invariant**: A multi-agent topology $\mathcal{M}$ provides positive systems utility if and only if it strictly Pareto-dominates a budget-matched single-agent baseline along at least one axis of Task Completion Quality ($Q$), Critical-Path Makespan ($\tau_{\text{crit}}$), or Total Resource Expenditure ($E$) without catastrophic degradation on the remaining two.

**Implication**: Multi-agent architectures must not claim superiority based on nominal task success alone; empirical evaluations must enforce resource parity ($\mathcal{B}_{\text{tokens}}, T_{\text{sandbox}}$) and account for distributed coordination overheads, communication latency, and container virtualization taxes.
...
| **6. Tri-Axial Dominance**      | Does topology achieve Pareto improvement on $Q, \tau, E$?         | Verified pass rate or latency justifies token expenditure                            | Revert to Single-Agent Runtime     |
