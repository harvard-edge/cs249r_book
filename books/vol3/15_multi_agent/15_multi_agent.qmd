...
![**Coordination Topologies for Multi-Agent Systems**: (a) Hierarchical Supervisor-Worker: centralized task decomposition, dispatch, and reconciliation. (b) Sequential Linear Pipeline: streaming uni-directional artifact handoffs. (c) Shared Blackboard: concurrent workers operating against an immutable artifact store. (d) Decentralized Actor Mesh: point-to-point typed message queues over isolated mailboxes.](images/svg/vol3/ch15/coordination_topologies.svg){#fig-vol3-multi-agent-topologies width="100%"}
...
3. *Process Group Supervision Trees:* Dynamically spawned agents must be registered within an operating system process group or container namespace tied directly to the parent's lifecycle. If the parent terminates, fails, or receives a cancellation signal, the host supervisor atomically propagates a termination signal (`SIGKILL` or container cancellation RPC) down the entire process subtree, instantaneously reclaiming GPU memory and preventing orphaned executions, as formalized in @fig-vol3-supervisor-tree-isolation.

![**Hierarchical Supervision Trees and Actor Isolation in Multi-Agent Systems**: Erlang-style hierarchical actor tree with private mailboxes, supervision restart policies (`one_for_one` vs. `one_for_all`), concurrency leases, and isolation firewalls. Root and sub-supervisors enforce fail-stop encapsulation, bounded restart intensities, and distributed workspace leases to prevent orphaned processes and cascade failures.](images/svg/supervisor_tree_isolation.svg){#fig-vol3-supervisor-tree-isolation width="95%"}
...
![**Optimistic Concurrency Control with Isolated Worktrees**: Concurrency lifecycle showing read snapshotting, sandboxed execution, validation against head state, and automated three-way merge reconciliation.](images/svg/fig-vol3-occ-worktrees.svg){#fig-vol3-occ-worktrees width="95%"}
...
![**Correlated Ensemble Failures vs. Independent Error Scaling**: Theoretical exponential decay under independent errors versus the irreducible error floor observed in homogeneous foundation model ensembles under positive pairwise correlation ($\rho = 0.40$).](images/svg/vol3/ch15/fig-vol3-correlated-ensemble-failure.svg){#fig-vol3-correlated-ensemble-failure width="95%"}
...
![**Hierarchical Capability Attenuation Tree**: The root supervisor derives scoped Macaroon tokens for child workers by appending HMAC-chained caveats restricting tool access, path prefixes, and execution budgets.](images/svg/capability-attenuation-tree.svg){#fig-vol3-capability-attenuation-tree width="95%"}

A child agent must never possess ambient authority, nor may it ever acquire permissions exceeding those of its parent. In a dependable multi-agent architecture, authority is modeled as a monotonically decreasing capability lattice where every delegated token cryptographically attenuates permissions across operational tools, filesystem scopes, network egress, and temporal lifetimes, as depicted in @fig-vol3-capability-attenuation-tree.
...
yielding the classical speedup ratio illustrated in @fig-vol3-amdahl-multiagent-scaling:
$$\text{Speedup}_{\text{ideal}}(M) = \frac{T_1}{T_M} = \frac{1}{(1 - f) + \frac{f}{M}}$$

![**Amdahl Scaling and the Universal Scalability Law in Multi-Agent Execution**: In the presence of coordination overhead $h(M)$, speedup peaks at an optimal worker count $M^*$ and degrades thereafter due to merge contention and context reconciliation overhead.](images/svg/fig-vol3-amdahl-multiagent-scaling.svg){#fig-vol3-amdahl-multiagent-scaling width="95%"}
