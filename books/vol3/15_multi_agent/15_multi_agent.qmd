...
::: {#exmp-15-the-amdahl-limits-of-multi-agent .callout-example title="The Amdahl limits of multi-agent parallelism"}
...
::: {#exmp-15-critical-path-makespan-and-dag-scheduling .callout-example title="DAG scheduling in multi-agent execution"}
...
::: {#exmp-15-context-desorption-and-serialization-overhead .callout-example title="Context desorption in task delegation"}
...
::: {#exmp-15-provisioning-latency-and-io-bandwidth .callout-example title="Provisioning latency in worker fleets"}
...
::: {#exmp-15-the-illusion-of-independence-in .callout-example title="The illusion of independence in majority voting"}
...
::: {#callout-def-vol3-heterogeneous-gating .callout-definition title="Definition 15.1: Heterogeneous invariant-gated coordination"}
Let a multi-agent system coordinate across multiple candidate proposals $\mathcal{P} = \{p_1, p_2, \dots, p_K\}$ for workspace mutation $\mathcal{W} \to \mathcal{W}'$:

1. **Unacceptable Architecture (Homogeneous Consensus Gate)**:
   Evaluating proposals via subjective voting among models sharing identical pretraining weights $\theta$:
   $$\text{Commit}(\mathcal{P}) = \text{Vote}_{\theta}(\mathcal{P})$$
   is vulnerable to correlated failure modes and Byzantine context poisoning.
2. **Architectural Discipline (Heterogeneous Proposal, Invariant-Gated Commit)**:
   The runtime decouples heuristic proposal generation from deterministic verification:

   - Proposals are generated across diverse model families (e.g. $\theta_A, \theta_B, \theta_C$).
   - A heuristic schema merger resolves syntactic divergences.
   - Commit to production integration branch is mediated by deterministic mechanical oracles:
     $$\text{Commit}(\mathcal{P}) \iff \text{ExitCode}\left(\text{Oracle}(\text{Apply}(\mathcal{W}, \text{Merge}(\mathcal{P})))\right) == 0$$
:::
...
::: {#callout-def-vol3-cancellation-context-tree .callout-definition title="Definition 15.2: Hierarchical cancellation context tree"}
A multi-agent execution topology is formalized as a directed rooted tree $\mathcal{T} = (\mathcal{V}, \mathcal{E})$ with root supervisor $v_0$. Each vertex $v_i \in \mathcal{V}$ maintains a cancellation context tuple:
$$C_i = \langle \text{id}_i, \sigma_i, \tau_{\text{deadline}}, \text{reason} \rangle$$
where lifecycle state $\sigma_i \in \{\textsc{Active}, \textsc{Canceling}, \textsc{Canceled}\}$.

The runtime enforces the **Downward Invariant Closure Principle**:
$$\forall (v_i, v_j) \in \mathcal{E}: \quad \sigma_i \in \{\textsc{Canceling}, \textsc{Canceled}\} \implies \sigma_j \to \textsc{Canceling}$$
guaranteeing that terminating an ancestor node instantaneously drains and cancels all descendant branches without orphaned resource leaks.
:::
...
::: {#exmp-15-hedged-subtasks-and-tail-latency .callout-example title="Hedged subtasks and tail latency reduction"}
...
::: {#exmp-15-cryptographic-macaroon-verification-latency-at .callout-example title="Macaroon verification at the tool gateway"}
...
::: {#callout-def-vol3-tri-axial-evaluation .callout-definition title="Definition 15.3: Tri-axial multi-agent evaluation frontier"}
A comprehensive systems evaluation of an agentic topology $\mathcal{M}$ requires tracking its position along three coupled, non-linear axes:

1. **Task Completion Quality ($Q \in [0, 1]$)**:
   $$Q = \frac{1}{|\mathcal{D}|} \sum_{i=1}^{|\mathcal{D}|} \mathbb{I}\left( \text{Verify}(\mathcal{A}_i) == \text{SUCCESS} \right)$$
2. **Critical-Path Makespan ($\tau$)**:
   $$\tau = t_{\text{completion}} - t_{\text{dispatch}}$$
   representing wall-clock latency along the longest dependency chain.
3. **Total Resource Expenditure ($E$)**:
   $$E = N_{\text{tokens}} \cdot C_{\text{token}} + T_{\text{sandbox}} \cdot C_{\text{compute}}$$
   capturing aggregate monetary and computational cost.

A multi-agent architecture is Pareto-superior if and only if it strictly dominates the budget-matched single-agent baseline on at least one axis without catastrophic degradation on the remaining two.
:::
...
::: {#exmp-15-quantitative-evaluation-of-a-parallel .callout-example title="Evaluation of a parallel refactoring fleet"}
...
::: {#callout-def-vol3-orphaned-agent-hazard .callout-definition title="Definition 15.4: Orphaned worker state corruption hazard"}
