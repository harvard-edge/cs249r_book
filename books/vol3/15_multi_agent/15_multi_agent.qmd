...
_When does decomposing a complex workload across multiple specialized agents actually improve task outcomes, and when does it merely multiply coordination overhead?_

Decomposing large engineering problems across specialized, collaborating agents is conceptually appealing: a planner breaks down requirements, a coder drafts implementations, a reviewer audits diffs, and a tester runs verification suites. In practice, multi-agent architectures frequently degrade system performance: delegating tasks across autonomous entities introduces severe communication overhead, compounding hallucination cascades, and deadlocks where agents endlessly debate conflicting suggestions. According to Amdahl's Law, the speedup achievable through multi-agent parallelization is strictly bounded by the serial fraction of the task, such as dependency graph constraints, global context synchronization, and final commit arbitration. Furthermore, unmediated inter-agent communication consumes millions of tokens, saturating serving capacity while creating diffuse accountability where no single model takes responsibility for task completion. Systems engineers must design formal multi-agent coordination protocols: organizing agents into hierarchical supervisor trees with explicit reporting contracts, isolating concurrent workers within git-based optimistic concurrency branches, and progressively attenuating authority across delegates. Building dependable multi-agent systems requires co-designing communication topologies, structured message passing interfaces, and deterministic invariant gates to ensure collaborative parallelism delivers genuine engineering throughput rather than wasteful coordination tax.
...
- Deconstruct the mathematics of correlated ensemble failures, proving why multi-agent majority voting fails to close semantic invariants when models share pretraining weights.
...
The single-agent runtime of the earlier parts already closes the loop around a model that holds zero ambient authority, committing only what passes runtime gates outside the model (@sec-vol3-intro-invariant-closure). Splitting the loop across agents changes none of that. What it changes is exposure, since a fleet multiplies each of the H·S·A exposures of @sec-vol3-intro-hsa. The horizon now runs across handoffs, so the open-loop ceiling of @sec-vol3-intro-temporal-stretching applies at every agent boundary, and a file path one worker hallucinates becomes the accepted premise of its peers. State is duplicated into every child's context and read by agents after it has gone stale. Authority passes down every delegation and must not grow on the way. The chapter asks what the split costs, when it pays for itself, and what the runtime must add so that a fleet is no less dependable than one agent.
...
The scaling limit of this tax follows from Amdahl's law, which @sec-vol3-intro-duration-accounting applied to the model's share of a single trajectory. Here it applies to the split itself. If a fraction $1 - f$ of an agentic workflow is inherently sequential (initial requirement analysis, global task decomposition, and final multi-branch verification) and the remaining fraction $f$ can be distributed across $M$ parallel agents, the speedup cannot exceed
...
```{python}
#| label: lego-15-the-amdahl-limits-of-multi-agent
#| echo: false
# │ Show: 4-agent parallel fleet achieves 2.07x speedup at 2.20x token expansion.
# │ How: Amdahl makespan with RPC dispatch, duplicated prefill, and reconciliation overhead.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_multiple, fmt_percent

class AmdahlLimitsMultiAgentParallelism:
    t_single = 645 * second
    t_multi = 311 * second
    speedup = t_single.to(second).magnitude / t_multi.to(second).magnitude
    tokens_single = 53_900
    tokens_multi = 118_400
    token_expansion = tokens_multi / tokens_single
    cost_increase = (tokens_multi - tokens_single) / tokens_single
    latency_reduction = (t_single.to(second).magnitude - t_multi.to(second).magnitude) / t_single.to(second).magnitude

    check(abs(speedup - 2.07395) < 0.001, "speedup")
    check(abs(token_expansion - 2.19666) < 0.001, "token_expansion")
    check(abs(cost_increase - 1.19666) < 0.001, "cost_increase")
    check(abs(latency_reduction - 0.5178) < 0.001, "latency_reduction")

    speedup_mult_str = fmt_multiple(round(speedup, 2), precision=2)
    tokens_single_str = fmt_int(tokens_single)
    tokens_multi_str = fmt_int(tokens_multi)
    token_expansion_mult_str = fmt_multiple(round(token_expansion, 2), precision=2)
    cost_increase_pct_str = fmt_percent(round(cost_increase, 2), precision=0, style="prose")
    latency_reduction_pct_str = fmt_percent(latency_reduction, precision=1, style="prose")
```

::: {#nbk-15-the-amdahl-limits-of-multi-agent .callout-notebook title="The Amdahl limits of multi-agent parallelism"}
...
$$\text{Speedup}(4) = \frac{645\text{ s}}{311\text{ s}} \approx 2.07\times.$$

Systems Cost Accounting: While the parallel fleet compresses wall-clock makespan by {python} AmdahlLimitsMultiAgentParallelism.speedup_mult_str, the underlying hardware accounting reveals the real systems trade-off. Due to redundant context prefill across the four workers (48,000 duplicate prefill tokens) and supervisory reconciliation overhead, total token consumption surges from {python} AmdahlLimitsMultiAgentParallelism.tokens_single_str to {python} AmdahlLimitsMultiAgentParallelism.tokens_multi_str—an expansion of {python} AmdahlLimitsMultiAgentParallelism.token_expansion_mult_str. The multi-agent architecture trades a {python} AmdahlLimitsMultiAgentParallelism.cost_increase_pct_str increase in compute cost and memory footprint to achieve a {python} AmdahlLimitsMultiAgentParallelism.latency_reduction_pct_str reduction in wall-clock latency. If task dependencies had forced serialized reconciliation between branches, $T_{\text{multi}}$ would have exceeded $T_{\text{single}}$, inverting the speedup curve despite burning more than double the compute cycles.
:::
...
: **Architectural Comparison: Single-Agent Deliberation vs. Multi-Agent Delegation**: Systems trade-offs across latency, cost, working memory, fault isolation, and consistency. {#tbl-vol3-delegation-matrix}

Whether a task runs as one reflective loop or as a fleet of twenty communicating workers, correctness is established only by the end-to-end evidence of @sec-vol3-intro-end-to-end-boundary, gathered outside every model. Three agents that repeatedly approve one another's code changes do not produce that evidence. They produce a distributed echo chamber of correlated errors, which @sec-vol3-multiagent-consensus quantifies.
...
::: {#fig-vol3-multi-agent-topologies fig-env="figure" fig-pos="htb" fig-cap="**Coordination Topologies for Multi-Agent Systems**: Structural graph topologies governing distributed foundation model interaction. Panel A illustrates centralized hierarchical dispatch; Panel B traces stage-gated linear pipelines with compensating rollback sagas; Panel C depicts decoupled asynchronous tuple-space mutation via atomic compare-and-swap; Panel D shows the quadratic channel expansion and semantic livelock hazard of unconstrained actor meshes." fig-alt="Four-panel systems schematic comparing Hierarchical Supervisor-Worker with O(M) tree edges, Sequential Linear Pipeline with verification gates, Shared Blackboard with atomic compare-and-swap operations, and Decentralized Actor Mesh with O(M^2) unmediated communication channels."}
![](images/svg/coordination_topologies.svg)
:::

### Taxonomy of coordination topologies

The architectural design of a multi-agent system begins by selecting an interconnection pattern that reflects the data-flow dependencies of the target application. As formalized in @fig-vol3-multi-agent-topologies, distributed agent runtimes structure execution across four canonical topological archetypes:
In the Hierarchical Supervisor-Worker model (Panel A), execution is organized as a directed star tree where root supervisor $A_0$ dispatches typed task envelopes to isolated worker enclaves ($W_1, W_2, W_3$), bounding communication edges strictly to $|E| = M - 1$ while trapping child exceptions. In the Sequential Linear Pipeline (Panel B), work progresses along a staged DAG where deterministic gates ($G_1, G_2$) verify intermediate artifacts before emission, triggering backward compensating rollback sagas ($\mathcal{A}^{-1}$) upon failure to prevent downstream taint. In the Shared Blackboard architecture (Panel C), concurrent workers operate without bilateral coupling, synchronizing state purely through atomic compare-and-swap (CAS) transactions against a centralized tuple space. Finally, in the Decentralized Actor Mesh (Panel D), unconstrained peer-to-peer communication produces a quadratic channel explosion ($|E| = M(M-1)/2$), creating severe risks of semantic deadlocks, context dilution, and unbounded failure propagation.
...
: **Comparison of Multi-Agent Coordination Topologies**: Critical-path latencies, memory footprints, failure isolation properties, and optimal workload profiles. {#tbl-vol3-topologies-comparison}
...
```{python}
#| label: lego-15-critical-path-makespan-and-dag-scheduling
#| echo: false
# │ Show: Critical path makespan bounds parallel speedup to 1.58x across 4 workers.
# │ How: DAG critical path analysis vs sequential execution work with decode/prefill rates.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_multiple

class DagSchedulingMultiAgentExecution:
    t_seq = 275.0 * second
    t_parallel = 174.5 * second
    speedup = t_seq.to(second).magnitude / t_parallel.to(second).magnitude

    check(abs(t_seq.to(second).magnitude - 275.0) < 1e-4, "t_seq")
    check(abs(t_parallel.to(second).magnitude - 174.5) < 1e-4, "t_parallel")
    check(abs(speedup - 1.57593) < 0.001, "speedup")

    speedup_mult_str = fmt_multiple(round(speedup, 2), precision=2)
```

::: {#nbk-15-critical-path-makespan-and-dag-scheduling .callout-notebook title="DAG scheduling in multi-agent execution"}
...
Despite deploying four parallel GPU workers, the speedup is capped at {python} DagSchedulingMultiAgentExecution.speedup_mult_str. The remaining compute capacity was lost to sequential dependencies ($v_1$ and $v_7$) and worker idle bubbles while waiting for join synchronization at $v_5$ and $v_7$.
:::
...
However, dynamic spawning introduces critical systems hazards that can rapidly destabilize the host runtime, as structured across the hierarchy in @tbl-vol3-subagent-spawn-lifecycle:
...
: **Recursive Dynamic Spawning and Budget Attenuation**: Structural hierarchy, parent-child delegation edges, and monotonic resource bounding across generations. {#tbl-vol3-subagent-spawn-lifecycle}
...
::: {#chk-15-amdahl-concurrency .callout-checkpoint title="Evaluating multi-agent coordination topologies"}
Before examining typed task envelopes and cross-agent contracts, verify your understanding of delegation trade-offs:

- [ ] **Amdahl bottleneck**: Why does decomposing a task across $N$ parallel subagents fail to achieve linear speedup when dependency graphs contain serial coordination paths?
- [ ] **Context amplification**: How does multiplexing context across independent agent prompts multiply total input token expenditure relative to a unified single-agent loop?
- [ ] **Supervision trees**: How do Erlang-style `one_for_one` versus `one_for_all` restart strategies isolate failures across coupled agent workers?
- [ ] **Distributed leases**: Why must child worker execution leases be bounded by periodic heartbeats to prevent orphaned GPU token consumption?
:::
...
**The systems defense against conversational decay extends the typed action contract (principle \ref{pri-vol3-strict-action-abi}) from calls between a model and its tools to handoffs between agents, formalized as typed remote procedure calls.** Drawing directly on the classical RPC architecture established by @birrell1984rpc, distributed agent runtimes cannot rely on shared conversational memory. Every delegation across a task dependency edge must be marshalled into an explicit, schema-validated task envelope that completely specifies identity, immutable inputs, delegated capabilities, resource budgets, and machine-checkable completion assertions before the downstream agent execution loop is scheduled.
...
::: {#psp-15-the-fallacy-of-conversational-coordination .callout-perspective title="The fallacy of conversational coordination"}
...
: **Conversational Handoffs versus Typed Task Envelopes**: Interface definitions, argument marshalling, state propagation, and completion invariants across delegation paradigms. {#tbl-vol3-handoff-comparison}
...
```json
{
  ...
  "contract": {
    ...
    "declared_inputs": ["repo:git@github.com:org/auth.git@ref/c84fa"],
...
#### Task metadata ($\mathcal{M}$) {.unnumbered}
...
#### Immutable input references ($\mathcal{R}_{\text{in}}$) {.unnumbered}
...
#### Delegated capability token ($\mathcal{C}_{\text{delegated}}$) {.unnumbered}
...
#### Resource budget ceiling ($\mathcal{B}$) {.unnumbered}
...
#### Completion verification schema ($\mathcal{S}_{\text{verify}}$) {.unnumbered}
...
```{python}
#| label: lego-15-context-desorption-and-serialization-overhead
#| echo: false
# │ Show: Pass-by-reference task envelopes achieve 137x reduction in input token broadcast.
# │ How: Pass-by-value context duplication vs typed immutable git commit pointers and HBM.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_multiple, fmt_usd

class ContextDesorptionTaskDelegation:
    tokens_val = 288_000
    tokens_ref = 2_100
    reduction = tokens_val / tokens_ref

    rate_per_m = 3.00
    cost_val = (tokens_val / 1_000_000) * rate_per_m
    cost_ref = ((6 * 3_950) / 1_000_000) * rate_per_m

    check(abs(reduction - 137.1428) < 0.01, "reduction")
    check(abs(cost_val - 0.864) < 1e-4, "cost_val")
    check(abs(cost_ref - 0.0711) < 1e-4, "cost_ref")

    reduction_mult_str = fmt_multiple(round(reduction))
    cost_val_str = fmt_usd(cost_val, precision=3)
    cost_ref_str = fmt_usd(round(cost_ref, 3), precision=3)
```

::: {#nbk-15-context-desorption-and-serialization-overhead .callout-notebook title="Context desorption in task delegation"}
...
2. Across all $W = 6$ workers, total input tokens dispatched:
   ...
   Representing a **{python} ContextDesorptionTaskDelegation.reduction_mult_str reduction** in network transmission and accelerator context ingestion.
...
4. On-Demand State Retrieval:
   ...
   Total input token cost across the cluster drops from {python} ContextDesorptionTaskDelegation.cost_val_str to {python} ContextDesorptionTaskDelegation.cost_ref_str, while TTFT drops from $15.0$ seconds to under $120$ milliseconds.
:::
...
: **Typed Task Envelope Lifecycle Transitions**: Formal validation, dispatch, execution, verification, and completion pipeline governing task handoffs. {#tbl-vol3-task-envelope-lifecycle}
...
::: {.column-margin}
![](images/svg/vol3_multi_agent_margin_002.svg){width="100%" fig-alt="Line plot showing write collision probability surging above 80% beyond five workers under Zipfian contention, compared to 12% under uniform access."}
...
```{python}
#| label: lego-15-provisioning-latency-and-io-bandwidth
#| echo: false
# │ Show: Git worktrees reduce fleet provisioning storage I/O by 99.77% over recursive copies.
# │ How: Shared object database and hardlinks vs naive full repository cloning.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_memory, fmt_percent

class ProvisioningLatencyWorkerFleets:
    n_workers = 32
    s_repo = 1.8 * GB
    io_naive = n_workers * s_repo
    io_worktree = n_workers * 4.2 * MB
    io_reduction = (io_naive.to(MB).magnitude - io_worktree.to(MB).magnitude) / io_naive.to(MB).magnitude

    check(abs(io_naive.to(GB).magnitude - 57.6) < 1e-4, "io_naive")
    check(abs(io_worktree.to(MB).magnitude - 134.4) < 1e-4, "io_worktree")
    check(abs(io_reduction - 0.997666) < 1e-4, "io_reduction")

    io_naive_gb_str = fmt_memory(io_naive, unit=GB, precision=1)
    io_worktree_mb_str = fmt_memory(io_worktree, unit=MB, precision=1)
    io_reduction_pct_str = fmt_percent(round(io_reduction, 4), precision=2, style="prose")
```

::: {#nbk-15-provisioning-latency-and-io-bandwidth .callout-notebook title="Provisioning latency in worker fleets"}
...
Total I/O bandwidth across all 32 workers is reduced from {python} ProvisioningLatencyWorkerFleets.io_naive_gb_str to {python} ProvisioningLatencyWorkerFleets.io_worktree_mb_str, a {python} ProvisioningLatencyWorkerFleets.io_reduction_pct_str reduction in storage I/O, dropping fleet initialization latency below $50\text{ ms}$.
:::
...
With isolated workspaces established, the runtime coordinates concurrent execution by implementing an Optimistic Concurrency Control (OCC) trajectory protocol. Adapting Kung and Robinson's classic transaction formulation to agentic systems, as illustrated in @fig-vol3-occ-worktrees, the protocol proceeds through four formal phases: *Read*, *Execute*, *Validate*, and *Commit/Reconcile*.

::: {#fig-vol3-occ-worktrees fig-env="figure" fig-pos="htb" fig-cap="**Optimistic Concurrency Control with Isolated Worktrees**: Four-phase trajectory concurrency protocol and Git commit Directed Acyclic Graph (DAG) reconciliation. The top panels detail the transaction lifecycle across the Read, Execute, Validate, and Commit/Reconcile phases; the bottom panels illustrate the underlying physical filesystem geometry of shared `.git/objects` and the automated three-way merge resolution rules (`Merge(C_{\text{base}}, C_{\text{head}}, C_i)`) governing branch convergence." fig-alt="Systems architecture diagram illustrating the four-phase Optimistic Concurrency Control protocol using isolated git worktrees, showing read snapshotting, sandboxed execution, OCC disjointness validation, atomic CAS fast-forward, and three-way merge DAG resolution."}
![](images/svg/fig-vol3-occ-worktrees.svg)
:::

As formalized in @fig-vol3-occ-worktrees, optimistic concurrency transforms code refactoring from destructive shared-directory mutation into a verifiable transaction pipeline, resolving through fast-forward commit in panel (a), a disjoint three-way merge in panel (b), or conflict abort in panel (c). In Phase 1 (Read Snapshot), worker agent $W_i$ provisions a private branch $B_i$ rooted at immutable base commit $C_{\text{base}}$. As detailed in the bottom-left storage geometry panel, Git Worktrees share the underlying `.git/objects` blob database across all workers via hardlinks, dropping workspace provisioning latency to approximately $14\text{ ms}$ with zero data duplication. In Phase 2 (Execute), the worker generates code diffs and executes hermetic test suites strictly within its container enclave, emitting a signed `TaskReceipt` recording its read set $\mathcal{R}_i$ and write set $\mathcal{W}_i$. In Phase 3 (Validation), the host supervisor interrogates the main integration branch ($C_{\text{head}}$) to identify any intervening commits $\Delta_{\text{merged}} = C_{\text{head}} \setminus C_{\text{base}}$ merged while $W_i$ was executing. If the worker's write set does not intersect intervening writes ($\mathcal{W}_i \cap \mathcal{W}_{\text{merged}} = \emptyset$), the invariant holds. In Phase 4 (Commit/Reconcile), the runtime executes atomic branch convergence as mapped in the bottom-right commit DAG: advancing $C_{\text{base}}$ via an atomic compare-and-swap pointer update if no drift occurred (Case 1), executing an automated three-way merge $C_{\text{new}} = \text{Merge}(C_{\text{base}}, C_{\text{head}}, C_i)$ under disjoint writes (Case 2), or dispatching an agentic reconciliation subtask when write collisions occur (Case 3). The quantitative advantages of this worktree protocol over naive shared directories are summarized in @tbl-vol3-occ-vs-naive.
...
: **Quantitative Systems Comparison of Direct Workspace Mutation versus Optimistic Git Worktrees**: Provisioning latencies, concurrency hazards, and conflict resolution mechanisms under concurrent multi-agent workloads. {#tbl-vol3-occ-vs-naive}
...
: **Concurrency Control Strategies for Multi-Agent Fleets**: Acquisition phases, abort penalties, fleet parallelism, and failure modes across optimistic and pessimistic mechanisms. {#tbl-vol3-concurrency-controls}
...
: **Classical Hardware NMR versus Agent Fleet Consensus**: Divergence of error models and quorum effectiveness under correlated neural blind spots. {#tbl-vol3-nmr-vs-fleet-consensus}
...
class CorrelatedFailuresMajorityVoting:
    ...
    p_k4 = 5 * (p**4) * (1 - p)
    p_k5_indep = p**5
    p_indep = p_k3 + p_k4 + p_k5_indep

    p_corr = 0.0729 + 0.0546 + 0.0392
    p_k5_corr = 0.0392
    ratio = p_k5_corr / p_k5_indep

    check(abs(p_indep - 0.05792) < 1e-5, "p_indep")
    check(abs(p_corr - 0.1667) < 1e-4, "p_corr")
    check(abs(ratio - 122.5) < 0.5, "ratio")

    p_maj_indep_pct_str = fmt_percent(round(p_indep, 4), precision=2, style="prose")
    p_maj_corr_pct_str = fmt_percent(round(p_corr, 4), precision=2, style="prose")
    unanimous_amp_mult_str = fmt_multiple(120, precision=0)
```
...
**Takeaway:** Correlation nearly triples the ensemble error rate ({python} CorrelatedFailuresMajorityVoting.p_maj_indep_pct_str to {python} CorrelatedFailuresMajorityVoting.p_maj_corr_pct_str) and increases the probability of a unanimous, confident false approval by more than **{python} CorrelatedFailuresMajorityVoting.unanimous_amp_mult_str**. Relying on majority voting among homogeneous agents creates a dangerous illusion of safety.
:::
...
: **Consensus Protocols versus Invariant Verification**: Grounding authorities, epistemic bases, failure modes, and compute scaling across validation strategies. {#tbl-vol3-consensus-vs-invariants}
...
***Heterogeneous invariant-gated coordination***\index{Heterogeneous gating!definition}\index{Multi-agent systems!invariant-gated coordination definition} is a multi-agent architectural discipline wherein diverse, independently parameterized model families propose candidate modifications whose integration is governed strictly by deterministic external invariant verifiers rather than peer consensus voting.
...
:::
#### Structural heterogeneity {.unnumbered}
...
#### Invariant-first gating {.unnumbered}
...
::: {#chk-15-orphaned-worker-hazard .callout-checkpoint title="Evaluating concurrency control and consensus failure"}
...
- [ ] **Optimistic concurrency control**: How does validating git tree SHAs before commit prevent concurrent subagents from overwriting workspace files?
- [ ] **Correlated errors**: Why does Condorcet's Jury Theorem collapse when voting agents share foundation model parameter weights and pretraining biases?
- [ ] **Mechanical oracles**: Why must reviewer agent approvals be subordinated to deterministic test execution and static AST security analyzers?
- [ ] **Phantom mutations**: How do delayed commits from orphaned workers silently overwrite rolled-back state long after a parent supervisor aborts?
:::
...
***Hierarchical cancellation context tree***\index{Cancellation context tree!definition}\index{Multi-agent systems!cancellation tree definition} is a directed dependency structure $\mathcal{T} = (\mathcal{V}, \mathcal{E})$ maintaining lifecycle tuples $C_i = \langle \text{id}_i, \sigma_i, \tau_{\text{deadline}}, \text{reason} \rangle$ across distributed agent processes to enforce downward invariant closure upon fault detection or preemption.
...
: **Hierarchical Cancellation Cascade Protocol**: Ordered propagation sequence arresting GPU decode loops, evicting KV cache blocks, and reclaiming sandbox resources upon task abort. {#tbl-vol3-cancellation-cascade}
...
```{python}
#| label: lego-15-hedged-subtasks-and-tail-latency
#| echo: false
# │ Show: Quorum fan-out with cancellation cascade improves 10s completion from 43.05% to 96.19%.
# │ How: Binomial order statistics of log-normal completion latencies and straggler pruning.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_percent

class HedgedSubtasksTailLatencyReduction:
    p_unhedged = 0.90**8
    p_quorum = 0.1488 + 0.3826 + 0.4305
    makespan_cut = 0.60

    check(abs(p_unhedged - 0.430467) < 1e-4, "p_unhedged")
    check(abs(p_quorum - 0.9619) < 1e-4, "p_quorum")

    p_unhedged_pct_str = fmt_percent(round(p_unhedged, 4), precision=2, style="prose")
    p_quorum_pct_str = fmt_percent(p_quorum, precision=2, style="prose")
    makespan_cut_pct_str = fmt_percent(makespan_cut, precision=0, style="prose")
```

::: {#nbk-15-hedged-subtasks-and-tail-latency .callout-notebook title="Hedged subtasks and tail latency reduction"}
...
The probability of completing by $10.0\text{ s}$ surges from {python} HedgedSubtasksTailLatencyReduction.p_unhedged_pct_str to {python} HedgedSubtasksTailLatencyReduction.p_quorum_pct_str, cutting the effective 95th-percentile makespan by more than {python} HedgedSubtasksTailLatencyReduction.makespan_cut_pct_str. Furthermore, terminating the remaining 2 stragglers at $t = 10.0\text{ s}$ prevents them from executing an additional average of $25\text{ seconds}$ of unnecessary generation. Across $1{,}000$ fan-out invocations, canceling the two stragglers saves:
...
In this protocol, a consumer agent grants a finite number of execution credits $W \in \mathbb{N}$ to upstream producers. The producer is permitted to emit messages if and only if its locally tracked credit balance satisfies $W_{\text{local}} > 0$. Each emitted message decrements $W_{\text{local}}$ by one. When the consumer successfully parses an envelope, completes its local inference trajectory, and flushes its state, it transmits an explicit `FLOW_CREDIT` frame back to the producer, replenishing $W_{\text{local}}$. The step-by-step frame exchanges and credit states are traced in @tbl-vol3-flow-control-protocol.
...
: **Credit-Based Multi-Agent Flow Control Protocol**: Step-by-step credit grant, consumption, and replenishment sequence preventing memory buffer bloat. {#tbl-vol3-flow-control-protocol}
...
2. **Ephemeral Worktree Cleanup:** If an agent was allocated an isolated git worktree for speculative editing (as detailed in @sec-vol3-multiagent-concurrency), the sweeper detects unreferenced directory paths under `.git/worktrees/` whose holding agent lease has lapsed, pruning the worktree and deleting the scratch branch.
...
::: {.column-margin}
![](images/svg/vol3_multi_agent_margin_003.svg){width="100%" fig-alt="Three stacked tiers of decreasing width, labelled root, worker and leaf."}
...
When a parent agent holding capability $\mathcal{C}_i$ spawns a child worker, the delegation obeys the attenuation rule of @sec-vol3-virtualization-least-privilege, now enforced across agents by monotonic delegation (principle \ref{pri-vol3-monotonic-delegation}). Under this rule, the child capability $\mathcal{C}_{i+1}$ represents a formal subset, a restriction, within the lattice of authority:
...
The structural distinction between these security postures is summarized in @tbl-vol3-ambient-vs-attenuated.
...
: **Comparison of Authorization Models in Agent Fleets**: Tool surfaces, filesystem scopes, network egress, and containment bounds under ambient authority versus attenuated capabilities. {#tbl-vol3-ambient-vs-attenuated}

The primary systems failure of ad-hoc multi-agent designs is the assumption that language model prompts can enforce authorization boundaries (e.g., instructing a worker in its system prompt: *"Do not run shell commands outside of `/build`"*). @Sec-vol3-intro-invariant-closure ruled that out for a single agent, and delegation does not change the answer. Authorization lives in the runtime, outside every model's context.
...
```{python}
#| label: lego-15-cryptographic-macaroon-verification-latency-at
#| echo: false
# │ Show: Chained macaroon HMAC verification requires under 2 us CPU latency per tool RPC.
# │ How: Iterative HMAC-SHA256 evaluation of delegation caveats on server hardware.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_memory, fmt_time

class MacaroonVerificationToolGateway:
    t_verify_bound = 2 * microsecond
    token_size = 256 * byte

    check(t_verify_bound.to(microsecond).magnitude == 2, "t_verify_bound")
    check(token_size.to(byte).magnitude == 256, "token_size")

    t_verify_bound_us_str = fmt_time(t_verify_bound, unit=microsecond, precision=0)
    token_size_b_str = fmt_memory(token_size, unit=byte, precision=0)
```

::: {#nbk-15-cryptographic-macaroon-verification-latency-at .callout-notebook title="Macaroon verification at the tool gateway"}
...
As established in @sec-vol3-multiagent-backpressure, when an orchestrator terminates an operational branch, all outstanding descendant processes must be aborted to conserve compute and prevent corrupted state mutations. If a delegated worker holds a valid cryptographic token with an unexpired TTL of 300 seconds, the worker could theoretically continue issuing valid tool RPCs to the gateway until the temporal caveat expires. Relying solely on expiration timestamps leaves a window of vulnerability during which orphaned or rogue subagents can mutate filesystems or exhaust resource quotas.

To enforce immediate, deterministic revocation without sacrificing $O(1)$ local verification, the runtime implements *Hierarchical Generation Epochs*, tracked via the lookup schema in @tbl-vol3-gateway-epoch-table.
...
: **Hierarchical Generation Epoch Revocation Check**: In-memory epoch validation table enabling $O(1)$ sub-tree authorization and instant revocation. {#tbl-vol3-gateway-epoch-table}
...
: **Human Escrow Escalation Protocol**: Secure containment boundary preventing direct user prompt injection and social engineering from unprivileged workers. {#tbl-vol3-escrow-escalation-protocol}
...
The architectural parameters governing valid baseline comparisons are contrasted in @tbl-vol3-baseline-protocols.
...
: **Architectural Comparison of Baseline Protocols for Agent Evaluation**: Deliberation budgets, environment feedback, search mechanisms, and token cost parity. {#tbl-vol3-baseline-protocols}
...
::: {#pri-15-tri-axial-evaluation .callout-principle title="Tri-axial multi-agent evaluation frontier"}
...
Verification is external and deterministic, performed by hermetic unit test suites, static analysis linters, or cryptographic signature validators, never by the agent's own report that a task is complete, which carries no evidential weight (@sec-vol3-intro-epistemic-boundary).
...
The third axis is *cost per accepted task* ($\mathcal{E}$), which charges every failed attempt to the tasks that pass (principle \ref{pri-vol3-trajectory-goodput}). The spend of one attempt $d$ sums the token and sandbox costs across all participating models and sandboxes:
...
The Amdahl bound of @sec-vol3-multiagent-need left its coordination term $h(M)$ abstract. Building it up from runtimes and then giving $h(M)$ a physical form explains why adding worker agents yields diminishing and eventually negative returns.
...
```{python}
#| label: lego-15-quantitative-evaluation-of-a-parallel
#| echo: false
# │ Show: Universal Scalability Law limits 4-agent speedup to 1.20x with 3.25x token amplification.
# │ How: Gunther USL contention and coherency coefficients vs single-agent baseline.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_multiple, fmt_percent

class EvaluationParallelRefactoringFleet:
    f_parallel = 0.70
    sigma_param = 0.04
    kappa_param = 0.02

    # M = 4
    h4 = sigma_param * 3 + kappa_param * 4 * 3
    denom4 = (1 - f_parallel) + f_parallel / 4 + h4
    speedup_4 = 1.0 / denom4
    t4 = 600 / speedup_4
    reduction = (600 - t4) / 600

    # Tokens
    tokens_single = 120_000
    tokens_multi = 40_000 + 50_000 + 4 * (60_000 + 15_000)
    token_amp = tokens_multi / tokens_single

    # M = 8
    h8 = sigma_param * 7 + kappa_param * 8 * 7
    denom8 = (1 - f_parallel) + f_parallel / 8 + h8
    speedup_8 = 1.0 / denom8

    check(abs(speedup_4 - 1.1976) < 0.001, "speedup_4")
    check(abs(reduction - 0.165) < 0.001, "reduction")
    check(abs(token_amp - 3.25) < 1e-4, "token_amp")
    check(abs(speedup_8 - 0.5594) < 0.001, "speedup_8")

    reduction_pct_str = fmt_percent(reduction, precision=1, style="prose")
    token_amplification_mult_str = fmt_multiple(token_amp, precision=2)
    speedup_mult_str = fmt_multiple(round(speedup_4, 2), precision=2)
    speedup_8_mult_str = fmt_multiple(round(speedup_8, 2), precision=2)
```

::: {#nbk-15-quantitative-evaluation-of-a-parallel .callout-notebook title="Evaluation of a parallel refactoring fleet"}
...
Despite provisioning four worker agents, the wall-clock execution time decreases by only 99 seconds—a meager {python} EvaluationParallelRefactoringFleet.reduction_pct_str reduction.
...
The multi-agent system incurs a **{python} EvaluationParallelRefactoringFleet.token_amplification_mult_str token amplification** (390,000 vs. 120,000) and quadruples the required sandbox concurrency to achieve a {python} EvaluationParallelRefactoringFleet.speedup_mult_str speedup.
...
At $M=6$, speedup peaks at only $1.24\times$ ($T_6 = 484\text{ s}$) before retrograding. Decomposing the workload across 8 workers yields $h(8) = 0.04(7) + 0.02(8)(7) = 0.28 + 1.12 = 1.40$, resulting in a speedup of {python} EvaluationParallelRefactoringFleet.speedup_8_mult_str ($T_8 = 1{,}073\text{ s}$)—nearly twice as slow as the single agent!
:::
...
Before decomposing an agentic workflow into a multi-agent cluster, systems architects must evaluate the six operational criteria outlined in @tbl-vol3-multi-agent-deployment-gates. If a proposed system fails any of these gates, the architectural default must remain an optimized single-agent runtime.
...
: **Six-Stage Multi-Agent Deployment Decision Gate**: Sequential qualification criteria for justifying multi-agent topology deployment over optimized single-agent baselines. {#tbl-vol3-multi-agent-deployment-gates}
...
4. *Error De-correlation Gate:* Are worker agents structurally protected against correlated failure modes? As proven in @sec-vol3-multiagent-consensus, deploying multiple instances of the same base model under identical prompting creates an illusion of consensus while sharing blind spots. Workers must use diverse prompts, heterogeneous model backends, or orthogonal decomposition strategies.
5. *Capability Attenuation Gate:* Does the architecture enforce strict least privilege across delegated child runtimes? Child agents must operate under cryptographically attenuated capability tokens ($\mathcal{C}_{\text{child}} \sqsubseteq \mathcal{C}_{\text{parent}}$) and execute within isolated sandboxes (@sec-vol3-multiagent-concurrency and @sec-vol3-multiagent-authority). An agent must never be granted ambient authority or the ability to escalate its own permissions.
6. *Net Tri-Axial Dominance Gate:* Does the multi-agent system dominate the budget-matched baseline on the tri-axial frontier (principle \ref{pri-15-tri-axial-evaluation})? Within the same total budget, and with no loss on the other axes, the collective must demonstrate either:
...
: **Optimistic Concurrency Control Workspace Timeline**: Sequence of isolated mutations and fast-forward versus conflicting reconciliation attempts. {#tbl-vol3-occ-timeline}
...
::: {.callout-takeaways title="Coordinate via typed dataflow, never unconstrained chatter"}
...
Multi-agent coordination turned out to be distributed systems engineering, not simulated organizational sociology, and the chapter put numbers on the coordination tax that engineering pays. Serialization, duplicated context, and reconciliation cap speedup after a few workers, so a fleet earns its place only through context partitioning, authority attenuation, or independent search, run as a typed dependency graph (principle \ref{pri-vol3-coordination-tax}). The correlated-ensemble analysis sharpens invariant closure (principle \ref{pri-invariant-closure}) for fleets, because agents that share weights share errors and no count of approvals can stand in for a sealed gate. Macaroons and generation epochs extend monotonic delegation (principle \ref{pri-vol3-monotonic-delegation}) to agents that must verify authority without a central lookup and lose it the moment an ancestor is canceled.
...
Operating distributed agent fleets and stateful single-agent runtimes in production demands comprehensive, fine-grained telemetry that exposes causal execution trajectories. In @sec-vol3-observability (*System Observability*), we develop the telemetry infrastructure required to monitor, trace, and evaluate non-deterministic systems: structured OpenTelemetry spans adapted for autoregressive decode loops, interactive evaluation environments and gyms, statistical confidence bounds on non-deterministic trajectory success, and post-mortem causal attribution for multi-agent cascades.
:::
