...
## 15.1 Coordination Tax and Amdahl's Law for Multi-Agent Systems {#sec-vol3-multiagent-coordination-tax}
...
::: {.callout-note title="Numerical derivation: Sizing an enterprise agent fleet"}
...
## 15.2 Swarm Topologies: Hierarchical, Peer-to-Peer, and Market-Based {#sec-vol3-multiagent-topologies}
...
## 15.3 Communication Protocols, Message Queues, and RPC Backbones {#sec-vol3-multiagent-rpc}
...
## 15.5 Byzantine Fault Tolerance and Compromised Agent Quarantines {#sec-vol3-multiagent-bft}
...
and quorums require a supermajority of at least $2f + 1$ matching signatures. Practical Byzantine Fault Tolerance (PBFT) [@castro2002byzantine] extended this to partially asynchronous networks through a three-phase protocol (Pre-prepare, Prepare, Commit) utilizing cryptographic message authentication codes (MACs).
...
- Identical post-training alignment (RLHF, DPO, instruction fine-tuning).
...
A compiler (`rustc`, `gcc`, `tsc`), an Abstract Syntax Tree (AST) linter (`ruff`, `eslint`), a type checker (`mypy`), or an integration test suite (`pytest`) is not an LLM:
...
Table @tbl-vol3-multiagent-synthesis synthesizes the distributed systems foundations established throughout this chapter across the core runtime subsystems:
...
::: {.callout-chapter-connection title="From multi-agent coordination to system tokenomics"}
Structuring distributed agent communication prevents message collapse, but evaluating whether a fleet of autonomous agents delivers viable unit economics requires rigorous capacity planning and financial governance. In @sec-vol3-tokenomics, we examine the economics of autonomous agent fleets: model cascading, provisioned throughput sizing, queuing theory under heavy-tailed agent workloads, and enterprise financial circuit breakers.
:::

## Quantitative Problems {#sec-vol3-multiagent-problems}

1. **Multi-Agent Amdahl's Law and Optimal Concurrency Calculation:**
   A distributed code refactoring pipeline evaluates an enterprise codebase with total normalized work $W = 1$. System telemetry reveals a strictly serial fraction $s = 0.12$ dedicated to repository dependency resolution, architectural task graph generation, and final integration testing. Inter-agent RPC serialization and git lock contention impose a quadratic coordination penalty $\alpha = 0.0035$, while agent prompt preambles and tool schema ingestion impose a linear provisioning penalty $\beta = 0.025$.
   a. Calculate the theoretical maximum parallel speedup $S_{\max}$ under classical Amdahl's Law as fleet size $M \to \infty$.
   b. Formulate the cubic stationary equation governing the optimal fleet size $M^*$ and calculate the optimal integer concurrency limit.
   c. Compute the effective wall-clock speedup $S(M^*)$ and parallel efficiency $E(M^*) = S(M^*)/M^*$ at this optimal operating point.
   d. Determine the crossover fleet size $M_{\text{cross}}$ at which the system enters the Negative Return Collapse Zone ($S(M) < 1.0$), taking longer to complete the work than a single monolithic agent.

2. **Token Ingestion and KV Cache Memory Tax in P2P vs. Hierarchical Topologies:**
   Consider an ensemble of $M = 8$ agents collaborating on a distributed debugging task over an operational horizon of $N = 10$ turns. In each turn, an active agent generates a message of length $T = 200$ tokens. The base system prompt and task specification require $H_0 = 1,500$ tokens.
   a. Calculate the total number of tokens ingested across the cluster if the agents operate in an unconstrained Peer-to-Peer (P2P) mesh where every message is broadcast to all peers and appended to a shared conversation buffer without prefix caching.
   b. Recalculate total tokens ingested if the ensemble is structured as a Hierarchical Supervisor Tree with one supervisor and seven isolated leaf workers, where workers communicate exclusively with the supervisor via structured RPC payloads averaging $T_{\text{rpc}} = 150$ tokens.
   c. The agents execute on a 70-billion parameter transformer model with $L = 80$ layers, hidden dimension $d_{\text{model}} = 8192$, and $H_{\text{kv}} = 8$ key-value heads ($d_h = 128$). In FP16 format (2 bytes per element), computing the KV-cache allocation requires:
      $$M_{\text{token}} = 2 \times 2 \times L \times H_{\text{kv}} \times d_h = 327,680 \quad \text{bytes/token}$$
      Calculate the peak High-Bandwidth Memory (HBM) footprint in gigabytes consumed by the accumulated KV caches across all agents under both topologies at turn $N = 10$.

3. **Byzantine Fault Tolerance and Correlated Neural Voting Probability:**
   An engineering team implements an autonomous continuous-deployment gate where a candidate database migration script is evaluated by an ensemble of $M = 5$ agents using majority voting (acceptance requires $\ge 3$ approvals). The script contains a subtle race condition under Read Committed transaction isolation. The base probability that a single agent fails to detect the bug and approves the script is $p = 0.18$.
   a. Assuming statistically independent node failures (the classical BFT assumption), calculate the probability $P_{\text{accept}}^{\text{indep}}$ that the ensemble erroneously approves the dangerous migration script.
   b. In production, the five agents run on identical foundation model weights. Due to shared pretraining and alignment distributions, failures are correlated. The conditional probability that Agent $j$ approves the bug given that Agent $i$ approved it is $P(A_j \mid A_i) = 0.82$. Using the chain rule of probability, calculate the true probability of unanimous $5-0$ approval:
      $$P(\text{Unanimous Approval}) = P(A_1) \prod_{k=2}^{5} P(A_k \mid A_1, \dots, A_{k-1}) \approx p \cdot [P(A_j \mid A_i)]^4$$
   c. The team introduces a deterministic SQL transaction analyzer that executes the migration script within an ephemeral sandboxed database and asserts serializability invariants. The analyzer has a false acceptance rate of $0.000$, an execution latency of $45\text{ ms}$, and consumes zero GPU memory. Quantify the error reduction achieved by deploying the deterministic oracle compared to expanding the neural voting ensemble to $M = 9$ agents.
