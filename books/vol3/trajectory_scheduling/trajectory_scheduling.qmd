...
```{python}
#| label: vol3-scheduling-idle-fraction
#| echo: false
# +-----------------------------------------------------------------------------
# | IDLE FRACTION AND THE UTILIZATION IT IMPLIES
# +-----------------------------------------------------------------------------
# | Context: @tbl-vol3-scheduling-workload-comparison and the paragraph that
# |          reads its tool-wait column.
# |
# | Goal:    Hold the tool-wait fractions once, so the table and the utilization
# |          claim drawn from it cannot drift apart.
# | Show:    The wait fraction per agentic workload, the span across them, and
# |          the utilization a scheduler that holds the accelerator through the
# |          wait would achieve.
# | How:     A scheduler that pins the accelerator across a tool wait achieves
# |          utilization equal to one minus the wait fraction, so the two ranges
# |          are complements of each other.
# |
# | Note:    The wait fractions are the chapter's trace figures. They are
# |          declared here rather than given registry provenance; the traces are
# |          not published, so a registry entry would imply a source that does
# |          not exist.
# +-----------------------------------------------------------------------------
from mlsysim import *
...
class IdleFraction:
    # +-- 1. LOAD (Constants) --------------------
    # trace figures, agentic rows only: web navigation, software engineering,
    # deep research. The two non-agentic rows wait on no tool at all.
    wait_fractions = (0.724, 0.841, 0.886)
    stateless_wait = 0.0
    chat_wait = 0.0

    # +-- 2. EXECUTE (The Compute) ---------------
    wait_low = min(wait_fractions)
    wait_high = max(wait_fractions)
    utilization_low = 1 - wait_high
    utilization_high = 1 - wait_low
    worst_wait = wait_high

    # +-- 3. GUARD (Invariants) ------------------
    check(0.70 < wait_low and wait_high < 0.90, f"Agentic waits should sit between 70 and 90 percent, got {wait_low} to {wait_high}")
    check(abs((utilization_low + wait_high) - 1.0) < 1e-9, "Utilization and wait fraction must be complements")
    check(utilization_low < utilization_high, "The utilization range must run from its worst case to its best")

    # +-- 4. OUTPUT (Formatting) -----------------
    wait_range_str = fmt_percent_range(wait_low, wait_high, precision=1, style="prose")
    utilization_range_str = fmt_percent_range(utilization_low, utilization_high, precision=1, style="prose")
    stateless_wait_str = fmt_percent(stateless_wait, precision=0, style="symbol")
    chat_wait_str = fmt_percent(chat_wait, precision=0, style="symbol")
    web_wait_str = fmt_percent(wait_fractions[0], precision=1, style="symbol")
    swe_wait_str = fmt_percent(wait_fractions[1], precision=1, style="symbol")
    research_wait_str = fmt_percent(wait_fractions[2], precision=1, style="symbol")
```
...
|:-------------------------|:----------------------:|:--------------------------:|:---------:|:--------------------------:|:------------------------------------------:|:--------------------------:|:------------------------:|
| **Stateless Serving**    |  LMSYS Chatbot Arena   |            1.0             |    1.0    |            0.0             | `{python} IdleFraction.stateless_wait_str` |          0 tokens          |          0.8 GB          |
| **Multi-Turn Chat**      |        MT-Bench        |            2.0             |    2.0    |            0.0             |   `{python} IdleFraction.chat_wait_str`    |         450 tokens         |          1.4 GB          |
| **Web Navigation**       |  WebArena / VisualWeb  |            7.4             |    35.0   |            48.2            |    `{python} IdleFraction.web_wait_str`    |        1,850 tokens        |         14.2 GB          |
| **Software Engineering** |   SWE-bench Verified   |            24.8            |   142.0   |           812.6            |    `{python} IdleFraction.swe_wait_str`    |         820 tokens         |         22.4 GB          |
| **Deep Research**        | GAIA / Research Fleets |            38.2            |   260.0   |          1,440.0           | `{python} IdleFraction.research_wait_str`  |        1,400 tokens        |         34.8 GB          |
...
As demonstrated in @tbl-vol3-scheduling-workload-comparison, autonomous agents spend `{python} IdleFraction.wait_range_str` of their end-to-end trajectory duration completely idle with respect to GPU compute. If a scheduler naively holds GPU resources during these extended tool-wait gaps, cluster utilization falls to `{python} IdleFraction.utilization_range_str` across these workloads, with the deep-research fleet the worst of them.
...
If a cluster scheduler performs capacity checks based solely on an agent's initial memory footprint ($1.15\text{ GB}$), it will admit far more concurrent sessions than the physical accelerator memory can sustain. As these admitted sessions progress concurrently, their aggregate KV-cache demand expands by $10\times$ to $20\times$. A scheduler that budgets on arrival is budgeting against a moving quantity. @Fig-vol3-context-length-scaling traces three orders of magnitude of growth in supported context, which is the distance between the footprint an admission check measures and the footprint the same session eventually occupies.

::: {#fig-vol3-context-length-scaling fig-env="figure" fig-pos="htb" fig-cap="**Historical Context Length Scaling**: Supported context length by model release date, rising from two thousand tokens in 2020 to two million in 2024. Admission control that sizes a session by its opening footprint is calibrated against the left edge of this curve while the sessions it admits live on the right." fig-alt="Log-scale scatter of foundation model context length in tokens by release year from 2018 to 2024, with labeled points rising from five hundred twelve tokens to two million."}
...
df = pd.read_csv("vol3/trajectory_scheduling/data/context_length_scaling.csv")
df['ReleaseDate'] = pd.to_datetime(df['ReleaseDate'])
df = df.sort_values('ReleaseDate')

fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 4.5))
ax.plot(df['ReleaseDate'], df['ContextLength'], marker='o', linestyle='-', linewidth=2, color='#2c3e50')

annotated = ['GPT-3', 'GPT-4 (32k)', 'Claude 1', 'GPT-4 Turbo', 'Gemini 1.5 Pro', 'Gemini 1.5 Pro v2']
for _, row in df.iterrows():
    if row['Model'] in annotated:
        ax.annotate(row['Model'], 
                    (row['ReleaseDate'], row['ContextLength']),
                    xytext=(-5, 8), textcoords='offset points',
                    ha='right', va='bottom', fontsize=9, fontweight='bold')

ax.set_yscale('log')
ax.set_ylabel('Maximum Context Length (Tokens)')
...
::: {#fig-vol3-scheduling-architecture fig-env="figure" fig-pos="htb" fig-cap="**Trajectory-Aware Serving and Scheduling Architecture**: Incoming heavy-tailed agent tasks undergo horizon prediction and capacity-aware admission control before being dispatched via cache-affinity routing to GPU workers maintaining warm Radix tree prefixes. Admission control and affinity routing solve different halves of the same problem. The first keeps a heavy-tailed arrival from committing capacity the cluster does not have, and the second keeps an admitted trajectory from paying its prefill twice." fig-alt="Diagram of a trajectory-aware serving and scheduling architecture in three stacked bands. The top band lists three agent workload properties: heavy-tailed Pareto service times, bimodal compute versus tool I/O, and multi-turn prefix redundancy. The middle band shows the trajectory scheduling controller with four numbered stages: predictive horizon and memory sizing, capacity-aware admission gate, cache-affinity session routing, and preemption and suspension coordination. The bottom band shows three GPU workers, one running an active session against a warm radix tree cache, one with its key-value cache swapped to host memory while it waits on a tool, and one balanced by dominant resource fairness."}
...
```{python}
#| label: vol3-scheduling-attained-service
#| echo: false
# +-----------------------------------------------------------------------------
# | REMAINING WORK UNDER A HEAVY-TAILED SERVICE TIME
# +-----------------------------------------------------------------------------
# | Context: Notebook \ref{nbk-scheduling-attained-service-residual}.
# |
# | Goal:    Size the work remaining for a trajectory that has already run,
# |          and contrast it with the memoryless model a scheduler would
# |          otherwise assume.
# | Show:    The fitted Pareto scale, a fit check against the measured P99,
# |          conditional survival, mean residual life, and both ratios against
# |          the exponential baseline.
# | How:     mlsysim's Pareto trajectory helpers. The mean and P99 are the
# |          SWE-bench Verified row of @tbl-vol3-scheduling-workload-comparison;
# |          the tail index is this section's working value.
# +-----------------------------------------------------------------------------
import math
...
class AttainedService:
    # +-- 1. LOAD (Constants) --------------------
    mean_steps = 24.8  # measured: SWE-bench Verified row, mean trajectory length
    p99_steps = 142  # measured: SWE-bench Verified row, 99th percentile
    alpha = 1.1  # scenario assumption: working tail index from the 1.0-1.4 range
    attained = 50  # scenario assumption: steps the trajectory has already run
    target = 100  # scenario assumption: steps it is asked to survive to

    # +-- 2. EXECUTE (The Compute) ---------------
    scale = calc_pareto_scale(mean_steps, alpha)
    tail_at_p99 = calc_pareto_survival(p99_steps, scale, alpha)
    conditional = calc_pareto_conditional_survival(attained, target, alpha)
    residual = calc_pareto_mean_residual_life(attained, alpha)
    memoryless = math.exp(-attained / mean_steps)
    residual_ratio = residual / mean_steps
    survival_ratio = conditional / memoryless

    # +-- 3. GUARD (Invariants) ------------------
    check(abs(tail_at_p99 - 0.01) < 0.002, f"The fit must reproduce the measured P99, got {tail_at_p99} beyond {p99_steps} steps")
    check(conditional > memoryless, "A heavy tail must make an aged trajectory likelier to continue than the memoryless model")
    check(residual > mean_steps, f"Remaining work must exceed a fresh draw, got {residual} against {mean_steps}")
    check(
        calc_pareto_mean_residual_life(2 * attained, alpha) > residual,
        "Mean residual life must grow with attained service, which is the decreasing hazard rate",
    )

    # +-- 4. OUTPUT (Formatting) -----------------
    mean_steps_str = fmt(mean_steps, precision=1, commas=False)
    p99_steps_str = fmt_int(p99_steps)
    alpha_str = fmt(alpha, precision=1, commas=False)
    attained_str = fmt_int(attained)
    target_str = fmt_int(target)
    scale_str = fmt(scale, precision=2, commas=False)
    tail_at_p99_pct_str = fmt_percent(tail_at_p99, precision=1, style="prose")
    conditional_str = fmt(conditional, precision=3, commas=False)
    memoryless_str = fmt(memoryless, precision=3, commas=False)
    residual_str = fmt_int(int(round(residual)))
    residual_ratio_mult_str = fmt_multiple(residual_ratio, precision=1)
    survival_ratio_mult_str = fmt_multiple(survival_ratio, precision=1)
```

::: {#nbk-scheduling-attained-service-residual .callout-notebook title="Remaining work after 50 attained steps"}

**Problem**: Under the Pareto service-time model, how many further steps should a scheduler expect from an agent that has already executed `{python} AttainedService.attained_str` steps, and how does that expectation compare with the one for a task drawn fresh from the same queue?

**Variables**: The SWE-bench Verified row of @tbl-vol3-scheduling-workload-comparison supplies two measured quantities, a mean $\mathbb{E}[N]$ of `{python} AttainedService.mean_steps_str` steps and a 99th percentile of `{python} AttainedService.p99_steps_str` steps. The tail index $\alpha$ is `{python} AttainedService.alpha_str`, this section's working value from the empirical range $1.0 < \alpha < 1.4$, and it is the one input not read off the table. Step counts are treated as continuous, and the survival function is the Pareto form of @sec-vol3-appendix-math-pareto, $\mathbb{P}(N > x) = (x_{\min}/x)^{\alpha}$. The memoryless baseline is an exponential distribution fitted to the same mean.

**Math**: The Pareto mean $\mathbb{E}[N] = \alpha x_{\min} / (\alpha - 1)$ pins the scale parameter to the measured mean:

$$x_{\min} = \frac{\mathbb{E}[N] \cdot (\alpha - 1)}{\alpha}$$

which gives `{python} AttainedService.scale_str` steps.

The fit is checkable, because those same two parameters must also reproduce the measured tail:

$$\mathbb{P}(N > x) = \left( \frac{x_{\min}}{x} \right)^{\alpha}$$

Evaluated at the measured 99th percentile, the fitted distribution places `{python} AttainedService.tail_at_p99_pct_str` of tasks beyond `{python} AttainedService.p99_steps_str` steps, which is what a P99 asserts, so the tail index and the table agree. Conditioning on 50 attained steps is then a ratio of survival probabilities, and the scale parameter cancels:

$$\mathbb{P}(N > x_2 \mid N > x_1) = \frac{(x_{\min}/x_2)^{\alpha}}{(x_{\min}/x_1)^{\alpha}} = \left( \frac{x_1}{x_2} \right)^{\alpha}$$

For `{python} AttainedService.attained_str` attained steps and a target of `{python} AttainedService.target_str` that is `{python} AttainedService.conditional_str`. The exponential baseline fitted to the same mean answers `{python} AttainedService.memoryless_str`. Integrating the conditional survival curve gives the mean residual life, $\mathbb{E}[N - x \mid N > x] = x/(\alpha - 1)$, which for an agent 50 steps in is:

$$\mathbb{E}[N - x \mid N > x] = \frac{x}{\alpha - 1}$$

which is `{python} AttainedService.residual_str` steps.

**Result**: The `{python} AttainedService.attained_str`-step trajectory is expected to run `{python} AttainedService.residual_str` more steps, against `{python} AttainedService.mean_steps_str` for a task pulled fresh from the same queue, a factor of `{python} AttainedService.residual_ratio_mult_str`. It is also `{python} AttainedService.survival_ratio_mult_str` likelier to survive another `{python} AttainedService.attained_str` steps than the memoryless model predicts.
...
::: {#fig-vol3-scheduling-preemption-matrix fig-env="figure" fig-pos="htb" fig-cap="**The Preemption Decision Matrix**: Categorization of agent execution into four zones governing whether an in-flight trajectory can be safely preempted, conditionally compensated, or must remain strictly pinned to completion. The axis that decides the zone is external side effects, not elapsed cost. A long trajectory that has touched nothing outside its sandbox is cheaper to preempt than a short one that has already sent a message." fig-alt="Diagram of four execution zones arranged along a reversibility spectrum, running from pure compute through idempotent reads, then across a marked boundary to reversible writes and irreversible actions. Each zone panel gives its state residency, its degree of reversibility, and its preemption policy, which moves from instant preemption to safe suspend, then to conditional preemption with a compensating action, and finally to strictly non-preemptible pinned execution."}
...
Scheduling agentic trajectories requires abandoning the memoryless assumptions of classical request queues. Because service times exhibit heavy-tailed Pareto distributions, multi-turn contexts expand dynamically, and external tool calls create multi-second idle stalls, schedulers must combine predictive admission control, Least Attained Service priorities, and multi-tier memory swapping.

::: {.callout-takeaways title="Predictive admission and heavy-tailed scheduling"}
...
Trajectory scheduling exposes the fundamental trade-off of stateful systems: accelerator compute cannot be managed independently of memory residency and external tool latency. By replacing memoryless request dispatch with lifetime-aware admission and preemption policies, schedulers preserve cluster throughput without starving long-horizon deliberative workloads.

::: {.callout-chapter-connection title="From memory scheduling to tool interfaces"}
Optimizing the memory and compute pipeline keeps inference clusters operational, but the real power of an agentic system lies in its ability to interact with the external world through external APIs and system tools. Concluding **Part III (Serving Runtimes and Memory Systems)**, we enter **Part IV (Security, Isolation, and Red Teaming)**: beginning in @sec-vol3-tool-interfaces with the design of typed Application Binary Interfaces (ABIs), structured schema generation, and the Model Context Protocol (MCP).
:::
