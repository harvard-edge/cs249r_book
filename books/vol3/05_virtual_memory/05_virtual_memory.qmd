...
In @sec-vol3-processor-tokenization, we established the tensor geometry of the Key-Value (KV) cache, demonstrating that avoiding quadratic $O(S^2)$ recomputation requires preserving intermediate projection vectors in accelerator High-Bandwidth Memory (HBM).[^fn-kv-cache-geometry] For an active context containing $S$ tokens across a concurrent batch of $B$ agent trajectories, aggregate attention state scales linearly as $\text{Mem}_{\text{batch}} = B \cdot S \cdot \text{Mem}_{\text{token}}$, where $\text{Mem}_{\text{token}} = 2 \cdot L \cdot H_{\text{kv}} \cdot d_{\text{head}} \cdot P$.

[^fn-kv-cache-geometry]: **KV Cache Geometry**: See @sec-vol3-processor-tokenization for the mathematical derivation of Key-Value tensor projections across transformer layers and attention heads. This chapter addresses the physical virtualization of these blocks on accelerator silicon.
...
In @sec-vol3-processor, this cache was analyzed under an idealized systems assumption: that the runtime allocates a contiguous physical memory buffer for each sequence. On physical accelerator hardware, however, this assumption fails catastrophically. Because an autonomous agent's execution horizon is non-deterministic—terminating after three turns or continuing for fifty tool interactions—contiguous memory allocators must pre-allocate physical buffers sized for the worst-case context length ($S_{\max}$). When actual trajectories terminate early or grow dynamically, this contiguous strategy induces severe internal and external memory fragmentation.

```{python}
#| label: lego-05-attention-state-sizing
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ ATTENTION STATE SIZING FOR 70B GQA MODEL (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-05-sizing-the-attention-state-for
# │ Goal: Quantify physical memory footprint of KV cache across context regimes.
# │ Show: 320 KiB/token scales to 2.5 GiB at 8k, 10 GiB at 32k, and 40 GiB at 128k; MHA demands 320 GiB.
# │ How: Multiply layers, KV heads, head dimension, and element precision by token count.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_memory_capacity

class AttentionStateSizing:
    """KV cache physical footprint across context scaling regimes."""
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    mem_token = 2 * n_layers * n_kv_heads * d_head * bytes_per_elem
    mem_standard = (s_standard * mem_token).to(GiB)
    mem_diagnostic = (s_diagnostic * mem_token).to(GiB)
    mem_repo = (s_repo * mem_token).to(GiB)
    mem_repo_mha = (s_repo * (2 * n_layers * n_kv_heads_mha * d_head * bytes_per_elem)).to(GiB)
    mha_scale = n_kv_heads_mha // n_kv_heads

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(mem_token.to(KiB).magnitude == 320.0, "mem_token KiB")
    check(abs(mem_standard.magnitude - 2.50) < 0.01, "mem_standard GiB")
    check(abs(mem_diagnostic.magnitude - 10.00) < 0.01, "mem_diagnostic GiB")
    check(abs(mem_repo.magnitude - 40.00) < 0.01, "mem_repo GiB")
    check(abs(mem_repo_mha.magnitude - 320.00) < 0.01, "mem_repo_mha GiB")
    check(mha_scale == 8, "mha_scale")
    ...
    s_standard_str = fmt_int(s_standard)
    s_diagnostic_str = fmt_int(s_diagnostic)
    s_repo_str = fmt_int(s_repo)
    n_kv_heads_mha_str = fmt_int(n_kv_heads_mha)
    num_h100_gpus_str = fmt_int(num_h100_gpus)
    gpu_hbm_gb_str = fmt_memory_capacity(gpu_hbm, unit=GB, precision=0)
```

::: {#nbk-05-sizing-the-attention-state-for .callout-notebook title="Sizing the attention state for a 70-billion parameter model"}
Consider an inference cluster serving an open-weights 70-billion parameter model structured with the canonical architectural parameters from @sec-vol3-processor-tokenization ($L=$ `{python} AttentionStateSizing.n_layers_str` layers, $H_{\text{q}}=$ `{python} AttentionStateSizing.n_q_heads_str` query heads, $H_{\text{kv}}=$ `{python} AttentionStateSizing.n_kv_heads_str` GQA key-value heads, head dimension $d_{\text{head}}=$ `{python} AttentionStateSizing.d_head_str`, and 16-bit `bfloat16` precision with $P =$ `{python} AttentionStateSizing.bytes_per_elem_str` bytes).

Applying the token footprint formulated in @sec-vol3-processor-tokenization:
...
1. **Standard Turn ($S =$ `{python} AttentionStateSizing.s_standard_str` tokens):**
   $$\text{Mem}_{\text{seq}} = 8{,}192 \cdot 320\text{ KiB} \approx 2.50\text{ GiB}$$

2. **Deep Diagnostic Trajectory ($S =$ `{python} AttentionStateSizing.s_diagnostic_str` tokens):**
   $$\text{Mem}_{\text{seq}} = 32{,}768 \cdot 320\text{ KiB} \approx 10.00\text{ GiB}$$

3. **Repository-Scale Context ($S =$ `{python} AttentionStateSizing.s_repo_str` tokens):**
   $$\text{Mem}_{\text{seq}} = 131{,}072 \cdot 320\text{ KiB} \approx 40.00\text{ GiB}$$

Had this model been designed using Multi-Head Attention ($H_{\text{kv}} =$ `{python} AttentionStateSizing.n_kv_heads_mha_str`), the memory consumption would scale eightfold: a single `{python} AttentionStateSizing.s_repo_str`-token sequence would demand $320.00 \text{ GiB}$ of physical memory purely for its key-value projections—an allocation that exceeds the entire onboard memory capacity of `{python} AttentionStateSizing.num_h100_gpus_str` `{python} AttentionStateSizing.gpu_hbm_gb_str` NVIDIA H100 GPUs combined.
:::
...
```{python}
#| label: lego-05-contiguous-kv-fragmentation
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ CONTIGUOUS KV CACHE FRAGMENTATION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-05-quantifying-contiguous-kv-cache-fragmentation
# │ Goal: Quantify internal reservation and external memory fragmentation.
# │ Show: 87.5% capacity idle under worst-case reservation; 100% external fragmentation stalls 1.5 GiB allocation.
# │ How: Model static worst-case reservations vs. dynamic non-contiguous buddy segment pools.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_memory_capacity, fmt_bandwidth, fmt_percent

class ContiguousKvFragmentation:
    """Internal and external fragmentation under contiguous KV cache allocators."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    m_device = 80.0 * GiB
    bw_mem = 3.35 * (TB / second)
    n_params = 70e9
    bytes_per_weight = 2 * byte
    tp_degree = 2
    m_act_cuda = 4.81 * GiB
    n_layers = 80
    n_kv_heads = 8
    ...
    n_requests_b = 10
    m_useful_b = 4.20 * GiB
    m_allocated_b = 6.40 * GiB
    segments_b = [512 * MiB, 256 * MiB, 1024 * MiB, 768 * MiB, 1024 * MiB]
    s_req11 = 4_096
    m_req11 = 1_536 * MiB

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    m_weights_total = (n_params * bytes_per_weight).to(GiB)
    m_weights_per_gpu = round((m_weights_total / tp_degree).magnitude, 2) * GiB
    m_pool = round((m_device - m_weights_per_gpu - m_act_cuda).magnitude, 2) * GiB
    n_kv_heads_local = n_kv_heads // tp_degree
    m_token_local = (2 * n_layers * n_kv_heads_local * d_head * bytes_per_weight).to(byte)
    a_static = (s_max * m_token_local).to(GiB)
    b_max_static = int((m_pool / a_static).to_base_units().magnitude)
    m_useful_static = b_max_static * (s_mean * m_token_local).to(GiB)
    m_allocated_static = b_max_static * a_static
    eta_alloc_static = (m_useful_static / m_allocated_static).to_base_units().magnitude
    m_idle_static = m_allocated_static - m_useful_static
    idle_ratio_static = (m_idle_static / m_allocated_static).to_base_units().magnitude

    eta_alloc_b = (m_useful_b / m_allocated_b).to_base_units().magnitude
    m_free_b = m_pool - m_allocated_b
    max_segment_b = max(segments_b).to(MiB)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(m_weights_total.magnitude - 130.38) < 0.01, "m_weights_total")
    check(abs(m_weights_per_gpu.magnitude - 65.19) < 0.01, "m_weights_per_gpu")
    check(abs(m_pool.magnitude - 10.00) < 0.01, "m_pool")
    check(m_token_local.to(KiB).magnitude == 160.0, "m_token_local")
    check(abs(a_static.magnitude - 2.50) < 0.01, "a_static")
    check(b_max_static == 4, "b_max_static")
    check(abs(m_useful_static.magnitude - 1.25) < 0.01, "m_useful_static")
    check(abs(eta_alloc_static - 0.125) < 0.001, "eta_alloc_static")
    check(abs(m_idle_static.magnitude - 8.75) < 0.01, "m_idle_static")
    check(abs(idle_ratio_static - 0.875) < 0.001, "idle_ratio_static")
    check(abs(m_free_b.magnitude - 3.60) < 0.01, "m_free_b")
    check(max_segment_b.magnitude == 1024.0, "max_segment_b")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    bw_mem_tb_per_s_str = fmt_bandwidth(bw_mem, unit=TB / second, precision=2)
    bytes_per_weight_str = fmt_int(bytes_per_weight.to(byte).magnitude)
    n_layers_str = fmt_int(n_layers)
    n_kv_heads_str = fmt_int(n_kv_heads)
    d_head_str = fmt_int(d_head)
    s_max_str = fmt_int(s_max)
    tp_degree_str = fmt_int(tp_degree)
    m_act_cuda_gb_str = fmt_memory_capacity(m_act_cuda, unit=GiB, precision=2)
    n_kv_heads_local_str = fmt_int(n_kv_heads_local)
    b_max_static_str = fmt_int(b_max_static)
    s_mean_str = fmt_int(s_mean)
    idle_ratio_static_pct_str = fmt_percent(idle_ratio_static, precision=1, style="prose")
    n_requests_b_str = fmt_int(n_requests_b)
    eta_alloc_b_pct_str = fmt_percent(round(eta_alloc_b * 1000) / 1000, precision=1, style="prose")
    s_req11_str = fmt_int(s_req11)
```
...
**Setting:** Consider an NVIDIA H100 SXM5 accelerator equipped with $M_{\text{device}} = 80 \text{ GiB}$ ($85{,}899{,}345{,}920 \text{ bytes}$) of High Bandwidth Memory (HBM3), delivering `{python} ContiguousKvFragmentation.bw_mem_tb_per_s_str` of memory bandwidth. The system serves a Llama-3-70B model using 16-bit half-precision weights (BF16, $P =$ `{python} ContiguousKvFragmentation.bytes_per_weight_str` bytes). The model features $L =$ `{python} ContiguousKvFragmentation.n_layers_str` transformer layers, a hidden dimension of $d_{\text{model}} = 8{,}192$, and uses Grouped-Query Attention (GQA) with $H_{\text{kv}} =$ `{python} ContiguousKvFragmentation.n_kv_heads_str` key-value heads, each with head dimension $d_{\text{head}} =$ `{python} ContiguousKvFragmentation.d_head_str`. The maximum supported context length is $S_{\max} =$ `{python} ContiguousKvFragmentation.s_max_str` tokens.
...
Because the weights exceed a single GPU, the model is tensor-parallelized across two H100 GPUs ($TP=$ `{python} ContiguousKvFragmentation.tp_degree_str`). On each GPU:
$$M_{\text{weights, per-GPU}} = \frac{130.38 \text{ GiB}}{2} = 65.19 \text{ GiB}$$
Reserving `{python} ContiguousKvFragmentation.m_act_cuda_gb_str` for activation working memory ($M_{\text{act}}$) and CUDA runtime overhead leaves:
$$M_{\text{pool}} = 80.00 - 65.19 - 4.81 = 10.00 \text{ GiB} \quad (10{,}737{,}418{,}240 \text{ bytes})$$

The key-value state footprint per token across the local shards on one GPU ($H_{\text{kv, local}} = 8 / 2 =$ `{python} ContiguousKvFragmentation.n_kv_heads_local_str` heads) is:
...
The runtime configures contiguous buffers sized for $S_{\max} =$ `{python} ContiguousKvFragmentation.s_max_str` tokens.
...
Suppose the active agent workload exhibits an empirical sequence-length distribution with a mean length of $\mathbb{E}[S] =$ `{python} ContiguousKvFragmentation.s_mean_str` tokens. When `{python} ContiguousKvFragmentation.b_max_static_str` requests are running at their average lengths:
...
A staggering $8.75 \text{ GiB}$ (`{python} ContiguousKvFragmentation.idle_ratio_static_pct_str`) of the available KV cache capacity is completely idle, trapped in internal reservation fragmentation. Concurrency is capped at `{python} ContiguousKvFragmentation.b_max_static_str`, collapsing serving throughput.

**Scenario B: Dynamic Contiguous Allocation with External Fragmentation:**
To combat static waste, the runtime switches to dynamic contiguous power-of-two buddy allocation. `{python} ContiguousKvFragmentation.n_requests_b_str` requests are admitted with varying sequence lengths drawn from the empirical distribution, occupying a total of $M_{\text{useful}} = 4.20 \text{ GiB}$ with allocated buffers totaling $M_{\text{allocated}} = 6.40 \text{ GiB}$ ($\eta_{\text{alloc}} =$ `{python} ContiguousKvFragmentation.eta_alloc_b_pct_str`).
...
An eleventh request arrives, bearing a prompt of `{python} ContiguousKvFragmentation.s_req11_str` tokens and requiring an initial reservation of $1{,}536 \text{ MiB}$ ($1.50 \text{ GiB}$).
...
class PhysicalBlockSizing:
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    n_kv_heads_gpu = n_kv_heads // tp_degree
    m_token_node = calc_kv_cache_bytes_per_token(n_layers=n_layers, n_kv_heads=n_kv_heads, head_dim=d_head, bytes_per_elem=bytes_per_elem)
    m_token_gpu = (m_token_node / tp_degree).to(byte)
...
```{python}
#| label: lego-05-radix-prefix-reuse
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TTFT AND COMPUTE SAVINGS FROM RADIX PREFIX REUSE (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-05-ttft-and-compute-savings-from
# │ Goal: Quantify TTFT reduction and compute savings achieved through Radix tree prefix caching.
# │ Show: 4,096-token prefix hit cuts TTFT by 167.4 ms (28x reduction) and saves 617 TFLOPs.
# │ How: Model linear and quadratic self-attention FLOPs divided by sustained cluster throughput.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_multiple

class RadixPrefixReuse:
    """TTFT latency and arithmetic work saved through radix tree prefix caching."""
    ...
    s_prefix = 4_096
    mfu_prefill = 0.45
    peak_dense_per_gpu = 989e12 / second
    t_resolve_cpu = 50.0 * microsecond
    s_novel = 32
    t_novel_prefill = 6.0 * millisecond

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    mem_token = calc_kv_cache_bytes_per_token(n_layers=n_layers, n_kv_heads=n_kv_heads, head_dim=d_head, bytes_per_elem=bytes_per_elem)
    mem_kv_prefix = (s_prefix * mem_token).to(GiB)
    mem_kv_prefix_per_gpu = (mem_kv_prefix / n_gpus).to(MiB)
    flops_linear = 2 * n_params * s_prefix
    flops_attn = 4 * n_layers * d_model * (s_prefix**2)
    flops_total = flops_linear + flops_attn
    peak_cluster = n_gpus * peak_dense_per_gpu
    throughput_sustained = mfu_prefill * peak_cluster
    t_prefill = (flops_total / throughput_sustained).to(millisecond)
    latency_saved = (t_prefill - t_novel_prefill).to(millisecond)
    ttft_reduction = (t_prefill / t_novel_prefill).magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(mem_kv_prefix.magnitude - 1.25) < 0.01, "mem_kv_prefix")
    check(abs(mem_kv_prefix_per_gpu.magnitude - 160.0) < 0.01, "mem_kv_prefix_per_gpu")
    check(abs(flops_linear / 1e12 - 573.44) < 0.1, "flops_linear")
    check(abs(flops_attn / 1e12 - 43.98) < 0.1, "flops_attn")
    check(abs(flops_total / 1e12 - 617.42) < 0.1, "flops_total")
    check(abs(t_prefill.magnitude - 173.4) < 0.5, "t_prefill")
    check(abs(latency_saved.magnitude - 167.4) < 0.5, "latency_saved")
    ...
    d_head_str = fmt_int(d_head)
    bytes_per_elem_str = fmt_int(bytes_per_elem.to(byte).magnitude)
    s_prefix_str = fmt_int(s_prefix)
    mfu_prefill_pct_str = fmt_percent(mfu_prefill, precision=0, style="prose")
    t_resolve_cpu_us_str = fmt_time(t_resolve_cpu, unit=microsecond, precision=0)
    s_novel_str = fmt_int(s_novel)
    t_novel_prefill_ms_str = fmt_time(t_novel_prefill, unit=millisecond, precision=0)
    latency_saved_ms_str = fmt_time(latency_saved, unit=millisecond, precision=1)
    ttft_reduction_mult_str = fmt_multiple(round(ttft_reduction), precision=0)
```

::: {#nbk-05-ttft-and-compute-savings-from .callout-notebook title="TTFT and compute savings from radix prefix reuse"}
Consider an autonomous coding agent operating on an `{python} RadixPrefixReuse.n_gpus_str`-GPU tensor-parallel node hosting a 70-billion parameter dense model ($N = 70 \times 10^9$). The model utilizes Grouped-Query Attention (GQA) with $L =$ `{python} RadixPrefixReuse.n_layers_str` layers, $H_Q =$ `{python} RadixPrefixReuse.n_q_heads_str` query heads, $H_{\text{kv}} =$ `{python} RadixPrefixReuse.n_kv_heads_str` key-value heads, and a head dimension of $d_{\text{head}} =$ `{python} RadixPrefixReuse.d_head_str`, represented in 16-bit brain floating-point format ($b_{\text{elem}} =$ `{python} RadixPrefixReuse.bytes_per_elem_str` bytes).

The agent's standard system instruction, formatting rules, and tool definition schema comprise an invariant prefix of $S_{\text{prefix}} =$ `{python} RadixPrefixReuse.s_prefix_str` tokens.

**1. Physical KV Memory Footprint:**
Applying the physical GQA tensor footprint established in @sec-vol3-processor-tokenization ($m_{\text{token}} = 320\text{ KiB/token}$):
$$M_{\text{KV}} = S_{\text{prefix}} \cdot m_{\text{token}} = 4{,}096 \times 320\text{ KiB} = 1{,}310{,}720\text{ KiB} = 1.25\text{ GiB}$$
Distributed across the `{python} RadixPrefixReuse.n_gpus_str` GPUs via tensor parallelism, this footprint requires only $160\text{ MiB}$ of HBM per accelerator.

**2. Prefill Compute Savings:**
Applying the foundation engine Roofline model from @sec-vol3-processor-cost, evaluating prompt context requires $2N$ FLOPs per token for linear projections plus the quadratic self-attention component ($4 \cdot L \cdot d_{\text{model}} \cdot S_{\text{prefix}}^2$):
...
**3. Latency Impact:**
The 8-GPU H100 node provides an aggregate theoretical dense BF16 peak of $8 \times 989 = 7{,}912 \text{ TFLOPS}$. At a realistic Model Flops Utilization (MFU) of `{python} RadixPrefixReuse.mfu_prefill_pct_str` for tensor-parallel prefill at batch size 1, the sustained cluster compute throughput is:
...
With a Radix tree prefix hit, the entire `{python} RadixPrefixReuse.s_prefix_str`-token prefill is bypassed. The host CPU resolves the block table in less than `{python} RadixPrefixReuse.t_resolve_cpu_us_str`. If the incoming query introduces `{python} RadixPrefixReuse.s_novel_str` novel tokens, the accelerator executes prefill only for those `{python} RadixPrefixReuse.s_novel_str` tokens attending across the cached `{python} RadixPrefixReuse.s_prefix_str` tokens, completing in under `{python} RadixPrefixReuse.t_novel_prefill_ms_str`. Prefix caching eliminates `{python} RadixPrefixReuse.latency_saved_ms_str` of latency—a `{python} RadixPrefixReuse.ttft_reduction_mult_str` reduction in TTFT—while saving over $600 \text{ TFLOPs}$ of accelerator work.
:::
...
The foundational premise of prefix caching is that token sequence identity implies key-value state identity. This equivalence holds strictly because autoregressive transformer architectures enforce a lower-triangular causal attention mask (formulated in @sec-vol3-processor).[^fn-causal-mask-invalidation]

[^fn-causal-mask-invalidation]: **Causal Mask Invariant**: See @sec-vol3-processor for the mathematical definition of causal attention masking $\text{softmax}(\mathbf{Q}\mathbf{K}^T/\sqrt{d_{\text{head}}} + \mathbf{M})\mathbf{V}$. Here, this causal dependency establishes that token modifications at index $k$ invalidate all downstream cached blocks $t \ge k$.
...
@Sec-vol3-working-sets-staging turned this rule into the ascending-volatility layout, together with the timestamp and tokenization hazards that break it. The serving engine does not enforce that layout. It only rewards it, with a hit that covers every full block before the first volatile token.
...
Prefill and decode conflict because they saturate different resources. Decode is bound by memory bandwidth (principle \ref{pri-vol3-memory-bandwidth-decoding}) while prefill is bound by compute, as @sec-vol3-processor-cost derived. Under the Roofline model, the operational intensity $\mathcal{I}$ of a workload measures the ratio of floating-point operations executed to memory bytes transferred across the accelerator memory bus:
...
```{python}
#| label: lego-05-chunked-prefill-budgeting
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ QUANTITATIVE INTERFERENCE AND CHUNK BUDGETING (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-05-quantitative-interference-and-chunk-budgeting
# │ Goal: Balance memory-bound decode iterations with compute-bound prefill chunks.
# │ Show: Monolithic prefill spikes ITL by 25.6x (1,195 ms); chunk size 512 bounds ITL to 158.7 ms.
# │ How: Calculate execution duration under max(t_mem, t_comp) for monolithic vs co-scheduled chunks.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_multiple

class ChunkedPrefillBudgeting:
    """Inter-token latency and chunk sizing under co-scheduled serving."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    peak_compute = 800e12 / second
    bw_mem = 3.0 * (TB / second)
    n_params = 70e9
    bytes_per_weight = 2 * byte
    weights_footprint = 140.0 * GB
    b_decode = 32
    s_prompt = 4_096
    mfu_decode = 0.50
    mfu_prefill = 0.60
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    flops_decode = 2 * n_params * b_decode
    t_mem_decode = (weights_footprint / bw_mem).to(millisecond)
    t_comp_decode = (flops_decode / (mfu_decode * peak_compute)).to(millisecond)
    t_iter_decode = max(t_mem_decode, t_comp_decode)
    flops_prefill = 2 * n_params * s_prompt
    t_comp_prefill = (flops_prefill / (mfu_prefill * peak_compute)).to(millisecond)
    itl_spike_mult = (t_comp_prefill / t_iter_decode).magnitude

    k_chunks = s_prompt // c_chunk
    tokens_chunk_iter = c_chunk + b_decode
    flops_chunk_iter = 2 * n_params * tokens_chunk_iter
    t_comp_chunk_iter = (flops_chunk_iter / (mfu_prefill * peak_compute)).to(millisecond)
    t_iter_chunk = max(t_mem_decode, t_comp_chunk_iter)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_mem_decode.magnitude - 46.7) < 0.1, "t_mem_decode")
    check(abs(t_comp_decode.magnitude - 11.2) < 0.1, "t_comp_decode")
    check(abs(t_comp_prefill.magnitude - 1194.7) < 1.0, "t_comp_prefill")
    check(abs(itl_spike_mult - 25.6) < 0.1, "itl_spike_mult")
    check(k_chunks == 8, "k_chunks")
    check(tokens_chunk_iter == 544, "tokens_chunk_iter")
    check(abs(t_comp_chunk_iter.magnitude - 158.7) < 0.2, "t_comp_chunk_iter")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    b_decode_str = fmt_int(b_decode)
    s_prompt_str = fmt_int(s_prompt)
    mfu_decode_pct_str = fmt_percent(mfu_decode, precision=0, style="prose")
    mfu_prefill_pct_str = fmt_percent(mfu_prefill, precision=0, style="prose")
    itl_spike_mult_str = fmt_multiple(itl_spike_mult, precision=1)
    c_chunk_str = fmt_int(c_chunk)
    k_chunks_str = fmt_int(k_chunks)
    tokens_chunk_iter_str = fmt_int(tokens_chunk_iter)
```
...
An inference engine runs on an accelerator delivering $T_{\text{peak}} = 800\text{ TFLOPS}$ ($8 \times 10^{14}\text{ FLOPs/s}$) dense 16-bit matrix compute and $B_{\text{mem}} = 3.0\text{ TB/s}$ ($3 \times 10^{12}\text{ Bytes/s}$) HBM bandwidth. The served model has $P = 70\text{ billion}$ parameters stored in 16-bit precision, occupying $2P = 140\text{ GB}$ of memory. The engine is actively servicing $B_{\text{decode}} =$ `{python} ChunkedPrefillBudgeting.b_decode_str` decode streams. An agent submits a new prompt with $S_{\text{prompt}} =$ `{python} ChunkedPrefillBudgeting.s_prompt_str` tokens.

**Question 1: What is the execution latency and ITL spike under monolithic run-to-completion scheduling?**

A decode iteration with `{python} ChunkedPrefillBudgeting.b_decode_str` sequences requires streaming the full $140\text{ GB}$ of model weights to execute $2 \cdot P \cdot B_{\text{decode}} = 2 \times (70 \times 10^9) \times 32 = 4.48 \times 10^{12}\text{ FLOPs} = 4.48\text{ TFLOPs}$.
The memory read time is:
$$t_{\text{mem, decode}} = \frac{140 \times 10^9\text{ Bytes}}{3.0 \times 10^{12}\text{ Bytes/s}} \approx 46.7\text{ ms}$$
The compute time on tensor cores running at an achieved `{python} ChunkedPrefillBudgeting.mfu_decode_pct_str` model FLOPs utilization (MFU, effective throughput $400\text{ TFLOPS}$) is:
...
When the `{python} ChunkedPrefillBudgeting.s_prompt_str`-token monolithic prefill executes, its computational load is:
...
Assuming `{python} ChunkedPrefillBudgeting.mfu_prefill_pct_str` achieved MFU for dense GEMM ($480\text{ TFLOPS}$ effective):
...
The `{python} ChunkedPrefillBudgeting.b_decode_str` active decode streams stall for $1.195\text{ seconds}$. The ITL spikes from $46.7\text{ ms}$ to $1{,}195\text{ ms}$, a `{python} ChunkedPrefillBudgeting.itl_spike_mult_str` latency degradation.

**Question 2: What is the iteration latency when chunking the prefill with $C_{\text{chunk}} =$ `{python} ChunkedPrefillBudgeting.c_chunk_str`?**

The prompt is partitioned into $K = 4{,}096 / 512 =$ `{python} ChunkedPrefillBudgeting.k_chunks_str` chunks. In each iteration, the scheduler co-schedules one `{python} ChunkedPrefillBudgeting.c_chunk_str`-token prefill chunk with the `{python} ChunkedPrefillBudgeting.b_decode_str` decode tokens, yielding $512 + 32 =$ `{python} ChunkedPrefillBudgeting.tokens_chunk_iter_str` tokens total.
...
Instead of a catastrophic 1,195 ms freeze followed by fast decode, all `{python} ChunkedPrefillBudgeting.b_decode_str` active decode streams advance by one token every $158.7\text{ ms}$ for `{python} ChunkedPrefillBudgeting.k_chunks_str` consecutive cycles. The maximum ITL is cut from $1{,}195\text{ ms}$ down to $158.7\text{ ms}$, preserving predictable streaming performance while keeping accelerator compute cores saturated.
:::
...
```{python}
#| label: lego-05-break-even-frontier
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ SIZING THE BREAK-EVEN SWAP FRONTIER (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-05-sizing-the-break-even-frontier-for
# │ Goal: Dimensioning the break-even wait time between DRAM offloading and recomputation.
# │ Show: 5 GiB state reloads in 107 ms over PCIe vs. 5.735 s recomputation (53.6x faster); round-trip swap takes 215 ms.
# │ How: Compare KV recomputation FLOPs/throughput against bidirectional PCIe DMA bandwidth.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_bandwidth, fmt_time

class BreakEvenFrontier:
    """Decision boundary between KV retention, swapping, and recomputation."""
    ...
    d_head = 128
    bytes_per_elem = 2 * byte
    bw_pcie = 50.0 * (GB / second)
    throughput_prefill = 400e12 / second
    s_seq = 16_384

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    m_token = calc_kv_cache_bytes_per_token(n_layers=n_layers, n_kv_heads=n_kv_heads, head_dim=d_head, bytes_per_elem=bytes_per_elem)
    m_kv = (s_seq * m_token).to(GiB)
    flops_recomp = 2 * n_params * s_seq
    t_recomp = (flops_recomp / throughput_prefill).to(second)
    t_reload = (m_kv.to(GB) / bw_pcie).to(second)
    speedup_reload = (t_recomp / t_reload).magnitude
    t_roundtrip = (2 * m_kv.to(GB) / bw_pcie).to(second)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(m_token.to(KiB).magnitude == 320.0, "m_token")
    check(abs(m_kv.magnitude - 5.0) < 0.01, "m_kv")
    check(abs(t_recomp.magnitude - 5.735) < 0.01, "t_recomp")
    check(abs(t_reload.to(millisecond).magnitude - 107) < 1.0, "t_reload")
    check(abs(t_roundtrip.to(millisecond).magnitude - 215) < 1.0, "t_roundtrip")
    ...
    d_head_str = fmt_int(d_head)
    bytes_per_elem_str = fmt_int(bytes_per_elem.to(byte).magnitude)
    bw_pcie_gb_per_s_str = fmt_bandwidth(bw_pcie, unit=GB / second, precision=0)
    s_seq_str = fmt_int(s_seq)
    t_reload_ms_str = fmt_time(round(t_reload.to(millisecond).magnitude) * millisecond, unit=millisecond, precision=0)
    t_roundtrip_ms_str = fmt_time(round(t_roundtrip.to(millisecond).magnitude) * millisecond, unit=millisecond, precision=0)
```
...
Consider an agentic coding service executing a 70-billion parameter dense model ($P = 70 \times 10^9$) configured with Grouped-Query Attention (GQA). The architecture features $L =$ `{python} BreakEvenFrontier.n_layers_str` layers, $H_{\text{kv}} =$ `{python} BreakEvenFrontier.n_kv_heads_str` key-value heads, head dimension $d_{\text{head}} =$ `{python} BreakEvenFrontier.d_head_str`, and operates in 16-bit half-precision ($b_{\text{elem}} =$ `{python} BreakEvenFrontier.bytes_per_elem_str` bytes). The serving node executes across an 8-GPU tensor-parallel group connected via PCIe Gen5 x16 to host memory, yielding an aggregate sustained transfer bandwidth of $B_{\text{pcie}} =$ `{python} BreakEvenFrontier.bw_pcie_gb_per_s_str`. The GPU subsystem sustains an aggregate prefill throughput of $R_{\text{prefill}} = 400\text{ TFLOPs/s}$ ($4.0 \times 10^{14}\text{ FLOPs/s}$).

An active agent generates a git diff and dispatches a compiler check at sequence length $S =$ `{python} BreakEvenFrontier.s_seq_str` tokens.
...
The floating-point operations required to prefill `{python} BreakEvenFrontier.s_seq_str` tokens are:
...
**System Takeaway:** If the compiler check is expected to take less than `{python} BreakEvenFrontier.t_roundtrip_ms_str`, the engine must retain the blocks in HBM to prevent pipeline stalls. If the compiler runs for $5\text{ seconds}$, offloading frees $5\text{ GiB}$ of HBM for nearly the entire duration, avoiding $25\text{ GiB}\cdot\text{s}$ of memory tax, and reloads the state in just `{python} BreakEvenFrontier.t_reload_ms_str` upon completion—beating full recomputation by over $5.6\text{ seconds}$ of execution latency.
:::
...
```{python}
#| label: lego-05-hbm-sizing-multi-turn-agent
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ SIZING HBM FOR MULTI-TURN AGENT SWARMS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-05-sizing-hbm-for-multi-turn-agent
# │ Goal: Dimensioning node-level HBM and concurrent agent capacity under prefix sharing.
# │ Show: 464 GB KV pool supports 36 unshared trajectories vs. 122 shared trajectories (3.38x density expansion).
# │ How: Deduct static weights/activations from HBM; divide remaining token pool by per-agent active context.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_multiple, fmt_memory_capacity

class HbmSizingMultiTurnAgent:
    """Cluster HBM sizing and multi-turn agent concurrency under prefix sharing."""
    ...
    bytes_per_weight = 2 * byte
    bytes_per_elem = 2 * byte
    c_hbm_device = 80.0 * GB
    tp_degree = 8
    m_non_kv_device = 22.0 * GB
    pool_headroom = 0.10
    m_avg = 32_768
    k_avg = 2_048
    s_shared = 24_576
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    m_weights_device = (n_params * bytes_per_weight / tp_degree).to(GB)
    m_kv_device = c_hbm_device - m_non_kv_device
    m_kv_node = tp_degree * m_kv_device
    m_token = calc_kv_cache_bytes_per_token(n_layers=n_layers, n_kv_heads=n_kv_heads, head_dim=d_head, bytes_per_elem=bytes_per_elem)
    m_token_device = m_token / tp_degree
    n_capacity_node = int(m_kv_node.to(byte) / m_token)
    n_usable_tokens = int(n_capacity_node * (1 - pool_headroom))
    b_unshared = n_usable_tokens // (m_avg + k_avg)
    m_shared = (s_shared * m_token).to(GB)
    m_pool_usable = m_kv_node * (1 - pool_headroom) - m_shared
    m_per_agent = ((m_unique + k_avg) * m_token).to(GB)
    b_shared = int(m_pool_usable / m_per_agent)
    density_expansion = 3.38

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(m_weights_device.magnitude == 17.5, "m_weights_device")
    check(m_kv_device.magnitude == 58.0, "m_kv_device")
    check(m_kv_node.magnitude == 464.0, "m_kv_node")
    check(m_token.to(KiB).magnitude == 320.0, "m_token")
    check(m_token_device.to(KiB).magnitude == 40.0, "m_token_device")
    check(n_capacity_node == 1416015, "n_capacity_node")
    check(b_unshared == 36, "b_unshared")
    check(b_shared == 122, "b_shared")
    ...
    d_head_str = fmt_int(d_head)
    bytes_per_weight_str = fmt_int(bytes_per_weight.to(byte).magnitude)
    c_hbm_device_gb_str = fmt_memory_capacity(c_hbm_device, unit=GB, precision=0)
    tp_degree_str = fmt_int(tp_degree)
    pool_headroom_pct_str = fmt_percent(pool_headroom, precision=0, style="prose")
    m_avg_str = fmt_int(m_avg)
    k_avg_str = fmt_int(k_avg)
    b_unshared_str = fmt_int(b_unshared)
    s_shared_str = fmt_int(s_shared)
    m_unique_str = fmt_int(m_unique)
    b_shared_str = fmt_int(b_shared)
    density_expansion_mult_str = fmt_multiple(density_expansion, precision=2)
```

::: {#nbk-05-sizing-hbm-for-multi-turn-agent .callout-notebook title="Sizing HBM for multi-turn agent swarms on an 8-GPU node"}
Consider an enterprise agent cluster deployed to run continuous code-refactoring agents using a 70-billion parameter model ($N_{\text{params}} = 70 \times 10^9$) equipped with Grouped-Query Attention. The architectural parameters are $L =$ `{python} HbmSizingMultiTurnAgent.n_layers_str` layers, $H_q =$ `{python} HbmSizingMultiTurnAgent.n_q_heads_str` query heads, $H_{\text{kv}} =$ `{python} HbmSizingMultiTurnAgent.n_kv_heads_str` key-value heads, and $d_{\text{head}} =$ `{python} HbmSizingMultiTurnAgent.d_head_str`. All model weights and KV states are hosted in 16-bit bfloat16 precision ($b_{\text{weight}} = b_{\text{elem}} =$ `{python} HbmSizingMultiTurnAgent.bytes_per_weight_str` bytes). The serving node consists of an 8-GPU NVIDIA H100 SXM5 server with `{python} HbmSizingMultiTurnAgent.c_hbm_device_gb_str` of HBM3 per GPU ($C_{\text{HBM}} = 80\text{ GB} = 80 \times 10^9\text{ bytes}$, yielding $640\text{ GB}$ aggregate node memory). Tensor parallelism is configured across all eight GPUs ($TP =$ `{python} HbmSizingMultiTurnAgent.tp_degree_str`, $PP = 1$).
...
Suppose the agents operate across repository contexts with an average accumulated context of $M_{\text{avg}} =$ `{python} HbmSizingMultiTurnAgent.m_avg_str` tokens ($32\text{k}$) and an average decode response of $K_{\text{avg}} =$ `{python} HbmSizingMultiTurnAgent.k_avg_str` tokens, with zero prefix sharing ($\sigma = 0$). Sizing with a safety headroom of $\text{Pool}_{\text{headroom}} =$ `{python} HbmSizingMultiTurnAgent.pool_headroom_pct_str` yields a usable token capacity of $0.90 \times 1,416,015 = 1,274,413\text{ tokens}$.
...
Now consider the identical agent workload operating with a standardized base system prompt and shared repository AST index consisting of $S_{\text{shared}} =$ `{python} HbmSizingMultiTurnAgent.s_shared_str` tokens common to all agents, leaving only `{python} HbmSizingMultiTurnAgent.m_unique_str` unique turn tokens per agent ($M_{\text{unique}} =$ `{python} HbmSizingMultiTurnAgent.m_unique_str`, $K_{\text{avg}} =$ `{python} HbmSizingMultiTurnAgent.k_avg_str`).
...
By structurally isolating and sharing invariant workspace context in the paged Radix tree, the identical physical hardware sustains a `{python} HbmSizingMultiTurnAgent.density_expansion_mult_str` expansion in concurrent agent density (from `{python} HbmSizingMultiTurnAgent.b_unshared_str` to `{python} HbmSizingMultiTurnAgent.b_shared_str` concurrent trajectories) without altering model weights or precision.
:::
...
## Fallacies and Pitfalls {#sec-vol3-virtual-memory-fallacies}
...
The failure mode of this policy is stranded capacity. Consider our canonical 70-billion parameter GQA foundation model ($m_{\text{token}} \approx 320\text{ KiB/token}$, as formulated in @sec-vol3-processor-tokenization). An agent engaged in a complex repository refactoring trajectory with an active context length of $S = 32{,}768$ tokens immobilizes:
...
## Summary {#sec-vol3-virtual-memory-summary}
