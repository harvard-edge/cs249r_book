...
@Algo-pagedattention-allocation formalizes the core virtual memory operations of PagedAttention: zero-copy sequence branching and token-append Copy-on-Write (CoW) page allocation. When an agent forks a candidate trajectory, the memory manager duplicates the logical block table while merely incrementing physical block reference counts. During subsequent token generation, if the tail block is shared ($C_{\text{ref}} > 1$), the allocator clones only that single tail block to private physical storage before mutating it, preserving zero-copy sharing for all preceding prefix blocks.
...
@Algo-radix-prefix-cache outlines the prefix lookup, node traversal, and leaf-only LRU eviction mechanisms of RadixAttention. When a new agent turn arrives with token sequence $\mathbf{t}$, the cache traverses the compressed trie to discover the longest matching prefix of physical blocks. When memory pressure demands block reclamation, the eviction policy targets only unpinned leaf nodes ($C_{\text{ref}} = 0$) from the LRU queue, preventing broken tree branches.
...
3. **The Leaf-Only LRU Eviction Invariant (Lines 13–18):** Accelerator memory is finite; when the physical free pool $\mathcal{F}_{\text{HBM}}$ approaches exhaustion, blocks must be reclaimed. However, a naive LRU policy that evicts arbitrary blocks would create catastrophic dangling pointers in the trie. RadixAttention enforces the **Leaf-Only Eviction Invariant**:
