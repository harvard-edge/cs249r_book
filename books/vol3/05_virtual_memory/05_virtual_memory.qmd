...
class PagedMemoryManager:
    """Manages physical KV block allocation, page tables, zero-copy fork, and CoW."""

    def __init__(self, num_blocks: int = 1024, block_size: int = 16, dim: int = 128):
        self.block_size, self.dim = block_size, dim
        self.free_pool = list(range(num_blocks))
        self.ref_counts: Dict[int, int] = {}
        self.block_tables: Dict[str, List[int]] = {}
        self.storage = np.zeros((num_blocks, block_size, dim), dtype=np.float32)
    ...
    def fork(self, parent_id: str, child_id: str) -> None:
        """Zero-copy branch: aliases parent block table and increments reference counts."""
        self.block_tables[child_id] = list(self.block_tables[parent_id])
        for bid in self.block_tables[child_id]:
            self.ref_counts[bid] += 1

    def append(self, seq_id: str, token_vec: np.ndarray, token_idx_in_block: int) -> None:
        """Appends activation; triggers Copy-on-Write if tail block is shared (ref > 1)."""
        table = self.block_tables[seq_id]
        tail_bid = table[-1]
        if self.ref_counts[tail_bid] > 1:
            # CoW fork: clone shared block into private physical frame and decrement ref
            new_bid = self.allocate()
            self.storage[new_bid] = self.storage[tail_bid].copy()
            self.ref_counts[tail_bid] -= 1
            table[-1] = new_bid
            tail_bid = new_bid
        self.storage[tail_bid, token_idx_in_block] = token_vec
```
:::
...
class RadixNode:
    """Compressed trie node indexing cached PagedAttention physical blocks."""

    def __init__(self, tokens: List[int], blocks: List[int], parent=None):
        self.tokens, self.blocks, self.parent = tokens, blocks, parent
        self.children: Dict[int, "RadixNode"] = {}
        self.ref_count, self.last_used = 0, time.time()


class RadixPrefixCache:
    """Indexes token prefixes to physical blocks with dynamic splitting and LRU leaf eviction."""

    def __init__(self):
        self.root = RadixNode([], [])

    def match(self, tokens: List[int]) -> Tuple[List[int], List[int], RadixNode]:
        """Traverses tree to match longest cached prefix, returning (blocks, remaining_tokens, node)."""
        curr, offset, matched_blocks = self.root, 0, []
        while offset < len(tokens) and tokens[offset] in curr.children:
            child = curr.children[tokens[offset]]
            m = 0
            while (
                m < len(child.tokens)
                and offset + m < len(tokens)
                and child.tokens[m] == tokens[offset + m]
            ):
                m += 1
            if m == len(child.tokens):
                ...
                break
        return matched_blocks, tokens[offset:], curr

    def evict_leaf(self, node: RadixNode) -> Optional[List[int]]:
        """Evicts unpinned (ref_count == 0) leaf node under physical memory pressure."""
        if not node.children and node.ref_count == 0 and node.parent:
            first_tok = node.tokens[0]
            del node.parent.children[first_tok]
            return node.blocks
        return None
```
:::
