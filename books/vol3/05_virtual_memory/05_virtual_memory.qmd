...
::: {#exmp-05-sizing-the-attention-state-for .callout-example title="Sizing the Attention State for a 70-Billion Parameter Model"}
...
*Paged attention collapses memory waste from 87.5% down to 0.12%, eliminating external fragmentation.*
:::
...
![**Contiguous Memory Fragmentation**: Memory allocation breakdown under contiguous tensor layouts, illustrating external fragmentation holes and internal reservation waste as requests of variable sequence lengths arrive, execute, and depart.](images/svg/fig-vol3-kv-fragmentation.svg){#fig-vol3-kv-fragmentation width="95%"}
...
::: {#exmp-05-quantifying-contiguous-kv-cache-fragmentation .callout-example title="Quantifying Contiguous KV Cache Fragmentation on an NVIDIA H100"}
...
::: {#exmp-05-sizing-physical-blocks-for-a .callout-example title="Sizing Physical Blocks for a 70B Parameter Model"}
...
**Prefix caching** virtualizes KV cache allocation across independent serving requests by organizing retained physical memory blocks into a globally indexed prefix search structure. By indexing physical KV blocks against token sequence prefixes, an inference engine transforms redundant prompt prefill operations from compute-bound matrix multiplications into constant-time cache lookups, reclaiming accelerator capacity for active autoregressive generation (@fig-vol3-radix-tree-cache).

![**Radix Tree Prefix Cache**: Radix tree prefix cache node structure and branch invalidation across shared prompt prefixes.](images/svg/fig-vol3-radix-tree-cache.svg){#fig-vol3-radix-tree-cache width="95%"}
...
::: {#exmp-05-ttft-and-compute-savings-from .callout-example title="TTFT and Compute Savings from Radix Prefix Reuse"}
...
When serving fleets of autonomous agents across multi-node accelerator clusters, this prefix locality must be coordinated globally. Rather than maintaining isolated, uncoordinated Radix trees on each worker node, the cluster routing fabric fingerprints incoming prompts and dispatches requests to nodes holding matching prefix blocks, as illustrated in @fig-vol3-cluster-prefix-routing.

![**Cluster-Scale Prefix Trie Routing and Cache-Aware Dispatch**: Distributed prefix caching across multi-node accelerator clusters. (1) Incoming request prompts ($S=40{,}000$ tokens) are partitioned into fixed chunks ($B_{\text{chunk}}=16$) and fingerprinted using rolling Merkle hashes ($h_k = \text{Hash}(h_{k-1} \parallel c_k)$). (2) A global prefix directory maintains a distributed Radix trie and evaluates a joint affinity-load objective ($w^* = \arg\max [\mu \cdot (\text{Match}/S) - (1-\mu) \cdot (L_j/L_{\max})^\gamma]$). (3) Selected worker nodes execute prefill only for uncached sequence tails, skipping prefill for shared prompt prefixes and eliminating redundant computation across the cluster.](images/svg/cache_aware_prefix_routing.svg){#fig-vol3-cluster-prefix-routing width="95%"}
...
The serving engine evaluates chunk $I_k$ in a single forward pass iteration, alongside other active requests. The physical challenge lies in computing multi-head attention across chunk boundaries while preserving the mathematical equivalence to a monolithic forward pass (@fig-vol3-chunked-attention).

![**Chunked Prefill Attention Slicing**: Slicing the active prompt into token chunks. In iteration $k$, queries $\mathbf{Q}_k$ for the active slice attend across historical $\mathbf{K}, \mathbf{V}$ projections retrieved directly from paged accelerator memory alongside newly projected activations, guaranteeing mathematical equivalence to a monolithic forward pass with zero recompute overhead.](images/svg/chunked_prefill_attention_slicing.svg){#fig-vol3-chunked-attention width="95%"}
...
::: {.column-margin}
![](images/svg/vol3_virtual_memory_margin_003.svg){width="100%" fig-alt="Execution timeline comparing monolithic prefill stalling decode loops for 1,195 milliseconds against chunked prefill pacing decode at 159 milliseconds."}

*Chunking prompts into 512-token slices cuts P99 decode latency from 1,195 ms to 159 ms.*
:::
...
If this ratio is less than one, streaming the attention state across the peripheral bus is strictly faster than recomputing it on the tensor cores, defining the break-even wait time regimes delineated in @fig-vol3-swapping-tradeoff.
...
![**Break-Even Wait Time Regimes**: Break-even wait time regimes comparing resident retention, PCIe swapping, and recomputation across sequence length and tool pause duration.](images/svg/fig-vol3-swapping-tradeoff.svg){#fig-vol3-swapping-tradeoff width=\"95%\"}
...
::: {#exmp-05-sizing-the-break-even-frontier-for .callout-example title="Sizing the Break-Even Frontier for a 70-Billion Parameter Trajectory"}
...
Implementing physical block swapping in an inference runtime requires deep integration with the paged memory manager. The virtual memory abstraction established in PagedAttention maps invariant logical sequence blocks to non-contiguous physical page allocations. To support multi-tier swapping, the page table must be extended into a dual-residency structure (@fig-vol3-hierarchical-paging).

![**Hierarchical KV Swapping Topology Across Physical Memory Tiers**: PagedAttention virtual memory management, asynchronous PCIe Gen5 DMA offloading, and predictive prefetching. The extended block table tracks multi-tier frame residency across accelerator HBM, pinned host DRAM, and local NVMe storage, overlapping swap latency behind external tool execution and chunked prompt ingestion.](images/svg/hierarchical_kv_paging_topology_v2.svg){#fig-vol3-hierarchical-paging width="95%"}
...
::: {#exmp-05-sizing-hbm-for-multi-turn-agent .callout-example title="Sizing HBM for Multi-Turn Agent Swarms on an 8-GPU Node"}
...
When dynamic memory occupancy approaches the physical boundary of accelerator HBM (the saturation threshold $\mu \to 1$), the variance of this heavy-tailed distribution triggers non-linear queuing delays. The serving engine exhausts its pool of free physical blocks. Incoming prefill requests cannot obtain initial block tables and stall at the admission boundary. Concurrently, active autoregressive decode loops stall mid-generation because the allocator cannot map physical blocks for newly generated tokens (@fig-vol3-thrashing-curve).

![**Memory Saturation and Cache Thrashing Boundary**: Concurrency inflation from heavy-tailed tool waits and correlated branching pushes memory occupancy beyond the operating ceiling. When instantaneous demand hits the 100% thrashing boundary, emergency preemptions cascade, destroying prefix cache hit rates and inducing catastrophic latency spikes.](images/svg/memory_saturation_thrashing_curve.svg){#fig-vol3-thrashing-curve width="95%"}
