...
> **The Systems Rule of Thumb (The 95% Prefix Locality Invariant):** In multi-turn agent execution, consecutive turns share over $95\%$ of their prompt tokens ($|X^{(t)}| \gg |\Delta \mathbf{x}^{(t+1)}|$). Any inference serving engine that treats consecutive agent turns as stateless invocations wastes more than $90\%$ of its arithmetic throughput recomputing identical Key-Value activations. Serving schedulers must maintain stateful session affinity to exploit this physical prefix invariance.
...
::: {.callout-principle title="Systems Rule of Thumb: The Asynchronous Prefetch Invariant"}
Because tool execution latency ($T_{\text{tool}} \sim 1 - 30\text{ s}$) exceeds PCIe Gen5 swap latency ($T_{\text{swap}} \approx 164\text{ ms}$ for 32k tokens) by one to two orders of magnitude, never hold idle KV cache in accelerator HBM during external I/O waits. Reclaim 100% of inactive agent memory via asynchronous host-DRAM paging, and initiate predictive prefetching upon early event triggers to resume autoregressive decode with zero TTFT penalty.
:::
...
- Rather than waiting for all $n_{\text{layers}}$ to complete before initiating network transfer, the $P$-instance initiates an asynchronous **Remote Direct Memory Access (RDMA) Write** transfer for layer $l$'s physical PagedAttention blocks immediately after layer $l$ completes its forward pass.
