...
- Cumulative prefill execution time: $T_{\text{prefill, stateless}} = 472\text{,}000 \times 2.8 \times 10^{-4}\text{ s} =$ \mathbf{`{python} VirtualMemoryTtftCollapse.t_stateless_total_str`\text{ seconds}}.
- Turn-20 latency: With terminal prompt length $S_{20} = 16\text{,}000 + (19 \times 800) =$ `{python} VirtualMemoryTtftCollapse.s_turn_20_str` tokens, the agent stalls waiting for first response: $\text{TTFT}_{\text{turn-20, stateless}} = 31\text{,}200 \times 2.8 \times 10^{-4}\text{ s} =$ \mathbf{`{python} VirtualMemoryTtftCollapse.ttft_turn_20_stateless_str`\text{ seconds}}.
...
- Cumulative prefill execution time: $T_{\text{prefill, cached}} = 4.48\text{ s} + 4.26\text{ s} =$ \mathbf{`{python} VirtualMemoryTtftCollapse.t_cached_total_str`\text{ seconds}}.
- Per-turn interactive latency ($t \ge 2$): $\text{TTFT}_{\text{turn-}t\text{, cached}} = 800 \times 2.8 \times 10^{-4}\text{ s} =$ \mathbf{`{python} VirtualMemoryTtftCollapse.ttft_cached_ms_str`\text{ milliseconds}}.
3. *Comparative Performance Speedup:*
- Trajectory prefill compute reduction: $\frac{132.16\text{ s}}{8.74\text{ s}} =$`{python} VirtualMemoryTtftCollapse.speedup_prefill_mult_str` faster.
- Interactive TTFT reduction at turn 20: $\frac{8.74\text{ s}}{0.224\text{ s}} =$`{python} VirtualMemoryTtftCollapse.speedup_ttft_mult_str` faster.
...
To illustrate how physical block aliasing and Copy-on-Write are orchestrated by the serving engine, @lst-vol3-vm-pagedattention-block-allocator implements a complete `PagedMemoryManager` executing zero-copy sequence branching and token-append Copy-on-Write mutations.

::: {#lst-vol3-vm-pagedattention-block-allocator lst-cap="**PagedAttention Block Allocation and Copy-on-Write Execution Engine**: Python reference implementation of physical block allocation, logical-to-physical block table translation, zero-copy sequence branching, and Copy-on-Write mutation under PagedAttention."}
```python
...
@dataclass
class PhysicalBlock:
    """Represents a fixed-capacity physical memory frame in accelerator HBM."""
    block_id: int
    block_size: int = 16
    token_dim: int = 128
    ref_count: int = 0
    ...
    def __post_init__(self):
        self.keys = np.zeros((self.block_size, self.token_dim), dtype=np.float32)
        self.values = np.zeros((self.block_size, self.token_dim), dtype=np.float32)


class PagedMemoryManager:
    """Orchestrates non-contiguous physical block allocation, address translation, and CoW."""

    def __init__(self, num_blocks: int, block_size: int = 16, token_dim: int = 128):
        self.block_size = block_size
        self.token_dim = token_dim
        self.blocks = [PhysicalBlock(i, block_size, token_dim) for i in range(num_blocks)]
        self.free_pool: List[int] = list(range(num_blocks))
        # Logical sequence ID -> list of physical block IDs
        self.block_tables: Dict[str, List[int]] = {}

    def allocate_block(self) -> int:
        """Pops an unallocated physical block from the free list in O(1) time."""
        if not self.free_pool:
            raise MemoryError("HBM exhaustion: No free physical blocks available.")
        bid = self.free_pool.pop()
        self.blocks[bid].ref_count = 1
        self.blocks[bid].num_tokens = 0
        return bid

    def free_block(self, bid: int):
        """Decrements reference count; reclaims block to free pool if unreferenced."""
        block = self.blocks[bid]
        block.ref_count -= 1
        if block.ref_count <= 0:
            block.ref_count = 0
            block.num_tokens = 0
            self.free_pool.append(bid)

    def fork_sequence(self, parent_seq_id: str, child_seq_id: str):
        """Creates a zero-copy child sequence by aliasing parent physical blocks (ref_count++)."""
        parent_table = self.block_tables[parent_seq_id]
        self.block_tables[child_seq_id] = list(parent_table)
        for bid in parent_table:
            self.blocks[bid].ref_count += 1

    def append_token(self, seq_id: str, k_vec: np.ndarray, v_vec: np.ndarray):
        """Appends a new KV activation, triggering Copy-on-Write if the tail block is shared."""
        ...
        tail_bid = table[-1]
        tail_block = self.blocks[tail_bid]

        # Case 1: Tail block is saturated; allocate fresh unshared block
        if tail_block.num_tokens == self.block_size:
            new_bid = self.allocate_block()
            table.append(new_bid)
            tail_bid = new_bid
            tail_block = self.blocks[tail_bid]

        # Case 2: Tail block is shared with parent/sibling (ref_count > 1) -> Copy-on-Write
        elif tail_block.ref_count > 1:
            cow_bid = self.allocate_block()
            cow_block = self.blocks[cow_bid]
            # Copy active tokens from shared block into private block
            cow_block.keys[:tail_block.num_tokens] = tail_block.keys[:tail_block.num_tokens]
            cow_block.values[:tail_block.num_tokens] = tail_block.values[:tail_block.num_tokens]
            cow_block.num_tokens = tail_block.num_tokens

            # Detach from shared block and update child table
            self.free_block(tail_bid)
            table[-1] = cow_bid
            tail_block = cow_block

        # Append activation into unshared tail block
        idx = tail_block.num_tokens
        tail_block.keys[idx] = k_vec
        tail_block.values[idx] = v_vec
        tail_block.num_tokens += 1
```
:::
...
To demonstrate the mechanics of prefix matching, edge splitting, and reference-counted leaf eviction, @lst-vol3-vm-radix-prefix-tree implements a complete `RadixPrefixCache` in Python.

::: {#lst-vol3-vm-radix-prefix-tree lst-cap="**RadixAttention Prefix Cache Engine**: Python reference implementation of compressed prefix trie indexing, dynamic edge splitting, prefix lookup, and reference-counted leaf-only LRU eviction for PagedAttention blocks."}
```python
from dataclasses import dataclass, field
import time
...
@dataclass
class RadixTreeNode:
    """Compressed trie node mapping a token sequence to resident physical PagedAttention blocks."""
    tokens: List[int]
...
class RadixPrefixCache:
    """Manages dynamic prefix sharing, edge splitting, and leaf LRU eviction."""

    def __init__(self):
        # Root node matches empty prefix with no physical blocks
        self.root = RadixTreeNode(tokens=[])

    def match_prefix(self, prompt_tokens: List[int]) -> Tuple[List[int], List[int], RadixTreeNode]:
        """Traverses the radix tree to discover the longest cached token prefix."""
        curr = self.root
        matched_blocks: List[int] = []
        token_offset = 0

        while token_offset < len(prompt_tokens):
            first_token = prompt_tokens[token_offset]
            ...
            match_len = 0
            while (
                match_len < len(edge_tokens)
                and token_offset + match_len < len(prompt_tokens)
                and edge_tokens[match_len] == prompt_tokens[token_offset + match_len]
            ):
                match_len += 1

            if match_len == len(edge_tokens):
                # Full edge matched; accumulate blocks and descend
                matched_blocks.extend(child.physical_blocks)
                token_offset += match_len
                curr = child
                curr.last_accessed = time.time()
            else:
                # Partial edge match terminates prefix traversal
                break

        remaining_tokens = prompt_tokens[token_offset:]
        return matched_blocks, remaining_tokens, curr

    def split_edge(
        self, parent: RadixTreeNode, child: RadixTreeNode, split_idx: int, prefix_blocks: List[int], suffix_blocks: List[int]
    ) -> RadixTreeNode:
        """Splits an edge into a shared parent node and a downstream suffix child node."""
        first_token = child.tokens[0]
        # Create intermediate node holding the matched prefix
        split_node = RadixTreeNode(
            tokens=child.tokens[:split_idx],
            physical_blocks=prefix_blocks,
            parent=parent,
            ref_count=child.ref_count,
            last_accessed=time.time()
        )
        # Update original child to hold remaining suffix
        child.tokens = child.tokens[split_idx:]
        child.physical_blocks = suffix_blocks
        child.parent = split_node

        split_node.children[child.tokens[0]] = child
        parent.children[first_token] = split_node
        return split_node

    def evict_lru_leaf(self) -> Optional[List[int]]:
        """Finds and evicts the least-recently-used unpinned (ref_count == 0) leaf node."""
        candidate_leaf: Optional[RadixTreeNode] = None
        oldest_time = float("inf")

        def find_leaves(node: RadixTreeNode):
            nonlocal candidate_leaf, oldest_time
        ...
        if candidate_leaf is None:
            return None  # All resident nodes are pinned by active requests

        parent = candidate_leaf.parent
        first_token = candidate_leaf.tokens[0]
        freed_blocks = candidate_leaf.physical_blocks
...
- Using hardware-offloaded NIC Memory Translation Tables (MTT) and GPUDirect RDMA, the network interface card bypasses host CPUs and operating system networking stacks entirely, transferring bytes directly from the local GPU's HBM across the InfiniBand/RoCE fabric into the remote $D$-instance's registered GPU physical page frames.
