...
To appreciate the physical scale of this footprint, consider a 70-billion parameter MHA foundation model ($n_{\text{layers}} = 80$, $d_{\text{model}} = 8192$) operating in 16-bit precision ($b = 2\text{ bytes}$). Storing the KV activations of a single token requires:

$$S_{\text{token}} = 2 \times 2\text{ bytes} \times 80\text{ layers} \times 8192\text{ elements} = 2\text{,}621\text{,}440\text{ bytes/token} \approx 2.62\text{ MB/token}$$

As detailed in @tbl-vol3-virtualmem-mha-sizing, scaling context depth across extended agent trajectories rapidly overwhelms physical accelerator memory:
...
2. **Grouped-Query Attention (GQA)**\index{Grouped-Query Attention} [@ainslie2023gqa]: Partitions Query heads into $G$ disjoint groups, assigning one Key head and one Value head to each group ($n_{\text{kv\_heads}} = G$). The memory footprint scales proportionally to the compression ratio $G / n_{\text{heads}}$:
   ...
   For example, in Llama-3 70B ($G = 8$ KV heads versus $64$ Query heads), the footprint shrinks by $8\times$, from $2.62\text{ MB/token}$ down to $327.68\text{ KB/token}$.
3. **Multi-Head Latent Attention (MLA)**\index{Multi-Head Latent Attention} [@deepseek2024v3]: Compresses Key and Value states into a low-dimensional shared latent space. Rather than caching full attention heads, the runtime projects the hidden state $\mathbf{x}_t$ into a compact compressed latent vector $\mathbf{c}_t^{\text{KV}} \in \mathbb{R}^{d_c}$ (where $d_c \ll n_{\text{heads}} \times d_{\text{head}}$), paired with a decoupled rotary key vector $\mathbf{k}_t^R \in \mathbb{R}^{d_R}$ used for Rotary Position Embeddings (RoPE). During decoding, the full Key and Value heads are reconstructed on the fly inside GPU SRAM using projection matrices $\mathbf{W}_{UK}, \mathbf{W}_{UV} \in \mathbb{R}^{d_c \times (n_{\text{heads}} \cdot d_{\text{head}})}$. The cached footprint per token is:
   $$S_{\text{token, MLA}} = b \times n_{\text{layers}} \times (d_c + d_R)$$
   In DeepSeek-V3 ($d_c = 512, d_R = 64$, FP8 precision $b = 1$, $n_{\text{layers}} = 61$), storing one token requires only $1 \times 61 \times (512 + 64) = 35\text{,}136\text{ bytes} \approx 34.3\text{ KB/token}$—a **$76\times$ reduction** relative to MHA. MLA exemplifies the classic systems trade-off: spending cheap Tensor Core arithmetic inside fast on-chip SRAM to avoid streaming multi-megabyte tensors over slow HBM buses.
...
The physical intuition behind @eq-vol3-virtualmem-prefill-flops is grounded in basic matrix multiplication: every weight parameter executes 1 multiply and 1 add (2 FLOPs) per token. For a 70B model ($P = 70 \times 10^9$), evaluating a single token requires $2 \times 70 \times 10^9 = 140\text{ GFLOPs/token}$. Across realistic context lengths ($S < 32\text{k}$), this linear projection term $2 P S$ accounts for over $95\%$ of prefill arithmetic, while the quadratic term $4 n_{\text{layers}} d_{\text{model}} S^2$ represents the full self-attention matrix multiplications ($\mathbf{Q}\mathbf{K}^T$ and $\mathbf{A}\mathbf{V}$).
...
4. **Token Insertion and Memory Fragmentation (Line 15):** The new key-value activation $\mathbf{x}$ writes into the next free slot of the private tail block. Because blocks are fixed at $B_{\text{block}} = 16$, memory waste is strictly confined to the unused slots of the single active tail block—bounding internal memory fragmentation to $< 4\%$ across the entire serving cluster.
...
1. **The Prefill Phase (Compute-Bound):** The prefill pass ingests the input prompt $\mathbf{X}$ of length $S_{\text{prompt}}$, computing the full self-attention matrix and Feed-Forward Network (FFN) projections across all tokens in parallel. The arithmetic intensity $\mathcal{I}_{\text{prefill}}$ (ratio of floating-point operations to memory access bytes) is derived from first principles:
   $$\mathcal{I}_{\text{prefill}} = \frac{\text{Compute FLOPs}}{\text{Memory Access Bytes}} \approx \frac{2 P S_{\text{prompt}} + 4 n_{\text{layers}} d_{\text{model}} S_{\text{prompt}}^2}{P \cdot b + 2 n_{\text{layers}} d_{\text{kv}} S_{\text{prompt}} \cdot b} \approx \frac{2 S_{\text{prompt}}}{b} \quad \left[\frac{\text{FLOPs}}{\text{byte}}\right]$$
   For a prompt of length $S_{\text{prompt}} = 4\text{,}096$ tokens in 16-bit precision ($b = 2\text{ bytes}$), the arithmetic intensity is $\mathcal{I}_{\text{prefill}} \approx 4\text{,}096\text{ FLOPs/byte}$ (or $\approx 2\text{,}000 - 3\text{,}000\text{ FLOPs/byte}$ when factoring in intermediate activation and KV cache writes). This value sits far to the right of the hardware ridge point on an NVIDIA H100 GPU ($\approx 150\text{ FLOPs/byte}$ for BF16 Tensor Cores vs HBM3). The prefill phase is **strictly compute-bound**, saturating Tensor Core matrix multiply units at peak floating-point throughput.
