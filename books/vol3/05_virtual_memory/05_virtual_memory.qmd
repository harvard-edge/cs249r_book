...
- Companion Code: src/vol3/05_virtual_memory/ (paged_kv_allocator.py, radix_cache_tree.py, cow_speculative_branch.py)
-->
...
_Why does an agent swarm with shared instructions waste eighty percent of accelerator memory?_

This chapter introduces the systems architecture required to transform volatile accelerator memory into a virtualized, demand-paged context substrate for autonomous agents. While the token dynamics of an agent's working set govern capacity from an information-theoretic perspective, this chapter focuses on the physical memory allocation mechanics required to sustain long-horizon agent execution. We explore how modern serving systems eliminate internal and external Key-Value cache fragmentation, examine the temporal and spatial prefix locality inherent in iterative tool execution loops, dissect the mechanics of block-level virtual memory translation, and analyze dynamic prefix indexing via Radix trees to support zero-overhead speculative branching in tree search. In agentic systems, the runtime must co-design the neural policy proposal engine with deterministic execution boundaries.
...
: Memory footprint per token across modern foundation model attention architectures. {#tbl-vol3-vm-model-kv-footprint tbl-colwidths="[20,10,10,10,10,10,10,10,10]"}
...
: Structural decomposition and mutation characteristics of an autonomous agent prompt. {#tbl-vol3-vm-prompt-anatomy tbl-colwidths="[20,40,15,10,15]"}
...
: Trade-offs in PagedAttention block size selection ($B$) across memory efficiency and GPU memory bus throughput. {#tbl-vol3-vm-block-size-tradeoffs tbl-colwidths="[10,20,20,20,15,15]"}
...
- Shared prefix length: $L_{\text{shared}} = 24\text{,}000\text{ tokens}$.
...
: Scaling arithmetic for speculative tree search algorithms under naive execution versus Radix tree prefix caching. {#tbl-vol3-vm-deliberation-scaling tbl-colwidths="[15,10,10,10,20,15,10,10]"}
...
These common misconceptions about prefix caching and memory paging often lead to degraded agent performance or catastrophic memory crashes in production inference systems.
...
::: {.callout-chapter-connection title="From Volatile Memory to Persistent State"}
...
## Quantitative Problems {#sec-vol3-virtual-memory-problems}

**1. PagedAttention Fragmentation Bounds**
Consider a deployment of Llama-3-70B (FP16, GQA 8:1, $S_{\text{token}} = 160\text{ KB}$) on a GPU cluster. A batch of $N = 256$ agent trajectories are actively generating. Their current context lengths are uniformly distributed between $1\text{,}000$ and $32\text{,}000$ tokens. The system uses PagedAttention with a block size of $B$. 
*(a)* Derive the expected aggregate internal fragmentation (in MB) across the entire batch for $B \in \{8, 16, 32, 64\}$. 
*(b)* If the GPU global memory bus requires 512-byte aligned coalesced reads for maximum bandwidth, which block sizes incur a memory access penalty, and why?

**2. Swarm Prefix Locality Economics**
An automated QA agent is instantiated as a swarm of $10$ parallel workers. Each worker shares an identical root prompt consisting of $18\text{,}000$ tokens of repository context. Each worker then executes a unique tree of thoughts exploring $K=3$ branches to a depth of $D=3$. The incremental tokens added per branch is $\Delta L = 400$ tokens.
*(a)* Calculate the total physical memory required to cache the KV activations for the entire swarm under a stateless serving architecture versus a Radix tree caching architecture (assume Llama-3-70B FP16 footprint).
*(b)* Calculate the theoretical FLOPs saved by the Radix cache during the generation of the full search trees across all $10$ workers.

**3. Chunked Prefill Latency**
A code-refactoring agent completes turn $t=15$ with a context length of $24\text{,}000$ tokens. For turn $t=16$, it appends a $1\text{,}200$-token compiler error and a $200$-token thought block. The inference server operates at an effective prefill throughput of $600\text{ TFLOPS}$ (using a 70B parameter model).
*(a)* If the server experiences a Radix cache hit on the $24\text{,}000$-token prefix, calculate the chunked prefill latency for the new tokens. 
*(b)* If a timestamp nonce at the beginning of the system prompt causes a complete cache miss, what is the resulting Time-to-First-Token (TTFT) latency penalty?
