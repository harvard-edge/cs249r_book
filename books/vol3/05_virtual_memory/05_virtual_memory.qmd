...
- In our 80-layer Llama-3 70B example, each layer produces $10.49\text{ GB} / 80 \approx 131\text{ MB}$ of KV state. Over an $800\text{ Gbps}$ ($100\text{ GB/s}$) fabric, transferring $131\text{ MB}$ takes only $1.31\text{ ms}$. Because computing the subsequent layer's forward pass on an H100 takes $5 - 10\text{ ms}$, the network transfer ($1.31\text{ ms}$) completes well before the next layer finish computation.
