...
## 5.1 The Physical Geometry of Attention Memory {#sec-vol3-virtualmem-geometry}
...
::: {.callout-note title="Checkpoint: Attention geometry and memory fragmentation"}
Before proceeding to block paging mechanics, verify your analytical grasp of physical KV cache geometry:

- [ ] **Head Geometry Scaling**: Can you derive the precise memory footprint per token ($S_{\text{token}}$) for a 64-layer model with $d_{\text{model}} = 8192$, using GQA with 64 query heads and 8 KV heads under FP8 precision ($b = 1$ byte)?
- [ ] **MLA Latent Decoupling**: Why does DeepSeek-V3 store only a compressed latent vector $d_c = 512$ and decoupled rotary key $d_R = 64$ per layer rather than physical attention heads? How are full Key and Value heads reconstructed during attention execution?
- [ ] **External Fragmentation Collapse**: Under dynamic tensor resizing without paging, describe the exact physical memory state where an inference engine triggers an Out-of-Memory (OOM) abort despite $40\text{ GB}$ of unallocated HBM being physically present on the device.
- [ ] **Prefix Sharing Asymptotics**: For an agent trajectory appending $\Delta S = 500$ tokens per turn to an initial prompt of $L_{\text{root}} = 10\text{,}000$ tokens, calculate the prefix sharing ratio $\rho(t, t+1)$ at turn $t = 1$, $t = 5$, and $t = 20$.
:::

## 5.2 PagedAttention: Virtual Memory for KV Caches {#sec-vol3-virtualmem-pagedattention}
...
::: {.callout-note title="Checkpoint: PagedAttention and address translation"}
Verify your systems understanding of virtual memory block mapping:

- [ ] **Address Calculation**: An agent sequence reaches logical token index $i = 1\text{,}047$. The runtime uses block size $B_{\text{block}} = 16$. Calculate the logical block index $j$, the intra-block offset $o$, and the byte offset within the physical block for layer $l = 12$ on Llama-3 70B ($S_{\text{token\_layer}} = 4\text{,}096$ bytes).
- [ ] **FlashAttention Fusion**: How does the PagedAttention kernel traverse the block table inside GPU Shared Memory (SRAM) without paying global memory latency penalties on every token dot product?
- [ ] **CoW Reference Lifecycle**: Trace the exact value of $\text{ref\_count}(P)$ for a physical block holding a system prompt when an agent forks into 8 speculative rollouts, 3 rollouts are pruned by a verifier, 4 rollouts complete execution, and 1 rollout continues generating.
- [ ] **Block Sizing Trade-Offs**: Why does selecting $B_{\text{block}} = 4$ degrade memory bus efficiency on an NVIDIA H100 GPU compared to $B_{\text{block}} = 16$, even though it reduces tail internal waste?
:::

## 5.3 Memory Defragmentation, Page Tables, and Block Allocation {#sec-vol3-virtualmem-defrag}
...
::: {.callout-note title="Checkpoint: Radix trees and memory scheduling"}
Verify your analytical command of dynamic prefix indexing and memory scheduling:

- [ ] **Radix Tree Splitting**: Trace the exact node and edge modifications when a Radix tree containing edge `"def test_login():\n    user = "` is matched by a new prompt `"def test_login():\n    admin = "`. What happens to the physical block mappings?
- [ ] **Leaf Eviction Safety**: Under severe memory pressure, why must the cache manager evict unpinned leaf nodes before internal branch nodes? What systemic failure occurs if an intermediate node is evicted while child leaves remain indexed?
- [ ] **Chunked Prefill Roofline**: How does co-scheduling a 512-token prefill chunk alongside 32 decode tokens alter the arithmetic intensity of the GPU execution batch?
- [ ] **Tool-Wait Swapping Economics**: If an agent context of 64,000 tokens ($20.97\text{ GB}$) is swapped to host DRAM over PCIe Gen5 x16 ($64\text{ GB/s}$), what is the minimum tool execution time required to completely hide the round-trip swap latency?
:::

## 5.4 Prefill-Decode Disaggregation and Remote KV Paging {#sec-vol3-virtualmem-disaggregation}
...
::: {.callout-note title="Checkpoint: Prefill-decode disaggregation"}
Evaluate your understanding of disaggregated serving mechanics:

- [ ] **Roofline Divergence**: Why does the arithmetic intensity of prompt prefill scale linearly with prompt length ($\mathcal{I} \approx S / b$), while the arithmetic intensity of autoregressive decode remains strictly constant ($\mathcal{I} \approx B_{\text{batch}} / b$)?
- [ ] **Network Bandwidth Ceiling**: Calculate the minimum network bandwidth required to transfer a 64,000-token KV cache on Llama-3 70B ($20.97\text{ GB}$) in less than $150\text{ ms}$.
- [ ] **Pipelined RDMA Overlap**: Explain how layer-by-layer RDMA Write operations hide network serialization latency behind GPU Tensor Core computation during the prefill phase.
- [ ] **Cluster Ratio Sizing**: An agent platform experiences an average prompt length of $\bar{S}_{\text{prompt}} = 20\text{,}000$ tokens and an average generation length of $\bar{S}_{\text{gen}} = 500$ tokens. If $T_{\text{prefill\_token}} = 0.25\text{ ms}$ and $T_{\text{decode\_token}} = 25\text{ ms}$, calculate the optimal ratio of Prefill nodes to Decode nodes ($N_P : N_D$).
:::

## 5.5 Hardware-Aware KV Cache Compression: Quantization and SVD {#sec-vol3-virtualmem-compression}
...
::: {.callout-note title="Checkpoint: KV cache compression and numerical hardware"}
Verify your analytical command of KV cache compression mechanics:

- [ ] **FP8 Format Selection**: Why is FP8 E4M3 strictly preferred for Value activation vectors, while FP8 E5M2 is preferred for Key vectors under Rotary Position Embeddings (RoPE)?
- [ ] **Outlier Channel Underflow**: Explain mathematically how a single outlier activation dimension of magnitude $100\times$ typical variance causes uniform quantization to destroy semantic precision across normal channels.
- [ ] **SVD vs Quantization**: Compare the computational overhead of truncated SVD low-rank compression against scalar INT4 quantization. Under what specific context horizon does SVD become computationally viable?
- [ ] **Decode Throughput Speedup**: On a memory-bandwidth-bound GPU cluster serving an agent context of 128,000 tokens, calculate the theoretical speedup in decode token throughput when migrating from FP16 to FP8 E4M3/E5M2.
:::
...
## Quantitative Problems {#sec-vol3-virtualmem-problems}

Apply the mathematical derivations and systems principles established throughout this chapter to solve the following quantitative problems.

### Problem 1: PagedAttention memory fragmentation and bus coalescing bounds

Consider an enterprise serving cluster hosting Llama-3 70B ($n_{\text{layers}} = 80$, $d_{\text{model}} = 8192$, GQA with $n_{\text{heads}} = 64$ query heads and $n_{\text{kv\_heads}} = 8$ KV heads, $d_{\text{head}} = 128$). The platform actively serves a batch of $N = 256$ concurrent autonomous coding agents. The sequence lengths $S_i$ of the active agents are uniformly distributed across the range $[2\text{,}000, \, 64\text{,}000]$ tokens:

$$S_i \sim \mathcal{U}(2\text{,}000, \, 64\text{,}000)$$

The cluster deploys PagedAttention with physical block size $B_{\text{block}}$ tokens.

1. **Footprint per Token:** Calculate the exact physical memory footprint per token $S_{\text{token}}$ (in bytes) under:
   - 16-bit precision (BF16, $b = 2$ bytes).
   - 8-bit precision (FP8, $b = 1$ byte).
2. **Internal Fragmentation Bounds:** For block sizes $B_{\text{block}} \in \{8, 16, 32, 64\}$ in BF16 precision:
   - Calculate the theoretical expected internal waste per sequence $\mathbb{E}[\text{Waste}_{\text{seq}}]$ (in kilobytes).
   - Calculate the expected aggregate internal fragmentation across the entire batch of 256 agents (in megabytes).
   - Calculate internal fragmentation as a percentage of total allocated KV cache memory.
3. **Memory Bus Coalescing and Access Penalties:** Modern GPU memory controllers execute global memory transactions in aligned 256-byte or 512-byte cache-line sectors.
   - For a single transformer layer, calculate the physical memory size of one block:
     $$M_{\text{block\_layer}} = 2 \times b \times B_{\text{block}} \times n_{\text{kv\_heads}} \times d_{\text{head}}$$
   - Evaluate $M_{\text{block\_layer}}$ for $B_{\text{block}} = 8$ versus $B_{\text{block}} = 16$ in BF16 precision. Explain which block size introduces unaligned memory transactions on a 512-byte memory bus sector, and quantify the resulting memory bandwidth efficiency loss.

### Problem 2: Prefill-decode disaggregation and RDMA network sizing

An autonomous software engineering platform serves an agent workload with the following operational profile:

- Average input prompt length: $\bar{S}_{\text{prompt}} = 32\text{,}768$ tokens.
- Average generated output length: $\bar{S}_{\text{gen}} = 512$ tokens.
- Foundation Model: 70B parameter model with GQA ($S_{\text{token}} = 320\text{ KB/token}$ in BF16).
- Prefill Cluster ($P$-nodes): Sustained cluster prefill throughput $R_{\text{prefill}} = 400\text{ TFLOPS}$ ($4 \times 10^{14}\text{ FLOPs/s}$).
- Decode Cluster ($D$-nodes): Sustained decode generation throughput per GPU $R_{\text{decode}} = 40\text{ tokens/second}$.
1. **Execution Time Asymmetry:**
   - Calculate the average prefill execution time $T_{\text{prefill}}$ for one incoming prompt on a $P$-node (assume prefill FLOPs $\approx 2 P \bar{S}_{\text{prompt}}$).
   - Calculate the average decode execution time $T_{\text{decode}}$ required to generate the 512 output tokens on a $D$-node.
   - Calculate the Prompt-to-Generation ratio $\gamma$ and the execution time ratio $T_{\text{decode}} / T_{\text{prefill}}$.
2. **Cluster Sizing Balance:**
   - To sustain a continuous throughput of 10 requests completed per second without queue buildup, calculate the number of active Prefill GPUs ($N_P$) and Decode GPUs ($N_D$) required.
   - What percentage of cluster GPU silicon is dedicated to prefill versus decode?
3. **Network Bandwidth and Disaggregation Viability:**
   - Calculate the total physical size of the Key-Value cache (in gigabytes) transferred from the $P$-node to the $D$-node for one 32,768-token prompt.
   - If the platform enforces a strict Service Level Objective (SLO) that network KV transfer time must not exceed $15\%$ of prefill execution time ($T_{\text{network}} \le 0.15 \cdot T_{\text{prefill}}$), calculate the minimum required sustained network bandwidth $B_{\text{network}}$ (in Gbps).
   - Can this SLO be satisfied over standard $100\text{ Gbps}$ Ethernet? What network fabric is required?

### Problem 3: Speculative tree search memory expansion and CoW savings

A deliberative software engineering agent utilizes Monte Carlo Tree Search (MCTS) to explore alternative patch strategies across a large codebase.
- Shared Root Context: $L_{\text{root}} = 24\text{,}000$ tokens (system prompt, MCP tool schemas, repository map, and issue description).
- MCTS Topology: Branching factor $K = 4$, search depth $D = 3$.
- Suffix Growth: Each node in the search tree appends an average thought and action trace of $\Delta L = 400$ tokens.
- Foundation Model: Llama-3 70B in BF16 ($S_{\text{token}} = 327\text{,}680\text{ bytes/token}$, $P = 70 \times 10^9$ parameters).
- The cluster operates with sustained prefill throughput $R_{\text{prefill}} = 500\text{ TFLOPS}$.
1. **Tree Topology Sizing:**
   - Calculate the total number of non-root nodes across the entire MCTS search tree:
     $$N_{\text{nodes}} = \sum_{d=1}^{D} K^d$$
   - Calculate the total number of terminal leaf trajectories ($K^D$).
2. **Physical Memory Evaluation:**
   - *Naive Contiguous Allocation:* Calculate the total physical memory (in gigabytes) required to store the KV caches if every path from the root to a leaf is allocated independently without paging or sharing.
   - *PagedAttention with Copy-on-Write:* Calculate the total physical memory (in gigabytes) required if all nodes share ancestral KV blocks via PagedAttention and Radix tree indexing.
   - Calculate the exact physical memory reclaimed (in gigabytes) and the compression ratio achieved by virtual memory block aliasing.
3. **Prefill Compute and Latency Elimination:**
   - Calculate the total prompt tokens ingested and cumulative prefill compute time (in GPU-seconds) under naive independent evaluation of all leaf paths.
   - Calculate the total prompt tokens ingested and cumulative prefill compute time under Radix tree prefix caching, where each search node is prefilled exactly once.
   - Calculate the net compute speedup and total PFLOPs eliminated by virtual context memory.
