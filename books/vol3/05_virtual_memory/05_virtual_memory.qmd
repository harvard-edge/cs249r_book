...
Consider an inference cluster serving an open-weights 70-billion parameter model structured with the canonical architectural parameters from @sec-vol3-processor-tokenization ($L=80$ layers, $H_{\text{q}}=64$ query heads, $H_{\text{kv}}=8$ GQA key-value heads, head dimension $d_{\text{head}}=128$, and 16-bit `bfloat16` precision with $P=2\text{ bytes}$).
