...
| `PAUSED`                          | Stopped at a turn boundary on request      | The record only                                     | `READY`, `FINALIZING`                                      |
...
Every model reply also reports why it ended, in the stop reason, and the harness treats each stop reason as an instruction about what to do next, as @tbl-vol3-stop-reasons summarizes. Providers name these values differently, but every call surface reports the same few cases.

| Stop reason                | What it means                               | Harness action                                                                     |
|:---------------------------|:--------------------------------------------|:-----------------------------------------------------------------------------------|
| **End of turn**            | The model proposes no further action        | Enter `FINALIZING` and run the completion check                                    |
| **Tool use**               | The reply contains one or more tool calls   | Validate each call; hold irreversible calls for approval, dispatch the rest        |
| **Output limit reached**   | The reply was cut off at the output ceiling | Dispatch nothing; return a truncation notice as an observation and count the event |
| **Stop sequence**          | A configured delimiter ended the reply      | Parse according to the delimiter's configured meaning                              |
| **Refusal or safety stop** | The provider or model declined to continue  | Record it; escalate or finalize as the task's policy directs                       |
| **Error or deadline**      | The call failed or exceeded its deadline    | Retry within the trajectory's budget; otherwise finalize as `FAILED`               |

: **Stop Reasons as Harness Control**: How the harness maps each class of stop reason reported by the call surface to its next action. {#tbl-vol3-stop-reasons}

Kill is the one request that does not wait. It exists for cases where finishing the turn is itself the hazard, such as a sandbox escape attempt or a runaway tool writing to shared storage. The harness aborts any model call, terminates every process in the trajectory's sandbox through the process-group teardown of @sec-vol3-tool-calling-streaming, and enters `FINALIZING` with the reason recorded. Breaking a turn has a price. A tool call dispatched just before the kill may or may not have taken effect, and its outcome is now in doubt. The harness marks such calls as in doubt rather than failed and leaves them to the settlement machinery of @sec-vol3-tool-calling-idempotency, which uses the call's idempotency key to learn whether the effect happened. When the trajectory has spawned subagents, cancel and kill must also reach them; @sec-vol3-multiagent-backpressure extends the same rule across agents.
...
# │
# │ Goal: Show which ceiling binds for a 50-turn trajectory, and how a broken
# │       prefix cache moves the binding dimension from turns to dollars.
# │ Show: Tokens processed, cached share, dollar cost with and without prefix
# │       caching, and the budget share used in each dimension.
# │ How:  Input re-sent per turn is an arithmetic series; with an append-only
# │       layout only the first prompt and each turn's new tokens are uncached.
# │       All inputs are illustrative scenario assumptions (no real price list
# │       or model is named), so they live in LOAD rather than the registry.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_int, fmt_usd, fmt_percent, check

class HarnessBudget:
    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    turns = 50  # scenario: turns completed so far
    h_max = 80  # scenario: turn ceiling
    n_max = 1_500_000  # scenario: token ceiling
    t_max = 900  # scenario: wall-clock ceiling, seconds
    c_max = 3.00  # scenario: dollar ceiling
    t_wall = 460  # scenario: harness clock reading, seconds
    s0 = 6_000  # scenario: first prompt (system prompt, tools, task)
    delta = 400  # scenario: tokens appended per turn
    ...
    hit_rate = n_cached / n_in
    n_out = turns * k_out
    n_total = n_in + n_out

    c_out = n_out * p_out / 1e6
    c_uncached = n_uncached * p_in / 1e6
    c_cached = n_cached * p_cache / 1e6
    c_total = c_out + c_uncached + c_cached
    c_in_nocache = n_in * p_in / 1e6
    c_total_nocache = c_in_nocache + c_out
    cache_saving = c_total_nocache / c_total
    ...
    check(c_in_nocache > c_out, "Re-sent input should dominate the uncached bill")
    check(rho_dollars_nocache < 0.90, "The uncached run should sit in the steer band, not restrict")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    turns_str = fmt_int(turns)
    h_max_str = fmt_int(h_max)
    n_max_m_str = fmt(n_max / 1e6, precision=1)
    ...
    n_in_str = fmt_int(n_in)
    n_uncached_str = fmt_int(n_uncached)
    n_cached_str = fmt_int(n_cached)
    hit_rate_str = fmt_percent(hit_rate, style="prose")
    n_out_str = fmt_int(n_out)
    n_total_str = fmt_int(n_total)
    c_out_str = fmt_usd(c_out, precision=2)
    c_uncached_str = fmt_usd(c_uncached, precision=2)
    c_cached_str = fmt_usd(c_cached, precision=2)
    c_total_str = fmt_usd(c_total, precision=2)
    c_in_nocache_str = fmt_usd(c_in_nocache, precision=2)
    c_total_nocache_str = fmt_usd(c_total_nocache, precision=2)
    cache_saving_str = fmt(cache_saving, precision=1)
    price_ratio_str = fmt_int(price_ratio)
...
**Problem**: A debugging trajectory has run `{python} HarnessBudget.turns_str` turns under a budget of `{python} HarnessBudget.h_max_str` turns, `{python} HarnessBudget.n_max_m_str` million tokens, `{python} HarnessBudget.t_max_str` s of wall-clock time, and `{python} HarnessBudget.c_max_str`. The inputs are illustrative assumptions. The first call sends `{python} HarnessBudget.s0_str` tokens (system prompt, tool definitions, task), each turn appends `{python} HarnessBudget.delta_str` tokens of action and observation, and each reply has `{python} HarnessBudget.k_out_str` output tokens. Prices are `{python} HarnessBudget.p_in_str` per million uncached input tokens, `{python} HarnessBudget.p_cache_str` per million cached input tokens, and `{python} HarnessBudget.p_out_str` per million output tokens. The context is laid out append-only, so every turn after the first reuses the previous prefix from the cache and pays full price only for its new tokens, and the harness's clock reads `{python} HarnessBudget.t_wall_str` s. Which ceiling binds, with and without a working prefix cache?

**Tokens**: Turn $k$ re-sends $s_0 + \Delta (k-1)$ input tokens, so input across the run is $\sum_{k=1}^{H} \left(s_0 + \Delta (k-1)\right)$, which here is `{python} HarnessBudget.n_in_str` tokens. Only the first prompt and each later turn's appended tokens miss the cache, `{python} HarnessBudget.n_uncached_str` tokens in all, leaving `{python} HarnessBudget.n_cached_str` cached tokens, a hit rate of `{python} HarnessBudget.hit_rate_str`. Output adds `{python} HarnessBudget.n_out_str` tokens, for `{python} HarnessBudget.n_total_str` tokens processed.

**Dollars with caching**: Output costs `{python} HarnessBudget.c_out_str`, uncached input `{python} HarnessBudget.c_uncached_str`, and cached input `{python} HarnessBudget.c_cached_str`, for a total of `{python} HarnessBudget.c_total_str`.

**Dollars without caching**: Billed entirely at the uncached price, input costs `{python} HarnessBudget.c_in_nocache_str` and the total becomes `{python} HarnessBudget.c_total_nocache_str`, about `{python} HarnessBudget.cache_saving_str` times more. Input, not output, carries the trajectory's cost, even though each output token is priced `{python} HarnessBudget.price_ratio_str` times higher.

**Budget used**: With caching, the trajectory has used `{python} HarnessBudget.rho_turns_str` of its turns, `{python} HarnessBudget.rho_tokens_str` of its tokens, `{python} HarnessBudget.rho_wall_str` of its wall-clock time, and `{python} HarnessBudget.rho_dollars_str` of its dollars, so the turn cap binds. Without caching, the dollar share jumps to `{python} HarnessBudget.rho_dollars_nocache_str` and dollars become the binding dimension.

**Systems insight**: The same trajectory sits comfortably inside its budget or close to its dollar ceiling depending on whether its prefix cache hits. A change that breaks the cache, such as reordering the tool list on every turn (@sec-vol3-context-engineering-staging), shows up first as a dollar ceiling approached early, which is one reason the budget meters dollars separately from tokens.
