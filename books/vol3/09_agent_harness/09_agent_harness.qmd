...
Consider an agent running a trajectory of $H = 50$ turns. Each turn requires five discrete state machine transitions:
...
While 3 percent CPU overhead appears manageable on paper, cache line evictions and TLB invalidations across thousands of thread working sets degrade L3 cache hit rates, increasing prefill prompt preparation latency. Furthermore, the maximum concurrency is strictly limited by the OS thread ceiling (`/proc/sys/kernel/threads-max`).
