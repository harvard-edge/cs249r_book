...
# │ How: Dual-panel plot: accuracy bar chart and evaluation cost bar chart.
# └─────────────────────────────────────────────────────────────────────────────
import sys
...
bars1 = ax1.bar(range(len(configs)), scores, color=bar_colors, edgecolor="#0F172A", linewidth=0.9, width=0.52, zorder=3)
ax1.set_ylim(0, 122)
ax1.set_xticks(range(len(configs)))
ax1.set_xticklabels(configs, fontsize=7.6, fontweight="bold")
ax1.set_ylabel("ARC-AGI Benchmark Accuracy (%)", fontsize=8.8, fontweight="bold")
ax1.set_title("Benchmark Solve Rate vs. Harness Architecture\n(ARC-AGI Interactive Reasoning)", fontsize=9.5, fontweight="bold", pad=8)
...
for bar, score in zip(bars1, scores):
    ...
    ax1.text(bar.get_x() + bar.get_width() / 2.0, yval + 2.5, score_txt, ha="center", va="bottom", fontsize=7.2, fontweight="bold")
...
ax1.text(1.0, -18, "Ephemeral While Loops", ha="center", fontsize=7.2, fontweight="bold", color="#64748B", clip_on=False)

ax1.plot([2.75, 5.25], [-14, -14], color="#7C3AED", linewidth=1.2, clip_on=False)
ax1.text(4.0, -18, "Stateful Agent Harnesses", ha="center", fontsize=7.2, fontweight="bold", color="#7C3AED", clip_on=False)

# Right: Evaluation Cost
bars2 = ax2.bar(range(len(configs)), costs_k, color=bar_colors, edgecolor="#0F172A", linewidth=0.9, width=0.52, zorder=3)
ax2.set_ylim(0, 62)
ax2.set_xticks(range(len(configs)))
ax2.set_xticklabels(configs, fontsize=7.6, fontweight="bold")
ax2.set_ylabel("Evaluation Dollar Cost ($k)", fontsize=8.8, fontweight="bold")
ax2.set_title("Total Evaluation Expenditure\n(Prefix Caching and State Compaction Savings)", fontsize=9.5, fontweight="bold", pad=8)
ax2.grid(axis="y", linestyle="--", linewidth=0.5, color="#CBD5E1", alpha=0.7, zorder=1)
ax2.spines["top"].set_visible(False)
ax2.spines["right"].set_visible(False)

for bar, cost in zip(bars2, costs_k):
    yval = bar.get_height()
    ax2.text(bar.get_x() + bar.get_width() / 2.0, yval + 1.2, f"${cost:.1f}k", ha="center", va="bottom", fontsize=7.2, fontweight="bold")
...
ax2.annotate(
    ...
    fontweight="bold",
    color="#166534",
    bbox=dict(boxstyle="round,pad=0.3", fc="#DCFCE7", ec="#BBF7D0", lw=0.8),
)

ax2.plot([-0.25, 2.25], [-7, -7], color="#64748B", linewidth=1.2, clip_on=False)
ax2.text(1.0, -9.0, "Ephemeral While Loops", ha="center", fontsize=7.2, fontweight="bold", color="#64748B", clip_on=False)

ax2.plot([2.75, 5.25], [-7, -7], color="#7C3AED", linewidth=1.2, clip_on=False)
ax2.text(4.0, -9.0, "Stateful Agent Harnesses", ha="center", fontsize=7.2, fontweight="bold", color="#7C3AED", clip_on=False)
