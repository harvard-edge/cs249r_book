...
Daly refines @eq-vol3-young-formula when $\delta$ is not small relative to $M$ [@daly2006]. Dividing $T^{\ast}$ by the mean turn time converts it into a cadence in turns, and pricing the lost turns in tokens shows what a wrong cadence costs.
...
A copy-on-write checkpoint costs little next to a turn that spends seconds in a model call, and preemptible workers have a short $M$, so the optimum is usually every few turns rather than every few hundred.
...
The model call is the source that surprises engineers most, because a served call is not reproducible even at temperature zero, where batch composition changes the order of floating-point sums and a near-tie between two tokens can flip (@sec-vol3-foundation-model-autoregressive). One changed token changes everything after it in the trajectory, and making serving bit-exact would slow it down and hold only on identical hardware and software. A runtime that wants reproducibility therefore records the model's outputs instead of trying to regenerate them.
...
The causal record never leaves the log, because replay depends on it. The bulk can be **offloaded**\index{Trajectory log!offloading}, moved out of the log into cheaper content-addressed storage while the log keeps a hash of the full output and a pointer to it. @Tbl-vol3-payload-compaction-schema shows one tool result before and after.

| **Field**             | **In the live log**                     | **After offloading**                                             |
|:----------------------|:----------------------------------------|:-----------------------------------------------------------------|
...
: **Offloading a Tool Result**: Offloading moves only the part of a tool result that the model never received. The delivered content and everything recovery or replay reads stay in the log. {#tbl-vol3-payload-compaction-schema tbl-colwidths="[20,36,44]"}

The rule that makes this safe is to offload only below the truncation line. A policy that summarized or trimmed the delivered content would leave replay showing an engineer an input the model never received, which is worse than no log at all.
