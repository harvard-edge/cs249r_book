...
To make these mechanics concrete, @lst-vol3-workingsets-streaming-buffer provides an operational implementation of the StreamingLLM KV cache manager, maintaining permanent initial sink tokens while rolling a contiguous local window buffer and evaluating attention without softmax denominator collapse.

::: {#lst-vol3-workingsets-streaming-buffer lst-cap="**Streaming Attention Sink and Rolling Local Window KV Cache Manager**: Python implementation of a fixed-capacity streaming KV cache buffer maintaining pinned attention sinks at offsets 0..3 alongside a circular rolling local window."}
```python
import numpy as np


class StreamingKVBuffer:
    """Fixed-capacity KV cache maintaining pinned attention sinks and a rolling window."""

    def __init__(self, num_sinks: int = 4, window_size: int = 1024, head_dim: int = 128):
        self.s, self.w, self.d = num_sinks, window_size, head_dim
        self.capacity = self.s + self.w
        self.k_buf = np.zeros((self.capacity, self.d), dtype=np.float32)
        self.v_buf = np.zeros((self.capacity, self.d), dtype=np.float32)
        self.total_tokens = 0

    def append(self, k: np.ndarray, v: np.ndarray) -> None:
        """Inserts token key/value vectors, pinning initial sinks and cycling rolling window."""
        if self.total_tokens < self.capacity:
            idx = self.total_tokens
        else:
            # Pinned sinks [0..s-1] remain untouched; overwrite circular window [s..s+w-1]
            idx = self.s + ((self.total_tokens - self.capacity) % self.w)
        self.k_buf[idx], self.v_buf[idx] = k, v
        self.total_tokens += 1

    def forward(self, q: np.ndarray) -> np.ndarray:
        """Computes scaled dot-product attention across resident sinks and local window."""
        length = min(self.total_tokens, self.capacity)
        scores = np.dot(q, self.k_buf[:length].T) / np.sqrt(self.d)
        weights = np.exp(scores - np.max(scores))
        weights /= np.sum(weights)
        return np.dot(weights, self.v_buf[:length])
```
:::
...
To demonstrate this end-to-end mechanism, @lst-vol3-workingsets-cot-cleaner implements an operational `TwoPhaseCommitCoTManager` paired with a simulated `RadixPrefixNode` allocator, verifying that scratchpad excision maintains full prefix cache hits while eliminating memory bloat.

::: {#lst-vol3-workingsets-cot-cleaner lst-cap="**Two-Phase Commit CoT Context Cleaner and Radix Prefix Reuser**: Python implementation of a transactional CoT manager excising ephemeral thought tokens post-sandbox execution and verifying Radix prefix cache hit preservation."}
```python
import re
from typing import Dict, Tuple


class TwoPhaseCommitCoTManager:
    """Manages transactional CoT execution: excises ephemeral thoughts and commits prefix."""

    def __init__(self, system_prompt: str):
        self.history = [("system", system_prompt)]
        self.ref_counts: Dict[str, int] = {"root": 1}

    def commit_turn(self, raw_response: str, execute_fn) -> Tuple[str, str, int]:
        """Phase 1: executes tool; Phase 2: excises thoughts and preserves radix prefix."""
        # 1. Parse ephemeral thought tokens and isolate canonical action
        match = re.search(r"<(?:thought|think)>(.*?)</(?:thought|think)>", raw_response, re.DOTALL)
        thought = match.group(1).strip() if match else ""
        action = re.sub(
            r"<(?:thought|think)>.*?</(?:thought|think)>", "", raw_response, flags=re.DOTALL
        ).strip()

        # 2. Phase 1 execution: run tool in isolated sandbox
        observation = execute_fn(action)

        # 3. Phase 2 commit: release scratchpad cache branch (ref_count -> 0)
        # Scratchpad KV pages are reclaimed by LRU; only canonical action/obs commit to prefix
        self.ref_counts["scratchpad"] = 0  # Marked unreferenced for GPU allocator reclaim
        self.history.append(("assistant", action))
        self.history.append(("tool", observation))
        self.ref_counts[f"turn_{len(self.history)}"] = 1

        pruned_tokens = len(thought) // 4
        return action, observation, pruned_tokens
```
:::
