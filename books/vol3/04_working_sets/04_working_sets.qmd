...
@Algo-streaming-attention-sink formalizes the buffer management policy of StreamingLLM. The manager divides a fixed memory buffer into two distinct regions: a pinned region holding the initial $S$ attention sink tokens, and a cyclic ring buffer holding the most recent $W$ local tokens. On each generation step, the algorithm writes incoming key-value vectors to their target physical slots without shifting historical memory, computing attention across the stabilized denominator.
...
1. **Dual-Zone Physical Buffer Layout (Lines 1–4):** The algorithm partitions a contiguous pre-allocated buffer of size $S + W$ in accelerator HBM into two functional regions:
...
@Algo-twophase-context-cleaner details the transactional lifecycle of reasoning tokens during agent deliberation. In Phase 1 (Proposal), the model decodes an ephemeral scratchpad $\mathbf{z}_t$ alongside a concrete tool invocation $a_t$, branching the resident Radix prefix tree. In Phase 2 (Commit), once the sandbox executes $a_t$ and yields observation $o_t$, the runtime excises $\mathbf{z}_t$, appends only $(a_t, o_t)$ to the persistent trajectory, and decrements reference counts on the scratchpad cache pages to trigger background memory reclamation.
