...
$$
T_{\text{prefill}}(N) \approx \frac{F_{\text{prefill}}(N)}{P_{\text{peak}} \cdot \eta_{\text{compute}}},
$$

where $P_{\text{peak}}$ is the peak dense tensor throughput of the accelerator (e.g., $989 \times 10^{12}$ FLOPs/s in 16-bit precision for an NVIDIA H100 SXM5) and $\eta_{\text{compute}} \in [0.45, 0.65]$ is the realized hardware compute efficiency. For an agent ingesting a 128k-token trajectory on a 70B model ($L = 80, d_{\text{model}} = 8{,}192, d_{\text{ff}} = 28{,}672$), @eq-autoregressive-prefill-flops evaluates to approximately $1.15 \times 10^{17}$ FLOPs. At 50 percent realized efficiency, prefill alone requires 38.8 seconds of sustained GPU compute before the agent can emit its first reasoning token.

```
Quadratic Prefill vs. Linear Decode Regimes:
┌────────────────────────────────────────────────────────────────────────┐
│ PREFILL REGIME (Prompt Ingestion)                                      │
│ • Compute-Bound: Arithmetic Intensity >> 100 FLOPs/Byte                │
│ • Scaled by Matrix-Matrix Tensor Cores (GEMM)                          │
│ • Complexity: O(N^2) FLOPs, TTFT grows from 0.4s to 40s+               │
└────────────────────────────────────────────────────────────────────────┘
                                 │
                                 ▼ Token-by-Token Handoff
┌────────────────────────────────────────────────────────────────────────┐
│ DECODE REGIME (Token Generation)                                       │
│ • Memory-Bandwidth Bound: Arithmetic Intensity ≈ 1-2 FLOPs/Byte        │
│ • Constrained by HBM Streaming Bus (GEMV)                              │
│ • KV Cache footprint scales as O(N), reading entire cache every token  │
└────────────────────────────────────────────────────────────────────────┘
```
...
Consider a representative frontier agent policy: Llama-3-70B, which uses Grouped-Query Attention (GQA, @ainslie2023gqa) with $L = 80$, $H_Q = 64$, $H_{\text{KV}} = 8$, $d_h = 128$, and BF16 precision ($b_{\text{elem}} = 2$). Evaluating @eq-kv-cache-footprint yields:

$$
M_{\text{KV, token}} = 2 \times 80 \times 8 \times 128 \times 2 = 327{,}680\text{ bytes} \approx 320\text{KB/token}.
$$

At a context length of $N = 131{,}072$ tokens (128k), the KV cache consumes:

$$
M_{\text{KV}}(131{,}072) = 131{,}072 \times 327{,}680\text{ bytes} \approx 42.95\text{ GB}.
$$

Because the model weights in 16-bit precision occupy approximately 140 GB, serving this model requires at least two 80 GB GPUs (160 GB total HBM). A single 128k agent stream consumes over 53 percent of the remaining free memory across both GPUs. If four concurrent agent tasks are executed in parallel across the node, aggregate KV cache demand reaches $171.8\text{ GB}$, exceeding the physical capacity of the accelerators and triggering catastrophic out-of-memory aborts (@kwon2023vllm).
...
Generating a single token requires reading the entire model weight tensor $\mathbf{W}$ and the entire resident KV cache tensor $\mathcal{K}, \mathcal{V}$ from accelerator HBM into on-chip SRAM registers:

$$
B_{\text{decode}}(N) = M_{\text{weights}} + M_{\text{KV}}(N).
$$

The computation performed during this decode step consists of matrix-vector multiplications across all layers:

$$
F_{\text{decode}}(N) \approx 2 P + 4 L H_Q d_h N,
$$

where $P$ is the total active parameter count of the model ($P \approx 70 \times 10^9$). The arithmetic intensity $\mathcal{I}_{\text{decode}}$ of this operation—defined as the ratio of floating-point operations executed to bytes transferred across the memory bus—is:

$$
\mathcal{I}_{\text{decode}}(N) = \frac{F_{\text{decode}}(N)}{B_{\text{decode}}(N)} = \frac{2 P + 4 L H_Q d_h N}{M_{\text{weights}} + 2 L H_{\text{KV}} d_h N b_{\text{elem}}}.
$$ {#eq-decode-arithmetic-intensity}

For a batch size of 1, because $P \gg 2 L H_Q d_h N$ for all practical context lengths ($N \le 10^6$), @eq-decode-arithmetic-intensity collapses to:

$$
\mathcal{I}_{\text{decode}} \approx \frac{2 P}{P \cdot b_{\text{elem}} + M_{\text{KV}}(N)} \approx \frac{2}{b_{\text{elem}}} \approx 1\text{ FLOP/Byte}.
$$
...
The time required to generate a single token $T_{\text{decode}}$ as a function of sequence length $N$ is governed by the memory bus streaming latency:

$$
T_{\text{decode}}(N) = \frac{M_{\text{weights}} + M_{\text{KV}}(N)}{B_{\text{mem}}}.
$$ {#eq-decode-latency-bandwidth}

As an agent trajectory accumulates tokens, $M_{\text{KV}}(N)$ expands monotonically. When $M_{\text{KV}}(N)$ approaches or exceeds $M_{\text{weights}}$, the time required to emit each individual token doubles, halving interactive token generation velocity:

$$
\text{Throughput}(N) = \frac{1}{T_{\text{decode}}(N)} = \frac{B_{\text{mem}}}{M_{\text{weights}} + 2 L H_{\text{KV}} d_h N b_{\text{elem}}}.
$$

In multi-tenant or concurrent agent environments, this memory bandwidth wall creates severe performance degradation: every additional unpruned token in an agent's history directly steals memory bus cycles from all co-scheduled inference streams.
...
Denning formalized a process's working set $W(t, \Delta t)$ at virtual time $t$ with window parameter $\Delta t$ as the collection of memory pages referenced during the process-time interval $(t - \Delta t, t]$:

$$
W(t, \Delta t) = \{ r_\tau \in S \mid t - \Delta t < \tau \le t \},
$$

where $S$ is the virtual address space and $r_\tau$ is the page reference at step $\tau$. If the physical memory allocated to the process is smaller than $|W(t, \Delta t)|$, the operating system encounters **thrashing**: physical page frames are continuously evicted and re-faulted from backing disk, causing system throughput to collapse toward zero while CPU utilization is consumed entirely by I/O wait.

```
Classical OS Virtual Memory vs. Neural Agent Working Sets:
┌────────────────────────────────────────┬────────────────────────────────────────┐
│ CLASSICAL VIRTUAL MEMORY (OS)          │ NEURAL AGENT CONTEXT (ML SYSTEMS)      │
├────────────────────────────────────────┼────────────────────────────────────────┤
│ Virtual Address Space S                │ Complete Trajectory History τ          │
│ Physical RAM (Resident Set Size)       │ Accelerator HBM Allocation (KV Cache)  │
│ Page References (Instructions/Data)    │ Query-to-Key Attention Weights         │
│ Working Set W(t, Δt)                   │ Active Attention Working Set W_ε(t)    │
│ Page Fault to Disk                     │ Context Recall / External Retrieval    │
│ Thrashing (Disk Swapping Paralysis)    │ Neural Thrashing (Dispersion / OOM)    │
└────────────────────────────────────────┴────────────────────────────────────────┘
```
...
In a neural context, "memory access" is defined by the self-attention mechanism. At step $t$, the query vector $\mathbf{q}_t$ of the current token computes an attention distribution across all preceding tokens $i \in \{1, \ldots, t\}$. We quantify the effective attention mass allocated to token $i$ across all $L$ layers and $H$ attention heads:

$$
\bar{\alpha}_i^{(t)} = \frac{1}{L \cdot H} \sum_{\ell=1}^L \sum_{h=1}^H \alpha_{\ell, h, t, i},
$$

where $\sum_{i=1}^t \bar{\alpha}_i^{(t)} = 1$. We define the **Neural Token Working Set** $W_\epsilon(t)$ as the minimal subset of historical tokens whose collective attention mass exceeds a coverage threshold $1 - \epsilon$:

$$
W_\epsilon(t) = \left\{ i \in \{1, \ldots, t\} \;\middle|\; i \in \arg\min_{\mathcal{S} \subseteq \{1, \dots, t\}} |\mathcal{S}| \quad \text{s.t.} \quad \sum_{j \in \mathcal{S}} \bar{\alpha}_j^{(t)} \ge 1 - \epsilon \right\},
$$

where $\epsilon \in (0, 0.1]$ is the information exclusion tolerance (typically $\epsilon = 0.05$).
...
As visualized in @fig-vol3-context-working-set-dynamics, treating context as an unmanaged append-only log forces the accelerator to retain 100 percent of historical tokens in HBM, even though the neural working set encompasses only a tiny fraction of the sequence. When the cumulative trajectory outgrows physical cache limits, the system encounters **Neural Thrashing**:

$$
|W_\epsilon(t)| > C_{\text{HBM}} \implies \text{Attention Dispersion and Cache Saturation}.
$$

In classical operating systems, thrashing wastes compute cycles moving pages across the slow PCIe/storage bus. In agentic neural systems, thrashing manifests as a dual failure:
...
To evaluate how effectively an attention head utilizes its context window, we formulate the **Attention Density** and **Attention Entropy** of query token $t$ across the sequence $i \in \{1, \ldots, t\}$. For head $h$ at layer $\ell$, the attention distribution $\boldsymbol{\alpha}_{t}^{(\ell, h)} \in \mathbb{R}^t$ forms a probability simplex:

$$
\sum_{i=1}^t \alpha_{t, i}^{(\ell, h)} = 1, \quad \alpha_{t, i}^{(\ell, h)} \ge 0.
$$

We quantify the dispersion of attention mass using the Shannon entropy of this distribution:

$$
H\left( \boldsymbol{\alpha}_t^{(\ell, h)} \right) = -\sum_{i=1}^t \alpha_{t, i}^{(\ell, h)} \ln \alpha_{t, i}^{(\ell, h)}.
$$

The maximum possible entropy occurs when attention is uniformly distributed across the entire sequence:

$$
H_{\max}(t) = \ln t.
$$

We define the **Effective Context Utilization** $U_{\text{eff}}(t)$ of an attention layer as the normalized inverse entropy ratio:

$$
U_{\text{eff}}(t) = 1 - \frac{1}{H_{\max}(t)} \cdot \left[ \frac{1}{H} \sum_{h=1}^H H\left( \boldsymbol{\alpha}_t^{(\ell, h)} \right) \right].
$$

When $U_{\text{eff}}(t) \to 0$, the attention distribution approaches a uniform distribution ($H \to \ln t$). The attention mechanism ceases to discriminate between relevant and irrelevant tokens, acting as an unweighted average filter that dilutes specialized representations into semantic noise. Conversely, when $U_{\text{eff}}(t)$ remains bounded away from zero, the attention head concentrates its probability mass on a sparse, informative subset of the context.

As sequence length $t$ increases from $1{,}000$ to $128{,}000$ tokens in unmanaged contexts, empirical measurements show that intermediate attention layers exhibit a monotonic drop in $U_{\text{eff}}(t)$. The denominator of the softmax function ($\sum_{j=1}^t \exp(z_{t, j})$) accumulates thousands of small positive exponentials from irrelevant tokens. Even when individual background tokens receive tiny dot-product values ($z_{t, j} \ll z_{t, \text{target}}$), the aggregate mass of $100{,}000$ background tokens can overwhelm the target token, causing the target's relative probability $\alpha_{t, \text{target}}$ to collapse below the threshold required to activate downstream feed-forward circuits.
...
```
Retrieval Accuracy vs. Token Position Depth (The U-Curve):
100% ────┐                                                 ┌────
         │                                                 │
 75%     │                                                 │
         │                                                 │
 ...
 25%        \───────────────────────────────────────────/
  0% ┌───────────┬───────────┬───────────┬───────────┬───────────┐
     0% (Start) 20%         40%         60%         80%  100% (End)
               Relative Position Depth in Context Window
```
...
To eliminate this bloat, agent runtimes implement a **Two-Phase Commit for Reasoning Context**:

```
TWO-PHASE COMMIT FOR EPHEMERAL SCRATCHPADS:
Phase 1: Deliberation (Transient Allocation in Ephemeral Buffer)
┌───────────────────────────┬───────────────────────────────┬───────────────────────────────┐
│ System Prompts & History  │ Active Action Proposal        │ Ephemeral CoT Reasoning       │
│ (Pinned in Active HBM)    │ a_t ~ π_θ(· | s_t)            │ z_t ∈ B_scratchpad (Transient)│
└───────────────────────────┴───────────────────────────────┴───────────────────────────────┘
                                           │
                                           ▼ Action Execution & Verification in Sandbox
Phase 2: Commit (State Mutation & Scratchpad Eviction)
┌───────────────────────────┬───────────────────────────────┬───────────────────────────────┐
│ System Prompts & History  │ Verified Action Committed     │ Environment Observation       │
│ (Pinned in Active HBM)    │ a_t (Canonical Patch)         │ o_t (Test Suite Output)       │
└───────────────────────────┴───────────────────────────────┴───────────────────────────────┘
  [ Ephemeral CoT Reasoning z_t Purged from Persistent KV Cache — 85% Token Reduction ]
```
...
```
Accelerator Roofline: Decode Latency vs. KV Cache Compression:
Achieved TFLOP/s
  ▲
  │                       COMPUTE CEILING (P_peak)
  │                   ┌────────────────────────────────────────
  │                  /
  │                 /  MLA / Pruned Attention Regime:
  │                /   • Higher arithmetic intensity (I ≈ 3.6 FLOP/B)
  │               /    • 4x to 8x higher concurrent streams before bus saturation
  │              /
  │             /  Standard MHA / Unpruned GQA Regime:
  │            /   • Low arithmetic intensity (I ≈ 0.8 to 2.4 FLOP/B)
...
Because autoregressive decoding operates in the memory-bandwidth-bound regime, the right-hand term in @eq-roofline-serving dominates. Compressing the KV cache via GQA, MLA, or structured head pruning directly reduces $M_{\text{KV}}(N)$:
...
| **Long-Range Association**   | $\langle \mathbf{q}_m, \mathbf{k}_n \rangle \propto \frac{1}{                                                  | m-n                                                     | ^\gamma}$ (@eq-rope-decay)                    | RoPE rotational phase destructive decay                                | "Lost-in-the-Middle" cognitive degradation | Heavy-Hitter retention (H2O); AST code pruning |
...
1. **KV Cache Memory Sizing and Autoregressive Decode Bandwidth Wall:**
   An autonomous software engineering agent runs on a dual-NVIDIA H100 SXM5 node. Each GPU provides 80 GB of HBM3 memory with a measured memory bandwidth of $B_{\text{mem}} = 3.35\text{ TB/s}$ (aggregate $6.70\text{ TB/s}$ across both GPUs). The underlying agent policy is a 70-billion parameter dense model using Grouped-Query Attention (GQA) with the following architectural specifications: $L = 80$ layers, $d_{\text{model}} = 8{,}192$, query heads $H_Q = 64$, key/value heads $H_{\text{KV}} = 8$, head dimension $d_h = 128$, and BF16 precision ($b_{\text{elem}} = 2\text{ bytes}$). The model weights occupy exactly $140\text{ GB}$ of HBM distributed equally across the two GPUs ($70\text{ GB}$ per GPU). The runtime reserves $4\text{ GB}$ per GPU for activation scratchpads and CUDA runtime overhead.
   a) Calculate the KV cache memory footprint per token ($M_{\text{KV, token}}$) in bytes.
   b) Calculate the maximum theoretical sequence length $N_{\max}$ that a single agent stream can sustain before the system encounters an Out-Of-Memory (OOM) fault.
   c) Assuming a current trajectory length of $N = 65{,}536$ tokens, calculate the total memory bytes transferred across the memory bus to decode a single token ($B_{\text{decode}}$), and determine the maximum theoretical autoregressive token generation rate ($\text{tokens/sec}$) bounded by memory bandwidth.
   d) If the orchestrator schedules $B = 4$ concurrent agent streams, each at $N = 65{,}536$ tokens, calculate the new per-token generation rate per stream. Does the aggregate memory footprint fit within the dual-GPU node?

2. **Attention Sink Probability Dynamics and Softmax Denominator Rescaling:**
   An inference engine serves an agent with an active context of $N = 2{,}048$ tokens. For a specific middle-layer attention head $\ell = 40, h = 4$, the pre-softmax attention logit vector is $\mathbf{z} \in \mathbb{R}^{2048}$. Empirical profiling reveals that the first four tokens ($i \in \{0, 1, 2, 3\}$, corresponding to initial attention sinks) have exceptionally large logits: $z_0 = 18.2, z_1 = 17.5, z_2 = 17.1, z_3 = 16.8$. The remaining $2{,}044$ tokens have logits normally distributed with mean $\mu = 4.2$ and standard deviation $\sigma = 1.1$.
   a) Compute the exact softmax denominator $D_{\text{full}} = \sum_{j=0}^{2047} \exp(z_j)$ using the approximation $\sum_{j=4}^{2047} \exp(z_j) \approx 2044 \cdot \exp\left(\mu + \frac{\sigma^2}{2}\right)$.
   b) Calculate the aggregate percentage of probability mass absorbed by the 4 attention sink tokens ($P_{\text{sink}} = \sum_{j=0}^3 \alpha_j$).
   c) Suppose a naive sliding-window eviction policy discards the 4 sink tokens, retaining only the remaining $2{,}044$ tokens. Calculate the new softmax denominator $D_{\text{evicted}}$ and determine the amplification factor $K = D_{\text{full}} / D_{\text{evicted}}$ by which the attention weights of all remaining intermediate tokens are artificially inflated. Explain the impact of this rescaling on the activation variance in downstream residual layers.

3. **Multi-Head Latent Attention (MLA) vs. GQA Memory and Concurrency Sizing:**
   A high-throughput enterprise agent cluster is evaluating an architectural migration from a standard GQA model to a Multi-Head Latent Attention (MLA) model for its 60-layer reasoning policy.
   - **Configuration A (GQA):** $L = 60$, $H_Q = 64$, $H_{\text{KV}} = 8$, $d_h = 128$, $b_{\text{elem}} = 2\text{ bytes}$ (BF16).
   - **Configuration B (MLA):** $L = 60$, latent compression dimension $d_c = 512$, decoupled rotary head dimension $d_h^R = 64$, $b_{\text{elem}} = 2\text{ bytes}$ (BF16).
   Both models have identical model weight footprints of $60\text{ GB}$ in FP16 and are deployed on single 80 GB NVIDIA H100 GPUs (leaving $16\text{ GB}$ available strictly for KV cache storage after reserving $4\text{ GB}$ for workspace buffers).
   a) Calculate the KV cache memory requirement per token (in KB/token) for both Configuration A and Configuration B. What is the exact compression ratio achieved by MLA?
   b) For an agentic coding benchmark where trajectories average $N = 32{,}768$ tokens, calculate the maximum number of concurrent agent streams ($B_{\max}$) that can be served on a single 80 GB GPU without exceeding the $16\text{ GB}$ KV cache budget for both configurations.
   c) For Configuration B (MLA) operating at its maximum concurrency $B_{\max}$ derived in part (b), calculate the total bytes transferred across the memory bus per decode step ($B_{\text{decode}}$), and compute the sustained aggregate generation throughput ($\text{tokens/sec}$) assuming a memory bandwidth of $B_{\text{mem}} = 3.35\text{ TB/s}$.
