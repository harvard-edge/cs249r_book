...
**First-principles streaming latency napkin math:** For Llama-3-70B in 16-bit precision, model weights occupy $M_{\text{weights}} \approx 140\text{ GB}$. On an H100 SXM5 GPU ($B_{\text{mem}} = 3{,}350\text{ GB/s}$):

- **Short context ($N \approx 0, M_{\text{KV}} \approx 0$):**
  $$T_{\text{decode}} \approx \frac{140\text{ GB}}{3{,}350\text{ GB/s}} \approx 41.8\text{ ms/token} \implies \text{Throughput} \approx 23.9\text{ tokens/s}.$$
- **128k context ($M_{\text{KV}} \approx 43\text{ GB}$):** Total data streamed per token is $140 + 43 = 183\text{ GB}$.
  $$T_{\text{decode}} \approx \frac{183\text{ GB}}{3{,}350\text{ GB/s}} \approx 54.6\text{ ms/token} \implies \text{Throughput} \approx 18.3\text{ tokens/s} \quad (\text{23\% slowdown}).$$
- **500k context ($M_{\text{KV}} \approx 168\text{ GB} > M_{\text{weights}}$):** Total data streamed is $140 + 168 = 308\text{ GB}$.
  $$T_{\text{decode}} \approx \frac{308\text{ GB}}{3{,}350\text{ GB/s}} \approx 91.9\text{ ms/token} \implies \text{Throughput} \approx 10.9\text{ tokens/s} \quad (\text{54\% slowdown}).$$
...
When an agent operates in a real-world multi-turn setting, the middle region is precisely where earlier tool outputs, intermediate file contents, and environmental observations reside, saturating accelerator HBM while degrading retrieval velocity.
...
MLA delivers a **$4.75\times$ concurrency multiplier** on identical physical GPU hardware, directly slashing the infrastructure cost per agent stream.
