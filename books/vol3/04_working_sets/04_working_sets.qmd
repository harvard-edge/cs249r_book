...
[^fn-workingsets-dpi-markov]: Because each summarization step $\hat{S}_k \sim P(\cdot \mid \hat{S}_{k-1})$ is conditionally independent of $X$ given $\hat{S}_{k-1}$, the Markov property $X \to \hat{S}_{k-1} \to \hat{S}_k$ strictly bounds information retention.
...
Compared to standard MHA ($2.56\text{ MB/token}$), MLA slashes resident KV cache memory footprint by **$38\times$**, and compared to an $8\times$ GQA architecture ($320\text{ KB/token}$), MLA slashes memory demand by **$4.74\times$**. During generation, key and value states are dynamically projected out of the latent vector on-chip within SRAM registers ($\mathbf{K}_t = \mathbf{c}_t^{KV} \mathbf{W}_{UK}, \mathbf{V}_t = \mathbf{c}_t^{KV} \mathbf{W}_{UV}$).
...
2. **Concurrency Expansion:** The maximum concurrent agent batch size $B_{\max} = \lfloor (C_{\text{HBM}} - M_{\text{weights}}) / M_{\text{KV}}(N) \rfloor$ supported within physical memory capacity $C_{\text{HBM}}$ scales inversely with per-token footprint. On an 80 GB accelerator serving a 128k context, transitioning from GQA to MLA increases maximum concurrent agent trajectories from 3 to 9 streams per node—a $3\times$ expansion in cluster density.
