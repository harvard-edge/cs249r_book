...
Part I established the foundation model as a stochastic processor core and demonstrated that complex problem solving requires test-time deliberation. However, as an agent deliberates across dozens of turns—executing tools, reading diagnostics, and evaluating hypotheses—it accumulates thousands of historical tokens. Modern models boast context windows reaching millions of tokens, tempting designers to append every intermediate trace into an ever-growing prompt buffer. In production runtimes, this append-only design collides violently with silicon limits: prefill compute scales quadratically ($O(C^2)$), KV cache memory expands linearly in expensive GPU HBM, and autoregressive decoding degrades under memory-bandwidth saturation. Furthermore, long contexts dilute attention entropy, causing the model to lose track of early system constraints. To prevent context thrashing, the runtime must abandon the illusion of infinite memory and govern active context as an operating system governs RAM: treating the context window not as an archival dump, but as an actively budgeted, eviction-driven L1 attention working set. This chapter formalizes the working set model for large language model (LLM) contexts, deriving the physical memory wall of KV caching and developing loss-bounded semantic compaction algorithms.
