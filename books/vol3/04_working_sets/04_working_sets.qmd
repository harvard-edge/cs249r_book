...
Part I established the foundation model as a stochastic processor core and demonstrated that complex problem solving requires test-time deliberation. However, as an agent deliberates across dozens of turns—executing tools, reading diagnostics, and evaluating hypotheses—it accumulates thousands of historical tokens. Modern models boast context windows reaching millions of tokens, tempting designers to append every intermediate trace into an ever-growing prompt buffer. In production runtimes, this append-only design collides violently with silicon limits: prefill compute scales quadratically ($O(C^2)$), Key-Value (KV) cache memory expands linearly in expensive GPU High-Bandwidth Memory (HBM), and autoregressive decoding degrades under memory-bandwidth saturation. Furthermore, long contexts dilute attention entropy, causing the model to lose track of early system constraints. To prevent context thrashing, the runtime must abandon the illusion of infinite memory and govern active context as an operating system governs RAM: treating the context window not as an archival dump, but as an actively budgeted, eviction-driven L1 attention working set. This chapter formalizes the working set model for LLM contexts, deriving the physical memory wall of KV caching and developing loss-bounded semantic compaction algorithms.
...
class AttentionSinkConfig:
    """Hardware buffer parameters for streaming attention with pinned sinks."""
    num_sink_tokens: int = 4        # S: Pinned initial tokens (positions 0..3)
    window_size: int = 1024         # W: Local sliding window capacity
    head_dim: int = 128             # d_h: Per-head activation dimension
    num_kv_heads: int = 8           # H_KV: Grouped-Query Attention heads
    dtype: np.dtype = np.float32    # Numerically stable evaluation dtype


class StreamingKVBuffer:
    """Fixed-capacity KV cache manager implementing StreamingLLM buffer eviction."""

    def __init__(self, config: AttentionSinkConfig):
        self.cfg = config
        self.max_capacity = config.num_sink_tokens + config.window_size
        self.total_tokens_seen = 0

        # Pre-allocate contiguous physical memory buffer: (H_KV, S + W, d_h)
        buf_shape = (config.num_kv_heads, self.max_capacity, config.head_dim)
        self.key_buffer = np.zeros(buf_shape, dtype=config.dtype)
        self.val_buffer = np.zeros(buf_shape, dtype=config.dtype)

    @property
    def current_length(self) -> int:
        """Returns the number of currently resident active tokens."""
        return min(self.total_tokens_seen, self.max_capacity)

    def append_token(self, k_t: np.ndarray, v_t: np.ndarray):
        """Inserts a newly decoded token Key and Value vector into the resident cache."""
        s = self.cfg.num_sink_tokens
        w = self.cfg.window_size
        if self.total_tokens_seen < self.max_capacity:
            # Phase A: Buffer is filling up monotonically [0 : s + w]
            idx = self.total_tokens_seen
            self.key_buffer[:, idx, :] = k_t
            self.val_buffer[:, idx, :] = v_t
        else:
            # Phase B: Cache saturation reached; write to circular rolling window in O(1)
            # Sinks [0 : s] remain permanently pinned in physical memory slots!
            # Window tokens are overwritten via modulo ring pointer without memory copies
            window_offset = (self.total_tokens_seen - self.max_capacity) % w
            target_idx = s + window_offset
            self.key_buffer[:, target_idx, :] = k_t
            self.val_buffer[:, target_idx, :] = v_t
    ...
    def compute_attention(
        ...
        """Computes scaled dot-product attention against resident sink and window tokens."""
        s = self.cfg.num_sink_tokens
        cur_len = self.current_length

        # Slice active resident keys and values
        keys = self.key_buffer[:, :cur_len, :]      # (H_KV, cur_len, d_h)
        values = self.val_buffer[:, :cur_len, :]    # (H_KV, cur_len, d_h)

        # Scaled dot-product: (1, d_h) x (cur_len, d_h)^T -> (1, cur_len)
        scale = 1.0 / np.sqrt(self.cfg.head_dim)
        scores = np.einsum("hd,hkd->hk", q_t, keys) * scale

        # Numerically stable softmax
        scores_max = np.max(scores, axis=-1, keepdims=True)
        exp_scores = np.exp(scores - scores_max)
        denom = np.sum(exp_scores, axis=-1, keepdims=True)
        attn_weights = exp_scores / (denom + 1e-12)

        # Context activation output
        out = np.einsum("hk,hkd->hd", attn_weights, values)

        # Quantify sink absorption ratio: percentage of mass dumped on positions 0..S-1
        sink_mass = float(np.sum(attn_weights[:, :s]) / self.cfg.num_kv_heads)
        return out, sink_mass
```
:::
...
class TrajectoryTurn:
    """Represents a discrete turn in the persistent agent trajectory."""
    turn_id: int
    role: str                       # 'user', 'assistant', or 'tool'
    content: str                    # Canonical text committed to persistent context
    is_ephemeral: bool = False      # If True, subject to Phase 2 purge
    token_count: int = 0


class RadixPrefixNode:
    """Simulated physical KV page tree node in an LLM serving runtime."""

    def __init__(self, prefix: str, ref_count: int = 1):
        self.prefix = prefix
        self.ref_count = ref_count
        self.children: Dict[str, "RadixPrefixNode"] = {}
        self.is_evicted = False

    def release(self):
        """Decrements reference counter; frees pages if zero."""
        self.ref_count -= 1
        if self.ref_count <= 0:
...
class TwoPhaseCommitCoTManager:
    """Orchestrates Two-Phase Commit for ephemeral Chain-of-Thought scratchpads."""

    def __init__(self, system_prompt: str):
        self.system_prompt = system_prompt
        self.radix_root = RadixPrefixNode(prefix=system_prompt, ref_count=1)
        self.active_prefix_node = self.radix_root
        ...
        self.total_tokens_committed = 0

    def get_prompt_for_generation(self) -> str:
        """Constructs the canonical prompt string for the next deliberation forward pass."""
        return "\n".join(f"[{turn.role.upper()}]: {turn.content}" for turn in self.history)

    def commit_turn(
        self,
        raw_llm_response: str,
        sandbox_executor: Callable[[str], str],
    ) -> Tuple[str, int, int]:
        """Phase 2 Commit: Excises thought tokens, executes tool, and commits canonical state."""
        turn_id = self.current_turn
        raw_token_count = len(raw_llm_response) // 4
        self.total_tokens_generated += raw_token_count

        # Step 1: Parse ephemeral thought scratchpad (supports <thought> and <think>)
        thought_match = re.search(r"<(?:thought|think)>(.*?)</(?:thought|think)>", raw_llm_response, re.DOTALL)
        thought_content = thought_match.group(1).strip() if thought_match else ""
        thought_tokens = len(thought_content) // 4

        # Simulate ephemeral branch in serving engine's Radix tree during Phase 1
        ephemeral_scratchpad_node = RadixPrefixNode(prefix=thought_content, ref_count=1)
        self.active_prefix_node.children["thought_scratchpad"] = ephemeral_scratchpad_node

        # Strip thought block to isolate canonical tool action
        action_content = re.sub(r"<(?:thought|think)>.*?</(?:thought|think)>", "", raw_llm_response, flags=re.DOTALL).strip()
        action_tokens = len(action_content) // 4

        # Step 2: Execute tool action inside isolated container sandbox
        tool_observation = sandbox_executor(action_content)
        obs_tokens = len(tool_observation) // 4

        # Step 3: Phase 2 Commit - Forcefully evict ephemeral scratchpad branch
        # In the Radix tree, omitting the scratchpad decrements ref_count to 0
        ephemeral_scratchpad_node.release()

        # Commit canonical transition tuple to persistent Radix branch and history
        canonical_node = RadixPrefixNode(prefix=f"{action_content}\n{tool_observation}", ref_count=1)
        self.active_prefix_node.children[f"turn_{turn_id}"] = canonical_node
        self.active_prefix_node = canonical_node

        self.history.append(
            TrajectoryTurn(turn_id=turn_id, role="assistant", content=action_content, token_count=action_tokens)
        )
        self.history.append(
            TrajectoryTurn(turn_id=turn_id, role="tool", content=tool_observation, token_count=obs_tokens)
        )

        committed_tokens = action_tokens + obs_tokens
        self.total_tokens_committed += committed_tokens
        self.current_turn += 1

        pruned_tokens = thought_tokens
        return action_content, pruned_tokens, committed_tokens
```
:::
