# Working Context {#sec-vol3-working-sets}
...
```{python}
#| label: lego-04-prefill-compute-scaling-and-latency
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ PREFILL COMPUTE SCALING AND LATENCY ON ACCELERATOR HARDWARE (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-04-prefill-compute-scaling-and-latency
# │ Goal: Quantify prefill FLOPs, quadratic attention fraction, and TTFT across context sizes.
# │ Show: 8x context expansion increases FLOPs by 22x; attention shifts from 25% to 73% of compute.
# │ How: Linear projections scale as 24*M*L*d^2; self-attention scales quadratically as 4*M^2*L*d.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_multiple

class PrefillComputeScaling:
    """Prefill compute scaling and latency on accelerator hardware."""
    ...
    m1 = 8_192
    m2 = 65_536
    p_peak = 989 * (TFLOP / second)
    mfu = 0.50
    n_turns = 30

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    expansion_ratio = int(m2 / m1)
    p_effective = mfu * p_peak

    # Policy A
    f_linear_a = 24 * m1 * n_layers * (d_model**2)
    f_attention_a = 4 * (m1**2) * n_layers * d_model
    f_prefill_a = f_linear_a + f_attention_a
    pct_attn_a = f_attention_a / f_prefill_a
    ttft_a = (f_prefill_a * flop / p_effective).to(second)

    # Policy B
    f_linear_b = 24 * m2 * n_layers * (d_model**2)
    f_attention_b = 4 * (m2**2) * n_layers * d_model
    f_prefill_b = f_linear_b + f_attention_b
    pct_attn_b = f_attention_b / f_prefill_b
    ttft_b = (f_prefill_b * flop / p_effective).to(second)

    compute_factor = round(f_prefill_b / f_prefill_a)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(expansion_ratio == 8, "expansion_ratio")
    check(abs(f_linear_a - 1.055e14) < 1e12, "f_linear_a")
    check(abs(f_attention_a - 3.518e13) < 1e11, "f_attention_a")
    check(abs(pct_attn_a - 0.250) < 0.005, "pct_attn_a")
    check(abs(ttft_a.to(millisecond).magnitude - 285) < 2, "ttft_a")
    check(abs(pct_attn_b - 0.727) < 0.005, "pct_attn_b")
    check(abs(ttft_b.magnitude - 6.26) < 0.05, "ttft_b")
    check(compute_factor == 22, "compute_factor")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    m1_str = fmt_int(m1)
    m2_str = fmt_int(m2)
    expansion_ratio_mult_str = fmt_multiple(expansion_ratio, precision=0)
    expansion_ratio_int_str = fmt_int(expansion_ratio)
    compute_factor_int_str = fmt_int(compute_factor)
    pct_attn_a_str = fmt_percent(pct_attn_a, precision=0, style="prose")
    pct_attn_b_str = fmt_percent(round(pct_attn_b * 100) / 100, precision=0, style="prose")
...
**Problem:** An autonomous software agent is evaluated under two different working context staging policies using an 8-billion parameter foundation model ($L = 32$ layers, hidden dimension $d = 4096$). Policy A enforces strict context compaction, staging an average working context of $M_1 =$ `{python} PrefillComputeScaling.m1_str` tokens. Policy B dumps raw compiler outputs and uncurated directory traces, staging an uncompacted context of $M_2 =$ `{python} PrefillComputeScaling.m2_str` tokens (`{python} PrefillComputeScaling.expansion_ratio_mult_str` expansion).
...
**1. Evaluation of Policy A ($M_1 =$ `{python} PrefillComputeScaling.m1_str` tokens):**
...
**2. Evaluation of Policy B ($M_2 =$ `{python} PrefillComputeScaling.m2_str` tokens):**
...
**Systems Takeaway:**
Expanding the context window by a factor of `{python} PrefillComputeScaling.expansion_ratio_int_str` ($8\text{k} \to 64\text{k}$) increases total prefill compute by a factor of `{python} PrefillComputeScaling.compute_factor_int_str` ($140.7\text{ TFLOPs} \to 3{,}096.2\text{ TFLOPs}$). The accelerator shifts from a linear-projection-dominated workload (where attention accounts for only `{python} PrefillComputeScaling.pct_attn_a_str` of FLOPs) into an attention-dominated quadratic regime (where attention accounts for nearly `{python} PrefillComputeScaling.pct_attn_b_str` of FLOPs). For an agent executing a `{python} PrefillComputeScaling.n_turns_str`-turn interactive debugging loop, Policy B introduces over three minutes ($30 \times 6.26\text{ s} \approx 188\text{ s}$) of pure prefill pipeline latency, compared to under nine seconds for Policy A, while consuming `{python} PrefillComputeScaling.compute_factor_int_str` times more energy on the accelerator cluster.
:::
...
```{python}
#| label: lego-04-quantitative-impact-of-prefix-layout
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ QUANTITATIVE IMPACT OF PREFIX LAYOUT ON PREFILL AMORTIZATION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-04-quantitative-impact-of-prefix-layout
# │ Goal: Contrast prefill compute and latency under unstable vs prefix-cached prompt layouts.
# │ Show: Stable layout achieves a 75.6% compute reduction and 4.1x speedup, saving 7.7 min over 20 turns.
# │ How: Model prefill FLOPs per turn with zero caching vs prefix hits on immutable root and trunk.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_multiple

class PrefixLayoutAmortization:
    """Quantitative impact of prefix layout on prefill amortization."""
    ...
    l_trunk = 24_576
    l_leaf = 4_096
    pi_prefill = 150 * (TFLOP / second)

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    l_trunk_and_leaf = l_trunk + l_leaf
    flops_per_token = 2 * n_params

    # Case 1: Unstable layout
    flops_unstable_turn = flops_per_token * l_ctx
    total_flops_unstable = t_turns * flops_unstable_turn
    t_unstable_turn = (flops_unstable_turn * flop / pi_prefill).to(second)
    t_unstable = (total_flops_unstable * flop / pi_prefill).to(second)

    # Case 2: Stable layout
    n_cold_turns = 1
    n_mutation_turns = 2
    n_cached_turns = t_turns - n_cold_turns - n_mutation_turns

    flops_turn1 = flops_per_token * l_ctx
    flops_mutation = flops_per_token * l_trunk_and_leaf
    flops_cached = flops_per_token * l_leaf

    total_flops_stable = n_cold_turns * flops_turn1 + n_mutation_turns * flops_mutation + n_cached_turns * flops_cached
    t_stable = (total_flops_stable * flop / pi_prefill).to(second)

    pct_savings = (total_flops_unstable - total_flops_stable) / total_flops_unstable
    speedup = total_flops_unstable / total_flops_stable
    time_saved = t_unstable - t_stable

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(t_turns == 20, "t_turns")
    check(n_cached_turns == 17, "n_cached_turns")
    check(abs(t_unstable_turn.magnitude - 30.58) < 0.1, "t_unstable_turn")
    check(abs(t_unstable.magnitude - 611.6) < 1.0, "t_unstable")
    check(abs(t_stable.magnitude - 149.1) < 1.0, "t_stable")
    check(abs(pct_savings - 0.756) < 0.005, "pct_savings")
    check(abs(speedup - 4.10) < 0.1, "speedup")
    check(abs(time_saved.to(minute).magnitude - 7.7) < 0.2, "time_saved")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    t_turns_str = fmt_int(t_turns)
    l_ctx_str = fmt_int(l_ctx)
    l_root_str = fmt_int(l_root)
    l_trunk_str = fmt_int(l_trunk)
    l_leaf_str = fmt_int(l_leaf)
    l_trunk_and_leaf_str = fmt_int(l_trunk_and_leaf)
    n_cached_turns_str = fmt_int(n_cached_turns)
    t_unstable_s_str = fmt_time(t_unstable, unit=second, precision=1)
    t_unstable_min_str = fmt_time(t_unstable, unit=minute, precision=1)
    pct_savings_str = fmt_percent(pct_savings, precision=1, style="prose")
    speedup_mult_str = fmt_multiple(speedup, precision=1)
    time_saved_min_str = fmt_time(time_saved, unit=minute, precision=1)
```
...
Consider an autonomous coding agent executing a debugging task over a trajectory of $T =$ `{python} PrefixLayoutAmortization.t_turns_str` turns. The host supervisor interacts with a 70-billion parameter dense model ($P = 70 \times 10^9$) operating at a context length of $L =$ `{python} PrefixLayoutAmortization.l_ctx_str` tokens.

The logical working set decomposes into three components:

- Static Root directives and tool schemas: $L_{\text{root}} =$ `{python} PrefixLayoutAmortization.l_root_str` tokens.
- Trunk workspace files and plan state: $L_{\text{trunk}} =$ `{python} PrefixLayoutAmortization.l_trunk_str` tokens. We assume the workspace is modified on turns $t=7$ and $t=14$, invalidating the Trunk cache at those two boundaries, but remains stable across all other turns.
- Leaf observations and scratchpad notes: $L_{\text{leaf}} =$ `{python} PrefixLayoutAmortization.l_leaf_str` tokens, replaced or appended on every turn.
...
Because token index $0$ mutates on every invocation, the prefix match length is $k = 0$ for all `{python} PrefixLayoutAmortization.t_turns_str` turns. In accordance with the foundation engine Roofline model established in @sec-vol3-processor-cost, the prefill engine processes all $L =$ `{python} PrefixLayoutAmortization.l_ctx_str` tokens on every turn without cache reuse:
...
Across `{python} PrefixLayoutAmortization.t_turns_str` turns, the cumulative time spent strictly computing prompt prefills is approximately `{python} PrefixLayoutAmortization.t_unstable_s_str` (`{python} PrefixLayoutAmortization.t_unstable_min_str`).

**Case 2: Stable Layout (Prefix Caching Enabled)**

- On Turn 1 (cold start), the engine computes all `{python} PrefixLayoutAmortization.l_ctx_str` tokens: $4.587 \times 10^{15}\text{ FLOPs}$.
- On Turns 7 and 14, the Trunk mutates. The Root (`{python} PrefixLayoutAmortization.l_root_str` tokens) hits the cache, but the Trunk and Leaf (`{python} PrefixLayoutAmortization.l_trunk_and_leaf_str` tokens) must be prefilled:
  $$\text{FLOPs}_{\text{mutation}} = 2 \times (70 \times 10^9) \times 28{,}672 \approx 4.014 \times 10^{15}\text{ FLOPs}$$

- On the remaining `{python} PrefixLayoutAmortization.n_cached_turns_str` turns, both the Root and the Trunk (`{python} PrefixLayoutAmortization.l_trunk_and_leaf_str` tokens) hit the cache. The engine prefills only the dynamic Leaf (`{python} PrefixLayoutAmortization.l_leaf_str` tokens):
...
Structuring the prompt to maintain prefix stability reduces total prefill compute and latency by `{python} PrefixLayoutAmortization.pct_savings_str` (a `{python} PrefixLayoutAmortization.speedup_mult_str` throughput speedup), saving over `{python} PrefixLayoutAmortization.time_saved_min_str` of execution delay across a single `{python} PrefixLayoutAmortization.t_turns_str`-step trajectory.
:::
...
```{python}
#| label: lego-04-token-budget-compaction-and-roofline
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TOKEN BUDGET COMPACTION AND ROOFLINE PREFILL SAVINGS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-04-token-budget-compaction-and-roofline
# │ Goal: Dimension KV cache memory reclamation and prefill speedup via multi-stage compaction.
# │ Show: Compacting 96 Ki to 24 Ki tokens frees 22.50 GiB VRAM and cuts prefill latency by 7.78x.
# │ How: Calculate GQA KV bytes per token; compute projection and attention FLOPs under sustained MFU.
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_multiple, fmt_memory_capacity
from mlsysim.physics.agents import calc_kv_cache_bytes_per_token

class TokenBudgetCompaction:
    """Token budget compaction and Roofline prefill savings."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    agent = CodingAgents.SWE_Bench_Runner
    model = agent.reference_model
    n_params = 70e9
    n_layers = model.layers
    d_model = model.hidden_dim
    h_kv = model.kv_heads
    d_k = model.hidden_dim // model.heads
    bytes_per_elem = 2 * byte
    p_effective = 494.5 * (TFLOP / second)

    n_raw = 98_304
    lossless_strip = 30 * 1024
    structured_prune = 25 * 1024
    semantic_reclaim = 17 * 1024

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    n_compact = n_raw - (lossless_strip + structured_prune + semantic_reclaim)
    bytes_per_token = calc_kv_cache_bytes_per_token(agent)

    mem_raw = (n_raw * bytes_per_token).to(GiB)
    mem_compact = (n_compact * bytes_per_token).to(GiB)
    delta_memory = mem_raw - mem_compact

    # FLOPs
    flops_proj_raw = 2 * n_params * n_raw
    flops_attn_raw = 4 * n_layers * d_model * (n_raw**2)
    flops_total_raw = flops_proj_raw + flops_attn_raw
    t_prefill_raw = (flops_total_raw * flop / p_effective).to(second)

    flops_proj_compact = 2 * n_params * n_compact
    flops_attn_compact = 4 * n_layers * d_model * (n_compact**2)
    flops_total_compact = flops_proj_compact + flops_attn_compact
    t_prefill_compact = (flops_total_compact * flop / p_effective).to(second)

    speedup = (t_prefill_raw / t_prefill_compact).magnitude
    time_saved = t_prefill_raw - t_prefill_compact

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(n_compact == 24_576, "n_compact")
    check(bytes_per_token.magnitude == 327_680, "bytes_per_token")
    check(abs(mem_raw.magnitude - 30.00) < 0.1, "mem_raw")
    check(abs(mem_compact.magnitude - 7.50) < 0.1, "mem_compact")
    check(abs(delta_memory.magnitude - 22.50) < 0.1, "delta_memory")
    check(abs(t_prefill_raw.magnitude - 78.99) < 0.1, "t_prefill_raw")
    check(abs(t_prefill_compact.magnitude - 10.15) < 0.1, "t_prefill_compact")
    check(abs(speedup - 7.78) < 0.05, "speedup")
    check(abs(time_saved.magnitude - 68.84) < 0.5, "time_saved")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_raw_str = fmt_int(n_raw)
    n_compact_str = fmt_int(n_compact)
    delta_memory_gib_str = fmt_memory_capacity(delta_memory, unit=GiB, precision=2)
    speedup_mult_str = fmt_multiple(speedup, precision=2)
    time_saved_floor = int(math.floor(time_saved.magnitude)) * second
    time_saved_s_str = fmt_time(time_saved_floor, unit=second, precision=0)
```
...
**Problem:** Over an active debugging trajectory of 45 turns, the uncompressed transcript accumulates $N_{\text{raw}} = 96\text{ Ki tokens}$ (`{python} TokenBudgetCompaction.n_raw_str` tokens). The runtime compaction pipeline processes this transcript:
...
For the uncompressed context ($N_{\text{raw}} =$ `{python} TokenBudgetCompaction.n_raw_str` tokens):
$$\text{Memory}_{\text{raw}} = 98{,}304 \times 327{,}680\text{ bytes} \approx 32.21\times 10^9\text{ bytes} \approx 30.00\text{ GiB}$$

For the compacted context ($N_{\text{compact}} =$ `{python} TokenBudgetCompaction.n_compact_str` tokens):
...
Compaction frees `{python} TokenBudgetCompaction.delta_memory_gib_str` of physical GPU VRAM, preventing out-of-memory crashes and permitting larger batch sizes or extended generation lengths.
...
For $N_{\text{raw}} =$ `{python} TokenBudgetCompaction.n_raw_str`:
...
For $N_{\text{compact}} =$ `{python} TokenBudgetCompaction.n_compact_str`:
...
Compaction delivers a `{python} TokenBudgetCompaction.speedup_mult_str` reduction in prefill latency (saving over `{python} TokenBudgetCompaction.time_saved_s_str` of blocking host time per turn), primarily driven by the quadratic reduction in self-attention matrix operations over long context sequences.
:::
...
```{python}
#| label: lego-04-quantitative-checkpoint-auditing-and-prefill
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ QUANTITATIVE CHECKPOINT AUDITING AND PREFILL RECLAIM (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-04-quantitative-checkpoint-auditing-and-prefill
# │ Goal: Evaluate latency reduction, memory recovery, and host CPU audit overhead of structured checkpoints.
# │ Show: Compressing 112k to 3.5k tokens yields 66.7x speedup and 36.41 GB HBM recovery; CPU audit takes 1.8 ms (<1.4%).
# │ How: Contrast 8-GPU tensor-parallel prefill FLOPs and KV memory for raw context vs hydrated checkpoint.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_multiple, fmt_memory_capacity

class CheckpointAuditingPrefill:
    """Quantitative checkpoint auditing and prefill reclaim."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_gpus = 8
    p_peak_per_gpu = 1975 * (TFLOP / second)
    mfu_prefill = 0.45
    d_model = 8192
    n_layers = 80
    n_heads = 64
    ...
    l_chk = 1_536
    l_tail = 2_048
    t_audit = 1.8 * millisecond

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    p_effective = n_gpus * p_peak_per_gpu * mfu_prefill
    l_hydrated = l_chk + l_tail
    bytes_per_token = calc_kv_cache_bytes_per_token(n_layers=n_layers, n_kv_heads=n_kv_heads, head_dim=d_head, bytes_per_elem=bytes_per_elem)

    # Raw
    flops_gemm_raw = 48 * n_layers * (d_model**2) * l_raw
    flops_attn_raw = 4 * n_layers * d_model * (l_raw**2)
    flops_total_raw = flops_gemm_raw + flops_attn_raw
    t_prefill_raw = (flops_total_raw * flop / p_effective).to(second)
    mem_kv_raw = (l_raw * bytes_per_token).to(GB)

    # Checkpoint
    flops_gemm_chk = 48 * n_layers * (d_model**2) * l_hydrated
    flops_attn_chk = 4 * n_layers * d_model * (l_hydrated**2)
    flops_total_chk = flops_gemm_chk + flops_attn_chk
    t_prefill_chk = (flops_total_chk * flop / p_effective).to(second)
    mem_kv_chk = (l_hydrated * bytes_per_token).to(GB)

    # Summary
    compression_ratio = l_raw / l_hydrated
    speedup_latency = (t_prefill_raw / t_prefill_chk).magnitude
    t_saved = t_prefill_raw - t_prefill_chk
    mem_reclaimed = mem_kv_raw - mem_kv_chk
    concurrency_increase = int(compression_ratio)
    audit_overhead_pct = (t_audit.to(second) / t_prefill_chk).magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(l_hydrated == 3584, "l_hydrated")
    check(abs(t_prefill_raw.magnitude - 9.00) < 0.1, "t_prefill_raw")
    check(abs(t_prefill_chk.magnitude - 0.135) < 0.01, "t_prefill_chk")
    check(abs(compression_ratio - 32.0) < 0.1, "compression_ratio")
    check(abs(speedup_latency - 66.7) < 0.5, "speedup_latency")
    check(abs(t_saved.magnitude - 8.865) < 0.05, "t_saved")
    check(abs(mem_reclaimed.magnitude - 36.41) < 0.1, "mem_reclaimed")
    check(abs(audit_overhead_pct - 0.0133) < 0.002, "audit_overhead_pct")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    l_raw_str = fmt_int(l_raw)
    l_chk_str = fmt_int(l_chk)
    l_tail_str = fmt_int(l_tail)
    t_prefill_raw_s_str = fmt_time(t_prefill_raw, unit=second, precision=2)
    t_prefill_chk_s_str = fmt_time(t_prefill_chk, unit=second, precision=3)
    speedup_latency_mult_str = fmt_multiple(speedup_latency, precision=1)
    t_saved_s_str = fmt_time(t_saved, unit=second, precision=3)
    mem_reclaimed_gb_str = fmt_memory_capacity(mem_reclaimed, unit=GB, precision=2)
    compression_ratio_mult_str = fmt_multiple(compression_ratio, precision=0)
    concurrency_increase_mult_str = fmt_multiple(concurrency_increase, precision=0)
    t_audit_ms_str = fmt_time(t_audit, unit=millisecond, precision=1)
    audit_overhead_pct_str = fmt_percent(audit_overhead_pct, precision=1, style="prose")
```
...
Consider an autonomous software engineering agent executing a multi-file refactoring task on a large repository. Over 48 turns of tool interactions, the raw context buffer accumulates $L_{\text{raw}} =$ `{python} CheckpointAuditingPrefill.l_raw_str` tokens ($112\text{k}$ tokens), consisting of compiler outputs, source diffs, and directory listings.
...
The runtime prompts the model to distill the history into a structured checkpoint $\mathcal{S}_{\text{checkpoint}}$, yielding $L_{\text{chk}} =$ `{python} CheckpointAuditingPrefill.l_chk_str` tokens ($1.5\text{k}$ tokens), alongside an active working tail of the most recent 2 turns ($L_{\text{tail}} =$ `{python} CheckpointAuditingPrefill.l_tail_str` tokens). The hydrated context length becomes:
...
- **Token Compression Ratio:** $\frac{114\text{,}688}{3\text{,}584} =$ `{python} CheckpointAuditingPrefill.compression_ratio_mult_str`
- **Prefill Latency Reduction:** From `{python} CheckpointAuditingPrefill.t_prefill_raw_s_str` to `{python} CheckpointAuditingPrefill.t_prefill_chk_s_str` (`{python} CheckpointAuditingPrefill.speedup_latency_mult_str` speedup, eliminating `{python} CheckpointAuditingPrefill.t_saved_s_str` of blocking GPU time per turn).
- **HBM Footprint Reclamation:** $37.58\text{ GB} - 1.17\text{ GB} =$ `{python} CheckpointAuditingPrefill.mem_reclaimed_gb_str` saved per batch instance, allowing a `{python} CheckpointAuditingPrefill.concurrency_increase_mult_str` increase in concurrent serving concurrency on identical physical hardware.
- **Verification Overhead:** The deterministic audit in `audit_checkpoint` parses the 48-event JSON log in host memory in `{python} CheckpointAuditingPrefill.t_audit_ms_str` on a standard CPU core—less than `{python} CheckpointAuditingPrefill.audit_overhead_pct_str` of the single-turn prefill time—completely preventing the propagation of corrupted milestones into the compacted state.
:::
...
1. **Hard Context Eviction (Purge):** The runtime excises the stale observation $o_k$ entirely from the message history, removing both the tool invocation and its returned payload. Hard eviction is clean, but it alters the token sequence of earlier turns, and cached attention state survives only up to the first changed token (principle \ref{pri-vol3-prefix-coherence}). As established in @sec-vol3-working-sets-staging, modifying historical tokens destroys prefix-cache sharing in runtimes utilizing RadixAttention or PagedAttention. If the runtime deletes 500 tokens from Turn 2 of an 18-turn trajectory, the inference engine cannot reuse the cached Key-Value (KV) tensors for Turns 3 through 18, forcing an expensive, full-context prefill phase.
...
```{python}
#| label: lego-04-latency-and-compute-roofline-of
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ LATENCY AND COMPUTE ROOFLINE OF CONTEXT INVALIDATION STRATEGIES (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-04-latency-and-compute-roofline-of
# │ Goal: Compare prefill latency of hard prefix eviction vs append-only semantic tombstoning.
# │ Show: Semantic tombstoning reduces prefill latency from 1,704 ms to 8.9 ms (191x speedup).
# │ How: Hard eviction triggers recompute from split point; append-only preserves KV prefix hits.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time, fmt_multiple

class ContextInvalidationRoofline:
    """Latency and compute Roofline of context invalidation strategies."""
    ...
    tokens_read = 850
    l_split = 4_200
    p_peak = 3350 * (TFLOP / second)
    efficiency = 0.60
    n_delta = 128

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    p_effective = efficiency * p_peak

    # Strategy A: Hard eviction
    n_recompute_hard = s_ctx - l_split
    flops_hard = 2 * p_params * n_recompute_hard
    t_hard = (flops_hard * flop / p_effective).to(second)

    # Strategy B: Semantic tombstoning (append-only)
    flops_tomb = 2 * p_params * n_delta
    t_tomb = (flops_tomb * flop / p_effective).to(second)

    speedup = (t_hard / t_tomb).magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(n_recompute_hard == 24_472, "n_recompute_hard")
    check(abs(t_hard.magnitude - 1.704) < 0.01, "t_hard")
    check(abs(t_tomb.to(millisecond).magnitude - 8.9) < 0.1, "t_tomb")
    check(abs(speedup - 191.0) < 2.0, "speedup")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    tokens_read_str = fmt_int(tokens_read)
    t_hard_ms = int(t_hard.to(millisecond).magnitude) * millisecond
    t_hard_ms_str = fmt_time(t_hard_ms, unit=millisecond, precision=0, commas=True)
    t_tomb_ms_str = fmt_time(t_tomb, unit=millisecond, precision=1)
    speedup_mult_str = fmt_multiple(round(speedup), precision=0)
    t_hard_s_str = fmt_time(t_hard, unit=second, precision=1)
```
...
**Strategy A: Hard Eviction**
The runtime excises the `{python} ContextInvalidationRoofline.tokens_read_str` tokens of `include/net/nic.h` from Turn 3.
...
The runtime replaces the `{python} ContextInvalidationRoofline.tokens_read_str`-token file excerpt in Turn 3 with a 24-token semantic tombstone:
...
**Trade-off Analysis:**
Append-only tombstoning reduces prefill latency from **`{python} ContextInvalidationRoofline.t_hard_ms_str` to `{python} ContextInvalidationRoofline.t_tomb_ms_str`**—a **`{python} ContextInvalidationRoofline.speedup_mult_str` speedup**—by preserving KV cache reuse. However, it incurs an attention tax: the model's attention heads must actively suppress the `{python} ContextInvalidationRoofline.tokens_read_str` stale tokens still present in the prompt prefix by attending to the invalidation record at the tail. If the model exhibits high attention distraction, the runtime must pay the `{python} ContextInvalidationRoofline.t_hard_s_str` prefill penalty of Hard Eviction to guarantee semantic purity.
:::
...
```{python}
#| label: lego-04-quantifying-working-set-efficiency-and-compaction
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ QUANTIFYING WORKING-SET EFFICIENCY AND COMPACTION TRADE-OFFS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-04-quantifying-working-set-efficiency-and-compaction
# │ Goal: Dimension trajectory context growth, compaction compliance, and reliability trade-offs.
# │ Show: Structured checkpointing ensures budget compliance and bounds wasted turns to 1.8 vs 18.
# │ How: Calculate uncompacted volume; assess compaction retention probability and error penalty.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_multiple

class WorkingSetEfficiency:
    """Quantifying working-set efficiency and compaction trade-offs."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_turns = 30
    tokens_per_turn = 2400
    s_max = 16_384
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    s_raw = n_turns * tokens_per_turn
    s_a = int(s_raw / cr_a)
    s_b = int(round(s_raw / cr_b))

    n_cycles = int(n_turns / cycle_interval)
    p_retain_a = (1.0 - p_omit_a) ** n_cycles

    wasted_turns_a = int(n_turns * eps_stale_a * penalty_turns)
    wasted_turns_b = n_turns * eps_stale_b * penalty_turns

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(s_raw == 72_000, "s_raw")
    check(s_a == 12_000, "s_a")
    check(s_b == 20_571, "s_b")
    check(n_cycles == 6, "n_cycles")
    check(abs(p_retain_a - 0.377) < 0.005, "p_retain_a")
    check(wasted_turns_a == 18, "wasted_turns_a")
    check(abs(wasted_turns_b - 1.8) < 0.01, "wasted_turns_b")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_turns_str = fmt_int(n_turns)
    tokens_per_turn_str = fmt_int(tokens_per_turn)
    s_max_str = fmt_int(s_max)
    cr_a_mult_str = fmt_multiple(cr_a, precision=0)
    cr_b_mult_str = fmt_multiple(cr_b, precision=1)
    s_b_pruned_str = fmt_int(s_b_pruned)
```
...
Consider an autonomous systems repair agent executing an incident remediation trajectory over $T =$ `{python} WorkingSetEfficiency.n_turns_str` turns.

- The raw trajectory generates an average of `{python} WorkingSetEfficiency.tokens_per_turn_str` tokens per turn (comprising shell outputs, log traces, and diffs), producing an uncompressed history of:
  $$S_{\text{raw}} = 30 \times 2\text{,}400 = 72\text{,}000 \text{ tokens}$$

- The host system limits the active working set to $S_{\max} =$ `{python} WorkingSetEfficiency.s_max_str` tokens.
- We evaluate two context management policies:
  1. **Policy A (Lossy Narrative Compaction):** Every 5 turns, an auxiliary prompt summarizes historical actions and tool outputs into an unstructured narrative paragraph, achieving an average compression ratio of $C_r =$ `{python} WorkingSetEfficiency.cr_a_mult_str`. However, compaction introduces a $15\%$ per-compaction probability of omitting minor operational constraints (e.g., specific environment flags or path parameters), and carries a stale-state error frequency of $\epsilon_{\text{stale}} = 0.20$.
  2. **Policy B (Structured State Checkpointing with Invalidation):** Maintains an explicit JSON state delta, exact file diffs, and tombstones for invalidated observations, achieving $C_r =$ `{python} WorkingSetEfficiency.cr_b_mult_str`. Constraint retention is guaranteed via deterministic schema validation ($100\%$ retention of tracked environment flags), and invalidation rules suppress stale references to $\epsilon_{\text{stale}} = 0.02$.
...
Because Policy B exceeds $S_{\max}$, the runtime must evict secondary observational logs (retaining only the structural state deltas, active file boundaries, and tombstones), bringing the staged working set to exactly $S_B' =$ `{python} WorkingSetEfficiency.s_b_pruned_str` tokens while preserving all primary constraints.
...
Even though Policy A achieved a higher compression ratio (`{python} WorkingSetEfficiency.cr_a_mult_str` vs. `{python} WorkingSetEfficiency.cr_b_mult_str`), its operational efficiency is destroyed by constraint forgetting and stale-state execution loops. Policy B delivers superior systems dependability under an identical physical context constraint.
:::
...
@Sec-vol3-virtual-memory (*Paged Attention Memory*) transitions from logical context composition to physical memory virtualization. It investigates how modern inference runtimes such as vLLM and SGLang manage physical KV cache allocations on accelerator hardware, partitioning contiguous logical token sequences into non-contiguous physical pages via PagedAttention, sharing prompt prefixes across branching rollouts through Radix trees, and balancing memory-bound GEMV decode loops against compute-bound GEMM prefill phases under tight multi-tenant memory constraints.
:::
