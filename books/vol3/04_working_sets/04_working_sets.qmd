...
*Accumulating 100k background tokens dilutes the softmax denominator, slashing target attention from 97% to 3%.*
:::
...
![**Multi-Head Latent Attention (MLA) Architecture and Cache Compression**: Architectural schematic of Multi-Head Latent Attention. (1) Per-token KV cache footprint comparison between Multi-Head Attention (MHA, 2,560 KB/tok), Grouped-Query Attention (GQA, 320 KB/tok), and Multi-Head Latent Attention (MLA, 67.5 KB/tok), achieving a 38× compression over MHA. (2) Low-rank down-projection into a compressed latent vector $\mathbf{c}_t^{KV} \in \mathbb{R}^{512}$ with decoupled Rotary Positional Embeddings (RoPE) key $\mathbf{k}_t^R \in \mathbb{R}^{64}$. (3) Matrix associativity trick during autoregressive decode, where key projection weights are absorbed into queries ($\mathbf{q}_{\text{latent}} = \mathbf{q}_t \mathbf{W}_{UK}^T$), allowing attention dot-products to execute directly against compressed latents in fast on-chip SRAM without materializing high-dimensional keys and values in HBM.](images/svg/multi_head_latent_attention_mla.svg){#fig-vol3-multi-head-latent-attention width="95%"}

### Workload-Driven Sizing


::: {.column-margin}
![](images/svg/vol3_working_sets_margin_004.svg){width="100%" fig-alt="Four-rung log-scale memory ladder showing per-token KV cache footprint dropping from 2,560 kilobytes in MHA down to 320 kilobytes in GQA, 67.5 kilobytes in MLA, and 40 kilobytes in MQA."}

*Multi-Head Latent Attention cuts KV cache footprint to 67.5 KB per token, a 38-fold compression over dense multi-head attention.*
:::
...
Consider a long-horizon systems debugging agent undergoing $k=5$ successive compaction cycles. Under lossy narrative summarization with an empirical constraint attenuation rate of $\lambda = 0.20$ (a 20% chance per cycle of dropping a seemingly minor parameter flag or negative boundary condition), the probability of retaining all critical constraints decays exponentially:
...
Chapter 05 (*Paged Attention Memory*, @sec-vol3-virtual_memory) transitions from logical context composition to physical memory virtualization. It investigates how modern inference runtimes such as vLLM and SGLang manage physical KV cache allocations on accelerator hardware, partitioning contiguous logical token sequences into non-contiguous physical pages via PagedAttention, sharing prompt prefixes across branching rollouts through Radix trees, and balancing memory-bound GEMV decode loops against compute-bound GEMM prefill phases under tight multi-tenant memory constraints.
:::
