...
By confining cache writes to a fixed set of physical slots, the memory footprint remains bounded at $S + W$ across arbitrarily long inference runs, eliminating GPU out-of-memory faults caused by unbounded token accumulation.
...
Excising the ephemeral scratchpad after execution yields a 60-fold reduction in long-horizon KV memory while preserving 100 percent cache hits on the shared prompt prefix $\tau_{t-1}$.
