...
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_time, fmt_multiple, fmt_memory_capacity
...
class TokenBudgetCompaction:
    ...
    bytes_per_token = 2 * (n_layers * h_kv * d_k * bytes_per_elem)
...
class CheckpointAuditingPrefill:
    ...
    t_prefill_raw = (flops_total_raw * flop / p_effective).to(second)
    mem_kv_raw = (4 * byte * n_layers * (n_kv_heads * d_head) * l_raw).to(GB)
    ...
    mem_kv_chk = (4 * byte * n_layers * (n_kv_heads * d_head) * l_hydrated).to(GB)
