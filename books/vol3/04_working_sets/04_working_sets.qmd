...
**The attention crossover point napkin math:** Comparing the linear and quadratic terms in @eq-autoregressive-prefill-flops reveals the exact sequence length where attention compute overtakes linear projections:
$$
2 L N^2 d_{\text{model}} \approx 4 L N d_{\text{model}} (2 d_{\text{model}} + d_{\text{ff}}) \implies N_{\text{crossover}} \approx 2 (2 d_{\text{model}} + d_{\text{ff}}).
$$
For a 70B parameter model ($d_{\text{model}} = 8{,}192, d_{\text{ff}} = 28{,}672$):
$$
N_{\text{crossover}} \approx 2 \cdot (2 \cdot 8{,}192 + 28{,}672) = 2 \cdot (16{,}384 + 28{,}672) = 90{,}112\text{ tokens}.
$$
Below 90k tokens, matrix projections and FFN layers consume the majority of prefill FLOPs. Above 90k tokens, the quadratic pairwise attention matrix sweeps past all linear projections. While memory-efficient attention algorithms such as FlashAttention-2 (@dao2023flashattention2) eliminate the materialization of the $N \times N$ attention matrix in HBM by tiling the computation across on-chip SRAM, they do not reduce the underlying arithmetic operation count. The physical execution time of prefill remains fundamentally quadratic in $N$: $T_{\text{prefill}}(N) \approx F_{\text{prefill}}(N) / (P_{\text{peak}} \cdot \eta_{\text{compute}})$, where $P_{\text{peak}}$ is the peak dense tensor throughput of the accelerator (e.g., $989 \times 10^{12}\text{ FLOPs/s}$ in 16-bit precision for an NVIDIA H100 SXM5) and $\eta_{\text{compute}} \in [0.45, 0.65]$ is the realized hardware compute efficiency. For an agent ingesting a 128k-token trajectory on a dual-H100 node with a 70B model, prefill compute demands over $3.8 \times 10^{16}$ FLOPs, imposing a multi-second pause before the agent can emit its first reasoning token.
...
**First-principles KV cache napkin math:** Consider a representative frontier agent policy: Llama-3-70B, which uses Grouped-Query Attention (GQA, @ainslie2023gqa) with $L = 80$, $H_Q = 64$, $H_{\text{KV}} = 8$, $d_h = 128$, and BF16 precision ($b_{\text{elem}} = 2$). Evaluating @eq-kv-cache-footprint for a single token ($B=1, N=1$) yields:
$$
M_{\text{KV, token}} = 2 \times 80\text{ layers} \times 8\text{ KV heads} \times 128\text{ dims} \times 2\text{ bytes} = 327{,}680\text{ bytes} \approx 320\text{ KB/token}.
$$
At a context length of $N = 131{,}072$ tokens (128k), a single resident stream consumes:
$$
M_{\text{KV}}(1, 128\text{k}) = 131{,}072 \times 320\text{ KB} \approx 42.95\text{ GB}.
$$
Because the 70B model weights in 16-bit precision occupy approximately 140 GB, serving this model requires at least two 80 GB GPUs (160 GB total HBM). A single 128k agent stream consumes over 53 percent of the remaining free memory across both GPUs. If four concurrent agent tasks are executed in parallel ($B=4$), aggregate KV cache demand reaches:
$$
M_{\text{KV}}(4, 128\text{k}) = 4 \times 42.95\text{ GB} = 171.8\text{ GB},
$$
exceeding physical accelerator capacity and triggering catastrophic out-of-memory aborts (@kwon2023vllm).
...
- **Short context ($N \approx 0, M_{\text{KV}} \approx 0$):**
  $$T_{\text{decode}} \approx \frac{140\text{ GB}}{6{,}700\text{ GB/s}} \approx 20.9\text{ ms/token} \implies \text{Throughput} \approx 47.8\text{ tokens/s}.$$
- **128k context ($M_{\text{KV}} \approx 43\text{ GB}$ across node):** Total data streamed per token is $140 + 43 = 183\text{ GB}$.
  $$T_{\text{decode}} \approx \frac{183\text{ GB}}{6{,}700\text{ GB/s}} \approx 27.3\text{ ms/token} \implies \text{Throughput} \approx 36.6\text{ tokens/s} \quad (\text{23\% slowdown}).$$
- **500k context ($M_{\text{KV}} \approx 168\text{ GB} > M_{\text{weights}}$):** Total data streamed across node is $140 + 168 = 308\text{ GB}$.
  $$T_{\text{decode}} \approx \frac{308\text{ GB}}{6{,}700\text{ GB/s}} \approx 46.0\text{ ms/token} \implies \text{Throughput} \approx 21.7\text{ tokens/s} \quad (\text{54\% slowdown}).$$
...
- **In a compact 100-token prompt ($N = 100$):** The 100 background tokens contribute $100 \times 0.135 = 13.5$ to the softmax denominator. The target captures:
  $$\alpha_{\text{target}} = \frac{403.4}{403.4 + 13.5} = \frac{403.4}{416.9} \approx 96.8\%\text{ of total attention mass}.$$
- **In an unmanaged 100,000-token prompt ($N = 100{,}000$):** Even though every background token is 8 logit units below the target, 100,000 background tokens contribute $100{,}000 \times 0.135 = 13{,}500$ to the denominator! The target's attention share collapses to:
  $$\alpha_{\text{target}} = \frac{403.4}{403.4 + 13{,}500} = \frac{403.4}{13{,}903.4} \approx 2.9\%\text{ of total attention mass}.$$
...
With this partition, language models can stream over millions of tokens with stable perplexity and constant $O(S + W)$ memory footprint.
...
**Deliberative reasoning bloat napkin math:** An agent powered by a reasoning model often emits an average of $6{,}000$ scratchpad tokens before issuing each tool call. Across a 30-turn debugging trajectory, if intermediate thoughts are naively retained in context:
$$
\text{Total scratchpad bloat} = 30\text{ turns} \times 6{,}000\text{ tokens/turn} = 180{,}000\text{ tokens}.
$$
On a 70B model with GQA ($320\text{ KB/token}$ in BF16), holding these $180{,}000$ inert scratchpad tokens consumes:
$$
M_{\text{KV}} = 180{,}000 \times 320\text{ KB} \approx 57.6\text{ GB of HBM},
$$
and adds over $57.6\text{ GB}$ of memory-bus streaming overhead to every subsequent generated token!
...
4. **Radix Prefix Hit and Automatic LRU Eviction:**
   - The serving engine hashes the prompt prefix. Tokens matching $\tau_{t-1}$ hit the resident Radix tree with **$100\%$ cache reuse**, requiring zero prefill FLOPs.
...
where $\mathbf{W}_h^{(\ell)}$ represents the set of parameters belonging to head $h$ across query, key, value, and output projections ($\mathbf{W}_Q^{h}, \mathbf{W}_K^{h}, \mathbf{W}_V^{h}, \mathbf{W}_O^{h}$). The systems intuition is simple: *if zeroing out a head's parameters causes near-zero change in loss ($I_h^{(\ell)} \approx 0$), that head is computing redundant representations (such as static broadcast or identity copies).* Heads with scores below an empirical threshold $\tau_{\text{prune}}$ are removed:
...
**The matrix associativity trick behind MLA:** A common concern is that MLA might require decompressing the latent vectors back into full Key and Value head dimensions in HBM before computing attention, which would re-introduce the memory bus bottleneck. MLA completely avoids this via matrix associativity.

Recall that attention logits require computing $\mathbf{q}_t \mathbf{k}_i^\top$. In MLA, key vectors are generated from the latent representation via up-projection matrix $\mathbf{W}_{UK}$: $\mathbf{k}_i^C = \mathbf{c}_i^{KV} \mathbf{W}_{UK}$. Because matrix multiplication is associative:
$$
\mathbf{q}_t (\mathbf{k}_i^C)^\top = \mathbf{q}_t \left( \mathbf{c}_i^{KV} \mathbf{W}_{UK} \right)^\top = \left( \mathbf{q}_t \mathbf{W}_{UK}^\top \right) (\mathbf{c}_i^{KV})^\top.
$$
Instead of decompressing all $N$ historical keys in HBM, the inference kernel pre-multiplies the *single active query vector* by $\mathbf{W}_{UK}^\top$ once in fast on-chip SRAM registers:
$$
\mathbf{q}_t^{\text{latent}} = \mathbf{q}_t \mathbf{W}_{UK}^\top \in \mathbb{R}^{d_c}.
$$
It then evaluates the inner product directly against the cached latent vectors $\mathbf{c}_i^{KV} \in \mathbb{R}^{d_c}$. An identical associative projection applies to the Value projections. As a result, the inference engine never materializes high-dimensional Key or Value states in HBM—streaming only $512$ latent dimensions across the bus instead of thousands!
...
**Serving concurrency napkin math:** Consider an 80 GB NVIDIA H100 accelerator serving a 70B parameter model at 128k context ($N = 131{,}072$).
In FP8 precision ($b_{\text{elem}} = 1$), model weights occupy $M_{\text{weights}} \approx 70\text{ GB}$. The remaining free memory for KV cache allocation is:
$$C_{\text{free}} = 80\text{ GB} - 70\text{ GB} = 10\text{ GB}.$$
- **Under Grouped-Query Attention (GQA, $H_{\text{KV}} = 8, d_h = 128, L = 80$):** A single 128k stream in FP8 demands:
  $$M_{\text{KV, GQA}} = 2 \times 80 \times 8 \times 128 \times 131{,}072 \times 1\text{ byte} \approx 21.47\text{ GB}.$$
  Because $21.47\text{ GB} > 10\text{ GB}$, maximum single-GPU concurrency is $B_{\max} = \lfloor 10 / 21.47 \rfloor = 0$. A single 128k agent stream cannot even be launched on the GPU without triggering an out-of-memory fault!
- **Under Multi-Head Latent Attention (MLA, $d_c = 512, d_h^R = 64, L = 60$):** A single 128k stream in FP8 demands:
  $$M_{\text{KV, MLA}} = 60 \times (512 + 64) \times 131{,}072 \times 1\text{ byte} \approx 4.53\text{ GB}.$$
  The maximum concurrency becomes:
  $$B_{\max} = \lfloor 10 / 4.53 \rfloor = 2\text{ concurrent streams}.$$
- **Across a dual-GPU node (160 GB total HBM, $90\text{ GB}$ free memory):**
  $$\text{GQA: } B_{\max} = \lfloor 90 / 21.47 \rfloor = 4\text{ concurrent streams}.$$
  $$\text{MLA: } B_{\max} = \lfloor 90 / 4.53 \rfloor = 19\text{ concurrent streams!}$$
