...
The systems impact of these compressed attention mechanisms is formalized through the **Accelerator Roofline Model** (@williams2009). On an accelerator with peak floating-point throughput $P_{\text{peak}}$ and memory bandwidth $B_{\text{mem}}$, operational throughput is bounded by the minimum of the compute ceiling and the memory streaming ceiling: $P_{\text{achieved}} = \min(P_{\text{peak}}, \mathcal{I} \cdot B_{\text{mem}})$.
