...
As training continues, average response length scales autonomously. The model discovers that spending test-time compute (inference tokens) directly increases its probability of receiving a verified reward. Nothing in the reward function asks for longer responses, which is what makes the coupling in @fig-vol3-rlvr-emergent-reasoning notable: pass rate and reasoning length rise through the same transition, so the model is discovering that inference-time compute is purchasable accuracy.

::: {#fig-vol3-rlvr-emergent-reasoning fig-env="figure" fig-pos="htb" fig-cap="**Emergent Reasoning under Pure RLVR**: Pass rate and average reasoning-token count over ten thousand reinforcement learning steps. Both rise through the same sigmoid transition between roughly two thousand and five thousand steps, evidence that the policy is buying accuracy with test-time compute rather than being instructed to deliberate." fig-alt="Dual-axis chart over ten thousand reinforcement learning training steps showing pass rate and average reasoning token count. Both rise together through a sharp sigmoid transition between two thousand and five thousand steps, then saturate."}
...
# Load data
data = pd.read_csv("vol3/reinforcement_learning/data/rlvr_emergent_reasoning.csv")
