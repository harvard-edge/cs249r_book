...
## Adversarial Threat Models {#sec-vol3-sandboxes-threat-model}
...
An agent runtime has no such bit to check. The model reads system directives, tool schemas, and a web page it fetched a moment ago as one sequence of tokens, and nothing in attention marks which of those tokens may issue commands. Whatever an attacker wrote into that page, or whatever the model misread, reaches the host as a well-formed tool call. @Sec-vol3-tool-calling made that call typed, authorized, and settled. It did not limit what the code behind the call can touch once it runs. In the terms of @sec-vol3-intro-hsa, that chapter bounded the authority a call names, not the authority its code carries. This chapter builds that limit beneath the model, from the threat it must stop to the start-up latency and memory it costs.
...
Because the 128-token exploit payload shares the same 12.5 percent attention footprint and directly attends to the preceding system prompt tokens, it shifts the softmax probabilities of downstream generated tokens toward tool actuation. The attention mechanism provides no hardware bit to isolate this 12.5 percent slice of memory from modifying the generation logits.
...
## In-Process Sandbox Failures {#sec-vol3-sandboxes-in-process-failure}
...
Confusing language-level variable scoping with hardware-enforced protection domains defeats complete mediation (@sec-vol3-tool-calling-unix-analogy). An in-process barrier can never mediate access to the memory space it inhabits.
:::
...
An in-process interpreter cannot provide the complete mediation of @sec-vol3-tool-calling-unix-analogy, because its policy enforcement code shares a failure domain with the code it evaluates. A script that overwrites an instruction pointer or walks an object reference chain compromises the reference monitor itself.
...
## Capability-Based Privilege Attenuation {#sec-vol3-sandboxes-least-privilege}
...
## MicroVM Kernel Isolation {#sec-vol3-sandboxes-microvms}
...
## WebAssembly Sandboxing {#sec-vol3-sandboxes-wasm}
...
## Copy-on-Write Filesystem Overlays {#sec-vol3-sandboxes-cow}
...
## Network Egress Firewalls {#sec-vol3-sandboxes-network}
...
## Pre-Warmed Sandbox Pooling {#sec-vol3-sandboxes-pooling}
...
### Memory snapshot re-hydration {#sec-vol3-sandboxes-pooling-snapshots}
...
### The reset contract: Ephemeral destruction vs. verified scrubbing {#sec-vol3-sandboxes-pooling-reset}
...
**Latency Saving:** The pre-warmed pool eliminates nearly 20 seconds of pure virtualization overhead across a single 40-turn agent session, accelerating task completion by 24.8 percent.
:::
...
Across @sec-vol3-tool-calling and this chapter, Part III established the execution substrate of an agentic computer system: typed tool interfaces, capability discovery via the Model Context Protocol (MCP), and streaming observation buffers in @sec-vol3-tool-calling, followed by microVM hypervisors, capability attenuation, copy-on-write storage, and network egress firewalls here in this chapter.
