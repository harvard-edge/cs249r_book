...
Unlike stateless inference serving where input prompts are brief and independent, an autonomous agent accumulates state over extended multi-step trajectories. As an agent inspects files, runs shell commands, parses compiler diagnostics, and reflects on intermediate errors, its prompt history expands monotonically. Developers and framework builders frequently treat modern long-context windows (128k to 1M+ tokens) as elastic, unbounded repositories. The hierarchy analogy holds only if the top tier is managed like one. @Fig-context-length-scaling shows four orders of magnitude of growth in raw window size, and an agent that appends to that window without eviction is running a cache with no replacement policy.

::: {#fig-context-length-scaling fig-env="figure" fig-pos="htb" fig-cap="**The Context Length Explosion (2017--2024)**: Context window length by release date, from 512 tokens in the original Transformer to over two million in Gemini 1.5 Pro. The capacity exists; treating it as an undifferentiated flat buffer rather than a managed hierarchy is what turns that capacity into a liability." fig-alt="Log-scale scatter of model context window length in tokens against release date from 2017 to 2024, colored by organization, rising from hundreds of tokens to two million."}
...
data = pd.read_csv("vol3/context_working_sets/data/context_length_scaling.csv")
...
_groups = list(dict.fromkeys(data['Organization']))
_cmap = plt.get_cmap('viridis', max(len(_groups), 2))
for _i, _g in enumerate(_groups):
    _sub = data[data['Organization'] == _g]
    ax.scatter(_sub['Date'], _sub['ContextLength'], color=_cmap(_i), label=str(_g), s=100, alpha=0.8, edgecolor='w')

for i, row in data.iterrows():
    ax.annotate(row['Model'], (mdates.date2num(row['Date']), row['ContextLength']), xytext=(5, 5), textcoords='offset points', fontsize=8, alpha=0.8)

ax.set_yscale('log', base=2)
ax.set_ylabel('Context Window Length (Tokens)', fontsize=10)
ax.set_xlabel('Release Date', fontsize=10)

ax.xaxis.set_major_locator(YearLocator())
ax.xaxis.set_major_formatter(DateFormatter('%Y'))

plt.legend(title='Organization', loc='upper left', bbox_to_anchor=(1, 1), fontsize=8, title_fontsize=9)
...
In production systems engineering, this append-only approach precipitates three severe failures:
