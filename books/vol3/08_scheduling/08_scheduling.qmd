...
#### Phase contention
When a monolithic continuous batching engine attempts to schedule an incoming prefill request alongside active decode sequences in the same iteration, a destructive phenomenon known as **phase contention**\index{Phase contention} (or phase interference) occurs:

```
Iteration Timeline: Monolithic Co-Located Continuous Batching
Time (ms) 0           50          100         150         200         250
Decode:   |--Tk 1--|--Tk 2--|...................................|--Tk 3--|
                            ^                                   ^
                            |=== MONOLITHIC PREFILL (64k) ======|
                            |   Tensor Cores Monopolized        |
                            |   Decode Iterations Stalled       |
                            |   ITL Jitter = +200 ms Spike      |
```
...
By fixing $T_{\text{budget}}$ such that the total execution time of the hybrid batch matches the target Service-Level Objective for Inter-Token Latency ($\Delta t_{\text{iter}} \le SLO_{\text{ITL}}$), the runtime guarantees bounded latency jitter for active decode streams while steadily advancing prefill computation.

```
Iteration Timeline: Chunked Prefill Piggybacking
Time (ms) 0           25          50          75          100
Batch:    | Chunk 1 | Chunk 2 | Chunk 3 | Chunk 4 |
          | +Decodes| +Decodes| +Decodes| +Decodes|
Decode:   |--Tk 1---|--Tk 2---|--Tk 3---|--Tk 4---| (Continuous, Stable ITL)
Prefill:  [==25%===][==50%===][==75%===][==100%==] -> Prompt Ready for Generation
```
...
Table 8.1 (@tbl-scheduling-batching-comparison) summarizes the trade-offs across batching architectures.
...
If the network transmission time of layer $l$'s KV cache is strictly less than or equal to the computation time of layer $l+1$'s forward pass:
...
Table 8.2 (@tbl-vol3-pcb-vs-acb) details the structural duality between classical POSIX processes and agent trajectories.
...
Table 8.3 (@tbl-drf-allocation-matrix) illustrates a numerical allocation across two competing agent classes on a cluster consisting of 8 H100 GPUs ($640\text{ GB}$ HBM, $8000\text{ TFLOPS}$) and $1024\text{ GB}$ host DRAM:
...
|:----------------------------|:----------------------------------------------------------------|:------------------|:-----------------------------|:------------------------------------|
| **Class A: Coding Agents**  | $[15\text{ GB HBM}, \, 40\text{ TFLOPS}, \, 32\text{ GB DRAM}]$ | Host DRAM         | $\frac{32}{1024} = 3.125\%$  | **20 instances** ($s_A^* = 62.5\%$) |
| **Class B: Reasoning Bots** | $[4\text{ GB HBM}, \, 250\text{ TFLOPS}, \, 8\text{ GB DRAM}]$  | Compute (TFLOPS)  | $\frac{250}{8000} = 3.125\%$ | **20 instances** ($s_B^* = 62.5\%$) |

: **Dominant Resource Fairness Multi-Resource Allocation**: Balancing high-memory coding agents against high-compute reasoning agents across heterogeneous hardware tiers. Dominant shares are equalized, maximizing aggregate cluster throughput. {#tbl-drf-allocation-matrix tbl-colwidths="[20,30,20,15,15]"}
...
Apply the scheduling formulations, Roofline models, and queuing equations derived in this chapter to solve the following quantitative problems.

### Problem 8.1: Chunked prefill budgeting and inter-token latency jitter
An inference serving cluster hosts a 70-billion parameter model with Grouped-Query Attention ($L = 80$ layers, $d_{\text{model}} = 8192$, $H_Q = 64$ query heads, $H_{\text{KV}} = 8$ key-value heads, $d_h = 128$) on an NVIDIA H100 SXM5 GPU ($P_{\text{peak}} = 989\text{ TFLOPS}$ dense FP16, $B_{\text{mem}} = 3.35\text{ TB/s}$). Numerical precision is 16-bit half precision ($2\text{ bytes/element}$).

The node currently serves an active batch of $B_{\text{dec}} = 32$ decoding agents. The historical average context length across these agents is $S_{\text{avg}} = 8192$ tokens. The target Service-Level Objective for Inter-Token Latency is $SLO_{\text{ITL}} \le 30\text{ ms}$.

A new autonomous agent arrives with an expansive repository context prompt of length $S_{\text{prompt}} = 32{,}768$ tokens.

1. Calculate the operational intensity $\mathcal{I}_{\text{decode}}$ of the current 32-sequence decode batch. Determine whether the iteration is compute-bound or memory-bound, and calculate the baseline decode iteration execution duration $\Delta t_{\text{decode}}$.
2. If the scheduler naively executes the incoming $32\text{k}$-token prefill monolithically in the next continuous iteration batch, calculate the prefill forward-pass duration $\Delta t_{\text{prefill}}$ (assume $P_{\text{eff}} = 0.60 \times P_{\text{peak}}$ sustained efficiency). What is the resulting Inter-Token Latency jitter observed by the 32 decoding agents?
3. Using the Roofline formulation from @eq-chunk-iter-time, calculate the maximum prefill chunk size $C^*$ that can be piggybacked alongside the 32 decode tokens in each iteration such that total iteration duration does not exceed the $30\text{ ms}$ SLA ceiling.
4. For the optimal chunk size $C^*$ derived in part (3), calculate the total number of iterations $K$ required to complete the prompt prefill, and compute the resulting Time-To-First-Token (TTFT) for the incoming agent.

### Problem 8.2: Disaggregated serving and the RDMA KV streaming invariant
An enterprise infrastructure architect designs a physically disaggregated serving cluster to host Llama-3-70B ($L = 80, H_{\text{KV}} = 8, d_h = 128, b_{\text{bytes}} = 2$). The system separates dedicated Prefill Workers ($P$-nodes running Tensor Parallelism $TP_P = 8$) from Decode Workers ($D$-nodes running $TP_D = 2$).

An incoming agent trajectory submits a prompt of length $S = 65{,}536$ tokens ($64\text{k}$).

1. Calculate the exact memory footprint (in gigabytes) of the Key-Value cache generated by the prefill phase for this sequence.
2. Calculate the raw, unpipelined network transmission duration $t_{\text{transfer}}$ of this KV cache from the $P$-node to the $D$-node across three candidate data center fabrics:
   - (a) 100 Gbps Ethernet ($B_{\text{net}} = 12.5\text{ GB/s}$)
   - (b) 400 Gbps RoCEv2 ($B_{\text{net}} = 50.0\text{ GB/s}$)
   - (c) 800 Gbps InfiniBand NDR ($B_{\text{net}} = 100.0\text{ GB/s}$)
3. Assume the $P$-node evaluates the 80 transformer layers at an effective sustained throughput of $P_{\text{eff}} = 4.8\text{ PFLOPS}$ across its 8-GPU tensor-parallel domain. Calculate the per-layer forward-pass computation duration $\tau_{\text{compute}}(l)$.
4. Formulate the layer-by-layer KV streaming condition (@eq-kv-streaming-invariant). Calculate the minimum network bandwidth $B_{\text{RDMA}}^*$ required to guarantee that the transmission of layer $l$'s KV cache is completely hidden behind the forward-pass compute of layer $l+1$. For a 400 Gbps interconnect, what is the net visible transfer overhead $\Delta t_{\text{visible}}$ added to the request's TTFT?

### Problem 8.3: Breakeven analysis for preemptive context swapping
An autonomous coding agent runs on a serving node equipped with an NVIDIA H100 GPU connected to a host server via a PCIe Gen5 x16 interface. The effective unidirectional DMA bandwidth across the PCIe bus is $B_{\text{PCIe}} = 55\text{ GB/s}$, and DMA setup latency is $\delta_{\text{setup}} = 3.5\text{ ms}$.

The agent operates on a 70B model with a context length of $S = 48\text{k} = 49{,}152$ tokens ($S_{\text{KV}} = 16.11\text{ GB}$). At step 12, the agent emits an external tool call to execute a test suite:
`bash(pytest tests/unit/ --maxfail=1)`

1. Calculate the round-trip context migration duration $T_{\text{swap\_overhead}}$ required to swap the agent's PagedAttention KV blocks from GPU HBM to Host DDR5 DRAM and subsequently restore them back to HBM upon tool completion.
2. Assuming a scheduling safety margin $\Delta t_{\text{slack}} = 150\text{ ms}$, calculate the analytical breakeven suspension duration $T_{\text{breakeven}}$. If the test suite executes in $\tau_{\text{tool}} = 18\text{ seconds}$, calculate the net memory-time product (in gigabyte-seconds) reclaimed in accelerator HBM by swapping versus pinning.
3. If the host server lacks spare DDR5 DRAM, the memory manager must choose between pinning the context in HBM or discarding it completely and recomputing the prompt from scratch upon tool return. Assuming the GPU executes prefill at an effective sustained throughput of $420\text{ TFLOPS}$, calculate the recomputation duration $T_{\text{recompute}}$. What is the critical tool wait threshold $T_{\text{crit}}$ above which recomputing is cheaper in opportunity cost than pinning?
