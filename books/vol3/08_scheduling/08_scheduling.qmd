...
In the prefill phase, the model ingests an entire prompt sequence of length $S$ tokens in a single forward pass. Because all $S$ tokens are known simultaneously, the matrix-vector products across linear projections ($W_Q, W_K, W_V, W_O$ and the feed-forward blocks $W_{\text{gate}}, W_{\text{up}}, W_{\text{down}}$) are batched across all $S$ positions into General Matrix Multiplies (GEMMs). Weights are read from HBM once ($2 N_{\text{params}}$ bytes in 16-bit precision), amortizing memory transfers across all $S$ tokens. Operational intensity scales directly with sequence length in @eq-vol3-scheduling-prefill-intensity:

$$
\mathcal{I}_{\text{prefill}}(S) \approx S \quad \left[ \frac{\text{FLOP}}{\text{byte}} \right]
...
where $M_{\text{KV}} = 4 L H_{\text{KV}} d_h \sum_{i=1}^{B_{\text{dec}}} S_i$ bytes.[^fn-scheduling-intensity-divergence] Because $M_{\text{KV}} \ge 0$, decode operational intensity is fundamentally capped by batch size: $\mathcal{I}_{\text{decode}} \le B_{\text{dec}}\text{ FLOP/byte}$. For typical serving batch sizes ($B_{\text{dec}} \in [1, 64]$), decode operational intensity satisfies $\mathcal{I}_{\text{decode}} \le 64\text{ FLOP/byte} \ll 295.2\text{ FLOP/byte}$. Autoregressive decoding is severely **memory-bandwidth bound**, stranding over 80 to 90 percent of peak Tensor Core capacity idle awaiting HBM weight streaming.
...
Notice the duality in @eq-chunk-iter-time: reading model weights requires a baseline memory transfer time $t_{\text{weights}} = 2 N_{\text{params}} / B_{\text{mem}}$ regardless of whether $C=0$ or $C>0$. For a 70B parameter model on an H100 ($B_{\text{mem}} = 3.35\text{ TB/s}$), streaming weights across the memory bus takes $t_{\text{weights}} = (2 \times 70 \times 10^9) / (3.35 \times 10^{12}) \approx 41.8\text{ ms}$.

During this $41.8\text{ ms}$ memory-read interval, the Tensor Cores are largely idle. Schedulers can "piggyback" prefill tokens $C$ into the computation without increasing step latency until floating-point execution time matches memory fetch time. The critical chunk threshold $C^*$ at which the iteration transitions from memory-bound to compute-bound is determined by @eq-vol3-scheduling-optimal-chunk:
...
where $\mathcal{I}_{\text{knee}} = P_{\text{peak}} / B_{\text{mem}}$ is the machine balance.[^fn-scheduling-chunk-derivation]
...
By the classical Pollaczek-Khinchine formula [@kleinrock1975queueing], the expected waiting time in the queue under a First-In-First-Out (FIFO) discipline is governed by @eq-pollaczek-khinchine:

$$
\mathbb{E}[W_{\text{FIFO}}] = \frac{\lambda \mathbb{E}[X^2]}{2(1 - \rho)}
$$ {#eq-pollaczek-khinchine}

Under @eq-pollaczek-khinchine, expected waiting time scales linearly with the second moment of service time. When $\alpha \le 2$, $\mathbb{E}[X^2] \to \infty$, causing the expected FIFO queue delay to diverge to infinity ($\mathbb{E}[W_{\text{FIFO}}] \to \infty$) regardless of how low the nominal server utilization $\rho$ is provisioned.

This result is not a mere theoretical curiosity; it is the mathematical explanation for real-world cluster freezes. Under FIFO, whenever a long-running 300-step agent enters the server, it blocks the head of the queue. Because variance is unbounded, long-horizon tasks frequently monopolize accelerator compute, causing the waiting queue to back up indefinitely and driving average waiting times toward infinity, even when server utilization appears healthy ($\rho \approx 0.6$). FIFO is structurally incapable of serving heavy-tailed agent workloads.
...
[^fn-scheduling-pareto-variance]: For a continuous Pareto distribution with probability density function $f(x) = \alpha x_m^\alpha x^{-(\alpha+1)}$, the second raw moment evaluates to $\int_{x_m}^\infty x^2 f(x) dx = [\alpha x_m^\alpha / (2 - \alpha)] [x^{2 - \alpha}]_{x_m}^\infty$, which diverges whenever the tail index satisfies $\alpha \le 2$.
...
The load penalty exponent $\gamma$ creates steep non-linear resistance as a node approaches memory saturation ($L_j \to L_{\text{max}}$), preventing queue hot-spotting and avoiding cache thrashing.
