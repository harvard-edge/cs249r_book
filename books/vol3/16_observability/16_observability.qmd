...
For example, a scheduler initializes a task with base budget $T_{\text{base}} = 120\text{ s}$. When the agent issues `cargo test --workspace`, the runtime inspects the tool registry and dynamically credits $T_{\text{tool\_allowance}} = +180\text{ s}$, extending the deadline to accommodate legitimate compiler and test suite latency. Conversely, if the agent executes three identical failing bash commands consecutively, or if token generation entropy drops below an anomalous threshold (signaling that the model has collapsed into an infinite conversational apology loop), the scheduler levies a penalty $\Delta T_{\text{penalty}} = 60\text{ s}$. This early-termination heuristic traps unproductive loops within seconds rather than stranding GPU memory for the remainder of a static timeout.
