...
2. **Kernel-Space eBPF Probes:** To achieve tamper-proof, zero-bypass observability across sandboxed tool executions, modern runtimes deploy extended Berkeley Packet Filter (eBPF) probes attached to kernel tracepoints (`sys_enter_execve`, `sys_enter_connect`, `sys_enter_openat`, `sys_enter_kill`) within the agent's Linux cgroup and network namespace. In-kernel eBPF bytecode is verified for memory safety and termination by the Linux kernel verifier prior to execution, guaranteeing that **eBPF probe overhead remains strictly below 1 percent** ($<1\%$) of host CPU cycles. When an agent attempts an unauthorized network socket connection or reads a sensitive filesystem path, the eBPF probe intercepts the operation at the kernel boundary before the syscall completes.
...
Before exploring distributed tracing topologies, verify your understanding of trajectory telemetry architecture:

- [ ] **Semantic vs. syntactic failure**: explain why a failed agent trajectory frequently exits with an HTTP 200 OK status code, blinding traditional APM dashboards.
- [ ] **Payload bloat mitigations**: explain why a 50-step agent trajectory generating 100 MB of context payloads requires separating scalar span metadata from compressed blob storage.
- [ ] **Telemetry collection overhead**: contrast user-space serialization CPU tax (15--25 percent) with kernel eBPF probe overhead (<1 percent) and ring buffer memory bandwidth.
...
## Distributed Spans, Causal Tracing, and Token Attribution {#sec-vol3-observability-tracing}
...
For instance, consider a Monte Carlo Tree Search exploring branching factor $B=4$ across depth 3 ($4^3 = 64$ leaf rollouts). If 63 candidate rollouts are pruned and only 1 winning trajectory $\tau^*$ of 15 steps is committed, the total cumulative expenditure might reach $C_{\text{cumulative}}(\text{root}) \approx \$12.50$, while the winning path consumed only $C_{\text{winning}}(\tau^*) \approx \$1.62$. The token efficiency is:

$$\eta_{\text{token}} = \frac{\$1.62}{\$12.50} \approx 0.13 \quad (13\%)$$

Over 87 percent of the financial expenditure was consumed by exploratory scaffolding. In unguided multi-agent swarms or deep search topologies ($B \ge 16$), $\eta_{\text{token}}$ frequently drops below $0.10$. Tracking $\eta_{\text{token}}$ via distributed trace spans provides the primary systems signal for identifying runaway deliberation and un-converged agent loops.
...
- **Anomalies and failures** ($4\%$ failure rate $\implies 400$ trajectories/day) are retained at $100\%$, generating $400 \times 25 \times 2\text{ KB} = 20\text{ MB/day}$ of index spans (with their compressed payloads stored in object storage).
- **Routine nominal successes** ($96\% \implies 9{,}600$ trajectories/day) are downsampled at $\beta = 0.01$ (retaining 96 runs), generating $96 \times 25 \times 2\text{ KB} \approx 4.8\text{ MB/day}$.

The active distributed tracing index stores less than $25\text{ MB/day}$—a **$95\%$ reduction** compared to full metadata ingestion—slashing durable trace indexing I/O throughput into ClickHouse or Jaeger to less than $300\text{ bytes/s}$ of sustained write bandwidth while guaranteeing that 100 percent of production incidents, security policy trips, and cost overruns are preserved with complete causal fidelity.
...
## Hermetic Evaluation Gyms and Benchmark Drift {#sec-vol3-observability-eval-gyms}
...
- **Pass@1 ($k=1$):** $1 - \frac{8}{10} = 0.20 \quad (20.0\%)$.
- **Pass@3 ($k=3$):** The chance that all 3 draws fail is $\frac{\binom{8}{3}}{\binom{10}{3}} = \frac{56}{120} \approx 0.467$. Thus, $\text{Pass}@3 = 1 - 0.467 = 0.533 \quad (53.3\%)$.
- **Pass@5 ($k=5$):** The chance that all 5 draws fail is $\frac{\binom{8}{5}}{\binom{10}{5}} = \frac{56}{252} \approx 0.222$. Thus, $\text{Pass}@5 = 1 - 0.222 = 0.778 \quad (77.8\%)$.
...
where $\bar{p} = (p_1 + p_2) / 2$, $z_{\alpha/2} = 1.96$ for a 95 percent confidence level, and $z_\beta = 0.84$ for 80 percent power. For an agent baseline pass rate of $p_1 = 0.30$ and a targeted capability shift of $5\%$ ($p_2 = 0.35$), @eq-vol3-sample-size dictates a minimum sample size of $N \approx 1{,}430$ independent task evaluations.
...
When engineering teams attempt to cut CI/CD costs by evaluating on only $N = 30$ tasks, a measured gain of $+6.6\%$ (e.g., solving 16 tasks instead of 14) has a $95\%$ confidence interval spanning from $34\%$ to $72\%$. The observed "improvement" is pure statistical noise. Reliable evaluation demands either budgeting for full sample sizes or employing sequential testing to detect regressions dynamically.[^fn-observability-twoprop-montecarlo] Benchmarking suites evaluating on fewer than 200 tasks cannot reliably detect a $5\%$ regression, routinely passing degraded models into production.
...
## Online Quality Scoring, Shadow Deployments, and Canary Routing {#sec-vol3-observability-canary}
...
To see the mechanics of this sequential test in production, consider concrete operational parameters: let baseline acceptable failure rate be $p_0 = 0.05$ ($5\%$) and unacceptable degraded rate be $p_1 = 0.15$ ($15\%$), with $\alpha = 0.01$ and $\beta = 0.05$:
...
However, if a candidate checkpoint suffers an undiscovered regression resulting in a $40\%$ failure rate, and encounters 5 failures across its first 8 production tasks ($k = 5, m - k = 3$):

$$
\ln \Lambda_8 = 5(+1.099) + 3(-0.111) = 5.495 - 0.333 = +5.162 \ge 4.55
$$

The cumulative log-likelihood ratio breaches the abort threshold $\ln A$ on the eighth execution! The sequential canary routing decision rule operates continuously as follows:
...
## Fleet-Wide SRE Metrics: Goodput, Trajectory Timeouts, and Error Budgets {#sec-vol3-observability-sre-metrics}
...
- **Option A (Compact Model, $P_{\text{active}} = 8\text{B}$):** Token pricing is $\$0.20/\text{1M tokens}$, yielding a raw execution cost of $20{,}000 \times \$0.20 \times 10^{-6} = \$0.004$ per attempt. If the standalone single-trajectory pass rate is $p = 0.40$ ($40\%$), the CPVG is:
  $$\text{CPVG}_A = \frac{\$0.004}{0.40} = \$0.010 \quad (\text{1.0 cent per verified goal})$$
- **Option B (Frontier Model, $P_{\text{active}} = 70\text{B}$):** Token pricing is $\$3.00/\text{1M tokens}$, costing $20{,}000 \times \$3.00 \times 10^{-6} = \$0.060$ per attempt. With a higher single-trajectory pass rate of $p = 0.65$ ($65\%$), the CPVG is:
  $$\text{CPVG}_B = \frac{\$0.060}{0.65} \approx \$0.092 \quad (\text{9.2 cents per verified goal})$$

Even though the frontier model is significantly more capable ($65\%$ vs $40\%$), its CPVG is **$9.2\times$ higher**! The compact model delivers nearly an order of magnitude higher Economic Goodput ($G_{\$}$).

Furthermore, consider applying Best-of-3 parallel sampling with the compact model: the probability that at least one of 3 independent rollouts succeeds is $1 - (1 - 0.40)^3 = 1 - 0.216 = 0.784$ ($78.4\%$). The total compute cost is $3 \times \$0.004 = \$0.012$. The resulting CPVG is:

$$\text{CPVG}_{A,\text{Best-of-3}} = \frac{\$0.012}{0.784} \approx \$0.0153 \quad (\text{1.5 cents per verified goal})$$

Best-of-3 on the compact model delivers **higher reliability than the single frontier model** ($78.4\%$ vs $65.0\%$) at **one-sixth the cost** ($\$0.015$ vs $\$0.092$). This trade-off is the exact mechanism driving the Pareto frontier in @fig-vol3-goodput-pareto.
...
Before analyzing latency decomposition and Amdahl's Law, test your grasp of agent SRE economics:

- [ ] **Trajectory goodput vs. token throughput**: explain why raw token throughput is a misleading health metric for an autonomous agent fleet experiencing conversational thrashing.
- [ ] **Cost-per-verified-goal (CPVG)**: calculate whether Model A (40 percent pass rate at \$0.004/attempt) or Model B (65% pass rate at \$0.060/attempt) delivers higher economic efficiency.
- [ ] **Best-of-$N$ Pareto leverage**: demonstrate how parallel sampling on a compact model can surpass both the accuracy and cost efficiency of a single frontier model invocation.
...
In classical site reliability engineering, service-level objectives (SLOs) are defined over simple uptime ratios (e.g., $99.9\%$ of HTTP requests return status 200 within $200\text{ ms}$). In autonomous agent systems, SLOs must be redefined over **multi-turn semantic validity** using service level indicators (SLIs):
...
Because agent execution is stochastic, traditional single-threshold alerting triggers intolerable alert fatigue: a temporary cluster dip to $87\%$ accuracy across 10 tasks is frequently a statistical artifact. Furthermore, relying on traditional Prometheus counter scraping intervals ($15\text{--}30\text{ s}$) blinds SRE dashboards to micro-burst conversational thrashing loops that ignite, burn through error budgets, and collapse in single-digit seconds between scrape pulls. Autonomous SRE runtimes therefore pair push-based OpenTelemetry metric streaming with sliding-window aggregators to implement **Multi-Window Multi-Burn-Rate Alerting**: tracking error budget consumption across short-term ($1\text{ hr}$) and long-term ($24\text{ hr}$) rolling windows.

To see why burn-rate alerting is essential, consider the physical arithmetic: over a standard 30-day billing month ($720\text{ hours}$), an SLO target of $90\%$ ($\text{SLI}_{\text{res}} \ge 0.90$) allows a total error budget of $10\%$. A $1\times$ burn rate consumes the budget evenly over 720 hours ($10\% / 720 \approx 0.0139\%$ of budget per hour). A **$14.4\times$ burn rate** accelerates failure consumption to $14.4 \times (1 / 720) = 2.0\%$ of the monthly budget consumed in just a single hour. At that burn velocity, the organization's entire monthly error allowance will be completely depleted in only $720 / 14.4 = 50\text{ hours}$ (roughly two days).
...
* **Statistical power and sequential canary routing**: Small benchmark subsets ($N \le 30$) lack statistical power ($<20\%$) to detect a $5\%$ capability regression. In production deployments, canary routing must abandon fixed-batch testing in favor of Wald's Sequential Probability Ratio Test (SPRT), evaluating log-likelihood ratios after every trajectory to abort degraded policies within minimal sample horizons.
