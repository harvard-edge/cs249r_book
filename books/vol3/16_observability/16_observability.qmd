...
::: {#psp-sprt-circuit-breaker .callout-perspective title="Sequential probability ratio test gating"}
...
As formalized in Systems Perspective \ref{psp-sprt-circuit-breaker}, the automated circuit breaker must operate with minimal Mean Time to Detect (MTTD) and Mean Time to Recovery (MTTR), adhering to the tail-tolerance principles articulated by @dean2013tail. Beyond sequential hypothesis tests on aggregate task success, the runtime enforces invariant-based circuit breakers that trip instantly upon observing hard safety violations:
