...
::: {.column-margin}
![](images/svg/vol3_observability_margin_004.svg){width="100%" fig-alt="Amdahl speedup curves showing overall agent speedup capped at 1.54x when accelerating model inference alone, versus 1.73x when optimizing system scaffolding tools."}

*Because foundation model inference accounts for only 35% of trajectory duration, accelerating host scaffolding tools yields higher end-to-end speedups than model optimization alone.*
:::
...
::: {#exmp-16-evaluating-the-four-layer-contract-on .callout-example title="Evaluating the Four-Layer Task Contract"}
...
::: {#exmp-16-storage-io-and-reset-amortization .callout-example title="Storage I/O and Reset Amortization"}
...
*Evaluating on fewer than 200 tasks produces wide confidence intervals (plus minus 16%) that obscure true model regressions behind statistical noise.*
:::
...
*Asynchronous payload offloading and tail-based sampling compress active trajectory telemetry indexes from 100 MB down to 25 KB, averting gRPC frame exhaustion while preserving 100% of production failure traces.*
:::
...
#### Model Invocation Spans (`gen_ai.client` / `gen_ai.server`) {.unnumbered}
...
::: {#exmp-16-critical-path-latency-and-bottleneck .callout-example title="Critical Path Latency Decomposition"}
...
::: {#exmp-16-telemetry-ingestion-inverted-economics .callout-example title="Telemetry Ingestion Inverted Economics"}
...
*Wald sequential probability ratio testing detects degraded candidate policies within 8 turns, cutting canary rollout blast radius by 80%.*
:::
...
::: {#callout-def-vol3-sprt-circuit-breaker .callout-tip title="Definition 16.1: Sequential Probability Ratio Test Gating"}
...
As formalized in Definition 16.1, the automated circuit breaker must operate with minimal Mean Time to Detect (MTTD) and Mean Time to Recovery (MTTR), adhering to the tail-tolerance principles articulated by Dean and Ghemawat (2013). Beyond sequential hypothesis tests on aggregate task success, the runtime enforces invariant-based circuit breakers that trip instantly upon observing hard safety violations:
...
::: {#exmp-16-sizing-canary-traffic-and-circuit .callout-example title="Sizing Canary Traffic and Circuit Breaker MTTR"}
**Problem Formulation**
...
#### Operational Goodput Criterion ($\mathcal{C}_{\text{goodput}}$) {.unnumbered}
...
::: {#exmp-16-automated-canary-verification-and-release .callout-example title="Canary Verification and Release Gates"}
