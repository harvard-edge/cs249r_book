...
## Architectural Overview: Trajectory Observability and Evaluation Systems {#sec-vol3-observability-intro}

Deploying autonomous agents and multi-agent swarms into mission-critical infrastructure (@sec-vol3-multiagent) introduces an unprecedented observability and reliability crisis. In classical microservice architectures, software components are deterministic binaries whose failures are syntactic: threads segfault, exceptions emit stack traces, and network partitions trigger TCP resets or HTTP 5xx status codes. Standard Application Performance Monitoring (APM) tools (such as Prometheus, Grafana, and Jaeger) monitor these systems effectively by scraping time-series counters and aggregating request-level spans.

In an agentic systems regime, however, execution is driven by an autoregressive neural policy $\pi_\theta(a_t \mid s_t)$ interacting with an open, mutating environment across dozens of discrete turns. When an agent experiences goal drift, enters an infinite reasoning loop, or proposes an unauthorized destructive mutation, it does not crash: it emits syntactically valid JSON, communicates over authenticated TLS sockets, and exits cleanly with an `HTTP 200 OK` status. This is the **Fail-Plausible fault model**: the system appears entirely healthy according to every structural metric of classical infrastructure monitoring while completely failing its semantic objective.

Furthermore, traditional pull-based metric scraping (such as Prometheus polling `/metrics` every 15 to 30 seconds) is physically incapable of capturing fast agent failure cascades. An agent stuck in a tight tool-retry loop can burn hundreds of thousands of tokens, execute destructive shell commands, and terminate within a four-second window—occurring entirely between two Prometheus scrape boundaries. Bridging this observability gap requires architecting an out-of-band, hierarchical telemetry and evaluation control plane.

This chapter develops the systems architecture for trajectory observability, tracing, and statistical evaluation. We first implement the OpenTelemetry GenAI semantic conventions, structuring flat logs into hierarchical execution DAGs (`agent.trajectory` $\to$ `agent.step` $\to$ `tool.execution`). We then design out-of-band telemetry offloading pipelines using kernel-space eBPF probes and lockless ring buffers to prevent multi-megabyte prompt strings from blowing up span buffers. Next, we build hermetic evaluation gyms that enforce Copy-on-Write isolation and network air-gapping to run reproducible benchmarks. Finally, we formalize canary trajectory routing using Sequential Probability Ratio Testing (SPRT) and establish production SRE metrics—specifically Trajectory Goodput ($G$) and Cost-per-Verified-Goal (CPVG)—to govern continuous agent deployment.

## OpenTelemetry Standards for Agent Trajectories {#sec-vol3-observability-otel}

The fundamental challenge in monitoring autonomous agent systems is the **Observability Gap**: the disconnect between classical infrastructure health signals and semantic execution correctness. In microservice architectures, software failures are syntactic: threads segfault, unhandled exceptions trigger stack traces, RPC calls return nonzero exit codes, and network partitions manifest as TCP connection timeouts. Furthermore, traditional Application Performance Monitoring (APM) relies on pull-based metric scrapers (such as Prometheus polling `/metrics` endpoints at 15-second or 30-second scraping intervals). In an agentic loop, however, an entire catastrophic retry sequence can execute, burn 100,000 tokens across 15 rapid turns, and corrupt database state in under four seconds—occurring entirely within the blind window between two discrete Prometheus scrape intervals. The scraping daemon computes a time-averaged rate over a multi-minute window ($\text{rate}[5m]$), smoothing away the brief burst and rendering the incident completely invisible on operational dashboards.

In an agentic systems regime, the execution unit is not a deterministic binary executing machine instructions; it is an autoregressive neural policy $\pi_\theta(a_t \mid s_t)$ generating actions conditioned on contextual token distributions. A policy that hallucinates an invalid state transition, enters a circular reasoning loop, or executes an unauthorized mutation emits syntactically valid JSON, communicates over authorized TLS sockets, and exits cleanly. This is the **Fail-Plausible fault model**: the system appears operational according to every structural metric of traditional operating systems while completely failing its semantic objective. As summarized in @tbl-vol3-apm-failure-contrast, traditional APM is fundamentally blind to semantic collapse, reporting nominal uptime while catastrophic mutations unfold unnoticed.

| Telemetry Dimension                | Traditional Microservice APM (Blind to Semantic Failure)                 | Hierarchical Trajectory Observability (OpenTelemetry GenAI)                         |
|:-----------------------------------|:-------------------------------------------------------------------------|:------------------------------------------------------------------------------------|
| **Monitored Pipeline Flow**        | `[Ingress Gateway]` $\to$ `[Agent Service]` $\to$ `[Model Endpoint]`     | `[agent.trajectory]` $\to$ `[agent.step #29]` $\to$ `[tool.execution]`              |
| **Observed Telemetry Signals**     | HTTP 200 OK ($p_{99} = 140\text{ ms}$), 0 exceptions, 2,400 tok/s        | `gen_ai.client` (4,120 tokens, $T=0.2$) $\to$ `exec_sql` target `prod_db`           |
| **Reported Runtime Status**        | Nominal Success (`HEALTHY`, all green dashboards)                        | Execution Trapped (`status: ERROR`, step rejected)                                  |
| **Ground-Truth Execution Reality** | **Silent Catastrophe:** Agent executes destructive table drop at Step 29 | **Deterministic Isolation:** Kernel sandbox denies permissions; root cause captured |

: **Failure Visibility Gap: Traditional APM vs. Hierarchical Trajectory Telemetry**: Comparison of runtime observability signals and failure detection capabilities under semantic agent collapse versus syntactic microservice errors. {#tbl-vol3-apm-failure-contrast tbl-colwidths="[20,40,40]"}
...
### Telemetry collection engines: User-space hooks, ebpf probes, and ring buffers

Implementing the OpenTelemetry GenAI span model in production requires resolving where and how telemetry is physically captured. Runtimes must choose between two distinct telemetry collection engines:

1. **User-Space Interception Hooks:** The runtime wraps foundation model SDKs (such as OpenAI, Anthropic, or Hugging Face client libraries) and tool dispatchers with user-space interceptors or dynamic monkey-patching. While user-space hooks capture rich, high-level application semantics—such as structured thought strings, prompt templates, and raw JSON-RPC payloads—they impose a severe physical tax: **log serialization CPU overhead**. Synchronously stringifying multi-megabyte prompt tensors, tool schemas, and environment observation strings into JSON payloads directly on the agent worker thread consumes between 15 and 25 percent of worker CPU cycles. This serialization overhead induces thread execution jitter and contends with model token parsing. Furthermore, user-space hooks suffer from an existential security limitation: if an untrusted or drifting agent spawns an uninstrumented subshell (`/bin/sh -c ...`), executes an arbitrary compiled binary, or calls low-level POSIX system calls directly inside its sandbox, user-space hooks are completely bypassed.
2. **Kernel-Space eBPF Probes:** To achieve tamper-proof, zero-bypass observability across sandboxed tool executions, modern runtimes deploy extended Berkeley Packet Filter (eBPF) probes attached to kernel tracepoints (`sys_enter_execve`, `sys_enter_connect`, `sys_enter_openat`, `sys_enter_kill`) within the agent's Linux cgroup and network namespace. In-kernel eBPF bytecode is verified for memory safety and termination by the Linux kernel verifier prior to execution, guaranteeing that **eBPF probe overhead remains strictly below 1 percent** of host CPU cycles. When an agent attempts an unauthorized network socket connection or reads a sensitive filesystem path, the eBPF probe intercepts the operation at the kernel boundary before the syscall completes.

To export telemetry events from the kernel to user-space collectors without creating lock contention on execution threads, runtimes deploy lockless BPF ring buffers (`BPF_MAP_TYPE_RINGBUF`). Unlike legacy `perf` ring buffers that duplicate memory allocations across every CPU core, the BPF ring buffer provides a single, memory-mapped, multi-producer single-consumer circular queue. Even under intense tool workloads—such as an agent executing recursive directory scans or compiling large C++ codebases emitting 5,000 syscall events per second—the **ring buffer kernel-to-user copy memory bandwidth** is bounded to less than $15\text{ MB/s}$. A dedicated user-space telemetry worker consumes events asynchronously via memory-mapped I/O (`mmap`), reconciling low-level kernel execution events with the active `agent.step` OpenTelemetry span.
...
If a runtime naively injects raw token strings directly into OpenTelemetry span attributes (such as `gen_ai.prompt.content` or `tool.execution.stdout`), a 50-step trajectory generates over $100\text{ MB}$ of raw telemetry data. Consider a modest production cluster of 100 concurrent agents, each completing an average task every 15 minutes (400 tasks per hour, or 9,600 trajectories per day). At $100\text{ MB}$ per trajectory, the fleet generates nearly **$1\text{ TB}$ of raw span JSON per day** ($960\text{ GB/day}$) directed at tracing collectors designed for 4 KB spans! When hundreds of concurrent workers stream these payloads over gRPC to an OpenTelemetry Collector, three physical bottlenecks immediately emerge:
...
3. **Severe Trace Ingestion Latency and Network Overhead:** Transmitting massive text blobs over the tracing channel creates severe **OpenTelemetry gRPC span export network overhead**. Streaming 960 GB/day of raw JSON over gRPC requires sustained network throughput exceeding $89\text{ Mbit/s}$ per collector, causing TCP socket buffer bloat and inducing thread lock contention that distorts the very latency metrics the telemetry system is deployed to measure.

To solve the payload bloat crisis, modern agent architectures enforce **Asynchronous Telemetry Payload Offloading** (@tbl-vol3-payload-offload):

| Pipeline Stage             | Telemetry Path: Control Plane (Span Metadata)                                  | Telemetry Path: Data Plane (Payload Blobs)                                         |
|:---------------------------|:-------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------|
| **Execution Hook**         | Synchronous span boundary completion                                           | Non-blocking enqueue to in-memory ring buffer                                      |
| **Payload Representation** | Lightweight scalar metadata, SHA-256 hashes ($\mathcal{H}(P_t)$), URI pointers | Raw prompt tensors, compiler logs, DOM trees ($500\text{ KB}\text{--}4\text{ MB}$) |
| **Transport Protocol**     | OTLP over gRPC ($<2\text{ KB}$ per span)                                       | Background asynchronous batch upload (zstd compressed)                             |
| **Ingestion Target**       | OpenTelemetry Collector $\to$ Distributed Tracing Store (Jaeger / ClickHouse)  | Content-addressed object store (S3 / MinIO / GCS)                                  |
| **Operational Invariant**  | Sub-millisecond latency; zero collector memory exhaustion                      | Zero thread blocking; decoupled retention and cold-storage tiering                 |

: **Asynchronous Telemetry Payload Offloading Architecture**: Dual-path architecture decoupling high-frequency lightweight span coordination from multi-megabyte raw prompt and tool payload storage. {#tbl-vol3-payload-offload tbl-colwidths="[20,40,40]"}
...
High-volume unstructured payloads (full prompt tokens, raw system observations, compiler stack traces, and image embeddings) are enqueued into a lock-free, circular in-memory ring buffer. A dedicated background I/O thread drains the ring buffer, compresses payloads using Zstandard (`zstd` level 3, achieving $4\text{--}6\times$ compression on raw code and JSON text), and asynchronously streams them to durable object storage (such as Amazon S3, MinIO, or an optimized columnar database like ClickHouse). For our 100-agent cluster, compression reduces the daily $960\text{ GB}$ payload volume to approximately $170\text{ GB/day}$, costing mere pennies ($\approx \$0.13/\text{day}$ at standard object storage rates of $\$0.023/\text{GB-month}$). Meanwhile, the high-frequency OTLP span stream is reduced from $100\text{ MB}$ down to under $100\text{ KB}$ per trajectory ($50\text{ steps} \times 2\text{ KB}$), slashing collector network traffic by 99.9 percent ($<100\text{ kbit/s}$ sustained network throughput fleet-wide). The tracing backend retains perfect causal linkability through URI dereferencing while the collector data plane operates entirely within bounded memory and network envelopes.
...
By standardizing on these conventions, the systems runtime establishes an invariant causal foundation.

::: {#chk-vol3-16-opentelemetry-spans .callout-checkpoint title="OpenTelemetry spans and telemetry collection"}
Before exploring distributed tracing topologies, test your grasp of trajectory telemetry architecture:

- [ ] **Semantic vs. Syntactic Failure**: can you explain why a failed agent trajectory frequently exits with an HTTP 200 OK status code, blinding traditional APM dashboards?
- [ ] **Payload Bloat Mitigations**: can you describe why a 50-step agent trajectory generating 100 MB of context payloads requires separating scalar span metadata from compressed blob storage?
- [ ] **Telemetry Collection Overhead**: can you distinguish why user-space serialization CPU overhead (15 to 25 percent) contrasts with kernel eBPF probe overhead (strictly below 1 percent)?
:::

However, when an agent spawns autonomous subagents across distributed worker pools, telemetry must propagate context across network boundaries and track non-linear branching topologies.

## Causal Trajectory Tracing {#sec-vol3-observability-tracing}
...
A major systems divergence between traditional distributed tracing and agentic telemetry lies in execution topology. Microservices follow DAGs representing deterministic RPC dispatch. Autonomous agents executing test-time deliberation—such as Tree-of-Thoughts (ToT) or Monte Carlo Tree Search (MCTS) (examined in @sec-vol3-deliberation-tree-search)—execute **non-linear speculative branching** (@tbl-vol3-span-linking-topology):

| Trajectory Node / Branch | Span Identification & Context           | Exploration Role & Outcome   | Telemetry Lifecycle & SpanLink Association                            |
|:-------------------------|:----------------------------------------|:-----------------------------|:----------------------------------------------------------------------|
| **Root Node (Step $t$)** | `trace_id: 4bf9...`, `span_id: root_01` | Active decision branch point | Parent span for speculative rollout forks                             |
| **Branch A (Rollout 1)** | `span_id: branch_A`, Score: $0.42$      | Speculative trajectory fork  | Pruned (`search.branch.status = "pruned"`); linked via `SpanLink`     |
| **Branch B (Rollout 2)** | `span_id: branch_B`, Score: $0.91^*$    | Optimal candidate selection  | **Winning path:** transitions to active execution path for Step $t+1$ |
| **Branch C (Rollout 3)** | `span_id: branch_C`, Score: $0.18$      | Speculative trajectory fork  | Pruned (`search.branch.status = "pruned"`); linked via `SpanLink`     |
| **Step $t+1$ Node**      | `span_id: c01a...`, Inherits `trace_id` | Committed operational state  | Embeds counterfactual `SpanLink` references to Branches A and C       |

: **Causal Span Linking in Non-Linear Search Topologies**: Structural coordination of speculative MCTS search rollouts, separating active operational traces from pruned counterfactual paths. {#tbl-vol3-span-linking-topology tbl-colwidths="[20,25,25,30]"}
...
To see this equation in practice, consider a concrete systems example: an agent step $v$ evaluates a prompt context of $32{,}000$ tokens ($N_{\text{prefill}}$) and generates an 800-token tool call ($N_{\text{decode}}$), running inside an isolated Firecracker microVM for $1{,}500\text{ ms}$ ($T_{\text{wall}} = 1.5\text{ s}$). At standard enterprise cloud pricing ($p_{\text{prefill}} = \$3.00 \times 10^{-6}$, $p_{\text{decode}} = \$15.00 \times 10^{-6}$, and microVM compute at $c_{\text{infra}} \approx \$0.02/\text{vCPU-hr} \approx \$5.55 \times 10^{-9}/\text{ms}$):

- Prefill cost: $32{,}000 \times \$3.00 \times 10^{-6} = \$0.0960$
- Decode cost: $800 \times \$15.00 \times 10^{-6} = \$0.0120$
- MicroVM hosting cost: $1{,}500\text{ ms} \times \$5.55 \times 10^{-9}/\text{ms} = \$0.000008$
- Total local cost: $C_{\text{local}}(v) \approx \$0.1080$ per step.

This napkin math exposes a fundamental physical reality of agent systems: the compute virtualization cost of the sandboxed microVM ($<\$0.00001$) is four orders of magnitude smaller than the model's token inference cost ($\$0.1080$). In agentic computing, *tokens dominate infrastructure hosting by more than $10{,}000\times$*.
...
where $\tau^* \subset \mathcal{V}$ is the subset of nodes that directly contributed to the verified terminal outcome, and $C_{\text{cumulative}}(\text{root})$ is the total expenditure across all nodes in the execution tree.

For instance, consider a Monte Carlo Tree Search exploring branching factor $B=4$ across depth 3 ($4^3 = 64$ leaf rollouts). If 63 candidate rollouts are pruned and only 1 winning trajectory $\tau^*$ of 15 steps is committed, the total cumulative expenditure might reach $C_{\text{cumulative}}(\text{root}) \approx \$12.50$, while the winning path consumed only $C_{\text{winning}}(\tau^*) \approx \$1.62$. The resulting token efficiency evaluates to $\eta_{\text{token}} = \$1.62 / \$12.50 \approx 0.13$ (13 percent). Over 87 percent of the financial expenditure was consumed by exploratory scaffolding. In unguided multi-agent swarms or deep search topologies ($B \ge 16$), $\eta_{\text{token}}$ frequently drops below $0.10$. Tracking $\eta_{\text{token}}$ via distributed trace spans provides the primary systems signal for identifying runaway deliberation and un-converged agent loops.

### Tail-based sampling vs. head-based sampling

In high-throughput web microservices (processing $10^5\text{--}10^7$ requests per second), distributed tracing infrastructures rely almost exclusively on **Head-Based Sampling**\index{Head-Based Sampling}: when an incoming HTTP request arrives at the API gateway, the ingress proxy rolls a random number. If the sampling rate is $\alpha = 0.01$, 99 percent of requests are stripped of tracing context at ingress, and only 1 percent are recorded. In microservices, this is mathematically sound because transactions are homogeneous and high-frequency.
...
3. **Heavy-Tailed, Non-Deterministic Failures:** Catastrophic failure modes (safety policy trips, circular loops, prompt injections) occur in the long tail (at or below 1 percent of runs). Dropping 99 percent of traces at ingress guarantees that when an agent experiences a critical failure, exactly zero telemetry spans will be available to debug the incident.

Production agent fleets must therefore implement **Tail-Based Sampling**\index{Tail-Based Sampling}, buffering all telemetry spans and deferring the sampling decision until the trajectory completes (@tbl-vol3-tail-sampling-rules):

| Sampling Tier                  | Triggering Operational Criteria                                    | Ingestion & Buffer Action                              | Telemetry Retention Policy                                              | Long-Term Storage Destination                         |
|:-------------------------------|:-------------------------------------------------------------------|:-------------------------------------------------------|:------------------------------------------------------------------------|:------------------------------------------------------|
| **Anomaly & Failure Capture**  | Terminal status `ERROR`, `TIMEOUT`, or `GOAL_DIVERGENCE`           | Buffered during execution in circular NVMe ring buffer | **Retain 100%**: Preserve all spans, prompt hashes, and tool diffs      | High-performance search tier (ClickHouse / Jaeger)    |
| **Safety & Policy Violations** | Constitutional filter trip or unauthorized tool invocation         | Instant flush from local buffer to persistent tier     | **Retain 100%**: Mandatory capture for security audit and retraining    | Immutable compliance store with forensic snapshot     |
| **SLA & Cost Overruns**        | Cumulative spend $C > C_{\max}$ or latency $T > p_{95}\text{ SLA}$ | Instant flush from local buffer to persistent tier     | **Retain 100%**: Financial and performance regression forensics         | SRE performance analytics and capacity planner        |
| **Nominal Trajectory Success** | Verified task completion with zero safety/cost flags               | Evaluated at trajectory termination                    | **Aggressive Downsampling ($\beta = 0.01$)**: Retain 1% baseline sample | Compressed cold storage; discard routine raw payloads |

: **Tail-Based Sampling Decision Matrix for Agent Trajectories**: Retention and downsampling rules governing the deferred trace decision engine, ensuring complete capture of long-tail anomalies while constraining telemetry costs. {#tbl-vol3-tail-sampling-rules tbl-colwidths="[18,25,21,21,15]"}

During execution, worker nodes stream all active spans into a local, lockless circular ring buffer backed by DRAM or high-speed NVMe SSD storage with an explicit sliding-window time-to-live ($T_{\text{TTL}} = 3,600\text{ s}$). Writing un-sampled spans sequentially to an append-only ring buffer sustains **trace storage I/O throughput** exceeding $500\text{ MB/s}$ with minimal write amplification and zero kernel context switches. When the root trajectory span terminates, the tail-sampling collector evaluates an ensemble of deterministic rules:
...
- If end-to-end wall-clock latency exceeds the 95th percentile service level agreement (SLA): **Retain 100 percent**.
...
To quantify the storage savings of this design, consider an enterprise fleet running $10{,}000$ agent trajectories per day, averaging 25 steps per run ($250{,}000$ total step spans). Without payload offloading, uncompressed prompts and tool logs would generate over $250\text{ GB/day}$ ($250{,}000 \times 1\text{ MB}$). With payload offloading, the OTLP span metadata index consumes only $500\text{ MB/day}$ ($250{,}000 \times 2\text{ KB}$). Applying tail-based sampling reduces this index footprint even further:

- **Anomalies and failures** (a 4 percent failure rate, or 400 trajectories per day) are retained at 100 percent, generating $400 \times 25 \times 2\text{ KB} = 20\text{ MB/day}$ of index spans (with their compressed payloads stored in object storage).
- **Routine nominal successes** (96 percent, or 9,600 trajectories per day) are downsampled at $\beta = 0.01$ (retaining 96 runs), generating $96 \times 25 \times 2\text{ KB} \approx 4.8\text{ MB/day}$.

The active distributed tracing index stores less than $25\text{ MB/day}$—a **95 percent reduction** compared to full metadata ingestion—slashing durable trace indexing I/O throughput into ClickHouse or Jaeger to less than $300\text{ bytes/s}$ of sustained write bandwidth while guaranteeing that 100 percent of production incidents, security policy trips, and cost overruns are preserved with complete causal fidelity.

Distributed tracing and tail-based sampling guarantee that every production incident, policy trip, and token expenditure is causally captured and attributed across asynchronous subagents and non-linear search rollouts. However, observing and diagnosing failures after they manifest in production is an expensive, reactive posture: every captured production incident represents compromised customer data, degraded reliability, and wasted token capital. To systematically prevent catastrophic policy divergence before models touch production traffic, the runtime must subject candidate agents to rigorous offline verification. But evaluating stochastic actors demands far more than static prompt datasets—it requires interactive, hermetically isolated operating environments that eliminate benchmark contamination and environment decay, the systems architecture to which we now turn.

## Hermetic Evaluation Gyms {#sec-vol3-observability-eval-gyms}
...
2. **Compounding Trajectory Error Propagation:** In a static benchmark, sample $i$ has zero causal influence on sample $i+1$; errors are strictly independent. In an autonomous agentic regime, execution is a closed-loop trajectory $\tau = (s_0, a_0, o_1, a_1, \dots, s_H)$ where every action $a_t$ mutates the physical environment state $s_t$, and subsequent observations $o_{t+1}$ reflect those mutations. If an agent possesses a single-step success probability of $p = 1 - \epsilon$, its open-loop trajectory success probability over horizon $H$ decays exponentially:
   ...
   Even if a model achieves an impressive 98 percent single-step accuracy ($\epsilon = 0.02$) on a static tool-calling benchmark, its probability of completing a 50-step autonomous software engineering task without intermediate verification decays exponentially to $P_{\text{success}}(50) \le (0.98)^{50} \approx 0.364$ (36.4 percent).
...
The physical intuition behind @eq-vol3-pass-at-k is rooted in hyper-geometric sampling without replacement. If an agent generates $n$ candidate trajectories for a task and $c$ of them pass the verification oracle, the pool contains $n - c$ failing trajectories. If a deployment system randomly selects $k$ candidates to run, the probability that *all $k$ selections fail* is the number of ways to pick $k$ failures from $n - c$ ($\binom{n - c}{k}$) divided by the total ways to pick $k$ candidates from $n$ ($\binom{n}{k}$). Subtracting this joint failure probability from 1 yields the probability that at least one of the $k$ chosen rollouts succeeds.

To see how this behaves in practice, suppose an agent generates $n = 10$ rollouts for a SWE-bench task, and $c = 2$ pass the unit test suite ($n - c = 8$ failures):

- **Pass@1 ($k=1$):** $1 - \frac{8}{10} = 0.20$ (20.0 percent).
- **Pass@3 ($k=3$):** The chance that all 3 draws fail is $\frac{\binom{8}{3}}{\binom{10}{3}} = \frac{56}{120} \approx 0.467$. Thus, $\text{Pass}@3 = 1 - 0.467 = 0.533$ (53.3 percent).
- **Pass@5 ($k=5$):** The chance that all 5 draws fail is $\frac{\binom{8}{5}}{\binom{10}{5}} = \frac{56}{252} \approx 0.222$. Thus, $\text{Pass}@5 = 1 - 0.222 = 0.778$ (77.8 percent).

In production evaluation systems where $n$ can be large (e.g., $n = 100$ or $1{,}000$), evaluating binomial coefficients directly using factorials triggers floating-point overflow because factorials exceed 64-bit IEEE 754 limits at $171! \approx 7.26 \times 10^{306}$. Systems runtimes implement the numerically stable, logarithmic evaluation formulation shown in @eq-vol3-pass-at-k-stable:[^fn-observability-passk-combinatorics]

[^fn-observability-passk-combinatorics]: For the formal combinatorial derivation and proof of numerical stability, see @sec-vol3-appendix-math-pass-at-k.
...
A chronic systems pitfall in agent evaluation is deploying model updates based on underpowered benchmark runs. When evaluating an agent across a benchmark subset of $N = 30$ tasks, observing a 6.6 percent gain (e.g., solving 16 tasks instead of 14) is statistically indistinguishable from random noise: the two-sided $p$-value under Fisher's exact test exceeds $0.75$, providing less than 20 percent statistical power.
...
where $\bar{p} = (p_1 + p_2) / 2$, $z_{\alpha/2} = 1.96$ for a 95 percent confidence level, and $z_\beta = 0.84$ for 80 percent power. For an agent baseline pass rate of $p_1 = 0.30$ and a targeted capability shift of 5 percent ($p_2 = 0.35$), @eq-vol3-sample-size dictates a minimum sample size of $N \approx 1{,}430$ independent task evaluations.

To appreciate why this sample size presents a major systems challenge, consider the physical compute footprint: in SWE-bench, evaluating a single task instance takes approximately 10 minutes of container execution and consumes 50,000 prompt and tool tokens. Running 1,430 independent task evaluations requires $14{,}300$ container-minutes ($\approx 238$ CPU-hours) and over $71$ million tokens ($\approx \$215$ on a frontier model).

When engineering teams attempt to cut CI/CD costs by evaluating on only $N = 30$ tasks, a measured gain of 6.6 percent (e.g., solving 16 tasks instead of 14) has a 95 percent confidence interval spanning from 34 percent to 72 percent. The observed "improvement" is pure statistical noise. Reliable evaluation demands either budgeting for full sample sizes or employing sequential testing to detect regressions dynamically.[^fn-observability-twoprop-montecarlo] Benchmarking suites evaluating on fewer than 200 tasks cannot reliably detect a 5 percent regression, routinely passing degraded models into production.

[^fn-observability-twoprop-montecarlo]: For the formal two-proportion hypothesis derivation and non-parametric Monte Carlo bootstrap confidence intervals, see @sec-vol3-appendix-math-bootstrap.
...
- **Canary Perplexity Tracking ($\Delta_{\text{canary}}$):** To detect pretraining contamination in newly released model checkpoints, evaluation gyms embed cryptographic "canary strings" (randomized UUIDs and un-indexed code comments) into benchmark task descriptions. By tracking the cross-entropy loss of the model on canary tokens versus authentic code tokens, the evaluation harness flags models that exhibit memorized token distributions. (Note that measuring prompt token cross-entropy requires self-hosted open-weights inference engines such as vLLM or TGI; closed commercial APIs that withhold prompt log-probabilities cannot support direct perplexity scoring, requiring evaluation gyms to rely on canary completion probing instead.)

While hermetic evaluation gyms and unbiased capability estimators establish an indispensable predeployment filter, offline benchmarks are intrinsically bounded by the synthetic nature of their tasks and environments. No offline gym can perfectly replicate the noisy user prompts, live external schema mutations, and non-stationary environment distributions of live production systems. A candidate policy that achieves a stellar Pass@1 in an isolated sandbox may harbor subtle behavioral quirks that only manifest under real-world distribution shifts. Safely transitioning candidate agents from offline gyms into production without exposing user data to unverified regressions requires progressive autonomy tiers, state-isolated shadow deployments, and online sequential verification—the operational deployment pipeline examined next.

::: {#chk-vol3-16-hermetic-eval-gyms .callout-checkpoint title="Hermetic evaluation gyms and unbiased metrics"}
Before examining canary routing and shadow execution, test your grasp of offline agent verification:

- [ ] **Compounding Trajectory Decay**: can you explain why a model with 98 percent single-step benchmark accuracy can fail to complete more than 36 percent of 50-step autonomous trajectories?
- [ ] **The Five Hermetic Invariants**: can you describe how ephemeral Copy-on-Write sandboxing and strict network air-gapping eliminate benchmark bit-rot and pretraining contamination?
- [ ] **Unbiased Pass@$k$ Estimation**: can you distinguish why evaluating pass rates via hypergeometric sampling without replacement prevents factorial overflow and eliminates estimator variance?
:::

## Canary Trajectory Routing {#sec-vol3-observability-canary}
...
To execute shadow deployments safely without state corruption, runtimes implement **Ephemeral Copy-on-Write Environment Forking** (@tbl-vol3-shadow-deployment-topology):

| Architectural Plane       | Production Execution Path (Active Policy $v_1$)              | Shadow Execution Path (Candidate Policy $v_2$)               | Differential Verifier Role                                                                   |
|:--------------------------|:-------------------------------------------------------------|:-------------------------------------------------------------|:---------------------------------------------------------------------------------------------|
| **Ingress Gateway**       | Receives live user request; initiates primary trajectory     | Duplicates request payload; forks execution asynchronously   | Synchronizes task input parameters and initial state vector                                  |
| **Environment Sandbox**   | Live production environment with active permissions          | Ephemeral Copy-on-Write sandbox (OverlayFS / ZFS snapshot)   | Provides strict state isolation; zero mutual contamination                                   |
| **External Side Effects** | Real database transactions and authorized external API calls | Local database clone at $t=0$; intercepted and mocked egress | Prevents duplicate mutations, charge triggers, or external leakage                           |
| **Observation Stream**    | Ground-truth production state observations ($o_t$)           | Virtualized observations returned from ephemeral snapshot    | Isolates candidate policy from production environment drift                                  |
| **Trajectory Evaluation** | Commits state upon successful verification or human sign-off | Emits speculative diffs; sandbox destroyed upon termination  | Computes AST action distance $\mathcal{D}_{\text{AST}}(a_{v1}, a_{v2})$ and LLM-judge scores |

: **Ephemeral Copy-on-Write Shadow Deployment Architecture**: Dual-track execution topology isolating candidate policies within disposable sandboxes to evaluate behavioral divergence without mutating production state. {#tbl-vol3-shadow-deployment-topology tbl-colwidths="[18,27,27,28]"}
...
1. **Abstract syntax tree (AST) action distance:** Parses tool calls emitted by v1 and v2 into ASTs, computing structural edit distance ($\mathcal{D}_{\text{AST}}(a_{\text{v1}}, a_{\text{v2}})$).
...
To see the mechanics of this sequential test in production, consider concrete operational parameters: let the baseline acceptable failure rate be $p_0 = 0.05$ (5 percent) and the unacceptable degraded rate be $p_1 = 0.15$ (15 percent), with $\alpha = 0.01$ and $\beta = 0.05$:

- **Abort Boundary:** $A = \frac{1 - 0.05}{0.01} = 95 \implies \ln A = \ln(95) \approx 4.55$
- **Promote Boundary:** $B = \frac{0.05}{1 - 0.01} = \frac{0.05}{0.99} \approx 0.0505 \implies \ln B = \ln(0.0505) \approx -2.99$
- **Incremental update per failure ($X_i = 1$):** $\ln\left( \frac{p_1}{p_0} \right) = \ln\left( \frac{0.15}{0.05} \right) = \ln(3) \approx +1.099$
- **Incremental update per success ($X_i = 0$):** $\ln\left( \frac{1 - p_1}{1 - p_0} \right) = \ln\left( \frac{0.85}{0.95} \right) \approx -0.111$

Notice the asymmetric systems leverage in these numbers: a single trajectory failure ($+1.10$) carries nearly **ten times the weight** of a success ($-0.11$). A healthy model requires approximately 27 consecutive successful tasks to drive $\ln \Lambda_m$ below $-2.99$ to trigger traffic promotion.

However, if a candidate checkpoint suffers an undiscovered regression resulting in a 40 percent failure rate, and encounters 5 failures across its first 8 production tasks ($k = 5, m - k = 3$), the cumulative log-likelihood ratio updates to $\ln \Lambda_8 = 5(+1.099) + 3(-0.111) = 5.495 - 0.333 = +5.162 \ge 4.55$, breaching the abort threshold $\ln A$ on the eighth execution! The sequential canary routing decision rule operates continuously as follows:
...
2. **Accept $H_0$ and Promote Canary ($\ln \Lambda_m \le \ln B$):** The data proves that the canary policy meets or exceeds reliability targets. The runtime automatically increments canary traffic allocation from 10 percent to 25 percent, 50 percent, and ultimately 100 percent.
...
Because SPRT evaluates likelihood ratios sequentially after every individual trajectory, a severely broken model is aborted in single-digit trials, capping user blast radius by over 99 percent compared to fixed-batch evaluation schemes that would have subjected hundreds of users to the faulty model before calculating metrics.[^fn-observability-wald-stopping]

[^fn-observability-wald-stopping]: For the formal derivation of Wald's stopping boundaries and Average Sample Number (ASN), see @sec-vol3-appendix-math-sprt.

Wald's sequential hypothesis testing and state-isolated shadow deployments provide rigorous statistical guarantees for gating individual model updates and bounding user blast radius under stochastic drift. However, governing a large-scale production fleet across thousands of concurrent, long-horizon agents requires moving beyond binary deployment decisions to continuous, fleet-wide operational health. When thousands of autonomous agents operate concurrently, traditional site reliability indicators like raw token throughput, GPU utilization, and HTTP response codes become deeply deceptive—masking pathological failure modes where fleets burn millions of tokens on conversational thrashing while delivering zero verified work. To maintain dependable operations, SRE teams must establish metrics grounded in useful computational progress, dynamic deadline scheduling, and multi-turn error budget accounting—the focus of our next section.

## Fleet-Wide Trajectory Goodput {#sec-vol3-observability-sre-metrics}
...
To see how CPVG governs architectural decisions, compare two serving strategies for an enterprise workflow where each attempt consumes 20,000 tokens:

- **Option A (Compact Model, $P_{\text{active}} = 8\text{B}$):** Token pricing is $\$0.20/\text{1M tokens}$, yielding a raw execution cost of $20{,}000 \times \$0.20 \times 10^{-6} = \$0.004$ per attempt. If the standalone single-trajectory pass rate is $p = 0.40$ (40 percent), the Cost-per-Verified-Goal evaluates to $\text{CPVG}_A = \$0.004 / 0.40 = \$0.010$ (1.0 cent per verified goal).
- **Option B (Frontier Model, $P_{\text{active}} = 70\text{B}$):** Token pricing is $\$3.00/\text{1M tokens}$, costing $20{,}000 \times \$3.00 \times 10^{-6} = \$0.060$ per attempt. With a higher single-trajectory pass rate of $p = 0.65$ (65 percent), the Cost-per-Verified-Goal evaluates to $\text{CPVG}_B = \$0.060 / 0.65 \approx \$0.092$ (9.2 cents per verified goal).

Even though the frontier model is significantly more capable (65 percent versus 40 percent), its CPVG is **$9.2\times$ higher**! The compact model delivers nearly an order of magnitude higher Economic Goodput ($G_{\$}$).

Furthermore, consider applying Best-of-3 parallel sampling with the compact model: the probability that at least one of 3 independent rollouts succeeds is $1 - (1 - 0.40)^3 = 1 - 0.216 = 0.784$ (78.4 percent). The total compute cost is $3 \times \$0.004 = \$0.012$. The resulting Cost-per-Verified-Goal evaluates to $\text{CPVG}_{A,\text{Best-of-3}} = \$0.012 / 0.784 \approx \$0.0153$ (1.5 cents per verified goal). Best-of-3 on the compact model delivers **higher reliability than the single frontier model** (78.4 percent versus 65.0 percent) at **one-sixth the cost** ($\$0.015$ versus $\$0.092$). This trade-off is the exact mechanism driving the Pareto frontier in @fig-vol3-goodput-pareto.
...
- $T_{\text{prefill}, t}$: Time spent processing prompt tokens (Time-To-First-Token, general matrix multiply (GEMM) compute-bound).
...
By Amdahl's Law, even if an engineering team spends months accelerating foundation model inference to achieve an infinite speedup ($S_{\text{model}} \to \infty$, instantaneous zero-latency inference), the maximum achievable end-to-end trajectory speedup is strictly capped at $S_{\text{max}} = 1 / (1 - f_{\text{model}}) = 1 / (1 - 0.35) = 1 / 0.65 \approx 1.54\times$.
...
where $T_{\text{tool\_allowance}}(a_i)$ dynamically credits time when an agent invokes known long-running operations, and $\Delta T_{\text{penalty}}$ deducts time when output token entropy collapses or when tool return codes indicate repetitive loop flapping.

For example, a scheduler initializes a task with base budget $T_{\text{base}} = 120\text{ s}$. When the agent issues `cargo test --workspace`, the runtime inspects the tool registry and dynamically credits $T_{\text{tool\_allowance}} = +180\text{ s}$, extending the deadline to accommodate legitimate compiler and test suite latency. Conversely, if the agent executes three identical failing bash commands consecutively, or if token generation entropy drops below an anomalous threshold (measured directly via vocabulary logits on self-hosted inference runtimes, or approximated via n-gram repetition counters on closed API endpoints), the scheduler levies a penalty $\Delta T_{\text{penalty}} = 60\text{ s}$. This early-termination heuristic traps unproductive loops within seconds rather than stranding GPU memory for the remainder of a static timeout.

### Non-deterministic error budgets and SRE alerting

In classical site reliability engineering, service-level objectives (SLOs) are defined over simple uptime ratios (e.g., 99.9 percent of HTTP requests return status 200 within $200\text{ ms}$). In autonomous agent systems, SLOs must be redefined over **multi-turn semantic validity** using service level indicators (SLIs):
...
Because agent execution is stochastic, traditional single-threshold alerting triggers intolerable alert fatigue: a temporary cluster dip to 87 percent accuracy across 10 tasks is frequently a statistical artifact. Furthermore, relying on traditional Prometheus counter scraping intervals ($15\text{--}30\text{ s}$) blinds SRE dashboards to micro-burst conversational thrashing loops that ignite, burn through error budgets, and collapse in single-digit seconds between scrape pulls. Autonomous SRE runtimes therefore pair push-based OpenTelemetry metric streaming with sliding-window aggregators to implement **Multi-Window Multi-Burn-Rate Alerting**: tracking error budget consumption across short-term ($1\text{ hr}$) and long-term ($24\text{ hr}$) rolling windows.

To see why burn-rate alerting is essential, consider the physical arithmetic: over a standard 30-day billing month ($720\text{ hours}$), an SLO target of 90 percent ($\text{SLI}_{\text{res}} \ge 0.90$) allows a total error budget of 10 percent. A $1\times$ burn rate consumes the budget evenly over 720 hours (10 percent / 720 $\approx 0.0139$ percent of budget per hour). A **$14.4\times$ burn rate** accelerates failure consumption to $14.4 \times (1 / 720) = 2.0$ percent of the monthly budget consumed in just a single hour. At that burn velocity, the organization's entire monthly error allowance will be completely depleted in only $720 / 14.4 = 50\text{ hours}$ (roughly two days).

By paging on-call engineers only when the 1-hour burn rate exceeds $14.4\times$ or the 6-hour burn rate exceeds $6\times$, the telemetry system guarantees rapid alerting during catastrophic policy degradation while completely ignoring transient, single-task statistical blips.

Establishing Trajectory Goodput, dynamic deadline scheduling, and multi-burn-rate alerting provides the operational framework to maintain fleet reliability under stochastic agent dynamics. Yet, when engineering teams design and deploy their first production observability pipelines, they repeatedly fall prey to architectural intuitions inherited from classical microservices or static machine learning benchmarks. These architectural misconceptions lead teams to optimize the wrong subsystems, adopt blind sampling strategies, or mistake superficial benchmark gains for production readiness. Before formalizing the core takeaways of agent telemetry, we systematically dissect these critical systems fallacies and pitfalls.

::: {#chk-vol3-16-benchmarks-pareto-frontier .callout-checkpoint title="Trajectory goodput and cost-per-verified-goal"}
Before examining fallacies and pitfalls, test your grasp of agent SRE economics:

- [ ] **Trajectory Goodput**: can you explain why raw token throughput and GPU utilization are deeply deceptive metrics during conversational thrashing episodes?
- [ ] **Cost-per-Verified-Goal (CPVG)**: can you describe why a compact model with a lower single-attempt pass rate can achieve a substantially lower Cost-per-Verified-Goal than a frontier model?
- [ ] **Amdahl's Law for Trajectories**: can you distinguish why accelerating external tool execution and container snapshotting delivers greater end-to-end trajectory speedup than optimizing model decode latency?
:::
...
Accelerating model decode by $2.5\times$ yields an end-to-end trajectory speedup of $S_{\text{overall}} = 1 / ((1 - 0.35) + 0.35 / 2.5) = 1 / (0.65 + 0.14) \approx 1.266\times$ (a 26.6 percent improvement). Conversely, optimizing the systems runtime—replacing container boots with instant Firecracker microVM snapshot restores ($8\times$ speedup across $f_{\text{sandbox}} = 0.18$) and caching build artifacts ($3.5\times$ speedup across $f_{\text{tool}} = 0.37$)—yields an overall speedup of $1.71\times$ (a 71 percent improvement). Optimizing systems scaffolding delivers nearly triple the performance gain of inference kernel optimizations.

:::

Dismantling these fallacies exposes the foundational reality of agentic systems: observing, evaluating, and operating non-deterministic actors cannot be solved by treating foundation models as stateless microservices or relying on model scale alone. Reliable autonomy demands a unified systems architecture where hierarchical telemetry, hermetic sandboxing, tail-based trace capture, and Amdahl-aware optimization are engineered directly into the execution runtime. The following summary codifies these architectural imperatives into the durable systems invariants that govern autonomous observability.
...
* **The fail-plausible fault model**: Autonomous agents fail semantically rather than syntactically, emitting valid tokens and clean exit codes while hallucinating actions or entering circular loops. Traditional microservice APM is blind to semantic collapse; observability requires structuring telemetry into hierarchical OpenTelemetry GenAI spans (`agent.trajectory` $\to$ `agent.step` $\to$ `gen_ai.client` / `tool.execution`).
* **Payload offloading and tail-based sampling**: Large context windows ($128\text{K}\text{--}1\text{M}$ tokens) trigger collector OOMs if raw strings are injected into span attributes. Runtimes must keep spans lightweight ($<2\text{ KB}$) by storing cryptographic hashes and content-addressed storage URI pointers, streaming raw payloads out-of-band to object stores. Tail-based sampling buffers all traces and retains 100 percent of anomalous and failed trajectories while downsampling routine successes.
