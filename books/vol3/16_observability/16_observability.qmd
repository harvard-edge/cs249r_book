...
The necessity for an explicit evaluation contract marks the architectural boundary between isolated component telemetry and fleet-scale systems verification. In previous chapters, we developed runtime mechanisms to contain actuation (@sec-vol3-virtualization), log events (@sec-vol3-interrupts), and coordinate multi-agent teams (@sec-vol3-multi-agent). However, evaluating an autonomous system across long horizons introduces a distinct systems challenge: an agent can produce fluent, syntactically clean tool invocations that exit with code 0 while silently corrupting persistent state.[^fn-observability-eval-contract]

When unverified proposals execute without empirical acceptance testing, joint task success decays exponentially across multi-step dependency chains ($p^k \to 0$, as established in @sec-vol3-intro-fail-plausible). Observability in agentic ML systems cannot rely on superficial process monitoring or conversational self-reports. The runtime must enforce a formal evaluation contract that joins candidate token emissions, capability grants, tool side effects, and persistent state deltas into an inspectable, causal trace graph.

::: {#psp-16-the-fallacy-of-green-dashboards .callout-perspective title="The fallacy of green dashboards in agentic systems"}
In modern site reliability engineering, distributed monitoring frameworks treat low error rates and fast RPC completion as the gold standard of system health: five-nines HTTP 200 responses, zero uncaught runtime exceptions, and clean process termination (`exit 0`) indicate a pristine deployment. In autonomous agent architectures, this direct mapping between operational telemetry and system health completely collapses into what systems engineers term **the fallacy of green dashboards**.

An agent platform can operate with all monitoring dashboards glowing green while executing massive operational catastrophes. Because foundation models are non-deterministic, unprivileged text transcoders, an agent can emit syntactically flawless JSON payloads that execute cleanly within containerized sandboxes, accumulate zero kernel faults, and exit with status zero—all while disbursing duplicate financial transactions, silently deleting production data tables, or entering circular hallucination loops that burn millions of inference tokens without progressing toward goal satisfaction. In classical microservices, operational telemetry measures the direct outcome of deterministic software paths. In agent systems, operational telemetry records only mechanism activity, which is fundamentally decoupled from semantic task convergence.

Observability for agentic systems requires moving past superficial process health to enforce end-to-end evaluation contracts. Every trajectory must be evaluated across an unbroken causal trace graph that joins model inferences, capability grants, tool side effects, and persistent state mutations. A system is healthy not when its API calls return HTTP 200, but when its physical state deltas satisfy sealed regression assertions, its token consumption remains bounded within allocated budgets, and its trajectory goodput demonstrates provable convergence on the user's objective.
:::
...
[^fn-observability-eval-contract]: **Causal Graph vs Telemetry**: As formalized in @sec-vol3-intro-task-contract, process return codes (`exit 0`) measure execution activity rather than semantic task completion; observability requires causal trace graphs joining model tokens, tool side effects, and state delta verification.

To bridge this measurement void, observability in agent systems operationalizes the closed-loop verification contract into an explicit, four-layer evaluation hierarchy (@tbl-vol3-eval-contract-hierarchy).[^fn-eval-contract-hierarchy] Rather than treating telemetry as passive log ingestion, the supervisory harness evaluates candidate trajectories through four strictly ordered, non-fungible layers. Failure at any lower layer immediately invalidates evaluation at higher layers, preventing synthetic or corrupted data from polluting the observability pipeline.

[^fn-eval-contract-hierarchy]: **Evaluation Contract Hierarchy**: The systems necessity of endpoint verification was introduced in @sec-vol3-intro-task-contract and applied to distributed agents in @sec-vol3-multi-agent. This chapter instruments this hierarchy into concrete observability mechanisms: AST parsing, kernel exit monitoring, git diff assertions, and eBPF budget meters.
...
***Trajectory goodput***\index{Trajectory goodput!definition}\index{Goodput!definition} is the proportion of total cluster compute and execution resources allocated to trajectories that successfully achieve independently verified terminal states within bounded cost and latency constraints: $\mathcal{G} = \frac{\sum_{i \in \mathcal{T}_{\text{success}}} R_i}{\sum_{j \in \mathcal{T}_{\text{all}}} R_j}$.

1.  **Significance**: Replaces raw token throughput and process availability (`HTTP 200` / `exit 0`) with an authentic systems reliability metric, exposing that high GPU kernel utilization masks catastrophic waste when clusters execute failing, repetitive trajectory loops.
2.  **Distinction**: Unlike network or batch serving goodput (which counts delivered bits or completed inference tokens), trajectory goodput conditions success strictly on deterministic external verification ($\text{Verify}(s_T) = 1$) under budget ceilings ($C \le C_{\max}, T \le T_{\max}$); unverified token generation constitutes computational badput ($1 - \mathcal{G}$).
3.  **Common pitfall**: Sizing and evaluating agent fleets using micro-efficiency benchmarks (tokens per second or MBU) rather than trajectory goodput, which rewards fast generation even when the agent is trapped in fail-plausible hallucination cycles.
...
- Maximum allowable goodput degradation fraction: $\epsilon_G = 0.10$ (10 percent).
