...
::: {#exmp-16-evaluating-the-four-layer-contract-on .callout-example title="Evaluating the Four-Layer Contract on an Automated Database Migration"}
...
![**The Hermetic Evaluation Gym Architecture**: Sandboxed runtime fixtures, network isolation boundaries, deterministic local proxy stubs, and ground-truth verification oracles.](images/svg/ch16-eval-gym-architecture.svg){#fig-vol3-eval-gym-architecture width="95%"}
...
![**Sample Size Scaling and Wilson Score Confidence Intervals**: Error margins as a function of sample count $N$ across baseline success rates, demonstrating the sample scale required to resolve fine-grained architectural regressions.](images/svg/sample_size_confidence_intervals.svg){#fig-vol3-sample-size-confidence-intervals width="95%"}

As illustrated in @fig-vol3-sample-size-confidence-intervals, the contraction of the 95% Wilson confidence interval width as sample size scales demonstrates that on a benchmark suite of $N=100$ tasks, a candidate runtime reporting a 50% pass rate carries a confidence span of $\pm 9.8\%$ ($[40.2\%, 59.8\%]$). Distinguishing whether an updated reasoning scaffold delivers a genuine 3% improvement or mere stochastic variation requires expanding the evaluation set to well over 1,000 independent fixtures.
...
::: {#exmp-16-resolving-benchmark-superiority-under-wilson .callout-example title="Resolving Benchmark Superiority under Wilson Score Bounds"}
...
![**OpenTelemetry Distributed Trace Waterfall for an Agent Trajectory**: Causal parent-child span hierarchy decomposing model inference, vector search, tool execution, and environment response across execution domains.](images/svg/fig-vol3-opentelemetry-trace-waterfall.svg){#fig-vol3-opentelemetry-trace-waterfall width="95%"}
...
::: {#exmp-16-critical-path-latency-and-bottleneck .callout-example title="Critical Path Latency and Bottleneck Decomposition"}
...
![**Tail-Based Sampling Pipeline for Agent Trajectory Telemetry**: Trace escrow buffer, anomaly classification predicates, and prioritized persistence policies for high-cardinality execution graphs.](images/svg/tail-sampling-pipeline.svg){#fig-vol3-tail-sampling-pipeline width="95%"}

As formalized in @fig-vol3-tail-sampling-pipeline and @tbl-vol3-tail-sampling-policies, the tail-sampling collector evaluates incoming trajectories against a priority cascade of deterministic policy predicates. Rather than applying a single uniform rule, the routing engine classifies each completed trajectory into one of four distinct operational categories:
...
::: {#exmp-16-counterfactual-ablation-of-a-silent .callout-example title="Counterfactual Ablation of a Silent Tool Truncation"}
...
::: {#exmp-16-automated-canary-verification-and-release .callout-example title="Automated Canary Verification and Release Gate Evaluation"}
...
::: {.callout-takeaways title="Core Systems Principles of Evaluation and Observability"}
