...
::: {#def-vol3-sprt-circuit-breaker .callout-tip}
...
As formalized in @def-vol3-sprt-circuit-breaker, the automated circuit breaker must operate with minimal Mean Time to Detect (MTTD) and Mean Time to Recovery (MTTR), adhering to the tail-tolerance principles articulated by Dean and Ghemawat (2013). Beyond sequential hypothesis tests on aggregate task success, the runtime enforces invariant-based circuit breakers that trip instantly upon observing hard safety violations:
