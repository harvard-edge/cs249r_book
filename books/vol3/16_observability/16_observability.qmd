...
::: {#chk-vol3-16-opentelemetry-spans .callout-checkpoint title="OpenTelemetry spans and payload offloading"}

Before exploring distributed tracing topologies, verify your understanding of trajectory telemetry architecture:

- [ ] **Semantic vs. Syntactic Failure**: Why does a failed agent trajectory frequently exit with an HTTP 200 OK status code, blinding traditional APM dashboards?
- [ ] **Payload Bloat Mitigations**: In a 50-step agent trajectory generating 100 MB of context payloads, why must telemetry separate scalar span metadata from compressed blob storage?
- [ ] **Span Hierarchy**: What specific execution metrics are captured at the `agent.trajectory`, `agent.step`, and `tool.execution` span levels?
...
The active distributed tracing index stores less than $25\text{ MB/day}$—a **$95\%$ reduction** compared to full metadata ingestion—while guaranteeing that 100 percent of production incidents, security policy trips, and cost overruns are preserved with complete causal fidelity.
...
- [ ] **Trajectory Goodput vs. Token TPS**: Why is raw token throughput a misleading health metric for an autonomous agent fleet experiencing conversational thrashing?
- [ ] **Cost-per-Verified-Goal (CPVG)**: If Model A has a 40 percent pass rate at \$0.004/attempt and Model B has a 65 percent pass rate at \$0.060/attempt, which model delivers higher economic efficiency?
- [ ] **Best-of-$N$ Pareto Leverage**: How can parallel sampling on a compact model surpass both the accuracy and cost profile of a single frontier model invocation?
