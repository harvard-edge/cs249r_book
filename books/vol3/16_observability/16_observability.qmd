...
Apply the distributed tracing standards, statistical evaluation metrics, and reliability formulations established throughout this chapter to solve the following quantitative problems.

### Problem 1: Compounding trajectory decay and verification checkpoint sizing

An autonomous software engineering agent is deployed to execute multi-step bug repairs across an enterprise repository. The task requires a sequential horizon of $H = 60$ interaction steps. In an unverified, open-loop baseline, the foundation model exhibits a constant per-step success probability of $p = 0.96$ (per-step error rate $\epsilon = 0.04$).

1. **Open-Loop Reliability:** Calculate the probability of end-to-end trajectory success ($P_{\text{open}}$) if the agent executes all 60 steps without intermediate verification.
2. **Intermediate Verification Sizing:** The systems team introduces an automated, sandboxed verification checkpoint (running a lightweight syntax linter and unit test runner) every $k$ steps. Each verification checkpoint detects any error that occurred in the preceding $k$-step block with 100 percent fidelity and forces an immediate single-step correction rollout. The correction rollout has a success probability of $p_{\text{repair}} = 0.85$. If the correction succeeds, execution resumes; if it fails, the entire trajectory aborts. Formulate the overall trajectory success probability $P_{\text{verified}}(k)$ as a function of block size $k$, given that $H$ is evenly divisible by $k$ (number of blocks $M = H / k$).
3. **Latency and Compute Trade-Off:** Each normal step consumes an average of $T_{\text{step}} = 4.0\text{ s}$ of wall-clock time and $N_{\text{step}} = 2{,}000$ tokens. Each verification checkpoint incurs a sandboxed virtualization overhead of $T_{\text{check}} = 6.0\text{ s}$ but consumes zero model tokens. Calculate the total wall-clock time ($T_{\text{total}}$) and total token consumption ($N_{\text{total}}$) for a successful 60-step trajectory (assuming zero failures) under checkpoint frequencies $k = 5$, $k = 10$, and $k = 20$.
4. **Trajectory Goodput Optimization:** Let the token compute cost be $p_{\text{tok}} = \$5.00 \times 10^{-6}$ per token and the sandbox infrastructure cost be $c_{\text{infra}} = \$0.001$ per second. Calculate the Cost-per-Verified-Goal (CPVG) for $k = 5$ versus $k = 20$. Which checkpoint frequency delivers superior economic efficiency?

---

### Problem 2: Telemetry ingestion Roofline, buffer sizing, and tail-based sampling

A distributed autonomous operations platform hosts a fleet of $M = 2{,}500$ concurrently active worker agents. Each agent executes a continuous sequence of tasks, where an average task trajectory spans $H = 30$ steps and each step takes $T_{\text{step}} = 10\text{ s}$ of wall-clock time.

During execution, each step generates:

- **Structural Trace Metadata:** $4\text{ KB}$ of OpenTelemetry span attributes (timings, hashes, counters).
- **Unstructured Payload:** $350\text{ KB}$ of raw prompt text, code diffs, and tool stdout logs.
1. **Ingestion Bandwidth Roofline:** Calculate the aggregate incoming telemetry ingestion bandwidth (in megabytes per second, $\text{MB/s}$) generated by the fleet across both metadata and raw payloads. If the organization routes raw telemetry directly over gRPC to an OpenTelemetry Collector cluster whose network interface is capped at $1\text{ Gbps}$ ($125\text{ MB/s}$), calculate the percentage of network bandwidth saturated.
2. **Payload Offloading Savings:** The architecture implements Asynchronous Telemetry Payload Offloading: OpenTelemetry spans retain only the $4\text{ KB}$ metadata plus a $200\text{ byte}$ S3 URI pointer, while raw payloads are compressed via Zstandard (`zstd` ratio $4.5:1$) and streamed out-of-band to an object store. Calculate the new bandwidth transmitted to the OpenTelemetry Collector cluster and the compressed storage write bandwidth to the object store.
3. **Tail-Based Buffer Sizing:** The runtime implements tail-based sampling. Every worker node buffers all step spans and payloads in a local in-memory circular ring buffer during execution. The maximum allowable task execution horizon is $H_{\text{max}} = 60$ steps. If each worker node hosts 20 concurrent agent threads, calculate the minimum RAM capacity (in gigabytes) that must be allocated per worker node for the telemetry ring buffer to prevent buffer overflow.
4. **Economic Sampling Comparison:** The fleet executes a total of $100{,}000$ completed trajectories per month. Of these, $96\%$ are nominal successes and $4\%$ encounter errors or safety trips.
   - **Policy A (Head-Based Sampling):** $10\%$ of all incoming trajectories are randomly selected at ingress for full retention; the remaining $90\%$ are discarded.
   - **Policy B (Tail-Based Sampling):** $100\%$ of failed/anomalous trajectories are retained, while nominal successes are downsampled to $1\%$.
   Calculate the total monthly compressed storage volume (in gigabytes) under Policy A versus Policy B. What percentage of actual production failures are captured under Policy A versus Policy B?

---

### Problem 3: Trajectory goodput, model cascading, and SPRT canary stopping times

A platform team is evaluating a model migration for an autonomous IT remediation fleet. The current production policy ($\pi_{\text{base}}$, powered by a dense 70B model) achieves a verified task success rate of $p_0 = 0.90$. The team fine-tuned a compact 14B sparse Mixture-of-Experts model ($\pi_{\text{canary}}$) designed to lower operational inference costs.

1. **Economic Baselines:**
   - $\pi_{\text{base}}$ consumes an average of $45{,}000$ input tokens ($p_{\text{in}} = \$2.00 / 1\text{M}$) and $1{,}500$ output tokens ($p_{\text{out}} = \$8.00 / 1\text{M}$) per trajectory. MicroVM infrastructure cost is $\$0.05$ per run. Calculate the average cost per trajectory ($C_{\text{base}}$) and the Cost-per-Verified-Goal ($\text{CPVG}_{\text{base}}$) at $p_0 = 0.90$.
   - $\pi_{\text{canary}}$ consumes identical token counts, but its serving costs are $p_{\text{in}} = \$0.40 / 1\text{M}$ and $p_{\text{out}} = \$1.60 / 1\text{M}$. Infrastructure cost remains $\$0.05$. Calculate $C_{\text{canary}}$. What is the minimum verified success rate $p_{\text{min}}$ that $\pi_{\text{canary}}$ must achieve such that $\text{CPVG}_{\text{canary}} \le \text{CPVG}_{\text{base}}$?
2. **SPRT Canary Design:** The team deploys $\pi_{\text{canary}}$ on a 10 percent traffic split governed by Wald's Sequential Probability Ratio Test (SPRT). We test the null hypothesis $H_0: p_{\text{fail}} = 0.10$ (acceptable failure rate, matching baseline) against the alternative hypothesis $H_1: p_{\text{fail}} = 0.18$ (degraded failure rate). The team sets Type I error $\alpha = 0.02$ and Type II error $\beta = 0.05$.
   - Calculate the numerical absorbing decision boundaries $A$ and $B$.
   - Formulate the decision inequality for the log-likelihood ratio $\ln \Lambda_m$ as a function of total observed trajectories $m$ and total observed failures $k = \sum_{i=1}^m X_i$.
3. **Early Abort Scenario:** In production, $\pi_{\text{canary}}$ suffers from an unexpected tool schema bug that elevates its true failure rate to $p_{\text{true}} = 0.25$. Calculate the expected value of the log-likelihood increment per trajectory:
   $$
   \mathbb{E}_1[z] = p_{\text{true}} \ln\left(\frac{p_1}{p_0}\right) + (1 - p_{\text{true}}) \ln\left(\frac{1 - p_1}{1 - p_0}\right)
   $$
   Using Wald's approximation for expected sample size:
   $$
   \mathbb{E}_1[M] \approx \frac{(1 - \beta)\ln A + \beta \ln B}{\mathbb{E}_1[z]}
   $$
   calculate the expected number of canary trajectories $\mathbb{E}_1[M]$ before the SPRT rule trips boundary $A$ and automatically aborts the deployment. How many failed customer tasks were avoided compared to a fixed-batch canary of $N = 1{,}000$ trajectories?
