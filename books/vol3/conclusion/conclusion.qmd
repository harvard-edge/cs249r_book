...
- The synthesis across the machine learning systems curriculum, bridging digital software agency to physical embodied systems.
...
@Fig-vol3-curriculum-arc places systems engineering as the structural container that guarantees invariant closure across the complete machine learning systems curriculum, spanning single-node kernel execution, fleet-scale distributed serving, multi-step trajectory runtimes, and physical robot actuation.

::: {#fig-vol3-curriculum-arc fig-env="figure" fig-pos="htb" fig-cap="**Invariant Evolution across the Machine Learning Systems Paradigm**: Computational abstractions, governing physics, fundamental triads, and systems boundaries across the complete MLSys curriculum. Read down a column rather than across a row: the abstraction changes at every volume while the governing physics does not, which is why a result derived at one scale keeps its force at the next." fig-alt="Four-panel diagram illustrating the MLSys curriculum across single-node systems (The Model), fleet-scale systems (The Fleet), agentic systems (The Agent), and embodied physical AI."}
...
```{python}
#| label: vol3-conclusion-horizon-ceiling
#| echo: false
# +-----------------------------------------------------------------------------
# | THE HORIZON CEILING AND WHAT SCALING WOULD HAVE TO BUY
# +-----------------------------------------------------------------------------
# | Context: The horizon bullets below and the Amdahl bullet later in the
# |          chapter.
# |
# | Goal:    Close the volume on the same compounding bound it opened with, and
# |          state what per-step accuracy a viable trajectory would demand.
# | Show:    Survival at three horizons, the accuracy required for a viable
# |          hundred-step trajectory, and the ceiling a fixed I/O wait imposes.
# | How:     calc_trajectory_reliability for the bound; its inverse, the H-th
# |          root of the target, for the accuracy scaling would have to reach;
# |          calc_amdahls_speedup for the wait-bound ceiling.
# +-----------------------------------------------------------------------------
from mlsysim import *

class HorizonCeiling:
    # +-- 1. LOAD (Constants) --------------------
    accuracy = 0.98  # scenario assumption: frontier per-step accuracy
    horizons = (5, 30, 100)  # scenario assumption: short, moderate, enterprise
    viable = 0.95  # scenario assumption: a commercially viable success rate
    io_wait = 0.60  # scenario assumption: wall clock blocked on external tools

    # +-- 2. EXECUTE (The Compute) ---------------
    survivals = []
    for h in horizons:
        survivals.append(calc_trajectory_reliability(accuracy, h))
    long_horizon = horizons[-1]
    required_accuracy = viable ** (1 / long_horizon)
    required_error = 1 - required_accuracy
    error_one_in = 1 / required_error
    wait_bound_speedup = calc_amdahls_speedup(1 - io_wait, 10**9)

    # +-- 3. GUARD (Invariants) ------------------
    check(survivals[0] > survivals[1] > survivals[2], "Survival must fall as the horizon grows")
    check(required_accuracy > accuracy, "A viable hundred-step trajectory must demand more than frontier accuracy")
    check(
        abs(calc_trajectory_reliability(required_accuracy, long_horizon) - viable) < 1e-6,
        "The required accuracy must reproduce the viability target when compounded",
    )
    check(
        abs(wait_bound_speedup - 1 / io_wait) < 0.01,
        f"An infinitely fast accelerator is bounded by the reciprocal of the wait, got {wait_bound_speedup}",
    )

    # +-- 4. OUTPUT (Formatting) -----------------
    accuracy_str = fmt_percent(accuracy, precision=0, style="prose")
    h_short_str, h_mid_str, h_long_str = [fmt_int(h) for h in horizons]
    s_short_str, s_mid_str, s_long_str = [fmt_percent(s, precision=1, style="prose") for s in survivals]
    s_short_num_str, s_mid_num_str, s_long_num_str = [fmt(s, precision=3, commas=False) for s in survivals]
    viable_str = fmt_percent(viable, precision=0, style="prose")
    required_accuracy_str = fmt(required_accuracy, precision=5, commas=False)
    error_one_in_str = fmt_int(int(round(error_one_in, -2)))
    io_wait_str = fmt_percent(io_wait, precision=0, style="prose")
    wait_bound_mult_str = fmt_multiple(wait_bound_speedup, precision=1)
```

As established in @eq-vol3-sovereign-law-horizon, the mathematical ceiling of unverified autonomy becomes apparent. If an engineering team deploys a frontier model with an impressive `{python} HorizonCeiling.accuracy_str` per-step accuracy:

- Over a short horizon of $H = $ `{python} HorizonCeiling.h_short_str` steps, $P(\text{success})$ is `{python} HorizonCeiling.s_short_num_str`, or `{python} HorizonCeiling.s_short_str`.
- Over a moderate horizon of $H = $ `{python} HorizonCeiling.h_mid_str` steps, `{python} HorizonCeiling.s_mid_num_str`, or `{python} HorizonCeiling.s_mid_str`.
- Over an enterprise horizon of $H = $ `{python} HorizonCeiling.h_long_str` steps, `{python} HorizonCeiling.s_long_num_str`, or `{python} HorizonCeiling.s_long_str`.

To achieve a commercially viable `{python} HorizonCeiling.viable_str` trajectory success rate over `{python} HorizonCeiling.h_long_str` steps through raw model scaling alone would require an absurd per-step accuracy of `{python} HorizonCeiling.required_accuracy_str`, an error rate of about one step in `{python} HorizonCeiling.error_one_in_str`. Because foundation models are probabilistic estimators trained on human web data, achieving such ultra-low error rates across out-of-distribution environments is mathematically intractable.
...
=======
In @eq-vol3-sovereign-law-verified, when $v_t \to 1$ (complete verification), $P(\tau_{\text{verified}}) \to 1$, independent of horizon $H$. Verification is not an optional software feature; it is the mathematical precondition for long-horizon autonomous systems. @Fig-vol3-trajectory-decay draws both families on one axis, and the distinction is structural rather than quantitative: raising step accuracy shifts an unverified curve slightly, while inserting a verifier changes the shape of the decay itself.

::: {#fig-vol3-trajectory-decay fig-env="figure" fig-pos="htb" fig-cap="**The Sovereign Law of Agency**: Trajectory success probability against horizon for unverified and verified execution at several step accuracies. Unverified curves fall steeply away from the 95 percent target while verified curves stay near it, which is the graphical form of verification breaking the exponent rather than improving the base." fig-alt="Four decay curves of trajectory success probability against task horizon comparing unverified and verified execution at different step accuracies. Unverified curves fall steeply below a marked 95 percent reliability target while verified curves stay near it."}
...
df = pd.read_csv("vol3/conclusion/data/trajectory_decay.csv")

fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 5))

plt.plot(df["Horizon"], df["Unverified_95"], linestyle='--', color='#d62728', label=r"Unverified ($p=0.95, v=0$)")
plt.plot(df["Horizon"], df["Unverified_98"], linestyle='--', color='#ff7f0e', label=r"Unverified ($p=0.98, v=0$)")
plt.plot(df["Horizon"], df["Verified_98_v70"], linewidth=2, color='#1f77b4', label=r"Verified ($p=0.98, v=0.7$)")
plt.plot(df["Horizon"], df["Verified_98_v90"], linewidth=2, color='#2ca02c', label=r"Verified ($p=0.98, v=0.9$)")

plt.axhline(y=0.95, color='gray', linestyle=':', label="95% Target Reliability")

plt.xlabel("Task Horizon ($H$ steps)", fontsize=11)
plt.ylabel(r"Trajectory Success Probability $P(\tau)$", fontsize=11)
plt.title("Exponential decay vs. verification", fontsize=12, fontweight='bold')
plt.ylim(0, 1.05)
plt.xlim(1, 100)
...
1. **Amdahl's Bottleneck in Agent Execution:** While GPU inference speeds (tokens per second) scale with newer hardware accelerators, agentic workflows are intrinsically bottlenecked by sequential environmental interactions. If an agent spends `{python} HorizonCeiling.io_wait_str` of its wall-clock time waiting for an external Python interpreter to execute a script or a search index to return results, an infinitely fast GPU can only ever yield a `{python} HorizonCeiling.wait_bound_mult_str` overall task speedup. System acceleration requires asynchronous tool dispatch, overlapping speculative decoding with I/O waits, and preemptive background retrieval.
...
::: {#fig-vol3-improvement-ladder fig-env="figure" fig-pos="htb" fig-cap="**The Autonomous Improvement Ladder, Systems Scaffolding vs. Weight Adaptation**: Five-tier decision hierarchy balancing compute cost, blast radius, and determinism before modifying model weights. Ordered by reversibility rather than capability, every rung upward costs more compute and returns less determinism, so the engineering question is the lowest rung that solves the problem." fig-alt="Vertical diagram showing 5 rungs from Prompt Engineering at the bottom to Post-Training at the top, annotated with cost and risk."}
...
Yet the central lesson of this book is that **system reliability is an emergent property of the software architecture, not the model weights**. By surrounding stochastic models with deterministic systems scaffolding—Agent Control Blocks, Radix prefix caches, microVM sandboxes, multi-agent supervisor trees, W3C distributed tracing, and sequential statistical gates—we build mission-critical, enterprise-grade software out of probabilistic components.

## Unsolved Frontiers in Agent Systems {#sec-vol3-conclusion-unsolved-frontiers-in-agent}

While this book has formalized the foundational architecture of agentic machine learning systems, several open research frontiers remain at the intersection of computer systems and artificial intelligence:
...
5. **The Bridge to Embodied Physical AI:** Expanding the agentic systems harness from digital sandboxes to real-time cyber-physical systems operating under hard real-time latency deadlines ($\tau \le 20\text{ ms}$) and physical Newtonian dynamics—the core focus of physical AI and embodied robotics.
...
The discipline of agentic machine learning systems is building deterministic, reliable software applications out of non-deterministic, probabilistic components. Across fifteen chapters, this book has established that system reliability, capability, and security are not innate properties of model weights, but emergent properties of the surrounding systems architecture.

::: {.callout-takeaways title="The durable principles of agentic systems"}

- **The bitter lesson at inference time:** General-purpose test-time search and dynamic memory hierarchies systematically outperform handcrafted prompt-chaining heuristics and bespoke conversational state machines.
- **The Invariant Closure Principle:** Critical operational invariants (token budgets, capability leases, taint isolation, transaction rollbacks) cannot be enforced per-token; they must close at the trajectory boundary governed by the systems runtime.
- **The Sovereign Law of Agency:** Unverified trajectory reliability decays exponentially ($p^H$), mandating intermediate deterministic verification oracles, bounded error tolerances, and dynamic context working sets.
- **The Reversibility Boundary:** Internal scratchpad mutations are private, ephemeral, and reversible; external operations mutate shared reality and mandate two-phase sandboxing, idempotency keys, and compensating Sagas.
- **The Broken Code/Data Separation:** Autoregressive Transformers lack hardware $W \oplus X$ protection, making indirect prompt injection an architectural invariant that requires virtualization sandboxing and runtime information flow control.
- **Postel's Law in Agentic I/O:** Runtimes must leverage neural networks to liberally accept unstructured environmental entropy while enforcing strict, deterministic, and conservative structured tool schemas.
- **Amdahl's Law and The Tail at Scale:** Agentic workflow speedup is fundamentally bound by sequential tool execution bottlenecks and the compounding latency variance of distributed stochastic subagents.
- **The Autonomous Improvement Ladder:** Engineering teams must exhaust prompt engineering, context working sets, tool ABIs, and runtime sandboxing before incurring the cost and drift of modifying neural weights.
- **The Progressive Autonomy Spectrum:** Operational authority must be calibrated progressively across shadow mode, guarded previews, time-bounded Sagas, and statistical canary gates bounded by verification density.
- **Fault Tolerance from Stochastic Parts:** High system reliability is an emergent property of the systems harness, fulfilling John von Neumann's and Jim Gray's foundational systems visions of synthesizing reliable software from unreliable components.
- **The Systems Architecture Arc:** This book establishes the digital systems foundations for stateful trajectory execution, bridging single-node kernel execution and distributed serving fleets to the ultimate frontier: real-time physical AI and embodied robotics.

:::

Agentic systems engineering proves that the path to dependable autonomy does not lie in waiting for perfectly reliable foundation models, but in architecting runtimes that hold stochastic reasoning to deterministic invariants. By bounding context, enforcing typed tool contracts, isolating side effects, and orchestrating distributed coordination, engineers transform fluid reasoning into dependable software systems.

::: {.callout-chapter-connection title="From software trajectories to physical embodiment"}
The journey across this book operated entirely behind the glass of digital systems, where failed actions can be sandboxed, rollbacks are possible via copy-on-write filesystems, and latency costs only developer patience. But when autonomous intelligence is embedded into physical machines—quadrupeds, manipulators, and vehicles—computational delay translates directly into physical displacement ($\Delta x = \int v \, dt$), actions cannot be un-executed, and safety requires hard real-time guarantees. The next domain takes this leap: turning from software trajectories to physical AI, where machine learning systems must sense and act within the irreversible constraints of the physical world.
:::
