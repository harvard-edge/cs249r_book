...
$$\text{request} = \langle \text{messages}, \text{tools}, \text{model}, K_{\max}, T_{\max}, B_{\text{reason}}, \text{stop sequences}, (\tau, p) \rangle$$ {#eq-vol3-call-request}
...
$$\text{envelope} = \langle \text{status}, \text{stop reason}, \text{content}, \text{usage} \rangle$$ {#eq-vol3-status-envelope}
...
These statuses describe one call. A trajectory has its own closed set of states (@sec-vol3-controlplane-statemachine), and where a name recurs there, as `COMPLETED` and `CANCELED` do, it describes the whole run rather than a single call. The closed set lets the runtime state its safety rule for a single call in terms of one field.
...
The runtime therefore checks proposals with tools that hold ground truth about the environment. Arranged in order of cost, they form a **verification perimeter**\index{Verification perimeter} that rejects cheap failures cheaply and spends time only on proposals that survive (@tbl-verification-layers). Each layer supplies one of the closure evidence levels of @sec-vol3-intro-closure-evidence.
