...
John von Neumann [-@vonneumann1945edvac] organized the stored-program computer as a small set of organs (arithmetic, control, memory, input, and output), each responsible for one part of the job. This book organizes the accountable machine built around a model in the same way and calls it the **Stochastic Computer**. Its subsystems decide which part of the book owns which problem. The model call is studied as the machine's processor in Part I, the state a task carries as its memory in Part II, the boundary where a proposal becomes an effect as its I/O in Part III, and the supervisor that orders, records, and recovers a trajectory as its operating system in Part IV. Two further subsystems work across tasks rather than within one, a policy compiler that turns verified trajectories into better weights (Part V) and a fleet that runs many trajectories on shared hardware (Part VI). The names locate each problem in the machine. They do not claim that a model is a processor or that a context window is a cache, and each chapter argues its subsystem in that subsystem's own terms. @Sec-vol3-intro-stochastic-computer draws the full blueprint.
...
```{python}
#| label: lego-01-temporal-expansion
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TEMPORAL EXPANSION AND ERROR COMPOUNDING ACROSS AGENT HORIZONS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-temporal-expansion
# │ Goal: Quantify trajectory duration and open-loop error compounding across multi-turn steps.
# │ Show: 45-step task takes 1,035 s (17.25 min); 96% step accuracy collapses to 15.9% open-loop success.
# │ How: Sum model, tool, and host latencies per turn; compound step fidelity exponentially (p^N).
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
class TemporalExpansion:
    """Temporal expansion and error compounding across agent horizons."""
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    t_step = t_model + t_tool + t_host
    t_total = n_steps * t_step
    t_total_min = t_total.to(minute)
    p_open_loop = p_step**n_steps
    p_required = p_target ** (1.0 / n_steps)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(t_step.to(second).magnitude == 23.0, "t_step")
    check(t_total.to(second).magnitude == 1035.0, "t_total")
    check(abs(t_total_min.magnitude - 17.25) < 0.01, "t_total_min")
    check(abs(p_open_loop - 0.1593) < 0.005, "p_open_loop")
    check(abs(p_required - 0.99766) < 0.0001, "p_required")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_steps_str = fmt_int(n_steps)
    tokens_proposal_str = fmt_int(tokens_proposal)
    t_model_s_str = fmt_time(t_model, unit=second, precision=1)
    t_tool_s_str = fmt_time(t_tool, unit=second, precision=0)
    t_host_s_str = fmt_time(t_host, unit=second, precision=1)
    p_step_pct_str = fmt_percent(p_step, precision=0, style="prose")
    p_required_approx_pct_str = fmt_percent(p_required, precision=1, style="prose")
```

::: {#nbk-temporal-expansion .callout-notebook title="Temporal expansion and error compounding across agent horizons"}
Consider an autonomous coding agent tasked with isolating and repairing a concurrency bug across a legacy codebase. The agent operates over a trajectory of $N =$ `{python} TemporalExpansion.n_steps_str` sequential execution turns. Each turn requires:

1. A foundation engine invocation that processes an average working context of $32\text{ KiB}$ ($8\text{k tokens}$) and generates an action proposal of `{python} TemporalExpansion.tokens_proposal_str` tokens ($\text{latency } t_{\text{model}} \approx$ `{python} TemporalExpansion.t_model_s_str`).
2. An actuation turn inside an isolated Linux container, executing a target test target via `pytest` or compiling a module via `cargo build` ($\text{latency } t_{\text{tool}} \approx$ `{python} TemporalExpansion.t_tool_s_str`).
3. Host supervisor overhead for sandboxed file I/O, output stream parsing, and invariant verification ($\text{latency } t_{\text{host}} \approx$ `{python} TemporalExpansion.t_host_s_str`).
...
Now consider the reliability boundary. Assume the foundation model possesses a per-step action selection accuracy of $p = 0.96$ (that is, the model emits an actionable, syntactically and semantically viable tool call `{python} TemporalExpansion.p_step_pct_str` of the time without entering a hallucinated or degenerative state).
...
Even with an extraordinarily capable model ($p = 0.96$), the open-loop completion rate collapses to less than one-in-six. To achieve an acceptable system-level completion rate ($P_{\text{target}} \ge 0.90$) across this `{python} TemporalExpansion.n_steps_str`-step horizon, an open-loop model would require a per-step fidelity of:
$$p = (P_{\text{target}})^{1/N} = (0.90)^{1/45} \approx 0.99766 \quad (99.77\%)$$
Demanding `{python} TemporalExpansion.p_required_approx_pct_str` unguided step accuracy from a stochastic language model over an ambiguous operational task is practically infeasible. Reliability cannot be solved at the token level; it must be engineered at the system level through closed-loop verification, state snapshotting, and transaction recovery.
:::
...
### The tripartite architecture {#sec-vol3-intro-tripartite-architecture}
...
### Eight systems dimensions of software execution {#sec-vol3-intro-eight-dimensions}
...
### Memory stranding friction {#sec-vol3-intro-memory-stranding}
...
```{python}
#| label: lego-01-tool-wait-tax
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ THE TOOL-WAIT MEMORY TAX (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-01-tool-wait-tax
# │ Goal: Dimension stranded HBM capacity and financial cost during synchronous tool calls.
# │ Show: Single 160k-token session pins 51.2 GB (10.67% dynamic pool); 4 agents strand 42.67% ($8/trajectory).
# │ How: Multiply context length by per-token KV footprint; evaluate fraction of dynamic memory and idle hourly rate.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_memory_capacity, fmt_usd
from mlsysim.physics.agents import calc_kv_cache_bytes_per_token, calc_tool_wait_stranded_tax

class ToolWaitTax:
    """Memory stranding and idle silicon expenditure during blocking tool calls."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    agent = CodingAgents.SWE_Bench_Runner
    n_gpus = agent.serving_node.accelerators_per_node
    c_hbm_node = 640.0 * GB
    cost_rate_hr = 24.0
    tp_degree = 8
    m_weights = 160.0 * GB
    t_context = 160_000
    m_token = calc_kv_cache_bytes_per_token(agent)
    t_wait = 60 * second
    n_agents_concurrent = 4
    n_tools_pipeline = 20

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    m_kv_gb = 51.2 * GB
    tax = calc_tool_wait_stranded_tax(
        node_hbm_capacity=c_hbm_node,
        model_weights_memory=m_weights,
        kv_memory=m_kv_gb,
        tool_wait_duration=t_wait,
        hourly_node_cost=cost_rate_hr,
        num_gpus=n_gpus,
        num_concurrent_agents=n_agents_concurrent,
        num_pipeline_tools=n_tools_pipeline,
    )
    cost_rate_s = cost_rate_hr / 3600.0
    m_hbm_dynamic = tax["m_hbm_dynamic"]
    m_hbm_dynamic_per_gpu = tax["m_hbm_dynamic_per_gpu"]
    pct_stranded_single = tax["pct_stranded_single"]
    m_stranded_concurrent = tax["m_stranded_concurrent"]
    pct_stranded_concurrent = tax["pct_stranded_concurrent"]
    cost_turn = tax["cost_turn"]
    cost_pipeline = tax["cost_pipeline"]

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(m_token.to(KiB).magnitude - 320.0) < 1e-6, "m_token is 320 KiB")
    check(m_hbm_dynamic.magnitude == 480.0, "m_hbm_dynamic")
    check(m_hbm_dynamic_per_gpu.magnitude == 60.0, "m_hbm_dynamic_per_gpu")
    check(abs(pct_stranded_single - 0.1067) < 0.001, "pct_stranded_single")
    check(m_stranded_concurrent.magnitude == 204.8, "m_stranded_concurrent")
    check(abs(pct_stranded_concurrent - 0.4267) < 0.001, "pct_stranded_concurrent")
    check(abs(cost_turn - 0.40) < 0.01, "cost_turn")
    check(abs(cost_pipeline - 8.00) < 0.01, "cost_pipeline")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_gpus_str = fmt_int(n_gpus)
    cost_rate_hr_str = fmt_usd(cost_rate_hr)
    tp_degree_str = fmt_int(tp_degree)
    m_weights_gb_str = fmt_memory_capacity(m_weights, unit=GB, precision=0)
    m_hbm_dynamic_gb_str = fmt_memory_capacity(m_hbm_dynamic, unit=GB, precision=0)
    m_hbm_dynamic_per_gpu_gb_str = fmt_memory_capacity(m_hbm_dynamic_per_gpu, unit=GB, precision=0)
    t_context_str = fmt_int(t_context)
    t_wait_s_str = fmt_time(t_wait, unit=second, precision=0)
    pct_stranded_single_pct_str = fmt_percent(pct_stranded_single, precision=2, style="prose")
    n_agents_concurrent_str = fmt_int(n_agents_concurrent)
    m_stranded_concurrent_gb_str = fmt_memory_capacity(m_stranded_concurrent, unit=GB, precision=1)
    pct_stranded_concurrent_pct_str = fmt_percent(pct_stranded_concurrent, precision=2, style="prose")
    cost_turn_str = fmt_usd(cost_turn, precision=2)
    n_tools_pipeline_str = fmt_int(n_tools_pipeline)
    cost_pipeline_str = fmt_usd(cost_pipeline)
```
...
* **Cluster Hardware**: `{python} ToolWaitTax.n_gpus_str`-GPU NVIDIA H100 SXM5 node ($640\text{ GB}$ HBM3, `{python} ToolWaitTax.cost_rate_hr_str`/hour or \$0.00667/s).
* **Model Partitioning**: 80B dense parameters partitioned across $TP =$ `{python} ToolWaitTax.tp_degree_str`. Static weights consume `{python} ToolWaitTax.m_weights_gb_str`, leaving `{python} ToolWaitTax.m_hbm_dynamic_gb_str` dynamic HBM (`{python} ToolWaitTax.m_hbm_dynamic_per_gpu_gb_str` per GPU).
* **Context State**: Active trajectory context of $T =$ `{python} ToolWaitTax.t_context_str` tokens. An 80B-parameter model requires $m_{\text{token}} = 320\text{ KiB}$ of Key-Value state per token (@sec-vol3-appendix-inference-kv-cache derives this tensor geometry).
* **Tool Duration**: A long-running compilation and integration test: `cmake --build out/ && ctest --timeout 60` requiring $t_{\text{wait}} =$ `{python} ToolWaitTax.t_wait_s_str`.
...
1. **Memory Stranding**: The $51.2\text{ GB}$ KV cache is pinned for the entire `{python} ToolWaitTax.t_wait_s_str`. This represents $51.2 / 480 =$ `{python} ToolWaitTax.pct_stranded_single_pct_str` of the entire dynamic serving capacity of the node. If `{python} ToolWaitTax.n_agents_concurrent_str` concurrent agents trigger compilation tasks simultaneously, `{python} ToolWaitTax.m_stranded_concurrent_gb_str` of dynamic memory—fully `{python} ToolWaitTax.pct_stranded_concurrent_pct_str` of the node's interactive capacity—is completely stranded, unable to process tokens for active users.
2. **Financial Cost**: Over the `{python} ToolWaitTax.t_wait_s_str` compilation wait interval:
$$\text{Cost} = 60\text{ s} \times \$0.00667/\text{s} = \$0.400$$
The system incurs `{python} ToolWaitTax.cost_turn_str` of direct compute expenditure per turn while performing zero floating-point operations. In an industrial pipeline executing `{python} ToolWaitTax.n_tools_pipeline_str` tool calls per resolution trajectory, tool-wait stranding burns `{python} ToolWaitTax.cost_pipeline_str` of capital entirely on idle silicon.

**Systems insight**: Software 3.0 runtimes cannot treat tool invocations as standard blocking function calls. The host supervisor must decouple tool execution asynchronously: serializing token state, releasing accelerator memory allocations, recording progress in a Write-Ahead Log (@sec-vol3-durable-execution), and leveraging prefix caching (@sec-vol3-kv-cache) when the observation returns.
...
The fundamental contrast in execution flow between traditional inference and an autonomous agent loop is visualized in @fig-vol3-agent-execution-timeline. In traditional model serving (top bar), inference runs as an uninterrupted burst of continuous token generation where accelerator tensor cores achieve near-100 percent utilization. In contrast, an autonomous agent trajectory (bottom bar) is severely fragmented: during an 8.5-minute software engineering trajectory on SWE-bench Verified [@jimenez2024swebench], the agent spends 420 seconds (82.4 percent of makespan) blocked on non-neural host operations—container workspace setup, filesystem search via `ripgrep`, and `pytest` suite execution. In a synchronous runtime, these tool waits keep accelerator memory pinned while the server chassis idles.

::: {#fig-vol3-agent-execution-timeline fig-env="figure" fig-pos="htb" fig-cap="**Autonomous Agent Loop Execution Timeline**: Comparison of continuous accelerator token generation in traditional stateless serving against the fragmented timeline of an autonomous agent trajectory on SWE-bench Verified [@jimenez2024swebench]. While stateless inference sustains continuous accelerator tensor core utilization, the agent loop spends 420 seconds (82.4 percent of makespan) waiting on non-neural host operations—Docker container boot, ripgrep workspace indexing, and pytest suite compilation. In a synchronous runtime, these tool waits keep accelerator memory pinned while the server chassis draws continuous idle-wait power." fig-alt="Execution timeline comparing traditional model serving against an autonomous agent loop. The top green bar represents traditional serving with continuous token generation and near-100 percent accelerator utilization. The bottom segmented bar illustrates an 8.5-minute autonomous agent loop with alternating green neural think bursts and long gray and red tool execution phases, with a dashed red bracket indicating 420 seconds of non-neural tool waits that strand accelerator high-bandwidth memory."}

```{python}
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ AUTONOMOUS AGENT LOOP EXECUTION TIMELINE (FIGURE)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @fig-vol3-agent-execution-timeline—execution timeline trace
# │
# │ Goal: Contrast continuous token generation against fragmented agent loops.
# │ Show: 420s of non-neural tool waits stranding GPU memory during trajectories.
# │ How: Dual-bar Gantt execution trace with tool category phases.
# └─────────────────────────────────────────────────────────────────────────────
import sys
...
phases = [
    ...
    (45, 30, "Neural\nThink", "#16A34A", "#FFFFFF", "GPU"),
    (75, 120, "File Search & Ripgrep\n(Host CPU & NVMe)", "#CBD5E1", "#1E293B", "Host OS"),
    (195, 30, "Neural\nThink", "#16A34A", "#FFFFFF", "GPU"),
    (225, 255, "Test Suite Compilation & Pytest Verification\n(Isolated Container Sandbox)", "#FEE2E2", "#991B1B", "Host Sandbox"),
    (480, 30, "Neural\nThink", "#16A34A", "#FFFFFF", "GPU"),
]
...
for x_start, width, label, fc, tc, sub in phases:
    ...
    ax.text(
        ...
        va="center",
        color=tc,
        fontsize=7.0 if width < 50 else 7.6,
        fontweight="bold",
    )
...
ax.text(
    ...
    fontsize=7.8,
    fontweight="bold",
    bbox=dict(boxstyle="round,pad=0.35", fc="#FEF2F2", ec="#FECACA", lw=0.8),
)
...
The empirical consequence of this physical boundary is mapped in @fig-vol3-agent-systems-tax. Profiling an autonomous coding agent executing a benchmark task on SWE-bench Verified [@jimenez2024swebench] across an NVIDIA H100 accelerator and an AMD EPYC host server reveals the striking magnitude of the **Agent Systems Tax**. As shown in the left panel of @fig-vol3-agent-systems-tax, non-neural host operations—isolated Docker workspace setup (8.1 kJ, 6.9 percent), file indexing and ripgrep queries (16.8 kJ, 14.4 percent), and pytest suite compilation and verification (56.1 kJ, 48.0 percent)—account for fully 69.3 percent of total AC mains electrical energy consumption (81.0 kJ), while pure neural token generation on accelerator tensor cores consumes only 30.7 percent (35.9 kJ). In the right panel, these tool waits and sandbox operations account for 82.4 percent of wall-clock makespan (420 seconds), while active token generation represents only 17.6 percent (90 seconds). This empirical profile mathematically confirms the central thesis: autonomous agent performance, reliability, and operating costs are dominated by host operating system orchestration, virtualization sandboxes, and verification harnesses, rather than neural model scaling alone.

::: {#fig-vol3-agent-systems-tax fig-env="figure" fig-pos="htb" fig-cap="**The Agent Systems Tax**: Measured AC mains electrical energy and wall-clock execution time breakdown across subsystems during an 8.5-minute autonomous code repair trajectory on SWE-bench Verified [@jimenez2024swebench]. The left panel shows AC mains electrical energy consumption, where non-neural host operations (pytest verification, file search, container setup) consume 69.3 percent of total energy (81.0 kJ), while pure neural inference on GPU tensor cores accounts for only 30.7 percent (35.9 kJ). The right panel shows the wall-clock time distribution, where tool execution and sandbox management account for 82.4 percent of makespan (420 seconds), while active token generation represents only 17.6 percent (90 seconds)." fig-alt="Two-panel horizontal bar chart illustrating the Agent Systems Tax. The left panel shows electrical energy share: Pytest Verification at 48.0 percent in red, Neural Token Generation at 30.7 percent in green, File Search at 14.4 percent in slate, and Docker Boot at 6.9 percent in light slate, summing to 69.3 percent host energy. The right panel shows wall-clock execution time share: Pytest Verification at 50.0 percent (255 seconds), File Search at 23.5 percent (120 seconds), Neural Token Generation at 17.6 percent (90 seconds), and Docker Boot at 8.8 percent (45 seconds), summing to 82.4 percent host time."}

```{python}
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ THE AGENT SYSTEMS TAX (FIGURE)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @fig-vol3-agent-systems-tax—energy and time breakdown
# │
# │ Goal: Quantify non-neural host operations vs. neural token generation.
# │ Show: 69.3 percent AC energy and 82.4 percent wall time in non-neural host tasks.
# │ How: Dual-panel horizontal bar charts for energy (kJ) and time (s).
# └─────────────────────────────────────────────────────────────────────────────
import sys
from pathlib import Path
...
fig, (ax_en, ax_tm) = plt.subplots(1, 2, figsize=(10.5, 3.2))

categories = [
    "Docker Boot & Workspace",
    "File Search & Ripgrep",
    "Neural Token Generation",
    "Pytest Verification",
]
energies_kj = [8.1, 16.8, 35.9, 56.1]
total_en = sum(energies_kj)
percentages_en = [e / total_en * 100.0 for e in energies_kj]

durations_s = [45, 120, 90, 255]
total_tm = sum(durations_s)
percentages_tm = [d / total_tm * 100.0 for d in durations_s]

y_pos = np.arange(len(categories))
bar_colors = ["#94A3B8", "#64748B", "#16A34A", "#CB202D"]

# Left: Energy
bars_en = ax_en.barh(y_pos, percentages_en, color=bar_colors, edgecolor="#1E293B", linewidth=0.8, height=0.52, zorder=3)
ax_en.set_xlim(0, 68)
ax_en.set_yticks(y_pos)
ax_en.set_yticklabels(categories, fontsize=8.0, fontweight="bold")
ax_en.set_xlabel("Share of total AC mains energy (%)", fontsize=8.5, fontweight="bold")
ax_en.grid(axis="x", linestyle="--", linewidth=0.5, color="#CBD5E1", alpha=0.7, zorder=1)
ax_en.spines["top"].set_visible(False)
ax_en.spines["right"].set_visible(False)

for bar, pct, kj in zip(bars_en, percentages_en, energies_kj):
    w = bar.get_width()
    ax_en.text(
        w + 1.2,
...
# Right: Time
bars_tm = ax_tm.barh(y_pos, percentages_tm, color=bar_colors, edgecolor="#1E293B", linewidth=0.8, height=0.52, zorder=3)
ax_tm.set_xlim(0, 68)
ax_tm.set_yticks(y_pos)
ax_tm.set_yticklabels([])
ax_tm.set_xlabel("Share of total wall-clock time (%)", fontsize=8.5, fontweight="bold")
ax_tm.grid(axis="x", linestyle="--", linewidth=0.5, color="#CBD5E1", alpha=0.7, zorder=1)
ax_tm.spines["top"].set_visible(False)
ax_tm.spines["right"].set_visible(False)

for bar, pct, dur in zip(bars_tm, percentages_tm, durations_s):
    w = bar.get_width()
    ax_tm.text(
        w + 1.2,
        bar.get_y() + bar.get_height() / 2.0,
...
Just as an operating system kernel tracks process state, open file descriptors, and virtual memory page tables inside a Process Control Block (PCB), an agent host runtime tracks token budgets, tool permissions, and context memory descriptors inside an Agent Control Block (ACB). We develop the full ACB data structure and scheduling queue discipline in @sec-vol3-agent-harness.
:::

The ACB decouples the state of the task from the lifetime of any single accelerator process or HTTP connection. If a GPU worker node crashes or an inference stream drops during step $t$, the runtime uses the ACB to reconstruct context state from persistent Write-Ahead Logs (@sec-vol3-durable-execution), re-establish sandboxed tool boundaries (@sec-vol3-sandboxes), and resume execution without losing intermediate progress. Furthermore, the ACB acts as an administrative boundary: enforcing hardware resource quotas (token ceilings and wall-clock duration), capability security profiles (restricting which system calls or network endpoints the agent may touch), and transactional rollback ledgers.
...
where $\mathcal{T}_{\text{all}}$ represents all dispatched trajectories, $\mathcal{T}_{\text{success}} \subseteq \mathcal{T}_{\text{all}}$ represents the subset whose deliverables pass all deterministic verification oracles $\mathcal{V}$, and $R_k$ measures physical resource expenditure (floating-point operations, GPU core-seconds, kilowatt-hours, or financial cost). Unverified token generation represents computational badput ($1 - \mathcal{G}$). We develop the full telemetry and evaluation mechanics of trajectory goodput in @sec-vol3-evaluation and analyze its macro-economic cost frontier in @sec-vol3-agent-economics.

```{python}
#| label: lego-01-trajectory-goodput
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TRAJECTORY GOODPUT AND COMPUTE WASTE (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-01-trajectory-goodput
# │ Goal: Compute trajectory goodput, badput ratio, and energy/dollar waste under cyclic failure.
# │ Show: 21.2% goodput (78.8% badput); 65 failed tasks waste 1.85 kWh ($12.33) on an 8x H100 node.
# │ How: Weight successful vs failed trajectory steps; compute decode token execution time and node energy.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt, fmt_int, fmt_percent, fmt_usd, fmt_power

class TrajectoryGoodput:
    """Trajectory goodput, compute waste, and energy dissipation in fail-plausible loops."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_gpus = 8
    power_cluster = 4_800 * watt
    throughput_decode = 1_200 / second
    n_trajectories = 100
    n_failed = 65
    t_max_steps = 50
    tokens_decode_step = 512
    tokens_prefill_step = 16_384
    n_success = 35
    steps_success = 25
    rate_h100_hr = 32.0
    n_tasks_enterprise = 100_000

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    steps_successful_total = n_success * steps_success
    steps_failed_total = n_failed * t_max_steps
    steps_total = steps_successful_total + steps_failed_total
    goodput = steps_successful_total / steps_total
    badput = 1.0 - goodput
    tokens_waste = n_failed * t_max_steps * tokens_decode_step
    t_waste_s = (tokens_waste / throughput_decode.magnitude) * second
    t_waste_hr = t_waste_s.to(hour)
    e_waste_mj = (power_cluster * t_waste_s).to(joule).magnitude / 1e6
    e_waste_kwh = (power_cluster * t_waste_s).to(kWh)
    cost_waste = t_waste_hr.magnitude * rate_h100_hr

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(steps_successful_total == 875, "steps_successful_total")
    check(steps_failed_total == 3250, "steps_failed_total")
    check(steps_total == 4125, "steps_total")
    check(abs(goodput - 0.212) < 0.005, "goodput")
    check(abs(badput - 0.788) < 0.005, "badput")
    check(tokens_waste == 1664000, "tokens_waste")
    check(abs(t_waste_s.magnitude - 1386.67) < 1.0, "t_waste_s")
    check(abs(t_waste_hr.magnitude - 0.385) < 0.01, "t_waste_hr")
    check(abs(e_waste_mj - 6.66) < 0.05, "e_waste_mj")
    check(abs(e_waste_kwh.magnitude - 1.85) < 0.05, "e_waste_kwh")
    check(abs(cost_waste - 12.33) < 0.05, "cost_waste")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_gpus_str = fmt_int(n_gpus)
    power_cluster_str = fmt_power(power_cluster, unit=watt, precision=0)
    throughput_decode_str = fmt_int(throughput_decode.magnitude)
    n_trajectories_str = fmt_int(n_trajectories)
    n_failed_str = fmt_int(n_failed)
    t_max_steps_str = fmt_int(t_max_steps)
    tokens_decode_step_str = fmt_int(tokens_decode_step)
    tokens_prefill_step_str = fmt_int(tokens_prefill_step)
    n_success_str = fmt_int(n_success)
    steps_success_str = fmt_int(steps_success)
    badput_pct_str = fmt_percent(badput, precision=1, style="prose")
    cost_waste_str = fmt_usd(cost_waste, precision=2)
```

::: {#nbk-01-trajectory-goodput .callout-notebook title="Trajectory goodput and compute waste"}

**Problem**: A data center cluster of `{python} TrajectoryGoodput.n_gpus_str` NVIDIA H100 SXM5 GPUs serves an open-weight 70B dense model in BF16 (drawing `{python} TrajectoryGoodput.power_cluster_str` at peak, generating `{python} TrajectoryGoodput.throughput_decode_str` decode tokens/s across streams). The cluster executes `{python} TrajectoryGoodput.n_trajectories_str` autonomous software repair trajectories. What is the cluster's Trajectory Goodput ($\mathcal{G}$) and energy waste under fail-plausible loops?

**Variables**:

* **Workload**: `{python} TrajectoryGoodput.n_trajectories_str` bug repair trajectories.
* **Failure Dynamics**: Due to unmitigated fail-plausible errors, `{python} TrajectoryGoodput.n_failed_str` trajectories enter cyclic loops, exhausting step budget $T_{\max} =$ `{python} TrajectoryGoodput.t_max_steps_str` (`{python} TrajectoryGoodput.tokens_decode_step_str` decode tokens and `{python} TrajectoryGoodput.tokens_prefill_step_str` prefill tokens per step).
* **Success Dynamics**: `{python} TrajectoryGoodput.n_success_str` trajectories pass all verification tests with an average of `{python} TrajectoryGoodput.steps_success_str` steps.
* **Power Draw**: `{python} TrajectoryGoodput.power_cluster_str` average active cluster power.
...
2. *Trajectory Goodput*:
   ...
   Despite high GEMM/GEMV kernel utilization, **`{python} TrajectoryGoodput.badput_pct_str` of the cluster's compute was badput.**

3. *Energy Waste and Financial Expenditure*:
   Failing trajectories generated $65 \times 50 \times 512 = 1{,}664{,}000\text{ decode tokens}$. At `{python} TrajectoryGoodput.throughput_decode_str` tokens/s, the wasted execution duration is:
   ...
   At an on-demand market rate of $\$32.00/\text{hr}$ for an 8x H100 SXM5 node, $0.385\text{ hours}$ of pure badput wastes `{python} TrajectoryGoodput.cost_waste_str` across just these `{python} TrajectoryGoodput.n_failed_str` failed tasks. At enterprise fleet scale ($100{,}000\text{ tasks}$), unmitigated cyclic failure inflates to $2{,}846\text{ hours}$ of stalled GPU time, $2.85\text{ MWh}$ of dissipated grid energy, and over $\$18{,}900$ in wasted capital for zero delivered software artifacts.
...
```{python}
#| label: lego-01-logit-vs-invariant
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ LOGIT CERTAINTY VERSUS SEMANTIC INVARIANT VIOLATION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-logit-vs-invariant
# │ Goal: Contrast statistical token certainty with database lock invariant violations.
# │ Show: Statement emits with 0.775 sequence certainty but incurs a 2.13 s table lock (42.7x over limit).
# │ How: Compute joint BPE likelihood vs NVMe table rewrite bandwidth over 10^7 rows.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt, fmt_int, fmt_time

class LogitVsInvariant:
    """Discrepancy between high autoregressive logit certainty and operational invariant violations."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    n_rows = 10_000_000
    t_lock_max = 50 * millisecond
    n_tokens = 14
    p_avg_token = 0.982
    h_token_nats = 0.084
    bw_nvme = 1.2 * (GB / second)
    bytes_per_row = 256 * byte

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    p_seq = p_avg_token**n_tokens
    data_volume = (n_rows * bytes_per_row).to(GB)
    t_rewrite = (data_volume / bw_nvme).to(second)
    t_rewrite_ms = t_rewrite.to(millisecond)
    violation_ratio = (t_rewrite_ms / t_lock_max).magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(p_seq - 0.775) < 0.005, "p_seq")
    check(abs(data_volume.magnitude - 2.56) < 0.01, "data_volume")
    check(abs(t_rewrite.magnitude - 2.133) < 0.01, "t_rewrite")
    check(abs(violation_ratio - 42.67) < 0.1, "violation_ratio")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    t_lock_max_ms_str = fmt_time(t_lock_max, unit=millisecond, precision=0)
    n_tokens_str = fmt_int(n_tokens)
    bytes_per_row_str = fmt_int(bytes_per_row.to(byte).magnitude)
    violation_ratio_str = fmt(violation_ratio, precision=1, commas=False)
    p_seq_str = fmt(p_seq, precision=3, commas=False)
    t_rewrite_s_val_str = fmt(t_rewrite.to(second).magnitude, precision=2, commas=False)
```

::: {#nbk-logit-vs-invariant .callout-notebook title="Logit certainty versus semantic invariant violation"}
Consider an autonomous database administration agent tasked with executing an online schema migration on a relational table containing $10^7$ rows. The operational invariant $\mathcal{I}_{\text{perf}}$ mandates that no table lock may be held for longer than `{python} LogitVsInvariant.t_lock_max_ms_str`, ensuring that concurrent production read transactions do not stall.
...
1. **Token Probability Calculation:**
   The statement decomposes into $L =$ `{python} LogitVsInvariant.n_tokens_str` BPE tokens. Across all `{python} LogitVsInvariant.n_tokens_str` tokens, the model's logits produce an average conditional probability:
...
2. **Environmental Invariant Evaluation:**
   ...
   The lock duration exceeds the operational invariant threshold by a factor of `{python} LogitVsInvariant.violation_ratio_str`:
   $$\text{Violation Ratio} = \frac{2130\text{ ms}}{50\text{ ms}} = 42.6 \implies s_{t+1} \notin \mathcal{I}_{\text{perf}}$$

3. **Systems Takeaway:**
   The execution core emitted the statement with high token certainty ($P =$ `{python} LogitVsInvariant.p_seq_str`) and zero syntax errors. When executed via a database CLI, the statement completes and exits with status 0. Yet it induces a `{python} LogitVsInvariant.t_rewrite_s_val_str`-second global lock that cascades into connection pool exhaustion across upstream microservices. The model's internal confidence metric was entirely blind to the physical constraints of storage and lock contention.
:::

### Context poisoning dynamics {#sec-vol3-intro-context-poisoning}
...
::: {.column-margin}
The Invariant Closure Principle is the agentic realization of the principle of least privilege: an unprivileged neural core operates under zero ambient authority, holding candidate actions in memory escrow until deterministic runtime gatekeepers validate them.
:::

### The dual guarantees of the end-to-end boundary {#sec-vol3-intro-end-to-end-boundary}
...
Lower runtime layers can easily enforce bounded physical properties, such as preventing a child process from issuing network sockets, bounding memory allocation to 2 gigabytes, or terminating execution after thirty seconds. These low-level mechanisms are indispensable because they guarantee physical containment: no matter what token sequence the neural policy $\pi_\theta$ emits, the host environment cannot be compromised beyond the declared sandbox boundary. By establishing Linux mount and user namespace boundaries and enforcing strict path traversal canonicalization (resolving symlinks and verifying `realpath` within designated worktrees to thwart relative path escapes such as `../../etc/passwd`), the supervisor makes zero ambient authority physical. However, the lower layers cannot certify that the application's ultimate goal has been satisfied. A Linux container runtime can guarantee that an agent did not write outside `/workspace/repo`, but it cannot determine whether the Python patch applied inside that directory fixed a race condition or introduced a subtle numerical regression.

Conversely, the neural core cannot substitute for low-level mechanical enforcement. Because the foundation model operates as an unprivileged predictor under zero ambient authority, it cannot be granted ambient permission to execute operating system primitives based on its own promise of good behavior. If an invariant must hold under all execution paths, such as the invariant that an agent must never bill more than \$5.00 of cloud infrastructure per task, that property must be closed mechanically by the host supervisor. Functional task correctness, meanwhile, can only be established at the end-to-end application boundary by collecting concrete evidence: compiler exit codes, unit test matrices, linters, and cryptographically verified artifact hashes.

### Mechanical invariant closure below the model {#sec-vol3-intro-mechanical-closure}
...
The concrete datapath of this operator is detailed in @fig-vol3-invariant-closure. When the unprivileged foundation model emits a candidate action proposal $a_{\text{prop}}$, the host supervisor immediately captures the payload in host memory escrow. Before any execution primitive is invoked, the proposal must clear three sequential mechanical gates:
...
### The epistemic boundary: Enforced envelopes versus semantic correctness {#sec-vol3-intro-epistemic-boundary}
...
```{python}
#| label: lego-01-blast-radius-containment
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ BLAST-RADIUS AND COST CONTAINMENT UNDER FAIL-PLAUSIBLE LOOPS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-blast-radius-containment
# │ Goal: Bound financial exposure under fail-plausible retry loops via hard invariants.
# │ Show: Mechanical escrow bounds spend to $4.98 at step 24 (94.7% reduction vs. $93.31 unbounded).
# │ How: Accumulate per-step prompt/completion token pricing up to escrow cap C_max.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_usd, fmt_time

class BlastRadiusContainment:
    """Financial blast-radius containment under fail-plausible retry loops."""
    ...
    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    cost_input_unbounded = n_steps_unbounded * k_context * (price_input_per_m / 1e6)
    cost_output_unbounded = n_steps_unbounded * l_completion * (price_output_per_m / 1e6)
    cost_total_unbounded = cost_input_unbounded + cost_output_unbounded
    tokens_total_unbounded = n_steps_unbounded * (k_context + l_completion)
    cost_per_step = (k_context * price_input_per_m / 1e6) + (l_completion * price_output_per_m / 1e6)
    cost_cumulative_24 = n_step_stop * cost_per_step
    cost_projected_25 = cost_cumulative_24 + cost_per_step
    cost_bounded = cost_cumulative_24
    reduction_pct = 1.0 - (cost_bounded / cost_total_unbounded)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(cost_input_unbounded - 86.40) < 0.01, "cost_input_unbounded")
    check(abs(cost_output_unbounded - 6.912) < 0.01, "cost_output_unbounded")
    check(abs(cost_total_unbounded - 93.312) < 0.01, "cost_total_unbounded")
    check(tokens_total_unbounded == 29_260_800, "tokens_total_unbounded")
    check(abs(cost_per_step - 0.20736) < 0.0001, "cost_per_step")
    check(abs(cost_cumulative_24 - 4.97664) < 0.0001, "cost_cumulative_24")
    check(abs(cost_projected_25 - 5.184) < 0.001, "cost_projected_25")
    check(abs(reduction_pct - 0.9467) < 0.001, "reduction_pct")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    k_context_str = fmt_int(k_context)
    l_completion_str = fmt_int(l_completion)
    price_input_per_m_str = fmt_usd(price_input_per_m)
    price_output_per_m_str = fmt_usd(price_output_per_m)
    n_steps_unbounded_str = fmt_int(n_steps_unbounded)
    t_max_steps_str = fmt_int(t_max_steps)
    c_max_str = fmt_usd(c_max)
    cost_bounded_str = fmt_usd(cost_bounded, precision=2)
    reduction_pct_str = fmt_percent(reduction_pct, precision=1, style="prose")
    n_step_stop_str = fmt_int(n_step_stop)
    t_timeout_hr_str = fmt_time(t_timeout, unit=hour, precision=0)
```

::: {#nbk-blast-radius-containment .callout-notebook title="Blast-radius and cost containment under fail-plausible loops"}
A software engineering team deploys an autonomous debugging agent driven by a foundation model $\pi_\theta$. The agent operates with an average context size of $K =$ `{python} BlastRadiusContainment.k_context_str` tokens per step. The API pricing structure costs `{python} BlastRadiusContainment.price_input_per_m_str` per million prompt tokens and `{python} BlastRadiusContainment.price_output_per_m_str` per million completion tokens. On each step, the model ingests the context and emits an average tool call completion of $L =$ `{python} BlastRadiusContainment.l_completion_str` tokens.
...
1. Hard Step Invariant: $T_{\max} =$ `{python} BlastRadiusContainment.t_max_steps_str` steps.
2. Hard Financial Escrow: Token bucket budget capped at $C_{\max} =$ `{python} BlastRadiusContainment.c_max_str`.
...
At step $t =$ `{python} BlastRadiusContainment.n_step_stop_str`, the cumulative expenditure reaches:
...
The process is terminated, the workspace diff is reverted via the write-ahead log, and the total financial expenditure is strictly bounded to `{python} BlastRadiusContainment.cost_bounded_str`. Mechanical invariant closure reduces financial blast radius by `{python} BlastRadiusContainment.reduction_pct_str` and limits execution steps from `{python} BlastRadiusContainment.n_steps_unbounded_str` to `{python} BlastRadiusContainment.n_step_stop_str`, regardless of the model's internal confusion.
:::
...
Not every task needs the top of this ladder, and not every task needs the same mechanical bounds. How much closure a task needs, in both halves, depends on how exposed the task is, which the task contract and its H·S·A exposures (@sec-vol3-intro-hsa) make precise.
...
Operational reliability in agentic systems begins not with prompt engineering, but with the formal specification of an operational contract. An unconstrained natural-language prompt conveys raw intent but zero systems boundaries; without a typed, bounded specification, the host supervisor cannot distinguish legitimate exploration from catastrophic state drift, nor can it mechanically verify whether a task has achieved completion. Bridging the epistemic gap between human delegation and autonomous execution demands translating ambiguous objectives into rigorous, five-part operational contracts whose horizon, state, and authority set the closure the runtime must supply.

### The five-part contract specification {#sec-vol3-intro-five-part-contract}
...
A task contract says what the agent may do and when it has finished. It does not say how much runtime the task needs. That depends on how far the task departs from a single model call, which reads a context, returns a proposal, and changes nothing. An agent departs from that call in three ways, and this book names them **H·S·A**, for Horizon, State, and Authority. Each is an exposure, a way the task can fail that one call cannot.
...
A task's H·S·A position says how exposed it is. The invariant closure principle (\ref{pri-invariant-closure}) says what the runtime must do about that exposure, and higher exposure demands more closure, both mechanical bounds below the model and stronger evidence above it. A long horizon needs step, time, and cost ceilings and checkpoints the runtime can resume from. Carried state needs a budget on what enters context and a rule for when a stored copy is stale. Authority needs a sandbox the runtime can reset at $A_1$, settlement of retries and compensations at $A_2$, and escrow with explicit approval before the irreversible step at $A_3$. The evidence level of @sec-vol3-intro-closure-evidence rises with the same exposure. Closure is therefore not a fourth axis. It is the runtime's answer to the other three. @Fig-vol3-hsa-taxonomy places four workloads in this space. A short, stateless query whose answer a person reads needs little beyond that reader, whereas a sandboxed repository repair over hundreds of turns needs step budgets, sandbox reset, and sealed tests, and an infrastructure change at $A_3$ needs all of that plus escrow.
...
Horizon has a price in wall-clock time, the serial time required to drive a trajectory from initial delegation to contract resolution. In interactive chatbots, latency is characterized primarily by Time-to-First-Token (TTFT) and token generation throughput. In an agentic system, model inference is merely one component of a multi-stage physical loop. The total wall-clock duration of a trajectory with horizon $H$ is governed by the serial duration accounting identity:
...
```{python}
#| label: lego-01-amdahl-accounting
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TRAJECTORY WALL-CLOCK AND AMDAHL LATENCY ACCOUNTING (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-amdahl-accounting
# │ Goal: Dimension wall-clock bottlenecks and speedups under Amdahl's Law for agent execution.
# │ Show: Model is 22.14% of runtime; 3.5x faster GPU yields 1.188x trajectory speedup vs 2.092x via toolchain caching.
# │ How: Profile prefill, decode, tool execution, and host runtime across multi-turn turns.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_multiple

class AmdahlAccounting:
    """Amdahl's law and wall-clock latency accounting in agent execution loops."""

    # ┌── 1. LOAD (Constants & Parameters) ─────────────────────────────────
    h_turns = 8
    s_prefill_tokens = 18_000
    t_prefill = 140 * millisecond
    t_decode = 4_950 * millisecond
    t_tool = 16.2 * second
    t_runtime = 1.4 * second
    t_wait = 0.3 * second
    s_model_speedup = 3.5
    t_tool_optimized = 4.2 * second

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    t_model_step = (t_prefill + t_decode).to(second)
    t_step = (t_model_step + t_tool + t_wait + t_runtime).to(second)
    t_task = (h_turns * t_step).to(second)
    t_model_total = (h_turns * t_model_step).to(second)
    f_model = (t_model_total / t_task).magnitude
    f_non_model = 1.0 - f_model
    s_trajectory_model = 1.0 / (f_non_model + (f_model / s_model_speedup))
    s_trajectory_max = 1.0 / f_non_model
    t_step_new = (t_model_step + t_tool_optimized + t_wait + t_runtime).to(second)
    t_task_new = (h_turns * t_step_new).to(second)
    s_trajectory_tool = (t_task / t_task_new).magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_model_step.magnitude - 5.09) < 0.01, "t_model_step")
    check(abs(t_step.magnitude - 22.99) < 0.01, "t_step")
    check(abs(t_task.magnitude - 183.92) < 0.05, "t_task")
    check(abs(t_model_total.magnitude - 40.72) < 0.05, "t_model_total")
    check(abs(f_model - 0.2214) < 0.001, "f_model")
    check(abs(f_non_model - 0.7786) < 0.001, "f_non_model")
    check(abs(s_trajectory_model - 1.188) < 0.005, "s_trajectory_model")
    check(abs(s_trajectory_max - 1.284) < 0.005, "s_trajectory_max")
    check(abs(t_step_new.magnitude - 10.99) < 0.01, "t_step_new")
    check(abs(t_task_new.magnitude - 87.92) < 0.05, "t_task_new")
    check(abs(s_trajectory_tool - 2.092) < 0.005, "s_trajectory_tool")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    h_turns_str = fmt_int(h_turns)
    s_prefill_tokens_str = fmt_int(s_prefill_tokens)
    t_prefill_ms_str = fmt_time(t_prefill, unit=millisecond, precision=0)
    t_decode_ms_str = fmt_time(t_decode, unit=millisecond, precision=0)
    t_tool_s_str = fmt_time(t_tool, unit=second, precision=1)
    t_runtime_s_str = fmt_time(t_runtime, unit=second, precision=1)
    t_wait_s_str = fmt_time(t_wait, unit=second, precision=1)
    s_model_speedup_mult_str = fmt_multiple(s_model_speedup, precision=1)
    t_tool_optimized_s_str = fmt_time(t_tool_optimized, unit=second, precision=1)
    f_non_model_pct_str = fmt_percent(f_non_model, precision=2, style="prose")
    s_trajectory_max_mult_str = fmt_multiple(s_trajectory_max, precision=3)
    s_trajectory_tool_mult_str = fmt_multiple(s_trajectory_tool, precision=3)
```

::: {#nbk-amdahl-accounting .callout-notebook title="Trajectory wall-clock and Amdahl latency accounting"}
**Problem:**
An autonomous coding agent executes a bug-repair task over a trajectory of $H =$ `{python} AmdahlAccounting.h_turns_str` turns. The system runs an open-weights 70B parameter dense model served across an 8-GPU node (NVIDIA H100 SXM5, aggregate memory bandwidth $26.8\text{ TB/s}$, FP8 Tensor Cores).

Profile telemetry collects the following per-step averages:

- Context prefill averages `{python} AmdahlAccounting.s_prefill_tokens_str` tokens ($T_{\text{prefill}} =$ `{python} AmdahlAccounting.t_prefill_ms_str`).
- Decode generation averages 450 tokens at $11\text{ ms/token}$ ($T_{\text{decode}} =$ `{python} AmdahlAccounting.t_decode_ms_str`).
- The local software environment compiles an altered C++ module and executes targeted regression tests, averaging $T_{\text{tool}} =$ `{python} AmdahlAccounting.t_tool_s_str` per step.
- Host runtime overhead (container snapshotting, diff extraction, sandbox verification) averages $T_{\text{runtime}} =$ `{python} AmdahlAccounting.t_runtime_s_str` per step.
- External wait latency averages $T_{\text{wait}} =$ `{python} AmdahlAccounting.t_wait_s_str` per step.
1. Compute the total task wall-clock duration $T_{\text{task}}$ and the model fraction $f_{\text{model}}$.
2. An infrastructure team proposes replacing the GPU cluster with next-generation accelerators that speed up model inference by `{python} AmdahlAccounting.s_model_speedup_mult_str` ($S_{\text{model}} = 3.5$). Compute the resulting trajectory speedup $S_{\text{trajectory}}$.
3. Alternatively, a systems team proposes caching compilation artifacts (`ccache`) and running an incremental test-runner, reducing average tool execution time from `{python} AmdahlAccounting.t_tool_s_str` to `{python} AmdahlAccounting.t_tool_optimized_s_str` ($S_{\text{tool}} = 3.86$). Compute the trajectory speedup achieved by this software engineering optimization.
...
Across the full $H =$ `{python} AmdahlAccounting.h_turns_str` turn trajectory:
...
Non-model operations account for $1 - f_{\text{model}} =$ `{python} AmdahlAccounting.f_non_model_pct_str` of total execution time.
...
Even an infinitely fast model achieves `{python} AmdahlAccounting.s_trajectory_max_mult_str` maximum speedup.

**Step 3: Toolchain and Systems Optimization**
Reducing tool execution time to `{python} AmdahlAccounting.t_tool_optimized_s_str` leaves model, wait, and runtime latencies unchanged:
...
Optimizing the host toolchain delivers a `{python} AmdahlAccounting.s_trajectory_tool_mult_str` speedup ($109\%$ throughput improvement), outperforming the GPU hardware upgrade by more than a factor of five while requiring zero additional accelerator capital expenditure.
:::
...
### Architectural decision matrix: Workflows versus model-directed loops {#sec-vol3-intro-workflow-vs-loop}
...
When the decision matrix dictates a model-directed loop, the system must translate the loose delegation into an explicit five-part contract, place it by its horizon, state, and authority exposure, and enforce invariant closure at every cycle of the trajectory.
...
**The Stochastic Computer is a software-level functional architecture in which learned computation, state, controlled interaction, and supervision form the live loop, while training and fleet operations improve and operate it across tasks.** Rather than casting the foundation model as an autonomous CPU,[^fn-stochastic-computer-karpathy] the architecture treats it as an unprivileged, non-deterministic inference engine that holds zero ambient authority. The model evaluates context and proposes candidate actions, but all state evolution, memory assembly, tool dispatch, and invariant verification are mediated by deterministic software layers executing on general-purpose host processors.

[^fn-stochastic-computer-karpathy]: **Andrej Karpathy (LLM OS Concept)**: Karpathy popularized the intuitive mapping of large language models to operating systems in late 2023, likening the neural network to a central processor, the context window to volatile RAM, and tools to peripheral devices. This book keeps the intuition only as a way to organize its parts, the Stochastic Computer, and argues each subsystem through its own mechanisms, such as memory bandwidth ceilings, paged KV caches, and transactional recovery. \index{Karpathy, Andrej!LLM OS}\index{Stochastic Computer!architectural lineage}

### The live execution loop: Four functional subsystems {#sec-vol3-intro-live-loop}
...
In the organizing names of @sec-vol3-intro-operational-incident, the foundation model engine is the machine's processor, the context memory hierarchy its memory, the execution sandbox its I/O boundary, and the agent runtime its operating system, while the policy compiler and the fleet complete it across tasks. The unified functional architecture of the Stochastic Computer is presented in @fig-vol3-agentic-architecture, bridging live task execution with across-task fleet infrastructure:

- *The Live Execution Loop (Core):* Couples the four live subsystems. Context memory stages working states across HBM and DRAM; the unprivileged Foundation Model Engine computes logits and emits candidate actions $a_{\text{prop}}$ under zero ambient authority; the Execution Sandbox isolates tool side effects inside containerized namespaces; and the Agent Runtime supervisor coordinates the entire datapath while enforcing mechanical invariant closure.
...
### Across-task fleet infrastructure {#sec-vol3-intro-across-task}
...
### The accelerator-host architectural boundary {#sec-vol3-intro-accelerator-host}
...
```{python}
#| label: lego-01-accelerator-host-boundary
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ LATENCY AND MEMORY ACROSS THE ACCELERATOR-HOST BOUNDARY (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-accelerator-host-boundary
# │ Goal: Profile memory footprint and execution phase latencies across the accelerator-host boundary.
# │ Show: 34,816-token session pins 10.62 GiB (13.2% of 80 GB GPU); decode is 97.6% of step duration.
# │ How: Contrast HBM KV cache reservation against host mediation and sandbox execution latencies.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.core.units import *
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_time, fmt_memory_capacity

class AcceleratorHostBoundary:
    """Latency and memory distribution across the accelerator-host boundary."""
    ...
    t_context = 34_816
    m_token = 320 * KiB
    t_decode = 1920.0 * millisecond
    t_tool = 45.0 * millisecond
    t_runtime = 1.4 * millisecond
    t_bus = 0.01 * millisecond

    # ┌── 2. EXECUTE (Compute) ─────────────────────────────────────────────
    m_kv_gb = 11.14 * GB
    m_kv_gib = 10.62 * GiB
    pct_pinned = 0.132
    t_total_turn = t_decode + t_tool + t_runtime + t_bus
    pct_decode = (t_decode / t_total_turn).magnitude
    pct_tool = (t_tool / t_total_turn).magnitude
    pct_runtime = (t_runtime / t_total_turn).magnitude
    t_mediation_total = (t_tool + t_runtime).to(millisecond)
    pct_mediation = (t_mediation_total / t_total_turn).magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(t_total_turn.magnitude - 1966.41) < 0.5, "t_total_turn")
    check(abs(pct_decode - 0.976) < 0.005, "pct_decode")
    check(abs(pct_tool - 0.023) < 0.005, "pct_tool")
    check(abs(t_mediation_total.magnitude - 46.4) < 0.1, "t_mediation_total")
    check(abs(pct_mediation - 0.024) < 0.005, "pct_mediation")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dram_capacity_gb_str = fmt_memory_capacity(dram_capacity, unit=GB, precision=0)
    gpu_hbm_gb_str = fmt_memory_capacity(gpu_hbm, unit=GB, precision=0)
    t_context_str = fmt_int(t_context)
    pct_pinned_pct_str = fmt_percent(pct_pinned, precision=1, style="prose")
    t_mediation_total_ms_str = fmt_time(t_mediation_total, unit=millisecond, precision=1)
    pct_mediation_pct_str = fmt_percent(pct_mediation, precision=1, style="prose")
```

::: {#nbk-accelerator-host-boundary .callout-notebook title="Latency and memory across the accelerator-host boundary"}
Consider an enterprise coding agent deployed on a host server equipped with host CPUs, `{python} AcceleratorHostBoundary.dram_capacity_gb_str` of DRAM, and an `{python} AcceleratorHostBoundary.gpu_hbm_gb_str` HBM3 GPU serving a 70B parameter model with Grouped-Query Attention ($L=80, H_{KV}=8, d_{\text{head}}=128$, 16-bit precision).

At step $t = 30$ of a trajectory, the agent has accumulated an active context length of `{python} AcceleratorHostBoundary.t_context_str` tokens (initial system instructions, previous tool invocations, and test traces). The model generates a 128-token tool command, which the host supervisor validates, writes to a Write-Ahead Log (WAL), and executes inside an isolated container test suite.
...
This single session pins over `{python} AcceleratorHostBoundary.pct_pinned_pct_str` of the GPU's entire `{python} AcceleratorHostBoundary.gpu_hbm_gb_str` device memory solely for historical activations, constraining concurrent serving capacity.
...
**Systems insight:** Interconnect bus transfer latency is negligible. Host runtime supervision and security verification (`{python} AcceleratorHostBoundary.t_mediation_total_ms_str`) add less than `{python} AcceleratorHostBoundary.pct_mediation_pct_str` overhead to the turn. The dominant physical bottleneck of the agentic execution loop is the computational and memory bandwidth cost of generating tokens over an expanding context working set.
:::
...
| **Part V: The Policy Compiler**                | Chapters 12–14       | Generic foundation models repeat execution errors, consume excessive tokens, and drift.      | Trajectory mining, Supervised Fine-Tuning (SFT) distillation, and verifiable RL.              |
| **Part VI: Distributed Fleets and Operations** | Chapters 15–17       | Complex tasks exceed single-agent token budgets and execution context bounds.                | Multi-agent DAG topologies, distributed OpenTelemetry tracing, and capacity economics.        |
| **Part VII: Synthesis**                        | Chapter 18           | Individual subsystem guarantees do not certify end-to-end operational correctness.           | Full-stack capstone implementation verified against sealed software benchmarks.               |
...
Establishes the foundational reference architecture, the fail-plausible fault model, the invariant closure principle with its closure evidence levels, the five-part task contract, and the H·S·A exposures that set how much closure a task needs.

**Part I: The Computational Engine (Chapters 02–03).**
Part I inspects the core inference engine of the system. @Sec-vol3-foundation-model formalizes the single model invocation under zero ambient authority, detailing the mechanics of Byte-Pair Encoding (BPE), the physical memory-bandwidth decode bottleneck, typed client-server status envelopes (`COMPLETED`, `TRUNCATED`, `REFUSED`, `TRANSPORT_FAILURE`), and GPU-level grammar-constrained logit masking. @Sec-vol3-test-time-compute expands the core across test-time compute scaling, analyzing multi-path deliberation, Best-of-$N$ sampling, tree search algorithms, step-level verification decoders, and test-time scaling dynamics.

**Part II: The Memory Hierarchy (Chapters 04–06).**
Part II constructs the multi-tier memory system required to support stateful execution. @Sec-vol3-context-engineering analyzes host-side logical working-set management: structuring prompt prefixes to maximize cache reuse, context budgeting, and lossy versus lossless observation compaction. @Sec-vol3-kv-cache moves to accelerator HBM, deriving the physical KV-cache footprint equation ($2LHd$), paged physical memory allocation, prefix-tree cache sharing, chunked prefill, and the economics of mitigating the Tool-Wait Memory Tax. @Sec-vol3-long-term-memory covers durable external storage, formalizing the architectural divide between authoritative source truth (filesystems, version control) and derivative search indexes (lexical BM25, dense vector embeddings, Abstract Syntax Tree property graphs).

**Part III: Execution Sandboxing (Chapters 07–08).**
Part III engineers the input/output subsystems that allow the agent to sense and modify its environment. @Sec-vol3-tool-calling establishes typed tool communication: the Model Context Protocol (MCP), structured JSON-RPC serialization, and observation sanitization using headless circular ring buffers that strip ANSI sequences and enforce output truncation. @Sec-vol3-sandboxes builds the Trusted Computing Base (TCB) and containment perimeter: configuring Linux container namespaces, cgroups, seccomp-bpf system call filters, microVM virtualization (Firecracker), copy-on-write workspace isolation, and default-deny egress firewalls to neutralize prompt injection and host escape.

**Part IV: Runtime Orchestration (Chapters 09–11).**
Part IV implements the supervisory runtime kernel that governs long-horizon execution. @Sec-vol3-agent-harness designs the central control plane: the Agent Control Block (ACB) state machine, multi-tenant scheduling queues, and cooperative preemption. @Sec-vol3-durable-execution establishes durability and crash consistency: implementing append-only Write-Ahead Logging (WAL) for trajectory events, intent/effect ledgers, checkpoint intervals, and deterministic replay debugging. @Sec-vol3-failure-recovery resolves distributed fault recovery across non-transactional effectors: constructing Saga orchestrators, executing backward compensating transactions ($C_i$), and establishing idempotent retry boundaries.

**Part V: Policy Optimization (Chapters 12–14).**
Part V closes the flywheel between runtime execution and offline model optimization. @Sec-vol3-trajectory-curation details trajectory telemetry mining: capturing production execution logs, filtering traces by deterministic verification success, and curating regression test fixtures. @Sec-vol3-fine-tuning examines Supervised Fine-Tuning (SFT) and knowledge distillation, compiling complex multi-turn prompt protocols into specialized, low-latency model weights. @Sec-vol3-rlvr explores Reinforcement Learning with Verifiable Rewards (RLVR), utilizing deterministic software environments (compilers, linters, sealed test suites) as non-gameable reward oracles to optimize exploration without reward hacking.

**Part VI: Distributed Systems (Chapters 15–17).**
Part VI scales agentic architecture to enterprise infrastructure. @Sec-vol3-multi-agent analyzes distributed multi-agent systems: asynchronous task dependency DAGs, shared git state synchronization, lock contention, and benchmarking multi-agent architectures against token-matched single-agent baselines. @Sec-vol3-evaluation establishes distributed observability: tracing multi-turn trajectories with OpenTelemetry, instrumenting SWE-bench evaluation harnesses, and categorizing failure taxonomies. @Sec-vol3-agent-economics covers systems economics and performance engineering: continuous batching schedulers, Tensor Parallelism ($TP \ge 2$) multi-GPU sharding, space-time memory occupancy ($O_{\text{mem}}$), and optimizing infrastructure for monetary cost per verified deliverable.
...
This misconception directly violates the End-to-End Argument of @saltzer1984endtoend. Low-level fluency or internal confidence cannot certify end-to-end task correctness. The model is an unprivileged predictor that holds zero ambient authority, possessing zero capability to audit its own proposals.

**Architectural Defense:** The host operating system must decouple candidate proposal generation from task completion certification. Operational completion requires verifiable evidence evaluated against an explicit five-part task contract ($\mathcal{C}$). The runtime must place all verification fixtures, regression test suites, and grading scripts inside read-only filesystem mounts inaccessible to the agent's write-capable tools (@sec-vol3-sandboxes). Task transitions to the `COMPLETED` state must depend strictly on deterministic software oracles: compiler exit codes, static analysis linters, diff analyzers, and sealed unit test suites executing inside hardened sandboxes (@sec-vol3-conclusion). A delegated task is complete if and only if independent, external software evidence confirms that all contract invariants are satisfied, completely bypassing the model's self-authored summaries.
:::
...
1. *Logit-Level Grammar Constraints:* Rather than repairing malformed syntax after emission, compile tool schemas into finite-state automata that mask invalid token logits to $-\infty$ during accelerator decode, mathematically preventing the generation of syntactically invalid tool invocations (@sec-vol3-foundation-model).
2. *Supervised Context Rollback:* When an execution error occurs, use the host Write-Ahead Log (@sec-vol3-durable-execution) to roll back the prompt context to the clean pre-invocation state, preventing toxic failure tokens from polluting the autoregressive attention history.
3. *Diagnostic Distillation:* Rather than injecting unbuffered, 500-line stack dumps, filter raw process output streams through circular ring buffers and diagnostic parsers (@sec-vol3-tool-calling), extracting concise, structured error codes (such as `ExitCode 1: UndefinedSymbol 'parse_timeout' at line 42`) for the subsequent invocation context.
4. *Search-Based Backtracking:* When repeated attempts fail within an execution branch, terminate the stalled path and backtrack to alternative exploration branches in the trajectory graph using structured search algorithms (@sec-vol3-test-time-compute, @sec-vol3-agent-harness).
:::
...
Dismantling these architectural fallacies grounds the engineering of agentic systems in the physical realities of the Stochastic Computer. The foundation model is neither an omniscient oracle capable of self-verification nor a conventional microprocessor equipped with internal hardware traps. It is an unprivileged, non-deterministic inference engine whose memory bandwidth boundaries, serialization costs, and output proposals must be strictly governed by the host runtime. With these foundational failure modes and systems boundaries clearly delineated, we turn to the authoritative synthesis of this chapter, distilling its governing principles before descending into the silicon mechanics of the foundation model engine in Part I.

## Summary {#sec-vol3-introduction-summary}

An accurate foundation model output fails to complete an operational task because generating candidate text alters zero host bits. A statistical neural network that holds zero ambient authority maps discrete token sequences to probability distributions over a fixed vocabulary, but operational agency requires mutating filesystems, spawning sandboxed subprocesses, managing network sockets, and surviving asynchronous execution faults. Left to operate open-loop, statistical policies fail plausibly: they emit syntactically pristine code that exits with code 0 while corrupting underlying semantic invariants. Autonomous agency is not an intrinsic attribute of neural weights; it is a property of the entire computing system. To advance from delegated intent to an accepted deliverable backed by empirical evidence, systems engineers must construct an accountable machine learning systems architecture that encapsulates an unprivileged stochastic engine within a deterministic runtime harness.
...
* **Architectural mapping assigns operational responsibilities.** Operational reliability emerges from strict modularity and separation of concerns across four decoupled subsystems: an unprivileged foundation model engine that proposes actions under zero ambient authority, a context memory hierarchy separating logical context from physical activations and persistent stores, an execution sandbox with capability-attenuated tools, and a supervisory agent runtime.
* **H·S·A exposure sets the closure a task needs.** Horizon ($H$), carried state ($S$), and authority over the world ($A$) are the three exposures a single model call lacks, and a task's position on them sets how much closure the runtime must supply, mechanical bounds below the model and a closure evidence level above it. Short, stateless tasks with narrow read access need little machinery, whereas long horizons, durable shared state, and irreversible authority demand isolated sandboxes, write-ahead logging, compensating sagas, and sealed evidence.
