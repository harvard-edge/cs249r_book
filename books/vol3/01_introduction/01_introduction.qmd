...
To cure this single-silo blindness, we anchor the architecture of agentic systems in the **Empirical Triangulation Method** shown in the margin, balancing three mutually reinforcing vertices: peer-reviewed academic foundations, physical hardware execution limits, and deployed production reality.

::: {.column-margin}
![](images/svg/empirical_triangulation_triangle.svg){width="100%" fig-alt="Diagram of a triangle representing the Empirical Triangulation Method, connecting Academic Foundations at the top apex, Systems Physics at the bottom left, and Production Reality at the bottom right, converging on Grounded Agentic MLSys Engineering at the center."}

*The Empirical Triangulation Method: Grounding agentic ML systems at the intersection of academic foundations, systems physics, and production reality.*
:::
...
By organizing context across these three tiers, the agentic runtime decouples working memory from physical accelerator bounds: idle KV pages swap out to host memory during tool-wait cycles, and long-horizon knowledge resides in durable secondary storage until referenced. Because transferring a 20 GB KV cache over PCIe Gen5 x16 ($\sim 64\text{ GB/s}$ nominal unidirectional bandwidth) requires approximately $300\text{--}500\text{ ms}$, hierarchical swapping is economically profitable for external tool operations exceeding $1\text{--}2\text{ seconds}$ (such as test suites, container builds, and remote network APIs), while sub-second tool operations remain buffered in accelerator HBM to avoid bus thrashing.
...
To keep this systems refactoring grounded in physical reality, three fundamental engineering distinctions must be established:
