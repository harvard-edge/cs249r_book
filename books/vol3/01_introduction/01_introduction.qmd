...
1. **The Single-Node Tensor Era:** In the early deep learning era, systems engineering was defined by the physics of the individual chip. The primary challenge was the memory wall: bridging the gap between arithmetic throughput and memory bandwidth. Engineers optimized general matrix multiply (GEMM) routines, fused activation layers, and balanced execution profiles against the Roofline model under the Data-Algorithm-Machine ($D \cdot A \cdot M$) framework.
2. **The Distributed Cluster and Serving Era:** As models outgrew single accelerators, the engineering crux shifted to cluster-scale orchestration and high-throughput inference serving. Systems architects devised tensor, pipeline, and data-parallel partitioning strategies, coordinating tens of thousands of GPUs over low-latency fabrics under the Compute-Communication-Coordination ($C^3$) taxonomy, and engineered continuous batching and paged memory systems to serve high-concurrency stateless requests.
3. **The Stateful Trajectory Era:** This book confronts the consequences of removing the passive serving constraint: *giving models agency*. Moving from passive statistical inference to stateful, multi-turn trajectories forces computer systems to confront the **Agency Dilemma**...
...
To appreciate the severity of this cubic tax, consider an agent executing a 50-turn trajectory ($K = 50$) with an average increment of $\Delta = 1{,}000$ tokens per turn. At turn 50 alone, the engine prefills $50{,}000$ tokens. But across all 50 turns, the cumulative prefill volume reaches:
$$\sum_{k=1}^{50} (1{,}000 \cdot k) = 1{,}000 \times \frac{50 \times 51}{2} = 1{,}275{,}000\text{ tokens evaluated}$$
and the cumulative attention compute scales as $\frac{1}{3} (50)^3 (1{,}000)^2 \approx 4.17 \times 10^{10}$ token-pair operations—over $40{,}000$ times the attention work performed in the initial turn! Without prefix caching, the agent repeatedly re-evaluates its own static history, burning massive compute budgets on redundant attention operations.
...
To provide an enduring mental model for systems engineering, this book organizes the components of autonomous runtimes around the core abstractions of computer architecture (@fig-vol3-intro-von-neumann-architecture).
...
The structural mapping decomposes the autonomous runtime into five classical computing subsystems, detailed below and synthesized in @tbl-von-neumann-agent-mapping:
...
2. **The Three-Tier Context Memory Hierarchy:** In classical microprocessors, memory is organized into an L1/L2/L3 cache hierarchy to balance high speed against physical capacity limits. Autonomous agents require an identical multi-tier hierarchy to manage state across long trajectories without exhausting scarce GPU memory:
   ...
   - *State Checkpointing and Write-Ahead Logging:* Trajectory state transitions are persisted via serialized Agent Control Blocks (ACBs) and event-sourced logs, providing crash consistency, deterministic replay, and compensating Saga transactions for non-transactional rollbacks (@sec-vol3-checkpointing).
3. **The Actuation and System Call Bus (I/O & Peripherals):** The boundary between internal cognitive deliberation and external physical environments:
   ...
   - **Peripheral Bus:** Mediated by the Model Context Protocol (MCP), establishing a standardized, typed JSON-RPC transport across decoupled client-host-server architectures (@sec-vol3-actuation).
   - **Interrupt Controller:** Trajectory-level signal handling (`SIGINT`, `SIGPAUSE`, `SIGBUDGET`) that enables asynchronous event demultiplexing and cryptographic approval escrow for Human-in-the-Loop (HITL) checkpoints (@sec-vol3-interrupts).
   - **Hardware Protection Rings:** Sandboxed virtualization mechanisms (microVMs, WebAssembly runtimes, Linux namespaces, and eBPF filters) that enforce least-privilege capability bounds and provide instant Copy-on-Write rollback (@sec-vol3-virtualization).
4. **The Policy Compiler (ISA & Compilation Pipeline):** The offline and online training pipelines that compile behavioral specifications, tool protocols, and multi-step reasoning capabilities into the continuous weight matrices of the neural policy:
   ...
   - Reinforcement Learning with Verifiable Rewards (RLVR) optimizes reasoning search paths and branch exploration via PPO and GRPO (@sec-vol3-rlvr).
5. **The Distributed Fleet (Cluster Interconnect & Distributed OS):** When tasks exceed the context window or cognitive capacity of a single agent, the runtime coordinates fleets of specialized agents across distributed topologies (hierarchical, debate, and swarm). The runtime monitors execution through OpenTelemetry distributed tracing and governs aggregate expenditure through fleet-wide tokenomics and capacity planning (@sec-vol3-multiagent, @sec-vol3-observability, @sec-vol3-tokenomics).
...
| **Instruction Decoder**               | Grammar-Constrained Decoding Mask  | Logit bias & Context-Free Grammar (CFG)        | Nanosecond masking before token sampling                                 | Chapter 02       |
...
| **L2 Cache (Prefix Cache)**           | Paged Radix Prefix Cache           | Paged HBM & Host DRAM over PCIe/CXL            | $5\text{--}20\text{ ms}$ paging; zero-FLOP prefix reuse                  | Chapters 05, 08  |
...
| **Write-Ahead Log (WAL)**             | Event-Sourced Trajectory Log       | Agent Control Block (ACB) & Snapshots          | Append-only durable ledger for replay & rollback                         | Chapter 07       |
| **System Call Interface (`syscall`)** | External Tool Invocations          | Sandbox CLI commands, HTTP APIs, DB queries    | $100\text{ ms}\text{--}60\text{ s}$ latency; mutates external state      | Chapter 09       |
| **Peripheral Bus (PCIe / USB)**       | Model Context Protocol (MCP)       | Typed JSON-RPC client-host-server transport    | Decoupled discovery, schemas, and capabilities                           | Chapter 09       |
...
| **Hardware Protection Rings**         | Virtualized Sandboxes              | MicroVMs (Firecracker), Wasm, eBPF             | Hermetic isolation, Copy-on-Write, network filters                       | Chapter 11       |
...
| **Distributed Cluster / OS**          | Multi-Agent Swarms & Fleet Routing | Actor message buses, OpenTelemetry, Tokenomics | Coordinates specialized agents under budget caps                         | Chapters 15–17   |
