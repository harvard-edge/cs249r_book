...
class MinimalAgentRuntime:
  ...
  def __init__(self, policy_fn, tool_dispatcher, verifier_fn):
    self.policy = policy_fn
    self.tools = tool_dispatcher
    self.verifier = verifier_fn

  def step(self, acb: AgentControlBlock) -> AgentControlBlock:
    # Watchdog budget enforcement
    if acb.turn >= acb.max_turns or acb.tokens_consumed >= acb.token_budget:
      acb.is_terminal = True
      acb.status = "BUDGET_EXHAUSTED"
      return acb

    # Phase 1 & 2: Context Assembly & Neural Dispatch
    prompt = f"Task: {acb.objective}\nHistory: {acb.history}"
    output, tokens = self.policy(prompt)
    acb.tokens_consumed += tokens

    # Phase 3: Action Classification (Compute vs. I/O Boundary)
    is_tool, payload = self._classify_action(output)
    if not is_tool:
      # Reversible internal deliberation: mutate working context only
      acb.history.append({"role": "thought", "content": payload})
    else:
      # Irreversible external actuation: execute in sandbox with verification
      obs = self.tools(payload["name"], payload["arguments"])
      if not self.verifier(obs):
        obs["verification_status"] = "INVARIANT_VIOLATION_FLAGGED"
      acb.history.append(
          {"role": "action", "tool": payload["name"], "obs": obs}
      )
...
This minimal harness represents the bare minimum kernel of autonomous agency. As we progress through this book, we will replace each simplistic component with production systems architecture: replacing prompt string concatenation with paged virtual memory (@sec-vol3-virtual-memory), unconstrained token generation with grammar-constrained decoding (@sec-vol3-processor), naive shell execution with microVM sandboxes (@sec-vol3-virtualization), and synchronous waiting with asynchronous trajectory schedulers (@sec-vol3-scheduling).
...
<!-- ======================================================================= -->
## The Agentic Systems Paradigm Shift {#sec-vol3-intro-the-tripartite-systems-comparison}
