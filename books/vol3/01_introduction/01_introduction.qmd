...
The architectural contrast between these two phases is governed by the **Roofline model**, which relates attainable computational performance to operational intensity (arithmetic operations executed per byte of memory transferred from off-chip storage):

1. **Context Prefill Phase (Compute-Bound):** When an agent initiates a turn, it submits a prompt consisting of system instructions, trajectory history, and new tool observations. The accelerator processes all prompt tokens concurrently. The dominant operations are dense General Matrix-Matrix Multiplications (GEMM). Because model weights are reused across hundreds or thousands of prompt tokens in parallel, operational intensity is high. The workload operates near the flat ceiling of the Roofline model: execution speed is bounded by peak accelerator Tensor Core arithmetic throughput.
2. **Autoregressive Decode Phase (Memory-Bandwidth Bound):** Once prefill completes, the model generates output action tokens sequentially, one token at a time. To emit each new token, the accelerator must stream its entire multi-gigabyte weight tensor from off-chip High-Bandwidth Memory (HBM) into on-chip registers to perform a Matrix-Vector Multiplication (GEMV). Because weights are read once to compute a single token update, operational intensity collapses to approximately $1\text{ FLOP/Byte}$. At this low intensity, execution speed is strictly throttled by HBM memory bandwidth. Over 99 percent of the accelerator's matrix compute units sit idle, stalled waiting for weight bytes to stream across the memory bus.

In an agentic workflow, execution oscillates continuously between these two regimes: brief compute-bound prefill spikes when ingesting observations, followed by memory-bandwidth-bound decode stalls while generating action plans.

### The key-value cache capacity wall

To prevent the autoregressive decode phase from recomputing self-attention over the entire historical sequence for every newly emitted token, inference engines store intermediate Key and Value activation vectors in accelerator memory. This memory structure is the **Key-Value (KV) cache**.

While the KV cache eliminates redundant attention recomputation, it introduces a severe physical memory footprint. Key-Value activations must be retained across all layers and attention heads for every token in the active trajectory. Consequently, the KV cache footprint grows linearly with sequence length:

- For a modern 70-billion-parameter model, retaining the KV cache across an extended agent horizon of $32\text{k}$ to $128\text{k}$ tokens requires between $10\text{ GB}$ and $40+\text{ GB}$ of high-bandwidth memory for a single trajectory.
- Once static model weights are loaded onto an accelerator (consuming tens of gigabytes even in quantized 8-bit precision), only a fraction of HBM remains available for dynamic activations and KV states.

This footprint creates the **KV cache capacity wall**. In production deployments, accelerator memory is rapidly saturated not by static weights, but by the accumulating KV caches of active agent sessions. A cluster of high-end accelerators can host only a handful of concurrent long-horizon trajectories before exhausting HBM, capping serving concurrency and forcing runtimes into aggressive memory throttling.

### The context accumulation wall

The linear growth of the KV cache is further compounded by the accumulation of context across multi-turn trajectories.
...
Overcoming the Tool-Wait Memory Tax requires decoupling compute allocation from the trajectory lifecycle:

- **Radix Prefix Caching:** Indexing shared prompt prefixes and static tool definitions in global radix trees, eliminating redundant prefill compute when trajectories resume (@sec-vol3-virtual-memory).
- **Hierarchical Memory Swapping:** Treating GPU HBM as an ephemeral working cache, asynchronously paging idle KV blocks across high-speed interconnects (PCIe Gen5 or CXL) into host DRAM or NVMe storage during blocking tool calls, and restoring them before the next token decode begins (@sec-vol3-scheduling).

### The mandate for systems co-design

The physical walls of agency—compounding reliability decay ($p^N$), Roofline decode bottlenecks, KV cache memory saturation, and the tool-wait memory tax—demonstrate that autonomous AI is fundamentally a systems problem. Algorithmic advances in model pretraining cannot overcome the exponential decay of sequential probability or the physical bandwidth limits of accelerator memory buses.
...
- [ ] **Compounding failure dynamics**: explain why a model with 95 percent single-step accuracy collapses to under 10 percent end-to-end success across a 50-step trajectory, and how closed-loop verification oracles reset error accumulation.
- [ ] **Roofline divergence**: contrast context prefill and autoregressive decode under the Roofline model. Why does single-token decode leave over 99 percent of accelerator matrix compute units stalled?
- [ ] **KV cache capacity limits**: explain why Key-Value caching is necessary during autoregressive decoding, and how context expansion across long trajectories caps serving concurrency in GPU HBM.
- [ ] **Tool-Wait memory tax**: describe how the latency mismatch between millisecond token generation and multi-second tool execution leads to resource stranding, and explain the role of hierarchical memory swapping in mitigating this tax.
