...
- Analyze the physical and mathematical limits of agency, including the Compounding Reliability Wall ($P_{\text{success}} \le p^N$), the Context Accumulation Wall ($\mathcal{O}(T^2)$), and the Tool-Wait memory tax.
- Navigate the Six-Part Architecture of the Stochastic Computer: understand how the runtime layers—stochastic processor, context memory and storage, tool actuation and I/O peripherals, the agent operating system, the policy compiler, and the distributed fleet—map to enduring computer systems abstractions.

:::

<!-- ======================================================================= -->
<!-- SECTION 1: THE AGENTIC SYSTEMS MOMENT                                   -->
<!-- ======================================================================= -->
...
1. **Non-Deterministic Control Flow:** The control flow of an agent is not compiled into deterministic branches or state machines; it is generated token-by-token by a stochastic neural network.[^fn-vol3-intro-sampling] Two identical system states can yield divergent execution trajectories.
...
[^fn-vol3-intro-sampling]: **GPU floating-point non-determinism**\index{Floating-Point!non-associativity}\index{Non-determinism!GPU inference}: Even when sampling with temperature $T = 0$ (greedy decoding), foundation model inference across parallel GPUs is not bitwise deterministic. Floating-point addition is non-associative: $(a + b) + c \ne a + (b + c)$. In multi-threaded CUDA or Triton reduction kernels across thousands of streaming multiprocessors, the summation order of attention weights varies with dynamic thread scheduling, producing minor numerical variations that can alter top-1 token selection at tie-break boundaries.

<!-- ======================================================================= -->
<!-- SECTION 2: THE EVOLUTION OF MACHINE LEARNING SYSTEMS                    -->
<!-- ======================================================================= -->
## The Evolution of Machine Learning Systems: The Road to Agency {#sec-vol3-intro-evolution-of-ml-systems}

When Maurice Wilkes and his colleagues designed the Electronic Delay Storage Automatic Calculator (EDSAC) in 1949,[^fn-vol3-intro-edsac] they established a mental model of computing that endured for over half a century: a program is a sequence of deterministic instructions loaded into memory, executed sequentially by a central processor, and halted when an answer is produced. Even as computers transitioned from vacuum tubes to silicon wafers and from isolated mainframes to planetary-scale clouds, the fundamental unit of execution remained brief, isolated, and deterministic.
...
- **Memory residency:** Operating systems and GPU runtimes expect to allocate memory for milliseconds, reclaim it, and serve the next request. An agent trajectory requires Key-Value (KV) cache memory[^fn-vol3-intro-kvcache] to persist across dozens of asynchronous tool calls, stranding expensive GPU high-bandwidth memory (HBM) during external network or disk latency.
...
[^fn-vol3-intro-edsac]: **EDSAC and the subroutine**\index{EDSAC!stored-program computing}: Designed by Maurice Wilkes and colleagues at Cambridge University in 1949, the Electronic Delay Storage Automatic Calculator introduced the first practical implementation of stored-program computing and reusable subroutine libraries, establishing the enduring computing contract that instructions advance deterministically through memory.

[^fn-vol3-intro-kvcache]: **Key-Value (KV) cache**\index{Key-Value Cache!definition}\index{KV Cache!memory footprint}: Autoregressive transformer inference generates output tokens sequentially. To avoid recomputing Key and Value projection matrices for historical tokens on every forward pass ($\mathcal{O}(T^2)$ FLOP overhead), the serving engine stores past Key and Value activation tensors directly in accelerator memory. For a model with $L$ layers, $n_{\text{heads}}$ attention heads, head dimension $d_{\text{head}}$, and sequence length $T$, the KV cache footprint scales as $2 \times L \times n_{\text{heads}} \times d_{\text{head}} \times T \times \text{sizeof(precision)}$, consuming tens of gigabytes per active trajectory.
...
Detecting and mitigating fail-plausible defects requires replacing passive error-code monitors with active, independent **semantic verification oracles**. These oracles execute external property tests, verify post-condition state transitions, and audit environment invariants before an action's side effects are committed to persistent storage. In hardened runtimes, these verification oracles are physically isolated from agent modification—for example, by mounting test suites and evaluation benchmarks on read-only overlay filesystems within the execution sandbox, preventing an agent from modifying test assertions to force exit code 0.

### Where requests lose the trajectory

The root cause of these failures is that modern cloud infrastructure is built entirely around **request-scoped abstractions**. As illustrated in @fig-request-scope-mismatches, forcing a multi-hour autonomous workload into isolated request containers creates six fatal systems mismatches across cost, memory, authority, recovery, evidence, and physical placement.

::: {#fig-request-scope-mismatches fig-env="figure" fig-pos="htb" fig-cap="**Where Requests Lose the Trajectory: Six Scope Mismatches**: Breakdown of the six systems failure modes that emerge when stateful agentic workflows are forced into stateless, request-scoped infrastructure. Request systems isolate individual model invocations, dropping the causal, temporal, and budgetary continuity required to govern long-horizon trajectories. Each systems surface—from monotonic cost accounting to persistent memory hierarchies, capability attenuation, saga-style compensating recovery, causal evidence auditing, and state-aware placement—is semantically valid only when attached to the execution trajectory as a unified unit." fig-alt="Six-card architectural diagram detailing the scope mismatches of request infrastructure across Cost Accumulation, Memory Span, Persisting Authority, Semantic Recovery, Causal Evidence, and Physical Placement."}
![](images/svg/request_scope_mismatches.svg){width="100%"}
:::

Each mismatch underscores the central thesis of invariant closure: a managed unit is too small when the invariants the system must preserve can only be stated by reaching outside that unit. A stateless request container cannot enforce monotonic spending caps, cannot maintain warm KV cache residency across asynchronous tool waits, and cannot orchestrate backward compensating actions when an external API call mutates production state. To govern autonomous agents reliably, systems infrastructure must promote the entire trajectory into a first-class, system-visible unit of management.

This tripartite paradigm shift reveals that Software 3.0 cannot be operated as a casual script or an unconstrained API loop. Enforcing invariant closure and mitigating fail-plausible semantic corruption requires a dedicated systems runtime. To engineer infrastructure capable of governing this closed loop, we must establish a precise architectural definition of an agentic machine learning system and deconstruct the layers of the Agentic Stack.

[^fn-vol3-intro-karpathy]: **Software 2.0 to Software 3.0**\index{Software 2.0!Karpathy}\index{Software 3.0!agentic systems}: Andrej Karpathy coined the term Software 2.0 in 2017 to describe systems where procedural logic is replaced by continuous neural network weights discovered through mathematical optimization. Software 3.0 extends this paradigm to autonomous execution, where the neural model acts as a non-deterministic reasoning controller governing deterministic Software 1.0 tools and APIs.
...
::: {#dfn-agentic-ml-system .callout-definition title="Agentic machine learning systems"}

***Agentic machine learning systems***\index{Agentic machine learning systems!definition} are autonomous, stateful computing systems that employ a stochastic neural foundation model as their central decision-making policy $\pi_\theta$, embedded within a deterministic runtime harness that maintains persistent context memory, orchestrates external tool actuation, and enforces invariant verification oracles across multi-turn, goal-directed operational trajectories.

1. **Significance**: Agentic systems invert the classical request-response serving contract ($10^{-1}\text{ s}$) into a stateful, long-horizon control trajectory ($10^1\text{--}10^4\text{ s}$), elevating the unit of systems optimization from token-level micro-efficiency (FLOPs/token) to task-level macro-efficiency (Joules, dollars, and goodput per verified outcome).
2. **Distinction**: Unlike Software 1.0 (deterministic instruction control flow) and Software 2.0 (open-loop tensor inference with zero external side effects), an agentic system operates as a closed-loop control system where stochastic neural actions directly mutate external environments, creating endogenous feedback loops and fail-plausible semantic failure modes.
3. **Common pitfall**: Treating an autonomous agent as an unconstrained API `while` loop prompting a stateless foundation model. Without a deterministic runtime that manages paged context hierarchies, isolates tool side effects in virtualized sandboxes, and verifies environmental invariants below the model, naive loops succumb to the compounding reliability wall, context poisoning, and memory exhaustion.
...
To govern this closed loop, systems infrastructure must make the entire course of execution visible. @Fig-trajectory-systems-boundary gives this systems boundary a concrete form.

::: {#fig-trajectory-systems-boundary fig-env="figure" fig-pos="htb" fig-cap="**The Systems Boundary of an Agentic Trajectory**: An autonomous agent trajectory visualized as an unfolding execution path from delegated goal to verified outcome, bounded within an enforceable policy guardrail envelope. Intermediate operations—requests, neural model calls, context assembly, tool invocations, human approvals, and environmental side effects—become discrete subevents along the trajectory. Underneath, the managed trajectory descriptor provides a system-visible handle binding identity, budgets, memory pointers, capability authority, saga recovery logs, causal traces, and physical placement." fig-alt="Systems diagram showing a delegated goal on the left connecting via a curving trajectory path through a bounded guardrail corridor to a verified outcome on the right, with subevents for request, model call, context, tool call, approval, and effect, bound to a managed trajectory descriptor at the bottom."}
![](images/svg/trajectory_systems_boundary.svg){width="100%"}
:::

Rather than managing disconnected API invocations, the infrastructure binds the execution to a **managed trajectory descriptor**. This descriptor travels alongside the trajectory, maintaining active state pointers, monotonic token and financial budgets, capability leases, and compensating write-ahead logs. The central systems question is how to coordinate and optimize this multi-turn trajectory while it is still unfolding in time.

### Grounding agency: The empirical triangulation method

Grounding agentic machine learning systems requires confronting a persistent obstacle: **single-silo architectural blindness**. When engineers attempt to design or reason about autonomous agents, they almost always approach the challenge through a single, incomplete technical discipline:

- **The Theoretical Trap (Theory without Physics):** Control theorists and reinforcement learning researchers model agency as Markov Decision Processes (MDPs) with elegant convergence proofs. Yet these abstractions assume infinite context, zero-cost state transitions, and free deliberation. On physical silicon, these designs collapse under the Context Accumulation Wall, saturate accelerator memory, and incur prohibitive monetary costs.
- **The Systems Silo (Physics without Semantics):** Hardware and infrastructure engineers focus on optimizing matrix multiplication kernels, FlashAttention, and raw token throughput ($\text{tokens/s}$). Yet maximizing token velocity in an agent that produces valid syntax while corrupting semantic objectives merely accelerates failure: the agent enters degenerative hallucination loops faster, burning Joules and dollars without making progress.
- **The Hacker's Heuristic (Software without Formal Bounds):** Application developers string together prompt templates, Python `while` loops, and REST APIs. These heuristic prototypes succeed on curated single-turn demos, but shatter in production when APIs time out, tools hang, or errors compound exponentially across long horizons.

To cure this single-silo blindness, we anchor the architecture of agentic systems in the **Empirical Triangulation Method** (@fig-empirical-triangulation), balancing three mutually reinforcing vertices: peer-reviewed academic foundations, physical hardware execution limits, and deployed production reality.

::: {#fig-empirical-triangulation fig-env="figure" fig-pos="htb" fig-cap="**The Empirical Triangulation Method**: Grounding agentic ML systems at the intersection of academic foundations, systems physics, and production reality. Academic foundations provide formal bounds on trajectory convergence and verified state-action semantics; systems physics dictates what accelerator memory hierarchies and interconnects can execute in silicon; and production telemetry surfaces real-world failure distributions like fail-plausible semantic drift and tool timeouts." fig-alt="Diagram of a triangle representing the Empirical Triangulation Method, connecting Academic Foundations at the top apex, Systems Physics at the bottom left, and Production Reality at the bottom right, converging on Grounded Agentic MLSys Engineering at the center."}
![](images/svg/empirical_triangulation_triangle.svg){width="85%"}
:::
...
To engineer an agentic system that survives production reality, we decompose the runtime into the five foundational layers of the **Agentic Stack**, organized around the closed-loop execution lifecycle shown in @fig-closed-loop-architecture.
...
Each layer of this architecture fulfills a distinct systems responsibility across the perception-deliberation-actuation-verification loop:
...
4. **Tool Actuation Fabric:** Once an action proposal is decoded, the actuation fabric takes responsibility for safe execution. Rather than executing commands directly on the host operating system, this layer mediates interactions through standardized protocols, such as the Model Context Protocol (MCP),[^fn-vol3-intro-mcp] and dispatches tasks into isolated hardware-virtualized sandboxes, such as Firecracker microVMs or gVisor application kernels. It enforces least-privilege capability tokens, rate limits, and execution timeouts.
...
To govern this execution cycle reliably, an autonomous runtime maintains an **Agent Control Block (ACB)**—an in-memory state descriptor directly analogous to the `task_struct` (Process Control Block) in a classical operating system kernel.[^fn-vol3-intro-pcb] The ACB encapsulates the running trajectory's active identity: unique task identifiers, allocated working context buffers, cumulative token and financial expenditures, active tool capability leases, and verification milestones. At each turn, the runtime references the ACB to enforce step-count and financial watchdog budgets, dispatches neural token generation, discriminates reversible internal deliberation from state-mutating external actuation, and gates state transitions through verification oracles before updating the trajectory history.
...
In agentic systems, micro-efficiency remains necessary, but it is insufficient. The primary engineering objective shifts to **macro-efficiency**: the total resources consumed to achieve verified task resolution. Three metrics govern macro-efficiency in agentic machine learning systems:
...
where $R$ represents the allocated resource expenditure (such as GPU wall-clock execution time, compute FLOPs, or total financial cost) of each trajectory, $\mathcal{T}_{\text{success}}$ is the set of trajectories reaching verified goal completion, and $\mathcal{T}_{\text{all}}$ is the set of all attempted trajectories. In production runtimes with multi-stage workflows, this binary formulation can be generalized to credit verified sub-goal milestones recorded in the Agent Control Block, distinguishing partial verified progress from degenerate trajectories that fail at step 1.

Focusing exclusively on micro-efficiency can paradoxically degrade macro-efficiency. Consider a small, highly compressed 7-billion parameter model optimized for maximum token throughput. If that model exhibits an 80 percent per-step action success rate, on a 10-step programming task its probability of successful completion is $(0.80)^{10} \approx 10.7$ percent. The runtime will burn hundreds of thousands of tokens across failed retries, stranded sandboxes, and manual rollbacks, driving trajectory goodput toward single digits and resulting in abysmal macro-efficiency. Conversely, deploying a larger 70-billion parameter model, or pairing a moderately sized model with a compiler-based verification oracle that intercepts errors at step 1, can achieve a 90 percent end-to-end completion rate. Even though the larger model exhibits lower raw token throughput and higher per-token inference cost, it completes the task in a fraction of the total attempts, yielding vastly superior trajectory goodput, lower energy consumption, and lower total dollar cost per resolved task.

::: {#chk-vol3-01-macro-efficiency .callout-checkpoint title="Micro-efficiency vs. macro-efficiency"}

Before examining how agentic runtimes diverge from classical systems, verify your grasp of trajectory economics:

- [ ] **Compound Reliability**: can you explain how exponential decay ($p^K$) across sequential steps alters the economic viability of autonomous execution compared to single-turn inference?
- [ ] **Trajectory Goodput**: can you distinguish why a smaller, higher-throughput model can yield substantially lower trajectory goodput than a larger, slower model on multi-step tasks?
- [ ] **Failure Amplification**: can you describe how unintercepted early-step errors induce context poisoning and memory stranding across multi-turn trajectories?
...
[^fn-vol3-intro-mcp]: **Model Context Protocol (MCP)**\index{Model Context Protocol!standardization}: An open specification introduced by Anthropic in late 2024 to standardize communication between foundation models and external tools. Modeled on the Language Server Protocol (LSP), MCP defines a JSON-RPC 2.0 wire protocol that cleanly separates client hosts (agent runtimes) from server providers (databases, filesystems, developer tools), enabling dynamic capability negotiation and secure isolation.

[^fn-vol3-intro-pcb]: **Process Control Block (`task_struct`)**\index{Process Control Block!Linux kernel}\index{Agent Control Block!OS heritage}: In the Linux operating system kernel, the `task_struct` maintains complete state for a running process: scheduling priority, virtual memory descriptors (`mm_struct`), open file tables (`files_struct`), signal handlers, and security credentials. The Agent Control Block (ACB) mirrors this abstraction for autonomous agent runtimes, tracking context pointers, KV page tables, tool leases, and trajectory audit logs.
...
Confronted with this collapse, engineering teams frequently assume the defect lies in model capacity. Upgrading from a 70-billion parameter model to a 405-billion parameter frontier variant quadruples inference operational expenditure and accelerator memory footprint, yet the end-to-end completion rate improves only to 18 percent.

The failure stems not from inadequate pretraining scale, but from the reality that autonomous agency collides with immutable mathematical laws and physical hardware bottlenecks. Understanding these limitations requires a formal taxonomy to classify agent workloads and analyze the physical walls that constrain them.

### The H-S-A-C systems taxonomy

To reason quantitatively about autonomous workloads, we classify agentic systems across a four-dimensional coordinate space: Horizon ($H$), State complexity ($S$), Authority level ($A$), and Invariant Closure ($C$), illustrated in @fig-hsac-taxonomy. This **H-S-A-C taxonomy** serves as the golden thread running throughout this book, providing the standard evaluation coordinate system against which every runtime abstraction, memory hierarchy, and execution sandbox is measured.

The four axes define the operational envelope by contrasting workload demand with runtime control:

- **Horizon ($H$) [Temporal Demand]:** The sequential trajectory depth, measured by the number of closed-loop action-observation turns required to satisfy the objective. Horizons range from shallow interactive queries ($H \le 5$) to multi-step workflows ($H \approx 10\text{--}50$) and open-ended exploration ($H \ge 100\text{--}1{,}000+$).
- **State Complexity ($S$) [Spatial Demand]:** The spatial distribution and volatility of environment state tracked during execution, scaling from ephemeral prompt buffers ($S_0$) to local filesystems ($S_1$) and distributed cloud APIs ($S_2$).
- **Authority Level ($A$) [Effector Blast Radius]:** The scope of the agent's effector actions, scaling from read-only advisory suggestions ($A_0$) to ephemeral sandboxed execution ($A_1$), reversible compensating transactions ($A_2$), and irreversible physical or financial mutations ($A_3$).
- **Invariant Closure ($C$) [The Verification Anchor]:** The degree to which execution correctness can be deterministically verified without human intervention. Invariant Closure represents the fundamental control axis of agency, spanning two distinct operating regimes:
  - **Closed Invariants ($C_{\text{closed}}$):** The system possesses a deterministic, computable decision oracle ($P = 1.0$) capable of evaluating state transitions hermetically—such as compiler syntax verification, unit test suites, formal type checkers, database constraints, and cryptographic signatures. In closed-closure regimes, the runtime can immediately trap flawed actions, reject corrupted state transitions, and backtrack to known-good checkpoints.
  - **Open Invariants ($C_{\text{open}}$):** The task lacks a deterministic automated oracle; correctness is subjective, fuzzy, or non-computable—such as drafting persuasive prose, synthesizing legal briefs, customer negotiation, or exploratory data interpretation. In open-closure regimes, correctness can only be audited through heuristic scoring or human evaluation in the loop.

::: {#fig-hsac-taxonomy fig-env="figure" fig-pos="htb" fig-cap="**The H-S-A-C Systems Taxonomy**: A four-dimensional coordinate space classifying autonomous agent architectures across three operational demand axes—Horizon ($H$), State complexity ($S$), and Authority level ($A$)—governed by the fundamental control axis: Invariant Closure ($C$). While $H$, $S$, and $A$ dictate the operational stress and blast radius of the workload, Invariant Closure ($C$) determines whether the runtime possesses a deterministic verification oracle to reset the Compounding Reliability Wall ($C_{\\text{closed}}$, green) or must operate open-loop under fail-plausible risk ($C_{\\text{open}}$, amber)." fig-alt="Four-dimensional coordinate space diagram mapping autonomous workloads across Horizon (1 to 1000+ steps), State (in-context, external, distributed), Authority (read-only to irreversible production mutations), and Invariant Closure (closed deterministic oracles vs open subjective evaluation)."}
![](images/svg/hsac_taxonomy.svg){width="100%"}
:::

**Demystifying invariant closure: Why closure is the fourth dimension.**

The necessity of Invariant Closure as the fourth dimension arises from a fundamental systems asymmetry: the first three axes ($H$, $S$, and $A$) define **workload stress and operational risk**, whereas Closure ($C$) defines **runtime verifiability and feasibility**.

If we reason about agent workloads through $(H, S, A)$ alone, mathematics dictates that any substantive autonomous system must fail. As an agent moves outward along the Horizon axis ($H$), the Compounding Reliability Wall ($P_{\text{success}} \le p^N$) guarantees that even a model with 95 percent per-step accuracy collapses to a 7.7 percent success rate by step 50. If the system operates open-loop, errors inevitably compound into catastrophic failure.

How, then, do production autonomous systems ever succeed across extended horizons?

The answer lies entirely in **Invariant Closure ($C$)**. The term "closure" is borrowed from control theory and formal systems: a control loop achieves *closure* when its feedback signal is closed within the automated machinery itself, rather than requiring an external human operator to inspect and steer the system. When a workload possesses closed invariants ($C_{\text{closed}}$), the runtime can construct automated verification gates. Every action proposal is audited before committing state mutations. If a compiler rejects a generated function or a test suite fails, the runtime traps the defect, rolls back the filesystem snapshot, and prompts the agent to explore an alternative reasoning branch. **This automated verification mechanism physically produces the saw-tooth error resets illustrated in @fig-vol3-intro-compounding-reliability.** Invariant closure converts an unreliable $p = 0.95$ stochastic processor into a reliable, high-goodput autonomous system.

This systems reality resolves the **Autonomy Paradox**: why modern autonomous coding agents (such as SWE-bench agents) can successfully execute trajectories spanning 100 sequential turns ($H = 100$), whereas autonomous strategic planning or open-ended prose agents fail after five turns ($H = 5$). Software engineering is not inherently simpler than prose composition; rather, software engineering possesses near-total **Invariant Closure ($C_{\text{closed}}$)**—every step can be verified by a compiler, a linter, or a test suite. Conversely, open-ended writing and strategic planning operate in an **Open Invariant regime ($C_{\text{open}}$)**, where no automated oracle exists to detect hallucinations or subtle semantic drift. Without closure, compounding errors run unchecked, and the trajectory collapses under $p^N$. For systems architects, the operational takeaway is structural: building viable agents for open-invariant domains requires decomposing high-level tasks into bounded pipelines of closed-invariant sub-steps—enforcing structured schemas, intermediate format validators, and capability escrow gates around every stochastic generation stage. In document extraction or enterprise workflow automation, for example, this translates to wrapping open-ended generation inside strict Pydantic JSON schemas, deterministic regex validators, and database referential integrity checks before passing intermediate state downstream.

As workloads advance along the demand axes ($H$, $S$, and $A$) without the protective anchor of closed invariants ($C_{\text{closed}}$), naive agent designs encounter three unyielding physical barriers: the Compounding Reliability Wall, the Context Accumulation Wall, and the Tool-Wait Memory Tax.
...
To visualize the contrast between open-loop degradation and closed-loop architectural recovery, @fig-vol3-intro-compounding-reliability plots these survival curves alongside a verified execution trajectory. In this architecture, each saw-tooth reset in the green trajectory represents a deterministic verification checkpoint—such as a passing test suite, linter clean bill, or compiler validation—that commits verified progress. Importantly, a verification gate resets the error accumulation baseline only when coupled with state rollback and alternative branch search: when an intermediate verification check fails, the runtime rolls back to the prior checkpoint and resamples actions, converting a multiplicative failure cascade ($\prod p_i$) into a multi-stage renewal process with bounded sub-goal retries.

::: {#fig-vol3-intro-compounding-reliability fig-env="figure" fig-pos="htb" fig-cap="**The Compounding Reliability Wall in Open-Loop Trajectories**: Mathematical decay of sequential task completion probability $P_{\\text{success}} \\le p^N$ contrasted with closed-loop architectural recovery. In open-loop execution, even models with 99 percent single-step accuracy ($p = 0.99$, purple line) suffer severe reliability collapse by step 100 ($P_{\\text{success}} \\approx 36.6$ percent), while standard 95 percent accuracy ($p = 0.95$, blue line) collapses to 7.7 percent by step 50. Achieving sustainable long-horizon reliability requires closed-loop systems interventions (green dashed line), where each saw-tooth reset represents a deterministic verification checkpoint coupled with state rollback and alternative action search, committing certified progress and resetting the compounding error exponent back to zero." fig-alt="Plot of task success probability versus trajectory horizon length from 0 to 100 steps, showing exponential decay curves for per-step accuracy rates of 0.90, 0.95, and 0.99, contrasted against a saw-tooth closed-loop verification recovery curve."}
![](images/svg/compounding_reliability_wall.svg)
:::

The engineering implication of @tbl-compounding-reliability is stark. To achieve an acceptable 80 percent end-to-end success rate on a 50-step autonomous task ($P_{\text{success}} = 0.80$), the required per-step accuracy is $p = (0.80)^{1/50} \approx 0.99554$ (99.55 percent). For a 100-step task, the required per-step accuracy rises to $p = (0.80)^{1/100} \approx 0.99778$ (99.78 percent).
...
In an autonomous agent loop spanning dozens of sequential turns, each step appends tool invocation arguments, compiler outputs, environment diagnostics, and scratchpad reflections to the prompt. This monotonic accumulation imposes three distinct systems penalties:
...
By organizing context across these three tiers, the agentic runtime decouples working memory from physical accelerator bounds: idle KV pages swap out to host memory during tool-wait cycles, and long-horizon knowledge resides in durable secondary storage until referenced. Because transferring a 20 GB KV cache over PCIe Gen5 x16 ($\sim 64\text{ GB/s}$ nominal unidirectional bandwidth) requires approximately $300\text{--}400\text{ ms}$ each way (a $600\text{--}800\text{ ms}$ roundtrip for offloading and prefetching), hierarchical swapping is economically profitable for external tool operations exceeding $1\text{--}2\text{ seconds}$ (such as test suites, container builds, and remote network APIs). For fast sub-second tool operations, the runtime avoids bus thrashing by retaining active pages in accelerator HBM under a delayed-offload hysteresis timer.
...
Before exploring the architectural frontiers of autonomous delegation, verify your grasp of physical and mathematical runtime constraints:

- [ ] **Compound Reliability**: can you explain why a model with 95 percent single-step accuracy collapses across a 50-step trajectory, and how closed-loop verification oracles reset error accumulation?
- [ ] **Context Accumulation Penalties**: can you contrast the quadratic prefill computational tax ($\mathcal{O}(T^2)$) with attention dispersion as trajectory history expands?
- [ ] **KV Cache Capacity Limits**: can you distinguish why Key-Value cache accumulation across long trajectories caps serving concurrency in GPU HBM?
- [ ] **Tool-Wait Memory Stranding**: can you describe how the latency mismatch between millisecond token generation and multi-second tool execution leads to resource stranding, and how hierarchical memory swapping mitigates this tax?

:::

<!-- ======================================================================= -->
<!-- SECTION 6: WHERE IS AGENTIC AI GOING?                                  -->
<!-- ======================================================================= -->
...
The transition from assistive copilots to autonomous digital fleets and embodied machines highlights the necessity of a unified systems framework. To engineer software agents capable of long-horizon delegation without collapsing under compounding errors or bankrupted by token economics, we must systematically master every layer of the autonomous computing stack. To navigate this landscape, the curriculum of this textbook is organized around the timeless principles of computer architecture and operating systems.

<!-- ======================================================================= -->
<!-- SECTION 7: WHAT YOU WILL LEARN IN THIS BOOK                            -->
<!-- ======================================================================= -->
...
### Refactoring the systems stack

To bridge classical operating systems principles with autonomous machine learning, @fig-stack-comparison-traditional-agentic compares the traditional software infrastructure stack against the emerging agentic systems stack across all six computing layers.

::: {#fig-stack-comparison-traditional-agentic fig-env="figure" fig-pos="htb" fig-cap="**Traditional Systems Stack vs. Agentic Systems Stack**: Layer-by-layer correspondence between traditional computing infrastructure (left) and the agentic ML systems stack (right). Rather than replacing classical abstractions, agentic systems refactor them to support long-horizon stateful execution: user requests become delegated goals, stateless RPCs become multi-turn trajectories, process containers become agent runtimes, kernels evolve into managed trajectory control layers, ISAs and system call drivers expand into AI-native serving and tool protocols (such as MCP), and homogeneous server hardware gives way to heterogeneous accelerator substrates governed by physical cost models." fig-alt="Two-column architectural diagram comparing the traditional stack on the left with the agentic stack on the right across six layers connected by transition arrows: request to goal, request to trajectory, process to runtime state, kernel to trajectory control, syscalls to tools and context, and hardware counters to physical cost."}
![](images/svg/stack_comparison_traditional_agentic.svg){width="100%"}
:::

This structural refactoring reveals that agentic machine learning is not an isolated AI subfield; it is the next evolutionary layer of computer systems architecture. Every traditional systems layer—from processes to device drivers and physical hardware—gains leverage once it exposes the state needed to govern autonomous trajectories.

To keep this systems refactoring grounded in physical reality, four fundamental engineering distinctions must be established:
...
4. **The Architectural Tension: Von Neumann Core, Harvard Runtime Harness:** In classical architecture, a Von Neumann machine stores instructions and data in a unified address space, whereas a Harvard architecture physically separates instruction memory from data memory. At the neural model level, an LLM is a classic Von Neumann computer: instructions (system prompts) and untrusted data (tool outputs, web content) are serialized into the exact same token sequence ($h_t$). This unified representation is the root cause of prompt injection vulnerabilities ($W \oplus X$ violation). Because the model cannot physically isolate instructions from data in its weights, the **Agentic Stack runtime** must enforce a software **Harvard architecture harness** around the model: isolating data in strongly-typed schemas (via MCP) and executing untrusted code inside sandboxes with non-executable memory and read-only mounts.

### Mapping the agentic stack to computer architecture

Connecting the Agentic Stack to these classical computer systems abstractions provides the structural blueprint that organizes the rest of this book (@fig-vol3-intro-von-neumann-architecture). Rather than inventing ad-hoc categories, we recognize that the autonomous runtime is the **Stochastic Computer**: each major layer of the agentic stack maps directly to an established building block of computer architecture—from the stochastic execution core and multi-tier memory hierarchy to the system call boundary, policy compiler, and distributed fleet coordination plane.
...
3. **The Execution Runtime & Control Plane (Operating System Kernel):** In classical systems, the operating system kernel manages executing threads, schedules CPU time-slices, maintains process control blocks, persists checkpoint state, and handles asynchronous interrupt signals. The agent execution runtime performs the identical role for multi-turn trajectories:
   ...
   - **State Persistence and Rollback:** Trajectory state transitions are persisted via event-sourced write-ahead logs and Copy-on-Write snapshots, providing crash consistency, deterministic replay, and compensating Saga transactions[^fn-vol3-intro-saga] for non-transactional rollbacks (@sec-vol3-checkpointing).
...
Before exploring the stochastic processor, verify your mental model of the agent computing stack:

- [ ] **Instruction Cycle Mapping**: can you describe how the classical fetch-decode-execute cycle maps to an autoregressive model executing tool calls under grammar-constrained decoding?
- [ ] **Memory Hierarchy Latency**: can you contrast the capacity and latency boundaries between L1 active attention working memory (HBM) and L3 episodic storage (vector databases)?
- [ ] **Deterministic Sandboxing**: can you explain why the Invariant Closure Principle requires capability bounds to be enforced at the virtualization layer rather than inside the system prompt?

:::

### The six-part structural spine

This book guides the reader through the end-to-end design, implementation, and optimization of this autonomous architecture across six cohesive parts:

**Part I: The Stochastic Processor (Chapters 02–03)**
Part I focuses on the core execution engine: treating the autoregressive foundation model as a stochastic processor core (the neural ALU).
...
**Part II: Context Memory & Storage (Chapters 04–06)**
Part II confronts the physical memory wall and state persistence challenges governing long-horizon agent execution.
- In @sec-vol3-working-sets, we model the active context window as an L1 working set, adapt Peter Denning's working set model to transformer token contexts, examine attention dispersion and the lost-in-the-middle phenomenon, and implement AST-aware token eviction policies.
...
Part III examines the system call interfaces, peripheral buses, and hardware isolation required to govern external actuation.
...
Part IV builds the operating system runtime executive that coordinates stateful, long-running agent processes.
...
Part V explores how to train, adapt, and compile base foundation models into disciplined, tool-competent autonomous policies.
...
- In @sec-vol3-rlvr, we scale reasoning capabilities through Reinforcement Learning with Verifiable Rewards (RLVR). We formulate policy gradients via Group Relative Policy Optimization (GRPO), evaluate sparse terminal rewards against dense process rewards, and resolve entropy collapse over long action horizons.

**Part VI: The Distributed Fleet (Chapters 15–17)**
Part VI scales agentic infrastructure from single machines to distributed enterprise fleets.
...
**Capstone Synthesis: The Autonomous Frontier (@sec-vol3-conclusion)**
In @sec-vol3-conclusion, we synthesize the full-stack architecture, consolidate the timeless invariants that govern autonomous systems, map open research frontiers, and establish the bridge to physical AI and robotics. We trace how the trajectory abstractions developed across software environments extend to cyber-physical systems, where safety cases, real-time deadlines, and non-reversible environmental mutations define the frontier of autonomous intelligence.
...
[^fn-vol3-intro-saga]: **The Saga pattern**\index{Saga pattern!compensating transactions}: Formulated by Hector Garcia-Molina and Kenneth Salem in 1987 for distributed database transactions [@garciamolina1987sagas]. Rather than holding distributed two-phase commit (2PC) locks across extended durations, a Saga decomposes a distributed workflow into a sequence of local transactions, where every forward action $A_i$ is paired with an idempotent compensating action $C_i$ that semantically undoes side effects if a subsequent step fails.

[^fn-vol3-intro-wx]: **The $W \oplus X$ invariant**\index{W XOR X!hardware security}: "Write XOR Execute" (also known as Data Execution Prevention or DEP) is a fundamental operating system and CPU architecture security invariant that prevents any virtual memory page from being simultaneously writable and executable. In autoregressive foundation models, prompt instructions and untrusted data are serialized into a single contiguous token stream, inherently violating $W \oplus X$ and creating the architectural root cause of prompt injection attacks.

<!-- ======================================================================= -->
<!-- SECTION 8: FALLACIES AND PITFALLS                                      -->
<!-- ======================================================================= -->
...
<!-- ======================================================================= -->
<!-- SECTION 9: SUMMARY & CHAPTER CONNECTION                                -->
<!-- ======================================================================= -->
