...
![**Closed-Loop Execution Architecture**: The runtime mediation loop transforming an unprivileged model proposal into a verified environment mutation. The host runtime intercepts candidate action $a_t$, executes it within an isolated sandbox, captures structured observation $o_{t+1}$, and evaluates deterministic verification evidence $E$ before committing changes.](images/svg/closed_loop_architecture.svg){#fig-closed-loop-architecture width="95%"}
...
![**The Three Epochs of Machine Learning Execution**: Historical evolution of machine learning systems units from static, compile-time tensor computation graphs to decoupled token-serving loops, and finally to stateful, multi-turn closed-loop agentic trajectories.](images/svg/three_epochs_execution_units.svg){#fig-three-epochs width="95%"}
...
![**Evolution of Execution Units**: Temporal progression of computer system execution units across thirteen orders of magnitude, from early subroutines to modern autonomous trajectories.](images/svg/evolution_execution_units_v2.svg){#fig-evolution-execution-units width=\"95%\"}
...
![**Open-Loop Serving versus Closed-Loop Agent Supervision**: In traditional serving (Era 2), the human operator provides the external closed-loop runtime by manually testing code and pasting back compiler diagnostics. In autonomous agent architectures (Era 3), the host supervisor programmatically manages context, intercepts candidate proposals under zero ambient authority, and validates execution state within an isolated sandbox.](images/svg/open_vs_closed_loop_control.svg){#fig-open-vs-closed-loop-control width="95%"}
...
Software 3.0 synthesizes these two paradigms into stateful, interactive trajectories. In this operational model, the core model is a pretrained, stochastic neural policy (Software 2.0), but its outputs are not terminal answers delivered directly to a human user. Instead, the policy emits structured action proposals directed at deterministic effectors (Software 1.0): shell commands, database queries, web requests, abstract syntax tree refactoring scripts, and test runners. The execution environment consumes these action proposals, executes them under strict operating system sandboxing, and captures structured observations—process return codes, standard output streams, compiler error diagnostics, and filesystem diffs. These observations are marshaled into the evolving context history of the agent, informing the next stochastic inference step, as illustrated in @fig-software-3-runtime-architecture.

![**The Software 3.0 control loop**: The agent runtime supervisor mediates interactions between the stochastic neural policy and isolated deterministic execution tools.](images/svg/software-3-runtime-architecture.svg){#fig-software-3-runtime-architecture width=\"95%\"}
...
*High GPU kernel utilization masks catastrophic macro-inefficiency when 79% of cluster FLOPs are consumed by failing trajectory badput.*
:::
...
::: {#exmp-01-trajectory-goodput .callout-example title="Trajectory Goodput and Compute Waste"}
...
**Systems insight**: Micro-optimizing decode kernels to gain 10% higher tokens/second saves fractions of a second. In contrast, an early-stopping verification harness that trips failing loops at step 10 reclaims over 60% of wasted cluster energy and capital, directly driving macro-efficiency.
...
The execution of an agentic system proceeds through a continuous finite-state machine managed by the host runtime. Rather than allowing the neural model to run unbounded, the host advances the system through six tightly coordinated phases on every turn $t$, as formalized in @fig-execution-lifecycle and summarized in @tbl-trajectory-primitives.

![**The six-phase execution lifecycle finite-state machine**: The host runtime advances synchronously across continuation checks, model invocation, action authorization, runtime dispatch, evidence capture, and completion assessment.](images/svg/ch01_execution_lifecycle.svg){#fig-execution-lifecycle width=\"95%\"}
...
When systems engineers attempt to host autonomous agents within standard stateless request containers, the architecture breaks down. An agent is not a stateless function; it is an extended, stateful, closed-loop trajectory. Forcing a multi-turn trajectory into a stateless request paradigm exposes six fundamental *scope mismatches* that undermine system stability, cost boundaries, and execution correctness (@fig-request-scope-mismatches).

![**Structural Scope Mismatches**: The six structural scope mismatches between stateless microservice request containers and stateful agentic trajectories. Standard server runtimes discard state, bound execution times to milliseconds, and assume idempotent, low-cost retries, whereas agentic systems require stateful context preservation, long-lived non-ambient authority, non-idempotent compensating recovery, and persistent GPU/sandbox cache locality.](images/svg/request_scope_mismatches.svg){#fig-request-scope-mismatches width=\"95%\"}
...
![**Mechanical Invariant Closure Operator**: Candidate action proposals emitted by the unprivileged foundation model ($A=0$) are intercepted in memory escrow and arbitrated across syntactic, resource, and isolation gates before being dispatched to an isolated effector or trapped as an escrow fault.](images/svg/invariant_closure_operator.svg){#fig-vol3-invariant-closure width="95%"}
...
Every task specification contract operates within a concrete physical budget. When transitioning from single-turn model queries to multi-step agentic trajectories, systems engineers must evaluate the execution envelope across Four Engineering Dimensions: Duration, State, Permitted Authority, and Completion Evidence (@fig-four-engineering-dimensions).

![**The Four Engineering Dimensions of an Agentic Trajectory**: A systems accounting framework decomposing agent execution into wall-clock duration summation, state tier boundaries, permitted blast radius authority ($A=0$), and deterministic verification evidence.](images/svg/four_engineering_dimensions.svg){#fig-four-engineering-dimensions width="95%"}
...
A foundational question in modern systems design is determining when an autonomous agent is actually required. In the era of Software 3.0, there is an engineering temptation to deploy closed-loop, model-directed agents for every problem. This represents poor systems discipline. A model-directed agent introduces non-determinism, high token costs, and variable execution latency. If a problem can be solved by a deterministic Software 1.0 program or a structured Software 2.0 model pipeline arranged as a static Directed Acyclic Graph (DAG), building an autonomous agent is an architectural anti-pattern (@fig-workflow-vs-loop-decision-tree).

![**Architectural Selection Flowchart: Fixed Workflows versus Model-Directed Loops**: A disciplined systems routing hierarchy. Tasks with pre-computable DAG paths execute as deterministic Software 1.0 workflows at sub-second latency and zero token drift. Irreversible mutations route to human-gated escrow workflows under guarded execution. Open-ended tasks with reversible sandbox actions and non-deterministic dependency paths deploy autonomous model-directed loops.](images/svg/workflow_vs_loop_decision_tree.svg){#fig-workflow-vs-loop-decision-tree width="95%"}
...
::: {.callout-tip title="Rule of Thumb for Systems Architects"}
...
Every subsystem in the Stochastic Computer exists to resolve a concrete physical bottleneck or operational failure mode exposed by the subsystem directly beneath it. If a systems engineer attempts to assemble an agentic runtime without respecting this causal hierarchy, the system inevitably succumbs to context exhaustion, memory stranding, security escape, or unrecoverable state corruption. @tbl-causal-spine formalizes this architectural progression across the volume.
...
![**Curriculum Dependency Spine**: Curriculum progression and causal dependency spine of Agentic Machine Learning Systems across the machine learning systems series. Volume III treats the foundation model as an unprivileged, stochastic inference engine operating under zero ambient authority ($A=0$), organizing the runtime into an accountable computer that progresses causally from a single model invocation to distributed multi-agent fleet operations.](images/svg/mlsys_curriculum_arc.svg){#fig-subsystem-dependency-tree width=\"95%\"}
...
: **Recommended Pedagogical Reading Paths**: Role-specific study tracks across the eighteen chapters of this volume. {#tbl-pedagogical-paths}
