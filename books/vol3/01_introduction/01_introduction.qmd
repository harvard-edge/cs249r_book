...
- [ ] **Parameter updates vs. operational faults**: can you explain why gradient descent over continuous weights $\theta$ cannot eliminate runtime network timeouts or file-lock contention?
- [ ] **Epistemic fracture**: can you identify the state desynchronization that occurs when a sandbox filesystem mutates while the prompt transcript remains static?
- [ ] **Runtime hardening**: can you identify which deterministic engineering primitives (supervisors, idempotency keys, circuit breakers) are required to guard stochastic policies?
