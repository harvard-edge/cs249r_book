...
The empirical manifestation of this physical boundary is mapped in @fig-vol3-agent-systems-tax. Profiling an autonomous coding agent executing a benchmark task on SWE-bench Verified reveals the striking magnitude of the **Agent Systems Tax**: across an 8.5-minute repair trajectory, non-neural host operations—isolated Docker workspace setup (8.1 kJ, 6.7 percent), file indexing and ripgrep queries (16.8 kJ, 13.8 percent), and pytest suite compilation and verification (56.1 kJ, 46.2 percent)—account for fully **69.3 percent** of total AC mains electrical energy consumption (81.0 kJ) and **82.4 percent** of wall-clock makespan. Pure neural token generation on accelerator tensor cores consumes only **30.7 percent** of energy (40.5 kJ) and **17.6 percent** of runtime. Furthermore, during the 420 seconds spent in host execution, synchronous tool waiting strands up to 51.2 GB of GPU HBM while the server chassis draws continuous idle baseline power. This empirical profile mathematically confirms the central thesis: autonomous agent performance, reliability, and operating costs are dominated by host operating system orchestration, virtualization sandboxes, and verification harnesses, rather than neural model scaling alone.

::: {#fig-vol3-agent-systems-tax fig-env="figure" fig-pos="htb" fig-cap="**The Agent Systems Tax**: Measured execution and AC mains electrical energy profile of an autonomous code repair trajectory on SWE-bench Verified. (Panel A) Execution timeline contrasting traditional single-burst LLM inference against an autonomous multi-turn agent loop, where synchronous tool waits strand accelerator memory while the host chassis draws baseline idle-wait power. (Panel B) Empirical energy breakdown across an 8.5-minute task: non-neural host operations (pytest verification, file search, container setup) consume 69.3 percent of total AC mains energy (81.0 kJ), while pure neural inference on GPU tensor cores accounts for only 30.7 percent (40.5 kJ). (Panel C) Software 3.0 systems balance matrix highlighting the 2.26× energy imbalance and why agency is fundamentally a computer systems engineering discipline." fig-alt="Three-panel diagram showing the Agent Systems Tax. Panel A compares a continuous green bar for traditional LLM inference against a segmented timeline for an autonomous agent loop with alternating green neural think bursts and long gray and red tool execution phases, with a red bracket indicating 420 seconds of memory stranding. Panel B displays horizontal bars for energy consumption: Pytest Compilation at 46.2 percent in red, Neural Token Generation at 33.3 percent in green, File Search at 13.8 percent in dark slate, and Docker Boot at 6.7 percent in light slate. Panel C presents a summary card with badges showing 69.3 percent Host Systems Tax and 30.7 percent Neural Inference, accompanied by runtime ratios and the core architectural thesis."}
![](images/svg/agent_systems_tax.svg)
:::
