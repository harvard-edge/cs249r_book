...
The empirical manifestation of this physical boundary is mapped in @fig-vol3-agent-systems-tax. Profiling an autonomous coding agent executing a benchmark task on SWE-bench Verified reveals the striking magnitude of the **Agent Systems Tax**: across an 8.5-minute repair trajectory, non-neural host operations—isolated Docker workspace setup (8.1 kJ, 6.9 percent), file indexing and ripgrep queries (16.8 kJ, 14.4 percent), and pytest suite compilation and verification (56.1 kJ, 48.0 percent)—account for fully **69.3 percent** of total AC mains electrical energy consumption (81.0 kJ) and **82.4 percent** of wall-clock makespan. Pure neural token generation on accelerator tensor cores consumes only **30.7 percent** of energy (35.9 kJ) and **17.6 percent** of runtime. Furthermore, during the 420 seconds spent in host execution, synchronous tool waiting strands up to 51.2 GB of GPU HBM while the server chassis draws continuous idle baseline power. This empirical profile mathematically confirms the central thesis: autonomous agent performance, reliability, and operating costs are dominated by host operating system orchestration, virtualization sandboxes, and verification harnesses, rather than neural model scaling alone.

::: {#fig-vol3-agent-systems-tax fig-env="figure" fig-pos="htb" fig-cap="**The Agent Systems Tax**: Measured execution and AC mains electrical energy profile of an autonomous code repair trajectory on SWE-bench Verified. (Panel A) Execution timeline contrasting traditional single-burst LLM inference against an autonomous multi-turn agent loop, where synchronous tool waits strand accelerator memory while the host chassis draws baseline idle-wait power. (Panel B) Empirical energy breakdown across an 8.5-minute task: non-neural host operations (pytest verification, file search, container setup) consume 69.3 percent of total AC mains energy (81.0 kJ), while pure neural inference on GPU tensor cores accounts for only 30.7 percent (35.9 kJ). (Panel C) Software 3.0 systems balance matrix highlighting the 2.26× energy imbalance and why agency is fundamentally a computer systems engineering discipline." fig-alt="Three-panel diagram showing the Agent Systems Tax. Panel A compares a continuous green bar for traditional LLM inference against a segmented timeline for an autonomous agent loop with alternating green neural think bursts and long gray and red tool execution phases, with a red bracket indicating 420 seconds of memory stranding. Panel B displays horizontal bars for energy consumption: Pytest Compilation at 48.0 percent in red, Neural Token Generation at 30.7 percent in green, File Search at 14.4 percent in dark slate, and Docker Boot at 6.9 percent in light slate. Panel C presents a summary card with badges showing 69.3 percent Host Systems Tax and 30.7 percent Neural Inference, accompanied by runtime ratios and the core architectural thesis."}

```{python}
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ AGENT SYSTEMS TAX (FIGURE)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @fig-vol3-agent-systems-tax—execution timeline and energy breakdown
# │
# │ Goal: Contrast neural inference vs. non-neural host systems tax in agents.
# │ Show: 69.3 percent AC energy and 82.4 percent time in non-neural host tasks;
# │       420s of stranded GPU HBM during tool waits.
# │ How: Multi-panel plot with timeline, bar chart, and systems balance matrix.
# └─────────────────────────────────────────────────────────────────────────────
import sys
...
gs = fig.add_gridspec(
    2, 2, height_ratios=[1.0, 1.25], width_ratios=[1.4, 0.95], hspace=0.45, wspace=0.26, top=0.92, bottom=0.08, left=0.08, right=0.96
)
...
for ax in [ax_top, ax_card]:
    ax.axis("off")

# Panel A: Execution Timeline Trace
ax_top.set_xlim(-10, 610)
ax_top.set_ylim(-0.4, 3.4)

ax_top.text(0, 3.25, "Panel A: Execution Timeline — Stateless Inference vs. Autonomous Agent Loop", fontsize=10.5, fontweight="bold", ha="left")

ax_top.text(0, 2.70, "Traditional LLM Query (Stateless Inference)", fontsize=8.8, fontweight="bold")
rect_llm = patches.Rectangle((0, 2.05), 510, 0.50, linewidth=1.0, edgecolor="#166534", facecolor="#16A34A")
ax_top.add_patch(rect_llm)
ax_top.text(
    255,
    2.30,
...
ax_top.text(520, 2.30, "100% Neural Compute\nZero Tool Wait", ha="left", va="center", color="#166534", fontsize=7.8, fontweight="bold")

ax_top.text(0, 1.55, "Autonomous Agent Loop (SWE-bench Verified 8.5-Minute Trajectory)", fontsize=8.8, fontweight="bold")
...
for x_start, width, label, fc, tc, sub in phases:
    rect = patches.Rectangle((x_start, 0.85), width, 0.52, linewidth=1.0, edgecolor="#0F172A" if sub == "GPU" else "#94A3B8", facecolor=fc)
    ax_top.add_patch(rect)
    ax_top.text(x_start + width / 2.0, 1.11, label, ha="center", va="center", color=tc, fontsize=7.0 if width < 50 else 7.6, fontweight="bold")

ax_top.plot([0, 510], [0.55, 0.55], color="#DC2626", linestyle="--", linewidth=1.4)
ax_top.plot([0, 0], [0.47, 0.63], color="#DC2626", linewidth=1.4)
ax_top.plot([510, 510], [0.47, 0.63], color="#DC2626", linewidth=1.4)
ax_top.text(
    255,
    0.16,
    "Host Systems Tax & Memory Stranding: 420s of non-neural host operations strand GPU HBM (KV cache pinned) while host draws 140–220W",
    ha="center",
    va="center",
    color="#B91C1C",
    fontsize=7.8,
...
# Panel B: Measured Electrical Energy Breakdown
categories = [
    "Docker Boot & Workspace Setup",
    "File Search & Ripgrep Indexing",
    "Neural Token Generation (GPU)",
    "Pytest Compilation & Verification",
]
durations = [45, 120, 90, 255]
power_watts = [180, 140, 399, 220]
energies_kj = [8.1, 16.8, 35.9, 56.1]
total_energy_kj = sum(energies_kj)
percentages = [e / total_energy_kj * 100.0 for e in energies_kj]

y_pos = np.arange(len(categories))
bar_colors = ["#94A3B8", "#64748B", "#16A34A", "#CB202D"]

bars = ax_bar.barh(y_pos, percentages, color=bar_colors, edgecolor="#1E293B", linewidth=0.8, height=0.55, zorder=3)

ax_bar.set_xlim(0, 62)
ax_bar.set_yticks(y_pos)
ax_bar.set_yticklabels(categories, fontsize=8.0, fontweight="bold")
ax_bar.set_xlabel("Share of Total AC Mains Electrical Energy (%)", fontsize=8.5, fontweight="bold")
ax_bar.set_title("Panel B: Measured AC Electrical Energy (116.9 kJ Total)", fontsize=10.0, fontweight="bold", pad=8)

ax_bar.grid(axis="x", linestyle="--", linewidth=0.5, color="#CBD5E1", alpha=0.7, zorder=1)
ax_bar.spines["top"].set_visible(False)
ax_bar.spines["right"].set_visible(False)

for i, (bar, pct, kj, dur, w) in enumerate(zip(bars, percentages, energies_kj, durations, power_watts)):
    w_val = bar.get_width()
    ax_bar.text(w_val + 1.2, i, f"{pct:.1f}% ({kj:.1f} kJ)\n{dur}s @ {w}W", va="center", ha="left", fontsize=7.2, fontweight="bold")

# Panel C: Systems Balance Card
...
ax_card.text(5.0, 9.15, "Panel C: Software 3.0 Systems Balance", ha="center", va="top", fontsize=9.8, fontweight="bold")

badge_host = patches.Rectangle((0.8, 6.7), 4.0, 1.8, facecolor="#FEE2E2", edgecolor="#FECACA", linewidth=1.0)
ax_card.add_patch(badge_host)
ax_card.text(2.8, 8.0, "Host Systems Tax", ha="center", va="center", fontsize=7.6, fontweight="bold", color="#991B1B")
ax_card.text(2.8, 7.2, "69.3%", ha="center", va="center", fontsize=15.0, fontweight="bold", color="#B91C1C")

badge_gpu = patches.Rectangle((5.2, 6.7), 4.0, 1.8, facecolor="#DCFCE7", edgecolor="#BBF7D0", linewidth=1.0)
ax_card.add_patch(badge_gpu)
ax_card.text(7.2, 8.0, "Neural Inference", ha="center", va="center", fontsize=7.6, fontweight="bold", color="#166534")
ax_card.text(7.2, 7.2, "30.7%", ha="center", va="center", fontsize=15.0, fontweight="bold", color="#15803D")

bullets = [
    ("• Wall-Clock Time:", "82.4% Host  vs.  17.6% GPU"),
    ("• Energy Imbalance:", "2.26× Non-Neural Energy Footprint"),
    ("• Host Effectors:", "Pytest, Ripgrep, Docker, Git"),
    ("• Memory Stranded:", "51.2 GB HBM pinned during tests"),
]

y_start = 5.7
for label, val in bullets:
    ax_card.text(0.9, y_start, f"{label} {val}", ha="left", va="center", fontsize=7.6, fontweight="bold", color="#1E293B")
    y_start -= 0.65

thesis_box = patches.Rectangle((0.7, 0.4), 8.6, 2.5, facecolor="#EFF6FF", edgecolor="#BFDBFE", linewidth=0.8)
ax_card.add_patch(thesis_box)
thesis_title = "THE ARCHITECTURAL THESIS:"
thesis_body = (
    "Autonomous agents are fundamentally a computer\n"
    "systems discipline. Optimizing neural token decode\n"
    "addresses less than one-third of the energy and\n"
    "less than one-fifth of the latency of agentic loops."
)
ax_card.text(1.0, 2.65, thesis_title, ha="left", va="top", fontsize=7.2, fontweight="bold", color="#1E40AF")
ax_card.text(1.0, 2.25, thesis_body, ha="left", va="top", fontsize=7.0, color="#1E293B", linespacing=1.2)
...
**Problem:**
An autonomous coding agent executes a bug-repair task over a trajectory of $H = 8$ turns. The system runs an open-weights 70B parameter dense model served across an 8-GPU node (NVIDIA H100 SXM5, aggregate memory bandwidth $26.8\text{ TB/s}$, FP8 Tensor Cores).
...
Across the full $H = 8$ turn trajectory:
