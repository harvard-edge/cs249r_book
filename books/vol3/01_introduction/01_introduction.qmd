...
Long horizons also change where the time goes. Every turn alternates between the model generating a proposal and a tool carrying it out, and tool calls that compile code, run test suites, or wait on remote services routinely outlast the model calls that requested them. The worked profile in @sec-vol3-intro-duration-accounting shows a tool running three times as long as the model call before it. While the tool runs, the model has nothing to do for this trajectory, yet the serving system may still be holding the trajectory's attention state, its KV cache, in accelerator memory so that the next call can resume quickly.
...
**Variables** (illustrative prices and sizes):

* **Context per turn**: $K = 64{,}000$ input tokens at \$3.00 per million.
* **Output per turn**: $L = 1{,}024$ tokens at \$15.00 per million.
* **Advisory limit**: The system prompt says to stop after five failed attempts; the instruction stops governing the model's proposals, and the loop runs until a four-hour platform timeout, $N = 450$ turns.
* **Mechanical limits**: A turn ceiling $T_{\max} = 25$ and a cost ceiling $C_{\max} = \$5.00$, both enforced by the runtime.

**Math**:

Each turn costs
$$C_{\text{turn}} = 64{,}000 \times \frac{\$3.00}{10^6} + 1{,}024 \times \frac{\$15.00}{10^6} = \$0.192 + \$0.015 = \$0.20736.$$

Under the advisory limit, the loop costs $450 \times \$0.20736 \approx \$93.31$ and bills about 29 million tokens.

Under the runtime limits, cumulative cost after 24 turns is $24 \times \$0.20736 \approx \$4.98$. Before dispatching turn 25, the runtime projects $\$4.977 + \$0.207 \approx \$5.18 > C_{\max}$ and rejects the call:
$$\mathcal{I}(a_{25}, \mathcal{S}_{\text{sys}}) = \bot \quad [\texttt{BUDGET\_EXHAUSTED}]$$

The trajectory ends as a failure at \$4.98, and the runtime discards the sandbox's uncommitted changes.

**Systems insight**: The ceiling cut the cost of this failure by about 95 percent without making the model any better at debugging. Input tokens dominate each turn's cost here because the whole context is re-sent every turn, so the price of a loop grows with the context it drags along.
...
**Problem**: A bug-repair trajectory runs $H = 8$ turns. Should the team make model calls faster or make the tools faster?

**Variables** (illustrative per-turn profile):

* **Model call**: 0.14 s to read an 18,000-token context plus 4.95 s to generate 450 tokens, so $T_{\text{model}} = 5.09$ s.
* **Tool**: Compiling a changed module and running targeted tests, $T_{\text{tool}} = 16.2$ s.
* **Runtime**: Snapshotting, diffing, and running checks, $T_{\text{runtime}} = 1.4$ s.
* **External wait**: $T_{\text{wait}} = 0.3$ s.
* **Option A**: A faster model or serving path that makes model calls $3.5\times$ faster.
* **Option B**: Build caching and incremental testing that cut tool time from 16.2 s to 4.2 s.

**Math**:

Each turn takes $5.09 + 16.2 + 0.3 + 1.4 = 22.99$ s, so $T_{\text{task}} = 8 \times 22.99 \approx 183.9$ s. Model time is $8 \times 5.09 = 40.72$ s, giving $f_{\text{model}} = 40.72 / 183.9 \approx 0.22$.

For option A,
$$S_{\text{trajectory}} = \frac{1}{0.78 + 0.22/3.5} \approx 1.19,$$
and even an infinitely fast model gives at most $1 / 0.78 \approx 1.28$.

For option B, each turn takes $5.09 + 4.2 + 0.3 + 1.4 = 10.99$ s, $T_{\text{task}} \approx 87.9$ s, and the speedup is $183.9 / 87.9 \approx 2.09$.

**Systems insight**: Here the tools, not the model, bound the trajectory. Cutting tool time more than doubled the trajectory's speed, while a model $3.5\times$ faster improved it by less than a fifth. Profile the whole turn before optimizing any part of it.
