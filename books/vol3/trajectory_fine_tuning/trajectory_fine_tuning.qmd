...
```{python}
#| label: vol3-tuning-memory-anchors
#| echo: false
# +-----------------------------------------------------------------------------
# | KV, QUANTIZED WEIGHT, AND ADAPTER FOOTPRINTS FOR A 70B AGENT
# +-----------------------------------------------------------------------------
# | Context: The KV-cache bullet below, the QLoRA paragraph, and the adapter
# |          multi-tenancy paragraph in this chapter.
# |
# | Goal:    Hold the three memory anchors this chapter quotes for a 70B agent,
# |          so the KV figure, the quantization figure, and the adapter figure
# |          all follow from the same model shape.
# | Show:    KV cache at agent context length under grouped-query and multi-head
# |          attention, base weights at 16-bit and 4-bit, and LoRA adapter size.
# | How:     Model shape and per-element widths from MLSysIM. The adapter count
# |          is 2 * r * d_model per projection, over the query and value
...
class TuningMemory:
    # +-- 1. LOAD (Constants) --------------------
    large = Models.Language.Llama3_70B
    small = Models.Language.Llama3_8B
    session_tokens = 32_000  # scenario assumption: context a 30-turn agent session reaches
    lora_rank = 16  # scenario assumption: rank used by the serving runtimes cited
    lora_projections = 2  # scenario assumption: adapters on the query and value projections

    # +-- 2. EXECUTE (The Compute) ---------------
    kv_gqa = large.get_kv_cache_size(seq_len=session_tokens, batch_size=1, precision=BYTES_FP16)
    group_ratio = large.heads // large.kv_heads
    kv_mha = kv_gqa * group_ratio
    weights_fp16 = memory_from_params(large.parameters, BYTES_FP16)
    weights_nf4 = memory_from_params(large.parameters, BYTES_INT4)
    quantization_ratio = (weights_fp16 / weights_nf4).to("").magnitude

    # A rank-r adapter on one projection holds two matrices of r by d_model.
    large_adapter = 2 * lora_rank * large.hidden_dim * large.layers * lora_projections * BYTES_FP16
    small_adapter = 2 * lora_rank * small.hidden_dim * small.layers * lora_projections * BYTES_FP16
    adapter_fraction = (large_adapter / weights_fp16).to("").magnitude

    # +-- 3. GUARD (Invariants) ------------------
    check(group_ratio == 8, f"Llama-3-70B groups eight query heads per key-value head, got {group_ratio}")
    check(abs(quantization_ratio - 4) < 0.01, f"NF4 should quarter the 16-bit footprint, got {quantization_ratio}")
    check(adapter_fraction < 0.001, f"A rank-{lora_rank} adapter must be a tiny fraction of the base model, got {adapter_fraction}")
    check(large_adapter > small_adapter, "The adapter for the larger model must be the larger of the two")

    # +-- 4. OUTPUT (Formatting) -----------------
    session_tokens_str = fmt_int(session_tokens)
    kv_gqa_str = fmt_memory(kv_gqa, unit=GB, precision=1)
    group_ratio_mult_str = fmt_multiple(group_ratio, precision=0)
    kv_heads_str = fmt_int(large.kv_heads)
    weights_fp16_str = fmt_memory(weights_fp16, unit=GB, precision=1)
    weights_nf4_str = fmt_memory(weights_nf4, unit=GB, precision=1)
    lora_rank_str = fmt_int(lora_rank)
    large_adapter_str = fmt_memory(large_adapter, unit=MB, precision=1)
    small_adapter_str = fmt_memory(small_adapter, unit=MB, precision=1)
```

   In an agent executing a 30-turn interaction where each turn appends tool definitions, execution logs, and intermediate reasoning, $T$ readily surpasses `{python} TuningMemory.session_tokens_str` tokens. In a 70-billion-parameter model using 16-bit precision and grouped-query attention with `{python} TuningMemory.kv_heads_str` key-value heads, serving a single agent session requires `{python} TuningMemory.kv_gqa_str` of high-bandwidth memory (HBM) exclusively for KV tensors. Multi-head attention over the full model width would cost `{python} TuningMemory.group_ratio_mult_str` that. When thousands of concurrent agents run in parallel, memory bandwidth and capacity become the primary throughput bottleneck [@kaplan2020scaling]. The window grew; the cost of occupying it did not fall. @Fig-vol3-tuning-context-scaling charts three orders of magnitude of headroom, and a thirty-turn session that appends tool definitions and execution logs at every step will consume that headroom long before the conversation ends.

::: {#fig-vol3-tuning-context-scaling fig-env="figure" fig-pos="htb" fig-cap="**The Context Window Explosion**: Supported context length by release date, spanning two thousand to two million tokens between 2020 and 2024. The ceiling rose faster than the cost of filling it fell, so a long window is an invitation to a memory bill rather than a solution to one." fig-alt="Log-scale scatter of model context length by release date, rising from two thousand tokens to two million across the 2020 to 2024 window."}
...
# Load data
df = pd.read_csv("vol3/trajectory_fine_tuning/data/context_length_scaling.csv")
...
# Plot
ax.scatter(df['Release_Date'], df['Context_Length'], color="#2c3e50", s=60, alpha=0.8, edgecolors="white", linewidth=1.5, zorder=3)

# Highlight milestones
highlights = ["GPT-1", "GPT-3", "GPT-4", "GPT-4 Turbo", "Gemini 1.5 Pro (2M)"]
for _, row in df.iterrows():
    if row['Model'] in highlights:
        ax.annotate(row['Model'], 
                    (mdates.date2num(row['Release_Date']), row['Context_Length']),
                    xytext=(-10, 10), textcoords='offset points',
                    fontsize=9, fontweight='bold', color="#2c3e50",
                    bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="gray", alpha=0.8))
    elif row['Model'] == "Claude 2.1":
        ax.annotate(row['Model'], 
                    (mdates.date2num(row['Release_Date']), row['Context_Length']),
                    xytext=(10, -10), textcoords='offset points',
...
ax.set_ylabel("Context Length (Tokens, Log Scale)", fontsize=10, fontweight='bold')
ax.set_xlabel("Release Year", fontsize=10, fontweight='bold')
ax.set_title("Evolution of foundation model context lengths", fontsize=12, fontweight='bold', pad=15)
ax.grid(True, which="major", ls="--", alpha=0.5)
ax.grid(True, which="minor", ls=":", alpha=0.2)

# Y-axis ticks
y_ticks = [512, 2048, 8192, 32768, 128000, 1048576, 2097152]
y_labels = ["512", "2K", "8K", "32K", "128K", "1M", "2M"]
...
The seminal Toolformer architecture introduced by @schick2023toolformer demonstrated that language models can autonomously teach themselves to invoke external tools via self-supervised bootstrapping without human annotation.
...
@Fig-vol3-trajectory-pipeline follows the synthetic trajectory pipeline through four coordinated stages:
...
3. **Execution-Based Rejection Sampling:**
   Candidate trajectories are verified strictly by execution. Rather than using a large language model (LLM) as a judge (which is vulnerable to style bias and circular hallucinations), verification relies on:
...
: **Trajectory Synthesis and Verification Methodologies**: Exploration method, verification mechanism, systems complexity, and data yield efficiency across the four dominant trajectory generation pipelines. {#tbl-vol3-synthesis-strategies}
...
In classical imitation learning, @ross2011reduction established that training a policy on expert demonstrations leads to quadratic error compounding in trajectory length: $\mathcal{O}(\epsilon H^2)$, where $\epsilon$ is the single-step error rate and $H$ is the trajectory horizon.
...
**Environmental causality masking**: During multi-turn agent fine-tuning, backpropagation must propagate gradients exclusively through tokens generated by the model's internal policy. All tokens injected by external environments—including stdout logs, API response payloads, and database query results—must be assigned a loss mask of zero.
...
Training on trajectories of 32K, 64K, or 128K tokens introduces severe memory constraints. While Data Parallelism (DP) and Tensor Parallelism (TP) partition model weights across accelerators, the activation memory required for the forward and backward passes scales quadratically with sequence length in standard attention. Even after FlashAttention-style streaming removes the materialized score matrix and full gradient checkpointing discards every intermediate tensor, an activation footprint survives at each layer boundary, and whether one 128K-token trajectory fits in the HBM of an 80 GB H100 is settled entirely by that residual.

::: {#nbk-finetuning-activation-memory .callout-notebook title="Activation memory for one 128K-token trajectory"}

**Problem**: How much accelerator memory does a single 128K-token agent trajectory consume in activations alone, and how far must that sequence be split before it fits?

**Variables**: A 70-billion-parameter dense transformer, taken as a scenario assumption rather than a measurement: $n_{\text{layers}} = 80$, $d_{\text{model}} = 8192$, bfloat16 at $2\text{ bytes}$ per value, micro-batch size 1, sequence length $T = 131{,}072\text{ tokens}$. Full gradient checkpointing is assumed, so only the input activation to each transformer layer survives the forward pass and every intermediate tensor is recomputed during the backward pass. An un-checkpointed layer instead retains roughly $34\text{ bytes}$ per token per hidden dimension against the $2\text{ bytes}$ counted here. The device is an $80\text{ GB}$ H100, and a training node holds eight of them.

**Math**: A layer boundary stores one bfloat16 vector of width $d_{\text{model}}$ per token.

$$\text{per token per layer} = 8192 \times 2\text{ bytes} = 16{,}384\text{ bytes}$$

$$\text{per layer} = 131{,}072 \times 16{,}384\text{ bytes} = 2.147\text{ GB}$$

$$\text{per trajectory} = 80 \times 2.147\text{ GB} = 171.8\text{ GB}$$

Measured against one device, and then split across the eight devices of a node:

$$\frac{171.8\text{ GB}}{80\text{ GB}} = 2.15 \qquad \frac{171.8\text{ GB}}{8} = 21.5\text{ GB per rank}$$

Dropping checkpointing multiplies the per-layer residency by $34 / 2 = 17$:

$$171.8\text{ GB} \times 17 = 2{,}921\text{ GB} \approx 2.9\text{ TB}$$

**Result**: $171.8\text{ GB}$, which is $2.15\times$ the entire HBM of one H100 before a single parameter, gradient, or optimizer moment is stored. Three ranks is the bare arithmetic minimum and leaves nothing for weights; splitting the sequence across a full eight-device node brings activations to $21.5\text{ GB}$ per rank, about a quarter of each device. The same trajectory trained without checkpointing needs $2.9\text{ TB}$.

**Systems insight**: The binding constraint is the sequence dimension, not the parameter count, and the two are partitioned along different axes. Tensor and data parallelism reduce the $171.8\text{ GB}$ not at all, because every rank in a tensor-parallel group still carries the full sequence of layer-boundary activations. Only a split along $T$ divides that number, which is why long-trajectory fine-tuning has to budget Sequence Parallelism or Ring Attention into the parallelism plan from the start rather than reaching for it after the first out-of-memory failure.
...
@Fig-vol3-lora-adapters contrasts the two paths. Rather than updating the massive base weight matrices during training, LoRA approximates the necessary weight updates using a pair of much smaller, low-rank matrices. (The formal algebraic derivation of this matrix decomposition, initialization distributions, and forward-pass scaling is detailed in @sec-vol3-appendix-math-lora).

Because these low-rank matrices represent only a tiny fraction of the original parameter count, they can be trained quickly and stored efficiently. In QLoRA [@dettmers2023qlora], the base weights are further quantized to 4-bit NormalFloat (NF4) with double dequantization, reducing the memory footprint of a 70B parameter model from `{python} TuningMemory.weights_fp16_str` to `{python} TuningMemory.weights_nf4_str`, allowing fine-tuning to execute on commodity server hardware.

### Dynamic multi-tenant adapter swapping

For agent serving systems, the true power of LoRA lies in runtime multi-tenancy. Modern serving runtimes (such as S-LoRA and Punica) exploit the property that a LoRA adapter is small next to the base model it steers: at rank $r$ of `{python} TuningMemory.lora_rank_str` over the query and value projections, `{python} TuningMemory.large_adapter_str` for a 70-billion-parameter model and `{python} TuningMemory.small_adapter_str` for an 8-billion-parameter one.
...
3. When an Agent Control Block (ACB, detailed in @sec-vol3-descriptors-the-agent-control-block) dispatches an execution step, the runtime applies a batched general matrix multiplication (GEMM) using the corresponding adapter without reloading base model weights, achieving sub-millisecond adapter switching latency.
...
Principle \ref{pri-vol3-trajectory-post-training} is quantified in this chapter rather than restated. The reason a base model cannot act as an agent without trajectory post-training is that the behaviors an agent needs, recovering from a failed tool call and carrying a scratchpad across turns, appear in no single-turn corpus, so no amount of prompting recovers them.

## Summary {#sec-vol3-fine-tuning-summary}

Trajectory-level fine-tuning compiles fluid prompt instructions and in-context demonstrations into permanent model weights. By training foundation models directly on state transitions, tool invocations, and error-recovery sequences, post-training aligns the policy's causal priors with the requirements of autonomous closed-loop execution.

::: {.callout-takeaways title="Trajectory alignment and policy dynamics"}
...
Fine-tuning transforms an agent from an ungrounded linguistic model into an execution policy attuned to operational feedback. Yet imitation learning alone cannot surpass the distribution of its training data; when confronted with novel environment failures or complex multi-step reasoning deadlocks, the policy must possess the exploratory capacity to discover its own corrective paths.

::: {.callout-chapter-connection title="From supervised trajectories to active reinforcement"}
Supervised fine-tuning compiles known demonstration trajectories into weights, but imitation remains inherently bounded by the coverage of human or synthetic demonstrations. @Sec-vol3-reinforcement-learning steps beyond static imitation to examine active environment reinforcement learning, where policies explore state spaces, assign credit across extended horizons, and optimize directly against verifiable execution rewards.
:::
