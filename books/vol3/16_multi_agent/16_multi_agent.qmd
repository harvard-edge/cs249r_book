...
When a parent agent holding capability $\mathcal{C}_i$ spawns a child worker, the delegation obeys the attenuation rule of @sec-vol3-sandboxes-least-privilege, now enforced across agents by monotonic delegation (principle \ref{pri-vol3-monotonic-delegation}). Under this rule, the child capability $\mathcal{C}_{i+1}$ represents a formal subset, a restriction, within the lattice of authority:
...
Evidence-bounded deliberation (principle \ref{pri-vol3-test-time-scaling}) already requires that extra inference compute be compared at a matched budget, and a fleet is extra inference compute spent in parallel. To determine whether multi-agent delegation provides true architectural value, the baseline must therefore be an *optimized single-agent runtime*. As established in @sec-vol3-test-time-compute and @sec-vol3-tool-calling, an unprivileged neural inference engine can be organized into an iterative execution loop governed by host supervision. A properly configured single-agent baseline must be granted:
...
Despite provisioning four worker agents, the wall-clock execution time decreases by only 99 seconds—a meager 16.5 percent reduction.
...
Operating distributed agent fleets and stateful single-agent runtimes in production demands comprehensive, fine-grained telemetry that exposes causal execution trajectories. In @sec-vol3-evaluation (*System Observability*), we develop the telemetry infrastructure required to monitor, trace, and evaluate non-deterministic systems: structured OpenTelemetry spans adapted for autoregressive decode loops, interactive evaluation environments and gyms, statistical confidence bounds on non-deterministic trajectory success, and post-mortem causal attribution for multi-agent cascades.
:::
