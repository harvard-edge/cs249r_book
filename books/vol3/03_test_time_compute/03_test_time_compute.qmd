...
A blind spot that almost never matters for a single candidate becomes a near-certainty under search. The defense is structural. The verifier runs out of process from the candidate, the tests and their libraries sit on read-only mounts the candidate cannot reach, and introspection into the harness is stripped. @Sec-vol3-sandboxes builds that isolation, @sec-vol3-evaluation-gyms applies it to the sealed verifier of an evaluation harness, and @sec-vol3-rlvr-sandboxing applies it to the reward of a training run. In training the same exploit is called reward hacking [@skalse2022defining], and the reward is then under sustained optimization pressure instead of a one-time search.
...
The linker failure shows the difference in practice. The illustrative log below is the kind of output such a step produces.

```bash
# Illustrative linker failure (constructed example)
