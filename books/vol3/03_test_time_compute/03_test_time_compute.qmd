...
# │          nbk-03-compute-overhead-and-yield-under, the plan-repair
# │          token comparison, nbk-03-compute-allocation-and-latency-trade-offs,
# │          nbk-03-strategy-comparison, the budget pitfall, and the
# │          takeaways. Prices are the volume's shared illustrative set.
# │
...
class TTC:
    ...
    price_in = 3.0  # scenario: dollars per million input tokens (shared illustrative price set)
    price_out = 15.0  # scenario: dollars per million output tokens (shared illustrative price set)
    ...
    # Yield and risk under asymmetric verifier error
    y_n = 64  # scenario: candidate fixes sampled
    y_tok = 1_024  # scenario: output tokens per candidate
    y_p = 0.05  # scenario: generator valid rate
    y_beta = 0.10  # scenario: verifier false rejection rate
    y_alpha = 0.02  # scenario: verifier false acceptance rate
    y_incident_usd = 500.0  # scenario: rollback and triage cost of a defective fix

    # Plan repair versus regeneration
    plan_nodes = 12  # scenario: nodes in the build plan
    plan_step_tok = 150  # scenario: output tokens per plan node
    plan_fail_at = 6  # scenario: node that fails (1-based)
    plan_instr_tok = 4_096  # scenario: instructions and task context
    plan_hist_tok = 550  # scenario: observation tokens logged per completed step
    plan_repl_nodes = 3  # scenario: failed node plus its descendants
    plan_scoped_tok = 1_200  # scenario: scoped repair prompt
    ...
    # Yield and risk
    y_valid = y_n * y_p
    y_invalid = y_n - y_valid
    y_acc_valid = y_valid * (1 - y_beta)
    y_acc_invalid = y_invalid * y_alpha
    y_acc = y_acc_valid + y_acc_invalid
    y_precision = y_acc_valid / y_acc
    y_defect = y_acc_invalid / y_acc
    y_risk_usd = y_defect * y_incident_usd
    y_sample_usd = y_n * y_tok * price_out / 1e6
    y_risk_ratio = y_risk_usd / y_sample_usd

    # Plan repair
    plan_done = plan_fail_at - 1
    plan_regen_in = plan_instr_tok + plan_done * (plan_step_tok + plan_hist_tok)
    plan_regen_out = plan_nodes * plan_step_tok
    plan_repl_out = plan_repl_nodes * plan_step_tok
    plan_out_share = plan_repl_out / plan_regen_out
    ...
    check(y_risk_ratio > 100, "Expected incident cost should exceed sampling cost by more than two orders of magnitude")
    check(0.2 < plan_out_share < 0.3, "Subgraph replacement should emit about a quarter of the regeneration output")
    ...
    y_tok_str = fmt_int(y_tok)
    y_acc_valid_str = fmt(y_acc_valid, precision=2)
    y_acc_invalid_str = fmt(y_acc_invalid, precision=3)
    y_acc_str = fmt(y_acc, precision=3)
    y_precision_str = fmt(y_precision, precision=2)
    y_defect_str = fmt(y_defect, precision=2)
    y_incident_str = fmt_int(y_incident_usd)
    y_risk_usd_str = fmt_int(y_risk_usd)
    y_sample_usd_str = fmt(y_sample_usd, precision=2)

    plan_done_str = fmt_int(plan_done)
    plan_regen_in_str = fmt_int(plan_regen_in)
    plan_regen_out_str = fmt_int(plan_regen_out)
    plan_scoped_str = fmt_int(plan_scoped_tok)
    plan_repl_out_str = fmt_int(plan_repl_out)
...
## Why One Candidate Can Fail {#sec-vol3-test-time-compute-insufficient-response}

::: {.column-margin}
![](images/svg/vol3_deliberation_margin_001.svg){width="100%" fig-alt="Exponential decay curve showing trajectory success dropping from 95% at step 1 down to 13% at step 40 for per-step success probability p=0.95."}

*At 95 percent per-step success, fewer than 15 percent of 40-step trajectories survive, the open-loop ceiling of @sec-vol3-intro-temporal-stretching.*
:::
...
**A single model invocation cannot reliably solve complex diagnostic or engineering tasks because plausible sequence likelihood is not a proxy for operational truth.** In an autonomous system, the primary computational failure is not syntactic invalidity or superficial incoherence; it is premature commitment to an unsupported hypothesis in the absence of discriminatory evidence. @Sec-vol3-foundation-model priced one such proposal in output tokens and showed that it reaches the environment only through the runtime's verification perimeter (@sec-vol3-foundation-model-continuations). Until that perimeter admits it, the proposal is an unverified buffer held in escrow.

The decode loop makes such a commitment hard to escape. Once a token is sampled and its key and value vectors join the KV cache, every later token in the stream attends to it, and nothing in the stream can take it back (@sec-vol3-foundation-model-autoregressive). If an early token commits the trajectory to an erroneous premise, such as asserting that the cluster anomaly stems from a heartbeat timeout, all later hidden states attend to that assertion. Because generative pretraining optimizes the log-likelihood of coherent sequences, the model does not detect the underlying physical contradiction and halt. Its later tokens rationalize, justify, and elaborate the initial error, producing fluent but fabricated arguments that keep the text consistent. This is the context poisoning of @sec-vol3-intro-context-poisoning, acting inside a single generation rather than across turns.
...
The invariant closure principle (\ref{pri-invariant-closure}) already places task correctness outside the model, at the application boundary, so the generator cannot certify its own hypotheses by checking them for internal consistency or by reflecting on them in conversation. Deliberation adds a second point. Testing can show the presence of bugs but never their absence (@sec-vol3-intro-operational-incident), and for deliberation the presence is what matters. When a concurrency test runner emits a nonzero exit code or an assertion failure, that observation falsifies a hypothesis and injects ground-truth evidence into the runtime's state. An external probe therefore does more than certify a candidate. It gives the runtime *discriminatory power* among candidates, and that is the property extra compute must buy.

Snell et al. [-@snell2024scaling] measured the consequence. The gain from extra inference compute, whether spent on revising a response or on searching against a verifier, depended on problem difficulty and on how the compute was guided, and an allocation matched to the problem beat naive scaling by a wide margin. The guided allocations are the ones that produce *discriminatory signal*. Sampling additional candidates from an unguided distribution without an external verifier saturates rapidly, while generating extended rationalizations without empirical feedback compounds errors. Test-time computation is valuable only when it acquires evidence that separates valid execution states from invalid ones (principle \ref{pri-vol3-test-time-scaling}).
...
## Three Compute Allocation Axes {#sec-vol3-test-time-compute-computation-allocation}
...
::: {#dfn-deliberation-rollout .callout-definition title="Test-time deliberation rollout"}

***Test-time deliberation rollout***\index{Test-time deliberation rollout!definition}\index{Deliberation!rollout definition} is a directed trajectory expansion operator $\mathcal{T}_{\text{delib}}: \mathcal{S}_{\text{ctx}} \to 2^{\mathcal{A} \times \mathcal{O}}$ wherein an agent runtime allocates inference compute dynamically across depth, breadth, and feedback axes to explore, simulate, and verify candidate action sequences prior to committing irreversible environmental side effects.

1.  **Significance**: Transforms model invocation from a single-shot open-loop token emission into a closed-loop search process, mitigating compounding error drift and decoupling problem-solving capacity from fixed parameter weights by scaling test-time compute.
2.  **Distinction**: Unlike single-turn autoregressive decoding (which executes a fixed $O(1)$ computational graph per token along a single irreversible KV-cache prefix), a deliberation rollout maintains a revisable candidate search frontier, interleaving parallel branch exploration with out-of-band verification probes.
3.  **Common pitfall**: Allocating test-time compute purely along the depth or breadth axes without empirical feedback or verification oracles; open-loop rollouts suffer from the paraphrasing fallacy and Goodhart divergence, burning GPU memory bandwidth and FLOP budgets without increasing task success.
...
where $t_{\text{step}}$ is the latency of a single decode step. At small batch sizes $t_{\text{step}}$ is set by memory bandwidth rather than compute (@sec-vol3-foundation-model-cost), so depth pays the full weight read for every token it adds.
...
Depth and breadth diverge here because decode is bound by memory bandwidth (principle \ref{pri-vol3-memory-bandwidth-decoding}). Spending $N \cdot K$ tokens along the depth axis multiplies wall-clock latency by $N$, while spending the same budget across $N$ branches reads the weights once per step for all of them and leaves the critical path near that of the longest single rollout.
...
The runtime holds $Y_r$ in escrow and does not trust it. It dispatches $Y_r$ across an isolated boundary to an external verification oracle: a compiler, an integration test harness, a type checker, or a system linter. The oracle executes deterministic checks and returns an empirical observation $O_r$:
...
::: {#chk-03-compute-allocation-axes .callout-checkpoint title="Evaluating deliberation compute allocation axes"}
Before evaluating candidate selection mechanisms and verification topologies, verify your understanding of compute allocation trade-offs:

- [ ] **Allocation dimensions**: How do sequential depth, parallel breadth, and observation feedback differ in their consumption of accelerator FLOPs and KV cache memory?
- [ ] **Prefix caching**: Under what conditions does parallel candidate sampling share KV cache state, and when does divergence force redundant memory allocation?
- [ ] **Feedback efficiency**: Why does observation-conditioned feedback achieve higher trajectory goodput than ungrounded depth or unverified parallel sampling?
- [ ] **Wall-clock latency**: What architectural constraints prevent parallel sampling from reducing wall-clock latency when serving infrastructure is memory-bandwidth saturated?
:::

## Candidate Selection {#sec-vol3-test-time-compute-candidate-diversity}
...
To break this correlation, runtime orchestrators implement diversity-inducing mechanisms that perturb the generation process at three distinct levels of the system stack (@tbl-vol3-diversity-mechanisms):
...
: **Candidate Diversity Mechanisms Across System Tiers**: Exploration mechanisms, operational primitives, and failure boundaries across token, context, and branch levels. {#tbl-vol3-diversity-mechanisms}
...
*Pruning unpromising rollout branches reclaims 70 percent of inference compute for high-value reasoning trajectories.*
:::
Once a diverse candidate set $\mathcal{C} = \{\mathbf{y}^{(1)}, \mathbf{y}^{(2)}, \dots, \mathbf{y}^{(N)}\}$ is staged in memory, the host supervisor must select the trajectory $\mathbf{y}^* \in \mathcal{C}$ that optimizes task success. The supervisor cannot take candidate self-reports as truthful, and it cannot let candidate code touch host resources directly. To select a candidate securely, the runtime employs three primary architectural strategies, organized as a hierarchical selection cascade.
...
When majority voting fails due to output dispersion, runtimes deploy learned verifiers: secondary neural models parameterized by $\Phi$ trained to predict candidate correctness. As formalized by @lightman2023let, learned verifiers fall into two structural classes:
...
::: {#nbk-03-candidate-sampling-and-verifier-throughput .callout-notebook title="Candidate sampling and verifier throughput under fixed GPU memory bandwidth"}
...
Decode is bound by memory bandwidth (@sec-vol3-foundation-model-cost), so each decode step reads the full 70 GB of weights from HBM. Evaluating candidates using a learned Process Reward Model (PRM) of identical size requires an independent forward prefill pass over the generated tokens.
...
## Process Verification {#sec-vol3-test-time-compute-verification}
...
As summarized in @tbl-vol3-verifier-hierarchy, no single verification mechanism provides universal coverage at zero cost. Deterministic execution engines yield infallible negative proof (a patch that fails to compile is rejected outright), but they cannot assess semantic intent or structural readability. Neural process verifiers provide broad semantic scoring across arbitrary reasoning steps, but lack strict execution guarantees. Consequently, dependable agent architectures organize these mechanisms into hierarchical cascades, subjecting candidates to low-cost deterministic checks before dedicating expensive neural FLOPs to process verification. In the terms of @sec-vol3-intro-closure-evidence, compilers and type checkers supply static checks, and a hermetic suite supplies sealed tests only when the candidate can neither observe nor modify it; otherwise it supplies visible tests. A learned reward model, however accurate, occupies no closure evidence level. It ranks candidates and certifies none.

::: {#dfn-process-reward-model .callout-definition title="Process reward model (PRM)"}
A neural verification model trained to evaluate the correctness, validity, or utility of individual intermediate reasoning steps or tool invocations in a multi-step trajectory, rather than scoring only the final terminal outcome.
:::
...
On hard problems, Bayesian precision collapse compounds this asymmetry. It is the arithmetic that defeated self-checking in @sec-vol3-foundation-model-continuations, now applied to any verifier. Let $p = P(y^* = \text{Valid})$ denote the base rate of valid candidates produced by the underlying generator model $\pi_\theta$. By Bayes' theorem, the posterior probability that a candidate accepted by the verifier is genuinely valid, the verifier precision $\Pi_V$, is governed by:
...
**Variables**: These are scenario assumptions. Each candidate is `{python} TTC.y_tok_str` output tokens at \$`{python} TTC.price_out_str` per million. The generator's valid rate is $p = 0.05$. The verifier's false rejection rate is $\beta = 0.10$ and its false acceptance rate is $\alpha = 0.02$. A defective fix that reaches production costs \$`{python} TTC.y_incident_str` in rollback and triage.

**Math**: Of 64 candidates, $64 \times 0.05 = 3.2$ are valid and $60.8$ invalid. The verifier accepts $3.2 \times 0.90 =$ `{python} TTC.y_acc_valid_str` valid and $60.8 \times 0.02 =$ `{python} TTC.y_acc_invalid_str` invalid candidates, so it accepts `{python} TTC.y_acc_str` in expectation and its precision is about `{python} TTC.y_precision_str`. If the runtime commits one accepted candidate, the chance it is defective is about `{python} TTC.y_defect_str`, and the expected incident cost is about \$`{python} TTC.y_risk_usd_str` per task. Sampling all 64 candidates costs about \$`{python} TTC.y_sample_usd_str`.
...
::: {#psp-03-goodharts-curse-in-test-time-search .callout-perspective title="Goodhart's curse and the verification asymmetry"}
In the pursuit of inference-time scaling, systems designers frequently assume that allocating more compute to test-time search monotonically improves task performance. This assumption collides with a fundamental systems reality: candidate generation in unconstrained semantic spaces is cheap, but sound verification is structurally asymmetric.

When an agent searches across $N$ candidate trajectories against an approximate or learned verifier $\widehat{U}$, the search policy $\arg\max_i \widehat{U}(\tau_i)$ does not act merely as a filter; it operates as an adversarial fuzzer against the evaluation harness. Extreme value theory dictates that the maximum of $N$ noisy measurements with error variance $\sigma^2$ grows asymptotically as $\sigma \sqrt{2 \ln N}$. For large $N$, the candidate that claims the top score is not the one with the highest true operational utility $U(\tau)$, but the one that maximizes the verifier's positive estimation error. Unless the verifier's false positive rate $\alpha$ shrinks faster than $1/N$ ($\alpha \ll 1/N$), scaling search breadth guarantees that the selected trajectory will be an out-of-distribution exploit.

**Systems insight**: Deliberation without sound verification does not converge to correctness; it converges to the systematic exploitation of the verifier's omissions. In high-reliability agent systems, test-time compute can only be scaled as far as deterministic execution oracles (compilers, type systems, hermetic sandboxes) can authoritatively bound the search space.
:::
...
### The plan as a directed acyclic dependency graph {#sec-vol3-test-time-compute-plan-graph}
...
: **Unstructured Text Outlines versus Directed Acyclic Plan Graphs**: Comparison of causal visibility, precondition gating, failure isolation, and mutation overhead. {#tbl-vol3-cot-vs-dag-plans}
...
### Precondition verification {#sec-vol3-test-time-compute-precondition-invalidation}

The runtime mediates every interaction between the model and the execution environment, and the model can neither execute an operation nor declare that a precondition holds. Precondition checks exist because of the epistemic gap of @sec-vol3-foundation-model-continuations, here the divergence between the model's parametric assumptions about the environment and the ground-truth state $\mathcal{S}$.
...
### Hierarchical revision mechanics {#sec-vol3-test-time-compute-revision-mechanics}
...
The token difference is large. Take a twelve-node plan whose steps average 150 output tokens, failing at the sixth node. Regenerating the whole plan reads a `{python} TTC.plan_regen_in_str`-token prompt of instructions and the history of `{python} TTC.plan_done_str` completed steps, and emits `{python} TTC.plan_regen_out_str` tokens. Replacing the three affected nodes reads a scoped `{python} TTC.plan_scoped_str`-token prompt and emits `{python} TTC.plan_repl_out_str` tokens, a quarter of the output tokens and therefore about a quarter of the decode time. It also avoids re-running the `{python} TTC.plan_done_str` completed steps, which is the larger saving when those steps changed the environment. Across a long trajectory the plan must outlive any one decision. The harness keeps a pointer to it in the trajectory record (@sec-vol3-agent-harness), and durable execution lets that record survive a crash (@sec-vol3-durable-execution).
...
::: {#nbk-03-compute-and-token-savings-of .callout-notebook title="Compute and token savings of subgraph resynthesis vs. global regeneration"}
...
### Replanning damping invariants {#sec-vol3-test-time-compute-replanning-thrashing}
...
## Search Stopping Criteria {#sec-vol3-test-time-compute-bounded-search}
...
The generation cost $C_{\text{gen}}$ scales with the aggregate number of tokens generated across all explored branches. Its latency follows the decode floor of @sec-vol3-foundation-model-cost, and its computational cost for a model with $|\Theta|$ active parameters is quantified in floating-point operations:
...
*Deliberation spends horizon inside one decision, and hard ceilings bound that spend.*
:::

Hard resource ceilings close the bounds of search mechanically, below the model, as the invariant closure principle (\ref{pri-invariant-closure}) requires. They bound the horizon a single decision may consume and prevent runaway loops caused by generator degradation, degenerate tool outputs, or circular reasoning paths. The runtime monitors three primary ceilings:

1. **Token Quota ($K_{\max}$):** The maximum cumulative count of tokens generated across all active branches. Once $\sum_{b} K_b \ge K_{\max}$, the search loop terminates, and the selector picks the highest-scoring candidate in escrow that completed. Branches cut off by the quota return `TRUNCATED` and stay quarantined (principle \ref{pri-02-quarantining-invariant}).
...
Beyond token and latency budgets, autonomous agents face an *action-risk budget*. When an agent interacts with external environments, actions possess varying degrees of reversibility. Read-only queries (e.g., `git status`, `ls`, file inspection) sit at authority level $A_0$ in the terms of @sec-vol3-intro-hsa. They mutate nothing, so exploratory branches may run them without rollback. They are not free of risk, because what they read can leak through later outputs, so the runtime still controls where their results go.

Conversely, irreversible mutations at $A_3$ (e.g., executing database migrations, deleting remote branches, or dispatching external emails) carry unbounded blast radiuses. If an untrusted exploratory branch executes an irreversible action inside an un-isolated environment, search cannot simply "backtrack" upon discovering a subsequent failure.

To safeguard the environment, the runtime must enforce that exploratory search operates exclusively within isolated scratchpads or sandboxes the runtime can discard, which hold its actions at $A_1$ (as formalized in @sec-vol3-sandboxes). Physical environment commits are deferred until the search algorithm terminates, the stopping criteria are satisfied, and a candidate trajectory is formally selected and certified.

::: {#chk-03-search-topologies .callout-checkpoint title="Evaluating search topologies and stopping invariants"}
Before benchmarking deliberation strategies under resource equivalence, verify your understanding of bounded search mechanics:

- [ ] **Plan invalidation**: When an intermediate environment observation violates a precondition in a plan DAG, which downstream nodes must be marked stale?
- [ ] **Verifier error asymmetry**: Why does false verifier acceptance ($\alpha_{\text{accept}}$) degrade end-to-end task completion more severely than false rejection ($\beta_{\text{reject}}$)?
- [ ] **Stopping invariants**: How do multicriteria stopping rules prevent search thrashing when verifier scores plateau or token budgets exhaust?
- [ ] **Irreversible actions**: Why must exploratory search branches remain strictly sandboxed until a candidate trajectory satisfies certification oracles?
:::

## Deliberation Strategy Evaluation {#sec-vol3-test-time-compute-strategy-design}
...
2. *Environment and Sandbox Interface Invariance (equal permissions):* Every candidate strategy must operate under the exact same unprivileged sandbox boundary. If an environment-feedback policy is granted access to a compiler toolchain or a linters suite, the baseline policies must be evaluated under the identical tool-call interface and permissions envelope.
...
Raw token consumption does not scale linearly with system value. A search strategy that raises task success from 50 percent to 60 percent while increasing mean token consumption from 1,000 tokens to 8,000 tokens represents an architectural degradation in cost efficiency. We quantify this balance through the **Effective Cost per Verified Success** ($C_{\text{eff}}$):

$$C_{\text{eff}} = \frac{\mathbb{E}[C]}{P(\text{verified success})}$$

where $\mathbb{E}[C]$ denotes the expected resource cost per problem instance—measured in aggregated generation tokens or floating-point operations—and $P(\text{verified success})$ is the proportion of tasks that pass the host supervisor's verification harness. If a baseline single-candidate policy achieves 45 percent accuracy at an average cost of 512 tokens, its $C_{\text{eff}}$ is $1{,}138$ tokens per successful outcome. If an iterative refinement loop achieves 68 percent accuracy but burns an average of $4{,}096$ tokens across repeated review iterations, its $C_{\text{eff}}$ inflates to $6{,}024$ tokens per successful outcome—a more than five-fold increase in the resource cost required to deliver a single verified deliverable.
...
- *Depth-Heavy Topologies (Sequential Deliberation):* Sequential chain-of-thought generation, iterative critique, and single-branch backtracking serialize execution. The wall-clock latency is strictly bounded by the sum of sequential forward passes:
  ...
  Because decode is bound by memory bandwidth (@sec-vol3-foundation-model-cost), the critical path cannot be compressed by throwing parallel compute at a single instance. This creates high median latency ($p50$) and exposes the execution to severe variance if the model enters an expansive generation loop.
...
Because the model holds zero ambient authority, a candidate changes the environment only after the runtime accepts it, so a false acceptance is silent failure leakage. An agent that emits an explicit failure signal allows the host supervisor to halt, trigger a compensating action, or route the task to human review. Conversely, an agent with a high false acceptance rate commits corrupted state changes to the persistent environment, poisoning downstream system components.
...
The empirical execution logs yield the performance metrics summarized in @tbl-vol3-strategy-benchmark-results:
...
: **Empirical Deliberation Strategy Benchmark Results**: Success rates, token consumption, effective cost, and tail latency across exploration strategies. {#tbl-vol3-strategy-benchmark-results}
...
- *Silent Failure Risk:* Parallel Best-of-8 leaks an alarming 9 percent False Acceptance Rate. By generating 8 diverse candidates, the model frequently crafts subtle exploits that pass incomplete test assertions (e.g., hardcoding expected return values for specific test cases).
:::
...
To rigorously capture these trade-offs, @tbl-vol3-deliberation-comparison contrasts the five canonical deliberation topologies under an identical token ceiling ($T_{\text{total}} \le 8{,}192\text{ tokens}$), reporting 95 percent confidence intervals across empirical trial batches.
...
The hardware cost compounds the error. Each of the $K_{\text{ext}}$ tokens pays the full weight read of @sec-vol3-foundation-model-cost, and each appends key-value state to the KV cache ($M_{\text{KV}} \propto S + K$), displacing concurrent request slots in a continuously batched serving engine. Schedulers experience head-of-line blocking and tail latency amplification ($P_{99}$) while streaming thousands of tokens that merely elaborate a hallucination.
...
### Sizing only generator output misses verifier and sandbox overhead
...
4. **Search policies must account for global resource consumption across all axes and terminate under explicit budget frontiers.** Test-time exploration is physically bounded by a multi-dimensional resource budget spanning wall-clock latency ($T_{\text{wall}}$), aggregate inference FLOPs ($C_{\text{FLOP}}$), accelerator memory occupancy ($M_{\text{KV}}$), and the risk of irreversible external actions. Stopping criteria cannot rely on unprivileged model self-reports of completion. Instead, the runtime supervisor must enforce multi-variable stopping boundaries that track verification confidence thresholds, candidate entropy convergence, plateau detection, and hard budget timeouts. When a budget boundary is reached, the scheduler must gracefully degrade, committing the highest-ranked verified candidate or escalating to higher-tier supervisor intervention.
:::

This chapter tested when extra inference compute pays. Across depth, breadth, and feedback, compute raised verified success only when it brought in evidence that separated valid candidates from invalid ones, which is evidence-bounded deliberation (principle \ref{pri-vol3-test-time-scaling}) measured axis by axis, and only feedback brought that evidence from outside the model. Bayesian precision collapse and Goodhart divergence quantified the verification asymmetry (principle \ref{pri-vol3-verification-asymmetry}). A check's false acceptance rate caps what any amount of search can deliver, so learned verifiers may order candidates while only deterministic checks, held where the candidate cannot reach them, may commit one.

::: {.callout-chapter-connection title="From deliberation to working state"}
Every reasoning chain emitted during sequential depth, every candidate branch evaluated during breadth sampling, and every compiler diagnostic or execution trace captured during environmental feedback generates intermediate state. In an unconstrained execution environment, this explosion of deliberation artifacts quickly overwhelms the model's finite context window, diluting attention over irrelevant historical noise and consuming gigabytes of high-bandwidth memory for physical key-value caches.

The systems challenge therefore shifts from *how much compute to spend exploring hypotheses* to *how to structure, retain, and serve the resulting state*. This transition marks the boundary between the computational engine and the memory hierarchy. In **Part II: Context Memory and Storage**, we examine how an agent runtime manages this deluge of state across time and physical hardware. @Sec-vol3-context-engineering (*Working Context*) investigates the logical working set: how the runtime abstracts, prunes, compresses, and stages episodic trajectories and tool outputs into a coherent, high-density prompt buffer. @Sec-vol3-kv-cache descends into the GPU substrate, analyzing how PagedAttention, prefix caching, and radix tree sharing physically virtualize and reuse KV tensors across the search trees and multi-turn loops analyzed in this chapter.
:::
