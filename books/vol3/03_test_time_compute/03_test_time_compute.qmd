...
**Problem**: In the stale-read diagnosis, how much generation does a 40 ms discriminating probe replace?

**Variables**: These are scenario assumptions. The serving endpoint decodes at $t_{\text{tok}} = 20$ ms per output token and charges \$10 per million output tokens (@sec-vol3-foundation-model-cost explains where such rates come from). The model's answer runs to $K = 1{,}024$ output tokens and commits to $H_1$ at token 16. The prior is $P(H_1) = 0.75$ and $P(H_2) = 0.25$. The probe runs for 40 ms on a CPU in the sandbox and returns about 60 tokens of output, which the next call reads as input.

**Math**: The full answer takes $1{,}024 \times 20\text{ ms} \approx 20.5$ s and costs $1{,}024 \times \$10 / 10^6 \approx \$0.010$. Everything after token 16 elaborates an unchecked premise, which is $1{,}008$ tokens, or $20.2$ s and 98 percent of the generation. Before the probe, the runtime's uncertainty over the two hypotheses is

$$H = -\left(0.75 \log_2 0.75 + 0.25 \log_2 0.25\right) \approx 0.81\text{ bits}$$

The probe falsifies one hypothesis, which takes the uncertainty to zero. The long answer leaves it at $0.81$ bits, because its content does not depend on which defect is present.

**Systems insight**: The probe costs about 1/500 of the generation's wall-clock time and resolves all of the uncertainty that the generation resolves none of. Neither costs much in dollars; the expensive outcome is a wrong patch deployed. The runtime should buy evidence per second, not tokens per second.
...
where $P$ is the prompt length and $t_{\text{tok}}$ the per-token decode time. Decode time scales with output length, and output tokens are the expensive ones (@sec-vol3-foundation-model-cost), so depth buys its gains with the costliest resource the call has.
...
First, reasoning tokens are output tokens. They are decoded and billed like any other output, and a hard problem can emit many thousands of them before the visible answer begins. Second, the call surface exposes that spend as a control. The reasoning-token budget field caps how much depth one call may use (@sec-vol3-foundation-model-contract), which makes depth a per-call allocation the runtime owns rather than a fixed property of the model. Third, trained reasoning still draws only on the prompt and the weights. A reasoning model can recheck its arithmetic inside the trace, but it cannot observe whether a patch passes a test it never ran. Training moves the depth axis a long way without supplying the external evidence of the feedback axis.
...
**Scenario**: An agent must patch a lock-free ring buffer that contains a memory-ordering race. The prompt is $P = 4{,}096$ tokens. Scenario assumptions: decode runs at $t_{\text{tok}} = 10$ ms per output token, a small batch of parallel streams decodes at nearly the same per-token rate as one stream, and the endpoint charges \$2 per million input tokens and \$10 per million output tokens with no prefix caching. Prefill time is small next to decode at these lengths and is ignored (@sec-vol3-foundation-model-cost).

**Option A (depth)**: One generation of $4{,}096$ output tokens. Latency is $4{,}096 \times 10\text{ ms} \approx 41$ s. Cost is $4{,}096 \times \$2/10^6 + 4{,}096 \times \$10/10^6 \approx \$0.049$. The answer argues at length that the design is correct and ships the race.

**Option B (breadth)**: Four candidates of $1{,}024$ tokens each, generated in parallel. Latency is $1{,}024 \times 10\text{ ms} \approx 10$ s. Each candidate is a separate call that reads the prompt, so input is $4 \times 4{,}096 = 16{,}384$ tokens and output is $4{,}096$ tokens, for about \$0.074. All four candidates omit the same barrier.

**Option C (feedback)**: Three rounds of $1{,}024$ tokens. After each round the sandbox compiles the candidate and runs a race detector in $t_{\text{check}} = 1.5$ s, returning a 340-token report. Prompts grow from $4{,}096$ to $5{,}460$ to $6{,}824$ tokens, so input totals $16{,}380$ tokens and output $3{,}072$ tokens, for about \$0.063. Latency is $3 \times (10.2 + 1.5)\text{ s} \approx 35$ s. The round-2 report names the unordered store, and round 3 passes.

**Math**: Cost per accepted task divides spend by accepted tasks. Options A and B spend \$0.049 and \$0.074 for zero accepted patches, so their cost per accepted task is unbounded. Option C spends \$0.063 for one.

**Systems insight**: Breadth was four times faster than depth and depth used the fewest input tokens, but only feedback bought evidence about the defect, so only feedback produced an accepted task. Price test-time compute per accepted task, not per call.
...
**Scenario**: A coding agent needs a patch built in four reasoning steps of 128 tokens each. Scenario assumptions: the generator decodes at $t_{\text{tok}} = 10$ ms per output token and a small batch decodes at nearly the one-stream rate; generator output costs \$10 per million tokens; a small PRM scores one 128-token step in 15 ms and costs \$0.50 per million input tokens; a sandboxed test run takes 0.5 s, with 8 runs in parallel.

**Option A (best-of-16)**: Sixteen complete candidates of 512 tokens, decoded as one batch. Output is $16 \times 512 = 8{,}192$ tokens, or about \$0.082. Decode takes $512 \times 10\text{ ms} \approx 5.1$ s. All 16 candidates are tested in two waves of 8, adding 1.0 s, so latency is about 6.1 s and the search uses 16 sandbox runs.

**Option B (beam, $B = 4$, $M = 3$)**: Step 1 expands the root into 3 extensions, step 2 expands those 3 into 9, and steps 3 and 4 expand 4 retained prefixes into 12 each, for $3 + 9 + 12 + 12 = 36$ step segments. Output is $36 \times 128 = 4{,}608$ tokens, or about \$0.046. The PRM reads the same $4{,}608$ tokens for about \$0.002. Decode takes four sequential steps of $128 \times 10\text{ ms}$, about 5.1 s; scoring adds $4 \times 15\text{ ms} = 0.06$ s; testing the 4 survivors in one wave adds 0.5 s. Latency is about 5.7 s with 4 sandbox runs.

**Math**: Beam search generates $1 - 4{,}608 / 8{,}192 \approx 44$ percent fewer output tokens and uses a quarter of the sandbox runs, at similar latency.
...
Benchmarks for these measurements should grade with checks the agent could not see. Repository-repair benchmarks built from real issues and graded by hidden tests [@jimenez2024swebench] are the model. A strategy evaluated on visible tests is partly evaluated on its ability to fit them.

### A worked comparison

::: {#nbk-03-strategy-comparison .callout-notebook title="Three strategies on one repair workload"}

**Scenario**: A repair service handles tasks with a 60-second deadline. Scenario assumptions: one attempt is $1{,}000$ output tokens at $t_{\text{tok}} = 20$ ms, or 20 s; one sealed test run takes 10 s; a single attempt passes with probability $p = 0.40$; output costs \$10 per million tokens, and input cost is left out for simplicity.

**Option A (single attempt)**: $P(\text{accepted}) = 0.40$. Spend is $1{,}000$ tokens, so $C_{\text{eff}} = 1{,}000 / 0.40 = 2{,}500$ tokens, about \$0.025. Latency is $20 + 10 = 30$ s.

**Option B (best-of-8, sealed tests as selector)**: The eight candidates are correlated and behave like three independent attempts, so $P(\text{accepted}) = 1 - 0.6^3 \approx 0.78$. Because the selector is the sealed suite itself, this equals the pass@8 ceiling. Spend is $8{,}000$ tokens, so $C_{\text{eff}} \approx 8{,}000 / 0.78 \approx 10{,}200$ tokens, about \$0.10. Candidates and tests run in parallel, so latency stays near 30 s, using 8 sandbox runs.

**Option C (feedback, up to three rounds)**: After a failure, the test report raises each revision's pass probability to $0.35$. Then $P(\text{accepted}) = 1 - 0.6 \times 0.65 \times 0.65 \approx 0.75$. Expected rounds are $1 + 0.6 + 0.6 \times 0.65 = 1.99$, so expected spend is $1{,}990$ tokens and $C_{\text{eff}} \approx 1{,}990 / 0.75 \approx 2{,}670$ tokens, about \$0.027. Each round takes 30 s, so the expected latency is about 60 s and the worst case 90 s.

**Math**: Option C reaches nearly the success rate of Option B at about a quarter of its cost per accepted task, and costs only about 7 percent more per accepted task than a single attempt. Option C also breaks the 60-second deadline in its third round, which occurs on $0.6 \times 0.65 = 39$ percent of tasks.

**Systems insight**: No strategy wins on every axis. Under the 60-second deadline, Option B is the only one that reaches about 0.78 success; with a relaxed deadline, feedback nearly matches it at a quarter of the cost. Raw accuracy cannot rank the three, because they spend different budgets; cost per accepted task at the deadline puts them on one scale.
...
Budgets written as candidates times output tokens times price miss most of what a search spends. Consider a tree search that generates 12 steps of 512 tokens, $6{,}144$ generator tokens in all, from a $4{,}096$-token context, and scores each step with a PRM that must re-read the whole $4{,}608$-token prefix. The PRM reads $12 \times 4{,}608 = 55{,}296$ tokens, about nine times what the generator wrote. Testing the four leaves in sandboxes that take 4.5 s to start and 35 s to run, two at a time, adds $4 \times 39.5 / 2 = 79$ s, compared with about 123 s of generation at 20 ms per token. Every open branch also holds its context in the serving system while it waits (@sec-vol3-kv-cache). A search budget must meter each resource separately, as a vector of ceilings
...
* **Only evidence pays**: Extra inference raises verified success only when it brings in an observation that separates valid candidates from invalid ones. A 40 ms probe that falsifies a hypothesis can be worth more than 20 s of generation that cannot.
