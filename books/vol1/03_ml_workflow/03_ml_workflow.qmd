...
These survey proportions and iteration dynamics demonstrate why data engineering cannot be treated as mere preparatory plumbing. Data pipelines are the primary source of operational friction, iteration cycles, and system-level risk. Mastering data infrastructure (@sec-data-engineering) provides maximum leverage before addressing the model architectures and training optimizations that ingest its output.

Late discoveries carry severe penalties,[^fn-boehm-cost-curve] as formalized by the constraint propagation principle (@sec-ml-workflow-decisions-cascade-system-98f4). Violations discovered after deployment force expensive rollbacks across every preceding stage. This asymmetry demands explicit **stage interface contracts**\index{Stage interface specification!contracts}: validating outputs at every stage boundary intercepts defects before they compound into systemic rework. @Sec-ml-workflow-stage-interface-specification-ae3c formalizes these contracts across the six stages.
...
Tracing MobileNetV2 through these transitions demonstrates that while a workflow diagram suggests sequential progression, production engineering is fundamentally cyclic. The stage prompts below test whether the lifecycle can be traced as a coupled system.
...
Beyond procedural orchestration, each lifecycle stage maps directly to specific terms in the performance equation. This mapping reveals a workflow-level manifestation of the iron law\index{Iron law of ML systems!workflow mapping}: decisions made during data collection lock in byte volume, model development dictates operation count and hardware efficiency, and deployment infrastructure bounds fixed latency. The stage-by-stage mapping connects lifecycle execution directly to the iron law of ML systems (@sec-introduction-iron-law-ml-systems-c32a).
...
Successful deployment requires more than technical optimization. Clinician trust depends on model calibration and usable explanations, not just aggregate accuracy: a clinician who cannot interpret the system's uncertainty cannot know when additional review is warranted. **Human-in-the-loop routing**\index{Human-in-the-loop!confidence-based routing} applies a validated policy to send designated cases to a specialist rather than presenting every prediction as actionable. Beyond serving as an operational safety net during inference, human-in-the-loop adjudication closes the active learning loop: ambiguous edge cases resolved by clinicians provide high-value ground-truth annotations. Feeding these adjudicated examples back into subsequent training iterations systematically hardens the model against its known clinical blind spots. Automated image-quality checks, calibrated routing rules, and stress testing for peak volumes all contribute evidence for reliable operation; none substitutes for clinical workflow validation.
...
Together, these three patterns—constraint propagation, multi-scale feedback, and emergent complexity—elevate ML development from ad hoc experimentation into systematic engineering practice. In the chapter's illustrative model, a defect caught at monitoring carries the steepest penalty because the fix cascades backward through every preceding stage. When ignored, these principles precipitate predictable operational failures. The recurring pitfalls in @sec-ml-workflow-fallacies-pitfalls-4d91 illustrate what happens when teams violate these principles in production.
