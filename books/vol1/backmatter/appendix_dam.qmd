...
The opening chapter uses @fig-ai-triad to establish the conceptual landscape. Here, @fig-dam-axis-triad turns that map into a diagnostic sequence. First identify the binding axis, then ask how an intervention will alter the other two.
...
While the landscape classifies where a technique operates, isolating which axis actively throttles a workload requires translating these relationships into wall-clock execution time—the role of the iron law.
...
While the iron law quantifies the execution time consumed by each stage, determining whether data movement or arithmetic throughput sets the active performance ceiling depends on their ratio—the domain of arithmetic intensity.
...
These three cases illustrate bottlenecks that cross axes. In live production incidents, symptoms frequently overlap and the dominant bottleneck can shift dynamically as optimizations land. Systematic elimination requires screening profiler signals against a structured diagnostic matrix.

## Production Troubleshooting {#sec-appdx-dam-taxonomy-production-troubleshooting-02fb}

Identifying the root cause of performance bottlenecks requires systematic elimination. @Tbl-dam-troubleshooting provides a diagnostic matrix mapping common production failure modes to candidate axes and validating measurements.
...
A diagnostic hypothesis requires concrete hardware and runtime counters before taking corrective action. @Tbl-dam-tooling connects theoretical D·A·M components to the specific Linux and Python profiling utilities that expose them.
...
Profiling tools generate raw metrics—utilization percentages, FLOP counts, and bandwidth measurements—that become actionable only when evaluated against workload constraints. The D·A·M Scorecard\index{DAM taxonomy@D·A·M taxonomy!scorecard} establishes screening signals rather than universal grades, isolating which measurements warrant immediate intervention.
