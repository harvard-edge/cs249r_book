...
The energy hierarchy and the signal-density heuristic converge on the same optimization lever: effective data engineering maximizes the Data Selection Gain defined in @eq-data-selection-gain. Data cleaning is not custodial hygiene; it is **signal-to-noise engineering**\index{Signal-to-noise engineering}\index{Data cleaning!signal-to-noise engineering}. Deduplication strips away redundant mass while preserving task-relevant signal, immediately amplifying the selection gain. Active learning prioritizes high-entropy examples over repetitive samples, concentrating information density per byte transferred. Every avoided byte preserves network bandwidth, conserves storage energy, and protects accelerator duty cycles.
...
To prevent silent failures from compounding across the pipeline, data infrastructure must balance four competing operational priorities. @Fig-four-pillars organizes these priorities into four structural pillars: input integrity, deployment stability, execution performance, and data governance.
...
With a candidate configuration selected, execution hinges on orchestrating complementary acquisition channels: baseline corpora anchor the acoustic prior, targeted crowdsourcing plugs demographic blind spots, and synthetic generators supply volumetric scale under controlled reverberation. @Sec-data-engineering-strategic-data-acquisition-418f examines these acquisition strategies and their economic trade-offs.
...
The contrast in this illustrative scenario matters: weeks for human labeling, hours for GPU training, milliseconds for serving. Here, labeling is the bottleneck. A `{python} LabelingComputeRatio.labeling_budget_str` labeling budget compares with `{python} LabelingComputeRatio.train_cost_low_str` to `{python} LabelingComputeRatio.train_cost_high_str` for one 8$\times$ A100 ResNet-50 run—an asymmetric ratio of `{python} LabelingComputeRatio.ratio_low_mult_str` to `{python} LabelingComputeRatio.ratio_high_mult_str`. Within that labeling spend, the assumed effort distribution is itself skewed: `{python} LabelingComputeRatio.effort_majority_pct_str` of the work goes to `{python} LabelingComputeRatio.effort_minority_pct_str` of features—the long tail of edge cases, rare categories, and quality exceptions.
...
In the KWS pipeline, `{python} KWSAcquisitionScale.dataset_size_million_samples_str` spanning `{python} KWSAcquisitionScale.languages_count_str` demand a volume that manual collection cannot economically provide. A multi-source strategy that combines curated datasets, web scraping of video platforms and speech databases, crowdsourced collection, and synthetic generation addresses this scale requirement while maintaining coverage across acoustic environments and speaker demographics.
...
The diversity of sources (crowdsourced audio, synthetic waveforms, web-scraped content) creates specific challenges at the boundary where external data enters the controlled pipeline. Each source arrives in a different format, at a different cadence, with different quality guarantees, and the infrastructure that receives, validates, and routes this heterogeneous data must reconcile all of them.
...
Each layer in @fig-pipeline-flow serves a distinct role in preparing data for training. Selecting the right engine requires evaluating how the four pillars interact across stages: quality checks at ingestion dictate downstream storage filtering, while latency budgets during serving constrain preprocessing complexity.
...
The storage difference favors ETL in this scenario, but the listing does not price schema-change labor; the accompanying worked calculation treats that labor as a separate decision factor.
...
The latency gap between on-chip registers and remote storage spans over eight orders of magnitude, causing unbuffered remote I/O requests to stall GPU execution pipelines. Scaling these microsecond delays to human timeframes (@tbl-ml-latencies) illustrates the performance penalty of an un-cached fetch.
