...
Architectures specify what computations neural networks perform, but knowing *what* to compute is entirely different from knowing *how* to compute it efficiently. A transformer's attention mechanism requires coordinating computation across memory hierarchies and accelerator cores in patterns where naive implementations execute 100$\times$ slower than optimized ones due to uncoalesced memory accesses and repeated round-trips to high-bandwidth memory (HBM). Implementing these operations from scratch for every model would make deep learning economically infeasible. ML frameworks exist to bridge this gap by lowering abstract model graphs into hardware-aligned execution pipelines that maximize compute unit utilization while minimizing off-chip memory traffic.

A framework\index{ML framework!compiler analogy} is to machine learning what a compiler is to traditional programming. A C compiler translates human-readable code into optimized machine instructions, managing register allocation, instruction scheduling, and memory layout. An ML framework translates high-level model definitions into hardware-specific execution plans, managing operator fusion, memory reuse, and device placement. The correspondence is operational rather than figurative: modern frameworks incorporate optimizing compilers (such as Accelerated Linear Algebra (XLA) or TorchInductor) that lower graph representations directly to target machine code.

Every ML framework\index{ML framework!execution problem}\index{ML framework!differentiation problem}\index{ML framework!hardware abstraction problem}, regardless of API or design philosophy, must solve three core problems. The first is the *execution problem*: deciding when and how computation runs. A framework can execute operations immediately as written (eager execution\index{Eager execution!definition}[^fn-eager-execution-tradeoff]) or construct a computational graph\index{Computational graph!definition}[^fn-computational-graph-frameworks]\index{Deferred execution!graph construction}—a structured intermediate representation of operations and data dependencies—to optimize before launching device kernels (graph execution). This choice determines the fundamental trade-off between interactive debugging and whole-program compiler optimization.

With an execution strategy in place, the framework must solve the *differentiation problem*: computing gradients automatically. Because gradient-based training requires derivatives of a scalar loss with respect to millions or billions of parameters (@sec-neural-computation), manual calculus is impossible at scale. Frameworks implement automatic differentiation by registering derivative rules across supported operator compositions, chaining vector-Jacobian products while managing numerical precision and the memory overhead of intermediate activations retained for backward passes.

The third problem is *hardware abstraction*: targeting diverse hardware from a single interface. The same model definition should be expressible across CPUs, GPUs, Tensor Processing Units (TPUs), and mobile accelerators, even though each target presents distinct memory hierarchies, instruction sets, and interconnect bandwidths. Within the GPU domain, framework ecosystems increasingly expose custom-kernel languages such as Triton [@tillet2019], allowing developers to write high-performance, block-level kernels without hand-crafting low-level CUDA or device assembly.
...
These three problems are tightly coupled: the execution model dictates when differentiation data structures are recorded, while the hardware abstraction layer determines which graph transformations can be lowered to silicon. Unifying execution, differentiation, and hardware abstraction under physical machine constraints requires evaluating frameworks through the governing equation of machine learning performance.
...
The "source code" is the model architecture (the $O$ term). The framework's job is to take this high-level mathematical specification and compile it into a series of hardware-specific kernel launches that:

1.  Minimize data movement $(D_{\text{vol}})$ through techniques such as kernel fusion.
2.  Maximize utilization $(\eta_{\text{hw}})$ by matching operations to specialized hardware units such as Tensor Cores.
3.  Minimize overhead $(L_{\text{lat}})$ through efficient asynchronous dispatch and graph capture.

Choosing a framework means choosing the compiler that dictates hardware efficiency. Precision matters here: a definition capturing all three responsibilities separates genuine frameworks from numerical libraries that address only one.
...
The first rung established reusable hardware primitives (1979–1992). The original Basic Linear Algebra Subprograms (BLAS)\index{BLAS!historical foundation}[^fn-blas-routine-acceleration] standardized low-level linear-algebra operations [@lawson1979], while LAPACK\index{LAPACK!framework foundation}[^fn-lapack-linear-algebra] [@bai2006] built higher-level numerical factorizations and eigensolvers on top of that foundation. Naive nested loops fail to exploit memory hierarchies: without cache tiling and register blocking, memory bandwidth saturates long before arithmetic units reach peak throughput. BLAS and LAPACK solved this hardware-primitive bottleneck by establishing stable interfaces that let numerical software delegate compute-heavy operations such as matrix multiplication (`C = A @ B`)[^fn-gemm-hardware-utilization] to vendor libraries tuned for specific cache line sizes, single instruction, multiple data (SIMD) widths, and register files, rather than requiring hand-written assembly for every target architecture.

The second rung introduced unified array abstractions to unlock developer velocity (2005–2006). NumPy\index{NumPy!framework foundation}[^fn-numpy-array-abstraction] addressed the productivity barrier of low-level languages by wrapping BLAS and LAPACK in high-level Python [@harris2020]. Under NumPy's strided array model, Python manages control flow and tensor metadata (shape, strides, and data type), while the underlying C runtime dispatches vector and matrix loops across contiguous memory buffers. This vectorization contract—orchestrating high-level algorithmic intent in an interpreted language while delegating inner loops to compiled native routines—became the architectural template for scientific and numerical computing.

The third rung automated gradient computation (2007–present). Deep learning frameworks\index{Deep learning framework!automatic differentiation} such as Theano,[^fn-theano-framework-origin] TensorFlow [@abadi2016tensorflow], and PyTorch [@paszke2019pytorch] solved the scalability limits of manual differentiation. While numerical packages required practitioners to derive and implement analytical backpropagation gradients by hand—a process error-prone and unmanageable for deep networks—modern frameworks turned automatic differentiation over computational graphs into a core runtime primitive. By capturing operations in a directed acyclic graph during the forward pass, these frameworks automatically construct the backward graph to evaluate vector-Jacobian products, transforming the calculus of the chain rule into automated reverse-mode execution.
...
This progression traces four decades of numerical computing abstractions, moving from assembly-tuned subroutines to compiled graph execution engines (@fig-mlfm-timeline).
...
With that contract risk in view, modern frameworks converge on three core problems: *how* to execute computation, *how* to differentiate it, and *how* to abstract across hardware. The execution problem comes first because its resolution determines what optimizations the other two problems can exploit.
...
The execution strategy of an ML framework is fundamentally constrained by the widening compute-bandwidth gap quantified by the memory-wall equation (@eq-memory-wall). Processor arithmetic has scaled far faster than memory bandwidth, creating the memory wall\index{Memory wall!execution strategy impact}\index{Memory wall!framework execution}. Modern accelerators can perform arithmetic far faster than they can fetch data. Element-wise operations such as ReLU use only a tiny fraction of peak compute capacity, not because the hardware is slow, but because they spend nearly all their time waiting for data. @Sec-appdx-machine-foundations-roofline-model-2529 formalizes this trade-off, showing exactly when operations are memory bound vs. compute bound.
...
To expose multi-operation optimization opportunities to compilers, frameworks represent programs as a computational graph\index{Computational graph!definition (DAG)}: a directed acyclic graph (DAG) where nodes represent operations and edges represent data dependencies. This graph serves as the framework's internal model of the computation.
...
Real machine learning models require much more complex graph structures. @Fig-mlfm-comp-graph extends this representation to show a neural network computation graph alongside the system components that reason about it. The left panel depicts data flowing through six operation nodes in a directed acyclic graph, where each node's output becomes the next node's input. The right panel shows how the runtime interfaces with this graph: querying tensor lifetimes to plan memory reuse and assigning operations to accelerator streams based on data dependencies rather than program order. The critical insight is that the graph exists independently of execution, enabling the framework to optimize before any arithmetic occurs.
...
This graph representation is more than a visualization; it is the data structure that enables both efficient execution and automatic differentiation. The timing of its construction forces a fundamental systems trade-off across debugging, optimization, deployment, and flexibility. Interactive debugging demands immediate execution so developers can inspect intermediate values and step through logic with standard tools. Global optimization requires observing large operator sequences at once to fuse kernels and plan memory reuse. Standalone deployment favors graphs that run without the host interpreter, while dynamic architectures require control flow that branches on computed tensor values.
...
In this define-by-run model, when host code evaluates `y = x * 2`, the runtime dispatches the multiplication kernel immediately to the device command queue and makes the resulting tensor available for subsequent operations without delay.
...
In eager execution frameworks, operations evaluate imperatively as host code encounters them. @Fig-mlfm-dynamic-graph-flow traces this execution loop, illustrating the define-and-dispatch sequence that sends individual operations directly to device stream queues.
...
This flexibility imposes latency costs that map directly to the iron law (@sec-introduction-iron-law-ml-systems-c32a). Each training step constructs an autograd tape dynamically, adding host-side Python dispatch latency and tape allocation to $L_{\text{lat}}$. Because every operation traverses the framework dispatcher before a device kernel launches, host overhead quickly dominates when individual kernels execute in single-digit microseconds. Eager execution also lacks a whole-graph view, preventing the framework from fusing element-wise chains\index{Kernel fusion!static graph optimization} into single GPU launches or pre-allocating a static memory arena for intermediate activations ($D_{\text{vol}}$). The autograd tape must retain every tensor requested by registered backward rules, holding memory until backpropagation consumes it. Together, these costs impose a throughput ceiling on fine-grained workloads where dispatch latency overshadows device arithmetic.
...
Static graph compilation separates graph construction from tensor execution. The lifecycle in @fig-mlfm-static-graph illustrates this phase division: symbolic graph definition on the left precedes ahead-of-time optimization and runtime execution on the right.
...
Compilers such as **XLA**\index{XLA!definition}\index{XLA!graph compilation}[^fn-xla-graph-compiler] [@GoogleXLA] extend this approach, compiling captured subgraphs into fused machine code specialized for target hardware.
...
This explains when compilation can help. Dispatch costs that appear negligible for a single operation—a few microseconds per kernel launch—accumulate across thousands of operations in a forward pass. The physics of software overhead in \ref{nbk-frameworks-physics-software-overhead} models this dispatch and memory traffic.
...
These three regimes create distinct regions in the compilation decision space. @Fig-compilation-continuum maps out these regions so engineers can identify where each strategy wins. The crossover points mark where each strategy becomes optimal: the steep eager curve (highest per-execution cost) is overtaken by the moderate JIT slope, while the gentle compiled trajectory (lowest per-execution cost but largest upfront investment) dominates at high execution volumes. The slopes reveal per-execution cost; the vertical offsets reveal compilation overhead. A project's execution volume and measured costs determine which strategy minimizes total time.
...
Summed over $N_{\text{ops}}$ operations, the per-operation dispatch cost $t_{\text{dispatch}}$ accumulates into a per-call tax on execution. Whether that tax dominates depends on how the hardware execution time $T_{\text{hw}}$ compares to the software overhead $T_{\text{sw}}$ (both measured in seconds), and the regimes split sharply by model size. Workloads composed of small operations benefit disproportionately from compilation when dispatch dominates execution; model parameter count alone does not determine the gain. Notebook \ref{nbk-frameworks-dispatch-tax} contrasts a small multilayer perceptron with a large transformer layer to quantify when host overhead or device arithmetic bounds the system.
...
The dispatch tax analysis demonstrates that small operations become overhead-bound when dispatch latency exceeds device execution time. This systems constraint reaches its logical extreme at the edge of the deployment spectrum, where the memory and latency footprint of the Python runtime exceeds the physical capacity of the target hardware.

### Frameworks for the edge: TinyML and micro-runtimes {#sec-ml-frameworks-tinyml-micro-runtimes-2a1b}

The compilation continuum reaches its extreme at the far edge\index{Edge deployment!TinyML frameworks}. While cloud frameworks such as PyTorch and TensorFlow 2.x prioritize flexibility through eager execution, TinyML\index{TinyML!micro-runtimes}[^fn-tinyml-aot-extreme] systems operating on MCUs with kilobytes of memory cannot afford the overhead of a Python interpreter or a fully dynamic runtime.
...
The **differentiation problem**\index{Differentiation problem!definition} is the task of computing gradients[^fn-gradient-descent-memory] automatically. Neural network training requires derivatives of a scalar loss $\mathcal{L}$ with respect to millions or billions of parameters. When executing the forward pass, the runtime records an execution tape and saves intermediate activation buffers for subsequent derivative evaluations. Because a single scalar loss depends on many parameters, reverse-mode automatic differentiation (AD)\index{Automatic differentiation!definition}[^fn-autodiff-reverse-mode] is efficient: a single reverse traversal computes all requested parameter gradients, whereas recovering the same full gradient with forward mode requires one tangent direction per parameter. Major ML frameworks therefore use reverse-mode AD for ordinary scalar-loss training [@baydin2018].
...
Building on the backpropagation algorithm introduced in @sec-neural-computation, the systems engineering of differentiation addresses how frameworks represent computation graphs, manage memory for intermediate values, and orchestrate the backward pass efficiently across accelerators. The framework mechanizes the chain rule as large-scale systems bookkeeping: tracking operator dependencies, staging activation buffers, and dispatching backward kernels for the training algorithms detailed in @sec-model-training. @Lst-auto_diff_intro illustrates the core idea with a simple three-operation function.
...
Frameworks decompose this function into elementary operations, each with a known local derivative, and then combine these local derivatives via the chain rule to compute gradients through supported compositions. The systems challenge is executing this transformation under physical hardware constraints: the framework must record the computation graph during forward execution, retain necessary intermediate tensors in device memory, and schedule backward kernels without exhausting accelerator HBM. Production runtimes address these challenges through structured graph recording, caching allocators, and reverse-mode traversal.

### Forward and reverse mode differentiation {#sec-ml-frameworks-forward-reverse-mode-differentiation-f70a}

Automatic differentiation\index{Automatic differentiation!forward vs. reverse mode} operates in two distinct traversal directions: forward mode and reverse mode. The choice between them governs whether gradient evaluation cost scales with the number of input dimensions or output dimensions. Because deep learning objectives evaluate a single scalar loss with respect to millions or billions of parameters, this mathematical asymmetry dictates the memory footprint and execution schedule of the underlying framework.

#### Forward mode {#sec-ml-frameworks-forward-mode-c3ff}

Forward mode automatic differentiation\index{Automatic differentiation!forward mode} computes directional derivatives concurrently with the primal evaluation. For each intermediate variable $v_i$, the runtime evaluates both the primal value $v_i$ and its directional derivative (tangent) $\dot{v}_i = \sum_j \frac{\partial v_i}{\partial x_j} \dot{x}_j$. Because tangents flow downstream alongside primal values, forward mode requires no execution tape and never retains forward activations for a subsequent backward pass.

To see the mechanism concretely, consider computing both the value and derivative of $f(x) = x^2 \sin(x)$. @Lst-forward_mode_ad shows how forward mode evaluates local derivative rules alongside each elementary operation, applying the chain and product rules at every step:
...
Forward mode mechanizes this paired tracking through dual numbers\index{Automatic differentiation!forward-mode dual numbers}\index{Dual number!forward mode AD}—values of the form $v + \dot{v}\epsilon$ where $\epsilon^2 = 0$. Evaluating an arithmetic operator or elementary function over dual numbers automatically yields the updated primal and tangent without graph retention. @Lst-forward_mode_dual demonstrates this trace evaluated at $x = 2.0$ with seed $\dot{x} = 1.0$:
...
A single forward pass with a seeded tangent vector $\dot{\mathbf{x}}$ evaluates a Jacobian-vector product (JVP), $J\mathbf{v}$, where $J \in \mathbb{R}^{M \times P}$ is the Jacobian matrix. When an objective maps $P$ parameter inputs to a single scalar loss ($M = 1$), recovering the full gradient $\nabla f \in \mathbb{R}^P$ requires setting $\dot{\mathbf{x}}$ to each of the $P$ standard basis vectors in turn. Recovering gradients with forward mode therefore requires $P$ distinct forward evaluations (or propagating $P$-wide tangent bundles), scaling compute and memory traffic as $\mathcal{O}(P)$ with parameter count. For networks with millions or billions of parameters, this scaling makes forward-mode gradient calculation intractable, although forward JVPs remain highly effective when input dimensions are small or for evaluating directional derivatives and Hessian-vector products.

#### Reverse mode {#sec-ml-frameworks-reverse-mode-d328}

Reverse-mode automatic differentiation\index{Automatic differentiation!reverse mode}\index{Automatic differentiation!reverse-mode backpropagation} inverts the traversal order to exploit the many-to-one geometry of neural network training\index{Backpropagation!computational asymmetry}. Instead of propagating tangents downstream from inputs, reverse mode propagates adjoints $\bar{v}_i = \partial \mathcal{L} / \partial v_i$ upstream from the scalar loss $\mathcal{L}$ to all parameters. Mathematically, while forward mode computes Jacobian-vector products ($J\mathbf{v}$), reverse mode evaluates vector-Jacobian products (VJPs), $\mathbf{u}^T J$. Seeding the reverse sweep with $\bar{\mathcal{L}} = 1$ yields the complete parameter gradient vector $\nabla_{\boldsymbol{\theta}}\mathcal{L}$ in a single backward pass, regardless of whether the model contains ten parameters or ten billion.

Reverse mode operates on the same three-operation function $f(x) = x^2\sin(x)$ from @lst-auto_diff_intro, where $x$ reaches the output through two distinct computational paths: the square and the sine. @Algo-reverse-mode-ad-trace defines the reverse-mode traversal algorithm over a directed acyclic graph before tracing this concrete evaluation.
...
The algorithmic structure of @algo-reverse-mode-ad-trace reveals its systems cost. The forward pass records operator dependencies and buffers intermediate values required by backward derivative rules until the reverse sweep consumes them. Reverse mode trades activation storage and graph bookkeeping for constant-time $\mathcal{O}(1)$ gradient passes with respect to parameter count.

For the concrete function in @lst-auto_diff_intro evaluated at $x=2.0$, the forward pass records $a=x^2=4.0$, $b=\sin(x)\approx0.9093$, and $c=ab\approx3.637$. Seeding $\bar{c}=1.0$, the product rule yields adjoints $\bar{a}=0.9093$ and $\bar{b}=4.0$. The square branch contributes $0.9093\cdot4.0\approx3.6372$ to $\bar{x}$, while the sine branch contributes $4.0\cos(2.0)\approx-1.6646$. Accumulating both paths gives the exact derivative $\partial c/\partial x = \bar{x} \approx 1.973$.

This single backward traversal computes $\partial c/\partial x$ regardless of how many fan-out paths connect $x$ to $c$. In a deep neural network, every weight tensor influences the scalar loss through thousands of divergent paths across successive layers; reverse mode aggregates all incoming adjoints in one reverse sweep. This efficiency makes training a `{python} GPT3ReverseModeParams.gpt3_params_b_str`-parameter model such as GPT-3 computationally tractable: reverse mode requires $\mathcal{O}(1)$ backward passes relative to parameter count.

Executing this traversal in hardware exposes a physical constraint: evaluating local derivatives during the backward pass requires the operating point established during forward execution. The framework must therefore determine which forward tensors to persist in device memory, when to allocate them, and when to free them. Modern frameworks manage this lifecycle through dynamic computational graphs and gradient accumulation.[^fn-gradient-accumulation-batch]
...
@Lst-reverse_simple_nn illustrates these requirements in a two-layer network, showing the forward pass storing intermediate activations and the backward pass consuming them to evaluate parameter gradients.
...
Three implementation requirements emerge from this example. First, the framework must track dependencies between operations to determine the correct reverse traversal order. Second, intermediate values (`hidden`, `activated`) must persist in device memory until the backward pass consumes them. Mathematically, this persistence is required because the chain rule evaluates local derivatives at the forward operating point—for example, evaluating $\frac{\partial}{\partial z}\text{ReLU}(z) = \mathbb{I}(z > 0)$ requires the pre-activation $z$, and computing a dense layer's weight gradient $\frac{\partial \mathcal{L}}{\partial W} = X^T \delta$ requires the forward input activation tensor $X$. Third, every operation needs both a forward implementation and a corresponding backward rule. These requirements define the engineering surface of automatic differentiation, where activation memory persistence constitutes the primary scaling bottleneck.
...
A `{python} GPT3MemoryFootprint.gpt3_params_b_str`-parameter model in FP16 requires `{python} GPT3MemoryFootprint.gpt3_fp16_gb_str` just for weights, exceeding the physical memory of any single accelerator. Reverse-mode AD compounds this memory pressure by retaining operation-dependent forward activations for backward derivative rules. In an illustrative 100-layer network trained with batch size 64, saved activations contribute an additional 8--12 GB alongside weights, gradients, and optimizer states. Memory capacity, rather than raw floating-point throughput, frequently establishes the feasibility boundary of training.

This saved-activation footprint accumulates across successive layers. @Lst-reverse_memory demonstrates how each forward layer adds an activation tensor that must persist until the backward sweep unwinds to that layer.
...
Frameworks mitigate this memory wall through two primary architectural strategies. The first is activation checkpointing\index{Activation checkpointing!definition}\index{Activation checkpointing!memory-compute trade-off} (or gradient checkpointing): rather than buffering every intermediate activation, the runtime stores only selected segment boundary tensors and recomputes dropped intermediates on demand during the backward pass. The essential systems mechanism is the runtime contract: the framework marks boundary activations as persistent and regenerates intermediate values when backward execution reaches that segment; @sec-model-training-gradient-accumulation-checkpointing-mechanics-fb09 details the placement policies used to partition networks. @Lst-checkpoint-recompute illustrates this contract: the forward pass preserves only segment boundaries, and the backward pass re-executes the forward segment to regenerate activations rather than loading them from global memory.
...
The second strategy is *operation fusion*\index{Kernel fusion}\index{Kernel fusion!eliminating intermediate allocations}.[^fn-operation-fusion-bandwidth] Rather than executing matrix multiplication, bias addition, and activation functions as separate kernels that serialize intermediate tensors to accelerator global memory (HBM), compilers fuse compatible operators into a single kernel that passes intermediate values through registers or on-chip shared memory.
...
Checkpointing, fusion, and specialized kernels address the physical bottlenecks of automatic differentiation, but production frameworks encapsulate these mechanisms beneath high-level training loops. A standard PyTorch step—`optimizer.zero_grad()`, forward pass, `loss.backward()`, and `optimizer.step()`—appears as four sequential calls. Behind this interface, the runtime builds and traverses dynamic computation graphs, allocates and frees intermediate buffers, schedules backward kernels across CUDA streams, and manages numerical precision. The same graph machinery supports second-order derivatives via nested `torch.autograd.grad` calls and automatic mixed precision via `autocast`\index{autocast!mixed-precision context}, which dispatches reduced-precision kernels while preserving FP32 master weights.

#### PyTorch autograd internals {#sec-ml-frameworks-pytorch-autograd-internals-4fa0}

The autograd system\index{Autograd!engine, internals}\index{PyTorch!autograd system} is the engine that resolves the differentiation problem introduced in @sec-ml-frameworks-three-problems-every-framework-must-solve-317d. Three systems principles govern its design: the data structure that enables efficient gradient computation, the memory cost of maintaining that data structure, and the control mechanisms that production workflows require. Reasoning through these principles clarifies why training memory substantially exceeds inference memory for identical architectures, and how frameworks manage that overhead.

##### The reverse-linked graph structure {.unnumbered}

During the forward pass, autograd dynamically constructs a directed acyclic graph of `Node` (or `Function`) objects\index{Autograd!reverse-linked graph}\index{Gradient function node!computation graph}. Each node encapsulates an operator's backward differentiation logic and retains references to the tensors required to evaluate its vector-Jacobian products. Because edges point backward from outputs toward inputs, the engine initiates topological traversal immediately upon receiving the scalar loss adjoint.

Every tensor produced by a differentiable operation contains a `grad_fn` attribute referencing the `Function` that computed it. Each `Function` links to antecedent nodes through its `next_functions` list, forming a chain from the scalar loss back to leaf parameter tensors. @Lst-grad-fn-chain inspects this pointer chain for a simple composite computation:
...
This reverse-linked graph imposes a fundamental memory constraint: the graph topology and all referenced forward tensors must remain resident in device memory until backward traversal executes their respective nodes. While the graph metadata (pointers and node descriptors) occupies minimal memory, the tensor buffers it references can consume gigabytes of accelerator capacity, scaling directly with model depth and batch size.

##### The memory-compute trade-off {#sec-ml-frameworks-principle-2-memorycompute-tradeoff-19f2}

Every forward activation saved for the backward pass remains pinned in device memory until its corresponding derivative rule executes. These retained activations represent a primary driver of training memory overhead, compounding the footprint of weights, gradients, and optimizer states. Because different operations require different forward tensors—for example, matrix multiplication requires input activations, while elementwise ReLU requires only the pre-activation sign bit—the exact memory profile depends on operator semantics.
...
Consider an illustrative ResNet-50\index{ResNet-50!training memory footprint} training scenario with `{python} ResNetMemory.resnet_params_m_str` parameters (~`{python} ResNetMemory.resnet_fp32_mb_str` of FP32 weights), batch size 64, and $224{\times}224$ images. With `{python} ResNetMemory.resnet_act_min_gb_str`--`{python} ResNetMemory.resnet_act_max_gb_str` of saved activations, gradients add ~`{python} ResNetMemory.resnet_fp32_mb_str`, and two Adaptive Moment Estimation (Adam) FP32 moment buffers add ~`{python} ResNetMemory.resnet_adam_mb_str`. Summing those four components gives an `{python} ResNetMemory.resnet_training_min_gb_str`--`{python} ResNetMemory.resnet_training_max_gb_str` training scenario, roughly `{python} ResNetMemory.resnet_training_ratio_range_mult_str` the FP32 weight storage alone. This metric reflects peak training memory, distinct from inference memory which requires no optimizer states, gradients, or backward activation buffers.

This disparity underscores why accelerator training capacity cannot be deduced from parameter counts alone. Total training memory obeys the four-component formulation ($M_{\text{total}} = M_{\text{weights}} + M_{\text{gradients}} + M_{\text{optimizer}} + M_{\text{activations}}$) plus allocator overhead, as derived in @sec-appdx-algorithm-foundations-true-cost-training-memory-e54e.

Frameworks provide three graph-level controls to manage this trade-off. Gradient checkpointing\index{Activation checkpointing!recomputation strategy} [@chen2016training] alters what the graph retains: the engine stores only coarse segment boundaries and regenerates intermediate activations during the backward pass. In iron law terms, checkpointing increases the arithmetic operation count ($O$) to curtail memory footprint and transfer volume ($D_{\text{vol}}$). Tensor detachment provides a complementary optimization: calling `.detach()` removes a tensor from autograd tracking, preventing the engine from retaining activations along upstream graph paths. This is essential for transfer learning where frozen feature extractors require no gradient computation, eliminating unnecessary activation storage ($D_{\text{vol}}$). Mixed-precision training provides a third lever: representing activations and weights in 16-bit formats directly halves tensor volume ($D_{\text{vol}}$) while accelerating arithmetic throughput on Tensor Cores ($R_{\text{peak}}$). @Sec-model-training formalizes these graph mechanisms into cluster sizing decisions.

##### Extensibility and control {.unnumbered}

Production training systems require fine-grained control over gradient propagation beyond standard backward sweeps. In practice, workloads demand three core capabilities: selective gradient computation\index{Selective gradient computation} for freezing layers during transfer learning via `requires_grad=False` or `.detach()`; gradient inspection and modification\index{Gradient inspection} for clipping, debugging, or logging intermediate adjoints via runtime hooks; and custom differentiation rules\index{Custom differentiation rule} for non-standard operations such as specialized CUDA kernels or novel activation functions.

These extension points operate via callbacks invoked during graph construction and reverse traversal, preserving the core engine's execution path. In PyTorch, these capabilities map to specific runtime mechanisms: retained graphs, gradient accumulation buffers, custom `autograd.Function` implementations, tensor hooks, and detachment boundaries. @Tbl-autograd-control summarizes these mechanisms, their systems function, and their memory or correctness trade-offs.
...
Custom autograd functions\index{Custom autograd function!backward implementation} expose the differentiation contract directly by transferring state retention responsibility from the engine to the programmer: the implementation explicitly defines which forward tensors to preserve in `ctx.save_for_backward()` and evaluates the corresponding vector-Jacobian products in `backward()`. @Lst-custom-autograd-function demonstrates this interface for a custom multiply-add operation:
...
These autograd mechanisms connect directly to the silicon contract. The reverse-linked graph defines the sequence of backward kernels the device must execute (the $O$ term). The memory-compute trade-off governs intermediate tensor residency and memory traffic through the memory hierarchy (the $D_{\text{vol}}$ term). The extensibility hooks allow engineers to prune unnecessary graph paths and intercept data flow. The interplay between graph retention and memory bandwidth leads directly to mixed-precision training, which reduces memory footprint while increasing hardware execution efficiency.

#### Mixed-precision training support {#sec-ml-frameworks-mixedprecision-training-support-d31d}

Mixed precision\index{Mixed-precision training!FP16 vs. FP32 trade-offs} exploits hardware specialization to improve two iron law terms simultaneously: Tensor Cores\index{Tensor Core!mixed-precision acceleration} execute 16-bit matrix multiplications at higher throughput than FP32 arithmetic units (increasing $R_{\text{peak}}$ and reducing $O/(R_{\text{peak}} \cdot \eta_{\text{hw}})$), while 16-bit activations halve global memory traffic (reducing $D_{\text{vol}}$). Improving both terms simultaneously is rare; most systems optimizations improve compute efficiency at the expense of memory footprint, or vice versa.

Automatic mixed-precision APIs dynamically assign numerical precisions based on operator sensitivity and hardware capabilities. Compute-intensive operations such as convolutions and linear projections execute in FP16 or BF16, while numerically sensitive operations such as softmax, reductions, and loss calculations execute in FP32. Because FP16 has a narrow dynamic range (5 exponent bits, with normal values bounded below by $\approx 6.10 \times 10^{-5}$), gradient values can underflow to zero during backpropagation. Loss scaling\index{Loss Scaling!gradient underflow prevention} counters underflow by multiplying the scalar loss by a scale factor $S$ prior to backward traversal, shifting gradients into FP16 representable range, and dividing parameter gradients by $S$ before optimizer updates.

Frameworks support three primary reduced-precision formats: IEEE FP16, BF16,[^fn-bf16-dynamic-range] and TF32 (an NVIDIA Ampere+ internal format that pairs FP32's 8-bit exponent with FP16's 10-bit mantissa for matrix multiplies). BF16 matches FP32's 8-bit dynamic range at the expense of mantissa precision (7 bits), preventing gradient underflow and eliminating the need for dynamic loss scaling in most workloads. @Sec-model-training-mixedprecision-training-9218 details the numerical stability and memory characteristics of these formats. @Lst-autocast-usage demonstrates PyTorch's mixed-precision API, pairing the `autocast` context manager with `GradScaler` to prevent underflow.
...
BF16 training avoids this loss-scaling machinery entirely. As @lst-bf16-training shows in comparison to @lst-autocast-usage, removing `GradScaler` simplifies the training loop: BF16's dynamic range eliminates underflow risk, allowing direct backward execution.
...
class Model7B:
    ...
    model_7b_adam_gb_str = fmt_memory(adam, unit=GB, precision=0, commas=False)
    model_7b_total_gb_str = fmt_memory(total, unit=GB, precision=0, commas=False)

# Note: Use Model7B.model_7b_total_gb_str directly.
```

Resuming training after preemption or hardware failure requires restoring model weights and optimizer states in lockstep: first-moment vectors, second-moment variance estimates, and step counters. For Adam, storing two FP32 states per parameter adds $4\times$ the memory of FP16 model weights, making the combined footprint $5\times$ the weight baseline. For a 7-billion-parameter model, weights and optimizer states require approximately `{python} Model7B.model_7b_total_gb_str` total (`{python} Model7B.model_7b_fp16_gb_str` weights plus `{python} Model7B.model_7b_adam_gb_str` optimizer states). Checkpoint volume directly governs recovery latency across distributed storage, connecting fault tolerance to the iron law's $D_{\text{vol}}$ term.

@Sec-model-training analyzes optimizer memory optimization strategies in distributed training. At the framework level, optimizers expose internal state dictionaries via `state_dict()` for serialization. @Lst-state-dict-interface demonstrates accessing optimizer state dictionaries, and @lst-checkpoint-save-load shows saving and restoring both model parameters and optimizer states to preserve training trajectory continuity.
...
The mathematical foundations of automatic differentiation predated the deep learning resurgence by decades; the engineering challenge was mechanizing those principles into high-performance software infrastructure. Manually deriving and writing backward kernels for a single layer requires tracking intermediate variables, implementing local chain rules, and managing device buffer lifecycles. For modern transformer architectures containing multi-head self-attention, layer normalizations, and residual projections across hundreds of layers, manual differentiation across thousands of tensor dependencies is infeasible. Autograd engines mechanize this derivation across arbitrary topologies, precision modes, and accelerator backends, transforming an intractable analytical problem into automated graph execution.

#### Memory management in gradient computation {#sec-ml-frameworks-memory-management-gradient-computation-bb77}

The memory management strategies examined in @sec-ml-frameworks-memory-management-strategies-b008 (checkpointing, gradient accumulation) exist because reverse-mode AD requires preserving computational history. As @lst-reverse_memory demonstrated, each forward layer adds an activation tensor that must persist until backward traversal consumes it. This creates a memory wave that peaks at the onset of backpropagation and recedes as gradients accumulate and intermediate buffers are released. Modern runtimes track tensor reference counts to free activations the moment their backward consumers finish executing.

Beyond tensor lifetimes, frameworks face a lower-level systems bottleneck: driver-level allocation latency. Invoking synchronous device memory allocations (such as `cudaMalloc`) incurs substantial OS and driver overhead, forcing host-device synchronizations that drain accelerator command queues. To eliminate this latency, modern frameworks intercept allocations with **caching allocators**\index{Memory management!caching allocators} that maintain pools of pre-reserved device memory blocks across training iterations. Suballocating from these pools amortizes device allocation costs and reduces kernel launch latency, though varying tensor lifetimes can still induce memory fragmentation.
...
2.  Fragmentation: Size classes and block splitting improve reuse but can leave free capacity in blocks that do not satisfy a request. Because tensors require contiguous physical addresses, external fragmentation occurs when variable tensor lifetimes leave small, non-contiguous free blocks in the pool; a large allocation can trigger an out-of-memory (OOM) error even when the total aggregate free memory across all blocks exceeds the requested size.
...
The AD system tracks dependencies so it can traverse independent graph branches in a valid order and accumulate gradients correctly. While independent host threads or explicit CUDA streams can expose device concurrency, default autograd dispatch serializes node execution across the primary compute stream unless explicitly directed otherwise. Concurrency is bounded by stream dependencies and hardware scheduling limits.

#### How different frameworks implement AD {#sec-ml-frameworks-different-frameworks-implement-ad-2f8e}

The execution models analyzed in @sec-ml-frameworks-execution-problem-e1e1—eager, static graph, and hybrid—directly dictate how frameworks implement automatic differentiation. PyTorch\index{PyTorch!autograd implementation} [@paszke2019pytorch] constructs its autograd tape dynamically during forward execution. Its `grad_fn` chain mechanism, examined in @sec-ml-frameworks-pytorch-autograd-internals-4fa0, grants unconstrained Python control flow and transparent breakpoint debugging, but forces the engine to retain tape metadata and saved tensors until backward traversal completes. Conversely, early TensorFlow\index{TensorFlow!symbolic differentiation} [@abadi2016tensorflow] performed symbolic differentiation ahead of time during graph construction, generating an immutable backward sub-graph amenable to whole-graph dead-code elimination and memory reuse before execution. Modern TensorFlow 2.x adopts eager execution as its default interface while exposing `tf.function` to compile static differentiation graphs when throughput requires it [@tensorflowFunctionGuide]. JAX\index{JAX!transform-based AD}[^fn-jax-functional-transforms] [@frostig2018compiling] rejects tape tracking entirely, treating differentiation as a pure function transformation: `jax.grad()` accepts a function and produces a derivative function, enabling seamless algebraic composition with vectorization (`jax.vmap()`) and XLA compilation (`jax.jit()`). While this functional paradigm demands strict referential transparency, it exposes clean whole-program IR to the compiler without the stateful side effects of imperative tapes.
...
from mlsysim.fmt import fmt, check, fmt_math, fmt_memory
...
class AdminTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    admin_weights_gb_str = fmt_memory(weights, unit=GB, precision=0, commas=False)
    admin_grads_gb_str = fmt_memory(grads, unit=GB, precision=0, commas=False)
    admin_opt_gb_str = fmt_memory(opt, unit=GB, precision=0, commas=False)
    admin_act_gb_str = fmt_memory(act_bytes, unit=GB, precision=1, commas=False)
    admin_tax_gb_str = fmt_memory(tax, unit=GB, precision=1, commas=False)
...
from mlsysim.fmt import fmt, check, fmt_memory, fmt_multiple, fmt_time, fmt_bandwidth, fmt_flop_rate
...
class DeviceBandwidthHierarchy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    pcie3_gb_per_s_str = fmt_bandwidth(pcie3, unit=GB / second, precision=1, commas=False)
    pcie3_4mb_ms_str = fmt_time(pcie3_4mb, millisecond, precision=3, commas=False)
    pcie4_gb_per_s_str = fmt_bandwidth(pcie4, unit=GB / second, precision=0, commas=False)
    pcie4_bidir_gb_per_s_str = fmt_bandwidth(pcie4 * 2, unit=GB / second, precision=0, commas=False)
    nvlink_a100_gb_per_s_str = fmt_bandwidth(nvlink_a100, unit=GB / second, precision=0, commas=False)
    nvlink_a100_per_direction_gb_per_s_str = fmt_bandwidth(nvlink_a100_per_direction, unit=GB / second, precision=0, commas=False)
    a100_bw_gb_per_s_str = fmt_bandwidth(a100_bw, unit=GB / second, precision=0, commas=False)
...
The movement mechanism is explicit. Calling `.to()` on a tensor returns the original tensor when its data type and device already match the request; otherwise it returns a converted copy unless `copy=True` forces a copy. In contrast, `nn.Module.to()` modifies registered parameters and buffers in place and returns the module.

The performance discipline follows directly from this bandwidth gap: allocate tensors on the target device from the start rather than creating them on the host and transferring them, reuse GPU memory across iterations rather than reallocating it, and colocate inputs, labels, and model parameters on the same device to eliminate implicit transfers. At `{python} DeviceBandwidthHierarchy.pcie4_gb_per_s_str`, violating any of these principles inserts PCIe transfers into the critical path that can dominate a training iteration that otherwise runs at `{python} DeviceBandwidthHierarchy.a100_bw_tb_per_s_str` on-device.
...
The performance difference between these approaches is categorical. Full synchronization after every operation converts a concurrent pipeline into a sequential one, entirely negating the hardware parallelism that streams expose. Event-based synchronization preserves the concurrent execution model while enforcing only the dependencies that correctness requires.

This synchronization discipline has one operational trap: debug code often leaves `torch.cuda.synchronize()` in the hot path, turning an overlapped pipeline into a serialized one. When overlap remains poor, profiling must separate scheduling stalls from kernel inefficiency. NVIDIA Nsight Systems\index{Nsight Systems!profiling tool} (`nsys profile`) shows CPU activity, GPU kernels, and memory transfers on one timeline. NVIDIA Nsight Compute\index{Nsight Compute!profiling tool} (`ncu`) then explains kernel behavior with hardware counters.
...
from mlsysim.fmt import fmt, check, fmt_percent, fmt_rate, fmt_time, fmt_bandwidth, fmt_memory
...
When training scales beyond a single machine, these same abstractions extend to named groups of devices. Frameworks use constructs such as `ProcessGroup`\index{ProcessGroup!distributed execution} (PyTorch) or `Mesh` (JAX) to describe which tensors and operations belong together, relying on communication libraries such as NCCL to execute collective primitives—such as **ring-AllReduce**\index{AllReduce!collective communication} for gradient aggregation and **AllGather**\index{AllGather!collective communication} for parameter gathering. The runtime manages these primitives to maximize interconnect utilization: intra-node NVLink links deliver up to $900\text{ GB/s}$ of bidirectional bandwidth per GPU, whereas inter-node InfiniBand or Ethernet networks drop to $50\text{--}400\text{ Gbps}$ ($6.25\text{--}50\text{ GB/s}$), making collective communication efficiency the primary bottleneck in distributed training scaling. The important framework idea is the abstraction boundary: user code names placement relationships, and the framework preserves those relationships while the hardware path changes underneath.
...
When an engineer writes `y = torch.matmul(x, w)`, the distance between Python syntax and silicon execution is substantial. Bridging a single high-level call to thousands of parallel GPU threads requires three tiers of operations\index{Operator kernel!dispatch hierarchy} working in coordination. @Fig-mlfm-core-ops illustrates this stack: hardware abstraction operations manage platform-specific kernel dispatch and memory transfers, basic numerical operations execute mathematical routines, and system-level operations coordinate scheduling, memory lifetimes, and accelerator resources across the computation graph. The discussion that follows examines this build-up from the hardware-specific kernel path toward system orchestration.
...
Tensors, streams, and kernel dispatch operate at the granularity of individual buffers and operations. While these primitives secure high execution efficiency on hardware, scaling model development requires an organizational abstraction: composing thousands of low-level operations into reusable, nestable building blocks called *modules*.

## nn.Module Abstraction {#sec-ml-frameworks-nnmodule-abstraction-2622}

Hardware-facing mechanisms—tensors, kernels, streams, and memory managers—ensure individual operations execute efficiently on silicon. Yet a network such as ResNet-50\index{ResNet-50!module hierarchy}, containing `{python} ResNetModuleParams.resnet_params_m_str` parameters across fifty layers, requires extensive coordination: registering weights with optimizers, tracking device placement, toggling training and evaluation modes, and serializing checkpoints. Managing this manually would drown model development in error-prone tensor plumbing. The organizational layer of the framework resolves this through hierarchical composition\index{Module abstraction!hierarchical composition}.
...
PyTorch's approach fits exploratory research for the same reason: it treats deep learning as standard Python programming. Developers can use Python loops, conditionals, and debuggers (such as `pdb`) directly within a model's forward pass, with no special syntax, no separate compilation step, and no waiting to see if the code works. Eager execution enables rapid iteration and intuitive model design, which is essential when architectures and training objectives are still changing.
...
JAX's power emerges from composition. `jax.grad`\index{JAX!gradient transformation} returns a gradient function; `jax.vmap`\index{JAX!automatic vectorization}\index{Vectorization!automatic batching} can vectorize a compatible function over mapped axes; and `jax.jit`\index{JAX!JIT compilation} can trace and compile a stable transformed function for a particular argument signature. Unlike an iterative Python loop that issues individual operations sequentially with repeated dispatch overhead, `jax.vmap` pushes the batch dimension directly into the underlying primitives—converting per-sample matrix-vector operations into batched matrix-matrix multiplications (GEMM) executed in a single parallel kernel launch. `jax.pmap`\index{JAX!device-parallel mapping} maps a function across devices, but synchronization or gradient aggregation must be expressed with collectives such as `lax.psum` or `lax.pmean`. These transformations compose according to their types and semantics, and different orders can compute different quantities.
...
Across the cloud-to-edge spectrum\index{Deployment target!cloud to edge spectrum}, framework selection\index{Edge deployment!framework selection} reweights the three core problems according to hardware boundaries\index{Edge deployment!three problems reweighting}. While the performance gaps measured in @sec-ml-frameworks-quantitative-platform-performance-analysis-816d determine training throughput and operational cost in a data center, deploying to edge environments turns runtime efficiency into a binary feasibility test. The execution problem shifts from trading debuggability against graph optimization to fitting computation strictly within latency and memory envelopes. The differentiation problem disappears for pure inference workloads, eliminating the storage overhead of activation stashes and backward graphs. Meanwhile, the abstraction problem intensifies as software targets heterogeneous compute engines: server x86 and ARM CPUs, mobile NPUs, edge TPUs, and bare-metal microcontrollers operating within tens of kilobytes of SRAM.

@Tbl-deployment-frameworks maps these runtime assumptions across the cloud-to-edge spectrum, detailing the optimization levers and binding physical constraints that define each hardware tier.
...
These boundaries demonstrate why target selection acts as a hard filter on framework architecture rather than a late packaging detail. For example, the Smart Doorbell KWS model in @sec-ml-frameworks-tinyml-micro-runtimes-2a1b exemplifies the microcontroller tier. Operating within kilobytes of memory requires a runtime with a fixed, preallocated arena and a compact C/C++ footprint, eliminating dynamic memory allocation and Python runtime overhead entirely. This physical restriction forces a practical separation: engineering teams train models in expressive, dynamic environments on cloud GPUs, but must deploy them into runtimes with drastically narrower execution assumptions.

The **ONNX**\index{ONNX!definition}\index{ONNX!cross-framework portability}[^fn-onnx-format-portability] format addresses this fragmentation by standardizing model exchange across runtimes [@bai2019onnx]. A pipeline can train an architecture in PyTorch, export the computation graph to an ONNX intermediate representation, and execute the serialized graph in ONNX Runtime or compile it for a target accelerator. In contrast, TensorFlow Lite relies on its own conversion path, translating TensorFlow SavedModels into FlatBuffers rather than routing through ONNX. Although standard interchange formats reduce manual model conversion between development and production environments, they do not eliminate compatibility testing, operator coverage gaps, or custom-kernel fallback overhead. The model compression and serving strategies analyzed in @sec-model-compression and @sec-model-serving build directly on this export boundary.

@Fig-onnx illustrates this hub-and-spoke interoperability\index{ONNX!hub-and-spoke interoperability} topology. Training frameworks export computation graphs into a unified intermediate representation, while downstream runtimes and hardware backends ingest that representation to execute across heterogeneous targets.
...
Interchange formats mitigate framework fragmentation, but an export boundary cannot salvage an architecture that violates the target's physical limits. Converting a graph to ONNX or an embedded runtime after development completes cannot resolve an excessive memory footprint, missing hardware operators, or unsupported dynamic control flow. Treating deployment as an afterthought guarantees costly rewrites, making systematic evaluation of hardware and runtime trade-offs essential before writing the first line of model code.

## Framework Selection {#sec-ml-frameworks-selecting-framework-2949}

Framework selection\index{Framework selection!trade-off analysis}\index{Framework selection!constrained optimization} is a constrained optimization problem across the execution, differentiation, and hardware abstraction dimensions established in this chapter. The question is not which framework is "best"; it is which execution model, differentiation system, and abstraction path survive the project's constraints.
...
This velocity-performance tension leads directly to the second: flexibility versus optimization depth. Eager execution makes host-language control flow natural but limits compiler scope until a larger region is captured. Static graphs can represent data-dependent control flow through dedicated graph operators, exposing larger subgraphs for inter-operator kernel fusion that eliminates intermediate round-trips to HBM and keeps intermediate activations in on-chip SRAM. As @tbl-mlfm-graphs demonstrated, this trade-off cascades through memory management, hardware utilization, and debugging workflows. It is an architectural constraint that governs the entire execution pipeline.

The flexibility-optimization tension, in turn, exposes a third: ecosystem breadth vs. specialization. General-purpose frameworks cover broad operator catalogs but often underperform specialized runtimes on workloads those runtimes can optimize deeply. Systems such as TensorRT and TVM optimize for narrower deployment targets through aggressive kernel fusion, precision selection, and hardware-specific memory layout scheduling [@nvidia2024tensorrt; @chen2018tvm]. While interchange formats such as ONNX bridge part of this gap [@bai2019onnx], runtime specialization introduces its own failure modes: when an exported model encounters an unsupported operator, execution must fall back to unoptimized kernels or trigger expensive host-device synchronizations.
...
The TensorFlow ecosystem illustrates how these axes interact concretely. Its three variants (TensorFlow\index{TensorFlow!full framework}, TensorFlow Lite\index{TensorFlow Lite!mobile deployment}, and TensorFlow Lite Micro\index{TensorFlow Lite Micro!microcontroller deployment}) trace a single design philosophy across progressively tighter physical constraints. @Tbl-tf-comparison traces the resulting trade-offs in model expressiveness.
...
This progression demonstrates that runtime efficiency is purchased by shedding generality: fewer supported operations enable smaller binaries, deterministic memory layouts, and aggressive lower-precision optimization.

### Framework selection criteria {#sec-ml-frameworks-model-requirements-2e01}

Three dimensions structure systematic framework evaluation: what the model requires (supported operations and graph semantics), what the software environment provides (operating system services, memory allocators, and accelerator delegation), and what the hardware physically permits (compute throughput, memory capacity, and thermal power limits). Each dimension acts as a filter: hard constraints eliminate non-viable candidates, while soft preferences rank the survivors.

#### Model requirements {#sec-ml-frameworks-model-requirements-ca8b}

The primary constraint is whether a framework can express the operations and execution semantics that a model requires. As shown in @tbl-tf-comparison, operator coverage narrows from the broad catalog of general-purpose TensorFlow to the inference-focused set in TensorFlow Lite, and finally to the minimal model-selected subset linked into TensorFlow Lite Micro. Each reduction strips dynamic operators and training machinery, concentrating compiler resources on static kernels. The underlying systems principle is that algorithmic expressiveness trades directly against machine efficiency: restricting the supported operator set allows compilers to prune dead code, generate tight register-resident inner loops, and eliminate dynamic dispatch overhead. Beyond operator coverage, model requirements also dictate graph semantics: whether computation must execute dynamically or can be captured ahead of time.
...
Once model expressiveness is confirmed, the framework must integrate with the target software environment. @Tbl-tf-sw-comparison outlines how operating system dependencies, model loading mechanisms, and accelerator delegation vary across deployment tiers.
...
These distinctions illustrate how eliminating software abstractions expands deployment reach. Standard TensorFlow relies on full POSIX operating system services, dynamic shared libraries, and virtual memory allocators. TensorFlow Lite relaxes these requirements for mobile platforms, accessing models via memory-mapped files and delegating operations to hardware accelerators through vendor runtime APIs. TensorFlow Lite Micro removes the operating system dependency entirely, enabling bare-metal execution on microcontrollers without a POSIX filesystem or dynamic heap allocator (though it can run alongside an RTOS such as FreeRTOS or Zephyr). The model FlatBuffer is compiled directly into a constant byte array residing in read-only flash memory, and hardware acceleration relies on statically linked CMSIS-NN or DSP assembly kernels rather than dynamic driver delegates. Every software dependency stripped away unlocks a more constrained tier of physical hardware.

#### Hardware constraints {#sec-ml-frameworks-hardware-constraints-e449}

Software compatibility alone does not guarantee execution; the framework runtime must fit within the physical limits of memory capacity, storage, and compute bandwidth. As summarized in @tbl-tf-hw-comparison, target processor architectures shift across deployment tiers from server-class x86 CPUs, GPUs, and TPUs in data centers through Arm Cortex-A application processors at the mobile edge, down to resource-constrained Arm Cortex-M microcontrollers and DSPs.
...
These hardware boundaries reflect the fundamental physical constraints—the light barrier, the power wall, and the memory wall—that divide the deployment spectrum into distinct operational regimes (@sec-ml-systems-deployment-spectrum-71be). In a data center, multi-megabyte runtime binaries and dynamic memory management overhead are negligible compared to gigabytes of high-bandwidth device memory. On a microcontroller with 256 KB of SRAM and 1 MB of flash, a general-purpose runtime cannot even be loaded. The framework must replace dynamic heap allocators with a single pre-allocated memory arena sized ahead of time to hold tensor activations and scratchpad buffers.

#### Production-ready evaluation factors {#sec-ml-frameworks-productionready-evaluation-factors-e088}

Satisfying model requirements, software dependencies, and hardware constraints establishes technical feasibility, but production viability requires evaluating operational reliability under sustained serving workloads. Runtimes that rely on dynamic memory allocation or host-language interpreters introduce tail-latency (p99) variance through garbage collection pauses and heap fragmentation. In contrast, production-oriented runtimes enforce static tensor arenas and ahead-of-time graph lowering to ensure deterministic execution times.

Precision optimization forms a central operational lever. Lowering numerical precision from FP32 to FP8 or INT8 halves memory bandwidth consumption and increases arithmetic throughput on accelerator tensor cores, but it requires native quantization tooling. A production framework must support both post-training quantization (PTQ) with representative calibration datasets and quantization-aware training (QAT) to recover accuracy margins before deployment. Beyond per-device execution, production viability depends on toolchain stability: whether model checkpoints maintain backward compatibility across runtime updates, whether the framework exposes telemetry hooks for profiling kernel execution, and whether serving infrastructure can deploy updated graph definitions without downtime.
...
Framework viability over a multi-year production lifecycle depends on whether the surrounding ecosystem keeps the chosen execution, differentiation, and abstraction paths maintainable. Community composition directly shapes engineering priorities: research-focused ecosystems prioritize experimentation velocity, interactive debugging, and rapid model prototyping; production-focused ecosystems invest in serving runtimes, backward compatibility, and hardware vendor integrations; and compiler-centric ecosystems advance mathematical transformations and code generation for emerging accelerators.

A framework's operational cost often depends more on surrounding ecosystem infrastructure than on the core tensor API. Integration with model registries, serving systems, cluster orchestrators, and accelerator profiling tools accelerates deployment, but each layer introduces an upstream dependency. Custom C++ or CUDA kernel implementations, specialized serialization formats, and vendor-specific compilation passes make framework migration prohibitively expensive once an organization standardizes its pipeline. Preserving an architectural exit path requires deliberate decoupling: standardizing intermediate representations through open formats such as ONNX, isolating data ingestion pipelines from framework-specific tensor classes, and maintaining modular operator boundaries.

The preceding sections evaluated execution, differentiation, and hardware abstraction as distinct architectural choices. In an executing model, however, these three systems operate simultaneously. A single training step is where they collide: eager dispatch or graph scheduling determines when work reaches silicon, reverse-mode automatic differentiation generates the backward pass while retaining intermediate activations, and hardware abstractions coordinate tensor layouts and kernel launches. Tracing one complete training step from input batch to parameter update reveals how these mechanisms integrate under physical machine constraints.

## Anatomy of a Training Step {#sec-ml-frameworks-putting-together-anatomy-training-step-c7f1}

Execution, differentiation, and hardware abstraction operate simultaneously during model training\index{Training!step, anatomy}. In @lst-training-step-anatomy, a minimal two-layer multilayer perceptron illustrates this convergence across eight executable statements. Tracing each statement through the runtime stack connects the quantitative principles developed in @sec-ml-frameworks-three-problems-every-framework-must-solve-317d to physical execution: tensor allocation, kernel dispatch, dynamic graph recording, gradient computation, and parameter updates.
...
1. Python dispatch\index{Python dispatch!overhead}: The Python interpreter invokes `torch.matmul`, which routes through PyTorch's C++ dispatcher to select the CUDA backend, incurring several microseconds of host CPU overhead before issuing work to the device.
...
4. GPU execution: The streaming multiprocessors stream `W1` from HBM[^fn-hbm-capacity-constraint] through L2 cache into registers and Tensor Cores, evaluate the matrix product, and write the output activations back to HBM.
5. Autograd recording: Concurrently on the host, PyTorch's autograd engine instantiates an `MmBackward` node on the dynamic tape, saving references to input tensors `x` and `W1` in device memory for reverse-mode differentiation.
...
The subsequent bias addition (`+ b1`) and activation (`torch.relu`) follow an identical eager path, with each operation dispatching a separate kernel, executing round-trips to HBM, and appending a node to the tape.
...
2. Gradient computation: The engine executes the backward kernel registered to each node. Traversing backward, `CrossEntropyBackward` evaluates $\frac{\partial \mathcal{L}}{\partial \text{logits}}$ using the softmax derivative; `MmBackward` for $W_2$ computes $\frac{\partial \mathcal{L}}{\partial W_2} = h^T \cdot \frac{\partial \mathcal{L}}{\partial \text{logits}}$ and $\frac{\partial \mathcal{L}}{\partial h}$; `ReluBackward` evaluates the activation mask, zeroing gradients where forward activations satisfy $h \leq 0$; and `MmBackward` for $W_1$ computes $\frac{\partial \mathcal{L}}{\partial W_1} = x^T \cdot \frac{\partial \mathcal{L}}{\partial z_1}$. Because the input batch `x` does not set `requires_grad=True`, traversal terminates without evaluating $\frac{\partial \mathcal{L}}{\partial x}$.
3. Gradient accumulation: Computed parameter gradients accumulate in-place into the `.grad` fields of leaf tensors ($W_1$, $b_1$, $W_2$, $b_2$).
4. Memory deallocation: As each backward node completes, the framework releases its saved forward activations (such as $h$ and $x$), allowing the caching allocator to reuse those memory regions for subsequent operations.
...
from mlsysim.fmt import check, fmt_arithmetic_intensity, fmt_flops, fmt_math, fmt_memory
...
class TrainingStepCalc:
    ...
    train_mm1_mem_mb_str = fmt_memory(mm1_mem, unit=MB, precision=1, commas=False)
    ...
    train_bwd_mem_mb_str = fmt_memory(bwd_mem, unit=MB, precision=1, commas=False)
...
The arithmetic-intensity column reveals the operational regime of each layer. Matrix multiplications exhibit operand reuse that drives intensity toward the compute roofline. Conversely, element-wise ReLU and reduction in cross-entropy perform negligible compute per byte transferred ($0.125$ to $0.375\text{ FLOP/byte}$), remaining strictly bound by memory bandwidth and launch latency. In un-fused eager execution, each element-wise operation incurs a separate kernel dispatch and round-trips intermediate tensors through HBM without saturating compute units.
...
from mlsysim.fmt import fmt, check, fmt_count, fmt_flops, fmt_time, fmt_bandwidth, fmt_flop_rate, fmt_memory
...
class MnistTrainingStepCalc:
    ...
    a100_peak_tflop_per_s_str = fmt_flop_rate(peak_flops, unit=TFLOP / second, precision=1, commas=False)
    a100_mem_bw_tb_per_s_str = fmt_bandwidth(mem_bw, unit=TB / second, precision=2, commas=False)
```

The analytical model yields ~`{python} MnistTrainingStepCalc.mnist_total_mflop_str` of total arithmetic work and ~`{python} MnistTrainingStepCalc.mnist_mem_traffic_mb_str` of memory traffic. Dividing these quantities by peak A100 hardware rates determines the idealized lower bounds:
...
Because $T_{\text{overhead}}$ exceeds $T_{\text{compute}}$ and $T_{\text{memory}}$ by more than an order of magnitude, the step is overhead-bound\index{Overhead-bound!small model training}. Accelerator execution units sit starved while the host CPU serializes dispatch. System engineering in this regime targets the launch tax rather than arithmetic efficiency:

- Kernel fusion (`torch.compile`): Fusing consecutive memory-bound operations into a single kernel avoids intermediate round-trips to HBM and collapses launch boundaries.
- Batch size scaling: Increasing the batch size scales the arithmetic payload linearly while keeping dispatch overhead constant, amortizing fixed launch latency across more compute operations.
- Graph capture: Static compilation or CUDA Graphs capture the dependency DAG once, replaying execution through a single driver invocation to eliminate host interpreter overhead entirely.

### Phase 4: Hardware abstraction (solving the abstraction problem) {.unnumbered}

The identical Python script executes across heterogeneous hardware architectures through backend abstraction layers, as @tbl-framework-backends summarizes.

Each backend maps common tensor semantics to silicon-specific primitives. A single `loss.backward()` invocation routes to different execution pipelines depending on device placement. Differences in floating-point summation order, reduction trees, and non-deterministic atomic operations introduce numerical variation across backends, requiring equivalence to be validated against explicit numerical tolerances rather than bitwise identity.
...
Implementation decisions—device placement, batch size, compilation mode—cascade through execution, differentiation, and abstraction layers simultaneously. When systems treat the framework as an opaque mathematical interpreter rather than a hardware-aware scheduler, these interactions trigger launch starvation, memory fragmentation, and throughput collapse.
...
- Execution (Machine scheduling): Eager mode preserves imperative line-by-line debugging at the cost of per-kernel launch and dispatch latency.
- Differentiation (Algorithmic graph): Reverse-mode automatic differentiation retains intermediate forward activations in HBM to compute exact vector-Jacobian products in reverse topological order.
- Abstraction (Hardware portability): Unified tensor APIs decouple model code from heterogeneous silicon, dispatching identical operations to cuBLAS, oneMKL, or XLA HLO kernels.
...
Machine learning frameworks solve three core systems challenges that make deep learning tractable on modern hardware. The first is execution: deciding when and how operations run. Frameworks balance the trade-off between eager evaluation for interactive debugging and graph execution for whole-program compilation, while modern hybrid approaches such as `torch.compile` capture graphs dynamically without sacrificing the Python control loop. The second is differentiation: computing gradients automatically. Frameworks implement reverse-mode automatic differentiation that applies the chain rule across compositions of operations, constructing an autograd tape or backward graph of vector-Jacobian products while retaining intermediate activations in memory. This primitive makes training large models feasible through a single `loss.backward()` call. The third is abstraction: targeting heterogeneous hardware from a unified interface. Frameworks provide multi-dimensional tensor abstractions, intermediate representations, and runtime dispatch systems that bridge high-level model definitions to optimized device kernels across CPUs, GPUs, TPUs, and custom accelerators.

These challenges are linked through the iron law of ML systems (@sec-introduction-iron-law-ml-systems-c32a): execution strategy governs dispatch latency $(L_{\text{lat}})$, differentiation governs memory traffic $(D_{\text{vol}})$, and abstraction governs hardware utilization efficiency $(\eta_{\text{hw}})$. Because accelerator compute throughput has grown much faster than memory bandwidth, execution is frequently memory-bandwidth bound ($D_{\text{vol}}/\text{BW}$) rather than compute bound. Frameworks mitigate this memory wall through compiler optimizations: kernel fusion merges adjacent element-wise operations to keep intermediates in registers or on-chip SRAM instead of round-tripping through global memory, while selective recomputation trades redundant floating-point operations $(O)$ to relieve peak activation memory pressure.

::: {.callout-takeaways title="The layer between math and hardware"}

* **Every framework solves three problems**: Execution dictates runtime dispatch, differentiation computes gradients, and abstraction targets heterogeneous hardware. TensorFlow, PyTorch, and JAX make distinct infrastructure wagers across these axes—from static graph capture to dynamic tracing and functional transformations—establishing architectural commitments that outlast API syntax.
* **The memory wall drives optimization**: Accelerator compute capacity has outpaced memory bandwidth for decades, making off-chip data movement the primary execution bottleneck. Kernel fusion, selective recomputation, and lower-precision data layouts directly target the memory traffic term ($D_{\text{vol}}/\text{BW}$) in the iron law of ML systems rather than reducing raw operation counts.
* **Compilation must amortize setup cost**: The compilation continuum principle in @eq-compilation-benefit quantifies when runtime speedup offsets compilation overhead. While dynamic workloads favor eager execution, repeatedly executed graphs benefit from JIT or ahead-of-time compilation, where the dispatch overhead law (@eq-dispatch-overhead) demonstrates that small operations gain disproportionately once host launch overhead ($L_{\text{lat}}$) is removed.
* **Module abstractions automate state management**: Automatic parameter registration, mode-dependent execution (training versus inference), and hierarchical state serialization enable multi-billion-parameter model updates in a single optimizer step across diverse hardware backends.
* **Deployment constraints should drive selection**: Specialized runtimes trade operator coverage, static memory allocation, and compilation latency against stringent hardware budgets. The gap between server-class accelerator memory and microcontroller SRAM requires evaluating the downstream deployment target before committing to a development framework.

:::

A framework presents itself as an ergonomic programming interface, yet its true role is translating mathematical graphs into physical hardware operations. That translation never eliminates hardware costs; it only shifts expenditure among the terms of the iron law of ML systems (principle \ref{pri-iron-law}). Eager execution prioritizes developer iteration at the cost of host dispatch latency ($L_{\text{lat}}$), graph compilation trades upfront optimization time to eliminate global memory traffic ($D_{\text{vol}}/\text{BW}$) through fused kernels, and automatic differentiation trades activation memory ($D_{\text{vol}}$) against recomputed arithmetic ($O$). What appears to be an API convenience is therefore a standing systems commitment: the framework cannot reduce the fundamental physical burden the iron law enforces, but only determine which term carries the load.

::: {.callout-chapter-connection title="From control room to power plant"}

Frameworks are the software substrate that translates abstract architectures into executable kernels. Computational graphs, autograd tapes, and kernel dispatch pipelines are the control room instruments: they give engineers visibility into and control over the training process. The machinery is built, but every budget decision it exposes remains unmade: which activations to checkpoint when memory runs short, how large a batch the hardware allows, and at what point a single device stops being enough. @Sec-model-training is where those bills come due, scaling framework execution, differentiation, and memory-management machinery into the distributed systems that train models at scale.
