...
Starting from the softmax normalization introduced in @sec-neural-computation-nonlinear-activation-functions-38bc, we use a **temperature parameter**\index{Knowledge distillation!temperature parameter}[^fn-temperature-boltzmann-scaling] $T_{\text{distill}}$ to soften the probability distribution. The softmax output for class $i$ becomes:
$$
...
Binary and ternary networks represent the extreme end of quantization, where weights and activations are constrained to one-bit (binary) or a small ternary set. Their raw representations are compact, but realized storage and energy depend on packing and hardware support, and task quality becomes increasingly method dependent. Our keyword-spotting lighthouse (@sec-network-architectures-efficient-architectures-keyword-spotting-0625) illustrates a regime in which extreme compression may be needed to fit the `{python} ModelDeviceComparison.tiny_cap_kb_str` SRAM device budget. INT8 is one candidate starting point; INT4, binary, or ternary representations spend additional numerical capacity to pursue still tighter memory and power constraints.
...
As @tbl-fusion-speedup shows, fusion benefits vary by workload. Memory-bound operations benefit most, while compute-bound operations see minimal improvement.
...
The chapter's illustrative BERT mobile pipeline reduces a BERT-Base-sized footprint from `{python} BertCompression.full_mb_str` to `{python} BertCompression.compressed_mb_str`, a `{python} BertCompression.compression_ratio_mult_str` raw-footprint ratio, through staged structural change, distillation, and quantization. This is an accounting and sequencing scenario rather than one reported deployment. Each stage changes a different object: pruning changes structure, distillation trains a smaller model against teacher behavior, and quantization changes numerical representation. Their combined quality and runtime still require measurement.

::: {#exmp-model-compression-bert-base-mobile-deployment-pipeline .callout-example title="BERT-Base mobile deployment pipeline"}

**Scenario**: A modeled deployment starts from a 110M-parameter BERT-Base-sized language model for a resource-constrained mobile device; DistilBERT provides related evidence for training smaller transformer students [@sanh2019distilbert].

**Diagnosis**: Structural pruning, student training, and quantization interact through tensor shapes and learned distributions. Applying them independently or in a fixed assumed order can miss the final quality or runtime target.

**Systems lesson**: In this modeled pipeline, structural reduction, distillation, and QAT produce a `{python} BertCompression.compression_ratio_mult_str` footprint ratio (`{python} BertCompression.full_mb_str` to `{python} BertCompression.compressed_mb_str`) with `{python} BertCompression.final_accuracy_loss_pct_str` task-quality loss. A real deployment must measure each intermediate artifact because another ordering may work better for a different model or runtime.

:::

Sequencing matters because these levers interact through the same weights and activations. Pruning changes the surviving distribution and tensor shapes, so subsequent quantization may become easier or harder depending on ranges, outliers, formats, and kernels. Distillation can recover behavior in a chosen student, but it adds a separate objective and training run rather than guaranteeing recovery. @Fig-compression-methods reports one comparison from the cited compression study; it can motivate candidate sequences, but it does not establish a universal ordering among pruning, quantization, and singular value decomposition (SVD). Evaluate every intermediate artifact so a failure can be attributed to the transformation that introduced it.
...
Reducing a model's FLOPs by `{python} AmdahlCompression.flops_reduction_pct_str` does not guarantee `{python} AmdahlCompression.flops_reduction_pct_str` latency reduction. Memory-bound operations see little benefit from arithmetic reduction because data movement, not multiplication, sets their time. Critically, Amdahl's Law applies at the system level (@sec-appdx-machine-foundations-amdahls-law-gustafsons-law-b741 derives its strong-scaling form): if model inference accounts for only `{python} AmdahlCompression.model_fraction_pct_str` of end-to-end latency (with the remaining `{python} AmdahlCompression.non_model_pct_str` spent on data loading, preprocessing, and postprocessing), then even perfect model optimization yields at most `{python} AmdahlCompression.max_speedup_mult_str` overall speedup. Tokenization and network transfer can occupy the non-model fraction in language-model serving; image decoding, resizing, and normalization can do the same in vision pipelines. Profile the complete path on target hardware before estimating optimization benefits.
...
* **Hardware sets the exchange rate**: Unstructured sparsity and theoretical FLOP cuts do not imply latency gains unless kernels and memory layouts can exploit them. The chapter's hypothetical 8$\times$ paper target versus 1.5$\times$ realized outcome illustrates why target-hardware profiling is mandatory.
