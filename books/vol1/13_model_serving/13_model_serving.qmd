...
::: {#fig-tail-latency-explosion fig-env="figure" fig-pos="htb" fig-cap="**The Tail Latency Explosion**: Request latency vs. serving utilization $\rho_{\text{serv}}$. Mean latency (blue) and the p99 approximation (red) both grow nonlinearly as utilization approaches saturation; the 70 percent marker is an illustrative planning knee rather than a model-derived breakpoint. This uses the simple M/M/1 approximation introduced later in the Queuing Theory section (p99 ≈ 4.6$\times$ mean), so the curve is illustrative rather than workload-specific." fig-alt="Line plot of normalized request latency against serving utilization. A dashed blue mean-latency curve and a solid red p99 curve rise nonlinearly toward saturation, with the red curve remaining 4.6 times the blue curve. Green shading marks 0–50 percent as the Safe Zone, red shading marks 70–100 percent as the Danger Zone, and an arrow labels 70 percent ‘The Knee.’"}
...
from mlsysim.fmt import MarkdownStr, check, fmt, fmt_time, fmt_rate, fmt_memory, fmt_memory_capacity, fmt_energy
...
from mlsysim.fmt import fmt_int, fmt, fmt_count, check, fmt_percent, fmt_rate, fmt_power, fmt_energy, fmt_multiple
...
class CarbonCostCalc:
    ...
    cc_joules_token_j_str = fmt_energy(cc_joules_per_token, unit=joule, precision=4, commas=False, per='token')
    ...
    cc_low_util_joules_j_str = fmt_energy(cc_low_util_joules, unit=joule, precision=1, commas=False, per='token')
...
from mlsysim.fmt import fmt, fmt_time, fmt_memory, fmt_memory_capacity, fmt_bandwidth
...
::: {#fig-kv-cache-growth fig-env="figure" fig-pos="htb" fig-cap="**The KV-Cache Explosion**: FP16 KV-cache usage vs. context length for a 70-billion-parameter-class model with GQA. On the modeled A100, 35.3 GB of INT4 weights leave 50.6 GB for KV cache before runtime overhead. Batch size 32 reaches that cache budget at roughly 4.8k tokens, forcing a trade-off between batch size and context length." fig-alt="Line chart of FP16 KV-cache size against context length for batch sizes 1, 4, 16, and 32. All four lines rise linearly, with steeper slopes for larger batches. A red dashed horizontal line marks the cache budget remaining after the INT4 weights on the modeled A100; batch size 32 crosses it near 4.8k tokens. The region above the budget line is shaded red and labeled Out of Memory (OOM) Zone."}
...
ax.text(
    27000,
    limit_gb + 2,
    f"KV Budget after INT4 Weights ({limit_gb:.0f} GB)",
    color=COLORS['RedLine'],
    fontweight='bold',
    ha='center',
    fontsize=8,
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_usd, check, fmt, fmt_count, fmt_time, fmt_rate, fmt_memory, fmt_memory_capacity
from mlsysim.physics import transfer_time
