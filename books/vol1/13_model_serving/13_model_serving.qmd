...
The choice between static and dynamic serving has direct economic implications. A stricter latency requirement raises infrastructure costs when it demands smaller batches, additional replicas, or more capable hardware. Quantifying this trade-off in dollar terms reveals whether the latency reduction justifies that premium.

The two modes optimize for contrasting regimes. Static inference maximizes accelerator throughput and storage efficiency during offline batch runs. Dynamic inference minimizes per-request tail latency under stochastic, concurrent load, requiring fine-grained tracking of latency across the end-to-end execution path.

Deciding when predictions are computed sets the baseline execution model. Equally binding is where the model physically executes, because deployment context dictates hardware availability, network boundaries, and resource budgets.

Standard serving cost models assume a fixed forward pass: a static computation graph executing once per request. Emerging reasoning models upend that assumption by deliberately increasing computation per query, trading latency for answer quality, with substantial cost implications.
...
Whether a system spends one forward pass or many reasoning steps per query, physical deployment context dictates the feasible latency and cost envelope.
...
Rigorous isolation transforms an ad-hoc inference script into a predictable, deterministic service—a prerequisite for safety-critical deployments in robotics, autonomous driving, and real-time control. Beyond hardware boundaries and replica balancers, the internal software architecture of the inference server itself must coordinate irregular request streams into structured, hardware-saturating batches.
...
Protocol selection and process architecture define the static structure of a serving system. Evaluating real-world efficiency, however, requires tracing how latency accumulates as individual requests traverse the runtime path.
...
Understanding where latency accumulates requires instrumenting each stage independently. A ResNet-50 latency budget breakdown illustrates how every millisecond is spent when the classifier receives a JPEG image.
...
Tracing an isolated request reveals the intrinsic execution time of each pipeline phase. In production, however, multiple requests arrive concurrently and contend for shared accelerators, CPU cores, and memory buses—introducing queueing delays that dwarf raw computation time.

## Queuing Theory {#sec-model-serving-queuing-theory-tail-latency-29a6}

When arrival streams fluctuate, queueing theory\index{Queuing theory!serving systems} formalizes how contention degrades latency distributions. Its governing laws explain why seemingly well-provisioned inference services experience sharp, nonlinear latency explosions as utilization approaches capacity.
...
The M/M/1 model assumes exponentially distributed service times, whereas fixed-shape inference at a fixed batch size can have lower service-time variance. An M/D/1\index{M/D/1 queue!deterministic service} model can then approximate the mean queueing wait more closely. The queueing formulation here adopts M/M/1 because it yields closed-form solutions and preserves a conservative margin when its assumptions hold. Real services still require measured arrival and service-time distributions, especially when input shapes, batching, preprocessing, or cache state vary.[^fn-kendall-notation-serving]
...
A single serving node forms the fundamental building block of inference infrastructure. Single-node queueing dynamics govern resource saturation and establish the baseline performance characteristics that persist when horizontally replicated across a cluster.
...
When traffic exceeds single-node capacity, the architecture transitions to replica-level scale-out: multiple independent serving nodes sit behind a load balancer, each running an identical model instance. The M/M/c queueing model\index{M/M/c queue!multi-server} extends M/M/1 to $c$ parallel servers, demonstrating how horizontal replicas\index{Replica!tail latency improvement} absorb load when traffic is evenly balanced. Realized p99 improvements depend on arrival patterns, service-time variance, dispatch algorithms, and per-replica utilization headroom. This replica architecture remains conceptually distinct from model parallelism (tensor or pipeline sharding), which splits a single request across multiple accelerators and introduces interconnect synchronization overheads.
...
Once a model is loaded, warmed up, and producing predictions consistent with training, request grouping becomes the next optimization lever. Batching directly dictates both the throughput and latency terms in queueing formulations.
...
Autoregressive models\index{Autoregressive model!token generation} like language models generate outputs token by token: each new token depends on all previously generated tokens, so generation is inherently sequential. Standard dynamic batching assumes fixed-length outputs. LLMs violate this assumption: if one sequence in a batch of eight finishes after ten tokens while others need 100 tokens, 90 percent of the compute for that sequence slot is wasted\index{Sequence length variability!batch waste} [@yu2022orca].
...
While PagedAttention optimizes memory allocation, large-language-model serving introduces broader algorithmic challenges across generation scheduling and parameter representation.
...
Although continuous batching maximizes throughput for shared language model workloads, batching is not universally advantageous. Sophisticated batch schedulers and dynamic memory allocators introduce coordination complexity that can degrade performance in latency-critical or resource-constrained environments.
...
This case study demonstrates how physical constraints govern serving economics: latency budgets partition into prefill and decode phases, queueing theory dictates batch sizing and capacity headroom, and memory bandwidth bounds decode throughput. Misinterpreting these interactions leads directly to common operational traps.
