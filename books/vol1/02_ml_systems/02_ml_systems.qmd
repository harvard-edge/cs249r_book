...
Cloud deployments range from single workstations and multi-GPU servers to distributed clusters spanning warehouse data centers. Systems engineering at this scale begins on a single node: before coordinating computation across networked hosts, an engineer must first understand memory bus saturation, PCIe and NVLink bandwidth, and accelerator utilization within a single machine.
...
The physical necessity of edge processing emerges most clearly when sensor ingestion rates exceed network uplink capacity. Edge deployment is defined less by geographic location than by data volume: when incoming sensor throughput outpaces available network bandwidth, the resulting bottleneck forces computation to the local device regardless of other considerations.
...
These capabilities require substantial trade-offs. Computational constraints impose severe limits: microcontrollers commonly provide $10^5$ to $10^6$ bytes of RAM, forcing models and intermediate activations into the tens-of-kilobytes to low-megabytes range depending on the workload. Crucially, microcontrollers enforce a strict two-tier memory partition: static model weights and code reside in non-volatile Flash (often a few megabytes), while mutable activations and the runtime buffer (the *tensor arena*) must fit within the much smaller on-chip SRAM (often under 256 KB). As a result, peak intermediate activation volume—rather than parameter storage—is frequently the binding barrier that prevents a model from executing. Development complexity requires expertise spanning neural network optimization, hardware-level memory management, embedded toolchains, and specialized debugging across diverse microcontroller architectures.
...
In **hierarchical processing**\index{Hierarchical processing!data flow}, data and intelligence flow between computational tiers. TinyML sensors perform basic anomaly detection, edge devices aggregate and analyze data from multiple sensors, and cloud systems handle complex analytics and model updates. Each tier handles tasks appropriate to its capabilities. In inference serving, this hierarchy is typically implemented as a confidence cascade: a lightweight local model processes the input first, returning immediately if its prediction confidence exceeds a preset threshold $\tau$, and offloading to the heavier cloud model only when uncertainty or ambiguity demands deeper analysis.
