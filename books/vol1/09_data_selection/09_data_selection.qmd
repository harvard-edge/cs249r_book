...
Augmentation transforms existing samples; generative synthesis creates new examples from parametric models or physical simulators. This generative path becomes essential when real-world acquisition stalls across three primary regimes: privacy-restricted domains where patient records or proprietary telemetry cannot be shared directly, safety-critical edge cases such as collision trajectories that rarely appear in nominal driving logs, and physical domains such as robotics where physical rollouts incur prohibitive hardware wear. Yet synthetic samples introduce their own systems tax: parametric generation consumes substantial inference FLOPs, and imperfect simulators introduce distribution shifts that corrupt downstream policies.
...
Equivalently, isolating the allowable selection overhead yields $T_{\text{selection}} < T_{\text{train}}(D_{\text{total}}) - T_{\text{train}}(D_{\text{subset}})$. The upper bound on selection latency depends strictly on how much training execution the subset eliminates; no arbitrary overhead fraction applies across workloads.
...
Standard cost models implicitly assume a centralized data loader where a single controller evaluates global scores. Production training runs shard terabyte- and petabyte-scale corpora across hundreds of compute nodes, introducing network communication and coordination barriers that erode single-node efficiencies.
...
Systematic data selection—encompassing deduplication, coreset extraction, dynamic curricula, active acquisition, and controlled synthesis—transforms data from an unyielding supply constraint into a tunable engineering lever. When calibrated against target workloads, these methods compress labeling, iteration, storage, and accelerator costs without compromising the generalization boundaries demanded by production deployment.

The governing principle of data selection is that training examples provide vastly unequal marginal signal. Empirical benchmarks demonstrate that curated subsets can shed half their volume without compromising generalization, whereas unprincipled pruning discards the rare slices that determine production reliability. Selection is the rigorous discipline of isolating high-signal evidence from redundant or noisy records while safeguarding operational coverage. In the D·A·M architecture, this is data-algorithm co-design operating at the head of the system: every sample safely eliminated upstream is an operation that never burdens downstream hardware or model compression.
