...
Within the fitted Chinchilla regime, compute-optimal model parameters and training tokens grew at roughly equal rates.[^fn-chinchilla-token-ratio] Combining that empirical fit with the dense-transformer compute approximation yields $D_{\text{opt}} \propto \sqrt{C}$, so doubling compute corresponds to about $\sqrt{2} - 1$, or `{python} ChinchillaDiagnostic.doubling_token_growth_pct_str`, more tokens under those assumptions. The commonly quoted `{python} ChinchillaDiagnostic.tokens_per_param_opt_str` tokens per parameter is a useful reference point from that study, not a universal optimum.
