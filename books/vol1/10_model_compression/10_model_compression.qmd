...
Concrete systems keep those trade-offs measurable: ResNet-50 and MobileNetV2 (the lighthouse models from @sec-network-architectures-lighthouse-roster-model-biographies-a763) for vision workloads, transformer-based language models for sequence tasks, DLRM for recommendation memory pressure [@naumov2019deep], and the depthwise-separable convolutional neural network (CNN) known as DS-CNN for TinyML keyword spotting [@zhang2017hello]. Evaluating these models provides consistent benchmarks, grounding trade-offs between accuracy, latency, memory, and energy under identical operating conditions.
...
These dimensions are interdependent. Pruning reduces complexity but may require architectural changes for hardware efficiency. Quantization reduces precision but alters numerical behavior and available execution paths. A rigorous deployment strategy combines techniques across layers when they relieve distinct binding constraints, as mapped in @sec-model-compression-decision-framework-0d69.
...
The structural mechanics of these choices appear directly in @fig-channel-layer-pruning. Channel pruning removes feature-map slices within convolutional layers, requiring downstream filter depths to contract while preserving overall network topology and dense kernel execution. Layer pruning excises entire computational stages, requiring skip-connection rewiring and structural reconfiguration. While both methods require retraining to recover accuracy, layer pruning accepts a steeper risk of unrecoverable capacity loss in exchange for eliminating entire kernel dispatches.
...
The three-row sequence in @fig-iterative-pruning demonstrates this gradual adaptation on a convolutional network pruned by six channels. Rather than slashing all six channels in a single pass, the pipeline removes two channels per cycle across three iterations, retraining after each cut. The initial cut drops accuracy from 0.995 to 0.971, but brief fine-tuning restores performance to 0.992. Two subsequent prune-and-tune cycles produce an end model at 0.991 accuracy—a negligible 0.4 percent degradation despite a 27 percent reduction in channel count. Staging structural surgery across multiple retraining intervals gives surviving weights time to compensate.
...
Starting from the softmax normalization introduced in @sec-neural-computation-nonlinear-activation-functions-38bc, introducing a **temperature parameter**\index{Knowledge distillation!temperature parameter}[^fn-temperature-boltzmann-scaling] $T_{\text{distill}}$ softens the output probability distribution. The temperature-scaled probability for class $i$ becomes:
$$
...
Here $\mathbf{p}_{\text{teacher}}^{(T_{\text{distill}})}$ and $\mathbf{p}_{\text{student}}^{(T_{\text{distill}})}$ are the teacher and student probability distributions computed with $T_{\text{distill}}$, $y$ is the hard label, and $\gamma_{\text{KD}} \in [0,1]$ weights the hard-label and distillation terms. The factor $T_{\text{distill}}^2$ compensates for the temperature-induced reduction in gradient magnitude used in this formulation. While this formulation captures response-based distillation matching final output logits, modern pipelines also employ feature-based distillation. In feature distillation, intermediate student layers are trained to directly match intermediate teacher activations or attention maps using mean squared error or cosine distance, providing richer intermediate guidance throughout deep student backbones. The resulting student must still be evaluated on task quality, calibration, memory, and latency.
...
Understanding the impact of numerics requires moving from the algorithm to silicon-level energy and data movement, where each bit represents both a storage allocation and a dynamic energy cost.

These savings explain why systems exploit quantization, not why neural networks tolerate it. Numerical tolerance depends on model- and layer-specific robustness to perturbation, while the hardware benefit requires preserving task accuracy. The practical envelope of quantization is governed by two constraints: how far precision can drop before task fidelity degrades, and whether target execution units can translate narrower formats into wall-clock speedup.
...
Binary and ternary networks represent the extreme limit of quantization, constraining weights and activations to single bits or small ternary value sets. While raw representations are compact, realized storage and energy depend on bit packing and hardware support, and task quality becomes sensitive to training stabilization. The keyword-spotting lighthouse (@sec-network-architectures-efficient-architectures-keyword-spotting-0625) illustrates an operating regime where extreme compression is required to satisfy the `{python} ModelDeviceComparison.tiny_cap_kb_str` SRAM budget. INT8 provides a baseline target; INT4, binary, or ternary representations trade additional numerical dynamic range to meet stricter memory and power bounds.
...
Executing low-precision inference requires transforming floating-point inputs before kernel execution and requantizing intermediate accumulator outputs. During integer matrix multiplication, multiplying two 8-bit integers produces a 16-bit product. Summing these products across an inner dimension of length $K$ causes bit-growth that quickly overflows 8-bit registers; hardware therefore accumulates into 32-bit integer registers (`INT32`). Before feeding the result into subsequent layers, requantization rescales the 32-bit accumulated sum by the layer's effective scale factor ($s_{\text{eff}} = \frac{s_W \cdot s_X}{s_Y}$) and clamps the values back into the 8-bit integer range. Follow the color-coded pipeline stages in @fig-weight-activations-quantization, observing how violet INT8 conversion blocks feed into integer SIMD matrix multiplication before red 32-bit accumulation.
...
The primary limitation is that PTQ freezes model parameters, preventing compensation for accuracy loss. When a quantized model falls below the production threshold, mitigations include recalibrating, refining scale granularity, adopting mixed precision, or escalating to quantization-aware training. QAT integrates precision constraints directly into the training graph using a **straight-through estimator (STE)**\index{Straight-through estimator!gradient approximation} to pass gradients through non-differentiable rounding operators. This enables weights to adapt to low-bit representations during retraining, recovering accuracy at the cost of full training compute ($O_{\text{train}}$) and hyperparameter tuning.
...
This decision framework establishes starting points for individual technique selection. Validating that a chosen technique achieves its intended goal requires systematic profiling and measurement (@sec-model-compression-efficiency-measurement-2424). Production deployments rarely rely on an isolated technique: combining pruning with quantization, or distillation with hardware-aware design, introduces non-linear interactions across the execution pipeline. Managing these cross-technique couplings requires deliberate sequencing protocols.
...
At the model-development layer, framework APIs are most useful for transformations that need access to training state, calibration data, or weight tensors before export. TensorFlow, PyTorch, and MXNet expose different APIs, but serve the same systems role. They insert quantization behavior into the graph, mutate or mask weights for pruning, and preserve enough metadata to reproduce the optimized artifact. While @sec-ml-frameworks examines framework architectures comprehensively, model compression depends specifically on this graph-transformation boundary.
