...
\noindent
![](images/png/cover_security_privacy_blueprint_labeled_print.png){fig-alt="Security and privacy boundary chamber separating data, model weights, runtime, users, and audit logs into protected and untrusted zones."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_security_privacy_blueprint_labeled.webp){fig-alt="Security and privacy boundary chamber separating data, model weights, runtime, users, and audit logs into protected and untrusted zones."}
:::
...
Many useful machine learning systems depend on personal data, institutional knowledge, or behavioral patterns, creating tension between utility and protection. Models can encode sensitive information and later reveal it through their outputs, extending risk beyond transient data processing. A high-performing model may still be undeployable if it exposes sensitive data, permits adversarial manipulation, or fails the regulatory requirements of its domain. Privacy and security are therefore system requirements wherever data sensitivity and adversarial risk are binding constraints. In C³ terms, they change Compute, Communication, and Coordination contracts. Throughput is insufficient unless each boundary also preserves confidentiality, integrity, and accountable control.
...
- Specify tested-accountant inputs and evaluate reported privacy budgets against sampling, composition, and utility evidence
...
\begin{tikzpicture}[
  ...
  % LEFT DEPTH AXIS
  % =========================================================================
  \fill[CrimsonRed] (-8.7, 5.8) circle (3pt);
  \node[above=3pt, font=\scriptsize\bfseries\color{CrimsonRed}] at (-8.7, 5.8) {Logical (API)};
  \draw[->, dashed, line width=1.4pt, CrimsonRed] (-8.7, 5.7) -- (-8.7, -5.8);
  \node[below=3pt, font=\scriptsize\bfseries\color{CrimsonRed}] at (-8.7, -5.8) {Physical (Silicon)};
  \node[rotate=90, font=\scriptsize\bfseries\color{CrimsonRed}] at (-9.0, 0) {INCREASING ACCESS DEPTH};

  % =========================================================================
  % TOP HEADER CARDS
  % =========================================================================
  \node[draw=OceanBlue!40, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=4.7cm, minimum height=0.7cm] at (-2.1, 5.8) {
    \textbf{\scriptsize\color{DarkNavy}TARGET ASSETS}
  };
  \node[draw=CoralRed!50, fill=SoftRed!40, rounded corners=3pt, line width=0.8pt, minimum width=4.7cm, minimum height=0.7cm] at (2.9, 5.8) {
    \textbf{\scriptsize\color{CrimsonRed}PRIMARY THREAT VECTORS}
  };
  \node[draw=MintGreen!50, fill=SoftGreen!40, rounded corners=3pt, line width=0.8pt, minimum width=4.7cm, minimum height=0.7cm] at (7.9, 5.8) {
  ...
  % LAYER 1: DATA LAYER
  % =========================================================================
  \draw[draw=BorderGrey, fill=white, rounded corners=5pt, line width=0.9pt] (-7.6, 3.2) rectangle (10.4, 5.2);

  % Left ID Card
  \draw[draw=BorderGrey!70, fill=SoftGrey, rounded corners=4pt, line width=0.6pt] (-7.4, 3.35) rectangle (-5.2, 5.05);
  ...
  % Target Assets
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, 4.8) {Pretraining \& SFT Corpora};
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, 4.2) {Ground-Truth Annotations};
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, 3.6) {Feature Stores \& Embeddings};

  % Threats
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, 4.8) {Data Poisoning \& Label Flipping};
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, 4.2) {Backdoor Trigger Injection};
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, 3.6) {Membership Inference \& PII Leak};

  % Defenses
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, 4.8) {DP-SGD / Differential Privacy};
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, 4.2) {Cryptographic Provenance / SBOM};
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, 3.6) {Anomaly Detection \& Sanitizing};

  % =========================================================================
  % LAYER 2: MODEL LAYER
  % =========================================================================
  \draw[draw=BorderGrey, fill=white, rounded corners=5pt, line width=0.9pt] (-7.6, 0.8) rectangle (10.4, 2.8);

  % Left ID Card
  \draw[draw=BorderGrey!70, fill=SoftGrey, rounded corners=4pt, line width=0.6pt] (-7.4, 0.95) rectangle (-5.2, 2.65);
  ...
  % Target Assets
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, 2.4) {Pretrained Weights \& Adapters};
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, 1.8) {Model Architecture Topology};
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, 1.2) {Inference Latents \& KV Cache};

  % Threats
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, 2.4) {Model Extraction \& Stealing};
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, 1.8) {Adversarial Evasion Perturbations};
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, 1.2) {Model Inversion \& Reconstruction};

  % Defenses
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, 2.4) {Adversarial Training \& Pruning};
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, 1.8) {Model Watermarking \& Signatures};
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, 1.2) {Rate Limits \& Logit Perturbation};

  % =========================================================================
  % LAYER 3: PLATFORM & OS
  % =========================================================================
  \draw[draw=BorderGrey, fill=white, rounded corners=5pt, line width=0.9pt] (-7.6, -1.6) rectangle (10.4, 0.4);

  % Left ID Card
  \draw[draw=BorderGrey!70, fill=SoftGrey, rounded corners=4pt, line width=0.6pt] (-7.4, -1.45) rectangle (-5.2, 0.25);
  ...
  % Target Assets
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, 0.0) {GPU / TPU Host Memory \& HBM};
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, -0.6) {Kubernetes / Slurm Orchestration};
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, -1.2) {Inter-Node InfiniBand / RoCE Fabric};

  % Threats
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, 0.0) {Container Breakout \& Host Tamper};
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, -0.6) {Fabric Snooping \& MiTM Attacks};
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, -1.2) {Side-Channel Power \& EM Leakage};

  % Defenses
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, 0.0) {Confidential VMs / AMD SEV-SNP};
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, -0.6) {IPsec / TLS Link-Level Encryption};
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, -1.2) {Strict RBAC \& Workload mTLS};

  % =========================================================================
  % LAYER 4: SUPPLY CHAIN
  % =========================================================================
  \draw[draw=BorderGrey, fill=white, rounded corners=5pt, line width=0.9pt] (-7.6, -4.0) rectangle (10.4, -2.0);

  % Left ID Card
  \draw[draw=BorderGrey!70, fill=SoftGrey, rounded corners=4pt, line width=0.6pt] (-7.4, -3.85) rectangle (-5.2, -2.15);
  \node[fill=SoftRed, rounded corners=2pt, minimum width=1.8cm, minimum height=0.4cm, font=\tiny\bfseries\color{CrimsonRed}] at (-6.3, -2.4) {LAYER 4};
  ...
  % Target Assets
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, -2.4) {Accelerator Silicon \& Board Schematics};
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, -3.0) {Pre-boot Firmware / Baseboard BMC};
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{DarkNavy}] at (-2.1, -3.6) {Open-Source Packages (PyPI / Hub)};

  % Threats
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, -2.4) {Hardware Trojans \& Interposer Taps};
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, -3.0) {Malicious Pickles / Deserialization RCE};
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{CrimsonRed}] at (2.9, -3.6) {Laser Fault Injection / Rowhammer};

  % Defenses
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, -2.4) {Hardware Root of Trust / PUF / TPM};
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, -3.0) {Secure Boot \& Measured Attestation};
  \node[draw=MintGreen!50, fill=white, rounded corners=3pt, minimum width=4.7cm, minimum height=0.52cm, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] at (7.9, -3.6) {SafeTensors (Zero-Code Serialization)};
...
1.  **Significance**: It limits the exposure risk of training data and user inputs. Differentially private training introduces a utility-privacy trade-off\index{Utility-Privacy Trade-Off}: tighter privacy bounds generally require more noise or data, which can increase the total operations $(O)$ required to reach a target utility.
...
A platform team hosts two models on a single H100 using Multi-Instance GPU (MIG) to provide hardware-level isolation. On a dedicated GPU, the model achieves `{python} MultiTenantIsolation.mt_peak_tokens_str` per second. After enabling secure partitioning, it achieves `{python} MultiTenantIsolation.mt_secure_tokens_str` per second. The gap between those two figures is the performance cost of security.

Isolation requires dedicated hardware resources (SRAM, cache) and adds context-switching overhead. The throughput loss\index{Throughput!loss} is `{python} MultiTenantIsolation.mt_peak_tokens_str` - `{python} MultiTenantIsolation.mt_secure_tokens_str` = `{python} MultiTenantIsolation.mt_throughput_loss_str` per second, an isolation tax of (`{python} MultiTenantIsolation.mt_throughput_loss_str`/`{python} MultiTenantIsolation.mt_peak_tokens_str`) = `{python} MultiTenantIsolation.mt_overhead_pct_str`.
...
**Fix**: Strava revised its product by improving privacy zones, restricting default heatmap visibility, and making heatmap inclusion strictly opt-in.
...
\begin{tikzpicture}[
  ...
  % =========================================================================
  \node[draw=OrangeBorder, fill=SoftOrange!40, rounded corners=4pt, line width=0.8pt, minimum width=6.2cm, minimum height=1.35cm] (spoof) at (2.6, 4.4) {
  ...
  \draw[->, dashed, line width=1.1pt, OrangeBorder] (spoof.south) -- (2.6, 3.25);
  ...
  % --- Card 1: Infected USB ---
  \draw[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt] (-7.6, 0.4) rectangle (-5.2, 3.25);
  \node[fill=SoftRed!60, rounded corners=2pt, minimum width=2.1cm, minimum height=0.35cm, font=\tiny\bfseries\color{CrimsonRed}] at (-6.4, 2.95) {PHYSICAL VECTOR};
  ...
  \draw[draw=BorderGrey!80, fill=SoftBlue!40, rounded corners=2pt] (-7.4, 0.6) rectangle (-5.4, 1.7);
  \node[font=\tiny\bfseries\color{CrimsonRed}] at (-6.4, 1.35) {Air-Gap Crossing};
  \node[font=\tiny\color{SlateGrey}] at (-6.4, 0.95) {LNK Shortcut Flaw};

  % Arrow 1 -> 2
  \draw[->, line width=1.2pt, CrimsonRed] (-5.1, 1.8) -- (-4.1, 1.8);

  % --- Card 2: Windows Host ---
  \draw[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt] (-4.0, 0.4) rectangle (-1.6, 3.25);
  \node[fill=SoftRed!60, rounded corners=2pt, minimum width=2.1cm, minimum height=0.35cm, font=\tiny\bfseries\color{CrimsonRed}] at (-2.8, 2.95) {HOST EXPLOIT};
  ...
  \draw[draw=BorderGrey!80, fill=SoftBlue!40, rounded corners=2pt] (-3.8, 0.6) rectangle (-1.8, 1.7);
  \node[font=\tiny\bfseries\color{CrimsonRed}] at (-2.8, 1.35) {$4\times$ Zero-Days};
  \node[font=\tiny\color{SlateGrey}] at (-2.8, 0.95) {Privilege Escalation};

  % Arrow 2 -> 3
  \draw[->, line width=1.2pt, CrimsonRed] (-1.5, 1.8) -- (-0.5, 1.8);

  % --- Card 3: Siemens Step7 ---
  \draw[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt] (-0.4, 0.4) rectangle (2.0, 3.25);
  \node[fill=SoftRed!60, rounded corners=2pt, minimum width=2.1cm, minimum height=0.35cm, font=\tiny\bfseries\color{CrimsonRed}] at (0.8, 2.95) {SOFTWARE LAYER};
  ...
  \draw[draw=BorderGrey!80, fill=SoftBlue!40, rounded corners=2pt] (-0.2, 0.6) rectangle (1.8, 1.7);
  \node[font=\tiny\bfseries\color{CrimsonRed}] at (0.8, 1.35) {Rogue s7otbxdx.dll};
  \node[font=\tiny\color{SlateGrey}] at (0.8, 0.95) {Intercepted Blocks};

  % Arrow 3 -> 4
  \draw[->, line width=1.2pt, CrimsonRed] (2.1, 1.8) -- (3.1, 1.8);

  % --- Card 4: Simatic S7-300 ---
  \draw[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt] (3.2, 0.4) rectangle (5.6, 3.25);
  \node[fill=SoftRed!60, rounded corners=2pt, minimum width=2.1cm, minimum height=0.35cm, font=\tiny\bfseries\color{CrimsonRed}] at (4.4, 2.95) {FIRMWARE ROOTKIT};
  \node[font=\small\bfseries\color{DarkNavy}] at (4.4, 2.45) {Simatic S7-300};
  \node[font=\tiny\color{SlateGrey}] at (4.4, 2.15) {PLC Controller};
  \draw[draw=BorderGrey!80, fill=SoftBlue!40, rounded corners=2pt] (3.4, 0.6) rectangle (5.4, 1.7);
  \node[font=\tiny\bfseries\color{CrimsonRed}] at (4.4, 1.35) {Injected Ladder Logic};
  \node[font=\tiny\color{SlateGrey}] at (4.4, 0.95) {Masked Diagnostic I/O};

  % Arrow 4 -> 5
  \draw[->, line width=1.2pt, CrimsonRed] (5.7, 1.8) -- (6.7, 1.8);

  % --- Card 5: Motor Inverters ---
  \draw[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt] (6.8, 0.4) rectangle (9.2, 3.25);
  ...
  \draw[draw=BorderGrey!80, fill=SoftBlue!40, rounded corners=2pt] (7.0, 0.6) rectangle (9.0, 1.7);
  \node[font=\tiny\bfseries\color{CrimsonRed}] at (8.0, 1.35) {$807 \leftrightarrow 1{,}410\text{ Hz}$};
  \node[font=\tiny\color{SlateGrey}] at (8.0, 0.95) {Rotor Resonant Strain};

  % Arrow 5 -> 6
  \draw[->, line width=1.2pt, CrimsonRed] (9.3, 1.8) -- (10.3, 1.8);

  % --- Card 6: Physical Impact ---
  \draw[draw=CrimsonRed, fill=white, rounded corners=4pt, line width=1.1pt] (10.4, 0.4) rectangle (12.8, 3.25);
  ...
  \draw[draw=CoralRed!50, fill=SoftRed!50, rounded corners=2pt] (10.6, 0.6) rectangle (12.6, 1.7);
  ...
  % =========================================================================
  \draw[draw=BorderGrey, fill=SoftGrey, rounded corners=4pt, line width=0.7pt] (-7.6, -0.6) rectangle (12.8, 0.1);
  ...
  \node[font=\footnotesize\bfseries\color{DarkNavy}, anchor=west] at (-4.8, -0.25) {First confirmed cyberweapon to cross digital-to-kinetic boundaries (Natanz, Iran, 2010).};
  \node[font=\tiny\color{SlateGrey}, anchor=east] at (12.5, -0.25) {Targets: Siemens S7-300 + Fararo Paya / Vacon drives};

  % =========================================================================
  % BOTTOM: SYSTEMS TAKEAWAYS
  % =========================================================================
  \draw[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt] (-7.6, -3.3) rectangle (12.8, -1.0);

  % Header Strip
  \draw[fill=SoftBlue!60, rounded corners=3pt] (-7.5, -1.75) rectangle (12.7, -1.1);
  \node[fill=DarkNavy, rounded corners=2pt, minimum width=2.6cm, minimum height=0.45cm, font=\tiny\bfseries\color{white}] at (-6.0, -1.42) {SYSTEMS TAKEAWAYS};
  \node[font=\footnotesize\bfseries\color{DarkNavy}, anchor=west] at (-4.5, -1.42) {Trust Boundaries \& Integrity Invariants for Autonomous \& AI Systems};

  % 3 Takeaway Cards
  \node[draw=BorderGrey!70, fill=SoftGrey, rounded corners=3pt, minimum width=6.4cm, minimum height=1.25cm, anchor=north west] at (-7.4, -1.9) {
  ...
  \node[draw=BorderGrey!70, fill=SoftGrey, rounded corners=3pt, minimum width=6.4cm, minimum height=1.25cm, anchor=north] at (2.6, -1.9) {
    \begin{minipage}{6.1cm}\raggedright
  ...
  \node[draw=BorderGrey!70, fill=SoftGrey, rounded corners=3pt, minimum width=6.4cm, minimum height=1.25cm, anchor=north east] at (12.6, -1.9) {
    \begin{minipage}{6.1cm}\raggedright
...
[^fn-membership-inference-sec]: **Membership inference attacks**\index{Membership Inference Attack}\index{Membership Inference!privacy}: First demonstrated against ML models by @shokri2017membership, these attacks determine whether a specific data point was used in training by exploiting setting-specific differences between model behavior on training and unseen data. Attack success depends on the target model, data, outputs, and adversary knowledge. Confirming that an individual's data was in a training set can still create privacy and compliance risk; whether that disclosure violates GDPR, HIPAA, or another rule depends on the data, parties, and deployment context [@gdpr2016; @hhs2003hipaaprivacy].
...
\begin{tikzpicture}[
  ...
  % =========================================================================
  % Y-axis
  \draw[->, line width=1.4pt, SlateGrey] (-0.3, 0.5) -- (-0.3, 9.4);
  \node[rotate=90, font=\footnotesize\bfseries\color{DarkNavy}, anchor=south] at (-0.3, 5.0) {SYSTEMIC IMPACT (BLAST RADIUS)};
  \node[font=\scriptsize\bfseries\color{SlateGrey}, anchor=east] at (-0.5, 8.8) {High Impact};
  \node[font=\scriptsize\bfseries\color{SlateGrey}, anchor=east] at (-0.5, 1.2) {Low Impact};

  % X-axis
  \draw[->, line width=1.4pt, SlateGrey] (0.2, 0.0) -- (16.2, 0.0);
  ...
  % =========================================================================
  \draw[draw=OrangeBorder!80, fill=SoftOrange!15, rounded corners=5pt, line width=1pt] (0.3, 5.1) rectangle (8.0, 9.3);

  % Header Strip
  \draw[draw=OrangeBorder!40, fill=white, rounded corners=3pt] (0.5, 8.5) rectangle (7.8, 9.1);
  ...
  % 3 Items
  \node[draw=BorderGrey!80, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\color{DarkNavy}, anchor=west] at (0.5, 8.0) {\hspace{6pt}Laser Fault Injection / Glitching};
  \node[draw=BorderGrey!80, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\color{DarkNavy}, anchor=west] at (0.5, 7.3) {\hspace{6pt}Silicon Supply Chain Trojan Insertion};
  \node[draw=BorderGrey!80, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\color{DarkNavy}, anchor=west] at (0.5, 6.6) {\hspace{6pt}Physical Side-Channel Cryptanalysis};

  % Strategy Box
  \node[draw=OrangeBorder!60, fill=SoftOrange!35, rounded corners=3pt, minimum width=7.3cm, minimum height=1.05cm, anchor=west] at (0.5, 5.75) {
  ...
  % =========================================================================
  \draw[draw=CrimsonRed!80, fill=SoftRed!20, rounded corners=5pt, line width=1pt] (8.4, 5.1) rectangle (16.1, 9.3);

  % Header Strip
  \draw[draw=CoralRed!40, fill=white, rounded corners=3pt] (8.6, 8.5) rectangle (15.9, 9.1);
  \node[fill=SoftRed, rounded corners=2pt, minimum width=1.8cm, minimum height=0.42cm, font=\tiny\bfseries\color{CrimsonRed}] at (9.6, 8.8) {CRITICAL};
  \node[font=\scriptsize\bfseries\color{CrimsonRed}, anchor=west] at (10.7, 8.8) {High Likelihood $\cdot$ High Impact};

  % 3 Items
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\bfseries\color{CrimsonRed}, anchor=west] at (8.6, 8.0) {\hspace{6pt}Untrusted Pickles / Deserialization RCE};
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\bfseries\color{CrimsonRed}, anchor=west] at (8.6, 7.3) {\hspace{6pt}Training Data Poisoning \& Backdoors};
  \node[draw=CoralRed!45, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\bfseries\color{CrimsonRed}, anchor=west] at (8.6, 6.6) {\hspace{6pt}Adversarial Physical Evasion Attacks};

  % Strategy Box
  \node[draw=CoralRed!60, fill=SoftRed!45, rounded corners=3pt, minimum width=7.3cm, minimum height=1.05cm, anchor=west] at (8.6, 5.75) {
    \begin{minipage}{6.9cm}\raggedright
      \textbf{\tiny\color{CrimsonRed}AUTOMATED RUNTIME GATES STRATEGY}\\[2pt]
  ...
  % =========================================================================
  \draw[draw=BorderGrey, fill=SoftGrey!40, rounded corners=5pt, line width=1pt] (0.3, 0.6) rectangle (8.0, 4.8);

  % Header Strip
  \draw[draw=BorderGrey!70, fill=white, rounded corners=3pt] (0.5, 4.0) rectangle (7.8, 4.6);
  ...
  % 3 Items
  \node[draw=BorderGrey!80, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\color{DarkNavy}, anchor=west] at (0.5, 3.5) {\hspace{6pt}Opportunistic Local Cache Snooping};
  \node[draw=BorderGrey!80, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\color{DarkNavy}, anchor=west] at (0.5, 2.8) {\hspace{6pt}Non-Targeted Objective Perturbations};
  \node[draw=BorderGrey!80, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\color{DarkNavy}, anchor=west] at (0.5, 2.1) {\hspace{6pt}Air-Gapped Traffic Frequency Jitter};

  % Strategy Box
  \node[draw=BorderGrey!80, fill=SoftGrey!80, rounded corners=3pt, minimum width=7.3cm, minimum height=1.05cm, anchor=west] at (0.5, 1.25) {
    \begin{minipage}{6.9cm}\raggedright
  ...
  % =========================================================================
  \draw[draw=OceanBlue!50, fill=SoftBlue!20, rounded corners=5pt, line width=1pt] (8.4, 0.6) rectangle (16.1, 4.8);

  % Header Strip
  \draw[draw=OceanBlue!35, fill=white, rounded corners=3pt] (8.6, 4.0) rectangle (15.9, 4.6);
  ...
  % 3 Items
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\color{DarkNavy}, anchor=west] at (8.6, 3.5) {\hspace{6pt}Model Stealing via Systematic API Queries};
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\color{DarkNavy}, anchor=west] at (8.6, 2.8) {\hspace{6pt}Membership Inference on Overfit Models};
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=7.3cm, minimum height=0.55cm, font=\scriptsize\color{DarkNavy}, anchor=west] at (8.6, 2.1) {\hspace{6pt}Prompt Scraping \& Behavioral Distillation};

  % Strategy Box
  \node[draw=OceanBlue!40, fill=SoftBlue!50, rounded corners=3pt, minimum width=7.3cm, minimum height=1.05cm, anchor=west] at (8.6, 1.25) {
    \begin{minipage}{6.9cm}\raggedright
...
- **Black-Box Access**\index{Black-Box Access}: The attacker has only input-output interaction with the model through APIs. This enables model extraction through prediction APIs [@tramer2016stealing] and query or transfer-based adversarial attacks [@papernot2016transferability], but limits attack precision.
- **Gray-Box Access**\index{Gray-Box Access}: The attacker has partial knowledge such as model architecture, training procedure, or dataset characteristics. This enables more targeted attacks like transferable adversarial examples.
- **White-Box Access**\index{White-Box Access}: The attacker has complete knowledge of model parameters, architecture, and training data. This enables direct gradient-based attacks [@goodfellow2015explaining] and, if model files are exposed, direct copying of weights.
...
- **Training-Time Attacks**\index{Attack!training-time}: Data poisoning or backdoor injection manipulates the learning process before the model is produced.
- **Deployment-Time Attacks**\index{Attack!deployment-time}: Model distribution, serialization, or installation is compromised before serving begins.
- **Inference-Time Attacks**\index{Attack!inference-time}: The deployed model is exploited through adversarial inputs or extraction queries.
- **Postdeployment Attacks**\index{Attack!postdeployment}: Model updates, monitoring systems, or feedback loops are targeted after the model is already in service.
...
The principle of **Minimal Attack Surface**\index{Minimal Attack Surface!definition} assumes that every exposed interface, stored artifact, and network endpoint represents a potential attack surface. Minimizing unnecessary exposure reduces risk through five mechanisms:
...
The principle of **Fail-Safe Defaults**\index{Fail-Safe Defaults!definition} requires systems to fail in ways that preserve security rather than availability when attacks succeed or anomalies occur. Four defaults make that posture concrete:
...
\begin{tikzpicture}[
  ...
  % HEADER ROW
  % =========================================================================
  \node[draw=CoralRed!50, fill=SoftRed!40, rounded corners=3pt, line width=0.8pt, minimum width=5.4cm, minimum height=0.65cm] at (-6.0, 5.2) {
    \textbf{\scriptsize\color{CrimsonRed}ACTIVE THREAT VECTORS}
  };
  \node[draw=OceanBlue!40, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.4cm, minimum height=0.65cm] at (0, 5.2) {
    \textbf{\scriptsize\color{DarkNavy}SYSTEM WORKFLOW STAGES}
  };
  \node[draw=MintGreen!50, fill=SoftGreen!40, rounded corners=3pt, line width=0.8pt, minimum width=5.4cm, minimum height=0.65cm] at (6.0, 5.2) {
  ...
  % =========================================================================
  % Threat 1
  \node[draw=CoralRed!60, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (t1) at (-6.0, 3.9) {
    \begin{minipage}{5.0cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut POISONING}}\hspace{5pt}\textbf{\scriptsize\color{CrimsonRed}Data Scraping Ingestion}\\[2pt]
      \tiny\color{DarkNavy}Malicious web corpora injection, compromised data vendors \& crawlers
    \end{minipage}
  };

  % Stage 1
  \node[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (s1) at (0, 3.9) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  % Defense 1
  \node[draw=MintGreen!70, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (d1) at (6.0, 3.9) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  \draw[->, thick, CrimsonRed] (t1.east) -- (s1.west);
  \draw[->, thick, EmeraldGreen] (s1.east) -- (d1.west);

  % =========================================================================
  % ROW 2: DATA PREPROCESSING
  % =========================================================================
  % Threat 2
  \node[draw=CoralRed!60, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (t2) at (-6.0, 2.2) {
    \begin{minipage}{5.0cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut BACKDOOR}}\hspace{5pt}\textbf{\scriptsize\color{CrimsonRed}Label \& Trigger Insertion}\\[2pt]
      \tiny\color{DarkNavy}Subtle pixel/token trigger pattern pairing with targeted class labels
    \end{minipage}
  };

  % Stage 2
  \node[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (s2) at (0, 2.2) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  % Defense 2
  \node[draw=MintGreen!70, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (d2) at (6.0, 2.2) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  \draw[->, thick, CrimsonRed] (t2.east) -- (s2.west);
  \draw[->, thick, EmeraldGreen] (s2.east) -- (d2.west);
  \draw[->, very thick, OceanBlue] (s1.south) -- (s2.north);

  % =========================================================================
  % ROW 3: MODEL TRAINING
  % =========================================================================
  % Threat 3
  \node[draw=CoralRed!60, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (t3) at (-6.0, 0.5) {
    \begin{minipage}{5.0cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut LEAKAGE}}\hspace{5pt}\textbf{\scriptsize\color{CrimsonRed}Gradient Inversion \& RCE}\\[2pt]
      \tiny\color{DarkNavy}Reconstructing batch inputs from gradients; untrusted torch checkpoint execution
    \end{minipage}
  };

  % Stage 3
  \node[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (s3) at (0, 0.5) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  % Defense 3
  \node[draw=MintGreen!70, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (d3) at (6.0, 0.5) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  \draw[->, thick, CrimsonRed] (t3.east) -- (s3.west);
  \draw[->, thick, EmeraldGreen] (s3.east) -- (d3.west);
  \draw[->, very thick, OceanBlue] (s2.south) -- (s3.north);

  % =========================================================================
  % ROW 4: MODEL EVALUATION
  % =========================================================================
  % Threat 4
  \node[draw=CoralRed!60, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (t4) at (-6.0, -1.2) {
    \begin{minipage}{5.0cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut EVASION}}\hspace{5pt}\textbf{\scriptsize\color{CrimsonRed}Metric Gaming \& Drift}\\[2pt]
      \tiny\color{DarkNavy}Benchmark data contamination; suppressed triggers evading standard tests
    \end{minipage}
  };

  % Stage 4
  \node[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (s4) at (0, -1.2) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  % Defense 4
  \node[draw=MintGreen!70, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (d4) at (6.0, -1.2) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  \draw[->, thick, CrimsonRed] (t4.east) -- (s4.west);
  \draw[->, thick, EmeraldGreen] (s4.east) -- (d4.west);
  \draw[->, very thick, OceanBlue] (s3.south) -- (s4.north);

  % =========================================================================
  % ROW 5: MODEL DEPLOYMENT
  % =========================================================================
  % Threat 5
  \node[draw=CoralRed!60, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (t5) at (-6.0, -2.9) {
    \begin{minipage}{5.0cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut EXFILTRATION}}\hspace{5pt}\textbf{\scriptsize\color{CrimsonRed}Artifact \& Weight Theft}\\[2pt]
      \tiny\color{DarkNavy}Direct checkpoint cloning from registries, unencrypted storage leaks, insider exfil
    \end{minipage}
  };

  % Stage 5
  \node[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (s5) at (0, -2.9) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  % Defense 5
  \node[draw=MintGreen!70, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (d5) at (6.0, -2.9) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  \draw[->, thick, CrimsonRed] (t5.east) -- (s5.west);
  \draw[->, thick, EmeraldGreen] (s5.east) -- (d5.west);
  \draw[->, very thick, OceanBlue] (s4.south) -- (s5.north);

  % =========================================================================
  % ROW 6: MODEL SERVING & INFERENCE
  % =========================================================================
  % Threat 6
  \node[draw=CoralRed!60, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (t6) at (-6.0, -4.6) {
    \begin{minipage}{5.0cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut EVASION}}\hspace{5pt}\textbf{\scriptsize\color{CrimsonRed}Adversarial Exploits \& Clones}\\[2pt]
      \tiny\color{DarkNavy}FGSM/PGD input perturbations, jailbreaks, systematic API model distillation
    \end{minipage}
  };

  % Stage 6
  \node[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (s6) at (0, -4.6) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  % Defense 6
  \node[draw=MintGreen!70, fill=white, rounded corners=4pt, line width=0.8pt, minimum width=5.4cm, minimum height=1.35cm] (d6) at (6.0, -4.6) {
    \begin{minipage}{5.0cm}\raggedright
  ...
  \draw[->, thick, CrimsonRed] (t6.east) -- (s6.west);
  \draw[->, thick, EmeraldGreen] (s6.east) -- (d6.west);
  \draw[->, very thick, OceanBlue] (s5.south) -- (s6.north);
...
\begin{tikzpicture}[
  ...
  % TOP HEADER BANNER
  % =========================================================================
  \node[draw=BorderGrey, fill=SoftGrey, rounded corners=3pt, line width=0.8pt, minimum width=7.2cm, minimum height=0.65cm] at (0, 5.2) {
  ...
  % =========================================================================
  \draw[draw=CoralRed!50, fill=white, rounded corners=5pt, line width=1pt] (-7.7, -2.6) rectangle (-0.3, 4.6);

  % Header
  \draw[draw=CoralRed!30, fill=SoftRed!30, rounded corners=3pt] (-7.5, 3.8) rectangle (-0.5, 4.4);
  \node[fill=SoftRed!80, rounded corners=2pt, minimum width=2.2cm, minimum height=0.42cm, font=\tiny\bfseries\color{CrimsonRed}] at (-6.3, 4.1) {DIRECT ACCESS};
  \node[font=\footnotesize\bfseries\color{CrimsonRed}, anchor=west] at (-5.0, 4.1) {Exact Artifact Theft};

  % Step 1
  \node[draw=CoralRed!35, fill=white, rounded corners=3pt, minimum width=6.8cm, minimum height=1.35cm] (l_s1) at (-4.0, 2.8) {
    \begin{minipage}{6.4cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut STEP 1}}\hspace{5pt}\textbf{\scriptsize\color{DarkNavy}Model Checkpoint Exfiltration}\\[2pt]
      \tiny\color{DarkNavy}Attacker breaches model registry, S3/GCS bucket, unprotected edge flash memory, or developer laptop.
    \end{minipage}
  };

  % Step 2
  \node[draw=CoralRed!35, fill=white, rounded corners=3pt, minimum width=6.8cm, minimum height=1.35cm] (l_s2) at (-4.0, 1.1) {
    \begin{minipage}{6.4cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut STEP 2}}\hspace{5pt}\textbf{\scriptsize\color{DarkNavy}Tensor \& Hyperparameter Parsing}\\[2pt]
  ...
  % Result
  \node[draw=CrimsonRed!80, fill=SoftRed!25, rounded corners=3pt, line width=0.9pt, minimum width=6.8cm, minimum height=1.35cm] (l_res) at (-4.0, -0.6) {
    \begin{minipage}{6.4cm}\raggedright
      \colorbox{CrimsonRed}{\textbf{\tiny\color{white}\strut RESULT}}\hspace{5pt}\textbf{\scriptsize\color{CrimsonRed}100\% Exact Bitwise Replica}\\[2pt]
  ...
  \node[draw=BorderGrey!70, fill=SoftGrey, rounded corners=3pt, minimum width=6.8cm, minimum height=1.1cm] at (-4.0, -2.0) {
    \begin{minipage}{6.4cm}\raggedright
  ...
  \draw[->, thick, CrimsonRed] (l_s1.south) -- (l_s2.north);
  \draw[->, thick, CrimsonRed] (l_s2.south) -- (l_res.north);

  % =========================================================================
  % RIGHT COLUMN: BEHAVIORAL MODEL STEALING (BLACK-BOX API)
  % =========================================================================
  \draw[draw=OceanBlue!40, fill=white, rounded corners=5pt, line width=1pt] (0.3, -2.6) rectangle (7.7, 4.6);

  % Header
  \draw[draw=OceanBlue!25, fill=SoftBlue!40, rounded corners=3pt] (0.5, 3.8) rectangle (7.5, 4.4);
  ...
  % Step 1
  \node[draw=OceanBlue!30, fill=white, rounded corners=3pt, minimum width=6.8cm, minimum height=1.35cm] (r_s1) at (4.0, 2.8) {
    \begin{minipage}{6.4cm}\raggedright
  ...
  % Step 2
  \node[draw=OceanBlue!30, fill=white, rounded corners=3pt, minimum width=6.8cm, minimum height=1.35cm] (r_s2) at (4.0, 1.1) {
    \begin{minipage}{6.4cm}\raggedright
  ...
  % Result
  \node[draw=OceanBlue!80, fill=SoftBlue!35, rounded corners=3pt, line width=0.9pt, minimum width=6.8cm, minimum height=1.35cm] (r_res) at (4.0, -0.6) {
    \begin{minipage}{6.4cm}\raggedright
  ...
  \draw[->, thick, OceanBlue] (r_s1.south) -- (r_s2.north);
  \draw[->, thick, OceanBlue] (r_s2.south) -- (r_res.north);

  % =========================================================================
  % BOTTOM BANNER: DEFENSIVE MEASURES
  % =========================================================================
  \draw[draw=MintGreen!60, fill=SoftGreen!25, rounded corners=4pt, line width=0.8pt] (-7.7, -4.3) rectangle (7.7, -3.0);
...
**Diagnosis**: Querying the API with 2 million synthetic inputs (costing ~\$50) provided enough high-information logits to train a student model achieving >97 percent behavioral agreement with the victim BERT model.

**Systems lesson**: API endpoints expose intellectual property when outputs carry raw decision-boundary logits. Serving platforms must enforce rate-limiting, query auditing, and confidence-score truncation to prevent model extraction.
...
- **Architecture details**: Layer sequences, activation functions, and connectivity patterns reveal the design choices that create the model's behavior and may be recovered through side-channel attacks,[^fn-ml-side-channel] reverse engineering, or analysis of observable outputs.
...
This type of theft often targets models deployed as services, where the model is exposed through an API or embedded in a user-facing application. Repeatedly querying the model and recording its responses allows an attacker to train their own model to mimic the behavior of the original. This process, often called model distillation[^fn-model-distillation-extraction] or knockoff modeling, allows attackers to achieve comparable functionality without access to the original model's proprietary internals [@orekondy2019].
...
\begin{tikzpicture}[font=\small\sffamily, node distance=1.5cm]
...
Data poisoning attacks can be classified by objective and scope of impact [@biggio2012poisoning]:

- **Availability Attacks**\index{Availability Attack}: Noise or label flips degrade overall model performance across tasks.
- **Targeted Attacks**\index{Targeted Attack}: A specific input or class is manipulated, leaving general performance intact but causing consistent misclassification in selected cases [@shafahi2018poison].
- **Backdoor Attacks**\index{Backdoor Attack}: Hidden triggers, often imperceptible patterns, elicit malicious behavior only when the trigger is present [@gu2017badnets].[^fn-backdoor-attack-triggers]
- **Subpopulation Attacks**\index{Subpopulation Attack}: Performance degrades on a group defined by shared features, making the attack particularly dangerous in fairness-sensitive applications [@jagielski2021subpopulation].
...
These attacks create significant real-world risks in domains such as autonomous driving, biometric authentication, and content moderation. The effectiveness can be striking: research demonstrates that small, visually hard-to-distinguish perturbations can cause image classifiers to make high-confidence errors [@szegedy2013intriguing; @goodfellow2015explaining]. Physical-world work showed that printed adversarial images can survive camera capture and remain misclassified by an ImageNet classifier [@kurakin2018]. Road-sign attacks then showed that small stickers or perturbations can cause signs to be misclassified under realistic viewing conditions [@eykholt2018]. The pace of that cycle is itself a systems parameter. @Fig-adversarial-arms-race tracks the cumulative literature, and its curvature is the reason a deployed defense should be treated as having a half-life rather than a fixed guarantee.

::: {#fig-adversarial-arms-race fig-env="figure" fig-pos="htb" fig-cap="**The adversarial arms race**: Cumulative adversarial machine learning papers on arXiv from 2014 to 2023, curving from near zero to roughly thirteen thousand. The growth is a proxy for defensive half-life: each published defense narrows the attack surface briefly before the next attack paper reopens it." fig-alt="Line chart of cumulative adversarial machine learning papers on arXiv from 2014 to 2023, curving upward from near zero to about thirteen thousand."}
...
data = pd.read_csv("vol2/13_security_privacy/data/adv_ml_papers_arxiv.csv")

fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 5))

ax.plot(data["Year"], data["Cumulative_Papers"], marker="o", linewidth=2.5, color="#d62728")

plt.title("Cumulative adversarial ML papers on arXiv", fontsize=14, pad=15)
plt.xlabel("Year", fontsize=12)
plt.ylabel("Total Papers Published", fontsize=12)
...
Adversarial attacks vary based on the attacker's level of access to the model. In white-box attacks, the adversary has full knowledge of the model's architecture, parameters, and training data, allowing them to craft highly effective adversarial examples. In black-box attacks, the adversary has no internal knowledge and must rely on querying the model and observing its outputs. Gray-box attacks fall between these extremes, with the adversary possessing partial information, such as access to the model architecture but not its parameters.
...
| **Gray-box**                  | Partial access (e.g., architecture only)     |   Limited or no access   | Attacks based on surrogate model approximation            | Known model family, unknown fine-tuning     |
...
::: {#fig-adversarial-boundary fig-env="figure" fig-pos="htb" fig-cap="**Adversarial decision boundary**: High-dimensional neural networks often have nonlinear decision boundaries. An adversary seeks to find a minimal perturbation $\delta$ that pushes a clean data point $x$ across the boundary into another class region, while ensuring the perturbation is small enough ($\epsilon_{\text{adv}}$) to remain imperceptible or physically plausible." fig-alt="Two-class plot with curved decision boundary, a clean point, and an adversarial point connected by a perturbation vector."}

```{.tikz}
\begin{tikzpicture}[font=\small\sffamily, node distance=2cm and 2cm]
  \tikzset{
    point/.style={circle, inner sep=2pt},
    arrow/.style={->, line width=1.0pt},
    decision boundary/.style={line width=1.0pt, BlueLine!60},
    class label/.style={font=\bfseries\sffamily\bfseries},
    constraint/.style={GrayLine, font=\tiny\sffamily},
    perturbation/.style={RedLine, font=\tiny\sffamily},
  ...
  \node[constraint] at (1, -0.8) {Constraint $\|\delta\| \leq \epsilon_{\text{adv}}$};
...
\begin{tikzpicture}[
  ...
  % HEADER ROW
  % =========================================================================
  \node[draw=CoralRed!50, fill=SoftRed!35, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=0.65cm] at (-5.6, 5.2) {
    \textbf{\scriptsize\color{CrimsonRed}THREAT VECTOR}
  };
  \node[draw=OceanBlue!40, fill=SoftBlue!40, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=0.65cm] at (0, 5.2) {
    \textbf{\scriptsize\color{DarkNavy}DETECTION \& AUDITING}
  };
  \node[draw=MintGreen!50, fill=SoftGreen!40, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=0.65cm] at (5.6, 5.2) {
    \textbf{\scriptsize\color{EmeraldGreen!90!black}DEFENSIVE MITIGATION}
  };

  % =========================================================================
  % ROW 1: MODEL THEFT
  % =========================================================================
  \draw[draw=BorderGrey!70, fill=SoftGrey!40, rounded corners=5pt, line width=0.7pt] (-8.5, 2.7) rectangle (8.5, 4.7);

  % Threat Card
  \node[draw=CoralRed!50, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (t1) at (-5.6, 3.7) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut MODEL THEFT}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}API Extraction}\\[3pt]
      \tiny\color{DarkNavy}Adversary clones model behavior or steals weights via high-volume systematic API queries.
    \end{minipage}
  };

  % Detection Card
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (d1) at (0, 3.7) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftBlue}{\textbf{\tiny\color{OceanBlue}\strut RATE PROFILING}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}Query Telemetry}\\[3pt]
      \tiny\color{DarkNavy}Detect burst query volume, out-of-distribution synthetic probes, and automated scraping patterns.
    \end{minipage}
  };

  % Mitigation Card
  \node[draw=MintGreen!60, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (m1) at (5.6, 3.7) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftGreen}{\textbf{\tiny\color{EmeraldGreen!90!black}\strut WATERMARKING}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}Rate Limits \& Enclaves}\\[3pt]
      \tiny\color{DarkNavy}Embed traceable verification signatures; enforce user token quotas; truncate soft logits to top-1 class labels.
    \end{minipage}
  };

  \draw[->, thick, SlateGrey] (t1.east) -- (d1.west);
  \draw[->, thick, EmeraldGreen] (d1.east) -- (m1.west);

  % =========================================================================
  % ROW 2: DATA POISONING
  % =========================================================================
  \draw[draw=BorderGrey!70, fill=SoftGrey!40, rounded corners=5pt, line width=0.7pt] (-8.5, 0.4) rectangle (8.5, 2.4);

  % Threat Card
  \node[draw=CoralRed!50, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (t2) at (-5.6, 1.4) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut POISONING}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}Corrupted Data}\\[3pt]
      \tiny\color{DarkNavy}Attacker injects backdoor triggers or flips critical ground-truth labels during pipeline ingestion.
    \end{minipage}
  };

  % Detection Card
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (d2) at (0, 1.4) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftBlue}{\textbf{\tiny\color{OceanBlue}\strut ANOMALY SCAN}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}Spectral Validation}\\[3pt]
      \tiny\color{DarkNavy}Track gradient variance trajectories; spectral signature analysis on feature embeddings to isolate clusters.
    \end{minipage}
  };

  % Mitigation Card
  \node[draw=MintGreen!60, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (m2) at (5.6, 1.4) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftGreen}{\textbf{\tiny\color{EmeraldGreen!90!black}\strut SANITIZATION}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}Provenance \& Robust Opt}\\[3pt]
      \tiny\color{DarkNavy}Cryptographic data provenance (SBOM); trimmed mean gradient aggregation; automatic quarantine of outlier batches.
    \end{minipage}
  };

  \draw[->, thick, SlateGrey] (t2.east) -- (d2.west);
  \draw[->, thick, EmeraldGreen] (d2.east) -- (m2.west);

  % =========================================================================
  % ROW 3: ADVERSARIAL EVASION
  % =========================================================================
  \draw[draw=BorderGrey!70, fill=SoftGrey!40, rounded corners=5pt, line width=0.7pt] (-8.5, -1.9) rectangle (8.5, 0.1);

  % Threat Card
  \node[draw=CoralRed!50, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (t3) at (-5.6, -0.9) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut EVASION}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}Adversarial Input}\\[3pt]
      \tiny\color{DarkNavy}Imperceptible norm-bounded or physical patch perturbations designed to trigger severe misclassification.
    \end{minipage}
  };

  % Detection Card
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (d3) at (0, -0.9) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftBlue}{\textbf{\tiny\color{OceanBlue}\strut INPUT PROBING}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}Boundary Proximity}\\[3pt]
      \tiny\color{DarkNavy}Feature squeezing divergence tests; high-frequency noise spectrum checks; monitoring decision boundary distance.
    \end{minipage}
  };

  % Mitigation Card
  \node[draw=MintGreen!60, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (m3) at (5.6, -0.9) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftGreen}{\textbf{\tiny\color{EmeraldGreen!90!black}\strut ROBUSTNESS}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}Adversarial Training}\\[3pt]
      \tiny\color{DarkNavy}Madry PGD adversarial training; randomized smoothing for certified $L_2$ radii; spatial transformations \& bit depth reduction.
    \end{minipage}
  };

  \draw[->, thick, SlateGrey] (t3.east) -- (d3.west);
  \draw[->, thick, EmeraldGreen] (d3.east) -- (m3.west);

  % =========================================================================
  % ROW 4: MEMBERSHIP INFERENCE / PRIVACY
  % =========================================================================
  \draw[draw=BorderGrey!70, fill=SoftGrey!40, rounded corners=5pt, line width=0.7pt] (-8.5, -4.2) rectangle (8.5, -2.2);

  % Threat Card
  \node[draw=CoralRed!50, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (t4) at (-5.6, -3.2) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut PRIVACY LEAK}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}Membership Inference}\\[3pt]
      \tiny\color{DarkNavy}Attacker queries target model to infer whether a specific individual's record was in the training set.
    \end{minipage}
  };

  % Detection Card
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (d4) at (0, -3.2) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftBlue}{\textbf{\tiny\color{OceanBlue}\strut GAP AUDITING}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}Generalization Discrepancy}\\[3pt]
      \tiny\color{DarkNavy}Evaluate loss/confidence variance between train set and holdout validation; empirical shadow model probing tests.
    \end{minipage}
  };

  % Mitigation Card
  \node[draw=MintGreen!60, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=5.25cm, minimum height=1.6cm] (m4) at (5.6, -3.2) {
    \begin{minipage}{4.95cm}\raggedright
      \colorbox{SoftGreen}{\textbf{\tiny\color{EmeraldGreen!90!black}\strut BOUNDED DP}}\hspace{4pt}\textbf{\scriptsize\color{DarkNavy}DP-SGD \& Regularization}\\[3pt]
      \tiny\color{DarkNavy}Formal $(\varepsilon, \delta)$-differential privacy bounds; gradient clipping with Gaussian noise; weight decay and dropout to prevent memorization.
    \end{minipage}
  };

  \draw[->, thick, SlateGrey] (t4.east) -- (d4.west);
  \draw[->, thick, EmeraldGreen] (d4.east) -- (m4.west);
...
\begin{tikzpicture}[
  ...
  % =========================================================================
  % LEFT PANEL: PHYSICAL FAULT INJECTION
  % =========================================================================
  \draw[draw=BorderGrey, fill=white, rounded corners=5pt, line width=0.9pt] (-8.6, -3.2) rectangle (-0.2, 5.2);

  % Header
  \node[fill=SoftRed, rounded corners=2pt, minimum width=3.3cm, minimum height=0.45cm, font=\tiny\bfseries\color{CrimsonRed}] at (-6.8, 4.8) {PHYSICAL FAULT INJECTION};
  \node[font=\tiny\bfseries\color{DarkNavy}, anchor=west] at (-5.0, 4.8) {Die Target \& Photocurrent Induction};

  % Card 1: Focused Laser Source
  \node[draw=CoralRed!50, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=8.0cm, minimum height=1.35cm] at (-4.4, 3.8) {
    \begin{minipage}{7.6cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut Focused Laser Source}}\hspace{6pt}\textbf{\tiny\color{DarkNavy}Near-Infrared ($\lambda = 1064\text{ nm}$) Pulsed Beam}\\[2pt]
      \tiny\color{DarkNavy}Sub-nanosecond optical pulse focused through microscope objective (spot size $\approx 1\,\mu\text{m}$)\\[1pt]
      \textbf{\tiny\color{CrimsonRed}Penetrates decapped silicon substrate directly targeting transistor junctions}
    \end{minipage}
  };

  % Card 2: Microcontroller Die View
  \draw[draw=BorderGrey!80, fill=white, rounded corners=3pt, line width=0.7pt] (-8.4, 0.4) rectangle (-0.4, 2.9);
  \node[font=\scriptsize\bfseries\color{DarkNavy}, anchor=north west] at (-8.2, 2.75) {Microcontroller Die View (Top-Down)};

  % Die Functional Blocks
  \draw[draw=BorderGrey!70, fill=SoftGrey, rounded corners=2pt] (-8.2, 0.6) rectangle (-6.1, 2.4);
  \node[font=\scriptsize\bfseries\color{DarkNavy}] at (-7.15, 2.05) {Flash / ROM};
  \node[font=\tiny\color{SlateGrey}] at (-7.15, 1.7) {Firmware \& Weights};
  \node[font=\tiny\color{LightSlate}] at (-7.15, 1.25) {Read-only store};

  \draw[draw=OceanBlue!35, fill=SoftBlue!40, rounded corners=2pt] (-5.9, 0.6) rectangle (-3.8, 2.4);
  \node[font=\tiny\bfseries\color{DarkNavy}, align=center] at (-4.85, 2.05) {CPU Execution\\Core};
  \node[font=\tiny\color{OceanBlue}] at (-4.85, 1.6) {ALU \& Status};
  \node[font=\tiny\color{SlateGrey}] at (-4.85, 1.2) {Pipeline};

  \draw[draw=CoralRed!60, fill=white, rounded corners=2pt] (-3.6, 0.6) rectangle (-0.6, 2.4);
  \node[font=\tiny\bfseries\color{CrimsonRed}, align=center] at (-2.1, 2.05) {SRAM / Register\\File};
  \node[font=\tiny\color{SlateGrey}] at (-2.1, 1.65) {Target Cell};
  % Laser spot box
  \draw[draw=CrimsonRed, fill=SoftRed!50, rounded corners=2pt, line width=0.8pt] (-3.4, 0.7) rectangle (-0.8, 1.5);
  \node[font=\tiny\bfseries\color{CrimsonRed}] at (-2.1, 1.3) {Laser Spot ($1\,\mu\text{m}$)};
  \node[font=\tiny\bfseries\color{CrimsonRed}] at (-2.1, 0.95) {Bit: $1 \rightarrow 0$ Flip};

  % Card 3: 6T SRAM Cell Dynamics
  \draw[draw=BorderGrey!80, fill=white, rounded corners=3pt, line width=0.7pt] (-8.4, -3.0) rectangle (-0.4, -0.2);
  \node[fill=SoftBlue, rounded corners=2pt, minimum width=2.4cm, minimum height=0.38cm, font=\tiny\bfseries\color{DarkNavy}] at (-6.9, -0.55) {6T SRAM Cell Dynamics};
  \node[font=\tiny\color{SlateGrey}, anchor=west] at (-5.5, -0.55) {Cross-Coupled CMOS Inverter State};

  % Pre-Attack State
  \draw[draw=MintGreen!70, fill=SoftGreen!25, rounded corners=2pt] (-8.2, -2.8) rectangle (-4.5, -0.9);
  \node[font=\tiny\bfseries\color{EmeraldGreen!90!black}] at (-6.35, -1.25) {Pre-Attack State (Stable)};
  \node[font=\tiny\bfseries\color{DarkNavy}] at (-6.35, -1.75) {$Q = 1$ (VDD), $Q\# = 0$ (GND)};
  \node[font=\tiny\color{EmeraldGreen!90!black}] at (-6.35, -2.35) {Check Flag: \texttt{AUTH\_VALID = 1}};

  % Post-Pulse State
  \draw[draw=CoralRed!70, fill=SoftRed!30, rounded corners=2pt] (-4.3, -2.8) rectangle (-0.6, -0.9);
  \node[font=\tiny\bfseries\color{CrimsonRed}] at (-2.45, -1.25) {Post-Pulse State (Inverted)};
  \node[font=\tiny\bfseries\color{CrimsonRed}] at (-2.45, -1.75) {$I_\text{photo}$ drains $Q$ node to GND};
  \node[font=\tiny\color{CrimsonRed}] at (-2.45, -2.35) {Latch flips: \texttt{AUTH\_CHECK = 0 / BYPASSED}};

  % =========================================================================
  % RIGHT PANEL: CONTROL FLOW HIJACKING
  % =========================================================================
  \draw[draw=BorderGrey, fill=white, rounded corners=5pt, line width=0.9pt] (0.2, -3.2) rectangle (8.6, 5.2);

  % Header
  \node[fill=SoftBlue, rounded corners=2pt, minimum width=3.3cm, minimum height=0.45cm, font=\tiny\bfseries\color{DarkNavy}] at (2.0, 4.8) {CONTROL FLOW HIJACKING};
  \node[font=\tiny\bfseries\color{DarkNavy}, anchor=west] at (3.8, 4.8) {Instruction Skipping \& Bypass};

  % --- Normal Execution Path ---
  \draw[draw=MintGreen!70, fill=white, rounded corners=3pt, line width=0.8pt] (0.4, 2.1) rectangle (8.4, 4.45);
  \node[fill=SoftGreen, rounded corners=2pt, minimum width=2.4cm, minimum height=0.35cm, font=\tiny\bfseries\color{EmeraldGreen!90!black}, anchor=north west] at (0.6, 4.35) {Normal Execution Path};

  % Steps
  \node[draw=BorderGrey!80, fill=SoftGrey, rounded corners=2pt, minimum width=2.1cm, minimum height=0.9cm] (n_s1) at (1.7, 3.4) {
    \begin{minipage}{1.9cm}\centering
      \textbf{\tiny\color{DarkNavy}\texttt{verify\_mac()}}\\[1pt]
      \tiny\color{SlateGrey}Check auth token\\[1pt]
      \tiny\color{SlateGrey}\texttt{CMP R0, \#1}
    \end{minipage}
  };

  \node[draw=BorderGrey!80, fill=SoftGrey, rounded corners=2pt, minimum width=2.4cm, minimum height=0.9cm] (n_s2) at (4.4, 3.4) {
    \begin{minipage}{2.2cm}\centering
      \textbf{\tiny\color{DarkNavy}\texttt{brne .deny\_access}}\\[1pt]
      \tiny\color{SlateGrey}Branch on invalid\\[1pt]
      \tiny\color{EmeraldGreen!90!black}Z-flag evaluated
    \end{minipage}
  };

  \node[draw=MintGreen!70, fill=SoftGreen!30, rounded corners=2pt, minimum width=2.1cm, minimum height=0.9cm] (n_s3) at (7.1, 3.4) {
    \begin{minipage}{1.9cm}\centering
      \textbf{\tiny\color{EmeraldGreen!90!black}\texttt{secure\_infer()}}\\[1pt]
      \tiny\color{DarkNavy}Execute ML Model\\[1pt]
      \tiny\color{EmeraldGreen!90!black}$\checkmark$ Validated Entry
    \end{minipage}
  };

  \draw[->, thick, EmeraldGreen] (n_s1.east) -- (n_s2.west);
  \draw[->, thick, EmeraldGreen] (n_s2.east) -- (n_s3.west);

  \node[font=\tiny\color{EmeraldGreen!90!black}, anchor=west] at (0.6, 2.65) {Condition Check: If signature or MAC fails, branch diverts to halt routine};
  \node[font=\tiny\color{SlateGrey}, anchor=west] at (0.6, 2.35) {Only cryptographically authenticated clients can trigger model inference};

  % --- Laser Fault Injected Path ---
  \draw[draw=CoralRed!70, fill=white, rounded corners=3pt, line width=0.8pt] (0.4, -1.0) rectangle (8.4, 1.85);
  \node[fill=SoftRed, rounded corners=2pt, minimum width=2.6cm, minimum height=0.35cm, font=\tiny\bfseries\color{CrimsonRed}, anchor=north west] at (0.6, 1.75) {Laser Fault Injected Path};

  % Steps
  \node[draw=CoralRed!60, fill=white, rounded corners=2pt, minimum width=2.1cm, minimum height=0.9cm] (f_s1) at (1.7, 0.8) {
    \begin{minipage}{1.9cm}\centering
      \textbf{\tiny\color{CrimsonRed}\texttt{verify\_mac()} $\rightarrow$ 0}\\[1pt]
      \tiny\color{CrimsonRed}Failed Auth Check\\[1pt]
      \tiny\color{CrimsonRed}Status: REJECT
    \end{minipage}
  };

  \node[draw=CrimsonRed, fill=SoftRed!60, rounded corners=2pt, line width=0.8pt, minimum width=2.2cm, minimum height=0.9cm] (f_s2) at (4.4, 0.8) {
    \begin{minipage}{2.0cm}\centering
      \textbf{\tiny\color{CrimsonRed}BIT FLIP / NOP}\\[1pt]
      \tiny\color{DarkNavy}Branch instruction\\[1pt]
      \textbf{\tiny\color{CrimsonRed}SKIPPED / ZEROED}
    \end{minipage}
  };

  \node[draw=CoralRed!70, fill=SoftRed!30, rounded corners=2pt, minimum width=2.1cm, minimum height=0.9cm] (f_s3) at (7.1, 0.8) {
    \begin{minipage}{1.9cm}\centering
      \textbf{\tiny\color{CrimsonRed}UNAUTH INFER}\\[1pt]
      \tiny\color{DarkNavy}Weights Extracted\\[1pt]
      \tiny\color{CrimsonRed}$\triangle$ Security Broken
    \end{minipage}
  };

  \draw[->, thick, CrimsonRed] (f_s1.east) -- (f_s2.west);
  \draw[->, thick, CrimsonRed] (f_s2.east) -- (f_s3.west);

  \node[font=\tiny\bfseries\color{CrimsonRed}, anchor=west] at (0.6, 0.05) {Exploit Result: Laser pulse forces instruction decoder to read opcode as NOP};
  \node[font=\tiny\color{DarkNavy}, anchor=west] at (0.6, -0.35) {Attacker bypasses signature check without knowing private keys or password};
  \node[font=\tiny\color{SlateGrey}, anchor=west] at (0.6, -0.7) {In ML systems: Corrupts ReLU bias values or extracts raw neural weights};

  % --- Consequences Box ---
  \node[draw=OrangeBorder!80, fill=SoftOrange!20, rounded corners=3pt, minimum width=8.0cm, minimum height=1.0cm] at (4.4, -2.1) {
    \begin{minipage}{7.6cm}\raggedright
      \textbf{\tiny\color{WarmOrange}Consequences for Machine Learning Workloads}\\[2pt]
      \tiny\color{DarkNavy}1. \textbf{Misclassification:} Inverting activation threshold forces false classification\\[1pt]
      \tiny\color{DarkNavy}2. \textbf{Model exfiltration:} Instruction skip dumps proprietary weights over bus
    \end{minipage}
  };

  % =========================================================================
  % BOTTOM WIDE BANNER: HARDWARE MITIGATIONS
  % =========================================================================
  \draw[draw=MintGreen!70, fill=SoftGreen!20, rounded corners=4pt, line width=0.8pt] (-8.6, -4.8) rectangle (8.6, -3.5);

  \node[fill=SoftGreen!90, rounded corners=2pt, minimum width=2.8cm, minimum height=0.45cm, font=\tiny\bfseries\color{EmeraldGreen!90!black}] at (-7.0, -4.15) {HARDWARE MITIGATIONS};

  \node[anchor=west] at (-5.4, -4.15) {
    \begin{minipage}{13.5cm}\raggedright
      \tiny\textbf{\color{DarkNavy}1. Dual-Core Lockstep (DCLS):} \color{DarkNavy}Redundant cores execute cycle-by-cycle comparisons; any mismatch triggers reset\\[1pt]
      \tiny\textbf{\color{DarkNavy}2. On-Die Optical Sensors:} \color{DarkNavy}Integrated photodiodes detect unauthorized optical exposure, zeroizing keys instantly\\[1pt]
      \tiny\textbf{\color{DarkNavy}3. Parity \& ECC Hardening:} \color{DarkNavy}SEC-DED ECC tags on weight buffers and branch registers trap bit-flip anomalies
    \end{minipage}
  };
...
\begin{tikzpicture}[
  ...
  % =========================================================================
  % LEFT PANEL: PROGRAM COUNTER TRACE
  % =========================================================================
  \draw[draw=BorderGrey, fill=white, rounded corners=5pt, line width=0.9pt] (-8.6, -4.4) rectangle (0.6, 4.4);

  % Header
  \draw[fill=SoftBlue!60, rounded corners=3pt] (-8.4, 3.75) rectangle (0.4, 4.25);
  \node[font=\footnotesize\bfseries\color{DarkNavy}, anchor=west] at (-8.2, 4.0) {1. PROGRAM COUNTER TRACE (AVR 8-BIT CORE)};

  % Subheaders
  \node[font=\scriptsize\bfseries\color{DarkNavy}, anchor=west] at (-8.1, 3.3) {Assembly Sequence};
  \node[font=\tiny\color{SlateGrey}, anchor=east] at (0.2, 3.3) {\texttt{TinyAES / Micro-ML}};

  % Assembly Code Table
  % Shaded else block (Lines 07 to 10)
  \fill[SoftRed!25, rounded corners=3pt] (-8.3, -3.35) rectangle (0.4, -1.35);

  % Fault Target Highlight Box (Line 03)
  \draw[draw=CoralRed, fill=SoftRed!40, rounded corners=3pt, line width=0.9pt] (-8.3, 1.25) rectangle (0.4, 1.85);

  % Code Lines
  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, 2.65) {\textbf{\color{SlateGrey}01}\quad \textbf{\color{DarkNavy}ldi}\quad \text{\color{DarkNavy}r1, 0}\qquad\qquad \text{\color{SlateGrey}; load 0 to r1}};
  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, 2.1)  {\textbf{\color{SlateGrey}02}\quad \textbf{\color{DarkNavy}cp}\quad\; \text{\color{DarkNavy}r1, r15}\qquad\quad \text{\color{SlateGrey}; compare MSB(Accum) to r1}};
  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, 1.55) {\textbf{\color{CrimsonRed}03}\quad \textbf{\color{CrimsonRed}brge}\; \text{\color{CrimsonRed}else}\qquad\quad \textbf{\color{CrimsonRed}; branch if $0 \ge \text{Accum}$ [FAULT TARGET]}};
  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, 0.95)  {\textbf{\color{SlateGrey}04}\quad \textbf{\color{DarkNavy}movw}\; \text{\color{DarkNavy}r10, r15}\quad\quad \text{\color{SlateGrey}; HiddenLayerOutput[i] = Accum}};
  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, 0.4)  {\textbf{\color{SlateGrey}05}\quad \textbf{\color{DarkNavy}movw}\; \text{\color{DarkNavy}r12, r17}\quad\quad \text{\color{SlateGrey}; HiddenLayerOutput[i] = Accum}};
  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, -0.15) {\textbf{\color{SlateGrey}06}\quad \textbf{\color{DarkNavy}jmp}\;\; \text{\color{DarkNavy}end}\qquad\qquad \text{\color{SlateGrey}; jump past else zeroing}};

  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, -1.6) {\textbf{\color{SlateGrey}07}\quad \textbf{\color{CrimsonRed}else:\;clr}\; \text{\color{DarkNavy}r10}\qquad\quad \text{\color{SlateGrey}; HiddenLayerOutput[i] = 0}};
  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, -2.1) {\textbf{\color{SlateGrey}08}\quad \phantom{\textbf{else:\;}}\textbf{\color{DarkNavy}clr}\; \text{\color{DarkNavy}r11}\qquad\quad \text{\color{SlateGrey}; HiddenLayerOutput[i] = 0}};
  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, -2.6) {\textbf{\color{SlateGrey}09}\quad \phantom{\textbf{else:\;}}\textbf{\color{DarkNavy}clr}\; \text{\color{DarkNavy}r12}\qquad\quad \text{\color{SlateGrey}; HiddenLayerOutput[i] = 0}};
  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, -3.1) {\textbf{\color{SlateGrey}10}\quad \phantom{\textbf{else:\;}}\textbf{\color{DarkNavy}clr}\; \text{\color{DarkNavy}r13}\qquad\quad \text{\color{SlateGrey}; HiddenLayerOutput[i] = 0}};

  \node[font=\footnotesize\ttfamily, anchor=west] at (-8.1, -3.9) {\textbf{\color{SlateGrey}11}\quad \textbf{\color{DarkNavy}end:}\;\; \text{\color{DarkNavy}\dots}\qquad\qquad\; \text{\color{SlateGrey}; continue network inference}};

  % Arrow across to panel 2
  \draw[->, line width=1.5pt, CrimsonRed] (0.6, 1.55) -- (1.3, 1.55);

  % =========================================================================
  % RIGHT PANEL: FAULT INJECTION DYNAMICS
  % =========================================================================
  \draw[draw=BorderGrey, fill=white, rounded corners=5pt, line width=0.9pt] (1.3, -4.4) rectangle (8.9, 4.4);

  % Header
  \draw[fill=SoftBlue!60, rounded corners=3pt] (1.5, 3.75) rectangle (8.7, 4.25);
  \node[font=\footnotesize\bfseries\color{DarkNavy}, anchor=west] at (1.7, 4.0) {2. FAULT INJECTION DYNAMICS};

  % Box 1: Hardware Trigger & Decode Impact
  \draw[draw=BorderGrey!80, fill=white, rounded corners=4pt, line width=0.7pt] (1.5, 0.4) rectangle (8.7, 3.4);
  \node[font=\scriptsize\bfseries\color{DarkNavy}, anchor=north west] at (1.7, 3.25) {Hardware Trigger \& Decode Impact};

  % Trigger item (red)
  \node[draw=CoralRed!70, fill=SoftRed!40, rounded corners=3pt, minimum width=6.8cm, minimum height=0.75cm] at (5.1, 2.45) {
    \begin{minipage}{6.5cm}\raggedright
      \textbf{\tiny\color{CrimsonRed}Trigger: Clock Glitch / Laser Pulse}\\[1pt]
      \tiny\color{CrimsonRed}Targeted at instruction cycle of line 03 (\texttt{brge})
    \end{minipage}
  };

  % Decode logic
  \node[draw=BorderGrey!70, fill=SoftGrey, rounded corners=3pt, minimum width=6.8cm, minimum height=0.45cm] at (5.1, 1.75) {
    \begin{minipage}{6.5cm}\raggedright
      \textbf{\tiny\color{DarkNavy}Decode Logic:}\quad \tiny\color{DarkNavy}Voltage droop forces decode to \texttt{NOP}
    \end{minipage}
  };

  % Status flags
  \node[draw=BorderGrey!70, fill=SoftGrey, rounded corners=3pt, minimum width=6.8cm, minimum height=0.45cm] at (5.1, 1.2) {
    \begin{minipage}{6.5cm}\raggedright
      \textbf{\tiny\color{DarkNavy}Status Flags:}\quad \tiny\color{DarkNavy}SREG condition flags (Z, S, V) corrupted
    \end{minipage}
  };

  % Branch state
  \node[draw=CoralRed!60, fill=SoftRed!30, rounded corners=3pt, minimum width=6.8cm, minimum height=0.45cm] at (5.1, 0.65) {
    \begin{minipage}{6.5cm}\raggedright
      \textbf{\tiny\color{CrimsonRed}Branch State:}\quad \textbf{\tiny\color{CrimsonRed}Branch skipped; forced fall-through}
    \end{minipage}
  };

  % Box 2: Model Representation Divergence
  \draw[draw=BorderGrey!80, fill=white, rounded corners=4pt, line width=0.7pt] (1.5, -4.1) rectangle (8.7, 0.1);
  \node[font=\scriptsize\bfseries\color{DarkNavy}, anchor=north west] at (1.7, -0.05) {Model Representation Divergence};

  % Nominal vs Corrupted Badges
  \node[draw=MintGreen!70, fill=SoftGreen!35, rounded corners=3pt, minimum width=3.25cm, minimum height=0.85cm] at (3.3, -0.9) {
    \begin{minipage}{2.9cm}\centering
      \textbf{\tiny\color{EmeraldGreen!90!black}NOMINAL PATH}\\[2pt]
      \scriptsize\textbf{\color{DarkNavy}Output = Accum}
    \end{minipage}
  };

  \node[draw=CoralRed!70, fill=SoftRed!45, rounded corners=3pt, minimum width=3.25cm, minimum height=0.85cm] at (6.9, -0.9) {
    \begin{minipage}{2.9cm}\centering
      \textbf{\tiny\color{CrimsonRed}CORRUPTED PATH}\\[2pt]
      \scriptsize\textbf{\color{CrimsonRed}Output = 0.0}
    \end{minipage}
  };

  % Activation shift
  \node[draw=BorderGrey!70, fill=SoftGrey, rounded corners=3pt, minimum width=6.8cm, minimum height=0.65cm] at (5.1, -1.95) {
    \begin{minipage}{6.5cm}\raggedright
      \textbf{\tiny\color{DarkNavy}Activation Shift:}\\[1pt]
      \tiny\color{DarkNavy}ReLU activation forcibly clamped to zero
    \end{minipage}
  };

  % Security bypass
  \node[draw=CoralRed!70, fill=SoftRed!35, rounded corners=3pt, minimum width=6.8cm, minimum height=0.75cm] at (5.1, -2.95) {
    \begin{minipage}{6.5cm}\raggedright
      \textbf{\tiny\color{CrimsonRed}Security Bypass:}\\[1pt]
      \tiny\color{CrimsonRed}Safety gate disabled $\cdot$ Layer output suppressed
    \end{minipage}
  };
...
[^fn-debug-port-vulnerability]: **Debug Port Vulnerabilities**\index{Debug Port!debug port vulnerabilities}\index{Debug Port!JTAG vulnerability}: Hardware debug interfaces like JTAG and Serial Wire Debug provide full memory read/write access for development but are left unsecured in an estimated 60–70 percent of shipped embedded devices. For ML edge devices, an exposed JTAG port allows an attacker to extract model weights directly from flash memory, bypassing all software-level protections including encryption and access control.
...
\begin{tikzpicture}[
  ...
  % LEFT ROW BADGES
  % =========================================================================
  \node[draw=CoralRed!50, fill=SoftRed!40, rounded corners=2pt, minimum width=2.6cm, minimum height=0.55cm, font=\tiny\bfseries\color{CrimsonRed}] at (-8.6, 2.5) {HARDWARE THREATS};
  \node[draw=OceanBlue!40, fill=SoftBlue!40, rounded corners=2pt, minimum width=2.6cm, minimum height=0.55cm, font=\tiny\bfseries\color{DarkNavy}] at (-8.6, 0.0) {LIFECYCLE STAGE};
  \node[draw=MintGreen!50, fill=SoftGreen!40, rounded corners=2pt, minimum width=2.6cm, minimum height=0.55cm, font=\tiny\bfseries\color{EmeraldGreen!90!black}] at (-8.6, -2.5) {VERIFICATION \& DEF};

  % =========================================================================
  % STAGES (CENTER ROW)
  % =========================================================================
  % Stage 1: Design
  \node[draw=BorderGrey, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.0cm] (s1) at (-6.2, 0.0) {
  ...
  % Stage 2: Fabrication
  \node[draw=BorderGrey, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.0cm] (s2) at (-3.1, 0.0) {
    \begin{minipage}{2.65cm}\centering
  ...
  % Stage 3: Assembly
  \node[draw=BorderGrey, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.0cm] (s3) at (0.0, 0.0) {
    \begin{minipage}{2.65cm}\centering
  ...
  \node[draw=BorderGrey, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.0cm] (s4) at (3.1, 0.0) {
    \begin{minipage}{2.65cm}\centering
  ...
  % Stage 5: Deployment
  \node[draw=BorderGrey, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.0cm] (s5) at (6.2, 0.0) {
    \begin{minipage}{2.65cm}\centering
  ...
  \draw[->, line width=1.4pt, DeepBlue] (s1.east) -- (s2.west);
  \draw[->, line width=1.4pt, DeepBlue] (s2.east) -- (s3.west);
  \draw[->, line width=1.4pt, DeepBlue] (s3.east) -- (s4.west);
  \draw[->, line width=1.4pt, DeepBlue] (s4.east) -- (s5.west);

  % =========================================================================
  % THREATS (TOP ROW)
  % =========================================================================
  % Threat 1
  \node[draw=CoralRed!60, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.5cm] (t1) at (-6.2, 2.5) {
    \begin{minipage}{2.65cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut DESIGN THREATS}}\\[3pt]
  ...
  % Threat 2
  \node[draw=CoralRed!60, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.5cm] (t2) at (-3.1, 2.5) {
    \begin{minipage}{2.65cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut FAB THREATS}}\\[3pt]
  ...
  % Threat 3
  \node[draw=CoralRed!60, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.5cm] (t3) at (0.0, 2.5) {
    \begin{minipage}{2.65cm}\raggedright
      \colorbox{SoftRed}{\textbf{\fontsize{5pt}{6pt}\selectfont\color{CrimsonRed}\strut ASSEMBLY THREATS}}\\[3pt]
  ...
  % Threat 4
  \node[draw=CoralRed!60, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.5cm] (t4) at (3.1, 2.5) {
    \begin{minipage}{2.65cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut TRANSIT THREATS}}\\[3pt]
  ...
  % Threat 5
  \node[draw=CoralRed!60, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.5cm] (t5) at (6.2, 2.5) {
    \begin{minipage}{2.65cm}\raggedright
      \colorbox{SoftRed}{\textbf{\tiny\color{CrimsonRed}\strut FIELD THREATS}}\\[3pt]
  ...
  % Vertical red arrows
  \draw[->, thick, CrimsonRed] (t1.south) -- (s1.north);
  \draw[->, thick, CrimsonRed] (t2.south) -- (s2.north);
  \draw[->, thick, CrimsonRed] (t3.south) -- (s3.north);
  \draw[->, thick, CrimsonRed] (t4.south) -- (s4.north);
  \draw[->, thick, CrimsonRed] (t5.south) -- (s5.north);

  % =========================================================================
  % ASSURANCE & DEFENSE (BOTTOM ROW)
  % =========================================================================
  % Def 1
  \node[draw=MintGreen!70, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.5cm] (d1) at (-6.2, -2.5) {
    \begin{minipage}{2.65cm}\raggedright
  ...
  % Def 2
  \node[draw=MintGreen!70, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.5cm] (d2) at (-3.1, -2.5) {
    \begin{minipage}{2.65cm}\raggedright
      \colorbox{SoftGreen}{\textbf{\tiny\color{EmeraldGreen!90!black}\strut ASSURANCE}}\\[3pt]
  ...
  % Def 3
  \node[draw=MintGreen!70, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.5cm] (d3) at (0.0, -2.5) {
    \begin{minipage}{2.65cm}\raggedright
      \colorbox{SoftGreen}{\textbf{\tiny\color{EmeraldGreen!90!black}\strut ASSURANCE}}\\[3pt]
  ...
  % Def 4
  \node[draw=MintGreen!70, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.5cm] (d4) at (3.1, -2.5) {
    \begin{minipage}{2.65cm}\raggedright
      \colorbox{SoftGreen}{\textbf{\tiny\color{EmeraldGreen!90!black}\strut ASSURANCE}}\\[3pt]
  ...
  % Def 5
  \node[draw=MintGreen!70, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=2.85cm, minimum height=2.5cm] (d5) at (6.2, -2.5) {
    \begin{minipage}{2.65cm}\raggedright
      \colorbox{SoftGreen}{\textbf{\tiny\color{EmeraldGreen!90!black}\strut ASSURANCE}}\\[3pt]
  ...
  \draw[->, thick, EmeraldGreen] (s1.south) -- (d1.north);
  \draw[->, thick, EmeraldGreen] (s2.south) -- (d2.north);
  \draw[->, thick, EmeraldGreen] (s3.south) -- (d3.north);
  \draw[->, thick, EmeraldGreen] (s4.south) -- (d4.north);
  \draw[->, thick, EmeraldGreen] (s5.south) -- (d5.north);
...
In the machine learning domain, this understanding becomes essential because ML amplifies both defensive and offensive capabilities. The same computational advantages that make ML effective for legitimate applications---pattern recognition, automation, and scalability---also enhance adversarial capabilities. Examining how machine learning can be weaponized allows security professionals to anticipate attack vectors, design defenses, and develop detection mechanisms.
...
### Case study: Deep learning for side-channel analysis {#sec-security-privacy-case-study-deep-learning-sca-b0b3}
...
\begin{tikzpicture}[
  ...
  % =========================================================================
  \draw[->, dotted, line width=1.4pt, DarkNavy] (-8.6, -5.6) -- (-8.6, 5.8);
  ...
  % =========================================================================
  \draw[draw=PurpleTheme!50, fill=white, rounded corners=4pt, line width=0.9pt] (-7.6, 3.2) rectangle (8.2, 5.7);

  % Left ID Box
  \draw[draw=PurpleTheme!30, fill=SoftPurple, rounded corners=3pt] (-7.4, 3.35) rectangle (-5.2, 5.55);
  ...
  % 4 Cards
  \node[draw=PurpleTheme!30, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (-3.45, 4.45) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  \node[draw=PurpleTheme!30, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (-0.15, 4.45) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  \node[draw=PurpleTheme!30, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (3.15, 4.45) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  \node[draw=PurpleTheme!30, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (6.45, 4.45) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  % =========================================================================
  \draw[draw=OrangeBorder!60, fill=white, rounded corners=4pt, line width=0.9pt] (-7.6, 0.4) rectangle (8.2, 2.9);

  % Left ID Box
  \draw[draw=OrangeBorder!30, fill=SoftOrange!40, rounded corners=3pt] (-7.4, 0.55) rectangle (-5.2, 2.75);
  ...
  % 4 Cards
  \node[draw=OrangeBorder!40, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (-3.45, 1.65) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  \node[draw=OrangeBorder!40, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (-0.15, 1.65) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  \node[draw=OrangeBorder!40, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (3.15, 1.65) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  \node[draw=OrangeBorder!40, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (6.45, 1.65) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  % =========================================================================
  \draw[draw=OceanBlue!50, fill=white, rounded corners=4pt, line width=0.9pt] (-7.6, -2.4) rectangle (8.2, 0.1);

  % Left ID Box
  \draw[draw=OceanBlue!30, fill=SoftBlue!40, rounded corners=3pt] (-7.4, -2.25) rectangle (-5.2, -0.05);
  ...
  % 4 Cards
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (-3.45, -1.15) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (-0.15, -1.15) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (3.15, -1.15) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  \node[draw=OceanBlue!35, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (6.45, -1.15) {
    \begin{minipage}{2.9cm}\raggedright
  ...
  % =========================================================================
  \draw[draw=BorderGrey, fill=white, rounded corners=4pt, line width=0.9pt] (-7.6, -5.2) rectangle (8.2, -2.7);

  % Left ID Box
  \draw[draw=BorderGrey!70, fill=SoftGrey, rounded corners=3pt] (-7.4, -5.05) rectangle (-5.2, -2.85);
  \node[fill=SoftGrey!80, rounded corners=2pt, minimum width=1.8cm, minimum height=0.35cm, font=\tiny\bfseries\color{SlateGrey}] at (-6.3, -3.1) {LAYER 1};
  \node[font=\small\bfseries\color{DarkNavy}, align=center] at (-6.3, -3.65) {Silicon\\Hardware};
  \node[font=\tiny\bfseries\color{DarkNavy}] at (-6.3, -4.15) {Root of Trust};
  \node[font=\tiny\color{SlateGrey}] at (-6.3, -4.6) {Hardware Anchor};

  % 4 Cards
  \node[draw=BorderGrey!70, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (-3.45, -3.95) {
    \begin{minipage}{2.9cm}\raggedright
      ...
      \textbf{\tiny\color{DarkNavy}Host Hypervisor Blind}
    \end{minipage}
  };

  \node[draw=BorderGrey!70, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (-0.15, -3.95) {
    \begin{minipage}{2.9cm}\raggedright
      ...
      \textbf{\tiny\color{DarkNavy}Root of Trust Chain}
    \end{minipage}
  };

  \node[draw=BorderGrey!70, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (3.15, -3.95) {
    \begin{minipage}{2.9cm}\raggedright
      ...
      \textbf{\tiny\color{DarkNavy}Hardware Key Vault}
    \end{minipage}
  };

  \node[draw=BorderGrey!70, fill=white, rounded corners=3pt, minimum width=3.1cm, minimum height=2.2cm] at (6.45, -3.95) {
    \begin{minipage}{2.9cm}\raggedright
      ...
      \textbf{\tiny\color{DarkNavy}Unforgeable Identity}
    \end{minipage}
  };
...
While differential privacy adds mathematical guarantees to data processing, FL offers a complementary approach that reduces privacy risks by restructuring the learning process itself. @Sec-edge-intelligence-federated-learning-6e7e examines the FL protocols and weighted aggregation mechanisms that provide structural privacy protection by keeping data localized on client devices. However, FL alone does not guarantee privacy. The gradient updates that clients transmit can leak sensitive information, making FL a necessary but insufficient component of a comprehensive privacy strategy.

#### Gradient leakage and privacy-preserving computation {#sec-security-privacy-gradient-inversion-risks-d2c1}

**Gradient Inversion Attacks**\index{Gradient Inversion Attack} (such as Deep Leakage from Gradients\index{Deep Leakage from Gradients}) demonstrate that an adversary with access to a client's raw update can reconstruct the original training data (for example, images or text) with high fidelity [@zhu2019deep; @nasr2019]. The gradient vector encodes information about the data that produced it, creating a channel for information leakage even when raw data never leaves the device. The defense against this vulnerability is secure aggregation, a cryptographic protocol that ensures the server can compute the aggregate update without ever observing any individual client's contribution. @Fig-secure-aggregation details this protocol: edge participants generate pairwise zero-sum masks that cancel upon summation at the server, hiding individual updates while preserving the exact aggregate.

::: {#fig-secure-aggregation fig-env="figure" fig-pos="htb" fig-cap="**Secure aggregation protocol**: Edge clients compute local gradient vectors and exchange pairwise zero-sum random masks ($s_{i,j} = -s_{j,i}$) over Diffie-Hellman key exchange. Each client transmits only its masked vector to the central aggregator. When summed across the fleet, the masks cancel identically to zero, allowing the server to reconstruct the exact fleet gradient aggregate without learning any individual participant's update." fig-alt="Four-step secure aggregation protocol diagram. Step 1: Clients compute local gradients. Step 2: Clients add pairwise zero-sum masks. Step 3: Masked gradients transmitted over TLS. Step 4: Central server aggregates updates where masks cancel to zero, preserving individual client privacy."}
...
\begin{tikzpicture}[
  ...
  % =========================================================================
  \draw[draw=OceanBlue!40, fill=white, rounded corners=4pt, line width=0.8pt] (-8.6, 3.4) rectangle (8.6, 5.35);
  \node[fill=SoftBlue, rounded corners=2pt, minimum width=1.6cm, minimum height=0.35cm, font=\tiny\bfseries\color{OceanBlue}] at (-7.6, 5.05) {STEP 1};
  \node[font=\scriptsize\bfseries\color{DarkNavy}, anchor=west] at (-6.6, 5.05) {Local Gradient Computation on Private Data};
  \node[font=\tiny\color{SlateGrey}, anchor=west] at (0.2, 5.05) {Each participant computes raw gradient vector $g_i$ locally; raw data never leaves device};

  % 4 Client Cards
  \node[draw=BorderGrey!80, fill=white, rounded corners=3pt, minimum width=3.95cm, minimum height=0.95cm] at (-6.2, 4.15) {
  ...
  \node[draw=BorderGrey!80, fill=white, rounded corners=3pt, minimum width=3.95cm, minimum height=0.95cm] at (-2.1, 4.15) {
    \begin{minipage}{3.7cm}\raggedright
  ...
  \node[draw=BorderGrey!80, fill=white, rounded corners=3pt, minimum width=3.95cm, minimum height=0.95cm] at (2.0, 4.15) {
    \begin{minipage}{3.7cm}\raggedright
  ...
  \node[draw=BorderGrey!80, fill=white, rounded corners=3pt, minimum width=3.95cm, minimum height=0.95cm] at (6.1, 4.15) {
    \begin{minipage}{3.7cm}\raggedright
  ...
  % =========================================================================
  \draw[draw=OrangeBorder!60, fill=SoftOrange!10, rounded corners=4pt, line width=0.8pt] (-8.6, 1.1) rectangle (8.6, 3.2);
  \node[fill=SoftOrange, rounded corners=2pt, minimum width=1.6cm, minimum height=0.35cm, font=\tiny\bfseries\color{WarmOrange}] at (-7.6, 2.9) {STEP 2};
  \node[font=\scriptsize\bfseries\color{DarkNavy}, anchor=west] at (-6.6, 2.9) {Diffie-Hellman Pairwise Mask Agreement};
  \node[font=\tiny\color{SlateGrey}, anchor=west] at (0.0, 2.9) {Pairwise mask invariant: $s_{i,j} = -s_{j,i}$. Total mask sum cancels: $\sum r_i = \sum \sum s_{i,j} = 0$};

  % 4 Masked Cards
  \node[draw=OrangeBorder!60, fill=white, rounded corners=3pt, minimum width=3.95cm, minimum height=1.35cm] at (-6.2, 1.95) {
    \begin{minipage}{3.7cm}\raggedright
      \textbf{\tiny\color{WarmOrange}Client 1 + Mask ($r_1$)}\\[1pt]
      \tiny\color{SlateGrey}$r_1 = [+1.4, +0.9, -0.8]$\\[1pt]
      \textbf{\scriptsize\color{DarkNavy}$y_1 = [+1.7, +0.8, -0.3]$}
    \end{minipage}
  };

  \node[draw=OrangeBorder!60, fill=white, rounded corners=3pt, minimum width=3.95cm, minimum height=1.35cm] at (-2.1, 1.95) {
    \begin{minipage}{3.7cm}\raggedright
      \textbf{\tiny\color{WarmOrange}Client 2 + Mask ($r_2$)}\\[1pt]
      \tiny\color{SlateGrey}$r_2 = [-0.9, +0.7, +0.8]$\\[1pt]
      \textbf{\scriptsize\color{DarkNavy}$y_2 = [-0.8, +1.1, +0.6]$}
    \end{minipage}
  };

  \node[draw=OrangeBorder!60, fill=white, rounded corners=3pt, minimum width=3.95cm, minimum height=1.35cm] at (2.0, 1.95) {
    \begin{minipage}{3.7cm}\raggedright
      \textbf{\tiny\color{WarmOrange}Client 3 + Mask ($r_3$)}\\[1pt]
      \tiny\color{SlateGrey}$r_3 = [+0.7, -0.7, +1.1]$\\[1pt]
      \textbf{\scriptsize\color{DarkNavy}$y_3 = [+0.5, -0.4, +1.2]$}
    \end{minipage}
  };

  \node[draw=OrangeBorder!60, fill=white, rounded corners=3pt, minimum width=3.95cm, minimum height=1.35cm] at (6.1, 1.95) {
    \begin{minipage}{3.7cm}\raggedright
      \textbf{\tiny\color{WarmOrange}Client 4 + Mask ($r_4$)}\\[1pt]
      \tiny\color{SlateGrey}$r_4 = [-1.2, -0.9, -1.1]$\\[1pt]
  ...
  % =========================================================================
  \draw[draw=OceanBlue!40, fill=white, rounded corners=4pt, line width=0.8pt] (-8.6, -0.6) rectangle (8.6, 0.9);
  \node[fill=SoftBlue, rounded corners=2pt, minimum width=1.6cm, minimum height=0.35cm, font=\tiny\bfseries\color{OceanBlue}] at (-7.6, 0.6) {STEP 3};
  \node[font=\scriptsize\bfseries\color{DarkNavy}, anchor=west] at (-6.6, 0.6) {Masked Transmission over Encrypted TLS Transport};
  \node[font=\tiny\color{SlateGrey}, anchor=west] at (0.2, 0.6) {Server observes only randomized pseudo-noise vectors $y_i$; individual $g_i$ remain strictly hidden};

  % 4 Transmit Badges
  \node[draw=OrangeBorder!70, fill=SoftOrange!30, rounded corners=3pt, minimum width=3.9cm, minimum height=0.6cm] at (-6.3, 0.0) {
    \fontsize{7pt}{8.5pt}\selectfont\textbf{\color{DarkNavy}Transmit $y_1 = [+1.7, +0.8, -0.3]\;\rightarrow$}
  };
  \node[draw=OrangeBorder!70, fill=SoftOrange!30, rounded corners=3pt, minimum width=3.9cm, minimum height=0.6cm] at (-2.1, 0.0) {
    \fontsize{7pt}{8.5pt}\selectfont\textbf{\color{DarkNavy}Transmit $y_2 = [-0.8, +1.1, +0.6]\;\rightarrow$}
  };
  \node[draw=OrangeBorder!70, fill=SoftOrange!30, rounded corners=3pt, minimum width=3.9cm, minimum height=0.6cm] at (2.1, 0.0) {
    \fontsize{7pt}{8.5pt}\selectfont\textbf{\color{DarkNavy}Transmit $y_3 = [+0.5, -0.4, +1.2]\;\rightarrow$}
  };
  \node[draw=OrangeBorder!70, fill=SoftOrange!30, rounded corners=3pt, minimum width=3.9cm, minimum height=0.6cm] at (6.3, 0.0) {
    \fontsize{7pt}{8.5pt}\selectfont\textbf{\color{DarkNavy}Transmit $y_4 = [-1.0, -1.2, -0.7]\;\rightarrow$}
  };

  % =========================================================================
  % STEP 4: CENTRAL SERVER AGGREGATION
  % =========================================================================
  \draw[draw=MintGreen!70, fill=SoftGreen!15, rounded corners=4pt, line width=0.8pt] (-8.6, -2.3) rectangle (8.6, -0.8);
  \node[fill=SoftGreen, rounded corners=2pt, minimum width=1.6cm, minimum height=0.35cm, font=\tiny\bfseries\color{EmeraldGreen!90!black}] at (-7.6, -1.1) {STEP 4};
  \node[font=\scriptsize\bfseries\color{EmeraldGreen!90!black}, anchor=west] at (-6.6, -1.1) {Central Server Aggregation (Zero-Knowledge Sum)};
  \node[font=\tiny\color{SlateGrey}, anchor=west] at (0.2, -1.1) {Server sums received vectors: masks cancel out algebraically to zero};

  % Equation Box
  \node[draw=BorderGrey!70, fill=white, rounded corners=3pt, minimum width=16.8cm, minimum height=0.7cm] at (0.0, -1.75) {
  ...
  \draw[draw=MintGreen!70, fill=SoftGreen!15, rounded corners=4pt, line width=0.8pt] (-8.6, -4.3) rectangle (-0.2, -2.55);
  \node[fill=SoftGreen, rounded corners=2pt, minimum width=3.8cm, minimum height=0.4cm, font=\tiny\bfseries\color{EmeraldGreen!90!black}, anchor=north west] at (-8.4, -2.7) {WHAT THE SERVER LEARNS};

  \node[anchor=north west] at (-8.4, -3.2) {
    \begin{minipage}{7.8cm}\raggedright
      \textbf{\scriptsize\color{DarkNavy}Exact Fleet Aggregate: $\sum g_i = [+0.4, +0.3, +0.8]$}\\[2pt]
      \tiny\textbf{\color{DarkNavy}Accuracy:} \color{DarkNavy}Global weights update cleanly with zero mathematical noise\\[1pt]
      \tiny\textbf{\color{DarkNavy}DP Synergy:} \color{DarkNavy}Directly compatible with central differential privacy
    \end{minipage}
  };

  % Right Card: What Server Cannot Learn
  \draw[draw=CoralRed!70, fill=SoftRed!20, rounded corners=4pt, line width=0.8pt] (0.2, -4.3) rectangle (8.6, -2.55);
  \node[fill=SoftRed, rounded corners=2pt, minimum width=4.5cm, minimum height=0.4cm, font=\tiny\bfseries\color{CrimsonRed}, anchor=north west] at (0.4, -2.7) {WHAT THE SERVER CANNOT LEARN};

  \node[anchor=north west] at (0.4, -3.2) {
    \begin{minipage}{7.8cm}\raggedright
      \textbf{\scriptsize\color{CrimsonRed}Zero Knowledge of Individual Client Updates}\\[2pt]
      \tiny\textbf{\color{DarkNavy}Privacy:} \color{DarkNavy}DLG gradient inversion attacks are mathematically defeated\\[1pt]
      \tiny\textbf{\color{DarkNavy}Robustness:} \color{DarkNavy}$t$-of-$n$ Shamir secret sharing tolerates device dropouts
    \end{minipage}
  };
...
This vulnerability drives the need for secure aggregation protocols [@bonawitz2017] and differential privacy, but the trust boundary determines which version of DP applies. **Central Differential Privacy**\index{Differential Privacy!central} assumes a trusted curator or aggregation service can see raw records or raw updates before releasing a noisy aggregate. **Local Differential Privacy**\index{Differential Privacy!local} randomizes each client contribution before any server sees it, protecting against a stronger adversary but usually paying a larger utility cost. Secure aggregation sits between those regimes: the server sees an aggregate rather than individual updates, and DP noise can then be applied so the released aggregate reveals bounded information about any participant. Interestingly, the gradient compression techniques examined in @sec-communication-collective-operations-collective-operations-gradient-compression-7a5c for communication efficiency can also affect privacy properties: compressed gradients may leak less information than full-precision updates, though the interaction between compression and privacy guarantees requires careful analysis. Federated deployments therefore layer FL, secure aggregation, and the appropriate DP threat model to achieve formal privacy guarantees beyond structural protection alone. Google Gboard's federated learning deployment exemplifies this layered approach.
...
1. **Distribution learning**: A generative model $G_\theta$ trains on real data $\mathcal{S}_{\text{real}} = \{x_1, x_2,\ldots, x_n\}$ to learn the data distribution $p(x)$.
2. **Synthetic sampling**: New samples $\mathcal{S}_{\text{synthetic}} = \{G_\theta(z_1), G_\theta(z_2),\ldots, G_\theta(z_m)\}$ are generated by sampling from random noise $z_i \sim \mathcal{N}(0,I)$.
...
Training generative models on real datasets and sampling from the learned distribution can create datasets that approximate selected statistical properties without directly copying the source table [@goncalves2020]. This transformation is not itself a privacy guarantee: synthetic records still require memorization, linkage, and re-identification evaluation against the deployment threat model.
...
Deployment environments must also enforce strong access control policies to ensure that only authorized users and services can interact with inference endpoints. Authentication protocols, including OAuth[^fn-oauth-token-authentication] tokens, mutual TLS,[^fn-mutual-tls-authentication] or API keys,[^fn-api-key-leakage] should be combined with role-based access control (RBAC)[^fn-rbac-access-control] to restrict access according to user roles and operational context. Hosted model APIs commonly authenticate requests with provider-issued API keys, service tokens, or mutually authenticated service identities. These controls are not merely billing or availability mechanisms: strong identity verification is a prerequisite for the rate-limiting and anomaly-detection defenses described in @sec-security-privacy-model-extraction-defenses-8f3b. Without it, an attacker conducting model extraction can execute a Sybil attack---querying the endpoint through thousands of distinct, unauthenticated sessions---and defeat per-identity rate limits entirely. Every OAuth token, mTLS certificate, or API key converts an anonymous query stream into an attributable identity, making systematic extraction economically and operationally detectable.

[^fn-oauth-token-authentication]: **OAuth (Open Authorization)**\index{OAuth!ML API authentication}: Standard developed in 2006 and used at internet scale, enabling API access without exposing credentials. For ML inference APIs, OAuth 2.0 token-based authentication adds 5--15 ms of latency per request for token validation, a negligible cost for batch inference but a meaningful overhead for real-time serving at thousands of requests per second where every millisecond of latency budget matters.

[^fn-mutual-tls-authentication]: **Mutual TLS (mTLS)**\index{Mutual TLS!ML serving}: Enhanced Transport Layer Security (introduced 1999) where both client and server authenticate via certificates. The 15--30 ms handshake overhead is a one-time cost per connection, amortized across subsequent requests via connection pooling. For ML model serving, mTLS ensures that only authenticated services can submit inference requests, preventing unauthorized model extraction through API access.

[^fn-api-key-leakage]: **API Keys**\index{API Keys!credential leakage}: Secret tokens passed in HTTP headers to authenticate API callers. In ML serving pipelines, a leaked API key grants an adversary unrestricted query access to execute model extraction or data poisoning, bypassing per-IP rate limits and converting a credential leak directly into IP theft.
...
An API key authenticates the client and allows the backend to enforce usage policies, monitor for abuse, and log access patterns. Secure implementations retrieve API keys from environment variables rather than hardcoding them into source code, preventing credential exposure in version control systems or application logs. Such key-based access control mechanisms are simple to implement but require careful key management and monitoring to prevent misuse, unauthorized access, or model extraction. Additional security measures in production deployments often include model integrity verification through SHA-256 hash checking, rate limiting to prevent abuse, input validation for size and format constraints, and comprehensive logging for security event tracking.
...
Even when inputs pass validation, adversarial or unexpected behavior may still emerge at the model's output. Output monitoring\index{Output Monitoring} is a containment mechanism, not a prevention mechanism: it does not stop exploitation but detects its symptoms in the model's predictions and triggers a fallback before a wrong output propagates. These mechanisms observe confidence, prediction entropy, class distribution, or response patterns to flag deviations from expected behavior.
...
[^fn-attention-map-saliency]: **Attention Maps**\index{Attention!maps, security monitoring}: Visualization of attention weights, building on the neural attention mechanism introduced by @bahdanau2015neural. In monitoring contexts, attention-style visualizations and related attribution signals can help analysts inspect which input regions or tokens influenced a model output, but they should be treated as diagnostic evidence rather than a standalone security guarantee.
...
One of the most common integrity mechanisms is cryptographic model verification. Before a model is loaded into memory, the system can compute a cryptographic hash (for example, SHA-256)[^fn-sha256-hash-security] of the model file and compare it against a known-good signature.
...
For example, in a regulated healthcare ML deployment,[^fn-healthcare-ml-compliance] integrity checks might include three gates:
...
These checks ensure compliance with regulations like HIPAA[^fn-hipaa-ml-requirements]'s integrity requirements and GDPR's accountability principle, limit the risk of silent failures, and create a forensic trail in case of audit or breach.
...
Hardware security mechanisms introduce measurable overhead that must be factored into system design. ARM TrustZone world-switching adds approximately `{python} TeeOverheadSpecs.tz_cycles_low_str`--`{python} TeeOverheadSpecs.tz_cycles_high_str` cycles depending on processor generation and cache state (`{python} TeeOverheadSpecs.tz_us_low_str`--`{python} TeeOverheadSpecs.tz_us_high_str` $\mu$s at 500 MHz) of latency per transition between secure and nonsecure worlds. Cryptographic operations in secure mode typically consume `{python} TeeOverheadSpecs.tz_power_low_str`--`{python} TeeOverheadSpecs.tz_power_high_str` percent additional power compared to normal execution, impacting battery life in mobile ML applications. Intel SGX context switching imposes `{python} TeeOverheadSpecs.sgx_ctx_low_str`--`{python} TeeOverheadSpecs.sgx_ctx_high_str` $\mu$s overhead per inference, representing 2 percent energy overhead for typical edge ML workloads.
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_memory_capacity, fmt_percent, fmt_qty, fmt_time
...
3. **Encrypted compute (fully homomorphic encryption [FHE])**: Inference performed on encrypted data.
...
\vspace*{-44mm}
```
![](images/svg/vol2_security_privacy_margin_002.svg){width="100%" fig-alt="Three gray latency bars on a log scale: FHE 200 s is much longer than AES 20.5 ms and Plain 20 ms."}
...
A TEE is first a trust-boundary decision: which parts of the model, key material, input data, or attestation path must remain protected even if the surrounding host is compromised. A **Trusted Execution Environment (TEE)**\index{Trusted Execution Environment}[^fn-tee-enclave-execution] is the hardware-isolated processor region that implements that boundary. TEEs enforce confidentiality, integrity, and runtime isolation, ensuring that even if the host operating system or application layer is attacked, sensitive operations within the TEE remain secure.
...
\providecolor{socborder}{RGB}{140,165,190}
\providecolor{normalbg}{RGB}{255,255,255}
\providecolor{normalborder}{RGB}{210,220,230}
\providecolor{dangerbg}{RGB}{254,242,242}
\providecolor{dangerborder}{RGB}{248,180,180}
\providecolor{dangertxt}{RGB}{185,28,28}
\providecolor{amberbg}{RGB}{254,250,238}
\providecolor{amberborder}{RGB}{245,200,120}
\providecolor{ambertxt}{RGB}{160,95,10}
\providecolor{enclaveborder}{RGB}{124,58,237}
\providecolor{enclavetxt}{RGB}{109,40,217}
\providecolor{enclavetint}{RGB}{245,240,255}
\providecolor{greenbg}{RGB}{236,253,245}
\providecolor{greenborder}{RGB}{110,231,183}
\providecolor{greentxt}{RGB}{4,120,87}
\providecolor{bluebg}{RGB}{239,246,255}
\providecolor{blueborder}{RGB}{147,197,253}
\providecolor{bluetxt}{RGB}{29,78,216}
\providecolor{phybg}{RGB}{250,252,255}
\providecolor{phyborder}{RGB}{190,205,225}
...
% ====================================================
\draw[fill=socbg, draw=socborder, line width=1pt, rounded corners=8pt] (-0.3, -7.6) rectangle (11.7, 4.6);
\node[fill=blueborder!30, draw=socborder, rounded corners=3pt, font=\sffamily\bfseries\footnotesize, text=socborder!90!black, inner sep=3.5pt]
    at (1.8, 4.25) {SYSTEM ON CHIP (SoC)};

% ====================================================
% 1. NORMAL WORLD (Untrusted)
% ====================================================
\draw[fill=white, draw=normalborder, line width=0.8pt, rounded corners=5pt] (0, 0.3) rectangle (7.4, 3.9);
\node[anchor=north west, inner sep=0pt] at (0.2, 3.7) {
    \textbf{\textcolor{dangertxt}{\footnotesize NORMAL WORLD (Untrusted)}}\quad \textcolor{gray!70!black}{\scriptsize Host OS \& Hypervisor}
};

% Application Processor (AP) container
\draw[fill=white, draw=normalborder!80, line width=0.5pt, rounded corners=4pt] (0.15, 1.55) rectangle (7.25, 3.35);
\node[anchor=north west, font=\sffamily\bfseries\scriptsize, text=black!80] at (0.3, 3.25) {Application Processor (AP)};

% 3 Sub-boxes inside AP
\node[draw=normalborder, fill=bluebg!20, rounded corners=3pt, inner sep=2.5pt, text width=2.1cm, align=center]
    at (1.35, 2.25) {
    {\scriptsize\bfseries CPU Core Cluster}\\[1pt]
    {\tiny\textcolor{gray!80!black}{Linux / OS Kernel}}
};

\node[draw=normalborder, fill=bluebg!20, rounded corners=3pt, inner sep=2.5pt, text width=2.1cm, align=center]
    at (3.7, 2.25) {
    {\scriptsize\bfseries Neural Engine / GPU}\\[1pt]
    {\tiny\textcolor{bluetxt}{ML Inference Pipeline}}
};

\node[draw=dangerborder, fill=dangerbg, rounded corners=3pt, inner sep=2.5pt, text width=2.1cm, align=center]
    at (6.05, 2.25) {
    {\scriptsize\bfseries\textcolor{dangertxt}{Untrusted Domain}}\\[1pt]
    {\tiny\textcolor{dangertxt}{Rootkits / DMA Exploits}}
};

% AXI / NoC System Fabric
\node[draw=blueborder!50, fill=bluebg!15, rounded corners=3pt, inner sep=3pt, text width=3.8cm, anchor=north west]
    at (0.15, 1.35) {
    {\scriptsize\bfseries AXI / NoC System Fabric}\\[1pt]
    {\tiny\textcolor{gray!80!black}{Direct DMA access to shared peripherals\\[-0.5pt]Untrusted memory routing}}
};

% Hardware Mailbox
\node[draw=amberborder, fill=amberbg, rounded corners=3pt, inner sep=3pt, text width=2.6cm, anchor=north west, align=center]
    at (4.45, 1.35) {
    {\scriptsize\bfseries\textcolor{ambertxt}{Hardware Mailbox}}\\[1pt]
    {\tiny\textcolor{ambertxt}{Register FIFO\\[-0.5pt]Opaque Descriptors}}
};

% ====================================================
% 2. SECURE ENCLAVE (TEE)
% ====================================================
\draw[fill=white, draw=enclaveborder, dashed, line width=1pt, rounded corners=6pt] (0, -7.3) rectangle (7.4, -0.2);
\node[fill=enclavetint, draw=enclaveborder!50, rounded corners=3pt, font=\sffamily\bfseries\scriptsize, text=enclavetxt, inner sep=2.5pt]
    at (1.6, -0.5) {SECURE ENCLAVE (TEE)};
\node[anchor=west, font=\sffamily\bfseries\scriptsize, text=enclavetxt] at (3.2, -0.5) {Hardware Isolated Boundary};

% Secure Processor Core
\draw[draw=enclaveborder!40, fill=white, rounded corners=4pt] (0.15, -2.8) rectangle (3.5, -0.9);
\node[fill=enclavetint, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=enclavetxt]
    at (1.82, -1.2) {Secure Processor Core};
\node[inner sep=2pt, text width=3.1cm, align=center] at (1.82, -2.0) {
    {\scriptsize\bfseries Dedicated CPU + sepOS}\\[2pt]
    {\tiny\textcolor{gray!80!black}{Isolated SRAM \& Boot ROM\\[0.5pt]JTAG permanently blown}}
};

% Hardware Accelerators
\draw[draw=enclaveborder!40, fill=white, rounded corners=4pt] (3.75, -2.8) rectangle (7.25, -0.9);
\node[font=\sffamily\bfseries\scriptsize, text=enclavetxt] at (5.5, -1.2) {Hardware Accelerators};

\node[fill=bluebg!30, draw=blueborder!50, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=bluetxt, text width=1.5cm, align=center] at (4.65, -1.75) {TRNG (True RNG)};
\node[fill=bluebg!30, draw=blueborder!50, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=bluetxt, text width=1.5cm, align=center] at (6.35, -1.75) {PKA (RSA / ECC)};
\node[fill=bluebg!30, draw=blueborder!50, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=bluetxt, text width=1.5cm, align=center] at (4.65, -2.35) {AES-256-GCM};
\node[fill=bluebg!30, draw=blueborder!50, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=bluetxt, text width=1.5cm, align=center] at (6.35, -2.35) {SHA-256 Engine};

% Silicon Key Vault (RoT)
\draw[draw=greenborder, fill=white, line width=0.7pt, rounded corners=4pt] (0.15, -7.0) rectangle (7.25, -3.1);
\node[fill=greenbg, draw=greenborder, rounded corners=2pt, font=\sffamily\bfseries\tiny, text=greentxt, inner sep=2pt]
    at (1.4, -3.4) {Silicon Key Vault (RoT)};
\node[font=\sffamily\bfseries\tiny, text=greentxt, anchor=west] at (2.6, -3.4) {Hardware Non-Exportable Key Anchors};

% 3 Sub-boxes inside Key Vault
\node[draw=greenborder!50, fill=greenbg!25, rounded corners=3pt, inner sep=3pt, text width=2.0cm, align=center]
    at (1.35, -5.2) {
    {\scriptsize\bfseries Unique Chip ID (UID)}\\[3pt]
    {\tiny\textcolor{gray!80!black}{Fused into OTP silicon}}\\[2pt]
    {\tiny\bfseries\textcolor{greentxt}{Never leaves enclave}}
};

\node[draw=greenborder!50, fill=white, rounded corners=3pt, inner sep=3pt, text width=2.0cm, align=center]
    at (3.7, -5.2) {
    {\scriptsize\bfseries Device Group ID (GID)}\\[3pt]
    {\tiny\textcolor{gray!80!black}{Model class key}}\\[2pt]
    {\tiny\textcolor{gray!80!black}{Firmware attestation}}
};

\node[draw=greenborder!50, fill=white, rounded corners=3pt, inner sep=3pt, text width=2.0cm, align=center]
    at (6.05, -5.2) {
    {\scriptsize\bfseries Anti-Replay Counter}\\[3pt]
    {\tiny\textcolor{gray!80!black}{Monotonic NV counters}}\\[2pt]
    {\tiny\bfseries\textcolor{dangertxt}{Downgrade defeat}}
};

% ====================================================
% 3. MEMORY & STORAGE PHY (Middle column inside SoC)
% ====================================================
\draw[fill=white, draw=phyborder, line width=0.8pt, rounded corners=5pt] (7.7, -7.3) rectangle (11.4, 3.9);
\node[font=\sffamily\bfseries\scriptsize, text=black!85] at (9.55, 3.6) {MEMORY \& STORAGE PHY};

% DRAM Controller
\draw[draw=blueborder!70, fill=bluebg!25, rounded corners=4pt] (7.85, 1.4) rectangle (11.25, 3.25);
\node[font=\sffamily\bfseries\scriptsize, text=black!90] at (9.55, 2.95) {DRAM Controller};
\node[font=\sffamily\tiny, text=bluetxt] at (9.55, 2.6) {Dual-channel LPDDR PHY};
\node[draw=normalborder!70, fill=white, rounded corners=2pt, inner sep=2pt, text width=3.0cm, align=center]
    at (9.55, 1.95) {
    {\scriptsize\bfseries Normal Allocation Bus}\\[0.5pt]
    {\tiny\textcolor{gray!80!black}{Direct AP plain DMA}}
};

% Memory Protection (MPE)
\draw[draw=enclaveborder!70, fill=enclavetint!35, rounded corners=4pt] (7.85, -3.1) rectangle (11.25, 1.05);
\node[fill=enclavetint, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\scriptsize, text=enclavetxt]
    at (9.55, 0.75) {Memory Protection (MPE)};

\node[draw=enclaveborder!40, fill=white, rounded corners=2pt, inner sep=2pt, text width=3.0cm, align=center]
    at (9.55, 0.1) {
    {\scriptsize\bfseries\textcolor{enclavetxt}{AES-XTS Encryption}}\\[0.5pt]
    {\tiny\textcolor{gray!80!black}{Hardware line-rate scrambler}}
};

\node[draw=enclaveborder!40, fill=white, rounded corners=2pt, inner sep=2pt, text width=3.0cm, align=center]
    at (9.55, -0.9) {
    {\scriptsize\bfseries\textcolor{enclavetxt}{Integrity Merkle Tree}}\\[0.5pt]
    {\tiny\textcolor{gray!80!black}{Anti-replay MAC verification}}
};

\node[draw=enclaveborder!40, fill=white, rounded corners=2pt, inner sep=2pt, text width=3.0cm, align=center]
    at (9.55, -1.9) {
    {\scriptsize\bfseries\textcolor{enclavetxt}{Ephemeral Silicon Keys}}\\[0.5pt]
    {\tiny\textcolor{gray!80!black}{Regenerated on reboot}}
};

% NAND Flash Controller
\draw[draw=normalborder, fill=white, rounded corners=4pt] (7.85, -6.1) rectangle (11.25, -3.6);
\node[font=\sffamily\bfseries\scriptsize, text=black!90] at (9.55, -3.95) {NAND Flash Controller};
\node[font=\sffamily\tiny, text=gray!80!black] at (9.55, -4.3) {NVMe / UFS Storage PHY};
\node[draw=normalborder!70, fill=white, rounded corners=2pt, inner sep=2pt, text width=3.0cm, align=center]
    at (9.55, -5.15) {
    {\scriptsize\bfseries Inline Storage AES}\\[0.5pt]
    {\tiny\textcolor{greentxt}{\textbf{Enclave-wrapped volume keys}}}
};

\node[font=\sffamily\tiny, text=gray!70!black] at (9.55, -6.8) {SoC Memory Boundary PHY};
...
\draw[fill=socbg, draw=socborder, line width=0.8pt, rounded corners=6pt] (12.2, 0.1) rectangle (16.8, 4.6);
\node[fill=blueborder!30, draw=socborder, rounded corners=3pt, font=\sffamily\bfseries\footnotesize, text=socborder!90!black, inner sep=3pt]
    at (14.5, 4.25) {EXTERNAL DRAM (Off-Chip)};

% Host OS DRAM Area
\node[draw=normalborder, fill=white, rounded corners=4pt, inner sep=3.5pt, text width=4.0cm, align=center]
    at (14.5, 3.1) {
    {\scriptsize\bfseries Host OS DRAM Area}\\[2pt]
    {\tiny\textcolor{gray!80!black}{Unencrypted User / OS space\\[0.5pt]Kernel buffers, public models}}\\[3pt]
    \tikz\node[fill=dangerbg, draw=dangerborder, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=dangertxt]{Physically Probing Vulnerable};
};

% Enclave Encrypted Carve-Out
\node[draw=enclaveborder, fill=enclavetint!40, line width=0.9pt, rounded corners=4pt, inner sep=3.5pt, text width=4.0cm, align=center]
    at (14.5, 1.25) {
    \tikz\node[fill=enclavetint, rounded corners=2pt, inner sep=1.5pt, font=\sffamily\bfseries\tiny, text=enclavetxt]{Enclave Encrypted Carve-Out};\\[2pt]
    {\scriptsize\bfseries\textcolor{enclavetxt}{AES-XTS Ciphertext + MACs}}\\[1pt]
    {\tiny\textcolor{gray!80!black}{Protected Model Weights\\[0.5pt]Private Biometric Templates}}\\[3pt]
    \tikz\node[fill=greenbg, draw=greenborder, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=greentxt]{Hardware Isolated Trust};
};

% NAND FLASH STORAGE
\draw[fill=socbg, draw=socborder, line width=0.8pt, rounded corners=6pt] (12.2, -2.7) rectangle (16.8, -0.4);
\node[fill=blueborder!30, draw=socborder, rounded corners=3pt, font=\sffamily\bfseries\scriptsize, text=socborder!90!black, inner sep=2.5pt]
...
\node[inner sep=2pt, text width=4.2cm, align=center] at (14.5, -1.7) {
    ...
    {\tiny\textcolor{gray!80!black}{AES-CBC / XTS Volume Crypt}}\\[2.5pt]
    {\scriptsize\bfseries\textcolor{greentxt}{Cryptographically Wrapped Weights}}
};

% SECURITY GUARANTEES
\draw[fill=greenbg!40, draw=greenborder, line width=0.8pt, rounded corners=6pt] (12.2, -7.6) rectangle (16.8, -3.2);
\node[fill=greenbg, draw=greenborder, rounded corners=3pt, font=\sffamily\bfseries\scriptsize, text=greentxt, inner sep=2.5pt]
    at (14.5, -3.55) {SECURITY GUARANTEES};

\node[anchor=north west, inner sep=3pt, text width=4.2cm] at (12.35, -3.95) {
    {\scriptsize\bfseries\textcolor{greentxt}{1. Host OS Blindness}}\\[1pt]
    {\tiny\textcolor{gray!85!black}{Compromised kernel cannot read enclave registers or memory.}}\\[4pt]
    {\scriptsize\bfseries\textcolor{greentxt}{2. Cold-Boot Defense}}\\[1pt]
    {\tiny\textcolor{gray!85!black}{DRAM contents scrambled with ephemeral keys on power-cycle.}}\\[4pt]
    {\scriptsize\bfseries\textcolor{greentxt}{3. Hardware Key Sealing}}\\[1pt]
    {\tiny\textcolor{gray!85!black}{Model weights decrypted only inside enclave silicon execution.}}
};

% ====================================================
% ARROWS & INTERCONNECTS
% ====================================================
% AP to DRAM controller
\draw[->, draw=socborder!90!black, line width=1pt] (7.4, 2.35) -- (7.85, 2.35);

% DRAM controller to Host OS DRAM
\draw[->, draw=socborder!90!black, line width=1pt] (11.25, 2.95) -| (11.75, 3.1) -- (12.2, 3.1);

% Mailbox to Enclave
\draw[->, draw=ambertxt, dashed, line width=1.1pt] (5.75, 0.4) -- (5.75, -0.2);

% Enclave to MPE
\draw[->, draw=enclaveborder, line width=1.2pt] (7.4, -1.85) -| (7.6, 0.1) -- (7.85, 0.1);

% MPE to Carve-Out
\draw[->, draw=enclaveborder, line width=1.3pt] (11.25, 0.1) -| (11.75, 1.25) -- (12.2, 1.25);

% Flash Controller to NAND Flash
\draw[->, draw=greentxt, dashed, line width=1.1pt] (11.25, -5.15) -| (11.75, -1.7) -- (12.2, -1.7);
...
\providecolor{greenborder}{RGB}{110,231,183}
\providecolor{greentxt}{RGB}{4,120,87}
\providecolor{purplebg}{RGB}{245,240,255}
\providecolor{purpleborder}{RGB}{196,181,253}
\providecolor{purpletxt}{RGB}{109,40,217}
\providecolor{redbg}{RGB}{254,242,242}
\providecolor{redborder}{RGB}{248,180,180}
\providecolor{redtxt}{RGB}{185,28,28}
\providecolor{grayborder}{RGB}{209,213,219}
...
% COLUMN HEADERS
% ====================================================
\node[draw=blueborder!60, fill=bluebg, rounded corners=4pt, inner sep=4pt, text width=4.5cm, align=center, font=\sffamily\bfseries\footnotesize, text=bluetxt!80!black]
    at (2.5, 4.8) {Hardware Boot Sequence};

\node[draw=greenborder!60, fill=greenbg, rounded corners=4pt, inner sep=4pt, text width=6.2cm, align=center, font=\sffamily\bfseries\footnotesize, text=greentxt]
    at (8.9, 4.8) {Cryptographic Verification \& Measurement};

\node[draw=redborder!60, fill=redbg, rounded corners=4pt, inner sep=4pt, text width=2.4cm, align=center, font=\sffamily\bfseries\footnotesize, text=redtxt]
    at (14.2, 4.8) {Tamper Gate};

% ====================================================
% STAGE 1
% ====================================================
% Boot sequence box
\node[draw=grayborder, fill=white, rounded corners=4pt, inner sep=5pt, text width=4.6cm] (s1) at (2.5, 3.4) {
    \tikz\node[fill=gray!20, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=gray!80!black]{STAGE 1};
    \hspace{4pt}{\scriptsize\bfseries Power-On \& Mask ROM}\\[3pt]
    {\tiny\textcolor{gray!80!black}{Immutable on-chip silicon Boot ROM (RoT)\\[0.5pt]Hardware-enforced read-only execution state}}
};

% Verification box
\node[draw=greenborder, fill=white, rounded corners=4pt, inner sep=5pt, text width=6.3cm] (v1) at (8.9, 3.4) {
    \tikz\node[fill=greenbg, draw=greenborder!60, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=greentxt]{eFuse Golden Hash};
    \hspace{4pt}{\scriptsize\bfseries SHA-256 Digest Verification}\\[3pt]
    {\tiny\textcolor{gray!80!black}{Computes SHA-256 of Primary Bootloader code in SRAM\\[0.5pt]Compares against immutable golden digest fused into OTP silicon}}
};

% ====================================================
% STAGE 2
% ====================================================
\node[draw=blueborder, fill=white, rounded corners=4pt, inner sep=5pt, text width=4.6cm] (s2) at (2.5, 1.6) {
    \tikz\node[fill=bluebg, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=bluetxt]{STAGE 2};
    \hspace{4pt}{\scriptsize\bfseries Secondary Bootloader}\\[3pt]
    {\tiny\textcolor{gray!80!black}{Initializes DRAM, clocks, and security hardware\\[0.5pt]Loads Linux kernel image \& device tree to RAM}}
};

\node[draw=greenborder, fill=white, rounded corners=4pt, inner sep=5pt, text width=6.3cm] (v2) at (8.9, 1.6) {
    \tikz\node[fill=greenbg, draw=greenborder!60, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=greentxt]{PKI Signature Check};
    \hspace{4pt}{\scriptsize\bfseries RSA-4096 / ECDSA P-384}\\[3pt]
    {\tiny\textcolor{gray!80!black}{Verifies digital signature on Kernel image \& DTB binaries\\[0.5pt]Public key validated against OEM Root CA anchored in RoT}}
};

% ====================================================
% STAGE 3
% ====================================================
\node[draw=blueborder, fill=white, rounded corners=4pt, inner sep=5pt, text width=4.6cm] (s3) at (2.5, -0.2) {
    \tikz\node[fill=bluebg, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=bluetxt]{STAGE 3};
    \hspace{4pt}{\scriptsize\bfseries Linux Kernel \& Core OS}\\[3pt]
    {\tiny\textcolor{gray!80!black}{Boots verified kernel; initializes LSM \& dm-verity\\[0.5pt]Mounts authenticated root filesystem (read-only)}}
};

\node[draw=greenborder, fill=white, rounded corners=4pt, inner sep=5pt, text width=6.3cm] (v3) at (8.9, -0.2) {
    \tikz\node[fill=greenbg, draw=greenborder!60, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=greentxt]{TPM PCR \& dm-verity};
    \hspace{4pt}{\scriptsize\bfseries Measured Boot \& Hash Tree}\\[3pt]
    {\tiny\textcolor{gray!80!black}{Extends PCR[0-7] with stage hashes (tamper-evident log)\\[0.5pt]Block-level rootfs integrity verified on-the-fly via Merkle tree}}
};

% ====================================================
% STAGE 4
% ====================================================
\node[draw=purpleborder, fill=white, line width=0.8pt, rounded corners=4pt, inner sep=5pt, text width=4.6cm] (s4) at (2.5, -2.0) {
    \tikz\node[fill=purplebg, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=purpletxt]{STAGE 4};
    \hspace{4pt}{\scriptsize\bfseries ML Runtime \& Weights}\\[3pt]
    {\tiny\textcolor{purpletxt}{Spawns inference container / TEE enclave workload}}\\[1pt]
    {\tiny\textcolor{gray!80!black}{Decrypts model weights using unsealed hardware key}}
};

\node[draw=greenborder, fill=white, rounded corners=4pt, inner sep=5pt, text width=6.3cm] (v4) at (8.9, -2.0) {
    \tikz\node[fill=greenbg, draw=greenborder!60, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=greentxt]{Model Attestation};
    \hspace{4pt}{\scriptsize\bfseries Weight Signature \& Unsealing}\\[3pt]
    {\tiny\textcolor{gray!80!black}{TPM unseals model decryption key only if PCR quote matches\\[0.5pt]Verifies cryptographic model weight signature before execution}}
};

% ====================================================
% TAMPER GATE (BOOT ABORT BOX)
% ====================================================
\draw[draw=redborder, fill=white, line width=0.9pt, rounded corners=5pt] (12.8, -1.2) rectangle (15.6, 2.5);
\node[fill=redbg, rounded corners=3pt, inner sep=2.5pt, font=\sffamily\bfseries\scriptsize, text=redtxt, text width=2.4cm, align=center]
    at (14.2, 2.1) {BOOT ABORT};

\node[text width=2.5cm, align=center, inner sep=2pt] at (14.2, 1.25) {
    {\scriptsize\bfseries\textcolor{redtxt!80!black}{Integrity Failure}}\\[2pt]
    {\tiny\textcolor{redtxt}{Signature Mismatch\\[0.5pt]or Tampered Hash}}
};

\draw[draw=redborder!50, line width=0.5pt] (13.1, 0.65) -- (15.3, 0.65);

\node[text width=2.5cm, align=center, inner sep=2pt] at (14.2, 0.0) {
    {\scriptsize\bfseries\textcolor{redtxt!80!black}{Execution Halted}}\\[2pt]
    {\tiny\textcolor{redtxt}{Hardware Keys Locked}}
};

% ====================================================
% BOTTOM BAR: ROOT OF TRUST ESTABLISHED
% ====================================================
\draw[draw=greenborder, fill=greenbg!40, line width=0.8pt, rounded corners=5pt] (0.0, -4.0) rectangle (15.6, -2.8);
\node[fill=greentxt, rounded corners=3pt, inner sep=3pt, font=\sffamily\bfseries\tiny, text=white]
    at (2.1, -3.4) {ROOT OF TRUST ESTABLISHED};

\node[anchor=west, text width=11.2cm] at (3.9, -3.4) {
    {\scriptsize\bfseries Guaranteed System State: Only Authenticated Code \& Verified Model Artifacts Can Execute}\\[2pt]
    {\tiny\textcolor{gray!80!black}{Pre-runtime tampering prevented; model substitution defeated; cryptographic telemetry verified against silicon identity}}
};

% ====================================================
% ARROWS
% ====================================================
% Hardware boot vertical flow
\draw[->, draw=bluetxt, line width=1.3pt] (s1.south) -- (s2.north);
\draw[->, draw=bluetxt, line width=1.3pt] (s2.south) -- (s3.north);
\draw[->, draw=bluetxt, line width=1.3pt] (s3.south) -- (s4.north);
\draw[->, draw=greentxt, line width=1.4pt] (s4.south) -- (2.5, -2.8);

% Horizontal dashed measurement arrows
\draw[->, draw=greentxt, dashed, line width=1pt] (s1.east) -- (v1.west);
\draw[->, draw=greentxt, dashed, line width=1pt] (s2.east) -- (v2.west);
\draw[->, draw=greentxt, dashed, line width=1pt] (s3.east) -- (v3.west);
\draw[->, draw=greentxt, dashed, line width=1pt] (s4.east) -- (v4.west);

% Tamper failure red dashed arrows into Tamper Gate
\draw[->, draw=redtxt, dashed, line width=0.9pt] (v1.east) -- ++(0.3,0) |- (12.8, 1.5);
\draw[->, draw=redtxt, dashed, line width=0.9pt] (v2.east) -- ++(0.3,0) |- (12.8, 0.9);
\draw[->, draw=redtxt, dashed, line width=0.9pt] (v3.east) -- ++(0.3,0) |- (12.8, 0.3);
\draw[->, draw=redtxt, dashed, line width=0.9pt] (v4.east) -- ++(0.3,0) |- (12.8, -0.3);
...
Upon device startup, signed boot chains verify the application processor software and the Secure Enclave firmware before sensitive services are available. The firmware loaded onto the Secure Enclave is digitally signed by Apple, and unauthorized modification causes verification to fail. Once verified, the Secure Enclave participates in the broader trusted boot chain that protects biometric data and the cryptographic keys used by the device [@apple2021bootprocess; @apple2024secureenclave].

After the device completes its secure boot sequence, the Face ID pipeline can authenticate the user. The TrueDepth camera projects and reads thousands of infrared dots to map a user's face, while the protected biometric pipeline computes a mathematical representation and compares it against enrolled facial data protected by the Secure Enclave. Face ID data is encrypted, remains on the device, and is not included in backups [@apple2024facialmatching]. The secure boot chain therefore protects not only the operating system, but also the trust assumptions under which biometric ML components execute.

To support continued integrity, secure boot also governs software updates. Apple devices use hardware-rooted authorization to install only Apple-signed system software and firmware versions that Apple is still signing, helping prevent tampered or downgraded components from entering the trusted stack [@apple2021securesoftwareupdates]. This process maintains a robust chain of trust over time, enabling secure evolution of the platform while preserving user privacy and device security.
...
These benefits are bought with a tamper-resistance tax. The dedicated, hardened silicon delivers roughly `{python} HsmGpuCrypto.throughput_ratio_mult_str` lower cryptographic throughput than a general-purpose GPU at `{python} HsmGpuCrypto.hsm_cost_low_str`--`{python} HsmGpuCrypto.hsm_cost_high_str`+ per unit, and that single trade-off cascades into every constraint a resource-limited ML deployment cares about: the latency it adds to key exchange and on-the-fly decryption can break real-time inference budgets; its continuous secure operations draw power that shortens battery life on edge devices; its size and dedicated APIs force circuit-board and software redesign; and provisioning unique keys across a fleet of edge nodes turns identity management into a distributed-systems problem. Regulated deployments add a final cost: certification and compliance processes[^fn-hsm-hardware-certification] such as FIPS 140-2[^fn-fips-security-standard] or Common Criteria can take longer than training the model the HSM protects.
...
% ====================================================
\draw[draw=grayborder, fill=white, line width=0.7pt, rounded corners=6pt] (0, 0.4) rectangle (8.4, 5.8);

% Title
\node[fill=bluebg, draw=blueborder!60, rounded corners=2.5pt, inner sep=2.5pt, font=\sffamily\bfseries\tiny, text=bluetxt]
    at (1.4, 5.5) {A: SILICON FINGERPRINT};
\node[anchor=west, font=\sffamily\tiny, text=gray!70!black] at (2.6, 5.5) {Manufacturing Process Variations};

% Sub-box: Microstructure
\draw[draw=grayborder!70, fill=white, rounded corners=3pt] (0.2, 2.5) rectangle (3.9, 5.15);
\node[font=\sffamily\bfseries\tiny, text=black!85] at (2.05, 4.95) {Silicon Substrate Microstructure};

% Scatter dots
\foreach \x/\y in {0.7/4.4, 1.3/4.6, 2.0/4.7, 2.8/4.5, 3.4/4.3, 1.0/3.9, 2.5/3.9, 3.2/3.8, 1.1/2.8, 2.6/2.8} {
    \fill[fill=gray!40] (\x, \y) circle (2pt);
}
\node[draw=blueborder!80, fill=bluebg!60, rounded corners=2pt, inner sep=2pt, text width=2.8cm, align=center]
    at (2.05, 3.35) {
    {\tiny\bfseries\textcolor{bluetxt}{Nanoscale Variations}}\\[0.5pt]
    {\tiny\textcolor{gray!80!black}{$T_{ox}$, channel length, dopant atoms}}
};

% Sub-box: Inherent Properties
\draw[draw=grayborder!70, fill=white, rounded corners=3pt] (4.1, 2.5) rectangle (8.2, 5.15);
\node[font=\sffamily\bfseries\tiny, text=black!85] at (5.3, 4.95) {Inherent Properties};

\node[anchor=north west, inner sep=2pt, text width=3.8cm] at (4.2, 4.8) {
...
\node[fill=greenbg, draw=greenborder, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=greentxt, text width=3.6cm, align=center]
    at (6.15, 2.8) {Physical Hardware Anchor};

% Explanation text
\node[anchor=north west, text width=8.0cm, inner sep=0pt] at (0.2, 2.3) {
    {\tiny\textcolor{gray!80!black}{Each chip has a unique fingerprint caused by microscopic manufacturing variations during photolithography, oxide deposition, and chemical-mechanical polishing (CMP).}}\\[3pt]
    {\tiny\bfseries Fuzzy Extractor + Helper Data reconstructs exact cryptographic keys on demand.}
};

% ====================================================
% QUADRANT B: OPTICAL PUF (Top Right)
% ====================================================
\draw[draw=grayborder, fill=white, line width=0.7pt, rounded corners=6pt] (8.8, 0.4) rectangle (17.2, 5.8);

% Title
\node[fill=redbg, draw=redborder!60, rounded corners=2.5pt, inner sep=2.5pt, font=\sffamily\bfseries\tiny, text=redtxt]
    at (9.9, 5.5) {B: OPTICAL PUF};
\node[anchor=west, font=\sffamily\tiny, text=gray!70!black] at (11.0, 5.5) {Laser Scattering \& Speckle};

% Laser Diode Box
\node[draw=redborder, fill=white, rounded corners=3pt, inner sep=3pt, text width=1.6cm, align=center] (laser) at (9.8, 4.15) {
    {\tiny\bfseries\textcolor{redtxt}{Laser Diode}}\\[2pt]
    {\tiny\textcolor{gray!80!black}{Challenge\\[-1pt](Angle $\theta$)}}
};

% Scattering Token Box
\node[draw=grayborder!80, fill=white, rounded corners=3pt, inner sep=3pt, text width=2.1cm, align=center] (token) at (12.4, 4.15) {
    {\tiny\bfseries Scattering Token}\\[2pt]
    \tikz{
        \fill[gray!50] (0.2,0.1) circle (2.5pt);
        \fill[gray!60] (0.8,0.3) circle (3.5pt);
        \fill[gray!40] (1.4,0.0) circle (3pt);
    }\\[-2pt]
    {\tiny\textcolor{gray!70!black}{Disordered particles}}
};

% CMOS Sensor Box
\node[draw=purpleborder, fill=purplebg!40, rounded corners=3pt, inner sep=3pt, text width=2.1cm, align=center] (cmos) at (15.5, 4.15) {
    {\tiny\bfseries\textcolor{purpletxt}{CMOS Image Sensor}}\\[2pt]
    \tikz{
        \fill[purpletxt!60] (0.2,0.2) circle (1.2pt);
        \fill[purpletxt!80] (0.5,0.0) circle (1.5pt);
        \fill[purpletxt!50] (0.8,0.3) circle (1.2pt);
        \fill[purpletxt!70] (1.1,0.1) circle (1.8pt);
        \fill[purpletxt!60] (1.4,0.25) circle (1.2pt);
    }\\[-2pt]
    {\tiny\bfseries\textcolor{purpletxt}{Unique 2D Speckle Hash}}
};

% Optical flow arrows
\draw[->, draw=redtxt, line width=1.1pt] (laser.east) -- (token.west);
\draw[->, draw=purpletxt, dashed, line width=1.1pt] (token.east) -- (cmos.west);

% Challenge-Response Mechanism box
\draw[draw=grayborder!70, fill=white, rounded corners=2.5pt] (8.95, 2.7) rectangle (17.05, 3.35);
\node[anchor=west, inner sep=2pt, font=\tiny] at (9.05, 3.02) {
    \textbf{Challenge-Response Mechanism:}\quad
    Challenge (C): Angle/$\lambda$ $\to$ Wave scattering $\to$ Response (R): 2D speckle
};

% Text below
\node[anchor=north west, text width=8.1cm, inner sep=0pt] at (9.0, 2.5) {
    {\tiny\textcolor{gray!80!black}{Optical wavefront interference yields astronomical state spaces ($> 10^{100}$ combos).}}\\[1pt]
    {\tiny\textcolor{gray!80!black}{Gabor hash transforms compress analog speckles into reproducible bit strings.}}\\[2pt]
    {\tiny\bfseries\textcolor{redtxt}{Wave scattering non-linearity provides high resistance to ML modeling attacks.}}
};

% ====================================================
% QUADRANT C: ARBITER RACE PUF (Bottom Left)
% ====================================================
\draw[draw=grayborder, fill=white, line width=0.7pt, rounded corners=6pt] (0, -5.6) rectangle (8.4, -0.2);

% Title
\node[fill=amberbg, draw=amberborder!60, rounded corners=2.5pt, inner sep=2.5pt, font=\sffamily\bfseries\tiny, text=ambertxt]
    at (1.6, -0.5) {C: ARBITER RACE PUF};
\node[anchor=west, font=\sffamily\tiny, text=gray!70!black] at (2.9, -0.5) {Switch MUX Delay Difference};

% Pulse In box
\node[draw=blueborder!60, fill=white, rounded corners=2pt, inner sep=2pt, text width=1.1cm, align=center] (pulse) at (0.8, -1.8) {
    {\tiny\bfseries Pulse In}\\[1pt]
    {\tiny $0 \to 1$}
};

% Top MUX chain
\node[draw=blueborder, fill=bluebg!30, rounded corners=2pt, inner sep=2pt, font=\sffamily\tiny] (m0a) at (2.2, -1.4) {MUX 0A};
\node[draw=blueborder, fill=bluebg!30, rounded corners=2pt, inner sep=2pt, font=\sffamily\tiny] (m1a) at (3.5, -1.4) {MUX 1A};
\node[font=\sffamily\tiny, text=gray!70!black] (mdota) at (4.5, -1.4) {$\cdots$};
\node[draw=blueborder, fill=bluebg!30, rounded corners=2pt, inner sep=2pt, font=\sffamily\tiny] (mna) at (5.5, -1.4) {MUX nA};

% Bottom MUX chain
\node[draw=greenborder, fill=greenbg!30, rounded corners=2pt, inner sep=2pt, font=\sffamily\tiny] (m0b) at (2.2, -2.2) {MUX 0B};
\node[draw=greenborder, fill=greenbg!30, rounded corners=2pt, inner sep=2pt, font=\sffamily\tiny] (m1b) at (3.5, -2.2) {MUX 1B};
\node[font=\sffamily\tiny, text=gray!70!black] (mdotb) at (4.5, -2.2) {$\cdots$};
\node[draw=greenborder, fill=greenbg!30, rounded corners=2pt, inner sep=2pt, font=\sffamily\tiny] (mnb) at (5.5, -2.2) {MUX nB};

% Challenge labels
\node[font=\sffamily\tiny, text=gray!70!black] at (2.2, -2.6) {$C[0]$};
\node[font=\sffamily\tiny, text=gray!70!black] at (3.5, -2.6) {$C[1]$};
\node[font=\sffamily\tiny, text=gray!70!black] at (5.5, -2.6) {$C[n{-}1]$};

% Arbiter Box
\node[draw=amberborder, fill=amberbg!40, rounded corners=3pt, inner sep=2.5pt, text width=1.7cm, align=center] (arbiter) at (7.2, -1.8) {
    {\tiny\bfseries\textcolor{ambertxt}{Arbiter (D-FF)}}\\[1pt]
    {\tiny Race Detector}\\[2pt]
    \tikz\node[draw=amberborder, fill=white, rounded corners=1.5pt, inner sep=1.5pt, font=\sffamily\bfseries\tiny, text=ambertxt]{Bit: 0 or 1};
};

% Connections
\draw[->, draw=bluetxt, line width=0.8pt] (pulse.east) |- (m0a.west);
\draw[->, draw=greentxt, line width=0.8pt] (pulse.east) |- (m0b.west);
\draw[->, draw=bluetxt, line width=0.8pt] (m0a.east) -- (m1a.west);
\draw[->, draw=bluetxt, line width=0.8pt] (m1a.east) -- (mdota.west);
\draw[->, draw=bluetxt, line width=0.8pt] (mdota.east) -- (mna.west);
\draw[->, draw=bluetxt, line width=0.8pt] (mna.east) -| (6.2, -1.6) -- (arbiter.160);

\draw[->, draw=greentxt, line width=0.8pt] (m0b.east) -- (m1b.west);
\draw[->, draw=greentxt, line width=0.8pt] (m1b.east) -- (mdotb.west);
\draw[->, draw=greentxt, line width=0.8pt] (mdotb.east) -- (mnb.west);
\draw[->, draw=greentxt, line width=0.8pt] (mnb.east) -| (6.2, -2.0) -- (arbiter.200);

% Text below
\node[anchor=north west, text width=8.0cm, inner sep=0pt] at (0.2, -2.9) {
    {\tiny\textcolor{gray!80!black}{Two symmetrical nominal delay paths race simultaneously upon a step transition.}}\\[1pt]
    {\tiny\textcolor{gray!80!black}{Challenge bits select straight vs crossed paths at each stage.}}\\[1pt]
    {\tiny\textcolor{gray!80!black}{Sub-picosecond delay mismatches ($\Delta t$) determine which wavefront hits latch first:}}
};

\draw[draw=grayborder!70, fill=white, rounded corners=2.5pt] (0.2, -5.35) rectangle (8.2, -4.75);
\node[font=\sffamily\bfseries\tiny, align=center] at (4.2, -5.05) {
...
% ====================================================
\draw[draw=grayborder, fill=white, line width=0.7pt, rounded corners=6pt] (8.8, -5.6) rectangle (17.2, -0.2);

% Title
\node[fill=greenbg, draw=greenborder!60, rounded corners=2.5pt, inner sep=2.5pt, font=\sffamily\bfseries\tiny, text=greentxt]
    at (10.2, -0.5) {D: SRAM STARTUP PUF};
\node[anchor=west, font=\sffamily\tiny, text=gray!70!black] at (11.7, -0.5) {Bistable Inverter Threshold Bias};

% Sub-box: 6T Cell
\draw[draw=grayborder!70, fill=white, rounded corners=3pt] (9.0, -3.2) rectangle (12.9, -0.85);
\node[font=\sffamily\bfseries\tiny, text=black!85] at (10.95, -1.1) {6T Cross-Coupled Inverter Cell};

\node[draw=greenborder, fill=greenbg!30, rounded corners=2pt, inner sep=2pt, text width=1.6cm, align=center] (inv1) at (9.85, -1.85) {
    {\tiny\bfseries\textcolor{greentxt}{INV 1 (M1/M2)}}\\[2pt]
    {\tiny $V_{\text{th1}} = 0.402$\,V}
};

\node[draw=greenborder, fill=greenbg!30, rounded corners=2pt, inner sep=2pt, text width=1.6cm, align=center] (inv2) at (12.05, -1.85) {
    {\tiny\bfseries\textcolor{greentxt}{INV 2 (M3/M4)}}\\[2pt]
    {\tiny $V_{\text{th2}} = 0.395$\,V}
};

% Cross-coupling arrows
\draw[->, draw=ambertxt, line width=0.8pt] (inv1.east) -- (inv2.west);
\draw[<-, draw=ambertxt, line width=0.8pt] ($(inv1.east)+(0,-0.2)$) -- ($(inv2.west)+(0,-0.2)$);

\node[draw=amberborder!70, fill=amberbg!40, rounded corners=2pt, inner sep=2pt, font=\sffamily\bfseries\tiny, text=ambertxt, text width=3.4cm, align=center]
    at (10.95, -2.85) {Threshold Delta: $\Delta Vth = 7$\,mV};

% Sub-box: Power-up dynamics
\draw[draw=grayborder!70, fill=white, rounded corners=3pt] (13.1, -3.2) rectangle (17.0, -0.85);
\node[font=\sffamily\bfseries\tiny, text=black!85] at (14.2, -1.1) {Power-Up Dynamics:};
...
\node[fill=greenbg, draw=greenborder, rounded corners=2pt, inner sep=1.5pt, font=\sffamily\bfseries\tiny, text=greentxt, text width=3.5cm, align=center]
    at (15.05, -2.85) {Stable Power-On Pattern (50\% 0s / 1s)};

% Text below
\node[anchor=north west, text width=8.0cm, inner sep=0pt] at (9.0, -3.45) {
    {\tiny\textcolor{gray!80!black}{Standard uninitialized SRAM power-up state acts as an inherent digital key.}}\\[1pt]
    {\tiny\textcolor{gray!80!black}{Requires zero custom circuitry; available on standard commodity microcontrollers.}}\\[1pt]
    {\tiny\textcolor{gray!80!black}{BCH error-correcting codes eliminate minor thermal noise (bit error rate $< 3\%$).}}\\[2pt]
    {\tiny\bfseries\textcolor{greentxt}{Silicon RoT Anchor: Keys vanish when unpowered, defeating chip deprocessing.}}
};

% ====================================================
% BOTTOM SUMMARY: ML SYSTEMS TAKEAWAYS
% ====================================================
\draw[draw=greenborder, fill=greenbg!35, line width=0.8pt, rounded corners=5pt] (0, -7.5) rectangle (17.2, -6.0);

\node[fill=greenbg, draw=greenborder, rounded corners=3pt, inner sep=3pt, font=\sffamily\bfseries\tiny, text=greentxt]
    at (1.9, -6.75) {ML SYSTEMS TAKEAWAYS};

\node[anchor=west, text width=12.6cm, inner sep=0pt] at (3.8, -6.75) {
...
TEEs provide runtime isolation for model inference and sensitive data handling. Secure boot enforces integrity from power-on, ensuring that only verified software is executed. HSMs offer tamper-resistant storage and cryptographic processing for secure key management, model signing, and firmware validation. PUFs bind secrets and authentication to the physical characteristics of a specific device, enabling lightweight and unclonable identities.
...
| **Critical Infrastructure** (autonomous vehicles, power grids) | Supply chain compromise; real-time adversarial attacks; safety-critical failures | Hardware attestation (trusted platform module, or TPM, and PUF); Secure Boot and runtime integrity checks; redundant model validation; fault injection detection; fail-safe fallback mechanisms                              | Development cost: 6--18 months additional engineering; 20--40% higher hardware costs; latency constraints limit cryptographic defenses; requires certified hardware           |

: **Defense selection framework**: Maps deployment contexts to threat-specific defensive strategies with quantified trade-offs. The framework provides starting points for security architecture design, highlighting primary threats, recommended defense combinations, and key implementation trade-offs across seven common ML system deployment scenarios. {#tbl-defense-selection-framework tbl-colwidths="[18,18,35,29]"}
...
```{.tikz}
\begin{tikzpicture}[font=\small\sffamily, scale=0.8,
  ...
  \node[anchor=north, font=\scriptsize\sffamily, GrayLine] at (0,-0.5) {Distributions must be within $e^\epsilon$ factor everywhere.};
...
The parameter $\epsilon$ quantifies the privacy budget, representing the maximum allowable privacy loss. For a fixed mechanism and adjacency relation, smaller values of $\epsilon$ impose a tighter likelihood-ratio bound and generally require more noise, which may reduce model utility. No universal $\epsilon$ value denotes "strong," "moderate," or acceptable privacy: the interpretation depends on adjacency, $\delta$, composition, the protected population, and the consequences of inference. The multiplicative factor $e^{\epsilon}$ bounds the likelihood ratio between algorithm outputs on adjacent datasets, constraining how much an individual's participation can influence any particular result.
...
When $y = f(D) + z$ for noise $z \sim \mathcal{N}(0, \sigma^2 I)$, the privacy loss becomes a shifted Gaussian with mean $\frac{\Delta_2 f^2}{2\sigma^2}$ and variance $\frac{\Delta_2 f^2}{\sigma^2}$. For one application of the mechanism and $0<\epsilon<1$, a classic sufficient calibration is:
$$
...
**Problem**: Determine the minimum noise multiplier $\sigma$ required by the Gaussian mechanism to achieve $(\epsilon, \delta)$-differential privacy for a single vector-valued query.
...
The next subsections build a privacy-accounting ladder: simple composition adds worst-case loss, moments accounting tracks its distribution, and Rényi differential privacy composes noisy SGD steps before conversion back to an $(\epsilon,\delta)$ budget.
...
A more refined approach to privacy analysis tracks the privacy loss random variable\index{Privacy Loss Random Variable}, which quantifies how much information about an individual leaks from a single observation. For adjacent datasets $D, D'$ and algorithm output $o$, the privacy loss is:
$$
L_{\text{priv},(D,D')}^{(o)} = \ln\frac{\Pr[\mathcal{A}(D) = o]}{\Pr[\mathcal{A}(D') = o]}
$$

The privacy loss random variable characterizes the log-likelihood ratio between outputs on adjacent datasets. For $(\epsilon, \delta)$-DP, the tail probability must satisfy $\Pr[L_{\text{priv}} > \epsilon] \leq \delta$. This formulation enables composition analysis through moment-generating functions.
...
#### Accountant-input example: DP-SGD for image classification {#sec-security-privacy-practical-privacy-budget-example-dpsgd-image-classification-2662}
...
::: {.column-margin}
![](images/svg/vol2_security_privacy_margin_003.svg){width="100%" fig-alt="Budget envelope showing ten data-dependent releases reaching a total epsilon budget limit of 10."}
...
The practical implication is that organizations must maintain a privacy ledger for the protected population and halt or redesign further data-dependent releases when the allocated bound would be exceeded.
...
The dataset and task then determine whether the guarantee is usable. Effective DP training depends on whether the clipped signal remains learnable after calibrated noise:

- **Effective sample size**\index{Differential Privacy!effective sample size}: More independent records can improve the signal-to-noise ratio, but no universal record-count threshold guarantees utility. Repeated observations, class imbalance, and subgroup rarity can make the effective sample size much smaller than the row count.
...
Finally, the computational budget decides whether the design can ship. DP training typically requires 2--5$\times$ more compute than nonprivate training because of per-sample gradient computation and larger batch sizes needed to overcome noise. If computational resources are constrained, this overhead may be prohibitive. Cloud deployments can scale compute, but edge training scenarios may find DP impractical.
...
- [ ] **Criterion 5 (compute)**: weigh the 8-GPU cloud training run and 2--5$\times$ DP-SGD overhead against the criteria that pull toward and away from DP, then identify the dominant deployment signal.
...
add noise N(0, sigma^2 C^2 I) to the clipped-gradient sum, then divide by lot size
...
::: {.column-margin}
![](images/svg/vol2_security_privacy_margin_004.svg){width="100%" fig-alt="Channel-count curve illustrating that topology determines how communication links scale with node count."}
...
Layered defense requires reasoning about where an ML system is vulnerable at each stage of its lifecycle. Hardware roots of trust, differential privacy budgets, and generative AI guardrails address threats that other layers cannot catch alone. No single mechanism suffices, but their composition can limit the effect of a compromised control. These defenses must be part of system design rather than additions made after deployment.

Secure aggregation applies C$^3$ to a specific privacy boundary. Cryptographic masking adds compute on each client and extra communication rounds, while dropout recovery and threshold reconstruction add coordination state to the server. The mechanism removes direct inspection of individual updates, but the operational bottleneck can move to slow clients, recovery shares, or round completion. Privacy is preserved only if the scheduler and capacity plan accommodate that shifted critical path rather than bypass the protocol under load.
...
The deepest of this chapter's threats is not an intrusion at all. It is the model's own competence. Fitting training data can create channels for extraction or membership inference, and a few poisoned examples can bend behavior from inside the training process, because a learned decision boundary cannot be patched like a vulnerable function. This is the information leakage invariant (principle \ref{pri-information-leakage}): privacy must be designed into the mechanism and accounted across data-dependent releases. Once a DP-trained model is released, ordinary inference from that fixed model is post-processing; an interactive mechanism that consults protected records for each answer instead composes across answers. Security therefore cannot be bolted on at the perimeter, because the model is both an asset and an attack surface.

::: {.callout-chapter-connection title="From security to resilience"}

This chapter owns adversarial intent and control: who can infer, manipulate, disable, or authorize what across data, model, interface, and infrastructure boundaries. Security mechanisms can constrain an attacker without proving that model behavior remains acceptable under every perturbation. @Sec-robust-ai therefore owns behavioral evidence. It asks how accuracy, calibration, abstention, and safety change under distribution shift, hardware faults, interacting production failures, and adversarial inputs. The same crafted input can appear in both chapters, but the engineering questions differ: this chapter constrains attacker access and privileges, while the next measures and bounds the model's response.

:::

[^fn-robustness-privacy-tradeoff]: **Robustness-privacy trade-off**\index{Robustness-Privacy Trade-Off}\index{Robustness-Privacy!trade-off}: Adversarial training can increase susceptibility to membership inference in the settings studied by @song2019privacy. The result means that a robustness intervention must be reevaluated against privacy invariants rather than assumed to improve every security property at once.
