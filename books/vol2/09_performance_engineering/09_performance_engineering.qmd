...
_Why does peak compute capacity fail to deliver real-time serving throughput without kernel and memory co-design?_
...
::: {#fig-performance-engineering-gpu-memory-hierarchy fig-env="figure" fig-pos="htb" fig-cap="**GPU memory hierarchy and data movement**: Visual hierarchy from fast on-chip registers and SRAM down through L2 cache to off-chip HBM3. Latency scales from 1 clock cycle (~0.01 pJ) to hundreds of nanoseconds (~640 pJ per transaction), illustrating the extreme cost disparity between SRAM-resident computation and off-chip memory traffic." fig-alt="Pyramid diagram of GPU memory hierarchy from registers at the top through shared memory and L2 cache to HBM at the base, with capacity, bandwidth, and energy metrics per level."}
```{=latex}
\begin{tikzpicture}[
    >=Stealth,
    font=\small,
    ...
    pill/.style={
        ...
        inner sep=3.5pt, font=\scriptsize\bfseries\color{white},
        align=center
    }
]
    ...
    % =========================================================================
    % MAIN HIERARCHY STACK (centered at x = 0)
    % =========================================================================

    % Level 1: Registers (RF)
    \node[level_box, draw=NavyBlue!85, fill=NavyBlue!6!white,
          minimum width=8.0cm, minimum height=1.35cm] (rf) at (0, 0) {
        {\normalsize\bfseries\color{NavyBlue!90!black} Registers (RF)}\\[2pt]
        {\scriptsize $\sim$33 MB total $\;\cdot\;$ $\sim$1 cycle latency $\;\cdot\;$ $\sim$0.01 pJ per access}\\[1pt]
        {\tiny\itshape\color{black!70} Private per thread $\;\cdot\;$ Unlimited bandwidth (in-register execution)}
    };

    % Level 2: L1 Cache / Shared Memory
    \node[level_box, draw=NavyBlue!80, fill=NavyBlue!5!white,
          minimum width=9.8cm, minimum height=1.35cm,
          below=14pt of rf] (l1) {
        {\normalsize\bfseries\color{NavyBlue!90!black} L1 Cache / Shared Memory (SRAM)}\\[2pt]
        {\scriptsize $\sim$228 KB/SM $\;\cdot\;$ 20--30 cycles latency ($\sim$20 ns) $\;\cdot\;$ $\sim$0.5 pJ per access}\\[1pt]
        {\tiny\itshape\color{black!70} Shared within thread block $\;\cdot\;$ Primary arena for operator fusion \& tiling}
    };

    % Level 3: L2 Cache
    \node[level_box, draw=SlateGrey!80, fill=SlateGrey!5!white,
          minimum width=11.6cm, minimum height=1.35cm,
          below=14pt of l1] (l2) {
        {\normalsize\bfseries\color{black!85} L2 Cache}\\[2pt]
        {\scriptsize $\sim$50 MB on H100 $\;\cdot\;$ $\sim$200 cycles latency ($\sim$130 ns) $\;\cdot\;$ $\sim$20 pJ per access}\\[1pt]
        {\tiny\itshape\color{black!70} Chip-wide shared buffer $\;\cdot\;$ Hardware-managed automatic reuse across SMs}
    };

    % Level 4: High Bandwidth Memory (HBM3)
    \node[level_box, draw=ForestGreen!85, fill=ForestGreen!6!white,
          minimum width=13.4cm, minimum height=1.35cm,
          below=14pt of l2] (hbm) {
        {\normalsize\bfseries\color{ForestGreen!85!black} High Bandwidth Memory (HBM3)}\\[2pt]
        {\scriptsize 80 GB capacity $\;\cdot\;$ 3.35 TB/s peak bandwidth $\;\cdot\;$ $\sim$300 ns latency}\\[1pt]
        {\tiny\itshape\color{black!70} Off-chip stacked DRAM $\;\cdot\;$ $\sim$640 pJ per 64-byte transaction $\;\cdot\;$ Primary model weight store}
    };

    % Downward arrows between levels
    \draw[->, line width=1.0pt, SlateGrey!80!black] (rf.south) -- (l1.north);
    \draw[->, line width=1.0pt, SlateGrey!80!black] (l1.south) -- (l2.north);
    \draw[->, line width=1.0pt, SlateGrey!80!black] (l2.south) -- (hbm.north);

    % =========================================================================
    % RIGHT BADGES & AXIS (Access Energy)
    % =========================================================================
    \coordinate (badge_x) at (8.2, 0);

    \node[align=center, font=\small] (energy_label) at (badge_x |- rf.north) [anchor=south] {
        \textbf{\color{Crimson}Access Energy}\\[1pt]
        {\scriptsize\color{Crimson}grows 1,280$\times$}
    };

    \node[pill, fill=NavyBlue!90!black, minimum width=2.2cm, minimum height=0.6cm]
        (b1) at (rf.east -| badge_x) {$\sim$0.01 pJ};

    \node[pill, fill=NavyBlue!80!black, minimum width=2.2cm, minimum height=0.6cm]
        (b2) at (l1.east -| badge_x) {$\sim$0.5 pJ ($1\times$)};

    \node[pill, fill=BurntOrange, minimum width=2.2cm, minimum height=0.6cm]
        (b3) at (l2.east -| badge_x) {$\sim$20 pJ ($40\times$)};

    \node[pill, fill=Crimson, minimum width=2.2cm, minimum height=0.6cm]
        (b4) at (hbm.east -| badge_x) {640 pJ ($1{,}280\times$)};

    % Red dashed vertical line connecting the badges
    \draw[dashed, line width=1.0pt, Crimson!70] (energy_label.south) -- (b1.north);
    \draw[dashed, line width=1.0pt, Crimson!70] (b1.south) -- (b2.north);
    \draw[dashed, line width=1.0pt, Crimson!70] (b2.south) -- (b3.north);
    \draw[dashed, line width=1.0pt, Crimson!70] (b3.south) -- (b4.north);
    \draw[dashed, ->, line width=1.0pt, Crimson!80] (b4.south) -- ++(0, -0.6);

    % =========================================================================
    % LEFT INDICATOR: Capacity
    % =========================================================================
    \coordinate (left_x) at (-7.9, 0);

    \node[align=center, font=\small] (cap_label) at (left_x |- rf.north) [anchor=south] {
        \textbf{\color{black!85}Capacity}\\[1pt]
        {\scriptsize\color{black!70}grows 2,400$\times$}
    };
    \draw[dashed, ->, line width=1.0pt, SlateGrey!70]
        (cap_label.south) -- (left_x |- hbm.south);

    % =========================================================================
    % BOTTOM NOTE
    % =========================================================================
    \node[draw=BorderGrey, fill=LightGrey!50!white, rounded corners=3pt, line width=0.7pt,
          inner sep=4pt, font=\scriptsize\color{black!75},
          below=14pt of hbm] (bot_note) {
        Pyramid width indicates relative capacity (33 MB RF $\rightarrow$ 80 GB HBM3)
    };
...
\begin{tikzpicture}[
    ...
    \node[font=\scriptsize\bfseries\color{Crimson}, rotate=27.4, align=center, anchor=south]
        at (2.35, 2.45) {slope = 3.35 TB/s\\[-1pt]{\tiny\color{Crimson!90!black}(HBM3 BW ceiling)}};
...
The prefill-decode decomposition has direct implications for system design, as visualized on the hardware roofline in @fig-performance-engineering-prefill-decode-roofline. A system optimized for prefill (maximizing FLOP/s utilization) operates at high arithmetic intensity in the compute-bound regime. Conversely, a system running decode (maximizing bandwidth utilization) operates far below the ridge point in the memory-bound regime. A real serving system must handle both phases, often simultaneously across different active requests.

::: {#fig-performance-engineering-prefill-decode-roofline fig-env="figure" fig-pos="htb" fig-cap="**Prefill versus decode roofline comparison**: Operational regimes of LLM prompt evaluation (prefill) versus autoregressive token generation (decode) plotted on the hardware roofline model. Prefill operates in the compute-bound regime at high arithmetic intensity ($I \gg I_{\text{ridge}}$), whereas decode is bound by HBM memory bandwidth ($I \ll I_{\text{ridge}}$)." fig-alt="Roofline comparison showing the compute-bound prefill phase near the Tensor Core ceiling and the memory-bound decode phase along the memory bandwidth slope."}
```{=latex}
\begin{tikzpicture}[
    >=Stealth,
    font=\small,
...
\begin{tikzpicture}[
    ...
    pill/.style={
        rectangle, rounded corners=3pt,
        line width=0.85pt, inner sep=4pt
    },
    kernel/.style={
        rectangle, rounded corners=5pt,
        draw=NavyBlue!80, fill=NavyBlue!6!white,
        line width=0.9pt, align=center,
        inner sep=5pt
    },
    subkernel/.style={
        rectangle, rounded corners=3pt,
        draw=NavyBlue!80, fill=white,
    ...
    % Header pill
    \node[pill, draw=Crimson!70, fill=Crimson!8!white, font=\bfseries\color{Crimson}, anchor=west]
        at (0, 7.8) {Unfused: 6 HBM Transfers (3 R / 3 W)};

    % HBM Bar (Top)
    \node[rectangle, rounded corners=4pt, draw=ForestGreen!80, fill=ForestGreen!10!white,
          line width=0.9pt, minimum width=13.0cm, minimum height=0.65cm,
          font=\small\bfseries\color{ForestGreen!90!black}, anchor=west]
        at (0, 7.0) {High Bandwidth Memory (Off-Chip HBM3 $\cdot$ $\sim$640~pJ $\cdot$ 300~ns)};

    % Three Unfused Kernels
    \node[kernel, minimum width=3.2cm, minimum height=1.1cm] (k1) at (2.2, 4.8) {
        {\bfseries\color{NavyBlue!95!black} LayerNorm}\\[1pt]
        {\scriptsize\color{black!65} Kernel 1}
    };

    \node[kernel, minimum width=3.2cm, minimum height=1.1cm] (k2) at (6.5, 4.8) {
        {\bfseries\color{NavyBlue!95!black} GELU}\\[1pt]
        {\scriptsize\color{black!65} Kernel 2}
    };

    \node[kernel, minimum width=3.2cm, minimum height=1.1cm] (k3) at (10.8, 4.8) {
        {\bfseries\color{NavyBlue!95!black} Linear}\\[1pt]
        {\scriptsize\color{black!65} Kernel 3}
    };

    % Dashed dependency arrows between kernels
    \draw[->, dashed, SlateGrey!70, line width=1.0pt] (k1.east) -- (k2.west);
    \draw[->, dashed, SlateGrey!70, line width=1.0pt] (k2.east) -- (k3.west);

    % Memory traffic arrows (Red)
    % Kernel 1
    \draw[->, Crimson, line width=1.2pt] (1.5, 6.67) -- (1.5, 5.35);
    \node[font=\tiny\bfseries\color{Crimson}, anchor=east] at (1.45, 6.0) {Read};
    \draw[<-, Crimson, line width=1.2pt] (2.9, 6.67) -- (2.9, 5.35);
    \node[font=\tiny\bfseries\color{Crimson}, anchor=west] at (2.95, 6.0) {Write};

    % Kernel 2
    \draw[->, Crimson, line width=1.2pt] (5.8, 6.67) -- (5.8, 5.35);
    \node[font=\tiny\bfseries\color{Crimson}, anchor=east] at (5.75, 6.0) {Read};
    \draw[<-, Crimson, line width=1.2pt] (7.2, 6.67) -- (7.2, 5.35);
    \node[font=\tiny\bfseries\color{Crimson}, anchor=west] at (7.25, 6.0) {Write};

    % Kernel 3
    \draw[->, Crimson, line width=1.2pt] (10.1, 6.67) -- (10.1, 5.35);
    \node[font=\tiny\bfseries\color{Crimson}, anchor=east] at (10.05, 6.0) {Read};
    \draw[<-, Crimson, line width=1.2pt] (11.5, 6.67) -- (11.5, 5.35);
    \node[font=\tiny\bfseries\color{Crimson}, anchor=west] at (11.55, 6.0) {Write};

    % Penalty Banner
    \node[pill, draw=Crimson!60, fill=Crimson!7!white, minimum width=13.0cm, minimum height=0.55cm,
          font=\scriptsize\bfseries\color{Crimson}, align=center, anchor=west]
        at (0, 3.65) {Memory Penalty: 3 Intermediate Writes + 3 Reads through HBM $\cdot$ High DRAM Latency Bubbles};

    % =========================================================================
    % SECTION DIVIDER
    % =========================================================================
    \draw[SlateGrey!35, line width=0.8pt] (0, 2.95) -- (13.0, 2.95);
    ...
    % Header pill
    \node[pill, draw=ForestGreen!70, fill=ForestGreen!8!white, font=\bfseries\color{ForestGreen}, anchor=west]
        at (0, 2.3) {Fused: 2 HBM Transfers (1 R / 1 W)};

    % HBM Bar (Bottom)
    \node[rectangle, rounded corners=4pt, draw=ForestGreen!80, fill=ForestGreen!10!white,
          line width=0.9pt, minimum width=13.0cm, minimum height=0.65cm,
          font=\small\bfseries\color{ForestGreen!90!black}, anchor=west]
        at (0, 1.5) {High Bandwidth Memory (Off-Chip HBM3 $\cdot$ $\sim$640~pJ $\cdot$ 300~ns)};

    % Fused Container
    \node[rectangle, rounded corners=5pt, draw=NavyBlue!80, fill=NavyBlue!5!white,
          line width=1.1pt, minimum width=12.2cm, minimum height=2.1cm,
          anchor=north west] (fused_box) at (0.4, 0.5) {};

    % Fused Title & Subtitle inside container
    \node[font=\small\bfseries\color{NavyBlue!95!black}, anchor=north]
        at (6.5, 0.35) {Fused Kernel: LayerNorm + GELU + Linear};
    \node[font=\tiny\color{black!70}, anchor=north]
        at (6.5, -0.05) {Intermediates retained entirely within on-chip SRAM / Registers ($\sim$0.5~pJ $\cdot$ 20~ns)};

    % Sub-boxes inside Fused Kernel
    \node[subkernel, minimum width=2.4cm, minimum height=0.65cm] (sub_ln) at (2.2, -0.85) {
        \footnotesize\bfseries LayerNorm
    };

    \node[subkernel, minimum width=2.4cm, minimum height=0.65cm] (sub_gelu) at (6.5, -0.85) {
        \footnotesize\bfseries GELU
    };

    \node[subkernel, minimum width=2.4cm, minimum height=0.65cm] (sub_lin) at (10.8, -0.85) {
        \footnotesize\bfseries Linear
    };

    % Internal SRAM arrows
    \draw[->, NavyBlue!90!black, line width=1.1pt] (sub_ln.east) -- (sub_gelu.west)
        node[midway, above, font=\tiny\bfseries\color{NavyBlue!90!black}] {SRAM};
    \draw[->, NavyBlue!90!black, line width=1.1pt] (sub_gelu.east) -- (sub_lin.west)
        node[midway, above, font=\tiny\bfseries\color{NavyBlue!90!black}] {SRAM};

    % Memory traffic arrows (Green)
    \draw[->, ForestGreen, line width=1.3pt] (2.2, 1.17) -- (2.2, 0.5);
    \node[font=\scriptsize\bfseries\color{ForestGreen}, anchor=east] at (2.1, 0.85) {1 Read};

    \draw[<-, ForestGreen, line width=1.3pt] (10.8, 1.17) -- (10.8, 0.5);
    \node[font=\scriptsize\bfseries\color{ForestGreen}, anchor=west] at (10.9, 0.85) {1 Write};

    % Payoff Banner
    \node[pill, draw=ForestGreen!70, fill=ForestGreen!8!white, minimum width=13.0cm, minimum height=0.55cm,
          font=\scriptsize\bfseries\color{ForestGreen!90!black}, align=center, anchor=west]
        at (0, -2.1) {Optimization Payoff: 1 Read + 1 Write = 2 HBM Transfers $\cdot$ 3$\times$ Memory Traffic Reduction};
...
::: {#fig-performance-engineering-flashattention-tiling fig-env="figure" fig-pos="htb" fig-cap="**FlashAttention memory hierarchy tiling**: Block-by-block scheduling of Q, K, and V matrices loaded into on-chip shared memory (SRAM). Attention scores are computed and rescaled online without materializing the quadratic $S \times S$ intermediate matrix in off-chip HBM, reducing memory traffic from $\mathcal{O}(S^2)$ to $\mathcal{O}(S)$." fig-alt="FlashAttention tiling diagram showing Q, K, V blocks loaded into fast SRAM with online softmax accumulation, avoiding materialization of the full intermediate attention matrix in HBM."}
```{=latex}
\begin{tikzpicture}[
    >=Stealth,
    font=\small,
    ...
    \node[pill, draw=ForestGreen!80, fill=ForestGreen!10!white,
        ...
        at (0.7, 7.6) {High Bandwidth Memory (Off-Chip HBM3 $\cdot$ 3.35~TB/s $\cdot$ $\sim$640~pJ)};
    ...
    \node[pill, draw=NavyBlue!60, fill=white,
        ...
        at (0.7, 2.0) {SRAM / Shared Memory (On-Chip SM $\cdot$ $\sim$228~KB $\cdot$ $\sim$0.5~pJ $\cdot$ $\sim$20~ns)};
...
\begin{tikzpicture}[
    ...
    \node[anchor=north west, text width=5.7cm, align=left] at ([shift={(0.85cm, -0.20cm)}] callout_right.north west) {
        ...
        {\tiny\color{black!65} \mbox{Standard in AWQ, GPTQ, FP4/FP8 ($B=32$ or $64$)}}
    };
...
\begin{tikzpicture}[
    ...
    \node[rounded corners=6pt, draw=BorderBlue!90, fill=BgCard, line width=0.9pt,
          minimum width=16.0cm, minimum height=6.8cm, anchor=north west] (topbox) at (0, 0) {};
    ...
    \node[anchor=north, align=center] (s1_sub) at ([shift={(1.725cm, -0.88cm)}] s1.north west) {
        \scriptsize\bfseries\color{PurpleStage!90!black} TorchDynamo $\cdot$ FX Tracing
    };
    \node[anchor=north west, text width=3.2cm, align=left] at ([shift={(0.15cm, -1.38cm)}] s1.north west) {
        \tiny\color{black!80}
    ...
    \node[anchor=north, align=center] (s2_sub) at ([shift={(1.725cm, -0.88cm)}] s2.north west) {
        \scriptsize\bfseries\color{DarkNavy} AOTAutograd $\cdot$ FX Passes
    };
    \node[anchor=north west, text width=3.2cm, align=left] at ([shift={(0.15cm, -1.38cm)}] s2.north west) {
        \tiny\color{black!80}
    ...
    \node[anchor=north, align=center] (s3_sub) at ([shift={(1.725cm, -0.88cm)}] s3.north west) {
        \scriptsize\bfseries\color{AmberStage!95!black} TorchInductor Fusion Pass
    };
    \node[anchor=north west, text width=3.2cm, align=left] at ([shift={(0.15cm, -1.38cm)}] s3.north west) {
        \tiny\color{black!80}
    ...
    \node[anchor=north, align=center] (s4_sub) at ([shift={(1.725cm, -0.88cm)}] s4.north west) {
        \scriptsize\bfseries\color{GreenStage!90!black} Triton $\cdot$ CUTLASS $\cdot$ cuBLAS
    };
    \node[anchor=north west, text width=3.2cm, align=left] at ([shift={(0.15cm, -1.38cm)}] s4.north west) {
        \tiny\color{black!80}
    ...
    \node[rounded corners=6pt, draw=BorderBlue!80, fill=BgCard, line width=0.85pt,
          minimum width=7.85cm, minimum height=3.0cm, anchor=north west] (irbox) at (0, -7.15cm) {};
    ...
    \node[rounded corners=6pt, draw=BorderGreen!80, fill=BgCard, line width=0.85pt,
          minimum width=7.85cm, minimum height=3.0cm, anchor=north east] (botbox) at (16.0cm, -7.15cm) {};
    ...
    \node[anchor=north west, text width=7.4cm, align=left] at ([shift={(0.25cm, -0.85cm)}] botbox.north west) {
        ...
        \textbf{\color{GreenStage!90!black}Net Acceleration:}\quad \textbf{1.4$\times$--2.0$\times$ training \& inference speedup}
    };
    ...
    \node[rounded corners=6pt, draw=BorderGrey!90, fill=BgCard, line width=0.85pt,
          minimum width=16.0cm, minimum height=2.85cm, anchor=north west] (tradebox) at (0, -10.45cm) {};
    ...
    \node[rounded corners=4pt, draw=BorderBlue!75, fill=white, line width=0.75pt,
          minimum width=5.05cm, minimum height=1.85cm, anchor=north west] (subc1)
    ...
    \node[rounded corners=4pt, draw=BorderAmber!75, fill=white, line width=0.75pt,
          minimum width=5.05cm, minimum height=1.85cm, anchor=north] (subc2)
    ...
    \node[rounded corners=4pt, draw=BorderGreen!75, fill=white, line width=0.75pt,
          minimum width=5.05cm, minimum height=1.85cm, anchor=north east] (subc3)
...
\begin{tikzpicture}[
    ...
    \node[anchor=north west, text width=10.2cm, align=left] at ([shift={(0.3cm, -0.78cm)}] l4.north west) {
        ...
        \textbf{Tools:} \textbf{\color{DarkNavy}Prometheus $\cdot$ Grafana $\cdot$ Weights \& Biases $\cdot$ Ray Dashboard $\cdot$ OpenTelemetry}
    };

    % ---- Level 3: Framework & Distributed Traces ----
    \node[levelcard, draw=BorderAmber!85, fill=SoftAmber!30] (l3) at (2.55cm, -2.35cm) {};
    ...
    \node[levelcard, draw=BorderBlue!85, fill=SoftBlue!30] (l2) at (2.55cm, -4.70cm) {};
    ...
    \node[levelcard, draw=BorderPurple!85, fill=SoftPurple!30] (l1) at (2.55cm, -7.05cm) {};
    ...
    % =========================================================================
    \node[rounded corners=6pt, draw=BorderGrey!90, fill=BgCard, line width=0.85pt,
          minimum width=2.25cm, minimum height=9.1cm, anchor=north west] (flowbox) at (0, 0) {};
    ...
    \node[circle, draw=BorderBlue!90, fill=white, line width=1.1pt, inner sep=0pt, minimum size=0.56cm]
          (c1) at ([shift={(1.125cm, -1.35cm)}] flowbox.north west) {
    ...
    \node[circle, draw=AmberStage!90, fill=white, line width=1.1pt, inner sep=0pt, minimum size=0.56cm]
          (c2) at ([shift={(1.125cm, -3.55cm)}] flowbox.north west) {
    ...
    \node[circle, draw=OceanBlue!80, fill=white, line width=1.1pt, inner sep=0pt, minimum size=0.56cm]
          (c3) at ([shift={(1.125cm, -5.75cm)}] flowbox.north west) {
    ...
    \node[circle, draw=PurpleStage!85, fill=white, line width=1.1pt, inner sep=0pt, minimum size=0.56cm]
          (c4) at ([shift={(1.125cm, -7.70cm)}] flowbox.north west) {
    ...
    % =========================================================================
    \node[rounded corners=6pt, draw=BorderGrey!90, fill=BgCard, line width=0.85pt,
          minimum width=2.65cm, minimum height=9.1cm, anchor=north west] (scopebox) at (13.65cm, 0) {};
    ...
    \node[scopecard, draw=BorderBlue!80] at ([shift={(1.325cm, -1.35cm)}] scopebox.north west) {
        \bfseries\scriptsize\color{DarkNavy} Fleet / Cluster\\[3pt]
        \tiny\color{black!55} Hours to Days
    };

    % Subcard 3 (aligned with Level 3)
    \node[scopecard, draw=BorderAmber!80] at ([shift={(1.325cm, -3.55cm)}] scopebox.north west) {
    ...
    \node[scopecard, draw=BorderBlue!80] at ([shift={(1.325cm, -5.75cm)}] scopebox.north west) {
    ...
    \node[scopecard, draw=BorderPurple!80] at ([shift={(1.325cm, -7.85cm)}] scopebox.north west) {
...
* **Bytes usually bind**: On modern high-throughput accelerators, the chapter's roofline recap places most transformer work below the FP16 ridge point of `{python} RooflineSummaryRecap.h100_fp16_ridge_str`, so useful speedups come first from reducing HBM traffic, keeping intermediates in SRAM, and spending compute only when it moves the active bottleneck.
