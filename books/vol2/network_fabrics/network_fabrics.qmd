...
- Design network virtualization strategies for multi-tenant GPU clusters using SR-IOV resource allocation and traffic isolation
...
class NetworkRunningExampleCluster:
    ...
    grad_bytes = params * (2 * byte / param)

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    n_gpus_str = fmt_count(n_gpus, commas=True, label="GPU")
    peer_gpus_str = fmt(n_gpus - 1, precision=0, commas=False)
    grad_gb_str = fmt_qty(grad_bytes, GB, precision=0, commas=False)
    host_staging_traffic_gb_str = fmt_qty(2 * grad_bytes, GB, precision=0, commas=False)
```
...
- **AllReduce**\index{AllReduce!definition}: An AllReduce (see @sec-collective-communication for algorithmic cost models) sums gradients across thousands of GPUs so that every device holds the identical average, forming the heartbeat of synchronous training.
- **AllGather**\index{AllGather!definition}: An AllGather (see @sec-collective-communication) collects different model portions so that every GPU can reconstruct the full model state.
- **AllToAll**\index{AllToAll!definition}: An AllToAll, the most demanding pattern, requires every GPU to send unique data to every other GPU, a requirement critical to expert parallelism (see @sec-inference-scale-expert-parallelism-moe-models-55f6).
...
In a single machine, the memory bus moves data between the processor and memory. In a distributed training cluster, the network fabric serves the analogous role: it is the **gradient bus**\index{Gradient!bus} that moves parameter updates between workers. As the memory wall in @sec-compute-memory-wall limits single-device throughput, network fabric bandwidth determines multi-device throughput (the **communication wall**\index{Communication wall!definition}). Protocols, topologies, congestion control, and collective-routing choices all determine how close that bus comes to its physical limits. That physical asymmetry is what the fabric layer contributes to the cost model: a gradient that stays on NVLink moves an order of magnitude faster than the same gradient crossing InfiniBand, and @fig-network-bandwidth-scaling shows that gap widening rather than closing over the last decade.
::: {#fig-network-bandwidth-scaling fig-env="figure" fig-pos="htb" fig-cap="**Historical network bandwidth scaling**: Intra-node NVLink, inter-node InfiniBand and Ethernet, and the PCIe host bus, plotted on a common log scale from 2010 to 2024. NVLink pulls roughly an order of magnitude clear of the others, so the same collective costs far more when it crosses a machine boundary than when it stays inside one." fig-alt="Log-scale chart of interconnect bandwidth from 2010 to 2024 for Ethernet, InfiniBand, NVLink, and PCIe. All four rise steadily, with NVLink roughly an order of magnitude above the others by 2024."}
...
data = pd.read_csv("vol2/network_fabrics/data/bandwidth_trends.csv")
...
for _i, _g in enumerate(_groups):
    ...
    ax.plot(_sub["Year"], _sub["Bandwidth_Gbps"], color=_cmap(_i), label=str(_g), marker="o", linewidth=2)
ax.set_yscale("log")
ax.legend(title="Interconnect", frameon=False, fontsize=9)
ax.set_ylabel("Bandwidth (Gbps)")
ax.set_title("Network and interconnect bandwidth over time")
...
\begin{tikzpicture}[font=\small\mlsysFigureSans]
  ...
  \tikzset{
    ganttbar/.style={draw=none, text=white, font=\scriptsize\mlsysFigureSans, align=center, minimum height=0.8cm, inner sep=0pt, anchor=west},
  ...
  \draw[RedLine, line width=1.2pt, decorate, decoration={brace, amplitude=4pt}]
    (9.2, -0.48) -- (8, -0.48)
    node[midway, below=3pt, font=\scriptsize\bfseries, text=RedLine] {30 ms idle per step};
...
: **ML networking inverts traditional data-center assumptions**: Where web services require fairness for millions of independent flows, ML training requires globally synchronized communication with predictable completion times for a handful of massive collective operations. {#tbl-network-assumptions}

The synchronicity inversion represents the most fundamental contrast (@tbl-network-assumptions). Web traffic is asynchronous; one user's slow connection does not affect another's. The bulk synchronous parallel (BSP)[^fn-bsp-valiant-routing] model governs distributed training: all 1,024 GPUs in a training job must complete their gradient exchange before any of them can proceed to the next step. The slowest link therefore dictates the performance of the entire cluster. A single congested switch port that delays one GPU's packets by 100 ms effectively wastes 100 ms of compute time for all 1,024 GPUs. *Tail latency is not an outlier; it is the bottleneck.*
...
The flow inversion compounds this problem. Traditional networks are designed for fairness among millions of "mice flows",[^fn-elephant-flow-collision] yet ML training is dominated by a few "elephant flows" corresponding to the gradient AllReduce. A single AllReduce on a 175B parameter model can involve exchanging `{python} NetworkRunningExampleCluster.grad_gb_str` of data. Standard flow control and routing mechanisms like equal-cost multi-path (ECMP), which hash each flow onto one of several equal-cost paths, are poorly suited to this traffic pattern. A static hash cannot subdivide one elephant flow across all available links, and it can inadvertently map multiple elephant flows to the same link, creating massive congestion while other links sit idle.

[^fn-elephant-flow-collision]: [offset=2mm] **Elephant and mice flows**\index{Elephant and mice flows!definition}: In network measurement literature, "mice" are many short flows and "elephants" are rare massive flows that dominate bandwidth. In ML clusters, one AllReduce elephant flow\index{Elephant flow!network traffic} can carry `{python} NetworkRunningExampleCluster.grad_gb_str`, and ECMP's static hash cannot subdivide it across every available path.

The loss inversion completes the picture. TCP was designed to recover across an unreliable network. Reliable InfiniBand and RoCE transports also detect loss and retransmit, but recovery granularity and tail latency depend on the transport implementation. A dropped packet can be especially costly on an RNIC that uses Go-Back-N-style recovery, which restarts transmission from the missing sequence point rather than repairing only the lost region. Production fabrics therefore aim to make loss rare and recovery bounded. InfiniBand normally suppresses buffer-overflow loss with credit-based flow control. Many Ethernet-based RoCE deployments instead use carefully tuned priority flow control (PFC), a link-layer backpressure mechanism that pauses senders before buffers overflow; PFC-free designs rely more heavily on endpoint congestion control and loss recovery [@mittal2018irn].
...
- **Level 2: transport** (@sec-network-fabrics-transport). InfiniBand and RoCE provide remote direct memory access (RDMA) primitives; the $\alpha$-$\beta$ model quantifies the latency-vs.-bandwidth trade-off for different message sizes.
...
The traffic pattern determines which fabric metric matters. A global reduction stresses bisection bandwidth and the slowest path; a point-to-point pipeline transfer stresses startup latency and hop count; an expert exchange stresses path diversity and egress balance; and checkpoint or tenant traffic stresses isolation from the synchronization path. @Tbl-network-fabrics-traffic-metrics provides the compact mapping that the remaining levels develop.
...
| Pipeline point-to-point             | Small or dependency-critical transfers | startup latency ($\alpha$); hop count                       | short paths, direct placement, transport offload                  |
| Expert AllToAll                     | Many simultaneous rank pairs           | path utilization; egress queue depth; tail latency          | adaptive routing, path diversity, balanced expert placement       |
...
Achieving `{python} IbNdrWireRecap.ib_ndr_gbit_per_s_str` and higher line rates cannot rely on toggling a voltage on and off faster. Signal attenuation in copper and chromatic dispersion in fiber impose a hard ceiling on the **symbol rate**\index{Symbol rate!definition} (the number of signal transitions per second). High-speed links overcome this by using PAM4 to pack more information into each transition.
...
|:--------------------------------------------------------|:--------------------|:----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------|:---------------------------------|
| **DAC (direct attach copper)**\index{DAC!definition}    | ~3 meters           | Lowest cost (~\$50) and no optical module power                       | Lowest media latency; passive copper avoids optical conversion                                 | Within a rack                    |
| **AOC (active optical cable)**\index{AOC!definition}    | 3 to 30 meters      | Higher cost (~\$500) from permanently attached transceivers           | Electrical/optical conversion plus forward error correction (FEC) adds hundreds of nanoseconds | Rack-to-rack or row-scale links  |
...
Large-scale training requires sustained, synchronized bulk transfers. A single AllReduce operation across 1,024 accelerators may move terabytes of gradient data. This pattern favors RDMA transport offload, reliable completion, and bounded loss-recovery time. A physically lossless fabric is one way to make that performance predictable, but it is not the only design.
...
1.  **Significance**: RDMA can reduce end-to-end message latency from tens of microseconds for a kernel TCP path to low-single-digit microseconds on a tuned fabric, cutting the $L_{\text{lat}}$ term in the iron law. For one participant sending a `{python} NetworkRunningExampleCluster.grad_gb_str` payload, GPUDirect RDMA also avoids about `{python} NetworkRunningExampleCluster.host_staging_traffic_gb_str` of host-memory traffic: one write into host RAM and one NIC read back out.
...
Conventional kernel TCP paths become increasingly expensive at `{python} IbNdrTransportRecap.ib_ndr_gbit_per_s_str` link rates. Interrupt processing, protocol work, and payload copies can consume substantial CPU time and memory bandwidth, especially when many streams or network interface cards (NICs) drive a host concurrently. Modern TCP stacks can sustain high throughput with segmentation offload, receive-side scaling, zero-copy interfaces, and careful non-uniform memory access (NUMA) placement, so one stream does not inherently exhaust a dual-socket server's memory bandwidth. The architectural problem is instead that host resources spent moving network data compete with the training process and become harder to provision predictably as aggregate I/O grows.

RDMA bypasses the kernel data path. Offloading transport logic to the NIC allows applications to read and write registered remote memory without per-message kernel intervention. For ML, GPUDirect RDMA\index{GPUDirect RDMA}[^fn-gpudirect-rdma-bypass] extends this principle to accelerator memory [@nvidia2026gpudirectRdma]. Without GPUDirect, an outgoing payload must stage through host memory before the NIC transmits it. GPUDirect instead lets the NIC perform direct memory access (DMA) to or from GPU memory over the PCIe peer-memory path, as @fig-gpudirect-data-path contrasts. For one `{python} NetworkRunningExampleCluster.grad_gb_str` payload, avoiding the host bounce removes about `{python} NetworkRunningExampleCluster.host_staging_traffic_gb_str` of host-memory traffic. The network still transfers the payload, and the PCIe path may comprise multiple DMA transactions; the optimization removes staging copies rather than collapsing the transfer into one transaction.
...
::: {#fig-gpudirect-data-path fig-env="figure" fig-pos="htb" fig-cap="**GPUDirect RDMA data path**: The red traditional path stages an accelerator payload in registered host memory before the NIC reads it. The green GPUDirect path lets the NIC access GPU memory over PCIe, eliminating the host-memory bounce." fig-alt="Single diagram with a red path from GPU memory through registered host memory to the network interface and a green direct path between GPU memory and the network interface."}
...
|:---------------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------------------|
| InfiniBand (IB)\index{InfiniBand}                              | Purpose-built HPC switched fabric [@infiniband2000spec] with credit-based flow control, so losslessness is native at the link layer | subnet manager\index{Subnet Manager} and virtual lanes (VLs)\index{Virtual Lane} govern routing and traffic isolation                                                                                                  | Dedicated training clusters where predictable tail latency outweighs Ethernet ecosystem flexibility |
| **RoCE (RDMA over Converged Ethernet)**\index{RoCE!definition} | RoCEv2 carries RDMA semantics by encapsulating InfiniBand transport headers in UDP/IP packets                                       | PFC-based deployments suppress loss with PFC, ECN, and routing policy [@guo2016rdma; @gangidi2024rdma]; PFC-free designs instead place more burden on end-to-end congestion control and loss recovery [@mittal2018irn] | Multi-vendor Ethernet fleets that can absorb more congestion-control tuning                         |
...
This stack choice is visible at the protocol boundary. @Fig-ib-roce-stack compares the two stacks, showing how RDMA-based protocols expose a user-space **verbs API**\index{Verbs API!definition} that bypasses the kernel's traditional TCP/IP stack.

::: {#fig-ib-roce-stack fig-env="figure" fig-pos="htb" fig-cap="**Traditional TCP/IP vs. RDMA/RoCE v2 stacks**: The traditional TCP/IP path (left) traverses the application, OS kernel (context switch), Socket API, TCP/UDP, IP/Ethernet, and NIC, with latency of 10--20 μs and two to three CPU-mediated copies. The RDMA/RoCE v2 path (right) exposes a user-space Verbs API that bypasses the kernel, offloading transport to the RDMA NIC (GPUDirect reads GPU HBM directly), with latency of 1--3 μs and zero-copy CPU overhead." fig-alt="Two protocol stacks. Left Traditional TCP/IP: Application, OS Kernel, Socket, TCP/UDP, IP, NIC (10-20 μs, 2 to 3 copies). Right RDMA/RoCE v2: user-space Verbs API bypasses kernel to RDMA NIC with GPUDirect (1-3 μs, zero copy)."}
...
For the modeled NDR InfiniBand path with $\alpha =$ `{python} NetworkAlphaBeta.ib_alpha_us_str` and $\beta =$ `{python} NetworkAlphaBeta.ib_beta_gb_per_s_str`, the crossover point $n^* = \alpha \cdot \beta \approx$ `{python} NetworkAlphaBeta.n_cross_ib_kb_str`. These are scenario parameters, not protocol constants: measured startup latency and delivered bandwidth vary with the NIC, switch path, software stack, and contention. Messages smaller than the measured crossover gain little from more bandwidth; messages larger than it gain little from lower latency. That crossover separates two fundamentally different optimization strategies: reducing hop count to lower $\alpha$, or adding delivered path bandwidth to raise $\beta$.
...
**Systems insight**: This scenario places the crossover $(n^* = \alpha\beta)$ near `{python} NetworkAlphaBeta.n_cross_ib_kb_str`. Because gradients are megabytes to gigabytes, their transfers are usually bandwidth dominated, whereas pipeline coordination can remain latency dominated\index{Latency!dominated regime}. A higher line rate produces little benefit for a message whose startup term dominates; transport processing, topology, and hop count then govern the improvement.

:::

To see why this distinction matters in practice, consider two messages that the 175B model training job sends every iteration. The first is a `{python} NetworkAlphaBeta.n_small_kb_str` pipeline-scheduling control message that coordinates microbatch handoffs between pipeline stages. Applying the model: $T(n) \approx$ `{python} NetworkAlphaBeta.t_small_ib_us_str` for the control message. The bandwidth term contributes only `{python} NetworkAlphaBeta.small_bw_pct_str` of the total. Doubling the link speed would save a negligible fraction of a microsecond. For this message, the most direct way to reduce transfer time is to reduce the hop count (which lowers $\alpha$), not to buy faster links.
...
@Fig-alpha-beta-crossover makes these two regimes visible across the full range of message sizes for the two modeled paths. The NDR scenario crosses at approximately `{python} NetworkAlphaBeta.n_cross_ib_kb_str`, while the 100 Gb/s RoCE scenario crosses at about `{python} NetworkAlphaBeta.n_cross_eth_kb_str`. The positions follow from the assumed $\alpha$ and $\beta$ values; they are not universal InfiniBand and RoCE constants. In this comparison, the startup-latency gap dominates small messages but becomes negligible beside the transfer term for multi-megabyte gradients.
...
**Setup**: This scenario assumes a `{python} AllReduceBottleneck.compute_phase_ms_str` compute phase at `{python} AllReduceBottleneck.gpu_util_pct_str` accelerator utilization and an NDR path modeled by `{python} AllReduceBottleneck.alpha_math` and `{python} AllReduceBottleneck.beta_math`. For a GPT-2-scale model, each rank contributes `{python} AllReduceBottleneck.grad_small_gb_str` of FP32 gradients.

**Math**: Applying the ring cost $T_{\text{ring}} \approx \frac{2(N-1)}{N}\frac{M}{\beta} + 2(N-1)\alpha$ gives `{python} AllReduceBottleneck.t_ring_small_math`. The modeled communication fraction is:

`{python} AllReduceBottleneck.comm_frac_display_math`

The collective-communication chapter derives the ring phases and cost expression; this chapter uses the result only to test whether the fabric can keep pace.
...
::: {#fig-fat-tree-detail fig-env="figure" fig-pos="htb" fig-cap="**Nonblocking fat-tree topology**: A three-tier Clos network built from radix-$k$ switches. By ensuring that the number of uplinks at each level matches the number of downlinks, the topology provides full $\\text{BW}_{\\text{bisect}}$ between any two pods. The multiple parallel paths between leaf and spine enable hardware-based adaptive routing to spray packets and avoid congestion." fig-alt="Hierarchical switch diagram with three tiers: Core, Leaf, and GPU Racks. Multiple parallel paths connect switches across tiers, illustrating full bisection bandwidth."}
![](images/svg/fat-tree-detail.svg)
:::

The fat-tree[^fn-fattree-clos-network] is a common default for ML clusters because a non-blocking design can provide full $\text{BW}_{\text{bisect}}$ across arbitrary partitions. This capacity benefits global AllReduce even though implementations such as rings and trees do not require simultaneous all-to-all communication. The network is constructed in hierarchical tiers: leaf switches (ToR) connect directly to servers, spine switches interconnect all leaves within a locality domain known as a pod\index{Pod}, and core switches bind multiple pods together.
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_count, fmt_int, fmt_qty, fmt, fmt_multiple, fmt_percent, fmt_usd, check, fmt_time

class BisectionScaling:
    # ┌── 1. LOAD ──────────────────────────────────────────
    n_gpus = Systems.Clusters.Training_1K.total_accelerators
    beta = Systems.Fabrics.InfiniBand_NDR.bandwidth
    gradient = 100 * GB  # scenario assumption: AllReduce payload per GPU
    oversubscription_ratio = 4.0  # scenario assumption: cost-optimized spine
    cluster_cost_m = 300  # scenario assumption: cluster cost in millions USD
    comm_fraction = 0.30  # scenario assumption: training time spent communicating
    ...
    total_data = gpus_per_side * gradient * ring_factor
    ...
    waste_m = throughput_loss * cluster_cost_m
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(n_gpus % 2 == 0, "Bisection model requires an even GPU count")
    check(abs(time_1_1.to(second).magnitude - 3.99609375) < 1e-9, "Ring AllReduce time should be about 3.996 s")
    check(round(slowdown, 3) == round(oversubscription_ratio, 3), "Slowdown must match oversubscription ratio")
    check(round(waste_m, 1) == 142.1, f"Expected oversubscription waste to be about $142.1M, got ${waste_m:.1f}M")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ...
    bbs_cluster_cost_m_str = fmt_usd(cluster_cost_m * MILLION, precision=0, commas=False, scale="M")
    bbs_comm_fraction_pct_str = fmt_percent(comm_fraction, precision=0, commas=False, style='prose')
    bbs_waste_m_str = fmt_usd(waste_m * MILLION, precision=1, commas=False, scale="M")
```
...
For a bandwidth-optimal ring, ReduceScatter plus AllGather contributes a traffic factor $2(N-1)/N$ = `{python} BisectionScaling.bbs_ring_factor_str` per GPU.
...
2. **Oversubscribed (`{python} BisectionScaling.bbs_oversubscription_ratio_str`:1)**: $\text{BW}_{\text{bisect}} =$ `{python} BisectionScaling.bbs_bw_1_1_gb_per_s_str` divided by `{python} BisectionScaling.bbs_oversubscription_ratio_str` = `{python} BisectionScaling.bbs_bw_4_1_gb_per_s_str` per direction.
...
[^fn-radix-switch-density]: **Switch radix**\index{Switch radix!port density}: High-end switches (for example, NVIDIA Quantum-2) can feature a radix of 64 ports, each at 400 Gb/s. This density allows a two-tier fat-tree to support up to 2,048 GPUs with only two switch hops. As model scale pushes toward 100,000 GPUs, increasing switch radix (to 128 or 256), grouping accelerators into larger local domains, or accepting topology constraints are the main ways to avoid adding more tiers and the resulting latency/cost explosion.

Scaling beyond this requires a three-tier architecture with core switches, enabling the fabric to reach `{python} FatTreeScaling.fat_tree_hosts_str` hosts, but the added scale carries both cost and latency. A non-blocking $k=64$ three-tier tree requires roughly $5k^2/4 \approx$ `{python} FatTreeScaling.fat_tree_switches_str` switches across the core, aggregation, and edge layers; at `{python} FatTreeScaling.switch_cost_low_str` to `{python} FatTreeScaling.switch_cost_high_str` per switch, the switching layer alone represents `{python} FatTreeScaling.infra_cost_low_m_str` to `{python} FatTreeScaling.infra_cost_high_m_str`. Typical **hop counts**\index{Hop count!definition} also rise from 2 for intra-pod traffic (leaf-spine-leaf) to 4 for inter-pod traffic (leaf-spine-core-spine-leaf), adding serialization delay and switch processing time to the $\alpha$ latency term across thousands of synchronization steps. Rail-optimized topologies respond to both pressures by matching the physical cabling pattern to the collective communication pattern.
...
::: {#fig-rail-optimized-cabling fig-env="figure" fig-pos="htb" fig-cap="**Rail-optimized physical wiring**: In dense accelerator nodes (for example, DGX H100), GPUs are partitioned into rails corresponding to their PCIe slot position. GPU 0 in every node connects to Switch Rail 0, GPU 1 to Switch Rail 1, and so on. This architecture isolates same-rank data-parallel traffic on dedicated rails, reducing competition for $\\text{BW}_{\\text{bisect}}$ and the hop count for latency-sensitive exchanges." fig-alt="Physical cabling diagram showing two server nodes, each with 4 GPUs. Cables connect GPU 0s to Rail Switch 0, GPU 1s to Switch Rail 1, and so on."}
...
@Fig-network-topologies gives one illustrative comparison using the stated bandwidth and cost assumptions. Quantitative results depend on radix, endpoint count, link speed, routing, and equipment cost; the topology name alone does not determine either bisection bandwidth or cost per Gb/s.

::: {#fig-network-topologies fig-env="figure" fig-pos="htb" fig-cap="**An illustrative bandwidth-cost scenario for ML network topologies**: Assumed $\\text{BW}_{\\text{bisect}}$ (TB/s, left axis) and cost per Gb/s (right axis) for five topology configurations on a 1,024-GPU cluster: Fat-tree (25.6 TB/s, \\$3.0/Gb/s), Dragonfly (20.0, \\$4.0), Torus 3D (8.0, \\$1.5), Rail-opt (12.8, \\$2.0), and Butterfly (6.4, \\$5.0). These scenario values illustrate the comparison method rather than universal properties of each topology family." fig-alt="Illustrative bar chart of five topology configurations with assumed bisection bandwidth in TB/s and cost per Gb/s; fat-tree has the highest assumed bandwidth and Torus 3D the lowest assumed cost per Gb/s."}
...
The danger of PFC lies in its cascading nature. When a switch port's buffer fills, it sends a PAUSE frame upstream, which can move congestion into the upstream switch and trigger another PAUSE frame. A slow receiver, routing hot spot, or true fan-in burst can therefore pause links unrelated to the original queue. This behavior, known as **congestion spreading**\index{Congestion spreading!definition} or **victim flows**\index{Victim flow!definition}, is the primary operational risk of PFC-based lossless Ethernet. True incast is one trigger, but it is not intrinsic to every collective: a bandwidth-optimal ring exchanges data point to point. Multiple ring flows can still collide at a shared egress or oversubscribed cut, creating a hot spot without a single logical aggregation receiver. @Fig-congestion-cascade isolates the true fan-in case.
...
::: {#fig-congestion-cascade fig-env="figure" fig-pos="htb" fig-cap="**A true incast burst**: This deliberately worst-case fan-in scenario sends 256 sources at 50 GB/s toward one 400 Gb/s (50 GB/s) egress with a 32 MB buffer. The 12.8 TB/s offered load oversubscribes the destination egress by 256$\times$. Flat aggregation or synchronized reads can create this pattern; bandwidth-optimal ring AllReduce does not, because each rank exchanges point-to-point chunks." fig-alt="Eight labeled GPU nodes, with 248 more implied, converge at 50 GB/s each on one switch egress with 400 Gb/s capacity and a 32 MB buffer."}
...
class IncastBurst:
    ...
    bandwidth_delay_product_kb_str = fmt_qty(bandwidth_delay_product, KB, precision=0, commas=False)
...
Synchronized ML traffic is susceptible to both true fan-in and shared-egress hot spots. When a layer finishes backward computation, many ranks can inject traffic at once. A ring AllReduce distributes point-to-point chunks rather than targeting one logical receiver, but its simultaneous flows can still converge on an oversubscribed switch port because of placement or routing. Flat or hierarchical aggregation phases can create true fan-in. Production clusters mitigate both cases through three complementary techniques:
...
In a well-designed cluster, the network fabric acts as a scheduler-aware extension of the system bus, not a passive pipe. The intra-node (NVLink) bandwidth is ~`{python} NetworkFabricsClusterRecap.nvlink_to_ib_ratio_mult_str` higher than inter-node (InfiniBand/RoCE) bandwidth, and one mechanism for hiding this cliff is **communication-computation overlap**\index{Communication-computation overlap!definition}. During the backward pass, gradients for the final layers are computed first. Instead of waiting for the entire backward pass to finish, the system can trigger an asynchronous AllReduce for these gradients while the GPUs continue computing gradients for earlier layers. If the backward pass requires 500 ms of computation and the AllReduce takes 300 ms, ideal overlap could hide the communication cost behind computation, reducing the exposed overhead to $\max(0, 300 - 500) = 0$. In practice, dependency chains and resource contention limit overlap, leaving a **last-mile problem**\index{Last-mile problem!definition}: gradients computed near the end of backpropagation have little subsequent computation behind which to hide their transfer time.
...
The primary tool is the virtual lane (VL) in InfiniBand (or **traffic class**\index{Traffic class!definition} in RoCE), which provides up to 16 independent logical channels on a single physical link. Mapping different traffic types to separate VLs ensures that a saturation event in one lane does not block progress in another. Each VL maintains its own independent credit-based flow control: if the storage traffic for the checkpoint fills up its buffer, the switch pauses only that specific lane. Gradient updates, tagged with a high-priority service level, continue to flow through their reserved lane unimpeded. On the Ethernet side, **enhanced transmission selection**\index{Enhanced transmission selection!definition} provides analogous bandwidth guarantees per traffic class, while advanced switch ASICs can partition their forwarding tables and buffer pools into isolated **network slices**\index{Network slice!definition}, ensuring that congestion in one tenant's slice cannot trigger PFC pauses in another's.
...
At the physical layer, hardware counters answer whether the wire still behaves like the model assumed. Every RDMA NIC and switch maintains these counters, accessible via the `perfquery` utility [@linuxManpages2026perfquery]. **PortXmitData**\index{PortXmitData!definition} and **PortRcvData**\index{PortRcvData!definition} count data in units of four octets, so utilization calculations multiply each counter delta by four to recover bytes. **PortXmitDiscards**\index{PortXmitDiscards!definition} tracks packets dropped by the switch, in a properly configured lossless fabric, discards should be exactly zero. Physical signal integrity is tracked via the **SymbolErrorCounter**\index{SymbolErrorCounter!definition}, which increments on bit-level errors caused by loose cables or dirty optics, and the **LinkDownedCounter**\index{LinkDownedCounter!definition}, which logs link flaps often triggered by intermittent hardware faults or overheating transceivers.
...
The conventional node bundles a fixed ratio of compute, memory, and networking, and @sec-compute-cxl established why that ratio is wasteful when one workload needs more memory capacity per GPU and another needs more network bandwidth: Compute Express Link (CXL) memory pooling lets a processor in one chassis reach memory in another with load/store semantics, and disaggregated designs\index{Architecture!disaggregated} decouple compute, memory, and networking into independently composable pools. The fabric-specific consequence shapes the interconnect hierarchy. Once memory pooling and resource composition cross the node boundary, the rigid distinction between intra-node NVLink domains and inter-node InfiniBand domains softens into a single memory fabric, and the synchronization backbone shifts from a fixed node boundary to a composable one.
...
class BisectionWasteRecap:
    ...
    cluster_cost_m_str = fmt_usd(cluster_cost_m * MILLION, precision=0, commas=False, scale="M")
...
In general-purpose clouds, 4:1 or 8:1 oversubscription at the spine is common because traffic is stochastic. Bulk-synchronous ML training can instead produce coordinated traffic bursts when collectives begin. As shown in the bisection bottleneck analysis (@sec-network-fabrics-fat-tree), a `{python} BisectionWasteRecap.oversub_ratio_b_value_str`:1 oversubscription slows the modeled synchronization by `{python} BisectionWasteRecap.oversub_ratio_b_mult_str`. For a `{python} BisectionWasteRecap.cluster_cost_m_str` cluster where synchronization accounts for `{python} BisectionWasteRecap.comm_fraction_pct_str` of time, this scenario wastes over `{python} BisectionWasteRecap.waste_m_str` in idle GPU cycles.
...
The five-level model shown in @fig-network-five-level-model structures the analysis of high-bandwidth fabrics, ascending from the physics of signal transmission to the architecture of warehouse-scale clusters. The framework reveals that network performance is the product of interactions between physical reach, transport protocols, topology, and congestion control, not link speed in isolation. Level 1 (Wire) established that PAM4 encoding and FEC impose irreducible latency floors constraining cluster diameter. At Level 2 (Transport), InfiniBand's native credit-based flow control and RoCE's deployment-dependent combination of PFC, ECN, and endpoint recovery yield different reliability guarantees, and the $\alpha$-$\beta$ model quantifies the bandwidth-latency trade-offs inherent in distributed collectives.
...
* **Link speed is not fabric speed**: A global AllReduce is limited by the narrowest bisection cut (principle \ref{pri-bisection-bandwidth}), not the fastest advertised port. Topology decides how much purchased accelerator throughput becomes useful training throughput and how much becomes idle silicon.
* **Latency and bandwidth bind differently**: The $\alpha$-$\beta$ model separates startup cost from per-byte transfer cost (principle \ref{pri-alpha-beta-model}), with $n^* = \alpha \cdot \beta$ marking the regime change. Message-size distributions, not vendor peak numbers, determine whether to optimize software latency or hardware bandwidth.
