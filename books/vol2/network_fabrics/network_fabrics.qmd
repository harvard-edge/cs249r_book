...
::: {#fig-network-bandwidth-scaling fig-env="figure" fig-pos="htb" fig-cap="**Historical network bandwidth scaling**: Over the past decade, intra-node interconnects (NVLink) have scaled significantly faster than inter-node fabrics (InfiniBand and Ethernet) and host buses (PCIe), exacerbating the communication wall for distributed training."}
...
data = pd.read_csv("data/bandwidth_trends.csv")

fig, ax = plt.subplots(figsize=(8, 5))
sns.lineplot(data=data, x="Year", y="Bandwidth_Gbps", hue="Technology", marker="o", linewidth=2, ax=ax)
ax.set_yscale("log")
...
::: {#fig-gpudirect-data-path fig-env="figure" fig-pos="htb" fig-cap="**GPUDirect RDMA data path**: RDMA without GPU peer-memory access (top) stages an accelerator payload in registered host memory before the NIC reads it. GPUDirect RDMA (bottom) lets the NIC DMA directly to or from GPU memory over PCIe, eliminating the host-memory bounce path." fig-alt="Two-panel diagram. Top: RDMA without GPUDirect follows GPU to host RAM to NIC. Bottom: GPUDirect RDMA lets the NIC access GPU memory over PCIe."}
...
***bulk synchronous parallel (BSP)***\index{Bulk Synchronous Parallel!definition} is a parallel execution model in which every worker completes a local computation phase, exchanges data with all other workers, and then waits at a global barrier before any worker begins the next phase, which makes the slowest participant the pacing constraint for the entire cluster.
