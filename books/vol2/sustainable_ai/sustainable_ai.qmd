...
[^fn-joule-energy-scale]: **Joule Energy Scale**\index{Joule!definition}: A single A100 GPU at peak load consumes ~400 Joules every second (400 W). Large-model training runs are measured in billions to trillions of joules ($10^9$--$10^{12}$ J), so small per-operation memory or compute inefficiencies scale directly into facility-level energy demand.
...
::: {#fig-ai-compute-growth fig-env="figure" fig-pos="htb" fig-cap="**Historical AI Compute Growth**: The computational requirements for training landmark machine learning models have scaled exponentially, far outpacing the doubling rates associated with hardware improvements."}
...
# Load data
df = pd.read_csv("data/ai_compute_trend.csv")
...
[^fn-pue-efficiency-evolution]: **PUE (Power Usage Effectiveness)**\index{Power Usage Effectiveness!definition}: In the early 2000s, PUE values of 2.0-2.5 were common, meaning more power went to cooling than to computing [@thegreengrid2007pue]. Google's 2009 disclosure of PUE 1.21 proved that free-air cooling could halve data center overhead. The shift from PUE to carbon usage effectiveness and water usage effectiveness reflects a systems-level insight: optimizing watts alone is insufficient when water and carbon constraints bind independently.

### Training-scale energy concentration {#sec-sustainable-ai-training-scale-energy-concentration}

The grid, siting, and heat-reuse constraints become severe because large training campaigns concentrate energy demand into long, synchronized runs. OpenAI's GPT-3[^fn-gpt3-energy-scale] exemplifies this scale: its `{python} Gpt3HouseholdEnergyAnchor.gpt3_training_mwh_str` training run, the chapter's roughly `{python} Gpt3HouseholdEnergyAnchor.gpt3_household_years_str`-household-year anchor, reflects the computation required to train large language models on large datasets, with additional energy overhead from distributed-training communication[^fn-comm-energy-overhead] [@maslej2023artificial].
...
For training-centric research workloads, the training phase often dominates operational emissions because mathematical optimization requires sustained parallel computation.[^fn-optimizer-memory-overhead] As demonstrated by the GPT-3 case study, large language model training runs exemplify this energy intensity. Geographic placement affects emissions: moving an identical workload between hydro-heavy and coal-heavy grids can create tens-fold differences in carbon intensity.[^fn-grid-carbon-variance]
...
Large-scale AI training inherently requires distributed systems coordination, creating additional energy overhead that compounds computational demands. The parallelism strategies examined in @sec-distributed-training-systems introduce network communication costs that can account for 20–40 percent of total energy consumption in large clusters.[^fn-parallelism-communication-overhead] This coordination across thousands of GPUs requires constant synchronization of computational updates and model parameters,[^fn-gradsync-communication-energy] generating data movement between nodes. This communication overhead scales poorly: increasing cluster size can increase networking energy superlinearly for all-to-all communication patterns in gradient aggregation.
...
- Scope 1 (Direct Emissions): Arise from direct company operations, such as backup generators and company-owned power generation.
...
- Scope 3 (Value Chain Emissions): Extend beyond direct control, including semiconductor manufacturing, hardware transportation, and end-of-life disposal of AI accelerators.
...
At higher power densities (above 30 kW per rack), the fan power begins to approach or exceed the compute power, at which point the cooling system is consuming more energy than the computation it supports. The PUE[^fn-pue-facility-metric] metric captures this overhead: a PUE of 1.5 means that for every watt consumed by compute, an additional 0.5 watts is consumed by cooling and power distribution overhead.
...
::: {#fig-jevons-ai fig-env="figure" fig-pos="htb" fig-cap="**Jevons Paradox**: Decreasing computation costs drive increased AI usage, potentially offsetting efficiency gains and leading to higher overall resource consumption; the figure maps this effect, showing how a cost reduction (A to B) fuels demand growth (C to D). This counterintuitive relationship highlights the importance of considering systemic effects when evaluating the environmental impact of AI advancements." fig-alt="Supply-demand graph with downward-sloping curve. Y-axis shows AI service costs dropping 50 percent from point A to B. X-axis shows usage more than doubling from C to D. Shaded regions show efficiency savings offset by increased consumption."}
...
The table's three levers have different deployment caveats: pruning[^fn-pruning-energy-savings] depends on sparsity structure and hardware support, quantization[^fn-quantization-energy-savings] compounds savings across arithmetic and memory movement, and knowledge distillation[^fn-distillation-sustainability-efficiency] amortizes its extra training cost across repeated serving.
...
To balance these restrictions, government incentives play a proactive role. Financial support, tax benefits, and grants for Green AI research can make sustainability a competitive advantage. Governments can also use their public procurement power, mandating that vendors meet sustainability benchmarks such as operating on carbon-neutral data centers or using energy-efficient models. Broader corporate reporting frameworks, such as the Greenhouse Gas Protocol, Task Force on Climate-related Financial Disclosures, and International Sustainability Standards Board, scrutinize Scope 3 emissions\index{Scope 3 Emissions}, encompassing the substantial embodied carbon of GPU procurement and data center construction alongside operational emissions of outsourced cloud compute.
