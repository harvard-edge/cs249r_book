...
PagedAttention provides four direct capacity benefits. By allocating pages only as tokens materialize, it eliminates internal fragmentation. By allowing any free page to serve any active sequence, it abolishes external fragmentation. Sequences can grow dynamically without worst-case reservation, and requests sharing identical prefixes map directly to the same physical pages. Together, these properties turn KV cache capacity into a schedulable resource rather than a fixed per-request reservation.
...
The communication pattern differs from tensor parallelism: instead of all-reduce (*same data to all devices*), expert parallelism uses all-to-all (*different data to different devices* based on routing). This AllToAll pattern creates a critical system challenge: load balancing. If the router consistently sends more tokens to certain experts than others, those experts' GPUs become bottlenecks while other GPUs sit idle. Systems counter this imbalance across both training and serving. An auxiliary loss\index{Auxiliary Loss} penalizes uneven expert assignment during training by rewarding uniform routing probabilities. At runtime, enforcing an expert capacity factor\index{Capacity Factor} caps the maximum tokens an individual expert accepts per batch (typically 1.25$\times$ its fair share), dropping or rerouting the excess. Finally, expert buffering\index{Expert Buffering} queues overflow tokens across subsequent iterations, smoothing out transient load spikes at the cost of modest latency variance. Together, these mechanisms keep expert utilization balanced enough that dynamic routing remains a throughput advantage rather than a straggler source.
...
For the Archetype B recommendation workload, Meta's infrastructure demonstrates embedding sharding at extreme scale, encompassing over 100 TB of embedding tables across 10+ trillion unique entities (dimension 128–256) distributed over `{python} MetaEmbeddingScaleCheck.n_shards_str`+ server shards.

These figures describe infrastructure-level totals, not one dense resident table in which every unique entity has a 128--256-dimensional vector. A dense INT8 table with `{python} MetaEmbeddingScaleCheck.entity_count_t_str` trillion entities and `{python} MetaEmbeddingScaleCheck.min_dimension_str` dimensions would require at least `{python} MetaEmbeddingScaleCheck.dense_int8_tb_str`, while `{python} MetaEmbeddingScaleCheck.storage_tb_str` across the same entity count averages only `{python} MetaEmbeddingScaleCheck.effective_bytes_per_entity_str` bytes per entity. The production system therefore relies on compression, tiering, cold storage, and uneven entity coverage rather than a single fully materialized dense matrix.

To balance memory footprint against extreme access skews, the serving layer tiers embeddings by popularity:

- **Hot embeddings** (top 1 percent by access frequency): Replicated across memory on all shards to eliminate hot spots.
- **Warm embeddings** (next `{python} MetaEmbeddingScaleCheck.warm_pct_str`): Column-sharded with `{python} MetaEmbeddingScaleCheck.tp_parallelism_str`-way parallelism across memory.
- **Cold embeddings** (remaining `{python} MetaEmbeddingScaleCheck.cold_pct_str`): Row-sharded with consistent hashing across NVMe flash storage.

Each inference request requires approximately `{python} MetaEmbeddingScaleCheck.lookups_per_request_str` embedding lookups. Executed naively, these would incur `{python} MetaEmbeddingScaleCheck.lookups_per_request_str` sequential network round trips, saturating the network stack and violating latency budgets. Instead, the pipeline batches and coordinates lookups: the engine accumulates requests across a 1 ms window, deduplicates redundant entity lookups, groups keys by destination shard, and dispatches queries concurrently. As responses stream back, the assembly layer reconstructs feature vectors in parallel with downstream scoring.
...
Evaluating any load-balancing policy therefore requires four complementary signals that balance tail latency against fleet utilization. Maximum queue length dictates worst-case latency, while load variance quantifies how evenly work distributes across replicas. In parallel, utilization spread exposes whether idle capacity coexists with saturated nodes, and decision overhead bounds the routing delay added by the load balancer itself. A policy that optimizes only one of these signals can appear healthy in aggregate while quietly pushing unlucky requests deep into the tail.
...
::: {#fig-pipeline-parallel-routing fig-env="figure" fig-pos="htb" fig-cap="**Pipeline Parallelism Request Flow**: (a) A single request flows sequentially through four stages, offering no latency benefit from parallelism. (b) Multiple requests fill the pipeline: once steady state is reached, throughput scales with the number of pipeline stages, while per-request latency remains approximately constant." fig-alt="Two-part diagram. Top: single request through four stage boxes labeled by layer ranges with GPU labels below. Bottom: Gantt-style grid showing four GPU rows with requests offset by one stage, illustrating pipeline fill and steady-state throughput."}
...
When an LLM fleet reaches memory saturation under intense KV cache pressure, generic backpressure is often insufficient. In response, serving engines invoke three specialized relief valves before dropping traffic outright: evicting older conversational turns from the KV cache to shrink active context windows, disabling speculative decoding to reclaim the GPU memory occupied by draft models, or shedding load by rerouting incoming prompts to smaller, quantized fallback models until primary queues drain. Each degradation mechanism trades output quality or latency for continued availability, which users typically prefer over outright request rejection. The system designer must specify which degradation modes activate at each pressure tier; an interactive assistant, for instance, tolerates context truncation far better than silent quality degradation.
...
For open-ended conversational workloads whose prompts lack exact lexical overlap, semantic caching[^fn-semantic-cache-tradeoff] (matching queries in embedding space rather than on exact syntax) can partially recover hit rates, though at the expense of dedicated vector lookups and potential false-positive retrievals.
...
Time-of-day routing can reduce costs by 20--40 percent for batch workloads while preserving tight SLOs for interactive traffic. Yet scaling and routing across global infrastructure remains capital-intensive: multi-region replication multiplies hardware footprints across every deployment zone. To bend the serving cost curve at its physical foundation, systems must alter the model's underlying numerical representation, compressing weights and activations so they execute faster and demand fewer accelerators through quantization.
