...
::: {#fig-tinytl-architecture fig-env="figure" fig-pos="htb" fig-cap="**TinyTL memory optimization**: The panels compare conventional fine-tuning (top) with TinyTL (bottom). Conventional methods store activations for all layers during backpropagation, whereas TinyTL freezes backbone weights and trains only bias terms, reducing memory requirements for on-device adaptation." fig-alt="Two-panel comparison of MobileNet training. Top: conventional fine-tuning with all layers storing activations in memory. Bottom: TinyTL with frozen backbone and trainable bias terms only, showing reduced memory footprint."}
...
\tikzset{
  Box/.style={align=center,outer sep=0pt,
   ...
   Box11/.style={Box, fill=GreenD,draw=GreenD,minimum height=10mm,text=white,font=\sffamily\bfseries,inner ysep=2pt},
   Box2/.style={Box, fill=BlueL!60,draw=BlueLine},
   Box22/.style={Box, fill=BlueLine,draw=BlueLine,minimum height=10mm,text=white,font=\sffamily\bfseries,inner ysep=2pt},
   Box3/.style={Box, fill=RedL!60,draw=RedLine},
   Box33/.style={Box, fill=RedLine,draw=RedLine,minimum height=10mm,text=white,font=\sffamily\bfseries,inner ysep=2pt},
...
\node[below=2pt of CLI2]{Client 2 $\theta_2^{t+1}$};
...
\draw[LineA](CLI2)-|node[Txt,above,pos=0.25]{(3) Send Updates $\theta_2^{t+1}$ (not raw data)}
