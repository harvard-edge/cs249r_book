...
\noindent
![](images/png/cover_edge_intelligence_blueprint_labeled_print.png){fig-alt="Cloud-to-edge blueprint where a central model reaches heterogeneous devices, private local data stays on device, and update streams return to aggregation."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_edge_intelligence_blueprint_labeled.webp){fig-alt="Cloud-to-edge blueprint where a central model reaches heterogeneous devices, private local data stays on device, and update streams return to aggregation."}
:::
...
_Why must intelligence adapt where it operates rather than remain frozen from distant training?_

A model trained in a data center encounters new conditions when deployed to a user's device. Vocabulary, lighting, acoustics, and usage patterns may differ from the distributions that shaped its parameters, and the gap can widen as behavior evolves. Centralized training cannot anticipate every context or personalize for every user. Raw observations also cannot always travel because of privacy constraints, limited bandwidth, or latency requirements. On-device learning brings adaptation to where the data is produced. In C³ terms, compute moves onto the device, communication shrinks from raw observations to selective updates, and coordination operates within edge power and memory budgets.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Reusable tiny-to-mobile endpoints for heterogeneity prose.
# │
...
# │ Show: Memory, clock, throughput, compute, and power endpoints.
...
class EdgeDeviceSpectrum:
    ...
    tiny_clock = profile.CortexMClock
    mobile_clock = profile.MobileClassClock
    tiny_cpu_mips = float(profile.TinyCpuThroughputMips)
    mobile_cpu_mips = float(profile.MobileCpuThroughputMips)
    sensor_power = profile.SensorPowerLow
    phone_power = profile.FlagshipPhonePowerHigh
    iot_compute = profile.IotMicrocontrollerComputeLow

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    memory_ratio = (flagship_ram / mcu_sram).to("").magnitude
    endpoint_memory_ratio = (flagship_ram / tiny_ram).to("").magnitude
    cpu_throughput_ratio = mobile_cpu_mips / tiny_cpu_mips
    power_ratio = (phone_power / sensor_power).to("").magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(tiny_ram < mcu_sram < mcu_sram_high < flagship_ram, "Device memory endpoints should be ordered.")
    check(50_000 < memory_ratio < 70_000, f"MCU-to-phone memory ratio drifted: {memory_ratio:.0f}x")
    check(500_000 <= endpoint_memory_ratio <= 600_000, f"Tiny-to-phone memory ratio drifted: {endpoint_memory_ratio:.0f}x")
    check(9_999 <= cpu_throughput_ratio <= 10_001, f"Tiny-to-mobile CPU throughput ratio drifted: {cpu_throughput_ratio:.0f}x")
    check(499_000 <= power_ratio <= 501_000, f"Tiny-to-phone power ratio drifted: {power_ratio:.0f}x")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tiny_ram_kb_str = fmt_memory_capacity(tiny_ram, unit=KiB, precision=0, commas=False)
    ...
    tiny_clock_mhz_str = fmt_qty(tiny_clock, unit=ureg.megahertz, precision=0, commas=False)
    mobile_clock_ghz_str = fmt_qty(mobile_clock, unit=ureg.gigahertz, precision=0, commas=False)
    tiny_cpu_mips_str = fmt_rate(tiny_cpu_mips, "MIPS", commas=False, approx=True)
    mobile_cpu_mips_str = fmt_rate(mobile_cpu_mips, "MIPS", commas=True, approx=True)
    cpu_throughput_ratio_mult_str = fmt_multiple(cpu_throughput_ratio, precision=0, commas=True)
    sensor_power_uw_str = fmt_power(sensor_power, unit=microwatt, precision=0, commas=False)
    phone_power_w_str = fmt_power(phone_power, unit=watt, precision=0, commas=False)
    iot_compute_tops_str = fmt_ops_rate(iot_compute, unit=TOPS, precision=2, commas=False)
```
...
These scenarios require on-device learning, where models train and adapt directly on the devices where they operate.[^fn-a11-bionic-enclave] Machine learning then moves from centralized training to distributed learning across heterogeneous devices, each operating under local constraints. Within the fleet stack, this pushes computation toward the limits of device power, memory, and thermal capacity. What makes on-device adaptation feasible is not a change in algorithm but a change in silicon: the mobile accelerator throughput charted in @fig-edge-npu-tops rose by more than an order of magnitude in seven years, moving training-shaped workloads inside the power envelope of a handset.

[^fn-a11-bionic-enclave]: **A11 Bionic (2017)**\index{A11 Bionic!definition}: Apple's first system on chip (SoC) with a dedicated Neural Engine, rated up to 600 billion operations per second for machine-learning inference workloads. The systems consequence was establishing the mobile NPU as a distinct power domain: on-device adaptation becomes more practical when inference-optimized silicon frees thermal headroom for short local training or personalization bursts.

::: {#fig-edge-npu-tops fig-env="figure" fig-pos="htb" fig-cap="**Mobile NPU performance scaling**: Throughput of dedicated mobile neural accelerators by release year, rising from under one TOPS in 2017 to roughly thirty-five by 2024 with a sharp step at the 2023 generation. On-device learning became a systems option only once this curve crossed the throughput a training step demands." fig-alt="Line chart of mobile neural processing unit throughput in TOPS by release year from 2017 to 2024, rising from under one to about thirty-five with a sharp step at the 2023 generation."}
...
ax.plot(data["Year"], data["TOPS"], marker="o", linewidth=2.5, markersize=8, color="#0072B2")

# Annotate points with chip names
for i, row in data.iterrows():
    ax.annotate(row['Chip'], (row['Year'], row['TOPS']), textcoords="offset points", xytext=(0, 10), ha='center', fontsize=9)

ax.set_title("Mobile NPU performance scaling over time", fontsize=12, pad=15)
ax.set_xlabel("Release Year", fontsize=11)
ax.set_ylabel("Performance (TOPS)", fontsize=11)
ax.set_ylim(0, 40)
...
These constraints run through Archetype C (Federated MobileNet) (@sec-introduction-archetype-c-federated-mobilenet-65a3), where autonomy demands that learning happen on-device under severe resource limits. Archetype C represents the extreme end of this spectrum. Operating on milliwatt power budgets, it cannot afford the energy cost of transmitting raw data to the cloud. Data locality is therefore a physical necessity imposed by the power wall, not merely a privacy feature. Quantized training, sparse updates, and federated coordination are the survival strategies that allow Archetype C to exist.
...
1.  **Significance**: It enables Hyper-Personalization and autonomous operation under severe resource constraints. Within the iron law\index{Iron Law}, on-device learning must minimize the total energy consumed per update, because every gradient step draws on limited battery power and competes with other system tasks for the available compute throughput $(R_{\text{peak}})$.
2.  **Distinction**: Unlike Edge Inference\index{Edge Inference}, which only executes a fixed model, on-device learning involves a Local Optimization Loop (forward and backward passes) that requires significantly more memory and compute.
3.  **Common pitfall**: A frequent misconception is that on-device learning requires training from scratch. In reality, it is almost always On-Device Adaptation\index{On-Device Adaptation}: fine-tuning a small subset of a pretrained base model's parameters to fit the specific data distribution of a local environment.
...
On-device learning challenges this established model by enabling systems to train or adapt directly on the device, without relying on constant connectivity to the cloud. The shift reflects changing application requirements and user expectations that demand responsive, personalized, and privacy-preserving[^fn-dp-leakage-bound] machine learning systems.

[^fn-dp-leakage-bound]: [offset=-18mm] **Privacy-Preserving (Differential Privacy)**\index{Differential Privacy!definition}: While federated learning avoids moving raw data, research has shown that raw gradients can be "inverted" to reconstruct input samples. Differential privacy adds calibrated noise to bound the privacy loss contributed by any one record or user under a stated adjacency definition; it reduces inference risk, but the strength of the guarantee depends on $\varepsilon$, $\delta$, composition, and implementation quality.

::: {.column-margin}
```{=latex}
\vspace*{7mm}
```
![](images/svg/vol2_edge_intelligence_margin_001.svg){width="100%" fig-alt="Blue memory ladder comparing a 300 MB app budget with a 75 MB local gradient update, with 25 percent marked as an annotation."}
...
| **Cloud-based fine-tuning with privacy controls** | Centralize adaptation while processing user data in batches during off-peak hours with privacy-preserving techniques such as differential privacy, federated analytics, secure aggregation [@bonawitz2017], or combinations of these controls. | This approach often achieves higher accuracy than resource-constrained on-device updates while maintaining acceptable privacy properties for many applications. |
...
These motivations are grounded in the broader concept of knowledge transfer\index{Knowledge Transfer}, where a pretrained model transfers useful representations to a new task or domain. This foundational principle makes on-device learning both feasible and effective, enabling sophisticated adaptation with minimal local resources. @Fig-transfer-conceptual illustrates how knowledge transfer occurs between closely related tasks such as playing different board games or musical instruments, or across domains that share structure such as from riding a bicycle to driving a scooter. In on-device learning, a model pretrained in the cloud adapts efficiently to a new context using only local data and limited updates, allowing fast adaptation without relearning from scratch even when the new task diverges in input modality or goal.

::: {#fig-transfer-conceptual fig-env="figure" fig-pos="htb" fig-cap="**Knowledge transfer**: Pretrained models accelerate learning on new tasks by using existing representations, as seen by adapting skills between related board games or musical instruments. This transfer extends across domains like bicycle riding and scooter operation, where shared underlying structures allow efficient adaptation with limited new data." fig-alt="Conceptual diagram with transfer learning examples: chess to checkers, violin to cello, bicycle to scooter, showing how shared structural representations transfer between related domains to enable efficient on-device adaptation."}
...
\begin{tikzpicture}[
    ...
    % =========================================================================
    % COLUMN 1: DISCRETE STATE SPACES
    % =========================================================================
    \node[colcard, minimum width=7.2cm, minimum height=10.2cm, anchor=north west] (col1) at (0, 0) {};

    % Header
    \node[anchor=north west, fill=SoftBlue!80, draw=BorderGrey, rounded corners=6pt,
          minimum width=7.2cm, minimum height=0.9cm, inner xsep=10pt] (h1) at (0, 0) {
        \bfseries\color{DarkNavy}\small 1. DISCRETE STATE SPACES
    };

    % Subcard 1: Task Transfer
    \node[subcard, minimum width=6.6cm, anchor=north] (sc1_top) at ([yshift=-1.2cm]col1.north) {
        \begin{minipage}{6.3cm}
            \textbf{\scriptsize\textcolor{DarkNavy}{Spatial Grid Task Transfer}}\\[6pt]
            \centering
            \begin{tikzpicture}[baseline=(c1.center)]
                \node[pill, fill=SoftGrey, draw=BorderGrey, minimum width=2.4cm, minimum height=1.1cm] (c1) {
                    \tiny\textcolor{SlateGrey}{SOURCE TASK}\\[1pt]
                    \textbf{\small\textcolor{DarkNavy}{Chess}}
                };
                \node[pill, fill=SoftGreen!60, draw=EmeraldGreen, minimum width=2.4cm, minimum height=1.1cm, right=0.9cm of c1] (c2) {
                    \tiny\textcolor{EmeraldGreen!80!black}{TARGET TASK}\\[1pt]
                    \textbf{\small\textcolor{EmeraldGreen!90!black}{Checkers}}
                };
                \draw[->, line width=1.3pt, DarkNavy] (c1) -- (c2);
            \end{tikzpicture}\\[6pt]
            \begin{tikzpicture}
                \node[fill=SoftGrey!70, draw=CardBorder, rounded corners=4pt, minimum width=6.1cm, inner sep=5pt, align=left] {
                    \textbf{\tiny\textcolor{SlateGrey}{Shared Inductive Priors:}}\\[2pt]
                    \scriptsize\textcolor{DarkNavy}{\textbullet\ Priors: 2D Cartesian board grid}\\[1pt]
                    \scriptsize\textcolor{DarkNavy}{\textbullet\ Dynamics: Minimax turn search}
                };
            \end{tikzpicture}
        \end{minipage}
    };

    % Subcard 2: Model Adaptation
    \node[subcard, minimum width=6.6cm, anchor=north] (sc1_bot) at ([yshift=-0.25cm]sc1_top.south) {
        \begin{minipage}{6.3cm}
            \textbf{\scriptsize\textcolor{DarkNavy}{On-Device Model Adaptation}}\\[6pt]
            \centering
            \begin{tikzpicture}[node distance=0.4cm]
                \node[pill, fill=SoftBlue!50, draw=DeepBlue, minimum width=5.9cm, inner sep=4pt] (b1) {
                    \textbf{\scriptsize\textcolor{DarkNavy}{Base Feature Extractor ($\boldsymbol{\theta_{\text{base}}}$)}}\\[1pt]
                    \tiny\textcolor{SlateGrey}{Frozen ResNet / ViT board encoder}
                };
                \node[pill, fill=SoftGrey, draw=BorderGrey, minimum width=5.9cm, inner sep=4pt, below=0.35cm of b1] (l1) {
                    \textbf{\scriptsize\textcolor{DarkNavy}{Shared Latent Space ($\boldsymbol{Z \in \mathbb{R}^d}$)}}\\[1pt]
                    \tiny\textcolor{SlateGrey}{Positional evaluation representations}
                };
                \node[pill, fill=SoftGreen!60, draw=EmeraldGreen, minimum width=5.9cm, inner sep=4pt, below=0.35cm of l1] (a1) {
                    \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{Edge Policy Head ($\Delta W_{\mathrm{local}}$)}}\\[1pt]
                    \tiny\textcolor{EmeraldGreen!80!black}{Few-shot LoRA adapter ($<0.5\%$ weights)}
                };
                \draw[->, line width=1.1pt, DeepBlue] (b1) -- (l1);
                \draw[->, line width=1.1pt, EmeraldGreen] (l1) -- (a1);
            \end{tikzpicture}
        \end{minipage}
    };

    % =========================================================================
    % COLUMN 2: SENSORIMOTOR DYNAMICS
    % =========================================================================
    \node[colcard, minimum width=7.2cm, minimum height=10.2cm, anchor=north west] (col2) at (7.6cm, 0) {};

    % Header
    \node[anchor=north west, fill=SoftOrange!90, draw=OrangeBorder, rounded corners=6pt,
          minimum width=7.2cm, minimum height=0.9cm, inner xsep=10pt] (h2) at (7.6cm, 0) {
        \bfseries\color{AmberOrange!90!black}\small 2. SENSORIMOTOR DYNAMICS
    };

    % Subcard 1: Task Transfer
    \node[subcard, minimum width=6.6cm, anchor=north] (sc2_top) at ([yshift=-1.2cm]col2.north) {
        \begin{minipage}{6.3cm}
            \textbf{\scriptsize\textcolor{DarkNavy}{Acoustic Feedback Task Transfer}}\\[6pt]
            \centering
            \begin{tikzpicture}[baseline=(c3.center)]
                \node[pill, fill=SoftGrey, draw=BorderGrey, minimum width=2.4cm, minimum height=1.1cm] (c3) {
                    \tiny\textcolor{SlateGrey}{SOURCE TASK}\\[1pt]
                    \textbf{\small\textcolor{DarkNavy}{Violin}}
                };
                \node[pill, fill=SoftGreen!60, draw=EmeraldGreen, minimum width=2.4cm, minimum height=1.1cm, right=0.9cm of c3] (c4) {
                    \tiny\textcolor{EmeraldGreen!80!black}{TARGET TASK}\\[1pt]
                    \textbf{\small\textcolor{EmeraldGreen!90!black}{Cello}}
                };
                \draw[->, line width=1.3pt, AmberOrange] (c3) -- (c4);
            \end{tikzpicture}\\[6pt]
            \begin{tikzpicture}
                \node[fill=SoftGrey!70, draw=CardBorder, rounded corners=4pt, minimum width=6.1cm, inner sep=5pt, align=left] {
                    \textbf{\tiny\textcolor{SlateGrey}{Shared Inductive Priors:}}\\[2pt]
                    \scriptsize\textcolor{DarkNavy}{\textbullet\ Priors: Continuous intonation tracking}\\[1pt]
                    \scriptsize\textcolor{DarkNavy}{\textbullet\ Dynamics: Real-time pitch feedback}
                };
            \end{tikzpicture}
        \end{minipage}
    };
    ...
    % Subcard 2: Model Adaptation
    \node[subcard, minimum width=6.6cm, anchor=north] (sc2_bot) at ([yshift=-0.25cm]sc2_top.south) {
        \begin{minipage}{6.3cm}
            \textbf{\scriptsize\textcolor{DarkNavy}{On-Device Model Adaptation}}\\[6pt]
            \centering
            \begin{tikzpicture}[node distance=0.4cm]
                \node[pill, fill=SoftOrange!70, draw=WarmOrange!90!black, minimum width=5.9cm, inner sep=4pt] (b2) {
                    \textbf{\scriptsize\textcolor{AmberOrange!90!black}{Base Audio Backbone ($\boldsymbol{\theta_{\text{base}}}$)}}\\[1pt]
                    \tiny\textcolor{SlateGrey}{Pretrained Whisper / audio encoder}
                };
                \node[pill, fill=SoftGrey, draw=BorderGrey, minimum width=5.9cm, inner sep=4pt, below=0.35cm of b2] (l2) {
                    \textbf{\scriptsize\textcolor{DarkNavy}{Shared Latent Space ($\boldsymbol{Z \in \mathbb{R}^d}$)}}\\[1pt]
                    \tiny\textcolor{SlateGrey}{Harmonic frequency representations}
                };
                \node[pill, fill=SoftGreen!60, draw=EmeraldGreen, minimum width=5.9cm, inner sep=4pt, below=0.35cm of l2] (a2) {
                    \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{Edge Pitch Adapter ($\Delta W_{\mathrm{local}}$)}}\\[1pt]
                    \tiny\textcolor{EmeraldGreen!80!black}{Recalibrates bass register offsets}
                };
                \draw[->, line width=1.1pt, AmberOrange] (b2) -- (l2);
                \draw[->, line width=1.1pt, EmeraldGreen] (l2) -- (a2);
            \end{tikzpicture}
        \end{minipage}
    };

    % =========================================================================
    % COLUMN 3: KINEMATIC DYNAMICS
    % =========================================================================
    \node[colcard, minimum width=7.2cm, minimum height=10.2cm, anchor=north west] (col3) at (15.2cm, 0) {};

    % Header
    \node[anchor=north west, fill=SoftGreen!80, draw=MintGreen!60, rounded corners=6pt,
          minimum width=7.2cm, minimum height=0.9cm, inner xsep=10pt] (h3) at (15.2cm, 0) {
        \bfseries\color{EmeraldGreen!90!black}\small 3. KINEMATIC DYNAMICS
    };

    % Subcard 1: Task Transfer
    \node[subcard, minimum width=6.6cm, anchor=north] (sc3_top) at ([yshift=-1.2cm]col3.north) {
        \begin{minipage}{6.3cm}
            \textbf{\scriptsize\textcolor{DarkNavy}{Inertial Control Task Transfer}}\\[6pt]
            \centering
            \begin{tikzpicture}[baseline=(c5.center)]
                \node[pill, fill=SoftGrey, draw=BorderGrey, minimum width=2.4cm, minimum height=1.1cm] (c5) {
                    \tiny\textcolor{SlateGrey}{SOURCE TASK}\\[1pt]
                    \textbf{\small\textcolor{DarkNavy}{Bicycle}}
                };
                \node[pill, fill=SoftGreen!60, draw=EmeraldGreen, minimum width=2.4cm, minimum height=1.1cm, right=0.9cm of c5] (c6) {
                    \tiny\textcolor{EmeraldGreen!80!black}{TARGET TASK}\\[1pt]
                    \textbf{\small\textcolor{EmeraldGreen!90!black}{E-Scooter}}
                };
                \draw[->, line width=1.3pt, EmeraldGreen] (c5) -- (c6);
            \end{tikzpicture}\\[6pt]
            \begin{tikzpicture}
                \node[fill=SoftGrey!70, draw=CardBorder, rounded corners=4pt, minimum width=6.1cm, inner sep=5pt, align=left] {
                    \textbf{\tiny\textcolor{SlateGrey}{Shared Inductive Priors:}}\\[2pt]
                    \scriptsize\textcolor{DarkNavy}{\textbullet\ Priors: Two-wheeled balance manifold}\\[1pt]
                    \scriptsize\textcolor{DarkNavy}{\textbullet\ Dynamics: Countersteering control}
                };
            \end{tikzpicture}
        \end{minipage}
    };
    ...
    % Subcard 2: Model Adaptation
    \node[subcard, minimum width=6.6cm, anchor=north] (sc3_bot) at ([yshift=-0.25cm]sc3_top.south) {
        \begin{minipage}{6.3cm}
            \textbf{\scriptsize\textcolor{DarkNavy}{On-Device Model Adaptation}}\\[6pt]
            \centering
            \begin{tikzpicture}[node distance=0.4cm]
                \node[pill, fill=SoftGreen!60, draw=MintGreen!80!black, minimum width=5.9cm, inner sep=4pt] (b3) {
                    \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{Base Dynamics Core ($\boldsymbol{\theta_{\text{base}}}$)}}\\[1pt]
                    \tiny\textcolor{SlateGrey}{Inertial odometry / state estimator}
                };
                \node[pill, fill=SoftGrey, draw=BorderGrey, minimum width=5.9cm, inner sep=4pt, below=0.35cm of b3] (l3) {
                    \textbf{\scriptsize\textcolor{DarkNavy}{Shared Latent Space ($\boldsymbol{Z \in \mathbb{R}^d}$)}}\\[1pt]
                    \tiny\textcolor{SlateGrey}{Balance manifold \& friction envelope}
                };
                \node[pill, fill=SoftGreen!60, draw=EmeraldGreen, minimum width=5.9cm, inner sep=4pt, below=0.35cm of l3] (a3) {
                    \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{Edge Actuator Head ($\Delta W_{\mathrm{local}}$)}}\\[1pt]
                    \tiny\textcolor{EmeraldGreen!80!black}{Motor torque response adaptation}
                };
                \draw[->, line width=1.1pt, MintGreen!80!black] (b3) -- (l3);
                \draw[->, line width=1.1pt, EmeraldGreen] (l3) -- (a3);
            \end{tikzpicture}
        \end{minipage}
    };
...
@Fig-ondevice-gboard demonstrates two inference-time prediction strategies\index{On-Device Prediction Strategy}. Next-word prediction suggests likely continuations from the current context, while Smart Compose uses on-the-fly rescoring to offer dynamic completions. These mechanisms personalize the immediate interaction through local context; longer-lived model improvement occurs separately when federated training aggregates eligible on-device updates.
...
![](images/png/ondevice_gboard_example.png)
...
Voice interaction technologies present another important application domain with unique acoustic challenges. Wake-word detection[^fn-wakeword-power-budget] and voice interfaces in devices such as smart speakers and earbuds must recognize voice commands quickly and accurately even in noisy or dynamic acoustic environments.
...
These applications show that on-device learning changes the training lifecycle, not only the deployment target. In a centralized system, models train in data centers on aggregated datasets, deploy to clients as static artifacts, and receive periodic updates through offline retraining. This approach offers substantial compute, access to diverse data, and mature debugging and validation pipelines. It also assumes reliable data transfer, acceptable centralized data custody, and infrastructure capable of managing fleet-wide updates---assumptions that edge deployments may violate.
...
Decentralization introduces a new class of systems challenges. Devices may operate with different model versions, leading to behavioral inconsistencies across the deployment fleet. Evaluation and validation grow more complex when clients participate intermittently and execute on heterogeneous hardware [@bonawitz2019towards]. Model updates must be carefully managed to prevent degradation, and safety guarantees become harder to enforce without centralized testing infrastructure.

Managing thousands of heterogeneous edge devices exceeds typical distributed systems complexity. Device heterogeneity extends beyond hardware differences to include varying operating system versions, security patches, network configurations, and power management policies. Large-scale federated systems must tolerate clients that fail eligibility checks, arrive late, or drop out during training rounds [@bonawitz2019towards]. Others have been disconnected for weeks or months, creating persistent coordination challenges.
...
Once deployed, local differences begin to emerge as each device encounters its own unique data distribution. Devices collect data that reflects individual user patterns, environmental conditions, and usage contexts. This data is often non-IID[^fn-noniid-federated-distribution] and noisy, requiring local model adaptation to maintain performance. This transition marks the shift from global generalization to local specialization.
...
::: {#fig-centralized-vs-decentralized fig-env="figure" fig-pos="htb" fig-cap="**Local-only, centralized-cloud, and federated learning**: The panels compare three architectures rather than imply a required evolution: (A) local-only learning, where one device trains with no external communication; (B) centralized-cloud learning, where devices send raw data to a server; and (C) federated learning, where devices send model updates ($\Delta\theta$) to a federated averaging (FedAvg) coordinator. The third design preserves data locality but requires separate privacy controls for updates and metadata." fig-alt="Three panels. A Local Only: one phone with an on-device loop. B Centralized Cloud: four phones send raw data to a cloud server. C Federated Learning: four phones send model updates to a FedAvg coordinator; raw examples stay local, but updates cross the trust boundary."}
...
\begin{tikzpicture}[
    ...
    % COLUMN 1: LOCAL ONLY
    % =========================================================================
    \node[mainpanel, minimum width=7.2cm, minimum height=14.4cm, anchor=north west] (col1) at (0, 0) {};

    % Header
    \node[fill=AmberOrange, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.35cm, yshift=-0.35cm]col1.north west) {A --- LOCAL ONLY};
    \node[anchor=north east, font=\scriptsize\bfseries\color{AmberOrange!90!black}]
        at ([xshift=-0.35cm, yshift=-0.42cm]col1.north east) {Device-Centric};

    % Diagram Area
    \node[diagbox, draw=WarmOrange!80, fill=SoftOrange!15, minimum width=6.5cm, minimum height=3.8cm, anchor=north] (diag1)
        at ([yshift=-1.2cm]col1.north) {};

    % Phone frame inside diag1
    \draw[draw=WarmOrange, fill=white, rounded corners=6pt, line width=1.2pt]
        ([xshift=-1.6cm, yshift=0cm]diag1.center) ++(-1.1, -1.5) rectangle ++(2.2, 3.0);
    % Phone details
    \draw[fill=WarmOrange, draw=none] ([xshift=-1.6cm, yshift=-1.2cm]diag1.center) circle (0.1cm);
    \node[font=\scriptsize\bfseries\color{AmberOrange!90!black}, align=center] at ([xshift=-1.6cm, yshift=0.4cm]diag1.center) {
        Edge Node\\[1pt]
        \tiny\color{SlateGrey}Sensor / Phone\\[3pt]
        \colorbox{SoftOrange}{\tiny\textcolor{AmberOrange!90!black}{Local Buffer}}
    };

    % Train locally box
    \node[draw=OrangeBorder, fill=SoftOrange!80, rounded corners=4pt, font=\scriptsize\bfseries\color{AmberOrange!90!black}, inner sep=5pt] (tl1)
        at ([xshift=1.4cm, yshift=0.3cm]diag1.center) {Train Locally};

    % Dashed loop
    \draw[->, dashed, line width=1.1pt, AmberOrange]
        ([xshift=-0.5cm, yshift=0.3cm]diag1.center) -- (tl1.west);
    \draw[->, dashed, line width=1.1pt, AmberOrange]
        (tl1.south) -- ++(0, -0.6) -| ([xshift=-0.5cm, yshift=-0.5cm]diag1.center);
    \node[fill=SoftOrange!15, inner sep=2pt, font=\tiny\bfseries\color{AmberOrange!90!black}]
        at ([xshift=1.4cm, yshift=-1.1cm]diag1.center) {No external network};

    % Status Callout
    \node[draw=OrangeBorder, fill=SoftOrange!70, rounded corners=4pt, minimum width=6.5cm, inner sep=5pt, anchor=north, align=center] (stat1)
        at ([yshift=-0.25cm]diag1.south) {
        \textbf{\scriptsize\textcolor{AmberOrange!90!black}{Zero Communication Egress}}\\[1pt]
        \tiny\textcolor{SlateGrey}{Raw telemetry strictly quarantined to device SRAM/flash}
    };

    % Advantages & Limitations
    \node[anchor=north west, inner sep=0pt] (text1) at ([xshift=0.35cm, yshift=-0.3cm]stat1.south west) {
        \begin{minipage}{6.5cm}
            \textbf{\scriptsize\textcolor{EmeraldGreen}{Advantages}}\\[2pt]
            {\tiny\textbf{Autonomy:} 100\% offline; zero connectivity required}\\[1pt]
            {\tiny\textbf{Privacy:} Absolute; zero raw data network exposure}\\[1pt]
            {\tiny\textbf{Latency:} Sub-10\,ms local inference \& adaptation}\\[5pt]
            \textbf{\scriptsize\textcolor{CrimsonRed}{Limitations}}\\[2pt]
            {\tiny\textbf{Isolation:} No fleet collaboration or shared knowledge}\\[1pt]
            {\tiny\textbf{Capacity:} Bounded by single-device compute/RAM}\\[1pt]
            {\tiny\textbf{Cold-Start:} Zero prior knowledge on new devices}
        \end{minipage}
    };

    % Profile card
    \node[profilebox, minimum width=6.5cm, anchor=south, inner sep=6pt] (prof1)
        at ([yshift=0.35cm]col1.south) {
        \begin{minipage}{6.2cm}
            \textbf{\tiny\textcolor{SlateGrey}{OPERATIONAL PROFILE}}\\[3pt]
            {\scriptsize\textbf{Latency:} \textcolor{SlateGrey}{$< 10$\,ms (Local execution)}}\\[1pt]
            {\scriptsize\textbf{Privacy:} \textcolor{SlateGrey}{Complete isolation (Max)}}\\[1pt]
            {\scriptsize\textbf{Bandwidth:} \textcolor{SlateGrey}{0\,B/s (Zero egress)}}
        \end{minipage}
    };

    % =========================================================================
    % COLUMN 2: CENTRALIZED CLOUD
    % =========================================================================
    \node[mainpanel, minimum width=7.2cm, minimum height=14.4cm, anchor=north west] (col2) at (7.6cm, 0) {};

    % Header
    \node[fill=DarkNavy, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.35cm, yshift=-0.35cm]col2.north west) {B --- CENTRALIZED CLOUD};
    \node[anchor=north east, font=\scriptsize\bfseries\color{DarkNavy}]
        at ([xshift=-0.35cm, yshift=-0.42cm]col2.north east) {Cloud-Harvested};

    % Diagram Area
    \node[diagbox, draw=DeepBlue!80, fill=SoftBlue!20, minimum width=6.5cm, minimum height=3.8cm, anchor=north] (diag2)
        at ([yshift=-1.2cm]col2.north) {};

    % Cloud Cluster Box
    \node[draw=DeepBlue, fill=white, rounded corners=4pt, line width=1pt, minimum width=5.4cm, minimum height=0.9cm, align=center, anchor=north] (cloud)
        at ([yshift=-0.3cm]diag2.north) {
        \textbf{\scriptsize\textcolor{DarkNavy}{Central Cloud GPU Cluster}}\\[1pt]
        \tiny\textcolor{SlateGrey}{Aggregates All Raw Telemetry}
    };

    % Devices 1-4
    \foreach \i/\lbl in {1/Dev 1, 2/Dev 2, 3/Dev 3, 4/Dev 4} {
        \node[draw=BorderGrey, fill=white, rounded corners=3pt, line width=0.6pt, minimum width=1.2cm, minimum height=0.9cm, align=center] (d\i)
            at ([xshift=(\i-2.5)*1.45cm, yshift=-1.1cm]diag2.center) {
            \tiny\textbf{\textcolor{DarkNavy}{\lbl}}\\[1pt]
            \tiny\textcolor{CrimsonRed}{Raw PII}
        };
        \draw[->, line width=1.1pt, DeepBlue] (d\i.north) -- (d\i.north |- cloud.south);
    }
    \node[fill=white, draw=BorderGrey, rounded corners=2pt, inner xsep=4pt, inner ysep=1.5pt, font=\tiny\bfseries\color{DarkNavy}]
        at ([yshift=0.4cm]diag2.center) {Raw Data Uploads};

    % Status Callout (Red)
    \node[draw=CoralRed, fill=SoftRed!80, rounded corners=4pt, minimum width=6.5cm, inner sep=5pt, anchor=north, align=center] (stat2)
        at ([yshift=-0.25cm]diag2.south) {
        \textbf{\scriptsize\textcolor{CrimsonRed}{Raw Data Leaves Device Boundary}}\\[1pt]
        \tiny\textcolor{CrimsonRed!80!black}{High privacy risk; subject to GDPR / HIPAA regulatory exposure}
    };

    % Advantages & Limitations
    \node[anchor=north west, inner sep=0pt] (text2) at ([xshift=0.35cm, yshift=-0.3cm]stat2.south west) {
        \begin{minipage}{6.5cm}
            \textbf{\scriptsize\textcolor{EmeraldGreen}{Advantages}}\\[2pt]
            {\tiny\textbf{Capacity:} Unbounded cloud FLOPs \& large batches}\\[1pt]
            {\tiny\textbf{Tooling:} Centralized debugging \& eval pipelines}\\[1pt]
            {\tiny\textbf{Convergence:} Fast optimization without stragglers}\\[5pt]
            \textbf{\scriptsize\textcolor{CrimsonRed}{Limitations}}\\[2pt]
            {\tiny\textbf{Bandwidth:} High cellular usage \& ingress cost}\\[1pt]
            {\tiny\textbf{Resilience:} Inoperable during network disconnection}\\[1pt]
            {\tiny\textbf{Privacy:} Severe risk for voice, health, or typing}
        \end{minipage}
    };

    % Profile card
    \node[profilebox, minimum width=6.5cm, anchor=south, inner sep=6pt] (prof2)
        at ([yshift=0.35cm]col2.south) {
        \begin{minipage}{6.2cm}
            \textbf{\tiny\textcolor{SlateGrey}{OPERATIONAL PROFILE}}\\[3pt]
            {\scriptsize\textbf{Latency:} \textcolor{SlateGrey}{100--500\,ms (RTT dependent)}}\\[1pt]
            {\scriptsize\textbf{Privacy:} \textcolor{SlateGrey}{Minimal (Raw data egress)}}\\[1pt]
            {\scriptsize\textbf{Bandwidth:} \textcolor{SlateGrey}{Very High (Uncompressed inputs)}}
        \end{minipage}
    };

    % =========================================================================
    % COLUMN 3: FEDERATED LEARNING
    % =========================================================================
    \node[mainpanel, minimum width=7.2cm, minimum height=14.4cm, anchor=north west] (col3) at (15.2cm, 0) {};

    % Header
    \node[fill=EmeraldGreen, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.35cm, yshift=-0.35cm]col3.north west) {C --- FEDERATED LEARNING};
    \node[anchor=north east, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}]
        at ([xshift=-0.35cm, yshift=-0.42cm]col3.north east) {Privacy-Preserving};

    % Diagram Area
    \node[diagbox, draw=EmeraldGreen!80, fill=SoftGreen!20, minimum width=6.5cm, minimum height=3.8cm, anchor=north] (diag3)
        at ([yshift=-1.2cm]col3.north) {};

    % Coordinator Box
    \node[draw=EmeraldGreen, fill=white, rounded corners=4pt, line width=1pt, minimum width=5.4cm, minimum height=0.9cm, align=center, anchor=north] (coord)
        at ([yshift=-0.3cm]diag3.north) {
        \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{FedAvg Coordinator}}\\[1pt]
        \tiny\textcolor{SlateGrey}{Aggregates Weights: $\theta \leftarrow \sum \frac{n_k}{N} \theta_k$}
    };

    % Devices 1-4
    \node[draw=EmeraldGreen, fill=white, rounded corners=3pt, line width=0.6pt, minimum width=1.2cm, minimum height=0.9cm, align=center] (fd1)
        at ([xshift=-2.175cm, yshift=-1.1cm]diag3.center) {
        \tiny\textbf{\textcolor{DarkNavy}{Dev 1}}\\[1pt]
        \tiny\textcolor{SlateGrey}{Local Data}\\[1pt]
        \tiny\textbf{\textcolor{EmeraldGreen}{$\Delta\theta_1$}}
    };
    \draw[->, dashed, line width=1.1pt, EmeraldGreen] (fd1.north) -- (fd1.north |- coord.south);

    \node[draw=EmeraldGreen, fill=white, rounded corners=3pt, line width=0.6pt, minimum width=1.2cm, minimum height=0.9cm, align=center] (fd2)
        at ([xshift=-0.725cm, yshift=-1.1cm]diag3.center) {
        \tiny\textbf{\textcolor{DarkNavy}{Dev 2}}\\[1pt]
        \tiny\textcolor{SlateGrey}{Local Data}\\[1pt]
        \tiny\textbf{\textcolor{EmeraldGreen}{$\Delta\theta_2$}}
    };
    \draw[->, dashed, line width=1.1pt, EmeraldGreen] (fd2.north) -- (fd2.north |- coord.south);

    \node[draw=EmeraldGreen, fill=white, rounded corners=3pt, line width=0.6pt, minimum width=1.2cm, minimum height=0.9cm, align=center] (fd3)
        at ([xshift=0.725cm, yshift=-1.1cm]diag3.center) {
        \tiny\textbf{\textcolor{DarkNavy}{Dev 3}}\\[1pt]
        \tiny\textcolor{SlateGrey}{Local Data}\\[1pt]
        \tiny\textbf{\textcolor{EmeraldGreen}{$\Delta\theta_3$}}
    };
    \draw[->, dashed, line width=1.1pt, EmeraldGreen] (fd3.north) -- (fd3.north |- coord.south);

    \node[draw=EmeraldGreen, fill=white, rounded corners=3pt, line width=0.6pt, minimum width=1.2cm, minimum height=0.9cm, align=center] (fd4)
        at ([xshift=2.175cm, yshift=-1.1cm]diag3.center) {
        \tiny\textbf{\textcolor{DarkNavy}{Dev 4}}\\[1pt]
        \tiny\textcolor{SlateGrey}{Local Data}\\[1pt]
        \tiny\textbf{\textcolor{EmeraldGreen}{$\Delta\theta_4$}}
    };
    \draw[->, dashed, line width=1.1pt, EmeraldGreen] (fd4.north) -- (fd4.north |- coord.south);
    \node[fill=white, draw=MintGreen, rounded corners=2pt, inner xsep=4pt, inner ysep=1.5pt, font=\tiny\bfseries\color{EmeraldGreen!90!black}]
        at ([yshift=0.4cm]diag3.center) {Model Updates $\Delta\theta$ Only};

    % Status Callout (Green)
    \node[draw=MintGreen, fill=SoftGreen!70, rounded corners=4pt, minimum width=6.5cm, inner sep=5pt, anchor=north, align=center] (stat3)
        at ([yshift=-0.25cm]diag3.south) {
        \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{Data Locality Fully Preserved}}\\[1pt]
        \tiny\textcolor{EmeraldGreen!80!black}{Raw samples never transmitted; ephemeral parameter deltas only}
    };

    % Advantages & Limitations
    \node[anchor=north west, inner sep=0pt] (text3) at ([xshift=0.35cm, yshift=-0.3cm]stat3.south west) {
        \begin{minipage}{6.5cm}
            \textbf{\scriptsize\textcolor{EmeraldGreen}{Advantages}}\\[2pt]
            {\tiny\textbf{Privacy:} Decoupled raw data; secure aggregation}\\[1pt]
            {\tiny\textbf{Diversity:} Learns from population distributions}\\[1pt]
            {\tiny\textbf{Efficiency:} Orders of magnitude less bandwidth}\\[5pt]
            \textbf{\scriptsize\textcolor{CrimsonRed}{Limitations}}\\[2pt]
            {\tiny\textbf{Non-IID Drift:} Client heterogeneity slows learning}\\[1pt]
            {\tiny\textbf{Stragglers:} Device heterogeneity throttles rounds}\\[1pt]
            {\tiny\textbf{Inference Risk:} Inversion requires differential privacy}
        \end{minipage}
    };

    % Profile card
    \node[profilebox, minimum width=6.5cm, anchor=south, inner sep=6pt] (prof3)
        at ([yshift=0.35cm]col3.south) {
        \begin{minipage}{6.2cm}
            \textbf{\tiny\textcolor{SlateGrey}{OPERATIONAL PROFILE}}\\[3pt]
            {\scriptsize\textbf{Latency:} \textcolor{SlateGrey}{$< 10$\,ms (On-device model)}}\\[1pt]
            {\scriptsize\textbf{Privacy:} \textcolor{SlateGrey}{High (Weights/deltas only)}}\\[1pt]
            {\scriptsize\textbf{Bandwidth:} \textcolor{SlateGrey}{Moderate (Compressed $\Delta\theta$ sync)}}
        \end{minipage}
    };

\end{tikzpicture}
```
The comparison from local-only learning (region A) to centralized cloud training (region B) to federated coordination (region C) represents a fundamental shift in machine learning architecture. Each phase introduces distinct operational characteristics, from isolated local models to uniform global models to coordinated distributed learning.
...
Fine-tuning a neural network on a smartphone is constrained by power and heat. A GPU training cluster may consume megawatts, while a smartphone must execute backpropagation within a roughly 10 W thermal budget. At a sustained 10 W draw, an idealized 15 Wh battery would be exhausted in about 1.5 hours, while thermal discomfort or throttling may constrain the workload sooner. On-device learning must therefore use compute, memory, and data sparingly.
...
Beyond the compression baseline, training amplifies memory footprint\index{Memory Footprint}, memory bandwidth\index{Memory Bandwidth}, and hardware utilization\index{Hardware Utilization} pressures in ways that are specific to local adaptation, and these pressures compound rather than add. @Tbl-training-amplification quantifies how compute operations grow 2--3$\times$ and energy consumption can balloon 10--50$\times$, while @fig-training-memory-amplifier shows a representative 9$\times$ Adaptive Moment Estimation (Adam)-style memory budget within that broader range.
...
A representative Adam-style training budget (@fig-training-memory-amplifier) can reach a 9$\times$ memory footprint increase across model scales; the broader 4--12$\times$ range depends on optimizer choice, batch size, activation checkpointing, and how many layers are updated.

::: {#fig-training-memory-amplifier fig-env="figure" fig-pos="htb" fig-cap="**The training memory amplifier**: The bars compare inference and training memory across model scales from 1M to 100M parameters. This illustrative configuration stores weights, gradients, optimizer state, and activations, producing a representative 9$\times$ memory footprint increase." fig-alt="Grouped bar chart comparing inference (weights only) vs. training (weights, gradients, optimizer state, activations) for 1M, 10M, and 100M parameter models. Amplification labels show 9 times increase."}
...
# │
# │ Goal: Grouped bar chart: inference (weights only) vs training (weights +
# │       grads + opt + activations) for 1M/10M/100M params; show 9x amp.
# │ Show: Side-by-side bars; stacked training components; amplification labels.
# │ How: weights, grads, opt_state, activations; bar; matplotlib.
# │
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
# │ Grouped bar chart: inference (weights only) vs training (weights +
import matplotlib.pyplot as plt
import numpy as np
...
# ┌── 2. ARRAYS ────────────────────────────────────────────────────────────────
models = ['1M Params', '10M Params', '100M Params']
x = np.arange(len(models))

weights = np.array([(params * MILLION * BYTES_FP32).to(MB).magnitude for params in [1, 10, 100]])
grads = weights
opt_state = weights * 2
...
# ┌── 3. RENDER ────────────────────────────────────────────────────────────────
rects1 = ax.bar(x - width / 2, weights, width, label='Inference (Weights Only)', color='#2c3e50', alpha=0.9)

rects2_w = ax.bar(x + width / 2, weights, width, label='Weights', color='#95a5a6', edgecolor='white', hatch='//')
rects2_g = ax.bar(x + width / 2, grads, width, bottom=weights, label='Gradients', color='#f39c12', edgecolor='white')
rects2_o = ax.bar(x + width / 2, opt_state, width, bottom=weights + grads, label='Optimizer State', color='#e67e22', edgecolor='white')
rects2_a = ax.bar(
    x + width / 2, activations, width, bottom=weights + grads + opt_state, label='Activations (Batch B)', color='#e74c3c', edgecolor='white'
)

for i, amp in enumerate(amplification):
    ax.annotate(
        f'{amp:.1f}×\n Amplification',
        xy=(x[i] + width / 2, total_training[i]),
        xytext=(0, 5),
...
# ┌── 4. DECORATE ──────────────────────────────────────────────────────────────
ax.set_ylabel('Memory Requirement (MB)', fontsize=12)
ax.set_title('Inference vs. Training Memory Footprint', fontsize=14, pad=15)
ax.set_xticks(x)
...
@Fig-ondevice-pretraining illustrates how the complete training pipeline combines offline pretraining with online adaptive learning on resource-constrained IoT devices. The system first undergoes meta-training with generic data. During deployment, device-specific constraints such as data availability, compute, and memory shape the adaptation strategy by ranking and selecting layers and channels to update. This selective fine-tuning\index{Fine-Tuning!selective, on-device} allows efficient on-device learning within limited resource envelopes.

::: {#fig-ondevice-pretraining fig-env="figure" fig-pos="htb" fig-cap="**On-device pretraining and selective fine-tuning**: The two-stage pipeline first meta-trains the full neural network on generic cloud data. Device constraints then guide an importance-ranking step that selects the layers and channels to update locally while the rest of the network remains frozen." fig-alt="Two-stage pipeline. Left: offline pretraining with full neural network. Center: device constraints (data, compute, memory) feed into importance ranking. Right: selected layers highlighted in green for on-device training."}
...
\node[below=1pt of BB2.north,anchor=north]{Online Adaptive Learning on IoT Devices};
\scoped[on background layer]
\node[LineD,draw=BackLine,inner xsep=3mm,inner ysep=5mm,
minimum height=74mm,
fill=yellow!05,yshift=14.7mm,fit=(T1)(PTB)(CIRCLE1)](BB2){};
\node[below=1pt of BB2.north,anchor=north]{Offline Pre-training};
\end{tikzpicture}
```
Resource-constrained devices use a two-stage learning process. Offline pretraining establishes initial model weights. Online adaptation then selectively updates layers based on available data, compute, and memory. This approach balances model performance with the practical limitations of edge deployment, enabling continuous learning in real-world environments.
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: Model constraints — MobileNetV2 and STM32 sizing.
from mlsysim import BYTES_FP32, BYTES_INT8, Datasets, KB, MB, Models, param
from mlsysim.fmt import (
    check,
    fmt_int,
    fmt_math,
...
class EdgeMobilenetSizing:
    ...
    stm32_sram_kb = 192
    stm32_hidden_inputs = 1_000
    stm32_hidden_outputs = 10
    ...
    mobilenetv2_alpha05_total_mb_str = fmt_qty(alpha05_total, MB, precision=1, commas=False)
    mobilenetv2_alpha05_feature_params_m_str = fmt_params(mobilenetv2_alpha05_feature_params, scale="M", precision=2, commas=False)
    mobilenetv2_alpha05_feature_mb_str = fmt_qty(alpha05_feature, MB, precision=1, commas=False)
    ...
    stm32_hidden_inputs_str = fmt_int(stm32_hidden_inputs, commas=False)
    stm32_hidden_kb_str = fmt_qty(stm32_hidden, KB, precision=3, commas=False)
    stm32_hidden_sram_pct_str = fmt_percent(stm32_hidden_sram_pct / 100, precision=3, commas=False, style='prose')
...
The scale of these constraints becomes concrete across the device spectrum. MobileNetV2, commonly used in mobile vision tasks, requires approximately `{python} EdgeMobilenetSizing.mobilenetv2_alpha10_mb_str` of storage in its standard configuration. While feasible for smartphones with gigabytes of available RAM, this far exceeds the `{python} EdgeDeviceSpectrum.mcu_sram_kb_str` of SRAM and `{python} EdgeDeviceSpectrum.arduino_flash_mb_str` of flash storage on microcontrollers such as the Arduino Nano 33 BLE Sense.[^fn-arduino-memory-wall] On such severely constrained platforms, even a single convolutional layer may exceed available RAM during training due to intermediate feature maps and gradient storage.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Edge on-device learning battery/thermal prose and nearby footnote.
# │
# │ Goal: Keep mobile sustained and burst power ranges sourced from MLSysIM.
# │ Show: Sustained and burst mobile ML power ranges.
...
[^fn-sgd-microcontroller-training]: **SGD (Stochastic Gradient Descent)**\index{Stochastic Gradient Descent!definition}: Updates parameters from single-sample or small-batch gradients, storing only current parameters and their gradients. This minimal memory footprint is what makes SGD one of the few plausible optimizers on microcontrollers: Adaptive Moment Estimation (Adam) requires 3$\times$ the memory of SGD for its per-parameter moment estimates, exceeding the SRAM budget on devices with $<512$ KB. The trade-off is slower convergence, requiring more gradient steps to reach comparable accuracy.

```{python}
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: Neural Engine evolution footnote.
class EdgeMobileNpuRecap:
    mobile_npu_rate = Hardware.Mobile.iPhone15Pro.compute.peak_flops
    from mlsysim.fmt import fmt_ops_rate, check

    check(mobile_npu_rate.to(TOPS).magnitude > 0, "Mobile NPU TOPS should be strictly positive")
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mobile_npu_tops_str = fmt_ops_rate(mobile_npu_rate, unit=TOPS, precision=0, commas=False)
```

Moving up the computational hierarchy, mobile-class hardware represents improvement but still operates under severe constraints. Platforms including the Qualcomm Snapdragon, Apple Neural Engine,[^fn-neuralengine-hardware-evolution] and Google Tensor SoC[^fn-tensorsoc-federated-learning] provide significantly more compute power than microcontrollers, often featuring dedicated AI accelerators and optimized support for 8-bit or mixed-precision[^fn-mixedprecision-edge-quantization] matrix operations. These accelerators offer dedicated matrix multiplication units, on-chip memory hierarchies, and power management features specifically designed for neural network inference, with varying support for local training workloads. While these platforms can support more sophisticated training routines, including full backpropagation over compact models, they still fall far short of the computational throughput and memory bandwidth available in centralized data centers. For instance, training a lightweight transformer[^fn-transformer-mobile-serving] on a smartphone is technically feasible but must be tightly bounded in both time and energy consumption to avoid degrading the user experience, highlighting the persistent tension between learning capabilities and practical deployment constraints.

[^fn-neuralengine-hardware-evolution]: **Apple Neural Engine**\index{Apple Neural Engine!definition}\index{Neural Engine!on-device training}: From the A11 (0.6 TOPS) to flagship-class mobile NPUs around `{python} EdgeMobileNpuRecap.mobile_npu_tops_str`, mobile neural acceleration improved by roughly two orders of magnitude across that device lineage. The systems consequence: fine-tuning a MobileNet classifier takes approximately 2 seconds on the neural accelerator vs. 45 seconds on CPU in this representative calculation, while consuming only approximately 500 mW additional power. This roughly 23$\times$ speedup at minimal power cost is what makes on-device adaptation thermally feasible during normal phone usage, allowing compact personalization jobs to run as short accelerator-assisted bursts within strict thermal and battery policies.

[^fn-tensorsoc-federated-learning]: **Google Tensor SoC (2021)**\index{Google Tensor SoC!definition}: Integrates custom ML acceleration with Android and TensorFlow Lite tooling, making it a useful example of hardware-software co-design for on-device inference and federated workflows. The systems consequence is that accelerator capability alone is insufficient: local adaptation also depends on compiler support, runtime operators, power management, and update orchestration.

[^fn-mixedprecision-edge-quantization]: **Mixed-Precision Training**\index{Mixed-Precision Training!definition}: Assigns different numerical precisions to different operations: FP16 for forward and backward passes, FP32 for parameter accumulation. This halves memory usage and doubles throughput on hardware with Tensor Cores. Mobile implementations go further, using INT8 for inference and FP16 for gradients, which reduces training memory by 4$\times$ compared to full FP32 while keeping accumulation errors bounded through loss scaling.

[^fn-transformer-mobile-serving]: **Lightweight Transformers**\index{Lightweight Transformers!definition}: Mobile-optimized architectures like MobileBERT achieve 4--6$\times$ speedup over full models through knowledge distillation and attention head pruning, retaining 97 percent of BERT-base accuracy at approximately 40 ms inference on mobile CPUs vs. 160 ms for full BERT. The constraint that matters for on-device learning: even these compressed transformers require 50--200 MB for training activations, pushing against the 2--4 GB available on mid-range phones.

Published benchmark results from MLPerf Tiny and official vendor data make these hardware tiers concrete. @Fig-edge-inference-landscape plots inference latency against energy per inference for representative devices, revealing three distinct clusters separated by approximately 100$\times$ in energy consumption. Dedicated neural processors such as the Syntiant Core 2 and STM32N6 NPU achieve keyword spotting in under 5 ms at 30 to 160 microjoules, while edge GPUs like the Jetson AGX Orin deliver sub-millisecond latency at 15 millijoules. The 100$\times$ energy gap between tiers determines which devices can operate on battery power for months vs. hours, fundamentally shaping the feasible design space for on-device learning.

::: {#fig-edge-inference-landscape fig-env="figure" fig-pos="htb" fig-cap="**The edge inference landscape**: Published inference latency and energy measurements from MLPerf Tiny and official benchmarks reveal three distinct hardware tiers separated by approximately 100$\times$ in energy consumption. Dedicated neural processors (Syntiant, STM32N6) achieve keyword spotting in under 5 ms at 30--160 microjoules, while edge GPUs (Jetson Orin) deliver sub-millisecond latency at 15 mJ." fig-alt="Scatter plot of inference latency vs. energy per inference showing three hardware tiers from MCU to edge GPU."}
...
tier_colors = [COLORS["RedLine"], COLORS["OrangeLine"], COLORS["BlueLine"]]
tier_labels = ["MCU/ASIC (Syntiant, STM32N6)", "Edge Accelerator (Coral)", "Edge GPU (Jetson)"]
tier_fills = [COLORS["RedL"], COLORS["OrangeL"], COLORS["BlueL"]]
...
# MCU/ASIC region
ax.axhspan(20, 600, xmin=0, xmax=1, alpha=0.06, color=COLORS["RedLine"])
...
# ┌── 4. DECORATE ──────────────────────────────────────────────────────────────
ax.text(20, 80, "MCU/ASIC Tier", fontsize=8, color=COLORS["RedLine"], fontweight="bold", ha="right", va="center", alpha=0.8)
...
plt.legend(loc='lower left', fontsize=8, frameon=True, facecolor='white', framealpha=0.95, edgecolor='gray', fancybox=True, borderpad=0.6)
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: Mobile memory wall — H100 HBM bandwidth reference.
...
from mlsysim.fmt import check, fmt, fmt_energy, fmt_percent, fmt_power, fmt_qty, fmt_qty_int, fmt_time
from mlsysim import ReferenceStats

class EdgeBattery:
    # ┌── 1. LOAD ──────────────────────────────────────────
    p_train_w = 4.5  # Typical sustained SoC power during training
...
::: {.column-margin}
![](images/svg/edge_intelligence_device_memory_ladder.svg){width="100%" fig-alt="Vertical ladder of four blue bars on a log scale showing device-class memory shrinking by orders of magnitude: smartphone 8 GB, IoT 1 GB, microcontroller flash 4 MB, microcontroller SRAM 520 KB."}
...
The memory bandwidth limitations become particularly acute during training. While inference workloads primarily read model weights sequentially, training requires bidirectional data flow for gradient computation and weight updates. This increased memory traffic can saturate the memory subsystem, creating bottlenecks that limit training throughput regardless of computational capacity. Advanced implementations employ techniques such as gradient checkpointing[^fn-checkpoint-edge-storage] to trade computation for memory [@chen2016training], and mixed-precision training to reduce bandwidth requirements while maintaining numerical stability.

[^fn-checkpoint-edge-storage]: **Gradient Checkpointing**\index{Gradient Checkpointing!definition}: Trades computation for memory by recomputing selected intermediate activations during the backward pass instead of storing all of them. The general checkpointing result is sublinear activation memory at the cost of extra forward computation [@chen2016training]. On edge devices where memory is the binding constraint, this trade-off can make the difference between a model that fits in `{python} EdgeAdaptationTierProfile.budget_keyboard_phone_ram_gb_str` RAM and one that requires `{python} EdgeAdaptationTierProfile.smartphone_ram_gb_str`, but the exact memory and compute factors are architecture- and schedule-dependent.
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: Mobile AI accelerator optimization prose.
class EdgeMobileNpuTierRecap:
    mobile_npu_rate = Hardware.Mobile.iPhone15Pro.compute.peak_flops
    from mlsysim.fmt import fmt_ops_rate
...
| **Information Scarcity**    | • Limited local datasets<br>• Non-IID distributions                                                                        | **Data Efficiency**                                                                                                                                                            |
|                             | • Privacy restrictions                                                                                                     | • Few-shot learning                                                                                                                                                            |
|                             |                                                                                                                            | • Meta-learning<br>• Transfer learning                                                                                                                                         |
...
Personalizing a billion-parameter model on a smartwatch with `{python} EdgeAdaptationTierProfile.wearable_system_memory_mb_str` of total system memory is impossible without abandoning the goal of updating the complete model. Instead of updating the whole model, the system freezes the vast majority of the network and trains only a compact, strategically placed set of parameters. This is the essence of resource-efficient model adaptation.
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_int, fmt_qty, fmt_qty_int
...
\vspace*{-4mm}
```
![](images/svg/vol2_edge_intelligence_margin_002.svg){width="100%" fig-alt="Blue memory ladder comparing a 40 MB full model copy with a 0.2 MB adapter, with 200x marked as an annotation."}
...
- Resource envelope determines which adaptation approaches are feasible. A tiny wearable-class device with `{python} EdgeAdaptationTierProfile.tiny_wearable_ram_mb_str` RAM requires fundamentally different strategies than a smartphone with `{python} EdgeAdaptationTierProfile.smartphone_ram_gb_str`.
...
The selection of adaptation techniques follows the same logic, starting with lightweight approaches (@sec-edge-intelligence-weight-freezing-3407) and progressing to more expressive but resource-intensive methods (@sec-edge-intelligence-sparse-updates-879b). Each technique represents a different point in the engineering trade-off space.

On top of the compression baseline established in @sec-edge-intelligence-design-constraints-c776, model adaptation adds a second move: complete model retraining is neither necessary nor feasible at the edge, so systems use pretrained representations strategically and adapt only the minimal parameter subset required to capture local variations: *preserve what works globally, adapt what matters locally*. Those three criteria turn adaptation into a device-matching decision. Weight freezing fits severe memory limits by updating only bias terms or final layers. Structured updates use low-rank and residual adaptations when the device can afford more expressiveness but not full fine-tuning. Sparse updates reserve scarce training capacity for the parameters with the highest adaptation value.
...
y = W x + b
$$
where $\mathbf{W} \in \mathbb{R}^{m \times n}$ is the weight matrix, $b \in \mathbb{R}^m$ is the bias vector, and $x \in \mathbb{R}^n$ is the input. In full training, gradients are computed and applied to both $W$ and $b$. In bias-only adaptation, the system freezes the weight tensor ($\Delta W = 0$) and computes gradients exclusively for the bias:
$$
W \leftarrow W, \quad b \leftarrow b - \eta \frac{\partial \mathcal{L}}{\partial b}
$$

This reduces the number of stored gradients and optimizer states, enabling training to proceed under memory-constrained conditions. On embedded devices that lack floating-point units, this reduction enables on-device learning. @Lst-bias-adaptation makes the constraint explicit: all convolutional and fully connected weights remain frozen, while only bias terms adapt to local data.
...
The practical effectiveness of this approach is demonstrated by Tiny Transfer Learning (TinyTL) [@cai2020tinytl], a framework explicitly designed to enable efficient adaptation of deep neural networks on microcontrollers and other severely memory-limited platforms. Rather than updating all network parameters during training (impossible on such constrained devices), TinyTL strategically freezes both the convolutional weights and the batch normalization statistics, training only the bias terms and, in some cases, lightweight residual components. This architectural constraint creates a profound shift in memory requirements during backpropagation, since the largest memory consumers (intermediate activations) no longer need to be stored for gradient computation across frozen layers.

@Fig-tinytl-architecture visualizes this architectural impact by contrasting standard training with the TinyTL\index{TinyTL!memory optimization} approach. Where conventional backpropagation requires storing activations across all layers, TinyTL freezes backbone weights and batch normalization statistics, training only the final classifier and lightweight bias modules. This eliminates the need to store activations for frozen layers, making adaptation possible within the severe memory constraints established earlier.

::: {#fig-tinytl-architecture fig-env="figure" fig-pos="htb" fig-cap="**TinyTL memory optimization**: The panels compare conventional fine-tuning with TinyTL. Conventional backpropagation stores activations across all layers, whereas TinyTL freezes backbone weights and trains only bias terms and lightweight residual modules, reducing memory requirements for on-device adaptation." fig-alt="Two-panel comparison of model training. Left: conventional fine-tuning with all layers storing activations in memory. Right: TinyTL with frozen backbone and trainable bias terms and lite residual adapter, showing reduced memory footprint."}
...
\begin{tikzpicture}[
    ...
    % =========================================================================
    % PANEL 1: CONVENTIONAL FINE-TUNING
    % =========================================================================
    \node[panel, minimum width=10.5cm, minimum height=14.4cm, anchor=north west] (p1) at (0, 0) {};

    % Header
    \node[fill=SoftRed!90, draw=CoralRed!60, rounded corners=6pt, anchor=north west,
          minimum width=10.5cm, minimum height=0.9cm, inner xsep=12pt] at (0, 0) {
        \bfseries\scriptsize\textcolor{CrimsonRed}{CONVENTIONAL FINE-TUNING $\cdot$ FULL ACTIVATION CACHING}
    };

    % Inner Architecture Box
    \node[subcard, minimum width=9.7cm, minimum height=6.2cm, anchor=north] (arch1)
        at ([yshift=-1.2cm]p1.north) {};

    % 4 Conv Blocks + Head
    \foreach \i/\w/\a in {1/W_1/A_1, 2/W_2/A_2, 3/W_3/A_3, 4/W_4/A_4} {
        \node[draw=CoralRed!60, fill=SoftRed!40, rounded corners=4pt, line width=0.7pt,
              minimum width=6.6cm, minimum height=0.85cm, align=center] (cb\i)
            at ([xshift=-0.8cm, yshift=2.3cm - (\i-1)*1.05cm]arch1.center) {
            \textbf{\scriptsize\textcolor{CrimsonRed}{Conv Block \i\ (Trainable)}}\\[1pt]
            \tiny\textcolor{SlateGrey}{Weights $\w$ + Activations $\a$ cached}
        };
        \ifnum \i>1
            \pgfmathtruncatemacro{\prev}{\i-1}
            \draw[->, line width=0.8pt, SlateGrey!60] (cb\prev) -- (cb\i);
        \fi
    }

    \node[draw=CrimsonRed, fill=CrimsonRed, text=white, rounded corners=4pt, line width=0.8pt,
          minimum width=6.6cm, minimum height=0.85cm, align=center] (head1)
        at ([xshift=-0.8cm, yshift=-1.9cm]arch1.center) {
        \textbf{\scriptsize Classifier Head (Trainable)}\\[1pt]
        \tiny Full loss computation \& gradient initiation
    };
    \draw[->, line width=0.8pt, SlateGrey!60] (cb4) -- (head1);

    % Full Backprop Arrow
    \draw[->, dashed, line width=1.2pt, CrimsonRed]
        (head1.east) -- ++(0.7, 0) -- ++(0, 4.2) -- node[right, font=\tiny\bfseries, align=left, text width=2.0cm, xshift=2pt] {Full Backprop\\Through All Layers} ([xshift=0.7cm]cb1.east) -- (cb1.east);

    % Memory Bottleneck Box
    \node[draw=CoralRed!60, fill=SoftRed!40, rounded corners=4pt, minimum width=9.7cm, inner sep=6pt, anchor=north] (bot1)
        at ([yshift=-0.25cm]arch1.south) {
        \begin{minipage}{9.3cm}
            \textbf{\scriptsize\textcolor{CrimsonRed}{Activation Memory Bottleneck}}\\[2pt]
            {\tiny\textbf{Cache:} Activations must be retained during forward pass for backward chain rule}\\[1pt]
            {\tiny\textbf{Scaling:} Footprint scales with batch $\times$ resolution $\times$ channels ($\sim 1{,}000$\,KB)}\\[1pt]
            {\tiny\textbf{Constraint:} Peak memory ($> 2{,}000$\,KB) exhausts MCU SRAM budget (512\,KB)}
        \end{minipage}
    };

    % Profile & Gauge Box
    \node[subcard, fill=SoftGrey!60, minimum width=9.7cm, minimum height=2.8cm, anchor=south] (prof1)
        at ([yshift=0.35cm]p1.south) {};

    \node[font=\tiny\bfseries\color{SlateGrey}, anchor=north west] at ([xshift=0.3cm, yshift=-0.22cm]prof1.north west) {MEMORY PROFILE (MICROCONTROLLER CLASS)};
    \node[font=\tiny\bfseries\color{DarkNavy}, anchor=north west] at ([xshift=0.3cm, yshift=-0.6cm]prof1.north west) {Activation Storage Footprint:};
    \node[font=\tiny\bfseries\color{CrimsonRed}, anchor=north east] at ([xshift=-0.3cm, yshift=-0.6cm]prof1.north east) {$\sim 1{,}000$\,KB (Exceeds Limit)};

    % Progress bar (downward rectangle)
    \fill[LightGrey, rounded corners=2.5pt] ([xshift=0.3cm, yshift=-0.95cm]prof1.north west) rectangle ++(9.1cm, -0.25cm);
    \fill[CrimsonRed, rounded corners=2.5pt] ([xshift=0.3cm, yshift=-0.95cm]prof1.north west) rectangle ++(8.8cm, -0.25cm);
    % Ceiling tick
    \draw[dashed, line width=1.1pt, DarkNavy] ([xshift=4.85cm, yshift=-0.85cm]prof1.north west) -- ++(0, -0.45cm);
    \node[font=\tiny\bfseries\color{DarkNavy}, anchor=north] at ([xshift=4.85cm, yshift=-1.35cm]prof1.north west) {MCU SRAM Ceiling (512\,KB)};

    % Status pill
    \node[fill=SoftRed, draw=CoralRed!50, rounded corners=3pt, font=\scriptsize\bfseries\color{CrimsonRed}, inner xsep=8pt, inner ysep=2.5pt]
        at ([yshift=-1.95cm]prof1.north) {$\times$ Out-of-Memory (OOM) Crash on MCU};

    % =========================================================================
    % PANEL 2: TINYTL ARCHITECTURE
    % =========================================================================
    \node[panel, minimum width=10.5cm, minimum height=14.4cm, anchor=north west] (p2) at (11.0cm, 0) {};

    % Header
    \node[fill=SoftGreen!90, draw=MintGreen!60, rounded corners=6pt, anchor=north west,
          minimum width=10.5cm, minimum height=0.9cm, inner xsep=12pt] at (11.0cm, 0) {
        \bfseries\scriptsize\textcolor{EmeraldGreen!90!black}{TINYTL ARCHITECTURE $\cdot$ FROZEN BACKBONE ADAPTATION}
    };

    % Inner Architecture Box
    \node[subcard, minimum width=9.7cm, minimum height=6.2cm, anchor=north] (arch2)
        at ([yshift=-1.2cm]p2.north) {};

    % 4 Frozen Conv Blocks + Head
    \foreach \i in {1, 2, 3, 4} {
        \node[draw=BorderGrey, fill=SoftGrey!80, rounded corners=4pt, line width=0.6pt,
              minimum width=4.3cm, minimum height=0.85cm, align=center] (fcb\i)
            at ([xshift=-2.2cm, yshift=2.3cm - (\i-1)*1.05cm]arch2.center) {
            \textbf{\scriptsize\textcolor{DarkNavy}{Conv Block \i\ (frozen $\ast$)}}\\[1pt]
            \tiny\textcolor{SlateGrey}{0 activation memory cached}
        };
        % Bias pill
        \node[draw=OrangeBorder, fill=SoftOrange!70, rounded corners=3pt, line width=0.6pt,
              minimum width=0.8cm, minimum height=0.65cm, font=\scriptsize\bfseries\color{AmberOrange!90!black}] (b\i)
            at ([xshift=0.7cm, yshift=2.3cm - (\i-1)*1.05cm]arch2.center) {$+b_\i$};

        \ifnum \i>1
            \pgfmathtruncatemacro{\prev}{\i-1}
            \draw[->, line width=0.8pt, SlateGrey!50] (fcb\prev) -- (fcb\i);
        \fi
    }

    % Lite Residual Adapter sidecar
    \node[draw=EmeraldGreen, fill=SoftGreen!40, rounded corners=5pt, line width=1.1pt,
          minimum width=2.2cm, minimum height=1.9cm, align=center] (lra)
        at ([xshift=2.7cm, yshift=0.725cm]arch2.center) {
        \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{Lite Residual}}\\[1pt]
        \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{Adapter}}\\[3pt]
        \tiny\textcolor{SlateGrey}{Trainable ($r=8$)}\\[1pt]
        \tiny\textcolor{DarkNavy}{\textbf{Activations: 30\,KB}}
    };
    % Bypass arrows routing around b2 and b3 cleanly
    \draw[->, line width=1.1pt, EmeraldGreen] ([yshift=0.15cm]fcb2.east) -- ++(0.25, 0) |- ([yshift=0.35cm]lra.west);
    \draw[->, line width=1.1pt, EmeraldGreen] ([yshift=-0.35cm]lra.west) -| ++(-0.85, 0) -- ([yshift=0.15cm]fcb3.east);

    % Classifier Head
    \node[draw=EmeraldGreen!80!black, fill=EmeraldGreen!90!black, text=white, rounded corners=4pt, line width=0.8pt,
          minimum width=5.6cm, minimum height=0.85cm, align=center] (head2)
        at ([xshift=-1.55cm, yshift=-1.9cm]arch2.center) {
        \textbf{\scriptsize Classifier Head (Trainable)}\\[1pt]
        \tiny Updates bias + residual adapter paths only
    };
    \draw[->, line width=0.8pt, SlateGrey!50] (fcb4) -- (fcb4 |- head2.north);

    % Local Backprop Arrow entering lra.east cleanly
    \draw[->, dashed, line width=1.2pt, EmeraldGreen]
        (head2.east) -- ++(2.4, 0) -- ++(0, 2.625) -- (lra.east);
    \node[font=\tiny\bfseries\color{EmeraldGreen!90!black}, rotate=-90]
        at ([xshift=4.35cm, yshift=-0.6cm]arch2.center) {Local Backprop Only};

    % Memory Bounded Box
    \node[draw=MintGreen!60, fill=SoftGreen!50, rounded corners=4pt, minimum width=9.7cm, inner sep=6pt, anchor=north] (bot2)
        at ([yshift=-0.25cm]arch2.south) {
        \begin{minipage}{9.3cm}
            \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{Memory-Bounded Transfer Learning}}\\[2pt]
            {\tiny\textbf{Zero Cache:} Backbone layers frozen (0 cached activations retained during forward pass)}\\[1pt]
            {\tiny\textbf{BitFit Biases:} Lightweight bias tuning shifts domain features ($+b_i$ vectors only)}\\[1pt]
            {\tiny\textbf{Lite Residual:} Sidecar adapter recovers full expressive capacity ($\sim 50$\,KB total RAM)}
        \end{minipage}
    };

    % Profile & Gauge Box
    \node[subcard, fill=SoftGrey!60, minimum width=9.7cm, minimum height=2.8cm, anchor=south] (prof2)
        at ([yshift=0.35cm]p2.south) {};

    \node[font=\tiny\bfseries\color{SlateGrey}, anchor=north west] at ([xshift=0.3cm, yshift=-0.22cm]prof2.north west) {MEMORY PROFILE (MICROCONTROLLER CLASS)};
    \node[font=\tiny\bfseries\color{DarkNavy}, anchor=north west] at ([xshift=0.3cm, yshift=-0.6cm]prof2.north west) {Activation Storage Footprint:};
    \node[font=\tiny\bfseries\color{EmeraldGreen!90!black}, anchor=north east] at ([xshift=-0.3cm, yshift=-0.6cm]prof2.north east) {$\sim 50$\,KB ($\sim 20\times$ Reduction)};

    % Progress bar (downward rectangle)
    \fill[LightGrey, rounded corners=2.5pt] ([xshift=0.3cm, yshift=-0.95cm]prof2.north west) rectangle ++(9.1cm, -0.25cm);
    \fill[EmeraldGreen, rounded corners=2.5pt] ([xshift=0.3cm, yshift=-0.95cm]prof2.north west) rectangle ++(0.9cm, -0.25cm);
    % Ceiling tick
    \draw[dashed, line width=1.1pt, DarkNavy] ([xshift=4.85cm, yshift=-0.85cm]prof2.north west) -- ++(0, -0.45cm);
    \node[font=\tiny\bfseries\color{DarkNavy}, anchor=north] at ([xshift=4.85cm, yshift=-1.35cm]prof2.north west) {MCU SRAM Ceiling (512\,KB)};

    % Status pill
    \node[fill=SoftGreen, draw=MintGreen!60, rounded corners=3pt, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}, inner xsep=8pt, inner ysep=2.5pt]
        at ([yshift=-1.95cm]prof2.north) {$\checkmark$ Fully Executes within 512\,KB MCU SRAM};
...
These design choices allow TinyTL to reduce training memory usage by 10$\times$. For instance, adapting a MobileNetV2 model using TinyTL can reduce the number of updated parameters from over 3 million to fewer than 50,000.[^fn-tinytl-memory-efficiency] Combined with quantization, this allows local adaptation on devices with only a few hundred kilobytes of memory, making on-device learning truly feasible in constrained environments.

```{python}
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: TinyTL memory savings footnote.
class EdgeTinyTlRecap:
    tinytl_param_reduction = 3_400_000 / 50_000
    from mlsysim.fmt import fmt_int, fmt_qty, check

    check(tinytl_param_reduction == 68, f"TinyTL parameter reduction should be 68x, got {tinytl_param_reduction:.1f}x")
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tinytl_param_reduction_mult_str = fmt_multiple(tinytl_param_reduction, commas=False, precision=0)
```

[^fn-tinytl-memory-efficiency]: **TinyTL Memory Savings**\index{TinyTL Memory Savings!definition}: The approximately `{python} EdgeTinyTlRecap.tinytl_param_reduction_mult_str` trainable-parameter reduction (3.4 M to 50 K) collapses MobileNetV2 training memory from approximately 20 MB (weights + activations in FP32) to approximately 600 KB, fitting within a 1 MB microcontroller budget. Real deployments on STM32H7 achieve 85 percent of full fine-tuning accuracy while completing updates in approximately 30 seconds vs. 8 minutes, demonstrating that bias-only adaptation trades expressivity for feasibility on the most constrained hardware.

@Fig-tinytl-memory breaks down this memory profile, contrasting the ~2,100 KB footprint of standard fine-tuning against the ~390 KB footprint of TinyTL relative to the 512 KB on-chip SRAM budget of typical microcontrollers.

::: {#fig-tinytl-memory fig-env="figure" fig-pos="htb" fig-cap="**TinyTL memory breakdown**: Comparison of training memory breakdown between standard fine-tuning and TinyTL on microcontroller hardware. Standard backpropagation requires storing intermediate activations that exceed the 512 KB SRAM ceiling, whereas TinyTL operates comfortably within SRAM by eliminating backbone activation caching." fig-alt="Stacked bar chart comparing Standard Fine-Tuning (~2,100 KB) with TinyTL (~390 KB) against a 512 KB MCU SRAM limit."}
...
\begin{tikzpicture}[
    ...
    \providecolor{SteelBlue}{HTML}{4A7C9B}
    ...
    \providecolor{CardBorder}{HTML}{E2E8F0}
    \providecolor{LightGrey}{HTML}{EDF2F7}
    \providecolor{AmberOrange}{HTML}{D97706}
    \providecolor{WarmOrange}{HTML}{F59E0B}
    \providecolor{SoftOrange}{HTML}{FEF3C7}
    \providecolor{OrangeBorder}{HTML}{FCD34D}
    ...
    \providecolor{PurpleHeart}{HTML}{7C3AED}
    \providecolor{SoftPurple}{HTML}{F3E8FF}

    % Main Container Card
    \node[draw=BorderGrey, fill=white, rounded corners=8pt, line width=0.8pt,
          minimum width=21.0cm, minimum height=13.0cm, anchor=north west] (card) at (0, 0) {};

    % Header Bar
    \node[fill=DarkNavy, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3.5pt, anchor=north west]
        at ([xshift=0.4cm, yshift=-0.4cm]card.north west) {MEMORY PROFILING BREAKDOWN};
    \node[anchor=north west, font=\small\bfseries\color{DarkNavy}]
        at ([xshift=6.4cm, yshift=-0.42cm]card.north west) {MCU SRAM Constraint Evaluation (MCUNet / TinyTL)};

    % Plot Area Box
    \node[draw=BorderGrey!70, fill=white, rounded corners=5pt, line width=0.6pt,
          minimum width=18.4cm, minimum height=7.6cm, anchor=north] (plot)
        at ([yshift=-1.2cm]card.north) {};

    % Coordinate system inside plot:
    \coordinate (O) at ([xshift=1.8cm, yshift=0.8cm]plot.south west);

    % Y-axis gridlines & ticks
    \foreach \kb/\y in {0/0.0, 500/1.375, 1000/2.75, 1500/4.125, 2000/5.5} {
        \draw[BorderGrey!50, line width=0.5pt] ([yshift=\y cm]O) -- ++(15.8cm, 0);
        \node[font=\tiny\color{SlateGrey}, anchor=east] at ([xshift=-0.2cm, yshift=\y cm]O) {\kb\,KB};
    }
    \draw[BorderGrey, line width=0.7pt] (O) -- ++(0, 5.8cm);
    \draw[BorderGrey, line width=0.7pt] (O) -- ++(15.8cm, 0);

    % 512 KB SRAM Ceiling Line: y = 512 * 0.00275 = 1.408cm
    \coordinate (SRAM) at ([yshift=1.408cm]O);
    \draw[dashed, line width=1.3pt, CrimsonRed] (SRAM) -- ++(12.8cm, 0);

    % SRAM Ceiling left badge
    \node[fill=SoftRed, draw=CoralRed, rounded corners=3pt, font=\tiny\bfseries\color{CrimsonRed}, inner xsep=4pt, inner ysep=2pt, anchor=west]
        at ([xshift=0.15cm]SRAM) {512\,KB MCU SRAM};
    % SRAM Ceiling badge between bars
    \node[fill=SoftRed, draw=CoralRed!80, rounded corners=3pt, font=\tiny\bfseries\color{CrimsonRed}, inner xsep=5pt, inner ysep=2pt, anchor=west]
        at ([xshift=7.2cm]SRAM) {Exceeds SRAM 4.1$\times$};

    % =========================================================================
    % BAR (a): STANDARD FINE-TUNING (x = 3.2cm from O)
    % =========================================================================
    \coordinate (B1) at ([xshift=3.2cm]O);
    % Segments:
    % 1. Weights: 340 KB -> height = 0.935cm
    \fill[SoftBlue!80, draw=BorderGrey, line width=0.6pt] (B1) rectangle ++(3.6cm, 0.935cm);
    \node[align=center] at ([xshift=1.8cm, yshift=0.467cm]B1) {
        \scriptsize\textbf{\textcolor{DarkNavy}{Model Weights}}\\[1pt]
        \tiny\textcolor{SlateGrey}{340\,KB}
    };

    % 2. Intermediate Activations: 1,000 KB -> height = 2.75cm (0.935 to 3.685cm)
    \coordinate (B1_act) at ([yshift=0.935cm]B1);
    \fill[SoftOrange!70, draw=OrangeBorder, line width=0.6pt] (B1_act) rectangle ++(3.6cm, 2.75cm);
    \node[align=center] at ([xshift=1.8cm, yshift=1.375cm]B1_act) {
        \textbf{\scriptsize\textcolor{AmberOrange!90!black}{Intermediate Activations}}\\[2pt]
        \scriptsize\textbf{\textcolor{AmberOrange!90!black}{1,000\,KB}}\\[2pt]
        \tiny\textcolor{SlateGrey}{Cached for all backward layers}
    };

    % 3. Weight Gradients: 340 KB -> height = 0.935cm (3.685 to 4.62cm)
    \coordinate (B1_grad) at ([yshift=2.75cm]B1_act);
    \fill[SoftRed!60, draw=CoralRed!80, line width=0.6pt] (B1_grad) rectangle ++(3.6cm, 0.935cm);
    \node[align=center] at ([xshift=1.8cm, yshift=0.467cm]B1_grad) {
        \scriptsize\textbf{\textcolor{CrimsonRed}{Weight Gradients}}\\[1pt]
        \tiny\textcolor{CrimsonRed!90!black}{340\,KB}
    };

    % 4. Optimizer States: 420 KB -> height = 1.155cm (4.62 to 5.775cm)
    \coordinate (B1_opt) at ([yshift=0.935cm]B1_grad);
    \fill[SoftPurple!70, draw=PurpleHeart!60, line width=0.6pt] (B1_opt) rectangle ++(3.6cm, 1.155cm);
    \node[align=center] at ([xshift=1.8cm, yshift=0.577cm]B1_opt) {
        \scriptsize\textbf{\textcolor{PurpleHeart}{Optimizer States}}\\[1pt]
        \tiny\textcolor{PurpleHeart!80!black}{420\,KB (Momenta)}
    };

    % Top badge for (a) at y = 5.775cm
    \node[fill=CrimsonRed, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3.5pt, anchor=south]
        at ([xshift=1.8cm, yshift=5.775cm + 0.15cm]B1) {TOTAL: $\sim 2{,}100$\,KB (OOM $\times$)};

    % Sublabel below bar (a)
    \node[align=center, anchor=north] at ([xshift=1.8cm, yshift=-0.25cm]B1) {
        \textbf{\small\textcolor{DarkNavy}{(a) Standard Fine-Tuning}}\\[2pt]
        \tiny\textcolor{SlateGrey}{Backprop through entire model}
    };

    % =========================================================================
    % BAR (b): TINYTL TRANSFER LEARNING (x = 11.0cm from O)
    % =========================================================================
    \coordinate (B2) at ([xshift=11.0cm]O);
    % Segments:
    % 1. Weights (Frozen): 340 KB -> height = 0.935cm
    \fill[SoftBlue!80, draw=BorderGrey, line width=0.6pt] (B2) rectangle ++(3.6cm, 0.935cm);
    \node[align=center] at ([xshift=1.8cm, yshift=0.467cm]B2) {
        \scriptsize\textbf{\textcolor{DarkNavy}{Model Weights}}\\[1pt]
        \tiny\textcolor{SlateGrey}{340\,KB (Frozen)}
    };

    % 2. Lite Residual Act: 30 KB -> height = 0.0825cm (0.935 to 1.0175cm)
    \coordinate (B2_lra) at ([yshift=0.935cm]B2);
    \fill[MintGreen, draw=EmeraldGreen, line width=0.5pt] (B2_lra) rectangle ++(3.6cm, 0.0825cm);

    % 3. Bias Grads + Head: 20 KB -> height = 0.055cm (1.0175 to 1.0725cm)
    \coordinate (B2_bias) at ([yshift=0.0825cm]B2_lra);
    \fill[EmeraldGreen!80!black, draw=EmeraldGreen, line width=0.5pt] (B2_bias) rectangle ++(3.6cm, 0.055cm);

    % Top badge for (b) at y = 1.0725cm
    \node[fill=EmeraldGreen!90!black, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3.5pt, anchor=south]
        at ([xshift=1.8cm, yshift=0.25cm]B2_bias) {TOTAL: $\sim 390$\,KB (FITS $\checkmark$)};

    % Angled callout lines for thin strips to avoid overlap
    \draw[line width=0.8pt, EmeraldGreen] ([xshift=3.6cm, yshift=0.027cm]B2_bias) -- ++(0.5cm, 0.55cm) -- ++(0.3cm, 0)
        node[right, font=\tiny\bfseries\color{EmeraldGreen!90!black}] {Bias Grads + Head: 20\,KB};
    \draw[line width=0.8pt, MintGreen] ([xshift=3.6cm, yshift=0.041cm]B2_lra) -- ++(0.5cm, -0.45cm) -- ++(0.3cm, 0)
        node[right, font=\tiny\bfseries\color{MintGreen!80!black}] {Lite Residual Act: 30\,KB};

    \node[fill=SoftGreen, draw=MintGreen, rounded corners=3pt, font=\tiny\bfseries\color{EmeraldGreen!90!black}, inner xsep=6pt, inner ysep=2.5pt, anchor=west]
        at ([xshift=4.4cm, yshift=-0.9cm]B2_lra) {76\% of SRAM Budget};

    % Sublabel below bar (b)
    \node[align=center, anchor=north] at ([xshift=1.8cm, yshift=-0.25cm]B2) {
        \textbf{\small\textcolor{DarkNavy}{(b) TinyTL Transfer Learning}}\\[2pt]
        \tiny\textcolor{SlateGrey}{Frozen backbone + lite residual}
    };

    % =========================================================================
    % CURVED ARROW: ~5.4x MEMORY REDUCTION
    % =========================================================================
    \draw[->, dashed, line width=1.5pt, EmeraldGreen]
        ([xshift=6.9cm, yshift=5.9cm]O) to[out=25, in=105]
        node[fill=white, draw=MintGreen, rounded corners=4pt, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}, inner sep=4pt] {$\sim 5.4\times$ Memory Reduction}
        ([xshift=12.8cm, yshift=1.8cm]O);

    % =========================================================================
    % LEGEND & TAKEAWAY
    % =========================================================================
    \node[anchor=south] (leg) at ([yshift=0.85cm]card.south) {
        \begin{tikzpicture}[font=\tiny]
            \fill[SoftBlue!80, draw=BorderGrey] (0, 0) rectangle ++(0.35, 0.22);
            \node[anchor=west] at (0.4, 0.11) {Model Weights};

            \fill[SoftOrange!70, draw=OrangeBorder] (2.4, 0) rectangle ++(0.35, 0.22);
            \node[anchor=west] at (2.8, 0.11) {Activations (Backprop)};

            \fill[SoftRed!60, draw=CoralRed!80] (5.4, 0) rectangle ++(0.35, 0.22);
            \node[anchor=west] at (5.8, 0.11) {Weight Gradients};

            \fill[SoftPurple!70, draw=PurpleHeart!60] (8.2, 0) rectangle ++(0.35, 0.22);
            \node[anchor=west] at (8.6, 0.11) {Optimizer States};

            \fill[MintGreen, draw=EmeraldGreen] (11.0, 0) rectangle ++(0.35, 0.22);
            \node[anchor=west] at (11.4, 0.11) {Lite Residual (TinyTL)};

            \draw[dashed, line width=1.2pt, CrimsonRed] (14.2, 0.11) -- ++(0.6, 0);
            \node[anchor=west, font=\tiny\bfseries\color{CrimsonRed}] at (14.9, 0.11) {512\,KB SRAM Ceiling};
        \end{tikzpicture}
    };

    \node[font=\scriptsize\itshape\color{SlateGrey}, anchor=south]
        at ([yshift=0.35cm]card.south) {Freezing the convolutional backbone eliminates intermediate activation caching, allowing on-device learning to operate strictly within MCU on-chip SRAM.};
...
A common implementation involves inserting residual adapters\index{Residual Adapter!parameter efficient finetuning}, which are small residual bottleneck layers, between existing layers in a pretrained model. Consider a hidden representation $h$ passed between layers. A residual adapter introduces a transformation:
$$
h' = h + A(h)
$$
...
[^fn-lora-adapter-efficiency]: **LoRA (Low-Rank Adaptation)**\index{LoRA!definition}: @hu2021lora learn rank-$r$ decomposition matrices $\mathbf{A}$ and $\mathbf{B}$ instead of updating full weight matrices, reducing trainable parameters by 100--10,000$\times$ while maintaining much of the adaptation quality. For on-device learning, this compression is decisive, but it affects adapter state rather than the full model footprint: a model with `{python} EdgeLoraParameterEfficiency.base_model_params_b_str` parameters has about a `{python} EdgeLoraParameterEfficiency.fp16_weight_footprint_gb_str` FP16 weight footprint before gradients, optimizer state, and activations, while rank `{python} EdgeAdaptationTierProfile.mid_tier_lora_rank_str` LoRA can reduce the trainable adapter parameters to tens or hundreds of megabytes. Practical phone deployment still requires storing, quantizing, or offloading the frozen base model.

@Fig-adapter-lora-arch illustrates the dual-path architecture of Low-Rank Adaptation (LoRA), separating the frozen pretrained matrix $W_0$ from the low-rank down- and up-projection matrices $A$ and $B$.

::: {#fig-adapter-lora-arch fig-env="figure" fig-pos="htb" fig-cap="**LoRA architecture and parameter efficiency**: The input $x$ passes through both the frozen base weight matrix $W_0$ and the low-rank trainable adapter path consisting of down-projection $A$ and up-projection $B$. At inference, the adapter weights can be folded directly into the base weights with zero additional latency." fig-alt="Architecture diagram of LoRA showing parallel frozen path and low-rank adapter path merging with addition."}
...
\begin{tikzpicture}[
    ...
    % =========================================================================
    % TOP PANEL: PARAMETER EFFICIENCY
    % =========================================================================
    \node[panelcard, minimum width=21.0cm, minimum height=2.2cm, anchor=north west] (topcard) at (0, 0) {};

    % Header
    \node[fill=DarkNavy, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.35cm, yshift=-0.35cm]topcard.north west) {Panel A: Parameter Efficiency};

    % Left: Subtitle & Bar Comparisons
    \node[anchor=north west] at ([xshift=0.35cm, yshift=-0.85cm]topcard.north west) {
        \begin{tikzpicture}
            \node[font=\scriptsize\bfseries\color{DarkNavy}, anchor=west] at (0, 0.95) {Linear Layer Benchmark ($d = 768$, rank $r = 8$)};

            \node[font=\tiny, anchor=west] at (0, 0.5) {Full Fine-Tuning ($d \times d$):};
            \node[font=\tiny\bfseries\color{CrimsonRed}, anchor=east] at (7.6, 0.5) {589,824 parameters (100\%)};
            \fill[CrimsonRed, rounded corners=2pt] (0, 0.22) rectangle (7.6, 0.40);

            \node[font=\tiny, anchor=west] at (0, -0.05) {LoRA Adapters $2 \times (d \times r)$:};
            \node[font=\tiny\bfseries\color{EmeraldGreen!90!black}, anchor=east] at (7.6, -0.05) {12,288 parameters (2.08\%)};
            \fill[LightGrey, rounded corners=2pt] (0, -0.32) rectangle (7.6, -0.14);
            \fill[EmeraldGreen, rounded corners=2pt] (0, -0.32) rectangle (0.2, -0.14);
        \end{tikzpicture}
    };

    % Middle: 97.9% Reduction Card
    \node[draw=CoralRed!60, fill=SoftRed!40, rounded corners=5pt, minimum width=5.8cm, minimum height=1.5cm, align=center, anchor=north east]
        at ([xshift=-6.2cm, yshift=-0.35cm]topcard.north east) {
        \textbf{\scriptsize\textcolor{CrimsonRed}{97.9\% Parameter Reduction}}\\[2pt]
        \tiny\textcolor{SlateGrey}{$2 \times (768 \times 8) = 12{,}288$ vs $589{,}824$ weights}\\[1pt]
        \tiny\textcolor{CoralRed!90!black}{$\sim 48\times$ fewer trainable parameters per transformed layer}
    };

    % Right: FP16 Memory Footprint Card
    \node[draw=MintGreen!70, fill=SoftGreen!50, rounded corners=5pt, minimum width=5.6cm, minimum height=1.5cm, align=center, anchor=north east]
        at ([xshift=-0.35cm, yshift=-0.35cm]topcard.north east) {
        \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{FP16 Memory Footprint}}\\[2pt]
        \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{24.5\,KB per Adapter}}\\[1pt]
        \tiny\textcolor{SlateGrey}{Fits comfortably into MCU / mobile L1/L2 cache}
    };

    % =========================================================================
    % MAIN ARCHITECTURE PANEL
    % =========================================================================
    \node[panelcard, minimum width=21.0cm, minimum height=9.6cm, anchor=north west] (maincard)
        at (0, -2.6cm) {};

    % Input Vector Node
    \node[subbox, minimum width=2.4cm, minimum height=2.8cm, align=center, anchor=west] (input_node)
        at ([xshift=0.8cm, yshift=0.2cm]maincard.west) {
        \colorbox{DarkNavy}{\tiny\bfseries\color{white}{INPUT VECTOR}}\\[8pt]
        \Large\textbf{\textcolor{DarkNavy}{$x$}}\\[4pt]
        \scriptsize\textcolor{SlateGrey}{$x \in \mathbb{R}^d$}\\[3pt]
        \tiny\textcolor{LightSlate}{$d = 768$ dim}
    };

    % Upper Path: Frozen Pretrained Weights Box
    \node[subbox, draw=DeepBlue, minimum width=8.2cm, minimum height=3.3cm, anchor=north west] (frozen_box)
        at ([xshift=4.2cm, yshift=-0.8cm]maincard.north west) {};

    \node[fill=DarkNavy, text=white, rounded corners=2.5pt, font=\tiny\bfseries, inner xsep=4pt, inner ysep=2pt, anchor=north west]
        at ([xshift=0.25cm, yshift=-0.25cm]frozen_box.north west) {FROZEN ($\ast$)};
    \node[font=\tiny\color{SlateGrey}, anchor=north west]
        at ([xshift=2.2cm, yshift=-0.28cm]frozen_box.north west) {Gradient: $\nabla W = 0$};

    % Grid visual for W0
    \draw[draw=BorderGrey, fill=SoftBlue!25, rounded corners=2pt]
        ([xshift=0.8cm, yshift=-2.2cm]frozen_box.north west) rectangle ++(1.6cm, 1.4cm);
    \draw[BorderGrey!60, line width=0.4pt]
        ([xshift=1.33cm, yshift=-2.2cm]frozen_box.north west) -- ++(0, 1.4cm);
    \draw[BorderGrey!60, line width=0.4pt]
        ([xshift=1.86cm, yshift=-2.2cm]frozen_box.north west) -- ++(0, 1.4cm);
    \draw[BorderGrey!60, line width=0.4pt]
        ([xshift=0.8cm, yshift=-1.73cm]frozen_box.north west) -- ++(1.6cm, 0);
    \draw[BorderGrey!60, line width=0.4pt]
        ([xshift=0.8cm, yshift=-1.26cm]frozen_box.north west) -- ++(1.6cm, 0);
    \node[font=\Large\bfseries\color{DarkNavy}]
        at ([xshift=1.6cm, yshift=-1.5cm]frozen_box.north west) {$W_0$};

    % Metadata beside grid
    \node[anchor=west, align=left] at ([xshift=3.1cm, yshift=-1.5cm]frozen_box.north west) {
        \textbf{\scriptsize\textcolor{DarkNavy}{Pretrained Weights}}\\[1pt]
        \scriptsize\textcolor{DarkNavy}{$W_0 \in \mathbb{R}^{d \times d}$}\\[1pt]
        \tiny\textcolor{SlateGrey}{Dimensions: $768 \times 768$}\\[1pt]
        \tiny\textcolor{SlateGrey}{Weights: 589,824 params}
    };

    % Output pill at bottom inside frozen_box
    \node[fill=SoftBlue!80, rounded corners=3pt, font=\scriptsize\bfseries\color{DarkNavy}, inner xsep=8pt, inner ysep=2.5pt, anchor=south]
        at ([yshift=0.2cm]frozen_box.south) {$h_{\mathrm{base}} = W_0 \cdot x$};

    % Lower Path: Trainable LoRA Adapter Box
    \node[subbox, draw=EmeraldGreen, line width=1pt, minimum width=8.2cm, minimum height=4.3cm, anchor=south west] (lora_box)
        at ([xshift=4.2cm, yshift=0.6cm]maincard.south west) {};

    \node[fill=EmeraldGreen, text=white, rounded corners=2.5pt, font=\tiny\bfseries, inner xsep=4pt, inner ysep=2pt, anchor=north west]
        at ([xshift=0.25cm, yshift=-0.25cm]lora_box.north west) {TRAINABLE (PEFT)};
    \node[font=\tiny\bfseries\color{EmeraldGreen!90!black}, anchor=north west]
        at ([xshift=2.9cm, yshift=-0.28cm]lora_box.north west) {Backprop confined to $A, B$};

    % LoRA Sub-diagram inside lora_box
    % Matrix A (d x r)
    \node[draw=MintGreen, fill=SoftGreen!40, rounded corners=3pt, minimum width=1.1cm, minimum height=1.7cm, align=center, anchor=west] (mA)
        at ([xshift=0.5cm, yshift=0.35cm]lora_box.west) {
        \normalsize\textbf{\textcolor{EmeraldGreen}{A}}\\[2pt]
        \tiny\textcolor{SlateGrey}{$d \times r$}\\[1pt]
        \tiny\textcolor{SlateGrey}{768$\times$8}
    };

    % Down-Proj Pill
    \node[fill=EmeraldGreen!90!black, text=white, rounded corners=3pt, font=\tiny\bfseries, inner sep=3pt, right=0.35cm of mA] (dp) {
        $r = 8$
    };
    \node[font=\tiny\bfseries\color{EmeraldGreen!90!black}, above=1pt of dp] {Down-Proj};
    \draw[->, line width=1.1pt, EmeraldGreen] (mA) -- (dp);

    % Matrix B (r x d)
    \node[draw=MintGreen, fill=SoftGreen!40, rounded corners=3pt, minimum width=1.8cm, minimum height=1.1cm, align=center, right=0.35cm of dp] (mB) {
        \normalsize\textbf{\textcolor{EmeraldGreen}{B}}\\[1pt]
        \tiny\textcolor{SlateGrey}{$r \times d$ ($8 \times 768$)}
    };
    \draw[->, line width=1.1pt, EmeraldGreen] (dp) -- (mB);

    % Scaling factor
    \node[draw=MintGreen, fill=white, rounded corners=3pt, inner sep=3pt, right=0.45cm of mB, align=center] (scale) {
        \scriptsize\textbf{\textcolor{EmeraldGreen}{$\alpha / r$}}\\[1pt]
        \tiny\textcolor{SlateGrey}{$\alpha=16$ ($\times 2.0$)}
    };
    \node[font=\tiny, text=SlateGrey, above=1pt of scale] {Scaling};

    % Formula card at bottom of lora_box
    \node[fill=SoftGreen!70, draw=MintGreen!60, rounded corners=4pt, minimum width=7.7cm, inner sep=4pt, align=center, anchor=south]
        at ([yshift=0.25cm]lora_box.south) {
        \scriptsize\textbf{\textcolor{EmeraldGreen!90!black}{Low-Rank Update: $\Delta W = (\alpha / r) \cdot B \cdot A$}}\\[1pt]
        \tiny\textcolor{SlateGrey}{At initialization: $B = 0 \implies \Delta W = 0$ (exact pretrained identity preservation)}
    };

    % Routing from input_node
    % Main Path arrow
    \draw[->, line width=1.2pt, DeepBlue] (input_node.east) -- ++(0.5, 0) |- (frozen_box.west);
    \node[fill=SoftBlue, draw=BorderGrey, rounded corners=3pt, font=\tiny\bfseries\color{DarkNavy}, inner sep=2pt]
        at ([xshift=0.9cm, yshift=1.4cm]input_node.east) {MAIN PATH};

    % Adapter Path arrow
    \draw[->, line width=1.2pt, EmeraldGreen] (input_node.east) -- ++(0.5, 0) |- (lora_box.west);
    \node[fill=SoftGreen, draw=MintGreen, rounded corners=3pt, font=\tiny\bfseries\color{EmeraldGreen!90!black}, inner sep=2pt]
        at ([xshift=0.9cm, yshift=-1.4cm]input_node.east) {ADAPTER PATH};

    % Addition Node (Summation)
    \coordinate (sum_center) at ([xshift=1.6cm]lora_box.east |- input_node.center);
    \node[draw=DarkNavy, fill=white, line width=1.2pt, circle, inner sep=3pt] (plus_node) at (sum_center) {
        \Large\textbf{\textcolor{DarkNavy}{$+$}}
    };

    % Arrows into Plus Node (Top and Bottom)
    \draw[->, line width=1.2pt, DeepBlue] (frozen_box.east) -- node[above, font=\scriptsize\bfseries\color{DarkNavy}] {$W_0 \cdot x$} (frozen_box.east -| plus_node.north) -- (plus_node.north);
    \draw[->, line width=1.2pt, EmeraldGreen] (lora_box.east) -- node[below, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}] {$\Delta W \cdot x$} (lora_box.east -| plus_node.south) -- (plus_node.south);

    % Output Box
    \node[subbox, draw=SlateGrey, fill=SoftGrey!40, minimum width=4.5cm, minimum height=4.2cm, anchor=west] (out_box)
        at ([xshift=1.2cm]plus_node.east) {};

    \node[fill=DarkNavy, text=white, rounded corners=2.5pt, font=\tiny\bfseries, inner xsep=4pt, inner ysep=2pt, anchor=north]
        at ([yshift=-0.25cm]out_box.north) {OUTPUT ACTIVATION};

    \node[font=\Large\bfseries\color{DarkNavy}, anchor=north] (hprime)
        at ([yshift=-0.75cm]out_box.north) {$h'$};
    \node[font=\scriptsize\bfseries\color{DarkNavy}, anchor=north]
        at ([yshift=-1.35cm]out_box.north) {$h' = W_0 x + (\alpha / r) B A x$};

    \node[fill=white, draw=BorderGrey, rounded corners=4pt, minimum width=4.0cm, inner sep=4pt, align=left, anchor=south]
        at ([yshift=0.25cm]out_box.south) {
        \textbf{\tiny\textcolor{DarkNavy}{Zero-Latency Deployment:}}\\[2pt]
        \tiny\textbf{\textcolor{SlateGrey}{Merge:}} \tiny Fold adapter into base at serve:\\[1pt]
        \scriptsize\textbf{\textcolor{EmeraldGreen}{$W_{\mathrm{eff}} = W_0 + (\alpha / r) B A$}}\\[2pt]
        \tiny\textbf{\textcolor{SlateGrey}{Cost:}} \tiny 0 extra latency or serving FLOPs
    };

    \draw[->, line width=1.3pt, DarkNavy] (plus_node.east) -- (out_box.west);
...
A device-tiered deployment can use different adapter configurations based on device capabilities. Flagship phones use wider bottlenecks for higher expressivity, mid-range devices use moderate bottlenecks for balanced performance, and budget devices use narrow bottlenecks to stay within about `{python} EdgeAdaptationTierProfile.budget_keyboard_phone_ram_gb_str` memory limits. These residual adapter modules can be implemented efficiently on edge devices, particularly when the down-projection and up-projection matrices are small and fixed-point representations are supported. @Lst-lowrank-adapter implements this low-rank adapter\index{Low-Rank Adapter} pattern as a residual bottleneck, showing how a compact down/up projection adds trainable capacity without updating the frozen backbone.
...
**Diagnosis**: Local training risks OS process eviction and user disruption if memory $M_{\text{peak}} \ge 50\text{ MB}$ or battery drops. The pipeline gates training to run strictly when the device is idle, charging, and on Wi-Fi, using quantized update vectors to minimize network payload $D_{\text{vol}} / \text{BW}$.

**Systems lesson**: On-device learning is viable only when local resource bounds ($M_{\text{peak}}$, $E_{\text{battery}}$), network bandwidth ($D_{\text{vol}} / \text{BW}$), and differential privacy are engineered together as an end-to-end systems protocol.

:::

Returning to the adapter mechanism, @lst-lowrank-adapter shows the residual bottleneck that makes the memory savings concrete: the frozen backbone remains untouched, while only the down-projection and up-projection weights are trained.
...
Adapters enable an efficient multi-tenant architecture for edge intelligence, where a single frozen backbone supports multiple personalized contexts. Consider a 10M parameter vision model deployed on a smartphone with `{python} EdgeAdapterRamRecap.smartphone_ram_gb_str` RAM. Full fine-tuning would require storing a separate 40 MB copy of the model weights for each personalization context (home, office, outdoor), rapidly consuming the device's storage. A rank-64 adapter introduces only approximately 50,000 parameters (roughly 200 KB) per context---a 200$\times$ reduction in storage. This efficiency allows the device to store dozens of specialized adapters that share the same frozen backbone, swapping them dynamically based on the user's location or activity.

This **Adapter Switching**\index{Adapter Switching!definition} pattern transforms the smartphone from a static inference engine into a context-aware learning system. When the user enters a low-light environment, the system can load the appropriate adapter into the vision pipeline instead of replacing the entire backbone. The modularity extends naturally to federated learning: in the 10M-parameter example introduced earlier in this section, transmitting a 200 KB adapter is far cheaper than transmitting a 40 MB full-model update over a bandwidth-constrained cellular connection. Residual-adapter studies show that small trainable modules can adapt a shared visual backbone across multiple visual domains [@rebuffi2017learning], making them a useful design point between personalization quality and system resources. The frozen backbone helps preserve general visual representations, while adapters provide a bounded place for user- or context-specific updates without exposing the entire model to catastrophic forgetting, the loss of previously learned general behavior when new updates overwrite it (formalized in @sec-edge-intelligence-experience-replay-737e later in this chapter).
...
In implementation, selective layer updating\index{Selective Layer Updating} turns the ranking into `requires_grad` decisions or equivalent graph masks. @Lst-selective-update extends this pattern to profiling-driven layer selection, demonstrating how hardware profiling determines which layers to update based on contribution scores and memory constraints.

::: {#lst-selective-update lst-cap="**Selective Layer Updating**: This technique allows fine-tuning specific layers of a pretrained model while keeping others frozen, optimizing computational resources for targeted improvements. Source: PyTorch Documentation."}
...
Task-adaptive sparse updates offer the greatest potential for task-specific finetuning by selectively updating only a subset of layers or parameters based on their contribution to downstream performance. While this method allows expressive local adaptation, it requires a mechanism for layer selection, through profiling, contribution analysis, or meta-training, which introduces additional complexity. Nonetheless, when deployed carefully, it allows for dynamic trade-offs between accuracy and efficiency, particularly in systems that experience large domain shifts or evolving input conditions. @Tbl-adaptation-strategies contrasts adaptation strategy trade-offs\index{Adaptation Strategy!trade-offs} across trainable parameters, memory overhead, expressivity, use-case suitability, and system requirements, revealing how the optimal choice depends on application domain, available hardware, latency constraints, and expected distribution shift.
...
- Lifecycle integration determines whether data efficiency techniques fit the model adaptation approaches from @sec-edge-intelligence-model-adaptation-6a82, federated coordination (@sec-edge-intelligence-federated-learning-6e7e), and operational monitoring for model deployment and lifecycle management.
...
::: {.column-margin}
![](images/svg/vol2_edge_intelligence_margin_003.svg){width="100%" fig-alt="Two trend lines: green new-task performance rises while red old-task accuracy falls."}
...
Experience replay addresses catastrophic forgetting in continuous learning scenarios by maintaining a buffer of representative examples from previous learning episodes. This technique, originally developed for reinforcement learning [@mnih2015], proves essential in on-device learning where sequential data streams can cause models to overfit to recent examples. Here, experience replay addresses the immediate stability need; bio-inspired lifelong learning (@sec-edge-intelligence-lifelong-adaptation-without-forgetting-9200) addresses the same stability problem through local plasticity and consolidation mechanisms.
...
A practical on-device implementation might use a ring buffer to store a small set of compressed feature vectors rather than full input examples. @Lst-replay-buffer implements this minimal replay buffer\index{Replay Buffer} design, demonstrating how a circular storage mechanism enables efficient memory management while balancing historical knowledge retention with new information incorporation.
...
In TinyML applications,[^fn-tinyml-deployment-scale] experience replay has been applied to problems such as gesture recognition, where devices must continuously improve predictions while observing a small number of events per day. Instead of training directly on the streaming data, the device stores representative feature vectors from recent gestures and uses them to finetune classification boundaries periodically. Similarly, in on-device keyword spotting, replaying past utterances can improve wake-word detection accuracy without the need to transmit audio data off-device.

[^fn-tinyml-deployment-scale]: **TinyML Scale**\index{TinyML Scale!definition}\index{TinyML!market scale}: Over 100 billion microcontrollers ship annually, but fewer than 1 percent support on-device learning due to memory ($<256$ KB) and power ($<1$ mW) constraints. The gap between deployed hardware and ML-capable hardware defines the engineering challenge: enabling experience replay and adaptation on chips costing under \$1 requires algorithms designed for kilobytes, not gigabytes.
...
1.  **Significance**: It transforms the constraint of data locality\index{Data Locality} into a privacy feature. Within the iron law, federated learning is constrained by the Wide-Area Bandwidth ($\text{BW}$) and the extreme Heterogeneity of the Fleet\index{Heterogeneity of the Fleet}, where device-specific efficiency $(\eta_{\text{hw}})$ and availability can vary by orders of magnitude.
2.  **Distinction**: Unlike Centralized Training\index{Training!centralized}, where data is moved to the compute, federated learning moves compute to the data\index{Compute to the Data}. Raw training examples can remain local, but model updates, participation metadata, and protocol messages still cross the device boundary and may carry personal information.
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt_int, fmt_qty, fmt_qty_int
...
class FederatedSavings:
    ...
    fs_update_mb_str = fmt_qty(update, MB, precision=1, commas=False)
...
::: {.column-margin}
![](images/svg/vol2_edge_intelligence_margin_004.svg){width="100%" fig-alt="Violet network ladder comparing 200 MB of raw upload with a 2.5 MB federated update, with 80x marked as an annotation."}
...
The three-phase evolution introduced in @fig-centralized-vs-decentralized (local-only, centralized cloud, federated) now resolves into federated learning's defining position\index{Edge Learning!paradigm}. @Fig-learning-paradigms restates that comparison with the emphasis the algorithms section needs: offline learning trains centrally and deploys static models; on-device learning adapts locally but in isolation, sharing no insight across users; and federated learning bridges the two by coordinating updates globally while raw data stays local, so it benefits from distributed model improvements without centralizing the data that produced them.

::: {#fig-learning-paradigms fig-env="figure" fig-pos="htb" fig-cap="**Edge learning paradigms**: The panels compare three approaches to model training at the edge. Offline learning trains in the cloud and deploys static models; on-device learning adapts models locally using device data; and federated learning coordinates distributed training while keeping raw data local." fig-alt="Three-panel comparison of learning paradigms. Left: offline learning with cloud training. Center: on-device learning with local model adaptation. Right: federated learning with model updates exchanged between devices and central server."}
...
Across application domains such as Gboard's keyboard personalization, wearable health monitoring, and voice interfaces, compact on-device updates solve only the data-movement side of the problem. Federated learning (FL) adds the coordination layer: it trains a shared model across a population of devices without transferring raw data to a central server [@mcmahan2017communication]. Unlike traditional centralized training pipelines, which require aggregating all training data in a single location, federated learning distributes the training process itself. Each participating device computes updates based on its local data and contributes to a global model through an aggregation protocol, typically coordinated by a central server.

This shift aligns closely with mobile, edge, and embedded systems because it preserves data locality while still improving the shared model. However, the privacy and personalization benefits are real only if the system also handles client variability, communication efficiency, and non-i.i.d. data distributions[^fn-noniid-convergence-bound] through specialized protocols and coordination mechanisms.

[^fn-noniid-convergence-bound]: **Non-IID Data**\index{Non-IID Data!definition}: Data that is *not* independently and identically distributed; in data-center training, data shuffling ensures every batch represents the global distribution. On the edge, each user's data is inherently biased (non-IID): a user in Tokyo mostly types Japanese, while a user in London types English. This bias creates gradient divergence, making standard FedAvg 2--5$\times$ slower to converge than centralized SGD.
...
#### Straggler mitigation {#sec-edge-intelligence-straggler-mitigation-b49d}

In a fleet of millions of heterogeneous devices, waiting for every selected client to report its update is impractical. Network latency, battery depletion, or background process contention can cause some devices ("stragglers") to take 10$\times$ longer than average. At the level of round semantics, the consequence is direct: the weighted aggregation in @eq-fedavg-weighted-aggregation cannot proceed until enough updates arrive, so the round duration is set by the slowest devices the server is still waiting on. The scheduling answer to this, over-selection, is a client-scheduling policy and is developed with the rest of client scheduling in @sec-edge-intelligence-client-scheduling-f675.

This cyclical coordination protocol forms the foundation of federated learning. @Fig-federated-averaging-cycle breaks down the Federated Averaging protocol\index{Federated Averaging Protocol} into four phases: client selection (with over-selection to handle stragglers), local training on private data, parameter upload with optional compression, and weighted aggregation that produces the updated global model.

::: {#fig-federated-averaging-cycle fig-env="figure" fig-pos="htb" fig-cap="**Federated averaging protocol**: The four-phase cycle begins when the server broadcasts a global model to participating clients. Clients train locally on private data, upload model updates, and the server aggregates those updates into the next global model." fig-alt="Four-panel federated averaging cycle. Panel 1: server distributes global model to clients. Panel 2: clients train locally. Panel 3: clients upload model updates. Panel 4: server aggregates updates into new global model."}
...
\begin{tikzpicture}[
    ...
    % =========================================================================
    % TOP CONTAINER: CENTRAL COORDINATOR
    % =========================================================================
    \node[panelcard, draw=DeepBlue, fill=SoftBlue!25, minimum width=18.6cm, minimum height=3.2cm, anchor=north] (coord_box)
        at (0, 0) {};

    \node[fill=DarkNavy, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.35cm, yshift=-0.35cm]coord_box.north west) {CENTRAL COORDINATOR};
    \node[anchor=north west, font=\scriptsize\bfseries\color{DarkNavy}]
        at ([xshift=4.6cm, yshift=-0.38cm]coord_box.north west) {Cloud Fleet Orchestrator};

    % Phase 1 Card (Broadcast)
    \node[subcard, minimum width=8.6cm, minimum height=2.0cm, anchor=north west] (ph1)
        at ([xshift=0.35cm, yshift=-0.9cm]coord_box.north west) {
        \begin{minipage}{8.2cm}
            \textbf{\scriptsize\textcolor{DarkNavy}{Phase 1: Global Model Broadcast}}\\[2pt]
            {\tiny\textbf{Sampling:} Cohort selection of size $C \cdot K$}\\[1pt]
            {\tiny\textbf{Broadcast:} Global weights $\theta^{(t)}$ to selected nodes}\\[4pt]
            \centering
            \colorbox{SoftBlue}{\tiny\bfseries\textcolor{DarkNavy}{Broadcast $\theta^{(t)}$ Downlink}}
        \end{minipage}
    };

    % Phase 4 Card (Aggregation)
    \node[subcard, minimum width=8.6cm, minimum height=2.0cm, anchor=north east] (ph4)
        at ([xshift=-0.35cm, yshift=-0.9cm]coord_box.north east) {
        \begin{minipage}{8.2cm}
            \textbf{\scriptsize\textcolor{DarkNavy}{Phase 4: FedAvg Aggregation}}\\[2pt]
            {\tiny\textbf{Weighted Sum:} Scale by local sample size $n_k$}\\[4pt]
            \centering
            \colorbox{SoftGreen!80}{\scriptsize\bfseries\textcolor{EmeraldGreen!90!black}{$\theta^{(t+1)} \leftarrow \sum \frac{n_k}{N} \theta_k^{(t)}$}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{Advances fleet to communication round $t+1$}}
        \end{minipage}
    };

    % =========================================================================
    % BOTTOM CONTAINER: LOCAL TRAINING
    % =========================================================================
    \node[panelcard, minimum width=21.0cm, minimum height=4.6cm, anchor=north] (client_box)
        at (0, -6.6cm) {};

    \node[fill=AmberOrange, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.35cm, yshift=-0.35cm]client_box.north west) {PHASE 2: ON-DEVICE LOCAL TRAINING};
    \node[anchor=north west, font=\scriptsize\bfseries\color{AmberOrange!90!black}]
        at ([xshift=6.4cm, yshift=-0.38cm]client_box.north west) {Private Data Boundary (Zero External Network Comm During Epochs)};

    % 4 Clients
    \foreach \i/\n/\data/\descr in {
        1/1{,}420/\mathcal{D}_1/Local sensor \& keyboard telemetry,
        2/850/\mathcal{D}_2/Camera frames \& audio samples,
        3/2{,}100/\mathcal{D}_3/Wearable biometric signals,
        4/530/\mathcal{D}_4/App usage \& navigation logs%
    } {
        \node[subcard, minimum width=4.8cm, minimum height=3.3cm, anchor=north] (cl\i)
            at ([xshift=(\i-2.5)*5.05cm, yshift=-1.0cm]client_box.north) {
            \begin{minipage}{4.5cm}
                \colorbox{SoftOrange}{\tiny\bfseries\textcolor{AmberOrange!90!black}{Client \i}}\hfill
                {\tiny\textcolor{SlateGrey}{$n_\i = \n$}}\\[5pt]
                \colorbox{SoftOrange!60}{\begin{minipage}{4.3cm}
                    \tiny\textbf{\textcolor{AmberOrange!90!black}{Private Dataset $\data$}}\\[1pt]
                    \tiny\textcolor{SlateGrey}{\descr}
                \end{minipage}}\\[5pt]
                {\tiny\textbf{SGD Optimization ($E = 5$):}}\\[1pt]
                {\tiny $\theta_\i^{(t)} \leftarrow \theta^{(t)} - \eta \sum \nabla L(w; x)$}\\[6pt]
                \centering
                \colorbox{SoftGreen}{\scriptsize\bfseries\textcolor{EmeraldGreen!90!black}{Update: $\Delta\theta_\i = \theta_\i^{(t)} - \theta^{(t)}$}}
            \end{minipage}
        };
    }

    % =========================================================================
    % COMMUNICATION FLOWS (BETWEEN TOP AND BOTTOM)
    % =========================================================================
    % Downlink flow (left)
    \node[draw=DeepBlue!50, fill=white, rounded corners=4pt, align=center, inner sep=5pt, line width=0.8pt] (downlink)
        at (-6.6, -4.9) {
        \textbf{\scriptsize\textcolor{DarkNavy}{DOWNLINK}}\\[2pt]
        \tiny\textcolor{DarkNavy}{Broadcasts $\theta^{(t)}$}\\[1pt]
        \tiny\textcolor{SlateGrey}{Single global copy}
    };
    \draw[->, line width=1.3pt, DarkNavy]
        ([xshift=-3.5cm]coord_box.south) -- ++(0, -0.4) -| (downlink.north);
    \draw[->, line width=1.3pt, DarkNavy]
        (downlink.south) -- (downlink.south |- client_box.north);

    % Uplink flow (right)
    \node[draw=EmeraldGreen, fill=white, rounded corners=4pt, align=center, inner sep=5pt, line width=0.8pt] (uplink)
        at (6.6, -4.9) {
        \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{UPLINK}}\\[2pt]
        \tiny\textcolor{EmeraldGreen!90!black}{\textbf{Updates $\Delta\theta_k$ Only}}\\[1pt]
        \tiny\textcolor{SlateGrey}{Zero raw data egress}
    };
    \draw[line width=1.3pt, dashed, EmeraldGreen]
        (uplink.south |- client_box.north) -- (uplink.south);
    \draw[->, line width=1.3pt, dashed, EmeraldGreen]
        (uplink.north) |- ([xshift=3.5cm, yshift=-0.4cm]coord_box.south) -- ([xshift=3.5cm]coord_box.south);

    % Center: Round t Iterative Loop
    \draw[->, line width=1.2pt, DeepBlue] (0, -4.9) ++(30:0.75cm) arc (30:150:0.75cm);
    \draw[->, line width=1.2pt, EmeraldGreen] (0, -4.9) ++(210:0.75cm) arc (210:330:0.75cm);
    \node[align=center] at (0, -4.9) {
        \textbf{\scriptsize\textcolor{DarkNavy}{ROUND $t$}}\\[1pt]
        \tiny\textcolor{SlateGrey}{Iterative Loop}
    };
...
Whether federated learning converges and how fast it reaches acceptable accuracy are essential questions for system design. Unlike centralized training where convergence depends primarily on learning rate and batch size, federated learning convergence depends on the interplay between communication rounds, local computation, client participation, and data heterogeneity. These factors determine whether a federated deployment will reach target accuracy in hours, days, or never. @Sec-appdx-fleet-foundations-weak-scaling-fleet develops the weak-scaling argument for why adding more participating devices grows total work faster than it shrinks wall-clock time, which sets the diminishing return that bounds how much client participation can accelerate convergence here.

##### Convergence rate fundamentals {#sec-edge-intelligence-convergence-rate-fundamentals-bfef}

A useful systems abstraction is to track participating clients, local epochs, and communication rounds together. @mcmahan2017communication define FedAvg in terms of these controls, but they do not establish a universal convergence rate based only on their product. To illustrate how the controls can pull in opposing directions, this chapter uses the following dimensionless toy sensitivity score:
...
Here, $C$ is the number of participating clients per round, $E$ is the number of local epochs, $R$ is the number of communication rounds, $\sigma$ is a dimensionless scenario constant, and $\beta_{\text{het}}$ is a dimensionless knob for client drift. The first term decreases as more client-epochs are processed, while the second deliberately penalizes large $E$ under heterogeneity. This score is not a convergence theorem, an empirically calibrated estimator, or a predictor of an objective gap.

The product $CER$ is useful only as a bookkeeping count of client-epochs. Equal $CER$ does not imply equal convergence because client sampling, examples per client, batch size, step size, local-update drift, and data distributions can differ. Production choices therefore require measurements or a convergence result whose assumptions match the deployed optimizer and data.

##### Non-IID data impact {#sec-edge-intelligence-noniid-data-impact-d8b9}

Federated-learning surveys and empirical studies identify non-IID data as a core convergence challenge and use measures such as local dissimilarity, earth mover's distance, or weight divergence [@li2020; @zhao2018federated]. These measures are not interchangeable. In the toy score, $\beta_{\text{het}}$ is only a scenario knob: setting it to zero removes the drift term, while increasing it makes additional local epochs more costly. It has no universal scale or direct estimator across applications.

The $E^2/R$ term encodes one qualitative interaction: more local work can reduce communication frequency but can also increase client drift when objectives differ. The exponent and coefficient are assumptions of this sensitivity exercise, not a claim about every FedAvg deployment.

##### Toy heterogeneity sensitivity example {#sec-edge-intelligence-quantifying-heterogeneity-practice-eece}

Because $\beta_{\text{het}}$ is not an observed metric, it should not be assigned empirical ranges by application domain. The worked example below chooses $\beta_{\text{het}}=1.5$ only to expose the arithmetic of the toy score; the value does not characterize keyboard, language, or health data.

```{python}
#| echo: false
#| label: federated-convergence-worked-example
# ┌─────────────────────────────────────────────────────────────────────────────
# │ FEDERATED TOY SENSITIVITY WORKED EXAMPLE
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-edge-intelligence-federated-learning-convergence-analysis-c1fc
# │
# │ Goal: Evaluate threshold crossings in the dimensionless toy score.
# │ Show: Scenario outputs of 200, 5,625, and 3,125 rounds.
# │ How: R_iid = sigma^2/(C*E*epsilon^2);
# │      R_non_iid = max((1+beta)^2/(C*E*epsilon^2),
# │                      beta^2*E^2/epsilon); client_epochs = R*C*E.
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt

class FederatedConvergence:
    # ┌── 1. LOAD ──────────────────────────────────────────
    total_clients = 100
    clients_per_round = 10
    local_epochs = 5
    reduced_local_epochs = 2
    epsilon = 0.01  # Per-component threshold in the toy score
    sigma = 1  # Dimensionless scenario constant
    beta_iid = 0
    beta_non_iid = 1.5  # Dimensionless scenario knob, not an empirical estimate

    # ┌── 2. EXECUTE ───────────────────────────────────────
    epsilon_sq = epsilon**2
    beta_non_iid_sq = beta_non_iid**2
    local_epochs_sq = local_epochs**2
    reduced_local_epochs_sq = reduced_local_epochs**2
    iid_rounds = int(sigma**2 / (clients_per_round * local_epochs * epsilon_sq))
    iid_client_epochs = iid_rounds * clients_per_round * local_epochs
    non_iid_variance_rounds = int((1 + beta_non_iid) ** 2 / (clients_per_round * local_epochs * epsilon_sq))
    non_iid_heterogeneity_rounds = int(beta_non_iid_sq * local_epochs_sq / epsilon)
    non_iid_rounds = max(non_iid_variance_rounds, non_iid_heterogeneity_rounds)
    non_iid_client_epochs = non_iid_rounds * clients_per_round * local_epochs
    reduced_variance_rounds = int((1 + beta_non_iid) ** 2 / (clients_per_round * reduced_local_epochs * epsilon_sq))
    reduced_heterogeneity_rounds = int(beta_non_iid_sq * reduced_local_epochs_sq / epsilon)
    reduced_rounds = max(reduced_variance_rounds, reduced_heterogeneity_rounds)
    reduced_client_epochs = reduced_rounds * clients_per_round * reduced_local_epochs
    round_increase = non_iid_rounds / iid_rounds
    round_reduction = non_iid_rounds / reduced_rounds
    local_compute_ratio = local_epochs / reduced_local_epochs
    client_epoch_reduction = non_iid_client_epochs / reduced_client_epochs

    # ┌── 3. GUARD ─────────────────────────────────────────
    check(iid_rounds == 200, f"Toy IID rounds {iid_rounds} unexpected")
    check(non_iid_rounds == 5625, f"Toy non-IID rounds {non_iid_rounds} unexpected")
    check(reduced_rounds == 3125, f"Toy reduced-E rounds {reduced_rounds} unexpected")

    # ┌── 4. OUTPUT ────────────────────────────────────────
    fc_total_clients_str = fmt_int(total_clients, commas=False)
    fc_clients_per_round_str = fmt_int(clients_per_round, commas=False)
    fc_local_epochs_str = fmt_int(local_epochs, commas=False)
    fc_reduced_local_epochs_str = fmt_int(reduced_local_epochs, commas=False)
    fc_epsilon_str = fmt(epsilon, precision=2, commas=False)
    fc_epsilon_sq_str = fmt(epsilon_sq, precision=4, commas=False)
    fc_sigma_str = fmt_int(sigma, commas=False)
    fc_beta_iid_str = fmt_int(beta_iid, commas=False)
    fc_beta_non_iid_str = fmt(beta_non_iid, precision=1, commas=False)
    fc_beta_non_iid_sq_str = fmt(beta_non_iid_sq, precision=2, commas=False)
    fc_local_epochs_sq_str = fmt_int(local_epochs_sq, commas=False)
    fc_reduced_local_epochs_sq_str = fmt_int(reduced_local_epochs_sq, commas=False)
    fc_iid_rounds_str = fmt_int(iid_rounds)
    fc_iid_client_epochs_str = fmt_int(iid_client_epochs)
    fc_non_iid_rounds_str = fmt_int(non_iid_rounds)
    fc_non_iid_variance_rounds_str = fmt_int(non_iid_variance_rounds)
    fc_non_iid_client_epochs_str = fmt_int(non_iid_client_epochs)
    fc_reduced_rounds_str = fmt_int(reduced_rounds)
    fc_reduced_variance_rounds_str = fmt_int(reduced_variance_rounds)
    fc_reduced_heterogeneity_rounds_str = fmt_int(reduced_heterogeneity_rounds)
    fc_reduced_client_epochs_str = fmt_int(reduced_client_epochs)
    fc_round_increase_factor_mult_str = fmt_multiple(round_increase, precision=1, commas=False)
    fc_round_reduction_mult_str = fmt_multiple(round_reduction, precision=2, commas=False)
    fc_local_compute_ratio_mult_str = fmt_multiple(local_compute_ratio, precision=1, commas=False)
    fc_client_epoch_reduction_mult_str = fmt_multiple(client_epoch_reduction, precision=1, commas=False)
```

A hypothetical configuration with `{python} FederatedConvergence.fc_total_clients_str` clients makes the toy arithmetic concrete: $C$ = `{python} FederatedConvergence.fc_clients_per_round_str` clients selected per round, $E$ = `{python} FederatedConvergence.fc_local_epochs_str` local epochs per round, per-component score threshold $\epsilon$ = `{python} FederatedConvergence.fc_epsilon_str`, and two scenario settings, $\beta_{\text{het}}$ = `{python} FederatedConvergence.fc_beta_iid_str` and $\beta_{\text{het}}$ = `{python} FederatedConvergence.fc_beta_non_iid_str`. These inputs do not describe a production deployment.

Zero-drift toy case $(\beta_{\text{het}} = 0)$: With the second component equal to zero and the scenario choice $\sigma=1$, setting the first component at or below $\epsilon$ gives:

$$
R_{\text{IID}} \geq \frac{\sigma^2}{C \cdot E \cdot \epsilon^2} = \frac{1}{10 \cdot 5 \cdot 0.0001} = 200 \text{ rounds}
$$

With `{python} FederatedConvergence.fc_clients_per_round_str` clients per round and `{python} FederatedConvergence.fc_local_epochs_str` local epochs, this represents `{python} FederatedConvergence.fc_iid_rounds_str` $\times$ `{python} FederatedConvergence.fc_clients_per_round_str` $\times$ `{python} FederatedConvergence.fc_local_epochs_str` = `{python} FederatedConvergence.fc_iid_client_epochs_str` total client-epochs of computation.

Positive-drift toy case $(\beta_{\text{het}} = 1.5)$: Apply the threshold separately to both components. With $E=5$, the first component requires `{python} FederatedConvergence.fc_non_iid_variance_rounds_str` rounds, while the assumed drift component requires:
...
The larger component threshold is roughly `{python} FederatedConvergence.fc_round_increase_factor_mult_str` above the zero-drift scenario. It corresponds to `{python} FederatedConvergence.fc_non_iid_rounds_str` $\times$ `{python} FederatedConvergence.fc_clients_per_round_str` $\times$ `{python} FederatedConvergence.fc_local_epochs_str` = `{python} FederatedConvergence.fc_non_iid_client_epochs_str` client-epochs. These are outputs of the chosen toy constants, not predicted convergence costs.

The assumed $E^2$ drift term makes the score sensitive to local-epoch choice. With $E = 2$ instead of $E = 5$, the drift-component threshold shrinks to `{python} FederatedConvergence.fc_reduced_heterogeneity_rounds_str` rounds while the first-component threshold grows to `{python} FederatedConvergence.fc_reduced_variance_rounds_str`:
...
= \max(3,125, 900) = 3,125 \text{ rounds}
$$

Within this toy score, the threshold-crossing round count falls by `{python} FederatedConvergence.fc_round_reduction_mult_str`, while local work per round falls by `{python} FederatedConvergence.fc_local_compute_ratio_mult_str`. The client-epoch count becomes `{python} FederatedConvergence.fc_reduced_rounds_str` $\times$ `{python} FederatedConvergence.fc_clients_per_round_str` $\times$ `{python} FederatedConvergence.fc_reduced_local_epochs_str` = `{python} FederatedConvergence.fc_reduced_client_epochs_str`, a `{python} FederatedConvergence.fc_client_epoch_reduction_mult_str` reduction from the $E$ = `{python} FederatedConvergence.fc_local_epochs_str` scenario. This demonstrates only the sensitivity encoded by the assumed formula; an actual deployment must estimate the trade-off empirically or use an applicable convergence result.

##### Communication-computation trade-off {#sec-edge-intelligence-communicationcomputation-tradeoff-6e31}

The interaction between local epochs and communication rounds creates a fundamental design trade-off, visualized in @fig-comm-compute-tradeoff. More local epochs reduce communication frequency (the computational pull) but increase client drift (the statistical penalty)\index{Client Drift!federated learning}, while fewer local epochs maintain tighter synchronization at higher communication cost.

::: {#fig-comm-compute-tradeoff fig-env="figure" fig-pos="htb" fig-cap="**Communication-computation trade-off**: This schematic shows the qualitative tension between performing more local work per round and accumulating client drift under non-IID data. Its $\epsilon$-optimal axis and operating-point labels are schematic shorthand, not a convergence guarantee or a universal optimal epoch range; system designers must measure the deployed optimizer, data, and client population." fig-alt="Schematic plot of a qualitative round-count score versus local epochs, with IID and non-IID curves and an illustrative operating point; the epsilon-optimal axis label is not a proven convergence bound."}

```{.tikz}
\begin{tikzpicture}[font=\small\sffamily, scale=0.85, node distance=1cm and 1cm]
  \tikzset{
    Axis/.style={->, line width=0.75pt},
    CurveIID/.style={BlueLine, line width=1.0pt},
    CurveNonIID/.style={RedLine, line width=1.0pt},
    Annot/.style={dashed, BrownLine, line width=1.0pt},
    Label/.style={font=\tiny\sffamily}
  }

  % Axes
  \node (origin) at (0,0) {};
  \draw[Axis] (origin) -- ++(8,0) node[right] {Local Epochs $E$};
  \draw[Axis] (origin) -- ++(0,5) node[above] {Rounds to $\epsilon$-optimal};

  % IID curve (nearly flat)
  \begin{scope}[shift={(origin)}]
    \draw[CurveIID] plot[smooth] coordinates {(0.5,1.2) (2,1.0) (4,0.9) (6,0.85) (7.5,0.82)};
    \node[BlueLine, right] at (7.5,1.2) {IID ($\beta_{\text{het}}=0$)};
  \end{scope}

  % Non-IID curve (quadratic growth)
  \begin{scope}[shift={(origin)}]
    \draw[CurveNonIID] plot[smooth] coordinates {(0.5,0.8) (2,1.5) (4,3.2) (6,4.5) (7,4.8)};
    \node[RedLine, right] at (5.5,4.8) {Non-IID ($\beta_{\text{het}}=1.5$)};
  \end{scope}

  % Optimal region annotation
  \begin{scope}[shift={(origin)}]
    \draw[Annot] (2,0) -- (2,1.5);
    \node[Label, below] at (2,0) {Sweet spot};
    \node[Label, align=center] at (4,2.2) {Client drift\\dominates};
  \end{scope}

  % Axis labels
  \node[Label, below=of origin, xshift=1cm] {1};
  \node[Label, below=of origin, xshift=4cm] {5};
  \node[Label, below=of origin, xshift=7cm] {10};
  \node[Label, left=of origin, yshift=1cm] {100};
  \node[Label, left=of origin, yshift=2.5cm] {500};
  \node[Label, left=of origin, yshift=4cm] {1000};
\end{tikzpicture}
```
...
##### When federated learning works {#sec-edge-intelligence-federated-learning-work-93a2}

Federated learning achieves practical convergence only when several deployment conditions are favorable, a theme emphasized in surveys of federated-learning systems and open problems [@kairouz2021]:
...
Scheduling must also bound how long a round waits on its slowest participants. As established under @sec-edge-intelligence-straggler-mitigation-b49d, stragglers can take 10$\times$ longer than the median device, and the round cannot aggregate until enough updates arrive.

::: {.column-margin}
```{=latex}
\vspace*{-25mm}
```
![](images/svg/edge_intelligence_straggler_cutoff_strip.svg){width="100%" fig-alt="Sequence strip showing the first K federated-client updates accepted before the round closes, while a late straggler is dropped."}
...
Update compression is a bottleneck diagnosis before it is a technique choice: if each round transmits full model weights or gradients, mobile bandwidth and battery budgets determine convergence before the optimizer does.[^fn-wireless-bandwidth-bottleneck] The design decision is which information can be quantized, sparsified, or kept local while preserving enough gradient fidelity for FedAvg to converge.

[^fn-wireless-bandwidth-bottleneck]: **Wireless Upload Asymmetry**\index{Wireless Upload Asymmetry!definition}: LTE uploads average 5--10 Mbps vs. 50+ Mbps downloads, creating the asymmetric bottleneck that dominates federated learning design; transmitting a 50 MB model update consumes approximately 100 mAh (2--3 percent of battery) and takes 40--80 seconds over LTE. Low-power protocols are far worse: LoRaWAN maxes at 50 kbps with 1 percent duty cycle, making uncompressed updates physically impossible. This asymmetry is why gradient compression is not optional but architecturally required.

The compression decision separates into three levers, each changing a different part of the communication bottleneck: model compression reduces bits per update, selective sharing reduces which parameters travel, and architectural partitioning keeps some state local. Model compression methods aim to reduce the size of transmitted updates through quantization,[^fn-quantization-edge-compression] sparsification, or subsampling, as @fig-gradient-compression illustrates. Instead of sending full-precision gradients, a client transmits 8-bit quantized updates or communicates only the top-$k$ gradient elements[^fn-sparsification-topk-gradient] with highest magnitude. These transformations are communication mechanisms, not privacy mechanisms; a smaller embedding, sparse update, or low-precision gradient may still expose membership, attributes, or reconstructable features.

[^fn-quantization-edge-compression]: **Gradient Quantization**\index{Gradient Quantization!definition}: Converts FP32 gradients to lower precision (INT8, INT4, or 1-bit) before transmission. Techniques like quantized SGD and signSGD show that quantized or sign-only updates can greatly reduce communication while relying on stochastic quantization or error compensation to preserve convergence behavior [@alistarh2017qsgd; @bernstein2018signsgd]. The achieved byte reduction and accuracy impact depend on the optimizer, model, data heterogeneity, and residual-handling scheme.

[^fn-sparsification-topk-gradient]: **Gradient Sparsification**\index{Gradient Sparsification!definition}: Transmits only the top 1--10 percent of gradients by magnitude, exploiting the observation that most gradient elements are near-zero and contribute minimally to convergence. Local gradient accumulation stores the untransmitted residuals until they grow large enough to send, achieving 10--100$\times$ compression while preserving training quality, a pattern used in sparsified SGD and deep gradient compression [@stich2018sparsified; @lin2018deep]. The trade-off: aggressive sparsification can bias updates toward frequently active parameters, potentially reducing personalization for rare user patterns.
...
Split models and architectural partitioning divide the model into a shared global component and a local component. Clients train and maintain local modules independently while synchronizing only shared parts with the server. This reduces communication and narrows what is exposed, but the shared updates can still leak information; the privacy claim depends on the threat model and on controls such as secure aggregation or differential privacy.

All of these approaches operate within the FedAvg aggregation protocol (@sec-edge-intelligence-federated-aggregation-protocols-5d37): compression changes what each client transmits, not how the server combines updates. The full communication ledger for round $r$ is

$$
B_r = B_{\text{download},r} + B_{\text{upload},r} + B_{\text{protocol},r} + B_{\text{retry},r},
$$ {#eq-edge-federated-communication-ledger}

@Eq-edge-federated-communication-ledger makes download, upload, protocol, and retry traffic explicit. The download distributes model state, the upload carries client updates, protocol traffic covers authentication and cryptographic setup, and retries account for failed or repeated transfers. Optimizing only $B_{\text{upload},r}$ can therefore move rather than remove the bottleneck. While @fig-fl-communication-computation illustrates the trade-off between local computation and network bandwidth, other communication-efficient updates introduce their own costs. Compression may degrade gradient fidelity, selective updates can limit model capacity, and split architectures may complicate coordination. Effective federated learning requires balancing communication, privacy, and convergence against the capabilities and variability of the client population. @Sec-appdx-comm-foundations-compression works the compression break-even threshold through a worked example.

Unlike the qualitative convergence schematic in @fig-comm-compute-tradeoff, @fig-fl-communication-computation models end-to-end training time under three illustrative network regimes. It makes communication latency part of the objective rather than treating round count alone as the proxy.

::: {#fig-fl-communication-computation fig-env="figure" fig-pos="htb" fig-cap="**Communication-computation trade-off in federated learning**: As network bandwidth decreases from fast to slow, the time-minimizing number of local epochs shifts rightward because additional local computation amortizes more communication. Excessive local work eventually increases total time when client drift requires more global rounds to converge." fig-alt="Three curves of total training time vs. local epochs for fast, medium, and slow networks. Optimal points shift right as bandwidth decreases."}
...
# ┌── 3. RENDER ────────────────────────────────────────────────────────────────
ax.plot(E, y_fast, label='Fast Network (Wi-Fi, 100 Mbps)', color='#2ca02c', linewidth=2.5)
ax.plot(E, y_med, label='Medium Network (4G, 10 Mbps)', color='#ff7f0e', linewidth=2.5)
ax.plot(E, y_slow, label='Slow Network (Edge, 1 Mbps)', color='#d62728', linewidth=2.5)

opt_fast_idx = np.argmin(y_fast)
opt_med_idx = np.argmin(y_med)
opt_slow_idx = np.argmin(y_slow)

ax.scatter(E[opt_fast_idx], y_fast[opt_fast_idx], color='#2ca02c', s=100, zorder=5, edgecolor='white')
ax.scatter(E[opt_med_idx], y_med[opt_med_idx], color='#ff7f0e', s=100, zorder=5, edgecolor='white')
ax.scatter(E[opt_slow_idx], y_slow[opt_slow_idx], color='#d62728', s=100, zorder=5, edgecolor='white')
...
ax.annotate(
    ...
    xytext=(28, 50),
    arrowprops=dict(
        arrowstyle="->", connectionstyle="arc3, rad=-0.2", color='#444444', shrinkB=5, relpos=(1.0, 0.5), lw=0.75  # east side of the textbox
    ),
    fontsize=10,
    color='#444444',
    ha='center',
...
ax.set_xlabel('Local Epochs per Round ($E$)', fontsize=12)
ax.set_ylabel('Total Training Time (Hours)', fontsize=12)
ax.set_title('The FL Communication-Computation Trade-off', fontsize=14, pad=15)
ax.set_xlim(0, 50)
ax.set_ylim(0, max(y_slow) * 1.1)

plt.legend(loc='upper right', fontsize=8, frameon=True, facecolor='white', framealpha=0.95, edgecolor='gray', fancybox=True, borderpad=0.6)
...
\begin{tikzpicture}[font=\small\sffamily, node distance=0pt]
  \tikzset{
    Box/.style={draw=BlueLine, line width=0.75pt, minimum width=0.8cm, minimum height=0.8cm, font=\tiny\sffamily, fill=BlueL, outer sep=0pt},
    BoxOrange/.style={Box, draw=OrangeLine, fill=OrangeL},
    BoxGreen/.style={Box, draw=GreenLine, fill=GreenL},
    Line/.style={->, line width=1.0pt, BlueLine},
    Title/.style={font=\bfseries\sffamily\bfseries, align=center, anchor=south},
    Sub/.style={font=\tiny\sffamily, align=center, anchor=north}
  }

  \begin{scope}[local bounding box=std]
    \node[Box] (s1) {0.5};
    \node[Box, right=of s1] (s2) {-0.2};
    \node[Box, right=of s2] (s3) {0.01};
    \node[Box, right=of s3] (s4) {0.8};
    \node[Title, above=0.2cm of s2.north east] {\textbf{(a) Standard}};
    \node[Sub, below=0.1cm of s2.south east] {FP32: 128 bits};
  \end{scope}

  \begin{scope}[local bounding box=quant]
    \node[BoxOrange, right=1.5cm of std] (q1) {+1};
    \node[BoxOrange, right=of q1] (q2) {0};
    \node[BoxOrange, right=of q2] (q3) {0};
    \node[BoxOrange, right=of q3] (q4) {+2};
    \node[Title, above=0.2cm of q2.north east] {\textbf{(b) Quantization}};
    \node[Sub, below=0.1cm of q2.south east] {INT8: 32 bits (4$\times$)};
  \end{scope}

  \begin{scope}[local bounding box=sparse]
    \node[BoxGreen, right=1.5cm of quant] (sp1) {0.5};
    \node[BoxGreen, right=of sp1] (sp2) {-};
    \node[BoxGreen, right=of sp2] (sp3) {-};
    \node[BoxGreen, right=of sp3] (sp4) {0.8};
    \node[Title, above=0.2cm of sp2.north east] {\textbf{(c) Sparsification}};
    \node[Sub, below=0.1cm of sp2.south east] {Top-k FP32: $\geq$64 bits + idx ($\leq$2$\times$)};
  \end{scope}

  \node[draw=BlueLine, dashed, rounded corners=2pt, line width=0.75pt, minimum width=10cm, minimum height=1cm, below=1.5cm of quant] (server) {Federated Aggregator (Server)};

  \draw[Line] (std.south) -- (std.south |- server.north) node[midway, right, font=\tiny\sffamily, text=black] {Network Transfer};
  \draw[Line] (quant.south) -- (server.north);
  \draw[Line] (sparse.south) -- (sparse.south |- server.north);
...
Another effective technique involves personalization layers, where the model is partitioned into a shared backbone and a lightweight, client-specific head, typically the final classification layer [@arivazhagan2019federated]. Only the head is updated on-device, reducing memory usage and training time. This approach is particularly well-suited for scenarios in which the primary variation across clients lies in output categories or decision boundaries.

Clustered federated learning\index{Clustered Federated Learning} offers an alternative by grouping clients according to similarities in their data or performance characteristics, and training separate models for each cluster. This strategy can enhance accuracy within homogeneous subpopulations but introduces additional system complexity and may require exchanging metadata to determine group membership.
...
Each of these strategies reflects a different point in the personalization trade-off space\index{Personalization!trade-offs}. Examine @tbl-personalization-strategies to see how compute overhead, privacy guarantees, and adaptation latency vary across local finetuning, personalization layers\index{Personalization Layer}, clustered federated learning, and meta-learning approaches.
...
Selecting the appropriate personalization method depends on deployment constraints, data characteristics, and the desired balance between accuracy, privacy, and computational efficiency. @Fig-personalization-arch contrasts three architectural approaches: full fine-tuning offers maximum expressivity at high compute cost, head-only adaptation provides low-cost but limited adaptation, and adapter-based methods balance efficiency with deep representation adaptation through small trainable modules.

::: {#fig-personalization-arch fig-env="figure" fig-pos="htb" fig-cap="**Federated personalization architectures**: The panels compare strategies for adapting global models to local data. Full fine-tuning updates every parameter; head-only adaptation freezes the feature extractor; and adapter-based learning, such as LoRA, inserts small trainable modules into a frozen backbone. Each step trades additional local compute and state for greater adaptation capacity." fig-alt="Three-column architecture comparison. Column A: full fine-tuning with all layers trainable. Column B: head-only with frozen backbone and trainable classifier. Column C: adapter-based with frozen layers and small orange adapter modules."}
...
\begin{tikzpicture}[
    ...
    layer_train/.style={
        ...
        font=\tiny\bfseries\color{CrimsonRed}
    },
    layer_froz/.style={
        draw=BorderGrey,
        fill=LightGrey!50,
    ...
    % =========================================================================
    % COLUMN A: FULL FINE-TUNING
    % =========================================================================
    \node[colcard, anchor=north west] (colA) at (0, 0) {};

    % Header
    \node[fill=CrimsonRed, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=6pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.35cm, yshift=-0.35cm]colA.north west) {A --- FULL FINE-TUNING};
    \node[anchor=north east, font=\tiny\bfseries\color{CrimsonRed}]
        at ([xshift=-0.35cm, yshift=-0.42cm]colA.north east) {High Overhead};

    % Subcard A1: Architecture Stack
    \node[subbox, minimum width=6.0cm, minimum height=4.2cm, anchor=north] (archA)
        at ([yshift=-1.0cm]colA.north) {};

    \node[layer_train] (a_l1) at ([xshift=-0.75cm, yshift=-0.55cm]archA.north) {Layer 1 (trainable)};
    \node[layer_train] (a_l2) at ([xshift=-0.75cm, yshift=-1.20cm]archA.north) {Layer 2 (trainable)};
    \node[layer_train] (a_l3) at ([xshift=-0.75cm, yshift=-1.85cm]archA.north) {Layer 3 (trainable)};
    \node[layer_train] (a_l4) at ([xshift=-0.75cm, yshift=-2.50cm]archA.north) {Layer 4 (trainable)};
    \node[head_node, fill=CrimsonRed] (a_head) at ([xshift=-0.75cm, yshift=-3.25cm]archA.north) {Task Head (trainable)};

    \draw[dotted, thick, BorderGrey] (a_l1) -- (a_l2);
    \draw[dotted, thick, BorderGrey] (a_l2) -- (a_l3);
    \draw[dotted, thick, BorderGrey] (a_l3) -- (a_l4);
    \draw[dotted, thick, BorderGrey] (a_l4) -- (a_head);

    % Backprop arrow & text
    \draw[->, dashed, line width=0.9pt, CrimsonRed]
        (a_head.east) -- ++(0.7, 0)
        -- node[midway, fill=white, inner sep=2pt, rotate=270, font=\tiny\bfseries\color{CrimsonRed}] {Full Backpropagation}
        ([xshift=0.7cm]a_l1.east) -- (a_l1.east);

    % Subcard A2: Characteristics
    \node[subbox, draw=CrimsonRed!30, fill=SoftRed!30, minimum width=6.0cm, anchor=north] (infoA)
        at ([yshift=-5.45cm]colA.north) {
        \begin{minipage}{5.5cm}
            \textbf{\scriptsize\color{CrimsonRed}Maximum Expressivity, Heavy Footprint}\\[3pt]
            {\tiny\textbf{Weights:} Updates all layers throughout backbone}\\[1.5pt]
            {\tiny\textbf{Caching:} Retains all intermediate activations}\\[1.5pt]
            {\tiny\textbf{Footprint:} 3--5$\times$ inference RAM during training}\\[1.5pt]
            {\tiny\textbf{Risk:} Catastrophic forgetting on edge data}
        \end{minipage}
    };

    % Subcard A3: Resource Comparison
    \node[subbox, minimum width=6.0cm, anchor=north] (resA)
        at ([yshift=-8.1cm]colA.north) {
        \begin{minipage}{5.5cm}
            \textbf{\tiny\textcolor{DarkNavy}{RESOURCE COMPARISON (768-DIM MODEL)}}\\[4pt]
            {\tiny Activation Memory: \hfill \textbf{\textcolor{CrimsonRed}{$\sim$1,000 KB (High)}}}\\[2pt]
            \tikz{\fill[CrimsonRed, rounded corners=1.5pt] (0, 0) rectangle (5.4cm, 0.16cm);}\\[4pt]
            {\tiny Trainable Parameters: \hfill \textbf{\textcolor{CrimsonRed}{100\% (589,824)}}}\\[2pt]
            \tikz{\fill[CrimsonRed, rounded corners=1.5pt] (0, 0) rectangle (5.4cm, 0.16cm);}\\[6pt]
            {\tiny\textbf{\textcolor{SlateGrey}{Deployment Fit:}} Edge servers / mains gateways}
        \end{minipage}
    };

    % =========================================================================
    % COLUMN B: HEAD-ONLY ADAPTATION
    % =========================================================================
    \node[colcard, anchor=north west] (colB) at (7.1, 0) {};

    % Header
    \node[fill=AmberOrange, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=6pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.35cm, yshift=-0.35cm]colB.north west) {B --- HEAD-ONLY ADAPTATION};
    \node[anchor=north east, font=\tiny\bfseries\color{AmberOrange}]
        at ([xshift=-0.35cm, yshift=-0.42cm]colB.north east) {Low Overhead};

    % Subcard B1: Architecture Stack
    \node[subbox, minimum width=6.0cm, minimum height=4.2cm, anchor=north] (archB)
        at ([yshift=-1.0cm]colB.north) {};

    \node[layer_froz, minimum width=3.8cm] (b_l1) at ([xshift=-0.4cm, yshift=-0.55cm]archB.north) {Layer 1 (frozen $\ast$)};
    \node[layer_froz, minimum width=3.8cm] (b_l2) at ([xshift=-0.4cm, yshift=-1.20cm]archB.north) {Layer 2 (frozen $\ast$)};
    \node[layer_froz, minimum width=3.8cm] (b_l3) at ([xshift=-0.4cm, yshift=-1.85cm]archB.north) {Layer 3 (frozen $\ast$)};
    \node[layer_froz, minimum width=3.8cm] (b_l4) at ([xshift=-0.4cm, yshift=-2.50cm]archB.north) {Layer 4 (frozen $\ast$)};
    \node[head_node, fill=AmberOrange, minimum width=3.8cm] (b_head) at ([xshift=-0.4cm, yshift=-3.25cm]archB.north) {Task Head (trainable)};

    \draw[dotted, thick, BorderGrey] (b_l1) -- (b_l2);
    \draw[dotted, thick, BorderGrey] (b_l2) -- (b_l3);
    \draw[dotted, thick, BorderGrey] (b_l3) -- (b_l4);
    \draw[dotted, thick, BorderGrey] (b_l4) -- (b_head);

    % Head grad arrow
    \draw[<-, line width=1.1pt, AmberOrange] (b_head.east) -- ++(0.45, 0) node[right, font=\tiny\bfseries\color{AmberOrange}] {Head grad};

    % Subcard B2: Characteristics
    \node[subbox, draw=WarmOrange!40, fill=SoftOrange!30, minimum width=6.0cm, anchor=north] (infoB)
        at ([yshift=-5.45cm]colB.north) {
        \begin{minipage}{5.5cm}
            \textbf{\scriptsize\color{AmberOrange}Ultra-Low Overhead, Bound Expressivity}\\[3pt]
            {\tiny\textbf{Weights:} Backbone frozen; tunes task head only}\\[1.5pt]
            {\tiny\textbf{Caching:} Zero intermediate activation storage}\\[1.5pt]
            {\tiny\textbf{Footprint:} Memory drops $>90\%$ relative to full FT}\\[1.5pt]
            {\tiny\textbf{Constraint:} Cannot shift internal representations}
        \end{minipage}
    };

    % Subcard B3: Resource Comparison
    \node[subbox, minimum width=6.0cm, anchor=north] (resB)
        at ([yshift=-8.1cm]colB.north) {
        \begin{minipage}{5.5cm}
            \textbf{\tiny\textcolor{DarkNavy}{RESOURCE COMPARISON (768-DIM MODEL)}}\\[4pt]
            {\tiny Activation Memory: \hfill \textbf{\textcolor{AmberOrange}{$\sim$150 KB (Low)}}}\\[2pt]
            \tikz{
                \fill[BorderGrey!40, rounded corners=1.5pt] (0, 0) rectangle (5.4cm, 0.16cm);
                \fill[AmberOrange, rounded corners=1.5pt] (0, 0) rectangle (0.8cm, 0.16cm);
            }\\[4pt]
            {\tiny Trainable Parameters: \hfill \textbf{\textcolor{AmberOrange}{$\sim$1.5\% (7,680)}}}\\[2pt]
            \tikz{
                \fill[BorderGrey!40, rounded corners=1.5pt] (0, 0) rectangle (5.4cm, 0.16cm);
                \fill[AmberOrange, rounded corners=1.5pt] (0, 0) rectangle (0.25cm, 0.16cm);
            }\\[6pt]
            {\tiny\textbf{\textcolor{SlateGrey}{Deployment Fit:}} Mobile apps with static features}
        \end{minipage}
    };

    % =========================================================================
    % COLUMN C: ADAPTER / LORA
    % =========================================================================
    \node[colcard, anchor=north west] (colC) at (14.2, 0) {};

    % Header
    \node[fill=EmeraldGreen, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=6pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.35cm, yshift=-0.35cm]colC.north west) {C --- ADAPTER / LORA};
    \node[anchor=north east, font=\tiny\bfseries\color{EmeraldGreen}]
        at ([xshift=-0.35cm, yshift=-0.42cm]colC.north east) {Parameter-Efficient};

    % Subcard C1: Architecture Stack
    \node[subbox, minimum width=6.0cm, minimum height=4.2cm, anchor=north] (archC)
        at ([yshift=-1.0cm]colC.north) {};

    % Layers with adapters/LoRA
    \node[layer_froz, minimum width=2.9cm] (c_l1) at ([xshift=-1.1cm, yshift=-0.55cm]archC.north) {Layer 1 (frozen $\ast$)};
    \node[draw=EmeraldGreen, fill=SoftGreen, rounded corners=3pt, line width=0.6pt, minimum width=1.9cm, minimum height=0.46cm, font=\tiny\bfseries\color{EmeraldGreen!90!black}]
        (c_ad1) at ([xshift=1.65cm, yshift=-0.55cm]archC.north) {Adapter ($r{=}8$)};
    \draw[line width=0.6pt, EmeraldGreen!60] (c_l1) -- (c_ad1);

    \node[layer_froz, minimum width=2.9cm] (c_l2) at ([xshift=-1.1cm, yshift=-1.20cm]archC.north) {Layer 2 (frozen $\ast$)};
    \node[draw=EmeraldGreen, fill=SoftGreen, rounded corners=3pt, line width=0.6pt, minimum width=1.9cm, minimum height=0.46cm, font=\tiny\bfseries\color{EmeraldGreen!90!black}]
        (c_ad2) at ([xshift=1.65cm, yshift=-1.20cm]archC.north) {Adapter ($r{=}8$)};
    \draw[line width=0.6pt, EmeraldGreen!60] (c_l2) -- (c_ad2);

    \node[layer_froz, minimum width=2.9cm] (c_l3) at ([xshift=-1.1cm, yshift=-1.85cm]archC.north) {Layer 3 (frozen $\ast$)};
    \node[draw=MintGreen!80!DarkNavy, fill=SoftGreen, rounded corners=3pt, line width=0.6pt, minimum width=1.9cm, minimum height=0.46cm, font=\tiny\bfseries\color{EmeraldGreen!90!black}]
        (c_lora1) at ([xshift=1.65cm, yshift=-1.85cm]archC.north) {LoRA ($r{=}8$)};
    \draw[line width=0.6pt, EmeraldGreen!60] (c_l3) -- (c_lora1);

    \node[layer_froz, minimum width=2.9cm] (c_l4) at ([xshift=-1.1cm, yshift=-2.50cm]archC.north) {Layer 4 (frozen $\ast$)};
    \node[draw=MintGreen!80!DarkNavy, fill=SoftGreen, rounded corners=3pt, line width=0.6pt, minimum width=1.9cm, minimum height=0.46cm, font=\tiny\bfseries\color{EmeraldGreen!90!black}]
        (c_lora2) at ([xshift=1.65cm, yshift=-2.50cm]archC.north) {LoRA ($r{=}8$)};
    \draw[line width=0.6pt, EmeraldGreen!60] (c_l4) -- (c_lora2);

    \node[head_node, fill=EmeraldGreen, minimum width=5.2cm] (c_head) at ([yshift=-3.25cm]archC.north) {Task Head (trainable)};

    \draw[dotted, thick, BorderGrey] (c_l1) -- (c_l2);
    \draw[dotted, thick, BorderGrey] (c_l2) -- (c_l3);
    \draw[dotted, thick, BorderGrey] (c_l3) -- (c_l4);
    \draw[dotted, thick, BorderGrey] (c_l4.south) -- (c_l4.south |- c_head.north);

    % Subcard C2: Characteristics
    \node[subbox, draw=EmeraldGreen!40, fill=SoftGreen!40, minimum width=6.0cm, anchor=north] (infoC)
        at ([yshift=-5.45cm]colC.north) {
        \begin{minipage}{5.5cm}
            \textbf{\scriptsize\color{EmeraldGreen}Near-Full Accuracy at $\sim$1\% Parameter Cost}\\[3pt]
            {\tiny\textbf{Weights:} Low-rank decomposition ($W + BA$)}\\[1.5pt]
            {\tiny\textbf{Backprop:} Confined to small rank-$r$ adapter matrices}\\[1.5pt]
            {\tiny\textbf{Inference:} Zero latency; weights folded at deploy}\\[1.5pt]
            {\tiny\textbf{Flexibility:} Modular; multiple adapters swappable}
        \end{minipage}
    };

    % Subcard C3: Resource Comparison
    \node[subbox, minimum width=6.0cm, anchor=north] (resC)
        at ([yshift=-8.1cm]colC.north) {
        \begin{minipage}{5.5cm}
            \textbf{\tiny\textcolor{DarkNavy}{RESOURCE COMPARISON (768-DIM MODEL)}}\\[4pt]
            {\tiny Activation Memory: \hfill \textbf{\textcolor{EmeraldGreen}{$\sim$50 KB (Tiny)}}}\\[2pt]
            \tikz{
                \fill[BorderGrey!40, rounded corners=1.5pt] (0, 0) rectangle (5.4cm, 0.16cm);
                \fill[EmeraldGreen, rounded corners=1.5pt] (0, 0) rectangle (0.3cm, 0.16cm);
            }\\[4pt]
            {\tiny Trainable Parameters: \hfill \textbf{\textcolor{EmeraldGreen}{$<$ 1\% (6,144)}}}\\[2pt]
            \tikz{
                \fill[BorderGrey!40, rounded corners=1.5pt] (0, 0) rectangle (5.4cm, 0.16cm);
                \fill[EmeraldGreen, rounded corners=1.5pt] (0, 0) rectangle (0.15cm, 0.16cm);
            }\\[6pt]
            {\tiny\textbf{\textcolor{SlateGrey}{Deployment Fit:}} Microcontrollers, IoT \& edge devices}
        \end{minipage}
    };
...
[^fn-membership-inference-edge]: **Membership Inference Attack**\index{Membership Inference Attack!definition}\index{Membership Inference!privacy risk}: Determines whether specific data points were used to train a model by exploiting the confidence gap between training data (high confidence, low loss) and unseen data [@shokri2017membership]. Published attacks can perform substantially above chance on overfitted models. In federated learning on personal data, this means an adversary could infer whether a specific patient's records trained a health model, making regularization and differential privacy architecturally important whenever the threat model includes training-data disclosure.
...
[^fn-secagg-secure-aggregation]: **Secure Aggregation**\index{Secure Aggregation!definition}: Cryptographic protocol where client pairs exchange random masks that cancel in the server-side sum, revealing only the aggregate gradient without exposing individual contributions [@bonawitz2017]. Production-scale federated systems such as Gboard combine secure aggregation with eligibility checks, drop-out handling, and round scheduling [@bonawitz2019towards]. The communication expansion and minimum-participation requirements depend on protocol parameters and the security model, so aggregation systems must wait for enough eligible devices before releasing a round.

These controls solve different problems. Secure aggregation limits what the coordinator learns about each submitted update, whereas differential privacy bounds how much a declared privacy unit can affect a released result. A deployment therefore needs a privacy ledger that records the unit of protection, clipping rule, noise mechanism, sampling assumptions, $(\epsilon,\delta)$ expenditure for every release, and cumulative composition across training and federated analytics [@abadi2016deep; @kairouz2021]. Resetting a round counter does not reset privacy loss for a user who participates again.

Secure aggregation also creates a robustness conflict. Byzantine-resilient rules such as Krum, coordinate-wise filtering, or per-client anomaly inspection need access to individual updates, while secure aggregation deliberately hides those updates [@blanchard2017machine; @bonawitz2017]. A system cannot claim both arbitrary per-client inspection and conventional secure aggregation over the same messages. It must instead validate clients before masking, use attested execution and bounded local updates, compute only aggregation-compatible checks, or adopt a protocol designed to evaluate approved robust statistics without revealing raw client contributions. @Fig-secure-agg illustrates the privacy side of this boundary.

::: {#fig-secure-agg fig-env="figure" fig-pos="htb" fig-cap="**Secure aggregation protocol**: The diagram simplifies how cryptographic masking protects individual updates. Pairs of clients agree on shared random masks that are added by one and subtracted by the other. The central server sums the masked updates, causing the masks to cancel in the aggregate without exposing either client's raw contribution." fig-alt="Secure aggregation diagram with two clients exchanging a shared mask. Client A adds the mask to its update, Client B subtracts it, and the server receives both masked updates."}
...
\begin{tikzpicture}[
    ...
    \providecolor{MintGreen}{HTML}{10B981}
    \providecolor{SoftGreen}{HTML}{ECFDF5}
    \providecolor{TealDark}{HTML}{0F766E}

    % =========================================================================
    % TOP BANNER: PAIRWISE KEY AGREEMENT
    % =========================================================================
    \node[draw=MintGreen!70, fill=SoftGreen!60, rounded corners=5pt, line width=0.7pt, minimum width=19.4cm, minimum height=0.75cm, anchor=north] (topbanner)
        at (0, 0) {
        \scriptsize\textcolor{EmeraldGreen!90!black}{\textbf{Pairwise Key Agreement:} Clients A and B negotiate a shared random secret mask $r$ via Diffie--Hellman key exchange prior to round upload}
    };

    % =========================================================================
    % COLUMN 1: CLIENT A (LEFT)
    % =========================================================================
    \node[colcard, fill=SoftBlue!20, draw=DeepBlue!40, anchor=north west] (colA) at (-9.7cm, -1.0cm) {};
    \node[anchor=north west, font=\scriptsize\bfseries\color{DarkNavy}]
        at ([xshift=0.35cm, yshift=-0.35cm]colA.north west) {CLIENT A $\cdot$ EDGE NODE};

    % Step 1: Local Model Update
    \node[subcard, minimum width=5.5cm, anchor=north] (a_step1)
        at ([yshift=-0.9cm]colA.north) {
        \begin{minipage}{5.2cm}
            \tiny\textbf{\textcolor{DarkNavy}{1. Local Model Update}}\\[3pt]
            \centering
            \colorbox{SoftBlue!80}{\begin{minipage}{4.8cm}
                \centering\tiny\texttt{\textbf{\textcolor{DeepBlue}{$\Delta\theta_A = [+0.12, -0.05, +0.34]$}}}
            \end{minipage}}
        \end{minipage}
    };

    % Plus node
    \node[opcircle] (plusA) at ([yshift=-0.4cm]a_step1.south) {$+$};
    \draw[line width=0.7pt, DeepBlue!60] (a_step1.south) -- (plusA.north);

    % Step 2: Add Shared Mask (+r)
    \node[subcard, draw=MintGreen!80, fill=white, minimum width=5.5cm, anchor=north] (a_step2)
        at ([yshift=-0.4cm]plusA.south) {
        \begin{minipage}{5.2cm}
            \tiny\textbf{\textcolor{EmeraldGreen!90!black}{2. Add Shared Mask (+r)}}\\[3pt]
            \centering
            \colorbox{SoftGreen!70}{\begin{minipage}{4.8cm}
                \centering\tiny\texttt{\textbf{\textcolor{EmeraldGreen!90!black}{$+r = [+0.45, -0.22, +0.18]$}}}
            \end{minipage}}
        \end{minipage}
    };
    \draw[line width=0.7pt, DeepBlue!60] (plusA.south) -- (a_step2.north);

    % Equals node
    \node[opcircle] (eqA) at ([yshift=-0.35cm]a_step2.south) {$=$};
    \draw[line width=0.7pt, DeepBlue!60] (a_step2.south) -- (eqA.north);

    % Step 3: Transmitted Payload (y_A)
    \node[subcard, draw=DeepBlue, fill=white, minimum width=5.5cm, anchor=north] (a_step3)
        at ([yshift=-0.35cm]eqA.south) {
        \begin{minipage}{5.2cm}
            \tiny\textbf{\textcolor{DarkNavy}{3. Transmitted Payload ($y_A$)}}\\[3pt]
            \centering
            \colorbox{DeepBlue}{\begin{minipage}{4.8cm}
                \centering\scriptsize\texttt{\textbf{\textcolor{white}{$y_A = \Delta\theta_A + r$}}}
            \end{minipage}}
        \end{minipage}
    };
    \draw[line width=0.7pt, DeepBlue!60] (eqA.south) -- (a_step3.north);

    % Client A Security Invariant
    \node[subcard, minimum width=5.5cm, anchor=north] (a_sec)
        at ([yshift=-0.45cm]a_step3.south) {
        \begin{minipage}{5.2cm}
            \textbf{\tiny\textcolor{DarkNavy}{LOCAL SECURITY INVARIANT}}\\[2pt]
            {\tiny\textbf{Pad:} Mask $r$ acts as a one-time cryptographic pad}\\[1pt]
            {\tiny\textbf{Blinding:} Server sees only pseudo-random noise}\\[1pt]
            {\tiny\textbf{Isolation:} Raw weights $\Delta\theta_A$ strictly quarantined}\\[1pt]
            {\tiny\textbf{Leakage:} Zero gradient recovery for single round}\\[3pt]
            \centering
            \colorbox{SoftGreen}{\tiny\bfseries\textcolor{EmeraldGreen!90!black}{\checkmark\ Provably Blinded Transmission}}
        \end{minipage}
    };

    % =========================================================================
    % COLUMN 3: CLIENT B (RIGHT)
    % =========================================================================
    \node[colcard, fill=SoftOrange!20, draw=WarmOrange!50, anchor=north east] (colB) at (9.7cm, -1.0cm) {};
    \node[anchor=north west, font=\scriptsize\bfseries\color{AmberOrange!95!black}]
        at ([xshift=0.35cm, yshift=-0.35cm]colB.north west) {CLIENT B $\cdot$ EDGE NODE};

    % Step 1: Local Model Update
    \node[subcard, minimum width=5.5cm, anchor=north] (b_step1)
        at ([yshift=-0.9cm]colB.north) {
        \begin{minipage}{5.2cm}
            \tiny\textbf{\textcolor{AmberOrange!90!black}{1. Local Model Update}}\\[3pt]
            \centering
            \colorbox{SoftOrange!80}{\begin{minipage}{4.8cm}
                \centering\tiny\texttt{\textbf{\textcolor{AmberOrange!95!black}{$\Delta\theta_B = [-0.08, +0.19, -0.11]$}}}
            \end{minipage}}
        \end{minipage}
    };

    % Minus node
    \node[opcircle, draw=WarmOrange!70, fill=SoftOrange!50, text=AmberOrange!95!black] (minusB) at ([yshift=-0.4cm]b_step1.south) {$-$};
    \draw[line width=0.7pt, WarmOrange!70] (b_step1.south) -- (minusB.north);

    % Step 2: Subtract Shared Mask (-r)
    \node[subcard, draw=MintGreen!80, fill=white, minimum width=5.5cm, anchor=north] (b_step2)
        at ([yshift=-0.4cm]minusB.south) {
        \begin{minipage}{5.2cm}
            \tiny\textbf{\textcolor{EmeraldGreen!90!black}{2. Subtract Shared Mask ($-r$)}}\\[3pt]
            \centering
            \colorbox{SoftGreen!70}{\begin{minipage}{4.8cm}
                \centering\tiny\texttt{\textbf{\textcolor{EmeraldGreen!90!black}{$-r = [-0.45, +0.22, -0.18]$}}}
            \end{minipage}}
    ...
    \draw[line width=0.7pt, WarmOrange!70] (minusB.south) -- (b_step2.north);

    % Equals node
    \node[opcircle, draw=BorderGrey, fill=SoftGrey, text=SlateGrey] (eqB) at ([yshift=-0.35cm]b_step2.south) {$=$};
    \draw[line width=0.7pt, BorderGrey] (b_step2.south) -- (eqB.north);

    % Step 3: Transmitted Payload (y_B)
    \node[subcard, draw=WarmOrange, fill=white, minimum width=5.5cm, anchor=north] (b_step3)
        at ([yshift=-0.35cm]eqB.south) {
        \begin{minipage}{5.2cm}
            \tiny\textbf{\textcolor{AmberOrange!90!black}{3. Transmitted Payload ($y_B$)}}\\[3pt]
            \centering
            \colorbox{WarmOrange}{\begin{minipage}{4.8cm}
                \centering\scriptsize\texttt{\textbf{\textcolor{white}{$y_B = \Delta\theta_B - r$}}}
            \end{minipage}}
        \end{minipage}
    };
    \draw[line width=0.7pt, BorderGrey] (eqB.south) -- (b_step3.north);

    % Client B Security Invariant
    \node[subcard, minimum width=5.5cm, anchor=north] (b_sec)
        at ([yshift=-0.45cm]b_step3.south) {
        \begin{minipage}{5.2cm}
            \textbf{\tiny\textcolor{DarkNavy}{LOCAL SECURITY INVARIANT}}\\[2pt]
            {\tiny\textbf{Offset:} Negative mask $-r$ offsets A's pad}\\[1pt]
            {\tiny\textbf{Blinding:} Server sees only pseudo-random noise}\\[1pt]
            {\tiny\textbf{Isolation:} Raw weights $\Delta\theta_B$ strictly quarantined}\\[1pt]
            {\tiny\textbf{Leakage:} Zero gradient recovery for single round}\\[3pt]
            \centering
            \colorbox{SoftGreen}{\tiny\bfseries\textcolor{EmeraldGreen!90!black}{\checkmark\ Provably Blinded Transmission}}
        \end{minipage}
    ...
    % =========================================================================
    % COLUMN 2: AGGREGATION SERVER (CENTER)
    % =========================================================================
    \node[colcard, fill=SoftBlue!10, draw=DeepBlue!40, minimum width=6.0cm, anchor=north] (colServ) at (0, -1.0cm) {};
    \node[anchor=north west, font=\scriptsize\bfseries\color{DeepBlue}]
        at ([xshift=0.35cm, yshift=-0.35cm]colServ.north west) {AGGREGATION SERVER $\cdot$ SEMI-HONEST};

    % Ingress Updates
    \node[subcard, minimum width=5.4cm, anchor=north] (serv_ing)
        at ([yshift=-0.9cm]colServ.north) {
        \begin{minipage}{5.1cm}
            \tiny\textbf{\textcolor{DarkNavy}{Ingress Updates Received}}\\[3pt]
            \colorbox{SoftBlue!70}{\begin{minipage}{4.8cm}
                \centering\tiny\texttt{\textbf{\textcolor{DeepBlue}{From A: $y_A = \Delta\theta_A + r$}}}
            \end{minipage}}\\[3pt]
            \colorbox{SoftOrange!70}{\begin{minipage}{4.8cm}
                \centering\tiny\texttt{\textbf{\textcolor{AmberOrange!95!black}{From B: $y_B = \Delta\theta_B - r$}}}
            \end{minipage}}
        \end{minipage}
    };

    % Sum node
    \node[opcircle, minimum size=0.55cm, draw=DeepBlue, fill=SoftBlue!60] (sumNode) at ([yshift=-0.4cm]serv_ing.south) {$\sum$};
    \draw[line width=0.7pt, DeepBlue!60] (serv_ing.south) -- (sumNode.north);

    % Algebraic Mask Cancellation
    \node[subcard, draw=DeepBlue!40, fill=white, minimum width=5.4cm, anchor=north] (serv_canc)
        at ([yshift=-0.4cm]sumNode.south) {
        \begin{minipage}{5.1cm}
            \textbf{\tiny\textcolor{DeepBlue}{Algebraic Mask Cancellation}}\\[3pt]
            \centering
            \colorbox{SoftGrey}{\begin{minipage}{4.8cm}
                \centering\tiny\texttt{$y_{\text{sum}} = y_A + y_B$}
            \end{minipage}}\\[2pt]
            \colorbox{SoftGrey}{\begin{minipage}{4.8cm}
                \centering\tiny\texttt{$= (\Delta\theta_A + r) + (\Delta\theta_B - r)$}
            \end{minipage}}\\[2pt]
            \colorbox{SoftGrey}{\begin{minipage}{4.8cm}
                \centering\tiny\texttt{$= \Delta\theta_A + \Delta\theta_B + (r - r)$}
            \end{minipage}}\\[3pt]
            \colorbox{SoftGreen!90}{\begin{minipage}{4.8cm}
                \centering\tiny\texttt{\textbf{\textcolor{EmeraldGreen!90!black}{$= \Delta\theta_A + \Delta\theta_B$ \checkmark\ (Mask cancels)}}}
            \end{minipage}}
        \end{minipage}
    };
    \draw[line width=0.7pt, DeepBlue!60] (sumNode.south) -- (serv_canc.north);

    % Exact Unmasked Aggregate
    \node[subcard, draw=EmeraldGreen!60, fill=white, minimum width=5.4cm, anchor=north] (serv_agg)
        at ([yshift=-0.45cm]serv_canc.south) {
        \begin{minipage}{5.1cm}
            \textbf{\tiny\textcolor{EmeraldGreen!90!black}{EXACT UNMASKED AGGREGATE}}\\[2pt]
            {\tiny\textbf{Linearity:} Server computes exact sum $\Delta\theta_A + \Delta\theta_B$}\\[1pt]
            {\tiny\textbf{Precision:} Zero noise added; rate untainted}\\[1pt]
            {\tiny\textbf{Security:} Cannot isolate client contributions}\\[3pt]
            \centering
            \colorbox{EmeraldGreen}{\begin{minipage}{4.8cm}
                \centering\scriptsize\texttt{\textbf{\textcolor{white}{$\theta_{\text{next}} \leftarrow \theta_{\text{global}} + \eta \cdot (y_{\text{sum}} / 2)$}}}
            \end{minipage}}
        \end{minipage}
    };
    ...
    % UPLINK ARROWS
    % =========================================================================
    \draw[->, line width=1.1pt, DeepBlue] (a_step3.east) -- (a_step3.east -| colServ.west)
        node[midway, fill=white, draw=DeepBlue!40, rounded corners=2pt, inner sep=2pt, font=\tiny\bfseries\color{DeepBlue}] {UPLINK};

    \draw[->, line width=1.1pt, WarmOrange] (b_step3.west) -- (b_step3.west -| colServ.east)
        node[midway, fill=white, draw=WarmOrange!50, rounded corners=2pt, inner sep=2pt, font=\tiny\bfseries\color{AmberOrange!95!black}] {UPLINK};

    % =========================================================================
    % BOTTOM BANNER: PRIVACY GUARANTEE
    % =========================================================================
    \node[draw=BorderGrey, fill=white, rounded corners=5pt, line width=0.7pt, minimum width=19.4cm, minimum height=0.85cm, anchor=north] (botbanner)
        at (0, -11.0cm) {
        \begin{minipage}{19.0cm}
            \colorbox{TealDark}{\tiny\bfseries\textcolor{white}{PRIVACY GUARANTEE}}\quad
            {\scriptsize\textbf{Zero-Knowledge Fleet Aggregation:} The coordinator computes exact fleet gradient updates without being able to reconstruct, decrypt, or inspect any individual participant's weights or private examples.}
        \end{minipage}
    };
...
That algebraic cancellation makes secure aggregation composable with differential privacy for layered defense: the protocol hides individual contributions first, then privacy noise can bound what the aggregate still reveals. It does not establish that the contributing devices or updates are trustworthy. Federated learning transforms machine learning into a massive distributed systems challenge that extends beyond traditional algorithmic considerations. Coordinating thousands or millions of heterogeneous devices with intermittent connectivity requires protocols that handle Byzantine failures [clients sending arbitrary or malicious updates], network partitions\index{Network Partition!federated learning} [temporary loss of connectivity between participants], and constrained communication. These challenges differ from controlled data center training, where high-bandwidth networks and managed infrastructure narrow the trust and availability envelope.
...
The relationship between communication requirements and participation rates exhibits sharp threshold effects. Large model transfers sharply reduce sustained client participation because devices must remain idle, connected, sufficiently charged, and willing to spend upload bandwidth for the whole transfer window. Keeping updates small, often through adapter-only sharing or aggressive compression, increases the eligible device pool and shortens round duration. This communication efficiency directly translates to model quality improvements: higher participation rates provide better statistical diversity and more robust gradient estimates for global model updates. The compression techniques that make these small updates possible (gradient quantization, sparsification, and top-$k$ selection with error accumulation) are the same ones established in @sec-edge-intelligence-bandwidthaware-update-compression-7730; here they serve the participation argument rather than the bandwidth argument, because shrinking the update is what widens the eligible device pool.
...
class EdgeFedNpuRecap:
    ...
    compute_ratio_mult_str = fmt_multiple(compute_ratio, precision=1, commas=True)
```
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: Brain power efficiency footnote.
class EdgeBrainEfficiencyRecap:
    brain_ops_per_s = 1e15
    brain_power_w = 20
    mobile_npu_eff_low_tops_per_w = 1
    mobile_npu_eff_high_tops_per_w = 5
    brain_tops_per_w = brain_ops_per_s / brain_power_w / TRILLION
    brain_vs_npu_low = brain_tops_per_w / mobile_npu_eff_high_tops_per_w
    brain_vs_npu_high = brain_tops_per_w / mobile_npu_eff_low_tops_per_w
    from mlsysim.fmt import fmt_int, fmt, check

    check(brain_tops_per_w == 50, f"Brain efficiency should be 50 TOPS/W under stated operands, got {brain_tops_per_w:.1f}")
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    brain_tops_per_w_str = fmt_qty(brain_tops_per_w * TOPS / watt, TOPS / watt, precision=0, commas=False)
    brain_vs_npu_low_str = fmt_int(brain_vs_npu_low, commas=False)
    brain_vs_npu_high_mult_str = fmt_multiple(brain_vs_npu_high, precision=0, commas=False)
```

[^fn-brain-energy-efficiency]: **Brain Power Efficiency**\index{Brain Power Efficiency!definition}\index{Efficiency!brain, edge learning}: At 20 W, the brain processes approximately $10^{15}$ operations per second, equivalent to about `{python} EdgeBrainEfficiencyRecap.brain_tops_per_w_str` under this coarse operation-counting model; compared with the 1--5 TOPS/W mobile-NPU range used earlier in this chapter, that is roughly `{python} EdgeBrainEfficiencyRecap.brain_vs_npu_low_str`--`{python} EdgeBrainEfficiencyRecap.brain_vs_npu_high_mult_str` higher energy efficiency, though biological synaptic events and digital operations are only an approximate analogy. This efficiency derives from extreme sparsity (only 1--2 percent of 86 billion neurons active simultaneously) and event-driven processing that activates computation only on input change. These two principles directly inform edge ML design: sparse activation patterns and opportunistic training scheduling are not heuristics but attempts to approach biological efficiency limits.
...
Meta-learning approaches address these challenges by learning algorithms themselves rather than just learning specific tasks. MAML trains models to quickly adapt to new tasks with minimal data, exactly the capability required for personalized on-device adaptation where collecting large user-specific datasets is impractical. Few-shot learning techniques enable rapid specialization from small user-specific datasets, allowing models to personalize based on just a handful of examples while maintaining general capabilities learned during pretraining.
...
::: {#fig-odl-design-flow fig-env="figure" fig-pos="htb" fig-cap="**On-device learning design flow**: This decision framework selects a technique for one device and policy. It maps adaptation complexity, compute availability, and data-sharing constraints to bias-only updates, adapters, full fine-tuning, localized learning, or federated coordination." fig-alt="Decision flowchart with diamond and rectangular nodes. Starts at deployment, branches on adaptation complexity to bias-only or adapters, then on compute availability to fine-tuning, finally on data sharing to federated or localized approaches."}
...
\begin{tikzpicture}[
    ...
    branchlbl/.style={
        ...
        fill=LightGrey!70,
        rounded corners=3pt,
        font=\tiny\bfseries\color{SlateGrey},
        inner xsep=6pt,
        inner ysep=2.5pt
    },
    branchyes/.style={
        ...
        fill=SoftGreen,
        rounded corners=3pt,
        font=\tiny\bfseries\color{EmeraldGreen!90!black},
        inner xsep=6pt,
        inner ysep=2.5pt
    },
    branchno/.style={
        ...
        fill=SoftBlue,
        rounded corners=3pt,
        font=\tiny\bfseries\color{OceanBlue},
        inner xsep=6pt,
        inner ysep=2.5pt
    },
    branchorange/.style={
        ...
        fill=SoftOrange,
        rounded corners=3pt,
        font=\tiny\bfseries\color{AmberOrange!95!black},
        inner xsep=6pt,
        inner ysep=2.5pt
    }
]
    ...
    \providecolor{MintGreen}{HTML}{10B981}
    \providecolor{SoftGreen}{HTML}{ECFDF5}
    \providecolor{PurpleViolet}{HTML}{7C3AED}
    \providecolor{SoftPurple}{HTML}{F5F3FF}
    \providecolor{TealDark}{HTML}{0D9488}
    \providecolor{SoftTeal}{HTML}{F0FDFA}

    % =========================================================================
    % CENTRAL DECISION SPINE
    % =========================================================================
    % Root: Deploy Model
    \node[draw=DeepBlue, fill=SoftBlue!30, rounded corners=5pt, line width=0.8pt, minimum width=8.2cm, minimum height=1.15cm, anchor=north] (root)
        at (0, 0) {
        \begin{minipage}{7.8cm}
            \colorbox{DeepBlue}{\tiny\bfseries\textcolor{white}{DEPLOY}}\quad
            \textbf{\scriptsize\textcolor{DarkNavy}{Deploy Model to Edge Target Hardware}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{Static quantized baseline flashed to target device ROM/Flash}}
        \end{minipage}
    };

    % Decision 1
    \node[decision, anchor=north] (d1)
        at (0, -1.8cm) {
        \begin{minipage}{7.8cm}
            \colorbox{PurpleViolet}{\tiny\bfseries\textcolor{white}{DECISION 1}}\quad
            \textbf{\scriptsize\textcolor{PurpleViolet!90!black}{Adaptation Needed Post-Deploy?}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{Personalization, user shift, or acoustic domain drift expected?}}
        \end{minipage}
    };
    \draw[->, line width=1.0pt, SlateGrey] (root.south) -- (d1.north);

    % Decision 2
    \node[decision, anchor=north] (d2)
        at (0, -3.8cm) {
        \begin{minipage}{7.8cm}
            \colorbox{PurpleViolet}{\tiny\bfseries\textcolor{white}{DECISION 2}}\quad
            \textbf{\scriptsize\textcolor{PurpleViolet!90!black}{Working Memory $<$ 1 MB (MCU)?}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{Constrained SRAM budget limits backpropagation caching}}
        \end{minipage}
    };
    \draw[->, line width=1.0pt, SlateGrey] (d2.south) -- (d2.north)
        node[midway, branchyes] {Yes};

    % Decision 3
    \node[decision, anchor=north] (d3)
        at (0, -5.8cm) {
        \begin{minipage}{7.8cm}
            \colorbox{PurpleViolet}{\tiny\bfseries\textcolor{white}{DECISION 3}}\quad
            \textbf{\scriptsize\textcolor{PurpleViolet!90!black}{Labeled Data Available On-Device?}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{Direct user feedback, touch clicks, or explicit ground truth}}
        \end{minipage}
    };
    \draw[->, line width=1.0pt, SlateGrey] (d2.south) -- (d3.north)
        node[midway, branchlbl] {No};

    % Decision 4
    \node[decision, anchor=north] (d4)
        at (0, -7.8cm) {
        \begin{minipage}{7.8cm}
            \colorbox{PurpleViolet}{\tiny\bfseries\textcolor{white}{DECISION 4}}\quad
            \textbf{\scriptsize\textcolor{PurpleViolet!90!black}{Fleet Collaboration \& Privacy?}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{Cross-device learning required without raw telemetry egress}}
        \end{minipage}
    };
    \draw[->, line width=1.0pt, SlateGrey] (d3.south) -- (d4.north)
        node[midway, branchyes] {Yes};

    % =========================================================================
    % RIGHT BRANCHES (PERFECTLY HORIZONTAL)
    % =========================================================================
    % Decision 1 -> Static Deployment (No)
    \node[action, draw=SlateGrey!70, anchor=west] (out_static)
        at ([xshift=2.2cm]d1.east) {
        \begin{minipage}{5.8cm}
            \colorbox{SlateGrey}{\tiny\bfseries\textcolor{white}{STATIC DEPLOYMENT}}\\[2pt]
            \textbf{\scriptsize\textcolor{DarkNavy}{Frozen Inference Only}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{Zero on-device training or state tracking overhead}}
        \end{minipage}
    };
    \draw[->, line width=1.0pt, SlateGrey] (d1.east) -- (out_static.west)
        node[midway, branchlbl] {No};

    % Decision 4 -> Isolated On-Device FT (No)
    \node[action, draw=OceanBlue, fill=SoftBlue!25, anchor=west] (out_isolated)
        at ([xshift=2.2cm]d4.east) {
        \begin{minipage}{5.8cm}
            \colorbox{OceanBlue}{\tiny\bfseries\textcolor{white}{ISOLATED ON-DEVICE FT}}\\[2pt]
            \textbf{\scriptsize\textcolor{DeepBlue}{Local LoRA / Adapter Training}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{Rank-$r$ parameter adaptation; zero cross-fleet sync}}
        \end{minipage}
    };
    \draw[->, line width=1.0pt, SlateGrey] (d4.east) -- (out_isolated.west)
        node[midway, branchlbl] {No};

    % =========================================================================
    % LEFT BRANCHES (PERFECTLY HORIZONTAL)
    % =========================================================================
    % Decision 2 -> Bias-Only (Yes)
    \node[action, draw=WarmOrange, fill=SoftOrange!30, anchor=east] (out_bias)
        at ([xshift=-2.2cm]d2.west) {
        \begin{minipage}{5.8cm}
            \colorbox{AmberOrange}{\tiny\bfseries\textcolor{white}{BIAS-ONLY FINE-TUNING}}\\[2pt]
            \textbf{\scriptsize\textcolor{AmberOrange!95!black}{TinyTL / BitFit Adaptation}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{Backbone frozen; zero activation storage; fits in SRAM}}
        \end{minipage}
    };
    \draw[->, line width=1.0pt, AmberOrange] (d2.west) -- (out_bias.east)
        node[midway, branchorange] {Yes};

    % Decision 3 -> Test-Time Adaptation (No)
    \node[action, draw=TealDark, fill=SoftTeal!40, anchor=east] (out_tta)
        at ([xshift=-2.2cm]d3.west) {
        \begin{minipage}{5.8cm}
            \colorbox{TealDark}{\tiny\bfseries\textcolor{white}{TEST-TIME ADAPTATION (TTA)}}\\[2pt]
            \textbf{\scriptsize\textcolor{TealDark!90!black}{Self-Supervised Adaptation}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{Entropy minimization (TENT) \& batch norm update}}
        \end{minipage}
    };
    \draw[->, line width=1.0pt, TealDark] (d3.west) -- (out_tta.east)
        node[midway, branchno] {No};

    % Decision 4 -> Federated Learning (Yes)
    \node[action, draw=EmeraldGreen, fill=SoftGreen!40, anchor=east] (out_fl)
        at ([xshift=-2.2cm]d4.west) {
        \begin{minipage}{5.8cm}
            \colorbox{EmeraldGreen}{\tiny\bfseries\textcolor{white}{FEDERATED LEARNING (FL)}}\\[2pt]
            \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{FedAvg + Secure Aggregation}}\\[2pt]
            {\tiny\textcolor{SlateGrey}{$\Delta\theta$ updates aggregated; raw data strictly quarantined}}
        \end{minipage}
    };
    \draw[->, line width=1.0pt, EmeraldGreen] (d4.west) -- (out_fl.east)
        node[midway, branchyes] {Yes};

    % =========================================================================
    % BOTTOM CAPTION NOTE
    % =========================================================================
    \node[anchor=north, font=\scriptsize\itshape\color{SlateGrey}, align=center]
        at (0, -9.8cm) {A systematic triage of memory envelope, supervisory signals, and fleet governance boundaries dictates the optimal on-device learning strategy.};
...
@Fig-edge-mlops-pipeline outlines the end-to-end edge MLOps lifecycle, connecting cloud pretraining, compression, and over-the-air deployment with local on-device adaptation and privacy-safe telemetry feedback loops.

::: {#fig-edge-mlops-pipeline fig-env="figure" fig-pos="htb" fig-cap="**Edge MLOps lifecycle pipeline**: The lifecycle links cloud pretraining and model compression with over-the-air deployment, on-device adaptation, and privacy-preserving telemetry that closes the loop without transmitting raw user data." fig-alt="Five-stage edge MLOps pipeline showing cloud training, compression, OTA deployment, on-device adaptation, and telemetry feedback loop."}
...
\begin{tikzpicture}[
    ...
    % OUTER FRAME
    % =========================================================================
    \node[panelcard, fill=SoftGrey!25, minimum width=20.4cm, minimum height=11.2cm, anchor=north] (outer)
        at (0, 0) {};

    % Top Headers
    \node[fill=DarkNavy, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.4cm, yshift=-0.4cm]outer.north west) {CLOUD INFRASTRUCTURE};
    \node[anchor=north west, font=\scriptsize\bfseries\color{DarkNavy}]
        at ([xshift=4.6cm, yshift=-0.43cm]outer.north west) {Centralized Fleet Management};

    \node[draw=BorderGrey, fill=white, rounded corners=3pt, font=\tiny\bfseries\color{SlateGrey}, inner xsep=8pt, inner ysep=3pt, anchor=north]
        at (0, -0.4cm) {NETWORK};

    \node[fill=EmeraldGreen, text=white, rounded corners=3pt, font=\scriptsize\bfseries, inner xsep=8pt, inner ysep=3pt, anchor=north west]
        at ([xshift=0.9cm, yshift=-0.4cm]outer.north) {EDGE ENVIRONMENT};
    \node[anchor=north west, font=\scriptsize\bfseries\color{EmeraldGreen!90!black}]
        at ([xshift=4.6cm, yshift=-0.43cm]outer.north) {Distributed Heterogeneous Devices};

    % Vertical Cloud / Edge Boundary
    \draw[dashed, line width=0.8pt, BorderGrey] (0, -1.0cm) -- (0, -10.8cm);

    % =========================================================================
    % TOP ROW: 4 STAGES
    % =========================================================================
    % Stage 1: Cloud Pretraining
    \node[stagecard, draw=DeepBlue!50, minimum width=4.5cm, minimum height=2.6cm, anchor=north] (st1)
        at (-7.35cm, -1.1cm) {
        \begin{minipage}{4.2cm}
            \colorbox{DarkNavy}{\tiny\bfseries\textcolor{white}{1}}\enspace
            \textbf{\tiny\textcolor{DarkNavy}{Cloud Pretraining}}\\[3pt]
            {\tiny\textbf{Corpus:} TB-scale global pretraining}\\[1pt]
            {\tiny\textbf{Compute:} Multi-node GPU clusters}\\[1pt]
            {\tiny\textbf{Duration:} Weeks of distributed compute}\\[4pt]
            \centering
            \colorbox{SoftBlue!80}{\tiny\texttt{\textbf{\textcolor{DeepBlue}{Base Model $w^*$ (14 GB)}}}}
        \end{minipage}
    };

    % Stage 2: Compression & PEFT
    \node[stagecard, draw=WarmOrange!60, minimum width=4.5cm, minimum height=2.6cm, anchor=north] (st2)
        at (-2.45cm, -1.1cm) {
        \begin{minipage}{4.2cm}
            \colorbox{AmberOrange}{\tiny\bfseries\textcolor{white}{2}}\enspace
            \textbf{\tiny\textcolor{AmberOrange!95!black}{Compression \& PEFT}}\\[3pt]
            {\tiny\textbf{Quantization:} INT8 / INT4 PTQ}\\[1pt]
            {\tiny\textbf{Pruning:} Magnitude \& structured}\\[1pt]
            {\tiny\textbf{Adapters:} LoRA matrix initialization}\\[4pt]
            \centering
            \colorbox{SoftOrange!80}{\tiny\texttt{\textbf{\textcolor{AmberOrange!95!black}{Tier Packages ($\sim$350 MB)}}}}
        \end{minipage}
    };

    % Stage 3: OTA Deployment
    \node[stagecard, draw=PurpleViolet!50, minimum width=4.5cm, minimum height=2.6cm, anchor=north] (st3)
        at (2.45cm, -1.1cm) {
        \begin{minipage}{4.2cm}
            \colorbox{PurpleViolet}{\tiny\bfseries\textcolor{white}{3}}\enspace
            \textbf{\tiny\textcolor{PurpleViolet!90!black}{OTA Deployment}}\\[3pt]
            {\tiny\textbf{Canary:} Staged rollout (1\% $\to$ 10\%)}\\[1pt]
            {\tiny\textbf{Artifacts:} Binary delta packages}\\[1pt]
            {\tiny\textbf{Targets:} Tiered device configuration}\\[4pt]
            \centering
            \colorbox{SoftPurple}{\tiny\texttt{\textbf{\textcolor{PurpleViolet!90!black}{Delta Payload (25 MB)}}}}
        \end{minipage}
    };

    % Stage 4: On-Device Adaptation
    \node[stagecard, draw=EmeraldGreen!60, minimum width=4.5cm, minimum height=2.6cm, anchor=north] (st4)
        at (7.35cm, -1.1cm) {
        \begin{minipage}{4.2cm}
            \colorbox{EmeraldGreen}{\tiny\bfseries\textcolor{white}{4}}\enspace
            \textbf{\tiny\textcolor{EmeraldGreen!90!black}{On-Device Adaptation}}\\[3pt]
            {\tiny\textbf{Inference:} Sub-10 ms local latency}\\[1pt]
            {\tiny\textbf{Adaptation:} LoRA / TinyTL private data}\\[1pt]
            {\tiny\textbf{Validation:} Shadow model safety check}\\[4pt]
            \centering
            \colorbox{SoftGreen!80}{\tiny\texttt{\textbf{\textcolor{EmeraldGreen!90!black}{Adapted $\theta_{\text{local}}$}}}}
        \end{minipage}
    };

    % Stage flow arrows
    \draw[->, line width=1.1pt, DeepBlue] (st1.east) -- (st2.west);
    \draw[->, line width=1.1pt, WarmOrange] (st2.east) -- (st3.west);
    \draw[->, line width=1.1pt, PurpleViolet] (st3.east) -- (st4.west);

    % =========================================================================
    % TELEMETRY FEEDBACK LOOP (STAGE 5)
    % =========================================================================
    \node[draw=EmeraldGreen, fill=white, rounded corners=4pt, line width=0.8pt, inner xsep=8pt, inner ysep=3pt] (st5)
        at (0, -4.3cm) {
        \colorbox{EmeraldGreen}{\tiny\bfseries\textcolor{white}{5}}\enspace
        \textbf{\scriptsize\textcolor{EmeraldGreen!90!black}{Privacy-Preserved Telemetry Loop}}
    };

    \draw[line width=1.2pt, dashed, EmeraldGreen]
        (st4.south) -- ++(0, -0.4cm) -| (st5.east);
    \draw[->, line width=1.2pt, dashed, EmeraldGreen]
        (st5.west) -| (st1.south);

    \node[font=\tiny\bfseries\color{EmeraldGreen!90!black}, anchor=north]
        at (4.9cm, -4.4cm) {Differential Privacy ($\varepsilon < 8$)};
    \node[font=\tiny\bfseries\color{EmeraldGreen!90!black}, anchor=north]
        at (-4.9cm, -4.4cm) {Triggers Retraining};

    % =========================================================================
    % BOTTOM ROW: 4 DETAIL PILLARS
    % =========================================================================
    % Pillar 1: Training Infrastructure
    \node[detailcard, minimum width=4.5cm, minimum height=5.2cm, anchor=north] (det1)
        at (-7.35cm, -5.2cm) {
        \begin{minipage}{4.1cm}
            \centering
            \colorbox{SoftBlue!80}{\begin{minipage}{3.8cm}
                \centering\tiny\bfseries\textcolor{DeepBlue}{Training Infrastructure}
            \end{minipage}}\\[4pt]
            \raggedright
            {\tiny\textbf{Compute Fabric:} Multi-node GPU}\\[1.5pt]
            {\tiny\textbf{Clusters:} Distributed FP8/BF16 nodes}\\[1.5pt]
            {\tiny\textbf{Scaling:} Megatron-LM tensor parallel}\\[4pt]
            {\tiny\textbf{Dataset Scope:} Global corpus}\\[1.5pt]
            {\tiny\textbf{Volume:} Trillions of tokens (TB scale)}\\[1.5pt]
            {\tiny\textbf{Quality:} Rigorous deduplication \& filters}\\[4pt]
            {\tiny\textbf{Cadence:} Monthly / quarterly}\\[1.5pt]
            {\tiny\textbf{Models:} Foundation base versions v1, v2}\\[1.5pt]
            {\tiny\textbf{Lineage:} Checkpoint registry tracking}
        \end{minipage}
    };

    % Pillar 2: Compression Techniques
    \node[detailcard, minimum width=4.5cm, minimum height=5.2cm, anchor=north] (det2)
        at (-2.45cm, -5.2cm) {
        \begin{minipage}{4.1cm}
            \centering
            \colorbox{SoftOrange!80}{\begin{minipage}{3.8cm}
                \centering\tiny\bfseries\textcolor{AmberOrange!95!black}{Compression Techniques}
            \end{minipage}}\\[4pt]
            \raggedright
            {\tiny\textbf{Quantization:} INT8 / INT4}\\[1.5pt]
            {\tiny\textbf{Technique:} SmoothQuant \& AWQ PTQ}\\[1.5pt]
            {\tiny\textbf{Footprint:} $4\times$ memory size reduction}\\[4pt]
            {\tiny\textbf{Sparsity \& Distillation:}}\\[1.5pt]
            {\tiny\textbf{Pruning:} 2:4 semi-structured patterns}\\[1.5pt]
            {\tiny\textbf{Distill:} Compact student model transfer}\\[4pt]
            {\tiny\textbf{PEFT Preparation:}}\\[1.5pt]
            {\tiny\textbf{Adapters:} LoRA matrix initialization}\\[1.5pt]
            {\tiny\textbf{Packaging:} Tailored device-tier profiles}
        \end{minipage}
    };

    % Pillar 3: On-Device Constraints
    \node[detailcard, minimum width=4.5cm, minimum height=5.2cm, anchor=north] (det3)
        at (2.45cm, -5.2cm) {
        \begin{minipage}{4.1cm}
            \centering
            \colorbox{SoftPurple}{\begin{minipage}{3.8cm}
                \centering\tiny\bfseries\textcolor{PurpleViolet!90!black}{On-Device Constraints}
            \end{minipage}}\\[4pt]
            \raggedright
            {\tiny\textbf{Memory Budget:} 256 MB -- 8 GB}\\[1.5pt]
            {\tiny\textbf{Quarantine:} Strict RAM ceiling per app}\\[1.5pt]
            {\tiny\textbf{Tolerance:} Zero OS page fault tolerance}\\[4pt]
            {\tiny\textbf{Energy \& Battery:}}\\[1.5pt]
            {\tiny\textbf{Drain:} $< 5\%$ battery budget per day}\\[1.5pt]
            {\tiny\textbf{Trigger:} Train only when charging + idle}\\[4pt]
            {\tiny\textbf{Operational Safeguards:}}\\[1.5pt]
            {\tiny\textbf{Network:} Wi-Fi only telemetry uplinks}\\[1.5pt]
            {\tiny\textbf{Rollback:} Automatic shadow fallback}
        \end{minipage}
    };

    % Pillar 4: Privacy-Safe Telemetry
    \node[detailcard, minimum width=4.5cm, minimum height=5.2cm, anchor=north] (det4)
        at (7.35cm, -5.2cm) {
        \begin{minipage}{4.1cm}
            \centering
            \colorbox{SoftGreen!80}{\begin{minipage}{3.8cm}
                \centering\tiny\bfseries\textcolor{EmeraldGreen!90!black}{Privacy-Safe Telemetry}
            \end{minipage}}\\[4pt]
            \raggedright
            {\tiny\textbf{Differential Privacy:} $\varepsilon < 8$}\\[1.5pt]
            {\tiny\textbf{Perturbation:} Local Gaussian noise}\\[1.5pt]
            {\tiny\textbf{Accounting:} Strict cumulative privacy budget}\\[4pt]
            {\tiny\textbf{Payload Boundaries:}}\\[1.5pt]
            {\tiny\textbf{Metrics:} Aggregate confusion matrix only}\\[1.5pt]
            {\tiny\textbf{Safety:} Zero raw audio/text/image egress}\\[4pt]
            {\tiny\textbf{Continuous Drift Triggers:}}\\[1.5pt]
            {\tiny\textbf{Detection:} Population performance drop}\\[1.5pt]
            {\tiny\textbf{Cloud Sync:} Autonomous retraining trigger}
        \end{minipage}
    };
...
Privacy-preserving telemetry represents the first fundamental departure from traditional monitoring. Collecting performance metrics without compromising user privacy requires federated analytics\index{Federated Analytics!privacy-preserving telemetry} where devices share only aggregate statistics or differentially private summaries. Systems cannot simply log individual predictions or training samples as centralized systems do. Instead, devices report distribution summaries such as mean accuracy and confidence histograms rather than per-example metrics. When formal privacy guarantees are required, reported statistics need differential privacy mechanisms that bound information leakage through carefully calibrated noise addition. Secure aggregation protocols prevent the server from observing individual device contributions, reducing the risk that the aggregation process itself reconstructs private information from any single device's data.

Drift detection\index{Drift Detection!on-device models} presents additional challenges without access to ground truth labels. Traditional monitoring compares model predictions against labeled validation sets maintained on centralized infrastructure. On-device systems must detect drift using only local signals available during deployment:

- Confidence calibration\index{Confidence Calibration!drift detection} compares probabilities with eventual outcomes when labels exist. Without outcomes, confidence shifts only flag a change in score distribution.
...
- Task performance proxies use implicit feedback such as corrections or abandonment only after validating the causal link to task quality; the model's own suggestions can change the feedback it later observes.
- Shadow baseline comparison runs a frozen base model alongside the adapted model to measure divergence. Without labels or a validated invariant, the comparison flags change rather than identifying which model is better.
...
\begin{tikzpicture}[font=\small\sffamily, node distance=1.2cm]
  \tikzset{
    BaseBlock/.style={draw=black!50, fill=black!10, line width=0.75pt, rounded corners=2pt, align=center, minimum width=2.5cm, minimum height=1cm},
    ShadowBlock/.style={draw=BlueLine, fill=BlueL, line width=0.75pt, rounded corners=2pt, align=center, minimum width=2.5cm, minimum height=1cm},
    ActiveBlock/.style={draw=OrangeLine, fill=OrangeL, line width=0.75pt, rounded corners=2pt, align=center, minimum width=2.5cm, minimum height=1cm},
    ArbiterBlock/.style={draw=GreenLine, fill=GreenL, line width=0.75pt, rounded corners=2pt, align=center, minimum width=2.5cm, minimum height=1cm},
    DecisionBlock/.style={draw=RedLine, fill=RedLine!8, line width=0.75pt, rounded corners=2pt, align=center, minimum width=2.5cm, minimum height=0.7cm},
    Line/.style={->, line width=1.0pt, draw=black!70}
  }

  % Blocks
  \node[BaseBlock] (Input) {User Data Stream\\(Sensor/Input)};

  \node[ShadowBlock, below left=of Input] (Shadow) {\textbf{Shadow Model}\\Frozen Baseline (v1)};
  \node[ActiveBlock, below right=of Input] (Active) {\textbf{Active Model}\\Locally Adapted (v1.1)};

  \node[ArbiterBlock, below=2.5cm of Input, anchor=center] (Arbiter) {\textbf{Arbiter/Comparator}\\Change Detection};

  % Flows
  \draw[Line] (Input) -| (Shadow);
  \draw[Line] (Input) -| (Active);

  \draw[Line] (Shadow) |- (Arbiter);
  \draw[Line] (Active) |- (Arbiter);

  \node[DecisionBlock, right=1.0cm of Arbiter, font=\scriptsize\sffamily] (Logic) {Hold for validated\\outcome or invariant};
  \draw[dashed, ->, line width=1.0pt, draw=RedLine] (Arbiter.east) -- (Logic.west);
...
## Putting the Pillars Together {#sec-edge-intelligence-pillar-integration-7e21}
...
The data efficiency layer implements adaptive strategies across the entire device population while respecting individual resource constraints. All devices implement experience replay,[^fn-replay-buffer-forgetting] but with device-appropriate buffer sizes, `{python} EdgeAdaptationTierProfile.budget_replay_buffer_mb_str` on budget devices vs. `{python} EdgeAdaptationTierProfile.flagship_replay_buffer_mb_str` on flagship models, ensuring that memory-constrained devices can still benefit from replay-based learning. Few-shot learning enables rapid adaptation to new users within their first `{python} EdgeAdaptationTierProfile.few_shot_interactions_range_str`, reducing the cold-start problem[^fn-coldstart-edge-personalization] that plagues systems requiring extensive training data. Streaming updates accommodate continuous voice pattern evolution as users' speaking styles naturally change over time or as they use the assistant in new acoustic environments.

[^fn-replay-buffer-forgetting]: [offset=-15mm] **Experience Replay**\index{Experience Replay!definition}: Borrowed from reinforcement learning [@mnih2015], stores past input-output pairs in a buffer and interleaves them with new training data to reduce catastrophic forgetting in continual learning [@rolnick2019experience]. Memory-efficient edge implementations may store compressed embeddings or summaries rather than raw inputs, reducing storage and privacy pressure at the cost of less complete gradient information. Buffer size, sampling policy, and representation choice jointly determine the stability-memory trade-off: too small and old tasks can degrade, too large and the buffer competes with model weights for scarce edge memory.

[^fn-coldstart-edge-personalization]: **Cold-Start Problem**\index{Cold-Start Problem!definition}\index{Cold-Start Problem!personalization}: New users or devices have no local data for personalization, so early predictions must rely on a global model, coarse context, or explicit user input. Meta-learned initializations (such as MAML) or privacy-preserving population statistics can bootstrap personalization, but the fundamental trade-off remains: faster cold-start requires more information from the global model, which may not represent the new user's distribution. Few-shot adaptation from `{python} EdgeAdaptationTierProfile.few_shot_interactions_range_str` offers one balance of speed and local relevance.

The federated coordination layer orchestrates privacy-preserving collaboration across the device population. Devices participate in federated training rounds opportunistically based on connectivity status and battery level, ensuring that coordination does not degrade user experience. LoRA adapters aggregate efficiently with just `{python} EdgeAdaptationTierProfile.federated_lora_update_mb_str` per update compared to `{python} EdgeLoraParameterEfficiency.fp16_weight_footprint_gb_str` for full model synchronization, making federated learning practical over mobile networks. Raw audio examples can remain local, but LoRA updates and participation metadata derived from individual voice patterns leave the device and remain sensitive. Secure aggregation limits per-client inspection, differential privacy with cumulative accounting bounds release leakage, and retention and access controls bound how long the update path preserves sensitive state.

These layers only work as a system when the integration policy protects against capability mismatch, component failure, resource conflicts, and untested emergent behavior. @Tbl-edge-integration-controls maps edge-learning integration controls\index{Edge Learning!integration controls} for each system risk: hierarchical capability matching\index{Hierarchical Capability Matching}, graceful degradation\index{Graceful Degradation}, explicit priority policy, and performance validation\index{Validation!performance}.
...
This integrated approach transforms on-device learning from a collection of techniques into a coherent systems capability that provides robust personalization within real-world deployment constraints. A tiered adaptation strategy (@fig-tiered-adaptation) maps these techniques to device capabilities.

@Fig-tiered-adaptation extends the single-device choice in @fig-odl-design-flow into a fleet policy: each device tier receives an adaptation method that fits its resource envelope, while shared data value determines whether coordination remains local or becomes federated.
...
The chapter built three pillars for on-device learning: model adaptation (bias-only updates, adapters, full fine-tuning), data efficiency (few-shot, experience replay, contrastive), and federated coordination (aggregation, secure aggregation, drift handling). @Fig-tiered-adaptation maps each technique to device-class capability. Test integration of the three pillars across the same deployment.
...
Hardware tiers still shape the execution path. Arm Cortex-M-class devices, mobile-class A-series CPUs, and phones with dedicated NPUs[^fn-armcortex-processor-heterogeneity] may all participate in the same federated population, but each tier supports a different adaptation envelope. The system must therefore assign model formats, training algorithms, validation duties, and update frequencies by device class rather than treating the fleet as uniform.

[^fn-armcortex-processor-heterogeneity]: **Arm Cortex Spectrum**\index{Arm Cortex Spectrum!definition}\index{Arm Cortex!device heterogeneity}: Representative endpoints span nearly six orders of magnitude, from Cortex-M-class devices around `{python} EdgeDeviceSpectrum.tiny_clock_mhz_str`, `{python} EdgeDeviceSpectrum.tiny_ram_kb_str` RAM, and `{python} EdgeDeviceSpectrum.sensor_power_uw_str` power to mobile-class devices around `{python} EdgeDeviceSpectrum.mobile_clock_ghz_str`, `{python} EdgeDeviceSpectrum.flagship_ram_gb_str` RAM, and `{python} EdgeDeviceSpectrum.phone_power_w_str` [@warden2020tinyml; @lai2018cmsis]. Federated-learning surveys and production-system reports identify this kind of client heterogeneity and availability skew as a core systems challenge [@kairouz2021; @bonawitz2019towards]. The design consequence is tiering: quantized inference on the smallest microcontrollers, lightweight bias-only updates on larger embedded or wearable tiers, and fuller adapter or backpropagation loops only on mobile-class SoCs.

Software heterogeneity compounds the challenge. Devices may run different versions of operating systems, kernel-level drivers, and runtime libraries. Some environments support optimized ML runtimes like TensorFlow Lite[^fn-tflite-runtime-inference] Micro or ONNX Runtime Mobile,[^fn-onnx-runtime-inference] while others rely on custom inference stacks or restricted APIs. These discrepancies can lead to subtle inconsistencies in behavior, especially when models are compiled differently or when floating-point precision varies across platforms.

[^fn-tflite-runtime-inference]: **TensorFlow Lite**\index{TensorFlow Lite!definition}: Google's mobile inference runtime, optimized for Arm with specialized 8-bit and 16-bit kernels that achieve 3$\times$ faster inference than full TensorFlow. TFLite Micro extends to microcontrollers with $<1$ MB memory. The systems consequence for federated learning: runtime differences between TFLite and other frameworks can cause numerically different gradient updates from identical models, introducing aggregation noise that compounds across heterogeneous device fleets.

[^fn-onnx-runtime-inference]: **ONNX Runtime Mobile**\index{ONNX Runtime Mobile!definition}\index{ONNX Runtime!portable inference}: Microsoft's cross-platform inference engine using the vendor-neutral Open Neural Network Exchange format, enabling training in one framework and deployment across iOS, Android, and embedded Linux. The mobile variant achieves 2--3$\times$ faster inference than TFLite on some models through operator fusion. For federated learning, ONNX's portability reduces the numerical divergence problem: a single model representation across heterogeneous devices produces more consistent gradient updates during aggregation.
...
Mitigating this problem requires on-device comparison and update gating. The shadow-model and checkpoint mechanisms introduced in @sec-edge-intelligence-validation-strategy-adaptation-ab3d compare the adapted model with a stable baseline and retain a known-good state. In the absence of labels, disagreement or lower confidence can justify holding an update, but cannot establish that adaptation degraded behavior. Rollback gates therefore need delayed outcomes, a validated task proxy, or a safety invariant whose violation is sufficient even without a quality label.

In some cases, federated validation offers a partial solution. Devices can contribute protected model updates or summary statistics that a server aggregates to identify population-level change. Aggregation does not anonymize these signals automatically; the telemetry path needs data minimization, secure aggregation where appropriate, a declared privacy unit, and privacy accounting for repeated releases. Even then, aggregate validation may miss rare or user-specific failures.
...
Device-envelope fit measures whether the improvement stays inside resource budgets. Memory-constrained convergence evaluates validation loss under a fixed RAM budget, such as "convergence within 512 KB training footprint," while energy-per-update measures millijoules consumed per gradient step; background adaptation commonly budgets `{python} EdgeAdaptationPowerMetrics.background_range_str`, translating to roughly `{python} EdgeAdaptationPowerMetrics.background_energy_per_hour_str` of adaptation before noticeably affecting battery life. Time-to-adaptation extends the same idea to scheduling by measuring wall-clock time from new data to measurable improvement, including waiting for idleness, charging status, and thermal headroom rather than only raw accelerator throughput.
...
Energy consumption compounds this problem. Adaptation typically involves sustained computation over multiple input samples, which taxes battery-powered systems and may lead to rapid energy depletion. For instance, performing a single epoch of adaptation on a microcontroller-class device can consume several millijoules,[^fn-mcu-milliwatt-power] an appreciable fraction of the energy budget for a duty-cycled system operating on harvested power. This necessitates careful scheduling, such that learning occurs only during idle periods, when energy reserves are high and user latency constraints are relaxed.

[^fn-mcu-milliwatt-power]: **Microcontroller Power Budget**\index{Microcontroller Power Budget!definition}: A microcontroller drawing 10 mW during training consumes 36 J per hour, so a small 200 mAh coin-cell battery (about 2,200 J of usable energy at 3 V) sustains roughly 60 hours of continuous compute. Indoor energy harvesting provides only 10--100 $\mu$W continuously, two to three orders of magnitude below the training draw, so real deployments duty-cycle: training 10 seconds per hour consumes only about 0.1 J per session and limits adaptation to a few hundred gradient steps per session. This energy ceiling, not compute throughput, is the binding constraint that determines which algorithms are feasible on harvested-power devices.

From a memory perspective, training incurs higher peak usage than inference, especially because backpropagation must retain or recompute intermediate activations;[^fn-activationcaching-memory-saving] efficient on-device training systems reduce this activation burden and limit the amount of trainable state [@cai2020tinytl; @kwon2024tinytrain].

[^fn-activationcaching-memory-saving]: **Activation Caching**\index{Activation Caching!definition}: Forward pass activations must be stored or recomputed for backpropagation, and for many CNNs this activation memory can exceed the model-weight footprint. On devices with $<512$ KB RAM, activation storage alone can exceed total memory, precluding multi-layer adaptation entirely unless gradient checkpointing [@chen2016training], low-rank updates, or sparse-update methods are employed.

These resource demands must also be balanced against QoS[^fn-qos-edge-training] goals. Users expect edge devices to respond reliably and consistently, regardless of whether learning is occurring in the background. Any observable degradation, including dropped audio in a wake-word detector or lag in a wearable display, can erode user trust.

[^fn-qos-edge-training]: **QoS for Edge ML**\index{Quality of Service!definition}: Latency guarantees that on-device training must not violate: voice assistants, video applications, and wearables each have tight interactive response budgets. Background training that noticeably increases latency, drops frames, or delays sensor processing can damage user trust. Systems enforce this through priority scheduling where inference preempts training, and resource governors throttle learning when QoS metrics approach deployment-specific thresholds.

In some deployments, adaptation is further gated by cost constraints imposed by networked infrastructure. For instance, devices may offload portions of the learning workload to nearby gateways or cloudlets,[^fn-cloudlet-edge-training] introducing bandwidth and communication trade-offs.

[^fn-cloudlet-edge-training]: **Cloudlet**\index{Cloudlet!definition}: A small-scale compute tier located close to users, often near access networks or enterprise sites, provides lower-latency offload than a distant cloud region. This intermediate tier enables hybrid on-device learning: time-sensitive inference runs locally while aggregation, validation, or heavier adaptation work can offload nearby when the network and trust model allow it. The trade-off is deployment complexity: cloudlets add an orchestration layer that must decide per-update whether to compute locally, offload nearby, or defer to cloud.
...
Beyond individual device drift, federated learning systems face the challenge of participation bias amplification at the population level. Devices with reliable power and connectivity participate more frequently in federated rounds [@li2020]. This uneven participation creates scenarios where models become more strongly optimized for users with high-end devices while performance degrades for those with limited resources. The resulting feedback loop exacerbates digital inequality: better-served users receive better models, while underserved populations experience declining performance, reducing their engagement and further diminishing their representation in training rounds [@wang2021field]. These fairness and bias amplification concerns highlight the ethical implications of distributed learning systems.
...
**Context**: A 2015 Tesla Model S operating with Mobileye EyeQ3/NVIDIA Mobile SoC hardware performed perception and control decisions locally while relying on human supervision [@ntsb2017tesla]. For the worked design exercise, the team adopts $L_{\text{lat}} \le 50\text{ ms}$ as an explicit safety-loop scenario requirement rather than a measured property of that vehicle.

**Mechanism**: In the May 7, 2016 Williston crash, bright daylight and a white tractor-trailer side created an out-of-distribution optical environment where vision sensors failed to differentiate the trailer background. The system operated outside its operational design domain, while human takeover required $\sim 1.5\text{–}2.5\text{ s}$—orders of magnitude slower than the sensor loop $L_{\text{lat}}$.

**Impact**: The vehicle drove under the tractor-trailer without braking, resulting in a fatal collision.

**Fix**: Tesla retrofitted the fleet with radar-primary signal processing, upgraded local vision compute hardware, and instituted mandatory hands-on-wheel monitoring alerts.

**Systems lesson**: Edge autonomy is a real-time closed loop. When sensor latency $L_{\text{lat}}$ and execution bounds $O / (R_{\text{peak}} \cdot \eta_{\text{hw}})$ conflict with human reaction speed, fallback assumptions fail; safety must be designed into the control loop itself.
...
Teams expect local learning to match centralized training's model improvements, ignoring fundamental resource constraints. On-device learning amplifies resource needs by 4--12$\times$ compared to inference-only deployment due to activation caching, gradient storage, optimizer state, and bidirectional memory traffic (@sec-edge-intelligence-model-adaptation-6a82). Local datasets are typically small, biased, and nonrepresentative, while compute budgets remain orders of magnitude below cloud GPUs. A smartphone with `{python} EdgeAdaptationTierProfile.smartphone_ram_gb_str` RAM and `{python} EdgeDeviceSpectrum.phone_power_w_str` power budget cannot replicate the adaptation achieved by data center systems with terabytes of memory and kilowatts of power. Effective on-device learning requires designing adaptation strategies that provide meaningful improvements within these constraints rather than attempting to replicate cloud-scale learning capabilities.
...
Teams design on-device learning systems assuming uniform hardware capabilities across deployment devices. Real-world deployments span diverse hardware with varying computational power, memory capacity, energy constraints, and networking capabilities. Edge device capabilities span nearly six orders of magnitude in memory and power, and four orders in CPU throughput: from `{python} EdgeDeviceSpectrum.tiny_ram_kb_str` RAM microcontrollers to `{python} EdgeDeviceSpectrum.flagship_ram_gb_str` smartphones, `{python} EdgeDeviceSpectrum.tiny_clock_mhz_str` Cortex-M-class processors (`{python} EdgeDeviceSpectrum.tiny_cpu_mips_str`) to `{python} EdgeDeviceSpectrum.mobile_clock_ghz_str` mobile-class processors (`{python} EdgeDeviceSpectrum.mobile_cpu_mips_str`), and `{python} EdgeDeviceSpectrum.sensor_power_uw_str` sensor nodes to `{python} EdgeDeviceSpectrum.phone_power_w_str` flagship phones [@warden2020tinyml; @lai2018cmsis]. A learning algorithm that works well on high-end smartphones may fail catastrophically on resource-constrained IoT devices. Federated learning algorithms must account for heterogeneous clients and availability [@kairouz2021; @bonawitz2019towards], using selective participation, compression, and tiered aggregation strategies when the deployment spans `{python} EdgeDeviceSpectrum.cpu_throughput_ratio_mult_str` performance differences.
...
Many teams focus on individual device optimization without considering the system-level challenges of coordinating learning across thousands or millions of edge devices. Edge systems orchestration (@sec-edge-intelligence-largescale-device-orchestration-1360) must handle intermittent connectivity, varying power states, different time zones, and unpredictable device availability patterns that create complex scheduling and synchronization challenges. Device clustering, federated rounds coordination, model versioning across diverse deployment contexts, and handling partial participation from unreliable devices require sophisticated infrastructure beyond simple aggregation servers. Additionally, real-world edge deployments involve multiple stakeholders with different incentives, security requirements, and operational procedures that must be balanced against learning objectives. Effective edge learning systems require robust orchestration frameworks that can maintain system coherence despite constant device churn, network partitions, and operational disruptions.

:::

## Summary {#sec-edge-intelligence-summary-0af9}

Edge systems must account for thermal throttling, device heterogeneity, and federated orchestration. Edge intelligence operates at the resource-constrained boundary of the Machine Learning Fleet. Data center infrastructure and global serving systems assume abundant power, memory, and connectivity; edge deployments instead include sensors with microwatt power budgets, microcontrollers with kilobytes of memory, and smartphones facing the mobile memory wall.
...
* **The edge reverses the fleet**: Data centers fight to keep many accelerators busy; edge devices fight to learn under batteries, kilobytes, thermal throttling (principle \ref{pri-power-density-wall}), and intermittent links. The same Compute, Communication, Coordination constraints produce a different discipline when scarcity, not scale, is binding.
* **Bandwidth beats advertised TOPS**: On-device LLMs are limited by mobile memory bandwidth (principle \ref{pri-autoregressive-bottleneck}), not NPU peak compute. The 30--50$\times$ gap between mobile RAM and data-center HBM makes quantization and locality survival requirements for interactive decode.
...
Earlier chapters pushed toward more accelerators, bandwidth, and power, treating the data center as one machine. Edge systems apply the same physical constraints at the opposite extreme. A data center must keep thousands of accelerators busy; an edge device must operate within a battery, a thermal envelope, and sometimes only kilobytes of memory. A phone is not a small data center, and a sensor is not a slow GPU. When scarcity sets the terms, system design must obtain more from less.

::: {.callout-chapter-connection title="From devices to operations"}

Edge intelligence pushes models to the boundary where hardware diversity, battery budgets, intermittent connectivity, and local autonomy all become first-order constraints. Deploying models across this spectrum, from data center GPUs to edge sensors, is only the beginning. Sustaining these deployments at production scale demands systematic operational practices. @Sec-ops-scale examines the platform engineering, monitoring, and lifecycle management frameworks required to keep the entire Machine Learning Fleet running reliably.
