...
from mlsysim.fmt import MarkdownStr, check, fmt, fmt_int, fmt_math, fmt_memory
...
class NetworkRunningExampleCluster:
    ...
    host_staging_traffic_gb_str = fmt_memory(2 * grad_bytes, unit=GB, precision=0, commas=False)
```
...
\begin{tikzpicture}[
  ...
  % -------------------------------------------------------------
  % 5 STACKED LEVELS (L5 at top, L1 at bottom)
  % -------------------------------------------------------------
  \def\wcard{13.2}
  \def\hcard{1.35}

  % LEVEL 5: Cluster Design
  \node[level_card] (c5) at (0, 6.0) {};
  % Header banner
  \path[fill=PurpleFill, rounded corners=4pt]
    ($(c5.north west)$) rectangle ($(c5.north east) + (0, -0.42)$);
  \path[fill=PurpleFill]
    ($(c5.north west) + (0, -0.3)$) rectangle ($(c5.north east) + (0, -0.42)$);
  \draw[PurpleLine!50, line width=0.7pt]
    ($(c5.north west) + (0, -0.42)$) -- ($(c5.north east) + (0, -0.42)$);
  \node[anchor=west, font=\bfseries\scriptsize, text=PurpleLine] at ($(c5.north west) + (0.35, -0.21)$)
    {LEVEL 5 --- Cluster Design \& Fleet Integration};
  \node[badge, fill=PurpleLine, anchor=east] at ($(c5.north east) + (-0.35, -0.21)$) {L5};
  % Body text
  \node[anchor=west, font=\bfseries\scriptsize, text=TextDark] at ($(c5.north west) + (0.35, -0.64)$)
    {NVIDIA SuperPOD $\cdot$ TPU Pods $\cdot$ Meta Grand Teton $\cdot$ Multi-Rail Cluster Fabric};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c5.north west) + (0.35, -1.02)$)
    {Topology-aware job placement $\cdot$ Failure domain isolation $\cdot$ Multi-rail chassis routing $\cdot$ Operational runbooks};

  % LEVEL 4: Fabric Behavior
  \node[level_card] (c4) at (0, 4.5) {};
  % Header banner
  \path[fill=OrangeL, rounded corners=4pt]
    ($(c4.north west)$) rectangle ($(c4.north east) + (0, -0.42)$);
  \path[fill=OrangeL]
    ($(c4.north west) + (0, -0.3)$) rectangle ($(c4.north east) + (0, -0.42)$);
  \draw[OrangeLine!50, line width=0.7pt]
    ($(c4.north west) + (0, -0.42)$) -- ($(c4.north east) + (0, -0.42)$);
  \node[anchor=west, font=\bfseries\scriptsize, text=OrangeLine!90!black] at ($(c4.north west) + (0.35, -0.21)$)
    {LEVEL 4 --- Fabric Behavior \& Congestion Dynamics};
  \node[badge, fill=OrangeLine, anchor=east] at ($(c4.north east) + (-0.35, -0.21)$) {L4};
  % Body text
  \node[anchor=west, font=\bfseries\scriptsize, text=TextDark] at ($(c4.north west) + (0.35, -0.64)$)
    {PFC $\cdot$ DCQCN $\cdot$ HPCC $\cdot$ Packet Trimming $\cdot$ Adaptive Routing};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c4.north west) + (0.35, -1.02)$)
    {Head-of-line blocking $\cdot$ PFC deadlock loops $\cdot$ Incast burst collapse $\cdot$ Tail-latency stragglers $\cdot$ Buffer bloat};

  % LEVEL 3: Switch & Topology
  \node[level_card] (c3) at (0, 3.0) {};
  % Header banner
  \path[fill=SlateFill, rounded corners=4pt]
    ($(c3.north west)$) rectangle ($(c3.north east) + (0, -0.42)$);
  \path[fill=SlateFill]
    ($(c3.north west) + (0, -0.3)$) rectangle ($(c3.north east) + (0, -0.42)$);
  \draw[SlateLine!40, line width=0.7pt]
    ($(c3.north west) + (0, -0.42)$) -- ($(c3.north east) + (0, -0.42)$);
  \node[anchor=west, font=\bfseries\scriptsize, text=SlateLine] at ($(c3.north west) + (0.35, -0.21)$)
    {LEVEL 3 --- Switch Architecture \& Interconnect Topologies};
  \node[badge, fill=SlateLine, anchor=east] at ($(c3.north east) + (-0.35, -0.21)$) {L3};
  % Body text
  \node[anchor=west, font=\bfseries\scriptsize, text=TextDark] at ($(c3.north west) + (0.35, -0.64)$)
    {Fat-Tree (Clos) $\cdot$ Rail-Optimized $\cdot$ Dragonfly+ $\cdot$ Torus Mesh};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c3.north west) + (0.35, -1.02)$)
    {Switch radix limits $\cdot$ Non-blocking bisection $\cdot$ ECMP hashing $\cdot$ Optical vs.\ electrical switching $\cdot$ Hop penalty};

  % LEVEL 2: Transport & Direct Memory Access
  \node[level_card] (c2) at (0, 1.5) {};
  % Header banner
  \path[fill=LightBlue, rounded corners=4pt]
    ($(c2.north west)$) rectangle ($(c2.north east) + (0, -0.42)$);
  \path[fill=LightBlue]
    ($(c2.north west) + (0, -0.3)$) rectangle ($(c2.north east) + (0, -0.42)$);
  \draw[SteelBlue!40, line width=0.7pt]
    ($(c2.north west) + (0, -0.42)$) -- ($(c2.north east) + (0, -0.42)$);
  \node[anchor=west, font=\bfseries\scriptsize, text=SteelBlue] at ($(c2.north west) + (0.35, -0.21)$)
    {LEVEL 2 --- Transport Protocols \& Direct Memory Access};
  \node[badge, fill=SteelBlue, anchor=east] at ($(c2.north east) + (-0.35, -0.21)$) {L2};
  % Body text
  \node[anchor=west, font=\bfseries\scriptsize, text=TextDark] at ($(c2.north west) + (0.35, -0.64)$)
    {GPUDirect RDMA $\cdot$ RoCEv2 $\cdot$ InfiniBand Verbs $\cdot$ $\alpha$-$\beta$ Communication Model};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c2.north west) + (0.35, -1.02)$)
    {Zero-copy kernel bypass $\cdot$ NIC PCIe P2P access $\cdot$ Reliable connection (RC) QP state $\cdot$ Latency vs.\ transfer scaling};

  % LEVEL 1: Physical Layer
  \node[level_card] (c1) at (0, 0.0) {};
  % Header banner
  \path[fill=GreenFill, rounded corners=4pt]
    ($(c1.north west)$) rectangle ($(c1.north east) + (0, -0.42)$);
  \path[fill=GreenFill]
    ($(c1.north west) + (0, -0.3)$) rectangle ($(c1.north east) + (0, -0.42)$);
  \draw[DarkGreen!40, line width=0.7pt]
    ($(c1.north west) + (0, -0.42)$) -- ($(c1.north east) + (0, -0.42)$);
  \node[anchor=west, font=\bfseries\scriptsize, text=DarkGreen] at ($(c1.north west) + (0.35, -0.21)$)
    {LEVEL 1 --- Physical Layer: SerDes, Signaling \& Optics};
  \node[badge, fill=DarkGreen, anchor=east] at ($(c1.north east) + (-0.35, -0.21)$) {L1};
  % Body text
  \node[anchor=west, font=\bfseries\scriptsize, text=TextDark] at ($(c1.north west) + (0.35, -0.64)$)
    {PAM4 Modulation $\cdot$ 106 Gbps SerDes $\cdot$ Direct Attach Copper (DAC) $\cdot$ AOC $\cdot$ Pluggable Optics};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c1.north west) + (0.35, -1.02)$)
    {Bit error rate (BER) $\cdot$ Forward Error Correction (FEC) $\cdot$ Insertion loss budget $\cdot$ Optical reach $\cdot$ Transceivers};

  % -------------------------------------------------------------
  % LEFT: ABSTRACTION AXIS ARROW
  % -------------------------------------------------------------
  \draw[->, line width=1.1pt, SteelBlue!80!black]
    (-7.1, -0.55) -- node[above=2pt, rotate=90, font=\bfseries\scriptsize, text=SteelBlue!80!black] {Increasing Architectural Abstraction \& Scope} (-7.1, 6.55);

  \node[anchor=east, font=\tiny\bfseries, text=DarkGreen] at (-7.3, -0.3) {Link Physics};
  \node[anchor=east, font=\tiny\bfseries, text=PurpleLine] at (-7.3, 6.35) {Cluster Scale};
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_bandwidth, fmt_flop_rate, fmt_qty
...
\begin{tikzpicture}[
  font=\sffamily,
  >=Stealth,
  bar_hbm/.style={draw=SteelBlue, fill=LightBlue, line width=0.8pt, rounded corners=1.5pt},
  bar_nvlink/.style={draw=OrangeLine, fill=OrangeL, line width=0.8pt, rounded corners=1.5pt},
  bar_ib/.style={draw=PurpleLine, fill=PurpleFill, line width=0.8pt, rounded corners=1.5pt},
  bar_pcie/.style={draw=SlateLine, fill=SlateFill, line width=0.8pt, rounded corners=1.5pt},
  ratio_badge/.style={
    rectangle,
    rounded corners=2pt,
    draw=DarkRed,
  ...
  % Grid lines & Y-ticks
  \foreach \y/\val in {0.542/10, 2.342/100, 4.142/{1,000}, 5.942/{10,000}} {
    \draw[GridLine, line width=0.6pt, dashed] (0, \y) -- (\waxis, \y);
    \node[anchor=east, font=\scriptsize, text=TextMuted] at (-0.15, \y) {\val};
  }

  % Axes
  \draw[CardBorder!80!black, line width=0.9pt] (0, 0) -- (\waxis, 0);
  \draw[CardBorder!80!black, line width=0.9pt] (0, 0) -- (0, \haxis);

  % Y-axis Title
  \node[rotate=90, font=\bfseries\scriptsize, text=TextDark, anchor=south] at (-0.9, 3.1)
    {Bandwidth (GB/s, log scale)};

  % -------------------------------------------------------------
  % HORIZONTAL LEGEND (Top)
  % -------------------------------------------------------------
  \node[anchor=west] at (0.3, 6.7) {
    \begin{tikzpicture}[baseline=-0.5ex]
      \draw[bar_hbm] (0, -0.15) rectangle ++(0.35, 0.25);
      \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at (0.42, -0.02) {HBM (On-Package)};

      \draw[bar_nvlink] (3.6, -0.15) rectangle ++(0.35, 0.25);
      \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at (4.02, -0.02) {NVLink (Intra-Node)};

      \draw[bar_ib] (7.2, -0.15) rectangle ++(0.35, 0.25);
      \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at (7.62, -0.02) {InfiniBand / RoCE};

      \draw[bar_pcie] (10.6, -0.15) rectangle ++(0.35, 0.25);
      \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at (11.02, -0.02) {PCIe Host Bus};
    \end{tikzpicture}
  };
  ...
  % GENERATIONS & BARS
  % Bar width = 0.42 cm
  % Offsets: HBM (-0.75), NVLink (-0.25), IB (+0.25), PCIe (+0.75)
  % -------------------------------------------------------------
  \def\bw{0.42}

  % === V100 (2017) ===
  \def\xone{1.8}
  \draw[bar_hbm] (\xone - 0.75 - \bw/2, 0) rectangle (\xone - 0.75 + \bw/2, 4.06);
  \draw[bar_nvlink] (\xone - 0.25 - \bw/2, 0) rectangle (\xone - 0.25 + \bw/2, 3.20);
  \draw[bar_ib] (\xone + 0.25 - \bw/2, 0) rectangle (\xone + 0.25 + \bw/2, 0.72);
  \draw[bar_pcie] (\xone + 0.75 - \bw/2, 0) rectangle (\xone + 0.75 + \bw/2, 0.91);

  % Cliff drop indicator V100
  \draw[DarkRed!60, line width=0.6pt, dashed] (\xone - 0.04, 3.20) -- (\xone + 0.25, 3.20);
  \draw[{Latex[length=3pt]}-{Latex[length=3pt]}, line width=0.7pt, DarkRed]
    (\xone + 0.25, 0.77) -- (\xone + 0.25, 3.15)
    node[midway, ratio_badge] {12$\times$};

  % Label
  \node[anchor=north, align=center, font=\scriptsize] at (\xone, -0.15) {
    \textbf{V100}\\[-1pt]\tiny\textcolor{TextMuted}{(2017)}
  };

  % === A100 (2020) ===
  \def\xtwo{5.2}
  \draw[bar_hbm] (\xtwo - 0.75 - \bw/2, 0) rectangle (\xtwo - 0.75 + \bw/2, 4.70);
  \draw[bar_nvlink] (\xtwo - 0.25 - \bw/2, 0) rectangle (\xtwo - 0.25 + \bw/2, 3.74);
  \draw[bar_ib] (\xtwo + 0.25 - \bw/2, 0) rectangle (\xtwo + 0.25 + \bw/2, 1.26);
  \draw[bar_pcie] (\xtwo + 0.75 - \bw/2, 0) rectangle (\xtwo + 0.75 + \bw/2, 1.45);

  % Cliff drop indicator A100
  \draw[DarkRed!60, line width=0.6pt, dashed] (\xtwo - 0.04, 3.74) -- (\xtwo + 0.25, 3.74);
  \draw[{Latex[length=3pt]}-{Latex[length=3pt]}, line width=0.7pt, DarkRed]
    (\xtwo + 0.25, 1.31) -- (\xtwo + 0.25, 3.69)
    node[midway, ratio_badge] {24$\times$};

  % Label
  \node[anchor=north, align=center, font=\scriptsize] at (\xtwo, -0.15) {
    \textbf{A100}\\[-1pt]\tiny\textcolor{TextMuted}{(2020)}
  };

  % === H100 (2022) ===
  \def\xthree{8.6}
  \draw[bar_hbm] (\xthree - 0.75 - \bw/2, 0) rectangle (\xthree - 0.75 + \bw/2, 5.09);
  \draw[bar_nvlink] (\xthree - 0.25 - \bw/2, 0) rectangle (\xthree - 0.25 + \bw/2, 4.06);
  \draw[bar_ib] (\xthree + 0.25 - \bw/2, 0) rectangle (\xthree + 0.25 + \bw/2, 1.80);
  \draw[bar_pcie] (\xthree + 0.75 - \bw/2, 0) rectangle (\xthree + 0.75 + \bw/2, 1.99);

  % Cliff drop indicator H100
  \draw[DarkRed!60, line width=0.6pt, dashed] (\xthree - 0.04, 4.06) -- (\xthree + 0.25, 4.06);
  \draw[{Latex[length=3pt]}-{Latex[length=3pt]}, line width=0.7pt, DarkRed]
    (\xthree + 0.25, 1.85) -- (\xthree + 0.25, 4.01)
    node[midway, ratio_badge] {18$\times$};

  % Label
  \node[anchor=north, align=center, font=\scriptsize] at (\xthree, -0.15) {
    \textbf{H100}\\[-1pt]\tiny\textcolor{TextMuted}{(2022)}
  };

  % === B200 (2024) ===
  \def\xfour{12.0}
  \draw[bar_hbm] (\xfour - 0.75 - \bw/2, 0) rectangle (\xfour - 0.75 + \bw/2, 5.77);
  \draw[bar_nvlink] (\xfour - 0.25 - \bw/2, 0) rectangle (\xfour - 0.25 + \bw/2, 4.60);
  \draw[bar_ib] (\xfour + 0.25 - \bw/2, 0) rectangle (\xfour + 0.25 + \bw/2, 2.34);
  \draw[bar_pcie] (\xfour + 0.75 - \bw/2, 0) rectangle (\xfour + 0.75 + \bw/2, 1.99);

  % Cliff drop indicator B200
  \draw[DarkRed!60, line width=0.6pt, dashed] (\xfour - 0.04, 4.60) -- (\xfour + 0.25, 4.60);
  \draw[{Latex[length=3pt]}-{Latex[length=3pt]}, line width=0.7pt, DarkRed]
    (\xfour + 0.25, 2.39) -- (\xfour + 0.25, 4.55)
    node[midway, ratio_badge] {18$\times$};

  % Label
  \node[anchor=north, align=center, font=\scriptsize] at (\xfour, -0.15) {
    \textbf{B200}\\[-1pt]\tiny\textcolor{TextMuted}{(2024)}
  };

  % -------------------------------------------------------------
  % BOTTOM INVARIANT TAKEAWAY CARD
  % -------------------------------------------------------------
  \node[
    rectangle,
    rounded corners=4pt,
    ...
    fill=CardFill,
    line width=0.8pt,
    minimum width=\waxis cm,
    inner sep=6pt,
    align=center,
    anchor=north
  ] at (6.6, -1.1) {
    \textbf{\scriptsize Fleet Hierarchy Invariant: Bandwidth Tiers Scale at Different Exponents}\\[2.5pt]
    \tiny\textcolor{TextDark}{While on-package HBM scales 8.9$\times$ from V100 to B200, inter-node InfiniBand scales 8$\times$ and PCIe scales only 4$\times$,}\\[1.5pt]
    \tiny\textcolor{SteelBlue}{\textbf{mandating communication avoidance and tensor-parallel locality within the high-bandwidth node boundary.}}
  };
...
\begin{tikzpicture}[
  ...
  % =============================================================
  % LEFT PANEL: NRZ SIGNALING
  % =============================================================
  \def\xl{-3.6}
  \node[panel] (p_nrz) at (\xl, 3.0) {};

  % Header
  \path[fill=LightBlue!60, rounded corners=4pt]
    ($(p_nrz.north west)$) rectangle ($(p_nrz.north east) + (0, -0.45)$);
  \path[fill=LightBlue!60]
    ($(p_nrz.north west) + (0, -0.3)$) rectangle ($(p_nrz.north east) + (0, -0.45)$);
  \draw[SteelBlue!40, line width=0.7pt]
    ($(p_nrz.north west) + (0, -0.45)$) -- ($(p_nrz.north east) + (0, -0.45)$);

  \node[anchor=west, font=\scriptsize\bfseries, text=SteelBlue] at ($(p_nrz.north west) + (0.3, -0.225)$)
    {NRZ Signaling};
  \node[header_badge, fill=SteelBlue, anchor=east] at ($(p_nrz.north east) + (-0.3, -0.225)$)
    {2 Levels $\cdot$ 1 Bit/Baud};
  ...
  % Grid Lines
  \draw[GridLine, dashed] (\xorgl, \yorl + 0.8) -- ++(3.8, 0);
  \draw[GridLine, dashed] (\xorgl, \yorl - 0.8) -- ++(3.8, 0);

  % Axes: stops at +3.8 cm before the bracket
  \draw[->, line width=0.6pt, SlateLine] (\xorgl, \yorl - 1.1) -- ++(0, 2.3) node[above, font=\tiny, text=TextMuted] {$V$};
  \draw[->, line width=0.6pt, SlateLine] (\xorgl, \yorl) -- ++(3.85, 0) node[above, font=\tiny, text=TextMuted] {$t$};

  % Voltage tick labels
  \node[anchor=east, font=\tiny\bfseries, text=SteelBlue] at (\xorgl - 0.05, \yorl + 0.8) {$+V$};
  \node[anchor=east, font=\tiny\bfseries, text=SteelBlue] at (\xorgl - 0.05, \yorl - 0.8) {$-V$};

  % NRZ Waveform: 1, 0, 1, 0
  \def\wint{0.82}
  \coordinate (w0) at (\xorgl + 0.25, \yorl + 0.8);
  \coordinate (w1) at ($(\xorgl + 0.25, \yorl + 0.8) + (\wint, 0)$);
  \coordinate (w2) at ($(\xorgl + 0.25, \yorl - 0.8) + (\wint, 0)$);
  \coordinate (w3) at ($(\xorgl + 0.25, \yorl - 0.8) + (2*\wint, 0)$);
  \coordinate (w4) at ($(\xorgl + 0.25, \yorl + 0.8) + (2*\wint, 0)$);
  \coordinate (w5) at ($(\xorgl + 0.25, \yorl + 0.8) + (3*\wint, 0)$);
  \coordinate (w6) at ($(\xorgl + 0.25, \yorl - 0.8) + (3*\wint, 0)$);
  \coordinate (w7) at ($(\xorgl + 0.25, \yorl - 0.8) + (4*\wint, 0)$);

  \draw[line width=1.5pt, SteelBlue, line join=round]
    (w0) -- (w1) -- (w2) -- (w3) -- (w4) -- (w5) -- (w6) -- (w7);

  % Bit values
  \node[font=\bfseries\tiny, text=NavyBlue] at ($(\xorgl + 0.25 + 0.5*\wint, \yorl + 1.05)$) {1};
  \node[font=\bfseries\tiny, text=NavyBlue] at ($(\xorgl + 0.25 + 1.5*\wint, \yorl - 1.05)$) {0};
  \node[font=\bfseries\tiny, text=NavyBlue] at ($(\xorgl + 0.25 + 2.5*\wint, \yorl + 1.05)$) {1};
  \node[font=\bfseries\tiny, text=NavyBlue] at ($(\xorgl + 0.25 + 3.5*\wint, \yorl - 1.05)$) {0};

  % Eye Height Callout Bracket
  \draw[SteelBlue, line width=0.8pt, decorate, decoration={brace, amplitude=4pt, mirror}]
    ($(\xorgl + 4.15, \yorl - 0.8)$) -- ($(\xorgl + 4.15, \yorl + 0.8)$)
    node[midway, right=5pt, align=left, font=\tiny] {
      \textbf{\textcolor{SteelBlue}{Eye: $2V$}}\\[1.5pt]
      \textcolor{TextMuted}{Full margin}\\[1.5pt]
      \textcolor{DarkGreen}{\textbf{No FEC}}
    };

  % Specs footer card
  \node[card_spec, anchor=south] at (\xl, 0.3) {
    \begin{minipage}{6.0cm}
      \tiny
      \textbf{Bit Density:} 1 bit per symbol (binary signaling)\\[1.5pt]
      \textbf{Raw Error Rate:} $\text{BER} < 10^{-12}$ (clean detection, no FEC)\\[1.5pt]
      \textbf{Latency Floor:} \textbf{\textcolor{DarkGreen}{5--10 ns}} PHY processing
    \end{minipage}
  };


  % =============================================================
  % RIGHT PANEL: PAM4 SIGNALING
  % =============================================================
  \def\xr{3.6}
  \node[panel] (p_pam) at (\xr, 3.0) {};

  % Header
  \path[fill=OrangeL, rounded corners=4pt]
    ($(p_pam.north west)$) rectangle ($(p_pam.north east) + (0, -0.45)$);
  \path[fill=OrangeL]
    ($(p_pam.north west) + (0, -0.3)$) rectangle ($(p_pam.north east) + (0, -0.45)$);
  \draw[OrangeLine!40, line width=0.7pt]
    ($(p_pam.north west) + (0, -0.45)$) -- ($(p_pam.north east) + (0, -0.45)$);

  \node[anchor=west, font=\scriptsize\bfseries, text=OrangeLine!90!black] at ($(p_pam.north west) + (0.3, -0.225)$)
    {PAM4 Signaling};
  \node[header_badge, fill=OrangeLine, anchor=east] at ($(p_pam.north east) + (-0.3, -0.225)$)
    {4 Levels $\cdot$ 2 Bits/Baud};

  % PAM4 Waveform Area
  \def\xorgr{0.7}
  \def\yorr{3.3}

  % 4 Voltage Levels: +3V (0.9cm), +1V (0.3cm), -1V (-0.3cm), -3V (-0.9cm)
  \draw[GridLine, dashed] (\xorgr, \yorr + 0.9) -- ++(3.8, 0);
  \draw[GridLine, dashed] (\xorgr, \yorr + 0.3) -- ++(3.8, 0);
  \draw[GridLine, dashed] (\xorgr, \yorr - 0.3) -- ++(3.8, 0);
  \draw[GridLine, dashed] (\xorgr, \yorr - 0.9) -- ++(3.8, 0);

  % Axes: stops at +3.8 cm before the bracket
  \draw[->, line width=0.6pt, SlateLine] (\xorgr, \yorr - 1.1) -- ++(0, 2.3) node[above, font=\tiny, text=TextMuted] {$V$};
  \draw[->, line width=0.6pt, SlateLine] (\xorgr, \yorr) -- ++(3.85, 0) node[above, font=\tiny, text=TextMuted] {$t$};

  % Voltage tick labels
  \node[anchor=east, font=\tiny\bfseries, text=OrangeLine!90!black] at (\xorgr - 0.05, \yorr + 0.9) {$+3$};
  \node[anchor=east, font=\tiny\bfseries, text=OrangeLine!90!black] at (\xorgr - 0.05, \yorr + 0.3) {$+1$};
  \node[anchor=east, font=\tiny\bfseries, text=OrangeLine!90!black] at (\xorgr - 0.05, \yorr - 0.3) {$-1$};
  \node[anchor=east, font=\tiny\bfseries, text=OrangeLine!90!black] at (\xorgr - 0.05, \yorr - 0.9) {$-3$};

  % PAM4 Waveform: 11 (+3), 01 (-1), 10 (+1), 00 (-3)
  \coordinate (u0) at (\xorgr + 0.25, \yorr + 0.9);
  \coordinate (u1) at ($(\xorgr + 0.25, \yorr + 0.9) + (\wint, 0)$);
  \coordinate (u2) at ($(\xorgr + 0.25, \yorr - 0.3) + (\wint, 0)$);
  \coordinate (u3) at ($(\xorgr + 0.25, \yorr - 0.3) + (2*\wint, 0)$);
  \coordinate (u4) at ($(\xorgr + 0.25, \yorr + 0.3) + (2*\wint, 0)$);
  \coordinate (u5) at ($(\xorgr + 0.25, \yorr + 0.3) + (3*\wint, 0)$);
  \coordinate (u6) at ($(\xorgr + 0.25, \yorr - 0.9) + (3*\wint, 0)$);
  \coordinate (u7) at ($(\xorgr + 0.25, \yorr - 0.9) + (4*\wint, 0)$);

  \draw[line width=1.5pt, OrangeLine, line join=round]
    (u0) -- (u1) -- (u2) -- (u3) -- (u4) -- (u5) -- (u6) -- (u7);

  % Bit pair values
  \node[font=\bfseries\tiny, text=OrangeLine!90!black] at ($(\xorgr + 0.25 + 0.5*\wint, \yorr + 1.15)$) {11};
  \node[font=\bfseries\tiny, text=OrangeLine!90!black] at ($(\xorgr + 0.25 + 1.5*\wint, \yorr - 0.05)$) {01};
  \node[font=\bfseries\tiny, text=OrangeLine!90!black] at ($(\xorgr + 0.25 + 2.5*\wint, \yorr + 0.55)$) {10};
  \node[font=\bfseries\tiny, text=OrangeLine!90!black] at ($(\xorgr + 0.25 + 3.5*\wint, \yorr - 1.15)$) {00};

  % Reduced Eye Bracket
  \draw[DarkRed, line width=0.8pt, decorate, decoration={brace, amplitude=4pt, mirror}]
    ($(\xorgr + 4.15, \yorr - 0.3)$) -- ($(\xorgr + 4.15, \yorr + 0.3)$)
    node[midway, right=5pt, align=left, font=\tiny] {
      \textbf{\textcolor{DarkRed}{Eye: $\frac{2}{3}V$}}\\[1.5pt]
      \textcolor{DarkRed}{$-9.5\text{ dB SNR}$}\\[1.5pt]
      \textcolor{DarkRed}{\textbf{Needs FEC}}
    };

  % Specs footer card
  \node[card_spec, anchor=south] at (\xr, 0.3) {
    \begin{minipage}{6.0cm}
      \tiny
      \textbf{Bit Density:} 2 bits per symbol ($2\times$ throughput at same baud)\\[1.5pt]
      \textbf{Raw Error Rate:} $\text{BER} \approx 10^{-4}$ (requires mandatory FEC)\\[1.5pt]
      \textbf{Latency Floor:} \textbf{\textcolor{DarkRed}{+100--200 ns}} per hop (FEC encode/decode)
    \end{minipage}
  };


  % =============================================================
  % BOTTOM SUMMARY COMPARISON CARD
  % =============================================================
  \node[
    rectangle,
    rounded corners=4pt,
    ...
    \textbf{\scriptsize Distributed Systems Trade-Off: Bandwidth Density vs.\ Collective Latency Floor}\\[2.5pt]
    \begin{minipage}{13.5cm}
      \tiny
      \textbf{Bandwidth Doubling:} PAM4 delivers $2\times$ bits per baud, enabling 400G and 800G optical rails across standard data-center fiber infrastructure.\\[1.5pt]
      \textbf{\textcolor{DarkRed}{Irreducible Latency Floor:}} The $\frac{1}{3}\times$ eye opening requires Reed-Solomon RS(544,514) FEC at each PHY layer, adding $100\text{--}200\text{ ns}$ per hop to the cluster's fixed $\alpha$ latency.
    \end{minipage}
  };
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_memory, fmt_percent, fmt_power, fmt_qty
...
\begin{tikzpicture}[
  ...
  % =============================================================
  % 1. SENDER NODE (Host A)
  % =============================================================
  \def\xs{-4.75}
  \node[node_box] (n_send) at (\xs, 3.25) {};

  % Header
  \path[fill=SlateFill, rounded corners=5pt]
    ($(n_send.north west)$) rectangle ($(n_send.north east) + (0, -0.45)$);
  \path[fill=SlateFill]
    ($(n_send.north west) + (0, -0.3)$) rectangle ($(n_send.north east) + (0, -0.45)$);
  \draw[CardBorder, line width=0.7pt]
    ($(n_send.north west) + (0, -0.45)$) -- ($(n_send.north east) + (0, -0.45)$);

  \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at ($(n_send.north west) + (0.25, -0.225)$)
    {Sender Node};
  \node[anchor=east, font=\tiny\bfseries, text=TextMuted] at ($(n_send.north east) + (-0.25, -0.225)$)
    {Host A};

  % Sender Components (centered at -4.0, leaves 1.9cm left corridor for P2P DMA)
  \def\xcs{-4.0}
  \node[comp_box, draw=SteelBlue, fill=LightBlue] (s_gpu) at (\xcs, 4.8) {
    \textbf{\scriptsize GPU Memory (HBM3)}\\[1pt]
    \tiny\textcolor{NavyBlue}{Gradient payload ($350\text{ GB}$)}
  };

  \node[comp_box, draw=SlateLine!60, fill=SlateFill!40] (s_host) at (\xcs, 3.3) {
    \textbf{\scriptsize Host CPU \& RAM}\\[1pt]
    \tiny\textcolor{TextMuted}{Bounce buffer (TCP stack)}
  };

  \node[comp_box, draw=OrangeLine, fill=OrangeL] (s_nic) at (\xcs, 1.7) {
    \textbf{\scriptsize Host NIC (ConnectX-7)}\\[1pt]
    \tiny\textcolor{OrangeLine!85!black}{$400\text{ Gb/s}$ RDMA engine}
  };


  % =============================================================
  % 2. CENTER: NETWORK FABRIC
  % =============================================================
  \def\xf{0}
  \node[fabric_box] (n_fab) at (\xf, 3.25) {};

  % Header
  \path[fill=PurpleFill!70, rounded corners=5pt]
    ($(n_fab.north west)$) rectangle ($(n_fab.north east) + (0, -0.45)$);
  \path[fill=PurpleFill!70]
    ($(n_fab.north west) + (0, -0.3)$) rectangle ($(n_fab.north east) + (0, -0.45)$);
  \draw[PurpleLine!40, line width=0.7pt]
    ($(n_fab.north west) + (0, -0.45)$) -- ($(n_fab.north east) + (0, -0.45)$);

  \node[anchor=center, font=\scriptsize\bfseries, text=PurpleLine] at ($(n_fab.north) + (0, -0.225)$)
    {Network Fabric};

  % Fabric spec card (top)
  \node[rectangle, rounded corners=3pt, draw=PurpleLine!50, fill=white, line width=0.6pt, inner sep=3.5pt, align=center, minimum width=3.0cm]
    at (\xf, 4.85) {
      \textbf{\scriptsize InfiniBand / RoCEv2}\\[1.5pt]
      \tiny\textcolor{TextMuted}{$400\text{--}800\text{ Gb/s}$ non-blocking}\\[1pt]
      \tiny\textcolor{TextMuted}{$1\text{--}3\text{ }\mu\text{s}$ flight time}
    };

  % Hero Badge: 700 GB Bounce Eliminated (middle)
  \node[rectangle, rounded corners=3pt, draw=DarkRed, fill=RedFill, line width=0.8pt, inner sep=3.5pt, align=center, minimum width=3.0cm]
    at (\xf, 3.3) {
      \textbf{\scriptsize\textcolor{DarkRed}{700 GB ELIMINATED}}\\[1.5pt]
      \tiny\textbf{\textcolor{DarkRed}{per training step}}\\[1pt]
      \tiny\textcolor{TextDark}{Zero host CPU thrashing}\\[1pt]
      \tiny\textcolor{TextMuted}{Direct PCIe peer-to-peer}
    };


  % =============================================================
  % 3. RECEIVER NODE (Host B)
  % =============================================================
  \def\xr{4.75}
  \node[node_box] (n_recv) at (\xr, 3.25) {};

  % Header
  \path[fill=SlateFill, rounded corners=5pt]
    ($(n_recv.north west)$) rectangle ($(n_recv.north east) + (0, -0.45)$);
  \path[fill=SlateFill]
    ($(n_recv.north west) + (0, -0.3)$) rectangle ($(n_recv.north east) + (0, -0.45)$);
  \draw[CardBorder, line width=0.7pt]
    ($(n_recv.north west) + (0, -0.45)$) -- ($(n_recv.north east) + (0, -0.45)$);

  \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at ($(n_recv.north west) + (0.25, -0.225)$)
    {Receiver Node};
  \node[anchor=east, font=\tiny\bfseries, text=TextMuted] at ($(n_recv.north east) + (-0.25, -0.225)$)
    {Host B};

  % Receiver Components (centered at 4.0, leaves 1.9cm right corridor for P2P DMA)
  \def\xcr{4.0}
  \node[comp_box, draw=SteelBlue, fill=LightBlue] (r_gpu) at (\xcr, 4.8) {
    \textbf{\scriptsize GPU Memory (HBM3)}\\[1pt]
    \tiny\textcolor{NavyBlue}{AllReduce accumulator}
  };

  \node[comp_box, draw=SlateLine!60, fill=SlateFill!40] (r_host) at (\xcr, 3.3) {
    \textbf{\scriptsize Host CPU \& RAM}\\[1pt]
    \tiny\textcolor{TextMuted}{Bounce buffer (TCP stack)}
  };

  \node[comp_box, draw=OrangeLine, fill=OrangeL] (r_nic) at (\xcr, 1.7) {
    \textbf{\scriptsize Host NIC (ConnectX-7)}\\[1pt]
    \tiny\textcolor{OrangeLine!85!black}{$400\text{ Gb/s}$ RDMA engine}
  };
  ...
  % =============================================================
  % 4. DATA PATHS: TRADITIONAL VS GPUDIRECT
  % =============================================================

  % PATH A: Traditional (Red dashed)
  % Sender: GPU -> Host RAM -> NIC
  \draw[->, line width=1.1pt, DarkRed, dashed] (s_gpu.south) -- (s_host.north);
  \draw[->, line width=1.1pt, DarkRed, dashed] (s_host.south) -- (s_nic.north);

  % Upper wire lane: TCP/IP wire transfer across fabric at y = 1.95
  \draw[->, line width=1.1pt, DarkRed, dashed]
    ($(s_nic.east) + (0, 0.25)$) -- ($(r_nic.west) + (0, 0.25)$);
  \node[font=\tiny\bfseries, text=DarkRed, fill=PurpleFill!25, inner sep=1.5pt] at (\xf, 2.18) {TCP/IP Wire Transport};

  % Receiver: NIC -> Host RAM -> GPU
  \draw[->, line width=1.1pt, DarkRed, dashed] (r_nic.north) -- (r_host.south);
  \draw[->, line width=1.1pt, DarkRed, dashed] (r_host.north) -- (r_gpu.south);

  % PATH B: GPUDirect RDMA (Green solid)
  % Sender: GPU direct to NIC via internal PCIe switch (centered in left corridor at x = -6.55)
  \draw[->, line width=1.4pt, DarkGreen]
    (s_gpu.west) -- (-6.55, 4.8) -- node[p2p_badge, midway] {P2P DMA} (-6.55, 1.7) -- (s_nic.west);

  % Lower wire lane: GPUDirect RDMA direct wire transfer across fabric at y = 1.45
  \draw[->, line width=1.4pt, DarkGreen]
    ($(s_nic.east) + (0, -0.25)$) -- ($(r_nic.west) + (0, -0.25)$);
  \node[font=\tiny\bfseries, text=DarkGreen, fill=PurpleFill!25, inner sep=1.5pt] at (\xf, 1.22) {GPUDirect RDMA Wire Transfer};

  % Receiver: NIC direct to GPU via internal PCIe switch (centered in right corridor at x = 6.55)
  \draw[->, line width=1.4pt, DarkGreen]
    (r_nic.east) -- (6.55, 1.7) -- node[p2p_badge, midway] {P2P DMA} (6.55, 4.8) -- (r_gpu.east);


  % =============================================================
  % 5. BOTTOM COMPARISON CARDS
  % =============================================================
  \node[
    rectangle,
    rounded corners=3pt,
    ...
    \begin{minipage}{6.7cm}
      \textbf{\scriptsize\textcolor{DarkRed}{Traditional TCP/IP Staging Path}}\\[2pt]
      \tiny
      $\bullet$ \textbf{4 Host Copies:} GPU $\to$ Host RAM $\to$ NIC $\to$ Fabric $\to$ NIC $\to$ Host $\to$ GPU\\[1pt]
      $\bullet$ \textbf{Bottleneck:} Saturates CPU memory bus; competes with data loading\\[1pt]
      $\bullet$ \textbf{CPU Tax:} Interrupt overhead \& kernel context switches stall workers
    \end{minipage}
  };
  ...
  \node[
    ...
    inner sep=5pt,
    anchor=north east
  ] (c_gpudirect) at (7.5, 0.2) {
    \begin{minipage}{6.7cm}
      \textbf{\scriptsize\textcolor{DarkGreen}{GPUDirect RDMA Zero-Copy Path}}\\[2pt]
      \tiny
      $\bullet$ \textbf{0 Host Copies:} NIC reads/writes GPU HBM directly over PCIe switch\\[1pt]
      $\bullet$ \textbf{Traffic Saved:} Eliminates $700\text{ GB}$ of host memory staging per step\\[1pt]
      $\bullet$ \textbf{Overlap Ready:} Enables true computation--communication overlap
    \end{minipage}
  };
...
\begin{tikzpicture}[
  ...
  col_box/.style={
    ...
    fill=CardFill,
    line width=0.8pt,
    minimum width=8.2cm,   % 7.1cm was narrower than its own summary rows
  ...
  % =============================================================
  % 1. LEFT COLUMN: TRADITIONAL TCP/IP STACK
  % =============================================================
  \def\xl{-4.30}
  \node[col_box] (col_tcp) at (\xl, 4.3) {};

  % Header
  \path[fill=SlateFill, rounded corners=5pt]
    ($(col_tcp.north west)$) rectangle ($(col_tcp.north east) + (0, -0.45)$);
  \path[fill=SlateFill]
    ($(col_tcp.north west) + (0, -0.3)$) rectangle ($(col_tcp.north east) + (0, -0.45)$);
  \draw[CardBorder, line width=0.7pt]
    ($(col_tcp.north west) + (0, -0.45)$) -- ($(col_tcp.north east) + (0, -0.45)$);

  \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at ($(col_tcp.north west) + (0.25, -0.225)$)
    {Traditional TCP/IP Stack};
  \node[anchor=east, font=\tiny\bfseries, text=DarkRed] at ($(col_tcp.north east) + (-0.25, -0.225)$)
    {Kernel Traversal Tax};

  % Section label: User Space
  \node[anchor=west, font=\bfseries\fontsize{6.5pt}{7pt}\selectfont, text=SlateLine] at ($(col_tcp.north west) + (0.35, -0.65)$)
    {USER SPACE};

  % Layer 1: Training Framework
  \node[layer_node, draw=SlateLine!60, fill=white] (t_layer1) at (\xl, 7.8) {
    \textbf{\scriptsize Distributed Training Framework}\\[1pt]
    \tiny\textcolor{TextMuted}{PyTorch DDP / Horovod (MPI Backend)}
  };

  \draw[->, line width=0.9pt, SlateLine] (t_layer1.south) -- ++(0, -0.25);

  % Layer 2: Host DRAM Buffer
  \node[layer_node, draw=SteelBlue, fill=LightBlue] (t_layer2) at (\xl, 6.9) {
    \textbf{\scriptsize Application User Buffer (Host DRAM)}\\[1pt]
    \tiny\textcolor{NavyBlue}{Gradient tensors staged in pinned system memory}
  };

  % Kernel Boundary dashed line and badge
  \draw[dashed, line width=0.8pt, DarkRed] (\xl-3.2, 6.2) -- (\xl+3.2, 6.2);
  \node[rectangle, rounded corners=2pt, draw=DarkRed, fill=RedFill, line width=0.6pt, inner xsep=4pt, inner ysep=1.5pt]
    (k_badge) at (\xl, 6.2) {
      \bfseries\fontsize{6.5pt}{7pt}\selectfont\textcolor{DarkRed}{Kernel Boundary (Syscall Trap \& Context Switch)}
    };

  \draw[->, line width=1.0pt, DarkRed] (t_layer2.south) -- (k_badge.north);
  ...
  % Inside Kernel Container: 3 Sublayers with clean gaps
  \node[kernel_sublayer, draw=DarkRed!50, fill=white] (k_sub1) at (\xl, 5.3) {
    \textbf{\fontsize{7.5pt}{8pt}\selectfont\textcolor{DarkRed}{BSD Socket API (send / recv)}}\\[1pt]
    \tiny\textcolor{TextDark}{\textbf{CPU Copy \#1:} User DRAM $\to$ Kernel \texttt{sk\_buff}}
  };

  \draw[->, line width=1.0pt, DarkRed] (k_badge.south) -- (k_sub1.north);

  \node[kernel_sublayer, draw=DarkRed!50, fill=white] (k_sub2) at (\xl, 4.05) {
    \textbf{\fontsize{7.5pt}{8pt}\selectfont\textcolor{DarkRed}{TCP/IP Network Protocol Stack}}\\[1pt]
    \tiny\textcolor{TextDark}{Flow Control · Retransmission · Checksumming (CPU core burn)}
  };

  \draw[->, line width=0.9pt, DarkRed] (k_sub1.south) -- (k_sub2.north);

  \node[kernel_sublayer, draw=DarkRed!50, fill=white] (k_sub3) at (\xl, 2.8) {
    \textbf{\fontsize{7.5pt}{8pt}\selectfont\textcolor{DarkRed}{Network Device Driver (DMA Rings)}}\\[1pt]
    \tiny\textcolor{TextDark}{\textbf{CPU Copy \#2:} Kernel \texttt{sk\_buff} $\to$ NIC TX Ring}
  };

  \draw[->, line width=0.9pt, DarkRed] (k_sub2.south) -- (k_sub3.north);

  % Layer 3: Standard Ethernet NIC
  \node[layer_node, draw=OrangeLine, fill=OrangeL] (t_nic) at (\xl, 1.6) {
    \textbf{\scriptsize Standard Ethernet NIC}\\[1pt]
    \tiny\textcolor{OrangeLine!85!black}{Packets framed onto physical fabric cable}
  };

  \draw[->, line width=0.9pt, SlateLine] (k_sub3.south) -- (t_nic.north);

  % Metrics Badge (Bottom Left)
  \node[
    rectangle,
    rounded corners=3pt,
    draw=DarkRed,
    ...
    \textbf{\scriptsize\textcolor{DarkRed}{Latency: $10\text{--}25\text{ }\mu\text{s}$ \quad$\vert$\quad 2--3 CPU Memory Copies}}\\[1.5pt]
    \tiny\textcolor{TextDark}{CPU cores consumed by interrupt handlers and staging copies}\\[1pt]
    \tiny\textcolor{DarkRed}{\textbf{High tail latency}} under synchronized multi-node AllReduce barriers
  };


  % =============================================================
  % 2. RIGHT COLUMN: RDMA / ROCE V2 STACK
  % =============================================================
  \def\xr{4.30}
  \node[col_box] (col_rdma) at (\xr, 4.3) {};

  % Header
  \path[fill=SlateFill, rounded corners=5pt]
    ($(col_rdma.north west)$) rectangle ($(col_rdma.north east) + (0, -0.45)$);
  \path[fill=SlateFill]
    ($(col_rdma.north west) + (0, -0.3)$) rectangle ($(col_rdma.north east) + (0, -0.45)$);
  \draw[CardBorder, line width=0.7pt]
    ($(col_rdma.north west) + (0, -0.45)$) -- ($(col_rdma.north east) + (0, -0.45)$);

  \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at ($(col_rdma.north west) + (0.25, -0.225)$)
    {RDMA / RoCEv2 Stack};
  \node[anchor=east, font=\tiny\bfseries, text=DarkGreen] at ($(col_rdma.north east) + (-0.25, -0.225)$)
    {Zero-Copy Bypass};

  % Section label: User Space
  \node[anchor=west, font=\bfseries\fontsize{6.5pt}{7pt}\selectfont, text=SlateLine] at ($(col_rdma.north west) + (0.35, -0.65)$)
    {USER SPACE};

  % Layer 1: Collective Library
  \node[layer_node, draw=SteelBlue, fill=LightBlue, minimum width=4.9cm] (r_layer1) at (\xr-0.5, 7.8) {
    \textbf{\scriptsize NVIDIA NCCL / AMD RCCL}\\[1pt]
    \tiny\textcolor{NavyBlue}{Topology-Aware Collective Engine (Ring / Tree AllReduce)}
  };

  \draw[->, line width=1.1pt, DarkGreen] (r_layer1.south) -- ++(0, -0.25);

  % Layer 2: Verbs API
  \node[layer_node, draw=DarkGreen, fill=GreenFill, minimum width=4.9cm] (r_layer2) at (\xr-0.5, 6.9) {
    \textbf{\scriptsize Direct Verbs API (\texttt{ibv\_post\_send})}\\[1pt]
    \tiny\textcolor{DarkGreen}{Direct user-space work queue posting into NIC doorbells}
  };

  % Kernel Bypass Region Box
  \node[
    rectangle,
    rounded corners=3pt,
    draw=DarkGreen!60,
    ...
    inner sep=4pt,
    align=center
  ] (r_bypass) at (\xr-0.5, 4.15) {
    \begin{minipage}{4.6cm}
      \centering
      \colorbox{GreenFill}{\parbox{4.4cm}{\centering\bfseries\fontsize{7pt}{8pt}\selectfont\textcolor{DarkGreen}{OS KERNEL BYPASSED}}}\\[10pt]
      \raggedright\tiny
      $\bullet$ \textbf{Syscall Overhead:} \textcolor{DarkGreen}{\textbf{0}} (hardware-mapped doorbells)\\[5pt]
      $\bullet$ \textbf{Memory Copies:} \textcolor{DarkGreen}{\textbf{0}} intermediate bounce buffers\\[5pt]
      $\bullet$ \textbf{Host CPU Load:} \textcolor{DarkGreen}{\textbf{0\%}} core load during payload transfer\\[5pt]
      $\bullet$ \textbf{Memory Protection:} Virtual-to-physical translation in NIC MMU
    \end{minipage}
  };

  % Layer 3: Hardware RDMA Engine & SmartNIC
  \node[layer_node, draw=OrangeLine, fill=OrangeL, minimum width=4.9cm] (r_nic) at (\xr-0.5, 1.6) {
    \textbf{\scriptsize RDMA SmartNIC (ConnectX-7 / BlueField-3)}\\[1pt]
    \tiny\textcolor{OrangeLine!85!black}{\textbf{GPUDirect RDMA:} offloads Go-Back-N / Selective ACK}
  };

  % Bypass Green Arrow: routed on the right side of the bypass box
  \draw[->, line width=1.5pt, DarkGreen]
    (r_layer2.east) -- (\xr+3.75, 6.9) -- coordinate (bp_mid) (\xr+3.75, 1.6) -- (r_nic.east);
  ...
  \node[
    ...
    inner ysep=1.5pt,
    rotate=-90
  ] at (bp_mid) {\bfseries\fontsize{6.5pt}{7pt}\selectfont\textcolor{DarkGreen}{Zero-Copy Bypass Path}};
  ...
  \node[
    ...
    \textbf{\scriptsize\textcolor{DarkGreen}{Latency: $1\text{--}3\text{ }\mu\text{s}$ \quad$\vert$\quad 0 Memory Copies (Zero-Copy)}}\\[1.5pt]
    \tiny\textcolor{TextDark}{Host CPU free: cores dedicated to forward/backward matrix math}\\[1pt]
    \tiny\textcolor{DarkGreen}{\textbf{Deterministic microsecond latency}} across thousands of cluster ranks
  };


  % =============================================================
  % 3. BOTTOM COMPARISON CARD
  % =============================================================
  \node[
    rectangle,
    rounded corners=4pt,
    ...
    \textbf{\scriptsize Architectural Takeaway: Why Distributed ML Stacks Require Kernel-Bypass RDMA}\\[2.5pt]
    \begin{minipage}{15.7cm}
      \tiny
      $\bullet$ \textbf{Synchronization Frequency Barrier:} Training iterations sync gradients every $50\text{--}200\text{ ms}$; a $20\text{ }\mu\text{s}$ TCP stack jitter penalty multiplies across collective participants, stalling all GPUs at the barrier.\\[1.5pt]
      $\bullet$ \textbf{Memory Bus Isolation:} GPUDirect DMA over PCIe keeps massive gradient flows ($350\text{ GB}$ AllReduce) completely off host DDR5 channels, preventing memory starvation during async dataset loading.
    \end{minipage}
  };
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_bandwidth, fmt_math, fmt_memory, fmt_multiple, fmt_percent, fmt_qty, fmt_time
...
import matplotlib.ticker as ticker
import numpy as np
from mlsysim.fmt import fmt_bandwidth, fmt_memory, fmt_qty
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt_bandwidth, fmt_count, fmt_math, fmt_multiple, fmt_qty
...
\begin{tikzpicture}[
  ...
  % =============================================================
  \def\xtb{-6.65}
  \node[tier_badge, draw=OrangeLine, fill=OrangeL] at (\xtb, 7.0) {
    \bfseries\tiny\textcolor{OrangeLine!90!black}{Core}\\[1pt]
    \tiny\textcolor{TextDark}{Spine}
  };

  \node[tier_badge, draw=OrangeLine, fill=white] at (\xtb, 4.4) {
    \bfseries\tiny\textcolor{OrangeLine!90!black}{Leaf}\\[1pt]
    \tiny\textcolor{TextDark}{Switches}
  };

  \node[tier_badge, draw=SteelBlue, fill=LightBlue] at (\xtb, 2.35) {
    \bfseries\tiny\textcolor{NavyBlue}{GPU}\\[1pt]
    \tiny\textcolor{NavyBlue}{Racks}
  };


  % =============================================================
  % 2. TOP FABRIC TIER: CORE SWITCHES (4 Units centered over gaps)
  % =============================================================
  \foreach \c/\xc in {1/-3.45, 2/-1.15, 3/1.15, 4/3.45} {
    \node[switch_core] (core-\c) at (\xc, 7.0) {
      \bfseries\fontsize{7pt}{8pt}\selectfont\textcolor{OrangeLine!90!black}{SPINE}\\[1pt]
      \textbf{\scriptsize Core-\c}
    };
  }

  % Top Right Non-blocking badge
  \node[
    rectangle,
    rounded corners=3pt,
    draw=DarkGreen,
    ...
    \bfseries\scriptsize\textcolor{DarkGreen}{Full Bisection}\\[1.5pt]
    \bfseries\tiny\textcolor{DarkGreen}{Non-Blocking $1:1$}
  };


  % =============================================================
  % 3. MIDDLE TIER: LEAF SWITCHES (5 Units)
  % =============================================================
  \foreach \l/\xl in {1/-4.6, 2/-2.3, 3/0.0, 4/2.3, 5/4.6} {
    \node[switch_leaf] (leaf-\l) at (\xl, 4.4) {
      \textbf{\scriptsize Leaf-\l}\\[1pt]
      \tiny\textcolor{TextMuted}{Radix-$k$ Switch}
    };
  }


  % =============================================================
  % 4. BISECTION CUT & INTERCONNECT MESH
  % =============================================================

  % Bipartite Mesh: Core <-> Leaf
  % 4 core switches x 5 leaf switches = 20 links
  \foreach \c in {1,2,3,4} {
    \foreach \l in {1,2,3,4,5} {
      % Calculate offset along bottom of core (\l goes 1..5 -> offset -0.56 to +0.56)
      \pgfmathsetmacro{\xco}{-0.56 + (\l-1)*0.28}
      % Calculate offset along top of leaf (\c goes 1..4 -> offset -0.42 to +0.42)
      \pgfmathsetmacro{\xlo}{-0.42 + (\c-1)*0.28}
      \draw[OrangeLine!45, line width=0.6pt]
        ($(core-\c.south) + (\xco, 0)$) -- ($(leaf-\l.north) + (\xlo, 0)$);
    }
  }

  % Bisection Cut Badge (Right)
  \node[
    rectangle,
    rounded corners=2pt,
    draw=DarkRed,
    ...
    inner xsep=4pt,
    inner ysep=2.5pt
  ] (bisect_badge) at (6.4, 5.65) {
    \bfseries\tiny\textcolor{DarkRed}{Bisection Cut}
  };

  % Red dashed Bisection Cut line across the mesh to the badge
  \draw[dashed, line width=1.1pt, DarkRed] (-5.6, 5.65) -- (bisect_badge.west);


  % =============================================================
  % 5. BOTTOM COMPUTE TIER: GPU RACKS (5 Units)
  % =============================================================
  \foreach \r/\xr in {1/-4.6, 2/-2.3, 3/0.0, 4/2.3, 5/4.6} {
    % Direct vertical downlinks (Leaf -> Rack)
    \draw[->, line width=1.5pt, SteelBlue] (leaf-\r.south) -- ++(0, -0.65);

    \node[rack_box] (rack-\r) at (\xr, 2.35) {
      \textbf{\scriptsize Rack \r}\\[1.5pt]
      \colorbox{LightBlue}{\parbox{1.6cm}{\centering\tiny\bfseries\textcolor{NavyBlue}{8 $\times$ GPUs}}}\\[2pt]
      \tiny\textcolor{TextDark}{$400\text{ Gb/s}$ Uplink}
    };
  }


  % =============================================================
  % 6. MIDDLE ARCHITECTURAL METRICS CARDS
  % =============================================================
  \node[
    rectangle,
    rounded corners=3pt,
    ...
    \textbf{\scriptsize Subscription Ratio}\\[1.5pt]
    \textbf{\fontsize{7.5pt}{8.5pt}\selectfont\textcolor{DarkGreen}{1:1 Non-Blocking Fabric}}\\[1pt]
    \tiny\textcolor{TextMuted}{Total Uplink Bandwidth = Total Downlink Bandwidth}
  };
  ...
  \node[
    ...
    \textbf{\scriptsize Aggregate Bisection Bandwidth}\\[1.5pt]
    \textbf{\fontsize{7.5pt}{8.5pt}\selectfont\textcolor{NavyBlue}{$5 \times 50\text{ GB/s} = 250\text{ GB/s}$}}\\[1pt]
    \tiny\textcolor{TextMuted}{Zero cross-spine saturation or oversubscription}
  };


  % =============================================================
  % 7. BOTTOM SUMMARY CARD: FAT-TREE SCALING INVARIANTS
  % =============================================================
  \node[
    rectangle,
    rounded corners=4pt,
    ...
    \textbf{\scriptsize Clos Scaling Invariants: Three-Tier Fat-Tree Topology}\\[2pt]
    \begin{minipage}{13.6cm}
      \centering\tiny
      $\bullet$ \textbf{Radix Scaling Law:} A radix-$k$ switch sustains $k^3/4$ endpoints without oversubscription ($k=64 \implies 65{,}536\text{ hosts}$).\\[1.5pt]
      $\bullet$ \textbf{Fabric Multiplicity:} At 4,096-GPU scale, the network requires $\sim 2{,}048$ fabric switches and $\sim 40{,}000$ optical transceivers.\\[1.5pt]
      $\bullet$ \textbf{All-to-All Floor:} Bisection bandwidth $\text{BW}_{\text{bisect}}$ sets the fundamental floor for distributed All-to-All and AllReduce collectives.
    \end{minipage}
  };
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_bandwidth, fmt_count, fmt_int, fmt_memory, fmt_multiple, fmt_percent, fmt_qty, fmt_time, fmt_usd
...
\begin{tikzpicture}[
  ...
  \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at ($(spine.west) + (0.3, 0)$)
    {Spine Fabric Layer};
  \node[anchor=east, font=\tiny\bfseries, text=TextMuted] at ($(spine.east) + (-0.3, 0)$)
    {Inter-Rail Cluster Routing (Only for cross-rank communication)};

  % =============================================================
  % 2. RAIL SWITCHES (Leaf Tier)
  % =============================================================
  % Switch 0 (Rail 0) - Steel Blue
  \node[rail_switch, draw=SteelBlue, fill=LightBlue] (sw0) at (-5.1, 7.4) {
    \bfseries\fontsize{7pt}{8pt}\selectfont\textcolor{NavyBlue}{RAIL 0 FABRIC}\\[1.5pt]
    \textbf{\scriptsize Switch-0}
  };

  % Switch 1 (Rail 1) - Orange
  \node[rail_switch, draw=OrangeLine, fill=OrangeL] (sw1) at (-1.7, 7.4) {
    \bfseries\fontsize{7pt}{8pt}\selectfont\textcolor{OrangeLine!90!black}{RAIL 1 FABRIC}\\[1.5pt]
    \textbf{\scriptsize Switch-1}
  };

  % Switch 2 (Rail 2) - Purple
  \node[rail_switch, draw=PurpleLine, fill=PurpleFill] (sw2) at (1.7, 7.4) {
    \bfseries\fontsize{7pt}{8pt}\selectfont\textcolor{PurpleLine}{RAIL 2 FABRIC}\\[1.5pt]
    \textbf{\scriptsize Switch-2}
  };

  % Switch 3 (Rail 3) - Teal
  \node[rail_switch, draw=TealLine, fill=TealFill] (sw3) at (5.1, 7.4) {
    \bfseries\fontsize{7pt}{8pt}\selectfont\textcolor{TealLine!90!black}{RAIL 3 FABRIC}\\[1.5pt]
    \textbf{\scriptsize Switch-3}
  };

  % Dashed uplinks from Rail Switches to Spine
  \draw[dashed, line width=0.8pt, SlateLine] (sw0.north) -- ++(0, 0.65);
  \draw[dashed, line width=0.8pt, SlateLine] (sw1.north) -- ++(0, 0.65);
  \draw[dashed, line width=0.8pt, SlateLine] (sw2.north) -- ++(0, 0.65);
  \draw[dashed, line width=0.8pt, SlateLine] (sw3.north) -- ++(0, 0.65);


  % =============================================================
  % 3. COMPUTE NODES (Node 0 and Node 1)
  % =============================================================

  % --- NODE 0 (Left) ---
  \def\xnzero{-3.6}
  \node[node_chassis] (n0) at (\xnzero, 3.55) {};

  % Footer Banner (at bottom of chassis, so top is clear for cables)
  \path[fill=SlateFill, rounded corners=5pt]
    ($(n0.south west)$) rectangle ($(n0.south east) + (0, 0.45)$);
  \path[fill=SlateFill]
    ($(n0.south west) + (0, 0.3)$) rectangle ($(n0.south east) + (0, 0.45)$);
  \draw[CardBorder, line width=0.7pt]
    ($(n0.south west) + (0, 0.45)$) -- ($(n0.south east) + (0, 0.45)$);

  \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at ($(n0.south west) + (0.25, 0.225)$)
    {Node 0 (DGX H100 Server)};
  \node[anchor=east, font=\tiny\bfseries, text=TextMuted] at ($(n0.south east) + (-0.25, 0.225)$)
    {8-way NVSwitch};

  % GPUs in Node 0
  \node[gpu_box, draw=SteelBlue, fill=LightBlue] (n0_g0) at (\xnzero-2.1, 4.45) {
    \textbf{\scriptsize\textcolor{NavyBlue}{G0}}\\[1pt]
    \tiny\textcolor{SteelBlue}{Rail 0}
  };

  \node[gpu_box, draw=OrangeLine, fill=OrangeL] (n0_g1) at (\xnzero-0.7, 4.45) {
    \textbf{\scriptsize\textcolor{OrangeLine!90!black}{G1}}\\[1pt]
    \tiny\textcolor{OrangeLine}{Rail 1}
  };

  \node[gpu_box, draw=PurpleLine, fill=PurpleFill] (n0_g2) at (\xnzero+0.7, 4.45) {
    \textbf{\scriptsize\textcolor{PurpleLine}{G2}}\\[1pt]
    \tiny\textcolor{PurpleLine}{Rail 2}
  };

  \node[gpu_box, draw=TealLine, fill=TealFill] (n0_g3) at (\xnzero+2.1, 4.45) {
    \textbf{\scriptsize\textcolor{TealLine!90!black}{G3}}\\[1pt]
    \tiny\textcolor{TealLine}{Rail 3}
  };

  % NVLink Connectors in Node 0
  \draw[line width=1.4pt, OrangeLine] (n0_g0.south) -- ++(0, -0.4);
  \draw[line width=1.4pt, OrangeLine] (n0_g1.south) -- ++(0, -0.4);
  \draw[line width=1.4pt, OrangeLine] (n0_g2.south) -- ++(0, -0.4);
  \draw[line width=1.4pt, OrangeLine] (n0_g3.south) -- ++(0, -0.4);

  % Intra-Node NVLink Bus Node 0
  \node[
    rectangle,
    rounded corners=3pt,
    draw=OrangeLine,
    ...
    inner sep=2pt,
    align=center
  ] (nv_bus0) at (\xnzero, 2.7) {
    \textbf{\fontsize{7.5pt}{8pt}\selectfont\textcolor{OrangeLine!90!black}{Intra-Node NVLink 4.0 Fabric ($900\text{ GB/s}$)}}\\[1pt]
    \tiny\textcolor{TextDark}{Dedicated High-Bandwidth Mesh for Tensor Parallelism}
  };


  % --- NODE 1 (Right) ---
  \def\xnone{3.6}
  \node[node_chassis] (n1) at (\xnone, 3.55) {};

  % Footer Banner (at bottom of chassis, so top is clear for cables)
  \path[fill=SlateFill, rounded corners=5pt]
    ($(n1.south west)$) rectangle ($(n1.south east) + (0, 0.45)$);
  \path[fill=SlateFill]
    ($(n1.south west) + (0, 0.3)$) rectangle ($(n1.south east) + (0, 0.45)$);
  \draw[CardBorder, line width=0.7pt]
    ($(n1.south west) + (0, 0.45)$) -- ($(n1.south east) + (0, 0.45)$);

  \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at ($(n1.south west) + (0.25, 0.225)$)
    {Node 1 (DGX H100 Server)};
  \node[anchor=east, font=\tiny\bfseries, text=TextMuted] at ($(n1.south east) + (-0.25, 0.225)$)
    {8-way NVSwitch};

  % GPUs in Node 1
  \node[gpu_box, draw=SteelBlue, fill=LightBlue] (n1_g0) at (\xnone-2.1, 4.45) {
    \textbf{\scriptsize\textcolor{NavyBlue}{G0}}\\[1pt]
    \tiny\textcolor{SteelBlue}{Rail 0}
  };

  \node[gpu_box, draw=OrangeLine, fill=OrangeL] (n1_g1) at (\xnone-0.7, 4.45) {
    \textbf{\scriptsize\textcolor{OrangeLine!90!black}{G1}}\\[1pt]
    \tiny\textcolor{OrangeLine}{Rail 1}
  };

  \node[gpu_box, draw=PurpleLine, fill=PurpleFill] (n1_g2) at (\xnone+0.7, 4.45) {
    \textbf{\scriptsize\textcolor{PurpleLine}{G2}}\\[1pt]
    \tiny\textcolor{PurpleLine}{Rail 2}
  };

  \node[gpu_box, draw=TealLine, fill=TealFill] (n1_g3) at (\xnone+2.1, 4.45) {
    \textbf{\scriptsize\textcolor{TealLine!90!black}{G3}}\\[1pt]
    \tiny\textcolor{TealLine}{Rail 3}
  };

  % NVLink Connectors in Node 1
  \draw[line width=1.4pt, OrangeLine] (n1_g0.south) -- ++(0, -0.4);
  \draw[line width=1.4pt, OrangeLine] (n1_g1.south) -- ++(0, -0.4);
  \draw[line width=1.4pt, OrangeLine] (n1_g2.south) -- ++(0, -0.4);
  \draw[line width=1.4pt, OrangeLine] (n1_g3.south) -- ++(0, -0.4);

  % Intra-Node NVLink Bus Node 1
  \node[
    rectangle,
    rounded corners=3pt,
    draw=OrangeLine,
  ...
  % =============================================================
  % 4. RAIL CABLING BUS ROUTES (Orthogonal Non-Overlapping Lanes)
  % =============================================================

  % RAIL 0 (Steel Blue): Node 0 G0 & Node 1 G0 -> Switch-0
  % Port offsets on Switch-0: -0.35 (N0) and +0.35 (N1)
  \draw[line width=1.4pt, SteelBlue]
    (n0_g0.north) -- ++(0, 1.3) -- ($(-5.1, 6.28) + (-0.35, 0)$) -- ($(sw0.south) + (-0.35, 0)$);
  \draw[line width=1.4pt, SteelBlue]
    (n1_g0.north) -- ++(0, 1.0) -- ($(-5.1, 5.98) + (0.35, 0)$) -- ($(sw0.south) + (0.35, 0)$);

  % RAIL 1 (Orange Line): Node 0 G1 & Node 1 G1 -> Switch-1
  % Port offsets on Switch-1: -0.35 (N0) and +0.35 (N1)
  \draw[line width=1.4pt, OrangeLine]
    (n0_g1.north) -- ++(0, 1.5) -- ($(-1.7, 6.48) + (-0.35, 0)$) -- ($(sw1.south) + (-0.35, 0)$);
  \draw[line width=1.4pt, OrangeLine]
    (n1_g1.north) -- ++(0, 1.2) -- ($(-1.7, 6.18) + (0.35, 0)$) -- ($(sw1.south) + (0.35, 0)$);

  % RAIL 2 (Purple Line): Node 0 G2 & Node 1 G2 -> Switch-2
  % Port offsets on Switch-2: -0.35 (N0) and +0.35 (N1)
  \draw[line width=1.4pt, PurpleLine]
    (n0_g2.north) -- ++(0, 0.9) -- ($(1.7, 5.88) + (-0.35, 0)$) -- ($(sw2.south) + (-0.35, 0)$);
  \draw[line width=1.4pt, PurpleLine]
    (n1_g2.north) -- ++(0, 1.4) -- ($(1.7, 6.38) + (0.35, 0)$) -- ($(sw2.south) + (0.35, 0)$);

  % RAIL 3 (Teal Line): Node 0 G3 & Node 1 G3 -> Switch-3
  % Port offsets on Switch-3: -0.35 (N0) and +0.35 (N1)
  \draw[line width=1.4pt, TealLine]
    (n0_g3.north) -- ++(0, 0.7) -- ($(5.1, 5.68) + (-0.35, 0)$) -- ($(sw3.south) + (-0.35, 0)$);
  \draw[line width=1.4pt, TealLine]
    (n1_g3.north) -- ++(0, 1.6) -- ($(5.1, 6.58) + (0.35, 0)$) -- ($(sw3.south) + (0.35, 0)$);


  % =============================================================
  % 5. ANNOTATION CARD (Principles & Invariants)
  % =============================================================
  \node[
    rectangle,
    rounded corners=4pt,
    ...
    \textbf{\scriptsize Rail-Optimized Architecture Principles}\\[3pt]
    \begin{minipage}{13.4cm}
      \tiny
      $\bullet$ \textbf{Homogeneous Rank Mapping:} Every GPU of rank $i$ across all cluster nodes connects to the exact same dedicated leaf switch (Rail $i$).\\[1.5pt]
      $\bullet$ \textbf{Contention-Free AllReduce:} Data-parallel gradient averaging between matching shards stays entirely within a single rail switch (1 switch hop, zero spine traversal, eliminating $\text{BW}_{\text{bisect}}$ contention).\\[1.5pt]
      $\bullet$ \textbf{Hybrid Physical Hierarchy:} Latency-critical tensor-parallel exchanges route over intra-node NVLink ($900\text{ GB/s}$), while inter-rail cross-rank communication traverses the spine layer only when required.
    \end{minipage}
  };


  % =============================================================
  % 6. BOTTOM LEGEND (Well-spaced across full width)
  % =============================================================
  \begin{scope}[shift={(0, -0.4)}]
    % Rail 0
    \node[rectangle, rounded corners=2pt, draw=SteelBlue, fill=LightBlue, minimum width=0.4cm, minimum height=0.3cm] at (-6.2, 0) {};
    \node[anchor=west, font=\tiny\bfseries, text=TextDark] at (-5.9, 0) {Rail 0};

    % Rail 1
    \node[rectangle, rounded corners=2pt, draw=OrangeLine, fill=OrangeL, minimum width=0.4cm, minimum height=0.3cm] at (-4.2, 0) {};
    \node[anchor=west, font=\tiny\bfseries, text=TextDark] at (-3.9, 0) {Rail 1};

    % Rail 2
    \node[rectangle, rounded corners=2pt, draw=PurpleLine, fill=PurpleFill, minimum width=0.4cm, minimum height=0.3cm] at (-2.2, 0) {};
    \node[anchor=west, font=\tiny\bfseries, text=TextDark] at (-1.9, 0) {Rail 2};

    % Rail 3
    \node[rectangle, rounded corners=2pt, draw=TealLine, fill=TealFill, minimum width=0.4cm, minimum height=0.3cm] at (-0.2, 0) {};
    \node[anchor=west, font=\tiny\bfseries, text=TextDark] at (0.1, 0) {Rail 3};

    % Intra-Node NVLink
    \draw[line width=1.5pt, OrangeLine] (1.7, 0) -- (2.2, 0);
    \node[anchor=west, font=\tiny\bfseries, text=TextDark] at (2.3, 0) {NVLink ($900\text{ GB/s}$)};

    % Spine Interconnect
    \draw[dashed, line width=1.0pt, SlateLine] (4.9, 0) -- (5.4, 0);
    \node[anchor=west, font=\tiny\bfseries, text=TextDark] at (5.5, 0) {Spine Fabric};
  \end{scope}
...
\begin{tikzpicture}[
  ...
  % =============================================================
  % 0. COORDINATE PARAMETERS
  % =============================================================
  \def\xleft{-5.3}
  \def\xright{5.3}
  \def\hplot{5.5} % height corresponding to 25 TB/s and $6.0/Gb/s

  % Five topology X-centers
  \def\xft{-4.1}
  \def\xdf{-2.05}
  \def\xtr{0.0}
  \def\xro{2.05}
  ...
  \def\barw{1.25} % bar width

  % =============================================================
  % 1. TOP LEGEND (Symmetrically Centered)
  % =============================================================
  \begin{scope}[shift={(0, 6.75)}]
    ...
    \node[anchor=west, font=\scriptsize\bfseries, text=NavyBlue] at (-3.55, 0)
      {Bisection Bandwidth ($\text{TB/s}$)};

    % Cost item
    \draw[line width=2.0pt, OrangeLine] (0.8, 0) -- (1.4, 0);
    \fill[OrangeLine] (1.1, 0) circle (3pt);
    \fill[white] (1.1, 0) circle (1.2pt);
    \node[anchor=west, font=\scriptsize\bfseries, text=OrangeLine!90!black] at (1.55, 0)
      {Interconnect Cost ($\$\text{ / Gb/s}$)};
  \end{scope}

  % =============================================================
  % 2. GRID LINES & Y-AXES
  % =============================================================
  % 6 divisions: 0, 5, 10, 15, 20, 25 TB/s <-> $0, $1.2, $2.4, $3.6, $4.8, $6.0
  \foreach \i/\tb/\cost in {
    0/0/\$0,
    1/5/\$1.2,
    2/10/\$2.4,
    ...
    \pgfmathsetmacro{\ypos}{\i * (\hplot / 5)}
    \ifnum \i > 0
      \draw[SlateLine!25, line width=0.5pt] (\xleft, \ypos) -- (\xright, \ypos);
    \fi
    \node[anchor=east, font=\scriptsize, text=TextMuted] at (\xleft - 0.15, \ypos) {\tb};
    \node[anchor=west, font=\scriptsize, text=OrangeLine!90!black] at (\xright + 0.15, \ypos) {\cost};
  }

  % Left Y-Axis (SteelBlue)
  \draw[->, line width=1.1pt, SteelBlue]
    (\xleft, 0) -- (\xleft, \hplot + 0.65);
  \node[
    rotate=90,
    anchor=south,
    font=\scriptsize\bfseries,
    text=SteelBlue
  ] at (\xleft - 0.75, \hplot / 2) {Bisection Bandwidth ($\text{TB/s}$)};

  % Right Y-Axis (OrangeLine)
  \draw[->, line width=1.1pt, OrangeLine]
    (\xright, 0) -- (\xright, \hplot + 0.65);
  \node[
    rotate=-90,
    anchor=south,
    font=\scriptsize\bfseries,
    text=OrangeLine!90!black
  ] at (\xright + 0.95, \hplot / 2) {Interconnect Cost ($\$\text{ / Gb/s}$)};

  % X-Axis Baseline
  \draw[line width=1.0pt, SlateLine!80] (\xleft, 0) -- (\xright, 0);


  % =============================================================
  % 3. BARS: BISECTION BANDWIDTH (TB/s)
  % =============================================================
  \pgfmathsetmacro{\tbscale}{\hplot / 25.0}

  % 1. Fat-Tree: 25.6 TB/s
  \pgfmathsetmacro{\yft}{25.6 * \tbscale}
  \filldraw[
    draw=SteelBlue,
    fill=LightBlue,
    line width=0.9pt,
    rounded corners=2pt
  ] ({\xft - \barw/2}, 0) rectangle ({\xft + \barw/2}, \yft);
  \node[anchor=south, font=\scriptsize\bfseries, text=NavyBlue] at (\xft, \yft + 0.05) {25.6};

  % 2. Dragonfly: 20.0 TB/s
  \pgfmathsetmacro{\ydf}{20.0 * \tbscale}
  \filldraw[
    draw=SteelBlue,
    fill=LightBlue,
    line width=0.9pt,
    rounded corners=2pt
  ] ({\xdf - \barw/2}, 0) rectangle ({\xdf + \barw/2}, \ydf);
  \node[anchor=south, font=\scriptsize\bfseries, text=NavyBlue] at (\xdf, \ydf + 0.05) {20.0};

  % 3. Torus 3D: 8.0 TB/s
  \pgfmathsetmacro{\ytr}{8.0 * \tbscale}
  \filldraw[
    draw=SteelBlue,
    fill=LightBlue,
    line width=0.9pt,
    rounded corners=2pt
  ] ({\xtr - \barw/2}, 0) rectangle ({\xtr + \barw/2}, \ytr);
  \node[anchor=south, font=\scriptsize\bfseries, text=NavyBlue] at (\xtr, \ytr + 0.05) {8.0};

  % 4. Rail-Opt.: 12.8 TB/s
  \pgfmathsetmacro{\yro}{12.8 * \tbscale}
  \filldraw[
    draw=SteelBlue,
    fill=LightBlue,
    line width=0.9pt,
    rounded corners=2pt
  ] ({\xro - \barw/2}, 0) rectangle ({\xro + \barw/2}, \yro);
  \node[anchor=south, font=\scriptsize\bfseries, text=NavyBlue] at (\xro, \yro + 0.05) {12.8};

  % 5. Butterfly: 6.4 TB/s
  \pgfmathsetmacro{\ybf}{6.4 * \tbscale}
  \filldraw[
    draw=SteelBlue,
    fill=LightBlue,
    line width=0.9pt,
    rounded corners=2pt
  ] ({\xbf - \barw/2}, 0) rectangle ({\xbf + \barw/2}, \ybf);
  \node[anchor=south, font=\scriptsize\bfseries, text=NavyBlue] at (\xbf, \ybf + 0.05) {6.4};


  % =============================================================
  % 4. X-AXIS CATEGORY LABELS
  % =============================================================
  \node[anchor=north, align=center] at (\xft, -0.15) {
    \textbf{\scriptsize\textcolor{TextDark}{Fat-Tree}}\\[1.5pt]
    \tiny\textcolor{TextMuted}{Non-Blocking}
  };

  \node[anchor=north, align=center] at (\xdf, -0.15) {
    \textbf{\scriptsize\textcolor{TextDark}{Dragonfly}}\\[1.5pt]
    \tiny\textcolor{TextMuted}{Direct Interconnect}
  };

  \node[anchor=north, align=center] at (\xtr, -0.15) {
    \textbf{\scriptsize\textcolor{TextDark}{Torus 3D}}\\[1.5pt]
    \tiny\textcolor{TextMuted}{Mesh Ring}
  };

  \node[anchor=north, align=center] at (\xro, -0.15) {
    \textbf{\scriptsize\textcolor{TextDark}{Rail-Opt.}}\\[1.5pt]
    \tiny\textcolor{TextMuted}{Rank-Segregated}
  };

  \node[anchor=north, align=center] at (\xbf, -0.15) {
    \textbf{\scriptsize\textcolor{TextDark}{Butterfly}}\\[1.5pt]
    \tiny\textcolor{TextMuted}{Multi-Stage}
  };


  % =============================================================
  % 5. LINE: INTERCONNECT COST ($ / Gb/s)
  % =============================================================
  \pgfmathsetmacro{\cscale}{\hplot / 6.0}

  \pgfmathsetmacro{\cft}{3.0 * \cscale}
  \pgfmathsetmacro{\cdf}{4.0 * \cscale}
  \pgfmathsetmacro{\ctr}{1.5 * \cscale}
  \pgfmathsetmacro{\cro}{2.0 * \cscale}
  \pgfmathsetmacro{\cbf}{5.0 * \cscale}

  % Orange Connecting Line
  \draw[line width=2.2pt, OrangeLine, line join=round]
    (\xft, \cft) -- (\xdf, \cdf) -- (\xtr, \ctr) -- (\xro, \cro) -- (\xbf, \cbf);

  % Vertices & Badges
  % 1. Fat-tree ($3.0)
  \fill[OrangeLine] (\xft, \cft) circle (3.5pt);
  \fill[white] (\xft, \cft) circle (1.3pt);
  \node[cost_badge] at ($(\xft, \cft) + (0, 0.48)$) {\$3.0};

  % 2. Dragonfly ($4.0)
  \fill[OrangeLine] (\xdf, \cdf) circle (3.5pt);
  \fill[white] (\xdf, \cdf) circle (1.3pt);
  \node[cost_badge] at ($(\xdf, \cdf) + (0, -0.48)$) {\$4.0};

  % 3. Torus 3D ($1.5)
  \fill[OrangeLine] (\xtr, \ctr) circle (3.5pt);
  \fill[white] (\xtr, \ctr) circle (1.3pt);
  \node[cost_badge] at ($(\xtr, \ctr) + (0, -0.45)$) {\$1.5};

  % 4. Rail-Opt. ($2.0)
  \fill[OrangeLine] (\xro, \cro) circle (3.5pt);
  \fill[white] (\xro, \cro) circle (1.3pt);
  \node[cost_badge] at ($(\xro, \cro) + (0, -0.45)$) {\$2.0};

  % 5. Butterfly ($5.0)
  \fill[OrangeLine] (\xbf, \cbf) circle (3.5pt);
  \fill[white] (\xbf, \cbf) circle (1.3pt);
  \node[cost_badge] at ($(\xbf, \cbf) + (0, 0.48)$) {\$5.0};


  % =============================================================
  % 6. BOTTOM COMPARISON CARD
  % =============================================================
  \def\ycard{-1.25}
  \node[
    rectangle,
  ...
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at ($(card.north) + (0, -0.21)$)
    {Topology Trade-Offs (1,024-GPU Training Cluster)};

  % Card Body Rows
  \node[anchor=north west, inner sep=0pt] at ($(card.north west) + (0.35, -0.56)$) {
    \begin{minipage}{12.9cm}
      \fontsize{6.8pt}{9.2pt}\selectfont
      \textbf{\textcolor{NavyBlue}{Fat-Tree Clos:}}\quad Delivers non-blocking 1:1 bisection bandwidth ($25.6\text{ TB/s}$), but requires high switch count and radix.\\[2pt]
      \textbf{\textcolor{OrangeLine!90!black}{Dragonfly:}}\quad Balances optical links with high-radix routers ($20.0\text{ TB/s}$, $\$4.0\text{/Gb/s}$); susceptible to AllToAll contention.\\[2pt]
      \textbf{\textcolor{OrangeLine}{Torus 3D:}}\quad Direct neighbor mesh cuts switch costs by 50\% ($\$1.5\text{/Gb/s}$), but chokes global AllToAll shuffles ($8.0\text{ TB/s}$).\\[2pt]
      \textbf{\textcolor{GreenLine}{Rail-Optimized:}}\quad Matches data-parallel communication patterns to dedicated leaf domains ($\$2.0\text{/Gb/s}$, $12.8\text{ TB/s}$).\\[2pt]
      \textbf{\textcolor{SlateLine}{Butterfly:}}\quad Eliminates redundant paths ($\$5.0\text{/Gb/s}$, $6.4\text{ TB/s}$), but suffers from severe internal blocking.
    \end{minipage}
  };
...
\begin{tikzpicture}[
  ...
  % =============================================================
  % 1. TOP GPU SENDER NODES (Row at y = 6.85)
  % =============================================================
  \def\ytop{6.85}
  \node[gpu_node] (n1) at (-5.1, \ytop) {
    \textbf{\scriptsize\textcolor{NavyBlue}{GPU Node 1}}\\[1.5pt]
    \tiny\textcolor{SteelBlue}{$50\text{ GB/s}$ Burst}
  };

  \node[gpu_node] (n2) at (-2.55, \ytop) {
    \textbf{\scriptsize\textcolor{NavyBlue}{GPU Node 2}}\\[1.5pt]
    \tiny\textcolor{SteelBlue}{$50\text{ GB/s}$ Burst}
  };

  \node[gpu_node] (n3) at (0.0, \ytop) {
    \textbf{\scriptsize\textcolor{NavyBlue}{GPU Node 3}}\\[1.5pt]
    \tiny\textcolor{SteelBlue}{$50\text{ GB/s}$ Burst}
  };

  \node[gpu_node] (n4) at (2.55, \ytop) {
    \textbf{\scriptsize\textcolor{NavyBlue}{GPU Node 4}}\\[1.5pt]
    \tiny\textcolor{SteelBlue}{$50\text{ GB/s}$ Burst}
  };

  \node[gpu_node] (n5) at (5.1, \ytop) {
    \textbf{\scriptsize\textcolor{NavyBlue}{GPU Node 5}}\\[1.5pt]
    \tiny\textcolor{SteelBlue}{$50\text{ GB/s}$ Burst}
  };


  % =============================================================
  % 2. CENTER DESTINATION SWITCH EGRESS & BUFFER (y = 3.8)
  % =============================================================
  \node[
    rectangle,
    rounded corners=4pt,
  ...
  \path[fill=OrangeL!90!OrangeLine, rounded corners=4pt]
    ($(dest.north west)$) rectangle ($(dest.north east) + (0, -0.44)$);
  \path[fill=OrangeL!90!OrangeLine]
    ($(dest.north west) + (0, -0.28)$) rectangle ($(dest.north east) + (0, -0.44)$);
  \draw[OrangeLine, line width=0.7pt]
    ($(dest.north west) + (0, -0.44)$) -- ($(dest.north east) + (0, -0.44)$);

  % 6.2pt, not \scriptsize: at \scriptsize this label is wider than the 7.4cm
  % dest box and spilled past both rounded corners.
  \node[anchor=center, font=\fontsize{6.2pt}{7pt}\selectfont\bfseries, text=OrangeLine!95!black]
    at ($(dest.north) + (0, -0.22)$)
    {Destination Switch Egress Port \& Shared Memory Buffer};

  % Destination capacity & specs
  \node[anchor=north, align=center, inner sep=0pt] at ($(dest.north) + (0, -0.56)$) {
    \textbf{\scriptsize\textcolor{TextDark}{Max Port Drain Rate: 400 Gb/s (50 GB/s)}}\\[2.5pt]
    \textbf{\scriptsize\textcolor{OrangeLine!90!black}{On-Chip Shared Memory Buffer: 32 MB}}
  };

  % Buffer Alert Strip (Red warning banner)
  \node[
    rectangle,
    rounded corners=2.5pt,
    draw=DarkRed,
    ...
    inner sep=2pt,
    align=center
  ] at ($(dest.south) + (0, 0.38)$) {
    \textbf{\fontsize{6.8pt}{8pt}\selectfont\textcolor{DarkRed}{BUFFER OVERFLOW $\cdot$ PACKET LOSS $\cdot$ PFC PAUSE BLAST}}
  };

  % Switch Ingress Port Pins (Top, Sides, Bottom)
  % Top ports:
  \node[port_pin] (p_top1) at (-2.2, 4.975) {};
  \node[port_pin] (p_top2) at (-1.1, 4.975) {};
  \node[port_pin] (p_top3) at (0.0, 4.975) {};
  \node[port_pin] (p_top4) at (1.1, 4.975) {};
  \node[port_pin] (p_top5) at (2.2, 4.975) {};

  % Side ports:
  \node[port_pin, minimum width=0.14cm, minimum height=0.28cm] (p_left) at (-3.7, 3.8) {};
  \node[port_pin, minimum width=0.14cm, minimum height=0.28cm] (p_right) at (3.7, 3.8) {};

  % Bottom ports:
  \node[port_pin] (p_bot1) at (-1.8, 2.625) {};
  \node[port_pin] (p_bot2) at (0.0, 2.625) {};
  \node[port_pin] (p_bot3) at (1.8, 2.625) {};


  % =============================================================
  % 3. SIDE GPU NODES (Left GPU 6 & Right GPU 7 at y = 3.8)
  % =============================================================
  \node[gpu_node] (n6) at (-5.1, 3.8) {
    \textbf{\scriptsize\textcolor{NavyBlue}{GPU Node 6}}\\[1.5pt]
    \tiny\textcolor{SteelBlue}{$50\text{ GB/s}$ Burst}
  };

  \node[gpu_node] (n7) at (5.1, 3.8) {
    \textbf{\scriptsize\textcolor{NavyBlue}{GPU Node 7}}\\[1.5pt]
    \tiny\textcolor{SteelBlue}{$50\text{ GB/s}$ Burst}
  };


  % =============================================================
  % 4. BOTTOM SENDER NODES (Row at y = 1.35)
  % =============================================================
  \node[gpu_node] (n8) at (-5.1, 1.35) {
    \textbf{\scriptsize\textcolor{NavyBlue}{GPU Node 8}}\\[1.5pt]
    \tiny\textcolor{SteelBlue}{$50\text{ GB/s}$ Burst}
  };

  % Expansion cluster (248 nodes)
  \node[
    rectangle,
    rounded corners=3pt,
    draw=CardBorder,
    ...
    \textbf{\scriptsize\textcolor{TextDark}{$\cdots$ 248 Additional Cluster Nodes $\cdots$}}\\[1.5pt]
    \tiny\textcolor{TextMuted}{Synchronized AllReduce / Flat Gather Incast Burst}
  };

  \node[gpu_node] (n256) at (5.1, 1.35) {
    \textbf{\scriptsize\textcolor{NavyBlue}{GPU Node 256}}\\[1.5pt]
    \tiny\textcolor{SteelBlue}{$50\text{ GB/s}$ Burst}
  };


  % =============================================================
  % 5. CONVERGING RED TRAFFIC FLOW ARROWS (Staggered tracks)
  % =============================================================
  % --- From Top Nodes ---
  % GPU 1 (outer top-left): track y = 5.75
  \draw[flow_arrow]
    (n1.south) -- ++(0, -0.66) -- ($(-2.2, 5.75)$) -- (p_top1.north);

  % GPU 2 (inner top-left): track y = 5.35
  \draw[flow_arrow]
    (n2.south) -- ++(0, -1.06) -- ($(-1.1, 5.35)$) -- (p_top2.north);

  % GPU 3 (center top): direct straight vertical
  \draw[flow_arrow]
    (n3.south) -- (p_top3.north);

  % GPU 4 (inner top-right): track y = 5.35
  \draw[flow_arrow]
    (n4.south) -- ++(0, -1.06) -- ($(1.1, 5.35)$) -- (p_top4.north);

  % GPU 5 (outer top-right): track y = 5.75
  \draw[flow_arrow]
    (n5.south) -- ++(0, -0.66) -- ($(2.2, 5.75)$) -- (p_top5.north);

  % --- From Side Nodes ---
  % GPU 6 (left)
  \draw[flow_arrow] (n6.east) -- (p_left.west);

  % GPU 7 (right)
  \draw[flow_arrow] (n7.west) -- (p_right.east);

  % --- From Bottom Nodes ---
  % GPU 8 (bottom left): track y = 2.15
  \draw[flow_arrow]
    (n8.north) -- ++(0, 0.36) -- ($(-1.8, 2.15)$) -- (p_bot1.south);

  % Expansion cluster (center): direct straight vertical
  \draw[flow_arrow]
    (n_multi.north) -- (p_bot2.south);

  % GPU 256 (bottom right): track y = 2.15
  \draw[flow_arrow]
    (n256.north) -- ++(0, 0.36) -- ($(1.8, 2.15)$) -- (p_bot3.south);


  % =============================================================
  % 6. BOTTOM ANALYTICAL BREAKDOWN CARD
  % =============================================================
  \def\ycard{-0.65}
  \node[
    rectangle,
  ...
  % Header bar
  \path[fill=SlateFill, rounded corners=5pt]
    ($(bcard.north west)$) rectangle ($(bcard.north east) + (0, -0.42)$);
  \path[fill=SlateFill]
    ($(bcard.north west) + (0, -0.28)$) rectangle ($(bcard.north east) + (0, -0.42)$);
  \draw[CardBorder, line width=0.7pt]
    ($(bcard.north west) + (0, -0.42)$) -- ($(bcard.north east) + (0, -0.42)$);

  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at ($(bcard.north) + (0, -0.21)$)
    {Incast Mathematics \& Fleet Architectural Mitigation};

  % Left Math / Systems Table (Clean Column Alignment via \makebox)
  \node[anchor=north west, inner sep=0pt] at ($(bcard.north west) + (0.35, -0.56)$) {
    \begin{minipage}{10.4cm}
      \fontsize{6.8pt}{9.4pt}\selectfont
      \makebox[3.35cm][l]{\textbf{\textcolor{DarkRed}{Aggregate Ingress Incast:}}}$256 \times 50\text{ GB/s} = 12{,}800\text{ GB/s}\ (12.8\text{ TB/s}\text{ offered load})$\\[2.5pt]
      \makebox[3.35cm][l]{\textbf{\textcolor{TextDark}{Egress Drain Rate:}}}$400\text{ Gb/s} = 50\text{ GB/s wire capacity}\ (1/256\text{ of ingress load})$\\[2.5pt]
      \makebox[3.35cm][l]{\textbf{\textcolor{DarkRed}{Buffer Depletion Time:}}}$32\text{ MB} \,/\, (12{,}750\text{ GB/s}) \approx \mathbf{2.5}\ \mu\text{s}\text{ until packet drops occur}$\\[2.5pt]
      % Plain text, not \text{}: amsmath's \text builds an unbreakable \hbox, so
      % this line could not wrap and ran out of the minipage under the badge.
      \makebox[3.35cm][l]{\textbf{\textcolor{DarkGreen}{Fleet Mitigation:}}}\parbox[t]{6.9cm}{\raggedright Multi-stage Ring/Tree collectives (no hotspot) + DCQCN/ECN throttling}
    \end{minipage}
  };

  % Right Hero Stat Badge (256 : 1 Fan-In Ratio) - Centered in body area
  \node[
    rectangle,
    rounded corners=4pt,
    draw=DarkRed,
    ...
    align=center,
    anchor=east
  ] at ($(bcard.east) + (-0.35, -0.25)$) {
    \textbf{\fontsize{13pt}{14pt}\selectfont\textcolor{DarkRed}{256 : 1}}\\[2pt]
    \textbf{\scriptsize\textcolor{DarkRed}{Fan-In Ratio}}\\[1pt]
    \tiny\textcolor{DarkRed!80!black}{Tail Latency Spike}
  };
...
#### Lossless fabric hazards: Deadlock loops, storms, and watchdog recovery {#sec-network-pfc-hazards}
...
In a lossless network, a switch cannot forward a packet to downstream buffer $B_j$ if $B_j$ is full; instead, it holds the packet in upstream buffer $B_i$ and awaits credit or pause clearance. A directed **buffer dependency graph** $G = (V, E)$ formalizes this relationship:

-   Each vertex $B_k \in V$ represents a packet buffer or virtual lane queue at a switch port.
-   A directed edge $(B_i, B_j) \in E$ exists if a packet currently occupying buffer $B_i$ requires buffer $B_j$ along its routing path to make forward progress.

A PFC credit deadlock (definition \ref{dfn-network-fabrics-pfc-deadlock}) occurs if and only if the buffer dependency graph contains a directed cycle, governed by the cyclic dependency condition in @eq-network-buffer-dependency-cycle:

$$B_1 \to B_2 \to \dots \to B_k \to B_1$$ {#eq-network-buffer-dependency-cycle}

When each buffer in the cycle fills beyond its configured pause threshold, each port asserts a PFC PAUSE frame to its predecessor. Because no buffer can advance a packet into a paused successor, no buffer can drain, and no port can de-assert its PAUSE signal. The entire cycle freezes permanently.

In production AI fabrics, deadlocks typically arise from three operational triggers:

1.  **Transient routing loops**: During link failure or BGP convergence, routing tables across neighboring switches can become temporarily inconsistent, routing packets in a multi-hop forwarding loop. Under lossy IP routing, such packets loop until their Time-to-Live (TTL) expires and are dropped. Under lossless PFC, the loop instantly fills switch buffers and locks the entire path into an unrecoverable credit cycle.
2.  **Multicast and broadcast flooding**: As revealed in Microsoft's hyperscale incident (war story \ref{ws-network-fabrics-microsoft-pfc-deadlock}), address resolution requests or flooded unknown unicast packets replicated across all ports can overwhelm egress buffers, causing pause frames to loop back into shared switch memory.
3.  **Non-tree physical links and failover paths**: Sibling cross-connects between leaf switches or asymmetric failover paths can violate strict hierarchical routing rules, forming cycles across switch tiers.
...
The attached Top-of-Rack (ToR) switch queue fills and sends a PAUSE frame to the sender. Because multi-port switches aggregate traffic from many sources across shared packet buffers, the paused queue blocks buffer allocation for unrelated packets (head-of-line blocking). Upstream leaf switches then fill their buffers and emit PAUSE frames to all connected spine switches. Within tens of milliseconds, pause frames cascade throughout the entire multi-tier fabric, pausing traffic between node pairs that never communicated with the original stalled worker. These impacted streams are termed **victim flows**\index{Victim flow!definition}.

##### Watchdog recovery and the lossy alternative {#sec-network-pfc-watchdogs-and-lossy}

To recover from deadlocks without rebooting physical switches, modern Ethernet switch operating systems implement **PFC watchdog timers**\index{PFC watchdog!definition}:

-   **Detection**: The switch continuously monitors the duration of active PAUSE states on each queue. If a queue remains paused for longer than a configured threshold $T_{\text{watchdog}}$ (typically $50\text{--}200\text{ ms}$), the switch declares a deadlock or slow-drainer anomaly.
-   **Mitigation**: The watchdog trips, disables PFC on that specific queue, and flushes the buffer by **dropping packets** until the queue empties.
-   **The systems trade-off**: Watchdogs successfully break deadlock cycles, but converting lossless traffic into packet loss forces transport layers to initiate recovery. In RoCEv2 deployments using Go-Back-N recovery, a dropped packet invalidates all subsequent packets in flight, triggering retransmission timeouts that stall the synchronous AllReduce collective for tens of milliseconds.

This systemic brittleness is driving hyperscale operators toward next-generation architectures such as the Ultra Ethernet Consortium (UEC) [@ultraEthernet2025specification], detailed in @sec-network-fabrics-protocol-convergence, and loss-tolerant RDMA designs [@mittal2018irn]. By combining packet spraying across all multipath links with line-rate packet trimming and hardware-accelerated selective retransmission, modern fabrics eliminate the need for link-layer PFC entirely, preventing deadlocks and storms by construction.
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt_bandwidth, fmt_count, fmt_memory, fmt_multiple, fmt_qty, fmt_time
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt_bandwidth, fmt_qty, fmt_ratio
...
from mlsysim.fmt import check, fmt, fmt_int, fmt_power, fmt_qty
