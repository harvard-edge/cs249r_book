...
In practice, MoE routing is rarely perfectly uniform. Popular experts receive more tokens than unpopular ones, creating load imbalance that translates to communication imbalance. If one expert receives 3$\times$ more tokens than average, the GPU hosting that expert receives 3$\times$ more incoming data, and dependent expert computation waits for that hotspot even though AllToAll is not a barrier. Large MoE systems use auxiliary load-balancing losses and capacity factors to bound this skew. Those controls trade routing freedom and possible token overflow against a lower maximum receive volume, so their value should be evaluated through end-to-end model quality and step time rather than communication balance alone.
