...
\noindent
![](images/png/cover_collective_communication_blueprint_labeled_print.png){fig-alt="Collective communication topology map where gradient fragments circulate through ring and tree reduction paths before broadcasting synchronized state."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_collective_communication_blueprint_labeled.webp){fig-alt="Collective communication topology map where gradient fragments circulate through ring and tree reduction paths before broadcasting synchronized state."}
:::
...
- Calibrate collective cost models against measurements with matching algorithms, process groups, and topology
...
The gap between that assumption and the wire reveals an asymmetry in how computation and communication scale. Computation is local because each GPU works on its own data, so aggregate compute grows with the number of GPUs\index{GPU}\index{Graphics Processing Unit|see{GPU}}. Communication is global because keeping the fleet synchronized requires information to cross physical links with finite latency, bandwidth, topology, and energy. Gradient synchronization provides the central case; alpha-beta cost models, collective primitives, AllReduce schedules, topology-aware routing, compression, and overlap make its mechanics concrete.
...
class GradSyncSignificance:
    ...
    gradient_top1_gb_str = fmt_qty(gradient * 0.01, GB, precision=1, commas=False)
    gradient_2x_gb_str = fmt_qty(gradient * 2, GB, precision=0, commas=False)
...
1.  **Significance**: A 70B-parameter model in BF16 generates `{python} GradSyncSignificance.gradient_gb_str` of gradient data per worker per step. Synchronizing across `{python} GradSyncSignificance.n_gpus_str` via ring AllReduce at `{python} GradSyncSignificance.beta_gb_per_s_str` per link requires approximately `{python} GradSyncSignificance.ring_operands_math` $\approx$ `{python} GradSyncSignificance.ring_time_s_str` of communication per step, making the bandwidth (data-movement) term large enough to dominate the iron law unless overlap, hierarchy, or compression reduces the exposed transfer.
...
Physical constraints govern data movement before algorithms are designed. @Sec-network-fabrics-wire-link establishes the wire-level physics behind these algorithms: the speed of light sets a latency floor[^fn-speed-of-light] (roughly 5 $\mu\text{s}$ per kilometer in fiber), the bandwidth-distance product limits how far a fast link can reach before it needs optics (PAM4 signaling and copper-vs.-optics reach are developed in @sec-network-fabrics-pam4), and kernel-bypass transports such as remote direct memory access (RDMA) and GPUDirect RDMA strip the per-message software tax down to a few microseconds (@sec-network-fabrics-rdma). Collective communication adds the energy cost of moving a bit, which the algorithms must respect as firmly as latency and bandwidth.

Moving data costs energy that scales exponentially with distance. @Fig-data-movement-hierarchy illustrates this data-movement energy hierarchy across four physical tiers: local on-chip SRAM ($\sim$0.5\,pJ/bit), on-package high-bandwidth memory (HBM)\index{HBM}\index{High-Bandwidth Memory|see{HBM}} ($\sim$20\,pJ/bit), intra-node NVLink ($\sim$100\,pJ/bit), and optical inter-node InfiniBand ($\sim$1{,}000\,pJ/bit). The figure plots higher-bound published estimates that include link-, switch-, and transceiver-side power; chapter prose elsewhere uses the lower bound of each range for back-of-the-envelope models. Both views agree on the central thermodynamic reality: crossing from on-package SRAM to a 50-meter optical data center fabric incurs a 2{,}000$\times$ energy penalty, making physical data locality the primary driver of sustainable distributed ML system design.

::: {#fig-data-movement-hierarchy fig-env="figure" fig-pos="htb" fig-cap="**The data-movement energy hierarchy**: The energy cost of moving a single bit climbs by orders of magnitude across the system hierarchy: from local SRAM (0.5\\,pJ/bit) through HBM (20\\,pJ/bit, $40\\times$) and intra-node NVLink (100\\,pJ/bit, $200\\times$) to inter-node InfiniBand (1{,}000\\,pJ/bit, $2{,}000\\times$). Higher-bound published estimates include link, switch, and laser transceiver overheads. This steep thermodynamic gradient makes data locality the primary driver of sustainable distributed ML system architecture." fig-alt="Horizontal bar chart showing increasing energy cost (pJ per bit) on a logarithmic scale from local SRAM through HBM, NVLink, and InfiniBand to remote data center nodes. Multiplier badges on the right highlight the 1x baseline, 40x, 200x, and 2,000x energy cliff."}
...
\node[tier label] (lbl4) at (2.5, 1.9) {
  \textbf{\normalsize InfiniBand NDR}\\[1.5pt]
...
The calculation reveals why data movement, not computation, becomes the governing constraint at scale. A single AllReduce on a 70B model's gradients consumes seconds of wall-clock time, during which all GPUs would otherwise sit idle. Kernel bypass matters because RDMA's zero-copy path[^fn-zerocopy-rdma-bypass] avoids staging each transfer through host memory. This asymmetry between local computation (which parallelizes perfectly) and global coordination (which requires physical data movement) motivates every collective algorithm that follows. Halving that multi-second AllReduce would save thousands of GPU-hours over a typical training campaign, translating directly to reduced cost and faster time to deployment.
...
# earlier SingleNodeModel model held per-GPU compute fixed while comm grew, driving
...
The specific "travel manifest" for this journey is dictated by the parallelism strategy chosen in @sec-distributed-training-systems. The choice of how the math is split determines how the data moves. For the Lighthouse Archetypes (@sec-vol2-introduction-archetypes), these manifests differ fundamentally. For Archetype A (GPT-4/Llama-3), the gradient is part of a massive, dense tensor that must meet every other gradient in the fleet to compute a global average, so its primary vehicle is the AllReduce. For Archetype B (DLRM at Scale),[^fn-dlrm-embedding-alltoall] the gradient or activation is sparse and targeted: it does not need to meet everyone, only the specific GPU that holds its embedding shard. Mixture-of-experts (MoE) routing creates the same targeted pattern, sending each token to the GPU that hosts its assigned expert, so both rely on the AllToAll.
...
| **Expert Parallelism (MoE)** | Targeted routing to a specialist      |          AllToAll          | Startup, injection/bisection, or skew |
...
The $\alpha$ and $\beta$ terms in that table are not converging. @Fig-bandwidth-divergence shows intra-node bandwidth pulling steadily away from inter-node bandwidth, which is why the critical message size that favors a hierarchical algorithm keeps moving rather than settling.

::: {#fig-bandwidth-divergence fig-env="figure" fig-pos="htb" fig-cap="**The bandwidth divergence**: Intra-node interconnect bandwidth across PCIe and NVLink generations against inter-node InfiniBand, from 2012 to 2024. The intra-node line holds roughly an order of magnitude above the inter-node line throughout, which sets the critical message size at which an algorithm should stop crossing machine boundaries." fig-alt="Log-scale two-line chart from 2012 to 2024 comparing intra-node interconnect bandwidth, labeled across PCIe and NVLink generations, against inter-node InfiniBand bandwidth. The intra-node line stays roughly an order of magnitude above the inter-node line throughout."}
...
The critical message size separates two distinct operating regimes. Below it, small messages such as MoE routing tokens or scalar reductions are latency-bound $(n < n^*)$, where time is dominated by $\alpha$ and optimization focuses on fusion (batching small messages), topology (reducing hop count), and software-stack tuning (kernel bypass via RDMA). Above it, large messages such as LLM gradients or optimizer states are bandwidth-bound $(n \gg n^*)$, where time is dominated by $n/\beta$ and optimization shifts to compression\index{Gradient Compression} (lower precision or sparsity), algorithm choice (Ring vs. Tree), and link aggregation\index{Link Aggregation} (multi-rail network interface cards (NICs)). The distinction between latency-bound and bandwidth-bound communication is the diagnostic skill to practice.
...
The α-β model assumes the processor is idle during communication. For pipelined systems where communication overlaps with computation, this assumption fails. The **LogP Model**\index{LogP Model!definition} [@culler1993] extends α-β with four parameters:

*   **$L_{\text{lat}}$ (Latency)**: The time for a message to traverse the network (similar to $\alpha$).
*   **$o$ (Overhead)**\index{LogP Model!processor overhead}: The processor time spent initiating or completing a transfer. During this work, the same execution resource cannot compute, making $o$ the nonoverlappable cost. In a distributed training context, dispatch, device-kernel launch, memory registration, and runtime progress can all contribute. The value is an effective parameter of a particular software and hardware path, so it must be measured with the same collective, process group, and topology used by the workload.
...
\begin{tikzpicture}[line join=round, line cap=round, font=\small\sffamily]
  ...
  \node[compute_box] (C_L4) at (0.0, 2.0) {\textbf{Backward $L_4$}\\[1pt]\footnotesize Compute $\nabla W_4$};
  \node[compute_box] (C_L3) at (2.4, 2.0) {\textbf{Backward $L_3$}\\[1pt]\footnotesize Compute $\nabla W_3$};
  \node[compute_box] (C_L2) at (4.8, 2.0) {\textbf{Backward $L_2$}\\[1pt]\footnotesize Compute $\nabla W_2$};
  \node[compute_box] (C_L1) at (7.2, 2.0) {\textbf{Backward $L_1$}\\[1pt]\footnotesize Compute $\nabla W_1$};
  ...
  \node[comm_box] (M_L4) at (2.4, 0.0) {\textbf{AllReduce $L_4$}\\[1pt]\footnotesize\color{GreenLine!90!black}\textbf{100\% Hidden}};
  \node[comm_box] (M_L3) at (4.8, 0.0) {\textbf{AllReduce $L_3$}\\[1pt]\footnotesize\color{GreenLine!90!black}\textbf{100\% Hidden}};
  \node[comm_box] (M_L2) at (7.2, 0.0) {\textbf{AllReduce $L_2$}\\[1pt]\footnotesize\color{GreenLine!90!black}\textbf{100\% Hidden}};
  \node[stall_comm] (M_L1) at (9.6, 0.0) {\textbf{AllReduce $L_1$}\\[1pt]\footnotesize\color{RedLine!90!black}\textbf{Exposed Latency}};
  ...
  \node[anchor=south, font=\footnotesize\bfseries\sffamily\color{GreenLine!90!black}, fill=white, inner sep=2.5pt, rounded corners=2pt]
    at (5.95, 3.05) {Pipelined Overlap Region ($L_{\text{lat}}$ Hidden)};
  ...
  \node[
    ...
    \begin{minipage}{14.5cm}\footnotesize\sffamily
      \begin{tabular}{@{}p{7.3cm}@{\hspace{0.3cm}}p{6.9cm}@{}}
        ...
        \textbf{Compute:} Reverse autograd ($L_4 \to L_1$), non-blocking &
        $\bullet$ \textbf{Hidden Latency:} $\sum_{i=2}^4 \text{Time}(L_i)$ masked by compute \\
        ...
        $\bullet$ \textbf{Exposed Barrier:} $\text{Time}(L_1)$ sets sync stall floor \\
        ...
        $\bullet$ \textbf{Step Time:} $T_{\text{step}} = T_{\text{backward}} + T_{\text{tail}} + T_{\text{optimizer}}$
      \end{tabular}
    \end{minipage}
  };
...
The choice between models depends on the analysis context. The $\alpha$-$\beta$ model is the right tool for back-of-envelope calculations, algorithm selection (Ring vs. Tree), and blocking communication whose completion lies on the dependency path. Its strength is simplicity, since the two parameters can be estimated from latency and bandwidth measurements. Its weakness is the assumption that the processor is idle during communication, which makes it overly pessimistic when overlap is possible. The LogP model earns its extra parameters when the analysis turns to **Pipelined Execution**\index{Pipelined Execution}, compute-communication overlap, or diagnosing why a theoretically fast algorithm underperforms. Its distinction between network latency $L_{\text{lat}}$ and processor overhead $o$ determines whether a given overlap strategy can hide communication. The profiling method in @sec-performance-engineering-profiling measures those terms and attributes the residual; this chapter uses them to choose the communication schedule.
...
The per-phase breakdown behind these numbers (the intra-node reduction, the reduced inter-node exchange, and the intra-node redistribution) builds on collective primitives developed in @sec-communication-collective-operations-collective-operations-collective-operation-vocabulary-fdc7. @Sec-communication-collective-operations-collective-operations-hierarchical-allreduce-1338 names those primitives and works the full three-phase derivation; this budget establishes that the bandwidth hierarchy is worth respecting, and by how much.

### Theory vs. practice: Calibrating a collective model {#sec-communication-nccl-reality}
...
If a GPU simply opens a socket and sends a massive gradient to another GPU, the entire cluster will rapidly collapse into an unmanageable web of deadlocks and congestion. With the terrain mapped, the gradient must now choose its vehicle: strictly choreographed group exchanges known as collective operations, a vocabulary standardized by MPI and inherited by modern ML communication libraries [@mpiforum2015mpi31]. @Fig-collective-primitives-overview illustrates four of the central primitives in this vocabulary: AllReduce, AllGather, ReduceScatter, and AllToAll. Each panel reads as a before-and-after across the process group, with one row per rank: blue cells mark the input each rank starts with, green cells mark the aggregated or reconstructed output tensors, and violet marks the AllToAll pairwise shuffle that transposes unique shards across all ranks. Two further primitives, Broadcast (rank 0 sends to all) and Reduce (all aggregate to rank 0), are foundational building blocks defined in @sec-collective-communication-six-core-primitives-8a66 and used implicitly in the compound patterns shown in the figure. @Sec-appdx-comm-foundations-collective-complexity formalizes the semantics and latency and bandwidth complexity of each primitive; the prose here introduces them through their workload use cases, and the reader who wants the formal complexity bounds before proceeding can establish them there first.
...
1.  **Significance**: The right collective algorithm determines whether communication scales with cluster size or remains constant. Ring AllReduce achieves bandwidth-optimal $2(N-1)/N \times M / \beta$ per node (constant in $N$), while a naive reduce-then-broadcast approach costs $\mathcal{O}(N \times M / \beta)$, making it 500$\times$ worse for $N=1024$. Algorithm selection thus directly determines whether the $\text{BW}$ term in the iron law is the training bottleneck.
...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 5.0)}, local bounding box=boxAllReduce]
    ...
    \node[anchor=west, font=\tiny\sffamily, text=TextMuted] at (0, 4.02) {Aggregates distributed arrays; all devices receive the identical sum.};
    ...
    % Footer Note
    \node[anchor=north, font=\scriptsize, text=TextDark] at (3.55, 0.95) {\textbf{Core Usage:} Data Parallel gradient synchronization (DDP)};
  \end{scope}
  ...
  \begin{scope}[shift={(7.8, 5.0)}, local bounding box=boxAllGather]
    ...
    \node[anchor=west, font=\tiny\sffamily, text=TextMuted] at (0, 4.02) {Collects partial shards from each device into the complete tensor.};
  ...
  \begin{scope}[shift={(0, 0)}, local bounding box=boxReduceScatter]
    ...
    \node[anchor=east, font=\bfseries\scriptsize, text=OrangeLine] at (7.1, 4.35) {Unique Shard Each};
    \node[anchor=west, font=\tiny\sffamily, text=TextMuted] at (0, 4.02) {Reduces full arrays across devices, then scatters disjoint reduced slices.};
    ...
    % Central Operator
    \node[op orange, minimum width=1.8cm, minimum height=0.75cm] (rs_op) at (3.55, 2.375) {ReduceScatter};
    ...
    % Footer Note
    \node[anchor=north, font=\scriptsize, text=OrangeLine] at (3.55, 0.95) {\textbf{Core Usage:} FSDP / ZeRO gradient reduction into sharded state};
  \end{scope}
  ...
  \begin{scope}[shift={(7.8, 0)}, local bounding box=boxAllToAll]
    ...
    % Central Operator
    \node[op violet, minimum width=1.8cm, minimum height=0.75cm] (ata_op) at (3.55, 2.375) {AllToAll $\rightleftarrows$};
...
The six primitives in @tbl-collective-primitives form a decision ladder: use the narrowest operation that preserves the model's mathematics, because every broader pattern adds data movement, coordination, or contention.
...
An AllToAll over $N$ ranks creates $N(N-1)$ logical source-destination transfers, yet each rank's aggregate payload can remain fixed under weak scaling. Its lower bound is governed by the largest of four terms: source injection time, destination reception time, traffic crossing the fabric bisection, and per-message startup or scheduling overhead. Oversubscription and routing determine how much physical capacity those flows share. Message granularity determines how often they pay $\alpha$. Routing skew determines whether one destination or link becomes the critical path. A full-bisection network carrying balanced, coarse-grained traffic can therefore scale well despite the quadratic logical connection count, whereas an oversubscribed fabric or skewed expert assignment can stall at much smaller $N$.

::: {.column-margin}
```{=latex}
\vspace*{-14mm}
```
![](images/svg/vol2_collective_communication_margin_001.svg){width="100%" fig-alt="All-to-All quadratic logical source-destination pairs compared with the linear number of ranks; physical cost depends on how those flows share the fabric."}

*Logical peer pairs grow quadratically; physical cost depends on the fabric.*
:::

Expert parallelism and large-scale recommendation systems\index{Recommendation System} can reach a communication wall earlier than standard data parallelism when their traffic exhausts injection, reception, or bisection capacity, or when routing and destination skew create hotspots. The primitive determines the traffic matrix; the physical fabric and routing distribution determine the scaling ceiling.
...
class MoeAllToAll:
    ...
    check(abs(parallel.to(microsecond).magnitude - 39.70016) < 1e-9, f"Parallel injection lower bound = {parallel.to(microsecond).magnitude} μs")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    alpha_us_str = fmt_time(alpha, microsecond, precision=0, commas=False)
    beta_gb_per_s_str = fmt_qty(beta, GB / second, precision=0, commas=False)
...
**Systems insight**: The injection result is only a lower bound. If the topology's usable bisection bandwidth is $B_{\mathrm{bisect}}$ and $V_{\mathrm{cut}}$ bytes must cross its minimum cut, the exchange also obeys $T\geq V_{\mathrm{cut}}/B_{\mathrm{bisect}}$. Oversubscription reduces $B_{\mathrm{bisect}}$, fine-grained peer messages add startup cost, and expert skew raises the maximum receive volume above the uniform estimate. The AllToAll also runs twice per layer, once for token dispatch and once for result collection.

:::

#### Dynamic routing and expert imbalance in MoE AllToAll {#sec-collective-communication-moe-dynamic-routing}

The worked trace in notebook \ref{nbk-collective-communication-alltoall-moe-token-routing} assumes uniform token routing: every expert receives an identical share of tokens. In production MoE models, routing decisions are dynamic and data-dependent, fundamentally altering the collective communication profile.

##### The dispatch and combine lifecycle

In an MoE transformer layer with $E$ experts distributed across $P$ ranks ($E/P$ experts per rank), each rank holds a local batch of $T$ tokens. A gating router computes top-$k$ routing probabilities over all $E$ experts for each token. This induces two consecutive collective communication phases per MoE layer:

1.  **Token Dispatch (All-to-All)**: Each rank partitions its $T$ local token activation vectors ($\mathbf{x} \in \mathbb{R}^{d_{\text{model}}}$) according to the destination ranks hosting the selected experts. Devices execute an AllToAll collective (or variable-size `AllToAllv`) to route tokens across the fabric to the expert host ranks.
2.  **Token Combine (All-to-All)**: After each rank computes forward passes through its assigned local expert feed-forward networks (FFNs), a second AllToAll collective routes the resulting output vectors back to the originating ranks. Each rank then computes the weighted combination of expert outputs:
    $$\mathbf{y}_i = \sum_{j \in \text{top-}k} g_{i,j} \cdot \text{Expert}_j(\mathbf{x}_i)$$

##### Capacity factors and bounded allocation

In dense models, collective buffer sizes are static and deterministic. In sparse MoE models, the number of tokens routed to expert $e$ is a stochastic variable $N_e \sim \text{Binomial}(P \cdot T, p_e)$. To avoid dynamic memory allocation and buffer reallocation overhead during communication, systems pre-allocate static expert buffers parameterized by an **expert capacity factor** $C \ge 1.0$ [@lepikhin2021gshard; @fedus2022switch]:
$$\text{Capacity}_e = \left\lceil \frac{P \times T \times k}{E} \times C \right\rceil$$

The capacity factor exposes a fundamental systems trade-off:

*   **When $C = 1.0$**: Buffers are sized strictly for the uniform expectation. Under routing skew, any tokens assigned to an expert beyond its capacity are *dropped* (bypassed via residual connections without expert computation), degrading training convergence and downstream task accuracy.
*   **When $C > 1.0$**: Buffers accommodate routing variance (typically $C = 1.25$–$1.5$), preventing token dropping. However, to maintain uniform collective communication, buffers must be padded with dummy tokens to fixed capacity, inflating the physical data volume crossing the network by $C\times$ and forcing GPUs to compute matrix multiplications on padded null tokens.
*   **Dropless dynamic routing**: Modern high-throughput frameworks utilize variable-size `AllToAllv` with dynamic memory buffers, eliminating padding overhead while preserving all tokens. However, dynamic routing exposes execution time directly to routing skew.

##### Bisection saturation and straggler tails

Because token routing is determined by input semantics, certain "celebrity" experts inevitably receive disproportionate token volumes. If one expert receives $\gamma \times$ the average load ($\gamma > 1$), the GPU hosting that expert receives $\gamma \times$ more incoming data during dispatch.

This skew creates two compounding performance bottlenecks:

1.  **Inbound reception bottleneck**: The completion time of the dispatch collective is bounded by the slowest receiver. Even if 95 percent of GPUs complete their AllToAll exchanges in tens of microseconds, the entire cluster stalls waiting for the hottest destination rank to ingest its oversized payload:
    $$T_{\text{dispatch}} \ge \frac{M_{\text{max}} \times d_{\text{model}} \times \text{sizeof(dtype)}}{\text{BW}_{\text{rx}}}$$
    where $M_{\text{max}} = \max_p \sum_{e \in \text{rank } p} N_e$.
2.  **Bisection fabric saturation**: Unlike Ring AllReduce (which maps communication onto nearest-neighbor ring links), AllToAll generates a complete bipartite exchange where $(P-1)/P \approx 1$ of all tokens must cross the cluster network fabric. In an oversubscribed fabric with oversubscription ratio $R:1$, the usable bisection bandwidth $\text{BW}_{\text{bisect}} = (P \cdot \text{BW}_{\text{inject}}) / (2R)$ throttles the exchange:
    $$T_{\text{dispatch}} \ge \frac{V_{\text{cut}}}{\text{BW}_{\text{bisect}}}$$

##### Hierarchical MoE exchange

To mitigate both bisection saturation and small-message overhead across thousands of GPUs, hierarchical MoE runtimes decompose the flat AllToAll into a two-level collective [@hwang2023tutel]:

1.  **Intra-node aggregation**: GPUs use intra-node NVLink to coalesce tokens destined for any expert residing on the same remote server node.
2.  **Inter-node exchange**: Nodes execute an inter-node AllToAll over InfiniBand or RoCE, exchanging aggregated, coarse-grained payloads rather than thousands of fine-grained pairwise messages.
3.  **Intra-node distribution**: Receiving GPUs use NVLink to scatter incoming tokens to the exact GPUs hosting the targeted experts.

This hierarchical decomposition reduces the number of concurrent inter-node network flows from $P(P-1)$ to $N_{\text{nodes}}(N_{\text{nodes}}-1)$, replacing fine-grained, latency-sensitive packets with bandwidth-saturating message bursts that shield the data center bisection from packet fragmentation.

#### Selecting an AllToAll schedule {#sec-collective-communication-alltoall-schedule-selection}
...
|:------------------------------------|:------------------------------------------------------------------------------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Direct pairwise exchange**        | Balanced, coarse per-peer chunks on a small or full-bisection process group                                                   | Each rank injects directly to its peers; fine granularity repeats startup and scheduling cost, while oversubscription can bind the bisection                      |
...
The selection test follows the lower bound developed above. If per-peer chunks are coarse and source injection or fabric bisection already dominates, coalescing cannot remove the byte term and may only add copies. If $N\alpha$ dominates because each remote destination receives a small chunk, hierarchy can replace many inter-node startups with fewer aggregate transfers. If counts are skewed, a variable-size interface prevents padding from inflating $V_{\mathrm{cut}}$, but the schedule must still provision for the maximum receive volume and expert compute load. Consequently, a runtime should benchmark candidate schedules with the observed count distribution, rank placement, and topology; a balanced fixed-size microbenchmark cannot select a production AllToAll schedule.

@Tbl-collective-selection maps these primitives to the parallelism strategies introduced in @sec-distributed-training-systems and the lighthouse archetypes introduced in @sec-vol2-introduction-archetypes.
...
| **MoE (Experts)**        |          AllToAll          | Startup, injection/bisection, or skew |
| **DLRM (RecSys)**        |          AllToAll          | Granularity, fabric capacity, or skew |
...
The FSDP/ZeRO strategy deserves special attention because its communication pattern differs fundamentally from standard data parallelism [@rajbhandari2020]. In standard data parallelism, each GPU holds a complete copy of the model, computes gradients locally, and synchronizes via a single AllReduce at the end of the backward pass. Full sharding eliminates this redundancy by partitioning the model parameters across GPUs, so each GPU holds only its local shard outside the moments when a layer must be reconstructed.
...
The higher operation count makes FSDP more sensitive to latency ($\alpha$) than standard data parallelism. With $2N_L$ collectives for $N_L$ layers, an effective startup cost $\alpha_{\mathrm{eff}}$ contributes approximately $2N_L\alpha_{\mathrm{eff}}$ before overlap. FSDP implementations mitigate this cost through prefetching\index{Prefetching} and communication-stream pipelining, but the relevant $\alpha_{\mathrm{eff}}$ must come from a matched collective benchmark rather than a wire-latency specification.
...
As $N$ grows large, this approaches $2M/\beta$, which is independent of $N$. An algorithm that achieves this bound is bandwidth-optimal. Ring AllReduce achieves this bound exactly; simple tree variants do not [@patarasuk2009; @thakur2005]. This distinction has practical consequences: on a 64-GPU cluster synchronizing a 1 GB gradient, the bandwidth difference between an optimal and a merely logarithmic algorithm amounts to several milliseconds per step, which accumulates to hours over a multi-day training run.

### Ring AllReduce {#sec-communication-collective-operations-collective-operations-ring-allreduce-ffce}

Ring AllReduce[^fn-ring-bandwidth-optimal] arranges nodes in a logical ring ($0 \to 1 \to \dots \to N-1 \to 0$). It achieves bandwidth optimality by pipelining: every node sends and receives simultaneously on every link [@patarasuk2009].
  ...
  \node[
    ...
    \begin{minipage}{14.5cm}\footnotesize\sffamily
      \begin{tabular}{@{}p{7.1cm}@{\hspace{0.4cm}}p{7.0cm}@{}}
        ...
        $\bullet$ \textbf{Latency Overhead:} $T_{\text{lat}} = 2(N-1) \cdot \alpha$ (scales linearly with $N$)
      \end{tabular}
    \end{minipage}
  };
...
::: {#exmp-collective-communication-ring-allreduce-step-by-step-trace-4-gpus .callout-example title="Ring AllReduce: Step-by-step trace (4 GPUs)"}
...
This bandwidth penalty is catastrophic for large-scale data parallelism when the implementation repeatedly exchanges full messages or creates root/interior bottlenecks. For a 70B-parameter model's `{python} GradSyncSignificance.gradient_gb_str` gradient AllReduce across 1,000 GPUs, a bandwidth-inefficient tree-like variant can erase the latency advantage that motivated it. Optimized double-tree algorithms exist precisely to keep logarithmic latency without imposing that naive bandwidth penalty.
...
Ring and Tree represent two extremes of the latency-bandwidth trade-off: Ring is bandwidth-optimal but latency-poor, while Tree is latency-optimal but bandwidth-poor. A third approach combines the logarithmic latency of Tree with better bandwidth utilization. The **Recursive Halving-Doubling**\index{Recursive Halving-Doubling} algorithm (sometimes called the Butterfly algorithm\index{Butterfly Algorithm}) operates in $2\log_2 N$ rounds total: $\log_2 N$ ReduceScatter rounds followed by $\log_2 N$ AllGather rounds. In each round $k$ of either phase, every GPU exchanges data with a partner at distance $2^k$ in the logical numbering, and the message size halves (in the ReduceScatter phase) or doubles (in the AllGather phase).
...
Evaluating the AllReduce algorithm comparison (@tbl-allreduce-comparison) and crossover boundaries (@fig-allreduce-crossover) reveals that algorithm selection depends strictly on message size: Tree dominates for small messages while Ring wins for large ones.
...
The AllReduce algorithms in @sec-communication-collective-operations-collective-operations-allreduce-algorithms-7545 assume a flat network where every link has the same bandwidth. Real data centers violate this assumption by an order of magnitude or more. Not all wires are created equal.
...
A naive flat Ring AllReduce ignores this structure, routing data across InfiniBand when NVLink would suffice and saturating scarce inter-node bandwidth. **Hierarchical AllReduce**\index{Hierarchical AllReduce} decomposes the global operation into three phases that exploit the bandwidth hierarchy (@fig-hierarchical-allreduce).
...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(0, 1.8)}, local bounding box=node0]
    % Header
    \node[anchor=west, font=\bfseries\small, text=TextDark] at (0, 4.9) {Node 0 (8-GPU Server Chassis)};
  ...
  \begin{scope}[shift={(7.4, 1.8)}, local bounding box=node1]
    % Header
    \node[anchor=west, font=\bfseries\small, text=TextDark] at (0, 4.9) {Node 1 (8-GPU Server Chassis)};
    \node[anchor=east, font=\scriptsize\bfseries, text=BlueLine] at (6.8, 4.9) {NVLink 4.0 Mesh};
...
@Fig-hierarchical-allreduce illustrates how this three-phase decomposition confines expensive inter-node traffic to Phase 2, where each GPU transmits only $M/G$ bytes instead of $M$. With $G = 8$ GPUs per node (a standard DGX configuration), this reduces inter-node optical fabric traffic by an exact factor of $8\times$, effectively multiplying scarce InfiniBand bandwidth by the number of accelerators per chassis.
...
**Systems insight**: Hierarchical AllReduce achieves a `{python} HierarchicalAllreduceCalc.speedup_mult_str` speedup by reducing the inter-node payload from 1 GB to `{python} HierarchicalAllreduceCalc.inter_data_mb_str` per GPU before the Ring traffic multiplier. With 8 GPUs per node, the hierarchy effectively delivers 8$\times$ the apparent inter-node bandwidth. This is why NVIDIA's NCCL and similar libraries often select hierarchical algorithms on multi-node clusters when the topology supports them.
...
@Fig-sharp-innetwork contrasts software-based tree AllReduce with in-switch hardware aggregation. In standard software tree AllReduce (panel a), worker GPUs stream complete gradient tensors up the fabric hierarchy to an assigned root GPU, concentrating arithmetic and PCIe bandwidth bottlenecks at that single endpoint before broadcasting results back down. In contrast, in-network reduction architectures such as NVIDIA's Scalable Hierarchical Aggregation and Reduction Protocol (SHARP) (panel b) offload associative reduction directly to switch application-specific integrated circuits (ASICs)\index{ASIC}\index{Application-Specific Integrated Circuit|see{ASIC}} at line rate, ensuring that only aggregated partial sums ascend to higher fabric tiers and completely eliminating the root GPU serialization penalty.

::: {#fig-sharp-innetwork fig-env="figure" fig-pos="htb" fig-cap="**In-network reduction with SHARP vs. standard tree AllReduce**: Panel (a) illustrates standard tree AllReduce, where full gradient tensors from all worker GPUs traverse intermediate switches unaggregated, converging on a single root GPU that performs store-and-forward arithmetic over saturated PCIe links. Panel (b) illustrates NVIDIA SHARP in-network computing, where switch ASICs aggregate partial sums in flight at wire speed, transmitting only reduced payloads upward and eliminating intermediate host-memory round trips." fig-alt="Left/right side-by-side comparison split by a vertical divider. Left side: Software Tree AllReduce with data flowing through GPUs. Right side: In-Network SHARP with data aggregated inside the switch."}
...
\node[banner node, draw=RedLine!80, fill=RedFill] (bannerA) at (3.65, 7.8) {
  ...
  All $N$ workers stream full tensors to root\\
  Root bandwidth saturation $= N \times \text{Bandwidth}$
};
...
\node[subhdr node] (subB) at (10.95, 8.85) {Switch ASICs reduce inline; only aggregates traverse upwards};

\node[banner node, draw=DarkGreen!80, fill=GreenFill] (bannerB) at (10.95, 7.8) {
  \textbf{\textcolor{DarkGreen}{Optimization: Line-Rate In-Switch Reduction}}\\[1.5pt]
  ASICs aggregate payloads inline at wire speed\\
  $2\text{--}3\times$ AllReduce speedup on Quantum fabrics
};
...
@Fig-torus-reduction illustrates dimension-ordered reduction across a 2D torus mesh (a pedagogical projection of 3D Tensor Processing Unit (TPU)\index{TPU}\index{Tensor Processing Unit|see{TPU}} pods). Rather than treating the cluster as an unconstrained all-to-all fabric, the collective decomposes along coordinate axes: Phase 1 executes independent Ring AllReduces along each horizontal row in parallel, followed by Phase 2 executing independent Ring AllReduces along each vertical column. This axis-aligned schedule routes traffic exclusively across direct physical neighbor links, avoiding bisection congestion while reducing step latency from $2(N-1) = 30$ steps on a flat 16-node ring down to $2(X-1) + 2(Y-1) = 12$ steps ($2.5\times$ latency reduction).

::: {#fig-torus-reduction fig-env="figure" fig-pos="htb" fig-cap="**Dimension-ordered reduction on a torus topology**: In a 2D torus mesh ($4 \times 4$ node grid representing a simplified TPU pod slice), AllReduce proceeds sequentially along orthogonal dimensions. Phase 1 (orange) executes four parallel horizontal rings across the X dimension ($2(X-1) = 6$ steps). Phase 2 (violet) then executes four parallel vertical rings across the Y dimension ($2(Y-1) = 6$ steps) on the row-aggregated partial sums. Dashed links indicate torus wrap-around boundary connections. This dimension-ordered decomposition cuts step latency by $2.5\times$ relative to a 1D global ring while preserving the optimal $2M/\beta$ bandwidth volume invariant." fig-alt="2D grid diagram of 16 nodes in a 4x4 torus topology. Orange arrows show Phase 1 horizontal row rings, violet arrows show Phase 2 vertical column rings, and dashed lines trace wrap-around boundary links. Cards on the right detail step complexity and bandwidth invariance."}
...
% Right Side Cards
% Card 1: Phase 1
\node[card, draw=OrangeLine!80, fill=OrangeL] (card1) at (11.0, 5.85) {
  \textbf{\large\textcolor{OrangeLine}{Phase 1: X-Dimension Rings}}\\[2pt]
  \small Ring AllReduce along each row in parallel.\\[3pt]
  \footnotesize\textbf{Topology:} 4 rings parallel \hfill \textbf{Steps:} $2(X - 1) = 6$
};

% Card 2: Phase 2
\node[card, draw=VioletLine!80, fill=VioletL] (card2) at (11.0, 4.25) {
  \textbf{\large\textcolor{VioletLine}{Phase 2: Y-Dimension Rings}}\\[2pt]
  \small Ring AllReduce along each column in parallel.\\[3pt]
  \footnotesize\textbf{Topology:} 4 rings parallel \hfill \textbf{Steps:} $2(Y - 1) = 6$
};

% Card 3: Latency & Step Complexity
\node[card, draw=CardBorder, fill=CardFill] (card3) at (11.0, 2.45) {
  \textbf{\normalsize Latency \& Step Complexity}\\[3pt]
  \footnotesize
  \begin{tabular}{@{}ll@{}}
    1D Global Ring ($N = 16$): & \textbf{\textcolor{RedLine}{30 steps}} \\[2pt]
    2D Torus $4\times 4$: $2(X-1) + 2(Y-1)$: & \textbf{\textcolor{DarkGreen}{12 steps}} \\
  \end{tabular}\\[4pt]
  \centering
...
\node[card, draw=CardBorder, fill=CardFill] (card4) at (11.0, 0.65) {
...
The most common method is **Top-k Compression**\index{Top-k Sparsification}: for a gradient vector $g \in \mathbb{R}^{d_{\text{grad}}}$, transmit only the $K_{\text{top}}$ elements with the largest absolute magnitude, setting the rest to zero [@aji2017; @lin2018deep]:
...
The error-feedback trace in @tbl-error-feedback runs the same sequence with a residual accumulator $e_t$ that re-injects the discarded remainder into the next step. The trace in example \ref{exmp-collective-communication-error-feedback-mechanism} works through both traces to show that the deferred information is conserved rather than lost.
...
The 1-bit Adam paper reports up to 5$\times$ communication-volume reduction while matching the convergence speed of uncompressed Adam, with experiments up to 256 GPUs showing up to 3.3$\times$ higher throughput for BERT-Large pretraining and up to 2.9$\times$ higher throughput for SQuAD fine-tuning [@tang20211bit]. The practical lesson is narrower than "compression always wins": the compression overhead must remain small compared with the communication time saved.
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ COMMUNICATION INFRASTRUCTURE & INTRA-NODE ALLREDUCE RECAP (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: §Communication-Computation Overlap summary and Fallacy 8
# │
# │ Goal: Compute single-node 70B Ring AllReduce napkin math and recap
# │       NVLink vs InfiniBand NDR bandwidth hierarchy speedup.
# │ Show: intra_ring_time_s_str ≈ 0.54 s, intra_ring_pct_str ≈ 26 percent,
# │       speedup_mult_str ≈ 4.8x.
# │ How: T_intra = 2*(7/8)*140 GB / 450 GB/s; compare flat vs. 3-step hierarchical.
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Models, Systems
from mlsysim.core.units import GB, MB, byte, microsecond, ms, param, second
from mlsysim.fmt import check, fmt_multiple, fmt_percent, fmt_qty, fmt_time, fmt_math
from mlsysim.physics import (
    calc_ring_allreduce_latency_time,
    calc_ring_allreduce_time,
    calc_ring_collective_data_factor,
...
class CommInfraRecap:
    # 1. Single-node 70B Ring AllReduce napkin math
    gradient_70b = (Models.Language.Llama2_70B.parameters * (2 * byte / param)).to(GB)
    nvlink_bw = Hardware.Cloud.H100.nvlink.bandwidth
    nvlink_bw_beta = Hardware.Cloud.H100.nvlink.bandwidth_per_direction
    intra_ring_time = (2 * (7 / 8) * gradient_70b / nvlink_bw_beta).to(second)
    compute_budget = 2.1 * second
    intra_ring_pct = (intra_ring_time / compute_budget).to_base_units().magnitude

    # 2. Multi-node hierarchy recap
    n_nodes_har = 8
    gpus_per_node = Systems.Nodes.DGX_H100.accelerators_per_node
    gradient = 1 * GB
    ...
    check(abs(intra_ring_time.to(second).magnitude - 0.54) < 0.02, "Intra-node ring time should be ~0.54 s")
    ...
    nvlink_bw_beta_gb_per_s_str = fmt_qty(nvlink_bw_beta, GB / second, precision=0, commas=False)
    ib_ndr_bw_gb_per_s_str = fmt_qty(ib_bw, GB / second, precision=0, commas=False)
    speedup_mult_str = fmt_multiple(speedup, precision=1, commas=False)

    _g70b_gb = int(gradient_70b.to(GB).magnitude)
    _nv_beta = int(nvlink_bw_beta.to(GB / second).magnitude)
    intra_ring_operands_math = fmt_math(rf"2(7/8) \times {_g70b_gb}\ \text{{GB}}/({_nv_beta}\ \text{{GB/s}})")
    intra_ring_time_s_str = fmt_time(intra_ring_time, second, precision=2, commas=False)
    compute_budget_s_str = fmt_time(compute_budget, second, precision=1, commas=False)
    intra_ring_pct_str = fmt_percent(round(intra_ring_pct * 100) / 100, precision=0, commas=False, style='prose')
```

Even the intra-node base case shows this stack succeeding before any of the multi-node machinery is needed: a 70B-parameter model on a single node of 8 NVLink-connected GPUs must AllReduce `{python} GradSyncSignificance.gradient_gb_str` of BF16 gradients per step. Using NVLink's `{python} CommInfraRecap.nvlink_bw_beta_gb_per_s_str` one-way rate in the $\alpha$-$\beta$ model and Ring's $2(7/8)$ traffic factor gives `{python} CommInfraRecap.intra_ring_operands_math` $\approx$ `{python} CommInfraRecap.intra_ring_time_s_str`, about `{python} CommInfraRecap.intra_ring_pct_str` of a `{python} CommInfraRecap.compute_budget_s_str` per-step compute budget. That quantitative success sets up the fallacies and pitfalls in @sec-communication-collective-operations-collective-operations-fallacies-pitfalls-9cd0, because each one begins by optimizing the wrong term, choosing the wrong primitive, or trusting a topology assumption that no longer matches the workload.
...
When even the fastest wires are not enough, gradient compression provides another lever. Quantization, sparsification, and compression-aware optimizers like 1-bit Adam reduce communication volume by 4--1000$\times$. Error feedback preserves residual information across steps under the assumptions developed in @sec-communication-collective-operations-error-feedback. Communication-computation overlap through layer-by-layer pipelining and bucket fusion can then hide remaining communication behind useful computation, reducing effective overhead to 5--15 percent of total step time in well-configured systems.
...
The "Travel Manifest" for a gradient depends on the system's objective function and constraint regime. Each lighthouse archetype faces a distinct communication challenge, and collective communication techniques map to those challenges differently, as @tbl-archetype-collective-mapping summarizes.
...
The archetypes face different communication bottlenecks even though they all train neural networks. Archetype A (GPT-4/Llama-3) commonly carries large AllReduce payloads and benefits from hierarchical reduction combined with communication-computation overlap. Archetype B (DLRM at Scale) generates destination-specific AllToAll traffic whose limiter can shift among startup, injection/reception, bisection, oversubscription, routing, and shard-access skew; message coalescing, topology-aware routing, and shard balancing address different terms. Archetype C (Federated MobileNet) is constrained by intermittent connectivity and small devices, so aggressive quantization and error feedback matter more than data-center topology. Applying the wrong optimization to the wrong archetype wastes engineering effort without improving performance.
...
* **The $\alpha$-$\beta$ model reveals the bottleneck**: The critical message size $n^* = \alpha \cdot \beta$ determines whether to optimize software latency (small messages) or hardware bandwidth (large payloads) (principle \ref{pri-alpha-beta-model}). Fit effective parameters with a benchmark that matches the workload's collective and topology.
...
* **Overlap is the final multiplier**: Communication-computation pipelining through gradient hooks and bucket fusion can hide 90--95 percent of communication time behind backward pass computation for deep models (principle \ref{pri-distributed-step-time}).
* **Topology discovery is not optional**: High-performance libraries like NCCL dynamically map logical rings to physical wires to avoid "hot spots" and maximize bisection bandwidth (principle \ref{pri-bisection-bandwidth}). Misaligned rank-to-GPU mapping can degrade performance by 2--4$\times$.
...
The fleet now has its logic (parallelism) and its traffic patterns (communication). In production, however, GPUs overheat, networks drop packets, and nodes fail mid-calculation. @Sec-fault-tolerance-reliability examines how checkpointing, recovery, and elastic training maintain a coherent computation on imperfect hardware.

:::

[^fn-allreduce-etymology-primitive]: **AllReduce**\index{AllReduce!etymology}: A compound term from MPI indicating a Reduce operation (summing data from all nodes to one) followed by a Broadcast (sending the sum to all nodes). Ring-based AllReduce implements this as a sequential ReduceScatter phase followed by an AllGather phase, each taking $N-1$ rounds for $2(N-1)$ rounds total; ranks send and receive concurrently within each round, and every GPU ends with the global sum without a centralized bottleneck.

[^fn-zerocopy-rdma-bypass]: **Zero-Copy Communication**\index{Zero-Copy Communication!RDMA}: RDMA allows the NIC to transfer data directly from the application's memory on one node to the application's memory on another, bypassing the operating system's kernel buffers. For a `{python} GradSyncSignificance.gradient_gb_str` gradient exchange, zero-copy avoids moving `{python} GradSyncSignificance.gradient_2x_gb_str` of data between the CPU and main memory, reclaiming significant memory bandwidth ($\text{BW}$).

[^fn-speed-of-light]: **The Speed of Light Constraint**\index{Speed of Light!latency bound}: Light travels through optical fiber at approximately 200,000 km/s, or 200 meters per microsecond. In a massive data center where cables between racks span 100 meters, the "wire delay" alone contributes 500 ns to every message, a physical limit that no amount of better networking hardware can reduce.
