...
The contrast in @fig-sharp-innetwork shows the next step: move the reduction work from endpoint GPUs into the switch application-specific integrated circuit (ASIC)\index{ASIC}\index{Application-Specific Integrated Circuit|see{ASIC}} so packets are aggregated while they traverse the fabric.

::: {#fig-sharp-innetwork fig-env="figure" fig-pos="htb" fig-cap="**In-network reduction (SHARP)**: The left panel shows a traditional tree AllReduce that performs reduction at intermediate GPUs and pays repeated store-and-forward delays. The right panel moves those intermediate reductions into switch ASICs, aggregating partial sums as packets traverse the tree. This design avoids intermediate endpoint memory traffic and can reduce collective latency when aggregation cost is material." fig-alt="Left/right side-by-side comparison split by a vertical divider. Left side: Software Tree AllReduce with data flowing through GPUs. Right side: In-Network SHARP with data aggregated inside the switch."}
