...
Tracking these components helps platform teams distinguish raw hardware utilization from true goodput—the rate at which valid, useful training progress actually occurs. This distinction is vital: a cluster pinned at 98% GPU utilization can still exhibit abysmal MPG if frequent worker failures trigger cascading rollbacks or un-fused kernels spend cycles waiting on memory buses.
