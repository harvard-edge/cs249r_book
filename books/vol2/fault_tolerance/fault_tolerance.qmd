...
::: {#fig-cluster-scaling-trend fig-env="figure" fig-pos="htb" fig-cap="**The Rise of Fleet-Scale Training**: Over the past decade, the number of accelerators used in state-of-the-art model training has grown exponentially. As cluster sizes scale from a few GPUs to tens of thousands, fault tolerance transitions from a convenience to a strict requirement, as system MTBF proportionally collapses."}
...
df = pd.read_csv("data/cluster_size_trend.csv")
...
- **Transient faults**\index{Transient Fault}: Temporary disruptions caused by external factors such as cosmic rays or electromagnetic interference [@ziegler1996]. Their danger is silent corruption: a rank may keep participating while sending a wrong gradient or activation.
- **Permanent faults**\index{Permanent Fault}: Irreversible damage from physical defects or component wear-out, such as stuck-at faults or device failures that require hardware replacement. Their danger is repeatability: retrying the same device reproduces the same bad computation or hard failure.
- **Intermittent faults**\index{Intermittent Fault}: Sporadic disruptions that appear and disappear due to unstable conditions like loose connections, aging components, or thermal stress. Their danger is ambiguity: the job may pass validation during one run and fail under a slightly different load.
...
These choices define the *fault model*\index{Fault Model}: how a physical defect manifests in the hardware. The corresponding *error model*\index{Error Model} represents how that fault propagates and alters observable system state.
...
The Young-Daly formula stated in @sec-fault-tolerance-young-daly gives the optimal checkpoint interval, $\tau_{\text{opt}} = \sqrt{2 \times T_{\text{write}} \times \text{MTBF}_{\text{system}}}$, but it cannot be applied until both inputs are known. The failure analysis in @sec-fault-tolerance-reliability-reliability-failure-analysis-scale-6b4b supplies the system MTBF term, and the preceding checkpoint sizing supplies $T_{\text{write}}$. With both in hand, the formula stops being an abstract law and becomes a concrete schedule for the chapter's own cluster.
...
- **Bounded asynchronous**\index{Checkpointing!bounded asynchronous}: Workers may be within $k$ steps of each other (typically $1 \le k \le 3$). The checkpoint manager tracks the checkpoint wavefront across workers. Recovery uses the earliest consistent cut across all shards, trading strict consistency for dramatically reduced synchronization overhead.
- **Eventual consistency**\index{Eventual Consistency}: Workers checkpoint when convenient and reconcile during recovery. Provides lowest overhead but requires complex recovery logic to reconstruct consistent state.
...
Modern distributed training frameworks partition model state across workers using techniques such as ZeRO and FSDP. In these configurations, no single worker holds complete model state. Each worker holds only its assigned parameter shard plus corresponding optimizer state.

Sharded checkpointing\index{Sharded Checkpointing}[^fn-sharded-checkpoint-state] [@rajbhandari2020] addresses this distribution directly: each worker writes only its locally held shard, dramatically reducing per-worker write volume. Recovery loads shards and redistributes state to workers based on the recovery configuration.
...
At the algorithmic layer, loss spike\index{Loss Spike} detection and training dynamics monitoring catch the most insidious failure mode: silent data corruption. Hardware errors that do not crash the process but corrupt mathematical results (bit flips in arithmetic logic units, for instance) manifest as sudden, catastrophic anomalies where the loss jumps 10–100$\times$ or collapses to NaN without any hyperparameter changes. Monitoring loss curves, gradient norms, and per-rank gradient distributions allows the training loop to pause immediately upon detecting such divergence, pinpoint the rank with the corrupted gradient via checksums or replay, and drain that node before resuming from the last healthy checkpoint.
...
The stateless pattern appears when the input itself carries all context: an image classifier processes one image, an object detector processes one frame, a single-turn text classifier processes one snippet, and an embedding service maps one input to one vector. Fault tolerance can then focus on replica health and request routing. Redundant replicas\index{Redundant Replicas} serve requests in parallel, load balancing\index{Load Balancing} sends traffic only to healthy replicas, and health checks\index{Health Check} remove failed replicas from rotation, while automatic replacement starts a new replica when capacity falls. When a replica fails, in-flight requests to that replica can be retried elsewhere, because no quality-bearing session state has been lost.
...
During training, gradient statistics make numerical faults visible before they corrupt the whole run. Tracking gradient norms reveals exploding or vanishing gradients before weights diverge, inspecting per-layer gradient distributions exposes localized saturation or dead activations, and monitoring optimizer loss scales catches FP16 dynamic-range underflows before they force repeated discarded steps.
...
Comparing component timing across replicas isolates tail effects: while p50 reflects typical throughput, p99 surfaces the outlier delays that drag down collective barriers. Comparing p99 latencies across replicas identifies workers whose slow path has become a fleet-wide bottleneck.

Attributing a straggler requires isolating which physical subsystem drives that tail latency. Thermal throttling and corrected memory errors degrade per-core execution throughput; input data skew imposes disproportionate token counts or sparse-feature lookups on specific ranks; background resource contention from OS daemons or sidecars steals CPU cycles; and degraded network optics or switch-port congestion create transmission stalls on collective barriers. Isolating the resource signature determines whether the correct remediation is migrating the rank, load-balancing input batches, or tuning kernel preemption.
...
Published production systems make the chapter's scale lesson concrete: failure detection, state preservation, and observability reappear as checkpoint tax, topology reconfiguration, chaos testing, and elastic recovery. Public reports from Meta, Google, Netflix, and Microsoft provide representative engineering patterns. Treat the operational numbers as reported magnitudes that reveal the design pressure, not as universal constants. Meta's Open Pre-trained Transformer (OPT-175B) training run on a 992-GPU cluster provides the first example: dozens of hardware failures had to be absorbed without aborting training.
...
- **Checkpoint loading**: Reading distributed checkpoints from storage.
- **Warmup time**\index{Warmup!time}: Recalculating learning-rate schedules and batch-normalization statistics.
- **Communication re-establishment**: Reconstructing NCCL communication rings and socket connections.
...
- **In-flight gradients**\index{Checkpoint Consistency!in-flight gradients}: An AllReduce in progress during a checkpoint may or may not be included.
...
Fault tolerance mechanisms are code paths that execute rarely in normal operation. As with backup systems never tested until disaster strikes, fault tolerance code paths accumulate bugs:
