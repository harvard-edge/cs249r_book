...
In a distributed setting, experts are partitioned across workers. If we have 8 GPUs and 8 experts, each GPU hosts one expert. The training process routes tokens through four sequential stages (@fig-moe-all-to-all-routing):
...
Across these abstraction layers, distributed training ultimately reduces to a small set of collective communication primitives. Where @sec-collective-communication analyzes the network mechanics of routing data across physical topologies, framework abstractions focus on *when* and *why* these primitives are invoked during execution.
