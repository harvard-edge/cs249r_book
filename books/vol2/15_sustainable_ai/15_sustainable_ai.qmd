...
\noindent
![](images/png/cover_sustainable_ai_blueprint_labeled_print.png){fig-alt="Sustainable fleet cutaway connecting compute racks to power delivery, cooling loops, grid sources, carbon accounting, efficiency controls, and lifecycle management."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_sustainable_ai_blueprint_labeled.webp){fig-alt="Sustainable fleet cutaway connecting compute racks to power delivery, cooling loops, grid sources, carbon accounting, efficiency controls, and lifecycle management."}
:::
...
Power is an operational expense and a physical constraint on what can be built. A data center has a fixed budget determined by electrical infrastructure and cooling capacity; a training run that exceeds it cannot proceed regardless of its financial budget. Grid capacity and cooling feasibility also constrain deployment location. At the largest scales, physical feasibility depends on energy efficiency as much as algorithmic capability. Sustainability therefore belongs alongside accuracy and latency in system design. In C³ terms, no combination of compute, communication, or coordination can exceed the facility's power capacity.
...
from mlsysim.fmt import fmt, fmt_params, fmt_math, fmt_qty, sci_latex, check
...
class EnergyOfIntelligenceCheckpoint:
    ...
    efficiency_gflop_per_j_str = fmt_qty(efficiency, unit=GFLOP / J, precision=0, commas=False)
```

::: {#chk-sustainable-ai-energy-intelligence .callout-checkpoint title="The energy of intelligence"}

A `{python} EnergyOfIntelligenceCheckpoint.model_params_b_str` parameter model requires approximately `{python} EnergyOfIntelligenceCheckpoint.train_flops_math` FLOPs to train. Assuming a data center PUE of `{python} EnergyOfIntelligenceCheckpoint.pue_str` and an end-to-end realized training efficiency of `{python} EnergyOfIntelligenceCheckpoint.efficiency_gflop_per_j_str`:
...
**Systems insight**: A single training run emits as much carbon as hundreds of trans-Atlantic passenger round trips. Optimization matters. Under this operational-electricity boundary, moving the job to a hydro-heavy grid (`{python} CarbonCostGPT3.grid_ci_low_kg_str`) would reduce emissions by `{python} CarbonCostGPT3.emissions_ratio_mult_str` to about `{python} CarbonCostGPT3.low_carbon_flights_str` passenger round trips. The comparison does not make hydroelectricity lifecycle-zero or remove local water and ecosystem impacts.
...
**Systems lesson**: Carbon-aware placement internalizes environmental externalities into the fleet scheduling objective. The optimizer proves that high-renewables grids minimize true total cost of ownership (TCO) when carbon penalties are factored into ML workload placement.

:::

The scheduling optimizer treats carbon intensity as a time-varying input, shifting workloads to low-carbon hours within a single region. Geographic placement extends the same logic across space: because national grids differ by an order of magnitude in carbon intensity, choosing where to run a job can dwarf any gain from choosing when to run it. The calculation in notebook \ref{nbk-sustainable-ai-geography-carbon} isolates this geographic factor by holding energy demand constant and varying only the grid.

::: {.column-margin}
![](images/svg/vol2_sustainable_ai_margin_001.svg){width="100%" fig-alt="Carbon-emissions ladder for the same training job: Poland is about 8,200 tonnes CO2, Quebec about 200 tonnes, with the 41 times difference shown as an annotation rather than a third bar."}
...
[^fn-ewaste-hardware-disposal]: **AI Hardware E-waste**\index{AI Hardware E-waste!definition}: Global e-waste reached 53.6 million metric tons in 2019, with computing equipment contributing 15 percent. AI accelerators compound this: 3-5 year obsolescence cycles driven by rapidly advancing architectures mean that a fleet of 10,000 GPUs generates 10–20 metric tons of toxic e-waste per refresh cycle, containing lead, mercury, and cadmium requiring specialized disposal.
...
Rapid growth in computational demands challenges the long-term sustainability of AI training and deployment. In the 2012--2019 window studied here, reported AI training compute increased `{python} ComputeGrowthScenario.compute_growth_factor_mult_str`[^fn-compute-growth-scaling] [@schwartz2020]. Many large-model training regimes since then have continued to favor larger models, larger training datasets, or higher computational budgets. Sustaining that trajectory poses sustainability challenges when hardware efficiency gains fail to keep pace with workload demand. Efficiency per operation has improved; total demand has improved faster. @Fig-ai-compute-growth makes the mismatch visible, and it is the reason a per-FLOP efficiency gain does not translate into a lower energy bill for the run.

::: {#fig-ai-compute-growth fig-env="figure" fig-pos="htb" fig-cap="**Historical AI compute growth**: Training compute for landmark models against release date, on a log axis spanning about eight orders of magnitude. The slope sits well above the doubling rate hardware efficiency has delivered over the same window, which is why total energy per training run has risen even as energy per FLOP has fallen." fig-alt="Log-scale scatter of total training compute in FLOPs against release date for landmark models from AlexNet in 2012 to Llama-3 in 2024, spanning about eight orders of magnitude along a near-straight line."}
...
# Load data
df = pd.read_csv("vol2/15_sustainable_ai/data/ai_compute_trend.csv")
df['Date'] = pd.to_datetime(df['Year'])

# Set style
fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 5))

# Plot landmark models
ax.scatter(df['Date'], df['Compute_FLOPs'], color="darkorange", s=120, alpha=0.7, edgecolor="black")

ax.set_yscale("log")

# Annotate key models
key_models = ["AlexNet", "ResNet-152", "Transformer", "GPT-3", "GPT-4", "Llama-3 (70B)"]
for i, row in df.iterrows():
    if row['Model'] in key_models:
        ax.annotate(row['Model'], (row['Date'], row['Compute_FLOPs']), xytext=(10, 5), textcoords='offset points', fontsize=9)

# Format axes
ax.set_title("Training compute for landmark ML models (2012-2024)", fontsize=11, fontweight="bold", pad=10)
ax.set_ylabel("Total Training Compute (FLOPs)", fontsize=10)
...
[^fn-compute-growth-scaling]: **AI Compute Growth Rate**\index{AI Compute Growth Rate!definition}\index{AI Compute Growth!doubling time}: The `{python} ComputeGrowthScenario.compute_growth_factor_mult_str` increase from 2012 to 2019 implies a doubling time of approximately `{python} ComputeGrowthScenario.compute_doubling_months_str`, roughly `{python} ComputeGrowthScenario.moore_speedup_mult_str` faster than Moore's Law's 2-year doubling. This divergence is one major driver of the energy wall: no physically realizable improvement in silicon efficiency can match a doubling cadence measured in months by process scaling alone, making algorithmic efficiency and carbon-aware scheduling central sustainability levers at scale.
...
\providecolor{darknavy}{RGB}{20,40,80}

\begin{tikzpicture}[
    font=\small,
    >=Stealth,
    gridline/.style={draw=gray!20, line width=0.6pt},
    axisline/.style={draw=gray!50, line width=0.9pt}
]

% Coordinate system for plot
% X: 2010 to 2030 -> width 13cm. 2010 at x=0, 2030 at x=13.0. (dx = 0.65cm per year)
% Y: 0 to 8000 TWh -> height 7.5cm. (dy = 0.9375cm per 1000 TWh)

\def\xyear#1{(#1 - 2010) * 0.65}
\def\ytwh#1{#1 * 0.0009375}

% Background grid
\foreach \y/\lbl in {0/0, 2000/{2,000}, 4000/{4,000}, 6000/{6,000}, 8000/{8,000}} {
    \draw[gridline] (0, {\ytwh{\y}}) -- (13.2, {\ytwh{\y}});
    \node[anchor=east, font=\scriptsize, text=gray!70!black] at (-0.15, {\ytwh{\y}}) {\lbl};
}

% X-axis ticks
\foreach \yr in {2010, 2012, 2014, 2016, 2018, 2020, 2022, 2024, 2026, 2028, 2030} {
    \node[anchor=north, font=\scriptsize, text=gray!70!black] at ({\xyear{\yr}}, -0.15) {\yr};
}

% Axes
\draw[axisline] (0, 0) -- (0, {\ytwh{8300}});
\draw[axisline] (0, 0) -- (13.3, 0);

% Axis titles
\node[rotate=90, anchor=south, font=\bfseries\footnotesize, text=darknavy] at (-1.3, {\ytwh{4000}}) {Global Data Center Electricity (TWh / yr)};
\node[anchor=north, font=\bfseries\footnotesize, text=darknavy] at ({\xyear{2020}}, -0.8) {Projection Timeline (Calendar Years)};

% Total US Electricity reference line
\draw[dashed, line width=1.1pt, gray!60] (0, {\ytwh{4100}}) -- (13.0, {\ytwh{4100}});
\node[draw=gray!40, fill=white, rounded corners=3pt, inner sep=3.5pt, font=\scriptsize\bfseries, text=gray!80!black]
    at ({\xyear{2013.5}}, {\ytwh{4100}}) {Total US Electricity ($\sim$4,100 TWh)};

% Historical / Model Split vertical dashed line
\draw[dashed, line width=1.0pt, gray!50] ({\xyear{2018}}, 0) -- ({\xyear{2018}}, {\ytwh{8200}});
\node[draw=gray!35, fill=gray!5!white, rounded corners=3pt, inner sep=3.5pt, font=\tiny\bfseries, text=gray!75!black]
    at ({\xyear{2018}}, {\ytwh{8400}}) {Historical / Model Split};

% Historical line (2010 to 2018): flat around 500 TWh
\draw[line width=2.0pt, darknavy!80]
    ({\xyear{2010}}, {\ytwh{500}}) -- ({\xyear{2018}}, {\ytwh{550}});

% Area shading between Worst (red) and Best (green) projections
\fill[color4!8!white]
    ({\xyear{2018}}, {\ytwh{1250}}) --
    ({\xyear{2020}}, {\ytwh{1600}}) --
    ({\xyear{2022}}, {\ytwh{2200}}) --
    ({\xyear{2024}}, {\ytwh{3000}}) --
    ({\xyear{2026}}, {\ytwh{4500}}) --
    ({\xyear{2028}}, {\ytwh{6000}}) --
    ({\xyear{2030}}, {\ytwh{7933}}) --
    ({\xyear{2030}}, {\ytwh{700}}) --
    ({\xyear{2028}}, {\ytwh{680}}) --
    ({\xyear{2026}}, {\ytwh{660}}) --
    ({\xyear{2024}}, {\ytwh{630}}) --
    ({\xyear{2022}}, {\ytwh{600}}) --
    ({\xyear{2020}}, {\ytwh{580}}) --
    ({\xyear{2018}}, {\ytwh{550}}) -- cycle;

% 1. Worst Case curve (Red)
\draw[line width=2.2pt, color4]
    ({\xyear{2018}}, {\ytwh{1250}}) --
    ({\xyear{2020}}, {\ytwh{1600}}) --
    ({\xyear{2022}}, {\ytwh{2200}}) --
    ({\xyear{2024}}, {\ytwh{3000}}) --
...
\foreach \yr/\val in {2018/1250, 2020/1600, 2022/2200, 2024/3000, 2026/4500, 2028/6000, 2030/7933} {
    \fill[color4] ({\xyear{\yr}}, {\ytwh{\val}}) circle (2.8pt);
}
\node[anchor=south east, font=\bfseries\scriptsize, text=color4!90!black]
    at ({\xyear{2030}}, {\ytwh{8050}}) {Worst: 7,933 TWh};

% 2. Expected Case curve (Blue)
\draw[line width=2.2pt, color1!80!black]
    ({\xyear{2018}}, {\ytwh{780}}) --
    ({\xyear{2020}}, {\ytwh{880}}) --
    ({\xyear{2022}}, {\ytwh{1020}}) --
    ({\xyear{2024}}, {\ytwh{1200}}) --
    ({\xyear{2026}}, {\ytwh{1600}}) --
    ({\xyear{2028}}, {\ytwh{2000}}) --
    ({\xyear{2030}}, {\ytwh{2967}});

\foreach \yr/\val in {2018/780, 2020/880, 2022/1020, 2024/1200, 2026/1600, 2028/2000, 2030/2967} {
    \fill[color1!80!black] ({\xyear{\yr}}, {\ytwh{\val}}) circle (2.8pt);
}
\node[anchor=south east, font=\bfseries\scriptsize, text=color1!80!black]
    at ({\xyear{2030}}, {\ytwh{3100}}) {Expected: 2,967 TWh};

% 3. Best Case curve (Green)
\draw[line width=2.2pt, color2!80!black]
    ({\xyear{2018}}, {\ytwh{550}}) --
    ({\xyear{2020}}, {\ytwh{580}}) --
    ({\xyear{2022}}, {\ytwh{600}}) --
    ({\xyear{2024}}, {\ytwh{630}}) --
...
\foreach \yr/\val in {2018/550, 2020/580, 2022/600, 2024/630, 2026/660, 2028/680, 2030/700} {
    \fill[color2!80!black] ({\xyear{\yr}}, {\ytwh{\val}}) circle (2.8pt);
}
\node[anchor=north east, font=\bfseries\scriptsize, text=color2!80!black]
    at ({\xyear{2030}}, {\ytwh{600}}) {Best: 700 TWh};


% ============================================================
% RIGHT SIDE: PROJECTION SENSITIVITY ENVELOPE (2030)
% ============================================================
\coordinate (env_origin) at (14.2, {\ytwh{8300}});

\node[anchor=north west, draw=gray!30, fill=gray!3!white, rounded corners=6pt, line width=0.8pt, inner sep=8pt, text width=6.8cm] (env_card) at (env_origin) {
    \textbf{\footnotesize\textcolor{darknavy}{PROJECTION SENSITIVITY ENVELOPE (2030)}}\\[6pt]

    % Worst Case Card
    \tikz[baseline=-0.5ex]{\node[draw=color4!60, fill=white, rounded corners=4pt, inner sep=5pt, text width=6.4cm] {
        \tikz[baseline=-0.5ex]{\node[fill=color4!15!white, text=color4!90!black, font=\bfseries\tiny, rounded corners=2pt, inner sep=2.5pt] {WORST CASE $\cdot$ 7,933 TWh};}\\[4pt]
        \tiny Workload: Unbounded algorithmic scaling\\[1.5pt]
        \tiny Hardware: Silicon efficiency gains stall\\[1.5pt]
        \tiny Cooling: Legacy air PUE persists $> 1.5$\\[2.5pt]
        \textbf{\tiny\textcolor{color4!90!black}{Grid Load: Exceeds $\sim$2$\times$ current US electricity}}
    };}\\[5pt]

    % Expected Case Card
    \tikz[baseline=-0.5ex]{\node[draw=color1!50, fill=white, rounded corners=4pt, inner sep=5pt, text width=6.4cm] {
        \tikz[baseline=-0.5ex]{\node[fill=color1!15!white, text=color1!85!black, font=\bfseries\tiny, rounded corners=2pt, inner sep=2.5pt] {EXPECTED CASE $\cdot$ 2,967 TWh};}\\[4pt]
        \tiny Workload: Steady industrial adoption\\[1.5pt]
        \tiny Hardware: Continued $\sim$1.5$\times$ annual silicon gains\\[1.5pt]
        \tiny Cooling: Gradual shift to liquid cooling\\[2.5pt]
        \textbf{\tiny\textcolor{color1!85!black}{Grid Load: Consumes $\sim$72\% of US grid output}}
    };}\\[5pt]

    % Best Case Card
    \tikz[baseline=-0.5ex]{\node[draw=color2!50, fill=white, rounded corners=4pt, inner sep=5pt, text width=6.4cm] {
        \tikz[baseline=-0.5ex]{\node[fill=color2!15!white, text=color2!80!black, font=\bfseries\tiny, rounded corners=2pt, inner sep=2.5pt] {BEST CASE $\cdot$ 700 TWh};}\\[4pt]
        \tiny Workload: Radical quantization \& MoE sparsity\\[1.5pt]
        \tiny Hardware: Specialized ultra-low-power ASICs\\[1.5pt]
        \tiny Cooling: Full liquid cooling deployment (PUE $< 1.10$)\\[2.5pt]
        \textbf{\tiny\textcolor{color2!80!black}{Grid Load: Contained within planned capacity}}
    };}\\[6pt]

    \centering\tiny\textcolor{gray!70!black}{Source: Nature / IEA High-Growth Envelope Modeling}
};
...
AI compute grew ~350,000$\times$ over the 2012--2019 period (@fig-energy-wall), while battery density and grid efficiency improved at only ~2–5 percent annually.
...
\begin{tikzpicture}[
    font=\small,
    >=Stealth,
    gridline/.style={draw=gray!15, line width=0.6pt},
    axisline/.style={draw=gray!60, line width=0.9pt}
]

% Coordinate mappings:
% X: 2016 at 0, 2026 at 13.0 -> 1.3cm per year
% Y: log scale from 0.01 (10^-2) to 100,000 (10^5) -> 7 decades -> 1.0cm per decade
% y = (log10(val) + 2)

\def\xyear#1{(#1 - 2016) * 1.3}

% Background grid and Y-axis labels
\foreach \pow/\lbl in {-2/{0.01}, -1/{0.1}, 0/{1}, 1/{10}, 2/{100}, 3/{1,000}, 4/{10,000}, 5/{100,000}} {
    \pgfmathsetmacro{\ypos}{\pow + 2}
    \draw[gridline] (0, \ypos) -- (13.0, \ypos);
    \node[anchor=east, font=\scriptsize, text=gray!70!black] at (-0.15, \ypos) {\lbl};
}

% X-axis ticks
\foreach \yr in {2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026} {
    \node[anchor=north, font=\scriptsize, text=gray!70!black] at ({\xyear{\yr}}, -0.15) {\yr};
}

% Axes
\draw[axisline, ->] (0, 0) -- (0, 7.5);
\draw[axisline, ->] (0, 0) -- (13.3, 0);

% Axis titles
\node[rotate=90, anchor=south, font=\bfseries\footnotesize, text=darknavy] at (-1.3, 3.5) {Energy per Training Run (MWh)};
\node[anchor=north, font=\bfseries\footnotesize, text=darknavy] at ({\xyear{2021}}, -0.8) {Frontier Foundation Model Release Year};

% Data Coordinates:
\coordinate (p_alphago) at (0, 1.0);
\coordinate (p_trans) at ({\xyear{2017}}, 1.699);
\coordinate (p_bert) at ({\xyear{2019}}, 3.079);
\coordinate (p_gpt3) at ({\xyear{2020}}, 5.110);
\coordinate (p_palm) at ({\xyear{2022}}, 5.531);
\coordinate (p_gpt4) at ({\xyear{2023}}, 6.699);
\coordinate (p_llama) at ({\xyear{2024}}, 6.477);

% Green capacity wall (10,000 MWh = 10^4 -> y = 6.0)
\draw[dashed, line width=1.1pt, color2!80!black] (0, 6.0) -- (13.0, 6.0);

% Shaded region above capacity wall
\begin{scope}[on background layer]
    \fill[color4!10!white] (8.322, 6.0) -- (p_gpt4) -- (p_llama) -- ({\xyear{2024}}, 6.0) -- cycle;
\end{scope}

% Green badge for Small Power Plant
\node[draw=color2!80, fill=white, rounded corners=4pt, inner sep=4pt, font=\scriptsize\bfseries, text=color2!70!black]
    at (10.5, 6.0) {Small Power Plant ($\sim$10 GWh/yr)};

% Top-left callout card
\node[draw=color4!60, fill=white, rounded corners=5pt, inner sep=6pt, anchor=north west] at (0.4, 7.2) {
    \begin{minipage}{5.0cm}
        \tikz[baseline=-0.5ex]{\node[fill=color4!12!white, text=color4!90!black, font=\bfseries\tiny, rounded corners=2pt, inner sep=2.5pt] {INFRASTRUCTURE CAPACITY WALL};}\\[3pt]
        \tiny Grid interconnect \& substation limits\\[1pt]
        \tiny Single training run equals regional city draw
    \end{minipage}
};

% Trend line
\draw[dashed, line width=1.8pt, color4] (p_alphago) -- (p_trans) -- (p_bert) -- (p_gpt3) -- (p_palm) -- (p_gpt4);

% Trend line label
\node[rotate=45, font=\bfseries\tiny, text=color4!90!black, fill=white, inner sep=2pt, rounded corners=2pt]
    at ({\xyear{2019.4}}, 3.9) {Exponential Compute Growth ($\sim$3.4$\times$ / yr)};

% Model Nodes
% AlphaGo
\filldraw[draw=color1!80!black, fill=white, line width=1.3pt] (p_alphago) circle (3.0pt);
\node[anchor=west, font=\bfseries\scriptsize, text=color1!80!black] at ($(p_alphago) + (0.15, 0)$) {AlphaGo (0.1 MWh)};

% Transformer
\filldraw[draw=color1!80!black, fill=white, line width=1.3pt] (p_trans) circle (3.0pt);
\node[anchor=north west, font=\bfseries\scriptsize, text=color1!80!black] at ($(p_trans) + (0.15, -0.05)$) {Transformer ($\sim$0.5 MWh)};

% BERT
\filldraw[draw=color1!80!black, fill=white, line width=1.3pt] (p_bert) circle (3.0pt);
\node[anchor=south east, font=\bfseries\scriptsize, text=color1!80!black] at ($(p_bert) + (-0.1, 0.1)$) {BERT ($\sim$12 MWh)};

% GPT-3
\filldraw[draw=color4, fill=white, line width=1.5pt] (p_gpt3) circle (3.5pt);
\node[anchor=south east, font=\bfseries\scriptsize, text=color4!90!black] at ($(p_gpt3) + (-0.1, 0.15)$) {GPT-3 (1,287 MWh)};

% PaLM
\filldraw[draw=color3!90!black, fill=white, line width=1.5pt] (p_palm) circle (3.5pt);
\node[anchor=south east, font=\bfseries\scriptsize, text=color3!90!black] at ($(p_palm) + (-0.15, 0.0)$) {PaLM ($\sim$3,400 MWh)};

% GPT-4
\filldraw[draw=color4, fill=white, line width=1.6pt] (p_gpt4) circle (4.0pt);
\node[anchor=south east, font=\bfseries\scriptsize, text=color4!90!black] at ($(p_gpt4) + (-0.1, 0.2)$) {GPT-4 ($\sim$50,000 MWh)};

% Llama 3.1
\filldraw[draw=color3!80!black, fill=white, line width=1.5pt] (p_llama) circle (3.5pt);
\node[anchor=west, font=\bfseries\scriptsize, text=color3!80!black] at ($(p_llama) + (0.2, 0.15)$) {Llama 3.1 ($\sim$30,000 MWh)};
...
While AI logic follows the "iron law" of software optimization, energy follows the laws of chemistry and thermodynamics. Over the same seven-year interval, battery energy density would improve by only ~`{python} EnergyWallScenario.battery_gain_pct_str` at a 5 percent annual rate, and grid efficiency by ~`{python} EnergyWallScenario.grid_gain_pct_str` at a 2 percent annual rate. The `{python} EnergyWallScenario.energy_wall_gap_mult_str` gap between these curves is the energy wall, the point where it is no longer possible to "buy a way out" of the efficiency problem with more power.
...
[^fn-pue-efficiency-evolution]: **PUE (power usage effectiveness)**\index{Power Usage Effectiveness!definition}: PUE compares total facility power with the power delivered to IT equipment [@thegreengrid2007pue]. Lowering non-IT overhead improves facility efficiency, but the metric does not capture the carbon intensity of electricity or the water consumed for cooling. Carbon usage effectiveness and water usage effectiveness therefore expose constraints that PUE alone leaves invisible.
...
The hardware choice is the first place where energy physics becomes an architecture decision. Different processor types affect environmental impact through their energy characteristics. Using pJ/FLOP as a common comparison point, central processing units consume approximately 100 pJ/FLOP, graphics processing units achieve roughly 10 pJ/FLOP for dense tensor operations, specialized tensor processors reach about 1--2 pJ/FLOP, and fixed-function low-precision accelerators approach 0.1 pJ/operation.[^fn-pjmac-energy-hierarchy] These hardware platforms require rare earth metals and complex manufacturing processes with embodied carbon.

[^fn-pjmac-energy-hierarchy]: **pJ/FLOP and pJ/multiply-accumulate (MAC)**\index{Multiply-Accumulate}: Energy-efficiency specifications often mix floating-point and multiply-accumulate operations (where 1 MAC = 2 FLOPs), requiring conversion across unit conventions and precision levels. The simplified hierarchy used here aligns with later comparisons in this chapter: CPUs at ~100 pJ/FLOP, GPUs around 10 pJ/FLOP for dense tensor ops, TPUs around 1--2 pJ/FLOP, and custom low-precision application-specific integrated circuits (ASICs) approaching 0.1 pJ/op (see @sec-appdx-fleet-system-assumptions for exact energy per operation scaling across FP64 to INT4 data formats and 7nm, 5nm, and 3nm silicon nodes). Choosing the right hardware tier for a given workload can reduce energy consumption by 100--1,000$\times$ without any algorithmic changes.
...
- Selective, Event-Driven Activation: Rather than processing all information continuously, high-efficiency systems are asynchronous. They activate only small portions of the network at any time and consume energy only when actively processing changing signals.[^fn-eventdriven-energy-efficiency]
- **Local Learning and Sample Efficiency**\index{Local Learning and Sample Efficiency!definition}: Dense language-model scaling often requires training on trillions of tokens to achieve broad competence. High-efficiency models use strong inductive biases and self-supervised local learning to acquire capabilities from 10,000$\times$ less data in the motivating biological comparison, reducing the cumulative energy cost of the training phase.
...
@Fig-energy-intervention shows how a six-step energy-gap intervention cascade\index{Energy!gap intervention cascade} can reduce the energy gap by approximately 10,000$\times$, transforming an intractable divergence into an engineering challenge. The six per-step factors multiply to a larger figure on paper, but the headline 10,000$\times$ is a deliberately conservative composite: overlapping levers do not stack cleanly, since each intervention erodes the savings available to the next. No single lever is sufficient; closing the gap requires simultaneous progress across algorithmic, hardware, and systemic fronts.

::: {#fig-energy-intervention fig-env="figure" fig-pos="htb" fig-cap="**Energy Gap Intervention Cascade**: Six successive intervention steps progressively close the energy gap. The combined cascade delivers a conservative composite of roughly $10{,}000\times$, below the naive product of the per-step factors because overlapping levers do not stack cleanly, leaving a tractable residual that engineering can close." fig-alt="Six horizontal step rows in the intervention cascade, decreasing in width as the gap closes from 350,000 times current down to a tractable residual, with combined reduction annotated as approximately 10,000 times."}
...
\begin{tikzpicture}[
    font=\small,
    >=Stealth,
    stagecard/.style={draw=color1!30, fill=color1!4!white, rounded corners=4pt, inner sep=5pt, text width=4.4cm, minimum height=1.3cm},
    multicard/.style={draw=color2!60, fill=white, rounded corners=4pt, inner sep=4pt, text width=7.4cm, minimum height=1.3cm}
]

% ============================================================
% 6 ROWS: LEFT STAGE CARD & CENTER MULTIPLIER CARD
% ============================================================

\foreach \i/\num/\title/\subA/\subB/\mult/\headB/\descB in {
    1/1/ALGORITHM EFFICIENCY/{Model Architecture \& Sparsity}/{Highest leverage intervention}/{100$\times$}/{Pruning, Quantization, MoE, Distillation, FlashAttention}/{Directly eliminates FLOPs from computation graph},
    2/2/HARDWARE SPECIALIZATION/{Domain Silicon Architecture}/{ASIC \& tensor core efficiency}/{10$\times$}/{Custom Accelerators (TPU / H100 / Trainium)}/{Advanced process lithography, systolic arrays},
    3/3/SYSTEM RUNTIME \& SERVING/{Distributed Execution \& Memory}/{Memory reuse \& pipeline scheduling}/{5$\times$}/{Continuous Batching \& KV-Cache}/{PagedAttention, overlap pipelining},
    4/4/DATACENTER FACILITY PUE/{Thermodynamics \& Cooling}/{Facility power efficiency ratio}/{1.5$\times$}/{Direct Liquid Cooling}/{Slashing PUE: 1.60 $\rightarrow$ 1.10},
    5/5/CARBON-AWARE SCHEDULING/{Spatial \& Temporal Shifting}/{Dynamic clean grid tracking}/{1.3$\times$}/{Temporal Load Shift}/{Clean solar/wind peak hours},
    6/6/CLEAN PROCUREMENT/{Energy PPAs \& Offsets}/{Grid-level generation contracts}/{1.1$\times$}/{24/7 CFE}/{Clean matching}
} {
    % Y position for row i
    \pgfmathsetmacro{\ypos}{-(\i - 1) * 1.65}

    % Left stage card
    \node[stagecard] (stage_\i) at (0, \ypos) {
        \textbf{\tiny\textcolor{darknavy}{\num\ $\cdot$ \title}}\\[1.5pt]
        \textbf{\scriptsize \subA}\\[1pt]
        \ifnum\i=1
            \tiny\textcolor{color2!80!black}{\textbf{\subB}}
        \else
            \tiny\textcolor{gray!70!black}{\subB}
        \fi
    };

    % Center multiplier card
    \node[multicard] (cascade_\i) at (6.6, \ypos) {
        \begin{minipage}{1.5cm}
            \centering
            \tikz[baseline=-0.5ex]{\node[fill=color2!15!white, text=color2!90!black, font=\bfseries\normalsize, rounded corners=3pt, inner sep=3pt, minimum width=1.4cm, align=center] {\mult};}
        \end{minipage}%
        \begin{minipage}{6.0cm}
            \textbf{\scriptsize \headB}\\[2pt]
            \tiny\textcolor{gray!70!black}{\descB}
        \end{minipage}
    };
}

% Vertical arrows between cascade cards
\foreach \i [evaluate=\i as \nexti using int(\i+1)] in {1,...,5} {
    \draw[->, >=Stealth, line width=1.4pt, gray!60] (cascade_\i.south) -- (cascade_\nexti.north);
}


% ============================================================
% RIGHT COLUMN: COMPOUND MULTIPLICATIVE GAIN
% ============================================================
\coordinate (right_top) at (11.0, 0.65);

\node[draw=color1!40, fill=color1!3!white, rounded corners=6pt, line width=0.8pt, anchor=north west, inner sep=8pt, text width=5.8cm] (summary_card) at (right_top) {
    \tikz[baseline=-0.5ex]{\node[fill=color1!15!white, text=color1!85!black, font=\bfseries\scriptsize, rounded corners=2pt, inner sep=3.5pt, text width=5.5cm] {COMPOUND MULTIPLICATIVE GAIN};}\\[10pt]

    % Big Total Decarbonization Callout Box
    \tikz[baseline=-0.5ex]{\node[draw=color1!60, fill=white, rounded corners=4pt, line width=0.8pt, inner sep=8pt, text width=5.2cm, align=center] {
        \tiny\textcolor{gray!70!black}{\textbf{Total Multiplicative Decarbonization}}\\[4pt]
        \Huge\bfseries\textcolor{darknavy}{$\sim$10,000$\times$}
    };}\\[12pt]

    \textbf{\scriptsize Intervention Hierarchy Principle:}\\[4pt]
    \tiny Decarbonization leverage concentrates at the top of the engineering stack.\\[4pt]
    \tiny Algorithm and hardware optimizations deliver orders of magnitude higher reductions than downstream offsets.\\[12pt]

    % Full cascade formula box
    \tikz[baseline=-0.5ex]{\node[draw=gray!25, fill=white, rounded corners=4pt, inner sep=6pt, text width=5.3cm] {
        \textbf{\tiny Full Cascade Formula:}\\[3pt]
        \scriptsize\texttt{\textcolor{color2!80!black}{$100 \times 10 \times 5 \times 1.5 \times 1.3 \times 1.1$}}\\[2pt]
        \textbf{\scriptsize\textcolor{darknavy}{$\approx$ 10,725$\times$ System Net Gain}}
    };}
};
...
The convergence of exponential computational demands with hard physical efficiency limits creates an unsustainable trajectory that threatens the long-term viability of AI scaling. Altering this trajectory requires moving beyond back-of-the-envelope calculations and establishing rigorous, systemic frameworks for measuring and assessing energy consumption across the entire ML infrastructure.
...
```{=latex}
\providecolor{bookred}{RGB}{192,41,43}
\providecolor{bookblue}{RGB}{31,110,165}
\providecolor{bookorange}{RGB}{211,84,0}
\providecolor{bookgreen}{RGB}{39,174,96}
\providecolor{cardbg}{RGB}{250,252,254}
\providecolor{bordercolor}{RGB}{200,214,229}
\providecolor{mutedtext}{RGB}{100,110,125}
\providecolor{bodytext}{RGB}{40,45,55}
...
% Top Left: Environmental Justice (Red)
\node[
    draw=bookred!80,
    fill=bookred!4,
    boxstyle,
    ...
    \raggedright
    {\large\bfseries\textcolor{bookred}{Environmental Justice}}\\[2pt]
    {\footnotesize\itshape\textcolor{mutedtext}{Who bears the burden?}}\\[5pt]
    {\color{bookred!25}\rule{\linewidth}{0.6pt}}\\[6pt]
    {\footnotesize\textcolor{bodytext}{Datacenters concentrate in low-cost regions where local communities bear water depletion, heat island effects, and grid strain while economic gains flow to distant technology hubs.}}\\[10pt]
    \fcolorbox{bookred!40}{white}{%
        \parbox{0.94\linewidth}{%
            \scriptsize\textbf{\textcolor{bookred}{Impact:}} 760B liters cooling water/yr; grid capacity competition with residential and municipal loads.%
        }%
    }
};

% Top Right: Intergenerational Equity (Orange)
\node[
    draw=bookorange!80,
    fill=bookorange!4,
    boxstyle,
    ...
    \raggedright
    {\large\bfseries\textcolor{bookorange}{Intergenerational Equity}}\\[2pt]
    {\footnotesize\itshape\textcolor{mutedtext}{What cost do future generations pay?}}\\[5pt]
    {\color{bookorange!25}\rule{\linewidth}{0.6pt}}\\[6pt]
    {\footnotesize\textcolor{bodytext}{Carbon emitted today has cumulative climate effects lasting decades. Frontier model training externalizes environmental costs onto future populations without consent.}}\\[10pt]
    \fcolorbox{bookorange!40}{white}{%
        \parbox{0.94\linewidth}{%
            \scriptsize\textbf{\textcolor{bookorange}{Impact:}} AI projected at up to 8\% of global power by 2030; cumulative $\text{CO}_2$ persists in atmosphere for centuries.%
        }%
    }
};

% Bottom Left: Resource Competition (Blue)
\node[
    draw=bookblue!80,
    fill=bookblue!4,
    boxstyle,
    ...
    \raggedright
    {\large\bfseries\textcolor{bookblue}{Resource Competition}}\\[2pt]
    {\footnotesize\itshape\textcolor{mutedtext}{What else could these resources power?}}\\[5pt]
    {\color{bookblue!25}\rule{\linewidth}{0.6pt}}\\[6pt]
    {\footnotesize\textcolor{bodytext}{AI infrastructure competes for water, land, rare-earth minerals, and grid interconnects with residential, agricultural, and broader industrial decarbonization needs.}}\\[10pt]
    \fcolorbox{bookblue!40}{white}{%
        \parbox{0.94\linewidth}{%
            \scriptsize\textbf{\textcolor{bookblue}{Impact:}} 15+ rare earth elements per accelerator; semiconductor fab requires 2,500+ L ultrapure water per die.%
        }%
    }
};

% Bottom Right: Transparency Gap (Green)
\node[
    draw=bookgreen!80,
    fill=bookgreen!4,
    boxstyle,
    ...
    \raggedright
    {\large\bfseries\textcolor{bookgreen}{Transparency Gap}}\\[2pt]
    {\footnotesize\itshape\textcolor{mutedtext}{What remains hidden from public scrutiny?}}\\[5pt]
    {\color{bookgreen!25}\rule{\linewidth}{0.6pt}}\\[6pt]
    {\footnotesize\textcolor{bodytext}{Most AI labs do not disclose model training energy, embodied carbon, or operational water footprints. Without open benchmarks, independent auditing is impossible.}}\\[10pt]
    \fcolorbox{bookgreen!40}{white}{%
        \parbox{0.94\linewidth}{%
            \scriptsize\textbf{\textcolor{bookgreen}{Impact:}} Frontier model training energy omitted from tech reports; absence of standardized reporting frameworks.%
        }%
    }
};
...
\node[
    ...
    line width=1.5pt,
    align=center
] at (0,0) {AI's Environmental\\Impact};
...
\node[
    ...
    inner sep=7pt,
    anchor=north,
    minimum width=15.7cm,
    font=\footnotesize\itshape,
    text=bodytext,
    align=center
] at ($(bmid) + (0, -0.3)$) {
    Sustainable AI requires addressing all four dimensions simultaneously: justice, equity, resources, and transparency.
};
...
[^fn-grid-carbon-variance]: **Carbon Intensity Variance**\index{Carbon Intensity Variance!definition}: Grid carbon intensity spans two orders of magnitude: coal at 820 g CO~2~/kWh vs. hydro at 10–30 g CO~2~/kWh. Critically, intensity also varies temporally: Texas fluctuates 10$\times$ within a single day based on wind generation. This dual geographic and temporal variance is what makes carbon-aware scheduling viable: combining the geographic lever (the 8 to 40 times range from @sec-sustainable-ai-scale-environmental-impact-ac9a) with temporal shifting pushes the achievable spread toward the high end of the 10 to 80 times span, so identical training runs can differ several-fold in emissions based solely on when and where they execute.
...
class VoltageScalingDynamicPower:
    # ┌── 1. LOAD ──────────────────────────────────────────
    v_high = 1.0  # nominal supply voltage (V)
    v_low = 0.8  # reduced supply voltage (V)
    ...
    v_high_str = fmt(v_high, precision=0, commas=False)
    v_low_str = fmt(v_low, precision=1, commas=False)
    power_reduction_pct_str = fmt_percent(power_reduction, precision=0, commas=False, style="prose")
```

The practical implications for AI systems follow directly from these physics. The quadratic voltage dependence means that reducing voltage from `{python} VoltageScalingDynamicPower.v_high_str`V to `{python} VoltageScalingDynamicPower.v_low_str`V decreases dynamic power by `{python} VoltageScalingDynamicPower.power_reduction_pct_str`, even before considering that lower voltages often enable frequency reduction with additional linear savings. This relationship explains why specialized AI accelerators operating at lower voltages but higher utilization can achieve order-of-magnitude efficiency improvements over general-purpose processors.
...
Beyond chip-level power, data center infrastructure imposes additional energy overhead. @Eq-sustainable-ai-pue captures this relationship through the power usage effectiveness (PUE) metric:
...
2.  **Distinction**: Unlike Computing Efficiency (which focuses on FLOPs per Watt), PUE focuses on Facility Efficiency\index{Efficiency!facility}: it captures how much energy is "wasted" before it even reaches the processor.
...
\providecolor{cardbg}{RGB}{250,252,254}
\providecolor{bordercolor}{RGB}{200,214,229}
\providecolor{gridline}{RGB}{235,240,245}
\providecolor{mutedtext}{RGB}{100,110,125}
\providecolor{bodytext}{RGB}{40,45,55}
...
\foreach \y in {100, 200, 300, 400, 500, 600, 700, 800} {
    \draw[gridline, line width=0.7pt] (-0.3, \y*\ys) -- (12.3, \y*\ys);
    \node[anchor=east, font=\footnotesize, text=mutedtext] at (-0.4, \y*\ys) {\y};
}
\node[anchor=east, font=\footnotesize, text=mutedtext] at (-0.4, 0) {0};

% Axis lines
\draw[bordercolor!80, line width=1pt] (-0.3, 0) -- (12.3, 0);
\draw[bordercolor!80, line width=1pt] (-0.3, 0) -- (-0.3, 850*\ys);

% Y axis label
\node[rotate=90, font=\bfseries\footnotesize, text=bodytext] at (-1.3, 425*\ys) {Carbon Intensity ($\text{gCO}_2$ / kWh)};

% Norway
\coordinate (c1) at (0.9, 0);
\draw[draw=bookgreen!85, fill=bookgreen!15, line width=1pt, rounded corners=2pt]
    ($(c1)+(-\w,0)$) rectangle ($(c1)+(\w, 23*\ys)$);
\node[above, font=\bfseries\footnotesize, text=bookgreen!90] (n1) at ($(c1)+(0, 23*\ys)$) {23};
\node[below=3pt, font=\bfseries\footnotesize, text=bodytext] at (c1) {Norway};
\node[below=15pt, font=\scriptsize, text=mutedtext] at (c1) {Hydro};

% France
\coordinate (c2) at (2.6, 0);
\draw[draw=bookgreen!85, fill=bookgreen!15, line width=1pt, rounded corners=2pt]
    ($(c2)+(-\w,0)$) rectangle ($(c2)+(\w, 85*\ys)$);
\node[above, font=\bfseries\footnotesize, text=bookgreen!90] at ($(c2)+(0, 85*\ys)$) {85};
\node[below=3pt, font=\bfseries\footnotesize, text=bodytext] at (c2) {France};
\node[below=15pt, font=\scriptsize, text=mutedtext] at (c2) {Nuclear};

% California
\coordinate (c3) at (4.3, 0);
\draw[draw=bookorange!85, fill=bookorange!15, line width=1pt, rounded corners=2pt]
    ($(c3)+(-\w,0)$) rectangle ($(c3)+(\w, 210*\ys)$);
\node[above, font=\bfseries\footnotesize, text=bookorange!90] at ($(c3)+(0, 210*\ys)$) {210};
\node[below=3pt, font=\bfseries\footnotesize, text=bodytext] at (c3) {California};
\node[below=15pt, font=\scriptsize, text=mutedtext] at (c3) {Mixed / Solar};

% EU Avg
\coordinate (c4) at (6.0, 0);
\draw[draw=bookorange!85, fill=bookorange!15, line width=1pt, rounded corners=2pt]
    ($(c4)+(-\w,0)$) rectangle ($(c4)+(\w, 270*\ys)$);
\node[above, font=\bfseries\footnotesize, text=bookorange!90] at ($(c4)+(0, 270*\ys)$) {270};
\node[below=3pt, font=\bfseries\footnotesize, text=bodytext] at (c4) {EU Avg};
\node[below=15pt, font=\scriptsize, text=mutedtext] at (c4) {Grid Mix};

% US Avg
\coordinate (c5) at (7.7, 0);
\draw[draw=bookorange!85, fill=bookorange!15, line width=1pt, rounded corners=2pt]
    ($(c5)+(-\w,0)$) rectangle ($(c5)+(\w, 386*\ys)$);
\node[above, font=\bfseries\footnotesize, text=bookorange!90] at ($(c5)+(0, 386*\ys)$) {386};
\node[below=3pt, font=\bfseries\footnotesize, text=bodytext] at (c5) {US Avg};
\node[below=15pt, font=\scriptsize, text=mutedtext] at (c5) {Gas / Coal};

% China
\coordinate (c6) at (9.4, 0);
\draw[draw=bookred!85, fill=bookred!15, line width=1pt, rounded corners=2pt]
    ($(c6)+(-\w,0)$) rectangle ($(c6)+(\w, 581*\ys)$);
\node[above, font=\bfseries\footnotesize, text=bookred!90] at ($(c6)+(0, 581*\ys)$) {581};
\node[below=3pt, font=\bfseries\footnotesize, text=bodytext] at (c6) {China};
\node[below=15pt, font=\scriptsize, text=mutedtext] at (c6) {Coal-heavy};

% Poland
\coordinate (c7) at (11.1, 0);
\draw[draw=bookred!85, fill=bookred!15, line width=1pt, rounded corners=2pt]
    ($(c7)+(-\w,0)$) rectangle ($(c7)+(\w, 773*\ys)$);
\node[above, font=\bfseries\footnotesize, text=bookred!90] (n7) at ($(c7)+(0, 773*\ys)$) {773};
\node[below=3pt, font=\bfseries\footnotesize, text=bodytext] at (c7) {Poland};
\node[below=15pt, font=\scriptsize, text=mutedtext] at (c7) {Coal};

% Green threshold line at 100
\draw[bookgreen, dashed, line width=1.2pt] (-0.3, 100*\ys) -- (12.3, 100*\ys);
\node[draw=bookgreen, fill=white, rounded corners=2pt, font=\scriptsize\bfseries, text=bookgreen, inner sep=3pt] at (11.1, 100*\ys + 0.35) {``Green'' $< 100$};

% Top Spread Bracket
\coordinate (b_top1) at ($(c1)+(0, 860*\ys)$);
\coordinate (b_top7) at ($(c7)+(0, 860*\ys)$);
\draw[mutedtext!60, dashed, line width=0.8pt] (n1.north) -- (b_top1);
\draw[mutedtext!60, dashed, line width=0.8pt] (n7.north) -- (b_top7);
\draw[bookred, line width=1.2pt] (b_top1) -- (b_top7);

\node[fill=bookred!10, draw=bookred!70, rounded corners=3pt, font=\footnotesize\bfseries, text=bookred, inner sep=4pt]
    at ($0.5*(b_top1) + 0.5*(b_top7)$) {33.6$\times$ REGIONAL EMISSION FACTOR SPREAD};
...
\node[
    ...
    \begin{minipage}{12.4cm}
        \textbf{\textcolor{bookred}{SPATIAL LOAD PLACEMENT LEVERAGE}} \hfill {\scriptsize\textcolor{mutedtext}{Sources: EIA (2023), IEA / Ember (2024)}}\\[3pt]
        {\color{bordercolor}\rule{\linewidth}{0.5pt}}\\[4pt]
        {\footnotesize\textcolor{bodytext}{Geographic site selection is the single highest-leverage operational decarbonization decision.}}\\[2pt]
        {\footnotesize\textcolor{bodytext}{Identical 10~GWh model training run: Norway (hydro) = \textbf{\textcolor{bookgreen}{230~t~$\text{CO}_2$}} vs.\ Poland (coal) = \textbf{\textcolor{bookred}{7,730~t~$\text{CO}_2$}}.}}
    \end{minipage}
};
...
Precision affects energy per operation through implementation-specific arithmetic and data movement. Narrower formats can reduce circuit area, switched capacitance, and memory traffic, but bit width alone does not determine wall energy, and it does not confer a universal voltage reduction. The architecture values in @tbl-energy-per-op are representative points from different platforms rather than a controlled INT8-versus-FP32 scaling law; the measurement contract requires a same-platform comparison at fixed quality.
...
Together, these levers make sustainability a locality-and-reuse problem, rather than a faster-arithmetic problem alone.
...
The maximum term identifies the dominant bottleneck for roofline reasoning; it is not the full energy in balanced cases. @Eq-ai-crossover defines the crossover arithmetic intensity\index{Crossover Arithmetic Intensity} where compute and memory energy balance:
...
class MatMulEnergyAnalysis:
    ...
    comp_large_eq_math = fmt_math(rf"2 \times 10^9 \times {e_compute:.0f} \text{{ pJ}} = {comp_large_pj/1e9:.0f}")
    mem_large_eq_math = fmt_math(rf"3 \times 10^6 \times {bytes_fp32:.0f} \times {e_move:.0f} \text{{ pJ}} = {mem_large_pj/1e9:.2f}")
    ai_vec_eq_math = fmt_math(rf"\text{{AI}} = {n_vec}/{vec_bytes:.0f} = {ai_vec:.3f}")
    comp_vec_eq_math = fmt_math(rf"{n_vec} \times {e_compute:.0f} \text{{ pJ}} = {comp_vec_pj/1e9:.5f}")
    mem_vec_eq_math = fmt_math(rf"{vec_bytes:.0f} \times {e_move:.0f} \text{{ pJ}} = {mem_vec_pj/1e9:.5f}")
```
...
% Dimensions: 4 decades X (0.1 to 1000), 4 decades Y (0.01 to 100)
\def\xstep{3.2}
\def\ystep{2.0}

% Grid lines
\foreach \i in {0, 1, 2, 3, 4} {
    \draw[gridline, line width=0.6pt] (\i*\xstep, 0) -- (\i*\xstep, 4*\ystep);
}
\foreach \j in {0, 1, 2, 3, 4} {
    \draw[gridline, line width=0.6pt] (0, \j*\ystep) -- (4*\xstep, \j*\ystep);
}

% Axes
\draw[-{Stealth[length=6pt]}, line width=1.2pt, draw=bodytext!80]
    (0,0) -- (4*\xstep + 0.6, 0) node[below=18pt, midway, font=\bfseries\footnotesize, text=bodytext] {Arithmetic Intensity (FLOP / Byte)};
\draw[-{Stealth[length=6pt]}, line width=1.2pt, draw=bodytext!80]
    (0,0) -- (0, 4*\ystep + 0.5);
\node[rotate=90, font=\bfseries\footnotesize, text=bodytext, anchor=south]
    at (-1.0, 2*\ystep) {Energy Efficiency (GFLOP / Joule)};

% X tick labels
\node[below=4pt, font=\footnotesize, text=bodytext] at (0*\xstep, 0) {0.1};
\node[below=4pt, font=\footnotesize, text=bodytext] at (1*\xstep, 0) {1.0};
\node[below=4pt, font=\footnotesize, text=bodytext] at (2*\xstep, 0) {10};
\node[below=4pt, font=\footnotesize, text=bodytext] at (3*\xstep, 0) {100};
\node[below=4pt, font=\footnotesize, text=bodytext] at (4*\xstep, 0) {1000};

% Y tick labels
\node[left=4pt, font=\footnotesize, text=bodytext] at (0, 0*\ystep) {0.01};
\node[left=4pt, font=\footnotesize, text=bodytext] at (0, 1*\ystep) {0.1};
\node[left=4pt, font=\footnotesize, text=bodytext] at (0, 2*\ystep) {1.0};
\node[left=4pt, font=\footnotesize, text=bodytext] at (0, 3*\ystep) {10};
\node[left=4pt, font=\footnotesize, text=bodytext] at (0, 4*\ystep) {100};

% Roofline curve
% Ridge point at x=10 (index 2), y=10 (index 3)
\coordinate (ridge) at (2*\xstep, 3*\ystep);

% Memory bound slope
\draw[bookblue, line width=2.4pt] (0,0) -- (ridge);
\path (0,0) -- (ridge) coordinate[pos=0.45] (mid_slope);
\node[bookblue, font=\bfseries\footnotesize, rotate=43, above=5pt]
    at (mid_slope) {Memory Energy Bound Region};

% Compute bound ceiling
\draw[bookred, line width=2.4pt] (ridge) -- (4*\xstep, 3*\ystep);
\node[bookred, font=\bfseries\footnotesize, above=6pt]
    at (3*\xstep, 3*\ystep) {Compute Energy Bound Ceiling};

% Ridge marker & drop line
\draw[mutedtext, dashed, line width=0.9pt] (ridge) -- (2*\xstep, 0);
\filldraw[bodytext] (ridge) circle (3.5pt);

\node[fill=white, draw=bordercolor, rounded corners=3pt, font=\scriptsize\bfseries, text=bodytext, inner sep=3.5pt]
    at (2*\xstep, 0.45*\ystep) {Ridge: 10 FLOP/B};

% Points
% Point A: Autoregressive Decode (x~1.6, y~0.07) -> X = 1.2 * 3.2 = 3.84, Y = 1.44 * (2.0/1.7) = 1.7
\coordinate (ptA) at (3.8, 1.7);
\draw[bookblue, fill=white, line width=2pt] (ptA) circle (4.5pt);
\node[
    fill=white,
    draw=bordercolor,
    rounded corners=3pt,
    ...
    \begin{minipage}{3.4cm}
        {\scriptsize\bfseries\textcolor{bookblue}{A $\cdot$ Autoregressive Decode}}\\[-1pt]
        {\tiny\textcolor{mutedtext}{Memory-bound, low efficiency}}
    \end{minipage}
};

% Point C: Sparse MoE Routing (x~4.0, y~0.22) -> X = 1.6 * 3.2 = 5.12, Y = 2.68
\coordinate (ptC) at (5.2, 2.7);
\draw[bookgreen, fill=white, line width=2pt] (ptC) circle (4.5pt);
\node[
    fill=white,
    draw=bordercolor,
    rounded corners=3pt,
    ...
    \begin{minipage}{3.7cm}
        {\scriptsize\bfseries\textcolor{bookgreen}{C $\cdot$ Sparse MoE Routing}}\\[-1pt]
        {\tiny\textcolor{mutedtext}{Intermediate intensity}}
    \end{minipage}
};

% Point B: Dense GEMM Training (x~60, y~7.0) -> X = 2.78 * 3.2 = 8.9, Y = 5.5
\coordinate (ptB) at (8.8, 5.5);
\draw[bookred, fill=white, line width=2pt] (ptB) circle (4.5pt);
\node[
    fill=white,
    draw=bordercolor,
    rounded corners=3pt,
    ...
    \begin{minipage}{3.4cm}
        {\scriptsize\bfseries\textcolor{bookred}{B $\cdot$ Dense GEMM Training}}\\[-1pt]
        {\tiny\textcolor{mutedtext}{Near peak compute efficiency}}
    \end{minipage}
};
...
\node[
    ...
    \begin{minipage}{5.0cm}
        \raggedright
        {\scriptsize\bfseries\textcolor{bodytext}{DATA MOVEMENT VS COMPUTE ENERGY}}\\[2pt]
        {\tiny\textcolor{mutedtext}{DRAM Transfer: $\sim$100~pJ / byte}}\\[1pt]
        {\tiny\textcolor{mutedtext}{FP32 ALU Compute: $\sim$10~pJ / FLOP}}\\[2pt]
        {\scriptsize\bfseries\textcolor{bookblue}{Ratio: 1 Byte Transfer $\approx$ 10 FLOPs Energy}}\\[1pt]
        {\tiny\textcolor{mutedtext}{Inference decode lies deep in memory-bound zone}}
    \end{minipage}
};
...
1. Operation & byte count: $2N_{\text{mat}}^3$ FLOPs and $12N_{\text{mat}}^2$ bytes (FP32 precision).
...
|:-----------------------------------------------------------------------------|:-----------------------------------------------------------|:------------------------------------------------------|:-----------------------------------------------------|:------------------------------------|
| Small matrix ($N_{\text{mat}}=$ `{python} MatMulEnergyAnalysis.n_small_str`) | `{python} MatMulEnergyAnalysis.ai_small_eq_math` FLOP/byte | `{python} MatMulEnergyAnalysis.comp_small_eq_math` mJ | `{python} MatMulEnergyAnalysis.mem_small_eq_math` mJ | Balanced at the crossover           |
| Large matrix ($N_{\text{mat}} = 1000$)                                       | `{python} MatMulEnergyAnalysis.ai_large_eq_math` FLOP/byte | `{python} MatMulEnergyAnalysis.comp_large_eq_math` mJ | `{python} MatMulEnergyAnalysis.mem_large_eq_math` mJ | Improve compute efficiency          |
| Vector addition ($N_{\text{vec}} = 1000$)                                    | `{python} MatMulEnergyAnalysis.ai_vec_eq_math` FLOP/byte   | `{python} MatMulEnergyAnalysis.comp_vec_eq_math` mJ   | `{python} MatMulEnergyAnalysis.mem_vec_eq_math` mJ   | Reduce data movement through fusion |
...
- **Qualcomm Trepn Profiler**\index{Qualcomm Trepn Profiler!definition}: Offers millisecond-resolution power measurement on Snapdragon platforms, correlating power traces with code execution for NPU workload optimization.
...
|:---------------------|:-------------------------------|:------------:|:---------------------------------|
| **INA219/INA226**    | 100 microsecond sampling       |  $\pm 1\%$   | Low-cost embedded profiling      |
| **PAC1934**          | 1 millisecond, 4 channels      |  $\pm 2\%$   | Multi-rail MCU measurement       |
| **Joulescope JS220** | Sub-microsecond, nanoamp range | $\pm 0.1\%$  | Professional TinyML benchmarking |
| **Otii Arc Pro**     | 10 microsecond, automation     | $\pm 0.5\%$  | Automated battery life testing   |
...
- Requirement: Measure baseline power consumption across all sleep states, as baseline power can vary from 1 microamp in deep sleep to 1 milliamp in idle active states on typical microcontrollers.
- **Setup**: Execute 100 or more inference iterations before measurement during the **Warm-up Period**\index{Warm-up Period!definition} to reach thermal equilibrium, as initial iterations may exhibit different power characteristics due to cache warming and voltage regulator settling.
- **Constraint**: Account for the **Duty Cycle**\index{Duty Cycle!definition} by reporting both peak inference power and average power at realistic duty cycles, because edge devices typically operate with significant idle periods between inferences.
- Isolation: Disable or account for **Peripheral**\index{Peripheral!definition} power consumption, such as sensors, radios, and displays, when measuring model inference energy, because these can dominate total system power.

For duty cycle accounting, @eq-edge-duty-cycle expresses the relationship between active and idle power:
...
Operational energy dominates total cost of ownership for typical deployments (@fig-carbon-tco), but embodied carbon from semiconductor fabrication becomes the binding constraint as the grid shifts to renewables.

::: {#fig-carbon-tco fig-env="figure" fig-pos="htb" fig-cap="**Five-year cluster TCO**: Cumulative total cost of ownership for an illustrative 256-GPU H100 cluster on an average US grid, separating hardware CapEx from the recurring operational stack of electricity, cooling, and a carbon tax. Hardware is front-loaded into year 1 and never recurs, while the operational stack is charged every year and overtakes it within the deployment lifetime. That crossover is why utilization and energy efficiency, rather than purchase price, govern what a cluster actually costs to own." fig-alt="Stacked bar chart of cumulative total cost of ownership in dollars across five years of operation. The year 1 bar is dominated by a large hardware CapEx block; years 2 through 5 are much shorter bars of electricity, cooling, and carbon tax only. A dashed line traces the rising cumulative total across the five years."}
...
\providecolor{mutedtext}{RGB}{100,110,125}
\providecolor{bodytext}{RGB}{40,45,55}
\providecolor{hwblue}{RGB}{220,235,248}
\providecolor{elecorange}{RGB}{254,237,205}
\providecolor{coolred}{RGB}{254,215,215}
\providecolor{carbongreen}{RGB}{210,245,225}

\begin{tikzpicture}[font=\small]

% Scale factors:
% X: 5 years, x = 1 to 5, step = 2.2cm
% Y: 0 to 8M, y * 0.9 cm per Million
...
\foreach \y in {1, 2, 3, 4, 5, 6, 7} {
    \draw[gridline, line width=0.6pt] (-0.3, \y*\ys) -- (5*\xs + 0.6, \y*\ys);
    \node[anchor=east, font=\footnotesize, text=bodytext] at (-0.4, \y*\ys) {\$\y M};
}
\node[anchor=east, font=\footnotesize, text=bodytext] at (-0.4, 0) {\$0};

% Axes
\draw[bordercolor!80, line width=1pt] (-0.3, 0) -- (5*\xs + 0.6, 0);
\draw[bordercolor!80, line width=1pt] (-0.3, 0) -- (-0.3, 7.6*\ys);

% Axis labels
\node[rotate=90, font=\bfseries\footnotesize, text=bodytext, anchor=south]
    at (-1.4, 3.8*\ys) {Cumulative TCO (\$)};
\node[font=\bfseries\footnotesize, text=bodytext]
    at (3*\xs, -1.0) {Year of Operation};

% Data:
% Year 1: HW 3.2, Elec 0.6, Cool 0.2, Carbon 0.06 (Total = 4.06)
% Year 2: HW 0,   Elec 0.6, Cool 0.2, Carbon 0.08 (Total = 0.88)
% Year 3: HW 0,   Elec 0.6, Cool 0.2, Carbon 0.12 (Total = 0.92)
% Year 4: HW 0,   Elec 0.6, Cool 0.2, Carbon 0.17 (Total = 0.97)
% Year 5: HW 0,   Elec 0.6, Cool 0.2, Carbon 0.26 (Total = 1.06)

% Year 1 bar
\coordinate (c1) at (1*\xs, 0);
\draw[draw=bookblue!80, fill=hwblue, line width=0.8pt]
    ($(c1)+(-\w, 0)$) rectangle ($(c1)+(\w, 3.2*\ys)$);
\node[font=\bfseries\scriptsize, text=bodytext] at ($(c1)+(0, 1.6*\ys)$) {HW \$3.2M};

\draw[draw=bookorange!80, fill=elecorange, line width=0.8pt]
    ($(c1)+(-\w, 3.2*\ys)$) rectangle ($(c1)+(\w, 3.8*\ys)$);
\draw[draw=bookred!80, fill=coolred, line width=0.8pt]
    ($(c1)+(-\w, 3.8*\ys)$) rectangle ($(c1)+(\w, 4.0*\ys)$);
\draw[draw=bookgreen!80, fill=carbongreen, line width=0.8pt]
    ($(c1)+(-\w, 4.0*\ys)$) rectangle ($(c1)+(\w, 4.06*\ys)$);

% Year 2 bar
\coordinate (c2) at (2*\xs, 0);
\draw[draw=bookorange!80, fill=elecorange, line width=0.8pt]
    ($(c2)+(-\w, 0)$) rectangle ($(c2)+(\w, 0.6*\ys)$);
\draw[draw=bookred!80, fill=coolred, line width=0.8pt]
    ($(c2)+(-\w, 0.6*\ys)$) rectangle ($(c2)+(\w, 0.8*\ys)$);
\draw[draw=bookgreen!80, fill=carbongreen, line width=0.8pt]
    ($(c2)+(-\w, 0.8*\ys)$) rectangle ($(c2)+(\w, 0.88*\ys)$);

% Year 3 bar
\coordinate (c3) at (3*\xs, 0);
\draw[draw=bookorange!80, fill=elecorange, line width=0.8pt]
    ($(c3)+(-\w, 0)$) rectangle ($(c3)+(\w, 0.6*\ys)$);
\draw[draw=bookred!80, fill=coolred, line width=0.8pt]
    ($(c3)+(-\w, 0.6*\ys)$) rectangle ($(c3)+(\w, 0.8*\ys)$);
\draw[draw=bookgreen!80, fill=carbongreen, line width=0.8pt]
    ($(c3)+(-\w, 0.8*\ys)$) rectangle ($(c3)+(\w, 0.92*\ys)$);

% Year 4 bar
\coordinate (c4) at (4*\xs, 0);
\draw[draw=bookorange!80, fill=elecorange, line width=0.8pt]
    ($(c4)+(-\w, 0)$) rectangle ($(c4)+(\w, 0.6*\ys)$);
\draw[draw=bookred!80, fill=coolred, line width=0.8pt]
    ($(c4)+(-\w, 0.6*\ys)$) rectangle ($(c4)+(\w, 0.8*\ys)$);
\draw[draw=bookgreen!80, fill=carbongreen, line width=0.8pt]
    ($(c4)+(-\w, 0.8*\ys)$) rectangle ($(c4)+(\w, 0.97*\ys)$);

% Year 5 bar
\coordinate (c5) at (5*\xs, 0);
\draw[draw=bookorange!80, fill=elecorange, line width=0.8pt]
    ($(c5)+(-\w, 0)$) rectangle ($(c5)+(\w, 0.6*\ys)$);
\draw[draw=bookred!80, fill=coolred, line width=0.8pt]
    ($(c5)+(-\w, 0.6*\ys)$) rectangle ($(c5)+(\w, 0.8*\ys)$);
\draw[draw=bookgreen!80, fill=carbongreen, line width=0.8pt]
    ($(c5)+(-\w, 0.8*\ys)$) rectangle ($(c5)+(\w, 1.06*\ys)$);

% Year labels below
\node[below=4pt, font=\bfseries\footnotesize, text=bodytext] at (c1) {Year 1};
\node[below=4pt, font=\bfseries\footnotesize, text=bodytext] at (c2) {Year 2};
\node[below=4pt, font=\bfseries\footnotesize, text=bodytext] at (c3) {Year 3};
\node[below=4pt, font=\bfseries\footnotesize, text=bodytext] at (c4) {Year 4};
\node[below=4pt, font=\bfseries\footnotesize, text=bodytext] at (c5) {Year 5};

% Cumulative dashed line & markers:
% Y1: 4.1M, Y2: 4.9M, Y3: 5.9M, Y4: 6.8M, Y5: 7.9M
\coordinate (cum1) at (1*\xs, 4.1*\ys);
\coordinate (cum2) at (2*\xs, 4.9*\ys);
\coordinate (cum3) at (3*\xs, 5.9*\ys);
\coordinate (cum4) at (4*\xs, 6.8*\ys);
\coordinate (cum5) at (5*\xs, 7.9*\ys);

\draw[bodytext, dashed, line width=1.5pt]
    (cum1) -- (cum2) -- (cum3) -- (cum4) -- (cum5);

\node[above=3pt, font=\bfseries\footnotesize, text=bodytext] at (cum1) {\$4.1M};
\node[above=3pt, font=\bfseries\footnotesize, text=bodytext] at (cum2) {\$4.9M};
\node[above=3pt, font=\bfseries\footnotesize, text=bodytext] at (cum3) {\$5.9M};
\node[above=3pt, font=\bfseries\footnotesize, text=bodytext] at (cum4) {\$6.8M};
\node[above=3pt, font=\bfseries\footnotesize, text=bodytext] at (cum5) {\$7.9M};

% Green dashed line connecting Year 1 top to Year 5 top
\draw[bookgreen, dashed, line width=1.2pt]
    ($(c1)+(0, 4.06*\ys)$) -- ($(c5)+(0, 1.06*\ys)$);
\path ($(c1)+(0, 4.06*\ys)$) -- ($(c5)+(0, 1.06*\ys)$) coordinate[pos=0.7] (gmid);
\node[bookgreen, font=\bfseries\scriptsize, rotate=-19, above=3pt]
    at (gmid) {Carbon cost rising 5$\times$};

% Callout box: Hardware front-loaded
\node[
    draw=bookred,
    fill=white,
    line width=0.9pt,
    ...
    {\bfseries\scriptsize\textcolor{bookred}{Hardware: front-loaded}}\\[2pt]
    {\tiny\textcolor{bodytext}{Energy + carbon: recurring and growing}}
};
...
\node[
    ...
    % 3.4cm: wide enough for the single-column legend and a one-line title,
    % narrow enough to clear the $4.9M and $5.9M cumulative callouts.
    \begin{minipage}{3.4cm}
        {\bfseries\fontsize{7pt}{8.5pt}\selectfont\textcolor{bodytext}{256-GPU H100 Cluster TCO}}\\[4pt]
        % Single column, not two: a two-column legend measures ~5.75cm, which is
        % wider than the gap to the $5.9M cumulative callout, so the card either
        % covered that label or its own columns collided.
        \begin{tikzpicture}[x=1cm, y=1cm]
            \draw[sharp corners, draw=bookblue!80, fill=hwblue, line width=0.6pt] (0, 0.84) rectangle (0.3, 0.96);
            \node[right=2pt, font=\tiny, text=bodytext] at (0.3, 0.90) {Hardware (CapEx)};
            \draw[sharp corners, draw=bookorange!80, fill=elecorange, line width=0.6pt] (0, 0.56) rectangle (0.3, 0.68);
            \node[right=2pt, font=\tiny, text=bodytext] at (0.3, 0.62) {Electricity (OpEx)};
            \draw[sharp corners, draw=bookred!80, fill=coolred, line width=0.6pt] (0, 0.28) rectangle (0.3, 0.40);
            \node[right=2pt, font=\tiny, text=bodytext] at (0.3, 0.34) {Cooling (OpEx)};
            \draw[sharp corners, draw=bookgreen!80, fill=carbongreen, line width=0.6pt] (0, 0.00) rectangle (0.3, 0.12);
            \node[right=2pt, font=\tiny, text=bodytext] at (0.3, 0.06) {Carbon Tax (OpEx)};
        \end{tikzpicture}
    \end{minipage}
};

% Provenance note carried over from the SVG this figure replaced: these are
% scenario values, not measured ones.
\node[anchor=north east, font=\itshape\tiny, text=mutedtext] at (5*\xs, -1.25)
  {Illustrative 256$\times$H100 cluster, US avg grid};
...
% Top 3 Header boxes
\node[headerbox, draw=bookorange, fill=bookorange!15] (h1) at (0, 0) {
    {\bfseries Manufacturing}\\[-1pt]
    {\scriptsize\textcolor{bodytext!80}{Embodied Carbon}}
};

\node[headerbox, draw=bookred, fill=bookred!15] (h2) at (5.2, 0) {
    {\bfseries Training}\\[-1pt]
    {\scriptsize\textcolor{bodytext!80}{Operational Carbon}}
};

\node[headerbox, draw=bookblue, fill=bookblue!15] (h3) at (10.4, 0) {
    {\bfseries Inference}\\[-1pt]
    {\scriptsize\textcolor{bodytext!80}{Operational Carbon}}
};

% Connecting arrows
\draw[-{Stealth[length=6pt]}, line width=1.4pt, draw=bodytext!70] (h1.east) -- (h2.west);
\draw[-{Stealth[length=6pt]}, line width=1.4pt, draw=bodytext!70] (h2.east) -- (h3.west);

% Column 1: Manufacturing
\node[colbox, draw=bookorange, fill=bookorange!8, anchor=north] (col1) at (0, -1.0) {};
\node[font=\itshape\scriptsize, text=bookorange!90, anchor=south] at ($(col1.north)+(0, 2pt)$) {150--200~kg~$\text{CO}_2$/GPU};
\node[font=\huge\bfseries, text=bookorange] at ($(col1.north)+(0, -1.5)$) {30\%};

\node[itembox, draw=bookorange!70] (c1_4) at ($(col1.south)+(0, 0.7)$) {Supply Chain};
\node[itembox, draw=bookorange!70] (c1_3) at ($(c1_4.north)+(0, 0.4)$) {Rare Earth Mining};
\node[itembox, draw=bookorange!70] (c1_2) at ($(c1_3.north)+(0, 0.4)$) {Server Assembly};
\node[itembox, draw=bookorange!70] (c1_1) at ($(c1_2.north)+(0, 0.4)$) {Chip Fabrication};

% Column 2: Training
\node[colbox, draw=bookred, fill=bookred!8, anchor=north] (col2) at (5.2, -1.0) {};
\node[draw=bookred!60, fill=white, rounded corners=2pt, font=\itshape\scriptsize, text=bookred, inner sep=3pt]
    at ($(col2.north)+(0, -0.45)$) {GPT-3: 1,287~MWh $\approx 552\ \text{t}\ \text{CO}_2$};
\node[font=\huge\bfseries, text=bookred] at ($(col2.north)+(0, -1.5)$) {50\%};

\node[itembox, draw=bookred!70] (c2_4) at ($(col2.south)+(0, 0.7)$) {Checkpoint I/O};
\node[itembox, draw=bookred!70] (c2_3) at ($(c2_4.north)+(0, 0.4)$) {Gradient Sync Overhead};
\node[itembox, draw=bookred!70] (c2_2) at ($(c2_3.north)+(0, 0.4)$) {Grid Carbon Intensity};
\node[itembox, draw=bookred!70] (c2_1) at ($(c2_2.north)+(0, 0.4)$) {$\text{Compute} \times \text{PUE} \times \text{Time}$};

% Column 3: Inference
\node[colbox, draw=bookblue, fill=bookblue!8, anchor=north] (col3) at (10.4, -1.0) {};
\node[font=\itshape\scriptsize, text=bookblue!90, anchor=south] at ($(col3.north)+(0, 2pt)$) {Scales with deployment};
\node[font=\huge\bfseries, text=bookblue] at ($(col3.north)+(0, -1.5)$) {20\%};

\node[itembox, draw=bookblue!70] (c3_3) at ($(col3.south)+(0, 0.8)$) {KV Cache Memory};
\node[itembox, draw=bookblue!70] (c3_2) at ($(c3_3.north)+(0, 0.45)$) {Grid Carbon Intensity};
\node[itembox, draw=bookblue!70] (c3_1) at ($(c3_2.north)+(0, 0.45)$) {$\text{Requests} \times \text{Energy/req}$};

% Bottom Section 1: Lifecycle Carbon Equation
\node[
    draw=bordercolor,
    fill=white,
    rounded corners=3pt,
    ...
    {\bfseries\footnotesize Lifecycle Carbon Equation}\\[3pt]
    {\footnotesize $\text{Total CO}_2 = \text{Embodied CO}_2 + \text{Training Energy} \times \text{PUE} \times \text{Grid CI} + \text{Request Count} \times \text{Energy/Request} \times \text{Grid CI}$}
};

% Bottom Section 2: Key Insight Banner
\node[
    draw=bookred,
    fill=white,
    rounded corners=3pt,
    ...
    \begin{minipage}{14.4cm}
        {\bfseries\textcolor{bookred}{Key Insight:}}\;\;%
        {\footnotesize\textcolor{bodytext}{In clean-grid regions (hydro, nuclear), embodied carbon can exceed operational carbon over the hardware lifetime. Hardware longevity and utilization rate become first-order sustainability levers alongside energy source.}}
    \end{minipage}
};
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Infrastructure, Systems
from mlsysim.fmt import check, fmt, fmt_count, fmt_int, fmt_qty, fmt_energy, fmt_emissions, fmt_time
from mlsysim.physics.quantities import energy_from_power, carbon_from_energy
...
# ┌── 3. RENDER ────────────────────────────────────────────────────────────────
bars = ax.bar(labels, values, color=colors, width=0.5, edgecolor='white', linewidth=1.2, zorder=3)
...
ax.axhline(y=58, color=COLORS['BlueLine'], linestyle=':', linewidth=1, alpha=0.4)
ax.text(2.71, 62, '2014 baseline', fontsize=7, color='gray', ha='right')

# Growth annotations
ax.annotate('', xy=(1, 176), xytext=(0, 58), arrowprops=dict(arrowstyle='<->', color=COLORS['BrownLine'], lw=1.5))
ax.text(0.35, 129, '3$\\times$ growth\nin 9 years', fontsize=8, color=COLORS['BrownLine'], ha='center')
...
- Dynamic workload placement: Consolidate compatible jobs when doing so lets genuinely unused hardware enter a lower-power state; verify savings at the facility meter rather than inferring them from node count.
- Carbon-aware scheduling: Shift flexible work toward times or regions with lower decision-relevant emissions factors, subject to data-movement, deadline, and reliability constraints.
- Multi-tenant sharing: Raise useful accelerator occupancy without allowing interference, queueing, or additional iterations to erase the energy benefit.
- Batch processing: Combine compatible small jobs when the higher utilization outweighs batching delay and any quality or convergence effect.
...
% X axis: 70 to 100, width = 11cm
\def\xorigin{70}
\def\xscale{0.3667}

% Y axis: 0.1 to 1000 (4 decades), ystep = 1.8cm
\def\ystep{1.8}

% Grid lines
\foreach \x in {70, 75, 80, 85, 90, 95, 100} {
    \draw[gridline, line width=0.6pt] ({(\x-\xorigin)*\xscale}, 0) -- ({(\x-\xorigin)*\xscale}, 4*\ystep);
    \node[below=4pt, font=\footnotesize, text=bodytext] at ({(\x-\xorigin)*\xscale}, 0) {\x};
}

\foreach \j/\val in {0/0.1, 1/1, 2/10, 3/100, 4/{1,000}} {
    \draw[gridline, line width=0.6pt] (0, \j*\ystep) -- (11.0, \j*\ystep);
    \node[left=4pt, font=\footnotesize, text=bodytext] at (0, \j*\ystep) {\val};
}

% Axes
\draw[bordercolor!90, line width=1pt] (0, 0) -- (11.0, 0);
\draw[bordercolor!90, line width=1pt] (0, 0) -- (0, 4*\ystep);

% Axis labels
\node[font=\bfseries\footnotesize, text=bodytext] at (5.5, -0.9) {Task Accuracy (\%)};
\node[rotate=90, font=\bfseries\footnotesize, text=bodytext, anchor=south] at (-1.2, 2*\ystep) {$\text{CO}_2$ per Inference (mg, log)};

% Efficiency Zone
\draw[bookgreen, dashed, line width=1pt, fill=bookgreen!4]
    ({(81.5-\xorigin)*\xscale}, 0) rectangle ({(91.2-\xorigin)*\xscale}, 2.3);
\node[font=\bfseries\scriptsize, text=bookgreen]
    at ({0.5*(81.5+91.2-\xorigin*2)*\xscale}, 0.5) {Efficiency Zone};

% Pareto frontier curve (smooth spline)
\draw[bookred, dashed, line width=1.8pt]
    (4.77, 0.98) to[out=25, in=200]
    (5.13, 1.18) to[out=20, in=210]
    (6.23, 1.67) to[out=30, in=220]
    (7.26, 2.66) to[out=40, in=230]
    (7.70, 3.09) to[out=50, in=270]
    (7.99, 5.94);

\node[font=\bfseries\small, text=bookred, align=left] at (8.5, 4.6) {Pareto\\Frontier};

% Data Points: Small efficient (green)
\coordinate (p_mob) at (4.77, 0.98);
\coordinate (p_tny) at (5.13, 1.18);
\coordinate (p_dis) at (6.23, 1.67);

\draw[bookgreen, fill=white, line width=1.8pt] (p_mob) circle (4pt);
\node[left=4pt, font=\bfseries\scriptsize, text=bookgreen] at (p_mob) {MobileBERT};

\draw[bookgreen, fill=white, line width=1.8pt] (p_tny) circle (4pt);
\node[right=5pt, font=\bfseries\scriptsize, text=bookgreen] at (p_tny) {TinyBERT};

\draw[bookgreen, fill=white, line width=1.8pt] (p_dis) circle (4pt);
\node[right=5pt, font=\bfseries\scriptsize, text=bookgreen] at (p_dis) {DistilBERT};

% Data Points: Medium models (blue)
\coordinate (p_bba) at (7.26, 2.66);
\coordinate (p_rob) at (7.70, 3.09);
\coordinate (p_bla) at (7.44, 3.43);

\draw[bookblue, fill=white, line width=1.8pt] (p_bba) circle (4pt);
\node[right=5pt, font=\bfseries\scriptsize, text=bookblue] at (p_bba) {BERT-base};

\draw[bookblue, fill=white, line width=1.8pt] (p_rob) circle (4pt);
\node[right=5pt, font=\bfseries\scriptsize, text=bookblue] at (p_rob) {RoBERTa};

\draw[bookblue, fill=white, line width=1.8pt] (p_bla) circle (4pt);
\node[left=5pt, font=\bfseries\scriptsize, text=bookblue] at (p_bla) {BERT-large};

% Data Points: Large frontier (red)
\coordinate (p_gpt3) at (7.99, 5.94);
\coordinate (p_palm) at (8.36, 6.69);
\coordinate (p_gpt4) at (8.73, 7.07);

\draw[bookred, fill=white, line width=1.8pt] (p_gpt3) circle (4pt);
\node[right=5pt, font=\bfseries\scriptsize, text=bookred] at (p_gpt3) {GPT-3 175B};

\draw[bookred, fill=white, line width=1.8pt] (p_palm) circle (4pt);
\node[right=5pt, font=\bfseries\scriptsize, text=bookred] at (p_palm) {PaLM 540B};

\draw[bookred, fill=white, line width=1.8pt] (p_gpt4) circle (4pt);
\node[right=5pt, font=\bfseries\scriptsize, text=bookred] at (p_gpt4) {GPT-4 (est.)};

% Callout: 10x CO2 for +1% accuracy
\coordinate (pt_elbow) at (6.3, 5.4);
\node[draw=bordercolor, fill=white, rounded corners=2pt, font=\scriptsize, text=bodytext, inner sep=4pt]
    (callout) at (6.3, 4.4) {10$\times\ \text{CO}_2$ for +1\% accuracy};
\draw[mutedtext, dashed, line width=0.8pt] (callout.north) -- (pt_elbow) -- (7.98, 5.4);
...
\node[
    ...
    \begin{minipage}{3.8cm}
        \begin{tikzpicture}[x=1cm, y=0.45cm]
            \draw[bookgreen, fill=white, line width=1.4pt] (0.2, 2) circle (3pt);
            \node[right=5pt, font=\scriptsize, text=bodytext] at (0.3, 2) {Small efficient models};
            \draw[bookblue, fill=white, line width=1.4pt] (0.2, 1) circle (3pt);
            \node[right=5pt, font=\scriptsize, text=bodytext] at (0.3, 1) {Medium models};
            \draw[bookred, fill=white, line width=1.4pt] (0.2, 0) circle (3pt);
            \node[right=5pt, font=\scriptsize, text=bodytext] at (0.3, 0) {Large frontier models};
        \end{tikzpicture}
    \end{minipage}
};
...
\providecolor{mutedtext}{RGB}{100,110,125}
\providecolor{bodytext}{RGB}{40,45,55}
\providecolor{darkblue}{RGB}{25,60,110}

\begin{tikzpicture}[font=\small]

% Dimensions:
% Y: 0 to 1.0, yscale: 1.0 -> 6.0cm (ys = 6.0)
\def\ys{6.0}
\def\bw{0.45}

% Grid lines
\foreach \y in {0.25, 0.50, 0.75, 1.00} {
    \draw[gridline, line width=0.6pt] (-0.3, \y*\ys) -- (14.2, \y*\ys);
    \node[anchor=east, font=\footnotesize, text=mutedtext] at (-0.4, \y*\ys) {\y};
}
\node[anchor=east, font=\footnotesize, text=mutedtext] at (-0.4, 0) {0.00};

% Axes
\draw[bordercolor!90, line width=1pt] (-0.3, 0) -- (14.2, 0);
\draw[bordercolor!90, line width=1pt] (-0.3, 0) -- (-0.3, 1.05*\ys);

% Y axis label
\node[rotate=90, font=\bfseries\footnotesize, text=bodytext, anchor=south]
    at (-1.3, 0.5*\ys) {$\text{CO}_2\text{e}$ (Million kg)};

% Divider line between Meta models and Public baselines
\draw[mutedtext!60, dashed, line width=1pt] (6.5, 0) -- (6.5, 1.05*\ys);

% Group 1: Meta Production Models (x = 0.6, 1.6, 2.6, 3.6, 4.6, 5.6)
% LM
\coordinate (m_lm) at (0.6, 0);
\draw[draw=darkblue!80, pattern=north east lines, pattern color=darkblue!60, line width=0.7pt]
    ($(m_lm)+(-\bw/2, 0)$) rectangle ($(m_lm)+(\bw/2, 0.02*\ys)$);
\draw[draw=darkblue!80, fill=darkblue, line width=0.7pt]
    ($(m_lm)+(-\bw/2, 0.02*\ys)$) rectangle ($(m_lm)+(\bw/2, 0.12*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\footnotesize\bfseries, text=bodytext] at (m_lm) {LM};

% RM-1
\coordinate (m_rm1) at (1.6, 0);
\draw[draw=darkblue!80, pattern=north east lines, pattern color=darkblue!60, line width=0.7pt]
    ($(m_rm1)+(-\bw/2, 0)$) rectangle ($(m_rm1)+(\bw/2, 0.35*\ys)$);
\draw[draw=bookorange!90, fill=bookorange, line width=0.7pt]
    ($(m_rm1)+(-\bw/2, 0.35*\ys)$) rectangle ($(m_rm1)+(\bw/2, 0.39*\ys)$);
\draw[draw=darkblue!80, fill=darkblue, line width=0.7pt]
    ($(m_rm1)+(-\bw/2, 0.39*\ys)$) rectangle ($(m_rm1)+(\bw/2, 0.85*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\footnotesize\bfseries, text=bodytext] at (m_rm1) {RM-1};

% RM-2
\coordinate (m_rm2) at (2.6, 0);
\draw[draw=darkblue!80, pattern=north east lines, pattern color=darkblue!60, line width=0.7pt]
    ($(m_rm2)+(-\bw/2, 0)$) rectangle ($(m_rm2)+(\bw/2, 0.25*\ys)$);
\draw[draw=bookorange!90, fill=bookorange, line width=0.7pt]
    ($(m_rm2)+(-\bw/2, 0.25*\ys)$) rectangle ($(m_rm2)+(\bw/2, 0.28*\ys)$);
\draw[draw=darkblue!80, fill=darkblue, line width=0.7pt]
    ($(m_rm2)+(-\bw/2, 0.28*\ys)$) rectangle ($(m_rm2)+(\bw/2, 0.56*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\footnotesize\bfseries, text=bodytext] at (m_rm2) {RM-2};

% RM-3
\coordinate (m_rm3) at (3.6, 0);
\draw[draw=darkblue!80, pattern=north east lines, pattern color=darkblue!60, line width=0.7pt]
    ($(m_rm3)+(-\bw/2, 0)$) rectangle ($(m_rm3)+(\bw/2, 0.13*\ys)$);
\draw[draw=bookorange!90, fill=bookorange, line width=0.7pt]
    ($(m_rm3)+(-\bw/2, 0.13*\ys)$) rectangle ($(m_rm3)+(\bw/2, 0.15*\ys)$);
\draw[draw=darkblue!80, fill=darkblue, line width=0.7pt]
    ($(m_rm3)+(-\bw/2, 0.15*\ys)$) rectangle ($(m_rm3)+(\bw/2, 0.39*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\footnotesize\bfseries, text=bodytext] at (m_rm3) {RM-3};

% RM-4
\coordinate (m_rm4) at (4.6, 0);
\draw[draw=darkblue!80, pattern=north east lines, pattern color=darkblue!60, line width=0.7pt]
    ($(m_rm4)+(-\bw/2, 0)$) rectangle ($(m_rm4)+(\bw/2, 0.14*\ys)$);
\draw[draw=bookorange!90, fill=bookorange, line width=0.7pt]
    ($(m_rm4)+(-\bw/2, 0.14*\ys)$) rectangle ($(m_rm4)+(\bw/2, 0.16*\ys)$);
\draw[draw=darkblue!80, fill=darkblue, line width=0.7pt]
    ($(m_rm4)+(-\bw/2, 0.16*\ys)$) rectangle ($(m_rm4)+(\bw/2, 0.36*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\scriptsize\bfseries, text=bodytext] at (m_rm4) {RM-4};

% RM-5
\coordinate (m_rm5) at (5.6, 0);
\draw[draw=darkblue!80, pattern=north east lines, pattern color=darkblue!60, line width=0.7pt]
    ($(m_rm5)+(-\bw/2, 0)$) rectangle ($(m_rm5)+(\bw/2, 0.16*\ys)$);
\draw[draw=bookorange!90, fill=bookorange, line width=0.7pt]
    ($(m_rm5)+(-\bw/2, 0.16*\ys)$) rectangle ($(m_rm5)+(\bw/2, 0.18*\ys)$);
\draw[draw=darkblue!80, fill=darkblue, line width=0.7pt]
    ($(m_rm5)+(-\bw/2, 0.18*\ys)$) rectangle ($(m_rm5)+(\bw/2, 0.44*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\scriptsize\bfseries, text=bodytext] at (m_rm5) {RM-5};

% Group 2: Public Benchmark Models (x = 7.3, 8.3, 9.3, 10.3, 11.3, 12.3, 13.3)
% BERT-NAS
\coordinate (p_nas) at (7.3, 0);
\draw[draw=bookgreen!90, pattern=north east lines, pattern color=bookgreen!80, line width=0.7pt]
    ($(p_nas)+(-\bw/2, 0)$) rectangle ($(p_nas)+(\bw/2, 0.28*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\scriptsize\bfseries, text=bodytext] at (p_nas) {BERT-NAS};

% Evolved Transformer
\coordinate (p_et) at (8.3, 0);
\draw[draw=bookgreen!90, fill=bookgreen!30, line width=0.7pt]
    ($(p_et)+(-\bw/2, 0)$) rectangle ($(p_et)+(\bw/2, 0.008*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\scriptsize\bfseries, text=bodytext] at (p_et) {Evolved Transformer};

% T5
\coordinate (p_t5) at (9.3, 0);
\draw[draw=bookgreen!90, pattern=north east lines, pattern color=bookgreen!80, line width=0.7pt]
    ($(p_t5)+(-\bw/2, 0)$) rectangle ($(p_t5)+(\bw/2, 0.05*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\scriptsize\bfseries, text=bodytext] at (p_t5) {T5};

% Meena
\coordinate (p_mee) at (10.3, 0);
\draw[draw=bookgreen!90, pattern=north east lines, pattern color=bookgreen!80, line width=0.7pt]
    ($(p_mee)+(-\bw/2, 0)$) rectangle ($(p_mee)+(\bw/2, 0.09*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\scriptsize\bfseries, text=bodytext] at (p_mee) {Meena};

% GShard-600B
\coordinate (p_gsh) at (11.3, 0);
\draw[draw=bookgreen!90, fill=bookgreen!30, line width=0.7pt]
    ($(p_gsh)+(-\bw/2, 0)$) rectangle ($(p_gsh)+(\bw/2, 0.008*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\scriptsize\bfseries, text=bodytext] at (p_gsh) {GShard-600B};

% Switch Transformer
\coordinate (p_swt) at (12.3, 0);
\draw[draw=bookgreen!90, pattern=north east lines, pattern color=bookgreen!80, line width=0.7pt]
    ($(p_swt)+(-\bw/2, 0)$) rectangle ($(p_swt)+(\bw/2, 0.07*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\scriptsize\bfseries, text=bodytext] at (p_swt) {Switch Transformer};

% GPT-3
\coordinate (p_gp3) at (13.3, 0);
\draw[draw=bookgreen!90, pattern=north east lines, pattern color=bookgreen!80, line width=0.7pt]
    ($(p_gp3)+(-\bw/2, 0)$) rectangle ($(p_gp3)+(\bw/2, 0.55*\ys)$);
\node[below=4pt, rotate=45, anchor=north east, font=\scriptsize\bfseries, text=bodytext] at (p_gp3) {GPT-3};

% Legend (top)
\node[anchor=south] at (6.8, 1.07*\ys) {
    \begin{tikzpicture}[x=1cm, y=0.35cm]
        % Offline
        \draw[draw=darkblue!80, pattern=north east lines, pattern color=darkblue!60, line width=0.6pt] (0, 0) rectangle (0.35, 0.45);
        \node[right=2pt, font=\scriptsize, text=bodytext] at (0.35, 0.22) {Offline training};
        % Online
        \draw[draw=bookorange!90, fill=bookorange, line width=0.6pt] (3.0, 0) rectangle (3.35, 0.45);
        \node[right=2pt, font=\scriptsize, text=bodytext] at (3.35, 0.22) {Online training};
        % Inference
        \draw[draw=darkblue!80, fill=darkblue, line width=0.6pt] (5.9, 0) rectangle (6.25, 0.45);
        \node[right=2pt, font=\scriptsize, text=bodytext] at (6.25, 0.22) {Inference};
        % Public baseline
        \draw[draw=bookgreen!90, pattern=north east lines, pattern color=bookgreen!80, line width=0.6pt] (8.5, 0) rectangle (8.85, 0.45);
        \node[right=2pt, font=\scriptsize, text=bodytext] at (8.85, 0.22) {Public baseline (training-only)};
    \end{tikzpicture}
};
...
Scope 2 emissions\index{Scope 2 Emissions} represent indirect emissions from purchased electricity, steam, heat, and cooling consumed by the reporting organization. Purchased electricity is often the dominant category for an operator's owned facility, and it varies dramatically by geographic location and grid energy mix. Electricity used by a cloud provider is Scope 2 for that provider, but a customer's purchased cloud service is generally accounted for in the customer's Scope 3 value chain. The physical electrons do not change; the reporting boundary does. As established in @sec-sustainable-ai-scale-environmental-impact-ac9a, the operational grid-intensity lever sits in the 8 to 40 times range for representative region pairs, although hydroelectricity still has nonzero lifecycle emissions and local ecological impacts.
...
The **Greenhouse Gas (GHG) Protocol**\index{Greenhouse Gas Protocol!definition}[^fn-ghg-protocol-scope] framework [@ghgprotocol2023] provides the standard categorization for these emissions, summarized in @fig-ghg-protocol. Three scopes provide an engineering classification checklist:

[^fn-ghg-protocol-scope]: **Greenhouse Gas (GHG) Protocol**\index{Greenhouse Gas Protocol!definition}: Developed jointly by the World Resources Institute and World Business Council for Sustainable Development, this framework is used by over 90 percent of Fortune 500 companies reporting to CDP. Its three-scope taxonomy matters for ML systems because most AI carbon hides in Scope 3 (hardware manufacturing, cloud compute supply chains), which companies historically underreport by 50–70 percent compared to Scopes 1 and 2.

- Scope 1 (Direct Emissions): Arise from direct company operations---backup generators, company-owned power generation.
- Scope 2 (Indirect Energy Emissions): Purchased energy consumed by the reporting organization; a cloud operator's electricity appears here, whereas its customer's cloud purchase generally appears in the customer's Scope 3 inventory.
...
% Concentric circles drawn first
% Scope 3 (outermost)
\draw[darkblue!60, fill=darkblue!4, line width=1.5pt] (0,0) circle (5.4cm);
% Scope 2 (middle)
\draw[bookorange!70, fill=bookorange!8, line width=1.5pt] (0,0) circle (3.6cm);
% Scope 1 (innermost)
\draw[bookred!70, fill=bookred!8, line width=1.5pt] (0,0) circle (1.8cm);

% Scope 3 Title Pill Badge on top of circle
\node[
    fill=white,
    draw=darkblue!60,
    rounded corners=3pt,
    ...
    font=\bfseries\small,
    text=darkblue
] at (0, 5.4) {Scope 3 --- Value Chain Emissions};

% Scope 2 Title
\node[align=center, anchor=center] at (-2.4, 1.2) {
    {\bfseries\textcolor{bookorange}{Scope 2}}\\[-1pt]
    {\scriptsize\textcolor{bookorange!90}{Indirect Energy}}
};

% Scope 1 Title
\node[align=center, anchor=center] at (0, 1.15) {
    {\bfseries\textcolor{bookred}{Scope 1}}\\[-1pt]
    {\scriptsize\textcolor{bookred!90}{Direct Emissions}}
};

% Center Data Center Icon
\node[
    draw=bordercolor!90,
    fill=white,
    rounded corners=2pt,
    ...
    \begin{tikzpicture}[x=1cm, y=0.18cm]
        \draw[fill=bookblue!20, draw=bookblue!50] (0, 2) rectangle (0.8, 2.7);
        \draw[fill=bookgreen!20, draw=bookgreen!50] (0, 0.9) rectangle (0.8, 1.6);
        \draw[fill=bookorange!20, draw=bookorange!50] (0, -0.2) rectangle (0.8, 0.5);
    \end{tikzpicture}
};
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Infrastructure
from mlsysim.fmt import fmt, fmt_percent, fmt_qty, fmt_usd, check
...
class DistributionEfficiencySavings:
    ...
    rack_power_kw_str = fmt_qty(rack_power, unit=kW, precision=0, commas=False)
    gain_pct_str = fmt_percent(gain, precision=0, commas=False, style="prose")
    per_rack_saved_kw_str = fmt_qty(per_rack_saved, unit=kW, precision=2, commas=False)
    n_racks_str = fmt(n_racks, precision=0, commas=False)
    facility_saved_kw_str = fmt_qty(facility_saved, unit=kW, precision=0, commas=False)
...
@Tbl-rack-power itemizes the full rack budget so facility sizing includes host, network, conversion, and cooling overhead rather than only GPU TDP.
...
| **Cooling overhead (PUE ~`{python} RackPowerBudget.cooling_pue_str`)** |  `{python} RackPowerBudget.cooling_kw_str`   |   `{python} RackPowerBudget.cooling_pct_str`  |
...
ML training workloads impose a unique challenge on this power chain: **Synchronous Transients**\index{Synchronous Transients!definition}. In traditional web-serving data centers, thousands of servers handle independent requests with uncorrelated power draws. The aggregate load is smooth and predictable, varying by perhaps 10--20 percent over the course of a day.

In a training cluster, the picture is radically different. All accelerators execute the same computation in lockstep. When a large matrix multiplication begins, thousands of Tensor Cores across the cluster activate simultaneously, and power demand surges by 40--60 percent within microseconds. When the computation pauses for gradient synchronization, demand drops just as sharply. These **Power Ramps**\index{Power Ramps!definition} stress every component in the delivery chain, from the VRMs on the baseboard to the transformers in the substation.
...
**Diagnosis**: Synchronous matrix multiplication steps create a 300 kW load step within microseconds (600 W idle-to-peak swing per GPU), exceeding upstream power-conditioning response times. Resulting voltage sags trigger accelerator undervoltage protection and hard resets.

**Systems lesson**: Synchronous training steps create microsecond power ramps that stress data center power delivery. Adding local ride-through capacity (supercapacitor banks) smooths transient load steps until UPS and facility systems respond.
...
::: {.column-margin}
![](images/svg/vol2_sustainable_ai_margin_003.svg){width="100%" fig-alt="Two curves over time: a blue low-amplitude load line and a red accelerating power spike."}
...
from mlsysim.fmt import fmt, fmt_power, fmt_qty, check
...
class MegaClusterFacilityPower:
    ...
    cluster_gpus_str = fmt(cluster_gpus, precision=0, commas=True)
    gpu_tdp_w_str = fmt_power(gpu_tdp, unit=watt, precision=0, commas=False)
    accelerator_power_mw_str = fmt_qty(accelerator_power, unit=MW, precision=0, commas=False)
    it_power_mw_str = fmt_qty(it_power, unit=MW, precision=1, commas=False)
    facility_power_mw_str = fmt_qty(facility_power, unit=MW, precision=1, commas=False)
```
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_percent, fmt_qty, check
...
class WindFarmEquivalent:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    facility_power_mw_str = fmt_qty(facility_power, unit=MW, precision=0, commas=False)
    facility_energy_gwh_str = fmt_qty(facility_energy, unit=GWh, precision=0, commas=True)
    capacity_factor_pct_str = fmt_percent(capacity_factor, precision=0, commas=False, style="prose")
    wind_energy_gwh_str = fmt_qty(wind_energy, unit=GWh, precision=1, commas=True)
...
from mlsysim.fmt import fmt, fmt_int, fmt_magnitude, fmt_qty, fmt_specific_heat, check
...
class AirCoolingMassFlow:
    ...
    delta_t_k_str = fmt_qty(delta_t, unit=K, precision=0, commas=False)
    delta_t_k_value_str = fmt_magnitude(delta_t, unit=K, precision=0, commas=False)
    heat_load_kw_str = fmt_qty(heat_load, unit=kW, precision=0, commas=False)
    heat_load_w_value_str = fmt_magnitude(heat_load, unit=watt, precision=0, commas=True)
    air_density_str = fmt_qty(air_density, unit=kg / _m3, precision=1, commas=False)
    mass_flow_kg_per_s_str = fmt_qty(mass_flow, unit=kg / second, precision=1, commas=False)
    volumetric_flow_m3_per_s_str = fmt_qty(volumetric_flow, unit=_m3 / second, precision=1, commas=False)
...
Reducing PUE is a primary engineering objective for ML data centers because the cooling overhead is wasted energy that produces no useful computation. At the scale of a 10,000-GPU cluster consuming `{python} PueSavings.it_power_mw_str` of IT power after rack support load, the difference between PUE 1.5 and PUE 1.1 is `{python} PueSavings.diff_15_to_11_mw_str` of wasted power, costing approximately `{python} PueSavings.savings_15_to_11_musd_str` per year in electricity and requiring proportionally more cooling infrastructure to dissipate.

[^fn-pue-facility-metric]: **PUE Cluster Savings**\index{Power Usage Effectiveness!savings}: Google's fleet-wide PUE averages 1.10 compared to the industry average of ~1.58. For a 10,000-GPU cluster consuming `{python} PueSavings.it_power_mw_str` of IT power after rack support load, reducing PUE from 1.58 to 1.10 saves `{python} PueSavings.diff_158_to_110_mw_str` of cooling overhead---roughly `{python} PueSavings.savings_158_to_110_musd_str` per year in electricity and the equivalent of removing `{python} PueSavings.households_158_to_110_str` residential homes from the grid. ML-specific facilities with direct-to-chip liquid cooling have demonstrated PUE values of 1.03--1.08.
...
The architecture comparison in @fig-cooling-comparison shows the physical reason for that threshold: air moves heat through bulk airflow, while liquid moves heat through a direct thermal path from chip to coolant.
...
- **IT Power**\index{IT Power!definition}: `{python} H100TdpCoolingRecap.n_gpus_str` $\times$ `{python} H100TdpCoolingRecap.tdp_w_str` = `{python} H100TdpCoolingRecap.it_power_kw_str`
...
% Panel A (Left) - Forced-Air Convection
\node[panel, anchor=north west, minimum width=7.4cm, minimum height=9.0cm] (panelA) at (0, 0) {};
...
\node[
    ...
    anchor=north west,
    minimum width=7.1cm
] (hdrA) at ($(panelA.north west)+(0.15, -0.15)$) {
    \begin{minipage}{7.0cm}
        {\bfseries\scriptsize\textcolor{bookred}{A $\cdot$ FORCED-AIR CONVECTION (LEGACY)}} \hfill
        {\tiny\textcolor{bookred!80}{Limited density $< 35$~kW}}
    \end{minipage}
};
...
\node[
    ...
    anchor=north
] (rackA) at ($(hdrA.south)+(0.5, -0.4)$) {};
\node[font=\bfseries\tiny, text=bodytext, anchor=north] at ($(rackA.north)+(0, -3pt)$) {42U Air-Cooled Rack};

\foreach \i in {1, 2, 3, 4, 5, 6} {
    \node[nodeitem] at ($(rackA.north)+(0, -0.28 - \i*0.44)$) {GPU Server Node \i};
}
\node[
    draw=mutedtext!60,
    fill=white,
    ...
    font=\tiny,
    text=mutedtext
] at ($(rackA.north)+(0, -3.55)$) {Chassis Fan Bank};

% Arrows into Rack A (Cold air)
\draw[-{Stealth[length=5pt]}, bookblue, line width=1.4pt] ($(rackA.west)+(-1.3, 0.4)$) -- ($(rackA.west)+(-0.2, 0.4)$);
\draw[-{Stealth[length=5pt]}, bookblue, line width=1.4pt] ($(rackA.west)+(-1.3, -0.2)$) -- ($(rackA.west)+(-0.2, -0.2)$);
\node[bookblue, font=\bfseries\tiny] at ($(rackA.west)+(-0.75, 0.75)$) {Cold Air};
\node[bookblue, font=\tiny] at ($(rackA.west)+(-0.75, -0.55)$) {$20^\circ\text{C}$};

% Arrows out of Rack A (Hot air)
\draw[-{Stealth[length=5pt]}, bookred, line width=1.4pt] ($(rackA.east)+(0.2, 0.4)$) -- ($(rackA.east)+(1.1, 0.4)$);
\draw[-{Stealth[length=5pt]}, bookred, line width=1.4pt] ($(rackA.east)+(0.2, -0.2)$) -- ($(rackA.east)+(1.1, -0.2)$);
\node[bookred, font=\bfseries\tiny] at ($(rackA.east)+(0.65, 0.75)$) {Hot Air};
\node[bookred, font=\tiny] at ($(rackA.east)+(0.65, -0.55)$) {$38$--$45^\circ\text{C}$};
...
\node[
    ...
    anchor=south,
    minimum width=7.0cm
] (metA) at ($(panelA.south)+(0, 0.18)$) {
    \begin{minipage}{6.7cm}
        {\bfseries\tiny\textcolor{bookred}{AIR CONVECTION PERFORMANCE METRICS}}\\[3pt]
        {\tiny $\bullet$ Power Density: 15--30~kW / rack max}\\[1pt]
        {\tiny $\bullet$ Facility PUE: 1.40--1.80 (High chill overhead)}\\[1pt]
        {\tiny $\bullet$ Fan Power Draw: 30--40\% of IT compute load}\\[2pt]
        {\tiny\bfseries\textcolor{bookred}{Thermal Choking: Fails above $\sim$35~kW/rack}}\\[1pt]
        {\tiny\textcolor{bookred!80}{Fan power scales cubically with required airflow}}
    \end{minipage}
};

% Panel B (Right) - Direct Liquid Cooling
\node[panel, anchor=north west, minimum width=7.4cm, minimum height=9.0cm] (panelB) at (7.9, 0) {};
...
\node[
    ...
    anchor=north west,
    minimum width=7.1cm
] (hdrB) at ($(panelB.north west)+(0.15, -0.15)$) {
    \begin{minipage}{7.0cm}
        {\bfseries\scriptsize\textcolor{bookgreen!90!black}{B $\cdot$ DIRECT-TO-CHIP LIQUID COOLING}} \hfill
        {\tiny\textcolor{bookgreen!80!black}{High density 100+ kW}}
    \end{minipage}
};

% Rack B (Liquid-cooled)
\node[
    draw=bookgreen,
    fill=cardbg,
    rounded corners=3pt,
    ...
    minimum height=4.1cm,
    anchor=north
] (rackB) at ($(hdrB.south)+(-0.6, -0.4)$) {};
\node[font=\bfseries\tiny, text=bookgreen!90!black, anchor=north] at ($(rackB.north)+(0, -3pt)$) {High-Density Rack};

\foreach \i in {1, 2, 3, 4, 5, 6} {
    \node[nodeitem, minimum width=2.5cm, align=left] (gn\i) at ($(rackB.north)+(0, -0.28 - \i*0.44)$) {\hspace{-12pt}GPU Node \i};
    \filldraw[darkblue, rounded corners=1pt] ($(gn\i.east)+(-0.4, -0.12)$) rectangle ($(gn\i.east)+(-0.1, 0.12)$);
}
\node[
    draw=bookgreen,
    fill=bookgreen!10,
    ...
    minimum height=0.4cm,
    font=\bfseries\tiny,
    text=bookgreen!90!black
] at ($(rackB.north)+(0, -3.55)$) {CDU In-Rack Manifold};

% Cool in / Warm out arrows
\draw[-{Stealth[length=5pt]}, darkblue, line width=1.5pt] ($(rackB.west)+(-1.0, 0.5)$) -- ($(rackB.west)+(-0.15, 0.5)$);
\node[darkblue, font=\bfseries\tiny] at ($(rackB.west)+(-0.6, 0.8)$) {Cool In};
\node[darkblue, font=\tiny] at ($(rackB.west)+(-0.6, 0.25)$) {$18^\circ\text{C}$};

\draw[{Stealth[length=5pt]}-, bookred, line width=1.5pt] ($(rackB.west)+(-1.0, -0.5)$) -- ($(rackB.west)+(-0.15, -0.5)$);
\node[bookred, font=\bfseries\tiny] at ($(rackB.west)+(-0.6, -0.8)$) {Warm Out};
\node[bookred, font=\tiny] at ($(rackB.west)+(-0.6, -1.05)$) {$45^\circ\text{C}$};
...
\node[
    ...
    inner sep=5pt,
    anchor=west
] (hx) at ($(rackB.east)+(0.6, 0)$) {
    \begin{minipage}{1.8cm}
        \centering
        {\bfseries\tiny\textcolor{bookgreen!90!black}{HEAT EXCHANGER}}\\[2pt]
        {\color{bordercolor}\rule{\linewidth}{0.4pt}}\\[2pt]
        {\tiny\textcolor{mutedtext}{Waste Heat Reuse}}\\[1pt]
        {\tiny\textcolor{bookgreen!80!black}{District Heating}}\\[1pt]
        {\tiny\textcolor{bookgreen!80!black}{Dry Coolers}}\\[1pt]
        {\tiny\textcolor{mutedtext}{Zero water evapor.}}
    \end{minipage}
};
\draw[-{Stealth[length=5pt]}, bookgreen, line width=1.4pt] (rackB.east) -- (hx.west);
...
\node[
    ...
    anchor=south,
    minimum width=7.0cm
] (metB) at ($(panelB.south)+(0, 0.25)$) {
    \begin{minipage}{6.7cm}
        {\bfseries\tiny\textcolor{bookgreen!90!black}{DIRECT LIQUID COOLING PERFORMANCE METRICS}}\\[3pt]
        {\tiny $\bullet$ Power Density: 100--250+~kW / rack}\\[1pt]
        {\tiny $\bullet$ Facility PUE: 1.05--1.10 (Ultra-low overhead)}\\[1pt]
        {\tiny $\bullet$ Pump Power Draw: $<5$\% of IT compute load}\\[2pt]
        {\tiny\bfseries\textcolor{bookgreen!90!black}{Mandatory for Modern Accelerators (H100/B200)}}\\[1pt]
        {\tiny\textcolor{bookgreen!80!black}{Warm water return allows direct thermal reuse}}
    \end{minipage}
};
...
\node[
    ...
    inner sep=7pt,
    anchor=north west,
    minimum width=15.3cm
] at (0, -8.7) {
    \begin{minipage}{14.9cm}
        {\bfseries\footnotesize\textcolor{bookblue}{THERMODYNAMIC DENSITY LIMIT PRINCIPLE}}\\[3pt]
        {\footnotesize\textcolor{bodytext}{Above 40~kW/rack, fan power requirements scale cubically with volumetric air velocity, creating an air-cooling wall. Direct liquid cooling (D2C) reduces thermal resistance by $1000\times$, slashing cooling overhead from 40\% to $<5\%$ of IT power while enabling 100+~kW rack clusters.}}
    \end{minipage}
};
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Systems, Infrastructure
from mlsysim.fmt import fmt, fmt_int, fmt_usd, fmt_time, check
...
\providecolor{bodytext}{RGB}{40,45,55}
\providecolor{darkblue}{RGB}{25,60,110}
\providecolor{graybar}{RGB}{145,160,180}
\providecolor{bluebar}{RGB}{95,115,245}
...
\node[
    ...
    inner sep=6pt,
    anchor=north west,
    minimum width=15.6cm
] (topbar) at (0, 0) {
    \begin{minipage}{15.2cm}
        \textbf{\footnotesize\textcolor{darkblue}{HARDWARE MARKET TRAJECTORY}}
        \hfill
        {\tiny\textbf{\textcolor{mutedtext}{\rule{8pt}{6pt}\hspace{3pt}2017 Baseline}}} \quad
        {\tiny\textbf{\textcolor{darkblue}{\rule{8pt}{6pt}\hspace{3pt}2025 Projected}}} \quad
        {\tiny\textbf{\textcolor{bookgreen!90!black}{- - - Growth Delta}}}
    \end{minipage}
};

% Panel 1: Data Center Total Market (Left)
\node[
    draw=bordercolor,
    fill=white,
    rounded corners=4pt,
...
\node[
    ...
    inner sep=4pt,
    anchor=north west
] at ($(panel1.north west)+(0.3, -0.3)$) {Data Center Total Market (\$ Billion)};

% Y axis grid & ticks for Panel 1
\def\yA{0.375} % 12B -> 4.5cm
\coordinate (o1) at ($(panel1.south west)+(1.0, 1.8)$);

\foreach \v/\lbl in {0/\$0B, 3/\$3B, 6/\$6B, 9/\$9B, 12/\$12B} {
    \draw[gridline, line width=0.5pt] ($(o1)+(-0.2, \v*\yA)$) -- ($(o1)+(6.0, \v*\yA)$);
    \node[anchor=east, font=\tiny, text=mutedtext] at ($(o1)+(-0.3, \v*\yA)$) {\lbl};
}
\draw[bordercolor!80, line width=0.8pt] ($(o1)+(-0.2, 0)$) -- ($(o1)+(6.0, 0)$);

% Data Center Bars
\def\bw{0.6}
% Inference 2017: 4.5B (gray)
\coordinate (dc_i17) at ($(o1)+(1.0, 0)$);
\draw[fill=graybar, draw=none, rounded corners=2pt]
    ($(dc_i17)+(-\bw/2, 0)$) rectangle ($(dc_i17)+(\bw/2, 4.5*\yA)$);
\node[above=1pt, font=\bfseries\tiny, text=bodytext] (lbl_45) at ($(dc_i17)+(0, 4.5*\yA)$) {\$4.5B};
\node[below=2pt, font=\tiny, text=mutedtext] at (dc_i17) {2017};

% Inference 2025: 9.5B (darkblue)
\coordinate (dc_i25) at ($(o1)+(2.0, 0)$);
\draw[fill=darkblue, draw=none, rounded corners=2pt]
    ($(dc_i25)+(-\bw/2, 0)$) rectangle ($(dc_i25)+(\bw/2, 9.5*\yA)$);
\node[above=2pt, font=\bfseries\tiny, text=darkblue] at ($(dc_i25)+(0, 9.5*\yA)$) {\$9.5B};
\node[below=2pt, font=\bfseries\tiny, text=darkblue] at (dc_i25) {2025*};

% Delta bracket (+111%)
\draw[bookgreen, dashed, line width=0.9pt]
    (lbl_45.north) -- ($(dc_i17)+(0, 9.5*\yA)$) -- ($(dc_i25)+(0, 9.5*\yA)$);
\node[fill=bookgreen!15, draw=bookgreen!60, rounded corners=2pt, font=\tiny\bfseries, text=bookgreen!90!black, inner sep=2pt]
    at ($(dc_i17)+(0.4, 7.3*\yA)$) {+111\%};

\node[below=14pt, font=\bfseries\footnotesize, text=bodytext] (lbl_inf1) at ($(dc_i17)!0.5!(dc_i25)$) {INFERENCE};

% Training 2017: 0.8B (gray)
\coordinate (dc_t17) at ($(o1)+(4.0, 0)$);
\draw[fill=graybar, draw=none, rounded corners=2pt]
    ($(dc_t17)+(-\bw/2, 0)$) rectangle ($(dc_t17)+(\bw/2, 0.8*\yA)$);
\node[above=2pt, font=\bfseries\tiny, text=mutedtext] at ($(dc_t17)+(0, 0.8*\yA)$) {$<$~\$1B};
\node[below=2pt, font=\tiny, text=mutedtext] at (dc_t17) {2017};

% Training 2025: 4.5B (bluebar)
\coordinate (dc_t25) at ($(o1)+(5.0, 0)$);
\draw[fill=bluebar, draw=none, rounded corners=2pt]
    ($(dc_t25)+(-\bw/2, 0)$) rectangle ($(dc_t25)+(\bw/2, 4.5*\yA)$);
\node[above=2pt, font=\bfseries\tiny, text=bluebar!90!black] at ($(dc_t25)+(0, 4.5*\yA)$) {\$4.5B};
\node[below=2pt, font=\bfseries\tiny, text=bluebar!90!black] at (dc_t25) {2025*};

\node[below=14pt, font=\bfseries\footnotesize, text=bodytext] (lbl_trn1) at ($(dc_t17)!0.5!(dc_t25)$) {TRAINING};

% Panel 1 subtext
\node[font=\tiny\itshape, text=mutedtext, below=12pt] at ($(lbl_inf1)!0.5!(lbl_trn1)$) {
    Data Center inference reaches $2\times$ data center training volume
};

% Panel 2: Edge Total Market (Right)
\node[
    draw=bordercolor,
    fill=white,
    rounded corners=4pt,
...
\node[
    ...
    inner sep=4pt,
    anchor=north west
] at ($(panel2.north west)+(0.3, -0.3)$) {Edge Total Market (\$ Billion)};

% Y axis grid & ticks for Panel 2
\def\yB{0.75} % 6.0B -> 4.5cm
\coordinate (o2) at ($(panel2.south west)+(1.0, 1.8)$);

\foreach \v/\lbl in {0.0/\$0.0B, 1.5/\$1.5B, 3.0/\$3.0B, 4.5/\$4.5B, 6.0/\$6.0B} {
    \draw[gridline, line width=0.5pt] ($(o2)+(-0.2, \v*\yB)$) -- ($(o2)+(6.0, \v*\yB)$);
    \node[anchor=east, font=\tiny, text=mutedtext] at ($(o2)+(-0.3, \v*\yB)$) {\lbl};
}
\draw[bordercolor!80, line width=0.8pt] ($(o2)+(-0.2, 0)$) -- ($(o2)+(6.0, 0)$);

% Edge Bars
% Inference 2017: 0.1B (gray)
\coordinate (ed_i17) at ($(o2)+(1.0, 0)$);
\draw[fill=graybar, draw=none, rounded corners=1pt]
    ($(ed_i17)+(-\bw/2, 0)$) rectangle ($(ed_i17)+(\bw/2, 0.08*\yB)$);
\node[above=2pt, font=\bfseries\tiny, text=mutedtext] at ($(ed_i17)+(0, 0.08*\yB)$) {$<$~\$0.1B};
\node[below=2pt, font=\tiny, text=mutedtext] at (ed_i17) {2017};

% Inference 2025: 4.25B (green)
\coordinate (ed_i25) at ($(o2)+(2.0, 0)$);
\draw[fill=bookgreen, draw=none, rounded corners=2pt]
    ($(ed_i25)+(-\bw/2, 0)$) rectangle ($(ed_i25)+(\bw/2, 4.25*\yB)$);
\node[above=2pt, font=\bfseries\tiny, text=bookgreen!90!black] at ($(ed_i25)+(0, 4.25*\yB)$) {\$4.25B};
\node[below=2pt, font=\bfseries\tiny, text=bookgreen!90!black] at (ed_i25) {2025*};

\node[fill=bookgreen!10, draw=bookgreen!60, rounded corners=2pt, font=\tiny\bfseries, text=bookgreen!90!black, inner sep=2pt]
    at ($(ed_i25)+(0, 4.9*\yB)$) {NEW SEGMENT ($\sim$40$\times$)};

\node[below=14pt, font=\bfseries\footnotesize, text=bodytext] (lbl_inf2) at ($(ed_i17)!0.5!(ed_i25)$) {INFERENCE};

% Training 2017: 0.1B (gray)
\coordinate (ed_t17) at ($(o2)+(4.0, 0)$);
\draw[fill=graybar, draw=none, rounded corners=1pt]
    ($(ed_t17)+(-\bw/2, 0)$) rectangle ($(ed_t17)+(\bw/2, 0.08*\yB)$);
\node[above=2pt, font=\bfseries\tiny, text=mutedtext] at ($(ed_t17)+(0, 0.08*\yB)$) {$<$~\$0.1B};
\node[below=2pt, font=\tiny, text=mutedtext] at (ed_t17) {2017};

% Training 2025: 1.25B (orange)
\coordinate (ed_t25) at ($(o2)+(5.0, 0)$);
\draw[fill=bookorange, draw=none, rounded corners=2pt]
    ($(ed_t25)+(-\bw/2, 0)$) rectangle ($(ed_t25)+(\bw/2, 1.25*\yB)$);
\node[above=2pt, font=\bfseries\tiny, text=bookorange!90!black] at ($(ed_t25)+(0, 1.25*\yB)$) {\$1.25B};
\node[below=2pt, font=\bfseries\tiny, text=bookorange!90!black] at (ed_t25) {2025*};

\node[below=14pt, font=\bfseries\footnotesize, text=bodytext] (lbl_trn2) at ($(ed_t17)!0.5!(ed_t25)$) {TRAINING};

% Panel 2 subtext
\node[font=\tiny\itshape, text=mutedtext, below=12pt] at ($(lbl_inf2)!0.5!(lbl_trn2)$) {
...
\node[
    ...
    \begin{minipage}{15.2cm}
        \textbf{\scriptsize\textcolor{darkblue}{SYSTEMS TAKEAWAYS}}\\[4pt]
        {\footnotesize\textbf{1. Inference Inversion:}\quad While training hardware dominates early capex headlines, operational inference market volume overtakes training by $>2\times$.}\\[2pt]
        {\footnotesize\textbf{2. Edge Growth Dominance:}\quad Edge inference expands from negligible market share in 2017 to rival the entire cloud training market by 2025.}
    \end{minipage}
};
...
```{=latex}
\providecolor{MainBlue}{HTML}{1E3A8A}
\providecolor{LightBlue}{HTML}{EFF6FF}
\providecolor{BorderBlue}{HTML}{93C5FD}
\providecolor{MainRed}{HTML}{B91C1C}
\providecolor{LightRed}{HTML}{FEF2F2}
\providecolor{BorderRed}{HTML}{FCA5A5}
\providecolor{MainGreen}{HTML}{15803D}
\providecolor{LightGreen}{HTML}{F0FDF4}
\providecolor{BorderGreen}{HTML}{86EFAC}
\providecolor{CardBorder}{HTML}{CBD5E1}
\providecolor{CardBg}{HTML}{F8FAFC}
\providecolor{DarkSlate}{HTML}{0F172A}
\providecolor{MutedSlate}{HTML}{475569}

\begin{tikzpicture}[
    font=\small,
    box/.style={rounded corners=4pt, align=center, inner sep=6pt},
    headerbox/.style={rounded corners=5pt, line width=1pt, align=center, inner sep=7pt},
    examplebox/.style={rounded corners=4pt, line width=0.8pt, align=center, inner sep=6pt}
]
    ...
    % ================= LEFT COLUMN: PREFILL =================
    % Header
    \node[headerbox, draw=MainBlue, fill=LightBlue!40, text width=\colw] (h_l) at (\xL, 0) {
        {\bfseries\large\color{MainBlue} Prefill Phase}\\[3pt]
        {\scriptsize\bfseries\color{MainBlue!80!black} COMPUTE-BOUND $\cdot$ HIGH ARITHMETIC INTENSITY}
    };

    % GPU Utilization Bar
    \node[below=14pt of h_l, minimum width=6.8cm, minimum height=0.65cm] (util_box_l) {};
    \draw[rounded corners=3pt, CardBorder, fill=LightBlue!30] (\xL-3.4, -1.35) rectangle (\xL+3.4, -0.7);
    \fill[MainBlue, rounded corners=3pt] (\xL-3.4, -1.35) rectangle (\xL+2.72, -0.7);
    \node[white, font=\bfseries\footnotesize] at (\xL-0.34, -1.025) {$\sim$90\% Active Compute};

    \node[font=\footnotesize\bfseries\color{MutedSlate}] at (\xL, -1.65) {GPU Utilization};

    % Energy per Token (Relative)
    \node[font=\footnotesize\bfseries\color{DarkSlate}] at (\xL, -2.2) {Energy per Token (Relative)};

    % Compute energy dominant bar
    \draw[rounded corners=3pt, draw=MainBlue, fill=LightBlue, line width=0.8pt] (\xL-3.4, -3.1) rectangle (\xL+3.4, -2.5);
    \node[font=\footnotesize\bfseries\color{MainBlue}] at (\xL, -2.8) {Compute Energy (Dominant)};

    % Memory overhead small box
    \draw[rounded corners=2pt, draw=BorderRed, fill=LightRed, line width=0.8pt] (\xL-3.4, -3.7) rectangle (\xL-2.5, -3.25);
    \node[anchor=west, font=\footnotesize\color{MutedSlate}] at (\xL-2.35, -3.475) {Memory overhead};

    % Arithmetic Intensity Card
    \node[box, draw=BorderGreen, fill=LightGreen, line width=0.8pt, text width=\colw] (card_ai_l) at (\xL, -4.85) {
        {\bfseries\color{MainGreen} High Arithmetic Intensity}\\[3pt]
        {\footnotesize\color{DarkSlate} Thousands of MACs per byte read\\ from HBM $\to$ near-peak pJ/FLOP efficiency}
    };

    % Energy / Token Badge
    \node[box, fill=MainBlue, text=white, text width=\colw] (badge_l) at (\xL, -6.35) {
        {\bfseries Energy / Token: LOW}\\[2pt]
        {\footnotesize $\sim$1$\times$ baseline reference (highly amortized)}
    };

    % Example Box
    \node[examplebox, draw=CardBorder, fill=LightBlue!20, text width=\colw] (ex_l) at (\xL, -8.05) {
        {\bfseries\footnotesize\color{DarkSlate} Example: Processing prompt tokens}\\[3pt]
        {\footnotesize\color{MutedSlate} All prompt tokens processed in parallel batch pass\\
        Full weight matrix reused across entire prompt length\\[2pt]
        \color{MainGreen}\bfseries $\to$ Tensor cores fully saturated}
    };

    % ================= RIGHT COLUMN: DECODE =================
    % Header
    \node[headerbox, draw=MainRed, fill=LightRed!40, text width=\colw] (h_r) at (\xR, 0) {
        {\bfseries\large\color{MainRed} Decode Phase}\\[3pt]
        {\scriptsize\bfseries\color{MainRed!80!black} BANDWIDTH-BOUND $\cdot$ LOW ARITHMETIC INTENSITY}
    };

    % GPU Utilization Bar
    \draw[rounded corners=3pt, CardBorder, fill=LightRed] (\xR-3.4, -1.35) rectangle (\xR+3.4, -0.7);
    \fill[MainBlue, rounded corners=3pt] (\xR-3.4, -1.35) rectangle (\xR-2.72, -0.7);
    \node[MainRed, font=\bfseries\footnotesize] at (\xR+0.34, -1.025) {$\sim$90\% Idle (waiting on HBM transfers)};

    \node[font=\footnotesize\bfseries\color{MutedSlate}] at (\xR, -1.65) {GPU Utilization};

    % Energy per Token (Relative)
    \node[font=\footnotesize\bfseries\color{DarkSlate}] at (\xR, -2.2) {Energy per Token (Relative)};

    % Static leakage dominant bar
    \draw[rounded corners=3pt, draw=MainRed, fill=LightRed, line width=0.8pt] (\xR-3.4, -3.1) rectangle (\xR+3.4, -2.5);
    \node[font=\footnotesize\bfseries\color{MainRed}] at (\xR, -2.8) {Static Leakage \& Clock Power (Wasted)};

    % Useful compute small box
    \draw[rounded corners=2pt, draw=BorderBlue, fill=LightBlue, line width=0.8pt] (\xR-3.4, -3.7) rectangle (\xR-2.5, -3.25);
    \node[anchor=west, font=\footnotesize\color{MutedSlate}] at (\xR-2.35, -3.475) {Useful compute};

    % Arithmetic Intensity Card
    \node[box, draw=BorderRed, fill=LightRed, line width=0.8pt, text width=\colw] (card_ai_r) at (\xR, -4.85) {
        {\bfseries\color{MainRed} Low Arithmetic Intensity}\\[3pt]
        {\footnotesize\color{DarkSlate} Entire model weights re-read from HBM\\ to generate a single auto-regressive token}
    };

    % Energy / Token Badge
    \node[box, fill=MainRed, text=white, text width=\colw] (badge_r) at (\xR, -6.35) {
        {\bfseries Energy / Token: HIGH}\\[2pt]
        {\footnotesize 10--50$\times$ prefill energy (memory-bound)}
    };

    % Example Box
    \node[examplebox, draw=CardBorder, fill=LightBlue!20, text width=\colw] (ex_r) at (\xR, -8.05) {
        {\bfseries\footnotesize\color{DarkSlate} Example: Generating output tokens}\\[3pt]
        {\footnotesize\color{MutedSlate} 1,000 generated tokens = 1,000 sequential passes\\
        Each token incurs full weight loading bandwidth\\[2pt]
        \color{MainRed}\bfseries $\to$ Compute units stalled waiting for memory}
    };

    % Central divider
    \draw[dashed, CardBorder, line width=1pt] (0, 0.6) -- (0, -9.1);

\end{tikzpicture}
```
:::

The prefill/decode distinction summarized in @fig-prefill-decode-energy extends beyond latency into energy efficiency. Recent analysis [@ma2026challenges] reveals that autoregressive generation is inherently energy-wasteful compared to batch processing because the two phases stress different hardware limits. During prefill\index{Inference!prefill phase}, high arithmetic intensity allows the GPU to perform thousands of operations for every byte read from memory, achieving near-peak energy efficiency in pJ/FLOP. During decode\index{Inference!decode phase}, the model must read the entire weight set from HBM to generate a single token; arithmetic intensity is low, so the compute units sit idle for much of the cycle.
...
**Problem**: Calculate the operating lifetime of an anomaly-detection TinyML autoencoder deployed on a factory sensor node running on two AA batteries (3000 mAh at 3.0V).

Variables:

- MCU: ARM Cortex-M4 at 80 MHz
- Active power: 12 mW during 5 ms inference at 10 Hz (5 percent duty cycle)
- Sleep power: 16.5 $\mu\text{W}$ (5 $\mu\text{A}$ at 3.3V) during 95 ms sleep
...
1.  **Phone Battery**\index{Phone Battery!definition}: Typical capacity is approximately `{python} OnDeviceLearningEnergy.phone_battery_wh_str`, or about `{python} OnDeviceLearningEnergy.phone_battery_j_str`.
...
**Systems insight**: Full fine-tuning is impossible within a reasonable daily battery budget. Sustainable on-device learning requires parameter-efficient fine-tuning\index{Parameter-Efficient Fine-Tuning} or sparse updates to reduce the energy cost per token by 100$\times$ or more.
...
Beyond individual device efficiency, architectural patterns determine total system energy consumption across edge-cloud boundaries. A cascade architecture deploys a small edge model (under 100 KB) to filter inputs before cloud inference. @Eq-cascade-energy expresses total energy as the sum of local processing plus probabilistically-triggered cloud costs:
...
class WakeWordPower:
    ...
    # ┌── 2. EXECUTE ──────────────────────────────────────────────────────────
    avg_power = vad_power + speech_activity * wake_detector_power + wake_occurrence * full_model_power * full_model_duty_s
    ...
    avg_power_math = fmt_display_math(
        ...
        f"{wake_occurrence:g} \\times {_full_mw:g} \\times {full_model_duty_s:g} = {_avg_power_mw_str}"
    )
```
...
```{=latex}
\providecolor{color1}{RGB}{0,102,204}    % primary blue
\providecolor{color2}{RGB}{22,101,52}    % green
\providecolor{color3}{RGB}{217,119,6}    % amber
\providecolor{color4}{RGB}{220,38,38}    % red
\providecolor{darknavy}{RGB}{20,40,80}
\providecolor{MutedGray}{HTML}{64748B}
\providecolor{LightBlue}{HTML}{F0F7FF}
\providecolor{LightGreen}{HTML}{F0FDF4}
\providecolor{LightAmber}{HTML}{FFFBEB}
\providecolor{LightRed}{HTML}{FEF2F2}
...
\begin{tikzpicture}[
    font=\small,
    >=Stealth,
    flowbox/.style={draw, rounded corners=3pt, line width=1pt, align=center, inner sep=6pt},
    banner/.style={draw=BorderGray, rounded corners=3pt, fill=white, inner sep=6pt, align=center}
]

    % Subtitle at top
    \node[font=\itshape\color{MutedGray}] at (0, 4.55) {Semiconductor Fab Water Cycle --- from Intake to Reclaim};

    % --- Main flow boxes ---
    % Box 1: Raw Water Intake
    \node[flowbox, draw=color1, fill=LightBlue, text width=2.6cm, minimum height=2.1cm] (raw) at (-5.4, 2.3) {
        {\bfseries\color{darknavy} Raw Water\\[1pt] Intake}\\[4pt]
        {\footnotesize\color{MutedGray} River / municipal\\ supply source}
    };

    % Box 2: UPW Treatment
    \node[flowbox, draw=color1, fill=LightBlue, text width=2.6cm, minimum height=2.1cm] (upw) at (-1.8, 2.3) {
        {\bfseries\color{darknavy} Ultra-Pure Water\\[1pt] (UPW) Treatment}\\[4pt]
        {\footnotesize\color{MutedGray} Deionization\\ Filtration $\cdot$ UV sterilization\\ ppb purity required}
    };

    % Box 3: Fab Processing
    \node[flowbox, draw=color3, fill=LightAmber, text width=2.6cm, minimum height=2.1cm] (fab) at (1.8, 2.3) {
        {\bfseries\color{color3!90!black} Fab Processing}\\[3pt]
        {\footnotesize\color{color3!80!black} Wafer cleaning\\ Chemical-mechanical\\ planarization (CMP)\\ Chemical bath rinses}
    };

    % Box 4: Lost
    \node[flowbox, draw=color4, fill=LightRed, text width=1.6cm, minimum height=2.1cm] (lost) at (4.8, 2.3) {
        {\bfseries\color{color4} Lost}\\[5pt]
        {\footnotesize\color{color4} 20--40\%\\ evaporation}
    };

    % Flow arrows top row
    \draw[->, line width=1.2pt, color1, shorten >=2pt, shorten <=2pt] (raw) -- (upw);
    \draw[->, line width=1.2pt, color1, shorten >=2pt, shorten <=2pt] (upw) -- (fab);
    \draw[->, dashed, line width=1.2pt, color4, shorten >=2pt, shorten <=2pt] (fab) -- (lost);

    % --- Wastewater Treatment ---
    \node[flowbox, draw=color2, fill=LightGreen, text width=3.8cm, minimum height=2.1cm] (waste) at (1.8, -0.6) {
        \vspace{2pt}
        {\bfseries\color{color2} Wastewater Treatment}\\[3pt]
        {\footnotesize\color{color2!90!black} Chemical neutralization\\ Filtration + settling\\ Quality testing before reuse\\ Contaminant removal}
    };

    % Arrow from Fab to Wastewater
    \draw[->, line width=1.2pt, color2, shorten >=2pt, shorten <=2pt] (fab.south) -- (waste.north);

    % --- Recycle Loop ---
    \node[flowbox, draw=color2, fill=LightGreen!40, text width=2.6cm, minimum height=0.9cm] (reclaim) at (-2.0, -0.6) {
        {\bfseries\footnotesize\color{color2} 60--80\% recycled /}\\[1pt]
        {\footnotesize\color{color2} reclaimed on-site}
    };

    \draw[line width=1.2pt, color2] (waste.west) -- (reclaim.east);
    \draw[line width=1.2pt, color2] (reclaim.west) -- (-7.2, -0.6) -- (-7.2, 0.7) -- (-5.4, 0.7);
    \draw[->, line width=1.2pt, color2, shorten >=2pt] (-5.4, 0.7) -- (raw.south);

    % --- Callout cards at bottom ---
    % Benchmark card
    \node[flowbox, draw=BorderGray, fill=white, text width=6.6cm, minimum height=1.4cm] (bench) at (-3.4, -2.8) {
        {\bfseries\color{darknavy} Industry benchmark:}\\[3pt]
        {\footnotesize\color{MutedGray} 1--3 gallons per cm$^2$ of silicon processed\\[2pt]
        Modern fabs: significant water recovery programs}
    };

    % UPW Importance card
    \node[flowbox, draw=color3, fill=LightAmber, text width=5.6cm, minimum height=1.4cm] (upw_why) at (3.3, -2.8) {
        {\bfseries\color{color3!90!black} Why ultra-pure water matters:}\\[3pt]
        {\footnotesize\color{color3!90!black} Trace ions destroy sub-nm transistors\\[2pt]
        Resistivity target: 18.2\,$\text{M}\Omega\cdot\text{cm}$}
    };

    % --- Bottom banner ---
    \node[banner, text width=13.6cm] (bot) at (0, -4.2) {
        {\footnotesize\color{darknavy} Chip manufacturing is water-intensive --- water stewardship is a key sustainability metric for AI hardware supply chains}
    };
...
\providecolor{BorderGreen}{HTML}{86EFAC}
\providecolor{LightAmber}{HTML}{FFFBEB}
\providecolor{BorderAmber}{HTML}{FDE68A}
\providecolor{LightRed}{HTML}{FEF2F2}
\providecolor{BorderRed}{HTML}{FCA5A5}
\providecolor{BorderGray}{HTML}{CBD5E1}
\providecolor{CardBg}{HTML}{F8FAFC}
...
\begin{tikzpicture}[
    font=\small,
    >=Stealth,
    phasecard/.style={draw=BorderGray, fill=white, rounded corners=5pt, line width=0.8pt},
    headerpill/.style={rounded corners=4pt, line width=0.8pt, align=center, inner sep=4pt},
    highlightbox/.style={rounded corners=4pt, line width=0.8pt, align=center, inner sep=5pt}
]
    ...
    \node[phasecard, minimum width=\cw, minimum height=\ch] (c1) at (-5.85, \yTop) {};
    % Header
    \node[headerpill, fill=LightGreen, text=color2, text width=\cw-0.4cm] at (-5.85, \yTop+3.15) {
        {\bfseries\footnotesize 1 $\cdot$ SYSTEM DESIGN}
    };
    % Body items
    \node[anchor=north west, align=left, text width=\cw-0.5cm, font=\scriptsize] at (-5.85-1.55, \yTop+2.65) {
        {\bfseries\color{darknavy} Architecture Selection}\\[1pt]
        {\color{MutedGray} Dense vs. MoE routing}\\[8pt]
        {\bfseries\color{darknavy} Parameter Budgeting}\\[1pt]
        {\color{MutedGray} Pruning \& distillation}\\[8pt]
        {\bfseries\color{darknavy} Hardware Spec}\\[1pt]
        {\color{MutedGray} ASIC vs. GPU efficiency}\\[8pt]
        {\bfseries\color{darknavy} Facility Planning}\\[1pt]
        {\color{MutedGray} PUE targets \& grid site}
    };
    % Bottom highlight
    \node[highlightbox, draw=BorderGreen, fill=LightGreen, text width=\cw-0.6cm] at (-5.85, \yTop-2.85) {
        {\bfseries\scriptsize\color{color2} System Leverage}\\[2pt]
        {\tiny\color{color2!90!black} Architects lifetime load}
    };

    % ================= CARD 2: MANUFACTURE =================
    \node[phasecard, minimum width=\cw, minimum height=\ch] (c2) at (-1.95, \yTop) {};
    % Header
    \node[headerpill, fill=LightBlue, text=color1, text width=\cw-0.4cm] at (-1.95, \yTop+3.15) {
        {\bfseries\footnotesize 2 $\cdot$ MANUFACTURE}
    };
    % Body items
    \node[anchor=north west, align=left, text width=\cw-0.5cm, font=\scriptsize] at (-1.95-1.55, \yTop+2.65) {
        {\bfseries\color{darknavy} Silicon Fabrication}\\[1pt]
        {\color{MutedGray} EUV lithography \& wafers}\\[8pt]
        {\bfseries\color{darknavy} Resource Extraction}\\[1pt]
        {\color{MutedGray} Rare-earth \& copper mining}\\[8pt]
        {\bfseries\color{darknavy} Board Assembly}\\[1pt]
        {\color{MutedGray} HBM packaging \& PCB}\\[8pt]
        {\bfseries\color{darknavy} Supply Transport}\\[1pt]
        {\color{MutedGray} Global freight logistics}
    };
    % Bottom highlight
    \node[highlightbox, draw=BorderBlue, fill=LightBlue, text width=\cw-0.6cm] at (-1.95, \yTop-2.85) {
        {\bfseries\scriptsize\color{color1} Embodied Carbon}\\[2pt]
        {\tiny\color{color1!90!black} High water \& chemical cost}
    };

    % Arrow c1 -> c2
    \draw[->, line width=1.5pt, gray!50] (c1.east) -- (c2.west);

    % ================= CARD 3: USE & OPERATE =================
    \node[phasecard, minimum width=\cw, minimum height=\ch] (c3) at (1.95, \yTop) {};
    % Header
    \node[headerpill, fill=LightAmber, text=color3, text width=\cw-0.4cm] at (1.95, \yTop+3.15) {
        {\bfseries\footnotesize 3 $\cdot$ USE \& OPERATE}
    };
    % Body items
    \node[anchor=north west, align=left, text width=\cw-0.5cm, font=\scriptsize] at (1.95-1.55, \yTop+2.65) {
        {\bfseries\color{darknavy} Model Training}\\[1pt]
        {\color{MutedGray} Megawatt-scale clusters}\\[8pt]
        {\bfseries\color{darknavy} Fleet Inference}\\[1pt]
        {\color{MutedGray} Autoregressive decode}\\[8pt]
        {\bfseries\color{darknavy} Facility Overhead}\\[1pt]
        {\color{MutedGray} PUE chillers \& power loss}\\[8pt]
        {\bfseries\color{darknavy} Continuous Retraining}\\[1pt]
        {\color{MutedGray} Online updates \& fine-tunes}
    };
    % Bottom highlight
    \node[highlightbox, draw=BorderAmber, fill=LightAmber, text width=\cw-0.6cm] at (1.95, \yTop-2.85) {
        {\bfseries\scriptsize\color{color3} Operational Energy}\\[2pt]
        {\tiny\color{color3!90!black} Grid carbon intensity draw}
    };

    % Arrow c2 -> c3
    \draw[->, line width=1.5pt, gray!50] (c2.east) -- (c3.west);

    % ================= CARD 4: DECOMMISSION =================
    \node[phasecard, minimum width=\cw, minimum height=\ch] (c4) at (5.85, \yTop) {};
    % Header
    \node[headerpill, fill=LightRed, text=color4, text width=\cw-0.4cm] at (5.85, \yTop+3.15) {
        {\bfseries\footnotesize 4 $\cdot$ DECOMMISSION}
    };
    % Body items
    \node[anchor=north west, align=left, text width=\cw-0.5cm, font=\scriptsize] at (5.85-1.55, \yTop+2.65) {
        {\bfseries\color{darknavy} Server Retirement}\\[1pt]
        {\color{MutedGray} 3--5 year refresh cadence}\\[8pt]
        {\bfseries\color{darknavy} E-Waste Stream}\\[1pt]
        {\color{MutedGray} Heavy metals \& toxics}\\[8pt]
        {\bfseries\color{darknavy} Component Recovery}\\[1pt]
        {\color{MutedGray} Low recycling rates ($<$20\%)}\\[8pt]
        {\bfseries\color{darknavy} Secondary Life}\\[1pt]
        {\color{MutedGray} Refurbishment \& reuse}
    };
    % Bottom highlight
    \node[highlightbox, draw=BorderRed, fill=LightRed, text width=\cw-0.6cm] at (5.85, \yTop-2.85) {
        {\bfseries\scriptsize\color{color4} E-Waste \& Toxics}\\[2pt]
        {\tiny\color{color4!90!black} Landfill \& recycling burden}
    };

    % Arrow c3 -> c4
    \draw[->, line width=1.5pt, gray!50] (c3.east) -- (c4.west);

    % ================= MIDDLE SECTION: CARBON FOOTPRINT SPLIT =================
    \def\midw{15.3cm}
    \node[draw=BorderGray, fill=CardBg, rounded corners=5pt, line width=0.8pt, minimum width=\midw, minimum height=1.9cm] (splitbox) at (0, -4.9) {};

    \node[anchor=west, font=\bfseries\scriptsize\color{darknavy}] at (-7.35, -4.25) {
        TYPICAL AI SYSTEM LIFECYCLE CARBON FOOTPRINT SPLIT
    };

    % Stacked Bar (coordinates explicitly defined)
    \def\yBarTop{-4.6}
    \def\yBarBot{-5.3}

    % 5% System Design
    \draw[rounded corners=2pt, draw=BorderGreen, fill=LightGreen, line width=0.8pt] (-7.35, \yBarTop) rectangle (-6.45, \yBarBot);
    \node[font=\bfseries\scriptsize\color{color2}] at (-6.9, -4.95) {5\%};

    % 45% Embodied
    \draw[rounded corners=2pt, draw=BorderBlue, fill=LightBlue, line width=0.8pt] (-6.35, \yBarTop) rectangle (-0.1, \yBarBot);
    \node[font=\bfseries\scriptsize\color{color1}] at (-3.22, -4.95) {45\% $\cdot$ Embodied Hardware Manufacturing};

    % 40% Operational
    \draw[rounded corners=2pt, draw=BorderAmber, fill=LightAmber, line width=0.8pt] (0.0, \yBarTop) rectangle (5.95, \yBarBot);
    \node[font=\bfseries\scriptsize\color{color3}] at (2.97, -4.95) {40\% $\cdot$ Operational Energy (Training + Serving)};

    % 10% Decommission
    \draw[rounded corners=2pt, draw=BorderRed, fill=LightRed, line width=0.8pt] (6.05, \yBarTop) rectangle (7.35, \yBarBot);
    \node[font=\bfseries\scriptsize\color{color4}] at (6.7, -4.95) {10\%};

    % ================= BOTTOM SECTION: EMBODIED CARBON DOMINANCE =================
    \node[draw=BorderBlue, fill=LightBlue!30, rounded corners=5pt, line width=0.8pt, text width=\midw-0.8cm, inner sep=8pt, anchor=north] (botcard) at (0, -6.1) {
        {\bfseries\scriptsize\color{color1!80!black} EMBODIED CARBON DOMINANCE PRINCIPLE}\\[4pt]
        {\footnotesize\color{darknavy} Fabricating accelerator silicon and high-bandwidth memory emits carbon equivalent to 1--2 years of active operational power draw.\\[2pt]
        Extending server lifespans from 3 to 5 years and optimizing GPU utilization deliver higher lifetime decarbonization than algorithmic tuning alone.}
    };
...
: **Model Carbon Footprint**: Training large AI models can generate substantial carbon emissions, but the final footprint depends on computation, hardware efficiency, data-center overhead, and grid carbon intensity. GPT-3's training emissions are equivalent to driving 1.9 million km under the accounting assumptions used here. {#tbl-training-emissions}
...
```{=latex}
\providecolor{color1}{RGB}{30,58,138}    % dark navy blue (historical)
\providecolor{color2}{RGB}{99,102,241}   % indigo / projection blue
\providecolor{color3}{RGB}{217,119,6}    % amber
\providecolor{color4}{RGB}{220,38,38}    % red
\providecolor{accentgreen}{RGB}{22,101,52}
\providecolor{darknavy}{RGB}{20,40,80}
\providecolor{MutedGray}{HTML}{64748B}
\providecolor{LightBlue}{HTML}{F0F7FF}
\providecolor{BorderBlue}{HTML}{93C5FD}
...
\begin{tikzpicture}[
    ...
    \draw[line width=0.8pt, BorderGray] (\xOrig-0.2, \yOrig) -- (\xOrig+10.6, \yOrig);
    \draw[line width=0.8pt, BorderGray] (\xOrig, \yOrig) -- (\xOrig, 36*\yScale);

    \foreach \y/\lbl in {0/0, 5/5B, 10/10B, 15/15B, 20/20B, 25/25B, 30/30B, 35/35B} {
        \draw[gridline] (\xOrig, \y*\yScale) -- (\xOrig+10.5, \y*\yScale);
        \node[anchor=east, font=\scriptsize\color{MutedGray}] at (\xOrig-0.1, \y*\yScale) {\lbl};
    }

    % Y-axis label
    \node[anchor=south, font=\bfseries\footnotesize\color{darknavy}, rotate=90] at (\xOrig-1.1, 17.5*\yScale) {Active Connected Devices (Billions)};

    % Data: year, value, is_proj
    % Historical
    \def\histdata{
        1/2019/8.6,
        2/2020/9.8,
    ...
    \foreach \idx/\yr/\val in \histdata {
        \pgfmathsetmacro{\bx}{\xOrig + (\idx - 0.4)*\xStep}
        \pgfmathsetmacro{\by}{\val * \yScale}
        \fill[color1, rounded corners=2pt] (\bx - \barW/2, 0) rectangle (\bx + \barW/2, \by);
        \node[anchor=south, font=\bfseries\tiny\color{color1}] at (\bx, \by) {\val};
        \node[anchor=north, font=\scriptsize\color{MutedGray}] at (\bx, -0.1) {\yr};
    }

    % Draw Projection bars
    \foreach \idx/\yr/\val in \projdata {
        \pgfmathsetmacro{\bx}{\xOrig + (\idx - 0.4)*\xStep}
        \pgfmathsetmacro{\by}{\val * \yScale}
        \fill[color2, rounded corners=2pt] (\bx - \barW/2, 0) rectangle (\bx + \barW/2, \by);
        \node[anchor=south, font=\bfseries\tiny\color{color2!80!black}] at (\bx, \by) {\val};
        \node[anchor=north, font=\scriptsize\bfseries\color{color2}] at (\bx, -0.1) {\yr};
    }

    % Split line
    \pgfmathsetmacro{\splitX}{\xOrig + 5.1*\xStep}
    \draw[dashed, line width=0.8pt, BorderGray] (\splitX, -0.2) -- (\splitX, 36*\yScale);
    \node[draw=BorderGray, fill=white, rounded corners=3pt, font=\tiny\bfseries\color{MutedGray}, inner sep=3pt] at (\splitX, 36*\yScale+0.25) {Historical / Projection Split};

    % Legend
    \node[anchor=west] at (\xOrig+0.3, 33*\yScale) {
        \begin{tikzpicture}
            \fill[color1, rounded corners=1pt] (0,0) rectangle (0.3, 0.2);
            \node[anchor=west, font=\scriptsize\color{darknavy}] at (0.35, 0.1) {Historical Data};
            \fill[color2, rounded corners=1pt] (2.4,0) rectangle (2.7, 0.2);
            \node[anchor=west, font=\scriptsize\color{darknavy}] at (2.75, 0.1) {Statista Projections (*)};
        \end{tikzpicture}
};
...
- **Right-to-Repair**\index{Right-to-Repair!definition}: Legislative and regulatory movements push back against repair restrictions by emphasizing access to parts, tools, diagnostics, and service information [@ftc2021nixingfix].
- Modular design: AI hardware designs that allow independent upgrade of accelerators, memory, or networking interfaces prevent entire systems from being discarded when only one component is obsolete, following the principle demonstrated by companies like Framework in consumer laptops [@framework2022modular].
- Extended support cycles: Longer software and firmware support keeps usable hardware secure and operational for longer, delaying its entry into the e-waste stream [@forti2020ewaste].
...
Implementation must account for Jevons Paradox[^fn-jevons-efficiency-paradox] (principle \ref{pri-jevons-paradox}): making models 10$\times$ more efficient can increase total usage enough to erase or even exceed the expected energy savings, because cheaper computation enables entirely new applications that were previously economically infeasible. This **Rebound Effect**\index{Rebound Effect!definition}\index{Jevons Paradox} is why sustainability strategies must focus on absolute limits (carbon budgets\index{Carbon Budget}, renewable sourcing) rather than just rate efficiency (FLOP/s per watt), combining technical optimization with usage governance that prevents efficiency gains from being offset by exponential growth in deployment scale.
...
```{=latex}
\providecolor{color1}{RGB}{30,58,138}    % navy blue
\providecolor{color2}{RGB}{22,101,52}    % green
\providecolor{color3}{RGB}{217,119,6}    % amber
\providecolor{color4}{RGB}{220,38,38}    % red
\providecolor{darknavy}{RGB}{20,40,80}
...
\begin{tikzpicture}[
    ...
    % Areas
    % 1. Retained Base
    \fill[white] (\xO, \yO) rectangle (\qZero, \pOne);
    \node[font=\scriptsize\color{MutedGray}] at (\qZero/2, \pOne/2) {Retained Base};

    % 2. Theoretical Savings (Green)
    \fill[LightGreen] (\xO, \pOne) rectangle (\qZero, \pZero);
    \draw[dashed, line width=1pt, color2] (\xO, \pZero) -- (\qZero, \pZero) -- (\qZero, \pOne);
    \node[align=center, font=\scriptsize\bfseries\color{color2}] at (\qZero/2, \pOne + 1.25) {
        THEORETICAL\\[1pt] SAVINGS\\[3pt]
        {\tiny\color{color2!80!black} (Cost Drop $\times$ $Q_0$)}
    };

    % 3. Rebound Consumption Surge (Red)
    \fill[LightRed] (\qZero, \yO) rectangle (\qOne, \pOne);
    \draw[dashed, line width=1pt, DarkSlate] (\qZero, \pOne) -- (\qOne, \pOne) -- (\qOne, \yO);
    \node[align=center, text width=5.2cm] at (5.0, 1.3) {
        {\bfseries\scriptsize\color{color4} REBOUND CONSUMPTION SURGE}\\[3pt]
        {\tiny\color{DarkSlate} Cheaper inference unlocks continuous background automation,\\
        mass agentic loops, and synthetic data generating runs}\\[4pt]
        {\bfseries\tiny\color{color4} NET ENERGY SURGE: Area(Rebound) $>$ Area(Savings)}
    };

    % Axes
    \draw[line width=1pt, DarkSlate] (\xO, \yO) -- (9.8, \yO) node[anchor=south east, font=\tiny\bfseries\color{DarkSlate}, yshift=1pt] {AI Compute Volume $\to$};
    \draw[line width=1pt, DarkSlate] (\xO, \yO) -- (\xO, 6.2) node[anchor=south west, font=\scriptsize\bfseries\color{DarkSlate}, yshift=2pt] {Unit Cost (\$ / FLOP)};

    % Curve: Elastic Demand Curve
    \draw[line width=2.2pt, color1] (0.8, 5.8) to[out=-40, in=145] (\qZero, \pZero) to[out=-35, in=165] (\qOne, \pOne) to[out=-15, in=170] (9.6, 2.0);

    % Curve label
    \node[draw=BorderBlue, fill=white, rounded corners=3pt, line width=0.8pt, font=\tiny\bfseries\color{darknavy}, inner sep=4pt] at (6.2, 3.8) {Elastic Demand Curve: D(P)};

    % Dots at (Q0, P0) and (Q1, P1)
    \fill[color1] (\qZero, \pZero) circle (3.2pt);
    \fill[white] (\qZero, \pZero) circle (1.2pt);

    \fill[color4] (\qOne, \pOne) circle (3.2pt);

    % Axis ticks & labels
    % Y ticks
    \draw[line width=0.8pt, DarkSlate] (-0.1, \pZero) -- (0, \pZero);
    \node[anchor=east, font=\scriptsize\bfseries\color{DarkSlate}] at (-0.15, \pZero) {$P_0$: Initial Unit Cost};

    \draw[line width=0.8pt, DarkSlate] (-0.1, \pOne) -- (0, \pOne);
    \node[anchor=east, font=\scriptsize\bfseries\color{DarkSlate}] at (-0.15, \pOne) {$P_1$: Reduced Cost};

    % X ticks
    \draw[line width=0.8pt, DarkSlate] (\qZero, -0.1) -- (\qZero, 0);
    \node[anchor=north, font=\scriptsize\bfseries\color{DarkSlate}] at (\qZero, -0.15) {$Q_0$: Base Volume};

    \draw[line width=0.8pt, DarkSlate] (\qOne, -0.1) -- (\qOne, 0);
    \node[anchor=north, font=\scriptsize\bfseries\color{DarkSlate}] at (\qOne, -0.15) {$Q_1$: Rebound Volume};

    % Left cost drop arrow
    \draw[->, line width=1.5pt, color2] (-0.6, \pZero-0.1) -- (-0.6, \pOne+0.1);
    \node[fill=LightGreen, draw=BorderGreen, line width=0.8pt, rounded corners=3pt, font=\tiny\bfseries\color{color2}, inner sep=3pt] at (-0.6, {(\pZero+\pOne)/2}) {$-$50\% COST};

    % Bottom usage surge arrow
    \draw[->, line width=1.5pt, color4] (\qZero+0.1, -0.85) -- (\qOne-0.1, -0.85);
    \node[fill=LightRed, draw=BorderRed, line width=0.8pt, rounded corners=3pt, font=\tiny\bfseries\color{color4}, inner sep=3pt] at ({(\qZero+\qOne)/2}, -0.85) {+145\% USAGE SURGE};

    % ================= RIGHT: THE JEVONS EFFICIENCY TRAP =================
    \def\pX{14.6}
    \def\pW{8.0cm}
    \def\pH{7.6cm}

    % Outer panel
    \node[draw=BorderGray, fill=CardBg, rounded corners=6pt, line width=0.8pt, minimum width=\pW, minimum height=\pH] (trap_panel) at (\pX, 2.9) {};

    % Header
    \node[font=\footnotesize\bfseries\color{darknavy}] at (\pX, 6.25) {THE JEVONS EFFICIENCY TRAP};

    % Subcard 1: Elasticity > 1
    \node[draw=BorderGray!60, fill=white, rounded corners=4pt, text width=\pW-0.7cm, inner sep=4pt, anchor=north] (t1) at (\pX, 5.85) {
        \tikz{\node[fill=LightAmber, text=color3!90!black, rounded corners=2pt, font=\tiny\bfseries, inner sep=2pt] {ELASTICITY $>$ 1 MECHANISM};}\\[2pt]
        {\scriptsize\color{darknavy}\textbf{Unit Gain:} Quantization \& pruning slash cost $-$50\%\\[1pt]
        \textbf{Transition:} Batch luxury becomes omnipresent utility\\[1pt]
        \textbf{Dynamics:} Lower price expands market aggregate demand}
    };

    % Subcard 2: Systems Rebound Levers
    \node[draw=BorderGray!60, fill=white, rounded corners=4pt, text width=\pW-0.7cm, inner sep=4pt, anchor=north] (t2) [below=0.3cm of t1] {
        \tikz{\node[fill=LightRed, text=color4, rounded corners=2pt, font=\tiny\bfseries, inner sep=2pt] {SYSTEMS REBOUND LEVERS};}\\[2pt]
        {\scriptsize\color{darknavy}\textbf{Agentic Loops:} Multi-step tool loops execute 100$\times$ calls\\[1pt]
        \textbf{Continuous AI:} Streaming edge computer vision \& audio\\[1pt]
        \textbf{Synthetic Data:} LLM pipelines generating pre-training tokens\\[1pt]
        \textbf{Speculation:} Speculative sampling and tree branch search}
    };

    % Subcard 3: Absolute Budget Controls
    \node[draw=BorderGreen, fill=LightGreen!30, rounded corners=4pt, text width=\pW-0.7cm, inner sep=4pt, anchor=north] (t3) [below=0.3cm of t2] {
        \tikz{\node[fill=LightGreen, text=color2, rounded corners=2pt, font=\tiny\bfseries, inner sep=2pt] {ABSOLUTE BUDGET CONTROLS};}\\[2pt]
        {\scriptsize\color{color2}\textbf{Datacenter:} Enforce hard aggregate MW power envelopes\\[1pt]
        \textbf{Matching:} Hourly 24/7 carbon matching \& curtailment\\[1pt]
        \textbf{Governance:} Quota throttling for automated agent loops}
    };

    % Principle footer
    \node[font=\tiny\color{MutedGray}, anchor=north] at ([yshift=-0.35cm] t3.south) {Economic Principle: Jevons (1865) / Khazzoom-Brookes};
...
An engineering team optimizes a translation service, reducing the computational cost per query by 50 percent (2$\times$ efficiency gain).
...
class EnergyEfficiencyRange:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(
        cpu_energy_per_flop > gpu_energy_per_flop > tpu_energy_per_flop > asic_energy_per_flop,
...
1.  Memory fit: Microcontrollers operate with 64 KB to 2 MB SRAM, so the memory pass starts with peak activation analysis, rather than parameter count alone.\index{Memory Analysis!layer-wise} A model that has small weights can still fail if one intermediate tensor exceeds the tensor arena, so TinyML runtimes rely on in-place operations,\index{In-Place Operations} tensor-arena planning,\index{Tensor Arena!optimization} and operator fusion\index{Operator Fusion} to reuse buffers and avoid fragmentation.
2.  Switching energy: Once the memory plan fits, the energy pass asks whether ordinary INT8 arithmetic is still too expensive for the deployment. On devices powered by solar, vibration, or RF harvesting, binary neural networks\index{Binary Neural Network} may be justified because XNOR-style operations\index{XNOR-Net Operations} replace multiply-accumulate work with bit operations [@courbariaux2016binarized]. That trade is not free: the accuracy loss is task-dependent, so binary networks belong in applications where always-on sensing within the harvested-energy budgets in @tbl-energy-harvesting matters more than full-precision classification margins.
...
The right efficiency metric depends on the serving regime. Batch inference is naturally expressed as correct samples per joule,\index{Samples Per Joule} latency-sensitive serving as SLO-compliant queries per joule,\index{Queries Per Joule} and generative workloads as joules per accepted output token\index{Joules Per Token} at a declared quality threshold. Raw throughput per joule rewards systems that lower accuracy, truncate outputs, or miss latency objectives. Standardization does not make one platform universally green; it fixes useful work, quality, the measurement window, and the energy denominator well enough for procurement and architecture decisions to be argued quantitatively.
...
Energy and latency still have to be read together through the energy-delay product (EDP)\index{Energy Delay Product} (@eq-energy-delay-product), which balances energy consumption against response time by penalizing solutions that save power only by taking too long:
...
Grid carbon intensity fluctuates dramatically based on the mix of power sources available at any given time---from 50 g CO~2~/kWh in nuclear-heavy France to 820 g/kWh in coal-dependent Poland. **Carbon-Aware Scheduling**\index{Carbon-Aware Scheduling!definition} dynamically shifts AI computations to times and locations where low-carbon energy is available. For deadline-tolerant workloads with geographic or temporal flexibility, it can be one of the highest-leverage emissions levers.

Carbon-aware scheduling is fundamentally a **Load Shifting Software Problem**\index{Load Shifting Software Problem!definition}\index{Load Shifting!software}. The scheduler queries real-time grid carbon intensity APIs (for example, ElectricityMap, WattTime) to pause nonurgent training jobs during carbon-intensive periods, such as the evening peak, and migrate workloads to geographic regions with excess renewable energy, such as solar peaks in California or wind peaks in Iowa.

Google's carbon-intelligent computing platform[^fn-scheduling-energy-results] demonstrated this approach at scale, achieving a 40 percent reduction in carbon footprint under its global workload-shifting assumptions [@radovanovic2021carbonaware]. Within the broader energy-gap cascade of @fig-carbon-scheduling, carbon-aware scheduling is one of the systemic-stage steps, contributing a more conservative 1.3$\times$ average reduction for mixed production fleets, where only some jobs are deadline-tolerant enough to move across time or geography.

::: {#fig-carbon-scheduling fig-env="figure" fig-pos="htb" fig-cap="**Carbon-Aware Workload Scheduling in the Intervention Cascade**: The six-step intervention cascade from @fig-energy-intervention, recalled here because carbon-aware scheduling is one of its systemic-stage steps, contributing a conservative 1.3$\times$ reduction for mixed fleets via temporal and geographic shifting." fig-alt="Six-step intervention cascade reused from the energy-intervention figure; carbon-aware scheduling sits among the systemic-stage steps with a conservative 1.3 times contribution."}
```{=latex}
\providecolor{color1}{RGB}{0,102,204}    % primary blue
\providecolor{color2}{RGB}{76,175,80}    % green
\providecolor{color3}{RGB}{255,152,0}    % amber / orange
...
[^fn-scheduling-energy-results]: **Carbon-Aware Scheduling at Scale**\index{Carbon-Aware Scheduling at Scale!definition}\index{Carbon-Aware Scheduling!results}: Google's carbon-aware data center study reports 15 percent carbon reduction through intra-region temporal shifting alone, and 40 percent globally by routing nonurgent batch compute across time zones to chase renewable peaks [@radovanovic2021carbonaware]. The key insight is that delay-tolerant workloads gain access to dramatically different grid carbon intensities without any model or infrastructure changes.

The effectiveness of carbon-aware scheduling depends on accurate real-time grid emissions data. Average grid intensity is useful for retrospective reporting because it estimates the emissions associated with energy already consumed. Marginal emissions are more useful for scheduling because they estimate which generator responds when the workload adds or removes demand. The Electricity Maps API provides real-time CO~2~ emissions data for power grids worldwide,[^fn-marginal-grid-emissions] while WattTime provides marginal emissions data showing which power plants turn on or off next. @Fig-carbon-aware-scheduling demonstrates the scheduling opportunity: shifting training jobs to low-carbon hours in lower-carbon regions reduces emissions by up to 8$\times$ without changing a single line of model code.

[^fn-marginal-grid-emissions]: **Marginal vs. Average Emissions**\index{Marginal vs. Average Emissions!definition}\index{Grid Carbon!marginal emissions}: WattTime's marginal emissions data identifies which power plant turns on next when load increases, enabling 2--5$\times$ better carbon optimization than grid-average intensity. The distinction is critical: average intensity smooths out peaks, but marginal data reveals that adding 1 MW of load at the wrong hour can activate a coal peaker plant at 900 g/kWh even on a nominally "clean" grid.
...
# ┌── 4. DECORATE ──────────────────────────────────────────────────────────────
ax.text(8, 500, 'Low-carbon\nwindow', fontsize=9, ha='center', va='center', fontweight="bold", color='#008F45')
ax.text(20, 500, 'Peak carbon', fontsize=9, ha='center', va='center', fontweight="bold", color='#CB202D')
...
AI-driven cooling optimization is a software-deployable lever for reducing data center energy consumption when the facility has sufficient sensing and controllable cooling equipment. Traditional cooling systems rely on fixed control policies with predefined temperature thresholds, often consuming more energy than necessary; a learned controller that adapts to real-time conditions reclaims much of that waste, as the DeepMind deployment analyzed in @sec-sustainable-ai-case-study-deepmind-energy-efficiency-84fd showed without any hardware change.

Complementing software optimization, liquid cooling\index{Liquid Cooling} and immersion cooling\index{Cooling!immersion} change the thermal design space for dense accelerator clusters. Liquid cooling removes heat near the accelerator with a fluid whose volumetric heat capacity is far greater than air, reducing the airflow needed at the rack. Immersion cooling submerges computing equipment in a nonconductive coolant and replaces much of the rack's air-handling path. These approaches enable higher compute densities, but lower facility power is not automatic because pumps, coolant distribution, and heat rejection remain in the boundary.
...
- **Measure First**\index{Measure First!definition}: Tools like CarbonTracker and CodeCarbon track the emissions of training runs. Teams cannot improve what they do not measure, and establishing baseline metrics is essential for validating the effectiveness of optimization efforts [@anthony2020carbontracker; @schmidt2021codecarbon].
- Choose the region: Train models in data centers powered by renewable energy. As established in @sec-sustainable-ai-scale-environmental-impact-ac9a, grid carbon intensity spans the 8 to 40 times range for representative region pairs; scheduling workloads where clean energy is most abundant yields immediate reductions.
- **Optimize the Model**\index{Optimize the Model!definition}: Avoid training the largest model possible by default. Pruning, quantization, and knowledge distillation can reduce resource use when the resulting implementation preserves the required quality, latency, and reliability objectives. Quantization does not guarantee a proportional energy reduction: realized savings depend on kernel support, memory traffic, conversion overhead, batching, and whether the workload remains bound by the resource whose cost changed.
...
- **Select Efficient Hardware**\index{Select Efficient Hardware!definition}: Energy-efficient accelerators, such as TPUs or specialized inference chips, reduce deployment costs. The full hardware lifecycle and workload-specific platform selection matter as much as raw throughput.
- Account for the full lifecycle: Longer hardware refresh cycles and responsible e-waste policies can reduce total environmental impact. Manufacturing can dominate on clean grids or under low utilization, whereas sustained operation on a carbon-intensive grid can dominate the same hardware's lifecycle. Hardware longevity is therefore a conditional trade-off, not an automatic environmental win.
...
The lifecycle carbon footprint spans from the per-H100 embodied carbon quantified in @sec-sustainable-ai-embodied-carbon-assessment-9de0 to the thousands of megawatt-hours consumed during training. During autoregressive decode, accelerators can draw power while waiting on memory, which makes quantization and memory-optimized hardware relevant only when they reduce measured energy at fixed quality, latency, and reliability. The Jevons rebound adds a demand constraint because efficiency gains do not reduce total impact when usage grows faster than per-query consumption falls.

Carbon budgets, power delivery, cooling capacity, and water availability impose measurable limits on fleet expansion. The measurement contract makes those limits comparable by fixing useful work, system boundaries, attribution, time, location, and uncertainty. Sustainability therefore requires workload accounting and demand governance in addition to component-level optimization.

Lifecycle accounting covers training, inference, and embodied manufacturing emissions, while carbon-aware scheduling incorporates grid intensity into placement decisions. These methods turn sustainability into a measurable engineering constraint. When regulation, procurement, or carbon pricing makes environmental impact part of the operating envelope, the accounting becomes as fundamental as fault tolerance or security.
