...
::: {#fig-adversarial-arms-race fig-env="figure" fig-pos="htb" fig-cap="**The Adversarial Arms Race**: The cumulative number of adversarial machine learning papers published on arXiv from 2014 to 2023. The exponential growth illustrates the intense, ongoing cycle of researchers proposing new defenses only to have them broken by subsequent adversarial attacks."}
...
data = pd.read_csv("data/adv_ml_papers_arxiv.csv")
...
While hardware-level isolation via TEEs provides a critical foundation for defending proprietary models against compromised infrastructure, protecting the model is only half the battle. The model itself can also be weaponized. The same pattern recognition capabilities that enable defense (such as automated anomaly detection, statistical classification, and sequence prediction) can be turned against the system when adversaries use machine learning as an attack tool.
...
|:---------------------------------------------------------------|:---------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Healthcare ML** (federated diagnostic models)                | Data leakage (HIPAA violation); membership inference; unauthorized access        | Differential privacy $(\epsilon \leq 4)$ for training; federated learning across hospitals; TEE for inference on sensitive data; audit logging and RBAC                                                                           | 2--5% accuracy loss acceptable for compliance; 50--100 ms inference latency from TEE overhead                                                                                              |
...
| **Multi-Party ML** (cross-organizational training)             | Data sharing restrictions; honest-but-curious participants; GDPR compliance      | Federated learning (no raw data sharing); SMPC for secure aggregation; differential privacy $(\epsilon \leq 1)$; homomorphic encryption (for sensitive ops)                                                                       | Communication overhead: 10--100$\times$ more rounds than centralized training; SMPC adds 1,000$\times$+ compute cost; accuracy may degrade 5--15%; requires legal agreements for liability |
