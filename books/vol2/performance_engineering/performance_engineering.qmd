...
\noindent
![](images/png/cover_performance_engineering_blueprint_labeled_print.png){fig-alt="Performance engineering workbench where a fleet workload is measured, bottlenecks are diagnosed, and tiling, fusion, memory, and scheduling rewrites improve the path."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_performance_engineering_blueprint_labeled.webp){fig-alt="Performance engineering workbench where a fleet workload is measured, bottlenecks are diagnosed, and tiling, fusion, memory, and scheduling rewrites improve the path."}
:::
...
_How can billion-parameter models run on millisecond timescales?_
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_qty_int, check
...
|:-------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Registers**\index{Register File}   | `{python} MemoryWallScenario.reg_per_sm_kib_str` per SM across `{python} MemoryWallScenario.sm_count_str` SMs, about `{python} MemoryWallScenario.register_total_mib_str` total | One clock cycle and ~0.01 pJ per access                                                                               | Private to each thread; FP32 accumulators can force register spilling\index{Register File!register spilling} to L1/shared memory at 20--30 clock cycles per access |
| **Shared memory (SRAM)**\index{SRAM} | Up to `{python} MemoryWallScenario.shared_mem_per_sm_kib_str` configurable shared memory per SM                                                                                 | 20--30 clock cycles (~20 ns) and ~0.5 pJ per access                                                                   | Shared within a thread block; operator fusion is profitable when intermediates fit here instead of returning to HBM                                                |
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_display_math, fmt_int, fmt_magnitude, fmt_multiple, check
...
::: {#fig-memory-wall-trend fig-env="figure" fig-pos="htb" fig-cap="**The Widening Memory Wall**: Historical scaling of Peak Compute (TFLOP/s) versus Memory Bandwidth (TB/s). Compute scales exponentially faster than memory bandwidth, increasing the ridge point and pushing more ML operations into the memory-bound regime."}
...
# Use a clean style for the book
plt.style.use('default')
fig, ax1 = plt.subplots(figsize=(8, 5))

# Load data
df = pd.read_csv("data/memory_wall_trend.csv")

color1 = '#1f77b4'
color2 = '#d62728'

# Plot Peak TFLOPS on left y-axis
ax1.set_xlabel('Year', fontsize=12)
ax1.set_ylabel('Peak Compute (TFLOP/s)', color=color1, fontsize=12)
line1 = ax1.plot(df['year'], df['peak_tflops'], marker='o', color=color1, linewidth=2, label='Peak TFLOP/s (FP16)')
ax1.tick_params(axis='y', labelcolor=color1)
ax1.set_yscale('log')

# Create a second y-axis for Memory Bandwidth
ax2 = ax1.twinx()
ax2.set_ylabel('Memory Bandwidth (TB/s)', color=color2, fontsize=12)
line2 = ax2.plot(df['year'], df['bandwidth_tbps'], marker='s', color=color2, linewidth=2, linestyle='--', label='HBM Bandwidth')
ax2.tick_params(axis='y', labelcolor=color2)
ax2.set_yscale('log')

# Annotate GPUs
for i, row in df.iterrows():
    ax1.annotate(row['gpu'], (row['year'], row['peak_tflops']), 
...
plt.title('Divergence of Compute and Memory Bandwidth (2017-2024)', fontsize=14)
...
1.  **Significance**: It characterizes the computational density of a workload. It is the independent variable in the Roofline Model, determining whether a system operates in the bandwidth-bound ($\text{BW}$) or compute-bound\index{Compute-Bound} ($R_{\text{peak}}$) regime.
...
class RooflineRidgeCalc:
    ...
    a100_hbm_bw_tb_per_s_str = fmt_qty(h_a100.memory.bandwidth, TB / second, precision=2, commas=False)
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_arithmetic_intensity, fmt_qty
...
class WorkloadIntensityCalc:
    ...
    gemm_bytes_mb_str = fmt_qty(gemm_bytes, MB, precision=1, commas=False)
```

Large general matrix multiply (GEMM) operations are the most compute-intensive operations in ML. A square matrix multiplication of dimension $4096{\times}4096$ in FP16 performs approximately `{python} WorkloadIntensityCalc.gemm_flops_billion_str` billion FLOPs while loading roughly `{python} WorkloadIntensityCalc.gemm_bytes_mb_str` of data, yielding an arithmetic intensity of approximately `{python} WorkloadIntensityCalc.gemm_intensity_str`. This sits well above the H100's ridge point, making large GEMMs firmly compute bound.

Element-wise operations tell the opposite story. A Gaussian error linear unit (GELU) activation applied to a $4096{\times}4096$ tensor performs roughly 5 operations per element but must load and store each element, yielding an arithmetic intensity of approximately `{python} WorkloadIntensityCalc.elem_intensity_str`. The GPU spends almost all its time waiting for data transfers rather than computing, making these operations profoundly memory-bound.
...
A critical enabler for large batch sizes is **PagedAttention**\index{PagedAttention},[^fn-paged-attention-os] vLLM's paged KV cache management technique [@kwon2023]. Traditional KV cache implementations preallocate contiguous memory for each request's maximum possible sequence length.

[^fn-paged-attention-os]: **PagedAttention**\index{PagedAttention!memory management}: Named by direct analogy to OS virtual memory paging, where the OS maps noncontiguous physical pages to contiguous virtual addresses. The vLLM paper presented this insight at SOSP 2023: the same mechanism eliminates internal fragmentation in KV caches, recovering the 60--80 percent of GPU memory wasted by worst-case preallocation [@kwon2023]. In the paper's end-to-end evaluation, vLLM improves throughput by 2--4$\times$ at the same latency relative to FasterTransformer and Orca, without changing model weights or precision.
...
An orthogonal technique for reducing the overhead term in the iron law is **CUDA Graphs**\index{CUDA!graphs, overhead reduction}.[^fn-cuda-graphs-constraint] While operator fusion combines multiple operations into fewer kernels, CUDA Graphs eliminate the CPU overhead of launching those kernels.

[^fn-cuda-graphs-constraint]: **CUDA Graphs**\index{CUDA Graphs!definition}: Introduced in CUDA 10 (2018), originally for graphics rendering pipelines that replay identical command sequences every frame. The strict determinism requirement (identical operations, shapes, and memory addresses per replay) directly conflicts with the dynamic shapes and variable batch sizes of LLM serving, restricting their use primarily to the decode phase where the computation pattern repeats per token.
...
class FlashAttentionSavings:
    ...
    naive_gb_str = fmt_qty(naive_attn_bytes_total, GB, precision=1, commas=False)
...
FlashAttention\index{FlashAttention} [@dao2022] reformulates attention using tiling\index{Tiling}. Instead of materializing the full $S{\times}S$ attention matrix, it processes $Q$, $K$, and $V$ in small blocks that fit in on-chip SRAM. The algorithm loads tiles of $Q$, $K$, and $V$, computes partial attention scores, and maintains running statistics (online softmax) to produce the exact result without ever storing the full attention matrix in HBM.

::: {.column-margin}
![](images/svg/performance_engineering_flash_ladder.svg){width="100%" fig-alt="A two-rung magnitude ladder on a log scale, in blue. Under the chapter's simplified traffic assumptions, the naive attention rung is about 35 GB and the FlashAttention rung is about 537 MB. The ratio is roughly 65."}
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_qty, fmt_memory_capacity, check
...
class FlashAttentionTiling:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    block_kb_str = fmt_qty(block_bytes, KB, precision=1, commas=False)
    total_kb_str = fmt_qty(total_bytes, KB, precision=1, commas=False)
...
For Archetype C (Federated MobileNet), quantization is a prerequisite for survival, not an optimization. On a microcontroller with only 512 KB of SRAM, an FP16 model is physically impossible to load. Binary neural networks\index{Binary Neural Network!edge quantization} and 1-bit quantization\index{1-bit Quantization!edge quantization} push this to the extreme, representing weights as single bits (+1/-1). While this trades significant accuracy, it reduces the weight memory footprint by 16$\times$ relative to FP16 (32$\times$ relative to FP32) and the energy per operation by up to 100$\times$, enabling intelligence on devices that operate on harvested energy (microwatts).
...
from mlsysim.fmt import fmt_int, fmt, check, fmt_math, fmt_multiple, fmt_percent, fmt_rate, fmt_time
...
Speculative decoding\index{Speculative Decoding!bandwidth optimization}[^fn-speculative-decoding-pe] is a latency optimization that breaks the sequential bottleneck of autoregressive generation by using a smaller draft model to predict multiple tokens, which are then verified in parallel by the target model [@leviathan2023speculative; @chen2023accelerating]. The performance-engineering relevance is arithmetic intensity: a single target-model forward pass can process $K$ candidate tokens for roughly the same weight-streaming cost as one token, shifting the decode operating point from the memory-bound slope toward the roofline ridge.

[^fn-speculative-decoding-pe]: **Speculative Decoding**\index{Speculative Decoding!definition}: The draft model may produce tokens that the target model rejects, so the technique trades extra draft-model compute for reduced target-model memory bandwidth pressure. The target model's weights are loaded from HBM once to process $K$ candidate tokens, effectively increasing the arithmetic intensity of the decode phase by $K\times$ when enough tokens are accepted.
...
A 1-trillion parameter dense model delivers superior reasoning capabilities, but its latency and compute costs are prohibitive for most serving budgets. The mixture-of-experts architecture\index{MoE!performance engineering} resolves this tension by training a massive model but activating only a small, relevant fraction of it for any given token. Routing inputs to specialized sub-networks allows MoE to break the iron link between model size and inference cost.

For performance engineering, the central question is whether active-parameter savings exceed the runtime costs of routing, memory residency, and cross-device communication.

A standard MoE transformer layer replaces the dense feed-forward network with multiple parallel "expert" sub-networks coordinated by a lightweight router\index{Router!Mixture of Experts}. When experts are partitioned across multiple accelerators, an AllToAll collective must exchange each token's hidden states with its assigned device before execution can resume. If that communication runs exposed on the critical path, interconnect latency quickly erodes the arithmetic savings of sparse execution. Preserving those savings requires the same principle that governs tensor-parallel GEMMs and data-parallel gradient exchanges: overlapping communication with computation so network fabrics transfer data while arithmetic engines remain fully engaged.

## Communication-Computation Overlap {#sec-performance-engineering-overlap}

In a multi-GPU training step or tensor-parallel inference pass, an accelerator can finish a local matrix multiplication in microseconds, only to sit idle while waiting for an AllReduce collective to transfer gradients or activations across the network. If communication runs sequentially after computation, interconnect latency directly limits throughput. Communication-computation overlap removes this communication time from the critical path by executing GPU compute kernels concurrently with data transfers over NVLink or InfiniBand, building on the CUDA streams, SM partitioning, and bucket hooks established in @sec-communication-overlap.
...
class NvlinkH100:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt, fmt_multiple, fmt_qty, fmt_qty_int, fmt_time, check
    ...
    b1_vol_kb_str = fmt_qty(b1_vol, KB, precision=1, commas=False)
    b1_time_ns_str = fmt_time(b1_time, nanosecond, precision=1, commas=False)
    b64_vol_mb_str = fmt_qty(b64_vol, MB, precision=1, commas=False)
...
```{python}
#| echo: false
from mlsysim.fmt import fmt_arithmetic_intensity, fmt_flop_rate, fmt_percent, fmt_qty
...
The profiler is therefore a triage tool, not the final performance proof: it identifies whether the next experiment should target fusion, memory planning, CUDA Graphs, or overlap. NVIDIA Nsight Systems\index{Nsight Systems} answers the trace-level question raised by higher-level profiling: whether the GPU timeline is continuous, whether the CPU is feeding it fast enough, and whether communication overlaps compute. The tool captures every GPU kernel launch, CUDA memory operation, NCCL communication, and CPU thread activity onto a unified timeline.
...
```{python}
#| echo: false
from mlsysim.fmt import fmt, fmt_int, fmt_percent, fmt_qty, fmt_qty_int, fmt_rate
...
**Diagnosis**: Nsight Compute shows the primary FP16 GEMM operates efficiently (`{python} ProfilerDetective.gemm_bandwidth_tb_per_s_str` bandwidth), but Nsight Systems reveals `{python} ProfilerDetective.non_gemm_share_pct_str` of step time is lost to unfused LayerNorm/GELU kernels, sampling, and CPU launch gaps across `{python} ProfilerDetective.kernels_per_layer_str` launches per layer.
...
2.  **Distinction**: Unlike hardware utilization\index{Hardware Utilization} (which reports how often the accelerator is "busy"), MFU reports how much of that activity contributes to model convergence or inference, excluding waste FLOPs from recomputation, padding, and gradient checkpointing overhead.
...
from mlsysim.fmt import check, MarkdownStr, fmt_count, fmt_flop_rate, fmt_flops, fmt_percent, fmt_time
...
**Problem**: A `{python} FleetEfficiencyCalc.fleet_params_b_str` parameter model achieves `{python} FleetEfficiencyCalc.fleet_local_mfu_str` MFU on a single 8-GPU node. When training scales to `{python} FleetEfficiencyCalc.fleet_nodes_str`, how much MFU does inter-node communication overhead consume?
...
: **Optimization Paths by Primary Bottleneck**: Diagnostic map from bottleneck class to the ordered interventions that should be tried before the next profiling pass. {#tbl-performance-engineering-bottleneck-paths tbl-colwidths="[14,16,70]"}

Closing the diagnostic loop requires verifying every intervention against fresh telemetry: implement the highest-impact optimization, reprofile the execution timeline, and verify that the target metric improved. Because resolving one bottleneck inevitably shifts pressure to another, engineers iterate from the roofline classification step with the updated profile until the system satisfies its target SLO.

### Combining techniques {#sec-performance-engineering-combining}

Production systems combine techniques because no single intervention covers every term in the iron law. A highly optimized LLM serving system may use FlashAttention-2 or later FlashAttention-family kernels to reduce attention memory traffic, INT4 weight quantization with GPTQ or AWQ to reduce HBM reads, and INT8 KV cache compression with per-channel scaling to increase feasible batch size. Compiler and runtime tools such as `torch.compile`\index{torch.compile!kernel fusion} or TensorRT\index{TensorRT!kernel selection} then handle element-wise fusion and kernel selection.

The serving layer adds another set of controls. Speculative decoding reduces latency at low batch sizes, continuous batching\index{Continuous Batching} refills the batch as requests finish (introduced in @sec-performance-engineering-batch-size), dynamic sequence grouping improves throughput, and tensor parallelism\index{Tensor Parallelism} spreads the model across GPUs while overlapping AllReduce. These techniques belong together only when the profile shows that each one moves a currently binding term.
...
The initial deployment uses FP16 weights, standard PyTorch eager execution, and tensor parallelism across `{python} KVCacheAnalysis.tensor_parallel_degree_str` GPUs. The 70B model in FP16 requires `{python} KVCacheAnalysis.weight_fp16_total_gb_str` of weight storage, distributed as approximately `{python} KVCacheAnalysis.weight_fp16_shard_gb_str` per GPU. Four baseline measurements reveal the optimization gap:
...
```
Performance engineering closes the gap between a model's theoretical efficiency and its hardware implementation by systematically optimizing the terms of the iron law of ML performance (@eq-iron-law-perf): total execution time $T$ is governed by data movement ($D_{\text{vol}} / \text{BW}$), compute execution ($O / (R_{\text{peak}} \cdot \eta_{\text{hw}})$), and latency overhead ($L_{\text{lat}}$). On modern accelerators like the NVIDIA H100, where the FP16 ridge point of the roofline model sits at `{python} RooflineSummaryRecap.h100_fp16_ridge_str`, most autoregressive transformer operations are bound by the memory wall ($D_{\text{vol}} / \text{BW}$).
...
1. **Attacking Data Movement ($D_{\text{vol}}/\text{BW}$)**: Operator fusion and FlashAttention eliminate redundant HBM transfers by tiling attention computations into fast SRAM, replacing quadratic $\mathcal{O}(S^2)$ memory traffic with $\mathcal{O}(S)$ streaming (yielding a `{python} FlashAttentionSummaryRecap.savings_mult_str` traffic reduction in the 8K sequence example). Precision engineering (FP8 formats, INT4 weight quantization, INT8/INT4 KV cache compression) further shrinks payload size $D_{\text{vol}}$, halving memory traffic per transfer and expanding per-node batch capacity.
2. **Eliminating Overhead ($L_{\text{lat}}$)**: Graph compilers (`torch.compile`, TorchInductor, TensorRT) fuse element-wise ops and lower them into single Triton/CUDA kernels, while CUDA Graphs eliminate Python GIL and runtime dispatch latency entirely. At the cluster boundary, communication-computation overlap hides inter-GPU AllReduce latency under useful backward-pass GEMMs.
3. **Restructuring Algorithmic Work ($O/R_{\text{peak}}$)**: Speculative decoding and mixture-of-experts alter the fundamental computational work: speculation generates multiple accepted tokens per expensive target model verification pass, while MoE routes tokens to sparse expert subnetworks, keeping per-token FLOPs $O$ low while scaling model capacity.

As demonstrated by the profiling case study in @sec-performance-engineering-case-study, these techniques interact dynamically. Halving weight precision expands batch size, which increases arithmetic intensity and pushes the system from a memory-bound regime toward a compute-bound regime. Profiling serves as the continuous feedback mechanism: it identifies which term of @eq-iron-law-perf currently binds execution, guiding the performance engineer to apply the next intervention where it actually moves wall-clock latency rather than wasting effort on non-binding bottlenecks.
...
* Fusion makes locality real: Operator fusion, CUDA graphs, and FlashAttention are not just kernel tricks; they remove launch overhead and avoid quadratic attention intermediates. In the 8K example, the simplified HBM-traffic scenario gives a `{python} FlashAttentionSummaryRecap.savings_mult_str` ratio, while realized traffic remains implementation-dependent.
...
::: {.callout-chapter-connection title="From single-node optimization to fleet-scale serving"}

With hardware compute and memory hierarchy fundamentals established in @sec-compute-infrastructure, this chapter focused on local execution mechanics: using operator fusion, precision reduction, graph compilation, speculative decoding, and MoE to conquer the memory wall. In production, however, models do not run in isolation; they serve continuous streams of concurrent requests under tight SLA latency budgets. @Sec-inference-scale moves from local execution mechanics to fleet-scale serving architecture, showing how the per-request memory savings and arithmetic intensity gains developed here enable dynamic batching, continuous scheduling, and tail-latency protection at scale.
