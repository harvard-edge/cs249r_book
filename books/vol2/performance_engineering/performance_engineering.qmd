...
- Evaluate when speculative decoding and sparse expert routing reduce exposed work after accounting for acceptance, communication, and load imbalance
...
@Tbl-widening-gap quantifies how the hardware balance shifts across accelerator generations. The key column is the ridge point: as compute grows faster than bandwidth, more operators need higher arithmetic intensity just to remain compute-bound. @Fig-perf-eng-memory-wall plots the two growth rates behind that column, and the ratio between them is the ridge point: because compute outruns bandwidth roughly two to one, an operator that was compute-bound on the V100 can be memory-bound on the B200 without changing a line of code.
...
::: {#fig-perf-eng-memory-wall fig-env="figure" fig-pos="htb" fig-cap="**The ridge point moves right**: Peak compute and memory bandwidth for NVIDIA data center accelerators, normalized to the V100. Compute rises 18$\times$ against bandwidth's 8.9$\times$, and that two-to-one divergence is precisely what pushes the roofline ridge point toward higher arithmetic intensity each generation. Specifications come from the MLSysIM hardware registry." fig-alt="Line chart normalized to the V100. The compute line rises to 18 times and the bandwidth line to 8.9 times, so the ratio that sets the roofline ridge point doubles."}
...
data = pd.read_csv("vol2/performance_engineering/data/memory_wall_trend.csv")
base = data.iloc[0]
...
ax.plot(labels, data["Compute"], marker="o", color=COLORS["RedLine"],
        linewidth=2, label="Peak compute")
ax.plot(labels, data["Bandwidth"], marker="s", color=COLORS["BlueLine"],
        linewidth=2, label="Memory bandwidth")

for col, colour in [("Compute", COLORS["RedLine"]), ("Bandwidth", COLORS["BlueLine"])]:
    ax.annotate(f'{data[col].iloc[-1]:.0f}x', xy=(len(data) - 1, data[col].iloc[-1]),
...
[^fn-paged-attention-os]: **PagedAttention**\index{PagedAttention!memory management}: The name draws a direct analogy to operating-system virtual memory, which maps noncontiguous physical pages into a contiguous virtual address space.
...
Fusion nevertheless has a resource ceiling. A larger fused kernel keeps more live values in registers and often reserves more shared memory per thread block. Either demand can reduce the number of resident warps, lowering occupancy and the latency-hiding capacity of the SM; excessive register demand can also spill values into local memory backed by the cache hierarchy or HBM. Fusion is profitable only when the avoided launches and HBM intermediates outweigh this loss of occupancy, spill traffic, and any duplicated computation. Kernel evaluation must therefore compare measured HBM bytes, register and shared-memory use, achieved occupancy, spill transactions, and end-to-end time rather than treating fewer launches as sufficient evidence.
...
### CUDA graphs: Amortizing launch overhead {#sec-performance-engineering-cuda-graphs}

An orthogonal technique for reducing the overhead term in the iron law is **CUDA graphs**\index{CUDA!graphs, overhead reduction}.[^fn-cuda-graphs-constraint] While operator fusion combines multiple operations into fewer kernels, CUDA Graphs amortize repeated CPU submission of a stable kernel sequence.
...
::: {.column-margin}
![](images/svg/vol2_performance_engineering_margin_003.svg){width="100%" fig-alt="Batch-capacity ladder comparing 407 requests with an FP16 baseline against 971 requests with INT4 weights and an INT8 KV cache, a 2.4-times increase."}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_count, fmt_rate, fmt_time, check
...
An **open-loop load generator**\index{Open-loop load generation} schedules requests independently of responses. A **closed-loop load generator**\index{Closed-loop load generation} issues replacements only as earlier requests finish. The modes hold attempted arrivals and outstanding work fixed, respectively.

At `{python} CoordinatedOmissionExample.intended_rate_str`, a `{python} CoordinatedOmissionExample.stall_duration_str` open-loop schedule contains `{python} CoordinatedOmissionExample.intended_arrivals_str`. At concurrency one, the closed loop records `{python} CoordinatedOmissionExample.slow_completion_str` but issues no replacements, omitting `{python} CoordinatedOmissionExample.suppressed_samples_str`. This **coordinated omission**\index{Coordinated omission} makes overload look less severe.
...
For the decode phase, FlashAttention's impact is more modest because decode attention is dominated by KV cache reads rather than the $S{\times}S$ score matrix. However, the combination of INT8 KV cache compression and the tiled attention kernel reduces attention decode time by approximately 30 percent in this scenario. The `reduce-overhead` mode in `torch.compile` captures the stable decode region in a CUDA Graph, reducing the measured launch-gap contribution from 12 percent to a residual graph-replay and uncaptured-runtime cost.
...
The result is a 3.1$\times$ improvement over the Round 2 ITL of 32 ms. Speculative decoding nevertheless interacts with batching. In this scenario, verification loses its compute headroom as the batch approaches `{python} KVCacheAnalysis.optimized_policy_batch_size_str`; the measured latency-throughput crossover occurs near batch size `{python} KVCacheAnalysis.speculation_policy_batch_size_str`. That number is a workload- and hardware-specific physical input, not a universal admission threshold. @Sec-inference-scale shows how a serving controller combines such measured crossover curves with offered load, cache state, and service objectives to decide whether a request cohort should use speculation.
...
Speculative decoding can reduce latency in a measured low-batch, memory-bound regime when the draft model is cheap, its proposals achieve a high acceptance rate, and the target has spare compute for batched verification. Verification is never free; its exposed cost depends on draft length, acceptance, batch and sequence distributions, and the target kernel's operating point. As batching consumes compute headroom, the same configuration can cross from a gain to a loss, while variable accepted lengths introduce additional scheduler work. This chapter measures that crossover for a fixed workload and machine; @sec-inference-scale owns the admission policy that applies the resulting curve under changing offered load and service objectives.
...
2. **Reducing Overhead ($L_{\text{lat}}$)**: Graph compilers fuse eligible operations and improve memory planning, while CUDA Graphs amortize repeated host submission for stable captured regions. At the cluster boundary, communication-computation overlap can hide inter-GPU AllReduce latency under useful backward-pass GEMMs when resource interference does not lengthen that compute.
...
With hardware compute and memory hierarchy fundamentals established in @sec-compute-infrastructure, this chapter focused on local execution mechanics: operator fusion, precision reduction, graph compilation, speculative verification, and sparse routing. Its measurements hold workload and offered load fixed so that changes in bytes, operations, and exposed overhead remain attributable to the implementation. @Sec-inference-scale consumes those measured mechanisms and owns the production policies around cache state, admission, routing, batching, scheduling, and SLO protection.
