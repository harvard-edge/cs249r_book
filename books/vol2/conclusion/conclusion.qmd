...
::: {#fig-conclusion-memory-wall fig-env="figure" fig-pos="htb" fig-cap="**The memory wall widens with every generation**: Peak compute and memory bandwidth for NVIDIA data center accelerators, normalized to the V100. Compute rose 18$\times$ between 2017 and 2024 while bandwidth rose 8.9$\times$, so the distance a decode step must cross roughly doubled over four generations. Specifications come from the MLSysIM hardware registry." fig-alt="Line chart of two series normalized to the V100. Peak compute climbs to 18 times by the B200 while memory bandwidth reaches 8.9 times, and the widening vertical gap between the two lines is the memory wall."}
...
data = pd.read_csv("vol2/conclusion/data/gpu_memory_wall.csv")
base = data.iloc[0]
