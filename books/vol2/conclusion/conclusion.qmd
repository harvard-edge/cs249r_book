...
[^fn-gradient-compression-allreduce]: **Gradient Compression**\index{Gradient Compression}: @Sec-collective-communication explores techniques that reduce communication volume 10--100$\times$ through sparsification, quantization, and error feedback, enabling bandwidth-limited distributed and federated training.
...
[^fn-lifecycle-assessment-sustainability]: **Lifecycle Assessment**\index{Lifecycle Assessment}: @Sec-sustainable-ai introduces a method for evaluating environmental impact across a system's entire lifespan, including embodied carbon in hardware manufacturing (often 20--50 percent of total impact).
...
*   **Brain synapses**: `{python} FermiEstimate.brain_synapses_math` synapses (connections).
*   **Brain firing rate**: Illustrative average spike rate $\approx$ `{python} FermiEstimate.brain_firing_rate_str`; estimates vary by neuron type and brain region, and average cortical firing is far below the 100 Hz peak rates often used in casual comparisons.
*   **Brain synaptic operation rate**: `{python} FermiEstimate.brain_ops_calc_math` = `{python} FermiEstimate.brain_ops_math` synaptic ops/s.
...
*A data center draws megawatts; the brain runs on ~`{python} FermiEstimate.brain_power_w_str`.*
:::
