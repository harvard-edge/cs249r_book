...
In 2012, training AlexNet took five to six days on two GPUs [@krizhevsky2012imagenet]. By 2023, public GPT-4-class estimates placed frontier training at roughly 25,000 GPUs running for about three months [@semianalysisGPT4]; OpenAI did not disclose the actual hardware configuration [@openai2023gpt4]. This is not the same engineering problem at a larger scale. It is a categorically different engineering problem.

On a single machine, performance is governed by the memory wall, the gap between processor speed and memory bandwidth. In a distributed cluster, a new wall emerges: the bisection bandwidth wall, where the minimum network bandwidth that bisects the cluster can cap throughput for traffic that must cross that cut. Data no longer moves through local hierarchies alone; it traverses optical fabrics governed by the speed of light between racks and across continents. Hardware failures, rare enough on a single machine to be treated as exceptions, become routine statistical events at fleet scale. A training job that does not plan for failure *will* fail.
...
*   **The Fairness Impossibility Law**: Except in special cases such as perfect prediction or equal base rates, calibrated risk scores generally can not also equalize error rates across groups. Demographic parity is a separate selection-rate constraint that can introduce additional trade-offs.
...
The full stack above shows each row with its scope, grouped into the four parts of the book. The Roman numerals (I--IV) mark those parts; the row intensity marks emphasis. The stack uses one accent color throughout so that shade, not hue, carries the chapter-focus signal.
...
::: {layout-ncol=4}
![](../02_compute_infrastructure/images/svg/compute_infrastructure_cxl_bandwidth_gap.svg){width="100%" fig-alt="Margin visual example: a log-scale bandwidth ladder comparing HBM3 with CXL3 and annotating the ratio."}

![](../03_network_fabrics/images/svg/network_fabrics_pfc_pause_blast.svg){width="100%" fig-alt="Margin visual example: a blast-radius fan showing one pause event propagating to multiple dependent flows."}

![](../04_data_storage/images/svg/data_storage_checkpoint_storm_write_time.svg){width="100%" fig-alt="Margin visual example: a log-scale time ladder comparing naive checkpoint write time with ZeRO-3 write time."}

![](../15_sustainable_ai/images/svg/vol2_sustainable_ai_margin_004.svg){width="100%" fig-alt="Margin visual example: an energy ladder showing radio transmission dwarfing local arithmetic on a log scale."}
:::
...
- **The Complete Path** (all chapters, in sequence): Follows the causal chain of constraints that governs the ML fleet from first principles of distribution. Start with Part I, then progress through Parts II, III, and IV.
- **The Infrastructure Path** (physical fleet and orchestration): Focuses on the physical and operational substrate. Focus on Part I (@sec-compute-infrastructure, @sec-network-fabrics, @sec-data-storage) and @sec-fleet-orchestration.
- **The Scaling Path** (distributed training depth): Skims Part I for infrastructure vocabulary, then focuses on @sec-distributed-training-systems, @sec-collective-communication, @sec-fault-tolerance-reliability, and @sec-performance-engineering.
- **The Production Path** (deployment and operations): After the introduction, focuses on @sec-inference-scale, @sec-ops-scale, @sec-edge-intelligence, and the Part IV Responsible Fleet chapters (@sec-security-privacy, @sec-robust-ai, @sec-sustainable-ai, and @sec-responsible-ai).

{{< include ../../vol1/frontmatter/_conventions.qmd >}}
...
::: {.content-visible when-format="html"}
Slides, assignments, and instructor materials for the advanced sequence are available at [mlsysbook.ai](https://mlsysbook.ai).
:::
