...
\noindent
![](images/png/cover_responsible_ai_blueprint_labeled_print.png){fig-alt="Fleet governance overlay wrapping data, training, serving, monitoring, policy, fairness, privacy, audit, accountability, and incident response."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_responsible_ai_blueprint_labeled.webp){fig-alt="Fleet governance overlay wrapping data, training, serving, monitoring, policy, fairness, privacy, audit, accountability, and incident response."}
:::
...
In high-risk regulated settings, a model that cannot explain or document its behavior, support audit, or meet applicable discrimination rules may fail deployment review regardless of its accuracy or latency. Legal and governance requirements can make transparency, oversight, documentation, and risk management part of deployment readiness. Responsible AI engineering implements these requirements through measurable controls rather than treating them as a final ethical review. In C³ terms, responsibility is a coordination constraint on the fleet. Required safety, fairness, and accountability controls must be established before deployment.
...
- Construct an auditable assurance lifecycle from intended use and evidence through launch, remedy, material change, and retirement
...
Software engineering provides precedent for this evolution. Early systems prioritized functional correctness alone. As complexity grew, the field developed methodologies for reliability engineering, security assurance, and maintainability analysis. ML deployment creates a comparable maturation pressure, but at larger social scale: models can mediate credit allocation, medical diagnosis, educational assessment, and criminal justice. Responsible AI failures can perpetuate systemic discrimination, compromise democratic institutions, and erode public confidence in beneficial technologies even when the underlying service remains available.
...
The resulting assurance case is not a single compliance document. It is a versioned set of linked artifacts:

1. **Claim**: The property required for a specified use and operating envelope.
2. **Argument**: The reasoning that connects controls and evidence to the claim.
3. **Evidence**: Evaluations, audits, logs, documentation, and user research that can be reproduced or inspected.
4. **Assumptions**: Environmental, population, supplier, and workflow conditions under which the claim holds.
5. **Owner and authority**: The person or body accountable for the claim and empowered to block launch, restrict use, or require remediation.
6. **Invalidation rule**: The measurable change or incident that expires the claim and triggers reassessment.
...
Named frameworks organize this work at different levels. The NIST AI Risk Management Framework structures activity around Govern, Map, Measure, and Manage; ISO/IEC 42001 specifies an organizational AI management system; ISO/IEC 23894 supplies AI risk-management guidance; and the OECD and UNESCO recommendations state broader principles and public responsibilities [@nist2023airmf; @isoiec420012023ai; @isoiec238942023ai; @oecd2024airecommendation; @unesco2022ethicsai]. @Tbl-responsible-ai-framework-crosswalk maps those perspectives onto the same operating lifecycle rather than treating them as interchangeable checklists.
...
Fairness operates as a stability constraint\index{Fairness!stability constraint}. In control theory terms, fairness ensures that the system's error distribution is invariant across population subgroups. A system that violates this constraint is unstable: it will degrade its own training data through feedback loops (for example, predictive policing) and lose user trust, leading to eventual system collapse. This principle encompasses both statistical metrics and broader normative concerns about equity, justice, and structural bias. The fairness analysis in @sec-responsible-ai-fairness-machine-learning-2ba4 examines the formal mathematical definitions in detail.

The same control-plane view makes **Explainability**\index{Explainability} one source of system evidence: attribution, examples, counterfactual analysis, and interpretable-model structure can expose patterns that human operators investigate [@phillips2020]. These methods do not reveal a model's complete causal reasoning, justify a decision, guarantee recourse, or prove that a safety constraint holds. Their value depends on a specified audience and decision: developers may need evidence for error analysis, auditors may need reproducible behavioral tests, and affected users may need understandable reasons plus a route to contest an outcome. Explanation therefore complements telemetry, documentation, evaluation, and human review rather than replacing them.
...
At the objective-function layer, **Value Alignment**\index{Value Alignment}[^fn-value-alignment-safety] asks whether AI systems pursue goals consistent with legitimate human intent and constraints. In practice, this involves technical challenges, including reward design and constraint specification, and broader questions about whose values are represented and enforced. Alignment does not subsume fairness: the former concerns fidelity to an objective, whereas the latter evaluates how outcomes and burdens are distributed and whether the objective itself is legitimate.
...
The lifecycle also needs **Human Oversight**\index{Human Oversight}: the role of human judgment in supervising, correcting, or halting automated decisions. This includes humans in the loop[^fn-hitl-human-oversight] during operation, as well as organizational structures that ensure AI use remains accountable to societal values and real-world complexity.

[^fn-hitl-human-oversight]: **HITL (Human-in-the-Loop)**\index{Human-in-the-Loop!systems overhead}: A design pattern where humans actively participate in model decisions rather than being replaced by automation; the systems trade-off is latency vs. safety: HITL adds 100 ms to 30+ seconds per decision depending on domain. Large content-moderation systems have historically paired automated triage with thousands of human reviewers, demonstrating that the pattern scales only with proportional human infrastructure cost. In ML serving architectures, HITL requires routing logic, confidence thresholds for escalation, and queue management that fundamentally reshape the inference pipeline.
...
@Tbl-principles-lifecycle maps the responsible AI lifecycle\index{Responsible AI!lifecycle} across the major phases of ML system development: data collection, model training, evaluation, deployment, and monitoring.
...
Fairness and privacy constraints begin at data collection; robustness and accountability become most critical during deployment and oversight. Explainability spans the full lifecycle, supporting model debugging at design time and user-facing justification at serving time. This mapping reinforces that responsible AI is a multiphase architectural commitment, not a post-hoc compliance step.
...
Explainability is the practice of producing evidence intended to help a specified audience understand aspects of a model's behavior. It includes both **Local Explanations**\index{Local Explanation}, which characterize influences or sensitivities around individual predictions, and **Global Explanations**\index{Global Explanation}, which summarize broader behavioral patterns. An attribution describes how a method assigns influence under its assumptions; it is not automatically a causal explanation, a normative justification, or a feasible path to recourse. Transparency, by contrast, encompasses openness about the broader system design and operation. This includes disclosure of data sources, feature engineering, model architectures, training procedures, evaluation protocols, known limitations, intended uses, system boundaries, and governance structures.

The importance of explainability and transparency extends beyond technical considerations to legal requirements. In many jurisdictions, these principles are legal obligations rather than optional guidance. Under GDPR, solely automated decisions with legal or similarly significant effects trigger Article 22 safeguards and related transparency duties, including meaningful information about the logic involved under access and notice provisions, subject to statutory exceptions [@gdpr2016; @edpb2018automatedProfiling].[^fn-gdpr-article-22] Similar regulatory pressures in other domains reinforce the need to treat explainability and transparency as core architectural requirements.
...
1.  **Significance**: It transforms fairness from an intuition into a multi-objective optimization problem. Within the iron law, achieving fairness often requires trading off total accuracy $(\text{Accuracy})$ for group-specific calibration\index{Calibration!group-specific}, where calibration is checked separately within each protected group, ensuring that the system's benefits and harms are distributed equitably.
...
3.  **Common pitfall**: A frequent misconception is that there is a single "fair" solution. In reality, different fairness definitions (for example, demographic parity vs. equalized odds) are often mathematically incompatible\index{Fairness!incompatibility} and not jointly satisfiable except in special cases: satisfying one necessitates violating another, requiring explicit policy choices by the engineer.
...
The approval-rate facts define the policy change; the scenario's accuracy values are measured on the held-out validation set, where the additional Group B approvals carry higher default risk.
...
***Demographic Parity***\index{Demographic Parity!definition} is the fairness constraint where a model's positive prediction rate is independent of group membership $(\Pr(h(x)=1 \mid A=a) = \Pr(h(x)=1 \mid A=b))$.
...
In the running example, this measure would ensure that among patients who do require care, both Black and white individuals have an equal chance of being identified by the model. In the case of the US hospital system, the algorithm's use of healthcare expenditure as a proxy variable led to a failure in meeting this criterion: Black patients with significant health needs were less likely to receive care due to their lower historical spending. The following worked example uses the loan decision scenario introduced at the start of this section to calculate all three criteria, showing why a single model can fail each fairness test in a different way.
...
1. **Group A approval rate**: $\Pr(h(x) = 1 \mid A = a) = \frac{70}{100} = 0.70$.
2. **Group B approval rate**: $\Pr(h(x) = 1 \mid A = b) = \frac{40}{100} = 0.40$.
...
The TPR values are calculated in the preceding step. For false positive rates among applicants who *would not repay* ($Y=0$):
\begin{gather*}
...
The preceding loan approval worked example revealed that this model violates all three fairness criteria simultaneously. This is not merely poor model design but reflects a fundamental mathematical tension that any classifier must confront when base rates differ between groups. These tensions motivate the impossibility results that constrain what any fair classifier can achieve.

These definitions capture different aspects of fairness and are generally incompatible[^fn-fairness-impossibility-theorem] [@kleinberg2016inherent; @chouldechova2017]. A university admissions example illustrates the tension concretely.
...
\begin{tikzpicture}[
    font=\small,
    >=Stealth,
    circledef/.style={circle, minimum size=5.8cm, draw=gray!40, line width=1.5pt},
    box/.style={draw=BorderGray, fill=white, rounded corners=3pt, line width=1pt, align=center, inner sep=6pt}
]

    % Coordinates of the three circle centers
    \def\r{2.9cm}
    \coordinate (C1) at (-1.8, 1.3);   % Top Left
    \coordinate (C2) at (1.8, 1.3);    % Top Right
    \coordinate (C3) at (0, -1.5);     % Bottom

    % Fill center intersection (C1 and C2 and C3)
    \begin{scope}
        \clip (C1) circle (\r);
        \clip (C2) circle (\r);
        \fill[LightRed] (C3) circle (\r);
    \end{scope}

    % Draw the 3 circles
    \draw[circledef] (C1) circle (\r);
    \draw[circledef] (C2) circle (\r);
    \draw[circledef] (C3) circle (\r);

    % Central Cross (Impossibility X)
    \coordinate (Center) at (0, 0.35);
    \draw[line width=2.8pt, mlsysred, line cap=round] ($(Center)+(-0.35,-0.35)$) -- ($(Center)+(0.35,0.35)$);
    \draw[line width=2.8pt, mlsysred, line cap=round] ($(Center)+(-0.35,0.35)$) -- ($(Center)+(0.35,-0.35)$);

    % --- Top Left Circle: Demographic Parity ---
    \node[align=center, text width=3.4cm] at (-2.8, 2.0) {
        {\bfseries\large\color{darknavy} Demographic\\[1pt] Parity}\\[6pt]
        {\footnotesize\color{MutedGray!90!black} $\Pr(h(X)=1 \mid G=a)$\\[2pt]
        $= \Pr(h(X)=1 \mid G=b)$}
    };

    % --- Top Right Circle: Equalized Odds ---
    \node[align=center, text width=3.4cm] at (2.8, 2.0) {
        {\bfseries\large\color{darknavy} Equalized\\[1pt] Odds}\\[6pt]
        {\footnotesize\color{MutedGray!90!black} Equal TPR and FPR\\[2pt] across groups}
    };

    % --- Bottom Circle: Calibration ---
    \node[align=center, text width=4.0cm] at (0, -2.6) {
        {\bfseries\large\color{darknavy} Calibration}\\[6pt]
        {\footnotesize\color{MutedGray!90!black} $\Pr(Y=1 \mid \text{score}=s, G=g) = s$\\[2pt]
        for all groups $g$}
    };

    % --- Intersections Text ---
    % Top intersection: C1 and C2
    \node[align=center, font=\itshape\footnotesize\color{MutedGray!90!black}] at (0, 2.5) {
        Achievable only if\\[1pt] base rates equal
    };

    % Left intersection: C1 and C3
    \node[align=center, font=\itshape\footnotesize\color{MutedGray!90!black}] at (-1.7, -0.3) {
        Violates\\[1pt] Eq. Odds
    };

    % Right intersection: C2 and C3
    \node[align=center, font=\itshape\footnotesize\color{MutedGray!90!black}] at (1.7, -0.3) {
        Violates\\[1pt] Dem. Parity
    };

    % ================= BOTTOM CARDS & TAKEAWAY =================
    \def\yBot{-5.2}

    % Group A box
    \node[box, text width=2.6cm] (grpA) at (-3.7, \yBot) {
        {\bfseries\color{darknavy} Group A}\\[2pt]
        {\footnotesize\color{MutedGray} Base rate: 30\%\\[1pt]
        $\Pr(Y=1) = 0.30$}
    };

    % Not-equal symbol between groups
    \node[font=\LARGE\bfseries\color{mlsysred}] at (-2.0, \yBot) {$\neq$};

    % Group B box
    \node[box, text width=2.6cm] (grpB) at (-0.3, \yBot) {
        {\bfseries\color{darknavy} Group B}\\[2pt]
        {\footnotesize\color{MutedGray} Base rate: 55\%\\[1pt]
        $\Pr(Y=1) = 0.55$}
    };

    % Red accent line
    \draw[line width=1.8pt, mlsysred] (1.5, \yBot) -- (2.3, \yBot);

    % Takeaway text
    \node[anchor=west, align=left, font=\bfseries\footnotesize\color{mlsysred}] at (2.5, \yBot) {
        When base rates differ,\\[2pt]
        satisfying any two\\[2pt]
        violates the third.
    };
...
@Fig-fairness-metric-disagreement makes this trade-off concrete by sweeping a classification threshold across a synthetic scenario with differing group base rates. Across the evaluated nontrivial operating range, at least one of the plotted selection-rate or error-rate metrics is substantially violated, illustrating the broader operational lesson that fairness criteria can conflict and require explicit prioritization.

::: {#fig-fairness-metric-disagreement fig-env="figure" fig-pos="htb" fig-cap="**Fairness Metric Disagreement Across Thresholds**: Three fairness metrics, demographic parity, equalized odds, and equal opportunity, computed on a classifier with differing group base rates as the classification threshold varies over a nontrivial operating range. At every evaluated threshold, at least one metric is substantially violated, illustrating practical trade-offs among selection-rate, error-rate, and opportunity criteria. Trivial always-positive or always-negative classifiers fall outside the evaluated range." fig-alt="Line plot showing three fairness metrics varying over a nontrivial classification-threshold range. No evaluated threshold brings all three metrics to zero simultaneously."}
...
# Check if there is any threshold where all three are below 0.05
...
A causal claim adds a **structural causal model**\index{Structural causal model!fairness} whose arrows state how protected attribute $A$, mediators $M$, outcome $Y$, and decision $\widehat Y$ are assumed to relate. **Counterfactual fairness**\index{Counterfactual fairness!definition} then asks whether the decision for the same individual, holding the modeled background factors fixed, would have changed in a counterfactual world where $A$ had taken another value [@kusner2017counterfactual]. This is a question about an assumed data-generating process, not a stronger version of demographic parity.

The healthcare proxy illustrates the distinction. A graph might include race $A \rightarrow$ access to care $M \rightarrow$ expenditure $X \rightarrow$ risk score $\widehat Y$, alongside pathways from health need $Y$ to expenditure. A **total effect**\index{Causal effect!total, fairness} asks how changing $A$ propagates through every modeled path. A **path-specific effect**\index{Causal effect!path-specific, fairness} isolates selected routes, such as the historically mediated access-to-expenditure path [@nabi2018fair]. Declaring one path permissible and another discriminatory is a domain and normative judgment; neither the graph nor the estimator makes that choice automatically.

Causal conclusions require assumptions that observational data alone cannot establish: which variables and arrows belong in the graph, whether relevant confounders are measured, and whether the counterfactual interventions are coherent. Different plausible structures can support different answers. Causal analysis therefore complements group metrics, domain knowledge, legal analysis, and stakeholder participation. The metrics continue to monitor realized disparities; the causal model makes the additional mechanism claim and its assumptions inspectable.
...
|:-------------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------------------------|:------------------------------------------------------------------------------------------|:---------------------------------------------:|:----------------------------------------------------------------------------------------------------------|
| Disparate impact (DI) ratio (four-fifths rule)\index{Disparate Impact Ratio!definition}\index{Four-Fifths Rule} [@feldman2015]             | lower approval rate divided by higher approval rate | $\dfrac{\min_g \Pr(h=1 \mid g)}{\max_g \Pr(h=1 \mid g)}$                                  | `{python} FairnessMetricsTable.di_ratio_math` | a legally recognized screen is needed; $\text{DI} < 0.8$ flags disparate impact                           |
| Statistical parity difference (SPD)\index{Statistical Parity Difference!definition} [@calders2010]                                         | additive gap in approval rates                      | $\Pr(h=1 \mid a) - \Pr(h=1 \mid b)$                                                       |    `{python} FairnessMetricsTable.spd_math`   | comparing across many groups or tracking drift; common audit threshold $\lvert\text{SPD}\rvert \leq 0.10$ |
...
| Average absolute odds difference\index{Average Absolute Odds Difference!definition} [@hardt2016equality; @bellamy2019; @bird2020fairlearn] | both true-positive and false-positive rates         | $\tfrac{1}{2}\big[\,\lvert \Delta\text{TPR}\rvert + \lvert \Delta\text{FPR}\rvert\,\big]$ |    `{python} FairnessMetricsTable.aod_math`   | both error types matter; the metric is 0 at perfect equalized odds                                        |

: **Fairness Metrics on the Loan Example**: Four group-fairness metrics, each equalizing a different quantity, evaluated on the loan approval model. The disparate impact ratio is the legally recognized screening test; the additive differences are easier to track over time; the average absolute odds difference captures both true-positive and false-positive error gaps. {#tbl-responsible-ai-fairness-metrics tbl-colwidths="[20,22,33,12,13]"}

The four metrics disagree by construction: the disparate impact ratio of `{python} FairnessMetricsTable.di_ratio_str` means the lower-approval group receives approvals at only that fraction of the higher-approval group's rate, already breaching the four-fifths rule. The additive `{python} FairnessMetricsTable.spd_str` gap, the `{python} FairnessMetricsTable.eod_str` opportunity gap, and the `{python} FairnessMetricsTable.aod_str` average absolute odds gap each quantify a distinct dimension of the same disparity. No single number is sufficient, which is why the impossibility results in this section force an explicit choice about which metric a deployment will prioritize.

##### Calibration {#sec-responsible-ai-calibration-8b73}

A model satisfies **Score Calibration**\index{Calibration!score} with respect to a sensitive attribute if, among individuals assigned score $s$ by the model, the fraction with positive outcomes is equal across groups [@kleinberg2016inherent]:
$$
\Pr(Y = 1 \mid s(x) = s, A = a) = \Pr(Y = 1 \mid s(x) = s, A = b), \quad \forall s
$$

Score calibration is a per-score condition; for hard binary predictions, one related parity condition is equal **Positive Predictive Value (PPV)**\index{Positive Predictive Value} (precision) among predicted positives, while score calibration is the stronger requirement that the equality holds at every score level:
$$
...
Defining these metrics is the easy part; computing them continuously across a deployed fleet is where the engineering cost appears. Computing these metrics at production scale introduces computational, data, and organizational costs that shape which metrics a system can afford to monitor before setting threshold trade-offs or conducting significance testing.
...
class CostOfFairness:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(abs(tax - 0.04) < 0.001, f"Expected 0.04 (4.0 pp) fairness tax, got {tax}")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    tax_pp_str = fmt_pp(tax * 100, precision=0, commas=False)
    tax_pp_attr_str = fmt_pp(tax * 100, precision=0, style="prose", attributive=True)
...
::: {#nbk-responsible-ai-engineering-metric-cost-fairness .callout-notebook title="Engineering metric: The cost of fairness"}

**Problem**: Quantify the overall accuracy drop ("fairness tax") when imposing Demographic Parity on a credit scoring model across Group A (majority) and Group B (minority).

**Variables**:

- Group A: Mean score 70, repayment rate high, unconstrained threshold = 60.
- Group B: Mean score 50, repayment rate lower, unconstrained threshold = 60.

**Math**:

1. **Unconstrained optimization**: Single threshold = 60 yields overall accuracy = `{python} CostOfFairness.acc_unconstrained_pct_str`.
2. **Demographic parity constraint**: Equalizing Group B approval with Group A requires lowering Group B threshold to 50.
3. **Fairness tax**: Overall accuracy drops to `{python} CostOfFairness.acc_constrained_pct_str`, incurring a `{python} CostOfFairness.tax_pp_attr_str` absolute accuracy penalty (`{python} CostOfFairness.tax_relative_pct_str` relative drop).

**Systems insight**: The "Cost of Fairness" is `{python} CostOfFairness.tax_pp_str` of accuracy. System designers must weigh this measured accuracy trade-off against social equity gains when configuring production thresholding rules.
...
- Group A ($Y=1$): $\mu_A^+ = 0.72$, $\sigma_A^+ = 0.15$.
- Group B ($Y=1$): $\mu_B^+ = 0.65$, $\sigma_B^+ = 0.16$.

**Math**:

1. **Single threshold ($\tau = 0.60$)**: Yields $\text{TPR}_a \approx 0.79$ vs. $\text{TPR}_b \approx 0.62$ (a 17 percentage point gap).
2. **Equalizing thresholds**: Lowering Group B's threshold to $\tau_b = 0.52$ while holding $\tau_a = 0.60$ equalizes $\text{TPR}_b \approx 0.79$.
3. **Trade-off**: Group B's false positive rate increases from 0.12 to 0.24, dropping precision for Group B from 0.78 to 0.69.

**Systems insight**: Achieving Equal Opportunity through threshold adjustment increases false positives and lowers precision for the group receiving the lower threshold. Threshold policies do not alter underlying score calibration, requiring designers to weigh opportunity equity against precision degradation.
...
With `{python} FairnessZTest.z_math` (far exceeding critical value $z_{0.05/2} = 1.96$), the test rejects $H_0$ and concludes the demographic parity violation is statistically significant at $p < 0.001$. Similar tests can be constructed for equal opportunity and equalized odds by restricting to subpopulations where $Y=1$ or $Y=0$ respectively. Statistical significance does not imply practical significance; even statistically significant disparities may be acceptable if the magnitude is small. Conversely, large disparities in small samples may not reach statistical significance but still warrant intervention.

The quantitative framework developed here transforms fairness from an abstract principle into measurable engineering constraints. Establishing metrics, thresholds, and statistical tests allows practitioners to systematically evaluate fairness throughout the ML lifecycle and make data-driven decisions about when intervention is required.
...
% Card 1: Input Domain
\node[draw=mlsysred!80, fill=white, rounded corners=6pt, line width=1.1pt, minimum width=\cardW cm, minimum height=\cardH cm] (c1) at (0, 0) {};
\node[fill=mlsysred!15, text=mlsysred!90!black, font=\bfseries\scriptsize, rounded corners=3pt, minimum width=\cardW cm - 4pt, minimum height=18pt, anchor=north] at ([yshift=-2pt]c1.north) {INPUT DOMAIN};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-26pt]c1.north) {Raw Data};
\draw[mlsysgray!30, line width=0.8pt] ([xshift=12pt, yshift=-44pt]c1.north west) -- ([xshift=-12pt, yshift=-44pt]c1.north east);
\node[font=\footnotesize, text=mlsysdark!90, align=center, text width=2.6cm, anchor=north] at ([yshift=-50pt]c1.north) {
    Local user records\\[3pt]
    PII \& telemetry\\[3pt]
    Sensitive attributes
};
\node[fill=mlsysred!15, rounded corners=3pt, font=\bfseries\scriptsize, text=mlsysred!90!black, minimum width=2.5cm, minimum height=15pt, anchor=south] at ([yshift=6pt]c1.south) {High Privacy Risk};

% Card 2: Perturbation
\node[draw=mlsysblue!80, fill=white, rounded corners=6pt, line width=1.1pt, minimum width=\cardW cm, minimum height=\cardH cm] (c2) at (3.5, 0) {};
\node[fill=mlsysblue!15, text=mlsysblue!90!black, font=\bfseries\scriptsize, rounded corners=3pt, minimum width=\cardW cm - 4pt, minimum height=18pt, anchor=north] at ([yshift=-2pt]c2.north) {PERTURBATION};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-26pt]c2.north) {Diff.\ Privacy};
\draw[mlsysgray!30, line width=0.8pt] ([xshift=12pt, yshift=-44pt]c2.north west) -- ([xshift=-12pt, yshift=-44pt]c2.north east);
\node[font=\footnotesize, text=mlsysdark!90, align=center, text width=2.6cm, anchor=north] at ([yshift=-50pt]c2.north) {
    Calibrated noise\\[3pt]
    Laplace / Gaussian\\[3pt]
    Gradient clipping
};
\node[fill=mlsysblue!15, rounded corners=3pt, font=\bfseries\scriptsize, text=mlsysblue!90!black, minimum width=2.5cm, minimum height=15pt, anchor=south] at ([yshift=6pt]c2.south) {$(\varepsilon, \delta)$-DP Bound};

% Card 3: Decentralized
\node[draw=mlsysgreen!80, fill=white, rounded corners=6pt, line width=1.1pt, minimum width=\cardW cm, minimum height=\cardH cm] (c3) at (7.0, 0) {};
\node[fill=mlsysgreen!15, text=mlsysgreen!90!black, font=\bfseries\scriptsize, rounded corners=3pt, minimum width=\cardW cm - 4pt, minimum height=18pt, anchor=north] at ([yshift=-2pt]c3.north) {DECENTRALIZED};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-26pt]c3.north) {Federated Learn};
\draw[mlsysgray!30, line width=0.8pt] ([xshift=12pt, yshift=-44pt]c3.north west) -- ([xshift=-12pt, yshift=-44pt]c3.north east);
\node[font=\footnotesize, text=mlsysdark!90, align=center, text width=2.6cm, anchor=north] at ([yshift=-50pt]c3.north) {
    On-device compute\\[3pt]
    Local updates $\Delta w$\\[3pt]
    Data stays on edge
};
\node[fill=mlsysgreen!15, rounded corners=3pt, font=\bfseries\scriptsize, text=mlsysgreen!90!black, minimum width=2.5cm, minimum height=15pt, anchor=south] at ([yshift=6pt]c3.south) {Isolated Raw Data};

% Card 4: Cryptographic
\node[draw=mlsysamber!80, fill=white, rounded corners=6pt, line width=1.1pt, minimum width=\cardW cm, minimum height=\cardH cm] (c4) at (10.5, 0) {};
\node[fill=mlsysamber!15, text=mlsysamber!90!black, font=\bfseries\scriptsize, rounded corners=3pt, minimum width=\cardW cm - 4pt, minimum height=18pt, anchor=north] at ([yshift=-2pt]c4.north) {CRYPTOGRAPHIC};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-26pt]c4.north) {SecAgg};
\draw[mlsysgray!30, line width=0.8pt] ([xshift=12pt, yshift=-44pt]c4.north west) -- ([xshift=-12pt, yshift=-44pt]c4.north east);
\node[font=\footnotesize, text=mlsysdark!90, align=center, text width=2.6cm, anchor=north] at ([yshift=-50pt]c4.north) {
    Zero-sum masks\\[3pt]
    Encrypted sums\\[3pt]
    Server sees $\sum \Delta w$
};
\node[fill=mlsysamber!15, rounded corners=3pt, font=\bfseries\scriptsize, text=mlsysamber!90!black, minimum width=2.5cm, minimum height=15pt, anchor=south] at ([yshift=6pt]c4.south) {Zero Client Leakage};

% Card 5: Result Artifact
\node[draw=mlsysgreen!90!black, fill=white, rounded corners=6pt, line width=1.1pt, minimum width=\cardW cm, minimum height=\cardH cm] (c5) at (14.0, 0) {};
\node[fill=mlsysgreen!15, text=mlsysgreen!80!black, font=\bfseries\scriptsize, rounded corners=3pt, minimum width=\cardW cm - 4pt, minimum height=18pt, anchor=north] at ([yshift=-2pt]c5.north) {RESULT ARTIFACT};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-26pt]c5.north) {Global Model};
\draw[mlsysgray!30, line width=0.8pt] ([xshift=12pt, yshift=-44pt]c5.north west) -- ([xshift=-12pt, yshift=-44pt]c5.north east);
\node[font=\footnotesize, text=mlsysdark!90, align=center, text width=2.6cm, anchor=north] at ([yshift=-50pt]c5.north) {
    FedAvg consensus\\[3pt]
    Broadcast to fleet\\[3pt]
    Certified bounds
};
\node[fill=mlsysgreen!15, rounded corners=3pt, font=\bfseries\scriptsize, text=mlsysgreen!80!black, minimum width=2.5cm, minimum height=15pt, anchor=south] at ([yshift=6pt]c5.south) {Privacy Preserved};

% Connectors between cards
\draw[->, mlsysgray!70, line width=1.6pt] (c1.east) -- (c2.west);
\draw[->, mlsysgray!70, line width=1.6pt] (c2.east) -- (c3.west);
\draw[->, mlsysgray!70, line width=1.6pt] (c3.east) -- (c4.west);
\draw[->, mlsysgray!70, line width=1.6pt] (c4.east) -- (c5.west);

% Middle section: Observability bar
\def\barY{-2.8}
\node[font=\bfseries\footnotesize, text=mlsysred!90!black, anchor=west] at (-1.5, \barY + 0.35) {Attacker Observability Surface:};
\node[font=\bfseries\footnotesize, text=mlsysgreen!80!black, anchor=east] at (15.5, \barY + 0.35) {Monotonically Decreasing Information Leakage $\blacktriangleright$};

% Shaded gradient bar
\shade[left color=mlsysred!75, right color=mlsysgreen!35, rounded corners=3pt] (-1.5, \barY - 0.2) rectangle (15.5, \barY + 0.1);
\draw[mlsysgray!40, rounded corners=3pt, line width=0.6pt] (-1.5, \barY - 0.2) rectangle (15.5, \barY + 0.1);

% Drop lines
\draw[dashed, mlsysgray!50, line width=0.8pt] (0, \barY - 0.2) -- (0, -3.5);
\draw[dashed, mlsysgray!50, line width=0.8pt] (3.5, \barY - 0.2) -- (3.5, -3.5);
\draw[dashed, mlsysgray!50, line width=0.8pt] (7.0, \barY - 0.2) -- (7.0, -3.5);
\draw[dashed, mlsysgray!50, line width=0.8pt] (10.5, \barY - 0.2) -- (10.5, -3.5);
\draw[dashed, mlsysgray!50, line width=0.8pt] (14.0, \barY - 0.2) -- (14.0, -3.5);

% Bottom panel: Details
\def\botTop{-3.5}
\def\botH{2.7}
\node[draw=mlsysgray!40, fill=white, rounded corners=6pt, line width=0.8pt, minimum width=17.0cm, minimum height=\botH cm, anchor=north] (botbox) at (7.0, \botTop) {};

% Dividers
\draw[mlsysgray!25, line width=0.8pt] (1.75, \botTop - 0.2) -- (1.75, \botTop - \botH + 0.2);
\draw[mlsysgray!25, line width=0.8pt] (5.25, \botTop - 0.2) -- (5.25, \botTop - \botH + 0.2);
\draw[mlsysgray!25, line width=0.8pt] (8.75, \botTop - 0.2) -- (8.75, \botTop - \botH + 0.2);
\draw[mlsysgray!25, line width=0.8pt] (12.25, \botTop - 0.2) -- (12.25, \botTop - \botH + 0.2);

% Bottom details with anchor=north
% Col 1
\node[font=\bfseries\footnotesize, text=mlsysred!90!black, anchor=north] at (0, \botTop - 0.25) {Full Exposure};
\node[font=\scriptsize, text=mlsysdark!90, align=center, text width=3.2cm, anchor=north] at (0, \botTop - 0.72) {
    Plaintext records\\[3pt]
    Direct re-identification\\[3pt]
    No defense
};

% Col 2
\node[font=\bfseries\footnotesize, text=mlsysamber!90!black, anchor=north] at (3.5, \botTop - 0.25) {Noisy Statistics};
\node[font=\scriptsize, text=mlsysdark!90, align=center, text width=3.2cm, anchor=north] at (3.5, \botTop - 0.72) {
    Query responses masked\\[3pt]
    Formal privacy budget\\[3pt]
    Limits reconstruction
};

% Col 3
\node[font=\bfseries\footnotesize, text=mlsysamber!80!brown, anchor=north] at (7.0, \botTop - 0.25) {Local Gradient Only};
\node[font=\scriptsize, text=mlsysdark!90, align=center, text width=3.2cm, anchor=north] at (7.0, \botTop - 0.72) {
    No raw data transmitted\\[3pt]
    Vulnerable to gradient\\[3pt]
    inversion if unmasked
};

% Col 4
\node[font=\bfseries\footnotesize, text=mlsysgreen!80!black, anchor=north] at (10.5, \botTop - 0.25) {Encrypted Sum};
\node[font=\scriptsize, text=mlsysdark!90, align=center, text width=3.3cm, anchor=north] at (10.5, \botTop - 0.72) {
    Server inspects aggregate\\[3pt]
    Single client gradients\\[3pt]
    are information-theoretically hidden
};

% Col 5
\node[font=\bfseries\footnotesize, text=mlsysgreen!60!black, anchor=north] at (14.0, \botTop - 0.25) {Population Model};
\node[font=\scriptsize, text=mlsysdark!90, align=center, text width=3.2cm, anchor=north] at (14.0, \botTop - 0.72) {
    Generalizes knowledge\\[3pt]
    Broadcast to fleet\\[3pt]
    Hardened against\\membership inference
};
...
The principles and techniques examined in the earlier sections of this chapter provide the conceptual and technical foundation for responsible AI, but their practical implementation depends critically on deployment architecture. Cloud systems can support complex Shapley-value explanations and real-time fairness monitoring, but TinyML devices must rely on static interpretability and compile-time privacy guarantees. Edge deployments enable local privacy preservation but limit global fairness assessment. These architectural constraints are not mere implementation details; they fundamentally shape which responsible AI protections are accessible to different users and applications.

::: {#exmp-responsible-ai-fairness-audit .callout-example title="Choosing a fairness metric"}

**Scenario**: A hiring recommendation engine must select its primary fairness constraint before production launch across heterogeneous applicant pools.

**Diagnosis**: Demographic Parity forces equal selection rates regardless of candidate qualification, while Calibration only ensures equal score precision across groups. Equal Opportunity specifically mandates equal True Positive Rates among qualified applicants ($\text{TPR}_A = \text{TPR}_B$), directly matching the goal of equal opportunity for qualified talent.
...
% Top: Historical Input Corpus
\node[draw=mlsysamber!80, fill=white, rounded corners=6pt, line width=1pt, minimum width=\boxW cm, minimum height=\boxH cm] (topBox) at (0, \trackY) {};
\node[fill=mlsysamber!15, text=mlsysamber!90!black, font=\bfseries\scriptsize, rounded corners=3pt, minimum width=\boxW cm - 4pt, minimum height=16pt, anchor=north] at ([yshift=-2pt]topBox.north) {HISTORICAL INPUT CORPUS};
\node[anchor=center, align=center, text width=5.2cm] at ([yshift=-8pt]topBox.center) {
    {\bfseries\small Biased Training Data}\\[3pt]
    {\scriptsize\color{mlsysdark!80} Underrepresented cohorts and historical disparities}
};

% Right: Deployed Model Artifact
\node[draw=mlsysblue!80, fill=white, rounded corners=6pt, line width=1pt, minimum width=\boxW cm, minimum height=\boxH cm] (rightBox) at (\trackX, 0) {};
\node[fill=mlsysblue!15, text=mlsysblue!90!black, font=\bfseries\scriptsize, rounded corners=3pt, minimum width=\boxW cm - 4pt, minimum height=16pt, anchor=north] at ([yshift=-2pt]rightBox.north) {DEPLOYED MODEL ARTIFACT};
\node[anchor=center, align=center, text width=5.2cm] at ([yshift=-8pt]rightBox.center) {
    {\bfseries\small Biased Model Predictions}\\[3pt]
    {\scriptsize\color{mlsysdark!80} Learns spurious demographic correlation;\\penalizes protected demographic groups}
};

% Bottom: Downstream Real-World Impact
\node[draw=mlsysred!80, fill=white, rounded corners=6pt, line width=1pt, minimum width=\boxW cm, minimum height=\boxH cm] (bottomBox) at (0, -\trackY) {};
\node[fill=mlsysred!15, text=mlsysred!90!black, font=\bfseries\scriptsize, rounded corners=3pt, minimum width=\boxW cm - 4pt, minimum height=16pt, anchor=north] at ([yshift=-2pt]bottomBox.north) {DOWNSTREAM REAL-WORLD IMPACT};
\node[anchor=center, align=center, text width=5.2cm] at ([yshift=-8pt]bottomBox.center) {
    {\bfseries\small\color{mlsysred!90!black} Disparate Outcomes}\\[3pt]
    {\scriptsize\color{mlsysdark!80} Asymmetric denial rates in lending \& hiring}
};

% Left: Data Collection Flywheel
\node[draw=mlsysamber!80, fill=white, rounded corners=6pt, line width=1pt, minimum width=\boxW cm, minimum height=\boxH cm] (leftBox) at (-\trackX, 0) {};
\node[fill=mlsysamber!15, text=mlsysamber!90!black, font=\bfseries\scriptsize, rounded corners=3pt, minimum width=\boxW cm - 4pt, minimum height=16pt, anchor=north] at ([yshift=-2pt]leftBox.north) {DATA COLLECTION FLYWHEEL};
\node[anchor=center, align=center, text width=5.2cm] at ([yshift=-8pt]leftBox.center) {
    {\bfseries\small Reinforced Bias in New Data}\\[3pt]
    {\scriptsize\color{mlsysdark!80} Suppressed participation produces\\fewer positive repayment labels}
};

% Center Circle: Systemic Feedback
\draw[mlsysblue!20, line width=1.5pt, fill=white] (0, 0) circle (1.25);
\draw[mlsysblue!40, line width=0.8pt] (0, 0) circle (1.15);
\node[font=\bfseries\scriptsize, text=mlsysred!90!black, align=center] at (0, 0.15) {SYSTEMIC\\FEEDBACK};
\node[font=\scriptsize, text=mlsysgray] at (0, -0.25) {Compound Drift};

% Connecting Red Arrows (Clockwise Loop)
% Top to Right: topBox.east -> (trackX, trackY) -> rightBox.north
\draw[mlsysred!90, line width=1.4pt] (topBox.east) -- (\trackX, \trackY)
    node[pos=0.45, fill=white, inner sep=2.5pt, draw=mlsysgray!40, rounded corners=2pt, font=\scriptsize, text=mlsysdark] {trains on};
\draw[->, mlsysred!90, line width=1.4pt] (\trackX, \trackY) -- (rightBox.north);

% Right to Bottom: rightBox.south -> (trackX, -trackY) -> bottomBox.east
\draw[mlsysred!90, line width=1.4pt] (rightBox.south) -- (\trackX, -\trackY);
\draw[->, mlsysred!90, line width=1.4pt] (\trackX, -\trackY) -- (bottomBox.east)
    node[pos=0.55, fill=white, inner sep=2.5pt, draw=mlsysgray!40, rounded corners=2pt, font=\scriptsize, text=mlsysdark] {produces};

% Bottom to Left: bottomBox.west -> (-trackX, -trackY) -> leftBox.south
\draw[mlsysred!90, line width=1.4pt] (bottomBox.west) -- (-\trackX, -\trackY)
    node[pos=0.4, fill=white, inner sep=2.5pt, draw=mlsysgray!40, rounded corners=2pt, font=\scriptsize, text=mlsysdark] {creates};
\draw[->, mlsysred!90, line width=1.4pt] (-\trackX, -\trackY) -- (leftBox.south);

% Left to Top: leftBox.north -> (-trackX, trackY) -> topBox.west
\draw[mlsysred!90, line width=1.4pt] (leftBox.north) -- (-\trackX, \trackY)
    node[pos=0.5, fill=white, inner sep=2.5pt, draw=mlsysgray!40, rounded corners=2pt, font=\scriptsize, text=mlsysdark] {feeds into};
\draw[->, mlsysred!90, line width=1.4pt] (-\trackX, \trackY) -- (topBox.west);

% Interventions (Green boxes)
% 1. Pre-Processing (above top-left segment)
\node[draw=mlsysgreen!90!black, fill=mlsysgreen!10, rounded corners=4pt, line width=1pt, minimum width=2.4cm, minimum height=0.9cm, align=center] (pre) at (-3.8, \trackY + 1.2) {
    \textbf{\scriptsize Pre-Processing}\\[2pt]
    \fontsize{7}{8.5}\selectfont Data Audit \& Re-weight
};
\draw[->, mlsysgreen!90!black, line width=1.4pt] (pre.south) -- (-3.8, \trackY);

% 2. In-Processing (to the right of right-to-bottom segment)
\node[draw=mlsysgreen!90!black, fill=mlsysgreen!10, rounded corners=4pt, line width=1pt, minimum width=2.4cm, minimum height=0.9cm, align=center, anchor=west] (inproc) at (\trackX + 0.8, -1.7) {
    \textbf{\scriptsize In-Processing}\\[2pt]
    \fontsize{7}{8.5}\selectfont Fairness Constraints
};
\draw[->, mlsysgreen!90!black, line width=1.4pt] (inproc.west) -- (\trackX, -1.7);

% 3. Post-Processing (below bottom-left segment)
\node[draw=mlsysgreen!90!black, fill=mlsysgreen!10, rounded corners=4pt, line width=1pt, minimum width=2.4cm, minimum height=0.9cm, align=center] (post) at (-4.2, -\trackY - 1.1) {
    \textbf{\scriptsize Post-Processing}\\[2pt]
    \fontsize{7}{8.5}\selectfont Threshold Calibration
};
\draw[->, mlsysgreen!90!black, line width=1.4pt] (post.north) -- (-4.2, -\trackY);

% Legend at bottom
\node[draw=mlsysgray!40, fill=white, rounded corners=4pt, line width=0.7pt, inner xsep=16pt, inner ysep=6pt] at (0, -\trackY - 2.3) {
    \begin{tikzpicture}[baseline=-2pt]
        \draw[->, mlsysred!90, line width=1.4pt] (0,0) -- (0.6,0);
    \end{tikzpicture}
};
...
The fairness definitions examined in @sec-responsible-ai-fairness-machine-learning-2ba4 provide mathematical precision for what fairness means: demographic parity, equalized odds, and equality of opportunity are mathematically defined. Practitioners, however, face a practical challenge: computing these metrics on production systems processing thousands of predictions per second. Manual calculation using these mathematical formulas is infeasible at scale. This gap between definition and deployment motivates specialized tooling.

Operationalizing fairness in deployed systems requires more than principled objectives or theoretical metrics; it demands system-aware methods that detect, measure, and mitigate bias across the machine learning lifecycle. Practical bias detection can be implemented using tools like Fairlearn[^fn-fairlearn-fairness-toolkit] [@bird2020fairlearn].

[^fn-fairlearn-fairness-toolkit]: **Fairlearn**\index{Fairlearn!bias toolkit}: Microsoft's open-source toolkit (2020) for computing fairness metrics and applying mitigation algorithms to scikit-learn compatible models [@bird2020fairlearn]. The systems integration pattern is that fairness measurement and mitigation wrap the model pipeline instead of living in a separate spreadsheet audit. The practical significance is that fairness monitoring becomes a CI/CD pipeline stage rather than an ad hoc review, enabling automated regression detection when model updates degrade subgroup performance.

@Lst-bias-detection enables offline or CI-stage fairness audits across demographic groups, revealing concerning disparities where loan approval rates vary substantially by ethnicity: from 94 percent for Asian applicants to 68 percent for Black applicants. Building on the system-level constraints in this chapter, fairness must be treated as an architectural consideration that intersects with data engineering, model training, inference design, monitoring infrastructure, and policy governance. While fairness metrics such as demographic parity, equalized odds, and equality of opportunity formalize different normative goals, their realization depends on the architecture's ability to measure subgroup performance, support adaptive decision boundaries, and store or surface group-specific metadata during runtime; deployment-time monitoring is handled later in this chapter.
...
% Top legend box
\node[draw=mlsysgray!35, fill=mlsysblue!4, rounded corners=5pt, line width=0.8pt, minimum width=15.4cm, minimum height=0.9cm] (topleg) at (7.5, 5.3) {};

% Blue Plus marker symbol in legend
\draw[mlsysblue, line width=2.2pt] (1.0, 5.3) -- (1.32, 5.3);
\draw[mlsysblue, line width=2.2pt] (1.16, 5.14) -- (1.16, 5.46);
\node[font=\footnotesize, text=mlsysdark, anchor=west] at (1.45, 5.3) {Qualified Applicant (Repays Loan, Ground Truth $Y = 1$)};

% Red Circle marker symbol in legend
\draw[mlsysred, line width=1.8pt, fill=white] (9.6, 5.3) circle (0.14);
\node[font=\footnotesize, text=mlsysdark, anchor=west] at (9.85, 5.3) {Default Applicant (Ground Truth $Y = 0$)};

% Main container
\node[draw=mlsysgray!35, fill=white, rounded corners=6pt, line width=0.8pt, minimum width=15.4cm, minimum height=4.6cm, anchor=north] (mainbox) at (7.5, 4.6) {};

% Subgroup A track
\def\yA{3.3}
\fill[mlsysblue!6, rounded corners=4pt] (3.1, \yA - 0.55) rectangle (14.6, \yA + 0.55);
\draw[dashed, mlsysgray!40, line width=0.8pt] (3.4, \yA) -- (14.3, \yA);

% Subgroup A badge
\node[draw=mlsysblue!70, fill=white, rounded corners=4pt, line width=0.9pt, minimum width=2.6cm, minimum height=1.1cm, align=center] at (1.6, \yA) {
    \textbf{\small Subgroup A}\\[2pt]
    \scriptsize\color{mlsysgray} Higher Base Score
};

% Subgroup A data points
% Red circles: Y=0 at 0.20, 0.52, 0.59, 0.66
\foreach \xv in {0.20, 0.52, 0.59, 0.66} {
    \pgfmathsetmacro{\px}{3.5 + 10.5 * \xv}
    \draw[mlsysred, line width=1.8pt, fill=white] (\px, \yA) circle (0.14);
}
% Blue plusses: Y=1 at 0.72, 0.78, 0.84, 0.94
\foreach \xv in {0.72, 0.78, 0.84, 0.94} {
    \pgfmathsetmacro{\px}{3.5 + 10.5 * \xv}
    \draw[mlsysblue, line width=2.2pt] (\px - 0.14, \yA) -- (\px + 0.14, \yA);
    \draw[mlsysblue, line width=2.2pt] (\px, \yA - 0.14) -- (\px, \yA + 0.14);
}

% Subgroup B track
\def\yB{1.7}
\fill[mlsysred!5, rounded corners=4pt] (3.1, \yB - 0.55) rectangle (14.6, \yB + 0.55);
\draw[dashed, mlsysgray!40, line width=0.8pt] (3.4, \yB) -- (14.3, \yB);

% Subgroup B badge
\node[draw=mlsysred!60, fill=white, rounded corners=4pt, line width=0.9pt, minimum width=2.6cm, minimum height=1.1cm, align=center] at (1.6, \yB) {
    \textbf{\small Subgroup B}\\[2pt]
    \scriptsize\color{mlsysgray} Lower Base Score
};

% Subgroup B data points
% Red circles: Y=0 at 0.06, 0.13, 0.20, 0.44
\foreach \xv in {0.06, 0.13, 0.20, 0.44} {
    \pgfmathsetmacro{\px}{3.5 + 10.5 * \xv}
    \draw[mlsysred, line width=1.8pt, fill=white] (\px, \yB) circle (0.14);
}
% Blue plusses: Y=1 at 0.36, 0.50, 0.62, 0.89
\foreach \xv in {0.36, 0.50, 0.62, 0.89} {
    \pgfmathsetmacro{\px}{3.5 + 10.5 * \xv}
    \draw[mlsysblue, line width=2.2pt] (\px - 0.14, \yB) -- (\px + 0.14, \yB);
    \draw[mlsysblue, line width=2.2pt] (\px, \yB - 0.14) -- (\px, \yB + 0.14);
}

% Threshold lines
% Amber line at theta_B = 0.45 (x = 3.5 + 10.5*0.45 = 8.225)
\def\xB{8.225}
\draw[dashed, mlsysamber!90!black, line width=1.4pt] (\xB, 0.65) -- (\xB, 4.1);
\node[draw=mlsysamber!90!black, fill=white, rounded corners=3pt, line width=0.9pt, font=\bfseries\scriptsize, text=mlsysamber!90!black, inner xsep=6pt, inner ysep=3pt] at (\xB, 4.35) {$\theta_B = 0.45$ (75\% TPR)};

% Blue line at theta_A = 0.75 (x = 3.5 + 10.5*0.75 = 11.375)
\def\xA{11.375}
\draw[dashed, mlsysblue!90!black, line width=1.4pt] (\xA, 0.65) -- (\xA, 4.1);
\node[draw=mlsysblue!90!black, fill=white, rounded corners=3pt, line width=0.9pt, font=\bfseries\scriptsize, text=mlsysblue!90!black, inner xsep=6pt, inner ysep=3pt] at (\xA, 4.35) {$\theta_A = 0.75$ (75\% TPR)};

% X-axis
\draw[mlsysgray!60, line width=0.9pt] (3.5, 0.65) -- (14.0, 0.65);
\foreach \val/\xv in {0.0/3.5, 0.2/5.6, 0.4/7.7, 0.6/9.8, 0.8/11.9, 1.0/14.0} {
    \draw[mlsysgray!60, line width=0.9pt] (\xv, 0.65) -- (\xv, 0.5);
    \node[font=\scriptsize, text=mlsysgray, anchor=north] at (\xv, 0.45) {\val};
}
\node[font=\bfseries\footnotesize, text=mlsysdark, anchor=north] at (8.75, 0.05) {Predicted Credit Risk Score / Model Output Probability};
...
% Left Card: Uniform Threshold
\node[draw=mlsysgray!35, fill=white, rounded corners=5pt, line width=0.8pt, minimum width=\cardW cm, minimum height=\cardH cm, anchor=north west] (cUnif) at (-0.2, \botY) {};
\node[anchor=north west, align=left, text width=7.2cm] at ([xshift=8pt, yshift=-8pt]cUnif.north west) {
    \colorbox{mlsysred!15}{\textcolor{mlsysred!90!black}{\bfseries\fontsize{7}{8}\selectfont UNIFORM THRESHOLD}}
    \hspace{6pt}
    {\bfseries\scriptsize $\theta = 0.75$ (Single Boundary)}\\[4pt]
    {\fontsize{7.5}{9}\selectfont\color{mlsysdark!90} Subgroup A: 75\% TPR (3/4) vs Subgroup B: 25\% TPR (1/4) $\rightarrow$ Severe Disparity}
};

% Right Card: Group-Adapted Threshold
\node[draw=mlsysgray!35, fill=white, rounded corners=5pt, line width=0.8pt, minimum width=\cardW cm, minimum height=\cardH cm, anchor=north east] (cAdapt) at (15.2, \botY) {};
\node[anchor=north west, align=left, text width=7.2cm] at ([xshift=8pt, yshift=-8pt]cAdapt.north west) {
    \colorbox{mlsysgreen!15}{\textcolor{mlsysgreen!80!black}{\bfseries\fontsize{7}{8}\selectfont GROUP-ADAPTED THRESHOLD}}
    \hspace{6pt}
    {\bfseries\scriptsize $\theta_A = 0.75$, $\theta_B = 0.45$}\\[4pt]
    {\fontsize{7.5}{9}\selectfont\color{mlsysdark!90} Subgroup A: 75\% TPR (3/4) = Subgroup B: 75\% TPR (3/4) $\rightarrow$ Equal Opportunity}
};
...
% Geometry
% Pipeline cards: x = 2.4, 5.7, 9.0, 12.3
\def\cardW{2.95}
\def\cardH{3.8}
\def\topY{4.0}

% Far left: User Request
\node[draw=mlsysgray!40, fill=white, rounded corners=5pt, line width=0.8pt, minimum width=1.5cm, minimum height=1.8cm, align=center] (user) at (-0.1, \topY) {
    \textbf{\footnotesize User}\\[1pt]
    \textbf{\footnotesize Request}\\[3pt]
    \fontsize{7}{8}\selectfont\color{mlsysgray} Inference
};

% Card 1: Input Guardrails
\node[draw=mlsysred!70, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm] (c1) at (2.4, \topY) {};
\node[fill=mlsysred!12, text=mlsysred!90!black, font=\bfseries\scriptsize, rounded corners=2.5pt, minimum width=\cardW cm - 4pt, minimum height=15pt, anchor=north] at ([yshift=-2pt]c1.north) {INPUT FILTER};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-20pt]c1.north) {Input Guardrails};
\draw[mlsysgray!25, line width=0.6pt] ([xshift=10pt, yshift=-34pt]c1.north west) -- ([xshift=-10pt, yshift=-34pt]c1.north east);
\node[font=\fontsize{7.5}{10}\selectfont, text=mlsysdark!90, align=center, text width=2.7cm, anchor=north] at ([yshift=-39pt]c1.north) {
    PII Anonymization\\[3pt]
    Content Moderation\\[3pt]
    Schema Validation\\[3pt]
    Injection Defense
};

% Card 2: Model Serving
\node[draw=mlsysblue!70, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm] (c2) at (5.7, \topY) {};
\node[fill=mlsysblue!12, text=mlsysblue!90!black, font=\bfseries\scriptsize, rounded corners=2.5pt, minimum width=\cardW cm - 4pt, minimum height=15pt, anchor=north] at ([yshift=-2pt]c2.north) {CORE INFERENCE};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-20pt]c2.north) {Model Serving};
\draw[mlsysgray!25, line width=0.6pt] ([xshift=10pt, yshift=-34pt]c2.north west) -- ([xshift=-10pt, yshift=-34pt]c2.north east);
\node[font=\fontsize{7.5}{10}\selectfont, text=mlsysdark!90, align=center, text width=2.7cm, anchor=north] at ([yshift=-39pt]c2.north) {
    Feature Store Lookup\\[3pt]
    Model Inference\\[3pt]
    Group Thresholding\\[3pt]
    Logit Calibration
};

% Card 3: Output Guard
\node[draw=mlsysgreen!70, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm] (c3) at (9.0, \topY) {};
\node[fill=mlsysgreen!12, text=mlsysgreen!90!black, font=\bfseries\scriptsize, rounded corners=2.5pt, minimum width=\cardW cm - 4pt, minimum height=15pt, anchor=north] at ([yshift=-2pt]c3.north) {VALIDATION};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-20pt]c3.north) {Output Guard};
\draw[mlsysgray!25, line width=0.6pt] ([xshift=10pt, yshift=-34pt]c3.north west) -- ([xshift=-10pt, yshift=-34pt]c3.north east);
\node[font=\fontsize{7.5}{10}\selectfont, text=mlsysdark!90, align=center, text width=2.7cm, anchor=north] at ([yshift=-39pt]c3.north) {
    Toxicity \& Bias Scan\\[3pt]
    Factual Grounding\\[3pt]
    Policy Alignment\\[3pt]
    Violation Detection
};

% Card 4: Audit Logger
\node[draw=mlsysamber!70, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm] (c4) at (12.3, \topY) {};
\node[fill=mlsysamber!12, text=mlsysamber!90!black, font=\bfseries\scriptsize, rounded corners=2.5pt, minimum width=\cardW cm - 4pt, minimum height=15pt, anchor=north] at ([yshift=-2pt]c4.north) {AUDIT TRAIL};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-20pt]c4.north) {Audit Logger};
\draw[mlsysgray!25, line width=0.6pt] ([xshift=10pt, yshift=-34pt]c4.north west) -- ([xshift=-10pt, yshift=-34pt]c4.north east);
\node[font=\fontsize{7.5}{10}\selectfont, text=mlsysdark!90, align=center, text width=2.7cm, anchor=north] at ([yshift=-39pt]c4.north) {
    Immutable Records\\[3pt]
    Sensitive Metadata\\[3pt]
    Decision Lineage\\[3pt]
    Compliance Ledger
};

% Connecting Arrows in Pipeline
\draw[->, mlsysgray!70, line width=1.4pt] (user.east) -- (c1.west);
\draw[->, mlsysgray!70, line width=1.4pt] (c1.east) -- (c2.west);
\draw[->, mlsysgray!70, line width=1.4pt] (c2.east) -- (c3.west);
\draw[->, mlsysgray!70, line width=1.4pt] (c3.east) -- (c4.west);

% Right Destination Boxes
% Human Review Queue (top right)
\node[draw=mlsysred!75, fill=mlsysred!6, rounded corners=5pt, line width=0.9pt, minimum width=2.6cm, minimum height=1.5cm, align=center] (human) at (15.5, 5.4) {
    \textbf{\scriptsize\color{mlsysred!90!black} Human Review Queue}\\[3pt]
    \fontsize{7}{8.5}\selectfont Policy Team Triage\\[1pt]
    \fontsize{7}{8.5}\selectfont Asynchronous Escalation
};

% Approved Response (straight to the right of Audit Logger)
\node[draw=mlsysgreen!75, fill=mlsysgreen!7, rounded corners=5pt, line width=0.9pt, minimum width=2.6cm, minimum height=1.5cm, align=center] (appr) at (15.5, \topY) {
    \textbf{\scriptsize\color{mlsysgreen!90!black} Approved Response}\\[3pt]
    \fontsize{7}{8.5}\selectfont Served to End User\\[1pt]
    \fontsize{7}{8.5}\selectfont Low Latency Path
};

\draw[->, mlsysgray!70, line width=1.4pt] (c4.east) -- (appr.west);

% Flagged Violation line from c3 to human
\draw[->, dashed, mlsysred!90, line width=1.3pt] (c3.north) -- ++(0, 2.3) -| (human.north)
    node[pos=0.28, fill=white, inner xsep=5pt, inner ysep=2.5pt, draw=mlsysred!60, rounded corners=2pt, font=\bfseries\scriptsize, text=mlsysred!90!black] {FLAGGED VIOLATION};

% Bottom Row: Observability & Explainability Cards
\def\botY{-0.4}
\def\botCardW{5.5}
\def\botCardH{3.6}

% Fairness & Bias Monitor (bottom left)
\node[draw=mlsysred!50, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\botCardW cm, minimum height=\botCardH cm, anchor=north] (obs) at (5.2, \botY) {};
\node[fill=mlsysred!12, text=mlsysred!90!black, font=\bfseries\scriptsize, rounded corners=2.5pt, minimum width=\botCardW cm - 4pt, minimum height=15pt, anchor=north] at ([yshift=-2pt]obs.north) {OBSERVABILITY};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-20pt]obs.north) {Fairness \& Bias Monitor};
\draw[mlsysgray!25, line width=0.6pt] ([xshift=12pt, yshift=-34pt]obs.north west) -- ([xshift=-12pt, yshift=-34pt]obs.north east);
\node[font=\fontsize{7.5}{10.5}\selectfont, text=mlsysdark!90, align=left, text width=5.0cm, anchor=north] at ([yshift=-40pt]obs.north) {
    $\bullet$\, Real-time Subgroup Disparity Tracking\\[3pt]
    $\bullet$\, True / False Positive Rate Equivalence\\[3pt]
    $\bullet$\, Demographic Calibration Shift Alerts\\[3pt]
    $\bullet$\, Multiple-Comparison Error Bounds
};

% Explainability Engine (bottom right)
\node[draw=mlsyspurple!50, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\botCardW cm, minimum height=\botCardH cm, anchor=north] (exp) at (11.2, \botY) {};
\node[fill=mlsyspurple!12, text=mlsyspurple!90!black, font=\bfseries\scriptsize, rounded corners=2.5pt, minimum width=\botCardW cm - 4pt, minimum height=15pt, anchor=north] at ([yshift=-2pt]exp.north) {INTERPRETABILITY};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-20pt]exp.north) {Explainability Engine};
\draw[mlsysgray!25, line width=0.6pt] ([xshift=12pt, yshift=-34pt]exp.north west) -- ([xshift=-12pt, yshift=-34pt]exp.north east);
\node[font=\fontsize{7.5}{10.5}\selectfont, text=mlsysdark!90, align=left, text width=5.0cm, anchor=north] at ([yshift=-40pt]exp.north) {
    $\bullet$\, Local Feature Attributions (SHAP / LIME)\\[3pt]
    $\bullet$\, Concept Activation Vectors (TCAV)\\[3pt]
    $\bullet$\, Counterfactual Recourse Testing\\[3pt]
    $\bullet$\, Audit Ledger Attestation Artifacts
};

% Vertical Connections to Bottom Cards
\draw[->, dotted, mlsysblue!80!black, line width=1.4pt] (c2.south) -- (c2.south |- obs.north);
\draw[->, dotted, mlsysblue!80!black, line width=1.4pt] (c3.south) -- (c3.south |- exp.north);
\draw[->, dotted, mlsyspurple!80!black, line width=1.4pt] (exp.north -| c4.south) -- (c4.south);

% Retraining / Rollback Feedback Loop (orthogonal path through y = 1.0)
\draw[->, dashed, mlsysred!90, line width=1.3pt] (obs.west) -- ++(-3.8, 0) -- ++(0, 3.2) -| ([xshift=-15pt]c2.south)
    node[pos=0.62, fill=white, inner xsep=5pt, inner ysep=2.5pt, draw=mlsysred!60, rounded corners=2pt, font=\bfseries\scriptsize, text=mlsysred!90!black] {RETRAINING / ROLLBACK};

% Bottom Legend
\def\legY{-4.8}
\node[draw=mlsysgray!35, fill=white, rounded corners=5pt, line width=0.8pt, inner xsep=18pt, inner ysep=6pt] at (7.8, \legY) {
    \begin{tikzpicture}[baseline=-2pt, font=\fontsize{7.5}{9}\selectfont]
        \fill[mlsysred!25, draw=mlsysred!70, rounded corners=2pt] (0, -0.15) rectangle (0.35, 0.15);
        \node[anchor=west, text=mlsysdark!90] at (0.45, 0) {Input \& Output Guardrails};

        \fill[mlsysblue!25, draw=mlsysblue!70, rounded corners=2pt] (4.4, -0.15) rectangle (4.75, 0.15);
        \node[anchor=west, text=mlsysdark!90] at (4.85, 0) {Core Model Serving Stack};

        \fill[mlsysamber!25, draw=mlsysamber!70, rounded corners=2pt] (8.9, -0.15) rectangle (9.25, 0.15);
        \node[anchor=west, text=mlsysdark!90] at (9.35, 0) {Audit Trail \& Attestation};

        \fill[mlsyspurple!25, draw=mlsyspurple!70, rounded corners=2pt] (13.3, -0.15) rectangle (13.65, 0.15);
        \node[anchor=west, text=mlsysdark!90] at (13.75, 0) {Observability \& Explainability};
    \end{tikzpicture}
};
...
The architecture works because three governance paths share the model serving stack rather than sitting beside it. The data anonymization layer implements privacy-preserving transformations before model inference, using techniques like $k$-anonymity[^fn-k-anonymity-privacy] or differential privacy noise injection. That layer is not free: privacy transformations consume latency, CPU, and memory, so they must be budgeted inside the model-serving service-level objective (SLO) rather than treated as an external compliance step.

[^fn-k-anonymity-privacy]: **k-Anonymity**\index{k-Anonymity!privacy technique}: A privacy guarantee [@sweeney2002] ensuring each record is indistinguishable from at least $k-1$ others by generalizing quasi-identifiers (for example, replacing exact ages with ranges, locations with regions). The systems trade-off is information loss: higher $k$ values provide stronger indistinguishability but coarsen features and can reduce downstream training signal. For ML preprocessing pipelines, $k$-anonymity adds a transformation stage that must balance privacy guarantees against the accuracy impact of coarser features.
...
The explanation engine generates SHAP or LIME explanations for model decisions, particularly for negative outcomes requiring user recourse. Exact explanations often exceed inline serving budgets, so production systems rely on approximation, caching, sampling, or asynchronous generation to trade latency against fidelity. The implementation that follows should be read as an architectural pattern rather than an API to memorize: fairness metrics enter the serving path, and alerts trigger when thresholds are exceeded. Delayed outcome ingestion and the full equalized-odds calculation are omitted. @Lst-production-fairness-monitoring integrates these components into a monitoring system that processes inference requests and computes rolling fairness metrics across protected groups.
...
## Governing Privacy, Unlearning, and Robustness Controls {#sec-responsible-ai-risk-mitigation-techniques-b4d6}

@Sec-security-privacy owns the threat models and privacy mechanisms, while @sec-robust-ai owns robustness behavior, attacks, defenses, and evaluation. This section does not redefine those mechanisms. It asks what evidence, ownership, and lifecycle interfaces governance must require when a production system inherits them. When a user invokes erasure or deletion rights under privacy laws such as the GDPR, deleting their row from a database is straightforward [@gdpr2016]. Deleting their influence from a model that has already trained on it is a harder systems problem involving lineage, retraining, validation, deployment, and audit.
...
Machine unlearning[^fn-machine-unlearning-cost] and privacy preservation address the deletion-and-leakage part of that challenge, providing mechanisms to excise specific training data influence from compiled models.

[^fn-machine-unlearning-cost]: **Machine Unlearning**\index{Machine Unlearning!privacy cost}: First formalized by Cao and Yang in 2015, this is the ability to remove specific training data influence from a model without full retraining; the naive approach (retrain from scratch) costs the same as the original training run. Sharded, isolated, sliced, and aggregated (SISA) training partitions training into independent shards, reducing unlearning to retraining only the affected shard; in the GPT-scale sizing scenario in @sec-responsible-ai-machine-unlearning-cdf1, that reduces the retraining surface substantially at the cost of model-quality and aggregation trade-offs. For GDPR-compliant ML systems, unlearning latency becomes a service-level concern: deletion requests must be handled within the organization's applicable legal and policy timelines.
...
The memorization risk extends beyond language models. @Fig-diffusion-model-example demonstrates that diffusion models trained on image datasets can regenerate visual instances from the training set [@carlini2023diffusion]. The left panel is an original training image and the right panel is the diffusion model's reconstruction of that same instance; their near-identity is the evidence of memorization, since a model that had only learned a general distribution of portraits could not reproduce a specific individual. Such behavior highlights a more general vulnerability: contemporary generative model architectures can internalize and reproduce training data, often without explicit signals or intent, and without easy detection or control.
...
To mitigate such vulnerabilities, a range of privacy-preserving techniques have been developed. A canonical formal method is differential privacy,[^fn-differential-privacy-guarantee] which provides guarantees that the inclusion or exclusion of a single datapoint has a statistically bounded effect on the model's output. Algorithms such as differentially private stochastic gradient descent (DP-SGD) operationalize this idea for deep learning by clipping gradients and injecting noise during training [@abadi2016deep]. When the privacy accounting, clipping, and noise calibration are appropriate for the threat model, these methods can limit memorization and reduce the risk of inference attacks.

[^fn-differential-privacy-guarantee]: **Differential Privacy**: A mathematical framework introduced by @dwork2006calibrating that bounds how much a single individual's inclusion or exclusion can affect a released computation, parameterized by a privacy budget such as $\epsilon$. DP-SGD adapts that framework to deep learning by clipping per-example gradients and injecting calibrated noise [@abadi2016deep]. The privacy budget quantifies a trade-off among privacy, utility, and training cost; Apple has publicly described local differential privacy deployments for selected telemetry tasks, but those deployments should not be treated as a universal DP-SGD cost benchmark [@apple2017learningprivacy].
...
Training a next-word predictor on sensitive messages with **DP-SGD**\index{DP-SGD!training cost} reduces training-data extraction risk, but the protection is priced in compute and accuracy.

The privacy parameter $\epsilon$ bounds privacy loss; smaller values provide a stronger bound when the neighboring relation and $\delta$ are fixed. A reproducible DP-SGD result must also specify the dataset size, batch sampling scheme and rate, number of optimization steps, clipping norm, noise multiplier, neighboring relation, $\delta$, and privacy accountant. These configuration values are illustrative planning assumptions, not outputs derived by the chapter's sizing code.

**Illustrative stronger-privacy configuration** (assumed accountant result $\epsilon =$ `{python} PrivacyPriceAnalysis.eps_strong_str`): The scenario assumes gradients are clipped at $C = 1.0$, `{python} PrivacyPriceAnalysis.clip_fraction_pct_str` of updates are affected by clipping, and the noise multiplier is $\sigma = 1.0$. It further assumes `{python} PrivacyPriceAnalysis.epochs_mult_str` more epochs, a training-cost increase from `{python} PrivacyPriceAnalysis.base_cost_str` to approximately `{python} PrivacyPriceAnalysis.cost_strong_str`, and an accuracy drop of `{python} PrivacyPriceAnalysis.acc_drop_strong_str`.

**Illustrative weaker-privacy configuration** (assumed accountant result $\epsilon =$ `{python} PrivacyPriceAnalysis.eps_mod_str`): The scenario assumes a noise multiplier of $\sigma = 0.5$, training overhead of `{python} PrivacyPriceAnalysis.overhead_mod_str`, and an accuracy drop of `{python} PrivacyPriceAnalysis.acc_drop_mod_str`.

**Illustrative weakest configuration** (assumed accountant result $\epsilon =$ `{python} PrivacyPriceAnalysis.eps_weak_str`): The scenario assumes less noise and less than 1 percent accuracy loss, with a weaker formal privacy bound than the other two configurations under otherwise comparable accounting assumptions.

Privacy is not binary. It is a continuous curve where organizations buy user trust with compute dollars and model accuracy. Once a fixed model has been trained with differential privacy, ordinary inference is post-processing and does not consume additional training-data privacy budget. Additional privacy loss must be accounted for when a postdeployment mechanism makes new accesses to private data or releases additional differentially private statistics. The key engineering decision is to document and account for every private-data access and communicate the resulting trade-offs to users and regulators.
...
The privacy mechanisms examined in this section protect data during collection and training, but they do not address a temporal problem: when users invoke their legal right to have their data forgotten, models trained on that data retain its influence in their learned parameters. The privacy violation persists even after the raw data is deleted from storage systems. Machine unlearning addresses this temporal dimension of privacy, ensuring that data deletion rights extend beyond databases to the models themselves.

Privacy preservation does not end at training time. In many real-world systems, users may have rights to revoke consent or request deletion of their data, even after a model has been trained and deployed [@gdpr2016; @california2023ccpa]. Supporting this requirement introduces a core technical challenge: removing the influence of specific datapoints from a trained model without full retraining, a task that is often infeasible in edge, mobile, or embedded deployments with constrained compute, storage, and connectivity.
...
::: {#fig-machine-unlearning fig-env="figure" fig-pos="htb" fig-cap="**Model Update Strategies**: Three approaches to removing a data point's influence. Full retraining gives exact unlearning at the highest cost; gradient ascent provides an approximate update by maximizing loss on the forget set; and SISA training partitions data into shards so only the affected shard is retrained. The accompanying cost model quantifies the full-retraining and SISA cases." fig-alt="Three-panel comparison: Full Retraining with exact unlearning and full-dataset requirements; Gradient Ascent with approximate unlearning and fast updates; SISA Training with exact per-shard unlearning and one-shard retraining."}
```{=latex}
...
% ==========================================
\node[draw=mlsysblue!40, fill=white, rounded corners=6pt, line width=0.9pt, minimum width=\colW cm, minimum height=\colH cm, anchor=north west] (c1) at (0, 0) {};
\node[fill=mlsysblue!12, text=mlsysblue!90!black, font=\bfseries\footnotesize, rounded corners=3pt, minimum width=\colW cm - 4pt, minimum height=20pt, anchor=north] at ([yshift=-2pt]c1.north) {Full Retraining (Baseline)};

% Diagram 1 (top of col 1)
\def\diagY{-1.8}
% Dataset D box
\node[draw=mlsysgreen!80, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=1.9cm, minimum height=1.3cm] (d1) at (1.25, \diagY) {};
\node[font=\bfseries\fontsize{7}{8}\selectfont, text=mlsysgreen!80!black] at (1.25, \diagY + 0.35) {Dataset $D$};
\node[draw=mlsysred!80, fill=white, rounded corners=2pt, line width=0.6pt, font=\bfseries\fontsize{6}{7}\selectfont, text=mlsysred!90!black, inner xsep=3pt, inner ysep=2pt] at (1.25, \diagY - 0.15) {Remove $D_{\text{forget}}$};

\draw[->, mlsysgray!70, line width=1.4pt] (2.3, \diagY) -- (2.7, \diagY);

% Train from scratch box
\node[draw=mlsysblue!80, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=1.9cm, minimum height=1.3cm, align=center] at (3.75, \diagY) {
    \textbf{\fontsize{7.5}{9}\selectfont Train from}\\[2pt]
    \textbf{\fontsize{7.5}{9}\selectfont Scratch}
};

\draw[mlsysgray!25, line width=0.6pt] ([xshift=10pt, yshift=-78pt]c1.north west) -- ([xshift=-10pt, yshift=-78pt]c1.north east);

% Operational Profile 1
\node[font=\bfseries\scriptsize, text=mlsysdark, anchor=west] at (0.35, -3.1) {Operational Profile};
\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (0.35, -3.5) {Guarantee:};
\node[font=\bfseries\fontsize{7.5}{9}\selectfont, text=mlsysgreen!80!black, anchor=west] at (0.35, -3.8) {Exact unlearning (zero residue)};

\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (0.35, -4.3) {Corpus Access:};
\node[font=\fontsize{7.5}{9}\selectfont, text=mlsysdark!90, anchor=west] at (0.35, -4.6) {Requires full training dataset};

\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (0.35, -5.1) {Primary Bottleneck:};
\node[font=\bfseries\fontsize{7.5}{9}\selectfont, text=mlsysred!90!black, anchor=west] at (0.35, -5.4) {Extreme CapEx / OpEx cost};

\draw[mlsysgray!25, line width=0.6pt] ([xshift=10pt, yshift=-165pt]c1.north west) -- ([xshift=-10pt, yshift=-165pt]c1.north east);

% Unlearning Fidelity 1
\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (0.35, -6.2) {Unlearning Fidelity:};
\fill[mlsysgreen!20, rounded corners=2.5pt] (0.35, -6.75) rectangle (4.65, -6.4);
\node[font=\bfseries\fontsize{7}{8}\selectfont, text=mlsysgreen!80!black] at (2.5, -6.57) {100\% (Provable Exact)};

% Compute Overhead 1
\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (0.35, -7.2) {Compute Overhead:};
\node[draw=mlsysred!70, fill=mlsysred!10, rounded corners=3pt, line width=0.7pt, minimum width=4.3cm, minimum height=14pt, font=\bfseries\fontsize{7}{8}\selectfont, text=mlsysred!90!black] at (2.5, -7.6) {Full Training Run ($\mathcal{O}(\text{days})$)};

% Serving Impact 1
\node[font=\fontsize{7.5}{9}\selectfont, text=mlsysdark!90, anchor=west] at (0.35, -8.3) {Serving Impact: \textbf{\textcolor{mlsysgreen!80!black}{Zero penalty}}};


% ==========================================
% COLUMN 2: Gradient Ascent (Approximate)
% ==========================================
\node[draw=mlsysamber!40, fill=white, rounded corners=6pt, line width=0.9pt, minimum width=\colW cm, minimum height=\colH cm, anchor=north west] (c2) at (5.4, 0) {};
\node[fill=mlsysamber!15, text=mlsysamber!90!black, font=\bfseries\footnotesize, rounded corners=3pt, minimum width=\colW cm - 4pt, minimum height=20pt, anchor=north] at ([yshift=-2pt]c2.north) {Gradient Ascent (Approximate)};

% Diagram 2 (top of col 2)
\node[draw=mlsysred!80, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=1.9cm, minimum height=1.3cm] (d2) at (6.65, \diagY) {};
\node[font=\bfseries\fontsize{7}{8}\selectfont, text=mlsysred!90!black] at (6.65, \diagY + 0.35) {Forget Set};
\node[draw=mlsysred!80, fill=white, rounded corners=2pt, line width=0.6pt, font=\bfseries\fontsize{6}{7}\selectfont, text=mlsysred!90!black, inner xsep=4pt, inner ysep=2pt] at (6.65, \diagY - 0.15) {Maximize Loss};

\draw[->, mlsysgray!70, line width=1.4pt] (7.7, \diagY) -- (8.1, \diagY);

\node[draw=mlsysamber!80, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=1.9cm, minimum height=1.3cm, align=center] at (9.15, \diagY) {
    \textbf{\fontsize{7.5}{9}\selectfont Ascent Step}\\[2pt]
    \fontsize{6.5}{7.5}\selectfont $\theta \leftarrow \theta + \eta \nabla L$
};

\draw[mlsysgray!25, line width=0.6pt] ([xshift=10pt, yshift=-78pt]c2.north west) -- ([xshift=-10pt, yshift=-78pt]c2.north east);

% Operational Profile 2
\node[font=\bfseries\scriptsize, text=mlsysdark, anchor=west] at (5.75, -3.1) {Operational Profile};
\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (5.75, -3.5) {Guarantee:};
\node[font=\bfseries\fontsize{7.5}{9}\selectfont, text=mlsysamber!90!black, anchor=west] at (5.75, -3.8) {Approximate (empirical heuristic)};

\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (5.75, -4.3) {Corpus Access:};
\node[font=\fontsize{7.5}{9}\selectfont, text=mlsysdark!90, anchor=west] at (5.75, -4.6) {Only forget samples required};

\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (5.75, -5.1) {Primary Bottleneck:};
\node[font=\bfseries\fontsize{7.5}{9}\selectfont, text=mlsysred!90!black, anchor=west] at (5.75, -5.4) {Collateral degradation / forgetting};

\draw[mlsysgray!25, line width=0.6pt] ([xshift=10pt, yshift=-165pt]c2.north west) -- ([xshift=-10pt, yshift=-165pt]c2.north east);

% Unlearning Fidelity 2
\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (5.75, -6.2) {Unlearning Fidelity:};
\fill[mlsysgray!20, rounded corners=2.5pt] (5.75, -6.75) rectangle (10.05, -6.4);
\fill[mlsysamber!30, rounded corners=2.5pt] (5.75, -6.75) rectangle (9.2, -6.4);
\node[font=\bfseries\fontsize{7}{8}\selectfont, text=mlsysamber!90!black] at (7.9, -6.57) {$\sim$80\% (Approximate)};

% Compute Overhead 2
\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (5.75, -7.2) {Compute Overhead:};
\node[draw=mlsysgreen!70, fill=mlsysgreen!10, rounded corners=3pt, line width=0.7pt, minimum width=4.3cm, minimum height=14pt, font=\bfseries\fontsize{7}{8}\selectfont, text=mlsysgreen!80!black] at (7.9, -7.6) {Minutes ($< 0.1\%$ cost)};

% Serving Impact 2
\node[font=\fontsize{7.5}{9}\selectfont, text=mlsysdark!90, anchor=west] at (5.75, -8.3) {Serving Impact: \textbf{\textcolor{mlsysgreen!80!black}{Zero penalty}}};


% ==========================================
% COLUMN 3: SISA Framework (Sharded)
% ==========================================
\node[draw=mlsysgreen!40, fill=white, rounded corners=6pt, line width=0.9pt, minimum width=\colW cm, minimum height=\colH cm, anchor=north west] (c3) at (10.8, 0) {};
\node[fill=mlsysgreen!15, text=mlsysgreen!80!black, font=\bfseries\footnotesize, rounded corners=3pt, minimum width=\colW cm - 4pt, minimum height=20pt, anchor=north] at ([yshift=-2pt]c3.north) {SISA Framework (Sharded)};

% Diagram 3 (top of col 3)
% Shards S1, S2, S3
\node[draw=mlsysgreen!80, fill=mlsysgreen!6, rounded corners=2pt, line width=0.7pt, minimum width=0.55cm, minimum height=1.3cm, font=\fontsize{7}{8}\selectfont, text=mlsysgreen!80!black] at (11.35, \diagY) {$S_1$};
\node[draw=mlsysgreen!80, fill=mlsysgreen!6, rounded corners=2pt, line width=0.7pt, minimum width=0.55cm, minimum height=1.3cm, font=\fontsize{7}{8}\selectfont, text=mlsysgreen!80!black] at (12.05, \diagY) {$S_2$};
\node[draw=mlsysred!80, fill=mlsysred!6, rounded corners=2pt, line width=0.7pt, minimum width=0.55cm, minimum height=1.3cm, align=center] at (12.75, \diagY) {
    \fontsize{7}{8}\selectfont\color{mlsysred!90!black} $S_3$\\[2pt]
    \fontsize{5}{6}\selectfont\color{mlsysred} retrain
};

\draw[->, mlsysgray!70, line width=1.4pt] (13.2, \diagY) -- (13.6, \diagY);

\node[draw=mlsysblue!80, fill=white, rounded corners=3pt, line width=0.8pt, minimum width=1.7cm, minimum height=1.3cm, align=center] at (14.65, \diagY) {
    \textbf{\fontsize{7.5}{9}\selectfont Retrain}\\[2pt]
    \textbf{\fontsize{7.5}{9}\selectfont Shard 3 Only}
};

\draw[mlsysgray!25, line width=0.6pt] ([xshift=10pt, yshift=-78pt]c3.north west) -- ([xshift=-10pt, yshift=-78pt]c3.north east);

% Operational Profile 3
\node[font=\bfseries\scriptsize, text=mlsysdark, anchor=west] at (11.15, -3.1) {Operational Profile};
\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (11.15, -3.5) {Guarantee:};
\node[font=\bfseries\fontsize{7.5}{9}\selectfont, text=mlsysgreen!80!black, anchor=west] at (11.15, -3.8) {Exact unlearning (within shard)};

\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (11.15, -4.3) {Corpus Access:};
\node[font=\fontsize{7.5}{9}\selectfont, text=mlsysdark!90, anchor=west] at (11.15, -4.6) {Only target shard data required};

\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (11.15, -5.1) {Primary Bottleneck:};
\node[font=\fontsize{7.5}{9}\selectfont, text=mlsysdark!90, anchor=west] at (11.15, -5.4) {Serving aggregation overhead};

\draw[mlsysgray!25, line width=0.6pt] ([xshift=10pt, yshift=-165pt]c3.north west) -- ([xshift=-10pt, yshift=-165pt]c3.north east);

% Unlearning Fidelity 3
\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (11.15, -6.2) {Unlearning Fidelity:};
\fill[mlsysgray!20, rounded corners=2.5pt] (11.15, -6.75) rectangle (15.45, -6.4);
\fill[mlsysgreen!25, rounded corners=2.5pt] (11.15, -6.75) rectangle (15.25, -6.4);
\node[font=\bfseries\fontsize{7}{8}\selectfont, text=mlsysgreen!80!black] at (13.3, -6.57) {$\sim$96\% (Shard-Exact)};

% Compute Overhead 3
\node[font=\fontsize{7}{8}\selectfont, text=mlsysgray, anchor=west] at (11.15, -7.2) {Compute Overhead:};
\node[draw=mlsysblue!70, fill=mlsysblue!10, rounded corners=3pt, line width=0.7pt, minimum width=4.3cm, minimum height=14pt, font=\bfseries\fontsize{7}{8}\selectfont, text=mlsysblue!90!black] at (13.3, -7.6) {$1/K$ Shard Cost ($\approx 5$--$10\%$)};

% Serving Impact 3
\node[font=\fontsize{7.5}{9}\selectfont, text=mlsysdark!90, anchor=west] at (11.15, -8.3) {Serving Impact: \textbf{\textcolor{mlsysamber!90!black}{$K$-model voting}}};
...
class UnlearningCostAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt, fmt_count, fmt_memory_capacity, fmt_percent, fmt_percent_range, fmt_rate, fmt_time, fmt_usd

    gpu_name_str = gpu.name
    gpu_mem_str = fmt_memory_capacity(gpu.memory.capacity, unit=GiB)
    gpu_rpeak_str = fmt(gpu.compute.peak_flops, precision=0)
...
**Baseline (full retraining)**: In this sizing scenario, a `{python} UnlearningCostAnalysis.model_params_str`-parameter model at GPT-3\index{GPT-3} scale retrains on `{python} UnlearningCostAnalysis.full_retrain_gpus_str` `{python} UnlearningCostAnalysis.gpu_name_str` (`{python} UnlearningCostAnalysis.gpu_mem_str` SXM4) GPUs (assuming dense FP16 $R_{\text{peak}} =$ `{python} UnlearningCostAnalysis.gpu_rpeak_str` per accelerator at model FLOPs utilization (MFU) $\eta_{\text{MFU}} \approx 32\%$) for approximately `{python} UnlearningCostAnalysis.full_time_days_str` at a cost of roughly `{python} UnlearningCostAnalysis.full_cost_m_str`.

**Engineering fix**: SISA training partitions data into $K =$ `{python} UnlearningCostAnalysis.n_shards_str` independent shards, training `{python} UnlearningCostAnalysis.n_shards_str` sub-models. To delete one datum, retrain only the specific shard containing it (`{python} UnlearningCostAnalysis.shard_fraction_pct_str` of data). New cost: `{python} UnlearningCostAnalysis.sisa_cost_str`. Time: approximately `{python} UnlearningCostAnalysis.sisa_time_hr_str`.

**Trade-off**: Accuracy drops `{python} UnlearningCostAnalysis.acc_drop_pct_range_str` because each sub-model sees less data. Inference slows because predictions must be aggregated across `{python} UnlearningCostAnalysis.n_shards_str` sub-models. The gain is real but rate-limited. SISA shrinks the unit of retraining from the whole model to one shard, so it pays off only while daily erasure requests stay well below `{python} UnlearningCostAnalysis.n_shards_str`, the shard count. At the `{python} UnlearningCostAnalysis.deletion_requests_per_day_str` in this scenario, a daily batch touches essentially every shard, and the cost converges back on a full retrain. Sharding buys tractable unlearning at sparse erasure rates, at the price of model quality; it does not make continuous erasure cheap.
...
**Context**: In 2019, Apple and Goldman Sachs faced intense public scrutiny when customers reported receiving credit limits up to 10$\times$ higher than their spouses despite shared tax filings and comparable credit scores [@nydfs2021applecard].

**Evidence boundary**: The public record documents complaints, the subsequent investigation, and the regulator's finding, but it does not disclose the underwriting pipeline's latency target, internal explanation architecture, or later platform changes. The systems account below is therefore an explicit reconstruction, not a report of Apple or Goldman Sachs implementation details.
...
The SHAP framework[^fn-shapley-value-attribution] computes each feature's contribution by evaluating the model on all possible feature subsets. Starting from the baseline prediction of 0.50, adding income (\$45K, slightly below average) decreases approval probability by 0.05.
...
Adding `debt_ratio` (0.55, high) decreases the modeled approval probability by an additional 0.25 relative to the SHAP background distribution. Adding `credit_score` (620) decreases it by 0.12. The additive accounting yields $0.50 - 0.05 - 0.25 - 0.12 = 0.08$, corresponding to $\Pr(\text{deny}) = 0.92$. Under this attribution setup, debt ratio has the largest assigned contribution to the output, followed by credit score and income. This ranking is a hypothesis about model sensitivity under the chosen background distribution, not evidence that debt ratio caused the decision or that lowering it below 40 percent would flip the output. A recourse claim requires a separately validated counterfactual that respects feature dependencies, immutable attributes, feasible user actions, and the decision policy that will apply when the user returns.
...
Another posthoc approach involves **Counterfactual Explanations**\index{Counterfactual Explanations},[^fn-counterfactual-recourse-explanation] which estimate how a model output changes under specified input modifications. These are relevant for decision-facing applications such as credit or hiring systems, but a mathematical output flip is not automatically recourse. A useful counterfactual must distinguish mutable from immutable features, preserve domain constraints and feature dependencies, identify an action within the user's control, and remain valid under the institution's future decision policy [@wachter2017]. Counterfactual generation and validation therefore require domain models, policy versioning, and operational commitments in addition to constrained optimization.
...
% Geometry: 6 columns
% x = 1.35, 4.25, 7.15, 10.05, 12.95, 15.85 (cardW = 2.65cm, total width = 17.2cm)
\def\cardW{2.65}
\def\cardH{6.3}
\def\txtW{2.35cm}

% Top Grouping Banners
% Left Banner (Cards 1 to 3: span x = 0.0 to 8.5)
\node[draw=mlsysgreen!60, fill=mlsysgreen!8, rounded corners=4pt, line width=0.8pt, minimum width=8.5cm, minimum height=0.7cm, font=\bfseries\scriptsize, text=mlsysgreen!90!black, anchor=west] at (0.0, 5.8) {Inherently Interpretable (Transparent by Construction)};

% Right Banner (Cards 4 to 6: span x = 8.7 to 17.2)
\node[draw=mlsyspurple!60, fill=mlsyspurple!8, rounded corners=4pt, line width=0.8pt, minimum width=8.5cm, minimum height=0.7cm, font=\bfseries\scriptsize, text=mlsyspurple!90!black, anchor=west] at (8.7, 5.8) {Surrogate Attribution Required (Post-Hoc Analysis)};

% Spectrum Arrow & Bar
\node[font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysblue!90!black, anchor=west] at (0.0, 5.15) {HIGH TRANSPARENCY / EXACT REASONING};
\node[font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysred!90!black, anchor=east] at (17.2, 5.15) {HIGH CAPACITY / OPAQUE REPRESENTATION};

\shade[left color=mlsysblue!80, middle color=mlsysamber!80, right color=mlsysred!85, rounded corners=2pt] (0.0, 4.8) rectangle (17.2, 4.95);
\draw[mlsysgray!40, rounded corners=2pt, line width=0.5pt] (0.0, 4.8) rectangle (17.2, 4.95);

% Card 1: Linear Models
\node[draw=mlsysblue!50, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm, anchor=north] (c1) at (1.35, 4.5) {};
\node[fill=mlsysblue!12, text=mlsysblue!90!black, font=\bfseries\fontsize{6.5}{7.5}\selectfont, rounded corners=2pt, minimum width=\cardW cm - 4pt, minimum height=14pt, anchor=north] at ([yshift=-2pt]c1.north) {INHERENT};
\node[font=\bfseries\scriptsize, text=mlsysdark, anchor=north] at ([yshift=-18pt]c1.north) {Linear Models};
\node[font=\fontsize{6.5}{7.5}\selectfont, text=mlsysgray, anchor=north] at ([yshift=-28pt]c1.north) {GLM / GAMs};
\draw[mlsysgray!25, line width=0.5pt] ([xshift=6pt, yshift=-37pt]c1.north west) -- ([xshift=-6pt, yshift=-37pt]c1.north east);
\node[anchor=north, align=left, text width=\txtW, text=mlsysdark!90] at ([yshift=-42pt]c1.north) {
...
\node[draw=mlsysgreen!50, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm, anchor=north] (c2) at (4.25, 4.5) {};
\node[fill=mlsysgreen!12, text=mlsysgreen!80!black, font=\bfseries\fontsize{6.5}{7.5}\selectfont, rounded corners=2pt, minimum width=\cardW cm - 4pt, minimum height=14pt, anchor=north] at ([yshift=-2pt]c2.north) {INHERENT};
\node[font=\bfseries\scriptsize, text=mlsysdark, anchor=north] at ([yshift=-18pt]c2.north) {Decision Trees};
\node[font=\fontsize{6.5}{7.5}\selectfont, text=mlsysgray, anchor=north] at ([yshift=-28pt]c2.north) {CART / Rule Lists};
\draw[mlsysgray!25, line width=0.5pt] ([xshift=6pt, yshift=-37pt]c2.north west) -- ([xshift=-6pt, yshift=-37pt]c2.north east);
\node[anchor=north, align=left, text width=\txtW, text=mlsysdark!90] at ([yshift=-42pt]c2.north) {
    {\bfseries\fontsize{6.5}{7.5}\selectfont Mechanism:}\\[2pt]
    {\fontsize{6}{7.2}\selectfont
...
\node[draw=mlsysteal!50, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm, anchor=north] (c3) at (7.15, 4.5) {};
\node[fill=mlsysteal!12, text=mlsysteal!80!black, font=\bfseries\fontsize{6.5}{7.5}\selectfont, rounded corners=2pt, minimum width=\cardW cm - 4pt, minimum height=14pt, anchor=north] at ([yshift=-2pt]c3.north) {HYBRID};
\node[font=\bfseries\scriptsize, text=mlsysdark, anchor=north] at ([yshift=-18pt]c3.north) {Prototypes / CBM};
\node[font=\fontsize{6.5}{7.5}\selectfont, text=mlsysgray, anchor=north] at ([yshift=-28pt]c3.north) {ProtoPNet / Concept};
\draw[mlsysgray!25, line width=0.5pt] ([xshift=6pt, yshift=-37pt]c3.north west) -- ([xshift=-6pt, yshift=-37pt]c3.north east);
\node[anchor=north, align=left, text width=\txtW, text=mlsysdark!90] at ([yshift=-42pt]c3.north) {
    {\bfseries\fontsize{6.5}{7.5}\selectfont Mechanism:}\\[2pt]
    {\fontsize{6}{7.2}\selectfont
...
\node[draw=mlsysamber!50, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm, anchor=north] (c4) at (10.05, 4.5) {};
\node[fill=mlsysamber!12, text=mlsysamber!90!black, font=\bfseries\fontsize{6.5}{7.5}\selectfont, rounded corners=2pt, minimum width=\cardW cm - 4pt, minimum height=14pt, anchor=north] at ([yshift=-2pt]c4.north) {POSTHOC};
\node[font=\bfseries\scriptsize, text=mlsysdark, anchor=north] at ([yshift=-18pt]c4.north) {Tree Ensembles};
\node[font=\fontsize{6.5}{7.5}\selectfont, text=mlsysgray, anchor=north] at ([yshift=-28pt]c4.north) {GBDT / XGBoost};
\draw[mlsysgray!25, line width=0.5pt] ([xshift=6pt, yshift=-37pt]c4.north west) -- ([xshift=-6pt, yshift=-37pt]c4.north east);
\node[anchor=north, align=left, text width=\txtW, text=mlsysdark!90] at ([yshift=-42pt]c4.north) {
    {\bfseries\fontsize{6.5}{7.5}\selectfont Mechanism:}\\[2pt]
    {\fontsize{6}{7.2}\selectfont
    ...
    {\fontsize{6}{7.2}\selectfont
    TreeSHAP (exact Shapley in poly-time); feature gain rankings}
};

% Card 5: Deep Neural Nets
\node[draw=mlsyspurple!50, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm, anchor=north] (c5) at (12.95, 4.5) {};
\node[fill=mlsyspurple!12, text=mlsyspurple!90!black, font=\bfseries\fontsize{6.5}{7.5}\selectfont, rounded corners=2pt, minimum width=\cardW cm - 4pt, minimum height=14pt, anchor=north] at ([yshift=-2pt]c5.north) {POSTHOC};
\node[font=\bfseries\scriptsize, text=mlsysdark, anchor=north] at ([yshift=-18pt]c5.north) {Deep Neural Nets};
\node[font=\fontsize{6.5}{7.5}\selectfont, text=mlsysgray, anchor=north] at ([yshift=-28pt]c5.north) {MLP / CNN / ResNet};
\draw[mlsysgray!25, line width=0.5pt] ([xshift=6pt, yshift=-37pt]c5.north west) -- ([xshift=-6pt, yshift=-37pt]c5.north east);
\node[anchor=north, align=left, text width=\txtW, text=mlsysdark!90] at ([yshift=-42pt]c5.north) {
    {\bfseries\fontsize{6.5}{7.5}\selectfont Mechanism:}\\[2pt]
    {\fontsize{6}{7.2}\selectfont
    Dense hidden layers\\[1.5pt]
    Distributed latents\\[1.5pt]
    Nonlinear activation}\\[6pt]
    {\bfseries\fontsize{6.5}{7.5}\selectfont Attribution Tool:}\\[2pt]
    {\fontsize{6}{7.2}\selectfont
...
\node[draw=mlsysred!50, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm, anchor=north] (c6) at (15.85, 4.5) {};
\node[fill=mlsysred!12, text=mlsysred!90!black, font=\bfseries\fontsize{6.5}{7.5}\selectfont, rounded corners=2pt, minimum width=\cardW cm - 4pt, minimum height=14pt, anchor=north] at ([yshift=-2pt]c6.north) {OPAQUE};
\node[font=\bfseries\scriptsize, text=mlsysdark, anchor=north] at ([yshift=-18pt]c6.north) {Foundation LLMs};
\node[font=\fontsize{6.5}{7.5}\selectfont, text=mlsysgray, anchor=north] at ([yshift=-28pt]c6.north) {Transformers / MoE};
\draw[mlsysgray!25, line width=0.5pt] ([xshift=6pt, yshift=-37pt]c6.north west) -- ([xshift=-6pt, yshift=-37pt]c6.north east);
\node[anchor=north, align=left, text width=\txtW, text=mlsysdark!90] at ([yshift=-42pt]c6.north) {
    {\bfseries\fontsize{6.5}{7.5}\selectfont Mechanism:}\\[2pt]
    {\fontsize{6}{7.2}\selectfont
    ...
    {\bfseries\fontsize{6.5}{7.5}\selectfont Attribution Tool:}\\[2pt]
    {\fontsize{6}{7.2}\selectfont
    Attention rollout, TCAV vectors, circuits}
};

% ==========================================
% BOTTOM CONTAINER: Dimensions
% ==========================================
\def\botTop{-2.1}
\def\botH{2.5}
\node[draw=mlsysgray!35, fill=white, rounded corners=6pt, line width=0.8pt, minimum width=17.2cm, minimum height=\botH cm, anchor=north west] (botbox) at (0.0, \botTop) {};

% Row 1: Serving Latency & Cost
\def\r1Y{-2.6}
\node[draw=none, fill=mlsysblue!10, rounded corners=3pt, minimum width=2.8cm, minimum height=15pt, font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysdark, anchor=west] at (0.2, \r1Y) {Serving Latency \& Cost};
\draw[->, mlsysgray!60, line width=0.8pt] (3.3, \r1Y) -- (16.9, \r1Y);
\node[font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysgreen!80!black, anchor=west] at (3.4, \r1Y + 0.22) {$< 1\text{ ms}$ (single CPU core)};
\node[font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysamber!90!black, anchor=center] at (10.1, \r1Y + 0.22) {$5$--$20\text{ ms}$ (Vectorized)};
\node[font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysred!90!black, anchor=east] at (16.8, \r1Y + 0.22) {$50$--$500+\text{ ms}$ (Multi-GPU Fleet)};

% Row 2: Regulatory Auditability
\def\r2Y{-3.35}
\node[draw=none, fill=mlsysblue!10, rounded corners=3pt, minimum width=2.8cm, minimum height=15pt, font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysdark, anchor=west] at (0.2, \r2Y) {Regulatory Auditability};
\draw[<-, mlsysgray!60, line width=0.8pt] (3.3, \r2Y) -- (16.9, \r2Y);
\node[font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysblue!90!black, anchor=west] at (3.4, \r2Y - 0.22) {Direct mathematical proof (court defensible)};
\node[font=\fontsize{6.5}{7.5}\selectfont, text=mlsyspurple!90!black, anchor=center] at (10.6, \r2Y - 0.22) {Surrogate-dependent (approximate)};
\node[font=\fontsize{6.5}{7.5}\selectfont, text=mlsysred!90!black, anchor=east] at (16.8, \r2Y - 0.22) {Heuristic / Plausible Rationales};

% Row 3: Expressive Capacity
\def\r3Y{-4.1}
\node[draw=none, fill=mlsysblue!10, rounded corners=3pt, minimum width=2.8cm, minimum height=15pt, font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysdark, anchor=west] at (0.2, \r3Y) {Expressive Capacity};
\draw[->, mlsysgray!60, line width=0.8pt] (3.3, \r3Y) -- (16.9, \r3Y);
\node[font=\fontsize{6.5}{7.5}\selectfont, text=mlsysdark!85, anchor=west] at (3.4, \r3Y - 0.22) {Low-dimensional / tabular patterns};
\node[font=\fontsize{6.5}{7.5}\selectfont, text=mlsysdark!85, anchor=center] at (10.1, \r3Y - 0.22) {Nonlinear feature interactions};
\node[font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysdark, anchor=east] at (16.8, \r3Y - 0.22) {Unstructured multimodal representations};
...
\node[draw=mlsysblue!70, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm] (n1) at (0, \topY) {};
\node[fill=mlsysblue!12, text=mlsysblue!90!black, font=\bfseries\fontsize{6.5}{7.5}\selectfont, rounded corners=2pt, minimum width=\cardW cm - 4pt, minimum height=14pt, anchor=north] at ([yshift=-2pt]n1.north) {1 $\cdot$ COLLECT};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-18pt]n1.north) {Predictions};
\draw[mlsysgray!25, line width=0.5pt] ([xshift=6pt, yshift=-30pt]n1.north west) -- ([xshift=-6pt, yshift=-30pt]n1.north east);
\node[font=\fontsize{7}{9}\selectfont, text=mlsysdark!90, align=center, text width=2.2cm, anchor=north] at ([yshift=-35pt]n1.north) {
    Inference stream\\[2pt]
    Logged features\\[2pt]
    \& metadata
};

% Node 2: Subgroup Drift (Measure)
\node[draw=mlsysgreen!70, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm] (n2) at (3.2, \topY) {};
\node[fill=mlsysgreen!12, text=mlsysgreen!80!black, font=\bfseries\fontsize{6.5}{7.5}\selectfont, rounded corners=2pt, minimum width=\cardW cm - 4pt, minimum height=14pt, anchor=north] at ([yshift=-2pt]n2.north) {2 $\cdot$ MEASURE};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-18pt]n2.north) {Subgroup Drift};
\draw[mlsysgray!25, line width=0.5pt] ([xshift=6pt, yshift=-30pt]n2.north west) -- ([xshift=-6pt, yshift=-30pt]n2.north east);
\node[font=\fontsize{7}{9}\selectfont, text=mlsysdark!90, align=center, text width=2.2cm, anchor=north] at ([yshift=-35pt]n2.north) {
    FPR / TPR parity\\[2pt]
    Per demographic\\[2pt]
    slice $G_i$
};
...
\node[diamond, draw=mlsysamber!90!black, fill=mlsysamber!10, line width=1.1pt, minimum width=2.4cm, minimum height=2.4cm, align=center, inner sep=0pt] (dia) at (6.5, \topY) {
    \textbf{\fontsize{8}{9}\selectfont Gap $> \varepsilon$\ ?}\\[2pt]
    \fontsize{6.5}{7.5}\selectfont\color{mlsysamber!90!black} Threshold
};

% Node 4: Fairness Alert (Respond)
\node[draw=mlsysred!70, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm] (n4) at (9.8, \topY) {};
\node[fill=mlsysred!12, text=mlsysred!90!black, font=\bfseries\fontsize{6.5}{7.5}\selectfont, rounded corners=2pt, minimum width=\cardW cm - 4pt, minimum height=14pt, anchor=north] at ([yshift=-2pt]n4.north) {4 $\cdot$ RESPOND};
\node[font=\bfseries\small, text=mlsysred!90!black, anchor=north] at ([yshift=-18pt]n4.north) {Fairness Alert};
\draw[mlsysgray!25, line width=0.5pt] ([xshift=6pt, yshift=-30pt]n4.north west) -- ([xshift=-6pt, yshift=-30pt]n4.north east);
\node[font=\fontsize{7}{9}\selectfont, text=mlsysdark!90, align=center, text width=2.2cm, anchor=north] at ([yshift=-35pt]n4.north) {
    Breach logged\\[2pt]
    Traffic throttled\\[2pt]
    Audit dispatch
};

% Node 5: Recalibration (Improve)
\node[draw=mlsysblue!70, fill=white, rounded corners=5pt, line width=0.9pt, minimum width=\cardW cm, minimum height=\cardH cm] (n5) at (13.0, \topY) {};
\node[fill=mlsysblue!12, text=mlsysblue!90!black, font=\bfseries\fontsize{6.5}{7.5}\selectfont, rounded corners=2pt, minimum width=\cardW cm - 4pt, minimum height=14pt, anchor=north] at ([yshift=-2pt]n5.north) {5 $\cdot$ IMPROVE};
\node[font=\bfseries\small, text=mlsysdark, anchor=north] at ([yshift=-18pt]n5.north) {Recalibration};
\draw[mlsysgray!25, line width=0.5pt] ([xshift=6pt, yshift=-30pt]n5.north west) -- ([xshift=-6pt, yshift=-30pt]n5.north east);
\node[font=\fontsize{7}{9}\selectfont, text=mlsysdark!90, align=center, text width=2.2cm, anchor=north] at ([yshift=-35pt]n5.north) {
    Bias-aware\\[2pt]
    re-weighting or\\[2pt]
    decision shift
};
...
\node[draw=mlsysgreen!80, fill=mlsysgreen!8, rounded corners=4pt, line width=0.9pt, minimum width=2.6cm, minimum height=1.1cm, align=center] (serv) at (6.5, 2.6) {
    \textbf{\fontsize{7.5}{8.5}\selectfont\color{mlsysgreen!90!black} Serving Continues}\\[2pt]
    \fontsize{6.5}{7.5}\selectfont Parity delta within budget
};

% Connectors in Flowchart
\draw[->, mlsysgray!70, line width=1.4pt] (n1.east) -- (n2.west);
\draw[->, mlsysgray!70, line width=1.4pt] (n2.east) -- (dia.west);

\draw[->, mlsysred!90, line width=1.4pt] (dia.east) -- (n4.west)
    node[pos=0.45, above=2pt, font=\bfseries\scriptsize, text=mlsysred!90!black] {YES};

\draw[->, mlsysgreen!80!black, line width=1.4pt] (dia.south) -- (serv.north)
    node[pos=0.45, right=2pt, font=\bfseries\scriptsize, text=mlsysgreen!80!black] {NO};

\draw[->, mlsysgray!70, line width=1.4pt] (n4.east) -- (n5.west);

% Feedback Loop
\draw[->, dashed, mlsysblue!90!black, line width=1.3pt] (n5.south) -- ++(0, -2.1) -| (n1.south)
    node[pos=0.25, fill=white, inner xsep=8pt, inner ysep=3.5pt, draw=mlsysblue!70, rounded corners=3pt, font=\bfseries\fontsize{7}{8}\selectfont, text=mlsysblue!90!black] {Feedback Loop: Retrained weights redeployed};
...
\def\botTop{1.0}
\def\botH{5.2}
\node[draw=mlsysgray!35, fill=mlsysblue!2, rounded corners=6pt, line width=0.8pt, minimum width=15.8cm, minimum height=\botH cm, anchor=north] (botbox) at (6.5, \botTop) {};

\node[font=\bfseries\small, text=mlsysdark, anchor=north] at (6.5, \botTop - 0.25) {Production Subgroup Metrics Tracked per Evaluation Window};
...
\node[draw=mlsysblue!40, fill=white, rounded corners=4pt, line width=0.7pt, minimum width=\mCardW cm, minimum height=\mCardH cm, anchor=north] (m1) at (1.6, \mY) {};
\node[fill=mlsysblue!10, text=mlsysblue!90!black, font=\bfseries\fontsize{7}{8}\selectfont, rounded corners=2pt, minimum width=\mCardW cm - 3pt, minimum height=14pt, anchor=north] at ([yshift=-1.5pt]m1.north) {Accuracy \& Error Parity};
\node[font=\bfseries\fontsize{7}{8}\selectfont, text=mlsysdark, anchor=north] at ([yshift=-20pt]m1.north) {$|\text{FPR}_A - \text{FPR}_B| \le \varepsilon$};
\node[font=\fontsize{6.5}{8.5}\selectfont, text=mlsysdark!85, align=left, text width=4.4cm, anchor=north] at ([yshift=-36pt]m1.north) {
    Threshold: $\varepsilon = 0.05$ max delta\\[2.5pt]
    Window: Evaluated every 10k requests\\[2.5pt]
    Ensures uniform false positive penalty
};

% Metric Card 2: Calibration by Demographic
\node[draw=mlsysgreen!40, fill=white, rounded corners=4pt, line width=0.7pt, minimum width=\mCardW cm, minimum height=\mCardH cm, anchor=north] (m2) at (6.5, \mY) {};
\node[fill=mlsysgreen!10, text=mlsysgreen!80!black, font=\bfseries\fontsize{7}{8}\selectfont, rounded corners=2pt, minimum width=\mCardW cm - 3pt, minimum height=14pt, anchor=north] at ([yshift=-1.5pt]m2.north) {Calibration by Demographic};
\node[font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysdark, anchor=north] at ([yshift=-20pt]m2.north) {$|\Pr(Y=1 \mid \text{score}=s,\, G=g) - s|$};
\node[font=\fontsize{6.5}{8.5}\selectfont, text=mlsysdark!85, align=left, text width=4.4cm, anchor=north] at ([yshift=-36pt]m2.north) {
    Threshold: ECE per group $\le 0.02$\\[2.5pt]
    Window: Two-sample KS-test daily\\[2.5pt]
    Validates confidence score fidelity
};

% Metric Card 3: Statistical Acceptance Rate
\node[draw=mlsysamber!40, fill=white, rounded corners=4pt, line width=0.7pt, minimum width=\mCardW cm, minimum height=\mCardH cm, anchor=north] (m3) at (11.4, \mY) {};
\node[fill=mlsysamber!10, text=mlsysamber!90!black, font=\bfseries\fontsize{7}{8}\selectfont, rounded corners=2pt, minimum width=\mCardW cm - 3pt, minimum height=14pt, anchor=north] at ([yshift=-1.5pt]m3.north) {Statistical Acceptance Rate};
\node[font=\bfseries\fontsize{6.5}{7.5}\selectfont, text=mlsysdark, anchor=north] at ([yshift=-20pt]m3.north) {$|\Pr(\hat{Y}=1 \mid G=a) - \Pr(\hat{Y}=1 \mid G=b)|$};
\node[font=\fontsize{6.5}{8.5}\selectfont, text=mlsysdark!85, align=left, text width=4.4cm, anchor=north] at ([yshift=-36pt]m3.north) {
    Threshold: Adverse impact ratio $> 0.80$\\[2.5pt]
    Window: Rolling 24-hour baseline\\[2.5pt]
    Detects systemic population shifts
};

% Bottom Telemetry Banner
\node[draw=mlsysgray!35, fill=white, rounded corners=3pt, line width=0.6pt, minimum width=15.0cm, minimum height=0.65cm, font=\fontsize{6.5}{7.5}\selectfont, text=mlsysdark!90] at (6.5, \botTop - 4.5) {
    Telemetry Overhead: $+12\text{ ms}$ logging per inference $\;\cdot\;$ $\sim$1.7 GB/day per 100M queries $\;\cdot\;$ 150 active monitor rules per model
};
...
from mlsysim.fmt import check, fmt, fmt_count, fmt_memory, fmt_percent, fmt_qty
...
These examples demonstrate that responsible AI implementation depends on alignment between technical capabilities and sociotechnical contexts, organizational incentives, human behavior, stakeholder values, and institutional governance structures. Sustaining that alignment requires monitoring mechanisms that provide operational observability. However, the emergence of generative AI has transformed the nature of the failures that must be monitored.

### Responsibility in the generative era {#sec-responsible-ai-responsibility-generative-era-3c3e}

Generative AI does not replace core fairness, privacy, and explainability concerns; it changes the control surface. Instead of only auditing labels or feature attributions, operators must govern prompts, retrieved context, reward models, and open-ended outputs.

The transition from discriminative classification to generative large language models (LLMs) broadens the engineering surface of responsibility. Fairness remains concerned with how quality, refusals, errors, representation, and access are distributed across people and groups; open-ended outputs make those quantities harder to define and sample. Alignment introduces a separate problem: constraining stochastic outputs and actions to remain consistent with intended objectives and policies across a combinatorial range of prompts. Production systems need both fairness evaluation and behavioral alignment rather than relabeling one as the other. A common alignment mechanism is the multi-stage process in @fig-rlhf-pipeline.

::: {#fig-rlhf-pipeline fig-env="figure" fig-pos="htb" fig-cap="**Reinforcement Learning from Human Feedback (RLHF) Alignment Pipeline**: Six-stage process for aligning generative models: (1) a pretrained Base Model, (2) supervised fine-tuning (SFT) on demonstrations, (3) Human Preference collection, (4) Reward Model training, (5) proximal policy optimization (PPO) training, and (6) a final Aligned Model. The figure also annotates label cost, compute overhead, and alignment tax, plus three RLHF failure modes (reward hacking, misaligned internal goals, power-seeking behavior)." fig-alt="Six-stage horizontal pipeline: Base Model -> SFT -> Human Preferences -> Reward Training -> PPO Training -> Aligned Model. Trade-off annotations for label cost, compute, alignment tax, plus three listed RLHF failure modes."}
```{=latex}
...
\begin{tikzpicture}[
  ...
  arr/.style={-{Stealth[scale=0.9]}, line width=1.3pt, color=mlsysgray}
]

  % Top Row: 6 Stages
  % 1. Pretrain
  \node[card, draw=mlsysblue, fill=white] (c1) at (0, 0) {
    {\fontsize{7.5}{9}\selectfont\textbf{\textcolor{mlsysblue}{1 $\cdot$ PRETRAIN}}}\\[4pt]
    {\fontsize{9}{11}\selectfont\textbf{\textcolor{mlsysdarkgray}{Base Checkpoint}}}\\[5pt]
    {\fontsize{7.5}{10}\selectfont\textcolor{mlsysgray}{Pre-trained\\dense / MoE\\[3pt]
    Next-token loss\\[3pt]
    $\mathcal{O}(\text{TB})$ tokens}}
  };

  % 2. SFT
  \node[card, draw=mlsysblue!90!black, fill=white, right=0.35cm of c1] (c2) {
    {\fontsize{7.5}{9}\selectfont\textbf{\textcolor{mlsysblue!90!black}{2 $\cdot$ SFT}}}\\[4pt]
    {\fontsize{9}{11}\selectfont\textbf{\textcolor{mlsysdarkgray}{Fine-Tuning}}}\\[5pt]
    {\fontsize{7.5}{10}\selectfont\textcolor{mlsysgray}{Curated dialog\\demonstrations\\[3pt]
    $\sim$10k--100k\\instruction pairs}}
  };

  % 3. Preference
  \node[card, draw=mlsysamber, fill=white, right=0.35cm of c2] (c3) {
    {\fontsize{7.5}{9}\selectfont\textbf{\textcolor{mlsysamber}{3 $\cdot$ PREFERENCE}}}\\[4pt]
    {\fontsize{9}{11}\selectfont\textbf{\textcolor{mlsysdarkgray}{Pairwise Data}}}\\[5pt]
    {\fontsize{7.5}{10}\selectfont\textcolor{mlsysgray}{Pairwise outputs\\[2pt]
    \textcolor{mlsysdarkgray}{\boldmath$(y_w \succ y_l)$}\\[3pt]
    50k--500k\\labeled prompts}}
  };

  % 4. Reward
  \node[card, draw=mlsysgreen, fill=white, right=0.35cm of c3] (c4) {
    {\fontsize{7.5}{9}\selectfont\textbf{\textcolor{mlsysgreen}{4 $\cdot$ REWARD}}}\\[4pt]
    {\fontsize{9}{11}\selectfont\textbf{\textcolor{mlsysdarkgray}{Reward Network}}}\\[5pt]
    {\fontsize{7.5}{10}\selectfont\textcolor{mlsysgray}{Bradley-Terry\\preference loss\\[3pt]
    \textcolor{mlsysgreen}{\textbf{$r(x, y) \in \mathbb{R}$}}\\[3pt]
    Scalar proxy}}
  };

  % 5. Policy RL
  \node[card, draw=mlsysblue!90!black, fill=white, right=0.35cm of c4] (c5) {
    {\fontsize{7.5}{9}\selectfont\textbf{\textcolor{mlsysblue!90!black}{5 $\cdot$ POLICY RL}}}\\[4pt]
    {\fontsize{9}{11}\selectfont\textbf{\textcolor{mlsysdarkgray}{PPO Tuning}}}\\[5pt]
    {\fontsize{7.5}{10}\selectfont\textcolor{mlsysgray}{Policy gradient\\[2pt]
    \textcolor{mlsysdarkgray}{\boldmath$\max \mathbb{E}[r(x,y)]$}\\[2pt]
    $- \beta\,\text{KL penalty}$\\[3pt]
    Actor-Critic}}
  };

  % 6. Deploy
  \node[card, draw=mlsysgreen, fill=white, right=0.35cm of c5] (c6) {
    {\fontsize{7.5}{9}\selectfont\textbf{\textcolor{mlsysgreen}{6 $\cdot$ DEPLOY}}}\\[4pt]
    {\fontsize{9}{11}\selectfont\textbf{\textcolor{mlsysdarkgray}{Aligned Policy}}}\\[5pt]
    {\fontsize{7.5}{10}\selectfont\textcolor{mlsysgray}{Helpful \& honest\\[2pt]
    Safety bound\\[3pt]
    \textcolor{mlsysred}{\textbf{2--8\% bench tax}}\\[3pt]
    Serving ready}}
  };
  ...
  \node[rounded corners=4pt, draw=mlsysamber, fill=mlsysamber!10, line width=1.1pt,
        inner sep=4pt, minimum width=2.4cm, below=0.7cm of c3] (raters) {
    {\fontsize{8}{10}\selectfont\textbf{\textcolor{mlsysamber!90!black}{Human Raters}}}
  };
  \draw[arr, color=mlsysamber] (raters) -- (c3);

  % Reward r connection from c4 to c5
  \draw[-{Stealth[scale=0.9]}, line width=1.2pt, color=mlsysgreen]
    (c4.south) -- ++(0,-0.55) -| (c5.south)
    node[pos=0.25, fill=mlsysgreen!10, draw=mlsysgreen!50, rounded corners=3pt, inner sep=2.5pt, text=mlsysgreen!90!black]
    {\fontsize{7.5}{9}\selectfont\textbf{Reward $r$}};

  % Midpoint for containers
  \coordinate (midX) at ($(c1.west)!0.5!(c6.east)$);

  % Middle Container: Key RLHF Engineering Trade-Offs
  \node[draw=bordergray, fill=mlsyslightgray!50, rounded corners=6pt, line width=0.8pt,
        minimum width=17.2cm, minimum height=3.6cm, anchor=north] (box_tradeoffs)
        at ([yshift=-1.45cm]midX |- c3.south) {};

  \node[anchor=north, text=mlsysdarkgray] (to_title) at ([yshift=-8pt]box_tradeoffs.north) {
    {\fontsize{9.5}{12}\selectfont\textbf{Key RLHF Engineering Trade-Offs}}
  };

  % Three columns inside Trade-Offs
  \node[draw=mlsysblue!40, fill=white, rounded corners=4pt, line width=0.7pt,
        text width=5.2cm, minimum height=2.2cm, anchor=north] (to2) at ([yshift=-8pt]to_title.south) {
    \vspace{-2pt}
    \begin{center}
      \colorbox{mlsysblue!12}{\makebox[4.9cm]{\strut\fontsize{8}{10}\selectfont\textbf{\textcolor{mlsysblue}{Compute Footprint}}}}\\[4pt]
    \end{center}
    \vspace{-4pt}
    \hspace{4pt}{\fontsize{8}{10}\selectfont\textbf{+50\% to +100\% vs.\ SFT alone}}\\[3pt]
    \hspace{4pt}{\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{4 models in memory: actor, critic, ref, reward}}
  };

  \node[draw=mlsysamber!40, fill=white, rounded corners=4pt, line width=0.7pt,
        text width=5.2cm, minimum height=2.2cm, left=0.4cm of to2] (to1) {
    \vspace{-2pt}
    \begin{center}
      \colorbox{mlsysamber!15}{\makebox[4.9cm]{\strut\fontsize{8}{10}\selectfont\textbf{\textcolor{mlsysamber!90!black}{Data \& Annotation Cost}}}}\\[4pt]
    \end{center}
    \vspace{-4pt}
    \hspace{4pt}{\fontsize{8}{10}\selectfont\textbf{\$0.50--\$5.00 per human label}}\\[3pt]
    \hspace{4pt}{\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{Requires 50k--500k comparison pairs}}
  };

  \node[draw=mlsysred!40, fill=white, rounded corners=4pt, line width=0.7pt,
        text width=5.2cm, minimum height=2.2cm, right=0.4cm of to2] (to3) {
    \vspace{-2pt}
    \begin{center}
      \colorbox{mlsysred!12}{\makebox[4.9cm]{\strut\fontsize{8}{10}\selectfont\textbf{\textcolor{mlsysred}{Alignment Capability Tax}}}}\\[4pt]
    \end{center}
    \vspace{-4pt}
    \hspace{4pt}{\fontsize{8}{10}\selectfont\textbf{\textcolor{mlsysred}{2\% to 8\% benchmark degradation}}}\\[3pt]
    \hspace{4pt}{\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{Over-cautious refusals \& entropy collapse}}
  };

  % Bottom Container: Failure Modes
  \node[draw=bordergray, fill=mlsyslightgray!50, rounded corners=6pt, line width=0.8pt,
        minimum width=17.2cm, minimum height=3.2cm, anchor=north] (box_failures)
        at ([yshift=-0.4cm]box_tradeoffs.south) {};

  \node[fill=mlsysred!15, minimum width=16.6cm, rounded corners=3pt, align=center, anchor=north, inner sep=4pt]
    (fail_hdr) at ([yshift=-7pt]box_failures.north) {
    {\fontsize{8.5}{11}\selectfont\textbf{\textcolor{mlsysred}{RLHF Failure Modes in Autonomous Training}}}
  };

  \node[draw=bordergray, fill=white, rounded corners=4pt, line width=0.6pt,
        text width=5.2cm, minimum height=2.0cm, anchor=north] (fm2) at ([yshift=-8pt]fail_hdr.south) {
    \vspace{2pt}
    \hspace{4pt}{\fontsize{8}{10}\selectfont\textbf{2 $\cdot$ Goal Misgeneralization}}\\[3pt]
    \hspace{4pt}{\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{Out-of-distribution divergence}}\\[2pt]
    \hspace{4pt}{\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{Reward optimal but unsafe in novel context}}
  };

  \node[draw=bordergray, fill=white, rounded corners=4pt, line width=0.6pt,
        text width=5.2cm, minimum height=2.0cm, left=0.4cm of fm2] (fm1) {
    \vspace{2pt}
    \hspace{4pt}{\fontsize{8}{10}\selectfont\textbf{1 $\cdot$ Reward Hacking}}\\[3pt]
    \hspace{4pt}{\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{Exploits flaws in reward proxy}}\\[2pt]
    \hspace{4pt}{\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{Verbosity bias \& sycophancy}}
  };

  \node[draw=bordergray, fill=white, rounded corners=4pt, line width=0.6pt,
        text width=5.2cm, minimum height=2.0cm, right=0.4cm of fm2] (fm3) {
    \vspace{2pt}
    \hspace{4pt}{\fontsize{8}{10}\selectfont\textbf{3 $\cdot$ Strategic Deception}}\\[3pt]
    \hspace{4pt}{\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{Faking alignment during eval}}\\[2pt]
    \hspace{4pt}{\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{Preserving capability at expense of safety}}
  };
...
**Mechanism**: To prevent global grade inflation across dataset volume $D_{\text{vol}}$, the batch standardization algorithm constrained predicted grade distributions to match each school's 3-year historical distribution. This zero-sum optimization matrix forced 39.1 percent of teacher-submitted grades to be downgraded, prioritizing historical cohort variance over individual feature signals.

**Impact**: Public outcry over systemic injustice and student protests forced the UK government to abandon the algorithmic outputs days later, reverting to unadjusted center-assessed grades.

**Fix**: The UK government discarded the standardization algorithm and instituted an appeal framework allowing students to receive whichever grade was higher between center-assessed grades and mock exam scores.

**Systems lesson**: Optimizing for global aggregate metrics across dataset volume $D_{\text{vol}}$ without enforcing individual fairness constraints creates a mathematically bounded optimization pass that is socially and operationally catastrophic.
...
Human-AI collaboration introduces both opportunities and risks. When designed appropriately, systems can augment human judgment, reduce cognitive burden, and enhance consistency in decision-making. However, when poorly designed, they may lead to **Automation Bias**\index{Automation Bias},[^fn-automation-bias-complacency] where users over-rely on model outputs even in the presence of clear errors.

[^fn-automation-bias-complacency]: **Automation Bias**\index{Bias!automation, human-AI interaction}: First studied in aviation in the 1990s, this is the paradox where humans defer to automated systems even when demonstrably incorrect -- and the effect intensifies as system accuracy increases. At 70--80 percent model accuracy, operators accept erroneous outputs at high rates when presented without uncertainty indicators. For ML serving systems, this means higher model accuracy can paradoxically reduce system-level safety by suppressing human oversight, requiring deliberate interface friction (uncertainty visualization, mandatory justification) that adds latency but preserves the human correction channel.
...
class AutomationBias:
    ...
    alpha_pct_math = fmt_math(r"\alpha = " + _alpha)
    ai_error_pct_math = fmt_math(r"1 - S_{\text{AI}} = " + _ai_error)
    sensitivity_pct_str = fmt_percent(sensitivity, precision=1, commas=False, style='prose')
    acc_low_pct_str = fmt_percent(round(acc_low_pct) / 100, precision=0, commas=False, style='prose')
    acc_high_pct_str = fmt_percent(round(acc_high_pct) / 100, precision=0, commas=False, style='prose')
    override_high_pct_str = fmt_percent(round(override_high_pct) / 100, precision=0, commas=False, style='prose')
    override_low_pct_str = fmt_percent(round(override_low_pct) / 100, precision=0, commas=False, style='prose')
```
...
- **Human sensitivity**: $S_{\text{human}} =$ `{python} AutomationBias.human_sensitivity_pct_str`
- **AI sensitivity**: $S_{\text{AI}} =$ `{python} AutomationBias.ai_sensitivity_pct_str`

One might assume the combined system performance would exceed `{python} AutomationBias.ai_sensitivity_prose_str`. However, studies in automation bias show that humans accept erroneous AI recommendations at rates of `{python} AutomationBias.alpha_range_pct_str`. If the AI makes an error (probability `{python} AutomationBias.ai_error_pct_math`) and the human blindly accepts it (`{python} AutomationBias.alpha_pct_math`), accepted AI errors alone create a failure probability of `{python} AutomationBias.fail_prob_math`. In this simplified upper-bound case, where every non-accepted AI error is corrected, sensitivity is `{python} AutomationBias.sensitivity_pct_str`; real workflows can do worse once human override errors, false positives, fatigue, and interface delays are included.
...
- At `{python} AutomationBias.acc_low_pct_str` AI accuracy, human override rate might be $R_{\text{override}} =$ `{python} AutomationBias.override_high_pct_str`.
- At `{python} AutomationBias.acc_high_pct_str` AI accuracy, $R_{\text{override}}$ drops to $\approx$ `{python} AutomationBias.override_low_pct_str`.
...
**Diagnosis**: Optimizing clinical efficacy via aggressive alert dispatch conflicts with patient autonomy and privacy protocols. Restricting data retention to protect user privacy limits offline retraining and explanation auditing, while routing high-confidence alerts to human counselors increases per-query serving cost and latency ($L_{\text{lat}}$). Local differential privacy bounds input exposure while strict service level agreement (SLA) budgets ($L_{\text{lat}} < 200\text{ ms}$) cap human-in-the-loop review queues.

**Systems lesson**: Responsible AI trade-offs are hard architectural requirements, not after-the-fact policy notes. The deployment pipeline must encode stakeholder choices into explicit latency ($L_{\text{lat}}$) and privacy budgets before optimization begins.
...
- **Recourse**\index{Recourse}: Individuals can alter their circumstances and receive a different outcome.
...
Architecturally, contestability requires a specialized **Contestability Stack**\index{Contestability Stack}, a design pattern analogous to distributed tracing in microservices. This stack must orchestrate four coupled components:

- **Decision Provenance**\index{Contestability Stack!decision provenance}: The system cryptographically links a specific output to the exact model binary and input vector used.
- **Explanation Generation**\index{Contestability Stack!explanation generation}: A high-latency service triggers resource-intensive interpretation methods only upon user request.
- **Appeal Routing**\index{Contestability Stack!appeal routing}: The workflow directs contested decisions to human reviewers with appropriate domain expertise.
- **Outcome Tracking**\index{Contestability Stack!outcome tracking}: The system closes the loop by recording whether the appeal overturned the machine decision.
...
\begin{tikzpicture}[
  ...
  % 4 Concentric Ellipses (drawn back to front)
  % Outermost: Government / Society boundary
  \draw[line width=1.4pt, draw=mlsysgray!80!black, fill=white] (0, -0.3) ellipse (7.5cm and 4.7cm);

  % Layer 3: Industry
  \draw[line width=1.4pt, draw=mlsysamber, fill=orange!7] (0, 0) ellipse (5.9cm and 3.85cm);

  % Layer 2: Organization
  \draw[line width=1.4pt, draw=mlsysgreen, fill=green!6] (0, 0) ellipse (4.3cm and 2.60cm);

  % Layer 1: Development Team & AI System
  \draw[line width=1.5pt, draw=mlsysblue, fill=blue!5] (0, -0.15) ellipse (2.6cm and 1.45cm);

  % Labels inside layers at top
  % Government / Society (above the gray ellipse)
  \node at (0, 4.90) {
    {\fontsize{10.5}{13}\selectfont\textbf{\textcolor{mlsysdarkgray}{Government / Society}}}\\[2pt]
    {\fontsize{8}{10}\selectfont\textcolor{mlsysgray}{regulation, law, public interest}}
  };

  % Industry (between orange top 3.85 and green top 2.60 -> center at 3.22)
  \node at (0, 3.22) {
    {\fontsize{9.5}{11.5}\selectfont\textbf{\textcolor{mlsysamber!90!black}{Industry}}}\\[2pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysamber!90!black}{standards, certifications, norms}}
  };

  % Organization (between green top 2.60 and blue top 1.30 -> center at 1.95)
  \node at (0, 1.95) {
    {\fontsize{9.5}{11.5}\selectfont\textbf{\textcolor{mlsysgreen}{Organization}}}\\[2pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgreen}{policies, audits, governance}}
  };

  % Innermost: Development Team & AI System
  \node at (0, -0.18) {
    {\fontsize{9}{11}\selectfont\textbf{\textcolor{mlsysblue}{Development Team}}}\\[1.5pt]
    {\fontsize{7}{8.5}\selectfont\textcolor{mlsysblue!85!black}{code, testing, bias checks}}\\[6pt]
    {\fontsize{9}{11}\selectfont\textbf{\textcolor{mlsysblue!90!black}{AI System}}}\\[1.5pt]
    {\fontsize{7}{8.5}\selectfont\textcolor{mlsysblue!85!black}{model + infrastructure}}
  };

  % Callout Boxes
  % 1. Right Box: Fairness Testing
  \node[draw=mlsysblue, fill=blue!4, rounded corners=4pt, line width=1.1pt,
        inner sep=6pt, anchor=west] (box_fairness) at (4.0, -0.15) {
    {\fontsize{8.5}{11}\selectfont\textbf{\textcolor{mlsysblue}{Fairness testing}}}\\[2.5pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysblue!85!black}{model cards}}\\[1.5pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysblue!85!black}{red-teaming}}
  };
  \draw[-{Stealth[scale=0.9]}, dashed, line width=1.2pt, color=mlsysblue] (2.6, -0.15) -- (box_fairness.west);

  % 2. Bottom-Right Box: Impact Assessments
  \node[draw=mlsysgreen, fill=green!5, rounded corners=4pt, line width=1.1pt,
        inner sep=6pt, anchor=north] (box_impact) at (3.4, -2.4) {
    {\fontsize{8.5}{11}\selectfont\textbf{\textcolor{mlsysgreen}{Impact assessments}}}\\[2.5pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgreen!90!black}{ethical review boards}}
  };
  \draw[dashed, line width=1.2pt, color=mlsysgreen] (3.4, -1.55) -- (box_impact.north);

  % 3. Top-Left Box: ISO Standards
  \node[draw=mlsysamber, fill=orange!5, rounded corners=4pt, line width=1.1pt,
        inner sep=6pt, anchor=east] (box_iso) at (-5.0, 2.4) {
    {\fontsize{8.5}{11}\selectfont\textbf{\textcolor{mlsysamber!90!black}{ISO standards}}}\\[2.5pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysamber!90!black}{external audits}}
  };
  \draw[-{Stealth[scale=0.9]}, dashed, line width=1.2pt, color=mlsysamber]
    (-4.0, 2.4) -- (box_iso.east);

  % 4. Bottom-Left Box: GDPR, AI Act
  \node[draw=mlsysgray!80!black, fill=white, rounded corners=4pt, line width=1.1pt,
        inner sep=6pt, anchor=north] (box_gdpr) at (-4.2, -2.4) {
    {\fontsize{8.5}{11}\selectfont\textbf{\textcolor{mlsysdarkgray}{GDPR, AI Act}}}\\[2.5pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{sector-specific law}}
  };
  \draw[-{Stealth[scale=0.9]}, dashed, line width=1.2pt, color=mlsysgray!80!black]
    (-4.2, -1.55) -- (box_gdpr.north);

  % Bottom Subtitle
  \node[anchor=north] at (0, -5.2) {
    {\fontsize{9}{12}\selectfont\textcolor{mlsysdarkgray}{Accountability extends outward: each layer enforces norms the inner layers cannot self-impose.}}
  };
...
This failed-transfer scenario is the concrete shape of the problem: a model certified fair under demographic parity on one benchmark can fail equalized odds in a new domain or jurisdiction, so the certification does not travel with the model. Closing that gap requires evaluation criteria that are measurable and auditable across domains, applied to full system pipelines rather than isolated models, and re-run as a recurring lifecycle activity so a passing result at launch does not silently expire as distributions drift. Until those practices are shared rather than ad hoc, responsible AI remains described in principles but difficult to verify in practice.
...
@Tbl-practitioner-decision-framework provides a practitioner decision framework that maps these deployment contexts to primary principles, implementation priorities, and acceptable trade-offs so the decision can remain context-sensitive rather than universal.
...
| **Archetype**              | **Minimum decision evidence**                                                                                                                          | **Required authority**                                                                                                                 |
|:---------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------|
| **A: GPT-4/Llama-3**       | Capability- and use-specific safety evaluations, harmful-output and misuse tests, uncertainty and fallback behavior, and versioned deployment evidence | Model and safety owners approve the allowed use envelope; service owners can halt or roll back when monitoring invalidates it          |
| **B: DLRM at Scale**       | Disaggregated ranking quality, exposure and error rates, feedback-loop analysis, and online effects under the serving policy                           | Product and responsible-AI owners choose the welfare and fairness criteria; operators can disable a model or policy that breaches them |
| **C: Federated MobileNet** | Per-device and subgroup quality, participation bias, privacy accounting, update integrity, and behavior across client versions                         | Device, privacy, and model owners approve participation and update policy; fleet control can suspend aggregation or revert clients     |
...
Value alignment challenges scale substantially as machine learning systems gain autonomy and capability. Established responsible AI techniques, including bias detection (@sec-responsible-ai-quantitative-fairness-measurement-a8f2), explainability (@sec-responsible-ai-explainability-interpretability-8d06), and privacy preservation (@sec-responsible-ai-privacy-preservation-91b1), provide essential capabilities but reveal fundamental limitations when systems operate with greater independence. Consider how these established methods break down in autonomous contexts.
...
Explanation systems assume an audience and an action path. An autonomous trading system might generate internally consistent SHAP attributions, but those attributions do not establish causal reasoning and cannot influence a decision if no control consumes them before the system executes thousands of trades. Without thresholds, review authority, and rollback, the explanation remains a post hoc record rather than an operational safeguard.
...
[^fn-reward-hacking-specification]: **Reward Hacking**\index{Reward Hacking!value misalignment}: When an AI system maximizes its reward function through unintended means that violate designer intent. A Tetris AI learned to pause indefinitely to avoid losing; a cleaning robot knocked over objects to create messes it could then clean up. For production ML systems, reward hacking manifests subtly: recommendation models that maximize engagement by promoting addictive content, or chatbots that maximize helpfulness ratings by being sycophantic rather than accurate. The failure mode scales with model capability.
...
\begin{tikzpicture}[
  ...
  scale=0.95,
  every node/.style={align=center},
  arr/.style={-{Stealth[scale=1.0]}, line width=1.4pt}
]

  % Top Banner: Goodhart's Law
  \node[draw=mlsysamber, fill=white, rounded corners=3pt, line width=1.1pt,
        text width=13.0cm, inner sep=6pt] (banner) at (0, 3.8) {
    {\fontsize{9.5}{11.5}\selectfont\textbf{\textcolor{mlsysamber!90!black}{Goodhart's Law}}}\\[3pt]
    {\fontsize{8}{10}\selectfont\textcolor{mlsysamber!90!black}{When a measure becomes a target, it ceases to be a good measure.}}
  };

  % 1. True Goal (Top Center)
  \node[draw=mlsysgreen, fill=green!6, rounded corners=4pt, line width=1.2pt,
        text width=4.8cm, minimum height=1.6cm, inner sep=6pt] (goal) at (0, 2.2) {
    {\fontsize{9.5}{11.5}\selectfont\textbf{\textcolor{mlsysgreen}{True Goal}}}\\[3pt]
    {\fontsize{8}{10}\selectfont\textcolor{mlsysdarkgray}{Maximize user value}}\\[2pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{long-term engagement and trust}}
  };

  % Center Box: Reinforcement Loop
  \node[draw=mlsysred!80!black, fill=red!7, rounded corners=4pt, line width=1.1pt,
        text width=4.2cm, minimum height=1.2cm, inner sep=5pt] (loop) at (0, 0) {
    {\fontsize{9}{11}\selectfont\textbf{\textcolor{mlsysred!90!black}{Reinforcement Loop}}}\\[2pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysred!90!black}{each cycle worsens misalignment}}
  };

  % 2. ML System (Right)
  \node[draw=mlsysblue, fill=blue!5, rounded corners=4pt, line width=1.2pt,
        text width=3.4cm, minimum height=1.6cm, inner sep=6pt] (mlsys) at (5.4, 0) {
    {\fontsize{9.5}{11.5}\selectfont\textbf{\textcolor{mlsysblue}{ML System}}}\\[3pt]
    {\fontsize{8}{10}\selectfont\textcolor{mlsysdarkgray}{Optimizes proxy}}\\[2pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{metric via gradient}}
  };

  % 3. Proxy Metric (Bottom Center)
  \node[draw=mlsysamber, fill=orange!7, rounded corners=4pt, line width=1.2pt,
        text width=4.8cm, minimum height=1.6cm, inner sep=6pt] (proxy) at (0, -2.4) {
    {\fontsize{9.5}{11.5}\selectfont\textbf{\textcolor{mlsysamber!90!black}{Proxy Metric}}}\\[3pt]
    {\fontsize{8}{10}\selectfont\textcolor{mlsysdarkgray}{Maximize clicks}}\\[2pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{easy to measure, easy to hack}}
  };

  % 4. Observed Behavior (Left)
  \node[draw=mlsysred, fill=red!6, rounded corners=4pt, line width=1.2pt,
        text width=4.0cm, minimum height=1.9cm, inner sep=6pt] (obs) at (-5.4, 0) {
    {\fontsize{9.5}{11.5}\selectfont\textbf{\textcolor{mlsysred}{Observed Behavior}}}\\[3pt]
    {\fontsize{8}{10}\selectfont\textcolor{mlsysdarkgray}{Clickbait content}}\\[2pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{Outrage amplification}}\\[2pt]
    {\fontsize{7.5}{9.5}\selectfont\textcolor{mlsysgray}{Filter bubbles}}
  };

  % Connecting Arrows
  % Goal -> ML System (Right, Down, Right)
  \draw[arr, color=mlsysgreen] (goal.east) -| (3.2, 0) -- (mlsys.west);
  \node[anchor=south west, text=mlsysgreen!90!black] at (3.25, 2.05) {
    {\fontsize{7.5}{9.5}\selectfont\textbf{designer sets}}\\[1pt]
    {\fontsize{7.5}{9.5}\selectfont\textbf{objective}}
  };

  % ML System -> Proxy Metric (Down, Left)
  \draw[arr, color=mlsysblue] (mlsys.south) |- (proxy.east)
    node[pos=0.72, above, text=mlsysblue]
    {\fontsize{7.5}{9.5}\selectfont\textbf{measured by}};

  % Proxy Metric -> Observed Behavior (Left, Up)
  \draw[arr, color=mlsysamber] (proxy.west) -| (obs.south)
    node[pos=0.75, left=3pt, text=mlsysamber!90!black]
    {\fontsize{8}{10}\selectfont\textbf{drives}};

  % Observed Behavior -> True Goal (Up, Right, dashed red)
  \draw[arr, dashed, color=mlsysred] (obs.north) |- (goal.west)
    node[pos=0.62, above=2pt, text=mlsysred]
    {\fontsize{7.5}{9.5}\selectfont\textbf{training data reflects the hack}};

  % Bottom Subtitle
  \node[anchor=north] at (0, -3.6) {
    {\fontsize{9}{12}\selectfont\textcolor{mlsysdarkgray}{Reward hacking optimizes the proxy, not the intended outcome.}}
  };
...
These concerns are not limited to speculative scenarios. @amodei2016concrete outline five concrete challenges for AI safety:
...
The engineering requirements for safety are shaped by a fragmented global regulatory landscape that treats AI risk as a verifiable metric. The EU AI Act\index{EU AI Act} (2024) uses a risk-based structure that includes prohibited practices, high-risk systems, transparency duties for some systems, and minimal or no-risk uses; high-risk deployments face conformity, documentation, and logging obligations [@euaiact2024]. In the United States, Executive Order 14110\index{Executive Order 14110} (2023) formerly established federal AI safety reporting thresholds, but it was revoked by Executive Order 14148\index{Executive Order 14148} in January 2025; Executive Order 14179\index{Executive Order 14179} then directed agencies to review and revise AI actions tied to the prior order, and Executive Order 14409\index{Executive Order 14409} in June 2026 shifted federal emphasis toward AI innovation, cybersecurity, and critical-infrastructure defense [@president2023ai; @president2025rescissions; @president2025aileadership; @president2026advancedai]. China's Interim Measures for Generative AI\index{Interim Measures for Generative AI} (2023) subject some public-facing services to security assessment and algorithm-filing requirements rather than a universal pre-release assessment [@china2023generativeai]. For a global ML fleet, compliance becomes a complex distributed systems problem: inference nodes in Frankfurt may require different safety configurations, data retention policies, and human-in-the-loop thresholds than those in Virginia or Singapore. This necessitates a flexible configuration control plane capable of pushing geo-specific safety policies to edge nodes without bifurcating the core model architecture.
...
ax.bar(data["Year"].astype(str), data["Cumulative_Miles_Millions"], color=COLORS["BlueLine"])
plt.title("Cumulative autonomous miles driven (proxy)", fontsize=12, pad=15)
plt.ylabel("Millions of Miles", fontsize=10)
plt.xlabel("Year", fontsize=10)
plt.xticks(rotation=45)
for i, v in enumerate(data["Cumulative_Miles_Millions"]):
...
: **Disaggregated Fairness Metrics**: A hypothetical loan approval system satisfies equalized false positive rates (`{python} FairnessGapCalc.fpr_gap_pp_attr_str` gap) but violates demographic parity (`{python} FairnessGapCalc.approval_gap_pp_attr_str` approval gap) and equal opportunity (`{python} FairnessGapCalc.tpr_gap_pp_attr_str` TPR gap). The metrics expose incompatible operational priorities, so production systems must make explicit choices about which fairness criterion to prioritize. {#tbl-responsible-ai-fairness-summary}
...
* **Fairness has no hidden optimum**: The chapter's table shows one loan system can have a `{python} FairnessGapCalc.fpr_gap_pp_attr_str` false-positive gap while still carrying `{python} FairnessGapCalc.approval_gap_pp_attr_str` approval and `{python} FairnessGapCalc.tpr_gap_pp_attr_str` true-positive gaps. Differing base rates turn fairness into an explicit normative choice, not a single metric.
...
Earlier constraints in this book could be measured and optimized against a stated objective and budget. Fairness introduces a different problem because the chapter's impossibility result offers no criterion-free optimum; it is not a gap to be closed but a fork in the road. That changes the job. The engineer can quantify each branch and hold the system to whichever is taken, but the taking is a human, normative act that no metric performs, and the most honest thing engineering can do here is refuse to disguise the choice as a calculation.

::: {.callout-chapter-connection title="From responsibility to synthesis"}

Responsible governance joins performance, security, robustness, and sustainability as a production constraint. @Sec-conclusion synthesizes these principles into a systems perspective that remains useful as specific technologies change.
...
[^fn-sociotechnical-system-etymology]: **Sociotechnical System**\index{Sociotechnical System!etymology}: Coined by the Tavistock Institute in the 1950s to describe the interdependent relationship between humans and technology in the workplace. ML fleets are the ultimate sociotechnical systems: their "performance" is not merely a benchmark score but an emergent property of how model outputs interact with user behavior, legal frameworks, and physical resource constraints.
