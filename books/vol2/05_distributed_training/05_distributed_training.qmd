...
\begin{tikzpicture}[
  ...
  \node[anchor=west, font=\tiny, text=TextMuted] at (6.0, 10.3) {Strict barrier per step; fast workers idle while straggler W3 completes};
  \node[badge, fill=NavyBlue, text=white] at (13.6, 10.3) {EXACT SGD};
  ...
  \foreach \step/\xstart in {0/2.2, 1/4.54, 2/6.88, 3/9.22, 4/11.56} {
    ...
    \node[compute block, draw=RedLine, fill=SteelBlue!15, minimum width=2.33cm, minimum height=0.45cm, anchor=west] at (\xstart, 8.35) {
      \ifnum\step=0\tiny\bfseries\textcolor{NavyBlue}{Compute (Slow Straggler)}\fi
    };
    % W4
  ...
  \node[anchor=west, font=\tiny, text=TextMuted] at (6.0, 6.8) {Bounded staleness window; workers proceed up to $s = 1$ step ahead of slowest};
  \node[badge, fill=OrangeLine, text=white] at (13.5, 6.8) {BOUNDED DRIFT};
  ...
  \node[anchor=west, font=\tiny, text=TextMuted] at (6.0, 3.3) {No barriers; lock-free gradient updates applied immediately to shared weights};
  \node[badge, fill=DarkGreen, text=white] at (13.4, 3.3) {MAX THROUGHPUT};
...
\begin{tikzpicture}[
  ...
  \node[subhead node] at (\lx, 6.95) {Weight split along columns: $W = [W_1 \mid W_2]$ (e.g., QKV projection, MLP 1)};
  ...
  % Summary card Left
  \node[summary card] (col_summary) at (\lx, -2.15) {
  ...
  \node[subhead node] at (\rx, 6.95) {Weight split along rows: $W = [W_1 ; W_2]$ (e.g., Attention Out, MLP 2)};
  ...
  % Full Output
  \node[neutral block, text width=6.8cm, minimum height=0.75cm] (row_out) at (\rx, -0.65) {
  ...
  % Summary card Right
  \node[summary card] (row_summary) at (\rx, -2.15) {
