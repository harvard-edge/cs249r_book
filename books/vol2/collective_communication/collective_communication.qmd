...
::: {#fig-bandwidth-divergence fig-env="figure" fig-pos="htb" fig-cap="**The Bandwidth Divergence**: Over the last decade, intra-node bandwidth (e.g., NVLink) has scaled significantly faster than inter-node network bandwidth (e.g., InfiniBand), widening the gap between local and global communication capabilities. This divergence forces large-scale distributed systems to adopt hierarchical collective communication strategies."}
...
fig, ax, COLORS, plt = book_style.setup_plot(figsize=(8, 4.5))
df = pd.read_csv("data/bandwidth_trends.csv")

# Intra-node
intra = df[df["Type"] == "Intra-node"]
ax.plot(intra["Year"], intra["Bandwidth_GBps"], marker='o', linestyle='-', linewidth=2.5, color=COLORS.get('OrangeLine', '#D55E00'), label='Intra-node (NVLink/PCIe)')
...
ax.plot(inter["Year"], inter["Bandwidth_GBps"], marker='s', linestyle='-', linewidth=2.5, color=COLORS.get('VioletLine', '#0072B2'), label='Inter-node (InfiniBand)')
...
The crossover formula provides the governing mental model for evaluating why libraries select specific algorithms. When a runtime chooses an unexpected path, this crossover analysis determines whether the selection reflects the physical bottleneck or whether a manual override is warranted. A concrete buffer-size calculation applies this framework to a realistic decision:
