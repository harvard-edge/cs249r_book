...
\noindent
![](images/png/cover_data_storage_blueprint_labeled_print.png){fig-alt="Fleet storage fuel-line hierarchy where data moves from object storage through NVMe, DRAM, and HBM tiers into accelerator workers."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_data_storage_blueprint_labeled.webp){fig-alt="Fleet storage fuel-line hierarchy where data moves from object storage through NVMe, DRAM, and HBM tiers into accelerator workers."}
:::
...
- Explain how ML storage workloads invert database assumptions across memory, flash, parallel file, object, and archive tiers
- Calculate required training bandwidth and multi-tier cache service times using the data pipeline equation
...
- Specify replay, idempotence, durability, and consistency contracts that preserve distributed-pipeline correctness
...
from mlsysim.fmt import fmt_int, MarkdownStr, fmt, fmt_qty, fmt_qty_int, check, fmt_memory, fmt_bandwidth
...
from mlsysim.fmt import MarkdownStr, fmt, fmt_int, check, fmt_qty, fmt_memory, fmt_bandwidth
...
class StorageFuelLineContext:
    ...
    ckpt_total = memory_from_params(gpt3_params * param, bytes_per_param_ckpt)
    checkpoint_count_val = int((training_window / checkpoint_interval).to("").magnitude)
    ckpt_fleet_total = ckpt_total * checkpoint_count_val
    ...
    bw_ratio_mult_str = fmt_multiple(bw_ratio_val, precision=1, commas=True)
...
Dense accelerator nodes can pack eight GPUs delivering petaFLOP/s of aggregate compute, and InfiniBand fabrics can connect thousands of such nodes at hundreds of gigabits per second. Within the fleet stack shown in @fig-fleet-stack, data storage completes that physical foundation by staging training data, model weights, optimizer state, and intermediate checkpoints at the right distance from the accelerator. An engine without fuel is expensive sculpture. The engineering question is how to deliver that fuel fast enough that 1,000 accelerators never starve.

::: {.column-margin}
![](images/svg/data_storage_dai_locator.svg){width="100%" fig-alt="Three side-by-side pills labeled D, A, and I. The I pill is filled blue and the D and A pills are gray, marking the Infrastructure axis as the active focus."}

*Storage is the Infrastructure axis of the fleet stack.*
:::

Consider the running example for the storage analysis. A 175-billion parameter language model trains on 1.5 trillion tokens of text: roughly `{python} StorageFuelLineContext.compressed_text_tb_str` in compressed source form, or `{python} StorageFuelLineContext.tokenized_text_tb_str` once represented as 4-byte token IDs. In this full-corpus pretraining scenario, each epoch reads every selected token once in an order determined by the shuffle seed. No small "hot" subset dominates access. Meanwhile, each accelerator processes its local batch in roughly 200 ms, then waits for the next. If storage cannot deliver data within that 200 ms window, the accelerator sits idle, and the organization pays for silicon that produces heat instead of gradients.
...
The storage problem joins physics with economics. Physics dictates that data closer to the accelerator (in both physical distance and interconnect hops) can be delivered faster but in smaller quantities. Economics dictates that cheaper storage can hold more data but at greater distance. The engineering task is to bridge these constraints, keeping the expensive top tier full by drawing from cheaper lower tiers fast enough that the accelerator never perceives the delay. The resulting design problem is quantitative. It asks how fast each tier must be, how deep the pipeline must run, and which bytes are worth keeping close to the accelerator. The imbalance is structural rather than incidental. Compute and high-bandwidth memory have advanced together while local storage has not, and @fig-storage-compute-gap shows the resulting divergence: the fuel line into the accelerator now limits the checkpoint far more tightly than the accelerator itself does.

::: {.column-margin}
![](images/svg/data_storage_checkpoint_dominance.svg){width="100%" fig-alt="A two-rung ladder on a log scale: a tall blue rung for checkpoint writes towering over a tiny rung for per-epoch training data, showing checkpoints move over a thousand times more bytes than the dataset."}

*Checkpoint writes dwarf training-data reads by about 1,000 times.*
:::

::: {#fig-storage-compute-gap fig-env="figure" fig-pos="htb" fig-cap="**The storage-compute gap**: Accelerator compute, HBM bandwidth, and NVMe bandwidth from 2010 to 2024 on a common log scale. Compute rises fastest and local storage slowest, so the fuel line into the accelerator becomes the binding constraint long before arithmetic does." fig-alt="Log-scale three-line chart from 2010 to 2024 comparing accelerator compute, HBM bandwidth, and NVMe bandwidth. Compute rises fastest and NVMe slowest, so the vertical distance between the top and bottom lines widens over time."}
...
data = pd.read_csv("vol2/04_data_storage/data/storage_compute_gap.csv")

fig, ax1 = plt.subplots(figsize=(8, 5))
ax1.set_yscale('log')

ax1.plot(data['Year'], data['Compute_TFLOPS'], marker='o', color='#E6550D', linewidth=2, label='Compute (TFLOPS)')
ax1.plot(data['Year'], data['Memory_Bandwidth_GBps'], marker='s', color='#3182BD', linewidth=2, label='HBM Bandwidth (GB/s)')
ax1.plot(data['Year'], data['Storage_Bandwidth_GBps'], marker='^', color='#756BB1', linewidth=2, label='NVMe Bandwidth (GB/s)')

ax1.set_xlabel('Year', fontsize=10, weight='bold')
ax1.set_ylabel('Performance (Log Scale)', fontsize=10, weight='bold')
ax1.set_title('The widening storage-compute gap (2010-2024)', fontsize=12, weight='bold')
ax1.grid(True, which="both", ls="--", alpha=0.3)
ax1.legend(loc='upper left')

for i, txt in enumerate(data['GPU_Architecture']):
    if data['Year'][i] in [2010, 2016, 2020, 2024]:
        ax1.annotate(txt, (data['Year'][i], data['Compute_TFLOPS'][i]), textcoords="offset points", xytext=(0, 10), ha='center', fontsize=8)
...
A database administrator moving to an ML infrastructure team would find that full-corpus pretraining workloads\index{Storage!workload inversion} invert many familiar storage design principles. The 175B running example makes the inversion concrete. Each epoch reads every selected token once, in shuffled order, and the next epoch reshuffles the same corpus. Under this scenario, no small "hot" subset dominates access. Fine-tuning, curriculum learning, hard-example mining, recommendation features, and retrieval do create skewed working sets, so the inversion is a workload property rather than a universal property of ML.

Traditional storage systems evolved to serve transactional databases, workloads characterized by small random accesses, strong consistency, and moderate bandwidth. A database server might issue thousands of 4 KB reads per second to serve user queries. The industry optimized for this pattern over decades, developing sophisticated caching algorithms, write-ahead logs, and redundant array of independent disks (RAID)[^fn-raid-array-redundancy] configurations tuned for small-block random access. Each optimization assumed that the most recently accessed data would likely be accessed again soon.

[^fn-raid-array-redundancy]: **RAID (redundant array of independent disks)**\index{RAID!ML storage}: A 1988 Berkeley taxonomy of drive-combining strategies, each trading redundancy against bandwidth [@patterson1988raid]. ML training inverts the database-era default: RAID 0 (striping, no parity) maximizes sequential read throughput at the cost of zero fault tolerance, a safe trade-off because training data is immutable and durably backed in object storage. Choosing RAID 5 or 6 instead wastes bandwidth on parity calculations that protect data already protected elsewhere.

ML workloads systematically invert these assumptions. Training data access is predominantly sequential, streaming through datasets that span hundreds of terabytes. Individual accesses are large (megabytes rather than kilobytes) because models consume batches of images or text sequences. Consistency requirements are relaxed, since slightly stale features rarely affect model quality. Bandwidth demands, however, are extreme: hundreds of gigabytes per second, sustained for days or weeks. The mismatch between what storage was optimized for and what ML actually needs creates the **I/O Wall**\index{I/O Wall!definition} (principle \ref{pri-io-wall}): when storage throughput cannot deliver training data as fast as accelerators consume it, GPUs idle regardless of their computational power. A storage system that was adequate for 8 GPUs becomes the bottleneck at 64, making the *data pipeline*, not the *model*, the limiting factor. @Sec-appdx-c3-taxonomy-bottleneck-table classifies this wall as a constraint that lives at the intersection of Communication and Compute in the fleet-scale diagnostic framework, so a storage engineer can confirm that the cure is more bandwidth rather than faster accelerators.
...
**Systems insight**: With 32 workers and 1,000 shards, the `{python} ShardContention.sc_collision_prob_str` collision probability creates a storage-shard hotspot\index{Shard Contention!collision probability} that can leave the fleet waiting for workers sharing one disk. Global shuffling and deterministic shard assignment reduce this tail latency\index{Tail Latency} by spreading workers across the storage fabric.
...
::: {#fig-storage-compute-chasm fig-env="figure" fig-pos="htb" fig-cap="**The storage-compute chasm**: GPU peak FP16 throughput (orange, left axis) and NVMe sequential read bandwidth (blue, right axis) from 2016 to 2024 appear on logarithmic scales. GPU throughput has grown 236$\times$ while storage bandwidth has grown only 4$\times$ over the same period. The shaded region highlights the widening gap that data-pipeline engineering must bridge through prefetching, caching, and format optimization." fig-alt="Dual-line log-scale plot showing GPU TFLOP/s growing from 21 to 5000 and storage bandwidth growing from 3.5 to 14 GB/s between 2016 and 2024."}
...
storage_gbs = [
    Hardware.Tech.Storage.NvmeGen3.bandwidth.to(GB / second).magnitude,
    Hardware.Tech.Storage.NvmeGen3.bandwidth.to(GB / second).magnitude,
    Hardware.Tech.Storage.NvmeGen4.bandwidth.to(GB / second).magnitude,
    Hardware.Tech.Storage.NvmeGen4.bandwidth.to(GB / second).magnitude,
    Hardware.Tech.Storage.NvmeGen5.bandwidth.to(GB / second).magnitude,
]
stor_labels = ["Gen3", "Gen3", "Gen4", "Gen4", "Gen5"]

# —Left axis: GPU TFLOP/s ---
ax1.semilogy(years_gpu, gpu_tflops, "o-", color=COLORS["OrangeLine"], markersize=8, linewidth=2, label="GPU FP16 TFLOP/s", zorder=5)
ax1.set_ylabel("GPU FP16 TFLOP/s (log scale)", color=COLORS["OrangeLine"])
ax1.tick_params(axis="y", labelcolor=COLORS["OrangeLine"])
ax1.set_ylim(5, 10000)

# —Right axis: Storage GB/s ---
ax2 = ax1.twinx()
ax2.semilogy(years_storage, storage_gbs, "s-", color=COLORS["BlueLine"], markersize=8, linewidth=2, label="NVMe Seq. Read (GB/s)", zorder=5)
ax2.set_ylabel("NVMe Sequential Read (GB/s, log scale)", color=COLORS["BlueLine"])
ax2.tick_params(axis="y", labelcolor=COLORS["BlueLine"])
ax2.set_ylim(1, 100)
ax2.spines["right"].set_visible(True)
ax2.spines["right"].set_color(COLORS["BlueLine"])
...
ax1.fill_between(years_gpu, gpu_tflops, gpu_bottom, alpha=0.10, color=COLORS["RedLine"], zorder=2)

# ┌── 3. RENDER ────────────────────────────────────────────────────────────────

# —Label GPU points ---
gpu_offsets = [(-10, -15), (-10, 10), (-15, 8), (-19, 8), (-10, 8)]  # P100  # V100  # A100  # H100  # B200

# —Label GPU points ---
for i, lbl in enumerate(gpu_labels):
    ax1.annotate(lbl, (years_gpu[i], gpu_tflops[i]), textcoords="offset points", xytext=gpu_offsets[i], fontsize=8, color=COLORS["OrangeLine"])
# —Label storage points ---
storage_offsets = [(-27, 10), (-15, -15), (-20, 10), (-22, -14), (-22, 12)]  # Gen3 2016  # Gen3 2017  # Gen4 2020  # Gen4 2022  # Gen5 2024
...
ax1.legend(lines1 + lines2, labels1 + labels2, loc="upper left", fontsize=9, frameon=True, fancybox=True, framealpha=0.9)
ax1.set_title("")
plt.show()
plt.close()
...
from mlsysim.fmt import fmt, check, fmt_qty, fmt_qty_int, fmt_int, fmt_time, fmt_memory
...
```
The second inversion is **Working Set Size**\index{Working Set Size}. Traditional applications exhibit temporal locality\index{Locality!temporal}: a web server repeatedly accesses popular pages, and a cache holding the top 10 percent of content serves 90 percent of requests. In the chapter's full-corpus pretraining scenario, the `{python} StorageAccessPatternFacts.tokenized_text_tb_str` serialized corpus has no "popular" token range; each selected token is consumed once per epoch. A cache of any practical size holds only a small fraction of the corpus, and most samples are effectively "cold" when accessed. This access pattern makes traditional recency caching much less effective. A least recently used (LRU)[^fn-lru-eviction-failure] cache can approach a 0 percent hit rate for a cyclic scan larger than the cache. With independently reshuffled epochs, cross-epoch hits are low but nonzero and depend on the cache fraction and sample order.

[^fn-lru-eviction-failure]: **LRU (Least Recently Used)**\index{LRU!ML failure mode}: LRU is effective when recently accessed data is likely to be accessed again soon; a cyclic full-dataset scan larger than the cache can evict each sample before its next access and drive the hit rate toward zero. Independent reshuffling changes the boundary behavior because some samples retained from the end of one epoch may be reused before eviction in the next, so the hit rate depends on the cache-to-dataset ratio and ordering rather than being universally zero. Teams should measure this reuse before allocating DRAM that could otherwise serve as prefetch buffers.
...
| **Locality**         |  Strong temporal locality  | Low locality in full-corpus scans |
...
\begin{tikzpicture}[
  ...
  % =============================================================
  % 0. COORDINATES & SCALES
  % =============================================================
  \def\xleft{-4.8}
  \def\xright{4.8}
  \def\hplot{6.0}

  % X-positions of 5 model sizes
  \def\xmone{-3.8} % 100M
  \def\xmtwo{-1.9} % 1B
  \def\xmthr{0.0}  % 10B
  \def\xmfou{1.9}  % 70B
  ...
  % Curve Y-coordinates:
  \def\yone{0.40} % 100M -> 10 GB/s
  \def\ytwo{1.00} % 1B   -> 25 GB/s
  \def\ythr{2.20} % 10B  -> 60 GB/s
  \def\yfou{3.60} % 70B  -> 150 GB/s
  \def\yfiv{5.00} % 175B -> 500 GB/s

  % =============================================================
  % 1. SHADED REGIONS (Compute-bound vs Storage-bottleneck)
  % =============================================================
  % Green Shaded Zone: between Lustre ceiling (y = 3.0) and curve up to intersection
  \fill[GreenFill!60]
    (\xmone, 3.0) -- (1.10, 3.0) --
    (\xmthr, \ythr) -- (\xmtwo, \ytwo) -- (\xmone, \yone) -- cycle;

  % Red Shaded Zone: above Lustre ceiling (y = 3.0) and under curve from intersection to 175B
  \fill[RedFill!70]
    (1.10, 3.0) -- (\xmfou, \yfou) -- (\xmfiv, \yfiv) -- (\xmfiv, 3.0) -- cycle;

  % =============================================================
  % 2. GRID LINES & Y-AXIS TICKS
  % =============================================================
  \foreach \y/\lbl in {
    1.0/25,
    2.0/50,
    ...
    \draw[SlateLine!20, line width=0.5pt] (\xleft, \y) -- (\xright, \y);
    \node[anchor=east, font=\scriptsize, text=TextMuted] at (\xleft - 0.15, \y) {\lbl};
  }
  \node[anchor=east, font=\scriptsize, text=TextMuted] at (\xleft - 0.15, 0) {0};

  % Axes
  \draw[->, line width=1.1pt, SlateLine] (\xleft, 0) -- (\xleft, \hplot + 0.45);
  \draw[line width=1.1pt, SlateLine] (\xleft, 0) -- (\xright, 0);
  ...
  \node[
    ...
    text=TextDark
  ] at (\xleft - 0.75, \hplot / 2) {Data Throughput ($\text{GB/s}$)};

  % =============================================================
  % 3. HORIZONTAL STORAGE TIER CEILINGS (Dashed lines + Right badges)
  % =============================================================
  % NVMe (7 GB/s -> y = 0.28 cm)
  \draw[dashed, line width=1.2pt, OrangeLine] (\xleft, 0.28) -- (\xright, 0.28);
  \node[anchor=west, align=left] at (\xright + 0.15, 0.28) {
    \textbf{\scriptsize\textcolor{OrangeLine!90!black}{NVMe SSD}}\\[1pt]
    \tiny\textcolor{TextMuted}{($7\text{ GB/s}$)}
  };

  % Object Store (50 GB/s -> y = 2.0 cm)
  \draw[dashed, line width=1.2pt, DarkGreen] (\xleft, 2.0) -- (\xright, 2.0);
  \node[anchor=west, align=left] at (\xright + 0.15, 2.0) {
    \textbf{\scriptsize\textcolor{DarkGreen}{Object Store}}\\[1pt]
    \tiny\textcolor{TextMuted}{($50\text{ GB/s}$)}
  };

  % Lustre PFS (100 GB/s -> y = 3.0 cm)
  \draw[dashed, line width=1.2pt, SteelBlue] (\xleft, 3.0) -- (\xright, 3.0);
  \node[anchor=west, align=left] at (\xright + 0.15, 3.0) {
    \textbf{\scriptsize\textcolor{NavyBlue}{Lustre PFS}}\\[1pt]
    \tiny\textcolor{TextMuted}{($100\text{ GB/s}$)}
  };

  % =============================================================
  % 4. X-AXIS TICKS & MODEL SIZE LABELS
  % =============================================================
  \foreach \x/\lbl in {
    \xmone/100M,
    \xmtwo/1B,
    ...
    \draw[line width=0.8pt, SlateLine] (\x, 0) -- (\x, -0.12);
    \node[anchor=north, font=\scriptsize\bfseries, text=TextDark] at (\x, -0.18) {\lbl};
  }

  % X-Axis Title
  \node[anchor=north, font=\scriptsize\bfseries, text=TextDark] at (0, -0.7)
    {Model Size (parameters)};

  % =============================================================
  % 5. REQUIRED I/O DEMAND CURVE (Smooth Red Curve + Points)
  % =============================================================
  \draw[line width=2.0pt, DarkRed, line join=round]
    (\xmone, \yone) -- (\xmtwo, \ytwo) -- (\xmthr, \ythr) -- (\xmfou, \yfou) -- (\xmfiv, \yfiv);
  ...
  \foreach \x/\y in {
    ...
    \fill[DarkRed] (\x, \y) circle (3.2pt);
    \fill[white] (\x, \y) circle (1.2pt);
  }

  % Label at top of curve
  \node[anchor=south east, align=right] at (\xmfiv - 0.1, \yfiv + 0.15) {
    \textbf{\scriptsize\textcolor{DarkRed}{Required I/O for}}\\[1pt]
    \textbf{\scriptsize\textcolor{DarkRed}{Full GPU Utilization}}
  };

  % =============================================================
  % 6. ANNOTATION CALLOUT BOXES (Non-colliding positions)
  % =============================================================
  % Compute Bound Box (neatly placed inside green zone below 50 GB/s)
  \node[
    rectangle,
    rounded corners=3pt,
    draw=DarkGreen,
    ...
    \textbf{\scriptsize\textcolor{DarkGreen}{Compute-Bound Zone}}\\[1.5pt]
    \tiny\textcolor{DarkGreen!80!black}{Storage bandwidth exceeds accelerator appetite}
  };

  % Storage Bottleneck Zone Box (neatly placed inside red zone above the curve)
  \node[
    rectangle,
    rounded corners=3pt,
    draw=DarkRed,
    ...
    \textbf{\scriptsize\textcolor{DarkRed}{Storage Bottleneck Zone}}\\[1.5pt]
    \tiny\textcolor{DarkRed!80!black}{I/O starving GPU compute pipelines}
  };
  ...
  % Header bar
  \path[fill=SlateFill, rounded corners=5pt]
    ($(card.north west)$) rectangle ($(card.north east) + (0, -0.42)$);
  \path[fill=SlateFill]
    ($(card.north west) + (0, -0.28)$) rectangle ($(card.north east) + (0, -0.42)$);
  \draw[CardBorder, line width=0.7pt]
    ($(card.north west) + (0, -0.42)$) -- ($(card.north east) + (0, -0.42)$);

  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at ($(card.north) + (0, -0.21)$)
    {Storage Throughput Scaling \& Bottleneck Dynamics};

  % Card Body Rows
  \node[anchor=north west, inner sep=0pt] at ($(card.north west) + (0.35, -0.56)$) {
    \begin{minipage}{12.9cm}
      \fontsize{6.8pt}{9.2pt}\selectfont
      $\bullet$ \textbf{Nonlinear I/O Demand:}\quad As model capacity scales from 100M to 175B parameters, sustaining full GPU compute requires a $50\times$ surge in throughput ($10 \to 500\text{ GB/s}$).\\[2pt]
      $\bullet$ \textbf{The Lustre Crossover:}\quad Beyond $\sim 30\text{B}$ parameters, required feeding bandwidth outstrips standard parallel filesystems ($100\text{ GB/s}$), creating severe data stalls.\\[2pt]
      $\bullet$ \textbf{Architectural Mitigation:}\quad Overcoming the storage wall demands hierarchical prefetching, GPUDirect Storage (GDS) zero-copy DMA, and sharded dataset caching.
    \end{minipage}
  };
...
class StorageHierarchyTable:
    ...
    host_dram_bw_gb_per_s_str = fmt_qty(host_dram_bw, GB / second, precision=0, commas=False)
```

| **Storage Tier**         | **Typical Capacity** |                       **Bandwidth**                        |                      **Latency**                      |     **Illustrative Cost Basis**      |
|:-------------------------|:--------------------:|:----------------------------------------------------------:|:-----------------------------------------------------:|:------------------------------------:|
| **GPU HBM**              |        80 GB         |   `{python} StorageHierarchyTable.h100_bw_tb_per_s_str`    |                        ~100 ns                        |      ~\$15/GB acquisition cost       |
| **Host DRAM**            |     512 GB--2 TB     | `{python} StorageHierarchyTable.host_dram_bw_gb_per_s_str` |                        ~100 ns                        |       ~\$3/GB acquisition cost       |
| **Local NVMe SSD**       |       4--30 TB       |                         7--25 GB/s                         | ~`{python} StorageHierarchyTable.nvme_latency_us_str` |     ~\$0.10/GB-month provisioned     |
...
class StoragePyramidContext:
    ...
    raid0_drive_count = 4  # scenario: 4-drive local NVMe RAID-0 array
...
::: {#fig-storage-pyramid fig-env="figure" fig-pos="htb" fig-cap="**ML storage hierarchy**: The hierarchy groups the canonical tiers in @tbl-storage-hierarchy-merged into hot, warm, and cold bands, from GPU HBM down to archival storage. Bandwidth generally falls as capacity grows and unit cost declines; the parallel filesystem is highlighted as the shared tier most likely to starve a large training cluster." fig-alt="Storage tiers from GPU HBM through host DRAM, local NVMe, a parallel file system, object storage, and archival storage. The parallel file system is highlighted as the shared cluster bottleneck. A left-hand ribbon groups the tiers into hot, warm, and cold bands."}
...
\begin{tikzpicture}[
  ...
  % =============================================================
  % 0. X POSITIONS & COLUMN HEADERS
  % =============================================================
  \def\xspeed{-6.9}
  \def\xribbon{-5.8}
  \def\xcard{-1.5}     % center of 7.4cm wide card -> [-5.2, 2.2]
  \def\xbwrel{1.3}     % center of relative bar area inside card
  \def\xbw{3.3}        % Bandwidth column
  \def\xcap{4.85}      % Capacity column
  \def\xcost{6.3}      % Cost basis column

  % Headers row at y = 5.85
  \node[anchor=center, font=\scriptsize\bfseries, text=TextMuted] at (\xbwrel, 5.85) {BW (relative)};
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at (\xbw, 5.85) {Bandwidth};
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at (\xcap, 5.85) {Capacity};
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at (\xcost, 5.85) {Cost basis};

  % =============================================================
  % 1. SPEED AXIS (Far Left)
  % =============================================================
  \draw[->, line width=1.1pt, SlateLine!60] (\xspeed, -0.85) -- (\xspeed, 5.1);
  \node[anchor=south, font=\scriptsize\itshape\bfseries, text=TextDark] at (\xspeed, 5.25) {Fastest};
  \node[anchor=north, font=\scriptsize\itshape\bfseries, text=TextMuted] at (\xspeed, -0.95) {Slowest};

  % =============================================================
  % 2. TIER GROUPING RIBBONS (Dashed side bars)
  % =============================================================
  % Hot Tier: spans y = 5.0, 3.8, 2.6 (center = 3.8, height = 3.35cm)
  \node[ribbon, draw=RedLine, fill=RedFill!25, minimum height=3.35cm] (r_hot) at (\xribbon, 3.8) {
    \rotatebox{90}{\textbf{\scriptsize\textcolor{DarkRed}{Hot Tier}}}
  };

  % Warm Tier: spans y = 1.4, 0.2 (center = 0.8, height = 2.15cm)
  \node[ribbon, draw=OrangeLine, fill=OrangeL!35, minimum height=2.15cm] (r_warm) at (\xribbon, 0.8) {
    \rotatebox{90}{\textbf{\scriptsize\textcolor{OrangeLine!90!black}{Warm}}}
  };

  % Cold Tier: spans y = -1.0 (center = -1.0, height = 0.95cm)
  \node[ribbon, draw=SteelBlue, fill=LightBlue!35, minimum height=0.95cm] (r_cold) at (\xribbon, -1.0) {
    \rotatebox{90}{\textbf{\scriptsize\textcolor{NavyBlue}{Cold}}}
  };

  % =============================================================
  % 3. SIX STORAGE TIER CARDS & DATA ROWS
  % =============================================================

  % --- Tier 1: GPU HBM (y = 5.0) ---
  \node[tier_card, draw=SteelBlue, fill=LightBlue!40] (c1) at (\xcard, 5.0) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=NavyBlue] at ($(c1.west) + (0.35, 0.16)$) {GPU HBM};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c1.west) + (0.35, -0.20)$) {On-chip accelerator memory (HBM3)};
  % Relative BW bar
  \fill[NavyBlue, rounded corners=1pt] (0.55, 4.88) rectangle (2.05, 5.12);
  % Metrics
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at (\xbw, 5.0) {3.35 TB/s};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcap, 5.0) {80 GB};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcost, 5.0) {$\sim$\$15};

  % --- Tier 2: CPU DRAM (y = 3.8) ---
  \node[tier_card, draw=DarkGreen, fill=GreenFill!40] (c2) at (\xcard, 3.8) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=DarkGreen] at ($(c2.west) + (0.35, 0.16)$) {CPU DRAM};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c2.west) + (0.35, -0.20)$) {Host DDR5 system memory};
  % Relative BW bar
  \fill[DarkGreen, rounded corners=1pt] (1.45, 3.68) rectangle (2.05, 3.92);
  % Metrics
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at (\xbw, 3.8) {200 GB/s};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcap, 3.8) {512 GB--2 TB};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcost, 3.8) {$\sim$\$4};

  % --- Tier 3: NVMe SSD (y = 2.6) ---
  \node[tier_card, draw=DarkGreen, fill=GreenFill!40] (c3) at (\xcard, 2.6) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=DarkGreen] at ($(c3.west) + (0.35, 0.16)$) {NVMe SSD};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c3.west) + (0.35, -0.20)$) {Local PCIe Gen5 flash storage};
  % Relative BW bar
  \fill[DarkGreen, rounded corners=1pt] (1.92, 2.48) rectangle (2.05, 2.72);
  % Metrics
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at (\xbw, 2.6) {7 GB/s};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcap, 2.6) {8 TB};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcost, 2.6) {$\sim$\$0.10};

  % --- Tier 4: Parallel File System (y = 1.4, BOTTLENECK HIGHLIGHT) ---
  \node[tier_card, draw=DarkRed, fill=RedFill!50, line width=1.0pt] (c4) at (\xcard, 1.4) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=DarkRed] at ($(c4.west) + (0.35, 0.16)$) {Parallel File System};
  \node[anchor=west, font=\tiny, text=DarkRed!80!black] at ($(c4.west) + (0.35, -0.20)$) {Shared cluster storage \textbf{(Primary cluster bottleneck)}};
  % Relative BW bar
  \fill[DarkRed, rounded corners=1pt] (1.10, 1.28) rectangle (2.05, 1.52);
  % Metrics
  \node[anchor=center, font=\scriptsize\bfseries, text=DarkRed] at (\xbw, 1.4) {1+ TB/s agg.};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcap, 1.4) {100+ PB};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcost, 1.4) {$\sim$\$0.03/mo.};

  % --- Tier 5: Object Storage (y = 0.2) ---
  \node[tier_card, draw=SteelBlue, fill=LightBlue!40] (c5) at (\xcard, 0.2) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=NavyBlue] at ($(c5.west) + (0.35, 0.16)$) {Object Storage};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c5.west) + (0.35, -0.20)$) {S3-compatible durable pool};
  % Relative BW bar
  \fill[NavyBlue, rounded corners=1pt] (1.60, 0.08) rectangle (2.05, 0.32);
  % Metrics
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at (\xbw, 0.2) {100 GB/s agg.};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcap, 0.2) {Very large};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcost, 0.2) {$\sim$\$0.02/mo.};

  % --- Tier 6: Archive / Cold Storage (y = -1.0) ---
  \node[tier_card, draw=SlateLine!70, fill=SlateFill!60] (c6) at (\xcard, -1.0) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=SlateLine!90!black] at ($(c6.west) + (0.35, 0.16)$) {Archive / Cold Storage};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c6.west) + (0.35, -0.20)$) {Offline or deep-archive media};
  % Relative BW bar
  \fill[SlateLine!80, rounded corners=0.5pt] (2.02, -1.12) rectangle (2.05, -0.88);
  % Metrics
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at (\xbw, -1.0) {1 GB/s};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcap, -1.0) {Very large};
  \node[anchor=center, font=\scriptsize, text=TextDark] at (\xcost, -1.0) {$\sim$\$0.004/mo.};

  % =============================================================
  % 4. BOTTOM EXPLANATORY CARD
  % =============================================================
  \node[
    rectangle,
    rounded corners=4pt,
    ...
    \textbf{\scriptsize\textcolor{TextDark}{Memory cost is acquisition \$/GB; storage cost is recurring \$/GB-month.}}\\[2pt]
    \tiny\textcolor{TextMuted}{Hot tier = active training data \quad$\bullet$\quad Warm = validation/checkpoints \quad$\bullet$\quad Cold = long-term archive}
  };
...
\begin{tikzpicture}[
  ...
  % Left Axis
  \draw[->, line width=1.1pt, SlateLine!60] (-7.0, -2.25) -- (-7.0, 5.05);
  \node[
    rotate=90,
    font=\scriptsize\bfseries,
    text=TextDark,
    anchor=south
  ] at (-7.0, 1.4) {Bandwidth (Log Scale)};

  % =============================================================
  % 1. WATERFALL CARDS & CLIFFS (Uniform 1.2cm step-down)
  % =============================================================

  % --- CARD 1: GPU HBM3e (y = 4.6, right = 2.4, width = 8.8cm) ---
  \node[
    rectangle, rounded corners=3pt, draw=SteelBlue, fill=LightBlue!40,
    line width=0.8pt, anchor=west, minimum height=\hcard cm, minimum width=8.8cm
  ] (c1) at (\xleft, 4.6) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=NavyBlue] at ($(c1.west) + (0.35, 0.15)$)
    {GPU HBM3e};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c1.west) + (0.35, -0.18)$)
    {3,350 GB/s $\cdot$ On-package $\cdot$ 80--141 GB capacity};
  \node[anchor=east, font=\scriptsize\bfseries, text=NavyBlue] at ($(c1.east) + (-0.35, 0)$)
    {3,350 GB/s};

  % --- CLIFF 1: HBM -> DRAM (y = 3.8) ---
  \draw[dashed, line width=1.0pt, RedLine] (\xleft, 3.8) -- (2.4, 3.8);
  \node[cliff_badge] at (-2.0, 3.8) {Cliff: 17$\times$ drop $\to$ refill window $\sim$400 ms};

  % --- CARD 2: Host DRAM (DDR5) (y = 3.0, right = 1.2, width = 7.6cm) ---
  \node[
    rectangle, rounded corners=3pt, draw=DarkGreen, fill=GreenFill!40,
    line width=0.8pt, anchor=west, minimum height=\hcard cm, minimum width=7.6cm
  ] (c2) at (\xleft, 3.0) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=DarkGreen] at ($(c2.west) + (0.35, 0.15)$)
    {Host DRAM (DDR5)};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c2.west) + (0.35, -0.18)$)
    {200 GB/s $\cdot$ PCIe Gen5-attached $\cdot$ 512 GB--2 TB};
  \node[anchor=east, font=\scriptsize\bfseries, text=DarkGreen] at ($(c2.east) + (-0.35, 0)$)
    {200 GB/s};

  % --- CLIFF 2: DRAM -> NVMe (y = 2.2) ---
  \draw[dashed, line width=1.0pt, RedLine] (\xleft, 2.2) -- (1.2, 2.2);
  \node[cliff_badge] at (-2.6, 2.2) {Cliff: 15--30$\times$ drop $\to$ refill window $\sim$3 s};

  % --- CARD 3: Local NVMe SSDs (y = 1.4, right = 0.0, width = 6.4cm) ---
  \node[
    rectangle, rounded corners=3pt, draw=DarkGreen, fill=GreenFill!40,
    line width=0.8pt, anchor=west, minimum height=\hcard cm, minimum width=6.4cm
  ] (c3) at (\xleft, 1.4) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=DarkGreen] at ($(c3.west) + (0.35, 0.15)$)
    {Local NVMe SSDs (RAID-0)};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c3.west) + (0.35, -0.18)$)
    {7--25 GB/s $\cdot$ Direct PCIe Gen5 $\cdot$ 4--30 TB};
  \node[anchor=east, font=\scriptsize\bfseries, text=DarkGreen] at ($(c3.east) + (-0.35, 0)$)
    {7--25 GB/s};

  % --- CLIFF 3: NVMe -> PFS (y = 0.6) ---
  \draw[dashed, line width=1.0pt, RedLine] (\xleft, 0.6) -- (0.0, 0.6);
  \node[cliff_badge] at (-3.2, 0.6) {Cliff: 3--10$\times$ drop $\to$ refill window $\sim$10 s};

  % --- CARD 4: Parallel File System (y = -0.2, right = -1.2, width = 5.2cm) ---
  \node[
    rectangle, rounded corners=3pt, draw=DarkGreen, fill=GreenFill!40,
    line width=0.8pt, anchor=west, minimum height=\hcard cm, minimum width=5.2cm
  ] (c4) at (\xleft, -0.2) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=DarkGreen] at ($(c4.west) + (0.35, 0.15)$)
    {Parallel File System};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c4.west) + (0.35, -0.18)$)
    {Lustre / GPFS / DAOS $\cdot$ 100+ PB};
  \node[anchor=east, font=\scriptsize\bfseries, text=DarkGreen] at ($(c4.east) + (-0.35, 0)$)
    {1--4 GB/s/node};

  % --- CLIFF 4: PFS -> Object Storage (y = -1.0) ---
  \draw[dashed, line width=1.0pt, RedLine] (\xleft, -1.0) -- (-1.2, -1.0);
  \node[cliff_badge] at (-3.8, -1.0) {Cliff: 10--100$\times$ drop $\to$ requires staging};

  % --- CARD 5: Object Storage (y = -1.8, right = -2.4, width = 4.0cm) ---
  \node[
    rectangle, rounded corners=3pt, draw=CardBorder, fill=SlateFill!50,
    line width=0.8pt, anchor=west, minimum height=\hcard cm, minimum width=4.0cm
  ] (c5) at (\xleft, -1.8) {};
  \node[anchor=west, font=\scriptsize\bfseries, text=TextDark] at ($(c5.west) + (0.35, 0.15)$)
    {Object Storage};
  \node[anchor=west, font=\tiny, text=TextMuted] at ($(c5.west) + (0.35, -0.18)$)
    {S3 / GCS $\cdot$ Scalable pool};
  \node[anchor=east, font=\scriptsize\bfseries, text=TextDark] at ($(c5.east) + (-0.35, 0)$)
    {$\sim$0.1 GB/s};


  % =============================================================
  % 2. RIGHT-SIDE CARD: REFILL WINDOWS
  % =============================================================
  \node[
    rectangle,
    rounded corners=4pt,
  ...
  % Header
  \path[fill=SlateFill, rounded corners=4pt]
    ($(rw_card.north west)$) rectangle ($(rw_card.north east) + (0, -0.42)$);
  \path[fill=SlateFill]
    ($(rw_card.north west) + (0, -0.3)$) rectangle ($(rw_card.north east) + (0, -0.42)$);
  \draw[CardBorder, line width=0.7pt]
    ($(rw_card.north west) + (0, -0.42)$) -- ($(rw_card.north east) + (0, -0.42)$);
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at ($(rw_card.north) + (0, -0.21)$)
    {Refill Windows};

  % Body Table
  \node[anchor=north west, inner sep=0pt] at ($(rw_card.north west) + (0.35, -0.58)$) {
    \fontsize{7.2pt}{11pt}\selectfont
    \renewcommand{\arraystretch}{1.2}
    \begin{tabular}{@{}ll@{}}
      \textbf{\textcolor{NavyBlue}{HBM Drain:}} & $\sim 50\text{ ms window}$ \\
      \textbf{\textcolor{DarkGreen}{DRAM Refill:}} & $\sim 400\text{ ms}$ \\
      \textbf{\textcolor{DarkGreen}{NVMe Refill:}} & $\sim 3\text{ s}$ \\
      \textbf{\textcolor{DarkGreen}{PFS Refill:}} & $\sim 10\text{ s}$ \\
    \end{tabular}
  };

  % Bottom highlight pill inside side card
  \node[
    rectangle,
    rounded corners=3pt,
    draw=RedLine,
    ...
    text=DarkRed,
    anchor=south
  ] at ($(rw_card.south) + (0, 0.22)$) {Each tier buffers tier above};


  % =============================================================
  % 3. BOTTOM PRINCIPLE CARD
  % =============================================================
  \node[
    rectangle,
    rounded corners=4pt,
    ...
    \textbf{\scriptsize\textcolor{TextDark}{Architectural Principle: Continuous Prefetching}}\\[2pt]
    \tiny\textcolor{TextMuted}{Each lower storage tier must prefetch deep enough to absorb the draining rate and latency jitter of the tier below.}
  };
...
The same hierarchy can be analyzed as a cache\index{Cache!effective service time} rather than as six isolated devices. If a request hits the hot tier with probability $h$, taking $T_{\text{hot}}$, and otherwise falls through to a colder tier, taking total end-to-end miss-path time $T_{\text{cold}}$ (including the failed hot-tier lookup), its mean service time is

$$
E[T] = hT_{\text{hot}} + (1-h)T_{\text{cold}}.
$$ {#eq-storage-cache-service-time}

For fixed-size records, the corresponding effective bandwidth is the record size divided by $E[T]$; an accelerator stalls whenever that service time exceeds the compute time not already hidden by prefetching. The arithmetic makes small miss rates consequential: with a 95 percent hit rate and a cold-tier access 100$\times$ slower than a hot-tier access, $E[T]=5.95T_{\text{hot}}$, so the effective request rate is only about one-sixth of the all-hot rate. This mean is a sizing baseline, not a latency guarantee. Correlated misses or a heavy-tailed cold tier can drain the prefetch buffer even when the average satisfies the pipeline budget, so production designs must check a tail percentile and miss bursts as well as $E[T]$.

The batch-volume arithmetic also explains why increasing cluster size creates storage pressure. A single node with 8 GPUs needs roughly 4 to 40 GB/s of storage bandwidth under the workload assumptions developed in @sec-storage-pipeline-equation. A cluster of 256 such nodes needs 1,000 to 10,000 GB/s. A cluster of 10,000 nodes needs 40 to 400 TB/s. At the upper end, even a world-class parallel file system with 1,000 object storage servers (the data-serving nodes introduced in @sec-storage-tier3-pfs) delivering 1 TB/s aggregate cannot satisfy the demand, and the architecture must rely on local NVMe caching to reduce the load on shared storage. The severity of that cluster-level pressure depends sharply on the input-batch volume and iteration time for each data modality.
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import MarkdownStr, fmt, fmt_count, check, fmt_time, fmt_bandwidth, fmt_memory

class TextImageBandwidth:
    # ┌── 1. LOAD ─────────────────────────────────────────
    n_gpus = 2048
    text_tokens_per_gpu = 4096
    token_bytes = BYTES_INT32
    image_batch_per_gpu = 256
    image_bytes = 150_000 * byte  # scenario image size assumption
    step = 0.2 * second  # scenario step time assumption

    # ┌── 2. EXECUTE ──────────────────────────────────────
    text_bw = n_gpus * text_tokens_per_gpu * token_bytes / step
    image_bw = n_gpus * image_batch_per_gpu * image_bytes / step
    ...
    check(393 < image_bw_gb_s_val < 394, "Image bandwidth should be about 393 GB/s")
    ...
    image_size_kb_str = fmt_memory(image_bytes, unit=KB, precision=0, commas=False)
...
::: {.column-margin}
![](images/svg/vol2_data_storage_margin_001.svg){width="100%" fig-alt="Text training bandwidth vs. image training bandwidth."}
...
For image training\index{Training!image}, the picture changes dramatically. Using a common batch size of `{python} TextImageBandwidth.image_batch_per_gpu_str` per GPU (ImageNet at $224{\times}224$, roughly `{python} TextImageBandwidth.image_size_kb_str`/image), the aggregate bandwidth explodes:

`{python} TextImageBandwidth.image_bw_display_math`

**Systems insight**: Image training requires `{python} TextImageBandwidth.ratio_rounded_mult_str` higher bandwidth than text training and demands a high-performance parallel file system. This fundamental difference drives hierarchy design: text training is *volume-heavy* but *bandwidth-light*, bottlenecked by total dataset size and checkpointing; image training is *bandwidth-heavy*, bottlenecked by the storage system's ability to feed the accelerators.
...
class HbmTierBandwidth:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_bw_tb_per_s_str = fmt_qty(h100_bw, TB / second, precision=2, commas=False)
    host_dram_bw_gb_per_s_str = fmt_qty(host_dram_bw, GB / second, precision=0, commas=False)
...
from mlsysim.fmt import fmt, fmt_math, check, fmt_memory
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import MarkdownStr, check, fmt, fmt_percent_range, fmt_qty_range, fmt_memory, fmt_memory_capacity
...
class HbmMemoryBudget:
    ...
    total_min = weight_per_gpu + optimizer_per_gpu + activation_min + gradient_per_gpu + comm_min
    total_max = weight_per_gpu + optimizer_per_gpu + activation_max + gradient_per_gpu + comm_max
    ...
    check(abs(total_min_pct - 65.9) < 0.2, f"Expected ~65.9% min occupancy, got {total_min_pct}")
...
Even with aggressive partitioning, the total memory footprint reaches `{python} HbmMemoryBudget.total_range_str` on a decimal gigabyte basis, about `{python} HbmMemoryBudget.total_pct_range_str` of the `{python} HbmMemoryBudget.hbm_capacity_gb_str` HBM pool when measured against its binary capacity. This leaves a limited buffer for the incoming data pipeline, especially once fragmentation and framework workspaces are included. Any delay in fetching the next batch from host memory risks starving the accelerator, forcing it to sit idle while the most expensive resource in the system produces heat instead of gradients.
...
4. **Collate**: The collation stage assembles individual samples into batches and places them in **Pinned Memory**\index{Pinned Memory!DMA transfer} for efficient direct memory access (DMA) transfer to the accelerator.
...
Some augmentation pipelines have moved from CPU to GPU execution, using libraries like NVIDIA Data Loading Library\index{NVIDIA Data Loading Library} to perform image decoding and augmentation on the accelerator itself. This approach eliminates the CPU augmentation bottleneck and reduces the host DRAM bandwidth demand, because compressed data (smaller) is transferred to the GPU instead of decoded data (larger). The trade-off is that augmentation on the GPU consumes HBM capacity and compute cycles that would otherwise be available for training. For *compute-bound* workloads (where the GPU is already saturated with matrix multiplications), GPU-based augmentation is counterproductive. For *I/O-bound* workloads (where the GPU waits for data), it can improve overall throughput by shifting work from the bottleneck (CPU) to the resource with spare capacity (GPU).
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_count, fmt_rate, fmt_memory, fmt_bandwidth

class NVMeTierCalcs:
    # ┌── 1. LOAD ─────────────────────────────────────────
    images_per_sec = 1000
    img_size = 150 * KB  # scenario ImageNet average image-size assumption
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    raw_bw_mb_per_s_str = fmt_bandwidth(raw_bw, unit=MB / second, precision=0, commas=False)
    images_per_s_str = fmt_rate(images_per_sec, 'images/s', precision=0, commas=False)
    img_size_kb_str = fmt_memory(img_size, unit=KB, precision=0, commas=False)
    imagenet_images_m_str = fmt_count(imagenet_images, scale='M', precision=2, commas=False)
    imagenet_avg_kb_str = fmt_memory(img_size, unit=KB, precision=0, commas=False)
...
from mlsysim import Hardware
from mlsysim.fmt import fmt, check, fmt_time, fmt_memory, fmt_bandwidth, fmt_multiple

class NvmeTierContext:
    LOCAL_NVME_DRIVES_PER_NODE_REF = 4 * count  # pedagogical: NVMe drives per node
    training_tokens = ReferenceStats.StorageTrainingCorpus.TrainingTokens
    token_id_bytes = ReferenceStats.StorageTrainingCorpus.TokenIdBytes
    tokenized_shard = 3 * GB  # scenario tokenized shard size assumption
    nvme_per_drive_bw = Hardware.Tech.Storage.NvmeGen4.bandwidth
    nvme_drive_capacity = 7.68 * TB  # scenario NVMe drive capacity assumption
    raid0_bw = LOCAL_NVME_DRIVES_PER_NODE_REF.magnitude * nvme_per_drive_bw
    hdd_bw = 100 * (MB / second)
    zstd1_eff_bw = raid0_bw * 3
    zstd9_eff_bw = raid0_bw * 4
    tokenized_text = training_tokens * token_id_bytes
    tokenized_shard_count_val = round((tokenized_text / tokenized_shard).to("").magnitude)
    ...
    multimodal_images = 20 * TB  # scenario multimodal image corpus assumption
    multimodal_text = 10 * TB  # scenario multimodal text corpus assumption
    multimodal_audio = 5 * TB  # scenario multimodal audio corpus assumption
    multimodal_total = multimodal_images + multimodal_text + multimodal_audio
...
**Problem**: A ResNet-50 training job on ImageNet\index{ImageNet!bottleneck analysis} (`{python} NVMeTierCalcs.imagenet_images_m_str`, ~`{python} NVMeTierCalcs.imagenet_avg_kb_str` average) targets `{python} NVMeTierCalcs.images_per_s_str`. The question is whether to use individual JPEG files on an HDD or NVMe.
...
**Systems insight**: The pipeline must either use NVMe (~`{python} ImageNetNvmeLatency.nvme_latency_us_str` random access, three orders of magnitude faster than object storage) or convert the dataset to a sequential format (TFRecord/WebDataset) to achieve sequential throughput.

:::

The central challenge at this tier is the I/O wall: a single NVMe drive is roughly 500$\times$ slower than HBM, and even a four-drive RAID-0 stripe remains roughly 120$\times$ slower. Bridging this gap requires pipelining (overlap I/O with compute, detailed in @sec-storage-pipeline-equation) and, increasingly, GPUDirect Storage (detailed in @sec-storage-gds) to bypass CPU overhead entirely. The I/O wall at this tier is particularly insidious because NVMe performance is excellent by historical standards. A storage engineer accustomed to HDD-era throughput of `{python} NvmeTierContext.hdd_bw_mb_per_s_str` might view `{python} NvmeTierContext.raid0_bw_gb_per_s_str` from a local RAID-0 stripe as superabundant. Relative to the accelerator's appetite, however, `{python} NvmeTierContext.raid0_bw_gb_per_s_str` is a trickle. The only way to bridge the gap is to overlap storage reads with computation so thoroughly that the accelerator never perceives the storage delay.

Local NVMe is also the primary tier for **Local Checkpoint Staging**\index{Checkpointing!local checkpoint staging}. When saving a model checkpoint, the fastest strategy is to write to local NVMe at full bandwidth (minimizing the time the training pipeline is paused), then asynchronously replicate to shared storage for durability, since a per-node shard written directly to a contended parallel file system takes several times longer than the same shard written to local drives. The optimal checkpoint frequency depends on cluster failure rates and checkpoint write time, but the storage-side design goal is already clear: minimize $T_{\text{write}}$ through tiered staging by writing to local NVMe at full bandwidth, then background-copying to shared storage. @Sec-storage-checkpointing works the timings through in full.
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_time, fmt_count, fmt_usd
...
The architecture of a Lustre-style parallel file system separates two concerns that traditional file systems handle together (@fig-storage-pfs-architecture) [@schwan2003lustre]. **Metadata Servers (MDS)**\index{Metadata Server}\index{MDS} manage the namespace: file creation, directory listings, permission checks, and lock management. **Object Storage Servers (OSS)**\index{Object Storage Server}\index{OSS}[^fn-oss-pfs-disambiguation] manage the actual data blocks, each serving a stripe of every large file. When a client opens a 10 GB training shard, the MDS tells the client which OSS nodes hold which stripes, and the client reads from all of them in parallel. A Lustre[^fn-lustre-parallel-origin] deployment with 100 OSS nodes, each providing 10 GB/s, delivers an aggregate 1 TB/s.

::: {#fig-storage-pfs-architecture fig-env="figure" fig-pos="htb" fig-cap="**Parallel file system architecture**: Separation of metadata operations (MDS $\\to$ MDT) from high-throughput parallel data streaming (Clients $\\to$ OSS $\\to$ OST), illustrating how file striping aggregates cluster storage bandwidth." fig-alt="Architecture diagram showing compute clients querying a metadata server for stripe maps, then issuing parallel reads across multiple object storage servers."}
...
\begin{tikzpicture}[
  ...
  \draw[dashed, line width=0.6pt, SlateLine!30] (-2.3, -1.8) -- (-2.3, 5.2);
  \draw[dashed, line width=0.6pt, SlateLine!30] (2.3, -1.8) -- (2.3, 5.2);

  % Column Headers
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at (\xcli, 5.2)
    {Compute Clients ($C_i$)};
  \node[anchor=center, font=\scriptsize\bfseries, text=TextDark] at (\xsrv, 5.2)
    {Storage / Metadata Nodes};
  \node[anchor=center, font=\scriptsize\bfseries, text=DarkGreen] at (\xtgt, 5.2)
  ...
  \node[mds_box] (mds) at (\xsrv, \ymeta) {
    \textbf{\scriptsize\textcolor{PurpleLine}{MDS}}\\[1pt]
    \tiny\textcolor{TextDark}{Metadata Server}
  };

  \node[mdt_target] (mdt) at (\xtgt, \ymeta) {
    \textbf{\scriptsize\textcolor{PurpleLine}{MDT}}
  };

  \draw[->, line width=1.0pt, PurpleLine]
    (mds.east) -- node[above, font=\tiny, text=PurpleLine] {namespace ops} (mdt.west);


  % =============================================================
  % 2. ROWS 1-3: CLIENTS, OSS, OST (Data Path)
  % =============================================================

  % --- Row 1 (y = 2.8) ---
  \def\yrone{2.8}
  \node[client_box] (cl1) at (\xcli, \yrone) {
    \textbf{\scriptsize\textcolor{NavyBlue}{Client 1}}\\[1pt]
    \tiny\textcolor{TextMuted}{GPU Trainer Node}
  };

  \node[server_box] (oss1) at (\xsrv, \yrone) {
    \textbf{\scriptsize\textcolor{DarkGreen}{OSS-1}}\\[1pt]
    \tiny\textcolor{TextDark}{Object Storage Server}
  };

  \node[disk_target] (ost1) at (\xtgt, \yrone) {
    \textbf{\scriptsize\textcolor{DarkGreen}{OST-1}}
  };

  % Metadata request from Client 1 to MDS
  \draw[->, dashed, line width=1.0pt, PurpleLine]
    ($(cl1.north) + (0.4, 0)$) -- ++(0, 1.0) --
    node[above, font=\tiny\bfseries, text=PurpleLine] {1. open() $\to$ stripe map} (mds.west);

  % Data flow: Client 1 -> OSS-1 -> OST-1
  \draw[->, line width=1.1pt, DarkGreen]
    (cl1.east) -- node[above, font=\tiny\bfseries, text=DarkGreen] {2. parallel reads} (oss1.west);
  \draw[->, line width=1.1pt, DarkGreen]
    (oss1.east) -- (ost1.west);
  ...
  \node[client_box] (cl2) at (\xcli, \yrtwo) {
    \textbf{\scriptsize\textcolor{NavyBlue}{Client 2}}\\[1pt]
    \tiny\textcolor{TextMuted}{GPU Trainer Node}
  };

  \node[server_box] (oss2) at (\xsrv, \yrtwo) {
    \textbf{\scriptsize\textcolor{DarkGreen}{OSS-2}}\\[1pt]
    \tiny\textcolor{TextDark}{Object Storage Server}
  };

  \node[disk_target] (ost2) at (\xtgt, \yrtwo) {
    \textbf{\scriptsize\textcolor{DarkGreen}{OST-2}}
  };

  \draw[->, line width=1.1pt, DarkGreen] (cl2.east) -- (oss2.west);
  \draw[->, line width=1.1pt, DarkGreen] (oss2.east) -- (ost2.west);


  % --- Row 3 (y = 0.4) ---
  \def\yrthr{0.4}
  \node[client_box] (cl3) at (\xcli, \yrthr) {
    \textbf{\scriptsize\textcolor{NavyBlue}{Client 3}}\\[1pt]
    \tiny\textcolor{TextMuted}{GPU Trainer Node}
  };

  \node[server_box] (oss3) at (\xsrv, \yrthr) {
    \textbf{\scriptsize\textcolor{DarkGreen}{OSS-3}}\\[1pt]
    \tiny\textcolor{TextDark}{Object Storage Server}
  };

  \node[disk_target] (ost3) at (\xtgt, \yrthr) {
    \textbf{\scriptsize\textcolor{DarkGreen}{OST-3}}
  };

  \draw[->, line width=1.1pt, DarkGreen] (cl3.east) -- (oss3.west);
  \draw[->, line width=1.1pt, DarkGreen] (oss3.east) -- (ost3.west);


  % --- Ellipsis Row (y = -0.4) ---
  \def\yrell{-0.4}
  \node[font=\bfseries, text=SlateLine] at (\xcli, \yrell) {$\vdots$};
  \node[font=\bfseries, text=SlateLine] at (\xsrv, \yrell) {$\vdots$};
  \node[font=\bfseries, text=SlateLine] at (\xtgt, \yrell) {$\vdots$};


  % --- Row N (y = -1.2) ---
  \def\yrfou{-1.2}
  \node[client_box] (cln) at (\xcli, \yrfou) {
    \textbf{\scriptsize\textcolor{NavyBlue}{Client N}}\\[1pt]
    \tiny\textcolor{TextMuted}{GPU Trainer Node}
  };

  \node[server_box] (ossn) at (\xsrv, \yrfou) {
    \textbf{\scriptsize\textcolor{DarkGreen}{OSS-N}}\\[1pt]
    \tiny\textcolor{TextDark}{Object Storage Server}
  };

  \node[disk_target] (ostn) at (\xtgt, \yrfou) {
    \textbf{\scriptsize\textcolor{DarkGreen}{OST-N}}
  };

  \draw[->, line width=1.1pt, DarkGreen] (cln.east) -- (ossn.west);
  \draw[->, line width=1.1pt, DarkGreen] (ossn.east) -- (ostn.west);


  % =============================================================
  % 3. BOTTOM CARDS: LEGEND & FILE STRIPING
  % =============================================================
  \def\ybot{-2.2}
  ...
  \node[anchor=west, font=\tiny\bfseries, text=PurpleLine]
    at ($(leg_card.west) + (1.1, 0.20)$) {Metadata Path (MDS / MDT)};

  \draw[line width=1.1pt, DarkGreen]
    ($(leg_card.west) + (0.35, -0.20)$) -- ($(leg_card.west) + (0.95, -0.20)$);
  \node[anchor=west, font=\tiny\bfseries, text=DarkGreen]
    at ($(leg_card.west) + (1.1, -0.20)$) {Data Path (OSS / OST, Parallel I/O)};
  ...
  \node[
    ...
    \textbf{\scriptsize\textcolor{TextDark}{File Striping \& Bandwidth Aggregation}}\\[1.5pt]
    \tiny\textcolor{TextMuted}{4 GB shard striped across 100 OSTs $\times$ 40 MB stripes $\to$ 1 TB/s cluster I/O}
  };
...
1.  **Significance**: `open`, `stat`, and `close` requests from 10,000 workers can serialize at the metadata service, idle the cluster, and reduce effective I/O bandwidth $(\text{BW}_{\text{io}})$.
2.  **Distinction**: This is a metadata request-rate and latency $(L_{\text{lat}})$ problem, not a bulk bit-rate problem.
3.  **Common pitfall**: Faster SSDs do not fix this format problem\index{Format Problem}. Packing samples into sequential containers such as TFRecord\index{TFRecord} or Parquet amortizes metadata operations across many samples.
...
Aggregation complements striping by reducing the number of files that the MDS must track. Small samples are bundled into large sequential files (such as WebDataset[^fn-webdataset-tar-streaming] tar archives or TFRecord sequences) to amortize metadata costs across thousands of samples. A single `open()` on a 4 GB tar file gives access to 40,000 samples, reducing metadata load by 40,000$\times$ compared to individual files.
...
- [ ] **Metadata cost**: calculate the time required to open 200 million 150 KB image files sequentially when each `open()` call takes 50 $\mu$s on the MDS, then state the practical implication.
...
At the scale of petabytes, object storage\index{Object Storage!erasure coding} provides the durable, cost-effective foundation of the hierarchy. Services like Amazon S3 and Google Cloud Storage organize data in object namespaces where objects are retrieved by keys rather than by POSIX directory paths [@awsS3ObjectKeys; @googleCloudStorageObjects]. Unlike file systems, which organize data in hierarchical directories with metadata-rich operations (rename, link, permission inheritance), object storage treats each object as an opaque blob identified by a unique key. This simplification eliminates the metadata complexity that plagues parallel file systems at scale and enables horizontal scaling to very large capacity pools.

Object storage targets "eleven nines"[^fn-durability-nines-availability] of durability (99.999999999 percent) through erasure coding[^fn-reedsolomon-erasure-coding] and geographic replication. That target makes object storage the natural source-of-truth tier for training data and checkpoint archives, while still requiring versioning, lifecycle policy, access control, and recovery testing for operational failures outside the storage service's durability model.
...
[^fn-reedsolomon-erasure-coding]: **Erasure Coding**\index{Erasure Coding!definition}: Built on Reed-Solomon codes\index{Erasure Coding!Reed-Solomon} [@reed1960], which use maximum-distance-separable coding to recover the original data from any sufficient subset of encoded fragments. In storage systems, Reed-Solomon-style codes and later local-reconstruction variants trade storage overhead against repair bandwidth and degraded-read cost [@plank1997; @huang2012erasure]. For ML storage, the trade-off is concrete: a 4+2 code tolerates two missing fragments at 1.5$\times$ storage overhead, while triple replication costs 3$\times$; degraded reads then pay extra network I/O and reconstruction work when fragments are unavailable.

[^fn-durability-nines-availability]: **Eleven Nines (99.999999999 Percent Durability)**\index{Durability!eleven nines}: A design target of 11 nines annual durability corresponds to an expected loss probability of about $10^{-11}$ per object-year; for one million objects, that is about one expected object loss per 100,000 years. This extreme durability is why object storage is the only tier where training data and checkpoint archives can be treated as permanent. The same service indirection, erasure coding, replication, and metadata lookup that support durability also make object storage a higher-latency tier than local NVMe or a warm parallel file system, so real-time data pipelines need prefetch buffering.

::: {#dfn-data-storage-erasure-coding .callout-definition title="Erasure coding"}

***Erasure Coding***\index{Erasure Coding!definition} is a storage protection scheme for ML datasets and checkpoint archives that fragments an object into $k$ data and $m$ parity blocks, ensuring the original is recoverable from any $k$ remaining fragments.
...
class ObjectStorageCost:
    # ┌── 1. LOAD ─────────────────────────────────────────
    dataset_size_tb = 100
    cost_s3_gb_mo = Infrastructure.Pricing.Storage.S3StandardLowPerTbMonth.rate.to(USD / (GB * ureg.month)).magnitude
    cost_nvme_gb_mo = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.to(USD / (GB * ureg.month)).magnitude
...
The cost advantage of object storage is significant: at a representative \$0.02/GB/month, it is 750$\times$ cheaper per byte than HBM and 5$\times$ cheaper than local NVMe. For a 100 TB training dataset, object storage costs roughly `{python} ObjectStorageCost.s3_annual_usd_str` per year under that price assumption, compared to `{python} ObjectStorageCost.nvme_annual_usd_str` for local NVMe. This cost advantage makes object storage a common home for the organization's training data lake, where raw data, preprocessed datasets, and archived model artifacts can reside durably.
...
from mlsysim.fmt import fmt, fmt_memory, fmt_bandwidth

class ObjectStreamContext:
    training_tokens = ReferenceStats.StorageTrainingCorpus.TrainingTokens
    token_id_bytes = ReferenceStats.StorageTrainingCorpus.TokenIdBytes
    tokenized_shard = 3 * GB  # scenario tokenized shard size assumption
    step = 0.2 * second  # scenario text-training step time assumption
    tokenized_text = training_tokens * token_id_bytes
    tokenized_shard_count_val = round((tokenized_text / tokenized_shard).to("").magnitude)
    text_bw = 2048 * 4096 * token_id_bytes / step
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    text_bw_mb_per_s_str = fmt_bandwidth(text_bw, unit=MB / second, precision=1, commas=False)
    tokenized_shard_count_str = fmt(tokenized_shard_count_val, precision=0)
    tokenized_shard_gb_str = fmt_memory(tokenized_shard, unit=GB, precision=0, commas=False)
```
Applying this streaming architecture to the running example makes the design concrete. The tokenized training set is stored in an S3-like object store as roughly `{python} ObjectStreamContext.tokenized_shard_count_str` shards of `{python} ObjectStreamContext.tokenized_shard_gb_str` each. The streaming architecture assigns each of the 256 compute nodes a unique subset of roughly 8 shards. Within each node, 8 data loader worker processes issue concurrent HTTP `Range` requests to fetch 256 MB chunks from their assigned shards. Across the cluster, this results in 2,048 concurrent requests, allowing aggregate throughput in the 50--100 GB/s range when the object store and network path are provisioned for it, easily saturating the roughly `{python} ObjectStreamContext.text_bw_mb_per_s_str` required for text model training. In this regime, the bottleneck often shifts from the object store's bandwidth to the network bandwidth between the compute VPC and the object-storage endpoint. Organizations that co-locate compute and storage in the same cloud availability zone can reduce cross-network latency, directly reducing the required prefetch buffer depth and improving overall pipeline efficiency.
...
class GlacierCost:
    # ┌── 1. LOAD ─────────────────────────────────────────
    dataset_size_tb = 100
    cost_glacier_gb_mo = Infrastructure.Pricing.Storage.GlacierStandardPerTbMonth.rate.to(USD / (GB * ureg.month)).magnitude

    # ┌── 2. EXECUTE ──────────────────────────────────────
    dataset_size = dataset_size_tb * THOUSAND * GB
    glacier_annual_cost = dataset_size * (cost_glacier_gb_mo * USD / (GB * ureg.month)) * 12 * ureg.month
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import MarkdownStr, check, fmt, fmt_time, fmt_bandwidth
import math
...
class PipelineCalcs:
    ...
    batch_img_gpu = 256
    util_target = 0.80
    t_step = 200 * ms  # scenario step time assumption
    t_comp = 200 * ms  # scenario compute time assumption
    t_io = 250 * ms  # scenario I/O time assumption

    # ┌── 2. EXECUTE ──────────────────────────────────────
    req_bw_imagenet = n_gpus_image * util_target * batch_img_gpu * img_size / t_step
    req_bw_imagenet_gbs_val = req_bw_imagenet.to(GB / second).magnitude
    stall_max_t = max(t_comp, t_io)
    data_stall_pct_val = ((stall_max_t - t_comp) / stall_max_t).to("").magnitude * 100
    t_step_seq = t_io + t_comp  # no overlap: I/O then compute
    stall_seq_val = (t_io / t_step_seq).to("").magnitude * 100

    # ┌── 3. GUARD ────────────────────────────────────────
    check(req_bw_imagenet_gbs_val > 10, f"ImageNet aggregate BW should be high, got {req_bw_imagenet_gbs_val:.1f} GB/s")
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    req_bw_imagenet_gb_per_s_str = fmt_bandwidth(req_bw_imagenet, unit=GB / second, precision=1, commas=False)
    ...
    tstep_seq_display_math = fmt_display_math(
        f"T_{{\\text{{step}}}} = T_{{\\text{{I/O}}}} + T_{{\\text{{compute}}}} = " f"{_io_ms:.0f} + {_comp_ms:.0f} = {_seq_ms:.0f} \\text{{ ms}}"
    )
...
$$\text{BW}_{\text{required}} = N_{\text{GPU}} \times \eta_{\text{target}} \times \frac{D_{\text{vol,batch}}}{T_{\text{iteration}}}$$ {#eq-pipeline-throughput}
...
The demand equation sizes the accelerator-facing endpoint, but bytes must traverse the complete service chain. Let $r_k$ denote the measured service rate of stage $k$, including object-store reads, network transfer, decoding and augmentation, host-memory copies, and host-to-device transfer. With bounded buffers and no sustained backlog, the pipeline can deliver at most

$$r_{\text{pipeline}} = \min_k r_k, \qquad r_{\text{pipeline}} \geq \text{BW}_{\text{required}}.$$ {#eq-storage-service-chain}
...
As a separate image-training example, consider 256 GPUs training with ImageNet-scale images, 200 ms iterations, and 80 percent target utilization. This configuration requires `{python} PipelineCalcs.req_bw_imagenet_gb_per_s_str` of aggregate storage throughput. The bandwidth must be sustained continuously for the duration of training, which may last days or weeks. If at any point the storage system delivers less than `{python} PipelineCalcs.req_bw_imagenet_gb_per_s_str`, accelerators begin to idle. For the chapter's 175B language-model running example, tokenized text has a much lower per-batch bandwidth demand, but the total data volume and checkpoint traffic over the run are much larger, shifting the bottleneck from peak training-data bandwidth to sustained throughput and checkpoint movement over weeks.
...
Without overlap, step time adds compute and I/O time. Pipelining reduces step time to the longer of the two durations and stall time to the positive difference between I/O and compute.
...
plt.text(threshold * 1.1, 10, 'Stall Threshold\n(~10 GB/s)', color='#444444', fontsize=10, verticalalignment='bottom')

# —Zones ---
plt.fill_between(x, 0, 100, where=(x < threshold), color='#ffcccc', alpha=0.3, label='Data Stall Zone')
plt.text(0.2, 65, 'Data Stall Zone', color='#cc0000', fontsize=12, fontweight='bold', alpha=0.7)
...
points = [
    ('HDD', 0.5),
    ('Network FS', 2.0),
    ('NVMe', Hardware.Tech.Storage.NvmeGen4.bandwidth.to(GB / second).magnitude),
    ('NVMe RAID', 25.0),
]
...
plt.xticks([0.1, 0.5, 1, 2, 5, 10, 25, 50, 100], ['0.1', '0.5', '1', '2', '5', '10', '25', '50', '100'])
...
The primary weapon against data stalls is **Pipelining**\index{Pipelining!data loading}: the CPU prepares batch $i+1$ while the GPU processes batch $i$, overlapping storage transfer with accelerator execution (@fig-data-pipeline-vol2). When I/O time is less than compute time, pipelining hides the storage latency entirely, and the accelerator never stalls.
...
\begin{tikzpicture}[
  ...
  \def\wstg{1.8} % width of one stage

  % =============================================================
  % PART A: WITHOUT PREFETCHING (SEQUENTIAL)
  % =============================================================
  \node[anchor=center, font=\small\bfseries, text=TextDark] at (5.4, 6.2)
    {Without Prefetching (Sequential)};

  % Track Labels
  \node[anchor=east, font=\scriptsize\bfseries, text=TextDark] at (-0.35, 5.2) {Storage};
  \node[anchor=east, font=\scriptsize\bfseries, text=TextDark] at (-0.35, 4.3) {CPU};
  \node[anchor=east, font=\scriptsize\bfseries, text=TextDark] at (-0.35, 3.4) {GPU};

  % Storage Track (y = 5.2)
  \node[read_box, minimum width=\wstg cm, anchor=west] at (0.0, 5.2) {Read 1};
  \node[idle_box, minimum width=2*\wstg cm, anchor=west] at (\wstg, 5.2) {idle};
  \node[read_box, minimum width=\wstg cm, anchor=west] at (3*\wstg, 5.2) {Read 2};
  \node[idle_box, minimum width=2*\wstg cm, anchor=west] at (4*\wstg, 5.2) {idle};

  % CPU Track (y = 4.3)
  \node[idle_box, minimum width=\wstg cm, anchor=west] at (0.0, 4.3) {};
  \node[prep_box, minimum width=\wstg cm, anchor=west] at (\wstg, 4.3) {Prep 1};
  \node[idle_box, minimum width=2*\wstg cm, anchor=west] at (2*\wstg, 4.3) {};
  \node[prep_box, minimum width=\wstg cm, anchor=west] at (4*\wstg, 4.3) {Prep 2};
  \node[idle_box, minimum width=\wstg cm, anchor=west] at (5*\wstg, 4.3) {};

  % GPU Track (y = 3.4)
  \node[idle_box, minimum width=2*\wstg cm, anchor=west] at (0.0, 3.4) {\textcolor{DarkRed!80}{GPU idle}};
  \node[train_box, minimum width=\wstg cm, anchor=west] at (2*\wstg, 3.4) {Train 1};
  \node[idle_box, minimum width=2*\wstg cm, anchor=west] at (3*\wstg, 3.4) {\textcolor{DarkRed!80}{GPU idle}};
  \node[train_box, minimum width=\wstg cm, anchor=west] at (5*\wstg, 3.4) {Train 2};

  % Timeline arrow
  \draw[->, line width=0.8pt, SlateLine!50] (0.0, 2.75) -- (11.2, 2.75);
  \node[anchor=west, font=\tiny, text=TextMuted] at (11.3, 2.75) {time};

  % Sequential Span Bracket
  \draw[line width=0.9pt, RedLine] (0.0, 2.35) -- (6*\wstg, 2.35);
  \draw[line width=0.9pt, RedLine] (0.0, 2.2) -- (0.0, 2.5);
  \draw[line width=0.9pt, RedLine] (6*\wstg, 2.2) -- (6*\wstg, 2.5);
  \node[anchor=north, font=\scriptsize\bfseries, text=DarkRed] at (3*\wstg, 2.15)
    {Total: 2 batches = 6 sequential stages};

  % =============================================================
  % DIVIDER
  % =============================================================
  \draw[SlateLine!25, line width=0.7pt] (-1.8, 1.45) -- (11.8, 1.45);

  % =============================================================
  % PART B: WITH PREFETCHING (ASYNC PIPELINE)
  % =============================================================
  \node[anchor=center, font=\small\bfseries, text=TextDark] at (5.4, 0.95)
    {With Prefetching (Async Pipeline)};

  % Track Labels
  \node[anchor=east, font=\scriptsize\bfseries, text=TextDark] at (-0.35, 0.1) {Storage};
  \node[anchor=east, font=\scriptsize\bfseries, text=TextDark] at (-0.35, -0.8) {CPU};
  \node[anchor=east, font=\scriptsize\bfseries, text=TextDark] at (-0.35, -1.7) {GPU};

  % Storage Track (y = 0.1)
  \node[read_box, minimum width=\wstg cm, anchor=west] at (0.0, 0.1) {Read 1};
  \node[read_box, minimum width=\wstg cm, anchor=west] at (\wstg, 0.1) {Read 2};
  \node[read_box, minimum width=\wstg cm, anchor=west] at (2*\wstg, 0.1) {Read 3};
  \node[read_box, minimum width=\wstg cm, anchor=west] at (3*\wstg, 0.1) {Read 4};

  % Prefetch depth badge at right of Storage
  \node[
    rectangle,
    rounded corners=3pt,
    draw=DarkGreen,
    ...
    \textbf{\scriptsize\textcolor{DarkGreen}{Prefetch depth = 2}}\\[1pt]
    \tiny\textcolor{TextDark}{2 batches in flight}
  };

  % CPU Track (y = -0.8)
  \node[idle_box, minimum width=\wstg cm, anchor=west] at (0.0, -0.8) {};
  \node[prep_box, minimum width=\wstg cm, anchor=west] at (\wstg, -0.8) {Prep 1};
  \node[prep_box, minimum width=\wstg cm, anchor=west] at (2*\wstg, -0.8) {Prep 2};
  \node[prep_box, minimum width=\wstg cm, anchor=west] at (3*\wstg, -0.8) {Prep 3};
  \node[prep_box, minimum width=\wstg cm, anchor=west] at (4*\wstg, -0.8) {Prep 4};

  % GPU Track (y = -1.7)
  \node[idle_box, minimum width=2*\wstg cm, anchor=west] at (0.0, -1.7) {\textcolor{TextMuted}{Ramp-up}};
  \node[train_box, minimum width=\wstg cm, anchor=west] at (2*\wstg, -1.7) {Train 1};
  \node[train_box, minimum width=\wstg cm, anchor=west] at (3*\wstg, -1.7) {Train 2};
  \node[train_box, minimum width=\wstg cm, anchor=west] at (4*\wstg, -1.7) {Train 3};
  \node[train_box, minimum width=\wstg cm, anchor=west] at (5*\wstg, -1.7) {Train 4};

  % Timeline arrow
  \draw[->, line width=0.8pt, SlateLine!50] (0.0, -2.35) -- (11.2, -2.35);
  \node[anchor=west, font=\tiny, text=TextMuted] at (11.3, -2.35) {time};

  % Pipelined Span Bracket
  \draw[line width=0.9pt, DarkGreen] (0.0, -2.75) -- (6*\wstg, -2.75);
  \draw[line width=0.9pt, DarkGreen] (0.0, -2.6) -- (0.0, -2.9);
  \draw[line width=0.9pt, DarkGreen] (6*\wstg, -2.6) -- (6*\wstg, -2.9);
  \node[anchor=north, font=\scriptsize\bfseries, text=DarkGreen] at (3*\wstg, -2.95)
    {4 batches in time of $\sim$3 sequential; GPU never idles after ramp-up};
  ...
  \node[
    ...
    \textbf{\scriptsize\textcolor{DarkRed}{Overlap hides storage latency: the GPU processes while the next batch loads}}
  };
...
::: {.column-margin}
![](images/svg/vol2_data_storage_margin_002.svg){width="100%" fig-alt="Sequence strip showing a 500 ms P99 I/O delay spanning three 200 ms compute windows, which implies a prefetch depth of three batches."}
...
from mlsysim.fmt import fmt, fmt_time, fmt_memory, fmt_math, check

class PrefetchDepth:
    # ┌── 1. LOAD ─────────────────────────────────────────
    compute_ms = 200  # scenario: per-batch compute time
    batch = 40 * MB  # scenario: collated batch size per GPU
    gpus_per_node = Systems.Nodes.DGX_H100.accelerators_per_node
    nvme_p99_ms = 50  # scenario: local NVMe P99 read latency
    pfs_p99_ms = 200  # scenario: parallel-file-system P99 latency
    obj_p99_ms = 500  # scenario: object-storage P99 latency
    obj_safety_depth = 5  # scenario: object-storage safety-margin depth
    # ┌── 2. EXECUTE ──────────────────────────────────────
    nvme_depth = math.ceil(nvme_p99_ms / compute_ms)
    pfs_depth = math.ceil(pfs_p99_ms / compute_ms)
    obj_depth = math.ceil(obj_p99_ms / compute_ms)
    obj_buf_gpu = obj_safety_depth * batch
    obj_buf_node = obj_buf_gpu * gpus_per_node
...
To illustrate the memory cost of prefetch buffer depth\index{Prefetch Buffer!depth calculation}, consider a large-batch text pipeline where each GPU processes a packed token batch in roughly `{python} PrefetchDepth.compute_ms_str` and the collated batch occupies about `{python} PrefetchDepth.batch_mb_str` per GPU. Reading from local NVMe, the P99 I/O latency for that batch is approximately `{python} PrefetchDepth.nvme_p99_ms_str`. The minimum prefetch depth is `{python} PrefetchDepth.nvme_depth_math` batch, and a safety margin of 2 is adequate. Reading from a parallel file system, the P99 I/O latency rises to roughly `{python} PrefetchDepth.pfs_p99_ms_str` due to network jitter and contention, requiring a minimum depth of `{python} PrefetchDepth.pfs_depth_math`, with a safety margin of 3 to account for occasional multi-hundred-millisecond outliers. Reading from object storage, the P99 latency can exceed `{python} PrefetchDepth.obj_p99_ms_str`, requiring a depth of at least `{python} PrefetchDepth.obj_depth_str`, with a safety margin of `{python} PrefetchDepth.obj_safety_str` or more. These numbers translate directly into host DRAM consumption: at `{python} PrefetchDepth.batch_mb_str` per batch, a depth-`{python} PrefetchDepth.obj_safety_str` prefetch buffer per GPU consumes `{python} PrefetchDepth.obj_buf_gpu_mb_str`, and `{python} PrefetchDepth.gpus_per_node_str` GPUs per node consume `{python} PrefetchDepth.obj_buf_node_gb_str`. At `{python} PrefetchDepth.batch_mb_str` per batch with a depth of 1, the same node needs only `{python} PrefetchDepth.min_buf_node_mb_str`. The storage tier directly determines the memory cost of the prefetch buffer.
...
- [ ] **Bandwidth requirement**: using @eq-pipeline-throughput, calculate the required aggregate storage bandwidth at 90 percent target utilization.
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: @sec-storage-multi-worker—worker allocation heuristic
from mlsysim import Systems
...
class WorkerAllocation:
    # ┌── 1. LOAD ─────────────────────────────────────────
    gpus_per_node = Systems.Nodes.DGX_H100.accelerators_per_node
    workers_per_gpu = 4  # scenario: W = 4 x G data-loader heuristic
    cpu_cores = 64  # scenario: server CPU core count per node
    # ┌── 2. EXECUTE ──────────────────────────────────────
    workers = workers_per_gpu * gpus_per_node
    remaining_cores = cpu_cores - workers
    # ┌── 3. GUARD ────────────────────────────────────────
    check(remaining_cores == 32, f"Expected 32 remaining cores, got {remaining_cores}")
    # ┌── 4. OUTPUT ───────────────────────────────────────
    gpus_per_node_str = fmt(gpus_per_node, precision=0)
    workers_str = fmt(workers, precision=0)
    cpu_cores_str = fmt(cpu_cores, precision=0)
    remaining_cores_str = fmt(remaining_cores, precision=0)
```

In PyTorch's `DataLoader`, each worker is a separate process with its own file descriptors and memory space. The `num_workers` parameter controls $W$. Setting $W$ too low leaves the GPU starved; setting $W$ too high wastes CPU resources on context switching and contention. A good heuristic is to start with $W = 4 \times G$, where $G$ is the number of GPUs per node, and profile the data stall ratio, adjusting until stalls are below 2 percent. For a node with `{python} WorkerAllocation.gpus_per_node_str` GPUs, this yields `{python} WorkerAllocation.workers_str` workers, which on a system with `{python} WorkerAllocation.cpu_cores_str` CPU cores leaves `{python} WorkerAllocation.remaining_cores_str` cores for the PyTorch runtime, GPU-to-GPU communication threads, and operating system overhead. The division of CPU resources between data loading and training orchestration is itself a capacity planning exercise.
...
```
For the `{python} NodeStagingContext.tokenized_text_tb_str` serialized dataset distributed across 256 nodes, each node is responsible for approximately `{python} NodeStagingContext.node_tokenized_gb_str` of data. If a node fails, its replacement must stage that shard subset from the PFS. At an uncontended PFS read speed of 4 GB/s, this staging takes about `{python} NodeStagingContext.node_stage_uncontended_s_str`; under recovery contention, an effective 400 MB/s per-node rate stretches the same copy to about `{python} NodeStagingContext.node_stage_contended_s_str`. If the orchestrator can instead schedule the replacement workload on an idle node that already has the data cached from a previous run, staging time is zero and training resumes instantly. At scale, this optimization compounds: in a 10,000-node cluster experiencing 10 node failures per day, locality-aware scheduling can save roughly a minute of staging time per failure, or about 10 minutes per day of aggregate cluster idle time, in the contended case.
...
class HashValidationCost:
    ...
    throughput_per_core = 18.5 * (MB / second)  # scenario SHA-256 throughput assumption
...
#### Execution semantics for distributed transformations {#sec-storage-distributed-transformation-semantics}

Versioned artifacts identify *what* a pipeline produced; execution semantics determine whether failures can change that result. Because a distributed stage completes at the pace of its slowest partition, **partition skew**\index{Partition skew!data pipelines} turns aggregate bandwidth into idle capacity when one key owns disproportionate work. If a downstream stage cannot sustain the arrival rate, **backpressure**\index{Backpressure!data pipelines} propagates its service-rate limit upstream by slowing admission or bounding in-flight data. It is a control response to overload, not another name for slow processing.

Failures create a separate correctness question. **At-most-once processing**\index{Processing semantics!at-most-once} can lose an output because it does not replay uncertain work; **at-least-once processing**\index{Processing semantics!at-least-once} can duplicate an effect because it does. An effectively **exactly-once outcome**\index{Processing semantics!exactly-once} requires replayable inputs and **idempotent writes**\index{Idempotent writes!data pipelines} or transactional output keyed by a stable event or transaction identifier [@akidau2015]. Transport delivery alone cannot provide this application-level guarantee.

::: {#exmp-storage-acknowledgment-gap .callout-example title="The acknowledgment gap"}

**Failure trace**: A worker commits event `e42`, then fails before its acknowledgment. Skipping the uncertain event risks loss; replaying a non-idempotent effect duplicates it. Writing by stable key `(feature_version, e42)` prevents the duplicate.
...
Streaming adds a time boundary to the contract. **Event time**\index{Event time!data pipelines} records when an event occurred; **processing time**\index{Processing time!data pipelines} records when a worker handled it. A **watermark**\index{Watermark!stream processing} estimates that event time has advanced far enough to close a window; a late-data policy determines whether later arrivals update, retract, quarantine, or drop its result [@akidau2015]. Thus, watermarks trade freshness against completeness rather than prove that no older event remains.

These mechanisms bound an optimization's blast radius. Buffering absorbs skew or a slow sink but increases recovery state and freshness lag; aggressive watermarks reduce delay but exclude more late data; retries help only when writes are safe to repeat. A reproducible training snapshot must therefore record source and code versions alongside partitioning, replay policy, stable identifiers, watermark state, and late-data decisions. Operations uses this contract to keep feature backfills and point-in-time joins from mixing histories.
...
from mlsysim.fmt import check, fmt_count, fmt_int, fmt_multiple, fmt_percent, fmt_qty, fmt_rate, fmt_time

class GDSLatency:
    # ┌── 1. LOAD ─────────────────────────────────────────
    gds_trad_latency = Systems.Storage.TraditionalGpuIoLatency
    gds_bypass_latency = Systems.Storage.GpuDirectStorageLatency
    ...
    gds_trad_cpu_seconds_val = total_images_per_s * gds_trad_latency.to(second).magnitude
    gds_bypass_cpu_seconds_val = total_images_per_s * gds_bypass_latency.to(second).magnitude
    ...
    image_kb_str = fmt_qty(150 * KB, KB, precision=0, commas=False)
...
1.  **Significance**: It eliminates the "bounce buffer" through system memory. In the small-transfer model detailed in notebook \ref{nbk-data-storage-cpu-bypass-dividend}, this reduces per-transfer latency by `{python} GDSLatency.gds_latency_reduction_pct_str` and makes the local transfer path `{python} GDSLatency.gds_speedup_mult_str` faster. It allows the GPU to saturate the NVMe link speed (for example, 7 GB/s) while reducing CPU utilization for I/O.
2.  **Distinction**: Unlike traditional I/O\index{Traditional I/O}, where payload data is staged through host buffers and copied between them, GDS provides direct memory access between the storage controller and the accelerator. CPU-issued cuFile calls and kernel storage drivers remain in the control path.
...
The latency reduction from GDS matters most when the training pipeline is already optimized and the remaining bottleneck is the CPU's ability to mediate transfers. In a node with 8 GPUs, each running a data loader with 4 workers, the CPU must manage 32 concurrent I/O streams. At `{python} GDSLatency.gds_trad_us_str` per transfer, the CPU spends significant time in interrupt handling and buffer management. GDS offloads this work to hardware DMA engines, freeing CPU cores for data augmentation and other preprocessing tasks. @Fig-gds-path contrasts the two data paths side by side, showing where GDS eliminates the CPU-mediated copies that dominate per-transfer overhead.

::: {#fig-gds-path fig-env="figure" fig-pos="htb" fig-cap="**Traditional vs. GPUDirect Storage path**: The traditional path (top) requires two CPU-mediated payload copies. The GDS path (bottom) uses DMA to transfer data directly from NVMe to GPU memory, eliminating those bounce copies while retaining CPU submission and coordination." fig-alt="Diagram comparing traditional three-hop storage path with GDS direct DMA path from NVMe to GPU."}
...
\begin{tikzpicture}[
  ...
  % =============================================================
  \node[anchor=center, font=\small\bfseries, text=TextDark] at (0, 5.2)
    {(a) Traditional Path: CPU Bounce Buffer};

  \def\ya{3.8}

  % 1. NVMe SSD
  \node[node_nvme, minimum width=2.3cm] (a_nvme) at (-4.85, \ya) {
    \textbf{\scriptsize\textcolor{OrangeLine!90!black}{NVMe SSD}}\\[2pt]
    \tiny\textcolor{TextDark}{7 GB/s seq read}
  };

  % 2. CPU RAM (Bounce Buffer)
  \node[node_ram, minimum width=2.4cm] (a_ram) at (-1.65, \ya) {
    \textbf{\scriptsize\textcolor{NavyBlue}{CPU RAM}}\\[1.5pt]
    \tiny\textcolor{TextDark}{(Bounce Buffer)}\\[1.5pt]
    \tiny\itshape\textcolor{DarkRed}{Extra copy here}
  };

  % 3. CPU (kernel involved)
  \node[node_cpu, minimum width=2.3cm] (a_cpu) at (1.65, \ya) {
    \textbf{\scriptsize\textcolor{TextDark}{CPU}}\\[2pt]
    \tiny\textcolor{TextMuted}{kernel involved}
  };

  % 4. GPU HBM
  \node[node_gpu, minimum width=2.3cm] (a_gpu) at (4.85, \ya) {
    \textbf{\scriptsize\textcolor{DarkGreen}{GPU HBM}}\\[2pt]
    \tiny\textcolor{TextDark}{Training data}
  };

  % Arrows (now over ~1.0cm gaps)
  \draw[->, line width=1.1pt, DarkRed]
    (a_nvme.east) -- node[above, font=\tiny\bfseries, text=DarkRed] {PCIe} (a_ram.west);
  \draw[->, line width=1.1pt, DarkRed]
    (a_ram.east) -- node[above, font=\tiny\bfseries, text=DarkRed] {memcpy} (a_cpu.west);
  \draw[->, line width=1.1pt, DarkRed]
    (a_cpu.east) -- node[above, font=\tiny\bfseries, text=DarkRed] {PCIe} (a_gpu.west);
  ...
  \node[
    ...
    \textbf{\scriptsize\textcolor{DarkRed}{2 PCIe crossings $+$ CPU memcpy overhead $+$ kernel context switches}}\\[1.5pt]
    \tiny\textcolor{TextDark}{Data touches CPU memory before reaching GPU}
  };
  ...
  % DIVIDER
  % =============================================================
  \draw[SlateLine!25, line width=0.7pt] (-6.4, 1.85) -- (6.4, 1.85);


  % =============================================================
  % PART B: GPUDIRECT STORAGE (CPU BYPASS)
  % =============================================================
  \node[anchor=center, font=\small\bfseries, text=TextDark] at (0, 1.35)
    {(b) GPUDirect Storage: CPU Bypass};

  \def\yb{-0.05}

  % 1. NVMe SSD
  \node[node_nvme, minimum width=2.3cm] (b_nvme) at (-4.85, \yb) {
    \textbf{\scriptsize\textcolor{OrangeLine!90!black}{NVMe SSD}}\\[2pt]
    \tiny\textcolor{TextDark}{7 GB/s seq read}
  };

  % 4. GPU HBM
  \node[node_gpu, minimum width=2.3cm] (b_gpu) at (4.85, \yb) {
    \textbf{\scriptsize\textcolor{DarkGreen}{GPU HBM}}\\[2pt]
    \tiny\textcolor{TextDark}{Training data}
  };

  % Direct DMA Green Arrow
  \draw[->, line width=1.4pt, DarkGreen]
    (b_nvme.east) --
    node[above=2pt, font=\scriptsize\bfseries, text=DarkGreen] {PCIe Direct DMA (GPU-initiated)}
    (b_gpu.west);

  % Bypassed CPU ghost box below arrow
  \node[
    rectangle,
    rounded corners=3pt,
    draw=SlateLine!40,
    ...
    \textbf{\scriptsize\textcolor{TextMuted}{CPU}}\\[1.5pt]
    \tiny\itshape\textcolor{TextMuted}{bypassed}
  };
  ...
  \node[
    ...
    \textbf{\scriptsize\textcolor{DarkGreen}{1 PCIe crossing, no CPU involvement, no bounce buffer}}\\[1.5pt]
    \tiny\textcolor{TextDark}{GPU DMA engine reads directly from NVMe controller}
  };
  ...
  \node[
    ...
    \textbf{\scriptsize\textcolor{DarkRed}{Performance Gain with GDS}}\\[2pt]
    \fontsize{6.8pt}{8.5pt}\selectfont
    \textcolor{TextDark}{75\% lower latency \quad$\vert$\quad 4$\times$ lower CPU time \quad$\vert$\quad No CPU-mediated copy}
  };
...
The architectural evolution from CPU-mediated copies to direct storage access reflects a broader trend of removing CPU payload copies from the critical data path wherever possible. @Sec-network-fabrics-rdma described RDMA, which allows network payloads to move without CPU copying. GDS does the same for storage payloads. Together, RDMA and GDS let data flow from remote storage, through the network fabric, and into GPU memory without a host-memory bounce copy. The CPU still submits and coordinates the operation, while more of its capacity remains available for data augmentation, pipeline orchestration, and error handling.
...
class EconRatios:
    ...
    cost_nvme_gb_mo = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.to(USD / (GB * ureg.month)).magnitude
    cost_s3_gb_mo = Infrastructure.Pricing.Storage.S3StandardLowPerTbMonth.rate.to(USD / (GB * ureg.month)).magnitude
    cost_egress = Infrastructure.Pricing.Cloud.EgressPerGB.rate
    dataset_size = 100 * TB  # scenario dataset size assumption

    # ┌── 2. EXECUTE ──────────────────────────────────────
    tier_cost_ratio_val = cost_nvme_gb_mo / cost_s3_gb_mo
    egress_100tb_val = (dataset_size * cost_egress).to(USD).magnitude
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    tier_cost_ratio_mult_str = fmt_multiple(tier_cost_ratio_val, precision=0, commas=False)
    egress_100tb_usd_str = fmt_usd(egress_100tb_val, precision=0)
```
...
class ObjectStorageCostRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    dataset_size = 100 * THOUSAND * GB
    s3_annual_cost = (dataset_size * Infrastructure.Pricing.Storage.S3StandardLowPerTbMonth.rate * 12 * ureg.month).to(USD)
    nvme_annual_cost = (dataset_size * Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate * 12 * ureg.month).to(USD)
    s3_annual_usd_str = fmt_usd(s3_annual_cost, precision=0)
    nvme_annual_usd_str = fmt_usd(nvme_annual_cost, precision=0)
```
...
from mlsysim import GB, THOUSAND, USD, ureg
from mlsysim.fmt import fmt_usd

class GlacierCostRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    dataset_size = 100 * THOUSAND * GB
    glacier_annual_cost = (dataset_size * Infrastructure.Pricing.Storage.GlacierStandardPerTbMonth.rate * 12 * ureg.month).to(USD)
    glacier_annual_usd_str = fmt_usd(glacier_annual_cost, precision=0)
```
...
class StorageIcebergCosts:
    # ┌── 1. LOAD ─────────────────────────────────────────
    dataset = 100 * TB  # scenario: representative training dataset
    epochs = 10  # scenario: epochs (dataset reads) per run
    file_count = 100_000_000  # scenario: number of individual files
    get_rate_per_1k = 0.0004  # scenario: S3 GET price per 1,000 requests
    get_request_unit = 1000  # scenario: S3 GET pricing denominator
    ...
    glacier_retrieval_rate = Infrastructure.Pricing.Storage.GlacierRetrievalPerGB.rate
    # ┌── 2. EXECUTE ──────────────────────────────────────
    egress_10epoch = (dataset * epochs * egress_rate).to(USD)
    egress_once = (dataset * egress_rate).to(USD)
    get_request_cost = file_count * (get_rate_per_1k / 1000) * USD
    storage_monthly = (dataset * s3_rate * ureg.month).to(USD)
    glacier_retrieval = (dataset * glacier_retrieval_rate).to(USD)
    # ┌── 3. GUARD ────────────────────────────────────────
    ...
    glacier_egress_usd_str = fmt_usd(egress_once.magnitude, precision=0)
...
# │ How: 100 TB x HBM $/GB; 1,000-GPU x Fleet GpuHourRef x utilization shortfall.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt_count, fmt_percent, fmt_time, fmt_usd

class StorageCostExtremes:
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    dataset_tb = 100  # scenario: 100 TB training dataset
    hbm_per_gb = Infrastructure.Pricing.Storage.HbmEquivalentPerGb.rate
    cluster_size = 1000  # scenario: round 1,000-GPU cluster
    gpu_hour_rate = Infrastructure.Pricing.Fleet.GpuHourRef.rate
    util_high = 0.90  # scenario: storage-healthy utilization
    util_low = 0.70  # scenario: storage-starved utilization
    run_days = 30
    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    hbm_storage = (dataset_tb * TB).to(GB) * hbm_per_gb
    idle_daily = cluster_size * gpu_hour_rate * (HOURS_PER_DAY * hour)
    idle_loss_daily = idle_daily * (util_high - util_low)
    idle_loss_run = idle_loss_daily * run_days
    # ┌── 3. GUARD ─────────────────────────────────────────────────────────
    ...
    util_delta_pct_str = fmt_percent(util_high - util_low, precision=0, commas=False, style="prose")
...
- **Object storage (S3 Standard)**: `{python} ObjectStorageCostRecap.s3_annual_usd_str`/year
- **Local NVMe (provisioned)**: `{python} ObjectStorageCostRecap.nvme_annual_usd_str`/year
- **Archive (Glacier)**: `{python} GlacierCostRecap.glacier_annual_usd_str`/year
- **HBM equivalent** (hypothetically storing 100 TB in GPU memory at \$15/GB): `{python} StorageCostExtremes.hbm_storage_usd_str` in hardware amortization

The `{python} EconRatios.tier_cost_ratio_mult_str` cost difference between local NVMe and object storage (with the full HBM-to-archive span exceeding 3,000$\times$) explains why the hierarchy exists: data must live at the cheapest tier possible, migrating upward only when needed and returning downward when done. This cost gradient reflects the underlying physics. Faster storage requires more expensive materials (HBM uses 3D-stacked silicon with through-silicon vias), more energy per bit accessed, and more physical proximity to the accelerator (which limits the amount that can be provisioned per node). Cheaper storage uses commodity components (standard hard drives for archive), consumes minimal energy when idle, and can be located anywhere with network connectivity.

::: {.column-margin}
![](images/svg/vol2_data_storage_margin_003.svg){width="100%" fig-alt="Cloud storage cost bar dominated by repeated egress."}

*Repeated egress, not storage, dominates cloud cost.*
:::

Storage cost, however, is only part of the equation. **Data Transfer Costs**\index{Data Transfer Cost} can dominate, especially in cloud environments. Under the representative cross-boundary egress assumption used here, reading the 100 TB dataset from object storage to compute instances incurs a transfer charge of `{python} EconRatios.egress_100tb_usd_str`. For multi-epoch training that reads the dataset `{python} StorageIcebergCosts.epochs_str` times, the egress cost alone exceeds `{python} StorageIcebergCosts.egress_10epoch_usd_str`, more than the annual storage cost. This inversion, where *reading* data costs more than *storing* it, drives the architecture decision to cache data on local NVMe rather than streaming from object storage each epoch.

The economic analysis changes further when accounting for the opportunity cost of accelerator idle time. A cluster of `{python} StorageCostExtremes.cluster_size_str` at `{python} StorageCostExtremes.gpu_hour_rate_str` costs `{python} StorageCostExtremes.idle_daily_usd_str` per day. If an undersized storage system reduces accelerator utilization from `{python} StorageCostExtremes.util_high_pct_str` to `{python} StorageCostExtremes.util_low_pct_str`, the organization loses `{python} StorageCostExtremes.idle_loss_daily_usd_str` per day in wasted compute. Over a `{python} StorageCostExtremes.run_days_str` training run, this amounts to `{python} StorageCostExtremes.idle_loss_run_usd_str` of lost compute value, an amount that would easily fund a parallel file system upgrade or additional NVMe capacity. The economically rational approach treats storage investment not as an expense but as an enabler of compute utilization: every dollar spent improving storage throughput returns several dollars of increased accelerator productivity.
...
- **Egress fees**\index{Egress Fee}: A representative cross-boundary egress charge of `{python} StorageIcebergCosts.egress_rate_gb_str` makes a `{python} StorageIcebergCosts.dataset_tb_str` dataset read once per epoch across `{python} StorageIcebergCosts.epochs_str` epochs cost `{python} StorageIcebergCosts.egress_10epoch_usd_str` in egress alone.
...
- **Idle accelerator cost**\index{Idle Accelerator Cost}: If storage stalls reduce GPU utilization from `{python} StorageCostExtremes.util_high_pct_str` to `{python} StorageCostExtremes.util_low_pct_str` on a cluster of `{python} StorageCostExtremes.cluster_size_str` at `{python} StorageCostExtremes.gpu_hour_rate_str`, the lost compute costs `{python} StorageCostExtremes.idle_loss_daily_usd_str`/day (`{python} StorageCostExtremes.util_delta_pct_str` of `{python} StorageCostExtremes.idle_daily_usd_str` daily spend), dwarfing any storage savings.
- **Retrieval fees**\index{Retrieval Fee}: Archive storage charges retrieval fees (`{python} StorageIcebergCosts.glacier_retrieval_rate_gb_str` for Glacier) in addition to egress. Restoring a `{python} StorageIcebergCosts.dataset_tb_str` dataset from Glacier costs `{python} StorageIcebergCosts.glacier_retrieval_usd_str` in retrieval fees plus `{python} StorageIcebergCosts.glacier_egress_usd_str` in egress.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @nbk-data-storage-egress-tax — the egress-tax worked example
...
# │      come from Infrastructure.Pricing (S3StandardPerTbMonth,
# │      NvmeLowPerTbMonth, EgressPerGB, Capital.H100GpuPurchaseUsd).
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_qty, fmt_usd, fmt_int, check

class StreamVsStageEconomics:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    dataset = 50 * TB  # scenario assumption: vision training corpus
    epochs = 20  # scenario assumption: epochs per training run
    runs_per_year = 4  # scenario assumption: training runs per year
    s3_rate = Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    s3_annual = (dataset * s3_rate * (12 * ureg.month)).to(USD)
    egress_per_run = (dataset * epochs * egress_rate).to(USD)
    stream_total = s3_annual + runs_per_year * egress_per_run
    nvme_annual = (dataset * nvme_rate * (12 * ureg.month)).to(USD)
    stage_egress_annual = (dataset * runs_per_year * egress_rate).to(USD)
    stage_total = s3_annual + nvme_annual + stage_egress_annual
    savings = stream_total - stage_total
    ...
    breakeven_epochs = ((nvme_annual + stage_egress_annual) / (runs_per_year * (dataset * egress_rate).to(USD))).to("").magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_tb_str = fmt_qty(dataset, TB, precision=0, commas=False)
    ...
    s3_rate_gb_mo_str = fmt_usd(s3_rate, precision=3, commas=False, per="GB/month")
...
class BuildVsBuyStorageEconomics:
    # ┌── 1. LOAD ─────────────────────────────────────────
    from mlsysim.fmt import check, fmt, fmt_rate, fmt_usd, fmt_time_range

    capex_low = 3_000_000
    capex_high = 5_000_000
    annual_ops = 500_000
    cloud_cost = Infrastructure.Pricing.Storage.S3StandardLowPerTbMonth.rate
    cloud_cost_gb_month = cloud_cost.to(USD / (GB * ureg.month)).magnitude
...
Data that must be retained for compliance or reproducibility but is not expected to be accessed during normal operations drops to the **Archive Tier**\index{Archive Tier} (Glacier or equivalent). Lifecycle policies automatically transition data from cold to archive after a configurable period, typically 90 to 365 days since last access.
...
# │ How: (in-place 500 TB - steady 100 TB) x PFS $/TB-month x 12 months.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_usd
...
class PfsTieringSavings:
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    n_jobs = 50  # scenario: 50 concurrent training jobs
    job_data_tb = 10  # scenario: 10 TB per job
    steady_state_tb = 100  # scenario: usage after automated demotion
...
The storage requirements for inference differ fundamentally from full-corpus training. Training streams datasets and writes checkpoints in bursts. In the weight-resident serving baseline, inference reads model weights at startup and then serves requests from HBM. The primary storage challenge shifts from sustained dataset throughput to model-loading latency, although retrieval indexes and KV-cache offload add serving-time I/O when those mechanisms are enabled.

The most critical metric for inference storage is **Model Loading Latency**\index{Model Loading!latency}, often called cold-start time: the duration required to load a model from storage into the accelerator's HBM. For the 175B parameter language model, the weights alone occupy 350 GB in FP16 format. Loading this sequentially from a single high-performance NVMe drive at 14 GB/s takes 25 seconds, an unacceptable delay for a user-facing application. Loading from a shared PFS at a per-node rate of 4 GB/s takes nearly 88 seconds. From object storage at 1 GB/s, the delay approaches six minutes.

To reduce cold-start time, the model is sharded and loaded in parallel. If the 350 GB model is striped across 8 NVMe drives, each loading a 43.75 GB shard at 7 GB/s, the total load time drops to roughly 6 seconds. When using tensor parallelism across 8 GPUs, each GPU loads only its 43.75 GB shard, which can be accomplished in under 4 seconds by reading from host DRAM. For the most latency-sensitive applications, organizations maintain **Warm Replicas**\index{Warm Replica}: one or more copies of the model weights kept preloaded in host DRAM, ready to be transferred to HBM in under two seconds. This trades DRAM capacity for near-instantaneous cold-start times.
...
Retrieval-augmented inference adds another serving-time working set. Graph-based approximate nearest-neighbor (ANN) indexes replace sequential model loading with random index traversal; flat scans, inverted-file (IVF) indexes, and product-quantized (PQ) indexes instead trade more sequential work or compression for fewer graph hops.

## Retrieval Infrastructure: Vector Indexes {#sec-data-storage-retrieval-infrastructure}

A retrieval system first converts documents into vectors that represent semantic meaning, then serves an inference request by finding nearby vectors and returning their source documents as context. With a graph-based ANN index, the storage system traverses a high-dimensional vector graph instead of streaming sequential blobs via formats such as TFRecord or Parquet. This is one index family, not a universal vector-database layout: exact flat search, IVF partitioning, and PQ-compressed variants impose different bandwidth, capacity, and recall trade-offs.
...
After a thousand GPUs have been training for six hours, a power supply failure turns checkpoint storage\index{Checkpoint!storage architecture} into the difference between resuming quickly and repeating hours of work. The decisive storage questions are how quickly the most recent checkpoint was saved, where it resides, and whether it can be read back. Checkpoints are the most demanding write workload in the storage hierarchy. They are also among the most consequential: a lost checkpoint after a hardware failure means repeating hours or days of training. The storage architecture must therefore minimize $T_{\text{write}}$, the time the training pipeline pauses to save a checkpoint.

A `{python} CheckpointModelIntro.gpt3_params_b_str` parameter model with mixed-precision Adam\index{Adam Optimizer!state size} generates a resumable checkpoint of approximately `{python} CheckpointModelIntro.ckpt_total_gb_str`. The byte contract comprises `{python} CheckpointModelIntro.weights_fp16_gb_str` of FP16 model weights, `{python} CheckpointModelIntro.master_weights_fp32_gb_str` of FP32 master weights, and `{python} CheckpointModelIntro.adam_moments_gb_str` for the two FP32 Adam moments. Learning-rate-scheduler state, random-number-generator state, and the data-loader cursor add little capacity but remain required for deterministic continuation. Gradients are omitted because the restored job recomputes them. Every rank saves its assigned shard simultaneously, creating a checkpoint storm that the storage system must absorb without disrupting ongoing training reads.

::: {.column-margin}
![](images/svg/data_storage_checkpoint_storm_write_time.svg){width="100%" fig-alt="Margin ladder comparing naive replicated checkpoint write time with ZeRO-3 sharded checkpoint write time."}
...
class CheckpointStormDefinition:
    ...
    resumable_state = weights + master_weights + adam_moments
    per_replica_state = resumable_state
    zero3_total = per_replica_state
    zero3_write = zero3_total / fabric_bw
    naive_total = per_replica_state * replicas
    naive_write = naive_total / fabric_bw
...
***Checkpoint Storm***\index{Checkpoint Storm!storage traffic} is a burst of synchronized network and storage traffic that occurs when all nodes in an ML training fleet save model state simultaneously.

1.  **Significance**: The storm magnitude scales as $T_{\text{write}} = R \times \text{per-replica state} / \text{BW}_{\text{fabric}}$, where $R$ is the number of replicas writing at once. Under the chapter's checkpoint contract, a `{python} CheckpointStormDefinition.params_b_str`-parameter model stores `{python} CheckpointStormDefinition.weights_gb_str` of FP16 weights, `{python} CheckpointStormDefinition.master_weights_gb_str` of FP32 master weights, and `{python} CheckpointStormDefinition.adam_moments_gb_str` of Adam moments. With vanilla data parallelism across `{python} CheckpointStormDefinition.replicas_str`, naively writing that complete state from every replica generates `{python} CheckpointStormDefinition.naive_total_tb_str` of simultaneous traffic. At 100 GB/s fabric bandwidth, the write takes roughly `{python} CheckpointStormDefinition.naive_write_s_str`, or over `{python} CheckpointStormDefinition.naive_write_min_str` of training stall. ZeRO-3 collapses the written state to one sharded logical copy of roughly `{python} CheckpointStormDefinition.zero3_total_tb_str`, bringing $T_{\text{write}}$ down to roughly `{python} CheckpointStormDefinition.zero3_write_s_str`. Gradients do not appear in either total because recovery recomputes them.
...
3.  **Common pitfall**: A frequent misconception is that checkpointing every 100 steps is "low overhead." At 70B scale, this assumption is catastrophically wrong: if each training step takes 10 seconds and a checkpoint storm takes `{python} CheckpointStormDefinition.naive_write_s_str`, checkpointing every 100 steps means spending most of the run on checkpoint I/O rather than useful training.
...
class CheckpointStormCalc:
    ...
    pfs_aggregate_bw = storage_path.durable_store.bandwidth
    checkpoint_interval = 600 * second  # scenario checkpoint interval assumption

    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    total_checkpoint = Models.Language.GPT3.parameters * CHECKPOINT_RESUME_BYTES_PER_PARAM
    per_node_shard = total_checkpoint / nodes
    local_bw = local_drive_count * local_drive_bw
    local_write = per_node_shard / local_bw
    pfs_copy = total_checkpoint / pfs_aggregate_bw
    per_node_pfs_bw = pfs_aggregate_bw / nodes
    per_node_pfs_copy = per_node_shard / per_node_pfs_bw
    overhead_pct = (local_write / checkpoint_interval).to("").magnitude * 100
...
class CheckpointTotalGbRecap:
    ckpt_total = Models.Language.GPT3.parameters * CHECKPOINT_RESUME_BYTES_PER_PARAM
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ckpt_total_gb_str = fmt_memory(ckpt_total, unit=GB, precision=0, commas=True)
```
...
5. **Overhead**\index{Checkpointing!overhead}: `{python} CheckpointStormCalc.local_write_s_str` pause every `{python} CheckpointStormCalc.checkpoint_interval_s_str` $\approx$ `{python} CheckpointStormCalc.overhead_pct_str` training time lost to checkpointing.
...
The **Tiered Staging**\index{Tiered Staging!checkpoint storage} strategy minimizes $T_{\text{write}}$ by writing in two phases. In the first phase, each node writes its checkpoint shard to local NVMe at full bandwidth. With `{python} CheckpointStormCalc.local_drive_count_str` NVMe drives providing `{python} CheckpointStormCalc.local_bw_gb_per_s_str` aggregate, a per-node shard of roughly `{python} CheckpointStormCalc.per_node_shard_gb_str` (a `{python} CheckpointStormCalc.nodes_str`-node ZeRO-3 split of the `{python} CheckpointTotalGbRecap.ckpt_total_gb_str` total checkpoint) completes in roughly `{python} CheckpointStormCalc.local_write_s_str`. The training pipeline can resume immediately after the local write completes.
...
Durability alone does not make a distributed checkpoint visible safely. Each rank first writes its shard under an immutable generation identifier, computes a checksum over the completed bytes, and acknowledges the shard only after the target tier has persisted it. A coordinator then writes a manifest containing the training step, checkpoint schema, expected shard set, sizes, and checksums. The manifest or commit marker is published last using the target backend's atomic publication primitive: an atomic create or rename on a file system, or an immutable manifest plus conditional creation or a versioned pointer under the object store's consistency guarantees. PyTorch Distributed Checkpoint exposes the ordering directly: storage writers write rank data before the coordinator's `finish()` operation writes metadata and marks the checkpoint successful, while the storage backend supplies the publication semantics [@pytorchDistributedCheckpointDocs]. Restore logic lists only committed generations, verifies every shard against the manifest, and rejects temporary, partial, or mixed-step state. This ordering separates *writing bytes* from *publishing a restorable checkpoint*: a crash can leave garbage to collect, but it cannot make an incomplete generation appear valid.
...
class CheckpointFleetRetention:
    ...
    training_window = ReferenceStats.StorageTrainingCorpus.TrainingWindow
    checkpoint_interval = ReferenceStats.StorageTrainingCorpus.CheckpointInterval
    ckpt_total = Models.Language.GPT3.parameters * bytes_per_param_ckpt
    checkpoint_count_val = int((training_window / checkpoint_interval).to("").magnitude)
    ckpt_fleet_total = ckpt_total * checkpoint_count_val
...
from mlsysim.fmt import fmt, fmt_time, fmt_bandwidth

class CheckpointStorageDesignPrompt:
    nodes = Systems.Clusters.Training_1K.count
    checkpoint_interval = ReferenceStats.StorageTrainingCorpus.CheckpointInterval
    local_nvme_bw = 28 * (GB / second)  # scenario aggregate local NVMe bandwidth assumption
    pfs_aggregate_bw = 1 * (TB / second)  # scenario aggregate PFS bandwidth assumption
...
- [ ] **Local write**: calculate $T_{\text{write}}$ for the local write when each node writes to local NVMe at `{python} CheckpointStorageDesignPrompt.local_nvme_bw_gb_per_s_str`.
...
In a sharded training setup with `{python} CheckpointStorageDesignPrompt.nodes_str`, no single node necessarily holds the complete training state. One node may hold a shard of the optimizer state, while other model-partitioning strategies split the weights themselves across devices. A complete checkpoint therefore requires every node to save its shard, and the checkpoint is publishable only after all expected shards have been durably written and verified against one manifest. This creates a coordination problem: the system must confirm that all `{python} CheckpointStorageDesignPrompt.nodes_str` have finished writing before it exposes the generation for restore.
...
Beyond the choice of synchronous or asynchronous methods, **Checkpoint Format Optimization**\index{Checkpoint Format Optimization} reduces the per-node I/O volume. In the simplest data-parallel setup, every node holds an identical copy of the model weights, so a naive checkpoint would redundantly save the same data from every node. Sharded checkpoint protocols avoid that redundancy by saving shards instead of full replicas. DeepSpeed's **Zero-Redundancy Checkpointing**\index{Checkpointing!zero-redundancy} uses its ZeRO optimizer-state partitioning: each of the $H$ nodes saves only its $1/H$ shard of the optimizer state, and the system saves one consolidated copy of the weights. PyTorch's **Distributed Checkpoint**\index{Distributed Checkpoint} protocol follows the same storage principle by writing each rank's sharded data to a distinct file. For the 175B model using ZeRO-3 across `{python} CheckpointStormCalc.nodes_str` nodes, the result is concrete: each node writes only its roughly `{python} CheckpointStormCalc.per_node_shard_gb_str` shard of the total `{python} CheckpointTotalGbRecap.ckpt_total_gb_str` state, reducing the local write from minutes to around a second on one NVMe drive, or well below a second on a local stripe. The tiered-staging derivation earlier in this section already threaded these per-node shard sizes and write times through the running example, and the same arithmetic governs distributed coordination: the exposed pipeline pause is the local NVMe write, while the durable copy to the parallel file system overlaps with subsequent training.
...
from mlsysim import Models
from mlsysim.fmt import fmt, fmt_memory, fmt_qty, fmt_count, fmt_ratio
...
class StorageFootprintTable:
    ...
    checkpoint_count_val = int((training_window / checkpoint_interval).to("").magnitude)
    ckpt_fleet_total = ckpt_total * checkpoint_count_val
    archive_lineage = (checkpoint_count_val // 100) * ckpt_total + compressed_text + tokenized_text

    # ┌── 4. OUTPUT ───────────────────────────────────────
    compressed_text_tb_str = fmt_memory(compressed_text, unit=TB, precision=0, commas=False)
    tokenized_text_tb_str = fmt_memory(tokenized_text, unit=TB, precision=0, commas=False)
    ckpt_fleet_total_pb_str = fmt_memory(ckpt_fleet_total, unit=PB, precision=1, commas=True)
    archive_lineage_tb_str = fmt_qty(archive_lineage, TB, precision=1, commas=False, approx=True)
    epochs_str = fmt_count(1, commas=False, label="epoch")
    ckpt_io_dominance_factor_str = fmt_ratio(
        int((ckpt_fleet_total / tokenized_text).to("").magnitude),
        precision=0,
        commas=True,
    )
...
@Tbl-data-storage-175b-footprint assembles the complete storage picture for the running example, a 30-day training run of a 175B-parameter model on `{python} StorageFootprintTable.training_nodes_str`.
...
| Master weights (FP32)                      |                          700 GB                          | GPU HBM (ZeRO-partitioned)                                 |
| Adam moments (two FP32 states)             |                         1,400 GB                         | GPU HBM (ZeRO-partitioned)                                 |
| Single resumable checkpoint                |                         2,450 GB                         | NVMe $\to$ PFS $\to$ Object Storage                        |
| All checkpoints (30 days, 10-min interval) | `{python} StorageFootprintTable.ckpt_fleet_total_pb_str` | NVMe (transient) $\to$ PFS (recent) $\to$ Object (archive) |
...
Feature stores and model registries, the operational systems that manage feature serving and model versioning, also depend on the hierarchy's tiering, durability, and latency boundaries. At fleet scale, their storage consequence is a shared rule rather than another lineage tutorial: immutable content identifiers and manifests let many experiments reuse unchanged artifacts without copying entire datasets or model packages.

## The Synthetic Fuel Line {#sec-storage-synthetic-fuel}

The storage architecture changes again when the fleet exhausts its supply of human-authored training data, because synthesized data must carry its own provenance. As high-quality human-generated text becomes harder to expand, the fleet encounters the **data wall**\index{Data Wall!definition}: the point where additional training samples must increasingly come from synthesis, simulation, curation, or expensive human collection [@villalobos2022will]. This shift from simple collection to managed data generation sends more than payload bytes through the hierarchy. A "synthetic fuel line" must also store the **Provenance Chain**\index{Provenance Chain!definition} of every sample: which model generated it, which filters accepted or rejected it, which judge or verifier models scored it, and which downstream run consumed it. Without that lineage, the system risks **Model Collapse**\index{Model Collapse!definition}, where a model degrades by training on its own unverified errors [@shumailov2024].
...
class SyntheticStorage:
    ...
    data_size = 1 * TB  # scenario assumption: synthetic dataset payload size
    provenance_metadata_overhead = SYNTHETIC_PROVENANCE_OVERHEAD
    verification_passes = SYNTHETIC_VERIFICATION_PASSES
    # ┌── 2. EXECUTE ───────────────────────────────────────
    stored_footprint = data_size * (1 + provenance_metadata_overhead)
    verification_read_traffic = data_size * verification_passes
    pipeline_traffic = data_size + verification_read_traffic + (data_size * provenance_metadata_overhead)
    capacity_amplification = (stored_footprint / data_size).to("").magnitude
    traffic_amplification = (pipeline_traffic / data_size).to("").magnitude
    # ┌── 3. GUARD ─────────────────────────────────────────
...
from mlsysim.fmt import fmt_multiple, fmt_qty
...
class StorageFallaciesBandwidth:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvme_bw_gb_per_s_str = fmt_qty(nvme_bw_raw, GB / second, precision=0, commas=False)
    h100_bw_tb_per_s_str = fmt_qty(h100_bw, TB / second, precision=2, commas=False)
    bw_ratio_mult_str = fmt_multiple(bw_ratio_val, precision=1, commas=True)
```
...
class GDSLatencyRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gds_speedup_mult_str = fmt_multiple(4, precision=0, commas=False)
```
...
from mlsysim.fmt import fmt_multiple

class EconRatiosRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tier_cost_ratio = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.to(USD / (GB * ureg.month)) / (0.02 * USD / (GB * ureg.month))
    tier_cost_ratio_mult_str = fmt_multiple(
        tier_cost_ratio.to("").magnitude,
        precision=0,
        commas=False,
    )
```
...
* **HBM is the destination**: Every lower tier exists to keep GPU HBM populated (principle \ref{pri-storage-hierarchy}). Size that path with $\text{BW}_{\text{required}} = N_{\text{GPU}} \times \eta_{\text{target}} \times D_{\text{vol,batch}} / T_{\text{iteration}}$; the roughly 30$\times$ aggregate bandwidth gap between HBM and object storage drives every tiering decision.
* **ML workloads invert storage assumptions**: Traditional caching, IOPS optimization, and small-write durability patterns all produce the wrong answer for ML training. Throughput (GB/s) matters more than IOPS, and small files make metadata the first bottleneck; aggregated shards reduce metadata operations by orders of magnitude (principle \ref{pri-io-wall}).
...
* **GPUDirect Storage eliminates CPU bottlenecks**: GDS bypasses the CPU in the data path, reducing per-transfer latency by `{python} GDSLatencyRecap.gds_speedup_mult_str` and freeing CPU cores for augmentation.
* **Checkpoint recovery is a storage protocol**: Local NVMe staging minimizes pause time, but a restorable checkpoint also needs durable publication, a committed manifest, and verified shards; durability, availability, and consistency remain distinct contracts.
...
* **Economics drive tiering**: The `{python} EconRatiosRecap.tier_cost_ratio_mult_str` cost difference between local NVMe and object storage (and the >3,000$\times$ span from HBM to archive) mandates a tiering strategy (principle \ref{pri-storage-hierarchy}). Egress fees often exceed storage fees; design for total cost of data delivery, not storage cost alone.

:::

Every other chapter in this book optimizes something the accelerators do. This one optimizes what reaches them. The whole storage hierarchy exists to keep the most expensive resource in the building from going idle, and that reframes where performance can be won. When data delivery is the binding constraint, a cluster can sit at half utilization with every accelerator healthy. The system is then only as fast as the tier that feeds it.

::: {.callout-chapter-connection title="From storage to splitting"}

Infrastructure is complete. Modern clusters combine accelerators that compute at petaFLOP/s (@sec-compute-infrastructure), fabrics that move gradients at TB/s (@sec-network-fabrics), and a storage hierarchy that feeds them data. The next challenge is not a hardware problem but an algorithmic one: partitioning a single training job across these thousands of resources. @Sec-distributed-training-systems explores the parallelism strategies (data, tensor, pipeline, and expert parallelism) that split computation across the fleet.
