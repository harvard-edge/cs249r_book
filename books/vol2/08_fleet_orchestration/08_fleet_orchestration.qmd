...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_memory_capacity, fmt_percent, fmt_qty, fmt_usd
...
\begin{tikzpicture}[
    ...
    conflictpill/.style={
        draw=BrickRed!40,
        fill=BrickRed!10!white,
        rounded corners=2pt,
        line width=0.7pt,
        font=\scriptsize\bfseries\color{BrickRed!90!black},
        inner sep=3.5pt,
        align=center
    }
]

    % Diamond Outer Connectors
    \draw[thick, dashed, BrickRed!40] (0, 3.2) -- (5.2, 0);
    \draw[thick, dashed, BrickRed!40] (5.2, 0) -- (0, -3.2);
    \draw[thick, dashed, BrickRed!40] (0, -3.2) -- (-5.2, 0);
    \draw[thick, dashed, BrickRed!40] (-5.2, 0) -- (0, 3.2);

    % Central Cross Connectors
    \draw[thick, dashed, BrickRed!90!black] (0, 3.2) -- (0, -3.2);
    \draw[thick, dashed, BrickRed!90!black] (-5.2, 0) -- (5.2, 0);

    % Central Card: No Pareto-Optimal Scheduler Exists
    \node[draw=BrickRed, fill=white, rounded corners=4pt, line width=1.2pt, minimum width=3.8cm, minimum height=1.6cm, inner sep=6pt, align=center] (center_card) at (0, 0) {
        {\bfseries\color{BrickRed!90!black} No Pareto-Optimal}\\[2pt]
        {\bfseries\color{BrickRed!90!black} Scheduler Exists}\\[3pt]
        {\scriptsize\color{black!70} Policy encodes trade-offs}
    };

    % Top Card: Throughput
    \node[objcard=NavyBlue] (c_tp) at (0, 3.2) {
        {\bfseries\color{NavyBlue!90!black} Throughput}\\[3pt]
        {\scriptsize Maximize GPU-hours utilized}\\[1pt]
        {\scriptsize\color{NavyBlue!80!black} Pack massive batch jobs}
    };

    % Bottom Card: Latency
    \node[objcard=BurntOrange] (c_lat) at (0, -3.2) {
        {\bfseries\color{BurntOrange!90!black} Latency}\\[3pt]
        {\scriptsize Minimize queue \& start wait time}\\[1pt]
        {\scriptsize\color{BurntOrange!80!black} Fast turnaround for interactive jobs}
    };

    % Left Card: Cost Efficiency
    \node[objcard=BrickRed] (c_cost) at (-5.2, 0) {
        {\bfseries\color{BrickRed!90!black} Cost Efficiency}\\[3pt]
        {\scriptsize Minimize \$/effective GPU-hour}\\[1pt]
        {\scriptsize\color{BrickRed!80!black} Spot instances \& off-peak packing}
    };

    % Right Card: Fairness
    \node[objcard=DarkOrchid] (c_fair) at (5.2, 0) {
        {\bfseries\color{DarkOrchid!90!black} Fairness}\\[3pt]
        {\scriptsize Equitable share per team / user}\\[1pt]
        {\scriptsize\color{DarkOrchid!80!black} DRF / hierarchical fair-share}
    };

    % Conflict Callout Pills
    \node[conflictpill] at (1.8, 1.8) {Starves small jobs};
    \node[conflictpill] at (3.8, -0.95) {Sacrifices peak utilization};
    \node[conflictpill] at (1.6, -1.85) {Fragments large gangs};
    \node[conflictpill] at (-3.8, -1.1) {Higher preemption tax};
...
::: {#fig-fragmentation-problem fig-env="figure" fig-pos="htb" fig-cap="**The fragmentation problem**: The heatmap shows a snapshot of an 8-node cluster (rows) with 8 GPUs each (columns). Different colors represent distinct active jobs, while gray slots represent free GPUs. Although 19 GPUs (roughly 30 percent of capacity) are available globally, they are fragmented across nodes. A new job requiring 8 contiguous GPUs (a full node) cannot be scheduled because no single row is completely empty, illustrating how bin-packing inefficiencies reduce effective cluster capacity." fig-alt="8-by-8 heatmap of cluster GPU allocation. Rows are nodes, columns are GPU slots. Colored cells show jobs; gray shows free. Status panel indicates 19 free GPUs but no contiguous block of 8."}
...
\begin{tikzpicture}[
    ...
    \providecolor{NavyBlue}{RGB}{0,102,204}
    \providecolor{DarkOrchid}{RGB}{153,50,204}
    \providecolor{ForestGreen}{RGB}{34,139,34}
    \providecolor{BrickRed}{RGB}{178,34,34}
    \providecolor{BurntOrange}{RGB}{204,102,0}

    % Vertical dividing line
    \draw[dashed, gray!40, line width=0.8pt] (0, -1.0) -- (0, 5.6);

    % =========================================================================
    % LEFT PANEL: Non-Gang Scheduler (Deadlock)
    % Center: x = -4.1
    % =========================================================================
    \node[font=\bfseries\color{BrickRed!90!black}] at (-4.1, 5.3) {Non-Gang Scheduler (Deadlock)};
    \node[font=\scriptsize\color{black!70}] at (-4.1, 4.9) {8 GPUs total; J1 and J2 each hold 4 and wait for 4};

    % 8 GPUs: G0..G3 (J1) and G4..G7 (J2)
    \foreach \i/\label in {0/G0, 1/G1, 2/G2, 3/G3} {
        \node[gpubox=NavyBlue] (gpu\i) at (-7.0 + \i*0.82, 4.2) {\label};
    }
    \node[font=\tiny\bfseries\color{NavyBlue!90!black}, below=2pt of $(gpu1.south)!0.5!(gpu2.south)$] {J1 holds 4 GPUs};

    \foreach \i/\label in {4/G4, 5/G5, 6/G6, 7/G7} {
        \node[gpubox=DarkOrchid] (gpu\i) at (-7.0 + \i*0.82, 4.2) {\label};
    }
    \node[font=\tiny\bfseries\color{DarkOrchid!90!black}, below=2pt of $(gpu5.south)!0.5!(gpu6.south)$] {J2 holds 4 GPUs};

    % Job J1 and Job J2 Cards with clear separation
    \node[draw=NavyBlue!40, fill=NavyBlue!6!white, rounded corners=3pt, line width=0.8pt, minimum width=2.7cm, minimum height=1.5cm, align=center, inner sep=4pt] (j1_card) at (-6.0, 2.6) {
        {\footnotesize\bfseries\color{NavyBlue!90!black} Job J1 (4/8 GPUs)}\\[2pt]
        {\scriptsize Waiting for 4 more}\\[2pt]
        {\footnotesize\bfseries\color{BrickRed!90!black} Blocked (0\%)}
    };

    \node[draw=DarkOrchid!40, fill=DarkOrchid!6!white, rounded corners=3pt, line width=0.8pt, minimum width=2.7cm, minimum height=1.5cm, align=center, inner sep=4pt] (j2_card) at (-2.2, 2.6) {
        {\footnotesize\bfseries\color{DarkOrchid!90!black} Job J2 (4/8 GPUs)}\\[2pt]
        {\scriptsize Waiting for 4 more}\\[2pt]
        {\footnotesize\bfseries\color{BrickRed!90!black} Blocked (0\%)}
    };

    % Arrows: J1 -> J2 (wants) and J2 -> J1 (wants)
    \draw[-{Stealth[length=4pt, width=3pt]}, thick, BrickRed] ([yshift=7pt]j1_card.east) -- node[above=1pt, font=\tiny\bfseries\color{BrickRed!90!black}] {wants} ([yshift=7pt]j2_card.west);
    \draw[-{Stealth[length=4pt, width=3pt]}, thick, BrickRed] ([yshift=-7pt]j2_card.west) -- node[below=1pt, font=\tiny\bfseries\color{BrickRed!90!black}] {wants} ([yshift=-7pt]j1_card.east);

    % Queue banner
    \node[draw=BurntOrange!40, fill=BurntOrange!8!white, rounded corners=3pt, line width=0.8pt, minimum width=7.2cm, minimum height=0.65cm, inner sep=3pt, font=\scriptsize\bfseries\color{BurntOrange!90!black}] at (-4.1, 1.3) {
        Job J3 in Queue: needs 4 GPUs, 0 available
    };

    % Circular Deadlock box
    \node[draw=BrickRed, fill=white, line width=1.1pt, rounded corners=3pt, minimum width=3.2cm, inner sep=4pt, font=\small\bfseries\color{BrickRed!90!black}] at (-4.1, 0.45) {
        CIRCULAR DEADLOCK
    };

    % Bottom Waste Banner
    \node[draw=BrickRed!40, fill=BrickRed!8!white, rounded corners=3pt, line width=0.8pt, minimum width=7.2cm, inner sep=4pt, align=center] at (-4.1, -0.55) {
        {\scriptsize\bfseries\color{BrickRed!90!black} 8 GPUs 100\% Allocated $\cdot$ 0 Useful Compute}\\[1pt]
        {\tiny\color{BrickRed!80!black} Waste: \$16/hr (\$2/GPU-hr) until timeout or operator intervention}
    };

    % =========================================================================
    % RIGHT PANEL: Gang Scheduler (All-or-Nothing)
    % Center: x = 4.1
    % =========================================================================
    \node[font=\bfseries\color{ForestGreen!90!black}] at (4.1, 5.3) {Gang Scheduler (All-or-Nothing)};
    \node[font=\scriptsize\color{black!70}] at (4.1, 4.9) {Atomic grants prevent hold-and-wait; jobs execute sequentially};

    % Step 1: Allocate J1 Atomically
    \node[stepbox=ForestGreen, minimum height=1.3cm] (step1) at (4.1, 4.0) {
        {\scriptsize\bfseries\color{ForestGreen!90!black} Step 1: Allocate J1 Atomically (8/8 GPUs)}\\[3pt]
        \tikz{
            \foreach \i in {0,...,7} {
                \node[draw=NavyBlue!60, fill=NavyBlue!10, rounded corners=2pt, minimum width=0.68cm, minimum height=0.52cm, inner sep=0pt, font=\tiny\bfseries\color{NavyBlue!90!black}] at (\i*0.75, 0) {J1};
            }
        }
    };

    % Step 2: J2 Queued Cleanly
    \node[stepbox=NavyBlue, minimum height=1.0cm] (step2) at (4.1, 2.5) {
        {\scriptsize\bfseries\color{NavyBlue!90!black} Step 2: J2 Queued Cleanly (Holds 0 GPUs)}\\[2pt]
        {\tiny\color{black!70} J2 waits without occupying partial slots $\cdot$ zero cluster lock}
    };

    % Step 3: J1 Completes; J2 Granted
    \node[stepbox=DarkOrchid, minimum height=1.3cm] (step3) at (4.1, 1.05) {
        {\scriptsize\bfseries\color{DarkOrchid!90!black} Step 3: J1 Completes; J2 Granted 8/8 GPUs}\\[3pt]
        \tikz{
            \foreach \i in {0,...,7} {
                \node[draw=DarkOrchid!60, fill=DarkOrchid!10, rounded corners=2pt, minimum width=0.68cm, minimum height=0.52cm, inner sep=0pt, font=\tiny\bfseries\color{DarkOrchid!90!black}] at (\i*0.75, 0) {J2};
            }
        }
    };

    % Right Summary Banner
    \node[stepbox=ForestGreen, fill=ForestGreen!8!white, minimum height=0.85cm] at (4.1, -0.55) {
        {\scriptsize\bfseries\color{ForestGreen!90!black} Zero Starvation $\cdot$ Sequential Execution}\\[1pt]
        {\tiny\color{ForestGreen!80!black} Both jobs complete in minimum time with 100\% productive GPU cycles}
    };

    % =========================================================================
    % BOTTOM PANEL: Coffman Deadlock Conditions
    % =========================================================================
    \node[draw=gray!40, fill=gray!3, rounded corners=4pt, line width=0.9pt, minimum width=15.6cm, minimum height=3.2cm] (coffman_panel) at (0, -3.1) {};

    \node[font=\small\bfseries] at (0, -1.8) {Resource Dependency Analysis: Coffman Deadlock Conditions};

    % Card 1: Mutual Exclusion
    \node[coffmancard] (c_c1) at (-5.5, -2.8) {
        {\footnotesize\bfseries 1. Mutual Exclusion}\\[2pt]
        {\scriptsize\color{black!70} Exclusive GPU access}\\[1pt]
        {\scriptsize\color{black!70} (no device sharing)}
    };

    % Card 2: Hold & Wait (Broken by Gang)
    \node[coffmancard, draw=ForestGreen, fill=ForestGreen!8!white, line width=1.1pt] (c_c2) at (-1.8, -2.8) {
        {\footnotesize\bfseries\color{ForestGreen!90!black} 2. Hold \& Wait}\\[2pt]
        {\scriptsize\bfseries Jobs hold partial GPUs}\\[1pt]
        {\scriptsize\bfseries\color{ForestGreen!90!black} BROKEN BY GANG}
    };

    % Pill below Card 2
    \node[draw=ForestGreen, fill=ForestGreen, rounded corners=2pt, inner sep=3pt, font=\scriptsize\bfseries\color{white}] (pill_break) at (-1.8, -3.9) {Breaks Hold \& Wait};
    \draw[->, very thick, ForestGreen] (c_c2.south) -- (pill_break.north);

    % Card 3: No Preemption
    \node[coffmancard] (c_c3) at (1.8, -2.8) {
        {\footnotesize\bfseries 3. No Preemption}\\[2pt]
        {\scriptsize\color{black!70} Allocations cannot be}\\[1pt]
        {\scriptsize\color{black!70} forcibly reclaimed}
    };

    % Card 4: Circular Wait
    \node[coffmancard, draw=BrickRed, fill=BrickRed!8!white, line width=1.1pt] (c_c4) at (5.5, -2.8) {
        {\footnotesize\bfseries\color{BrickRed!90!black} 4. Circular Wait}\\[2pt]
        {\scriptsize\color{BrickRed!90!black} J1 waits for J2's GPUs}\\[1pt]
        {\scriptsize\color{BrickRed!90!black} J2 waits for J1's GPUs}
    };

    % Connectors between Coffman cards
    \draw[-{Stealth[length=4pt, width=3pt]}, very thick, gray!60] (c_c1.east) -- (c_c2.west);
    \draw[-{Stealth[length=4pt, width=3pt]}, very thick, gray!60] (c_c2.east) -- (c_c3.west);
    \draw[-{Stealth[length=4pt, width=3pt]}, very thick, gray!60] (c_c3.east) -- (c_c4.west);
...
[^fn-deadlock-scheduling-cycle]: **Hold-and-wait**: One of the four classical conditions jointly sufficient for deadlock in resource-allocation systems. Gang scheduling removes this condition through atomic allocation, at the cost of reduced packing flexibility.
...
\begin{tikzpicture}[
    ...
    % Vertical dividing line
    \draw[dashed, gray!40, line width=0.8pt] (0, -1.95) -- (0, 5.6);

    % =========================================================================
    % LEFT PANEL: Slurm (Imperative HPC)
    % Center: x = -4.0
    % =========================================================================
    \node[font=\bfseries\color{NavyBlue!90!black}] at (-4.0, 5.3) {Slurm (Imperative HPC)};
    \node[font=\scriptsize\color{black!70}] at (-4.0, 4.9) {Exact resource specification $\cdot$ Authoritative start guarantee};

    % Step 1: User Submits Script
    \node[draw=NavyBlue!80!black, fill=NavyBlue!6!white, rounded corners=3pt, line width=1.1pt, minimum width=6.8cm, minimum height=1.05cm, align=center, inner sep=4pt] (s_step1) at (-4.0, 3.9) {
        {\footnotesize\bfseries\color{NavyBlue!90!black} 1. User Submits Script}\\[2pt]
        {\scriptsize\ttfamily\color{NavyBlue!85!black} sbatch -{}-gres=gpu:8 -{}-nodes=4}
    };

    % Step 2: Central Scheduler
    \node[draw=gray!50, fill=gray!4, rounded corners=3pt, line width=0.8pt, minimum width=6.8cm, minimum height=1.05cm, align=center, inner sep=4pt] (s_step2) at (-4.0, 2.45) {
        {\footnotesize\bfseries 2. Central Scheduler (slurmctld)}\\[2pt]
        {\scriptsize\color{black!70} Single authoritative global cluster state}
    };

    % Step 3: Node Execution
    \node[draw=gray!50, fill=gray!4, rounded corners=3pt, line width=0.8pt, minimum width=6.8cm, minimum height=1.05cm, align=center, inner sep=4pt] (s_step3) at (-4.0, 1.0) {
        {\footnotesize\bfseries 3. Node Execution (slurmd)}\\[2pt]
        {\scriptsize\color{black!70} Local execution on dedicated hardware}
    };

    % Connectors Slurm steps
    \draw[-{Stealth[length=5pt, width=3.5pt]}, very thick, NavyBlue!80!black] (s_step1.south) -- (s_step2.north);
    \draw[-{Stealth[length=5pt, width=3.5pt]}, very thick, NavyBlue!80!black] (s_step2.south) -- (s_step3.north);

    % GPUs Row
    \coordinate (s_gpu_center) at (-4.0, -0.35);
    \draw[-{Stealth[length=5pt, width=3.5pt]}, very thick, NavyBlue!80!black] (s_step3.south) -- (-4.0, 0.0);

    \foreach \i/\label in {0/GPU 0, 1/GPU 1, 2/GPU 2, 3/GPU 3} {
        \node[draw=NavyBlue!80!black, fill=NavyBlue!8!white, rounded corners=3pt, line width=0.9pt, minimum width=1.5cm, minimum height=0.68cm, font=\scriptsize\bfseries\color{NavyBlue!90!black}, inner sep=0pt] (s_gpu\i) at (-6.25 + \i*1.5, -0.35) {\label};
    }

    % Summary Banner Slurm
    \node[draw=NavyBlue!40, fill=NavyBlue!6!white, rounded corners=3pt, line width=0.8pt, minimum width=7.2cm, minimum height=0.85cm, align=center, inner sep=4pt] at (-4.0, -1.4) {
        {\scriptsize\bfseries\color{NavyBlue!90!black} Atomic Guarantee: 100\% Granted or Stays in Queue}\\[2pt]
        {\tiny\color{NavyBlue!80!black} Zero container virtualization overhead $\cdot$ Ideal for multi-node MPI/NCCL}
    };

    % =========================================================================
    % RIGHT PANEL: Kubernetes (Declarative Cloud)
    % Center: x = 4.0
    % =========================================================================
    \node[font=\bfseries\color{ForestGreen!90!black}] at (4.0, 5.3) {Kubernetes (Declarative Cloud)};
    \node[font=\scriptsize\color{black!70}] at (4.0, 4.9) {Desired state declaration $\cdot$ Continuous reconciliation loop};

    % Step 1: User Declares State
    \node[draw=ForestGreen!80!black, fill=ForestGreen!6!white, rounded corners=3pt, line width=1.1pt, minimum width=6.8cm, minimum height=1.05cm, align=center, inner sep=4pt] (k_step1) at (4.0, 3.9) {
        {\footnotesize\bfseries\color{ForestGreen!90!black} 1. User Declares State}\\[2pt]
        {\scriptsize\ttfamily\color{ForestGreen!85!black} kubectl apply -f deployment.yaml}
    };

    % Step 2: API Server & etcd
    \node[draw=gray!50, fill=gray!4, rounded corners=3pt, line width=0.8pt, minimum width=6.8cm, minimum height=1.05cm, align=center, inner sep=4pt] (k_step2) at (4.0, 2.45) {
        {\footnotesize\bfseries 2. API Server \& etcd}\\[2pt]
        {\scriptsize\color{black!70} Persists target spec in distributed store}
    };

    % Step 3: Controller Manager Loop
    \node[draw=gray!50, fill=gray!4, rounded corners=3pt, line width=0.8pt, minimum width=6.8cm, minimum height=1.05cm, align=center, inner sep=4pt] (k_step3) at (4.0, 1.0) {
        {\footnotesize\bfseries 3. Controller Manager Loop}\\[2pt]
        {\scriptsize\color{black!70} Continuous loop: actual $\rightarrow$ desired state}
    };

    % Connectors Kubernetes steps
    \draw[-{Stealth[length=5pt, width=3.5pt]}, very thick, ForestGreen!80!black] (k_step1.south) -- (k_step2.north);
    \draw[-{Stealth[length=5pt, width=3.5pt]}, very thick, ForestGreen!80!black] (k_step2.south) -- (k_step3.north);

    % Pods Row
    \coordinate (k_pod_center) at (4.0, -0.35);
    \draw[-{Stealth[length=5pt, width=3.5pt]}, very thick, ForestGreen!80!black] (k_step3.south) -- (4.0, 0.0);

    \foreach \i/\label in {0/Pod 0, 1/Pod 1, 2/Pod 2, 3/Pod 3} {
        \node[draw=ForestGreen!80!black, fill=ForestGreen!8!white, rounded corners=3pt, line width=0.9pt, minimum width=1.5cm, minimum height=0.68cm, font=\scriptsize\bfseries\color{ForestGreen!90!black}, inner sep=0pt] (k_pod\i) at (1.75 + \i*1.5, -0.35) {\label};
    }

    % Summary Banner Kubernetes
    \node[draw=ForestGreen!40, fill=ForestGreen!6!white, rounded corners=3pt, line width=0.8pt, minimum width=7.2cm, minimum height=0.85cm, align=center, inner sep=4pt] at (4.0, -1.4) {
        {\scriptsize\bfseries\color{ForestGreen!90!black} Self-Healing: Autonomous Node \& Pod Replacement}\\[2pt]
        {\tiny\color{ForestGreen!80!black} Dynamic scaling \& rolling upgrades $\cdot$ Ideal for inference \& microservices}
    };

    % =========================================================================
    % BOTTOM PANEL: Architectural Convergence
    % =========================================================================
    \node[draw=gray!40, fill=gray!3, rounded corners=4pt, line width=0.9pt, minimum width=15.6cm, minimum height=1.45cm] (bottom_panel) at (0, -2.7) {};

    \node[font=\small\bfseries] at (0, -2.25) {Architectural Convergence: Complementary Roles in Production Fleets};
    \draw[gray!30, line width=0.6pt] (-7.5, -2.5) -- (7.5, -2.5);
    \draw[gray!30, line width=0.6pt] (0, -2.55) -- (0, -3.3);

    \node[align=center] at (-3.8, -2.9) {
        {\scriptsize\bfseries\color{NavyBlue!90!black} Slurm: Dedicated large-scale batch pretraining}\\[1.5pt]
        {\tiny\color{black!70} Deterministic topology placement \& bare-metal performance}
    };

    \node[align=center] at (3.8, -2.9) {
        {\scriptsize\bfseries\color{ForestGreen!90!black} Kubernetes: Inference serving \& pipeline lifecycle}\\[1.5pt]
        {\tiny\color{black!70} Dynamic elasticity, HPA, and multi-tenant service orchestration}
    };
...
The scheduler must therefore model the cluster not as a flat list of compute slots, but as a hierarchical graph with distinct bandwidth cliffs at each level (GPU, Node, Rack, and Pod) corresponding to NVLink, InfiniBand, and spine-switch boundaries (@fig-fleet-orchestration-topology-hierarchy). Each boundary crossing introduces an order-of-magnitude bandwidth reduction, and at the rack and pod levels, network oversubscription (often 2:1 or 4:1 at the spine switches) further restricts bisection bandwidth beyond what the raw link speed suggests. A topology-aware scheduler explicitly optimizes for these constraints, treating "locality" as a first-class resource. For a distributed training job requiring 1,024 GPUs, a fragmented placement that scatters workers across random racks can degrade end-to-end training throughput by 15 to 30 percent compared to a compact placement within a single pod.

::: {#fig-fleet-orchestration-topology-hierarchy fig-env="figure" fig-pos="htb" fig-cap="**Physical topology hierarchy and bandwidth cliffs**: Hardware boundaries dictate communication performance across three distinct domains. Within a node, NVLink provides 900 GB/s bidirectional bandwidth for tensor-parallel communication. Within a rack, InfiniBand/RoCE links deliver 50 GB/s for pipeline and data parallelism. Across racks and pods, spine switch fabrics drop effective bandwidth to 12–25 GB/s with 2:1 to 4:1 oversubscription." fig-alt="Three-tier physical hierarchy showing Node domain (NVLink at 900 GB/s), Rack domain (InfiniBand/RoCE at 50 GB/s), and Pod domain (Spine switch fabric at 12 to 25 GB/s) with order-of-magnitude bandwidth cliffs."}
...
\begin{tikzpicture}[
    ...
    % =========================================================================
    % LEFT PANEL: Nested Topology Domains
    % =========================================================================

    % Outer: Pod / Fabric Domain
    \node[draw=BurntOrange, fill=BurntOrange!4!white, rounded corners=6pt, line width=1.3pt,
          minimum width=10.6cm, minimum height=8.4cm, anchor=south west] (pod_box) at (-8.0, -4.2) {};

    \node[draw=BurntOrange, fill=BurntOrange, rounded corners=3pt, inner sep=4pt, text=white, font=\footnotesize\bfseries, anchor=north west] (pod_badge) at (-7.7, 3.9) {Pod / Fabric Domain};
    \node[font=\scriptsize\bfseries\color{BurntOrange!80!black}, anchor=east] at (2.4, 3.68) {Cross-rack Spine: $\sim$25 GB/s (2:1 oversubscribed)};

    % Middle: Rack / Leaf Switch Domain
    \node[draw=DarkOrchid, fill=DarkOrchid!3!white, rounded corners=5pt, line width=1.2pt,
          minimum width=10.0cm, minimum height=6.8cm, anchor=south west] (rack_box) at (-7.7, -3.8) {};

    \node[draw=DarkOrchid, fill=DarkOrchid, rounded corners=3pt, inner sep=4pt, text=white, font=\footnotesize\bfseries, anchor=north west] (rack_badge) at (-7.4, 2.7) {Rack / Leaf Domain};
    \node[font=\scriptsize\bfseries\color{DarkOrchid!80!black}, anchor=east] at (2.1, 2.48) {InfiniBand NDR: 50 GB/s per port};

    % Inner: Node (Single Server Chassis)
    \node[draw=NavyBlue!80!black, fill=white, rounded corners=4pt, line width=1.2pt,
          minimum width=9.4cm, minimum height=5.2cm, anchor=south west] (node_box) at (-7.4, -3.4) {};

    \node[draw=NavyBlue!90!black, fill=NavyBlue!90!black, rounded corners=3pt, inner sep=4pt, text=white, font=\footnotesize\bfseries, anchor=north west] (node_badge) at (-7.1, 1.5) {Node (Single Server Chassis)};
    \node[font=\scriptsize\bfseries\color{NavyBlue!90!black}, anchor=east] at (1.8, 1.28) {NVLink 4: 900 GB/s bidirectional};

    % 8 GPUs inside Node
    \foreach \i in {0,...,7} {
        \node[draw=NavyBlue!80!black, fill=NavyBlue!6!white, rounded corners=2pt, line width=0.8pt,
              minimum width=0.96cm, minimum height=1.15cm, align=center, inner sep=1pt] (gpu\i) at (-6.375 + \i*1.05, 0.25) {
            {\scriptsize\bfseries GPU \i}\\[3pt]
            {\tiny\color{black!70} 80 GB}
        };
    }

    % NVSwitch Crossbar Box
    \node[draw=NavyBlue!80!black, fill=NavyBlue!4!white, rounded corners=2pt, line width=0.8pt,
          minimum width=8.6cm, minimum height=1.05cm, align=center, inner sep=3pt] (nvswitch) at (-2.7, -1.35) {
        {\footnotesize\bfseries NVSwitch Intra-Node Crossbar}\\[2pt]
        {\scriptsize\color{black!80} Full mesh all-to-all: 900 GB/s per GPU $\cdot$ zero OS/PCIe overhead}
    };

    % Connectors from each GPU to NVSwitch
    \foreach \i in {0,...,7} {
        \draw[thick, NavyBlue!80!black] (gpu\i.south) -- (gpu\i.south |- nvswitch.north);
    }

    % =========================================================================
    % RIGHT PANEL: 3 Tiers & Bandwidth Drops
    % Center: x = 5.4
    % =========================================================================

    % Tier 1 Card (Intra-Node)
    \node[draw=NavyBlue!50, fill=NavyBlue!5!white, rounded corners=4pt, line width=0.9pt,
          minimum width=4.0cm, minimum height=1.75cm, align=center, inner sep=4pt] (t1_card) at (5.4, 3.1) {
        {\footnotesize\bfseries\color{NavyBlue!90!black} Tier 1: Intra-Node}\\[2pt]
        {\Large\bfseries\color{NavyBlue!90!black} 900 GB/s}\\[2pt]
        {\scriptsize\color{black!80} NVLink Mesh (NVSwitch)}\\[1pt]
        {\tiny\color{black!70} Latency: $< 1\,\mu$s}\\[3pt]
        \colorbox{NavyBlue!12}{\strut\tiny\bfseries\color{NavyBlue!90!black}\;Tensor Parallelism (TP)\;}
    };

    % Tier 2 Card (Intra-Rack)
    \node[draw=DarkOrchid!50, fill=DarkOrchid!5!white, rounded corners=4pt, line width=0.9pt,
          minimum width=4.0cm, minimum height=1.75cm, align=center, inner sep=4pt] (t2_card) at (5.4, 0.0) {
        {\footnotesize\bfseries\color{DarkOrchid!90!black} Tier 2: Intra-Rack}\\[2pt]
        {\Large\bfseries\color{DarkOrchid!90!black} 50 GB/s}\\[2pt]
        {\scriptsize\color{black!80} InfiniBand NDR Leaf}\\[1pt]
        {\tiny\color{black!70} Latency: 2--5\,$\mu$s}\\[3pt]
        \colorbox{DarkOrchid!12}{\strut\tiny\bfseries\color{DarkOrchid!90!black}\;Pipeline Parallelism (PP)\;}
    };

    % Drop 1 (18x Drop)
    \draw[-{Stealth[length=4pt, width=3pt]}, dashed, line width=1.1pt, BrickRed] (t1_card.south) -- (t2_card.north);
    \node[draw=BrickRed!40, fill=BrickRed!10, rounded corners=2pt, inner sep=2pt, font=\tiny\bfseries\color{BrickRed!90!black}, anchor=west] at ($(t1_card.south)!0.5!(t2_card.north) + (0.15, 0)$) {18$\times$ Drop};

    % Tier 3 Card (Inter-Rack)
    \node[draw=BurntOrange!50, fill=BurntOrange!5!white, rounded corners=4pt, line width=0.9pt,
          minimum width=4.0cm, minimum height=1.75cm, align=center, inner sep=4pt] (t3_card) at (5.4, -3.1) {
        {\footnotesize\bfseries\color{BurntOrange!90!black} Tier 3: Inter-Rack}\\[2pt]
        {\Large\bfseries\color{BurntOrange!90!black} $\sim$25 GB/s}\\[2pt]
        {\scriptsize\color{black!80} Spine Switch Fabric (2:1)}\\[1pt]
        {\tiny\color{black!70} Latency: 10--30\,$\mu$s (tail-heavy)}\\[3pt]
        \colorbox{BurntOrange!15}{\strut\tiny\bfseries\color{BurntOrange!90!black}\;Data Parallelism (DP)\;}
    };

    % Drop 2 (2x Drop (36x))
    \draw[-{Stealth[length=4pt, width=3pt]}, dashed, line width=1.1pt, BrickRed] (t2_card.south) -- (t3_card.north);
    \node[draw=BrickRed!40, fill=BrickRed!10, rounded corners=2pt, inner sep=2pt, font=\tiny\bfseries\color{BrickRed!90!black}, anchor=west] at ($(t2_card.south)!0.5!(t3_card.north) + (0.15, 0)$) {2$\times$ Drop (36$\times$)};
...
\begin{tikzpicture}[
    ...
    % =========================================================================
    % LEFT PANEL: Topology-Unaware (Scattered Placement)
    % Center: x = -4.0
    % =========================================================================
    \node[font=\bfseries\color{BrickRed!90!black}] at (-4.0, 5.3) {Topology-Unaware (Scattered Placement)};
    \node[font=\scriptsize\color{black!70}] at (-4.0, 4.95) {4-GPU tensor group split across two racks via spine switch};

    % Spine Switch Box (Traversed)
    \node[draw=BrickRed, fill=BrickRed!8!white, rounded corners=3pt, line width=1.1pt,
          minimum width=3.4cm, minimum height=0.72cm, font=\footnotesize\bfseries\color{BrickRed!90!black}] (l_spine) at (-4.0, 4.15) {Spine Switch (Traversed)};
    \node[draw=white, fill=white, rounded corners=2pt, inner sep=1.5pt, font=\tiny\bfseries\color{BrickRed!90!black}] at (-4.0, 3.55) {3 Hops via Spine};

    % Rack A (Left)
    \node[draw=gray!40, fill=gray!3, rounded corners=4pt, line width=0.8pt, minimum width=3.1cm, minimum height=3.5cm] (rack_la) at (-5.7, 1.55) {};
    \node[font=\scriptsize\bfseries\color{black!80}, anchor=north west] at (-7.15, 3.2) {Rack A};
    \node[draw=gray!50, fill=white, rounded corners=2pt, minimum width=2.5cm, minimum height=0.52cm, font=\tiny\bfseries\color{black!70}] (leaf_la) at (-5.7, 2.45) {Leaf Switch A};

    % Rack A GPUs
    \node[draw=NavyBlue!80!black, fill=NavyBlue!8!white, rounded corners=2pt, line width=0.8pt, minimum width=1.15cm, minimum height=0.7cm, align=center] (g0) at (-6.35, 1.55) {{\scriptsize\bfseries G0}\\[-1pt]{\tiny\color{NavyBlue!80!black}Job}};
    \node[draw=NavyBlue!80!black, fill=NavyBlue!8!white, rounded corners=2pt, line width=0.8pt, minimum width=1.15cm, minimum height=0.7cm, align=center] (g1) at (-5.05, 1.55) {{\scriptsize\bfseries G1}\\[-1pt]{\tiny\color{NavyBlue!80!black}Job}};
    \node[draw=gray!30, fill=gray!8, rounded corners=2pt, minimum width=1.15cm, minimum height=0.7cm, align=center] at (-6.35, 0.6) {{\scriptsize\color{black!40}busy}};
    \node[draw=gray!30, fill=gray!8, rounded corners=2pt, minimum width=1.15cm, minimum height=0.7cm, align=center] at (-5.05, 0.6) {{\scriptsize\color{black!40}busy}};

    % Rack B (Left)
    \node[draw=gray!40, fill=gray!3, rounded corners=4pt, line width=0.8pt, minimum width=3.1cm, minimum height=3.5cm] (rack_lb) at (-2.3, 1.55) {};
    \node[font=\scriptsize\bfseries\color{black!80}, anchor=north west] at (-3.75, 3.2) {Rack B};
    \node[draw=gray!50, fill=white, rounded corners=2pt, minimum width=2.5cm, minimum height=0.52cm, font=\tiny\bfseries\color{black!70}] (leaf_lb) at (-2.3, 2.45) {Leaf Switch B};

    % Rack B GPUs
    \node[draw=NavyBlue!80!black, fill=NavyBlue!8!white, rounded corners=2pt, line width=0.8pt, minimum width=1.15cm, minimum height=0.7cm, align=center] (g2) at (-2.95, 1.55) {{\scriptsize\bfseries G2}\\[-1pt]{\tiny\color{NavyBlue!80!black}Job}};
    \node[draw=NavyBlue!80!black, fill=NavyBlue!8!white, rounded corners=2pt, line width=0.8pt, minimum width=1.15cm, minimum height=0.7cm, align=center] (g3) at (-1.65, 1.55) {{\scriptsize\bfseries G3}\\[-1pt]{\tiny\color{NavyBlue!80!black}Job}};
    \node[draw=gray!30, fill=gray!8, rounded corners=2pt, minimum width=1.15cm, minimum height=0.7cm, align=center] at (-2.95, 0.6) {{\scriptsize\color{black!40}busy}};
    \node[draw=gray!30, fill=gray!8, rounded corners=2pt, minimum width=1.15cm, minimum height=0.7cm, align=center] at (-1.65, 0.6) {{\scriptsize\color{black!40}busy}};

    % Paths on Left (Dashed Red)
    \draw[-{Stealth[length=4pt, width=3pt]}, thick, dashed, BrickRed] ([xshift=-0.8cm]l_spine.south) -- (leaf_la.north);
    \draw[-{Stealth[length=4pt, width=3pt]}, thick, dashed, BrickRed] (leaf_la.south -| g0.north) -- (g0.north);
    \draw[-{Stealth[length=4pt, width=3pt]}, thick, dashed, BrickRed] ([xshift=0.8cm]l_spine.south) -- (leaf_lb.north);
    \draw[-{Stealth[length=4pt, width=3pt]}, thick, dashed, BrickRed] (leaf_lb.south -| g2.north) -- (g2.north);

    % Summary Banner Left
    \node[draw=BrickRed!40, fill=BrickRed!8!white, rounded corners=3pt, line width=0.8pt, minimum width=7.2cm, minimum height=0.85cm, align=center, inner sep=4pt] at (-4.0, -0.65) {
        {\scriptsize\bfseries\color{BrickRed!90!black} Effective BW: $\sim$12 GB/s (Spine Congestion)}\\[1.5pt]
        {\tiny\color{BrickRed!80!black} 3 switch hops per AllReduce $\cdot$ High tail latency}
    };

    % =========================================================================
    % RIGHT PANEL: Topology-Aware (Packed Placement)
    % Center: x = 4.0
    % =========================================================================
    \node[font=\bfseries\color{ForestGreen!90!black}] at (4.0, 5.3) {Topology-Aware (Packed Placement)};
    \node[font=\scriptsize\color{black!70}] at (4.0, 4.95) {4-GPU tensor group co-located within a single rack under Leaf-A};

    % Spine Switch Box (Bypassed)
    \node[draw=gray!40, fill=gray!2, dashed, rounded corners=3pt, line width=0.8pt,
          minimum width=3.4cm, minimum height=0.72cm, font=\footnotesize\color{black!50}] (r_spine) at (4.0, 4.15) {Spine Switch (Bypassed)};

    % Rack A (Right - Highlighted)
    \node[draw=ForestGreen, fill=ForestGreen!4!white, rounded corners=4pt, line width=1.1pt, minimum width=3.1cm, minimum height=3.5cm] (rack_ra) at (2.3, 1.55) {};
    \node[font=\scriptsize\bfseries\color{ForestGreen!90!black}, anchor=north west] at (0.85, 3.2) {Rack A};
    \node[draw=ForestGreen, fill=white, rounded corners=2pt, line width=0.9pt, minimum width=2.5cm, minimum height=0.52cm, font=\tiny\bfseries\color{ForestGreen!90!black}] (leaf_ra) at (2.3, 2.45) {Leaf Switch A (1-Hop)};

    % Bypassed Spine connection
    \draw[dashed, gray!40, line width=0.7pt] (leaf_ra.north) -- ([xshift=-0.8cm]r_spine.south);

    % Rack A GPUs (All 4 in Job)
    \node[draw=NavyBlue!80!black, fill=NavyBlue!8!white, rounded corners=2pt, line width=0.8pt, minimum width=1.15cm, minimum height=0.7cm, align=center] (rg0) at (1.65, 1.55) {{\scriptsize\bfseries G0}\\[-1pt]{\tiny\color{NavyBlue!80!black}Job}};
    \node[draw=NavyBlue!80!black, fill=NavyBlue!8!white, rounded corners=2pt, line width=0.8pt, minimum width=1.15cm, minimum height=0.7cm, align=center] (rg1) at (2.95, 1.55) {{\scriptsize\bfseries G1}\\[-1pt]{\tiny\color{NavyBlue!80!black}Job}};
    \node[draw=NavyBlue!80!black, fill=NavyBlue!8!white, rounded corners=2pt, line width=0.8pt, minimum width=1.15cm, minimum height=0.7cm, align=center] (rg2) at (1.65, 0.6) {{\scriptsize\bfseries G2}\\[-1pt]{\tiny\color{NavyBlue!80!black}Job}};
    \node[draw=NavyBlue!80!black, fill=NavyBlue!8!white, rounded corners=2pt, line width=0.8pt, minimum width=1.15cm, minimum height=0.7cm, align=center] (rg3) at (2.95, 0.6) {{\scriptsize\bfseries G3}\\[-1pt]{\tiny\color{NavyBlue!80!black}Job}};

    % Local Arrows inside Rack A
    \draw[-{Stealth[length=3.5pt, width=2.5pt]}, thick, ForestGreen] (rg0.east) -- (rg1.west);
    \draw[-{Stealth[length=3.5pt, width=2.5pt]}, thick, ForestGreen] (rg0.south) -- (rg2.north);
    \draw[-{Stealth[length=3.5pt, width=2.5pt]}, thick, ForestGreen] (rg1.south) -- (rg3.north);

    % Rack B (Right - Grayed Out)
    \node[draw=gray!30, fill=gray!2, rounded corners=4pt, line width=0.7pt, minimum width=3.1cm, minimum height=3.5cm] (rack_rb) at (5.7, 1.55) {};
    \node[font=\scriptsize\bfseries\color{black!50}, anchor=north west] at (4.25, 3.2) {Rack B};
    \node[draw=gray!40, fill=white, rounded corners=2pt, minimum width=2.5cm, minimum height=0.52cm, font=\tiny\color{black!40}] at (5.7, 2.45) {Leaf Switch B};

    % Rack B GPUs (All other)
    \node[draw=gray!30, fill=gray!6, rounded corners=2pt, minimum width=1.15cm, minimum height=0.7cm, align=center] at (5.05, 1.55) {{\scriptsize\color{black!40}other}};
    \node[draw=gray!30, fill=gray!6, rounded corners=2pt, minimum width=1.15cm, minimum height=0.7cm, align=center] at (6.35, 1.55) {{\scriptsize\color{black!40}other}};
    \node[draw=gray!30, fill=gray!6, rounded corners=2pt, minimum width=1.15cm, minimum height=0.7cm, align=center] at (5.05, 0.6) {{\scriptsize\color{black!40}other}};
    \node[draw=gray!30, fill=gray!6, rounded corners=2pt, minimum width=1.15cm, minimum height=0.7cm, align=center] at (6.35, 0.6) {{\scriptsize\color{black!40}other}};

    % Summary Banner Right
    \node[draw=ForestGreen!40, fill=ForestGreen!6!white, rounded corners=3pt, line width=0.8pt, minimum width=7.2cm, minimum height=0.85cm, align=center, inner sep=4pt] at (4.0, -0.65) {
        {\scriptsize\bfseries\color{ForestGreen!90!black} Effective BW: $\sim$50 GB/s (4$\times$ Speedup)}\\[1.5pt]
        {\tiny\color{ForestGreen!80!black} 1 switch hop (leaf-local) $\cdot$ Zero spine link contention}
    };

    % =========================================================================
    % BOTTOM PANEL: Quantitative Summary Table
    % =========================================================================
    \node[draw=gray!40, fill=gray!3, rounded corners=4pt, line width=0.9pt, minimum width=15.6cm, minimum height=2.0cm] (table_panel) at (0, -2.15) {};

    \node[font=\small\bfseries] at (0, -1.35) {Quantitative Summary: Switch Hops vs. Collective Latency};
    \draw[gray!30, line width=0.6pt] (-7.5, -1.55) -- (7.5, -1.55);

    % Table Headers
    \node[font=\scriptsize\bfseries\color{black!80}] at (-5.0, -1.78) {Placement Strategy};
    \node[font=\scriptsize\bfseries\color{black!80}] at (-0.8, -1.78) {Switch Traversal};
    \node[font=\scriptsize\bfseries\color{black!80}] at (3.2, -1.78) {Effective Bandwidth};
    \node[font=\scriptsize\bfseries\color{black!80}] at (6.2, -1.78) {Relative Speed};
    \draw[gray!20, line width=0.5pt] (-7.3, -1.95) -- (7.3, -1.95);

    % Row 1: Topology-Unaware
    \node[font=\scriptsize\bfseries\color{BrickRed!90!black}] at (-5.0, -2.2) {Topology-Unaware (Scattered)};
    \node[font=\scriptsize\color{BrickRed!90!black}] at (-0.8, -2.2) {3 hops (Leaf $\rightarrow$ Spine $\rightarrow$ Leaf)};
    \node[font=\scriptsize\color{BrickRed!90!black}] at (3.2, -2.2) {$\sim$12 GB/s per rank};
    \node[font=\scriptsize\bfseries\color{BrickRed!90!black}] at (6.2, -2.2) {1.0$\times$};

    % Row 2: Topology-Aware
    \node[font=\scriptsize\bfseries\color{ForestGreen!90!black}] at (-5.0, -2.6) {Topology-Aware (Packed)};
    \node[font=\scriptsize\bfseries\color{ForestGreen!90!black}] at (-0.8, -2.6) {1 hop (Local Leaf only)};
    \node[font=\scriptsize\bfseries\color{ForestGreen!90!black}] at (3.2, -2.6) {$\sim$50 GB/s per rank};
    \node[font=\scriptsize\bfseries\color{ForestGreen!90!black}] at (6.2, -2.6) {4.2$\times$ Faster};
...
Production schedulers resolve these conflicts using weighted scoring functions that prioritize constraints based on workload characteristics (@fig-fleet-orchestration-hierarchical-placement). For large training jobs using 3D parallelism, tensor parallel locality receives the highest weight (because NVLink-to-InfiniBand performance degradation is the largest), followed by rail alignment (because it eliminates congestion for the most frequent communication pattern), followed by rack locality (because the performance benefit is smaller and can be partially compensated by compute-communication overlap). For pure data parallel jobs, rail alignment receives the highest weight. For single-node jobs, topology constraints are irrelevant and the scheduler optimizes purely for packing efficiency.

::: {#fig-fleet-orchestration-hierarchical-placement fig-env="figure" fig-pos="htb" fig-cap="**Hierarchical placement for 3D parallelism**: Optimal placement maps distributed training dimensions to matching physical interconnect layers. Tensor parallelism (TP) maps strictly inside nodes across NVLink (900 GB/s); pipeline parallelism (PP) maps across adjacent nodes within a rack (50 GB/s); and data parallelism (DP) spans across racks via rail-aligned spine fabrics." fig-alt="Architectural diagram mapping 3D parallelism dimensions to hardware domains: TP mapped to intra-node NVLink, PP mapped to intra-rack InfiniBand, and DP mapped across racks with rail-aligned network switches."}
...
\begin{tikzpicture}[
    ...
    % Column Headers
    \node[font=\footnotesize\bfseries\color{black!80}] at (-4.5, 3.8) {Parallelism Strategy};
    \node[font=\footnotesize\bfseries\color{black!50}] at (0.0, 3.8) {Interconnect Requirement};
    \node[font=\footnotesize\bfseries\color{black!80}] at (4.5, 3.8) {Target Hardware Domain};

    % =========================================================================
    % ROW 1: Tensor Parallelism (TP)
    % =========================================================================
    \node[draw=NavyBlue!40, fill=NavyBlue!4!white, rounded corners=4pt, line width=1.0pt,
          minimum width=6.2cm, minimum height=2.2cm, align=left, inner sep=7pt] (c1_l) at (-4.5, 2.3) {
        {\footnotesize\bfseries\color{NavyBlue!90!black} Tensor Parallelism (TP)}\\[3pt]
        {\scriptsize Splits matrix ops (GEMM) across ranks}\\[1pt]
        {\scriptsize Exchanges activations at every layer}\\[5pt]
        \colorbox{NavyBlue!12}{\strut\tiny\bfseries\color{NavyBlue!90!black}\;Highest Bandwidth Need\;}
    };

    \draw[-{Stealth[length=5pt, width=3.5pt]}, line width=1.3pt, NavyBlue!80!black] (-1.25, 2.3) --
        node[above=2pt, draw=NavyBlue!20, fill=NavyBlue!8!white, rounded corners=2pt, inner sep=2.5pt, font=\tiny\bfseries\color{NavyBlue!90!black}] {900 GB/s NVLink} (1.25, 2.3);

    \node[draw=NavyBlue!80!black, fill=NavyBlue!3!white, rounded corners=4pt, line width=1.0pt,
          minimum width=6.2cm, minimum height=2.2cm, align=left, inner sep=7pt] (c1_r) at (4.5, 2.3) {
        {\footnotesize\bfseries\color{NavyBlue!90!black} Single-Node NVLink Mesh}\\[3pt]
        {\scriptsize Confined strictly within 1 chassis (8 GPUs)}\\[1pt]
        {\scriptsize NVSwitch crossbar $\cdot$ latency $< 1\,\mu$s}\\[16pt]
        \mbox{}
    };

    \foreach \i in {0,...,7} {
        \node[fill=NavyBlue!90!black, text=white, rounded corners=1.5pt, minimum width=0.58cm, minimum height=0.46cm, font=\tiny\bfseries, inner sep=0pt] at (2.05 + \i*0.7, 1.55) {G\i};
    }

    % =========================================================================
    % ROW 2: Pipeline Parallelism (PP)
    % =========================================================================
    \node[draw=DarkOrchid!40, fill=DarkOrchid!4!white, rounded corners=4pt, line width=1.0pt,
          minimum width=6.2cm, minimum height=2.2cm, align=left, inner sep=7pt] (c2_l) at (-4.5, -0.2) {
        {\footnotesize\bfseries\color{DarkOrchid!90!black} Pipeline Parallelism (PP)}\\[3pt]
        {\scriptsize Partitions layers across pipeline stages}\\[1pt]
        {\scriptsize Sends activations at stage boundaries}\\[5pt]
        \colorbox{DarkOrchid!12}{\strut\tiny\bfseries\color{DarkOrchid!90!black}\;Moderate Bandwidth Need\;}
    };

    \draw[-{Stealth[length=5pt, width=3.5pt]}, line width=1.3pt, DarkOrchid!80!black] (-1.25, -0.2) --
        node[above=2pt, draw=DarkOrchid!20, fill=DarkOrchid!8!white, rounded corners=2pt, inner sep=2.5pt, font=\tiny\bfseries\color{DarkOrchid!90!black}] {50 GB/s IB NDR} (1.25, -0.2);

    \node[draw=DarkOrchid!80!black, fill=DarkOrchid!3!white, rounded corners=4pt, line width=1.0pt,
          minimum width=6.2cm, minimum height=2.2cm, align=left, inner sep=7pt] (c2_r) at (4.5, -0.2) {
        {\footnotesize\bfseries\color{DarkOrchid!90!black} Intra-Rack Leaf Domain}\\[3pt]
        {\scriptsize Spans adjacent nodes under same leaf switch}\\[1pt]
        {\scriptsize 1-hop leaf fabric $\cdot$ latency 2--5\,$\mu$s}\\[16pt]
        \mbox{}
    };

    \node[fill=DarkOrchid!90!black, text=white, rounded corners=2pt, inner sep=3pt, font=\tiny\bfseries] at (2.4, -0.95) {Stage 0 (Node A)};
    \node[fill=DarkOrchid!90!black, text=white, rounded corners=2pt, inner sep=3pt, font=\tiny\bfseries] at (4.5, -0.95) {Stage 1 (Node B)};
    \node[fill=DarkOrchid!90!black, text=white, rounded corners=2pt, inner sep=3pt, font=\tiny\bfseries] at (6.6, -0.95) {Stage 2 (Node C)};

    % =========================================================================
    % ROW 3: Data Parallelism (DP / ZeRO)
    % =========================================================================
    \node[draw=BurntOrange!40, fill=BurntOrange!4!white, rounded corners=4pt, line width=1.0pt,
          minimum width=6.2cm, minimum height=2.2cm, align=left, inner sep=7pt] (c3_l) at (-4.5, -2.7) {
        {\footnotesize\bfseries\color{BurntOrange!90!black} Data Parallelism (DP / ZeRO)}\\[3pt]
        {\scriptsize Replicates model; splits data batch}\\[1pt]
        {\scriptsize AllReduce gradients once per step (pipelined)}\\[5pt]
        \colorbox{BurntOrange!15}{\strut\tiny\bfseries\color{BurntOrange!90!black}\;Latency-Tolerant (Overlap)\;}
    };

    \draw[-{Stealth[length=5pt, width=3.5pt]}, line width=1.3pt, BurntOrange!80!black] (-1.25, -2.7) --
        node[above=2pt, draw=BurntOrange!20, fill=BurntOrange!8!white, rounded corners=2pt, inner sep=2.5pt, font=\tiny\bfseries\color{BurntOrange!90!black}] {$\sim$25 GB/s Spine} (1.25, -2.7);

    \node[draw=BurntOrange!80!black, fill=BurntOrange!3!white, rounded corners=4pt, line width=1.0pt,
          minimum width=6.2cm, minimum height=2.2cm, align=left, inner sep=7pt] (c3_r) at (4.5, -2.7) {
        {\footnotesize\bfseries\color{BurntOrange!90!black} Cross-Rack Pod Fabric}\\[3pt]
        {\scriptsize Scales across hundreds of racks via spines}\\[1pt]
        {\scriptsize Bucketed AllReduce overlaps backward pass}\\[16pt]
        \mbox{}
    };

    \node[fill=BurntOrange!90!black, text=white, rounded corners=2pt, minimum width=1.2cm, inner sep=2.5pt, font=\tiny\bfseries, align=center] at (2.3, -3.45) {Rack 1};
    \node[fill=BurntOrange!90!black, text=white, rounded corners=2pt, minimum width=1.2cm, inner sep=2.5pt, font=\tiny\bfseries, align=center] at (3.8, -3.45) {Rack 2};
    \node[fill=BurntOrange!90!black, text=white, rounded corners=2pt, minimum width=0.8cm, inner sep=2.5pt, font=\tiny\bfseries, align=center] at (5.05, -3.45) {$\dots$};
    \node[fill=BurntOrange!90!black, text=white, rounded corners=2pt, minimum width=1.2cm, inner sep=2.5pt, font=\tiny\bfseries, align=center] at (6.4, -3.45) {Rack N};
...
Elastic scheduling requires a well-defined contract between the scheduler and the training framework. The scheduler guarantees that resource changes are communicated through defined signals, with sufficient advance notice for graceful adaptation. The framework guarantees that it can handle resource changes without corrupting model state, using the batch size adjustment, learning rate recalibration, and communication group reconstruction mechanisms developed in @sec-fault-tolerance-reliability-reliability-elastic-training-mechanisms-49b1. The worker-membership lifecycle follows the state machine shown in @fig-fleet-orchestration-elastic-state-machine, transitioning workers through provisioning, active training, dynamic rebalancing, and self-healing fault recovery.

::: {#fig-fleet-orchestration-elastic-state-machine fig-env="figure" fig-pos="htb" fig-cap="**Elastic training worker lifecycle and state machine**: The state machine governing dynamic worker membership. Training begins in Pending/Provisioning, transitions through Rendezvous into Active Training, and adapts dynamically via Scale-Up/Rebalance or Draining/Scale-Down without full job restarts. Hardware faults trigger self-healing recovery." fig-alt="State machine diagram illustrating elastic worker states: Pending, Active Training, Scale-Up Rebalance, Draining Scale-Down, and Fault Recovery, connected by rendezvous, resize, and self-heal transitions."}
...
\begin{tikzpicture}[
    ...
    \node[draw=NavyBlue!90!black, fill=NavyBlue!4!white, rounded corners=4pt, line width=1.2pt,
          minimum width=5.0cm, minimum height=2.2cm, align=center, inner sep=6pt] (c_active) at (0, 0.7) {
        {\normalsize\bfseries\color{NavyBlue!90!black} Active Training}\\[3pt]
        {\footnotesize\bfseries N active worker ranks}\\[2pt]
        {\scriptsize\color{black!70} Synchronous AllReduce each step}\\[2pt]
        {\scriptsize\bfseries\color{NavyBlue!80!black} Local batch = B / N}
    };

    % Scale Up (Top-Left)
    \node[draw=ForestGreen, fill=ForestGreen!4!white, rounded corners=4pt, line width=1.1pt,
          minimum width=4.6cm, minimum height=1.9cm, align=center, inner sep=6pt] (c_scale_up) at (-5.0, 3.8) {
        {\normalsize\bfseries\color{ForestGreen!90!black} Scale Up (Grow)}\\[3pt]
        {\scriptsize\color{black!80} Admit +M spare workers}\\[1.5pt]
        {\scriptsize\color{black!80} Re-shard data batches}\\[1.5pt]
        {\scriptsize\color{black!80} Linear LR warmup step}
    };

    % Scale Down (Top-Right)
    \node[draw=BurntOrange, fill=BurntOrange!4!white, rounded corners=4pt, line width=1.1pt,
          minimum width=4.6cm, minimum height=1.9cm, align=center, inner sep=6pt] (c_scale_down) at (5.0, 3.8) {
        {\normalsize\bfseries\color{BurntOrange!90!black} Scale Down (Shrink)}\\[3pt]
        {\scriptsize\color{black!80} Release -K workers}\\[1.5pt]
        {\scriptsize\color{black!80} Drain in-flight gradient sync}\\[1.5pt]
        {\scriptsize\color{black!80} Recalibrate learning rate}
    };

    % Worker Replace (Bottom)
    \node[draw=BrickRed, fill=BrickRed!4!white, rounded corners=4pt, line width=1.1pt,
          minimum width=5.0cm, minimum height=1.9cm, align=center, inner sep=6pt] (c_replace) at (0, -2.4) {
        {\normalsize\bfseries\color{BrickRed!90!black} Worker Replace (Self-Heal)}\\[3pt]
        {\scriptsize\color{black!80} Substitute 1:1 failed host}\\[1.5pt]
        {\scriptsize\color{black!80} Fetch latest micro-checkpoint}\\[1.5pt]
        {\scriptsize\bfseries\color{BrickRed!90!black} Zero batch-size disruption}
    };
    ...
    % Self Loop: Step completion
    \draw[-{Stealth[length=4pt, width=3pt]}, thick, NavyBlue] ([yshift=10pt]c_active.west) arc[start angle=90, end angle=270, radius=0.45cm]
        node[midway, left=3pt, font=\scriptsize\bfseries\color{NavyBlue!90!black}] {Step completion};

    % Active -> Scale Up
    \draw[-{Stealth[length=4pt, width=3pt]}, very thick, ForestGreen] ([xshift=-10pt]c_active.north) -- ([xshift=30pt]c_scale_up.south)
        node[midway, above left=1pt, draw=ForestGreen!30, fill=ForestGreen!10, rounded corners=2pt, inner sep=2pt, font=\tiny\bfseries\color{ForestGreen!90!black}] {Idle GPUs free};

    % Scale Up -> Active
    \draw[-{Stealth[length=4pt, width=3pt]}, very thick, dashed, NavyBlue!80!black] ([xshift=-10pt]c_scale_up.south) -- ([yshift=15pt]c_active.west)
        node[midway, below left=1pt, font=\tiny\bfseries\color{NavyBlue!90!black}] {Group rebuilt};

    % Active -> Scale Down
    \draw[-{Stealth[length=4pt, width=3pt]}, very thick, BurntOrange] ([xshift=10pt]c_active.north) -- ([xshift=-30pt]c_scale_down.south)
        node[midway, above right=1pt, draw=BurntOrange!30, fill=BurntOrange!10, rounded corners=2pt, inner sep=2pt, font=\tiny\bfseries\color{BurntOrange!90!black}] {Preempt / high-pri};

    % Scale Down -> Active
    \draw[-{Stealth[length=4pt, width=3pt]}, very thick, dashed, NavyBlue!80!black] ([xshift=10pt]c_scale_down.south) -- ([yshift=15pt]c_active.east)
        node[midway, below right=1pt, font=\tiny\bfseries\color{NavyBlue!90!black}] {Ranks released};

    % Active -> Worker Replace (Failure)
    \draw[-{Stealth[length=4pt, width=3pt]}, very thick, BrickRed] ([xshift=-18pt]c_active.south) -- ([xshift=-18pt]c_replace.north);
    \node[draw=BrickRed!30, fill=BrickRed!10, rounded corners=2pt, inner sep=2.5pt, font=\tiny\bfseries\color{BrickRed!90!black}]
        at (-1.7, -0.85) {Host timeout / ECC};

    % Worker Replace -> Active (Healed)
    \draw[-{Stealth[length=4pt, width=3pt]}, very thick, dashed, NavyBlue!80!black] ([xshift=18pt]c_replace.north) -- ([xshift=18pt]c_active.south);
    \node[draw=NavyBlue!20, fill=NavyBlue!8, rounded corners=2pt, inner sep=2.5pt, font=\tiny\bfseries\color{NavyBlue!90!black}]
        at (1.7, -0.85) {Hot spare joined};

    % Scale Down -> Worker Replace (Spot Deadline)
    \draw[-{Stealth[length=4pt, width=3pt]}, very thick, dashed, BrickRed] ([xshift=20pt]c_scale_down.south) to[out=270, in=0]
        node[sloped, above=2pt, font=\tiny\bfseries\color{BrickRed!90!black}] {Spot 30s deadline} (c_replace.east);

    % =========================================================================
    % BOTTOM BANNER: Invariant
    % =========================================================================
    \node[draw=gray!30, fill=gray!4, rounded corners=3pt, line width=0.8pt,
          minimum width=14.5cm, minimum height=0.7cm, inner sep=3pt,
          font=\scriptsize\bfseries\color{black!80}] at (0, -3.95) {
        Core Invariant: Model parameters and optimizer states remain continuous across all worker transitions.
    };
...
As illustrated in @fig-fleet-orchestration-scaling-efficiency-curve, the decision to scale is therefore a function of marginal utility, not vacancy alone. For our 175B parameter model, the efficiency curve is derived from profiling: it scales linearly from 64 to 512 GPUs, becomes sublinear between 512 and 1,024, and hits diminishing returns beyond 1,024. Consequently, the scheduler enforces an elastic range of $[256, 1024]$. If the cluster has free resources but the job is already at 1,024 GPUs, the scheduler withholds the extra nodes, predicting that the 5 percent marginal throughput gain does not justify the re-sharding overhead or the opportunity cost of starving a pending job. Similarly, if only 128 GPUs are available, the scheduler may choose to queue the job rather than run it, as the training intensity per GPU would be too low to hide the communication latency.

::: {#fig-fleet-orchestration-scaling-efficiency-curve fig-env="figure" fig-pos="htb" fig-cap="**Empirical scaling efficiency curve and elastic operating range**: Observed scaling efficiency ($\eta_{\text{scaling}}$) across cluster sizes from 64 to 4,096 GPUs. Efficiency exhibits three distinct regimes: linear scaling ($\eta \ge 90\%$), sublinear scaling ($70\% \le \eta < 90\%$), and communication saturation ($\eta < 70\%$). Schedulers restrict elastic scaling to the viable window $[N_{\text{min}}, N_{\text{max}}]$ to prevent negative marginal utility." fig-alt="Empirical scaling efficiency curve plotting scaling efficiency percentage against GPU count from 64 to 4,096 GPUs, highlighting linear, sublinear, and saturation zones with minimum and maximum viable scale boundaries."}
...
\begin{tikzpicture}[
    ...
    % =========================================================================
    % LEFT PANEL: Scaling Efficiency Plot
    % Origin at (0,0).
    % X: 64 (0), 128 (1.6), 256 (3.2), 512 (4.8), 1024 (6.4), 2048 (8.0), 4096 (9.6)
    % Y: 0.0 to 1.0 (0 to 6.0 cm)
    % =========================================================================

    % Background zone fills
    % Zone 1: Linear (Compute) [0 to 3.2]
    \fill[ForestGreen!6!white] (0, 0) rectangle (3.2, 6.2);
    % Zone 2: Sublinear (Mixed) [3.2 to 6.4]
    \fill[Amber!6!white] (3.2, 0) rectangle (6.4, 6.2);
    % Zone 3: Saturation (Comm) [6.4 to 10.2]
    \fill[BrickRed!6!white] (6.4, 0) rectangle (10.2, 6.2);

    % Horizontal grid lines (dashed)
    \foreach \y/\val in {1.2/0.2, 2.4/0.4, 3.6/0.6, 4.8/0.8} {
        \draw[dashed, SlateGrey!30, line width=0.6pt, dash pattern=on 3pt off 3pt] (0, \y) -- (10.2, \y);
        \node[font=\scriptsize\color{SlateGrey!90!black}, anchor=east] at (-0.15, \y) {\val};
    }
    \node[font=\scriptsize\color{SlateGrey!90!black}, anchor=east] at (-0.15, 0) {0.0};
    \node[font=\scriptsize\color{SlateGrey!90!black}, anchor=east] at (-0.15, 6.0) {1.0};

    % Ideal linear line at y = 6.0
    \draw[dashed, SlateGrey!60, line width=0.9pt, dash pattern=on 4pt off 3pt] (0, 6.0) -- (10.2, 6.0);
    \node[font=\scriptsize\color{SlateGrey!80!black}, anchor=south east] at (10.15, 6.08) {Ideal Linear ($E = 1.0$)};

    % Vertical zone boundary dashed lines
    \draw[dashed, BurntOrange!90!black, line width=1.1pt, dash pattern=on 4pt off 3pt] (3.2, 0) -- (3.2, 6.2);
    \draw[dashed, BrickRed!90!black, line width=1.1pt, dash pattern=on 4pt off 3pt] (6.4, 0) -- (6.4, 6.2);

    % Region Badges at bottom
    \node[draw=ForestGreen!40, fill=ForestGreen!15, rounded corners=3pt, font=\scriptsize\bfseries\color{ForestGreen!85!black}, inner xsep=6pt, inner ysep=3pt] at (1.6, 0.45) {Linear (Compute)};
    \node[draw=Amber!50, fill=Amber!15, rounded corners=3pt, font=\scriptsize\bfseries\color{BurntOrange!95!black}, inner xsep=6pt, inner ysep=3pt] at (4.8, 0.45) {Sublinear (Mixed)};
    \node[draw=BrickRed!40, fill=BrickRed!15, rounded corners=3pt, font=\scriptsize\bfseries\color{BrickRed!90!black}, inner xsep=6pt, inner ysep=3pt] at (8.3, 0.45) {Saturation (Comm)};

    % Axes
    \draw[SlateGrey!70, line width=1.0pt] (0, 0) -- (10.2, 0);
    \draw[SlateGrey!70, line width=1.0pt] (0, 0) -- (0, 6.2);

    % Y-axis ticks
    \foreach \y in {0, 1.2, 2.4, 3.6, 4.8, 6.0} {
        \draw[SlateGrey!70, line width=0.8pt] (0, \y) -- (-0.1, \y);
    }

    % X-axis ticks and labels
    \draw[SlateGrey!70, line width=0.8pt] (0, 0) -- (0, -0.1) node[below=2pt, font=\scriptsize\color{SlateGrey!90!black}] {64};
    \draw[SlateGrey!70, line width=0.8pt] (1.6, 0) -- (1.6, -0.1) node[below=2pt, font=\scriptsize\color{SlateGrey!90!black}] {128};
    \draw[SlateGrey!70, line width=0.8pt] (3.2, 0) -- (3.2, -0.1) node[below=2pt, font=\scriptsize\bfseries\color{black!90}] {256};
    \draw[SlateGrey!70, line width=0.8pt] (4.8, 0) -- (4.8, -0.1) node[below=2pt, font=\scriptsize\color{SlateGrey!90!black}] {512};
    \draw[SlateGrey!70, line width=0.8pt] (6.4, 0) -- (6.4, -0.1) node[below=2pt, font=\scriptsize\bfseries\color{black!90}] {1,024};
    \draw[SlateGrey!70, line width=0.8pt] (8.0, 0) -- (8.0, -0.1) node[below=2pt, font=\scriptsize\color{SlateGrey!90!black}] {2,048};
    \draw[SlateGrey!70, line width=0.8pt] (9.6, 0) -- (9.6, -0.1) node[below=2pt, font=\scriptsize\color{SlateGrey!90!black}] {4,096};

    % Axis Titles
    \node[font=\footnotesize\bfseries\color{black!85}, rotate=90] at (-0.95, 3.1) {Scaling Efficiency $E(k)$};
    \node[font=\footnotesize\bfseries\color{black!85}] at (5.1, -0.95) {GPU Worker Count (Log Scale)};

    % Coordinates for points
    \coordinate (P64) at (0, 5.76);
    \coordinate (P128) at (1.6, 5.64);
    \coordinate (P256) at (3.2, 5.40);
    \coordinate (P512) at (4.8, 4.86);
    \coordinate (P1024) at (6.4, 4.08);
    \coordinate (P2048) at (8.0, 3.12);
    \coordinate (P4096) at (9.6, 2.52);

    % Straight segments with rounded joins
    \draw[line width=2.4pt, line cap=round, line join=round, ForestGreen!85!black] (P64) -- (P128) -- (P256);
    \draw[line width=2.4pt, line cap=round, line join=round, BurntOrange!90!black] (P256) -- (P512) -- (P1024);
    \draw[line width=2.4pt, line cap=round, line join=round, BrickRed] (P1024) -- (P2048) -- (P4096);

    % Point Markers and Labels
    \fill[ForestGreen!85!black] (P64) circle (3pt);
    \node[font=\scriptsize\bfseries\color{ForestGreen!85!black}, anchor=north] at ($(P64) + (0, -0.15)$) {96\%};

    \fill[ForestGreen!85!black] (P128) circle (3pt);
    \node[font=\scriptsize\bfseries\color{ForestGreen!85!black}, anchor=north] at ($(P128) + (0, -0.15)$) {94\%};

    \fill[BurntOrange!90!black] (P256) circle (3pt);
    \node[fill=Amber!6!white, inner sep=1.5pt, rounded corners=1pt,
          font=\scriptsize\bfseries\color{BurntOrange!90!black}, anchor=north] at ($(P256) + (0, -0.15)$) {90\%};

    \fill[BurntOrange!90!black] (P512) circle (3pt);
    \node[font=\scriptsize\bfseries\color{BurntOrange!90!black}, anchor=north] at ($(P512) + (0, -0.15)$) {81\%};

    \fill[BrickRed] (P1024) circle (3pt);
    \node[fill=BrickRed!6!white, inner sep=1.5pt, rounded corners=1pt,
          font=\scriptsize\bfseries\color{BrickRed}, anchor=north] at ($(P1024) + (0, -0.15)$) {68\%};

    \fill[BrickRed] (P2048) circle (3pt);
    \node[font=\scriptsize\bfseries\color{BrickRed}, anchor=north] at ($(P2048) + (0, -0.15)$) {52\%};

    \fill[BrickRed] (P4096) circle (3pt);
    \node[font=\scriptsize\bfseries\color{BrickRed}, anchor=north] at ($(P4096) + (0, -0.15)$) {42\%};

    % =========================================================================
    % RIGHT PANEL: Enforced Elastic Range & Saturation Trap Cards
    % =========================================================================

    % Card 1: Enforced Elastic Range
    \node[draw=ForestGreen!50, fill=ForestGreen!4!white, rounded corners=6pt, line width=1.0pt,
          minimum width=5.0cm, minimum height=2.9cm, anchor=north west] (card1) at (10.9, 6.2) {};

    \node[font=\small\bfseries\color{ForestGreen!85!black}, anchor=north west] at (11.15, 5.95) {Enforced Elastic Range};
    \node[font=\Large\bfseries\color{NavyBlue!90!black}, anchor=north west] at (11.15, 5.50) {[256, 1024] GPUs};

    \node[font=\scriptsize\bfseries\color{black!85}, anchor=north west] at (11.15, 4.75) {Min scale: 256 GPUs};
    \node[font=\tiny\color{black!70}, anchor=north west] at (11.15, 4.45) {Below 256: comm latency wastes allocation};

    \node[font=\scriptsize\bfseries\color{black!85}, anchor=north west] at (11.15, 3.95) {Max scale: 1,024 GPUs};
    \node[font=\tiny\color{black!70}, anchor=north west] at (11.15, 3.65) {Above 1024: 5\% gain not worth re-sharding};

    % Card 2: Saturation Trap
    \node[draw=BrickRed!40, fill=BrickRed!3!white, rounded corners=6pt, line width=1.0pt,
          minimum width=5.0cm, minimum height=3.35cm, anchor=north west] (card2) at (10.9, 3.1) {};

    \node[font=\small\bfseries\color{BrickRed!90!black}, anchor=north west] at (11.15, 2.85) {Saturation Trap};
    \node[font=\scriptsize\bfseries\color{BrickRed!90!black}, anchor=north west] at (11.15, 2.45) {At 4,096 GPUs, efficiency drops to 42\%.};

    \node[font=\tiny\color{black!75}, anchor=north west] at (11.15, 2.05) {58\% of all compute cycles};
    \node[font=\tiny\color{black!75}, anchor=north west] at (11.15, 1.75) {are lost to AllReduce rings.};

    % Divider
    \draw[draw=BrickRed!20, line width=0.8pt] (11.15, 1.45) -- (15.65, 1.45);

    \node[font=\scriptsize\bfseries\color{black!85}, anchor=north west] at (11.15, 1.25) {Scheduler Action:};
    \node[font=\tiny\color{black!75}, anchor=north west] at (11.15, 0.95) {Withhold nodes $>$1,024 to};
    \node[font=\tiny\color{black!75}, anchor=north west] at (11.15, 0.70) {clear pending queue backlog.};

    % Marginal Utility Policy button
    \node[draw=BrickRed!25, fill=BrickRed!12, rounded corners=3pt, font=\scriptsize\bfseries\color{BrickRed!90!black},
          minimum width=4.5cm, inner sep=3.5pt] at (13.4, 0.15) {Marginal Utility Policy};
...
# Utilization levels use line style and neutral tones; red is reserved for the
# highest-waste condition.
util_configs = [
    (0.30, COLORS['RedLine'], '-', '30% utilization'),
    (0.50, '#444444', '--', '50% utilization'),
    (0.70, '#777777', '-.', '70% utilization'),
    (0.90, '#aaaaaa', ':', '90% utilization'),
]
...
for util, color, linestyle, label in util_configs:
    ...
    ax.plot(cluster_sizes / THOUSAND, annual_waste, color=color, linestyle=linestyle, linewidth=2, label=label)
...
ax.fill_between(cluster_sizes / THOUSAND, waste_70, waste_30, alpha=0.10, color='#777777', label='Illustrative range')
...
ax.scatter([philly_cluster], [philly_waste], color='#222222', s=100, zorder=5, marker='D', edgecolors='white', linewidth=1.0)
...
ax.annotate(
    ...
    fontweight='normal',
    color='#222222',
    arrowprops=dict(arrowstyle='->', shrinkB=7, color='#222222', lw=0.75),
)
...
As @fig-fleet-orchestration-capacity-reservation illustrates, the optimal mix depends on workload characteristics and organizational priorities. An organization with 60 percent steady-state utilization and 40 percent burst demand might reserve capacity for 60 percent of peak need, use on-demand for latency-sensitive bursts that require immediate provisioning, and use spot for the remainder. The scheduling system must understand these capacity tiers and route workloads accordingly: inference workloads to reserved capacity for guaranteed availability, large training jobs to a mix of reserved (for the base allocation) and spot (for additional workers via elastic scaling), and exploratory work to spot exclusively where the cost savings are maximized and the interruption tolerance is highest.

::: {#fig-fleet-orchestration-capacity-reservation fig-env="figure" fig-pos="htb" fig-cap="**Capacity reservation tiers and portfolio optimization**: The three cloud capacity tiers (Reserved, On-Demand, and Spot/Preemptible) trade unit cost against commitment and availability guarantees. Schedulers map steady-state baseline demand to reserved capacity, diurnal spikes to on-demand instances, and restart-tolerant training to discounted spot pools." fig-alt="Comparison of three cloud capacity tiers: 1-to-3 year reserved capacity, on-demand instances, and spot/preemptible pools, showing pricing tiers, SLA availability, and workload routing across a diurnal demand curve."}
...
\begin{tikzpicture}[
    ...
    \providecolor{SlateGrey}{RGB}{112,128,144}
    \providecolor{LightGrey}{RGB}{245,247,250}
    \providecolor{DarkOrchid}{RGB}{130,50,200}

    % =========================================================================
    % BACKGROUND BANDS
    % X: 0 to 11.4 cm
    % Y: 0 to 6.0 cm (0% = 0, 60% = 3.6, 80% = 4.8, 100% = 6.0)
    % =========================================================================

    % Tier 1 Band (0% to 60%)
    \fill[NavyBlue!6!white] (0, 0) rectangle (11.4, 3.6);
    % Tier 2 Band (60% to 80%)
    \fill[Amber!8!white] (0, 3.6) rectangle (11.4, 4.8);
    % Tier 3 Band (80% to 100%)
    \fill[ForestGreen!6!white] (0, 4.8) rectangle (11.4, 6.0);

    % Horizontal threshold lines (dashed)
    \draw[dashed, NavyBlue!80!black, line width=0.9pt, dash pattern=on 4pt off 3pt] (0, 3.6) -- (11.4, 3.6);
    \draw[dashed, BurntOrange!90!black, line width=0.9pt, dash pattern=on 4pt off 3pt] (0, 4.8) -- (11.4, 4.8);

    % Axes
    \draw[SlateGrey!70, line width=1.0pt] (0, 0) -- (11.4, 0);
    \draw[SlateGrey!70, line width=1.0pt] (0, 0) -- (0, 6.3);

    % Y-axis ticks and labels
    \foreach \y/\val in {0/0\%, 1.5/25\%, 3.0/50\%, 4.5/75\%, 6.0/100\%} {
        \draw[SlateGrey!70, line width=0.8pt] (0, \y) -- (-0.12, \y);
        \node[font=\scriptsize\color{SlateGrey!90!black}, anchor=east] at (-0.18, \y) {\val};
    }

    % Axis labels
    \node[font=\footnotesize\bfseries\color{black!85}, rotate=90] at (-1.1, 3.0) {Fleet GPU Capacity Share};
    \node[font=\footnotesize\bfseries\color{black!85}] at (5.7, -0.55) {Timeline / Diurnal Cycle};

    % Curve: Actual Workload Demand (DarkOrchid)
    \draw[line width=2.4pt, DarkOrchid]
        (0, 2.46) to[out=5, in=230]
        (1.8, 3.24) to[out=50, in=180]
        (4.2, 5.52) to[out=0, in=140]
        (6.5, 4.08) to[out=-40, in=180]
        (8.8, 2.94) to[out=0, in=190]
        (11.4, 3.24);

    % Curve Label
    \node[font=\small\bfseries\color{DarkOrchid!90!black}] at (2.8, 2.1) {Actual Workload Demand};

    % Peak Demand Point & Callout Box
    \coordinate (Peak) at (4.2, 5.52);
    \fill[BrickRed] (Peak) circle (3pt);

    \node[draw=BrickRed, fill=white, rounded corners=4pt, line width=1.0pt, align=center, inner sep=4pt]
        (peak_box) at (4.2, 6.35) {
            {\scriptsize\bfseries\color{BrickRed} Peak Demand Burst (92\%)}\\[1pt]
            {\tiny\color{black!80} Absorbed by Spot \& On-Demand}
        };
    \draw[line width=0.8pt, BrickRed] (peak_box.south) -- (Peak);

    % =========================================================================
    % TIER CARDS (Inside Plot, Right Aligned)
    % Center x = 8.8, width = 3.8cm
    % =========================================================================

    % Tier 3 Card
    \node[draw=ForestGreen!60, fill=white, rounded corners=4pt, line width=0.8pt,
          minimum width=3.8cm, minimum height=1.05cm, align=center, inner sep=3pt] at (8.8, 5.4) {
        {\scriptsize\bfseries\color{ForestGreen!90!black} Tier 3: Spot / Preemptible}\\[2pt]
        {\tiny\color{black!80} Opportunistic $\cdot$ 60--90\% discount}\\[1pt]
        {\tiny\color{BrickRed} Subject to 30s--2m eviction notice}
    };

    % Tier 2 Card
    \node[draw=BurntOrange!60, fill=white, rounded corners=4pt, line width=0.8pt,
          minimum width=3.8cm, minimum height=0.95cm, align=center, inner sep=3pt] at (8.8, 4.2) {
        {\scriptsize\bfseries\color{BurntOrange!90!black} Tier 2: On-Demand}\\[2pt]
        {\tiny\color{black!80} Immediate provisioning $\cdot$ 20\% buffer}
    };

    % Tier 1 Card
    \node[draw=NavyBlue!60, fill=white, rounded corners=4pt, line width=0.8pt,
          minimum width=3.8cm, minimum height=1.15cm, align=center, inner sep=3pt] at (8.8, 1.8) {
        {\scriptsize\bfseries\color{NavyBlue!90!black} Tier 1: Reserved Instances}\\[2pt]
        {\tiny\color{black!80} 1--3 yr commitment $\cdot$ 60\% base}\\[2pt]
        {\scriptsize\bfseries\color{NavyBlue!90!black} Steady training \& base serving}
    };

    % =========================================================================
    % RIGHT PRICING CARDS
    % Center x = 12.8, width = 2.4cm
    % =========================================================================

    % Spot Rate Card
    \node[draw=ForestGreen!60, fill=ForestGreen!5!white, rounded corners=4pt, line width=0.8pt,
          minimum width=2.4cm, minimum height=1.15cm, align=center, inner sep=2pt] at (12.8, 5.4) {
        {\small\bfseries\color{ForestGreen!90!black} \$0.70}\\[1pt]
        {\tiny\color{black!70} per GPU-hour}\\[2pt]
        {\tiny\bfseries\color{ForestGreen!90!black} SPOT RATE}
    };

    % On-Demand Card
    \node[draw=BurntOrange!60, fill=Amber!6!white, rounded corners=4pt, line width=0.8pt,
          minimum width=2.4cm, minimum height=1.15cm, align=center, inner sep=2pt] at (12.8, 4.0) {
        {\small\bfseries\color{BurntOrange!95!black} \$2.00}\\[1pt]
        {\tiny\color{black!70} per GPU-hour}\\[2pt]
        {\tiny\bfseries\color{BurntOrange!95!black} ON-DEMAND}
    };

    % Reserved Card
    \node[draw=NavyBlue!60, fill=NavyBlue!5!white, rounded corners=4pt, line width=0.8pt,
          minimum width=2.4cm, minimum height=1.15cm, align=center, inner sep=2pt] at (12.8, 2.6) {
        {\small\bfseries\color{NavyBlue!90!black} \$0.80}\\[1pt]
        {\tiny\color{black!70} per GPU-hour}\\[2pt]
        {\tiny\bfseries\color{NavyBlue!90!black} RESERVED (3Y)}
    };

    % Optimal Mix Card
    \node[draw=SlateGrey!40, fill=white, rounded corners=4pt, line width=0.8pt,
          minimum width=2.4cm, minimum height=1.2cm, align=center, inner sep=2pt] at (12.8, 1.1) {
        {\scriptsize\bfseries\color{black!85} Optimal Mix}\\[2pt]
        {\scriptsize\bfseries\color{ForestGreen!90!black} 50--65\% Savings}\\[2pt]
        {\tiny\color{black!70} vs pure on-demand}
    };
...
[^fn-kv-cache-orchestration]: **Paged KV cache**\index{KV Cache!serving}: Paging changes how the cache is allocated and reduces fragmentation, but it does not change the cache's growth with sequence length and concurrent requests. Autoscalers must therefore monitor memory pressure even when compute utilization remains low.

This dual resource dimension (compute and memory) suggests that effective LLM autoscaling should monitor both GPU compute utilization and GPU memory pressure, scaling out when either approaches critical thresholds. Some production systems define a composite scaling metric that combines these dimensions, triggering scale-out when the maximum of normalized compute utilization and normalized memory pressure exceeds a threshold. This composite approach prevents the system from being surprised by either type of resource exhaustion. @Fig-fleet-orchestration-inference-autoscaling illustrates this closed-loop control architecture, where dual-signal telemetry feeds predictive and reactive controllers to coordinate pod provisioning and replica prewarming.

::: {#fig-fleet-orchestration-inference-autoscaling fig-env="figure" fig-pos="htb" fig-cap="**Inference autoscaling closed-loop control**: A dual-signal control architecture monitors both GPU memory pressure (KV cache occupancy) and compute load (SM utilization, queue depth). Telemetry feeds both a reactive HPA loop and a predictive prewarming controller that provisions replicas ahead of cold-start deadlines." fig-alt="Control loop architecture showing dual-signal telemetry (KV cache pressure and compute utilization) driving reactive and predictive autoscalers to actuate replica scaling and model prewarming."}
...
\begin{tikzpicture}[
    ...
    % =========================================================================
    % TOP PANEL: Autoscaling Control Feedback Loop
    % =========================================================================

    % 1. Continuous Monitor Card
    \node[draw=ForestGreen, fill=ForestGreen!3!white, rounded corners=6pt, line width=1.2pt,
          minimum width=4.4cm, minimum height=2.6cm, align=center, inner sep=5pt] (mon) at (-5.0, 3.6) {
        {\footnotesize\bfseries\color{ForestGreen!90!black} 1. Continuous Monitor}\\[5pt]
        \colorbox{ForestGreen!15}{\strut\scriptsize\bfseries\color{ForestGreen!90!black}\;KV Cache Occupancy\;}\\[4pt]
        {\tiny\color{black!80} + Queue backlog depth}\\[1.5pt]
        {\tiny\color{black!80} + Pending token arrival rate}\\[1.5pt]
        {\tiny\color{ForestGreen!80!black} PagedAttention block leak check}
    };

    % Evaluate Diamond
    \node[diamond, draw=BurntOrange, fill=white, line width=1.3pt, inner sep=2pt,
          minimum width=2.4cm, minimum height=2.4cm, align=center] (eval) at (-0.5, 3.6) {
        {\footnotesize\bfseries Evaluate}\\[1.5pt]
        {\footnotesize\bfseries\color{BurntOrange} Threshold}\\[1.5pt]
        {\tiny\color{black!80} $>$75\% / $<$40\%}
    };

    % Arrow: Monitor to Diamond
    \draw[-{Stealth[length=5pt, width=3.5pt]}, line width=1.2pt, ForestGreen] (mon.east) --
        node[above=2pt, font=\scriptsize\color{SlateGrey!90!black}] {metrics} (eval.west);

    % Scale Up Card (Top Right)
    \node[draw=NavyBlue, fill=NavyBlue!4!white, rounded corners=6pt, line width=1.2pt,
          minimum width=5.7cm, minimum height=1.9cm, align=left, inner sep=7pt] (scale_up) at (6.0, 4.8) {
        {\footnotesize\bfseries\color{NavyBlue!90!black} Scale Up (Add Replicas)}\\[3pt]
        {\scriptsize\textbf{Cooldown:} 0 min (rapid spike response)}\\[1.5pt]
        {\scriptsize\textbf{Headroom:} Buffer prevents OOM evictions}\\[2pt]
        {\scriptsize\bfseries\color{NavyBlue!90!black} Guarantees P99 TTFT SLO}
    };

    % Scale Down Card (Bottom Right)
    \node[draw=BurntOrange, fill=Amber!4!white, rounded corners=6pt, line width=1.2pt,
          minimum width=5.7cm, minimum height=1.9cm, align=left, inner sep=7pt] (scale_down) at (6.0, 2.4) {
        {\footnotesize\bfseries\color{BurntOrange!90!black} Scale Down (Reclaim GPUs)}\\[3pt]
        {\scriptsize\textbf{Cooldown:} 15--30 min (hysteresis guard)}\\[1.5pt]
        {\scriptsize\textbf{Draining:} Flushes active token streams}\\[2pt]
        {\scriptsize\bfseries\color{BurntOrange!90!black} Hysteresis prevents replica thrashing}
    };

    % High Pill & Arrow
    \node[draw=NavyBlue!40, fill=NavyBlue!12, rounded corners=3pt, font=\tiny\bfseries\color{NavyBlue!90!black}, inner sep=3pt]
        (pill_high) at (1.8, 4.8) {$>$ 75\% High};
    \draw[line width=1.1pt, NavyBlue] (eval.north east) -- (pill_high.west);
    \draw[-{Stealth[length=5pt, width=3.5pt]}, line width=1.1pt, NavyBlue] (pill_high.east) -- (scale_up.west);

    % Low Pill & Arrow
    \node[draw=Amber!50, fill=Amber!15, rounded corners=3pt, font=\tiny\bfseries\color{BurntOrange!95!black}, inner sep=3pt]
        (pill_low) at (1.8, 2.4) {$<$ 40\% Low};
    \draw[line width=1.1pt, BurntOrange] (eval.south east) -- (pill_low.west);
    \draw[-{Stealth[length=5pt, width=3.5pt]}, line width=1.1pt, BurntOrange] (pill_low.east) -- (scale_down.west);

    % Feedback Loop
    \draw[dashed, SlateGrey!70, line width=0.9pt] (scale_up.east) -- ++(0.5, 0) coordinate (fb_r);
    \draw[dashed, SlateGrey!70, line width=0.9pt] (scale_down.east) -- ++(0.5, 0);
    \draw[-{Stealth[length=5pt, width=3.5pt]}, dashed, SlateGrey!70, line width=0.9pt]
        (fb_r) -- (fb_r |- 0, 0.7) --
        node[above=2pt, font=\scriptsize\color{SlateGrey!90!black}] {Continuous Control Feedback Loop}
        (-5.0, 0.7) -- (mon.south);

    % =========================================================================
    % BOTTOM PANELS: Anti-Pattern vs. Correct Pattern
    % =========================================================================

    % Left Card: Anti-Pattern
    \node[draw=Crimson!70, fill=Crimson!2!white, rounded corners=6pt, line width=1.0pt,
          minimum width=8.6cm, minimum height=4.6cm, anchor=north west] (anti_card) at (-8.0, -0.1) {};

    % Red X badge
    \node[draw=Crimson, fill=Crimson!10, shape=circle, inner sep=2.5pt, font=\scriptsize\bfseries\color{Crimson}]
        at ($(anti_card.north east) + (-0.45, -0.45)$) {$\times$};

    \node[font=\footnotesize\bfseries\color{Crimson}, anchor=north west]
        at ($(anti_card.north west) + (0.35, -0.35)$) {Anti-Pattern: Scaling on Raw GPU Utilization};
    \node[font=\scriptsize\bfseries\color{Crimson!90!black}, anchor=north west]
        at ($(anti_card.north west) + (0.35, -0.85)$) {Long-context decodes saturate memory, not compute:};

    \node[font=\scriptsize, anchor=north west, align=left]
        at ($(anti_card.north west) + (0.35, -1.25)$) {
        \textbf{\color{Crimson}SM Load:} Compute sits idle at $< 45\%$ during decode\\[2pt]
        \textbf{\color{Crimson}KV Memory:} 80 GB HBM reaches 95\%+ capacity\\[2pt]
        \textbf{\color{Crimson}HPA Issue:} Compute-based HPA never triggers scale-out\\[2pt]
        \textbf{\color{Crimson}Failure:} OOM eviction cascades \& dropped requests
    };

    \draw[draw=Crimson!20, line width=0.8pt]
        ($(anti_card.north west) + (0.35, -2.85)$) -- ($(anti_card.north east) + (-0.35, -2.85)$);

    \node[font=\scriptsize\bfseries\color{black!90}, anchor=north west]
        at ($(anti_card.north west) + (0.35, -3.05)$) {Failure Mode:};
    \node[font=\tiny\color{Crimson!90!black}, anchor=north west, align=left]
        at ($(anti_card.north west) + (0.35, -3.45)$) {
        Autoregressive decoding is memory-bandwidth bound.\\[2pt]
        Scaling on SM load misdiagnoses memory starvation.
    };

    % Right Card: Correct Pattern
    \node[draw=NavyBlue!50, fill=NavyBlue!2!white, rounded corners=6pt, line width=1.0pt,
          minimum width=8.6cm, minimum height=4.6cm, anchor=north west] (corr_card) at (0.8, -0.1) {};

    % Target badge
    \node[draw=ForestGreen!40, fill=ForestGreen!15, rounded corners=3pt, font=\tiny\bfseries\color{ForestGreen!90!black}, inner sep=3pt]
        at ($(corr_card.north east) + (-0.55, -0.45)$) {Target};

    \node[font=\footnotesize\bfseries\color{NavyBlue!90!black}, anchor=north west]
        at ($(corr_card.north west) + (0.35, -0.35)$) {Correct Pattern: Dual-Signal Scaling Policy};

    \node[font=\scriptsize, anchor=north west, align=left]
        at ($(corr_card.north west) + (0.35, -0.85)$) {
        \textbf{Formula:} $S_{\mathrm{composite}} = \max(U_{\mathrm{compute}}, P_{\mathrm{memory}})$\\[3pt]
        \textbf{$P_{\mathrm{memory}}$:} $(\mathrm{KV}_{\mathrm{allocated}} + \mathrm{KV}_{\mathrm{pending}}) \,/\, \mathrm{KV}_{\mathrm{total}}$\\[2pt]
        \textbf{$U_{\mathrm{compute}}$:} Observed active SM fraction\\[2pt]
        \textbf{Threshold:} Scale up whenever $S_{\mathrm{composite}} > 0.75$\\[2pt]
        \textbf{Cold Start:} Pre-warmup buffer for 2--5 min weight load
    };

    \draw[draw=NavyBlue!20, line width=0.8pt]
        ($(corr_card.north west) + (0.35, -2.85)$) -- ($(corr_card.north east) + (-0.35, -2.85)$);

    \node[font=\scriptsize\bfseries\color{ForestGreen!90!black}, anchor=north west]
        at ($(corr_card.north west) + (0.35, -3.05)$) {Asymmetric Cooldown:};
    \node[font=\tiny\color{black!75}, anchor=north west, align=left]
        at ($(corr_card.north west) + (0.35, -3.45)$) {
        Immediate scale-up absorbs unpredictable traffic spikes.\\[2pt]
        Delayed scale-down (20 min) prevents weight reload flap.
    };
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_count, fmt_int, fmt_memory, fmt_memory_capacity, fmt_params, fmt_qty, fmt_qty_int, fmt_usd
...
Inference traffic typically follows a "human curve": low at 3 AM, ramping up at 8 AM, peaking at 2 PM, and tapering off at 10 PM. A smart orchestrator forecasts this curve 30 minutes in advance. At 7:30 AM, it preempts low-priority training jobs (like hyperparameter sweeps) to warm up inference replicas. At 10:30 PM, as traffic drops, it drains inference replicas and releases GPUs back to the training scheduler. @Fig-fleet-orchestration-training-serving-diurnal illustrates this 24-hour diurnal cycle, showing how an orchestrator shifts GPUs between serving and training to recover idle overnight capacity.

::: {#fig-fleet-orchestration-training-serving-diurnal fig-env="figure" fig-pos="htb" fig-cap="**Training vs. serving diurnal GPU capacity shift**: Diurnal serving demand peaks during working hours and troughs overnight. Rather than statically partitioning dedicated clusters, dynamic sharing preempts low-priority training to meet morning serving surges, and returns drained serving GPUs to training overnight, reclaiming millions of dollars in annual cluster capacity." fig-alt="24-hour diurnal timeline showing serving demand rising during daytime and dropping at night, with opportunistic training capacity dynamically expanding into the reclaimed serving envelope."}
...
\begin{tikzpicture}[
    ...
    % =========================================================================
    % LEFT PANEL: Diurnal Timeline Plot
    % X: 0 to 12.0 cm (0h to 24h, 1h = 0.5 cm)
    % Y: 0 to 6.0 cm (0% to 100%, 100% = 6.0 cm)
    % =========================================================================

    % Background fill for Batch Training (entire upper region)
    \fill[NavyBlue!6!white] (0, 0) rectangle (12.0, 6.0);

    % Inference Serving trapezoid
    \fill[ForestGreen!10!white]
        (0, 0) --
        (0, 0.9) --
        (3.0, 0.9) --
        (4.25, 5.1) --
    ...
    \draw[line width=1.6pt, ForestGreen!85!black]
        (0, 0.9) --
        (3.0, 0.9) --
        (4.25, 5.1) --
        (9.0, 5.1) --
    ...
    % Top dashed line (100% Capacity)
    \draw[dashed, NavyBlue!80!black, line width=1.0pt, dash pattern=on 4pt off 3pt]
        (0, 6.0) -- (12.0, 6.0);

    % Shift Vertical Dashed Lines
    \draw[dashed, Crimson, line width=1.1pt, dash pattern=on 4pt off 3pt] (4.0, 0) -- (4.0, 6.0);
    \draw[dashed, Crimson, line width=1.1pt, dash pattern=on 4pt off 3pt] (10.0, 0) -- (10.0, 6.0);

    % Shift Callout Badges
    \node[draw=Crimson, fill=white, rounded corners=3pt, line width=0.9pt,
          inner sep=3.5pt, font=\scriptsize\bfseries\color{Crimson}]
        at (4.0, 6.38) {Morning Shift ($\sim$08:00)};

    \node[draw=Crimson, fill=white, rounded corners=3pt, line width=0.9pt,
          inner sep=3.5pt, font=\scriptsize\bfseries\color{Crimson}]
        at (10.0, 6.38) {Evening Shift ($\sim$20:00)};

    % Switching Latency Callout Box
    \node[draw=BurntOrange, fill=white, rounded corners=4pt, line width=1.0pt,
          minimum width=3.4cm, align=center, inner sep=4.5pt, anchor=north west]
        (switch_box) at (4.15, 2.6) {
            {\scriptsize\bfseries\color{BurntOrange} Switching Latency: 5--20 min}\\[2pt]
            {\tiny\color{black!80} Checkpoint save (1--5 min)}\\[1pt]
            {\tiny\color{black!80} Weight load \& warmup (5--15 min)}
        };

    % Region Labels
    % Left Batch Training
    \node[align=center] at (1.8, 4.2) {
        {\footnotesize\bfseries\color{NavyBlue!90!black} Batch Training}\\[2pt]
        {\tiny\color{NavyBlue!80!black} $\sim$85\% Fleet Share}\\[1pt]
        {\tiny\color{black!70} Night pretraining bursts}
    };

    % Right Batch Training (anchor=west at 10.15)
    \node[align=left, anchor=west] at (10.15, 4.2) {
        {\scriptsize\bfseries\color{NavyBlue!90!black} Batch Training}\\[2pt]
        {\tiny\color{NavyBlue!80!black} $\sim$85\% Fleet Share}
    };

    % Inference Serving Center Label
    \node[align=center] at (6.6, 4.0) {
        {\footnotesize\bfseries\color{ForestGreen!90!black} Inference Serving}\\[2pt]
        {\tiny\color{ForestGreen!80!black} $\sim$85\% Fleet Share}\\[1pt]
        {\tiny\color{black!70} Business hours peak traffic}
    };

    % Axes
    \draw[SlateGrey!70, line width=1.0pt] (0, 0) -- (12.0, 0);
    \draw[SlateGrey!70, line width=1.0pt] (0, 0) -- (0, 6.1);
    ...
    % X-axis ticks and labels
    \foreach \x/\val in {0/00:00, 2.0/04:00, 6.0/12:00, 8.0/16:00, 12.0/24:00} {
        \draw[SlateGrey!70, line width=0.8pt] (\x, 0) -- (\x, -0.12);
        \node[font=\scriptsize\color{SlateGrey!90!black}, anchor=north] at (\x, -0.18) {\val};
    }
    % Highlighted X-ticks (08:00 and 20:00)
    \draw[Crimson, line width=1.0pt] (4.0, 0) -- (4.0, -0.12);
    \node[font=\scriptsize\bfseries\color{Crimson}, anchor=north] at (4.0, -0.18) {08:00};

    \draw[Crimson, line width=1.0pt] (10.0, 0) -- (10.0, -0.12);
    \node[font=\scriptsize\bfseries\color{Crimson}, anchor=north] at (10.0, -0.18) {20:00};

    % Axis Titles
    \node[font=\footnotesize\bfseries\color{black!85}, rotate=90] at (-1.1, 3.0) {Fleet GPU Allocation (\% of total)};
    \node[font=\footnotesize\bfseries\color{black!85}] at (6.0, -0.75) {Time of Day (24-Hour Cycle)};

    % =========================================================================
    % RIGHT CARDS: Fleet Capacity Recovery & Operational Rule
    % =========================================================================

    % Card 1: Fleet Capacity Recovery
    \node[draw=ForestGreen!50, fill=ForestGreen!3!white, rounded corners=6pt, line width=1.0pt,
          minimum width=5.2cm, inner sep=8pt, anchor=north west, align=left] (card1) at (12.6, 6.0) {
        {\small\bfseries\color{ForestGreen!85!black} Fleet Capacity Recovery}\\[4pt]
        {\scriptsize\textbf{Static Partitioning:}}\\[1.5pt]
        {\tiny\textbf{Peak:} Reserves 500 GPUs}\\[1pt]
        {\tiny\textbf{Night:} Idle capacity ($\sim$35\% util)}\\[5pt]
        {\color{ForestGreen!25}\rule{4.6cm}{0.6pt}}\\[5pt]
        {\scriptsize\textbf{Dynamic Sharing:}}\\[1.5pt]
        {\tiny\textbf{\color{ForestGreen!90!black}Reclaim:} $\sim$200 GPUs nightly}\\[1pt]
        {\tiny\textbf{\color{ForestGreen!90!black}for elastic training runs}}\\[8pt]
        \centering\colorbox{ForestGreen!15}{\strut\scriptsize\bfseries\color{ForestGreen!90!black}\quad +\$3.5M / Year Value \quad}
    };

    % Card 2: Operational Rule (proper anchor=north west)
    \node[draw=NavyBlue!40, fill=NavyBlue!2!white, rounded corners=6pt, line width=1.0pt,
          minimum width=5.2cm, inner sep=8pt, align=left,
          below=10pt of card1.south west, anchor=north west] (card2) {
        {\small\bfseries\color{NavyBlue!90!black} Operational Rule}\\[4pt]
        {\scriptsize\textbf{Cadence:} Scheduled diurnal shifts}\\[1.5pt]
        {\scriptsize\textbf{Latency:} 5--20 min switch time}\\[1pt]
        {\tiny\color{black!70}(not for rapid traffic spikes)}\\[5pt]
        {\color{NavyBlue!20}\rule{4.6cm}{0.6pt}}\\[5pt]
        {\scriptsize\textbf{\color{Crimson}Safety Headroom:}}\\[1.5pt]
        {\tiny\textbf{\color{Crimson}Buffer:} 15\% reserve retained for}\\[1pt]
        {\tiny\textbf{\color{Crimson}unpredicted traffic surges}}
    };
...
# ┌── 3. RENDER ────────────────────────────────────────────────────────────────
ax.plot(utilization * 100, wait_small, label='Small jobs (average 1 h)', linewidth=2, color='#777777', linestyle=':')
ax.plot(utilization * 100, wait_mixed, label='Mixed workload (average 4 h)', linewidth=2, color='#444444', linestyle='--')
ax.plot(utilization * 100, wait_large, label='Large jobs (average 12 h)', linewidth=2, color='#111111', linestyle='-')
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
from mlsysim.fmt import fmt, fmt_bandwidth, fmt_count, fmt_int, fmt_memory, fmt_memory_capacity, fmt_percent, fmt_pp, fmt_qty, fmt_usd
