...
\noindent
![](images/png/cover_compute_infrastructure_blueprint_labeled_print.png){fig-alt="Layered compute infrastructure cutaway from accelerator chip and HBM to server nodes, racks, power delivery, and cooling loops."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_compute_infrastructure_blueprint_labeled.webp){fig-alt="Layered compute infrastructure cutaway from accelerator chip and HBM to server nodes, racks, power delivery, and cooling loops."}
:::
...
Machine learning systems have a physical reality beyond their code. While the algorithms for training large models may be public, the ability to execute them at scale is constrained by infrastructure. Training must operate within thermal limits, power-delivery capacity, and data-movement costs. At large scale, progress therefore depends partly on who can build and operate the physical systems that make scale possible. Building an ML fleet requires engineering across four physical levels: the accelerator (silicon physics), the node (interconnect density), the rack (thermal management), and the pod (warehouse-scale networking). At each level, the goal is to reduce time spent moving data and increase time spent on useful computation. A large accelerator cluster also represents major capital expenditure and megawatts of continuous power, making infrastructure a participation boundary for organizations working at the largest scales. That physical stack spans from stacked memory dies to grid-scale power delivery. In C³ terms, this substrate establishes the compute and communication bounds examined in subsequent chapters.
...
- Select accelerator infrastructure using sustained throughput, power, workload fit, and inputs to fleet ownership analysis
...
The path from transistor to data center is anchored by the `{python} InfraSetup.frontier_params_b_str` model, which serves as the thread connecting each level.
...
```{=latex}
\providecolor{mlsysblue}{RGB}{31, 64, 122}
\providecolor{mlsysnavy}{RGB}{20, 41, 78}
\providecolor{mlsysbluelight}{RGB}{234, 241, 248}
\providecolor{mlsysamber}{RGB}{217, 119, 6}
\providecolor{mlsysamberdark}{RGB}{180, 83, 9}
\providecolor{mlsysamberlight}{RGB}{254, 243, 199}
\providecolor{mlsyspurple}{RGB}{124, 58, 237}
\providecolor{mlsyspurpledark}{RGB}{109, 40, 217}
\providecolor{mlsyspurplelight}{RGB}{243, 232, 255}
\providecolor{mlsysred}{RGB}{220, 38, 38}
\providecolor{mlsysreddark}{RGB}{185, 28, 28}
\providecolor{mlsysredlight}{RGB}{254, 226, 226}
\providecolor{mlsysslate}{RGB}{100, 116, 139}
\providecolor{mlsysdark}{RGB}{15, 23, 42}
\providecolor{mlsyssub}{RGB}{71, 85, 105}
\providecolor{mlsysline}{RGB}{148, 163, 184}
\providecolor{mlsyscardbg}{RGB}{248, 250, 252}
\providecolor{mlsysborder}{RGB}{203, 213, 225}
...
\begin{tikzpicture}[
  ...
  % Coordinates for 4 column centers: x = 1.8, 5.6, 9.4, 13.2
  % Total width = 15.0 cm (x = 0 to 15.0)

  % =========================================================================
  % WALL 1: POWER WALL (x = 1.8)
  % =========================================================================
  % Card background
  \draw[fill=mlsyscardbg, draw=mlsysborder, rounded corners=4pt, line width=0.9pt] (0.1, 4.0) rectangle (3.5, 9.6);
  % Header banner
  \fill[fill=mlsysamberlight, rounded corners=4pt] (0.1, 8.85) rectangle (3.5, 9.6);
  \fill[fill=mlsysamberlight] (0.1, 8.85) rectangle (3.5, 9.0);
  \draw[mlsysamber, line width=0.9pt] (0.1, 8.85) -- (3.5, 8.85);
  \node at (1.8, 9.22) {\fontsize{9.5}{11}\selectfont\bfseries\color{mlsysamberdark} 1. Power Wall};

  % Subhead
  \node at (1.8, 8.38) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysdark} Datacenter Delivery}\\[1.5pt]
    {\fontsize{7}{8.5}\selectfont\color{mlsysslate} Thermal \& MW limits}
  };
  \draw[color=mlsysborder!70, line width=0.5pt] (0.4, 7.85) -- (3.2, 7.85);

  % Metric badge
  \node[fill=mlsysamberlight!60, draw=mlsysamber, rounded corners=3pt, line width=0.7pt, inner xsep=4pt, inner ysep=3pt] at (1.8, 7.2) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysamberdark} $\sim$40--100 MW}\\[1pt]
    {\fontsize{6.5}{8}\selectfont\color{mlsysamberdark} PUE 1.15 $\cdot$ 100 kW/rack}
  };
  \draw[color=mlsysborder!70, line width=0.5pt] (0.4, 6.55) -- (3.2, 6.55);

  % Mitigations
  \node at (1.8, 5.35) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysdark} Mitigation:}\\[3pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Direct liquid cooling}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} 48V busbar distribution}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Workload power capping}
  };

  % =========================================================================
  % WALL 2: MEMORY WALL (x = 5.6)
  % =========================================================================
  \draw[fill=mlsyscardbg, draw=mlsysborder, rounded corners=4pt, line width=0.9pt] (3.9, 4.0) rectangle (7.3, 9.6);
  % Header banner
  \fill[fill=mlsysbluelight, rounded corners=4pt] (3.9, 8.85) rectangle (7.3, 9.6);
  \fill[fill=mlsysbluelight] (3.9, 8.85) rectangle (7.3, 9.0);
  \draw[mlsysblue, line width=0.9pt] (3.9, 8.85) -- (7.3, 8.85);
  \node at (5.6, 9.22) {\fontsize{9.5}{11}\selectfont\bfseries\color{mlsysnavy} 2. Memory Wall};

  % Subhead
  \node at (5.6, 8.38) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysdark} Bandwidth \& Capacity}\\[1.5pt]
    {\fontsize{7}{8.5}\selectfont\color{mlsysslate} DRAM vs compute gap}
  };
  \draw[color=mlsysborder!70, line width=0.5pt] (4.2, 7.85) -- (7.0, 7.85);

  % Metric badge
  \node[fill=mlsysbluelight!60, draw=mlsysblue, rounded corners=3pt, line width=0.7pt, inner xsep=4pt, inner ysep=3pt] at (5.6, 7.2) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysnavy} 3.35 TB/s vs 2 PFLOP}\\[1pt]
    {\fontsize{6.5}{8}\selectfont\color{mlsysnavy} 80--141 GB HBM capacity}
  };
  \draw[color=mlsysborder!70, line width=0.5pt] (4.2, 6.55) -- (7.0, 6.55);

  % Mitigations
  \node at (5.6, 5.35) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysdark} Mitigation:}\\[3pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} HBM3 3D-stacking}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} 2D block tiling in SRAM}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} ZeRO-3 weight sharding}
  };

  % =========================================================================
  % WALL 3: COMM. WALL (x = 9.4)
  % =========================================================================
  \draw[fill=mlsyscardbg, draw=mlsysborder, rounded corners=4pt, line width=0.9pt] (7.7, 4.0) rectangle (11.1, 9.6);
  % Header banner
  \fill[fill=mlsyspurplelight, rounded corners=4pt] (7.7, 8.85) rectangle (11.1, 9.6);
  \fill[fill=mlsyspurplelight] (7.7, 8.85) rectangle (11.1, 9.0);
  \draw[mlsyspurple, line width=0.9pt] (7.7, 8.85) -- (11.1, 8.85);
  \node at (9.4, 9.22) {\fontsize{9.5}{11}\selectfont\bfseries\color{mlsyspurpledark} 3. Comm. Wall};

  % Subhead
  \node at (9.4, 8.38) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysdark} Bisection Fabric}\\[1.5pt]
    {\fontsize{7}{8.5}\selectfont\color{mlsysslate} Cluster networking limit}
  };
  \draw[color=mlsysborder!70, line width=0.5pt] (8.0, 7.85) -- (10.8, 7.85);

  % Metric badge
  \node[fill=mlsyspurplelight!60, draw=mlsyspurple, rounded corners=3pt, line width=0.7pt, inner xsep=4pt, inner ysep=3pt] at (9.4, 7.2) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsyspurpledark} 400--800 Gb/s}\\[1pt]
    {\fontsize{6.5}{8}\selectfont\color{mlsyspurpledark} AllReduce barrier latency}
  };
  \draw[color=mlsysborder!70, line width=0.5pt] (8.0, 6.55) -- (10.8, 6.55);

  % Mitigations
  \node at (9.4, 5.35) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysdark} Mitigation:}\\[3pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Fat-tree rail optimization}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Overlap comm \& compute}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Hierarchical ring reduction}
  };

  % =========================================================================
  % WALL 4: RELIABILITY WALL (x = 13.2)
  % =========================================================================
  \draw[fill=mlsyscardbg, draw=mlsysborder, rounded corners=4pt, line width=0.9pt] (11.5, 4.0) rectangle (14.9, 9.6);
  % Header banner
  \fill[fill=mlsysredlight, rounded corners=4pt] (11.5, 8.85) rectangle (14.9, 9.6);
  \fill[fill=mlsysredlight] (11.5, 8.85) rectangle (14.9, 9.0);
  \draw[mlsysred, line width=0.9pt] (11.5, 8.85) -- (14.9, 8.85);
  \node at (13.2, 9.22) {\fontsize{9.5}{11}\selectfont\bfseries\color{mlsysreddark} 4. Reliability Wall};

  % Subhead
  \node at (13.2, 8.38) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysdark} MTBF Scale Collapse}\\[1.5pt]
    {\fontsize{7}{8.5}\selectfont\color{mlsysslate} Fleet availability limit}
  };
  \draw[color=mlsysborder!70, line width=0.5pt] (11.8, 7.85) -- (14.6, 7.85);

  % Metric badge
  \node[fill=mlsysredlight!60, draw=mlsysred, rounded corners=3pt, line width=0.7pt, inner xsep=4pt, inner ysep=3pt] at (13.2, 7.2) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysreddark} MTBF $<$ 2--5 Hours}\\[1pt]
    {\fontsize{6.5}{8}\selectfont\color{mlsysreddark} At 10K--25K GPU scale}
  };
  \draw[color=mlsysborder!70, line width=0.5pt] (11.8, 6.55) -- (14.6, 6.55);

  % Mitigations
  \node at (13.2, 5.35) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysdark} Mitigation:}\\[3pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Async multi-tier ckpt}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} N+1 hot standby spare}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Elastic training restart}
  };

  % =========================================================================
  % TIMELINE ROADWAY (SCALING JOURNEY)
  % =========================================================================
  \draw[fill=mlsyscardbg, draw=mlsysborder, rounded corners=3pt, line width=0.8pt] (0.1, 3.2) rectangle (14.9, 3.65);
  \draw[color=mlsysborder, line width=1pt, dashed] (0.2, 3.425) -- (14.2, 3.425);
  \draw[-{Stealth[scale=0.9]}, color=mlsysslate, line width=1.3pt] (14.2, 3.425) -- (14.8, 3.425);
  \node[fill=mlsyscardbg, inner xsep=6pt] at (7.5, 3.425) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysslate} SCALING JOURNEY}
  };

  % Scale level badges & connecting lines
  % Level 1: Facility Level (x = 1.8)
  \draw[color=mlsysamber, line width=1pt] (1.8, 3.2) -- (1.8, 2.7);
  \node[fill=mlsysamberlight, draw=mlsysamber, rounded corners=2pt, line width=0.7pt, inner xsep=5pt, inner ysep=2.5pt] at (1.8, 2.5) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysamberdark} Facility Level}
  };

  % Level 2: Device / SM Level (x = 5.6)
  \draw[color=mlsysblue, line width=1pt] (5.6, 3.2) -- (5.6, 2.7);
  \node[fill=mlsysbluelight, draw=mlsysblue, rounded corners=2pt, line width=0.7pt, inner xsep=5pt, inner ysep=2.5pt] at (5.6, 2.5) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysnavy} Device / SM Level}
  };

  % Level 3: Cluster Level (x = 9.4)
  \draw[color=mlsyspurple, line width=1pt] (9.4, 3.2) -- (9.4, 2.7);
  \node[fill=mlsyspurplelight, draw=mlsyspurple, rounded corners=2pt, line width=0.7pt, inner xsep=5pt, inner ysep=2.5pt] at (9.4, 2.5) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsyspurpledark} Cluster Level}
  };

  % Level 4: Fleet Scale (x = 13.2)
  \draw[color=mlsysred, line width=1pt] (13.2, 3.2) -- (13.2, 2.7);
  \node[fill=mlsysredlight, draw=mlsysred, rounded corners=2pt, line width=0.7pt, inner xsep=5pt, inner ysep=2.5pt] at (13.2, 2.5) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysreddark} Fleet Scale}
  };

  % Horizontal axis line with dots
  \draw[color=mlsysborder, line width=0.9pt] (0.6, 1.7) -- (14.4, 1.7);
  \foreach \x/\lbl in {1.8/{Facility (100 MW)}, 5.6/{1--8 GPUs (Node)}, 9.4/{1,024 GPUs (Pod)}, 13.2/{10K--25K+ GPUs}} {
    \fill[mlsysslate] (\x, 1.7) circle (2.5pt);
    \node[anchor=north, text=mlsysslate] at (\x, 1.55) {\fontsize{7}{8.5}\selectfont \lbl};
  }

  % =========================================================================
  % BOTTOM TAKEAWAY BANNER
  % =========================================================================
  \node[fill=mlsyscardbg, draw=mlsysborder, rounded corners=4pt, line width=0.8pt, inner sep=6pt, text width=14.4cm] at (7.5, 0.4) {
    {\fontsize{8}{10}\selectfont\bfseries\color{mlsysdark} The Hierarchical Invariant: Each wall creates the dominant design constraint at its operational scale.}\\[2.5pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} SRAM tiling mitigates the memory wall, fat-tree fabric bypasses the comm wall, and hot-spare elasticity overcomes the reliability wall.}
  };
...
[^fn-systolic-array-tpu]: **Systolic array**\index{Systolic array!etymology}: From Greek *systole* (contraction), the systolic-array model was introduced by H. T. Kung and Charles E. Leiserson [@kung1979systolic; @kung1982], where data pulses through a grid of processing elements in a heart-like rhythm. Each cell performs one MAC and passes partial results directly to its neighbor, eliminating register-file round-trips. Google's TPU v1 deployed a $256{\times}256$ array (65,536 MACs), achieving 92 TOPS within 75 W by hardwiring this dataflow to make data center-scale inference economically viable [@jouppi2017].
...
::: {#fig-wafer-scale-engine fig-env="figure" fig-pos="htb" fig-cap="**Wafer-scale engine (WSE) architecture**: Unlike a cluster assembled from discrete packages and boards, a WSE uses most of a 300 mm wafer as one routed compute fabric. Defect-aware software routes around unusable regions, while the on-wafer mesh and distributed SRAM replace off-package transfers for mapped operations." fig-alt="Side-by-side comparison. Left: 8-GPU cluster, NVLink 900 GB/s intra-node, InfiniBand 400 Gb/s inter-node, and microsecond cross-chip latency. Right: WSE-3 wafer fabric with 900,000 cores, 44 GB of distributed SRAM, 21 PB/s aggregate memory bandwidth, and on-wafer links."}
...
\providecolor{mlsysamberdark}{RGB}{180, 83, 9}
\providecolor{mlsysamberlight}{RGB}{254, 243, 199}
\providecolor{mlsysgreen}{RGB}{22, 163, 74}
\providecolor{mlsysgreendark}{RGB}{21, 128, 61}
\providecolor{mlsysgreenlight}{RGB}{220, 252, 231}
\providecolor{mlsysred}{RGB}{220, 38, 38}
\providecolor{mlsysreddark}{RGB}{185, 28, 28}
\providecolor{mlsysredlight}{RGB}{254, 226, 226}
\providecolor{mlsysslate}{RGB}{100, 116, 139}
...
\begin{tikzpicture}[
  ...
  \draw[dashed, color=mlsysline, line width=0.9pt] (7.5, -0.4) -- (7.5, 7.1);
  \node[fill=white, inner sep=3pt] at (7.5, 7.4) {
    {\fontsize{9}{11}\selectfont\bfseries\color{mlsysslate} VS.}
  };

  % =========================================================================
  % LEFT: GPU CLUSTER (8 GPUs)
  % =========================================================================
  \node at (3.6, 7.4) {
    {\fontsize{11}{13}\selectfont\bfseries\color{mlsysdark} GPU Cluster (8 GPUs)}
  };

  % NVLink Grid Lines (Drawn behind the GPUs)
  \begin{scope}[color=mlsysamber, line width=1.4pt]
    \draw (1.35, 6.2) -- (5.85, 6.2);
    \draw (1.35, 5.1) -- (5.85, 5.1);
    \draw (1.35, 6.2) -- (1.35, 5.1);
    \draw (2.85, 6.2) -- (2.85, 5.1);
    \draw (4.35, 6.2) -- (4.35, 5.1);
    \draw (5.85, 6.2) -- (5.85, 5.1);
  \end{scope}

  % 8 GPUs (2 rows x 4 cols)
  % Row 1: y = 6.2
  \node[gpubox] (gpu0) at (1.35, 6.2) {
    {\fontsize{7}{8}\selectfont\bfseries\color{mlsysnavy} GPU 0}\\[1pt]
    {\fontsize{5.5}{7}\selectfont\color{mlsysslate} 80 GB HBM}
  };
  \node[gpubox] (gpu1) at (2.85, 6.2) {
    {\fontsize{7}{8}\selectfont\bfseries\color{mlsysnavy} GPU 1}\\[1pt]
    {\fontsize{5.5}{7}\selectfont\color{mlsysslate} 80 GB HBM}
  };
  \node[gpubox] (gpu2) at (4.35, 6.2) {
    {\fontsize{7}{8}\selectfont\bfseries\color{mlsysnavy} GPU 2}\\[1pt]
    {\fontsize{5.5}{7}\selectfont\color{mlsysslate} 80 GB HBM}
  };
  \node[gpubox] (gpu3) at (5.85, 6.2) {
    {\fontsize{7}{8}\selectfont\bfseries\color{mlsysnavy} GPU 3}\\[1pt]
    {\fontsize{5.5}{7}\selectfont\color{mlsysslate} 80 GB HBM}
  };

  % Row 2: y = 5.1
  \node[gpubox] (gpu4) at (1.35, 5.1) {
    {\fontsize{7}{8}\selectfont\bfseries\color{mlsysnavy} GPU 4}\\[1pt]
    {\fontsize{5.5}{7}\selectfont\color{mlsysslate} 80 GB HBM}
  };
  \node[gpubox] (gpu5) at (2.85, 5.1) {
    {\fontsize{7}{8}\selectfont\bfseries\color{mlsysnavy} GPU 5}\\[1pt]
    {\fontsize{5.5}{7}\selectfont\color{mlsysslate} 80 GB HBM}
  };
  \node[gpubox] (gpu6) at (4.35, 5.1) {
    {\fontsize{7}{8}\selectfont\bfseries\color{mlsysnavy} GPU 6}\\[1pt]
    {\fontsize{5.5}{7}\selectfont\color{mlsysslate} 80 GB HBM}
  };
  \node[gpubox] (gpu7) at (5.85, 5.1) {
    {\fontsize{7}{8}\selectfont\bfseries\color{mlsysnavy} GPU 7}\\[1pt]
    {\fontsize{5.5}{7}\selectfont\color{mlsysslate} 80 GB HBM}
  };

  % NVLink badge
  \node[fill=mlsysamberlight, draw=mlsysamber, rounded corners=2pt, line width=0.8pt, inner xsep=6pt, inner ysep=2.5pt] at (3.6, 4.25) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysamberdark} NVLink 900 GB/s (intra-node)}
  };

  % Chip-to-chip boundary note
  \node at (3.6, 3.4) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysred} Chip-to-chip boundary}\\[1.5pt]
    {\fontsize{7}{8.5}\selectfont\color{mlsysslate} Separate dies on PCB $\cdot$ Latency: $\sim\mu\text{s}$}
  };

  % Inter-node InfiniBand box
  \node[fill=mlsysredlight!70, draw=mlsysred, rounded corners=3pt, line width=0.8pt, inner xsep=8pt, inner ysep=4pt] at (3.6, 2.15) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysreddark} Inter-node: InfiniBand 400 Gb/s}\\[2pt]
    {\fontsize{7}{8.5}\selectfont\color{mlsysdark} Network traversal required for}\\[1pt]
    {\fontsize{7}{8.5}\selectfont\color{mlsysdark} cross-node communication}
  };

  % Key difference callout
  \node[fill=white, draw=mlsysred, rounded corners=3pt, line width=1.1pt, inner xsep=8pt, inner ysep=4pt] at (3.6, 0.6) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysreddark} Key difference}\\[1.5pt]
    {\fontsize{7}{8.5}\selectfont\color{mlsysdark} WSE eliminates chip-to-chip crossings}\\[1pt]
    {\fontsize{7}{8.5}\selectfont\color{mlsysdark} making the cluster the chip}
  };

  % =========================================================================
  % RIGHT: WSE-3 (Single Wafer)
  % =========================================================================
  \node at (11.4, 7.4) {
    {\fontsize{11}{13}\selectfont\bfseries\color{mlsysdark} WSE-3 (Single Wafer)}
  };

  % Wafer circle & Clipped Core Grid
  \begin{scope}
    \draw[fill=mlsysgreenlight!40, draw=mlsysgreen, line width=1.4pt] (11.4, 4.9) circle (1.95cm);
    \clip (11.4, 4.9) circle (1.92cm);
    \foreach \ix in {-5, -4, ..., 5} {
      \foreach \iy in {-5, -4, ..., 5} {
        \draw[fill=mlsysgreenlight, draw=mlsysgreen!80, line width=0.5pt, rounded corners=0.5pt]
          ($(11.4, 4.9) + (\ix*0.33 - 0.13, \iy*0.33 - 0.13)$) rectangle ++(0.26, 0.26);
      }
    }
  \end{scope}

  % On-chip mesh label
  \node[fill=mlsysgreenlight, draw=mlsysgreen, rounded corners=2pt, line width=0.8pt, inner xsep=6pt, inner ysep=3pt] at (11.4, 2.25) {
    {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysgreendark} On-chip 2D mesh network}
  };

  % WSE-3 Specs Card
  \node[fill=mlsyscardbg, draw=mlsysborder, rounded corners=4pt, line width=0.9pt, inner sep=6pt, text width=6.2cm] at (11.4, 0.75) {
    {\fontsize{8.5}{10.5}\selectfont\bfseries\color{mlsysdark} WSE-3 Specifications}\\[3pt]
    {\fontsize{7.5}{9.5}\selectfont\color{mlsysslate} 900,000 AI cores $\mid$ 44 GB on-chip SRAM}\\[2pt]
    {\fontsize{7.5}{9.5}\selectfont\color{mlsysslate} 21 PB/s memory BW $\mid$ No chip boundaries}\\[2.5pt]
    {\fontsize{7.5}{9.5}\selectfont\bfseries\color{mlsysgreendark} Latency: nanoseconds (vs.\ microseconds)}
  };
...
::: {#fig-accelerator-spectrum fig-env="figure" fig-pos="htb" fig-cap="**The accelerator spectrum**: ML hardware design involves a fundamental trade-off between programmability and efficiency. General-purpose CPUs provide maximum flexibility but low compute density. Moving toward custom ASICs and systolic arrays increases throughput per watt by hardwiring common dataflows, at the cost of supporting a narrower range of model architectures." fig-alt="Four pie charts along a General-Purpose to Maximum-Efficiency axis showing die-area splits: CPU (90 percent Control, 10 percent Arithmetic), GPU (30/55), TPU (20/75), Custom ASIC (8 percent Control, over 90 percent Arithmetic)."}
```{=latex}
\providecolor{mlsysblue}{RGB}{31, 64, 122}
\providecolor{mlsysnavy}{RGB}{20, 41, 78}
\providecolor{mlsysbluelight}{RGB}{212, 226, 240}
\providecolor{mlsysamber}{RGB}{217, 119, 6}
\providecolor{mlsysamberdark}{RGB}{180, 83, 9}
\providecolor{mlsysamberlight}{RGB}{254, 243, 199}
\providecolor{mlsysgreen}{RGB}{22, 163, 74}
...
\providecolor{mlsyscardbg}{RGB}{248, 250, 252}
\providecolor{mlsysborder}{RGB}{203, 213, 225}
\providecolor{mlsyscontrollight}{RGB}{226, 232, 240}
...
\begin{tikzpicture}[
  ...
  % Coordinates: Total width 15.0cm (0 to 15.0)
  % 4 column centers: x = 1.8, 5.6, 9.4, 13.2

  % =========================================================================
  % TOP ARROW & TAX REDUCTION
  % =========================================================================
  \draw[line width=1.1pt, color=mlsysborder, ->] (0.4, 9.7) -- (14.6, 9.7);
  \node[anchor=west] at (0.4, 9.35) {\fontsize{8}{10}\selectfont\color{mlsysslate} General-Purpose};
  \node[anchor=east] at (14.6, 9.35) {\fontsize{8.5}{10}\selectfont\bfseries\color{mlsysdark} Maximum Compute Density};

  % Centered Generality Tax Arrow
  \draw[line width=1.1pt, dashed, color=mlsysamberdark, ->] (5.5, 9.35) -- (9.5, 9.35);
  \node[anchor=south] at (7.5, 9.4) {\fontsize{7.5}{9}\selectfont\bfseries\color{mlsysamberdark} Generality Tax eliminated $\to$};

  % =========================================================================
  % CARD 1: CPU (x = 1.8)
  % =========================================================================
  \draw[fill=mlsyscardbg, draw=mlsysborder, rounded corners=4pt, line width=0.9pt] (0.1, 1.2) rectangle (3.5, 8.8);
  \fill[fill=mlsysborder!40, rounded corners=4pt] (0.1, 8.15) rectangle (3.5, 8.8);
  \fill[fill=mlsysborder!40] (0.1, 8.15) rectangle (3.5, 8.3);
  \draw[mlsysborder, line width=0.8pt] (0.1, 8.15) -- (3.5, 8.15);
  \node at (1.8, 8.48) {\fontsize{10.5}{12}\selectfont\bfseries\color{mlsysnavy} CPU};

  \node at (1.8, 7.75) {\fontsize{8}{9.5}\selectfont\color{mlsysslate} General-Purpose};

  % CPU Pie / Donut
  \begin{scope}[shift={(1.8, 6.15)}]
    \filldraw[fill=mlsyscontrollight, draw=mlsysslate, line width=0.8pt]
      (0, 0) -- (90:1.15) arc (90:-234:1.15) -- cycle;
    \filldraw[fill=mlsysbluelight, draw=mlsysblue, line width=1pt]
      (0, 0) -- (126:1.15) arc (126:90:1.15) -- cycle;
    \filldraw[fill=white, draw=mlsysborder, line width=0.7pt] (0, 0) circle (0.38);
    \node at (0, 0) {\fontsize{7}{8}\selectfont\bfseries\color{mlsyssub} Die};
  \end{scope}

  % CPU Legend (Widened)
  \begin{scope}[shift={(0.15, 4.45)}]
    \filldraw[fill=mlsyscontrollight, draw=mlsysslate, rounded corners=1.5pt, line width=0.6pt] (0, 0.25) rectangle (0.22, 0.47);
    \node[anchor=west] at (0.28, 0.36) {\fontsize{6.8}{8}\selectfont\color{mlsyssub} Control / Cache};
    \node[anchor=east] at (3.15, 0.36) {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysdark} $\sim$90\%};

    \filldraw[fill=mlsysbluelight, draw=mlsysblue, rounded corners=1.5pt, line width=0.6pt] (0, -0.15) rectangle (0.22, 0.07);
    \node[anchor=west] at (0.28, -0.04) {\fontsize{7}{8.5}\selectfont\color{mlsyssub} Arithmetic};
    \node[anchor=east] at (3.15, -0.04) {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysnavy} $\sim$10\%};
  \end{scope}

  % CPU Details
  \node at (1.8, 2.5) {
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Branch predictors}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Out-of-order execution}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Multi-level SRAM cache}\\[4pt]
    {\fontsize{7.5}{9.5}\selectfont\bfseries\color{mlsysnavy} Full programmability}
  };

  % =========================================================================
  % CARD 2: GPU (x = 5.6)
  % =========================================================================
  \draw[fill=mlsyscardbg, draw=mlsysborder, rounded corners=4pt, line width=0.9pt] (3.9, 1.2) rectangle (7.3, 8.8);
  \fill[fill=mlsysborder!40, rounded corners=4pt] (3.9, 8.15) rectangle (7.3, 8.8);
  \fill[fill=mlsysborder!40] (3.9, 8.15) rectangle (7.3, 8.3);
  \draw[mlsysborder, line width=0.8pt] (3.9, 8.15) -- (7.3, 8.15);
  \node at (5.6, 8.48) {\fontsize{10.5}{12}\selectfont\bfseries\color{mlsysnavy} GPU};

  \node at (5.6, 7.75) {\fontsize{8}{9.5}\selectfont\color{mlsysslate} SIMT Parallel};

  % GPU Pie / Donut
  \begin{scope}[shift={(5.6, 6.15)}]
    \filldraw[fill=mlsyscontrollight, draw=mlsysslate, line width=0.8pt]
      (0, 0) -- (90:1.15) arc (90:-36:1.15) -- cycle;
    \filldraw[fill=mlsysamberlight, draw=mlsysamber, line width=0.8pt]
      (0, 0) -- (-36:1.15) arc (-36:-72:1.15) -- cycle;
    \filldraw[fill=mlsysbluelight, draw=mlsysblue, line width=1pt]
      (0, 0) -- (-72:1.15) arc (-72:-270:1.15) -- cycle;
    \filldraw[fill=white, draw=mlsysborder, line width=0.7pt] (0, 0) circle (0.38);
    \node at (0, 0) {\fontsize{7}{8}\selectfont\bfseries\color{mlsyssub} Die};
  \end{scope}

  % GPU Legend
  \begin{scope}[shift={(4.05, 4.45)}]
    \filldraw[fill=mlsyscontrollight, draw=mlsysslate, rounded corners=1.5pt, line width=0.6pt] (0, 0.25) rectangle (0.22, 0.47);
    \node[anchor=west] at (0.3, 0.36) {\fontsize{7}{8.5}\selectfont\color{mlsyssub} Control};
    \node[anchor=east] at (2.95, 0.36) {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysdark} $\sim$35\%};

    \filldraw[fill=mlsysbluelight, draw=mlsysblue, rounded corners=1.5pt, line width=0.6pt] (0, -0.15) rectangle (0.22, 0.07);
    \node[anchor=west] at (0.3, -0.04) {\fontsize{7}{8.5}\selectfont\color{mlsyssub} Arithmetic};
    \node[anchor=east] at (2.95, -0.04) {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysnavy} $\sim$55\%};
  \end{scope}

  % GPU Details
  \node at (5.6, 2.5) {
    {\fontsize{7}{9}\selectfont\color{mlsysslate} SIMT warp execution}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Hardware warp schedulers}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} High-speed HBM bus}\\[4pt]
    {\fontsize{7.5}{9.5}\selectfont\bfseries\color{mlsysnavy} Massive data parallelism}
  };

  % =========================================================================
  % CARD 3: TPU (x = 9.4)
  % =========================================================================
  \draw[fill=mlsyscardbg, draw=mlsysborder, rounded corners=4pt, line width=0.9pt] (7.7, 1.2) rectangle (11.1, 8.8);
  \fill[fill=mlsysborder!40, rounded corners=4pt] (7.7, 8.15) rectangle (11.1, 8.8);
  \fill[fill=mlsysborder!40] (7.7, 8.15) rectangle (11.1, 8.3);
  \draw[mlsysborder, line width=0.8pt] (7.7, 8.15) -- (11.1, 8.15);
  \node at (9.4, 8.48) {\fontsize{10.5}{12}\selectfont\bfseries\color{mlsysnavy} TPU};

  \node at (9.4, 7.75) {\fontsize{8}{9.5}\selectfont\color{mlsysslate} Systolic Matrix Unit};

  % TPU Pie / Donut
  \begin{scope}[shift={(9.4, 6.15)}]
    \filldraw[fill=mlsyscontrollight, draw=mlsysslate, line width=0.8pt]
      (0, 0) -- (90:1.15) arc (90:18:1.15) -- cycle;
    \filldraw[fill=mlsysamberlight, draw=mlsysamber, line width=0.8pt]
      (0, 0) -- (18:1.15) arc (18:0:1.15) -- cycle;
    \filldraw[fill=mlsysbluelight, draw=mlsysblue, line width=1pt]
      (0, 0) -- (0:1.15) arc (0:-270:1.15) -- cycle;
    \filldraw[fill=white, draw=mlsysborder, line width=0.7pt] (0, 0) circle (0.38);
    \node at (0, 0) {\fontsize{7}{8}\selectfont\bfseries\color{mlsyssub} Die};
  \end{scope}

  % TPU Legend
  \begin{scope}[shift={(7.85, 4.45)}]
    \filldraw[fill=mlsyscontrollight, draw=mlsysslate, rounded corners=1.5pt, line width=0.6pt] (0, 0.25) rectangle (0.22, 0.47);
    \node[anchor=west] at (0.3, 0.36) {\fontsize{7}{8.5}\selectfont\color{mlsyssub} Control};
    \node[anchor=east] at (2.95, 0.36) {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysdark} $\sim$20\%};

    \filldraw[fill=mlsysbluelight, draw=mlsysblue, rounded corners=1.5pt, line width=0.6pt] (0, -0.15) rectangle (0.22, 0.07);
    \node[anchor=west] at (0.3, -0.04) {\fontsize{7}{8.5}\selectfont\color{mlsyssub} Arithmetic};
    \node[anchor=east] at (2.95, -0.04) {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysnavy} $\sim$75\%};
  \end{scope}

  % TPU Details
  \node at (9.4, 2.5) {
    {\fontsize{7}{9}\selectfont\color{mlsysslate} 2D systolic array}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Zero instruction decode}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Direct tensor streaming}\\[4pt]
    {\fontsize{7.5}{9.5}\selectfont\bfseries\color{mlsysnavy} High energy density}
  };

  % =========================================================================
  % CARD 4: CUSTOM ASIC (x = 13.2)
  % =========================================================================
  \draw[fill=mlsyscardbg, draw=mlsysborder, rounded corners=4pt, line width=0.9pt] (11.5, 1.2) rectangle (14.9, 8.8);
  \fill[fill=mlsysborder!40, rounded corners=4pt] (11.5, 8.15) rectangle (14.9, 8.8);
  \fill[fill=mlsysborder!40] (11.5, 8.15) rectangle (14.9, 8.3);
  \draw[mlsysborder, line width=0.8pt] (11.5, 8.15) -- (14.9, 8.15);
  \node at (13.2, 8.48) {\fontsize{10.5}{12}\selectfont\bfseries\color{mlsysnavy} Custom ASIC};

  \node at (13.2, 7.75) {\fontsize{8}{9.5}\selectfont\color{mlsysslate} Hardwired Dataflow};

  % ASIC Pie / Donut
  \begin{scope}[shift={(13.2, 6.15)}]
    \filldraw[fill=mlsyscontrollight, draw=mlsysslate, line width=0.8pt]
      (0, 0) -- (90:1.15) arc (90:61.2:1.15) -- cycle;
    \filldraw[fill=mlsysbluelight, draw=mlsysblue, line width=1pt]
      (0, 0) -- (61.2:1.15) arc (61.2:-270:1.15) -- cycle;
    \filldraw[fill=white, draw=mlsysborder, line width=0.7pt] (0, 0) circle (0.38);
    \node at (0, 0) {\fontsize{7}{8}\selectfont\bfseries\color{mlsyssub} Die};
  \end{scope}

  % ASIC Legend
  \begin{scope}[shift={(11.65, 4.45)}]
    \filldraw[fill=mlsyscontrollight, draw=mlsysslate, rounded corners=1.5pt, line width=0.6pt] (0, 0.25) rectangle (0.22, 0.47);
    \node[anchor=west] at (0.3, 0.36) {\fontsize{7}{8.5}\selectfont\color{mlsyssub} Control};
    \node[anchor=east] at (2.95, 0.36) {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysdark} $\sim$8\%};

    \filldraw[fill=mlsysbluelight, draw=mlsysblue, rounded corners=1.5pt, line width=0.6pt] (0, -0.15) rectangle (0.22, 0.07);
    \node[anchor=west] at (0.3, -0.04) {\fontsize{7}{8.5}\selectfont\color{mlsyssub} Arithmetic};
    \node[anchor=east] at (2.95, -0.04) {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysnavy} $>$90\%};
  \end{scope}

  % ASIC Details
  \node at (13.2, 2.5) {
    {\fontsize{7}{9}\selectfont\color{mlsysslate} Hardwired pipeline}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} No general ISA overhead}\\[2pt]
    {\fontsize{7}{9}\selectfont\color{mlsysslate} 5--10$\times$ efficiency gain}\\[4pt]
    {\fontsize{7.5}{9.5}\selectfont\bfseries\color{mlsysnavy} Application-locked}
  };

  % =========================================================================
  % BOTTOM TRADEOFF BAR
  % =========================================================================
  \draw[line width=1pt, color=mlsysborder] (1.8, 0.4) -- (13.2, 0.4);

  \node[fill=mlsysbluelight!50, draw=mlsysblue, rounded corners=2pt, line width=0.8pt, inner xsep=8pt, inner ysep=3.5pt] at (3.2, 0.4) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysnavy} $\leftarrow$ Higher Programmability}
  };

  \node[fill=mlsysgreenlight, draw=mlsysgreen, rounded corners=2pt, line width=0.8pt, inner xsep=8pt, inner ysep=3.5pt] at (11.8, 0.4) {
    {\fontsize{8}{9.5}\selectfont\bfseries\color{mlsysgreendark} Higher Energy Efficiency $\rightarrow$}
  };
...
class ReticleLimitRecap:
    ...
    reticle_width = 26 * ureg.millimeter
    reticle_height = 33 * ureg.millimeter
    reticle_limit = reticle_width * reticle_height
    b200_equivalent_area = 1600 * ureg.millimeter**2
    ...
    b200_equivalent_area_mm2_str = fmt_area(b200_equivalent_area, unit=ureg.millimeter**2, commas=True)
```

A major accelerator design pattern is the **Chiplet**\index{Chiplet} architecture, exemplified by NVIDIA's Blackwell and AMD's Instinct MI300 series. Rather than fabricating a single monolithic die, chiplet-based designs partition the processor into multiple smaller dies connected by a high-bandwidth die-to-die interconnect on a common package substrate. This approach addresses two physical limitations that constrain monolithic designs.

First, the maximum die size is capped by the photolithographic reticle limit (the `{python} ReticleLimitRecap.reticle_limit_mm2_str` single-exposure ceiling examined in @sec-compute-infrastructure-evolution-gpu-architectures-566b), which limits the number of Tensor Cores, SMs, and HBM stacks a monolithic GPU can integrate. Chiplet designs bypass this limit by placing multiple dies on a single package, with the B200's dual-die design effectively creating a `{python} ReticleLimitRecap.b200_equivalent_area_mm2_str` equivalent processor.
...
The Ampere architecture (2020) expanded Tensor Core support to TF32 and BF16, after Turing had introduced INT8 and INT4 Tensor Core modes for quantized inference. The A100 also introduced Multi-Instance GPU (MIG), which partitions a single GPU into up to seven isolated instances, enabling efficient sharing of expensive hardware across multiple inference workloads. The most consequential change for infrastructure was Ampere's third-generation NVLink, which doubled bidirectional bandwidth to `{python} A100Specs.nvlink_gb_per_s_str` per GPU from Volta's `{python} V100NvlinkRecap.nvlink_gb_per_s_str`, directly addressing the communication wall for tensor parallelism, which splits each layer's matrices across GPUs.
...
The Hopper architecture (2022) added the Transformer Engine, whose FP8 recipes and autocast integration run supported Transformer operations in FP8 with managed scaling while retaining higher precision where the configured recipe or implementation requires it. Hopper also introduced NVLink 4.0 at `{python} H100Specs.nvlink_gb_per_s_str` per GPU and the NVLink Switch, enabling NVLink connectivity beyond the 8-GPU boundary.
...
Blackwell also introduced fifth-generation NVLink at `{python} B200NvlinkRecap.nvlink_gb_per_s_str` per GPU, doubling the intra-node bandwidth again. The die-to-die link within the B200 package operates at 10 TB/s, fast enough that the two dies can appear as a single logical GPU to the software for many operations.
...
::: {#fig-gpu-scaling-trends fig-env="figure" fig-pos="htb" fig-cap="**Compute, bandwidth, and power scale at three different rates**: Peak compute, memory bandwidth, and thermal design power for NVIDIA data center accelerators, normalized to the V100. Compute rises 18$\times$, bandwidth 8.9$\times$, and TDP 3.3$\times$, so each generation buys arithmetic faster than it buys the bytes or the watts to feed it. Specifications come from the MLSysIM hardware registry." fig-alt="Line chart with three series normalized to the V100: peak compute reaching 18 times, memory bandwidth 8.9 times, and thermal design power 3.3 times by the B200."}
...
data = pd.read_csv("vol2/02_compute_infrastructure/data/gpu_scaling_trends.csv")
base = data.iloc[0]
...
ax.plot(labels, data["TDP"], marker="^", color=COLORS["GreenLine"], linewidth=2, label="Thermal design power")
...
Unpacking raw throughput and power efficiency from @tbl-gpu-evolution reveals a fundamental hardware divergence (@fig-accelerator-efficiency-wall).
...
ax1.semilogy(years_arr, tflops, "o-", color=COLORS["BlueLine"], markersize=8, linewidth=2, label="Peak Tensor TFLOP/s", zorder=5)
...
ax2 = ax1.twinx()
ax2.semilogy(years_arr, eff, "s-", color=COLORS["GreenLine"], markersize=8, linewidth=2, label="TFLOP/s/W", zorder=5)
...
# —Label each GPU ---
offsets = [(8, -15), (5, -23), (-20, 9), (-5, -28), (10, -5), (-10, 10)]  # P100  # V100  # A100  # H100  # B100  # B200

for i, g in enumerate(gpus):
    ax1.annotate(g, (years[i], tflops[i]), textcoords="offset points", xytext=offsets[i], fontsize=8.5, color=COLORS["BlueLine"])

# —Growth annotations ---
ax1.annotate(
    ">200$\\times$ raw TFLOP/s",
    xy=(2021.9, 3500),
    fontsize=9,
...
# │ How: Read Hardware.Cloud.TPUv5p compute and memory specs.
# │      ICI bandwidth stays a table literal — no registry field yet
# │      (registry gap noted in the 2026-06 audit).
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_memory_capacity, fmt_bandwidth, fmt_flop_rate
...
class TpuV5pRecap:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # (spec recap — values read straight from the registry)
...
This comparison reveals a fundamental divergence in architectural philosophy (@tbl-tpu-evolution). GPU evolution is a race to pack more arithmetic and higher-precision Tensor Cores onto a single package, using chiplets (Blackwell) to overcome physical die-size limits. TPU evolution is a race to build more efficient warehouse-scale computers, where the individual chip's performance is secondary to the pod's collective bandwidth and reconfigurability [@barroso2019].

The H100's arithmetic engine can approach 2,000 TFLOP/s in its advertised low-precision mode, but workloads realize that throughput only when data reaches the compute units fast enough. For many ML workloads, the memory wall sets that limit.

## The memory wall {#sec-compute-memory-wall}

As arithmetic throughput grows faster than data-access speed, compute units can spend more time waiting for operands. This divergence is known as the **memory wall**\index{Memory Wall} [@wulf1995]. While transistor scaling has increased logic performance by orders of magnitude, the interconnects that feed data to those cores have not kept pace. Neural networks can require large weight and activation streams, making bandwidth rather than compute the governing constraint for some inference and training regimes. @Sec-appdx-c3-taxonomy-diagnostic-summary develops the diagnostic framework that classifies this memory-bound condition alongside the other bottleneck patterns a fleet encounters, so that a measured symptom maps to a named constraint rather than a guess.
...
::: {#fig-accelerator-memory-tier-map fig-env="figure" fig-pos="htb" fig-cap="**Accelerator memory tiers**: Peak memory bandwidth and capacity separate local-SRAM designs, HBM accelerators, and edge/shared-LPDDR devices. The dotted arrow marks the design trade-off: moving data closer to compute raises bandwidth but reduces capacity." fig-alt="Log-log scatter plot of memory capacity vs. peak memory bandwidth. Purple star markers appear at low capacity and very high bandwidth, blue circles appear at tens to hundreds of gigabytes and lower bandwidth, and green plus markers appear at lower bandwidth."}
...
fill_log_ellipse((1.15, 2.5e5), 2.45, 2.05, COLORS["VioletLine"])
fill_log_ellipse((105, 2600), 1.55, 1.25, COLORS["BlueLine"])
fill_log_ellipse((18, 82), 1.42, 1.0, COLORS["GreenLine"])

tier_styles = {
    "local SRAM": (COLORS["VioletLine"], "*", 145, 0.95, 5),
    "HBM": (COLORS["BlueLine"], "o", 50, 0.84, 4),
    "edge LPDDR": (COLORS["GreenLine"], "P", 72, 0.90, 4),
}
...
label_offsets = {
    "WSE-3": (5, 0, COLORS["VioletLine"], "left"),
    "Untether speedAI240": (5, -8, COLORS["VioletLine"], "left"),
    "Groq LPU": (5, -3, COLORS["VioletLine"], "left"),
    "Graphcore GC200": (5, -2, COLORS["VioletLine"], "left"),
    "d-Matrix Corsair": (5, 1, COLORS["VioletLine"], "left"),
    "V100": (-15, -13, COLORS["BlueLine"], "right"),
    "H100": (-11, 10, COLORS["BlueLine"], "right"),
    "B200": (6, 7, COLORS["BlueLine"], "left"),
    "Jetson AGX Orin": (5, 2, COLORS["GreenLine"], "left"),
}
...
```{python}
#| echo: false
from mlsysim.fmt import fmt_qty, fmt_memory_capacity
...
```{=latex}
\providecolor{mlsysblue}{RGB}{31, 64, 122}
\providecolor{mlsyslightblue}{RGB}{234, 241, 248}
\providecolor{mlsysmidblue}{RGB}{212, 226, 240}
\providecolor{mlsysdarkblue}{RGB}{20, 41, 78}
\providecolor{mlsysgreen}{RGB}{22, 163, 74}
\providecolor{mlsyslightgreen}{RGB}{220, 252, 231}
\providecolor{mlsysdarkgreen}{RGB}{21, 128, 61}
\providecolor{mlsysred}{RGB}{220, 38, 38}
\providecolor{mlsyslightred}{RGB}{254, 226, 226}
\providecolor{mlsysdarkred}{RGB}{185, 28, 28}
\providecolor{mlsysamber}{RGB}{217, 119, 6}
\providecolor{mlsyslightamber}{RGB}{254, 243, 199}
\providecolor{mlsysdarkamber}{RGB}{180, 83, 9}
\providecolor{mlsysgray}{RGB}{100, 116, 139}
\providecolor{mlsyslightgray}{RGB}{241, 245, 249}
\providecolor{mlsysbordergray}{RGB}{203, 213, 225}
\providecolor{mlsystext}{RGB}{51, 65, 85}

\begin{tikzpicture}[font=\sffamily]

  % =========================================================================
  % COLUMN 1: LARGE MATRIX (OFF-CHIP HBM3)
  % =========================================================================
  \begin{scope}[shift={(0,0)}]
    % Outer box (width 6.8, height 8.8)
    \draw[draw=mlsysbordergray, fill=white, rounded corners=4pt, line width=1.1pt] (0,0) rectangle (6.8, 8.8);
    % Header bar
    \path[fill=mlsyslightgray, rounded corners=4pt] (0, 8.15) rectangle (6.8, 8.8);
    \fill[mlsyslightgray] (0, 8.15) rectangle (6.8, 8.35);
    \draw[draw=mlsysbordergray, line width=0.8pt] (0, 8.15) -- (6.8, 8.15);
    \node at (0.35, 8.48) [anchor=west, font=\fontsize{9}{10.5}\selectfont\bfseries, color=mlsystext] {Large Matrix (Off-Chip HBM)};
    \node at (6.45, 8.48) [anchor=east, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {$4096 \times 4096$};

    % Axis labels
    \node at (3.65, 7.85) [anchor=center, font=\fontsize{7.2}{8.5}\selectfont, color=mlsysgray] {4096 columns ($K$-dim)};
    \node at (0.35, 5.15) [anchor=center, font=\fontsize{7.2}{8.5}\selectfont, color=mlsysgray, rotate=90] {4096 rows ($M$-dim)};

    % Matrix grid: 6x6 tiles from x=0.85 to 6.45 (width 5.6), y=2.65 to 7.65 (height 5.0)
    % tile width = 5.6 / 6 = 0.933, tile height = 5.0 / 6 = 0.833
    \draw[draw=mlsysbordergray, fill=white, line width=0.8pt] (0.85, 2.65) rectangle (6.45, 7.65);

    % Internal grid lines
    \foreach \i in {1,...,5} {
      \draw[draw=mlsyslightgray, line width=0.8pt] ({0.85 + \i*0.933}, 2.65) -- ({0.85 + \i*0.933}, 7.65);
      \draw[draw=mlsyslightgray, line width=0.8pt] (0.85, {2.65 + \i*0.833}) -- (6.45, {2.65 + \i*0.833});
    }

    % Completed tiles (green)
    % Row 6 (top): cols 1, 2, 3
    \foreach \c in {0, 1, 2} {
      \draw[draw=mlsysgreen, fill=mlsyslightgreen, line width=0.7pt, rounded corners=1.5pt]
        ({0.85 + \c*0.933 + 0.02}, {7.65 - 0.833 + 0.02}) rectangle ({0.85 + (\c+1)*0.933 - 0.02}, {7.65 - 0.02});
    }
    % Row 5: cols 1, 2
    \foreach \c in {0, 1} {
      \draw[draw=mlsysgreen, fill=mlsyslightgreen, line width=0.7pt, rounded corners=1.5pt]
        ({0.85 + \c*0.933 + 0.02}, {7.65 - 1.666 + 0.02}) rectangle ({0.85 + (\c+1)*0.933 - 0.02}, {7.65 - 0.833 - 0.02});
    }

    % Active tile: col 3, row 4 (from top: 3rd row, col 3)
    \draw[draw=mlsysblue, fill=mlsyslightblue, line width=1.4pt, rounded corners=1.5pt]
      (2.73, 5.17) rectangle (3.63, 5.96);
    \node at (3.18, 5.68) [font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkblue] {Active};
    \node at (3.18, 5.42) [font=\fontsize{7}{8}\selectfont, color=mlsysdarkblue] {Tile};

    % Tiling Imperative box
    \draw[draw=mlsysbordergray!80, fill=mlsyslightgray!40, rounded corners=3pt, line width=0.7pt] (0.35, 0.3) rectangle (6.45, 2.35);
    \node at (0.6, 2.0) [anchor=west, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsystext] {Tiling Imperative:};
    \node at (0.6, 1.45) [anchor=west, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {\textbf{Footprint:} $4096^2$ FP16 matrix is 32\,MB ($>$ SRAM)};
    \node at (0.6, 0.85) [anchor=west, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {\textbf{Sub-blocks:} Partitioned into $16 \times 16$ / $128 \times 128$ tiles};
  \end{scope}

  % =========================================================================
  % ARROW: MATRIX TO SRAM
  % =========================================================================
  \begin{scope}[shift={(6.8, 0)}]
    \draw[-{Stealth[scale=1.1]}, very thick, color=mlsysblue] (0.15, 5.3) -- (0.65, 5.3);
    \node at (0.4, 5.7) [font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkblue] {Tile};
    \node at (0.4, 4.9) [font=\fontsize{7}{8.5}\selectfont, color=mlsysgray] {Load};
  \end{scope}

  % =========================================================================
  % COLUMN 2: ON-CHIP SRAM & TENSOR CORE
  % =========================================================================
  \begin{scope}[shift={(7.6, 0)}]
    % Outer box (width 6.6, height 8.8)
    \draw[draw=mlsysbordergray, fill=white, rounded corners=4pt, line width=1.1pt] (0,0) rectangle (6.6, 8.8);
    % Header bar
    \path[fill=mlsyslightgray, rounded corners=4pt] (0, 8.15) rectangle (6.6, 8.8);
    \fill[mlsyslightgray] (0, 8.15) rectangle (6.6, 8.35);
    \draw[draw=mlsysbordergray, line width=0.8pt] (0, 8.15) -- (6.6, 8.15);
    \node at (3.3, 8.48) [anchor=center, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsysdarkblue] {On-Chip SRAM \& Tensor Core};

    % Sub-card 1: Shared Memory Buffer
    \draw[draw=mlsysbordergray!80, fill=white, rounded corners=3pt, line width=0.8pt] (0.35, 5.8) rectangle (6.25, 7.95);
    \node at (0.55, 7.65) [anchor=west, font=\fontsize{7.8}{9}\selectfont\bfseries, color=mlsystext] {SRAM Buffer};
    \node at (6.05, 7.65) [anchor=east, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkgreen] {$10\times$ faster than HBM};

    % Tile A
    \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=2pt, line width=1pt] (0.55, 5.95) rectangle (3.15, 7.35);
    \node at (1.85, 7.1) [font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsysdarkblue] {Tile A (Inputs)};
    \node at (1.85, 6.75) [font=\fontsize{7}{8.5}\selectfont, color=mlsystext] {$16 \times 16$ Activations};
    \draw[draw=none, fill=mlsysmidblue, rounded corners=2pt] (0.95, 6.1) rectangle (2.75, 6.5);
    \node at (1.85, 6.3) [font=\fontsize{7}{8}\selectfont\bfseries, color=mlsysdarkblue] {Reused $N$ times};

    % Tile B
    \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=2pt, line width=1pt] (3.45, 5.95) rectangle (6.05, 7.35);
    \node at (4.75, 7.1) [font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsysdarkblue] {Tile B (Weights)};
    \node at (4.75, 6.75) [font=\fontsize{7}{8.5}\selectfont, color=mlsystext] {$16 \times 16$ Parameters};
    \draw[draw=none, fill=mlsysmidblue, rounded corners=2pt] (3.85, 6.1) rectangle (5.65, 6.5);
    \node at (4.75, 6.3) [font=\fontsize{7}{8}\selectfont\bfseries, color=mlsysdarkblue] {Reused $M$ times};

    % Sub-card 2: Tensor Core Processing Unit
    \draw[draw=mlsysblue, fill=white, rounded corners=3pt, line width=1.2pt] (0.35, 2.4) rectangle (6.25, 5.65);
    % Sub-card 2 header
    \path[fill=mlsyslightblue, rounded corners=3pt] (0.35, 5.15) rectangle (6.25, 5.65);
    \fill[mlsyslightblue] (0.35, 5.15) rectangle (6.25, 5.35);
    \draw[draw=mlsysblue, line width=0.8pt] (0.35, 5.15) -- (6.25, 5.15);
    \node at (3.3, 5.4) [anchor=center, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsysdarkblue] {Tensor Core MMA Unit};

    \node at (3.3, 4.75) [anchor=center, font=\fontsize{11}{13}\selectfont\bfseries, color=mlsysblue] {$\mathbf{D = A \times B + C}$};
    \node at (3.3, 4.28) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {4,096 MAC ops per tile ($16 \times 16 \times 16$)};
    \node at (3.3, 3.85) [anchor=center, font=\fontsize{7}{8.5}\selectfont, color=mlsysgray] {FP16 / BF16 inputs $\to$ FP32 Accumulation};

    \draw[draw=mlsysgreen, fill=mlsyslightgreen, rounded corners=2pt, line width=0.8pt] (1.1, 2.8) rectangle (5.5, 3.28);
    \node at (3.3, 3.04) [anchor=center, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkgreen] {High Arithmetic Intensity ($>$150 FLOP/B)};

    % Sub-card 3: FP32 Register Accumulator
    \draw[draw=mlsysbordergray!80, fill=white, rounded corners=3pt, line width=0.8pt] (0.35, 0.3) rectangle (6.25, 2.2);
    \node at (0.55, 1.95) [anchor=west, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsysgreen] {FP32 Register Accumulator};
    \node at (0.55, 1.58) [anchor=west, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {Partial sums stay in fast RF until tile finishes};
    \draw[draw=mlsysbordergray, dashed, line width=0.6pt] (0.55, 1.2) -- (6.05, 1.2);
    \node at (3.3, 0.7) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {Stream back to HBM only after full $K$-reduction};
  \end{scope}

  % =========================================================================
  % COLUMN 3: ENERGY COST / OP
  % =========================================================================
  \begin{scope}[shift={(14.6, 0)}]
    % Outer box (width 3.8, height 8.8)
    \draw[draw=mlsysbordergray, fill=white, rounded corners=4pt, line width=1.1pt] (0,0) rectangle (3.8, 8.8);
    % Header bar
    \path[fill=mlsyslightgray, rounded corners=4pt] (0, 8.15) rectangle (3.8, 8.8);
    \fill[mlsyslightgray] (0, 8.15) rectangle (3.8, 8.35);
    \draw[draw=mlsysbordergray, line width=0.8pt] (0, 8.15) -- (3.8, 8.15);
    \node at (0.35, 8.48) [anchor=west, font=\fontsize{9}{10.5}\selectfont\bfseries, color=mlsystext] {Energy Cost / Op};

    % HBM DRAM Read
    \draw[draw=mlsysred, fill=mlsyslightred, rounded corners=3pt, line width=0.8pt] (0.25, 6.6) rectangle (3.55, 7.95);
    \node at (0.45, 7.65) [anchor=west, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsysdarkred] {HBM DRAM Read};
    \node at (0.45, 7.22) [anchor=west, font=\fontsize{11}{13}\selectfont\bfseries, color=mlsysred] {2--5 pJ / bit};
    \node at (0.45, 6.85) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysgray] {Off-chip capacitive traces};

    % SRAM Access
    \draw[draw=mlsysamber, fill=mlsyslightamber, rounded corners=3pt, line width=0.8pt] (0.25, 5.1) rectangle (3.55, 6.45);
    \node at (0.45, 6.15) [anchor=west, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsysdarkamber] {SRAM Access};
    \node at (0.45, 5.72) [anchor=west, font=\fontsize{11}{13}\selectfont\bfseries, color=mlsysamber] {0.1--0.5 pJ / bit};
    \node at (0.45, 5.35) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysgray] {On-chip shared memory};

    % Tensor Core MAC
    \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=3pt, line width=0.8pt] (0.25, 3.6) rectangle (3.55, 4.95);
    \node at (0.45, 4.65) [anchor=west, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsysdarkblue] {Tensor Core MAC};
    \node at (0.45, 4.22) [anchor=west, font=\fontsize{11}{13}\selectfont\bfseries, color=mlsysblue] {$\sim$1 pJ / MAC};
    \node at (0.45, 3.85) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysgray] {Register-file arithmetic};

    % 10-50x Energy Gap Callout
    \draw[draw=mlsysred, fill=mlsyslightred!60, rounded corners=3pt, line width=1pt] (0.25, 0.3) rectangle (3.55, 3.35);
    \draw[draw=none, fill=mlsysred, rounded corners=2pt] (0.45, 2.85) rectangle (3.35, 3.2);
    \node at (1.9, 3.02) [anchor=center, font=\fontsize{8}{9.5}\selectfont\bfseries, color=white] {10--50$\times$ Energy Gap};

    \node at (0.45, 2.55) [anchor=west, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkred] {The Locality Invariant:};
    \node at (0.45, 2.2) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {Moving 1 bit from HBM};
    \node at (0.45, 1.9) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {burns more energy than};
    \node at (0.45, 1.6) [anchor=west, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkblue] {running multiple MACs!};

    \node at (0.45, 1.05) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysgray] {Tiling amortizes HBM reads};
    \node at (0.45, 0.75) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysgray] {across thousands of reuse ops.};
  \end{scope}
...
::: {#fig-hbm-architecture fig-env="figure" fig-pos="htb" fig-cap="**HBM 3D-stacked architecture**: High-bandwidth memory achieves its performance by vertically stacking DRAM dies and connecting them via through-silicon vias. The entire stack sits on a silicon interposer alongside the GPU die, enabling a 1024-bit-wide bus and reducing signal travel distance from centimeters to micrometers. This physical proximity is the key to breaking the memory wall." fig-alt="Cross-section diagram of an HBM stack. Multiple DRAM dies are stacked on a logic die, connected by vertical TSVs. The assembly sits on a silicon interposer with microbumps connecting to the GPU die."}
```{=latex}
\providecolor{mlsysblue}{RGB}{31, 64, 122}
\providecolor{mlsyslightblue}{RGB}{234, 241, 248}
\providecolor{mlsysdarkblue}{RGB}{20, 41, 78}
...
\begin{tikzpicture}[font=\sffamily]
  ...
  % =========================================================================
  % CARD 1: HBM3 ASSEMBLY (ON-PACKAGE 2.5D)
  % =========================================================================
  \begin{scope}[shift={(0,0)}]
    % Outer box (width 7.8, height 9.0)
    \draw[draw=mlsysbordergray, fill=white, rounded corners=4pt, line width=1.1pt] (0,0) rectangle (7.8, 9.0);
    % Header bar
    \path[fill=mlsyslightgray, rounded corners=4pt] (0, 8.35) rectangle (7.8, 9.0);
    \fill[mlsyslightgray] (0, 8.35) rectangle (7.8, 8.55);
    \draw[draw=mlsysbordergray, line width=0.8pt] (0, 8.35) -- (7.8, 8.35);
    \node at (0.35, 8.68) [anchor=west, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsysdarkblue] {HBM3 Assembly (On-Package 2.5D)};

    % Bandwidth badge
    \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=3pt, line width=0.8pt] (0.35, 7.2) rectangle (3.75, 8.15);
    \node at (2.05, 7.82) [anchor=center, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsysdarkblue] {3.35 TB/s Bandwidth};
    \node at (2.05, 7.45) [anchor=center, font=\fontsize{8}{9.5}\selectfont, color=mlsysblue] {80--141 GB HBM3e};

    % Energy badge
    \draw[draw=mlsysgreen, fill=mlsyslightgreen, rounded corners=3pt, line width=0.8pt] (4.05, 7.2) rectangle (7.45, 8.15);
    \node at (5.75, 7.82) [anchor=center, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsysdarkgreen] {$\sim$2 pJ / bit Energy};
    \node at (5.75, 7.45) [anchor=center, font=\fontsize{8}{9.5}\selectfont, color=mlsysgreen] {10$\times$ lower than DDR5};

    % GPU Compute Die
    \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=3pt, line width=1.2pt] (0.45, 2.5) rectangle (3.45, 4.45);
    \node at (1.95, 4.05) [anchor=center, font=\fontsize{10}{11.5}\selectfont\bfseries, color=mlsysdarkblue] {GPU Compute};
    \node at (1.95, 3.65) [anchor=center, font=\fontsize{8}{9.5}\selectfont, color=mlsysgray] {528 Tensor Cores};
    \draw[draw=none, fill=mlsysblue, rounded corners=2pt] (0.85, 2.75) rectangle (3.05, 3.25);
    \node at (1.95, 3.0) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=white] {H100 / B200};

    % HBM Stack (8 DRAM dies + Base Logic)
    \begin{scope}[shift={(4.1, 2.5)}]
      % Base Logic Die
      \draw[draw=mlsysdarkgreen, fill=mlsyslightgreen!90, rounded corners=1.5pt, line width=1.1pt] (0, 0) rectangle (2.0, 0.45);
      \node at (1.0, 0.22) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkgreen] {Base / TSV Logic};

      % 8 DRAM Dies
      \foreach \k/\y in {1/0.5, 2/0.95, 3/1.4, 4/1.85, 5/2.3, 6/2.75, 7/3.2, 8/3.65} {
        \draw[draw=mlsysgreen, fill=mlsyslightgreen, rounded corners=1pt, line width=0.7pt] (0, \y) rectangle (2.0, \y+0.4);
        \node at (1.0, \y+0.2) [anchor=center, font=\fontsize{7.2}{8.5}\selectfont, color=mlsysdarkgreen] {DRAM Die \k};
      }

      % TSV line
      \draw[draw=mlsysgreen, dashed, line width=1.3pt] (2.25, 0.2) -- (2.25, 3.9);
      \node at (2.55, 2.05) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkgreen, rotate=90] {1024-bit TSVs};
    \end{scope}

    % Micro-bumps under GPU and HBM
    \draw[draw=mlsysblue, dashed, line width=1.4pt] (0.8, 2.3) -- (3.1, 2.3);
    \draw[draw=mlsysdarkgreen, dashed, line width=1.4pt] (4.3, 2.3) -- (5.9, 2.3);
    \node at (3.77, 2.32) [anchor=center, font=\fontsize{6.8}{8}\selectfont, color=mlsysgray] {$\mu$-bumps};

    % Silicon Interposer
    \draw[draw=mlsysgray, fill=mlsyslightgray, rounded corners=2pt, line width=1pt] (0.35, 1.55) rectangle (7.45, 2.15);
    \node at (3.9, 1.85) [anchor=center, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsystext] {Silicon Interposer ($<$\,5\,mm passive sub-micron routing)};

    % Package Substrate
    \draw[draw=mlsysbordergray, fill=mlsysbordergray!35, rounded corners=2pt, line width=0.8pt] (0.35, 0.9) rectangle (7.45, 1.4);
    \node at (3.9, 1.15) [anchor=center, font=\fontsize{7.8}{9}\selectfont, color=mlsysgray] {Organic Package Substrate};

    % Footer recap
    \node at (3.9, 0.45) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {Micrometer-scale 2.5D vertical stack minimizes RC parasitics};
  \end{scope}

  % =========================================================================
  % CARD 2: DDR5 REFERENCE (OFF-PACKAGE DIMM)
  % =========================================================================
  \begin{scope}[shift={(8.4,0)}]
    ...
    \fill[mlsyslightgray] (0, 8.35) rectangle (7.8, 8.55);
    \draw[draw=mlsysbordergray, line width=0.8pt] (0, 8.35) -- (7.8, 8.35);
    \node at (0.35, 8.68) [anchor=west, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsystext] {DDR5 Reference (Off-Package DIMM)};

    % Bandwidth badge
    \draw[draw=mlsysred, fill=mlsyslightred, rounded corners=3pt, line width=0.8pt] (0.35, 7.2) rectangle (3.75, 8.15);
    \node at (2.05, 7.82) [anchor=center, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsysdarkred] {$\sim$200 GB/s Bandwidth};
    \node at (2.05, 7.45) [anchor=center, font=\fontsize{8}{9.5}\selectfont, color=mlsysred] {16$\times$ slower than HBM};

    % Energy badge
    \draw[draw=mlsysred, fill=mlsyslightred, rounded corners=3pt, line width=0.8pt] (4.05, 7.2) rectangle (7.45, 8.15);
    \node at (5.75, 7.82) [anchor=center, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsysdarkred] {$\sim$20 pJ / bit Energy};
    \node at (5.75, 7.45) [anchor=center, font=\fontsize{8}{9.5}\selectfont, color=mlsysred] {Centimeter trace drive};

    % Host CPU Die
    \draw[draw=mlsysgray, fill=white, rounded corners=3pt, line width=1.1pt] (0.45, 2.5) rectangle (2.95, 4.45);
    \node at (1.7, 4.05) [anchor=center, font=\fontsize{10}{11.5}\selectfont\bfseries, color=mlsystext] {Host CPU};
    \node at (1.7, 3.65) [anchor=center, font=\fontsize{8}{9.5}\selectfont, color=mlsysgray] {Memory Ctrl};
    \draw[draw=none, fill=mlsyslightgray, rounded corners=2pt] (0.8, 2.75) rectangle (2.6, 3.25);
    \node at (1.7, 3.0) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {x86 / ARM};

    % Long PCB trace
    \draw[-{Stealth[scale=1.1]}, draw=mlsysred, dashed, line width=1.4pt] (3.05, 3.45) -- (4.65, 3.45);
    \node at (3.85, 3.8) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkred] {64-bit bus};
    \node at (3.85, 3.1) [anchor=center, font=\fontsize{7}{8.5}\selectfont, color=mlsysgray] {10--15\,cm trace};

    % DDR5 DIMM Slot
    \draw[draw=mlsysbordergray, fill=white, rounded corners=3pt, line width=1pt] (4.75, 2.5) rectangle (7.45, 6.75);
    \node at (6.1, 6.45) [anchor=center, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsystext] {DDR5 DIMM};

    % DRAM packages on DIMM
    \foreach \i/\y in {1/5.75, 2/5.2, 3/4.65, 4/4.1} {
      \draw[draw=mlsysgray, fill=mlsyslightgray, rounded corners=1.5pt, line width=0.6pt] (5.0, \y) rectangle (7.2, \y+0.42);
      \node at (6.1, \y+0.21) [anchor=center, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {DRAM Device \i};
    }
    \node at (6.1, 3.65) [anchor=center, font=\fontsize{9}{11}\selectfont, color=mlsysgray] {$\cdots$};

    % DIMM edge connector
    \draw[draw=mlsysamber, fill=mlsyslightamber, rounded corners=2pt, line width=0.9pt] (4.95, 2.6) rectangle (7.25, 3.15);
    \node at (6.1, 2.87) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkamber] {288-Pin Connector};

    % Motherboard FR4 PCB
    \draw[draw=mlsysgray, fill=mlsyslightgray, rounded corners=2pt, line width=1pt] (0.35, 1.55) rectangle (7.45, 2.15);
    \node at (3.9, 1.85) [anchor=center, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsystext] {Motherboard FR4 PCB (Centimeter traces)};

    % Server Chassis Motherboard Plane
    \draw[draw=mlsysbordergray, fill=mlsysbordergray!35, rounded corners=2pt, line width=0.8pt] (0.35, 0.9) rectangle (7.45, 1.4);
    \node at (3.9, 1.15) [anchor=center, font=\fontsize{7.8}{9}\selectfont, color=mlsysgray] {Server Chassis Motherboard Plane};

    % Footer recap
    \node at (3.9, 0.45) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {Long PCB traces force high drive voltage and narrow pin counts};
  \end{scope}
...
The vertical stacking represents a fundamental change in memory architecture: rather than increasing bandwidth by pushing signals faster through long copper traces (the DDR approach, which has diminishing returns), HBM increases bandwidth by multiplying the number of parallel signal paths through extremely short vertical connections. A single HBM stack bonds 8--12 DRAM dies vertically, threading thousands of TSVs through each die to form a 1024-bit-wide interface per stack, compared to 64 bits for a DDR5 channel. This 16$\times$ wider interface, combined with higher per-pin signaling rates, is the source of HBM's bandwidth advantage. The processor and HBM stacks share a package and communicate across the silicon interposer, shortening the electrical path relative to socketed DIMMs. This packaging primarily improves bandwidth and energy per transferred bit; it does not by itself imply that complete HBM load latency is only a few nanoseconds.

The shortened distance has three direct physical benefits:

- **Energy per bit**\index{HBM!energy efficiency}: The transfer cost drops by roughly 4--10$\times$, from approximately 20 pJ/bit for DDR5 to roughly 2--5 pJ/bit for HBM, because shorter traces have lower capacitance and require less driving current.
...
| **Bandwidth**       | ~`{python} MemoryBandwidthDdrRecap.ddr_bw_gb_per_s_str` | ~`{python} MemoryBandwidthPitfallRecap.h100_bw_tb_per_s_str` |    ~17$\times$ faster    |
...
class HbmPackagingAreaRecap:
    ...
    interposer_area = 1000 * ureg.millimeter**2
    hbm_stack_area = 100 * ureg.millimeter**2
...
The evolution of HBM tracks the growth of model sizes with close correspondence. Each generation increases the number of stacked dies, the signaling rate per pin, and the total capacity per stack, driven by the relentless growth in model parameters. @Tbl-hbm-evolution shows the scaling boundary: HBM4 must widen the interface because pin-rate increases alone are no longer enough.

| **Metric**           | **HBM2e (A100)** | **HBM3 (H100)** |             **HBM3e (B200)**            |    **HBM4 (future)**    |
|:---------------------|:----------------:|:---------------:|:---------------------------------------:|:-----------------------:|
| **Peak bandwidth**   |    ~2.0 TB/s     |    ~3.3 TB/s    |                ~8.0 TB/s                | 12--16 TB/s (projected) |
| **Typical capacity** |    40--80 GB     |    80--96 GB    | `{python} HbmB200Recap.capacity_gb_str` |         288 GB+         |
...
```{python}
#| echo: false
from mlsysim.fmt import fmt, fmt_int, fmt_qty, fmt_time
...
class TrillionModelFloor:
    ...
    hbm4_low_tbs_n_str = fmt_int(hbm4_low_bw.to(TB / second).magnitude)
    hbm4_tbs_n_str = fmt_int(hbm4_bw.to(TB / second).magnitude)
    weight_tb_str = fmt_qty(weight, TB, precision=0, commas=False)
    weight_gb_str = fmt_qty(weight, GB, precision=0, commas=False)
    b200_bw_tbs_str = fmt_qty(b200_bw, TB / second, precision=0, commas=False)
    token_b200_ms_str = fmt_time(token_b200, 'millisecond', precision=0, commas=False)
    hbm4_bw_tbs_str = fmt_qty(hbm4_bw, TB / second, precision=0, commas=False)
    token_hbm4_ms_str = fmt_time(token_hbm4, 'millisecond', precision=0, commas=False)
```

HBM4-class packages projected in the `{python} TrillionModelFloor.hbm4_low_tbs_n_str`--`{python} TrillionModelFloor.hbm4_tbs_n_str` TB/s aggregate range illustrate the planning pressure created by trillion-parameter-scale models. A `{python} TrillionModelFloor.model_params_t_str`T-parameter model requires roughly `{python} TrillionModelFloor.weight_tb_str` of weight storage in FP16. At B200-class HBM3e bandwidths (about `{python} TrillionModelFloor.b200_bw_tbs_str`), serving such a model at batch size 1 would produce tokens at `{python} TrillionModelFloor.weight_gb_str`/`{python} TrillionModelFloor.b200_bw_tbs_str` = `{python} TrillionModelFloor.token_b200_ms_str` per token, far too slow for interactive applications. Even at `{python} TrillionModelFloor.hbm4_bw_tbs_str`, the bandwidth floor remains roughly `{python} TrillionModelFloor.token_hbm4_ms_str` per token before batching, quantization, or tensor-parallel sharding. This calculation suggests that trillion-parameter models require aggressive quantization and multi-accelerator tensor parallelism even for single-request inference. The co-evolution of model scale and memory technology continues to drive infrastructure requirements.
...
Comparing the NVIDIA H100 against its successor, the H200\index{NVIDIA!H200, memory wall}, illustrates why the "memory wall" is the primary constraint for modern large language models (LLMs)\index{LLM!inference, memory bottleneck}. While both chips share the same compute cores (same peak TFLOP/s), the H200 provides `{python} H200Comparison.bw_gain_mult_str` higher memory bandwidth and `{python} H200Comparison.cap_gain_mult_str` more HBM capacity.
...
l1 = ax1.plot(years, tflops, color=color_compute, marker='o', markersize=8, linewidth=2.5, label='Compute (TFLOP/s)')
...
l2 = ax2.plot(years, bandwidth, color=color_mem, marker='s', markersize=8, linewidth=2.5, linestyle='--', label='BW (GB/s)')
...
The token latency calculation in notebook \ref{nbk-token-latency} demonstrated that a 70B model on an H100 is overwhelmingly memory-bound. The question is how to determine this systematically for any workload on any hardware. The **roofline model**\index{Roofline model},[^fn-roofline-origin-ci] introduced by Williams, Waterman, and Patterson [@williams2009], provides a visual and analytical framework that answers this question with a single number: the arithmetic intensity of the workload.
...
1.  **Significance**: It defines the Hardware Efficiency Threshold. Workloads with an intensity below the ridge point are bandwidth-bound $(\text{BW})$, while those above are compute-bound\index{Compute-Bound} $(R_{\text{peak}})$.
2.  **Distinction**: Unlike Peak FLOP/s\index{Peak FLOP/s} (which only describes the horizontal ceiling), the ridge point describes the Balance of the architecture. A rising ridge point over hardware generations indicates that compute is growing faster than bandwidth, making utilization harder.
...
- **LLM prefill**: Processing prompt tokens in parallel turns many matrix-vector products into matrix-matrix products and increases weight reuse. Its arithmetic intensity depends on prompt length, batch shape, tiling, attention implementation, and which memory traffic the model counts; prefill can therefore lie below, near, or above a given device's ridge point.
- **CNN training**: Convolution can reuse weights and activations across batch and spatial dimensions, but its position depends on tensor shapes, data layout, fusion, precision, and cache behavior. A measured or operation-specific byte count, rather than a model-family label, determines its roofline region.
- **Attention (long sequences)**: The self-attention mechanism scales quadratically with sequence length in FLOPs but linearly in memory traffic for the key-value cache, making its arithmetic intensity sequence-length-dependent. Short sequences are memory-bound; long sequences are compute-bound.
...
plt.fill_between(x, 0, y, where=(x <= RIDGE_POINT), color='#e6f3ff', alpha=0.5, label='Memory Bound')
plt.fill_between(x, 0, y, where=(x > RIDGE_POINT), color='#fff0e6', alpha=0.5, label='Compute Bound')

plt.axvline(x=RIDGE_POINT, color='#666666', linestyle='--', linewidth=1.5, alpha=0.7)
plt.text(RIDGE_POINT * 1.1, 10, f'ridge point\n~{int(RIDGE_POINT)} FLOP/byte', color='#666666', fontsize=11, verticalalignment='bottom')
...
for label, intensity in workloads.items():
    ...
    plt.scatter(intensity, perf, color='#D62728', s=90, zorder=5, edgecolors='white')
...
The roofline in @fig-roofline-landscape is defined by two ceilings: memory bandwidth (diagonal) and peak compute (horizontal), meeting at the FP16 ridge point (~`{python} RooflineCaptionRecap.h100_ridge_str` for `{python} RooflineCaptionRecap.h100_fp16_tflop_per_s_str` over `{python} RooflineCaptionRecap.h100_bw_tb_per_s_str`). ML workloads span the full range: LLM decode at batch size 1 is deeply memory bound, while LLM training at large batch sizes is compute bound. The position of each workload determines whether faster memory or faster compute would improve performance.
...
Capacity planning must use a measured or conservatively modeled sustained rate, not the peak specification. A run estimated at 1,000 GPU-hours from peak FLOP/s requires $1/u$ times that estimate when its measured utilization fraction is $u$. Infrastructure teams track MFU\index{Model FLOPs Utilization}, the ratio of the model's useful FLOP count to hardware peak FLOP/s multiplied by wall-clock time, but comparisons remain meaningful only when model-FLOP conventions, precision, hardware peak, and system scale match. @Sec-performance-engineering-fleet-efficiency-metric-dff4 formalizes MFU and develops its fleet-scale application.

### Power wall and thermal constraints {#sec-compute-power-wall}
...
::: {#fig-power-path fig-env="figure" fig-pos="htb" fig-cap="**The power delivery path**: The journey of energy from the high-voltage grid to the low-voltage transistor involves multiple conversion stages, each with its own efficiency loss. The cumulative delivery loss, combined with overhead for cooling (measured by PUE), dictates how much power a facility must draw from the grid. The critical engineering challenge is the rack power distribution unit (PDU) to voltage regulator module (VRM) transition, where 10--40 kW of power must be delivered within a single cabinet." fig-alt="Flowchart showing power journey from Grid Substation to data center uninterruptible power supply, to Rack PDU, to Server PSU, to Voltage Regulator Module, finally to GPU Die. Arrows show power flow."}
...
\begin{tikzpicture}[font=\sffamily]
  ...
  % =========================================================================
  \begin{scope}[shift={(0.4, 7.8)}]
    \draw[draw=mlsysbordergray, fill=mlsyslightgray!60, rounded corners=3pt, line width=1pt] (0,0) rectangle (2.2, 1.6);
    \node at (1.1, 1.25) [anchor=center, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsystext] {Utility Grid};
    \draw[draw=mlsysbordergray, line width=0.6pt, opacity=0.6] (0.2, 1.0) -- (2.0, 1.0);
    \node at (1.1, 0.7) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {High Voltage};
    \node at (1.1, 0.35) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {AC (13\,kV+)};
  \end{scope}

  % Arrow 1
  \draw[-{Stealth[scale=1.0]}, draw=mlsysamber, line width=1.4pt] (2.6, 8.6) -- (3.0, 8.6);

  % =========================================================================
  % STAGE 2: TRANSFORMER
  % =========================================================================
  \begin{scope}[shift={(3.0, 7.8)}]
    \draw[draw=mlsysamber, fill=mlsyslightamber!60, rounded corners=3pt, line width=1pt] (0,0) rectangle (2.2, 1.6);
    \node at (1.1, 1.25) [anchor=center, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsysdarkamber] {Transformer};
    \draw[draw=mlsysamber, line width=0.6pt, opacity=0.6] (0.2, 1.0) -- (2.0, 1.0);
    \node at (1.1, 0.7) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {Step-down to};
    \node at (1.1, 0.35) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {480V AC};
  \end{scope}
  \node at (4.1, 7.45) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysred] {$\sim$5\% loss};

  % Arrow 2
  \draw[-{Stealth[scale=1.0]}, draw=mlsysamber, line width=1.4pt] (5.2, 8.6) -- (5.6, 8.6);

  % =========================================================================
  % STAGE 3: UPS / GENERATOR
  % =========================================================================
  \begin{scope}[shift={(5.6, 7.8)}]
    \draw[draw=mlsysgreen, fill=mlsyslightgreen!60, rounded corners=3pt, line width=1pt] (0,0) rectangle (2.2, 1.6);
    \node at (1.1, 1.3) [anchor=center, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsysdarkgreen] {UPS /};
    \node at (1.1, 1.0) [anchor=center, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsysdarkgreen] {Generator};
    \draw[draw=mlsysgreen, line width=0.6pt, opacity=0.6] (0.2, 0.8) -- (2.0, 0.8);
    \node at (1.1, 0.55) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {Battery backup};
    \node at (1.1, 0.25) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {480V AC};
  \end{scope}
  \node at (6.7, 7.45) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysred] {$\sim$3\% loss};

  % Arrow 3
  \draw[-{Stealth[scale=1.0]}, draw=mlsysamber, line width=1.4pt] (7.8, 8.6) -- (8.2, 8.6);

  % =========================================================================
  % STAGE 4: PDU
  % =========================================================================
  \begin{scope}[shift={(8.2, 7.8)}]
    \draw[draw=mlsysblue, fill=mlsyslightblue!70, rounded corners=3pt, line width=1pt] (0,0) rectangle (2.2, 1.6);
    \node at (1.1, 1.25) [anchor=center, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsysdarkblue] {PDU};
    \draw[draw=mlsysblue, line width=0.6pt, opacity=0.6] (0.2, 1.0) -- (2.0, 1.0);
    \node at (1.1, 0.7) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {Power Distrib.};
    \node at (1.1, 0.35) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {to rack: 240V};
  \end{scope}
  \node at (9.3, 7.45) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysred] {$\sim$3\% loss};

  % Arrow 4
  \draw[-{Stealth[scale=1.0]}, draw=mlsysamber, line width=1.4pt] (10.4, 8.6) -- (10.8, 8.6);

  % =========================================================================
  % STAGE 5: SERVER PSU
  % =========================================================================
  \begin{scope}[shift={(10.8, 7.8)}]
    \draw[draw=mlsysblue, fill=mlsyslightblue!70, rounded corners=3pt, line width=1pt] (0,0) rectangle (2.2, 1.6);
    \node at (1.1, 1.25) [anchor=center, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsysdarkblue] {Server PSU};
    \draw[draw=mlsysblue, line width=0.6pt, opacity=0.6] (0.2, 1.0) -- (2.0, 1.0);
    \node at (1.1, 0.7) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {AC to 12V DC};
    \node at (1.1, 0.35) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {conversion};
  \end{scope}
  \node at (11.9, 7.45) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysred] {$\sim$5\% loss};

  % Arrow 5
  \draw[-{Stealth[scale=1.0]}, draw=mlsysamber, line width=1.4pt] (13.0, 8.6) -- (13.4, 8.6);

  % =========================================================================
  % STAGE 6: GPU DIE
  % =========================================================================
  \begin{scope}[shift={(13.4, 7.8)}]
    \draw[draw=mlsysred, fill=mlsyslightred!70, rounded corners=3pt, line width=1.2pt] (0,0) rectangle (2.2, 1.6);
    \node at (1.1, 1.25) [anchor=center, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsysdarkred] {GPU Die};
    \draw[draw=mlsysred, line width=0.6pt, opacity=0.6] (0.2, 1.0) -- (2.0, 1.0);
    \node at (1.1, 0.7) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {VRM to};
    \node at (1.1, 0.35) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {$<$\,1V DC};
  \end{scope}
  \node at (14.5, 7.45) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysred] {$\sim$8\% loss};

  % =========================================================================
  % SUMMARY: TOTAL POWER DELIVERY OVERHEAD
  % =========================================================================
  \draw[draw=mlsysbordergray, fill=white, rounded corners=4pt, line width=1pt] (0.4, 5.75) rectangle (15.6, 7.1);
  \node at (8.0, 6.75) [anchor=center, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsystext] {Total Power Delivery Overhead};
  \node at (8.0, 6.35) [anchor=center, font=\fontsize{8.5}{10}\selectfont, color=mlsystext] {For 1\,MW of GPU compute, draw $\sim$1.2\,MW from grid ($\sim$20\% delivery loss)};
  \node at (8.0, 6.0) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {Losses compound at each stage: transformer, UPS, PDU, PSU, VRM};

  % =========================================================================
  % LOWER SECTION: PUE FORMULA & TYPICAL PUE VALUES
  % =========================================================================
  % Left box: PUE Formula
  \draw[draw=mlsysbordergray, fill=mlsyslightgray!30, rounded corners=4pt, line width=1pt] (0.4, 3.55) rectangle (7.8, 5.45);
  \node at (4.1, 5.15) [anchor=center, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsystext] {PUE Formula};
  \node at (4.1, 4.65) [anchor=center, font=\fontsize{9}{10.5}\selectfont\bfseries, color=mlsysdarkblue] {$\text{PUE} = \frac{\text{Total Facility Power}}{\text{IT Equipment Power}}$};
  \node at (4.1, 4.15) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsystext] {Includes: cooling, lighting, networking overhead};
  \node at (4.1, 3.8) [anchor=center, font=\fontsize{7.5}{9}\selectfont\itshape, color=mlsysgray] {Ideal PUE = 1.0 (all power to compute)};

  % Right box: Typical PUE Values
  \draw[draw=mlsysbordergray, fill=mlsyslightgray!30, rounded corners=4pt, line width=1pt] (8.2, 3.55) rectangle (15.6, 5.45);
  \node at (11.9, 5.15) [anchor=center, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsystext] {Typical PUE Values};

  % Efficient (Google)
  \draw[draw=mlsysgreen, fill=mlsyslightgreen, rounded corners=2pt, line width=0.8pt] (8.6, 4.55) rectangle (12.8, 4.95);
  \node at (8.8, 4.75) [anchor=west, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkgreen] {Efficient (Google): PUE 1.1};

  % Industry Average
  \draw[draw=mlsysamber, fill=mlsyslightamber, rounded corners=2pt, line width=0.8pt] (8.6, 4.05) rectangle (13.8, 4.45);
  \node at (8.8, 4.25) [anchor=west, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkamber] {Industry Average: PUE 1.3};

  % Legacy Datacenter
  \draw[draw=mlsysred, fill=mlsyslightred, rounded corners=2pt, line width=0.8pt] (8.6, 3.55+0.05) rectangle (15.2, 3.95);
  \node at (8.8, 3.75) [anchor=west, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkred] {Legacy Datacenter: PUE 1.5+};

  % =========================================================================
  % BOTTOM COST CALLOUT
  % =========================================================================
  \draw[draw=mlsysred, fill=white, rounded corners=4pt, line width=1.2pt] (0.4, 2.1) rectangle (15.6, 3.25);
  \node at (8.0, 2.85) [anchor=center, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsysdarkred] {Cost implication: at \$0.07/kWh, a 10,000-GPU cluster (10\,MW)};
  \node at (8.0, 2.45) [anchor=center, font=\fontsize{8}{9.5}\selectfont, color=mlsystext] {costs $\sim$\$6.1M/year in electricity alone (PUE 1.0), rising to $\sim$\$9.2M at PUE 1.5};
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_qty, fmt, check
...
class FutureTdpScenario:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    future_tdp_w_str = fmt_qty(future_tdp, watt, precision=0, commas=True)
    gpus_per_node_str = fmt(gpus_per_node, precision=0, commas=False)
    node_gpu_power_kw_str = fmt_qty(node_gpu_power, kilowatt, precision=0, commas=False)
```

If this growth rate continued as a planning scenario, a generation after Blackwell would approach 1,200--1,400 W per accelerator, and another step could reach 1,500--2,000 W. At `{python} FutureTdpScenario.future_tdp_w_str` per accelerator, a single `{python} FutureTdpScenario.gpus_per_node_str`-GPU node would consume `{python} FutureTdpScenario.node_gpu_power_kw_str` of GPU power alone, requiring liquid cooling[^fn-liquid-cooling-flow] infrastructure that can remove heat at rates approaching those of industrial process cooling equipment.

[^fn-liquid-cooling-flow]: **Liquid cooling flow rate**\index{Liquid cooling flow rate}\index{Cooling!liquid flow rate}: A rack of GB200 NVL72 nodes (120 kW) requires approximately 150--200 liters per minute of coolant flow. At these rates, the primary risk shift is from thermal throttling to mechanical failure (leaks, pump cavitation), necessitating the industrial-grade manifolds and quick-disconnect fittings common in chemical processing plants.
...
To appreciate the physical challenge, consider what `{python} H100TdpWallRecap.tdp_w_str` means in terms of heat flux. An H100 die measures approximately `{python} H100TdpWallRecap.die_area_mm2_str` (roughly $29{\times}28$ mm). Dissipating `{python} H100TdpWallRecap.tdp_w_str` from this area produces a heat flux of approximately `{python} H100TdpWallRecap.h100_heat_flux_w_per_cm2_str`. For comparison, the surface of an electric stovetop burner at high heat produces approximately `{python} H100TdpWallRecap.stove_heat_flux_w_per_cm2_str`. An H100 die generates `{python} H100TdpWallRecap.stove_ratio_mult_str` the heat flux of a stove burner, concentrated on an area the size of a large postage stamp.
...
|:---------------------|:--------:|:------------:|:------------:|:----------------------------------------------------------------:|:--------------------------------:|
| **FP32**\index{FP32} |    32    |      8       |      23      |                           ~67 TFLOP/s                            | Master weights, loss computation |
| **TF32**\index{TF32} |    19    |      8       |      10      |                           494 TFLOP/s                            |    Training (NVIDIA default)     |
...
| **INT8**\index{INT8} |    8     |     N/A      |     N/A      |    `{python} TensorCorePrecisionRecap.h100_int8_tc_tops_str`     |    Post-training quantization    |
...
class Fp8CostEquivalence:
    ...
    gpus_per_node = 8  # DGX H100 node (8 accelerators)
...
The infrastructure implications are substantial. A fleet designed for BF16 training, a common baseline for large models, delivers `{python} TensorCorePrecisionRecap.h100_fp16_tc_tflop_per_s_str` per GPU on an H100. The same fleet running FP8 training delivers `{python} TensorCorePrecisionRecap.h100_fp8_tc_tflop_per_s_str` per GPU, effectively doubling the compute density without additional hardware. For a `{python} Fp8CostEquivalence.cluster_gpus_str`-GPU cluster priced at roughly `{python} Fp8CostEquivalence.node_price_str` per `{python} Fp8CostEquivalence.gpus_per_node_str`-GPU node, the difference between BF16 and FP8 training is equivalent to adding another `{python} Fp8CostEquivalence.cluster_gpus_str` GPUs, or roughly `{python} Fp8CostEquivalence.total_musd_str` million in node hardware.
...
At fleet scale, the energy cost of data movement determines the electricity bill. For a 10,000-GPU cluster running at `{python} H100TdpEnergyRecap.tdp_w_str` per GPU, roughly half the budget goes to the memory subsystem (HBM reads and writes, on-chip network transfers, register file accesses) and only a quarter to the Tensor Cores performing useful arithmetic, with the remainder spent on clock distribution, I/O, and leakage. Improving data reuse, through techniques like kernel fusion, FlashAttention, and activation checkpointing, reduces the energy wasted on data movement and increases the fraction of power that performs useful computation.
...
The iron law of modern computing is that moving data costs significantly more energy than manipulating it. Analyzing the energy hierarchy of a single operation at the 4nm process node proves this.
...
- **MFU**: The ratio of achieved FLOP/s during real training to peak hardware FLOP/s. An MFU of 50 percent means the hardware spends half its time on useful computation and half waiting for data, synchronizing, or idling.
- **Time-to-train**: The wall-clock time to train a reference model to a target metric. This captures all system-level effects that MFU alone does not, including I/O stalls, checkpointing overhead, and job restart delays.
...
|:------------------------------------------------|:----------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------|
| **GPU utilization**\index{GPU Utilization}      | Large batch sizes push workloads into the compute-bound regime, so clusters commonly achieve 30--50% MFU. | Low-batch serving is deeply memory bound and can achieve less than 5% MFU, so batching improves utilization at the cost of latency.                |
| **Model placement**\index{Model Replication}    | Tensor and pipeline parallelism distribute a single model across many GPUs.                               | Replicas of the model, or large shards of it, handle independent requests, so memory demand scales with concurrent serving capacity.               |
| **Network traffic**\index{Network Requirements} | Gradient synchronization requires high-bandwidth, low-latency inter-GPU communication.                    | Independent requests need little inter-GPU traffic but substantial client-facing bandwidth for millions of API requests per second.                |
...
The accelerator's physics establish a concrete problem: our `{python} InfraFrontierTdpTransitionRecap.frontier_params_b_str` model requires 350 GB of memory for its weights alone in FP16, and Adaptive Moment Estimation (Adam)-style training state raises the requirement to multiple terabytes once optimizer moments, gradients, and activations are included. A single H100 provides 80 GB of HBM. No single accelerator can hold this model. The system must expand to the next physical level: the node, where @fig-node-topology-comparison contrasts the two dominant approaches to wiring multiple accelerators within a single chassis.

## The node {#sec-compute-node}

The node is the first scale level where accelerator choice becomes an interconnect problem. Multiple GPUs can now share one chassis, but their useful behavior depends on whether the local fabric behaves like a slow ring\index{Ring}, a dense mesh\index{Full Mesh}, or a switch-backed crossbar\index{NVSwitch Crossbar}.
...
\providecolor{mlsysdarkamber}{RGB}{180, 83, 9}
\providecolor{mlsyspurple}{RGB}{124, 58, 237}
\providecolor{mlsyslightpurple}{RGB}{243, 232, 255}
\providecolor{mlsysdarkpurple}{RGB}{109, 40, 217}
\providecolor{mlsysgray}{RGB}{100, 116, 139}
\providecolor{mlsyslightgray}{RGB}{241, 245, 249}
\providecolor{mlsysbordergray}{RGB}{203, 213, 225}
\providecolor{mlsystext}{RGB}{51, 65, 85}
...
\begin{tikzpicture}[font=\sffamily]
  ...
  % =========================================================================
  % CARD 1: RING TOPOLOGY
  % =========================================================================
  \begin{scope}[shift={(0,0)}]
    % Outer box (width 5.4, height 8.2)
    \draw[draw=mlsysbordergray, fill=white, rounded corners=4pt, line width=1.1pt] (0,0) rectangle (5.4, 8.2);
    % Header bar
    \path[fill=mlsyslightgray, rounded corners=4pt] (0, 7.6) rectangle (5.4, 8.2);
    \fill[mlsyslightgray] (0, 7.6) rectangle (5.4, 7.8);
    \draw[draw=mlsysbordergray, line width=0.8pt] (0, 7.6) -- (5.4, 7.6);
    \node at (0.35, 7.9) [anchor=west, font=\fontsize{9}{10.5}\selectfont\bfseries, color=mlsystext] {1. Ring Topology};
    \node at (5.05, 7.9) [anchor=east, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {Pass-Through};

    % Ring center (2.7, 5.0)
    \draw[draw=mlsysblue, dashed, line width=1.2pt] (2.7, 5.0) circle (1.4cm);
    \node at (2.7, 5.15) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {1-Hop};
    \node at (2.7, 4.82) [anchor=center, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {Neighbors};

    % 8 GPU Nodes
    \foreach \i/\ang in {0/90, 1/45, 2/0, 3/315, 4/270, 5/225, 6/180, 7/135} {
      \begin{scope}[shift={({2.7 + 1.4*cos(\ang)}, {5.0 + 1.4*sin(\ang)})}]
        \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=2pt, line width=0.9pt] (-0.38, -0.2) rectangle (0.38, 0.2);
        \node at (0, 0) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkblue] {G\i};
      \end{scope}
    }

    % Properties Card
    \draw[draw=mlsysbordergray!80, fill=white, rounded corners=3pt, line width=0.8pt] (0.25, 0.3) rectangle (5.15, 2.65);
    \node at (0.45, 2.4) [anchor=west, font=\fontsize{8.2}{9.5}\selectfont\bfseries, color=mlsystext] {Ring Invariants:};
    \node at (0.45, 2.05) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Links:} 2 links per GPU (low pin cost)};
    \node at (0.45, 1.72) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Distance:} Max hop distance $N/2 = 4$ hops};

    % Bottleneck box
    \draw[draw=mlsysamber, fill=mlsyslightamber!60, rounded corners=2pt, line width=0.7pt] (0.35, 0.42) rectangle (5.05, 1.42);
    \node at (0.5, 1.18) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkamber] {Bottleneck:};
    \node at (0.5, 0.88) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysdarkamber] {Latency scales $O(N)$ with hop count;};
    \node at (0.5, 0.62) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysdarkamber] {single link failure stalls the entire ring};
  \end{scope}
  ...
  \begin{scope}[shift={(5.75,0)}]
    ...
    \fill[mlsyslightgray] (0, 7.6) rectangle (5.4, 7.8);
    \draw[draw=mlsysbordergray, line width=0.8pt] (0, 7.6) -- (5.4, 7.6);
    \node at (0.35, 7.9) [anchor=west, font=\fontsize{9}{10.5}\selectfont\bfseries, color=mlsystext] {2. Direct Mesh};
    \node at (5.05, 7.9) [anchor=east, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {Point-to-Point};

    % Mesh lines connecting nodes
    \foreach \a in {0, 45, 90, 135, 180, 225, 270, 315} {
      \foreach \b in {0, 45, 90, 135, 180, 225, 270, 315} {
        \draw[draw=mlsysbordergray!60, line width=0.4pt] ({2.7 + 1.4*cos(\a)}, {5.0 + 1.4*sin(\a)}) -- ({2.7 + 1.4*cos(\b)}, {5.0 + 1.4*sin(\b)});
      }
    }

    % Radial emphasis lines
    \foreach \ang in {0, 45, 90, 135, 180, 225, 270, 315} {
      \draw[draw=mlsysblue!70, line width=0.8pt] ({2.7 + 1.4*cos(\ang)}, {5.0 + 1.4*sin(\ang)}) -- (2.7, 5.0);
    }

    % Center circle
    \draw[draw=mlsysbordergray, fill=white, line width=0.8pt] (2.7, 5.0) circle (0.35cm);
    \node at (2.7, 5.0) [anchor=center, font=\fontsize{6.8}{8}\selectfont\bfseries, color=mlsysgray] {28 links};
    ...
    % Properties Card
    \draw[draw=mlsysbordergray!80, fill=white, rounded corners=3pt, line width=0.8pt] (0.25, 0.3) rectangle (5.15, 2.65);
    \node at (0.45, 2.4) [anchor=west, font=\fontsize{8.2}{9.5}\selectfont\bfseries, color=mlsystext] {Mesh Invariants:};
    \node at (0.45, 2.05) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Links:} $O(N^2)$: $N(N{-}1)/2 = 28$ links};
    \node at (0.45, 1.72) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Reach:} Direct 1-hop reach to all ranks};

    % Bottleneck box
    \draw[draw=mlsysred, fill=mlsyslightred!60, rounded corners=2pt, line width=0.7pt] (0.35, 0.42) rectangle (5.05, 1.42);
    \node at (0.5, 1.18) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkred] {Bottleneck:};
    \node at (0.5, 0.88) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysdarkred] {Unroutable PCB traces;};
    \node at (0.5, 0.62) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysdarkred] {physically impractical for $N > 8$};
  \end{scope}

  % =========================================================================
  % CARD 3: NVSWITCH CROSSBAR
  % =========================================================================
  \begin{scope}[shift={(11.5,0)}]
    ...
    \fill[mlsyslightgray] (0, 7.6) rectangle (5.4, 7.8);
    \draw[draw=mlsysbordergray, line width=0.8pt] (0, 7.6) -- (5.4, 7.6);
    \node at (0.35, 7.9) [anchor=west, font=\fontsize{9}{10.5}\selectfont\bfseries, color=mlsyspurple] {3. NVSwitch Fabric};
    \node at (5.05, 7.9) [anchor=east, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkgreen] {Non-Blocking};

    % Left 4 GPUs
    \foreach \i/\y in {0/6.1, 2/5.3, 4/4.5, 6/3.7} {
      \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=2pt, line width=0.9pt] (0.35, \y-0.2) rectangle (1.25, \y+0.2);
      \node at (0.8, \y) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkblue] {G\i};
      \draw[-{Stealth[scale=0.9]}, draw=mlsyspurple, line width=1pt] (1.25, \y) -- (1.95, {4.9 + 0.5*(\y - 4.9)});
    }

    % Center NVSwitch Chip
    \draw[draw=mlsyspurple, fill=mlsyslightpurple!60, rounded corners=3pt, line width=1.3pt] (1.95, 3.4) rectangle (3.45, 6.4);
    \node at (2.7, 6.1) [anchor=center, font=\fontsize{8.5}{10}\selectfont\bfseries, color=mlsysdarkpurple] {NVSwitch};
    \node at (2.7, 5.75) [anchor=center, font=\fontsize{7}{8.5}\selectfont, color=mlsyspurple] {Crossbar};

    \draw[draw=mlsyspurple, fill=white, rounded corners=2pt, line width=0.8pt] (2.1, 4.85) rectangle (3.3, 5.45);
    \node at (2.7, 5.25) [anchor=center, font=\fontsize{7}{8.5}\selectfont\bfseries, color=mlsysdarkpurple] {Non-};
    \node at (2.7, 5.0) [anchor=center, font=\fontsize{7}{8.5}\selectfont\bfseries, color=mlsysdarkpurple] {Blocking};

    \draw[draw=mlsysgreen, fill=mlsyslightgreen, rounded corners=2pt, line width=0.8pt] (2.1, 3.65) rectangle (3.3, 4.45);
    \node at (2.7, 4.15) [anchor=center, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkgreen] {900 GB/s};
    \node at (2.7, 3.85) [anchor=center, font=\fontsize{6.5}{8}\selectfont, color=mlsysdarkgreen] {Uniform};

    % Right 4 GPUs
    \foreach \i/\y in {1/6.1, 3/5.3, 5/4.5, 7/3.7} {
      \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=2pt, line width=0.9pt] (4.15, \y-0.2) rectangle (5.05, \y+0.2);
      \node at (4.6, \y) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkblue] {G\i};
      \draw[-{Stealth[scale=0.9]}, draw=mlsyspurple, line width=1pt] (4.15, \y) -- (3.45, {4.9 + 0.5*(\y - 4.9)});
    }

    % Properties Card
    \draw[draw=mlsysbordergray!80, fill=white, rounded corners=3pt, line width=0.8pt] (0.25, 0.3) rectangle (5.15, 2.65);
    \node at (0.45, 2.4) [anchor=west, font=\fontsize{8.2}{9.5}\selectfont\bfseries, color=mlsystext] {NVSwitch Invariants:};
    \node at (0.45, 2.05) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Topology:} Uniform 1-hop crossbar fabric};
    \node at (0.45, 1.72) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Bandwidth:} Full bisection BW (900 GB/s)};

    % Fleet Advantage box
    \draw[draw=mlsysgreen, fill=mlsyslightgreen!60, rounded corners=2pt, line width=0.7pt] (0.35, 0.42) rectangle (5.05, 1.42);
    \node at (0.5, 1.18) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkgreen] {Fleet Advantage:};
    \node at (0.5, 0.88) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysdarkgreen] {Enables ultra-fast intra-node TP};
    \node at (0.5, 0.62) [anchor=west, font=\fontsize{6.8}{8}\selectfont, color=mlsysdarkgreen] {with zero NUMA transfer skew};
  \end{scope}
...
Aggregating accelerators solves the capacity problem only if the interconnect between them is fast enough that the group behaves as one machine rather than as several machines passing messages. The `{python} InfraFrontierNodeMemRecap.frontier_params_b_str` model's weights span at least five accelerators, and the full training state spans an entire 8-H100 node\index{Node} and more, so the parameters of a single layer now live on different chips and must be reassembled on every forward and backward pass. Whether that reassembly is cheap or ruinous depends entirely on the local fabric, which is why the node is the first scale level where the accelerator's specifications matter less than the wires between accelerators. The training system must combine tensor parallelism with optimizer sharding and memory offload, and each of those strategies places a different demand on the interconnect.

```{python}
#| echo: false
from mlsysim.fmt import fmt_qty, fmt_multiple
...
class H100NvlinkNodeRecap:
    ...
    nvlink_per_dir_gb_per_s_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth_per_direction, GB / second, precision=0, commas=False)
...
|:---------------------|:------------------:|:---------------------------------------------------------------:|:----------------:|:------------------:|
| **Intra-package**    | Silicon interposer |                            ~3.3 TB/s                            | $<100\text{ ns}$ |    Single chip     |
| **Intra-node**       |     NVLink/ICI     | ~`{python} BandwidthHierarchyTableRecap.nvlink_bw_gb_per_s_str` |      ~1 μs       | Node (8--16 chips) |
| **Intra-node (I/O)** |   PCIe Gen5 x16    |  ~`{python} BandwidthHierarchyTableRecap.pcie_bw_gb_per_s_str`  |      ~2 μs       |  CPU-GPU, NIC-GPU  |
...
The concentric view in @fig-infra-bandwidth-hierarchy presents the same hierarchy spatially, so each outward boundary crossing reads as both a bandwidth drop and a latency increase.
...
\begin{tikzpicture}[font=\sffamily]
  ...
  % =========================================================================
  % CONCENTRIC BANDWIDTH ELLIPSES (Center at 7.2, 4.6)
  % =========================================================================
  \coordinate (C) at (7.2, 4.6);

  % Zone 4: Storage / PFS (outermost)
  \draw[draw=mlsysgreen, fill=mlsyslightgreen!25, line width=1.1pt] (C) ellipse (5.7cm and 3.7cm);

  % Zone 3: InfiniBand NDR (Inter-Node)
  \draw[draw=mlsyspurple, fill=mlsyslightpurple!30, line width=1.3pt] (C) ellipse (4.3cm and 2.8cm);

  % Zone 2: NVLink / NVSwitch (Intra-Node)
  \draw[draw=mlsysamber, fill=mlsyslightamber!35, line width=1.4pt] (C) ellipse (3.0cm and 1.9cm);

  % Zone 1: HBM3 (On-Package)
  \draw[draw=mlsysblue, fill=mlsyslightblue, line width=1.6pt] (C) ellipse (1.65cm and 1.05cm);

  % Center Zone 1 Text
  \node at (7.2, 4.88) [anchor=center, font=\fontsize{9.5}{11}\selectfont\bfseries, color=mlsysdarkblue] {HBM3 / HBM3e};
  \node at (7.2, 4.52) [anchor=center, font=\fontsize{10}{11.5}\selectfont\bfseries, color=mlsysblue] {3.35 TB/s};
  \node at (7.2, 4.22) [anchor=center, font=\fontsize{7}{8.5}\selectfont, color=mlsysgray] {$\sim$100\,ns $\cdot$ On-Package};

  % ===== Zone 2 Label Pill: NVLink =====
  \draw[draw=mlsysamber, fill=white, rounded corners=3pt, line width=0.8pt] (4.9, 6.2) rectangle (9.5, 6.8);
  \node at (7.2, 6.58) [anchor=center, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsysdarkamber] {NVLink 4 / NVSwitch $\cdot$ 900 GB/s};
  \node at (7.2, 6.35) [anchor=center, font=\fontsize{6.8}{8}\selectfont, color=mlsysgray] {1--3\,$\mu$s latency $\cdot$ 8-GPU intra-node mesh};

  % ===== Zone 3 Label Pill: InfiniBand =====
  \draw[draw=mlsyspurple, fill=white, rounded corners=3pt, line width=0.8pt] (4.5, 7.1) rectangle (9.9, 7.7);
  \node at (7.2, 7.48) [anchor=center, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsysdarkpurple] {InfiniBand NDR / RoCEv2 $\cdot$ 50 GB/s (400G)};
  \node at (7.2, 7.25) [anchor=center, font=\fontsize{6.8}{8}\selectfont, color=mlsysgray] {3--10\,$\mu$s latency $\cdot$ Inter-node fat-tree network};

  % ===== Zone 4 Label Pill: Storage =====
  \draw[draw=mlsysgreen, fill=white, rounded corners=3pt, line width=0.8pt] (4.1, 8.0) rectangle (10.3, 8.6);
  \node at (7.2, 8.38) [anchor=center, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsysdarkgreen] {Parallel File System (Lustre / NVMe-oF) $\cdot$ 7--15 GB/s};
  \node at (7.2, 8.15) [anchor=center, font=\fontsize{6.8}{8}\selectfont, color=mlsysgray] {100--1,000\,$\mu$s latency $\cdot$ Remote persistent storage};

  % =========================================================================
  % LEFT SCOPE BADGES (Clean standalone pills)
  % =========================================================================
  % Storage Badge
  \draw[draw=mlsysgreen, fill=mlsyslightgreen, rounded corners=2pt, line width=0.8pt] (0.2, 7.9) rectangle (1.9, 8.4);
  \node at (1.05, 8.15) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkgreen] {Persistent};

  % Inter-Node Badge
  \draw[draw=mlsyspurple, fill=mlsyslightpurple, rounded corners=2pt, line width=0.8pt] (0.2, 6.95) rectangle (1.9, 7.45);
  \node at (1.05, 7.2) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkpurple] {Inter-Node};

  % Intra-Node Badge
  \draw[draw=mlsysamber, fill=mlsyslightamber, rounded corners=2pt, line width=0.8pt] (0.2, 6.0) rectangle (1.9, 6.5);
  \node at (1.05, 6.25) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkamber] {Intra-Node};

  % On-Package Badge
  \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=2pt, line width=0.8pt] (0.2, 4.8) rectangle (1.9, 5.3);
  \node at (1.05, 5.05) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkblue] {On-Package};

  % =========================================================================
  % BANDWIDTH CLIFF INDICATORS (DROP CALLOUTS)
  % =========================================================================
  % HBM to NVLink (3.7x drop)
  \draw[-{Stealth[scale=0.9]}, draw=mlsysred, dashed, line width=1.1pt] (8.9, 4.9) -- (9.7, 5.25);
  \draw[draw=mlsysred, fill=mlsyslightred, rounded corners=2pt, line width=0.7pt] (9.6, 5.3) rectangle (10.9, 5.75);
  \node at (10.25, 5.52) [anchor=center, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkred] {3.7$\times$ drop};

  % NVLink to IB (18x drop)
  \draw[-{Stealth[scale=0.9]}, draw=mlsysred, dashed, line width=1.1pt] (9.9, 6.1) -- (10.6, 6.45);
  \draw[draw=mlsysred, fill=mlsyslightred, rounded corners=2pt, line width=0.7pt] (10.3, 6.5) rectangle (11.6, 6.95);
  \node at (10.95, 6.72) [anchor=center, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkred] {18$\times$ drop};

  % IB to Storage (7x drop)
  \draw[-{Stealth[scale=0.9]}, draw=mlsysred, dashed, line width=1.1pt] (10.4, 7.3) -- (11.1, 7.65);
  \draw[draw=mlsysred, fill=mlsyslightred, rounded corners=2pt, line width=0.7pt] (10.8, 7.7) rectangle (12.1, 8.15);
  \node at (11.45, 7.92) [anchor=center, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkred] {7$\times$ drop};

  % =========================================================================
  % RIGHT SCALE BAR (LOG SCALE)
  % =========================================================================
  \begin{scope}[shift={(14.2, 0.9)}]
    \node at (0.3, 7.8) [anchor=center, font=\fontsize{8}{9.5}\selectfont\bfseries, color=mlsystext] {Bandwidth};
    \node at (0.3, 7.5) [anchor=center, font=\fontsize{7}{8.5}\selectfont, color=mlsysgray] {(Log scale)};

    % Outer scale bar
    \draw[draw=mlsysbordergray, fill=white, rounded corners=3pt, line width=0.8pt] (0, 0.4) rectangle (0.6, 7.2);

    % Segment 1: HBM (3.35 TB/s) y from 5.5 to 7.2
    \draw[draw=none, fill=mlsyslightblue, rounded corners=2pt] (0.02, 5.5) rectangle (0.58, 7.18);
    \draw[draw=mlsysblue, line width=1pt] (-0.1, 6.35) -- (0, 6.35);
    \node at (-0.15, 6.35) [anchor=east, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkblue] {3.35 TB/s};

    % Segment 2: NVLink (900 GB/s) y from 3.8 to 5.5
    \draw[draw=none, fill=mlsyslightamber] (0.02, 3.8) rectangle (0.58, 5.5);
    \draw[draw=mlsysamber, line width=1pt] (-0.1, 4.65) -- (0, 4.65);
    \node at (-0.15, 4.65) [anchor=east, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkamber] {900 GB/s};

    % Segment 3: IB (50 GB/s) y from 2.0 to 3.8
    \draw[draw=none, fill=mlsyslightpurple] (0.02, 2.0) rectangle (0.58, 3.8);
    \draw[draw=mlsyspurple, line width=1pt] (-0.1, 2.9) -- (0, 2.9);
    \node at (-0.15, 2.9) [anchor=east, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkpurple] {50 GB/s};

    % Segment 4: Storage (7 GB/s) y from 0.42 to 2.0
    \draw[draw=none, fill=mlsyslightgreen, rounded corners=2pt] (0.02, 0.42) rectangle (0.58, 2.0);
    \draw[draw=mlsysgreen, line width=1pt] (-0.1, 1.2) -- (0, 1.2);
    \node at (-0.15, 1.2) [anchor=east, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkgreen] {7 GB/s};

    % Zero tick
    \node at (0.3, 0.15) [anchor=center, font=\fontsize{7}{8.5}\selectfont, color=mlsysgray] {0};
  \end{scope}
...
However, tensor parallelism requires frequent collectives, commonly AllReduce, ReduceScatter, and AllGather after sharded matrix operations. A physical ring can execute ring-AllReduce efficiently, but its fixed multi-hop paths constrain other simultaneous traffic patterns and make topology-aware scheduling important. A fully connected mesh, where every GPU has a direct link to every other GPU, eliminates the multi-hop constraint but requires $N(N-1) / 2 = 28$ links for 8 GPUs, a wiring challenge that becomes impractical as the link count grows quadratically. The NVSwitch\index{NVSwitch} crossbar\index{Crossbar Switch} provides full-bisection switched connectivity with a manageable number of switch chips.
...
class BandwidthHierarchyNvlinkRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_bw_gb_per_s_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth, GB / second, precision=0, commas=False)
    nvlink_per_direction_gb_per_s_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth_per_direction, GB / second, precision=0, commas=False)
```

Consider the NVIDIA DGX H100 architecture\index{DGX}. Eight H100 GPUs sit on a single baseboard, each connected to four NVSwitch chips via 18 NVLink 4.0 lanes. The NVSwitch chips form a non-blocking crossbar[^fn-crossbar-nvswitch-interconnect] through which any GPU can reach any other GPU. Each GPU's port budget is `{python} BandwidthHierarchyNvlinkRecap.nvlink_bw_gb_per_s_str` bidirectional, or `{python} BandwidthHierarchyNvlinkRecap.nvlink_per_direction_gb_per_s_str` in one direction, shared across that GPU's simultaneous transfers; it is not a separate full-bandwidth link for every GPU pair.
...
```{python}
#| echo: false
from mlsysim.fmt import fmt_int, fmt_percent, fmt_time, MarkdownStr

# ┌── LEGO ───────────────────────────────────────────────
class TensorParallelAllReduce:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    model = Models.Language.GPT3  # 175B: hidden 12,288, layers 96
    tp_gpus = 8  # GPUs per node (tensor parallel)
    microbatch = 4  # sequences per microbatch (scenario)
    seq_len = 2048  # tokens per sequence (scenario)
    ...
    allreduce_mb_str = fmt_int(allreduce_bytes.to(MB).magnitude)
    ring_traffic_factor_str = fmt(ring_traffic_factor, precision=2, commas=False)
    allreduce_traffic_mb_str = fmt_int(allreduce_traffic.to(MB).magnitude)
...
Beyond NVLink, the node also connects to the external network via InfiniBand host channel adapters and to host storage via PCIe. The DGX H100 includes eight InfiniBand ConnectX-7 HCAs, one per GPU, enabling GPUDirect RDMA\index{GPUDirect RDMA}: the network adapter can read from and write to GPU HBM directly, without staging data through host DRAM or involving the host CPU in the data path.

GPUDirect RDMA is critical for inter-node gradient synchronization, where each GPU's gradient shard must be sent to its peers on other nodes. Without GPUDirect, each gradient transfer would require two extra copies: first from GPU HBM to host DRAM (over PCIe), and then from host DRAM to the network adapter (over another PCIe path). The double-copy adds latency (two PCIe traversals instead of one direct memory access (DMA) operation) and halves the effective bandwidth (each byte traverses the PCIe bus twice).
...
The DGX H100 answers a general node-design problem by making local accelerator exchange cheap. A switch-rich design spends power, board area, and cost on universal any-to-any exchange. That choice is easy to justify when tensor parallelism is the critical path, because each Transformer layer turns local accelerator communication into part of the model's inner loop. The `{python} AlternativeNodeArchitectureRecap.h100_nvlink_gb_per_s_str` H100 NVLink fabric therefore buys a software abstraction: the training framework can treat eight accelerators as one tightly coupled device for the layers that must be split across chips.
...
@Tbl-memory-breakdown makes the partitioning concrete. The same `{python} InfraFrontierNodeWeightsRecap.total_footprint_tb_str` budget established earlier reappears here, but now decomposed by component and tier: the Adam optimizer state in FP32, not the FP16 weights, is the dominant term, and the activation peak (which scales with batch size and sequence length) is the most variable. The lesson for parallelism selection is that the weights are the smallest piece of what the node must hold, so a strategy that only shards weights leaves the largest consumers, optimizer state and activations, untouched.

```{python}
#| echo: false
from mlsysim.fmt import fmt_int, fmt_memory_capacity
...
class MemoryBandwidthNodeTableRecap:
    ...
    node_hbm_gb_str = fmt_memory_capacity(node_hbm, unit=GiB, precision=0, commas=False)
    host_dram_tb_str = fmt_memory_capacity(host_dram, unit=TiB, precision=0, commas=False)
    nvme_bw_gb_per_s_str = fmt_qty(nvme.bandwidth, GB / second, precision=0, commas=False)
...
class InfraFrontierNodeModelRecap:
    ...
    total_per_gpu = weight_per_gpu + grad + opt_state + act + buf
    # What offloading the optimizer state to host DDR5 actually leaves behind.
    post_offload_per_gpu = total_per_gpu - opt_state
...
class InterconnectCommMath:
    ...
    # One-way deliverable rate: NVLink per-direction (450 GB/s), not the 900 GB/s
    # bidirectional datasheet total; moving a buffer one way uses half the link,
    # so this matches the staircase notebook and the inter-node InfiniBand rate.
    _nvlink_bw = Hardware.Cloud.H100.nvlink.bandwidth_per_direction
    _ib_bw = Systems.Fabrics.InfiniBand_NDR.bandwidth
    _nvlink_bw_val = _nvlink_bw.to(GB / second).magnitude
    _ib_bw_gbs_val = _ib_bw.to(GB / second).magnitude
    _t_intra_ms = (gradient_buffer / _nvlink_bw).to(millisecond).magnitude
    _t_inter_ms = (gradient_buffer / _ib_bw).to(millisecond).magnitude
    _gap_nv_ib = (_nvlink_bw / _ib_bw).to_base_units().magnitude

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    t_intra_math = fmt_math(
        f"T_{{\\text{{intra}}}} = {gradient_buffer_value}\\text{{ GB}} / {_nvlink_bw_val:.0f}\\text{{ GB/s}} \\approx \\mathbf{{{_t_intra_ms:.1f} \\text{{ ms}}}}"
    )
    t_inter_math = fmt_math(
        f"T_{{\\text{{inter}}}} = {gradient_buffer_value}\\text{{ GB}} / {_ib_bw_gbs_val:.0f}\\text{{ GB/s}} \\approx \\mathbf{{{_t_inter_ms:.0f} \\text{{ ms}}}}"
    )
    gradient_buffer_gb_str = fmt_memory(gradient_buffer, unit=GB, precision=0, commas=False)
    gap_nv_ib_mult_str = fmt_multiple(_gap_nv_ib, precision=0, commas=False)
```
...
class BandwidthHierarchyNvlinkPodRecap:
    ...
    nvlink_bidirectional_gb_per_s_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth, GB / second, precision=0, commas=False)
```
...
1. **Intra-node (NVLink)**: Bandwidth = `{python} BandwidthHierarchyNvlinkPodRecap.nvlink_bw_gb_per_s_str` one-way effective bandwidth; the bidirectional aggregate is `{python} BandwidthHierarchyNvlinkPodRecap.nvlink_bidirectional_gb_per_s_str`, but this transfer-time calculation moves one buffer in one direction.
...
**Systems insight**: Crossing the node boundary increases communication time by approximately `{python} InterconnectCommMath.gap_nv_ib_mult_str`. In a training loop where gradient synchronization happens at every step, this penalty would stall the accelerators for the majority of each iteration, reducing utilization to well below 50 percent. This is why training frameworks use tensor parallelism within the node (fast NVLink) and data parallelism across nodes (tolerant of slower InfiniBand).

:::

### Data loading and I/O pipeline {#sec-compute-data-loading}

The discussion so far has focused on how data moves within the node (HBM to Tensor Core, GPU to GPU via NVLink\index{NVLink}, GPU to host via PCIe). Training also requires feeding the node with a continuous stream of training data from external storage. If the data loading pipeline\index{Data loading!pipeline}\index{I/O pipeline} cannot keep up with the GPU's consumption rate, the most expensive component in the system (the GPU) sits idle waiting for data. This is the **I/O bottleneck**\index{I/O bottleneck}, and avoiding it requires careful engineering of the data loading pipeline.
...
As cluster sizes expand, a new storage bottleneck emerges. When training our `{python} InfraFrontierNodeIoRecap.frontier_params_b_str` model across 128 nodes, the system acts as a synchronized "thundering herd": all 128 nodes simultaneously demand the next microbatch of tokens at the start of every training step. A shared parallel filesystem like Lustre, even with 500 GB/s of aggregate throughput, will buckle when 128 clients simultaneously pull data, causing read latencies to spike from milliseconds to seconds. The architectural solution is **tiered storage**\index{Tiered storage} with aggressive local caching. Provisioning each training node with local NVMe SSDs capable of delivering 25 GB/s per node decouples the cluster's immediate data dependency from the shared filesystem. A background process prefetches data from the central store to the local NVMe cache asynchronously, smoothing out I/O spikes. For the 128-node cluster, local NVMe creates an aggregate read bandwidth of 3.2 TB/s ($128 \times 25$ GB/s), eclipsing the capability of even the most expensive centralized storage arrays and ensuring the GPUs are never starved.

::: {#psp-compute-infrastructure-data-loading-trap .callout-perspective title="The data loading trap"}

A subtle failure mode occurs when a local data-loading benchmark satisfies the node but the shared path cannot satisfy the fleet. The hardware contract is end-to-end sustained bandwidth at the intended concurrency, including storage servers, network paths, preprocessing, host memory, and accelerator transfer. @Sec-data-storage develops the storage tiers and data paths that must meet this contract.
...
A standard 42U server rack\index{Rack} in a traditional data center draws 5--10 kW and can be cooled by room-temperature air pushed through perforated floor tiles. The chapter's four-node DGX H100 rack scenario draws `{python} RackPowerRackRecap.rack_power_kw_str` at the modeled IT boundary. At this density, the engineering constraints shift from silicon and signal integrity to power delivery\index{Power Delivery} and thermodynamics. The rack is where the power wall and the laws of heat transfer\index{Cooling} become dominant design forces.
...
For our `{python} InfraFrontierRackPowerRecap.frontier_params_b_str` model, training across 1,024 GPUs requires `{python} RackPowerRackRecap.racks_for_1k_str` racks (4 nodes of 8 GPUs each per rack). Across the cluster, the modeled rack IT power sums to approximately `{python} RackPowerRackRecap.cluster_power_mw_str`, before facility cooling overhead. Delivering this power reliably and removing the resulting heat without allowing any component to exceed its thermal limit is a multidisciplinary engineering challenge spanning electrical, mechanical, and civil engineering. A failure at any point in the power delivery chain, from the utility substation to the individual GPU voltage regulator, can halt the entire training run, wasting hours of computation and potentially corrupting the training state.
...
1.  **Significance**: AI rack power density has grown dramatically. The chapter's four-node DGX H100 scenario contains 32 GPUs at `{python} H100TdpRackRecap.tdp_w_str` each and draws `{python} RackPowerRackRecap.rack_power_kw_str` at the modeled IT boundary. This exceeds the 5–10 kW typical of general-purpose data center racks and requires purpose-designed electrical distribution and cooling capacity. Whether that cooling is engineered air or liquid depends on the system configuration and facility design.
...
The rack opener promised that the laws of heat transfer become a dominant design force at this level. Air has much lower volumetric heat capacity than liquid, so an air-cooled rack needs sufficient airflow, pressure, and temperature rise to carry away its heat. There is no universal 30 kW cutoff: the practical limit depends on server and facility design. The chapter's `{python} RackPowerRackRecap.rack_power_kw_str` scenario can use engineered air cooling in a suitable facility; denser systems can make liquid cooling the practical choice.

Cooling selection is therefore configuration-dependent rather than a consequence of one rack-power threshold. Direct-to-chip liquid cooling raises heat-removal capacity and can reduce fan energy, while engineered air remains viable for systems and facilities designed for its airflow requirements. @Sec-sustainable-ai-cooling compares these architectures and the facility-power overhead each choice carries every hour the cluster runs.
...
The transition from traditional server form factors to ML-optimized designs is driven by the same power density challenge that forced the transition from air cooling to liquid cooling. A traditional 1U server dissipates 300--500 W and requires only modest airflow for cooling. A DGX H100 node dissipates approximately `{python} RackPowerRackRecap.node_power_kw_str` across its 8 GPUs, NVSwitches, and supporting components, requiring either massive airflow (in air-cooled configurations) or liquid cooling plumbing (in liquid-cooled configurations). The form factor must accommodate both the compute hardware and the thermal management infrastructure, which is why ML nodes are substantially taller (8U vs. 1U) than traditional servers.
...
As rack power densities climb toward 100 kW for liquid-cooled clusters, the inefficiency of per-server AC conversion becomes untenable. Traditional designs dedicate volume in every chassis to redundant AC power supply units (PSUs), while many dense-rack designs use **rack-level DC power distribution**\index{Rack-level DC power distribution}. In this architecture, a centralized power shelf converts mains AC to a **48V DC busbar**\index{48V DC busbar} that runs the height of the rack, replacing individual server PSUs. This consolidation eliminates one conversion stage, yielding a system-wide efficiency gain of 2--3 percent -- a meaningful reduction in thermal load when training a 175B-parameter model across thousands of GPUs. Power is delivered via the 48V bus directly to the server backplane, where compact DC-to-DC converters step it down to 12V and finally to the GPU's sub-1V operating voltage. Pioneered by Google and Meta through the Open Compute Project, this topology reduces failure points and recovers valuable chassis volume for hydraulic cooling loops and high-bandwidth interconnects.

#### Power delivery networks and transient dynamics {#sec-compute-power-delivery-transients}

Stepping voltage down from 48V to sub-1V operating levels at the silicon die introduces an acute physical challenge: managing dynamic current slew rates ($di/dt$) across the **Power Delivery Network (PDN)**\index{Power Delivery Network!definition}.

Modern AI accelerators operate at supply voltages below one volt ($V_{\text{core}} \approx 0.7\text{--}0.9\text{ V}$) while consuming $700\text{--}1{,}200\text{ W}$ each. By conservation of power ($P = V \cdot I$), a single accelerator draws sustained currents exceeding $I_{\text{core}} \approx 1{,}000\text{--}1{,}500\text{ A}$. Across a dense liquid-cooled rack housing 36 to 72 accelerators (e.g., drawing $100\text{--}120\text{ kW}$), the aggregate core current reaches $50{,}000\text{--}100{,}000\text{ A}$ at the silicon level.
...
***Power delivery network (PDN)***\index{Power Delivery Network!definition}\index{Transient Voltage Droop!definition} is the physical electrical distribution hierarchy connecting the facility power supply to the on-die transistor gates, whose parasitic loop inductance ($L_{\text{parasitic}}$) induces transient voltage fluctuations ($\Delta V = L \frac{di}{dt}$) during rapid changes in processor switching activity.

1.  **Significance**: Synchronous distributed training creates severe step-function current transients when thousands of GPUs transition simultaneously between communication stalls and dense matrix multiplication, threatening voltage collapse or catastrophic dielectric breakdown.
2.  **Distinction**: Unlike thermal constraints (which operate over seconds or minutes due to thermal mass), PDN transients operate in the nanosecond-to-microsecond domain, far too fast for feedback loops in data-center power shelves to compensate.
3.  **Common pitfall**: Relying solely on passive board-level capacitors to suppress transients. High-frequency transients must be mitigated through a multi-tier decoupling hierarchy spanning on-die trench capacitors, package capacitors, and firmware-coordinated software power ramping.

:::

In high-speed electrical circuits, any change in current through an inductive path generates an induced voltage offset governed by Faraday's law of induction, formulated in @eq-compute-pdn-droop:

$$\Delta V = L_{\text{parasitic}} \frac{di}{dt}$$ {#eq-compute-pdn-droop}

where $L_{\text{parasitic}}$ is the cumulative loop inductance of the PDN (encompassing the rack busbar, board power planes, voltage regulator module (VRM) traces, socket connector pins, and chip package interposers), and $\frac{di}{dt}$ is the current slew rate.

In distributed deep learning, this physical property becomes an operational hazard due to the synchronous nature of the bulk synchronous parallel (BSP) execution model:

-   **The transient trigger**: During a collective communication barrier (such as an AllReduce or pipeline bubble), Tensor Cores sit idle while waiting for network packets. Core current drops to an idle floor of roughly $200\text{ A}$. The instant communication completes, all 32 to 72 GPUs in the rack simultaneously issue dense Tensor Core GEMM instructions within nanoseconds.
-   **Transient magnitude**: Core current spikes from $200\text{ A}$ to $1{,}200\text{ A}$ in less than $10\text{ ns}$, producing a current slew rate exceeding:
    $$\frac{di}{dt} \approx \frac{1{,}000\text{ A}}{10^{-8}\text{ s}} = 10^{11}\text{ A/s}$$
-   **Voltage droop**: Even with an extraordinarily low parasitic inductance of only $1\text{ nH}$ ($10^{-9}\text{ H}$) between the VRM and the silicon die, @eq-compute-pdn-droop dictates a voltage collapse of:
    $$\Delta V = 10^{-9}\text{ H} \times 10^{11}\text{ A/s} = 100\text{ mV}$$
    When operating at a nominal $V_{\text{core}}$ of $0.8\text{ V}$, a $100\text{ mV}$ droop represents a $12.5$ percent voltage drop—well beyond the standard $\pm 5$ percent operating tolerance window.

This transient excursion creates two distinct failure modes:

1.  **Under-voltage droop**: If core voltage droops below the minimum operating threshold ($V_{\text{min}}$), gate propagation delay increases ($t_{\text{prop}} \propto 1/V$). Logic paths fail to meet clock setup times, leading to silent data corruption (flipped bits in activations) or immediate processor machine-check exceptions and unrecoverable node resets.
2.  **Over-voltage overshoot**: When a massive GEMM finishes abruptly, the sudden collapse of current causes the stored magnetic energy in the parasitic inductors ($\frac{1}{2} L I^2$) to discharge into the circuit, generating a positive voltage spike. If this spike exceeds the transistor gate dielectric breakdown voltage, it causes physical punch-through, permanently destroying silicon gates.

To contain these transients (definition \ref{dfn-compute-infrastructure-power-delivery-network}), modern AI compute infrastructure employs a **multi-tier decoupling hierarchy** paired with software power smoothing:

-   **High-frequency on-die decoupling**: Deep-trench capacitors (DTC) and land-side capacitors (LSC) embedded directly within or underneath the silicon interposer. With ultra-low parasitic inductance ($<10\text{ pH}$), they provide immediate charge storage for the first $0.1\text{--}1\text{ ns}$ of current transition.
-   **Mid-frequency package and socket decoupling**: High-density multilayer ceramic capacitors (MLCC) mounted directly on the package substrate and adjacent to the socket on the baseboard, buffering transients in the $1\text{--}100\text{ ns}$ window.
-   **Low-frequency bulk decoupling**: Polymer tantalum and aluminum electrolytic capacitors on the VRM and power shelf, sustaining charge over the microsecond domain until the switched-mode power supplies adjust their duty cycle.
-   **Firmware and software power smoothing**: Because physical capacitors consume valuable silicon and board area, modern GPU architectures implement software-coordinated power ramping. When launching massive GEMM kernels following a barrier, GPU microcode or CUDA runtime introduces micro-throttling or staggered pipeline ramp-up cycles over tens of microseconds, bounding the maximum $\frac{di}{dt}$ slope and ensuring voltage excursions remain safely within operational margins.
...
The rail-optimized design spends additional switches, ports, and cabling to align communication paths with accelerator rails. Its benefit depends on the collective traffic matrix, routing, oversubscription, and job placement rather than a fixed efficiency gain. @Sec-network-fabrics develops those conditions and the fabric metrics needed to evaluate them.
...
The rack concentrates power and heat into a physical volume where thermodynamics, not software, sets the limits. A single rack of 32 GPUs, however, is far from sufficient for our `{python} InfraFrontierStragglerRecap10.frontier_params_b_str` model, which may require thousands of accelerators to train in a reasonable timeframe. The next level of infrastructure aggregates hundreds of racks into a unified computing system: the pod, whose network topology (@fig-pod-network-topology) is designed around the communication pattern the workload must sustain. Full any-to-any bisection is one design point; torus and rail-optimized designs trade that universality for lower cost or better locality.

## The pod {#sec-compute-pod}

Training our `{python} InfraFrontierStragglerRecap10.frontier_params_b_str` model on a single DGX H100 node (8 GPUs, roughly 16,000 TFLOP/s aggregate) would take several months, assuming the model can fit at all. Reducing training time to weeks requires 100--1,000 nodes operating in concert. A pod is the cluster-scale unit that makes this possible: many racks under one high-speed fabric, power envelope, and cooling design. Pod design is the first point where topology, power, and failure domains become shared infrastructure rather than per-rack concerns.
...
\begin{tikzpicture}[font=\sffamily]
  ...
  % =========================================================================
  % CARD 1: FAT-TREE (3-TIER NETWORK FABRIC)
  % =========================================================================
  \begin{scope}[shift={(0,0)}]
    % Outer box (width 7.6, height 8.8)
    \draw[draw=mlsysbordergray, fill=white, rounded corners=4pt, line width=1.1pt] (0,0) rectangle (7.6, 8.8);
    % Header bar
    \path[fill=mlsyslightgray, rounded corners=4pt] (0, 8.15) rectangle (7.6, 8.8);
    \fill[mlsyslightgray] (0, 8.15) rectangle (7.6, 8.35);
    \draw[draw=mlsysbordergray, line width=0.8pt] (0, 8.15) -- (7.6, 8.15);
    \node at (0.35, 8.48) [anchor=west, font=\fontsize{9}{10.5}\selectfont\bfseries, color=mlsystext] {Fat-Tree (3-Tier Fabric)};
    \node at (7.25, 8.48) [anchor=east, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {InfiniBand / RoCEv2};

    % Left row labels
    \node at (0.35, 7.35) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysgray] {Core};
    \node at (0.35, 6.25) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysgray] {Spine};
    \node at (0.35, 5.15) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysgray] {Leaf (ToR)};
    \node at (0.35, 3.95) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysgray] {GPU Racks};

    % Inter-tier Connections (all x shifted right by 0.55 cm)
    % Core to Spine (dashed faint lines)
    \foreach \c in {3.45, 6.05} {
      \foreach \s in {2.55, 3.65, 5.25, 6.35} {
        \draw[draw=mlsysbordergray!80, dashed, line width=0.5pt] (\c, 7.15) -- (\s, 6.45);
      }
    }

    % Spine to Leaf (bipartite per pod)
    % Pod 1: (2.55, 3.65) <-> (2.55, 3.65)
    \foreach \s in {2.55, 3.65} {
      \foreach \l in {2.55, 3.65} {
        \draw[draw=mlsysbordergray, line width=0.7pt] (\s, 6.05) -- (\l, 5.35);
      }
    }
    % Pod 2: (5.25, 6.35) <-> (5.25, 6.35)
    \foreach \s in {5.25, 6.35} {
      \foreach \l in {5.25, 6.35} {
        \draw[draw=mlsysbordergray, line width=0.7pt] (\s, 6.05) -- (\l, 5.35);
      }
    }

    % Leaf to Racks
    \draw[draw=mlsysdarkblue, line width=0.8pt] (2.55, 4.95) -- (2.15, 4.15);
    \draw[draw=mlsysdarkblue, line width=0.8pt] (2.55, 4.95) -- (2.75, 4.15);
    \draw[draw=mlsysdarkblue, line width=0.8pt] (3.65, 4.95) -- (3.35, 4.15);
    \draw[draw=mlsysdarkblue, line width=0.8pt] (3.65, 4.95) -- (3.95, 4.15);

    \draw[draw=mlsysdarkblue, line width=0.8pt] (5.25, 4.95) -- (4.95, 4.15);
    \draw[draw=mlsysdarkblue, line width=0.8pt] (5.25, 4.95) -- (5.55, 4.15);
    \draw[draw=mlsysdarkblue, line width=0.8pt] (6.35, 4.95) -- (6.15, 4.15);
    \draw[draw=mlsysdarkblue, line width=0.8pt] (6.35, 4.95) -- (6.75, 4.15);

    % Core Nodes (2 nodes)
    \foreach \x/\label in {3.45/{Core 1}, 6.05/{Core 2}} {
      \draw[draw=mlsyspurple, fill=mlsyslightpurple, rounded corners=2pt, line width=0.9pt] (\x-0.75, 7.15) rectangle (\x+0.75, 7.55);
      \node at (\x, 7.35) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkpurple] {\label};
    }

    % Spine Nodes (4 nodes)
    \foreach \x/\label in {2.55/{Sp 1}, 3.65/{Sp 2}, 5.25/{Sp 3}, 6.35/{Sp 4}} {
      \draw[draw=mlsysamber, fill=mlsyslightamber, rounded corners=2pt, line width=0.9pt] (\x-0.45, 6.05) rectangle (\x+0.45, 6.45);
      \node at (\x, 6.25) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkamber] {\label};
    }

    % Leaf Nodes (4 nodes)
    \foreach \x/\label in {2.55/{Lf 1}, 3.65/{Lf 2}, 5.25/{Lf 3}, 6.35/{Lf 4}} {
      \draw[draw=mlsysamber, fill=mlsyslightamber, rounded corners=2pt, line width=0.9pt] (\x-0.45, 4.95) rectangle (\x+0.45, 5.35);
      \node at (\x, 5.15) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkamber] {\label};
    }

    % Rack Nodes (8 nodes)
    \foreach \x/\i in {2.15/1, 2.75/2, 3.35/3, 3.95/4, 4.95/5, 5.55/6, 6.15/7, 6.75/8} {
      \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=1.5pt, line width=0.8pt] (\x-0.24, 3.75) rectangle (\x+0.24, 4.15);
      \node at (\x, 3.95) [anchor=center, font=\fontsize{7}{8.5}\selectfont\bfseries, color=mlsysdarkblue] {R\i};
    }

    % Properties Card
    \draw[draw=mlsysbordergray!80, fill=white, rounded corners=3pt, line width=0.8pt] (0.35, 0.35) rectangle (7.25, 2.75);
    \draw[draw=mlsysgreen, fill=mlsyslightgreen, rounded corners=2pt, line width=0.7pt] (0.55, 2.25) rectangle (4.25, 2.62);
    \node at (2.4, 2.43) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkgreen] {Full Bisection Bandwidth};

    \node at (0.55, 1.95) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Non-blocking:} Any-to-any without oversubscription};
    \node at (0.55, 1.55) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Multipath:} ECMP routing spreads elephant flows};
    \node at (0.55, 1.15) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Optics cost:} High transceiver count drives cluster CapEx};
    \node at (0.55, 0.7) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkgreen] {Preferred for synchronous AllReduce collectives};
  \end{scope}

  % =========================================================================
  % CARD 2: 2D TORUS (DIRECT MESH FABRIC)
  % =========================================================================
  \begin{scope}[shift={(8.2,0)}]
    ...
    \fill[mlsyslightgray] (0, 8.15) rectangle (7.6, 8.35);
    \draw[draw=mlsysbordergray, line width=0.8pt] (0, 8.15) -- (7.6, 8.15);
    \node at (0.35, 8.48) [anchor=west, font=\fontsize{9}{10.5}\selectfont\bfseries, color=mlsystext] {2D Torus (Direct Mesh Fabric)};
    \node at (7.25, 8.48) [anchor=east, font=\fontsize{7.5}{9}\selectfont, color=mlsysgray] {Google TPU Pods};

    % 4x4 Torus Grid (Center at x=3.8, cols at 1.7, 3.1, 4.5, 5.9; rows at 7.1, 6.0, 4.9, 3.8)
    % Grid lines (horizontal)
    \foreach \y in {7.1, 6.0, 4.9, 3.8} {
      \draw[draw=mlsysbordergray, line width=1.1pt] (1.7, \y) -- (5.9, \y);
    }
    % Grid lines (vertical)
    \foreach \x in {1.7, 3.1, 4.5, 5.9} {
      \draw[draw=mlsysbordergray, line width=1.1pt] (\x, 3.8) -- (\x, 7.1);
    }

    % Torus wrap-around curves
    % Top wrap (row 0: 5.9 to 1.7)
    \draw[draw=mlsyspurple, dashed, line width=1pt] (5.9, 7.1) .. controls (6.4, 7.85) and (4.8, 7.85) .. (3.8, 7.85) .. controls (2.8, 7.85) and (1.2, 7.85) .. (1.7, 7.1);
    \draw[draw=mlsyspurple, fill=mlsyslightpurple, rounded corners=2pt, line width=0.6pt] (3.05, 7.65) rectangle (4.55, 8.05);
    \node at (3.8, 7.85) [anchor=center, font=\fontsize{7}{8.5}\selectfont\bfseries, color=mlsysdarkpurple] {Torus Wrap};

    % Left wrap (col 0: 3.8 to 7.1)
    \draw[draw=mlsyspurple, dashed, line width=1pt] (1.7, 3.8) .. controls (0.9, 3.8) and (0.9, 7.1) .. (1.7, 7.1);

    % 16 Nodes
    \foreach \r/\y in {0/7.1, 1/6.0, 2/4.9, 3/3.8} {
      \foreach \c/\x in {0/1.7, 1/3.1, 2/4.5, 3/5.9} {
        \draw[draw=mlsysblue, fill=mlsyslightblue, rounded corners=2pt, line width=0.9pt] (\x-0.38, \y-0.22) rectangle (\x+0.38, \y+0.22);
        \node at (\x, \y) [anchor=center, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkblue] {N\r\c};
      }
    }

    % Properties Card
    \draw[draw=mlsysbordergray!80, fill=white, rounded corners=3pt, line width=0.8pt] (0.35, 0.35) rectangle (7.25, 2.75);
    \draw[draw=mlsysamber, fill=mlsyslightamber, rounded corners=2pt, line width=0.7pt] (0.55, 2.25) rectangle (4.25, 2.62);
    \node at (2.4, 2.43) [anchor=center, font=\fontsize{7.5}{9}\selectfont\bfseries, color=mlsysdarkamber] {Bisection Bandwidth: $O(\sqrt{N})$};

    \node at (0.55, 1.95) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Direct wiring:} Zero external switches; node-to-node links};
    \node at (0.55, 1.55) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Bisection cut:} Scales as $O(N^{1-1/d})$ with dimension $d$};
    \node at (0.55, 1.15) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont, color=mlsystext] {\textbf{Locality:} Demands tight coordinate locality};
    \node at (0.55, 0.7) [anchor=west, font=\fontsize{7.2}{8.5}\selectfont\bfseries, color=mlsysdarkamber] {Optical Circuit Switches (OCS) reconfigure 3D topologies};
  \end{scope}
...
The engineering challenge of the pod\index{Pod} is wiring those nodes together into a network fast enough to keep gradient synchronization from becoming the bottleneck. The pod aggregates hundreds of racks into a single, coordinated computing system where the network fabric serves the same role that the system bus serves within a single machine.
...
class PodTrainingMath:
    ...
    cluster_throughput_math = fmt_math(f"{n_gpus_fmt} \\times {h100_fp8_tflop_per_s_value_str} \\times 10^{{12}} \\times {mfu}")
...
Training a `{python} InfraFrontierPodTrainingRecap.frontier_params_b_str` model on this cluster requires `{python} PodTrainingMath.pod_flops_math` total floating-point operations (assuming `{python} PodTrainingMath.gpt3_tokens_billion_str` training tokens). At 45 percent MFU -- a strong but achievable figure for well-tuned large-model training -- the cluster sustains `{python} PodTrainingMath.cluster_throughput_math` FLOP/s, or about `{python} PodTrainingMath.cluster_eflop_per_s_str`, yielding a physics-limit training time of approximately 12 hours.
...
class TrainingTime175B:
    ...
    t_base = dTime(total_ops=6 * n_params * n_tokens * flop, num_devices=n_gpus, peak_flops_per_device=peak_flops, efficiency_eta=mfu)
...
2. **Cluster throughput**: `{python} PodTrainingMath.n_gpus_str` H100 GPUs at `{python} PodTrainingMath.h100_fp8_tflop_per_s_str` peak, operating at `{python} PodTrainingMath.mfu_pct_str` MFU:
   `{python} PodTrainingMath.cluster_throughput_math` FLOP/s sustained $\approx$ `{python} PodTrainingMath.cluster_eflop_per_s_str`
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: WSC definition callout in @sec-compute-pod — aggregate FP16/FP8
...
::: {#dfn-compute-infrastructure-warehouse-scale-computer-wsc .callout-definition title="WSC"}

***WSC***\index{Warehouse-Scale Computer!definition}\index{WSC!data center scale} is a building-scale computing system in which thousands of servers are operated as a single coherent machine (with the network fabric serving as the system bus, distributed storage as the disk subsystem, and a cluster orchestrator as the operating system), enabling training workloads that would be physically impossible on any single machine.

1.  **Significance**: A WSC of `{python} WscThroughput.n_gpus_str` H100 GPUs delivers approximately `{python} WscThroughput.wsc_fp16_eflop_per_s_str` FP16 peak (and ~`{python} WscThroughput.wsc_fp8_eflop_per_s_str` at FP8), enabling 175B-class training workloads at scales impossible on a single node. However, the system is only as fast as its bisection bandwidth: non-blocking InfiniBand at 50 GB/s per GPU provides `{python} WscThroughput.wsc_inject_tb_per_s_str` aggregate injection bandwidth; if the fabric is 4:1 oversubscribed, effective bisection bandwidth drops to `{python} WscThroughput.wsc_oversub_tb_per_s_str`, potentially bottlenecking AllReduce and dropping training throughput by 30–60 percent.
...
Luiz Andre Barroso, Urs Holzle, and Parthasarathy Ranganathan formalized the concept in *The Datacenter as a Computer* [@barroso2019]. At Google's scale, *the data center is the computer*, so computer-architecture principles apply at the building level as well as the chip level.

### WSC architecture principles {#sec-compute-wsc-principles}

::: {.column-margin}
![](images/svg/vol2_compute_infrastructure_margin_003.svg){width="100%" fig-alt="Three-rung time ladder: one GPU has about 50,000 hours mean time to failure, 1,000 GPUs about 50 hours, and 10,000 GPUs about 5 hours."}

*Fleet-level MTBF falls inversely with GPU count.*
:::

The Warehouse-Scale Computer concept reframes the data center from a *room full of computers* to a *single computer that happens to fill a room* [@barroso2019]. This view changes how engineers approach physical design.
...
::: {.column-margin}
![](images/svg/compute_infrastructure_cxl_bandwidth_gap.svg){width="100%" fig-alt="Two-rung bandwidth ladder comparing HBM3 with CXL memory and showing a large bandwidth gap."}
...
Underpinning both directions is the evolution of **advanced packaging**\index{Advanced packaging}. Packaging moves the boundary inside the accelerator package rather than all the way out to the rack or all the way in to a single wafer, and it matters because the reticle limit is only about `{python} ReticleLimitRecap.reticle_limit_mm2_str` for a single exposure. **2.5D interposers**\index{2.5D interposers}, such as TSMC's CoWoS, mount compute dies and HBM stacks on a shared silicon substrate, allowing chiplet-based accelerators to behave like a single device for most software. **3D stacking**\index{3D stacking}, with approaches such as SoIC, places SRAM caches or logic directly above one another and shortens data paths further. The H100-to-B200 transition shows the fleet consequence: the package can deliver far more local arithmetic and die-to-die bandwidth, but the rack must absorb higher power density, tighter cooling requirements, and less tolerance for power transients before the silicon advantage becomes useful infrastructure.

### Reconfigurable topologies and optical circuit switching {#sec-compute-reconfigurable-topologies-ocs}

While CXL and advanced packaging address memory and die boundaries, warehouse-scale computing increasingly encounters an interconnect power and topology constraint. In modern clusters spanning tens of thousands of accelerators, conventional multi-tier Clos networks rely on electrical packet switches. Every network hop incurs an Optical-Electrical-Optical (OEO) conversion: incoming photons hit a photodiode, high-speed SerDes circuits deserialize the electrical signal, switch ASICs buffer and route packets across crossbars, and output SerDes drive lasers to re-transmit light over optical fiber.

At 800 Gb/s and 1.6 Tb/s per-port data rates, SerDes and transceiver electronics consume significant energy—often accounting for 10 to 20 percent of total data center power. Furthermore, fixed electrical Clos topologies are cabled statically, locking the facility into a single oversubscription ratio and rigid bisection pattern regardless of whether the running workload is pipeline-dominated, data-parallel, or all-to-all expert-routed.
...
1.  **Significance**: Because photons pass through mirror reflections without digital processing, an OCS consumes virtually zero active switching power ($<5$ W per switch chassis, compared to multi-kilowatt electrical switch chassis) and is inherently protocol- and bitrate-agnostic. A fiber infrastructure deployed with OCS can scale from 400 Gb/s to 800 Gb/s and 1.6 Tb/s without replacing switches.
...
This architecture delivers two fundamental operational capabilities:

1.  **Workload-tailored topologies**: The physical interconnect can be reconfigured dynamically to match the communication structure of the running workload. A model utilizing 1D pipeline parallelism can configure its slice as an elongated cylinder; a 2D/3D tensor-parallel model can configure a compact, cubic 3D torus with minimal average hop count; and an MoE workload can maximize bisection bandwidth across specific expert groups.
2.  **Dynamic fault recovery and boundary wrapping**: In massive accelerator pods, optical transceivers and cables are among the most frequent failure components. In a static torus or tree, a single link or node failure can isolate a dimension or degrade the entire pod. With OCS, the cluster manager detects the defective tray or fiber and commands the MEMS mirrors to rotate, optically bypassing the failed unit and splicing healthy nodes into a complete, smaller torus slice in tens of milliseconds without manual cable intervention.

The trade-off is switching latency. Because MEMS mirrors physically rotate to steer collimated light beams, reconfiguration takes $10$ to $30$ milliseconds—seven orders of magnitude slower than electrical packet switching ($10$ to $100$ nanoseconds). Consequently, OCS moves the power and protocol wall while leaving reconfiguration latency as the binding discipline: it is deployed for job-level topology allocation and fault isolation, not for per-message dynamic routing.

The common theme is relocation rather than elimination. CXL moves capacity outward into a fabric and leaves bandwidth as the limiting discipline. Wafer-scale integration moves communication inward and leaves capacity and yield as the limiting disciplines. Advanced packaging compresses die-to-die distance and leaves power density as the limiting discipline. Optical circuit switching eliminates transceiver OEO power and leaves switching reconfiguration latency as the limiting discipline. Infrastructure planning therefore still begins by identifying where the next byte moves, how quickly it must arrive, and what physical system must be built so that movement does not dominate the workload.
...
class GpuHourEquivalence:
    ...
    h100_flops = Hardware.Cloud.H100.compute.precision_flops["fp8"]  # FP8 tensor
...
A common error in cost comparisons is to treat one GPU-hour on an H100 as interchangeable with one GPU-hour on an A100 or V100. In reality, the effective cost per unit of useful computation varies enormously across GPU generations: newer accelerators deliver far more useful throughput per dollar. An H100 GPU-hour delivers `{python} GpuHourEquivalence.ratio_a100_mult_str` the TFLOP/s of an A100 GPU-hour and `{python} GpuHourEquivalence.ratio_v100_mult_str` the TFLOP/s of a V100 GPU-hour. A training run that takes `{python} GpuHourEquivalence.v100_hours_str` V100 GPU-hours would require only approximately `{python} GpuHourEquivalence.equiv_h100_hours_str` H100 GPU-hours to complete the same amount of computation. The relevant metric is not cost per GPU-hour but cost per useful TFLOP-hour, which accounts for both the price per hour and the effective throughput delivered. Organizations that benchmark cloud options by GPU-hour cost alone systematically overestimate the expense of newer, more efficient hardware.
...
class NetworkUpgradeStepTime:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    ib_hdr_str = fmt_qty(ib_hdr, Gbps, precision=0, commas=False)
    ib_ndr_str = fmt_qty(ib_ndr, Gbps, precision=0, commas=False)
...
```{python}
#| echo: false
from mlsysim.fmt import fmt_int, fmt_time, fmt_memory_capacity
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_multiple
...
Our `{python} InfraFrontierClosingRecap.frontier_params_b_str`-parameter model has served as the architectural forcing function connecting every layer of this hierarchy. At the accelerator level, processing a token requires about 350 billion floating-point operations, yet arithmetic is not the limiting resource on Tensor Cores. At the memory level, the 350 GB FP16 weight tensor exceeds one accelerator's HBM capacity; even after sharding or compression solves capacity, streaming the weights at `{python} SummaryScaleScenario.h100_bw_tb_per_s_str` imposes a latency floor of over 100 ms per token for one accelerator-equivalent HBM path. At the node level, training state expands beyond 2 TB, requiring tensor parallelism, data-parallel sharding, and memory offload. At the rack level, 32 accelerators drawing `{python} SummaryScaleScenario.h100_tdp_w_str` each, plus host and network overhead, require `{python} SummaryScaleScenario.rack_power_kw_str` of power delivery and liquid cooling. At the pod level, a viable training window requires synchronizing over 1,000 GPUs across a non-blocking fabric and checkpointing against inevitable failures. The economics finally depend on whether sustained utilization can amortize hardware, power, cooling, and facility risk. Every constraint traces back to one fact: this model does not fit on a single chip.
...
Applied through C$^3$, choosing liquid cooling for a dense accelerator rack is not merely a facility upgrade. It permits more compute per rack, which concentrates communication traffic onto fewer top-of-rack links and increases the coordination burden of power capping, failure isolation, and maintenance. The original thermal ceiling recedes, but the binding constraint can move to rack power delivery or network injection; a design that does not provision those downstream limits converts cooling headroom into idle silicon.

::: {.callout-takeaways title="The data center is the computer"}

* **The generality tax**: Accelerators achieve 10--100$\times$ higher throughput than CPUs by dedicating silicon area to matrix arithmetic rather than branch prediction and out-of-order execution (principle \ref{pri-generality-tax}). The trade-off is reduced flexibility. Each step along the spectrum from GPU to TPU to custom ASIC reclaims more die area for arithmetic.
...
* **Bandwidth hierarchy dictates parallelism**: The `{python} SummaryScaleScenario.nvlink_ib_gap_mult_str` per-direction bandwidth cliff between NVLink and InfiniBand confines tensor parallelism to within a node and data parallelism to across nodes. Pipeline parallelism spans nodes when model depth exceeds a single node's capacity.
