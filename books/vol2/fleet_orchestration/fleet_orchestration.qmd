...
Orchestration became a first-order systems problem for the same reason: @fig-cluster-size-scaling shows accelerator counts per training run rising by orders of magnitude between 2012 and 2024, and a scheduler that was adequate for tens of devices is not adequate for tens of thousands.
...
data = pd.read_csv("vol2/fleet_orchestration/data/cluster_scaling.csv")

fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 5))
ax.plot(data["Year"], data["Accelerators"], marker="o", linewidth=2, markersize=8, color="#1f77b4")
