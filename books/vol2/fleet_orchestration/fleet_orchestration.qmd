...
::: {#fig-cluster-size-scaling fig-env="figure" fig-pos="htb" fig-cap="**Cluster Size Scaling**: The exponential growth of state-of-the-art ML training clusters from 2012 to 2024. As models scale, the number of accelerators required has increased by orders of magnitude, transforming orchestration into a massive distributed systems challenge." fig-alt="Line chart showing exponential growth of ML training cluster sizes from 2 GPUs for AlexNet in 2012 to 100,000 GPUs in 2024."}

```{python}
#| echo: false

# ┌─────────────────────────────────────────────────────────────────────────────
# │ CLUSTER SIZE SCALING (FIGURE)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @fig-cluster-size-scaling - introduction
# │
# │ Goal: Plot the exponential growth of ML training cluster sizes over time.
# │ Show: Line chart with log scale on y-axis, annotating key models.
# │ How: Load data from data/cluster_scaling.csv and plot using matplotlib.
# │
# └─────────────────────────────────────────────────────────────────────────────
...
try:
    from mlsysim.style import book_style
    book_style.setup_plot()
except ImportError:
    sns.set_theme(style="whitegrid")

data = pd.read_csv("data/cluster_scaling.csv")

fig, ax = plt.subplots(figsize=(8, 5))
sns.lineplot(data=data, x="Year", y="Accelerators", marker="o", ax=ax, color="#1f77b4", linewidth=2, markersize=8)

for i in range(len(data)):
    model = data['Model'].iloc[i]
    if model in ['AlexNet', 'ResNet', 'BERT', 'GPT-3', 'Llama-3', 'Grok-3']:
        ax.annotate(model, (data['Year'].iloc[i], data['Accelerators'].iloc[i]),
                    textcoords="offset points", xytext=(-5, 10), ha='right', fontsize=9)

ax.set_yscale("log")
ax.set_ylabel("Number of Accelerators")
ax.set_xlabel("Year")
ax.set_title("The Exponential Growth of ML Training Clusters")
plt.tight_layout()
plt.show()
plt.close()
```
...
This data feeds the utilization dashboard, the cluster operator's primary instrument for fleet health. This view must synthesize per-team utilization against quotas, queue depth by priority class, and the **Fragmentation Index**\index{Fragmentation Index!definition}, a measure of free GPUs that cannot be allocated due to topology constraints. The dashboard must also track the "cost of chaos": preemption rates, spot instance interruption frequencies, and the recovery overhead they induce. When these metrics are visible, they drive behavior. A `{python} ChargebackExample.chargeback_gpus_str`-GPU fine-tuning run for our 175B model held for `{python} ChargebackExample.chargeback_hours_h_str` consumes about `{python} ChargebackExample.chargeback_cost_str` of capacity (`{python} ChargebackExample.chargeback_gpus_str` GPUs $\times$ `{python} ChargebackExample.chargeback_hours_h_str` $\times$ `{python} ChargebackExample.chargeback_rate_str`). Without proper accounting, this cost is invisible to the team that requested it. With chargeback, the team must justify this expenditure against the model's expected business value.
