...
% Full-size stack with data bar for introductory/explanatory use
\newcommand{\stacklayerfull}[4]{
    % Helper for full-size layers
    % #1: y-position
    % #2: Layer name
    % #3: Intensity (0-100)
    % #4: Icon name
    ...
    \node[anchor=west, text=\textcol, font=\usefont{T1}{phv}{b}{n}\fontsize{9.5}{10.5}\selectfont]
        at ($(box.west)+(0.25,0)$) {#2};
    
    % Icon (positioned on the right)
    \path (box.east) +(-0.5,0) pic[scale=0.85, draw=\textcol, fill=\textcol, text=\textcol] {#4};
}
...
\newcommand{\mlsysstackfull}[8]{
    \begin{tikzpicture}
        ...
        % Layer 7: Applications
        \stacklayerfull{6.60}{Applications}{#7}{icon_applications}

        % Layer 6: Operations
        \stacklayerfull{5.50}{Operations}{#6}{icon_operations}

        % Layer 5: Serving
        \stacklayerfull{4.40}{Serving}{#5}{icon_serving}

        % Layer 4: Training
        \stacklayerfull{3.30}{Training}{#4}{icon_training}

        % Layer 3: Models
        \stacklayerfull{2.20}{Models}{#3}{icon_models}

        % Layer 2: Frameworks
        \stacklayerfull{1.10}{Frameworks}{#2}{icon_frameworks}

        % Layer 1: Hardware
        \stacklayerfull{0.0}{Hardware}{#1}{icon_hardware}
