...
def _find_qmd_roots() -> list[tuple[Path, Path]]:
    ...
    """
    candidates = [
        Path("."),                 # run from the project root (most common)
        Path("..") / "..",         # run from books/shared/scripts/
        Path("books"),             # run from the repo root
    ]
    # Since 2026-09 the volumes sit directly under the Quarto project root
    # (books/vol1, books/vol2, ...) rather than under a contents/ subdirectory,
    # so the project root and the scan root are the same directory.
    for c in candidates:
        if (c / "vol1").exists():
            return [(c, c)]
    return []
...
PRINCIPLE_PARTS_ORDER: dict[str, list[str]] = {
    "vol1": [
        "vol1/parts/foundations_principles.qmd",
        "vol1/parts/build_principles.qmd",
        "vol1/parts/optimize_principles.qmd",
        "vol1/parts/deploy_principles.qmd",
    ],
    "vol2": [
        "vol2/parts/fleet_principles.qmd",
        "vol2/parts/distributed_ml_principles.qmd",
        "vol2/parts/deployment_principles.qmd",
        "vol2/parts/responsible_fleet_principles.qmd",
    ],
}
...
def calculate_relative_path(
    ...
    Args:
        ...
        to_path: String path from build root (e.g., "vol1/chapter/file.html#anchor")
