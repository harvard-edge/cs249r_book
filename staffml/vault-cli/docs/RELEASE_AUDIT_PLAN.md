...
- **Persistent progress** — writes `staffml/vault/_pipeline/audit-progress.json` after each call so a Ctrl-C / kill / overnight pause picks back up cleanly
...
**Action:** create `staffml/vault/AUTHORING.md` that codifies:
...
- This document at `staffml/vault-cli/docs/RELEASE_AUDIT_PLAN.md`
- All tooling under `staffml/vault-cli/scripts/`
- Format-audit findings already cached at
  `staffml/vault/_pipeline/format-audit-full.json` (rerun if stale)
...
2. Run baseline validators:
   ...
   pytest staffml/vault-cli/tests/ -q  # expect: 74/74 pass
   ruff check staffml/vault-cli         # expect: clean
   ```
3. **Read** this entire document, then `staffml/vault/README.md`
   §"Pipeline artifacts" for the `_pipeline/` convention.
4. **Read** `staffml/vault-cli/docs/CHAIN_ROADMAP.md` Progress Log
...
2. **Run mechanical cleanups** (no Gemini cost):
   ...
   c. Write `staffml/vault/AUTHORING.md` (1 commit)
...
`staffml/vault/_pipeline/audit-cost-ledger.json`:
