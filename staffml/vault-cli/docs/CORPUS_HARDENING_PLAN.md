...
question from `staffml/vault/AUTHORING.md` alone, and `vault new`
...
- [ ] `staffml/vault/schema/question_schema.yaml` (LinkML) has `pattern`
...
- [ ] `staffml/vault/AUTHORING.md` exists. Includes: required-field
...
- [ ] Single new script `audit_corpus_batched.py` exists under
      `staffml/vault-cli/scripts/`. Batches 30-40 questions per
...
- [ ] Old / wrong-design tooling deleted: `audit_corpus.py` (this
      ...
      `staffml/vault/scripts/` listed in DEPRECATED.md.

### 2.5 Pipeline cleanliness

- [ ] `pytest staffml/vault-cli/tests/` — 74/74 (or higher; new tests welcome).
- [ ] `ruff check staffml/vault-cli` — clean.
- [ ] `mypy staffml/vault-cli/src` — clean (currently non-blocking).
...
`staffml/vault/_pipeline/format-audit-full.json` (regenerable;
gitignored).
...
1. **Provenance backfill.** Script:
   `staffml/vault-cli/scripts/backfill_provenance.py`. For each
...
1. **Write `staffml/vault/AUTHORING.md`.** Required sections:
...
New file:
`staffml/vault-cli/scripts/audit_corpus_batched.py`.
...
- `staffml/vault-cli/scripts/_judges.py` — single source of truth
  for all judge prompts. Imported by both `validate_drafts.py` (drafts)
  and `audit_corpus_batched.py` (corpus).
- `staffml/vault-cli/scripts/_batching.py` — `pack_batches`,
  `MAX_PROMPT_CHARS`.
...
2. **Surface findings** in a triage doc
   `staffml/vault-cli/docs/AUDIT_FINDINGS_<DATE>.md`:
...
1. **Update LinkML schema** `staffml/vault/schema/question_schema.yaml`:
   ```yaml
   Details:
     attributes:
       common_mistake:
...
2. **Set `extra="forbid"`** on `Details` Pydantic model (was `allow`).
   ...
   keys = Counter()
   for p in Path('staffml/vault/questions').rglob('*.yaml'):
...
|---|---|---|
| `staffml/vault-cli/scripts/audit_chains_with_gemini.py` | **keep** — extend with `corpus_sample` category if helpful | gold-standard batched pattern; reuse |
| `staffml/vault-cli/scripts/build_chains_with_gemini.py` | **keep** — extract `plan_batches` into shared `_batching.py` | active; reusable helper |
| `staffml/vault-cli/scripts/validate_drafts.py` | **keep** — refactor to import judges from `_judges.py` | active for drafts; prompts are gold |
| `staffml/vault-cli/scripts/audit_math.py` | **keep, deprecate slowly** — useful for spot-checking specific qids | active but redundant with `audit_corpus_batched.py` |
| `staffml/vault-cli/scripts/generate_question_for_gap.py` | **keep** — extract `SCHEMA_SUMMARY` into a shared module | active; reusable schema text block |
| `staffml/vault-cli/scripts/audit_corpus.py` | **DELETE** in Phase 0 | this session's wrong-design dead-end |
| `staffml/vault-cli/scripts/_judges.py` | **NEW (Phase 3)** — extracted shared judge prompts | DRY |
| `staffml/vault-cli/scripts/_batching.py` | **NEW (Phase 3)** — extracted batching helper | DRY |
| `staffml/vault-cli/scripts/audit_corpus_batched.py` | **NEW (Phase 3)** — the right corpus audit tool | the user's actual ask |
| `staffml/vault-cli/scripts/apply_corrections.py` | **NEW (Phase 5)** — interactive accept/reject for Gemini-proposed corrections | safe-by-default human review |
| `staffml/vault-cli/scripts/backfill_provenance.py` | **NEW (Phase 1)** — explicit provenance for 407 questions | one-shot mechanical pass |
| `staffml/vault/scripts/*` (legacy dir) | **already deleted in this session** — phase 0 commit | pre-YAML-migration cruft |
...
The `EVOLUTION.md` doc at `staffml/vault/schema/EVOLUTION.md` already
...
- **Topic taxonomy revision.** The 87 topics in
  `staffml/vault/taxonomy.yaml` are accepted as-is.
