...
4. **Check for in-flight artifacts** (all gitignored under `_pipeline/`,
   see [`staffml/vault/README.md`](../../vault/README.md) §"Pipeline
   artifacts" for the convention):
   ```bash
   ls -la staffml/vault/_pipeline/chains.proposed*.json 2>/dev/null
   ls -la staffml/vault/_pipeline/gaps.proposed*.json   2>/dev/null
   ls -la staffml/vault/_pipeline/runs/                 2>/dev/null
   ```
...
- **Hierarchy:** all questions at `staffml/vault/questions/<track>/<area>/<id>.yaml`.
...
- **Gap backlog:** 138 (strict) + 269 (lenient) = 407 entries across
  ...
  (gitignored — see [`staffml/vault/README.md`](../../vault/README.md)
  §"Pipeline artifacts").
- **Pre-Gemini chains backed up:** `staffml/vault/chains.json.bak` (726 chains).
- **Validators:** all green as of last commit `1ac7d4c56`.
- **UI tests:** `staffml/app/tests/chain-and-vault-smoke.mjs` — 13/13 pass.
...
**Deliverable:** `staffml/vault-cli/scripts/diagnose_chain_coverage.py`
...
- For each (track, topic) bucket, reports:
  ...
  - Current chain count
  - List of qids
- Output: `staffml/vault/chain-coverage.json` with two arrays:
...
**Files:**
- create: `staffml/vault-cli/scripts/diagnose_chain_coverage.py`
- create: `staffml/vault/chain-coverage.json` (gitignored — regeneratable)
...
**Files:**
- modify: `staffml/vault-cli/scripts/build_chains_with_gemini.py`
...
**Validation:** New unit test in `staffml/vault-cli/tests/test_chain_validation.py`
...
**Files:**
- modify: `staffml/vault-cli/scripts/build_chains_with_gemini.py`
- modify: `staffml/vault-cli/scripts/apply_proposed_chains.py`
- create: `staffml/vault-cli/tests/test_chain_validation.py` (or add to existing)
...
**Deliverable:** `staffml/vault/chains.proposed.lenient.json` with chains
for the ~150 uncovered buckets.

**Command:**
```bash
python3 staffml/vault-cli/scripts/build_chains_with_gemini.py \
  --mode lenient \
  --buckets-from staffml/vault/chain-coverage.json \
  --output staffml/vault/chains.proposed.lenient.json
```
...
- Check progress: `python3 -c "import json; print(len(json.load(open('staffml/vault/chains.proposed.lenient.json'))))"`
...
**Files:**
- create: `staffml/vault/chains.proposed.lenient.json`
- update: `staffml/vault/gaps.proposed.json` (lenient pass also emits gaps)
...
**Deliverable:** `staffml/vault-cli/scripts/merge_chain_passes.py`
...
**Files:**
- create: `staffml/vault-cli/scripts/merge_chain_passes.py`
- modify: `staffml/vault/chains.json`
...
- `staffml/vault-cli/src/vault_cli/validator.py`: tier required, default to "primary" if missing
- `staffml/vault-cli/src/vault_cli/legacy_export.py`: include tier in
  corpus.json output as `chain_tiers: {chain_id: "primary"|"secondary"}`
  per question (mirrors `chain_positions` shape)

**Files:**
- modify: `staffml/vault/chains.json` (backfill primary tag)
- modify: `staffml/vault-cli/src/vault_cli/validator.py`
- modify: `staffml/vault-cli/src/vault_cli/legacy_export.py`
...
**Files:**
- modify: `staffml/app/src/lib/corpus.ts`
...
**Files to modify:**
- `staffml/app/src/components/ChainStrip.tsx` — default to primary; subtle "Alternative path" hint when only secondary
- `staffml/app/src/app/practice/page.tsx` — "next in chain" prefers primary
- `staffml/app/src/app/explore/page.tsx` — filter dropdown "Primary only / All"
...
**Files:**
- modify: `staffml/app/tests/chain-and-vault-smoke.mjs`
...
**Deliverable:** `staffml/vault-cli/scripts/generate_question_for_gap.py`
...
6. Writes to `staffml/vault/questions/<track>/<area>/<auto-id>.yaml.draft`
...
**Files:**
- create: `staffml/vault-cli/scripts/generate_question_for_gap.py`
...
**Deliverable:** `staffml/vault-cli/scripts/validate_drafts.py`
...
**Files:**
- create: `staffml/vault-cli/scripts/validate_drafts.py`
...
**Files:**
- create: `staffml/vault/questions/<track>/<area>/<auto-id>.yaml.draft` × 30
- create: `staffml/vault/draft-validation-scorecard.json`
...
```bash
python3 staffml/vault-cli/scripts/build_chains_with_gemini.py --all \
  --output staffml/vault/chains.proposed.json
python3 staffml/vault-cli/scripts/apply_proposed_chains.py
vault check --strict
...
**When:** low priority
**Action:** re-run `staffml/vault-cli/scripts/cross_encoder_rerank_experiment.py`
...
`staffml/vault/questions/` is gitignored. Implementation: `git
...
**What was done:**
- Created this roadmap document (`staffml/vault-cli/docs/CHAIN_ROADMAP.md`)
...
**What was done:**
- Added `staffml/vault-cli/scripts/diagnose_chain_coverage.py`
...
- Output: `staffml/vault/chain-coverage.json` (gitignored —
  regeneratable). Added `/chain-coverage.json` to `staffml/vault/.gitignore`.
...
**Files committed:**
- `staffml/vault-cli/scripts/diagnose_chain_coverage.py` (new)
- `staffml/vault/.gitignore` (add `/chain-coverage.json`)
...
- **Phase 1.5 — merge:** Wrote
  `staffml/vault-cli/scripts/merge_chain_passes.py`. Backfills
...
- **Phase 1.6 — validate:**
  ...
  - Started `next dev` server; ran
    `node staffml/app/tests/chain-and-vault-smoke.mjs` →
...
- `staffml/vault-cli/scripts/merge_chain_passes.py` (new)
- `staffml/vault/chains.json` (373 → 879)
- `staffml/vault/chains.proposed.lenient.json` (durable record of the
  lenient pass)
- `staffml/vault/gaps.proposed.lenient.json` (269 new gaps for Phase 3)
- `staffml/app/src/data/vault-manifest.json` (chainCount + releaseHash)
- `staffml/app/src/data/corpus-summary.json` (chain memberships
  per question — derived by `vault build`)
- `staffml/vault-cli/docs/CHAIN_ROADMAP.md` (this Progress Log entry,
...
- `app/src/lib/corpus.ts`: `Question.chain_tiers?` added (optional
...
- `pytest staffml/vault-cli/tests/`: 74/74 pass
- `npx tsc --noEmit`: 0 errors
- `node staffml/app/tests/chain-and-vault-smoke.mjs`: 17/17
...
- Reads a gap entry (`{track, topic, missing_level, between, rationale}`)
  ...
  `staffml/vault/questions/<track>/<area>/<id>.yaml.draft`.
...
- Output: `staffml/vault/draft-validation-scorecard.json` with per-row
...
1. Pick 30 gaps from `gaps.proposed.lenient.json` where the bucket has
   ...
   ```bash
   python3 staffml/vault-cli/scripts/generate_question_for_gap.py \
     --gaps-from staffml/vault/gaps.proposed.lenient.json \
     --limit 30
   ```
2. Validate:
   ```bash
...
**Files committed:**
- `staffml/vault-cli/scripts/generate_question_for_gap.py` (new)
- `staffml/vault-cli/scripts/validate_drafts.py` (new)
- `staffml/vault-cli/docs/CHAIN_ROADMAP.md` (this Progress Log entry +
  status flips)
...
- `staffml/vault/questions/edge/cross-cutting/edge-2537.yaml` (new)
- `staffml/vault/questions/edge/optimization/edge-2536.yaml` (new)
- `staffml/vault/questions/mobile/deployment/mobile-2147.yaml` (new)
- `staffml/vault/questions/mobile/power/mobile-2146.yaml` (new)
- `staffml/vault/questions/edge/latency/edge-2535.yaml.draft` (new — failed validation, awaiting reviewer disposition)
- `staffml/vault/draft-validation-scorecard.json` (new — per-row record)
- `staffml/vault/id-registry.yaml` (4 appended entries)
- `staffml/vault-cli/docs/CHAIN_ROADMAP.md` (this entry)
...
`staffml/vault/audit-runs/20260501T213817Z/`; rollup at
`staffml/vault/audit-runs/AUDIT_REPORT.md`.
...
- `staffml/vault-cli/scripts/audit_chains_with_gemini.py` (new — `5a1cb3d3b`)
- `staffml/vault/audit-runs/AUDIT_REPORT.md` (new)
- `staffml/vault/audit-runs/20260501T213817Z/*.json` (per-call traces, 7 files)
- this Progress Log entry
...
**Changes:**
- `staffml/vault/chains.json`: 879 → 824 chains. The 55 dropped
...
- Full data: `staffml/vault/_pipeline/format-audit-full.json` (gitignored)
...
**Release audit plan (NEW — `staffml/vault-cli/docs/RELEASE_AUDIT_PLAN.md`):**
...
- `staffml/vault-cli/docs/RELEASE_AUDIT_PLAN.md` (new)
- This Progress Log entry
