...
Usage:
    ...
    python3 staffml/vault-cli/scripts/ship_d1.py

    # Dry-run: write SQL to /tmp/d1_cutover.sql, don't apply:
    python3 staffml/vault-cli/scripts/ship_d1.py --dry-run

    # Stage instead of production:
    python3 staffml/vault-cli/scripts/ship_d1.py --env staging

Requires:
    - `vault` CLI installed (`pip install -e staffml/vault-cli/`)
...
VAULT_DIR = REPO / "staffml" / "vault"
VAULT_DB = VAULT_DIR / "vault.db"
WORKER_DIR = REPO / "staffml" / "vault-worker"
D1_SCHEMA = REPO / "staffml" / "vault-cli" / "scripts" / "d1-schema.sql"
