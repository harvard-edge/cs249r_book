...
Outputs per accepted draft:
  staffml/vault/questions/<track>/<area>/<auto-id>.yaml.draft
...
Usage:
  ...
  python3 generate_question_for_gap.py --gaps-from staffml/vault/gaps.proposed.json --limit 5
...
def schema_validate(draft: dict[str, Any]) -> tuple[bool, str]:
    ...
    if Question is None:
        return False, "vault_cli not importable; install with `pip install -e staffml/vault-cli/`"
