...
ARTIFACTS = [
    Path("staffml/vault-cli/src/vault_cli/models.py"),
    Path("staffml/vault-cli/scripts/d1-schema.sql"),
    Path("staffml/vault-types/index.ts"),
]
...
def register(app: typer.Typer) -> None:
    ...
    def codegen_cmd(
        ...
        if check:
            ...
            baseline = Path("staffml/vault-cli/codegen-hashes.txt")
            ...
            if drift:
                ...
                console.print(
                    ...
                    "`staffml/vault-cli/codegen-hashes.txt` and commit.[/yellow]"
                )
                raise typer.Exit(code=ExitCode.VALIDATION_FAILURE)
