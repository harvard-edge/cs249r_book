...
def register(app: typer.Typer) -> None:
    ...
    def snapshot_cmd(
        ...
        vault_db: Path = typer.Option(Path("staffml/vault/vault.db"), "--vault-db"),
    ...
    def export_paper_cmd(
        ...
        releases_dir: Path = typer.Option(Path("staffml/vault/releases"), "--releases-dir"),
        paper_dir: Path = typer.Option(Path("staffml/paper"), "--paper-dir"),
    ) -> None:
        ...
        try:
            ...
            try:
                ...
                if not vault_dir.exists():
                    vault_dir = Path("staffml/vault/questions").resolve()
                pub = 0
                val = 0
    ...
    def tag_cmd(
        ...
        add_r = subprocess.run(
            ["git", "add", "staffml/vault/releases"],
            capture_output=True, text=True, check=False,
        )
    ...
    def deploy_cmd(
        ...
        releases_dir: Path = typer.Option(Path("staffml/vault/releases"), "--releases-dir"),
        worker_dir: Path = typer.Option(Path("staffml/vault-worker"), "--worker-dir"),
    ...
    def ship_cmd(
        ...
        worker_dir: Path = typer.Option(
            Path("staffml/vault-worker"), "--worker-dir",
            help="Path to the vault-worker directory for wrangler invocations.",
        ),
        site_dir: Path = typer.Option(
            Path("staffml/app"), "--site-dir",
    ...
    def verify_cmd(
        ...
        if from_source:
            ...
            if resolved_ref is None:
                ...
                # Historical path: `git archive <ref> -- staffml/vault | tar -x -C tmp`
                ...
                with tempfile.TemporaryDirectory(prefix="vault-verify-") as tmp_str:
                    ...
                    historical_vault = tmp / "staffml" / "vault"
