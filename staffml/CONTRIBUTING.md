...
(question corpus) and the site (`staffml/app/`).
...
vault build                                      # produces staffml/vault/vault.db
vault api --db staffml/vault/vault.db --port 8002 &
...
The corpus under `staffml/vault/` is licensed **CC-BY-NC-4.0** — see
...
The `staffml/vault-cli/` tooling is a separate artifact; its license is
