...
```bash
python3 staffml/vault/scripts/analyze_coverage_gaps.py \
...
```bash
python3 staffml/vault/scripts/iterate_coverage_loop.py \
  --max-iters 30 \
  --max-calls 80 \
  --gen-batch-size 30 \
...
```bash
python3 staffml/vault/scripts/promote_validated.py
PYTHONPATH=staffml/vault-cli/src \
