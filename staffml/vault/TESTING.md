...
### 3.6 Worker contract tests (`vault-worker/tests/`)
...
### 4.1 `.github/workflows/staffml-validate-vault.yml` (every PR touching `staffml/vault/` or `staffml/vault-cli/`)
...
jobs:
  validate:
    ...
    steps:
      ...
      - run: pip install -e staffml/vault-cli/[dev]
      - run: vault --version
      ...
      - name: Compare release_hash vs corpus-equivalence-hash.txt
        run: |
          EXPECTED=$(cat staffml/vault/corpus-equivalence-hash.txt)
      ...
      - run: |
          ...
          python staffml/vault-cli/scripts/check_registry_append_only.py

      # Unit + integration + CLI contract tests
      - run: pytest staffml/vault-cli/tests/ -x --timeout=60

      # Rollback symmetry (if this is a release commit on main)
      - name: Rollback symmetry property test
        if: github.event_name == 'push' && startsWith(github.ref, 'refs/tags/v')
        run: pytest staffml/vault-cli/tests/release/ --rollback-symmetry
```
