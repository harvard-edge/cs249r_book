...
vault/chains.json
         │
         ├─→ vault/scripts/export_to_staffml.py → app/src/data/*.json
...
│
├── vault-worker/               ← NEW Cloudflare Worker
│   ├── src/index.ts
│   ├── src/queries.ts
│   ├── wrangler.toml
│   └── migrations/
...
**2. Sidecar chain architecture.** `staffml/vault/chains.json` is the
...
**New: `staffml/app/src/lib/vault-api.ts`**
```ts
...
**Rewritten: `staffml/app/src/lib/corpus.ts`**
...
- Migrations versioned under `vault-worker/migrations/`
...
- `app/scripts/sync-vault.py`: same path. Deprecated in Phase 4 (site reads from Worker API).
- `app/src/data/corpus.json`: this is separate from `vault/corpus.json`. It's the site's bundled artifact, produced by `sync-vault.py`. Unchanged during Phases 1–3. Deleted (with retention policy, see §7.1) after Phase 4 cutover.
...
The equivalence-hash baseline at `staffml/vault/corpus-equivalence-hash.txt`
...
- CI on every PR: `vault build` produces `vault.db` + `release.json`. CI compares the **release_hash** (§3.5 Merkle) against a committed `corpus-equivalence-hash.txt` checksum that was computed from the current `app/src/data/corpus.json` at Phase 1 exit.
...
- Write `staffml/CONTRIBUTING.md` documenting clone → first-question-visible path. (Fixes H-17.)
...
- Scaffold `vault-worker/` with cursor-paginated endpoints + ETag + Cache API keyed by `release_id`.
...
- Package directory — **`staffml/vault-cli/`**.
...
- **Monorepo discipline** — this architecture touches 4 subprojects (vault, vault-cli, app, vault-worker). Document which PRs are cross-cutting.
...
Testing is NOT an afterthought. It's the gate between stages. This section is the *skeleton*; full spec lives in `staffml/vault/TESTING.md` (written before Phase 0 implementation).
...
1. On every PR touching `staffml/vault/` or `staffml/vault-cli/`:
   ...
   - Run equivalence check vs `app/src/data/corpus.json` during Phase 1–3 (§11.5).
...
2. **Working from** a dedicated StaffML worktree.
...
- Anything touches data outside `staffml/vault/`, `staffml/vault-cli/`, `staffml/app/`, `staffml/vault-worker/`, `staffml/paper/scripts/` without being pre-authorized
- The user asks
