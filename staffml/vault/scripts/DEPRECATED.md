# Legacy scripts — `staffml/vault/scripts/`
...
1. **Removed** — replaced by `vault` CLI subcommands or by
   ...
   `git log --diff-filter=D -- staffml/vault/scripts/`.
...
`git log --diff-filter=D -- staffml/vault/scripts/` if you need to
...
| `export_to_staffml.py` | Copied `corpus.json` → `app/src/data/` with field massaging | `vault build --local-json` (writes site-compatible JSON) |
...
1. **Replace corpus loading.** Most preserved scripts read
   `staffml/vault/corpus.json`. That file no longer exists in git;
   it's a build artifact. Adapt to walk `staffml/vault/questions/**/*.yaml`
...
3. **Update output paths.** Many preserved scripts wrote to
   `staffml/vault/scripts/_validation_results/<UTC>/`. The current
   convention is `staffml/vault/_pipeline/runs/<UTC>/` (see
   `staffml/vault/README.md` § "Pipeline artifacts").
