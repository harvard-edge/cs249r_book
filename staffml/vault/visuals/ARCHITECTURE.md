...
```
staffml/vault/visuals/<track>/<id>.svg     # the asset the website ships (always present)
staffml/vault/visuals/<track>/<id>.dot     # iff source_format=dot     — build input
staffml/vault/visuals/<track>/<id>.py      # iff source_format=matplotlib — build input
```
...
`staffml/vault/scripts/render_visuals.py` is the single entry point.
...
`staffml/vault/scripts/gemini_cli_generate_questions.py`:
