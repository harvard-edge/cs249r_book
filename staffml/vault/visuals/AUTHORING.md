...
2. **Save** to `staffml/vault/visuals/<track>/<id>.svg` where
...
4. **Build** — `vault build --local-json`. This copies the SVG to
   `staffml/app/public/question-visuals/<track>/` and surfaces
...
The directory `staffml/app/public/question-visuals/` is a
build output (written by `copy_visual_assets` during `vault build
--local-json`). Source files live under `staffml/vault/visuals/`.
