...
git add staffml/vault/questions/<track>/<area>/<id>.yaml
git commit
```
...
the LinkML file at `staffml/vault/schema/question_schema.yaml`; this
...
`staffml/vault/schema/enums.py:VALID_TOPICS`. The CLI tab-completes
...
- `staffml/vault/schema/question_schema.yaml` — the LinkML schema (canonical)
