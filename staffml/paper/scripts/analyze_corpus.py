...
**Preferred:** ``staffml/vault/vault.db`` — the same SQLite file that
``staffml/vault-cli/scripts/ship_d1.py`` serializes to Cloudflare D1 (``SELECT … FROM
...
(see ``staffml/vault/ARCHITECTURE.md`` §11), plus ``chains.json`` for chain metadata
not embedded in the JSON in older shapes.

Input resolution:
  1. ``--db PATH`` or ``$PAPER_VAULT_DB`` or, if present, ``staffml/vault/vault.db``
  2. Else ``--json PATH``, else the first existing of
     ``staffml/app/src/data/corpus.json`` / ``staffml/vault/corpus.json``,
     with ``../vault/chains.json`` for chain registries (JSON path only).

  For a **tagged release** matching ``export-paper``, pass
  ``--db staffml/vault/releases/<v>/vault.db`` so figures match that DB exactly.
...
STAFFML_DIR = PAPER_DIR.parent
VAULT_DIR = STAFFML_DIR / "vault"
# Generated by `vault build --local-json` (not hand-edited)
STAFFML_CORPUS_JSON = STAFFML_DIR / "app" / "src" / "data" / "corpus.json"
