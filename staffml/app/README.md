...
YAML files in `staffml/vault/questions/` so contributors can edit a
...
`staffml/vault/questions/`. Restart `npm run dev` (or just `Ctrl+C`
...
1. **`scripts/build-local-corpus.mjs`** — runs `vault build --local`
   ...
   - emits `staffml/app/src/data/corpus.json` (legacy bundle
     used by some build tooling),
   - mirrors that into `staffml/app/public/data/corpus.json`
   ...
   - copies any `vault/visuals/*.svg` into
     `staffml/app/public/question-visuals/`.
...
1. **`vault` CLI not installed.** `which vault` → empty? Run
   `pip install -e staffml/vault-cli/` from the repo root.
...
- **Worker source of truth:** `staffml/app/worker/`. Deploy
