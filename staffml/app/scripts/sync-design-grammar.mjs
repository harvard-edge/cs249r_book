#!/usr/bin/env node
/**
 * Sync the design-grammar YAML -> staffml/app/src/data/designGrammar.ts
 *
 ...
 *
 * Usage: npm run sync:design-grammar   (from staffml/app)
 *        node staffml/app/scripts/sync-design-grammar.mjs   (from anywhere)
 */
...
const TS_PATH = path.join(REPO, "staffml", "app", "src", "data", "designGrammar.ts");
...
function renderTs(doc, parsedAssemblies) {
  ...
  out += "// Regenerate: npm run sync:design-grammar   (from staffml/app)\n";
  out += "//\n";
