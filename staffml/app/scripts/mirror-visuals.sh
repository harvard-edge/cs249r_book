...
# Runs without the vault CLI (pip install -e staffml/vault-cli).
#
# Usage:
#   staffml/app/scripts/mirror-visuals.sh
#
...
VAULT_VISUALS="$(cd "$STAFFML_DIR/../../staffml/vault/visuals" 2>/dev/null && pwd)" || {
  echo "[mirror-visuals] ERROR: Cannot find staffml/vault/visuals/ relative to this script." >&2
  exit 1
}
