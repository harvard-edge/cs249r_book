 ...
 *        staffml/app/src/data/corpus.json   (legacy bundle)
 *        staffml/app/public/data/corpus.json (the path the loader fetches)
 *      and mirrors visual SVGs into public/question-visuals/.
 *
 * Skipped silently if `vault` is not installed (e.g. on a fresh checkout
 * that hasn't run `pip install -e staffml/vault-cli` yet). The dev
