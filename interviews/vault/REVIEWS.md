...
| H-6 | XSS surface: scenario/solution/resources[].url rendered client-side with unspecified format | C | Define per-field content format (plaintext / restricted Markdown / KaTeX). Validator rejects raw HTML, `javascript:`/`data:` URLs. `resources[].url` allowlist: `https:` only. Sanitize at render (DOMPurify). Next.js CSP forbids inline scripts | INTEGRATE §3.3, §5, §7 |
