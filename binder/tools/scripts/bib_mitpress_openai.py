...
in ``binder/cli/checks/bib_lint.py`` and adds a small MIT Press production layer for
...
REPO_ROOT = Path(__file__).resolve().parents[3]  # script lives at binder/tools/scripts/<this>.py
BOOK_DIR = REPO_ROOT / "binder"
if str(BOOK_DIR) not in sys.path:
    sys.path.insert(0, str(BOOK_DIR))

DEFAULT_MODEL = "gpt-5.4-mini"
DEFAULT_REPORT_DIR = REPO_ROOT / "binder" / "tools" / "audit" / "out"
...
def main() -> int:
    ...
    parser.add_argument("--json", action="store_true", help="Write a JSON report under binder/tools/audit/out")
