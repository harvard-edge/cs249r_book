...
plus the Book Tools modules under `binder/tools/figures/`. Do not delete, move, or
...
`binder/tools/figures/style.py`, and stable margin drawing devices live in
`binder/tools/figures/margin/devices.py`. This directory retains script
...
4. If the same label-placement or geometry issue affects a whole device family,
   change `binder/tools/figures/margin/devices.py` instead.
...
```bash
python3 binder/tools/scripts/margin_figures/render_margin_reader_alignment_html.py
```
