...
Usage:
    from binder.tools.figures.margin.devices import new_fig, ladder, knee, sparkline, roofline, \
...
from binder.tools.figures import style as figure_style
...
MECHANICAL = "#D97706"  # amber/ochre — contact forces (N), torque (Nm), joint acceleration, gear ratios
...
#   newtons / N* (mech)  → MECHANICAL   : forces, torques, gear ratios
...
DOMAIN_COLOR = {
    ...
    'mechanical': MECHANICAL, 'force': MECHANICAL, 'torque': MECHANICAL,
    'electrical': COMP, 'voltage': COMP, 'current': COMP,
    'certified': DATA, 'safety': DATA,
}
...
FIGSIZE = {
    ...
    'budget-envelope': (1.32, 0.75), 'sequence-strip': (1.32, 0.75),
    'causal-chain': (1.35, 0.80), 'all-to-all-topology': (1.15, 0.98),
    'other-new': (1.1, 1.0),
}
        ...
        if bar_w > 0.19:
            if value > limit:
                ...
                if over_w > 0.18:
                    ...
                    value_x = limit_x - 0.03
                    ha_val = "right"
            else:
                value_x = x0 + bar_w / 2
                ha_val = "center"
            ax.text(value_x, y, f"{value:g}", ha=ha_val, va="center",
...
def sequence_strip(ax, steps=(("1", GRID), ("2", GRID), ("3", GRID)), bracket=None,
    ...
    n = len(steps)
    x0, w, gap, y, h = 0.08, 0.84, 0.015, 0.38, 0.20
    cell = (w - gap * (n - 1)) / n
...
def causal_chain(ax, labels=("cause", "effect"), style='chain', colors=None):
    ...
    for i, (x, label) in enumerate(zip(xs, labels)):
        ...
        ax.plot(x, 0.62, "o", color=c, ms=8)
        ax.text(x, 0.26, label, ha="center", va="center", color=INK,
                fontsize=4.1, multialignment="center")
        if i < len(labels) - 1:
            ax.annotate("", xy=(xs[i + 1] - 0.055, 0.62), xytext=(x + 0.055, 0.62),
