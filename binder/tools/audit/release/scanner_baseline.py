...
1. ``binder/tools/audit/scan.py --scope vol1|vol2`` ledger counts
2. The 6 detector self-tests (123/123 expected)
3. ``./binder/binder check bib --json`` (curated bibliography publication gate)
...
def main() -> None:
    ...
    for scope in ("vol1", "vol2"):
        ...
        rc, out = run(
            ["python3", "binder/tools/audit/scan.py", "--scope", scope, "-v",
    ...
    for module in EXPECTED_SELF_TESTS:
        ...
        rc, out = run(
            ["python3", f"binder/tools/audit/checks/{module}.py"],
            log,
            env={"PYTHONPATH": str(REPO / "binder/tools")},
        )
    ...
    rc, out = run(["python3", "binder/binder", "check", "bib", "--json"], log)
    try:
