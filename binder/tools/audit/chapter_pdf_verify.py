...
Usage (repo root)::
    ...
    python3 binder/tools/audit/chapter_pdf_verify.py --list
    python3 binder/tools/audit/chapter_pdf_verify.py --vol1 introduction
    python3 binder/tools/audit/chapter_pdf_verify.py --vol1 --all
    python3 binder/tools/audit/chapter_pdf_verify.py --report

Ledger: binder/tools/audit/artifacts/chapter_pdf_audit.json
Table:   binder/tools/audit/artifacts/chapter_pdf_audit.md

Builds one chapter at a time via ``./binder/binder build pdf --volN <chapter>``.
...
LEDGER_JSON = REPO_ROOT / "binder/tools/audit/artifacts/chapter_pdf_audit.json"
LEDGER_MD = REPO_ROOT / "binder/tools/audit/artifacts/chapter_pdf_audit.md"
...
def _build_pdf(vol: str, ch_path: str) -> tuple[bool, float, str]:
    ...
    t0 = time.monotonic()
    proc = subprocess.run(
        ["./binder/binder", "build", "pdf", f"--{vol}", name],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
    ...
    try:
        proc = subprocess.run(
            [sys.executable, "-W", "ignore::UserWarning",
             str(REPO_ROOT / "binder/tools/audit/fmt/audit_prose.py"), str(qmd)],
            cwd=REPO_ROOT,
            env=env,
            capture_output=True,
            text=True,
...
def _write_markdown_table(ledger: dict) -> None:
    ...
    lines.extend([
        ...
        "",
        "Re-run one chapter: `python3 binder/tools/audit/chapter_pdf_verify.py --vol1 training`",
        "",
