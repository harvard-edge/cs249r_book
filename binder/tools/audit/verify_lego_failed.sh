...
#
# Usage (repo root):
#   ./binder/tools/audit/verify_lego_failed.sh
#   ./binder/tools/audit/verify_lego_failed.sh binder/tools/audit/artifacts/lego_chapter_failures.txt
set -euo pipefail
...
while IFS= read -r line; do
  ...
  ch="${line#*/}"
  echo "=== retry $vol/$ch ==="
  if ./binder/tools/audit/verify_lego_chapter.sh "$vol" "$ch"; then
    echo "OK $line"
  else
