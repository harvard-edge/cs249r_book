# Fmt / notation audit (`binder/tools/audit/fmt`)
...
# or: ./binder/binder check math --scope canonical --path "$CH"

# 5. HTML render + final scan
./binder/binder build html --vol1 vol1/training --skip-hygiene --skip-validate
python3 binder/tools/audit/fmt/audit_html.py \
...
./binder/tools/audit/fmt/render_html.sh vol2   # extend chapter list in script
```
...
- **Build:** `./binder/binder` from repo root
