...
AUDIT_PY="binder/tools/audit/fmt/audit_html.py"
...
for ch in "${CHAPTERS[@]}"; do
  ...
  binder_ch="${VOL}/${name}"
  printf "%-28s " "$name"
  if ! ./binder/binder build html "${BINDER_VOL[@]}" "$binder_ch" --skip-hygiene --skip-validate >/tmp/render_${VOL}_${name}.log 2>&1; then
