...
|-----|------|---------|------|
| **L0** | static gates | `./binder/binder check math --scope canonical` · `--scope suffix-consistency` · `--scope suffix-semantics` · `--scope multiplier-style` ; `fmt_prose_contract.py` ; `pytest test_fmt_prose_contract.py` | after every edit batch |
...
| **L2** | spot HTML render | `./binder/binder build html <2–3 rep chapters>` then scan rendered HTML (below) | end of each phase |
| **L3** | FULL HTML render (ultimate) | `./binder/binder build html --all` + `audit_lego_html.py` + scans | end of each migration |
| **L4** | FULL PDF (final gatekeeper) | `./binder/binder build pdf --all` (runs pdftotext cross-ref scan) | once, after both migrations |
...
python3 binder/tools/audit/fmt/audit_lego_html.py --report /tmp/lego_html.json
```
...
```bash
python3 binder/tools/audit/fmt/inventory_design_b_rates.py \
  --root books \
  --json binder/tools/audit/artifacts/fmt_design_b_inventory.json
```
...
- **L2 spot render:** `./binder/binder build html training network_fabrics` → run the
...
- **L2 spot render:** `./binder/binder build html network_fabrics compute_infrastructure` →
...
```bash
./binder/binder build html --all
python3 binder/tools/audit/fmt/audit_lego_html.py --report /tmp/lego_html_all.json
...
```bash
./binder/binder build pdf --all        # runs pdftotext cross-ref scan after render
```
