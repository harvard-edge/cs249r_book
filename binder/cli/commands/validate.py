...
Every `book-*` pre-commit hook dispatches through `./binder/binder check <group>`
...
Some checks still delegate to scripts under binder/tools/scripts/ (tables,
spelling, epub, sources, grid-tables, images) and to standalone audit
scripts under binder/tools/audit/index/. Prefer new logic in cli/checks/
with normal imports; see binder/cli/README.md "Check implementation layout".
...
#   ./binder/binder check tables --scope caption-required --update-baseline
...
class ValidateCommand:
    ...
    No new pre-commit hook needed. Pre-commit runs `./binder/binder check <group>`
    ...
    GROUPS: Dict[str, List[Scope]] = {
        "cli": [
            ...
            Scope("binder-canonical", "_run_binder_canonical",
                  note="book-content pre-commit hooks must dispatch through ./binder/binder",
                  default=True),
        ],
        "refs": [
        ...
        "index": [
            ...
            # Migrated 2026-05-06: was binder/tools/audit/index/check_anti_patterns.py
            Scope("anti-patterns", "_run_index_anti_patterns",
                  note="anti-patterns from the project index rules §9"),
            # Migrated 2026-05-06: was binder/tools/audit/index/check_tag_placement.py
            Scope("tag-placement", "_run_index_tag_placement",
                  note='\\index{} not inside **bold**, *italic*, `code`, or headings'),
            # Migrated 2026-05-06: was binder/tools/audit/index/check_xref_resolves.py
    ...
    def run(self, args: List[str]) -> bool:
        ...
        # Per-group help: `./binder/binder check <group> help` prints a
    ...
    def _print_check_help(self) -> None:
        ...
        console.print("  [cyan]./binder/binder check refs[/cyan]                [dim]# curated default scopes for refs[/dim]")
        console.print("  [cyan]./binder/binder check refs --scope citations[/cyan]  [dim]# one specific scope[/dim]")
        console.print("  [cyan]./binder/binder check refs --all-scopes[/cyan]   [dim]# every scope including opt-in (* in table)[/dim]")
        console.print("  [cyan]./binder/binder check figures --vol1[/cyan]      [dim]# default figure scopes, Vol I only[/dim]")
        console.print("  [cyan]./binder/binder check all[/cyan]                 [dim]# every group, default scopes[/dim]")
        console.print("  [cyan]./binder/binder check <group> help[/cyan]        [dim]# per-group error codes + guidance[/dim]")
    ...
    def _print_group_help(self, group: str) -> None:
        ...
        console.print(f"[dim]Run [cyan]./binder/binder check {group}[/cyan] for the curated set "
    ...
    def _print_epub_help(self) -> None:
        ...
        console.print(Panel(scope_table, title="./binder/binder check epub — scopes", border_style="cyan"))
        ...
        console.print("  [cyan]./binder/binder check epub[/cyan]                        [dim]# hygiene + smoke + epubcheck[/dim]")
        console.print("  [cyan]./binder/binder check epub --scope hygiene[/cyan]        [dim]# source-only, fast[/dim]")
        console.print("  [cyan]./binder/binder check epub --scope hygiene --fix[/cyan]  [dim]# auto-repair source issues[/dim]")
        console.print("  [cyan]./binder/binder check epub --scope epubcheck --max-fatal 0 --max-errors 0[/cyan]")
        console.print("  [dim]                                                    # CI-style strict gating[/dim]")
        console.print("  [cyan]./binder/binder check epub --json[/cyan]                 [dim]# structured output[/dim]")
        console.print("  [cyan]./binder/binder build epub --vol1 --skip-hygiene[/cyan]  [dim]# emergency build bypass[/dim]")
        console.print()
        console.print("[dim]Deep dive: binder/cli/README.md → \"EPUB Checks — Two Layers, One CLI Surface\"[/dim]")
        console.print()
    ...
    def _run_binder_canonical(self, root: Path) -> ValidationRunResult:
        """Assert book-content pre-commit hooks dispatch through ./binder/binder."""
        start = time.time()
    ...
    def _run_inline_refs(self, root: Path, check_patterns: bool,
        ...
        for file in files:
            ...
            if check_scope:
                from binder.tools.scripts.maintenance.validate_inline_refs import check_scope as _check_scope, BOOK_ROOT
                try:
    ...
    # at binder/tools/audit/baselines/captions_baseline.json grandfathers
    ...
    def _run_inline_ref_order(self, root: Path) -> ValidationRunResult:
        ...
        try:
            ...
            from binder.cli.checks.inline_ref_order import check_inline_order
    ...
    def _run_concept_maps(self, root: Path) -> ValidationRunResult:
        ...
        try:
            ...
            from binder.cli.checks.concept_maps import check_concept_maps
    ...
    def _run_mitpress_percent_in_tables(self, root: Path) -> ValidationRunResult:
        ...
        Auto-fixable: ``./binder/binder format percent-tables``.
        """
                ...
                issues.append(
                    ValidationIssue(
                        ...
                        message=(
                            ...
                            "'./binder/binder format percent-tables' to auto-fix."
                        ),
                        severity="error",
                        context=hit.context,
    ...
    def _run_mitpress_spelling_dict(self, root: Path) -> ValidationRunResult:
        ...
        scope. Auto-fix: ``./binder/binder format mitpress-terms``.
        """
                ...
                issues.append(
                    ValidationIssue(
                        ...
                        message=(
                            ...
                            "'./binder/binder format mitpress-terms'."
                        ),
                        severity="error",
                        context=hit.context,
                        suggestion=f"{hit.match} → {hit.replacement}",
    ...
    # binder/tools/scripts/mit_press/footnote_caps_allowlist.txt.
    ...
    def _run_footnote_capitalization(self, root: Path) -> ValidationRunResult:
        ...
        try:
            ...
            from binder.cli.checks.footnote_caps import DEFAULT_ALLOWLIST, load_allowlist, scan_file
    ...
    def _run_footnote_slug_taxonomy(self, root: Path) -> ValidationRunResult:
        ...
        try:
            ...
            from binder.cli.checks.footnote_caps import DEFAULT_ALLOWLIST, load_allowlist
    ...
    def _run_table_content(self, root: Path) -> ValidationRunResult:
        ...
        try:
            ...
            from binder.cli.checks.table_content import validate_file
    ...
    def _run_spelling_prose(self, root: Path) -> ValidationRunResult:
        ...
        try:
            ...
            from binder.cli.checks.spelling_prose import check_file
    ...
    def _run_spelling_tikz(self, root: Path) -> ValidationRunResult:
        ...
        try:
            ...
            from binder.cli.checks.spelling_tikz import check_file
    ...
    # the EPUB build pipeline. See binder/cli/commands/_epub_checks.py for
        ...
        if update_baseline:
            payload = {
                ...
                "description": (
                    ...
                    "`./binder/binder check epub --scope epubcheck --baseline <this>` "
        ...
        if not baseline_file.exists():
            return [ValidationIssue(
                ...
                message=(
                    ...
                    "with `./binder/binder check epub --scope epubcheck "
    ...
    def _run_grid_tables(self, root: Path) -> ValidationRunResult:
        ...
        try:
            ...
            from binder.cli.checks.grid_tables import check_grid_tables
    ...
    def _run_bib_hygiene(self, root: Path) -> ValidationRunResult:
        ...
        only NEW errors (not in binder/tools/bib_lint_baseline.json) block;
                ...
                for v in bib_lint.validate_entry(entry):
                    ...
                    issues.append(ValidationIssue(
                        ...
                        message=(
                            ...
                            "binder/tools/bib_lint_style_baseline.json; new issues must be fixed "
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Native audit-check adapters (binder/tools/audit/checks/*).
    #
    ...
    def _run_audit_check(
        ...
        The audit package lives at `binder/tools/audit/`. It uses relative
        imports against its own `audit.*` top-level package, so the parent
        directory (`binder/tools`) must be on sys.path for the import to
        resolve. This method does the path-prepend once per call.
        """
        import importlib
        import sys as _sys
        # Prefer native binder/cli/audit, fallback to binder/tools
    ...
    def _run_mitpress_abbreviation_first_use(self, root: Path) -> ValidationRunResult:
        ...
        `binder/cli/data/abbreviations.yaml`. Terms under
    ...
    def _run_notation_consistency(self, root: Path) -> ValidationRunResult:
        ...
        accept-list at binder/tools/audit/accepted_fps_notation.json is
        ...
        subprocess. Replaces the standalone binder/tools/audit/run_notation_consistency.py.
        """
        import importlib
        import sys as _sys
    ...
    def _run_external_images(self, root: Path) -> ValidationRunResult:
        ...
        try:
            ...
            from binder.cli.checks.external_images import ImageDownloader
    ...
    def _run_unit_tests(self, root: Path) -> ValidationRunResult:
        ...
        Native import of binder/tests/test_units.py via importlib — no
    ...
    def _run_index_placement_contexts(self, root: Path) -> ValidationRunResult:
        ...
        Wraps the native audit check at binder/tools/audit/checks/index_placement.py.
        """
        return self._run_audit_check(
    ...
    def _run_lego_units(self, root: Path) -> ValidationRunResult:
        ...
        qmd_files = self._qmd_files(root)
        baseline = repo / "binder" / "tools" / "audit" / "lego_units_baseline.json"
