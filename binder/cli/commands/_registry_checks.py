...
def check_registry_sources(root: Path, paths: list[Path] | None = None) -> list[RegistryIssue]:
    ...
    mod = _load_script_module("book_check_registry_sources", root / "binder" / "tools" / "audit" / "book_check_registry_sources.py")
...
def run_registry_pytest(root: Path) -> list[RegistryIssue]:
    ...
    tests = [
        ...
        root / "binder" / "tests" / "test_appendix_constants.py",
        root / "binder" / "tests" / "test_appendix_lineage.py",
        root / "binder" / "tests" / "test_no_legacy_constant_refs.py",
        root / "binder" / "tests" / "test_mlsysim_registry_coverage.py",
    ]
    proc = subprocess.run(
...
def verify_appendix_lego(root: Path) -> list[RegistryIssue]:
    ...
    mod = _load_script_module(
        ...
        root / "binder" / "tools" / "audit" / "generate_appendix_constants.py",
    )
...
def check_yaml_pending(root: Path) -> list[RegistryIssue]:
    ...
    return [RegistryIssue(
        ...
        message=(
            ...
            "run: python3 binder/tools/audit/refresh_mlsysim_constants_yamls.py --finalize"
        ),
    )]
