...
class LayoutCommand:
    ...
    def _tables(
        ...
        out_dir = repo_root / "binder" / ".layout" / "tables" / volume
    ...
    def _auto_layout(
        ...
        if not pdf_path.exists():
            console.print(
                ...
                f"[yellow]Run `./binder/binder build pdf --{volume}` or omit "
                "`--no-build`.[/yellow]"
            )
            return False
    ...
    def _default_volume_pdf(self, volume: str) -> Path:
        ...
        try:
            ...
            from binder.cli.commands._pdf_checks import default_pdf_path
