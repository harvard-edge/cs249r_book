...
class DoctorCommand:
    ...
    def _check_configuration_files(self) -> None:
        ...
        # Check the generated active config
        source = self.config_manager.active_config_source()
        if source:
            self.checks.append({
                "category": "Configuration",
                "name": "Active Config",
            ...
            self.checks.append({
                ...
                "name": "Active Config",
                "passed": False,
                "message": "⚠️ Not generated by binder",
                "details": "Run ./binder/binder switch <format> to regenerate _quarto.yml"
            })
        else:
            self.checks.append({
                "category": "Configuration",
    ...
    def _check_epub_health(self) -> None:
        ...
        Emits four checks under the "EPUB" category:
          ...
          3. Built EPUB artifacts — whether there is something for
             `./binder/binder check epub --scope epubcheck` to validate.
        ...
        if _discover_built_epubs is not None:
            ...
            if epubs:
                ...
                self.checks.append({
                    ...
                    "message": "📁 No built EPUBs yet — run `./binder/binder build epub --vol1`",
                    "details": None,
                })
        else:
            self.checks.append({
        ...
        if not issues:
            ...
            self.checks.append({
                ...
                "details": (
                    f"{summary}. Fix with "
                    "`./binder/binder check epub --scope hygiene --fix`."
                ),
            })
    ...
    def _check_permissions(self) -> None:
        ...
        key_files = [
            ("binder", self.config_manager.root_dir / "binder" / "binder"),
        ]
