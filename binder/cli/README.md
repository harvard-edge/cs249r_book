...
The `./binder/binder` CLI is the **single source of truth** for building, checking, fixing, and formatting MLSysBook. Every `book-check-*` pre-commit hook dispatches through `./binder/binder check <group>`; CI uses the same commands.

**User-facing command reference:** [`binder/docs/BINDER.md`](../docs/BINDER.md) (check/fix tables, pre-commit mapping, examples).
...
**Goal:** every `./binder/binder check …` scope is backed by code under `binder/cli/`,
not by loading arbitrary scripts from `binder/tools/` at runtime.
...
`binder/tools/scripts/*.py` or `binder/tools/bib_lint.py`. Older files may remain
...
2. Pre-commit and CI integrations should call `./binder/binder ...` for
   book workflows. Do not add new hooks that invoke `python3 binder/tools/...`
   directly.
...
4. New checks start in `binder/cli/checks/` unless they are tiny enough to keep as
   an inline `_run_*` in `validate.py`.

Do **not** add new pre-commit hooks that call `python3 binder/tools/...` directly.
...
```bash
./binder/binder layout --vol1                     # Build PDF, then emit one layout plan
./binder/binder layout --vol1 --no-build          # Reuse existing PDF for the plan
./binder/binder layout --vol1 --no-build --json /tmp/layout-plan.json
./binder/binder layout check <pdf> --csv          # Main-flow whitespace scanner only
./binder/binder layout margins <pdf> --csv        # Native margin-geometry scanner only
```
...
```bash
./binder/binder check <group>                  # default=True scopes only
./binder/binder check <group> --scope <name>   # one specific scope
./binder/binder check <group> --all-scopes     # every scope, including default=False
./binder/binder check all                      # every group's default=True scopes
./binder/binder check all --vol1               # ... scoped to Volume I
./binder/binder check <group> help             # per-group scope listing
./binder/binder check cli                      # read-only Binder command contract
```

#### How scopes are discovered

`./binder/binder check` (no args) prints the full group catalogue. Scopes marked with `*` are `default=False` — opt-in only, reachable via `--scope` or `--all-scopes`. Per-group help (`./binder/binder check <group> help`) prints the same catalogue with one row per scope, plus the runner method name and a one-line note.

The authoritative list is the `GROUPS` dict in `binder/cli/commands/validate.py`. That dict is the only place a new scope needs to be wired; pre-commit, CI, and ad-hoc invocations all read from it.

#### Bibliography checks

Bibliography publication gates belong under `./binder/binder check bib`, not under
...
source. `hygiene` and `style` use reviewed baselines under `binder/tools/` so
known debt is documented but new debt fails. Use `./binder/binder check bib --json`
...
```bash
./binder/binder format blanks             # Collapse extra blank lines
./binder/binder format python             # Format Python blocks (Black, 70 chars)
./binder/binder format lists              # Fix list spacing
./binder/binder format divs               # Fix div/callout spacing
./binder/binder format tables             # Format grid tables
./binder/binder format prettify           # Align pipe table columns
./binder/binder format all                # Run all formatters
```

### Other Commands

```bash
./binder/binder info stats --vol1         # Word counts, figure counts
./binder/binder info figures --vol1       # Extract figure list
./binder/binder info concepts --vol1      # Extract concept list
./binder/binder bib sync --vol1           # Sync bibliography
...
- **CLI contract hook** (`book-check-cli-contract`) — always runs `./binder/binder check cli` because it guards help text, removed command aliases, and reset/build command shape. It is read-only and does not inspect staged files.
...
1. **Implement the runner** in `binder/cli/commands/validate.py` as `_run_<scope>(self, root: Path) -> ValidationRunResult`. For per-file regex checks, walk `self._qmd_files(root)`. For graph / corpus checks, build the model once and emit `ValidationIssue` entries. If the logic is reusable or more than a few lines, put it in `binder/cli/checks/` and have the runner adapt those structured results to Binder's `ValidationIssue` format.
...
3. **Document examples in the code and CLI docs**: every new check module should include bad/good source examples in its module docstring, one example per error code. User-facing scopes should also add the same examples to [`binder/docs/BINDER.md`](../docs/BINDER.md) or the relevant Binder doc page. The goal is that a maintainer, author, or automated repair pass can open either the checker or CLI docs and immediately see what mistake it is looking for and what the canonical fix looks like.
...
|------------|------------------------------|--------------|
| `cli_contract_exit` | `./binder/binder pdf reset --vol1` exits `0`, or help paths exit `1` | Preserve the public CLI contract: help returns `0`; removed commands and parse errors return `1`. |
| `cli_contract_missing_output` | `./binder/binder check` no longer lists `cli` / `contract` | Update help text and docs when the command tree intentionally changes. |
| `cli_contract_unexpected_output` | `./binder/binder build --help` still mentions `build reset` | Remove stale help; YAML reset is `./binder/binder reset <fmt\|all>`. |
...
```
./binder/binder check epub                       # all scopes (default)
./binder/binder check epub --scope hygiene       # layer 1 — source-level, pre-commit grade
./binder/binder check epub --scope epubcheck     # layer 2 — post-build W3C validator
./binder/binder check epub --scope epubcheck \
    --max-fatal 0 --max-errors 0          # tighten thresholds (CI-style)
./binder/binder check epub --json                # machine-readable output (tools, CI)
```

Implementation: `binder/cli/commands/validate.py` (`_run_epub_hygiene`, `_run_epubcheck` native methods) and the shared primitives in `binder/cli/commands/_epub_checks.py`.
...
Requires the `epubcheck` Python package (pinned in `binder/tools/dependencies/requirements.txt`) plus a Java runtime (JRE 8+). If neither `python -m epubcheck` nor the `epubcheck` system binary is available, the check emits an `epubcheck-missing` issue with install instructions rather than silently passing.
...
The canonical baseline lives at `binder/tools/audit/epubcheck-baseline.json`. The CI workflow (`book-validate-dev.yml`) uses the ratchet by default — a PR cannot land if epubcheck counts regress past that file. Initial state (April 2026) is `0/0/0` for both volumes, so *any* new FATAL, ERROR, or WARNING blocks the PR.
...
2. **Commit.** The `book-epub-hygiene` pre-commit hook runs `./binder/binder check epub --scope hygiene` if you touched SVG, BibTeX, or the check implementation. Fails in <1s if an SVG has duplicate `<marker>` ids or a bib URL has `\_` escapes. The failure message tells you to auto-repair with `./binder/binder check epub --scope hygiene --fix`.
3. **Build locally.** `./binder/binder build epub --vol1` runs:
...
4. **Push.** CI builds both volumes and re-runs `./binder/binder check epub --scope epubcheck --baseline …`. Any regression past the recorded counts blocks the PR.

### Release workflow (preparing an EPUB for readers)

1. **Build:** `./binder/binder build epub --all` (or per-volume). Post-flight already validates — if the final line is `✓ smoke` and `✓ epubcheck`, the file is ready.
...
- **Real regression:** fix the underlying issue. `./binder/binder check epub help` maps every error code to a source-level fix. `./binder/binder check epub --scope hygiene --fix` auto-repairs the four mechanical classes.
- **Accepted increase** (e.g., a Quarto upgrade introduced a new warning class you're OK living with): `./binder/binder check epub --scope epubcheck --baseline binder/tools/audit/epubcheck-baseline.json --update-baseline` — commit the updated JSON in the same PR so the audit trail is clear.

### Escape hatches

Rare but real cases where you need to build anyway:

- `./binder/binder build epub --vol1 --skip-hygiene` — bypass pre-render hygiene (e.g., debugging a source-level issue that's triggering the check you're trying to investigate).
- `./binder/binder build epub --vol1 --skip-validate` — bypass post-render validation (e.g., iterating on a known-broken build; no Java locally).

Both are loud about being bypassed — the skipped step prints a yellow warning so the bypass is never silent.

### Onboarding

A new contributor runs `./binder/binder doctor` once. The EPUB section reports Java availability, epubcheck availability, existing EPUB artifacts, and source hygiene in one table, with install commands inline if anything is missing. No separate EPUB setup document to hunt for.

## Script Delegation

Some commands still delegate to scripts under `binder/tools/scripts/` (spelling, image formats, table formatting, Python formatting). These will be migrated to native CLI modules over time. EPUB checks have already been migrated: the `hygiene` and `epubcheck` scopes live in `binder/cli/commands/_epub_checks.py` as pure Python, not subprocess-to-script.
