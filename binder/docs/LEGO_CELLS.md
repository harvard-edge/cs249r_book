...
- **Legacy constant output names** — do not mirror removed `constants.py`
  ...
  with registry paths on the RHS. `./binder/binder check registry --scope sources` enforces this.
- **Hardcoded walkthrough operands** — in callout **Problem** / **Setup** / **Step** prose that
  ...
  `./binder/binder check code --scope lego-prose-literals` flags common cases.
...
4. Re-run `fmt/audit_prose.py` and `./binder/binder check math --scope canonical` on that chapter.
...
- `binder/tools/audit/fmt/README.md` — spurious `.0` / fmt precision workflow
- `./binder/binder check math --scope canonical` — static fmt and suffix lint
...
`binder/tools/audit/artifacts/lego_audit_ranked_chapters.md`.
...
python3 binder/tools/audit/fmt/fmt_prose_contract.py "$CH"
python3 binder/tools/audit/audit_math_canonical.py "$CH"
./binder/binder check math --scope canonical --path "$CH"
python3 binder/tools/scripts/maintenance/validate_inline_refs.py --path "$CH"
```
