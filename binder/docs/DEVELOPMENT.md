...
./binder/binder doctor     # Health check & diagnostics
./binder/binder preview    # Live preview with hot reload
...
```bash
# First time setup
./binder/binder setup              # Configure environment and tools

# Daily workflow (most common commands)
./binder/binder clean              # Clean build artifacts
./binder/binder build              # Build HTML (complete book)
./binder/binder doctor             # Health check

# Preview & development
./binder/binder preview intro      # Preview a chapter with live reload
./binder/binder build intro        # Build specific chapter
```
...
./binder/binder build html            # Build HTML version
./binder/binder build pdf             # Build PDF version
./binder/binder publish               # Build and publish

# Using binder (recommended)
./binder/binder build                 # HTML version
./binder/binder build pdf             # PDF version
./binder/binder build epub            # EPUB version
```
...
The `./binder/binder setup` command provides a complete environment configuration:
...
The `./binder/binder publish` command provides a complete publishing workflow:
...
```bash
./binder/binder doctor     # Overall project health
./binder/binder status     # Detailed project status
...
```bash
./binder/binder doctor     # Run comprehensive health check
...
./binder/binder --help     # Detailed help
```
...
./binder/binder clean      # Clean workspace
./binder/binder doctor     # Verify health
```
...
```bash
./binder/binder clean      # Full cleanup
./binder/binder build      # Clean build
./binder/binder doctor     # Run all checks
```

### Release Preparation

```bash
./binder/binder doctor     # Comprehensive validation
./binder/binder build      # Build HTML
./binder/binder build pdf  # Build PDF
