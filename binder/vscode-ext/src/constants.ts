...
export const PUBLISH_ACTIONS: ActionDef[] = [
  { id: 'mit-press-vol1', label: 'MIT Press Vol1', command: 'bash binder/tools/scripts/publish/mit-press-release.sh --vol1', icon: 'rocket' },
  { id: 'mit-press-vol2', label: 'MIT Press Vol2', command: 'bash binder/tools/scripts/publish/mit-press-release.sh --vol2', icon: 'rocket' },
  { id: 'mit-press-vol1-copyedit', label: 'MIT Press Vol1 (Copy-edit)', command: 'bash binder/tools/scripts/publish/mit-press-release.sh --vol1 --copyedit', icon: 'rocket' },
  { id: 'extract-figures-vol1', label: 'Extract Figures Vol1', command: './binder/binder info figures --vol1 --format markdown', icon: 'file-media' },
  { id: 'extract-figures-vol2', label: 'Extract Figures Vol2', command: './binder/binder info figures --vol2 --format markdown', icon: 'file-media' },
];
...
export const MAINTENANCE_ACTIONS: ActionDef[] = [
  { id: 'clean', label: 'Clean Build Artifacts', command: './binder/binder clean', icon: 'trash' },
  { id: 'doctor', label: 'Doctor (Health Check)', command: './binder/binder doctor', icon: 'heart' },
  { id: 'glossary', label: 'Show Glossary Paths', command: './binder/binder fix glossary paths', icon: 'book' },
  { id: 'compress-images', label: 'Compress Images (Dry Run)', command: './binder/binder fix images compress --all --smart-compression', icon: 'file-media' },
  { id: 'repo-health', label: 'Repo Health Check', command: './binder/binder fix repo-health', icon: 'pulse' },
  { id: 'bib-list', label: 'Bib: List All', command: './binder/binder bib list', icon: 'list-unordered' },
  { id: 'bib-clean-dry', label: 'Bib: Clean (Dry Run)', command: './binder/binder bib clean --dry-run', icon: 'eye' },
  { id: 'bib-sync', label: 'Bib: Sync (Clean + Update)', command: './binder/binder bib sync', icon: 'sync' },
];
...
export const CHECK_ACTIONS: ActionDef[] = [
  { id: 'check-all', label: 'Check: All', command: './binder/binder check all', icon: 'shield' },
  { id: 'check-refs', label: 'Check: References', command: './binder/binder check refs', icon: 'link' },
  { id: 'check-labels', label: 'Check: Labels', command: './binder/binder check labels', icon: 'search' },
  { id: 'check-headers', label: 'Check: Headers', command: './binder/binder check headers', icon: 'symbol-structure' },
  { id: 'check-footnotes', label: 'Check: Footnotes', command: './binder/binder check footnotes', icon: 'note' },
  { id: 'check-figures', label: 'Check: Figures', command: './binder/binder check figures', icon: 'file-media' },
  { id: 'check-markup', label: 'Check: Markup', command: './binder/binder check markup', icon: 'warning' },
  { id: 'check-code', label: 'Check: Code', command: './binder/binder check code', icon: 'code' },
  { id: 'check-prose', label: 'Check: Prose', command: './binder/binder check prose', icon: 'whole-word' },
  { id: 'check-images', label: 'Check: Images', command: './binder/binder check images', icon: 'device-camera' },
  { id: 'check-tables', label: 'Check: Tables', command: './binder/binder check tables', icon: 'table' },
  { id: 'check-math', label: 'Check: Math', command: './binder/binder check math', icon: 'symbol-math' },
  { id: 'check-index', label: 'Check: Index', command: './binder/binder check index', icon: 'list-ordered' },
  { id: 'check-bib', label: 'Check: Bibliography', command: './binder/binder check bib', icon: 'book' },
  { id: 'check-json', label: 'Check: JSON', command: './binder/binder check json', icon: 'json' },
  { id: 'check-units', label: 'Check: Units', command: './binder/binder check units', icon: 'beaker' },
  { id: 'check-epub', label: 'Check: EPUB', command: './binder/binder check epub', icon: 'book' },
  { id: 'check-sources', label: 'Check: Sources', command: './binder/binder check sources', icon: 'references' },
];
...
export const INFO_ACTIONS: ActionDef[] = [
  { id: 'info-stats', label: 'Info: Book Stats', command: './binder/binder info stats', icon: 'graph' },
  { id: 'info-stats-vol1', label: 'Info: Stats Vol1', command: './binder/binder info stats --vol1 --by-chapter', icon: 'graph' },
  { id: 'info-stats-vol2', label: 'Info: Stats Vol2', command: './binder/binder info stats --vol2 --by-chapter', icon: 'graph' },
  { id: 'info-figures', label: 'Info: Figure List', command: './binder/binder info figures', icon: 'file-media' },
  { id: 'info-concepts', label: 'Info: Key Concepts', command: './binder/binder info concepts', icon: 'lightbulb' },
  { id: 'info-headers', label: 'Info: Section Headers', command: './binder/binder info headers', icon: 'symbol-structure' },
  { id: 'info-acronyms', label: 'Info: Acronyms', command: './binder/binder info acronyms', icon: 'text-size' },
];
