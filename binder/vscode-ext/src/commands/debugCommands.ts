...
async function runTestAllChapters(
  ...
  const parallelCmd = `./binder/binder ${format} reset --${volumeId} && [parallel: ${workers} workers × ./binder/binder build ${format} <chapter> --${volumeId} -v]`;
  showBuildManifest({
    repoRoot,
    vol: volumeId,
    format,
...
/**
 * Debug All Chapters (Sequential) — runs `./binder/binder debug pdf --vol1` in a
...
async function runDebugAllChapters(
  ...
  const cmd = `./binder/binder debug pdf --${selection.id} -v`;
  showBuildManifest({
    repoRoot: root,
    vol: selection.id,
    format: 'pdf',
