...
6. **CLI plan semantics cleanup.** `hardware.nodes` is historically interpreted
   as total accelerators in existing examples. A future schema version should
   rename this to `accelerators` or add an explicit `node_count` field to avoid
   ambiguity.
7. **Solver module decomposition.** Solver implementations now live in
   `engine/solvers/` by domain, while `engine/solver.py` remains the
   backward-compatible import surface for textbook examples.
