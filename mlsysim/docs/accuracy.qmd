...
4. **Empirical calibration refresh.** Re-run the benchmark comparisons in
   ...
   published benchmark reports, then update the recommended efficiency ranges.
...
6. **CLI plan semantics cleanup.** `hardware.nodes` is historically interpreted
   ...
   ambiguity.
7. **Solver module decomposition.** `engine/solver.py` remains large. Splitting
   it by domain would make review, ownership, and formula-level documentation
   easier without changing public APIs.
