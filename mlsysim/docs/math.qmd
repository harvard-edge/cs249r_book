...
*Used by [`mlsysim.physics.calc_transformer_training_flops`]({{< var github_repo >}}/blob/dev/mlsysim/mlsysim/mlsysim/physics/).*
...
This estimates a coarse scheduling proxy used by chunked-prefill systems: smaller chunks do not reduce total prefill compute, but they reduce the largest decode stall caused by a co-scheduled prefill. MLSYSIM does not implement Sarathi-Serve's full scheduler. Disaggregated systems such as Splitwise and DistServe mitigate prefill/decode interference by assigning the phases to separate resources, trading scheduler interference for KV-cache transfer and resource-partitioning costs [@agrawal2025; @patel2024; @zhong2024distserve].
...
*Implemented in [`mlsysim.physics.calc_failure_probability`]({{< var github_repo >}}/blob/dev/mlsysim/mlsysim/mlsysim/physics/).*
...
*Implemented in [`mlsysim.physics.calc_effective_flops`]({{< var github_repo >}}/blob/dev/mlsysim/mlsysim/mlsysim/physics/).*
...
Real systems deviate from these assumptions. Trust MLSYSIM for bottleneck classification and relative comparisons. Absolute latency is workload-dependent; well-calibrated cases are often within ±15–30%, while production serving can be 1.5–2× slower than idealized roofline bounds. See [Accuracy & Validation](accuracy.qmd) for detailed comparisons against MLPerf benchmarks.
:::
