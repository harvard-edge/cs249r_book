...
---
MLSys·im avoids "black box" heuristics. Every output traces back to one of the equations below, each grounded in a published systems paper. Before diving into code, read this page to understand *what* the 22 resolvers (20 models and 2 solvers) resolving the [22 Systems Walls](architecture.qmd#the-22-systems-walls) are computing and *why*.
...
*Literature: @zheng2024sglang (RadixAttention for automatic prefix caching).*
...
*Literature: @patel2024splitwise (Splitwise: phase-aware LLM inference).*
...
1. The prefill node has higher compute throughput (e.g., more FLOPS)
2. The decode node has higher memory bandwidth (e.g., more HBM bandwidth per GPU)
3. The KV-cache transfer time is small relative to the TTFT savings

### 3.6 Speculative Decoding

*Literature: @leviathan2023fast (Fast Inference from Transformers via Speculative Decoding).*

Speculative decoding uses a small *draft* model to generate $K$ candidate tokens cheaply, then *verifies* all $K$ in a single forward pass of the target model. The expected number of accepted tokens per step follows a geometric series [@leviathan2023fast]:
...
*Implemented in [`mlsysim.core.solver.DataModel`](api/core.solver.DataModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.ScalingModel`](api/core.solver.ScalingModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.OrchestrationModel`](api/core.solver.OrchestrationModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.CompressionModel`](api/core.solver.CompressionModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.TailLatencyModel`](api/core.solver.TailLatencyModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.WeightStreamingModel`](api/core.solver.WeightStreamingModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.ResponsibleEngineeringModel`](api/core.solver.ResponsibleEngineeringModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.EfficiencyModel`](api/core.solver.EfficiencyModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.ContinuousBatchingModel`](api/core.solver.ContinuousBatchingModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.TransformationModel`](api/core.solver.TransformationModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.TopologyModel`](api/core.solver.TopologyModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.InferenceScalingModel`](api/core.solver.InferenceScalingModel.qmd).*
...
*Implemented in [`mlsysim.core.solver.CheckpointModel`](api/core.solver.CheckpointModel.qmd).*
