...
| baseline_precision | str          | Configurable precision of the uncompressed model that ratios and sizes are measured against, resolved through `core.units.PRECISION_MAP`. Defaults to `"fp32"` (INT8 is 4x, INT4 is 8x). Pass `"fp16"` for models trained and served from FP16/BF16 weights, such as most LLMs, where INT4 is quoted as 4x. | `'fp32'`         |
