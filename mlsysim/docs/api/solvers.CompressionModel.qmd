...
| baseline_precision | str          | Precision of the uncompressed model that ratios and sizes are measured against, resolved through `core.units.PRECISION_MAP` (default `"fp16"`: LLMs are trained and served from FP16/BF16 weights, so INT4 is quoted as 4x). Pass `"fp32"` for a model whose reference artifact is FP32 (for example a CNN exported from FP32 training), where INT8 is 4x and INT4 is 8x. | `'fp16'`         |

#### Returns {.doc-section .doc-section-returns}

| Name   | Type              | Description                                                                                                                                                  |
|--------|-------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------|
|        | CompressionResult | Compression metrics including memory savings, inference speedup, and estimated accuracy delta, plus the `baseline_precision` they are relative to.            |
