...
print(f"Throughput:     {profile.throughput:.0f} img/s")
...
print(f"CPU transform time:     {t.transform_time:~.2f}")
print(f"GPU step time:          {t.accelerator_step_time:~.2f}")
print(f"CPU is bottleneck:      {t.is_bottleneck}")
print(f"GPU utilization:        {t.accelerator_utilization:.1%}")
print(f"Slowdown factor:        {t.slowdown_factor:.2f}x")
```
...
```{python}
print(f"{'Workers':>8}  {'Throughput':>12}  {'GPU Util @ bs=512':>18}")
print("-" * 44)
...
for n_workers in [8, 16, 32]:
    ...
    print(
        f"{n_workers:>8}  "
        f"{cpu_tp.to('GB/s'):>12.1f~}  "
        f"{t.accelerator_utilization:>17.1%}"
    )
```
