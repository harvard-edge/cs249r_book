...
subtitle: "Your GPU is ready for the next batch. Your CPU is still decoding JPEGs."
...
The GPU finishes this batch in well under a tenth of a second. That is the ceiling. Now let's
...
Compare the two step times above: the GPU could get through each batch several times
faster than your 8 CPU workers can prepare it. The GPU sits idle waiting for data. This is why production training pipelines use
...
crossover = next(r[0] for r in rows if r[3] == "Transformation")
info("Crossover", First_CPU_bound_batch=crossover)
```

The crossover sits at the very bottom of the sweep. At the smallest batches the CPU keeps
up and the GPU runs at full utilization, but only a few samples later Wall 9 binds and
never lets go. CPU transform time is strictly proportional to batch size. The GPU step is
a fixed dispatch overhead plus a per-sample compute cost, and that fixed overhead only
shields the GPU while batches are tiny. Once it is amortized, both costs grow linearly, so
GPU utilization levels off at the ratio of per-sample GPU compute to per-sample CPU
transform time instead of falling to zero.
