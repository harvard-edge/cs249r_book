...
print(f"Model:          {model.name} ({model.parameters.to('Gcount'):.0f} params)")
...
print(f"{'K':>3}  {'Total Time':>12}  {'Tokens':>8}  {'Energy (J)':>11}  {'Multiplier':>11}")
print("-" * 52)
...
print(f"{'K':>3}  {'Query (ms)':>11}  {'QPS/GPU':>8}  {'GPUs':>6}  {'Annual TCO ($)':>16}")
print("-" * 52)
...
print(f"{'Strategy':<25} {'GPUs':>6}  {'Annual TCO ($)':>16}")
print("-" * 52)
print(f"{'All queries -> GPT-4 K=8':<25} {fleet_objects[8].total_accelerators:>6}  ${tco_all_k8.tco_usd:>14,.0f}")
print(f"{'70/30 routed':<25} {fleet_routed.total_accelerators:>6}  ${tco_routed.tco_usd:>14,.0f}")
print(f"{'Savings':<25} {'':>6}  ${savings:>14,.0f} ({pct_savings:.0f}%)")
```
