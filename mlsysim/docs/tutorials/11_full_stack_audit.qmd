...
print(f"Model:           {model.name} ({model.parameters.to('Bparam'):.1f~})")
print(f"Fleet:           {fleet.count} nodes x {node.accelerators_per_node} GPUs = {fleet.total_accelerators} GPUs")
print(f"Intra-node:      NVLink 4.0 ({NVLINK_H100_BW.to('GB/s'):.0f~})")
print(f"Inter-node:      IB NDR ({INFINIBAND_NDR_BW.to('Gbps'):.0f~})")
print(f"Region:          {Grids.Quebec.name}")
```
...
print(f"=== Domain: Node (Walls 1-3) ===")
print(f"Bottleneck:    {node_result.bottleneck}")
print(f"Per-GPU latency: {node_result.latency.to('ms'):~.2f}")
print(f"Throughput:    {node_result.throughput:.0f} samples/s")
```
...
print(f"=== Domain: Data (Walls 8-10) ===")
print(f"Data demand:   {data_result.demand_bw:~.2f}")
print(f"Data supply:   {data_result.supply_bw:~.2f}")
print(f"Utilization:   {data_result.utilization:.1%}")
print(f"Stalled:       {data_result.is_stalled}")
...
print(f"=== Domain: Algorithm (Walls 11-13) ===")
print(f"Compute budget:     {compute_budget.to('EFLOP'):.1f~}")
print(f"Optimal tokens:     {scaling_result.optimal_tokens.magnitude:.2e}")
print(f"Tokens/parameter:   {scaling_result.tokens_per_parameter:.1f}")
print(f"Chinchilla ratio:   {'OVER' if scaling_result.tokens_per_parameter > 20 else 'UNDER'}-trained")
```
...
print(f"=== Domain: Fleet (Walls 14-16) ===")
print(f"Scaling efficiency:  {dist_result.scaling_efficiency:.2%}")
print(f"Step latency:        {dist_result.step_latency_total.to('ms'):~.2f}")
print(f"DP comm latency:     {dist_result.dp_communication_latency.to('ms'):~.2f}")
print(f"TP comm latency:     {dist_result.tp_communication_latency.to('ms'):~.2f}")
print(f"Bubble fraction:     {dist_result.bubble_fraction:.2%}")
```
...
print(f"Fleet MTBF:          {rel_result.fleet_mtbf.to('hour'):.1f~}")
print(f"Failure probability: {rel_result.failure_probability:.2%}")
print(f"Expected failures:   {rel_result.expected_failures:.1f}")
print(f"Optimal ckpt interval: {rel_result.optimal_checkpoint_interval.to('minute'):~.1f}")
```
...
print(f"=== Domain: Ops (Walls 17-20) ===")
print(f"CapEx:               ${econ_result.capex_usd:,.0f}")
print(f"OpEx (energy):       ${econ_result.opex_energy_usd:,.0f}")
print(f"OpEx (maintenance):  ${econ_result.opex_maintenance_usd:,.0f}")
print(f"Total TCO:           ${econ_result.tco_usd:,.0f}")
```
...
print(f"IT Energy:           {sust_result.it_energy_kwh.to('MWh'):.1f~}")
print(f"Total Energy (PUE):  {sust_result.total_energy_kwh.to('MWh'):.1f~}")
print(f"Carbon footprint:    {sust_result.carbon_footprint_kg:.0f} kg CO2")
print(f"Water usage:         {sust_result.water_usage_liters:.0f} liters")
print(f"PUE:                 {sust_result.pue}")
print(f"Region:              {sust_result.region_name}")
```
...
print(f"=== Domain: Analysis (Walls 21-22) ===")
print(f"Binding constraint:  {sens_result.binding_constraint}")
for param, val in sens_result.sensitivities.items():
...
print(f"")
print(f"Synthesis (200ms per-GPU training step target):")
print(f"  Required BW:       {synth_result.required_bw.to('TB/s'):~.2f}")
print(f"  Required FLOPS:    {synth_result.required_flops.to('TFLOPs/s'):~.1f}")
print(f"  Required memory:   {synth_result.required_memory.to('GB'):~.1f}")
```
...
```{python}
header = f"{'Domain':<12} {'Solver':<22} {'Key Metric':<30} {'Binding Wall':>16}"
print(header)
print("=" * len(header))
...
for domain, solver, metric, wall in rows:
    print(f"{domain:<12} {solver:<22} {metric:<30} {wall:>16}")
```
