...
flowchart TB
    ...
    E["<b>Layer E: Solvers (Analysis)</b><br/>SingleNode · Distributed · Serving<br/>Data · Scaling · Orchestration · Compression<br/>Economics · Sustainability · Reliability"]
...
*   **`ServingSolver`**: Models the Pre-fill and Decode phases of LLM inference.
*   **`DistributedSolver`**: Maps Layer A to Layer D using Ring All-Reduce and Pipeline schedules.
*   **`DataSolver`**: Analyzes the "Data Wall" — the throughput bottleneck between storage and compute.
*   **`ScalingSolver`**: Calculates compute-optimal model size and data (Chinchilla Laws).
*   **`OrchestrationSolver`**: Models cluster wait times and utilization (Little's Law).
*   **`CompressionSolver`**: Models the "Compression Tax" — accuracy vs. memory/latency trade-offs.
