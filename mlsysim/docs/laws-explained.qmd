...
## Domain 2: Data — Can Storage Feed the Pipeline?
...
**Equation:** `Compression = baseline_bits/bits (quantization); Compression = 1/(1-sparsity) (pruning)`

**Plain English:** You can shrink a model by using fewer bits than a baseline precision
(quantization: FP32→INT8 = 4x smaller; FP16→INT4 = 4x for an LLM served from FP16/BF16
weights) or by removing weights (pruning: 50% sparse = 2x smaller). `CompressionModel`
measures against a configurable `baseline_precision` that defaults to `"fp32"`; pass
`"fp16"` for FP16/BF16-served models, or INT4 reads as 8x instead of 4x. But shrinking costs
