...
**Plain English:** You can shrink a model by using fewer bits than the precision it ships
in (quantization: FP16→INT4 = 4x smaller for an LLM served from FP16/BF16 weights;
FP32→INT8 = 4x for a model whose reference artifact is FP32) or by removing weights
(pruning: 50% sparse = 2x smaller). `CompressionModel` measures against
`baseline_precision="fp16"` unless you pass `"fp32"`. But shrinking costs
