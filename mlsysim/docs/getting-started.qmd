...
::: {.callout-note}
Editable installs (`pip install -e .`) are not currently supported because the Hatch build configuration uses a `sources` rewrite that the editables backend cannot handle. Use a regular `pip install` from the source tree, or rebuild after each change with `make build && pip install --force-reinstall dist/*.whl`.
:::
