...
seconds). Bytes, FLOPs, counts, and dollars are dimensionless aliases in pint,
so a bare `.to("flop/s")` on a bandwidth will not raise -- MLSYSIM adds stricter
**unit-family checks** at every schema boundary (`require_unit_family` in
`mlsysim.core.types`), so a registry or solver input declared as compute
throughput rejects `900 GB/s`, and a memory bandwidth rejects `1 TFLOP/s`.
