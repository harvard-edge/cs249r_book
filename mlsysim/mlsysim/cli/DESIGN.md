...
Cluster YAML follows the same rule. New plans should use `hardware.accelerators`
or `hardware.node_count` plus `hardware.accelerators_per_node`. The legacy
`hardware.nodes` field remains accepted as an alias for total accelerators so
existing examples keep working. Whichever form is used, the total accelerator
count must divide cleanly by `accelerators_per_node`. The CLI may infer
