...
from mlsysim.cli.context import OUTPUT_FORMAT_HELP, validate_output_format
...
@app.callback()
def main(ctx: typer.Context, output: str = typer.Option("text", "--output", "-o", help=OUTPUT_FORMAT_HELP)):
    """Global configuration for the CLI."""
    state["output_format"] = validate_output_format(output)
...
app.command(name="schema", help="Export the JSON Schema for the mlsys.yaml configuration file.")(schema_main)
