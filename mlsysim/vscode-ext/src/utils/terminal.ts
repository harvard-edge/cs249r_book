...
const TERMINAL_NAME = 'MLSysim';
const STATE_HISTORY_KEY = 'mlsysim.commandHistory';
...
/** Run a command in the dedicated MLSysim terminal */
...
export function rerunLastCommand(): void {
  ...
  if (!last) {
    vscode.window.showWarningMessage('MLSysim: no previous command to rerun.');
    return;
  }
  rerunCommand(last);
}

/** Show the MLSysim terminal */
