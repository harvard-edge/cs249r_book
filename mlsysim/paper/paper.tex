...
print(f"Scaling Efficiency: {perf.scaling_efficiency:.1%}")   # 55.8%
print(f"Training Duration:  {duration.to('day'):.1f}")        # 67.0 day
print(f"Total Job Cost:     ${tco.tco_usd:,.0f}")             # $18,540,181
\end{lstlisting}
...
To validate the Tier 3 design-space search, we configure the \texttt{ParallelismOptimizer} with the Meta Llama~3 405B model and its 16{,}384 H100 cluster constraints~\citep{llama3team2024}. The optimizer sweeps power-of-two TP and PP degrees with $\text{TP} \times \text{PP} \times \text{DP} = 16{,}384$, pre-screening each candidate so that the per-GPU weight-plus-gradient shard fits in 90\% of the 80\,GB HBM capacity, then ranking survivors by MFU. It recovers Meta's binding decision, $\text{TP}{=}8$, pinning tensor parallelism to a single node so AllReduce traffic stays on intra-node NVLink rather than crossing InfiniBand, and selects $\text{PP}{=}4$ with $\text{DP}{=}512$: the shallowest pipeline whose first-order memory screen passes. Meta's production split uses a deeper $\text{PP}{=}16$ with context parallelism, driven by activation-memory and batch-size constraints beyond the optimizer's weight-level screen. The optimizer therefore reproduces the qualitative strategy (NVLink-bound TP, minimum memory-feasible pipeline depth, remaining degrees to DP) rather than Meta's batch-size-specific 4D tuning bit-for-bit.
...
3: Llama~3 MFU       & 39.6\%       & 38--43\%        & in range \\
4: PaLM scaling      & 43\% MFU     & $\sim$46\% MFU  & 6.5\% \\
5: Chinchilla $P^*$  & 70.0B        & 70B             & $<$1\% \\
6: GPT-3 CO$_2$      & 552\,t       & 552\,t          & 0.0\% \\
7: Llama~3 Parallelism & TP=8, PP=4, DP=512 & TP=8, PP=16, CP=16 & TP exact \\
\bottomrule
\end{tabularx}
\end{table*}
...
Dimensional strictness (\Cref{sec:dimensional}) catches unit errors at runtime. A complementary question is whether the framework's \emph{architectural} invariants hold as the codebase evolves: do all hardware specifications flow through typed registries? Does every formula carry \texttt{pint} units end-to-end? Can a contributor accidentally reintroduce a bare constant that bypasses the registry? \mlsysim answers these with four machine-checked structural invariants, enforced by a 751-test suite, each encoding a design decision that would otherwise erode through incremental changes:

\begin{enumerate}[leftmargin=*,itemsep=2pt]
\item \textbf{Specification separation.} The legacy constants module (\texttt{core/constants.py}) is reduced to a units-only re-export surface, and an AST-based CI gate rejects \emph{any} module-level value defined there, alongside name-pattern checks for hardware, model, and network constants. Every domain figure must live in its category registry, not in a monolithic constants file.
...
\begin{table}[!t]
\centering
\caption{\textbf{Hardware Registry.} All 34 registered devices organized by deployment tier. Peak FLOP/s is FP16 Tensor Core (or equivalent) throughput; BW is primary-memory bandwidth (HBM, LPDDR, or on-die SRAM as appropriate). Memory uses the marketed (binary) capacity labels.\protect\footnotemark\ The last row shows a user-defined custom entry.}
...
# Total Carbon: 106.3 tonnes CO2e
\end{lstlisting}
