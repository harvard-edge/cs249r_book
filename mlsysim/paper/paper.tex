...
\toprule
\textbf{\#} & \textbf{Wall} & \textbf{Solver} & \textbf{Bounded} & \textbf{Core Equation} & \textbf{Ref.} \\
\midrule
...
6  & Streaming     & WeightStream    & Injection BW      & $T = \max(T_{\text{inj}}, T_{\text{comp}})$ & \citealt{lie2023cerebras} \\
7  & Tail Latency  & TailLatency     & P99 queueing      & Erlang-C M/M/$c$ & \citealt{dean2013tail} \\
\midrule
\multicolumn{6}{@{}l}{\textit{Data (Movement \& Pipelines)}} \\
8  & Ingestion     & Data            & Storage I/O       & $\rho = BW_{\text{demand}} / BW_{\text{supply}}$ & \citealt{mohan2022analyzing} \\
9  & Transform.    & Transform.      & CPU preproc.      & $T = B \cdot S / C_{\text{tput}}$ & \citealt{murray2021tf} \\
...
12 & Reasoning     & Inf.\ Scaling   & Inf.-time comp.   & $T = K \times T_{\text{step}}$ & \citealt{brown2024inference} \\
13 & Fidelity      & Compression     & Acc.--efficiency  & $r = 32/b;\; r = 1/(1{-}s)$ & \citealt{han2015deep} \\
\midrule
...
\midrule
\multicolumn{6}{@{}l}{\textit{Ops (Economics, Sustainability \& Safety)}} \\
17 & Capital       & Economics       & TCO               & $\text{TCO} = \text{CapEx} + \text{OpEx}$ & \citealt{barroso2018datacenter} \\
18 & Sustain.      & Sustainability  & Carbon + water    & $\text{CO}_2 = E \times \text{PUE} \times \text{CI}$ & \citealt{patterson2022carbon} \\
19 & Checkpoint    & Checkpoint      & I/O burst penalty & $\text{penalty} = T_{\text{write}} / T_{\text{interval}}$ & \citealt{eisenman2022checknrun} \\
20 & Safety        & Resp.\ Eng.     & DP-SGD overhead   & $\sigma \propto 1/\epsilon$ & \citealt{abadi2016deep} \\
\midrule
...
\textbf{Wall~4: The Serving Wall.} Autoregressive LLM inference exhibits two distinct phases with fundamentally different Roofline characteristics~\citep{pope2023llm}. The \texttt{ServingSolver} decomposes end-to-end inference latency as:
\begin{align}
\label{eq:serving}
...
\end{align}
where $S_{\text{in}}$ is the input sequence length and $P$ is the parameter count. The prefill phase processes all input tokens in parallel and is compute-bound; the decode phase generates one token at a time and is memory-bandwidth-bound. The solver incorporates modern paradigms including \textbf{Speculative Decoding} (which alters decode logic via probability-weighted verification using a smaller draft model) and \textbf{Disaggregated Serving} (which separates prefill and decode phases onto different hardware, introducing KV-cache network transfer times). This duality explains why batching strategies that improve prefill throughput may have no effect on decode latency, because the two phases are bound by different resources.
...
\end{equation}
The two terms inside the $\max$ represent the injection time (weight delivery) and the compute time (matrix arithmetic) for one layer. When $B$ is small, weight injection dominates and the compute engine sits idle; when $B$ is large, compute dominates and the injection link sits idle. Setting the two terms equal yields the optimal batch size $B^{*} = (BW_{\text{inject}} \times \text{Peak} \times \eta) / (2|W_{\ell}|)$, the unique operating point where injection and compute perfectly overlap, maximizing utilization of both resources. \textbf{Assumptions:} Layer weights dominate the injection payload; 10\% overhead is reserved for working memory; perfect within-layer pipelining is assumed.
...
\end{equation}
where $B$ is the batch size, $S_{\text{sample}}$ is the per-sample processing cost, and $C_{\text{throughput}}$ is the aggregate CPU preprocessing throughput. \textbf{Assumption:} Preprocessing is CPU-bound and scales linearly with worker count up to core saturation.
...
\end{equation}
This wall becomes binding when collective communication patterns demand bandwidth that the topology cannot supply at scale. \textbf{Assumption:} $\beta$ values are topology-specific constants; real networks may exhibit dynamic congestion not captured by this static model.
...
\end{equation}
where $T_{\text{step}}$ is the per-step latency, itself a function of model size $P$ and context length $S_{\text{context}}$. Chain-of-thought and tree-search strategies can increase $K$ by $10$--$100\times$ relative to single-pass inference~\citep{brown2024inference}, fundamentally altering serving cost economics. \textbf{Assumption:} Each reasoning step is an independent decode sequence; KV-cache is not shared across steps.
...
\end{align}
The accuracy impact $\Delta_{\text{acc}}$ is modeled as a configurable function rather than a hardcoded constant, since the fidelity--compression frontier varies by architecture and task. \textbf{Assumptions:} Accuracy degradation follows empirical curves from~\citet{gholami2021survey}; pruning speedup assumes structured sparsity with hardware support.
...
\end{equation}
where $\delta$ is the time to write one checkpoint. The \texttt{ReliabilitySolver} uses these relations to estimate the fraction of compute lost to checkpointing and recovery. \textbf{Assumption:} Failures are independent and exponentially distributed (memoryless).
...
\subsection{Ops (Economics, Sustainability \& Safety)}
\label{sec:walls-operations}

The Ops walls capture constraints that are not about \emph{how fast} a system runs but \emph{whether it should run at all}: economic viability, environmental impact, checkpoint overhead, and responsible deployment.
...
\end{equation}
The per-step clipping and noise addition incur a training slowdown of approximately $2$--$10\times$. Fairness constraints require sufficient representation of minority subgroups, demanding additional data proportional to $O(1/p_{\min})$ where $p_{\min}$ is the smallest subgroup prevalence. \textbf{Assumption:} Privacy budget $\varepsilon$ is a hard constraint; the solver reports the compute multiplier, not the privacy guarantee itself.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{images/pdf/solver-chaining.pdf}
\caption{\textbf{Solver Composition: Demand--Supply Separation.} Three input layers (Workload, Hardware, Infrastructure) feed a pipeline of four solvers. Each solver is a pure function: typed inputs in, dimensionally correct outputs out. The \texttt{SystemEvaluation} scorecard aggregates results across feasibility, performance, latency, economics, and sustainability.}
...
\textbf{MLPerf ResNet-50 on A100.}
...
\textbf{vLLM Llama-2-70B on H100.}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{images/pdf/design-space-heatmap.pdf}
...
Students interact with \mlsysim primarily through single-solver queries and short solver chains. The design space is broad: a student might profile the memory footprint of a new architecture, sweep batch sizes to find the Roofline crossover, compare parallelism strategies for a fixed GPU budget, estimate the carbon cost of a training run, or stress-test an SLA under traffic spikes. Each query requires specifying a model, a hardware target, and a solver, and the type system then ensures dimensional consistency throughout.

We present two examples chosen to illustrate complementary aspects of the framework. The first (S1) demonstrates \emph{vertical} solver composition, chaining inference and economics solvers to connect a modeling decision (chain-of-thought reasoning) to its infrastructure cost. The second (S2) demonstrates \emph{horizontal} solver composition, combining data-pipeline and compute solvers to diagnose a bottleneck that shifts between walls as batch size changes.
...
The crossover from memory-bound to compute-bound occurs near batch 64, where the arithmetic intensity $\text{AI} = 2B \cdot P / |W|$ crosses the hardware ridge point ($F_{\text{peak}} / \text{BW}_{\text{HBM}} \approx 62$ FLOP/byte for H100). \Cref{fig:roofline} visualizes this transition on the Roofline diagram. The entire sweep executes in $<$50\,ms, enabling real-time interaction during lecture.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{images/pdf/roofline-crossover.pdf}
\caption{\textbf{Roofline Crossover: Batch Size Sweep on H100.} Increasing batch size moves the operating point rightward along the Roofline, transitioning from memory-bound (red) to compute-bound (blue) near $B{\approx}64$. The ridge point at 62~FLOP/byte marks the crossover where HBM bandwidth and peak compute ceilings intersect.}
\label{fig:roofline}
\end{figure}

\subsubsection{I2: Sustainability What-If, Iowa vs.\ Qu\'ebec}
An instructor poses a policy question: \emph{does geography matter for carbon footprint?} Using the \texttt{DistributedSolver}, they configure a 256-GPU cluster training a 70B model for 30 days. The \texttt{SustainabilitySolver} then computes emissions under two grid profiles: Iowa (680~gCO$_2$/kWh) and Qu\'ebec (17~gCO$_2$/kWh). The configuration is identical in hardware, model, and MFU, yet carbon footprint differs by $40{\times}$ (412 vs.\ 10.3 tonnes CO$_2$; 231 vs.\ 5.8\,kL water), demonstrating that grid carbon intensity is a first-order systems design variable (Wall~18). \Cref{fig:carbon} visualizes this contrast.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{images/pdf/carbon-comparison.pdf}
\caption{\textbf{Geography as a Systems Variable: Iowa vs.\ Qu\'ebec.} An identical 256-GPU cluster training a 70B model for 30 days produces $40{\times}$ more CO$_2$ in Iowa (coal/gas grid, 680~gCO$_2$/kWh) than in Qu\'ebec (95\% hydroelectric, 17~gCO$_2$/kWh). Water usage follows the same ratio. Hardware, model, parallelism strategy, and MFU are identical; only the regional carbon intensity differs.}
\label{fig:carbon}
\end{figure}
