...
Reproducible analysis requires reproducible inputs. \mlsysim organizes vetted specifications into six curated registries collectively called the \emph{MLSys Zoo}: \textbf{Hardware} (accelerators and boards), \textbf{Models} (workload architectures), \textbf{Datasets} (benchmark catalog entries), \textbf{Systems} (nodes, fabrics, and fleets), \textbf{Infrastructure} (grids and datacenters), and \textbf{Platforms} (deployment envelopes). Executable \texttt{Scenarios.*} then compose existing model, hardware, and system entries with local constraints such as latency or power; they do not redefine the underlying facts. Non-executable real-world anchors, such as Waymo data-rate ranges or mobile power envelopes, live in \texttt{ReferenceStats.*}. Cited field figures (MFU bands, scaling laws) live in \texttt{Literature.*}; economics in \texttt{Infrastructure.Pricing}; solver fallbacks in \texttt{mlsysim.engine.calibration}; architecture formulas (Roofline, AllReduce, Young--Daly) live in \texttt{mlsysim.physics}. Each registry entry is a fully typed object whose quantities are sourced from datasheets with provenance metadata including source URLs and verification dates. For example, \texttt{Hardware.Cloud.H100} returns a \texttt{HardwareNode} with all physical quantities dimensioned via \texttt{pint}.
...
res = solver.solve(model=mlsysim.Models.Language.Llama2_70B,
