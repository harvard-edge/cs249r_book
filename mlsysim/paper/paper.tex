...
4: PaLM scaling      & 43\% MFU     & $\sim$46\% MFU  & 6.5\% \\
...
A student investigates the inference economics of chain-of-thought (CoT) prompting. Using Llama-3 70B on H100 hardware, they configure the \texttt{InferenceScalingModel} with $K{=}8$ reasoning steps. Each step generates $\sim$50 tokens (the calibrated default) at the memory-bound decode rate, so the total reasoning time is $T_{\text{reason}} = \text{TTFT} + K \cdot 50 \cdot \text{ITL}$. The solver reports a total reasoning time of ${\sim}17.9$\,s, a $6.5{\times}$ per-query latency multiplier relative to a single-step answer (the prefill TTFT amortizes across steps). Feeding this multiplier into the \texttt{EconomicsModel} translates it into fleet capacity: a deployment sized for 100~QPS of single-pass queries must grow its H100 fleet, and hence its TCO, by the same ${\sim}6.5{\times}$ to sustain the same query rate under CoT. This result quantifies CoT as a direct multiplier on infrastructure cost, not merely on latency.
...
The MLSys Zoo comprises eight registries spanning the full systems stack (plus the \texttt{Literature} and \texttt{ReferenceStats} auxiliary surfaces). This appendix catalogs the three most referenced registries; the complete YAML source for all of them is available in the repository.
...
\begin{table}[!t]
\centering
\caption{\textbf{Hardware Registry.} All 37 registered devices organized by deployment tier. Peak FLOP/s is FP16 Tensor Core (or equivalent) throughput; BW is primary-memory bandwidth (HBM, LPDDR, or on-die SRAM as appropriate). Memory uses the marketed (binary) capacity labels.\protect\footnotemark\ The last row shows a user-defined custom entry.}
...
\emph{Scenario:} You want to know how much decode throughput continuous batching can buy on an H100 before KV-cache and activation traffic saturate the memory system. Instead of running hours of empirical profiling, you can use the core \texttt{Engine} to sweep the batch dimension algebraically in under a millisecond per point.

\begin{lstlisting}[language=Python, caption={\textbf{Programmatic Sweeps (Tier 1).} Using the \texttt{Engine} to sweep a workload across a hardware constraint space (batch size). Throughput rises from 178 to ${\sim}$2{,}000 tokens/s and then saturates: per-request memory traffic grows with the batch, so decode stays memory-bound at every point. Because execution takes $<$1\,ms per point, sweeping hundreds of configurations is trivial.}, label={lst:appendix-sweep}, float=t]
...
\emph{Scenario:} You have a fixed cluster and a large model and want the framework to \emph{find} the best 3D-parallelism split rather than specifying it by hand. The Tier~3 \texttt{ParallelismOptimizer} sweeps power-of-two $\text{TP}\times\text{PP}\times\text{DP}$ factorizations, screens each for per-GPU memory feasibility (weights, gradients, and optimizer state), and ranks the survivors by MFU.
