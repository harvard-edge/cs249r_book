...
\newcommand{\predict}{%
  \begin{alertblock}{\faLightbulbO\; Predict Before You Peek}%
}
\newcommand{\endpredict}{\end{alertblock}}
...
\begin{frame}
\note{
% -- NARRATE: Welcome attendees, set the tone.
Welcome to the MLSys-im tutorial. Today we will build quantitative
intuition for ML systems from first principles. Every equation we
cover has a matching Python one-liner you will run on your laptop.
...
\note{
% -- NARRATE: Walk through objectives quickly. Emphasize that by the
...
\begin{frame}{Setup: Install \& Verify}
\note{
% -- NARRATE: Give attendees 2 minutes to run these commands.
...
\note{
% -- NARRATE: This is the road map for the day. Point out that
% we will hit walls 1--7 (Node) before lunch and walls 8--22
% after lunch. Each wall has one equation and one mlsysim solver.
% -- ENGAGE: Ask: ``How many of these walls have you personally
% hit in your own work?'' Show of hands.
% -- FLEX: [CORE]
}
...
\note{
% -- NARRATE: Pose the question dramatically. Pause for 5 seconds.
% ``This is the most important question in ML systems engineering.
% By the end of this section you will be able to answer it in
% 3 lines of Python.''
% -- FLEX: [CORE]
}
...
\note{
% -- LINK: Connects to the 22-wall taxonomy from Part 0.
% -- NARRATE: ``You don't choose a Transformer because it's trendy;
...
\note{
% -- LINK: Now we make ``constraints drive architecture'' quantitative.
% -- NARRATE: Draw the two regimes on the board. Left = memory-bound
% (line slopes up with arithmetic intensity). Right = compute-bound
% (flat ceiling). The ridge point is where they cross.
% -- ENGAGE: ``Before I show numbers: if a model does 16B FLOPs and
% loads 16 GB of weights, what is its arithmetic intensity?''
% Expected: 1 FLOP/byte. That is far left on the Roofline.
% -- WARN: Students conflate FLOPS with throughput. Throughput also
% depends on whether you can actually feed data fast enough.
% -- FLEX: [CORE]
}
...
\note{
% -- LINK: Left side of the Roofline is memory; now the right side.
% -- NARRATE: ``This is the speed limit. No software trick can make
...
\note{
% -- LINK: Now the other regime --- most LLM inference lives here.
% -- NARRATE: ``Your model's weights sit in HBM. To use them, the GPU
% must read them through a pipe with finite width. Bigger model =
% longer loading time.''
% -- ENGAGE: ``Quick mental math: 16 GB model, 3.35 TB/s bandwidth.
% How long to load?'' Give 10 seconds.
% Expected: 16 / 3350 = 4.8 ms.
...
\begin{frame}{Predict: A100 vs H100}
\note{
% -- ENGAGE: This is a predict-before-reveal moment.
% Give the audience 60 seconds to think.
% Expected answer: memory-bound. The A100 has 2 TB/s vs H100 3.35 TB/s,
% so the speedup is ~1.7x, not 3.2x (the FLOPS ratio).
...
\centering
\Large\bfseries
The H100 has 3.2$\times$ more FLOPS than the A100.\\[0.5cm]
\normalsize
Llama-3 8B, batch size 1, FP16 inference.\\[0.5cm]

\pause

\begin{tabular}{lcc}
\toprule
              & \textbf{A100} & \textbf{H100} \\
\midrule
Peak FP16     & 312 TFLOPS    & 989 TFLOPS \\
HBM BW        & 2.0 TB/s      & 3.35 TB/s \\
\midrule
Bottleneck    & Memory        & Memory \\
Weight-load time & 8.0 ms     & 4.8 ms \\
\textbf{Speedup} & ---        & \textbf{1.7$\times$} \\
\bottomrule
\end{tabular}

\vspace{0.3cm}
\pause
\alert{3.2$\times$ more FLOPS $\neq$ 3.2$\times$ speedup when memory-bound.}
\end{frame}

% --- Slide 1.7: Live Demo --- Engine.solve ---
\begin{frame}{Live Demo: Your First Roofline Analysis}
\note{
% -- NARRATE: Run this live on screen. Show the output.
% Emphasize: 3 lines of Python, no configuration files, no YAML.
% Point out the bottleneck field and MFU.
% -- FLEX: [CORE]
}
...
prof_a100 = mlsysim.Engine.solve(model, a100, batch_size=1)
prof_h100 = mlsysim.Engine.solve(model, h100, batch_size=1)
...
\vspace{0.2cm}
\begin{exampleblock}{What to look for in the output}
  \begin{itemize}
    \item \texttt{bottleneck}: Memory for both
    \item \texttt{mfu}: very low (the GPU is starved)
    \item \texttt{latency}: H100 $\approx$ 1.7$\times$ faster (BW ratio, not FLOPS ratio)
  \end{itemize}
\end{exampleblock}
\end{frame}
...
\note{
% -- LINK: Engine.solve reports MFU. What does it mean?
% -- NARRATE: ``MFU measures the gap between what the hardware could do
...
\note{
% -- LINK: We have seen compute (Wall 1), memory (Wall 2), MFU (Wall 3).
% Now we put them all together.
% -- NARRATE: Walk through each denominator term. Point out that every
% wall in the 22-wall taxonomy maps to exactly one term.
% -- ENGAGE: ``Which term do you think is hardest to improve?''
% -- FLEX: [CORE]
}
...
\note{
% -- LINK: We said AI determines the regime. What changes AI?
% -- NARRATE: ``Batch size is the primary knob. Each additional sample
...
\begin{frame}{Live Demo: Finding the Ridge Point}
\note{
% -- NARRATE: Run this loop live. Show how the bottleneck flips
% from Memory to Compute as batch size increases.
% -- ENGAGE: Before running, ask: ``At what batch size do you
...
for bs in [1, 4, 16, 64, 128, 256]:
    p = mlsysim.Engine.solve(model, hw, batch_size=bs)
    print(f"bs={bs:>3d}  {p.bottleneck:<8s}  "
          f"MFU={p.mfu:.3f}  lat={p.latency:~P}")
\end{lstlisting}
...
\begin{frame}{Exercise 1: Find the Crossover}
\note{
% -- ENGAGE: [5 minutes] Attendees work individually.
% Question: At what batch size does Llama-3 8B on H100 transition
% from memory-bound to compute-bound?
% Expected answer: around bs=32--64 depending on precision.
% Have attendees share their answers.
...
for bs in range(1, 129):
    ...
    if p.bottleneck == "Compute":
        print(f"Crossover at batch size {bs}"); break
\end{lstlisting}
...
\note{
% -- LINK: The exercise found the crossover. Now explain why.
% -- NARRATE: ``The ridge point is a property of the hardware,
...
\begin{tabular}{lccc}
\toprule
\textbf{Hardware} & \textbf{Peak FP16} & \textbf{HBM BW} & \textbf{Ridge} \\
\midrule
A100 SXM  & 312 TFLOPS  & 2.0 TB/s  & 156 FLOP/B \\
H100 SXM  & 989 TFLOPS  & 3.35 TB/s & 295 FLOP/B \\
B200      & 2.25 PFLOPS & 8.0 TB/s  & 281 FLOP/B \\
\bottomrule
\end{tabular}
...
\note{
% -- ENGAGE: Predict-before-reveal moment.
% Ask: ``ResNet-50 at batch 256 vs Llama-3 8B at batch 1.
% Which is compute-bound and which is memory-bound?''
% Give 30 seconds.
% Expected: ResNet at high batch is compute-bound;
...
\note{
% -- NARRATE: Summarize in one sentence. Repeat it twice.
...
\begin{frame}{Key Question}
\note{
% -- NARRATE: ``You have seen that LLM inference at batch 1 is
...
\note{
% -- LINK: In Part 1 we treated inference as a single pass.
% LLM serving splits into two physically distinct phases.
% -- NARRATE: ``Prefill is like reading a book fast (compute-intensive).
...
\note{
% -- LINK: Decode reuses past attention states. Where do they live?
% -- NARRATE: ``Each active request carries its own memory of the
% conversation. This KV cache grows with sequence length and eats
% into the GPU's finite HBM.''
% -- ENGAGE: ``Quick math: how much KV cache does one Llama-3 8B
% request at 4K context need in FP16?''
% Expected: 2 * 32 layers * 32 heads * 128 dim * 4096 * 2 bytes
% = ~500 MB. The point: it adds up fast.
% -- FLEX: [CORE]
}
...
\begin{frame}{Live Demo: Two-Phase Serving Analysis}
\note{
% -- NARRATE: Run ServingModel live. Point out TTFT vs ITL in output.
...
result  = serving.solve(model, hw, seq_len=4096, batch_size=1)

print(f"TTFT (prefill): {result.ttft:~P}")
print(f"ITL  (decode):  {result.itl:~P}")
print(f"Prefill regime: {result.prefill_bottleneck}")
print(f"Decode regime:  {result.decode_bottleneck}")
...
\note{
% -- LINK: Static batching wastes GPU time. Continuous batching
% interleaves requests as they complete.
% -- NARRATE: ``In static batching, the GPU waits for the longest
...
\note{
% -- LINK: KV cache limits concurrency. PagedAttention fixes
% the fragmentation problem.
% -- NARRATE: ``Just like OS virtual memory pages physical RAM,
...
\note{
% -- ENGAGE: Predict-before-reveal. Give 60 seconds.
% Question: H100 80 GB, Llama-3 8B FP16, 4K context.
% How many concurrent requests with PagedAttention?
% Expected: weights = 16 GB, remaining = 64 GB.
% With PagedAttention (95\% util): ~30 requests.
% Without (50\% util): ~16 requests.
% -- FLEX: [CORE]
}
...
\begin{frame}{Speculative Decoding: Betting on the Draft}
\note{
% -- LINK: Decode is memory-bound. Can we make the GPU do useful
% compute during memory stalls?
% -- NARRATE: ``Use a small draft model to guess the next K tokens.
...
\begin{frame}{Disaggregated Serving: Right Hardware for Each Phase}
\note{
% -- LINK: Prefill is compute-bound, decode is memory-bound.
% Why use the same hardware for both?
% -- NARRATE: ``Split prefill and decode onto different node types.
...
\begin{frame}{Exercise 2: Serving Capacity Planning}
\note{
% -- ENGAGE: [5 minutes] Attendees work individually or in pairs.
% Question: How many concurrent Llama-3 8B requests (4K context)
% can an H100 serve while maintaining ITL < 10ms?
% Expected: ~30 with PagedAttention at FP16. The binding constraint
% is memory capacity (KV cache), not compute.
% -- FLEX: [CORE]
}
...
\note{
% -- NARRATE: Walk through each fallacy with the quantitative
% counter-evidence.
...
\note{
% -- NARRATE: One sentence summary, repeat twice.
% -- FLEX: [CORE]
}
...
\begin{frame}{Key Question}
\note{
% -- NARRATE: ``If we can shrink the model, we move less data,
...
\note{
% -- LINK: Part 2 showed memory is the bottleneck. Compression
% attacks the numerator of the memory wall equation.
% -- NARRATE: ``Storage always shrinks. But inference speedup
...
\note{
% -- NARRATE: Walk through the precision ladder. Emphasize that
% FP16 to INT8 is nearly lossless for most models.
% INT4 is where accuracy starts to hurt noticeably.
% -- ENGAGE: ``Before I show the numbers: if you quantize
% Llama-3 8B from FP16 to INT4, how much memory does it use?''
...
\begin{frame}{Live Demo: Quantization Impact}
\note{
% -- NARRATE: Run CompressionModel live. Show storage savings,
% speedup, and accuracy delta side by side.
% -- FLEX: [CORE]
}
...
\begin{frame}{Pruning: The Structure Matters}
\note{
% -- LINK: Quantization shrinks each weight. Pruning removes weights.
% -- NARRATE: ``Unstructured pruning zeros out individual weights.
...
\vspace{0.3cm}
\begin{lstlisting}
# Compare pruning types
for stype in ["unstructured", "structured", "n_m"]:
    r = comp.solve(model, hw, method="pruning",
            sparsity=0.5, sparsity_type=stype)
    print(f"{stype:>14s}: speedup={r.inference_speedup:.1f}x")
\end{lstlisting}
\end{frame}
...
\note{
% -- ENGAGE: Predict-before-reveal.
% Question: If you quantize Llama-3 8B to INT4 and run at bs=1
% on H100, is it still memory-bound?
% Expected: Yes! Weight size drops to 4 GB, load time drops to
% 4/3350 = 1.2 ms. Compute is still ~0.03 ms. Still memory-bound,
% but 4x faster decode.
% -- FLEX: [CORE]
}
...
\begin{frame}{Exercise 3: Compression Tradeoffs}
\note{
% -- ENGAGE: [5 minutes] Attendees explore compression options.
% Question: Compare INT8 quantization vs 50\% structured pruning
% for Llama-3 8B on H100. Which gives better speedup per accuracy
% point lost?
% Expected: INT8 gives ~2x speedup with <1\% accuracy loss.
% Structured pruning gives ~2x speedup with 2-5\% accuracy loss.
% INT8 is the better deal.
% -- FLEX: [CORE]
}
...
\begin{lstlisting}
comp = mlsysim.CompressionModel()
r1 = comp.solve(model, hw, method="quantization",
                target_bitwidth=8)
r2 = comp.solve(model, hw, method="pruning",
...
\begin{frame}{Key Question}
\note{
% -- NARRATE: ``Your model does not fit on one GPU. Or it fits but
...
\note{
% -- LINK: Part 1--3 covered single-node analysis. Now we scale out.
% -- NARRATE: ``Two reasons to go distributed: (1) the model does not
...
\note{
% -- NARRATE: ``Every distributed strategy is a combination of three
% dimensions. Data parallelism replicates the model. Tensor parallelism
% splits layers. Pipeline parallelism splits stages.''
% -- WARN: Students often confuse TP and PP. TP splits within a layer
% (activations), PP splits between layers (pipeline stages).
% -- FLEX: [CORE]
}
...
\note{
% -- LINK: DP is conceptually simple. The cost is AllReduce.
% -- NARRATE: ``After each training step, every GPU must share its
% gradients with every other GPU. The ring AllReduce algorithm
% sends 2(N-1)/N times the data. As N grows, this approaches 2x.''
% -- ENGAGE: ``Quick: 1 GB of gradients, 8 GPUs, 50 GB/s NVLink.
...
\note{
% -- LINK: When the model is too big for one GPU, TP splits it.
% -- NARRATE: ``TP splits each layer's weight matrix across GPUs.
% Every forward and backward pass requires 2 AllReduce ops per
% layer to recombine the activations. That is why TP only works
% on NVLink (within a node), not across nodes.''
% -- WARN: Students think TP splits weights like PP splits layers.
...
\note{
% -- LINK: TP needs fast interconnect. PP needs less bandwidth
% but introduces idle time (bubbles).
% -- NARRATE: ``In PP, GPU 0 processes layers 1--10, GPU 1 processes
% 11--20, etc. While GPU 0 works on microbatch 1, GPU 1 is idle.
% The bubble fraction is (P-1)/(M+P-1) where P is the number
% of pipeline stages and M is the number of microbatches.''
% -- ENGAGE: ``With 4 stages and 4 microbatches, what fraction
% of time is wasted?'' Expected: 3/7 = 43\%.
% With 4 stages and 32 microbatches: 3/35 = 8.6\%.
% Lesson: more microbatches = smaller bubble.
% -- FLEX: [CORE]
}
...
\note{
% -- LINK: Pipeline parallelism needs many microbatches to fill
% the pipeline. Gradient accumulation provides exactly this.
% -- NARRATE: ``Instead of one large batch, process K small
% microbatches and accumulate the gradients before the optimizer
% step. Effective batch size = K * microbatch size.
% This fills the pipeline and amortizes AllReduce.''
% -- FLEX: [OPTIONAL]
}
...
\note{
% -- LINK: In real clusters, GPUs within a node talk via NVLink
% (900 GB/s) and across nodes via InfiniBand (50 GB/s). A flat
% AllReduce across all GPUs is suboptimal.
% -- NARRATE: ``Hierarchical AllReduce first reduces within each
...
\begin{frame}{Live Demo: Distributed Training Analysis}
\note{
% -- NARRATE: Run DistributedModel live. Show communication overhead,
...
result = dist.solve(model, fleet, batch_size=4096,
         tp_size=8, pp_size=1, microbatch_count=32,
         seq_len=4096)

print(f"Scaling eff:    {result.scaling_efficiency:.1%}")
print(f"Bubble frac:    {result.bubble_fraction:.1%}")
...
\note{
% -- LINK: Scaling adds communication overhead. It also adds
% failure risk.
% -- NARRATE: ``If one GPU fails every 50,000 hours, 1000 GPUs
% give you a failure every 50 hours. At 10,000 GPUs, every 5 hours.
% This is why checkpointing exists.''
% -- ENGAGE: ``If you have 10,000 GPUs each with 50,000 hour MTBF,
...
\note{
% -- LINK: Communication + failures reduce the benefit of more GPUs.
% -- NARRATE: ``Scaling efficiency is the fraction of ideal speedup
% you actually achieve. It includes communication overhead, pipeline
% bubbles, straggler effects, and failure recovery costs.''
% -- FLEX: [CORE]
}
...
\note{
% -- ENGAGE: Predict-before-reveal.
% Question: You have 64 H100s. Llama-3 70B (140 GB FP16).
% What TP x PP x DP configuration do you choose?
% Give 90 seconds.
% Expected: TP=8 (within node, NVLink), PP=1 (avoid bubbles),
% DP=8 (64/8). The 140 GB model fits across 8 GPUs with TP.
% -- FLEX: [CORE]
}
...
\begin{frame}{Exercise 4: Distributed Training Design}
\note{
% -- ENGAGE: [5 minutes] Attendees work individually.
% Question: Use DistributedModel to find the best TP x PP config
% for Llama-3 70B on 64 H100s. Sweep TP in [1,2,4,8] and
% PP in [1,2,4,8] where TP*PP*DP=64.
% Expected: TP=8, PP=1, DP=8 gives highest scaling efficiency.
% -- FLEX: [CORE]
}
...
for tp in [1, 2, 4, 8]:
    for pp in [1, 2, 4, 8]:
        ...
        r = dist.solve(model, fleet, batch_size=512,
            tp_size=tp, pp_size=pp, seq_len=4096,
            microbatch_count=max(4, 64//(tp*pp)))
        print(f"TP={tp} PP={pp} DP={64//(tp*pp):>2d} "
              f" eff={r.scaling_efficiency:.1%}")
\end{lstlisting}
\end{frame}
...
\note{
% -- LINK: Scaling efficiency includes straggler effects.
% -- NARRATE: ``In synchronous training, every GPU must finish
% before the next step begins. The slowest GPU determines the
% step time. At 1000 GPUs, even 1\% variation means 10 GPUs
% are significantly slow on any given step.''
% -- FLEX: [OPTIONAL]
}
