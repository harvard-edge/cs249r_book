...
![](vertopal_ee1dac92e9a84172a09aeb26aee803be/media/image5.png){width="6.5in" height="3.7083333333333335in"}
...
![](vertopal_ee1dac92e9a84172a09aeb26aee803be/media/image3.png){width="6.5in" height="1.8055555555555556in"}
...
Example: Introduced by Baidu, is a good example of something that asseses the above. DeepBench assesses the performance of basic operations in deep learning models, providing insights into how different hardware platforms handle neural network training and inference.
...
-   MLPerf Inference: An industry-standard set of benchmarks for measuring the performance of machine learning software and hardware. MLPerf has a dedicated benchmark for TinyML, called MLPerf Tiny, which focuses on microcontrollers and other resource-constrained devices.
...
-   AI-Benchmark: A benchmarking tool designed for Android devices, it valuates the performance of AI tasks on mobile devices, encompassing various real-world scenarios like image recognition, face parsing, and optical character recognition.
...
9. Ease of Use and Flexibility: The ease with which the training process can be set up and used, as well as its flexibility in handling different types of data and models. In companies like Google, efficiency can sometimes be measured in terms of the GCUs saved or number of SWE years saved, since that translates directly to impact. Ease of use and flexibility can reduce the time and effort required to train a model. TensorFlow and PyTorch are popular machine learning frameworks that provide user-friendly interfaces and flexible APIs for building and training machine learning models. These frameworks support a wide range of model architectures and are equipped with tools that simplify the training process.
...
MLPerf is a suite of benchmarks designed to measure the performance of machine learning hardware, software, and services. The MLPerf Training benchmark focuses on the time it takes to train models to a target quality metric. It includes a diverse set of workloads, such as image classification, object detection, translation, and reinforcement learning.
...
DAWNBench is a benchmark suite that focuses on end-to-end deep learning training time and inference performance. It includes common tasks such as image classification and question answering.
...
Fathom is a benchmark from Harvard University that includes a diverse set of workloads to evaluate the performance of deep learning models. It includes common tasks such as image classification, speech recognition, and language modeling.
...
By and large, the challenges in picking representative tasks for benchmarking inference machine learning systems are somewhat of the same taxonomy as what we have provided for training (Section XXX). Nevertheless, to be pedantic, let's discuss those in the context of inference machine learning systems.

1. Diversity of Applications: Inference machine learning is employed across numerous domains such as healthcare, finance, entertainment, security, and more. Each domain has its unique tasks, and what's representative in one domain might not be in another. For example, an inference task for predicting stock prices in the financial domain might not be representative for image recognition tasks in the medical domain.
...
Google Speech Commands was selected as the best dataset to represent the task. The dataset is well established in the research community and has permissive licensing which allows it to be easily used in a benchmark.
...
Resource: [[Benchmarking TinyML Systems: Challenges and Directions]{.underline}](https://arxiv.org/pdf/2003.04821.pdf)

![](vertopal_ee1dac92e9a84172a09aeb26aee803be/media/image6.png){width="6.5in" height="6.027777777777778in"}
...
![](vertopal_ee1dac92e9a84172a09aeb26aee803be/media/image2.png){width="5.802083333333333in" height="3.40625in"}
...
**Multimodal Datasets:** These datasets contain multiple types of data, such as text, images, and audio, to better represent real-world situations. An example is the VQA (Visual Question Answering) dataset, where models are tested on their ability to answer text-based questions about images.
...
**Out-of-Distribution Generalization**: Testing how well models perform on data that is different from the original training distribution. This evaluates the model's ability to generalize to new, unseen data. Example benchmarks are Wilds, RxRx, and ANC-Bench.

**Adversarial Robustness:** Evaluating model performance under adversarial attacks or perturbations to the input data. This tests the model's robustness. Example benchmarks are ImageNet-A, ImageNet-C, and CIFAR-10.1.
...
The [[Speech Commands dataset]{.underline}](https://arxiv.org/pdf/1804.03209.pdf), and its successor [[MSWC]{.underline}](https://datasets-benchmarks-proceedings.neurips.cc/paper/2021/file/fe131d7f5a6b38b23cc967316c13dae2-Paper-round2.pdf), are common benchmarks for one of the quintessential TinyML applications, keyword spotting. Speech Commands establish streaming error metrics beyond the standard top-1 classification accuracy that are more relevant to the keyword spotting use case. Use case relevant metrics are what elevates a dataset to a model benchmark.
...
![](vertopal_ee1dac92e9a84172a09aeb26aee803be/media/image4.png){width="6.5in" height="2.625in"}
...
[[DataComp]{.underline}](https://www.datacomp.ai/) is a recently-launched dataset competition which targets evaluation of large corpora. DataComp focuses on language-image pairs used to train CLIP models. The introductory whitepaper finds that, when the total compute budget for training is held constant, the best-performing CLIP models on downstream tasks such as ImageNet classification are trained on just 30% of the available training sample pool. This suggests that proper filtering of large corpora is critical to improving the accuracy of foundation models. Similarly, [[Demystifying CLIP Data (Xu et al
2023)]{.underline}](https://arxiv.org/abs/2309.16671) asks whether the success of CLIP is attributable to the architecture or the dataset.

[[DataPerf]{.underline}](https://www.dataperf.org/) is another recent effort which focuses on benchmarking data in a wide range of modalities. DataPerf provides rounds of online competition to spur improvement in datasets. The inaugural offering launched with challenges in vision, speech, acquisition, debugging, and text prompting for image generation.
...
![](vertopal_ee1dac92e9a84172a09aeb26aee803be/media/image8.png){width="2.5300885826771653in" height="2.32415135608049in"}
