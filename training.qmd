...
**RMSProp:** Divides the learning rate by an exponentially decaying average of squared gradients. This has a similar normalizing effect as Adagrad but does not accumulate the gradients over time, avoiding a rapid decay of learning rates. [@rmsprop]

**Adagrad:** An adaptive learning rate algorithm that maintains a per-parameter learning rate that is scaled down proportionate to the historical sum of gradients on each parameter. This helps eliminate the need to manually tune learning rates. [@adagrad]

**Adadelta:** A modification to Adagrad which restricts the window of accumulated past gradients thus reducing the aggressive decay of learning rates. [@adelta]

**Adam:** - Combination of momentum and rmsprop where rmsprop modifies the learning rate based on average of recent magnitudes of gradients. Displays very fast initial progress and automatically tunes step sizes. [@adam]
...
* **Bayesian Optimization:** An advanced probabilistic approach for adaptive exploration based on a surrogate function to model performance over iterations. It is very sample efficient - finds highly optimized hyperparameters in fewer evaluation steps. Requires more investment in setup. [@bayes_hyperparam]
...
* **Neural Architecture Search:** An approach to designing well-performing architectures for neural networks. Traditionally, NAS approaches use some form of reinforcement learning to propose neural network architectures which are then repeatedly evaluated. [@nas]
...
I have removed all the dash symbols from the table formatting to clean it up. Let me know if you need any other changes to the table or have additional pros/cons to cover for the various activation functions!
...
In training neural networks, we need to repeatedly estimate the gradient of the loss function with respect to the network parameters (i.e. weights and biases). This gradient indicates which direction the parameters should be updated in order to minimize the loss.

The most straightforward approach is to estimate the gradient based on a single training example, compute the parameter update, lather, rinse, and repeat for the next example. However, this involves very small and frequent parameter updates that can be inefficient.

Instead, mini-batch gradient descent strikes a balance between stability and efficiency. Rather than compute the gradient on single examples, we estimate the gradient based on small "mini-batches" of data - usually between 8 to 256 examples.
...
By tuning the mini-batch size, we can control the tradeoff between the smoothness of the estimate (larger batches are better) and the frequency of updates (smaller batches allow more regular updates). Mini-batch sizes are usually powers of 2 so they can leverage parallelism across GPU cores efficiently.

In summary, mini-batch gradient descent refers to updating the neural network weights based on the gradient estimated from small mini-batches rather than individual examples. This strikes a balance between estimate accuracy and computational efficiency.
:::
...
As a result, techniques that enhance the efficiency of general dense/sparse matrix-matrix and matrix-vector operations would directly improve overall efficiency. Some key methods include:
...
* Employing specialized hardware like Tensor Processing Units with hardware matrix multiplication
