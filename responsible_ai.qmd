...
height="3.5951629483814522in"}{#fig-diffusion-model-example}

*Figure @fig-diffusion-model-example: Diffusion models memorize individual training examples and
generate them at test time (@carlini2023extracting).*
...
machine unlearning techniques (@bourtoule2021machine) for remote embedded ML systems fail
...
perturbations are made to their inputs (@szegedy2013intriguing). This surprising fragility highlights
...
deep neural networks (@szegedy2013intriguing). This
...
fragility to tiny input perturbations (@szegedy2013intriguing).
...
height="1.28125in"}{#fig-adversarial-example}
...
tasks like NSFW detection (@bhagoji2018practical),
ad-blocking (@tramer2019adversarial), and speech recognition (@carlini2016hidden).
...
obfuscation with robust optimization (@gu2014towards, @papernot2017practical, @ross2018improving, @athalye2018obfuscated).
...
around inputs (@wong2018provable). Effectively, this trains models to resist common perturbation tactics.
...
models is much slower, and accuracy on clean test data often decreases (@muhammad2022survey). 
...
artifacts that deviation from natural image properties (@carlini2017adversarial).
...
had not occurred, Y would not have occurred" (@wachter2017counterfactual).
...
explanations because they "resemble human reasoning and explanations" (@ramaswamy2023ufo).
Popular concept based explanation methods include TCAV (@kim2018interpretability), Network Dissection (@bau2017network), 
and interpretable basis decomposition (@zhou2018interpretable).
...
understandability to humans (@ramaswamy2023overlooked).
...
transparency is imperative (@rudin2019stop).
...
structural knowledge of the domain, such as monotonicity (@gupta2016monotonic),
causality, or additivity (@lou2013accurate, @beck1998beyond).
...
prediction. Examples of this include Concept Bottleneck Models (@koh2020concept),
which predict a concept set c that is passed into a linear classifier,
and ProtoPNets (@chen2019looks),
...
interpretability views models as a computational graph (@geiger2021causal)
and circuits are subgraphs with distinct functionality (@wang2022interpretability).
...
visualizations produced by circuits (@olah2020zoom). 
Alternative approaches build sparse autoencoders that encourage neurons
to encode disentangled interpretable features (@bricken2023towards).
...
tools (@kaur2020interpreting) and that a user's trust in a model due to an explanation can be
independent of the correctness of the explanations (@lakkaraju2020fool).
...
-   **Subgroup imbalance.** This is likely what comes to mind when
    ...
    > technology (@buolamwini2018genderShades);
    ...
    > likelihood that a child will eventually be removed from a home (@alleghenycounty2018).
    > The AFST produces [disproportionate scores for different subgroups](https://www.aclu.org/the-devil-is-in-the-details-interrogating-values-embedded-in-the-allegheny-family-screening-tool#4-2-the-more-data-the-better),
...
-   *Data collection can be harmful.* This is the *paradox of exposure*,
    ...
    > the collection process (@d2023dataFeminism, Chapter 4). For example,
...
-   *The data collection process itself can be biased.* For example,
    ...
    > disproportionately used on black patients (@himmelstein2022examination).
...
effective tools for data exploration. Finally,
...
machine is the purpose which we really desire" (@wiener1960some).
...
pursuing unintended or undesirable goals, has become a source of concern (@russell2021human).
Within the field of AI safety, a particular goal concerns "value alignment," or the
...
source of evidence, namely, all of the human choices ever made." (@russell2021human, pg. 8)*

The absence of this alignment can lead to a number of AI safety issues,
as have been documented in a variety of [deep learning models](https://deepmind.google/discover/blog/specification-gaming-the-flip-side-of-ai-ingenuity/).
...
from human feedback (RLHF) (@christiano2017deep).
Ngo 2022 (@ngo2022alignment) argue that
...
distribution, and power-seeking strategies. Similarly, Amodei et al. 2016 (@amodei2016concrete)
...
citing ["unreasonable risks to public safety"](https://www.cnbc.com/2023/10/24/california-dmv-suspends-cruises-self-driving-car-permits.html).
One such [accident](https://www.google.com/url?q=https://www.cnbc.com/2023/10/17/cruise-under-nhtsa-probe-into-autonomous-driving-pedestrian-injuries.html&sa=D&source=docs&ust=1701805725902865&usg=AOvVaw2-S0v9p1_riCDeH36fEjKa) occurred when a vehicle struck a pedestrian who
...
to accurately classify her moving body as an object to be avoided](https://www.bbc.com/news/technology-54175359).
...
drones are already [reshaping warfare](https://www.reuters.com/technology/human-machine-teams-driven-by-ai-are-about-reshape-warfare-2023-09-08/).
While such incidents bring up important ethical questions regarding [who
should be held responsible](https://www.cigionline.org/articles/who-responsible-when-autonomous-systems-fail/) when these systems fail,
...
on machine autonomy (@calvo2020supporting).
...
action to succeed (@mccarthy1981epistemological).
These logical limitations have given rise to mathematical approaches
such as Responsibility-sensitive safety (RSS) (@shalev2017formal),
which is aimed at breaking down the end goal of an automated driving system
...
control. Value-sensitive design (@friedman1996value)
...
to the design systems that promote wellbeing and human flourishing (@peters2018designing).
...
for either independence or being in control (@ryan2000self). Calvo 2020
elaborates on METUX and its six "spheres of technology experience" in
the context of AI-recommender systems (@calvo2020supporting). They propose these spheres --
...
*displacement* rather than replacement (@shneiderman2022human, Chapter 4).
...
automation"](https://hbr.org/2023/03/a-smarter-strategy-for-using-robots), in which process flexibility is sacrificed for increased
productivity.
...
metrics for measuring success (see MIT's [Work of the Future](https://workofthefuture-mit-edu.ezp-prod1.hul.harvard.edu/wp-content/uploads/2021/01/2020-Final-Report4.pdf)).
...
result in higher minimum wages and benefits. 
...
than people, unlimited, fast, mysterious, and frightening" (@martin1993myth).
...
magic - "understandable only in terms of what it did, not how it worked" (@handlin1965science).
...
"science communication" and "artificial intelligence" (@schafer2023notorious).
...
identify potential gaps in AI literacy among adults (@lindgren2023handbook).
...
AI as a tool online, at home, and in the workplace" (@long2020ai) will be an important tool in helping people whose skills are rendered
obsolete by AI automation (@ng2021ai).
