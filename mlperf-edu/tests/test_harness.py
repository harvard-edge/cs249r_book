...
def test_offline_harness_batches_samples_deterministically():
    ...
    result = run_harness(ScenarioConfig(scenario="offline", sample_count=5, batch_size=2), handler)
...
def test_single_stream_harness_dispatches_one_sample_at_a_time():
    ...
    result = run_harness(ScenarioConfig(scenario="SingleStream", sample_count=4, batch_size=4), handler)
...
def test_server_harness_latency_includes_queueing_delay():
    ...
    result = run_harness(ScenarioConfig(scenario="server", sample_count=4, target_qps=100000.0, seed=1), handler)
