...
default is the four-workload starter path, `max` covers all seven workloads,
and `pro` selects the four-workload research collection. Use `--collection all`
to run every workload at `min` or `pro`. The `coverage` validation preset
runs all seven `min` paths. `--suite` narrows to one domain, `--workload`
selects one benchmark, and `--mode` or `--phase` selects an execution boundary
when the workload exposes one.
