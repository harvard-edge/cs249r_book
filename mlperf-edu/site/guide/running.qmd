...
`min` path and twelve current `max` paths run on local hardware. DLRM and MiniGo
still require their legacy research environments. They remain first-milestone
blockers until the planned local backends pass parity and quality checks.

## Blocked Local Max Runs

DLRM and MiniGo currently fail closed on an ordinary classroom laptop, but their
legacy setup is machine-readable. Ask `doctor` for the relevant handoff when
reviewing the existing reference path.
...
command. A handoff documents the reference path. It is not a quality result and
does not satisfy the local-execution milestone.
