...
`min` path and thirteen current `max` paths run on local hardware. MiniGo still
requires its legacy research environment and remains a first-milestone blocker
until a local backend passes parity and quality checks.
