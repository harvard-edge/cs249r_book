...
def load_registry(path: str | Path | None = None, *, validate: bool = True) -> dict[str, Workload]:
    ...
    return normalize_registry_data(data, registry_name=str(registry_path), validate=validate)
...
def load_registry_directory(registry_dir: Path) -> dict[str, Any]:
    ...
    if (registry_dir / "workloads.yaml").is_file() and not (registry_dir / "suites").is_dir():
        with (registry_dir / "workloads.yaml").open("r") as f:
            return yaml.safe_load(f)

    suites_dir = registry_dir / "suites"
    if not suites_dir.is_dir():
        raise ValueError(f"registry directory '{registry_dir}' must contain suites/ or workloads.yaml")
...
def load_native_workload_directory(workload_dir: Path) -> dict[str, Any]:
    ...
    variant_order = [str(item) for item in base.get("variant_order", [])] if isinstance(base.get("variant_order"), list) else []
    variant_paths = sorted(variants_dir.glob("*.y*ml"), key=lambda path: variant_sort_key(path, variant_order))
...
def strip_registry_identity(raw: dict[str, Any]) -> dict[str, Any]:
    return {key: value for key, value in raw.items() if key not in {"id", "variants", "variant_order"}}
...
def infer_product_suite(workload_id: str, source_suite: str) -> str:
    ...
    if (
        ...
        return "language"
    return SUITE_ALIASES.get(source_suite, source_suite if source_suite in PRODUCT_SUITES else "language")
...
def validate_registry(workloads: dict[str, Workload]) -> None:
    ...
    if missing_starter:
        raise ValueError(f"starter workload ids missing from workloads.yaml: {missing_starter}")

    missing_standard = sorted(STANDARD_WORKLOADS.difference(workloads))
    if missing_standard:
        raise ValueError(f"standard workload ids missing from workloads.yaml: {missing_standard}")

    missing_research = sorted(RESEARCH_WORKLOADS.difference(workloads))
    if missing_research:
        raise ValueError(f"research workload ids missing from workloads.yaml: {missing_research}")

    invalid_suites = sorted({w.suite for w in workloads.values() if w.suite not in PRODUCT_SUITES})
    if invalid_suites:
        raise ValueError(f"invalid product suites: {invalid_suites}")

    invalid_scenarios = sorted(
        {str(w.scenario) for w in workloads.values() if not w.scenario or w.scenario not in EDU_SCENARIOS}
    )
    if invalid_scenarios:
        raise ValueError(f"invalid EDU scenarios: {invalid_scenarios}")

    invalid_public_statuses = sorted(
        {w.public_status for w in workloads.values() if w.public_status not in PUBLIC_STATUSES}
    )
    ...
    for workload in workloads.values():
        if workload.variant and not workload.canonical_workload:
            canonical_metadata_issues.append(f"{workload.id}:variant without canonical_workload")
        if workload.default_variant and not workload.canonical_workload:
            canonical_metadata_issues.append(f"{workload.id}:default_variant without canonical_workload")
        if workload.canonical_workload and not workload.variant:
            canonical_metadata_issues.append(f"{workload.id}:canonical_workload without variant")
        if workload.canonical_workload:
            ...
            if workload.variant in seen:
                canonical_metadata_issues.append(f"{workload.id}:duplicate variant {workload.variant}")
            seen.add(str(workload.variant))
            if workload.default_variant:
                default_variants_by_canonical.setdefault(workload.canonical_workload, []).append(workload.id)
    duplicate_defaults = {
        canonical: ids for canonical, ids in default_variants_by_canonical.items() if len(ids) > 1
    }
    if duplicate_defaults:
        canonical_metadata_issues.extend(
            f"{canonical}:multiple default variants {ids}" for canonical, ids in sorted(duplicate_defaults.items())
        )
    if canonical_metadata_issues:
        raise ValueError(f"invalid canonical workload metadata: {canonical_metadata_issues}")

    missing_public_rationale = sorted(w.id for w in workloads.values() if not w.public_rationale.strip())
    if missing_public_rationale:
        raise ValueError(f"public rationale missing for workloads: {missing_public_rationale}")
    ...
    if missing_runner_profiles:
        raise ValueError(f"runner profiles missing from workloads.yaml: {missing_runner_profiles}")
    if invalid_runner_specs:
        raise ValueError(f"invalid runner specs in workloads.yaml: {invalid_runner_specs}")
...
def public_contract_issues(workload: Workload) -> list[str]:
    ...
    elif workload.public_status in {"score-bearing", "performance-bearing"} and workload.scenario not in PUBLIC_RESULT_SCENARIOS:
        issues.append(
            "score-bearing and performance-bearing workloads must use single_stream, offline, or server scenarios"
        )
    ...
    if workload.public_status == "score-bearing":
        ...
        if workload.quality_direction not in {"higher", "lower"}:
            issues.append("score-bearing workload must declare quality_target.direction as higher or lower")
        ...
        if not workload.quality_reference_runs:
            issues.append("score-bearing workload must declare quality_target.reference_runs")
        elif workload.quality_target_basis == "reference_runs":
            try:
                if int(workload.quality_reference_runs) < 3:
                    issues.append("score-bearing reference-run targets require at least 3 reference runs")
            except (TypeError, ValueError):
                issues.append("score-bearing quality_target.reference_runs must be an integer")
        variance = workload.quality_variance_summary
        if not isinstance(variance, dict) or not variance:
            issues.append("score-bearing workload must declare quality_target.variance_summary")
        else:
        ...
        if workload.quality_target_basis == "reference_runs":
            ...
            if not isinstance(protocol, dict) or not protocol:
                issues.append("reference-run score-bearing workload must declare quality_target.reference_protocol")
            else:
                for key in REFERENCE_PROTOCOL_FIELDS:
                    if not protocol.get(key):
                        issues.append(f"quality_target.reference_protocol must declare {key}")
                seeds = protocol.get("seeds")
                if not isinstance(seeds, list) or len(seeds) != int(workload.quality_reference_runs or 0):
                    issues.append("quality_target.reference_protocol.seeds must list every reference run seed")
        if not workload.quality_reviewer_notes:
            issues.append("score-bearing workload must declare quality_target.reviewer_notes")
        baseline = verified_baseline_value(workload)
        if baseline is None:
            issues.append("score-bearing verified_baseline must include the quality metric or a known alias")
        elif workload.quality_direction in {"higher", "lower"} and workload.quality_value is not None:
            try:
                ...
                issues.append("quality target and verified_baseline values must be numeric")

    if workload.public_status == "performance-bearing":
        if not any(workload.raw.get(key) for key in ("model_source", "shared_checkpoint", "dataset")):
            issues.append("performance-bearing workload must declare a model_source, shared_checkpoint, or dataset")
        ...
        if workload.dataset and not workload.raw.get("dataset_source"):
            issues.append("performance-bearing workload with dataset must declare dataset_source")
        if workload.dataset and not has_asset_dossier(workload.dataset):
            issues.append("performance-bearing dataset must have a structured asset dossier")
        if workload.raw.get("shared_checkpoint") and not workload.raw.get("quality_dependency"):
            issues.append("performance-bearing shared_checkpoint workload must declare quality_dependency")

    return issues


QUALITY_BASELINE_ALIASES = {
    "accuracy": ("accuracy", "val_accuracy", "test_accuracy", "top1_accuracy", "binary_accuracy"),
    ...
    "reconstruction_mse": ("reconstruction_mse", "final_reconstruction_mse", "train_loss", "val_loss"),
}
...
def verified_baseline_value(workload: Workload) -> Any:
    ...
    if not isinstance(baseline, dict) or not workload.quality_metric:
        return None
    for key in QUALITY_BASELINE_ALIASES.get(workload.quality_metric, (workload.quality_metric,)):
        if key in baseline:
            return baseline[key]
...
def public_contract_report(workloads: dict[str, Workload]) -> dict[str, list[str]]:
    ...
    return {workload.id: public_contract_issues(workload) for workload in workloads.values()}
...
def select_workloads(
    ...
    *,
    ...
    if collection == "starter":
        collection_order = {workload_id: idx for idx, workload_id in enumerate(STARTER_WORKLOAD_ORDER)}
        selected = [workloads[workload_id] for workload_id in STARTER_WORKLOAD_ORDER]
    elif collection == "standard":
        collection_order = {workload_id: idx for idx, workload_id in enumerate(STANDARD_WORKLOAD_ORDER)}
        selected = [workloads[workload_id] for workload_id in STANDARD_WORKLOAD_ORDER]
    elif collection == "research":
        collection_order = {workload_id: idx for idx, workload_id in enumerate(RESEARCH_WORKLOAD_ORDER)}
    ...
    if collection_order:
        return sorted(selected, key=lambda w: collection_order.get(w.id, len(collection_order)))

    order = {workload_id: idx for idx, workload_id in enumerate(STANDARD_WORKLOAD_ORDER)}
