...
MLPerf EDU organizes 16 workloads across 4 divisions (\Cref{tab:workloads}). We distinguish a \textbf{Canonical Core} of 5 workloads from an \textbf{Extension Suite} of 11 workloads. Each workload is an \emph{instantiation} of the evaluation tuple (Definition~1): the table columns directly correspond to $\mathcal{S}$ components (Model, Data, Scenario) and $\mathcal{M}$ metrics (Accuracy/Target, Time).
...
\begin{table*}[t]
\centering
\caption{MLPerf EDU workload suite. $\bigstar$ = Canonical Core (5 workloads spanning distinct positions on the three-axis regime grid; see §\ref{sec:suite}). All models are pure PyTorch \texttt{nn.Module}. Training times measured on Apple Silicon hardware (auto-detected; canonical reference is M1 base, measurements in this work taken on M5 Max per \texttt{bench/measure\_peaks.py}).}
\label{tab:workloads}
\footnotesize
...
\midrule
Cloud & Language & NanoGPT$\;\bigstar$ & GPT-3 & 11.1M & TinyShakes. (char) & Loss $<$ 2.3 & 89s & $O(N^2)$ attention, KV-cache \\
