...
in this paper comes from one machine (\Cref{sec:characterization}), and the floor of the
...
The suite structures execution profile comparisons around Data (D), Algorithm (A), and Machine (M) ablations (the DAM Taxonomy). We motivate all three dimensions through a data-centric AI and systems optimization lens. \Cref{fig:quality} shows that \ExecutedContractsPassing{} of the \ExecutedContracts{} contracts meet their inherited targets, while \ExecutedContractsMissing{} shortfalls are recorded under Fail-Closed admission. The benchmark harness continuously captures system telemetry, recording peak memory footprint (MB) and compute execution time (s/ms) alongside quality scores.

\subsection{Data Lens Ablations}
The Data (D) lens emphasizes dataset quality and optimization. Ablations explore sample budget pruning (from 100\% down to 10\%) to measure time-to-accuracy scaling under semantic target gates, demonstrating how data constraints impact execution efficiency.

\subsection{Algorithm Lens Ablations}
The Algorithm (A) lens addresses network compression and architecture trade-offs. Variations evaluate algorithmic optimizations such as FP32 versus FP16 and INT8 dynamic quantization precision sweeps, detailing memory footprint (MB), latency (ms), bandwidth (GB/s), and Fail-Closed verdicts. These optimizations are permitted only if they satisfy the Fail-Closed Admission Rule.

\subsection{Machine Lens Ablations}
The Machine (M) lens focuses on hardware characterization. System hardware scaling, CPU versus MPS GPU execution, kernel dispatch overhead, operational intensity, and UMA memory bus contention are evaluated across all 14 workloads.

\subsection{Systems and Educational Utility}
Grounding ML systems education~\citep{reddi2020mlperf,rush2021} in quality-gated benchmarking shifts instruction from building operators to evaluating end-to-end systems. Researchers and students translate the DAM lenses into inspectable teaching workflows and structured hands-on profiling labs. Following \citet{stodden2014}, the suite integrates versioned benchmark releases, provenance verification, and course lab grading into a unified workflow. The cryptographic provenance manifest secures the lineage of every submitted run for reproducible grading via \texttt{.provd.json} manifests and HTML dashboards. These reports record peak memory RSS, device VRAM usage, FLOP counts, memory bandwidth, and timing variance. Execution reports automatically generate visual plots (quality vs target ratios, execution timing distributions, training curves, Roofline model, CPU vs GPU backend comparisons) so results are scannable and easy to interpret. A student submission carries its hardware configuration, asset hashes, and metric evaluations, so instructors can verify execution integrity without re-running the workload.
