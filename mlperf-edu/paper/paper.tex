...
\definecolor{brandlight}{RGB}{239,246,255}
\definecolor{brandaccent}{RGB}{99,102,241}
\definecolor{brandgreen}{RGB}{16,185,129}
...
\begin{tikzpicture}[
  ...
  box/.style={
    draw=slateborder,
    rounded corners=3pt,
    minimum width=0.96\columnwidth,
    minimum height=10mm,
    align=center,
  ...
  arrow/.style={->,>=stealth,thick,draw=brandblue}
]

% Block 1: Industrial Foundation
\node[box, fill=brandlight, draw=brandblue] (upstream) {
  \textbf{Industrial MLPerf / MLCommons Standards}\\
  \textcolor{slateborder!80!black}{\tiny Non-Negotiable Quality Targets, Datasets, \& Evaluation Protocols}
};

% Block 2: MLPerf EDU Single-Laptop Suite
\node[box, below=of upstream, fill=white, draw=brandaccent] (edu) {
  \textbf{MLPerf EDU Single-Laptop Engine}\\
  \tiny Zero-Discretion Target Inheritance $\,\cdot\,$ Fail-Closed Verification\\
  \tiny Execution Profiles: \texttt{min} (30s) $\mid$ \texttt{max} (Quality Gate) $\mid$ \texttt{pro} (DAM Ablations)
};

% Block 3: Verification & Pedagogy
\node[box, below=of edu, fill=brandgreen!10, draw=brandgreen] (outputs) {
  \textbf{Verifiable Telemetry \& Cryptographic Provenance}\\
  \tiny Merkle Lineage Manifests (\texttt{.provd.json}) $\,\cdot\,$ Automated Grading\\
  \tiny Single-Node Systems Profiling (Roofline FLOPs/B, MPS vs CUDA)
};

% Connectors
\draw[arrow] (upstream) -- node[right,font=\tiny\sffamily,text=brandblue]{Inherit Targets} (edu);
\draw[arrow] (edu) -- node[right,font=\tiny\sffamily,text=brandgreen!80!black]{Fail-Closed Audit} (outputs);

\end{tikzpicture}
\caption{\textbf{MLPerf EDU system architecture.} Industrial MLCommons targets and evaluation protocols are inherited without local discretion, executing on single-laptop hardware under a fail-closed verification engine.}
