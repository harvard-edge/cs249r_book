...
\end{aligned}
\]
where $G_{\mathrm{sem}}$ checks functional correctness, $G_{\mathrm{protocol}}$ verifies measurement rules, $G_{\mathrm{provenance}}$ audits asset integrity, and $G_{\mathrm{state}}$ confirms host stability.

This paper details what MLPerf EDU does and its empirical findings. The suite executes 14 workloads across 8 MLCommons domains using three execution profiles (\texttt{min}, \texttt{max}, and \texttt{pro}). We evaluate 112 DAM Taxonomy ablation runs (Data, Algorithm, Machine), achieving 100\% target conformance on single-node hardware. \Cref{fig:architecture} summarizes the system architecture. The rest of the paper is organized as follows. \Cref{sec:related} positions the work against existing benchmark surfaces. \Cref{sec:design} details the system design. \Cref{sec:suite} defines the workload portfolio. \Cref{sec:profiles} explains the execution profiles. \Cref{sec:results} reports the measured evidence, \Cref{sec:education} discusses the classroom progression, and \Cref{sec:limitations} states the limitations.
