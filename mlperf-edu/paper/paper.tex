...
Evaluating optimizer selection (AdamW~\citep{loshchilov2019adamw}, SGD with momentum, or Lion~\citep{chen2023lion}) and model depth/width scaling quantifies convergence rates and optimizer memory overhead. While AdamW maintains 2 momentum/variance states per parameter ($8\text{ bytes/param}$ state overhead), Lion (EvoLved Sign Momentum) reduces optimizer memory state by 50\% ($4\text{ bytes/param}$), enabling higher training batch sizes within sub-gigabyte laptop memory boundaries while satisfying target loss gates.
