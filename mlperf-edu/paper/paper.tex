...
Evaluating hot-swappable data augmentations (RandAugment~\citep{cubuk2020randaugment}, CutMix~\citep{yun2019cutmix}, MixUp~\citep{zhang2018mixup}) shows that data pipeline prefetching and input transformation pipelines enhance sample efficiency, enabling models to reach quality gates under reduced sample budgets.
...
Drawing from AlgoPerf~\citep{dahl2023algoperf}, the Algorithm (A) lens evaluates precision quantization, optimizer selection, and model architecture scaling under Fail-Closed admission rules.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_algo_lens.pdf}
\caption{\textbf{Algorithm Lens precision quantization ablations.} Quantizing model weights from FP32 to FP16 and INT8 achieves $2\times$--$4\times$ weight memory reduction and $3.8\times$ inference speedup.}
\label{fig:algo_lens}
\end{figure}

\subsection{Precision Quantization (FP32 vs FP16 vs INT8)}
Evaluating model quantization across FP32, FP16 mixed precision, and INT8 dynamic quantization reveals dramatic memory footprint and latency reductions (\Cref{fig:algo_lens}). Quantizing to INT8 reduces weight memory footprints from 440 MB to 110 MB ($4.0\times$ reduction) and cuts inference latency from 5.63 ms to 1.48 ms ($3.8\times$ speedup). However, aggressive INT8 quantization can degrade task quality below inherited thresholds, triggering Fail-Closed admission rejection when accuracy loss exceeds target tolerances.

\subsection{Optimizer Selection and Model Scaling}
Evaluating optimizer selection (AdamW~\citep{loshchilov2019adamw}, SGD with momentum, or Lion~\citep{chen2023lion}) and model depth/width scaling quantifies convergence rates and memory bandwidth demand. AdamW provides stable convergence on Transformer workloads, while Lion reduces optimizer memory state footprint by 50\%, improving memory bandwidth efficiency during single-node training.
