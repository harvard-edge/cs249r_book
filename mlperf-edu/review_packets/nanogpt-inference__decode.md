...
| Measurement protocol | primary_metric=output_tokens_per_sec; warmup_runs=3; measured_runs=20; decode_steps_per_request=64; latency_statistics=median, p90, p99; raw_sample_metrics=request-level TTFT, per-token ITL; timing_scope=synchronized checkpoint-backed requests with a separate untimed warmup request |
