...
from mlperf.reference.cloud.nanogpt_train import NanoGPTWhiteBox  # noqa: E402
from mlperf.reference.cloud.nanogpt_prefill import NanoGPTPrefill  # noqa: E402
from mlperf.reference.cloud.nanogpt_decode import NanoGPTDecode  # noqa: E402
...
def main():
    ...
    print(
        f"KV bytes/token: {kv_bytes_per_token:,} ({kv_bytes_per_token / 1024:.1f} KB)"
    )
    print(
        f"At prefill_ctx={PREFILL_CTX}: KV stream/step = "
    ...
    print(
        f"  peak act:   {prefill_result['peak_activation_bytes'] / 1024 / 1024:7.2f} MB (estimate)"
    )
    print()
    ...
    decode_flops_per_step = (
        2 * n_params + 4 * cfg["n_layer"] * cfg["n_head"] * head_dim * PREFILL_CTX
    )
    ...
    print(f"  Request TTFT:   {decode_result['request_ttft_s'] * 1000:7.2f} ms")
    print(f"  First decode:   {decode_result['first_decode_latency_s'] * 1000:7.2f} ms")
    ...
    prefill_tok_per_decode_step = (
        decode_result["itl_median_s"] * prefill_result["prefill_tokens_per_sec"]
    )
    print(
        f"Visceral: 1 decode step = {prefill_tok_per_decode_step:.0f} tokens'-worth "
    ...
    if ratio >= 5.0:
        print(
            f"PASS: intensity ratio {ratio:.1f}x (>= 5x). Compute/BW split confirmed."
        )
        return 0
    else:
        print(f"FAIL: intensity ratio only {ratio:.1f}x; expected >= 5x.")
        print(
            "      Try: bump prefill_ctx (currently {}) toward 3584.".format(
                PREFILL_CTX
            )
        )
        return 1
