...
The two inner loops are not sequential work in the real chip; they are the $M \times N$ cells all acting in the same clock tick. Only the outer loop over `t` is time. Reading it that way, the listing says exactly what the hardware does. At each tick, every cell that has an operand pair multiplies and accumulates, and the whole product is done after $M + N + K - 2$ ticks. Notice that `A[i][k]` and `B[k][j]` are indexed but never copied; in the chip those reads are the values arriving from the neighboring cell, which is why each element of $A$ and $B$ leaves memory exactly once. Scale the grid from $2 \times 2$ to $128 \times 128$ and this is the matrix unit inside a TPU.
