...
Why a floor rather than a fourth term in a weighted sum? Because a sum lets one axis buy another. A model that is 100× faster and 20 points less accurate can outscore a careful one under almost any weighting, and the people submitting to a benchmark will find that weighting. A floor cannot be bought. Above it, memory and latency compete on their own; below it, nothing else is looked at.
...
The first script trains `DigitMLP` for ten epochs on TinyDigits, then walks through the optimization steps in order. Each step is a function that takes the model and one of Part III's classes. Profiling comes first because it establishes the "before" that every later ratio divides by.
...
Pruning does run the pruned model, on a copy so the original survives.
...
Step $t$ processes $t$ tokens, so the loop processes $1 + 2 + \dots + 16 = 136$ token positions in total. With a cache, each step should process one new token and read the other keys and values back from storage, 16 positions in all, so the napkin expectation is a ratio near $136 / 16 = 8.5$ on the attention work alone, before the fixed costs that do not shrink. That is where the script's advertised "6 to 10×" comes from.

The cached loop in the script does not reach that far. It embeds the new token, adds its positional encoding, writes the result into the cache with `cache.update`, and stops; it never calls `cache.get` and never runs attention against the cached keys. What it times is embedding alone versus a full forward pass, so the speedup it prints is an upper bound on what a real cached attention could achieve, and a generous one. The `KVCache` class itself does what Chapter 18 said it does. The script simply stops short of using it. Exercise 3 finishes the wiring and measures the real number.

### The scorecard

The rest of the chapter uses a pure-Python scorecard that runs anywhere. It takes a baseline and a candidate, each described by three numbers, and returns the three ratios and the MLPerf-style verdict. A second function checks a candidate against the event floors of Chapter 19's benchmark module.
...
There is deliberately nothing clever here. The three ratios are three divisions, the floor is one comparison, and the event rules are a dictionary. The value of the scorecard is not in the code but in what it refuses to do: it will not combine the ratios into a single number, and it will not report a size or a speed for a candidate that has not been run.
...
Take the six weights $w = [-1.0,\ 0.0,\ 2.0,\ 0.5,\ 1.5,\ -0.5]$, the example from Module 15's own docstring. Min-max INT8 quantization first widens the range to include zero (it already does), then sets the scale to the range divided by 255 and the zero point to whatever integer maps the minimum onto $-128$.
