...
declares every listing:
    ...
    - target: _listings/06_autograd/Function.qmd
      module: 01_tensor             # or `path: milestones/.../script.py` for a
      symbol: Function              # file outside src/; a class, a function,
                                    # Class.method, or a method attached with
                                    # @method_of(Class)
...
def module_source(module: str = None, path: str = None) -> str:
    """Source text of a module in src/ (by module name) or of any file under the package (by path)."""
    if path is not None:
...
def extract(module: str, symbol: str, elide=(), keep_scaffold=False, doc="first", path: str = None) -> str:
    text = module_source(module, path)
    tree = ast.parse(text)
...
def load_manifest():
    ...
    for e in entries:
        ...
        if "module" not in e and "path" not in e:
            raise ValueError(f"manifest entry needs a module or a path: {e}")
    return entries


def render(entry) -> str:
    code = extract(entry.get("module"), entry["symbol"], entry.get("elide", ()), entry.get("keep_scaffold", False), entry.get("doc", "first"), entry.get("path"))
    origin = entry["path"] if "path" in entry else f"src/{entry['module']}/{entry['module']}.py"
    header = f"<!-- generated by tools/listings.py from {origin} :: {entry['symbol']} -->\n"
