...
In pure Python, `matrix` is not a block of 9 numbers packed neatly together in memory. It is a list of pointers pointing to three independent list objects, each containing three pointers pointing to individual Python `float` objects scattered arbitrarily across the heap.
...
1. **Massive Memory Overhead**: In standard 64-bit Python, a simple 4-byte float is wrapped inside a 24-byte `PyFloatObject`. Storing a million floats requires over 32 megabytes of RAM instead of 4 megabytes—an $8\times$ memory bloat.
2. **Cache Line Thrashing**: Modern CPUs do not read single floats from DRAM; they fetch 64-byte chunks called **cache lines**. In a contiguous array, loading one float automatically loads the next 15 floats into the L1 cache for near-instant access (1 nanosecond latency). With nested Python lists, every element access requires dereferencing an arbitrary heap address, causing repeated L1/L2 cache misses and stalling the CPU on slow DRAM (50–100 nanoseconds latency).
...
If physical memory is strictly a flat one-dimensional line of bytes, how does our framework create the illusion of multidimensional matrices, 3D image batches, and 4D attention heads?
...
3. **Strides (`strides`)**: A tuple of integers $(s_0, s_1, \dots, s_{N-1})$ defining how many elements to step across in the 1D storage buffer to advance by 1 in logical dimension $k$.
...
It enables **Zero-Copy Views**: operations that reshape, slice, or transpose a tensor in $O(1)$ constant time and $0$ allocated bytes of memory.
...
What happens when we want to add a 1D bias vector of shape $(3,)$ to a batch of 100 samples of shape $(100, 3)$?

![Broadcasting Semantics and the Stride-0 Expansion](assets/images/diagrams/01_tensor-diag-3.svg){#fig-tensor-broadcast}

In standard math, broadcasting expands the bias vector by duplicating it 100 times. In physical memory, duplicating arrays wastes megabytes of DRAM bandwidth.
...
When the compute engine steps across the batch dimension ($i=0, 1, \dots, 99$), multiplying $i \times 0 = 0$ means the read pointer always points back to the original 3 elements in memory. We get the illusion of a $100 \times 3$ matrix while storing only 3 floats in DRAM.
...
## 1.6 The Structural Bridge: The Linear Collapse Trap

We now have a fast, contiguous multidimensional tensor engine capable of executing matrix multiplications:

$$Y = X \cdot W_1$$

Suppose we build a "deep" neural network by chaining five linear matrix multiplications together:

$$Y = ((((X \cdot W_1) \cdot W_2) \cdot W_3) \cdot W_4) \cdot W_5$$

Because matrix multiplication is associative, we can group the weight matrices together:
