...
Module 02 (`src/02_activations/02_activations.py`) is the NumPy-backed implementation students build in the TinyTorch course. The listings in this chapter are a stripped-down pure-Python model of the same design, small enough to show every element-by-element step of an activation. The real module also implements `Softmax`; this book builds the softmax it needs in Chapter 4, as `log_softmax`, alongside the loss it belongs with.
:::
