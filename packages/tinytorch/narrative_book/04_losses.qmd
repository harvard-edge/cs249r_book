...
So the need is concrete. We want a loss for classification that behaves like the textbook formula for ordinary inputs and returns a finite, correct number for any input at all.
...
Module 04 (`src/04_losses/04_losses.py`) is the NumPy-backed implementation students build in the TinyTorch course. The listings in this chapter are a stripped-down pure-Python model of the same design, small enough to show every step of the max-shifted log-softmax and the losses built on it. The real module also ships a `BinaryCrossEntropyLoss` that works on probabilities rather than logits; the first exercise asks you to improve on it.
:::

The listings need only one thing from a `Tensor`, the array of numbers it holds, so they reuse the small NumPy-backed stand-in that Chapter 3 introduced in place of Chapter 1's strided `Tensor`. It is repeated here, minus the parts this chapter does not use, so the chapter runs on its own.

### A loss is one scalar in a `Tensor`

Here is where the number goes. The training loop we assemble in Chapter 8 does the same three things every step: run a batch forward, call `backward()` on the loss, let the optimizer take one step. Chapter 6's `backward()` needs one number to start from, and Chapter 7's optimizer needs one direction to push in. Hand them the 64 per-example errors of a Chapter 5 batch as a vector and neither has what it needs, because 64 errors point in 64 directions. The mean over the batch is the answer, chosen once and written into the loss, so no caller has to remember it.

The cheaper thing to return would be a bare Python float, and for this chapter a float would work. It stops working in Chapter 6, where `backward()` walks from the loss back through every operation that produced it, and a float has no place to record where it came from. So every loss returns a `Tensor` holding a single value. A base class fixes that contract, and `MSELoss` is the smallest thing that satisfies it.
...
class Loss:
    """Every loss maps (predictions, targets) to one scalar Tensor."""
...
The fix is The Idea's two moves, in that order. Subtract each row's maximum, so every exponential is at most 1 and every sum is at least 1. Then take the log of the sum and subtract it from the shifted logits, so no probability is ever formed and nothing is ever divided. It is four lines of NumPy. The helper takes a `Tensor` of logits with shape `[N, C]`, one row per example, and returns a `Tensor` of log-probabilities with the same shape.

```python
def log_softmax(x: Tensor) -> Tensor:
    """log(softmax(z)) for each row of a [N, C] tensor, without overflow."""
    logits = x.data
    # 1. One maximum per row, kept as an [N, 1] column so it broadcasts.
    max_logits = np.max(logits, axis=-1, keepdims=True)
    # 2. Shifted logits are all <= 0, so exp() never exceeds 1.0.
    shifted = logits - max_logits
    # 3. The sum is >= 1.0 because the max term contributes exp(0) = 1.
    log_sum_exp = np.log(np.sum(np.exp(shifted), axis=-1, keepdims=True))
    # 4. log p_i = (z_i - max) - log(sum). The shift happens first.
    return Tensor(shifted - log_sum_exp)
```

Notice two things. The `keepdims=True` keeps the per-row maximum as a column of shape `[N, 1]` rather than a flat vector of length `N`, so that subtracting it from the `[N, C]` logits lines up one maximum with each row; that is the broadcasting rule from Chapter 1 doing the work. And the return value is `shifted - log_sum_exp`, not `logits - (max_logits + log_sum_exp)`. Both are equal on paper; only the first is exact in float32, for the reason the trace makes visible. PyTorch ships this same function as `torch.log_softmax`, and its scalar half, $c + \log \sum_j e^{z_j - c}$, as `torch.logsumexp`.
...
class CrossEntropyLoss(Loss):
    """Cross-entropy over a batch of logits and integer class targets."""
    def forward(self, predictions: Tensor, targets: Tensor) -> Tensor:
        logits = predictions.data                   # shape [N, C]
        N, C = logits.shape
        target_idx = targets.data.astype(np.int64)  # shape [N]

        # NumPy would accept a negative index silently; refuse it here.
        if np.any((target_idx < 0) | (target_idx >= C)):
            raise ValueError(f"target index outside [0, {C - 1}]")

        log_probs = log_softmax(predictions).data   # shape [N, C]
        picked = log_probs[np.arange(N), target_idx]  # one entry per row
        return Tensor(-np.mean(picked))
```

Targets stay integers from the loader to the gather; nothing builds the one-hot matrix just to multiply most of it by zero. The range check exists because NumPy treats a negative index as counting from the end, so a corrupted label of `-1` would silently select the last class instead of failing. PyTorch calls this gather, negate, and average step `nll_loss`, and its `cross_entropy` is `nll_loss` applied to the output of `log_softmax`, the same two functions in the same order.
...
The second number is $(0.5^2 + 0.5^2 + 0.2^2) / 3 = 0.18$, which you can do in your head. The first takes a page, and it is the page that follows.

---

## A Worked Trace

Take the first row of the batch above, logits $\mathbf{z} = [2.0, 1.0, 0.1]$ with target $t = 0$, and follow it through `log_softmax` one line at a time.
...
The stable column never handles a number larger than 1002 or smaller than 0.1353, and the answer, 0.4076, is the same loss the row would have if all three logits were 998 smaller. That is the shift invariance from The Idea, now visible as arithmetic.
...
What production changes is not the math but the number of passes over memory. The TinyTorch `log_softmax` walks the logits three times, once for the maximum, once for the exponentials and their sum, and once for the subtraction, and each pass writes an `[N, C]` temporary that the next pass reads back. The operation does almost no arithmetic per element, one `exp` and a couple of subtractions, so its cost is set by how many bytes move rather than how many operations run; it is memory bound, in the sense Chapter 2 introduced. A GPU kernel therefore computes the whole thing in one pass. Each thread holds a few logits of one row in registers, the threads of a warp[^fn-warp-softmax-reduction] exchange partial maximums and partial sums among themselves without writing to memory, and the row is read from memory exactly once. PyTorch keeps a family of such kernels in `aten/src/ATen/native/cuda/SoftMax.cu`, chosen by row length.
...
Precision is the other production concern. Modern training runs most of the network in 16-bit floats to halve memory traffic, and 16-bit formats have far less headroom than float32. A float16 overflows above 65,504, which is $e^{11.09}$, so a logit of 12 is enough to break it. Frameworks handle this by refusing to run the loss in low precision. PyTorch's `autocast` lists `softmax`, `log_softmax`, and `cross_entropy` among the operations it always promotes to float32, whatever precision the rest of the model uses. The max shift is necessary but not sufficient; it needs a format wide enough to hold the exponentials it produces.
...
| Passes over the logits | Three, with `[N, C]` temporaries | One fused kernel per row; fused with the final `Linear` in some libraries |
| Precision | float32 throughout | Promoted to float32 even when the model runs in 16-bit |
...
For the practitioner, three habits follow. Pass logits to the loss and let it do the softmax; if a model has a `Softmax` as its last layer and a `CrossEntropyLoss` after it, one of them is a bug. Use the library's `log_softmax` or `logsumexp` rather than composing `log` and `softmax` by hand, because `log(softmax(x))` returns `-inf` the moment a probability underflows to zero, and it runs as two kernels instead of one. And check the loss at initialization. A freshly initialized classifier should be close to guessing uniformly, which puts its cross-entropy near $\log C$, about 2.30 for ten classes. A first loss of 2.3 says the plumbing is right; a first loss of 15 or `nan` says the logits are already too large, and this chapter says where to look.

---

## Summary

You built `MSELoss` and `CrossEntropyLoss`, and the `log_softmax` helper that makes the second one safe. Cross-entropy is the negative log of the probability the model gives the right class, and the textbook softmax that produces that probability overflows float32 as soon as a logit passes 88.7. Subtracting the row maximum before exponentiating leaves the probabilities unchanged on paper and makes every exponential at most 1 and every denominator at least 1, so nothing overflows and nothing divides by zero. The invariant to carry forward is that softmax is shift invariant, and the shift by the maximum is the one that fits in a float. Inside that fix, subtract the maximum from the target logit before subtracting the log of the sum, or precision leaks in the last digits. Production systems keep exactly this math and change how many times the logits cross memory, fusing the three passes into one kernel and promoting the computation to float32 when the model runs in 16-bit. Chapter 5 takes for granted that a batch of logits and a batch of integer targets collapse to one scalar loss, and asks where those batches come from and how to keep the model fed with them.
...
3. **Temperature**: In text generation, logits are divided by a temperature $T > 0$ before the softmax. Work out what happens to the probabilities as $T \to 0$ and as $T \to \infty$, and show that the max shift in `log_softmax` keeps the computation safe for every $T > 0$, including values small enough to push $z / T$ past the float32 limit.
