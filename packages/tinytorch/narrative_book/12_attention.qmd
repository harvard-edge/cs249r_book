...
## The Mental Model: Database Retrieval and Causal Masking
...
```python
# packages/tinytorch/src/tinytorch/attention.py
import numpy as np
...
| Step | Operation | Formula | Token 0 Output (Row 0) | Token 1 Output (Row 1) |
| :--- | :--- | :--- | :--- | :--- |
| **1. Dot Product** | Raw Scores $\mathbf{Q} \mathbf{K}^T$ | $Q_{i,:} \cdot K_{j,:}^T$ | $[1(1) + 0(1), 1(1) + 0(0)] = [1.00, 1.00]$ | $[0(1) + 2(1), 0(1) + 2(0)] = [2.00, 0.00]$ |
| **2. Variance Scaling** | Scale by $1/\sqrt{2}$ | $\mathbf{S} / 1.4142$ | $[0.7071, 0.7071]$ | $[1.4142, 0.0000]$ |
| **3. Causal Masking** | Add Mask $\mathbf{M}$ | $\mathbf{S}_{\text{scaled}} + \mathbf{M}$ | $[0.7071, -\infty]$ | $[1.4142, 0.0000]$ |
| **4. Softmax Weights** | Softmax Row-wise | $\frac{\exp(s_j)}{\sum \exp(s_k)}$ | $[1.0000, 0.0000]$ | $[\frac{4.1132}{5.1132}, \frac{1.0}{5.1132}] = [0.8044, 0.1956]$ |
| **5. Value Projection** | $\mathbf{A} \cdot \mathbf{V}$ | $\sum_j A_{ij} V_{j,:}$ | $1.0[2, 1] + 0.0[0, 3] = \mathbf{[2.0000, 1.0000]}$ | $0.8044[2, 1] + 0.1956[0, 3] = \mathbf{[1.6088, 1.3912]}$ |
