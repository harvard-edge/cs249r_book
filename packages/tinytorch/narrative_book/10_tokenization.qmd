...
### The Memory Trade-Off: Embedding Tables vs. Attention Context Windows
...
2. **Character-Level Attention Memory Footprint**: A character-level model with $V = 256$ has an embedding table of only $1\text{ MB}$, but an average English word is 5 to 6 characters long. For an input document of 2,048 words, character-level tokenization produces $S \approx 12,000$ tokens. Because self-attention memory scales as $O(S^2)$, the attention score matrix $\mathbf{Q} \mathbf{K}^T$ requires:
   $$\text{Attention Memory} = 12,000^2 \times 4\text{ bytes} \approx 576\text{ MB per attention head!}$$
   Across 32 attention heads and 32 layers, intermediate attention activations explode into **590 GB of VRAM**, crashing the GPU cluster.
...
Each BPE merge operation replaces the most frequent pair $(t_a, t_b)$ with a new symbol $t_{ab}$, reducing the total sequence length $|\mathcal{T}_{\mathcal{D}}|$ by the count of that pair $\text{Freq}(t_a, t_b)$ while slightly increasing vocabulary entropy $H(\mathcal{V})$. The optimal vocabulary size $V^*$ represents the Pareto frontier that minimizes the product of sequence length and embedding parameter memory.
...
```python
# packages/tinytorch/src/tinytorch/bpe.py
