...
On my laptop (an Apple M5 Max running NumPy 2.5 with Apple's Accelerate library), one run printed `1.50 ms` and `1.50 ms`. Eight times the FLOPs, and the clock did not move. Your numbers will differ, but the shape will not. The weight matrix is far larger than any cache, and on most machines the two calls take about the same time.
...
The profiler is a handful of short pieces: two counters, a timer, a memory probe, and a few lines that combine them into a verdict. Each piece works on any model built from the book's layers, because it only asks for what Chapter 3's `Layer` already provides: a `parameters()` list and a `forward` method. Each piece also exists for a reason the previous sections set up, and I want that reason on the page before the code every time. The code takes a minute to read; the reason is what you will carry to a real framework.
...
Two conventions are baked into that line and both matter later. Every parameter in TinyTorch is a float32, so the module multiplies by a constant `BYTES_PER_FLOAT32` of 4 rather than asking each array for its `nbytes`; that is correct for all of Parts I and II and stops being correct in Chapter 15, where a weight stored as int8 costs a quarter as much, and the honest count becomes `p.data.nbytes`. And `MB_TO_BYTES` is $1024 \times 1024$, so the module's megabyte is what a memory chip calls one; TinyGPT's 4,328,448 bytes print as 4.13 MB, not 4.33. To a first approximation this parameter memory is also the byte count $Q$ for a forward pass. Every weight has to be read from memory once per pass, and for a small-row pass the weights dwarf the activations.
...
The parameter bytes are the floor on $Q$, not all of it. A forward pass allocates activations, and for a long prompt they are not small; the 64-token prompt through TinyGPT allocates more than half again the weight bytes on the way through. A framework wants to see that number, both because it is the rest of $Q$ and because it is what decides whether a batch fits in memory. The module measures it rather than deriving it, by asking Python's allocator what the high-water mark was while `forward` ran.

{{< include _listings/14_profiling/Profiler_measure_memory.qmd >}}

`tracemalloc` records every allocation Python makes, including NumPy's, so the peak minus the baseline is the most memory the pass held at once. The dummy input is built to suit the model: a token model, recognized by its `vocab_size`, gets integer ids, and everything else gets floats. The activation estimate, twice the input's bytes, is deliberately rough; it stands for the input and one output the same size, and the peak is what the module trusts. The efficiency is the fraction of the peak that was weights and input rather than intermediate results. In the trace, a one-token decode through TinyGPT peaks at exactly its parameter memory and a 64-token prompt at 6.56 MB, so the intermediates cost 2.4 MB, which is where Chapter 17's fusion will look for savings.
...
The first rate is achieved FLOP/s, the second is the peak memory divided by the time, which is bytes per second on the assumption that everything the pass held was moved once. The `theoretical_peak_gflops` of 100 is a placeholder for a laptop-class CPU, and the efficiency it produces is only meaningful once you replace it with a number you measured, which the trace does. The verdict comes from comparing the two rates.
...
One call returns the parameter count, the FLOPs, the memory figures, the median latency, the two rates, and the verdict, which is what the trace runs on TinyGPT. The one thing missing is the number Nsight Compute leads with: the fastest time the machine could have delivered, so that the gap to the measured time says how far the implementation is from what the hardware permits. That takes the two measured ceilings, and fifteen lines hold them. This class is not in Module 14; it is the version of `_analyze_bottleneck` you build once you have measured your machine.
...
One run on the same laptop as before printed the following. Two repeats of the sweep agreed to within a few percent at every row.
...
The decode step is memory bound by any test. The prompt is the interesting row. By the module's fixed ridge of 10 it is compute bound, at 78 GFLOP/s against 4 GB/s, or 19 FLOPs per byte of peak memory. Against the ridge this machine just measured, 37, the same pass is memory bound, and the hand count's 27.4 says the same. Both verdicts are correct for the ridge they assume, which is the point of measuring one.
...
Decoding at 0.42 FLOPs per byte is ninety times left of the ridge. The bound for one decode step is $4.3\text{ MB} / 46\text{ GB/s} \approx 0.094$ ms; the arithmetic would need $1.83\text{ MFLOP} / 1.7\text{ TFLOP/s} \approx 0.001$ ms. The step measured 0.40 ms, four times its bound, and the 64-token prompt measured 1.6 ms against a bound of 0.1 ms. Neither gap is the silicon. A pass through TinyGPT builds about 140 of Chapter 6's operation objects, each wrapping one NumPy call, and each costs microseconds of Python before any bytes move; that overhead, not bandwidth, is what a NumPy framework pays, and Chapter 17 is where it gets attacked. What the roofline says survives the overhead. Every optimization in the next four chapters that works on this model works by lowering the bound's first term.
...
| Bytes | Parameter bytes plus the allocator's peak during the pass | Measured DRAM traffic, cache hits excluded |
