...
Take the `TinyGPT` that Milestone II trained, which is Module 13's NumPy `GPT` under the name the milestone gave it, at `TinyGPT(vocab_size=1000, embed_dim=128, num_heads=4, num_layers=4, max_seq_len=256)`. Its parameters live in two lookup tables, four identical blocks, a final LayerNorm, and the head, and each entry is a product of shapes plus a bias where the layer has one.
...
At four bytes per parameter that is 4,328,448 bytes, or 4.3 MB, and this is $Q$ up to the small activation terms.
...
Running `count_parameters` and `count_flops` on the model itself returns the same three numbers, and the walker finds all 25 Linear layers on the way (I checked). `profile` reports the same byte counts too, which is why the input term above is four bytes per token rather than eight. Chapter 6's `Tensor` stores token IDs as float32 like everything else. The arithmetic above is the version you can check on paper.
...
Now put TinyGPT on this roofline. Decoding at 0.42 FLOPs per byte is eighty-five times left of the ridge. The profiler's `bound_ms` for one decode step is $4.3\text{ MB} / 33\text{ GB/s} \approx 0.13$ ms; the arithmetic would need $1.83\text{ MFLOP} / 1.2\text{ TFLOP/s} \approx 0.0015$ ms. Every optimization in the next four chapters that works on this model works by lowering the first number.
