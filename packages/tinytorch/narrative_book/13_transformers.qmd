...
```python
# packages/tinytorch/src/tinytorch/transformer.py
import numpy as np
...
| Pipeline Stage | Mathematical Transformation | Incoming Tensor $\mathbf{x}_{\text{in}}$ | Output Tensor $\mathbf{x}_{\text{out}}$ | Systems Property Verified |
| :--- | :--- | :--- | :--- | :--- |
| **Input Residual State** | Initial Embedding | N/A | $[1.000, 2.000, 3.000, 4.000]$ | Baseline activation |
| **1. Pre-LN Normalization** | $\text{LN}_1(\mathbf{x})$ | $[1.000, 2.000, 3.000, 4.000]$ | $[-1.342, -0.447, +0.447, +1.342]$ | Mean $\mu=0$, Variance $\sigma^2=1$ |
| **2. Multi-Head Attention** | $\text{Attn}(\text{LN}_1(\mathbf{x}))$ | $[-1.342, -0.447, +0.447, +1.342]$ | $[+0.100, +0.200, +0.100, +0.200]$ | Context aggregation |
...
| **6. Residual Addition 2** | $\mathbf{x}_1 + \text{MLP}$ | $[1.100, 2.200, 3.100, 4.200]$ | **$[1.150, 2.300, 3.050, 4.250]$** | Final block output |
