...
::: {.callout-note title="Napkin Math: Sizing Weights for Cache Line Residency"}
Why do transformers use a $4\times$ expansion ratio ($D \to 4D \to D$)?
- In language models, token routing (attention) determines *where* to look, but MLP projection determines *what* facts to recall from parameter memory.
- For $D = 64$, the expansion layer $W_1$ has shape $(64, 256)$, requiring $16,384$ floats ($65.5\text{ KB}$). The projection layer $W_2$ has shape $(256, 64)$, requiring another $65.5\text{ KB}$.
- Together, the block's MLP weights consume $131\text{ KB}$ of physical memory---fitting comfortably within the 512 KB L2 cache of a modern CPU core or GPU Streaming Multiprocessor (SM). This allows high-throughput dense GEMM execution with zero DRAM bus stalls during inference.
:::
