...
The answer is a repeating unit called the transformer block, built from attention plus two pieces you have not written yet. Layer normalization holds every token's vector at a fixed scale no matter how deep the stack grows. A small two-layer network, the MLP, lets each token transform its own features after attention has mixed features across tokens. Wrap both in residual connections, stack $N$ copies, put embeddings underneath and a vocabulary projection on top, and the result is GPT.
...
By the end of the chapter you will have `TinyGPT`, which takes a sequence of $S$ token IDs and returns an $S \times V$ matrix of logits, one row of next-token scores for every position. Milestone 2 trains it and samples text from it.
...
The first is that attention cannot transform a token on its own. Its output for a token is a weighted average of value vectors, and a value vector is a linear projection of another token's input. The softmax weights decide *which* tokens contribute, but what comes out is still a blend of what went in. Stacking attention on attention produces more elaborate blends and never a per-token nonlinear function such as "if these features say *verb*, flip those features." Every network you built in Part I got its expressive power from a nonlinearity between linear layers; a transformer needs the same thing somewhere inside each block.
...
where $\mathcal{L}$ is the loss and $I$ is the identity matrix. The identity term means the gradient arriving at layer $l+1$ reaches layer $l$ unchanged, with the layer's own contribution added on. Across $N$ layers the gradient at the bottom is the gradient at the top plus corrections, never the gradient at the top multiplied by $N$ Jacobians. In code this needs nothing new. Chapter 6's autograd already sums the gradients of a tensor that is used on two paths, and $x_l$ is used on two paths: once into $f_l$, once straight into the addition.

A useful picture is to think of the residual path as a **stream**, a $D$-dimensional scratchpad that each token carries through the whole network. Each sub-layer reads the stream, computes a delta, and writes the delta back by addition. No sub-layer can erase what an earlier one wrote; it can only add. A 12-block GPT has 24 sub-layers reading and writing the same stream.

### Layer normalization

The stream keeps the scale from exploding, but each sub-layer still prefers inputs at a known scale, for the same reason Chapter 3 initialized weights carefully. **Layer normalization** standardizes each token's vector across its $D$ features. For one token with features $x_1, \dots, x_D$,
...
Module 13 (`src/13_transformers/13_transformers.py`) is the NumPy-backed implementation students build in the TinyTorch course. It exposes `LayerNorm`, `MLP`, `TransformerBlock`, and a `GPT` class (aliased `TinyGPT`) with `forward`, `generate`, and `parameters`. The listings in this chapter are a stripped-down pure-Python model of the same design, working on one sequence stored as nested lists, small enough to show every step of a Pre-LN transformer block and the GPT that stacks them. The real module runs whole batches with array operations, uses learned positional embeddings, and carries its own backward function for LayerNorm; the listings simplify all three.
:::

The listings continue Chapter 12's conventions. A sequence is a list of $S$ rows, each a list of $D$ floats; `matmul`, `causal_mask`, and `MultiHeadAttention` come from Chapter 12 unchanged, and weights are plain nested lists drawn from a seeded generator. What this chapter adds that Chapter 12 never needed is elementwise work. A block adds an $S \times D$ update into the stream twice, and the MLP pushes every one of its $S \times 4D$ hidden values through a nonlinearity. Three helpers cover it. `add_rows` is the residual addition, `gelu` is Chapter 2's tanh approximation for one number, and `weight` draws a matrix at the $1/\sqrt{\text{rows}}$ scale Chapter 3 chose so that a fresh layer neither inflates nor shrinks what passes through it.

```python
import math, random

def add_rows(A, B):
    """Elementwise sum of two (S, D) matrices: the residual addition."""
    return [[a + b for a, b in zip(ra, rb)] for ra, rb in zip(A, B)]

def gelu(x):
    """GELU, tanh form (Chapter 2), for one number."""
    return 0.5 * x * (1.0 + math.tanh(math.sqrt(2.0 / math.pi) * (x + 0.044715 * x ** 3)))

def weight(rows, cols, rng):
    """A rows x cols matrix with entries scaled by 1/sqrt(rows), as in Chapter 12."""
    return [[rng.gauss(0.0, 1.0) / math.sqrt(rows) for _ in range(cols)] for _ in range(rows)]
```
...
```python
class LayerNorm:
    """Normalize each row across its features, then scale and shift."""
    def __init__(self, dim, eps=1e-5):
        self.eps = eps
        self.gamma = [1.0] * dim     # scale, starts at identity
        self.beta = [0.0] * dim      # shift, starts at zero
    ...
    def forward(self, X):
        ...
        for row in X:
            ...
            out.append([g * (x - mean) * inv_std + b
                        for x, g, b in zip(row, self.gamma, self.beta)])
        return out
```

The loop over rows makes the per-token property visible; nothing computed for one row touches any other. The layer owns exactly $2D$ parameters, `gamma` and `beta`, however long the sequence. Notice what is absent: no running averages, no dependence on other rows, nothing that behaves differently in training and evaluation. PyTorch's `nn.LayerNorm` is the same three lines of arithmetic, and its `normalized_shape` argument is our `dim`.

`MLP` answers the first problem from the start of the chapter. Attention's output for a token is a weighted average of value vectors, so however many attention layers you stack, a token's vector stays a blend of linear projections of other tokens' vectors, and nothing in the stack computes a per-token nonlinear function. The naive response is to stack more attention and hope. Each extra attention layer costs $4D^2$ weights and an $S \times S$ score matrix per head, and what it buys is another blend, not the missing nonlinearity. What we want is what every network in Part I had between its linear layers, a nonlinearity applied to each token on its own. The mechanism is a two-layer network applied at every position with the same weights. Widen from $D$ to $4D$, apply GELU, project back to $D$. The $4\times$ width is a convention inherited from the 2017 paper, and it decides where a block spends its budget. Attention's four projection matrices hold $4D^2$ weights; the MLP's two hold $8D^2$. Two thirds of a block's weights sit in the MLP, and since every weight is used once per token, most of the block's arithmetic does too.

```python
class MLP:
    """Per-token feed-forward network: D -> 4D -> GELU -> D."""
    def __init__(self, embed_dim, seed=0):
        rng = random.Random(seed)
        self.W1 = weight(embed_dim, 4 * embed_dim, rng)
        self.b1 = [0.0] * (4 * embed_dim)
        self.W2 = weight(4 * embed_dim, embed_dim, rng)
...
class TransformerBlock:
    ...
    def forward(self, X, mask=None):
        X = add_rows(X, self.attn.forward(self.ln1.forward(X), mask))   # tokens exchange context
        X = add_rows(X, self.mlp.forward(self.ln2.forward(X)))          # each token transforms itself
        return X
```

Two lines do the work. Read them against the Problem section's loop: `ln1` is the `s / rms(stream)`, the sub-layer is the `apply(W, ...)`, and `add_rows` is the running total. The input and output have the same shape, which is what lets blocks stack without any adapter between them. PyTorch's `nn.TransformerEncoderLayer` selects this same arrangement with `norm_first=True`.

`TinyGPT` exists to give Milestone 2 a training signal. The label at every position of a text is the next token, so a sequence of $S$ tokens holds $S$ training examples. The naive way to collect them is one forward pass per example. Feed tokens $1$ through $s$, read one prediction, advance $s$, repeat. A window of 1,024 tokens, GPT-2's context length, would cost 1,024 forward passes, and every early prefix would be recomputed inside every later one. The causal mask from Chapter 12 collapses this to a single pass. Because the mask lets position $s$ see only tokens $1$ through $s$, the logits at position $s$ are a prediction of token $s+1$ made without peeking, and one forward pass returns all $S$ predictions as an $S \times V$ matrix. Every position of every sequence is a training example, all computed at once. So the model stacks the blocks and adds the two ends. The token table is Chapter 11's embedding; the position table is a second lookup indexed by position, the learned form GPT-2 and the real module use (Chapter 11's sinusoidal table works equally well here, and the block does not care which). The mask is built once per forward pass from the actual sequence length and handed to every block, since causality belongs to the sequence and not to any block.

```python
class TinyGPT:
    """Token IDs (length S) -> next-token logits (S, V)."""
    def __init__(self, vocab_size=1000, embed_dim=128, num_heads=4, num_layers=3,
                 max_seq_len=256, seed=0):
        rng = random.Random(seed)
        self.tok_emb = [[rng.gauss(0.0, 1.0) for _ in range(embed_dim)] for _ in range(vocab_size)]
        self.pos_emb = [[rng.gauss(0.0, 1.0) for _ in range(embed_dim)] for _ in range(max_seq_len)]
        self.blocks = [TransformerBlock(embed_dim, num_heads, seed=seed + 10 * i)
                       for i in range(num_layers)]
        self.ln_final = LayerNorm(embed_dim)
        self.W_head = weight(embed_dim, vocab_size, rng)    # (D, V), no bias

    def parameters(self):
        params = [self.tok_emb, self.pos_emb]
        for block in self.blocks:
            params.extend(block.parameters())
        return params + self.ln_final.parameters() + [self.W_head]

    def forward(self, tokens):
        mask = causal_mask(len(tokens))
        X = [[t + p for t, p in zip(self.tok_emb[tok], self.pos_emb[i])]
             for i, tok in enumerate(tokens)]                 # (S, D)
        for block in self.blocks:
            X = block.forward(X, mask)                        # (S, D), N times
        return matmul(self.ln_final.forward(X), self.W_head)  # (S, V)
```

Three things to notice. The loop over `self.blocks` is the entire depth of the model; making GPT deeper is a change to one integer, and Chapter 14 walks this same list when it counts what the model costs. The final `LayerNorm` is the only normalization applied to the stream itself, in keeping with Pre-LN. And `W_head` is $D \times V$, which for a real vocabulary is a large matrix. With $D = 768$ and $V = 50{,}257$ it holds 38.6 million weights, close to a third of a GPT-2 small model on its own. The production bridge returns to that number. GPT-2's code calls these five pieces `wte`, `wpe`, `h`, `ln_f`, and `lm_head`, and the mapping to ours is one to one.
...
| 2. `attn` | $\text{Attn}(\text{LN}(\mathbf{x}))$ (fixed) | $[-1.342, -0.447, +0.447, +1.342]$ | $[+0.100, +0.200, +0.100, +0.200]$ |
...
Running the chapter's own `LayerNorm` and `add_rows` on the same numbers reproduces the table. On my machine it prints the following.

```python
ln = LayerNorm(4)
x = [[1.0, 2.0, 3.0, 4.0]]
attn_out = [[0.10, 0.20, 0.10, 0.20]]      # stands in for attn(ln1(x))
mlp_out = [[0.05, 0.10, -0.05, 0.05]]      # stands in for mlp(ln2(x1))

n1 = ln.forward(x)
x1 = add_rows(x, attn_out)
n2 = ln.forward(x1)
x2 = add_rows(x1, mlp_out)
for name, M in [("ln1", n1), ("x1 ", x1), ("ln2", n2), ("x2 ", x2)]:
    print(name + ":", [round(v, 4) for v in M[0]])
```
...
The same block, seen from the point of view of shapes rather than values, is the second half of the trace. A `TinyGPT` with $D = 8$, two heads, two blocks, and a vocabulary of 16 tokens, given a sequence of three token IDs, moves the following matrices:

| Stage | Matrix | Shape (rows $\times$ columns) |
| :--- | :--- | :--- |
| Input | token IDs | $3$ integers |
| `tok_emb` + `pos_emb` | stream $h_0$ | $3 \times 8$ |
| Block 1, `ln1` output | normalized copy | $3 \times 8$ |
| Block 1, `attn`, per head | $Q, K, V$ | $3 \times 4$ each, 2 heads |
| Block 1, `attn`, per head | scores $QK^{\top}/\sqrt{4}$ + mask | $3 \times 3$ |
| Block 1, `mlp` hidden | after `W1` and `gelu` | $3 \times 32$ |
| Block 1 output, Block 2 output | stream $h_1$, $h_2$ | $3 \times 8$ |
| `ln_final`, `W_head` | logits | $3 \times 16$ |

: Matrix shapes through a two-block `TinyGPT` on a three-token input. {#tbl-tinygpt-shapes tbl-colwidths="[36,34,30]"}

The parameter count is the other number worth having. Chapter 14 charges every generated token for reading every weight in the model, and the production bridge below does this arithmetic for GPT-2 small by hand. Reading a count off a configuration file is guesswork until you have totaled one yourself, so here is a helper that walks `parameters()` and sums the scalars, run on the same two-block model.

```python
def count_parameters(model):
    """Total scalars across every matrix and vector in parameters()."""
    return sum(len(p) * (len(p[0]) if isinstance(p[0], list) else 1) for p in model.parameters())

model = TinyGPT(vocab_size=16, embed_dim=8, num_heads=2, num_layers=2, max_seq_len=8)
logits = model.forward([3, 7, 1])
print(len(logits), "x", len(logits[0]))   # 3 x 16: one row of 16 scores per position
print(count_parameters(model))            # 2016
```

Of the 2,016 parameters, 1,680 sit in the two blocks (840 each, of which 552 are the MLP's), matching the two-thirds share I quoted beside the `MLP` listing once the biases are counted. The stream keeps shape $3 \times 8$ from the embeddings to the final normalization. Everything that changes shape (the per-head split, the $3 \times 3$ score matrix, the $4\times$ hidden layer) lives inside a sub-layer and is folded back before the addition. Position 3's row of logits, 16 numbers, is the model's prediction for a fourth token it has not seen.

---

## From TinyTorch to Production

GPT-2 (Radford et al., 2019) is this chapter's block almost line for line: Pre-LN, GELU, a $4\times$ MLP, a final LayerNorm, learned positional embeddings, and a bias-free head. Its differences are small and each has a reason. It processes a batch of sequences at once as a three-dimensional array, applies dropout of $0.1$ to embeddings, attention weights, and residual updates during training, and ties the head to the embedding table. The models that followed changed more, and the changes cluster around the two cheap-looking operations in the block, normalization and activation. Llama (Touvron et al., 2023) replaces LayerNorm with **RMSNorm** (Zhang and Sennrich, 2019), which drops the mean subtraction and divides by $\sqrt{\frac{1}{D}\sum_i x_i^2 + \epsilon}$ alone, saving one pass over each vector; experiments showed the centering step was not what made LayerNorm work. It replaces the GELU MLP with a gated variant, SwiGLU,[^fn-swiglu-mlp-gate] and the added position table with RoPE, (the rotary scheme introduced in Chapter 11) which rotates $Q$ and $K$ inside attention instead of adding anything to the stream. The block structure, the stream, and the Pre-LN placement are untouched.
...
| Design choice | `TinyGPT` (this chapter) | GPT-2 (2019) | Llama 2 (2023) |
| :--- | :--- | :--- | :--- |
...
| MLP | $D \to 4D \to D$, GELU | $D \to 4D \to D$, GELU | gated, $D \to 2.7D \to D$, SwiGLU |
...
3. **Weight tying.** Modify `TinyGPT` so that the head reuses `tok_emb` instead of allocating `W_head` (multiply by the transpose of the embedding table), then use `count_parameters` from the worked trace to confirm the tied and untied totals for the GPT-2 small configuration match the 124.4 million and 163 million figures in the production bridge.
