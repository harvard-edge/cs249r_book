...
```python
# packages/tinytorch/src/tinytorch/conv.py
import numpy as np
...
| Spatial Patch Coordinate | Window Slices $[h:h+2, w:w+2]$ | Unrolled `im2col` Row Vector | Kernel Weight Column Vector | Dot Product Sum + Bias | Final Output $Y[0,0,h,w]$ |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Top-Left $(0, 0)$** | $\begin{bmatrix} 1.0 & 2.0 \\ 0.0 & 1.0 \end{bmatrix}$ | $[1.0, 2.0, 0.0, 1.0]$ | $[1.0, 0.0, -1.0, 1.0]^T$ | $1(1) + 2(0) + 0(-1) + 1(1) + 1.0$ | **$3.0$** |
| **Top-Right $(0, 1)$** | $\begin{bmatrix} 2.0 & 0.0 \\ 1.0 & 3.0 \end{bmatrix}$ | $[2.0, 0.0, 1.0, 3.0]$ | $[1.0, 0.0, -1.0, 1.0]^T$ | $2(1) + 0(0) + 1(-1) + 3(1) + 1.0$ | **$5.0$** |
| **Bottom-Left $(1, 0)$** | $\begin{bmatrix} 0.0 & 1.0 \\ 2.0 & 1.0 \end{bmatrix}$ | $[0.0, 1.0, 2.0, 1.0]$ | $[1.0, 0.0, -1.0, 1.0]^T$ | $0(1) + 1(0) + 2(-1) + 1(1) + 1.0$ | **$0.0$** |
| **Bottom-Right $(1, 1)$**| $\begin{bmatrix} 1.0 & 3.0 \\ 1.0 & 0.0 \end{bmatrix}$ | $[1.0, 3.0, 1.0, 0.0]$ | $[1.0, 0.0, -1.0, 1.0]^T$ | $1(1) + 3(0) + 1(-1) + 0(1) + 1.0$ | **$1.0$** |
