...
Because the XOR truth table produces positive labels ($y=1$) at $(0,1)$ and $(1,0)$ but negative labels ($y=0$) at $(0,0)$ and $(1,1)$, no single linear hyperplane ($w_1 x_1 + w_2 x_2 + b = 0$) can separate the two classes in 2D space. Minsky and Papert further proved that single-layer systems could not compute topological connectivity or parity. Crucially, they conjectured that extending networks to multiple layers was computationally intractable because no general mathematical algorithm existed to adjust the weights of hidden layers that did not directly touch the output error.
...
$$\begin{matrix} \text{Input Space } (2\text{D}) \\ [x_1, x_2] \end{matrix} \xrightarrow{\text{Linear}(2, 4)} \mathbf{z} \xrightarrow{\text{ReLU}} \begin{matrix} \text{Warped Manifold } (4\text{D}) \\ \mathbf{h} \in \mathbb{R}^{B \times 4} \end{matrix} \xrightarrow{\text{Linear}(4, 1)} \begin{matrix} \text{Output Scalar} \\ \hat{y} \in \{0, 1\} \end{matrix}$$
...
::: {#nbk-m1-xor-arithmetic-intensity .callout-notebook title="Arithmetic intensity of the XOR two-layer perceptron"}
Consider evaluating a forward-backward training pass on the complete XOR dataset ($B = 4$ samples) through our two-layer network: $\text{Linear}(2, 4) \to \text{ReLU} \to \text{Linear}(4, 1)$:

- **Floating-Point Operations (FLOPs)**:
  - Layer 1 Forward GEMM ($2 \times B \times D_{\text{in}} \times D_{\text{hidden}}$): $2 \times 4 \times 2 \times 4 = 64\text{ FLOPs}$
  - ReLU non-linear gating: $4 \times 4 = 16\text{ FLOPs}$
  - Layer 2 Forward GEMM ($2 \times B \times D_{\text{hidden}} \times D_{\text{out}}$): $2 \times 4 \times 4 \times 1 = 32\text{ FLOPs}$
  - Mean Squared Error Loss: $4 \times 3 = 12\text{ FLOPs}$
  $$\text{Total Forward Compute} = 124\text{ FLOPs}$$

- **Memory Traffic (Bytes Transferred)**:
  - Input batch ($4 \times 2$ floats): $32\text{ bytes}$
  - Layer 1 weights and bias ($2 \times 4 + 4 = 12$ floats): $48\text{ bytes}$
  - Layer 2 weights and bias ($4 \times 1 + 1 = 5$ floats): $20\text{ bytes}$
  - Target labels ($4$ floats): $16\text{ bytes}$
  $$\text{Total DRAM Traffic} = 116\text{ bytes}$$

- **Operational Intensity**:
  $$I = \frac{124\text{ FLOPs}}{116\text{ bytes}} \approx 1.07\text{ FLOPs/Byte}$$

**Systems Takeaway**: With an operational intensity near $1\text{ FLOP/byte}$, small multi-layer networks operate deep within the **memory-bandwidth bound** regime of modern processors (which require $I \ge 100\text{ FLOPs/byte}$ to saturate compute ALUs). Over $98\%$ of execution time is spent moving data between cache and registers rather than executing arithmetic.
:::

::: {.callout-checkpoint title="Milestone I Checkpoint: The Core Engine Is Complete"}
1. **Zero External ML Dependencies**: Built purely on Python and NumPy buffers.
2. **Exact Analytical Autograd**: Evaluated reverse topological VJPs with zero numerical error.
3. **Universal Function Separation**: Conquered Minsky's 1969 XOR Crisis via non-linear coordinate warping.
4. **Production Digit Recognition**: Achieved $>96\%$ test accuracy on TinyDigits handwritten classification.
:::
