...
By scaling the output by $2$, we obtain exact binary classification ($0, 1, 1, 0$). The non-linear $\text{ReLU}$ folds the coordinate space, converting the concave XOR problem into a convex decision boundary.
...
```python
# packages/tinytorch/tests/milestone_01_xor.py
import numpy as np
...
from tinytorch.losses import MSELoss
from tinytorch.optim import SGD, AdamW
...
| Input Coordinate $(x_1, x_2)$ | Target Label $y$ | Hidden Pre-Activation $\mathbf{z}_1$ | Hidden Post-ReLU $\mathbf{h}$ | Output Logit $\hat{y}$ | Prediction Rounded |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **$(0.0, 0.0)$** | $0.0$ | $[-0.52, -1.48, -0.81, -0.35]$ | $[0.00, 0.00, 0.00, 0.00]$ | $-0.012$ | **$0$ (Correct ✅)** |
| **$(0.0, 1.0)$** | $1.0$ | $[+0.49, -0.51, +0.62, +0.12]$ | $[0.49, 0.00, 0.62, 0.12]$ | $+0.994$ | **$1$ (Correct ✅)** |
| **$(1.0, 0.0)$** | $1.0$ | $[+0.51, -0.49, +0.60, +0.15]$ | $[0.51, 0.00, 0.60, 0.15]$ | $+0.996$ | **$1$ (Correct ✅)** |
| **$(1.0, 1.0)$** | $0.0$ | $[+1.52, +0.52, +1.84, +0.72]$ | $[1.52, 0.52, 1.84, 0.72]$ | $+0.008$ | **$0$ (Correct ✅)** |
...
from tinytorch.training import CosineSchedule
...
In modern production systems, Multi-Layer Perceptrons are executed across high-throughput hardware architectures:

| System Layer | TinyTorch Pure Python | Production PyTorch C++ / CUDA | Hardware Execution Engine |
| :--- | :--- | :--- | :--- |
| **Model Hierarchy** | `Sequential([Linear, ReLU])` | `torch::nn::Sequential` | C++ graph traversal |
