...
The listings below take the NumPy-backed `Tensor` stand-in, `Linear`, and `Sequential` from Chapter 3 as given. Chapter 3's `Linear` keeps its weight as an `(in_features, out_features)` array and computes `x @ weight + bias`, and every shape comment below follows that layout. Any object whose parameters expose a `.data` array and whose layers expose `.weight` and `.bias` in that layout will run them.
