...
It scores each output unit of a `Linear` by the L2 norm of its column, uses `argpartition` to find the `num_to_prune` smallest without sorting the rest, and zeros those columns and their bias entries. Two things to know before you call it. The arrays keep their shape, so both matrices in the MLP above still have 67.1 million cells and the multiply still reads every one; and the loop visits every `Linear` in the model, the last one included, so on a classifier with ten outputs and `prune_ratio=0.5` it zeros five of the ten output logits. Call it on the hidden layers you mean, or wrap the classifier's head somewhere the loop cannot see.
