...
The listings need only two things from Chapter 1's `Tensor`, the numbers it holds and its `requires_grad` flag. Chapter 1's `Tensor` keeps the numbers in a NumPy array; the stand-in here does the same and drops everything else, so that a matrix multiply is one line. To keep the chapter runnable on its own, we start from a ten-line stand-in that provides exactly those two things.
