...
In this chapter, we engineer **The KV Cache Engine (`KVCache`)**.[^vllm-paged] We explore why naive autoregression recomputes identical Key and Value vectors for past tokens at every step, derive the $O(S^2) \to O(S)$ complexity reduction, and build static, pre-allocated ring buffers that transform generation into an **$O(1)$ memory bandwidth matrix-vector operation**.[^kv-footprint]

[^vllm-paged]: In production inference engines like vLLM, KV cache fragmentation is managed via *PagedAttention*, which borrows virtual memory page tables from operating systems to allocate non-contiguous physical DRAM blocks for token sequences.
[^kv-footprint]: For a 32-layer transformer with 32 heads of dimension 128 in FP16 (2 bytes per element), caching 1 token requires $2 \times 32 \times 32 \times 128 \times 2 \approx 524 \text{ KB}$. A batch of 64 requests with 4,096 tokens consumes $\approx 137 \text{ GB}$ of GPU VRAM.
...
## The Mental Model: Dynamic Append-Only Memoization
...
## The Production Bridge: vLLM PagedAttention and Multi-Query Attention
