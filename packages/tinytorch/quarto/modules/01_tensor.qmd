...
<a href="https://mybinder.org/v2/gh/harvard-edge/cs249r_book/main?labpath=packages%2Ftinytorch%2Fmodules%2F01_tensor%2Ftensor.ipynb" class="action-btn btn-orange">Open in Binder →</a>
</div>
<div class="action-card">
...
@tbl-01-tensor-implementation-roadmap lays out the implementation in order, one part at a time.
...
@tbl-01-tensor-rank-examples anchors each rank in a concrete example you have seen before.
...
@tbl-01-tensor-broadcasting-rules walks through the compatibility check on representative shape pairs.
...
@tbl-01-tensor-view-vs-copy contrasts the memory and time cost of view operations against the copying ones.
...
Multiplying two $n{\times}n$ matrices requires n³ operations: for each of the n² output elements, you compute a dot product of n values. This cubic scaling is why a $2000{\times}2000$ matmul takes 8$\times$ longer than a $1000{\times}1000$ matmul, not 4$\times$. In neural networks, matrix multiplications consume over 90% of compute time. This computational density is precisely why GPUs exist for ML: a modern GPU has thousands of cores that can compute thousands of dot products simultaneously, turning an 800ms CPU operation into an 8ms GPU operation.

@tbl-01-tensor-operation-complexity summarises the time and memory cost of the core operations.
...
@tbl-01-tensor-vs-pytorch places your implementation side by side with the production reference for direct comparison.
...
- **[Launch Binder](https://mybinder.org/v2/gh/harvard-edge/cs249r_book/main?urlpath=lab/tree/packages/tinytorch/modules/01_tensor/tensor.ipynb)** - Run interactively in browser, no setup required
