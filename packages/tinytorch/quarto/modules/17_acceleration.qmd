...
<a href="https://mybinder.org/v2/gh/harvard-edge/cs249r_book/main?labpath=packages%2Ftinytorch%2Fmodules%2F17_acceleration%2Facceleration.ipynb" class="action-btn btn-orange">Open in Binder →</a>
</div>
<div class="action-card">
...
::: {#fig-17_acceleration-diag-1 fig-env="figure" fig-pos="htb" fig-cap="**TinyTorch Acceleration System**: Vectorized kernels and SIMD optimization." fig-alt="Block diagram of the TinyTorch acceleration stack: tensor inputs flow into BLAS-backed vectorized matmul, cache-aware tiled matmul, and a fused GELU kernel, with SIMD lanes and multi-threaded execution feeding the output."}
...
: **Fused and baseline GELU kernel functions.** {#tbl-17-acceleration-fusion-functions}
...
: **Listing 17.2 — Cache-aware tiled matmul API**. {#lst-17-acceleration-tiled-matmul}
...
:::{.callout-note title="Systems Implication: Kernel Fusion and Warp Divergence"}
...
- [ ] Why matrix multiplication sits at ~171 FLOPs/byte (compute-bound) while GELU sits at ~1 FLOPs/byte (memory-bound), and what each regime demands.
...
- **[Launch Binder](https://mybinder.org/v2/gh/harvard-edge/cs249r_book/main?urlpath=lab/tree/packages/tinytorch/modules/17_acceleration/acceleration.ipynb)** - Run interactively in browser, no setup required
