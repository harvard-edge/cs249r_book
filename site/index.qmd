    ...
    <a href="newsletter/" class="mls-rich-card" data-accent="follow">
      ...
      <div class="mls-rich-card-demo">
        <svg viewBox="0 0 260 140" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
          ...
          <text x="32" y="101" font-size="7.5" fill="#555" font-family="Inter,sans-serif">Milestone: {{stats.stars}} GitHub stars</text>
          <!-- Footer -->
...
<section class="mls-section mls-section-impact" id="support">
  ...
  <div class="mls-reader-map">
    ...
    <p class="mls-map-caption">Live readership — {{stats.countries}} countries</p>
  </div>

  <div class="mls-impact-bottom">
    <p class="mls-impact-stats-line"><span class="gh-star-count">{{stats.stars}}</span> stars &middot; {{stats.readers}} readers &middot; {{stats.countries}} countries</p>

    <div class="mls-goal-block">
      <p class="mls-impact-goal">Our goal: <strong>1,000,000 AI engineers by 2030</strong></p>
      <p class="mls-impact-goal-sub">Next milestone: 100,000 ★ — we're at <span class="gh-star-count">{{stats.stars}}</span>.<br/>Every star helps others discover this resource.</p>
    </div>
...
// Refresh the star count between deploys. The build already injected the
// value that renders on load, so this only matters for a page served from
// cache after the number has moved. Mirrors the "+" floor in build_stats.py
// so the runtime and build-time figures never disagree in style.
fetch('https://api.github.com/repos/harvard-edge/cs249r_book')
  .then(r => r.json())
  .then(d => {
    const stars = d.stargazers_count;
    if (!stars) return;
    const magnitude = stars < 100 ? 1 : Math.pow(10, String(stars).length - 2);
    const floored = Math.floor(stars / magnitude) * magnitude;
    const fmt = floored.toLocaleString() + (stars < 100 ? '' : '+');
