...
fetch('/stats.json?t=' + Date.now())
  ...
  .then(d => {
    const display = (d && d.display) || {};
    document.querySelectorAll('[data-stat]').forEach(el => {
      const value = display[el.getAttribute('data-stat')];
      if (value) el.textContent = value;
    });
  })
  .catch(err => console.warn('[mlsysbook] stats refresh failed, showing build-time values:', err));
...
fetch('https://api.github.com/repos/harvard-edge/cs249r_book')
  ...
  .catch(err => console.warn('[mlsysbook] star refresh failed, showing build-time value:', err));
</script>
```
