...
To illustrate this concretely: consider implementing a wake word detector (like "Hey Siri"). The **model development** involves choosing an architecture and training on speech data. But the **systems engineering** determines whether this runs on a cloud server (powerful but requires internet), a smartphone (moderate resources, local processing), or a smart speaker's embedded chip (severe constraints, must run 24/7). Each deployment target requires different engineering decisions—from the model architecture itself to how it's optimized, quantized, and integrated with the surrounding system.
...
[^fn-perceptron]: **Perceptron**: The fundamental building block of neural networks, invented by Frank Rosenblatt in 1957. Mathematically, it's a linear classifier that computes a weighted sum of inputs plus a bias term, then applies an activation function. Think of it as: `if (w1*x1 + w2*x2 + ... + b > threshold) then output=1 else output=0`. While simple, this concept scales up to form the deep neural networks we use today.

### Essential Terminology for Diverse Backgrounds {#sec-introduction-essential-terminology}

Before we explore AI's evolution, let's establish a common vocabulary. Different fields use these terms in various contexts, so we'll clarify their meaning in machine learning:

::: {.callout-note title="For Systems Students"}
- **Gradient Descent**: Think of it as an optimization loop that repeatedly adjusts program variables to minimize error. Similar to how a compiler optimizes code, but instead of optimizing for speed, we optimize for accuracy. Each iteration: `parameters = parameters - learning_rate * gradient`.
- **Perceptron**: The simplest ML model—essentially a function that takes inputs, multiplies them by weights, adds a bias, and outputs a decision. In code: `output = activate(sum(weights * inputs) + bias)`.
- **Training**: The process of finding optimal parameters through repeated gradient descent steps—like a very sophisticated parameter tuning process.
:::

::: {.callout-note title="For ML Students"}  
- **Deployment**: Moving your trained model from Jupyter notebooks to production systems where real users interact with it. This involves packaging models, setting up servers, and handling real-time requests.
- **Infrastructure**: The computing systems (CPUs, GPUs, memory, storage) and software stack (operating systems, frameworks, containers) that support ML training and serving.
- **System Constraints**: Hard limits on memory, compute, power, and latency that determine which models can actually run in production.
:::

::: {.callout-note title="For Embedded Engineers"}
- **Model Compression**: Techniques to shrink neural networks from gigabytes to kilobytes while maintaining accuracy. Methods include quantization (using 8-bit instead of 32-bit numbers) and pruning (removing unnecessary connections).
- **Edge ML vs Cloud ML**: Edge ML runs models directly on devices (like your microcontroller), while Cloud ML runs on remote servers. Edge ML prioritizes efficiency; Cloud ML prioritizes capability.
- **Inference**: Running a trained model to make predictions—this is what happens on your embedded device, as opposed to training which typically happens on powerful servers.
:::
