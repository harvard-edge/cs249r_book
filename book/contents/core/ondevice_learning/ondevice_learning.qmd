...
::: {#lst-bias-adaptation lst-cap="**Bias-Only Adaptation**: Freezes model parameters except for biases to reduce memory usage and enable on-device learning."}
```{.python}
...
::: {.content-visible when-format="html"}
::: {#lst-selective-update lst-cap="**Selective Layer Updating**: This technique enables fine-tuning specific layers of a pre-trained model while keeping others frozen, optimizing computational resources for targeted improvements. *Source: PyTorch Documentation*"}
```{.python}
