...
## Overview {#sec-ai-workflow-overview-279f}
...
### Traditional vs. AI Lifecycles {#sec-ai-workflow-traditional-vs-ai-lifecycles-65e3}
...
## Lifecycle Stages {#sec-ai-workflow-lifecycle-stages-d874}
...
## Problem Definition {#sec-ai-workflow-problem-definition-6463}
...
### Requirements and System Impact {#sec-ai-workflow-requirements-system-impact-796d}
...
However, real-world challenges complicate this picture. ML systems must function effectively in diverse environments, where factors like computational constraints, data variability, and integration requirements play significant roles. For example, the DR system needed to detect subtle features like microaneurysms[^fn-microaneurysms], hemorrhages[^fn-hemorrhages], and hard exudates[^fn-hard-exudates] across retinal images of varying quality while operating within the limitations of hardware in rural clinics. A model that performs well in isolation may falter if it cannot handle operational realities, such as inconsistent imaging conditions or time-sensitive clinical workflows. Addressing these factors requires aligning learning objectives with system constraints, ensuring the system's long-term viability in its intended context.

[^fn-microaneurysms]: **Microaneurysms**: Small bulges in blood vessels of the retina commonly seen in diabetic retinopathy.
[^fn-hemorrhages]: **Hemorrhages**: Blood that has leaked from the blood vessels into the surrounding tissues.
[^fn-hard-exudates]: **Hard Exudates**: Deposits of lipids or fats indicative of leakage from impaired retinal blood vessels.

### Definition Workflow {#sec-ai-workflow-definition-workflow-5804}
...
### Scale and Distribution {#sec-ai-workflow-scale-distribution-3bf6}
...
### Systems Thinking {#sec-ai-workflow-systems-thinking-3f80}
...
Emergent behaviors[^fn-emergent-behavior] also play a role. A system that was initially designed to detect retinopathy might reveal additional use cases, such as identifying other conditions like diabetic macular edema, which can reshape the problem's scope and requirements. In the DR project, insights from deployment highlighted potential extensions to other imaging modalities, such as 3D Optical Coherence Tomography (OCT)[^fn-3D-OCT].

[^fn-emergent-behavior]: **Emergent Behavior**: Unexpected phenomena or behaviors not foreseen by designers, arising from the interaction of system components.
[^fn-3D-OCT]: **3D Optical Coherence Tomography (OCT)**: A non-invasive imaging technique used to obtain high resolution cross-sectional images of the retina.
...
### Lifecycle Implications {#sec-ai-workflow-lifecycle-implications-997b}
...
## Data Collection {#sec-ai-workflow-data-collection-7863}
...
### Data Requirements and Impact {#sec-ai-workflow-data-requirements-impact-6bf8}
...
### Data Infrastructure {#sec-ai-workflow-data-infrastructure-67f4}
...
### Scale and Distribution {#sec-ai-workflow-scale-distribution-8dbc}
...
Scaling challenges highlight how decisions made during the data collection phase ripple through the lifecycle, impacting subsequent stages like model development, deployment, and monitoring. For instance, accommodating higher-resolution data during collection directly influences computational requirements for training and inference, emphasizing the need for lifecycle thinking[^fn-lifecycle-thinking] even at this early stage.

[^fn-lifecycle-thinking]: **Lifecycle Thinking**: Considering all phases of a system's life from design to decommissioning to optimize overall performance.

### Data Validation {#sec-ai-workflow-data-validation-6a14}
...
### Systems Thinking {#sec-ai-workflow-systems-thinking-95f2}
...
\draw[Line,-latex](B2)|-node[Text,pos=0.7]{Data gaps}(DB1);
\draw[Line,-latex](B2)-|node[Text,pos=0.25]{Validation Issues}(GB1);
\draw[Line,-latex,](B3)--++(270:2.2)-|node[Text,pos=0.25]{Performance Insights}(DB1);
\draw[Line,-latex](B2)-|(GB2);
\draw[Line,-latex](GB2)-|(B3.130);
\draw[Line,-latex](B3)--++(90:2.6)-|node[Text,pos=0.2]{Model Updates}(GB1);
\draw[Line,-latex](B3.50)--++(90:2.8)-|node[Text,pos=0.35]{Data Quality Issues}(B1.120);
...
### Lifecycle Implications {#sec-ai-workflow-lifecycle-implications-974a}
...
## Model Development {#sec-ai-workflow-model-development-9195}
...
For DR detection, the model needed to achieve expert-level accuracy while handling the high resolution and variability of retinal images. Using a deep neural network trained on their meticulously labeled dataset, the team achieved an F-score of 0.95, slightly exceeding the median score of the consulted ophthalmologists (0.91). This outcome highlights the effectiveness of state-of-the-art methods, such as transfer learning[^fn-transfer-learning], and the importance of interdisciplinary collaboration between data scientists and medical experts to refine features and interpret model outputs.

[^fn-transfer-learning]: **Transfer Learning**: A method where a model developed for a task is reused as the starting point for a model on a second task.

### Model Requirements and Impact {#sec-ai-workflow-model-requirements-impact-8a34}
...
### Development Workflow {#sec-ai-workflow-development-workflow-6c44}
...
### Scale and Distribution {#sec-ai-workflow-scale-distribution-de67}
...
### Systems Thinking {#sec-ai-workflow-systems-thinking-576c}
...
### Lifecycle Implications {#sec-ai-workflow-lifecycle-implications-ef22}
...
## Deployment {#sec-ai-workflow-deployment-5772}
...
### Deployment Requirements and Impact {#sec-ai-workflow-deployment-requirements-impact-0951}
...
### Deployment Workflow {#sec-ai-workflow-deployment-workflow-c14c}
...
### Scale and Distribution {#sec-ai-workflow-scale-distribution-5f16}
...
### Robustness and Reliability {#sec-ai-workflow-robustness-reliability-5ec7}
...
### Systems Thinking {#sec-ai-workflow-systems-thinking-05ef}
...
### Lifecycle Implications {#sec-ai-workflow-lifecycle-implications-ffeb}
...
## Maintenance {#sec-ai-workflow-maintenance-8385}
...
### Monitoring Requirements and Impact {#sec-ai-workflow-monitoring-requirements-impact-831c}
...
### Maintenance Workflow {#sec-ai-workflow-maintenance-workflow-584e}
...
### Scale and Distribution {#sec-ai-workflow-scale-distribution-2f51}
...
### Proactive Maintenance {#sec-ai-workflow-proactive-maintenance-2af2}
...
### Systems Thinking {#sec-ai-workflow-systems-thinking-2cc8}
...
### Lifecycle Implications {#sec-ai-workflow-lifecycle-implications-98fc}
...
## AI Lifecycle Roles {#sec-ai-workflow-ai-lifecycle-roles-20f6}
...
### Collaboration in AI {#sec-ai-workflow-collaboration-ai-7eee}
...
### Role Interplay {#sec-ai-workflow-role-interplay-ba00}
...
## Summary {#sec-ai-workflow-summary-6cdd}
