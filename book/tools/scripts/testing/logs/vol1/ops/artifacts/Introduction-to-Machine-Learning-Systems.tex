...
\begin{fbx}{callout-perspective}{Systems Perspective:}{The Operational Mismatch}
...
and deployment strategies for different risk profiles. Volume II
(Operations at Scale) extends these foundations to platform operations
managing hundreds of models, cross-model dependencies, multi-region
coordination, and organization-wide ML platform engineering.
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{The Three Critical Interfaces}
...
\begin{fbx}{callout-definition}{Definition:}{MLOps}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2091}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Data Layer} \textbf{Training Layer} \textbf{Serving Layer}
\textbf{Monitoring Layer} & Feature computation, storage, serving Model
development, hyperparameter optimization Inference, scaling, latency
management Drift detection, performance tracking & Changes with data
schema evolution Changes with algorithm research Changes with traffic
patterns Changes with business requirements \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Sudden accuracy drop} \textbf{Gradual drift} \textbf{Subgroup
degradation} \textbf{Latency increase} & Threshold alerts Trend analysis
Cohort monitoring Percentile tracking & Immediate rollback Scheduled
retraining Targeted data collection Infrastructure scaling \\
\end{longtable}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2841}}@{}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{1. Reproducibility} \textbf{2. Separation of Concerns}
\textbf{3. Consistency} \textbf{4. Observable Degradation} \textbf{5.
Cost-Aware Automation} & Version all artifacts Independent layer
evolution Training equals Serving Make failures visible Optimize total
cost & Complete artifact hash Layer coupling score Feature skew rate
Time to detection Net retraining value \\
\end{longtable}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{Monitoring Strategy by Archetype}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1611}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2886}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2349}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3020}}@{}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{ResNet-50} \textbf{(Compute Beast)} & Visual distribution shift
(lighting, camera, new object classes) & \textbf{Accuracy on holdout
set} (ground truth available) & Accuracy drops \textgreater{} 2\% from
baseline (\(\sim\)monthly for stable domains) \\
\textbf{GPT-2} \textbf{(Bandwidth Hog)} & Vocabulary drift, topic shift,
emerging entities & \textbf{Perplexity on live traffic} (no ground truth
needed) & Perplexity increases \textgreater{} 10\%; new vocabulary
detected (\(\sim\)weekly for news domains) \\
\textbf{DLRM} \textbf{(Sparse Scatter)} & User behavior shift, item
catalog churn, cold-start items & \textbf{CTR/CVR delta} vs.~historical
cohorts & Engagement drops \textgreater{} 5\%; catalog refresh
(\(\sim\)daily for e-commerce) \\
\textbf{DS-CNN} \textbf{(Tiny Constraint)} & Acoustic environment change
(noise floor shift) & \textbf{Duty cycle} (wakeups/hour) + \textbf{false
positive rate} & False wake rate \textgreater{} 1\%; battery drain
exceeds spec (\(\sim\)quarterly OTA update) \\
\end{longtable}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/5c57fe979acb93623fc15e0ab531d23a4a517585.pdf}}
...
\begin{fbx}{callout-checkpoint}{Checkpoint:}{The MLOps Loop}
...
\begin{fbx}{callout-definition}{Definition:}{Technical Debt in ML}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Compound Cost of Manual Operations}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/b220b6cdbab86d97d8c2aa3b85cf3a286a47957c.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/54b385f519cca4e7a19dc51f0a88805513f05408.pdf}}
...
Figure~\ref{fig-correction-cascades-flowchart} visualizes \emph{how}
these cascading effects propagate through ML system development.
\ldots{}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/49499d3e5844a36d0d8e8971275dd46e76b0df15.pdf}}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2696}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3217}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/1d6339a435378bfc3afa33a8cb1e3fd33b8e2a7d.pdf}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3876}}@{}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Feature preprocessing} \textbf{Missing data handling}
\textbf{Time-dependent features} \textbf{Library version drift} &
Normalization uses different statistics Training fills NaN with mean;
serving uses 0 Features computed with different time cutoffs NumPy or
Pandas version differences & Statistical comparison of feature
distributions Schema validation with explicit null handling Timestamp
validation in feature pipelines Environment hash comparison \\
\end{longtable}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{Uber Michelangelo Feature Store}
...
\caption{\label{lst-ops-validate-skew}Feature Skew Validation: This
function compares training and serving feature distributions using the
Kolmogorov-Smirnov test, rejecting deployment when any feature diverges
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/748b4c64dc7f51164597a63327c4553165053954.pdf}}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{Google TFX Production ML Pipelines}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Cost of Silent Failures}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Ad click prediction} \textbf{Fraud detection} \textbf{Demand
forecasting} \textbf{Medical imaging} & Daily Weekly Monthly Quarterly &
User interests shift rapidly Attack patterns evolve continuously
Seasonal patterns change slowly Disease presentations are stable \\
\end{longtable}
...
\caption{\label{lst-ops-triggered-retraining}Triggered Retraining
Configuration: This example defines three trigger conditions for
automatic retraining---accuracy degradation, feature drift measured by
PSI, and prediction distribution shift---each with configurable
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Half-Life of a Model}
...
\begin{fbx}{callout-example}{Example:}{Fraud Detection Retraining}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{\(Q\)} \textbf{\(V\)} \textbf{\(A_0\)} \textbf{\(\lambda\)}
\textbf{Retraining Cost} & 1,000,000 \$0.50 0.95 0.02 \$5,000 &
Transactions per day Value per accuracy point Initial accuracy Daily
decay rate (2\% per day) Total retraining expense \\
\end{longtable}

\textbf{Applying the formula:}

\[T^* \approx \sqrt{\frac{2 \times 5000}{1000000 \times 0.50 \times 0.95 \times 0.02}} \approx \sqrt{\frac{10\,000}{9\,500}} \approx 1\.03 \text{ days}\]
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.3056}}@{}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{4x retraining cost} \textbf{4x query volume} \textbf{4x decay
rate} & 2x longer interval 2x shorter interval 2x shorter interval \\
\end{longtable}
...
drift (\citeproc{ref-ibm_data_drift}{Ríos et al. 2019}).
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/fbb53558f5744fa4fae2d013622569a67717b911.pdf}}
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{The Latency Budget}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1368}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1966}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1641}}@{}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2043}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2258}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3333}}@{}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Compute-bound} & \begin{minipage}[t]{\linewidth}\raggedleft
\begin{quote}
85\%
\end{quote}
\end{minipage} & \textless70\% & Larger batch sizes, tensor parallelism
within node \\
\textbf{Memory-bound} & 50-85\% &
\begin{minipage}[t]{\linewidth}\raggedleft
\begin{quote}
85\%
\end{quote}
\end{minipage} & Reduce model size, quantize, optimize memory access \\
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Drift Detection Delay}
...
\begin{fbx}{callout-definition}{Definition:}{Data Drift}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Economics of Observability}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3039}}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{Netflix ML Monitoring at Scale}
...
\caption{\label{lst-ops-schema-validation}Input Data Validation with
Great Expectations: Schema validation rules check column existence, data
types, null values, and statistical bounds to catch data quality issues
before they propagate to model inference.}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2268}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Population Stability Index (PSI)} \textbf{Kolmogorov-Smirnov
statistic} \textbf{Jensen-Shannon divergence} & PSI \textgreater{} 0.2
KS \textgreater{} 0.1 JS \textgreater{} 0.1 & Categorical and binned
features Continuous feature distributions Probability distributions \\
\end{longtable}
...
\caption{\label{lst-ops-freshness-alert}Data Freshness Alert
...
\begin{fbx}{callout-checkpoint}{Checkpoint:}{The Monitoring Stack}
...
\begin{fbx}{callout-example}{Example:}{Single-Model Monitoring Budget}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1101}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3853}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1835}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{P0} \textbf{P1} \textbf{P2} \textbf{P3} & Complete model
failure, serving errors Significant accuracy degradation
(\textgreater10\%) Moderate drift, localized impact Minor anomalies, no
user impact & 15 minutes 1 hour 4 hours 24 hours & Model returns null
predictions Recommendation CTR drops 15\% One feature shows PSI
\textgreater{} 0.3 Training pipeline delay \\
\end{longtable}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1928}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Desktop users} \textbf{Mobile (iOS)} \textbf{Mobile (Android)}
\textbf{Tablet users} \textbf{Overall} & 45\% 30\% 20\% \textbf{5\%}
\textbf{100\%} & 94\% 92\% 88\% \textbf{62\%} \textbf{91\%} & Nominal
Nominal Minor degradation \textbf{Severe---investigate} \textbf{Masks
tablet problem} \\
\end{longtable}
...
\caption{\label{lst-ops-shap-debugging}SHAP-Based Debugging Workflow:
This code filters mispredicted examples from a problematic slice (tablet
users), computes SHAP values to explain model decisions, and generates a
summary plot revealing which features contribute most to the errors.}
...
\textbf{ML-Specific On-Call Challenges}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Ambiguous alerts}: A 3\% accuracy drop might be noise, drift,
  or critical failure. On-call engineers need statistical context to
  distinguish.
\item
  \textbf{Delayed impact visibility}: Unlike latency spikes visible
  immediately, ML degradation may take hours or days to manifest in
  business metrics.
\item
  \textbf{Cross-system dependencies}: ML issues often originate in
  upstream data systems owned by different teams.
\item
  \textbf{Expertise requirements}: Effective response requires
  understanding model behavior, not just infrastructure health.
\end{enumerate}

\textbf{On-Call Rotation Design}

Table~\ref{tbl-oncall-structure} provides a recommended on-call
structure for ML teams:
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.4756}}@{}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Tier 1} \textbf{(Primary)} & ML Engineer & Initial triage,
standard runbooks, escalation decisions \\
\textbf{Tier 2} \textbf{(Escalation)} & Senior ML Engineer / Data
Scientist & Complex debugging, cross-system investigation,
model-specific issues \\
\textbf{Tier 3} \textbf{(Critical)} & ML Platform Lead & Architecture
decisions, major incidents, vendor escalation \\
\textbf{Data On-Call} \textbf{(Parallel)} & Data Engineer & Data
pipeline issues, feature store problems, upstream dependencies \\
\end{longtable}

\textbf{Runbook Requirements}
...
\tightlist
\item
  \textbf{Model overview}: What it does, who owns it, business
  criticality
\item
  \textbf{Normal operating parameters}: Expected latency, throughput,
  accuracy ranges
\item
  \textbf{Common failure modes}: Historical incidents and their
  resolutions
\item
  \textbf{Diagnostic commands}: How to check model health, recent
  predictions, feature distributions
\item
  \textbf{Escalation criteria}: When to wake up Tier 2, when to rollback
  without approval
\item
  \textbf{Rollback procedure}: Step-by-step instructions with expected
  recovery time
\end{enumerate}
...
\tightlist
\item
  \textbf{Alert consolidation}: Group related alerts (e.g., multiple
  features drifting simultaneously)
\item
  \textbf{Adaptive thresholds}: Use dynamic baselines that account for
  weekly/seasonal patterns
\item
  \textbf{Signal-to-noise tracking}: Measure alert actionability rate;
  retire alerts acted on \textless10\% of the time
\item
  \textbf{Snooze with accountability}: Allow temporary silencing with
  required follow-up ticket
\end{itemize}

\textbf{Handoff Practices}
...
\textbf{Burnout Prevention}

ML on-call can be particularly stressful due to incident ambiguity.
Mitigate through:
...
\textbf{Role Boundaries and Handoffs}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Data Scientist → ML Engineer}: The ``notebook to production''
  gap. Mitigate with standardized model interfaces, required
  documentation, and reproducibility requirements before handoff.
\item
  \textbf{ML Engineer → SRE}: Production readiness reviews should verify
  monitoring, alerting, runbooks, and rollback procedures exist before
  SRE accepts operational responsibility.
\item
  \textbf{Data Engineer → ML Team}: Feature contracts specify schema,
  freshness SLOs, and quality guarantees. Breaking changes require
  coordination, not silent updates.
\end{enumerate}

\textbf{Anti-Pattern: The ``Full-Stack ML Engineer''}

Some organizations expect individuals to span all roles. While appealing
for small teams, this creates:
...
A single engineer can prototype across roles, but production systems
benefit from role specialization. Small teams should explicitly
acknowledge coverage gaps and prioritize based on risk.

\textbf{Scaling Team Structure}
...
\tightlist
\item
  \textbf{1-3 models}: Generalist ML Engineers cover most
  responsibilities. Data Scientists may deploy their own models.
\item
  \textbf{3-10 models}: Specialize roles. Dedicated Data Engineer for
  feature pipelines. ML Engineers focus on serving.
\item
  \textbf{10+ models}: Platform team emerges to provide shared
  infrastructure. Consider embedded ML Engineers in product teams
  supported by central platform.
\end{itemize}
...
\begin{verbatim}
/Library/Frameworks/Python.framework/Versions/3.13/lib/python3.13/site-packages/IPython/core/pylabtools.py:170: UserWarning: Tight layout not applied. The bottom and top margins cannot be made large enough to accommodate all Axes decorations.
...
\pandocbounded{\includegraphics[keepaspectratio]{contents/vol1/ops/ops_files/figure-pdf/fig-business-cost-curve-output-2.pdf}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2764}}@{}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.5325}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/83b7d47060ade113c28ad85bc36289260bfdca76.pdf}}
...
\begin{fbx}{callout-example}{Example:}{Principle Mapping Guide}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2481}}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{Oura Ring: Principles Summary}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/26c42872ea5f3b9a8dcffe3269f7a3caef31754f.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/e50d6bd5147e5ce4e0e9e8691b02330d0108f703.pdf}}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{ClinAIOps: Principles Summary}
...
This chapter established MLOps as the engineering discipline that
transforms ML prototypes into sustainable production systems. The core
insight driving this discipline is that \textbf{machine learning systems
fail silently}: they degrade along a continuum rather than crash,
requiring fundamentally different operational practices than traditional
software.

The foundational principles introduced at the chapter's opening
(Section~\ref{sec-machine-learning-operations-mlops-foundational-principles-44e6})
provide an evaluation framework for any MLOps implementation:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Reproducibility Through Versioning}: Every production incident
  we examined traced back to untracked artifacts: data versions,
  configuration changes, or environment drift. Feature stores, model
  registries, and infrastructure-as-code implement this principle.
\item
  \textbf{Separation of Concerns}: The technical debt patterns (boundary
  erosion, correction cascades) emerge when layers become entangled.
  Modular architecture separating data, training, serving, and
  monitoring layers contains blast radius and enables independent
  evolution.
\item
  \textbf{The Consistency Imperative}: Training-serving skew caused
  5-15\% accuracy degradation in our examples because feature
  computation diverged between pipelines. Feature stores eliminate this
  divergence by computing features once and serving them everywhere.
\item
  \textbf{Observable Degradation}: The ``silent failure'' problem
  requires making degradation visible before users notice. Layered
  monitoring, tracking data freshness, feature distributions, model
  outputs, and business metrics, transforms the \textbf{Statistical
  Drift Invariant} from an abstract equation into actionable alerts.
\item
  \textbf{Cost-Aware Automation}: The retraining economics framework
  (\(T^* \approx \sqrt{\frac{2C}{QVA_0\lambda}}\)) quantifies the
  tradeoff between staleness cost and retraining expense, enabling
  principled decisions rather than arbitrary schedules.
\end{enumerate}

The infrastructure components directly implement these principles. CI/CD
pipelines enforce reproducibility, feature stores ensure consistency,
monitoring systems make degradation observable, and the retraining
decision framework enables cost-aware automation. The case studies
demonstrated that domain constraints reshape \emph{how} principles are
implemented without changing \emph{which} principles matter:

\begin{itemize}
\item
  \textbf{Oura Ring} showed how edge constraints force proactive
  graceful degradation design, with tiered model architectures enabling
  fallback when resources are constrained. The 62\% to 79\% accuracy
  improvement came from systematic data management and configuration
  control, not algorithmic innovation.
\item
  \textbf{ClinAIOps} showed how regulatory requirements transform
  graceful degradation from optional to mandatory, with
  human-in-the-loop governance as the primary safety mechanism. The
  three feedback loops (patient-AI, clinician-AI, patient-clinician) are
  architectural patterns, not operational overhead.
\end{itemize}
...
MLOps closes the operational loop that began with model development,
optimization, and benchmarking. The five foundational principles provide
the evaluation framework for any MLOps implementation regardless of
scale or domain: reproducibility through versioning, separation of
concerns, the consistency imperative, observable degradation, and
...
Martín, and Andoni Beristain. 2025. {``Scalable Big Data Platform with
...
TensorFlow™ Inception in Classifying Clear Cell Renal Cell Carcinoma and
Oncocytoma on Multiphasic CT.''} \emph{Abdominal Radiology} 44 (6):
2009--20. \url{https://doi.org/10.1007/s00261-019-01929-0}.
...
Ríos, Carlos, Nathan Youngblood, Zengguang Cheng, Manuel Le Gallo,
...
Services, Amazon Web. 2020. {``CloudFormation.''} Wiley.
