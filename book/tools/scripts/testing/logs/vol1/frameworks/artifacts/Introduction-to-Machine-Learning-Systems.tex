...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, leftrule=.75mm, colbacktitle=quarto-callout-tip-color!10!white, bottomrule=.15mm, rightrule=.15mm, breakable, toprule=.15mm, colframe=quarto-callout-tip-color-frame, toptitle=1mm, left=2mm, titlerule=0mm, opacityback=0, bottomtitle=1mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Learning Objectives}, colback=white, opacitybacktitle=0.6, arc=.35mm]
...
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{The ML Compiler}
...
\begin{fbxSimple}{callout-definition}{Definition:}{Machine Learning Frameworks}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/f63bbab4e43248ad9fdc137e758bec8dbac3d091.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/918815d042f3500301723bb15d79eb764817a2fe.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/0613cabf3df5214d811837736ce24fcb1cad15f5.pdf}}
...
\begin{fbxSimple}{callout-example}{Example:}{Eager vs. Graph Execution Code Comparison}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/c756350f4ae0041a6879bdf6f6615c6cd1bb01a8.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/922aeda7a5c63d8a26b4bafc70eb55208f692f33.pdf}}
...
(\textbf{?@lst-torchscript-conditional}), language restrictions
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{The Physics of Software Overhead}
...
\item
  ...
  \tightlist
  \item
    Launch \texttt{Add} Kernel: 15 µs overhead.
  \item
    Read/Write Memory: \(2N\) bytes.
  \item
    Launch \texttt{ReLU} Kernel: 15 µs overhead.
  \item
    Read/Write Memory: \(2N\) bytes.
  \end{enumerate}
\item
  \textbf{Total Overhead:} 30 µs.
\item
...
\item
  ...
  \tightlist
  \item
    Launch \texttt{Fused} Kernel: 15 µs overhead.
  \item
    Read/Write Memory: \(2N\) bytes (intermediate result stays in
    registers).
  \end{enumerate}
\item
  \textbf{Total Overhead:} 15 µs (\textbf{2× speedup}).
\item
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2517}}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2443}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3969}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3588}}@{}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2460}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2063}}
...
\begin{fbxSimple}{callout-lighthouse}{Lighthouse:}{Framework Strategy by Archetype}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/478dc6cc0266f2b7fb3ca3f0e9d82caa45f72877.pdf}}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{The Dispatch Tax}
...
\begin{fbxSimple}{callout-lighthouse}{Lighthouse:}{Lighthouse Example: TinyML Frameworks (KWS)}
...
\begin{fbxSimple}{callout-checkpoint}{Checkpoint:}{Execution Models}
...
(Listing~\ref{lst-forward_structure}), and application to sensitivity
...
and how that value changes with respect to the input. Sensitivity
analysis measures how changing a single pixel or feature affects the
model's output. Forward mode tracks input perturbations through each
layer to the final prediction.
...
gradient hooks (\textbf{?@lst-gradient-hooks}), and safe gradient
detachment (\textbf{?@lst-detach-vs-data}).
...
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{Caching Allocator and Utilization}
...
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{Tape-based vs. Transform-based Autodiff}
...
\textbf{?@sec-ai-frameworks-major-framework-platform-analysis-fe96}
examines in detail.
...
\begin{fbxSimple}{callout-checkpoint}{Checkpoint:}{The Systems Cost of Gradients}
...
\begin{fbxSimple}{callout-definition}{Definition:}{Tensor}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{The Administrative Tax}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/da6901e2e161b845f459ff50e1abb584b6f52d6a.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/c427b4bbf2426bd1d871d375aaabae3cfafd58bd.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/e54fdcd4b47792ca2a62cafdd9fa22b7980eb3f3.pdf}}
...
training techniques that constitute an advanced topic, but the
single-node implementation above illustrates the core synchronization
principles that underlie all pipeline-parallel systems.
...
torch.cuda.set\_device(1) y = torch.randn(100, device=``cuda'') \# Uses
cuda:1 print(y.device) \# cuda:1

\bookmarksetup{startatroot}

\chapter{Context manager for scoped device
changes}\label{context-manager-for-scoped-device-changes}

print(torch.cuda.current\_device()) \# 0

with torch.cuda.device(1): x = torch.randn(100, device=``cuda'')
print(x.device) \# cuda:1 print(torch.cuda.current\_device()) \# 1

print(torch.cuda.current\_device()) \# 0 (restored)
...
fault tolerance---constitute advanced topics that build on these
single-node foundations.
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/f330ef1a7ed91b83e1be213e068fdad90a801dcb.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/c9b5e0a62997b0bf688552d2f74a1ead277a7841.pdf}}
...
and replacing layers. f''std=\{output.std():.3f\}'' )

\bookmarksetup{startatroot}

\chapter{Backward hook to inspect
gradients}\label{backward-hook-to-inspect-gradients}

def backward\_hook(module, grad\_input, grad\_output): print(f''Gradient
norm: \{grad\_output{[}0{]}.norm():.3f\}``)

\bookmarksetup{startatroot}

\chapter{Register hooks on specific
layer}\label{register-hooks-on-specific-layer}

handle\_fwd = model{[}0{]}.register\_forward\_hook(forward\_hook)
handle\_bwd =
model{[}0{]}.register\_full\_backward\_hook(backward\_hook)

\bookmarksetup{startatroot}

\chapter{Execute forward and backward
pass}\label{execute-forward-and-backward-pass}

x = torch.randn(32, 10) y = model(x) loss = y.sum() loss.backward()

\bookmarksetup{startatroot}

\chapter{Remove hooks when done}\label{remove-hooks-when-done}

handle\_fwd.remove() handle\_bwd.remove()
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1775}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1479}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1716}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1657}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1893}}@{}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3058}}
...
Table~\ref{tbl-deployment-frameworks} reveals a fragmented landscape:
different deployment targets favor different frameworks. This
fragmentation creates a practical problem when organizations train in
one framework but deploy on a target best served by another.
\textbf{Interoperability through ONNX.} The Open Neural Network Exchange
(ONNX)\sidenote{\textbf{ONNX (Open Neural Network Exchange)}
(\citeproc{ref-bai2019onnx}{Flint et al. 2019}): Launched September 2017
by Facebook and Microsoft to solve framework fragmentation. Originally
named ``Toffee'' internally at Facebook, the name change emphasized its
role as an exchange format. Became a Linux Foundation project in 2019.
Enables training in PyTorch and deploying via TensorFlow Lite or
TensorRT without manual conversion. } format addresses this by enabling
model portability across frameworks: train in PyTorch, deploy via
TensorFlow Lite or ONNX Runtime. This standardization eliminates manual
conversion when moving between development and production environments.
Figure~\ref{fig-onnx} illustrates this hub-and-spoke interoperability
model. Detailed deployment optimization (quantization, pruning,
hardware-specific compilation) appears in
\textbf{?@sec-model-compression} and
...
\phantomsection\label{callout-perspectiveux2a-6.1}
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{Framework Selection Constraints}
\phantomsection\label{callout-perspective*-6.1}
...
\phantomsection\label{callout-perspectiveux2a-6.2}
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{Dynamic vs Static Computational Graphs}
\phantomsection\label{callout-perspective*-6.2}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3883}}@{}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3732}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2299}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2069}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2759}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Component}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
\tightlist
\item
  \(T_{\text{compute}}\) ≈ 40M / 312T ≈ 0.1µs
\item
  \(T_{\text{memory}}\) ≈ 5MB / 2TB/s ≈ 2.5µs
\item
...
\phantomsection\label{callout-perspectiveux2a-6.3}
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{The Three Problems in Action}
\phantomsection\label{callout-perspective*-6.3}
...
\phantomsection\label{callout-takeawaysux2a-6.4}
\begin{fbxSimple}{callout-takeaways}{Takeaways:}{Key Takeaways}
\phantomsection\label{callout-takeaways*-6.4}
...
\phantomsection\label{callout-chapter-connectionux2a-6.5}
\begin{fbxSimple}{callout-chapter-connection}{What’s Next:}{From Control Room to Power Plant}
\phantomsection\label{callout-chapter-connection*-6.5}
