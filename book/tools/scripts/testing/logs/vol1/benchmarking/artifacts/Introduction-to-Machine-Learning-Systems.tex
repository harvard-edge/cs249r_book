...
\begin{tcolorbox}[enhanced jigsaw, colback=white, breakable, bottomtitle=1mm, bottomrule=.15mm, left=2mm, titlerule=0mm, leftrule=.75mm, colframe=quarto-callout-tip-color-frame, colbacktitle=quarto-callout-tip-color!10!white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Learning Objectives}, toprule=.15mm, opacityback=0]
...
\begin{fbx}{callout-definition}{Definition:}{Machine Learning Benchmarking}
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{Benchmarks as Moving Targets}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{MobileNet Deployment Validation}
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, breakable, bottomtitle=1mm, bottomrule=.15mm, left=2mm, titlerule=0mm, leftrule=.75mm, colframe=quarto-callout-note-color-frame, colbacktitle=quarto-callout-note-color!10!white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Related Efficiency Metrics}, toprule=.15mm, opacityback=0]
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1371}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.0887}}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Statistical Confidence Trap}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{Goodhart's Law in Action}
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{The Fallacy of Peak Performance}
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, breakable, bottomtitle=1mm, bottomrule=.15mm, left=2mm, titlerule=0mm, leftrule=.75mm, colframe=quarto-callout-warning-color-frame, colbacktitle=quarto-callout-warning-color!10!white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Decoding Vendor Benchmark Claims}, toprule=.15mm, opacityback=0]
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.3168}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.6832}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Vendor Claim}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{fbx}{callout-definition}{Definition:}{Machine Learning System Benchmarks}
...
\texttt{\{python\}\ a100\_tflops\_fp16} TFLOPS (trillion FLOPS) for
FP16/BF16 Tensor Core operations (624 TFLOPS with structured sparsity),
while high-end CPUs achieve 1-10 TFLOPS. FLOPS measurements help compare
hardware capabilities and determine computational bottlenecks in ML
workloads. } per byte of data movement, determines performance limits.
Consider an NVIDIA A100 GPU with \texttt{\{python\}\ a100\_tflops\_fp16}
TFLOPS of tensor performance (TF32/FP16 with Tensor Cores; FP32 is
\texttt{\{python\}\ a100\_tflops\_fp32} TFLOPS) and
\texttt{\{python\}\ a100\_bw\_tbs} TB/s memory bandwidth (SXM variant),
yielding an arithmetic intensity threshold of
\texttt{\{python\}\ a100\_ridge} FLOPS/byte. The architectural
...
performance (approximately 280 TFLOPS achieved vs
\texttt{\{python\}\ a100\_tflops\_fp16} TFLOPS theoretical)
(\citeproc{ref-nvidia2020a100}{Choquette et al. 2021}). Conversely,
low-intensity operations like activation functions and certain
lightweight operations (\textless10 FLOPS/byte) become memory bandwidth
limited, utilizing only a fraction of the GPU's computational capacity.
A BERT inference with batch size 1 achieves only 8 FLOPS/byte arithmetic
intensity, limiting performance to 16 TFLOPS (2.0 TB/s × 8 FLOPS/byte),
representing just 5\% of peak computational capability.
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{Roofline Analysis for BERT Inference}
...
\tightlist
\item
  Peak compute: \texttt{\{python\}\ a100\_tflops\_fp16} TFLOPS (Tensor
  Core, TF32/FP16)
\item
  Memory bandwidth: \texttt{\{python\}\ a100\_bw\_tbs} TB/s
\item
  Ridge point: \texttt{\{python\}\ a100\_tflops\_fp16} \(\div\)
  \texttt{\{python\}\ a100\_bw\_tbs} = \texttt{\{python\}\ a100\_ridge}
  FLOPS/byte
\end{itemize}

Any workload with arithmetic intensity below
\texttt{\{python\}\ a100\_ridge} FLOPS/byte is memory-bound; above is
compute-bound.
...
\item
  ...
  memory)
\item
  Arithmetic intensity: \(22 \times 10^9 \div 440 \times 10^6 =\)
  \texttt{\{python\}\ bert\_ai\_b1\_str} FLOPS/byte
\end{itemize}
...
Since 50 \textless{} \texttt{\{python\}\ a100\_ridge}, BERT at batch=1
is \textbf{memory-bound}:

\[\text{Achievable perf} = `{python} bert_ai_b1_str` \text{ FLOPS/byte} \times 2.0 \text{ TB/s} = `{python} bert_perf_b1_str` \text{ TFLOPS}\]

\[\text{GPU utilization} = `{python} bert_perf_b1_str` \div `{python} a100_tflops_fp16` = `{python} bert_util_b1_str`\%\]
...
\item
  New FLOPs:
  \(22 \times 10^9 \times 32 = `{python} bert_batch32_flops_str` \times 10^9\)
\item
  New intensity:
  \(`{python} bert_batch32_flops_str` \times 10^9 \div 440 \times 10^6 =\)
  \texttt{\{python\}\ bert\_ai\_b32\_str} FLOPS/byte
\end{itemize}

Since 1600 \textgreater{} \texttt{\{python\}\ a100\_ridge}, batch=32 is
\textbf{compute-bound}:

\[\text{Achievable perf} \approx 0.85 \times `{python} a100_tflops_fp16` = `{python} bert_perf_b32_str` \text{ TFLOPS}\]
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/ea67f41592c710407f74d9b26cb167147b84cb27.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/734d44c72233a9298b16784a556836989ccd6696.pdf}}
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{Micro-Benchmarking Rules}
...
\item
  ...
  TFLOPS on an H100 (peak
  \textasciitilde{}\texttt{\{python\}\ h100\_tflops\_fp16} TFLOPS for
  FP8 dense, or \textasciitilde4,000 TFLOPS FP8 with sparsity), don't
  just optimize the code---ask \emph{why} the utilization is so low. Is
  it a \textbf{Kernel Launch Latency} issue (too many small kernels)?
\item
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{Measuring the Iron Law Terms}
...
capture the ``logical'' execution flow. They answer: * ``Which layer is
taking the most time?'' * ``Are my CPU and GPU synchronized or
overlapped?'' * ``Is the data loader keeping up?'' * \emph{Key Metric}:
\textbf{Step Time Breakdown} (Data Loading vs.~Compute
vs.~Communication).
...
answer: * ``Is this matrix multiplication Compute-Bound or
Memory-Bound?'' * ``Are we hitting 100\% occupancy on the Streaming
Multiprocessors?'' * ``Are memory coalescing rules being respected?'' *
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/73de714a4d9e08d3f95b306de0dc80e05de523d2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/fe9c8e19fbac0cdbe4f715a3eb0d6224bf1ff88e.pdf}}
...
\begin{fbx}{callout-example}{Example:}{Benchmarking a Vision Model for Edge Deployment}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2317}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2805}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2683}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Precision}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
\bottomrule\noalign{}
\endlastfoot
\textbf{FP32} \textbf{INT8} & 120 ms 35 ms & 71.8\% 71.2\% & 14.0 3.5 \\
\end{longtable}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{Interpreting a Benchmark Claim}
...
decoding, on NVIDIA H100 at \texttt{\{python\}\ h100\_tdp}W TDP.''
...
\begin{fbx}{callout-example}{Example:}{Benchmarking the Edge}
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{Edge Benchmark Reality Check}
...
GPT-3\sidenote{\textbf{GPT-3}: OpenAI's 2020 language model with
\texttt{\{python\}\ gpt3\_params\_b} billion parameters, trained on 300
billion tokens using 10,000 NVIDIA V100 GPUs for several months at an
estimated cost of \$4.6 million or more (Lambda Labs estimate; actual
cost likely higher due to scaling inefficiencies). GPT-3 demonstrated
emergent abilities like few-shot learning and in-context reasoning,
establishing the paradigm of scaling laws where larger models
consistently outperform smaller ones across diverse language tasks. }
(\citeproc{ref-brown2020language}{Brown et al. 2020}), which consists of
\texttt{\{python\}\ gpt3\_params\_b} billion parameters trained on
...
\begin{fbx}{callout-definition}{Definition:}{ML Training Benchmarks}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/68fb3d31443dc92841fb394817b44afbb86866d2.pdf}}
...
model such as GPT-3, which consists of
\texttt{\{python\}\ gpt3\_params\_b} billion parameters and requires
processing terabytes of data, places enormous demands on modern
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{Scaling Efficiency Calculation}
...
\[\text{Efficiency}(8) = \frac{24 \text{ hours}}{8 \times 4 \text{ hours}} \times 100\% = \frac{24}{`{python} eff_denom_str`} = `{python} eff_str`\%\]

With perfect scaling, 8 GPUs would complete in
\texttt{\{python\}\ ideal\_str} hours (24/8). The actual 4 hours
represents \texttt{\{python\}\ eff\_str}\% efficiency.

\subsubsection*{\texorpdfstring{Step 3: Account for the
\texttt{\{python\}\ loss\_str}\%
Loss}{Step 3: Account for the \{python\} loss\_str\% Loss}}\label{step-3-account-for-the-python-loss_str-loss}
\addcontentsline{toc}{subsubsection}{Step 3: Account for the
\texttt{\{python\}\ loss\_str}\% Loss}

The ``missing'' 25\% decomposes into measurable overhead:

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3196}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2784}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Gradient synchronization} \textbf{Memory copy (CPU↔GPU)}
\textbf{Load imbalance} \textbf{Batch size effects} & 10-15\% 3-5\%
2-5\% 2-5\% & AllReduce time per step Data transfer profiling Per-GPU
step time variance Larger batches converge differently \\
\end{longtable}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{Why INT8 Saves Energy}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.4028}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2778}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Precision}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
\textbf{Multiplier Energy}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
\textbf{Relative Cost}
\end{minipage} \\
\midrule\noalign{}
...
\textasciitilde{}\texttt{\{python\}\ energy\_fp32} pJ
\textasciitilde{}\texttt{\{python\}\ energy\_fp16} pJ
\textasciitilde{}\texttt{\{python\}\ energy\_int8} pJ & 1.0× 0.3×
0.05× \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Register} \textbf{L1 Cache} \textbf{L2 Cache} \textbf{DRAM} &
\textasciitilde0.01 pJ \textasciitilde0.5 pJ \textasciitilde2 pJ
\textasciitilde10 pJ & 1× 50× 200× 1000× \\
\end{longtable}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2867}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2867}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2333}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Component}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Model load from DRAM} \textbf{Compute (300M MACs)}
\textbf{Total} & \texttt{\{python\}\ f"\{e\_fp32\_load:.0f\}"} μJ
\texttt{\{python\}\ f"\{e\_fp32\_compute:,.0f\}"} μJ
\textbf{\texttt{\{python\}\ f"\{e\_fp32\_total:,.0f\}"} μJ} &
\texttt{\{python\}\ f"\{e\_int8\_load:.0f\}"} μJ
\texttt{\{python\}\ f"\{e\_int8\_compute:.0f\}"} μJ
\textbf{\texttt{\{python\}\ f"\{e\_int8\_total:,.0f\}"} μJ} &
\texttt{\{python\}\ f"\{s\_load:.0f\}"}×
\texttt{\{python\}\ f"\{s\_compute:.0f\}"}×
\textbf{\texttt{\{python\}\ f"\{s\_total:.0f\}"}×} \\
\end{longtable}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1858}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.5354}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2743}}@{}}
...
\begin{fbx}{callout-definition}{Definition:}{ML Inference Benchmarks}
...
\pandocbounded{\includegraphics[keepaspectratio]{contents/vol1/benchmarking/benchmarking_files/figure-pdf/fig-power-differentials-1.pdf}}
...
\begin{fbx}{callout-definition}{Definition:}{SLO vs. SLA}
...
\begin{fbx}{callout-checkpoint}{Checkpoint:}{Metric Selection}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{Amdahl's Law: Optimization Ceiling}
...
where preprocessing (JPEG decode, resize, normalize) consumes
\texttt{\{python\}\ bench\_preprocess\_ms}ms and inference consumes
\texttt{\{python\}\ bench\_inference\_ms}ms. Optimizing inference by
\texttt{\{python\}\ bench\_inf\_speedup}x (from
\texttt{\{python\}\ bench\_inference\_ms}ms to
\texttt{\{python\}\ bench\_opt\_inf\_str}ms) reduces total latency from
\texttt{\{python\}\ bench\_total\_ms}ms to only
\texttt{\{python\}\ bench\_opt\_total\_str}ms, a
\texttt{\{python\}\ bench\_e2e\_str}x improvement rather than
\texttt{\{python\}\ bench\_inf\_speedup}x.
...
most \(1/f\) speedup. With preprocessing at 45\% of latency
(\(f = `{python} preprocess_fraction`\)), the maximum achievable speedup
is
\(1/`{python} preprocess_fraction` \approx `{python} amdahl_ceiling_str`\times\)
regardless of model optimization.
...
\begin{fbx}{callout-checkpoint}{Checkpoint:}{Benchmarking Methodology}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.4303}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3576}}@{}}
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{The Cost of Comprehensive Benchmarking}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{MobileNet on EdgeTPU}
...
\textasciitilde{}\texttt{\{python\}\ edgetpu\_latency\_ms}ms inference
for INT8 MobileNetV2, approximately
\texttt{\{python\}\ inference\_speedup\_str}× speedup over ARM Cortex-M7
CPU (\textasciitilde{}\texttt{\{python\}\ cpu\_latency\_ms}ms).

\textbf{Validation protocol} (SingleStream scenario):

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2621}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2136}}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2233}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Metric}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Inference latency} \textbf{End-to-end latency} \textbf{Power
consumption} \textbf{Energy per inference} & \textasciitilde15ms
\textasciitilde18ms \textasciitilde120mW \textasciitilde1.8mJ &
\textasciitilde2ms \textasciitilde6ms \textasciitilde500mW
\textasciitilde1.0mJ & 7× faster --- --- --- & ✓ \textasciitilde3×
faster \textasciitilde4× higher \textasciitilde1.8× more efficient \\
\end{longtable}

\textbf{What this reveals}: The
\texttt{\{python\}\ inference\_speedup\_str}× inference speedup is real,
but end-to-end improvement is only
\textasciitilde{}\texttt{\{python\}\ e2e\_speedup\_str}× because
preprocessing (image capture, resize, normalize) runs on the CPU in both
cases. Additionally, EdgeTPU consumes more power but completes faster,
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3243}}@{}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/60f9fda1d3b9d13aadbaa15e80e18f15678a7d31.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/14b73f9753eb14ae5d2f9942d8b2bdcdb808e716.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/53dc5f5f4e1c0731aca9ab63cf1504ff4fa5d646.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/ca969b481349e5b9bb945b203bcfcdfeb0788324.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{contents/vol1/benchmarking/benchmarking_files/figure-pdf/fig-imagenet-challenge-1.pdf}}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{MobileNet INT8 Compression}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Top-1 accuracy} \textbf{Top-5 accuracy} \textbf{Calibration ECE}
\textbf{Edge-case accuracy} & 71.8\% 91.0\% 0.031 68.2\% & 70.9\% 90.4\%
0.089 61.4\% & ✓ (\textless2\% drop) ✓ ⚠ (degraded) ⚠ (10\% drop) \\
\end{longtable}
...
\item
  ...
  tokens/second means \texttt{\{python\}\ slow\_str} seconds vs
  \texttt{\{python\}\ fast\_str} seconds.
\item
...
\textbf{Coverage Metrics: What Does the Data Represent?} Coverage
metrics assess whether training data spans the input space the model
will encounter in deployment.

\textbf{Class balance} measures the distribution of labels. Severe
imbalance (99\% negative, 1\% positive) causes models to learn the
majority class while ignoring minorities, achieving high accuracy by
being useless. Imbalance ratios above 10:1 typically require mitigation
(oversampling, class weighting, or threshold adjustment). More subtle
imbalance across subgroups within classes creates demographic
disparities invisible to overall class balance.

\textbf{Feature coverage} assesses whether the training data spans the
relevant input dimensions. A model trained on daytime images will fail
on nighttime inputs; a model trained on formal text will fail on
colloquial language. Feature coverage is harder to quantify than class
balance; it requires domain knowledge about what input variations matter
for deployment.

\textbf{Demographic representation} measures coverage across
human-relevant subgroups. For applications affecting people, training
data must represent the deployment population across relevant
demographic dimensions (age, gender, ethnicity, geography, language).
Systematic underrepresentation of any group creates systematic
underperformance for that group, even if aggregate metrics look
acceptable.

\textbf{Quality Metrics: Can the Labels Be Trusted?} Quality metrics
assess whether the training signal itself is reliable.

\textbf{Label accuracy} measures the fraction of correctly labeled
examples. Studies consistently find 3-6\% label error rates in major
datasets, including ImageNet
(\citeproc{ref-northcutt2021pervasive}{Northcutt, Athalye, and Mueller
2021}). These errors become learned ground truth: models trained on
noisy labels reproduce those errors. For small datasets, manual audit of
a random sample estimates label accuracy. For large datasets, confident
learning techniques identify likely mislabeled examples.

\textbf{Inter-annotator agreement} measures consistency across human
labelers. Cohen's kappa or Fleiss' kappa quantify agreement beyond
chance. Low agreement indicates either ambiguous labeling guidelines,
inherently subjective tasks, or labeler quality problems. Agreement
below 0.6 on tasks with clear ground truth suggests quality issues;
agreement below 0.4 is problematic for any supervised learning
application.

\textbf{Systematic vs random errors} have different consequences. Random
label noise is partially averaged out during training; systematic errors
(consistently mislabeling a particular subclass) are learned as ground
truth. A dataset where ``wolves'' are systematically labeled ``dogs''
will produce a model that calls wolves dogs, and no amount of additional
data fixes this without correcting the systematic error.

\textbf{Distribution Alignment: Will It Generalize?} Distribution
metrics assess whether training data distributions match deployment
conditions.

\textbf{Train/test alignment} is the standard assumption: held-out test
data comes from the same distribution as training data. This assumption
is often violated in practice. Test sets constructed years after
training data may reflect distribution drift; test sets from different
geographic regions may reflect population shift. Standard held-out
evaluation overestimates deployment performance when this assumption
fails.

\textbf{Train/production alignment} is the true test. Production data
often differs from training data in ways that held-out test sets do not
capture: different cameras, different user populations, different time
periods, different edge cases. The WILDS\sidenote{\textbf{WILDS}: A
curated benchmark of 10 datasets spanning distribution shifts
encountered in real-world ML deployments, introduced by Stanford
researchers in 2021. WILDS tests include hospital patient population
shifts (Camelyon17), wildlife camera location changes (iWildCam), and
satellite imagery temporal drift (PovertyMap). The benchmark quantifies
the gap between in-distribution and out-of-distribution performance,
revealing that models achieving 97\% in-distribution accuracy can drop
to 70\% under realistic shifts. } benchmark
(\citeproc{ref-koh2021wilds}{Koh et al. 2021}) evaluates models under
realistic distribution shifts---hospital systems (different patient
populations), wildlife cameras (different locations), satellite imagery
(different time periods). Models achieving 90\%+ on in-distribution test
sets may drop to 60\% under these shifts.

\textbf{Shift detection} methods identify when production distributions
diverge from training distributions. Statistical tests (KS test, MMD) on
input features can detect covariate shift. Monitoring model confidence
distributions can detect when the model encounters unfamiliar inputs.
Early detection enables intervention before performance degrades
catastrophically.
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/af1f74809b5a8598f7c04ce66db75cf1e01b84d7.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/3e7bd85853dd5e7be61a2cb778024d441bf7cfb0.pdf}}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1441}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2793}}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3113}}
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, breakable, bottomtitle=1mm, bottomrule=.15mm, left=2mm, titlerule=0mm, leftrule=.75mm, colframe=quarto-callout-important-color-frame, colbacktitle=quarto-callout-important-color!10!white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Pre-Deployment Benchmark Checklist}, toprule=.15mm, opacityback=0]
