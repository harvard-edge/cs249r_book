...
\begin{tcolorbox}[enhanced jigsaw, toptitle=1mm, colback=white, leftrule=.75mm, colbacktitle=quarto-callout-tip-color!10!white, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Learning Objectives}, opacityback=0, left=2mm, breakable, opacitybacktitle=0.6, bottomrule=.15mm, coltitle=black, bottomtitle=1mm, arc=.35mm, titlerule=0mm, rightrule=.15mm, toprule=.15mm, colframe=quarto-callout-tip-color-frame]
...
KB of memory cannot hold a model that requires 300 GB. These are not
...
as illustrated in Figure~\ref{fig-cloud-edge-TinyML-comparison} and
summarized in Table~\ref{tbl-deployment-paradigms-overview}.

\begin{figure}[t]

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/e871e41f8903ea9bf7328cb3a37c320e1fdaacbc.pdf}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.2615}}
...
inference, privacy \\
\textbf{Mobile ML} & Smartphones & 5-50 ms & 3-5 W & GB & Personal AI,
offline \\
...
\begin{fbxSimple}{callout-checkpoint}{Checkpoint:}{Physical Constraints and Deployment}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{The Energy of Transmission}
...
\tightlist
\item
  \textbf{Data (\(D_{vol}\))}: 1 MB (e.g., 1 second of audio).
\item
...
\item
  ...
  \(E_{cloud} \approx D_{vol} \times E_{tx} = 1 \text{ MB} \times 100 \text{ mJ/MB} = \mathbf{100 \text{ mJ}}\).
\item
...
The Iron Law
(\(T = \frac{D_{vol}}{BW} + \frac{O}{R_{peak} \cdot \eta} + L_{lat}\))
governs all ML performance. For a formal dimensional analysis and
physical derivation of this equation, see
...
\tightlist
\item[$\square$]
  \textbf{Bandwidth (\(BW\))}: Is your problem memory-bound? (Does
  increasing compute power \(R_{peak}\) fail to speed it up?).
\item[$\square$]
  \textbf{Overhead (\(L_{lat}\))}: Why does batching improve throughput?
  (It amortizes the fixed overhead \(L_{lat}\) across many samples).
\end{itemize}
...
\begin{fbxSimple}{callout-lighthouse}{Lighthouse:}{Five Reference Workloads}
\phantomsection\label{callout-lighthouse*-1.4}
Throughout this book, we use five \textbf{Lighthouse Models} introduced
in \textbf{?@sec-introduction}---concrete workloads that span the
deployment spectrum and isolate distinct system bottlenecks.
...
practice, we analyze the \textbf{Lighthouse Models} introduced in
...
but uses depthwise separable convolutions to reduce computation by 8-9×,
enabling real-time inference on smartphones at 3-5 watts.
...
the book, each isolating a distinct system bottleneck that we will
revisit in every chapter.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2809}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3596}}@{}}
...
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{System Balance Across Paradigms}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1620}}
...
duty cycling \\
\textbf{TinyML} & \(D_{vol}/\text{Capacity}\) & 256 KB total; model must
fit on-chip & Extreme compression, binary networks \\
\end{longtable}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.4331}}@{}}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{ResNet-50 on Cloud vs Mobile}
...
\textbf{Given} (from Lighthouse Models):
...
\item
  Compute time: \(T_{comp}\) =
  \(\frac{4.10 \times 10^{9}}{3.12 \times 10^{14}}\) = 0.013 ms
\item
  Memory time: \(T_{mem}\) =
  \(\frac{5.12 \times 10^{7}}{2.04 \times 10^{12}}\) = 0.025 ms
\item
  \textbf{Bottleneck}: Memory (2× slower than compute)
\item
  \textbf{Arithmetic Intensity}:
  \(\frac{4.10 \times 10^{9}}{5.12 \times 10^{7}}\) = 80 FLOPs/byte
\end{itemize}
...
\item
  Compute time: \(T_{comp}\) =
  \(\frac{4.10 \times 10^{9}}{3.50 \times 10^{13}}\) = 0.12 ms
\item
  Memory time: \(T_{mem}\) =
  \(\frac{2.56 \times 10^{7}}{1.00 \times 10^{11}}\) = 0.26 ms
\item
  \textbf{Bottleneck}: Memory (2× slower than compute)
\end{itemize}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0321}}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.1012}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.1407}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0840}}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0765}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0716}}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0716}}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.1531}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.2691}}@{}}
...
1-10 TOPS compute, \textless2W sustained power, \textless5-50ms UI
response \\
...
spanning multiple data centers. This book focuses on single-machine
...
\begin{fbxSimple}{callout-definition}{Definition:}{Cloud ML}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/3c2022b706d29d90df765ffe68bf8da66f68471c.pdf}}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{The Distance Penalty}
...
\item
  ...
  km/s.
\item
  \textbf{Round-trip Propagation}: (1,500 km × 2) / 200,000.0 kilometer
  / second km/s = \textbf{15 ms}.
\item
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{Cloud vs. Edge TCO}
\phantomsection\label{callout-notebook*-1.9}
\textbf{Scenario}: A vision system serving 1M daily inferences
(ResNet-50 scale, 10ms latency, 100KB response).

\textbf{Cloud Implementation} (AWS/GCP pricing, 2024)

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1846}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.6154}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{GPU inference (A10G)} & 4 instances × 8,760 hrs × \$0.75/hr &
\textasciitilde\$26,280 \\
\textbf{Network egress} & 95 GB/day × 365 × \$0.09/GB &
\textasciitilde\$3,133 \\
\textbf{Load balancer} & \$0.025/hr + LCU charges &
\textasciitilde\$3,723 \\
\textbf{CloudWatch/logging} & Monitoring, alerts &
\textasciitilde\$2,000 \\
\textbf{Total Cloud} & & \textbf{\textasciitilde\$35,136/year} \\
\end{longtable}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.6102}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2203}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Cost Component}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
\textbf{DevOps labor} & 0.1 FTE × \$150,000 salary & \$15,000.0 \\
\textbf{Total Edge} & & \textbf{\textasciitilde\$21,610/year} \\
\end{longtable}
...
\textasciitilde38\%. The crossover occurs around \textbf{60\% sustained
utilization}.

\textbf{Key insight}: Edge TCO is dominated by \textbf{labor} (69\%),
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{Wake-Word Economics}
...
\begin{fbxSimple}{callout-definition}{Definition:}{Edge ML}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/6d18a131808c14bdfa87b47270091f295fcef57a.pdf}}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{The Bandwidth Bottleneck}
...
\tightlist
\item
  \textbf{Raw data rate per camera}: 1920 × 1080 × 3 byte bytes × 30 FPS
  ≈ \textbf{187 MB/s}.
\item
  \textbf{Total data rate}: 100 cameras × 187 MB/s = \textbf{18.7 GB/s}.
\item
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{The 1000× Energy Gap}
...
\tightlist
\item
  \textbf{Transmission (5G)}: Sending 1MB consumes ≈ \textbf{100 mJ}
  (milli-Joules).
\item
  \textbf{Computation (Edge NPU)}: Running a MobileNet inference on that
  1MB consumes ≈ \textbf{0.1 mJ}.
\item
  \textbf{The Gap}: Transmitting data is \textbf{1000× more
...
transmission would kill the device's battery 1000× faster than Edge ML.
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{Edge Inference Sizing}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.5221}}
...
\begin{fbxSimple}{callout-definition}{Definition:}{Mobile ML}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/3c5f6ce58ce5ed74126dd16587c59b5c977c627f.pdf}}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{The Battery Tax}
...
\tightlist
\item
  \textbf{Ideal Runtime}: \(\frac{15 Wh}{2 W}\) = \textbf{7.5 hours}
\item
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{The Thermal Wall}
...
intermediate constraints: 8-16 GB RAM (varying from mid-range to
flagship), 128 GB-1 TB storage, 1-10 TOPS AI compute through Neural
...
\textbf{?@sec-model-compression} details. Battery constraints (15-22Wh
...
\textless5-50 ms UI response times.
...
to \textless100 KB (fitting in on-chip SRAM), driving innovation in
...
\begin{fbxSimple}{callout-definition}{Definition:}{TinyML}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{Energy Per Inference}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3810}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Paradigm}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/29de7c975adb6a6ce2efd0275ac0d6fb729d036d.pdf}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1559}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2849}}@{}}
...
\textbf{Latency} & 100 ms-1000 ms+ & 10-100 ms & 5-50 ms & 1-10 ms \\
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/b7c1e513ba743f3321d2066a8aad81ac39e22b4c.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/c593ab7dcf9ec852e669be59a70ccccc0323da46.pdf}}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{Autonomous Vehicle Emergency Braking}
...
\phantomsection\label{callout-perspectiveux2a-1.21}
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{The Complexity Tax}
\phantomsection\label{callout-perspective*-1.21}
...
\begin{fbxSimple}{callout-checkpoint}{Checkpoint:}{System Design}
...
\begin{fbxSimple}{callout-definition}{Definition:}{Hybrid ML}
...
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{Pattern Selection Guide}
...
which enable privacy-preserving distributed training across devices,
constitute advanced topics that build on the single-node foundations
covered here.
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/da4e66cf8aefcffb0a91c15d849021f509c82b44.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/e789af694ee32f51833f407aa51f078d81015ef1.pdf}}
...
The remaining chapters explore each layer:
...
\begin{fbxSimple}{callout-example}{Example:}{The Latency Mirage}
\phantomsection\label{fallacy-edge-deployment-always-reduces-latency-compared-to-cloud.}
...
versus mobile devices with 4--12 GB, a 10,000x difference requiring
