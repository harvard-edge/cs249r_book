...
\textbf{similarity}. In Attention mechanisms:
...
\item
  \textbf{Small matrices are inefficient}: A 64×64 GEMM has intensity
  64/3 ≈ 21 FLOP/byte, well below the ridge point (153), achieving only
  \textasciitilde13\% of peak throughput.
\end{itemize}
...
\textbf{Example}: A vocabulary embedding matrix of size 100,000×10,000
(1 billion parameters).

\begin{itemize}
\tightlist
\item
  \textbf{Dense (FP32)}: 1 × 10⁹ × 4 bytes = \textbf{4 GB}.
\item
  \textbf{Sparse (1\% density)}: Storing only non-zeros requires roughly
  10 × 10⁶ × (4 bytes value + 4 bytes index) ≈ \textbf{80 MB}.
\item
  \emph{Result}: A 50x reduction in memory footprint, fitting a model
  that would otherwise OOM (Out of Memory).
\end{itemize}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3306}}@{}}
...
\textbf{Attention (Single Head)} & \((B, S, d_{model})\) &
\(4 \times d_{model}^2\) & \(2 S^2 d_{model} + 8 S d_{model}^2\) \\
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/27fcd285b994bb681da452052095bd0926a4e68f.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/a72f80d375afab27e615ff49424c5703fd3d89de.pdf}}
...
\tightlist
\item
  \textbf{Weights}: 2 bytes (FP16/BF16) or 4 bytes (FP32).
\item
  \textbf{Gradients}: Same size as weights.
\item
  \textbf{Optimizer State}: 8-12 bytes per parameter (Momentum +
  Variance + FP32 Master Weights).
\item
  \textbf{Activations}: The hidden giant.
  \(O(Batch \times Sequence \times Layers \times Width)\).
\end{itemize}
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, arc=.35mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Pitfall: Assuming sparse matrices always save memory.}, toprule=.15mm, rightrule=.15mm, toptitle=1mm, colbacktitle=quarto-callout-warning-color!10!white, left=2mm, leftrule=.75mm, titlerule=0mm, breakable, coltitle=black, bottomtitle=1mm, colframe=quarto-callout-warning-color-frame, colback=white, opacitybacktitle=0.6, opacityback=0]
