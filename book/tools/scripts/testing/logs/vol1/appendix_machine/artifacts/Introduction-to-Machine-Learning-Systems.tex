...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, leftrule=.75mm, toprule=.15mm, bottomtitle=1mm, colbacktitle=quarto-callout-note-color!10!white, breakable, opacitybacktitle=0.6, rightrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-note-color-frame, toptitle=1mm, opacityback=0, titlerule=0mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{A Note on Terminology: GPUs and Accelerators}, arc=.35mm, bottomrule=.15mm]

Throughout this book, we often use ``GPU'' when discussing hardware
acceleration. However, the principles---roofline analysis, memory
hierarchies, numerical precision, and performance modeling---apply
equally to \textbf{TPUs}, \textbf{NPUs}, \textbf{custom ASICs}, and
other specialized AI accelerators. We use ``GPU'' as convenient
shorthand given its prevalence in the ML ecosystem, but readers should
understand these concepts as \textbf{accelerator-general} unless we
explicitly discuss vendor-specific features (e.g., CUDA, NVLink).
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/781aef4f441145e91857215747cdc224c475dacb.pdf}}
...
\textbf{GEMM (Matrix Multiplication)}: For two 4096×4096 matrices,
arithmetic intensity is approximately 1365 FLOP/byte. Since 1365
\textgreater{} 153, this operation is compute-bound. You are using the
hardware efficiently.

\textbf{ReLU (Element-wise)}: For a 4096×4096 tensor, intensity is
approximately 0.25 op/byte. Since 0.25 ≪ 153, this operation is severely
memory-bound, achieving only about 0.16\% of peak TFLOP/s. The hardware
is mostly waiting for data.
...
\subsection{Dimensional
Analysis}\label{sec-system-foundations-dimensional-analysis}

Dimensional analysis is a fundamental engineering technique for
verifying that equations are physically meaningful. Any valid
performance equation must be \textbf{dimensionally homogeneous}---every
term must resolve to the same units. If they do not, the equation
contains an error.

Consider the \textbf{Iron Law of ML Systems} introduced in
\textbf{?@sec-silicon-contract}:

\[ T = \frac{D_{vol}}{BW} + \frac{O}{R_{peak} \cdot \eta} + L_{lat} \]

We verify correctness by proving that every term resolves to
\textbf{Time (seconds)}:
...
\item
  \textbf{Overhead Term}: Already in seconds.
\end{itemize}

The equation is physically consistent. This technique applies to any
systems equation you encounter: if the dimensions do not match, the
formula is wrong. Any optimization you perform must also respect these
dimensions---you cannot trade ``FLOPs'' for ``Bandwidth'' without
converting through Time.
...
\item
  With \(n=8\), speedup is \(\frac{1}{0.05 + 0.95/8}\) ≈ 5.9×.
\item
  With \(n=\infty\), speedup is capped at \(1/0.05\) = 20×.
\end{itemize}

No matter how many accelerators you buy, you cannot make this fixed
workload run faster than 20×.
...
\item
  With \(n=1\), speedup is 1.
\item
  With \(n=8\), Scaled Speedup is \(8 - 0.05 \times (7) = 8 - 0.35\) =
  7.65×.
\item
  With \(n=1000\), Scaled Speedup is \(1000 - 0.05 \times (999)\) ≈
  950×.
\end{itemize}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Training Time Equation}
...
\tightlist
\item
  \textbf{\(6\)}: The factor deriving from the forward pass (\(2PD\))
  and backward pass (\(4PD\)) FLOPs per token.
\item
  \textbf{\(P\)}: Number of model parameters.
\item
  \textbf{\(D\)}: Number of training tokens.
\item
  \textbf{\(N\)}: Number of accelerators (GPUs).
\item
  \textbf{\(X\)}: Peak FLOP/s of one accelerator.
\item
  \textbf{\(U\)}: Model FLOPS Utilization (MFU), typically 30\%--50\%.
\end{itemize}
...
The computed result: \textbf{961538 seconds} (≈ 16026 minutes, or about
11 days).
...
average latency, your system must support 1000 × 0.05 = 50 concurrent
requests.
...
requests requires 50 GB of memory. If your accelerator only has 24 GB,
you are physically limited to 24 concurrent requests. Your maximum
throughput is capped at \(L/W = 24 / 0.05 = 480\) QPS, regardless of how
...
\bottomrule\noalign{}
\endlastfoot
\textbf{L1 Cache / Register} & \textasciitilde1 ns & 3-4 cycles & 1
minute \\
\textbf{L2 Cache} & \textasciitilde4 ns & 12 cycles & 4 minutes \\
\textbf{HBM3 (GPU Memory)} & \textasciitilde300 ns & 1,000 cycles & 5
hours \\
\textbf{NVLink (GPU-GPU)} & \textasciitilde500 ns & 1,500 cycles & 8
hours \\
\textbf{PCIe (CPU-GPU)} & \textasciitilde1000 ns & 3,000 cycles & 1
day \\
\textbf{InfiniBand (Network)} & \textasciitilde5000 ns & 15,000 cycles &
1 week \\
\textbf{SSD (NVMe)} & \textasciitilde100000 ns & 300,000 cycles & 3
months \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{FP16/BF16 Peak} & 989 TFLOPS & 459 TFLOPS & The ``Speed Limit''
(\(R_{peak}\)) \\
\textbf{Memory Bandwidth} & 3.35 TB/s & 2.76 TB/s & The ``Width of the
Pipe'' (\(BW\)) \\
\textbf{HBM Capacity} & 80 GB & 95 GB & Max Model Size (\(P\)) / Batch
Size (\(B\)) \\
\textbf{L2/SRAM Cache} & 50 MB & \textasciitilde100 MB & Critical for
Operator Fusion \\
\textbf{Interconnect} & 900 GB/s (NVLink) & 1600 GB/s (ICI) & Determines
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/3faca4af281ecd220b4fa56f3b0a9a6ddeaea3a6.pdf}}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The High Cost of Data Movement}
\phantomsection\label{callout-notebook*-1.3}
Fetching a 32-bit value from DRAM costs roughly \textbf{1000× more
energy} than performing a floating-point operation on it (e.g.,
...
\item
  ...
  \tightlist
  \item
    Transmission: 1KB / 10Gbps ≈ 0.8 μs.
  \item
    Total Time ≈ 10ms + 0.8μs ≈ 10ms.
  \item
...
\bottomrule\noalign{}
\endlastfoot
\textbf{FP32} & 32 & 8 & 23 & \(\sim 10^{-38}\) to \(10^{38}\) &
Training (full precision), reference inference \\
\textbf{FP16} & 16 & 5 & 10 & \(\sim 10^{-5}\) to \(6.5 \times 10^{4}\)
& Training with loss scaling, inference \\
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/db9330eec9357de0291f040ba0589ee2327d832f.pdf}}
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{The Dynamic Range Wall}
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, leftrule=.75mm, toprule=.15mm, bottomtitle=1mm, colbacktitle=quarto-callout-warning-color!10!white, breakable, opacitybacktitle=0.6, rightrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-warning-color-frame, toptitle=1mm, opacityback=0, titlerule=0mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Fallacy: Doubling GPUs halves training time.}, arc=.35mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, leftrule=.75mm, toprule=.15mm, bottomtitle=1mm, colbacktitle=quarto-callout-warning-color!10!white, breakable, opacitybacktitle=0.6, rightrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-warning-color-frame, toptitle=1mm, opacityback=0, titlerule=0mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Fallacy: Higher precision (FP32) is always better.}, arc=.35mm, bottomrule=.15mm]
