...
\chapter{Notation and Conventions}\label{notation-and-conventions}

This book spans two distinct disciplines: \textbf{Machine Learning}
(computer science/statistics) and \textbf{Systems} (computer
architecture/hardware). Each field often reuses variables for different
concepts. To avoid ambiguity, this text adheres to the following
standard notation.

\section{The Iron Law of ML Systems}\label{the-iron-law-of-ml-systems}

The fundamental performance equation of this book
(\textbf{?@sec-silicon-contract}) uses the following specific variables:
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.0449}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.7921}}@{}}
\toprule\noalign{}
...
\textbf{Unit}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Systems Context}
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{\(T\)} & \textbf{Time} & seconds & The total wall-clock time for
an operation. \\
\textbf{\(D_{vol}\)} & \textbf{Data Volume} & bytes & The total amount
of data moved (e.g., model weights + activations). Distinguished from
dataset size. \\
\textbf{\(BW\)} & \textbf{Bandwidth} & bytes/s & The sustained transfer
rate of the memory or network link. \emph{(Note: Often denoted as \(B\)
in physics, but we reserve \(B\) for Batch Size)}. \\
\textbf{\(O\)} & \textbf{Operations} & FLOPs & Total floating-point
operations required. \\
\textbf{\(R_{peak}\)} & \textbf{Peak Rate} & FLOP/s & The theoretical
maximum throughput of the hardware. \emph{(Note: Often denoted as \(P\)
in roofline models, but we reserve \(P\) for Parameters)}. \\
\textbf{\(\eta\)} & \textbf{Efficiency} & \% & Hardware utilization
factor (\(0 \le \eta \le 1\)). \\
\textbf{\(L_{lat}\)} & \textbf{Latency} & seconds & Fixed overhead time
(kernel launch, network ping). \\
\end{longtable}

\section{Deep Learning Notation}\label{deep-learning-notation}

We follow standard deep learning conventions (Goodfellow et al.) with
explicit disambiguation for systems variables.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7040}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Symbol}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Definition}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Dimensions / Type}
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{\(B\)} & \textbf{Batch Size} & Integer. The number of samples
processed in parallel. \\
\textbf{\(P\)} & \textbf{Parameters} & Integer. The total count of
trainable weights in a model. \\
\textbf{\(S\)} & \textbf{Sequence Length} & Integer. Number of tokens or
time steps. \\
\textbf{\(d\)} & \textbf{Hidden Dimension} & Integer. Size of the hidden
state vector. \\
\textbf{\(\mathcal{L}\)} & \textbf{Loss Function} & Scalar. The
objective function minimized during training. \\
\textbf{\(\eta\)} & \textbf{Learning Rate} & Scalar. Step size for the
optimizer (overloaded with efficiency, context distinguishes). \\
\textbf{\(\theta\)} & \textbf{Model Weights} & Vector/Matrix. The set of
all learnable parameters. \\
\end{longtable}

\section{Units and Precision}\label{units-and-precision}

\begin{itemize}
\tightlist
\item
  \textbf{Storage}: We use industry-standard decimal prefixes (KB, MB,
  GB, TB) throughout the book for consistency with common ML literature.
  While memory is technically addressed in binary units (1024-based), we
  follow industry convention where ``80 GB GPU'' means 80 × \(1024^3\)
  bytes.

  \begin{itemize}
  \tightlist
  \item
    1 KB = 1000 bytes (industry convention, also used for
    \textasciitilde1024 bytes in memory contexts)
  \item
    1 GB = \(10^9\) bytes (storage/network) or \(\approx 2^{30}\) bytes
    (memory, per industry convention)
  \end{itemize}
\item
  \textbf{Compute}: We use decimal prefixes for operations.
...
\item
  ...
  \tightlist
  \item
    \textbf{FP32}: Single precision (4 bytes)
  \item
    \textbf{FP16}: Half precision (2 bytes, standard range)
  \item
    \textbf{BF16}: Brain float (2 bytes, wide dynamic range)
  \item
    \textbf{INT8}: 8-bit integer (1 byte)
  \end{itemize}
\end{itemize}

\section{Common Collisions to Avoid}\label{common-collisions-to-avoid}

Be careful with these overloaded terms that appear in both ML and
Systems contexts:

\begin{itemize}
\tightlist
\item
  \textbf{\(B\)}: Usually \textbf{Batch Size}, never Bandwidth (use
  \(BW\)).
\item
  \textbf{\(P\)}: Usually \textbf{Parameters}, never Peak FLOPs (use
  \(R_{peak}\)).
\item
  \textbf{\(D\)}: Usually \textbf{Dataset Size} (number of
  samples/tokens), not Data Volume in bytes (use \(D_{vol}\)).
\item
  \textbf{\(M\)}: Often used for Model Size (bytes) or simply
  ``Millions''. Context is key.
\end{itemize}
