...
\begin{tcolorbox}[enhanced jigsaw, breakable, colback=white, leftrule=.75mm, opacitybacktitle=0.6, toptitle=1mm, bottomrule=.15mm, arc=.35mm, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Learning Objectives}, colbacktitle=quarto-callout-tip-color!10!white, titlerule=0mm, rightrule=.15mm, opacityback=0, bottomtitle=1mm, toprule=.15mm, left=2mm, coltitle=black]
...
unpredictable load. The \textbf{Latency term}
(\(\text{Latency}_{\text{fixed}}\)), the irreducible overhead of request
scheduling, network round-trips, and system orchestration, becomes the
dominant constraint rather than a rounding error. This inversion, from
throughput to \emph{latency}, from controlled conditions to
\emph{unpredictable traffic}, from offline to \emph{real time}, defines
the serving challenge. \textbf{?@sec-benchmarking-ai} established
techniques for measuring performance under controlled conditions;
serving faces traffic patterns that no benchmark could anticipate.
\textbf{?@sec-model-compression} provided quantization methods that
reduced model size; serving must validate that those optimizations
preserve accuracy under real traffic distributions. This fundamental
shift in constraints creates what we call the \emph{serving inversion}.

\phantomsection\label{callout-perspectiveux2a-1.1}
\begin{fbx}{callout-perspective}{Systems Perspective:}{The Serving Inversion}
...
\begin{fbx}{callout-example}{Example:}{The 'Black Friday' Traffic Spike}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/81ef38cdcd5068939ac182c8623dc3e256194f8b.pdf}}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Cost of Latency}
\phantomsection\label{notebook-cost-latency}
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{ResNet-50 Across the Serving Spectrum}
\phantomsection\label{perspective-resnet-serving}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/ea6d5788396b0ce6b91a21c76848a9b7991f8794.pdf}}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{JSON vs Protobuf Serialization}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{ResNet-50: Latency Budget Breakdown}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Preprocessing} & JPEG decode &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.5229} + 2\tabcolsep}@{}}{%
3.0ms \textbar{} 30\% \textbar{}} \\
\textbf{Preprocessing} & Resize to 224×224 &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.5229} + 2\tabcolsep}@{}}{%
1.0ms \textbar{} 10\% \textbar{}} \\
\textbf{Preprocessing} & Normalize (mean/std) &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.5229} + 2\tabcolsep}@{}}{%
0.5ms \textbar{} 5\% \textbar{}} \\
\textbf{Data Transfer} & CPU→GPU copy &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.5229} + 2\tabcolsep}@{}}{%
0.5ms \textbar{} 5\% \textbar{}} \\
\textbf{Inference} & \textbf{ResNet-50 forward pass} &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.5229} + 2\tabcolsep}@{}}{%
\textbf{5.0ms} \textbar{} \textbf{50\%} \textbar{}} \\
\textbf{Postprocessing} & Softmax + top-5 &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.5229} + 2\tabcolsep}@{}}{%
0.1ms \textbar{} \textasciitilde1\% \textbar{}} \\
\textbf{Total} & &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.5229} + 2\tabcolsep}@{}}{%
\textbf{10.1ms} \textbar{} \textbf{100\%} \textbar{}} \\
\end{longtable}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{Lighthouse Example: DLRM Serving}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1650}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Phase}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Input Parsing} & Request parsing &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.4757} + 2\tabcolsep}@{}}{%
0.5ms \textbar{} CPU \textbar{}} \\
\textbf{Embedding Look} & \textbf{Fetch 100+ dense vectors} &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.4757} + 2\tabcolsep}@{}}{%
\textbf{6.0ms} \textbar{} \textbf{Memory BW} \textbar{}} \\
\textbf{Inference} & MLP forward pass & 1.5ms \textbar{} Compute & \\
\textbf{Postprocessing} & Ranking \& Filtering &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.4757} + 2\tabcolsep}@{}}{%
1.0ms \textbar{} CPU \textbar{}} \\
\textbf{Total} & &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.4757} + 2\tabcolsep}@{}}{%
\textbf{9.0ms} \textbar{} \textbar{}} \\
\end{longtable}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Quantitative Approach to Serving}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1939}}
...
(\citeproc{ref-barroso2017attack}{Barroso et al. 2017}). \ldots{} The
latency budget framework provides a systematic approach to optimization.
First, measure each phase to identify the true bottleneck. Then allocate
engineering effort proportionally to \emph{where} time is actually
spent. Finally, consider architectural changes such as GPU preprocessing
or batching strategies that can shift work between phases.
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/b87c10521aa6120488b45c46fa4a94a9d15bbeeb.pdf}}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{Little's Law}
\phantomsection\label{notebook-littles-law}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2911}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3544}}@{}}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{ResNet-50 Capacity Planning}
...
\begin{fbx}{callout-example}{Example:}{ResNet-50: Image Preprocessing Skew}
...
\textbf{\emph{Cold Start}}\sidenote{\textbf{Cold Start}: A metaphor
borrowed from automotive engineering. Internal combustion engines
operate inefficiently and suffer high wear until they reach thermal
equilibrium. Similarly, ML inference systems suffer high latency during
their ``warm-up'' phase (loading weights, compiling kernels via JIT, and
populating caches) before they can serve traffic at peak efficiency. }
is the \textbf{Initialization Latency} incurred when instantiating a new
model replica. It represents the fixed cost of \textbf{State Hydration}
(loading weights, compiling graphs) that effectively blocks the system's
ability to scale elastically in response to traffic bursts.
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{ResNet-50: Cold Start Timeline}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Weight loading (SSD)} &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7009} + 2\tabcolsep}@{}}{%
0.5s \textbar{} 98MB FP32 weights from local storage \textbar{}} \\
\textbf{Weight loading (S3)} &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7009} + 2\tabcolsep}@{}}{%
3-5s \textbar{} Network latency dominates for cloud storage
\textbar{}} \\
\textbf{CUDA context} &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7009} + 2\tabcolsep}@{}}{%
0.3-0.5s \textbar{} GPU driver initialization and memory setup
\textbar{}} \\
\textbf{TensorRT compilation} &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7009} + 2\tabcolsep}@{}}{%
15-30s \textbar{} Converts PyTorch model to optimized engine
\textbar{}} \\
\textbf{Warmup (10 inferences)} &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7009} + 2\tabcolsep}@{}}{%
0.2s \textbar{} Triggers remaining lazy initialization \textbar{}} \\
\textbf{Total (local, optimized)} &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7009} + 2\tabcolsep}@{}}{%
\textbf{\textasciitilde1.5s} \textbar{} With pre-compiled TensorRT
engine, warm container \textbar{}} \\
\textbf{Total (cloud, first deploy)} &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7009} + 2\tabcolsep}@{}}{%
\textbf{\textasciitilde35s} \textbar{} Including compilation from cold
state \textbar{}} \\
\end{longtable}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{ResNet-50 Batching Efficiency}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Batching Sweet Spot}
...
The following plot (Figure~\ref{fig-throughput-latency-knee}) visualizes
this trade-off, showing the \textbf{``Knee''} of the curve. This is the
optimal operating point where throughput is maximized before latency
spikes due to queuing.
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{Why Latency Spikes Under Load}
...
size 1 to 32 (64 to 936 img/s), but total latency more than doubles
...
The throughput gains in Table~\ref{tbl-batching-throughput} trace
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Iron Law of Batching Efficiency}
...
\item
  ...
  Efficiency \(\approx 80\%\). The GPU is crunching numbers.
\end{itemize}
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{Practitioner's Perspective: The Latency-Throughput Trade-off}
...
\begin{fbx}{callout-perspective}{Systems Perspective:}{LLM Serving: Beyond the Fundamentals}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{Multi-Camera Autonomous Vehicle Serving}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{ResNet-50: Mobile Serving}
\phantomsection\label{callout-notebook*-1.26}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2252}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2883}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2883}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Camera buffer read} &
\multicolumn{3}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.7568} + 4\tabcolsep}@{}}{%
8ms \textbar{} 0.08mJ \textbar{} System API \textbar{}} \\
\textbf{JPEG decode (CPU)} &
\multicolumn{3}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.7568} + 4\tabcolsep}@{}}{%
15ms \textbar{} 1.5mJ \textbar{} Single-threaded \textbar{}} \\
\textbf{Resize + Normalize} &
\multicolumn{3}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.7568} + 4\tabcolsep}@{}}{%
5ms \textbar{} 0.4mJ \textbar{} CPU preprocessing \textbar{}} \\
\textbf{NPU inference} &
\multicolumn{3}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.7568} + 4\tabcolsep}@{}}{%
12ms \textbar{} 0.8mJ \textbar{} 82\% utilization \textbar{}} \\
\textbf{Post-process + UI} &
\multicolumn{3}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.7568} + 4\tabcolsep}@{}}{%
5ms \textbar{} 0.2mJ \textbar{} Result rendering \textbar{}} \\
\textbf{Total} &
\multicolumn{3}{>{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.7568} + 4\tabcolsep}@{}}{%
\textbf{45ms} \textbar{} \textbf{3.0mJ} \textbar{} 22 FPS sustained
\textbar{}} \\
\end{longtable}
...
\begin{fbx}{callout-lighthouse}{Lighthouse:}{LLM Serving Latency Targets}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{The Carbon Cost of a Chat}
\phantomsection\label{notebook-carbon-chat}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{ResNet-50: Runtime Comparison}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{ResNet-50: Precision Tradeoffs on V100}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{JIT vs. AOT Compilation}
...
\begin{fbx}{callout-example}{Example:}{Loading Speed: Safetensors vs. Pickle}
\phantomsection\label{callout-example*-1.33}
Loading a 5GB Stable Diffusion model: * \textbf{Pickle
(\texttt{torch.load})}: \textasciitilde15 seconds. High CPU usage. *
...
\begin{fbx}{callout-example}{Example:}{The Profiling Loop}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1610}}
...
\begin{fbx}{callout-checkpoint}{Checkpoint:}{The Optimization Hierarchy}
...
\begin{fbx}{callout-notebook}{AI Engineer’s Notebook:}{ResNet-50: Cost Analysis}
...
This memory pressure, visualized in Figure~\ref{fig-kv-cache-growth},
...
\(114 \text{ batch} \times \frac{3600 \text{ s/hr}}{2\.68 \text{ s/req}} \times 1\,256 \text{ tokens/req} \approx 192 \text{ million tokens/hour}\).
* \textbf{Cost per million tokens}:
\(\frac{\$3\.00}{192} \approx \mathbf{\$0\.016}\).
