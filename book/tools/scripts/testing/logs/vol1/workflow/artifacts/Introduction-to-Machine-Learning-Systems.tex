...
\begin{tcolorbox}[enhanced jigsaw, titlerule=0mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Learning Objectives}, left=2mm, coltitle=black, breakable, colback=white, colbacktitle=quarto-callout-tip-color!10!white, bottomrule=.15mm, colframe=quarto-callout-tip-color-frame, bottomtitle=1mm, toptitle=1mm, toprule=.15mm, leftrule=.75mm, opacityback=0]
...
\begin{fbxSimple}{callout-lighthouse}{Lighthouse:}{The Workflow of ResNet-50 vs. GPT-2}
...
\begin{fbxSimple}{callout-definition}{Definition:}{Machine Learning Lifecycle}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/259eb87570d0ba27b2b92263e9b5be45b38af1b0.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/2153b971df1b1023cfab16c9e9bf29f99a4c6d42.pdf}}
...
\textbf{Data Collection} & Objectives; deployment target; & Versioned
dataset with schema; & Distribution approximates anticipated \\
\textbf{\& Preparation} & quality requirements & preprocessing pipeline;
data validation rules & production environment; labeling meets accuracy
requirements \\
\textbf{Model Development} & Dataset; accuracy targets; & Trained model
weights; & Meets accuracy thresholds within \\
\textbf{\& Training} & resource constraints & training configuration;
experiment logs & computational budget; architecture compatible with
deployment target \\
\textbf{Evaluation} & Trained model; held-out & Performance metrics
across & No critical subgroup falls below \\
\textbf{\& Validation} & test data; evaluation criteria & subgroups;
failure mode analysis; validation certificate & minimum thresholds;
calibration meets domain requirements \\
\textbf{Deployment} & Validated model; infrastructure & Serving
endpoint; monitoring & Latency and throughput meet \\
\textbf{\& Integration} & requirements; SLA targets & instrumentation;
rollback procedures & paradigm requirements; integration tests pass \\
\textbf{Monitoring} & Live system; performance & Drift detection alerts;
& Performance stays within acceptable \\
\textbf{\& Maintenance} & baselines; alert thresholds & retraining
triggers; incident reports & bounds; degradation detected before user
impact \\
\end{longtable}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{The Iteration Tax}
...
model reaches: \(95\% + (26 \times 0\.5\%) = 108\%\) (capped at
ceiling). In practice, the small model's rapid iteration enables
discovering better architectures, data augmentations, and
hyperparameters.
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1106}}
...
\begin{fbxSimple}{callout-checkpoint}{Checkpoint:}{ML vs. Traditional DevOps}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/e3729f2de0f78738a50d7189f8cac0df796e0bea.pdf}}
...
Models targeting mobile deployment. A DR screening model optimized for
...
without reliable connectivity. The Lighthouse Models illustrate these
...
\begin{fbxSimple}{callout-checkpoint}{Checkpoint:}{The Workflow Cycle}
...
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{The Iron Law of Workflow}
...
\item
  ...
  \textbf{Data (\(D_{vol}\))} term. High-quality curation reduces the
  volume of data needed to reach a target accuracy.
\item
  \textbf{Model Development \& Training}: Defines the \textbf{Operations
  (\(O\))} term. Architectural choices (e.g., Transformers vs.~CNNs) set
...
workflow stages manifest for three of the five \textbf{Lighthouse
Models} introduced in \textbf{?@sec-introduction}:

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.0433}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3177}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3177}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3213}}@{}}
\caption{\textbf{Workflow Variations by Lighthouse Model}: The same
...
\begin{fbxSimple}{callout-example}{Example:}{Auditing Stage Transitions}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{Bandwidth vs. Compute}
...
\item
  \textbf{Upload Time}:
  \(5\,000 \text{ MB} / (2/8 \text{ MB/s}) = 20\,000 \text{ seconds} \approx \mathbf{5\.6 \text{ hours}}\).
\item
  \textbf{The Constraint}: If the clinic operates for 8 hours, you are
  consuming \textbf{69\%} of their total bandwidth just for one AI
  feature.
\end{enumerate}
...
KB/patient), reducing bandwidth usage by \textbf{5,000\(\times\)}.
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/b2a44abc7acdcc3b3e8779d526aa225c1d779e30.pdf}}
...
Figure~\ref{fig-mlops-leverage} visualizes this scaling dynamic and
...
Without shared infrastructure, the `Coordination Tax' consumes
engineering capacity: adding more data scientists eventually
\emph{reduces} total velocity as they spend more time debugging
conflicts than running experiments. Platform investment breaks this
ceiling, converting the linear input of team effort into the exponential
output of experimental velocity.
...
\begin{fbxSimple}{callout-definition}{Definition:}{Model Validation}
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{Cloud vs. Edge Deployment Economics}
...
\begin{fbxSimple}{callout-definition}{Definition:}{The Constraint Propagation Principle}
...
\begin{fbxSimple}{callout-checkpoint}{Checkpoint:}{The Cost of Late Discovery}
