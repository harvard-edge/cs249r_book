...
\begin{tcolorbox}[enhanced jigsaw, opacitybacktitle=0.6, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, bottomrule=.15mm, colframe=quarto-callout-tip-color-frame, opacityback=0, colbacktitle=quarto-callout-tip-color!10!white, breakable, coltitle=black, colback=white, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Learning Objectives}, toprule=.15mm, left=2mm, leftrule=.75mm, arc=.35mm]
...
that structure the rest of this book.

Artificial intelligence, broadly conceived, aims to create systems
capable of tasks that typically require human intelligence. Machine
learning represents a specific approach to this goal: rather than
programming explicit rules, engineers design systems that learn patterns
from data. This shift from rule-based to data-driven systems constitutes
the foundational transformation in modern AI.

This transformation illustrates \emph{why} ML has become the dominant
approach. In rule-based systems, humans translate domain expertise
directly into code. In ML systems, humans curate training data, design
learning architectures, and define success metrics, allowing the system
to extract its own operational logic from examples. Data-driven systems
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1975}}
...
Google researchers quantified this paradigm shift in a landmark 2015
paper.

\phantomsection\label{callout-exampleux2a-1.1}
\begin{fbxSimple}{callout-example}{Example:}{The Hidden Technical Debt of ML Systems}
...
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{The Verification Gap}
...
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{Deterministic vs. Probabilistic Engineering}
...
\begin{fbxSimple}{callout-checkpoint}{Checkpoint:}{The Paradigm Shift}
...
computationally cheap (running on 256 KB mainframes) but brittle---it
had no learning capability and no memory of past interactions. } (1966),
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/8d92c826b995f0f7ea63b0823a35e5c35bb82b15.pdf}}
...
\begin{fbxSimple}{callout-example}{Example:}{STUDENT (1964)}
...
\begin{fbxSimple}{callout-example}{Example:}{MYCIN (1976)}
...
\begin{fbxSimple}{callout-example}{Example:}{Early Spam Detection Systems}
...
\begin{fbxSimple}{callout-example}{Example:}{Traditional Computer Vision Pipeline}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/ac3a3d20d12df77acc417408856fe84043adea1c.pdf}}
...
describe a cat's ear?'' to ``\emph{how} do we coordinate 1,024 GPUs
without failure?''
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.0895}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1684}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2263}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1465}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1210}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.3376}}@{}}
...
Yet this understanding demands more than familiarity with any single
technical domain. The convergence of these systems-level challenges
suggests that no existing discipline addresses \emph{what} modern AI
requires. While Computer Science advances ML algorithms
...
\begin{fbxSimple}{callout-definition}{Definition:}{Machine Learning Systems}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/45073530e8b307382d4263df9c8177dd5c8cf1f5.pdf}}
...
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{Samples per Dollar}
...
\begin{fbxSimple}{callout-definition}{Definition:}{AI Engineering}
...
For a deeper mathematical treatment of these concepts, including the
Roofline Model and performance analysis techniques, see
\textbf{?@sec-machine-foundations}.

Throughout this book, every optimization technique we study, from
...
\begin{fbxSimple}{callout-notebook}{AI Engineer’s Notebook:}{Training GPT-3}
...
\item
  \textbf{Efficiency (\(\eta\))}: ≈ 45\% (typical for large-scale
...
\begin{fbxSimple}{callout-checkpoint}{Checkpoint:}{The Iron Law}
...
term does it improve?}

\end{fbxSimple}

The equation is dimensionally consistent---each term resolves to
seconds. (For a general treatment of dimensional analysis as an
engineering verification technique, see
\textbf{?@sec-system-foundations-dimensional-analysis}.)
...
\subsection{Lighthouse Models: The Systems
...
Models}. We do not use these models merely as examples. They are
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.4035}}@{}}
\caption{\textbf{Lighthouse Models as Systems Detectives}: Each workload
isolates a distinct bottleneck, enabling systematic investigation of how
system constraints affect different architectural patterns. Quantitative
specifications and architectural details appear in
\textbf{?@sec-dnn-architectures}.}\label{tbl-lighthouse-examples}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Lighthouse Model}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
(Table~\ref{tbl-dam-taxonomy}), listed here in the order they
historically matured:

\begin{itemize}
\tightlist
\item
  \textbf{Algorithmic Efficiency (Logic)}: The earliest frontier.
  Reduces computational requirements through better model design and
  training procedures. Techniques include \textbf{Model Compression}
  (pruning, quantization, knowledge distillation), efficient
  architectures like MobileNet, and neural architecture search.
\item
  \textbf{Compute Efficiency (Physics)}: Accelerated as algorithms
  demanded more. Maximizes hardware utilization by aligning the
  ``Algorithm'' logic with the ``Machine'' physics. This dimension
  encompasses the evolution from general-purpose CPUs to specialized
  accelerators (GPUs, TPUs) and the hardware-software co-design
  principles that translate theoretical TFLOPS into real-world speedups.
\item
  \textbf{Data Selection (Information)}: The most recent emphasis.
  Extracts more learning signal from limited examples, reducing the
  ``Data'' numerator of the Iron Law. Techniques include transfer
  learning, active learning, and curriculum design that ensure every
  sample provides maximum learning value.
\end{itemize}
...
eras at different rates. While history progressed from algorithmic
breakthroughs to hardware acceleration to data-centric methods, Part III
of this book reverses that sequence: we begin with data selection, then
model compression, then hardware acceleration. This pedagogical order
reflects how practitioners actually build systems---quality data is
prerequisite to effective model optimization, and understanding the
model is prerequisite to mapping it efficiently onto hardware.

\begin{figure}[htb]

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/8762ab1a1c6a55da6852deb74dde2596fae1c754.pdf}}
...
\begin{fbxSimple}{callout-perspective}{Systems Perspective:}{The Efficiency Paradox}
...
Which efficiency dimensions to prioritize depends heavily on deployment
context---cloud systems optimize for throughput while edge devices
optimize for power. We examine this deployment spectrum and its
implications for the ML lifecycle in
Section~\ref{sec-introduction-understanding-ml-system-lifecycle-deployment-ea09}.
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/47a3dfdca6725e8e5a9be2e0c30805cc67aaa82e.pdf}}
...
the Arduino Uno have only 32 KB of storage and 2 KB of RAM, yet can run
...
Each position on this deployment spectrum creates distinct bottlenecks
that determine which efficiency dimensions matter most, as summarized in
...
\item
  ...
  edge} deployment pattern: models under 500 KB run inference on
...
These systems complement the Lighthouse Models introduced earlier by
...
inference on models under 500 KB transmitted over TV white-space
...
identified. As Figure~\ref{fig-pillars} illustrates, these pillars
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1863}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7019}}@{}}
...
\textbf{Lighthouse Models} introduced above (ResNet-50, GPT-2/Llama,
...
engineering never confronts.

Through the Bitter Lesson and AI's historical evolution, the chapter
demonstrated why systems engineering has become central to AI progress
and how learning-based approaches came to dominate the field. This
context enabled a formal definition of AI Engineering as a distinct
...
computational platforms. The five Lighthouse Models introduced here
(ResNet-50, GPT-2/Llama, MobileNetV2, DLRM, and Keyword Spotting) serve
as recurring touchstones throughout subsequent chapters, grounding
abstract principles in the concrete engineering challenges of real
workloads.

\phantomsection\label{callout-takeawaysux2a-1.15}
\begin{fbxSimple}{callout-takeaways}{Takeaways:}{Key Takeaways}
\phantomsection\label{callout-takeaways*-1.15}
...
Lighthouse Models ground abstract concepts in concrete engineering
problems you will encounter throughout your career.

\phantomsection\label{callout-chapter-connectionux2a-1.16}
\begin{fbxSimple}{callout-chapter-connection}{What’s Next:}{From Vision to Architecture}
\phantomsection\label{callout-chapter-connection*-1.16}
