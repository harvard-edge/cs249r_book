...
physical-unit suffix sites in `nn_architectures` with typed quantity formatters.

Touched chapters and equivalence:

| Chapter file | Calls | Value exports checked | Inline prose lines checked | Result |
|---|---:|---:|---:|---|
| `vol1/nn_architectures/nn_architectures.qmd` | 50 | 174 | 92 | identical values + prose |

Validation details:

- Migrated GPT/ResNet/MobileNet/DLRM/KWS memory footprints, H100/A100 bandwidth
  and TFLOP/s anchors, MLP and attention memory examples, KV-cache sizes,
  capacity-wall examples, wildlife edge sizing, transformer pitfall budgets,
  and cluster-vs-edge memory comparisons.
- FLOP-throughput displays use `TFLOP / second` so the visible house label stays
  `TFLOP/s`.
- Existing explicit integer rounding for the runtime-memory total is preserved
  with `fmt_qty_int(round(...), MB)`.
- `assess_equiv.py` baseline/snapshot/diff reported `IDENTICAL values` and
  `IDENTICAL prose`.
- `nn_architectures` now has zero `suffix=` calls.
- `audit_fmt_usage.py` reports `physical_unit` suffixes dropped 461 -> 411,
  `fmt_qty` at 892, and `fmt_qty_int` at 116.
- Verification: `git diff --check` PASS; py_compile PASS for formatter,
  math-canonical, and fmt audit modules; focused pytest suite PASS, 190 tests;
  `fmt_prose_contract.py` PASS, 0 violations; `codemod_fmt.py queue` PASS,
  `by kind: {}`; `./book/binder check math` PASS;
...
## 2026-05-31 — `vol2/inference` physical-unit cleanup

Change type: byte-identical formatter relocation. Replaced all 44 remaining
physical-unit suffix sites in `inference` with typed quantity formatters.

Touched chapters and equivalence:

| Chapter file | Calls | Value exports checked | Inline prose lines checked | Result |
|---|---:|---:|---:|---|
| `vol2/inference/inference.qmd` | 44 | 208 | 119 | identical values + prose |

Validation details:

- Migrated H100/A100 memory and bandwidth specs, NVLink/PCIe rates, activation
  and message sizes, KV-cache capacity and prefix-cache sizes, tensor-parallel
  model/activation sizes, MoE memory/capacity planning, vector-index storage,
  quantized-serving headroom, and final hardware-summary anchors.
- GiB-backed hardware memory capacities preserve the visible `GB` house label
  with `unit_label="GB"` while keeping the display unit as `GiB`.
- Existing rounded integer displays are preserved with `fmt_qty_int(...)`.
- `assess_equiv.py` baseline/snapshot/diff reported `IDENTICAL values` and
  `IDENTICAL prose`.
- `inference` now has zero `suffix=` calls.
- `audit_fmt_usage.py` reports `physical_unit` suffixes dropped 505 -> 461,
  `fmt_qty` at 843, and `fmt_qty_int` at 115.
- Verification: `git diff --check` PASS; py_compile PASS for formatter,
  math-canonical, and fmt audit modules; focused pytest suite PASS, 190 tests;
  `fmt_prose_contract.py` PASS, 0 violations; `codemod_fmt.py queue` PASS,
  `by kind: {}`; `./book/binder check math` PASS;
...
Change type: byte-identical formatter relocation. Replaced all 39 remaining
physical-unit suffix sites in `frameworks` with typed quantity formatters.

Touched chapters and equivalence:

| Chapter file | Calls | Value exports checked | Inline prose lines checked | Result |
|---|---:|---:|---:|---|
| `vol1/frameworks/frameworks.qmd` | 39 | 131 | 70 | identical values + prose |

Validation details:

- Migrated A100/V100 compute and bandwidth anchors, GPT-3/7B memory footprints,
  ResNet training memory, tensor footprints, administrative-tax memory,
  CPU/GPU transfer rates, dataloader throughput, HBM capacity, MNIST memory
  traffic, and batch-size capacity examples.
- GiB-backed A100 memory values preserve the visible `GB` label with
  `unit_label="GB"`.
- `assess_equiv.py` baseline/snapshot/diff reported `IDENTICAL values` and
  `IDENTICAL prose`.
- `frameworks` now has zero `suffix=` calls.
- `audit_fmt_usage.py` reports `physical_unit` suffixes dropped 623 -> 584 and
  `fmt_qty` at 752.
- Verification: `git diff --check` PASS; py_compile PASS for formatter,
  math-canonical, and fmt audit modules; focused pytest suite PASS, 190 tests;
  `fmt_prose_contract.py` PASS, 0 violations; `codemod_fmt.py queue` PASS,
...
Change type: byte-identical formatter relocation. Replaced all 23 remaining
physical-unit suffix sites in `performance_engineering` with typed quantity
formatters.
...
| Chapter file | Calls | Value exports checked | Inline prose lines checked | Result |
|---|---:|---:|---:|---|
| `vol2/performance_engineering/performance_engineering.qmd` | 23 | 109 | 58 | identical values + prose |

Validation details:

- Migrated memory-wall register/cache/energy values, fusion traffic, KV-cache
  sizing, NVLink bandwidth, profiled TFLOP/s/TB/s, and decode profiler
  traffic/bandwidth.
- Binary register/shared-memory quantities retain old decimal hardware labels
  through `ureg.kibibyte` plus `unit_label="KB"`.
- KV-cache scalar capacity arithmetic is preserved; checked quantity values are
  reconstructed only for display.
- `assess_equiv.py` baseline/snapshot/diff reported `IDENTICAL values` and
  `IDENTICAL prose`.
- `performance_engineering` now has zero `suffix=` calls.
- `audit_fmt_usage.py` reports `physical_unit` suffixes dropped 773 -> 750,
  `fmt_qty` at 623, and `fmt_qty_int` at 46.
- Verification: `git diff --check` PASS; py_compile PASS for formatter,
  math-canonical, and fmt audit modules; focused pytest suite PASS, 190 tests;
  `fmt_prose_contract.py` PASS, 0 violations; `codemod_fmt.py queue` PASS,
  `by kind: {}`; `./book/binder check math` PASS;
...
physical-unit suffix sites in `fleet_orchestration` with typed quantity
formatters.
...
| Chapter file | Calls | Value exports checked | Inline prose lines checked | Result |
|---|---:|---:|---:|---|
| `vol2/fleet_orchestration/fleet_orchestration.qmd` | 12 | 160 | 60 | identical values + prose |

Validation details:

- Migrated NVLink/InfiniBand bandwidths, the topology activation-size display,
  GPU sharing memory totals/footprints, and the debug inventory NVLink value.
- A100 memory capacity is GiB-backed in the registry but remains displayed as
  `GB` through `fmt_qty(..., GiB, unit_label="GB")` to preserve existing visible
  output.
- GPU sharing computations kept the existing scalar arithmetic while preserving
  quantity-bearing values for formatter-owned unit checks at output.
- `assess_equiv.py` baseline/snapshot/diff reported `IDENTICAL values` and
  `IDENTICAL prose`.
- `fleet_orchestration` now has zero `suffix=` calls.
- `audit_fmt_usage.py` reports `physical_unit` suffixes dropped 844 -> 832,
  `fmt_qty` at 554, and `fmt_qty_int` at 33.
- Verification: `git diff --check` PASS; py_compile PASS for formatter,
  math-canonical, and fmt audit modules; focused pytest suite PASS, 190 tests;
  `fmt_prose_contract.py` PASS, 0 violations; `codemod_fmt.py queue` PASS,
  `by kind: {}`; `./book/binder check math` PASS;
...
## 2026-05-31 — `vol2/ops_scale` physical-unit cleanup

Change type: byte-identical formatter relocation. Replaced all 12 remaining
physical-unit suffix sites in `ops_scale` with typed quantity formatters.

Touched chapters and equivalence:

| Chapter file | Calls | Value exports checked | Inline prose lines checked | Result |
|---|---:|---:|---:|---|
| `vol2/ops_scale/ops_scale.qmd` | 12 | 195 | 95 | identical values + prose |

Validation details:

- Migrated GPU TDP, per-watt TFLOP/s efficiency displays, planning throughput
  in TFLOP/s and PFLOP/s, rack and facility power in kW/MW, and checkpoint
  state/bandwidth in TB and GB/s.
- Per-watt efficiency strings now keep quantity type as
  `TFLOPs/second/watt`, while `unit_label="TFLOP/s"` preserves the existing
  table display because the column/prose owns "per watt".
- Checkpoint state now keeps weights, gradients, optimizer state, and write
  bandwidth as Pint quantities until `fmt_qty_int` / `fmt_qty`.
- `assess_equiv.py` baseline/snapshot/diff reported `IDENTICAL values` and
  `IDENTICAL prose`.
- `ops_scale` now has zero `suffix=` calls.
- `audit_fmt_usage.py` reports `physical_unit` suffixes dropped 856 -> 844,
  `fmt_qty` at 543, and `fmt_qty_int` at 32.
- Verification: `git diff --check` PASS; py_compile PASS for formatter,
  math-canonical, and fmt audit modules; focused pytest suite PASS, 190 tests;
  `fmt_prose_contract.py` PASS, 0 violations; `codemod_fmt.py queue` PASS,
  `by kind: {}`; `./book/binder check math` PASS;
