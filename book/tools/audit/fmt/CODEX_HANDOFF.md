...
**A51 — `vol1/ml_workflow` physical-unit cleanup: DONE.**
Migrated all 8 remaining physical-unit suffix sites in `ml_workflow` to typed
quantity formatters, leaving the file with 0 `suffix=` calls. The lane covered
MobileNet model size, stage-interface memory constraints, bandwidth-vs-compute
daily data volume/uplink rates, and deployment image size. `audit_fmt_usage.py`
now reports physical-unit suffixes down to 889, `fmt_qty` at 499, and
`fmt_qty_int` at 31. Verification: `assess_equiv.py` values/prose identical for
`ml_workflow`; `git diff --check` PASS; py_compile PASS; focused pytest suite
PASS (190 tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue`
`by kind: {}`; `./book/binder check math` PASS; `./book/binder check code
--scope lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A52 — `vol2/introduction` physical-unit cleanup: DONE.**
Migrated all 8 remaining physical-unit suffix sites in `vol2/introduction` to
typed quantity formatters, leaving the file with 0 `suffix=` calls. The lane
covered GPT-3 gradient sizes, InfiniBand HDR/NDR bandwidth recaps, NVLink H100
bandwidth, and A100 FP16 throughput. Split-rate values are now checked as
`Gb/s` or `GB/s` while preserving the existing prose-owned `/s`. `audit_fmt_usage.py`
now reports physical-unit suffixes down to 881, `fmt_qty` at 507, and
`fmt_qty_int` at 31. Verification: `assess_equiv.py` values/prose identical for
`vol2/introduction`; `git diff --check` PASS; py_compile PASS; focused pytest
suite PASS (190 tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue`
`by kind: {}`; `./book/binder check math` PASS; `./book/binder check code
--scope lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A53 — `vol1/backmatter/appendix_algorithm` physical-unit cleanup: DONE.**
Migrated all 11 remaining physical-unit suffix sites in `appendix_algorithm` to
typed quantity formatters, leaving the file with 0 `suffix=` calls. The lane
covered sparse embedding storage and the GPT-2 XL training-memory breakdown:
weights, gradients, optimizer state, model state, accelerator memory, remaining
memory, small-batch total, large-batch activations, and the 80 GB accelerator
class reference. `audit_fmt_usage.py` now reports physical-unit suffixes down to
870, `fmt_qty` at 518, and `fmt_qty_int` at 31. Verification:
`assess_equiv.py` values/prose identical for `appendix_algorithm`; `git diff
--check` PASS; py_compile PASS; focused pytest suite PASS (190 tests);
`fmt_prose_contract.py` 0; `codemod_fmt.py queue` `by kind: {}`;
`./book/binder check math` PASS; `./book/binder check code --scope
lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A54 — `vol1/responsible_engr` physical-unit cleanup: DONE.**
Migrated all 14 remaining physical-unit suffix sites in `responsible_engr` to
typed quantity formatters, leaving the file with 0 `suffix=` calls. The lane
covered GPU power, training/inference carbon mass, metric-ton carbon summaries,
and GPT-3-scale training energy in MWh/kWh. `audit_fmt_usage.py` now reports
physical-unit suffixes down to 856, `fmt_qty` at 532, and `fmt_qty_int` at 31.
Verification: `assess_equiv.py` values/prose identical for `responsible_engr`;
`git diff --check` PASS; py_compile PASS; focused pytest suite PASS (190
tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue` `by kind: {}`;
`./book/binder check math` PASS; `./book/binder check code --scope
lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A55 — `vol2/ops_scale` physical-unit cleanup: DONE.**
Migrated all 12 remaining physical-unit suffix sites in `ops_scale` to typed
quantity formatters, leaving the file with 0 `suffix=` calls. The lane covered
GPU TDP, per-watt TFLOP/s efficiency displays, planning throughput in
TFLOP/s/PFLOP/s, rack/facility power in kW/MW, and checkpoint state/bandwidth in
TB and GB/s. Efficiency values keep a checked `TFLOPs/second/watt` source unit
while preserving the existing table display (`TFLOP/s`) because the column/prose
owns "per watt". `audit_fmt_usage.py` now reports physical-unit suffixes down
to 844, `fmt_qty` at 543, and `fmt_qty_int` at 32. Verification:
`assess_equiv.py` values/prose identical for `ops_scale`; `git diff --check`
PASS; py_compile PASS; focused pytest suite PASS (190 tests);
`fmt_prose_contract.py` 0; `codemod_fmt.py queue` `by kind: {}`;
`./book/binder check math` PASS; `./book/binder check code --scope
lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A56 — `vol2/fleet_orchestration` physical-unit cleanup: DONE.**
Migrated all 12 remaining physical-unit suffix sites in `fleet_orchestration`
to typed quantity formatters, leaving the file with 0 `suffix=` calls. The lane
covered NVLink/InfiniBand bandwidths, the one activation-size display, GPU
sharing memory totals/footprints, and the debug inventory NVLink value. A100
capacity remains displayed as `GB` from a GiB-backed source via
`unit_label="GB"` to preserve the book's existing visible style. `audit_fmt_usage.py`
now reports physical-unit suffixes down to 832, `fmt_qty` at 554, and
`fmt_qty_int` at 33. Verification: `assess_equiv.py` values/prose identical
for `fleet_orchestration`; `git diff --check` PASS; py_compile PASS; focused
pytest suite PASS (190 tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue`
`by kind: {}`; `./book/binder check math` PASS; `./book/binder check code
--scope lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A57 — `vol1/backmatter/appendix_machine` physical-unit cleanup: DONE.**
Migrated all 17 remaining physical-unit suffix sites in `appendix_machine` to
typed quantity formatters, leaving the file with 0 `suffix=` calls. The lane
covered A100/H100/TPUv5p FLOP/s and bandwidth specs, Little's-law memory
displays, hardware cheat-sheet capacity/cache/interconnect values, and the
bandwidth-latency setup packet size. H100/TPUv5p decimal-GB capacity displays
preserve the old explicit `int(...m_as(GB))` floor by formatting a floored
quantity, rather than letting precision guards round the underlying fractional
GB value. `audit_fmt_usage.py` now reports physical-unit suffixes down to 815
and `fmt_qty` at 571. Verification: `assess_equiv.py` values/prose identical
for `appendix_machine`; `git diff --check` PASS; py_compile PASS; focused
pytest suite PASS (190 tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue`
`by kind: {}`; `./book/binder check math` PASS; `./book/binder check code
--scope lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.
...
**A59 — `vol1/model_serving` physical-unit cleanup: DONE.**
Migrated all 23 remaining physical-unit suffix sites in `model_serving` to typed
quantity formatters, leaving the file with 0 `suffix=` calls. The lane covered
model-size and activation-memory displays, mobile energy in mJ, model-swap
PCIe bandwidth, battery Wh, precision-tradeoff memory, H100/A100 bandwidth
anchors, Llama case-study model weight, and remaining VRAM. Pedagogical scalars
were converted to Pint quantities at load time while existing scalar equation
variables were preserved for math strings. `audit_fmt_usage.py` now reports
physical-unit suffixes down to 773, `fmt_qty` at 612, and `fmt_qty_int` at 34.
Verification: `assess_equiv.py` values/prose identical for `model_serving`;
`git diff --check` PASS; py_compile PASS; focused pytest suite PASS (190
tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue` `by kind: {}`;
`./book/binder check math` PASS; `./book/binder check code --scope
lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A60 — `vol2/performance_engineering` physical-unit cleanup: DONE.**
Migrated all 23 remaining physical-unit suffix sites in
`performance_engineering` to typed quantity formatters, leaving the file with
0 `suffix=` calls. The lane covered memory-wall cache/register/energy values,
fusion traffic, KV-cache sizing, NVLink bandwidth, profiled TFLOP/s/TB/s, and
decode profiler traffic/bandwidth. Binary register/shared-memory quantities keep
their old decimal hardware labels through `ureg.kibibyte` plus
`unit_label="KB"`. KV-cache scalar capacity arithmetic is preserved, with typed
display quantities reconstructed at output. `audit_fmt_usage.py` now reports
physical-unit suffixes down to 750, `fmt_qty` at 623, and `fmt_qty_int` at 46.
Verification: `assess_equiv.py` values/prose identical for
`performance_engineering`; `git diff --check` PASS; py_compile PASS; focused
pytest suite PASS (190 tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue`
`by kind: {}`; `./book/binder check math` PASS; `./book/binder check code
--scope lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.
...
**A62 — `vol1/nn_computation` physical-unit cleanup: DONE.**
Migrated all 30 remaining physical-unit suffix sites in `nn_computation` to
typed quantity formatters, leaving the file with 0 `suffix=` calls. The lane
covered tensor-layout KB traffic, rule/HOG/DL memory examples, historical
training energy in kWh/MWh, MNIST model-memory comparisons, training vs
inference memory tables, GPT-2/Adam state memory, backprop activation storage,
and the USPS LeNet footprint. The compound `16 GB GPU` display is now a
Markdown-composed `fmt_qty(..., GB) + " GPU"` string rather than a raw formatter
suffix. `audit_fmt_usage.py` now reports physical-unit suffixes down to 697,
`fmt_qty` at 649, and `fmt_qty_int` at 73. Verification: `assess_equiv.py`
values/prose identical for `nn_computation`; `git diff --check` PASS;
py_compile PASS; focused pytest suite PASS (190 tests); `fmt_prose_contract.py`
0; `codemod_fmt.py queue` `by kind: {}`; `./book/binder check math` PASS;
`./book/binder check code --scope lego-dead-code` PASS;
`audit_prose_semantics.py` CLEAN across 81 files.

**A63 — `vol1/data_engineering` physical-unit cleanup: DONE.**
Migrated all 36 remaining physical-unit suffix sites in `data_engineering` to
typed quantity formatters, leaving the file with 0 `suffix=` calls. The lane
covered dataset sizes, storage throughput rates, pJ energy examples, KWS memory
limits, streaming/ETL storage, SNR in dB, GPT-3 storage, compression rates, and
lifecycle storage bandwidth. The dB display uses `ureg.Quantity(..., ureg.decibel)`
because decibel is a logarithmic/offset unit and cannot be multiplied directly.
`audit_fmt_usage.py` now reports physical-unit suffixes down to 661, `fmt_qty`
at 684, and `fmt_qty_int` at 74. Verification: `assess_equiv.py` values/prose
identical for `data_engineering`; `git diff --check` PASS; py_compile PASS;
focused pytest suite PASS (190 tests); `fmt_prose_contract.py` 0;
`codemod_fmt.py queue` `by kind: {}`; `./book/binder check math` PASS;
`./book/binder check code --scope lego-dead-code` PASS;
`audit_prose_semantics.py` CLEAN across 81 files.
...
**A65 — `vol1/frameworks` physical-unit cleanup: DONE.**
Migrated all 39 remaining physical-unit suffix sites in `frameworks` to typed
quantity formatters, leaving the file with 0 `suffix=` calls. The lane covered
A100/V100 compute and bandwidth anchors, GPT-3/7B memory footprints, ResNet
training memory, tensor footprints, administrative-tax memory, CPU/GPU transfer
rates, dataloader throughput, HBM capacity, MNIST memory traffic, and batch-size
capacity examples. GiB-backed A100 memory values preserve the visible `GB` label
with `unit_label="GB"`. `audit_fmt_usage.py` now reports physical-unit suffixes
down to 584 and `fmt_qty` at 752. Verification: `assess_equiv.py` values/prose
identical for `frameworks`; `git diff --check` PASS; py_compile PASS; focused
pytest suite PASS (190 tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue`
`by kind: {}`; `./book/binder check math` PASS; `./book/binder check code
--scope lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.
...
**A67 — `vol1/benchmarking` physical-unit cleanup: DONE.**
Migrated all 40 remaining physical-unit suffix sites in `benchmarking` to typed
quantity formatters, leaving the file with 0 `suffix=` calls. The lane covered
FLOP/s roofline and SOL displays, model sizes, bandwidth rates, inference
energy/power, GPT-3 training energy, EdgeTPU energy/power, cold-start transfer
size/rate, and the remaining benchmark-summary anchors. FLOP throughput uses
`TFLOP / second` to preserve `TFLOP/s`; lux uses `unit_label="lux"` to preserve
the spelled-out display. `audit_fmt_usage.py` now reports physical-unit suffixes
down to 505 and `fmt_qty` at 829. Verification: `assess_equiv.py` values/prose
identical for `benchmarking`; `git diff --check` PASS; py_compile PASS; focused
pytest suite PASS (190 tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue`
`by kind: {}`; `./book/binder check math` PASS; `./book/binder check code
--scope lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A68 — `vol2/inference` physical-unit cleanup: DONE.**
Migrated all 44 remaining physical-unit suffix sites in `inference` to typed
quantity formatters, leaving the file with 0 `suffix=` calls. The lane covered
H100/A100 memory and bandwidth specs, NVLink/PCIe rates, activation/message
sizes, KV-cache and prefix-cache sizes, tensor-parallel model sizes, MoE
capacity planning, vector-index storage, and quantized-serving headroom.
GiB-backed hardware memory capacities preserve visible `GB` with
`unit_label="GB"`, and old rounded integer displays remain explicit through
`fmt_qty_int(...)`. `audit_fmt_usage.py` now reports physical-unit suffixes down
to 461, `fmt_qty` at 843, and `fmt_qty_int` at 115. Verification:
`assess_equiv.py` values/prose identical for `inference`; `git diff --check`
PASS; py_compile PASS; focused pytest suite PASS (190 tests);
`fmt_prose_contract.py` 0; `codemod_fmt.py queue` `by kind: {}`;
`./book/binder check math` PASS; `./book/binder check code --scope
lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.
...
**A70 — `vol2/distributed_training` physical-unit cleanup: DONE.**
Migrated all 60 remaining physical-unit suffix sites in `distributed_training`
to typed quantity formatters, leaving the file with 0 `suffix=` calls. The lane
covered model/state memory, NVLink/InfiniBand bandwidth, activation headroom,
pipeline payloads, hybrid parallel memory budgets, PPO/DPO/RLHF memory, and
recap anchors. GiB-backed hardware capacities preserve visible `GB` with
`unit_label="GB"`, and rounded displays remain explicit through
`fmt_qty_int(...)`. `audit_fmt_usage.py` now reports physical-unit suffixes down
to 351, `fmt_qty` at 951, and `fmt_qty_int` at 117. Verification:
`assess_equiv.py` values/prose identical for `distributed_training`; `git diff
--check` PASS; py_compile PASS; focused pytest suite PASS (190 tests);
`fmt_prose_contract.py` 0; `codemod_fmt.py queue` `by kind: {}`;
`./book/binder check math` PASS; `./book/binder check code --scope
lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A71 — `vol1/hw_acceleration` physical-unit cleanup: DONE.**
Migrated all 65 remaining physical-unit suffix sites in `hw_acceleration` to
typed quantity formatters, leaving the file with 0 `suffix=` calls. The lane
covered accelerator bandwidth/throughput, TPU/Gaudi/NVLink examples, PFLOP/s
dense/sparse displays, tensor/attention/conv/dense/layernorm memory traffic,
roofline values, fusion memory traffic, edge memory headroom, and CPU/NPU power
and energy examples. TFLOP/s uses `TFLOP / second`; PFLOP/s uses checked
`unit_label="PFLOP/s"` to preserve singular house style. `audit_fmt_usage.py`
now reports physical-unit suffixes down to 286, `fmt_qty` at 1013, and
`fmt_qty_int` at 120. Verification: `assess_equiv.py` values/prose identical
for `hw_acceleration`; `git diff --check` PASS; py_compile PASS; focused pytest
suite PASS (190 tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue`
`by kind: {}`; `./book/binder check math` PASS; `./book/binder check code
--scope lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A72 — `vol2/data_storage` physical-unit cleanup: DONE.**
Migrated all 67 remaining physical-unit suffix sites in `data_storage` to typed
quantity formatters, leaving the file with 0 `suffix=` calls. The lane covered
dataset/tokenization/checkpoint sizes, shard/node storage, host/NVLink/local/PFS
bandwidth rates, multimodal data, loader/ImageNet examples, ZeRO/checkpoint
storage math, retention/archive examples, and final footprint anchors. Existing
rounded displays remain explicit through `fmt_qty_int(round(...), TB)`;
`archive_lineage_tb_str` now uses `fmt_qty(..., approx=True)`. `audit_fmt_usage.py`
now reports physical-unit suffixes down to 219, `fmt_qty` at 1071, and
`fmt_qty_int` at 129. Verification: `assess_equiv.py` values/prose identical
for `data_storage`; `git diff --check` PASS; py_compile PASS; focused pytest
suite PASS (190 tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue`
`by kind: {}`; `./book/binder check math` PASS; `./book/binder check code
--scope lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.

**A73 — `vol2/compute_infrastructure` physical-unit cleanup: DONE.**
Migrated all 82 remaining physical-unit suffix sites in `compute_infrastructure`
to typed quantity formatters, leaving the file with 0 `suffix=` calls. The lane
covered accelerator FLOP/s, bandwidth, TDP, NVLink, rack power, per-watt
efficiency values, static training-state memory, HBM/host memory, bandwidth
hierarchy, tensor/activation communication, WSC bandwidth, checkpoint
bandwidth/storage, and option-comparison anchors. Per-watt values remain visible
as `N TFLOP/s` because the prose owns "per watt"; rack-power expressions now
stay Pint quantities through formatting. `audit_fmt_usage.py` now reports
physical-unit suffixes down to 137, `fmt_qty` at 1147, and `fmt_qty_int` at
135. Verification: `assess_equiv.py` values/prose identical for
`compute_infrastructure`; `git diff --check` PASS; py_compile PASS; focused
pytest suite PASS (190 tests); `fmt_prose_contract.py` 0; `codemod_fmt.py queue`
`by kind: {}`; `./book/binder check math` PASS; `./book/binder check code
--scope lego-dead-code` PASS; `audit_prose_semantics.py` CLEAN across 81 files.
