...
| **Volume**    | **Title**                                         | **Focus**                                          | **Analogy**                        |
|:--------------|:--------------------------------------------------|:---------------------------------------------------|:-----------------------------------|
| **Volume I**  | **Introduction to Machine Learning Systems**      | Single-machine ML systems, foundational principles | "Computer Organization and Design" |
| **Volume II** | **Machine Learning Systems at Scale**             | Distributed systems at production scale            | "Computer Architecture"            |

**Volume I** teaches you to *understand* ML systems. **Volume II** teaches you to *build* ML systems at scale.

**Volume I: Introduction to Machine Learning Systems** establishes the foundations through four progressive stages:
...
- **Machine Learning Basics**: Prior exposure to supervised learning concepts (classification, regression), neural network fundamentals, and common architectures (CNNs, RNNs) is helpful but not strictly required. The Neural Computation chapter provides essential background for those new to these topics.
...
Traditional systems courses—computer architecture, operating systems, compilers, databases—study systems that are fundamentally deterministic. Given the same inputs, a compiler produces the same output; an OS scheduler follows the same algorithm; a cache behaves predictably based on access patterns. Machine learning systems differ in ways that demand new engineering approaches:

**Stochastic by nature.** ML systems are probabilistic: models output confidence distributions, training involves random initialization, and production behavior depends on data distributions that shift over time. Engineers must reason about accuracy distributions, drift detection thresholds, and failure modes that manifest statistically rather than as crashes. A compiler bug produces wrong code; an ML system bug produces slightly worse predictions—silently.

**Data as code.** In traditional software, behavior is defined by code; in ML systems, behavior is defined by data. Changing a training dataset changes system behavior as fundamentally as rewriting source code. This elevates data engineering from "preprocessing" to a first-class engineering discipline with its own versioning, testing, and debugging requirements—analogous to how compilers treat source code.

**Human-in-the-loop.** Many ML system components involve humans: data labeling, quality assessment, governance decisions, fairness audits. These aren't peripheral concerns—they're engineering constraints with quantifiable costs, throughput limits, and quality trade-offs. Amdahl's Law applies to human bottlenecks just as it does to computational ones.

**Interdisciplinary by design.** A computer architecture course can focus on hardware; an OS course on resource management; a compilers course on program transformation. ML systems span all of these plus mathematics (optimization, statistics) and human factors (ethics, governance). No single discipline's tools suffice. This textbook therefore combines quantitative analysis where physics allows (bandwidth, energy, latency), statistical reasoning where systems are stochastic (drift, label noise), economic modeling where humans are involved (labeling costs, annotation throughput), and framework-oriented thinking where abstractions are still emerging.

**Evolving abstractions.** Computer architecture has stable abstractions refined over decades—instruction sets, cache hierarchies, virtual memory. Operating systems have processes, threads, and file systems. Compilers have lexing, parsing, optimization passes, and code generation. ML systems is still discovering its fundamental abstractions. Concepts like training-serving consistency, feature stores, and data drift are emerging best practices, not settled science. This textbook captures current consensus while acknowledging that the field continues to evolve.

These differences explain why this textbook necessarily takes a broader approach than traditional systems courses—encompassing not just computational efficiency but also data quality, human processes, and system-level thinking across disciplinary boundaries.
...
![Bloom's Taxonomy (2021 edition).](images/png/bloom_revised_taxonomy.png){#fig-bloom fig-alt="Bloom's Taxonomy (2021 edition)."}
...
This section provides guidance on navigating the book effectively. We cover the organizational structure across volumes and parts, suggest learning paths for different backgrounds, and explain how to engage with the interactive elements and community.
...
1. **Part I: Foundations of Scale**
   *Master the algorithms of scale.* Understand distributed training strategies, collective communication primitives, and fault tolerance mechanisms that enable training across thousands of devices.

2. **Part II: Building the Machine Learning Fleet**
   *Build the physical computer.* Architect the infrastructure, from datacenters and accelerators to high-performance storage systems, required to execute distributed workloads.

3. **Part III: Deployment at Scale**
   *Serve the world.* Navigate the shift from training to inference, deploy intelligence to the edge, and manage the operational lifecycle of production fleets.

4. **Part IV: Production Concerns**
   *Harden the system.* Address the non-functional requirements of privacy, security, robustness, and environmental sustainability in large-scale operations.

5. **Part V: Responsible AI at Scale**
   *Shape the future.* Explore responsible AI governance, AI for social good, and the emerging frontiers of AGI systems.
...
Scaling requires fundamentally different algorithms. This Part establishes the logical foundations of distributed systems, covering distributed training strategies (data, model, pipeline parallelism), the communication primitives that synchronize them, and the fault tolerance mechanisms that keep them running despite routine failures.

**Part II: Building the Machine Learning Fleet** asks: *What physical infrastructure powers distributed AI?*
Logical algorithms need physical hardware. This Part builds the "supercomputer" required for modern ML, examining the datacenter architecture, specialized accelerators, high-bandwidth interconnects, and massive-scale storage systems that fuel the training fleet.

**Part III: Deployment at Scale** asks: *How do we serve predictions to millions of users globally?*
Training is only half the battle. This Part addresses the challenges of serving models at scale, from low-latency datacenter inference to resource-constrained edge intelligence, and the MLOps practices needed to manage production lifecycles.

**Part IV: Production Concerns** asks: *How do we ensure systems are secure, robust, and sustainable?*
Scale amplifies risks. This Part tackles the critical non-functional requirements of production: protecting privacy and security, ensuring robustness against distribution shifts, and minimizing the environmental footprint of large-scale AI.

**Part V: Responsible AI at Scale** asks: *How do we govern AI to ensure beneficial societal impact?*
Technical excellence must serve human values. This Part explores frameworks for responsible AI governance, applications of AI for social good, and the frontiers of AGI systems that will shape the future.
