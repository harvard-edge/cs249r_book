...
Parts I and II established the foundations for building machine learning systems: where they deploy across cloud, edge, and embedded environments (@sec-ml-systems), how development workflows organize the iterative process (@sec-ai-workflow), why data infrastructure determines success or failure (@sec-data-engineering), what mathematical operations neural networks perform (@sec-dl-primer), how specialized architectures exploit data structure (@sec-dnn-architectures), which frameworks enable practical implementation (@sec-ai-frameworks), and how training transforms data into learned models (@sec-ai-training). You now understand how to build ML systems that work. Part III addresses the next challenge: making them work efficiently.

The gap between training a model that achieves research accuracy and deploying one that meets production constraints defines the efficiency imperative. Models emerging from the training pipelines examined in @sec-ai-training often exceed practical resource budgets by orders of magnitude. State-of-the-art language models require hundreds of gigabytes for inference. Vision models demand computational throughput exceeding mobile device capabilities. The sophisticated architectures from @sec-dnn-architectures achieve remarkable accuracy but create deployment barriers that efficiency engineering must address.
...
**Machine Learning System Efficiency** refers to the optimization of ML systems to minimize _computational_, _memory_, and _energy_ demands while maintaining performance, achieved through improvements in _algorithms_, _hardware utilization_, and _data usage_.
...
The photo search example revealed how efficiency dimensions interact, but how do we know when scaling hits fundamental limits? Scaling laws provide the quantitative framework for understanding exactly where these walls appear and why efficiency becomes the only path forward.
...
Where scaling laws prescribe what resources to allocate, the efficiency framework prescribes how to use them optimally. Three complementary dimensions target the specific limitations that scaling analysis revealed: **algorithmic efficiency** tackles computational intensity through better model design, **compute efficiency** addresses hardware utilization gaps by maximizing the work extracted from each computational unit, and **data efficiency** solves the data saturation problem by extracting maximum information from limited examples. Targeted optimizations across these dimensions can achieve sustainable, accessible, high-performance AI systems that brute-force scaling cannot deliver.
...
**Precision Optimization** reduces computational requirements through quantization, which maps high-precision floating-point values to lower-precision representations. But why does reducing precision work at all? Consider that most neural network weights cluster near zero, and most activations fall within predictable ranges. The rich detail of 32-bit floating point often captures noise rather than signal. Neural networks demonstrate inherent robustness to precision reduction precisely because they are overparameterized: the redundancy that enables pruning also provides tolerance for reduced numerical precision. Empirically, INT8 quantization achieves 4x memory reduction and 2-4x inference speedup while typically maintaining 98-99% of FP32 accuracy [@Jacob_et_al_2018]. Quantization exploits this redundancy systematically, and understanding the mathematical foundations of quantization error enables informed decisions about when precision reduction will succeed and when it will fail.

##### Quantization: Mathematical Foundations {#sec-efficient-ai-quantization-foundations-a1b2}

To understand when quantization succeeds and when it fails, we must formalize the precision-accuracy trade-off. Neural networks typically use 32-bit floating point (FP32) for weights and activations, providing high precision but consuming substantial memory and computation. Quantization reduces bit width to INT8, INT4, or lower to improve efficiency at the cost of approximation error. The fundamental quantization operation maps continuous floating-point values to discrete integer levels:

$$
q = \text{clamp}\left(\left\lfloor \frac{x}{s} \right\rfloor + z, 0, 2^b - 1\right)
$$ {#eq-quantization-function}

where $x$ represents the original floating-point value, $s$ is the scale factor determining the range per quantized level, $z$ is the zero-point ensuring 0.0 maps to an integer, $b$ is the bit width (8 for INT8, 4 for INT4), and clamp ensures the result fits in $[0, 2^b - 1]$. Dequantization recovers the approximate value:

$$
\hat{x} = (q - z) \cdot s
$$ {#eq-dequantization}

The scale and zero-point parameters depend on the value range $[x_{\min}, x_{\max}]$. For symmetric quantization with $z = 0$:
...
$$
s = \frac{x_{\max} - x_{\min}}{2^b - 1}, \quad z = \left\lfloor -\frac{x_{\min}}{s} \right\rfloor
$$ {#eq-asymmetric-scale}

Symmetric quantization provides simpler arithmetic (no zero-point addition) and efficient zero multiplication, but asymmetric quantization achieves better range utilization for unbalanced distributions where $|x_{\min}| \neq |x_{\max}|$.

**Quantization Error Analysis.** The quantization error $e = \hat{x} - x$ depends on the rounding operation. For uniform quantization with step size $\Delta = s$, the error is bounded:

$$
|e| \leq \frac{\Delta}{2}
$$ {#eq-error-bound}

Assuming uniform distribution of values within each quantization interval, the error variance is:

$$
\mathbb{E}[e^2] = \frac{\Delta^2}{12}
$$ {#eq-error-variance}

The Signal-to-Quantization-Noise Ratio (SQNR) quantifies relative precision:

$$
\text{SQNR} = 6.02b + 1.76 \text{ dB}
$$ {#eq-sqnr}

This relationship shows each additional bit improves SQNR by approximately 6 dB. For INT8 ($b=8$), SQNR $\approx$ 49.9 dB, while for INT4 ($b=4$), SQNR $\approx$ 25.8 dB. Lower SQNR indicates higher relative error, establishing quantitative thresholds for acceptable precision reduction.

**When Quantization Preserves Accuracy.** Neural networks tolerate quantization error when:

1. **Model has redundant precision**: Many weights cluster near zero with minimal gradient contribution, making them prunable candidates. Smooth loss landscapes with small gradients indicate robustness to perturbation.

2. **Careful calibration**: Representative calibration data ensures scale factors capture actual value distributions. Per-channel scaling (separate $s, z$ for each output channel) provides 2-3% accuracy improvement over per-tensor scaling with minimal hardware overhead, as modern Tensor Cores support per-channel quantization natively.

3. **Architecture compatibility**: Batch normalization layers absorb quantization error by renormalizing activations. Deeper networks demonstrate greater robustness through error averaging across layers. However, attention mechanisms prove sensitive due to outlier activations requiring dynamic per-token quantization.

**Bit Width Selection.** @tbl-quantization-bitwidth summarizes compression-accuracy-speedup trade-offs across bit widths.

| Bit Width | Memory Reduction | Typical Speedup | Accuracy Impact | Primary Use Case |
|-----------|------------------|-----------------|-----------------|------------------|
| FP32 | 1× (baseline) | 1× | Baseline | Training default |
| FP16/BF16 | 2× | 1.5-3× | Negligible | Training standard |
| INT8 | 4× | 2-4× | <1% typical | Inference default |
| INT4 | 8× | 3-6× | 1-3% typical | Aggressive inference |
| INT2/Binary | 16-32× | 5-10× | 5-10% typical | Extreme constraints |

: Quantization bit width trade-offs for neural network inference. {#tbl-quantization-bitwidth}

Selection factors include hardware support (Tensor Cores prefer INT8), accuracy tolerance (production systems typically require <1% loss), deployment constraints (mobile and edge favor INT4+), and development time (post-training quantization is faster, but quantization-aware training achieves better accuracy).

**Quantization Granularity.** The granularity at which scale and zero-point are computed significantly affects accuracy:

- **Per-tensor quantization**: Single $(s, z)$ pair for the entire weight tensor. Simple and hardware-efficient but performs poorly with outliers.

- **Per-channel quantization**: Separate $(s, z)$ for each output channel. Provides 2-3% accuracy improvement with minimal hardware overhead. Recommended for weights as modern Tensor Cores support this natively.

- **Per-token quantization** (activations): Dynamic scaling for each token or sample. Essential for transformer activations with outlier values but introduces higher overhead. Necessary for maintaining accuracy in attention mechanisms.

The specific quantization algorithms including post-training quantization (PTQ) calibration methods, quantization-aware training (QAT) with fake quantization nodes, mixed-precision strategies, and hardware-specific optimizations are detailed in @sec-model-optimizations.

##### Pruning Criteria: Choosing What to Remove {#sec-efficient-ai-pruning-criteria-c3d4}

While quantization reduces numerical precision, pruning eliminates unnecessary parameters entirely. The critical question is: which weights should be removed? Multiple pruning criteria have been developed, each with distinct theoretical foundations and practical trade-offs.

**Magnitude-Based Pruning.** The simplest and most widely used criterion prunes weights with smallest absolute values:

$$
\text{Prune if: } |w_{ij}| < \tau
$$ {#eq-magnitude-pruning}

where $\tau$ is a threshold typically set to achieve target sparsity. The rationale: small weights contribute minimally to network output, making them safe to remove. For a linear layer $y = Wx + b$, removing weight $w_{ij}$ eliminates its contribution $w_{ij}x_j$ to output $y_i$. When $|w_{ij}| \approx 0$, this contribution is negligible.

**Advantages:** Simple to compute, requires only forward pass, no second-order information needed, works well empirically (90%+ sparsity achievable on many networks).

**Limitations:** Ignores gradient information (a small weight might have large gradients, indicating importance for optimization), does not account for weight correlations (multiple small weights might collectively be important), scale-dependent (batch normalization scaling affects magnitude rankings).

**Gradient-Based Pruning.** Incorporates optimization dynamics by considering both magnitude and gradient:

$$
\text{Score}(w_{ij}) = |w_{ij}| \cdot |\frac{\partial \mathcal{L}}{\partial w_{ij}}|
$$ {#eq-gradient-pruning}

where $\mathcal{L}$ is the loss function. This criterion approximates the change in loss when removing $w_{ij}$ using first-order Taylor expansion:

$$
\Delta \mathcal{L} \approx -\frac{\partial \mathcal{L}}{\partial w_{ij}} \cdot \Delta w_{ij} = -\frac{\partial \mathcal{L}}{\partial w_{ij}} \cdot w_{ij}
$$ {#eq-gradient-taylor}

Pruning weights with smallest $|w_{ij} \cdot \nabla_{w_{ij}} \mathcal{L}|$ minimizes loss increase.

**Advantages:** Incorporates gradient information reflecting training dynamics, better theoretical justification than magnitude alone, accounts for optimization trajectory.

**Limitations:** Requires gradient computation (more expensive than magnitude alone), gradients vary during training (scores unstable), still first-order approximation (ignores curvature).

**Hessian-Based Pruning (Second-Order Methods).** Uses second-order information to estimate loss change more accurately. The second-order Taylor expansion provides:

$$
\Delta \mathcal{L} \approx \frac{\partial \mathcal{L}}{\partial w_{ij}} \cdot \Delta w_{ij} + \frac{1}{2} \Delta w_{ij}^T H_{ij} \Delta w_{ij}
$$ {#eq-hessian-taylor}

where $H_{ij}$ is the Hessian (second derivative of loss with respect to weights). For pruning ($\Delta w_{ij} = -w_{ij}$):

$$
\Delta \mathcal{L} \approx -\frac{\partial \mathcal{L}}{\partial w_{ij}} \cdot w_{ij} + \frac{1}{2} w_{ij}^2 H_{ij}
$$ {#eq-hessian-loss}

At trained network where $\frac{\partial \mathcal{L}}{\partial w_{ij}} \approx 0$, the dominant term becomes:

$$
\text{Score}(w_{ij}) \approx \frac{1}{2} w_{ij}^2 H_{ij}
$$ {#eq-hessian-score}

This is the **Optimal Brain Damage** criterion: prune weights with smallest $w_{ij}^2 H_{ij}$ (saliency).

**Advantages:** Most accurate loss change estimate, accounts for weight interactions through Hessian, strong theoretical foundation, can achieve higher sparsity with less accuracy loss.

**Limitations:** Computing full Hessian is prohibitively expensive for large networks ($O(n^2)$ parameters), typically use diagonal approximation $H_{ii}$ (assumes parameter independence, less accurate), requires well-trained network where $\nabla \mathcal{L} \approx 0$.

**Structured Pruning Criteria.** While the above criteria apply to individual weights, structured pruning removes entire groups (channels, filters, attention heads). For channel pruning in CNNs, common criteria include:

**Channel norm:** Remove channels with smallest L1 or L2 norm:

$$
\text{Score}(\text{channel}_k) = \|\mathbf{w}_k\|_1 = \sum_i |w_{ik}|
$$ {#eq-channel-norm}

**Feature map statistics:** Measure activation magnitudes across training data:

$$
\text{Score}(\text{channel}_k) = \mathbb{E}_{\mathbf{x}}[|\mathbf{a}_k(\mathbf{x})|]
$$ {#eq-channel-activation}

where $\mathbf{a}_k(\mathbf{x})$ is the activation of channel $k$ for input $\mathbf{x}$.

**Taylor expansion for structured pruning:** Extend gradient-based methods to measure importance of entire structures.

@tbl-pruning-criteria compares these criteria systematically.

| Criterion | Computational Cost | Accuracy | Sparsity | Theoretical Foundation | Best Use Case |
|-----------|-------------------|----------|----------|----------------------|---------------|
| Magnitude | O(n) (single pass) | Good | High (90%+) | Weak (intuitive) | Fast pruning, initial experiments |
| Gradient | O(n) (+ backprop) | Better | High (90%+) | First-order Taylor | Incorporating training dynamics |
| Hessian | O(n²) or O(n) approx | Best | Highest (95%+) | Second-order optimal | Maximum accuracy retention |
| Channel Norm | O(n) (per channel) | Moderate | Moderate (50-70%) | Structural importance | Guaranteed speedup on hardware |

: Pruning criteria comparison: computational requirements, accuracy preservation, achievable sparsity, and appropriate use cases. {#tbl-pruning-criteria}

**Practical Recommendations:**

1. **Start with magnitude pruning**: Simple, fast, works well for initial exploration. Achieves 80-90% sparsity on many networks with <1% accuracy loss.

2. **Use gradient-based for better accuracy**: When magnitude pruning shows accuracy degradation, incorporate gradient information. Particularly useful during iterative pruning where network continues training.

3. **Apply Hessian methods for extreme sparsity**: When pushing beyond 90% sparsity or working with accuracy-critical applications. Use diagonal Hessian approximation for tractability.

4. **Choose structured pruning for deployment**: When actual speedup matters more than theoretical compression. Guarantees hardware-friendly computation regardless of criterion used.

**Iterative vs. One-Shot Pruning.** All criteria can be applied iteratively (prune-train-prune cycles) or in one shot. Iterative pruning typically achieves higher sparsity with better accuracy:

- **One-shot**: Prune once to target sparsity, fine-tune. Faster but may lose accuracy.
- **Iterative**: Gradually increase sparsity (e.g., 20% → 40% → 60% → 80%). Each cycle allows network to recover. Slower but better accuracy retention.

The **Lottery Ticket Hypothesis** suggests that iterative pruning's success comes from finding "winning tickets"—sparse subnetworks that, when trained from the original initialization, match full network performance. This theoretical insight validates iterative approaches empirically.

The specific implementation details including pruning schedules, fine-tuning strategies, layer-wise sensitivity analysis, and structured pruning algorithms are covered in @sec-model-optimizations.

##### Knowledge Distillation: Learning from Teacher Models {#sec-efficient-ai-knowledge-distillation-e5f6}

Knowledge distillation[^fn-knowledge-distillation] transfers capabilities from large teacher models to efficient student models through a fundamentally different approach than quantization or pruning. While those techniques modify an existing architecture, distillation creates an entirely new smaller architecture trained to mimic the teacher's behavior.
...
**Core Concept.** The key insight is that a teacher's probability distribution contains more information than hard labels alone. Consider image classification: the hard label is "cat," but the teacher's output reveals similarity structure: "cat" (90%), "dog" (8%), "car" (2%). This "dark knowledge"—the pattern of similarities encoded in soft predictions—helps the student generalize more effectively.

**Distillation Loss Function.** Training combines two objectives:
...
$$
\mathcal{L}_{\text{hard}} = \text{CrossEntropy}(y_{\text{true}}, \mathbf{p}_{\text{student}})
$$ {#eq-hard-loss}

is the standard loss against true labels, and:

$$
\mathcal{L}_{\text{soft}} = T^2 \cdot \text{KL}(\mathbf{p}_{\text{teacher}}^T || \mathbf{p}_{\text{student}}^T)
$$ {#eq-soft-loss}

is the distillation loss measuring divergence between teacher and student probability distributions. The hyperparameters are $\alpha$ (weight balancing hard/soft losses, typically 0.1-0.5) and $T$ (temperature for softening distributions, typically 3-20). The $T^2$ factor compensates for gradient scaling with temperature.

**Temperature Softening.** Temperature $T$ controls distribution sharpness through softmax with temperature:

$$
p_i = \frac{\exp(z_i / T)}{\sum_j \exp(z_j / T)}
$$ {#eq-temperature-softmax}

where $z_i$ are the logits (pre-softmax values). As $T$ increases, the distribution becomes more uniform, revealing similarity structure:

- **$T = 1$** (standard softmax): Sharp, winner-take-all distribution. Example: Cat (0.90), Dog (0.08), Car (0.02).
- **$T = 5$** (higher temperature): Softer distribution revealing similarities. Example: Cat (0.50), Dog (0.30), Car (0.15), revealing that "dog is more similar to cat than car."
- **$T \to \infty$**: Approaches uniform distribution where all classes are equally likely.

Higher temperature reveals the relative similarity between incorrect classes, providing rich training signal beyond the binary correct/incorrect from hard labels.

**Why Soft Labels Help: Dark Knowledge.** The information content differs dramatically:

- **Hard labels**: $[0, 0, 1, 0, 0]$ (one-hot encoding, no similarity information)
- **Soft labels**: $[0.05, 0.30, 0.50, 0.10, 0.05]$ (similarity structure preserved)

Soft labels encode:

1. **Relative similarity between classes**: Which incorrect classes are "close" to the correct one
2. **Uncertainty in predictions**: How confident the teacher is about its prediction
3. **Generalization structure**: Patterns learned beyond the training labels

**Example:** MNIST digit classification for a "3" image:

- **Hard label**: 3 (no ambiguity, binary signal)
- **Soft label**: 3 (0.8), 5 (0.10), 8 (0.08), others (0.02)
- **Dark knowledge revealed**: The digit "3" shares visual features with "5" (curved bottom) and "8" (curved shapes), but not with "1" or "4" (straight lines)

The student learns this similarity structure, improving generalization to ambiguous or out-of-distribution examples.

**Mathematical Justification.** Why does matching soft targets work better than hard targets? Consider the gradient of the cross-entropy loss with respect to student logits $z_i$:

$$
\frac{\partial \mathcal{L}_{\text{soft}}}{\partial z_i} = \frac{1}{T}(p_i^{\text{student}} - p_i^{\text{teacher}})
$$ {#eq-soft-gradient}

The gradient magnitude is inversely proportional to temperature, and the direction points toward matching the teacher's distribution. For incorrect classes with $p_i^{\text{teacher}} > 0$ (even if small), the student receives positive gradient signal to increase its probability for those classes, encoding the similarity structure.

In contrast, hard labels provide no gradient signal for incorrect classes (probability is exactly 0), losing this similarity information.

**Distillation Effectiveness.** Empirical results demonstrate significant compression with minimal accuracy loss:

**BERT → DistilBERT:**
- Compression: 110M → 66M parameters (40% reduction)
- Accuracy: 97% of BERT's GLUE performance
- Speedup: 1.6× inference speedup
- Training: 40% less compute than training BERT from scratch

**ResNet-34 → MobileNetV2:**
- Compression: 21M → 3.4M parameters (6× reduction)
- Accuracy: 92% of ResNet-34 ImageNet performance
- Speedup: 3× inference speedup on mobile devices

General observation across tasks: distillation typically enables 2-4× compression with 95-98% accuracy retention.

**When Distillation Works Best.**

1. **Teacher is overparameterized**: Large capacity models (billions of parameters) learn rich representations with substantial redundancy. The teacher's expressiveness provides high-quality soft targets. Example: Distilling GPT-3.5 → Phi-2 benefits from the teacher's massive pre-training.

2. **Student architecture is reasonable**: The student must have sufficient capacity to capture the teacher's knowledge. Generally, 0.25-0.5× teacher size works well. Too small: cannot represent the learned function. Too large: minimal benefit from distillation.

3. **Sufficient training data**: Distillation requires the teacher to be well-trained first. More data → better teacher → better distillation quality. In low-data regimes, distillation provides less benefit as the teacher itself may not have learned robust representations.

4. **Combined with other techniques**: Distillation composes multiplicatively with other compression methods:
   - Distillation + quantization: DistilBERT INT8 achieves 8× compression (2× from distillation, 4× from quantization)
   - Distillation + pruning: Sparse student networks further reduce computation
   - Multiplicative benefits: combining techniques yields larger total compression than either alone

**Feature-Based Distillation.** Beyond matching output distributions, intermediate layer representations can also be transferred:

$$
\mathcal{L}_{\text{feature}} = \|\mathbf{h}_{\text{student}} - W\mathbf{h}_{\text{teacher}}\|^2
$$ {#eq-feature-distillation}

where $\mathbf{h}_{\text{student}}$ and $\mathbf{h}_{\text{teacher}}$ are intermediate layer activations, and $W$ is a learned projection matrix (when dimensions differ). This approach transfers the teacher's internal representations, not just final predictions, often improving student performance particularly for vision tasks where intermediate features capture visual hierarchies.

**Practical Considerations.** Key design choices include:

- **Temperature selection**: Start with $T=3$ to 5, increase if student struggles to learn. Too low (T=1): distributions too sharp, minimal dark knowledge. Too high (T>20): distributions too uniform, signal diluted.

- **Balance parameter $\alpha$**: Typical range 0.1 to 0.5. Higher $\alpha$ emphasizes hard labels (ground truth), lower $\alpha$ emphasizes teacher matching. Start with $\alpha=0.5$ and adjust based on student performance.

- **Data augmentation**: Distillation benefits from augmented data even if the teacher was trained without it, as the teacher provides consistent soft targets across augmentations.

- **Multiple teachers**: Ensemble distillation aggregates predictions from multiple teachers, improving student robustness and accuracy beyond single-teacher distillation.

The specific implementation details including distillation training loops, temperature tuning strategies, feature-based distillation architectures, self-distillation, and online distillation variants are covered in @sec-model-optimizations.
...
[^fn-mobilenet]: **MobileNet**: Google's efficient architecture (2017) using depthwise separable convolutions that decompose standard convolutions into depthwise (spatial) and pointwise (channel) operations. MobileNet-v1 achieves 70.6% ImageNet accuracy with 4.2M parameters vs. VGG-16's 138M, enabling real-time inference on smartphones at 15-30 FPS while consuming <100mW.
...
[^fn-squeezenet]: **SqueezeNet**: Compact CNN architecture (2016) demonstrating that 50× parameter reduction is achievable without accuracy loss. Uses "fire modules" with squeeze (1×1 convolutions reducing channels) and expand (mixed 1×1 and 3×3) layers. The 0.5MB compressed model fits in on-chip SRAM, eliminating memory bandwidth bottlenecks for embedded inference.
...
##### AutoML for Efficiency: Neural Architecture Search {#sec-efficient-ai-automl-nas-b7c8}

Compact architectures like MobileNet and EfficientNet demonstrate that efficiency can be designed in rather than compressed after. But designing such architectures requires extensive expertise and experimentation, often taking years of researcher effort to develop effective design principles. Neural Architecture Search (NAS) automates this discovery process, systematically exploring architecture design spaces to discover models optimized for efficiency constraints without manual intervention.

**The NAS Problem Formulation.** NAS frames architecture design as an optimization problem:

$$
\mathbf{a}^* = \arg\max_{\mathbf{a} \in \mathcal{A}} \text{Accuracy}(\mathbf{a}) \quad \text{subject to} \quad \text{Cost}(\mathbf{a}) \leq C
$$ {#eq-nas-problem}

where $\mathbf{a}$ represents an architecture from search space $\mathcal{A}$, and $C$ is the resource budget (FLOPs, latency, energy, or memory). This formulation enables discovering architectures that balance accuracy and efficiency rather than maximizing accuracy alone.

**Search Space Design.** The search space defines what architectures can be discovered. Three common approaches offer different trade-offs:

**Macro search spaces** define entire network structures from scratch, searching over layer types, connections, and topology. Highly flexible but expensive to search (thousands of GPU-days). Example: discovering novel architectures like NASNet.

**Cell-based search spaces** discover reusable building blocks ("cells") that are stacked to form complete networks. More efficient search (hundreds of GPU-days) by exploiting modularity. Example: EfficientNet's discovered cells.

**Network morphism spaces** start from existing architectures and apply transformations (widening, deepening, inserting layers). Fastest search (tens of GPU-days) by leveraging transfer learning. Example: Efficient architectures derived from ResNet.

**Search Strategies.** Finding optimal architectures requires navigating massive search spaces. Three primary strategies balance exploration quality and computational cost:

**Reinforcement Learning (RL)-based search** treats architecture selection as sequential decision-making. An RL agent proposes architectures, trains them to measure accuracy, and receives accuracy as reward signal. The agent learns to propose better architectures over time.

$$
\mathbb{E}_{\mathbf{a} \sim \pi_\theta} [R(\mathbf{a})] \to \max_\theta
$$ {#eq-nas-rl}

where $\pi_\theta$ is the policy network parameterized by $\theta$, and $R(\mathbf{a})$ is the reward (validation accuracy minus cost penalty).

**Advantages:** Can discover novel architectures, handles discrete search spaces naturally.

**Limitations:** Sample inefficient (requires training thousands of architectures), expensive (10,000+ GPU-hours typical).

**Evolutionary algorithms** maintain a population of architectures, mutating and recombining high-performing candidates. Better exploration than random search but still expensive.

**Gradient-based search (differentiable NAS)** relaxes the discrete architecture search to continuous optimization. The key insight: represent the search space as a differentiable supernet where all candidate operations are computed in parallel, weighted by architecture parameters $\alpha$:

$$
\bar{o}^{(i,j)} = \sum_{o \in \mathcal{O}} \frac{\exp(\alpha_o^{(i,j)})}{\sum_{o' \in \mathcal{O}} \exp(\alpha_{o'}^{(i,j)})} \cdot o(x)
$$ {#eq-darts}

where $\mathcal{O}$ is the set of candidate operations (e.g., 3×3 conv, 5×5 conv, identity, zero), and $\alpha$ weights determine which operations are selected. After training, discrete architectures are extracted by selecting the operation with highest weight per edge.

**Advantages:** Much faster (1-4 GPU-days), gradient-based optimization is sample efficient.

**Limitations:** Memory intensive (all operations computed), discretization gap (continuous relaxation may not match discrete performance).

**Performance Estimation.** Training thousands of architectures to convergence is prohibitively expensive. Efficiency-focused NAS employs several acceleration techniques:

**Weight sharing (one-shot NAS):** Train a single supernet containing all candidate architectures as subnetworks. Evaluate candidates by inheriting shared weights rather than training from scratch. Reduces search cost from thousands to single GPU-days, but weight sharing introduces bias (shared weights may not reflect independent training performance).

**Early stopping:** Train candidates for few epochs (e.g., 5-10) and use early performance to predict final accuracy. Assumes relative ranking remains stable, which holds for many architectures but not all.

**Learning curve extrapolation:** Fit parametric curves to early training dynamics and extrapolate to convergence. More accurate than raw early stopping but still approximates final performance.

**Efficiency-Aware Search Objectives.** Pure accuracy maximization often yields inefficient architectures. Efficiency-aware NAS incorporates resource constraints directly:

**Latency-constrained search:**

$$
\mathbf{a}^* = \arg\max_{\mathbf{a}} \text{Accuracy}(\mathbf{a}) \quad \text{s.t.} \quad \text{Latency}(\mathbf{a}) \leq L_{\text{target}}
$$ {#eq-nas-latency}

**Multi-objective optimization:**

$$
\mathbf{a}^* = \arg\max_{\mathbf{a}} [\text{Accuracy}(\mathbf{a}) - \lambda \cdot \text{Cost}(\mathbf{a})]
$$ {#eq-nas-multiobjective}

where $\lambda$ balances accuracy and efficiency. By varying $\lambda$, we discover the Pareto frontier of accuracy-efficiency trade-offs.

**Hardware-Aware NAS.** Efficiency depends on target hardware. Hardware-aware NAS incorporates platform-specific costs:

- **Measured latency:** Profile candidate operations on target hardware (mobile CPU, GPU, edge TPU) and use measured latencies as cost.
- **Learned cost models:** Train regression models predicting latency from architecture features when profiling is expensive.
- **Energy modeling:** Incorporate energy consumption models for battery-constrained devices.

MnasNet pioneered this approach, discovering MobileNetV2-like architectures by searching directly for mobile inference latency [@tan2019mnasnet]. This hardware-awareness proved crucial: FLOPs alone correlate poorly with actual latency due to memory access patterns, parallelism, and hardware-specific optimizations.

**Practical Impact: EfficientNet Case Study.** EfficientNet demonstrates NAS impact on efficiency. The search discovered:

1. **Optimal baseline architecture (EfficientNet-B0):** 5.3M parameters, 0.39B FLOPs, 77.1% ImageNet accuracy—matching larger ResNet-50 (25M parameters, 4.1B FLOPs).

2. **Compound scaling coefficients:** Rather than manually tuning depth/width/resolution, NAS discovered scaling relationships: depth $\propto 1.2^\phi$, width $\propto 1.1^\phi$, resolution $\propto 1.15^\phi$. This principled scaling yielded EfficientNet-B7: 84.4% accuracy (state-of-the-art 2019) with 8.4× fewer parameters than previous best.

3. **Efficiency gains:** Across the EfficientNet family, NAS architectures achieve 5-10× better accuracy/FLOPs ratio than manually designed counterparts (ResNet, DenseNet, Inception).

**Limitations and Considerations.** NAS is not a panacea:

**Search cost:** Even efficient NAS requires significant resources (100-1000 GPU-hours typical). Diminishing returns for well-studied domains where manual designs are mature.

**Transferability:** Architectures discovered for one task may not transfer to others. Domain-specific NAS often required.

**Interpretability:** Discovered architectures can be complex and difficult to understand, hindering manual improvements.

**Overfitting to search objective:** Architectures may overfit validation sets used during search, requiring careful held-out test evaluation.

**When to Use NAS.** NAS provides most value in:

- **Novel deployment scenarios:** New hardware platforms (emerging edge accelerators) where no established architectures exist.
- **Extreme efficiency requirements:** Applications needing last 1-2% accuracy with tight resource budgets.
- **Large-scale production:** Efficiency gains (e.g., 20% latency reduction) amortize search costs across millions of inferences.
- **Research exploration:** Discovering architectural principles rather than single architectures.

For standard deployments with mature architecture families (ResNet, ViT, BERT), manual architecture selection often suffices. NAS shines when pushing efficiency frontiers for specific, well-defined deployment constraints.

The implementation details of NAS algorithms, search space encodings, and efficient search strategies are covered in @sec-model-optimizations within the broader context of architecture optimization techniques.
...
Active learning[^fn-active-learning] prioritized labeling only the most informative data points [@Settles_2009].
...
While efficiency in machine learning is often framed as a technical challenge, it is also deeply tied to broader questions about AI systems' purpose and impact. Designing efficient systems involves navigating not only practical trade-offs but also complex ethical and philosophical considerations.
