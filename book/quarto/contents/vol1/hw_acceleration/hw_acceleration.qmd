...
cpu_utilization_min_value = 5
cpu_utilization_max_value = 10
cpu_gflops_value = 100
...
# PURPOSE
# =============================================================================
# Purpose: Calculate Amdahl's Law speedup for ResNet-50 and GPT-2 on H100
# Used in: Lighthouse callout "Amdahl's Law on H100"
...
for y, v, l in [(2012, 1.0, "AlexNet"), (2017, 10**(demand_slope*5), "Transformer"), (2020, 10**(demand_slope*8), "GPT-3"), (2023, 10**(demand_slope*11), "GPT-4")]:
    ...
    ax.annotate(l, (y, v), xytext=(0, 8), textcoords='offset points', fontsize=8, ha='center', color=COLORS['RedLine'], fontweight='bold')
...
# PURPOSE
# =============================================================================
# Purpose: Calculate energy efficiency comparison between vector unit and systolic array
# Used in: Callout "The Energy Advantage of Pulsing Data"
...
systolic_size_value = SYSTOLIC_ARRAY_DIM
compute_energy_pj_value = 1 # pJ per MAC operation (approximate for INT8/FP16 mix)
...
ax.text(2022, 3e6, 'Memory Wall', rotation=90, va='bottom', fontsize=10, color=COLORS['primary'], fontweight='bold')
...
# DATA
# =============================================================================
proc_names = ["NVIDIA Tesla K80", "Google TPU v2", "NVIDIA Tesla V100", "NVIDIA A100", "Google TPU v4", "NVIDIA H100", "Google TPU v6e"]
proc_years = [2014, 2017, 2017, 2020, 2021, 2022, 2024]
proc_bw = [480, 600, 900, 2000, 1200, 3000, 1640]
proc_log_bw = np.log10(proc_bw)

model_names = ["AlexNet", "VGG-16", "ResNet-50", "BERT Large", "GPT-3", "PaLM", "GPT-4", "Gemini 1"]
...
for name, year, val in zip(proc_names[:3], proc_years[:3], proc_log_bw[:3]):
    ax.annotate(name, (year, val), textcoords='offset points', xytext=(0, 10), fontsize=7, color=COLORS['BlueLine'], ha='center')
for name, year, val in zip(model_names[-4:], model_years[-4:], model_log_params[-4:]):
    ax.annotate(name, (year, val), textcoords='offset points', xytext=(0, 8), fontsize=7, color=COLORS['RedLine'], ha='center')

mid_y = (np.polyval(proc_fit, 2020) + np.polyval(model_fit, 2020)) / 2
ax.text(2020, mid_y, 'AI Memory Wall', fontsize=10, fontweight='bold', ha='center', color=COLORS['primary'])
...
#| echo: false
from physx.constants import A100_MEM_BW, H100_MEM_BW, A100_FLOPS_FP16_TENSOR, H100_FLOPS_FP16_TENSOR
from physx.formatting import fmt
from physx.constants import flop, byte
...
legacy_ai_str = fmt(legacy_ai_value, precision=0, commas=False)
bandwidth_ratio_str = fmt(bandwidth_ratio_value, precision=1, commas=False)
flops_ratio_str = fmt(flops_ratio_value, precision=1, commas=False)
relu_below_roofline_str = fmt(relu_below_roofline_value, precision=0, commas=True)
```
...
#| echo: false
from physx.formatting import fmt
from physx.constants import byte, MB, flop, GFLOPs, MFLOPs
# =============================================================================
# PURPOSE
# =============================================================================
# Purpose: Analyze transformer layer arithmetic intensity for QKV projection and softmax
# Used in: Callout "Transformer Layer Analysis"
...
from physx.constants import GB, GPT2_PARAMS, BYTES_FP16, flop, GFLOPs
...
tensor_dim = 1024
bytes_fp32 = BYTES_FP32.magnitude
...
llama7b_params = 7e9  # Hypothetical 7B model for worked example
llama7b_weights_gb = model_memory(llama7b_params, BYTES_FP16, GB)  # 14 GB
gpu_mem_gb = 16
headroom_gb = gpu_mem_gb - llama7b_weights_gb

# Bandwidth check: 70B model on 1TB/s GPU
model_70b_params = 70e9  # Hypothetical 70B model for worked example
model_70b_size_gb = model_memory(model_70b_params, BYTES_FP16, GB)  # 140 GB
