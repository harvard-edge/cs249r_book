...
class SystolicOpsCalc:
    ...
    tpuv4_bw_str     = fmt_qty(tpuv4_bw_gbs * GB / second, GB / second, precision=0, commas=True)
```
...
class InterconnectBandwidth:
    ...
    hbm_bw_str = fmt_qty(h_h100.memory.bandwidth, TB/second, precision=1, commas=False)
    nvlink_bw_str = fmt_qty_int(nvlink_bw * GB / second, GB / second, commas=False)
    pcie_bw_str = fmt_qty_int(pcie_bw * GB / second, GB / second, commas=False)
    net_bw_str = fmt_qty_int(net_bw * GB / second, GB / second, commas=False)
    gap_ratio_str = fmt_multiple(round(hbm_bw/pcie_bw), precision=0, commas=False)
```
...
class AcceleratorEconomics:
    ...
    # Gaudi 2 specs
    gaudi_tflops_str = fmt_qty(gaudi_tf * TFLOP / second, TFLOP / second, precision=0, commas=False)
    gaudi_bw_str     = fmt_qty(gaudi_bw_value * GB / second, GB / second, precision=0, commas=True)
...
class InterconnectHierarchy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_a100_str = fmt_qty(nvlink_a100 * GB / second, GB / second, precision=0, commas=False)
    nvlink_h100_str = fmt_qty(nvlink_h100 * GB / second, GB / second, precision=0, commas=False)
    ib_hdr_str = fmt_qty(Systems.Fabrics.InfiniBand_HDR.bandwidth, Gbps, precision=0, commas=False, unit_label="Gb/s")
...
class RooflineGap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_tflops_str = fmt_qty(v100_tflops_value * TFLOP / second, TFLOP / second, precision=0, commas=False)
    v100_bw_tbs_str = fmt_qty(v100_bw_tbs_value * TB / second, TB / second, precision=1, commas=False)
    a100_tflops_fp16_str = fmt_qty(a100_tflops_fp16_value * TFLOP / second, TFLOP / second, precision=0, commas=False)
    a100_bw_tbs_str = fmt_qty(a100_bw_tbs_value * TB / second, TB / second, precision=2, commas=False)
    h100_tflops_fp16_str = fmt_qty(h100_tflops_fp16_value * TFLOP / second, TFLOP / second, precision=0, commas=False)
    h100_bw_tbs_str = fmt_qty(h100_bw_tbs_value * TB / second, TB / second, precision=2, commas=False)
...
class TransformerLayerCalc:
    ...
    qkv_mb_str          = fmt_qty(qkv_mb_value * MB, MB, precision=1, commas=False)
    ...
    softmax_mb_str      = fmt_qty(softmax_mb_value * MB, MB, precision=1, commas=False)
...
class Conv2dAnalysisCalc:
    ...
    conv_input_mb_str      = fmt_qty(conv_input_mb * MB, MB, precision=1, commas=False)
    conv_weights_mb_str    = fmt_qty(conv_weights_mb * MB, MB, precision=1, commas=False)
    conv_output_mb_str     = fmt_qty(conv_output_mb * MB, MB, precision=1, commas=False)
    conv_total_mb_str      = fmt_qty(conv_total_mb * MB, MB, precision=1, commas=False)
...
class DenseLayerAnalysisCalc:
    ...
    dense_input_kb_str     = fmt_qty(dense_input_kb * KB, KB, precision=1, commas=False)
    dense_weights_mb_str   = fmt_qty(dense_weights_mb * MB, MB, precision=1, commas=False)
    dense_output_kb_str    = fmt_qty(dense_output_kb * KB, KB, precision=1, commas=False)
    dense_total_mb_str     = fmt_qty(dense_total_mb * MB, MB, precision=1, commas=False)
    dense_ai_str           = fmt(dense_ai, precision=1, commas=False)
    dense_attainable_str   = fmt_qty(dense_attainable_tflops * TFLOP / second, TFLOP / second, precision=1, commas=False)
    dense_util_pct_str     = fmt_percent(dense_util_pct/100, precision=1, commas=False, style='symbol')
    a100_bw_str            = fmt_qty(a100_bw_gbs_value * GB / second, GB / second, precision=0, commas=True)
    a100_ridge_str         = fmt_int(round(a100_ridge_value), commas=False)
```
...
class LayernormAnalysisCalc:
    ...
    ln_input_mb_str     = fmt_qty(ln_input_mb * MB, MB, precision=1, commas=False)
    ln_params_kb_str    = fmt_qty(ln_params_kb * KB, KB, precision=1, commas=False)
    ln_output_mb_str    = fmt_qty(ln_output_mb * MB, MB, precision=1, commas=False)
    ln_total_mb_str     = fmt_qty(ln_total_mb * MB, MB, precision=1, commas=False)
...
class Gpt2ThroughputCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpt2_weight_gb_str    = fmt_qty(gpt2_weight_gb * GB, GB, precision=0, commas=False)
    ...
    gpt2_max_tflops_str   = fmt_qty(gpt2_max_tflops * TFLOP / second, TFLOP / second, precision=1, commas=False)
    gpt2_utilization_str  = fmt(gpt2_utilization, precision=1, commas=False)
    a100_tflops_fp16_str  = fmt_qty(a100_tflops_fp16_val * TFLOP / second, TFLOP / second, precision=0, commas=False)
    a100_bw_tbs_str       = fmt_qty(a100_bw_tbs_val * TB / second, TB / second, precision=2, commas=False)
...
class MemoryFootprintCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tensor_mb_str       = fmt_qty(tensor_mb * MB, MB, precision=1, commas=False)
    total_mb_str        = fmt_qty(total_mb * MB, MB, precision=1, commas=False)
```
...
class FusionBenefitsCalc:
    ...
    naive_mb_str = fmt_qty(naive_mb * MB, MB, precision=1, commas=False)
    fused_mb_str = fmt_qty(fused_mb * MB, MB, precision=1, commas=False)
```
...
class FpMemoryEnergyCalc:
    ...
    fp_layernorm_tflops_str = fmt_qty(layernorm_tflops * TFLOP / second, TFLOP / second, precision=0, commas=False)
    ...
    fp_peak_tflops_str      = fmt_qty(peak_tflops * TFLOP / second, TFLOP / second, precision=0, commas=False)
    fp_peak_bw_tbs_str      = fmt_qty(peak_bw_tbs * TB / second, TB / second, precision=0, commas=False)
...
class FpMultigpuScalingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fp_nvlink_bw_str     = fmt_qty(nvlink_bw_gbs * GB / second, GB / second, precision=0, commas=False)
    fp_sync_time_str     = fmt_time(sync_time_ms, 'millisecond', precision=2, commas=False)
...
class FpSmallBatchCalc:
    ...
    fp_t4_flops_str = fmt_qty(t4_flops * TFLOP / second, TFLOP / second, precision=0, commas=False)
    fp_t4_bw_str    = fmt_qty(t4_bw * GB / second, GB / second, precision=0, commas=False)
    fp_a100_roof_b1_str = fmt_qty(a100_roof_b1 * TFLOP / second, TFLOP / second, precision=1, commas=False)
    fp_t4_roof_b1_str = fmt_qty(t4_roof_b1 * TFLOP / second, TFLOP / second, precision=1, commas=False)
```
...
class FeasibilityAssessment:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    headroom_str = fmt_qty(headroom_gb * GB, GB, precision=0, commas=False)
    model_7b_gb_str = fmt_qty(model_7b_gb * GB, GB, precision=0, commas=False)
    model_70b_gb_str = fmt_qty(model_70b_gb * GB, GB, precision=0, commas=False)
    mem_bw_tb_s_str = fmt_qty((mem_bw_gb_s / THOUSAND) * TB / second, TB / second, precision=0, commas=False)
    latency_target_ms_str = fmt_time(latency_target_ms, 'millisecond', precision=0, commas=False)
    token_latency_ms_str = fmt_time(token_latency_ms, 'millisecond', precision=0, commas=False)
    frame_budget_str = fmt_time(frame_budget_ms, 'millisecond', precision=1, commas=False)
    params_7b_billion_str = fmt(params_7b / BILLION, precision=0, commas=False)
    model_7b_gb_str = fmt_qty(model_7b_gb * GB, GB, precision=0, commas=False)
    gpu_mem_gb_str = fmt_qty(gpu_mem_gb * GB, GB, precision=0, commas=False)
```
...
class CarbonRoiCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cpu_power_str      = fmt_qty(cpu_power_w * watt, watt, precision=0, commas=True)
    npu_power_str      = fmt_qty(npu_power_w * watt, watt, precision=0, commas=True)
    cpu_servers_str    = fmt(cpu_servers, precision=0)
    npu_chips_str      = fmt(npu_chips, precision=0)
    cpu_tflops_str     = fmt_qty(cpu_tflops * TFLOP / second, TFLOP / second, precision=0, commas=True)
    npu_tflops_str     = fmt_qty(npu_tflops * TFLOP / second, TFLOP / second, precision=0, commas=True)
    cpu_eff_str        = fmt_qty(cpu_eff * TFLOP / second / watt, TFLOP / second / watt, precision=2, unit_label="TFLOP/s per W")
    npu_eff_str        = fmt_qty(npu_eff * TFLOP / second / watt, TFLOP / second / watt, precision=0, unit_label="TFLOP/s per W")
    eff_gap_str        = fmt(eff_gap, precision=0)
    workload_str       = fmt(inferences_per_day / BILLION, precision=0)
    cpu_energy_day_str = fmt_qty(cpu_energy_kwh_day * ureg.kilowatt_hour, ureg.kilowatt_hour, precision=0, commas=True)
    npu_energy_day_str = fmt_qty(npu_energy_kwh_day * ureg.kilowatt_hour, ureg.kilowatt_hour, precision=0, commas=True)
    carbon_intensity_str = fmt_qty(carbon_kg_per_kwh * ureg.kilogram / ureg.kilowatt_hour, ureg.kilogram / ureg.kilowatt_hour, precision=3)
