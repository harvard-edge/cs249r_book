...
1.  **DRAM (HBM)**: The tensor starts here.
    ...
    *   **Latency**: Fetching this from off-chip memory takes ~`{python} TensorLifecycleCalc.latency_hbm_ns_str` ns (plus queuing delay).
...
2.  **L2 Cache**: The GPU's DMA engine pulls it here.
    *   **Latency**: ~`{python} TensorLifecycleCalc.latency_l2_ns_str` ns.
...
3.  **L1 Cache/Shared Memory**: A specific SM claims a tile of the audio.
    *   **Latency**: ~`{python} TensorLifecycleCalc.latency_l1_ns_str` ns.
...
class InterconnectHierarchy:
    ...
    ib_hdr_gbps_str = fmt_qty(ib_hdr_bw, Gbps, precision=0, commas=False, unit_label="Gb/s")
...
::: {.column-margin}
![](images/svg/vol1_hw_acceleration_margin_004.svg){width="100%" fig-alt="Three labeled cells side by side, weight-stationary, output-stationary, and input-stationary, each marking which tensor stays fixed in the array."}

*Three dataflow choices: weight-, output-, or input-stationary, by which tensor stays fixed.*
:::
...
class FpWorkloadFitRecap:
    ...
    a100_tflops_fp16_str = fmt_flop_rate(a100_peak_flops, unit=TFLOP / second, precision=0, commas=False)
    softmax_ai_low_str = fmt_arithmetic_intensity(softmax_ai_low, unit=flop / byte, precision=0, commas=False)
    softmax_ai_high_str = fmt_arithmetic_intensity(softmax_ai_high, unit=flop / byte, precision=0, commas=False)
    ...
    cpu_ridge_low_str = fmt_arithmetic_intensity(cpu_ridge_low, unit=flop / byte, precision=0, commas=False)
    cpu_ridge_high_str = fmt_arithmetic_intensity(cpu_ridge_high, unit=flop / byte, precision=0, commas=False)
