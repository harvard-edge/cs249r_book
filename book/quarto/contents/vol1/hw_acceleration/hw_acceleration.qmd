...
1.  **Vector Unit**: Loads $A$, loads $B$, computes $A \times B + C$, writes $C$.
    ...
    - **Energy**: $\approx 4 \times 640\text{ pJ} = \mathbf{`{python} vector_energy_str` \text{ pJ/OP}}$.

2.  **Systolic Array ($128 \times 128$ size)**: Loads $A$ and $B$ once at the edges. Data "pulses" through 128 processing elements.
    - **Data Movement**: 2 loads per 128 operations = `{python} systolic_access_str` DRAM accesses (per operation).
    - **Energy**: $\approx `{python} systolic_access_str` \times 640\text{ pJ} + 1 \text{ pJ (compute)} \approx \mathbf{`{python} systolic_energy_str` \text{ pJ/OP}}$.

**The Systems Conclusion**:
A systolic array is **`{python} energy_ratio_str`x more energy-efficient** than a naive vector unit for large matrix multiplications.
...
- FLOPs: $3 \times 32 \times 512 \times 768 \times 768 = `{python} qkv_flops_b_str`$ billion FLOPs
- Bytes: (input + weights + output) $= (32 \times 512 \times 768 + 3 \times 768 \times 768 + 32 \times 512 \times 768 \times 3) \times 2 \approx `{python} qkv_mb_str`$ MB
- AI $= `{python} qkv_flops_b_str`\text{B} / `{python} qkv_mb_str`\text{M} = `{python} qkv_ai_str`$ FLOP/byte, which is **compute-bound on A100** (above `{python} a100_ridge` threshold)

*Softmax*:

- FLOPs: $32 \times 12 \times 512 \times 512 \times 3 \approx `{python} softmax_flops_m_str`\text{M}$ FLOPs (exp, sum, div)
- Bytes: $32 \times 12 \times 512 \times 512 \times 2 \times 2 = `{python} softmax_mb_str`$ MB
- AI $= `{python} softmax_flops_m_str`\text{M} / `{python} softmax_mb_str`\text{M} = `{python} softmax_ai_str`$ FLOP/byte, which is **memory-bound**
...
# Memory traffic (FP16)
dense_input_kb = dense_batch * dense_in * 2 / 1024  # 128 KB → 131 KB
dense_weights_mb = dense_in * dense_out * 2 / 1e6  # 8.4 MB
dense_output_kb = dense_batch * dense_out * 2 / 1024  # 128 KB → 131 KB
...
dense_attainable_tflops = 2039 * dense_ai / 1e3  # ~63 TFLOPS (rounded from 2039 * 31.5 / 1000)
dense_util_pct = dense_attainable_tflops / 312 * 100  # % of peak FP16
...
ln_attainable_tflops = 2000 * ln_ai / 1e3  # 3 TFLOPS (using rounded 2000 GB/s)
...
*   **Maximum Throughput:** $`{python} gpt2_decode_ai_str` \text{ FLOP/Byte} \times `{python} a100_bw_tbs` \text{ TB/s} = \mathbf{`{python} gpt2_max_tflops_str` \text{ TFLOPS}}$.
*   **Utilization Ceiling:**
