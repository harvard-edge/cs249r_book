...
Hardware acceleration targets specific terms in the **Iron Law of ML Systems** ($Time = \frac{Data}{Bandwidth} + \frac{Ops}{Throughput \times Utilization}$). While data selection reduced the total data and model compression reduced the ops per sample, hardware acceleration increases the rate at which those ops execute by maximizing the Throughput and Bandwidth denominators. Yet acceleration has a hard ceiling, established by the following law.
