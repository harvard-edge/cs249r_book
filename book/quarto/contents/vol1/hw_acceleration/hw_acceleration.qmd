...
- H100 delivers $S = 500\times$ speedup over CPU for matrix multiply (`{python} h100_tflops_int8` TOPS INT8 vs. ~8 TOPS on baseline CPU without AMX extensions)
...
+:==================+=====================:+======================:+=====================:+======================:+
| **NVIDIA V100**   | ~$9,000              | `{python} v100_tflops` TFLOPS            | `{python} v100_bw` GB/s             | $72/TFLOP             |
+-------------------+----------------------+-----------------------+----------------------+-----------------------+
| **NVIDIA A100**   | ~$15,000             | `{python} a100_tflops_fp16` TFLOPS            | `{python} a100_bw` GB/s           | $48/TFLOP             |
+-------------------+----------------------+-----------------------+----------------------+-----------------------+
| **NVIDIA H100**   | ~$25,000-30,000      | `{python} h100_tflops_tf32` TFLOPS (TF32)     | `{python} h100_bw` GB/s           | ~$33/TFLOP            |
+-------------------+----------------------+-----------------------+----------------------+-----------------------+
| **Google TPUv4**  | ~$8,000*             | `{python} tpuv4_tflops` TFLOPS (BF16)     | `{python} tpuv4_bw` GB/s           | ~$29/TFLOP            |
+-------------------+----------------------+-----------------------+----------------------+-----------------------+
...
*   **A100 (2020)**: $`{python} a100_tflops_fp16` \text{ TF} / `{python} a100_bw_tbs` \text{ TB/s} \approx \mathbf{`{python} a100_ridge` \text{ Ops/Byte}}$.
*   **H100 (2023)**: $`{python} h100_tflops_fp16` \text{ TF} / `{python} h100_bw_tbs` \text{ TB/s} \approx \mathbf{`{python} h100_ridge` \text{ Ops/Byte}}$.
...
3.  **Ridge Point**: $`{python} h100_tflops_fp16` \text{ TFLOPS} / `{python} h100_bw_tbs` \text{ TB/s} \approx \mathbf{`{python} h100_ridge` \text{ Ops/Byte}}$.
...
$$P_{\text{attainable}} = `{python} a100_bw` \text{ GB/s} \times 31.5 \text{ FLOP/byte} = 63 \text{ TFLOPS}$$
...
*   **Ridge Point (Compute/BW):** $`{python} a100_tflops_fp16` / `{python} a100_bw_tbs` = \mathbf{`{python} a100_ridge` \text{ FLOPs/Byte}}$ (for FP16 Tensor Core).
...
Since Actual Intensity ($1.0$) $\ll$ Ridge Point ($`{python} a100_ridge`$), the system is **Bandwidth Bound**.

*   **Maximum Throughput:** $1.0 \text{ FLOP/Byte} \times `{python} a100_bw_tbs` \text{ TB/s} = \mathbf{`{python} a100_bw_tbs` \text{ TFLOPS}}$.
*   **Utilization Ceiling:**
    $$ \frac{`{python} a100_bw_tbs` \text{ TFLOPS (Actual)}}{`{python} a100_tflops_fp16` \text{ TFLOPS (Peak)}} \approx \mathbf{0.6\%} $$
