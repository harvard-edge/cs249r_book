...
from mlsysim.fmt import (
    fmt_int, fmt, fmt_qty, fmt_qty_int, check,
    fmt_memory, fmt_bandwidth, fmt_flop_rate, fmt_power, fmt_energy, fmt_count,
    fmt_params, fmt_carbon_intensity, fmt_compute_efficiency,
)
...
class HwAccelSetup:
    ...
    cpu_gap_max_str      = fmt_multiple(cpu_gap_max, precision=0, commas=False)
```
...
*   **Serial fraction** ($1-p$): Data loading, Python overhead, and kernel launch latency.\index{Serial Fraction!Amdahl bottleneck}\index{Kernel Launch Latency!serial overhead}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, fmt_math, fmt_ratio, check
...
class AmdahlH100:
    ...
    hw_speedup_str = fmt_multiple(hw_speedup_factor, precision=0, commas=False)
    ...
    amdahl_resnet_speedup_str = fmt_multiple(speedup_resnet, precision=1, commas=False)
    ...
    amdahl_gpt2_ceiling_speedup_str = fmt_multiple(ceiling_gpt2, precision=0, commas=False)
...
Despite a `{python} AmdahlH100.hw_speedup_str`$\times$ hardware advantage, total system speedup is only `{python} AmdahlH100.amdahl_resnet_speedup_str`$\times$. The `{python} AmdahlH100.serial_resnet_pct_str` serial fraction caps practical gains.
...
**The legacy**: This result ended the "General Purpose" era for AI. It proved that tailoring silicon to the **Algorithmic Primitive**\index{Algorithmic Primitive!matrix multiply} (Matrix Multiply) yields order-of-magnitude gains that Moore's Law alone could not deliver for decades.
...
class SystemsGapRates:
    ...
    # OUTPUT
    gpu_growth_per_year_factor_str = fmt_multiple(round(gpu_growth_per_year), precision=0, commas=False)
    demand_growth_per_year_factor_str = fmt_multiple(demand_growth_per_year, precision=1, commas=False)
    demand_supply_gap_per_year_str = fmt_multiple(demand_supply_gap_per_year, precision=1, commas=False)
```

[^fn-moores-law-scaling]: **Moore's Law**: The consequence for ML is not just slower hardware improvement but a structurally widening gap: in the illustrative assumptions used by @fig-systems-gap, model compute demand grows roughly `{python} SystemsGapRates.demand_growth_per_year_factor_str`$\times$ per year while accelerator peak supply improves roughly `{python} SystemsGapRates.gpu_growth_per_year_factor_str`$\times$ per year, widening the demand/supply gap by about `{python} SystemsGapRates.demand_supply_gap_per_year_str`$\times$ per year. This divergence makes algorithmic efficiency techniques—model compression, quantization, sparsity—structurally necessary rather than optional optimizations. \index{Moore's Law!ML systems consequence}
...
Every computing paradigm follows a distinct lifecycle of three phases: ferment (initial slow progress), take-off (exponential growth), and saturation (diminishing returns at physical limits). The two overlapping curves in @fig-tech-s-curve illustrate how general-purpose computing has entered its saturation phase, and the industry is now riding the steep take-off of a new S-curve driven by domain-specific architectures.
...
The trajectory yields a durable engineering rule: the era of "free" performance gains from general-purpose scaling is over. For decades, software engineers could rely on Moore's Law to accelerate existing code without architectural changes. The breakdown of Dennard scaling forced a decisive change: engineers can no longer wait for faster CPUs to solve computational bottlenecks but must instead design the hardware to fit the algorithm. This necessity of hardware-software co-design is why modern AI engineering requires deep understanding of the underlying silicon. Performance is now determined by how well the algorithm's memory access patterns and parallelism map to the specialized physical structures of domain-specific architectures.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_percent, fmt_flop_rate
...
class CpuMlInefficiency:
    ...
    cpu_gflops_str = fmt_flop_rate(cpu_delivered_flops, unit=GFLOP / second, precision=0, commas=False)

    # A100/Mobile specs for footnote comparison
    a100_tflops_fp16 = f"{Hardware.Cloud.A100.compute.peak_flops.to(TFLOP/second).magnitude:.0f}"
    a100_tflops_tf32 = fmt(Hardware.Cloud.A100.compute.precision_flops['tf32'].to(TFLOP/second).magnitude, precision=0, commas=False)
...
class MlAcceleratorCallout:
    ...
    # OUTPUT
    a100_tflops_fp16_str = fmt_flop_rate(a100_peak_flops, unit=TFLOP / second, precision=0, commas=False)
    a100_bw_tbs_str      = fmt_bandwidth(a100_bandwidth, unit=TB / second, precision=2, commas=False)
    cpu_gap_min_str      = fmt(cpu_gap_min, precision=0, commas=False)
    cpu_gap_max_str      = fmt_multiple(cpu_gap_max, precision=0, commas=False)
```
...
Three constraints shape accelerator architecture: massive parallelism, predictable data flow, and tolerance for reduced precision. Before examining the computational primitives that exploit these characteristics, we examine the architectural organization that enables their efficient execution. Modern AI accelerators achieve their dramatic performance improvements through a carefully orchestrated hierarchy of specialized components operating in concert.
...
class SystolicEnergy:
    ...
    energy_ratio_str = fmt_multiple(efficiency_ratio, precision=1, commas=False)
```
...
class AcceleratorEfficiencyAnchor:
    ...
    energy_dividend_str = fmt_multiple(energy_dividend, precision=0, commas=False)
...
class TilingPrinciple:
    ...
    reuse_factor_str = fmt_multiple(reuse_factor, precision=0, commas=False)
```
...
# │          SystolicOpsCalc.tpuv4_bw_gb_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int,  fmt
...
# │ Exports: InterconnectBandwidth.hbm_bw_tb_s_str,
# │          InterconnectBandwidth.nvlink_bw_gb_s_str,
# │          InterconnectBandwidth.pcie_bw_gb_s_str,
# │          InterconnectBandwidth.net_bw_gb_s_str,
# │          InterconnectBandwidth.gap_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware
from mlsysim.fmt import fmt_int, fmt, check, MarkdownStr, fmt_qty

class InterconnectBandwidth:
    """Namespace for Interconnect Hierarchy bandwidth taper."""
    h_h100 = Hardware.Cloud.H100

    hbm_bw = h_h100.memory.bandwidth
    nvlink_bw = Hardware.Cloud.H100.nvlink.bandwidth
    pcie_bw = Hardware.Cloud.H100.interconnect.bandwidth
    net_bw = Systems.Fabrics.InfiniBand_NDR.bandwidth
    ...
    hbm_bw_tb_s_str = fmt_bandwidth(hbm_bw, unit=TB/second, precision=1, commas=False)
    nvlink_bw_gb_s_str = fmt_bandwidth(nvlink_bw, unit=GB / second, precision=0, commas=False)
    pcie_bw_gb_s_str = fmt_bandwidth(pcie_bw, unit=GB / second, precision=0, commas=False)
    net_bw_gb_s_str = fmt_bandwidth(net_bw, unit=GB / second, precision=0, commas=False)
    gap_ratio_str = fmt_multiple(round((hbm_bw / pcie_bw).to_base_units().magnitude), precision=0, commas=False)
```
...
*   **HBM3**: `{python} InterconnectBandwidth.hbm_bw_tb_s_str`
*   **NVLink 4.0**: `{python} InterconnectBandwidth.nvlink_bw_gb_s_str`
*   **PCIe Gen5**\index{PCIe Gen5!bandwidth scaling}: `{python} InterconnectBandwidth.pcie_bw_gb_s_str`
*   **Network (NDR)**: `{python} InterconnectBandwidth.net_bw_gb_s_str`

**Systems insight**: The `{python} InterconnectBandwidth.gap_ratio_str`$\times$ gap between HBM and PCIe means that any data transfer from the CPU is a catastrophic performance event. **NVLink**\index{NVLink!PCIe wall} is the only way to scale computation across multiple GPUs without hitting the "PCIe Wall."
...
# │ Exports: AcceleratorEconomics.v100_tflops_str, AcceleratorEconomics.v100_bw_gb_s_str,
# │          AcceleratorEconomics.v100_price_str, AcceleratorEconomics.v100_pp_usd_str,
# │          AcceleratorEconomics.a100_tflops_fp16_str, AcceleratorEconomics.a100_bw_gb_s_str,
# │          AcceleratorEconomics.a100_bw_tbs_str, AcceleratorEconomics.a100_price_str,
# │          AcceleratorEconomics.a100_pp_usd_str, AcceleratorEconomics.h100_tflops_tf32_str,
# │          AcceleratorEconomics.h100_tflops_fp16_str, AcceleratorEconomics.h100_bw_gb_s_str,
# │          AcceleratorEconomics.h100_bw_tbs_str, AcceleratorEconomics.h100_price_str,
# │          AcceleratorEconomics.h100_pp_usd_str, AcceleratorEconomics.h100_tflops_fp8_str,
# │          AcceleratorEconomics.tpuv4_tflops_str,
# │          AcceleratorEconomics.tpuv4_bw_gb_s_str, AcceleratorEconomics.tpu_price_str,
# │          AcceleratorEconomics.tpu_pp_usd_str, AcceleratorEconomics.gaudi_tflops_str,
# │          AcceleratorEconomics.gaudi_bw_gb_s_str, AcceleratorEconomics.gaudi_price_str,
...
class AcceleratorEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # V100 specs
    v100_tflops_str = fmt_qty(h_v100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
    v100_bw_gb_s_str     = fmt_qty(h_v100.memory.bandwidth, GB/second, precision=0, commas=False)
    ...
    a100_tflops_fp16_str = fmt_qty(h_a100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
    a100_bw_gb_s_str          = fmt_qty(h_a100.memory.bandwidth, GB/second, precision=0, commas=True)
    a100_bw_tbs_str      = fmt_qty(h_a100.memory.bandwidth, TB/second, precision=2, commas=False)
    ...
    h100_tflops_fp16_str = fmt_flop_rate(h_h100.compute.peak_flops, unit=TFLOPs/second, precision=0, commas=False)
    h100_tflops_fp8_str  = fmt_flop_rate(Hardware.Cloud.H100.compute.precision_flops["fp8"], unit=TFLOPs/second, precision=0)
    h100_bw_gb_s_str          = fmt_qty(h_h100.memory.bandwidth, GB/second, precision=0, commas=True)
    ...
    # TPUv4 specs
    tpuv4_tflops_str = fmt_qty(h_tpu.compute.peak_flops, TFLOP/second, precision=0, commas=False)
    tpuv4_bw_gb_s_str     = fmt_qty(h_tpu.memory.bandwidth, GB/second, precision=0, commas=True)
    ...
    # Gaudi 2 specs
    gaudi_tflops_str = fmt_flop_rate(gaudi_flops, unit=TFLOP / second, precision=0, commas=False)
    gaudi_bw_gb_s_str     = fmt_bandwidth(gaudi_bw, unit=GB / second, precision=0, commas=True)
...
|:-----------------------------------|:-----------------------------------------------:|:-----------------------------------------------------------:|:-------------------------------------------------:|:------------------------------------------------:|
| **NVIDIA V100**\index{NVIDIA!V100} |  `{python} AcceleratorEconomics.v100_price_str` |       `{python} AcceleratorEconomics.v100_tflops_str`       |  `{python} AcceleratorEconomics.v100_bw_gb_s_str` | `{python} AcceleratorEconomics.v100_pp_usd_str`  |
| **NVIDIA A100**\index{NVIDIA!A100} |  `{python} AcceleratorEconomics.a100_price_str` |     `{python} AcceleratorEconomics.a100_tflops_fp16_str`    |  `{python} AcceleratorEconomics.a100_bw_gb_s_str` | `{python} AcceleratorEconomics.a100_pp_usd_str`  |
| **NVIDIA H100**                    |  `{python} AcceleratorEconomics.h100_price_str` | `{python} AcceleratorEconomics.h100_tflops_tf32_str` (TF32) |  `{python} AcceleratorEconomics.h100_bw_gb_s_str` | `{python} AcceleratorEconomics.h100_pp_usd_str`  |
| **Google TPUv4**                   |  `{python} AcceleratorEconomics.tpu_price_str`  |   `{python} AcceleratorEconomics.tpuv4_tflops_str` (BF16)   | `{python} AcceleratorEconomics.tpuv4_bw_gb_s_str` |  `{python} AcceleratorEconomics.tpu_pp_usd_str`  |
| **Intel Gaudi 2**                  | `{python} AcceleratorEconomics.gaudi_price_str` |    `{python} AcceleratorEconomics.gaudi_tflops_str` (FP8)   | `{python} AcceleratorEconomics.gaudi_bw_gb_s_str` | `{python} AcceleratorEconomics.gaudi_pp_usd_str` |
...
class B200FP4Specs:
    ...
    dense_pflops_str  = fmt_flop_rate(dense_flops, unit=PFLOP / second, precision=0, commas=False)
    sparse_pflops_str = fmt_flop_rate(sparse_flops, unit=PFLOP / second, precision=0, commas=False)
```
...
class MemoryWallH100Specs:
    ...
    h100_tflops_fp16_str = fmt_flop_rate(h100_peak_flops, unit=TFLOP / second, precision=0, commas=False)
    h100_bw_tbs_str      = fmt_bandwidth(h100_bandwidth, unit=TB / second, precision=2, commas=False)
```
...
# │
# │ Imports: book.tools.figures.style (book_style), mlsysim.core.constants
# │ Exports: (figure only, no prose variables)
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, MarkdownStr, fmt_energy_per_bit, fmt_flop_rate, fmt_memory, fmt_qty, fmt_time
...
class TensorLifecycleCalc:
    ...
    a100_tflops_fp16_str   = fmt_flop_rate(a100_fp16_flops, unit=TFLOP / second, precision=0, commas=False)
...
1.  **DRAM (HBM)**: The tensor starts here.
    *   **Size**: `{python} TensorLifecycleCalc.kws_samples_str` samples $\times$ `{python} TensorLifecycleCalc.kws_bytes_str` bytes (FP16) = `{python} TensorLifecycleCalc.kws_tensor_kb_str`.
...
# │          InterconnectHierarchy.ib_ndr, InterconnectHierarchy.ib_hdr_gbps_str,
# │          InterconnectHierarchy.ib_ndr_gbps_str, InterconnectHierarchy.taper_eq_math
# └─────────────────────────────────────────────────────────────────────────────
...
class InterconnectHierarchy:
    ...
    check(hbm_bw > nvlink_h100 > pcie_gen4 > ib_hdr_bw,
          "Bandwidth hierarchy violated. HBM > NVLink > PCIe > Network")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_a100_gb_s_str = fmt_bandwidth(nvlink_a100, unit=GB / second, precision=0, commas=False)
    nvlink_h100_gb_s_str = fmt_bandwidth(nvlink_h100, unit=GB / second, precision=0, commas=False)
    ib_hdr_gbps_str = fmt_bandwidth(ib_hdr_bw, unit=Gbps, precision=0, commas=False)
    ...
    check(taper_hbm > taper_nvlink > taper_pcie > taper_net,
...
1. **Device-Device Interconnect (NVLink/Infinity Fabric)**[^fn-nvlink-bandwidth]\index{NVLink!GPU interconnect}\index{GPU Interconnect!switching fabric}\index{Infinity Fabric!AMD interconnect}: Modern multi-GPU nodes use specialized high-speed bridges like NVLink to connect accelerators directly, bypassing the host CPU. Bandwidth ranges from `{python} InterconnectHierarchy.nvlink_a100_gb_s_str` to `{python} InterconnectHierarchy.nvlink_h100_gb_s_str` per GPU. The primary use case is gradient synchronization (AllReduce)\index{AllReduce!gradient synchronization}[^fn-allreduce-gradient-sync] during distributed training. This bandwidth is critical for scaling; without it, multi-GPU training often scales poorly.
...
class RooflineGap:
    ...
    check(300 <= a100_fp16_flops.to(TFLOP/second).magnitude <= 320,
          f"A100 FP16 peak should be near 312 TFLOP/s, got {a100_fp16_flops.to(TFLOP/second).magnitude:.0f}.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_tflops_str = fmt_flop_rate(v100_flops, unit=TFLOP / second, precision=0, commas=False)
    v100_bw_tbs_str = fmt_bandwidth(v100_bandwidth, unit=TB / second, precision=1, commas=False)
    a100_tflops_fp16_str = fmt_flop_rate(a100_fp16_flops, unit=TFLOP / second, precision=0, commas=False)
    a100_bw_tbs_str = fmt_bandwidth(a100_bandwidth, unit=TB / second, precision=2, commas=False)
    h100_tflops_fp16_str = fmt_flop_rate(h100_fp16_flops, unit=TFLOP / second, precision=0, commas=False)
    h100_bw_tbs_str = fmt_bandwidth(h100_bandwidth, unit=TB / second, precision=2, commas=False)
    ...
    bandwidth_ratio_str = fmt_multiple(bw_growth, precision=1, commas=False)
    flops_ratio_str = fmt_multiple(flops_growth, precision=1, commas=False)
    relu_below_roofline_str = fmt_multiple(relu_gap, precision=1, commas=True)
...
class Conv2dAnalysisCalc:
    ...
    conv_total_gflops_str  = fmt_flops(conv_total_flops, unit=GFLOPs, precision=1, commas=False)
...
$\text{AI}$ = `{python} Conv2dAnalysisCalc.conv_total_gflops_str` / `{python} Conv2dAnalysisCalc.conv_total_mb_str` = `{python} Conv2dAnalysisCalc.conv_ai_str` FLOP/byte
...
# │          DenseLayerAnalysisCalc.dense_attainable_tflops_str,
# │          DenseLayerAnalysisCalc.dense_util_pct_str,
# │          DenseLayerAnalysisCalc.a100_bw_gb_s_str,
...
class DenseLayerAnalysisCalc:
    ...
    dense_attainable_tflops_str   = fmt_flop_rate(dense_attainable, unit=TFLOP / second, precision=1, commas=False)
    dense_util_pct_str     = fmt_percent(dense_util_pct/100, precision=1, commas=False, style='symbol')
    a100_bw_gb_s_str            = fmt_bandwidth(a100_bandwidth, unit=GB / second, precision=0, commas=True)
    a100_ridge_str         = fmt_int(round(a100_ridge_value), commas=False)
```
...
$R_{\text{attainable}}$ = `{python} DenseLayerAnalysisCalc.a100_bw_gb_s_str` $\times$ `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte = `{python} DenseLayerAnalysisCalc.dense_attainable_tflops_str`
...
# │          LayernormAnalysisCalc.ln_attainable_tflops_str,
# │          LayernormAnalysisCalc.a100_bw_gb_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, check
...
class LayernormAnalysisCalc:
    ...
    ln_ridge_gap_str    = fmt_multiple(round(ln_ridge_gap), commas=False, precision=0)
    ln_attainable_tflops_str   = fmt_flop_rate(ln_attainable, unit=TFLOP / second, precision=1, commas=False)
    a100_bw_gb_s_str         = fmt_bandwidth(a100_bandwidth, unit=GB / second, precision=0, commas=False)
...
$R_{\text{attainable}}$ = `{python} LayernormAnalysisCalc.a100_bw_gb_s_str` $\times$ `{python} LayernormAnalysisCalc.ln_ai_str` FLOP/byte = `{python} LayernormAnalysisCalc.ln_attainable_tflops_str`
...
class Gpt2ThroughputCalc:
    ...
    gpt2_decode_gflops_str = fmt_flops(gpt2_decode_flops, unit=GFLOPs, precision=0, commas=False)
    gpt2_decode_ai_str    = fmt(gpt2_decode_ai, precision=0, commas=False)
    gpt2_max_tflops_str   = fmt_flop_rate(gpt2_max_flops, unit=TFLOP / second, precision=1, commas=False)
    gpt2_utilization_pct_str = fmt_percent(gpt2_utilization / 100, precision=1, commas=False, style='symbol')
    a100_tflops_fp16_str  = fmt_flop_rate(a100_fp16_flops, unit=TFLOP / second, precision=0, commas=False)
    a100_bw_tbs_str       = fmt_bandwidth(a100_bandwidth, unit=TB / second, precision=2, commas=False)
    a100_tflops_fp32_str  = fmt_flop_rate(a100_fp32_flops, unit=TFLOP / second, precision=1, commas=False)
    a100_ridge_str        = fmt_int(round(a100_ridge_value), commas=False)
...
*   **Utilization ceiling**:
    `{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` (Actual) / `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` (Peak) ≈ `{python} Gpt2ThroughputCalc.gpt2_utilization_pct_str`
...
class CompilerSpeedupCalc:
    ...
    compiler_speedup_str = fmt_multiple(compiler_speedup, precision=1, commas=False)
```
...
Overlapping computation with memory movement mitigates the bottleneck that has governed the chapter: data movement between memory hierarchy levels limits computation speed. AI runtimes implement asynchronous execution and double buffering so computations proceed without waiting for memory transfers to complete. In a large-scale model, image data can be prefetched while computations run on the previous batch, maintaining a steady flow of data and avoiding pipeline stalls.
...
class Gpt3SingleH100Calc:
    ...
    # OUTPUT
    h100_pflops_fp8_str = fmt_flop_rate(Hardware.Cloud.H100.compute.precision_flops["fp8"], unit=PFLOPs/second, precision=2, commas=False)
...
Modern AI systems scale beyond individual accelerators by moving the communication boundary outward, and each boundary changes the dominant trade-off.

\index{Chiplet!modular die interconnect}
Chiplet-based architectures keep the boundary inside one package. They partition large designs into smaller, modular dies interconnected within the package, bypassing manufacturing limits of monolithic chips while preserving relatively low communication latency.

Multi-accelerator nodes move the boundary to board- or server-level interconnects. Each accelerator has dedicated memory and compute resources, so workloads split through data parallelism\index{Data Parallelism!multi-accelerator scaling} (each accelerator processes different batches) or model parallelism\index{Model Parallelism!multi-accelerator scaling} (different accelerators handle different network layers). High-bandwidth intra-node interconnects can enable efficient gradient synchronization, though realized performance depends on topology and collective communication efficiency.

At data center scale, the boundary moves to purpose-built interconnect fabrics that coordinate hundreds of accelerators. Cluster topology and collective communication algorithms become central determinants of scaling efficiency, and near-linear scaling is achievable on some workloads when communication overhead is controlled.

Wafer-scale integration attacks the boundary by reducing or eliminating it inside a unified compute fabric. Platforms such as Cerebras WSE-class systems integrate extremely large numbers of transistors and cores on a single device, reducing inter-chip communication overhead. This approach introduces its own challenges in thermal dissipation, fault tolerance, and manufacturing yield, representing the frontier of single-system compute density.
...
No single processor architecture can optimally handle the diverse computational patterns in modern AI applications, so heterogeneous acceleration becomes a coordination problem rather than a hardware inventory. Efficient mobile AI systems deliver high performance only when processor assignment, memory coherence, thermal limits, and latency constraints are managed together.
...
class FpWorkloadFitRecap:
    ...
    # GUARD
    check(150 * (flop / byte) <= a100_ridge <= 160 * (flop / byte),
          f"Unexpected A100 ridge: {a100_ridge.to(flop / byte).magnitude:.1f}")
    check(4 * (TFLOP / second) <= softmax_flops_low <= 5 * (TFLOP / second),
          f"Unexpected softmax low throughput: {softmax_flops_low.to(TFLOP / second).magnitude:.1f} TFLOP/s")
    check(10 * (TFLOP / second) <= softmax_flops_high <= 11 * (TFLOP / second),
    ...
    softmax_tflops_low_str = fmt_flop_rate(softmax_flops_low, unit=TFLOP / second, precision=1, commas=False)
    softmax_tflops_high_str = fmt_flop_rate(softmax_flops_high, unit=TFLOP / second, precision=1, commas=False)
...
Engineers assume specialized accelerators automatically outperform general-purpose processors for all AI workloads. In reality, specialized hardware achieves peak performance only when workloads match architectural assumptions. As demonstrated in @sec-hardware-acceleration-roofline-model-42ff, operations must exceed the accelerator's ridge point to be compute bound; an A100 GPU has a ridge point of `{python} FpWorkloadFitRecap.a100_ridge_str`, meaning operations with arithmetic intensity below this threshold are memory bound regardless of the accelerator's `{python} FpWorkloadFitRecap.a100_tflops_fp16_str` peak compute. A transformer attention softmax with AI = `{python} FpWorkloadFitRecap.softmax_ai_low_str`--`{python} FpWorkloadFitRecap.softmax_ai_high_str` achieves only `{python} FpWorkloadFitRecap.softmax_tflops_low_str`--`{python} FpWorkloadFitRecap.softmax_tflops_high_str` (`{python} FpWorkloadFitRecap.softmax_util_high_str` utilization) on an A100. CPUs with ridge points around `{python} FpWorkloadFitRecap.cpu_ridge_low_str`--`{python} FpWorkloadFitRecap.cpu_ridge_high_str` still treat this kernel as memory-bound, but the same AI range corresponds to about `{python} FpWorkloadFitRecap.cpu_softmax_util_low_str`--`{python} FpWorkloadFitRecap.cpu_softmax_util_high_str` of a CPU's lower peak. Models with irregular memory access, small batch sizes, or dynamic computation graphs may perform better on flexible processors. Effective hardware selection requires matching workload arithmetic intensity to architectural ridge points, not assuming specialization always wins.
...
class FpMemoryEnergyCalc:
    ...
    fp_layernorm_tflops_str = fmt_flop_rate(layernorm_flops, unit=TFLOP / second, precision=0, commas=False)
    ...
    fp_peak_tflops_str      = fmt_flop_rate(peak_flops, unit=TFLOP / second, precision=0, commas=False)
    fp_peak_bw_tbs_str      = fmt_bandwidth(peak_bw, unit=TB / second, precision=0, commas=False)
...
# │          mlsysim.book (fmt)
# │ Exports: FpMultigpuScalingCalc.fp_nvlink_bw_gb_s_str,
...
class FpMultigpuScalingCalc:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    sync_time        = (gradient_size / nvlink_bw).to(millisecond)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fp_nvlink_bw_gb_s_str     = fmt_bandwidth(nvlink_bw, unit=GB / second, precision=0, commas=False)
    ...
    fp_no_overlap_speedup_str = fmt_multiple(no_overlap_speedup, precision=1, commas=False)
...
class FpPeakVsSustainedRecap:
    ...
    # GUARD
    check(300 * (TFLOP / second) <= a100_peak_flops <= 320 * (TFLOP / second),
    ...
    # OUTPUT
    a100_tflops_fp16_str = fmt_flop_rate(a100_peak_flops, unit=TFLOP / second, precision=0, commas=False)
    transformer_tflops_low_str = fmt_flop_rate(transformer_flops_low, unit=TFLOP / second, precision=0, commas=False)
    transformer_tflops_high_str = fmt_flop_rate(transformer_flops_high, unit=TFLOP / second, precision=0, commas=False)
    ...
    recsys_tflops_low_str = fmt_flop_rate(recsys_flops_low, unit=TFLOP / second, precision=0, commas=False)
    recsys_tflops_high_str = fmt_flop_rate(recsys_flops_high, unit=TFLOP / second, precision=0, commas=False)
...
# │          FpSmallBatchCalc.fp_t4_flops_tflops_str,
# │          FpSmallBatchCalc.fp_t4_bw_gb_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int,  fmt
...
class FpSmallBatchCalc:
    ...
    fp_t4_flops_tflops_str = fmt_flop_rate(t4_flops, unit=TFLOP / second, precision=0, commas=False)
    fp_t4_bw_gb_s_str    = fmt_bandwidth(t4_bw, unit=GB / second, precision=0, commas=False)
    fp_a100_roof_b1_tflops_str = fmt_flop_rate(a100_roof_b1, unit=TFLOP / second, precision=1, commas=False)
    fp_t4_roof_b1_tflops_str = fmt_flop_rate(t4_roof_b1, unit=TFLOP / second, precision=1, commas=False)
```

**Pitfall**: *Deploying small-batch inference workloads on high-compute accelerators.*

Teams deploy high-throughput training accelerators (A100, H100) for latency-sensitive inference with batch size 1–4. As the Roofline Model (@sec-hardware-acceleration-roofline-model-42ff) predicts, small batches severely reduce arithmetic intensity: a dense layer with M=N=2048 achieves AI = `{python} FpSmallBatchCalc.fp_ai_b1_str` FLOP/byte at batch=1 vs. AI = `{python} FpSmallBatchCalc.fp_ai_b256_str` FLOP/byte at batch=256. At batch=1, the memory-bound roofline ceiling is only about `{python} FpSmallBatchCalc.fp_a100_roof_b1_tflops_str` on A100 and `{python} FpSmallBatchCalc.fp_t4_roof_b1_tflops_str` on T4. The T4's peak is `{python} FpSmallBatchCalc.fp_t4_flops_tflops_str` (FP16 Tensor Core) with a ridge point of `{python} FpSmallBatchCalc.fp_t4_ridge_str` FLOP/byte (`{python} FpSmallBatchCalc.fp_t4_flops_tflops_str` / `{python} FpSmallBatchCalc.fp_t4_bw_gb_s_str`). Small-batch inference remains memory bound on both accelerators, so the T4's lower cost can make it more economical despite its much lower peak compute. Training-class accelerator instances often rent for several times more than inference-class instances for little latency gain in this regime. Inference deployments should match batch size to accelerator characteristics, using high-compute accelerators only for batched serving where arithmetic intensity exceeds ridge points.
...
class FeasibilityAssessment:
    ...
    mem_bw_tb_s_str = fmt_bandwidth(mem_bw, unit=TB / second, precision=0, commas=False)
...
*   **Example**: Processing video at 30 FPS requires completing all inference within `{python} FeasibilityAssessment.frame_budget_ms_str`.
...
class CarbonRoiCalc:
    ...
    check(co2_saved_year < cpu_co2_year,
    ...
    cpu_tflops_str     = fmt_flop_rate(cpu_flops, unit=TFLOP / second, precision=0, commas=True)
    npu_tflops_str     = fmt_flop_rate(npu_flops, unit=TFLOP / second, precision=0, commas=True)
    cpu_eff_str        = fmt_compute_efficiency(cpu_eff, unit=TFLOP / second / watt, precision=2)
    npu_eff_str        = fmt_compute_efficiency(npu_eff, unit=TFLOP / second / watt, precision=0)
    eff_gap_str        = fmt_multiple(eff_gap, precision=0, commas=True)
...
*   **CPU inference**: `{python} CarbonRoiCalc.cpu_power_w_str` for `{python} CarbonRoiCalc.cpu_tflops_str` (efficiency = `{python} CarbonRoiCalc.cpu_eff_str`).
*   **NPU inference**: `{python} CarbonRoiCalc.npu_power_w_str` for `{python} CarbonRoiCalc.npu_tflops_str` (efficiency = `{python} CarbonRoiCalc.npu_eff_str`).
...
class SummaryRooflineRecap:
    ...
    # GUARD
    check(150 * (flop / byte) <= a100_ridge <= 160 * (flop / byte),
          f"Unexpected A100 summary ridge: {a100_ridge.to(flop / byte).magnitude:.1f}")
