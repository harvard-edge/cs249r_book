...
2.  **Distinction (Durable):** Unlike layered abstraction (where software calls a hardware API without knowing the silicon details), co-design exposes hardware constraints — data alignment requirements, precision formats, memory access patterns — directly to algorithm and compiler authors so that global cross-layer optimizations are possible.
...
This hierarchy invites an apparently simple solution: build larger, faster off-chip memory and eliminate the need for on-chip SRAM entirely. The answer is rooted in physics, specifically the *speed of light limit* on signal propagation within and between chips.
