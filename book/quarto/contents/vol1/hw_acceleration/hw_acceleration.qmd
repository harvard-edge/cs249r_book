...
The first-order quantity is the gradient payload. A model with $P$ parameters and $b$ bytes per gradient exposes roughly $P \cdot b$ bytes of state to synchronize each step before optimizer state, padding, and protocol overhead enter. Scaling therefore improves only when the saved compute time exceeds the time to move this payload through the chosen interconnect and collective algorithm.
