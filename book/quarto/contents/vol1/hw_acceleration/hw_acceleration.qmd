...
Hardware acceleration does not speed up the entire system; it *only speeds up the parallelizable fraction ($p$)*. This is governed by **Amdahl's Law**\index{Amdahl's Law!parallel fraction}\index{Parallel Computing!Amdahl's Law} [@amdahl1967], formalized in @eq-amdahl:
...
Special Function Units (SFUs)\index{Special Function Unit!SFU} provide dedicated hardware for these non-linear computations, completing the trio of core processing primitives. The need for such units is not new—floating-point co-processors in the 1970s-1980s and SSE/NEON instruction set extensions in the 1990s addressed similar demands for scientific computing [@broesch1997; @palmer1980]. Neural networks have intensified this demand because activation functions, normalization layers, and softmax transformations appear after every linear layer, making them a throughput bottleneck rather than an occasional convenience.
