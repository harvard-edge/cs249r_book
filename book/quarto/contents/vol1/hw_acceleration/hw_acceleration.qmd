...
A recurring pitfall is this: *Serial work caps total accelerator speedup.* If data loading takes 10 percent of the time ($p=0.9$), even an *infinite speed* accelerator ($G_{\text{accel}}=\infty$) can only achieve a 10$\times$ total speedup. The serial component dominates the parallel accelerator component once the latter is sufficiently fast.
...
A common fallacy is this: *More specialized hardware always provides better performance than general-purpose alternatives.*
...
A recurring pitfall is this: *Ignoring memory bandwidth limitations when selecting acceleration strategies.*
...
A common fallacy is this: *Hardware acceleration benefits scale linearly with additional accelerators.*
...
A recurring pitfall is this: *Planning accelerator capacity from peak FLOP/s specifications.*
...
A common fallacy is this: *Any FLOP/s rating can estimate a low-precision workload.*
...
A recurring pitfall is this: *Deploying small-batch inference workloads on high-compute accelerators.*
...
A common fallacy is this: *Vendor-specific optimizations have no long-term portability cost.*
...
A recurring pitfall is this: *Embedding vendor-specific kernels directly into application logic.*
