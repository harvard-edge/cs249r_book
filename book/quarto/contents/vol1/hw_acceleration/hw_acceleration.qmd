...
[^fn-cray-vector]: **Cray-1 Vector Legacy**: The Cray-1 (1975) cost $8.8 million (approximately $40-45 million in 2024 dollars) but could perform 160 million floating-point operations per second, 1000 $\times$ faster than typical computers. Its 64-element vector registers and pipelined vector units established the architectural template that modern AI accelerators still follow: process many data elements simultaneously with specialized hardware pipelines.
...
[^fn-hwacc-tensor-cores]: **Tensor Core**\index{V100!first tensor core GPU}: The term "tensor" derives from Latin "tendere" (to stretch), originally describing mathematical objects that transform under coordinate changes. In ML, tensors generalize matrices to arbitrary dimensions. NVIDIA introduced tensor cores in the V100 (2017) to accelerate the small matrix operations (4 $\times$ 4 $\times$ 4 tiles) common in neural networks. On supported kernels and reduced-precision modes, tensor cores deliver large speedups over conventional GPU execution, with modern accelerators reaching hundreds of TFLOPS for reduced-precision dense kernels.
...
- Concretely, a 128x128 array can achieve over `{python} systolic_macs_cycle_str` MACs per cycle with `{python} accelerator_energy_dividend_str` better energy efficiency than a standard vector unit by pulsing data through processing elements instead of repeatedly loading it from DRAM.
