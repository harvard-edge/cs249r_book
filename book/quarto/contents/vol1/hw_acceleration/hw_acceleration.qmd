...
*   **Use Case**: Gradient synchronization (AllReduce) during distributed training. This bandwidth is critical for scaling; without it, multi-GPU training often scales poorly.
...
::: {.callout-chapter-connection title="From Understanding to Measurement"}

This chapter has equipped you to analyze hardware architectures and predict performance characteristics. But analysis must be validated through measurement. Consider: if you've quantized a model from FP32 to INT8 (following @sec-model-compression) and deployed it on a GPU with Tensor Core support, how do you verify the expected 2-4× speedup actually materializes? How do you isolate whether a performance gap stems from memory bandwidth, kernel inefficiency, or suboptimal batch sizing?

@sec-benchmarking-ai provides the measurement discipline that transforms hardware understanding into validated engineering decisions:

- **Metrics selection**: Latency vs. throughput, TFLOPS vs. TOPS, energy efficiency—which metrics matter for your deployment scenario?
- **Fair comparison**: How to evaluate accelerator options while controlling for confounding factors like different compiler stacks or memory configurations
- **Bottleneck identification**: Using profiling tools to determine whether your workload hits the compute or memory roof
- **Cost-performance analysis**: Translating hardware characteristics into dollars-per-inference or time-to-train metrics that drive procurement decisions

The roofline analysis techniques introduced here will inform how you interpret benchmark results; the memory hierarchy understanding will help you diagnose unexpected performance gaps; and the dataflow concepts will guide optimization priorities when benchmarks reveal underutilization.

We have built the factory (the model) and tuned the engine (the hardware). Now, we must take it to the track. How do we know if our optimizations actually worked? In **Benchmarking** (@sec-benchmarking-ai), we move from theoretical peaks to empirical truth, learning to measure the speed, accuracy, and efficiency of our systems in the wild.
