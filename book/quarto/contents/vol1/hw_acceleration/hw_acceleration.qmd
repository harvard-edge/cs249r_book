...
[^fn-gflops]: **GFLOPS/TOPS Performance Metrics**: GFLOPS (10⁹ floating-point ops/second) measures floating-point throughput; TOPS (10¹² ops/second) typically measures INT8 operations in AI accelerators. The A100 delivers `{python} a100_tflops_fp16` TFLOPS for FP16/BF16 dense Tensor Core operations (`{python} a100_tflops_tf32` TFLOPS for TF32), with approximately 2× higher throughput when structured sparsity is enabled; Apple A17 NPU achieves `{python} mobile_tops` INT8 TOPS. Real workload performance depends on memory bandwidth, achieving 10–30% of peak on typical ML models.
...
3. **Tolerance for Reduced Precision**\index{Quantization!reduced precision tolerance}: Neural networks typically remain robust even when using 8-bit or 4-bit integers instead of 64-bit floating-point numbers. This flexibility allows architects to fit 10× more compute units in the same silicon area.
...
```{.c}
mload mr1, (weight_ptr)     # Load e.g., 16 × 16 block of
                            # weight matrix
...
# │
# │ Goal: Compare the economics of accelerator generations.
# │ Show: The collapse of cost-per-TFLOP alongside the rise in power density.
# │ How: Contrast V100, A100, and H100 pricing and performance metrics.
# │
...
# --- Inputs (hardware specs from Hardware Digital Twin) ---
...
# V100
v100_tf = h_v100.peak_flops.to(TFLOPs/second).magnitude
v100_ratio = price_v100 / v100_tf

# A100
a100_tf = h_a100.peak_flops.to(TFLOPs/second).magnitude
a100_ratio = price_a100 / a100_tf

# H100 (using TF32 for price ratio, FP16 for spec)
h100_tf = h_h100.tf32_flops.to(TFLOPs/second).magnitude
h100_ratio = price_h100 / h100_tf

# TPUv4
tpu_tf = h_tpu.peak_flops.to(TFLOPs/second).magnitude
...
# V100 specs
v100_tflops = f"{h_v100.peak_flops.to(TFLOPs/second).magnitude:.0f}"     # e.g. "125"
v100_bw = f"{h_v100.memory_bw.to(GB/second).magnitude:.0f}"                         # e.g. "900"
...
a100_tflops_fp16 = f"{h_a100.peak_flops.to(TFLOPs/second).magnitude:.0f}" # e.g. "312"
a100_bw = f"{h_a100.memory_bw.to(GB/second).magnitude:,.0f}"                        # e.g. "2,039"
a100_bw_tbs = f"{h_a100.memory_bw.to(TB/second).magnitude:.1f}"                     # e.g. "2.0"
a100_price_str = f"~${price_a100:,}"                                           # e.g. "~$15,000"
a100_pp_str = f"${a100_ratio:.0f}/TFLOP"                                       # e.g. "$48/TFLOP"

# H100 specs
h100_tflops_tf32 = f"{h_h100.tf32_flops.to(TFLOPs/second).magnitude:.0f}"        # e.g. "495"
h100_tflops_fp16 = f"{h_h100.peak_flops.to(TFLOPs/second).magnitude:.0f}" # e.g. "990"
h100_bw = f"{h_h100.memory_bw.to(GB/second).magnitude:,.0f}"                        # e.g. "3,350"
h100_bw_tbs = f"{h_h100.memory_bw.to(TB/second).magnitude:.2f}"                     # e.g. "3.35"
h100_price_str = f"~${price_h100:,}-30,000"                                    # e.g. "~$25,000-30,000"
h100_pp_str = f"~${h100_ratio:.0f}/TFLOP"                                      # e.g. "~$51/TFLOP"

# TPUv4 specs
tpuv4_tflops = f"{h_tpu.peak_flops.to(TFLOPs/second).magnitude:.0f}"           # e.g. "275"
tpuv4_bw = f"{h_tpu.memory_bw.to(GB/second).magnitude:,.0f}"                      # e.g. "1,200"
...
System efficiency depends on keeping data as high up this hierarchy as possible. Once data drops to PCIe or Network speeds, it encounters a 30–100× slowdown.
...
# │
# │ Goal: Demonstrate why newer accelerators are increasingly difficult to saturate.
# │ Show: How the rising ridge point pushes workloads into the memory-bound regime.
# │ How: Calculate ridge points for V100, A100, and H100 architectures.
# │
...
class RooflineGap:
    ...
    v100_ridge = h_v100.ridge_point().magnitude
    a100_ridge = h_a100.ridge_point().magnitude
    h100_ridge = h_h100.ridge_point().magnitude

    # FP32 Ridge for A100
    a100_ridge_fp32 = (h_a100.peak_flops_fp32 / h_a100.memory_bw).to('flop/byte').magnitude
...
## weights: [512 × 256] - model parameters
## input:   [256 × 32]  - batch of activations
## Z:       [512 × 32]  - output activations
...
tensor_dim = 1024                        # 1024 × 1024 tensor for worked example
...
2. **Kernel selection** chooses Tensor Core implementations for the 3 × 3 convolutions, exploiting the high arithmetic intensity (50-200 FLOP/byte) we calculated in the Roofline analysis
...
Teams deploy high-throughput training accelerators (A100, H100) for latency-sensitive inference with batch size 1–4. As the Roofline Model (@sec-hardware-acceleration-roofline-model-42ff) predicts, small batches severely reduce arithmetic intensity: a dense layer with M=N=2048 achieves AI = `{python} fp_ai_b1_str` FLOP/byte at batch=1 versus AI = `{python} fp_ai_b256_str` FLOP/byte at batch=256. At batch=1, an A100 achieves 4 TFLOPS (1.3% utilization) due to memory bottlenecks, while a lower-cost T4 achieves 3.5 TFLOPS. The T4's peak is `{python} fp_t4_flops_str` TFLOPS (FP16 Tensor Core) with a ridge point of `{python} fp_t4_ridge_str` FLOP/byte (`{python} fp_t4_flops_str` TFLOPS / `{python} fp_t4_bw_str` GB/s). Small-batch inference remains memory-bound on both accelerators, but the T4's lower cost makes it more economical. A100 instances cost 3–4× more than T4 instances for identical latency. Inference deployments should match batch size to accelerator characteristics, using high-compute accelerators only for batched serving where arithmetic intensity exceeds ridge points.
...
# │
# │ Goal: Provide worked examples for hardware feasibility assessment.
# │ Show: How to verify memory, bandwidth, and compute constraints before deployment.
# │ How: Calculate model fit, latency targets, and throughput for a representative model.
# │
