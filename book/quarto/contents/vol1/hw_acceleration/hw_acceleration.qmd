...
This historical progression reveals a crucial insight: each wave of hardware specialization responded to a specific computational bottleneck. Floating-point coprocessors addressed arithmetic precision limitations. GPUs addressed graphics throughput limitations. But what bottleneck does AI acceleration address? Understanding this question is essential because it reveals _why_ modern accelerators are designed the way they are—and why simply adding more transistors to general-purpose processors cannot solve the fundamental challenge.
...
::: {.callout-lighthouse title="Roofline Analysis: ResNet-50 vs. GPT-2"}

These two widely-deployed models illustrate how different architectures interact with hardware constraints:

**ResNet-50 (Image Classification)**

- Dominated by 3×3 convolutions with high weight reuse
- Typical arithmetic intensity: 50-200 FLOP/byte (depending on layer)
- Classification: **Compute-bound** on modern GPUs
- Hardware implication: Benefits from Tensor Cores and high TFLOPS; memory bandwidth is rarely the bottleneck
- Achieved utilization: 70-85% of peak on A100 for inference

**GPT-2 (Language Generation)**

- Mix of dense projections (high AI) and attention operations (low AI)
- Dense layers: AI ≈ 100-200 FLOP/byte (compute-bound)
- Attention softmax: AI ≈ 2-5 FLOP/byte (severely memory-bound)
- LayerNorm: AI ≈ 1-2 FLOP/byte (severely memory-bound)
- Classification: **Mixed**—projection layers are compute-bound, attention and normalization are memory-bound
- Hardware implication: End-to-end performance limited by memory-bound operations; adding more TFLOPS helps less than expected
- Achieved utilization: 40-60% of peak on A100 for training

**The Engineering Lesson**: ResNet-50's consistent compute-bound behavior means hardware selection is straightforward—buy more TFLOPS. GPT-2's heterogeneous profile means that memory bandwidth and specialized attention optimizations (like FlashAttention) matter more than raw compute. This is why transformer optimization has become a specialized discipline.

:::

## Hardware Mapping Fundamentals for Neural Networks {#sec-ai-acceleration-hardware-mapping-fundamentals-neural-networks-f9a9}

The Roofline analysis in the previous section taught us to diagnose whether specific operations are compute-bound or memory-bound on given hardware. We saw that ResNet-50's convolutions achieve high arithmetic intensity (50-200 FLOP/byte) and operate in the compute-bound regime, while GPT-2's attention layers achieve only 2-5 FLOP/byte and are severely memory-bound. But diagnosis is only half the challenge. Once we know that LayerNorm achieves just 1-2 FLOP/byte on an A100, the question becomes: how do we execute it efficiently despite this limitation? This is the domain of hardware mapping, the art of translating abstract computational graphs into concrete execution plans that exploit accelerator architectures while respecting their constraints.

The memory system challenges examined in @sec-ai-acceleration-understanding-ai-memory-wall-3ea9 established *why* memory access dominates modern AI systems: DRAM access consumes 100-200x more energy than a multiply-accumulate operation. The Roofline model established *how to measure* whether a workload is compute-bound or memory-bound. This section addresses the critical follow-up: *how to map* computations to maximize data reuse and minimize the energy-intensive transfers that the Roofline analysis revealed as the primary bottleneck. A systolic array with 1,200 GB/s on-chip bandwidth and sophisticated memory hierarchies delivers no performance benefit if computations are mapped without considering these memory access patterns.
...
The mapping search space analysis revealed a daunting combinatorial explosion: billions of possible configurations for even simple neural network layers. How do practitioners routinely achieve near-optimal performance despite this vast space? The answer lies in a key insight: while the full configuration space is intractable, a small set of principled dataflow patterns capture the essential trade-offs.
...
Each of these building blocks structures high-performance execution, forming the basis for both heuristic and model-driven optimization techniques. The next section explores how these strategies are adapted to different types of AI models.
