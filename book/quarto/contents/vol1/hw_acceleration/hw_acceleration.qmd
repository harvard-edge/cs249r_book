...
[^fn-tpu-origin]: **TPU (Tensor Processing Unit)**: Google developed the TPU when projections showed that serving voice search on general-purpose CPUs would require doubling its datacenter footprint. The design made a radical trade-off, stripping away non-essential logic like caches and branch predictors to dedicate silicon to a massive 256$\times$256 systolic array for matrix multiplication. This specialization delivered an immediate 15--30$\times$ improvement in throughput-per-watt over contemporary GPUs, validating the domain-specific approach. \index{TPU!origin}
...
[^fn-moores-law-scaling]: **Moore's Law**: Gordon Moore's 1965 observation that transistor density doubles every 18–24 months, which for decades delivered predictable and exponential performance gains for general-purpose computing. This economic scaling has ceased, as the capital cost to build a new fabrication facility now exceeds \$20 billion, making density improvements prohibitively expensive. The direct consequence is a performance gap, as ML model compute requirements grow 4–10× faster than the remaining gains from transistor density, necessitating the shift to domain-specific architectures. \index{Moore's Law!ML systems consequence}

[^fn-dennard-scaling-power]: **Dennard Scaling**: The 1974 principle that as transistor dimensions shrank, their operating voltage could be lowered to keep power density constant. Its breakdown after ~2005 meant that clock speeds could no longer be increased without violating the chip's thermal design power (TDP) limits. This "power wall" created the efficiency bottleneck that forced the architectural shift from frequency scaling to the massively parallel, domain-specific designs mentioned. \index{Dennard Scaling!power constraint}

[^fn-hennessy-patterson-dsa]: **Domain-Specific Architecture (DSA)**: Hennessy and Patterson's central argument is that with the end of Dennard scaling, energy efficiency—not transistor count—became the primary bottleneck for general-purpose CPUs. Continued performance gains thus require abandoning the one-size-fits-all model and co-designing hardware for a specific application domain, trading generality for massive efficiency. Their rule of thumb is that a new architecture must be at least 10x faster to be commercially viable, a threshold that ML workloads consistently met. \index{Hennessy, John!domain-specific architecture thesis}\index{Patterson, David!domain-specific architecture thesis}
...
[^fn-gflops-tops-gap]: **GFLOPS (Billions of Floating-point Operations Per Second)**: This inefficiency stems directly from a CPU's architectural mismatch for neural network workloads, which require massive parallelism rather than the single-thread latency CPUs are optimized for. A CPU's arithmetic units are starved for data because its cache hierarchies, designed for kilobyte-scale tasks, cannot service the gigabyte-scale weight transfers required by a single neural network layer. This creates the severe peak-to-realized performance gap, where a processor theoretically capable of over 1,000 GFLOPS delivers only the `{python} cpu_gflops_str` GFLOPS stated. \index{GFLOPS!peak vs. realized}

[^fn-memory-bw-cpu-hbm]: **Memory Bandwidth**: This inefficiency arises because CPU memory channels are architected for small, irregular data accesses that fit in on-chip caches, not the gigabyte-scale parameter streams of a neural network layer. When a model's parameters overwhelm the cache, the CPU is forced to fetch data from off-chip DRAM, which provides less than 1/10th the data transfer rate of on-chip resources. This stall, not a lack of arithmetic units, is what throttles the processor's computational throughput. \index{Memory Bandwidth!CPU-HBM gap}

[^fn-cache-nn-mismatch]: **Cache Hierarchy**: CPU caches are designed for small, kilobyte-scale working sets with high data reuse, an architecture optimized for general-purpose code. Neural networks violate this principle by streaming gigabytes of parameters for a single layer, ensuring data is evicted from the cache before it can be reused. This constant cache "trashing" forces the CPU to stall while waiting for data from main memory, reducing effective performance by over 95% and creating the core bandwidth bottleneck. \index{Cache Hierarchy!neural network mismatch}
...
[^fn-backprop-memory-cost]: **Backpropagation Memory Cost**: To compute gradients via the chain rule, the backward pass must access all intermediate activations created during the forward pass. This forces training accelerators to hold these activations in memory, whereas an inference-only pass can discard them. This requirement directly makes memory capacity, not just compute, a primary design constraint, increasing memory needs by 2--3$\times$ over inference and driving the specialized architectures mentioned. \index{Backpropagation!memory cost}
...
[^fn-riscv-ai-customization]: **RISC-V (Reduced Instruction Set Computer V)**: This parallel execution is enabled by the Vector "V" extension, which adds Single Instruction, Multiple Data (SIMD) capabilities to the instruction set. A single vector instruction replaces a conventional software loop, avoiding the repeated fetch-decode-execute overhead required by a scalar processor. For the 8-element operation shown, this reduces the loop control overhead by a factor of 8x. \index{RISC-V!AI customization}
...
[^fn-simd-simt-parallelism]: **SIMD (Single Instruction, Multiple Data)**: SIMD executes one instruction over a fixed-width vector register, whereas SIMT (Single Instruction, Multiple Thread) assigns an independent, lightweight thread to each data element. This architectural distinction directly explains the performance progression on the 512-element ReLU vector: a CPU's 512-bit SIMD unit must execute 32 sequential instructions (512 elements / 16 floats per instruction). A GPU's SIMT warp scheduler, however, can issue all 512 threads for concurrent execution in a single wave. \index{SIMD!SIMT distinction for ML}
...
[^fn-dally-gpu-precision]: **William Dally and Reduced-Precision ML**: Dally's research demonstrated that the massive SIMT parallelism used for graphics was inefficient for neural networks that are resilient to lower precision. This insight validated an architectural trade-off where numerical precision is exchanged for higher throughput, a pivotal moment for ML-specific hardware. By proving networks could maintain accuracy with 8-bit integers (INT8), his work enabled GPUs to pack 4$\times$ more operations into the same silicon area compared to 32-bit floating point (FP32), directly boosting computational efficiency. \index{Dally, William!reduced-precision ML}
...
[^fn-sparsity-nm-regularity]: **N:M Structured Sparsity**: Unstructured pruning fails to yield speedups because its random, scattered memory accesses are hostile to the parallel fetch mechanisms in hardware accelerators. The N:M constraint restores this regularity, creating a predictable data pattern that hardware like NVIDIA's 2:4 Sparse Tensor Cores can exploit to deliver up to a 2x throughput gain over dense operations. \index{Sparsity!N:M regularity constraint}
...
[^fn-systolic-array-dataflow]: **Systolic Array**: The name reflects the rhythmic "pulse" of data through the processing grid, which directly enables the spatial and temporal operand reuse mentioned in the text. This rigid data path is the architecture's core trade-off, excelling at the dense matrix multiplication described but proving inflexible for other workloads. By streaming operands this way, a single weight is reused for all 128 MAC operations in a TPUv4 array column, eliminating hundreds of individual register file or memory accesses that a general-purpose core would require. \index{Systolic Array!dataflow design}
...
[^fn-von-neumann-bottleneck]: **Von Neumann Bottleneck**: The physical separation of the processor from its memory forces all instructions and data to traverse an energy-intensive bus. This distance is the direct cause of the high energy cost of data movement; every byte must be fetched, paying a physical tax. Accessing a value from external DRAM can cost over 20,000x more energy than performing an 8-bit integer operation on that value. \index{Von Neumann Bottleneck!ML accelerator constraint}
...
[^fn-compute-memory-bound-analogy]: **Compute-Bound vs. Memory-Bound**: The distinction hinges on the ratio of arithmetic operations to memory accesses for a given task. A memory-bound workload performs few computations for each byte it fetches, causing the memory system to be the bottleneck that starves the otherwise powerful compute units. This is diagnosed when a workload’s arithmetic intensity falls below the machine's hardware balance, often less than 10 FLOPs per byte for modern accelerators. \index{Compute-Bound vs. Memory-Bound!diagnostic}
...
**The Systems Lesson**: Hardware features are brittle contracts. If you don't send the exact data type (FP16/BF16) aligned to the exact dimensions (multiples of 8/16), the accelerator silently degrades to a generic processor. You cannot use hardware you do not conform to [@jia2018highly].
:::
...
[^fn-roofline-diagnostic]: **Roofline Model**: This framework provides quantitative reasoning by plotting a kernel’s arithmetic intensity (FLOPs/Byte) against its performance (GFLOPs/s) to reveal its proximity to the hardware's physical limits. This plot directly guides optimization by classifying the workload: kernels hitting the sloped memory-bandwidth line are memory-bound, while those hitting the flat compute-peak line are compute-bound. A kernel operating at only 50% of the performance ceiling for its given arithmetic intensity has a clear 2x utilization gap to close. \index{Roofline Model!optimization diagnostic}
