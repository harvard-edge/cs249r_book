...
The Intel 8087\index{Intel 8087!floating-point coprocessor}\index{Floating-Point Unit!history} (1980)[^fn-intel-8087-specialization] addressed this bottleneck by offloading arithmetic-intensive tasks to a dedicated unit. By implementing floating-point logic in hardware rather than software emulation, the 8087 produced large speedups for arithmetic-heavy scientific workloads [@palmer1980intel8087]. This established a core principle: when a specific data type or operation consumes the majority of execution cycles, moving it to specialized silicon can outperform general-purpose scaling.

[^fn-intel-8087-specialization]: **Intel 8087**: The coprocessor implemented floating-point logic directly in silicon, avoiding the CPU's slow, multi-instruction software emulation for each calculation. This offload strategy mattered because scientific workloads spent a large share of their cycles on these specific arithmetic operations. The 8087's success thus provided an early proof that specializing hardware for a dominant computational kernel can yield improvements unavailable from a general-purpose instruction path alone. \index{Intel 8087!specialization pattern}
...
[^fn-nvlink-bandwidth]: **NVLink (NVIDIA Link)**\index{NVLink!definition}: This direct GPU-to-GPU interconnect exists to keep accelerator-to-accelerator traffic off PCIe when tensors must move inside a server. Its aggregate bandwidth is much higher than a host PCIe path, so workloads with frequent cross-device tensor movement can remain in the fast part of the bandwidth taper [@NVIDIA2020nvlink; @choquette2023]. The training algorithms that determine how much tensor traffic must cross this link are developed later; the hardware fact needed here is the bandwidth gap. \index{NVLink!training scaling}
...
This data movement overhead is expensive: fetching from DRAM costs far more time and energy than reusing data from on-chip cache, scratchpad, or registers [@horowitz2014; @sze2017]. The solution is tiling.
...
Accelerators have separate datapaths for different precisions, and the peak throughput varies dramatically across them. NVIDIA's H100, for example, has a much higher peak rate for low-precision tensor operations than for FP32 CUDA-core operations [@choquette2023]. Estimating training time with the FP32 number when the workload actually uses BF16 produces utilization figures that look catastrophic for no reason, and matching against the wrong roofline misclassifies kernels as compute-bound when they are memory-bound (or vice versa). Always match the peak constant to the precision the workload actually issues, and quote precision explicitly when reporting MFU.
