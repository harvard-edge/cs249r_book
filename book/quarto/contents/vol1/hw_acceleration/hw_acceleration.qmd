...
*   **Example**: Running Llama-7B (14GB weights @ FP16) on a 16GB GPU leaves only `{python} FeasibilityAssessment.headroom_str` GB for context. Long prompts will OOM (Out of Memory).

**2. Bandwidth Check**

*   **Formula**: $T_{token} = \frac{\text{Model Size}}{\text{Memory Bandwidth}}$
*   **Constraint**: $T_{token} < \text{Latency Target}$
*   **Example**: Serving a 70B model (140GB) on a GPU with 1TB/s bandwidth yields ≈ `{python} FeasibilityAssessment.token_latency_ms_str` ms per token. If you need 50ms latency, this hardware fails regardless of compute power.
