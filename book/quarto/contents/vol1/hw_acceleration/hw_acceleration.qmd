...
Amdahl's Law is not just theory—it explains why many GPU upgrades disappoint in practice. Before examining specific hardware architectures, test your intuition about these fundamental limits.
...
: **Operations on the Roofline.** Neural network layers span a wide range of arithmetic intensities. By mapping these operations to the **Lighthouse Examples**, ResNet-50 emerges as compute-bound (high AI) while MobileNet and DLRM are memory-bound (low AI). {#tbl-roofline-operations}
...
: **Intermediate Tensor Storage.** Naive execution models require substantial memory to store intermediate tensors generated by each operation. For a 1024x1024 tensor, storing intermediate results (even when only the final output is needed) quadruples the total memory footprint from 4 MB to 16 MB. Minimizing intermediate data storage is essential for improving memory efficiency. {#tbl-memory-footprint}
...
::: {.callout-lighthouse title="Smart Doorbell: The Case for Heterogeneous Microcontrollers"}
...
Engineers who internalize the Roofline model and arithmetic intensity analysis gain a powerful diagnostic framework: when inference runs slower than expected, they can immediately determine whether the bottleneck lies in compute throughput, memory bandwidth, or software overhead—and select the appropriate optimization strategy accordingly. This systems-level understanding transforms hardware selection from vendor comparison into principled engineering.
