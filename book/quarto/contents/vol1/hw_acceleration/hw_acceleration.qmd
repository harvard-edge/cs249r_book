...
***Hardware-Software Co-design***\index{Hardware-Software Co-design!definition} is the ML accelerator development methodology that deliberately crosses conventional hardware-software abstraction boundaries, allowing algorithm constraints to inform silicon design and hardware capabilities to shape algorithm formulation.

1.  **Significance**: Co-design enables gains unavailable to either layer acting alone. 8-bit integer (INT8) quantization can deliver multi-fold throughput improvement on a supported tensor-core path, not because 8-bit arithmetic is faster in the abstract, but because the hardware can execute lower-precision operations more densely and move fewer bytes than an FP32 path; the exact gain depends on the architecture and workload [@NVIDIA2020; @dally2021; @dally2023].
...
Hardware acceleration *does not speed up the entire system*; it speeds up only the fraction of baseline execution time that uses the accelerated path. For accelerated fraction $p$ and accelerator gain $G_{\text{accel}}$, **Amdahl's Law for AI**\index{Amdahl's Law!parallel fraction}\index{Parallel Computing!Amdahl's Law} [@amdahl1967] governs this limit. @Eq-amdahl formalizes the resulting speedup bound.
$$ \text{Speedup} = \frac{1}{(1 - p) + \frac{p}{G_{\text{accel}}}} $$ {#eq-amdahl}

*   **Accelerated fraction**\index{Parallel fraction!definition} ($p$): The portion of baseline execution time, often in matrix multiplication or another supported kernel, that receives the stated accelerator gain.
...
*   **Unaccelerated fraction**\index{Serial Fraction!Amdahl bottleneck} ($1-p$): Work not improved by the chosen accelerator path, which may include data loading, host overhead, unsupported operators, and kernel launch latency\index{Kernel Launch Latency!serial overhead}.

**Pitfall**: *Unaccelerated work caps total speedup.* If 10 percent of baseline execution time is unaffected by the accelerator ($p=0.9$), even an infinite gain on the accelerated path ($G_{\text{accel}}=\infty$) can only achieve a 10$\times$ total speedup. The unaffected component dominates once the accelerated component becomes sufficiently fast.
...
Amdahl's Law also explains *why* many GPU upgrades disappoint in practice. @Fig-iron-law-heatmap visualizes the **acceleration wall**\index{Acceleration Wall!diminishing returns}, the diminishing returns from faster hardware when a meaningful share of baseline time remains unaffected. As $G_{\text{accel}}$ grows, the ceiling $1/(1-p)$ makes further investment progressively less valuable unless $p$ is correspondingly close to one. The contour values are illustrative ranges for intuition.
...
- [ ] **Workload variation**: compare the accelerated fractions of ResNet-50\index{ResNet-50!compute-bound workload} and MobileNet under a proposed hardware change, then predict which benefits more from accelerator throughput.
...
- The scenario assumes that H100 provides $G_{\text{accel}}$ = `{python} AmdahlH100.hw_speedup_mult_str` on the matrix-multiplication path relative to a reference CPU without matrix extensions (`{python} AmdahlH100.h100_int8_tops_str` INT8 vs. ~`{python} AmdahlH100.cpu_int8_tops_str`) [@choquette2023].
- It also assumes $p$ = `{python} AmdahlH100.p_resnet_str`: `{python} AmdahlH100.p_resnet_pct_str` of baseline time receives that gain, while `{python} AmdahlH100.serial_resnet_pct_str` remains in data loading, preprocessing, postprocessing, and other unaffected work.
$$
\text{`{python} AmdahlH100.resnet_speedup_eq_math`}
$$
Under these assumptions, a `{python} AmdahlH100.hw_speedup_mult_str` gain on the accelerated path produces only `{python} AmdahlH100.amdahl_resnet_speedup_mult_str` end-to-end speedup. The `{python} AmdahlH100.serial_resnet_pct_str` unaffected fraction sets the ceiling.

**Contrast with GPT-2 (autoregressive)**:

- The GPT-2 token-generation scenario instead assumes $p$ = `{python} AmdahlH100.p_gpt2_str`; the remaining `{python} AmdahlH100.serial_gpt2_pct_str` includes host overhead, sampling, and portions of the sequential decoding path that do not receive the stated matrix-multiplication gain.
$$
\text{`{python} AmdahlH100.gpt2_speedup_eq_math`}
$$
Less of this token-generation path benefits from the assumed matrix-multiplication gain. Even an infinite gain on that path yields only $1/(1-p)$ = `{python} AmdahlH100.amdahl_gpt2_ceiling_speedup_mult_str` maximum speedup. Serving-side techniques therefore change more than raw hardware speed: batching improves utilization across requests, while speculative decoding uses a small draft model to reduce the number of sequential verification steps.
