...
***The AI Memory Wall***\index{AI Memory Wall!definition} is the ML accelerator performance constraint that arises when arithmetic throughput $(R_{\text{peak}})$ outpaces memory bandwidth $(\text{BW})$. The core issue is whether the memory system can supply operands and absorb results quickly enough to sustain the specialized primitives introduced above.
...
**Scenario**: Locality is not enough: an operation must also match the accelerator's supported execution path. Consider a transformer workload moving from older GPUs to NVIDIA A100 GPUs, expecting a large speedup from Tensor Cores.
...
The dataflow building blocks are now in place: data locality, tensor layout, and kernel fusion. These fusion decisions are ultimately about data locality, which ties together the chapter's core data movement strategies. You should be able to reason through each one:
...
Before procuring hardware, validate all three hard constraints. The fallacies above reduce to a concrete procurement test:
