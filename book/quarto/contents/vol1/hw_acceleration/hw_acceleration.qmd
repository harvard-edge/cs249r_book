...
Non-linear functions play a fundamental role in machine learning by allowing neural networks to model complex relationships [@Goodfellow-et-al-2016]. @lst-nonlinear_layer illustrates a typical neural network layer sequence.
...
Tensor cores[^fn-hwacc-tensor-cores] provide an example of this approach. They expose matrix computation capabilities through specialized instructions, such as the tensor core operation shown in @lst-tensor_core_op.
...
**Hardware Ridge Points**: The ridge point determines the arithmetic intensity threshold where the transition from memory-bound to compute-bound occurs, as shown in @tbl-ridge-points:
...
**Neural Network Operations on the Roofline**: Different operations fall at different positions on the roofline as summarized in @tbl-roofline-operations:
