...
**Amdahl's Law** is not merely theoretical: it explains *why* many GPU upgrades disappoint in practice. The following heatmap (@fig-iron-law-heatmap) visualizes this "Acceleration Wall," showing that unless your workload is highly parallelizable ($P > 0.99$), investing in faster hardware yields diminishing returns. The contour values are illustrative ranges for intuition.
...
The systolic array structure shown in @fig-systolic-array illustrates these general principles, yet systolic architectures vary significantly across different accelerator designs. Training-focused architectures like Google's TPU employ large arrays (128×128 or larger) optimized for high computational throughput, while inference-oriented designs found in edge devices prioritize energy efficiency with smaller configurations (8×8 to 32×32).
...
The underlying cause of this wall is physical: moving data costs orders of magnitude more energy than processing it. As visualized in @fig-energy-hierarchy, the "Horowitz Numbers"\index{Horowitz Numbers!energy hierarchy}\index{Energy Hierarchy!silicon costs} reveal the fundamental energy constants of silicon.
...
@fig-compute-memory-imbalance illustrates this "AI Memory Wall," which continues to widen, making memory bandwidth rather than compute the primary constraint in AI acceleration. The values are illustrative to show the divergence.
...
As visualized in @fig-rising-ridge, which shows how the hardware's "Ridge Point"\index{Arithmetic Intensity!ridge points} has skyrocketed, making sparse operations increasingly inefficient. The trend is representative across recent accelerator generations.
