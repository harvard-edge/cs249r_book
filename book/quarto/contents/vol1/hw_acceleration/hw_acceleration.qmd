...
The key insight from this assembly sequence is that the vector multiply instruction processes several element pairs at once, while the reduction completes each partial dot product and the vector loads amortize instruction overhead across multiple data elements. The run-time vector length determines how many values each iteration transfers and processes, so the same loop works across implementations with different vector widths. The illustrative workload is divided into roughly `{python} WeightMatrixCalc.wm_vector_chunks_str` vector chunks, each executed by several vector and scalar instructions.
...
for feature in range(512):
    ...
    for batch in range(32):
        output[batch, feature] = (
            temp[batch, feature] * scale + shift
        )
```
...
# in memory
X2 = F.batch_norm(X1, running_mean, running_var,
                  training=False)  # Frozen inference BatchNorm
