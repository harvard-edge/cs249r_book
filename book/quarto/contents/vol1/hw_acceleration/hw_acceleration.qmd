...
class HwAccelSetup:
    ...
    cpu_gap_max_mult_str      = fmt_multiple(cpu_gap_max, precision=0, commas=False)
```
...
# │          AmdahlH100.hw_speedup_mult_str, AmdahlH100.h100_tops_int8_str
# └─────────────────────────────────────────────────────────────────────────────
...
class AmdahlH100:
    ...
    hw_speedup_mult_str = fmt_multiple(hw_speedup_factor, precision=0, commas=False)
...
- H100 delivers $G_{\text{accel}}$ = `{python} AmdahlH100.hw_speedup_mult_str` speedup over CPU for matrix multiply (`{python} AmdahlH100.h100_tops_int8_str` TOPS INT8 vs. ~8 TOPS on baseline CPU without AMX extensions)
...
# │ Exports: SystolicEnergy.energy_ratio_mult_str, SystolicEnergy.vector_energy_str,
...
- Concretely, a $128{\times}128$ array can achieve over `{python} AcceleratorEfficiencyAnchor.systolic_macs_cycle_str` MACs per cycle with `{python} AcceleratorEfficiencyAnchor.energy_dividend_mult_str` better energy efficiency than a standard vector unit by pulsing data through processing elements instead of repeatedly loading it from DRAM.
...
# │          InterconnectBandwidth.gap_ratio_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
class InterconnectBandwidth:
    ...
    gap_ratio_mult_str = fmt_multiple(round((hbm_bw / pcie_bw).to_base_units().magnitude), precision=0, commas=False)
```
...
**Systems insight**: The `{python} InterconnectBandwidth.gap_ratio_mult_str` gap between HBM and PCIe means that any data transfer from the CPU is a catastrophic performance event. **NVLink**\index{NVLink!PCIe wall} is the only way to scale computation across multiple GPUs without hitting the "PCIe Wall."
