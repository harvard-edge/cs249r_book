...
from mlsysim.fmt import fmt_int, fmt, fmt_flops, fmt_time, check
...
class TensorLifecycleCalc:
    ...
    latency_hbm_str        = fmt(Hardware.Tech.Memory.DRAM.latency.to(nanosecond).magnitude, precision=0, commas=False)
    latency_l2_str         = fmt(Hardware.Tech.Memory.L2.latency.to(nanosecond).magnitude, precision=0, commas=False)
    latency_l1_str         = fmt(Hardware.Tech.Memory.L1.latency.to(nanosecond).magnitude, precision=0, commas=False)
...
class Conv2dAnalysisCalc:
    ...
    conv_total_gflops_str  = fmt(conv_total_flops.to(GFLOPs).magnitude, precision=1, commas=False)
...
class LayernormAnalysisCalc:
    ...
    ln_attainable_str   = fmt(ln_attainable.to(TFLOP / second).magnitude, precision=1, commas=False)
...
class Gpt2ThroughputCalc:
    ...
    gpt2_decode_gflops_str = fmt(gpt2_decode_flops.to(GFLOPs).magnitude, precision=0, commas=False)
    ...
    a100_tflops_fp16_str  = fmt_flop_rate(a100_fp16_flops, unit=TFLOP / second, precision=0, commas=False)
    a100_bw_tbs_str       = fmt_bandwidth(a100_bandwidth, unit=TB / second, precision=2, commas=False)
    a100_tflops_fp32_str  = fmt(a100_fp32_flops.to(TFLOP / second).magnitude, precision=1, commas=False)
    a100_ridge_str        = fmt_int(round(a100_ridge_value), commas=False)
...
class FpWorkloadFitRecap:
    ...
    # OUTPUT
    a100_ridge_str = fmt_int(round(a100_ridge.to(flop / byte).magnitude), commas=False)
    a100_tflops_fp16_str = fmt_flop_rate(a100_peak_flops, unit=TFLOP / second, precision=0, commas=False)
    softmax_ai_low_str = fmt(softmax_ai_low.to(flop / byte).magnitude, precision=0, commas=False)
    softmax_ai_high_str = fmt(softmax_ai_high.to(flop / byte).magnitude, precision=0, commas=False)
    ...
    cpu_ridge_low_str = fmt(cpu_ridge_low.to(flop / byte).magnitude, precision=0, commas=False)
    cpu_ridge_high_str = fmt(cpu_ridge_high.to(flop / byte).magnitude, precision=0, commas=False)
...
class CarbonRoiCalc:
    ...
    co2_saved_str      = fmt(co2_saved_year.to(metric_ton).magnitude, precision=1)
```
