...
\begin{marginfigure}
\mlsysstack{90}{40}{15}{25}{20}{0}{0}{0}
\end{marginfigure}
...
**Model Definition**: Performance is bounded by two ceilings, as @eq-roofline formalizes:
...
**Calculating Memory Bandwidth Bounds**: The roofline model's memory-bound region is determined by the peak memory bandwidth. For an operation to achieve throughput $T_{\text{ops}}$ (TFLOPS) in the memory-bound regime, @eq-required-bandwidth gives the required bandwidth:
