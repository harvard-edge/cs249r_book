...
*   **Parallel fraction**\index{Parallel fraction!definition} ($p$): The portion of the workload, often matrix multiplication, that the accelerator can execute.
...
**Systems lesson**: Tailoring hardware silicon directly to core algorithmic primitives (matrix multiplication) delivers 15–30$\times$ higher inference throughput and 30–80$\times$ better performance-per-watt than general-purpose processor scaling.
...
**Systems lesson**: Custom hardware acceleration becomes mandatory when workload volume crosses fleet-level economic thresholds. Aggregate arithmetic intensity and server TCO drive custom ASIC deployment decisions.
...
::: {#nbk-hw-acceleration-loop-ordering-small-convolution .callout-notebook title="Loop ordering in a small convolution"}
...
|:-----------------|:-----------------------------------------------:|
| **X**\index{X!definition}            |   `{python} MemoryFootprintCalc.tensor_mb_str`  |
| **X'**\index{X'!definition}           |   `{python} MemoryFootprintCalc.tensor_mb_str`  |
| **X''**\index{X''!definition}          |   `{python} MemoryFootprintCalc.tensor_mb_str`  |
| **Y**\index{Y!definition}            |   `{python} MemoryFootprintCalc.tensor_mb_str`  |
...
|:---------------------------------|:-------------------------------------------------------------------------------------|:--------------------------------------------------|:------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Dataflow Strategy**\index{Dataflow Strategy!definition}            | Weight Stationary                                                                    | Input Stationary                                  | Weight Stationary | CNNs reuse filters across spatial locations; transformers reuse activations (KV-cache) under the input-stationary strategy introduced earlier; MLPs reuse weights across batches. |
...
|:----------------------------|:------------------------------------------------------------|:---------------------------------------------------------------|
| **Input Representation**\index{Input Representation!definition}    | Linear program code (C, Python)                             | Computational graph (ML models)                                |
| Execution Model         | Sequential or multi-threaded execution                      | Massively parallel tensor-based execution                      |
| **Optimization Priorities**\index{Optimization Priorities!definition} | Instruction scheduling, loop unrolling, register allocation | Graph transformations, kernel fusion, memory-aware execution   |
| **Memory Management**\index{Memory Management!definition}       | Stack and heap memory allocation                            | Tensor layout transformations, tiling, memory-aware scheduling |
| Target Hardware         | CPUs and other general-purpose targets                      | CPUs, GPUs, TPUs, and custom accelerators                      |
| **Compilation Output**\index{Compilation Output!definition}      | CPU-specific machine code                                   | Hardware-specific execution plan (kernels, memory scheduling)  |
...
| **Task Scheduling**\index{Task Scheduling!definition}         | CPU thread management                    | Kernel dispatch across accelerators                       |
...
| **Adaptability**\index{Adaptability!definition}            | Mostly static execution plan             | Selects among prepared shape profiles and kernel variants |
...
We have now explored the three core efficiency pillars of the ML optimization stack: data selection (@sec-data-selection) minimizes training sample requirements, model compression (@sec-model-compression) reduces algorithmic parameter and precision complexity, and hardware acceleration (@sec-hardware-acceleration) maximizes physical machine throughput. Optimization without empirical measurement, however, remains incomplete. @Sec-benchmarking moves from theoretical FLOP bounds to measured wall-clock latency, applying standardized benchmark suites and statistical protocols to test optimization claims against operational reality.
