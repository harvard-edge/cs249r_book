...
::: {.callout-chapter-connection title="From Algorithmic to Physical Optimization"}

The Model Compression chapter (@sec-model-compression) introduced techniques that reduce computational requirements through algorithmic means: quantization shrinks precision from FP32 to INT8, pruning removes unnecessary weights, and knowledge distillation trains smaller networks. But *why* does reducing precision from 32 bits to 8 bits yield 2-4× speedup? Why does structured pruning improve performance while unstructured pruning often doesn't?

The answers lie in hardware architecture. Quantization works because accelerators pack 4× more INT8 operations into the same silicon. Structured pruning works because it preserves memory access patterns that hardware can optimize. This chapter explains the physical constraints that make algorithmic optimizations effective—and reveals why some theoretically promising techniques fail to deliver real-world speedups.

**The Optimization Pipeline**: Think of ML optimization as a three-stage pipeline:

1. **Algorithmic** (Model Compression): Reduce what needs to be computed
2. **Mapping** (This Chapter): Match computation to hardware capabilities
3. **Deployment** (Benchmarking, Ops): Validate and operationalize the gains

Understanding hardware acceleration transforms you from a consumer of optimization techniques into an engineer who can reason about *why* techniques work and *when* they'll fail.

:::

## AI Hardware Acceleration Fundamentals {#sec-ai-acceleration-ai-hardware-acceleration-fundamentals-2096}

If Data Efficiency optimizes the "Source Code" and Compression optimizes the "Binary," then **Hardware Acceleration** optimizes the machine itself. It is the final physical frontier of the **Iron Law** (@sec-optimize-principles), where we design silicon to maximize "Math per Second" while minimizing "Energy per Bit."

Modern machine learning systems challenge the architectural assumptions underlying general-purpose processors. The efficiency framework established in @sec-introduction identified three pillars of ML system efficiency: algorithmic, compute, and data. While the software optimization techniques examined in @sec-model-compression address the algorithmic pillar through precision reduction, structural pruning, and execution refinements, hardware acceleration targets the compute pillar through architectural redesign rather than algorithmic modification.
...
::: {.callout-perspective title="The Central Question: Compute-Bound or Memory-Bound?"}

Throughout this chapter, one question will recur: *Is this workload limited by how fast we can compute, or how fast we can move data?* The answer determines everything—which accelerator to choose, which optimizations matter, and whether a 10× more powerful chip will actually help.

The **Roofline Model** (@sec-ai-acceleration-roofline-model) provides the analytical framework for answering this question. It plots an operation's **arithmetic intensity** (operations per byte of memory traffic) against hardware capabilities, revealing whether performance is capped by compute or bandwidth. A dense matrix multiplication with high arithmetic intensity benefits from more TFLOPS; a LayerNorm with low arithmetic intensity benefits from more memory bandwidth. Most real neural networks contain both types of operations, requiring careful analysis to optimize end-to-end performance.

Keep this distinction in mind as we examine hardware architectures. When we discuss systolic arrays, ask: "What arithmetic intensity do they require to achieve full utilization?" When we examine memory hierarchies, ask: "How do they shift the roofline boundary?" This analytical lens transforms hardware selection from guesswork into engineering.
...
This section provides awareness of multi-chip scaling challenges. The implementation details of distributed training systems, including gradient synchronization protocols, multi-node communication patterns, and cluster-scale orchestration, are covered in Volume II on distributed ML systems.
