...
The execution units examined in previous sections (SIMD units, tensor cores, and systolic arrays) provide impressive computational throughput: modern accelerators can reach hundreds of TFLOP/s or more for low-precision neural-network operations [@NVIDIA2020; @choquette2023; @nvidia2024blackwell]. Yet these theoretical capabilities remain unrealized in practice when memory subsystems cannot supply data at sufficient rates. This constraint, termed the AI memory wall\index{AI Memory Systems!bandwidth bottleneck}, represents the dominant bottleneck in real-world accelerator performance. It is also the physical core of the systems gap that @fig-systems-gap charted: of all the ways model demand outruns hardware supply, the lag of memory bandwidth behind arithmetic throughput is the dominant component, and the rest of this section quantifies it.
...
**AI runtimes**\index{AI Runtime!dynamic execution management} bridge this gap by providing a dynamic layer of execution management that extends compile-time optimizations with runtime decisions about memory, kernels, and execution profiles; TensorRT is one representative production inference runtime for this role [@nvidia2024tensorrt]. Unlike traditional compiled programs that execute a fixed instruction sequence, AI workloads require adaptive control over memory allocation, kernel execution, and resource scheduling—continuously monitoring execution conditions and making on-the-fly adjustments to maintain hardware utilization despite changing production conditions.
