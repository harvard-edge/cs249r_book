...
- Explain why **systolic arrays** and **Tensor Cores** achieve 10$\times$--100$\times$ better efficiency than general-purpose processors for matrix operations
...
*   **$S$ (Speedup):** The raw speed advantage of the GPU or Tensor Processing Unit (TPU) over the CPU (typically 100$\times$--1,000$\times$).
...
**The Pitfall:** If data loading takes 10 percent of the time ($p=0.9$), even an **infinite speed** accelerator ($S=\infty$) can only achieve a **10$\times$** total speedup. The "boring" serial part dominates the "exciting" AI part.
...
Speedup = 1 / ((1-`{python} AmdahlH100.p_resnet_str`) + `{python} AmdahlH100.p_resnet_str` / `{python} AmdahlH100.hw_speedup_str`) = 1 / (`{python} AmdahlH100.serial_resnet_str` + `{python} AmdahlH100.p_resnet_per_s_str`) ≈ `{python} AmdahlH100.amdahl_resnet_str`$\times$
...
Speedup = 1 / ((1-`{python} AmdahlH100.p_gpt2_str`) + `{python} AmdahlH100.p_gpt2_str` / `{python} AmdahlH100.hw_speedup_str`) = 1 / (`{python} AmdahlH100.serial_gpt2_str` + `{python} AmdahlH100.p_gpt2_per_s_str`) ≈ `{python} AmdahlH100.amdahl_gpt2_str`$\times$
...
[^fn-tpu-origin]: **TPU (Tensor Processing Unit)**: Google developed the TPU when projections showed that serving voice search on general-purpose CPUs would require doubling its data center footprint. The design made a radical trade-off, stripping away non-essential logic like caches and branch predictors to dedicate silicon to a massive $256\times256$ systolic array for matrix multiplication. This specialization delivered an immediate 15$\times$--30$\times$ improvement in throughput-per-watt over contemporary GPUs, validating the domain-specific approach. \index{TPU!origin}
...
These efficiency gains extend beyond instruction count reduction. Memory bandwidth utilization improves as vector loads transfer multiple values per operation, and energy efficiency increases because control logic is amortized across many data elements. These improvements compound across the deep layers of modern neural networks, where billions of element-wise operations execute per forward pass. The architectural pattern is not new. The Cray-1[^fn-cray-vector-legacy] pioneered the same approach for scientific computing in 1975 [@jordan1982guide], but neural networks have given it unprecedented commercial importance.
...
1.  **Vector Unit**: Loads $A$, loads $B$, computes $A \times B + C$, writes $C$.
    ...
    - **Energy**: ≈ `{python} SystolicEnergy.vector_accesses_str` $\times$ `{python} SystolicEnergy.dram_access_str` pJ = **`{python} SystolicEnergy.vector_energy_str` pJ/OP**.
...
**The Systems Conclusion**: A systolic array is **`{python} SystolicEnergy.energy_ratio_str`$\times$ more energy-efficient** than a naive vector unit for large matrix multiplications.

- Concretely, a $128\times128$ array can achieve over `{python} AcceleratorEfficiencyAnchor.systolic_macs_cycle_str` MACs per cycle with `{python} AcceleratorEfficiencyAnchor.energy_dividend_str` better energy efficiency than a standard vector unit by pulsing data through processing elements instead of repeatedly loading it from DRAM.
...
# │
# │ Imports: mlsysim.core.constants (SYSTOLIC_ARRAY_DIM), mlsysim.book (fmt)
...
3.  **PCIe Gen5 (64 GB/s)**: The host-to-device interface. This is the "OS" layer where **DMA (Direct Memory Access)** moves data from CPU RAM to GPU VRAM. It is 15$\times$--30$\times$ slower than NVLink, making it the primary bottleneck for data loading.
...
class InterconnectBandwidth:
    ...
    hbm_bw_str = f"{hbm_bw/1000:.1f} TB/s"
...
| **Intel Sapphire**\index{Intel!Sapphire Rapids} |    512-bit AVX |   $32\times32$ INT8/BF16 |                56 cores | Inference             |
...
# │
# │ Imports: mlsysim.core.constants (V100/A100/H100/TPUV5P FLOPS, \text{BW}, TFLOPs, second, GB, TB)
...
# │          AcceleratorEconomics.h100_pp_str, AcceleratorEconomics.tpuv4_tflops,
...
| **Intel Gaudi 2**                  | `{python} AcceleratorEconomics.gaudi_price_str` |     `{python} AcceleratorEconomics.gaudi_tflops` TFLOPS (INT8) | `{python} AcceleratorEconomics.gaudi_bw` GB/s | `{python} AcceleratorEconomics.gaudi_pp_str` |
...
model_names = ["AlexNet", "VGG-16", "ResNet-50", "BERT Large", "GPT-3", "PaLM",
               "GPT-4", "Gemini 1", "Llama 3.1", "DeepSeek-V3", "Llama 4 Maverick"]
...
visible_model_labels = {
    ...
    "GPT-4":              {"xytext": (0, 5), "ha": "center"},
    "Gemini 1":           {"xytext": (0, 5), "ha": "center"},
...
**Problem**: Why is on-chip SRAM necessary? Why not fetch all data from HBM?
...
**The Systems Conclusion**: Data cannot be fetched from DRAM in a single cycle. It is physically impossible. Local registers and SRAM (L1) are required to feed compute units at 2 GHz. The "memory wall" is partially a **Distance Wall**.
...
[^fn-nvlink-bandwidth]: **NVLink (NVIDIA Link)**\index{NVIDIA!T4}: This direct GPU-to-GPU interconnect is required because gradient synchronization (AllReduce) operations must exchange the entire model's gradients on every training step. Without the 600--900 GB/s of bandwidth this provides---roughly 10$\times$--14$\times$ more than the standard PCIe bus---the communication overhead causes training to become bottlenecked, preventing scaling beyond two to four GPUs in a node. \index{NVLink!training scaling}
...
System efficiency depends on keeping data as high up this hierarchy as possible. Once data drops to PCIe or Network speeds, it encounters a 30$\times$--100$\times$ slowdown.
...
The roofline model[^fn-roofline-diagnostic] [@williams2009roofline] provides the standard framework for understanding whether workloads are compute bound\index{Compute-Bound!definition} or memory bound\index{Memory-Bound!definition}, directly connecting the memory wall discussion to practical performance analysis. This model enables quantitative reasoning about accelerator utilization and guides optimization decisions.
...
1.  **Significance (Quantitative):** The intensity threshold separating memory-bound from compute-bound regimes is the roofline ridge point: $R_{\text{peak}} / \text{BW}$. For an A100 (312 TFLOPS BF16, 2 TB/s), the ridge point is $312 \times 10^{12} / (2 \times 10^{12}) = 156$ FLOP/byte. A large matrix-multiply achieves about 100--200 FLOP/byte (compute bound); a pointwise ReLU achieves about 0.5 FLOP/byte (memory bound)—placing these two operations in completely different optimization regimes on the same hardware.
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ CONV2D LAYER ROOFLINE ANALYSIS\index{Roofline Analysis!conv2d layer}
# ├─────────────────────────────────────────────────────────────────────────────
...
AI = `{python} Conv2dAnalysisCalc.conv_total_gflops_str` GFLOPs / `{python} Conv2dAnalysisCalc.conv_total_mb_str` MB = `{python} Conv2dAnalysisCalc.conv_ai_str` FLOP/byte
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ DENSE LAYER\index{Dense Layer!roofline analysis} ROOFLINE ANALYSIS\index{Roofline Analysis!dense layer}
# ├─────────────────────────────────────────────────────────────────────────────
...
**Arithmetic Intensity**:
AI = `{python} DenseLayerAnalysisCalc.dense_total_mflops_str` MFLOPs / `{python} DenseLayerAnalysisCalc.dense_total_mb_str` MB = `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte
...
Pattainable = `{python} DenseLayerAnalysisCalc.a100_bw` GB/s $\times$ `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte = `{python} DenseLayerAnalysisCalc.dense_attainable_str` TFLOPS
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ LAYERNORM ROOFLINE ANALYSIS\index{Roofline Analysis!layer normalization}
# ├─────────────────────────────────────────────────────────────────────────────
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
from mlsysim.core.constants import byte, MB, A100_MEM_BW, GB, GFLOPs, TFLOPs, second
...
**Arithmetic Intensity**:
AI = `{python} LayernormAnalysisCalc.ln_total_mflops_str` MFLOPs / `{python} LayernormAnalysisCalc.ln_total_mb_str` MB = `{python} LayernormAnalysisCalc.ln_ai_str` FLOP/byte

This is **severely memory-bound** (102$\times$ below ridge point). Performance is limited to:
Pattainable = `{python} LayernormAnalysisCalc.a100_bw_str` GB/s $\times$ `{python} LayernormAnalysisCalc.ln_ai_str` FLOP/byte = `{python} LayernormAnalysisCalc.ln_attainable_str` TFLOPS
...
3. **Low AI (<20 FLOP/byte)**: Memory-bound operations like small dense layers, element-wise operations
   ...
   - Impact: 2$\times$--4$\times$ speedup possible through fusion alone
...
This explains the 10$\times$--100$\times$ throughput improvement from batching in production inference systems, as MLPerf\index{MLPerf!inference benchmarking} inference scenarios demonstrate.
...
*   **Ridge Point (Compute/BW):** `{python} TensorLifecycleCalc.a100_tflops_fp16` / `{python} AcceleratorEconomics.a100_bw_tbs` = **`{python} RooflineGap.a100_ridge_str` FLOP/byte** (for FP16 Tensor Core).
...
1.  **Significance (Quantitative):** Within the D·A·M taxonomy, mapping is the Machine-axis decision that determines whether the Algorithm's operations run at $R_{\text{peak}}$ or at $\text{BW} \times I$. Specifically, a general matrix multiply (GEMM) with arithmetic intensity $AI$ runs at $\min(R_{\text{peak}},\; \text{BW} \times I)$; a poor tiling choice that forces unnecessary DRAM accesses can reduce effective $I$ by 10$\times$--50$\times$, collapsing a compute-bound operation into a bandwidth-bound one and causing a commensurate drop in sustained throughput.
...
$$
four^{5 \times 3} = 4^{15} = 1,073,741,824.
$$
...
2. **How is data organized?** Tensor layouts (NHWC vs. NCHW) determine whether memory accesses align with hardware preferences, with performance impacts of 2$\times$--5$\times$.
3. **How are operations combined?** Kernel fusion and tiling restructure computation to minimize memory traffic, often achieving 2$\times$--10$\times$ speedups through reduced data movement alone.
...
To understand how row-major layout works, consider a single RGB image represented as a tensor of shape (Height, Width, Channels). If the image has a size of $3\times3$ pixels with 3 channels (RGB), the corresponding tensor is structured as (3, 3, 3). The values are stored in memory as follows:
\begin{gather*}
...
[^fn-memory-coalescing-gpu]: **Memory Coalescing**: The GPU hardware mechanism that fuses memory requests from threads in a warp into a single transaction when those threads access contiguous memory. A channel-major layout (NCHW) is designed for this, ensuring that threads processing different pixels within the same channel access a contiguous data block. A layout that interleaves channels (NHWC) forces scattered, uncoalesced accesses that must be serialized, reducing effective memory bandwidth by 10$\times$--20$\times$ and starving compute units. \index{Memory Coalescing!tensor layout impact}
...
[^fn-nhwc-nchw-layout]: **NHWC vs. NCHW**: Frameworks perform this automatic switch because the NCHW layout groups data by channel, enabling a GPU to fetch data for many pixels in a single, wide "coalesced" memory transaction. Using the CPU-friendly NHWC layout on a GPU breaks this pattern, forcing many small, inefficient "scattered" reads that underutilize memory bandwidth ($\text{BW}$). This layout-to-hardware mismatch is not a micro-optimization; it is a primary factor that commonly creates performance gaps of 2$\times$--5$\times$. \index{NHWC vs. NCHW!performance impact}
...
In practice, modern AI compilers such as TensorFlow's XLA and PyTorch's TorchScript perform automatic layout transformations, converting tensors between NHWC and NCHW as needed to optimize performance across different processing units. The result is that machine learning models achieve high throughput without requiring developers to manually specify tensor layouts.
...
[^fn-kernel-fusion-memory]: **Kernel Fusion**: The "intermediate data movement" referenced occurs because each separate GPU function, or kernel, must write its result back to high-bandwidth memory (HBM) before the next one begins. By compiling multiple operations into a single kernel, fusion allows intermediate values to live in fast on-chip memory, completely avoiding the HBM write/read cycle. For memory-bound operations common in transformers, this reduction in memory traffic (often 2$\times$--3$\times$) translates directly to a proportional increase in performance. \index{Kernel Fusion!memory traffic reduction}
...
**How are operations combined?** Kernel fusion eliminates intermediate memory writes by merging consecutive operations (Conv2D + BatchNorm + ReLU becomes a single kernel). This optimization is most effective for element-wise operations that share data dependencies and can achieve 2$\times$--10$\times$ speedups by avoiding round-trips to DRAM.
...
This data movement overhead is expensive: fetching from DRAM is 100$\times$--1,000$\times$ slower than accessing on-chip cache or registers. The solution is tiling.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
from mlsysim.core.constants import T4_FLOPS_FP16_TENSOR, T4_MEM_BW, A100_MEM_BW, TFLOPs, second, GB
...
class FpSmallBatchCalc:
    ...
    a100_roof_b1 = a100_bw * ai_b1 / 1000
    t4_roof_b1 = t4_bw * ai_b1 / 1000
...
*   **Example**: Serving a 70B model (140 GB) on a GPU with 1 TB/s bandwidth yields ≈ `{python} FeasibilityAssessment.token_latency_ms_str` ms per token. If you need 50 ms latency, this hardware fails regardless of compute power.
...
class CarbonRoiCalc:
    ...
    cpu_energy_kwh_day = cpu_servers * cpu_power_w * 24 / 1000
    npu_energy_kwh_day = npu_chips * npu_power_w * 24 / 1000
...
* **Hardware-software co-design can achieve 10$\times$--100$\times$ performance improvements**: Matching algorithm patterns to architectural capabilities (systolic arrays for dense GEMM, sparse accelerators for pruned models) typically outperforms raw hardware upgrades.
