...
[^fn-amdahls-law-acceleration]: **Amdahl's Law**: Named after computer architect Gene Amdahl, who argued in 1967 that the serial fraction of a program sets an absolute ceiling on parallel speedup regardless of processor count. In ML systems, the serial fraction includes data loading, Python overhead, and gradient synchronization, and these components routinely cap GPU-era speedups at 5--20$\times$ even when accelerator throughput improves by 100$\times$. Gustafson's counterpoint --- scaling problem size alongside processor count --- explains why larger batch sizes partially circumvent this ceiling during training but offer no relief for latency-bound inference. \index{Amdahl's Law!acceleration ceiling}
...
[^fn-arithmetic-intensity-roofline]: **Arithmetic Intensity**: The ratio of floating-point operations to bytes of DRAM traffic (FLOP/byte), formalized as the x-axis of the roofline model by Williams, Waterman, and Patterson in 2009. This single metric determines whether adding compute or bandwidth will improve a given kernel: a dense matrix multiply at 100+ FLOP/byte is compute-bound, while a vector addition at 0.25 FLOP/byte is memory-bound. For ML systems, arithmetic intensity is the diagnostic that separates workloads that benefit from faster tensor cores from those that benefit from higher-bandwidth HBM. \index{Arithmetic Intensity!roofline diagnostic}
...
[^fn-tpu-origin]: **TPU (Tensor Processing Unit)**: Google began developing the TPU in 2013 after internal projections showed that voice search demand would require doubling their datacenter fleet if served on CPUs alone. The TPUv1 stripped away caches, branch predictors, and out-of-order logic to fill the die with a 256$\times$256 systolic array, achieving 15--30$\times$ better inference throughput per watt than the contemporary K80 GPU. Norman Jouppi's 2017 paper documenting the design established the blueprint for domain-specific AI accelerators: sacrifice generality to maximize throughput on the single primitive (matrix multiplication) that dominates neural network computation. \index{TPU!origin}
...
[^fn-intel-8087-specialization]: **Intel 8087**: The first commercial floating-point coprocessor (1980), which achieved up to 100$\times$ speedup over software emulation by implementing IEEE 754 arithmetic in dedicated silicon. Its success established the recurring pattern that drives modern AI accelerators: when a single operation class (floating-point then, matrix multiplication now) dominates execution time, moving it to specialized hardware yields order-of-magnitude gains that general-purpose scaling cannot match. \index{Intel 8087!specialization pattern}
...
[^fn-huangs-law-gpu]: **Huang's Law**: The observation that GPU performance for AI workloads has roughly doubled every year, outpacing Moore's Law through architectural innovation rather than transistor scaling alone. CUDA (2006) opened GPUs to general-purpose computing, Tensor Cores (2017) added dedicated matrix-multiply units, and the H100 (2022) integrated transformer-specific acceleration. Despite this pace, the Systems Gap (@fig-systems-gap) shows that model compute demands still outstrip hardware supply by growing roughly 10$\times$ faster, explaining why software optimizations (quantization, sparsity, efficient attention) remain essential even as hardware improves. \index{Huang's Law!GPU scaling}
...
[^fn-dsa-efficiency]: **Domain-Specific Architecture (DSA)**: Silicon optimized for a single application domain, sacrificing general-purpose programmability for efficiency gains that general scaling cannot deliver. Google's TPU achieves 15--30$\times$ better performance per watt than GPUs on neural network inference by eliminating branch prediction, caches, and out-of-order logic in favor of a systolic array filling the die. The trade-off is inflexibility: a DSA that excels at dense matrix multiplication may perform worse than a CPU on irregular workloads like graph traversal or sparse attention, making workload-hardware alignment the central design decision. \index{Domain-Specific Architecture!efficiency trade-off}

[^fn-moores-law-scaling]: **Moore's Law**: Gordon Moore's 1965 observation that transistor density doubles every 18--24 months. For four decades this exponential scaling delivered "free" performance gains, but power density constraints and manufacturing costs exceeding \$10 billion per advanced node have slowed cost-effective scaling. For ML systems, the consequence is decisive: model compute demands grow 4--10$\times$ faster than transistor density improves, forcing the shift from general-purpose scaling to domain-specific architectures that extract more performance per transistor through specialization. \index{Moore's Law!ML systems consequence}

[^fn-dennard-scaling-power]: **Dennard Scaling**: Robert Dennard's 1974 principle that shrinking transistors keeps power density constant, enabling higher clock frequencies without proportional power increases. Its breakdown around 2005 --- caused by quantum tunneling leakage and threshold voltage limits --- ended the era of "free" frequency scaling and forced the shift to parallelism. For ML accelerators, this is the physical reason why peak performance now comes from thousands of low-frequency cores (GPUs, TPUs) rather than a few high-frequency ones: power, not transistor count, is the binding constraint. \index{Dennard Scaling!power constraint}

[^fn-hennessy-patterson-dsa]: **Hennessy & Patterson**: John Hennessy and David Patterson, recipients of the 2017 Turing Award for pioneering RISC architectures, used their Turing Lecture to argue that the end of Moore's Law and Dennard scaling marks a "new golden age" for domain-specific architectures. Their thesis --- that general-purpose performance gains are exhausted and future progress requires matching silicon to workload --- is the intellectual foundation for every AI accelerator discussed in this chapter. \index{Hennessy, John!domain-specific architecture thesis}\index{Patterson, David!domain-specific architecture thesis}
...
[^fn-asic-flexibility]: **ASIC (Application-Specific Integrated Circuit)**: Custom silicon designed for a single application, achieving $10^3$ to $10^5\times$ better energy efficiency than general-purpose processors by eliminating all unused circuitry. The trade-off is inflexibility: an ASIC hardwired for one algorithm becomes obsolete if that algorithm changes. For ML systems, this tension is acute because model architectures evolve rapidly; Google's TPU mitigates it by targeting matrix multiplication, a primitive stable across architectures, while more specialized inference ASICs risk obsolescence when attention mechanisms or activation functions shift. \index{ASIC!flexibility trade-off}
...
Machine learning computational requirements reveal limitations in traditional processors. CPUs reach only `{python} cpu_utilization_min_str`–`{python} cpu_utilization_max_str`% utilization on neural network workloads, delivering approximately `{python} cpu_gflops_str` GFLOPS (billions of floating-point operations per second)[^fn-gflops-tops-gap] while consuming hundreds of watts. This inefficiency results from architectural mismatches: CPUs optimize for single-thread performance and irregular memory access, while neural networks require massive parallelism and predictable data streams. The memory bandwidth[^fn-memory-bw-cpu-hbm] constraint — the data transfer rate between memory and processors — becomes particularly severe: a single neural network layer may require accessing gigabytes of parameters, overwhelming CPU cache hierarchies[^fn-cache-nn-mismatch] designed for kilobyte-scale working sets.

[^fn-gflops-tops-gap]: **GFLOPS vs. TOPS**: GFLOPS ($10^{9}$ floating-point ops/second) measures floating-point throughput; TOPS ($10^{12}$ integer ops/second) measures INT8 throughput in AI accelerators. The gap between peak and realized performance is the critical systems insight: the A100 delivers `{python} a100_tflops_fp16` TFLOPS peak FP16 but typical ML workloads achieve only 10--30% of peak because memory bandwidth, not arithmetic, is the binding constraint. This peak-to-realized gap explains why comparing accelerators by peak FLOPS alone is misleading --- what matters is whether the workload's arithmetic intensity is high enough to saturate the compute units. \index{GFLOPS!peak vs. realized}

[^fn-memory-bw-cpu-hbm]: **Memory Bandwidth**: The data transfer rate between memory and processors, measured in GB/s. The gap between CPU DRAM bandwidth (50--100 GB/s) and accelerator HBM bandwidth (2--3 TB/s) represents a 20--60$\times$ difference that explains why specialized hardware achieves order-of-magnitude speedups on bandwidth-bound neural network operations. This bandwidth gap, not the arithmetic throughput gap, is the primary physical reason accelerators outperform CPUs on ML workloads. \index{Memory Bandwidth!CPU-HBM gap}

[^fn-cache-nn-mismatch]: **Cache Hierarchy**: CPU caches (L1/L2/L3) are designed for kilobyte-to-megabyte working sets with temporal and spatial locality. Neural network layers routinely require gigabytes of weights and activations, exceeding cache capacity by orders of magnitude and forcing frequent DRAM accesses. This architectural mismatch --- caches optimized for branch-heavy code with small working sets versus neural networks with massive, streaming data access --- is a primary reason CPUs achieve only 1--5% utilization on ML workloads, driving the need for accelerators with software-managed scratchpads and HBM. \index{Cache Hierarchy!neural network mismatch}
...
Training and inference present distinct computational profiles that influence accelerator design. Training requires high-precision arithmetic (FP32 or FP16)\index{FP16!training precision}\index{FP32!gradient computation} for gradient computation and weight updates, bidirectional data flow for backpropagation\index{Backpropagation!memory requirements}[^fn-backprop-memory-cost], and large memory capacity for storing activations. Inference can exploit reduced precision (INT8 or INT4), requires only forward computation, and prioritizes latency over throughput[^fn-latency-throughput-hw]. These differences drive specialized architectures: training accelerators maximize FLOPS and memory bandwidth, while inference accelerators optimize for energy efficiency and deterministic latency.

[^fn-backprop-memory-cost]: **Backpropagation Memory Cost**: Backward propagation of errors requires storing all intermediate activations from the forward pass so that gradients can be computed layer by layer via the chain rule. This bidirectional data flow increases memory requirements 2--3$\times$ compared to inference-only forward passes, making memory capacity rather than compute throughput the binding constraint for training accelerator design. Techniques like activation checkpointing (see @sec-model-training) trade roughly 33% extra compute for up to 10$\times$ memory reduction, a trade-off that only makes sense because of this memory-dominated cost structure. \index{Backpropagation!memory cost}

[^fn-latency-throughput-hw]: **Latency vs. Throughput in Accelerator Design**: Training accelerators optimize for throughput (samples/second) because large batches amortize fixed overheads, while inference accelerators optimize for latency (milliseconds per request) because real-time applications impose strict response deadlines. These opposing objectives produce fundamentally different hardware: training chips maximize parallel arithmetic units and memory bandwidth, while inference chips minimize pipeline depth and prioritize deterministic scheduling. An H100 optimized for training throughput can deliver 3$\times$ worse tail latency than an inference-tuned accelerator at the same arithmetic capability. \index{Latency vs. Throughput!accelerator design}
...
[^fn-scratchpad-ml]: **Scratchpad Memory**: Software-managed fast memory near compute units, contrasting with hardware-managed caches that guess what data to keep. For ML workloads, scratchpads are decisive because neural network data movement is mathematically determined by the network's layer dimensions, making programmer-controlled placement far more efficient than cache heuristics designed for unpredictable access patterns. Google's TPU uses a 24 MB scratchpad rather than a cache hierarchy, and this design choice alone accounts for much of its energy advantage over GPUs on dense matrix workloads. \index{Scratchpad Memory!ML advantage}
...
dense = Dense(512)(input_tensor)  # 256$\times$512 = 131K MACs per sample
```
:::
...
# Example: 32$\times$512$\times$256 = 4.2M multiply-accumulate
# operations
```
:::
...
[^fn-riscv-ai-customization]: **RISC-V for AI**: The open-source instruction set architecture from UC Berkeley (2010) that allows hardware designers to add custom ML instructions without licensing fees. Unlike proprietary ISAs, RISC-V enables domain-specific extensions --- companies like SiFive and Google have added dedicated matrix-multiply and vector-reduction instructions tailored to neural network primitives. For ML systems, the significance is architectural freedom: designers can co-design the instruction set with their dataflow strategy, eliminating the overhead of encoding ML operations in instructions designed for general-purpose workloads. \index{RISC-V!AI customization}
...
Z = matmul(weights, input)  # Matrix: transforms [256$\times$32]
# input to [512$\times$32] output
...
[^fn-im2col-memory-tradeoff]: **Im2col (Image-to-Column)**: Converts convolution operations into matrix multiplications by unfolding overlapping image patches into column vectors. A 3$\times$3 convolution on a 224$\times$224 image creates a matrix with roughly 50,000 columns, enabling efficient GEMM execution on tensor cores but increasing memory usage 9$\times$ due to patch overlap. This memory-for-compute trade-off is why modern ML accelerators treat convolutions as matrix operations: the 9$\times$ memory overhead is acceptable because it unlocks the full throughput of hardware optimized for dense GEMM, which would otherwise sit idle during native convolution. \index{im2col!memory-compute trade-off}
...
[^fn-simd-simt-parallelism]: **SIMD (Single Instruction, Multiple Data)**: From Michael Flynn's 1966 taxonomy. SIMD applies one instruction to multiple data elements simultaneously; SIMT (Single Instruction, Multiple Thread), coined by NVIDIA, extends this by giving each thread its own program counter while sharing instruction fetch. The distinction matters for ML systems because SIMT handles the control-flow divergence that arises in operations like masked attention and conditional computation, whereas pure SIMD requires all lanes to execute identically. CPUs use 512-bit SIMD (AVX-512, processing 16 floats), while GPUs coordinate tens of thousands of SIMT threads, explaining the 100$\times$ throughput gap on regular ML workloads. \index{SIMD!SIMT distinction for ML}
...
[^fn-dally-gpu-precision]: **William Dally and Reduced-Precision ML**: Stanford professor and NVIDIA Chief Scientist (2009--present) whose research on interconnection networks and domain-specific computing shaped modern GPU architecture. His 2015 demonstration that neural networks achieve AlexNet-level accuracy using only 8-bit precision directly motivated the INT8 tensor cores now standard in inference hardware. This finding reshaped accelerator design: by proving that neural networks tolerate reduced precision, Dally's work enabled architects to pack 4$\times$ more INT8 operations into the same silicon area as FP32, fundamentally changing the performance-per-watt equation for ML inference. \index{Dally, William!reduced-precision ML}

[^fn-sm-gpu-building-block]: **Streaming Multiprocessor (SM)**: The fundamental building block of NVIDIA GPU architectures, containing execution lanes, shared memory (typically 128--228 KB), and warp schedulers. An H100 contains 132 SMs, each capable of executing thousands of threads concurrently. For ML workloads, SM occupancy --- the fraction of available warps actively scheduled --- directly determines whether the GPU reaches its theoretical throughput or idles waiting for memory, making SM-level optimization the primary lever for improving $\eta$ in the Iron Law. \index{Streaming Multiprocessor!occupancy}

[^fn-warp-divergence]: **Warp**: NVIDIA's basic execution unit of 32 threads that share a single instruction fetch and execute in lock-step. The name borrows from weaving, where parallel threads are held taut on a loom. The critical ML systems consequence is *warp divergence*: when threads within a warp take different control-flow paths (as in masked attention or conditional computation), the hardware serializes both paths, reducing effective throughput by up to 50%. This penalty explains why ML operations that appear "simple" (like ReLU with its conditional branch) must be implemented as branchless predicated operations to maintain full warp efficiency. \index{Warp!divergence penalty}
...
[^fn-tensor-core-origin]: **Tensor Core**: Introduced by NVIDIA in the V100 (2017) as dedicated matrix-multiply-accumulate units operating on small tiles (originally 4$\times$4$\times$4). By executing an entire tile multiplication in a single instruction, tensor cores bypass the instruction fetch, decode, and scheduling overhead that conventional CUDA cores pay per element. The result on supported kernels is an order-of-magnitude throughput improvement over standard GPU execution at reduced precision (FP16/BF16/INT8), which is why every major ML framework auto-dispatches to tensor cores when available --- failing to do so leaves 90% of the hardware's capability unused. \index{Tensor Core!throughput mechanism}
...
[^fn-npu-on-device-strategy]: **On-Device Neural Engine (NPU)**: Mobile SoCs include dedicated neural processing units that hardcode common dataflows (systolic arrays for matrix multiplication) rather than supporting general computation. This trade-off delivers 10--100$\times$ better energy efficiency on supported operations compared to running the same kernels on the SoC's GPU, enabling always-on features like face detection and voice activation within milliwatt power budgets. The constraint is programmability: operations not natively supported by the NPU's fixed dataflow must fall back to the GPU or CPU, making operator coverage the primary deployment bottleneck for on-device ML. \index{NPU!on-device strategy}
...
[^fn-sparsity-nm-regularity]: **N:M Structured Sparsity**: The N:M notation specifies that exactly N values must be non-zero in every contiguous block of M values, imposing regularity on an otherwise irregular optimization. This regularity is the key: unstructured pruning (random zeros) produces scattered memory accesses that defeat hardware prefetching and coalescing, yielding no speedup despite fewer operations. Structured sparsity preserves the predictable access patterns that tensor cores and systolic arrays require, enabling NVIDIA's 2:4 Sparse Tensor Cores to deliver a theoretical 2$\times$ throughput gain with minimal accuracy loss after fine-tuning. \index{Sparsity!N:M regularity constraint}
...
- Concretely, a 128$\times$128 array can achieve over `{python} systolic_macs_cycle_str` MACs per cycle with `{python} accelerator_energy_dividend_str` better energy efficiency than a standard vector unit by pulsing data through processing elements instead of repeatedly loading it from DRAM.
...
Kung and Leiserson\index{Kung, H.T.!systolic array inventor}[^fn-systolic-array-dataflow] [@kung1979systolic] first introduced systolic arrays, formalizing their use in parallel computing architectures for efficient matrix operations [@Kung1982]. Unlike general-purpose execution units, systolic arrays exploit spatial and temporal locality\index{Data Locality!spatial and temporal} by reusing operands as they propagate through the grid. Google's TPU\index{TPU!systolic array scaling} exemplifies this architectural approach: in the TPUv4, a $128\times128$ systolic array\index{Systolic Arrays!128$\times$128 TPU} of multiply-accumulate units\index{Multiply-Accumulate!MAC unit} processes matrix operations by streaming data through the array in a pipelined manner [@jouppi2017datacenter]. Follow the data paths in @fig-systolic-array to see how inputs stream horizontally while weights flow vertically, with each processing element performing one multiply-accumulate before passing operands to its neighbors.

[^fn-systolic-array-dataflow]: **Systolic Array**: Named from the Greek "systole" (cardiac contraction), capturing how data pulses through the processing element grid in waves. The etymology explains the design: just as blood flows through heart chambers in a coordinated rhythm, each processing element computes and passes operands to its neighbors, so a single weight participates in dozens of multiply-accumulate operations as it traverses the array. This maximizes data reuse and minimizes DRAM accesses, which is why Google's TPUv4 pairs a 128$\times$128 systolic array with HBM to sustain high utilization on dense matrix workloads. \index{Systolic Array!dataflow design}
...
The preceding sections revealed impressive computational machinery: vector units achieving 8$\times$ parallelism through SIMD execution, matrix operations processing 256 elements simultaneously, and tensor cores executing 16$\times$16$\times$16 fused multiply-accumulate blocks in single cycles. An NVIDIA A100's tensor cores can execute `{python} a100_tflops_fp16` trillion operations per second, and an H100 pushes this further to nearly 2 petaFLOPS (dense) in FP8 precision. At these rates, the pure arithmetic for a ResNet-50 forward pass could complete in microseconds.
...
[^fn-von-neumann-bottleneck]: **Von Neumann Bottleneck**: John Backus coined this term in his 1977 Turing Award lecture for the throughput constraint inherent in von Neumann's stored-program architecture, where instructions and data share the same memory bus. For ML accelerators, this bottleneck is the fundamental adversary: moving a single value from DRAM costs roughly 20,000$\times$ more energy than an INT8 addition (see @fig-energy-hierarchy). Every architectural innovation in this chapter --- systolic arrays, scratchpad memories, HBM, kernel fusion --- exists to circumvent this bottleneck by keeping data close to compute and minimizing round trips to external memory. \index{Von Neumann Bottleneck!ML accelerator constraint}
...
The memory wall manifests through three critical constraints.[^fn-compute-memory-bound-analogy] First, the energy disparity: accessing DRAM can consume orders of magnitude more energy than a multiply-accumulate operation [@horowitz2014computing; @sze2020efficient], which often shifts bottlenecks from raw compute to power and data movement. Second, the bandwidth limitation: even TB/s memory systems may not feed large parallel compute arrays continuously on memory-bound workloads, leaving compute underutilized. Third, the latency hierarchy: off-chip memory access can require hundreds of cycles, creating pipeline stalls that cascade through parallel execution units.

[^fn-compute-memory-bound-analogy]: **Compute-Bound vs. Memory-Bound**: A compute-bound system has more data than it can process (the arithmetic units are the bottleneck), while a memory-bound system has more compute than it can feed (the memory bus is the bottleneck, and arithmetic units sit idle). For ML systems, this distinction is diagnostic: adding tensor cores to a memory-bound workload like LayerNorm yields zero speedup, while adding HBM bandwidth to a compute-bound workload like large GEMM yields zero speedup. The roofline model (@eq-roofline) formalizes this dichotomy through arithmetic intensity. \index{Compute-Bound vs. Memory-Bound!diagnostic}
...
[^fn-sparsity-memory-access]: **Sparsity and Memory Irregularity**: Sparsity introduces non-uniform memory access because sparse representations require fetching scattered elements rather than sequential blocks. Natural sparsity from ReLU activations (50--90% zeros) and artificial sparsity from pruning (90--99% of weights removed) both create this irregularity. The hardware consequence is severe: without structured sparsity support (like NVIDIA's 2:4 pattern), sparse models can run *slower* than their dense counterparts because the memory access overhead of gathering non-zero elements exceeds the savings from reduced computation. \index{Sparsity!memory access irregularity}
...
**The Systems Lesson**: Hardware features are brittle contracts. If you don't send the exact data type (FP16/BF16) aligned to the exact dimensions (multiples of 8/16), the accelerator silently degrades to a generic processor. You cannot "utilize" hardware you do not conform to [@jia2018highly].
:::
...
[^fn-nvlink-bandwidth]: **NVLink**: NVIDIA's proprietary high-bandwidth GPU interconnect, providing 600 GB/s (A100, NVLink 3.0) to 900 GB/s (H100, NVLink 4.0) --- roughly 10--14$\times$ PCIe Gen4 bandwidth. NVLink enables NVSwitch fabrics where every GPU in a node communicates at full bandwidth, which is critical because gradient synchronization during multi-GPU training requires exchanging the entire model's gradients every iteration. Without NVLink-class bandwidth, multi-GPU training on large models becomes communication-bound at 2--4 GPUs rather than scaling to 8. \index{NVLink!training scaling}
...
[^fn-infiniband-rdma]: **InfiniBand**: A high-speed networking standard whose key advantage for ML training is RDMA (Remote Direct Memory Access), allowing GPUs to read and write remote memory without CPU involvement. This kernel-bypass capability reduces inter-node gradient synchronization latency from milliseconds (TCP/IP) to microseconds, which is critical because AllReduce operations occur every training iteration. NVIDIA's 2020 acquisition of Mellanox (the primary InfiniBand vendor) for \$6.9 billion gave NVIDIA control over both compute and networking for AI training, vertically integrating the two components that most constrain distributed scaling. \index{InfiniBand!RDMA for ML training}
...
[^fn-dma-overlap]: **DMA (Direct Memory Access)**: A hardware mechanism that transfers data between system memory and accelerator memory without CPU intervention. For ML training pipelines, DMA enables the critical optimization of overlapping data transfer with computation: while the GPU computes on the current batch, a DMA engine copies the next batch from host memory in the background. Without this overlap, the accelerator idles during every PCIe transfer, reducing effective throughput by 20--40% on typical training workloads. \index{DMA!computation overlap}
...
[^fn-roofline-diagnostic]: **Roofline Model**: Named for its visual appearance when plotted --- a flat compute ceiling meets a sloped memory ceiling, forming a roofline shape. Introduced by Williams, Waterman, and Patterson in 2009, the model's diagnostic power for ML systems lies in its ability to immediately classify any kernel as compute-bound or memory-bound on a given accelerator. This classification determines the correct optimization strategy: compute-bound kernels benefit from reduced precision or more tensor cores, while memory-bound kernels benefit from kernel fusion, tiling, or higher-bandwidth memory. \index{Roofline Model!optimization diagnostic}
...
- Weights: 256$\times$128$\times$3$\times$3$\times$2 ≈ `{python} conv_weights_mb_str` MB (FP16)
...
[^fn-memory-coalescing-gpu]: **Memory Coalescing**: The GPU hardware mechanism that combines multiple memory requests from threads in a warp into a single efficient transaction when threads access consecutive addresses. Uncoalesced access (scattered addresses) reduces effective bandwidth by 10--20$\times$, which is why tensor layout (NCHW vs. NHWC) has such outsized impact on GPU performance: a layout that aligns channel data contiguously enables coalescing, while one that interleaves channels forces scattered accesses that starve the compute units. \index{Memory Coalescing!tensor layout impact}
...
[^fn-nhwc-nchw-layout]: **NHWC vs. NCHW**: Tensor layout formats where letters indicate dimension order: N (batch), H (height), W (width), C (channels). NHWC interleaves channels pixel-by-pixel (CPU-friendly for sequential access), while NCHW groups all values per channel contiguously (GPU-friendly for coalesced access). A 224$\times$224 RGB image in NHWC stores as [R1,G1,B1,R2,G2,B2,...] while NCHW stores as [R1,R2,...,G1,G2,...,B1,B2,...]. This seemingly minor layout difference causes 2--5$\times$ performance differences because it determines whether GPU memory accesses coalesce or scatter, directly affecting the $BW$ term in the Iron Law. \index{NHWC vs. NCHW!performance impact}
...
[^fn-kernel-fusion-memory]: **Kernel Fusion**: In GPU computing, a "kernel" is a compiled function launched on the accelerator. Without fusion, a sequence like MatMul followed by ReLU followed by LayerNorm requires three separate kernel launches, each writing intermediate results to HBM and reading them back. Fusing these into a single kernel eliminates the intermediate writes entirely, reducing memory traffic by 2--3$\times$ for typical transformer blocks. For memory-bound operations (which most non-GEMM operations are), this memory traffic reduction translates directly to proportional speedup. \index{Kernel Fusion!memory traffic reduction}
...
[^fn-tiling-cache-reuse]: **Tiling (Loop Blocking)**: Partitions large matrix computations into blocks sized to fit in fast on-chip memory (shared memory, registers, or L1 cache), maximizing data reuse before eviction. Monica Lam formalized the technique in her 1991 PhD thesis on cache optimization. For ML systems, tiling is what makes the gap between naive and optimized GEMM a 10--50$\times$ difference: naive matrix multiplication fetches each element from DRAM $O(N)$ times, while tiled multiplication fetches each element once per tile, reducing DRAM traffic by the tile dimension. FlashAttention applies the same principle to attention computation, tiling along the sequence dimension to avoid materializing the full $n \times n$ attention matrix. \index{Tiling!DRAM traffic reduction}
...
[^fn-npu-efficiency-tradeoff]: **NPU (Neural Processing Unit)**: As introduced in @sec-ml-systems, NPUs achieve 10--100$\times$ better energy efficiency than GPUs by hardcoding common dataflows (systolic arrays for matrix multiplication, depthwise convolution engines) rather than supporting general computation. This specialization creates a sharp coverage boundary: operators that match the NPU's fixed datapath run at peak efficiency, while unsupported operators must fall back to the CPU or GPU, making operator coverage the critical deployment constraint for on-device ML. \index{NPU!efficiency trade-off}
