...
compute_energy_str = str(compute_energy_pj)
...
2.  **Systolic Array ($`{python} systolic_size_str` \times `{python} systolic_size_str`$ size)**: Loads $A$ and $B$ once at the edges. Data "pulses" through `{python} systolic_size_str` processing elements.
    ...
    - **Energy**: $\approx `{python} systolic_access_str` \times `{python} dram_access_str`\text{ pJ} + `{python} compute_energy_str` \text{ pJ (compute)} \approx \mathbf{`{python} systolic_energy_str` \text{ pJ/OP}}$.
...
$$P_{\text{attainable}} = `{python} a100_bw_str` \text{ GB/s} \times `{python} ln_ai_str` \text{ FLOP/byte} = `{python} ln_attainable_str` \text{ TFLOPS}$$
