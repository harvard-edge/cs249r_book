...
| **Tensor**       | **Size (MiB) for 1024 $\times$ 1024 Tensor** |
| **X**            | `{python} tensor_mb_str` MiB                 |
| **X'**           | `{python} tensor_mb_str` MiB                 |
| **X''**          | `{python} tensor_mb_str` MiB                 |
| **Y**            | `{python} tensor_mb_str` MiB                 |
| **Total Memory** | **`{python} total_mb_str` MiB**              |

: **Intermediate Tensor Storage.** Naive execution models require substantial memory to store intermediate tensors generated by each operation. For a 1024x1024 tensor, storing intermediate results (even when only the final output is needed) quadruples the total memory footprint from `{python} tensor_mb_str` MiB to `{python} total_mb_str` MiB. Minimizing intermediate data storage is essential for improving memory efficiency. {#tbl-memory-footprint}
...
# Fusion benefits
naive_mb = total_mb # 16 MiB
fused_mb = tensor_mb # 4 MiB (Just Y, or intermediates = 0?)
# Text says "lowering ... to 4 MiB". Let's assume 4 MiB is the target.

naive_mb_str = f"{naive_mb:.0f} MiB"
fused_mb_str = f"{fused_mb:.0f} MiB"
```

+---------------------+---------------------------------+------------------------------+
| **Execution Model** | **Intermediate Tensors Stored** | **Total Memory Usage (MiB)** |
+:====================+:================================+=============================:+
...
3. **Memory planning** determines that intermediate activations require approximately 2.1 GiB at batch size 32, fitting comfortably in the A100's 40 GiB HBM
