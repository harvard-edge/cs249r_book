...
These diverse domains of specialization share several common characteristics: identification of domain-specific computational patterns, development of specialized processing elements and memory hierarchies, creation of domain-specific programming models, and progressive evolution toward more flexible architectures. This pattern of architectural co-evolution established the foundation for contemporary AI hardware design. The GPU's success in parallelizing 3D graphics pipelines[^fn-pipeline-etymology] enabled its subsequent adoption for training deep neural networks, exemplified by AlexNet[^fn-hwacc-alexnet] in 2012, which executed on consumer-grade NVIDIA GPUs.

[^fn-pipeline-etymology]: **Pipeline**: From the physical pipes that transport oil or water in stages. Computer architects borrowed this industrial metaphor in the 1960s to describe breaking instruction execution into sequential stages (fetch, decode, execute, writeback), allowing multiple instructions to be "in flight" simultaneously like fluid flowing through connected pipes. Graphics pipelines extended this concept to sequences of rendering stages, establishing the architectural pattern that GPUs later adapted for general parallel computation. DSP innovations in low-power signal processing facilitated real-time inference on edge devices, including voice assistants and wearables. These domains informed ML hardware designs and established that accelerators could be deployed across both cloud and embedded contexts, principles that continue to influence contemporary AI ecosystem development.
...
Matrix operations form the computational workhorse of neural networks, transforming high-dimensional data through structured patterns of weights, activations, and gradients[^fn-gradient-etymology] [@Goodfellow-et-al-2016].

[^fn-gradient-etymology]: **Gradient**: From Latin "gradus" (step), describing the rate of change of a function as you "step" in each direction. In calculus, the gradient points toward steepest ascent. Neural network training inverts this to find steepest descent toward minimum loss, hence "gradient descent." The term perfectly captures the iterative stepping process: each training step uses the gradient to determine which direction to move parameters. While vector operations process elements independently, matrix operations orchestrate computations across multiple dimensions simultaneously. These operations reveal patterns that drive hardware acceleration strategies.
...
While unstructured pruning reduces model size, it rarely translates to hardware speedup because memory access becomes irregular. Hardware accelerators solve this with **N:M Structured Sparsity**[^fn-sparsity-etymology], a pattern-based approach that enforces regularity.

[^fn-sparsity-etymology]: **Sparsity**: From Latin "sparsus" (scattered), describing how zeros are distributed throughout a matrix rather than densely packed. In ML, sparse matrices have most elements as zero. The N:M notation indicates that exactly N elements must be non-zero in every M-element block, combining the computational benefits of zeros with the predictable access patterns hardware requires.
...
[^fn-allreduce]: **AllReduce**: From MPI (Message Passing Interface) terminology, where "reduce" operations combine values across processes using a function like sum or max, and "all" means every process receives the result. MPI standardized these collective operations in 1994. In distributed training, AllReduce aggregates gradients from all GPUs (the reduce), then distributes the averaged result back to all GPUs (the all). Ring AllReduce algorithms achieve optimal bandwidth utilization by having each process send and receive simultaneously.
