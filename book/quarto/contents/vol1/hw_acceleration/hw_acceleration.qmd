...
1.  **Significance (Quantitative):** Within the D·A·M taxonomy, mapping is the Machine-axis decision that determines whether the Algorithm's operations run at $R_{peak}$ or at $BW \times I$. Specifically, a GEMM with arithmetic intensity $I$ runs at $\min(R_{peak},\; BW \times I)$; a poor tiling choice that forces unnecessary DRAM accesses can reduce effective $I$ by 10–50$\times$, collapsing a compute-bound operation into a bandwidth-bound one and causing a commensurate drop in sustained throughput.
