...
3.  **Common Pitfall:** A frequent misconception is that hardware acceleration uniformly speeds up ML systems. Acceleration is a multiplier only on the parallelizable fraction of the workload (Amdahl's Law): if data loading, Python overhead, and kernel launches constitute 10% of total time, an infinitely fast accelerator can at most achieve a 10$\times$ end-to-end speedup.
...
2.  **NVLink (900 GB/s)**: The GPU-to-GPU interconnect. In a single-machine system, NVLink allows 8 GPUs to share data at speeds approaching on-device bandwidth, enabling **Single-Node Model Parallelism**.
...
# │          InterconnectHierarchy.gap_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware
from mlsysim.core.constants import GB, second, TB, PCIE_GEN5_BW, INFINIBAND_NDR_BW
...
class InterconnectHierarchy:
    ...
    nvlink_bw = 900.0 # H100 NVLink 4.0
...
# │
# │ Imports: mlsysim.core.constants (V100/A100/H100/TPUV4 FLOPS, BW, TFLOPs, second, GB, TB)
# │          mlsysim.Hardware (Cloud.V100, Cloud.A100, Cloud.H100, Cloud.TPUv4)
...
*   **A100 (2020)**: `{python} TensorLifecycleCalc.a100_tflops_fp16` TF / `{python} AcceleratorEconomics.a100_bw_tbs` TB/s ≈ **`{python} RooflineGap.a100_ridge_str` FLOP/byte**.
...
*   **Utilization Ceiling:**
    `{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` TFLOPS (Actual) / `{python} TensorLifecycleCalc.a100_tflops_fp16` TFLOPS (Peak) ≈ **`{python} Gpt2ThroughputCalc.gpt2_utilization_str`%**
...
Most practitioners never interact directly with ML compilers, yet compiler quality often determines whether a model achieves 20% or 80% of hardware peak performance. Calling `model.compile()` in Keras, `torch.compile()` in PyTorch, or deploying through TensorRT invokes multi-stage optimization pipelines that:
...
Cloud accelerators are often shared, leading to multi-tenant interference. A model might run 20% slower when a neighbor workload competes for memory bandwidth. Production systems need monitoring to detect and respond to this interference.

Sustained workloads may trigger thermal throttling that was not observed in short benchmarks. The A100 SXM operates at `{python} RuntimeProductionTdp.a100_tdp` W TDP while the A100 PCIe operates at 300 W TDP; these represent different form factors with different cooling requirements, not boost versus sustained states. Production performance depends on which variant is deployed and whether thermal limits are approached.
...
Teams deploy high-throughput training accelerators (A100, H100) for latency-sensitive inference with batch size 1–4. As the Roofline Model (@sec-hardware-acceleration-roofline-model-42ff) predicts, small batches severely reduce arithmetic intensity: a dense layer with M=N=2048 achieves AI = `{python} FpSmallBatchCalc.fp_ai_b1_str` FLOP/byte at batch=1 versus AI = `{python} FpSmallBatchCalc.fp_ai_b256_str` FLOP/byte at batch=256. At batch=1, an A100 achieves 4 TFLOPS (1.3% utilization) due to memory bottlenecks, while a lower-cost T4 achieves 3.5 TFLOPS. The T4's peak is `{python} FpSmallBatchCalc.fp_t4_flops_str` TFLOPS (FP16 Tensor Core) with a ridge point of `{python} FpSmallBatchCalc.fp_t4_ridge_str` FLOP/byte (`{python} FpSmallBatchCalc.fp_t4_flops_str` TFLOPS / `{python} FpSmallBatchCalc.fp_t4_bw_str` GB/s). Small-batch inference remains memory-bound on both accelerators, but the T4's lower cost makes it more economical. A100 instances cost 3--4$\times$ more than T4 instances for identical latency. Inference deployments should match batch size to accelerator characteristics, using high-compute accelerators only for batched serving where arithmetic intensity exceeds ridge points.
