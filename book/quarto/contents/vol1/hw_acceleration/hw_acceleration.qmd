...
class Conv2dAnalysisCalc:
    ...
    conv_out_dims_math = MarkdownStr(f"${conv_batch} \\times {conv_cout} \\times {conv_h} \\times {conv_w}$")
    conv_flops_per_out_math = MarkdownStr(f"${conv_cin} \\times {conv_k} \\times {conv_k} \\times 2$")
    conv_input_dims_math = MarkdownStr(f"${conv_batch} \\times {conv_cin} \\times {conv_h} \\times {conv_w} \\times {_fpb}$")
    conv_weights_dims_math = MarkdownStr(f"${conv_cout} \\times {conv_cin} \\times {conv_k} \\times {conv_k} \\times {_fpb}$")
    conv_output_dims_math = MarkdownStr(f"${conv_batch} \\times {conv_cout} \\times {conv_h} \\times {conv_w} \\times {_fpb}$")
```
...
class DenseLayerAnalysisCalc:
    ...
    # Calc-line LHS products pinned to cell constants (inline {python} cannot
    # evaluate inside $...$): each renders the explicit derivation expression.
    dense_matmul_dims_math = MarkdownStr(f"$({dense_batch} \\times {dense_in}) \\times ({dense_in} \\times {dense_out})$")
    dense_total_flops_math = MarkdownStr(f"$2 \\times {dense_batch} \\times {dense_in} \\times {dense_out}$")
    dense_input_dims_math = MarkdownStr(f"${dense_batch} \\times {dense_in} \\times 2$")
    dense_weights_dims_math = MarkdownStr(f"${dense_in} \\times {dense_out} \\times 2$")
    dense_output_dims_math = MarkdownStr(f"${dense_batch} \\times {dense_out} \\times 2$")
```
...
class LayernormAnalysisCalc:
    ...
    # Calc-line LHS products pinned to cell constants (inline {python} cannot
    # evaluate inside $...$).
    ln_elements_dims_math = MarkdownStr(f"${ln_batch} \\times {ln_seq} \\times {ln_hidden}$")
    ln_params_dims_math = MarkdownStr(f"${ln_hidden} \\times 2 \\times {ln_fp_bytes}$")
```
...
- Batch = `{python} BatchAiCalc.batch_small_str`: AI ≈ `{python} BatchAiCalc.batch1_ai_str` FLOP/byte (memory bound)
- Batch = `{python} BatchAiCalc.batch_medium_str`: AI ≈ `{python} BatchAiCalc.batch32_ai_str` FLOP/byte (memory bound)
- Batch = `{python} BatchAiCalc.batch_large_str`: AI ≈ `{python} BatchAiCalc.batch256_ai_str` FLOP/byte (compute bound on A100)
