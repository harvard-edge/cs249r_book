...
To understand why "Structured" patterns are required for hardware speedup, consider how sparse matrices are actually stored in memory. Compare the storage layouts in @fig-sparse-formats: a sparse format (like CSR or Block Sparse\index{Block Sparse!storage format}) must store indices alongside values. If the sparsity is random, the index overhead and irregular access kill performance. Structured sparsity, whether at the *large block* scale or the *fine-grained N:M* scale, makes this indexing predictable and compact, allowing hardware to fetch data efficiently.
...
| **Apple M1**\index{Apple M1!NPU efficiency}     |   128-bit NEON |        $16 \times 16$ FP16 |             8 NPU cores | Mobile inference      |
