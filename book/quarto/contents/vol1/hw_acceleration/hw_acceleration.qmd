...
Hardware acceleration targets specific terms in the **Iron Law of ML Systems** (@principle-iron-law). While data selection reduced the total data and model compression reduced the ops per sample, hardware acceleration increases the rate at which those ops execute by maximizing the Throughput and Bandwidth denominators. Yet acceleration has a hard ceiling, established by the following law.

::: {.callout-notebook title="The Iron Law of Acceleration"}

Hardware acceleration does not speed up the entire system; it only speeds up the parallelizable fraction ($P$). This is governed by **Amdahl's Law for AI** (@principle-amdahls-law):
...
#| echo: false
#| fig-cap: "**The Iron Law Heatmap.** Total system speedup as a function of Accelerator Speed ($S$) and Parallel Fraction ($P$). Notice the 'Accelerator Wall' at the top: if your workload is even slightly serial ($P < 0.9$), increasing hardware speed ($S$) from 100x to 1000x yields almost no benefit. You must be in the yellow/green zone (High $P$) to justify High $S$."
...
The underlying cause of this wall is physical: moving data costs orders of magnitude more energy than processing it. The following chart visualizes the "Horowitz Numbers," the fundamental energy constants of silicon.

```{python}
#| label: fig-energy-hierarchy
#| echo: false
#| fig-cap: "**The Energy Hierarchy.** Energy cost per operation (Log Scale). Fetching data from off-chip DRAM costs ~128x more energy than an SRAM access and ~20,000x more than an INT8 addition. This energy disparity drives the design of every AI accelerator: data must be kept local (in SRAM/Registers) to be processed efficiently."
...
- Bytes: (input + weights + output) $= (32 \times 512 \times 768 + 3 \times 768 \times 768 + 32 \times 512 \times 768 \times 3) \times 2 \approx 104$ MB
- AI $= 29B / 104M = 278$ FLOP/byte, which is **compute-bound on A100** (above 156 threshold)
...
$$\text{AI} = \frac{59.2 \text{ GFLOPs}}{77.7 \text{ MB}} = 761 \text{ FLOP/byte}$$
