...
Hardware acceleration is not merely the substitution of faster silicon; it is the co-design of compute engines, memory hierarchies, numerical formats, and dataflows. For a chosen memory interface, the roofline compares the compute bound $O/(R_{\text{peak}}\eta_{\text{hw}})$ with the movement bound $D_{\text{vol}}/\text{BW}$. End-to-end latency can include additional launch, synchronization, communication, and fixed overheads. Specialized accelerators improve efficiency when the workload uses their supported paths and captures reuse in local storage. Hardware selection therefore begins with the workload's measured or modeled bottleneck rather than one vendor peak.
...
* **Arithmetic intensity guides optimization**: An operation falls on the memory-bound side when its measured intensity is below the target's ridge point and on the compute-bound side when it is above. The ridge point, such as `{python} SummaryRooflineRecap.a100_ridge_str` for the A100 assumptions used here, is specific to the selected precision and memory interface.

:::

An accelerator is easy to mistake for a uniformly faster computer. It is better understood as a machine that provides efficient paths for particular operations and dataflows. Tiling, fusion, hierarchy-aware scheduling, and systolic execution capture reuse and reduce traffic at the modeled memory interface; they help only when the workload, numerical format, and software path can use them.

The Roofline Model provides a disciplined first diagnostic for these trade-offs. Comparing arithmetic intensity with the relevant ridge point narrows the likely bottleneck, after which profiling must account for latency, occupancy, communication, and implementation overhead. The result is not deterministic prediction, but a testable systems hypothesis.
