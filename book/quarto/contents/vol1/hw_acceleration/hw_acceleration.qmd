...
[^fn-von-neumann-bottleneck]: **Von Neumann Bottleneck**: John von Neumann's 1945 reference architecture separated the processing unit from the memory unit, connected by a shared bus. While this simplified programming (code and data are the same), it created a fundamental physical limit: the processor can only work as fast as data moves across that bus. Modern AI accelerators attack this 75-year-old bottleneck by moving memory *into* the compute units (SRAM) or stacking it directly on top (HBM).

::: {.callout-perspective title="The Price of Data Movement"}
...
#| echo: false
#| fig-cap: "**Compute-Bandwidth Divergence.** Compute performance and memory bandwidth plotted on a log scale from 2000 to 2025. Computational capability has grown exponentially while memory bandwidth has improved at a much slower rate, creating a widening gap that constrains AI accelerator efficiency."
