...
In contrast, mobile and edge systems face stringent power, memory, and latency constraints that demand sophisticated hardware-software co-design\index{Hardware-Software Co-design!edge deployment}. Efficient architectures introduced in @sec-network-architectures (such as depthwise separable convolutions and neural architecture search) combined with compression techniques from @sec-model-compression (such as quantization and pruning) enable deployment on devices where the book's reference mobile NPU has about `{python} EdgeReferenceRatios.int8_peak_ratio_mult_str` lower INT8 peak throughput, about `{python} EdgeReferenceRatios.memory_ratio_mult_str` less memory headroom, and about `{python} EdgeReferenceRatios.power_ratio_mult_str` smaller power envelope than an H100-class accelerator. Systems that cannot run on billions of edge devices cannot achieve global impact, making edge deployment essential for AI democratization[^fn-ai-democratization].
...
Generative AI, the frontier that GPT-2/Llama exposed, stresses the same constraints at token-serving scale. Autoregressive generation is inherently memory-bound (each token requires loading the full model weights), making the arithmetic intensity law the governing constraint. Techniques such as dynamic model partitioning, where layers or tensor shards move across devices as memory pressure and request mix change, and speculative decoding[^fn-speculative-decoding-conclusion] reshape the silicon contract by trading compute for latency, demonstrating how the principles adapt even as technologies push infrastructure boundaries.
...
The most ambitious stress test for these invariants lies ahead: engineering the path toward artificial general intelligence[^fn-agi-def]. Where societal benefit applications require robustness within defined domains, AGI demands systems that generalize across all cognitive tasks while maintaining the reliability, efficiency, and safety that these invariants ensure. The governing challenge is systems engineering as much as algorithm design.
