...
from formatting import md_frac, md_sci, md_math, md
...
- **Compute ($O$):** $\approx 2 \times P$ per token = `{python} llama_compute_gflops_str` GFLOPs.
- **Hardware:** H100 ($BW = `{python} h100_bw_tb_str`$ TB/s, $R_{peak} \approx `{python} h100_peak_tflops_str`$ TFLOPs FP16).
...
$T_{mem}$ is `{python} ratio_str`$\times$ larger than $T_{comp}$. The system is heavily **Memory Bound** (**Arithmetic Intensity** $\approx 1$). To honor the **Silicon Contract**, we must either increase **Arithmetic Intensity** (via batching users to reuse $D_{vol}$) or reduce Data Volume (via quantization to INT4). A systems engineer who optimizes compute kernels ($T_{comp}$) without addressing memory ($T_{mem}$) wastes 100% of their effort.
:::
