...
: **The Thirteen Quantitative Invariants of ML Systems Engineering**: Each invariant was introduced in the Part where its governing constraint first becomes visible. Together, they form the complete analytical framework for reasoning about ML system design, optimization, and deployment. The meta-principle that unifies them all is the conservation of complexity: you cannot destroy complexity, only move it between Data, Algorithm, and Machine. {#tbl-twelve-principles tbl-colwidths="[3,18,14,30,35]"}

The thirteen invariants are not independent axioms. They form an integrated framework unified by a single meta-principle: the conservation of complexity[^fn-conservation-complexity]\index{Conservation of Complexity!meta-principle}. Complexity in an ML system cannot be destroyed; it can only be moved between Data, Algorithm, and Machine. Every invariant in @tbl-twelve-principles quantifies a specific consequence of where complexity currently resides. The sections that follow trace how each Part's invariants connect to the Lighthouse models and to each other.
