...
# │ Exports: llama_params_count_str, llama_params_modifier_str, llama_dvol_gb_str,
...
We can apply the iron law (Principle \ref{pri-iron-law}) and Arithmetic Intensity (Principle \ref{pri-arithmetic-intensity}) to a real-world problem: serving one token from a `{python} ConclusionRoofline.llama_params_modifier_str`-parameter model (like a 70-billion-parameter Llama 2 model\index{Llama 2!70-billion-parameter memory-bound workload}) on an NVIDIA H100. @Sec-appdx-machine-foundations-ai-hardware-cheat-sheet-modern-reference-4dc1 supplies the H100 memory bandwidth and peak FLOP specifications that anchor this calculation.
...
The systems integration perspective explains why ethical considerations cannot be separated from technical ones. The same iron law (Principle \ref{pri-iron-law}) that enables efficient systems determines who can access them: a model requiring several high-end data center accelerators for inference excludes organizations that cannot afford that infrastructure. The same data as code invariant (Principle \ref{pri-data-as-code}) that gives models their capabilities also encodes the biases present in training data. The same energy-movement invariant that governs chip-level efficiency scales to data-center-level carbon footprints that affect the planet. Technical decisions are ethical decisions, viewed through a wider lens.
