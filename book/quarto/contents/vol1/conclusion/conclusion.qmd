...
The thirteen invariants are not independent axioms. They form an integrated framework unified by a single meta-principle: the conservation of complexity\index{Conservation of Complexity!meta-principle}[^fn-conservation-complexity]. Complexity in an ML system *cannot* be destroyed; it can only be moved between data, algorithm, and machine. Every invariant in @tbl-thirteen-principles quantifies a specific consequence of where complexity currently resides; conservation of complexity is not a fourteenth invariant but the law the thirteen share. The sections that follow trace how each Part's invariants connect to the Lighthouse models and to each other.
