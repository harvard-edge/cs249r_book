...
Each chapter contributed a piece. But the real lesson is not in any individual piece—it is in *how the pieces constrain each other*. An architecture choice (depthwise separable convolutions in MobileNetV2) enabled a compression choice (INT8 quantization with minimal accuracy loss), which enabled an acceleration choice (mobile NPU deployment), which shaped a serving constraint (P99 < 50 ms), which defined an operational requirement (drift monitoring across heterogeneous device populations). Every decision propagated forward, and the engineer who understands only one layer cannot predict how changes ripple through the rest.

This chapter distills that integrated perspective into a framework for reasoning about ML systems as wholes rather than as collections of parts. We begin by revisiting the Lighthouse Models that traced these constraint interactions across chapters, then formalize twelve quantitative invariants—rooted in physics, information theory, and statistics—that govern ML system behavior regardless of framework, hardware generation, or model family. From there, we examine how these principles apply across three critical domains (technical foundations, engineering at scale, and production reality), explore the future directions where systems thinking will matter most (robust AI, compound systems, and the path toward artificial general intelligence), and close with the engineering responsibility that accompanies the ability to build systems of this power.

The five **Lighthouse Models** introduced in @sec-introduction-lighthouse-archetypes-systems-detectives-a216 made this propagation concrete throughout the book, serving as our systems detectives. Each revealed how different workloads expose different bottlenecks: []{#sec-conclusion-lighthouse-journey-c7ee}
...
### The Next Horizon: The Machine Learning Fleet {#sec-conclusion-next-horizon-machine-learning-fleet-c5f1 .unnumbered}
...
:::: {.callout-chapter-connection title="From Volume I to Volume II"}

The foundations in this volume establish the physics, economics, and systems principles that govern ML at any scale. Volume II extends those principles into advanced architectures, large-scale deployment, and the frontier challenges of robust and responsible AI, where systems thinking becomes not just helpful but indispensable.
