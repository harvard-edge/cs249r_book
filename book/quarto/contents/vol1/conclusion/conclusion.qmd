...
| **Architecture (@sec-network-architectures)** | Algorithmic Efficiency         | **Depthwise Separable Convolutions**: `{python} MobileNetJourneyMath.depthwise_reduction_str`$\times$ fewer FLOPs for a representative 3-by-3, 256-output-channel layer and `{python} MobileNetJourneyMath.resnet_ops_reduction_str`$\times$ fewer operations than ResNet-50 at ImageNet scale |
...
| **Compression (@sec-model-compression)**      | Navigating the Pareto Frontier | **INT8 Quantization**: `{python} MobileNetJourneyMath.int8_fp32_reduction_str`$\times$ memory reduction versus FP32 (`{python} MobileNetJourneyMath.int8_fp16_reduction_str`$\times$ versus FP16), with accuracy revalidated per deployment                                                    |
...
class EnergyMovementMath:
    ...
    dram_vs_fp32_str = fmt_multiple(dram_vs_fp32_display, precision=0, commas=False)
    dram_vs_fp16_str = fmt_multiple(dram_vs_fp16_display, precision=0, commas=False)
    dram_vs_range_str = fmt_multiple_range(
...
| \ref{pri-energy-movement}       | Energy-Movement Invariant   | III: Optimize  | $E_{\text{move}} \gg E_{\text{compute}}$ (`{python} EnergyMovementMath.dram_vs_range_str`$\times$ for DRAM access vs. FP32/FP16 FLOPs) | Data locality, not raw FLOP/s, drives efficiency                               |
...
class TailLatencyRatio:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    conclusion_tail_ratio_str = fmt_multiple(ratio, precision=0, commas=False)
```
