...
# Llama-2-70B
llama_params = 70e9
llama_dvol = llama_params * BYTES_FP16
llama_compute_per_token = 2 * llama_params

# H100 Specs
h100_bw = H100_MEM_BW
h100_peak = H100_FLOPS_FP16_TENSOR

# Calculations
t_mem = (llama_dvol / h100_bw).to('ms')
t_comp = (llama_compute_per_token / h100_peak).to('ms')
ratio = t_mem / t_comp

# Formatting for text
llama_params_str = "70B"
llama_dvol_gb_str = f"{llama_dvol.to('GB').magnitude:.0f}"
llama_compute_gflops_str = f"{(llama_compute_per_token * flop).to(GFLOPs).magnitude:.0f}"

h100_bw_tb_str = f"{h100_bw.to('TB/s').magnitude:.2f}"
h100_peak_tflops_str = f"{h100_peak.to('TFLOPs/s').magnitude:.0f}"

t_mem_ms_str = f"{t_mem.magnitude:.1f}"
t_comp_ms_str = f"{t_comp.magnitude:.2f}"
ratio_str = f"{ratio.magnitude:.0f}"

# Math strings
# Note: 3.35 TB/s is 3350 GB/s.
...
- **Hardware:** H100 with $BW$ = `{python} h100_bw_tb_str` TB/s, $R_{peak}$ ≈ `{python} h100_peak_tflops_str` TFLOPS FP16.
...
The memory time $T_{mem}$ is `{python} ratio_str`× larger than compute time $T_{comp}$. The system is heavily **Memory Bound** (**Arithmetic Intensity** ≈ 1). To honor the **Silicon Contract**, we must either increase **Arithmetic Intensity** (via batching users to reuse $D_{vol}$) or reduce Data Volume (via quantization to INT4). A systems engineer who optimizes compute kernels ($T_{comp}$) without addressing memory ($T_{mem}$) wastes 100% of their effort.
:::
...
conclusion_mean_latency_ms_value = 50
conclusion_p99_latency_ms_value = 2000
conclusion_tail_ratio_value = (
    conclusion_p99_latency_ms_value / conclusion_mean_latency_ms_value
)
conclusion_tail_ratio_str = fmt(conclusion_tail_ratio_value, precision=0, commas=False)
```
...
The robustness challenge connects directly to the **Verification Gap** (Invariant 9) and **Statistical Drift Invariant** (Invariant 10) established earlier—the deployment invariants that guarantee some failures reach production and that systems degrade over time, making continuous monitoring essential rather than optional.
