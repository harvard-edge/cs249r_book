...
::: {.callout-tip title="Lighthouse Examples: Quantitative Reference" collapse="true"}
...
The physical laws of the universe—specifically the speed of light, the thermodynamics of power, and the signal integrity of memory—dictate that we cannot have a single "ideal" computer. Instead, we must navigate a **Distributed Intelligence Spectrum**. Where a system runs does not just change its performance; it fundamentally alters the "Silicon Contract" between the model and the hardware.

This environment-driven necessity creates four distinct engineering paradigms: **cloud machine learning**, which maximizes computational capabilities while accepting network-induced latency constraints; **edge computing**, which positions inference proximate to data sources when latency requirements preclude cloud-based processing; **mobile machine learning**, which extends computational capabilities to personal devices where user proximity and offline operation represent critical requirements; and **tiny machine learning**, which enables distributed intelligence on severely resource-constrained devices where energy efficiency supersedes computational sophistication.

Modern deployment strategies transcend traditional dichotomies between centralized and distributed processing. Contemporary applications implement hybrid architectures that allocate computational tasks across multiple paradigms to optimize system-wide performance. Voice recognition systems exemplify this sophistication: wake-word detection operates on ultra-low-power embedded processors for continuous monitoring, speech-to-text conversion uses mobile processors to maintain privacy and minimize latency, while semantic understanding uses cloud infrastructure for complex natural language processing. Optimal machine learning systems require this architectural heterogeneity.

A systems engineering perspective is necessary for designing machine learning architectures that balance algorithmic capabilities with operational constraints. This approach provides methodological foundations for translating theoretical machine learning advances into production systems with reliable performance at scale. Paradigm integration strategies for hybrid architectures and core design principles govern all machine learning deployment contexts.
...
::: {.callout-tip title="Latency Numbers for ML System Design" collapse="true"}
These order-of-magnitude latencies should inform every deployment decision. Detailed hardware latencies are covered in @sec-ai-acceleration.
...
**Decision rule**: If your latency budget is $X$ ms, you cannot use any operation with latency $> X$ in your critical path.
:::
...
Representative hardware platforms spanning this spectrum illustrate how these physical constraints shape real-world system design. @tbl-representative-systems quantifies these differences, demonstrating the range of computational resources, power requirements, and cost considerations[^fn-cost-spectrum] across the ML systems spectrum.[^fn-pue]
...
::: {.callout-chapter-connection title="Where to Go Deeper"}
This chapter establishes *where* ML systems run. Subsequent chapters provide the *how*:

| Concept Introduced Here | Detailed Treatment | What You'll Learn |
|------------------------|-------------------|-------------------|
| Workload Archetypes (Compute Beast, Bandwidth Hog, etc.) | @sec-ai-acceleration | Roofline analysis to diagnose whether your workload is compute-bound or memory-bound |
| Quantization, pruning for deployment | @sec-model-compression | INT8/INT4 quantization achieving 99% accuracy with 4× memory reduction |
| Federated learning pattern | @sec-privacy-security | FedAvg algorithm, differential privacy, secure aggregation |
| TCO analysis and cost models | @sec-benchmarking-ai | MLPerf methodology for reproducible performance claims |
| Data pipeline requirements | @sec-data-engineering | ETL patterns, data versioning, feature stores |
| Hybrid serving architectures | @sec-serving | Batching strategies, model routing, A/B testing |
| Operational monitoring | @sec-ml-operations | Drift detection, retraining triggers, incident response |
:::
