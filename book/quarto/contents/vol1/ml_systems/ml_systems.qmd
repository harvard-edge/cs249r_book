...
# │          gpt2_params_b_str, llama_range_str, MLSystemsSetup.dlrm_embedding_str,
# │          MLSystemsSetup.mobilenet_flops_reduction_str, MLSystemsSetup.mobile_tdp_range_str,
# │          MLSystemsSetup.kws_params_str, MLSystemsSetup.kws_size_kb_str
# │
# │ Note: MLSystemsSetup.mobile_tdp_range_str and MLSystemsSetup.mobilenet_flops_reduction_str are also
# │       consumed at @sec-ml-systems-mobile-ml-personal-offline-intelligence-0983
...
| **Mobile ML** | Flagship Smartphone |                    Mobile SoC (CPU + GPU + NPU) | `{python} MLSystemsSetup.mobile_ram_range_str` GB RAM | `{python} MLSystemsSetup.mobile_storage_range_str`   |                         `{python} LighthouseModels.mobile_tdp_range_str` W | USD 999+                                                       |
...
| **Mobile ML** | `{python} MLSystemsSetup.mobile_npu_range_str` TOPS         | `{python} MLSystemsSetup.mobile_bw_range_str` GB/s  | <2 W                              | <`{python} MLSystemsSetup.mobile_latency_range_str` ms |
...
Mobile devices exemplify intermediate constraints: `{python} MLSystemsSetup.mobile_ram_range_str` GB RAM (varying from mid-range to flagship), `{python} MLSystemsSetup.mobile_storage_range_str` storage, `{python} MLSystemsSetup.mobile_npu_range_str` TOPS AI compute through Neural Processing Units[^fn-npu-energy-efficiency] consuming `{python} LighthouseModels.mobile_tdp_range_str` W power. System-on-Chip architectures[^fn-soc-integration] integrate computation and memory to minimize energy costs. Memory bandwidth of `{python} MLSystemsSetup.mobile_bw_range_str` GB/s limits models to 10–100 MB parameters, requiring the aggressive optimization techniques that @sec-model-compression details. Battery constraints (`{python} MLSystemsSetup.phone_battery_str`–22 Wh capacity) make energy optimization critical: 1 W continuous ML processing reduces device lifetime from 24 to 18 hours. Specialized frameworks provide hardware-optimized inference enabling <`{python} MLSystemsSetup.mobile_latency_range_str` ms UI response times.
...
Compression techniques do not scale indefinitely against physics. Consider a smartphone with a `{python} MLSystemsSetup.phone_battery_str` Wh battery:
