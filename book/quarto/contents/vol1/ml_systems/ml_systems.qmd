...
class MLSystemsNvlinkAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_bw_str = fmt_qty_int(Hardware.Cloud.H100.nvlink.bandwidth, GB/second, commas=False)
```
...
from mlsysim.fmt import fmt, check, fmt_time, MarkdownStr
...
class LightLatency:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    min_latency_str = MarkdownStr(
        f"{fmt_time(min_latency_ms, 'millisecond', precision=0, commas=False)} round-trip"
    )
...
class EnergyTransmission:
    ...
    compute_energy_str = fmt_qty(local_energy_op, ureg.millijoule, precision=1, commas=False, per="inference")
...
from mlsysim.fmt import fmt_int, fmt, fmt_qty, fmt_qty_int, check, MarkdownStr, fmt_count
...
class LighthouseModels:
    ...
    resnet_fp32_mb = resnet_fp32_size.m_as(MB)
    gpt2_params_b = m_gpt2.parameters.m_as(Bparam)
    dlrm_embedding = m_dlrm.size_in_bytes()
    dlrm_embedding_gb = dlrm_embedding.m_as(GB)
    ...
    dlrm_embedding_str = fmt_qty_int(dlrm_embedding, GB)
...
class HardwareSpectrumSetup:
    ...
    cloud_pwr_mw_str = fmt_qty(Systems.Pods.TPUv4_4096.power, ureg.megawatt, precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.fmt import fmt, fmt_count, fmt_usd, check
...
class DistancePenalty:
    ...
    distance_km = distance_km_value * ureg.km
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sol_kms_str = fmt_qty(SPEED_OF_LIGHT_FIBER_KM_S, ureg.kilometer/ureg.second, precision=0, commas=True)
    rtt_formatted_str = fmt_time(round_trip_ms_value, 'millisecond', precision=0, commas=False)
    deficit_str = fmt_time(deficit_ms_value, 'millisecond', precision=0, commas=False, allow_negative=True)
    distance_km_str = fmt_qty(distance_km, ureg.km, precision=0, commas=True)
    safety_budget_str = fmt_time(safety_budget_ms_value, 'millisecond', precision=0, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware
from mlsysim.fmt import fmt_percent, fmt_int, fmt_usd, fmt, check, MarkdownStr, fmt_count, fmt_qty
...
class CloudEdgeTCO:
    ...
    inference_str = fmt_time(inference_ms, 'millisecond', precision=0, commas=False)
    response_str = fmt_qty(response_kb * KB, KB, precision=0, commas=False)
    ...
    egress_gb_str = fmt_qty(egress_gb_per_day * GB, GB, precision=0, commas=False, per="day")
    ...
    power_str = fmt_qty(server.tdp, watt, precision=0, commas=False)
...
class VoiceAssistantWall:
    ...
    vi_audio_kb_per_sec = vi_audio_rate.m_as(KB/second)
    ...
    vi_total_audio_tb_per_sec = vi_total_audio_rate.m_as(TB/second)
    ...
    vi_audio_kb_str = fmt_qty(vi_audio_rate, KB/second, precision=0, commas=False)
    vi_audio_tb_str = fmt_qty(vi_total_audio_rate, TB/second, precision=0, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, MarkdownStr, fmt_qty
...
class DataLocalityInvariant:
    ...
    tx_time_ms_str = fmt_time(tx_time_ms, 'millisecond', precision=1, commas=True)
    remote_ms_str = fmt_time(remote_total_ms, 'millisecond', precision=0)
    net_bw_str = fmt_qty(net_bw_mbps * ureg.megabit / second, ureg.megabit / second, precision=0, commas=False, unit_label="Mb/s")
...
class EdgeSizingFleetTCO:
    ...
    coral_peak_tops = coral_peak.m_as(TOPS)
    coral_sustained_tops = coral_sustained.m_as(TOPS)
    ...
    coral_cluster_peak_tops = coral_cluster_peak.m_as(TOPS)
    coral_cluster_sustained_tops = coral_cluster_sustained.m_as(TOPS)
    ...
    coral_cluster_peak_tops_str = fmt_qty(coral_cluster_peak, TOPS, precision=0, commas=False, unit_label="TOPS peak")
    coral_cluster_sustained_tops_str = fmt_qty(coral_cluster_sustained, TOPS, precision=0, commas=False, unit_label="TOPS derated")
    ...
    stores_str = fmt(stores, precision=0, commas=False)
    hours_per_year_str = fmt_time(HOURS_PER_YEAR, 'hour', precision=0, commas=False)
...
class BatteryTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    runtime_str = fmt_time(runtime_hours, "hour", precision=1, commas=False, style="word")
...
from mlsysim.fmt import fmt_percent, fmt, check, fmt_rate, fmt_qty
...
class ThermalQuantCalc:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    baseline_power = 12 * watt                                                       # W, unoptimized LLM power
    baseline_power_w_value = baseline_power.m_as(watt)
    quant_reduction_value = 4                                                        # FP32→INT8 power reduction
    mobile_tdp = Scenarios.MobilePower.MobileNpuTypical
    mobile_tdp_w_value = mobile_tdp.m_as(watt)
    temp_rise_c_per_s_value = 1
    temp_rise = temp_rise_c_per_s_value * ureg.delta_degC / second
    trip_time_s_value = 60
    trip_temp_c_value = 80
    trip_temp = ureg.Quantity(trip_temp_c_value, ureg.degC)
    fps_before_value = 100
    fps_after_value = 30

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    quant_power = baseline_power/quant_reduction_value                             # 12 W / 4 = 3 W
    quant_power_w_value = quant_power.m_as(watt)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    baseline_str = fmt_qty(baseline_power, watt, precision=0, commas=False)
    mobile_tdp_str = fmt_qty(mobile_tdp, watt, precision=0, commas=False)
    quant_power_str = fmt_qty(quant_power, watt, precision=0, commas=False)
    quant_red_str = fmt_multiple(quant_reduction_value, precision=0, commas=False)
    temp_rise_str = fmt_qty(temp_rise, ureg.delta_degC/second, precision=0, commas=False, unit_label="°C/s")
    trip_time_str = fmt_time(trip_time_s_value, 'second', precision=0, commas=False)
    trip_temp_str = fmt_qty(trip_temp, ureg.degC, precision=0, commas=False, unit_label="°C")
    fps_before_str = fmt_rate(fps_before_value, 'FPS', precision=0, commas=False)
    fps_after_str = fmt_rate(fps_after_value, 'FPS', precision=0, commas=False)
```
...
class MobileBatteryCapacity:
    ...
    phone_battery = h_phone.battery_capacity if h_phone.battery_capacity else 15 * ureg.Wh
    phone_battery_wh = phone_battery.m_as('Wh')
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    phone_battery_str = fmt_qty(phone_battery, ureg.Wh, precision=0, commas=True)
    added_ml_runtime_low_str = fmt_time(added_ml_runtime_low_h, "hour", precision=1, commas=False, style="word")
    added_ml_runtime_high_str = fmt_time(added_ml_runtime_high_h, "hour", precision=1, commas=False, style="word")
```
...
class EnergyInference:
    ...
    e_resnet_cloud_str = fmt_qty(e_resnet_cloud_mj * ureg.mJ, ureg.mJ, precision=1)
    e_resnet_edge_str = fmt_qty(e_resnet_edge_mj * ureg.mJ, ureg.mJ, precision=1)
    e_mobilenet_str = fmt_qty(e_mobilenet_mj * ureg.mJ, ureg.mJ, precision=2)
    e_kws_str = fmt_qty(e_kws_uj * ureg.microjoule, ureg.microjoule, precision=0, commas=False)
    ...
    batt_cap_mah_str = fmt_qty(Scenarios.PhoneBattery.CapacityMah, ureg.mAh, precision=0, commas=False)
    batt_volt_str = fmt_qty(Scenarios.PhoneBattery.VoltageV, ureg.V, precision=1, commas=False)
```
...
class WildlifeMonitoring:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    raw_audio_gb_str = fmt_qty(raw_audio_per_day, GB, precision=1, commas=False, unit_label="GB per day")
    summary_kb_str = fmt_qty(summary_per_day, KB, precision=0, commas=False, unit_label="KB of detection summaries")
...
class BrakingDistance:
    ...
    latency_str = fmt_time(latency_ms, 'millisecond', precision=0, commas=False)
```
...
from mlsysim import Hardware
from mlsysim.fmt import fmt_percent, fmt, check, fmt_frac, fmt_qty
...
class MobilePowerFallacyCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    h_phone = Hardware.Mobile.iPhone15Pro
    battery = h_phone.battery_capacity if h_phone.battery_capacity else 15 * ureg.Wh
    battery_wh_value = battery.m_as('Wh')
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    phone_battery_str = fmt_qty(battery, ureg.Wh, precision=0, commas=False)
    low_power_w_str = fmt_qty(low_power_w_value * watt, watt, precision=0, commas=False)
    high_power_w_str = fmt_qty(high_power_w_value * watt, watt, precision=0, commas=False)
...
class AmdahlCameraCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cam_isp_str = fmt_time(cam_isp_ms_value, 'millisecond', precision=0, commas=False)
    cam_ml_str = fmt_time(cam_ml_ms_value, 'millisecond', precision=0, commas=False)
    cam_post_str = fmt_time(cam_post_ms_value, 'millisecond', precision=0, commas=False)
    cam_total_str = fmt_time(cam_total_ms_value, 'millisecond', precision=0, commas=False)
    ...
    cam_ml_opt_str = fmt_time(cam_ml_optimized_ms_value, 'millisecond', precision=0, commas=False)
    cam_total_opt_str = fmt_time(cam_total_optimized_ms_value, 'millisecond', precision=0, commas=False)
