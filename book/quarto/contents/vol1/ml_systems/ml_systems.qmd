...
Position on the spectrum is only the qualitative picture. @Tbl-deployment-paradigms-overview pins down the quantitative trade-offs that separate the envelopes:
...
The eight-order span of energy per inference\index{Energy per Inference!paradigm comparison}, shown visually in @fig-energy-per-inference, becomes concrete in battery terms here: @tbl-ml-systems-energy-per-inference reports representative full-system energy per inference and the resulting smartphone-battery query counts:
