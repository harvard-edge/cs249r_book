...
Throughout this book, we use five **Lighthouse Archetypes** introduced in @sec-introduction—concrete workloads that span the deployment spectrum and isolate distinct system bottlenecks. @sec-dnn-architectures provides full architectural details and model biographies.
...
To ground the abstract interdependencies of the Iron Law in concrete practice, we analyze the **Lighthouse Archetypes** introduced in @sec-introduction. The following summaries recap each workload from a systems perspective, connecting them to the specific Iron Law bottlenecks they exemplify.
...
**Given** (from Lighthouse Archetypes):
...
resnet_flops_latex = sci_latex(RESNET50_FLOPs)
a100_flops_latex = sci_latex(A100_FLOPS_FP16_TENSOR)
resnet_fp16_bytes_latex = sci_latex(resnet_fp16_bytes)
a100_bw_latex = sci_latex(A100_MEM_BW)
...
# Pre-compute sci_latex() for use inside LaTeX fractions
mobile_npu_flops_latex = sci_latex(MOBILE_NPU_TOPS_INT8)
resnet_int8_bytes_latex = sci_latex(resnet_int8_bytes)
mobile_npu_bw_latex = sci_latex(MOBILE_NPU_MEM_BW)
