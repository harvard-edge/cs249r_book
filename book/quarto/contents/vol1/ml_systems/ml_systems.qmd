...
+:===========================================+:==========================+==================:+======================:+=====================:+:===============================+
| ****Image Classifier** (e.g., ResNet-50)** | Compute Beast             | ~25M              | ~4 GFLOPs             | ~100 MB              | Cloud training, edge inference |
| ****Language Model** (e.g., GPT-2 scale)** | Bandwidth Hog             | ~1.5B             | ~3 TFLOPs             | ~3 GB                | Cloud inference                |
| ****Efficient Vision** (e.g., MobileNet)** | Compute Beast (efficient) | ~3M               | ~300 MFLOPs           | ~15 MB               | Mobile, edge                   |
| ****Recommendation System** (e.g., DLRM)** | Sparse Scatter            | 10B+ (embeddings) | Variable              | 100+ GB              | Cloud only                     |
| ****Keyword Spotter** (e.g., DS-CNN)**     | Tiny Constraint           | ~250K             | ~20 MOPs              | ~500 KB              | TinyML, always-on              |
+--------------------------------------------+---------------------------+-------------------+-----------------------+----------------------+--------------------------------+

**Key insight**: The 10,000× range in compute requirements (20 MOPs → 3 TFLOPs) and 200,000× range in memory (500 KB → 100 GB) explain why no single deployment paradigm fits all workloads. A keyword spotter runs comfortably on a $2 microcontroller; a recommendation system requires a warehouse-scale computer.
:::
