...
To navigate these operating regimes, we anchor our engineering decisions in a four-layer architectural model of the **Single-Node Stack**\index{Single-Node Stack!architecture layers}. Relative to the D·A·M taxonomy, this stack refines the Machine side for one host while Data and Algorithm still enter through workload requirements. This model provides the foundational framework for analyzing any ML system before it is projected onto a larger distributed fleet. Understanding how these layers interact within a single machine is the technical prerequisite for mastering larger scales.

1.  **Application (The Mission)**: The top layer where high-level requirements, such as training throughput or inference latency, are defined. At this layer we name the goal; @sec-model-training and @sec-model-serving work out the mechanisms that meet it.
2.  **ML Framework (The Compiler)**\index{ML Framework!compiler role}: The translation layer that maps high-level model code to executable operations; @sec-ml-frameworks examines how frameworks represent those operations and manage memory.
...
4.  **Hardware (The Silicon)**: The physical foundation where bits are transformed. This layer is defined by high-bandwidth memory (HBM) capacity, memory bandwidth, high-speed intra-node interconnects such as NVLink, and compute throughput. Here, the memory wall acts as the primary physical constraint (@sec-hardware-acceleration).
...
Energy per inference\index{Energy per Inference!paradigm comparison} spans eight orders of magnitude across deployment paradigms. @Tbl-ml-systems-energy-per-inference reports representative full-system energy per inference and the resulting smartphone-battery query counts:
