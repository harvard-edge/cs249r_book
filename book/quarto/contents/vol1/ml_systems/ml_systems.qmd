...
**Problem**: Should a battery-powered sensor process data locally (TinyML) or send it to the cloud when the energy of transmission\index{Energy of Transmission!local vs. cloud} collides with the battery-driven energy wall\index{Energy Wall!battery constraints}?
...
*   **Compute energy $(E_{\text{op}})$**: `{python} EnergyTransmission.compute_energy_mj_str` (MobileNetV2 on a neural processing unit, or NPU).
...
Physical constraints translate latency-vs-throughput trade-offs\index{Latency vs. Throughput!trade-offs} into engineering decisions through concrete numbers. @Tbl-latency-numbers provides order-of-magnitude latencies\index{Latency!decision thresholds} that should inform every deployment decision—spanning eight orders of magnitude from nanosecond compute operations to hundreds of milliseconds for cross-region network calls. Detailed hardware latencies and bandwidth constraints are covered in @sec-hardware-acceleration. The key decision rule is simple: an operation with latency $> X$ *cannot* appear on the critical path of a system whose latency budget is $X$ ms.[^fn-critical-path-ml]
...
The pipelined form of the iron law of ML systems from @sec-introduction-iron-law-ml-systems-c32a states that execution time is bounded by the dominant system bottleneck\index{System Bottleneck!dominant constraints}, as @eq-iron-law-extended formalizes:
...
# │       TPU v4 Pod (Cloud), DGX Spark (Edge), and ESP32-S3 (TinyML).
...
|:--------------|:-------------------:|:----------------------------------------------------------------------------------------------------------------:|:--------------------------------------------------------------:|:------------------------------------------------------------------:|:--------------------------------------------------------------------------------------------------------------------------:|:--------------------------------------------------------------------------------------------------------:|
| **Cloud ML**  |  Google TPU v4 Pod  | `{python} HardwareSpectrumSetup.tpu_chip_count_str`, `{python} HardwareSpectrumSetup.cloud_peak_eflop_per_s_str` |     `{python} HardwareSpectrumSetup.cloud_mem_tb_str` HBM2     |                          Cloud-scale (PB)                          |                                     ~`{python} HardwareSpectrumSetup.cloud_pwr_mw_str`                                     |                                          Cloud service (rental)                                          |
| **Edge ML**   |   NVIDIA DGX Spark  |               GB10 Grace Blackwell, `{python} HardwareSpectrumSetup.edge_peak_pflop_per_s_str` AI                |    `{python} HardwareSpectrumSetup.edge_mem_gb_str` LPDDR5x    |       `{python} HardwareSpectrumSetup.edge_stor_tb_str` NVMe       |                                      ~`{python} HardwareSpectrumSetup.edge_pwr_w_str`                                      | ~`{python} HardwareSpectrumSetup.edge_price_min_str`–`{python} HardwareSpectrumSetup.edge_price_max_str` |
...
| **TinyML**    |       ESP32-S3      |                                               Dual-core @ 240 MHz                                                |                           520 KB RAM                           |      `{python} HardwareSpectrumSetup.tiny_flash_mb_str` Flash      | `{python} HardwareSpectrumSetup.tiny_pwr_min_w_str`–`{python} HardwareSpectrumSetup.tiny_pwr_max_w_str` active board power |                           `{python} HardwareSpectrumSetup.tiny_price_usd_str`                            |
...
[^fn-tco-deployment]: [offset=65mm] **Total Cost of Ownership (TCO)**: Quantifies the gap between sticker price and true system cost by including all direct and indirect costs (power, cooling, labor) over a system's lifetime, trading upfront capital expense (CapEx) against recurring operational expense (OpEx). For an on-premise GPU, the purchase price is often only 30–40 percent of the three-year TCO, the rest dominated by operating costs. \index{Total Cost of Ownership!deployment economics}
...
::: {#fig-energy-per-inference fig-env="figure" fig-pos="htb" fig-cap="**Illustrative Energy per Inference Anchors**: Order-of-magnitude scenario anchors span eight orders of magnitude, from ~10 µJ for TinyML keyword spotting to ~1 kJ for a cloud LLM query. These are pedagogical deployment anchors rather than measurements produced by the analytical node model used later in @tbl-ml-systems-energy-per-inference." fig-alt="Horizontal log-scale bar chart showing illustrative energy anchors for five workloads across four deployment paradigms. TinyML keyword spotting is 10 microjoules, Mobile MobileNetV2 is 50 millijoules, Edge ResNet-50 is 500 millijoules, Cloud ResNet-50 is 10 joules, and a Cloud GPT-4 query is 1 kilojoule."}
...
Choosing where a system runs settles its physics; it does not protect it from time. Zillow's \$304 million write-down was not a failure of model accuracy but of systems reasoning: no process traced how a drifting market propagated through the valuation model into irreversible commitments. @Sec-ml-workflow establishes that process, the systematic development discipline that carries an ML system from conception through deployment and is built to catch exactly this class of failure before it compounds.
