...
[^fn-tinyml-energy-gap]: **TinyML Energy Gap**: This differential is rooted in hardware design philosophy; cloud GPUs are optimized for raw throughput, consuming hundreds of watts, while TinyML microcontrollers are designed for near-zero power sleep states. This architectural trade-off means a single cloud inference consumes ~1 Joule, whereas a specialized TinyML device uses less than one microjoule—the $1{,}000{,}000\times$ gap that mandates different system designs for battery-powered operation. \index{TinyML!energy efficiency}
...
**Key insight**: Latency constraints eliminated 75 percent of options before we considered compute or cost.
