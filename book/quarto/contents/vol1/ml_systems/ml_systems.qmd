...
This convergence explains why techniques transfer between scales when they attack a shared bottleneck. Cloud-trained models deploy to edge because both training and inference minimize the same loss function, and only the compute budget differs. Lower-precision representations developed for edge deployment reduce cloud serving costs; later chapters formalize these methods as quantization. Strategies for splitting training work across devices likewise inform edge model parallelism.
