...
from mlsysim.core.constants import (
    ...
    VIDEO_FPS_STANDARD, CLOUD_EGRESS_PER_GB, MB, GB, KB, second, MILLION, USD,
)
...
# │          EdgeSizing.coral_power_w_str, EdgeSizing.coral_tco_str,
...
**TCO over `{python} EdgeSizing.years_str` years** (Coral): Hardware USD `{python} EdgeSizing.coral_fleet_str` + Power (`{python} EdgeSizing.coral_power_w_str` W/1000 $\times$ `{python} EdgeSizing.stores_str` $\times$ `{python} EdgeSizing.hours_per_year_str` h$\times$ `{python} EdgeSizing.years_str` yr$\times$ USD `{python} EdgeSizing.elec_cost_str`/kWh = USD `{python} EdgeSizing.coral_power_opex_str`) = **USD `{python} EdgeSizing.coral_tco_str` total** vs. cloud inference at ~USD `{python} EdgeSizing.cloud_cost_str`.
