...
class HardwareSpectrumSetup:
    ...
    cloud_compute_envelope_str = MarkdownStr(
        f"{tpu_chip_count_str}, "
        f"{fmt_flop_rate(cloud_peak_flops, unit=EFLOP / second, precision=1, commas=False)}"
    )
    edge_compute_envelope_str = MarkdownStr(
        f"GB10 Grace Blackwell, "
        f"{fmt_flop_rate(edge_peak_ai_flops, unit=PFLOP / second, precision=0, commas=False)} AI"
    )
```
...
class DeploymentThresholdsTable:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_thresh_tflop_per_s_str = MarkdownStr(
        f">{fmt_flop_rate(cloud_compute_threshold, unit=TFLOPs / second, precision=0, commas=False)}"
    )
    cloud_thresh_bw_gb_per_s_str = MarkdownStr(
        f">{fmt_bandwidth(cloud_bandwidth_threshold, unit=GB / second, precision=0, commas=False)}"
    )
    cloud_latency_range_str = MarkdownStr(Platforms.Cloud.latency_range_ms)
    edge_thresh_pflop_per_s_str = MarkdownStr(
        f"~{fmt_flop_rate(edge_compute_threshold, unit=PFLOP / second, precision=0, commas=False)}"
    )
    edge_thresh_bw_gb_per_s_str = MarkdownStr(
        f">{fmt_bandwidth(edge_bandwidth_threshold, unit=GB / second, precision=0, commas=False)}"
    )
    edge_latency_range_str = MarkdownStr(Platforms.Edge.latency_range_ms)
    ...
    mobile_tdp_range_str = MarkdownStr(f"{Platforms.Mobile.tdp_range_w} W")
    mobile_latency_range_str = MarkdownStr(Platforms.Mobile.latency_range_ms)
    tiny_thresh_tops_str = MarkdownStr(
        f"<{fmt_ops_rate(tiny_compute_threshold, unit=TOPS, precision=0, commas=False)}"
    )
    tiny_thresh_mw_str = MarkdownStr(
        f"<{fmt_power(tiny_power_threshold, unit=milliwatt, precision=0, commas=False)}"
    )
    tiny_latency_range_str = MarkdownStr(Platforms.Tiny.latency_range_ms)
```
