...
These hardware differences translate directly into performance bottlenecks. To understand which constraint dominates in each paradigm, we apply the **Bottleneck Principle** (@sec-ml-systems-bottleneck-principle) across deployment contexts.

::: {.callout-perspective title="System Balance Across Paradigms"}

\index{Iron Law of ML Systems!paradigm comparison} \index{system bottlenecks!dominant constraints}The **Bottleneck Principle** states that pipelined execution time is bounded by the slowest resource. Here we extend it with a third bottleneck: **I/O bandwidth** ($BW_{IO}$), which captures storage and network throughput as distinct from memory bandwidth ($BW$). This distinction matters because cloud and edge systems often hit I/O limits (disk, PCIe, or network) before saturating memory bandwidth:
...
TinyML brings intelligence to the smallest devices, from microcontrollers[^fn-microcontrollers-specs] to embedded sensors, enabling real-time computation in severely resource-constrained environments [@banbury2021mlperftiny; @lin2020mcunet]. This paradigm excels in applications requiring ubiquitous sensing, autonomous operation, and maximal energy efficiency. TinyML systems power applications such as predictive maintenance, environmental monitoring, and simple gesture recognition while optimized for energy efficiency[^fn-energy-efficiency], often running for months or years on limited power sources such as coin-cell batteries[^fn-coin-cell]—the remarkably small device kits in @fig-TinyML-example convey just how compact this hardware can be. These systems deliver actionable insights in remote or disconnected environments where power, connectivity, and maintenance access are impractical.
