...
class DistancePenalty:
    ...
    rtt_formatted_str = fmt_time(round_trip, millisecond, precision=0, commas=False)
    deficit_str = fmt_time(deficit, millisecond, precision=0, commas=False, allow_negative=True)
    distance_km_str = fmt_qty(distance, kilometer, precision=0, commas=True)
    safety_budget_str = fmt_time(safety_budget, millisecond, precision=0, commas=False)
```
...
class CloudEdgeTCO:
    ...
    response_str = fmt_qty(response_size, KB, precision=0, commas=False)
    ...
    electricity_str = fmt_usd(electricity_rate, precision=2, commas=False, per="kWh")
...
class EdgeSizingThroughput:
    ...
    sustained_gflops_value = sustained_flop_rate.to(GFLOPs / second).magnitude
    ...
    sustained_gf_str = fmt(sustained_gflops_value, precision=0)
...
class ThermalQuantCalc:
    ...
    quant_red_str = fmt_multiple(quant_reduction_factor, precision=0, commas=False)
    temp_rise_str = fmt_qty(temp_rise, ureg.delta_degC / second, precision=0, commas=False, unit_label="°C/s")
    trip_time_str = fmt_time(trip_time, second, precision=0, commas=False)
...
from mlsysim import Hardware
from mlsysim.fmt import fmt_percent, fmt, check, fmt_frac, fmt_qty, fmt_multiple
...
class MobilePowerFallacyCalc:
    ...
    low_power_hours_str = fmt(low_power_runtime, hour, precision=0, commas=False)
    high_power_hours_str = fmt(high_power_runtime, hour, precision=0, commas=False)
    mobile_tdp_str = fmt_qty(mobile_tdp, watt, precision=0, commas=False)
    quant_red_str = fmt_multiple(quant_reduction_factor, precision=0, commas=False)
...
class TcoPitfallCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_compute_str = fmt_usd(cloud_compute, precision=0, commas=True, per="month")
    edge_hw_str = fmt_usd(edge_hardware, precision=0, commas=False, per="month")
    edge_network_str = fmt_usd(edge_network, precision=0, commas=True, per="month")
    edge_maint_str = fmt_usd(edge_maintenance, precision=0, commas=False, per="month")
    edge_reliability_str = fmt_usd(edge_reliability, precision=0, commas=True, per="month")
    edge_total_str = fmt_usd(edge_total, precision=0, commas=True, per="month")
    tco_ratio_str = fmt_multiple(tco_ratio, precision=0, commas=False)
```
...
from mlsysim.fmt import fmt_int,  fmt_percent, fmt, check, fmt_time
...
class AmdahlCameraCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cam_isp_str = fmt_time(cam_isp, millisecond, precision=0, commas=False)
    cam_ml_str = fmt_time(cam_ml, millisecond, precision=0, commas=False)
    cam_post_str = fmt_time(cam_post, millisecond, precision=0, commas=False)
    cam_total_str = fmt_time(cam_total, millisecond, precision=0, commas=False)
    ...
    cam_speedup_10x_str = fmt(cam_speedup_10x, precision=2, commas=False)          # "1.37"
    cam_speedup_inf_str = fmt(cam_speedup_inf, precision=2, commas=False)          # "1.43"
    cam_ml_opt_str = fmt_time(cam_ml_optimized, millisecond, precision=0, commas=False)
    cam_total_opt_str = fmt_time(cam_total_optimized, millisecond, precision=0, commas=False)
    range_speedup_low_str = fmt(range_speedup_low, precision=1, commas=False)
    range_speedup_high_str = fmt_int(round(range_speedup_high), commas=False)
```
