...
4.  **Hardware (The Silicon)**: The physical foundation where bits are transformed. This layer is defined by HBM (High Bandwidth Memory) capacity and high-speed intra-node interconnects like **NVLink (`{python} MLSystemsNvlinkAnchor.nvlink_h100_bw_str`)**\index{NVLink!intra-node interconnect}. Here, the memory wall acts as the primary physical constraint (@sec-hardware-acceleration).
...
*Places batch-1 inference on the memory-bound side of the roofline.*
:::
...
class HardwareSpectrumSetup:
    ...
    edge_price_min_str = fmt(Hardware.Workstation.DGX_Spark.unit_cost.m_as(USD), precision=0, prefix="$")
    edge_price_max_str = fmt(Hardware.Workstation.DGX_Spark.unit_cost_max.m_as(USD), precision=0, prefix="$")
    ...
    tiny_price_str = fmt(10, precision=0, commas=False, prefix="$")
...
| **Mobile ML** | Flagship Smartphone |                       Mobile SoC (CPU + GPU + NPU)                      | `{python} RepresentativeSystemsMobileRow.mobile_ram_range_str` | `{python} RepresentativeSystemsMobileRow.mobile_storage_range_str` |                             `{python} RepresentativeSystemsMobileRow.mobile_tdp_range_str`                             |                                                 USD 999+                                                 |
...
class GPT3TrainingScale:
    ...
    gpt3_cost_m_str = fmt(gpt3_cost_m, precision=1, prefix="$", suffix="M")
```
...
# │          Infrastructure.Pricing.Cloud.ElectricityPerKwh.rate, USD, GB, KB, watt, ureg, MILLION)
...
class CloudEdgeTCO:
    ...
    gpu_instances = 4
    egress_per_gb = Infrastructure.Pricing.Cloud.EgressPerGB.rate.m_as(USD/GB)
    ...
    electricity_per_kwh = Infrastructure.Pricing.Cloud.ElectricityPerKwh.rate.m_as(USD/ureg.kilowatt_hour)
    ...
    gpu_price_str = fmt(gpu_price_per_hr, precision=2, commas=False, prefix="$", suffix="/hr")
    egress_gb_str = fmt(egress_gb_per_day, precision=0, commas=False, suffix=" GB/day")
    egress_price_str = MarkdownStr(f"USD {egress_per_gb:.2f}/GB")
    hours_per_year_str = fmt(HOURS_PER_YEAR, precision=0)
    days_per_year_str = fmt(DAYS_PER_YEAR, precision=0, commas=False)

    c_gpu_str = fmt(c_gpu, precision=0, prefix="~$")
    c_egress_str = fmt(c_egress, precision=0, prefix="~$")
    c_lb_str = fmt(c_lb, precision=0, prefix="~$")
    c_logs_str = fmt(c_logs, precision=0, prefix="~$")
    c_total_str = fmt(c_total, precision=0, prefix="~$", suffix="/year")

    e_capex_str = fmt(e_capex, precision=0, prefix="~$")
    e_power_str = fmt(e_power, precision=1, prefix="~$")
    e_cool_str = fmt(e_cool, precision=1, prefix="~$")
    e_net_str = fmt(e_net, precision=0, prefix="~$")
    e_labor_str = fmt(e_labor, precision=0, prefix="~$")
    e_total_str = fmt_int(e_total, prefix="~$", suffix="/year", commas=True)
    ...
    server_cost_str = fmt(server_cost, precision=0, prefix="$")
    ...
    electricity_str = fmt(electricity_per_kwh, precision=2, commas=False, prefix="$", suffix="/kWh")
    devops_fte_str = fmt(devops_fte, precision=1, commas=False)
    devops_salary_str = fmt(devops_salary, precision=0, prefix="$")
```
...
- **Cloud cost**: ~USD `{python} VoiceAssistantWall.ww_cloud_cost_str` per device/year → `{python} VoiceAssistantWall.ww_devices_b_str` billion devices = USD `{python} VoiceAssistantWall.ww_total_cost_str`/year. Economically prohibitive for a free feature.
- **TinyML alternative**: `{python} VoiceAssistantWall.ww_edge_power_range_str` local wake-word detection, <USD `{python} VoiceAssistantWall.ww_edge_cost_str`/year per device. Viable at any scale.
...
# │          MB, GB, KB, second, MILLION, USD), mlsysim.physics (calc_monthly_egress_cost)
...
class BandwidthBottleneck:
    ...
    egress_cost_str = fmt(Infrastructure.Pricing.Cloud.EgressPerGB.rate.m_as(USD/GB), precision=2, commas=False)
...
3.  **Cloud transfer exposure**: Uploading raw camera feeds is primarily a bandwidth, ingest, storage, and processing problem; USD/GB cloud egress charges apply when data is transferred back out of the cloud. If the raw stream were later retrieved at USD `{python} BandwidthBottleneck.egress_cost_str`/GB data-transfer-out pricing, the transfer charge alone would reach USD `{python} BandwidthBottleneck.monthly_cost_m_str` M/month.
...
class EdgeSizingFleetTCO:
    ...
    coral_cluster_power_opex = (coral.tdp.m_as(kilowatt) * coral_units_per_store * stores * HOURS_PER_YEAR * years * Infrastructure.Pricing.Cloud.ElectricityPerKwh.rate.m_as(USD/ureg.kilowatt_hour))
    ...
    coral_cluster_unit_cost_str = fmt(coral_cost * coral_units_per_store, precision=0, commas=False)
    ...
    jetson_cost_str = fmt(jetson_cost, precision=0, commas=False)
    ...
    jetson_fleet_str = fmt(jetson_cost * stores, precision=0, commas=True)
    nuc_cost_str = fmt(nuc_cost, precision=0, commas=False)
    ...
    nuc_fleet_str = fmt(nuc_cost * stores, precision=0, commas=True)
    coral_cluster_fleet_str = fmt(coral_cluster_capex, precision=0, commas=True)
    ...
    elec_cost_str = fmt(Infrastructure.Pricing.Cloud.ElectricityPerKwh.rate.m_as(USD/ureg.kilowatt_hour), precision=2, commas=False, prefix="$", suffix="/kWh")
    coral_cluster_power_opex_str = fmt(coral_cluster_power_opex, precision=1, commas=True)
    coral_cluster_tco_str = fmt(coral_cluster_capex + coral_cluster_power_opex, precision=1, commas=True)
    cloud_cost_str = fmt(cloud_tco_3yr, precision=0, commas=True)
```

| **Edge Device**                                                                          |                                                        **INT8 TOPS**                                                        |                        **Power**                        |                         **Unit Cost**                         |                       **Fleet Cost**                      |
|:-----------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------:|:-------------------------------------------------------:|:-------------------------------------------------------------:|:---------------------------------------------------------:|
| **NVIDIA Jetson Orin NX**                                                                |                                        `{python} EdgeSizingFleetTCO.jetson_tops_str`                                        |   `{python} EdgeSizingFleetTCO.jetson_power_range_str`  |       USD `{python} EdgeSizingFleetTCO.jetson_cost_str`       |     USD `{python} EdgeSizingFleetTCO.jetson_fleet_str`    |
| **Intel NUC + Movidius**                                                                 |                                          `{python} EdgeSizingFleetTCO.nuc_tops_str`                                         |      `{python} EdgeSizingFleetTCO.nuc_power_w_str`      |         USD `{python} EdgeSizingFleetTCO.nuc_cost_str`        |      USD `{python} EdgeSizingFleetTCO.nuc_fleet_str`      |
| **Google Coral Dev (`{python} EdgeSizingFleetTCO.coral_units_per_store_str` per store)** | `{python} EdgeSizingFleetTCO.coral_cluster_peak_tops_str`<br>`{python} EdgeSizingFleetTCO.coral_cluster_sustained_tops_str` | `{python} EdgeSizingFleetTCO.coral_cluster_power_w_str` | USD `{python} EdgeSizingFleetTCO.coral_cluster_unit_cost_str` | USD `{python} EdgeSizingFleetTCO.coral_cluster_fleet_str` |
...
**TCO over `{python} EdgeSizingFleetTCO.years_span_str`** (`{python} EdgeSizingFleetTCO.coral_units_per_store_str` Coral boards/store): Hardware USD `{python} EdgeSizingFleetTCO.coral_cluster_fleet_str` + Power (`{python} EdgeSizingFleetTCO.coral_cluster_power_kw_str` $\times$ `{python} EdgeSizingFleetTCO.stores_str` $\times$ `{python} EdgeSizingFleetTCO.hours_per_year_str` $\times$ `{python} EdgeSizingFleetTCO.years_count_str` $\times$ `{python} EdgeSizingFleetTCO.elec_cost_str` = USD `{python} EdgeSizingFleetTCO.coral_cluster_power_opex_str`) = USD `{python} EdgeSizingFleetTCO.coral_cluster_tco_str` total vs. cloud inference at ~USD `{python} EdgeSizingFleetTCO.cloud_cost_str`.
...
::: {.column-margin}
![](images/svg/vol1_ml_systems_margin_003.svg){width="100%" fig-alt="A line that holds at a high peak throughput, then steps down and flattens at a lower throttled sustained rate after thermal limits engage."}

*Distinguishes burst performance from sustained thermal performance.*
:::
...
class ParadigmsTable:
    ...
    #—Cost structure ---
    cloud_cost_str = MarkdownStr("High (USD 1000s+/month)")                             # usage-based cloud
    edge_cost_str = MarkdownStr("Moderate (USD 100s–1000s)")                            # hardware capex
    mobile_cost_str = MarkdownStr("Moderate (USD 200–1000+ device)")                 # handset hardware
    tiny_cost_str = MarkdownStr("Very Low (USD 1–10)")                                 # MCU unit cost
```
...
class TcoPitfallCalc:
    ...
    cloud_compute_str = fmt(cloud_compute_value, precision=0, commas=True, prefix="$", suffix="/month")
    edge_hw_str = fmt(edge_hardware_value, precision=0, commas=False, prefix="$", suffix="/month")
    edge_network_str = fmt(edge_network_value, precision=0, commas=True, prefix="$", suffix="/month")
    edge_maint_str = fmt(edge_maintenance_value, precision=0, commas=False, prefix="$", suffix="/month")
    edge_reliability_str = fmt(edge_reliability_value, precision=0, commas=True, prefix="$", suffix="/month")
    edge_total_str = fmt(edge_total_value, precision=0, commas=True, prefix="$")
