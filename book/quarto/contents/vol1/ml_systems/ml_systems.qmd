...
class EnergyTransmission:
    ...
    ratio_str = fmt_multiple(ratio, precision=0, commas=True)
```
...
class ResnetCloud:
    ...
    cloud_ratio_x_str = fmt_multiple(cloud_ratio_x_value, precision=1, commas=False)
...
class ResnetMobile:
    ...
    mobile_ratio_x_str = fmt_multiple(mobile_ratio_x_value, precision=1, commas=False)
    mobile_bottleneck_str = MarkdownStr(p.bottleneck)
    bw_advantage_x_str = fmt_multiple(bw_advantage_x_value, precision=1, commas=False)
    inference_speed_x_str = fmt_multiple(inference_speed_x_value, precision=1, commas=False)
```
...
class VoiceAssistantWall:
    ...
    vi_peak_ratio_str = fmt_multiple(vi_peak_ratio, precision=1, commas=False)
...
class BandwidthBottleneck:
    ...
    bw_short_x_str = fmt_multiple(shortfall_ratio, precision=1, commas=False)
    ...
    metadata_reduction_x_str = fmt_multiple(metadata_reduction, precision=0, commas=True)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.fmt import fmt, fmt_flop_rate, fmt_flops, fmt_ops_rate, fmt_ratio
...
| **Required throughput**  | `{python} EdgeSizingThroughput.sustained_gflops_str` $\times$ `{python} EdgeSizingThroughput.headroom_factor_str` (headroom) | ~`{python} EdgeSizingThroughput.req_tops_equiv_str` equivalent |
...
class EdgeSizingFleetTCO:
    ...
    jetson_vs_coral_cluster_capex_ratio_str = fmt_multiple(jetson_cost / (coral_cost * coral_units_per_store), precision=1, commas=False)
...
# │          ThermalQuantCalc.quant_reduction_factor_str
# └─────────────────────────────────────────────────────────────────────────────
...
class ThermalQuantCalc:
    ...
    quant_reduction_factor_str = fmt_multiple(quant_reduction_factor, precision=0, commas=False)
...
**Systems insight**: Quantization from FP32 to INT8 reduces power by approximately `{python} ThermalQuantCalc.quant_reduction_factor_str`$\times$, but with a baseline of `{python} ThermalQuantCalc.baseline_w_str` the result is still `{python} ThermalQuantCalc.quant_power_w_str`---the absolute limit of the hardware. Physics sets a hard ceiling that no optimization can exceed.
...
class WildlifeMonitoring:
    ...
    reduction_x_str = fmt_multiple(reduction, precision=0, commas=True)
```
...
# │          MobilePowerFallacyCalc.quant_reduction_factor_str,
...
# │          AmdahlCameraCalc.range_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
...
class AmdahlCameraCalc:
    ...
    cam_speedup_10x_str = fmt_multiple(cam_speedup_10x, precision=2, commas=False)          # "1.37"
    cam_speedup_inf_str = fmt_multiple(cam_speedup_inf, precision=2, commas=False)          # "1.43"
    ...
    range_speedup_str = fmt_multiple_range(
