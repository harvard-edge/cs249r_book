...
\index{deployment paradigms!four paradigms} \index{Cloud ML|seealso{Edge ML}} \index{Edge ML|seealso{Mobile ML}} \index{Mobile ML|seealso{TinyML}}Before diving into the physics that creates these boundaries, it helps to have a concrete picture of each paradigm. Think of them as four distinct "operating envelopes"—each defined by how much power, memory, and network connectivity is available. Every ML application must fit within at least one of these envelopes, and that fit determines which algorithms, hardware, and engineering trade-offs apply. These four paradigms are:
...
Given a specific ML workload, *how* do we determine which paradigm fits? The answer requires analytical tools that connect workload characteristics to physical constraints. The following section introduces these tools.
...
Each section follows a consistent structure: definition, key characteristics, benefits and trade-offs, and representative applications. This parallel treatment reveals both what distinguishes each paradigm and what principles they share, setting the stage for the hybrid architectures that combine them.
...
Edge ML emerged from this constraint, trading unlimited computational resources for sub-100 ms latency and local data retention. In Archetype terms, edge deployment transforms the optimization target: a **Bandwidth Hog** workload like LLM inference that is memory-bound in the cloud becomes *latency-bound* at the edge, where the 50-100 ms network penalty dominates the 10-20 ms compute time. Edge hardware with sufficient local memory can eliminate this penalty entirely, shifting the bottleneck back to the fundamental memory bandwidth constraint. Recall the Iron Law from @eq-iron-law-extended: by processing locally, edge deployment eliminates the $D_{vol}/BW_{IO}$ (network I/O) term entirely, collapsing the latency to $\max(D_{vol}/BW, O/(R_{peak} \cdot \eta)) + L_{lat}$—the same memory-vs-compute trade-off, but without the network penalty that dominates cloud inference.
...
\index{deployment paradigms!quantitative comparison} \index{latency vs throughput!paradigm trade-offs}Given a specific application, which paradigm fits? Answering this question requires seeing all four paradigms side by side across the dimensions that matter for deployment decisions. The paradigm-by-paradigm analysis above revealed individual strengths and constraints; this section puts them on a common scale so engineers can make systematic comparisons.
