...
class EnergyTransmission:
    ...
    tx_energy_str = fmt_qty(tx_energy_per_mb, ureg.millijoule/MB, precision=0, commas=False)
    compute_energy_str = fmt_qty(local_energy_op, ureg.millijoule, precision=1, commas=False, extra_suffix='/inference')
    cloud_total_str = fmt_qty(cloud_energy_total, ureg.millijoule, precision=0, commas=False)
    local_total_str = fmt_qty(local_energy_total, ureg.millijoule, precision=1, commas=False)
...
from mlsysim.fmt import fmt, sci_latex, fmt_frac, MarkdownStr, fmt_qty
...
class ResnetCloud:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_tflops_str = fmt_qty(p.peak_flops_actual, TFLOP/second, precision=0, commas=False)
    a100_bw_tbs_str = fmt_qty(p.peak_bw_actual, TB/second, precision=2, commas=False)
...
class ResnetMobile:
    ...
    mobile_bw_gbs_str = fmt_qty(p.peak_bw_actual, GB/second, precision=0, commas=False)
...
from mlsysim.fmt import fmt_percent, fmt_int, fmt_usd, fmt, check, MarkdownStr, fmt_qty
...
class HardwareSpectrumSetup:
    ...
    edge_pwr_w_str = fmt_qty(Hardware.Workstation.DGX_Spark.tdp, watt, precision=0, commas=False)
    ...
    tiny_pwr_min_str = fmt_qty(Hardware.Tiny.ESP32_S3.tdp_min, watt, precision=2, commas=False)
    tiny_pwr_max_str = fmt_qty(Hardware.Tiny.ESP32_S3.tdp_max, watt, precision=1, commas=False)
...
from mlsysim.fmt import fmt_int, fmt_usd, fmt, check, fmt_qty
...
class BandwidthBottleneck:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cam_rate_mbs_str = fmt_qty(bytes_per_sec_single, MB/second, precision=1, commas=False)
    total_rate_gbs_str = fmt_qty(total_bytes_per_sec, GB/second, precision=1, commas=False)
    monthly_cost_m_str = fmt_usd(monthly_cost/MILLION, precision=1, commas=False, suffix="M/month")
    net_cap_gbs_str = fmt_qty(network.bandwidth, GB/second, precision=2, commas=False)
...
from mlsysim.physics import calc_fleet_tco
from mlsysim.fmt import fmt_percent, fmt, fmt_usd, check, MarkdownStr, fmt_qty
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware
from mlsysim.fmt import fmt_frac, fmt_percent, fmt, check, fmt_qty
...
class BatteryTax:
    ...
    pwr_w_str = fmt_qty(power_draw, ureg.watt, precision=0, commas=False)
    batt_wh_str = fmt_qty(battery_wh, ureg.Wh, precision=0, commas=False)
