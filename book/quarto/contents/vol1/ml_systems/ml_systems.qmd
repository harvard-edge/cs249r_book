...
This principle dictates that if your system is **Memory Bound** ($D_{vol}/BW > O/(R_{peak} \cdot \eta)$), buying faster processors ($R_{peak}$) yields exactly **0% speedup**. You must identify the dominant term before optimizing. This tradeoff is governed by *the energy of transmission*.
...
**ResNet-50** classifies images into 1,000 categories, processing each image through approximately `{python} resnet_gflops` billion floating-point operations using `{python} resnet_params_m` million parameters (`{python} resnet_fp32_mb` MB at FP32). Its regular, compute-dense structure makes it the canonical benchmark for hardware accelerator performance.

**GPT-2 / Llama** generate text one token at a time, requiring the model to read its full parameter set (`{python} gpt2_params_b` billion for GPT-2, 7 to 70 billion for Llama) from memory for each output token. This sequential memory access pattern creates the autoregressive bottleneck that dominates serving costs.

**DLRM** (Deep Learning Recommendation Model) maps users and items to embedding vectors stored in tables that can exceed `{python} dlrm_embedding_str` GB, making memory capacity rather than computation the binding constraint.

**MobileNet** performs the same image classification task as ResNet but uses depthwise separable convolutions to reduce computation by `{python} mobilenet_flops_reduction`×, enabling real-time inference on smartphones at `{python} mobile_tdp_range` watts.

**Keyword Spotting (KWS)** represents the always-on TinyML archetype. Used in applications like Smart Doorbells, it detects wake words ("Ding Dong", "Hello") using a depthwise separable CNN with approximately `{python} kws_params_str` parameters fitting in under `{python} kws_size_kb` KB, running continuously at under 1 milliwatt.
...
For a systematic diagnostic guide to identifying these bottlenecks, consult the **DAM Taxonomy** in @sec-appendix-dam.
