...
et_energy_ratio_str = f"{et_energy_ratio_value}"                         # "1000" × ratio
```
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Callout "Wake-Word Economics" (Cloud ML large-scale section)
# │
# │ Why: At 1 billion devices, even $0.50/device/year cloud costs become
# │      $500M annually — economically prohibitive. TinyML shifts cost to
# │      user's battery (~$0.01/year), making the feature viable at scale.
# │
# │ Imports: physx.formatting (fmt)
# │ Exports: ww_*_str variables for callout
# └─────────────────────────────────────────────────────────────────────────────
from physx.formatting import fmt

# --- Inputs (global scale scenario) ---
...
#| label: energy-gap
# ┌─────────────────────────────────────────────────────────────────────────────
# │ ENERGY GAP: TRANSMISSION VS LOCAL COMPUTE
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Callout "The N× Energy Gap" — shows why edge beats cloud for energy
# │
# │ Why: Quantifies the energy cost asymmetry between wireless transmission and
# │      local inference. This ratio (often 100-1000×) explains why battery-
# │      powered devices must process locally rather than offload to cloud.
# │
# │ Imports: physx.constants (MB, NETWORK_5G_ENERGY_PER_MB_MJ, ENERGY_MOBILENET_INF_MJ)
# │ Exports: tx_energy_str, npu_energy_str, energy_gap_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.formatting import fmt
from physx.constants import (
    MB,
...
data_size_value = 1 * MB                                                        # 1 MB video frame

tx_energy_total_value = NETWORK_5G_ENERGY_PER_MB_MJ * data_size_value            # 5G transmission cost
npu_energy_total_value = ENERGY_MOBILENET_INF_MJ                                 # Local NPU inference cost
energy_gap_ratio_value = tx_energy_total_value / npu_energy_total_value          # Transmission/compute ratio

tx_energy_str = fmt(tx_energy_total_value.to(ureg.millijoule).magnitude, precision=0, commas=False)  # e.g. "500" mJ
npu_energy_str = f"{npu_energy_total_value.to(ureg.millijoule).magnitude}"                           # e.g. "5" mJ
energy_gap_str = fmt(energy_gap_ratio_value.magnitude, precision=0, commas=False)                    # e.g. "100" ×
```
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ LATENCY MIRAGE: CLOUD VS EDGE END-TO-END COMPARISON
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Callout "The Latency Mirage" — debunks "closer = faster" assumption
# │
# │ Why: Shows that edge deployment does not automatically reduce latency. A slow
# │      edge processor (Raspberry Pi, 80ms) with fast local network (5ms) can be
# │      SLOWER than fast cloud (A100, 10ms) with moderate network (50ms). Students
# │      must learn to compute end-to-end latency, not assume proximity wins.
# │
# │ Imports: physx.formatting (fmt)
# │ Exports: cloud_net_str, cloud_inf_str, cloud_total_str,
# │          edge_net_str, edge_inf_str, edge_total_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.formatting import fmt

cloud_network_ms_value = 50                                                      # ms, cross-region round-trip
cloud_inference_ms_value = 10                                                    # ms, A100 GPU inference
edge_network_ms_value = 5                                                        # ms, local network hop
edge_inference_ms_value = 80                                                     # ms, Raspberry Pi (8× slower)

cloud_total_ms_value = cloud_network_ms_value + cloud_inference_ms_value         # 50 + 10 = 60 ms
edge_total_ms_value = edge_network_ms_value + edge_inference_ms_value            # 5 + 80 = 85 ms

cloud_net_str = f"{cloud_network_ms_value}"                                      # e.g. "50" ms
cloud_inf_str = f"{cloud_inference_ms_value}"                                    # e.g. "10" ms
cloud_total_str = fmt(cloud_total_ms_value, precision=0, commas=False)           # e.g. "60" ms
edge_net_str = f"{edge_network_ms_value}"                                        # e.g. "5" ms
edge_inf_str = f"{edge_inference_ms_value}"                                      # e.g. "80" ms
edge_total_str = fmt(edge_total_ms_value, precision=0, commas=False)             # e.g. "85" ms
```
...
Compression techniques do not scale indefinitely against physics. A smartphone with a `{python} phone_battery_str` Wh battery running a 1 W inference workload depletes in `{python} low_power_hours_str` hours, but a 5 W workload (common for large on-device models) exhausts the battery in `{python} high_power_hours_str` hours while triggering thermal throttling that reduces performance by 40--60 percent. As @sec-ml-system-architecture-mobile-ml-benefits-resource-constraints-c568 establishes, sustained mobile inference cannot exceed 2--3 W without active cooling. Reducing numerical precision (using fewer bits to represent each weight) cuts power by approximately 4x, but aggressive precision reduction often causes 5--10 percent accuracy loss. Applications requiring continuous inference beyond mobile thermal envelopes remain physically impossible regardless of algorithmic improvements.
...
##### Pitfall: *Assuming more training data always improves deployed model performance.* {.unnumbered}
