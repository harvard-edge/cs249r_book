...
# │
# │ Imports: mlsys.constants (MOBILE_TDP_W, MOBILE_NPU_TOPS_INT8, PHONE_BATTERY_WH,
# │          KWS_DSCNN_PARAMS, DLRM_MODEL_SIZE_FP32), mlsys.formatting (fmt)
...
from mlsys.constants import (
    ...
    TFLOPs, Kparam, second, watt, hour, GB
)
...
class MLSystemsSetup:
    ...
    # Mobile Specifics
    mobile_tdp_w = 3 # Typical
    mobile_npu_tops = 35 # Typical (Apple/Qualcomm)
    phone_battery_wh = 15

    # ┌── 2. CALCULATION (The Physics) ─────────────────────────────────────────
    # GPT-3 Petaflop-days: 3.14e23 FLOPs / (1e15 * 86400)
    gpt3_petaflop_days = (m_gpt3.training_ops / (1e15 * 86400)).magnitude

    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if gpt3_petaflop_days < 3000:
        raise ValueError(f"GPT-3 training should be >3000 PF-days, got {gpt3_petaflop_days:.0f}")
    ...
    # DLRM Embedding (using legacy constant for now as DLRM isn't fully in Models yet)
    dlrm_embedding_str = fmt(DLRM_MODEL_SIZE_FP32.to(GB).magnitude, precision=0)
...
class LightLatency:
    ...
    # ┌── 2. CALCULATION (The Physics) ─────────────────────────────────────────
    # Latency = (Distance * 2) / Speed of Light
    # Round-trip time
    min_latency = (distance_km * 2) / SPEED_OF_LIGHT_FIBER_KM_S
    min_latency_ms = min_latency.to(ureg.ms).magnitude

    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if min_latency_ms < safety_budget_ms:
        raise ValueError(f"Narrative broken: Physics allows cloud ({min_latency_ms:.1f}ms) within {safety_budget_ms}ms budget!")
...
class MemoryWall:
    ...
    if divergence_ratio <= 1.0:
        raise ValueError("Narrative broken: Memory is keeping up with Compute (Gap <= 1x).")
...
\index{Silicon Contract!Iron Law}The central analytical tool for this chapter is the **Iron Law of ML Systems**, established in @sec-introduction (@sec-silicon-contract) and restated here as @eq-iron-law:
...
class EnergyTransmission:
    ...
    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if ratio < 500:
        raise ValueError(f"Narrative broken: Transmission ({cloud_energy_total}mJ) is not expensive enough vs Compute ({local_energy_total}mJ). Ratio: {ratio:.1f}x")
...
class LighthouseModels:
    ...
    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if resnet_fp32_mb < 90:
        raise ValueError(f"ResNet50 size should be ~98MB, got {resnet_fp32_mb:.0f}MB")
...
\index{system bottlenecks!dominant constraints}The pipelined form of the **Iron Law of ML Systems** from @sec-silicon-contract states that execution time is bounded by the slowest resource, as @eq-iron-law-extended formalizes:
...
Cloud ML excels at processing massive data volumes through parallelized architectures, enabling training on datasets requiring hundreds of terabytes of storage and petaflops of computation—resources that remain impractical on constrained devices. The training techniques covered in @sec-ai-training and the hardware analysis in @sec-ai-acceleration explain how this scale is achieved.
...
class CloudEdgeTCO:
    ...
    egress_gb_per_day = (requests_per_day * response_kb) / (1024 * 1024)
    ...
    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if c_total < e_total:
        # Note: At 1M/day, edge should be cheaper. If not, scenario changed.
        raise ValueError(f"Narrative broken: Edge should be cheaper at 1M volume. Cloud=${c_total:.0f}, Edge=${e_total:.0f}")
...
# │      Part 2: 1B devices streaming audio = 32 TB/s, exceeding global internet
...
vi_total_audio_tb_per_sec_value = (vi_audio_bytes_per_sec_value * vi_devices_b_value * 1e9) / 1e12
...
vi_audio_tb_str = fmt(vi_total_audio_tb_per_sec_value, precision=0, commas=False)
```
...
from mlsys.constants import (
    ...
    VIDEO_FPS_STANDARD, CLOUD_EGRESS_PER_GB, MB, GB,
)
...
class BandwidthBottleneck:
    ...
    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if total_bytes_per_sec <= network_cap_bytes:
        raise ValueError(f"Narrative broken: Bandwidth ({total_bytes_per_sec}) fits within Network ({network_cap_bytes})! No bottleneck.")
    if shortfall_ratio < 2:
        raise ValueError(f"Narrative broken: Shortfall ({shortfall_ratio:.1f}x) is too small to be a 'crisis'.")
    ...
    monthly_cost_m_str = fmt(monthly_cost / 1e6, precision=1, commas=False)
...
1.  **Raw data rate per camera**: `{python} video_width_str` × `{python} video_height_str` × `{python} bytes_per_pixel_str` bytes × `{python} bb_fps_str` FPS ≈ **`{python} cam_rate_mbs_str` MB/s**.
...
class EdgeSizing:
    ...
    # YOLOv8 Nano FLOPs (stored in training_ops for now, treated as inference FLOPs in this context or use constant)
    # Actually models.py defines training_ops. I should have added inference_flops.
    # For now, I'll access the value I put in training_ops which was 8.7 GFLOPs
    yolo_flops = model.training_ops
    ...
    cloud_cost_k_str = "500" # Placeholder estimate for context
    int8_mult_str = "4"      # Standard INT8 vs FP32 throughput multiplier
...
class BatteryTax:
    ...
    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if runtime_hours.magnitude > 24:
        raise ValueError(f"Narrative broken: Always-on ML should drain battery fast, but got {runtime_hours:.1f} hours.")
...
Mobile optimization insights inform cloud efficiency because memory bandwidth constraints appear at every scale. Techniques like operator fusion and activation checkpointing, developed for mobile's tight memory budgets, reduce cloud inference costs by 2-3× when applied to batch serving. TinyML innovations drive cross-paradigm advances because extreme constraints force genuinely novel algorithmic breakthroughs: binary neural networks, developed for microcontrollers, now accelerate cloud recommendation systems, and sparse attention mechanisms, essential for fitting transformers in kilobytes, reduce cloud training costs.

The remaining chapters explore each layer: @sec-data-engineering-ml for data pipelines, @sec-model-compression for optimization, and @sec-machine-learning-operations-mlops for operational aspects. All of these apply whether you deploy to a TPU Pod or an ESP32. But shared principles also mean shared vulnerabilities: the same operational challenges—data drift, model decay, monitoring—appear at every tier and demand attention before we consider the chapter's remaining lessons.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.formatting import fmt, md_frac
