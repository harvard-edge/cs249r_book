...
[^fn-archetype]: **Archetype**: From Greek *arkhetypon* ("original form"). In ML systems, archetypes categorize workloads by their dominant physical bottleneck (e.g., compute, bandwidth, or memory capacity) rather than their specific model architecture. \index{Archetype!etymology}

\index{paradigm!etymology}

[^fn-paradigm]: **Paradigm**: From Greek *paradeigma* ("pattern" or "model"). In this chapter, deployment paradigms (Cloud, Edge, Mobile, TinyML) represent distinct operational frameworks defined by unique constraints on power, memory, and connectivity. \index{Paradigm!etymology}

\index{latency!etymology}

[^fn-latency-etymology]: **Latency**: From Latin *latere* ("to lie hidden"). In systems, it represents the time delay between an input and its corresponding output. While throughput measures total volume, latency defines the responsiveness floor for real-time control loops (see @sec-serving). \index{Latency!etymology}

\index{bandwidth!etymology}

[^fn-bandwidth]: **Bandwidth**: Originally a radio engineering term (1930s) describing the width of frequency bands. In computing, it measures the data transfer rate between components (bytes/second). The "Memory Wall" describes the growing gap where compute speed outpaces memory bandwidth, making data movement the primary bottleneck in modern ML. \index{Bandwidth!etymology}

\index{critical path!latency determination}

[^fn-critical-path]: **Critical Path**: A systems engineering term for the longest sequence of dependent operations that determines total completion time. In ML, the critical path consists of the specific computations and data transfers that set the end-to-end latency; no operation outside this path can reduce total execution time. \index{Critical Path!definition}
...
[^fn-cost-spectrum]: **ML Hardware Cost Spectrum**: AI infrastructure spans six orders of magnitude in cost, from \$10 microcontrollers to multi-million dollar supercomputers. This 100,000$\times$ range allows ML to be deployed across vastly different economic contexts, from simple hobbyist projects to hyperscale industrial analytics.

[^fn-pue]: **Power Usage Effectiveness (PUE)**: A data center efficiency metric defined as total facility power divided by IT equipment power. While a theoretical PUE of 1.0 is impossible, modern liquid-cooled AI facilities achieve ratios as low as 1.06, indicating that nearly all energy is dedicated to computation rather than cooling (see @sec-hardware-acceleration). \index{PUE!definition}
...
[^fn-cloud-evolution]: **Cloud Infrastructure Evolution**: Public cloud computing for ML emerged in the mid-2000s, transforming internal infrastructure into a utility service. By decoupling hardware ownership from model development, the cloud democratized access to the peta-scale compute required for modern training. \index{Cloud Infrastructure!historical context}

[^fn-nlp-compute]: **NLP Computational Demands**: Large-scale language model training (like GPT-3) requires thousands of GPUs and millions of dollars in electricity and compute time. This massive scale drives the need for the specialized training systems and bisection-bandwidth-optimized fabrics developed in Volume 2. \index{NLP!computational requirements}
...
[^fn-mlsys-tpu]: **Tensor Processing Unit (TPU)**: A custom ASIC developed by Google specifically to accelerate the tensor operations that dominate deep learning. By trading general-purpose flexibility for specialized matrix-unit throughput, TPUs achieve high energy efficiency ($\eta$) for both massive training pods and low-power inference. \index{TPU!definition}
...
[^fn-ml-apis]: **ML APIs**: Cloud-hosted interfaces that provide pre-trained model capabilities (e.g., vision, speech) as a service. These APIs democratized AI by allowing developers to integrate complex inference into their applications without managing the underlying training systems or specialized hardware. \index{ML APIs!democratization role}

[^fn-paas-pricing]: **Pay-as-You-Go Pricing**: A cloud economic model where users pay only for the compute cycles consumed (e.g., GPU-hours) rather than the hardware's capital cost. This model enables elastic scaling, allowing organizations to burst to thousands of GPUs for short training runs that would be economically unfeasible to own. \index{Cloud Economics!pay-as-you-go}
...
[^fn-gdpr]: **GDPR (General Data Protection Regulation)**: A European privacy framework (2018) that mandates strict control over personal data. In ML systems, GDPR necessitates "Right to be Forgotten" implementation and model auditability, establishing legal constraints that influence both data engineering and lifecycle management. \index{GDPR!ML implications}

[^fn-hipaa]: **HIPAA**: The US healthcare privacy law (1996) that governs the storage and processing of Protected Health Information (PHI). For ML fleets, HIPAA compliance requires specialized audit trails and encryption, adding significant infrastructure overhead to medical diagnostic systems. \index{HIPAA!healthcare ML}
...
[^fn-tco]: **Total Cost of Ownership (TCO)**: The sum of all direct and indirect costs over a system's lifetime, from hardware acquisition to electricity and maintenance labor. In ML systems, the purchase price of a GPU is often only a small fraction of the 3-year TCO, which is dominated by energy costs and operational complexity. \index{TCO!definition}
...
[^fn-dlrm]: **Deep Learning Recommendation Model (DLRM)**: A model architecture developed by Meta (2019) that exemplifies the "Sparse Scatter" archetype. With embedding tables often exceeding 100 TB, DLRM is constrained by memory capacity and communication bandwidth ($BW$) rather than raw FLOPs ($R_{peak}$), requiring specialized cluster designs for recommendation at scale. \index{DLRM!systems impact}
...
[^fn-industrial-iot]: **Industrial IoT (IIoT)**: The integration of AI and sensors into manufacturing and supply chains. Edge ML enables real-time quality control and predictive maintenance in these environments, reducing the need to transmit massive sensor streams to the cloud for processing. \index{IIoT!edge ML application}

[^fn-iot-hubs]: **IoT Hubs**: Hardware gateways that aggregate data from hundreds of decentralized sensors before cloud transmission. By performing local preprocessing and anomaly detection, these hubs reduce network traffic by over 90% while providing the low-latency response times required for industrial safety loops. \index{IoT Hubs!definition}
...
[^fn-iot-growth]: **IoT Device Growth**: The explosive increase in connected devices, projected to exceed 25 billion by 2030 [@mckinsey2021iot]. This growth drives the "Data Wall," where the aggregate data generated by sensors outpaces the world's ability to transmit it to centralized data centers, making edge intelligence mandatory. \index{IoT!device growth}
...
[^fn-latency-critical]: **Latency-Critical Applications**: Systems where response time directly impacts safety or user experience (e.g., autonomous braking or VR/AR). For these applications, the 100+ ms "Distance Penalty" of the cloud is physically unacceptable, necessitating the sub-10 ms deterministic response of edge deployment. \index{Latency-Critical!definition}
...
[^fn-endpoint-constraints]: **Edge Server Constraints**: Unlike cloud infrastructure, edge hardware operates with limited memory (1--8 GB) and power (5--50 W). These physical constraints necessitate the specialized model compression and quantization techniques developed in @sec-model-compression to achieve sustainable duty cycles ($\eta$). \index{Edge Hardware!constraints}

[^fn-edge-coordination]: **Edge Network Coordination**: Managing a fleet of distributed edge devices requires sophisticated orchestration to handle intermittent connectivity and hardware heterogeneity. These operational patterns, including model registries and automated health checks, are examined in @sec-ml-operations. \index{Edge Orchestration!definition}
...
[^fn-amazon-go]: **Amazon Go**: A checkout-free retail system that demonstrates edge ML at scale by processing over 1 TB/hour of video data locally. This architecture bypasses the "Ingestion Bottleneck" of the cloud, enabling real-time multi-object tracking without the prohibitive cost of uploading raw video streams. \index{Amazon Go!edge ML case study}

[^fn-industry-40]: **Industry 4.0**: The fourth industrial revolution, characterized by the integration of AI, IoT, and cyber-physical systems into manufacturing. By closing the feedback loop between sensors and actuators locally, edge ML drives significant productivity gains and cost reductions across global production lines. \index{Industry 4.0!definition}

[^fn-predictive-maintenance]: **Predictive Maintenance**: An ML application that analyzes sensor data (vibration, heat) to predict equipment failure before it occurs. Industry deployments report substantial savings through the reduction of unplanned downtime, illustrating the economic ROI of continuous edge monitoring. \index{Predictive Maintenance!edge AI application}
...
[^fn-voice-recognition]: **Voice Recognition Evolution**: On-device voice models have evolved from simple keyword spotting to complex transformer-based systems that process audio in <30 ms. This shift from cloud to local processing addresses both the "Distance Penalty" ($L_{lat}$) and the privacy risks of transmitting audio data. \index{Voice Recognition!on-device evolution}

[^fn-computational-photography]: **Computational Photography**: The use of ML algorithms to enhance or transcend the physical limits of optical sensors. Techniques like multi-frame fusion and depth estimation (Portrait Mode) process raw sensor data in real-time, illustrating the shift from passive optics to active, software-defined imaging. \index{Computational Photography!definition}

[^fn-depthwise-separable]: **Depthwise Separable Convolutions**: An architectural innovation that factorizes standard convolutions into two smaller operations, reducing computational cost by 8--9$\times$ for typical models. This efficiency is critical for running vision workloads within the strict thermal and battery budgets of mobile devices. \index{Depthwise Separable Convolution!compute efficiency}
...
[^fn-mobile-soc]: **Mobile SoC (System-on-Chip)**: A highly integrated processor that combines CPUs, GPUs, and specialized NPUs on a single die. Integration reduces the energy cost of data movement ($\eta$) by orders of magnitude compared to discrete components, making sustained AI inference possible on personal devices. \index{SoC!definition}

[^fn-npu]: **Neural Processing Unit (NPU)**: A specialized hardware accelerator optimized for the low-precision, high-throughput arithmetic of neural network inference. By trading general-purpose flexibility for specialized data paths, NPUs achieve 10--100$\times$ better energy efficiency than mobile CPUs (see @sec-hardware-acceleration). \index{NPU!definition}

[^fn-tflite]: **TensorFlow Lite**: A lightweight ML framework designed specifically for mobile and embedded devices. It applies optimizations like quantization and operator fusion to reduce model size, enabling complex vision and language applications to run on 4B+ devices worldwide. \index{TensorFlow Lite!mobile optimization}

[^fn-coreml]: **Core ML**: Apple’s on-device ML framework that automatically optimizes model execution for the available CPU, GPU, and Neural Engine. By providing a hardware-aware compiler, it ensures that models achieve maximum efficiency ($\eta$) without requiring manual kernel tuning by developers. \index{Core ML!hardware-aware execution}
