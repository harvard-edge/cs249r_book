...
# │ Exports: ResnetCloud.a100_tflops_str, .a100_bw_tbs_str, .cloud_ratio_mult_str,
...
# │
# │ Imports: mlsysim (Models, Hardware, Engine), mlsysim.fmt
# │ Exports: ResnetMobile.mobile_tops_str, .mobile_bw_gbs_str, .mobile_ratio_mult_str,
...
**Key Insight**\index{Quantization!deployment benefits}: Both platforms are memory bound for single-image inference! The A100's faster memory bandwidth (`{python} ResnetCloud.a100_bw_tbs_str` vs. `{python} ResnetMobile.mobile_bw_gbs_str` = `{python} ResnetMobile.bw_advantage_mult_str`) translates to roughly `{python} ResnetMobile.inference_speed_mult_str` faster inference; peak compute alone is not the limiting comparison. This explains why quantization (reducing bytes) often beats faster hardware (increasing FLOP/s) for deployment.
...
4.  **Network reality**: Even a dedicated 10 Gbps line (`{python} BandwidthBottleneck.net_cap_gbs_str`) cannot carry the load---the workload demands `{python} BandwidthBottleneck.bw_short_mult_str` more bandwidth than exists.
