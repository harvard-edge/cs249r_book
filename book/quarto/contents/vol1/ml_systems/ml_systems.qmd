...
::: {#fig-doorbell-scorecard fig-env="figure" fig-pos="htb" fig-cap="**Smart Doorbell Constraint Scorecard**: Constraints are checked in order, fit before speed. The model clears the ESP32-S3's kilobyte-scale memory budget (Level 1: PASS), but its 101 ms baseline latency exceeds the deliberately tightened 50 ms interaction target (Level 2: FAIL) even though it clears the scenario's canonical 200 ms budget. Passing the memory gate first is what promotes latency to the binding constraint." fig-alt="A horizontal bar chart showing two levels of constraints. Level 1: Memory (RAM) shows a green bar (PASS). Level 2: Latency (SLA) shows a red bar (FAIL), exceeding the limit line."}
...
That comparison turns model choice into a system-design decision spanning physics, infrastructure cost, and the ongoing burden of maintaining accuracy.
