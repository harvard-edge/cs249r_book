...
[^fn-iot-data-wall]: **IoT Data Wall**: Connected devices are projected to exceed 25 billion by 2030, each generating continuous sensor streams. The aggregate $D_{\text{vol}}$ from these devices already exceeds global network $\text{BW}$ capacity for centralized ingestion, making local edge processing not an optimization but a physical necessity: the data simply cannot all reach the cloud. \index{IoT!Data Wall}
