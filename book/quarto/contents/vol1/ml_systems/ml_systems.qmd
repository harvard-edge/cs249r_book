...
The deployment spectrum from cloud to embedded systems exists not by choice, but by necessity imposed by physical laws governing computing systems. These constraints impose hard lower bounds and unavoidable trade-offs that engineering cannot eliminate, only manage, motivating specialized deployment paradigms optimized for different operational contexts.
...
**Economics of scale** create significant cost-per-unit differences that justify different deployment approaches. A single cloud server, at typical enterprise prices, can support many users through virtualization, driving down per-user costs. However, applications requiring strict latency SLOs or strong data isolation may not be able to share resources as aggressively, reducing this economic advantage. Meanwhile, embedded processors in the single-digit-to-tens-of-dollars range enable deployment at billions of endpoints where individual cloud connections would be economically infeasible.
...
To understand cloud ML's position in the deployment spectrum, we must first consider its defining characteristics. Cloud ML aggregates computational resources in data centers, operating at unprecedented scale. @fig-cloudml-example illustrates this concept with an example from Google's Cloud TPU[^fn-mlsys-tpu] data center. As detailed in @tbl-representative-systems, cloud systems can provide orders-of-magnitude more compute and memory bandwidth than mobile devices, at correspondingly higher power and operational cost. A durable anchor is that modern cloud accelerator systems operate at the scale of *petaflops to exaflops* of peak reduced-precision throughput and require *megawatt-scale* facility power when deployed in large clusters. These data center facilities enable computational workloads that are impractical on resource-constrained devices. However, the remote location of cloud resources introduces critical trade-offs: network round-trip latency of 100-500ms eliminates real-time applications, while operational costs scale linearly with usage.
...
A common misconception assumes that Cloud ML's vast computational resources make it universally superior to alternative deployment approaches. Cloud infrastructure offers exceptional computational power and storage, yet this advantage doesn't automatically translate to optimal solutions for all applications. Cloud deployment introduces significant trade-offs including network latency (often tens to hundreds of milliseconds round trip), privacy concerns when transmitting sensitive data, ongoing operational costs that scale with usage, and dependence on network connectivity. Edge and embedded deployments excel in scenarios requiring real-time response (for example, sub-10ms decision making in autonomous control loops), strict data privacy (medical devices processing patient data), predictable costs (one-time hardware investment versus recurring cloud fees), or operation in disconnected environments (industrial equipment in remote locations). The optimal deployment paradigm depends on specific application requirements rather than raw computational capability.
...
TinyML succeeds across domains where its advantages—ultra-low power, low per-node cost, and local processing—enable applications that are impractical with other paradigms. Industrial predictive maintenance demonstrates TinyML's ability to transform traditional infrastructure through distributed intelligence.
...
@tbl-big_vs_tiny quantifies these paradigm differences across performance, operational, and deployment dimensions, revealing clear gradients in latency (cloud: 100-1000ms → edge: 10-100ms → mobile: 5-50ms → tiny: 1-10ms) and privacy properties (often strongest when TinyML keeps raw data local).
