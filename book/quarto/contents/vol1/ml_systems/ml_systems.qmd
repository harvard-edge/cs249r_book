...
@Eq-memory-wall quantifies this divergence: processors have doubled in compute capacity roughly every `{python} MemoryWallTrends.compute_doubling_months_str`, but memory bandwidth has improved only ~`{python} MemoryWallTrends.mem_bw_growth_pct_str` annually. This widening gap makes data movement\index{Data Movement!energy dominance} a frequent bottleneck and energy cost in ML workloads. The constraint affects all paradigms but is especially acute for TinyML, where devices have only kilobytes of memory to work with. We examine the hardware architectural responses to the memory wall, including HBM and on-chip SRAM hierarchies, in detail in @sec-hardware-acceleration-understanding-ai-memory-wall-3ea9.
...
The battery life and resource constraints listed earlier translate directly into engineering requirements. Always-on ML features incur what we call **the battery tax**\index{Battery Tax!always-on constraints}, because continuous inference spends the phone's finite energy budget even before the rest of the system runs.
