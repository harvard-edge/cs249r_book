...
# ML Systems {#sec-ml-system-architecture} \index{ML systems!deployment paradigms} \index{deployment paradigms|see{ML systems}}
...
### The Four Deployment Paradigms {#sec-ml-system-architecture-four-deployment-paradigms} \index{deployment paradigms!four paradigms} \index{Cloud ML|seealso{Edge ML}} \index{Edge ML|seealso{Mobile ML}} \index{Mobile ML|seealso{TinyML}}
...
### Physical Constraints: Why Paradigms Exist {#sec-ml-system-architecture-deployment-spectrum-71be} \index{Silicon Contract!physical constraints} \index{physical constraints!speed of light} \index{physical constraints!thermodynamics} \index{physical constraints!memory signaling}
...
### Analyzing Workloads: The Iron Law and Archetypes {#sec-ml-system-architecture-analyzing-workloads} \index{Iron Law of ML Systems!components} \index{Silicon Contract!Iron Law}

The Introduction established the **Iron Law of ML Systems** (@sec-silicon-contract):
...
#### The Bottleneck Principle {#sec-ml-systems-bottleneck-principle} \index{bottleneck principle!pipelined execution} \index{system bottlenecks!identifying} \index{compute-bound vs memory-bound!definition}
...
::: {.callout-notebook title="The Energy of Transmission"} \index{energy of transmission!local vs cloud} \index{Energy Wall!battery constraints}
...
To navigate these constraints systematically, we categorize ML workloads into four **Archetypes**\index{Workload Archetypes}\index{Workload Archetypes!physical bottlenecks}.[^fn-archetype] These represent fundamental physical bottlenecks, not just specific model architectures:
...
::: {.callout-lighthouse title="Five Reference Workloads"} \index{Lighthouse Models!deployment mapping} \index{reference workloads!paradigm selection}
Throughout this book, we use five **Lighthouse Models** introduced in @sec-introduction—concrete workloads that span the deployment spectrum and isolate distinct system bottlenecks. @sec-dnn-architectures provides full architectural details and model biographies.
...
### From Framework to Practice {#sec-ml-system-architecture-framework-to-practice} \index{latency!decision thresholds} \index{latency vs throughput!trade-offs}
...
::: {.callout-perspective title="System Balance Across Paradigms"} \index{Equation of System Balance!paradigm comparison} \index{system bottlenecks!dominant constraints}
The **Equation of System Balance**\index{Equation of System Balance!execution time bound} from @sec-introduction states that execution time is bounded by the slowest resource:

$$T = \max\left( \frac{O}{R_{peak} \cdot \eta}, \frac{D_{vol}}{BW}, \frac{D_{vol}}{BW_{IO}} \right)$$

For a systematic diagnostic guide to identifying these bottlenecks, consult the **DAM Taxonomy**\index{DAM Taxonomy!bottleneck diagnosis} in @sec-appendix-dam.
...
::: {.callout-notebook title="ResNet-50 on Cloud vs Mobile"} \index{ResNet-50!cloud vs mobile}\index{arithmetic intensity!bottleneck analysis}
...
## Cloud ML: Maximizing Computational Power {#sec-ml-system-architecture-cloud-ml-maximizing-computational-power-a338} \index{Cloud ML!datacenter scale} \index{data centers!ML infrastructure}
...
::: {.callout-notebook title="The Distance Penalty"} \index{distance penalty!cloud latency} \index{Light Barrier!safety-critical systems}
**Problem**: You are deploying a real-time safety monitor for a robotic arm. The safety logic requires a **`{python} safety_budget_str` ms** end-to-end response time to prevent injury. Your model runs in a high-performance cloud data center `{python} distance_km_str` km away.
...
### Cloud Infrastructure and Scale {#sec-ml-system-architecture-cloud-infrastructure-scale-f0b1} \index{Cloud ML!TPU infrastructure} \index{exaflop-scale training|see{Cloud ML}}
...
### Cloud ML Trade-offs and Constraints {#sec-ml-system-architecture-cloud-ml-tradeoffs-constraints-96ed} \index{Cloud ML!latency limitations} \index{Cloud ML!privacy concerns} \index{GDPR compliance!cloud deployment} \index{HIPAA compliance!cloud deployment}
...
### Large-Scale Training and Inference {#sec-ml-system-architecture-largescale-training-inference-e16d} \index{Cloud ML!training at scale} \index{hybrid architectures!wake-word detection}
...
## Edge ML: Reducing Latency and Privacy Risk {#sec-ml-system-architecture-edge-ml-reducing-latency-privacy-risk-2625} \index{Edge ML!distance penalty} \index{Edge ML!data sovereignty}
...
::: {.callout-notebook title="The Bandwidth Bottleneck"} \index{bandwidth bottleneck!video streaming} \index{Edge ML!bandwidth reduction}
**Problem**: You are designing a quality control system for a factory floor with **`{python} num_cameras_str` cameras** running at **`{python} fps_str` FPS** with **1080p resolution**. Should you stream to the cloud or process at the edge?
...
### Edge ML Benefits and Deployment Challenges {#sec-ml-system-architecture-edge-ml-benefits-deployment-challenges-b2d0} \index{Edge ML!distributed processing} \index{Edge ML!deployment challenges}
...
### Real-Time Industrial and IoT Systems {#sec-ml-system-architecture-realtime-industrial-iot-systems-373a} \index{Edge ML!autonomous vehicles} \index{Edge ML!industrial IoT} \index{Edge ML!smart retail} \index{predictive maintenance!edge deployment}
...
## Mobile ML: Personal and Offline Intelligence {#sec-ml-system-architecture-mobile-ml-personal-offline-intelligence-0983} \index{Mobile ML!battery constraints} \index{Mobile ML!thermal envelope}
...
::: {.callout-notebook title="The Battery Tax"} \index{battery life!ML impact} \index{Mobile ML!energy budget}
**Problem**: You want to deploy a "real-time" background object detector on a smartphone. The model consumes **`{python} pwr_w_str` Watts** of continuous power when active. The phone has a standard **`{python} batt_wh_str` Watt-hour (Wh)** battery.
...
::: {.callout-notebook title="The Thermal Wall"} \index{thermal wall!mobile constraints} \index{Power Wall!mobile implications}
...
### Personal Assistant and Media Processing {#sec-ml-system-architecture-personal-assistant-media-processing-98d7} \index{Mobile ML!computational photography} \index{Mobile ML!voice recognition} \index{Mobile ML!health monitoring}
...
## TinyML: Ubiquitous Sensing at Scale {#sec-ml-system-architecture-tinyml-ubiquitous-sensing-scale-a67b} \index{TinyML!ubiquitous sensing} \index{TinyML!cost efficiency}
...
::: {.callout-notebook title="Energy Per Inference"} \index{energy per inference!paradigm comparison} \index{TinyML!energy efficiency}
...
### TinyML Advantages and Operational Trade-offs {#sec-ml-system-architecture-tinyml-advantages-operational-tradeoffs-2d40} \index{TinyML!resource constraints} \index{TinyML!model compression} \index{microcontrollers!ML deployment}
...
### Environmental and Health Monitoring {#sec-ml-system-architecture-environmental-health-monitoring-14ad} \index{TinyML!wake-word detection} \index{TinyML!precision agriculture} \index{TinyML!medical wearables}
...
## Comparative Analysis and Paradigm Selection {#sec-ml-system-architecture-comparative-analysis-paradigm-selection-bf66} \index{deployment paradigms!comparative analysis} \index{paradigm selection!decision framework}
...
### Quantitative Trade-off Analysis {#sec-ml-system-architecture-quantitative-tradeoff-analysis-56a8} \index{deployment paradigms!quantitative comparison} \index{latency vs throughput!paradigm trade-offs}
...
### Decision Framework {#sec-ml-system-architecture-decision-framework-241f} \index{decision framework!paradigm selection} \index{deployment paradigms!selection criteria}
...
::: {.callout-perspective title="The Complexity Tax"} \index{complexity tax!ML vs heuristics} \index{deployment paradigms!complexity considerations}
...
## Hybrid Architectures: Combining Paradigms {#sec-ml-system-architecture-hybrid-architectures-combining-paradigms-7cdd} \index{hybrid architectures!combining paradigms} \index{Hybrid ML!integration strategies}
...
### Integration Patterns {#sec-ml-system-architecture-integration-patterns-5935} \index{Hybrid ML!train-serve split} \index{Hybrid ML!hierarchical processing} \index{Hybrid ML!progressive deployment}
...
### Production System Integration {#sec-ml-system-architecture-production-system-integration-3bb3} \index{Hybrid ML!production systems} \index{data pipelines!hybrid architectures}
...
### Why Hybrid Approaches Work {#sec-ml-system-architecture-hybrid-approaches-work-4bb8} \index{Hybrid ML!convergence principles} \index{ML systems!shared foundations}
...
## Fallacies and Pitfalls {#sec-ml-system-architecture-fallacies-pitfalls-3dfe} \index{ML systems!common fallacies} \index{deployment paradigms!pitfalls}
...
::: {.callout-example title="The Latency Mirage"} \index{latency mirage!edge vs cloud}
##### Fallacy: *Edge deployment always reduces latency compared to cloud.* {.unnumbered}
