...
\index{ML systems!deployment paradigms} \index{deployment paradigms|see{ML systems}}Physical laws—not engineering limitations—determine where ML models can run. The speed of light makes distant cloud servers useless for emergency braking. Thermodynamics prevents datacenter-class models from running in your pocket. Memory physics dictates that moving data often costs more energy than processing it. The deployment paradigms—Cloud, Edge, Mobile, and TinyML—are *not style choices but necessary adaptations* to physical boundaries that no amount of engineering can overcome.
...
# PURPOSE
# =============================================================================
# Purpose: Setup constants for the deployment spectrum overview.
# Used in: Deployment spectrum table + surrounding narrative.

# =============================================================================
# INPUT
# =============================================================================
# Mobile RAM/storage (typical ranges)
mobile_ram_range_str = "8-16"
mobile_storage_range_str = "128 GB-1 TB"
mobile_bw_range_str = "25-50"

# Cloud/Edge latency (typical ranges)
cloud_latency_range_str = "100-500"
edge_latency_range_str = "10-100"
mobile_latency_range_str = "5-50"
tiny_latency_range_str = "1-10"

# MobileNet efficiency (typical range)
mobilenet_flops_reduction_str = "8-9"

# Compute growth assumptions
compute_doubling_months_value = 18
mem_bw_growth_pct_value = 20

# GPT-3 compute scale (illustrative)
gpt3_petaflop_days_value = 3_640
gpt3_v100_count_value = 10_000
gpt3_days_value = 15
gpt3_cost_m_value = 4.6
...
# OUTPUT
# =============================================================================
mobile_tdp_str = fmt(mobile_tdp_w_value, precision=0, commas=False)
mobile_tdp_range_str = "3-5"
mobile_npu_tops_str = fmt(mobile_npu_tops_value, precision=0, commas=False)
...
gpt3_petaflop_days_str = fmt(gpt3_petaflop_days_value, precision=0, commas=True)
gpt3_v100_count_str = fmt(gpt3_v100_count_value, precision=0, commas=True)
gpt3_days_str = fmt(gpt3_days_value, precision=0, commas=False)
gpt3_cost_m_str = fmt(gpt3_cost_m_value, precision=1, commas=False)
phone_battery_str = fmt(phone_battery_wh_value, precision=0, commas=False)
kws_params_str = "70,000"  # Typical small KWS model; DS-CNN (MLPerf Tiny) uses ~200K
kws_size_kb_str = "100"
dlrm_embedding_str = f"{dlrm_embedding_gb_value}"
```

## Deployment Paradigm Framework {#sec-ml-system-architecture-deployment-paradigm-framework-0d25}

\index{ML systems!deployment spectrum} \index{physical constraints!deployment implications}Where an ML model runs fundamentally shapes what is possible. Yet deployment is far harder than it appears, and the reason is not the model itself. In production ML systems, the model accounts for roughly 5% of the codebase [@sculley2015hidden]. The remaining 95% consists of data collection, feature processing, serving infrastructure, monitoring, and resource management. All of this surrounding infrastructure changes dramatically depending on where the model executes.
...
This section builds the analytical framework for understanding *why* these four paradigms exist and *how* to reason about which one fits a given workload. We introduce the physical laws that govern each regime, apply the ***Iron Law of ML Systems*** (@sec-introduction) as a quantitative tool for analyzing performance bottlenecks, and define a set of reference workloads that serve as concrete examples throughout the book. Each paradigm then receives detailed treatment, with quantitative analysis of its operational characteristics and engineering trade-offs. The chapter concludes with a comparative decision framework and hybrid architectures that combine paradigms to satisfy requirements no single deployment target can meet alone.

### The Four Deployment Paradigms {#sec-ml-system-architecture-four-deployment-paradigms}

\index{deployment paradigms!four paradigms} \index{Cloud ML|seealso{Edge ML}} \index{Edge ML|seealso{Mobile ML}} \index{Mobile ML|seealso{TinyML}}These four paradigms are:

**Cloud ML**\index{Cloud ML!characteristics} aggregates computational resources in data centers, offering virtually unlimited compute and storage at the cost of network latency. When you train a large language model or run complex inference that can tolerate 100+ ms delays, Cloud ML is the natural choice.

**Edge ML**\index{Edge ML!latency benefits} moves computation closer to where data originates—factory floors, retail stores, hospitals. By processing locally rather than sending data to distant servers, edge systems achieve lower latency (10-100 ms) and keep sensitive data on-premises.

**Mobile ML**\index{Mobile ML!energy constraints} brings intelligence directly to smartphones and tablets. These devices must balance computational capability against battery life and thermal constraints, making them ideal for personal AI applications that need to work offline.

**TinyML**\index{TinyML!always-on sensing} pushes intelligence to the smallest devices—microcontrollers costing dollars and consuming milliwatts. When you need always-on sensing (wake-word detection, motion recognition) that runs for months on a coin-cell battery, TinyML is the only viable option.
...
**The Light Barrier**\index{Light Barrier!latency floor} establishes the absolute latency[^fn-latency-etymology] floor. The minimum round-trip time is governed by:
...
# PURPOSE
# =============================================================================
# Purpose: Calculate minimum light-speed latency for cross-country distance.
# Used in: The Light Barrier narrative.

# =============================================================================
# INPUT
# =============================================================================
distance_km_value = 3600 # California to Virginia
...
# OUTPUT
# =============================================================================
min_latency_str = fmt(min_latency_ms_value, precision=0, commas=False)
distance_str = f"{distance_km_value:,}"
```
...
**The Power Wall**\index{Power Wall!thermal limits}\index{Power Wall!frequency scaling}\index{thermal design power (TDP)|see{Power Wall}} emerged because thermodynamics limits how much computation can occur in a given volume. Under classical Dennard scaling (which held until approximately 2006), the relationship between power and frequency was cubic:
...
# PURPOSE
# =============================================================================
# Purpose: Illustrate compute vs memory bandwidth growth (Memory Wall).
# Used in: Memory Wall equation and narrative.

# =============================================================================
# INPUT
# =============================================================================
compute_growth_per_yr_value = 1.6  # x/year
mem_bw_growth_per_yr_value = 1.2   # x/year
...
# OUTPUT
# =============================================================================
compute_growth_str = fmt(compute_growth_per_yr_value, precision=1, commas=False)
mem_bw_growth_str = fmt(mem_bw_growth_per_yr_value, precision=1, commas=False)
mem_wall_ratio_str = fmt(mem_wall_ratio_value, precision=2, commas=False)
```

**The Memory Wall**\index{Memory Wall!bandwidth divergence}\index{Memory Wall!compute-memory gap}\index{memory bandwidth|see{Memory Wall}} [@wulf1995memory] reflects the widening bandwidth[^fn-bandwidth] gap:

Compute Growth / Memory BW Growth ≈ `{python} compute_growth_str` x/year / `{python} mem_bw_growth_str` x/year ≈ `{python} mem_wall_ratio_str` x/year

Historically, processors have doubled in compute capacity roughly every `{python} compute_doubling_months_str` months, but memory bandwidth has improved only ~`{python} mem_bw_growth_pct_str`% annually. This widening gap makes data movement the dominant bottleneck and energy cost for most ML workloads. This constraint affects all paradigms but is especially acute for **TinyML**, where devices have only kilobytes of memory to work with.
...
These physical laws explain *why* the four paradigms exist. But given a specific ML workload, *how* do we determine which paradigm fits? The answer requires analytical tools that connect workload characteristics to physical constraints. The following section introduces these tools.

### Analyzing Workloads: The Iron Law and Archetypes {#sec-ml-system-architecture-analyzing-workloads}

\index{Iron Law of ML Systems!components} \index{Silicon Contract!Iron Law}@sec-introduction established the **Iron Law of ML Systems** (@sec-silicon-contract):

$$
T = \frac{D_{vol}}{BW} + \frac{O}{R_{peak} \cdot \eta} + L_{lat}
$$

This equation decomposes total latency into three terms: data movement ($D_{vol}/BW$), compute ($O / (R_{peak} \cdot \eta)$), and fixed overhead ($L_{lat}$). But which term dominates? The answer depends entirely on the deployment environment. A model that is compute-bound during training may become memory-bound during inference; a system that runs efficiently in the cloud may hit power limits on mobile devices.

#### The Bottleneck Principle {#sec-ml-systems-bottleneck-principle}

\index{bottleneck principle!pipelined execution} \index{system bottlenecks!identifying} \index{compute-bound vs memory-bound!definition}To navigate deployment environments, we apply the **Bottleneck Principle**. The Iron Law tells us the cost of each operation, but which operation actually limits performance? Unlike traditional software where optimizing the average case works, ML systems are dominated by their slowest component—optimizing fast operations yields zero benefit while the slowest stage remains unchanged. For pipelined execution (typical in serving), the effective cycle time (which determines throughput) is governed by the slowest stage. While the latency for a single item is the sum of all stages, the system's throughput is limited by the maximum of the stage latencies:

$$ T_{bottleneck} = \max\left(\frac{D_{vol}}{BW}, \frac{O}{R_{peak} \cdot \eta}, T_{network}\right) + L_{lat} $$
...
# PURPOSE
# =============================================================================
# Purpose: Compare energy to transmit vs compute locally.
# Used in: Callout "The Energy of Transmission".

# =============================================================================
# INPUT
# =============================================================================
et_data_mb_value = 1               # MB (1 second of audio)
et_tx_energy_mj_value = 100        # mJ/MB (Wi-Fi/LTE)
...
# OUTPUT
# =============================================================================
et_data_mb_str = fmt(et_data_mb_value, precision=0, commas=False)
et_tx_energy_str = fmt(et_tx_energy_mj_value, precision=0, commas=False)
et_compute_energy_str = fmt(et_compute_energy_mj_value, precision=1, commas=False)
et_cloud_energy_str = str(int(et_cloud_energy_mj_value))
et_local_energy_str = fmt(et_local_energy_mj_value, precision=1, commas=False)
et_energy_ratio_str = f"{et_energy_ratio_value}"
```
...
**The Systems Conclusion**: Transmitting raw data is **`{python} et_energy_ratio_str` x more expensive** than processing it locally. Even if the cloud had infinite speed ($Time \approx 0$), the **Energy Wall** makes cloud offloading physically impossible for always-on battery devices. The "Machine" constraint (Battery) dictates the "Algorithm" choice (TinyML).
:::
...
@sec-introduction presented the Iron Law as a sum. *Why* do we now use $\max()$? The distinction lies in whether operations happen sequentially or overlap. In **Serial Execution**, costs add up—you wait for memory, then compute, then wait for the next batch. However, modern accelerators use **Pipelined Execution** to overlap memory transfers with computation: while the GPU computes on batch $n$, the memory system prefetches batch $n+1$. With perfect overlap, the total time is determined by whichever operation is slower—the other operation effectively "hides" behind it. The Iron Law tells you the *cost of each ingredient*; the Bottleneck Principle tells you the *speed of the assembly line*.
...
[^fn-latency-etymology]: **Latency**: See the etymology in @sec-introduction. For ML systems, latency budgets often determine deployment paradigm: sub-10ms requirements mandate edge deployment, while 100ms+ tolerances permit cloud inference.
...
# PURPOSE
# =============================================================================
# Purpose: Provide lighthouse workload sizes for summary table.
# Used in: Lighthouse Models summary.

# =============================================================================
# INPUT
# =============================================================================
# (constants imported from physx.constants)
...
# Llama range (7B to 70B)
llama_min_b_value = 7
llama_max_b_value = 70

# =============================================================================
# OUTPUT
# =============================================================================
resnet_gflops_str = fmt(resnet_flops_g_value, precision=1, commas=False)
...
llama_range_str = f"{llama_min_b_value} to {llama_max_b_value}"
```

**ResNet-50**\index{ResNet-50!systems characteristics} classifies images into 1,000 categories, processing each image through approximately `{python} resnet_gflops_str` billion floating-point operations using `{python} resnet_params_m_str` million parameters (`{python} resnet_fp32_mb_str` MB at FP32). Its regular, compute-dense structure makes it the canonical benchmark for hardware accelerator performance.

**GPT-2 / Llama**\index{GPT-2!autoregressive bottleneck}\index{Llama!memory-bound inference} generate text one token at a time, requiring the model to read its full parameter set (`{python} gpt2_params_b_str` billion for GPT-2, `{python} llama_range_str` billion for Llama) from memory for each output token. This sequential memory access pattern creates the autoregressive bottleneck that dominates serving costs.

**DLRM**\index{DLRM!memory capacity bound}\index{recommendation systems!DLRM} (Deep Learning Recommendation Model) maps users and items to embedding vectors stored in tables that can exceed `{python} dlrm_embedding_str` GB, making memory capacity rather than computation the binding constraint.

**MobileNet**\index{MobileNet!depthwise separable convolutions}\index{MobileNet!efficiency gains} performs the same image classification task as ResNet but uses depthwise separable convolutions to reduce computation by `{python} mobilenet_flops_reduction_str`×, enabling real-time inference on smartphones at `{python} mobile_tdp_range_str` watts.
...
\index{latency!decision thresholds} \index{latency vs throughput!trade-offs}We have now established the conceptual foundation: four deployment paradigms exist because of physical constraints, and workload archetypes help us match applications to paradigms. Translating this foundation into engineering practice requires quantitative benchmarks. @tbl-latency-numbers provides order-of-magnitude latencies that should inform every deployment decision. Detailed hardware latencies and bandwidth constraints are covered in @sec-ai-acceleration. The key decision rule: if your latency budget is $X$ ms, you cannot use any operation with latency $> X$ in your critical path.
...
# PURPOSE
# =============================================================================
# Purpose: Provide order-of-magnitude latency constants for the table.
# Used in: Table "Latency Numbers for ML System Design".

# =============================================================================
# INPUT
# =============================================================================
# (display constants; already formatted strings)
...
# =============================================================================
# Compute
lat_compute_str = "~1 ns"
lat_npu_str = "5–20 ms"
lat_llm_str = "20–100 ms"

# Memory
lat_l1_str = "~1 ns"
lat_hbm_str = "20–50 ns"
lat_dram_str = "50–100 ns"

# Network
lat_net_dc_str = "0.5 ms"
lat_net_region_str = "1–5 ms"
lat_net_cross_str = "50–150 ms"

# ML Ops
lat_kws_str = "100 μs"
lat_face_str = "10–30 ms"
lat_gpt4_str = "200–500 ms"
lat_train_str = "200–400 ms"
```
...
\index{Iron Law of ML Systems!paradigm comparison} \index{system bottlenecks!dominant constraints}The pipelined form of the **Iron Law of ML Systems**\index{Iron Law of ML Systems!execution time bound} from @sec-silicon-contract states that execution time is bounded by the slowest resource:

$$T = \max\left( \frac{O}{R_{peak} \cdot \eta}, \frac{D_{vol}}{BW}, \frac{D_{vol}}{BW_{IO}} \right) + L_{lat}$$
...
: **D·A·M × Phase**\index{D·A·M taxonomy!training vs inference}: The same model imposes fundamentally different demands on Data, Algorithm, and Machine depending on whether the system is training or serving. When bottlenecks shift unexpectedly, check which phase you're optimizing for. {#tbl-dam-phase}
...
# PURPOSE
# =============================================================================
# Purpose: Compute ResNet-50 sizes for cloud vs mobile comparison.
# Used in: Callout "ResNet-50 on Cloud vs Mobile".
...
# PROCESS
# =============================================================================
resnet_fp32_bytes_value = RESNET50_PARAMS.magnitude * 4 * byte
resnet_fp16_bytes_value = RESNET50_PARAMS.magnitude * 2 * byte
resnet_int8_bytes_value = RESNET50_PARAMS.magnitude * 1 * byte
...
resnet_params_m_str = fmt(resnet_params_m_value, precision=1, commas=False)
resnet_fp32_mb_str = fmt(resnet_fp32_bytes_value.to(MB).magnitude, precision=0, commas=False)
resnet_fp16_mb_str = fmt(resnet_fp16_bytes_value.to(MB).magnitude, precision=0, commas=False)
resnet_int8_mb_str = fmt(resnet_int8_bytes_value.to(MB).magnitude, precision=0, commas=False)
```
...
# PURPOSE
# =============================================================================
# Purpose: Compute cloud (A100) bottleneck for ResNet-50 inference.
# Used in: Callout "ResNet-50 on Cloud vs Mobile".

# =============================================================================
# INPUT
# =============================================================================
# Uses ResNet-50 sizes from resnet-setup block.
...
cloud_stats = calc_bottleneck(
    ops=RESNET50_FLOPs,
    model_bytes=resnet_fp16_bytes_value,
...
# Pre-compute sci() for plain text (Unicode superscripts)
resnet_flops_sci_str = sci(RESNET50_FLOPs)
resnet_fp16_bytes_sci_str = sci(resnet_fp16_bytes_value)

# Pre-compute sci_latex() for use inside LaTeX fractions
# Convert to consistent base units for display (flop, byte, flop/s, byte/s)
...
# Create LaTeX fractions using Markdown() for nice rendering
cloud_compute_frac = md_frac(
    resnet_flops_latex, a100_flops_latex, f"{cloud_compute_ms_value:.3f}", "ms"
)
cloud_memory_frac = md_frac(
    resnet_fp16_bytes_latex, a100_bw_latex, f"{cloud_memory_ms_value:.3f}", "ms"
)
cloud_ai_frac = md_frac(
    resnet_flops_latex, resnet_fp16_bytes_latex, f"{cloud_ai_value:.0f}", "FLOPs/byte"
)

# =============================================================================
# OUTPUT
# =============================================================================
a100_tflops_str = fmt(a100_tflops_value, precision=0, commas=False)
a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=0, commas=False)
...
cloud_ratio_x_str = fmt(cloud_ratio_x_value, precision=0, commas=False)
cloud_bottleneck_str = cloud_bottleneck_value  # Already a string ("Memory" or "Compute")
```
...
- Compute time: Tcomp = `{python} cloud_compute_frac`
- Memory time: Tmem = `{python} cloud_memory_frac`
...
from physx.formulas import calc_bottleneck
from physx.formatting import sci_latex, md_frac
from physx.formatting import fmt
# =============================================================================
# PURPOSE
# =============================================================================
# Purpose: Compute mobile (NPU) bottleneck for ResNet-50 inference.
# Used in: Callout "ResNet-50 on Cloud vs Mobile".
...
mobile_stats = calc_bottleneck(
    ops=RESNET50_FLOPs,
    model_bytes=resnet_int8_bytes_value,
...
inference_speed_x_value = mobile_memory_ms_value / cloud_stats["memory_ms"]
...
mobile_compute_frac = md_frac(
    resnet_flops_latex, mobile_npu_flops_latex, f"{mobile_compute_ms_value:.2f}", "ms"
)
mobile_memory_frac = md_frac(
    resnet_int8_bytes_latex, mobile_npu_bw_latex, f"{mobile_memory_ms_value:.2f}", "ms"
)

# =============================================================================
# OUTPUT
# =============================================================================
mobile_tops_str = fmt(mobile_tops_value, precision=0, commas=False)
mobile_bw_gbs_str = fmt(mobile_bw_gbs_value, precision=0, commas=False)
mobile_ratio_x_str = fmt(mobile_ratio_x_value, precision=0, commas=False)
mobile_bottleneck_str = mobile_bottleneck_value  # Already a string ("Memory" or "Compute")
bw_advantage_x_str = fmt(bw_advantage_x_value, precision=0, commas=False)
inference_speed_x_str = fmt(inference_speed_x_value, precision=0, commas=False)
```
...
- Compute time: Tcomp = `{python} mobile_compute_frac`
- Memory time: Tmem = `{python} mobile_memory_frac`
...
# PURPOSE
# =============================================================================
# Purpose: Format hardware specs for representative systems table.
# Used in: Table "Hardware Spectrum".
...
# =============================================================================
# Cloud
tpu_chips_str = f"{TPU_POD_CHIPS:,}"
cloud_mem_tb_str = fmt(TPU_POD_MEM.to(TB).magnitude, precision=0, commas=False)
cloud_pwr_mw_str = fmt(TPU_POD_POWER.to("megawatt").magnitude, precision=0, commas=False)

# Edge
edge_mem_gb_str = fmt(DGX_RAM.to(GB).magnitude, precision=0, commas=False)
edge_stor_tb_str = fmt(DGX_STORAGE.to(TB).magnitude, precision=0, commas=False)
edge_pwr_w_str = fmt(DGX_POWER.to(watt).magnitude, precision=0, commas=False)
edge_price_min_str = f"{DGX_PRICE_MIN.magnitude:,.0f}"
edge_price_max_str = f"{DGX_PRICE_MAX.magnitude:,.0f}"

# TinyML
tiny_ram_kb_str = fmt(ESP32_RAM.to(KiB).magnitude, precision=0, commas=False)
tiny_flash_mb_str = fmt(ESP32_FLASH.to(MB).magnitude, precision=0, commas=False)
tiny_pwr_min_str = f"{ESP32_POWER_MIN.magnitude}"
tiny_pwr_max_str = f"{ESP32_POWER_MAX.magnitude}"
tiny_price_str = f"{ESP32_PRICE.magnitude}"

# Thresholds
cloud_thresh_tflops_str = "1000"
cloud_thresh_bw_str = "100"
edge_thresh_pflops_str = "1"
edge_thresh_bw_str = "270"
tiny_thresh_tops_str = "1"
tiny_thresh_mw_str = "1"
```

| **Category**  | **Example Device**                        |                                             **Processor** |                             **Memory** | **Storage**                           |                                                 **Power** | **Price Range**                                                                | **Example Models/Tasks**                                       |
|:------------|:----------------------------------------|--------------------------------------------------------:|-------------------------------------:|:------------------------------------|--------------------------------------------------------:|:-----------------------------------------------------------------------------|:-------------------------------------------------------------|
| **Cloud ML**  | Google TPU v4 Pod                         |   `{python} tpu_chips_str` TPU v4 chips (>1 exaflop peak) |    `{python} cloud_mem_tb_str` TB HBM2 | Cloud-scale (PB-scale)                |                           ~`{python} cloud_pwr_mw_str` MW | Cloud service (rental only)                                                    | Large language models, massive-scale training                  |
| **Edge ML**   | NVIDIA DGX Spark[^fn-dgx-spark-edge]      | GB10 Grace Blackwell Superchip (20-core Arm, 1 PFLOPS AI) |  `{python} edge_mem_gb_str` GB LPDDR5x | `{python} edge_stor_tb_str` TB NVMe   |                              ~`{python} edge_pwr_w_str` W | ~\$`{python} edge_price_min_str`–`{python} edge_price_max_str` (2025 estimate) | Model fine-tuning, on-premise inference, prototype development |
| **Mobile ML** | Flagship Smartphone (e.g., iPhone, Pixel) |              Modern mobile SoC (multi-core CPU, GPU, NPU) | `{python} mobile_ram_range_str` GB RAM | `{python} mobile_storage_range_str`   |                         `{python} mobile_tdp_range_str` W | USD 999+                                                                       | Face ID, computational photography, voice recognition          |
| **TinyML**    | ESP32-CAM                                 |                                        Dual-core @ 240MHz |      `{python} tiny_ram_kb_str` KB RAM | `{python} tiny_flash_mb_str` MB Flash | `{python} tiny_pwr_min_str`–`{python} tiny_pwr_max_str` W | \$`{python} tiny_price_str`                                                    | Image classification, motion detection                         |

: **Hardware Spectrum (Concrete Platforms)**\index{hardware spectrum!deployment platforms}\index{TPU!datacenter infrastructure}\index{NVIDIA DGX!edge deployment}\index{ESP32!TinyML platform}: Representative devices that instantiate each deployment paradigm from @tbl-deployment-paradigms-overview. Where the conceptual table defines operating regimes, this table provides the specific processors, memory capacities, power envelopes, and price points that practitioners use to match workloads to hardware. Note: TFLOPS values refer to floating-point operations per second; TOPS values refer to total operations per second including integer (INT8) operations. These metrics are not directly comparable. {#tbl-representative-systems}

[^fn-dgx-spark-edge]: The DGX Spark sits at the high end of the edge spectrum. Most edge deployments use far smaller devices (IoT gateways, compact accelerators like the Jetson Orin Nano). We include it here because it represents the maximum capability available for local, non-cloud deployment — illustrating the *ceiling* of edge computing rather than the typical case.
...
With the physical constraints (Light Barrier, Power Wall, Memory Wall) and analytical tools (**Iron Law**, Bottleneck Principle) established, we now examine each paradigm in depth. The quantitative thresholds in @tbl-deployment-thresholds help practitioners determine which paradigm suits their workload. The following four sections progress from cloud to TinyML, tracing the gradient from maximum computational resources to maximum efficiency constraints.

Each section follows a consistent structure: definition, key characteristics, benefits and trade-offs, and representative applications. This systematic treatment reveals both what distinguishes each paradigm and what principles they share, setting the stage for the hybrid architectures that combine them.

## Cloud ML: Maximizing Computational Power {#sec-ml-system-architecture-cloud-ml-maximizing-computational-power-a338}

\index{Cloud ML!datacenter scale} \index{data centers!ML infrastructure}We begin at the resource-rich end of the spectrum. Cloud ML emerged as the default paradigm for machine learning because it eliminates resource constraints almost entirely: if you can tolerate latency, you can access virtually unlimited compute, memory, and storage. This makes Cloud ML the optimal choice when computational power matters more than response time, and it remains the only viable option for training large models and serving inference workloads that can tolerate network delays.
...
[^fn-nlp-compute]: **NLP Computational Demands**: Modern language models require unprecedented compute: GPT-3 used `{python} gpt3_petaflop_days_str` petaflop-days [@brown2020language], equivalent to `{python} gpt3_v100_count_str` V100 GPUs for approximately `{python} gpt3_days_str` days (estimated at ~$`{python} gpt3_cost_m_str`M at 2020 V100 cloud rates, per external analyses). GPT-4 estimated at 10-100× more. This scale drove hyperscaler investment in specialized infrastructure and raised concerns about AI's environmental impact [@strubell2019energy] and access inequality.

We define this first paradigm formally as *Cloud ML*.

::: {.callout-definition title="Cloud ML"}

***Cloud Machine Learning***\index{Cloud ML!resource elasticity}\index{Cloud ML!network latency constraint}\index{resource elasticity|see{Cloud ML}} is the deployment paradigm that optimizes for **Resource Elasticity**. It decouples computational capacity from physical location, allowing systems to dynamically scale resources proportional to workload variance (bursting to petaflops), fundamentally constrained only by **Network Latency** (Speed of Light) and operational dependence.
:::
...
# PURPOSE
# =============================================================================
# Purpose: Quantify light-speed round-trip latency for safety-critical systems.
# Used in: Callout "The Distance Penalty".

# =============================================================================
# INPUT
# =============================================================================
distance_km_value = 1500
safety_budget_ms_value = 10
...
# OUTPUT
# =============================================================================
sol_kms_str = f"{SPEED_OF_LIGHT_FIBER_KM_S:,}"
rtt_formatted_str = fmt(round_trip_ms_value, precision=0, commas=False)
deficit_str = fmt(deficit_ms_value, precision=0, commas=False)
distance_km_str = f"{distance_km_value:,}"
safety_budget_str = f"{safety_budget_ms_value}"
```
...
Cost management introduces operational complexity requiring total cost of ownership (TCO)\index{Total Cost of Ownership (TCO)!cloud vs. edge}\index{TCO analysis!deployment decisions} analysis rather than naive unit comparisons. A worked *cloud vs. edge TCO* comparison illustrates the gap between sticker price and true system cost.
...
# PURPOSE
# =============================================================================
# Purpose: Compare cloud vs edge total cost of ownership (TCO).
# Used in: Callout "Cloud vs. Edge TCO".

# =============================================================================
# INPUT
# =============================================================================
requests_per_day_value = 1_000_000
inference_ms_value = 10
response_kb_value = 100  # JSON scores, metadata, headers
days_per_year_value = DAYS_PER_YEAR
hours_per_year_value = HOURS_PER_YEAR

# Cloud pricing (AWS 2024)
gpu_price_per_hr_value = 0.75      # A10G (g5.xlarge)
gpu_instances_value = 4            # HA, peak handling, redundancy
egress_per_gb_value = CLOUD_EGRESS_PER_GB.to(USD / GB).magnitude  # After first 100GB/month
lb_base_per_hr_value = 0.025       # ALB hourly
lb_lcu_per_hr_value = 0.008        # LCU-hour (connections, data)
avg_lcu_value = 50                 # Typical for 1M req/day with bursts

# Edge pricing
server_cost_value = 15000
server_life_years_value = 3
...
cooling_overhead_value = 0.30
fiber_annual_value = 1200
devops_fte_value = 0.1
devops_salary_value = 150000
...
c_lb_value = (
    lb_base_per_hr_value * hours_per_year_value
    + lb_lcu_per_hr_value * avg_lcu_value * hours_per_year_value
)
c_logs_value = 2000
...
# PURPOSE
# =============================================================================
# Purpose: Compare cloud vs TinyML costs for wake-word detection.
# Used in: Callout "Wake-Word Economics".

# =============================================================================
# INPUT
# =============================================================================
ww_devices_b_value = 1  # billion devices
ww_cloud_cost_per_device_value = 0.50  # $ per device/year

ww_edge_power_min_mw_value = 0.1  # mW
ww_edge_power_max_mw_value = 1    # mW
ww_edge_cost_per_year_value = 0.01  # $ per year (battery cost)
...
# OUTPUT
# =============================================================================
ww_devices_b_str = fmt(ww_devices_b_value, precision=0, commas=False)
ww_cloud_cost_str = fmt(ww_cloud_cost_per_device_value, precision=2, commas=False)
ww_total_cost_str = fmt(ww_total_cloud_cost_value, precision=0, commas=True)
ww_edge_power_range_str = f"{ww_edge_power_min_mw_value}--{ww_edge_power_max_mw_value}"
ww_edge_cost_str = fmt(ww_edge_cost_per_year_value, precision=2, commas=False)
```

::: {.callout-notebook title="Wake-Word Economics"}
**Scenario**: `{python} ww_devices_b_str` billion voice assistant devices using continuous audio streaming.

**Cloud-Only Approach**

- **Compute Cost**: ~USD `{python} ww_cloud_cost_str` per device/year (conservative estimate)
- **Total Annual Cost**: `{python} ww_devices_b_str`,000,000,000 × USD `{python} ww_cloud_cost_str` = USD `{python} ww_total_cost_str`
- **Feasibility**: Economically prohibitive for a free/low-cost feature.

**Edge (TinyML) Approach**

- **Power Consumption**: `{python} ww_edge_power_range_str` mW for local wake-word detection
- **Cost Shift**: Transferred to user's battery (<USD `{python} ww_edge_cost_str` per year)
...
# PURPOSE
# =============================================================================
# Purpose: Compute bandwidth + egress costs for factory camera streaming.
# Used in: Callout "The Bandwidth Bottleneck".

# =============================================================================
# INPUT
# =============================================================================
num_cameras_value = 100

# =============================================================================
# PROCESS
# =============================================================================
raw_bytes_per_frame_value = (
...
monthly_cost_m_str = fmt(monthly_cost_value / 1e6, precision=1, commas=False)
net_cap_gbs_str = fmt(NETWORK_10G_BW.to(GB/second).magnitude, precision=2, commas=False)
bw_short_x_str = fmt(bw_shortage_value, precision=0, commas=False)
...
The bandwidth calculation above reveals why edge processing is mandatory for high-volume sensor deployments. However, bandwidth is only half the story. Even when network capacity *exists*, transmitting data costs energy—and for battery-powered edge devices (wireless cameras, drones, wearables), the energy cost of radio transmission creates an even more severe constraint. While a wired edge server can process unlimited data within its bandwidth budget, a battery-powered device faces a fundamental trade-off between connectivity and operational lifetime.
...
# PURPOSE
# =============================================================================
# Purpose: Compare transmission vs local compute energy for 1MB video.
# Used in: Callout "The Energy Gap".

# =============================================================================
# INPUT
# =============================================================================
data_size_value = 1 * MB

# =============================================================================
# PROCESS
# =============================================================================
tx_energy_total_value = NETWORK_5G_ENERGY_PER_MB_MJ * data_size_value
npu_energy_total_value = ENERGY_MOBILENET_INF_MJ
energy_gap_ratio_value = tx_energy_total_value / npu_energy_total_value

# =============================================================================
# OUTPUT
# =============================================================================
tx_energy_str = fmt(tx_energy_total_value.to(ureg.millijoule).magnitude, precision=0, commas=False)
npu_energy_str = f"{npu_energy_total_value.to(ureg.millijoule).magnitude}"
energy_gap_str = fmt(energy_gap_ratio_value.magnitude, precision=0, commas=False)
```

::: {.callout-notebook title="The `{python} energy_gap_str`× Energy Gap"}
**Problem**: Should your smart camera process video locally or send it to the cloud?

**The Physics (per 1MB of video)**:

1.  **Transmission (5G)**: Sending 1MB consumes ≈ **`{python} tx_energy_str` mJ** (milli-Joules).
2.  **Computation (Edge NPU)**: Running a MobileNet inference on that 1MB consumes ≈ **`{python} npu_energy_str` mJ**.
3.  **The Gap**: Transmitting data is **`{python} energy_gap_str`× more energy-expensive** than processing it locally.

**The Engineering Conclusion**: In battery-powered nodes, the bottleneck is not just "Time" (latency); it is **Energy Bandwidth**. Even if the cloud had 0ms latency, the energy cost of transmission would kill the device's battery `{python} energy_gap_str`× faster than Edge ML.
:::
...
#| echo: false
#| label: edge-sizing
from physx.constants import YOLOV8_NANO_FLOPs, GFLOPs, CLOUD_ELECTRICITY_PER_KWH
...
# PURPOSE
# =============================================================================
# Purpose: Size edge inference for retail deployment (YOLOv8).
# Used in: Edge ML sizing example.

# =============================================================================
# INPUT
# =============================================================================
stores_value = 500
cameras_per_store_value = 20
fps_value = 15
headroom_value = 2  # 2× engineering headroom

coral_cost_value = 150
coral_power_w_value = 2
coral_tops_value = 4
jetson_cost_value = 600
jetson_power_w_value = 25  # midpoint of 10-40W
jetson_power_range_str = "10-40"
jetson_tops_value = 100
nuc_cost_value = 400
nuc_power_w_value = 15
nuc_tops_value = 4
years_value = 3
hours_per_year_value = HOURS_PER_YEAR
cloud_inference_cost_k_value = 500  # approx cloud cost for comparison
int8_throughput_mult_value = 4  # INT8 quantization throughput multiplier

# =============================================================================
# PROCESS
# =============================================================================
inferences_per_sec_value = cameras_per_store_value * fps_value
sustained_gflops_value = (
    inferences_per_sec_value * YOLOV8_NANO_FLOPs.to(GFLOPs).magnitude
)
...
coral_fleet_value = coral_cost_value * stores_value
jetson_fleet_value = jetson_cost_value * stores_value
coral_power_cost_value = coral_tco_value - coral_fleet_value

# =============================================================================
# OUTPUT
# =============================================================================
stores_str = f"{stores_value}"
cameras_per_store_str = f"{cameras_per_store_value}"
fps_str = f"{fps_value}"
headroom_str = f"{headroom_value}"
inf_per_sec_str = f"{inferences_per_sec_value}"
yolo_gflops_str = fmt(YOLOV8_NANO_FLOPs.to(GFLOPs).magnitude, precision=1, commas=False)
sustained_gf_str = fmt(sustained_gflops_value, precision=0, commas=False)
req_tflops_str = fmt(required_tflops_value, precision=0, commas=False)
coral_cost_str = f"{coral_cost_value}"
coral_power_w_str = f"{coral_power_w_value}"
coral_tops_str = f"{coral_tops_value}"
jetson_cost_str = f"{jetson_cost_value}"
jetson_tops_str = f"{jetson_tops_value}"
nuc_cost_str = f"{nuc_cost_value}"
nuc_power_w_str = f"{nuc_power_w_value}"
nuc_tops_str = f"{nuc_tops_value}"
coral_fleet_k_str = fmt(coral_fleet_value / 1000, precision=0, commas=False)
jetson_fleet_k_str = fmt(jetson_fleet_value / 1000, precision=0, commas=False)
nuc_fleet_k_str = fmt(nuc_cost_value * stores_value / 1000, precision=0, commas=False)
coral_pwr_k_str = fmt(coral_power_cost_value / 1000, precision=0, commas=False)
coral_tco_k_str = fmt(coral_tco_value / 1000, precision=0, commas=False)
years_str = f"{years_value}"
hours_per_year_str = f"{hours_per_year_value}"
power_ratio_str = fmt(jetson_power_w_value / coral_power_w_value, precision=0, commas=False)
elec_cost_str = f"{CLOUD_ELECTRICITY_PER_KWH.magnitude}"
cloud_cost_k_str = f"{cloud_inference_cost_k_value}"
int8_mult_str = f"{int8_throughput_mult_value}"
cost_ratio_str = f"{jetson_cost_value // coral_cost_value}"
```
...
The mobile environment introduces a critical constraint absent from stationary deployments: *energy per inference* becomes a first-order design parameter. A **Compute Beast** workload like image classification must be transformed through architectural efficiency (e.g., depthwise separable convolutions[^fn-depthwise-separable] in MobileNet) to reduce FLOPs by `{python} mobilenet_flops_reduction_str`× while preserving accuracy. This is not merely optimization; it represents a fundamental shift in the arithmetic intensity trade-off, accepting lower peak throughput in exchange for sustainable operation within a `{python} mobile_tdp_range_str` W thermal envelope.
...
from physx.formatting import md_frac
from physx.formatting import fmt

# =============================================================================
# PURPOSE
# =============================================================================
# Purpose: Quantify battery tax of always-on mobile ML.
# Used in: Callout "The Battery Tax".
...
# PROCESS
# =============================================================================
runtime_q_value = PHONE_BATTERY_WH / OBJECT_DETECTOR_POWER_W
daily_budget_pct_value = (OBJECT_DETECTOR_POWER_W * runtime_q_value) / PHONE_BATTERY_WH * 100

# =============================================================================
# OUTPUT
# =============================================================================
runtime_str = fmt(runtime_q_value.to(ureg.hour).magnitude, precision=1, commas=False)
pwr_w_str = fmt(OBJECT_DETECTOR_POWER_W.to(ureg.watt).magnitude, precision=0, commas=False)
batt_wh_str = fmt(PHONE_BATTERY_WH.to(ureg.watt * ureg.hour).magnitude, precision=0, commas=False)
budget_pct_str = fmt(daily_budget_pct_value.magnitude, precision=0, commas=False)
...
# PURPOSE
# =============================================================================
# Purpose: Quantization power reduction example for thermal limits.
# Used in: Callout "The Thermal Wall".

# =============================================================================
# INPUT
# =============================================================================
baseline_power_w_value = 12
quant_reduction_value = 4

# =============================================================================
# PROCESS
# =============================================================================
quant_power_w_value = baseline_power_w_value / quant_reduction_value

# =============================================================================
# OUTPUT
# =============================================================================
baseline_str = fmt(baseline_power_w_value, precision=0, commas=False)
quant_power_str = fmt(quant_power_w_value, precision=0, commas=False)
...
[^fn-microcontrollers-specs]: **Microcontrollers**: Single-chip computers with integrated CPU, memory, and peripherals, typically operating at 1-100MHz with 32KiB-2MiB RAM. Arduino Uno uses an ATmega328P with 32KiB flash and 2KiB RAM, while ESP32 provides WiFi capability with 520KiB RAM, still thousands of times less than a smartphone.

[^fn-energy-efficiency]: **Energy Efficiency in TinyML**: Ultra-low power enables decade-long deployment in remote locations. ARM Cortex-M0+ consumes <1µW in sleep, 100-300µW/MHz active. Specialized accelerators (Syntiant NDP, MAX78000) achieve <1µJ per inference.
...
# PURPOSE
# =============================================================================
# Purpose: Calculate energy per inference and battery life for comparison table.
# Used in: Table "Energy Per Inference".

# =============================================================================
# INPUT
# =============================================================================
# Energy per inference (approximate values for table)
e_gpt4_j = 1000 * ureg.joule
e_resnet_cloud_j = 10 * ureg.joule
e_resnet_edge_j = 0.5 * ureg.joule
e_mobilenet_j = 0.05 * ureg.joule
e_kws_j = 0.00001 * ureg.joule
...
q_kws_str = fmt(q_kws.magnitude / 1e9, precision=0, commas=False) + " billion"
...
\index{deployment paradigms!quantitative comparison} \index{latency vs throughput!paradigm trade-offs}The relationship between computational resources and deployment location forms one of the most consequential comparisons across ML systems. Moving from cloud deployments to tiny devices reveals a dramatic reduction in available computing power, storage, and energy consumption. Cloud ML systems leverage virtually unlimited resources, processing petabytes of data and training models with billions of parameters. Edge ML systems, while more constrained, still offer significant computational capability through specialized hardware such as edge GPUs and neural processing units. Mobile ML balances computational power with energy efficiency on smartphones and tablets. At the far end of the spectrum, TinyML operates under severe resource constraints, often limited to kilobytes of memory and milliwatts of power consumption.
...
# PURPOSE
# =============================================================================
# Purpose: Provide qualitative trade-off descriptors for paradigm table.
# Used in: Table "Big vs Tiny" comparison.
...
# =============================================================================
# Latency
cloud_lat_str = "100 ms-1000 ms+"
edge_lat_str = "10-100 ms"
mobile_lat_str = "5-50 ms"
tiny_lat_str = "1-10 ms"

# Compute
cloud_comp_str = "Very High (Multiple GPUs/TPUs)"
edge_comp_str = "High (Edge GPUs)"
mobile_comp_str = "Moderate (Mobile NPUs/GPUs)"
tiny_comp_str = "Very Low (MCU/tiny processors)"

# Storage
cloud_stor_str = "Unlimited (petabytes+)"
edge_stor_str = "Large (terabytes)"
mobile_stor_str = "Moderate (gigabytes)"
tiny_stor_str = "Very Limited (kilobytes-megabytes)"

# Energy
cloud_pwr_str = "Very High (kW-MW range)"
edge_pwr_str = "High (100 s W)"
mobile_pwr_str = "Moderate (1-10 W)"
tiny_pwr_str = "Very Low (mW range)"

# Cost
cloud_cost_str = "High ($1000s+/month)"
edge_cost_str = "Moderate ($100s-1000s)"
mobile_cost_str = "Low ($0-10s)"
tiny_cost_str = "Very Low ($1-10s)"
```
...
Additional patterns including federated and collaborative learning, which enable privacy-preserving distributed training across devices, constitute advanced topics that build on the single-node foundations covered here.
:::
...
The remaining chapters explore each layer: @sec-data-engineering-ml examines data pipelines, @sec-model-compression addresses optimization, and @sec-machine-learning-operations-mlops covers operational aspects. All of these apply whether you deploy to a TPU Pod or an ESP32.

### From Deployment to Operations {#sec-ml-system-architecture-from-deployment-to-operations}

The preceding sections addressed *where* to deploy ML systems and *how* to combine paradigms effectively. But deployment is not the end of the engineering challenge—it is the beginning of a new one.

Traditional software, once deployed correctly, remains correct indefinitely. A sorting algorithm that works today will work tomorrow, next year, and a decade from now. ML systems face a fundamentally different reality: **System Entropy (statistical decay)**\index{system entropy!model decay}\index{model drift|see{system entropy}}.
...
# PURPOSE
# =============================================================================
# Purpose: Compare cloud vs edge latency to show the "latency mirage".
# Used in: Fallacy "The Latency Mirage".

# =============================================================================
# INPUT
# =============================================================================
cloud_network_ms_value = 50   # Cross-region round-trip
cloud_inference_ms_value = 10 # A100 GPU inference
edge_network_ms_value = 5     # Local network
edge_inference_ms_value = 80  # Raspberry Pi inference (8× slower than A100)

# =============================================================================
# PROCESS
# =============================================================================
cloud_total_ms_value = cloud_network_ms_value + cloud_inference_ms_value
edge_total_ms_value = edge_network_ms_value + edge_inference_ms_value

# =============================================================================
# OUTPUT
# =============================================================================
cloud_net_str = f"{cloud_network_ms_value}"
cloud_inf_str = f"{cloud_inference_ms_value}"
cloud_total_str = fmt(cloud_total_ms_value, precision=0, commas=False)
edge_net_str = f"{edge_network_ms_value}"
edge_inf_str = f"{edge_inference_ms_value}"
edge_total_str = fmt(edge_total_ms_value, precision=0, commas=False)
```

::: {.callout-example title="The Latency Mirage"}

\index{latency mirage!edge vs cloud}**Fallacy:** *Edge deployment always reduces latency compared to cloud.*
...
*   **Cloud Path**: `{python} cloud_net_str` ms network + `{python} cloud_inf_str` ms inference (A100) = **`{python} cloud_total_str` ms total**.
*   **Edge Path**: `{python} edge_net_str` ms network + `{python} edge_inf_str` ms inference (Raspberry Pi) = **`{python} edge_total_str` ms total**.
...
##### Fallacy: *Model optimization overcomes mobile device power and thermal limits.* {.unnumbered}

Compression techniques do not scale indefinitely against physics. A smartphone with a `{python} phone_battery_str` Wh battery running a 1 W inference workload depletes in 15 hours, but a 5 W workload (common for large on-device models) exhausts the battery in 3 hours while triggering thermal throttling that reduces performance by 40--60 percent. As @sec-ml-system-architecture-mobile-ml-benefits-resource-constraints-c568 establishes, sustained mobile inference cannot exceed 2--3 W without active cooling. Reducing numerical precision (using fewer bits to represent each weight) cuts power by approximately 4x, but aggressive precision reduction often causes 5--10 percent accuracy loss. Applications requiring continuous inference beyond mobile thermal envelopes remain physically impossible regardless of algorithmic improvements.

##### Fallacy: *TinyML represents scaled-down mobile ML.* {.unnumbered}

The difference is qualitative, not just quantitative. As @sec-ml-system-architecture-tinyml-advantages-operational-tradeoffs-2d40 establishes, TinyML microcontrollers provide 256 KB to 1 MB of memory versus mobile devices with 4--12 GB, a 10,000x difference requiring fundamentally different algorithms. Mobile ML uses reduced-precision arithmetic with minimal accuracy loss; TinyML requires extreme precision reduction that sacrifices 10--15 percent accuracy for 32x memory reduction. Mobile devices run models with millions of parameters; TinyML models contain 10,000--100,000 parameters, demanding distinct architectural choices such as specialized lightweight operations designed to minimize multiply-accumulate counts. Power budgets show similar discontinuities: mobile inference consumes 1--5 W, while TinyML targets 1--10 mW for battery-free energy harvesting. These thousand-fold gaps make TinyML a distinct problem class, not a smaller version of mobile ML.

##### Pitfall: *Minimizing computational resources minimizes total cost.* {.unnumbered}
...
##### Fallacy: *More training data always improves deployed model performance.* {.unnumbered}

\index{scaling laws!data limitations}Three constraints limit data scaling benefits. First, model size limits what can be learned: a keyword spotting model with 250K parameters achieves 95% accuracy on 50K samples but only 96.5% on 1M samples, a 0.3% gain for 5x more data, storage, and labeling cost. The model simply cannot represent more complex patterns. Second, data quality dominates quantity: 1M curated samples often outperform 100M noisy web-scraped samples, because mislabeled examples and misleading patterns degrade performance even as dataset size grows. Third, deployment distribution matters more than training scale: a model trained on 1B web images may perform worse on medical imaging than one trained on 100K domain-specific samples. Effective data strategy requires understanding the capacity-data relationship for the target model and deployment domain, not simply maximizing dataset scale.
...
The evolution from centralized cloud systems to distributed edge and mobile deployments—from megawatt data centers to milliwatt microcontrollers—illustrates how resource constraints drive innovation. Each paradigm emerged to address specific limitations: Cloud ML leverages centralized power for complex processing but must accept latency and privacy costs. Edge ML brings computation closer to data sources, reducing latency while introducing intermediate resource constraints. Mobile ML extends these capabilities to personal devices, balancing user experience with battery life and thermal management. TinyML enables ubiquitous sensing and intelligence with minimal resources.

::: {.callout-takeaways title="Key Takeaways"}

* **Physical constraints are permanent**\index{physical constraints!permanent boundaries}: Speed of light (5ms cross-country), power wall, and memory wall create hard boundaries that engineering cannot overcome—only navigate.
* **Identify bottlenecks before optimizing**\index{bottleneck principle!optimization strategy}: The same model is compute-bound in training but memory-bound in inference. Use the Iron Law and Bottleneck Principle first; analytical estimates often outperform prototype-based approaches.
...
* **Hybrid architectures are prevalent in production systems**\index{hybrid architectures!voice assistant example}: Voice assistants span TinyML (wake-word), Mobile (speech-to-text), and Cloud (language understanding). Rarely does one paradigm suffice.
* **Latency budgets reveal feasibility**\index{latency budgets!feasibility analysis}: 100ms round-trip to cloud eliminates real-time applications; 10ms edge inference enables them. Deployment paradigms must be matched to latency requirements.

:::

As these deployment models mature, hybrid architectures emerge that combine their strengths: cloud-based training paired with edge inference, federated learning across mobile devices, and hierarchical processing that optimizes across the entire spectrum. The question that opened this chapter now has a clear answer: *why we cannot simply deploy the best model everywhere*: because the speed of light, the power wall, and the memory wall are permanent physical constraints, not temporary engineering challenges. The deployment spectrum exists because physics demands it.
