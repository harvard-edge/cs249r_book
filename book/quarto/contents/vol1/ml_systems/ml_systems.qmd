...
\begin{marginfigure}
\mlsysstack{60}{25}{30}{15}{40}{45}{70}{35}
\end{marginfigure}
...
- Apply the Iron Law and Bottleneck Principle to determine whether a workload is compute-bound, memory-bound, or I/O-bound, and map workload archetypes to deployment paradigms using Lighthouse Model examples.
...
What makes these systems so different? The physical constraints that govern each environment—latency, power, and memory—force ML deployment into four distinct paradigms: Cloud, Edge, Mobile, and TinyML, each with its own engineering trade-offs and system design patterns. The next subsection introduces these paradigms concretely before examining the physics behind them.
...
As these deployment models mature, hybrid architectures emerge that combine their strengths: cloud-based training paired with edge inference, hierarchical processing that cascades intelligence across tiers, and progressive deployment that compresses a single model for each target environment. Beyond architecture, every deployed model faces **System Entropy**—accuracy degradation as the world drifts from its training distribution—making monitoring and retraining infrastructure as important as the model itself. The question that opened this chapter now has a clear answer: *why we cannot simply deploy the best model everywhere*: because the speed of light, the power wall, and the memory wall are permanent physical constraints, not temporary engineering challenges. The deployment spectrum exists because physics demands it. The decision framework (@fig-mlsys-playbook-flowchart) and 14-dimension comparison (@tbl-big_vs_tiny) provide practical tools for navigating it.
