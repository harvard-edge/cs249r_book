...
|:---|---:|:---|:---|---:|
| **NVIDIA Jetson Orin NX** | 100 TOPS | 10-40 W | \$`{python} jetson_cost_str` | \$`{python} jetson_fleet_k_str`,000 |
| **Intel NUC + Movidius** | 4 TOPS | `{python} nuc_power_w_str` W | \$`{python} nuc_cost_str` | \$`{python} nuc_fleet_k_str`,000 |
| **Google Coral Dev** | 4 TOPS | `{python} coral_power_w_str` W | \$`{python} coral_cost_str` | \$`{python} coral_fleet_k_str`,000 |

**Decision**: At `{python} req_tflops_str` TFLOPS required and INT8 quantization providing ~4× effective throughput, the Coral Dev Board (4 TOPS) meets requirements at 1/4 the cost of Jetson, with `{python} power_ratio_str`× lower power consumption.

**TCO over `{python} years_str` years** (Coral): Hardware \$`{python} coral_fleet_k_str`K + Power (\$`{python} coral_power_w_str`×`{python} stores_str`×8760h×`{python} years_str`yr×\$`{python} elec_cost_str`/kWh) = \$`{python} coral_fleet_k_str`K + \$`{python} coral_pwr_k_str`K = **\$`{python} coral_tco_k_str`,000 total** vs. cloud inference at ~\$500K.
:::
