...
To navigate these operating regimes, we anchor our engineering decisions in a four-layer model of the **Single-Node Stack**\index{Single-Node Stack!architecture layers}, which refines the Machine side of the D·A·M taxonomy for one host while Data and Algorithm enter through workload requirements. At the top of the stack, the application defines the mission: the training throughput or inference latency targets whose mechanisms @sec-model-training and @sec-model-serving work out. The ML framework\index{ML Framework!compiler role} translates model code into executable operations (@sec-ml-frameworks), the operating system\index{Operating System!runtime role} orchestrates resources and moves data between host memory and accelerator memory, and the hardware itself, defined by high-bandwidth memory (HBM) capacity, memory bandwidth, intra-node interconnects such as NVLink, and compute throughput, sets the physical limits, with the memory wall as the primary constraint (@sec-hardware-acceleration). Every chapter in the first half of this text interrogates one or more of these layers, because understanding how they interact within a single machine is the technical prerequisite for mastering larger distributed scales. Mastery of this single-node regime establishes the "silicon contract": the fixed performance bargain a given piece of silicon offers a model, set by its memory bandwidth, peak compute rate, and fixed overhead, which governs all subsequent optimization and scaling efforts.
...
# │
# │ Imports: mlsysim.core.units, mlsysim.physics, mlsysim.book
# │ Exports: min_latency_str, distance_km_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.physics import calc_network_latency_ms
from mlsysim.fmt import fmt, check, fmt_time, fmt_length, MarkdownStr
...
# │
# │ Imports: mlsysim.core.units (Platforms.Mobile.tdp_range_w)
...
***The iron law***\index{Iron Law!definition} is the fundamental physical constraint governing machine learning system time, expressed as the total time $T$ required for a workload:

$$T = \frac{D_{\text{vol}}}{\text{BW}} + \frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}} + L_{\text{lat}}$$

1.  **Significance (quantitative)**: It defines the physical ceiling for any system by quantifying the relationship between data volume $(D_{\text{vol}})$, compute capacity $(R_{\text{peak}})$, and fixed overhead $(L_{\text{lat}})$.
2.  **Observation**: Unlike Amdahl's Law, which focuses on parallel speedup, the iron law binds model work to data movement and fixed overhead at the deployment boundary.
3.  **Pitfall**: A frequent misconception is that these terms are independent. In reality, they are Trade-off Axes: for example, increasing batch size may improve the duty cycle $(\eta_{\text{hw}})$ but also increase the data volume $(D_{\text{vol}})$ per request, potentially shifting a compute-bound problem to a memory-bound one.
...
The first lighthouse, **ResNet-50**\index{ResNet-50!systems characteristics}, classifies images into 1,000 categories, processing each image through approximately `{python} LighthouseModels.resnet_gflop_str` using `{python} LighthouseModels.resnet_params_m_str` (`{python} LighthouseModels.resnet_fp32_mb_str` at FP32). Used in medical imaging diagnostics, autonomous vehicle perception pipelines, and as the backbone for content moderation systems, its regular, compute-dense structure makes it the canonical benchmark for hardware accelerator performance.
...
A quantitative comparison applies this analysis to ResNet-50 inference on a high-end data center GPU (NVIDIA A100 class) and a mobile NPU. Treat these as point-in-time hardware anchors: the arithmetic is about compute rate, memory bandwidth, and batch size, while @sec-hardware-acceleration explains the accelerator architecture behind the numbers.

::: {#nbk-ml-systems-resnet-50-cloud-vs-mobile .callout-notebook title="ResNet-50 on cloud vs. mobile"}

**Problem**: Is ResNet-50 inference\index{ResNet-50!cloud vs. mobile} compute bound or memory bound on (a) a high-end data center GPU (NVIDIA A100 class) and (b) a flagship mobile NPU?
...
# │
# │ Imports: mlsysim Systems, Hardware, ReferenceStats, units, and formatters
# │ Exports: HardwareSpectrumSetup.tpu_chip_count_str,
...
# │          HardwareSpectrumSetup.tiny_price_usd_str,
...
class HardwareSpectrumSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cloud_compute_envelope_str = MarkdownStr(
        f"{tpu_chip_count_str}, " f"{fmt_flop_rate(cloud_peak_flops, unit=EFLOP / second, precision=1, commas=False)}"
    )
    edge_compute_envelope_str = MarkdownStr(
        f"GB10 Grace Blackwell, " f"{fmt_flop_rate(edge_peak_ai_flops, unit=PFLOP / second, precision=0, commas=False)} AI"
    )
```
...
# │
# │ Imports: mlsysim Platforms and formatters
...
# │      phone twins.
# │
# │ Imports: mlsysim Platforms, Hardware, units, and formatters
...
class DeploymentThresholdsTable:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_thresh_tflop_per_s_str = MarkdownStr(f">{fmt_flop_rate(cloud_compute_threshold, unit=TFLOPs / second, precision=0, commas=False)}")
    cloud_thresh_bw_gb_per_s_str = MarkdownStr(f">{fmt_bandwidth(cloud_bandwidth_threshold, unit=GB / second, precision=0, commas=False)}")
    cloud_latency_range_str = MarkdownStr(Platforms.Cloud.latency_range_ms)
    edge_thresh_pflop_per_s_str = MarkdownStr(f"~{fmt_flop_rate(edge_compute_threshold, unit=PFLOP / second, precision=0, commas=False)}")
    edge_thresh_bw_gb_per_s_str = MarkdownStr(f">{fmt_bandwidth(edge_bandwidth_threshold, unit=GB / second, precision=0, commas=False)}")
    edge_latency_range_str = MarkdownStr(Platforms.Edge.latency_range_ms)
    ...
    mobile_tdp_range_str = MarkdownStr(f"{Platforms.Mobile.tdp_range_w} W")
    mobile_latency_range_str = MarkdownStr(Platforms.Mobile.latency_range_ms)
    tiny_thresh_tops_str = MarkdownStr(f"<{fmt_ops_rate(tiny_compute_threshold, unit=TOPS, precision=0, commas=False)}")
    tiny_thresh_mw_str = MarkdownStr(f"<{fmt_power(tiny_power_threshold, unit=milliwatt, precision=0, commas=False)}")
    tiny_latency_range_str = MarkdownStr(Platforms.Tiny.latency_range_ms)
```
...
2.  **Observation**: Unlike edge ML, which prioritizes latency determinism and data locality at fixed $R_{\text{peak}}$, cloud ML prioritizes elastic $R_{\text{peak}}$ at the cost of variable $L_{\text{lat}}$.
3.  **Pitfall**: A frequent misconception is that cloud ML is "unlimited compute." In reality, the distance penalty $(L_{\text{lat}})$ and the ingestion bottleneck $(D_{\text{vol}}/\text{BW})$ are physics constraints that no software optimization can eliminate, setting a hard floor on response time for any workload whose data originates outside the data center.
...
# │
# │ Imports: mlsysim.core.units (DAYS_PER_YEAR, HOURS_PER_YEAR, Infrastructure.Pricing.Cloud.EgressPerGB.rate,
...
# │
# │ Imports: mlsysim.core.units (BILLION, TRILLION, SEC_PER_HOUR, HOURS_PER_DAY,
...
2.  **Observation**: Unlike Cloud ML, which prioritizes Throughput, edge ML prioritizes Determinism and privacy. Unlike TinyML, edge ML may still use workstation-class accelerators (GPGPUs).
3.  **Pitfall**: A frequent misconception is that edge ML refers to a specific hardware class. In reality, it is a Location Paradigm: it spans from IoT gateways to on-premise servers, unified by physical proximity to the data source.
...
# │
# │ Imports: mlsysim.core.units (VIDEO_1080P_WIDTH, VIDEO_1080P_HEIGHT,
...
2.  **Observation**: Unlike the iron law, which decomposes execution time into additive terms for any workload, the data locality invariant is a binary feasibility test: it determines whether remote offloading is architecturally viable before any optimization of the individual terms begins.
3.  **Pitfall**: A frequent misconception is that 5G/6G "solves" locality. While these technologies improve $\text{BW}_{\text{network}}$, they do not reduce $L_{\text{lat,network}}$ below the speed-of-light floor, meaning latency-critical tasks remain inherently local regardless of link bandwidth.
...
# │
# │ Imports: mlsysim.core.units (VIDEO_4K_WIDTH, VIDEO_4K_HEIGHT,
# │          VIDEO_BYTES_PER_PIXEL_RGB, MB, byte, bit, megabit, second,
# │          millisecond), mlsysim.fmt (fmt, check, ...)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, MarkdownStr, fmt_bandwidth, fmt_qty, fmt_time, fmt_rate
from mlsysim.core.units import (
    VIDEO_4K_WIDTH,
    VIDEO_4K_HEIGHT,
...
# │
# │ Imports: mlsysim.Models, mlsysim.core.units, mlsysim.book (fmt, check)
...
2.  **Observation**: Unlike Edge ML, which may have active cooling, mobile ML must operate within a Personal Energy Budget. Unlike TinyML, it still provides a rich OS and multi-watt compute capacity.
3.  **Pitfall**: A frequent misconception is that mobile ML performance is a fixed value. In reality, it is a Time-Varying Constraint: performance often drops as the device hits its thermal wall, triggering throttling that reduces the duty cycle $(\eta_{\text{hw}})$.
...
# │
# │ Imports: ReferenceStats.MobilePower.ObjectDetector (+ MobileNpuTypical), mlsysim.core.units
...
# │
# │ Imports: mlsysim.Hardware, mlsysim.core.units, mlsysim.book (fmt, check)
...
# │      from ReferenceStats.
# │
# │ Imports: mlsysim Models, ReferenceStats, units, and formatters
# │ Exports: MobileResnetPitfallRecap.resnet_fp32_mb_str,
# │          MobileResnetPitfallRecap.resnet_gflop_str,
# │          MobileResnetPitfallRecap.edge_ram_mb_str,
# │          MobileResnetPitfallRecap.edge_compute_gflop_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
[^fn-ubiquitous-computing]: **Ubiquitous Computing**\index{Ubiquitous Computing!etymology}\index{Ubiquitous Computing!TinyML realization}: Mark Weiser's vision of "invisible" technology is achieved when the cost and power of an intelligent sensor become so low that the economic barrier to mass deployment vanishes. This forces the optimization objective to shift from performance (throughput) to power (energy per inference), the central trade-off of the Tiny Constraint archetype. A keyword spotter achieving a million-fold energy efficiency gain can thus operate for years on a coin-cell battery, making ubiquitous intelligence practical.
...
2.  **Observation**: Unlike Mobile ML, which uses multi-watt processors and a full OS, TinyML runs on Microcontrollers (MCUs) with no operating system abstraction.
3.  **Pitfall**: A frequent misconception is that TinyML is just "small models." In reality, it is an Energy-Bound Paradigm: the primary metric is Energy per Inference (microjoules), not just the parameter count.
...
# │
# │ Imports: mlsysim (Models, Hardware, Engine), mlsysim.core.units
...
from mlsysim.fmt import fmt_int, fmt, check, MarkdownStr, fmt_count, fmt_qty, fmt_multiple
...
# │
# │ Imports: mlsysim.core.units (GB, KB), mlsysim.book (fmt, check)
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @tbl-big_vs_tiny -- 14-dimension paradigm comparison
# │
...
# │ Exports: BrakingDistance.speed_kmh_str, BrakingDistance.distance_m_str,
# │          BrakingDistance.latency_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_time, fmt_length, fmt_rate
...
class BrakingDistance:
    ...
    detection_fps_str = fmt_rate(detection_fps, "FPS", precision=0, commas=False)
...
2.  **Observation**: Unlike cloud-only deployment (which accepts the distance penalty for all stages) or edge-only deployment (which accepts limited $R_{\text{peak}}$ for all stages), hybrid ML dynamically assigns each pipeline stage to the tier where its binding iron law term is minimized.
3.  **Pitfall**: A frequent misconception is that hybrid ML is just "running two models." In reality, the two tiers must share synchronized state—feature definitions, model versions, and preprocessing logic—so that the edge and cloud paths produce consistent results. Without this synchronization, training-serving skew emerges at the tier boundary.
...
**Failure mode**: The company concluded that home-price forecasting had become more unpredictable than expected. Scaling the automated buying operation under that uncertainty created inventory and balance-sheet volatility that the business could not tolerate.
...
# │
# │ Imports: mlsysim.Hardware, mlsysim.core.units, mlsysim.book (fmt, fmt_frac)
...
class AmdahlCameraCalc:
    ...
    cam_ml_pct_str = fmt_percent(cam_ml_fraction, precision=0, commas=False, style="prose")
