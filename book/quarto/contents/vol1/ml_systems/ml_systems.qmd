...
kws_params_value = int(KWS_DSCNN_PARAMS.magnitude / 1000)
...
\index{deployment paradigms!four paradigms} \index{Cloud ML|seealso{Edge ML}} \index{Edge ML|seealso{Mobile ML}} \index{Mobile ML|seealso{TinyML}}Machine learning models must execute somewhere—but where? The answer is not "wherever is most convenient." Different deployment locations offer fundamentally different trade-offs in latency, power, memory, and cost. This chapter introduces four **deployment paradigms** that span the full spectrum of ML system design:
...
Historically, processors have doubled in compute capacity roughly every 18 months, but memory bandwidth has improved only ~20% annually. This widening gap makes data movement the dominant bottleneck and energy cost for most ML workloads. This constraint affects all paradigms but is especially acute for **TinyML**, where devices have only kilobytes of memory to work with.
...
#| echo: false
from physx.constants import RESNET50_FLOPs, RESNET50_PARAMS, GPT2_PARAMS, GFLOPs, Mparam, byte, MB
from physx.formatting import fmt
...
gpt2_params_b_value = GPT2_PARAMS.to(Mparam).magnitude / 1000
...
| **Edge ML**   | NVIDIA DGX Spark                          | GB10 Grace Blackwell Superchip (20-core Arm, 1 PFLOPS AI) |  `{python} edge_mem_gb_str` GB LPDDR5x | `{python} edge_stor_tb_str` TB NVMe   |                              ~`{python} edge_pwr_w_str` W | ~\$`{python} edge_price_min_str`–`{python} edge_price_max_str` (2025 estimate) | Model fine-tuning, on-premise inference, prototype development |
...
required_tflops_value = sustained_gflops_value * headroom_value / 1000
...
How do these paradigm characteristics connect to the Workload Archetypes introduced earlier? As established in @sec-ml-system-architecture-analyzing-workloads, each archetype gravitates toward paradigms that address its binding constraint: **Compute Beasts** and **Sparse Scatter** toward cloud, **Bandwidth Hogs** spanning cloud and edge depending on latency requirements, **Tiny Constraint** exclusively on TinyML, and Mobile hosting efficiency-optimized variants (e.g., MobileNet) that trade peak performance for sustainable power consumption.
