...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, fmt_qty, fmt_count
...
class MobileResnetPitfallRecap:
    ...
    resnet_gflops = resnet_flops.to(GFLOPs).magnitude
    resnet_fp32_mb = resnet_fp32_bytes.to(MB).magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(resnet_fp32_mb >= 90, f"ResNet-50 FP32 size should be near 100 MB; got {resnet_fp32_mb:.0f} MB.")
    check(resnet_gflops >= 4, f"ResNet-50 compute should be about 4 GFLOPs; got {resnet_gflops:.1f}.")
...
class EnergyInference:
    ...
    e_resnet_cloud_str = fmt_qty(e_resnet_cloud_j, mJ, precision=1)
    e_resnet_edge_str = fmt_qty(e_resnet_edge_j, mJ, precision=1)
    e_mobilenet_str = fmt_qty(e_mobilenet_j, mJ, precision=2)
