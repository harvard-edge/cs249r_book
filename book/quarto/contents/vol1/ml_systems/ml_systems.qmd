...
### The Iron Law of System Balance

To navigate these environments, we must apply the **Bottleneck Principle**. Unlike traditional software where optimizing the average case works, ML systems are dominated by their slowest component:
...
This principle dictates that if your system is **Memory Bound** ($T_{memory} > T_{compute}$), buying a GPU with 2x more FLOPS yields exactly **0% speedup**. You must identify the dominant term before optimizing.

#### From Sums to Bottlenecks: The Roofline Transition

The Introduction presented the Iron Law as a sum: $L = \frac{D}{B} + \frac{Ops}{P \cdot \eta} + L_{fixed}$. Why do we now use $\max()$? The distinction lies in whether operations happen one after another or at the same time. 

In **Serial Execution**, operations execute sequentially, and the costs are additive ($T = T_{mem} + T_{compute}$). However, modern accelerators use **Pipelined Execution** to overlap memory transfers with computation. With perfect overlap, the total time is determined by whichever operation is slower. The Iron Law from the Introduction tells you the *cost of each ingredient*; the Bottleneck Law here tells you the *speed of the assembly line*. 
...
Now that we understand how bandwidth and compute become pipelined bottlenecks rather than additive costs, we can classify workloads by which term dominates. This classification allows us to group models not by their use cases (e.g., "vision" vs "audio") but by their physical constraints. 

### Workload Archetypes

The systems engineer's goal is to optimize the **Samples per Dollar** within the AI Triad. However, in the real world, this optimization is bounded by the deployment environment. We categorize ML workloads into four **Archetypes**.[^fn-archetype] These represent fundamental physical constraints, not just specific model architectures. 

**Archetype I: The Compute Beast.** These workloads exhibit high Arithmetic Intensity ($> 100$ FLOPs/Byte), performing many calculations per byte of data loaded. The binding constraint is raw TFLOPS—the system's peak computational throughput. Optimization focuses on faster matrix multiplication and parallel accelerators.

**Archetype II: The Bandwidth Hog.** With low Arithmetic Intensity ($< 10$ FLOPs/Byte), these workloads spend more time loading data than computing. Memory Bandwidth becomes the binding constraint. Autoregressive text generation (like ChatGPT) falls into this category. Optimization targets reducing data size through quantization and caching.

**Archetype III: The Sparse Scatter.** Irregular memory access patterns with poor cache locality define this archetype. Memory Capacity and access latency constrain performance. Recommendation systems with large tables are canonical examples. Optimization requires high-bandwidth networking and data sharding.

**Archetype IV: The Tiny Constraint.** Extreme power envelopes ($< 1$ mW) and memory limits ($< 256$ KB) characterize these workloads. The binding constraint is Joules per Inference—energy efficiency, not raw speed. Always-on sensing operates in this regime. Optimization demands aggressive compression and specialized low-power hardware.
...
With these concrete examples in mind, we can now formalize the relationship between workload archetypes and deployment paradigms. Each archetype's binding constraint maps to a specific deployment paradigm because of the physical laws that govern all computing systems.

### The Deployment Spectrum {#sec-ml-system-architecture-deployment-spectrum-71be}

The physical laws of **speed of light**, **power thermodynamics**, and **memory signaling** dictate that no single "ideal" computer exists. Where a system runs fundamentally alters the contract between model and hardware.

These laws are the root causes of the four paradigms:[^fn-paradigm] **Cloud ML** accepts network latency to maximize computational power. **Edge ML** moves inference near data sources when the speed of light makes cloud latency unacceptable. **Mobile ML** extends intelligence to personal devices where the power wall dominates. **TinyML** enables ubiquitous sensing where energy efficiency—measured in micro-Joules—is the only metric that matters.

Understanding these constraints precisely—their mathematical formulations and engineering implications—provides the foundation for making deployment decisions that physics will not override.
...
California to Virginia (~3,600 km straight-line) requires **~36 ms minimum** before any computation begins. Actual cloud services typically add 60–150 ms of software overhead. Applications requiring sub-10 ms response *cannot* use distant cloud infrastructure—physics forbids it.
...
Processors double in compute capacity every 18 months, but memory bandwidth improves only ~20% annually. This widening gap makes data movement the dominant bottleneck and energy cost for most ML workloads. This is why a **Bandwidth Hog** model gains **zero** speedup from a faster processor—it is simply "starving" for data.
...
To decide which paradigm to use, an engineer must weigh the **Complexity Tax** of a system. 

Consider a classification problem that could be solved with a simple **Heuristic** (if-then rules) or a **Deep Learning Pipeline**. 

1.  **The Heuristic**: 50 lines of code. $Ops \approx 0$. **Tax**: ~1 hour/month to update a rule. **Entropy**: Zero (rules don't drift).
2.  **The ML System**: 50 lines of model code + 2,000 lines of "Glue Code" (Data pipelines, monitoring, GPU drivers). **Tax**: ~40 hours/month (debugging drift, managing infra).

If the ML system provides 95% accuracy and the heuristic provides 90%, is that 5% gain worth a **40x increase** in complexity? ML systems engineering is the art of minimizing this tax through robust architecture. If you cannot afford the operational cost to fight **System Entropy**, the heuristic is the superior systems choice.

@tbl-latency-numbers provides order-of-magnitude latencies that should inform every deployment decision. The key decision rule: if your latency budget is $X$ ms, you cannot use any operation with latency $> X$ in your critical path.
