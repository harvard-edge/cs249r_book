...
\index{bottleneck principle!pipelined execution} \index{system bottlenecks!identifying} \index{compute-bound vs memory-bound!definition}To navigate deployment environments, we apply the **Bottleneck Principle**. Unlike traditional software where optimizing the average case works, ML systems are dominated by their slowest component. For pipelined execution (typical in serving), the total time is determined by the maximum of the stage latencies:
