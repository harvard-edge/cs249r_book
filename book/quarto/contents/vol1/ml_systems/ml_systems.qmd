...
The Introduction established the **Iron Law of ML Systems** (@principle-iron-law):
...
#| echo: false
#| label: resnet-cloud
from calc.formulas import calc_bottleneck
...
- Compute time: $\frac{`{python} resnet_gflops` \times 10^9 \text{ FLOPs}}{`{python} a100_tflops` \times 10^{12} \text{ FLOP/s}} = `{python} cloud_compute_ms` \text{ ms}$
- Memory time: $\frac{`{python} resnet_fp16_mb` \times 10^6 \text{ bytes}}{`{python} a100_bw_tbs` \times 10^{12} \text{ B/s}} = `{python} cloud_memory_ms` \text{ ms}$
- **Bottleneck**: `{python} cloud_bottleneck` (`{python} cloud_ratio_x`× slower than compute)
- **Arithmetic Intensity**: $\frac{`{python} resnet_gflops` \times 10^9}{`{python} resnet_fp16_mb` \times 10^6} \approx `{python} cloud_ai`$ FLOPs/byte
...
- Compute time: $\frac{`{python} resnet_gflops` \times 10^9 \text{ Ops}}{`{python} mobile_tops` \times 10^{12} \text{ Op/s}} = `{python} mobile_compute_ms` \text{ ms}$
- Memory time: $\frac{`{python} resnet_int8_mb` \times 10^6 \text{ bytes}}{`{python} mobile_bw_gbs` \times 10^{9} \text{ B/s}} = `{python} mobile_memory_ms` \text{ ms}$
...
raw_bytes_per_sec = raw_bytes_per_frame * VIDEO_FPS_STANDARD
...
bw_shortage = total_bytes_per_sec / NETWORK_10G_BW

cam_rate_mbs   = f"{raw_bytes_per_sec / MB:.0f}"
total_rate_gbs = f"{total_bytes_per_sec / GB:.1f}"
monthly_cost_m = f"{monthly_cost / 1e6:.1f}"
net_cap_gbs    = f"{NETWORK_10G_BW / GB:.2f}"
...
::: {.callout-fallacy title="One deployment paradigm solves all ML problems."}
:::
...
::: {.callout-fallacy title="Model optimization overcomes mobile device power and thermal limits."}
:::
...
::: {.callout-fallacy title="TinyML represents scaled-down mobile ML."}
:::
...
::: {.callout-pitfall title="Minimizing computational resources minimizes total cost."}
:::
...
::: {.callout-fallacy title="More training data always improves deployed model performance."}
:::
