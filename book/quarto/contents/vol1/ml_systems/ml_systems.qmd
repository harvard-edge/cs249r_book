...
class MobilePowerFallacyCalc:
    ...
    low_power = 1 * watt                                                           # light inference
    high_power = 5 * watt                                                          # heavy on-device model
    mobile_tdp = Scenarios.MobilePower.MobileNpuTypical
    quant_reduction_value = 4                                                       # FP32 to INT8 power reduction

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    low_power_hours = (battery / low_power).to(hour).magnitude
    high_power_hours = (battery / high_power).to(hour).magnitude
    ...
    check(high_power_hours < low_power_hours,
          "Higher-power mobile workload should drain the battery faster.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    phone_battery_str = fmt_qty(battery, Wh, precision=0, commas=False)
    ...
    low_power_hours_str = fmt(low_power_hours, precision=0, commas=False)
    high_power_hours_str = fmt(high_power_hours, precision=0, commas=False)
    mobile_tdp_str = fmt_qty(mobile_tdp, watt, precision=0, commas=False)
