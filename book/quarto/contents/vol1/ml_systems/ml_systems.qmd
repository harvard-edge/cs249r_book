...
class MLSystemsSetup:
    ...
    # Assumptions (ranges) from Centralized Constants
    ...
    # Latency ranges (ms) from Centralized Constants
...
[^fn-archetype-bottleneck]: **Workload Archetype**: A classification of ML workloads by their dominant Iron Law bottleneck rather than their model family. The distinction matters because the optimization strategy differs fundamentally: a compute-bound workload benefits from faster arithmetic ($R_{peak}$), while a bandwidth-bound workload benefits only from wider memory buses ($BW$). Misidentifying the archetype wastes optimization effort on the wrong term of the Iron Law, as when teams add GPU FLOPS to a memory-bound inference pipeline and observe zero speedup. \index{Archetype!workload classification}
...
[^fn-ml-apis-abstraction]: **ML APIs (Application Programming Interfaces)**: This flexibility is achieved by decoupling the client from the server via a standard network contract, making the model accessible to any platform that can make a web request. The trade-off for such accessible, pay-as-you-go infrastructure is latency; every API call incurs the full round-trip cloud network penalty of 100-500 ms, making it too slow for real-time systems. \index{ML APIs!latency trade-off}
...
[^fn-iot-hubs-aggregation]: **IoT Hubs**: These gateways create the deployment "middle ground" by aggregating and preprocessing sensor data locally, rather than streaming it all to the cloud. This local filtering provides acceptable real-time performance by making decisions without incurring the typical 100-500 ms cloud round-trip latency. Operating under these intermediate resource constraints is the trade-off for reducing network data volume by over 90%, sending only critical metadata and alerts. \index{IoT Hubs!bandwidth reduction}
...
[^fn-latency-critical-edge]: **Latency-Critical Applications**: Systems whose operational viability depends on a strict latency ceiling, making network outages (functionally infinite latency) a critical failure. For the industrial systems described, where downtime carries immediate cost, this transforms resilience from a desired property into a core design constraint. Cloud-based systems with 100+ ms of network latency are therefore architecturally disqualified, not merely because they are slow, but because they are not resilient to network failure. \index{Latency-Critical!edge requirement}
...
[^fn-voice-on-device]: **On-Device Voice Recognition**: This approach provides immediate responsiveness and privacy by processing audio locally, which eliminates the >100 ms network latency and data transfer risks of a cloud round-trip. The explicit trade-off for this capability is computational sophistication: on-device models are >10x smaller (e.g., 50–200 MB vs. 2+ GB), sacrificing accuracy on rare vocabulary to guarantee low-latency (<30ms) and offline operation. \index{Voice Recognition!on-device trade-off}

[^fn-computational-photo-ml]: **Computational Photography**: Uses ML algorithms (e.g., multi-frame fusion, neural denoising) to overcome the physical limits of small mobile camera sensors. This exemplifies the mobile computing trade-off, as a pipeline of 10-15 models must execute within the user's perceived shutter delay (~200 ms) while adhering to a strict, shared `{python} mobile_tdp_range_str` W thermal budget. \index{Computational Photography!pipeline constraint}

[^fn-depthwise-mobile-efficiency]: **Depthwise Separable Convolutions**: An architectural factorization that splits a standard convolution into a depthwise spatial filter and a pointwise channel mixer, reducing FLOPs by 8--9$\times$ for typical layer configurations. This reduction is not merely an efficiency improvement but a prerequisite for real-time vision on mobile devices, where the Power Wall caps sustained computation at `{python} mobile_tdp_range_str` W. \index{Depthwise Separable Convolution!mobile power constraint}
...
[^fn-tflite-mobile]: **TensorFlow Lite**: Enables hardware-optimized inference by using a "delegate" architecture to dispatch operations to specialized mobile hardware like NPUs, which is essential for meeting sub-100ms UI latency targets. Its converter automates optimizations like quantization, reducing model size by up to 4x (FP32 to INT8) to fit within the strict power budgets and memory bandwidth limits of mobile devices. \index{TensorFlow Lite!precision bridge}
...
[^fn-offline-translation]: **On-Device Translation**: This functionality is enabled by aggressive model compression techniques like knowledge distillation and quantization, which distill large cloud-based models into a format that can run locally. The trade-off is a modest reduction in translation quality in exchange for eliminating network dependency and fitting within strict mobile resource budgets. For example, Google's language models are compressed over 50x to ~40 MB, allowing them to run entirely on-device. \index{Translation!mobile compression trade-off}
...
[^fn-mobile-resource-gap]: **Mobile Resource Gap**: The 10--100$\times$ deficit in memory and power budgets relative to cloud servers directly constrains the model size and batch behaviors available. This forces a focus away from peak throughput toward *sustained* performance, where thermal throttling can cause a model running at 100 FPS to drop to 30 FPS to stay within a 500 mW power envelope. \index{Mobile!thermal throttling}
...
[^fn-mcu-resource-floor]: **Microcontroller (MCU)**: A single-chip computer whose design prioritizes minimal cost and power over performance, creating the "radical constraint" mentioned. This constraint is a hard memory ceiling: ML models must fit entirely within kilobytes of on-chip SRAM (e.g., 32-512 KB), as there is no virtual memory or DRAM like in mobile devices. This resource floor, often 1,000x lower than a smartphone's, forces the development of entirely new, memory-centric ML architectures. \index{Microcontroller!memory ceiling}

[^fn-tinyml-energy-gap]: **TinyML Energy Gap**: This differential is rooted in hardware design philosophy; cloud GPUs are optimized for raw throughput, consuming hundreds of watts, while TinyML microcontrollers are designed for near-zero power sleep states. This architectural trade-off means a single cloud inference consumes ~1 Joule, whereas a specialized TinyML device uses less than 1 microjoule—the 1,000,000x gap that mandates different system designs for battery-powered operation. \index{TinyML!energy efficiency}
