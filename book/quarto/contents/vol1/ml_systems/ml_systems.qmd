...
class HardwareSpectrumSetup:
    ...
    edge_pwr_w_str = fmt_power(Hardware.Workstation.DGX_Spark.tdp, precision=0, commas=False)
    ...
    tiny_pwr_min_str = fmt_power(Hardware.Tiny.ESP32_S3.tdp_min, precision=2, commas=False)
    tiny_pwr_max_str = fmt_power(Hardware.Tiny.ESP32_S3.tdp_max, precision=1, commas=False)
...
from mlsysim.core.units import kilometer, millisecond
...
class DistancePenalty:
    ...
    distance_km = distance_km_value * kilometer
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sol_kms_str = fmt_qty(SPEED_OF_LIGHT_FIBER_KM_S, kilometer / second, precision=0, commas=True)
    ...
    distance_km_str = fmt_qty(distance_km, kilometer, precision=0, commas=True)
...
class CloudEdgeTCO:
    ...
    gpu_instances = 4
    egress_per_gb = Infrastructure.Pricing.Cloud.EgressPerGB.rate.to(USD / GB).magnitude
    ...
    egress_volume_per_day = requests_per_day * response_kb * KB
    egress_gb_per_day = egress_volume_per_day.to(GB).magnitude
    ...
    e_power = annual_energy.to(kWh).magnitude * electricity_rate.to(USD / kWh).magnitude
    ...
    power_str = fmt_power(server_tdp, precision=0, commas=False)
    electricity_str = fmt_usd(electricity_rate.to(USD / kWh).magnitude, precision=2, commas=False, per="kWh")
...
class BandwidthBottleneck:
    ...
    bb_fps_str = fmt_int(fps.to("Hz").magnitude, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, MarkdownStr, fmt_qty, fmt_time
...
class DataLocalityInvariant:
    ...
    tx_time_ms = (frame_bits / net_bw).to(millisecond).magnitude
    ...
    net_bw_str = fmt_qty(net_bw, megabit / second, precision=0, commas=False, unit_label="Mb/s")
...
class EdgeSizingThroughput:
    ...
    sustained_gflops = sustained_flop_rate.to(GFLOPs / second).magnitude
    ...
    yolo_gflops_str = fmt_qty(yolo_flops, GFLOPs, precision=1)
