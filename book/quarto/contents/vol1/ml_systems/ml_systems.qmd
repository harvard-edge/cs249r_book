...
The physical constraints that govern each environment (latency, power, and memory) force ML deployment into four distinct paradigms, each with its own engineering trade-offs and system design patterns. **Cloud ML**\index{Cloud ML!characteristics} aggregates computational resources in data centers, offering elastic compute and large-scale storage at the cost of network latency. **Edge ML**\index{Edge ML!latency benefits} moves computation closer to where data originates, including factory floors, retail stores, and hospitals, achieving lower latency and keeping sensitive data on-premises [@shi2016]. **Mobile ML**\index{Mobile ML!energy constraints} brings intelligence directly to smartphones and tablets, balancing computational capability against battery life and thermal constraints. **TinyML**\index{TinyML!always-on sensing} pushes intelligence to the smallest devices: microcontrollers costing dollars and consuming milliwatts, enabling battery-powered, always-on sensing [@janapa_reddi2022]. These four paradigms span nine orders of magnitude in power consumption (megawatts to milliwatts) and memory capacity (terabytes to kilobytes). The same engineering principles apply across the spectrum, but their priorities and implementations differ sharply.
...
Decentralized processing reduces latency and bandwidth pressure, but it also pushes maintenance and security problems out to distributed hardware that is harder to secure than a centralized data center (@fig-edge-ml).
...
::: {#fig-energy-per-inference fig-env="figure" fig-pos="htb" fig-cap="**Illustrative Energy per Inference Anchors**: Pedagogical scenarios span ~10 µJ for TinyML keyword spotting to ~1 kJ for a cloud LLM query; they are not outputs of the later analytical node model. The MobileNetV2 bar is a separate full-device scenario, not the preceding 0.1 mJ neural-processing-unit anchor." fig-alt="Horizontal log-scale bar chart showing illustrative energy anchors for five workloads across four deployment paradigms. TinyML keyword spotting is 10 microjoules, Mobile MobileNetV2 is 50 millijoules, Edge ResNet-50 is 500 millijoules, Cloud ResNet-50 is 10 joules, and a Cloud GPT-4 query is 1 kilojoule."}
...
Sensor integration and on-device processing enable real-time response and stronger privacy properties, but battery life and limited compute force engineers to optimize for sustained efficiency over raw performance (@fig-mobile-ml).
...
TinyML sits at the endpoint of the resource envelope: milliwatt power and kilobyte memory (@fig-tiny-ml). These limits enable always-on sensing that no other paradigm can sustain, but they force engineers to solve extreme model compression before the application can exist.
...
Production hybrid ML systems\index{Hybrid ML!production systems} integrate multiple design patterns into cohesive solutions. @Fig-hybrid makes these interactions concrete through specific connection types in a hybrid data pipeline\index{Data Pipeline!hybrid architectures}. The key feature is bidirectional flow: "Deploy" paths show how models flow *downward* from cloud training to various devices, while "Data" and "Results" flow *upward* from sensors through processing stages to cloud analytics. "Sync" connections demonstrate device coordination across tiers. This bidirectional architecture, models flowing down and data flowing up, is the defining characteristic of production hybrid systems.
...
Hybrid architectures work because the paradigms differ in resource budgets, not in the underlying systems jobs. Their convergence principles\index{Hybrid ML!convergence principles} are visible in @fig-ml-systems-convergence: implementations spanning cloud to tiny devices meet at the same core system challenges of managing data pipelines, balancing resource constraints, and implementing reliable architectures. Those shared foundations in turn raise the same cross-cutting considerations across every paradigm, namely optimization and efficiency, operational aspects, and trustworthy AI.

::: {#fig-ml-systems-convergence fig-env="figure" fig-pos="htb" fig-cap="**Convergence of ML Systems**: Three layers show how diverse deployments converge. Cloud, edge, mobile, and TinyML implementations share three core foundations—data pipelines, resource management, and system architecture—which in turn support optimization and efficiency, operational aspects, and trustworthy AI." fig-alt="Three-layer diagram. The top layer contains cloud ML, edge ML, mobile ML, and TinyML. The middle layer contains data pipeline, resource management, and system architecture, with sublabels for collection, processing, deployment, compute, memory, energy, network, models, hardware, and software. The bottom layer contains optimization and efficiency, operational aspects, and trustworthy AI. Two downward arrows connect the layers."}
...
An ML model's accuracy can change when the world drifts away from its training distribution, even if its code and weights remain unchanged. @Eq-degradation provides a local diagnostic approximation\index{Degradation Equation!distribution shift}: system quality may decline as the distance between the training distribution and the live data distribution grows, at a rate proportional to the model's sensitivity to distributional shift. Reliability therefore requires monitoring to determine whether updates are warranted. The operational aspects covered in @sec-ml-operations address precisely this challenge.
...
Choosing where a system runs settles its physics; it does not protect it from time. Zillow's \$304 million write-down involved forecast uncertainty and a systems-design failure to contain its propagation into inventory and operational commitments [@zillow2021offers]. @Sec-ml-workflow establishes that process, the systematic development discipline that carries an ML system from conception through deployment and is built to catch exactly this class of failure before it compounds.
