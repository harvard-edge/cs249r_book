...
To navigate this environment, we categorize ML workloads into four **Archetypes**. These archetypes represent fundamental physical constraints, not just specific model architectures. Throughout this book, we will refer to these archetypes to explain why certain optimizations work for some models but fail for others.

::: {.callout-note title="Volume 1 Archetypes: The Physics of Workloads"}
**Archetype I: The Compute Beast (ResNet, BERT)**
*   **Physics**: High Arithmetic Intensity ($> 100$ FLOPs/Byte).
*   **Constraint**: Raw TFLOPS.
*   **Optimization**: Matrix multiplication kernels, systolic arrays.

**Archetype II: The Bandwidth Hog (LLM Decoding, GPT-4)**
*   **Physics**: Low Arithmetic Intensity ($< 10$ FLOPs/Byte) during generation.
*   **Constraint**: Memory Bandwidth (HBM GB/s).
*   **Optimization**: Kv-caching, quantization (INT4/8) to reduce weight size.

**Archetype III: The Sparse Scatter (Recommendation Systems, GNNs)**
*   **Physics**: Irregular memory access patterns (Embedding lookups).
*   **Constraint**: Latency and Memory Capacity (TB of embeddings).
*   **Optimization**: High-bandwidth networking (All-to-All), caching.

**Archetype IV: The Tiny Constraint (Wake Word, Sensors)**
*   **Physics**: Extreme power envelope ($< 1$ mW).
*   **Constraint**: Joules per Inference and SRAM size ($< 256$ KB).
*   **Optimization**: Deep compression, specialized DSPs, analog computing.
:::
...
This environment-driven necessity creates four distinct engineering paradigms...
...
*   **Mobile ML** emphasizes data locality with constrained infrastructure.
...
Edge ML emerged as a direct response to these limitations, moving computation closer to data sources and trading unlimited computational resources for sub-100 ms latency and local data sovereignty.
...
TinyML completes the deployment spectrum by pushing intelligence to its physical limits, using devices costing less than $10 and consuming less than 1 milliwatt of power, making ubiquitous sensing economically practical at massive scale.
...
Yet production systems rarely confine themselves to a single paradigm. A voice assistant that uses TinyML for wake-word detection, mobile ML for local speech recognition, edge ML for contextual processing, and cloud ML for complex natural language understanding demonstrates this approach. Hybrid Machine Learning formalizes this integration strategy, creating unified systems that leverage each paradigm's complementary strengths while mitigating individual limitations.
