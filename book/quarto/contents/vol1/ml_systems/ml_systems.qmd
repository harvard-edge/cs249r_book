...
#| echo: false
#| fig-cap: "**The Systems Gap.** Relative Compute Growth (Log Scale). The gray dotted line shows the **CPU Performance Trend** (~1.3x/year), which lags Moore's Law due to power constraints. The blue dashed line shows **GPU Peak (Huang's Law)** (accelerated scaling, ~1.7x/year), which bridged the gap from 2012-2018. The red solid line shows **Model Demand** (Scaling Laws, ~6x/year). The shaded purple region represents the **Systems Gap**: the massive deficit that cannot be closed by better chips alone, but must be filled by **Parallelism, Architecture, and Hardware-Software Co-design**."
...
This principle dictates that if your system is **Memory Bound** ($D/B > O/(P \cdot \eta)$), buying faster processors ($P$) yields exactly **0% speedup**. You must identify the dominant term before optimizing. This tradeoff is governed by *the energy of transmission*.
