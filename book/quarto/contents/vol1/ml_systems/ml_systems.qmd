...
class DataLocalityInvariant:
    ...
    net_bw_s_str = fmt_bandwidth(net_bw, unit=megabit / second, precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.fmt import fmt, fmt_flop_rate, fmt_flops, fmt_ops_rate
...
class EdgeSizingThroughput:
    ...
    yolo_gflops_per_inference_str = fmt_flops(yolo_flops, unit=GFLOPs, precision=1, per="inference")
    sustained_gflops_str = fmt_flop_rate(sustained_flop_rate, unit=GFLOPs / second, precision=0)
...
| **Model compute**        |                       YOLOv8-nano: `{python} EdgeSizingThroughput.yolo_gflops_per_inference_str`                      |      `{python} EdgeSizingThroughput.sustained_gflops_str`      |
| **Required throughput**  | `{python} EdgeSizingThroughput.sustained_gflops_str` $\times$ `{python} EdgeSizingThroughput.headroom_str` (headroom) | ~`{python} EdgeSizingThroughput.req_tops_equiv_str` equivalent |
...
from mlsysim.fmt import fmt_percent, fmt, fmt_usd, check, MarkdownStr, fmt_qty, fmt_time, fmt_ops_rate
