...
$$\frac{\text{Compute Growth}}{\text{Memory BW Growth}} \approx \frac{1.6\times/\text{year}}{1.2\times/\text{year}} \approx `{python} mem_wall_ratio_str`\times/\text{year}$$
...
| **TinyML** | $D_{vol}/\text{Capacity}$ | 256 KiB total; model must fit on-chip | Extreme compression, binary networks |
...
| **TinyML** | ESP32-CAM | Dual-core @ 240MHz | 520 KiB RAM | 4 MB Flash | 0.05-0.25 W | $10 | Image classification, motion detection | <1 TOPS compute, <1mW power, microsecond response times |
...
c_lb = 4000
c_logs = 2000
c_total = c_inf + c_egress + c_lb + c_logs
...
cam_rate_mbs   = f"{raw_bytes_per_sec.to(MB/second).magnitude:.0f}"
total_rate_gbs = f"{total_bytes_per_sec.to(GB/second).magnitude:.1f}"
monthly_cost_m = f"{monthly_cost / 1e6:.1f}"
net_cap_gbs    = f"{NETWORK_10G_BW.to(GB/second).magnitude:.2f}"
...
yolo_gflops    = f"{YOLOV8_NANO_FLOPs.to(GFLOPs).magnitude:.1f}"
...
*   **Cloud Path**: 50 ms network + 10 ms inference (A100) = **`{python} cloud_total_str` ms total**.
*   **Edge Path**: 5 ms network + 80 ms inference (Raspberry Pi) = **`{python} edge_total_str` ms total**.
