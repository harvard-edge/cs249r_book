...
[^fn-endpoint-constraints]: **Edge Server Constraints**: Typical edge servers have 1-8GB RAM and 2-32GB storage, versus cloud servers with 128-1024GB RAM and access to effectively unbounded storage. Processing throughput can differ by an order of magnitude (or more), depending on accelerator availability and workload, which motivates model compression and system optimization techniques.
...
[^fn-tflite]: **TensorFlow Lite**: Google's mobile ML framework launched in 2017 for on-device inference. It targets resource-constrained deployments where model size and latency budgets are materially tighter than typical cloud deployments.

[^fn-coreml]: **Core ML**: Apple's framework introduced in iOS 11 (2017), optimized for on-device inference, including automatic conversion and device-specific execution optimizations.
...
[^fn-real-time-translation]: **Real-Time Translation**: Offline translation illustrates the trade-offs of Mobile ML: smaller on-device models can enable low-latency translation without internet connectivity, typically with some accuracy loss relative to larger cloud models.

[^fn-face-detection]: **Mobile Face Detection**: Apple's Face ID processes biometric data on-device within a hardware-isolated Secure Enclave. This design aims to reduce exposure of sensitive biometric information, though no system provides absolute guarantees under all threat models.
...
[^fn-mobile-constraints]: **Mobile Device Constraints**: Mobile devices typically operate with materially less memory and tighter sustained power envelopes than servers. Inference workloads must contend with shared device resources (CPU, GPU, NPU, memory bandwidth), background activity, and thermal limits, which can significantly affect latency and throughput.
...
[^fn-device-size]: **TinyML Device Scale**: TinyML-capable systems range from millimeter-scale chips to palm-sized developer boards; exact dimensions and capabilities vary by device generation and configuration.
...
[^fn-model-compression]: **TinyML Model Optimization**: Specialized techniques can reduce model size by two to three orders of magnitude relative to mobile deployments while retaining much of the original task performance (see @sec-model-optimizations for techniques and trade-offs).
