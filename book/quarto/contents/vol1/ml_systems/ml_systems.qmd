...
: **The Deployment Spectrum**: Four paradigms span six orders of magnitude in power (MW to mW) and memory (TB to KB), each optimized for different constraints. {#tbl-deployment-paradigms-overview}
...
Why do these four paradigms exist? The answer lies not in engineering choices but in physical laws that no amount of optimization can overcome.
...
This equation decomposes total latency into three terms: data movement ($D/B$), compute ($Ops/P \cdot \eta$), and fixed overhead ($L_{fixed}$). But which term dominates? The answer depends entirely on the deployment environment. A model that is compute-bound during training may become memory-bound during inference[^fn-inference-etymology]; a system that runs efficiently in the cloud may hit power limits on mobile devices.

#### The Bottleneck Principle

To navigate deployment environments, we apply the **Bottleneck Principle**. Unlike traditional software where optimizing the average case works, ML systems are dominated by their slowest component:

$$ T_{system} = \max(T_{compute}, T_{memory}, T_{network}, T_{io}) + L_{overhead} $$

*   **$T_{compute}$**: Time to execute the calculations.
*   **$T_{memory}$**: Time to move data between memory and processor.
*   **$T_{network}$**: Time for network communication.
*   **$T_{io}$**: Time to load data from storage.

This principle dictates that if your system is **Memory Bound** ($T_{memory} > T_{compute}$), buying faster processors yields exactly **0% speedup**. You must identify the dominant term before optimizing.
...
These archetypes map naturally to deployment paradigms: **Compute Beasts** and **Sparse Scatter** workloads gravitate toward **Cloud ML** where resources are abundant. **Bandwidth Hogs** span Cloud and Edge depending on latency requirements. **Tiny Constraint** workloads are exclusively **TinyML** territory.
...
**Key insight**: The huge range in compute requirements (20 MFLOPs → 4 GFLOPs) and memory (800 KB → 100 GB) explain why no single deployment paradigm fits all workloads. A keyword spotter runs comfortably on a $2 microcontroller; a recommendation system requires a warehouse-scale computer.
:::
...
The physical constraints and workload archetypes we have established are not abstract limitations—they manifest concretely in how each deployment paradigm optimizes the fundamental ML system components. This section shows *how* each paradigm navigates these constraints in practice before we examine each paradigm in detail.
...
::: {.callout-notebook title="ResNet-50 on Cloud vs Mobile" collapse="true"}
...
::: {.callout-notebook title="Cloud vs. Edge TCO" collapse="true"}
...
::: {.callout-notebook title="Wake-Word Economics" collapse="true"}
...
::: {.callout-notebook title="Edge Inference Sizing" collapse="true"}
