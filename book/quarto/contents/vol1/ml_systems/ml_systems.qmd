...
# 1) Inputs (from constants)
mobile_tdp_w = MOBILE_TDP_W.magnitude  # 3W typical
mobile_npu_tops = MOBILE_NPU_TOPS_INT8.to(TFLOPs/second).magnitude  # 35 TOPS
phone_battery_wh = PHONE_BATTERY_WH.to(watt * hour).magnitude  # 15 Wh

# Mobile RAM/storage (typical ranges)
mobile_ram_range = "8-16"
mobile_storage_range = "128 GB-1 TB"
mobile_bw_range = "25-50"
...
# KWS model specs
kws_params = int(KWS_DSCNN_PARAMS.magnitude / 1000)  # ~200K → display as 70K typical

# DLRM embedding size
dlrm_embedding_gb = int(DLRM_MODEL_SIZE_FP32.to(GB).magnitude)

# MobileNet efficiency
mobilenet_flops_reduction = "8-9"

# 3) Display strings
mobile_tdp_str = f"{mobile_tdp_w:.0f}"
mobile_tdp_range = "3-5"
mobile_npu_tops_str = f"{mobile_npu_tops:.0f}"
mobile_npu_range = "1-10"
phone_battery_str = f"{phone_battery_wh:.0f}"
kws_params_str = "70,000"  # Typical KWS model (text uses canonical 70K)
kws_size_kb = "100"
dlrm_embedding_str = f"{dlrm_embedding_gb}"
```
...
|:------------|:---------------|---------------------------------:|:----------------------------|:---------|:---------------------------|
| **Cloud ML**  | Data centers     |  `{python} cloud_latency_range` ms | MW                            | TB         | Training, complex inference  |
| **Edge ML**   | Local servers    |   `{python} edge_latency_range` ms | 100s W                        | GB         | Real-time inference, privacy |
| **Mobile ML** | Smartphones      | `{python} mobile_latency_range` ms | `{python} mobile_tdp_range` W | GB         | Personal AI, offline         |
| **TinyML**    | Microcontrollers |   `{python} tiny_latency_range` ms | mW                            | KB         | Always-on sensing            |
...
# Memory wall: compute growth outpaces memory bandwidth growth
# 1) Inputs
compute_growth_per_yr = 1.6  # x/year
mem_bw_growth_per_yr = 1.2   # x/year

# 2) Computation
mem_wall_ratio = compute_growth_per_yr / mem_bw_growth_per_yr  # 1.6 / 1.2 = 1.33...

# 3) Display strings
compute_growth_str = f"{compute_growth_per_yr:.1f}"
mem_bw_growth_str = f"{mem_bw_growth_per_yr:.1f}"
mem_wall_ratio_str = f"{mem_wall_ratio:.2f}"
```
...
$$\frac{\text{Compute Growth}}{\text{Memory BW Growth}} \approx \frac{`{python} compute_growth_str`\times/\text{year}}{`{python} mem_bw_growth_str`\times/\text{year}} \approx `{python} mem_wall_ratio_str`\times/\text{year}$$
...
et_data_mb = 1            # MB (1 second of audio)
et_tx_energy_mj = 100     # mJ/MB (Wi-Fi/LTE)
et_compute_energy_mj = 0.1  # mJ/inference (MobileNet on NPU)

et_cloud_energy_mj = et_data_mb * et_tx_energy_mj
et_local_energy_mj = et_compute_energy_mj
et_energy_ratio = int(et_cloud_energy_mj / et_local_energy_mj)

et_data_mb_str = str(et_data_mb)
et_tx_energy_str = str(et_tx_energy_mj)
et_compute_energy_str = str(et_compute_energy_mj)
et_cloud_energy_str = str(int(et_cloud_energy_mj))
et_local_energy_str = str(et_local_energy_mj)
et_energy_ratio_str = f"{et_energy_ratio}"
```
...
1.  **Cloud Approach**: $E_{cloud} \approx D_{vol} \times E_{tx} = `{python} et_data_mb_str` \text{ MB} \times `{python} et_tx_energy_str` \text{ mJ/MB} = \mathbf{`{python} et_cloud_energy_str` \text{ mJ}}$.
2.  **Local Approach**: $E_{local} \approx \text{Inference} = \mathbf{`{python} et_local_energy_str` \text{ mJ}}$.
...
resnet_params_m_raw = RESNET50_PARAMS.to(Mparam).magnitude
resnet_fp32_mb_raw = (RESNET50_PARAMS.magnitude * 4 * byte).to(MB).magnitude
gpt2_params_b_raw = GPT2_PARAMS.to(Mparam).magnitude / 1000

# 3) Display strings
resnet_gflops = f"{resnet_flops_g:.1f}"
resnet_params_m = f"{resnet_params_m_raw:.1f}"
resnet_fp32_mb = f"{resnet_fp32_mb_raw:.0f}"
gpt2_params_b = f"{gpt2_params_b_raw:.1f}"
```

**ResNet-50**\index{ResNet-50!systems characteristics} classifies images into 1,000 categories, processing each image through approximately `{python} resnet_gflops` billion floating-point operations using `{python} resnet_params_m` million parameters (`{python} resnet_fp32_mb` MB at FP32). Its regular, compute-dense structure makes it the canonical benchmark for hardware accelerator performance.

**GPT-2 / Llama**\index{GPT-2!autoregressive bottleneck}\index{Llama!memory-bound inference} generate text one token at a time, requiring the model to read its full parameter set (`{python} gpt2_params_b` billion for GPT-2, 7 to 70 billion for Llama) from memory for each output token. This sequential memory access pattern creates the autoregressive bottleneck that dominates serving costs.
...
# 1) Display constants (already formatted strings)
# Compute
lat_compute = "~1 ns"
lat_npu = "5–20 ms"
lat_llm = "20–100 ms"
...
# Network
lat_net_dc = "0.5 ms"
lat_net_region = "1–5 ms"
lat_net_cross = "50–150 ms"

# ML Ops
lat_kws = "100 μs"
lat_face = "10–30 ms"
lat_gpt4 = "200–500 ms"
lat_train = "200–400 ms"
```

| **Operation**                    | **Latency**               | **Deployment Implication**       |
|:-------------------------------|:------------------------|:-------------------------------|
| **Compute**                      |                           |                                  |
| **GPU matrix multiply (per op)** | `{python} lat_compute`    | Compute is rarely the bottleneck |
| **NPU inference (MobileNet)**    | `{python} lat_npu`        | Mobile can do real-time vision   |
| **LLM token generation**         | `{python} lat_llm`        | Perceived as "typing speed"      |
| **Memory**                       |                           |                                  |
| **L1 cache hit**                 | `{python} lat_l1`         | Keep hot data in registers       |
| **HBM read (GPU)**               | `{python} lat_hbm`        | 100× slower than compute         |
| **DRAM read (mobile)**           | `{python} lat_dram`       | Memory-bound on most devices     |
| **Network**                      |                           |                                  |
| **Same datacenter**              | `{python} lat_net_dc`     | Microservices feasible           |
| **Same region**                  | `{python} lat_net_region` | Edge servers viable              |
| **Cross-region**                 | `{python} lat_net_cross`  | Batch processing only            |
| **ML Operations**                |                           |                                  |
| **Wake-word detection (TinyML)** | `{python} lat_kws`        | Always-on feasible at <1 mW      |
| **Face detection (mobile)**      | `{python} lat_face`       | Real-time at 30 FPS              |
| **GPT-4 first token**            | `{python} lat_gpt4`       | User notices delay               |
| **ResNet-50 training step**      | `{python} lat_train`      | Throughput-optimized             |
...
#| echo: false
#| label: resnet-cloud
from calc.formulas import calc_bottleneck, sci, sci_latex, fmt, md_frac
...
a100_bw_tbs     = f"{A100_MEM_BW.to(TB/second).magnitude:.0f}"
cloud_compute_ms_str = f"{cloud_stats['compute_ms']:.3f}"
cloud_memory_ms_str = f"{cloud_stats['memory_ms']:.3f}"
...
resnet_flops_sci = sci(RESNET50_FLOPs)
resnet_fp16_bytes_sci = sci(resnet_fp16_bytes)
...
resnet_fp16_bytes_latex = sci_latex(resnet_fp16_bytes.to(byte))
a100_bw_latex = sci_latex(A100_MEM_BW.to(byte/second))

# Create LaTeX fractions using Markdown() for nice rendering
cloud_compute_frac = md_frac(resnet_flops_latex, a100_flops_latex, cloud_compute_ms_str, "ms")
cloud_memory_frac = md_frac(resnet_fp16_bytes_latex, a100_bw_latex, cloud_memory_ms_str, "ms")
cloud_ai_frac = md_frac(resnet_flops_latex, resnet_fp16_bytes_latex, cloud_ai, "FLOPs/byte")
```
...
- Compute time: $T_{comp}$ = `{python} cloud_compute_frac`
- Memory time: $T_{mem}$ = `{python} cloud_memory_frac`
...
mobile_bw_gbs     = f"{MOBILE_NPU_MEM_BW.to(GB/second).magnitude:.0f}"
mobile_compute_ms_str = f"{mobile_stats['compute_ms']:.2f}"
mobile_memory_ms_str  = f"{mobile_stats['memory_ms']:.2f}"
...
resnet_int8_bytes_latex = sci_latex(resnet_int8_bytes.to(byte))
mobile_npu_bw_latex = sci_latex(MOBILE_NPU_MEM_BW.to(byte/second))

# Create LaTeX fractions using Markdown() for nice rendering
mobile_compute_frac = md_frac(resnet_flops_latex, mobile_npu_flops_latex, mobile_compute_ms_str, "ms")
mobile_memory_frac = md_frac(resnet_int8_bytes_latex, mobile_npu_bw_latex, mobile_memory_ms_str, "ms")
```
...
- Compute time: $T_{comp}$ = `{python} mobile_compute_frac`
- Memory time: $T_{mem}$ = `{python} mobile_memory_frac`
...
| **Mobile ML** | Flagship Smartphone (e.g., iPhone, Pixel) |              Modern mobile SoC (multi-core CPU, GPU, NPU) | `{python} mobile_ram_range` GB RAM | `{python} mobile_storage_range` | `{python} mobile_tdp_range` W | $999+                         | Face ID, computational photography, voice recognition          | `{python} mobile_npu_range` TOPS compute, <2W sustained power, <`{python} mobile_latency_range`ms UI response |
...
rtt_formatted_str = f"{round_trip_ms:.0f}"
deficit_str       = f"{deficit_ms:.0f}"
distance_km_str   = f"{distance_km:,}"
safety_budget_str = f"{safety_budget_ms}"
```
...
inference_ms = 10
response_kb = 100  # Realistic: JSON scores, metadata, headers
days_per_year = 365
hours_per_year = 24 * days_per_year  # 8,760

# === Cloud Pricing (AWS 2024) ===
gpu_price_per_hr = 0.75      # A10G (g5.xlarge)
gpu_instances = 4            # For HA, peak handling, redundancy
egress_per_gb = 0.09         # After first 100GB/month
lb_base_per_hr = 0.025       # ALB hourly
lb_lcu_per_hr = 0.008        # LCU-hour (connections, data)
avg_lcu = 50                 # Typical for 1M req/day with bursts

# === Cloud TCO Calculations ===
# GPU: Must provision instances 24/7 for availability (not just compute seconds)
c_gpu = gpu_instances * hours_per_year * gpu_price_per_hr

# Egress: 1M requests × 100KB response = 100 GB/day
egress_gb_per_day = (requests_per_day * response_kb) / (1024 * 1024)  # Convert KB to GB
c_egress = egress_gb_per_day * days_per_year * egress_per_gb

# Load balancer: base + LCU charges
...
# === Edge TCO Calculations ===
e_capex = server_cost / server_life_years
e_power = (power_watts * hours_per_year * electricity_per_kwh) / 1000  # W to kW
...
requests_str = f"{requests_per_day/1e6:.0f}M"
...
egress_gb_str = f"{egress_gb_per_day:.0f}"
...
| **Network egress**       |                                 `{python} egress_gb_str` GB/day × 365 × $0.09/GB |    `{python} c_egress_str` |
| **Load balancer**        |                                                          $0.025/hr + LCU charges |        `{python} c_lb_str` |
...
# Wake-word economics calculation
ww_devices_b = 1  # billion devices
ww_cloud_cost_per_device = 0.50  # $ per device/year
ww_total_cloud_cost = ww_devices_b * 1e9 * ww_cloud_cost_per_device

ww_edge_power_min_mw = 0.1  # mW
ww_edge_power_max_mw = 1    # mW
ww_edge_cost_per_year = 0.01  # $ per year (battery cost)

ww_devices_b_str = str(ww_devices_b)
ww_cloud_cost_str = f"{ww_cloud_cost_per_device:.2f}"
ww_total_cost_str = f"{ww_total_cloud_cost:,.0f}"
ww_edge_power_range_str = f"{ww_edge_power_min_mw}--{ww_edge_power_max_mw}"
ww_edge_cost_str = f"{ww_edge_cost_per_year:.2f}"
```
...
- **Compute Cost**: ~$`{python} ww_cloud_cost_str` per device/year (conservative estimate)
- **Total Annual Cost**: $`{python} ww_devices_b_str`,000,000,000 \times \$`{python} ww_cloud_cost_str` = \$`{python} ww_total_cost_str`$
...
- **Cost Shift**: Transferred to user's battery (<$`{python} ww_edge_cost_str` per year)
...
cam_rate_mbs_str   = f"{raw_bytes_per_sec.to(MB/second).magnitude:.0f}"
total_rate_gbs_str = f"{total_bytes_per_sec.to(GB/second).magnitude:.1f}"
monthly_cost_m_str = f"{monthly_cost / 1e6:.1f}"
net_cap_gbs_str    = f"{NETWORK_10G_BW.to(GB/second).magnitude:.2f}"
bw_short_x_str     = f"{bw_shortage:.0f}"
...
3.  **Cloud upload cost**: At \$`{python} egress_cost_str`/GB egress, streaming 24/7 costs **\$`{python} monthly_cost_m_str`M/month**.
...
tx_energy_str     = f"{tx_energy_total.to(ureg.millijoule).magnitude:.0f}"
npu_energy_str    = f"{npu_energy_total.to(ureg.millijoule).magnitude}"
energy_gap_str    = f"{energy_gap_ratio.magnitude:.0f}"
```
...
sustained_gflops = inferences_per_sec * YOLOV8_NANO_FLOPs.to(GFLOPs).magnitude  # GFLOPs/sec
...
years = 3
hours_per_year = 8760
cloud_inference_cost_k = 500  # approx cloud cost for comparison
int8_throughput_mult = 4  # INT8 quantization throughput multiplier
...
nuc_tco = calc_fleet_tco(nuc_cost, nuc_power_w, stores, years, CLOUD_ELECTRICITY_PER_KWH)
...
# Format all values as strings for markdown use
stores_str           = f"{stores}"
cameras_per_store_str = f"{cameras_per_store}"
fps_str              = f"{fps}"
headroom_str         = f"{headroom}"
inf_per_sec_str      = f"{inferences_per_sec}"
yolo_gflops_str      = f"{YOLOV8_NANO_FLOPs.to(GFLOPs).magnitude:.1f}"
sustained_gf_str     = f"{sustained_gflops:.0f}"
req_tflops_str       = f"{required_tflops:.0f}"
coral_cost_str       = f"{coral_cost}"
coral_power_w_str    = f"{coral_power_w}"
coral_tops_str       = f"{coral_tops}"
jetson_cost_str      = f"{jetson_cost}"
jetson_tops_str      = f"{jetson_tops}"
jetson_power_range_str = jetson_power_range
nuc_cost_str         = f"{nuc_cost}"
nuc_power_w_str      = f"{nuc_power_w}"
nuc_tops_str         = f"{nuc_tops}"
coral_fleet_k_str    = f"{coral_fleet / 1000:.0f}"
jetson_fleet_k_str   = f"{jetson_fleet / 1000:.0f}"
nuc_fleet_k_str      = f"{nuc_cost * stores / 1000:.0f}"
coral_pwr_k_str      = f"{coral_power_cost / 1000:.0f}"
coral_tco_k_str      = f"{coral_tco / 1000:.0f}"
years_str            = f"{years}"
hours_per_year_str   = f"{hours_per_year}"
power_ratio_str      = f"{jetson_power_w / coral_power_w:.0f}"
elec_cost_str        = f"{CLOUD_ELECTRICITY_PER_KWH.magnitude}"
cloud_cost_k_str     = f"{cloud_inference_cost_k}"
int8_mult_str        = f"{int8_throughput_mult}"
cost_ratio_str       = f"{jetson_cost // coral_cost}"
```
...
|:------------------------|------------------------------:|:----------------------------------|:---------------------------|----------------------------------:|
| **NVIDIA Jetson Orin NX** | `{python} jetson_tops_str` TOPS | `{python} jetson_power_range_str` W | \$`{python} jetson_cost_str` | \$`{python} jetson_fleet_k_str`,000 |
| **Intel NUC + Movidius**  |    `{python} nuc_tops_str` TOPS | `{python} nuc_power_w_str` W        | \$`{python} nuc_cost_str`    |    \$`{python} nuc_fleet_k_str`,000 |
| **Google Coral Dev**      |  `{python} coral_tops_str` TOPS | `{python} coral_power_w_str` W      | \$`{python} coral_cost_str`  |  \$`{python} coral_fleet_k_str`,000 |
...
**TCO over `{python} years_str` years** (Coral): Hardware \$`{python} coral_fleet_k_str`K + Power (\$`{python} coral_power_w_str`×`{python} stores_str`×`{python} hours_per_year_str`h×`{python} years_str`yr×\$`{python} elec_cost_str`/kWh) = \$`{python} coral_fleet_k_str`K + \$`{python} coral_pwr_k_str`K = **\$`{python} coral_tco_k_str`,000 total** vs. cloud inference at ~\$`{python} cloud_cost_k_str`K.
:::
...
The mobile environment introduces a critical constraint absent from stationary deployments: *energy per inference* becomes a first-order design parameter. A **Compute Beast** workload like image classification must be transformed through architectural efficiency (e.g., depthwise separable convolutions[^fn-depthwise-separable] in MobileNet) to reduce FLOPs by `{python} mobilenet_flops_reduction`× while preserving accuracy. This is not merely optimization; it represents a fundamental shift in the arithmetic intensity trade-off, accepting lower peak throughput in exchange for sustainable operation within a `{python} mobile_tdp_range` W thermal envelope.
...
#| echo: false
#| label: battery-tax
from calc.formulas import md_frac
...
runtime_str = f"{runtime_q.to(ureg.hour).magnitude:.1f}"
...
budget_pct_str = f"{daily_budget_pct_calc.magnitude:.0f}"

# Create a nice LaTeX fraction for the runtime calculation
...
baseline_power_w = 12
quant_reduction = 4
quant_power_w = baseline_power_w / quant_reduction

baseline_str = f"{baseline_power_w:.0f}"
quant_power_str = f"{quant_power_w:.0f}"
quant_red_str = f"{quant_reduction:.0f}"
```
...
\index{Mobile ML!NPU inference} \index{Mobile ML!memory bandwidth limits} \index{Neural Processing Unit (NPU)!mobile devices}**Battery and Thermal Constraints.** Mobile devices exemplify intermediate constraints: `{python} mobile_ram_range` GB RAM (varying from mid-range to flagship), `{python} mobile_storage_range` storage, `{python} mobile_npu_range` TOPS AI compute through Neural Processing Units[^fn-npu] consuming `{python} mobile_tdp_range`W power. System-on-Chip architectures[^fn-mobile-soc] integrate computation and memory to minimize energy costs. Memory bandwidth of `{python} mobile_bw_range` GB/s limits models to 10-100MB parameters, requiring the aggressive optimization techniques that @sec-model-compression details. Battery constraints (`{python} phone_battery_str`-22Wh capacity) make energy optimization critical: 1W continuous ML processing reduces device lifetime from 24 to 18 hours. Specialized frameworks (TensorFlow Lite[^fn-tflite], Core ML[^fn-coreml]) provide hardware-optimized inference enabling <`{python} mobile_latency_range` ms UI response times.
...
# Latency
cloud_lat = "100 ms-1000 ms+"
edge_lat = "10-100 ms"
mobile_lat = "5-50 ms"
tiny_lat = "1-10 ms"

# Compute
cloud_comp = "Very High (Multiple GPUs/TPUs)"
edge_comp = "High (Edge GPUs)"
mobile_comp = "Moderate (Mobile NPUs/GPUs)"
tiny_comp = "Very Low (MCU/tiny processors)"

# Storage
cloud_stor = "Unlimited (petabytes+)"
edge_stor = "Large (terabytes)"
mobile_stor = "Moderate (gigabytes)"
tiny_stor = "Very Limited (kilobytes-megabytes)"

# Energy
cloud_pwr = "Very High (kW-MW range)"
edge_pwr = "High (100 s W)"
mobile_pwr = "Moderate (1-10 W)"
tiny_pwr = "Very Low (mW range)"

# Cost
cloud_cost = "High ($1000s+/month)"
edge_cost = "Moderate ($100s-1000s)"
mobile_cost = "Low ($0-10s)"
tiny_cost = "Very Low ($1-10s)"
```
...
| **Latency**                | `{python} cloud_lat`                     | `{python} edge_lat`                    | `{python} mobile_lat`         | `{python} tiny_lat`                                   |
| **Compute Power**          | `{python} cloud_comp`                    | `{python} edge_comp`                   | `{python} mobile_comp`        | `{python} tiny_comp`                                  |
| **Storage Capacity**       | `{python} cloud_stor`                    | `{python} edge_stor`                   | `{python} mobile_stor`        | `{python} tiny_stor`                                  |
| **Energy Consumption**     | `{python} cloud_pwr`                     | `{python} edge_pwr`                    | `{python} mobile_pwr`         | `{python} tiny_pwr`                                   |
...
| **Cost**                   | `{python} cloud_cost`                    | `{python} edge_cost`                   | `{python} mobile_cost`        | `{python} tiny_cost`                                  |
...
# Cloud: fast inference on powerful hardware, but network overhead
cloud_network_ms = 50   # Cross-region round-trip
cloud_inference_ms = 10 # A100 GPU inference
cloud_total_ms = cloud_network_ms + cloud_inference_ms

# Edge: slow inference on weak hardware, but low network latency
edge_network_ms = 5     # Local network
edge_inference_ms = 80  # Raspberry Pi inference (8× slower than A100)
edge_total_ms = edge_network_ms + edge_inference_ms

# Format strings
cloud_net_str = f"{cloud_network_ms}"
cloud_inf_str = f"{cloud_inference_ms}"
cloud_total_str = f"{cloud_total_ms:.0f}"
edge_net_str = f"{edge_network_ms}"
edge_inf_str = f"{edge_inference_ms}"
edge_total_str = f"{edge_total_ms:.0f}"
```
