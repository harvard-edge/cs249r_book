...
Where an ML model runs is not merely a deployment decision—it fundamentally shapes what is possible. Physical constraints of latency, power, and memory create distinct operating regimes, each with its own engineering trade-offs. This framework introduces four deployment paradigms and examines the physical laws that make each one necessary.
...
Throughout this volume, we use five **Lighthouse Examples** introduced in @sec-introduction—concrete workloads that span the deployment spectrum and isolate distinct system bottlenecks. @sec-dnn-architectures provides full architectural details and model biographies.
...
+----------------------------------+---------------------------+--------------------------------+
| **KWS (DS-CNN)**                 | Tiny Constraint           | TinyML, always-on              |
+----------------------------------+---------------------------+--------------------------------+
...
::: {.callout-notebook title="The 1000x Energy Gap"}
...
::: {.callout-notebook title="Energy Per Inference: The Million-Fold Gap"}
