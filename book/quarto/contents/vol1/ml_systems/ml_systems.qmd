...
::: {#fig-cloud-edge-TinyML-comparison fig-env="figure" fig-pos="htb" fig-cap="**Distributed Intelligence Spectrum**: Deployment tiers form a continuum governed by physical constraints. The representative 3 MW and 50 mW anchors span nearly eight orders of magnitude in power. Moving from centralized cloud infrastructure to on-device TinyML trades raw computational capacity and elastic scale for deterministic latency, data locality, and network independence." fig-alt="Horizontal spectrum showing five deployment tiers from left to right: ultra-low-power devices and sensors, intelligent device, gateway, on-premises servers, and cloud. Arrows indicate TinyML, edge AI, and cloud AI spans across the spectrum."}
...
::: {#fig-mlsys-playbook-flowchart fig-env="figure" fig-pos="htb" fig-cap="**Deployment Decision Screening**: Four layers record privacy, latency, compute-demand, and cost constraints. The feasible deployment set is the intersection of all four results; no single branch selects an architecture in isolation." fig-alt="Screening diagram with four question layers for privacy, low-latency requirements, compute demand, and cost. Each question records one of two constraint outcomes. The four results feed an intersection step that yields feasible cloud ML, edge ML, mobile ML, and TinyML candidates."}
...
\node[Box, right=of 2B1](2B3){Low-Latency\\ Local Target};
%
...
\node[start,below =1.85 of 4B3](5B4){TinyML};
...
\draw[Line](2B1)--(3B1);
\draw[Line](3B1)--(4B1);
\node[start,text width=24mm,minimum width=28mm,minimum height=8mm,inner ysep=2pt]
  (INT) at ($(5B2.north east)!0.5!(5B3.north west)+(0,9mm)$)
  {Intersect Recorded Constraints};
\draw[Line](4B1)--(INT);
\draw[Line](INT.south)--++(0,-5mm)-|(5B1);
\draw[Line](INT.south)--++(0,-5mm)-|(5B2);
\draw[Line](INT.south)--++(0,-5mm)-|(5B3);
\draw[Line](INT.south)--++(0,-5mm)-|(5B4);
...
\node[draw=BlueLine,inner xsep=8mm,inner ysep=6mm,yshift=-0.85mm,
  fill=cyan!03,fit=(INT)(5B1)(5B4),  minimum width=155mm, line width=0.35pt](BB5){};
%
