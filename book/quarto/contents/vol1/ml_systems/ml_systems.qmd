...
The **Compute Beast**\index{Arithmetic Intensity!high intensity workloads}\index{Compute Beast!definition} appears when arithmetic work dominates. The workload performs many calculations per byte loaded, so progress comes from higher arithmetic throughput, better utilization, and more parallel execution. Large neural-network training is the canonical case.

The **Bandwidth Hog**\index{Autoregressive Generation!memory-bound}\index{Bandwidth Hog!definition} appears when dense weight or activation movement dominates. Each token or example waits on memory bandwidth, so wider memory buses and better data reuse matter more than additional peak FLOP/s. Autoregressive text generation illustrates this regime.

The **Sparse Scatter**\index{Embedding Table!memory capacity bound}\index{Sparse Scatter!definition} appears when data movement is dominated by irregular table lookups and poor cache locality. Memory capacity, access latency, and communication shape performance, as in recommendation systems with massive embedding tables.

The **Tiny Constraint**\index{Energy per Inference!binding constraint}\index{Always-On Sensing!power constraints}\index{Tiny Constraint!definition} appears when the machine envelope itself dominates. Energy per inference and memory footprint, not raw speed, determine feasibility for always-on sensing.
...
class HardwareSpectrumSetup:
    ...
    cloud_threshold_bw = 1000 * (GB / second)  # scenario assumption: cloud deployment boundary
    edge_threshold_bw = 270 * (GB / second)  # scenario assumption: edge deployment boundary
    tiny_threshold_compute = 1 * TOPS  # scenario assumption: TinyML deployment boundary
    tiny_threshold_power = 1 * milliwatt  # scenario assumption: TinyML always-on target
    ...
    edge_thresh_bw_gb_per_s_str = fmt_bandwidth(
        edge_threshold_bw,
        unit=GB / second,
        precision=0,
        commas=False,
    )
    tiny_thresh_tops_str = fmt_ops_rate(
        tiny_threshold_compute,
        unit=TOPS,
        precision=0,
        commas=False,
    )
    tiny_thresh_mw_str = fmt_power(
        tiny_threshold_power,
        unit=milliwatt,
        precision=0,
        commas=False,
    )
...
class DeploymentThresholdsTable:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    cloud_compute_threshold = 1000 * (TFLOPs / second)  # scenario assumption: cloud deployment boundary
    cloud_bandwidth_threshold = 1000 * (GB / second)  # scenario assumption: cloud deployment boundary
    edge_compute_threshold = 1 * (PFLOP / second)  # scenario assumption: edge deployment boundary
    edge_bandwidth_threshold = 270 * (GB / second)  # scenario assumption: edge deployment boundary
    tiny_compute_threshold = 1 * TOPS  # scenario assumption: TinyML compute boundary
    tiny_power_threshold = 1 * milliwatt  # scenario assumption: TinyML always-on target
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, fmt_qty, fmt_count, fmt_flops, fmt_flop_rate
...
class MobileResnetPitfallRecap:
    ...
    edge_ram = 512 * MB  # scenario assumption: low-end edge target RAM
    edge_compute = 1 * (GFLOPs / second)  # scenario assumption: low-end edge target compute
