...
# │ Exports: MLSystemsNvlinkAnchor.nvlink_h100_bw_gb_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
class MLSystemsNvlinkAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_bw_gb_s_str = fmt_qty_int(Hardware.Cloud.H100.nvlink.bandwidth, GB/second, commas=False)
```
...
# │          EnergyTransmission.local_total_mj_str, EnergyTransmission.ratio_str
# └─────────────────────────────────────────────────────────────────────────────
...
*   **Compute energy $(E_{\text{op}})$**: `{python} EnergyTransmission.compute_energy_mj_str` (MobileNet on NPU).
...
# │          .dlrm_embedding_gb_str, .mobilenet_flops_reduction_str,
...
# │ Exports: DistancePenalty.sol_kms_s_str, DistancePenalty.rtt_formatted_ms_str,
...
class DistancePenalty:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sol_kms_s_str = fmt_qty(SPEED_OF_LIGHT_FIBER_KM_S, kilometer / second, precision=0, commas=True)
...
1.  **Light in Fiber**: ~`{python} DistancePenalty.sol_kms_s_str`.
2.  **Round-trip propagation**: (`{python} DistancePenalty.distance_km_str` $\times$ 2)/`{python} DistancePenalty.sol_kms_s_str` = `{python} DistancePenalty.rtt_formatted_ms_str`.
...
class VoiceAssistantWall:
    ...
    vi_audio_tb_s_str = fmt_qty(vi_total_audio_rate, TB/second, precision=0, commas=False)
```
...
**The bandwidth wall**: Wake-word detection requires continuous audio monitoring. If devices streamed audio to the cloud (16 kHz, 16-bit), each transmits ~`{python} VoiceAssistantWall.vi_audio_kb_str`. Across `{python} VoiceAssistantWall.vi_devices_str` billion devices: `{python} VoiceAssistantWall.vi_audio_tb_s_str`, a significant fraction of total global internet backbone capacity.
...
# │          DataLocalityInvariant.net_bw_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
class DataLocalityInvariant:
    ...
    net_bw_s_str = fmt_qty(net_bw, megabit / second, precision=0, commas=False, unit_label="Mb/s")
...
- **Bandwidth $(\text{BW}_{\text{network}})$**: `{python} DataLocalityInvariant.net_bw_s_str` home broadband (up).
- **Remote response $(L_{\text{lat,network}} + T_{\text{remote}})$**: `{python} DataLocalityInvariant.remote_ms_str` (round-trip + remote compute).

**Math**:

1.  **Transmission time**: `{python} DataLocalityInvariant.frame_mb_str` $\times$ 8 bits/`{python} DataLocalityInvariant.net_bw_s_str` = `{python} DataLocalityInvariant.tx_time_ms_str`.
...
class ThermalQuantCalc:
    ...
    temp_rise_s_str = fmt_qty(temp_rise, ureg.delta_degC / second, precision=0, commas=False, unit_label="°C/s")
...
**Systems insight**: Quantization from FP32 to INT8 reduces power by approximately `{python} ThermalQuantCalc.quant_red_str`$\times$, but with a baseline of `{python} ThermalQuantCalc.baseline_w_str` the result is still `{python} ThermalQuantCalc.quant_power_w_str`---the absolute limit of the hardware. Physics sets a hard ceiling that no optimization can exceed.
...
| **Cloud**    | ResNet-50 (A100)     | `{python} EnergyInference.e_resnet_cloud_mj_str` |                                                            `{python} EnergyInference.q_resnet_cloud_str` |
...
class MobilePowerFallacyCalc:
    ...
    low_power_frac = fmt_frac(f"{phone_battery_wh_str}", f"{low_power_w_str}", f"**{low_power_hours_str} hours**")
    high_power_frac = fmt_frac(f"{phone_battery_wh_str}", f"{high_power_w_str}", f"**{high_power_hours_str} hours**")
```
