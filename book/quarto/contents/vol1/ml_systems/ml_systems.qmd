...
# ┌─────────────────────────────────────────────────────────────────────────────
# │ NVLINK ANCHOR (SINGLE-NODE STACK)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Architectural Anchor section — Hardware layer bullet
# │
# │ Goal: Cite H100 NVLink bandwidth in the single-node stack description.
# │ Show: NVLink H100 intra-node interconnect bandwidth.
# │ How: Read Hardware.Cloud.H100.nvlink.bandwidth from mlsysim constants.
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.nvlink.bandwidth, GB, second)
# │ Exports: MLSystemsNvlinkAnchor.nvlink_h100_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int

# ┌── LEGO ───────────────────────────────────────────────
class MLSystemsNvlinkAnchor:
    """NVLink bandwidth anchor for the single-node stack hardware layer."""

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_bw_str = fmt_int(Hardware.Cloud.H100.nvlink.bandwidth.m_as(GB/second), commas=False, suffix=" GB/s")
```
...
1.  **Application (The Mission)**: The top layer where high-level requirements (throughput for training loops, latency for inference serving) are defined. This is where the "Dual Mandate" of accuracy and physics is managed (@sec-model-training, @sec-model-serving).
2.  **ML Framework (The Compiler)**\index{ML Framework!compiler role}: The translation layer (PyTorch, JAX) that maps high-level math to hardware-specific execution plans. It manages the computational graph, automatic differentiation, and memory scheduling (@sec-ml-frameworks).
3.  **Operating System (The Runtime)**\index{Operating System!runtime role}: The interface between framework and hardware, responsible for the low-level orchestration of resources. This includes the **CUDA Runtime**\index{CUDA Runtime!kernel management} for kernel management and **PCIe DMA**\index{PCIe DMA!data movement} (Direct Memory Access) for efficient data movement between host and device.
4.  **Hardware (The Silicon)**: The physical foundation where bits are transformed. This layer is defined by HBM (High Bandwidth Memory) capacity and high-speed intra-node interconnects like **NVLink**\index{NVLink!intra-node interconnect} (`{python} MLSystemsNvlinkAnchor.nvlink_h100_bw_str`). Here, the memory wall acts as the primary physical constraint (@sec-hardware-acceleration).
...
class DeploymentOverviewTable:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_latency_range_str = Platforms.Cloud.latency_range_ms
    edge_latency_range_str = Platforms.Edge.latency_range_ms
    mobile_latency_range_str = Platforms.Mobile.latency_range_ms
    tiny_latency_range_str = Platforms.Tiny.latency_range_ms
...
class LightLatency:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    min_latency_str = fmt(min_latency_ms, precision=0, commas=False, suffix=" ms round-trip")
    distance_str = fmt(distance_km.m_as('km'), precision=0, suffix=" km")
```
...
class ThrottlingScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    fps_start_str = fmt(fps_start, precision=0, commas=False, suffix=" FPS")
    fps_throttled_str = fmt(fps_throttled, precision=0, commas=False, suffix=" FPS")
...
class EnergyTransmission:
    ...
    Scenario: Cost of sending 1 MB to cloud vs running MobileNet locally.
    """

    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    data_size = 1.0 * MB       # 1 sec audio
    tx_energy_per_mb = Systems.NetworkEnergy.Per5gMb
    local_energy_op = Models.Vision.MobileNetV2.inference_energy

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    cloud_energy_total = (data_size * tx_energy_per_mb).to(ureg.millijoule)
    local_energy_total = local_energy_op

    ratio = cloud_energy_total/local_energy_total

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(ratio.m_as("") >= 500,
          f"Transmission ({cloud_energy_total}) is not expensive enough vs Compute ({local_energy_total}). Ratio: {ratio.m_as(''):.1f}x")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    data_mb_str = fmt(data_size.m_as(MB), precision=0, commas=False, suffix=" MB")
    tx_energy_str = fmt(tx_energy_per_mb.m_as(ureg.millijoule/MB), precision=0, commas=False, suffix=" mJ/MB")
    compute_energy_str = fmt(local_energy_op.m_as(ureg.millijoule), precision=1, commas=False, suffix=" mJ/inference")
    cloud_total_str = fmt(cloud_energy_total.m_as(ureg.millijoule), precision=0, commas=False, suffix=" mJ")
    local_total_str = fmt(local_energy_total.m_as(ureg.millijoule), precision=1, commas=False, suffix=" mJ")
    ratio_str = fmt(ratio.m_as(""), precision=0, commas=True)
```
...
*   **Transmission energy $(E_{\text{tx}})$**: `{python} EnergyTransmission.tx_energy_str` (Wi-Fi/LTE).
*   **Compute energy $(E_{\text{op}})$**: `{python} EnergyTransmission.compute_energy_str` (MobileNet on NPU).
...
\vspace*{-3pt}
```
1.  **Cloud approach**: $E_{\text{cloud}} \approx D_{\text{vol}} \times E_{\text{tx}}$ = `{python} EnergyTransmission.data_mb_str` $\times$ `{python} EnergyTransmission.tx_energy_str` = `{python} EnergyTransmission.cloud_total_str`.
2.  **Local approach**: $E_{\text{local}} \approx$ Inference = `{python} EnergyTransmission.local_total_str`.

**Systems insight**: Transmitting raw data is `{python} EnergyTransmission.ratio_str`$\times$ more expensive than processing it locally. Even if the cloud had infinite speed $(T \approx 0)$, the energy wall makes cloud offloading physically impossible for always-on battery devices. The "machine" constraint (battery) dictates the "algorithm" choice (TinyML).
...
To navigate these constraints systematically, we categorize ML workloads into four **Archetypes**\index{Workload Archetype!definition}[^fn-archetype-bottleneck]. These represent the primary physical bottlenecks, not just specific model architectures. We introduce each archetype briefly here; the Lighthouse Models that follow will ground each category in concrete, recurring examples.

The first archetype, the **Compute Beast**\index{Arithmetic Intensity!high intensity workloads}\index{Compute Beast!definition}, describes workloads that perform many calculations per byte of data loaded. The binding constraint is raw computational throughput. Training large neural networks falls into this category.

The second archetype, the **Bandwidth Hog**\index{Autoregressive Generation!memory-bound}\index{Bandwidth Hog!definition}, describes workloads that spend more time loading data than computing. Memory bandwidth becomes the binding constraint. Autoregressive text generation (like ChatGPT producing one token at a time) falls into this category.

The third archetype, the **Sparse Scatter**\index{Embedding Table!memory capacity bound}\index{Sparse Scatter!definition}, describes workloads with irregular memory access patterns and poor cache locality. Memory capacity and access latency constrain performance. Recommendation systems with massive embedding tables are canonical examples.
...
```{python}
#| echo: false
#| label: lighthouse-archetype-table
# ┌─────────────────────────────────────────────────────────────────────────────
# │ LIGHTHOUSE ARCHETYPE TABLE
# ├─────────────────────────────────────────────────────────────────────────────
# │ Goal: Render the book's five canonical workload archetypes from MLSysIM.
# │ How: Use Scenarios.Archetypes.Workload; those records point back to Models.*.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Scenarios as BookScenarios
from mlsysim.fmt import MarkdownStr

class LighthouseArchetypeTable:
    """Display fields for the canonical Volume I workload archetypes."""

    resnet = BookScenarios.Archetypes.Workload.ResNet50
    gpt2 = BookScenarios.Archetypes.Workload.GPT2
    dlrm = BookScenarios.Archetypes.Workload.DLRM
    mobilenet = BookScenarios.Archetypes.Workload.MobileNetV2
    kws = BookScenarios.Archetypes.Workload.KWS

    resnet_label_str = MarkdownStr(resnet.label)
    resnet_archetype_str = MarkdownStr(resnet.archetype)
    resnet_deployment_str = MarkdownStr(resnet.deployment_paradigm)
    gpt2_label_str = MarkdownStr("GPT-2/Llama")
    gpt2_archetype_str = MarkdownStr(gpt2.archetype)
    gpt2_deployment_str = MarkdownStr(gpt2.deployment_paradigm)
    dlrm_label_str = MarkdownStr(dlrm.label)
    dlrm_archetype_str = MarkdownStr(dlrm.archetype)
    dlrm_deployment_str = MarkdownStr(dlrm.deployment_paradigm)
    mobilenet_label_str = MarkdownStr(mobilenet.workload_label)
    mobilenet_archetype_str = MarkdownStr(mobilenet.archetype)
    mobilenet_deployment_str = MarkdownStr(mobilenet.deployment_paradigm)
    kws_label_str = MarkdownStr(kws.label)
    kws_archetype_str = MarkdownStr(kws.archetype)
    kws_deployment_str = MarkdownStr(kws.deployment_paradigm)
```

| **Lighthouse**                                              | **Archetype**                                               | **Deployment Paradigm**                                      |
|:------------------------------------------------------------|:------------------------------------------------------------|:-------------------------------------------------------------|
| **`{python} LighthouseArchetypeTable.resnet_label_str`**    | `{python} LighthouseArchetypeTable.resnet_archetype_str`    | `{python} LighthouseArchetypeTable.resnet_deployment_str`    |
| **`{python} LighthouseArchetypeTable.gpt2_label_str`**      | `{python} LighthouseArchetypeTable.gpt2_archetype_str`      | `{python} LighthouseArchetypeTable.gpt2_deployment_str`      |
| **`{python} LighthouseArchetypeTable.dlrm_label_str`**      | `{python} LighthouseArchetypeTable.dlrm_archetype_str`      | `{python} LighthouseArchetypeTable.dlrm_deployment_str`      |
| **`{python} LighthouseArchetypeTable.mobilenet_label_str`** | `{python} LighthouseArchetypeTable.mobilenet_archetype_str` | `{python} LighthouseArchetypeTable.mobilenet_deployment_str` |
| **`{python} LighthouseArchetypeTable.kws_label_str`**       | `{python} LighthouseArchetypeTable.kws_archetype_str`       | `{python} LighthouseArchetypeTable.kws_deployment_str`       |
...
doorbell_strict = mlsysim.Applications.Doorbell.model_copy(
...
# │
# │ Goal: Provide specs for the five Lighthouse Models backed by the canonical
# │       Scenarios.Archetypes.Workload records.
# │ Show: Parameter count, FLOP profile, and memory footprint per workload.
# │ How: Resolve each archetype to its Models.* workload, then derive sizes
# │       using size_in_bytes() with 4-byte (FP32) precision.
# │
# │ Imports: mlsysim.Models, mlsysim.Scenarios, mlsysim.fmt
...
# │          .mobile_tdp_range_str, .kws_params_str, .kws_flops_m_str,
# │          .kws_size_kb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models, Scenarios as BookScenarios
...
class LighthouseModels:
    ...
    # ┌── 1. LOAD (Archetype records resolve to canonical Models.* objects) ─
    a_resnet = BookScenarios.Archetypes.Workload.ResNet50
    a_gpt2 = BookScenarios.Archetypes.Workload.GPT2
    a_dlrm = BookScenarios.Archetypes.Workload.DLRM
    a_mobilenet = BookScenarios.Archetypes.Workload.MobileNetV2
    a_kws = BookScenarios.Archetypes.Workload.KWS

    m_resnet = a_resnet.workload
    m_gpt2 = a_gpt2.workload
    m_dlrm = a_dlrm.workload
    m_mobilenet = a_mobilenet.workload
    m_kws = a_kws.workload

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    resnet_flops_g = m_resnet.inference_flops.m_as(GFLOPs)
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(resnet_fp32_mb >= 90, f"ResNet50 size should be ~98 MB, got {resnet_fp32_mb:.0f} MB")
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    resnet_gflops_str = fmt(resnet_flops_g, precision=1, suffix=" GFLOPs")
    ...
    mobilenet_flops_reduction_str = fmt(mobilenet_flops_reduction, precision=1, suffix="×")
    mobile_tdp_range_str = MarkdownStr("2 to 5 W")
    kws_params_str = fmt(kws_params, precision=0, commas=False, suffix=" K parameters")
    kws_flops_m_str = fmt(kws_flops_m, precision=0, suffix=" MFLOPs")
    kws_size_kb_str = fmt(kws_size_kb, precision=0, suffix=" KB")
```

The first lighthouse, **ResNet-50**\index{ResNet-50!systems characteristics}, classifies images into one thousand categories, processing each image through approximately `{python} LighthouseModels.resnet_gflops_str` using `{python} LighthouseModels.resnet_params_m_str` (`{python} LighthouseModels.resnet_fp32_mb_str` at FP32). Used in medical imaging diagnostics, autonomous vehicle perception pipelines, and as the backbone for content moderation systems, its regular, compute-dense structure makes it the canonical benchmark for hardware accelerator performance.
...
class LatencyConstants:
    ...
    lat_npu_str = MarkdownStr("5–20 ms")                                  # NPU inference (MobileNet)
...
| **Cloud LLM Inference** | $D_{\text{vol}}/\text{BW}$ (memory bandwidth)          | Autoregressive: ~1 FLOP/byte, memory-bound                 | KV-caching, quantization, batching           |
| **Edge Inference**      | $D_{\text{vol}}/\text{BW}$ (memory bandwidth)          | Limited HBM; models often memory-bound                     | Model compression, operator fusion           |
| **Mobile**              | Energy (implicit)                                      | Battery = $\int \text{Power} \cdot dt$; thermal throttling | Reduced precision, duty cycling              |
| **TinyML**              | $M_{\text{model}} \le C_{\text{mem}}$ (Capacity)       | 256 KB total; model must fit on-chip                       | Extreme compression, binary networks         |
...
: **D·A·M × Phase**\index{D·A·M Taxonomy!training vs. inference}: The same model imposes starkly different demands on Data, Algorithm, and Machine depending on whether the system is training or serving. When bottlenecks shift unexpectedly, check which phase is currently being optimized. {#tbl-dam-phase tbl-colwidths="[20,40,40]"}
...
# │ Show: How quantization reduces the data volume term of the iron law.
...
class ResnetSetup:
    ...
    resnet_gflops_value = m.inference_flops.m_as(GFLOPs)
    resnet_params_m_value = m.parameters.m_as(Mparam)

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    resnet_gflops_str = fmt(resnet_gflops_value, precision=1, commas=False, suffix=" GFLOPs")
    resnet_params_m_str = fmt(resnet_params_m_value, precision=1, commas=False, suffix=" million parameters")
    resnet_fp32_mb_str = fmt(resnet_fp32_bytes_value.m_as(MB), precision=1, commas=False, suffix=" MB")
    resnet_fp16_mb_str = fmt(resnet_fp16_bytes_value.m_as(MB), precision=1, commas=False, suffix=" MB")
    resnet_int8_mb_str = fmt(resnet_int8_bytes_value.m_as(MB), precision=1, commas=False, suffix=" MB")
```

- ResNet-50: `{python} ResnetSetup.resnet_gflops_str` per inference, `{python} ResnetSetup.resnet_params_m_str` (`{python} ResnetSetup.resnet_fp32_mb_str` FP32, `{python} ResnetSetup.resnet_fp16_mb_str` FP16)
...
class ResnetCloud:
    ...
    cloud_compute_ms_value = p.latency_compute.m_as("ms")
    cloud_memory_ms_display = (p.memory_footprint / p.peak_bw_actual).m_as("ms")
    cloud_ratio_x_value = cloud_memory_ms_display / cloud_compute_ms_value
    cloud_ai_display = m.inference_flops.m_as("flop") / p.memory_footprint.m_as("byte")
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_tflops_str = fmt(p.peak_flops_actual.m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
    a100_bw_tbs_str = fmt(p.peak_bw_actual.m_as(TB/second), precision=2, commas=False, suffix=" TB/s")
    cloud_ratio_x_str = fmt(cloud_ratio_x_value, precision=1, commas=False)
    cloud_bottleneck_str = MarkdownStr(p.bottleneck)
```

- Peak compute: `{python} ResnetCloud.a100_tflops_str` (FP16)
- Memory bandwidth: `{python} ResnetCloud.a100_bw_tbs_str` (HBM2e)
...
# │ How: Engine.solve for mobile NPU; duplicate A100 FP16 memory time locally.
# │
# │ Imports: mlsysim (Models, Hardware, Engine), mlsysim.fmt
# │ Exports: ResnetMobile.mobile_tops_str, .mobile_bw_gbs_str, .mobile_ratio_x_str,
# │          .mobile_bottleneck_str, .bw_advantage_x_str, .inference_speed_x_str,
# │          .mobile_compute_frac, .mobile_memory_frac
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models, Hardware, Engine
from mlsysim.fmt import sci_latex, fmt_frac, fmt, MarkdownStr
...
class ResnetMobile:
    ...
    cloud_memory_ms_display = (p_cloud.memory_footprint / p_cloud.peak_bw_actual).m_as("ms")
    mobile_compute_ms_display = (m.inference_flops / p.peak_flops_actual).m_as("ms")
    mobile_memory_ms_display = (p.memory_footprint / p.peak_bw_actual).m_as("ms")
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mobile_tops_str = fmt(p.peak_flops_actual.m_as(TOPS), precision=0, commas=False, suffix=" TOPS")
    mobile_bw_gbs_str = fmt(p.peak_bw_actual.m_as(GB/second), precision=0, commas=False, suffix=" GB/s")
    mobile_ratio_x_str = fmt(mobile_ratio_x_value, precision=1, commas=False)
    mobile_bottleneck_str = MarkdownStr(p.bottleneck)
    bw_advantage_x_str = fmt(bw_advantage_x_value, precision=1, commas=False)
    inference_speed_x_str = fmt(inference_speed_x_value, precision=1, commas=False)
```

- Peak compute: ~`{python} ResnetMobile.mobile_tops_str` (INT8)—representative of modern mobile NPUs
- Memory bandwidth: ~`{python} ResnetMobile.mobile_bw_gbs_str` (LPDDR5)
...
**Key Insight**\index{Quantization!deployment benefits}: Both platforms are memory bound for single-image inference! The A100's faster memory bandwidth (`{python} ResnetCloud.a100_bw_tbs_str` vs. `{python} ResnetMobile.mobile_bw_gbs_str` = `{python} ResnetMobile.bw_advantage_x_str`$\times$) translates to roughly `{python} ResnetMobile.inference_speed_x_str`$\times$ faster inference; peak compute alone is not the limiting comparison. This explains why quantization (reducing bytes) often beats faster hardware (increasing FLOP/s) for deployment.

ResNet-50 becomes compute bound when batching and data reuse raise arithmetic intensity above the hardware balance point, $I > R_{\text{peak}}/\text{BW}$, where $I = O/D_{\text{vol}}$. The crossover is architecture- and implementation-dependent because activation traffic, input/output movement, cache reuse, and kernel fusion all change the effective bytes moved per inference.
...
from mlsysim.fmt import fmt_percent, fmt_int, fmt_usd, fmt, check, MarkdownStr
...
class HardwareSpectrumSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tpu_chips_str = fmt(Systems.Pods.TPUv4_4096.chips, precision=0)          # e.g. "4,096" chips
    cloud_mem_tb_str = fmt(Systems.Pods.TPUv4_4096.memory.m_as(TB), precision=0, commas=False, suffix=" TB")
    cloud_pwr_mw_str = fmt(Systems.Pods.TPUv4_4096.power.m_as("megawatt"), precision=0, commas=False, suffix=" MW")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    edge_mem_gb_str = fmt(Hardware.Workstation.DGX_Spark.memory.capacity.m_as(GB), precision=0, commas=False, suffix=" GB")
    edge_stor_tb_str = fmt(Hardware.Workstation.DGX_Spark.storage.capacity.m_as(TB), precision=0, commas=False, suffix=" TB")
    edge_pwr_w_str = fmt(Hardware.Workstation.DGX_Spark.tdp.m_as(watt), precision=0, commas=False, suffix=" W")
    edge_price_min_str = fmt_usd(Hardware.Workstation.DGX_Spark.unit_cost.m_as("dollar"), precision=0)
    edge_price_max_str = fmt_usd(Hardware.Workstation.DGX_Spark.unit_cost_max.m_as("dollar"), precision=0)

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tiny_flash_mb_str = fmt(Hardware.Tiny.ESP32_S3.memory.flash_capacity.m_as(MB), precision=0, commas=False, suffix=" MB")
    tiny_pwr_min_str = fmt(Hardware.Tiny.ESP32_S3.tdp_min.m_as(watt), precision=2, commas=False, suffix=" W")
    tiny_pwr_max_str = fmt(Hardware.Tiny.ESP32_S3.tdp_max.m_as(watt), precision=1, commas=False, suffix=" W")
    tiny_price_str = fmt_usd(10, precision=0, commas=False)

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cloud_thresh_tflops_str = MarkdownStr("1000")            # TFLOP/s threshold for cloud
    cloud_thresh_bw_str = MarkdownStr("1000 GB/s")
    edge_thresh_pflops_str = MarkdownStr("1")                # PFLOP/s AI compute threshold
    edge_thresh_bw_str = MarkdownStr("270 GB/s")
...
class RepresentativeSystemsMobileRow:
    ...
    mobile_storage_range_str = Platforms.Mobile.storage_range
    mobile_tdp_range_str = MarkdownStr(f"{Platforms.Mobile.tdp_range_w} W")
```
...
|:--------------|:-------------------:|:-----------------------------------------------------------------------:|:--------------------------------------------------------------:|:------------------------------------------------------------------:|:----------------------------------------------------------------------------------------------------------------------:|:--------------------------------------------------------------------------------------------------------:|
| **Cloud ML**  |  Google TPU v4 Pod  | `{python} HardwareSpectrumSetup.tpu_chips_str` TPU v4 chips, >1 EFLOP/s |     `{python} HardwareSpectrumSetup.cloud_mem_tb_str` HBM2     |                          Cloud-scale (PB)                          |                                   ~`{python} HardwareSpectrumSetup.cloud_pwr_mw_str`                                   |                                          Cloud service (rental)                                          |
| **Edge ML**   |   NVIDIA DGX Spark  |                    GB10 Grace Blackwell, 1 PFLOP/s AI                   |    `{python} HardwareSpectrumSetup.edge_mem_gb_str` LPDDR5x    |       `{python} HardwareSpectrumSetup.edge_stor_tb_str` NVMe       |                                    ~`{python} HardwareSpectrumSetup.edge_pwr_w_str`                                    | ~`{python} HardwareSpectrumSetup.edge_price_min_str`–`{python} HardwareSpectrumSetup.edge_price_max_str` |
...
| **TinyML**    |      ESP32-CAM      |                           Dual-core @ 240 MHz                           |                           520 KB RAM                           |      `{python} HardwareSpectrumSetup.tiny_flash_mb_str` Flash      | `{python} HardwareSpectrumSetup.tiny_pwr_min_str`–`{python} HardwareSpectrumSetup.tiny_pwr_max_str` active board power |                             `{python} HardwareSpectrumSetup.tiny_price_str`                              |
...
class DeploymentThresholdsTable:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    mobile_npu_values = [phone.compute.peak_flops.m_as(TOPS) for phone in representative_phones]
    mobile_bw_values = [phone.memory.bandwidth.m_as(GB/second) for phone in representative_phones]
    mobile_npu_min = min(mobile_npu_values)
    mobile_npu_max = max(mobile_npu_values)
    mobile_bw_min = min(mobile_bw_values)
    mobile_bw_max = max(mobile_bw_values)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(mobile_npu_max > mobile_npu_min, "Mobile NPU range collapsed to a single value.")
    check(mobile_bw_max > mobile_bw_min, "Mobile bandwidth range collapsed to a single value.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_thresh_tflops_str = MarkdownStr(">1000 TFLOP/s")
    cloud_thresh_bw_str = MarkdownStr(">1000 GB/s")
    cloud_latency_range_str = Platforms.Cloud.latency_range_ms
    edge_thresh_pflops_str = MarkdownStr("~1 PFLOP/s")
    edge_thresh_bw_str = MarkdownStr(">270 GB/s")
    edge_latency_range_str = Platforms.Edge.latency_range_ms
    mobile_npu_range_str = MarkdownStr(f"{fmt(mobile_npu_min, precision=0, commas=False)}-{fmt(mobile_npu_max, precision=0, commas=False)} TOPS")
    mobile_bw_range_str = MarkdownStr(f"{fmt(mobile_bw_min, precision=0, commas=False)}-{fmt(mobile_bw_max, precision=0, commas=False)} GB/s")
    mobile_tdp_range_str = MarkdownStr(f"{Platforms.Mobile.tdp_range_w} W")
    mobile_latency_range_str = Platforms.Mobile.latency_range_ms
...
|:--------------|:------------------------------------------------------------:|:--------------------------------------------------------:|:--------------------------------------------------------------------------------:|:-------------------------------------------------------------:|
| **Cloud ML**  | `{python} DeploymentThresholdsTable.cloud_thresh_tflops_str` | `{python} DeploymentThresholdsTable.cloud_thresh_bw_str` |                              MW class (PUE 1.1–1.3)                              |  `{python} DeploymentThresholdsTable.cloud_latency_range_str` |
| **Edge ML**   | `{python} DeploymentThresholdsTable.edge_thresh_pflops_str`  | `{python} DeploymentThresholdsTable.edge_thresh_bw_str`  |                                   100 W class                                    |  `{python} DeploymentThresholdsTable.edge_latency_range_str`  |
...
class GPT3TrainingScale:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    gpt3_pflop_days = (m_gpt3.training_ops / (PFLOPs * SEC_PER_DAY)).to_base_units().m_as(ureg.dimensionless)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(gpt3_pflop_days >= 3000, f"GPT-3 training should be >=3000 PFLOP-days, got {gpt3_pflop_days:.0f}")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpt3_pflop_days_str = fmt(gpt3_pflop_days, precision=1, commas=True, suffix=" PFLOP-days")
    gpt3_v100_count_str = fmt(gpt3_v100_count, precision=0, commas=True, suffix=" V100 GPUs")
    gpt3_days_str = fmt(gpt3_days, precision=0, suffix=" days")
    gpt3_cost_m_str = fmt_usd(gpt3_cost_m, precision=1, suffix="M")
```
...
[^fn-nlp-training-scale]: **Large Language Model (LLM) Training Scale**: GPT-3 required approximately `{python} GPT3TrainingScale.gpt3_pflop_days_str`, 10,000 V100 GPUs, and an estimated \$4.6M in compute at 2020 cloud rates. This scale illustrates the core cloud ML trade-off: only centralized infrastructure can aggregate enough $R_{\text{peak}}$ for PFLOP-scale training, but the resulting $L_{\text{lat}}$ penalty (100--500 ms network round trip) makes that same infrastructure unsuitable for real-time inference. \index{LLM!training scale}
...
%
\node[Box4,below=0.5 of B1](B11){Immense Computational Power};
...
%
\node[Box2,below=0.5 of B2](B21){Scalable Data Processing and Model Training};
...
%
\node[Box,below=0.5 of B3](B31){Vendor Lock-In};
...
%
\node[Box3,below=0.5 of B4](B41){Virtual Assistants};
...
class DistancePenalty:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sol_kms_str = fmt(SPEED_OF_LIGHT_FIBER_KM_S.m_as("km/s"), precision=0, suffix=" km/s")
    rtt_formatted_str = fmt(round_trip_ms_value, precision=0, commas=False, suffix=" ms")
    deficit_str = fmt(deficit_ms_value, precision=0, commas=False, suffix=" ms")
    distance_km_str = fmt(distance_km_value, precision=0, suffix=" km")
    safety_budget_str = fmt(safety_budget_ms_value, precision=0, commas=False, suffix=" ms")
```

::: {#nbk-ml-systems-distance-penalty .callout-notebook title="The distance penalty"}

**Problem**\index{Distance Penalty!cloud latency}\index{Light Barrier!safety-critical systems}: Consider a real-time safety monitor for a robotic arm. The safety logic requires a `{python} DistancePenalty.safety_budget_str` end-to-end response time to prevent injury. The model runs in a high-performance cloud data center `{python} DistancePenalty.distance_km_str` away. Can the safety budget be met?

**Physics**:

1.  **Light in Fiber**: ~`{python} DistancePenalty.sol_kms_str`.
2.  **Round-trip propagation**: (`{python} DistancePenalty.distance_km_str` $\times$ 2)/`{python} DistancePenalty.sol_kms_str` = `{python} DistancePenalty.rtt_formatted_str`.
3.  **Result**: Round-trip propagation alone requires `{python} DistancePenalty.rtt_formatted_str`, exceeding the `{python} DistancePenalty.safety_budget_str` end-to-end budget (`{python} DistancePenalty.deficit_str` headroom) before the model performs any inference.
...
class CloudEdgeTCO:
    ...
    gpu_instances = 4
    egress_per_gb = Infrastructure.Pricing.Cloud.EgressPerGB.rate.m_as("dollar / GB")
    ...
    electricity_per_kwh = Infrastructure.Pricing.Cloud.ElectricityPerKwh.rate.m_as("dollar / kilowatt_hour")
    ...
    egress_gb_per_day = (requests_per_day * response_kb * KB).m_as(GB)
    ...
    # Power = (W * hours_per_year) / 1000 * electricity_per_kwh
    e_power = (power_watts * HOURS_PER_YEAR / 1000.0) * electricity_per_kwh
    ...
    requests_str = fmt(requests_per_day/MILLION, precision=0, commas=False, suffix="M")
    inference_str = fmt(inference_ms, precision=0, commas=False, suffix=" ms")
    response_str = fmt(response_kb, precision=0, commas=False, suffix=" KB")
    gpu_instances_str = fmt(gpu_instances, precision=0, commas=False)
    gpu_price_str = fmt_usd(gpu_price_per_hr, precision=2, commas=False, suffix="/hr")
    egress_gb_str = fmt(egress_gb_per_day, precision=0, commas=False, suffix=" GB/day")
    egress_price_str = fmt_usd(egress_per_gb, precision=2, commas=False, suffix="/GB")
    hours_per_year_str = fmt(HOURS_PER_YEAR, precision=0)
    days_per_year_str = fmt(DAYS_PER_YEAR, precision=0, commas=False)

    c_gpu_str = fmt_usd(c_gpu, precision=0, approx=True)
    c_egress_str = fmt_usd(c_egress, precision=0, approx=True)
    c_lb_str = fmt_usd(c_lb, precision=0, approx=True)
    c_logs_str = fmt_usd(c_logs, precision=0, approx=True)
    c_total_str = fmt_usd(c_total, precision=0, approx=True, suffix="/year")

    e_capex_str = fmt_usd(e_capex, precision=0, approx=True)
    e_power_str = fmt_usd(e_power, precision=1, approx=True)
    e_cool_str = fmt_usd(e_cool, precision=1, approx=True)
    e_net_str = fmt_usd(e_net, precision=0, approx=True)
    e_labor_str = fmt_usd(e_labor, precision=0, approx=True)
    e_total_str = fmt_usd(e_total, approx=True, suffix="/year", commas=True)

    edge_savings_str = fmt(edge_savings_pct, precision=1, commas=False, suffix="%")
    labor_pct_str = fmt(labor_pct, precision=1, commas=False, suffix="%")
    breakeven_requests_str = fmt_int(breakeven_requests_per_day/ THOUSAND, commas=False, suffix="K")
    breakeven_utilization_str = fmt(breakeven_fraction * 100, precision=2, commas=False, suffix="%")

    # Additional Outputs for Prose
    server_cost_str = fmt_usd(server_cost, precision=0)
    server_life_years_str = fmt(server_life_years, precision=0, commas=False, suffix=" years")
    power_str = fmt(power_watts, precision=0, commas=False, suffix=" W")
    electricity_str = fmt_usd(electricity_per_kwh, precision=2, commas=False, suffix="/kWh")
    devops_fte_str = fmt(devops_fte, precision=1, commas=False)
    devops_salary_str = fmt_usd(devops_salary, precision=0)
```
...
|:-------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------:|
| **GPU inference (A10G)** | `{python} CloudEdgeTCO.gpu_instances_str` instances $\times$ `{python} CloudEdgeTCO.hours_per_year_str` $\times$ `{python} CloudEdgeTCO.gpu_price_str` |       `{python} CloudEdgeTCO.c_gpu_str` |
| **Network egress**       | `{python} CloudEdgeTCO.egress_gb_str` $\times$ `{python} CloudEdgeTCO.days_per_year_str` $\times$ `{python} CloudEdgeTCO.egress_price_str`             |    `{python} CloudEdgeTCO.c_egress_str` |
| **Load balancer**        | \$0.025/hr + LCU charges                                                                                                                               |        `{python} CloudEdgeTCO.c_lb_str` |
...
| **Power (24/7)**     | `{python} CloudEdgeTCO.power_str` $\times$ `{python} CloudEdgeTCO.hours_per_year_str` $\times$ `{python} CloudEdgeTCO.electricity_str` |     `{python} CloudEdgeTCO.e_power_str` |
| **Cooling overhead** | ~30 percent of power                                                                                                                   |      `{python} CloudEdgeTCO.e_cool_str` |
...
class VoiceAssistantWall:
    ...
    vi_audio_kb_per_sec = (vi_audio_bytes_per_sec * byte).m_as(KB)
    ...
    ww_cloud_cost_str = fmt_usd(ww_cloud_cost_per_device, precision=2, commas=False, suffix="/device/year")
    ww_total_cost_str = fmt_usd(ww_total_cloud_cost, precision=0, commas=True, suffix="/year")
    ...
    ww_edge_cost_str = fmt_usd(ww_edge_cost_per_year, precision=2, commas=False, suffix="/device/year")
    ...
    vi_gpu_ms_str = fmt(vi_gpu_ms_per_query, precision=0, commas=False, suffix=" ms")
    vi_gpu_hours_str = fmt_int(vi_gpu_hours_day, commas=True)
    vi_gpu_hours_dc_str = fmt_int(vi_gpu_hours_dc_value, commas=True)
    vi_gpus_dc_str = fmt(vi_gpus_per_datacenter, precision=0, commas=True, suffix=" GPUs")
...
1.  **Query volume**: `{python} VoiceAssistantWall.vi_devices_str` billion devices $\times$ `{python} VoiceAssistantWall.vi_queries_str` queries/day = `{python} VoiceAssistantWall.vi_total_queries_str` billion queries/day.
...
**The bandwidth wall**: Wake-word detection requires continuous audio monitoring. If devices streamed audio to the cloud (16 kHz, 16-bit), each transmits ~`{python} VoiceAssistantWall.vi_audio_kb_str`. Across `{python} VoiceAssistantWall.vi_devices_str` billion devices: `{python} VoiceAssistantWall.vi_audio_tb_str`, a significant fraction of total global internet backbone capacity.
...
::: {.column-margin}
![](images/svg/vol1_ml_systems_margin_002.svg){width="100%" fig-alt="Vertical ladder of two violet bars: a tall bar for the raw bandwidth demand of 100 cameras towering over a short bar for the 10G network link capacity."}
...
# │          BandwidthBottleneck.metadata_reduction_x_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Systems
from mlsysim.physics import calc_monthly_egress_cost
from mlsysim.fmt import fmt_int, fmt_usd, fmt, check
...
class BandwidthBottleneck:
    ...
    metadata_reduction = (total_bytes_per_sec / metadata_bytes_per_sec).m_as("")
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cam_rate_mbs_str = fmt(bytes_per_sec_single.m_as(MB/second), precision=1, commas=False, suffix=" MB/s")
    total_rate_gbs_str = fmt(total_bytes_per_sec.m_as(GB/second), precision=1, commas=False, suffix=" GB/s")
    monthly_cost_m_str = fmt_usd(monthly_cost/MILLION, precision=1, commas=False, suffix="M/month")
    net_cap_gbs_str = fmt(network.bandwidth.m_as(GB/second), precision=2, commas=False, suffix=" GB/s")
    bw_short_x_str = fmt(shortfall_ratio, precision=1, commas=False)
    metadata_events_per_sec_str = fmt(metadata_events_per_sec, precision=0, commas=False)
    metadata_kb_per_event_str = fmt(metadata_kb_per_event.m_as(KB), precision=0, commas=False, suffix=" KB")
    metadata_reduction_x_str = fmt(metadata_reduction, precision=0, commas=True)

    num_cameras_str = fmt(num_cameras, precision=0, commas=False)
    bb_fps_str = fmt_int(fps.m_as('Hz'), commas=False)
    egress_cost_str = fmt_usd(Infrastructure.Pricing.Cloud.EgressPerGB.rate.m_as("dollar / GB"), precision=2, commas=False, suffix="/GB")
...
1.  **Raw data rate per camera**: `{python} BandwidthBottleneck.video_width_str` $\times$ `{python} BandwidthBottleneck.video_height_str` $\times$ `{python} BandwidthBottleneck.bytes_per_pixel_str` bytes $\times$ `{python} BandwidthBottleneck.bb_fps_str` FPS ≈ `{python} BandwidthBottleneck.cam_rate_mbs_str`.
2.  **Total data rate**: `{python} BandwidthBottleneck.num_cameras_str` cameras $\times$ `{python} BandwidthBottleneck.cam_rate_mbs_str` = `{python} BandwidthBottleneck.total_rate_gbs_str`.
...
4.  **Network reality**: Even a dedicated 10 Gbps line (`{python} BandwidthBottleneck.net_cap_gbs_str`) cannot carry the load---the workload demands `{python} BandwidthBottleneck.bw_short_x_str`$\times$ more bandwidth than exists.

**Systems insight**: Physics has made cloud streaming *impossible* for this application. Edge processing is not optional—it is mandatory. If an edge server transmits only defect metadata (`{python} BandwidthBottleneck.metadata_kb_per_event_str` per detection at roughly `{python} BandwidthBottleneck.metadata_events_per_sec_str` detections/sec across the floor), bandwidth falls by about `{python} BandwidthBottleneck.metadata_reduction_x_str`$\times$.
...
# │ Exports: EdgeEnergyTransmissionRecap.ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class EdgeEnergyTransmissionRecap:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(ratio.m_as("") >= 500,
          f"Transmission should dominate local compute; got {ratio.m_as(''):.1f}x.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ratio_str = fmt(ratio.m_as(""), precision=0, commas=True)
```

The preceding bandwidth calculation reveals why edge processing is mandatory for high-volume sensor deployments. For battery-powered edge devices (wireless cameras, drones, wearables), the constraint is even more severe: as "The Energy of Transmission" (@sec-ml-systems-bottleneck-principle-3514) established, radio transmission costs `{python} EdgeEnergyTransmissionRecap.ratio_str`$\times$ more energy than local inference, making cloud offloading physically impossible for battery-powered devices regardless of available bandwidth. @Fig-energy-per-inference quantifies this asymmetry across deployment tiers.
...
# │          DataLocalityInvariant.net_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, MarkdownStr
...
class DataLocalityInvariant:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    frame_mb = (width * height * bpp * byte).m_as(MB)
    tx_time_ms = (frame_mb * 8 / net_bw_mbps) * THOUSAND
    remote_total_ms = cloud_lat_ms + remote_compute_ms

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(tx_time_ms > remote_total_ms, "4K video should require locality at 100 Mbps.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frame_mb_str = fmt(frame_mb, precision=1, suffix=" MB")
    tx_time_ms_str = fmt(tx_time_ms, precision=1, suffix=" ms")
    remote_ms_str = fmt(remote_total_ms, precision=0, suffix=" ms")
    net_bw_str = fmt(net_bw_mbps, precision=0, commas=False, suffix=" Mb/s")
...
- **Bandwidth $(\text{BW}_{\text{network}})$**: `{python} DataLocalityInvariant.net_bw_str` home broadband (up).
- **Remote response $(L_{\text{lat,network}} + T_{\text{remote}})$**: `{python} DataLocalityInvariant.remote_ms_str` (round-trip + remote compute).

**Math**:

1.  **Transmission time**: `{python} DataLocalityInvariant.frame_mb_str` $\times$ 8 bits/`{python} DataLocalityInvariant.net_bw_str` = `{python} DataLocalityInvariant.tx_time_ms_str`.
...
# │          .inf_per_sec_str, .yolo_gflops_str, .sustained_gf_str,
# │          .req_tops_equiv_str, .headroom_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.fmt import fmt
...
class EdgeSizingThroughput:
    ...
    required_tops_equiv = (sustained_gflops * headroom * GFLOPs / second).m_as(TOPS)

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    stores_str = fmt(stores, precision=0, commas=False)
    cameras_per_store_str = fmt(cameras_per_store, precision=0, commas=False)
    fps_str = fmt(fps, precision=0, commas=False)
    headroom_str = fmt(headroom, precision=0, commas=False)
    inf_per_sec_str = fmt(inf_per_sec, precision=0, commas=False)
    yolo_gflops_str = fmt(yolo_flops.m_as(GFLOPs), precision=1)
    sustained_gf_str = fmt(sustained_gflops, precision=0)
    req_tops_equiv_str = fmt(required_tops_equiv, precision=2, commas=False)

```

::: {#nbk-ml-systems-edge-inference-sizing .callout-notebook title="Edge inference sizing"}

**Problem**: A smart retail chain deploys person detection across `{python} EdgeSizingThroughput.stores_str` stores, each with `{python} EdgeSizingThroughput.cameras_per_store_str` cameras at `{python} EdgeSizingThroughput.fps_str` FPS. Which edge accelerator (USB-scale TPU, workstation-class embedded GPU, or general-purpose mini-PC) delivers the required throughput at the lowest three-year fleet cost?
...
|:-------------------------|:-------------------------------------------------------------------------------------------------------------------------:|:-------------------------------------------------------------------:|
| **Inferences per store** |     `{python} EdgeSizingThroughput.cameras_per_store_str` cameras $\times$ `{python} EdgeSizingThroughput.fps_str` FPS    |    `{python} EdgeSizingThroughput.inf_per_sec_str` inferences/sec   |
| **Model compute**        |                       YOLOv8-nano: `{python} EdgeSizingThroughput.yolo_gflops_str` GFLOPs/inference                       |       `{python} EdgeSizingThroughput.sustained_gf_str` GFLOP/s      |
| **Required throughput**  | `{python} EdgeSizingThroughput.sustained_gf_str` GFLOP/s $\times$ `{python} EdgeSizingThroughput.headroom_str` (headroom) | ~`{python} EdgeSizingThroughput.req_tops_equiv_str` TOPS equivalent |
...
class EdgeSizingFleetTCO:
    ...
    sustained_gflops = (inf_per_sec * yolo_flops).m_as(GFLOPs)
    required_tops_equiv = (sustained_gflops * headroom * GFLOPs / second).m_as(TOPS)
    coral_peak_tops = coral.compute.peak_flops.m_as(TOPS)
    coral_sustained_tops = coral_peak_tops * derate
    coral_units_per_store = ceil(required_tops_equiv / coral_sustained_tops)
    coral_cluster_peak_tops = coral_units_per_store * coral_peak_tops
    coral_cluster_sustained_tops = coral_units_per_store * coral_sustained_tops
    coral_cluster_quantity = stores * coral_units_per_store
    coral_cluster_capex = coral_cost * coral_cluster_quantity
    coral_cluster_power_opex = (coral.tdp.m_as(kilowatt) * coral_units_per_store * stores * HOURS_PER_YEAR * years * Infrastructure.Pricing.Cloud.ElectricityPerKwh.rate.m_as("dollar / kilowatt_hour"))
    ...
    check(coral_cluster_sustained_tops >= required_tops_equiv, "Clustered Coral configuration should clear the requirement.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    req_tops_equiv_str = fmt(required_tops_equiv, precision=2, commas=False)
    derate_pct_str = fmt(derate * 100, precision=0, commas=False, suffix="%")
    coral_tops_str = fmt(coral_peak_tops, precision=0, commas=False, suffix=" TOPS")
    coral_sustained_tops_str = fmt(coral_sustained_tops, precision=0, commas=False, suffix=" TOPS")
    coral_units_per_store_str = fmt(coral_units_per_store, precision=0, commas=False)
    coral_cluster_peak_tops_str = fmt(coral_cluster_peak_tops, precision=0, commas=False, suffix=" TOPS peak")
    coral_cluster_sustained_tops_str = fmt(coral_cluster_sustained_tops, precision=0, commas=False, suffix=" TOPS derated")
    coral_cluster_unit_cost_str = fmt_usd(coral_cost * coral_units_per_store, precision=0, commas=False)
    coral_cluster_power_w_str = fmt(coral.tdp.m_as(watt) * coral_units_per_store, precision=0, commas=False, suffix=" W")
    coral_cluster_power_kw_str = fmt(coral.tdp.m_as(kilowatt) * coral_units_per_store, precision=3, commas=False, suffix=" kW")
    jetson_cost_str = fmt_usd(jetson_cost, precision=0, commas=False)
    jetson_power_range_str = MarkdownStr("10-40 W")
    jetson_tops_str = fmt(jetson.compute.precision_flops["int8"].m_as(TOPS), precision=0, commas=False, suffix=" TOPS")
    jetson_fleet_str = fmt_usd(jetson_cost * stores, precision=0, commas=True)
    nuc_cost_str = fmt_usd(nuc_cost, precision=0, commas=False)
    nuc_power_w_str = fmt(nuc.tdp.m_as(watt), precision=0, commas=False, suffix=" W")
    nuc_tops_str = fmt(nuc.compute.peak_flops.m_as(TOPS), precision=0, commas=False, suffix=" TOPS")
    nuc_fleet_str = fmt_usd(nuc_cost * stores, precision=0, commas=True)
    coral_cluster_fleet_str = fmt_usd(coral_cluster_capex, precision=0, commas=True)
    jetson_vs_coral_cluster_capex_ratio_str = fmt(jetson_cost / (coral_cost * coral_units_per_store), precision=1, commas=False)
    years_count_str = fmt(years, precision=0, commas=False)
    years_span_str = fmt(years, precision=0, commas=False, suffix=" years")
    stores_str = fmt(stores, precision=0, commas=False)
    hours_per_year_str = fmt(HOURS_PER_YEAR, precision=0, commas=False, suffix=" h")
    elec_cost_str = fmt_usd(Infrastructure.Pricing.Cloud.ElectricityPerKwh.rate.m_as("dollar / kilowatt_hour"), precision=2, commas=False, suffix="/kWh")
...
|:-----------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------:|:-------------------------------------------------------:|:---------------------------------------------------------:|:-----------------------------------------------------:|
| **NVIDIA Jetson Orin NX**                                                                |                                        `{python} EdgeSizingFleetTCO.jetson_tops_str`                                        |   `{python} EdgeSizingFleetTCO.jetson_power_range_str`  |       `{python} EdgeSizingFleetTCO.jetson_cost_str`       |     `{python} EdgeSizingFleetTCO.jetson_fleet_str`    |
| **Intel NUC + Movidius**                                                                 |                                          `{python} EdgeSizingFleetTCO.nuc_tops_str`                                         |      `{python} EdgeSizingFleetTCO.nuc_power_w_str`      |         `{python} EdgeSizingFleetTCO.nuc_cost_str`        |      `{python} EdgeSizingFleetTCO.nuc_fleet_str`      |
| **Google Coral Dev (`{python} EdgeSizingFleetTCO.coral_units_per_store_str` per store)** | `{python} EdgeSizingFleetTCO.coral_cluster_peak_tops_str`<br>`{python} EdgeSizingFleetTCO.coral_cluster_sustained_tops_str` | `{python} EdgeSizingFleetTCO.coral_cluster_power_w_str` | `{python} EdgeSizingFleetTCO.coral_cluster_unit_cost_str` | `{python} EdgeSizingFleetTCO.coral_cluster_fleet_str` |
...
# │ Exports: MobileEfficiencyRecap.mobilenet_flops_reduction_str,
...
class MobileEfficiencyRecap:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    mobilenet_flops_reduction = (Models.Vision.ResNet50.inference_flops / m_mobilenet.inference_flops).m_as("")

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(mobilenet_flops_reduction > 10, "MobileNet reduction should be >10x vs ResNet.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mobilenet_flops_reduction_str = fmt(mobilenet_flops_reduction, precision=1, commas=False, suffix="×")
    mobile_tdp_range_str = MarkdownStr(mobile_tdp_range.replace("-", " to ") + " W")
```

\index{Depthwise Separable Convolution!power reduction}
The mobile environment introduces a critical constraint absent from stationary deployments: energy per inference becomes a first-order design parameter. Under the iron law in @eq-iron-law-extended, cloud and edge systems optimize for minimizing $T$, total latency. Mobile systems face an additional constraint: $\text{Energy} = \text{Power} \times T$, and the power wall described by @eq-power-scaling caps sustained power at `{python} MobileEfficiencyRecap.mobile_tdp_range_str`. In archetype terms, a compute beast workload like image classification must be transformed through architectural efficiency (for example, depthwise separable convolutions[^fn-depthwise-mobile-efficiency] in MobileNet) into an efficient variant, reducing FLOPs by `{python} MobileEfficiencyRecap.mobilenet_flops_reduction_str`, preserving accuracy. This is not merely optimization; it represents a qualitative shift in the arithmetic intensity trade-off, accepting lower peak throughput in exchange for sustainable operation within a `{python} MobileEfficiencyRecap.mobile_tdp_range_str` thermal envelope.
...
[^fn-depthwise-mobile-efficiency]: **Depthwise Separable Convolutions**: An architectural factorization that splits a standard convolution into a depthwise spatial filter and a pointwise channel mixer, reducing FLOPs by 8--9$\times$ for typical layer configurations. This reduction is not merely an efficiency improvement but a prerequisite for real-time vision on mobile devices, where the power wall caps sustained computation at `{python} MobileEfficiencyRecap.mobile_tdp_range_str`. \index{Depthwise Separable Convolution!mobile power constraint}
...
***Mobile Machine Learning***\index{Mobile Machine Learning|see{Mobile ML}}\index{Mobile ML!definition} is the deployment paradigm bounded by Thermal Design Power (TDP)\index{Thermal Design Power!mobile constraint} and battery energy.
...
%
\node[Box4,below=0.17 of B1](B11){On-Device Processing};
...
%
\node[Box2,below=0.17 of B2](B21){Real-Time Processing};
...
%
\node[Box,below=0.17 of B3](B31){Limited Computational Resources};
...
%
\node[Box3,below=0.17 of B4](B41){Voice Recognition};
...
# │
# │ Imports: Scenarios.MobilePower.ObjectDetector (+ MobileNpuTypical), mlsysim.core.constants
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware
from mlsysim.fmt import fmt_frac, fmt_percent, fmt, check
...
class BatteryTax:
    ...
    power_draw = Scenarios.MobilePower.ObjectDetector

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    battery_wh = phone.battery_capacity.to(ureg.Wh)
    runtime_hours = (battery_wh/power_draw).to(ureg.hour)
    full_day_energy = power_draw * HOURS_PER_DAY * ureg.hour
    daily_budget_pct = (full_day_energy / battery_wh) * 100

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(runtime_hours.m_as(ureg.hour) < 24, "Narrative Violation: Battery now lasts more than a day with continuous ML!")
    check(runtime_hours.m_as(ureg.hour) < 8, f"Narrative Violation: Battery lasts too long ({runtime_hours:.1f} h) for the 'few hours' narrative.")
    check(daily_budget_pct.m_as("") > 100, "Always-on detector should exceed a full-day phone battery budget.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    runtime_str = fmt(runtime_hours.m_as(ureg.hour), precision=1, commas=False, suffix=" hours")
    pwr_w_str = fmt(power_draw.m_as(ureg.watt), precision=0, commas=False, suffix=" W")
    batt_wh_str = fmt(battery_wh.m_as(ureg.Wh), precision=0, commas=False, suffix=" Wh")
    budget_pct_str = fmt(daily_budget_pct.m_as(""), precision=0, commas=False, suffix="%")
    phone_day_hours_str = fmt(HOURS_PER_DAY, precision=0, commas=False, suffix=" hours")

    runtime_hours_display = fmt(runtime_hours.m_as(ureg.hour), precision=1, commas=False)
...
2.  **The reality**: A user expects their phone to last `{python} BatteryTax.phone_day_hours_str`. Running this single feature continuously for a day would require `{python} BatteryTax.budget_pct_str` of the phone's daily energy budget.

**Systems insight**: The model cannot simply be "deployed." The techniques in @sec-model-compression (quantization, duty-cycling) must reduce power to <100 mW for the feature to stay on all day.
...
class ThermalQuantCalc:
    ...
    mobile_tdp_w_value = Scenarios.MobilePower.MobileNpuTypical.m_as(watt)
    temp_rise_c_per_s_value = 1
    trip_time_s_value = 60
    trip_temp_c_value = 80
    fps_before_value = 100
    fps_after_value = 30

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    quant_power_w_value = baseline_power_w_value/quant_reduction_value             # 12 W / 4 = 3 W

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(quant_power_w_value - mobile_tdp_w_value) < 0.1,
          f"Quantized power ({quant_power_w_value} W) should land near the mobile TDP ({mobile_tdp_w_value} W).")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    baseline_str = fmt(baseline_power_w_value, precision=0, commas=False, suffix=" W")
    mobile_tdp_str = fmt(mobile_tdp_w_value, precision=0, commas=False, suffix=" W")
    quant_power_str = fmt(quant_power_w_value, precision=0, commas=False, suffix=" W")
    quant_red_str = fmt(quant_reduction_value, precision=0, commas=False, suffix="×")
    temp_rise_str = fmt(temp_rise_c_per_s_value, precision=0, commas=False, suffix=" °C/s")
    trip_time_str = fmt(trip_time_s_value, precision=0, commas=False, suffix=" s")
    trip_temp_str = fmt(trip_temp_c_value, precision=0, commas=False, suffix=" °C")
    fps_before_str = fmt(fps_before_value, precision=0, commas=False, suffix=" FPS")
    fps_after_str = fmt(fps_after_value, precision=0, commas=False, suffix=" FPS")
```

::: {#nbk-ml-systems-thermal-wall .callout-notebook title="The thermal wall"}

**Problem**\index{Thermal Wall!mobile constraints}\index{Power Wall!mobile implications}: An unoptimized LLM requires `{python} ThermalQuantCalc.baseline_str` peak compute. Can it be deployed on a mobile device?

**Physics**:

1. **Thermal design power (TDP)**: A mobile system on chip (SoC) allows approximately `{python} ThermalQuantCalc.mobile_tdp_str` for passive cooling.
2. **Temperature rise**: At `{python} ThermalQuantCalc.baseline_str`, the device temperature rises at approximately `{python} ThermalQuantCalc.temp_rise_str`.
3. **Thermal trip**: Within `{python} ThermalQuantCalc.trip_time_str`, the hardware reaches the **Thermal Trip Point**\index{Thermal Trip Point!hardware safety} (`{python} ThermalQuantCalc.trip_temp_str`), triggering OS throttling.
...
**Systems insight**: Quantization from FP32 to INT8 reduces power by approximately `{python} ThermalQuantCalc.quant_red_str`, but with a baseline of `{python} ThermalQuantCalc.baseline_str` the result is still `{python} ThermalQuantCalc.quant_power_str`---the absolute limit of the hardware. Physics sets a hard ceiling that no optimization can exceed.
...
# │          MobileBatteryCapacity.added_ml_runtime_low_str,
# │          MobileBatteryCapacity.added_ml_runtime_high_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware
from mlsysim.fmt import fmt, check
...
class MobileBatteryCapacity:
    ...
    phone_battery_high_wh = 22
    baseline_runtime_h = 24
    added_ml_power_w = 1

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    baseline_power_w = phone_battery_wh / baseline_runtime_h
    baseline_power_high_w = phone_battery_high_wh / baseline_runtime_h
    added_ml_runtime_low_h = phone_battery_wh / (baseline_power_w + added_ml_power_w)
    added_ml_runtime_high_h = phone_battery_high_wh / (baseline_power_high_w + added_ml_power_w)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    phone_battery_str = fmt(phone_battery_wh, precision=0, suffix=" Wh")
    added_ml_runtime_low_str = fmt(added_ml_runtime_low_h, precision=1, commas=False, suffix=" hours")
    added_ml_runtime_high_str = fmt(added_ml_runtime_high_h, precision=1, commas=False, suffix=" hours")
```
...
class MobileResourceRecap:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    mobile_npu_tops_values = [
        phone.compute.peak_flops.m_as(TOPS)
        for phone in representative_phones
    ]
    mobile_bw_gbs_values = [
        phone.memory.bandwidth.m_as(GB/second)
        for phone in representative_phones
    ]
    mobile_npu_min = min(mobile_npu_tops_values)
    mobile_npu_max = max(mobile_npu_tops_values)
    mobile_bw_min = min(mobile_bw_gbs_values)
    mobile_bw_max = max(mobile_bw_gbs_values)
...
# │          MobileResnetPitfallRecap.resnet_gflops_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class MobileResnetPitfallRecap:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    resnet_fp32_bytes_value = resnet_params.m_as('param') * 4 * byte
    resnet_gflops_value = resnet_flops.m_as(GFLOPs)
    resnet_fp32_mb_value = resnet_fp32_bytes_value.m_as(MB)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(resnet_fp32_mb_value >= 90, f"ResNet-50 FP32 size should be near 100 MB; got {resnet_fp32_mb_value:.0f} MB.")
    check(resnet_gflops_value >= 4, f"ResNet-50 compute should be about 4 GFLOPs; got {resnet_gflops_value:.1f}.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    resnet_fp32_mb_str = fmt(resnet_fp32_mb_value, precision=1, commas=False, suffix=" MB")
    resnet_gflops_str = fmt(resnet_gflops_value, precision=1, commas=False)
```

These successes can create a misleading sense of ease. A common pitfall involves attempting to deploy desktop-trained models directly to mobile or edge devices without architecture modifications. Models developed on powerful workstations often fail when deployed to resource-constrained devices. A desktop ResNet-50 pipeline may require gigabytes once activations, batches, preprocessing buffers, and runtime overhead are included, even though the FP32 weights alone are about `{python} MobileResnetPitfallRecap.resnet_fp32_mb_str`. Such a pipeline, with `{python} MobileResnetPitfallRecap.resnet_gflops_str` billion FLOPs per inference, cannot run unchanged on a device with 512 MB of RAM and a 1 GFLOP/s processor. Beyond simple resource violations, desktop-optimized models may use operations unsupported by mobile hardware (specialized mathematical operations), assume floating-point precision unavailable on embedded systems, or require batch processing incompatible with single-sample inference. Successful deployment demands architecture-aware design from the beginning, including specialized architectural techniques for mobile devices such as MobileNet's depthwise separable convolutions [@howard2017mobilenets] (detailed in @sec-network-architectures), integer-only operations for microcontrollers, and optimization strategies that maintain accuracy while reducing computation.
...
# │          (Scenarios.PhoneBattery.{EnergyJ,CapacityMah,VoltageV}, ureg, BILLION)
...
class EnergyInference:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    batt_energy_j = Scenarios.PhoneBattery.EnergyJ
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    e_gpt4_j = 1000 * ureg.joule  # ~1 kJ cloud LLM query (empirical anchor)
    e_resnet_cloud_j = p_resnet_cloud.energy
    e_resnet_edge_j = p_resnet_edge.energy
    e_mobilenet_j = p_mobilenet.energy
    e_kws_j = 10 * ureg.microjoule  # representative always-on KWS inference anchor
    ...
    e_resnet_cloud_mj = round(e_resnet_cloud_j.m_as("mJ"), 1)
    e_resnet_edge_mj = round(e_resnet_edge_j.m_as("mJ"), 1)
    e_mobilenet_mj = round(e_mobilenet_j.m_as("mJ"), 2)
    e_kws_uj = round(e_kws_j.m_as("uJ"), 0)
    e_resnet_cloud_str = fmt(e_resnet_cloud_mj, precision=1, suffix=" mJ")
    e_resnet_edge_str = fmt(e_resnet_edge_mj, precision=1, suffix=" mJ")
    e_mobilenet_str = fmt(e_mobilenet_mj, precision=2, suffix=" mJ")
    e_kws_str = fmt(e_kws_uj, precision=0, commas=False, suffix=" µJ")

    q_gpt4_str = fmt_int(q_gpt4.m_as(''), commas=True, suffix=" queries")
    q_resnet_cloud_str = fmt_int(batt_energy_j.m_as("J") / (e_resnet_cloud_mj * 1e-3), commas=True, suffix=" queries")
    q_resnet_edge_str = fmt_int(batt_energy_j.m_as("J") / (e_resnet_edge_mj * 1e-3), commas=True, suffix=" queries")
    q_mobilenet_str = fmt_int(batt_energy_j.m_as("J") / (e_mobilenet_mj * 1e-3), commas=True, suffix=" queries")
...
| **Cloud**    | ResNet-50 (A100)     | `{python} EnergyInference.e_resnet_cloud_str` |                                                            `{python} EnergyInference.q_resnet_cloud_str` |
| **Edge**     | ResNet-50 (Jetson)   |  `{python} EnergyInference.e_resnet_edge_str` |                                                             `{python} EnergyInference.q_resnet_edge_str` |
| **Mobile**   | MobileNet (NPU)      |   `{python} EnergyInference.e_mobilenet_str`  |                                                               `{python} EnergyInference.q_mobilenet_str` |
...
%
\node[Box4,below=0.37 of B1](B11){Low Power and Resource Constrained Environments};
\node[Box4,below=of B11](B12){On-Device Machine Learning};
\node[Box4,below=of B12](B13){Ultra-Small Form Factor};
%
\node[Box2,below=0.27 of B2](B21){Extremely Low Latency};
...
# │ Exports: WildlifeMonitoring.raw_audio_gb_str,
# │          WildlifeMonitoring.summary_kb_str,
# │          WildlifeMonitoring.reduction_x_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class WildlifeMonitoring:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    raw_audio_per_day = 4.3 * GB
    summary_per_day = 400 * KB

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    reduction = (raw_audio_per_day / summary_per_day).m_as("")

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(reduction > 10_000, f"Expected >10,000x bandwidth reduction, got {reduction:.0f}x.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    raw_audio_gb_str = fmt(raw_audio_per_day.m_as(GB), precision=1, commas=False, suffix=" GB per day")
    summary_kb_str = fmt(summary_per_day.m_as(KB), precision=0, commas=False, suffix=" KB of detection summaries")
    reduction_x_str = fmt(reduction, precision=0, commas=True)
```

\index{Wildlife Conservation!TinyML monitoring}
Wildlife conservation uses TinyML for remote environmental monitoring. Researchers deploy solar-powered audio sensors consuming 100–500 mW that process continuous audio streams for species identification. By performing local analysis, these systems reduce satellite transmission requirements from `{python} WildlifeMonitoring.raw_audio_gb_str` to `{python} WildlifeMonitoring.summary_kb_str`, a `{python} WildlifeMonitoring.reduction_x_str`$\times$ reduction that makes large-scale deployments of 100–1,000 sensors economically feasible.
...
class BrakingDistance:
    ...
    distance_m = speed_ms * (latency_ms * ms).m_as(second)
    # ┌── 4. OUTPUT ────────────────────────────────────────
    speed_str = fmt(speed_kmh, precision=0, commas=False)
    distance_str = fmt(distance_m, precision=1, commas=False)
    latency_str = fmt(latency_ms, precision=0, commas=False, suffix=" ms")
```
...
***Hybrid Machine Learning***\index{Hybrid Machine Learning|see{Hybrid ML}}\index{Hybrid ML!definition} is the deployment strategy that splits an ML pipeline across cloud and edge tiers, assigning latency-critical stages to local hardware and compute-intensive stages to remote data centers.
...
Voice assistants combine Train-Serve Split, Progressive Deployment, and Hierarchical Processing; autonomous vehicles combine Hierarchical Processing with Progressive Deployment to run optimized models at each tier. Additional patterns including federated and collaborative learning extend this menu to privacy-preserving distributed training across devices.
...
# │          MobilePowerFallacyCalc.phone_battery_str,
# │          MobilePowerFallacyCalc.mobile_tdp_str,
# │          MobilePowerFallacyCalc.quant_red_str,
...
from mlsysim import Hardware
from mlsysim.fmt import fmt_percent, fmt, check, fmt_frac
...
class MobilePowerFallacyCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    h_phone = Hardware.Mobile.iPhone15Pro
    battery_wh_value = h_phone.battery_capacity.m_as('Wh') if h_phone.battery_capacity else 15
    low_power_w_value = 1                                                           # W, light inference
    high_power_w_value = 5                                                          # W, heavy on-device model
    mobile_tdp_w_value = Scenarios.MobilePower.MobileNpuTypical.m_as(watt)
    quant_reduction_value = 4                                                       # FP32 to INT8 power reduction

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    low_power_hours_value = battery_wh_value/low_power_w_value
    high_power_hours_value = battery_wh_value/high_power_w_value

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(10 <= battery_wh_value <= 25,
          f"Representative phone battery should be in a realistic Wh range, got {battery_wh_value:.1f}.")
    check(high_power_hours_value < low_power_hours_value,
          "Higher-power mobile workload should drain the battery faster.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    phone_battery_str = fmt(battery_wh_value, precision=0, commas=False, suffix=" Wh")
    low_power_w_str = fmt(low_power_w_value, precision=0, commas=False, suffix=" W")
    high_power_w_str = fmt(high_power_w_value, precision=0, commas=False, suffix=" W")
    low_power_hours_str = fmt(low_power_hours_value, precision=0, commas=False)
    high_power_hours_str = fmt(high_power_hours_value, precision=0, commas=False)
    mobile_tdp_str = fmt(mobile_tdp_w_value, precision=0, commas=False, suffix=" W")
    quant_red_str = fmt(quant_reduction_value, precision=0, commas=False)

    # Inline fractions showing the physics.
    low_power_frac = fmt_frac(f"{phone_battery_str}", f"{low_power_w_str}", f"**{low_power_hours_str} hours**")
    high_power_frac = fmt_frac(f"{phone_battery_str}", f"{high_power_w_str}", f"**{high_power_hours_str} hours**")
```

**Fallacy**: *Model optimization overcomes mobile device power and thermal limits.*

Compression techniques do not scale indefinitely against physics. Consider a smartphone with a `{python} MobilePowerFallacyCalc.phone_battery_str` battery:
...
The `{python} MobilePowerFallacyCalc.high_power_w_str` workload also triggers thermal throttling\index{Thermal Throttling!performance limits} that substantially reduces performance. As @sec-ml-systems-mobile-ml-benefits-resource-constraints-c568 establishes, sustained mobile inference cannot exceed approximately `{python} MobilePowerFallacyCalc.mobile_tdp_str` without active cooling. Reducing numerical precision (using fewer bits to represent each weight; see @sec-model-compression) cuts power by approximately `{python} MobilePowerFallacyCalc.quant_red_str`$\times$, but aggressive precision reduction often causes accuracy loss. Applications requiring continuous inference beyond mobile thermal envelopes remain physically impossible regardless of algorithmic improvements.
...
class TcoPitfallCalc:
    ...
    cloud_compute_str = fmt_usd(cloud_compute_value, precision=0, commas=True, suffix="/month")
    edge_hw_str = fmt_usd(edge_hardware_value, precision=0, commas=False, suffix="/month")
    edge_network_str = fmt_usd(edge_network_value, precision=0, commas=True, suffix="/month")
    edge_maint_str = fmt_usd(edge_maintenance_value, precision=0, commas=False, suffix="/month")
    edge_reliability_str = fmt_usd(edge_reliability_value, precision=0, commas=True, suffix="/month")
    edge_total_str = fmt_usd(edge_total_value, precision=0, commas=True)
    tco_ratio_str = fmt(tco_ratio_value, precision=0, commas=False, suffix="×")
```

**Pitfall**: *Minimizing computational resources minimizes total cost.*

Teams optimize per-unit resource consumption while ignoring operational overhead and development velocity. As the decision framework in @sec-ml-systems-decision-framework-241f emphasizes, paradigm selection requires evaluating total cost of ownership, not just compute costs. A cloud inference service costing `{python} TcoPitfallCalc.cloud_compute_str` in compute appears expensive vs. `{python} TcoPitfallCalc.edge_hw_str` edge hardware amortization, but edge deployments add network engineering (`{python} TcoPitfallCalc.edge_network_str`), hardware maintenance (`{python} TcoPitfallCalc.edge_maint_str`), and reliability engineering (`{python} TcoPitfallCalc.edge_reliability_str`), totaling `{python} TcoPitfallCalc.edge_total_str`---a `{python} TcoPitfallCalc.tco_ratio_str` difference. Development velocity compounds the gap: cloud deployments reaching production in two months vs. six months for custom edge infrastructure represent four months of delayed revenue. The optimal cost solution requires total cost of ownership analysis including development time, operational complexity, and opportunity costs, not merely minimizing compute expenses.
...
# │          AmdahlCameraCalc.range_speedup_low_str,
# │          AmdahlCameraCalc.range_speedup_high_str
# └─────────────────────────────────────────────────────────────────────────────
...
class AmdahlCameraCalc:
    ...
    general_ml_frac_low_value = 0.30
    general_ml_frac_high_value = 0.50
    general_model_speedup_value = 100
    ...
    range_speedup_low_value = 1 / ((1 - general_ml_frac_low_value) + general_ml_frac_low_value/general_model_speedup_value)
    range_speedup_high_value = 1 / ((1 - general_ml_frac_high_value) + general_ml_frac_high_value/general_model_speedup_value)

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cam_isp_str = fmt(cam_isp_ms_value, precision=0, commas=False, suffix=" ms")
    cam_ml_str = fmt(cam_ml_ms_value, precision=0, commas=False, suffix=" ms")
    cam_post_str = fmt(cam_post_ms_value, precision=0, commas=False, suffix=" ms")
    cam_total_str = fmt(cam_total_ms_value, precision=0, commas=False, suffix=" ms")
    cam_ml_pct_str = MarkdownStr(f"{fmt_percent(cam_ml_frac_value, precision=0, commas=False)}%")
    cam_non_ml_pct_str = MarkdownStr(f"{fmt_percent(cam_non_ml_frac_value, precision=0, commas=False)}%")
    ...
    cam_ml_opt_str = fmt(cam_ml_optimized_ms_value, precision=0, commas=False, suffix=" ms")
    cam_total_opt_str = fmt(cam_total_optimized_ms_value, precision=0, commas=False, suffix=" ms")
    range_speedup_low_str = fmt(range_speedup_low_value, precision=1, commas=False)
    range_speedup_high_str = fmt_int(round(range_speedup_high_value), commas=False)
```
