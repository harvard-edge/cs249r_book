...
The same neural network that achieves state-of-the-art accuracy in a data center may be physically impossible to run on a smartphone, not because of engineering limitations we can overcome, but because of fundamental constraints we must navigate. Light takes 40 milliseconds to travel coast-to-coast, making cloud inference impossible for autonomous vehicle braking decisions. A smartphone's 15 Wh battery cannot sustain continuous 5W inference without dying in three hours. A microcontroller's 256 KB of memory cannot hold a 100 MB model regardless of how cleverly we compress it.

These are not temporary engineering challenges awaiting better solutions—they are permanent physical constraints that create the deployment spectrum from cloud to TinyML. This chapter examines how these constraints shape the four fundamental deployment paradigms, explains why hybrid architectures dominate production systems, and provides decision frameworks for selecting the right approach. By understanding what physics permits and forbids, engineers can make informed architectural decisions rather than discovering constraints through costly failures.
