...
The **power wall**\index{Power Wall!thermal limits}\index{Power Wall!frequency scaling} emerged because thermodynamics limits how much computation can occur in a given volume. Under classical Dennard scaling\index{Dennard Scaling!breakdown}[^fn-dennard-scaling-origin] (which held until approximately 2006), the relationship between power and frequency was cubic. Here $C$ is effective capacitance, $V$ is voltage, and $f$ is clock frequency. As voltage tracks frequency $(V \propto f)$, power rises as $f^3$, as @eq-power-scaling shows:
...
class CloudEdgeTCO:
    ...
    breakeven_utilization_str = fmt_percent(breakeven_fraction, precision=2, commas=False, style='prose')
...
Under this steady-capacity scenario, the edge total is almost entirely fixed cost while the cloud total scales with volume, so the division reduces to the ratio of the two annual totals: `{python} CloudEdgeTCO.e_total_usd_str` ÷ `{python} CloudEdgeTCO.c_total_usd_str` ≈ `{python} CloudEdgeTCO.breakeven_utilization_str` of the `{python} CloudEdgeTCO.requests_str`/day operating point, or roughly `{python} CloudEdgeTCO.breakeven_requests_str` inferences/day.

**Result**: Above that crossover, edge wins. At full volume the gap is `{python} CloudEdgeTCO.c_total_usd_str` − `{python} CloudEdgeTCO.e_total_usd_str` = `{python} CloudEdgeTCO.cost_gap_usd_str`, about `{python} CloudEdgeTCO.edge_savings_str` of the cloud bill; below the crossover, cloud elasticity usually wins.
...
**Problem**: `{python} VoiceAssistantWall.ww_devices_b_str` voice assistant devices (smartphones, smart speakers, earbuds) each issue `{python} VoiceAssistantWall.vi_queries_str` as wake-word traffic. What would the infrastructure scaling\index{Scaling!infrastructure, voice assistants} cost if all queries were served from cloud ML\index{Cloud ML!scaling limits}, and how many dedicated data centers would peak load require?

**Economic wall**: First, the cost of serving wake-word traffic from the cloud.

- **Cloud cost**: ~`{python} VoiceAssistantWall.ww_cloud_cost_str` → `{python} VoiceAssistantWall.ww_devices_b_str` devices = `{python} VoiceAssistantWall.ww_total_cost_str`. Economically prohibitive for a free feature.
...
\vspace*{-1ex}
```
**Bandwidth wall**: Third, the physics of moving the audio itself. If devices streamed audio to the cloud (16 kHz, 16-bit), each transmits ~`{python} VoiceAssistantWall.vi_audio_kb_per_s_str`. Across `{python} VoiceAssistantWall.vi_devices_str` billion devices: `{python} VoiceAssistantWall.vi_audio_tb_per_s_str`, a significant fraction of total global internet backbone capacity.
...
The decision between local edge processing and remote cloud processing is governed by a bandwidth-latency trade-off\index{Bandwidth!latency trade-off}: data must stay local when the time to transmit it exceeds the total time for remote processing (including network latency and remote compute).
...
**Math**: Quantization from FP32 to INT8 (reducing numerical precision to fewer bits per weight; see @sec-model-compression) cuts power by approximately `{python} ThermalQuantCalc.quant_reduction_factor_mult_str`: `{python} ThermalQuantCalc.baseline_w_str` ÷ `{python} ThermalQuantCalc.quant_reduction_factor_str` = `{python} ThermalQuantCalc.quant_power_w_str`, which lands right at the `{python} ThermalQuantCalc.mobile_tdp_w_str` passive-cooling ceiling with no headroom to spare.
...
class EnergyInference:
    ...
    kws_efficiency_mult_str = fmt_multiple(kws_efficiency_ratio, precision=0, commas=True)
...
The energy values in @tbl-ml-systems-energy-per-inference represent *full-system energy* (including server CPUs, memory, networking, and cooling overhead), not *isolated accelerator compute energy*. The A100 GPU alone executes ResNet-50 inference in under `{python} EnergyInference.isolated_a100_resnet_latency_bound_ms_str` (`{python} EnergyInference.isolated_a100_resnet_energy_j_str`), but the full server draws `{python} EnergyInference.full_server_draw_kw_str` when amortized across queuing, preprocessing, and idle power. The query counts in the final column make the same point in human terms: the same smartphone battery sustains a TinyML wake-word detector for `{python} EnergyInference.kws_efficiency_mult_str` more inferences than a cloud LLM query.

This TinyML energy-efficiency gap\index{TinyML!energy efficiency} explains why always-on sensing is only practical at the TinyML tier. A smartphone running continuous cloud queries would drain in minutes, whereas the same energy budget supports months of local sensing.
...
: **Energy per inference across paradigms**: Representative full-system energy per inference and resulting smartphone-battery query counts for cloud, edge, mobile, and TinyML workloads, illustrating the eight-order-of-magnitude span that makes always-on sensing feasible only at the TinyML tier. {#tbl-ml-systems-energy-per-inference tbl-colwidths="[18,29,25,28]"}
