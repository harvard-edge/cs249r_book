...
At the heart of this problem is a representation called the **computational graph**\index{Computational Graph!definition (DAG)}, a directed acyclic graph (DAG) where nodes represent operations and edges represent data dependencies. This graph is the framework's internal model of your computation. @fig-comp-graph illustrates a simple example: computing $z = x \times y$ requires two input nodes ($x$ and $y$), one operation node (multiplication), and one output node ($z$). The execution problem asks: *when* is this graph constructed, and *when* is it executed?
...
- **Operation fusion**\index{Kernel Fusion!static graph optimization}: Combine `y = x * 2` and `z = y + 1` into a single fused kernel `z = x * 2 + 1`, eliminating the intermediate `y` and reducing memory traffic by 50%.
...
Compilation frameworks like XLA\index{XLA (Accelerated Linear Algebra)!graph compilation}[^fn-xla] (Accelerated Linear Algebra) [@GoogleXLA] take this further, compiling the TensorFlow graph to optimized machine code for specific hardware. For a transformer encoder block, XLA can achieve 1.5--2$\times$ speedup over unoptimized execution through aggressive fusion and hardware-specific code generation.
...
@fig-3d-parallelism visualizes how large-scale training distributes computation. The grid layout illustrates three orthogonal scaling axes: **Data Parallelism**\index{Data Parallelism!model replication} (replicating the model across columns), **Pipeline Parallelism**\index{Pipeline Parallelism!layer splitting} (splitting layers across rows), and **Model Parallelism**\index{Model Parallelism!tensor sharding}\index{Tensor Parallelism|see{Model Parallelism}} (sharding tensors within each cluster). This "3D" approach allows frameworks to scale beyond the memory limits of any single device [@mcmahan2023communicationefficient].
