...
class FusionSpeedup:
    ...
    overhead_speedup_str = fmt_multiple(speedup, precision=0, commas=False)
    bw_efficiency_str = fmt_multiple(bw_efficiency, precision=0, commas=False)
...
*   **Total overhead**: `{python} FusionSpeedup.compiled_overhead_str` (`{python} FusionSpeedup.overhead_speedup_str`$\times$ speedup).
...
# │          torch_compile_speedup_range_str, tensorrt_speedup_range_str,
...
class CompilationBenchmark:
    ...
    torch_compile_speedup_range_str = fmt_multiple_range(
        min(torch_compile_speedups), max(torch_compile_speedups), precision=1, commas=False
    )
    tensorrt_speedup_range_str = fmt_multiple_range(
    ...
    resnet_tensorrt_speedup_str = fmt_multiple(resnet_tensorrt_speedup, precision=1, commas=False)
...
class DispatchTaxSmallLarge:
    ...
    dispatch_compilation_speedup_str = fmt_multiple(small_speedup_limit, precision=1, commas=False)
...
# │          resnet_training_ratio_range_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
...
class ResNetMemory:
    ...
    resnet_training_ratio_range_str = fmt_multiple_range(
...
# │ Exports: ResNetAdminTaxRatio.resnet_training_ratio_range_str
from mlsysim import Models
...
The memory breakdown for ResNet-50 in @sec-ml-frameworks-principle-2-memorycompute-tradeoff-19f2 showed a concrete ~`{python} ResNetAdminTaxRatio.resnet_training_ratio_range_str`$\times$ ratio between training and inference memory. Here we generalize that analysis to reveal the full administrative overhead at billion-parameter scale.
...
class DeviceBandwidthHierarchy:
    ...
    pcie_to_hbm_gap_str = fmt_multiple(pcie_to_hbm_gap, precision=1, commas=False)
```
...
class CompilationOverheadCalc:
    ...
    compiled_slowdown_str = fmt_multiple(compiled_slowdown, precision=1, commas=False)  # e.g. "44×"
```
