...
- **Python interpreter overhead** ($L_{lat}$): Every operation goes through Python dispatch, including function lookup, argument parsing, and type checking. At ~1μs per operation, this becomes significant for models with thousands of operations.
...
- **backend='tensorrt'**\index{TensorRT!torch.compile backend}: Compiles to NVIDIA TensorRT inference engine with aggressive optimizations (int8 quantization, layer fusion, kernel autotuning). Inference-only (no backward pass), NVIDIA GPUs only, often achieves 2--5$\times$ speedup over TorchInductor for inference.
