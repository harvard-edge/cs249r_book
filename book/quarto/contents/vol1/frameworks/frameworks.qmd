...
In the context of the **Iron Law** (@principle-iron-law), a framework is fundamentally a **Compiler for the Silicon Contract**.
...
***Machine Learning Frameworks*** are the **Compilers** for the Silicon Contract (@principle-silicon-contract). They translate high-level mathematical definitions into hardware-specific execution plans, managing the **Abstraction Gap** between algorithmic logic and physical silicon constraints (memory layout, kernel dispatch, differentiation).
...
The following visualization demonstrates this tax in action. Eager execution (top) suffers from "gaps" where the GPU sits idle while Python dispatches the next kernel. Compilation (bottom) fuses these operations into a single kernel launch, eliminating the gaps.

```{python}
#| label: fig-python-tax
#| echo: false
#| fig-cap: "**The Python Tax.** Execution timeline for a sequence of small operations (e.g., LayerNorm). In Eager Mode (top), the GPU (blue) finishes quickly but must wait for the Python interpreter (red) to dispatch the next instruction. In Compiled Mode (bottom), the compiler fuses operations into a single kernel, eliminating dispatch overhead and keeping the GPU fully saturated."
...
These micro-runtimes represent the "Pure AOT" endpoint of the continuum. By sacrificing all dynamic flexibility, they enable machine learning to run on devices consuming milliwatts of power, fulfilling the **Energy-Movement Invariant** (@principle-energy-movement) by keeping all data movement local to the chip.
...
Extending this pattern across multiple machines requires distributed training techniques covered in *Distributed Training Systems* (Volume II), but the single-node implementation above illustrates the core synchronization principles that underlie all pipeline-parallel systems.
