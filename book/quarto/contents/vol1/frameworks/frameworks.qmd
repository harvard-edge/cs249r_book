...
*   **Execution:**
    1.  Launch `Add` Kernel: `{python} kernel_launch_us` $\mu$s overhead.
    2.  Read/Write Memory: $2N$ bytes.
    3.  Launch `ReLU` Kernel: `{python} kernel_launch_us` $\mu$s overhead.
    4.  Read/Write Memory: $2N$ bytes.
*   **Total Overhead:** `{python} eager_overhead_str` $\mu$s.
...
*   **Execution:**
    1.  Launch `Fused` Kernel: `{python} compiled_overhead_str` $\mu$s overhead.
    2.  Read/Write Memory: $2N$ bytes (intermediate result stays in registers).
*   **Total Overhead:** `{python} compiled_overhead_str` $\mu$s (**`{python} overhead_speedup_str`x speedup**).
...
We introduce these concepts here because they shape framework API design even for single-node code. The implementation details of distributed training, including gradient compression, communication topologies, and fault tolerance, are covered in Volume II (*Distributed Training Systems*).
...
a100_peak_tflops = 312e12         # 312 TFLOPS (FP16 tensor)
a100_mem_bw_bytes = 2e12          # 2 TB/s
...
- $T_{\text{compute}} \approx 40\text{M} / 312\text{T} \approx `{python} mnist_t_compute_us`\mu\text{s}$
- $T_{\text{memory}} \approx 5\text{MB} / 2\text{TB/s} \approx `{python} mnist_t_memory_us`\mu\text{s}$
