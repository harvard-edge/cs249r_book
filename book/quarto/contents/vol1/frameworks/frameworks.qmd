...
This overlap principle extends naturally to model-stage overlap within a single node. Different model stages on separate GPUs can process different microbatches concurrently, with each stage's computation overlapping the next stage's data reception (see @lst-pipeline-parallelism-streams). The model training chapter later names and analyzes the distributed-training strategies built from this scheduling pattern.

Extending this pattern across multiple machines requires distributed training techniques that constitute an advanced topic, but the preceding single-node implementation illustrates the core synchronization principles that underlie all pipeline-parallel systems. With computation and communication overlapping effectively, the remaining challenge is ensuring correctness when operations complete out of order.
