...
Neural networks are defined by mathematics (matrix multiplications, gradient computations, activation functions), but mathematics does not execute itself. Between the equations and the silicon lies a translation layer that decides how operations are scheduled on hardware, how memory is allocated across the compute hierarchy, and how gradients flow backward through the computational graph. The framework is this translation layer, and the translation is not neutral.

An eager-mode framework executes each operation immediately; that choice prioritizes debugging flexibility but can sacrifice graph-level optimizations because the compiler no longer sees the whole computation that can halve inference latency. A framework lacking support for the target accelerator renders the hardware investment useless. A framework with a rich training API but no export path to edge devices means the model cannot reach the deployment target it was designed for.

Architecture choices are at least visible: engineers debate model size, layer count, and attention mechanisms explicitly. Framework choices are more insidious because they operate below the level of daily attention, silently determining which optimizations are possible, which hardware is reachable, and which deployment paths exist. In the AI Triad, the framework is the invisible mediator between Algorithm and Machine, and its design choices---baked into its compilation stack, memory management, and operator libraries, the hardware-tuned kernels that execute common operations---are difficult to reverse. Migrating between frameworks invalidates data pipelines, serving infrastructure, model checkpoints, and team expertise, typically requiring months of engineering effort for production systems. Framework selection is therefore an infrastructure commitment that determines what the system can do on the hardware it must run on.
...
- Analyze how the memory wall drives framework optimization strategies such as kernel fusion, lower-precision execution, and selective recomputation
...
Two lines of code: `model = transformer(...)` followed by `loss.backward()`, the framework call that starts reverse-mode automatic differentiation. Between them, invisible to the programmer, the framework orchestrates billions of floating-point operations across memory hierarchies, computes exact gradients through millions of parameters using **automatic differentiation**\index{Automatic Differentiation!framework role} (the systematic application of the chain rule to compute derivatives), schedules thousands of GPU kernel launches that dispatch compiled operations to the accelerator, and manages gigabytes of intermediate state. The simplicity is an illusion. Those two lines trigger machinery as complex as a compiler, because that is exactly what a modern ML framework is.
...
2.  **Solving Usability (2006)**: **NumPy**\index{NumPy!framework foundation}[^fn-numpy-abstraction] solved the problem of developer velocity. By wrapping low-level BLAS routines in high-level Python [@harris2020], it allowed scientists to write code in a friendly language while executing it in optimized C/Fortran. This "Vectorization" pattern, where the slow language handles logic and the fast language handles loops, became a durable contract for scientific computing. Jupyter notebooks later extended this usability layer into readable, executable computational workflows for combining code, results, and explanations [@kluyver_thomas2016].
3.  **Solving Differentiation (2015–present)**: **Deep Learning Frameworks**\index{Deep Learning Framework!automatic differentiation} (Theano[^fn-theano-origin], TensorFlow, PyTorch) solved the problem of gradient computation. While NumPy required manual derivation of backpropagation gradients (error-prone and slow), these frameworks introduced Automatic Differentiation via the computational graph. This turned the chain rule into a software primitive, allowing researchers to define *forward* passes and get *backward* passes for free.
...
\node[Box={col5!50},below=2 of B5](BB5){TensorFlow popularizes static graphs};
...
# │       a100_bw_tb_per_s_str also reused in Device Bandwidth Hierarchy (line ~2615).
...
# │
# │ Note: PERSISTENT—MemoryWallSpecs.a100_bw_tb_per_s_str reused at line ~2615
# │       (Device and Memory Management section prose).
# │
# │ Imports: mlsysim.core.units (TFLOPs, TB, second, flop, byte), mlsysim.book (fmt, check)
# │ Exports: MemoryWallSpecs.a100_fp16_tflop_per_s_str, MemoryWallSpecs.a100_bw_tb_per_s_str,
# │          MemoryWallSpecs.relu_peak_util_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
class MemoryWallSpecs:
    ...
    flops_fp16_q = gpu.compute.peak_flops
    bw_q = gpu.memory.bandwidth

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # Step 1: Arithmetic Intensity "Ridge Point" (FLOP/byte)
    ridge_point_q = gpu.ridge_point().to(flop/byte)
    ridge_point = ridge_point_q.magnitude
    relu_bytes_per_element = 2 * BYTES_FP16
    relu_arithmetic_intensity = (1 * flop / relu_bytes_per_element).to(flop / byte)
    relu_peak_util = (relu_arithmetic_intensity / ridge_point_q).to("").magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(ridge_point >= 100, f"A100 ridge point ({ridge_point:.1f}) is too low to claim a 'Memory Wall'.")
    check(relu_peak_util < 0.01, "Element-wise ReLU should use less than 1 percent of A100 peak FLOP/s.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_fp16_tflop_per_s_str = fmt_qty(flops_fp16_q, TFLOP/second, precision=0, commas=False)
    a100_bw_tb_per_s_str = fmt_qty(bw_q, TB/second, precision=2, commas=False)
    relu_peak_util_pct_str = fmt_percent(relu_peak_util, precision=1, commas=False, style="prose")

# Note: Use MemoryWallSpecs.a100_fp16_tflop_per_s_str directly.
```
...
# │ Exports: A100BLASMemoryWallRecap.dense_tflop_per_s_str
class A100BLASMemoryWallRecap:
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    gpu = Hardware.Cloud.A100

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    dense_flops_q = gpu.compute.peak_flops

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    dense_tflop_per_s_str = fmt_qty(dense_flops_q, TFLOP/second, precision=0, commas=False)
```

To understand why execution strategy matters so much, return to the memory wall\index{Memory Wall!execution strategy impact} introduced in @sec-ml-systems\index{Memory Wall!framework execution}: processor arithmetic has grown faster than memory bandwidth. Modern GPUs can perform arithmetic far faster than they can fetch data. On an A100 GPU with `{python} A100BLASMemoryWallRecap.dense_tflop_per_s_str` of compute and `{python} MemoryWallSpecs.a100_bw_tb_per_s_str` of memory bandwidth, element-wise operations like ReLU achieve about `{python} MemoryWallSpecs.relu_peak_util_pct_str` of peak compute capacity, not because the hardware is slow, but because they spend nearly all their time waiting for data. @Sec-appdx-machine-foundations-roofline-model-2529 formalizes this trade-off, showing exactly when operations are memory bound vs. compute bound.
...
[^fn-flashattention-fusion-fw]: **Fused Attention Kernel**: A fused attention kernel combines the $QK^T$ product, softmax, and value-weighted output into a tiled implementation that keeps intermediate values in on-chip memory rather than materializing the full attention matrix in HBM. FlashAttention is the canonical named example introduced in @sec-network-architectures and reports 10--20$\times$ lower HBM traffic with 2--4$\times$ wall-clock speedups [@dao2022]. The framework lesson is broader than the specific algorithm: fusion can shift an operation's position on the Roofline Model from bandwidth-limited toward throughput-limited execution by reducing round-trips through external memory. \index{Attention Mechanism!fused kernel}
...
class DispatchTax:
    ...
    small_tax_pct_str = fmt_percent(round(small_tax_pct)/100, precision=0, commas=False, style='symbol')
    large_tax_pct_str = fmt_percent(round(large_tax_pct)/100, precision=0, commas=False, style='symbol')
```
...
class GraphOptimizationStats:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    flop_reduction_range_str = fmt_percent_range(flop_reduction_min_pct / 100, flop_reduction_max_pct / 100, precision=0, commas=False, style="prose")
```
...
# │
# │ Imports: mlsysim.book (fmt_time, fmt_multiple, check)
...
# │          overhead_speedup_mult_str, bw_efficiency_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
class FusionSpeedup:
    ...
    memory_access = Ops.RuntimeOverheads.TinyMemoryAccess
    ...
    memory_access_us_str = fmt_time(memory_access, microsecond, precision=0, commas=False)
    ...
    overhead_speedup_mult_str = fmt_multiple(speedup, precision=0, commas=False)
    bw_efficiency_mult_str = fmt_multiple(bw_efficiency, precision=0, commas=False)
...
*   **Python dispatch**: ~`{python} FusionSpeedup.python_dispatch_us_str` per operation.
*   **Kernel launch**: ~`{python} FusionSpeedup.kernel_launch_only_us_str` per operation.
*   **Memory access (VRAM)**: ~`{python} FusionSpeedup.memory_access_us_str`.
...
*   **Total memory traffic**: 2N bytes (`{python} FusionSpeedup.bw_efficiency_mult_str` bandwidth efficiency).
...
1. **TorchDynamo**\index{TorchDynamo!graph capture} (graph capture): Intercepts Python bytecode execution using CPython's PEP 523 frame evaluation API, the interpreter hook that lets a tool take control when a Python frame is evaluated. Unlike `torch.jit.trace`, which records a single execution path and silently ignores alternative branches, TorchDynamo also captures operations during execution but inserts *graph breaks* when it encounters unsupported code (print statements, arbitrary Python), ensuring correctness rather than silent failure. The current graph is finalized for compilation, unsupported code executes eagerly, and a new graph begins after.
...
The generated code is cached on disk: TorchInductor maintains its own compilation cache, and Triton kernels are additionally cached in `~/.triton/cache/`. The first execution therefore pays the full compilation path: TorchDynamo intercepts bytecode and records operations into an FX graph, TorchInductor compiles the graph (5--30 seconds for transformer models), and the compiled code is cached and executed. Subsequent runs with the same input shapes dispatch directly to compiled code with microseconds of overhead; if input shapes change, TorchInductor must recompile for the new shapes, and PyTorch maintains separate compiled versions for each unique shape configuration.

##### Graph breaks: Causes and detection {.unnumbered}

Graph breaks mark the boundary where compilation stops applying: unsupported code returns execution to eager mode and reintroduces the dispatch overhead that compilation was meant to amortize. Diagnosing them therefore belongs in performance analysis rather than only in debugging.

Data-dependent control flow is a common break because it requires tensor values unavailable at compile time, as shown in @lst-graph-break-control-flow.
...
TorchDynamo creates a graph break: operations before the if statement are compiled, the if statement executes eagerly (evaluating which branch to take), and the chosen branch is compiled as a separate region. Unsupported operations also cause graph breaks, as @lst-graph-break-io demonstrates.
...
The diagnostic step is to surface break locations and reasons, as @lst-graph-break-detect demonstrates.
...
The measurement discipline has three parts. Use `torch.cuda.synchronize()` because CUDA operations are asynchronous; without synchronization, timing measures only kernel launch time, not execution time. Warm up compilation by calling once before timing to exclude compilation from measurements. Run 100+ iterations to amortize measurement overhead.
...
These execution models present a spectrum of trade-offs, but engineers need more than intuition to navigate them. Two quantitative principles formalize the decision. The Compilation Continuum Principle\index{Compilation Continuum!principle} establishes when the performance gains from compilation justify its development cost, expressed as a ratio of production executions to development iterations. The Dispatch Overhead Law quantifies the per-operation cost of framework flexibility, revealing why small operations in eager mode can spend more time in Python overhead than in actual computation. Together, these principles transform framework selection from subjective preference into measurable engineering analysis.
...
# │ Exports: CompilationBenchmark.resnet_eager_img_per_s_str, resnet_compile_img_per_s_str,
# │          torch_compile_speedup_range_mult_str, tensorrt_speedup_range_mult_str,
# │          resnet_compile_speedup_pct_str,
# │          resnet_tensorrt_speedup_mult_str, compile_time_ref_s_str,
...
class CompilationBenchmark:
    ...
    torch_compile_speedup_range_mult_str = fmt_multiple_range(
        min(torch_compile_speedups), max(torch_compile_speedups), precision=1, commas=False
    )
    tensorrt_speedup_range_mult_str = fmt_multiple_range(
        min(tensorrt_speedups), max(tensorrt_speedups), precision=1, commas=False
    )

    resnet_compile_speedup_pct_str = fmt_percent(resnet_compile_speedup_pct/100, precision=1, commas=False, style='symbol')
...
**Systems insight**: Compilation benefits scale with how much of the workload is optimizable. Compute Beasts (@tbl-training-benchmark: ResNet-50 sees `{python} CompilationBenchmark.resnet_tensorrt_speedup_mult_str` speedup from TensorRT) benefit most. Sparse Scatter workloads gain little because their bottleneck (embedding lookups) is inherently irregular.
...
For training runs ($N_{\text{prod}} \gg N_{\text{dev}}$), compilation pays off. A typical training run executes millions of forward/backward passes, so even 60 seconds of compilation amortizes to microseconds per step. From @tbl-training-benchmark, torch.compile provides `{python} CompilationBenchmark.resnet_compile_speedup_pct_str` higher throughput on ResNet-50 (`{python} CompilationBenchmark.resnet_compile_img_per_s_str` vs. `{python} CompilationBenchmark.resnet_eager_img_per_s_str`). Using a `{python} CompilationBenchmark.compile_time_ref_s_str` compile cost, this pays off after the breakeven point in @eq-compile-breakeven:
...
# │
# │ Imports: mlsysim.book (fmt, fmt_count, fmt_math)
# │ Exports: dispatch_n_ops_str, dispatch_us_per_op_str, dispatch_hw_time_us_str,
# │          dispatch_sw_time_us_str, dispatch_ratio_small_str, dispatch_overhead_pct_str,
# │          dispatch_compilation_speedup_mult_str, gpt3_hw_time_us_str, gpt3_sw_time_us_str,
...
class DispatchTaxSmallLarge:
    ...
    dispatch_overhead_pct_str = fmt_percent(round(small_overhead_pct)/100, precision=0, commas=False, style='symbol')
    dispatch_compilation_speedup_mult_str = fmt_multiple(small_speedup_limit, precision=1, commas=False)

    gpt3_hw_time_us_str = fmt_time(large_hw_us, 'microsecond', precision=0, commas=False)
...
*   **Small-model outcome**: The system spends `{python} DispatchTaxSmallLarge.dispatch_overhead_pct_str` of time in host-side dispatch and kernel-launch overhead. Compilation yields `{python} DispatchTaxSmallLarge.dispatch_compilation_speedup_mult_str` speedup.
...
# │
# │ Imports: mlsysim.core.units (Bparam), mlsysim.book (fmt, check)
...
Forward mode achieves this systematic derivative computation by augmenting each number with its derivative value, creating what mathematicians call a "dual number"\index{Forward Mode AD!dual numbers}\index{Dual Number!forward mode AD}. @Lst-forward_mode_dual traces a concrete execution with $x = 2.0$, revealing how each intermediate result carries both its value and derivative through the computation.
...
Every modern ML framework defaults to reverse mode\index{Automatic Differentiation!reverse mode}\index{Reverse Mode AD!backpropagation} for training because of computational asymmetry\index{Backpropagation!computational asymmetry}, one of the most consequential design decisions in machine learning software.
...
This asymmetry makes reverse mode the only viable option for training. Consider a function where $x$ influences the output through two distinct paths. @Lst-reverse_simple defines such a function, and @alg-reverse-mode-ad-trace states the general reverse-mode contract before the following worked trace makes one concrete input visible.
...
::: {#alg-reverse-mode-ad-trace name="Reverse-mode automatic differentiation"}

**Require**: Scalar output node $y$ in a directed acyclic computation graph; topologically ordered nodes $v_1,\ldots,v_n$; for each node, the stored forward value and local derivative rule.

**State**: For each node $v_i$, the adjoint $\bar{v}_i=\partial y/\partial v_i$ stores the accumulated sensitivity of the output to that node.

**Output**: Adjoint values for the requested input or parameter nodes.

1. Run the graph forward in topological order, storing each node value and the parent edges required by its local derivative rule.
2. Initialize every adjoint to zero and seed the output adjoint: $\bar{y}\gets 1$.
3. Traverse nodes in reverse topological order.
4. For each parent $u$ of the current node $v$, accumulate $\bar{u}\gets \bar{u}+\bar{v}\,\partial v/\partial u$ using the stored forward values.
5. Release any stored forward value whose local derivative rules have all consumed it.
6. Return the adjoints for the requested input or parameter nodes.

**Systems cost**: Step 1 retains forward values and graph edges until steps 3 through 5 consume them in reverse, so reverse mode computes all requested parameter gradients in one backward traversal but trades that compute efficiency for activation and graph memory proportional to the live forward state.

:::

For the concrete function in @lst-reverse_simple with $x=2.0$, the forward pass stores $a=x^2=4.0$, $b=\sin(x)\approx0.9093$, and $c=ab\approx3.637$. Seeding $\bar{c}=1.0$, the reverse multiplication gives $\bar{a}=0.9093$ and $\bar{b}=4.0$; the square path contributes $0.9093\cdot4.0=3.637$ to $\bar{x}$, and the sine path adds $4.0\cos(2.0)$. The final derivative is $\partial c/\partial x=\bar{x}\approx1.972$.
...
# │
# │ Imports: mlsysim.core.units (Models.Language.GPT3.parameters, BYTES_FP16, GB, Bparam),
...
The second strategy is *operation fusion*\index{Kernel Fusion}\index{Kernel Fusion!eliminating intermediate allocations}[^fn-operation-fusion-bandwidth]. Rather than executing matrix multiplication, bias addition, and ReLU as three separate operations, each writing intermediate results to memory, frameworks fuse them into a single kernel. This eliminates intermediate memory allocations entirely and achieves 2--3$\times$ speedup on modern GPUs by keeping data in registers and caches.
...
# │
# │ Imports: mlsysim.core.units (BYTES_FP32, BYTES_ADAM_STATE, MB, Mparam),
...
# │          resnet_training_ratio_range_mult_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
...
class ResNetMemory:
    ...
    training_min = 10 * GB  # scenario assumption
    training_max = 15 * GB  # scenario assumption
    ...
    resnet_training_min_gb_str = fmt_qty(training_min, GB, precision=0, commas=False)
    resnet_training_max_gb_str = fmt_qty(training_max, GB, precision=0, commas=False)
    resnet_training_ratio_range_mult_str = fmt_multiple_range(
        training_ratio_min, training_ratio_max, precision=1, commas=False
    )
```

::: {.column-margin}
![](images/svg/vol1_frameworks_margin_001.svg){width="100%" fig-alt="Vertical memory ladder comparing a ResNet-50 training footprint around 10 to 15 GB against an inference footprint around 102 MB."}
...
Graph retention\index{Graph Retention!multiple backward passes} is a memory trade-off, not a harmless flag. By default, `backward()` frees the graph after use. Multiple backward passes for multi-loss optimization or higher-order derivatives require `retain_graph=True`, at the cost of doubled memory, as shown in @lst-retain-graph.
...
Gradient accumulation is the complementary correctness hazard: the framework preserves accumulated gradients until the training loop resets them. As @lst-gradient-accumulation demonstrates, without calling `zero_grad()`, successive backward passes sum their gradients:
...
Custom autograd functions\index{Custom Autograd Function!backward implementation} move part of the differentiation contract from the framework to the implementer. The developer explicitly specifies what to save for the backward pass and how to compute gradients. @Lst-custom-autograd-function shows the pattern:
...
Gradient hooks\index{Gradient Hook!inspection and modification} expose a controlled interception point during backpropagation: they let a system inspect or modify gradients without changing the core traversal algorithm, as shown in @lst-gradient-hooks:
...
Detachment is the mechanism for intentionally cutting gradient flow\index{Gradient Flow!tensor detachment}; cloning the result before in-place mutation preserves that boundary. @Lst-detach-vs-data illustrates how the legacy `.data` attribute can silently corrupt gradient computation through in-place operations:
...
# │
# │ Imports: mlsysim.core.units (BYTES_FP16, BYTES_ADAM_STATE, GB, ureg),
...
When "OOM" (Out of Memory) errors\index{Memory Fragmentation!OOM errors} appear despite `nvidia-smi` showing free memory, fragmentation is often the culprit. The allocator cannot find a contiguous block large enough for the requested tensor.
...
To make this concrete, trace what must happen when a programmer writes `model(input)`. The framework must resolve five decisions in rapid succession: data representation (tensor shape, memory layout, numeric precision), device placement (the bandwidth hierarchy connecting CPU, GPU, and accelerator memory), input delivery (data pipelines that sustain hundreds of MB/s to keep the accelerator fed), scaling context (parameter synchronization and distributed execution contexts), and kernel execution (dispatch, scheduling, and resource optimization). The discussion follows those decisions in order, building from the data container up to the hardware execution layer.
...
# │
# │ Imports: mlsysim.core.units (Models.Vision.ResNet50.parameters), mlsysim.book (fmt)
...
At the foundation of every framework's data representation lies a single abstraction: the **tensor**\index{Tensor!n-dimensional arrays}\index{Tensor!etymology}.
...
# │
# │ Imports: mlsysim.core.units (BYTES_FP32, byte, MB), mlsysim.book (fmt, check)
...
2.  **Observation**: Unlike a Python list or generic NumPy array, a framework tensor carries device placement metadata (CPU vs. GPU), dtype (FP32, BF16, INT8), and stride information that enables zero-copy view operations and CUDA kernel dispatch without any runtime type checking or data movement.
3.  **Pitfall**: A frequent misconception is that tensor operations are always in-place. Framework tensor operations return new tensors by default, allocating fresh GPU memory for each intermediate result. In a long computation graph, these intermediate allocations accumulate and can exhaust GPU memory before any weights are updated.
...
# │
# │ Imports: mlsysim.core.units (BYTES_FP16, BYTES_ADAM_STATE, GB), mlsysim.book (fmt, fmt_math)
...
class AdminTax:
    ...
    admin_act_calc_math = fmt_math(f"{admin_batch_str} \\times {admin_layers_str} \\times {admin_seq_len_str} \\times {admin_width_str} \\times {admin_saved_tensors_str} \\times 2 \\approx \\mathbf{{{admin_act_gb_str}}}")
```
...
# │ Exports: ResNetAdminTaxRatio.resnet_training_ratio_range_mult_str
from mlsysim import Models
...
The memory breakdown for ResNet-50 in @sec-ml-frameworks-principle-2-memorycompute-tradeoff-19f2 showed a concrete ~`{python} ResNetAdminTaxRatio.resnet_training_ratio_range_mult_str` ratio between training and inference memory. Here we generalize that analysis to reveal the full administrative overhead at billion-parameter scale.
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.A100.interconnect.bandwidth, Hardware.Cloud.A100.nvlink.bandwidth, Hardware.Cloud.A100.memory.bandwidth, Hardware.Cloud.A100.memory.capacity,
# │          Hardware.Cloud.A100.compute.peak_flops), mlsysim.book (fmt)
# │ Exports: pcie4_gb_per_s_str, pcie4_bidir_gb_per_s_str, nvlink_a100_gb_per_s_str, a100_bw_gb_per_s_str,
# │          a100_bw_tb_per_s_str, a100_fp16_tflop_per_s_str, pcie4_4mb_ms_str, nvlink_4mb_ms_str,
# │          hbm_4mb_ms_str, pcie4_1gb_ms_str, pcie4_1gb_equiv_ops_tflop_str, a100_mem_gib_str,
# │          pcie_to_hbm_gap_mult_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, fmt_memory, fmt_time, fmt_qty
...
class DeviceBandwidthHierarchy:
    ...
    a100_mem_gib_str = fmt_memory(a100_mem, binary=True, precision=0, commas=False)
    ...
    pcie4_1gb_equiv_ops_tflop_str = fmt_qty(pcie4_1gb_equiv_ops, TFLOP, precision=1, commas=False)
...
[^fn-nvlink-bandwidth-hierarchy]: **NVLink**: NVIDIA's high-bandwidth GPU-to-GPU interconnect (see @sec-hardware-acceleration), providing `{python} DeviceBandwidthHierarchy.nvlink_a100_gb_per_s_str` bidirectional bandwidth (NVLink 3.0 on A100) compared to `{python} DeviceBandwidthHierarchy.pcie4_bidir_gb_per_s_str` for PCIe 4.0 x16. This ~10$\times$ bandwidth advantage determines whether tensor parallelism, splitting one large tensor computation across multiple GPUs, is practical for a given model size: GPUs connected only by PCIe can make the $D_{\text{vol}}/\text{BW}$ communication term dominate total training time, erasing the benefit of additional compute. \index{NVLink!bandwidth hierarchy}

| **Interconnect** |                                                                             **Bandwidth**                                                                             |                   **Transfer Time**                   | **Relative to Compute**            |
|:-----------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------:|:-----------------------------------------------------:|:-----------------------------------|
| **PCIe 3.0 x16** |                                                                                16 GB/s                                                                                |                        0.25 ms                        | 10$\times$ slower than GPU compute |
| **PCIe 4.0 x16** |                                                         `{python} DeviceBandwidthHierarchy.pcie4_gb_per_s_str`                                                        |  `{python} DeviceBandwidthHierarchy.pcie4_4mb_ms_str` | 5$\times$ slower than GPU compute  |
| **NVLink 3.0**   | `{python} DeviceBandwidthHierarchy.nvlink_a100_per_direction_gb_per_s_str` per direction (`{python} DeviceBandwidthHierarchy.nvlink_a100_gb_per_s_str` bidirectional) | `{python} DeviceBandwidthHierarchy.nvlink_4mb_ms_str` | Comparable to GPU compute          |
| **GPU Memory**   |                                                        `{python} DeviceBandwidthHierarchy.a100_bw_gb_per_s_str`                                                       |   `{python} DeviceBandwidthHierarchy.hbm_4mb_ms_str`  | Optimal                            |
...
These numbers connect directly to the iron law of performance. Every cross-device transfer inflates the data movement term $(D_{\text{vol}}/\text{BW})$ at a fraction of the available on-device bandwidth. A PCIe 4.0 transfer at `{python} DeviceBandwidthHierarchy.pcie4_gb_per_s_str` means moving a 1 GB activation tensor adds approximately `{python} DeviceBandwidthHierarchy.pcie4_1gb_ms_str` to the data movement cost, equivalent to roughly `{python} DeviceBandwidthHierarchy.pcie4_1gb_equiv_ops_tflop_str` operations on a GPU delivering `{python} DeviceBandwidthHierarchy.a100_fp16_tflop_per_s_str`. For a model forward pass taking 0.5 ms on GPU, transferring inputs and outputs over PCIe 3.0 doubles the total latency. When batches are small or models are lightweight, transfer overhead can exceed computation time entirely.
...
The `non_blocking=True` flag enables asynchronous transfers that return immediately without waiting for completion. This works only when the source tensor uses pinned memory\index{Pinned Memory!DMA transfers} (page-locked memory that enables DMA transfers\index{DMA Transfer!GPU copy engine}). Without pinned memory, the transfer blocks even when `non_blocking=True` is specified, because the GPU's copy engine cannot initiate a DMA transfer from pageable host memory.
...
The same synchronization discipline has one operational trap: developers often insert `torch.cuda.synchronize()` calls for debugging and forget to remove them, silently converting an overlapped pipeline into a serialized one.

When overlap remains poor, profiling must separate scheduling stalls from kernel inefficiency. **Nsight Systems**\index{Nsight Systems!profiling tool} (`nsys profile`) captures system-wide timelines correlating CPU activity, GPU kernel execution, and memory transfers, identifying which kernels dominate runtime. **Nsight Compute**\index{Nsight Compute!profiling tool} (`ncu`) provides kernel-level analysis with hardware counters, revealing *why* those kernels underperform. In this table, SM means streaming multiprocessor, the GPU execution block that schedules groups of threads; a warp is one such scheduled thread group. @Tbl-nsight-metrics lists the key metrics to examine when optimizing ML kernels.
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.A100.interconnect.bandwidth, BYTES_FP32), mlsysim.book (fmt)
# │ Exports: img_res_str, dataloader_mb_per_s_str, batch_mb_str, batch_transfer_ms_str,
# │          pcie4_gb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, fmt_percent, fmt_rate, fmt_time
...
When training scales beyond a single machine, these same abstractions extend to manage process groups and communication primitives. Frameworks use constructs like `ProcessGroup`\index{ProcessGroup!distributed execution} (PyTorch) or `Mesh` (JAX) to define logical collections of devices, specify group operations such as broadcast and reduction, maintain shared synchronization state, and redistribute tensors as needed. The important framework idea is the abstraction boundary: user code names logical device groups, and the framework maps those groups onto the physical communication layer.

These concepts appear here because they shape framework API design even for single-node code, while the full implementation details of distributed training---including gradient compression, communication topologies, and fault tolerance---build on these foundations later. When models exceed single-device memory, frameworks must give the training system more than one placement dimension. A GPT-3 scale model, for instance, cannot fit on a single GPU---its `{python} GPT3ParameterStructures.gpt3_params_b_str` parameters alone require `{python} GPT3ParameterStructures.gpt3_fp16_gb_str` in FP16, far exceeding any GPU's memory. @Fig-3d-parallelism previews the framework placement idea without requiring the full distributed-training machinery yet: a system can split work across layer groups, across replicated batches, or within very large tensors. @Sec-model-training-scaling-training-systems-adfd later formalizes these dimensions as training strategies and analyzes their communication costs.
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.A100.compute.peak_flops, TFLOPs, second)
# │ Exports: KernelDispatchAnchor.a100_fp16_tflop_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_qty, check
...
# │
# │ Imports: mlsysim.core.units (Models.Vision.ResNet50.inference_flops), mlsysim.book (fmt)
# │ Exports: resnet_gflop_str
# └─────────────────────────────────────────────────────────────────────────────
...
Every major framework answers this question through a *module abstraction* that bundles parameters, forward computation, and state management into a single reusable unit. PyTorch's `nn.Module`\index{Module Abstraction!PyTorch nn.Module}[^fn-nn-module-composition] provides an instructive case study because its design patterns recur across frameworks: Keras uses similar layer abstractions [@chollet2018keras], JAX's Flax employs analogous module structures, and TensorFlow's functional API shares conceptual parallels. Rather than catalog its API, we extract three enduring design principles that every framework must address regardless of its syntax or programming paradigm.
...
Module hooks\index{Module Hooks!forward and backward} are the inspection counterpart to parameter freezing: they intercept intermediate computations without modifying model code, enabling gradient flow diagnosis and activation monitoring. @Lst-module_hooks illustrates both hook types.
...
While TensorFlow 2.0 introduced eager execution to bridge the gap between research and production, its core strength remains the robust, compiled path from research to global-scale deployment. The model training and serving chapters later examine the scaling and production infrastructure that use these export paths.
...
: **Framework Design Trade-Offs**: Each column reflects a distinct answer to the three core problems. TensorFlow makes deployment and graph capture central, PyTorch makes eager execution and debugging central, and JAX makes functional transformation and compiler visibility central. The table is a diagnostic map, not a benchmark ranking. {#tbl-mlfm-comparison}

The measurement implication is straightforward: profile the constraint each framework makes most visible. In PyTorch, check whether eager dispatch or graph breaks dominate. In TensorFlow, check whether the captured graph covers the operators and deployment target. In JAX, check whether shapes and purity let XLA compile the program you actually run. A framework-level comparison or leaderboard can orient a decision, but it cannot replace profiling the specific workload on the target hardware.
...
No framework optimizes all three problems simultaneously; each makes deliberate trade-offs that shape everything from API design to performance characteristics. PyTorch prioritizes the Execution Problem (eager debugging, dynamic graphs) at the cost of optimization potential. TensorFlow prioritizes the Abstraction Problem (unified deployment from cloud to microcontroller) at the cost of development flexibility. JAX reframes the Differentiation Problem (composable function transformations) at the cost of a steeper learning curve. These are the same design tensions examined in the preceding subsections, now visible even in a ten-line program. Exploratory research often benefits from PyTorch's debugging immediacy, deployment pipelines often benefit from TensorFlow's graph/export path, and algorithmic research can benefit from JAX's composable transformations. Each philosophy shapes code syntax, team workflows, debugging practices, and deployment pipelines, which is why framework migration costs are measured in engineer-months rather than engineer-days.

These design differences are not arbitrary; they reflect different trade-offs between compiler visibility and human iteration latency. TensorFlow's graph compilation can reduce runtime overhead and intermediate data movement through ahead-of-time optimization. PyTorch's eager execution reduces human iteration latency outside the iron-law runtime equation, often at the cost of less compiler visibility until a graph-capture path is used. JAX's functional style gives XLA more freedom to fuse, reorder, and parallelize operations, reducing launch overhead and data movement when the traced program is stable.
...
As ML models move from cloud servers to edge devices\index{Deployment Target!cloud to edge spectrum}, the efficiency gaps measured earlier transform from optimization opportunities into hard deployment constraints. Framework selection\index{Edge Deployment!framework selection} must reweight the three core problems dramatically at the edge\index{Edge Deployment!three problems reweighting}. The *execution problem* shifts from choosing between eager and graph execution to fitting computation inside 10 ms latency and 50 KB memory budgets. The *differentiation problem* often disappears entirely, since edge devices run inference only. The *abstraction problem* intensifies: targeting ARM vs. x86, mobile NPUs vs. edge TPUs, microcontrollers with kilobytes of memory.
...
: **Deployment Target Constraint Map**: Runtime assumptions, optimization levers, and binding constraints for each deployment tier, from cloud servers to microcontrollers. {#tbl-deployment-frameworks tbl-colwidths="[19,31,25,25]"}
...
The first question is whether a framework can express the models a project requires. Examine @tbl-tf-comparison: notice how operator count drops from approximately $10^3$ (full TensorFlow) to $10^2$ (TensorFlow Lite) to $10^1$ (TensorFlow Lite Micro). Each reduction eliminates training capability and general-purpose operations while adding native lower-precision tooling. The engineering principle is that expressiveness and efficiency trade against each other: fewer supported operations enable tighter code generation, smaller binaries, and hardware-specific optimization paths. This progressive constraint model applies to any framework family, not just TensorFlow. Layered on top of operator count is a separate axis with its own trade-off: when the graph is captured statically before execution versus assembled dynamically at runtime.
...
The concepts developed throughout this chapter---eager vs. graph execution, reverse-mode autodiff, tensor abstractions, kernel dispatch\index{Forward Pass!kernel dispatch}---remain abstract until we see them interact inside a real execution. To solidify understanding, we trace a single training step\index{Training!step, anatomy} through the PyTorch stack, revealing how seven executable Python statements trigger the execution, differentiation\index{Backward Pass!gradient computation}, and abstraction machinery simultaneously.
...
# │ Show: A100 HBM capacity as a binary-memory prose anchor in this chapter.
...
# │ Exports: HbmCapacityFootnote.a100_mem_gib_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.units (byte, KB, MB, flop, KFLOPs, MFLOPs), mlsysim.book (fmt)
# │ Exports: train_mm1_mflop_str, train_mm1_mem_mb_str, train_mm1_ai_str,
# │          train_relu_kflop_str, train_relu_mem_kb_str, train_relu_ai_str,
# │          train_mm2_kflop_str, train_mm2_mem_kb_str, train_mm2_ai_str, train_ce_kflop_str,
# │          train_ce_mem_kb_str, train_ce_ai_str, train_bwd_mflop_str,
# │          train_bwd_mem_mb_str, train_bwd_ai_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, MarkdownStr, fmt_arithmetic_intensity, fmt_qty, fmt_flops
...
class TrainingStepCalc:
    ...
    ce_flops = _batch * _output * 3 * flop                                      # ~1K FLOPs (exp + sum + div)
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check((mm1_flops / mm1_mem).to(flop / byte).magnitude > (relu_flops / relu_mem).to(flop / byte).magnitude,
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    train_mm1_mflop_str = fmt_flops(mm1_flops, unit=MFLOPs, precision=1, commas=False)
    train_mm1_mem_mb_str = fmt_qty(mm1_mem, MB, precision=1, commas=False)
    train_mm1_ai_str = fmt_arithmetic_intensity((mm1_flops / mm1_mem), unit=flop / byte, precision=1, commas=False)

    train_relu_kflop_str = fmt_flops(relu_flops, unit=KFLOPs, precision=1, commas=False)
    ...
    train_mm2_kflop_str = fmt_flops(mm2_flops, unit=KFLOPs, precision=1, commas=False)
    train_mm2_mem_kb_str = fmt_qty(mm2_mem, KB, precision=1, commas=False)
    train_mm2_ai_str = fmt_arithmetic_intensity((mm2_flops / mm2_mem), unit=flop / byte, precision=1, commas=False)

    train_ce_kflop_str = fmt_flops(ce_flops, unit=KFLOPs, precision=1, commas=False)
    train_ce_mem_kb_str = fmt_qty(ce_mem, KB, precision=1, commas=False)
    train_ce_ai_str = fmt_arithmetic_intensity((ce_flops / ce_mem), unit=flop / byte, precision=1, commas=False)

    train_bwd_mflop_str = fmt_flops(bwd_flops, unit=MFLOPs, precision=1, commas=False)
    train_bwd_mem_mb_str = fmt_qty(bwd_mem, MB, precision=1, commas=False)
    train_bwd_ai_str = fmt_arithmetic_intensity((bwd_flops / bwd_mem), unit=flop / byte, precision=1, commas=False)
```

| **Component**                    |                                         **FLOPs**                                         |                 **Memory Traffic**                |            **Arithmetic Intensity**           |
|:---------------------------------|:-----------------------------------------------------------------------------------------:|:-------------------------------------------------:|:---------------------------------------------:|
| **MatMul (x @ W1)**              | 2 $\times$ 32 $\times$ $784{\times}256$ = `{python} TrainingStepCalc.train_mm1_mflop_str` |  `{python} TrainingStepCalc.train_mm1_mem_mb_str` |  `{python} TrainingStepCalc.train_mm1_ai_str` |
| **ReLU**                         |             $32{\times}256$ = `{python} TrainingStepCalc.train_relu_kflop_str`            | `{python} TrainingStepCalc.train_relu_mem_kb_str` | `{python} TrainingStepCalc.train_relu_ai_str` |
| **MatMul (h @ W2)**              |  2 $\times$ 32 $\times$ $256{\times}10$ = `{python} TrainingStepCalc.train_mm2_kflop_str` |  `{python} TrainingStepCalc.train_mm2_mem_kb_str` |  `{python} TrainingStepCalc.train_mm2_ai_str` |
| **Cross-entropy**                |                      ~`{python} TrainingStepCalc.train_ce_kflop_str`                      |  `{python} TrainingStepCalc.train_ce_mem_kb_str`  |  `{python} TrainingStepCalc.train_ce_ai_str`  |
| **Backward (2$\times$ forward)** |                      ~`{python} TrainingStepCalc.train_bwd_mflop_str`                     |  `{python} TrainingStepCalc.train_bwd_mem_mb_str` |  `{python} TrainingStepCalc.train_bwd_ai_str` |
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.A100.compute.peak_flops, Hardware.Cloud.A100.memory.bandwidth), mlsysim.book (fmt)
# │ Exports: mnist_total_mflop_str, mnist_mem_traffic_mb_str, a100_peak_tflop_per_s_str,
# │          a100_mem_bw_tb_per_s_str, mnist_t_compute_us_str, mnist_t_memory_us_str,
# │          mnist_n_ops_str, mnist_us_per_op_str, mnist_t_overhead_us_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, MarkdownStr, fmt_time, fmt_qty, fmt_flops
...
class MnistTrainingStepCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(t_overhead_us > 10 * max(t_compute.to(microsecond).magnitude, t_memory.to(microsecond).magnitude),
    ...
    a100_peak_tflop_per_s_str = fmt_qty(peak_flops, TFLOP/second, precision=0, commas=False)
    a100_mem_bw_tb_per_s_str = fmt_qty(mem_bw, TB/second, precision=1, commas=False)
```

Total: ~`{python} MnistTrainingStepCalc.mnist_total_mflop_str`, ~`{python} MnistTrainingStepCalc.mnist_mem_traffic_mb_str` memory traffic. On an A100:

- $T_{\text{compute}} \approx$ `{python} MnistTrainingStepCalc.mnist_total_mflop_str`/`{python} MnistTrainingStepCalc.a100_peak_tflop_per_s_str` ≈ `{python} MnistTrainingStepCalc.mnist_t_compute_us_str`
...
- $T_{\text{overhead}} \approx$ `{python} MnistTrainingStepCalc.mnist_n_ops_str` $\times$ `{python} MnistTrainingStepCalc.mnist_us_per_op_str` ≈ `{python} MnistTrainingStepCalc.mnist_t_overhead_us_str`
...
- **TPU**\index{TPU!XLA compilation}: XLA compilation to TPU-specific high-level optimizer (HLO) operations
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.A100.memory.capacity, BYTES_FP16),
# │          mlsysim.physics (model_memory)
# │ Exports: Model7BMemory.model_7b_fp16_gib_str,
# │          Model7BMemory.a100_remaining_7b_gib_str, Model7BMemory.a100_mem_gib_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.physics import model_memory
from mlsysim.fmt import check, fmt_memory
from mlsysim import Models
...
class Model7BMemory:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(remaining.to(GiB).magnitude > 60, f"7-billion-parameter FP16 weights leave less A100 memory than expected: {remaining.to(GiB).magnitude:.1f} GiB.")
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    model_7b_fp16_gib_str = fmt_memory(fp16, binary=True, precision=2, commas=False)                  # e.g. "13.04 GiB"
    a100_remaining_7b_gib_str = fmt_memory(remaining, binary=True, precision=2, commas=False)          # e.g. "66.96 GiB"
    a100_mem_gib_str = fmt_memory(a100_mem, binary=True, precision=0, commas=False)                   # e.g. "80 GiB"
```

**Pitfall**: *Increasing batch size without modeling activation memory.*

Engineers assume that if memory is available, larger batches always improve throughput. Larger mini-batches can amortize fixed dispatch overhead, but they also scale the activation memory that must remain live during training. A 7-billion-parameter model in FP16 consumes `{python} Model7BMemory.model_7b_fp16_gib_str`, leaving `{python} Model7BMemory.a100_remaining_7b_gib_str` on an `{python} Model7BMemory.a100_mem_gib_str` A100. Increasing batch size from 8 to 32 quadruples the batch-dependent activation footprint; transformer attention adds a large $\mathcal{O}(S^2)$ term in sequence length that makes each sample expensive. The resulting memory pressure can trigger recomputation strategies that reduce throughput by 20--30 percent despite the larger batch. Teams that blindly maximize batch size often achieve lower throughput than smaller batches that avoid these memory management pathways; @sec-model-training-minibatch-processing-4eb0 formalizes the throughput target.
...
# │          n_images_str, n_recompilations_str, eager_total_s_str, compiled_total_s_str,
# │          compilation_time_s_str, compiled_slowdown_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
Engineers assume compilation overhead is a one-time cost that pays off quickly. @Tbl-training-benchmark shows torch.compile achieves `{python} CompilationOverheadCalc.resnet_compile_speedup_pct_str` higher ResNet-50 throughput, but the same table also assigns ResNet-50 a compile window of `{python} CompilationOverheadCalc.compile_min_s_str` to `{python} CompilationOverheadCalc.compile_max_s_str` per graph change. For a `{python} CompilationOverheadCalc.n_images_str`-image experiment with `{python} CompilationOverheadCalc.n_recompilations_str` code changes: Eager completes in `{python} CompilationOverheadCalc.eager_total_s_str`, whereas Compiled requires `{python} CompilationOverheadCalc.compiled_total_s_str` (including `{python} CompilationOverheadCalc.n_recompilations_str` $\times$ `{python} CompilationOverheadCalc.compilation_time_s_str` recompilation overhead), making the compiled workflow about `{python} CompilationOverheadCalc.compiled_slowdown_mult_str` slower in this rapid-prototyping scenario. Teams that enable compilation during rapid prototyping can waste minutes waiting for recompilations that negate any throughput gains.
...
* **The memory wall drives optimization**: Compute has grown approximately 1000$\times$ faster than memory bandwidth. Kernel fusion, selective recomputation, lower-precision execution, and data layout optimizations all target the data movement term $(D_{\text{vol}})$ in the iron law, not the compute term.
