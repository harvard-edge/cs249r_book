...
_Why is the most lightweight decision in a project—which library to import—often the heaviest constraint on its future?_

Architectures define *what* computations neural networks must perform—convolutions for spatial data, attention for sequences, recurrence for temporal dependencies. But knowing what to compute differs fundamentally from knowing *how* to compute it efficiently across diverse hardware. A framework is not merely a tool you use; it is the substrate you exist within. The choice to adopt PyTorch, TensorFlow, or JAX dictates far more than syntax: it defines your hardware compatibility, limits your deployment targets, and bounds your optimization strategies. Migrating frameworks is rarely a refactor; it is an architectural rebuild that invalidates data pipelines, serving infrastructure, and team expertise accumulated over years. This lock-in explains why framework selection carries strategic weight disproportionate to its apparent simplicity: the freedom of research prototyping transforms into the architectural destiny of production. Organizations that treat frameworks as interchangeable tools discover too late that their inference pipeline requires a graph representation their eager-mode framework cannot provide, or that their target hardware lacks support in the ecosystem they chose.
...
**Machine Learning Frameworks** refer to _software platforms_ that solve the _execution_, _differentiation_, and _abstraction_ problems, providing _standardized interfaces_ for model development, training, and deployment across _diverse computational infrastructure_.
...
##### Two Capture Strategies: Tracing vs. Scripting {.unnumbered}
...
##### Comparison: When to Use Each {.unnumbered}
...
For a ResNet-50 (25M parameters, ~100 MB in FP32) processing batch size 64 with 224×224 images:

- Forward activations: ~8-12 GB (varies by implementation and checkpointing)
- Parameter gradients: ~100 MB (same size as parameters)
- Optimizer state (Adam): ~200 MB (2 states per parameter)
- **Total**: ~10-15 GB for training vs. ~100 MB + batch activations for inference
...
Not every tensor in a computation should contribute gradients. Transfer learning, for example, requires freezing pretrained features so that only new layers receive gradient updates. Frameworks therefore provide mechanisms to detach tensors from the computation graph. @lst-detach-vs-data contrasts the safe `.detach()` method with the deprecated `.data` attribute, revealing how in-place operations on `.data` can silently corrupt gradient computation:
...
1. **Gradient checkpointing** [@chen2016training]: Recompute activations during backward instead of storing them. Trades computation for memory (typically 2× slower backward, 50% less memory).
...
Beyond sequential execution, modern networks frequently contain independent branches whose gradients can be computed concurrently. Exploiting this parallelism requires the AD system to analyze the computation graph for independent subgraphs and schedule their backward passes on separate hardware execution units. @lst-parallel_ad illustrates a branching network where two convolutional paths process the same input independently before merging, creating an opportunity for concurrent gradient computation that the framework's scheduler must recognize.
...
The way frameworks implement automatic differentiation varies significantly, with consequences for both *optimization potential* and developer experience.
...
While tensors provide a clean abstraction for computation, the memory required to store model weights, gradients, and optimizer states in tensor form quickly accumulates beyond what practitioners expect. The following notebook exercise quantifies this hidden overhead, which we call the *administrative tax*.
...
##### Tensor Device Placement and the .to() Method {.unnumbered}
...
##### CUDA Contexts and Current Device {.unnumbered}
...
##### CUDA Streams for Concurrent Operations {.unnumbered}
...
##### Device Placement Best Practices {.unnumbered}
...
##### Memory Transfer Overhead Quantification {.unnumbered}
...
##### Dataset Abstraction: Map-Style and Iterable Datasets {.unnumbered}
...
##### DataLoader Multiprocessing Architecture {.unnumbered}
...
##### Worker Process Management {.unnumbered}
...
These architectural differences manifest in distinct programming paradigms and API design choices. @lst-framework-hello-world illustrates how the same simple neural network (a single linear layer mapping 10 inputs to 1 output) varies dramatically across these major frameworks, revealing their core design philosophies:
...
**Interoperability through ONNX.** The Open Neural Network Exchange (ONNX)[^fn-onnx] format enables model portability across frameworks—train in PyTorch, deploy via TensorFlow Lite or ONNX Runtime. This standardization eliminates manual conversion when moving between development and production environments. @fig-onnx illustrates this hub-and-spoke interoperability model. Detailed deployment optimization—quantization, pruning, hardware-specific compilation—appears in @sec-model-compression and @sec-model-serving-systems.
...
A key factor in this evaluation is how each framework constructs and executes its *computational graph*, a design choice with cascading effects on debugging, optimization, and deployment.
