...
Consider a modern GPU in the NVIDIA A100 class. A useful *representative snapshot* is that it offers on the order of \(10^2\) TFLOPS of reduced-precision tensor compute and on the order of \(10^{12}\) bytes per second (TB/s) of memory bandwidth. The ratio of peak compute to peak bandwidth defines the *ridge point* of the system. For example, \(312\) TFLOPS and \(2\) TB/s implies a ridge point of about \(156\) FLOPS per byte. Operations below this arithmetic intensity cannot fully utilize peak compute; they are *memory-bound* rather than *compute-bound*.

:::{.callout-definition title="Arithmetic Intensity"}
**Arithmetic intensity** refers to the ratio of floating-point operations to bytes of memory traffic (also called *operational intensity*):
...
[^fn-theano]: **Theano**: Pioneering ML framework from Yoshua Bengio's Montreal lab (2007-2017), introducing symbolic computation graphs and automatic differentiation in Python. Named after Greek mathematician Theano of Croton. Its computational graph paradigm directly influenced TensorFlow, while PyTorch adopted Theano's successor Torch. Development ceased in 2017 but its abstractions persist in every modern framework.

[^fn-comp-graphs]: **Computational Graphs**: DAG representations of mathematical operations enabling automatic differentiation, formalized by Wengert in 1964. Nodes represent operations; edges represent data flow. Modern frameworks (TensorFlow, PyTorch) trace graphs with millions of nodes, enabling compiler optimizations like operator fusion and memory planning that achieve 2-10× speedups over naive execution.
...
Static graphs implement a clear separation between graph construction and execution:
...
During construction, `x`, `y`, and `z` are not tensors containing values but rather symbolic nodes in a graph. Operations like `*` and `+` add nodes to the graph definition without performing any arithmetic.
...
**Performance**: Static graphs achieve high performance through ahead-of-time optimization. Kernel fusion reduces memory bandwidth requirements (often the bottleneck for ML workloads), and hardware-specific compilation enables near-peak utilization.

**Limitations**: The cost of this performance is reduced flexibility. Static graphs require fixed control flow: you cannot have conditionals or loops that depend on computed values. While TensorFlow provides `tf.cond` and `tf.while_loop`, these require static unrolling or special handling. Debugging is difficult because stack traces point to graph construction code, not execution code. Error messages often reference symbolic node names rather than the actual operations that failed.
...
Tracing captures operations by executing the function once with example inputs and recording every operation that occurs:
...
**Tracing limitations**: Because tracing records a single execution path, it cannot handle data-dependent control flow. Consider:
...
Scripting analyzes Python source code directly and compiles it to TorchScript IR without executing:
...
The scripting compiler parses Python abstract syntax tree (AST), converts supported operations to TorchScript IR operations, and handles control flow (if/else, for loops, while loops) by preserving the branching structure in the IR. This enables data-dependent control flow to work correctly:
...
**Scripting limitations**: Scripting requires a restricted Python subset because TorchScript cannot support all Python language features:
...
Example of scripting restrictions:
...
Both tracing and scripting produce TorchScript IR, a lower-level representation of computations:
...
**torch.compile: Compiler-Based Execution**

PyTorch 2.0 introduced `torch.compile`, a major advancement that combines eager execution's flexibility with static graph performance. Unlike TorchScript, torch.compile uses Python's frame evaluation API (PEP 523) to capture execution without requiring manual tracing or Python subset restrictions.
...
# First call: captures execution, compiles optimized kernel
result1 = forward(torch.tensor([1.0]))

# Subsequent calls: runs compiled version (~2-5× faster)
result2 = forward(torch.tensor([2.0]))
```

**How torch.compile Works**

The torch.compile compilation pipeline has three stages:

1. **TorchDynamo** (graph capture): Intercepts Python bytecode execution using PEP 523 frame evaluation API. As operations execute, TorchDynamo records them into FX (PyTorch's intermediate representation). Unlike tracing, this happens at bytecode level, allowing capture without manually invoking functions.

2. **FX Graph** (intermediate representation): Operations are represented in a structured format that can be analyzed and transformed. The FX graph is PyTorch's equivalent of LLVM IR or TensorFlow's GraphDef—a standardized representation enabling optimization passes.

3. **TorchInductor** (code generation): Compiles the FX graph to optimized kernels for specific hardware (CUDA, CPU, ROCm). TorchInductor performs kernel fusion, generates efficient memory access patterns, and produces specialized code matching hardware characteristics. For CUDA, it generates Triton kernels; for CPU, it produces C++ with vectorization.

**Graph Breaks**

torch.compile cannot compile all Python code. When it encounters unsupported operations, it creates a **graph break**: the compiled region ends, execution falls back to eager mode, and a new compilation region begins after the unsupported code.

Common causes of graph breaks:

- **Data-dependent control flow**: `if x.sum() > 0: ...` requires knowing tensor values, which aren't available at compile time.
- **Unsupported operations**: Calls to arbitrary Python functions, debugging code (`print`, `breakpoint`), or operations outside torch.compile's scope.
- **Dynamic shapes**: Tensors that change size across iterations prevent compiled code reuse (though dynamic shapes support has improved).
- **Python object manipulation**: Creating or modifying lists, dictionaries, or custom objects in ways torch.compile cannot analyze.

Each graph break incurs overhead: results must be marshalled from compiled code back to Python, then from Python into the next compiled region. Minimizing graph breaks is key to achieving good performance with torch.compile.

**Systems Implications**

**Performance**: Compiled regions achieve 2-5× speedup for many models through kernel fusion and optimized code generation. The speedup approaches static graph performance while maintaining eager mode's programming model. For transformer models, torch.compile typically achieves 1.5-2× training speedup and 2-3× inference speedup.

**Flexibility**: Unlike static graphs or TorchScript, torch.compile preserves eager execution's debugging experience. You can add print statements, use standard Python debuggers, and inspect intermediate values. Compilation is automatic and transparent; removing `@torch.compile` reverts to standard eager execution without code changes.

**Memory overhead**: Compilation requires tracing execution and storing compiled artifacts. For large models, compilation can add 1-2 GB memory overhead. Additionally, the first execution (compilation phase) is slower than eager execution (5-30 seconds for transformer models), amortized over subsequent iterations.
...
+-------------------------+-----------------------------+-----------------------------+-----------------------------+
**torch.compile: Bridging Flexibility and Performance**

PyTorch 2.0 introduced `torch.compile`, a major advancement that combines eager execution's flexibility with static graph performance. Unlike TorchScript, torch.compile uses Python's frame evaluation API (PEP 523) to capture execution without requiring manual tracing or Python subset restrictions.

The core problem torch.compile addresses is the research-production gap: eager mode provides flexibility (data-dependent control flow, easy debugging, natural Python code) but suffers from Python interpreter overhead and limited optimization. Static graphs provide performance (kernel fusion, memory optimization) but sacrifice flexibility. torch.compile aims to achieve compiled performance while maintaining eager mode's programming experience.
...
# First call: captures execution, compiles optimized kernel
result1 = forward(torch.tensor([1.0]))  # ~100ms (compilation)

# Subsequent calls: runs compiled version (~2-5× faster)
result2 = forward(torch.tensor([2.0]))  # ~1ms (compiled)
```

**Architecture: Three-Stage Compilation Pipeline**

The torch.compile system consists of three coordinated components that transform Python code into optimized machine code:

**1. TorchDynamo: Bytecode-Level Graph Capture**

TorchDynamo intercepts Python execution at the bytecode level using CPython's PEP 523 frame evaluation API. When a decorated function executes, TorchDynamo hooks into the Python interpreter's frame evaluation mechanism, observing each bytecode instruction as it executes. This bytecode-level capture enables TorchDynamo to record operations without manual tracing or invoking functions repeatedly.

As operations execute, TorchDynamo identifies PyTorch operations (tensor operations, nn.Module calls) and records them into a graph. When it encounters unsupported operations (print statements, arbitrary Python code), it creates a graph break: the current graph is finalized for compilation, the unsupported code executes eagerly, and a new graph begins after the unsupported region.

TorchDynamo's key advantage over tracing is that it operates at bytecode level, not tensor level. Tracing requires executing code with sample inputs and recording operations, which captures only one execution path. TorchDynamo analyzes bytecode to understand control flow structure before execution, enabling better handling of dynamic behavior.

**2. FX Graph: Intermediate Representation**

Operations captured by TorchDynamo are converted to FX graph format, PyTorch's intermediate representation for computation. The FX graph is a node-based directed acyclic graph where each node represents an operation (matrix multiplication, ReLU activation, addition) with explicit inputs and outputs.

The FX graph serves as the framework-agnostic representation that enables multiple backends. It is analogous to LLVM IR in compiler design: a standardized format that separates frontend (Python code capture) from backend (hardware-specific code generation). This design allows different backends (TorchInductor, ONNX Runtime, TensorRT) to consume FX graphs and generate optimized code for their target platforms.

The FX graph structure enables optimization passes: dead code elimination, constant folding, operation reordering, and pattern matching for fusion opportunities. These transformations operate on the graph before code generation, similar to compiler optimization passes on intermediate representations.

**3. TorchInductor: Code Generation and Optimization**

TorchInductor is the default backend that compiles FX graphs to optimized machine code. For CUDA GPUs, TorchInductor generates Triton kernels (a Python-based GPU kernel language that compiles to PTX). For CPUs, it generates C++ code with vectorization instructions (AVX2, AVX-512).

TorchInductor performs several critical optimizations:

- **Kernel fusion**: Combines multiple operations into single kernels to reduce memory traffic. For example, `(x * 2).relu()` becomes one fused kernel instead of two separate kernels, eliminating the intermediate result write to memory.
- **Memory layout optimization**: Chooses optimal tensor layouts (row-major vs column-major, channel-first vs channel-last) to minimize memory access overhead.
- **Autotuning**: For operations with multiple implementation strategies, TorchInductor can measure actual performance and select the fastest variant for the specific hardware and tensor shapes.

The generated code is cached on disk (in `~/.triton/cache/` for Triton kernels). Subsequent runs with the same input shapes can skip compilation and directly execute cached code, amortizing compilation overhead across multiple training runs.

**Execution Flow: First Call vs Subsequent Calls**

The first execution of a torch.compile decorated function follows a multi-step process:

1. TorchDynamo intercepts bytecode and begins recording operations.
2. Operations are captured into FX graph until graph break or function end.
3. FX graph is passed to TorchInductor for compilation.
4. TorchInductor generates and compiles optimized kernels (5-30 seconds for large models).
5. Compiled code is cached and executed.

Subsequent executions with the same input shapes:

1. TorchDynamo recognizes the cached compiled graph for these shapes.
2. Execution directly dispatches to compiled code (microseconds overhead).
3. No Python interpreter overhead or graph construction cost.

If input shapes change, TorchInductor must recompile for the new shapes. PyTorch maintains separate compiled versions for each unique shape configuration (shape specialization). For workloads with highly variable shapes, this can lead to frequent recompilation overhead.

**Graph Breaks: Causes and Detection**

Graph breaks occur when torch.compile encounters code it cannot compile, forcing execution to fall back to eager mode. Understanding graph break causes is essential for achieving good performance with torch.compile.

**Data-Dependent Control Flow**

Control flow that depends on tensor values cannot be compiled without knowing those values:
...
# Creates two compiled regions: before and after the if statement
# The if statement itself executes eagerly
```

The condition `x.sum() > 0` requires evaluating the tensor, which is not available at compile time. TorchDynamo creates a graph break: the operations before the if statement are compiled, the if statement executes eagerly (evaluating which branch to take), and the chosen branch is compiled as a separate region.

**Unsupported Operations**

Operations outside torch.compile's scope cause graph breaks:

```python
@torch.compile
def debug_compute(x):
    y = x * 2
    print(f"y = {y}")  # Graph break: print is I/O operation
    z = y + 1
    return z
...
Changing tensor shapes across iterations prevents compiled code reuse:
...
Each unique shape triggers recompilation because TorchInductor generates shape-specialized code. If a model processes variable-length sequences, padding to fixed lengths can eliminate recompilation overhead at the cost of wasted computation on padding.

**Detecting Graph Breaks**

PyTorch provides logging to identify graph breaks:

```bash
TORCH_LOGS="graph_breaks" python train.py
```

This prints each graph break location and reason:

```
Graph break in user code at file.py:15
Reason: call to unsupported function print
```

Minimizing graph breaks is key to achieving good performance. The TORCH_LOGS output guides refactoring: moving unsupported operations outside compiled regions, replacing data-dependent control flow with conditional execution (torch.where), or accepting eager execution for inherently dynamic sections.

**Compilation Modes: Trading Off Compilation Time and Performance**

torch.compile supports three compilation modes that balance compilation time against runtime performance:

**Default Mode**

```python
@torch.compile  # Equivalent to mode='default'
def forward(x):
    return x * 2 + 1
```

Default mode provides moderate optimization with fast compilation time (5-30 seconds for transformer models). It performs standard fusion and memory optimizations without extensive autotuning. This mode is suitable for development and training, where compilation overhead is amortized over many iterations but fast iteration time is valuable.
...
Reduce-overhead mode minimizes Python interpreter overhead by aggressively capturing operations into compiled regions. It also enables CUDA graphs for GPU execution, which batches kernel launches to reduce launch overhead (CUDA kernel launch overhead is ~5-10 microseconds per kernel; for models with hundreds of small kernels, this overhead becomes significant).

This mode is optimized for inference: it assumes fixed shapes, minimal graph breaks, and prioritizes minimizing per-iteration overhead. Compilation time is similar to default mode, but generated code has lower overhead. For inference servers processing many requests, reduce-overhead mode can improve throughput by 20-40% over default mode.
...
Max-autotune mode performs extensive autotuning: for operations with multiple implementation strategies, TorchInductor generates and benchmarks multiple variants to select the fastest. This includes trying different tiling strategies for matrix multiplications, evaluating fusion patterns, and selecting optimal kernel implementations.

Compilation time increases significantly (minutes to hours for large models) because each candidate implementation must be compiled and benchmarked. However, runtime performance improves by an additional 10-30% over default mode. This mode is best for production training where compilation is performed once and the compiled model trains for days or weeks.

**Backend Options: Alternative Code Generators**

While TorchInductor is the default backend, torch.compile supports multiple backends for different use cases:
...
TorchInductor generates Triton kernels for CUDA and C++ for CPU. It is optimized for PyTorch workloads and provides the best general-purpose performance for both training and inference. TorchInductor is actively developed and receives ongoing optimization improvements.
...
The ONNX Runtime backend exports the FX graph to ONNX format and executes using ONNX Runtime. This enables cross-platform deployment: ONNX models can run on CPU, GPU, mobile, and edge devices. ONNX Runtime also provides quantization and pruning optimizations. However, not all PyTorch operations are supported in ONNX, potentially causing more graph breaks.
...
The TensorRT backend compiles to NVIDIA TensorRT, an optimized inference engine for NVIDIA GPUs. TensorRT provides aggressive optimizations for inference: int8 quantization, layer fusion, kernel autotuning. For inference on NVIDIA GPUs, TensorRT often achieves 2-5× speedup over TorchInductor. However, TensorRT is inference-only (no backward pass) and requires NVIDIA GPUs.

**Practical Examples: Measuring Speedup**

To quantify torch.compile benefits, measure performance before and after compilation:
...
# Typical output:
# Eager time: 0.045s
# Compiled time: 0.018s
# Speedup: 2.50×
```

Critical details for accurate benchmarking:

1. **Use torch.cuda.synchronize()**: CUDA operations are asynchronous. Without synchronization, timing measures only kernel launch time, not execution time.
2. **Warmup compilation**: The first call compiles the function. Exclude this from timing by calling once before the timed loop.
3. **Sufficient iterations**: Run 100+ iterations to amortize measurement overhead and get stable timing.

For realistic workloads, measure end-to-end training iteration time:
...
for batch in train_loader:
    ...
    optimizer.step()
    optimizer.zero_grad()
    break  # One iteration to trigger compilation
...
print(f"Training throughput: {100 / training_time:.2f} it/s")
```
...
The first execution of compiled code includes compilation time:

- Small models: 5-10 seconds
- Transformer models (BERT-base): 30-60 seconds
- Large language models (GPT-3 scale): 5-10 minutes

This overhead is amortized across training (compile once, train for thousands of iterations) but impacts development iteration time. For rapid prototyping, defer torch.compile until model architecture stabilizes. For production training, compilation time is negligible compared to days of training.

Compiled kernels are cached on disk in `~/.triton/cache/`. Subsequent runs with the same model architecture and input shapes skip compilation and directly load cached kernels, reducing startup time to seconds.

**Memory Usage**

Compilation adds memory overhead:

- FX graph construction: 100-500 MB for transformer models
- Kernel compilation: 500 MB-2 GB peak during Triton compilation
- Compiled kernel storage: 10-100 MB per compiled graph

The memory overhead is transient during compilation and persistent for cached graphs. Runtime memory usage is similar to eager mode: kernel fusion can reduce intermediate tensor memory, but compiled code may allocate temporary buffers. Overall, compiled models typically use 90-110% of eager mode memory.

For large models near GPU memory limits, the compilation memory overhead can trigger out-of-memory errors. In this case, compile on a machine with more memory, then load cached kernels on the training machine, or reduce batch size during the first (compilation) iteration.

**Debugging Compiled Code**

Debugging becomes more difficult with compilation:

- Errors in compiled code produce stack traces pointing to generated code, not source Python code.
- Print statements inside compiled regions cause graph breaks (executed eagerly, not compiled).
- Inspection of intermediate values requires stopping compilation at that point.

For debugging, remove `@torch.compile` to revert to eager execution. Once the bug is fixed, re-enable compilation. During development, use `mode='default'` for faster compilation and more interpretable errors. Switch to `mode='max-autotune'` only for production training after thorough testing.

PyTorch provides debugging environment variables:

```bash
TORCH_COMPILE_DEBUG=1 python train.py  # Verbose compilation logs
TORCHDYNAMO_VERBOSE=1 python train.py  # Detailed graph capture logs
```

**When to Use torch.compile**

torch.compile is most beneficial when:

**Training**: Long training runs (hundreds or thousands of iterations) amortize compilation overhead. Stable model architectures with fixed control flow minimize graph breaks. Production training workloads benefit most from max-autotune mode.

**Inference**: Deployed models compile once at startup and serve thousands of requests with compiled code. Reduce-overhead mode minimizes per-request overhead. For inference servers, compilation time is negligible compared to cumulative inference time.

**Avoid torch.compile when**:
...
- Debugging: Compiled code obscures error locations. Use eager mode to identify bugs, then re-enable compilation.
...
Every tensor with `requires_grad=True` stores a `grad_fn` attribute pointing to the `Function` object that created it. These Function objects link to their input tensors, forming a backward computation graph. Consider this example:
...
This accumulation behavior explains why gradients sum across multiple `backward()` calls:
...
When implementing custom operations, you explicitly specify what to save and how to compute gradients:
...
Two ways to create tensors that don't track gradients:
...
These autograd internals enable PyTorch's flexibility: users write forward passes in natural Python, and autograd automatically constructs backward passes with correct gradients and efficient memory management. Understanding these mechanisms is crucial for debugging training issues, implementing custom operations, and optimizing memory usage in production systems.

##### Mixed-Precision Training Mechanics {#sec-ai-frameworks-mixed-precision-training}

Mixed-precision training combines lower-precision arithmetic (FP16) with higher-precision accumulation (FP32) to reduce memory usage and accelerate computation while maintaining numerical stability. Modern GPUs provide 2-8× higher throughput for FP16 operations compared to FP32, making mixed precision essential for training large models efficiently. However, naive FP16 training fails due to gradient underflow—many gradients fall below FP16's representable range (minimum ~6×10⁻⁸), becoming zero and preventing learning.

PyTorch's Automatic Mixed Precision (AMP) system solves this through two key mechanisms: the `autocast` context manager for automatic precision selection and `GradScaler` for loss scaling to prevent gradient underflow.

**autocast Context Manager**

The `autocast` context manager automatically selects appropriate precision for each operation based on numerical stability requirements. Not all operations benefit equally from reduced precision—some are sensitive to numerical errors while others are memory-bound and benefit from FP16's reduced bandwidth requirements.

@lst-autocast-usage demonstrates basic autocast usage in a training loop:

::: {#lst-autocast-usage lst-cap="**Autocast Usage**: The autocast context manager automatically casts operations to FP16 where safe, maintaining FP32 precision for numerically sensitive operations."}
```python
import torch
...
Inside the `autocast` context, PyTorch automatically applies precision rules:

- **FP16 operations**: Matrix multiplications, convolutions (memory-bound, benefit from reduced bandwidth)
- **FP32 operations**: Loss functions, softmax, layer normalization (numerically sensitive)
- **Automatic casting**: Tensors are cast as needed; if FP16 and FP32 tensors interact, FP16 is promoted to FP32

This selective precision maintains accuracy while achieving speedups. On NVIDIA A100 GPUs, FP16 matrix multiplication achieves 312 TFLOPS compared to 156 TFLOPS for FP32—a 2× throughput improvement.

**GradScaler for Loss Scaling**

Even with careful operation selection, gradients often underflow in FP16. Consider a gradient of magnitude 10⁻⁷: this rounds to zero in FP16 (minimum normal value ≈6×10⁻⁸), effectively stopping learning for those parameters.

Loss scaling solves this by multiplying the loss by a large scale factor before backward pass, shifting gradients into FP16's representable range. After computing scaled gradients, we divide by the scale factor to recover true gradient values before the optimizer step.

@lst-gradscaler-usage shows GradScaler integration:

::: {#lst-gradscaler-usage lst-cap="**GradScaler Usage**: Loss scaling prevents gradient underflow by scaling losses before backward pass, then unscaling gradients before optimizer update."}
```python
...
for inputs, targets in dataloader:
    ...
    scaler.step(
        optimizer
    )  # grads /= scale_factor, then optimizer.step()
    scaler.update()  # Adjust scale factor for next iteration
```
:::

**Why Loss Scaling Prevents Underflow**

Loss scaling shifts gradient magnitudes into FP16's representable range through multiplication:

$$
\text{scaled\_loss} = \text{loss} \times s
$$

where $s$ is the scale factor (typically 2¹⁶ = 65536 initially). During backward pass:

$$
\frac{\partial (\text{scaled\_loss})}{\partial w} = s \cdot \frac{\partial \text{loss}}{\partial w}
$$

For a gradient of magnitude 10⁻⁷:

- **Without scaling**: 10⁻⁷ → 0 in FP16 (underflow)
- **With scaling** (s=65536): 65536 × 10⁻⁷ = 6.5×10⁻³ (representable in FP16)

After backward pass, GradScaler divides gradients by $s$ before the optimizer step, recovering the true gradient values in FP32 for the parameter update.

**Dynamic Loss Scaling Algorithm**

Static scaling factors fail when gradients vary across training. Too large a scale causes overflow (gradients exceed FP16's maximum ~65504), while too small a scale still permits underflow. Dynamic loss scaling adapts the scale factor automatically.

The algorithm maintains a scale factor $s$ and adjusts it based on gradient overflow detection:

1. **Overflow check**: After backward pass, check if any gradients are inf or NaN (indicating overflow)
2. **Overflow response**: If overflow detected:
   - Skip optimizer step (gradients are invalid)
   - Reduce scale factor: $s \leftarrow s / 2$
   - Reset counter
3. **No overflow**: If no overflow for $N$ consecutive iterations (typically $N=2000$):
   - Increase scale factor: $s \leftarrow s \times 2$
   - Maintains aggressive scaling without causing frequent overflows

@lst-dynamic-scaling shows the conceptual implementation:

::: {#lst-dynamic-scaling lst-cap="**Dynamic Loss Scaling**: Scale factor adapts based on gradient overflow, maximizing precision utilization while preventing numerical errors."}
```python
class GradScaler:
    def __init__(self):
        self.scale = 2**16  # Initial scale factor
        self.growth_interval = 2000
        self.no_overflow_iters = 0

    def step(self, optimizer):
        # Unscale gradients (divide by scale factor)
        for param in optimizer.param_groups[0]["params"]:
            if param.grad is not None:
                param.grad.div_(self.scale)

        # Check for inf/NaN (overflow indicator)
        found_inf = any(
        ...
        if found_inf:
            # Overflow: reduce scale, skip update
            self.scale /= 2.0
            self.no_overflow_iters = 0
        else:
            # No overflow: proceed with update
            optimizer.step()
            self.no_overflow_iters += 1

    def update(self):
        # Increase scale after sustained success
        if self.no_overflow_iters >= self.growth_interval:
            self.scale *= 2.0
            self.no_overflow_iters = 0
```
:::
...
Mixed precision reduces memory consumption across multiple components:

1. **Activation storage**: FP16 activations require half the memory of FP32 (2 bytes vs 4 bytes per element)
2. **Gradient storage**: Gradients computed in FP16 before unscaling
3. **Master weights**: Optimizer maintains FP32 master copy of weights for accurate updates

Total memory savings typically range from 25-50% depending on model architecture. For a model with $P$ parameters and batch size $B$ with $L$ layers:

- **FP32 training**: $4P$ (weights) + $4P$ (gradients) + $4P$ (optimizer state) + $4BL$ (activations) ≈ $12P + 4BL$ bytes
- **Mixed precision**: $2P$ (FP16 weights) + $4P$ (FP32 master) + $4P$ (optimizer state) + $2BL$ (FP16 activations) ≈ $10P + 2BL$ bytes

For large models where activations dominate (large $B$ and $L$), the $4BL \rightarrow 2BL$ reduction provides substantial savings, often enabling 2× larger batch sizes.

**Numerical Stability Considerations**

Mixed precision succeeds because neural network training has inherent noise tolerance. Gradient descent convergence depends on gradient direction more than exact magnitude, and the combination of FP16 computation with FP32 accumulation preserves sufficient precision for learning.

However, certain operations require special handling:

1. **Large reductions**: Summing many FP16 values accumulates rounding error. Use FP32 accumulation for operations like `torch.sum()` over large tensors.
2. **Exponentials**: Softmax and layer normalization involve $e^x$, which can overflow in FP16 for moderate $x$ values. These operations automatically use FP32 under autocast.
3. **Small learning rates**: For learning rates below 10⁻⁵, FP32 optimizer updates are essential to prevent weight staleness (updates smaller than FP16 representable increments).

Modern frameworks handle these cases automatically through autocast's operation whitelist/blacklist, ensuring that numerically sensitive operations maintain FP32 precision while memory-bound operations benefit from FP16's bandwidth advantages.

##### Optimizer State Mechanics {#sec-ai-frameworks-optimizer-state-mechanics}

While gradients capture how to adjust parameters, optimizers maintain additional state to improve training convergence. Understanding optimizer state structure is essential for checkpointing, memory optimization, and debugging training resumption issues.

**State Dictionary Structure**

PyTorch optimizers expose their internal state through the `state_dict()` method, which returns a dictionary containing all information needed to resume training:
...
# Initial state is empty (no steps taken yet)
print(optimizer.state_dict())
# {'state': {}, 'param_groups': [{'lr': 0.001, 'betas': (0.9, 0.999), ...}]}
...
# Now state contains momentum buffers
state = optimizer.state_dict()
print(state.keys())  # dict_keys(['state', 'param_groups'])
```

The state dictionary has two main components:

1. **`state`**: Maps parameter IDs to optimizer-specific state tensors
2. **`param_groups`**: Lists parameter groups with their hyperparameters (learning rate, momentum, weight decay)

**Momentum Buffer Storage**

SGD with momentum maintains a velocity buffer for each parameter. For parameter $\theta$ with gradient $g_t$, momentum updates the velocity $v_t$ and parameter:
...
where $\mu$ is the momentum coefficient (typically 0.9) and $\eta$ is the learning rate.
...
# Train for one step
output = model(torch.randn(8, 100))
loss = output.sum()
loss.backward()
optimizer.step()

# Inspect momentum buffers
state = optimizer.state_dict()
for param_id, param_state in state["state"].items():
    print(f"Parameter {param_id}:")
    print(
        f"  Momentum buffer shape: {param_state['momentum_buffer'].shape}"
    )
    # For Linear(100, 10): weight [10, 100], bias [10]
```

Each momentum buffer has the same shape as its corresponding parameter. For a model with $P$ parameters using FP32:

- Parameters: $P \times 4$ bytes
- Gradients: $P \times 4$ bytes (during backward)
- Momentum buffers: $P \times 4$ bytes
- **Total: 12 bytes per parameter**

**Adam State: First and Second Moments**

Adam maintains two exponential moving averages per parameter: first moment $m_t$ (momentum) and second moment $v_t$ (variance). The update rule is:
...
where $\beta_1 = 0.9$, $\beta_2 = 0.999$, and $\epsilon = 10^{-8}$ are default values.

```python
model = nn.Linear(100, 10)  # 1,010 parameters total
optimizer = optim.Adam(model.parameters(), lr=0.001)
...
for param_id, param_state in state["state"].items():
    print(f"Parameter {param_id}:")
    print(
        f"  Step: {param_state['step']}"
    )  # Iteration count for bias correction
    print(
        f"  exp_avg shape: {param_state['exp_avg'].shape}"
    )  # m_t (first moment)
    print(
        f"  exp_avg_sq shape: {param_state['exp_avg_sq'].shape}"
    )  # v_t (second moment)
```

Each parameter requires storing two state tensors (`exp_avg` and `exp_avg_sq`), both the same shape as the parameter. Memory cost for $P$ parameters in FP32:
...
For a 7B parameter model (GPT-style):

- Parameters: 28 GB
- Adam state: 56 GB (2× parameters)
- **Total optimizer state: 56 GB**

This explains why training large models requires substantially more memory than inference.

**Loading Optimizer State for Training Resumption**

Resuming training requires loading both model parameters and optimizer state to continue from the exact training state:
...
# Training continues with momentum buffers intact
for epoch in range(start_epoch, num_epochs):
    # ... training loop ...
```

**Critical detail**: The optimizer must be created with the same parameter references as when the state was saved. Loading parameters into a different model instance and then loading optimizer state will fail:

```python
# INCORRECT: Optimizer loses parameter references
model = MyModel()
...
# New model instance (different parameter tensors)
new_model = MyModel()
new_model.load_state_dict(torch.load("ckpt.pt")["model"])
optimizer = optim.Adam(new_model.parameters())
optimizer.load_state_dict(
    torch.load("ckpt.pt")["opt"]
)  # May fail or load incorrectly!

# CORRECT: Load parameters first, then create optimizer
new_model = MyModel()
new_model.load_state_dict(torch.load("ckpt.pt")["model"])
optimizer = optim.Adam(new_model.parameters())
optimizer.load_state_dict(
...
**Memory Implications and Optimization Strategies**

Optimizer state dominates memory consumption for large models. Consider a Llama-7B model:

- Model parameters: 7B × 4 bytes = 28 GB
- Adam optimizer state: 7B × 8 bytes = 56 GB (first + second moments)
- Gradients: 7B × 4 bytes = 28 GB
- Activations (batch 8, seq 2048): ~20 GB
- **Total: 132 GB for single-GPU training**

This exceeds most consumer GPU memory (24 GB for RTX 4090, 80 GB for A100). Memory reduction strategies:

1. **8-bit optimizers** (e.g., `bitsandbytes`): Store moments in INT8 with block-wise quantization
   - Reduces optimizer state from 56 GB to 14 GB (4× reduction)
   - Minimal impact on convergence

2. **SGD instead of Adam**: Reduces optimizer state from 2× to 1× parameter count
   - May require more careful tuning (learning rate, momentum)
   - Often sufficient for fine-tuning pre-trained models

3. **Gradient accumulation**: Reduce batch size (smaller activations), accumulate gradients over multiple micro-batches
   - Does not reduce optimizer state, but reduces activation memory

4. **ZeRO optimizer** (Volume II, distributed training): Shard optimizer state across GPUs
   - 8 GPUs: Each stores 1/8 of optimizer state (7 GB instead of 56 GB per GPU)

Understanding optimizer state structure enables effective checkpointing, memory optimization, and debugging of training resumption issues. The memory overhead of optimizer state becomes the dominant factor in training large models, motivating distributed optimization strategies covered in Volume II.
...
#### GPU Memory Management: PyTorch's CUDA Caching Allocator {#sec-ai-frameworks-gpu-memory-management}

Understanding PyTorch's GPU memory management is essential for debugging out-of-memory (OOM) errors, optimizing batch sizes, and efficiently utilizing GPU resources during training and inference. PyTorch implements a sophisticated caching allocator that trades memory space for allocation speed, but this design introduces complexity that every practitioner must understand.

**The Problem: Naive GPU Memory Allocation**

A naive approach would call `cudaMalloc()` for each tensor allocation and `cudaFree()` when tensors are no longer needed. This approach is prohibitively slow because CUDA memory allocation synchronizes the GPU, taking milliseconds per call. Training a neural network allocates and frees thousands of tensors per iteration, making naive allocation a performance bottleneck.

Consider a simple training iteration:
```python
# Naive approach (hypothetical, not actual PyTorch)
forward_tensors = []  # ~100 tensors per forward pass
for layer in model:
    activation = layer(input)  # cudaMalloc() - milliseconds!
    forward_tensors.append(activation)

loss.backward()  # cudaFree() for each tensor - more milliseconds!
```

At 1ms per allocation and 100 tensors, a single iteration would waste 100ms just on memory management before any computation begins.

**The Solution: Caching Allocator**

PyTorch maintains a memory pool (cache) to avoid repeated CUDA allocation calls:

1. **Initial allocation**: `cudaMalloc()` allocates large blocks from GPU memory
2. **Subdivision**: Large blocks are subdivided to satisfy tensor allocation requests
3. **Deallocation**: When tensors are freed (reference count reaches zero), memory returns to the pool rather than calling `cudaFree()`
4. **Reuse**: New tensor allocations reuse cached memory from the pool

This design achieves microsecond-scale allocation by caching, providing a 1000× speedup compared to naive allocation.

**Allocated vs. Reserved Memory**

PyTorch distinguishes between two memory quantities:

**Allocated memory**: Memory actively in use by tensors
```python
allocated = torch.cuda.memory_allocated()  # e.g., 1.5 GB
```

**Reserved memory**: Total memory PyTorch has obtained from CUDA (includes cached free blocks)
```python
reserved = torch.cuda.memory_reserved()  # e.g., 4.0 GB
```

The gap between reserved and allocated represents cached free memory that PyTorch holds for future allocations:

```python
cached_free = reserved - allocated  # e.g., 2.5 GB
```

PyTorch does not return cached memory to the operating system automatically. This behavior optimizes for training workloads where memory usage patterns repeat across iterations.

**Memory Fragmentation: The Hidden OOM Cause**

Fragmentation occurs when free memory exists but not in a contiguous block large enough for the requested allocation. This can cause OOM errors even when substantial memory appears available:

```
Reserved:    4.0 GB
Allocated:   2.0 GB
Free total:  2.0 GB
Largest free block: 512 MB  ← Problem!
```

Attempting to allocate a 1 GB tensor fails despite 2 GB of free memory because no single contiguous block is large enough.

**Fragmentation Example**

```python
# Iteration 1: Allocate three tensors
a = torch.randn(256, 1024, 1024)  # 1 GB
b = torch.randn(128, 1024, 1024)  # 512 MB
c = torch.randn(256, 1024, 1024)  # 1 GB

# Memory layout: [1GB-a] [512MB-b] [1GB-c]

# Iteration 2: Free middle tensor
del b

# Memory layout: [1GB-a] [512MB-FREE] [1GB-c]

# Iteration 3: Try to allocate 1GB
d = torch.randn(256, 1024, 1024)  # OOM!
# 512MB hole cannot accommodate 1GB tensor
```

The 512 MB free block remains unusable for larger allocations, creating fragmentation.

**Common Solutions to OOM Errors**

**1. torch.cuda.empty_cache()**

Returns cached memory to CUDA, making it available to other processes:

```python
torch.cuda.empty_cache()
```

**Important**: This does NOT free allocated tensors. It only returns cached free blocks. If tensors are actively in use (allocated), `empty_cache()` has no effect on available memory for your training loop.

```python
# Misconception
allocated = torch.cuda.memory_allocated()  # 8 GB in use
torch.cuda.empty_cache()
# allocated still 8 GB! No change.
```

**2. Reduce Batch Size**

The most effective solution for training OOM is reducing batch size:

```python
# Before: OOM at batch size 64
batch_size = 64
loss = model(data[:batch_size])
loss.backward()

# After: Success with batch size 32
batch_size = 32
loss = model(data[:batch_size])
loss.backward()
```

Memory usage is roughly proportional to batch size because activations scale linearly with batch dimension.

**3. Gradient Accumulation**

Maintain effective batch size while reducing peak memory:

```python
# Effective batch size 64, physical batch size 16
optimizer.zero_grad()
for micro_batch in range(4):  # 4 × 16 = 64
    data_slice = data[micro_batch * 16 : (micro_batch + 1) * 16]
    loss = model(data_slice) / 4  # Scale loss
    loss.backward()  # Accumulate gradients

optimizer.step()  # Update with accumulated gradients
```

This approach uses 1/4 the peak memory (activations for 16 samples, not 64) while maintaining the same effective batch size for optimizer updates.
...
- Active allocations by size
- Cached memory by block size
...
The snapshot can be visualized with PyTorch's memory visualization tools to identify allocation patterns causing OOM.

**Allocator Configuration**

Environment variable `PYTORCH_CUDA_ALLOC_CONF` controls allocator behavior:

```bash
# Smaller block sizes (reduces fragmentation, increases overhead)
export PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:128

# Disable caching (slow but helps debug memory issues)
export PYTORCH_CUDA_ALLOC_CONF=backend:native

# Round up allocations to power-of-2 (reduces fragmentation)
export PYTORCH_CUDA_ALLOC_CONF=roundup_power2_divisions:4
```

**Systems Implications for Batch Size Selection**

Total memory consumption during training:

$$\text{Memory} = \text{Model Parameters} + \text{Batch Size} \times \text{Activations} + \text{Optimizer State}$$

For a ResNet-50 with Adam optimizer:

- Model parameters: ~90 MB (25M params × 4 bytes)
- Optimizer state: ~360 MB (2 moments × 4 bytes × 25M params)
- **Activations**: Batch-size dependent

For 224×224 RGB images:

- Batch size 32: ~3.5 GB activations
- Batch size 64: ~7.0 GB activations (roughly linear scaling)

**Finding maximum batch size**: Binary search until OOM, then reduce by 20% for safety margin.

**Gradient Checkpointing Trade-offs**

Activation checkpointing (gradient checkpointing) trades computation for memory:
...
def forward(x):
    x = checkpoint(layer1, x)  # Don't save activations
    x = checkpoint(layer2, x)  # Recompute during backward
...
- Memory reduction: 40-60% (fewer saved activations)
- Training time increase: 20-30% (recomputation during backward)
- When to use: When batch size is memory-limited but training time is acceptable

For a ResNet-50:

- Without checkpointing: 7 GB, 100 ms/iteration
- With checkpointing: 3 GB, 130 ms/iteration (30% slower, 57% less memory)

**Practical Guidelines**

1. **Monitor both allocated and reserved memory**: Large gaps indicate cached memory that could be freed if needed
2. **Profile before optimizing**: Use memory summary to understand actual allocation patterns
3. **Prefer gradient accumulation over batch size reduction**: Maintains effective batch size for optimizer convergence
4. **Gradient checkpointing as last resort**: Significant performance cost
5. **Don't call empty_cache() in training loops**: It's slow and usually ineffective since memory is allocated, not just cached

Understanding PyTorch's CUDA allocator transforms OOM errors from mysterious failures into debuggable memory management issues with clear solutions.
...
The `.to()` method implements copy-on-write semantics: if the tensor is already on the target device, `.to()` returns the original tensor without creating a copy. This behavior prevents unnecessary memory allocation and enables idempotent device placement code:
...
This design allows defensive programming where calling `.to(device)` multiple times is safe and efficient. However, the behavior changes when converting types simultaneously:

```python
x = torch.randn(100, device="cuda", dtype=torch.float32)
y = x.to(
    device="cuda", dtype=torch.float16
)  # Creates copy due to dtype change
...
Context managers provide scoped device changes that automatically restore the previous device:
...
| NVLink 3.0 | 600 GB/s | 0.007 ms | Comparable to GPU compute |
| GPU Memory | 2000 GB/s | 0.002 ms | Optimal |

: Transfer overhead for 4 MB tensor across different interconnects. PCIe transfers are significantly slower than on-device memory access, making device placement critical for performance. {#tbl-device-transfer-overhead}
...
Understanding nn.Module mechanics enables effective model construction, debugging, and optimization. The automatic parameter registration simplifies optimizer setup, state dictionaries provide checkpoint management, hooks enable gradient inspection, and hierarchical composition supports modular architecture design. These mechanisms form the foundation for practical PyTorch development, bridging the gap between high-level model definitions and framework internals.
