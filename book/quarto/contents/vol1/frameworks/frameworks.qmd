...
[^fn-flashattention]: **FlashAttention**: Memory-efficient attention algorithm by Dao et al. (2022) that reduces attention's memory complexity from O(N^2) to O(N) by computing attention in tiles that fit in GPU SRAM, avoiding materialization of the full N x N attention matrix in HBM. Achieves 2-4x speedup and enables training with longer context lengths by reducing memory traffic between HBM (1.5 TB/s on A100) and SRAM (19 TB/s), demonstrating how algorithmic redesign can overcome the memory wall.
...
[^fn-triton]: **Triton**: Open-source GPU programming language developed by OpenAI that enables writing custom kernels in Python-like syntax without low-level CUDA knowledge. Triton compiles to PTX (NVIDIA's intermediate assembly) and handles memory coalescing, shared memory management, and thread synchronization automatically. Achieves 80-95% of hand-tuned CUDA performance while reducing kernel development time from weeks to hours, making custom GPU optimization accessible to ML researchers.
...
[^fn-hbm]: **HBM (High Bandwidth Memory)**: 3D-stacked DRAM technology providing 2-3 TB/s bandwidth on modern GPUs (A100: 2 TB/s, H100: 3.35 TB/s) compared to 50-100 GB/s for standard DDR memory. HBM stacks multiple DRAM dies vertically with through-silicon vias (TSVs), enabling thousands of parallel data paths. The 80GB HBM2e on an A100 is the primary bottleneck for large model training: model weights, activations, and gradients must fit in HBM, making memory capacity and bandwidth the practical limits on model size.
