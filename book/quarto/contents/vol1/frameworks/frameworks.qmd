...
Machine learning frameworks build on decades of foundational work in computational libraries. @fig-mlfm-timeline traces this evolution from BLAS (1979) through modern deep learning frameworks, revealing a consistent pattern: each generation abstracts the complexity of its predecessor while enabling new capabilities. BLAS and LAPACK established mathematical foundations that enabled user-friendly tools like NumPy and SciPy, which set the stage for today's deep learning frameworks.
...
This pattern demonstrates single-node pipeline parallelism, where different model stages on GPUs within the same machine process different microbatches concurrently. Extending this pattern across multiple machines requires the distributed training techniques covered in @sec-distributed-training (Volume II), but the single-node implementation (see @lst-pipeline-parallelism-streams) illustrates the core synchronization principles:
...
::: {#fig-3d-parallelism fig-env="figure" fig-pos="htb" fig-cap="**3D Parallelism**: Training can be parallelized across multiple dimensions including data batches, pipeline stages, and model partitions. This figure illustrates how large-scale training systems distribute computation across a grid of accelerators. These distributed training strategies are covered in @sec-distributed-training (Volume II)."}
```{.tikz}
