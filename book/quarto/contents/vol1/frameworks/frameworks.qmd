...
To ground this abstraction, examine @fig-comp-graph: computing $z = f(x, y)$ maps onto two input nodes ($x$ and $y$), one operation node ($f$), and one output node ($z$). The execution problem turns on *when* this graph is constructed and *when* it is executed.
...
::: {#fig-python-tax fig-env="figure" fig-pos="htb" fig-cap="**The Python Tax**: A schematic execution timeline contrasts repeated host dispatch and short GPU kernels in eager execution with one dispatch and one fused compute block after compilation." fig-alt="Gantt chart of execution timeline. Eager mode shows alternating red Python dispatch and blue GPU kernel blocks. Compiled mode shows one red dispatch block followed by one fused blue compute block."}
