...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: [^fn-blas] footnote + [^fn-gemm] footnote in Ladder of Abstraction;
...
# │       sparsity" — in [^fn-blas] footnote and Device Bandwidth Hierarchy prose.
...
Two lines of code: `model = Transformer(...)` followed by `loss.backward()`. Between them, invisible to the programmer, the framework orchestrates billions of floating-point operations across memory hierarchies, computes exact gradients through millions of parameters using **automatic differentiation** (the systematic application of the chain rule[^fn-chain-rule-autodiff] to compute derivatives), schedules thousands of GPU kernel launches, and manages gigabytes of intermediate state. The simplicity is an illusion. Those two lines trigger machinery as complex as a compiler, because that is exactly what a modern ML framework is.

[^fn-chain-rule-autodiff]: **Automatic Differentiation**: The `loss.backward()` command triggers a mechanical application of the chain rule, traversing the underlying computational graph in reverse from the final loss back to every parameter. This guarantees mathematically exact gradients but at a steep cost: every intermediate value from the forward pass must be stored in memory for this backward pass. This is the direct cause of the "gigabytes of intermediate state" a framework must manage, a footprint that can easily exceed the size of the model parameters themselves. \index{Chain Rule!automatic differentiation}
...
[^fn-computational-graph-frameworks]: **Computational Graph**: By building a graph of operations *before* executing them, the framework gains a global view of the entire program, an idea pioneered by Theano. This enables optimizations like operator fusion that are impossible in eager mode, but it complicates debugging by separating an error's runtime location from its source code definition. The trade-off is often justified by performance: fusing just two operations can reduce kernel launch overhead and memory access costs by over 50%. \index{Computational Graph!framework optimization}

[^fn-eager-execution-tradeoff]: **Eager Execution**\index{Eager Execution!optimization trade-off}: Also called "define-by-run," eager execution runs each operation immediately, producing concrete results debuggable with standard Python tools. The trade-off is fundamental: because the framework sees only one operation at a time, it cannot fuse adjacent kernels or pre-plan memory, sacrificing the $D_{vol}$ and $L_{lat}$ reductions that graph-level optimization provides. Chainer (2015) and PyTorch (2016) popularized this mode; hybrid approaches like `torch.compile` attempt to recover graph-level optimization while preserving the eager programming model.
...
[^fn-gemm-utilization]: **GEMM (General Matrix Multiply)**: The specific BLAS routine that executes the `C = A @ B` operation. The claim of running at "near-peak silicon speed" is non-trivial; it depends on the GEMM implementation correctly aligning matrix dimensions and memory access patterns to the underlying hardware, such as the fixed 8x8 blocks of a GPU's Tensor Core. This alignment is what separates a naive implementation achieving 30% of theoretical hardware performance from an optimized one exceeding 90%. \index{GEMM!hardware utilization}

[^fn-lapack-algebra]: **LAPACK (Linear Algebra PACKage)**: Released in 1992, LAPACK provides a standard API for high-level linear algebra routines by calling hardware-vendor-tuned BLAS primitives. This ensures a matrix multiplication (`C = A @ B`) is not re-implemented in every framework, but is executed through a common, optimized path to the silicon. On modern CPUs and GPUs, these standardized calls can achieve over 90% of the theoretical peak floating-point performance. \index{LAPACK!ML initialization}

[^fn-blas-performance]: **BLAS (Basic Linear Algebra Subprograms)**: A specification published in 1979 defining portable APIs across three levels: vector (L1), matrix-vector (L2), and matrix-matrix (L3, including GEMM). Hardware vendors implement optimized BLAS libraries: NVIDIA cuBLAS uses Tensor Cores for up to `{python} A100BLAS.dense_tflops_str` TFLOPS (FP16/BF16/TF32) on A100 GPUs, or `{python} A100BLAS.sparse_tflops_str` TFLOPS with structured sparsity. This 45-year-old interface remains the performance foundation of every modern ML framework, because every framework ultimately calls vendor BLAS to reach $R_{peak}$. \index{BLAS!performance foundation}
...
[^fn-numpy-abstraction]: **NumPy (Numerical Python)**: NumPy established the core "vectorization" contract by using Python's C API to delegate array operations to pre-compiled, optimized C and Fortran libraries (BLAS). This allows developers to write in a high-level language while executing at low-level speeds, but requires them to structure logic as whole-array operations. The performance gain is not trivial; a single vectorized NumPy function can execute 10-100x faster than an equivalent native Python `for` loop. \index{NumPy!framework lineage}

3.  **Solving Differentiation (2015–present)**: **Deep Learning Frameworks** (Theano, TensorFlow, PyTorch) solved the problem of *Gradient Computation*. While NumPy required manual derivation of backpropagation gradients (error-prone and slow), these frameworks introduced **Automatic Differentiation** via the computational graph. This turned the chain rule into a software primitive, allowing researchers to define *forward* passes and get *backward* passes for free.
...
[^fn-flashattention-fusion-fw]: **FlashAttention**: Fuses the entire attention operation by using tiling to avoid materializing the large N x N attention matrix in HBM. This direct-to-SRAM computation is what reduces HBM traffic by 10-20x, turning a memory-bound operation into a compute-bound one and yielding a 2-4x wall-clock speedup. \index{FlashAttention!kernel fusion}
...
Kernel fusion is the key optimization for memory-bound operations, but fusion requires seeing multiple operations together. How do frameworks represent computation in a way that makes this visibility possible? The answer is the **computational graph**\index{Computational Graph!definition (DAG)}, a directed acyclic graph (DAG)[^fn-dag-execution] where nodes represent operations and edges represent data dependencies. This graph is the framework's internal model of the computation.

[^fn-dag-execution]: **DAG (Directed Acyclic Graph)**: The "directed" property ensures one-way data flow defining a valid execution order; the "acyclic" property guarantees termination. For ML frameworks, the DAG is not merely a data structure but an optimization contract: topological sorting reveals which operations can execute in parallel on different GPU streams, dependency analysis identifies fusion candidates that share data, and memory lifetime analysis determines exactly when intermediate tensors can be freed. Without the DAG, frameworks cannot reduce $D_{vol}$ (no fusion), cannot overlap operations (no parallelism analysis), and cannot pre-plan memory (no lifetime analysis). \index{DAG!framework optimization}
...
For gradient computation, the framework records a history of operations in what's called an **autograd tape**\index{Autograd Tape!dynamic graph construction}[^fn-autograd-tape-memory], a transient data structure built during execution. Each tensor operation creates a node that records: the operation performed, references to input tensors, and how to compute gradients. These nodes form a directed acyclic graph (DAG) of operations built **during** forward pass execution, not before.

[^fn-autograd-tape-memory]: **Autograd Tape**: The tape is built *during* the forward pass to save the specific intermediate values needed to apply the chain rule on the backward pass, explaining its dynamic nature. The memory cost of this tape scales with network depth and for deep models can consume more memory than the model weights themselves. This memory pressure is the direct reason techniques like activation checkpointing—which trade recomputation for memory—are critical for training large models. \index{Autograd Tape!memory cost}
...
**The Systems Lesson**: Frameworks are not just libraries; they are graph construction engines. In-place operations violate the immutability required for automatic differentiation. When you write `x += 1`, you are not just adding numbers; you are sabotaging the graph's history [@paszke2019pytorch].
:::
...
[^fn-dce-graph-optimization]: **Dead Code Elimination (DCE)**: This optimization is only possible because the framework has a global view of the computation graph, allowing it to prove that no downstream operation depends on a candidate node's output. It commonly prunes debugging operations left in production code or, more significantly, removes the entire backward pass for frozen layers during fine-tuning. For large transformer models, DCE typically eliminates 5–15% of graph nodes, reducing both operational cost ($O$) and kernel launch latency ($L_{lat}$). \index{Dead Code Elimination!graph optimization}

These optimizations map directly to **Iron Law** terms: kernel fusion reduces $D_{vol}$ by eliminating intermediate memory writes, constant folding reduces $O$ by computing values once, memory pre-allocation reduces $L_{lat}$ by avoiding runtime allocation overhead, and dead code elimination reduces both $O$ and $D_{vol}$. Concretely, in large Transformer models, constant folding and dead code elimination can reduce total FLOPs by `{python} graph_flop_reduction_str` before the first batch even arrives.
...
**The Conclusion:**
Compilation is not magic; it is **overhead amortization**. For small, element-wise operations (like LayerNorm, GELU, Add), overhead often exceeds compute time by 10–100$\times$. Fusing them is the only way to utilize the hardware.
:::
...
[^fn-torch-compile-hybrid]: **`torch.compile`**: PyTorch 2.0's (March 2023) answer to the eager-vs-graph trade-off. Rather than requiring code rewrites for a static graph system, `torch.compile` uses Python bytecode interception (TorchDynamo) to extract computational graphs from existing eager code and compile them into fused kernels. Typical speedups range from 1.3--2$\times$ for transformer models, with compilation overhead (5--30 seconds) amortized via on-disk caching. The single-line invocation `model = torch.compile(model)` recovers graph-level $D_{vol}$ and $L_{lat}$ reductions that eager execution forfeits, without sacrificing debuggability during development. \index{torch.compile!hybrid execution}
...
[^fn-torchinductor-codegen]: **TorchInductor**: The code generation backend for `torch.compile` (PyTorch 2.0). TorchInductor receives an FX graph intermediate representation, applies fusion and layout optimization passes, and emits target-specific code: Triton kernels for GPUs, C++ with AVX vectorization for CPUs. The decision to generate Triton rather than raw CUDA trades peak performance (80--95% of hand-tuned CUDA) for dramatically faster code generation, reducing the compilation barrier that historically prevented eager frameworks from competing with static graph systems on $\eta$. \index{TorchInductor!code generation}

[^fn-ptx-portability]: **PTX (Parallel Thread Execution)**: NVIDIA's intermediate assembly language, occupying the same role for GPU computing that LLVM IR occupies for CPUs: a portable intermediate representation abstracting hardware-specific details. Both Triton kernels and CUDA code compile to PTX, which the driver then compiles to architecture-specific machine code (SASS). This indirection enables forward compatibility (code for one GPU generation runs on newer ones) at the cost of potentially suboptimal performance. For framework developers, PTX provides a stable compilation target that shields code generation from the rapid pace of GPU architecture evolution. \index{PTX!compilation portability}

[^fn-triton-kernel-language]: **Triton**: OpenAI's GPU programming language (2021) enabling custom kernels in Python-like syntax without low-level CUDA knowledge. Triton operates at the block level rather than the thread level, handling memory coalescing and thread synchronization automatically while compiling to PTX. The trade-off is deliberate: 80--95% of hand-tuned CUDA performance in exchange for reducing kernel development time from weeks to hours, which is why TorchInductor generates Triton rather than raw CUDA as its GPU backend. \index{Triton!performance trade-off}
...
*   **Conclusion**: You spend `{python} DispatchTax.dispatch_overhead_pct_str`% of time waiting for Python. Compilation yields **`{python} DispatchTax.dispatch_compilation_speedup_str`$\times$ speedup**.
...
[^fn-tinyml-aot-extreme]: **TinyML**: Machine learning inference on microcontrollers with power budgets under 1 milliwatt and memory measured in kilobytes. TinyML frameworks (TensorFlow Lite Micro, PyTorch ExecuTorch) compile models into bare-metal C/C++ with no dynamic memory allocation, no Python interpreter, and no runtime graph construction. This represents the extreme AOT endpoint of the compilation continuum: every byte of memory is statically planned at compile time because a single `malloc()` failure on a device with 256 KB of RAM is unrecoverable. See @sec-model-compression for the model optimization techniques that make this possible. \index{TinyML!AOT compilation}
...
[^fn-gradient-descent-memory]: **Gradient**: The vector of partial derivatives $\nabla_\theta \mathcal{L}$, pointing in the direction of steepest ascent of the loss landscape. Training descends this slope, hence "gradient descent." From a systems perspective, gradients are the primary memory consumer beyond model weights: each parameter requires a corresponding gradient tensor of equal size, and optimizers like Adam store two additional moment vectors per parameter, resulting in 3--4$\times$ the memory of the weights alone. \index{Gradient!memory cost}

[^fn-autodiff-reverse-mode]: **Automatic Differentiation (AD)**: Reverse-mode applies the chain rule backwards from a single output (the loss) to all inputs (the parameters). This structure is what enables the $O(1)$ backward pass efficiency, unlike the $O(N)$ passes required by forward-mode for $N$ parameters. The trade-off for this efficiency is memory, as all intermediate values from the forward pass must be stored for the backward computation, a cost that scales directly with model depth. \index{Automatic Differentiation!reverse mode}

Building on the backpropagation[^fn-backprop-memory-cost-fw] algorithm introduced in @sec-neural-computation (where we established that gradients flow backward through the computation graph via the chain rule),

[^fn-backprop-memory-cost-fw]: **Backpropagation**: Gradients are "propagated backward" from the output loss through each layer to the input, independently discovered by Werbos (1974), LeCun (1985), and popularized by Rumelhart, Hinton, and Williams (1986). From a systems perspective, backpropagation's key cost is memory: the forward pass must store all intermediate activations so the backward pass can compute local gradients. For a transformer with $N_L$ layers and hidden dimension $d$, this requires $O(N_L \cdot d \cdot N)$ memory for batch size $N$, often exceeding the model weights themselves. This memory pressure is why training requires 3--4$\times$ more memory than inference and motivates activation checkpointing (@sec-model-training). \index{Backpropagation!memory cost}
...
[^fn-gradient-accumulation-batch]: **Gradient Accumulation**: Simulates larger effective batch sizes by summing gradients over multiple mini-batches before updating parameters. This trades compute time (multiple forward-backward passes) for memory savings (only one mini-batch of activations in memory at a time), enabling batch sizes that would otherwise exceed accelerator memory. Covered in detail in @sec-model-training. \index{Gradient Accumulation!memory trade-off}
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
gpt3_params_b_str = GPT3MemoryFootprint.gpt3_params_b_str
gpt3_fp16_gb_str = GPT3MemoryFootprint.gpt3_fp16_gb_str
```
...
[^fn-operation-fusion-bandwidth]: **Operation Fusion**: Compiler optimization combining multiple sequential operations into a single kernel, eliminating intermediate memory writes between them. The speedup (2--3$\times$ for typical matmul-bias-ReLU sequences) comes primarily from reducing $D_{vol}$: unfused, each operation reads from and writes to HBM; fused, intermediate values stay in registers or L1 cache, which are 10--100$\times$ faster. Fusion is the single most impactful optimization a framework compiler can perform for memory-bound operations. \index{Operation Fusion!bandwidth reduction}
...
[^fn-jax-functional-transforms]: **JAX**: Google Research's framework (December 2018) providing composable *function transformations* rather than a neural network library: `grad` (differentiation), `jit` (compilation via XLA), `vmap` (vectorization), and `pmap` (parallelization). The functional purity requirement (no side effects, no mutation) constrains programming style but enables the framework to reason mathematically about program transformations, composing `jit(grad(vmap(f)))` into a single optimized kernel. This approach achieves strong $\eta$ on TPUs through XLA compilation and directly inspired PyTorch's `torch.compile` and `torch.vmap`. \index{JAX!composable transformations}
...
***Tensors***\index{Tensor!definition} are the fundamental unit of **Data Parallelism**—a multi-dimensional array abstraction that maps mathematical operations onto vectorized hardware.
...
[^fn-tensor-hardware-contract]: **Tensor**: From Latin "tendere" (to stretch), originally formalizing stress distributions in elastic materials (Ricci and Levi-Civita, 1900). In ML, a tensor is not merely an n-dimensional array but a hardware contract: its contiguous memory layout, stride metadata, and dtype guarantee that data can be dispatched to Tensor Cores, SIMD units, or DMA engines without CPU-side reformatting. This contract is why changing a tensor's layout (e.g., NCHW to NHWC) can yield 2$\times$ throughput differences on the same hardware for the same arithmetic. \index{Tensor!hardware contract}
...
4.  **Activations**: For a batch size of `{python} admin_batch_str` and a `{python} admin_layers_str`-layer network, you must store every intermediate layer output for the backward pass.
    ...
    For a `{python} admin_width_str`-width model: `{python} admin_act_calc_md`. (Each layer's activation is a `Width$\times$ Width` matrix per sample---appropriate for transformer-style models where intermediate projections scale with hidden dimension squared.)

**The Systems Conclusion**: Your `{python} admin_weights_gb_str` GB model has an **"Administrative Tax"** of ~`{python} admin_tax_gb_str` GB (`{python} admin_grads_gb_str` GB gradients + `{python} admin_opt_gb_str` GB optimizer + `{python} admin_act_gb_str` GB activations) before you even process the first batch. During training, **Data Movement** includes saving and retrieving these activations, which is why training is often 3–4$\times$ slower than pure inference.
:::
...
[^fn-nvlink-bandwidth-hierarchy]: **NVLink**: NVIDIA's high-bandwidth GPU-to-GPU interconnect (see @sec-hardware-acceleration), providing `{python} nvlink_a100_gbs_str` GB/s bidirectional bandwidth (NVLink 3.0 on A100) compared to `{python} pcie4_bidir_gbs_str` GB/s for PCIe 4.0 x16. This ~10$\times$ bandwidth advantage determines whether tensor parallelism is practical for a given model size: splitting a model across GPUs connected by PCIe can make the $D_{vol}/BW$ communication term dominate total training time, erasing the benefit of additional compute. \index{NVLink!bandwidth hierarchy}

| **Interconnect** |                                     **Bandwidth** |               **Transfer Time** | **Relative to Compute**             |
|:-----------------|--------------------------------------------------:|--------------------------------:|:------------------------------------|
| **PCIe 3.0 x16** |                                           16 GB/s |                         0.25 ms | 10$\times$ slower than GPU compute |
| **PCIe 4.0 x16** |                     `{python} pcie4_gbs_str` GB/s |  `{python} pcie4_4mb_ms_str` ms | 5$\times$ slower than GPU compute  |
...
# │ How: model_memory() formula; m_as(Bparam) for param count extraction.
# │
# │ Note: PERSISTENT — gpt3_params_b_str / gpt3_fp16_gb_str reused again at
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
gpt3_params_b_str = GPT3ParameterStructures.gpt3_params_b_str
gpt3_fp16_gb_str = GPT3ParameterStructures.gpt3_fp16_gb_str
```
...
The kernel manager dispatches each operation to the fastest available implementation for the current hardware. When a framework encounters a matrix multiplication, it selects among AVX-512 vector instructions on modern CPUs, [cuBLAS](https://developer.nvidia.com/cublas) on NVIDIA GPUs, or dedicated tensor processing instructions on AI accelerators. The dispatch decision depends on input dimensions, data layout, and hardware capabilities. A 4096$\times$ 4096 GEMM on an A100 GPU routes to cuBLAS Tensor Core kernels that sustain up to `{python} A100BLAS.dense_tflops_str` TFLOPS in FP16, while the same operation on a CPU falls back to an AVX-512 path at roughly 2 TFLOPS. When no specialized kernel exists, the manager falls back to a generic implementation rather than failing.
...
[^fn-nn-module-composition]: **nn.Module**: PyTorch's base class (2016) for all neural network components. The class provides three mechanisms automatically: parameter registration (via Python's `__setattr__` metaclass interception), recursive tree traversal (for device placement, serialization, and optimizer access), and mode propagation (training vs. evaluation). This uniform interface means a single `.to("cuda")` call traverses the entire module tree and moves every parameter to GPU memory, and a single `.parameters()` call collects every trainable tensor for the optimizer. Without this automation, multi-million-parameter models would require manual bookkeeping that scales linearly with model complexity. \index{nn.Module!parameter management}
...
**JAX** makes the most radical trade-off by treating the model as a pure function[^fn-pure-function-jax] with immutable data[^fn-immutable-data-parallelism] and no internal state[^fn-stateless-function-optimization]. This functional purity solves the Differentiation Problem most elegantly: `grad`, `vmap` (automatic vectorization[^fn-vectorization-simd]), and `jit` (just-in-time compilation[^fn-jit-compilation-tradeoff]) are composable transformations on stateless functions, not infrastructure bolted onto an object system. The cost is explicit parameter management and a programming model unfamiliar to most engineers.

[^fn-immutable-data-parallelism]: **Immutable Data Structures**: Cannot be modified after creation; any apparent change creates a new copy. In ML frameworks, immutability enables safe parallel execution because no two threads can race on shared mutable state. JAX exploits this guarantee to compile `vmap` and `pmap` transformations into parallel GPU kernels without synchronization barriers, directly improving $\eta$ for embarrassingly parallel workloads like per-example gradient computation. \index{Immutable Data!parallel execution}

[^fn-stateless-function-optimization]: **Stateless Function**: Produces the same output for the same inputs every time, without relying on or modifying external state. This determinism is what allows a compiler to cache, reorder, and parallelize function calls: if the output depends only on the inputs, the compiler can prove that rearranging execution order preserves correctness. The constraint forces explicit parameter passing (no hidden global state), which is why JAX models must thread parameters through every function call. \index{Stateless Function!compiler optimization}

[^fn-vectorization-simd]: **Automatic Vectorization**: Transforms operations on individual data points into batch operations over entire arrays, exploiting SIMD hardware (one instruction processing multiple data elements simultaneously). JAX's `vmap` applies this transformation at the framework level: a function written for a single example is automatically vectorized across a batch, achieving the same throughput as manually batched code without the programmer writing batch-aware logic. The result is higher arithmetic intensity and better $\eta$ on parallel hardware. \index{Vectorization!SIMD utilization}

[^fn-jit-compilation-tradeoff]: **Just-in-Time (JIT) Compilation**: In JAX, the `jit` transformation traces a pure function to generate optimized machine code tailored to specific data shapes and hardware. This incurs a one-time compilation latency on the first run (5-30 seconds for large models) that is paid again if input shapes change, forcing a recompilation. Subsequent calls with matching shapes bypass this cost and execute with microsecond dispatch overhead. \index{JIT Compilation!shape specialization}
...
[^fn-xla-compiler]: **XLA (Accelerated Linear Algebra)**: Google's domain-specific compiler (2017) that compiles entire computational graphs into fused, hardware-specific machine code for CPUs, GPUs, and TPUs. The "linear algebra" in the name reflects that matrix multiplies and convolutions dominate ML computation. XLA achieves 3--10$\times$ speedups by fusing operations that eager frameworks execute as separate kernels, directly reducing $D_{vol}$ (fewer intermediate memory writes) and $L_{lat}$ (fewer kernel launches). Now part of the cross-industry OpenXLA project (2022), it serves as JAX's default compilation backend. \index{XLA!compilation speedup}

[^fn-onnx-portability]: **ONNX (Open Neural Network Exchange)**: Launched September 2017 by Facebook and Microsoft, now a Linux Foundation project (2019). ONNX defines a hardware-agnostic graph representation that decouples the training framework from the deployment runtime: train in PyTorch, export to ONNX, deploy via TensorRT or TF Lite. Without this exchange format, organizations face engineer-months of manual model conversion when the optimal training framework differs from the optimal serving runtime, a common scenario given the deployment-tier fragmentation described in this section. \index{ONNX!framework portability}
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
train_batch = TrainingStepDims.train_batch
train_input = TrainingStepDims.train_input
train_hidden = TrainingStepDims.train_hidden
train_output = TrainingStepDims.train_output
```
...
2. **Kernel Selection**\index{BLAS Library!kernel selection} (~0.5μs): cuBLAS selects an optimized GEMM kernel based on matrix dimensions (`{python} train_batch`$\times$ `{python} train_input`$\times$ `{python} train_input`$\times$ `{python} train_hidden`). For these dimensions, it might choose a tiled algorithm optimized for L2 cache.
...
class TrainingStepCalc:
    ...
    # Backward pass (2x forward FLOPs)
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
train_mm1_flops_str = TrainingStepCalc.train_mm1_flops_str
train_mm1_mem_str = TrainingStepCalc.train_mm1_mem_str
train_mm1_ai_str = TrainingStepCalc.train_mm1_ai_str
train_relu_flops_str = TrainingStepCalc.train_relu_flops_str
train_relu_mem_str = TrainingStepCalc.train_relu_mem_str
train_relu_ai_str = TrainingStepCalc.train_relu_ai_str
train_mm2_flops_str = TrainingStepCalc.train_mm2_flops_str
train_mm2_mem_str = TrainingStepCalc.train_mm2_mem_str
train_mm2_ai_str = TrainingStepCalc.train_mm2_ai_str
train_ce_flops_str = TrainingStepCalc.train_ce_flops_str
train_ce_mem_str = TrainingStepCalc.train_ce_mem_str
train_ce_ai_str = TrainingStepCalc.train_ce_ai_str
train_bwd_flops_str = TrainingStepCalc.train_bwd_flops_str
train_bwd_mem_str = TrainingStepCalc.train_bwd_mem_str
train_bwd_ai_str = TrainingStepCalc.train_bwd_ai_str
```

| **Component**                     |                                                                **FLOPs** |            **Memory Traffic** |     **Arithmetic Intensity** |
|:----------------------------------|-------------------------------------------------------------------------:|------------------------------:|-----------------------------:|
| **MatMul (x @ W1)**               | 2$\times$ 32$\times$ 784$\times$ 256 = `{python} train_mm1_flops_str` |  `{python} train_mm1_mem_str` |  `{python} train_mm1_ai_str` |
| **ReLU**                          |                        32$\times$ 256 = `{python} train_relu_flops_str` | `{python} train_relu_mem_str` | `{python} train_relu_ai_str` |
| **MatMul (h @ W2)**               |  2$\times$ 32$\times$ 256$\times$ 10 = `{python} train_mm2_flops_str` |  `{python} train_mm2_mem_str` |  `{python} train_mm2_ai_str` |
| **Cross-entropy**                 |                                           ~`{python} train_ce_flops_str` |   `{python} train_ce_mem_str` |   `{python} train_ce_ai_str` |
| **Backward (2$\times$ forward)** |                                          ~`{python} train_bwd_flops_str` |  `{python} train_bwd_mem_str` |  `{python} train_bwd_ai_str` |
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
mnist_t_compute_us_str = MnistTrainingStepCalc.mnist_t_compute_us_str
mnist_t_memory_us_str = MnistTrainingStepCalc.mnist_t_memory_us_str
mnist_t_overhead_us_str = MnistTrainingStepCalc.mnist_t_overhead_us_str
mnist_total_flops_str = MnistTrainingStepCalc.mnist_total_flops_str
mnist_mem_traffic_str = MnistTrainingStepCalc.mnist_mem_traffic_str
mnist_n_ops_str = MnistTrainingStepCalc.mnist_n_ops_str
mnist_us_per_op_str = MnistTrainingStepCalc.mnist_us_per_op_str
a100_peak_tflops_str = MnistTrainingStepCalc.a100_peak_tflops_str
a100_mem_bw_tbs_str = MnistTrainingStepCalc.a100_mem_bw_tbs_str
```
...
- Toverhead ≈ `{python} mnist_n_ops_str` ops$\times$ `{python} mnist_us_per_op_str` μs ≈ `{python} mnist_t_overhead_us_str` μs
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
pytorch_ms_str = FrameworkGapsCalc.pytorch_ms_str
tensorrt_ms_str = FrameworkGapsCalc.tensorrt_ms_str
pytorch_mobile_mb_str = FrameworkGapsCalc.pytorch_mobile_mb_str
tflite_micro_kb_str = FrameworkGapsCalc.tflite_micro_kb_str
perf_gap_str = FrameworkGapsCalc.perf_gap_str
memory_ratio_str = FrameworkGapsCalc.memory_ratio_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
model_7b_fp16_gb_str = Model7BMemory.model_7b_fp16_gb_str
a100_remaining_7b_gb_str = Model7BMemory.a100_remaining_7b_gb_str
a100_mem_str = Model7BMemory.a100_mem_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
n_images_str = CompilationOverheadCalc.n_images_str
n_recompilations_str = CompilationOverheadCalc.n_recompilations_str
eager_throughput_str = CompilationOverheadCalc.eager_throughput_str
compiled_throughput_str = CompilationOverheadCalc.compiled_throughput_str
compilation_time_s_str = CompilationOverheadCalc.compilation_time_s_str
eager_total_str = CompilationOverheadCalc.eager_total_str
compiled_total_str = CompilationOverheadCalc.compiled_total_str
```
