...
a100_tflops_fp16_str = fmt(a100_tflops_fp16_value, precision=0, commas=False)  # e.g. "312"  # Note: also defined in a100-memory-wall, device-bandwidth-hierarchy; produces same value
...
# --- Outputs (formatted strings for prose) ---
a100_tflops_fp16_str = fmt(a100_tflops_fp16_value, precision=0, commas=False)  # e.g. "312"  # Note: also defined in a100-specs-blas, device-bandwidth-hierarchy; produces same value
...
# │
# │ Why: Visualizes how eager execution creates GPU idle gaps between kernel
# │      launches (the "Python Tax"), while compilation fuses operations into
# │      a single kernel that eliminates dispatch latency entirely.
# │
# │ Imports: physx.viz (viz)
...
# │
# │ Why: Maps the three compilation regimes onto total execution time as a
# │      function of production executions. Reveals crossover points where
# │      compilation overhead is amortized and compiled mode wins.
# │
# │ Imports: numpy (np), physx.viz (viz)
# │ Exports: figure only, no prose variables
# └─────────────────────────────────────────────────────────────────────────────
import numpy as np
from physx import viz
...
# --- Outputs (formatted strings for prose) ---
gpt3_params_b_str = fmt(gpt3_params_b_value, precision=0, commas=False)        # e.g. "175"  # Note: also defined in gpt3-parameter-structures; produces same value
```
...
##### Principle 2: The Memory-Compute Trade-off {#sec-frameworks-autograd-principle-memory}
...
# --- Outputs (formatted strings for prose) ---
resnet_params_m_str = fmt(resnet_params_m_value, precision=1, commas=False)    # e.g. "25.6"  # Note: also defined in resnet50-params-intro; produces same value
...
# --- Outputs (formatted strings for prose) ---
model_7b_fp16_gb_str = fmt(model_7b_fp16_gb_value, precision=0, commas=False)  # e.g. "14"  # Note: also defined in model-7b-memory; produces same value
...
# --- Outputs (formatted strings for prose) ---
resnet_params_m_str = fmt(resnet_params_m_value, precision=1, commas=False)    # e.g. "25.6"  # Note: also defined in resnet50-memory-breakdown; produces same value
```
...
a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=1, commas=False)            # e.g. "2.0"
a100_tflops_fp16_str = fmt(a100_tflops_fp16_value, precision=0, commas=False)  # e.g. "312"  # Note: also defined in a100-specs-blas, a100-memory-wall; produces same value
a100_mem_str = fmt(a100_mem_value, precision=0, commas=False)                  # e.g. "80"  # Note: also defined in model-7b-memory; produces same value
...
[^fn-nvlink-frameworks]: **NVLink**: NVIDIA's high-bandwidth interconnect for GPU-to-GPU communication (see @sec-ai-acceleration for architecture details), providing `{python} nvlink_a100_gbs_str` GB/s bidirectional bandwidth (NVLink 3.0 on A100) compared to `{python} pcie4_bidir_gbs_str` GB/s bidirectional for PCIe 4.0 x16. Critical for multi-GPU training where gradient synchronization requires moving gigabytes per iteration. NVSwitch extends NVLink to connect 8 GPUs in a fully-connected topology (DGX systems), enabling all-to-all communication without bottlenecks. The ~10× bandwidth advantage over PCIe determines whether tensor parallelism is practical for a given model size.
...
#### Data Pipelines and Loading {#sec-ai-frameworks-domainspecific-data-organizations-48d9}
...
The abstraction problem has two layers, and so far we have examined only the lower one. Tensors, device placement, kernel dispatch, and memory management solve the hardware-facing half: making operations execute efficiently on diverse silicon. But a ResNet-50 contains fifty layers, each with multiple parameter tensors, buffers, and mode-dependent behaviors. Manually wiring each tensor to the correct device, registering it with an optimizer, toggling dropout behavior between training and inference, and serializing state for checkpointing---for every layer---would drown practitioners in bookkeeping that has nothing to do with model design. The upper layer of the abstraction problem is organizational: how do frameworks compose thousands of low-level primitives into the clean, composable APIs that practitioners actually use?

Every major framework answers this question through a *module abstraction* that bundles parameters, forward computation, and state management into a single reusable unit. PyTorch's `nn.Module` provides an instructive case study because its design patterns recur across frameworks: Keras uses similar layer abstractions, JAX's Flax employs analogous module structures, and TensorFlow's functional API shares conceptual parallels. Rather than catalog its API, we extract three enduring design principles that every framework must address regardless of its syntax or programming paradigm.
...
## Framework Selection {#sec-ai-frameworks-selecting-framework-2949}
