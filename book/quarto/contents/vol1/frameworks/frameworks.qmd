...
Architectures define *what* computations neural networks must perform, but knowing what to compute differs fundamentally from knowing *how* to compute it efficiently across diverse hardware. Framework selection determines hardware compatibility, deployment targets, and available optimization strategies. Migrating between frameworks is rarely a simple refactor: it invalidates data pipelines, serving infrastructure, and team expertise, typically requiring three to six engineer-months for production systems. This lock-in explains why framework selection matters more than it appears to. Organizations that treat frameworks as interchangeable tools discover *too late* that their inference pipeline requires a graph representation their eager-mode framework cannot provide, or that their target hardware lacks support in the ecosystem they chose.
...
# OUTPUT
# =============================================================================
a100_mem_str = f"{a100_mem_value:.0f}"
a100_bw_tbs_str = f"{a100_bw_tbs_value:.1f}"
a100_tflops_fp16_str = f"{a100_tflops_fp16_value:.0f}"
a100_tflops_sparse_str = f"{a100_tflops_sparse_value:.0f}"
resnet_params_m_str = f"{resnet_params_m_value:.1f}"
gpt3_params_b_str = f"{gpt3_params_b_value:.0f}"
```
...
The previous section established that kernel fusion is the key optimization for memory-bound operations, but fusion requires seeing multiple operations together. How do frameworks represent computation in a way that makes this visibility possible? The answer is the **computational graph**\index{Computational Graph!definition (DAG)}, a directed acyclic graph (DAG) where nodes represent operations and edges represent data dependencies. This graph is the framework's internal model of your computation. @fig-comp-graph illustrates a simple example: computing $z = x \times y$ requires two input nodes ($x$ and $y$), one operation node (multiplication), and one output node ($z$). The execution problem asks: *when* is this graph constructed, and *when* is it executed?
...
- **Operation fusion**\index{Kernel Fusion!static graph optimization}: Combine `y = x * 2` and `z = y + 1` into a single fused kernel `z = x * 2 + 1`, eliminating the intermediate `y` and reducing memory traffic by 50%. This directly addresses the Memory Wall constraint introduced in @sec-ai-frameworks-execution-strategy-matters-memory-wall-1ce8.
...
# PURPOSE
# =============================================================================
# Purpose: Visualize Python dispatch overhead comparing eager vs compiled execution
# Used in: Figure showing GPU idle time due to Python dispatch overhead
...
# OUTPUT
# =============================================================================
# Figure: Gantt chart showing execution timeline
```
...
# PURPOSE
# =============================================================================
# Purpose: Visualize compilation continuum showing when eager vs compiled execution is optimal
# Used in: Figure showing crossover points for different execution strategies
...
viz.plot_compilation_continuum(ax)
plt.show()

# =============================================================================
# OUTPUT
# =============================================================================
# Figure: Graph showing total time vs production executions for different strategies
```
...
# OUTPUT
# =============================================================================
dispatch_ratio_small_str = f"{dispatch_ratio_small_value:.1f}"
dispatch_sw_time_str = f"{dispatch_sw_time_value:.0f}"
dispatch_ratio_large_str = f"{dispatch_ratio_large_value:.4f}"
dispatch_overhead_pct_str = f"{dispatch_overhead_pct_value:.0f}"
dispatch_compilation_speedup_str = f"{dispatch_compilation_speedup_value:.0f}"
```
...
*   **Hardware Time**: Thw ≈ 2.6 μs (mostly memory latency).
*   **Software Overhead**: Tsw ≈ 6 ops × 5 μs/op = `{python} dispatch_sw_time_str` μs.
...
*   **Hardware Time**: Thw ≈ 100 ms = 100,000 μs.
...
# Second backward fails - graph was freed
y = x**2
y.backward()
...
**Detach vs. Data.**\index{Tensor!detach() method}\index{Gradient Flow!breaking with detach} Use `.detach()` to safely break gradient flow. The deprecated `.data` attribute can silently corrupt gradient computation through in-place operations:

::: {#lst-detach-vs-data lst-cap="**Safe Gradient Detachment**: Use `.detach()` to safely break gradient flow. The deprecated `.data` attribute can silently corrupt gradients through in-place operations."}
```{.python}
x = torch.tensor([1.0], requires_grad=True)
y = x * 2
...
z_safe = y.detach()
z_safe.mul_(100)  # In-place operation on detached tensor
# y is unchanged, gradient computation is safe
...
@tbl-deployment-frameworks reveals a fragmented landscape: different deployment targets favor different frameworks. This fragmentation creates a practical problem when organizations train in one framework but deploy on a target best served by another. **Interoperability through ONNX.** The Open Neural Network Exchange (ONNX)\index{ONNX!cross-framework portability}[^fn-onnx] format addresses this by enabling model portability across frameworks: train in PyTorch, deploy via TensorFlow Lite or ONNX Runtime. This standardization eliminates manual conversion when moving between development and production environments. @fig-onnx illustrates this hub-and-spoke interoperability model. Detailed deployment optimization (quantization, pruning, hardware-specific compilation) appears in @sec-model-compression and @sec-model-serving-systems.
...
# OUTPUT
# =============================================================================
mnist_t_compute_us_str = f"{mnist_t_compute_us_value:.1f}"
mnist_t_memory_us_str = f"{mnist_t_memory_us_value:.1f}"
mnist_t_overhead_us_str = f"{mnist_t_overhead_us_value:.0f}"
mnist_total_flops_str = f"{mnist_total_flops_value/1e6:.0f}M"
mnist_mem_traffic_str = f"{mnist_mem_traffic_bytes_value/1e6:.0f}MB"
a100_peak_tflops_str = f"{a100_peak_tflops_value/1e12:.0f}"
a100_mem_bw_tbs_str = f"{a100_mem_bw_bytes_value/1e12:.0f}"
```
...
- Production training uses much larger models where computlwe dominates
...
# OUTPUT
# =============================================================================
perf_gap_str = f"{perf_gap_value:.0f}"
memory_ratio_str = f"{memory_ratio_value:.0f}"
```
...
# OUTPUT
# =============================================================================
eager_total_str = f"{eager_total_value:.1f}"
compiled_total_str = f"{compiled_total_value:.1f}"
```
