...
::: {.callout-notebook title="Napkin Math: The Dispatch Tax"}
...
*   **Software Overhead**: $T_{sw} \approx 6 \text{ ops} \times 5 \mu s/\text{op} = 30 \mu s$.
*   **Ratio**: $\frac{30}{2.6} \approx \mathbf{11.5}$.
*   **Conclusion**: You spend 92% of time waiting for Python. Compilation yields **10x speedup**.
...
*   **Ratio**: $\frac{50}{100,000} \approx \mathbf{0.0005}$.
...
**GPU-Level Profiling with NVIDIA Nsight.** For hardware-level bottleneck diagnosis, NVIDIA provides two complementary tools. Nsight Systems captures system-wide timelines correlating CPU activity, GPU kernel execution, and memory transfers:
