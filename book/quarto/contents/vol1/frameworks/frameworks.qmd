...
: **Per-Operation Roofline Analysis**: FLOPs, memory traffic, and arithmetic intensity for each operation in a two-layer MLP training step. MatMul operations have much higher arithmetic intensity than element-wise operations, while ReLU and cross-entropy are clearly memory-bound; on high-end GPUs, small MatMuls may still fall below the ridge point, which is why this training step remains overhead- and memory-sensitive. {#tbl-training-step-roofline tbl-colwidths="[29,29,20,22]"}
