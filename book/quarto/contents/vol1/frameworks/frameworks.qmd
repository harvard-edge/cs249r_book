...
[^fn-computational-graph-frameworks]: **Computational Graph**: The "optimize before executing" distinction in the triggering sentence is the key design choice. Capturing the program as a data structure lets a framework fuse compatible operations into fewer GPU kernels before execution, reducing launch overhead and intermediate memory traffic. The engineering cost of this visibility is that the executed program differs from the source code, making debugging harder, a trade-off every graph-based framework must justify against the performance gain. \index{Computational Graph!framework optimization}

[^fn-eager-execution-tradeoff]: **Eager Execution**: This mode executes each operation immediately, which enables direct debugging with standard tools but sacrifices the global view needed for graph-level optimizations. Without capturing a larger region of computation, an eager runtime cannot fuse across that region or preplan its memory, leaving workload-dependent optimization opportunities for compilers such as `torch.compile`. \index{Eager Execution!optimization trade-off}
...
::: {#cs-frameworks-mars-climate-orbiter-units .callout-case-study title="The interface that forgot its units"}
...
[^fn-autograd-tape-memory]: **Autograd Tape**: A transient data structure built during forward execution, where nodes record the operation type, graph dependencies, values requested by derivative rules, and the backward function for chain rule application. Its memory cost depends on the operations and saved values, and the graph is normally released as backward consumes it. For deep models, saved activations can consume more memory than the model weights, so frameworks can retain selected activations and recompute others during the backward pass. This recomputation-for-memory trade-off is called activation checkpointing later in the chapter. \index{Autograd Tape!memory cost}
...
Follow this "define-by-run" execution model step by step in @fig-mlfm-dynamic-graph-flow. Notice the alternating pattern: define, dispatch, define, dispatch. On a CPU, an operation commonly finishes before Python proceeds; accelerator work such as CUDA kernels is normally queued asynchronously. Standard Python debuggers can still inspect eager program state between calls, though reading a pending accelerator result may require synchronization. This contrasts with static graphs, where operations are staged before graph execution.

::: {#fig-mlfm-dynamic-graph-flow fig-env="figure" fig-pos="htb" fig-cap="**Dynamic Graph Execution Flow**: In eager execution, each operation is issued as the program encounters it. Accelerator work may execute asynchronously after dispatch. This define-by-run model enables natural debugging and data-dependent control flow while limiting optimization across Python calls." fig-alt="Flow diagram showing start to operation 1 to operation 1 issued to operation 2 to operation 2 issued to end. Above arrows show define operation, dispatch operation, define next operation, dispatch operation, repeat until done."}
...
The dynamic autograd tape expresses data-dependent behavior directly in the host language. Conditionals and loops can depend on tensor values computed during execution, enabling algorithms like beam search, dynamic recurrent neural network (RNN) lengths, or adaptive computation that adjust their behavior based on intermediate results. Static graphs can also represent data-dependent control flow and dynamic dimensions through graph operations, but eager execution makes these patterns natural to write and debug in Python. Because operations are issued immediately, developers can print tensors, inspect values, and use standard debuggers (`pdb`, breakpoints) to diagnose errors much as they would in another Python program.

##### Systems implications: Overhead {.unnumbered}

This flexibility comes with performance costs that map directly to the iron law (@sec-introduction-iron-law-ml-systems-c32a). Each training forward pass constructs an autograd tape, adding host-side dispatch and graph-recording work to $L_{\text{lat}}$. Operations pass through the framework dispatcher before device kernels are launched, so overhead becomes significant when individual kernels are short. Eager execution alone also lacks a captured region across which a compiler could fuse compatible operations, increasing launch overhead $(L_{\text{lat}})$ and intermediate data movement $(D_{\text{vol}})$. The autograd tape retains the values requested by backward rules, adding memory pressure until those values can be released. Together, these costs create a performance ceiling that becomes visible as operations grow smaller and dispatch overhead dominates computation.
...
TorchScript's historical alternative, scripting\index{JIT Compilation!scripting}, analyzed supported Python source directly and compiled it to TorchScript IR without tracing a sample path [@pytorchTorchScriptDocs]. The scripting compiler preserved supported branching structure but accepted only a restricted Python subset. Current projects should follow the supported `torch.export` and target-runtime guidance rather than select between TorchScript tracing and scripting for new deployment pipelines.

Within the historical TorchScript workflow, tracing fit feed-forward models whose tensor paths remained stable for the supported inputs, while scripting handled supported data-dependent control flow without a Python interpreter.
...
**Systems insight**: In the illustrative scenario, Compute Beasts (the ResNet-50 row in @tbl-training-benchmark) benefit most because their dense kernels expose the most surface for layout selection, precision lowering, and fusion. Sparse Scatter workloads such as DLRM gain little because their bottleneck, irregular embedding lookups, leaves almost nothing for the compiler to optimize.
...
Evaluating @eq-compile-breakeven with the hypothetical ResNet-50 values gives approximately `{python} CompilationBenchmark.resnet_breakeven_images_str`; this scenario therefore reaches breakeven before 1.28 million images.
...
[^fn-autodiff-reverse-mode]: **Reverse-Mode AD**: The $\mathcal{O}(1)$-vs.-$\mathcal{O}(P)$ asymmetry mentioned earlier has a concrete price: reverse mode retains the operation-dependent values required by derivative rules during the backward traversal. Activation checkpointing changes this saved set by omitting selected values and recomputing them during the backward pass. \index{Automatic Differentiation!reverse mode}
...
Forward mode is therefore inefficient for the usual training shape of one scalar loss and millions of parameters. It remains useful when the number of seeded input directions is small and in mixed-mode methods for specialized derivatives such as Jacobian-vector and Hessian-vector products.

For the usual scalar-loss training objective, reverse mode reverses this scaling: one backward traversal computes the requested gradients for all parameters that influence the loss.

#### Reverse mode {#sec-ml-frameworks-reverse-mode-d328}

Modern ML frameworks normally use reverse mode\index{Automatic Differentiation!reverse mode}\index{Reverse Mode AD!backpropagation} for scalar-loss training because of computational asymmetry\index{Backpropagation!computational asymmetry}. Forward mode propagates derivatives for selected input directions, while one reverse traversal propagates adjoints from the scalar loss to all requested parameters. The advantage grows with the number of parameters, although the actual cost also depends on the graph and derivative rules.
...
# │ Show: "~102 MB FP32 weights vs. 10–15 GB illustrative training peak"—inline
...
::: {.column-margin}
![](images/svg/vol1_frameworks_margin_001.svg){width="100%" fig-alt="Vertical memory ladder comparing an illustrative ResNet-50 training footprint around 10 to 15 GB against approximately 102 MB of FP32 weight storage."}

*The illustrative training peak dwarfs weight storage alone.*
:::

The saved-activation footprint depends on layer shapes, batch size, and which values each backward rule requires. Consider an illustrative ResNet-50 scenario with `{python} ResNetMemory.resnet_params_m_str` parameters (~`{python} ResNetMemory.resnet_fp32_mb_str` of FP32 weights), batch size 64, and $224{\times}224$ images. Assuming 8--12 GB of saved activations, parameter gradients add ~`{python} ResNetMemory.resnet_fp32_mb_str`, and Adam's two FP32 moment buffers add ~`{python} ResNetMemory.resnet_adam_mb_str`. The resulting `{python} ResNetMemory.resnet_training_min_gb_str`--`{python} ResNetMemory.resnet_training_max_gb_str` training scenario is roughly `{python} ResNetMemory.resnet_training_ratio_range_mult_str` the FP32 weight storage alone. It is not a comparison with complete inference memory, which also includes activations and runtime workspaces.

The example shows why training capacity cannot be estimated from weights alone. It does not establish that data movement dominates latency or that training exactly doubles inference traffic; both depend on the operations, implementation, and memory reuse. @Sec-appdx-algorithm-foundations-true-cost-training-memory-e54e derives the four-component training-state equation ($M_{\text{total}} = M_{\text{weights}} + M_{\text{gradients}} + M_{\text{optimizer}} + M_{\text{activations}}$) and discusses additional runtime memory.
...
[^fn-tensor-hardware-contract]: **Tensor**: In mathematics, tensors obey coordinate-transformation laws. ML frameworks use the term more broadly for multidimensional arrays carrying shape, dtype, strides, device placement, and often automatic-differentiation metadata. A transpose commonly creates a strided view, while reshape may return either a view or a copy depending on the layout. These storage semantics make layout relevant to kernel selection and data movement. \index{Tensor!hardware contract}
...
::: {#fig-tensor-data-structure-a fig-env="figure" fig-pos="htb" fig-cap="**Tensor Rank Hierarchy**: Four shapes illustrate ascending tensor ranks, from a single value (rank 0, scalar) to a column of values (rank 1, vector), a grid of values (rank 2, matrix), and a cube of values (rank 3, three-dimensional tensor)." fig-alt="Four shapes showing tensor ranks left to right: single box labeled Rank 0, vertical column of numbers labeled Rank 1, 2D grid of numbers labeled Rank 2, and 3D cube labeled Rank 3."}
...
# A pageable source may require staging and should not be assumed to overlap
...
Across all four rows, the durable pattern consists of a trainable-parameter accessor for the optimizer and a separate nontrainable-state channel for values that must persist but must not receive gradients.
...
[^fn-jit-compilation-tradeoff]: **Just-in-Time (JIT) Compilation**: Traces a function for an argument signature and lowers it to backend-specific executable code. The first call pays tracing and compilation costs; compatible later calls can reuse the cached executable. New shapes, dtypes, devices, or static arguments may trigger another compilation. Both compilation time and cached-call overhead depend on the program, backend, and hardware. \index{JIT Compilation!shape specialization}

[^fn-pure-function-jax]: **Pure Function**: Returns outputs determined by its inputs without relying on untracked side effects. JAX transformations are designed for pure functions. Ordinary Python effects may run during tracing but are not represented in the compiled computation, and impure code can fail or behave unexpectedly. Runtime printing or external effects require supported mechanisms such as `jax.debug.print` or callbacks; random draws require explicitly managed keys. \index{Pure Function!JIT requirement}
...
| **TF Lite Micro/microTVM**      | Microcontroller inference      | KB-scale RAM budgets on many TinyML deployments                   | Static allocation, INT8 kernels               | Selected operators and constrained memory |
...
: **TensorFlow Variant Software Comparison**: Design trade-offs across TensorFlow, TensorFlow Lite, and TensorFlow Lite Micro, balancing model expressiveness, binary size, and resource constraints. Operator coverage narrows from a general training framework to deployment-focused runtimes whose supported set depends on the model and build. Native lower-precision tooling enables further optimization for constrained environments. {#tbl-tf-comparison tbl-colwidths="[28,26,20,26]"}
...
The model estimates ~`{python} MnistTrainingStepCalc.mnist_total_mflop_str` of work and ~`{python} MnistTrainingStepCalc.mnist_mem_traffic_mb_str` of memory traffic. Dividing by A100 peak rates gives ideal lower bounds:
...
Under the stated launch-count and per-launch assumptions, the modeled training step is overhead-bound.\index{Overhead-Bound!small model training} Small models are often sensitive to Python dispatch and kernel launch costs, which drives three common production practices:
