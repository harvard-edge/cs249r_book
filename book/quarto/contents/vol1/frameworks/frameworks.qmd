...
class TrainingStepCalc:
    ...
    train_mm1_mem_str = fmt_qty(mm1_mem, MB, precision=1, commas=False)
    train_mm1_ai_str = fmt((mm1_flops / mm1_mem).to(flop / byte).magnitude, precision=1, commas=False)

    train_relu_flops_str = fmt_qty(relu_flops, KFLOPs, precision=1, commas=False)
    train_relu_mem_str = fmt_qty(relu_mem, KB, precision=1, commas=False)
    train_relu_ai_str = fmt((relu_flops / relu_mem).to(flop / byte).magnitude, precision=3, commas=False)

    train_mm2_flops_str = fmt_qty(mm2_flops, KFLOPs, precision=1, commas=False)
    train_mm2_mem_str = fmt_qty(mm2_mem, KB, precision=1, commas=False)
    train_mm2_ai_str = fmt((mm2_flops / mm2_mem).to(flop / byte).magnitude, precision=1, commas=False)

    train_ce_flops_str = fmt_qty(ce_flops, KFLOPs, precision=1, commas=False)
    train_ce_mem_str = fmt_qty(ce_mem, KB, precision=1, commas=False)
    train_ce_ai_str = fmt((ce_flops / ce_mem).to(flop / byte).magnitude, precision=1, commas=False)

    train_bwd_flops_str = fmt_qty(bwd_flops, MFLOPs, precision=1, commas=False)
    train_bwd_mem_str = fmt_qty(bwd_mem, MB, precision=1, commas=False)
...
class Model7BMemory:
    ...
    a100_mem_str = fmt_qty(a100_mem_display, GB, precision=0, commas=False)                    # e.g. "80 GB"
```
