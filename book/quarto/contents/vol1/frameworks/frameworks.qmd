...
- **JAX** transforms functions rather than tracking operations. The `jax.grad()` transformation returns a new function that computes gradients, enabling composition with `jax.vmap()` for vectorization and `jax.jit()` for compilation. This fundamentally different approach requires pure functions but enables powerful program transformations.
