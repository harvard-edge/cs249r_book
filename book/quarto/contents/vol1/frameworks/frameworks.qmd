...
Before examining specific framework features, we must understand the physical constraints that drive framework design. Two fundamental principles govern ML system performance: **memory bandwidth limitations** and **computational complexity**. Understanding these constraints explains why frameworks are designed as they are and enables reasoning about framework optimization techniques.
...
**cuBLAS** (CUDA Basic Linear Algebra Subprograms) provides matrix multiplication (GEMM) implementations that achieve 95% or more of theoretical peak throughput for large matrices. The library contains hundreds of kernel variants optimized for specific matrix dimensions, data types, and GPU architectures. For a given GEMM operation, cuBLAS selects the optimal variant based on problem characteristics:
...
**cuDNN** (CUDA Deep Neural Network library) provides optimized primitives for neural network operations including convolutions, normalization, and attention:
...
cuDNN's autotuning feature benchmarks multiple algorithm variants at runtime to select the fastest implementation for specific input shapes:
...
Understanding library coverage matters because operations outside library support may run 10-100x slower. Custom operations that cannot be expressed using library primitives (e.g., novel activation functions, non-standard attention patterns) require hand-written CUDA kernels or accept significant performance penalties. When possible, express computations using supported primitives even if the mathematical formulation seems less elegant.
...
# Subsequent calls: runs compiled version (~1ms, 2-5× faster)
...
- **backend='tensorrt'**: Compiles to NVIDIA TensorRT inference engine with aggressive optimizations (int8 quantization, layer fusion, kernel autotuning). Inference-only (no backward pass), NVIDIA GPUs only, often achieves 2-5× speedup over TorchInductor for inference.
...
# Typical: 2-5× for matrix operations
```
...
**Compilation Overhead**: First execution includes compilation time: 5-10 seconds for small models, 30-60 seconds for BERT-base transformers, 5-10 minutes for GPT-3 scale models. This overhead is amortized across training (compile once, train for thousands of iterations) but impacts development iteration time. Compiled kernels are cached on disk; subsequent runs skip compilation.

**Memory Usage**: Compilation adds overhead: 100-500 MB for FX graph construction, 500 MB-2 GB peak during Triton compilation, 10-100 MB per compiled graph for storage. Runtime memory usage is similar to eager mode (kernel fusion can reduce intermediate tensors but compiled code may allocate temporary buffers). Compiled models typically use 90-110% of eager mode memory.

**Debugging**: Errors in compiled code produce stack traces pointing to generated code, not source Python code. Print statements inside compiled regions cause graph breaks (executed eagerly, not compiled). For debugging, remove `@torch.compile` to revert to eager execution, fix bugs, then re-enable compilation. Use `TORCH_COMPILE_DEBUG=1` for verbose compilation logs.
...
::: {#lst-forward_mode_dual lst-cap="**Forward Mode**: Computing derivatives alongside function values using dual numbers to track changes in both the result and its rate of change."}
```{.python}
x = 2.0  # Initial value
...
```python
import torch
from torch.amp import autocast  # Note: torch.cuda.amp is deprecated
...
This selective precision maintains accuracy while achieving speedups. On NVIDIA A100 GPUs, FP16 matrix multiplication achieves 312 TFLOPS compared to 156 TFLOPS for FP32, a 2x throughput improvement.

**Hardware Support: Tensor Cores** {#sec-ai-frameworks-tensor-cores}

The throughput improvement from reduced precision operations is not simply faster floating-point arithmetic. NVIDIA Tensor Cores, introduced with the Volta architecture in 2017, are specialized matrix multiply-accumulate (MMA) units that compute $D = A \times B + C$ where $A$ and $B$ are reduced-precision matrices and accumulation occurs in higher precision. Each Tensor Core computes a 4x4x4 matrix multiply per clock cycle, enabling the dramatic throughput gains observed with mixed-precision training.

Understanding Tensor Core requirements is essential for achieving advertised performance. Tensor Core utilization depends on several architectural constraints:

1. **Dimension alignment**: Matrix dimensions must be multiples of 8 for FP16 operations (ideally 16 for maximum efficiency). Misaligned dimensions fall back to standard CUDA cores, reducing throughput by up to 16x.
2. **Memory layout**: Tensors must be contiguously laid out in memory. Strided access patterns prevent Tensor Core dispatch.
3. **Supported data types**: Operations must use Tensor Core compatible types (FP16, BF16, TF32, INT8, INT4 on newer architectures).

Without understanding these constraints, practitioners may observe their code running at CUDA core speeds (19.5 TFLOPS FP32 on A100) rather than Tensor Core speeds (312 TFLOPS FP16), a 16x performance difference with no obvious indication of the cause.

@tbl-tensor-core-requirements summarizes Tensor Core activation requirements:

| Requirement | Specification | Consequence of Violation |
|-------------|---------------|--------------------------|
| Matrix M dimension | Multiple of 8 (FP16) | Falls back to CUDA cores |
| Matrix N dimension | Multiple of 8 (FP16) | Falls back to CUDA cores |
| Matrix K dimension | Multiple of 8 (FP16) | Falls back to CUDA cores |
| Memory layout | Contiguous | Falls back to CUDA cores |
| Data type | FP16, BF16, TF32, INT8 | Falls back to CUDA cores |

: Tensor Core activation requirements. Violating any requirement causes fallback to standard CUDA cores with dramatically reduced throughput. {#tbl-tensor-core-requirements}

**Modern Precision Formats** {#sec-ai-frameworks-precision-formats}

Beyond FP16, modern GPU architectures support additional precision formats that address specific training and inference requirements. Understanding these formats enables practitioners to select optimal precision for their workloads.

@tbl-precision-formats compares the available precision formats:

| Format | Exponent | Mantissa | Range | Primary Use Case |
|--------|----------|----------|-------|------------------|
| FP32 | 8 bits | 23 bits | ±3.4×10³⁸ | Reference precision, optimizer states |
| TF32 | 8 bits | 10 bits | ±3.4×10³⁸ | Drop-in FP32 replacement for matrix ops |
| BF16 | 8 bits | 7 bits | ±3.4×10³⁸ | Training stability (same range as FP32) |
| FP16 | 5 bits | 10 bits | ±65504 | Inference, memory-bound training |
| FP8 (E4M3) | 4 bits | 3 bits | ±448 | Inference on Hopper+ architectures |
| FP8 (E5M2) | 5 bits | 2 bits | ±57344 | Training gradients on Hopper+ |

: Precision formats available on modern NVIDIA GPUs. The exponent determines dynamic range; the mantissa determines precision within that range. {#tbl-precision-formats}

**TensorFloat-32 (TF32)** uses FP32's 8-bit exponent for range but truncates the mantissa to 10 bits. On Ampere and later architectures, TF32 is enabled by default for matrix operations, providing Tensor Core acceleration for existing FP32 code without any code changes. This implicit acceleration can surprise practitioners who observe different performance characteristics on different hardware:

```python
# TF32 is enabled by default on Ampere+ GPUs
# FP32 matrix operations automatically use TF32 precision

# Disable TF32 for debugging numerical differences:
torch.backends.cuda.matmul.allow_tf32 = False
torch.backends.cudnn.allow_tf32 = False
```

**BFloat16 (BF16)** maintains FP32's dynamic range by keeping the 8-bit exponent while reducing mantissa precision to 7 bits. This design eliminates most gradient underflow issues that require GradScaler with FP16, simplifying mixed-precision training:
...
# No GradScaler needed - gradients stay in representable range
loss.backward()
optimizer.step()
```

The choice between FP16 and BF16 depends on workload characteristics. FP16 offers higher mantissa precision (10 bits vs 7 bits), beneficial for inference where small numerical differences affect output quality. BF16's larger dynamic range makes it preferable for training, where gradient magnitudes vary widely across layers and training stages.
...
from torch.amp import (
    autocast,
    GradScaler,
)  # Note: torch.cuda.amp is deprecated
