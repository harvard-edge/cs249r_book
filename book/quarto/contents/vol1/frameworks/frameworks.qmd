...
All modern frameworks converge on the same three fundamental problems: *how* to execute computation, *how* to differentiate it, and *how* to abstract across hardware. We begin with the most visible of these: the execution problem, because its resolution determines what optimizations the other two problems can exploit.

## Execution Problem {#sec-ai-frameworks-execution-problem-e1e1}

\index{Execution Problem!definition}
The execution problem asks a seemingly simple question: should operations execute immediately as written, or should the framework record them for later execution? This choice creates a cascade of engineering trade-offs shaping every aspect of framework behavior, from debugging workflows to deployment options to peak hardware utilization.
...
The execution problem told us *when* to compute; the differentiation problem told us *how* to train. The abstraction problem asks the remaining question: how should frameworks target this diverse hardware from a single programming interface? A GPU offers 1,000x the parallelism of a CPU but has different memory semantics. A TPU provides higher throughput but requires static shapes. A microcontroller has kilobytes where a server has gigabytes. Frameworks must provide abstractions that hide this hardware complexity while enabling efficient utilization of each target's unique capabilities.
...
The hardware abstraction layer answers *how* operations reach silicon; the module abstraction layer answers *how* practitioners organize thousands of those operations into coherent models. Frameworks solve this organizational problem through higher-level abstractions that compose tensors, device management, and operations into reusable components.
...
As ML models move from cloud servers to edge devices, the efficiency gaps measured above transform from optimization opportunities into hard deployment constraints. The three fundamental problems reweight dramatically at the edge. The *execution problem* shifts from "eager vs. graph" to "can we execute at all within 10ms and 50KB?" The *differentiation problem* often disappears entirely, since edge devices run inference only. The *abstraction problem* intensifies: targeting ARM vs. x86, mobile NPUs vs. edge TPUs, microcontrollers with kilobytes of memory.
...
We have now examined the three fundamental problems individually, compared how major frameworks resolve them, and established criteria for choosing among alternatives. What remains is to see all three problems interact inside a single execution---to watch the machinery we have studied operate as one integrated system.
