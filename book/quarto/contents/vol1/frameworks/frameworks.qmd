...
[^fn-chain-rule-autodiff]: **Automatic Differentiation**: This process mechanizes the chain rule as a graph traversal algorithm, allowing the `loss.backward()` call to compute exact gradients for any model architecture by traversing its computational graph in reverse. The critical trade-off is that every intermediate value from the forward pass must be stored for this traversal, meaning the "gigabytes of intermediate state" required for activations can easily exceed the memory needed for the parameters themselves. \index{Chain Rule!automatic differentiation}
...
[^fn-numpy-abstraction]: **NumPy (Numerical Python)**: The "vectorization" contract described here (write logic in Python, execute loops in C/Fortran via BLAS) became the design template for every ML framework that followed. PyTorch tensors and TensorFlow arrays are direct descendants: they extend the same n-dimensional array abstraction to GPUs, adding autograd and device placement while preserving the array-oriented programming model. The consequence is that the Developer Velocity rung on this ladder is also a lock-in rung: Python's dominance in ML is a direct inheritance from NumPy's 2005 decision to wrap BLAS in Python rather than any other language. \index{NumPy!framework lineage}

3.  **Solving Differentiation (2015–present)**: **Deep Learning Frameworks** (Theano, TensorFlow, PyTorch) solved the problem of *Gradient Computation*. While NumPy required manual derivation of backpropagation gradients (error-prone and slow), these frameworks introduced **Automatic Differentiation** via the computational graph [@rumelhart1986learning]. This turned the chain rule into a software primitive, allowing researchers to define *forward* passes and get *backward* passes for free.
...
[^fn-dag-execution]: **DAG (Directed Acyclic Graph)**: The graph's structure, with operations as nodes and data as edges, provides a formal model of the computation before it runs. This visibility allows the framework to identify adjacent, memory-bound operations that can be fused into a single GPU kernel, eliminating round-trips to global memory. Without this graph-level view, an interpreter executes operations sequentially, incurring a >100x performance penalty for memory-bound kernels by materializing intermediate tensors. \index{DAG!framework optimization}
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
python_overhead_str = DispatchTax.python_overhead_str
small_kernel_str = DispatchTax.small_kernel_str
large_kernel_str = DispatchTax.large_kernel_str
small_tax_pct_str = DispatchTax.small_tax_pct_str
large_tax_pct_str = DispatchTax.large_tax_pct_str
```
...
Every operation in an eager framework (like standard PyTorch) must pay a fixed "Tax" of approximately **`{python} python_overhead_str` $\mu$s** for Python to lookup the function, check tensor types, and launch the kernel. 

*   **For small operations** (e.g., a ReLU on a small vector), the kernel might execute in only **`{python} small_kernel_str` $\mu$s**. The dispatch tax is **`{python} small_tax_pct_str`%**, meaning the GPU spends the vast majority of its time waiting for the next command.
*   **For large operations** (e.g., a massive $4096\times4096$ matrix multiply), the kernel executes for **`{python} large_kernel_str` $\mu$s**. The dispatch tax drops to **`{python} large_tax_pct_str`%**, and the system becomes compute-bound.
...
[^fn-backprop-memory-cost-fw]: **Backpropagation**: The "building on" phrasing signals a shift: @sec-neural-computation covered the mathematics, but here the focus is the *systems cost* the framework must manage. Reverse-mode chain rule application requires caching all intermediate activations from the forward pass, and these stored values typically exceed the model weights in size, making a model's training memory footprint 3--4$\times$ larger than its inference footprint. This gap is the central engineering challenge that drives the checkpointing, fusion, and memory management strategies examined in the remainder of this section. \index{Backpropagation!memory cost}
...
[^fn-tensor-hardware-contract]: **Tensor**: The reason "every computation" uses this single abstraction is that tensors guarantee contiguous memory with a fixed data type and layout, allowing the framework to dispatch batches, activations, gradients, and optimizer states to hardware through one code path instead of four. The systems consequence of this uniformity is layout sensitivity: choosing NCHW when the target accelerator prefers NHWC (or vice versa) can halve computational throughput for the same arithmetic, because misaligned memory access patterns break hardware coalescing. \index{Tensor!hardware contract}
...
[^fn-stateless-function-optimization]: **Stateless Function**: A function whose output depends *only* on its inputs, a deterministic guarantee that makes compiler transformations like `jit` and `vmap` mathematically pure and thus safely composable. This purity is what forces the "explicit parameter management" mentioned in the text, as the model's state (weights) must be passed as an argument in every single call, a trade-off for performance gains that can exceed 10x. \index{Stateless Function!compiler optimization}

[^fn-vectorization-simd]: **Automatic Vectorization (`vmap`)**: Because JAX functions are stateless, `vmap` can transform a function written for a single example into one that processes an entire batch without side effects. This avoids the complex, stateful batching logic required by object-oriented systems, as the transformation is a guaranteed compiler-level optimization. The resulting code can saturate parallel hardware, achieving near-100% utilization of a processor's SIMD/SIMT vector lanes. \index{Vectorization!SIMD utilization}
...
|:---------------------------------|----------------------------------------------------------------------:|------------------------------:|-----------------------------:|
| **MatMul (x @ W1)**              | 2$\times$ 32$\times$ $784\times256$ = `{python} train_mm1_flops_str` |  `{python} train_mm1_mem_str` |  `{python} train_mm1_ai_str` |
| **ReLU**                         |                      $32\times256$ = `{python} train_relu_flops_str` | `{python} train_relu_mem_str` | `{python} train_relu_ai_str` |
| **MatMul (h @ W2)**              |  2$\times$ 32$\times$ $256\times10$ = `{python} train_mm2_flops_str` |  `{python} train_mm2_mem_str` |  `{python} train_mm2_ai_str` |
