...
[^fn-flashattention-fusion-fw]: **Fused Attention Kernel**: A fused attention kernel combines the $\mathbf{Q}\mathbf{K}^T$ product, softmax, and value-weighted output into a tiled implementation that keeps intermediate values in on-chip memory rather than materializing the full attention matrix in HBM. FlashAttention is the canonical named example introduced in @sec-network-architectures and reports up to 9$\times$ fewer HBM accesses with workload-specific speedups from 15 percent to 3$\times$ [@dao2022]. The framework lesson is broader than the specific algorithm: fusion can shift an operation's position on the Roofline Model from bandwidth-limited toward throughput-limited execution by reducing round-trips through external memory. \index{Attention Mechanism!fused kernel}
...
# ┌── 2. ARRAYS ────────────────────────────────────────────────────────────────
t_dispatch, t_compute, n_ops = 15, 1, 5
...
# │ Show: That small MLPs are 97% overhead-bound, while GPT-3 is <0.05% overhead.
...
class DispatchTaxSmallLarge:
    ...
    small_ops_count = 6
    # Full host-side cost per launch (Python dispatch + kernel launch), not the
    # kernel-launch component alone.
    small_dispatch_us = calibration.KERNEL_LAUNCH_LATENCY_US
    small_hw_us = 2.6
...
*   **Small-model outcome**: The system spends `{python} DispatchTaxSmallLarge.dispatch_overhead_pct_str` of time in host-side dispatch and kernel-launch overhead. Compilation yields at most `{python} DispatchTaxSmallLarge.dispatch_compilation_speedup_mult_str` speedup.
...
Every modern ML framework defaults to reverse mode\index{Automatic Differentiation!reverse mode}\index{Reverse Mode AD!backpropagation} for training because of computational asymmetry\index{Backpropagation!computational asymmetry}, one of the most consequential design decisions in machine learning software. A neural network has one scalar loss but millions of parameters. Forward mode computes one parameter's gradient per pass, requiring $P$ passes for $P$ parameters. Reverse mode computes all $P$ gradients in a single backward pass. For a model with 100 million parameters, that is the difference between 100 million forward passes and exactly one backward pass, a speedup proportional to the parameter count.
...
# │      Assume 12 ops × 15 μs dispatch = 180 μs overhead. The step is overhead-bound!
...
class MnistTrainingStepCalc:
    ...
    us_per_op = calibration.KERNEL_LAUNCH_LATENCY_US  # 15 μs host-side per launch
    ...
    t_overhead_us = n_ops * us_per_op  # 180 μs (dominant!)
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(
...
- Assuming `{python} MnistTrainingStepCalc.mnist_n_ops_str` launches, $T_{\text{overhead}} \approx$ `{python} MnistTrainingStepCalc.mnist_n_ops_str` $\times$ `{python} MnistTrainingStepCalc.mnist_us_per_op_str` ≈ `{python} MnistTrainingStepCalc.mnist_t_overhead_us_str`
...
- `torch.compile` can provide 2--3$\times$ speedup in favorable small-model cases by fusing operations and reducing kernel launches
...
class Model7BMemory:
    ...
    a100_mem = Hardware.Cloud.A100.memory.capacity
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # Subtract against the branded "80 GB" label the prose shows, not the raw
    # binary magnitude, so the displayed arithmetic closes for the reader.
    a100_mem_budget = a100_mem.to(GiB) * (GB / GiB)
    fp16 = params * BYTES_FP16  # 14 GB
    remaining = a100_mem_budget - fp16
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(
    ...
    a100_remaining_7b_gb_str = fmt_memory(remaining, unit=GB, precision=0, commas=False)
    a100_mem_gb_str = fmt_memory_capacity(a100_mem, unit=GiB, precision=0, commas=False)
```
