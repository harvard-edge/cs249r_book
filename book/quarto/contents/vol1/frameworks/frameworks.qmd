...
print(x.grad, y.grad, z.grad)  # tensor([3.]), tensor([2.]), tensor([1.])
```
...
for inputs, targets in dataloader:
    ...
    scaler.step(optimizer)  # grads /= scale_factor, then optimizer.step()
...
class GradScaler:
    ...
    def step(self, optimizer):
        ...
        found_inf = any(torch.isinf(p.grad).any() or torch.isnan(p.grad).any()
...
for param_id, param_state in state['state'].items():
    ...
    print(f"  Momentum buffer shape: {param_state['momentum_buffer'].shape}")
...
for param_id, param_state in state['state'].items():
    print(f"Parameter {param_id}:")
    print(f"  Step: {param_state['step']}")  # Iteration count for bias correction
    print(f"  exp_avg shape: {param_state['exp_avg'].shape}")  # m_t (first moment)
    print(f"  exp_avg_sq shape: {param_state['exp_avg_sq'].shape}")  # v_t (second moment)
```
...
optimizer.load_state_dict(torch.load('ckpt.pt')['opt'])  # May fail or load incorrectly!

# CORRECT: Load parameters first, then create optimizer
new_model = MyModel()
new_model.load_state_dict(torch.load('ckpt.pt')['model'])
optimizer = optim.Adam(new_model.parameters())
optimizer.load_state_dict(torch.load('ckpt.pt')['opt'])  # Works correctly
```
