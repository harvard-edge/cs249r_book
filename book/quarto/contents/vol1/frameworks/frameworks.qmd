...
# │
# │ Why: Establishes that BLAS remains the foundation of modern ML; shows the
# │      ~312 TFLOPS (dense) / 624 TFLOPS (sparse) capabilities of A100 tensor
# │      cores that vendor libraries like cuBLAS exploit.
# │
# │ Imports: physx.constants (A100_FLOPS_FP16_TENSOR), physx.formatting (fmt)
# │ Exports: a100_tflops_fp16_str, a100_tflops_sparse_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import A100_FLOPS_FP16_TENSOR, TFLOPs, second
from physx.formatting import fmt

# --- Inputs (from hardware specs) ---
a100_tflops_fp16_value = A100_FLOPS_FP16_TENSOR.to(TFLOPs/second).magnitude  # 312 TFLOPS dense

# --- Outputs (formatted strings for prose) ---
a100_tflops_fp16_str = fmt(a100_tflops_fp16_value, precision=0, commas=False)  # e.g. "312"
a100_tflops_sparse_str = fmt(a100_tflops_fp16_value * 2, precision=0, commas=False)  # e.g. "624" sparse
```
...
The architectural foundations established in @sec-dnn-architectures defined *what* computations neural networks perform. Knowing *what* to compute, however, is fundamentally different from knowing *how* to compute it efficiently. A Transformer's attention mechanism (introduced in @sec-dnn-architectures) requires coordinating computation across memory hierarchies and accelerator cores in patterns that naive implementations would execute 100× slower than optimized ones. Implementing these operations from scratch for every model would make deep learning economically infeasible. ML frameworks exist to bridge this gap by translating high-level model definitions into hardware-specific execution plans that extract maximum performance from silicon.
...
In the context of the **Iron Law** (@sec-silicon-contract), a framework is fundamentally a **Compiler for the Silicon Contract**\index{Framework!compiler for the Silicon Contract}.
...
1. **Hardware Primitives (1979–1992)**: The **Basic Linear Algebra Subprograms (BLAS)**\index{BLAS!historical foundation}[^fn-blas] established standardized, hardware-optimized implementations of operations like matrix multiplication (`GEMM`) [@lawson1979blas]. **LAPACK** extended this with higher-level solvers. These libraries remain the hidden foundation of every modern ML system; vendors like Intel (MKL) and NVIDIA (cuBLAS) provide highly tuned versions for their silicon (see @sec-ai-acceleration for the hardware acceleration hierarchy).

[^fn-blas]: **BLAS (Basic Linear Algebra Subprograms)**: A standardized specification (Level 1: vector operations; Level 2: matrix-vector; Level 3: matrix-matrix like GEMM) published in 1979 that defines portable APIs for dense linear algebra. Hardware vendors implement optimized BLAS libraries: Intel MKL achieves near-peak FLOPS on x86 CPUs through AVX-512 vectorization, while NVIDIA cuBLAS uses Tensor Cores for up to `{python} a100_tflops_fp16_str` TFLOPS (FP16/BF16/TF32) on A100 GPUs, or `{python} a100_tflops_sparse_str` TFLOPS with structured sparsity. This 45-year-old interface remains the performance foundation of modern ML frameworks.

2. **Vectorized Productivity (2006)**: **NumPy**[^fn-numpy] made Python viable for numerical computing by delegating heavy computation to underlying C and Fortran BLAS libraries. This "vectorization" approach (writing code in high-level Python but executing it in low-level C) became the dominant pattern, drastically reducing the gap between research ideas and execution speed. @sec-benchmarking-ai quantifies this trade-off.
...
Each generation abstracted away details that consumed engineering effort in the previous one, yet each abstraction introduced new trade-offs. BLAS hid assembly-level optimization but fixed the interface. NumPy hid memory management but required manual differentiation. Modern frameworks hide gradient computation but introduce the execution model choice we examine next. All modern frameworks converge on the same three fundamental problems: *how* to execute computation, *how* to differentiate it, and *how* to abstract across hardware. The sections that follow examine each problem in turn.

## The Execution Problem {#sec-ai-frameworks-execution-problem-e1e1}

\index{Execution Problem!definition}
The execution problem asks a seemingly simple question: should operations execute immediately as written, or be recorded for later execution? This choice creates a cascade of engineering trade-offs shaping every aspect of framework behavior.
...
# │
# │ Why: Demonstrates the fundamental compute-vs-bandwidth imbalance that
# │      drives framework design decisions. The 312 TFLOPS vs 2 TB/s ratio
# │      explains why memory-bound ops achieve <1% compute utilization.
# │
# │ Imports: physx.constants (A100_FLOPS_FP16_TENSOR, A100_MEM_BW)
# │ Exports: a100_tflops_fp16_str, a100_bw_tbs_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import A100_FLOPS_FP16_TENSOR, A100_MEM_BW, TFLOPs, TB, second
from physx.formatting import fmt

# --- Inputs (from hardware specs) ---
a100_tflops_fp16_value = A100_FLOPS_FP16_TENSOR.to(TFLOPs/second).magnitude  # 312 TFLOPS
a100_bw_tbs_value = A100_MEM_BW.to(TB/second).magnitude                       # ~2 TB/s

# --- Outputs (formatted strings for prose) ---
a100_tflops_fp16_str = fmt(a100_tflops_fp16_value, precision=0, commas=False)  # e.g. "312"
a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=1, commas=False)            # e.g. "2.0"
```

\index{Memory Wall!execution strategy impact}
To understand why execution strategy matters so much, consider the **Memory Wall**\index{Memory Wall!definition} (first introduced in @sec-deep-learning-systems-foundations), the growing gap between processor computational speed and memory bandwidth. Modern GPUs can perform arithmetic far faster than they can fetch data. On an A100 GPU with `{python} a100_tflops_fp16_str` TFLOPS of compute and `{python} a100_bw_tbs_str` TB/s of memory bandwidth, element-wise operations like ReLU achieve less than 1% of peak compute capacity, not because the hardware is slow, but because they spend nearly all their time waiting for data. The Roofline Model (@sec-system-foundations-roofline-model-5f7c) formalizes this trade-off, showing exactly when operations are memory-bound versus compute-bound.
...
**The critical constraint**: a framework can only fuse operations it can see together. If operations execute immediately one at a time (eager execution)\index{Eager Execution!optimization limitations}, the framework cannot fuse them. If operations are recorded first into a graph (deferred execution)\index{Graph Execution!optimization advantages}, the framework can analyze and optimize the entire computation. This is why execution strategy matters so much: it determines what optimizations are even possible.

### The Computational Graph {#sec-ai-frameworks-computational-graph-00f7}

The previous section established that kernel fusion is the key optimization for memory-bound operations, but fusion requires seeing multiple operations together. How do frameworks represent computation in a way that makes this visibility possible? The answer is the **computational graph**\index{Computational Graph!definition (DAG)}, a directed acyclic graph (DAG) where nodes represent operations and edges represent data dependencies. This graph is the framework's internal model of your computation. To ground this abstraction, examine @fig-comp-graph: computing $z = x \times y$ maps onto two input nodes ($x$ and $y$), one operation node (multiplication), and one output node ($z$). The execution problem asks: *when* is this graph constructed, and *when* is it executed?
...
This graph representation is more than a visualization; it is the data structure that enables both efficient execution and automatic differentiation. The answer to *when* this graph is constructed creates a fundamental design choice with cascading implications:
...
The computational graph representation is powerful, but it raises a critical design question: *when* should the framework build this graph? Consider a simple operation like `y = x * 2`. Two fundamentally different approaches exist:
...
This provides the flexibility of normal programming: you can print intermediate values, use conditionals based on computed results, and debug with standard tools. The framework records operations as they happen, constructing a **dynamic graph**\index{Dynamic Graphs!runtime construction} that reflects the actual execution path taken.
...
**Systems Implications: Overhead.** The flexibility of eager execution with autograd tape comes with performance costs. Each overhead category maps to a specific term in the Iron Law of performance (introduced in @sec-silicon-contract), revealing why eager execution creates a performance ceiling for small models:

- **Graph construction overhead** ($L_{lat}$): Each forward pass rebuilds the autograd tape from scratch, adding Python object creation, reference counting, and node linking cost to every iteration.
- **Python interpreter overhead** ($L_{lat}$): Every operation goes through Python dispatch, including function lookup, argument parsing, and type checking. At ~10μs per operation, this becomes significant for models with thousands of operations.
- **Limited optimization opportunities** ($O$, $D_{vol}$): Because the graph is built during execution, the framework cannot optimize across operations. Each operation launches its own GPU kernel, preventing kernel fusion that would reduce both operation count and memory traffic.
- **Memory overhead** ($D_{vol}$): The autograd tape stores references to all intermediate tensors and Function nodes, increasing memory consumption by 2--3$\times$ compared to forward-only execution and adding pressure to the memory hierarchy.
...
The fundamental difference from eager execution is that during construction, `x`, `y`, and `z` are not tensors containing values but rather symbolic nodes in a graph. Operations like `*` and `+` add nodes to the graph definition without performing any arithmetic. The `print(y)` line in the code example would reveal this distinction—it would print tensor metadata, not a computed value. Execution is triggered explicitly through `sess.run()`, at which point the framework analyzes the complete graph, optimizes it, and executes the optimized version with the provided input data.

**Ahead-of-Time Optimization.**\index{Ahead-of-Time (AOT) Optimization} Because the framework has the complete graph before execution, it can perform optimizations impossible in eager mode:

- **Kernel fusion**\index{Kernel Fusion!static graph optimization} (also called operation fusion or operator fusion): Combine `y = x * 2` and `z = y + 1` into a single fused kernel `z = x * 2 + 1`, eliminating the intermediate `y` and reducing memory traffic by 50%. This directly addresses the Memory Wall constraint introduced in @sec-ai-frameworks-execution-strategy-matters-memory-wall-1ce8.
...
- **Dead code elimination**\index{Dead Code Elimination}: Remove operations not needed to compute the requested outputs.
- **Constant folding**\index{Constant Folding}: Pre-compute operations on constant values at graph construction time.
...
**Systems Implications.**\index{Static Graphs!optimization benefits} Static graphs achieve high performance through ahead-of-time optimization. Kernel fusion reduces memory bandwidth requirements (often the bottleneck for ML workloads), and hardware-specific compilation enables near-peak utilization.
...
PyTorch's TorchScript\index{TorchScript!tracing vs. scripting} exemplifies both strategies. Tracing\index{JIT Compilation!tracing} executes a function once with example inputs and records every tensor operation into a static computation graph. @lst-torchscript-trace demonstrates the approach: the traced module becomes a compiled artifact that can be serialized, optimized, and executed independently of the Python interpreter:
...
The alternative, scripting\index{JIT Compilation!scripting}\index{TorchScript!IR compilation}, achieves generality by analyzing Python source code directly and compiling it to TorchScript IR without executing. The scripting compiler parses the abstract syntax tree (AST), converts supported operations to IR operations, and preserves the branching structure so that both branches of a conditional exist in the compiled representation. The cost of this generality is a restricted Python subset: type annotations are required where inference fails, arbitrary Python objects and standard library modules are excluded, and dynamic metaprogramming is forbidden.
...
To understand what the compiler produces, you can inspect the generated intermediate representation directly.
...
\index{torch.compile!PyTorch 2.0}
...
# │
# │ Why: Quantifies why kernel fusion matters—eager mode pays 2x kernel launch
# │      overhead and 2x memory traffic. Shows 2x speedup from fusing Add+ReLU.
# │
...
# --- Inputs (typical overhead values) ---
python_dispatch_us = 10                                                         # ~10 μs per op
kernel_launch_us = 5                                                            # ~5 μs per op
memory_access_us = 1                                                            # ~1 μs (VRAM)
kernel_launch_us_value = python_dispatch_us + kernel_launch_us                  # combined overhead

eager_n_ops_value = 2                                                           # 2 kernel launches
compiled_n_ops_value = 1                                                        # 1 fused kernel
eager_mem_factor_value = 4                                                      # 4N bytes (2R + 2W)
compiled_mem_factor_value = 2                                                   # 2N bytes (fused)

# --- Process ---
eager_overhead_us_value = eager_n_ops_value * kernel_launch_us_value           # 30 μs
compiled_overhead_us_value = compiled_n_ops_value * kernel_launch_us_value     # 15 μs
overhead_speedup_value = eager_overhead_us_value // compiled_overhead_us_value # 2x
bw_efficiency_value = eager_mem_factor_value // compiled_mem_factor_value      # 2x

# --- Outputs (formatted strings for prose) ---
python_dispatch_us_str = f"{python_dispatch_us}"                                # e.g. "10"
kernel_launch_only_us_str = f"{kernel_launch_us}"                               # e.g. "5"
memory_access_us_str = f"{memory_access_us}"                                    # e.g. "1"
kernel_launch_us_str = f"{kernel_launch_us_value}"                              # e.g. "15"
eager_overhead_str = f"{eager_overhead_us_value}"                               # e.g. "30"
compiled_overhead_str = f"{compiled_overhead_us_value}"                         # e.g. "15"
overhead_speedup_str = f"{overhead_speedup_value}"                              # e.g. "2"
bw_efficiency_str = f"{bw_efficiency_value}"                                    # e.g. "2"
```
...
*   **Python Dispatch:** ~`{python} python_dispatch_us_str` μs per operation.
*   **Kernel Launch:** ~`{python} kernel_launch_only_us_str` μs per operation.
*   **Memory Access (VRAM):** ~`{python} memory_access_us_str` μs.
...
*   **Execution:**
    1.  Launch `Add` Kernel: `{python} kernel_launch_us_str` µs overhead.
    2.  Read/Write Memory: $2N$ bytes.
    3.  Launch `ReLU` Kernel: `{python} kernel_launch_us_str` µs overhead.
...
*   **Total Memory Traffic:** 2N bytes (**`{python} bw_efficiency_str`× bandwidth efficiency**).
...
3. **TorchInductor**\index{TorchInductor!code generation} (code generation): The default backend that compiles FX graphs to optimized machine code. For CUDA GPUs, TorchInductor generates Triton\index{Triton!GPU kernel language}[^fn-triton] kernels, a Python-based GPU kernel language that compiles to PTX. For CPUs, it generates C++ code with vectorization instructions (AVX2, AVX-512). TorchInductor applies three key optimizations: kernel fusion (combining operations to reduce memory traffic), memory layout optimization (choosing tensor layouts that minimize access overhead), and autotuning (measuring performance across implementation variants to select the fastest).
...
**Graph Breaks: Causes and Detection.**\index{Graph Breaks!causes and detection} Graph breaks occur when torch.compile encounters code it cannot compile, forcing execution to fall back to eager mode. Understanding graph break causes provides the foundation for achieving good performance.
...
- **backend='onnxrt'**\index{ONNX Runtime!torch.compile backend}: Exports FX graph to ONNX format and executes using ONNX Runtime. Enables cross-platform deployment (CPU, GPU, mobile, edge devices) but may cause more graph breaks due to limited ONNX operation coverage.
- **backend='tensorrt'**\index{TensorRT!torch.compile backend}: Compiles to NVIDIA TensorRT inference engine with aggressive optimizations (int8 quantization, layer fusion, kernel autotuning). Inference-only (no backward pass), NVIDIA GPUs only, often achieves 1.5--2$\times$ speedup over TorchInductor for inference.
...
: **Additional Graph Trade-Offs.** Systems-level distinctions between static and dynamic graphs that complement the execution model comparison above. These trade-offs reappear when selecting frameworks in @sec-ai-frameworks-selecting-framework-2949. {#tbl-mlfm-graphs}
...
**The Dispatch Overhead Law.**\index{Dispatch Overhead!law} A second principle emerges from the Dispatch Overhead Equation (@eq-dispatch-overhead): when does framework overhead, rather than compute or memory, dominate execution time? Framework overhead dominates when operations are small relative to dispatch cost:
...
# │
# │ Why: Demonstrates that small models spend 92% of time in dispatch overhead
# │      (ratio ~11.5), while GPT-3 layers spend <0.05% (ratio 0.0005). This
# │      explains why compilation benefits small models disproportionately.
# │
# │ Imports: physx.formatting (fmt, md_math)
...
# --- Inputs: Scenario 1 (Small MLP - Overhead Bound) ---
dispatch_n_ops_value = 6                                                        # 6 small ops
dispatch_us_per_op_value = 5                                                    # 5 μs dispatch/op
dispatch_hw_time_us_value = 2.6                                                 # 2.6 μs actual compute

# --- Inputs: Scenario 2 (GPT-3 Layer - Compute Bound) ---
gpt3_hw_time_us_value = 100_000                                                 # 100 ms hardware time
gpt3_sw_time_us_value = 50                                                      # 50 μs dispatch overhead

# --- Process: Scenario 1 ---
dispatch_sw_time_value = dispatch_n_ops_value * dispatch_us_per_op_value       # 30 μs overhead
dispatch_ratio_small_value = dispatch_sw_time_value / dispatch_hw_time_us_value  # ~11.5
dispatch_total_us_value = dispatch_sw_time_value + dispatch_hw_time_us_value   # 32.6 μs total
dispatch_overhead_pct_value = dispatch_sw_time_value / dispatch_total_us_value * 100  # ~92%
dispatch_compilation_speedup_value = dispatch_total_us_value / dispatch_hw_time_us_value  # ~13x

# --- Process: Scenario 2 ---
dispatch_ratio_large_value = gpt3_sw_time_us_value / gpt3_hw_time_us_value     # 0.0005

# --- Outputs (formatted strings for prose) ---
dispatch_ratio_small_str = fmt(dispatch_ratio_small_value, precision=1, commas=False)  # e.g. "11.5"
dispatch_sw_time_str = fmt(dispatch_sw_time_value, precision=0, commas=False)  # e.g. "30"
dispatch_ratio_large_str = fmt(dispatch_ratio_large_value, precision=4, commas=False)  # e.g. "0.0005"
dispatch_overhead_pct_str = fmt(dispatch_overhead_pct_value, precision=0, commas=False)  # e.g. "92"
dispatch_compilation_speedup_str = fmt(dispatch_compilation_speedup_value, precision=0, commas=False)  # e.g. "13"
t_sw_md = md_math(f"T_{{sw}} \\approx {gpt3_sw_time_us_value} \\, \\mu s")
```
...
*   **Compute**: `{python} dispatch_n_ops_value` small matrix/element-wise operations.
*   **Hardware Time**: T_hw ≈ `{python} dispatch_hw_time_us_value` μs (mostly memory latency).
*   **Software Overhead**: T_sw ≈ `{python} dispatch_n_ops_value` ops × `{python} dispatch_us_per_op_value` μs/op = `{python} dispatch_sw_time_str` μs.
*   **Ratio**: `{python} dispatch_sw_time_str` / `{python} dispatch_hw_time_us_value` ≈ **`{python} dispatch_ratio_small_str`**.
...
*   **Hardware Time**: T_hw ≈ 100 ms = `{python} gpt3_hw_time_us_value` μs.
*   **Software Overhead**: `{python} t_sw_md`.
*   **Ratio**: `{python} gpt3_sw_time_us_value` / `{python} gpt3_hw_time_us_value` ≈ **`{python} dispatch_ratio_large_str`**.
...
# │
# │ Why: Demonstrates that compilation benefits scale inversely with model size;
# │      small models are overhead-bound while GPT-3 scale is compute-bound,
# │      so dispatch overhead becomes negligible at 175B parameters.
# │
# │ Imports: physx.constants (GPT3_PARAMS), physx.formatting (fmt)
...
**The Principle's Implication**: Small models benefit *disproportionately* from compilation. A 100-parameter toy model might see 10× speedup from torch.compile, while a `{python} gpt3_params_b_str` B-parameter model sees only 1.3×. This explains why compilation matters most for efficient inference on smaller, deployed models.

The dispatch tax analysis reveals that small operations are overhead-bound regardless of hardware capability. This observation has particular significance at the extreme edge of the deployment spectrum, where the entire Python runtime is itself an unacceptable overhead.
...
Micro-frameworks like **TensorFlow Lite Micro (TFLM)**\index{TensorFlow Lite Micro!extreme AOT} and **PyTorch ExecuTorch**\index{ExecuTorch} solve this through **Extreme AOT Compilation**:
...
This spectrum of execution strategies, from dynamic eager execution to static graph compilation and specialized micro-runtimes, requires developers to make deliberate trade-offs. The following checkpoint summarizes the key decision points before we turn to the second fundamental problem.
...
Before examining framework implementations, we must understand the two fundamental approaches to automatic differentiation. This choice (forward mode versus reverse mode) determines whether gradient computation scales with the number of inputs or the number of outputs, a distinction that explains why neural network training universally uses one mode over the other.
...
Although neural network training universally uses reverse mode (covered next), understanding forward mode illuminates *why* reverse mode is necessary. Forward mode automatic differentiation computes derivatives alongside the original computation, tracking how changes propagate from input to output. This approach mirrors manual derivative computation, making it intuitive to understand and implement.
...
Given forward mode's $O(N)$ scaling with parameter count, we need a fundamentally different approach for training. Reverse mode provides exactly this: by propagating gradients backward from output to input, it computes all $N$ parameter gradients in a single pass.
...
\index{Backpropagation|see{Reverse Mode AD}}
Why does every modern ML framework default to reverse mode for training? The answer is computational asymmetry. Understanding this asymmetry explains one of the most important design decisions in all of machine learning software.
...
The critical observation is that this single backward pass computed dc/dx regardless of how many paths connect x to c. In a neural network, each weight can affect the loss through thousands of paths across layers, and reverse mode handles them all in one traversal. This is why training a `{python} gpt3_params_b_str` B parameter model like GPT-3 is feasible at all: reverse mode's O(1) backward passes (relative to parameter count) keeps gradient computation tractable.
...
# │
# │ Why: Shows that even storing just weights for a 175B model requires 350 GB
# │      (FP16), far exceeding any single GPU. Establishes memory as the binding
# │      constraint on what models frameworks can train.
# │
# │ Imports: physx.constants (GPT3_PARAMS, BYTES_FP16), physx.formulas (model_memory)
# │ Exports: gpt3_params_b_str, gpt3_fp16_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import GPT3_PARAMS, BYTES_FP16, GB, Bparam
...
Frameworks attack this memory wall with two primary strategies. The first is *activation checkpointing*\index{Gradient Checkpointing!memory-compute trade-off}\index{Activation Checkpointing|see{Gradient Checkpointing}} (also called gradient checkpointing): rather than storing every activation, the framework stores only selected checkpoints and recomputes the intermediate activations during the backward pass. @sec-ai-training examines checkpointing strategies in detail, including optimal checkpoint placement algorithms. @lst-checkpoint_scheme shows the pattern: save activations at checkpoint boundaries, recompute everything between them.
...
The second strategy is *operation fusion*\index{Operation Fusion|see{Kernel Fusion}}[^fn-operation-fusion]. Rather than executing matrix multiplication, bias addition, and ReLU as three separate operations, each writing intermediate results to memory, frameworks fuse them into a single kernel. This eliminates intermediate memory allocations entirely and achieves 2 to 3x speedup on modern GPUs by keeping data in registers and caches.
...
Checkpointing, fusion, and specialized kernels solve the systems problems of AD. Practitioners, however, never interact with these mechanisms directly. Instead, frameworks expose AD through high-level APIs that hide the underlying machinery behind simple method calls.
...
\index{Autograd!PyTorch internals}
...
\index{grad\_fn!computation graph}
...
# │
# │ Why: Concrete example showing that training ResNet-50 requires 10-100x more
# │      memory than inference, due to storing activations for backward pass.
# │      The ~100x ratio (102 MB inference vs 10-15 GB training) quantifies why
# │      Data Movement dominates training in the Iron Law.
# │
# │ Imports: physx.constants (RESNET50_PARAMS), physx.formulas (model_memory)
...
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import RESNET50_PARAMS, BYTES_FP32, BYTES_ADAM_STATE, MB, Mparam
from physx.formulas import model_memory
from physx.formatting import fmt

# --- Inputs (from model specs) ---
resnet_params_m_value = RESNET50_PARAMS.to(Mparam).magnitude                   # 25.6 million

# --- Process ---
resnet_fp32_mb_value = model_memory(RESNET50_PARAMS, BYTES_FP32, MB)           # ~102 MB
resnet_adam_mb_value = model_memory(RESNET50_PARAMS, BYTES_ADAM_STATE, MB)     # ~410 MB
resnet_training_min_gb_value = 10                                               # typical minimum
resnet_training_max_gb_value = 15                                               # typical maximum
resnet_training_ratio_min_value = resnet_training_min_gb_value * 1024 / resnet_fp32_mb_value  # ~100x

# --- Outputs (formatted strings for prose) ---
resnet_params_m_str = fmt(resnet_params_m_value, precision=1, commas=False)    # e.g. "25.6"
resnet_fp32_mb_str = fmt(resnet_fp32_mb_value, precision=0, commas=False)      # e.g. "102"
resnet_adam_mb_str = fmt(resnet_adam_mb_value, precision=0, commas=False)      # e.g. "410"
resnet_training_min_gb_str = fmt(resnet_training_min_gb_value, precision=0, commas=False)  # e.g. "10"
resnet_training_max_gb_str = fmt(resnet_training_max_gb_value, precision=0, commas=False)  # e.g. "15"
resnet_training_ratio_str = fmt(resnet_training_ratio_min_value, precision=0, commas=False)  # e.g. "100"
```

For a network with $L$ layers, the system must save approximately $L$ activation tensors, one per layer, for the entire batch. Consider a concrete example: ResNet-50 has `{python} resnet_params_m_str` M parameters (~`{python} resnet_fp32_mb_str` MB in FP32) and processes batch size 64 with 224x224 images. The memory breakdown reveals the scale of this trade-off:

- **Forward activations**: ~8-12 GB (varies by implementation and checkpointing)
- **Parameter gradients**: ~`{python} resnet_fp32_mb_str` MB (same size as parameters)
- **Optimizer state (Adam)**: ~`{python} resnet_adam_mb_str` MB (two momentum buffers per parameter)
- **Total training footprint**: ~`{python} resnet_training_min_gb_str`–`{python} resnet_training_max_gb_str` GB versus ~`{python} resnet_fp32_mb_str` MB for inference

This `{python} resnet_training_ratio_str`x ratio between training and inference memory quantifies why the Data Movement ($D_{vol}$) term dominates training latency in the **Iron Law**. During training, the framework must write all activations to memory during the forward pass and read them back during the backward pass, doubling the memory traffic compared to inference alone.
...
**Retaining the Computation Graph.**\index{retain\_graph!multiple backward passes} By default, `backward()` frees the graph after use. To run multiple backward passes (for multi-loss optimization or higher-order derivatives), use `retain_graph=True` at the cost of doubled memory, as shown in @lst-retain-graph.
...
**Gradient Accumulation Behavior.** Gradients accumulate across backward passes by default. As @lst-gradient-accumulation demonstrates, without calling `zero_grad()`, successive backward passes sum their gradients:
...
**Custom Autograd Functions.**\index{Custom Autograd Functions!backward implementation} When implementing custom operations, you explicitly specify what to save for the backward pass and how to compute gradients. @lst-custom-autograd-function shows the pattern:
...
**Gradient Hooks.**\index{Gradient Hooks!inspection and modification} Register hooks on tensors to inspect or modify gradients during backpropagation, as shown in @lst-gradient-hooks:
...
**Detach vs. Data.**\index{Tensor!detach() method}\index{Gradient Flow!breaking with detach} Use `.detach()` to safely break gradient flow. @lst-detach-vs-data illustrates how the legacy `.data` attribute can silently corrupt gradient computation through in-place operations:
...
**Mixed-Precision Training Support.**\index{Mixed Precision!FP16 vs. FP32 trade-offs} Mixed precision exploits a hardware asymmetry to improve two Iron Law terms simultaneously: Tensor Cores execute FP16 matrix multiplications at 2x the throughput of FP32 (increasing effective $O/R_{peak}$), while FP16 activations halve the memory footprint (reducing $D_{vol}$). Improving both terms simultaneously is rare; most optimizations improve one at the expense of the other.
...
# │
# │ Why: Shows that Adam optimizer quintuples memory footprint—7B params need
# │      14 GB weights (FP16) + 56 GB optimizer state (2×FP32) = 70 GB total.
# │      This is why checkpoint size bounds fault tolerance.
# │
# │ Imports: physx.constants (BYTES_FP16, BYTES_ADAM_STATE), physx.formulas (model_memory)
# │ Exports: model_7b_fp16_gb_str, model_7b_adam_gb_str, model_7b_total_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import BYTES_FP16, BYTES_ADAM_STATE, GB
from physx.formulas import model_memory
from physx.formatting import fmt
...
model_7b_adam_gb_value = model_memory(model_7b_params, BYTES_ADAM_STATE, GB)   # 56 GB
model_7b_total_gb_value = model_7b_fp16_gb_value + model_7b_adam_gb_value      # 70 GB

# --- Outputs (formatted strings for prose) ---
model_7b_fp16_gb_str = fmt(model_7b_fp16_gb_value, precision=0, commas=False)  # e.g. "14"
model_7b_adam_gb_str = fmt(model_7b_adam_gb_value, precision=0, commas=False)  # e.g. "56"
model_7b_total_gb_str = fmt(model_7b_total_gb_value, precision=0, commas=False)  # e.g. "70"
```

**Optimizer State and Checkpointing.** Resuming training after interruption requires restoring not just model weights but optimizer state: momentum buffers, adaptive learning rates, and gradient statistics. For Adam, optimizer state typically quintuples the memory footprint beyond weights alone (since two FP32 states are stored for each FP16 parameter), meaning a 7B-parameter model requires approximately `{python} model_7b_total_gb_str` GB total (`{python} model_7b_fp16_gb_str` GB weights + `{python} model_7b_adam_gb_str` GB optimizer state). Checkpoint size therefore bounds recovery speed after failure, connecting fault tolerance directly to the Iron Law's $D_{vol}$ term.
...
Modern frameworks track the lifetime of each intermediate value automatically, freeing memory as soon as it is no longer needed for gradient computation. Even with precise lifetime tracking, however, a deeper problem remains: the cost of acquiring memory from the GPU in the first place.
...
The memory and system integration challenges examined above (caching allocators, activation storage, and checkpoint overhead) affect all frameworks. Yet *how* frameworks implement automatic differentiation in the first place varies significantly, with consequences for both *optimization potential* and developer experience. The distinction between *tape-based and transform-based autodiff* captures this fundamental divergence.
...
The execution and differentiation problems together enable the training loop: the execution model determines when computation happens, while automatic differentiation computes the gradients that drive learning. Both problems, however, assume that the same code can run across diverse hardware: CPUs, GPUs, TPUs, mobile devices, and microcontrollers. This hardware diversity creates the third and final fundamental problem.
...
The execution problem told us *when* to compute; the differentiation problem told us *how* to train. But both assumed something that cannot be taken for granted: that the same code can run across wildly different hardware. The abstraction problem asks: how should frameworks target diverse hardware from a single programming interface? The same model definition should run on CPUs, GPUs, TPUs, mobile devices, and microcontrollers, each with radically different capabilities, memory constraints, and optimal execution patterns. A GPU offers 1000× the parallelism of a CPU but has different memory semantics. A TPU provides higher throughput but requires static shapes. A microcontroller has kilobytes where a server has gigabytes. Frameworks must provide abstractions that hide this hardware complexity while enabling efficient utilization.
...
# │
# │ Why: Grounds the abstraction discussion in concrete numbers—25.6M parameters
# │      that must be managed without programmer intervention. Motivates why
# │      frameworks need sophisticated data structures.
# │
# │ Imports: physx.constants (RESNET50_PARAMS), physx.formatting (fmt)
...
A ResNet-50 forward pass touches `{python} resnet_params_m_str` million parameters, produces intermediate activations at every layer, and must coordinate memory across CPU and GPU address spaces. How do frameworks organize all of this data so that a single Python call like `model(input)` executes millions of operations without the programmer managing a single pointer?

The answer lies in a hierarchy of specialized data structures that we examine in this section. Computational graphs specify the logical flow of operations, but data structures determine how those operations access and manipulate data in physical memory. This distinction matters because the same mathematical operation can differ by an order of magnitude in throughput depending on whether data is contiguous in cache, pinned for DMA transfer, or scattered across pages.
...
# │
# │ Why: Demonstrates that a 2 GB model (1B params in FP16) actually requires
# │      ~19 GB during training: +2 GB gradients, +8 GB optimizer, +6.7 GB
# │      activations. This is why training is memory-constrained.
# │
# │ Imports: physx.constants (BYTES_FP16, BYTES_ADAM_STATE, GB), physx.formatting (fmt, md_math)
...
# --- Inputs (scenario: 1B parameter model) ---
admin_params_count = 1e9                                                        # 1 billion params
admin_batch_size = 32                                                           # batch size
admin_layers = 100                                                              # 100-layer network
admin_width = 1024                                                              # hidden dimension

# --- Process: memory breakdown ---
admin_weights_gb_value = (admin_params_count * BYTES_FP16).to(GB).magnitude    # 2 GB
admin_grads_gb_value = admin_weights_gb_value                                   # 2 GB (same as weights)
admin_opt_gb_value = (admin_params_count * BYTES_ADAM_STATE).to(GB).magnitude  # 8 GB (Adam: 2 FP32 states)
admin_act_bytes = admin_batch_size * admin_layers * (admin_width ** 2) * BYTES_FP16
admin_act_gb_value = admin_act_bytes.to(GB).magnitude                          # ~6.7 GB

admin_total_gb_value = admin_weights_gb_value + admin_grads_gb_value + admin_opt_gb_value + admin_act_gb_value
admin_tax_gb_value = admin_total_gb_value - admin_weights_gb_value             # ~17 GB "tax"

# --- Outputs (formatted strings for prose) ---
admin_weights_gb_str = fmt(admin_weights_gb_value, precision=0, commas=False)  # e.g. "2"
admin_grads_gb_str = fmt(admin_grads_gb_value, precision=0, commas=False)      # e.g. "2"
admin_opt_gb_str = fmt(admin_opt_gb_value, precision=0, commas=False)          # e.g. "8"
admin_act_gb_str = fmt(admin_act_gb_value, precision=1, commas=False)          # e.g. "6.7"
admin_total_gb_str = fmt(admin_total_gb_value, precision=0, commas=False)      # e.g. "19"
admin_tax_gb_str = fmt(admin_tax_gb_value, precision=0, commas=False)          # e.g. "17"
admin_batch_str = fmt(admin_batch_size, precision=0, commas=False)             # e.g. "32"
admin_layers_str = fmt(admin_layers, precision=0, commas=False)                # e.g. "100"
admin_width_str = fmt(admin_width, precision=0, commas=False)                  # e.g. "1024"
...
# │
# │ Why: Quantifies the 60x+ bandwidth gap between PCIe (32 GB/s) and HBM (2 TB/s)
# │      that explains why careless device placement erases GPU speedups. Essential
# │      for understanding the Iron Law's data movement term.
# │
# │ Imports: physx.constants (PCIE_GEN4_BW, NVLINK_A100_BW, A100_MEM_BW, A100_MEM_CAPACITY,
# │          A100_FLOPS_FP16_TENSOR), physx.formatting (fmt)
...
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import (PCIE_GEN4_BW, NVLINK_A100_BW, A100_MEM_BW, A100_MEM_CAPACITY,
                             A100_FLOPS_FP16_TENSOR, GB, TB, GiB, TFLOPs, flop, byte, second)
from physx.formatting import fmt

# --- Inputs (from hardware specs) ---
pcie4_gbs_value = PCIE_GEN4_BW.to(GB/second).magnitude                         # 32 GB/s unidirectional
nvlink_a100_gbs_value = NVLINK_A100_BW.to(GB/second).magnitude                 # 600 GB/s bidirectional
a100_bw_gbs_value = A100_MEM_BW.to(GB/second).magnitude                        # ~2039 GB/s
a100_bw_tbs_value = A100_MEM_BW.to(TB/second).magnitude                        # ~2 TB/s
a100_mem_value = A100_MEM_CAPACITY.to(GiB).magnitude                           # 80 GB
a100_tflops_fp16_value = A100_FLOPS_FP16_TENSOR.to(TFLOPs/second).magnitude    # 312 TFLOPS

# --- Process: transfer times for 4 MB tensor ---
tensor_4mb = 4e6                                                                # bytes
pcie4_4mb_ms_value = (tensor_4mb / (PCIE_GEN4_BW.to(byte/second).magnitude)) * 1e3
nvlink_4mb_ms_value = (tensor_4mb / (NVLINK_A100_BW.to(byte/second).magnitude)) * 1e3
hbm_4mb_ms_value = (tensor_4mb / (A100_MEM_BW.to(byte/second).magnitude)) * 1e3

# --- Process: 1 GB transfer cost analysis ---
pcie4_1gb_ms_value = (1e9 / (PCIE_GEN4_BW.to(byte/second).magnitude)) * 1e3
pcie4_1gb_equiv_ops_value = (pcie4_1gb_ms_value / 1e3) * A100_FLOPS_FP16_TENSOR.to(flop/second).magnitude

# --- Outputs (formatted strings for prose) ---
pcie4_gbs_str = fmt(pcie4_gbs_value, precision=0, commas=False)                # e.g. "32"
pcie4_bidir_gbs_str = fmt(pcie4_gbs_value * 2, precision=0, commas=False)      # e.g. "64"
nvlink_a100_gbs_str = fmt(nvlink_a100_gbs_value, precision=0, commas=False)    # e.g. "600"
a100_bw_gbs_str = fmt(a100_bw_gbs_value, precision=0, commas=False)            # e.g. "2039"
a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=1, commas=False)            # e.g. "2.0"
a100_tflops_fp16_str = fmt(a100_tflops_fp16_value, precision=0, commas=False)  # e.g. "312"
a100_mem_str = fmt(a100_mem_value, precision=0, commas=False)                  # e.g. "80"
pcie4_4mb_ms_str = fmt(pcie4_4mb_ms_value, precision=3, commas=False)          # e.g. "0.125"
nvlink_4mb_ms_str = fmt(nvlink_4mb_ms_value, precision=3, commas=False)        # e.g. "0.007"
hbm_4mb_ms_str = fmt(hbm_4mb_ms_value, precision=3, commas=False)              # e.g. "0.002"
pcie4_1gb_ms_str = fmt(pcie4_1gb_ms_value, precision=0, commas=False)          # e.g. "31"
pcie4_1gb_equiv_ops_str = fmt((pcie4_1gb_equiv_ops_value * flop).to(TFLOPs).magnitude, precision=1, commas=False)  # e.g. "9.8"
```
...
The cost of moving data between devices varies by orders of magnitude depending on the interconnect.[^fn-nvlink] Before examining optimization strategies, we need to understand these costs quantitatively. @tbl-device-transfer-overhead shows transfer times for a 1000x1000 float32 tensor (4 MB)—roughly the size of a typical activation tensor in a moderately sized model. The numbers reveal why careless device placement can erase any speedup from GPU acceleration:

[^fn-nvlink]: **NVLink**: NVIDIA's high-bandwidth interconnect for GPU-to-GPU communication, providing `{python} nvlink_a100_gbs_str` GB/s bidirectional bandwidth (NVLink 3.0 on A100) compared to `{python} pcie4_bidir_gbs_str` GB/s bidirectional for PCIe 4.0 x16. Critical for multi-GPU training where gradient synchronization requires moving gigabytes per iteration. NVSwitch extends NVLink to connect 8 GPUs in a fully-connected topology (DGX systems), enabling all-to-all communication without bottlenecks. The ~10× bandwidth advantage over PCIe determines whether tensor parallelism is practical for a given model size.

| **Interconnect** |                                     **Bandwidth** |               **Transfer Time** | **Relative to Compute**     |
|:---------------|------------------------------------------------:|------------------------------:|:--------------------------|
| **PCIe 3.0 x16** |                                           16 GB/s |                         0.25 ms | 10x slower than GPU compute |
| **PCIe 4.0 x16** |                     `{python} pcie4_gbs_str` GB/s |  `{python} pcie4_4mb_ms_str` ms | 5x slower than GPU compute  |
...
\index{CUDA Events!synchronization}
...
Full device synchronization (`torch.cuda.synchronize()`)\index{torch.cuda.synchronize()} blocks all streams and the CPU until every queued operation completes. This creates a global serialization point that eliminates all overlap benefits. CUDA events\index{CUDA Events!fine-grained synchronization} provide the alternative: fine-grained synchronization that blocks only the dependent stream, allowing other streams and the CPU to continue execution (see @lst-cuda-events):
...
# │
# │ Why: Quantifies the throughput requirements for data pipelines (150 MB/s for
# │      1000 images/s at 224x224) and the cost of batch transfers (38 MB batch
# │      takes ~1.2 ms over PCIe 4.0). Shows why pin_memory matters for DMA.
# │
# │ Imports: physx.constants (PCIE_GEN4_BW, BYTES_FP32), physx.formatting (fmt)
# │ Exports: img_res, dataloader_mbs_str, batch_mb_str, batch_transfer_ms_str, pcie4_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import PCIE_GEN4_BW, BYTES_FP32, MB, GB, byte, second
from physx.formatting import fmt

# --- Inputs (from scenario) ---
img_per_sec = 1000                                                              # GPU consumption rate
img_res = 224                                                                   # standard ImageNet resolution
img_channels = 3                                                                # RGB channels
batch_size = 64                                                                 # typical training batch

# --- Process: dataloader throughput requirement ---
dataloader_mbs_value = (img_per_sec * img_res * img_res * img_channels * byte).to(MB).magnitude  # ~150 MB/s

# --- Process: batch transfer time ---
batch_bytes = batch_size * img_res * img_res * img_channels * BYTES_FP32.magnitude
batch_mb_value = (batch_bytes * byte).to(MB).magnitude                         # ~38 MB
batch_transfer_ms_value = batch_bytes / (PCIE_GEN4_BW.to(byte/second).magnitude) * 1e3  # ~1.2 ms

# --- Additional: PCIe bandwidth for reference ---
pcie4_gbs_value = PCIE_GEN4_BW.to(GB/second).magnitude                         # 32 GB/s

# --- Outputs (formatted strings for prose) ---
dataloader_mbs_str = fmt(dataloader_mbs_value, precision=0, commas=False)      # e.g. "150"
batch_mb_str = fmt(batch_mb_value, precision=0, commas=False)                  # e.g. "38"
batch_transfer_ms_str = fmt(batch_transfer_ms_value, precision=1, commas=False)  # e.g. "1.2"
pcie4_gbs_str = fmt(pcie4_gbs_value, precision=0, commas=False)                # e.g. "32"
```
...
# │
# │ Why: Demonstrates the scale challenge of modern LLMs—175B parameters require
# │      350 GB just for weights in FP16, far exceeding single-GPU memory. This
# │      motivates distributed execution contexts and 3D parallelism.
# │
# │ Imports: physx.constants (GPT3_PARAMS, BYTES_FP16), physx.formulas (model_memory)
# │ Exports: gpt3_params_b_str, gpt3_fp16_gb_str
# └─────────────────────────────────────────────────────────────────────────────
...
**Parameter Structures.** A GPT-3 scale model stores `{python} gpt3_params_b_str` billion parameters, occupying `{python} gpt3_fp16_gb_str` GB in FP16. Managing these parameters across devices, keeping gradients synchronized, and maintaining optimizer state (which can triple the memory footprint, as the Administrative Tax notebook showed) is a core framework responsibility.
...
**Distributed Execution Contexts.**\index{Distributed Training!execution contexts} The computational graph defines *what* to compute, but *where* and *how* that computation runs across devices is the job of execution contexts. On a single node, execution contexts manage CUDA streams and events (discussed earlier in this chapter) to overlap computation and data transfer across GPUs.
...
When models exceed single-device memory, frameworks combine multiple parallelism strategies simultaneously. A GPT-3 scale model, for instance, cannot fit on a single GPU—its `{python} gpt3_params_b_str` B parameters alone require `{python} gpt3_fp16_gb_str` GB in FP16, far exceeding any GPU's memory. How do practitioners train such models? By distributing computation across multiple devices using three complementary strategies. @fig-3d-parallelism lays out how large-scale training distributes computation across three orthogonal dimensions to overcome this constraint. In the figure, look for how each dimension addresses a different scaling need: **Data Parallelism**\index{Data Parallelism!model replication} (replicating the model across columns) scales throughput by processing different batches in parallel; **Pipeline Parallelism**\index{Pipeline Parallelism!layer splitting} (splitting layers across rows) distributes a single model's depth across devices; and **Model Parallelism**\index{Model Parallelism!tensor sharding}\index{Tensor Parallelism|see{Model Parallelism}} (sharding tensors within each cluster) partitions individual layers that are too large for one device. This "3D" approach allows frameworks to scale beyond the memory limits of any single device [@mcmahan2017federated]. @sec-ai-training examines these parallelism strategies in depth, including their implementation trade-offs and communication patterns.
...
# │
# │ Why: Shows that ResNet-50 performs ~8.2 billion FLOPs per forward pass,
# │      nearly all of which are GEMM operations. Motivates kernel optimization.
# │
# │ Imports: physx.constants (RESNET50_FLOPs), physx.formatting (fmt)
...
# --- Inputs (from model specs) ---
resnet_gflops_value = RESNET50_FLOPs.to(GFLOPs).magnitude                      # ~8.2 GFLOPs

# --- Outputs (formatted strings for prose) ---
resnet_gflops_str = fmt(resnet_gflops_value, precision=1, commas=False)        # e.g. "8.2"
```
...
The preceding sections examined how frameworks represent data (tensors), manage devices (streams and events), and dispatch operations (kernel selection and scheduling). These mechanisms operate at the level of individual tensors and operations—the raw materials of machine learning computation. But practitioners rarely write code at this level. A ResNet-50 has `{python} resnet_params_m_str` million parameters organized into dozens of layers; manually tracking each tensor, registering it with an optimizer, and handling device placement would be error-prone and tedious. How do frameworks organize these low-level primitives into the clean APIs that practitioners actually use?

## The nn.Module Abstraction {#sec-ai-frameworks-nnmodule-abstraction-2622}

\index{nn.Module!PyTorch abstraction}
\index{Module Abstraction!hierarchical composition}
Frameworks solve the complexity problem through higher-level abstractions that organize tensors, device management, and operations into reusable components.

PyTorch's `nn.Module` provides an instructive case study because its design patterns recur across frameworks. Keras uses similar layer abstractions, JAX's Flax employs analogous module structures, and even TensorFlow's functional API shares conceptual parallels. By understanding how one framework addresses parameter management, state tracking, and compositional design, we develop vocabulary for comparing alternatives. Rather than catalog its API, we extract three enduring design principles that every framework must address regardless of its syntax or programming paradigm.
...
::: {.callout-note title="Cross-Framework Parameter Discovery" collapse="true"}
...
\index{state\_dict!model serialization}
...
**State Dictionary and Model Serialization.** The `state_dict()` mechanism provides checkpoint management, and `load_state_dict()` restores model state. Using `strict=False` enables partial loading for architecture modifications during fine-tuning.

::: {#lst-state_dict lst-cap="**State Dictionary Serialization**: Save and load model state using `state_dict()` and `load_state_dict()`. The `strict=False` parameter enables partial loading for transfer learning."}
```{.python}
import torch
import torch.nn as nn
...
# Load into modified architecture (e.g., added layers)
# strict=False ignores missing/unexpected keys
new_model = nn.Sequential(
    nn.Linear(784, 256),
    nn.ReLU(),
    nn.Linear(256, 128),  # Modified
    nn.ReLU(),
    nn.Linear(128, 10),
)
new_model.load_state_dict(
    torch.load("model.pt"), strict=False  # Load matching layers only
)
```
:::

The `nn.Module` patterns above illustrate PyTorch's approach to the abstraction problem. PyTorch, however, is only one of several major frameworks, and its choices (mutable state, class inheritance, eager execution by default) are not the only valid design points. TensorFlow centralizes state differently, and JAX avoids mutable state entirely. These are not superficial API differences; they reflect fundamentally different answers to the three problems we examined at the chapter's start.
...
While TensorFlow 2.0 introduced eager execution to bridge the gap between research and production, its core strength remains the robust, compiled path from research to global-scale deployment. @sec-ai-training examines how TensorFlow's distribution strategies enable large-scale training, while @sec-model-serving-systems covers its production serving infrastructure.
...
- **`jax.grad`**\index{JAX!jax.grad}: Automatic differentiation. Unlike PyTorch's tape-based autograd, `grad` returns a *new function* that computes gradients. Supports both forward-mode (`jacfwd`) and reverse-mode (`jacrev`) differentiation.
- **`jax.jit`**\index{JAX!jax.jit}: Just-in-time compilation to XLA. Traces the function once, compiles to optimized machine code, caches the result. Subsequent calls execute the compiled version without Python overhead.
- **`jax.vmap`**\index{JAX!jax.vmap}\index{Vectorization!automatic batching}: Automatic vectorization. Transforms a function operating on single examples into one operating on batches, without manual batching code.
- **`jax.pmap`**\index{JAX!jax.pmap}: Parallel execution across devices. Maps a function over data distributed across multiple GPUs/TPUs, automatically handling communication.
...
**Trade-offs and Use Cases.** JAX excels in scenarios where its constraints become advantages:

- Custom differentiation (higher-order gradients, custom VJP/JVP rules), where pure functions make differentiation rules composable
- Research on optimization algorithms, where transformations let you experiment with gradient manipulation
- Large-scale distributed training (particularly on TPUs), where XLA compilation extracts maximum hardware utilization
- Scientific computing with AD requirements, where functional purity enables mathematical reasoning about code

JAX requires more upfront investment than PyTorch: the functional paradigm has a learning curve, state management requires explicit patterns, and debugging compiled code is harder than eager execution. The ecosystem is also younger, with fewer pre-built components than PyTorch or TensorFlow. Teams should choose JAX when its strengths align with project requirements, not as a default.
...
: **Framework Characteristics.** Comparison of TensorFlow, PyTorch, and JAX across graph construction, data mutability, automatic differentiation, GPU utilization, and distributed scalability. {#tbl-mlfm-comparison}
...
These three implementations solve the same mathematical problem but reveal fundamentally different answers to the Three Problems. The differences are not cosmetic; they shape debugging workflows, deployment options, and optimization potential.
...
The efficiency gaps measured above (17x latency differences, 7x memory differences) transform from optimization opportunities into hard deployment constraints as we move to resource-constrained devices. The three fundamental problems reweight dramatically at the edge. The *execution problem* shifts from "eager vs. graph" to "can we execute at all within 10ms and 50KB?" The *differentiation problem* often disappears entirely, since edge devices run inference only. The *abstraction problem* intensifies: targeting ARM vs. x86, mobile NPUs vs. edge TPUs, microcontrollers with kilobytes of memory.
...
The preceding sections have examined the three fundamental problems and framework selection criteria. To solidify understanding of how these concepts interact in practice, we trace a single training step through the PyTorch stack. This case study reveals how abstract Python code triggers concrete system interactions across the memory hierarchy and accelerator.
...
2. **Kernel Selection**\index{cuBLAS!kernel selection} (~0.5μs): cuBLAS selects an optimized GEMM kernel based on matrix dimensions (`{python} train_batch`×`{python} train_input` × `{python} train_input`×`{python} train_hidden`). For these dimensions, it might choose a tiled algorithm optimized for L2 cache.
...
**Phase 2: Backward Pass (Solving the Differentiation Problem).** When `loss.backward()`\index{loss.backward()!gradient computation} executes:
...
# │
# │ Why: Quantifies the arithmetic intensity of each operation in a forward pass.
...
# │
# │ Imports: physx.constants (byte, KB, MB, flop, KFLOPs, MFLOPs), physx.formatting (fmt)
# │ Exports: train_batch, train_input, train_hidden, train_mm1_flops_str, train_mm1_mem_str,
# │          train_mm1_ai_str, train_relu_flops_str, train_relu_mem_str, train_relu_ai_str,
...
# └─────────────────────────────────────────────────────────────────────────────
from physx.formatting import fmt
from physx.constants import byte, KB, MB, flop, KFLOPs, MFLOPs

# --- Inputs (2-layer MLP dimensions) ---
train_batch = 32                                                                # batch size
train_input = 784                                                               # MNIST input (28x28)
...
# │
# │ Why: On an A100, 40M FLOPs takes 0.1 μs and 5 MB memory takes 2.5 μs, but
...
# │
# │ Imports: physx.constants (A100_FLOPS_FP16_TENSOR, A100_MEM_BW), physx.formatting (fmt)
...
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import A100_FLOPS_FP16_TENSOR, A100_MEM_BW, flop, byte, second
from physx.formatting import fmt
...
mnist_t_compute_us_value = mnist_total_flops_value / a100_peak_tflops_value * 1e6  # ~0.1 μs
mnist_t_memory_us_value = mnist_mem_traffic_bytes_value / a100_mem_bw_bytes_value * 1e6  # ~2.5 μs
...
mnist_total_flops_str = f"{mnist_total_flops_value/1e6:.0f}M"
mnist_mem_traffic_str = f"{mnist_mem_traffic_bytes_value/1e6:.0f}MB"
...
a100_peak_tflops_str = fmt(a100_peak_tflops_value/1e12, precision=0, commas=False)
a100_mem_bw_tbs_str = fmt(a100_mem_bw_bytes_value/1e12, precision=0, commas=False)
```
...
- **CUDA GPU**\index{CUDA!backend implementation}: cuBLAS GEMM kernels, CUDA streams for async execution
- **CPU**\index{CPU Backend!MKL and OpenBLAS}: Intel MKL or OpenBLAS, OpenMP for parallelism
- **TPU**\index{TPU!XLA compilation}: XLA compilation to TPU-specific HLO operations
- **Apple Silicon**\index{MPS Backend!Apple Silicon}: Metal Performance Shaders via MPS backend

Each backend implements the same tensor operations with hardware-specific optimizations. The framework's abstraction layer (@sec-ai-frameworks-abstraction-problem-37a5) ensures identical numerical results (within floating-point tolerance) across platforms. This is the abstraction problem solved: a single `loss.backward()` call triggers completely different code paths depending on hardware, yet produces mathematically equivalent gradients.
...
The training step analysis above demonstrates how abstract framework concepts translate to concrete system behavior. With this foundation, we can identify common reasoning errors that lead engineers astray.

Framework selection and usage involve subtle trade-offs, and intuitions formed in other areas of software engineering often mislead. The following fallacies and pitfalls address seven failure modes: assuming framework equivalence, choosing by popularity rather than requirements, believing abstractions eliminate systems knowledge, ignoring vendor lock-in, neglecting production infrastructure, misunderstanding batch size scaling, and underestimating compilation overhead. Each is accompanied by quantitative evidence from earlier sections.
...
# │
# │ Why: Shows 17x performance gap (52ms vs 3ms) between PyTorch and TensorRT
...
memory_ratio_value = pytorch_mobile_mb_value * 1024 / tflite_micro_kb_value    # ~7040x gap
...
This assumption fails empirically. Engineers reason that since ResNet-50 is mathematically identical across frameworks, performance must be equivalent. @tbl-framework-efficiency-matrix disproves this: PyTorch achieves `{python} pytorch_ms_str`ms inference at 32% hardware utilization, while TensorRT delivers `{python} tensorrt_ms_str`ms at 88% utilization, a **`{python} perf_gap_str` x performance gap** on identical hardware. The difference arises from kernel fusion, graph optimization depth, and memory access patterns. Organizations assuming framework equivalence routinely miss production targets by 5-10×.

##### Pitfall: *Choosing frameworks based on popularity rather than project requirements.* {.unnumbered}

This approach ignores deployment constraints. @tbl-framework-efficiency-matrix shows the deployment spectrum: PyTorch Mobile requires `{python} pytorch_mobile_mb_str`MB memory, TensorFlow Lite needs 180MB, and TensorFlow Lite Micro runs in `{python} tflite_micro_kb_str`KB, a **`{python} memory_ratio_str` x difference**. Teams building edge applications with PyTorch face either massive memory overhead or costly framework migration. Match framework capabilities to deployment constraints *before* development begins.

##### Fallacy: *"Framework abstractions eliminate the need for systems knowledge."* {.unnumbered}

This belief ignores the physics of computation. The **Roofline Model** (@sec-system-foundations-roofline-model-5f7c) disproves the fallacy: element-wise operations like ReLU achieve arithmetic intensity of 0.125 FLOPs/byte, utilizing **under 0.1%** of an A100's peak compute regardless of framework. Understanding when operations are memory-bound vs. compute-bound and which sequences can be fused separates efficient implementations from those leaving 80-90% of hardware capacity unused. No framework can optimize away fundamental physics.

##### Pitfall: *Ignoring vendor lock-in from framework-specific formats.* {.unnumbered}

This oversight becomes costly during migration. Converting TensorFlow SavedModel to PyTorch requires: rewriting custom operations, validating numerical equivalence across 10,000+ test cases, and retraining when operations lack exact equivalents, typically **3-6 engineer-months** for production systems. ONNX provides portability but supports only 80-85% of operations. Design for portability from the start by avoiding framework-specific features where standard alternatives exist.

##### Pitfall: *Selecting development frameworks without evaluating production infrastructure.* {.unnumbered}

This mistake surfaces late in the development cycle. Framework-infrastructure mismatches impose substantial operational overhead. TensorFlow Serving provides atomic model swaps with zero downtime; PyTorch deployments often require container restarts imposing 30-60 second outages. TensorFlow integrates natively with Prometheus/OpenTelemetry; PyTorch requires custom instrumentation adding 2-4 weeks of development. Evaluate the *complete deployment stack*, not just training APIs.
...
# │
# │ Why: Demonstrates that a "small" 7B model still consumes 14 GB in FP16,
# │      leaving only 66 GB on an 80 GB A100. Shows that reported free memory
# │      doesn't account for activation memory that scales with batch size.
# │
# │ Imports: physx.constants (A100_MEM_CAPACITY, BYTES_FP16), physx.formulas (model_memory)
# │ Exports: model_7b_fp16_gb_str, a100_remaining_7b_gb_str, a100_mem_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import A100_MEM_CAPACITY, BYTES_FP16, GB, GiB
from physx.formulas import model_memory
from physx.formatting import fmt
...
# │
# │ Why: For 10,000 images with 10 recompilations, eager (6.9s) beats compiled
# │      (304.7s) because 10×30s = 300s compilation overhead dominates. Shows
# │      compilation only pays off for long runs with stable code.
# │
# │ Imports: physx.formatting (fmt)
...
@tbl-training-benchmark shows torch.compile achieves 48% higher ResNet-50 throughput than eager PyTorch, but incurs 15-60 seconds compilation overhead. For a `{python} n_images_str`-image experiment requiring `{python} n_recompilations_str` recompilations due to code changes:

- Eager: `{python} n_images_str` / `{python} eager_throughput_str` = `{python} eager_total_str` seconds
- Compiled: `{python} n_images_str` / `{python} compiled_throughput_str` + `{python} n_recompilations_str` × `{python} compilation_time_s_str` = `{python} compiled_total_str` seconds
...
We have established the software substrate of ML: the frameworks that translate abstract architectures into executable kernels. A control room without a source of energy, however, is just a room with glowing lights. We turn next to @sec-ai-training, where we scale these frameworks across massive datasets and hardware to build the power plant of modern AI.
