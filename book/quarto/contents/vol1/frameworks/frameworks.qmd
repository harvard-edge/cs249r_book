...
### Hardware-Driven Framework Architecture {#sec-ai-frameworks-hardwaredriven-framework-architecture-evolution-2605}

The evolution of frameworks is inextricably linked to the hardware they target. The introduction of NVIDIA's **CUDA** platform[^fn-cuda] (2007) marked the most critical pivot in framework design. GPUs excel at parallel matrix operations, but programming them directly required expert-level knowledge of kernel scheduling and memory coalescing. Frameworks succeeded because they bridged this gap, providing high-level Python APIs that automatically dispatched "Tensors" to the GPU's thousands of cores. Modern frameworks are, at their core, sophisticated "dispatchers" that orchestrate the flow of data between heterogeneous compute resources.
