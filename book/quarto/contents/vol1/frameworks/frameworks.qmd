...
PyTorch's TorchScript provides two ways to capture computations for compilation:

**Tracing** captures operations during one execution:
...
# traced is now a compiled version of forward()
# Can serialize, optimize, run without Python
```

Tracing records operations during execution, producing a static graph from the observed execution path. This works well for feed-forward models but cannot handle data-dependent control flow: if your function contains `if x.sum() > 0`, tracing only captures the branch taken during the example execution.

**Scripting** compiles a subset of Python to TorchScript:
...
# Compiles Python code to TorchScript IR
# Supports control flow but requires TorchScript-compatible Python subset
```

Scripting analyzes Python source code and compiles it to an intermediate representation that can be optimized and executed without Python. This supports dynamic control flow but only works with a restricted Python subset (no arbitrary Python objects, limited standard library access).
