...
The **Silicon Contract** (@sec-silicon-contract) defines the agreement between algorithms and hardware. Frameworks implement that agreement in software, translating high-level model definitions into hardware-specific execution. Where the **Iron Law** defines the physics of latency, the framework is the orchestrator that manages data movement to minimize the dominant terms of the equation.
...
**2. The Differentiation Problem**: How do we compute gradients automatically? Neural network training requires computing derivatives of a loss function with respect to millions or billions of parameters. Manual differentiation is error-prone and impractical. Frameworks must implement *automatic differentiation* systems that compute exact gradients for arbitrary compositions of operations, while managing the memory overhead of storing intermediate values.

**3. The Abstraction Problem**: How do we target diverse hardware from a single interface? The same model definition should run on CPUs, GPUs, TPUs, mobile devices, and microcontrollers. Each hardware target has different capabilities, memory constraints, and optimal execution patterns. Frameworks must provide abstractions that hide hardware complexity while enabling efficient utilization across this diverse landscape.
...
**Machine Learning Frameworks** refer to *software platforms* that solve the *execution*, *differentiation*, and *abstraction* problems, providing *standardized interfaces* for model development, training, and deployment across *diverse computational infrastructure*.
...
[^fn-blas]: **BLAS (Basic Linear Algebra Subprograms)**: A standardized specification (Level 1: vector operations; Level 2: matrix-vector; Level 3: matrix-matrix like GEMM) published in 1979 that defines portable APIs for dense linear algebra. Hardware vendors implement optimized BLAS libraries: Intel MKL achieves near-peak FLOPS on x86 CPUs through AVX-512 vectorization, while NVIDIA cuBLAS uses Tensor Cores for up to 312 TFLOPS (FP16/BF16/TF32) on A100 GPUs, or 624 TFLOPS with structured sparsity. This 45-year-old interface remains the performance foundation of modern ML frameworks.
...
\node[Box={col4!50},below= 2mm of BB4](BBB4){Theano introduces computational graphs [@bergstra2010theano]};
...
The critical limitation of tracing reveals the fidelity-generality trade-off concretely. Because tracing records a single execution path, it cannot handle data-dependent control flow. The following example illustrates a silent correctness failure:
...
::: {.callout-note title="Notebook: TorchScript Compilation Patterns" collapse="true"}

The following examples demonstrate scripting syntax, TorchScript restrictions, IR inspection, and a comparison of when to use each approach.
...
The optimal framework execution strategy depends on which Iron Law term dominates your workload:

+-------------------+------------------------------------------+------------------------------------------+--------------------------------------------+
| **Archetype**     | **Dominant Iron Law Term**               | **Optimal Framework Strategy**           | **Rationale**                              |
+:==================+:=========================================+:=========================================+:===========================================+
| **ResNet-50**     | $\frac{Ops}{P \cdot \eta}$ (Compute)     | **TensorRT** (inference)                 | Kernel fusion maximizes MFU; compute-bound |
| (Compute Beast)   |                                          | **torch.compile** (training)             | workloads benefit most from optimization   |
+-------------------+------------------------------------------+------------------------------------------+--------------------------------------------+
| **GPT-2**         | $\frac{D}{B}$ (Memory Bandwidth)         | **torch.compile**                        | Kernel fusion reduces HBM round-trips;     |
| (Bandwidth Hog)   |                                          |                                          | keeps data in cache to mitigate bandwidth  |
+-------------------+------------------------------------------+------------------------------------------+--------------------------------------------+
| **DLRM**          | $\frac{D}{B}$ (Random Access) +          | **Eager** with specialized kernels       | Embedding lookups are inherently irregular |
| (Sparse Scatter)  | $T_{network}$                            | (FBGEMM)                                 | and dynamic; compilation gains are small   |
+-------------------+------------------------------------------+------------------------------------------+--------------------------------------------+
| **DS-CNN**        | $L_{fixed}$ (Overhead)                   | **AOT compilation** (TFLite, ONNX)       | Sub-ms inference; every microsecond of     |
| (Tiny Constraint) |                                          |                                          | Python overhead is unacceptable            |
+-------------------+------------------------------------------+------------------------------------------+--------------------------------------------+

: Framework execution strategy aligned to workload archetype and dominant Iron Law term. {#tbl-framework-archetype-strategy}
...
Forward mode's strength in single-input analysis becomes its fatal weakness for training. A neural network has one scalar loss but millions of parameters, and forward mode would require a separate pass for each one.

::: {.callout-note title="Notebook: Forward Mode AD Examples" collapse="true"}

The following examples provide additional perspectives on forward mode AD: the bare computation structure, dual-number tuple representation, and application to sensitivity analysis and feature importance.
...
The critical observation is that this single backward pass computed dc/dx regardless of how many paths connect $x$ to $c$. In a neural network, each weight can affect the loss through thousands of paths across layers, and reverse mode handles them all in one traversal. This is why training a 175B parameter model like GPT-3 is feasible at all: reverse mode's $O(1)$ backward passes (relative to parameter count) keeps gradient computation tractable.
...
print(z.grad_fn.next_functions[0][0].next_functions)  # -> AccumulateGrad (leaf)
```
:::
...
These control mechanisms share a common systems design: they are callback-based extensions that the autograd engine invokes at specific points during graph traversal, without modifying the core differentiation algorithm. This extensibility pattern allows the framework to maintain a single optimized backward pass while supporting arbitrarily complex gradient manipulation.

::: {.callout-note title="Notebook: Exploring Autograd Internals" collapse="true"}
The following examples demonstrate how to inspect and control PyTorch's autograd system. These patterns are valuable for debugging gradient issues, implementing custom operations, and understanding memory behavior in practice.
...
**Retaining the Computation Graph.** By default, `backward()` frees the graph after use. To run multiple backward passes (for multi-loss optimization or higher-order derivatives), use `retain_graph=True` at the cost of doubled memory:
...
**Mixed-Precision Training Support.** Mixed precision exploits a hardware asymmetry to improve two Iron Law terms simultaneously: Tensor Cores execute FP16 matrix multiplications at 2x the throughput of FP32 (increasing effective $Ops/P$), while FP16 activations halve the memory footprint (reducing $D$). Improving both terms simultaneously is rare; most optimizations improve one at the expense of the other.
...
Resuming training requires loading both model parameters and optimizer state. @lst-checkpoint-save-load shows the complete checkpoint pattern:
...
**How Different Frameworks Implement AD.** The execution models covered in @sec-ai-frameworks-execution-problem-e1e1—eager, static graph, and hybrid—directly shape how each framework implements automatic differentiation:
...
A ResNet-50 forward pass touches 25 million parameters, produces intermediate activations at every layer, and must coordinate memory across CPU and GPU address spaces. How do frameworks organize all of this data so that a single Python call like `model(input)` executes millions of operations without the programmer managing a single pointer?
...
**Tensors** refer to multidimensional arrays that serve as the primary data structure in machine learning systems, providing *unified representation* for scalars, vectors, matrices, and higher-dimensional data with *hardware-optimized operations*.
...
::: {.callout-tip title="Scope: Advanced GPU Programming"}
...
**Principle 1: The Device Bandwidth Hierarchy.** The cost of moving data between devices varies by orders of magnitude depending on the interconnect.[^fn-nvlink] @tbl-device-transfer-overhead quantifies these costs for a 1000x1000 float32 tensor (4 MB), the size of a typical activation tensor in a moderately sized model:
...
Extending this pattern across multiple machines requires distributed training techniques covered in @sec-distributed-training, but the single-node implementation above illustrates the core synchronization principles that underlie all pipeline-parallel systems.

**Principle 3: Synchronization and Correctness.** []{#sec-ai-frameworks-stream-events} Concurrent execution introduces ordering constraints. When one stream's output becomes another stream's input, the system must enforce a happens-before relationship without unnecessarily serializing independent work. Two synchronization mechanisms exist, with dramatically different performance implications.
...
The performance difference between these approaches is not incremental but categorical. Full synchronization after every operation converts a concurrent pipeline into a sequential one, entirely negating the hardware parallelism that streams expose. Event-based synchronization preserves the concurrent execution model while enforcing only the dependencies that correctness requires. A common mistake in production code is inserting `torch.cuda.synchronize()` calls for debugging and forgetting to remove them, silently converting an overlapped pipeline into a serialized one.

::: {.callout-note title="Notebook: GPU Device Management Patterns" collapse="true"}

The following examples demonstrate PyTorch's device management API, including device placement, context managers, error handling, and profiling tools. These patterns implement the three systems principles discussed above.
...
**Device Placement Patterns.** Minimizing transfers requires consistent device placement and memory reuse:
...
Module `.to()` recursively moves all parameters and buffers:
...
**Synchronization Patterns.** Avoid full synchronization when events suffice:
...
**GPU-Level Profiling with NVIDIA Nsight.** {#sec-ai-frameworks-nsight-profiling} For hardware-level bottleneck diagnosis, NVIDIA provides two complementary tools. Nsight Systems captures system-wide timelines correlating CPU activity, GPU kernel execution, and memory transfers:
...
Nsight Compute provides kernel-level analysis with hardware counters:
...
: **Nsight Compute Metrics**: Key metrics for ML kernel optimization. Low values indicate specific optimization opportunities. Use Nsight Systems to identify which kernels dominate runtime, then Nsight Compute to understand why those kernels underperform. {#tbl-nsight-metrics}
...
::: {.callout-note title="Notebook: Dataset and DataLoader Patterns" collapse="true"}
...
### Principle 1: Automatic Parameter Discovery {#sec-nnmodule-param-discovery}
...
### Principle 2: Mode-Dependent Behavior {#sec-nnmodule-mode-behavior}
...
### Principle 3: Hierarchical Composition and Serialization {#sec-nnmodule-composition}
...
TensorFlow was developed by the Google Brain team and was released as an open-source software library on November 9, 2015 [@abadi2016tensorflow]. It was designed for numerical computation using data flow graphs and has since become popular for a wide range of machine learning applications.
...
TensorFlow provides built-in functionality to handle everything from model creation and training to deployment, as shown in @fig-tensorflow-architecture. The ecosystem spans four variants that illustrate the deployment spectrum central to this chapter:

- [TensorFlow Core](https://www.tensorflow.org/tutorials): the primary package for defining, training, and deploying models, with [tf.keras](https://www.tensorflow.org/guide/keras) as its high-level API.

- [TensorFlow Lite](https://www.tensorflow.org/lite) (rebranded as LiteRT in 2024): deploys lightweight models on mobile, embedded, and edge devices by converting models to a compact format optimized for limited-resource hardware.

- [TensorFlow Lite Micro](https://www.tensorflow.org/lite/microcontrollers): runs models on microcontrollers without operating system support, standard C/C++ libraries, or dynamic memory allocation, using only a few kilobytes of memory.

- [TensorFlow Serving](https://www.tensorflow.org/tfx/guide/serving): serves models in production with versioning and dynamic model updates without service interruption.

The broader ecosystem extends further to browser deployment, edge accelerators, federated learning, and end-to-end pipeline management. This breadth follows from TensorFlow's commitment to graph-based compilation, which enables ahead-of-time optimization for each deployment target.
...
**Production-Scale Deployment.** Real-world production systems demonstrate how framework selection directly impacts system performance under operational constraints. Framework optimization often achieves dramatic improvements: production systems commonly see 4--10$\times$ latency reductions and 2--5$\times$ cost savings through systematic optimization including quantization, operator fusion, and hardware-specific acceleration.

These optimizations require significant engineering investment, typically 4 to 12 weeks of specialized effort for custom operator implementation, validation testing, and performance tuning. The detailed production deployment examples, optimization techniques, and quantitative trade-off analysis are covered in @sec-ai-training and @sec-serving, where operational constraints and deployment strategies are systematically addressed.
...
**Transformations over State.** While PyTorch and TensorFlow build computational graphs (dynamically or statically), JAX transforms functions. The core insight is that automatic differentiation, vectorization, and JIT compilation are all *program transformations* that can compose:
...
:::{.callout-note title="Framework Selection Constraints"}
