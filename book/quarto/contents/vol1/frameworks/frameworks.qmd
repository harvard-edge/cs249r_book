...
# │          hbm_4mb_ms_str, pcie4_1gb_ms_str, pcie4_1gb_equiv_ops_str, a100_mem_gb_str,
# │          pcie_to_hbm_gap_str
# └─────────────────────────────────────────────────────────────────────────────
...
class DeviceBandwidthHierarchy:
    ...
    a100_mem_display = Hardware.Cloud.A100.memory.capacity
    ...
    a100_mem_gb_str = fmt_qty(a100_mem_display, GiB, precision=0, commas=False, unit_label="GB")
...
# │ Exports: HbmCapacityFootnote.a100_mem_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class HbmCapacityFootnote:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(a100_mem.to(GiB).magnitude >= 80, "A100 HBM capacity anchor should be at least 80 GB.")
...
# │          Model7BMemory.a100_remaining_7b_gb_str, Model7BMemory.a100_mem_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.physics import model_memory
from mlsysim.fmt import fmt_qty, check
...
class Model7BMemory:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    params = Models.Language.Llama2_7B.parameters
    a100_mem_display = 80 * GB  # pedagogical 80 GB nominal
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    fp16 = params * BYTES_FP16                                                  # 14 GB
    remaining = a100_mem_display - fp16
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(remaining.to(GB).magnitude > 60, f"7-billion-parameter FP16 weights leave less A100 memory than expected: {remaining.to(GB).magnitude:.1f} GB.")
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    model_7b_fp16_gb_str = fmt_qty(fp16, GB, precision=0, commas=False)             # e.g. "14 GB"
    a100_remaining_7b_gb_str = fmt_qty(remaining, GB, precision=0, commas=False)     # e.g. "66 GB"
    a100_mem_gb_str = fmt_qty(a100_mem_display, GB, precision=0, commas=False)                    # e.g. "80 GB"
```
