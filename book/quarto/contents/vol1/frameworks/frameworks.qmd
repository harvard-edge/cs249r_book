...
```{python}
#| echo: false
#| label: framework-archetype-labels
# ┌─────────────────────────────────────────────────────────────────────────────
# │ FRAMEWORK ARCHETYPE LABELS
# ├─────────────────────────────────────────────────────────────────────────────
# │ Goal: Reuse canonical workload archetype labels from MLSysIM.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Scenarios as BookScenarios
from mlsysim.fmt import MarkdownStr

class FrameworkArchetypeLabels:
    """Display labels for framework strategy rows."""
...
|:---------------------------------------------------------|:--------------------------------------------------------------------------|:---------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------|
| **`{python} FrameworkArchetypeLabels.resnet_label_str`** | $\frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}}$ (Compute)              | **TensorRT** (inference)<br>**torch.compile** (training) | Kernel fusion improves achieved FLOP/s relative to $R_{\text{peak}}$; compute-bound workloads benefit most from optimization |
| **`{python} FrameworkArchetypeLabels.gpt2_label_str`**   | $\frac{D_{\text{vol}}}{\text{BW}}$ (Memory Bandwidth)                     | **FlashAttention** + **graph compilation**               | Fused attention and compilation reduce HBM round-trips and improve cache reuse                                               |
| **`{python} FrameworkArchetypeLabels.dlrm_label_str`**   | $\frac{D_{\text{vol}}}{\text{BW}_{\text{random}}}+L_{\text{lat,network}}$ | **Eager** with specialized kernels  (FBGEMM)             | Embedding lookups are inherently irregular and dynamic; compilation gains are small                                          |
| **`{python} FrameworkArchetypeLabels.kws_label_str`**    | $L_{\text{lat}}$ (Overhead)                                               | **AOT compilation** (TFLite, ONNX)                       | Sub-ms inference; every microsecond of  Python overhead is unacceptable                                                      |
