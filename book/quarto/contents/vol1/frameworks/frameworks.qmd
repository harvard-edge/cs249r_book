...
*   **Python Dispatch:** ~`{python} python_dispatch_us_str` $\mu$s per operation.
*   **Kernel Launch:** ~`{python} kernel_launch_only_us_str` $\mu$s per operation.
*   **Memory Access (VRAM):** ~`{python} memory_access_us_str` $\mu$s.
...
**Custom Autograd Functions.**\index{Custom Autograd Functions!backward implementation} When implementing custom operations, you explicitly specify what to save for the backward pass and how to compute gradients:
...
**Gradient Hooks.**\index{Gradient Hooks!inspection and modification} Register hooks on tensors to inspect or modify gradients during backpropagation:
...
**Detach vs. Data.**\index{Tensor!detach() method}\index{Gradient Flow!breaking with detach} Use `.detach()` to safely break gradient flow. The legacy `.data` attribute can silently corrupt gradient computation through in-place operations:
