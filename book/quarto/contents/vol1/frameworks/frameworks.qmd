...
These three regimes create distinct regions in the compilation decision space. @fig-compilation-continuum maps out these regions so you can identify where each strategy wins. Watch for the crossover points: the steep eager line catches up with JIT's initial compilation penalty after only hundreds of executions, while the even steeper compiled line (with its larger upfront cost) only wins after thousands. The slopes reveal execution speed; the vertical offsets reveal compilation cost. Your project's position on the x-axis determines which line you want to be on.
...
This approach prioritizes the **Deployment Spectrum**. Because the framework sees the entire graph, it can perform aggressive optimizations like constant folding, operator fusion, and memory layout optimization before the first byte of data is processed. This is why TensorFlow remains the standard for complex production ecosystems. @fig-tensorflow-architecture maps the full training-to-deployment pipeline—trace how a model flows from data preprocessing through distributed training on the left, then fans out to serving, mobile, browser, and embedded targets on the right.
...
- Tcompute ≈ 40M / `{python} a100_peak_tflops_str` T ≈ `{python} mnist_t_compute_us_str`µs
- Tmemory ≈ 5MB / `{python} a100_mem_bw_tbs_str` TB/s ≈ `{python} mnist_t_memory_us_str`µs
- Toverhead ≈ 6 ops × 5 μs ≈ `{python} mnist_t_overhead_us_str` μs
...
- Eager: 10,000 / 1,450 = `{python} eager_total_str` seconds
- Compiled: 10,000 / 2,150 + 10 × 30 = `{python} compiled_total_str` seconds
