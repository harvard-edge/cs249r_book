...
@Tbl-deployment-frameworks is a constraint map rather than a product catalog: each row identifies the runtime family whose assumptions fit the target envelope.

| **Environment**              | **Primary Frameworks**        | **Key Optimizations**                        | **Typical Constraints**           |
|:-----------------------------|:------------------------------|:---------------------------------------------|:----------------------------------|
| **Cloud/Server**             | PyTorch, TensorFlow, JAX      | Graph compilation, batching, lower precision | Throughput, cost                  |
| **Edge**                     | TensorFlow Lite, ONNX Runtime | Static graphs, lower-precision kernels       | Latency <10 ms, limited memory    |
| **Mobile**                   | TF Lite, Core ML, ExecuTorch  | Accelerator delegates, compact model formats | Battery, thermal, app size limits |
| **Microcontroller (TinyML)** | TF Lite Micro, microTVM       | Static allocation, small integer kernels     | <256 KB RAM, no dynamic memory    |
