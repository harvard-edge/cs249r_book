...
%%
\foreach \x in{1,2,...,7}
\draw[dashed,thick,-latex](B\x)--++(270:4.25);
...
Data-dependent control flow is a common break because it requires tensor values unavailable at compile time, as shown in @lst-graph-break-control-flow. This is the same `if x.sum() > 0` construct that silently mis-traced under TorchScript tracing in @lst-tracing-silent-failure; under `torch.compile`, the compiler stops at an honest graph break rather than baking in one branch and returning a wrong answer.
...
The execution models form a continuum from maximum flexibility to maximum optimization. @Eq-execution-continuum lays out the four positions on that axis, and each labeled arrow names the mechanism that carries a project one step rightward toward hardware.
...
Forward mode achieves this systematic derivative computation by augmenting each number with its derivative value, creating what mathematicians call a "dual number"\index{Forward Mode AD!dual numbers}\index{Dual Number!forward mode AD}. @Lst-forward_mode_dual runs the same function at $x = 2.0$, so the bookkeeping becomes concrete: each intermediate value carries its derivative alongside it through a single pass.
...
::: {#lst-reverse_memory lst-cap="**Activation Persistence Scales with Depth**: Each layer marks its activation to be stored for the backward pass, so the live activation set, and peak memory, grows linearly with network depth."}
