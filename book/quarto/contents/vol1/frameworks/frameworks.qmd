...
This asymmetry makes reverse mode the only viable option for training. Consider a function where $x$ influences the output through two distinct paths. @Lst-reverse_simple defines such a function, and @alg-reverse-mode-ad-trace traces its forward and backward computation for a concrete input.
...
::: {#alg-reverse-mode-ad-trace name="Reverse-mode automatic differentiation trace"}

**Require**: Stored forward values for $x=2.0$, $a=x^2=4.0$, $b=\sin(x)\approx 0.9093$, and $c=a b\approx 3.637$.

**State**: For each intermediate value $v$, the adjoint $\bar{v}$ stores $\partial c/\partial v$.

**Output**: The derivative $\partial c/\partial x$.

1. Seed the output adjoint: $\bar{c}\gets 1.0$; initialize $\bar{a},\bar{b},\bar{x}\gets 0$.
2. Reverse the multiplication $c=a b$: accumulate $\bar{a}\gets \bar{a}+\bar{c}b=0.9093$ and $\bar{b}\gets \bar{b}+\bar{c}a=4.0$.
3. Reverse the square path $a=x^2$: accumulate $\bar{x}\gets \bar{x}+\bar{a}(2x)=0.9093\cdot 4.0=3.637$.
4. Reverse the sine path $b=\sin(x)$: accumulate $\bar{x}\gets \bar{x}+\bar{b}\cos(x)=3.637+4.0\cos(2.0)\approx 1.972$.
5. Return $\partial c/\partial x=\bar{x}\approx 1.972$.

**Systems cost**: The reverse traversal computes every path contribution in one pass, but it can do so only because the forward pass retained $x$, $a$, $b$, and the graph edges until the backward rules consumed them.
