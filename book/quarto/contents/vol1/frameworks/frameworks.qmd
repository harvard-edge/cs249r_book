...
+-------------------------+-----------------------------+-----------------------------+-----------------------------+
| **Memory Overhead**     | 2-3× (autograd tape)        | 1× (pre-allocated)          | 1.5-2× (compiled cache)     |
+-------------------------+-----------------------------+-----------------------------+-----------------------------+
| **Compilation Time**    | Zero                        | Graph construction time     | 5-30s first execution       |
+-------------------------+-----------------------------+-----------------------------+-----------------------------+
| **First Iteration Time**| Fast                        | Slow (optimization)         | Slow (compilation)          |
+-------------------------+-----------------------------+-----------------------------+-----------------------------+
| **Typical Speedup**     | 1× (baseline)               | 1.5-3× (vs eager)           | 2-5× (vs eager)             |
+-------------------------+-----------------------------+-----------------------------+-----------------------------+

: **Framework Execution Models**: Eager execution with autograd tape provides maximum flexibility through immediate operation execution and dynamic graph construction. Static graphs enable aggressive optimization through ahead-of-time analysis at the cost of limited control flow. JIT compilation combines eager execution flexibility with compiled performance through transparent bytecode-level capture and kernel fusion. {#tbl-framework-execution-models}

#### Computational Graphs as Foundation for Automatic Differentiation {#sec-ai-frameworks-graphs-enable-autodiff}

All three execution models rely on computational graphs to implement automatic differentiation, but they construct and traverse these graphs differently. In eager execution, the autograd tape is built during forward pass and traversed immediately during backward pass. In static graphs, the computation graph is built before execution and includes both forward and backward operations. In JIT compilation, the captured graph is analyzed to generate optimized forward and backward kernels.

The key insight connecting execution models to automatic differentiation is that **all frameworks need a representation of operation dependencies to apply the chain rule**. The execution model determines when this representation is built (before, during, or after first execution) and how long it persists (destroyed after each backward pass, or cached for reuse).

The graph structure encodes dependency relationships needed for the chain rule. Each edge represents a partial derivative, and reverse traversal composes these partial derivatives according to the chain rule. This design enables automatic differentiation to scale to networks with millions of parameters because complexity is linear in the number of operations, not exponential in the number of variables. The graph structure ensures each gradient computation is performed exactly once, and shared subcomputations are handled through dependency tracking.

We examine the mechanics of automatic differentiation using these graph structures in @sec-ai-frameworks-automatic-differentiation-e286, including the mathematical foundations of reverse-mode differentiation, memory management for saved tensors, and gradient accumulation strategies.
