...
1.  **Significance (Quantitative):** Frameworks directly determine the system efficiency ($\eta_{\text{hw}}$) term in the iron law. Accelerated Linear Algebra (XLA)'s operator fusion, for example, eliminates intermediate memory writes between consecutive elementwise operations: fusing a matrix multiplication, bias add, and rectified linear unit (ReLU) into a single kernel reduces the total data movement ($D_{\text{vol}}$) by 2$\times$--3$\times$ vs. three separate kernel launches, yielding observed end-to-end speedups of 1.5$\times$--2$\times$ on transformer training without any model changes.
...
1.  **Solving Performance (1979–1992)**: The **Basic Linear Algebra Subprograms (BLAS)**\index{BLAS!historical foundation}[^fn-blas-performance] [@blas_netlib] and **LAPACK**[^fn-lapack-algebra] [@lapack_netlib] solved the problem of *Hardware Primitives*. They provided standardized, highly optimized implementations of matrix operations such as general matrix multiply (GEMM)[^fn-gemm-utilization]. This layer ensures that `C = A @ B` runs at near-peak silicon speed, regardless of the language calling it.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-ml-frameworks-execution-strategy-matters-memory-wall-1ce8—# │          opening paragraph of "Why Execution Strategy Matters: The Memory Wall"
# │
...
[^fn-flashattention-fusion-fw]: **FlashAttention**: Kernel fusion taken to its logical extreme, fusing the entire attention computation (Q, K, V projections, softmax, output) into a single kernel that tiles data to fit in SRAM (introduced in @sec-network-architectures). By reducing HBM traffic 10$\times$--20$\times$, FlashAttention transforms a memory-bound operation into a compute-bound one, demonstrating that framework-level fusion can shift an operation's position on the Roofline Model from bandwidth-limited to throughput-limited. \index{FlashAttention!kernel fusion}
...
*   **For large operations** (for example, a massive $4096\times4096$ matrix multiply), the kernel executes for **`{python} DispatchTax.large_kernel_str` $\mu$s**. The dispatch tax drops to **`{python} DispatchTax.large_tax_pct_str` percent**, and the system becomes compute bound.
...
**The Conclusion:**
Compilation is not magic; it is **overhead amortization**. For small, element-wise operations such as LayerNorm, Gaussian Error Linear Unit (GELU), and Add, overhead often exceeds compute time by 10$\times$--100$\times$. Fusing them is the only way to use the hardware effectively.
...
: **Training and Inference Throughput.** Representative throughput comparison across execution modes for common model architectures on NVIDIA A100 GPU with batch size 32. torch.compile typically provides 1.4$\times$--1.5$\times$ speedup over eager mode, while TensorRT provides 2$\times$--3$\times$ speedup but requires longer compilation and is inference only. Compile times vary based on model complexity and optimization level. {#tbl-training-benchmark}
...
|:------------------------------|:---------------------------------------------------------------------|:------------------------------------------------------------------------------|:------------------------------------------------------------------------------------|
| **ResNet-50 (Compute Beast)** | $\frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}}$ (Compute)         | **TensorRT** (inference)<br>**torch.compile** (training)<br>**torch.compile** | Kernel fusion maximizes MFU; compute-bound workloads benefit most from optimization |
| **GPT-2 (Bandwidth Hog)**     | $\frac{D_{\text{vol}}}{BW}$ (Memory Bandwidth)                       |                                                                               | Kernel fusion reduces HBM round-trips;  keeps data in cache to mitigate bandwidth   |
| **DLRM (Sparse Scatter)**     | $\frac{D_{\text{vol}}}{BW}(\text{Random Access})+T_{\text{network}}$ | **Eager** with specialized kernels  (FBGEMM)                                  | Embedding lookups are inherently irregular and dynamic; compilation gains are small |
...
# ┌── 4. DECORATE ──────────────────────────────────────────────────────────────
ax.text(1e4, 3.5, "Eager wins", color=COLORS['BlueLine'], ha='center', va='center', fontweight='normal', fontsize=9, bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=0.25))
ax.text(1e4, 14, "JIT wins", color=COLORS['OrangeLine'], ha='center', va='center', fontweight='normal', fontsize=9, bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=0.25))
ax.text(1e4, 35.5, "AOT wins", color=COLORS['GreenLine'], ha='center', va='top', fontweight='normal', fontsize=9, bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=0.25))
...
*   **Hardware Time**: T_hw ≈ `{python} DispatchTaxSmallLarge.dispatch_hw_time_us_value` μs (mostly memory latency).
*   **Software Overhead**: T_sw ≈ `{python} DispatchTaxSmallLarge.dispatch_n_ops_value` ops $\times$ `{python} DispatchTaxSmallLarge.dispatch_us_per_op_value` μs/op = `{python} DispatchTaxSmallLarge.dispatch_sw_time_str` μs.
...
*   **Conclusion**: The system spends `{python} DispatchTaxSmallLarge.dispatch_overhead_pct_str` percent of time waiting for Python. Compilation yields **`{python} DispatchTaxSmallLarge.dispatch_compilation_speedup_str`$\times$ speedup**.

**Scenario two: GPT-3 Layer (Compute Bound)**

*   **Compute**: Huge matrix multiplications.
*   **Hardware Time**: T_hw ≈ 100 ms = `{python} DispatchTaxSmallLarge.gpt3_hw_time_us_value` μs.
...
# │       line ~2905 and ~2922 (Parameter Structures / 3D Parallelism\index{3D Parallelism!scaling strategy} paragraphs).
# │
...
The second strategy is *operation fusion*\index{Kernel Fusion}[^fn-operation-fusion-bandwidth]. Rather than executing matrix multiplication, bias addition, and ReLU as three separate operations, each writing intermediate results to memory, frameworks fuse them into a single kernel. This eliminates intermediate memory allocations entirely and achieves 2$\times$--3$\times$ speedup on modern GPUs by keeping data in registers and caches.

[^fn-operation-fusion-bandwidth]: **Operation Fusion**: The 2$\times$--3$\times$ speedup cited in the triggering sentence arises from a specific hardware fact: GPU registers and L1 cache deliver 10$\times$--100$\times$ higher bandwidth than HBM. When matmul, bias, and ReLU execute as separate kernels, each writes its output to HBM and the next reads it back, a round-trip that dominates execution time for memory-bound operations. Fusing them into one kernel keeps intermediates in registers, converting three HBM round-trips into zero. \index{Operation Fusion!bandwidth reduction}
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-ml-frameworks-principle-2-memorycompute-tradeoff-19f2—# │          "Principle 2: The Memory-Compute Trade-off" subsection
# │
...
for inputs, targets in dataloader\index{DataLoader!throughput optimization}:
...
- **TensorFlow** (in its one.x incarnation) performed symbolic differentiation during graph construction, enabling ahead-of-time optimization. Modern TensorFlow two.x uses eager execution by default but provides `tf.function` for graph compilation when performance matters.
...
A ResNet-50 forward pass touches `{python} ResNetAbstraction.resnet_params_m_str` million parameters, produces intermediate activations at every layer, and must coordinate memory across CPU and GPU address spaces. How do frameworks organize all of this data so that a single Python call like `model(input)` executes millions of operations without the programmer managing a single pointer? Answering this question requires solving four problems in sequence: defining a universal data container (tensors), placing it on the right device (memory management), feeding data fast enough (data pipelines), and dispatching the right hardware kernel (core operations). We trace this path from data representation to hardware execution.
...
1.  **Significance (Quantitative):** Tensor memory footprint is fully deterministic from its metadata: a contiguous FP32 tensor of shape $[1024, 1024]$ occupies exactly $1024\times1024\times4 = 4$ MB. Non-contiguous layouts (for example, from a transpose operation) require explicit `.contiguous()` calls before certain CUDA kernels can execute, adding a memory-copy overhead that can dominate the $L_{\text{lat}}$ term for tensors under 1 MB.
...
**The Systems Conclusion**: A `{python} AdminTax.admin_weights_gb_str` GB model has an **"Administrative Tax"** of ~`{python} AdminTax.admin_tax_gb_str` GB (`{python} AdminTax.admin_grads_gb_str` GB gradients + `{python} AdminTax.admin_opt_gb_str` GB optimizer + `{python} AdminTax.admin_act_gb_str` GB activations) before the first batch is even processed. During training, **Data Movement** includes saving and retrieving these activations, which is why training is often 3$\times$--4$\times$ slower than pure inference.
...
This rank hierarchy maps directly onto ML data. A color image is a rank-3 tensor: height x width x three channels (red, green, blue). @fig-tensor-data-structure-b breaks this apart, stacking the three color channels illustrating how a single photograph becomes a three-layer numerical grid. Stacking a batch of $N$ images adds a fourth dimension, producing a rank-4 tensor of shape $[N, 3, H, W]$. Every convolutional layer in a vision model consumes and produces tensors of exactly this shape, which is why the tensor abstraction is so central to framework design.

::: {#fig-tensor-data-structure-b fig-env="figure" fig-pos="htb" fig-cap="**Image as RGB Tensor.** Three stacked grids representing the red, green, and blue color channels of an image, with dimension labels showing width, height, and channel depth forming a rank-3 tensor. *Credit: Niklas Lang [https://towardsdatascience.com/what-are-tensors-in-machine-learning-5671814646ff](https://towardsdatascience.com/what-are-tensors-in-machine-learning-5671814646ff)*." fig-alt="Three stacked 5x5 grids in red, green, and blue representing RGB channels. Dimension labels show width five pixels, height five pixels, and three color channels, forming a rank-3 tensor for image data."}
...
::: {#fig-3d-parallelism fig-env="figure" fig-pos="htb" fig-cap="**3D Parallelism.** A grid of eight accelerator clusters arranged in two rows and four columns, each containing stacked computational units. Distinct colors encode the three parallelism dimensions: data parallelism across columns, pipeline parallelism across rows, and model parallelism within each cluster." fig-alt="Grid of 8 GPU clusters in two rows and four columns. Each cluster contains four stacked cubes. Colors vary: blue, red, green, orange in bottom row; olive, yellow, brown, pink in top row."}
...
The kernel manager dispatches each operation to the fastest available implementation for the current hardware. When a framework encounters a matrix multiplication, it selects among AVX-512 vector instructions on modern CPUs, [cuBLAS\index{cuBLAS!kernel selection}](https://developer.nvidia.com/cublas) on NVIDIA GPUs, or dedicated tensor processing instructions on AI accelerators. The dispatch decision depends on input dimensions, data layout, and hardware capabilities. A $4096\times4096$ GEMM on an A100 GPU routes to cuBLAS Tensor Core kernels that sustain up to `{python} A100BLAS.dense_tflops_str` TFLOPS in FP16, while the same operation on a CPU falls back to an AVX-512 path at roughly 2 TFLOPS. When no specialized kernel exists, the manager falls back to a generic implementation rather than failing.
...
Numerical precision adds another dimension. Training in FP32 uses 4 bytes per parameter; quantizing to INT8 reduces this to 1 byte, cutting memory by 4$\times$ and enabling 2$\times$--4$\times$ throughput improvements on hardware with INT8 acceleration. Training typically requires FP32 for gradient stability, while inference runs at FP16 or INT8 with minimal accuracy loss. Frameworks maintain separate kernel implementations for each precision format and handle mixed-precision workflows where different layers operate at different bit widths within a single forward pass.
...
| **Compilation Optimization**  | XLA: 3$\times$--10$\times$ speedup          | TorchScript: 2$\times$  | XLA: 3$\times$--10$\times$ speedup       |
...
How do these architectural differences look in practice? @lst-framework-hello-world implements the same neural network (a single linear layer mapping ten inputs to one output) across all three frameworks, revealing how design philosophy shapes even the simplest code:
...
[^fn-xla-compiler]: **XLA (Accelerated Linear Algebra)**: The "optimized machine code" in the triggering sentence means XLA fuses an entire subgraph into one kernel, eliminating both launch overhead ($L_{\text{lat}}$) and intermediate memory writes ($D_{\text{vol}}$). The 1.5$\times$--2$\times$ speedup for transformer blocks is modest because their large GEMM operations are already compute bound, leaving little overhead for fusion to remove. Memory-bound models see 3$\times$--10$\times$ gains, where fusion hides the relative cost of many small, sequential operations. \index{XLA!compilation speedup}
...
: **Framework Efficiency Comparison.** Quantitative comparison of major ML frameworks across efficiency dimensions using ResNet-50 inference on representative hardware (NVIDIA A100 GPU for server frameworks, ARM Cortex-A78 for mobile). Metrics reflect production workloads with accuracy maintained within one percent of baseline. Hardware utilization represents percentage of theoretical peak performance on typical operations. {#tbl-framework-efficiency-matrix tbl-colwidths="[24,14,12,15,17,19]"}

\index{TensorRT!hardware utilization}
\index{Apache TVM!ML compiler}
The efficiency data reveals several important patterns. First, specialized inference frameworks (TensorRT, Apache TVM) achieve 10$\times$--15$\times$ lower latency than general-purpose training frameworks (PyTorch, TensorFlow) on identical hardware, demonstrating that framework selection has quantitative performance implications beyond qualitative design preferences. Second, mobile-optimized variants (TF Lite, PyTorch Mobile) reduce memory requirements by 10$\times$ compared to their full counterparts while maintaining accuracy within one percent through quantization and graph optimization [@pytorch_mobile]. Third, hardware utilization varies dramatically: TensorRT achieves 88 percent GPU utilization through aggressive kernel fusion while vanilla PyTorch achieves only 32 percent, a 2.75$\times$ efficiency gap that directly translates to cost differences in production deployment.
...
| **Microcontroller (TinyML)** | TF Lite Micro,  uTensor          | 4-bit quantization, static allocation | <256 KB RAM,  no dynamic memory   |
...
The flexibility-optimization tension, in turn, exposes a third: ecosystem breadth vs. specialization. General-purpose frameworks cover broad operation sets but underperform specialized runtimes. TensorRT achieves 88 percent GPU utilization vs. PyTorch's 32 percent (@tbl-framework-efficiency-matrix) precisely because it optimizes for a narrower problem. ONNX bridges this gap through standardized interchange, but the underlying trade-off remains [@bai2019onnx]: the more a runtime specializes, the faster it runs and the less it supports.
...
- `torch.compile` provides 2$\times$--3$\times$ speedup by fusing operations and reducing kernel launches
