...
[^fn-flashattention-fusion-fw]: **Fused Attention Kernel**: A fused attention kernel combines the $QK^T$ product, softmax, and value-weighted output into a tiled implementation that keeps intermediate values in on-chip memory rather than materializing the full attention matrix in HBM. FlashAttention is the canonical named example introduced in @sec-network-architectures. The framework lesson is broader than the specific algorithm: fusion can shift an operation's position on the Roofline Model by reducing round-trips through external memory. \index{Attention Mechanism!fused kernel}
