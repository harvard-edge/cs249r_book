...
class DispatchTaxSmallLarge:
    ...
    gpt3_hw_time_ms_str = fmt_time(large_hw_us / 1000, 'millisecond', precision=0, commas=False)
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.V100.interconnect.bandwidth, Hardware.Cloud.A100.interconnect.bandwidth, Hardware.Cloud.A100.nvlink.bandwidth, Hardware.Cloud.A100.memory.bandwidth, Hardware.Cloud.A100.memory.capacity,
# │          Hardware.Cloud.A100.compute.peak_flops), mlsysim.book (fmt)
# │ Exports: pcie3_gb_per_s_str, pcie3_4mb_ms_str, pcie4_gb_per_s_str, pcie4_bidir_gb_per_s_str, nvlink_a100_gb_per_s_str, a100_bw_gb_per_s_str,
...
|:-----------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------:|:-----------------------------------------------------:|:-----------------------------------|
| **PCIe 3.0 x16** |                                                         `{python} DeviceBandwidthHierarchy.pcie3_gb_per_s_str`                                                        |  `{python} DeviceBandwidthHierarchy.pcie3_4mb_ms_str` | 10$\times$ slower than GPU compute |
...
class TrainingStepCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    train_mm1_dims_math = MarkdownStr(f"$2 \\times {_batch} \\times {_input}{{\\times}}{_hidden}$")
    train_relu_dims_math = MarkdownStr(f"${_batch}{{\\times}}{_hidden}$")
    train_mm2_dims_math = MarkdownStr(f"$2 \\times {_batch} \\times {_hidden}{{\\times}}{_output}$")
