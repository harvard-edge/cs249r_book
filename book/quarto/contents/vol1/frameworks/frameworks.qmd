...
class GraphOptimizationStats:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(reduction_span_pct > 0, "Graph optimization FLOP reduction range must be increasing.")
...
class Model7BMemory:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(
        remaining.to(GiB).magnitude > 60,
        f"7-billion-parameter FP16 weights leave less A100 memory than expected: {remaining.to(GiB).magnitude:.1f} GiB.",
    )
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
