...
- **CUDA GPU**\index{GPU Backend!CUDA implementation}: cuBLAS GEMM kernels and CUDA streams for async execution [@nvidia_cublas; @choquette2021]
- **CPU**\index{CPU Backend!optimized BLAS libraries}: Intel oneMKL or OpenBLAS, with thread-level parallelism around optimized kernels [@intelOneMKL; @openblasDocs]
- **TPU**\index{TPU!XLA compilation}: XLA compilation to TPU-specific high-level optimizer (HLO) operations [@GoogleXLA]
