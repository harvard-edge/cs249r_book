...
The execution problem asks a seemingly simple question—should operations execute immediately as written, or be recorded for later execution?—that creates a cascade of engineering trade-offs shaping every aspect of framework behavior.
...
The compilation overhead in these examples (approximately 100ms to compile the first time, microseconds to reuse) illustrates why torch.compile is so effective. But *why* does compilation help so much? The answer lies in understanding the physics of software overhead. Dispatch costs that seem negligible for a single operation—a few microseconds here and there—compound dramatically across the thousands of operations in a forward pass. The following analysis quantifies this phenomenon.
...
@fig-python-tax visualizes this tax in action. Notice how eager execution (top) creates "gaps" where the GPU sits idle while Python dispatches the next kernel. The blue compute regions are short; the red dispatch regions are comparatively long. Compilation (bottom) fuses these operations into a single kernel launch, eliminating the gaps entirely—the GPU spends nearly all its time computing rather than waiting.
...
The natural question is: can this fusion happen automatically? PyTorch 2.0's `torch.compile` attempts exactly this—capturing eager code and compiling it into fused kernels without requiring users to write custom CUDA.
...
The execution problem determines *when* computation happens and *what* optimizations are possible. But neural network training requires a capability that no amount of clever scheduling can provide: the ability to compute gradients automatically.
...
Why does every modern ML framework default to reverse mode for training? The answer is computational asymmetry—and understanding this asymmetry explains one of the most important design decisions in all of machine learning software.
...
The memory and system integration challenges examined above—caching allocators, activation storage, and checkpoint overhead—affect all frameworks. But *how* frameworks implement automatic differentiation in the first place varies significantly, with consequences for both *optimization potential* and developer experience. The distinction between *tape-based and transform-based autodiff* captures this fundamental divergence.
...
The execution and differentiation problems together enable the training loop: the execution model determines when computation happens, while automatic differentiation computes the gradients that drive learning. But both assume that the same code can run across diverse hardware: CPUs, GPUs, TPUs, mobile devices, and microcontrollers. This hardware diversity creates the third and final fundamental problem.
...
Tensors and their memory layouts establish *what* the framework computes with. But where that data physically resides—and how it moves between locations—determines whether computation happens at full speed or crawls.
...
The `nn.Module` patterns above illustrate PyTorch's approach to the abstraction problem. But PyTorch is only one of several major frameworks, and its choices—mutable state, class inheritance, eager execution by default—are not the only valid design points. TensorFlow centralizes state differently, and JAX avoids mutable state entirely. These are not superficial API differences; they reflect fundamentally different answers to the three problems we examined at the chapter's start.
...
This approach won the research community for a simple reason: it treats deep learning as standard Python programming. You can use Python loops, conditionals, and debuggers (like `pdb`) directly within your model's forward pass—no special syntax, no separate compilation step, no waiting to see if your code works. This "Eager Execution" model enables rapid iteration and intuitive model design, which is essential for the trial-and-error nature of frontier AI research.
...
- Custom differentiation (higher-order gradients, custom VJP/JVP rules)—pure functions make differentiation rules composable
- Research on optimization algorithms—transformations let you experiment with gradient manipulation
- Large-scale distributed training (particularly on TPUs)—XLA compilation extracts maximum hardware utilization
- Scientific computing with AD requirements—functional purity enables mathematical reasoning about code
...
The preceding sections described each framework's design philosophy in qualitative terms—graph-first versus eager-first, stateful versus functional. But design philosophy claims are only meaningful when backed by measurement. @tbl-mlfm-comparison quantifies how the architectural choices of TensorFlow, PyTorch, and JAX translate to system characteristics. When examining this comparison, note particularly the differences in execution mode, compilation optimization potential, and distributed scalability—these dimensions most directly impact production deployment decisions.
...
The efficiency gaps measured above—17× latency differences, 7× memory differences—transform from optimization opportunities into hard deployment constraints as we move to resource-constrained devices. The three fundamental problems reweight dramatically at the edge. The *execution problem* shifts from "eager vs. graph" to "can we execute at all within 10ms and 50KB?" The *differentiation problem* often disappears entirely, since edge devices run inference only. The *abstraction problem* intensifies: targeting ARM vs. x86, mobile NPUs vs. edge TPUs, microcontrollers with kilobytes of memory.
