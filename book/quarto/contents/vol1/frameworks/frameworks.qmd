...
# PURPOSE
# =============================================================================
# Purpose: Extract GPU and model specifications for use throughout chapter
# Used in: Multiple inline references for A100 specs and model parameters
...
# PROCESS
# =============================================================================
# Extract GPU memory capacity
a100_mem_value = A100_MEM_CAPACITY.to(GiB).magnitude
# Extract GPU memory bandwidth
a100_bw_tbs_value = A100_MEM_BW.to(TB/second).magnitude
# Extract GPU FP16 TFLOPS (tensor core)
a100_tflops_fp16_value = A100_FLOPS_FP16_TENSOR.to(TFLOPs/second).magnitude
# Calculate sparse TFLOPS (2x dense)
a100_tflops_sparse_value = a100_tflops_fp16_value * 2
# Extract ResNet-50 parameters
resnet_params_m_value = RESNET50_PARAMS.to(Mparam).magnitude
# Extract GPT-3 parameters (convert to billions)
gpt3_params_b_value = GPT3_PARAMS.to(Mparam).magnitude / 1000
# Extract ResNet-50 FLOPs (in GFLOPs)
resnet_gflops_value = RESNET50_FLOPs.to(GFLOPs).magnitude

# Derived: model memory footprints (using model_memory helper for unit safety)
gpt3_fp16_gb_value = model_memory(GPT3_PARAMS, BYTES_FP16, GB)
resnet_fp32_mb_value = model_memory(RESNET50_PARAMS, BYTES_FP32, MB)
resnet_adam_mb_value = model_memory(RESNET50_PARAMS, BYTES_ADAM_STATE, MB)
# Derived: 7B model reference (used repeatedly for examples)
model_7b_params = 7e9
model_7b_fp16_gb_value = model_memory(model_7b_params, BYTES_FP16, GB)
model_7b_adam_gb_value = model_memory(model_7b_params, BYTES_ADAM_STATE, GB)
model_7b_total_gb_value = model_7b_fp16_gb_value + model_7b_adam_gb_value
# Derived: interconnect bandwidths
pcie4_gbs_value = PCIE_GEN4_BW.to(GB/second).magnitude
nvlink_a100_gbs_value = NVLINK_A100_BW.to(GB/second).magnitude
a100_bw_gbs_value = A100_MEM_BW.to(GB/second).magnitude
# Derived: transfer times for 4 MB tensor
...
# Derived: 1 GB transfer over PCIe 4.0
...
# Derived: DataLoader throughput calculations
img_per_sec = 1000
img_res = 224
img_channels = 3
dataloader_mbs_value = img_per_sec * img_res * img_res * img_channels / 1e6
# Batch of 64 images at 224x224x3 in FP32
batch_size = 64
batch_bytes = batch_size * img_res * img_res * img_channels * BYTES_FP32.magnitude
batch_mb_value = batch_bytes / 1e6
batch_transfer_ms_value = batch_bytes / (PCIE_GEN4_BW.to(byte/second).magnitude) * 1e3
...
a100_bw_gbs_str = fmt(a100_bw_gbs_value, precision=0, commas=False)
a100_tflops_fp16_str = fmt(a100_tflops_fp16_value, precision=0, commas=False)
a100_tflops_sparse_str = fmt(a100_tflops_sparse_value, precision=0, commas=False)
...
resnet_adam_mb_str = fmt(resnet_adam_mb_value, precision=0, commas=False)
...
model_7b_fp16_gb_str = fmt(model_7b_fp16_gb_value, precision=0, commas=False)
model_7b_adam_gb_str = fmt(model_7b_adam_gb_value, precision=0, commas=False)
model_7b_total_gb_str = fmt(model_7b_total_gb_value, precision=0, commas=False)
a100_remaining_7b_gb_str = fmt(a100_mem_value - model_7b_fp16_gb_value, precision=0, commas=False)
pcie4_gbs_str = fmt(pcie4_gbs_value, precision=0, commas=False)
pcie4_bidir_gbs_str = fmt(pcie4_gbs_value * 2, precision=0, commas=False)
nvlink_a100_gbs_str = fmt(nvlink_a100_gbs_value, precision=0, commas=False)
pcie4_4mb_ms_str = fmt(pcie4_4mb_ms_value, precision=3, commas=False)
nvlink_4mb_ms_str = fmt(nvlink_4mb_ms_value, precision=3, commas=False)
hbm_4mb_ms_str = fmt(hbm_4mb_ms_value, precision=3, commas=False)
pcie4_1gb_ms_str = fmt(pcie4_1gb_ms_value, precision=0, commas=False)
pcie4_1gb_equiv_ops_str = fmt(pcie4_1gb_equiv_ops_value / 1e12, precision=1, commas=False)
dataloader_mbs_str = fmt(dataloader_mbs_value, precision=0, commas=False)
batch_mb_str = fmt(batch_mb_value, precision=0, commas=False)
batch_transfer_ms_str = fmt(batch_transfer_ms_value, precision=1, commas=False)
```
...
[^fn-flashattention-frameworks]: FlashAttention (introduced in @sec-dnn-architectures) exemplifies kernel fusion taken to its logical extreme. By fusing the entire attention computation into a single kernel that tiles data to fit in SRAM, it reduces HBM traffic by 10–20× and achieves 2–4× wall-clock speedup. This demonstrates that frameworks enabling such fusions can transform memory-bound operations into compute-bound ones.
...
# PURPOSE
# =============================================================================
# Purpose: Calculate overhead reduction and bandwidth efficiency from kernel fusion
# Used in: Callout "The Physics of Software Overhead" comparing eager vs compiled mode
...
kernel_launch_us = 5
memory_access_us = 1
kernel_launch_us_value = python_dispatch_us + kernel_launch_us  # Python dispatch + kernel launch per op

eager_n_ops_value = 2  # Eager mode: 2 kernel launches (Add + ReLU)
compiled_n_ops_value = 1  # Compiled mode: 1 fused kernel launch
eager_mem_factor_value = 4  # 4N bytes (2 reads + 2 writes)
compiled_mem_factor_value = 2  # 2N bytes (fused, intermediate stays in registers)

# =============================================================================
# PROCESS
# =============================================================================
# Calculate eager mode overhead
eager_overhead_us_value = eager_n_ops_value * kernel_launch_us_value
# Calculate compiled mode overhead
compiled_overhead_us_value = compiled_n_ops_value * kernel_launch_us_value
# Calculate overhead speedup from fusion
overhead_speedup_value = eager_overhead_us_value // compiled_overhead_us_value
# Calculate bandwidth efficiency improvement
bw_efficiency_value = eager_mem_factor_value // compiled_mem_factor_value
...
kernel_launch_us_str = f"{kernel_launch_us_value}"
eager_overhead_str = f"{eager_overhead_us_value}"
compiled_overhead_str = f"{compiled_overhead_us_value}"
overhead_speedup_str = f"{overhead_speedup_value}"
bw_efficiency_str = f"{bw_efficiency_value}"
```
...
# PURPOSE
# =============================================================================
# Purpose: Calculate dispatch overhead ratios for small vs large models
# Used in: Callout "The Dispatch Tax" comparing overhead-bound vs compute-bound scenarios

# =============================================================================
# INPUT
# =============================================================================
# Scenario 1: Small MLP (Overhead Bound)
dispatch_n_ops_value = 6
dispatch_us_per_op_value = 5
dispatch_hw_time_us_value = 2.6
# Scenario 2: GPT-3 Layer (Compute Bound)
gpt3_hw_time_us_value = 100_000
gpt3_sw_time_us_value = 50

# =============================================================================
# PROCESS
# =============================================================================
# Scenario 1 calculations
dispatch_sw_time_value = dispatch_n_ops_value * dispatch_us_per_op_value
dispatch_ratio_small_value = dispatch_sw_time_value / dispatch_hw_time_us_value
dispatch_total_us_value = dispatch_sw_time_value + dispatch_hw_time_us_value
dispatch_overhead_pct_value = dispatch_sw_time_value / dispatch_total_us_value * 100
dispatch_compilation_speedup_value = dispatch_total_us_value / dispatch_hw_time_us_value
# Scenario 2 calculations
dispatch_ratio_large_value = gpt3_sw_time_us_value / gpt3_hw_time_us_value

# =============================================================================
# OUTPUT
# =============================================================================
dispatch_ratio_small_str = fmt(dispatch_ratio_small_value, precision=1, commas=False)
dispatch_sw_time_str = fmt(dispatch_sw_time_value, precision=0, commas=False)
dispatch_ratio_large_str = fmt(dispatch_ratio_large_value, precision=4, commas=False)
dispatch_overhead_pct_str = fmt(dispatch_overhead_pct_value, precision=0, commas=False)
dispatch_compilation_speedup_str = fmt(dispatch_compilation_speedup_value, precision=0, commas=False)
```
...
*   **Software Overhead**: $T_{sw} \approx {python} gpt3_sw_time_us_value \mu s$.
...
- **Total training footprint**: ~10–15 GB versus ~`{python} resnet_fp32_mb_str` MB for inference
...
Mixed-precision training offers a third approach: storing activations in FP16 rather than FP32 halves the activation memory while modern Tensor Cores execute FP16 matrix multiplications at 2x the throughput of FP32. The net effect is both reduced $D_{vol}$ (smaller activations) and increased hardware utilization ($\eta$).
...
# PURPOSE
# =============================================================================
# Purpose: Calculate memory breakdown for "Administrative Tax" callout
# Used in: Callout "The Administrative Tax"
...
# Scenario: 1B parameter model (FP16 weights)
admin_params_count = 1e9
admin_batch_size = 32
admin_layers = 100
admin_width = 1024
...
# PROCESS
# =============================================================================
# Weights: 1B * 2 bytes (FP16) = 2 GB
admin_weights_gb_value = (admin_params_count * BYTES_FP16).to(GB).magnitude
# Gradients: Same as weights (FP16)
admin_grads_gb_value = admin_weights_gb_value
# Optimizer (Adam): 2 states * 4 bytes (FP32) = 8 bytes/param = 4x weights(FP16)
# Wait, text says 8 GB. 1B * 8 bytes = 8 GB. Correct.
admin_opt_gb_value = (admin_params_count * BYTES_ADAM_STATE).to(GB).magnitude

# Activations: Batch * Layers * Width^2 * 2 bytes
# 32 * 100 * 1024^2 * 2 = 6,710,886,400 bytes = 6.71 GB
admin_act_bytes = admin_batch_size * admin_layers * (admin_width ** 2) * BYTES_FP16
admin_act_gb_value = admin_act_bytes.to(GB).magnitude

admin_total_gb_value = admin_weights_gb_value + admin_grads_gb_value + admin_opt_gb_value + admin_act_gb_value
admin_tax_gb_value = admin_total_gb_value - admin_weights_gb_value

# =============================================================================
# OUTPUT
# =============================================================================
admin_weights_gb_str = fmt(admin_weights_gb_value, precision=0, commas=False)
admin_grads_gb_str = fmt(admin_grads_gb_value, precision=0, commas=False)
admin_opt_gb_str = fmt(admin_opt_gb_value, precision=0, commas=False)
admin_act_gb_str = fmt(admin_act_gb_value, precision=1, commas=False)
admin_total_gb_str = fmt(admin_total_gb_value, precision=0, commas=False)
admin_tax_gb_str = fmt(admin_tax_gb_value, precision=0, commas=False)
admin_batch_str = fmt(admin_batch_size, precision=0, commas=False)
admin_layers_str = fmt(admin_layers, precision=0, commas=False)
admin_width_str = fmt(admin_width, precision=0, commas=False)
```
...
4.  **Activations**: For a batch size of `{python} admin_batch_str` and a `{python} admin_layers_str`-layer network, you must store every intermediate layer output for the backward pass.
    ...
    For a `{python} admin_width_str`-width model: ${python} admin_batch_str \times {python} admin_layers_str \times {python} admin_width_str^{2} \times 2 \approx \mathbf{{python} admin_act_gb_str \text{ GB}}$. (Each layer's activation is a `Width × Width` matrix per sample—appropriate for transformer-style models where intermediate projections scale with hidden dimension squared.)
...
# PURPOSE
# =============================================================================
# Purpose: Calculate FLOPs and memory traffic for training step anatomy
# Used in: "Training Step Anatomy" section
...
# PROCESS
# =============================================================================
# MatMul 1: [32, 784] @ [784, 256] -> [32, 256]
# FLOPs = 2 * M * N * K
train_mm1_flops = 2 * train_batch * train_input * train_hidden
train_mm1_mem_bytes = (train_batch * train_input + train_input * train_hidden + train_batch * train_hidden) * 4 # FP32

# ReLU: [32, 256]
train_relu_flops = train_batch * train_hidden
train_relu_mem_bytes = (train_batch * train_hidden) * 2 * 4 # Read + Write

# MatMul 2: [32, 256] @ [256, 10] -> [32, 10]
train_mm2_flops = 2 * train_batch * train_hidden * train_output
train_mm2_mem_bytes = (train_batch * train_hidden + train_hidden * train_output + train_batch * train_output) * 4

# Cross Entropy: [32, 10]
train_ce_flops = train_batch * train_output * 3 # Approx (exp + sum + div)
...
# Backward memory: Read activations, weights, grads. Approx 2x forward traffic?
# Text uses ~3.2 MB for backward memory.
train_bwd_mem_str = "~3.2 MB"
train_bwd_ai_str = "~8.0"

# =============================================================================
# OUTPUT
# =============================================================================
train_mm1_flops_str = fmt(train_mm1_flops / 1e6, precision=1, commas=False) + "M"
train_mm1_mem_str = fmt(train_mm1_mem_bytes / 1e6, precision=1, commas=False) + " MB"
train_mm1_ai_str = fmt(train_mm1_flops / train_mm1_mem_bytes, precision=1, commas=False)

train_relu_flops_str = fmt(train_relu_flops / 1e3, precision=0, commas=False) + "K"
train_relu_mem_str = fmt(train_relu_mem_bytes / 1e3, precision=0, commas=False) + " KB"
train_relu_ai_str = fmt(train_relu_flops / train_relu_mem_bytes, precision=3, commas=False)

train_mm2_flops_str = fmt(train_mm2_flops / 1e3, precision=0, commas=False) + "K"
train_mm2_mem_str = fmt(train_mm2_mem_bytes / 1e3, precision=0, commas=False) + " KB"
train_mm2_ai_str = fmt(train_mm2_flops / train_mm2_mem_bytes, precision=1, commas=False)

train_ce_flops_str = fmt(train_ce_flops / 1e3, precision=0, commas=False) + "K"
train_ce_mem_str = fmt(train_ce_mem_bytes / 1e3, precision=0, commas=False) + " KB"
train_ce_ai_str = fmt(train_ce_flops / train_ce_mem_bytes, precision=1, commas=False)

train_bwd_flops_str = fmt(train_bwd_flops / 1e6, precision=0, commas=False) + "M"
```
...
# PURPOSE
# =============================================================================
# Purpose: Calculate MNIST training step timing breakdown using roofline analysis
# Used in: Training step analysis showing compute, memory, and overhead components
...
from physx.constants import *
from physx.formatting import fmt
# MNIST training step workload
mnist_total_flops_value = 40e6  # ~40M FLOPs
mnist_mem_traffic_bytes_value = 5e6  # ~5 MB
# A100 hardware specs
a100_peak_tflops_value = A100_FLOPS_FP16_TENSOR.to(flop/second).magnitude
a100_mem_bw_bytes_value = A100_MEM_BW.to(byte/second).magnitude
# Overhead parameters
mnist_n_ops_value = 6
mnist_us_per_op_value = 5

# =============================================================================
# PROCESS
# =============================================================================
# Calculate compute time
mnist_t_compute_us_value = mnist_total_flops_value / a100_peak_tflops_value * 1e6
# Calculate memory time
mnist_t_memory_us_value = mnist_mem_traffic_bytes_value / a100_mem_bw_bytes_value * 1e6
# Calculate overhead time
mnist_t_overhead_us_value = mnist_n_ops_value * mnist_us_per_op_value
...
# PURPOSE
# =============================================================================
# Purpose: Calculate framework performance gap and deployment memory spectrum ratios
# Used in: Fallacies section comparing PyTorch vs TensorRT performance and memory requirements
...
pytorch_mobile_mb_value = 220
tflite_micro_kb_value = 32

# =============================================================================
# PROCESS
# =============================================================================
# Calculate performance gap ratio
perf_gap_value = pytorch_ms_value / tensorrt_ms_value
# Calculate memory ratio (convert MB to KB for comparison)
memory_ratio_value = pytorch_mobile_mb_value * 1024 / tflite_micro_kb_value

# =============================================================================
# OUTPUT
# =============================================================================
pytorch_ms_str = fmt(pytorch_ms_value, precision=0, commas=False)
tensorrt_ms_str = fmt(tensorrt_ms_value, precision=0, commas=False)
pytorch_mobile_mb_str = fmt(pytorch_mobile_mb_value, precision=0, commas=False)
tflite_micro_kb_str = fmt(tflite_micro_kb_value, precision=0, commas=False)
perf_gap_str = fmt(perf_gap_value, precision=0, commas=False)
memory_ratio_str = fmt(memory_ratio_value, precision=0, commas=False)
```
...
# PURPOSE
# =============================================================================
# Purpose: Calculate total execution time comparing eager vs compiled mode with recompilation overhead
# Used in: Pitfall section demonstrating when compilation overhead negates performance benefits

# =============================================================================
# INPUT
# =============================================================================
# Compilation overhead pitfall calculation
n_images_value = 10_000
eager_throughput_value = 1_450  # images/sec
compiled_throughput_value = 2_150  # images/sec
n_recompilations_value = 10
compilation_time_s_value = 30  # seconds per recompilation

# =============================================================================
# PROCESS
# =============================================================================
# Calculate eager mode total time
eager_total_value = n_images_value / eager_throughput_value
# Calculate compiled mode total time (includes recompilation overhead)
compiled_total_value = n_images_value / compiled_throughput_value + n_recompilations_value * compilation_time_s_value

# =============================================================================
# OUTPUT
# =============================================================================
n_images_str = f"{n_images_value:,}"
n_recompilations_str = fmt(n_recompilations_value, precision=0, commas=False)
eager_throughput_str = f"{eager_throughput_value:,}"
compiled_throughput_str = f"{compiled_throughput_value:,}"
compilation_time_s_str = fmt(compilation_time_s_value, precision=0, commas=False)
eager_total_str = fmt(eager_total_value, precision=1, commas=False)
compiled_total_str = fmt(compiled_total_value, precision=1, commas=False)
```
