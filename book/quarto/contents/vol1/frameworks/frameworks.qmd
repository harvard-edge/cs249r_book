...
train_relu_flops_str = fmt((train_relu_flops * flop).to(KFLOPs).magnitude, precision=1, commas=False) + "K"
train_relu_mem_str = fmt((train_relu_mem_bytes * byte).to(KB).magnitude, precision=1, commas=False) + " KB"
train_relu_ai_str = fmt(train_relu_flops / train_relu_mem_bytes, precision=2, commas=False)

train_mm2_flops_str = fmt((train_mm2_flops * flop).to(KFLOPs).magnitude, precision=1, commas=False) + "K"
train_mm2_mem_str = fmt((train_mm2_mem_bytes * byte).to(KB).magnitude, precision=1, commas=False) + " KB"
train_mm2_ai_str = fmt(train_mm2_flops / train_mm2_mem_bytes, precision=1, commas=False)

train_ce_flops_str = fmt((train_ce_flops * flop).to(KFLOPs).magnitude, precision=1, commas=False) + "K"
train_ce_mem_str = fmt((train_ce_mem_bytes * byte).to(KB).magnitude, precision=1, commas=False) + " KB"
train_ce_ai_str = fmt(train_ce_flops / train_ce_mem_bytes, precision=2, commas=False)

train_bwd_flops_str = fmt((train_bwd_flops * flop).to(MFLOPs).magnitude, precision=1, commas=False) + "M"

# ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
check(train_mm1_flops / train_mm1_mem_bytes > train_relu_flops / train_relu_mem_bytes,
