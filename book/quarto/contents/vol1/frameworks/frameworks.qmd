...
Machine learning frameworks evolved by progressively abstracting hardware execution details into higher-level API primitives. Trace the historical progression from 1979 to 2018 in @fig-mlfm-timeline, observing how numerical software moved up the ladder from low-level linear algebra routines to compiled graph execution engines.
...
Mathematical operations can be decoupled from physical execution through graph representations. Examine the directed acyclic graph in @fig-comp-graph, noting how tensor variables map to data nodes while operations map to transformation nodes.
