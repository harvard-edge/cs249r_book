...
class ResNetMemory:
    ...
    training_ratio = (training_min_gb * 1024) / fp32_mb
...
The memory breakdown for ResNet-50 in @sec-frameworks-autograd-principle-memory showed a concrete ~`{python} ResNetMemory.resnet_training_ratio_str`x ratio between training and inference memory. Here we generalize that analysis to reveal the full administrative overhead at billion-parameter scale.
