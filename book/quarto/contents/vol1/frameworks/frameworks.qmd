...
Each step rightward sacrifices flexibility for performance. The practical question is *where* on this continuum a given project should operate. The optimal compilation strategy depends on production executions, development recompilations, and their respective time costs, combined in @eq-compilation-benefit:
...
This model assumes that the compiled artifact remains valid between development changes and that the initial compilation and each recompilation have the same cost.
...
When Overhead Ratio $> 1$, the model is overhead-bound. Compilation can provide its largest gains for overhead-bound workloads because fusion reduces the number of dispatches.
...
- [ ] **Dispatch-overhead estimate**: If six operations each incur 5 $\mu$s of dispatch while hardware work takes 2.6 $\mu$s, which term dominates?
