...
# Derived: model memory footprints
gpt3_fp16_gb_value = (GPT3_PARAMS.magnitude * BYTES_FP16.magnitude) / 1e9
resnet_fp32_mb_value = (RESNET50_PARAMS.magnitude * BYTES_FP32.magnitude) / 1e6
# Derived: 7B model reference (used repeatedly for examples)
model_7b_params = 7e9
model_7b_fp16_gb_value = (model_7b_params * BYTES_FP16.magnitude) / 1e9
model_7b_adam_gb_value = (model_7b_params * BYTES_ADAM_STATE.magnitude) / 1e9
...
# INPUT
# =============================================================================
kernel_launch_us_value = 15  # Python dispatch + kernel launch per op
...
*   **Software Overhead**: T_sw ≈ 6 ops × 5 μs/op = `{python} dispatch_sw_time_str` μs.
*   **Ratio**: `{python} dispatch_sw_time_str` / 2.6 ≈ **`{python} dispatch_ratio_small_str`**.
...
*   **Hardware Time**: T_hw ≈ 100 ms = 100,000 μs.
*   **Software Overhead**: $T_{sw} \approx 50 \mu s$.
*   **Ratio**: 50 / 100,000 ≈ **`{python} dispatch_ratio_large_str`**.
...
- **Optimizer state (Adam)**: ~`{python} fmt(RESNET50_PARAMS.magnitude * BYTES_ADAM_STATE.magnitude / 1e6, precision=0, commas=False)` MB (two momentum buffers per parameter)
...
4.  **Activations**: For a batch size of 32 and a 100-layer network, you must store every intermediate layer output for the backward pass.

    $$ \text{Activations} \approx \text{Batch} \times \text{Layers} \times \text{Width}^{2} \times 2 \text{ bytes} $$
    For a 1024-width model: $32 \times 100 \times 1024^{2} \times 2 \approx \mathbf{6.7 \text{ GB}}$. (Each layer's activation is a `Width × Width` matrix per sample—appropriate for transformer-style models where intermediate projections scale with hidden dimension squared.)

**The Systems Conclusion**: Your 2 GB model has an **"Administrative Tax"** of ~19 GB (2 GB gradients + 8 GB optimizer + 6.7 GB activations) before you even process the first batch. During training, **Data Movement** includes saving and retrieving these activations, which is why training is often 3–4× slower than pure inference.
:::
...
[^fn-nvlink]: **NVLink**: NVIDIA's high-bandwidth interconnect for GPU-to-GPU communication, providing `{python} nvlink_a100_gbs_str` GB/s bidirectional bandwidth (NVLink 3.0 on A100) compared to `{python} fmt(pcie4_gbs_value * 2, precision=0, commas=False)` GB/s bidirectional for PCIe 4.0 x16. Critical for multi-GPU training where gradient synchronization requires moving gigabytes per iteration. NVSwitch extends NVLink to connect 8 GPUs in a fully-connected topology (DGX systems), enabling all-to-all communication without bottlenecks. The ~10× bandwidth advantage over PCIe determines whether tensor parallelism is practical for a given model size.
...
The device bandwidth hierarchy and overlap techniques above govern how tensors move between devices. With these hardware-level constraints in place, we turn to the higher-level structures that organize data for machine learning workflows. The core systems principle is straightforward: the data pipeline must sustain the accelerator's consumption rate. A GPU processing 1,000 images per second at 224×224 resolution requires approximately `{python} fmt(1000 * 224 * 224 * 3 / 1e6, precision=0, commas=False)` MB/s of sustained data throughput. If the pipeline cannot maintain this rate, the accelerator idles and the effective utilization term in the **Iron Law** drops below 1.
...
2. **Kernel Selection**\index{cuBLAS!kernel selection} (~0.5μs): cuBLAS selects an optimized GEMM kernel based on matrix dimensions (32×784 × 784×256). For these dimensions, it might choose a tiled algorithm optimized for L2 cache.
