...
# │      Assume 12 ops × 5 μs dispatch = 60 μs overhead. The step is overhead-bound!
...
class MnistTrainingStepCalc:
    ...
    t_overhead_us = n_ops * us_per_op  # 60 μs (dominant!)
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(
...
: **Hardware Backends Behind a Single Code Path**: Each backend implements the same tensor operations through a different library and execution mechanism, so identical Python maps onto distinct silicon. {#tbl-framework-backends tbl-colwidths="[22,37,41]"}
