...
class MemoryWallSpecs:
    ...
    flops_fp16 = gpu.compute.peak_flops.to(TFLOPs/second).magnitude
    bw_tbs = gpu.memory.bandwidth.to(TB/second).magnitude
...
class A100BLASMemoryWallRecap:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    dense_flops = gpu.compute.peak_flops.to(TFLOPs/second).magnitude
...
class DispatchTaxSmallLarge:
    ...
    dispatch_sw_time_str = fmt_time(small_sw_total, 'microsecond', precision=0, commas=False)
...
class ResNetMemory:
    ...
    fp32_mb = fp32.to(MB).magnitude
    adam_mb = adam.to(MB).magnitude
    training_min = training_min_gb * GB
    training_max = training_max_gb * GB
...
class TensorFootprint:
    ...
    footprint_mb = footprint.to(MB).magnitude
...
class AdminTax:
    ...
    grads_gb = grads.to(GB).magnitude
    opt_gb = opt.to(GB).magnitude
...
class DeviceBandwidthHierarchy:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    pcie4_gbs = Hardware.Cloud.A100.interconnect.bandwidth.to(GB/second).magnitude
    ...
    nvlink_a100_per_direction = nvlink_a100_per_direction_gbs * GB / second
    a100_bw_gbs = Hardware.Cloud.A100.memory.bandwidth.to(GB/second).magnitude
    a100_bw = Hardware.Cloud.A100.memory.bandwidth
    a100_bw_tbs = Hardware.Cloud.A100.memory.bandwidth.to(TB/second).magnitude
    a100_mem = Hardware.Cloud.A100.memory.capacity.to(GiB).magnitude  # pedagogical 80 GB nominal
    a100_mem_display = a100_mem * GiB
    ...
    pcie4_4mb_ms = (tensor_4mb/Hardware.Cloud.A100.interconnect.bandwidth.to(byte/second).magnitude) * THOUSAND
    nvlink_4mb_ms = (tensor_4mb/(Hardware.Cloud.A100.nvlink.bandwidth.to(byte/second).magnitude / 2)) * THOUSAND
    hbm_4mb_ms = (tensor_4mb/Hardware.Cloud.A100.memory.bandwidth.to(byte/second).magnitude) * THOUSAND

    # 1 GB transfer cost analysis
    # 1 GB/BW (B/s) * THOUSAND ms
    pcie4_1gb_ms = (BILLION/Hardware.Cloud.A100.interconnect.bandwidth.to(byte/second).magnitude) * THOUSAND
    # Equiv Ops: (ms / 1000) * FLOP/s
    pcie4_1gb_equiv_ops = (pcie4_1gb_ms/THOUSAND) * Hardware.Cloud.A100.compute.peak_flops.to(flop/second).magnitude
    ...
    a100_mem_str = fmt_qty(a100_mem_display, GiB, precision=0, commas=False, unit_label="GB")
    pcie4_4mb_ms_str = fmt_time(pcie4_4mb_ms, 'millisecond', precision=3, commas=False)
    nvlink_4mb_ms_str = fmt_time(nvlink_4mb_ms, 'millisecond', precision=3, commas=False)
    hbm_4mb_ms_str = fmt_time(hbm_4mb_ms, 'millisecond', precision=3, commas=False)
    pcie4_1gb_ms_str = fmt_time(pcie4_1gb_ms, 'millisecond', precision=1, commas=False)
    pcie4_1gb_equiv_ops_str = fmt_qty(pcie4_1gb_equiv_ops * flop, TFLOP, precision=1, commas=False)
...
class DataloaderThroughput:
    ...
    dataloader = dataloader_mbs * MB / second

    # Step 2: Batch transfer
    batch_bytes = batch_size * img_res * img_res * img_channels * BYTES_FP32
    batch_mb = batch_bytes.to(MB).magnitude
    batch_transfer_ms = (batch_bytes / Hardware.Cloud.A100.interconnect.bandwidth).to(second).magnitude * MS_PER_SEC
    ...
    batch_transfer_ms_str = fmt_time(batch_transfer_ms, 'millisecond', precision=1, commas=False)
    pcie4_gbs_str = fmt_qty(pcie4, GB/second, precision=0, commas=False)
```
...
class Module7bRuntimeMemory:
    ...
    model_7b_fp16 = model_7b_fp16_gb * GB
...
class HbmCapacityFootnote:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    a100_mem_gb = Hardware.Cloud.A100.memory.capacity.to(GiB).magnitude  # pedagogical 80 GB nominal
    a100_mem = a100_mem_gb * GiB
...
class TrainingStepCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    train_mm1_flops_str = fmt_qty(mm1_flops * flop, MFLOPs, precision=1, commas=False)
    train_mm1_mem_str = fmt_qty(mm1_mem_bytes * byte, MB, precision=1, commas=False)
    train_mm1_ai_str = fmt(mm1_flops/mm1_mem_bytes, precision=1, commas=False)

    train_relu_flops_str = fmt_qty(relu_flops * flop, KFLOPs, precision=1, commas=False)
    train_relu_mem_str = fmt_qty(relu_mem_bytes * byte, KB, precision=1, commas=False)
    train_relu_ai_str = fmt(relu_flops/relu_mem_bytes, precision=3, commas=False)

    train_mm2_flops_str = fmt_qty(mm2_flops * flop, KFLOPs, precision=1, commas=False)
    train_mm2_mem_str = fmt_qty(mm2_mem_bytes * byte, KB, precision=1, commas=False)
    train_mm2_ai_str = fmt(mm2_flops/mm2_mem_bytes, precision=1, commas=False)

    train_ce_flops_str = fmt_qty(ce_flops * flop, KFLOPs, precision=1, commas=False)
    train_ce_mem_str = fmt_qty(ce_mem_bytes * byte, KB, precision=1, commas=False)
    train_ce_ai_str = fmt(ce_flops/ce_mem_bytes, precision=1, commas=False)

    train_bwd_flops_str = fmt_qty(bwd_flops * flop, MFLOPs, precision=1, commas=False)
    train_bwd_mem_str = fmt_qty(bwd_mem_bytes * byte, MB, precision=1, commas=False)
...
class MnistTrainingStepCalc:
    ...
    peak_flops = Hardware.Cloud.A100.compute.peak_flops.to(flop/second).magnitude                      # 312e12 FLOP/s
    peak_flops_q = Hardware.Cloud.A100.compute.peak_flops
    mem_bw_bytes = Hardware.Cloud.A100.memory.bandwidth.to(byte/second).magnitude                               # ~2e12 B/s
    mem_bw_q = Hardware.Cloud.A100.memory.bandwidth
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    ...
    mem_traffic = mem_traffic_bytes * byte
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mnist_t_compute_us_str = fmt_time(t_compute_us, 'microsecond', precision=1, commas=False)
    mnist_t_memory_us_str = fmt_time(t_memory_us, 'microsecond', precision=1, commas=False)
    mnist_t_overhead_us_str = fmt_time(t_overhead_us, 'microsecond', precision=0, commas=False)
    mnist_total_flops_str = fmt_qty(total_flops * flop, MFLOPs, precision=1, commas=False)
    mnist_mem_traffic_str = fmt_qty(mem_traffic, MB, precision=1, commas=False)
    ...
    a100_peak_tflops_str = fmt_qty(peak_flops_q, TFLOP/second, precision=0, commas=False)
    a100_mem_bw_tbs_str = fmt_qty(mem_bw_q, TB/second, precision=1, commas=False)
```
...
class Model7BMemory:
    ...
    a100_mem_str = fmt_qty(a100_mem_display, GiB, precision=0, commas=False, unit_label="GB")                    # e.g. "80 GB"
```
...
class CompilationOverheadCalc:
    ...
    eager_total_str = fmt_time(eager_total, 'second', precision=1, commas=False)              # e.g. "6.9 s"
    compiled_total_str = fmt_time(compiled_total, 'second', precision=1, commas=False)        # e.g. "304.7 s"
