...
Machine learning models are often trained on large datasets under the assumption that increasing data volume improves performance. While this holds in many cases, it is not always necessary. A significant portion of a dataset is often redundant or contributes little to learning. Models can achieve the same—or even better—performance by training on a smaller, carefully selected subset of data. Identifying and retaining only the most valuable samples reduces dataset size without compromising accuracy, leading to more efficient training, faster inference, and lower storage and labeling costs.
...
A related technique, curriculum learning, improves training efficiency by structuring the order in which data is presented to the model. Instead of training on all samples at once, curriculum learning starts with simpler examples and gradually introduces more complex ones. Curriculum learning mirrors how humans learn by mastering fundamental concepts before tackling advanced topics. By prioritizing easier samples in the early stages of training, models can learn more efficiently, making better use of limited data and improving generalization.

These techniques—core-set selection, data pruning, and curriculum learning—demonstrate that bigger datasets are not always better. Careful selection and structuring of training data enables machine learning models to achieve the same or better performance with fewer examples, reducing computational demands and making AI development more efficient. Data-centric optimization is particularly beneficial in scenarios where collecting large-scale datasets is costly, impractical, or unnecessary.

## Dynamic Data Selection: Active Learning

Supervised learning has been a cornerstone of modern AI, enabling strong task performance. However, it comes with a significant limitation: it requires vast amounts of labeled data, which is often resource-intensive to obtain. In specialized fields such as medical diagnosis, autonomous driving, and scientific research, annotation requires domain expertise, further increasing costs. The reliance on large-scale labeled datasets raises a critical challenge—how can machine learning models learn effectively while minimizing the need for manual labeling?
...
Machine learning models often rely on large datasets to achieve strong performance, but collecting and labeling data is resource-intensive. Rather than continuously expanding datasets, machine learning systems can be designed to extract more value from the data that is already available. Value extraction improves data efficiency by ensuring that each sample contributes as much useful information as possible, reducing the need for additional data collection while still enhancing model performance.
...
Even when high-quality data is available, the way it is structured, retrieved, and used can significantly impact the efficiency and effectiveness of a machine learning system. Large data volumes alone are insufficient; models must be designed to make the most of the data they have. Optimizing data utilization involves ensuring that the right data is used at the right time and in the right way, minimizing waste while maximizing learning efficiency.
...
Through the integration of these strategies, machine learning systems can achieve higher performance with fewer resources, making AI more accessible and environmentally sustainable. Table @tbl-data-efficiency summarizes these optimization goals, their approaches, key techniques, and benefits.
...
Focusing on data quality rather than sheer volume allows machine learning practitioners to reduce computational overhead, decrease energy consumption, and accelerate training—all while maintaining or even improving model performance.
...
These methods enable machine learning systems to achieve the same level of performance with significantly less data, reducing training time, energy costs, and storage requirements.
...
Optimizing DUE enables machine learning practitioners to develop more scalable, cost-effective, and environmentally sustainable AI systems. This ensures that every data point contributes meaningfully to learning, leading to leaner, more efficient models that perform well without excessive resource consumption.
...
These integrated approaches allow machine learning practitioners to reduce compute costs, speed up training, and improve model scalability, all while maintaining accuracy.
...
Removing redundant data allows machine learning pipelines to accelerate training, lower compute costs, and improve scalability without affecting accuracy.
...
Active monitoring and redundancy reduction enable machine learning practitioners to develop more efficient, scalable, and sustainable AI systems. Removing redundant data not only reduces training costs and energy consumption but also improves deployment efficiency, making AI systems more adaptable to real-world constraints.
...
Monitoring and optimizing DGE ensures that dataset expansion remains cost-effective, preventing unnecessary data collection and focusing instead on data quality, curation, and augmentation strategies. Efficient dataset growth leads to more scalable, efficient, and sustainable AI systems, making the best use of computational resources while minimizing storage and energy costs.
...
Analysis of these metrics enables practitioners to identify bottlenecks and apply targeted interventions to optimize dataset composition, reduce computational overhead, and improve overall system efficiency. The following sections outline a structured approach to diagnosing inefficiencies, selecting appropriate optimization strategies, and evaluating the impact of interventions.
...
A stepwise approach to data efficiency optimization ensures that each improvement builds upon the previous one, leading to a streamlined and computationally efficient dataset.
...
Sequential application of each optimization enables machine learning pipelines to achieve higher performance while minimizing resource consumption. This ensures that data remains a valuable asset rather than an unnecessary burden in machine learning system design.
...
Adaptable evaluation protocols across different tasks allow benchmarks to enable meaningful and fair comparison of data efficiency improvements, rather than imposing one-size-fits-all evaluation criteria.
...
Structuring benchmarking methodologies around specific, measurable criteria ensures that DataPerf 2.0 provides transparent and meaningful insights, rather than relying on subjective efficiency claims.
