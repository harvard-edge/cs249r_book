...
The preceding chapters taught you to build, optimize, benchmark, and serve ML systems. Benchmarking (@sec-benchmarking-ai) told you how a model performs at a point in time; serving infrastructure (@sec-serving) showed how to answer requests in milliseconds. You deploy to production, and week one looks excellent. But what happens next? Data distributions shift, user behavior changes, and the world moves on from the conditions under which the model was trained. This chapter addresses what happens *over time*: how to ensure that a model's measured performance is maintained as the environment evolves around it.
