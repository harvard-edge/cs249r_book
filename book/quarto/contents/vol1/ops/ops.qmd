...
$$ T^* \approx \sqrt{\frac{2 \times `{python} retrain_cost_str`}{`{python} Q_str` \times `{python} V_str` \times `{python} A0_str` \times `{python} lam_str`}} \approx \mathbf{`{python} T_star_str` \text{ Day}} $$
...
1.  **Required Samples**: To distinguish `{python} baseline_acc_pct_str`% from `{python} target_acc_pct_str`% with high confidence, you need $\approx \mathbf{`{python} samples_needed_str`}$ labeled samples.
