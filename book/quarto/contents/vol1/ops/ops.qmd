...
The transition from single-model to platform operations represents a qualitative shift in complexity. Master the "ML Node" first; platform thinking builds upon this foundation.
...
* **ML systems fail silently**: Unlike software that crashes, ML degrades gradually. A model can maintain 100% uptime while accuracy drops 15% over weeks. Outcome monitoring is essential, not merely uptime tracking.
...
::: {.callout-chapter-connection title="The Safety Control Loop"}

MLOps provides the control loop for **reliability**—keeping the system up and the model accurate. But operational excellence alone does not guarantee beneficial outcomes. A system can achieve 99.9% uptime, sub-10ms latency, and continuous retraining, and still cause harm by amplifying bias or leaking data.

We have built a system that is efficient, scalable, and reliable. But is it *good*?

Efficiency is not ethics. Reliability is not responsibility. In **Responsible Engineering** (@sec-responsible-engineering), we face the final and most difficult constraint: aligning our technical optimization with human values, ensuring that what we build serves the world we want to live in.
