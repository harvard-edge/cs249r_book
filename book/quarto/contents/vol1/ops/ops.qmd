...
This chapter focuses on **single-model operations**—the practices required to deploy, monitor, and maintain an individual ML system in production. We call this the "ML Node": a complete operational unit comprising data pipelines, feature computation, model training, serving infrastructure, and monitoring for one machine learning application. The chapter covers operating one ML system end-to-end (training → deployment → monitoring → retraining), technical debt patterns that emerge within a single model's lifecycle, feature stores and CI/CD pipelines for individual model workflows, monitoring and drift detection for a single production model, and edge deployment patterns for embedded ML applications. Advanced topics such as platform operations managing hundreds of models, cross-model dependencies, multi-region coordination, and organization-wide ML platform engineering build upon this foundation but represent a qualitative shift in complexity beyond our scope here.
...
:::: {.callout-example title="Worked Example: Fraud Detection Retraining Economics" collapse="true"}
...
:::: {.callout-example title="Worked Example: Single-Model Monitoring Budget" collapse="true"}
...
::: {.callout-tip title="Principle Mapping Guide"}
