...
# PURPOSE
# =============================================================================
# Purpose: Estimate annual cost of training-serving skew.
# Used in: Principle 3 paragraph (Skew Cost).

# =============================================================================
# INPUT
# =============================================================================
queries_daily_value = 1_000_000
error_rate_value = 0.01
error_cost_value = 0.10
...
# OUTPUT
# =============================================================================
queries_daily_str = f"{queries_daily_value:,}"
error_rate_pct_str = f"{error_rate_value * 100:.0f}"
error_cost_str = f"{error_cost_value:.2f}"
skew_cost_str = fmt(annual_skew_cost_value, precision=0, commas=True)
```
...
# PURPOSE
# =============================================================================
# Purpose: Compound cost of manual operations.
# Used in: Callout "The Compound Cost of Manual Operations".
...
mc_manual_hours_per_week = 4
mc_pipeline_build_hours = 80
mc_time_horizon_years = 1
mc_manual_final_pct = 100
mc_pipeline_final_pct = 0
...
# OUTPUT
# =============================================================================
mc_manual_hours_str = fmt(mc_manual_hours_per_week, precision=0, commas=False)
mc_pipeline_hours_str = fmt(mc_pipeline_build_hours, precision=0, commas=False)
mc_breakeven_str = fmt(mc_breakeven_weeks, precision=0, commas=False)
mc_time_horizon_str = fmt(mc_time_horizon_years, precision=0, commas=False)
mc_manual_final_str = fmt(mc_manual_final_pct, precision=0, commas=False)
mc_pipeline_final_str = fmt(mc_pipeline_final_pct, precision=0, commas=False)
```
...
**Training-Serving Skew: Diagnosis and Prevention.**\index{Training-Serving Skew!silent accuracy degradation}
Training-serving skew\index{Training-Serving Skew!diagnosis and prevention} occurs when the model sees different features during inference than during training, causing silent accuracy degradation. The model continues to produce predictions without errors, but those predictions are simply less accurate.
...
# PURPOSE
# =============================================================================
# Purpose: Silent failure cost analysis for recommendation engine.
# Used in: Callout "The Cost of Silent Failures".
...
quality_drop_pct_str = f"{quality_drop * 100:.0f}"
quality_drop_str = f"{quality_drop}"
manual_detect_days_str = f"{manual_detect_days}"
auto_detect_days_str = f"{auto_detect_days}"
...
loss_diff_str = fmt(loss_diff, precision=0, commas=True)
...
# PURPOSE
# =============================================================================
# Purpose: Napkin math for fraud detection retraining interval.
# Used in: Callout "The Half-Life of a Model" and "Fraud Detection Retraining".
...
# Fraud detection retraining parameters
...
# Pre-formatted equation (for display math that can't contain inline Python)
...
Deployment strategies determine how models reach production; format optimization\index{Model Optimization!format conversion} determines how efficiently they run once deployed. Before deployment, models typically undergo format conversion and optimization to maximize serving performance. The gap between research frameworks (PyTorch, TensorFlow) and production serving can be substantial, and optimized formats often achieve 2-10x latency improvements over naive deployment approaches.
...
- **Post-Training Quantization (PTQ)**\index{Post-Training Quantization!INT8 conversion}: Convert trained FP32 model to INT8 without retraining. Typically achieves 2-4× speedup with <1% accuracy loss for well-behaved models. Requires calibration dataset (1000-10000 samples).
- **Quantization-Aware Training (QAT)**\index{Quantization-Aware Training!accuracy recovery}: Simulate quantization during training. Recovers accuracy lost in PTQ, essential for sensitive models. Adds 10-20% training time overhead.
- **Mixed Precision**\index{Mixed Precision!accuracy-performance tradeoff}: Keep sensitive layers (first, last, attention) in FP16/FP32 while quantizing others to INT8. Balances accuracy and performance.
...
Optimized models need runtime infrastructure to handle production traffic. Serving infrastructure\index{Inference Serving!production infrastructure} provides the interface between trained models and real-world systems, enabling predictions to be delivered reliably and efficiently. In large-scale settings, serving systems process tens of trillions of inference queries per day [@wu2019machine]. The measurement frameworks established in @sec-benchmarking-ai become essential for validating performance claims. Meeting such demand requires careful design that balances latency, scalability, and robustness.
...
1. Online Serving\index{Online Serving!real-time inference} provides low-latency, real-time predictions for interactive systems such as recommendation engines or fraud detection.
2. Offline Serving\index{Offline Serving!batch inference} processes large batches of data asynchronously, typically in scheduled jobs used for reporting or model retraining.
3. Near-Online (Semi-Synchronous) Serving\index{Near-Online Serving!hybrid latency} offers a balance between latency and throughput, appropriate for scenarios like chatbots or semi-interactive analytics.

Each of these approaches introduces different constraints in terms of availability, responsiveness, and throughput. The efficiency techniques from @sec-model-compression become crucial for meeting these performance requirements, particularly when serving models at scale. Serving systems are therefore constructed to meet specific Service Level Agreements (SLAs)\index{Service Level Agreement (SLA)!ML systems}[^fn-sla-examples] and Service Level Objectives (SLOs)\index{Service Level Objective (SLO)!latency and error rates}[^fn-slo-reality], which quantify acceptable performance boundaries along dimensions such as latency, error rates, and uptime. Achieving these goals requires a range of optimizations in request handling, scheduling, and resource allocation. Decomposing *the latency budget* reveals that model inference is often a minority of end-to-end response time.
...
# PURPOSE
# =============================================================================
# Purpose: Latency budget breakdown.
# Used in: Callout "The Latency Budget".
...
# assert total == slo_p99 # Commented out to avoid build failure if logic changes, but good for check
...
1. **Request scheduling and batching**\index{Request Batching!throughput optimization}: Aggregates inference requests to improve throughput. Clipper [@crankshaw2017clipper] applies batching and caching for low-latency online prediction.
...
3. **Load balancing**\index{Load Balancing!ML serving}: Distributes workloads evenly across instances. MArk [@zhang2019mark] demonstrates effective techniques for ML serving.
...
# PURPOSE
# =============================================================================
# Purpose: Cost per inference calculation.
# Used in: "Cost-Per-Inference Tracking" section.
...
# OUTPUT
# =============================================================================
hourly_gpu_cost_str = f"{hourly_gpu_cost:.0f}" # $3
...
# PURPOSE
# =============================================================================
# Purpose: Drift detection delay calculation.
# Used in: Callout "The Drift Detection Delay".
...
confidence = 0.95
qps_high = 1
# qps_low = 100 / (24 * 3600) # 100 per day
...
2.  **Detection Latency**: `{python} samples_needed_str` \text{ samples} / `{python} qps_high_str` \text{ QPS} = \mathbf{`{python} seconds_needed_high_str` \text{ seconds}} \approx \mathbf{`{python} minutes_needed_high_str` \text{ minutes}}$.
...
[^fn-drift-intuition]: **Drift in Practice**: Data Drift occurs when the input distribution $P(X)$ shifts (e.g., "everyone bought webcams in 2020," changing the frequency of webcam images). Concept Drift occurs when the relationship $P(Y|X)$ shifts (e.g., "users stopped clicking on clickbait titles they used to like," changing the predicted outcome for the same input).
...
ax.text(10, threshold + 0.01, "Drift Threshold", color=COLORS['RedLine'], fontsize=9)
...
# PURPOSE
# =============================================================================
# Purpose: Single-model monitoring cost breakdown.
# Used in: Callout "Single-Model Monitoring Budget".
...
n_variants = 3
metrics_per_variant = 50
samples_per_min = 4          # every 15 seconds
days = 30
hours_per_day = 24
mins_per_hour = 60
...
datapoints_mo_m = datapoints_mo / 1e6
ingestion_cost_per_m = 0.30
ingestion_cost = datapoints_mo_m * ingestion_cost_per_m

# Storage
bytes_per_point = 8
storage_gb = datapoints_mo * bytes_per_point / 1e9
storage_cost_per_gb = 1.00
storage_cost = storage_gb * storage_cost_per_gb

# Query compute
n_dashboards = 2
n_users = 3
queries_per_hr = 12   # 5-min refresh
work_hours = 8
work_days = 22
...
# PURPOSE
# =============================================================================
# Purpose: Monitoring cost-benefit analysis.
# Used in: Monitoring ROI formula example.
...
# =============================================================================
# PLOT: The Business Cost Curve
# =============================================================================
threshold = np.linspace(0.01, 0.99, 200)

# Asymmetric error costs for fraud detection
FP_cost, FN_cost = 100, 500

# Sigmoid-based rates model realistic classifier behavior:
...
ax.annotate(f"Optimal: T={opt_thresh:.2f}", (opt_thresh, opt_cost),
            ...
            fontsize=10, fontweight='bold', color=COLORS['GreenLine'])
...
# PURPOSE
# =============================================================================
# Purpose: Fraud detection improvement calculation.
# Used in: "Stakeholder Communication" section.
...
current_rate = 0.92
target_rate = 0.94
infra_cost_increase = 0.30
annual_loss_prevented = 2_000_000
false_positives_reduced = 50_000
...
# Single-model ROI calculation
incidents_avoided = 4
incident_cost = 25_000         # $25K each
hours_saved_monthly = 20
hourly_cost = 150              # $150/hr
mlops_investment = 30_000      # $30K/year
