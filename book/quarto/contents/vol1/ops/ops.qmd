...
**The Failure**: A deployment bug causes **Training-Serving Skew**, dropping recommendation quality by **2%**. This degrades conversion rate proportionally.

**Cost Analysis**:

1.  **Manual Ops (Monthly Review)**:
    *   Detection Time: ~4 weeks (28 days).
    *   Revenue Loss: USD 100M × 0.02 × 28/365 ≈ **USD `{python} loss_manual_str`**.

2.  **Automated MLOps (Daily Checks)**:
    *   Detection Time: 1 day.
    *   Revenue Loss: USD 100M × 0.02 × 1/365 ≈ **USD `{python} loss_auto_str`**.

**The Systems Conclusion**: A single silent failure costs **USD `{python} loss_diff_str`** more without MLOps. If you experience 3–5 such incidents a year (typical for high-drift domains), MLOps saves nearly **USD `{python} annual_savings_str`** (at 5 incidents). The "expensive" infrastructure pays for itself by reducing the **Time-to-Detection (TTD)** for the silent failures defined in the **Verification Gap**.
:::
...
# Used in: Callout "Fraud Detection Retraining".
...
# Fraud detection retraining parameters (from table above)
...
T_star_str = fmt(T_star, precision=2, commas=False)
```
...
- Data points per month: 3 × 50 × (4/min × 60 × 24 × 30) = `{python} dp_m_str` million
- Cost at USD 0.30/million: **USD `{python} ingest_str`/month**

**Storage:**

- At 8 bytes/point compressed: `{python} dp_m_str` M × 8 = `{python} storage_gb_str` GB
- Cost at USD 1.00/GB: **USD `{python} storage_cost_str`/month**

**Query compute:**

- Queries per month: 2 dashboards × 3 users × (12/hr × 8 hours × 22 days) = `{python} queries_str`
- Cost at USD 0.02/query: **USD `{python} query_cost_str`/month**
...
- 4 incidents/year avoided (at USD 25K each) = USD `{python} incident_savings_k` K saved
- 20 hours/month deployment time saved (at USD 150/hr) = USD `{python} time_savings_k` K saved
