...
The term DevOps was coined in 2009 by [Patrick Debois](https://www.jedi.be/), a consultant and Agile practitioner who organized the first [DevOpsDays](https://www.devopsdays.org/) conference in Ghent, Belgium. DevOps extended the principles of the [Agile](https://agilemanifesto.org/) movement, which emphasized close collaboration among development teams and rapid, iterative releases, by bringing IT operations into the fold.
...
MLOps systems decompose into distinct functional layers summarized in @tbl-mlops-layers that can evolve independently:
...
ML systems must make silent failures visible through continuous measurement. The key insight is that model performance degrades along a continuum as shown in @tbl-degradation-types rather than failing discretely:
...
These five principles form the evaluation framework summarized in @tbl-mlops-principles-summary for all MLOps tooling and practices. When assessing any tool or approach, ask: Does it enable reproducibility? Does it respect separation of concerns? Does it ensure consistency? Does it make degradation observable? Does it support cost-aware decisions?
...
Consider a scenario where a team fine-tunes a customer churn prediction model for a new product. The original model may embed product-specific behaviors or feature encodings that are not valid in the new setting. As performance issues emerge, teams may attempt to patch the model. They discover that the true problem lies several layers upstream, perhaps in the original feature selection or labeling criteria.
...
Feature stores centralize engineered features for reuse across models and teams. See @sec-ml-operations-feature-stores-e9a4 for details.
...
Common causes of training-serving skew are summarized in @tbl-training-serving-skew:
...
Feature stores address this problem by computing features once and serving them consistently to both training and serving pipelines, as shown in @lst-feature-store-consistency.
...
By computing `session_length` once in the feature pipeline, training and serving see identical values. Organizations report 8-15% accuracy improvements after eliminating skew through centralized feature stores.
...
Deciding when to retrain a model requires balancing accuracy maintenance against computational costs. Three common strategies exist as compared in @tbl-retraining-schedules, each with distinct tradeoffs:
...
Consider a fraud detection model with the parameters shown in @tbl-retraining-parameters:
...
The optimal interval scales with the square root of costs and inversely with the square root of value and decay rate, as shown in @tbl-retraining-sensitivity:
...
Model registries, such as [Vertex AI's model registry](https://cloud.google.com/vertex-ai/docs/model-registry/introduction), act as centralized repositories for storing and managing trained models. These registries not only facilitate version comparisons but also often include access to base models. These may be open source, proprietary, or a hybrid, such as [LLAMA](https://ai.meta.com/llama/). Deploying a model from the registry to an inference endpoint is streamlined, handling resource provisioning, model weight downloads, and hosting.
...
Track feature distributions against training baselines using statistical distance measures summarized in @tbl-feature-distribution-thresholds:
...
The infrastructure and operational practices examined above do not implement themselves. Translating these technical capabilities into reliable production systems requires coordinated contributions from professionals with diverse expertise. Unlike traditional software where a single engineer might own the full stack, ML systems span data engineering, statistical modeling, infrastructure, and governance, necessitating specialized roles with clear handoffs. Understanding who is responsible for each component of the MLOps lifecycle is essential for effective implementation.

## Roles and Responsibilities {#sec-ml-operations-roles-responsibilities-79d5}

The technical debt patterns, infrastructure components, and governance practices examined throughout this chapter require coordinated execution across multiple specialized roles. Data engineers build the data infrastructure that prevents data dependency debt. ML engineers implement the CI/CD pipelines that address pipeline debt. DevOps engineers provision the monitoring systems that make degradation observable. Data scientists develop the models that these systems support. No single role can independently manage the end-to-end machine learning lifecycle because each brings domain expertise essential to different phases of the workflow. @fig-roles-and-responsibilities illustrates how these roles interact.
...
Data engineers construct and maintain the data infrastructure that underpins machine learning systems. Building on the pipeline principles established in @sec-data-engineering, they ensure data flows reliably through the MLOps lifecycle from collection through model training and serving. In the context of MLOps, data engineers play a foundational role by building the **data infrastructure** components discussed earlier, including feature stores, data versioning systems, and validation frameworks, that enable scalable and reproducible data pipelines.

Their core responsibilities extend the data pipeline architecture from @sec-data-engineering into production contexts: orchestrating ingestion workflows using tools such as Apache Airflow, Prefect, or dbt [@garg2020practical]; implementing the ETL transformations that prepare data for downstream ML tasks; and maintaining the infrastructure that supports data-intensive workflows. This includes configuring distributed storage systems, managing compute clusters, and maintaining metadata catalogs that document data schemas, lineage, and access controls. To ensure reproducibility and governance, data engineers implement dataset versioning, maintain historical snapshots, and enforce data retention and auditing policies.
...
Data scientists design, develop, and evaluate machine learning models. Applying the development practices from @sec-ai-workflow and training techniques from @sec-ai-training, they translate business problems into learning tasks and iterate on model performance through systematic experimentation. Within the MLOps lifecycle, data scientists operate at the intersection of exploratory analysis and model development, contributing directly to the creation of predictive or decision-making capabilities.

Their MLOps-specific responsibilities center on integrating model development with operational infrastructure. This includes using experiment tracking tools such as MLflow and Weights & Biases to log configuration settings, evaluation results, and model artifacts, ensuring that model development remains reproducible and auditable [@rainio2024evaluation]. Data scientists collaborate with data engineers to ensure feature consistency across development and deployment environments, and with ML engineers to define validation criteria for production readiness.

Beyond initial development, data scientists participate in post-deployment monitoring and retraining workflows. They assist in analyzing data drift, interpreting shifts in model performance, and incorporating new data to maintain predictive accuracy over time. In collaboration with ML engineers, they define retraining strategies and evaluate the impact of updated models on operational metrics.
...
[^fn-telemetry-ml]: **ML Telemetry**: Automated collection of ML operational metrics including prediction latency, feature distributions, model outputs, and infrastructure utilization. Production systems generate 10GB-1TB daily across thousands of models. Time-series analysis of telemetry enables drift detection (statistical divergence from baseline), performance regression alerts, and capacity planning for GPU clusters.
...
The Security and Privacy Engineer safeguards machine learning systems against adversarial threats and privacy risks. As ML systems increasingly rely on sensitive data and are deployed in high-stakes environments, security and privacy become essential dimensions of system reliability. This role brings expertise in both traditional security engineering and ML-specific threat models, ensuring that systems are resilient to attack and compliant with data protection requirements.

Within the MLOps lifecycle, Security and Privacy Engineers enforce access control, encryption, and secure handling of training and inference data. They collaborate with data engineers to apply privacy-preserving techniques and implement deployment controls including endpoint hardening, API rate limiting, and access logging. The foundational principles of privacy and security in ML systems are examined in @sec-responsible-engineering, while advanced distributed privacy techniques are covered in Volume II.
...
[^fn-microservices-ml]: **Microservices in ML**: Architecture where models run as independent services with separate databases and deployment lifecycles. Netflix operates 700+ microservices including 100+ for ML recommendations; Uber's Michelangelo serves thousands of models independently. Enables independent scaling, A/B testing, and faster iteration—though adds complexity in orchestration and debugging.
...
Machine learning operations introduces unique complexities that distinguish it from traditional software deployment. Many teams underestimate these differences and attempt to apply conventional practices without adaptation. The probabilistic nature of ML systems, the central role of data quality, and the need for continuous model maintenance create operational challenges that require specialized approaches and tooling.
...
Many teams view model deployment as the final step in the ML lifecycle, similar to shipping software releases. This approach ignores the reality that ML models degrade over time. Data drift, changing user behavior, and evolving business requirements all contribute to degradation. Production models require continuous monitoring, performance evaluation, and potential retraining or replacement. Without ongoing operational support, deployed models become unreliable and may produce increasingly poor results. Successful MLOps treats deployment as the beginning of a model's operational lifecycle rather than its conclusion.
