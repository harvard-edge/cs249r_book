...
For serialized phases, the performance decomposition introduced in the opening chapter is:
...
| **$\text{BW}$**        | **Bandwidth**   | bytes/s  | **Avoids collision with $B$ (Batch Size)**. Systems literature often uses $B$ for bandwidth, while ML literature often uses $B$ for batch size. We preserve the ML convention.              |
| **$O$**                | **Operations**  | FLOPs    | Total floating-point operations. Clean in equations (vs. "$\text{Ops}$").                                                                                                                   |
| **$R_{\text{peak}}$**  | **Peak Rate**   | FLOP/s   | **Avoids collision with $P$ (Parameters)**. Roofline presentations often use $P$ for performance, while ML literature commonly uses $P$ for parameter count. We preserve the ML convention. |
...
*   Compute: We distinguish operation counts from rates. Total work uses FLOPs (for example, GFLOPs, TFLOPs), while throughput uses FLOP/s with decimal prefixes (for example, GFLOP/s, TFLOP/s).
    ...
    *   Arithmetic intensity conventionally uses FLOP/byte as a unit ratio (floating-point \mbox{operations} per byte moved). This is a ratio unit, not a total-work symbol or throughput symbol.
