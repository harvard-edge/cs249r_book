...
- $\text{BW}_{\text{disk}}$, $\text{BW}_{\text{network}}$, $\text{BW}_{\text{accelerator}}$, not $BW_{\text{disk}}$
- $T_{\text{compute}}$, $T_{\text{comm}}$, $T_{\text{step}}$, not $T_\text{compute}$ (missing braces) and not $T_{compute}$ (no `\text{}`)
- $E_{\text{move}}$, $E_{\text{compute}}$, $E_{\text{total}}$, same rule
...
$$\text{Accuracy}(t) \approx \text{Accuracy}_0 - \lambda \cdot D(P_t \lVert P_0)$$
