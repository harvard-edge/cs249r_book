...
In @sec-hardware-acceleration, the layer intensity concentrates at the bottom of the stack while the data bar is faint---this chapter is about silicon, not data. In @sec-model-training, the middle layers glow and the data bar is moderately lit, reflecting that training consumes data but is fundamentally about optimization. In @sec-responsible-engineering, the top layers dominate and the data bar carries a moderate tint, because biased training data surfaces as biased applications even though several layers sit between them. The same stack, three different stories, with data as an independent dimension.
