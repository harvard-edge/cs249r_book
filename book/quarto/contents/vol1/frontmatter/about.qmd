...
Throughout the book, a margin figure highlights which layers each chapter addresses, so you always know where you are in this stack. The data bar's shading reflects how data flows between the active layers: where a layer is strongly relevant, its connection into the data bus is pronounced, and the gradient flows between connection points---making visible the "skip connections" that data creates between distant layers. In a chapter on responsible engineering, for example, data links applications directly to training even though several layers sit between them. This is a deliberate departure from traditional systems stacks, where only adjacent layers interact.
