...
**The Principle**: The latency ($L$) of any machine learning operation is determined by the sum of three components, divided by the system's efficiency ($\eta$):
...
::: {.callout-definition title="The Silicon Contract"}
...
::: {.callout-definition title="The Depth-Efficiency Law"}
**The Principle**: In hierarchical feature extractors, computational efficiency is maximized when early layers process high-resolution, low-semantic data (edges, textures) and later layers process low-resolution, high-semantic data (objects, concepts).
...
::: {.callout-definition title="The Residual Gradient Invariant"}
**The Principle**: Deep network trainability is preserved only if there exists a direct path for gradient flow that bypasses non-linear transformations.
...
## Part II Roadmap: From Math to Infrastructure {#sec-principles-model-building-part-ii-roadmap-math-infrastructure-78fe}

This section translates these principles into the components of the ML stack:

1.  **The Primer (@sec-deep-learning-systems-foundations)**: The mathematical foundations of the gradient flow.
