...
where $D_{\text{vol}}$ is data volume (bytes moved), $\text{BW}$ is memory bandwidth, $O$ is total floating-point operations, $R_{\text{peak}}$ is peak compute rate, $\eta_{\text{hw}}$ is hardware utilization efficiency, and $L_{\text{lat}}$ is fixed latency overhead such as kernel launch or network round-trip time. The notation rationale appears in @sec-notation-conventions-iron-law-ml-systems-ce9d.
