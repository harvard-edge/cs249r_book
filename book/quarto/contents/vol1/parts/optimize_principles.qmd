...
**Invariant**: Attainable throughput ($R$) is no greater than the minimum of peak compute ($R_{\text{peak}}$) and memory bandwidth ($\text{BW}$) times arithmetic intensity ($I$) [@williams2009]:
...
**Invariant**: Total workload energy depends jointly on per-event energy and event counts. In the cited reference, moving a 32-bit value from DRAM can cost roughly 100–1,000$\times$ more energy per event than a 32-bit arithmetic operation, depending on operation type and process technology [@horowitz2014].
