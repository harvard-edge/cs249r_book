...
The following table is the most important diagnostic tool in this book. Memorize it:
...
| **Latency-Bound** | $L_{lat}$ | Batching, kernel fusion, async dispatch | Everything else |
...
Part III puts the AI Triad to work. @sec-introduction established that every ML system comprises **Data**, **Algorithm**, and **Machine**. Here, we refine this taxonomy into a rigorous engineering framework, mapping the components to their roles in the **Iron Law of Efficiency**.
...
$$ \text{Time} = \underbrace{ \frac{\text{Data} (D_{vol})}{\text{Bandwidth} (BW)} }_{\text{Reduce } D_{vol}} + \underbrace{ \frac{\text{Ops} (O)}{\text{Peak} (R_{peak}) \times \text{Efficiency} (\eta)} }_{\text{Reduce O, Maximize } \eta} + L_{lat} $$
