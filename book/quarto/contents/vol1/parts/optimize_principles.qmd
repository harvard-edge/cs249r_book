...
**The Principle**: System throughput ($P$) is bounded by the minimum of peak compute ($P_{peak}$) and memory bandwidth ($B_{mem}$) relative to the workload's arithmetic intensity ($I$):
...
**The Principle**: Moving 1 bit of data from DRAM costs 100–500$	imes$ more energy than performing an arithmetic operation on it.
...
**The Principle**: The maximum speedup of a system is limited by the fraction of the workload that cannot be accelerated.
...
## Part III Roadmap: The Optimization Pipeline {#sec-principles-optimization-part-iii-roadmap-optimization-pipeline-be0d}
