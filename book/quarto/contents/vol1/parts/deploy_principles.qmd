...
**Invariant**: Over a measured deployment range, accuracy declines as the production distribution diverges from the baseline, an effect labeled outcomes can fit as:
...
**Invariant**: For scalar model scores, training-serving divergence is the expected absolute difference between the training and serving functions over a stated comparison-input distribution:
...
**Invariant**: Under serial request-path accounting, end-to-end latency is the sum of network, preprocessing, inference, postprocessing, and queueing delays. In latency-sensitive serving, the product selects a deadline $D$ and an acceptable completion quantile $q$, such as 0.95, 0.99, or 0.999; throughput is optimized within that constraint:
