...
# │              Infrastructure.Pricing.Cloud.TpuV4PerHour.rate, Gbps, GB, dollar, hour, second, MILLION,
...
# │          DataGravity.tpu_hours_str, DataGravity.network_gb_per_s_str,
# │          DataGravity.dataset_gb_str, DataGravity.network_gbit_per_s_str,
...
class DataGravity:
    ...
    transfer_days_10g_str = fmt_time(transfer_days_10g, "day", precision=1, commas=False, style="word")
    ...
    tpu_cost_per_hour_str = fmt_usd(tpu_rate, precision=0, commas=False, per="hour")
```
...
*Selection gain is highest where entropy is high and data gravity low.*
:::
...
# │ Exports: FeedingProblem.req_bw_gb_per_s_str, FeedingProblem.disk_bw_mb_per_s_str,
# │          FeedingProblem.feeding_tax_pct_str, FeedingProblem.img_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
According to the iron law, the system is only as fast as its slowest term. If a modern accelerator running ResNet-50 can process `{python} FeedingProblem.img_per_s_str`, but the storage pipeline delivers only `{python} FeedingProblem.disk_bw_mb_per_s_str`, the expensive silicon sits idle. We quantify this as **The Feeding Tax**\index{Feeding Tax!I/O wait cost}: the wall-clock time lost to I/O wait, which directly reduces the system efficiency $(\eta_{\text{hw}})$\index{System Efficiency!feeding tax} term. For a standard cloud volume, the feeding tax can exceed `{python} FeedingProblem.feeding_tax_pct_str`, meaning the GPU spends the majority of its time waiting for bits. This tax transforms the Data Pipeline from a simple storage concern into the primary regulator of the system's Duty Cycle. Matching the accelerator's consumption rate often requires `{python} FeedingProblem.req_bw_gb_per_s_str` transfer rates, forcing the shift from traditional file systems to the specialized storage architectures we examine in @sec-data-engineering-ml-storage-systems-architecture-options-67fa.
...
# │
# │ Imports: mlsysim Hardware.Tech.Memory.DRAM.energy_per_access, Hardware.Tech.Op.FlopFp32.energy;
# │          mlsysim.fmt (fmt, fmt_int, check)
# │ Exports: EnergyAccessCosts.energy_flop_fp32_str, EnergyAccessCosts.energy_dram_str,
# │          EnergyAccessCosts.dram_mult_str, EnergyAccessCosts.local_ssd_mult_str,
# │          EnergyAccessCosts.network_mult_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, fmt_qty, check, fmt_percent, fmt_multiple
...
class EnergyAccessCosts:
    ...
    energy_flop_fp32_pj = energy_flop_fp32.to(pJ).magnitude
    ...
    energy_flop_fp32_str = fmt_qty(energy_flop_fp32, pJ, precision=1, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_rate, fmt_sci, fmt_time, check, MarkdownStr
...
| **Local vs. cloud inference**       | −2 percent accuracy risk  |      10 ms vs. 100 ms     |   \$0 vs. \$0.001/query   |   16 KB vs. unlimited    |
...
# │          BudgetAllocation.labeling_k_usd_str, BudgetAllocation.storage_k_usd_str,
# │          BudgetAllocation.governance_k_usd_str, BudgetAllocation.effective_cost_str,
...
| **Inference**         | Local, compact format    | Memory-constrained                                        |
...
# │
# │ Imports: mlsysim.core.constants (LABELING_COST_*, Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate, dollar),
# │          mlsysim.fmt (fmt_usd, fmt_usd_range, check)
# │ Exports: DataEngineeringCostConstants.labeling_cost_*_range_str,
...
| **Train ResNet-50 on ImageNet**    |  4–6 hours   | Compute (eight accelerators, optimized) |
...
# │
# │ Imports: mlsysim.core.constants (Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate, THOUSAND, dollar,
...
# │          LabelingComputeRatio.ratio_low_mult_str,
# │          LabelingComputeRatio.ratio_high_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
The contrast matters: weeks for human labeling, hours for GPU training, milliseconds for serving. Labeling is the bottleneck. It often costs hundreds to more than a thousand times more than a single optimized training run: a `{python} LabelingComputeRatio.labeling_budget_str` labeling budget compares with `{python} LabelingComputeRatio.train_cost_low_str`--`{python} LabelingComputeRatio.train_cost_high_str` for one eight-accelerator ResNet-50 run, a `{python} LabelingComputeRatio.ratio_low_mult_str`--`{python} LabelingComputeRatio.ratio_high_mult_str` ratio. And within that labeling spend, the effort distribution itself is skewed: `{python} LabelingComputeRatio.effort_majority_pct_str` of the work goes to `{python} LabelingComputeRatio.effort_minority_pct_str` of features—the long tail of edge cases, rare categories, and quality exceptions.
...
# │
# │ Imports: mlsysim.core.constants (Infrastructure.Pricing.Storage.GlacierRetrievalPerGB.rate, dollar), mlsysim.fmt (fmt, check)
# │ Exports: GlacierRetrievalPricing.retrieval_cost_glacier_str,
# │          GlacierRetrievalPricing.retrieval_cost_glacier_expedited_str,
# │          GlacierRetrievalPricing.retrieval_ratio_expedited_standard_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
\index{Data Acquisition!strategies}Data acquisition begins when the team names the coverage gap the model must close. ImageNet's 14.2 million labeled images took 49,000 crowdsourced workers to assemble. GPT-3's training corpus used tens of terabytes of raw Common Crawl text filtered to hundreds of gigabytes, combined with curated web, books, and Wikipedia data [@brown2020language]. Our KWS system needs 23 million audio samples spanning fifty languages, but the hard part is not only volume. The model must recognize wake words across accents, microphones, rooms, ages, and background noises that no single collection method can economically cover. Acquisition strategy is therefore a sequence of gap-closing decisions: reuse what already matches deployment, collect what is missing, scrape or synthesize where scale is the binding constraint, and reject sources whose provenance or consent constraints make them unusable.
...
**System**: The data pipeline generates synthetic audio variants with pitch shifting, additive noise, and room impulse simulation, then evaluates how different synthetic-to-real ratios affect KWS model accuracy.
...
# │
# │ Imports: math (sqrt), mlsysim.Ops.Monitoring.KsTestCoefficient,
# │          mlsysim.book (fmt)
...
**Result**: With sample sizes $n_0 = n_t = 1000$, the critical value is approximately `{python} KSTest.ks_dcrit_str`:
...
# │ Exports: RealtimeCost.throughput_gb_per_s_str, RealtimeCost.hourly_tb_str,
# │          RealtimeCost.stream_cost_str, RealtimeCost.batch_cost_str,
# │          RealtimeCost.cost_ratio_mult_str, RealtimeCost.events_per_sec_str,
...
class RealtimeCost:
    ...
    batch_window_hr_str = fmt_time(
        1, "hour", precision=0, commas=False, style="word", attributive=True
    )
...
\index{ETL!vs. ELT}\index{ELT!vs. ETL}After timing, the next ingestion decision is where transformation authority lives. Extract, Transform, Load (ETL)[^fn-etl-quality-first] cleanses and structures data before it enters storage, ensuring only validated data reaches the warehouse; Extract, Load, Transform (ELT)[^fn-elt-flexibility-first] loads raw data first and applies transformations within the target system, preserving flexibility at the cost of storing uncleaned data. The side-by-side flows in @fig-etl-vs-elt make the ordering difference visible by placing the "Transform" step before or after "Load." That ordering determines where computational resources are consumed, how quickly data becomes available for analysis, and how easily transformation logic can evolve as requirements change.
...
# │
# │ Imports: mlsysim.book (fmt, check)
# │ Exports: EtlEltCost.etl_spark_usd_str, EtlEltCost.etl_storage_usd_str,
# │          EtlEltCost.elt_storage_tb_str, EtlEltCost.elt_storage_usd_str,
# │          EtlEltCost.elt_query_usd_str, EtlEltCost.savings_str,
# │          EtlEltCost.daily_raw_tb_str, EtlEltCost.spark_per_tb_usd_str,
# │          EtlEltCost.etl_datasets_str, EtlEltCost.etl_tb_each_str,
# │          EtlEltCost.etl_total_storage_tb_str, EtlEltCost.s3_per_tb_mo_usd_str,
# │          EtlEltCost.retention_days_str, EtlEltCost.query_cost_per_str,
# │          EtlEltCost.n_models_str, EtlEltCost.schema_change_ratio_mult_str,
# │          EtlEltCost.etl_spark_monthly_usd_str, EtlEltCost.etl_total_usd_str,
# │          EtlEltCost.elt_total_usd_str, EtlEltCost.total_savings_str
# └─────────────────────────────────────────────────────────────────────────────
...
class KWSScalabilityTargets:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_m_round_str = fmt_int(round(dataset_size_m))
    memory_limit_edge_kb_str = fmt_qty(memory_limit_edge, KB, precision=0, commas=False)
    model_size_limit_kb_str = fmt_qty(model_size_limit, KB, precision=0, commas=False)
```
...
# │          SegmentationLabelScale.label_entry_ratio_mult_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class KWSAutomatedLabelingScale:
    ...
    dataset_size_m_round_str = fmt_int(round(dataset_size_m))
    label_hours_str = fmt_time(label_hours, "hour", precision=0, commas=True, style="word")
...
# │
# │ Imports: Infrastructure.Pricing.Storage.*, mlsysim.core.constants (GB, TB, dollar, ureg, byte, MILLION),
# │          mlsysim.book (fmt, check)
...
# │          KWSStorageEconomics.load_speedup_mult_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Systems
from mlsysim.fmt import fmt, fmt_usd, fmt_qty, check, fmt_multiple
from mlsysim.core.units import GB, TB, byte, USD, ureg
...
class KWSStorageEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_gb_str = fmt_qty(dataset_size, GB, precision=0, commas=False)
...
# │
# │ Imports: mlsysim.book (fmt, check)
# │ Exports: StorageLoading.nvme_load_s_str, StorageLoading.obj_load_s_str,
# │          StorageLoading.load_speedup_mult_str, StorageLoading.nvme_bw_gb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Systems
from mlsysim.fmt import fmt, fmt_qty, check, fmt_time, fmt_multiple
from mlsysim.core.units import GB, byte, second
...
class StorageLoading:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvme_load_s_str = fmt_time(nvme_load, 'second', precision=1, commas=False)
    obj_load_s_str = fmt_time(obj_load, 'second', precision=0, commas=True)
    load_speedup_mult_str = fmt_multiple(load_speedup, precision=0, commas=False)
    nvme_bw_gb_per_s_str = fmt_qty(nvme_bw, GB/second, precision=0, commas=False)
```
...
# │          mlsysim.book (fmt, check)
# │ Exports: ModelStorageScale.gpt3_fp32_gb_str,
# │          ModelStorageScale.gpt3_params_b_str,
# │          ModelStorageScale.gpt3_fp16_gb_str,
# │          ModelStorageScale.alexnet_params_m_str,
# │          ModelStorageScale.model_growth_factor_mult_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.fmt import fmt, fmt_multiple, fmt_params, fmt_qty, check, MarkdownStr
from mlsysim.core.units import Bparam, Mparam, GB
...
# │ Exports: StorageBandwidth.max_img_per_s_str,
# │          StorageBandwidth.required_bw_gb_per_s_str,
# │          StorageBandwidth.a100_fp16_tflop_per_s_str,
# │          StorageBandwidth.resnet_fwdbwd_gflop_str,
# │          StorageBandwidth.image_size_kb_str,
# │          StorageBandwidth.sata_bw_mb_per_s_str,
# │          StorageBandwidth.sata_max_img_per_s_str,
# │          StorageBandwidth.sata_utilization_pct_str,
# │          StorageBandwidth.s3_bw_mb_per_s_str,
# │          StorageBandwidth.s3_workers_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Models, Systems
from mlsysim.fmt import fmt_int, fmt, fmt_count, check, fmt_qty, fmt_qty_int, fmt_rate, fmt_percent, fmt_flops
...
class StorageBandwidth:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(required_bw > 3.0 * GB / second, f"A100 storage budget should be multiple GB/s, got {required_bw_value:.1f}.")
    ...
    required_bw_gb_per_s_str = fmt_qty(required_bw, GB/second, precision=1, commas=False)
    ...
    sata_bw_mb_per_s_str = fmt_qty(sata_bw, MB/second, precision=0, commas=False)
    ...
    s3_bw_mb_per_s_str = fmt_qty(s3_bw, MB/second, precision=0, commas=False)
    s3_workers_str = fmt_count(s3_workers, commas=False, label="concurrent worker thread")
    nvme_bw_low_gb_per_s_str = fmt_qty(nvme_bw_low, GB/second, precision=0, commas=False)
    nvme_bw_high_gb_per_s_str = fmt_qty(nvme_bw_high, GB/second, precision=0, commas=False)
```
...
**Problem**: What storage configuration keeps a high-throughput accelerator from being data-starved while training ResNet-50?

*The compute term (throughput ceiling)*

*   **Hardware peak**: reference accelerator = `{python} StorageBandwidth.a100_fp16_tflop_per_s_str` (dense low-precision operations).
*   **Model cost**: ResNet-50 requires approximately four GFLOPs for the forward pass per image; including the backward pass, total is approximately `{python} StorageBandwidth.resnet_fwdbwd_gflop_str` per training step.
*   **Maximum theoretical throughput**: `{python} StorageBandwidth.max_img_per_s_str` (accelerator peak throughput divided by ResNet-50 forward-plus-backward FLOPs per image).

*The data term (bandwidth requirement)*

*   **Image size**: `{python} StorageBandwidth.image_size_kb_str` (JPEG compressed).
*   **Required bandwidth**:
    `{python} StorageBandwidth.max_img_per_s_str` $\times$ `{python} StorageBandwidth.image_size_kb_str` per image ≈ `{python} StorageBandwidth.required_bw_gb_per_s_str`

*Storage options:*
To saturate the reference accelerator, storage must deliver `{python} StorageBandwidth.required_bw_gb_per_s_str`.

*   **S3 Standard**: ~`{python} StorageBandwidth.s3_bw_mb_per_s_str` per thread. The pipeline needs `{python} StorageBandwidth.s3_workers_str`.
*   **SATA SSD**: ~`{python} StorageBandwidth.sata_bw_mb_per_s_str` sequential. Totally insufficient (bottleneck).
*   **NVMe SSD**\index{NVMe SSD!data pipeline}: approximately `{python} StorageBandwidth.nvme_bw_low_gb_per_s_str`–`{python} StorageBandwidth.nvme_bw_high_gb_per_s_str`. Required.
...
The `{python} StorageBandwidth.sata_bw_mb_per_s_str` figure represents SATA III sequential read throughput (the interface maximum is 600 MB/s). Real-world random read performance with small files can be significantly lower.
...
# │          FormatEfficiency.throughput_ratio_mult_str,
...
**Systems insight**: Switching from CSV to Parquet is not just a file change; it is mathematically equivalent to buying a `{python} FormatEfficiency.throughput_ratio_mult_str` faster hard drive. @Tbl-serialization-cost quantifies the serialization overhead of different formats. For a deeper treatment of row vs. columnar storage layouts and the algebra of data operations (selection, projection, join), see @sec-appdx-data-foundations-row-vs-columnar-formats-bca2.
...
# │ Exports: CompressionTradeoff.decompress_speedup_mult_str,
# │          CompressionTradeoff.gzip_min_str,
# │          CompressionTradeoff.snappy_min_str,
# │          CompressionTradeoff.time_diff_min_str,
# │          CompressionTradeoff.total_diff_hours_str,
# │          CompressionTradeoff.snappy_decompress_mb_per_s_str,
# │          CompressionTradeoff.gzip_decompress_mb_per_s_str,
...
\index{ML Lifecycle!storage requirements}Storage requirements evolve because each lifecycle stage asks the same data to serve a different access pattern. The same dataset is accessed through random sampling during exploratory analysis, sequential scanning during model training, and random access during production serving. These diverse patterns require storage architectures that accommodate all three access modes.
...
# │ Exports: LifecycleStorageRequirements.train_img_per_s_str,
# │          LifecycleStorageRequirements.image_size_kb_str,
# │          LifecycleStorageRequirements.training_bw_gb_per_s_str,
# │          LifecycleStorageRequirements.storage_reduction_mult_str,
...
Data versioning\index{Data Versioning!model reproducibility} is the storage analogue of source-control provenance. It connects model versions to exact training data, enabling debugging and reproducibility. Without it, teams cannot identify the exact data that trained model v47.
...
Teams meticulously optimize accelerator kernels while leaving data loading as JSON or CSV. Text-based formats can be at least 10$\times$ slower to decode than columnar binary formats (Parquet, Arrow), and the CPU, not the accelerator, becomes the bottleneck. The consequence is the worst-case profile of the data-throughput hierarchy: an expensive accelerator that should sustain high throughput instead idles at single-digit utilization waiting for the next batch. The fix is structural, not algorithmic: convert the training corpus to Parquet once, version it, and let every downstream job read the columnar form. A one-time conversion pays back within hours of training time.
...
# │          dollar, hour), mlsysim.book (fmt, check)
# │ Exports: DataEngineeringSummaryRecap.ratio_low_mult_str,
# │          DataEngineeringSummaryRecap.ratio_high_mult_str,
...
from mlsysim.fmt import fmt_int, fmt, fmt_qty, check, fmt_multiple
from mlsysim.core.units import GB, byte, USD, hour
...
class DataEngineeringSummaryRecap:
    ...
    kws_dataset_size_gb_str = fmt_qty(kws_dataset_size, GB, precision=0, commas=False)
```
...
# │ Exports: StorageLoadingRecap.load_speedup_mult_str, StorageLoadingRecap.nvme_bw_gb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Systems
from mlsysim.fmt import fmt, fmt_qty, check, fmt_multiple
from mlsysim.core.units import GB, byte, second
