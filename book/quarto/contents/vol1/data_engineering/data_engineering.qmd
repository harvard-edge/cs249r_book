...
dedup_train_weeks_value = 2
dedup_train_days_value = dedup_train_weeks_value * 7
dedup_train_cost_value = 5000
dedup_duplicate_pct_value = 0.30
dedup_runs_value = 10
dedup_eng_days_value = 2
dedup_eng_cost_value = 2000
...
dedup_new_days_str = fmt(dedup_new_days_value, precision=0, commas=False)
dedup_savings_per_run_str = fmt(dedup_savings_per_run_value, precision=0, commas=True)
dedup_total_savings_str = fmt(dedup_total_savings_value, precision=0, commas=True)
dedup_roi_str = fmt(dedup_roi_value, precision=1, commas=False)
```
...
Teams compare raw storage costs (S3 Standard at USD `{python} s3_per_tb_mo_str` per TB per month vs Glacier Deep Archive at USD 1 per TB per month) and select the cheapest option without analyzing access patterns. However, @sec-data-engineering-ml-ml-storage-systems-architecture-options-67fa establishes that total cost of ownership includes retrieval costs, access latency, and throughput limitations. Glacier Deep Archive charges USD 0.02 per GB for retrieval plus 12-hour access latency, meaning a single full retrieval of 100 TB costs USD `{python} retrieval_str`, equivalent to `{python} equiv_months_str` months of S3 Standard storage for that data. Training workloads requiring random access to 10% of a 100 TB dataset daily would spend USD `{python} daily_retrieval_str` per day on Glacier retrievals versus USD `{python} s3_monthly_str` per month for S3 Standard storage with 3,500 MB/s sustained read throughput. Local NVMe storage at USD 0.10 per GB (USD `{python} nvme_per_tb_str` per TB) appears expensive but delivers 7,000 MB/s sequential reads and sub-millisecond random access latency, completing training epochs 15 to 30x faster than S3 for I/O-bound workloads. Organizations must match storage tiers to access patterns: cold archival data to Glacier, training datasets requiring sequential scans to S3, and active training data with random access patterns to local SSDs or high-performance networked storage.
