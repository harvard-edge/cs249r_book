...
# │ Exports: FeedingProblem.req_bw_gb_per_s_str,
# │          FeedingProblem.disk_bw_mb_per_s_str,
# │          FeedingProblem.feeding_tax_pct_str,
# │          FeedingProblem.img_per_s_str,
# │          FeedingProblem.accelerator_peak_tflop_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, check, fmt_qty, fmt_rate, fmt_percent, fmt_flop_rate
...
class FeedingProblem:
    ...
    accelerator_peak_tflop_per_s_str = fmt_flop_rate(h.compute.peak_flops, unit=TFLOPs / second, precision=0, commas=False)
```
...
According to the iron law, the system is only as fast as its slowest term. If an A100 running ResNet-50 can process `{python} FeedingProblem.img_per_s_str`, but the storage pipeline delivers only `{python} FeedingProblem.disk_bw_mb_per_s_str`, the expensive silicon sits idle. We quantify this as **The Feeding Tax**\index{Feeding Tax!I/O wait cost}: the wall-clock time lost to I/O wait, which directly reduces the system efficiency $(\eta_{\text{hw}})$\index{System Efficiency!feeding tax} term. For a standard cloud volume, the feeding tax can exceed `{python} FeedingProblem.feeding_tax_pct_str`, meaning the GPU spends the majority of its time waiting for bits. This tax transforms the Data Pipeline from a simple storage concern into the primary regulator of the system's Duty Cycle. Feeding the `{python} FeedingProblem.accelerator_peak_tflop_per_s_str` accelerator in this example often requires `{python} FeedingProblem.req_bw_gb_per_s_str` transfer rates, forcing the shift from traditional file systems to the specialized storage architectures we examine in @sec-data-engineering-ml-storage-systems-architecture-options-67fa.
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: Data serialization pitfall hardware anchor.
# │ Exports: DataSerializationPitfall.h100_peak_tflop_per_s_str
from mlsysim.fmt import fmt_flop_rate

class DataSerializationPitfall:
    """Concrete accelerator peak for the serialization pitfall."""

    # ┌── 1. LOAD ──────────────────────────────────────────
    h100 = Hardware.Cloud.H100
    # ┌── 4. OUTPUT ────────────────────────────────────────
    h100_peak_tflop_per_s_str = fmt_flop_rate(
        h100.compute.peak_flops,
        unit=TFLOPs / second,
        precision=0,
        commas=False,
    )
...
Teams meticulously optimize accelerator kernels while leaving data loading as JSON or CSV. Text-based formats can be at least 10$\times$ slower to decode than columnar binary formats (Parquet, Arrow), and the CPU, not the accelerator, becomes the bottleneck. The consequence is the worst-case profile of the data-throughput hierarchy: an H100 that should sustain `{python} DataSerializationPitfall.h100_peak_tflop_per_s_str` on dense low-precision kernels instead idles at single-digit utilization waiting for the next batch. The fix is structural, not algorithmic: convert the training corpus to Parquet once, version it, and let every downstream job read the columnar form. A one-time conversion pays back within hours of training time.
