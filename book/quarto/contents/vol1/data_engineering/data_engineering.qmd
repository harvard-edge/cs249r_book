...
PSI's primary advantage is its invariance to feature scale and its proven track record in financial risk modeling where it originated. The binning approach makes it robust to outliers while remaining computationally efficient for real-time monitoring.
...
KL divergence thresholds depend more on application context than PSI, but typical production systems alert when $D_{\text{KL}} > 0.5$ for critical features and investigate when $D_{\text{KL}} > 1.0$ for any feature. The metric's continuous nature makes it more sensitive to subtle shifts than PSI's binned approach, useful for detecting drift early but potentially producing more false positives.
...
**Automated retraining** triggers model updates when drift exceeds thresholds and retraining pipelines have proven robust. When $\text{PSI} \geq 0.2$ persists for 48 hours across multiple features, the system automatically initiates retraining on a recent data window (typically past 3-6 months) that better represents current distributions. This approach works well for covariate and label shift where relationships remain valid but distributions have changed.

However, automated retraining requires caution. Drift metrics may indicate data pipeline bugs rather than genuine distribution changes. A sudden spike in null values or out-of-range features likely reflects upstream system failures, not real-world changes. Retraining on corrupted data would degrade model quality. Production systems implement validation gates: automated retraining proceeds only if (1) data passes all quality checks, (2) newly trained model outperforms current model on hold-out validation set, and (3) distribution shift persists for sufficient time to rule out transient anomalies.
