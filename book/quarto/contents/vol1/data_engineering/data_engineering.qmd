...
2.  **Stream requirements**: To sustain `{python} RealtimeCost.throughput_str` GB/s with < 100 ms latency, the system needs ~50 dedicated cores + redundant backups (always on). Cost: `{python} RealtimeCost.stream_cores_str` cores $\times$ `{python} RealtimeCost.stream_hours_str` hrs $\times$ USD `{python} RealtimeCost.stream_cost_per_hr_str`/hr = **USD `{python} RealtimeCost.stream_cost_str`/day**.
...
**Engineering conclusion**: ETL saves USD `{python} EtlEltCost.savings_str`/month in storage but costs 8$\times$ more engineering time per schema change. If feature definitions change weekly, ELT's flexibility pays for itself. If schemas are stable, ETL's lower storage cost dominates. The break-even point occurs when schemas change fewer than once per month; below that rate, ETL wins on total cost.
...
**Engineering conclusion**: Algorithm choice is a 10$\times$ lever on data budget. Spending ten percent of the budget on compute to select data can save 90 percent of the budget on human labeling.
