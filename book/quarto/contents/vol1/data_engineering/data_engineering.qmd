...
class DataPrepEffortIntro:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    data_prep_effort_low_str = fmt_percent(data_prep_effort_low / 100, precision=0, commas=False, style='prose')
    data_prep_effort_high_str = fmt_percent(data_prep_effort_high / 100, precision=0, commas=False, style='prose')
```

## Dataset Compilation {#sec-data-engineering-data-engineering-dataset-compilation-0496}

The workflow becomes concrete in the data pipeline: raw inputs pass through collection, ingestion, analysis, labeling, validation, and preparation before they become ML-ready datasets. The effort breakdown in @fig-ds-time explains why that pipeline needs its own systems treatment: industry surveys have reported that data work consumes `{python} DataPrepEffortIntro.data_prep_effort_low_str` to `{python} DataPrepEffortIntro.data_prep_effort_high_str` of ML project effort [@crowdflower2016data], and the data axis of the D·A·M taxonomy becomes real only as infrastructure: acquisition systems, validation checks, labeling workflows, storage layouts, and governance.
...
# │
# │ Goal: Demonstrate the mismatch between pipeline demand and disk bandwidth.
# │ Show: That a standard cloud disk costs roughly a tenth of the duty cycle.
# │ How: Take the solver's overhead-limited image rate, multiply by the compressed
# │      on-disk image size, and compare the result against baseline disk supply.
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, check, fmt_qty, fmt_rate, fmt_percent
from mlsysim.core.units import GB, MB, KB, second, TFLOPs
...
class FeedingProblem:
    ...
    # Storage-path payload: JPEG-compressed images, matching the scenario used
    # in the storage-bandwidth budget. Decoding to tensors happens after the read,
    # so decoded FP32 size never crosses the storage interface.
    image_size_kb = 150
    image_size = image_size_kb * KB
    img_size_bytes = image_size.to(byte).magnitude
    ...
    p = SingleNodeModel().solve(m, h, batch_size=1, efficiency=1.0)
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    req_bw_value = req_bw.to(MB / second).magnitude
    check(req_bw > 200 * MB / second, f"Required bandwidth ({req_bw_value:.0f} MB/s) is lower than expected.")
    check(feeding_tax_pct > 5, f"Feeding tax ({feeding_tax_pct:.1f}%) should be material for standard disks.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    req_bw_mb_per_s_str = fmt_qty(req_bw, MB / second, precision=1, commas=False)
...
According to the iron law, the system is only as fast as its slowest term. Consider a small-batch image pipeline that sustains `{python} FeedingProblem.img_per_s_str`. That rate is not a statement about the accelerator's arithmetic peak; in this configuration the per-image fixed overhead dominates, so a much cheaper model or a much faster device would land within a few percent of the same number. What the rate does fix is the demand the storage path must satisfy. If that path delivers only `{python} FeedingProblem.disk_bw_mb_per_s_str`, the expensive silicon sits idle waiting on bits. We quantify this as **The Feeding Tax**\index{Feeding Tax!definition}: the wall-clock time lost to I/O wait, which directly reduces the system efficiency $(\eta_{\text{hw}})$\index{System Efficiency!feeding tax} term. For a standard cloud volume, the feeding tax here reaches `{python} FeedingProblem.feeding_tax_pct_str`, so nearly one accelerator-hour in ten is spent waiting rather than computing. This tax transforms the data pipeline from a simple storage concern into the primary regulator of the system's duty cycle. Feeding this pipeline requires a sustained `{python} FeedingProblem.req_bw_mb_per_s_str` even at `{python} FeedingProblem.image_size_kb_str` per compressed image, forcing the shift from traditional file systems to the specialized storage architectures we examine in @sec-data-engineering-ml-storage-systems-architecture-options-67fa. These physical properties also carry an energy cost: as data moves farther from the processor, movement increasingly dominates the budget.
...
The `{python} BudgetAllocation.total_budget_k_usd_str` budget splits across three cost categories:
...
[^fn-imagenet-data-scale]: **ImageNet**: The canonical "cost-efficient starting point" -- 14.2 million images labeled by 49,000 Mechanical Turk workers across 21,841 categories (2009) [@deng2009; @russakovsky2015; @imagenet_website]. Its value as a benchmark is inseparable from its data engineering: Fei-Fei Li's\index{Li, Fei-Fei!ImageNet creation} team spent two years building the labeling infrastructure, yet every subsequent team reuses that investment for free. The catch is benchmark overfitting: models tuned to ImageNet's distribution systematically underperform on related but shifted test distributions, making it a starting point that must be augmented, never a finishing line [@recht2019imagenet; @beyer2020we]. \index{ImageNet!data scale}
...
: **Data Engineering Cost Constants (2024)**: Per-unit costs that ML engineers should memorize in the same way systems engineers memorize latency numbers. Labeling commonly dominates storage and compute, so teams should reason from ratios rather than isolated prices. {#tbl-data-engineering-cost-constants tbl-colwidths="[39,27,34]"}
...
::: {#fig-synthetic-data fig-env="figure" fig-pos="htb" fig-cap="**Synthetic Data Augmentation**: Two parallel sources feed one training run. A simulation model produces synthetic data while a real system supplies historical data; the two are combined and passed to an ML algorithm, yielding a trained ML model. Source: AnyLogic [@anylogic_synthetic]." fig-alt="Diagram with two parallel branches. A simulation model produces synthetic data; a real system produces historical data. The two datasets combine and flow into an ML algorithm, which produces a trained ML model."}
...
[^fn-ks-test-drift]: **Kolmogorov-Smirnov (K-S) Test**: A nonparametric test measuring the maximum distance between two cumulative distribution functions, requiring no assumptions about underlying distributions [@berger2014kolmogorov]. In ML pipelines, the K-S test is a common univariate continuous-feature drift detector: comparing serving distributions against training baselines, with thresholds such as $p < 0.05$ used as operational investigation triggers rather than universal failure rules. Its distribution-free nature makes it useful across many continuous features, but it applies only to univariate continuous comparisons -- categorical drift requires metrics such as PSI or chi-squared tests instead [@yurdakul2020]. \index{K-S Test!drift detection}
...
[^fn-cohens-kappa-label-quality]: **Cohen's Kappa**: Introduced by @cohen1960 to measure inter-rater agreement while correcting for chance agreement, which raw percentage agreement ignores. The correction matters: two annotators labeling 90 percent of images as "not spam" in a binary task will agree `{python} CohensKappaChance.chance_agreement_pct_str` of the time by pure chance, making raw agreement a dangerously misleading quality metric. The statistic is denoted $\kappa$; for ML label quality monitoring, $\kappa$ below 0.4 signals unreliable training data whose noise the model will inherit [@landis1977measurement]. \index{Cohen's Kappa!label quality}
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_usd, fmt, fmt_count, fmt_rate, fmt_time, fmt_qty, check, fmt_multiple
...
1.  **Significance**: It predicts that performance degradation is proportional to the KL Divergence $(\mathcal{D}_{\text{KL}}(p_{g_{\text{serve}}} \lVert p_{g_{\text{train}}}))$ between feature distributions induced by the serving transformation $g_{\text{serve}}$ (current) and training transformation $g_{\text{train}}$ (baseline).
...
@Fig-labels contrasts five label granularities, and the choice depends on system requirements and resource constraints [@johnson-roberson2017]. Classification suffices for traffic counting, but autonomous vehicles need segmentation maps for precise navigation. Production systems often maintain hybrid annotations: a single camera frame might carry classification labels (scene type), bounding boxes (obstacle detection), and segmentation masks (path planning), with each label type serving distinct downstream models.

::: {#fig-labels fig-env="figure" fig-pos="htb" fig-cap="**Data Annotation Granularity**: Five label types across two input modalities. Four apply to the same image of a dog on a blanket (a classification label, a bounding box, a segmentation mask, and a free-text caption); the fifth transcribes an audio clip. Each type progressively increases labeling cost and storage requirements while providing a richer training signal." fig-alt="Five-row figure of label types. Four rows use an image of a dog on a blanket: classification (Dog, Blanket, No cat), a bounding box around the dog, a segmentation mask on the dog, and a free-text caption. The fifth row shows an audio icon with a transcription."}
...
# │ How: Compute KWS raw-audio size from the corpus count, then compare storage costs and bandwidth.
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
class KWSStorageEconomics:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(dataset_size_value - 748.8) < 0.05, f"KWS storage example should be 748.8 GB, got {dataset_size_value:.1f} GB.")
...
|:-----------------------------------------|:----------------------:|:----------------------------------:|:---------------------------:|:----------------------------------------------|
| **NVMe SSD (local)**                     |       \$100–300        |              3–7 GB/s              |          10–100 μs          | Training data loading, active feature serving |
...
# │ How: Calculate total load time for the computed KWS dataset size.
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
1.  **Significance**: It manifests as silent degradation, where the cost of maintenance scales superlinearly with system age due to unmanaged dependencies and distribution shifts $(\mathcal{D}(P_t \lVert P_0))$.
...
# │ How: Recompute load times for the KWS corpus at its full sample count.
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
* **The degradation equation becomes actionable through drift detection**: The divergence term $\mathcal{D}(P_t \lVert P_0)$ from the Introduction is exactly what statistical drift measures and KL divergence quantify. Data engineering operationalizes this theoretical equation into monitoring infrastructure that catches silent model degradation before users are affected.
