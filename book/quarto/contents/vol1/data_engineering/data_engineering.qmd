...
Pipeline architecture dictates when and where computational resources are applied to raw incoming records. Compare the two sequence flows in @fig-etl-vs-elt to see where the transformation stage executes relative to the primary data lakehouse storage boundary.[^fn-etl-quality-first][^fn-elt-flexibility-first]
...
[^fn-cap-theorem-ml-storage]: **CAP (Consistency, Availability, Partition Tolerance) Theorem**\index{CAP Theorem!feature store trade-off}: Conjectured by @brewer2000 and formally proved by @gilbert2002. During a network partition, linearizable consistency may require rejecting requests, whereas an available system may return stale or divergent values. CAP does not guarantee feature-transformation parity or point-in-time correctness.
