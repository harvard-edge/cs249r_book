...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_usd, fmt, fmt_count, check, fmt_math, fmt_qty, fmt_time
from mlsysim.core.units import GB, Gbps, second, hour, USD, ureg
...
class DataGravity:
    ...
    egress_cost_gb = egress_rate.to(USD / GB).magnitude
    tpu_hourly_cost = tpu_rate.to(USD / hour).magnitude
    ...
    transfer_days_10g = t_10g.to(ureg.day).magnitude
...
class FeedingProblem:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    req_bw_gbs = req_bw.to(GB / second).magnitude
...
class EnergyAccessCosts:
    ...
    energy_local_ssd = 10_000 * pJ
    energy_network = 50_000 * pJ
    prune_pct = 50
    ...
    energy_local_ssd_pj = energy_local_ssd.to(pJ).magnitude
    energy_network_pj = energy_network.to(pJ).magnitude
    ...
    local_ssd_energy_str = fmt_qty(energy_local_ssd * 32, pJ, precision=0, commas=True)
    network_energy_str = fmt_qty(energy_network * 32, pJ, precision=0, commas=True)
...
class KWSStorageEconomics:
    ...
    bytes_per_sample = 2
    storage_cost_s3 = Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate.to(USD / TB / ureg.month).magnitude
    storage_cost_nvme_low = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.to(USD / TB / ureg.month).magnitude
    storage_cost_nvme_high = Infrastructure.Pricing.Storage.NvmeHighPerTbMonth.rate.to(USD / TB / ureg.month).magnitude
    nvme_bw_gbs = 5
    obj_bw_gbs = 0.1
    ...
    dataset_size_gb = dataset_size.to(GB).magnitude
    dataset_size_tb = dataset_size.to(TB).magnitude
...
class StorageLoading:
    ...
    nvme_load_s = (kws_dataset / nvme_bw).to(second).magnitude
    obj_load_s = (kws_dataset / (obj_bw_gbs * GB / second)).to(second).magnitude
...
class ModelStorageScale:
    ...
    gpt3_fp32_gb = gpt3_fp32.to(GB).magnitude
    gpt3_fp16_gb = gpt3_fp16.to(GB).magnitude
...
class StorageBandwidth:
    ...
    required_bw_gbs = required_bw.to(GB / second).magnitude
    ...
    s3_workers = int(round(required_bw.to(MB / second).magnitude / s3_bw_mbs))
...
class LifecycleStorageRequirements:
    ...
    training_bw_gbs = training_bw.to(GB / second).magnitude
...
class DataEngineeringSummaryRecap:
    ...
    kws_dataset_size_gb = kws_dataset_size.to(GB).magnitude
    train_cost_low = a100_count * train_hours_low * gpu_hour_cost_low
    train_cost_high = a100_count * train_hours_high * gpu_hour_cost_high
    ratio_low = labeling_budget/train_cost_high
    ratio_high = labeling_budget/train_cost_low
