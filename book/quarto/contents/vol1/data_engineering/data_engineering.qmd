...
class StorageBandwidth:
    ...
    a100_fp16_tflop_per_s_str = fmt_qty_int(h_a100.compute.peak_flops, TFLOPs/second, commas=False)
...
*   **Hardware peak**: NVIDIA A100 = `{python} StorageBandwidth.a100_fp16_tflop_per_s_str` (dense FP16 operations).
...
*   **Maximum theoretical throughput**: `{python} StorageBandwidth.max_img_per_s_str` (A100 peak throughput divided by ResNet-50 forward-plus-backward FLOPs per image).
...
*Storage options:*
To saturate a single A100, storage must deliver `{python} StorageBandwidth.required_bw_gb_per_s_str`.
