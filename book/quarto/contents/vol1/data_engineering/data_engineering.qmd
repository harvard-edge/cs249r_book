...
**Systems insight**: In this projection-only scenario, switching from CSV to Parquet provides the same effective read-throughput gain as a `{python} FormatEfficiency.throughput_ratio_mult_str` faster data path. The data foundations appendix quantifies serialization overhead and @sec-appdx-data-foundations-row-vs-columnar-formats-bca2 treats row vs. columnar storage layouts and the algebra of data operations (selection, projection, join) in depth.
