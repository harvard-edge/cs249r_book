...
The methodologies examined in @sec-ai-workflow establish the foundations of machine learning development. Every phase of these workflows requires reliable data infrastructure. In traditional software, computational logic is defined by code. In machine learning, system behavior is defined by data. This shift makes data a first-class engineering artifact, requiring the same rigor we apply to code.

We reframe data engineering as **Dataset Compilation**. Just as a compiler transforms human-readable source code into an optimized binary executable, a data pipeline transforms raw, noisy observations into a clean, optimized training set (the binary) that the model (the processor) consumes.
