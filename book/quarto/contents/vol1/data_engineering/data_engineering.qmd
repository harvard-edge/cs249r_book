...
Deciding where to run these transformations involves calculating *the cost of transformation placement*.
...
1.  **ETL Approach**: Transform before loading. Compute all three aggregation windows in a Spark cluster before loading into the warehouse.
    ...
    *   Storage: 3 transformed datasets, ~2 TB each = 6 TB at $23/TB/month = $`{python} etl_storage_str`/month
    *   Schema change cost: Re-run full pipeline (~4 hours) per change

2.  **ELT Approach**: Load raw data first, transform in warehouse.
    *   Storage: 10 TB raw/day, 30-day retention = `{python} elt_storage_tb_str` TB at $23/TB/month = $`{python} elt_storage_str`/month
