...
class DataEngineeringSetup:
    ...
    # ┌── 1. PARAMETERS (Inputs) ───────────────────────────────────────────────
    # Energy
    energy_add_int32_pj = ENERGY_ADD_INT32_PJ.to(ureg.picojoule).magnitude
    energy_add_fp32_pj = ENERGY_ADD_FP32_PJ.to(ureg.picojoule).magnitude
    energy_network_1kb_pj = NETWORK_ENERGY_1KB_PJ.to(ureg.picojoule).magnitude
    alexnet_params_m = ALEXNET_PARAMS.to(Mparam).magnitude
    ...
    # Labeling Costs
    labeling_cost_crowd_low = LABELING_COST_CROWD_LOW.magnitude
    labeling_cost_crowd_high = LABELING_COST_CROWD_HIGH.magnitude
    labeling_cost_expert_low = LABELING_COST_EXPERT_LOW.magnitude
    labeling_cost_expert_high = LABELING_COST_EXPERT_HIGH.magnitude
    labeling_cost_box_low = LABELING_COST_BOX_LOW.magnitude
    labeling_cost_box_high = LABELING_COST_BOX_HIGH.magnitude
    labeling_cost_seg_low = LABELING_COST_SEG_LOW.magnitude
    labeling_cost_seg_high = LABELING_COST_SEG_HIGH.magnitude
    labeling_cost_medical_low = LABELING_COST_MEDICAL_LOW.magnitude
    labeling_cost_medical_high = LABELING_COST_MEDICAL_HIGH.magnitude

    # Storage Costs
    storage_cost_s3 = STORAGE_COST_S3_STD.to(USD / TB / ureg.month).magnitude
    storage_cost_glacier = STORAGE_COST_GLACIER.to(USD / TB / ureg.month).magnitude
    storage_cost_nvme_low = STORAGE_COST_NVME_LOW.to(USD / TB / ureg.month).magnitude
    storage_cost_nvme_high = STORAGE_COST_NVME_HIGH.to(USD / TB / ureg.month).magnitude
    retrieval_cost_glacier = RETRIEVAL_COST_GLACIER.to(USD / GB).magnitude
    ...
    a100_tflops_fp16_sparse = (A100_FLOPS_FP16_TENSOR * 2).to(TFLOPs/second).magnitude

    energy_dram_pj = ENERGY_DRAM_ACCESS_PJ.magnitude
    energy_flop_fp32_pj = ENERGY_FLOP_FP32_PJ.magnitude
    energy_flop_int8_pj = ENERGY_FLOP_INT8_PJ.magnitude
    ...
    # KWS Labeling
    kws_label_hours = kws_dataset_size_m * MILLION * kws_label_seconds_per_example / 3600
    kws_label_years = kws_label_hours / 2000

    # KWS Storage
    kws_dataset_size_tb = (kws_dataset_size_gb * GB).to(TB).magnitude
    ...
    kws_dataset_size_m_round_str = f"{kws_dataset_size_m_round}"
