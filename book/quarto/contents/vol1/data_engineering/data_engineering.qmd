...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_usd, fmt, fmt_count, check, fmt_math, fmt_qty
...
class DataGravity:
    ...
    egress_cost_gb = Infrastructure.Pricing.Cloud.EgressPerGB.rate.m_as("dollar / GB")
    tpu_hourly_cost = Infrastructure.Pricing.Cloud.TpuV4PerHour.rate.m_as("dollar / hour")
    ...
    network_gbps_str = fmt_qty(network_bw, Gbps, precision=0, commas=False, unit_label="Gb/s")
    egress_cost_per_gb_str = fmt_usd(egress_cost_gb, precision=2, commas=False, per="GB")
    tpu_cost_per_hour_str = fmt_usd(tpu_hourly_cost, precision=0, commas=False, per="hour")
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int,  fmt, check, fmt_qty, fmt_rate
...
class FeedingProblem:
    ...
    req_bw = req_bw_gbs * GB / second
    disk_bw = disk_bw_mbs * MB / second
...
class EnergyAccessCosts:
    ...
    energy_dram = energy_dram_pj * ureg.picojoule
    energy_flop_fp32 = energy_flop_fp32_pj * ureg.picojoule
    energy_local_ssd = energy_local_ssd_pj * ureg.picojoule
    energy_network = energy_network_pj * ureg.picojoule
    prune_pct = 50
    ...
    energy_dram_str = fmt_qty(energy_dram, ureg.picojoule, precision=0, commas=False)
    energy_flop_fp32_str = fmt_qty(energy_flop_fp32, ureg.picojoule, precision=1, commas=False)
    local_ssd_energy_str = fmt_qty(energy_local_ssd * 32, ureg.picojoule, precision=0, commas=True)
    network_energy_str = fmt_qty(energy_network * 32, ureg.picojoule, precision=0, commas=True)
    dram_multiplier_str = fmt_multiple(round(energy_dram_pj / energy_flop_fp32_pj), precision=0, commas=True)
    local_ssd_multiplier_str = fmt_multiple(energy_local_ssd_pj/energy_flop_fp32_pj_per_bit, precision=1, commas=True)
    network_multiplier_str = fmt_multiple(energy_network_pj/energy_flop_fp32_pj_per_bit, precision=1, commas=True)
```
...
class BudgetAllocation:
    ...
    labeling_k_str = fmt_usd(labeling_budget, precision=0, commas=False, scale="K")
    storage_k_str = fmt_usd(storage_budget, precision=1, commas=False, scale="K")
    governance_k_str = fmt_usd(governance_budget, precision=1, commas=False, scale="K")
    effective_cost_str = fmt_usd(effective_cost, precision=2, commas=False, per="label")
    labeled_k_str = fmt_count(labeled_examples, scale='K', precision=0, commas=False)
...
class DataEngineeringCostConstants:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    labeling_cost_crowd_low = Infrastructure.Pricing.Labeling.CrowdLow.rate.m_as("dollar")
    labeling_cost_crowd_high = Infrastructure.Pricing.Labeling.CrowdHigh.rate.m_as("dollar")
    labeling_cost_box_low = Infrastructure.Pricing.Labeling.BoxLow.rate.m_as("dollar")
    labeling_cost_box_high = Infrastructure.Pricing.Labeling.BoxHigh.rate.m_as("dollar")
    labeling_cost_medical_low = Infrastructure.Pricing.Labeling.MedicalLow.rate.m_as("dollar")
    labeling_cost_medical_high = Infrastructure.Pricing.Labeling.MedicalHigh.rate.m_as("dollar")
    storage_cost_s3 = Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate.m_as("dollar / TB / month")
    retrieval_cost_glacier = Infrastructure.Pricing.Storage.GlacierRetrievalPerGB.rate.m_as("dollar / GB")
...
class LabelingComputeRatio:
    ...
    train_hours_low = 4
    train_hours_high = 6
    gpu_hour_cost_low = Infrastructure.Pricing.Fleet.GpuHourRef.rate.m_as("dollar / hour")
    gpu_hour_cost_high = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as("dollar / hour")
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_usd, fmt, fmt_count, fmt_time, fmt_qty, fmt_qty_int, check
...
class RealtimeCost:
    ...
    throughput = throughput_gbs * GB / second
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    throughput_str = fmt_qty(throughput, GB/second, precision=0, commas=False)
    ...
    events_per_sec_str = fmt_count(events_per_sec, scale='M', precision=0, commas=False)
...
class EtlEltCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    etl_spark_str = fmt_usd(etl_spark_daily, precision=0, commas=False, per="day")
    etl_storage_str = fmt_usd(etl_storage_mo, precision=0, commas=False, per="month")
    elt_storage_tb_str = fmt_qty(elt_storage, TB, precision=0, commas=False)
    elt_storage_str = fmt_usd(elt_storage_mo, precision=0, per="month")
    elt_query_str = fmt_usd(elt_query_mo, precision=0, commas=False, per="month")
    savings_str = fmt_usd(storage_savings_mo, precision=0, per="month")
    etl_spark_monthly_str = fmt_usd(etl_spark_monthly, precision=0, per="month")
    etl_total_str = fmt_usd(etl_total_mo, precision=0, per="month")
    elt_total_str = fmt_usd(elt_total_mo, precision=0, per="month")
    total_savings_str = fmt_usd(total_savings_mo, precision=0, per="month")
    daily_raw_tb_str = fmt_qty(daily_raw_tb * TB, TB, precision=0, commas=False)
    spark_per_tb_str = fmt_usd(spark_per_tb, precision=0, commas=False, per="TB")
    etl_datasets_str = fmt(etl_datasets, precision=0, commas=False)
    etl_tb_each_str = fmt_qty(etl_tb_each * TB, TB, precision=0, commas=False)
    etl_total_storage_tb_str = fmt_qty(etl_storage, TB, precision=0, commas=False)
    s3_per_tb_mo_str = fmt_usd(s3_per_tb_mo, precision=0, commas=False, per="TB/month")
...
class KWSQualityThresholds:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    snr_threshold_db_str = fmt_qty(snr_threshold, ureg.decibel, precision=0, commas=False, unit_label="dB")
...
::: {.column-margin}
![](images/svg/vol1_data_engineering_margin_003.svg){width="100%" fig-alt="Margin illustration showing Gather-all normalization versus local aggregation time ladder."}
...
::: {.column-margin}
![](images/svg/vol1_data_engineering_margin_004.svg){width="100%" fig-alt="Margin illustration showing Segmentation mask scalar labels versus bounding-box coordinates."}
...
class ActiveLearningBudget:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_images_str = fmt_count(dataset_images, scale='M', precision=0, commas=False)
    ...
    random_labels_str = fmt_count(random_labels_needed, scale='M', precision=0, commas=False)
    random_label_cost_str = fmt_usd(random_label_cost, precision=0, commas=False, scale="K")
    active_labels_low_str = fmt_count(active_labels_low, scale='K', precision=0, commas=False)
    active_labels_high_str = fmt_count(active_labels_high, scale='K', precision=0, commas=False)
    ...
    active_total_low_str = fmt_usd(active_total_low, precision=0, commas=False, scale="K")
    active_total_high_str = fmt_usd(active_total_high, precision=0, commas=False, scale="K")
    ...
    candidate_pool_str = fmt_count(candidate_pool_images, scale='K', precision=0, commas=False)
    affordable_labels_str = fmt_count(affordable_labels_after_compute, scale='K', precision=0, commas=False)
...
class KWSStorageEconomics:
    ...
    bytes_per_sample = 2
    storage_cost_s3 = Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate.m_as("dollar / TB / month")
    storage_cost_nvme_low = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.m_as("dollar / TB / month")
    storage_cost_nvme_high = Infrastructure.Pricing.Storage.NvmeHighPerTbMonth.rate.m_as("dollar / TB / month")
    nvme_bw_gbs = 5
    obj_bw_gbs = 0.1
    ...
    dataset_size_gb = dataset_size.m_as(GB)
    dataset_size_tb = dataset_size.m_as(TB)
...
class StorageLoading:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvme_load_str = fmt_time(nvme_load_s, 'second', precision=1, commas=False)
    obj_load_str = fmt_time(obj_load_s, 'second', precision=0, commas=True)
    load_speedup_str = fmt_multiple(load_speedup, precision=0, commas=False)
    nvme_bw_gbs_str = fmt_qty(nvme_bw, GB/second, precision=0, commas=False)
```

The performance difference directly impacts iteration velocity. Training that loads data at `{python} StorageLoading.nvme_bw_gbs_str` completes dataset loading in `{python} StorageLoading.nvme_load_str`, compared to `{python} StorageLoading.obj_load_str` at typical object storage speeds. This `{python} StorageLoading.load_speedup_str`$\times$ difference determines whether teams can iterate multiple times daily or must wait hours between experiments.
...
class ModelStorageScale:
    ...
    gpt3_fp32_gb = gpt3_fp32.m_as(GB)
    gpt3_fp16_gb = gpt3_fp16.m_as(GB)
    ...
    gpt3_fp32_str = fmt_qty(gpt3_fp32, GB, precision=0, commas=False)
    gpt3_fp16_str = fmt_qty(gpt3_fp16, GB, precision=0, commas=False)
...
from mlsysim.fmt import fmt_int, fmt, fmt_count, check, fmt_qty, fmt_qty_int, fmt_rate
...
class StorageBandwidth:
    ...
    sata_bw = sata_bw_mbs * MB / second
    s3_bw = s3_bw_mbs * MB / second
    nvme_bw_low = nvme_bw_low_gbs * GB / second
    nvme_bw_high = nvme_bw_high_gbs * GB / second
    ...
    required_bw = required_bw_gbs * GB / second
    ...
    a100_flops_t_str = fmt_qty_int(h_a100.compute.peak_flops, TFLOP/second, commas=False)
    resnet_flops_g_str = fmt_qty(resnet_flops_per_img * flop, GFLOP, precision=1, commas=False)
...
class CompressionTradeoff:
    ...
    time_diff_str = fmt_time(time_diff_min, "minute", precision=1, commas=False, style="word")
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_qty, check, fmt_rate
...
class LifecycleStorageRequirements:
    ...
    training_bw = training_bw_gbs * GB / second
...
class DataEngineeringSummaryRecap:
    ...
    kws_dataset_size_gb = kws_dataset_size.m_as(GB)
    train_cost_low = a100_count * train_hours_low * gpu_hour_cost_low
    train_cost_high = a100_count * train_hours_high * gpu_hour_cost_high
    ratio_low = labeling_budget/train_cost_high
    ratio_high = labeling_budget/train_cost_low
