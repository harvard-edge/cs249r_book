...
::: {#fig-misalignment fig-env="figure" fig-pos="htb" fig-cap="**Shared Dataset Bias Propagation**: Five models (A through E) all train on a single central dataset repository. One arrow labeled shared training data carries the dataset to every model, and the labeled arrows beneath the models converge on a single outcome: bias propagation, shared limits, and blind spots surface not as independent defects but as one correlated failure across the ecosystem." fig-alt="Five model boxes labeled A through E at center all connect upward to one central training dataset repository. Arrows downward show shared limitations, biases, and blind spots propagating to all models."}
