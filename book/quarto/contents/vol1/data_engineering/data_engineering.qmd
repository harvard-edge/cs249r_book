...
The preceding discussion of data cascades reveals a fundamental insight: most ML system failures trace back to data problems rather than model architecture choices. This observation has catalyzed a paradigm shift in how practitioners approach ML development, moving from model-centric to data-centric AI [@ng2021datacentric].
...
Understanding the three fundamental types of drift enables targeted detection and response strategies. Each type manifests differently in production systems and requires distinct monitoring approaches.
...
[^fn-covariate-shift]: **Covariate Shift**: A type of data drift where the distribution of input features $P(X)$ changes between training and serving while the conditional relationship $P(Y|X)$ remains constant. Common in production ML when data collection contexts change (new sensors, different user populations, seasonal effects) but fundamental relationships persist. Detectable through feature distribution monitoring without requiring ground truth labels.
...
[^fn-concept-drift]: **Concept Drift**: A type of data drift where the relationship between features and labels $P(Y|X)$ changes over time, fundamentally altering what the model must learn. Most challenging drift type to detect and respond to, requiring ground truth labels and potentially model retraining with recent data emphasizing new patterns. May indicate need for architectural changes if relationships have fundamentally altered.
...
Model updates in response to confirmed concept drift require retraining with recent data that emphasizes new patterns. When the feature-label relationship has fundamentally changed, simply retraining on the same architecture with new data may prove insufficient. The system might need additional features capturing new patterns, architectural changes enabling the model to learn more complex relationships, or expanded training data covering previously rare scenarios that have become common.
...
@tbl-storage summarizes these fundamental trade-offs, comparing databases, warehouses, and data lakes across purpose, data types, scale, and performance optimization.
...
Data versioning connects model versions to exact training data, enabling debugging and reproducibility. Without data versioning, teams cannot answer fundamental questions like "what exact data trained model v47?"
