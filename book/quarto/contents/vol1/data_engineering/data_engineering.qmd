...
# FP16 with structured sparsity (2:4 pattern) = 2x dense FP16
a100_tflops_fp16_sparse = f"{(A100_FLOPS_FP16_TENSOR * 2).to(TFLOPs/second).magnitude:.0f}"

# Energy
energy_dram = f"{ENERGY_DRAM_ACCESS_PJ.magnitude:.0f}"
energy_flop_fp32 = f"{ENERGY_FLOP_FP32_PJ.magnitude:.1f}"
energy_flop_int8 = f"{ENERGY_FLOP_INT8_PJ.magnitude:.1f}"

# Energy for addition operations (Horowitz 2014, 45nm)
energy_add_int32_pj = 0.1
energy_add_fp32_pj = 0.9
energy_dram_pj = ENERGY_DRAM_ACCESS_PJ.magnitude  # 640 pJ
energy_network_1kb_pj = 1_000_000  # ~1 mJ for 1KB network transfer

# Relative costs for data locality table
dram_vs_int = int(energy_dram_pj / energy_add_int32_pj)  # 6400×
network_vs_int = int(energy_network_1kb_pj / energy_add_int32_pj)  # 10,000,000×

# Strings
energy_add_int32_str = f"{energy_add_int32_pj}"
energy_add_fp32_str = f"{energy_add_fp32_pj}"
energy_dram_str = f"{energy_dram_pj:.0f}"
dram_vs_int_str = f"{dram_vs_int:,}"
network_vs_int_str = f"{network_vs_int:,}"

# Multipliers (legacy)
dram_multiplier = f"{int(ENERGY_DRAM_ACCESS_PJ.magnitude / ENERGY_FLOP_FP32_PJ.magnitude):,}"

# GPT-3 model sizes
gpt3_params_b = GPT3_PARAMS.magnitude / 1e9  # 175B
gpt3_fp32_gb = gpt3_params_b * 4  # 700 GB
gpt3_fp16_gb = gpt3_params_b * 2  # 350 GB

# AlexNet for comparison
alexnet_params_m = 60  # 60M parameters (approximate)

# Growth factor
model_growth_factor = int(gpt3_params_b * 1e9 / (alexnet_params_m * 1e6))  # ~2900×

# Strings
gpt3_params_str = f"{gpt3_params_b:.0f}"
gpt3_fp32_str = f"{gpt3_fp32_gb:.0f}"
gpt3_fp16_str = f"{gpt3_fp16_gb:.0f}"
...
|:---------------------------------|-----------------------------:|------------------------------:|
| **32-bit Floating Point Add**      | `{python} energy_flop_fp32` pJ |                              1× |
| **DRAM Memory Access (32-bit)**    |  **`{python} energy_dram` pJ** | **`{python} dram_multiplier`×** |
...
# Data gravity physics
dataset_pb = 1
dataset_gb_gravity = dataset_pb * 1_000_000  # 1 PB in GB
network_gbps = 100  # Gbps
network_gbs = network_gbps / 8  # GB/s
transfer_seconds = dataset_gb_gravity / network_gbs
transfer_hours = transfer_seconds / 3600

egress_cost_per_gb = 0.01  # $/GB
transfer_cost = dataset_gb_gravity * egress_cost_per_gb

tpuv4_cost_per_hour = 4.0  # approximate $/hour
equivalent_tpu_hours = transfer_cost / tpuv4_cost_per_hour

transfer_seconds_str = f"{transfer_seconds:,.0f}"
transfer_hours_str = f"{transfer_hours:.0f}"
transfer_cost_str = f"{transfer_cost:,.0f}"
tpu_hours_str = f"{equivalent_tpu_hours:,.0f}"
network_gbs_str = f"{network_gbs:.1f}"
dataset_gb_str = f"{dataset_gb_gravity:,}"
```

**The Physics**:

1.  **Network Bandwidth**: A dedicated `{python} network_gbps` Gbps line = `{python} network_gbs_str` GB/s.
...
3.  **Cost**: At \$`{python} egress_cost_per_gb`/GB egress, moving 1 PB costs **\$`{python} transfer_cost_str`**.

**The Engineering Conclusion**:
If training takes < `{python} transfer_hours_str` hours, you spend more time moving data than training.
If training costs < \$`{python} transfer_cost_str` (approx. `{python} tpu_hours_str` TPUv4-hours), you spend more on bandwidth than compute.
...
# False positive target for always-on KWS
seconds_per_minute = 60
minutes_per_hour = 60
hours_per_day = 24
days_per_month = 30
windows_per_month = seconds_per_minute * minutes_per_hour * hours_per_day * days_per_month

# Pre-formatted strings
sec_str = str(seconds_per_minute)
min_str = str(minutes_per_hour)
hr_str = str(hours_per_day)
day_str = str(days_per_month)
windows_per_month_str = f"{windows_per_month:,}"

fpr = 1 / windows_per_month
fpr_str = f"{fpr:.1e}"
rejection_pct = (1 - fpr) * 100
rejection_pct_str = f"{rejection_pct:.5f}"
```
...
- **False Positive Rate (FPR)**: $\frac{1}{\text{windows}}$ ≈ `{python} fpr_str`
...
At $`{python} cost_per_label_str`/label with `{python} review_overhead_pct_str`% review overhead: $`{python} labeling_k`K ÷ $`{python} effective_cost_str` = **`{python} labeled_k`K labeled examples**
...
# KS test critical value example
ks_n = 1000
ks_coefficient = 1.36
ks_dcrit = ks_coefficient / math.sqrt(ks_n)
ks_dcrit_str = f"{ks_dcrit:.3f}"
```
...
- **Result**: $0.08 > `{python} ks_dcrit_str`$ $\rightarrow$ **Reject Null Hypothesis**. Significant drift detected. Trigger retraining or investigation.
:::
...
event_size_kb = 1
throughput_gbs = events_per_sec * event_size_kb / 1e6  # GB/s

# Stream: 100 cores × 24 hrs × $0.05/hr
stream_cores = 100
stream_hours = 24
stream_cost_per_hr = 0.05
stream_cost_day = stream_cores * stream_hours * stream_cost_per_hr

# Batch: 3.6 TB/hour, process in 10 min
hourly_data_tb = throughput_gbs * 3600 / 1000  # TB per hour
batch_core_hours = 200 * (10/60) * 24  # 200 cores × 10min/hr × 24 hours = 800 core-hours/day
batch_cost_day = batch_core_hours * stream_cost_per_hr

cost_ratio = stream_cost_day / batch_cost_day

throughput_str = f"{throughput_gbs:.0f}"
hourly_tb_str = f"{hourly_data_tb:.1f}"
stream_cost_str = f"{stream_cost_day:.0f}"
batch_cost_str = f"{batch_cost_day:.0f}"
cost_ratio_str = f"{cost_ratio:.0f}"
events_per_sec_str = f"{events_per_sec / 1e6:.0f}M"
event_size_kb_str = str(event_size_kb)
stream_cores_str = str(stream_cores)
stream_hours_str = str(stream_hours)
stream_cost_per_hr_str = f"{stream_cost_per_hr}"
batch_core_hours_str = f"{batch_core_hours:.0f}"
```
...
2.  **Stream Requirements**: To sustain `{python} throughput_str` GB/s with <100ms latency, you need ~50 dedicated cores + redundant backups (always on). Cost: `{python} stream_cores_str` cores $\times$ `{python} stream_hours_str` hrs $\times$ $`{python} stream_cost_per_hr_str`/hr = **$`{python} stream_cost_str`/day**.
3.  **Batch Requirements**: Process `{python} hourly_tb_str`TB (1 hour data) in 10 mins. High throughput (sequential I/O) is efficient. You need 200 cores for 10 mins/hour $\times$ 24 hours = `{python} batch_core_hours_str` core-hours/day. Cost: `{python} batch_core_hours_str` $\times$ $`{python} stream_cost_per_hr_str` = **$`{python} batch_cost_str`/day**.
...
# ETL vs ELT cost comparison (hidden computation for inline values)
daily_raw_tb = 10
s3_per_tb_mo = 23
spark_per_tb = 5
n_models = 3
...
daily_raw_tb_str = str(daily_raw_tb)
spark_per_tb_str = str(spark_per_tb)
etl_datasets_str = str(etl_datasets)
etl_tb_each_str = str(etl_tb_each)
etl_storage_tb_str2 = str(etl_storage_tb)
s3_per_tb_mo_str = str(s3_per_tb_mo)
retention_days_str = str(retention_days)
query_cost_per_str = str(query_cost_per)
n_models_str = str(n_models)
```
...
1.  **ETL Approach**: Transform before loading. Compute all three aggregation windows in a Spark cluster before loading into the warehouse.
    *   Spark compute: `{python} daily_raw_tb_str` TB at $`{python} spark_per_tb_str`/TB = $`{python} etl_spark_str`/day
    *   Storage: `{python} etl_datasets_str` transformed datasets, ~`{python} etl_tb_each_str` TB each = `{python} etl_storage_tb_str2` TB at $`{python} s3_per_tb_mo_str`/TB per month = $`{python} etl_storage_str`/month
    *   Schema change cost: Re-run full pipeline (~4 hours) per change

2.  **ELT Approach**: Load raw data first, transform in warehouse.
    *   Storage: `{python} daily_raw_tb_str` TB raw/day, `{python} retention_days_str`-day retention = `{python} elt_storage_tb_str` TB at $`{python} s3_per_tb_mo_str`/TB per month = $`{python} elt_storage_str`/month
    *   Query compute: `{python} n_models_str` models × $`{python} query_cost_per_str`/query × `{python} retention_days_str` days = $`{python} elt_query_str`/month
    *   Schema change cost: Rewrite SQL query (~30 minutes) per change

**The Engineering Conclusion**: ETL saves $`{python} savings_str`/month in storage but costs 8× more engineering time per schema change. If your feature definitions change weekly, ELT's flexibility pays for itself. If schemas are stable, ETL's lower storage cost dominates. The break-even point: if you change schemas fewer than once per month, ETL wins on total cost.
:::
...
# Amdahl's Law applied to data pipelines
serial_fraction = 0.70
parallel_fraction = 1 - serial_fraction
amdahl_speedup = 1 / serial_fraction
amdahl_speedup_str = f"{amdahl_speedup:.2f}"
amdahl_improvement_pct = int(round((amdahl_speedup - 1) * 100))
```
...
$$ \text{Speedup}_{\max} = \frac{1}{0.70 + \frac{0.30}{\infty}} = \frac{1}{0.70} \approx `{python} amdahl_speedup_str`\times $$

Even infinite parallelization of non-labeling work yields only **`{python} amdahl_improvement_pct`% improvement**.
...
c_total = c_acquire + c_label + c_store + c_process + c_govern

# Percentages
p_acquire = c_acquire / c_total * 100
p_label = c_label / c_total * 100
p_store = c_store / c_total * 100
p_process = c_process / c_total * 100
p_govern = c_govern / c_total * 100

# Format strings
c_acquire_str = f"${c_acquire}K"
c_label_str = f"${c_label}K"
c_store_str = f"${c_store}K"
c_process_str = f"${c_process}K"
c_govern_str = f"${c_govern}K"
c_total_str = f"${c_total}K"

p_acquire_str = f"{p_acquire:.0f}%"
p_label_str = f"{p_label:.0f}%"
p_store_str = f"{p_store:.0f}%"
p_process_str = f"{p_process:.0f}%"
p_govern_str = f"{p_govern:.0f}%"
```
...
# From TCDO worked example above
tcdo_label_pct = 59  # Label as % of total
tcdo_process_pct = 4  # Process (compute) as % of total
tcdo_label_vs_compute = tcdo_label_pct / tcdo_process_pct
tcdo_label_vs_compute_str = f"{tcdo_label_vs_compute:.0f}"
```

**The Optimization Priority**: Labeling dominates at `{python} tcdo_label_pct`%, nearly `{python} tcdo_label_vs_compute_str`x the compute cost. Teams optimizing GPU utilization while ignoring labeling efficiency are optimizing the wrong `{python} tcdo_process_pct`%.
:::
...
# Dataset loading time comparison: NVMe vs object storage
kws_dataset_gb = 736
...
nvme_load_str = f"{nvme_load_s:.0f}"
obj_load_str = f"{obj_load_s:,.0f}"
load_speedup_str = f"{load_speedup:.0f}"
```
...
# Storage bandwidth budget for A100 + ResNet-50
a100_flops_val = 312e12          # FP16 tensor core FLOPs/sec
resnet_flops_per_img = 12e9      # forward + backward FLOPs per image
max_img_per_sec = int(a100_flops_val / resnet_flops_per_img)
max_img_per_sec_str = f"{max_img_per_sec:,}"

image_size_kb = 150
required_bw_gbs = max_img_per_sec * image_size_kb * 1e3 / 1e9
required_bw_gbs_str = f"{required_bw_gbs:.1f}"

# Pre-formatted strings for inputs
a100_flops_t_str = str(int(a100_flops_val / 1e12))
resnet_flops_g_str = str(int(resnet_flops_per_img / 1e9))
image_size_kb_str = str(image_size_kb)

# SATA SSD bottleneck
sata_bw_mbs = 500
sata_bw_mbs_str = str(sata_bw_mbs)
sata_max_img_sec = int(sata_bw_mbs * 1e6 / (image_size_kb * 1e3))
sata_max_img_sec_str = f"{sata_max_img_sec:,}"
sata_utilization_pct = int(round(sata_max_img_sec / max_img_per_sec * 100))

# S3 worker threads
s3_bw_mbs = 100
s3_bw_mbs_str = str(s3_bw_mbs)
...
*   **Hardware Peak:** NVIDIA A100 = `{python} a100_tflops_fp16` TFLOPS (FP16 Tensor Core, dense operations).
*   **Model Cost:** ResNet-50 = ~4 GFLOPs per image (forward + backward $\approx$ 12 GFLOPs).
*   **Maximum Theoretical Throughput**:
    $$ \frac{`{python} a100_flops_t_str` \times 10^{12} \text{ FLOPs/sec}}{`{python} resnet_flops_g_str` \times 10^9 \text{ FLOPs/img}} = \mathbf{`{python} max_img_per_sec_str` \text{ images/sec}} $$

**2. The Data Term (Bandwidth Requirement)**

*   **Image Size:** `{python} image_size_kb_str` KB (JPEG compressed).
*   **Required Bandwidth**:
    $$ `{python} max_img_per_sec_str` \text{ img/sec} \times `{python} image_size_kb_str` \text{ KB/img} \approx \mathbf{`{python} required_bw_gbs_str` \text{ GB/s}} $$

**The Systems Conclusion:**
To saturate a *single* A100, your storage must deliver **`{python} required_bw_gbs_str` GB/s**.
*   **S3 Standard**: ~`{python} s3_bw_mbs_str` MB/s per thread. You need **`{python} s3_workers` concurrent worker threads**.
*   **SATA SSD**: ~`{python} sata_bw_mbs_str` MB/s. Totally insufficient (bottleneck).
*   **NVMe SSD**: ~3-7 GB/s. **Required.**

**Iron Law Implication:** If you use SATA SSDs, your maximum throughput is capped at $`{python} sata_bw_mbs_str` \text{ MB/s} / `{python} image_size_kb_str` \text{ KB} \approx `{python} sata_max_img_sec_str` \text{ img/s}$. Your \$15,000 GPU will run at **`{python} sata_utilization_pct`% utilization** ($`{python} sata_max_img_sec_str`/`{python} max_img_per_sec_str`$). Storage physics dictates training speed.
:::
...
# Format efficiency: columnar vs row-oriented
needed_cols = 20
total_cols = 100
...
eta_csv_str = f"{eta_csv:.1f}"
eta_parquet_str = f"{eta_parquet:.1f}"
waste_pct_str = f"{waste_pct:.0f}"
throughput_ratio_str = f"{throughput_ratio:.0f}"
```

**Scenario**: Training a fraud model using `{python} needed_cols` features from a `{python} total_cols`-column table.

- **Row-Oriented (CSV)**: Must read all `{python} total_cols` columns to get the `{python} needed_cols` needed.
    - $\eta_{format}$ = `{python} needed_cols`/`{python} total_cols` = `{python} eta_csv_str`.
    - **Result**: You waste `{python} waste_pct_str`% of your disk bandwidth.
- **Column-Oriented (Parquet)**: Reads only the `{python} needed_cols` columns needed.
    - $\eta_{format}$ ≈ `{python} eta_parquet_str` (ignoring metadata overhead).
...
decompress_speedup_str = f"{decompress_speedup:.0f}"
gzip_min_str = f"{gzip_decompress_min:.0f}"
snappy_min_str = f"{snappy_decompress_min:.0f}"
time_diff_str = f"{time_diff_min:.0f}"
total_diff_hours_str = f"{total_diff_hours:.0f}"
```

Compression algorithm\index{Compression!algorithm selection} selection involves trade-offs between compression ratio and decompression speed. While gzip achieves higher compression ratios of six to eight times, Snappy achieves only two to three times compression but decompresses at `{python} snappy_decompress_mbs` megabytes per second—roughly `{python} decompress_speedup_str` times faster than gzip's `{python} gzip_decompress_mbs` megabytes per second. For ML training where throughput matters more than storage costs, Snappy's speed advantage often outweighs gzip's space savings. Training on a `{python} dataset_gb` gigabyte dataset compressed with gzip requires `{python} gzip_min_str` minutes of decompression time, while Snappy requires only `{python} snappy_min_str` minutes. When training iterates over data for `{python} n_epochs` epochs, this `{python} time_diff_str`-minute difference per epoch compounds to `{python} total_diff_hours_str` hours total—potentially the difference between running experiments overnight versus waiting multiple days for results. The choice cascades through the system: faster decompression enables higher batch sizes (fitting more examples in memory after decompression), reduced buffering requirements (less decompressed data needs staging), and better GPU utilization (less time idle waiting for data).
...
s3_per_tb_mo = 23        # $/TB/month for S3 Standard
glacier_per_tb_mo = 1    # $/TB/month for Glacier Deep Archive
glacier_retrieval_per_gb = 0.02  # $/GB retrieval cost
nvme_per_gb = 0.10       # $/GB purchase cost
dataset_tb = 100

# Full retrieval cost
retrieval_cost = dataset_tb * 1000 * glacier_retrieval_per_gb
equiv_months = retrieval_cost / s3_per_tb_mo
# Note: equivalence is per-TB ($2000 / $23/mo ≈ 87 months of storing 1 TB,
# but the data is 100 TB so S3 cost is $23×100 = $2300/mo)
# The text says "equivalent to 87 months of S3 Standard storage for that data"
# meaning $2000 / $23 = 87 (treating it as cost-per-TB equivalence)
equiv_months_per_tb = retrieval_cost / s3_per_tb_mo

# Daily random access: 10% of dataset
daily_access_pct = 0.10
daily_retrieval = dataset_tb * daily_access_pct * 1000 * glacier_retrieval_per_gb
s3_monthly_cost = s3_per_tb_mo * dataset_tb

# NVMe cost per TB
nvme_per_tb = nvme_per_gb * 1000

retrieval_str = f"{retrieval_cost:,.0f}"
equiv_months_str = f"{equiv_months_per_tb:.0f}"
daily_retrieval_str = f"{daily_retrieval:,.0f}"
s3_monthly_str = f"{s3_monthly_cost:,.0f}"
nvme_per_tb_str = f"{nvme_per_tb:.0f}"
```

Teams compare raw storage costs (S3 Standard at $23 per TB per month vs Glacier Deep Archive at $1 per TB per month) and select the cheapest option without analyzing access patterns. However, @sec-data-engineering-ml-ml-storage-systems-architecture-options-67fa establishes that total cost of ownership includes retrieval costs, access latency, and throughput limitations. Glacier Deep Archive charges $0.02 per GB for retrieval plus 12-hour access latency, meaning a single full retrieval of 100 TB costs $`{python} retrieval_str`, equivalent to `{python} equiv_months_str` months of S3 Standard storage for that data. Training workloads requiring random access to 10% of a 100 TB dataset daily would spend $`{python} daily_retrieval_str` per day on Glacier retrievals versus $`{python} s3_monthly_str` per month for S3 Standard storage with 3,500 MB/s sustained read throughput. Local NVMe storage at $0.10 per GB ($`{python} nvme_per_tb_str` per TB) appears expensive but delivers 7,000 MB/s sequential reads and sub-millisecond random access latency, completing training epochs 15 to 30x faster than S3 for I/O-bound workloads. Organizations must match storage tiers to access patterns: cold archival data to Glacier, training datasets requiring sequential scans to S3, and active training data with random access patterns to local SSDs or high-performance networked storage.
