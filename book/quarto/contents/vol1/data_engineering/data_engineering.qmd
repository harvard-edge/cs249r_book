...
![](images/png/cs249r_labels_new.png){width="80%"}
...
\node[Box, node distance=0.5,below=of B1,xshift=9mm](B2){\textbf{Traditional Supervision:} Have subject
              matter experts (SMEs) hand-label more training data};
\node[Box,below=0.7 of B2](B3){\textbf{Semi-supervised Learning:} Use structural
            assumptions to automatically leverage unlabeled data};
\node[Box,below=0.7of B3](B4){\textbf{Weak Supervision:}\\ Get lower-quality
            labels more efficiently and/or at a higher abstraction level};
\node[Box,below=0.7of B4](B5){\textbf{Transfer Learning:}\\ Use models
...
%
\node[Box2,above right=1.2 and 1.75 of B4](2B3){Get cheaper, lower-quality labels from nonexperts};
\node[Box2,below =0.4 of 2B3](2B4){Get higher-level supervision
            over unlabeled data from SMEs};
\node[Box2,below =0.4 of 2B4](2B5){Use one or more
...
% Branch 1: No -> D2
\node[Diamond, below=1.1 of D1](D2){Training Acc $\gg$ Validation Acc?};
...
% Branch 2: No -> D3
\node[Diamond, below=1.1 of D2](D3){Validation Acc $\gg$ Production Acc?};
...
% Branch 3: No -> D4
\node[Diamond, below=1.1 of D3](D4){Performance inconsistent across subgroups?};
