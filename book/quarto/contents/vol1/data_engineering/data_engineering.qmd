...
Every data engineering decision, from choosing storage formats to designing ingestion pipelines, should be evaluated against the four foundational principles shown in @fig-four-pillars. Each pillar contributes to system success through systematic decision-making.
...
The Four Pillars framework provides diagnostic utility when systems fail. @tbl-four-pillars-diagnostic maps common symptoms to likely root causes:
...
Data scientists spend 60-80% of their time on data preparation tasks according to industry surveys (@fig-ds-time)[^fn-data-quality-stats]. This reflects a state where data engineering practices are often ad-hoc rather than systematic. Applying the four-pillar framework consistently can reduce data preparation time while building more reliable and maintainable systems.
...
@fig-cascades illustrates potential data pitfalls at every stage and how they propagate through the pipeline. Data collection errors are especially problematic. Lapses in this initial stage become apparent during model evaluation and deployment (@sec-ai-training and @sec-ml-operations), potentially requiring abandoning the entire model and restarting. Investing in data engineering techniques from the start helps detect errors early, mitigating these cascading effects.
...
The shift to data-centric AI does not abandon model development. It rebalances engineering effort. Consider the contrast in development workflows shown in @tbl-model-vs-data-centric.
...
: Model-centric vs data-centric development approaches {#tbl-model-vs-data-centric}
...
**Labeling Functions**: Domain experts encode labeling heuristics as programmatic functions. For example, a sentiment classifier might use rules like "reviews mentioning 'return' or 'refund' are likely negative" as weak labels. Individual labeling functions may be noisy. However, combining multiple functions through probabilistic models produces training labels that approach expert quality.

**Active Learning Integration**: Data-centric development combines programmatic labeling with strategic human annotation. Active learning identifies examples where model uncertainty is highest or where labeling functions disagree, directing human effort to cases with maximum impact on model improvement.
...
**Consistency Score**: Measures agreement across labelers or labeling functions for the same examples. Low consistency indicates ambiguous labeling guidelines or inherently difficult examples requiring special handling.
...
**Label Error Rate**: Estimated through model confidence analysis, cross-validation disagreement, or expert audit of random samples. Even small label error rates significantly impact model ceiling performance.

**Freshness**: For production systems, measures how well training data reflects current deployment distribution. Stale data causes distribution shift problems discussed in subsequent sections.
...
**Declarative Expectations** formalize quality requirements in code. Using libraries like Great Expectations or Pandera, teams define assertions that execute on every pipeline run, as shown in @lst-data-expectations.
...
**Population Stability Index (PSI)** provides an industry-standard metric for detecting changes in feature distributions [@siddiqi2006credit]. PSI discretizes continuous features into bins (typically 10-20 bins) and compares actual serving distribution against expected training distribution:
...
To build engineering judgment, practitioners must internalize the orders of magnitude separating these tiers. @tbl-ml-latencies provides the "numbers every ML systems engineer should know," using a human-scale analogy to illustrate the staggering disparities in access time.
