...
\begin{marginfigure}
\mlsysstack{0}{10}{0}{40}{30}{50}{15}{90}
\end{marginfigure}
...
- Design data acquisition strategies combining curated datasets, crowdsourcing, and synthetic generation based on cost-quality trade-offs
- Architect data pipelines selecting batch vs. streaming ingestion and ETL vs. ELT patterns with validation and graceful degradation
...
- Evaluate storage architectures and file formats for ML workloads including feature store design for training-serving consistency
...
This compilation metaphor establishes the engineering mindset that runs through the chapter. We begin with the physical constraints that govern data movement and storage (@sec-data-engineering-ml-physics-of-data), then introduce a systematic **Four Pillars Framework** (@sec-data-engineering-ml-four-pillars-framework-4ef1) organizing decisions around Quality, Reliability, Scalability, and Governance. The remaining sections follow data through the pipeline: acquisition, architecture and ingestion, processing, labeling, storage, and operational health—each illustrated through a Keyword Spotting (KWS) case study.
...
\index{Data Labeling!systems engineering}\index{Ground Truth!establishing}The processing pipelines examined above transform raw data into structured features, but one critical input remains: the labels that tell our models what patterns to learn. Unlike automated transformations, labeling introduces human judgment into the pipeline, creating unique engineering challenges. As training datasets grow to millions or billions of examples, the infrastructure supporting labeling operations becomes critical to system performance. The goal is to establish **ground truth**\index{Ground Truth!definition}[^fn-ground-truth-etymology], the foundational reference labels the model will learn from.
...
The feature computation placement trade-off (@sec-data-engineering-ml-feature-computation-placement-a998) is especially acute during training: precomputing ResNet features achieves 30-fold storage reduction (150 KB images to 5 KB vectors) but introduces staleness risk when extraction logic changes, requiring full-dataset recomputation.
...
With the pipeline fully assembled—acquisition, ingestion, processing, labeling, and storage—we shift from building infrastructure to maintaining it. The final section addresses the ongoing health of these systems in production.

## Operational Data Health {#sec-data-engineering-ml-data-debt-hidden-liability-3335}

\index{Data Debt!hidden liability}\index{Technical Debt!data systems}Production data systems require ongoing maintenance, much like physical infrastructure. This section covers two interrelated concerns: the *data debt* that accumulates from engineering shortcuts, and the *debugging techniques* for diagnosing pipeline failures when that debt comes due. Every shortcut—from deferred label corrections to undocumented schema changes—creates technical debt\index{Technical Debt!data engineering} that compounds silently. Unlike code debt, which manifests as slower development velocity, data debt directly degrades model performance and can remain invisible until failures become catastrophic.

### Categories of Data Debt {#sec-data-engineering-ml-categories-data-debt-ee90}

\index{Data Debt!categories}Data debt is the ML counterpart to software technical debt: accumulated compromises in data quality, documentation, and infrastructure that compound over time, degrading model performance and increasing operational costs.
...
* **Pipeline architecture choices have order-of-magnitude cost implications.** Real-time streaming costs ~3x more than batch for the same data volume; ETL saves storage but costs 8x more engineering time per schema change. Select ingestion patterns based on the value of latency, not the appeal of real-time.
