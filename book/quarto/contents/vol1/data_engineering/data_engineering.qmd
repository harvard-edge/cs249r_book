...
1.  **Network Bandwidth**: A dedicated `{python} network_gbps` Gbps line = `{python} f"{network_gbs:.1f}"` GB/s.
2.  **Transfer Time**: $`{python} f"{dataset_gb_gravity:,}"` \text{{ GB}} / `{python} f"{network_gbs:.1f}"` \text{{ GB/s}} = `{python} transfer_seconds_str` \text{{ seconds}} \approx \mathbf{`{python} transfer_hours_str` \text{{ hours}}}$.
...
- **False Positive Rate (FPR)**: $\frac{1}{`{python} windows_per_month_str`} \approx `{python} fpr_str`$
...
- **Row-Oriented (CSV)**: Must read all `{python} total_cols` columns to get the `{python} needed_cols` needed.
    - $\eta_{format} = \frac{`{python} needed_cols`}{`{python} total_cols`} = `{python} eta_csv_str`$.
...
- **Column-Oriented (Parquet)**: Reads only the `{python} needed_cols` columns needed.
    - $\eta_{format} \approx `{python} f"{eta_parquet:.1f}"`$ (ignoring metadata overhead).
    - **Result**: You get **`{python} throughput_ratio_str`x higher effective throughput**.
