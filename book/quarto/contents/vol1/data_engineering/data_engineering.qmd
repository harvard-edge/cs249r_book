...
# │
# │ Why: Registers this chapter with the physx runtime so that cross-chapter
# │      references and figure numbering resolve correctly.
# │
# │ Imports: physx.registry (start_chapter)
...
# │
# │ Why: Centralizes energy, scale, labeling-cost, and storage-cost constants
# │      so that every downstream cell and inline reference draws from a
# │      single source of truth. Covers the Energy-Movement Invariant table,
# │      KWS case-study parameters, labeling-cost ranges, and storage tiers.
# │
# │ Imports: physx.constants (*), physx.formatting (fmt, sci, md_math),
...
# --- Inputs (energy, hardware, and KWS constants) ---
...
# │
# │ Why: Quantifies the physical cost of moving 1 PB across regions—transfer
# │      time on 100 Gbps and 10 Gbps links, egress charges, and the
# │      equivalent TPU-hours those charges could buy. Demonstrates that data
# │      gravity makes "move compute to data" the only viable strategy.
# │
# │ Imports: physx.constants (NETWORK_100G_BW, CLOUD_EGRESS_PER_GB,
# │              TPU_V4_PER_HOUR, SECONDS_PER_MINUTE, MINUTES_PER_HOUR,
# │              HOURS_PER_DAY, Gbps, GB, USD, hour, second),
# │          physx.formatting (fmt, md_math)
...
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import (
    NETWORK_100G_BW, CLOUD_EGRESS_PER_GB, TPU_V4_PER_HOUR,
    SECONDS_PER_MINUTE, MINUTES_PER_HOUR, HOURS_PER_DAY,
    Gbps, GB, USD, hour, second,
)
from physx.formatting import fmt, md_math

# --- Inputs (network, cloud pricing, and TPU cost) ---
dataset_pb_value = 1
...
# --- Process (transfer time and cost) ---
...
: **Four Pillars Diagnostic Guide**: When ML systems exhibit failures, this mapping helps teams identify which pillar to investigate first based on observed symptoms. Note that many failures involve multiple pillars—features differing between training and serving implicate both Quality (incorrect values) and Reliability (inconsistent computation)—so the table serves as a starting point for investigation, not a definitive diagnosis. {#tbl-four-pillars-diagnostic}
...
# │
# │ Why: Derives the false-positive-rate ceiling for an always-on KWS system
# │      that must produce at most 1 false wake-up per month. Shows students
# │      how operational SLAs translate into quantitative ML constraints
# │      (rejection rate > 99.999%).
# │
# │ Imports: physx.constants (SECONDS_PER_MINUTE, MINUTES_PER_HOUR,
...
# --- Process (windows and false-positive rate) ---
...
# │
# │ Why: Breaks a $150K KWS data budget into labeling/storage/governance
# │      slices and computes how many labeled examples the labeling budget
# │      buys after review overhead. Teaches students that data budget
# │      planning is an engineering optimization problem.
# │
...
# --- Process (budget allocation and effective cost) ---
...
# │
# │ Why: Computes the critical value D_crit for the Kolmogorov–Smirnov test
# │      at alpha = 0.05 with n = 1000 samples. Gives students a concrete
# │      numeric threshold they can use to detect distribution drift between
# │      training and serving data.
# │
# │ Imports: math (sqrt), physx.constants (KS_TEST_COEFFICIENT),
# │          physx.formatting (fmt)
# │ Exports: ks_dcrit_str
# └─────────────────────────────────────────────────────────────────────────────
import math
from physx.constants import KS_TEST_COEFFICIENT
from physx.formatting import fmt

# --- Inputs (sample size and coefficient) ---
ks_n_value = 1000
...
The K-S test is one tool for detecting drift in continuous features; @sec-data-engineering-ml-detecting-responding-data-drift-509a provides the complete taxonomy of distribution shifts (covariate, label, concept, and label-quality drift) along with Population Stability Index (PSI) and KL divergence metrics for operationalizing the Degradation Equation.
...
# │
# │ Why: Visualizes the min(CPU supply, GPU demand) bottleneck that governs
# │      training throughput. Shows the starvation vs. saturation regions
# │      so students can diagnose data-pipeline bottlenecks from throughput
# │      curves.
# │
...
This section examines the two primary ingestion patterns, batch and streaming, along with the ETL and ELT processing paradigms that govern how transformations are applied during ingestion. Together, these choices shape the cost, latency, and reliability profile of every downstream pipeline stage.
...
# │
# │ Why: Compares the daily dollar cost of always-on stream processing vs.
# │      periodic batch processing for 1 M events/sec. Quantifies the
# │      order-of-magnitude cost premium of real-time so students can make
# │      informed latency-vs-cost trade-offs.
# │
# │ Imports: physx.constants (KB, GB, TB), physx.formatting (fmt)
...
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import KB, GB, TB
from physx.formatting import fmt

# --- Inputs (event rate and cost parameters) ---
...
# --- Process (throughput and cost comparison) ---
...
# │
# │ Why: Compares the monthly storage and compute costs of ETL (transform
# │      before loading) vs. ELT (load raw, transform on query). Quantifies
# │      the storage premium of retaining raw data in ELT and the Spark
# │      compute cost of ETL, helping students reason about when each
# │      paradigm is cost-optimal.
# │
# │ Imports: physx.formatting (fmt)
...
# --- Inputs (data volumes and cost parameters) ---
...
# --- Process (ETL vs ELT cost comparison) ---
...
To address these scaling bottlenecks, data must be partitioned across multiple computing resources, which introduces coordination challenges. Distributed coordination is constrained by network round-trip times: local operations complete in microseconds while network coordination requires milliseconds, creating a 1000$\times$ latency difference. This constraint explains why operations requiring global coordination (like computing normalization statistics across 100 machines) create bottlenecks. Each partition computes local statistics quickly, but combining them requires information from all partitions.
...
The choice between databases, data warehouses, and data lakes is a choice about which of these metrics to optimize:
...
# │
# │ Why: Compares NVMe vs. object-storage load time for the 736 GB KWS
# │      dataset. The 50× speedup illustrates why storage tier selection
# │      directly determines iteration velocity during model development.
# │
# │ Imports: physx.formatting (fmt)
...
# --- Inputs (dataset size and bandwidth tiers) ---
...
# │
# │ Why: Derives the storage bandwidth required to keep a single A100 fully
# │      utilized while training ResNet-50. Shows that SATA SSDs leave the
# │      GPU at ~13% utilization, making NVMe mandatory—a concrete Iron Law
# │      application connecting storage physics to GPU economics.
# │
# │ Imports: physx.constants (KB, GB, flop, GFLOPs, TFLOPs),
# │          physx.formatting (fmt)
...
# │          s3_workers_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import KB, GB, flop, GFLOPs, TFLOPs
from physx.formatting import fmt

# --- Inputs (hardware specs and storage tiers) ---
...
# --- Process (throughput ceiling and bandwidth requirement) ---
...
# │
# │ Why: Quantifies the I/O waste of row-oriented formats (CSV) vs.
# │      columnar formats (Parquet) when training uses only a subset of
# │      columns. Shows that switching formats is mathematically equivalent
# │      to buying a 5× faster drive—no hardware change needed.
# │
# │ Imports: physx.formatting (fmt)
...
# --- Inputs (column counts and format efficiency) ---
...
# │
# │ Why: Compares gzip vs. Snappy decompression speed and shows how the
# │      per-epoch time difference compounds over 50 epochs into hours of
# │      wasted wall-clock time. Teaches students that compression ratio is
# │      not the only metric—decompression throughput dominates for
# │      iterative ML training workloads.
# │
# │ Imports: physx.formatting (fmt)
...
Schema debt\index{Schema Contracts!data producers}\index{Data Contracts|see{Schema Contracts}} requires preventive infrastructure rather than reactive fixes. Explicit schema contracts between data producers and consumers, codified through tools like Great Expectations or Pandera, fail fast when schemas drift rather than allowing workarounds to accumulate. The investment in contract enforcement pays dividends by preventing the brittle parsing logic that characterizes mature systems with unmanaged schema evolution.
...
Freshness debt\index{Continuous Retraining!drift-triggered} cannot be addressed through periodic heroics; it requires automated retraining triggers based on drift detection rather than calendar schedules, connecting directly to the PSI and KL divergence monitoring infrastructure established in @sec-data-engineering-ml-detecting-responding-data-drift-509a.
