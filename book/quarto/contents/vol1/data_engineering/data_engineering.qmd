...
- **Windows per Month**: $60 \times 60 \times 24 \times 30$ = `{python} windows_per_month_str` windows.
...
At $0.10/label with 20% review overhead: $90K ÷ $`{python} effective_cost_str` = **`{python} labeled_k`K labeled examples**
...
*   **Maximum Theoretical Throughput**:
    $$ \frac{312 \times 10^{12} \text{ FLOPs/sec}}{12 \times 10^9 \text{ FLOPs/img}} = \mathbf{`{python} max_img_per_sec_str` \text{ images/sec}} $$

**2. The Data Term (Bandwidth Requirement)**

*   **Image Size:** 150 KB (JPEG compressed).
*   **Required Bandwidth**:
    $$ `{python} max_img_per_sec_str` \text{ img/sec} \times 150 \text{ KB/img} \approx \mathbf{`{python} required_bw_gbs_str` \text{ GB/s}} $$

**The Systems Conclusion:**
To saturate a *single* A100, your storage must deliver **`{python} required_bw_gbs_str` GB/s**.
*   **S3 Standard**: ~100 MB/s per thread. You need **`{python} s3_workers` concurrent worker threads**.
