...
class DataEngineeringSetup:
    ...
    # Hardware/Energy
    a100_tflops_fp16 = h_a100.peak_flops.to(TFLOPs/second).magnitude
    a100_tflops_fp16_sparse = (h_a100.peak_flops * 2).to(TFLOPs/second).magnitude
    ...
    gpt3_fp32_gb = m_gpt3.size_in_bytes(BYTES_FP32).to(GB).magnitude
    gpt3_fp16_gb = m_gpt3.size_in_bytes(BYTES_FP16).to(GB).magnitude
...
# --- Inputs (hardware specs and storage tiers from Digital Twins) ---
h_a100 = Hardware.Cloud.A100
m_resnet = Models.Vision.ResNet50

a100_flops_val_value = h_a100.peak_flops.to(flop/second).magnitude
resnet_flops_per_img_value = (m_resnet.inference_flops * 3).to(flop).magnitude # forward (1) + backward (2)
...
a100_flops_t_str = str(int(h_a100.peak_flops.to(TFLOPs/second).magnitude))
