...
- **Precision Requirement**: `{python} FalsePositiveTarget.rejection_pct_str`% rejection of non-keywords.
...
- **Target**: `{python} DataEngineeringSetup.kws_accuracy_target_str`% accuracy, <`{python} DataEngineeringSetup.kws_false_wake_month_str` false wake per month
...
- **Labeling** (~60%): `{python} BudgetAllocation.labeling_k` K available
- **Storage/Processing** (~25%): `{python} BudgetAllocation.storage_k` K
- **Governance/Other** (~15%): `{python} BudgetAllocation.governance_k` K

At USD `{python} BudgetAllocation.cost_per_label_str`/label with `{python} BudgetAllocation.review_overhead_pct_str`% review overhead: `{python} BudgetAllocation.labeling_k` K ÷ `{python} BudgetAllocation.effective_cost_str` = **`{python} BudgetAllocation.labeled_k` K labeled examples**
...
![](images/png/cs249r_labels_new.png){width=90%}
...
This scale makes manual annotation infeasible: `{python} DataEngineeringSetup.kws_dataset_size_m_str` million examples at even 10 seconds per label would require approximately `{python} DataEngineeringSetup.kws_label_hours_str` person-hours, roughly `{python} DataEngineeringSetup.kws_label_years_str` person-years of full-time effort. Achieving `{python} DataEngineeringSetup.kws_accuracy_target_str`% accuracy across diverse environments requires millions of training examples covering acoustic variations (background noises, speaking styles, recording environments), and transparent sourcing across `{python} DataEngineeringSetup.kws_languages_str` languages ensures the technology serves diverse speaker populations.
...
**Iron Law Implication:** If you use SATA SSDs, your maximum throughput is capped at `{python} StorageBandwidth.sata_bw_mbs_str` MB/s / `{python} StorageBandwidth.image_size_kb_str` KB ≈ `{python} StorageBandwidth.sata_max_img_sec_str` img/s. Your USD 15,000 GPU will run at **`{python} StorageBandwidth.sata_utilization_pct_str`% utilization** (`{python} StorageBandwidth.sata_max_img_sec_str`/`{python} StorageBandwidth.max_img_per_sec_str`). Storage physics dictates training speed.
...
- **Row-Oriented (CSV)**: Must read all `{python} FormatEfficiency.total_cols_str` columns to get the `{python} FormatEfficiency.needed_cols_str` needed.
    ...
    - **Result**: You waste `{python} FormatEfficiency.waste_pct_str`% of your disk bandwidth.
