...
3.  **Determine significance**: Compare $\mathcal{D}_{\text{KS}}$ to critical value $\mathcal{D}_{\text{crit}}$ based on training-baseline sample size $n_0$, serving sample size $n_t$, and significance level $(\alpha = 0.05)$.
...
For systems with calibrated model probabilities, predictive entropy\index{Predictive Entropy} provides an additional uncertainty signal. Let $p_i$ represent the model's probability assigned to label category $i$. @Eq-label-entropy defines this measure:
...
Here, $R_{\text{train}}$, $R_{\text{compute}}$, and $R_{\text{data}}$ are measured in samples per second, $B_{\text{storage}}$ in bytes per second, and $S_{\text{sample}}$ in bytes per sample.
