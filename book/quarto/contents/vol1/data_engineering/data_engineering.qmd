...
::: {#fig-spectrogram-example fig-env="figure" fig-pos="t!" fig-cap="**Audio Feature Transformation**: Three stacked panels show how raw audio is compressed into ML-ready features: the raw waveform (top), the spectrogram (middle), and an MFCC coefficient approximation (bottom). These representations emphasize perceptually relevant characteristics, reducing noise and data dimensionality while preserving essential speech information for applications like keyword spotting." fig-alt="Three stacked panels: raw audio waveform (amplitude vs. time), spectrogram (frequency vs. time with power in dB as color), and MFCC approximation (coefficient index vs. time with coefficient value as color)."}
...
::: {#fig-labels fig-env="figure" fig-pos="htb" fig-cap="**Data Annotation Granularity**: Five label types applied to the same dog-on-blanket image, plus an audio transcript example: classification label (Dog, Blanket, No cat), bounding box, segmentation map, free-text caption, and audio transcript. Each type progressively increases labeling cost and storage requirements while providing a richer training signal." fig-alt="Five-row table of label types applied to a dog-on-purple-blanket image: classification label (Dog, Blanket, No cat), bounding box overlay, segmentation map, free-text caption, and a separate row showing an audio transcript example."}
