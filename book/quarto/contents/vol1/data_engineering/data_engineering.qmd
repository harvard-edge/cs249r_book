...
The methodologies examined in @sec-ai-workflow establish the foundations of machine learning development. Every phase of these workflows requires robust data infrastructure. In traditional software, computational logic is defined by code. In machine learning, system behavior is defined by data. This shift makes data a first-class engineering artifact, requiring the same rigor we apply to code.

To surpass the traditional view of data engineering as "ETL plumbing," we must reframe it as **Dataset Compilation**. Just as a compiler transforms human-readable source code into an optimized binary executable, a data pipeline transforms raw, noisy observations into a clean, optimized training set (the binary) that the model (the processor) consumes.
...
Building effective ML systems requires understanding data engineering and implementing a structured framework for principled decisions about data infrastructure. The Five-Pillar Framework introduced in @sec-introduction provides overarching structure for ML systems thinking. Within data engineering specifically, four foundational pillars emerge that translate those high-level principles into concrete infrastructure decisions. Choices about storage formats, ingestion patterns, processing architectures, and governance policies require systematic evaluation rather than ad-hoc selection. This framework organizes data engineering around four foundational pillars that ensure systems achieve functionality, robustness, scalability, and trustworthiness.

### The Four Foundational Pillars {#sec-data-engineering-four-foundational-pillars-19bd}

Every data engineering decision, from choosing storage formats to designing ingestion pipelines, should be evaluated against four foundational principles. @fig-four-pillars illustrates how these pillars interact, with each contributing to system success through systematic decision-making.
...
Data scientists spend 60-80% of their time on data preparation tasks.[^fn-data-quality-stats] @fig-ds-time quantifies this reality, revealing that data cleaning alone consumes up to 60% of practitioners' effort, reflecting a state where data engineering practices are often ad-hoc rather than systematic. Applying the four-pillar framework consistently can reduce data preparation time while building more reliable and maintainable systems.
...
These governance principles work with our technical pillars of quality, reliability, and scalability. A system is not operationally reliable if it violates user privacy. Quality metrics lose their value if they perpetuate unfair outcomes.
...
Data-centric development combines programmatic labeling with strategic human annotation through active learning integration. Active learning identifies examples where model uncertainty is highest or where labeling functions disagree, directing human effort to cases with maximum impact on model improvement.
...
PSI's primary advantage is its invariance to feature scale and its proven track record in financial risk modeling where it originated. The binning approach makes it robust to outliers while remaining computationally efficient for real-time monitoring. For high-cardinality categorical features like product categories or user segments, PSI applies directly without binning, comparing category frequency proportions between training and serving data.
...
Automated retraining triggers model updates when drift exceeds thresholds and retraining pipelines have proven robust. A typical automated retraining policy implements decision logic based on multiple quantitative criteria:
