...
\index{Data Debt!hidden liability}\index{Technical Debt!data systems}Production data systems require ongoing maintenance, much like physical infrastructure. This section addresses *data debt* from engineering shortcuts and *debugging techniques* for diagnosing pipeline failures. Every shortcut—from deferred label corrections to undocumented schema changes—creates technical debt\index{Technical Debt!data engineering} that compounds silently. Unlike code debt, which manifests as slower development velocity, data debt directly degrades model performance and can remain invisible until failures become catastrophic.
