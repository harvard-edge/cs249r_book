...
# Energy (from constants + Horowitz 2014)
...
*   **Hardware Peak:** NVIDIA A100 = `{python} a100_tflops_fp16` TFLOPS (FP16 Tensor Core, dense operations).[^fn-a100-sparsity]
...
[^fn-a100-sparsity]: **A100 Sparsity Support**: NVIDIA also advertises `{python} a100_tflops_fp16_sparse` TFLOPS with structured sparsity enabled (2:4 pattern), doubling effective throughput for models that can leverage sparse computation. The `{python} a100_tflops_fp16` TFLOPS dense figure used here represents the baseline for general workloads where sparsity cannot be assumed.
