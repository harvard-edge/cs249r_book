...
that many images per second times `{python} StorageBandwidth.image_size_kb_str` per image, or approximately `{python} StorageBandwidth.required_bw_gb_per_s_str`.
...
- **Row-oriented (CSV)**: Must read all `{python} FormatEfficiency.total_cols_str` columns to get the `{python} FormatEfficiency.needed_cols_str` needed.
    - The useful-byte fraction is `{python} FormatEfficiency.needed_cols_str` needed columns out of `{python} FormatEfficiency.total_cols_str`, or `{python} FormatEfficiency.eta_csv_str`.
