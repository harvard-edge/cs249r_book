...
batch_core_hours = 33  # 200 cores × 10min/hr = 33 core-hours/day
...
batch_cost_str = f"{batch_cost_day:.2f}"
...
batch_core_hours_str = str(batch_core_hours)
```
...
3.  **Batch Requirements**: Process `{python} hourly_tb_str`TB (1 hour data) in 10 mins. High throughput (sequential I/O) is efficient. You need 200 cores for 10 mins/hour = `{python} batch_core_hours_str` core-hours/day. Cost: `{python} batch_core_hours_str` $\times$ $`{python} stream_cost_per_hr_str` = **$`{python} batch_cost_str`/day**.
