...
class DataGravity:
    ...
    network_gbs_str = fmt_qty(network_bw, GB/second, precision=1, commas=False)
    dataset_gb_str = fmt_qty(dataset, GB, precision=0, commas=True)
    network_gbps_str = fmt_bandwidth(network_bw, unit=Gbps, precision=0, commas=False)
...
class FeedingProblem:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    req_bw_gbs_str = fmt_qty(req_bw, GB/second, precision=1, commas=False)
    disk_bw_mbs_str = fmt_qty(disk_bw, MB/second, precision=0, commas=False)
    feeding_tax_pct_str = fmt_percent(feeding_tax_pct/100, precision=1, commas=False, style='symbol')
    img_per_sec_str = fmt_rate(round(img_per_sec), 'img/s', precision=0, commas=True)
```
...
class EnergyAccessCosts:
    ...
    dram_multiplier_str = fmt_multiple(round(energy_dram_pj / energy_flop_fp32_pj), precision=0, commas=True)
    local_ssd_multiplier_str = fmt_multiple(energy_local_ssd_pj_per_bit/energy_flop_fp32_pj_per_bit, precision=1, commas=True)
    network_multiplier_str = fmt_multiple(energy_network_pj_per_bit/energy_flop_fp32_pj_per_bit, precision=1, commas=True)
```
...
| **DRAM Memory Access (32-bit)** |    `{python} EnergyAccessCosts.energy_dram_str`   |    `{python} EnergyAccessCosts.dram_multiplier_str`$\times$   |
| **Local SSD Access (32-bit)**   | `{python} EnergyAccessCosts.local_ssd_energy_str` | `{python} EnergyAccessCosts.local_ssd_multiplier_str`$\times$ |
| **Network Transfer (32-bit)**   |  `{python} EnergyAccessCosts.network_energy_str`  |  `{python} EnergyAccessCosts.network_multiplier_str`$\times$  |
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_usd, fmt_qty, check, fmt_time
...
class LabelingComputeRatio:
    ...
    ratio_low_str = fmt_multiple(ratio_low, precision=1, commas=True)
    ratio_high_str = fmt_multiple(ratio_high, precision=1, commas=True)
...
The contrast matters: weeks for human labeling, hours for GPU training, milliseconds for serving. Labeling is the bottleneck. It often costs hundreds to more than a thousand times more than a single optimized training run: a `{python} LabelingComputeRatio.labeling_budget_str` labeling budget compares with `{python} LabelingComputeRatio.train_cost_low_str`--`{python} LabelingComputeRatio.train_cost_high_str` for one 8$\times$ A100 ResNet-50 run, a `{python} LabelingComputeRatio.ratio_low_str`$\times$--`{python} LabelingComputeRatio.ratio_high_str`$\times$ ratio. And within that labeling spend, the effort distribution itself is skewed: `{python} LabelingComputeRatio.effort_majority_pct_str` of the work goes to `{python} LabelingComputeRatio.effort_minority_pct_str` of features—the long tail of edge cases, rare categories, and quality exceptions.
...
Preexisting datasets from repositories such as Kaggle, UCI [@uci_repo], and ImageNet\index{ImageNet!benchmark dataset}[^fn-imagenet-data-scale] [@imagenet_website] offer cost-efficient starting points when the deployment distribution resembles the benchmark enough to make reuse meaningful. Their value is speed and comparability, not guaranteed coverage.
...
# │ Exports: RealtimeCost.throughput_gb_s_str, RealtimeCost.hourly_tb_str,
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_usd, fmt, fmt_count, fmt_time, fmt_qty, fmt_qty_int, check, fmt_multiple
...
class RealtimeCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    throughput_gb_s_str = fmt_qty(throughput, GB/second, precision=0, commas=False)
    ...
    cost_ratio_str = fmt_multiple(cost_ratio, precision=0, commas=False)
    events_per_sec_str = fmt_count(events_per_sec, scale='M', commas=False)
...
class EtlEltCost:
    ...
    schema_change_ratio_str = fmt_multiple(schema_change_ratio, precision=0, commas=False)
...
**Systems insight**: ETL saves `{python} EtlEltCost.savings_str` in storage. After normalizing Spark compute to a monthly cost, ETL totals `{python} EtlEltCost.etl_total_usd_str` (`{python} EtlEltCost.etl_spark_monthly_usd_str` compute + `{python} EtlEltCost.etl_storage_usd_str` storage), while ELT totals `{python} EtlEltCost.elt_total_usd_str` (`{python} EtlEltCost.elt_storage_usd_str` storage + `{python} EtlEltCost.elt_query_usd_str` query compute), for a cloud-cost advantage of `{python} EtlEltCost.total_savings_str` before engineering labor. ETL still costs `{python} EtlEltCost.schema_change_ratio_str`$\times$ more engineering time per schema change. The exact break-even point depends on engineering labor cost and schema-change frequency: stable schemas favor ETL's lower cloud cost, while frequent feature-definition changes favor ELT's faster iteration.
...
class SegmentationLabelScale:
    ...
    label_entry_ratio_str = fmt_multiple(label_entry_ratio, precision=0, commas=True)
```
...
class KWSStorageEconomics:
    ...
    load_speedup_str = fmt_multiple(load_speedup, precision=0, commas=False)
```

@Tbl-storage-performance reveals why ML systems employ tiered storage architectures. Consider the economics of storing our KWS training dataset (`{python} KWSStorageEconomics.dataset_size_gb_str`): object storage costs `{python} KWSStorageEconomics.storage_s3_cost_str`, enabling affordable long-term retention of raw audio, while maintaining working datasets on NVMe[^fn-nvme-gpu-utilization] for active training costs `{python} KWSStorageEconomics.storage_nvme_low_cost_str`–`{python} KWSStorageEconomics.storage_nvme_high_cost_str` but provides `{python} KWSStorageEconomics.load_speedup_str`$\times$ faster data loading.
...
class StorageLoading:
    ...
    load_speedup_str = fmt_multiple(load_speedup, precision=0, commas=False)
    nvme_bw_gbs_str = fmt_qty(nvme_bw, GB/second, precision=0, commas=False)
```

The performance difference directly impacts iteration velocity. Training that loads data at `{python} StorageLoading.nvme_bw_gbs_str` completes dataset loading in `{python} StorageLoading.nvme_load_s_str`, compared to `{python} StorageLoading.obj_load_s_str` at typical object storage speeds. This `{python} StorageLoading.load_speedup_str`$\times$ difference determines whether teams can iterate multiple times daily or must wait hours between experiments.
...
# │          ModelStorageScale.model_growth_factor_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.fmt import fmt, fmt_multiple, fmt_qty, check, MarkdownStr
...
# │          StorageBandwidth.a100_flops_tflops_str,
...
from mlsysim.fmt import fmt_int, fmt, fmt_count, check, fmt_qty, fmt_qty_int, fmt_rate, fmt_percent
...
class StorageBandwidth:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    max_img_per_sec_str = fmt_rate(max_img_per_sec, 'img/s', precision=0)
    required_bw_gbs_str = fmt_qty(required_bw, GB/second, precision=1, commas=False)
    a100_flops_tflops_str = fmt_qty_int(h_a100.compute.peak_flops, TFLOP/second, commas=False)
    resnet_flops_g_str = fmt_qty(m_resnet.inference_flops * 3, GFLOPs, precision=1, commas=False)
    image_size_kb_str = fmt_qty(image_size, KB, precision=0, commas=False)
    sata_bw_mbs_str = fmt_qty(sata_bw, MB/second, precision=0, commas=False)
    sata_max_img_sec_str = fmt_rate(sata_max_img_sec, 'img/s', precision=0)
    sata_utilization_pct_str = fmt_percent(sata_utilization_pct/100, precision=0, commas=False, style='symbol')
    s3_bw_mbs_str = fmt_qty(s3_bw, MB/second, precision=0, commas=False)
    s3_workers_str = fmt_count(s3_workers, commas=False, label="concurrent worker thread")
    nvme_bw_low_gbs_str = fmt_qty(nvme_bw_low, GB/second, precision=0, commas=False)
    nvme_bw_high_gbs_str = fmt_qty(nvme_bw_high, GB/second, precision=0, commas=False)
```
...
*   **Hardware peak**: NVIDIA A100 = `{python} StorageBandwidth.a100_flops_tflops_str` (FP16 Tensor Core, dense operations).
...
**Systems insight**: Switching from CSV to Parquet is not just a file change; it is mathematically equivalent to buying a `{python} FormatEfficiency.throughput_ratio_str`$\times$ faster hard drive. @Tbl-serialization-cost quantifies the serialization overhead of different formats. For a deeper treatment of row vs. columnar storage layouts and the algebra of data operations (selection, projection, join), see @sec-appdx-data-foundations-row-vs-columnar-formats-bca2.
...
class CompressionTradeoff:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    decompress_speedup_str = fmt_multiple(decompress_speedup, precision=1, commas=False)
    ...
    snappy_decompress_mbs_str = fmt_qty(snappy_decompress, MB/second, precision=0, commas=False)
    gzip_decompress_mbs_str = fmt_qty(gzip_decompress, MB/second, precision=0, commas=False)
...
class LifecycleStorageRequirements:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    train_img_per_sec_str = fmt_rate(train_img_per_sec, 'img/s', precision=0, commas=True)
    image_size_kb_str = fmt_qty(image_size, KB, precision=0, commas=False)
    training_bw_gbs_str = fmt_qty(training_bw, GB/second, precision=0, commas=False)
    storage_reduction_str = fmt_multiple(storage_reduction, precision=0, commas=False)
...
class DataEngineeringSummaryRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    ratio_low_str = fmt_multiple(round(ratio_low), precision=0, commas=True)
    ratio_high_str = fmt_multiple(round(ratio_high), precision=0, commas=True)
...
* **Labeling costs dominate and require substantial resource allocation**: Labeling can cost hundreds to more than a thousand times one optimized training run; in the reference calculation above, the ratio is `{python} DataEngineeringSummaryRecap.ratio_low_str`$\times$--`{python} DataEngineeringSummaryRecap.ratio_high_str`$\times$. Labeling is the serial bottleneck that parallelization cannot solve.
* **Storage hierarchy determines iteration speed**: The `{python} StorageLoadingRecap.load_speedup_str`$\times$ throughput gap between local NVMe (`{python} StorageLoadingRecap.nvme_bw_gbs_str`) and cloud object storage (100 MB/s) determines whether iterations occur daily or weekly.
