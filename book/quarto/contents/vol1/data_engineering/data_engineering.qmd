...
Allocating our \$150K budget across three cost categories (using the data engineering cost constants that @sec-data-engineering-scalability-cost-optimization-b9b3 establishes):
...
# │          StorageBandwidth.sata_max_bw_mb_per_s_str,
...
The second category, **schema debt**\index{Schema Debt!workarounds}, emerges from accumulated schema workarounds and migrations. When upstream systems change data formats, quick fixes such as string parsing instead of proper type handling or NULL coercion instead of error handling accumulate into fragile transformation logic. Teams can diagnose schema debt by looking for telltale patterns: multiple date format handlers for the same logical field, defensive null checks scattered throughout pipeline code, version-specific parsing branches that grow with each upstream change, and undocumented enum value mappings that break when new values appear. For neural network pipelines, schema drift carries a structural consequence that distinguishes it from ordinary software failures: model input layers have fixed, compiled dimensions. A schema change that adds a new categorical column or introduces a previously unseen embedding ID does not merely produce a wrong answer—it crashes the forward pass because the input tensor dimension no longer matches the first layer's weight matrix. The pipeline jungle example in @sec-data-engineering-four-foundational-pillars-c119 illustrates the upstream failure mode; here the point is that the crash site is inside the model graph, making schema debt a direct cause of training job failures that look like model bugs until the data contract violation is traced back.
