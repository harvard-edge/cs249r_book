...
2.  **Observation**: Unlike Data Science\index{Data Science!vs. data engineering}, which focuses on Inference and Insight, Data Engineering addresses the Scalability and Reliability of the data pipeline.
3.  **Pitfall**: A frequent misconception is that Data Engineering is "data cleaning." In reality, it is Dataset Compilation\index{Dataset Compilation}: transforming raw, noisy observations into an optimized binary that the model consumes.
...
# │
# │ Imports: mlsysim (Systems.Fabrics.Ethernet_100G.bandwidth, Infrastructure.Pricing.Cloud.EgressPerGB.rate,
...
# │
# │ Imports: mlsysim.core.units (SECONDS_PER_MINUTE, MINUTES_PER_HOUR,
...
# │
# │ Imports: mlsysim.core.units (KB), mlsysim.fmt
...
# │
# │ Imports: mlsysim.book (fmt, check), mlsysim.core.units (THOUSAND)
...
# │
# │ Imports: mlsysim.core.units (THOUSAND), mlsysim.fmt (fmt, check)
...
# │
# │ Imports: mlsysim.core.units (LABELING_COST_*, Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate, dollar),
...
# │
# │ Imports: mlsysim.core.units (Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate, THOUSAND, dollar,
...
# │
# │ Imports: mlsysim.core.units (Infrastructure.Pricing.Storage.GlacierRetrievalPerGB.rate, dollar), mlsysim.fmt (fmt, check)
...
# │
# │ Imports: mlsysim (Datasets, count, fmt_count, check)
# │ Exports: KWSAcquisitionScale.dataset_size_million_samples_str,
# │          KWSAcquisitionScale.languages_count_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
For our KWS system, `{python} KWSAcquisitionScale.dataset_size_million_samples_str` spanning `{python} KWSAcquisitionScale.languages_count_str` demand a volume that manual collection cannot economically provide. The multi-source strategy introduced in @sec-data-engineering-framework-application-keyword-spotting-case-study-c8e9, combining curated datasets with web scraping, crowdsourcing, and synthetic generation, addresses this scale requirement while maintaining coverage across acoustic environments and speaker demographics.
...
# │
# │ Imports: mlsysim.core.units (KB, GB, TB, MILLION, SEC_PER_HOUR),
...
2.  **Observation**: Unlike Data Quality, which focuses on the Cleanliness of a single record, the consistency imperative focuses on the Alignment of the entire transformation pipeline.
3.  **Pitfall**: A frequent misconception is that consistency is "fixed" by sharing code. In reality, it is a State Synchronization problem: parameters computed on training data (for example, means, standard deviations) must be stored and reused during serving.
...
- [ ] **Definition**: Skew happens when the code processing data during training differs from the code processing live requests.
- [ ] **Mechanism**: If training normalizes data using `mean=0.5` but serving uses `mean=0.0`, the model sees "alien" data and fails silently.
- [ ] **Invariant**: Do you understand why architectural guarantees (*shared code*, not *copied code*) are the only reliable solution to skew?
...
# │
# │ Imports: mlsysim (Datasets, KB, count, fmt_count, fmt_qty, check)
# │ Exports: KWSScalabilityTargets.dataset_size_million_examples_str,
...
# │
# │ Imports: mlsysim.core.units (THOUSAND, MILLION), mlsysim.fmt (fmt, check)
...
# │
# │ Imports: mlsysim (Datasets, count, second, fmt_count, fmt_time, fmt_percent, check)
# │ Exports: KWSAutomatedLabelingScale.dataset_size_million_examples_str,
# │          KWSAutomatedLabelingScale.dataset_size_million_samples_str,
...
# │          KWSAutomatedLabelingScale.languages_count_str,
# │          KWSAutomatedLabelingScale.keywords_count_str,
...
# │
# │ Imports: mlsysim (Datasets, Infrastructure, Systems, units, formatters)
...
# │
# │ Imports: mlsysim (Datasets, Systems, units, formatters)
...
# │
# │ Imports: mlsysim digital twins, mlsysim.core.units (Bparam, Mparam,
...
# │
# │ Imports: mlsysim.core.units (KB, GB, flop, GFLOPs, TFLOPs),
...
The storage options now have a concrete target: saturating this single A100 requires `{python} StorageBandwidth.required_bw_gb_per_s_str` of sustained bandwidth.
...
The `{python} StorageBandwidth.sata_bw_mb_per_s_str` figure represents SATA III sequential read throughput (the interface maximum is 600 MB/s), and real-world random read performance with small files can be significantly lower. That caveat reinforces the general principle governing data pipelines: training throughput is bounded by the lower of compute capacity and the data supply rate. The bottleneck relation in @eq-training-throughput captures this limit, with the data supply rate defined in @eq-data-supply.
...
# │
# │ Imports: mlsysim.book (fmt, check), mlsysim.core.units (GB, MB)
...
# │
# │ Imports: mlsysim.core.units (KB, GB), mlsysim.book (fmt, check)
...
2.  **Observation**: Unlike a general-purpose database, a feature store is designed for dual storage modes: an *offline store* (columnar/batch) for training and an *online store* (key-value/low-latency) for serving.
3.  **Pitfall**: A frequent misconception is that a feature store is just "a place to store *data*." In reality, it is a transformation engine: it stores the *logic* to compute features consistently across the entire ML lifecycle.
...
2.  **Observation**: Unlike technical debt in code, which manifests as slower development, data debt manifests as lower accuracy even when the code is perfectly maintained.
3.  **Pitfall**: A frequent misconception is that data debt is just "bad data." In reality, it is a systems architecture failure: it occurs when the assumptions of the training distribution are no longer enforced at the system boundary.
...
Teams meticulously optimize accelerator kernels while leaving data loading as JSON or CSV. Text-based formats can be at least 10$\times$ slower to decode than columnar binary formats (Parquet, Arrow), and the CPU, not the accelerator, becomes the bottleneck. The consequence is the worst-case profile of the data-throughput hierarchy: an H100, used here as a concrete high-throughput accelerator anchor, should sustain `{python} DataSerializationPitfall.h100_peak_tflop_per_s_str` on dense low-precision kernels but instead idles at single-digit utilization waiting for the next batch. The fix is structural, not algorithmic: convert the training corpus to Parquet once, version it, and let every downstream job read the columnar form. A one-time conversion pays back within hours of training time.
...
# │
# │ Imports: mlsysim.core.units (Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate, MILLION,
...
# │          DataEngineeringSummaryRecap.kws_dataset_size_million_samples_str,
...
# │
# │ Imports: mlsysim.core.units (MILLION, GB, byte), mlsysim.fmt (fmt, check)
