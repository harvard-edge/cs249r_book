...
class FeedingProblem:
    ...
    req_bw_gbs = req_bw_bytes_sec / GB.m_as('B')
    
    # Efficiency (eta) = Disk_BW / (Required_BW in MB/s)
    eta = min(disk_bw_mbs / (req_bw_bytes_sec / MB.m_as('B')), 1.0)
...
[^fn-soc-always-on]: **System on Chip (SoC)**: An integrated circuit featuring a low-power "always-on" island that continuously monitors sensors while the main processor sleeps to conserve energy. This design directly imposes the memory constraint, as the island's SRAM is severely limited to stay within a sub-milliwatt power budget. Consequently, the entire keyword-spotting model, including weights and all preprocessing code, must fit within this tiny memory footprint, often just tens of kilobytes. \index{SoC!always-on domain}
...
[^fn-p95-latency-monitoring]: **95th Percentile (P95)**: The threshold below which 95% of observed values fall, and the standard SLO metric for ML pipelines because it captures the tail behavior that averages hide. A pipeline averaging 5 seconds may have a P95 of 30 seconds, meaning 1 in 20 batches stalls long enough to cause feature staleness at serving time. This is precisely the scenario described: when P95 exceeds the 10-second SLA, identifying *which* pipeline stage introduced the delay requires per-stage P95 tracking, not just end-to-end averages. \index{P95 Latency!pipeline monitoring}
...
[^fn-tfdv-schema-inference]: **TFDV (TensorFlow Data Validation)**: The "automatically infer" capability matters because manually specifying schemas for hundreds of features is error-prone and never complete. TFDV statistically learns expected types, value ranges, and presence rates from training data, then enforces them as a synchronous gatekeeper at ingestion. A feature present in 100% of training data that drops to 99% in production triggers an alert before the gap propagates into a data cascade -- the exact failure pattern from @fig-cascades that motivates systematic validation over ad hoc checks. \index{TFDV!schema inference}
...
[^fn-covariate-shift-detection]: **Covariate Shift**: Input feature distributions $P(X)$ change while the conditional relationship $P(Y|X)$ holds constant. The engineering advantage is detectability: because only inputs shift, feature distribution monitoring (PSI, K-S tests) catches covariate shift without requiring ground truth labels -- unlike concept drift, which demands labeled serving data for detection. This makes covariate shift the most operationally tractable drift type but also the most common, arising whenever data collection contexts change (new sensors, different user populations, seasonal effects). \index{Covariate Shift!detection}
...
[^fn-concept-drift-retraining]: **Concept Drift**: The core statistical relationship $P(Y|X)$ changes, meaning the model's logic is invalidated even if the input data distribution $P(X)$ appears unchanged. This change is only detectable with new ground truth labels, creating a mandatory delay between when the model breaks and when it can be fixed. For a system with a weekly labeling pipeline, this guarantees at least 7 days of stale, incorrect predictions are served before a retrain can even be triggered. \index{Concept Drift!detection delay}
...
[^fn-cohens-kappa-label-quality]: **Cohen's Kappa ($\kappa$)**: Measures inter-rater agreement while correcting for chance agreement, which raw percentage agreement ignores. The correction matters: two annotators labeling 90% of images as "not spam" will agree 81% of the time by pure chance, making raw agreement a dangerously misleading quality metric. For ML label quality monitoring, $\kappa$ below 0.4 signals that labeling guidelines need clarification or genuine ambiguity exists -- either way, the training data is unreliable and the resulting model will inherit the annotators' confusion. \index{Cohen's Kappa!label quality}
...
[^fn-weak-supervision-cost]: **Weak Supervision**: This method exchanges manual annotation labor for the upfront engineering effort of writing programmatic "labeling functions." The trade-off is justified because each function can then label millions of data points at near-zero marginal cost, whereas manual labeling costs scale linearly with dataset size. \index{Weak Supervision!cost trade-off}
