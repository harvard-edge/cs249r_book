...
The workflow becomes concrete in the data pipeline: raw inputs pass through collection, ingestion, analysis, labeling, validation, and preparation before they become ML-ready datasets. The effort breakdown in @fig-ds-time explains why that pipeline needs its own systems treatment: data work consumes `{python} DataPrepEffortIntro.data_prep_effort_low_str` to `{python} DataPrepEffortIntro.data_prep_effort_high_str` of ML project effort, and the Data axis of the D·A·M taxonomy becomes real only as infrastructure: acquisition systems, validation checks, labeling workflows, storage layouts, and governance.
...
DVC (Data Version Control)\index{DVC!git semantics} provides Git-like semantics for file snapshots, while @lst-delta-time-travel\index{Delta Lake!time travel} demonstrates querying historical data states directly in SQL.
