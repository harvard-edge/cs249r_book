...
::: {.column-margin}
![](images/svg/vol1_introduction_margin_001.svg){width="100%" fig-alt="A small labeled box marked ML code nested inside a much larger surrounding frame labeled production system; the inner box occupies a tiny fraction of the outer area."}
...
The D·A·M taxonomy provides the diagnostic lens, but to build systems, we must organize these axes into a reproducible hierarchy. This is formalized throughout the book as the **engineering crux**\index{Engineering Crux!definition}: a four-layer stack that transforms raw physical constraints into functional user applications.
...
class GPT3Training:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Text strings
    num_gpus_str = fmt(num_gpus, precision=0, commas=False, suffix=" GPUs")
...
# │
# │ Imports: mlsysim Hardware.Tech.Memory.DRAM.energy_per_byte, Hardware.Tech.Op.FlopFp16.energy,
...
::: {.column-margin}
![](images/svg/introduction_energy_hierarchy.svg){width="100%" fig-alt="Vertical ladder of three blue bars on a log scale, longest at top: DRAM 160 pJ towers far above FP16 1.1 pJ and INT8 0.2 pJ, showing data movement costs orders of magnitude more energy than arithmetic."}
...
```{python}
#| echo: false
#| label: introduction-lighthouse-table
# ┌─────────────────────────────────────────────────────────────────────────────
# │ LIGHTHOUSE MODEL REFERENCE TABLE
# ├─────────────────────────────────────────────────────────────────────────────
# │ Goal: Reuse the same workload archetype records that drive later chapters.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Scenarios as BookScenarios
from mlsysim.fmt import MarkdownStr

class IntroductionLighthouseTable:
    """Display fields for the introductory Lighthouse Model table."""
    ...
    resnet_label_str = MarkdownStr(resnet.workload_label)
    resnet_bottleneck_str = MarkdownStr(resnet.bottleneck)
    resnet_role_str = MarkdownStr(resnet.textbook_role)
    gpt2_label_str = MarkdownStr("GPT-2/Llama")
    gpt2_bottleneck_str = MarkdownStr(gpt2.bottleneck)
    gpt2_role_str = MarkdownStr(gpt2.textbook_role)
    dlrm_label_str = MarkdownStr(dlrm.workload_label)
    dlrm_bottleneck_str = MarkdownStr(dlrm.bottleneck)
    dlrm_role_str = MarkdownStr(dlrm.textbook_role)
    mobilenet_label_str = MarkdownStr("MobileNetV2")
    mobilenet_bottleneck_str = MarkdownStr(mobilenet.bottleneck)
    mobilenet_role_str = MarkdownStr(mobilenet.textbook_role)
    kws_label_str = MarkdownStr("Keyword Spotting")
    kws_bottleneck_str = MarkdownStr(kws.bottleneck)
    kws_role_str = MarkdownStr(kws.textbook_role)
```

| **Lighthouse Model**                                                                            | **System Bottleneck**                                           | **What It Reveals**                                       | **Key Engineering Questions**                  |
|:------------------------------------------------------------------------------------------------|:----------------------------------------------------------------|:----------------------------------------------------------|:-----------------------------------------------|
| **`{python} IntroductionLighthouseTable.resnet_label_str`**                                     | `{python} IntroductionLighthouseTable.resnet_bottleneck_str`    | `{python} IntroductionLighthouseTable.resnet_role_str`    | Is my hardware doing math or waiting for data? |
| **`{python} IntroductionLighthouseTable.gpt2_label_str`**                                       | `{python} IntroductionLighthouseTable.gpt2_bottleneck_str`      | `{python} IntroductionLighthouseTable.gpt2_role_str`      | How fast can I move model weights to compute?  |
| **`{python} IntroductionLighthouseTable.dlrm_label_str`**                                       | `{python} IntroductionLighthouseTable.dlrm_bottleneck_str`      | `{python} IntroductionLighthouseTable.dlrm_role_str`      | How do I fit terabyte-scale models in memory?  |
| **`{python} IntroductionLighthouseTable.mobilenet_label_str`**\index{MobileNetV2!edge latency}  | `{python} IntroductionLighthouseTable.mobilenet_bottleneck_str` | `{python} IntroductionLighthouseTable.mobilenet_role_str` | Can I meet real-time constraints on battery?   |
| **`{python} IntroductionLighthouseTable.kws_label_str`**\index{Keyword Spotting!power envelope} | `{python} IntroductionLighthouseTable.kws_bottleneck_str`       | `{python} IntroductionLighthouseTable.kws_role_str`       | Can I run always-on inference on milliwatts?   |
...
training_compute_data = [
    {"Model": e["model"], "Year": e["year"], "FLOPs": e["flops"], "Era": e["era"]}
    for e in Literature.ComputeTrend.entries
]
...
# │ How: Pull mission metadata from Scenarios.Archetypes.Missions and executable
# │      constraints from mlsysim.Applications.
# │
# │ Imports: mlsysim.Applications, mlsysim.Scenarios, mlsysim.fmt
# │ Exports: ScenarioRegistry.frontier_limit_str, ScenarioRegistry.drive_limit_str,
# │          ScenarioRegistry.mobile_limit_str, ScenarioRegistry.doorbell_limit_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Applications, Scenarios as BookScenarios
...
class ScenarioRegistry:
    ...
    # ┌── 1. LOAD (Canonical mission records + executable scenario twins) ───
    mission_frontier = BookScenarios.Archetypes.Missions.FrontierTraining
    mission_drive = BookScenarios.Archetypes.Missions.AutonomousPerception
    mission_mobile = BookScenarios.Archetypes.Missions.MobileAssistant
    mission_doorbell = BookScenarios.Archetypes.Missions.SmartDoorbell

    app_frontier = Applications.Frontier
    app_drive = Applications.AutoDrive
    app_mobile = Applications.Mobile
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(app_frontier.workload is mission_frontier.workload, "Frontier mission must use the executable scenario workload.")
    check(app_drive.workload is mission_drive.workload, "Autonomous mission must use the executable scenario workload.")
    check(app_mobile.workload is mission_mobile.workload, "Mobile mission must use the executable scenario workload.")
    check(app_doorbell.workload is mission_doorbell.workload, "Doorbell mission must use the executable scenario workload.")
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_mission_str = MarkdownStr(mission_frontier.mission)
    frontier_system_str = MarkdownStr(mission_frontier.system_archetype)
    frontier_workload_str = MarkdownStr(mission_frontier.workload_label)
    frontier_limit_str = MarkdownStr(
        f"Target: {fmt(frontier_latency_ms, precision=0, commas=False, suffix=' ms/step')}"
    )
    drive_mission_str = MarkdownStr(mission_drive.mission)
    drive_system_str = MarkdownStr(mission_drive.system_archetype)
    drive_workload_str = MarkdownStr(mission_drive.workload_label)
    drive_limit_str = MarkdownStr(
        f"SLA: {fmt(drive_latency_ms, precision=0, commas=False, suffix=' ms latency')}"
    )
    mobile_mission_str = MarkdownStr(mission_mobile.mission)
    mobile_system_str = MarkdownStr(mission_mobile.system_archetype)
    mobile_workload_str = MarkdownStr(mission_mobile.workload_label)
    mobile_limit_str = MarkdownStr(mission_mobile.critical_constraint)
    doorbell_mission_str = MarkdownStr(mission_doorbell.mission)
    doorbell_system_str = MarkdownStr(mission_doorbell.system_archetype)
    doorbell_workload_str = MarkdownStr(mission_doorbell.workload_label)
...
|:--------------------------------------------------------------------------------------------------|:------------------------------------------------|:--------------------------------------------------|:-----------------------------------------------|
| **`{python} ScenarioRegistry.frontier_mission_str`**\index{Frontier Training!cloud cluster}       | `{python} ScenarioRegistry.frontier_system_str` | `{python} ScenarioRegistry.frontier_workload_str` | `{python} ScenarioRegistry.frontier_limit_str` |
| **`{python} ScenarioRegistry.drive_mission_str`**\index{Autonomous Perception!edge robotics}      | `{python} ScenarioRegistry.drive_system_str`    | `{python} ScenarioRegistry.drive_workload_str`    | `{python} ScenarioRegistry.drive_limit_str`    |
| **`{python} ScenarioRegistry.mobile_mission_str`**\index{Mobile Assistant!smartphone constraints} | `{python} ScenarioRegistry.mobile_system_str`   | `{python} ScenarioRegistry.mobile_workload_str`   | `{python} ScenarioRegistry.mobile_limit_str`   |
| **`{python} ScenarioRegistry.doorbell_mission_str`**                                              | `{python} ScenarioRegistry.doorbell_system_str` | `{python} ScenarioRegistry.doorbell_workload_str` | `{python} ScenarioRegistry.doorbell_limit_str` |
...
This volume covers the **single-node**\index{Single-node regime} regime: 1–8 accelerators connected by shared memory, where the binding constraint is the memory wall (the rate at which data moves from HBM to compute units). Volume II extends these principles to the **distributed fleet**\index{Distributed fleet}\index{Memory Wall!bandwidth ceiling} regime, where thousands of nodes coordinate across network fabrics and the bottleneck shifts to bisection bandwidth between nodes. For detailed guidance on reading paths, learning outcomes, prerequisites, and how to get the most from this textbook, the front-of-book About This Book chapter provides the orientation.
