...
[^fn-thermal-power-constraints]: **Thermal and Power Constraints**: Heat dissipation and power delivery limit how much compute can be packed into a chip, a server, or a rack. Modern accelerators often operate in the hundreds-of-watts range per device, so large-scale training clusters can require megawatt-scale facility power and cooling capacity. These constraints influence hardware design, data center architecture, and system-level trade-offs, motivating efficiency techniques discussed in @sec-ai-acceleration and @sec-efficient-ai.
