...
::: {.callout-tip title="Dimensional Analysis: Proving the Iron Law" icon="true"}
Engineering requires dimensional homogeneity. We verify the Iron Law by proving that every term resolves to **Time (seconds)**:
...
*   **Data Term**: $\frac{\text{Bytes}}{\text{Bytes} / \text{s}} = \text{Bytes} \times \frac{\text{s}}{\text{Bytes}} = \mathbf{s}$
*   **Compute Term**: $\frac{\text{FLOPs}}{\text{FLOPs} / \text{s}} = \text{FLOPs} \times \frac{\text{s}}{\text{FLOPs}} = \mathbf{s}$

The equation is physically consistent. Any optimization you perform must respect these dimensions; you cannot trade "FLOPs" for "Bandwidth" without converting through Time.
:::
