...
gpt3_training_zflops_value = round(GPT3_TRAINING_OPS.to(ZFLOPs).magnitude)
...
Deep learning effectively traded the **Feature Engineering Bottleneck** for a new **Compute Bottleneck**\index{Compute Bottleneck}. Models like GPT-3\index{GPT-3!training scale} [@brown2020language] (`{python} gpt3_params_billion_str` parameters) illustrate the scale of this new challenge. Training required `{python} gpt3_training_zflops_str` zettaFLOPs of compute (where 1 zettaFLOP = $10^{21}$ floating-point operations), consumed approximately 500 billion tokens (~`{python} data_scale_gb_str` GB) from filtered web text, books, and Wikipedia, and demanded `{python} gpt3_gpus_str` GPUs running for weeks. The primary engineering challenge shifted from "*how* do we describe a cat's ear?" to "*how* do we coordinate `{python} gpt3_gpus_str` GPUs without failure?"
...
# │
# │ Why: Applies the Iron Law to estimate GPT-3 training time on an A100
# │ cluster, then shows how improving software efficiency (η) from 45% to
# │ 60% saves significant compute days — grounding the equation in practice.
# │
# │ Imports: physx.constants (GPT3_TRAINING_OPS, A100_FLOPS_FP16_TENSOR,
# │          TFLOPs, second, day),
# │          physx.formulas (dTime), physx.formatting (fmt, md_math)
...
# --- Outputs (LaTeX math strings for callout) ---
...
# │
# │ Why: Plots efficiency gains (44×) from AlexNet to EfficientNet to show
# │ that algorithmic innovation delivers substantial compute savings
# │ independent of hardware improvements.
# │
# │ Imports: sys, os, matplotlib.pyplot, numpy, physx.viz
...
# │
# │ Why: Plots training compute (FLOPs) across the Deep Learning and
# │ Transformer eras to show the 10^8 growth that far outpaces Moore's
# │ Law, motivating the need for specialized ML infrastructure.
# │
# │ Imports: sys, os, matplotlib.pyplot, numpy, physx.viz
# │ Exports: (figure output)
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Why: Converts Waymo's per-hour sensor data volume from physx constants
# │ into formatted strings for the prose discussing real-world data scale.
# │
# │ Imports: physx.constants (WAYMO_DATA_PER_HOUR_LOW, WAYMO_DATA_PER_HOUR_HIGH,
# │          TB, hour), physx.formatting (fmt)
...
# │          system interactions"
# │
# │ Why: Demonstrates Amdahl's Law applied to an ML inference pipeline where
# │ a 3× speedup on one component yields far less end-to-end improvement,
# │ illustrating why systems thinking matters.
# │
# │ Imports: physx.formulas (calc_amdahls_speedup), physx.formatting (fmt)
...
# │          indefinitely"
# │
# │ Why: Applies the Degradation Equation to show how a recommendation system
# │ loses accuracy over months due to data drift, motivating continuous
# │ monitoring and retraining as operational requirements.
# │
