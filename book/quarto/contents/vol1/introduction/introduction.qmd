...
- Explain D·A·M and use it to diagnose ML system bottlenecks
- Explain AI's shift from symbolic reasoning to deep learning and the bitter lesson
- Distinguish ML systems from traditional software by silent degradation, data dependencies, and continuous iteration
- Apply the iron law to decompose performance into data movement, computation, and overhead
- Describe efficiency dimensions and how Lighthouse Models stress-test bottlenecks
- Compare the ML lifecycle with traditional software and deployment contexts from cloud to TinyML
- Apply the degradation equation and five-pillar framework to organize ML systems engineering
...
The shift from rule-based to data-driven systems constitutes a deep reconception of computing. Andrej Karpathy[^fn-karpathy-sw2] formalized this distinction as the shift from **Software 1.0**\index{Software 1.0} to **Software 2.0**\index{Software 2.0} [@karpathy2017software], a framing that captures *why* ML systems require entirely new engineering approaches. @Tbl-software-1-vs-2 summarizes this paradigm shift.
...
Google researchers quantified the resulting technical debt\index{Technical Debt} in a landmark 2015 paper.
...
In Go, DeepMind's AlphaGo\index{AlphaGo}[^fn-alphago-scale] [@silver2016] achieved superhuman performance by combining supervised learning from expert games with reinforcement learning through self-play and neural-network-guided tree search, rather than relying on hand-coded Go strategy.
...
1.  **Significance (quantitative)**: Every performance budget traces back to three physical costs: bytes moved, arithmetic work performed, and fixed latency overhead. In a production recommendation system serving 10 million requests per day, reducing the bytes moved per request cuts total data movement proportionally, while upgrading the processor only helps the computation portion of the request. The binding constraint must be identified before any optimization investment yields returns.
2.  **Observation**: Unlike traditional software, whose correctness degrades only when code changes, an ML system's accuracy degrades when the world changes. Model weights are fixed after deployment, but the distribution of inputs relative to what the model learned shifts continuously, eroding accuracy silently without any error or exception.
3.  **Pitfall**: A frequent misconception is that an ML system is the model. Google's analysis of technical debt in production ML systems used a schematic where the model code is a small central box, roughly 5 percent of the diagram, surrounded by much larger support infrastructure; data pipelines, serving infrastructure, monitoring, and other support code often dominate the engineering burden [@sculley2015hidden].

:::

This definition motivates the **Data · Algorithm · Machine (D·A·M) taxonomy**\index{D·A·M Taxonomy}, which we now formalize as a diagnostic tool: when performance stalls or behavior degrades, ask *"Which axis is the bottleneck?"*
...
2.  **Observation**: Unlike traditional software performance analysis, which treats code and data as separate concerns, the D·A·M taxonomy recognizes that algorithm choice directly determines both the training dataset size required (a transformer needs orders of magnitude more data than a linear model to generalize) and the machine required to run it.
3.  **Pitfall**: A frequent misconception is that the three axes are independent. Changing from a simple classifier to a larger model can require more memory, different serving infrastructure, and a broader data distribution. The axes move together.
...
Every machine learning system analyzed in this text is constructed from four hierarchical layers. This engineering crux\index{Engineering Crux!hierarchy} transforms raw physical constraints into functional user applications, ensuring that a decision made at the silicon level is traceable to its impact on the final mission.

1.  **Hardware (The Silicon)**: The physical foundation (The Engine). This layer defines the raw capabilities: $R_{\text{peak}}$, $\text{BW}$, and memory capacity; concrete hardware twins instantiate those quantities when deployment scenarios need numeric constraints.
...
This hierarchy ensures that when we build a lab or a case study, engineers are not starting from scratch, but rather inheriting the constraints of a deployment paradigm and applying a scenario workload to a specific mission. @Tbl-introduction-engineering-missions makes this concrete by pairing each recurring mission with its workload and binding constraint. This structured approach allows us to reason about the "Physics of ML" across any application domain.
...
We retain "iron law" because, like Amdahl's Law\index{Amdahl's Law!diagnostic analogy}, its value lies in diagnostic power: identifying which physical constraint dominates before optimizing. The iron law is useful precisely because it simplifies the complexity of the full stack into three manageable terms. @Sec-appdx-dam-taxonomy presents the refined treatment, including pipelining and overlap techniques that transform the additive model into the max-based formulation used in practice.
...
class GPT3Training:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Text strings
    accelerator_name_str = MarkdownStr("A100-class")
...
*   **Scale $(N_{\text{accel}})$**: `{python} GPT3Training.num_accelerators_str` (`{python} GPT3Training.accelerator_name_str` accelerators)
...
2.  **Observation**: Unlike machine learning research, which targets a single objective (validation loss) on a static dataset, AI Engineering targets a multi-objective constraint surface (latency, throughput, accuracy, cost, fairness, and robustness) on a distribution that shifts continuously after deployment.
3.  **Pitfall**: A frequent misconception is that AI Engineering is just "Software Engineering for ML." The critical difference is that the system specification is probabilistic: an ML system's output is statistically valid or invalid relative to a shifting distribution, not correct or incorrect relative to a fixed deterministic contract. This makes continuous monitoring a structural requirement, not an operational choice.
...
A traditional software project follows a well-understood arc: design, implement, test, deploy, maintain. An ML project follows a different arc shaped by data-dependent behavior, silent degradation, and production feedback loops that can force the system back into data collection and training after deployment. The lifecycle matters because the engineering object is no longer only code; it is code, data, model behavior, deployment context, and monitoring evidence evolving together.
...
[^fn-latency-responsiveness]: **Latency**\index{Latency!etymology}\index{Latency!edge deployment}: From Latin *latere* ("to lie hidden"), a fitting etymology because delay is invisible until it causes failure. Autonomous braking requires <10 ms end-to-end; at highway speeds, every additional millisecond adds roughly 3 cm of stopping distance. This makes $L_{\text{lat}}$ rather than throughput the binding constraint for edge deployment, and explains why latency-critical systems *cannot* offload inference to distant cloud servers regardless of their superior compute.
...
A common fallacy is this: *Better algorithms automatically produce better systems.*
...
A recurring pitfall is this: *Treating ML systems as traditional software that happens to include a model.*
...
A common fallacy is this: *High accuracy on benchmark datasets indicates production readiness.*
...
A recurring pitfall is this: *Optimizing individual components without considering system interactions.*
...
A common fallacy is this: *ML systems can be deployed once and left to run indefinitely.*
...
A recurring pitfall is this: *Assuming that ML expertise alone is sufficient for ML systems engineering.*
...
This book makes a stronger claim: ML systems engineering is a distinct engineering discipline because every design sits under constraints no software layer can repeal. The iron law decomposes inference into data movement, computation, and overhead. The degradation equation explains why deployed models drift as the live world separates from the training distribution. The memory wall explains why faster arithmetic cannot compensate when bytes cannot reach the processor quickly enough. The same constraints govern data pipelines, neural computation, architectures, frameworks, training, optimization, serving, operations, and responsible engineering. The shared vocabulary is physical rather than intuitive.
...
* **Five pillars require integration**: ML systems engineering encompasses Data Engineering, Training Systems, Deployment Infrastructure, Operations and Monitoring, and Ethics and Governance. Missing one pillar creates blind spots in data quality, training cost, serving performance, monitoring, or governance that later appear as expensive production failures.
