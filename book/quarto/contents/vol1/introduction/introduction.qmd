...
To motivate these objectives, we begin with the concrete reality of modern AI deployments: the scale, the latency constraints, and the dual mandate that forces statistical and physical reasoning into the same design loop.
...
The D·A·M taxonomy tells us what an ML system is made of; the next question is how those systems fail, and why their failure modes diverge from traditional software.
...
Because this failure mode is silent, we cannot rely on crash logs to detect it. We must rely on math. When failures do not announce themselves, we need quantitative signals that connect measurable distribution shift to expected performance loss. Just as Patterson and Hennessy's Iron Law [@patterson2021hardware] decomposed CPU performance into fundamental components, we can decompose ML system degradation into its constituent factors. The **Degradation Equation**\index{Degradation Equation!drift measurement} in @eq-degradation captures how model performance evolves over time:
