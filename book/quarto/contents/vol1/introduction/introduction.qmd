...
**Why this feels different from traditional engineering:**

In classic Systems Engineering (Operating Systems, Computer Architecture, Networks), success is defined by **Determinism**. We build machines where the same input always yields the same output. The goal is to isolate and eliminate variance.
...
This shift effectively traded the **Feature Engineering Bottleneck** for a new **Compute Bottleneck**. Models like GPT-3 [@brown2020language] (`{python} gpt3_params_billion` parameters) required:
...
To ground the abstract interdependencies of the Iron Law in concrete practice, this textbook employs five recurring **Lighthouse Archetypes**. We do not use these models merely as examples. They are **The Cast of Characters**, our **Systems Detectives**: canonical workloads that we will use in every chapter to "interrogate" the Silicon Contract.
