...
AI's evolution reveals a progression of bottlenecks, each overcome by systems innovations that expanded what was computationally possible. The field traces its origin to Turing's[^fn-turing-outputs] paper "Computing Machinery and Intelligence" [@turing1950computing], which posed the foundational question: *Can machines think?* Early systems that attempted to answer this question, such as the Perceptron (1957) and ELIZA[^fn-eliza-brittleness] (1966) [@weizenbaum1966eliza], ran into the limits of manual logic and mainframe-era hardware, resulting in brittleness. Subsequent eras hit the knowledge acquisition bottleneck: manual knowledge entry could not scale. Modern systems face a different constraint: computational throughput.
...
3.  **Common Pitfall:** A frequent misconception is that an ML system is the model. Google's 2015 audit of production ML systems found that model code comprises approximately five percent of total lines; the other 95 percent is data pipelines, serving infrastructure, and monitoring [@sculley2015hidden].
