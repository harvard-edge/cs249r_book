...
class AIMomentStats:
    ...
    accelerator_fp16_tflop_per_s_str = fmt_qty(accelerator_tflops, TFLOPs / second, precision=0, commas=False)
...
# │ How: Compare an illustrative 25,000-accelerator, 90-day run with the mlsysim
# │      GPT-4-class accelerator-day reference anchor.
# │
...
# │ Exports: GPT4ScaleEstimate.accelerator_count_str, training_days_str, accelerator_days_m_str
# └─────────────────────────────────────────────────────────────────────────────
...
class GPT4ScaleEstimate:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    accelerator_count = Models.Language.GPT4.training_accelerators_ref.to(count).magnitude
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    illustrative_accelerator_days = accelerator_count * training_days
    illustrative_accelerator_days_m = illustrative_accelerator_days / MILLION
    accelerator_days_m = reference_gpu_days / MILLION
    estimate_ratio = illustrative_accelerator_days / reference_gpu_days

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(accelerator_days_m > 1.0, "GPT-4-class public estimate should be million-accelerator-day scale.")
    check(0.8 <= estimate_ratio <= 1.1, "Illustrative 25k accelerator × 90 day estimate should be near the mlsysim anchor.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    accelerator_count_str = fmt(accelerator_count, precision=0, commas=True)
    training_days_str = fmt_time(training_days, "day", precision=0, commas=False, style="word")
    accelerator_days_m_str = fmt_int(accelerator_days_m, commas=False)
```

@Tbl-ai-evolution-performance provides quantitative validation of this principle. Examine the rightmost column carefully: as computational resources grew from rule evaluation to CPU-era feature pipelines, multi-GPU training, and eventually large-scale distributed training, performance improved from amateur-level to superhuman. GPT-4's exact training configuration was not disclosed, but common public estimates place GPT-4-class runs in the same regime as `{python} GPT4ScaleEstimate.accelerator_count_str` data-center accelerators for roughly `{python} GPT4ScaleEstimate.training_days_str`. The mlsysim reference anchor is about `{python} GPT4ScaleEstimate.accelerator_days_m_str` million accelerator-days, so this value is a scale anchor rather than an official benchmark result.
...
| **Modern Deep Learning (2023)**  | Foundation models              | MMLU benchmark                | 86.4 percent (GPT-4)                                                 | Not disclosed; public-estimate regime of about `{python} GPT4ScaleEstimate.accelerator_days_m_str`M accelerator-days (same scale as `{python} GPT4ScaleEstimate.accelerator_count_str` accelerators for `{python} GPT4ScaleEstimate.training_days_str`) |
...
1.  **Significance (diagnostic)**: The diagnostic power is concrete even before detailed hardware arithmetic enters the story. If the spam filter misses a new phishing campaign because the training set never contained that tactic, the binding axis is Data. If the training examples are adequate but the model cannot express the pattern, the binding axis is Algorithm. If both are adequate but the service cannot classify messages quickly enough during a traffic spike, the binding axis is Machine. Later roofline analysis makes that Machine diagnosis quantitative: small-batch inference below the ridge point is memory-bound, so buying peak FLOP/s alone does not help until data reuse, bandwidth, or utilization improves.
...
# │ Exports: GPT4TrainingCostRecap.accelerator_days_m_str
# └─────────────────────────────────────────────────────────────────────────────
