...
[^fn-latency-responsiveness]: [offset=-20mm] **Latency**\index{Latency!etymology}\index{Latency!edge deployment}: From Latin *latere* ("to lie hidden"), a fitting etymology because delay is invisible until it causes failure. Autonomous braking requires <10 ms end-to-end; at highway speeds, every additional millisecond adds roughly 3 cm of stopping distance. This makes $L_{\text{lat}}$ rather than throughput the binding constraint for edge deployment, and explains why latency-critical systems *cannot* offload inference to distant cloud servers regardless of their superior compute.
