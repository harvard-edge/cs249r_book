...
AI represents the broad goal of creating systems that perform tasks requiring human-like intelligence: recognizing images, understanding language, making decisions, and solving problems. It is the vision of intelligent machines that learn, reason, and adapt.

Machine Learning (ML) provides the methodological approach for creating systems that demonstrate intelligent behavior. Rather than implementing intelligence through predetermined rules, ML offers computational techniques that automatically discover patterns in data through mathematical processes, transforming AI's theoretical insights into functioning systems. These two ideas, AI as vision and ML as methodology, form the *core concepts* that underpin every system discussed in this book.
...
Understanding this paradigm shift is essential before tracing the historical evolution that produced it. Take a moment to verify your grasp of these foundational concepts.
...
\textbf{Dartmouth Workshop} A formative conference organized by AI pioneer John McCarthy [@mccarthy1956dartmouth].};
...
The practical implication: **knowing when to retrain is as important as knowing how to train.** A system that retrains when $D(P_t \| P_0) > \tau$ for some threshold $\tau$ maintains accuracy within bounds. A system without drift monitoring operates blind to its own degradation.
...
The magnitude of efficiency improvements is measurable. Between 2012 and 2019, computational resources needed to train a neural network to achieve AlexNet-level performance on ImageNet classification decreased by approximately $44\times$ [@Hernandez_et_al_2020]. This improvement, which halved every 16 months, outpaced hardware efficiency gains predicted by Moore's Law[^fn-moores-law], demonstrating that algorithmic innovation drives efficiency as much as hardware advances.
...
@fig-comp_efficiency shows the countervailing trend: the exponential growth in compute demand that makes efficiency optimization essential rather than optional.
...
Taken together, these two figures reveal a seeming contradiction that defines the economics of modern AI development.
...
**Transparency and Privacy** requirements constrain design. Many deep networks function as "black boxes," yet in domains like healthcare or finance, stakeholders require interpretability. Systems must also be resilient against inference attacks[^fn-inference] that attempt to extract sensitive training data from model predictions.
...
The five-pillar framework emerged from the systems challenges that distinguish ML from traditional software. Each pillar addresses specific challenge categories while recognizing their interdependencies.
