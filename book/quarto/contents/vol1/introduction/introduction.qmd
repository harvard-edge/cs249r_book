...
AI's evolution reveals a progression of bottlenecks, each overcome by systems innovations that expanded what was computationally possible. The field traces its origin to Turing's[^fn-turing-outputs] paper "Computing Machinery and Intelligence" [@turing1950computing], which posed the foundational question: *Can machines think?* Early systems that attempted to answer this question, such as the Perceptron (1957) and ELIZA[^fn-eliza-brittleness] [@weizenbaum1966eliza], ran into the limits of manual logic and mainframe-era hardware, resulting in brittleness. Subsequent eras hit the knowledge acquisition bottleneck: manual knowledge entry could not scale. Modern systems face a different constraint: computational throughput.
...
In the 1980s, engineers pivoted from general logic to capturing deep domain expertise. MYCIN\index{MYCIN!expert system}, designed to diagnose blood infections, encoded approximately six hundred rules derived from interviews with medical experts [@shortliffe1976mycin].
...
[^fn-unreasonable-data]: **Unreasonable Effectiveness of Data**: The principle that a simple statistical model fed with massive amounts of data often outperforms a more sophisticated model with less data [@halevy2009unreasonable]. This validated the pivot from brittle, hand-crafted expert systems to probabilistic models by showing that engineering investment in data scaling yielded more accuracy than investment in algorithmic complexity alone. For language tasks of that era, increasing a training dataset by $10\times$ often reduced error rates more than switching to a completely new, more complex algorithm. \index{Unreasonable Effectiveness of Data!systems impact}
...
[^fn-federated-learning-locality]: **Federated Learning**: A distributed training paradigm where models learn from data residing on decentralized devices without centralizing the raw information [@li2020federated]. This approach transforms the constraint of data locality into a privacy feature, but introduces new systems challenges: gradient aggregation across unreliable networks, non-IID data distributions across devices, and communication $\text{BW}$ that often dominates total training time. \index{Federated Learning!definition}
...
- **AlphaFold**\index{Deployment!compute-intensive cloud deployment, scientific computing}\index{AlphaFold!cloud deployment} [@jumper2021highly] is DeepMind's protein structure prediction system that solved a 50-year grand challenge in biology. AlphaFold represents the *compute-intensive cloud* deployment pattern: training required 128 TPUv3 cores for weeks, accessing the entire Protein Data Bank to predict structures for over 200 million proteins.
