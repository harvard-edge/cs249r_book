...
class AlexNetBreakthrough:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    alexnet_relative_improvement_str = fmt_percent(relative_improvement/100, precision=1, style='symbol')
    alexnet_params_m_str = fmt_count(
        model.parameters.m_as('param'), scale="M", precision=0, scale_style="word"
    )
```
...
class GPT3Scale:
    ...
    gpt3_params_billion_str = fmt_count(
        model.parameters.m_as(param),
        scale="B",
        precision=0,
        commas=False,
        scale_style="word",
...
class GPT3TrainingEnergy:
    ...
    energy_q = energy_mwh * ureg.megawatt_hour
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    energy_mwh_str = fmt_qty(energy_q, ureg.megawatt_hour, precision=0, commas=True)
    households_str = fmt_int(households, commas=False)
    household_mwh_year_str = fmt_qty(household_mwh_year * ureg.megawatt_hour, ureg.megawatt_hour, precision=1, commas=False, per="household-year")
```
...
class ResNet50DamExample:
    ...
    memory_time_us_str = fmt_time(p.latency_memory.m_as(US), 'microsecond', precision=1, commas=False)
    ...
    compute_time_us_str = fmt_time(p.latency_compute.m_as(US), 'microsecond', precision=1, commas=False)
    a100_ridge_str = fmt_qty(round(a100_ridge) * flop / byte, flop / byte, precision=0, commas=False, unit_label="FLOP/byte")
    workload_ai_str = fmt_qty(round(workload_ai) * flop / byte, flop / byte, precision=0, commas=False, unit_label="FLOP/byte")
```
...
from mlsysim.fmt import fmt, fmt_int, check, fmt_math, fmt_qty, fmt_qty_int
import math
...
class DeploymentSystems:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_mem_str = fmt_qty_int(t_cloud.ram, GB, commas=False)
    cloud_compute_str = fmt_qty(h_cloud.compute.peak_flops, TFLOP/second, precision=0, commas=True)
    cloud_power_str = fmt_qty(h_cloud.tdp, watt, precision=0, commas=True)

    edge_mem_str = fmt_qty_int(t_edge.ram, GB, commas=False)
    edge_compute_str = fmt_qty(h_edge.compute.peak_flops, TFLOP/second, precision=0, commas=True)
    edge_power_str = fmt_qty(h_edge.tdp, watt, precision=0, commas=True)

    mobile_mem_str = fmt_qty_int(t_mobile.ram, GB, commas=False)
    mobile_compute_str = fmt_qty(h_mobile.compute.peak_flops, TOPS, precision=0, commas=False)
    mobile_power_str = fmt_qty(h_mobile.tdp, watt, precision=0, commas=True)

    tiny_mem_str = fmt_qty(t_tiny.ram, KB, precision=1, commas=False)
    tiny_compute_str = fmt_qty(h_tiny.compute.peak_flops, TFLOP/second, precision=4, commas=True)
    tiny_power_str = fmt_qty(h_tiny.tdp, milliwatt, precision=0, commas=True)
...
from mlsysim.fmt import fmt_int,  fmt, fmt_math, check, fmt_qty
...
class GPT3Training:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Text strings
    num_gpus_str = fmt_count(num_gpus, commas=False, label="GPU")
    eta_base_pct_str = fmt_percent(eta_base, precision=0, commas=False, style='symbol')
    eta_opt_pct_str = fmt_percent(eta_opt, precision=0, commas=False, style='symbol')
    days_initial_str = fmt_time(round(days_base), "day", precision=0, commas=False, style="word")
    days_optimized_str = fmt_time(round(days_opt), "day", precision=0, commas=False, style="word")
    days_saved_str = fmt_time(round(days_saved), "day", precision=0, commas=False, style="word")

    # LaTeX math components
    _ops_mag = f"{ops:.2e}"
    _ops_coeff = _ops_mag.split("e+")[0]
    ops_exp_value = int(_ops_mag.split("e+")[1])
    peak_tflops_str = fmt_qty(peak_flops_per_device, TFLOPs/second, precision=0, commas=False, unit_label="TFLOP/s")
...
class EfficiencyGainsMetrics:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    algo_efficiency_max_str = fmt_multiple(algo_efficiency_max, precision=1)
    algo_halving_months_str = fmt_time(round(algo_halving_months), "month", precision=0, commas=False, style="word")
    moores_window_speedup_str = fmt_multiple(moores_window_speedup, precision=1, commas=False)
    growth_gap_ratio_str = fmt_multiple(growth_gap_ratio, precision=1, commas=False)
```
...
from mlsysim.fmt import fmt, fmt_time, MarkdownStr, check
...
class ScenarioRegistry:
    ...
    frontier_limit_str = MarkdownStr(
        f"Target: {fmt_time(frontier_latency_ms, 'millisecond', precision=0, commas=False, per='step')}"
    )
    drive_limit_str = MarkdownStr(
        f"SLA: {fmt_time(drive_latency_ms, 'millisecond', precision=0, commas=False)} latency"
    )
    doorbell_limit_str = MarkdownStr(
        f"Power: {fmt(doorbell_power_mw, precision=0)} mW"
        if doorbell_power_mw is not None
        else f"SLA: {fmt_time(doorbell_latency_ms, 'millisecond', precision=0, commas=False)} latency"
    )
```
...
class AmdahlsPitfall:
    ...
    t_inference_str = fmt_time(t_inference, 'millisecond', precision=0, commas=False)
    t_inf_new_str = fmt_time(t_inf_new, 'millisecond', precision=0, commas=False)
    t_pre_str = fmt_time(t_pre, 'millisecond', precision=0, commas=False)
    t_post_str = fmt_time(t_post, 'millisecond', precision=0, commas=False)
    ...
    improv_pct_str = fmt_percent(round(improvement_pct)/100, precision=0, commas=False, style='symbol')
    naive_p_str = fmt_percent(round(naive_pct)/100, precision=0, commas=False, style='symbol')
```
...
class DriftFallacy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    acc_initial_str = fmt_percent(acc_initial/100, precision=0, commas=False, style='symbol')
    acc_final_str = fmt_percent(acc_final/100, precision=1, commas=False, style='symbol')
