...
class AlexNetBreakthrough:
    ...
    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if relative_improvement < 40:
        raise ValueError(f"AlexNet improvement should be ~42%, got {relative_improvement:.1f}%")
...
class GPT3Scale:
    ...
    # Data scale in GB
    data_gb = (tokens_b * 1e9 * avg_token_bytes * byte).to(GB).magnitude

    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if params_b != 175:
        raise ValueError(f"GPT-3 should be 175B params, got {params_b}")
...
class GPT4Scale:
    ...
    # ┌── 2. CALCULATION (The Physics) ─────────────────────────────────────────
    gpu_days_m = model.training_gpu_days / 1e6

    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if gpu_days_m < 1.0:
        raise ValueError(f"GPT-4 scale should be >1M GPU days, got {gpu_days_m:.1f}M")
