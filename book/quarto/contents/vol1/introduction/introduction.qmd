...
@Tbl-ai-evolution-performance compares representative systems across eras while keeping their task-specific metrics separate. The rightmost column shows the systems pattern: computational resources grew from rule evaluation to CPU-era feature pipelines, multi-GPU training, and eventually large-scale distributed training. GPT-4's exact training configuration was not disclosed, so the table uses an illustrative `mlsysim` reference anchor rather than an official training disclosure: `{python} GPT4ScaleEstimate.a100_gpu_days_million_str`, the same order of magnitude as `{python} GPT4ScaleEstimate.a100_class_gpu_count_str` reference GPUs for roughly `{python} GPT4ScaleEstimate.training_days_str` [@semianalysisGPT4].
