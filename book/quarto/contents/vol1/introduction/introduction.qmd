...
[^fn-petabyte-scale]: **Petabyte Scale Data**: One petabyte equals 1,000 terabytes, or roughly one million gigabytes. At this scale, data movement becomes as important as storage: even at 10 GB/s sustained throughput, scanning 1 PB takes on the order of a day; at 100 GB/s, it still takes hours. ML systems that operate at petabyte scale therefore require robust storage, parallel processing, and data validation pipelines. We develop the engineering methods for these pipelines in @sec-data-engineering.
...
[^fn-memory-bandwidth]: **Memory Bandwidth**: The rate at which data can be transferred between memory and processors. Many ML workloads are constrained by moving model parameters and activations rather than by arithmetic throughput. A useful anchor: 70B parameters in FP16 is about 140 GB of weights, so a full read of weights takes on the order of 0.1 seconds even at 1 TB/s bandwidth, before accounting for activations and intermediate traffic. This constraint motivates high-bandwidth memory and on-chip caching in accelerators. We develop quantitative analysis of memory bandwidth and arithmetic intensity in @sec-ai-acceleration.
