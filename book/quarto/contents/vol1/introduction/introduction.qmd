...
#### The Equation of System Balance {#sec-intro-balance-equation}

To design an ML system, we must satisfy the **Equation of System Balance**. This fundamental identity dictates that the total time ($T$) to execute a workload is bounded by the slowest resource in the chain:

$$ T_{\text{total}} = \max\left( \frac{\text{Ops}}{\text{Throughput}_{\text{peak}}}, \frac{\text{Bytes}_{\text{memory}}}{\text{Bandwidth}_{\text{memory}}}, \frac{\text{Bytes}_{\text{I/O}}}{\text{Bandwidth}_{\text{I/O}}} \right) $$

where:
*   **Ops**: Total floating-point operations (determined by the algorithm).
*   **Bytes**: Total data movement (determined by precision and dataset scale).
*   **Bandwidth**: The physical limits of the silicon and interconnects.

An "Efficient" system is one where these terms are **balanced**. If your compute term is $10\text{ms}$ but your memory term is $100\text{ms}$, your expensive processor is idle $90\%$ of the time. This book is the study of how to align these terms through hardware acceleration, model compression, and pipeline engineering.
...
**ELIZA (1966)**: A therapist chatbot using pattern matching rules:
```
IF input contains "I feel X" → respond "Why do you feel X?"
IF input contains "mother" → respond "Tell me more about your family."
```

**Modern LLMs (2020s)**: Learn language patterns from billions of text examples, generating contextually appropriate responses without explicit rules.

Same goal (conversational AI), fundamentally different approaches.
:::
...
Optimal performance requires coordinated optimization across multiple dimensions. No single resource (model parameters, training data, or compute budget) can be scaled indefinitely to achieve efficiency. Modern techniques demonstrate the potential: $10$-$100\times$ gains in algorithmic efficiency through optimized architectures, $5$-$50\times$ improvements in hardware utilization through specialized processors, and $10$-$1000\times$ reductions in data requirements through advanced learning methods.
...
## Algorithmic Efficiency {#sec-introduction-achieving-algorithmic-efficiency-ef15}

Algorithmic efficiency achieves maximum performance per unit of computation through optimized model architectures and training procedures. Modern techniques achieve $10$-$100\times$ improvements in computational requirements while maintaining or improving accuracy, providing the most direct path to practical AI deployment.
...
[^fn-efficientnet]: **EfficientNet**: Google Brain's NAS-discovered architecture (2019) using compound scaling that uniformly scales depth, width, and resolution. EfficientNet-B7 achieves 84.4% ImageNet accuracy with $8.4\times$ fewer parameters than GPipe. The scaling coefficients (φ for depth=$1.2^\phi$, width=$1.1^\phi$, resolution=$1.15^\phi$) provide a principled approach to model sizing.

[^fn-squeezenet]: **SqueezeNet**: Compact CNN architecture (2016) demonstrating that $50\times$ parameter reduction is achievable without accuracy loss [@Iandola_et_al_2016]. Uses "fire modules" with squeeze ($1\times 1$ convolutions reducing channels) and expand (mixed $1\times 1$ and $3\times 3$) layers. The 0.5 MB compressed model fits in on-chip SRAM, eliminating memory bandwidth bottlenecks for embedded inference.
...
NAS-discovered architectures demonstrate remarkable efficiency. EfficientNet achieves $5$-$10\times$ better accuracy/FLOPs ratios than manually designed counterparts, matching ResNet-50's accuracy with approximately $5\times$ fewer parameters [@tan2019mnasnet]. These gains emerge from systematic exploration of design spaces that human designers might overlook.
...
[^fn-param-efficient]: **Parameter-Efficient Fine-tuning (PEFT)**: Techniques updating $<1$% of parameters while matching full fine-tuning performance. LoRA (Low-Rank Adaptation) adds trainable rank decomposition matrices to frozen weights; Adapters insert small bottleneck layers. Fine-tuning LLaMA-65B drops from 130 GB (full) to 1–5 GB (LoRA), enabling consumer GPU adaptation of billion-parameter models.

The practical impact is transformative. Fine-tuning GPT-3 traditionally requires storing gradients for 175 billion parameters, consuming over 700 GB of GPU memory. LoRA reduces this to under 10 GB by learning low-rank decompositions of weight updates [@hu2021lora], enabling efficient adaptation on single consumer GPUs while requiring only hundreds of examples rather than thousands for effective adaptation.

Between 2012 and 2019, computational resources needed to train a neural network to achieve AlexNet[^fn-efficient-alexnet]-level performance on ImageNet[^fn-efficient-imagenet] classification decreased by approximately $44\times$. This improvement, which halved every 16 months, outpaced hardware efficiency gains predicted by Moore's Law[^fn-efficient-moores-law], demonstrating the role of algorithmic advancements in driving efficiency [@Hernandez_et_al_2020]. @fig-algo-efficiency traces this progression across major architectural innovations.
...
[^fn-efficient-moores-law]: **Moore's Law**: Intel co-founder Gordon Moore's 1965 observation that transistor density doubles every $\sim 2$ years. Traditional Moore's Law predicted $\sim 2\times$ transistor density every 18–24 months, though this rate has slowed significantly since $\sim 2015$, while AI algorithmic efficiency improved $44\times$ in 7 years (2012–2019).
...
\end{tikzpicture}
```
: **Algorithmic Efficiency Progress**: Neural network training compute requirements decreased $44\times$ between 2012 and 2019, outpacing hardware improvements and demonstrating the significant impact of algorithmic advancements on model efficiency. Innovations in model architecture and optimization techniques can drive substantial gains in AI system sustainability via this halving of compute every 16 months. Source: [@Hernandez_et_al_2020].
:::
...
[^fn-efficient-model-parallelism]: **Model Parallelism**: Distributing model components across multiple processors due to memory constraints. GPT-3 (175B parameters) requires 350 GB memory, exceeding A100's 40 GB capacity by $9\times$, necessitating tensor parallelism where each transformer layer splits across 8–16 GPUs with all-gather communication for activation synchronization.
...
Real-world efficiency optimization demonstrates practical impact across deployment contexts. Production systems routinely achieve $5$-$10\times$ efficiency gains through coordinated application of optimization techniques while maintaining 95%+ of original model performance.

Mobile applications achieve $4$-$7\times$ model size reduction and $3$-$5\times$ latency improvements through combined quantization, pruning, and distillation, enabling real-time inference on mid-range devices. Modern mobile AI systems distribute workloads across specialized processors (NPU for ultra-low power inference, GPU for parallel compute, CPU for control logic) based on power, performance, and real-time constraints.
...
Cloud serving infrastructure reduces costs by 70–80% through systematic optimization combining dynamic batching, quantization, and knowledge distillation, serving $4$-$5\times$ more requests at comparable quality levels.
