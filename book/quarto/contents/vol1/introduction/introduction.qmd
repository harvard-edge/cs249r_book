...
#### The Principle of System Balance {#sec-intro-balance-equation}

This data-centric paradigm introduces a fundamental systems principle: **performance is limited by the slowest component in the chain**. An ML system's execution time depends on three factors—computational throughput (how fast the processor computes), memory bandwidth (how fast data moves to the processor), and I/O bandwidth (how fast data loads from storage). Like a convoy that travels at the speed of its slowest vehicle, an ML system performs only as fast as its most constrained resource.

This principle has profound implications. If your processor completes computations in 10ms but waits 100ms for data to arrive from memory, that expensive processor sits idle 90% of the time. Many ML workloads, particularly the large language models powering systems like ChatGPT, are *memory-bound* rather than *compute-bound*—their performance is limited by how fast data can be fed to processors, not by how fast those processors can compute. @sec-ai-acceleration develops the quantitative tools to analyze these bottlenecks, introducing the **Roofline Model** for systematically reasoning about whether a workload is compute-bound or memory-bound. @sec-benchmarking-ai establishes methodologies for measuring system performance against these fundamental bounds.
