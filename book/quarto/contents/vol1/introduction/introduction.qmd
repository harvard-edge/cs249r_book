...
class GPT3TrainingEnergy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    energy_mwh_str = fmt_qty(energy, MWh, precision=0, commas=True)
...
from mlsysim.fmt import fmt_qty, fmt_time, MarkdownStr, check
...
class ScenarioRegistry:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_limit_value = fmt_time(frontier_latency_ms, 'millisecond', precision=0, commas=False, per='step')
    drive_limit_value = fmt_time(drive_latency_ms, 'millisecond', precision=0, commas=False)
    doorbell_power_value = (
        fmt_qty(doorbell_power_mw * milliwatt, milliwatt, precision=0, commas=False)
        if doorbell_power_mw is not None
        else None
    )
    doorbell_latency_value = fmt_time(doorbell_latency_ms, 'millisecond', precision=0, commas=False)
    frontier_limit_str = MarkdownStr(f"Target: {frontier_limit_value}")
    drive_limit_str = MarkdownStr(f"SLA: {drive_limit_value} latency")
    doorbell_limit_str = MarkdownStr(
        f"Power: {doorbell_power_value}"
        if doorbell_power_value is not None
        else f"SLA: {doorbell_latency_value} latency"
    )
```
