...
class AlexNetEvolutionTable:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    alexnet_top5_accuracy_str = fmt_percent(alexnet_top5_accuracy / 100, precision=1, commas=False, style='prose')
```
...
class ScenarioRegistry:
    ...
    doorbell_power_value = fmt_power(doorbell_power, unit=milliwatt, precision=0, commas=False) if doorbell_power is not None else None
    ...
    doorbell_limit_str = MarkdownStr(
        f"Power: {doorbell_power_value}" if doorbell_power_value is not None else f"SLA: {doorbell_latency_value} latency"
    )
```
