...
This shift from rule-based to data-driven systems represents more than a methodological change—it constitutes a fundamental paradigm shift in how we think about computing itself.
...
This distinction has profound implications. Debugging an ML system means debugging the *data*, not the Python scripts. Version control must track *datasets*, not just git commits. Testing must validate data distributions, not just code paths.
...
This data-centric paradigm requires rethinking the entire computing stack—from how we design specialized AI accelerators to how we manage the massive memory bandwidth demands of modern foundation models. It constitutes an engineering revolution.
...
With this quantitative foundation established, we can describe how this textbook organizes around three imperatives that address these engineering challenges. First, we must *build* the components of ML systems from data pipelines through model architectures, establishing the infrastructure and workflows that enable machine learning. Second, we must *optimize* these systems for efficiency, performance, and deployment constraints, ensuring they can operate effectively under real-world resource limitations. Third, we must *operate* them reliably in production environments, maintaining performance and adapting to changing conditions over time.

These imperatives establish the theoretical and practical foundations of ML systems engineering as a distinct academic discipline. With this engineering perspective in mind, we can now formally define what constitutes a machine learning system.

## Defining ML Systems {#sec-introduction-defining-ml-systems-bf7d}

To understand how ML principles manifest in reality, we must define what constitutes a **machine learning system**. Rather than beginning with an abstract definition, consider a system you likely interact with daily.
...
+:=====================+===================:+===============:+:=========================+:=======================+:======================+
| **ResNet-50**        | 25M                | ~100 MB        | High (compute-bound)     | Throughput             | Cloud / Edge          |
+----------------------+--------------------+----------------+--------------------------+------------------------+-----------------------+
| **GPT-2/Llama**      | 1.5B--70B          | 6 GB--140 GB   | Medium (memory-bound)    | Memory bandwidth       | Cloud                 |
+----------------------+--------------------+----------------+--------------------------+------------------------+-----------------------+
| **MobileNet**        | 3--4M              | ~14 MB         | Medium                   | Latency, power         | Mobile                |
+----------------------+--------------------+----------------+--------------------------+------------------------+-----------------------+
| **DLRM**             | 100B+ (embeddings) | TB-scale       | Very low (memory-bound)  | Memory capacity        | Cloud (distributed)   |
+----------------------+--------------------+----------------+--------------------------+------------------------+-----------------------+
| **Keyword Spotting** | &lt;100K           | &lt;500 KB     | Low                      | Power, memory          | TinyML / MCU          |
+----------------------+--------------------+----------------+--------------------------+------------------------+-----------------------+
...
The AI Triad framework reveals what ML systems comprise: data that guides behavior, algorithms that extract patterns, and infrastructure that enables learning and inference. However, understanding these components alone does not capture how ML systems engineering differs from traditional software engineering. The critical distinction lies in how these systems fail.
...
This operational reality establishes why ML systems developed in research settings require specialized engineering practices to reach production deployment. The unique lifecycle and monitoring requirements that ML systems demand stem directly from this failure characteristic, establishing the core motivation for ML systems engineering as a distinct discipline.

Understanding how ML systems fail differently raises an important question about strategy. Given the three components of the AI Triad—data, algorithms, and infrastructure—which should we prioritize to advance AI capabilities? Should we invest in better algorithms, larger datasets, or more powerful computing infrastructure? To answer this, we trace seven decades of AI history through four distinct paradigms: Symbolic AI, Expert Systems, Statistical Learning, and Deep Learning. Each era was defined by the bottleneck it encountered, and progress came from systems innovations that overcame those constraints.
...
This transition defines an emerging discipline. We formally define it as follows:
...
This formal definition raises a strategic question: given the three components of the AI Triad—data, algorithms, and infrastructure—which should we prioritize to advance AI capabilities? Should organizations invest in better algorithms, larger datasets, or more powerful computing infrastructure? The answer emerges from examining seven decades of AI history, which reveal a consistent and perhaps surprising pattern: each paradigm transition was enabled by systems innovations that overcame the bottleneck of the previous era. This insight has been crystallized into a guiding principle by one of AI's leading researchers.
...
With these efficiency principles and historical context established, we can now examine how modern ML systems operate in practice—their lifecycle characteristics and deployment patterns that distinguish them fundamentally from traditional software.

## Understanding ML System Lifecycle and Deployment {#sec-introduction-understanding-ml-system-lifecycle-deployment-0ab0}

The evolution from symbolic reasoning to data-driven learning changed not only what we build but how we build and operate AI systems. Modern ML systems operate in practice through specialized lifecycles and deployment patterns. Understanding this landscape is important because these factors shape every engineering decision we make.
