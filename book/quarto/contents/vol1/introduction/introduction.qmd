...
google_search_b = f"{GOOGLE_SEARCHES_PER_DAY / 1e9:.1f}"
gpt4_gpu_days = f"{GPT4_TRAINING_GPU_DAYS:,}"
```
...
+----------------------------------+--------------------------------+-------------------------+---------------------+--------------------------------+
| **Modern Deep Learning (2023)**  | Foundation models              | MMLU benchmark          | 86.4% (GPT-4)       | Estimated `{python} gpt4_gpu_days` A100 GPU-days |
+----------------------------------+--------------------------------+-------------------------+---------------------+--------------------------------+

: **AI Performance Evolution Across Paradigms**: Each paradigm transition correlates with increased computational scale rather than algorithmic sophistication. Performance improved from amateur-level expert systems (2000 Elo) to superhuman foundation models (86.4% MMLU), while computational requirements grew from single CPUs to `{python} gpt4_gpu_days` A100 GPU-days. Training time initially increased (hours to days) but later decreased as distributed systems enabled parallelization. {#tbl-ai-evolution-performance}
...
ops_sci = f"{GPT3_TRAINING_OPS.magnitude:.2e}"
...
*   **Ops ($O$)**: $\approx `{python} ops_sci` \text{ FLOPs}$ (from paper).
*   **Peak ($P$)**: `{python} peak_tflops` TFLOPS (A100 FP16 tensor core peak).
*   **Efficiency ($\eta$)**: $\approx `{python} int(efficiency_eta*100)`\%$ (typical for large-scale distributed training).
*   **Scale ($N$)**: `{python} num_gpus` GPUs.

**2. The Calculation**:
$$ \text{Time} \approx \frac{O}{N \cdot P \cdot \eta} $$
$$ \text{Time} \approx \frac{`{python} ops_sci`}{`{python} num_gpus` \times (`{python} peak_tflops` \times 10^{12}) \times `{python} efficiency_eta`} $$
$$ \text{Time} \approx `{python} days_initial` \text{ days} $$

**3. The Result**:
**`{python} days_initial` days**.

**The Systems Insight**: If we improve software efficiency ($\eta$) from `{python} int(efficiency_eta*100)`% to `{python} int(target_eta*100)`% through kernel fusion and better scheduling, training time drops to **`{python} days_optimized` days**, saving nearly `{python} days_saved` days of expensive compute time.
:::
...
The following plot visualizes this **Systems Gap**: the divergence between what models demand and what hardware naturally provides.

```{python}
#| label: fig-systems-gap
#| echo: false
#| fig-cap: "**The Systems Gap.** Relative Compute (Log Scale) vs. Year. The red line shows the demand from frontier models (growing ~10x/year). The blue line shows hardware supply (Moore's Law + Huang's Law, growing ~2x/2 years). The shaded purple region represents the **Systems Gap**: the massive deficit that cannot be closed by better chips alone. This gap must be filled by **Systems Engineering**: parallelization, compiler optimization, and algorithmic efficiency."
...
These measurements emerge from rigorous empirical methodology that tracked training compute across hundreds of published models; @sec-benchmarking-ai develops the measurement frameworks that enable such systematic analysis of ML system performance.
...
Together, @fig-algo-efficiency and @fig-comp_efficiency reveal a paradox central to ML systems engineering: **algorithmic efficiency improved 44× while compute usage grew 300,000×**. Efficiency gains enabled larger experiments, which demanded more compute, which motivated further efficiency research. This feedback loop, where efficiency enables scale and scale demands efficiency, defines the modern AI engineering landscape. Understanding this dynamic is essential for making informed decisions about where to invest optimization effort.
:::
...
**Data Quality and Heterogeneity** present the first hurdle. Real-world data is often noisy and inconsistent. Waymo's autonomous vehicles serve as roving data centers, processing between `{python} int(WAYMO_DATA_PER_HOUR_LOW.to(TB/hour).magnitude)` and `{python} int(WAYMO_DATA_PER_HOUR_HIGH.to(TB/hour).magnitude)` terabytes of data per hour across their sensor suite, including LiDAR[^fn-lidar], radar[^fn-radar], and cameras. Engineers must solve for sensor interference, such as rain obscuring cameras, and temporal misalignment across asynchronous data streams.
