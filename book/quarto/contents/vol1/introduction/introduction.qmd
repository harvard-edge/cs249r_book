...
Google Flu Trends\index{Google Flu Trends!proxy drift} (GFT) estimated influenza activity from patterns in aggregated search queries [@ginsberg2009detecting; @lazer2014]. Its attraction came from the speed and scale at which searches arrived relative to clinical reports. If people searched for flu symptoms when they became ill, query volume appeared to offer a timely proxy for infection.

But the proxy was not stable. News coverage changed what people searched for, and autocomplete changed how they expressed those searches. Query volume began to measure public attention and product behavior as well as illness. During the 2012--2013 season, GFT estimated roughly twice the CDC-reported proportion of doctor visits for influenza-like illness and overestimated for 100 out of 108 weeks.

The remedy was not simply a larger search dataset or a better-tuned model. Researchers combined search signals with CDC sentinel clinical data and recalibrated the relationship over time. The systems lesson is that data volume is not ground truth. A behavioral proxy needs a feedback loop to a trusted measurement and continual evidence that the proxy still represents the quantity the system claims to estimate.
...
### The prelearning era and its logic and knowledge bottlenecks {#sec-introduction-prelearning-era-logic-knowledge-bottlenecks-f312}
...
Sutton's bitter lesson explains why ML systems engineering matters. If AI progress depends on scaling computation effectively, practitioners must understand how to build, deploy, and maintain the systems that make that scale possible. That work spans data management, algorithm design, and infrastructure, so no single technical domain is sufficient. The next step is to define the engineering object that joins those domains, beginning with a familiar production workload.
...
The classifier is therefore only one component of a changing data, software, and infrastructure system. That observation gives us the object this book studies.
...
The definition exposes three diagnostic variables: the data that describes the task, the algorithm that turns those examples into behavior, and the machine that executes that behavior within the operating budget. The D·A·M taxonomy gives those variables a reusable form.
...
|:--------------|:-------------------------------------|:----------------------------------------------|
| **Data**      | Information that guides behavior     | The fuel for what the system learns           |
| **Algorithm** | Mathematical structures that learn   | The blueprint for how patterns are captured   |
| **Machine**   | Hardware and software infrastructure | The engine for computation speed and location |
...
The D·A·M taxonomy provides the diagnostic lens, but building systems requires organizing its axes into a reproducible four-layer hierarchy that transforms raw physical constraints into functional user applications.
...
### Return on compute (RoC) as an economic invariant {#sec-introduction-roc-invariant}
