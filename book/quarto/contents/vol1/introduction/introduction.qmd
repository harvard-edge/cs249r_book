...
# │
# │ Imports: mlsysim.fmt (fmt_percent, check)
...
class AlexNetEvolutionTable:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    alexnet_top5_accuracy_str = fmt_percent(
        alexnet_top5_accuracy / 100, precision=1, commas=False, style='prose'
    )
```
...
# │          compute_time_us_str, a100_ridge_flops_per_byte_str, workload_ai_flops_per_byte_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models, Hardware, Engine
from mlsysim.fmt import fmt_int, fmt, fmt_arithmetic_intensity, fmt_qty, check, fmt_time
...
class ResNet50DamExample:
    ...
    workload_ai_flops_per_byte_str = fmt_arithmetic_intensity(workload_ai, commas=False)
```
...
# │ Exports: DeploymentSystems.cloud_mem_gb_str, DeploymentSystems.cloud_compute_tflops_str,
# │          DeploymentSystems.cloud_power_w_str, DeploymentSystems.edge_mem_gb_str,
# │          DeploymentSystems.edge_compute_tflops_str, DeploymentSystems.edge_power_w_str,
# │          DeploymentSystems.mobile_mem_gb_str, DeploymentSystems.mobile_compute_str,
# │          DeploymentSystems.mobile_power_w_str,
# │          DeploymentSystems.tiny_mem_kb_str, DeploymentSystems.tiny_compute_tflops_str,
# │          DeploymentSystems.tiny_power_mw_str,
...
class DeploymentSystems:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_mem_gb_str = fmt_qty_int(t_cloud.ram, GB, commas=False)
    cloud_compute_tflops_str = fmt_qty(h_cloud.compute.peak_flops, TFLOP / second, precision=0, commas=True)
    cloud_power_w_str = fmt_power(h_cloud.tdp, unit=watt, precision=0, commas=True)

    edge_mem_gb_str = fmt_qty_int(t_edge.ram, GB, commas=False)
    edge_compute_tflops_str = fmt_qty(h_edge.compute.peak_flops, TFLOP / second, precision=0, commas=True)
    edge_power_w_str = fmt_power(h_edge.tdp, unit=watt, precision=0, commas=True)

    mobile_mem_gb_str = fmt_qty_int(t_mobile.ram, GB, commas=False)
    mobile_compute_str = fmt_qty(h_mobile.compute.peak_flops, TOPS, precision=0, commas=False)
    mobile_power_w_str = fmt_power(h_mobile.tdp, unit=watt, precision=0, commas=True)

    tiny_mem_kb_str = fmt_memory(t_tiny.ram, unit=KB, precision=1, commas=False)
    tiny_compute_tflops_str = fmt_qty(h_tiny.compute.peak_flops, TFLOP / second, precision=4, commas=True)
    tiny_power_mw_str = fmt_power(h_tiny.tdp, unit=milliwatt, precision=0, commas=True)
...
|:--------------|:-------------------|:----------------------------------------------:|:-----------------------------------------------------:|:-----------------------------------------------:|
| **Cloud**     | H100 GPU           | `{python} DeploymentSystems.cloud_mem_gb_str`  | `{python} DeploymentSystems.cloud_compute_tflops_str` |  `{python} DeploymentSystems.cloud_power_w_str` |
| **Edge**      | Jetson Robotics    |  `{python} DeploymentSystems.edge_mem_gb_str`  |  `{python} DeploymentSystems.edge_compute_tflops_str` |  `{python} DeploymentSystems.edge_power_w_str`  |
| **Mobile**    | Smartphone         | `{python} DeploymentSystems.mobile_mem_gb_str` |    `{python} DeploymentSystems.mobile_compute_str`    | `{python} DeploymentSystems.mobile_power_w_str` |
| **TinyML**    | ESP32-S3           |  `{python} DeploymentSystems.tiny_mem_kb_str`  |  `{python} DeploymentSystems.tiny_compute_tflops_str` |  `{python} DeploymentSystems.tiny_power_mw_str` |
...
from mlsysim.fmt import fmt_int, fmt, fmt_flop_rate, fmt_math, check, fmt_qty, fmt_count, fmt_percent, fmt_time
...
class GPT3Training:
    ...
    peak_tflops_str = fmt_flop_rate(peak_flops_rate, unit=TFLOPs / second, precision=0, commas=False)
...
# │          mlsysim.fmt (fmt, check)
# │ Exports: EnergyMovementRatios.fp16_ratio_mult_str, int8_ratio_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │ Exports: EfficiencyGainsMetrics.algo_efficiency_max_mult_str,
# │          EfficiencyGainsMetrics.algo_halving_months_str,
# │          EfficiencyGainsMetrics.moores_window_speedup_mult_str,
# │          EfficiencyGainsMetrics.growth_gap_ratio_mult_str
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt_int, fmt, fmt_qty, check
...
# │
# │ Imports: mlsysim.fmt (fmt, check)
# │ Exports: EfficiencyAlgoFigureCaption.algo_efficiency_max_mult_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
# │
# │ Imports: sys, os, matplotlib.pyplot, numpy, book.tools.figures.style
...
sys.path.insert(0, ".")
from book.tools.figures import style as book_style
fig, ax, COLORS, plt = book_style.setup_plot(figsize=(10, 6))
...
# │
# │ Imports: math (standard library), mlsysim.fmt (fmt, check)
# │ Exports: EfficiencyParadoxBridge.algo_efficiency_max_mult_str,
...
- **Waymo**\index{Deployment!high-stakes hybrid deployment, autonomous vehicles}[^fn-waymo-hybrid] binds on safety-critical latency and data freshness. Alphabet's autonomous vehicle division operates a fleet of self-driving taxis that must make safety-critical decisions in real time. Waymo represents the *high-stakes hybrid* deployment pattern: on-vehicle perception models run at the edge with <10 ms latency requirements, while massive cloud infrastructure supports training on petabytes of driving data.
...
- **FarmBeats**\index{Deployment!resource-constrained edge, precision agriculture}[^fn-farmbeats-edge] binds on connectivity and model freshness. Microsoft's precision agriculture platform deploys ML models to farms with limited connectivity. FarmBeats represents the *resource-constrained edge* deployment pattern: models under 500 KB run inference on low-power devices using TV white-space bandwidth measured in kilobits per second.
...
The **Data Engineering**\index{Data Engineering!pillar definition} pillar (@sec-data-engineering) owns the failure modes where data quality or distribution shift determines system behavior. It encompasses building robust data pipelines that ensure quality, handle massive scale, maintain privacy, and provide the infrastructure upon which all ML systems depend. For systems like Waymo, this means managing terabytes of sensor data per vehicle, validating data quality in real time, detecting distribution shifts across different cities and weather conditions, and maintaining data lineage for debugging and compliance. The techniques include data versioning, quality monitoring, drift detection algorithms, and privacy-preserving data processing.

The **Training Systems**\index{Training Systems!pillar definition} pillar (@sec-model-training) owns the model-complexity and scale problems created by the bitter lesson. It covers developing training systems that can manage large datasets and complex models while optimizing computational resource utilization across distributed environments. Modern foundation models require coordinating thousands of GPUs, implementing parallelization strategies, managing training failures and restarts, and balancing training costs against model quality. The techniques include distributed training architectures, optimization algorithms, hyperparameter tuning at scale, and the frameworks that make large-scale training practical.

The **Deployment Infrastructure**\index{Deployment Infrastructure!pillar definition} pillar owns the training-serving divide and the operational complexity of placing models under real latency, cost, and device constraints. It encompasses measuring and optimizing inference performance across deployment tiers, from cloud to edge devices. @Sec-benchmarking covers inference metrics, latency analysis, and the MLPerf scenarios that characterize different deployment contexts. @Sec-ml-operations covers A/B testing, staged rollouts, and operational playbooks for production systems.

The **Operations and Monitoring**\index{Operations and Monitoring!pillar definition}\index{MLOps} pillar owns the silent performance degradation patterns distinctive to ML systems. It covers creating monitoring and maintenance systems that ensure continued performance, enable early issue detection, and support safe system updates in production. Unlike traditional software monitoring focused on infrastructure metrics, ML operations requires four-dimensional monitoring: infrastructure health, model performance, data quality, and business impact. The techniques include metrics design, alerting strategies, incident response procedures, debugging techniques for production ML systems, and continuous evaluation approaches that catch degradation before it affects users.

The **Ethics and Governance**\index{Ethics and Governance!pillar definition}\index{Responsible AI} pillar (@sec-responsible-engineering) owns the ethical and societal constraints around fairness, transparency, privacy, and safety. It implements responsible engineering practices throughout the system lifecycle rather than treating them as an afterthought. This book introduces core methods and workflows, and the companion book extends these ideas to governance and deployment at scale.
...
# │ Exports: AmdahlsPitfall.t_inference_ms_str, AmdahlsPitfall.t_inf_new_ms_str,
# │          AmdahlsPitfall.t_pre_ms_str, AmdahlsPitfall.t_post_ms_str,
...
# │
# │ Imports: math (standard library), mlsysim.fmt (fmt, check)
# │ Exports: EfficiencyScaleSummary.algo_efficiency_max_mult_str,
...
# │ Exports: AmdahlsPitfallRecap.t_inference_ms_str, .t_inf_new_ms_str, .t_pre_ms_str,
# │          .t_post_ms_str, .improv_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.physics import calc_amdahls_speedup
...
* **The iron law decomposes performance into three terms**: Data movement, computation, and overhead each resolve to time; the slowest term dominates. Reducing inference from `{python} AmdahlsPitfallRecap.t_inference_ms_str` to `{python} AmdahlsPitfallRecap.t_inf_new_ms_str` yields only `{python} AmdahlsPitfallRecap.improv_pct_str` end-to-end improvement when preprocessing (`{python} AmdahlsPitfallRecap.t_pre_ms_str`) and postprocessing (`{python} AmdahlsPitfallRecap.t_post_ms_str`) dominate total latency.
...
* **Efficiency and scale co-evolve across the deployment spectrum**: Algorithmic improvements delivered a `{python} EfficiencyScaleSummary.algo_efficiency_max_mult_str` compute reduction, yet total AI compute grew by roughly `{python} EfficiencyScaleSummary.compute_growth_order_str` orders of magnitude. Deployment context (from cloud to TinyML) determines which efficiency dimension (algorithmic, compute, or data) to prioritize.
