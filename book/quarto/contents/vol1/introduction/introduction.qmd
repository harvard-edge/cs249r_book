...
class DeploymentSystems:
  ...
  #  mem_span_str = f"$10^{{{int(math.log10(mem_scaling))}}}$"
  #  compute_span_str = f"$10^{{{int(math.log10(compute_scaling))}}}$"
    mem_span_exp = int(math.log10(mem_scaling))
    compute_span_exp = int(math.log10(compute_scaling))
```
...
**The Scaling Gap**: The gap between the Cloud and TinyML archetypes is roughly $10^{`{python} DeploymentSystems.mem_span_exp`}$ in memory and $10^{`{python} DeploymentSystems.compute_span_exp`}$ in compute power. This divergence is precisely why we cannot simply "shrink" a cloud model to run at the edge; each tier requires a fundamental redesign of the D·A·M axes.
