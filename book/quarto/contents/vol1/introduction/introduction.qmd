...
The ***D·A·M Taxonomy***\index{D·A·M taxonomy!definition} is a diagnostic framework that classifies any machine learning system performance bottleneck along three axes — **Data** ($D_{vol}$, $BW$), **Algorithm** ($O$, model architecture), and **Machine** ($R_{peak}$, memory capacity) — to identify which axis is the binding constraint.

1.  **Significance (Quantitative):** The diagnostic power is concrete. A ResNet-50 inference run at batch size 1 is memory-bandwidth-bound (Machine axis): the A100's 2 TB/s bandwidth moves 25 MB of weights per forward pass in 12.5 μs, while the 4 GFLOP compute finishes in 2 μs — a 6× gap meaning hardware upgrades to $R_{peak}$ yield no improvement until the bandwidth bottleneck is resolved first.
...
| **TinyML**    | ESP32-S3           | `{python} DeploymentSystems.tiny_mem_str` KiB  | `{python} DeploymentSystems.tiny_compute_str` TFLOPS   | `{python} DeploymentSystems.tiny_power_str` mW  |
