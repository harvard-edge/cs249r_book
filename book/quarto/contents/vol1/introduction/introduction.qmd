...
# PURPOSE
# =============================================================================
# Purpose: Core intro stats used in prose + footnotes.
# Used in: Intro narrative + footnotes in this chapter.
# =============================================================================
# INPUT
# =============================================================================
...
# Verification Gap
vg_pixels = 224 * 224 * 3
vg_digits_value = vg_pixels * math.log10(256)
imagenet_test_images_value = 50_000
imagenet_test_images_str = fmt(imagenet_test_images_value, precision=0, commas=True)
gmail_emails_t_value = GMAIL_EMAILS_PER_DAY * 365 / 1e12
gpt4_gpu_m_value = GPT4_TRAINING_GPU_DAYS / 1e6

# AlexNet relative improvement
alexnet_top5_error_value = 15.3
alexnet_second_place_error_value = 26.4
alexnet_relative_improvement_value = (
    (alexnet_second_place_error_value - alexnet_top5_error_value)
    / alexnet_second_place_error_value
    * 100
)

# Data scale conversion (approximate tokens -> GB)
data_scale_tokens_b_value = 500
token_bytes_value = 1.4
data_scale_gb_value = (data_scale_tokens_b_value * 1e9 * token_bytes_value * byte).to(GB).magnitude

# Moore's Law comparison
moores_doubling_months_value = 24
ai_compute_doubling_months_value = 3.4
moores_speedup_value = moores_doubling_months_value / ai_compute_doubling_months_value

gpt3_params_b_value = GPT3_PARAMS.to(Bparam).magnitude
gpt3_training_zflops_value = int(GPT3_TRAINING_OPS.to(ZFLOPs).magnitude)
gpt3_gpus_value = 1024
algo_efficiency_max_value = 44.5

# =============================================================================
# OUTPUT
# =============================================================================
google_search_b_str = fmt(google_search_b_value, precision=1)
alexnet_params_m_str = fmt(alexnet_params_m_value, precision=0)
imagenet_images_m_str = fmt(imagenet_images_m_value, precision=1)
vg_digits_str = fmt(vg_digits_value, precision=0, commas=True)
alexnet_relative_improvement_str = fmt(alexnet_relative_improvement_value, precision=0)
data_scale_gb_str = fmt(data_scale_gb_value, precision=0, commas=False)
moores_speedup_str = fmt(moores_speedup_value, precision=0, commas=False)
gmail_emails_t_str = fmt(gmail_emails_t_value, precision=0)
gpt4_gpu_m_str = fmt(gpt4_gpu_m_value, precision=1)

gpt3_params_b_str = fmt(gpt3_params_b_value, precision=0, commas=False)
gpt3_params_billion_str = f"{gpt3_params_b_str} billion"
gpt3_training_zflops_str = fmt(gpt3_training_zflops_value, precision=0, commas=False)
algo_efficiency_max_str = fmt(algo_efficiency_max_value, precision=0, commas=False)

gpt3_gpus_str = fmt(gpt3_gpus_value, precision=0, commas=True)
...
A common mistake is applying traditional software testing and deployment practices to ML systems, but these systems fail in fundamentally different ways (@sec-introduction-ml-systems-differ-traditional-software-7ea8). Traditional software bugs produce stack traces within milliseconds and get fixed within hours; ML systems can silently degrade over months before accuracy drops become noticeable. A/B tests in conventional software often show clear winner signals within days; ML system comparisons may require weeks to detect small accuracy differences across diverse subpopulations. Unit tests can verify deterministic code paths in traditional systems; ML systems additionally require monitoring infrastructure that catches the fraction of predictions where models produce unreliable outputs. Teams that deploy ML systems using only CI/CD pipelines without drift detection risk silent failures affecting a significant share of predictions before intervention.
...
Engineers optimize models for inference latency in isolation, but system-wide effects dominate production performance. A team reduces model inference time from `{python} t_inference_str` ms to `{python} t_inf_new_str` ms through model optimization, expecting proportional end-to-end improvement. Preprocessing consumes `{python} t_pre_str` ms and postprocessing adds `{python} t_post_str` ms, however, so total latency decreases only from `{python} total_ms` ms to `{python} new_total_ms` ms: a `{python} improv_pct`% improvement rather than the expected `{python} naive_p`%. The D·A·M taxonomy and the "Samples per Dollar" perspective show that data, algorithms, and machines form interdependent systems where optimizing one component often shifts bottlenecks rather than eliminating them. For example, switching to a more accurate model that requires 3x more preprocessing can increase total cost substantially while improving accuracy only marginally. Similarly, a cheaper inference solution that crashes even a small fraction of the time can cost more than premium infrastructure once retries, support overhead, and user impact are factored in. Teams that optimize components independently often find that a significant share of their engineering effort fails to improve end-to-end metrics.

##### Fallacy: *ML systems can be deployed once and left to run indefinitely.*  {.unnumbered}

The intuition from traditional software, that deployed systems maintain performance indefinitely, does not transfer to ML. Accuracy degrades as the world changes around static models. A recommendation system trained on pre-pandemic user behavior, for instance, can lose significant accuracy within months as purchasing patterns shift dramatically. The ML development lifecycle (@sec-introduction-ml-development-lifecycle-f595) shows continuous monitoring and retraining as operational requirements, not optional enhancements. Fraud detection models tend to degrade as attackers adapt to detection patterns. Natural language systems experience gradual accuracy decline from vocabulary drift and evolving slang. Without monitoring infrastructure, systems appear to run successfully while silently failing on a growing fraction of requests. Organizations that treat deployment as a one-time event rather than an ongoing operation typically discover failures only through customer complaints months after degradation begins.

##### Pitfall: *Assuming that ML expertise alone is sufficient for ML systems engineering.*  {.unnumbered}

Many organizations hire ML researchers and expect production-ready systems. The five engineering disciplines (@sec-introduction-five-engineering-disciplines-6ef9), however, require integrated expertise across ML algorithms, software engineering, systems design, and operations. In practice, teams with strong ML skills but limited systems experience often struggle to deploy models because they lack experience with API design, database optimization, and monitoring infrastructure, shipping systems that fall far short of throughput requirements. Conversely, experienced software engineers without ML understanding can build technically sound infrastructure that inadvertently introduces preprocessing bugs causing accuracy degradation that goes undetected for months. Industry surveys consistently report that the majority of ML projects fail due to insufficient systems engineering expertise, not algorithmic limitations. Effective teams combine ML researchers, software engineers, and operations specialists rather than expecting any single role to master all required skills.
