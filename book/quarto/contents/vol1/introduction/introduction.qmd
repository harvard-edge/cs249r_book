...
gpt3_training_zflops_value = int(GPT3_TRAINING_OPS.magnitude / 1e21)
...
[^fn-sgd]: **Stochastic Gradient Descent (SGD)**\index{Stochastic Gradient Descent (SGD)|see{Gradient Descent}}: The name reveals the method: "stochastic" derives from Greek *stochastikos* (able to guess or aim at a target), reflecting the algorithm's use of random sampling; "gradient" comes from Latin *gradiens* (stepping), describing the incremental steps down the error surface. Rather than computing gradients over the entire dataset (computationally prohibitive for large datasets), SGD estimates gradients from small random batches, typically 32–256 samples. This stochasticity introduces noise that paradoxically helps optimization avoid poor solutions. Modern variants are discussed in @sec-ai-training.
...
These three components—Data, Algorithm, and Machine—are the axes of the **D·A·M taxonomy**\index{D·A·M taxonomy!definition}\index{D·A·M taxonomy!diagnostic framework}, the diagnostic framework introduced earlier for identifying bottlenecks and understanding system constraints. An analogy clarifies the role of each axis before we examine their interactions: **Data** is the fuel that powers the journey, **Algorithm** is the blueprint that defines the flight path, and **Machine** is the engine that makes it all move. Without fuel, the engine sits idle. Without a blueprint, the fuel burns aimlessly. Without an engine, fuel and blueprints remain theoretical.
...
*   **Algorithm**[^fn-algorithm] (Logic): More efficient architectures (like Transformers vs RNNs) improve the rate at which samples translate to accuracy.
...
We retain "Iron Law" because, like **Amdahl's Law**, its value lies in **diagnostic power**: identifying which physical constraint dominates before optimizing.[^fn-box-model] The Iron Law is useful precisely because it simplifies the complexity of the full stack into three manageable terms. For the refined treatment including overlap techniques, see @sec-appendix-dam.
:::
...
[^fn-imagenet]: **ImageNet**: A dataset of over 14 million labeled images organized into 22,000 categories, created at Stanford and Princeton starting in 2006. The associated ImageNet Large Scale Visual Recognition Challenge (ILSVRC), held from 2010 to 2017, became the definitive benchmark for computer vision systems. ImageNet's scale (`{python} imagenet_images_m_str` million training images across 1,000 categories in the competition subset) demanded systems engineering solutions: distributed data loading, GPU memory optimization, and efficient preprocessing pipelines. The benchmark methodology established here influenced all subsequent ML systems evaluation. As examined in the historical evolution section earlier (@sec-introduction-deep-learning-era-infrastructure-bottleneck-490a), the lesson is immediate: coordinating all three D·A·M axes enables capabilities that any single axis cannot achieve alone.
...
The magnitude of efficiency improvements is measurable. Between 2012 and 2019, computational resources needed to train a neural network to achieve AlexNet-level performance on ImageNet classification decreased by approximately $44\times$ [@hernandez2020measuring]. This improvement, which halved every 16 months, outpaced hardware efficiency gains predicted by Moore's Law\index{Moore's Law!comparison to AI scaling}[^fn-moores-law], demonstrating that algorithmic innovation drives efficiency as much as hardware advances.
...
We have now established the fundamental physics of ML systems:

*   **The Bitter Lesson** shows that computational scale drives progress.
*   **The Degradation Equation** reveals that this scale introduces silent reliability challenges.
*   **The Iron Law** governs the physical performance limits of data, compute, and latency.
*   **The Efficiency Framework** provides the levers to optimize within those limits.

Together, these principles define the scope of *AI Engineering* as an emerging discipline:
...
\index{Five-Pillar Framework!emergence from D·A·M taxonomy}The five-pillar framework organizes the practice of **ML Systems Engineering**, which is the operational realization of the broader **AI Engineering** discipline defined earlier. While AI Engineering defines the *what* (building stochastic systems with deterministic reliability), the Five-Pillar Framework defines the *how*: the specific organizational structures and technical domains required to execute that mandate.
