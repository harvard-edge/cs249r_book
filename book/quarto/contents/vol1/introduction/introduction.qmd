...
*   **Efficiency (η)**: ≈ `{python} GPT3Training.eta_base_pct_str`% (typical for large-scale distributed training).
...
* **The Iron Law decomposes performance into three terms**: Data movement, computation, and overhead each resolve to time; the slowest term dominates. Reducing inference from `{python} AmdahlsPitfall.t_inference_str` ms to `{python} AmdahlsPitfall.t_inf_new_str` ms yields only `{python} AmdahlsPitfall.improv_pct`% end-to-end improvement when preprocessing (`{python} AmdahlsPitfall.t_pre_str` ms) and postprocessing (`{python} AmdahlsPitfall.t_post_str` ms) dominate total latency.
