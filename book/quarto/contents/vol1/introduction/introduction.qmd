...
Deep learning effectively traded the feature engineering bottleneck for a new **compute bottleneck**\index{Compute Bottleneck}. Models like GPT-3 [@brown2020language] (`{python} GPT3Scale.gpt3_params_billion_str` parameters) illustrate the scale of this new challenge. Training used about `{python} GPT3Scale.gpt3_training_tokens_billion_str` tokens from filtered web text, books, and Wikipedia and required an estimated `{python} GPT3Scale.gpt3_training_zflops_str` zettaFLOPs of compute; the GPT-3 paper does not specify the exact hardware configuration, but that compute corresponds to hundreds of V100 GPU-years under common throughput assumptions. (One zettaFLOP equals $10^{21}$ floating-point operations; the training corpus comprised roughly `{python} GPT3Scale.data_scale_gb_str` of text.) The primary engineering challenge shifted from "*how* do we describe a cat's ear?" to "*how* do we coordinate large-scale distributed training without failure?"
