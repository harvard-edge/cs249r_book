...
| **Modern Deep Learning (2023)**  | Foundation models              | MMLU benchmark          |       86.4% (GPT-4) | Estimated `{python} GPT4Scale.gpt4_gpu_m_str` million A100 GPU-days |
...
4.  **Missions (The Scenarios)**: The application context (The Destination). This is the top of the stack, where a system is deployed to solve a specific problem. A **Mission**, such as the **Smart Doorbell**, introduces high-level requirements (e.g., "one-year battery life") that dictate the configuration of every layer below.
