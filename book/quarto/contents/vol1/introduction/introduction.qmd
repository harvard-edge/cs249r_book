...
# │          AIMomentStats.h100_fp16_tflop_per_s_str,
# │          AIMomentStats.cpu_fp32_tflop_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
class AIMomentStats:
    ...
    h100_fp16_tflop_per_s_str = fmt_qty(gpu_tflops, TFLOPs / second, precision=0, commas=False)
    cpu_fp32_tflop_per_s_str = fmt_qty(cpu_tflops, TFLOPs / second, precision=0, commas=False)
```
...
from mlsysim.fmt import fmt_int, fmt, fmt_count, fmt_memory, fmt_params, check, MarkdownStr
...
@Tbl-ai-evolution-performance provides quantitative validation of this principle. Examine the rightmost column carefully: as computational resources grew from rule evaluation to CPU-era feature pipelines, multi-GPU training, and eventually large-scale distributed training, performance improved from amateur-level to superhuman. GPT-4's exact training configuration was not disclosed, but common public estimates place GPT-4-class runs in the same regime as `{python} GPT4ScaleEstimate.gpu_count_str` A100-class GPUs for roughly `{python} GPT4ScaleEstimate.training_days_str`. The mlsysim reference anchor is about `{python} GPT4ScaleEstimate.gpu_days_m_str` million A100 GPU-days, so this value is a scale anchor rather than an official benchmark result.
...
| **Modern Deep Learning (2023)**  | Foundation models              | MMLU benchmark                | 86.4 percent (GPT-4)                                                 | Not disclosed; public-estimate regime of about `{python} GPT4ScaleEstimate.gpu_days_m_str`M A100 GPU-days (same scale as `{python} GPT4ScaleEstimate.gpu_count_str` for `{python} GPT4ScaleEstimate.training_days_str`) |
...
# │ Imports: mlsysim.fmt (fmt, check)
# │ Exports: ResNet50DamExample.weight_mb_str, memory_traffic_mb_str, a100_hbm_tb_per_s_str,
# │          memory_time_us_str, resnet_gflop_str, a100_peak_tflop_per_s_str,
...
class ResNet50DamExample:
    ...
    a100_hbm_tb_per_s_str = fmt_qty(p.peak_bw_actual, TB / second, precision=1, commas=False)
    memory_time_us_str = fmt_time(p.latency_memory, US, precision=1, commas=False)
    resnet_gflop_str = fmt_qty(resnet_flops, GFLOPs, precision=1, commas=False)
    a100_peak_tflop_per_s_str = fmt_qty(p.peak_flops_actual, TFLOPs / second, precision=0, commas=False)
...
# │ Exports: DeploymentSystems.cloud_mem_gb_str, DeploymentSystems.cloud_compute_tflop_per_s_str,
# │          DeploymentSystems.cloud_power_w_str, DeploymentSystems.edge_mem_gb_str,
# │          DeploymentSystems.edge_compute_tflop_per_s_str, DeploymentSystems.edge_power_w_str,
# │          DeploymentSystems.mobile_mem_gb_str, DeploymentSystems.mobile_compute_str,
# │          DeploymentSystems.mobile_power_w_str,
# │          DeploymentSystems.tiny_mem_kb_str, DeploymentSystems.tiny_compute_tflop_per_s_str,
...
class DeploymentSystems:
    ...
    cloud_compute_tflop_per_s_str = fmt_qty(h_cloud.compute.peak_flops, TFLOP / second, precision=0, commas=True)
    ...
    edge_compute_tflop_per_s_str = fmt_qty(h_edge.compute.peak_flops, TFLOP / second, precision=0, commas=True)
    ...
    tiny_compute_tflop_per_s_str = fmt_qty(h_tiny.compute.peak_flops, TFLOP / second, precision=4, commas=True)
...
|:--------------|:-------------------|:----------------------------------------------:|:----------------------------------------------------------:|:-----------------------------------------------:|
| **Cloud**     | H100 GPU           | `{python} DeploymentSystems.cloud_mem_gb_str`  | `{python} DeploymentSystems.cloud_compute_tflop_per_s_str` |  `{python} DeploymentSystems.cloud_power_w_str` |
| **Edge**      | Jetson Robotics    |  `{python} DeploymentSystems.edge_mem_gb_str`  | `{python} DeploymentSystems.edge_compute_tflop_per_s_str`  |  `{python} DeploymentSystems.edge_power_w_str`  |
...
| **TinyML**    | ESP32-S3           |  `{python} DeploymentSystems.tiny_mem_kb_str`  | `{python} DeploymentSystems.tiny_compute_tflop_per_s_str`  |  `{python} DeploymentSystems.tiny_power_mw_str` |
...
# │          GPT3Training.peak_tflop_per_s_str, GPT3Training.time_formula_math,
...
class GPT3Training:
    ...
    peak_tflop_per_s_str = fmt_flop_rate(peak_flops_rate, unit=TFLOPs / second, precision=0, commas=False)
...
*   **Peak $(R_{\text{peak}})$**: `{python} GPT3Training.peak_tflop_per_s_str`
...
To appreciate the magnitude of these gains, consider the trajectory in @fig-algo-efficiency: starting from AlexNet as the baseline, each successive architecture (VGG, ResNet, MobileNet, EfficientNet) achieves comparable accuracy with dramatically fewer computational resources, reaching a `{python} EfficiencyAlgoFigureCaption.algo_efficiency_max_mult_str` improvement from AlexNet in 2012 to EfficientNet-B0 in 2019, a roughly seven-year span corresponding to algorithmic efficiency doubling about every 16 months. Architectures introduced in the early 2020s, such as EfficientNetV2 and ConvNeXt, sustained this exponential pace even as ImageNet classification matured as a benchmark.

::: {#fig-algo-efficiency fig-env="figure" fig-pos="htb" fig-cap="**Algorithmic Efficiency Trajectory**: Training efficiency factor relative to AlexNet (2012 baseline) for ImageNet classification. Most later architectures achieve comparable accuracy with fewer computational resources, although individual points vary. The trajectory from AlexNet (1$\times$) through VGG, ResNet, MobileNet, and ShuffleNet to EfficientNet (44$\times$) demonstrates a forty-four-fold reduction in required compute over seven years, independent of hardware improvements. Early-2020s architectures including EfficientNetV2, DeiT, and ConvNeXt fall along the same fitted exponential, sustaining the roughly 16-month doubling cadence even as ImageNet classification matured as an efficiency benchmark." fig-alt="Scatter plot of training efficiency factors from AlexNet in 2012 to ConvNeXt in 2022. Red dots rise from AlexNet at 1x to early-2020s models above 80x. A dashed exponential trend line fit through all points curves steeply upward."}
