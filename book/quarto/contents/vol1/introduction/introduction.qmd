...
[^fn-iron-law-analogy]: **A Note on the 'Iron Law'**: We call this the "Iron Law" by analogy to Patterson & Hennessy's Iron Law of Processor Performance [@patterson2021computer]. However, there are important differences. P&H's law is a **multiplicative decomposition** (a tautology factoring CPU time), whereas our equation is an **additive first-order model** that approximates performance under simplifying assumptions. The additive form assumes sequential execution; in practice, systems can **overlap** these terms, transforming the sum into a max: $T_{pipelined} = \max\left(\frac{D_{vol}}{BW}, \frac{O}{R_{peak} \cdot \eta}\right) + L_{lat}$. We retain "Iron Law" because, like Amdahl's Law, its value lies in **diagnostic power**: identifying which physical constraint dominates before optimizing. For the refined treatment including overlap techniques, see @sec-appendix-dam.
