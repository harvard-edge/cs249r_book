...
- Explain the **AI Triad**\index{AI Triad!definition} (Data, Algorithm, Machine) and apply the **D·A·M taxonomy** to diagnose ML system bottlenecks
- Explain AI's evolution from symbolic reasoning to deep learning and *why* computational scale outperforms encoded expertise (the **bitter lesson**\index{Bitter Lesson!definition})
...
**The Context**: Google engineers published a landmark paper [@sculley2015hidden] that changed how the industry views ML engineering.
...
**The Systems Lesson**: "Machine Learning" is easy; **"Machine Learning Systems"** are hard. The friction in deployment rarely comes from the matrix multiplication (the five percent); it comes from the interface between that math and the messy reality of the other 95 percent. Optimizing only the model optimizes the smallest part of the problem.
...
AI's evolution reveals a progression of bottlenecks, each overcome by systems innovations that expanded what was computationally possible. The field traces its origin to Turing's[^fn-turing-outputs] paper "Computing Machinery and Intelligence" [@turing1950], which posed the foundational question: *Can machines think?* Early systems that attempted to answer this question, such as the Perceptron (1957) and ELIZA[^fn-eliza-brittleness] [@weizenbaum1966], ran into the limits of manual logic and mainframe-era hardware, resulting in brittleness. Subsequent eras hit the knowledge acquisition bottleneck: manual knowledge entry could not scale. Modern systems face a different constraint: computational throughput.
...
::: {#fig-ai-timeline fig-env="figure" fig-pos="t!" fig-cap="**AI Development Timeline.** A chronological curve traces AI research activity from the 1950s to the 2020s, with gray bands marking the two AI Winter periods (1974 to 1980, 1987 to 1993). Callout boxes highlight key milestones including the Turing Test [@turing1950], the Dartmouth conference [@mccarthy2006], the Perceptron, ELIZA, Deep Blue, and GPT-3." fig-alt="Timeline from the 1950s to the 2020s with a red line showing AI publication frequency. Gray bands mark two AI winters. Callout boxes mark milestones including the Turing Test, Dartmouth conference, Perceptron, ELIZA, Deep Blue, and GPT-3."}
...
The first era of AI engineering (1950s–1970s) attempted to reduce intelligence to **Symbolic AI**\index{Symbolic AI} manipulation. Researchers at the 1956 **Dartmouth Conference**\index{Dartmouth Conference}[^fn-dartmouth-systems] [@mccarthy2006] hypothesized that if they could formalize the rules of logic, machines could "think." Even then, some saw a different path: Arthur Samuel at IBM demonstrated in 1959 that a checkers program could improve through self-play, coining the very term "**machine learning**\index{Machine Learning}." But the dominant paradigm remained symbolic. Daniel Bobrow's *STUDENT*[^fn-bobrow-student] system exemplifies this approach [@bobrow1964student; @samuel1959].
...
In the 1980s, engineers pivoted from general logic to capturing deep domain expertise. MYCIN\index{MYCIN!expert system}, designed to diagnose blood infections, encoded approximately six hundred rules derived from interviews with medical experts [@shortliffe1975].
...
# │          GPT3Scale.gpt3_training_zflops_str, GPT3Scale.gpt3_gpus_str,
# │          GPT3Scale.data_scale_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Models
from mlsysim.core.constants import Bparam, ZFLOPs, byte, GB
from mlsysim.fmt import fmt_percent, fmt, check
...
Deep learning effectively traded the **feature engineering bottleneck** for a new **compute bottleneck**\index{Compute Bottleneck}. Models like GPT-3 [@brown2020language] (`{python} GPT3Scale.gpt3_params_billion_str` parameters) illustrate the scale of this new challenge. Training required `{python} GPT3Scale.gpt3_training_zflops_str` zettaFLOPs of compute, consumed approximately 500 billion tokens from filtered web text, books, and Wikipedia, and demanded `{python} GPT3Scale.gpt3_gpus_str` GPUs running for weeks. (One zettaFLOP equals $10^{21}$ floating-point operations; the training corpus comprised roughly `{python} GPT3Scale.data_scale_gb_str` GB of text.) The primary engineering challenge shifted from "*how* do we describe a cat's ear?" to "*how* do we coordinate `{python} GPT3Scale.gpt3_gpus_str` GPUs without failure?"
...
# │
# │ Imports: mlsysim.core.constants (GPT4_TRAINING_GPU_DAYS)
# │ Exports: GPT4Scale.gpt4_gpu_m_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.core.constants import MILLION, GPT4_TRAINING_GPU_DAYS
from mlsysim.fmt import fmt_percent, fmt, check
...
class GPT4Scale:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    gpu_days_m = GPT4_TRAINING_GPU_DAYS/MILLION
...
| **Statistical ML (1990s-2000s)** | Feature engineering + learning | ImageNet top-5 accuracy |          50–60 percent | Hours on single CPU                                                 |
...
: **AI Performance Evolution Across Paradigms**: Each paradigm transition correlates with increased computational scale rather than algorithmic sophistication. Performance improved from amateur-level expert systems (2000 Elo) to superhuman foundation models (86.4 percent Massive Multitask Language Understanding (MMLU)), while computational requirements grew from single CPUs to 2.5 million A100 GPU-days. Training time initially increased (hours to days) but later decreased as distributed systems enabled parallelization. {#tbl-ai-evolution-performance tbl-colwidths="[17,19,20,18,26]"}

MMLU\index{MMLU!benchmark} (Multitask Multiple-choice Language Understanding) is a standard benchmark for broad knowledge across many subjects; we formalize benchmarking in @sec-benchmarking.
...
3.  **Common Pitfall:** A frequent misconception is that an ML system is the model. Google's audit of production ML systems found that model code comprises approximately five percent of total lines; the other 95 percent is data pipelines, serving infrastructure, and monitoring [@sculley2015hidden].
...
1.  **Significance (Quantitative):** The diagnostic power is concrete. A ResNet-50 inference run at batch size one is memory-bandwidth-bound (Machine axis): the A100's 2 TB/s bandwidth moves 25 MB of weights per forward pass in 12.5 μs, while the 4 GFLOP compute finishes in 2 μs. This 6$\times$ gap means hardware upgrades to $R_{\text{peak}}$ yield no improvement until the bandwidth bottleneck is resolved first.
...
3.  **Workloads (The Models)**: The algorithmic demand (The Route). This layer defines the mathematical workload: operation count ($O$), parameter volume ($D_{\text{vol}}$), and data layout. We use **Lighthouse Workloads** like **GPT-4** and **Wake Vision**\index{Wake Vision!smart doorbell}.
...
This hierarchy ensures that when we build a lab or a case study, engineers are not starting from scratch, but rather inheriting the constraints of a System Archetype and applying a Lighthouse Model to a specific mission. For instance, the **Smart Doorbell** scenario (@sec-introduction-deployment-case-studies-636f) inherits the **TinyML Archetype**, uses the **Wake Vision** model, and operates on **ESP32** hardware. This structured approach allows us to reason about the "Physics of ML" across any application domain.
...
# │          DeploymentSystems.mobile_power_str, DeploymentSystems.tiny_mem_str,
...
from mlsysim.core.constants import GB, MB, KiB, watt, milliwatt, TFLOPs, second, flop
...
class DeploymentSystems:
    ...
    tiny_mem_str = fmt(t_tiny.ram.m_as(KiB), precision=0)
    ...
    mem_span_str = md_math(f"10^{{{int(math.log10(mem_scaling))}}}")
    compute_span_str = md_math(f"10^{{{int(math.log10(compute_scaling))}}}")
```

::: {.callout-perspective #perspective-deployment-archetypes title="The ML Systems Landscape: Four Archetypes"}

The machine learning systems landscape spans nine orders of magnitude in computational power and memory capacity. We categorize this continuum into four **System Archetypes**\index{System Archetype!deployment constraints} that define the constraints for every subsequent chapter:
...
Systems engineering is the art of balancing this equation. As a rough illustration: a 10 percent gain in hardware efficiency allows for a 10 percent larger dataset, which might yield a one percent gain in accuracy. The engineer's job is to determine if that trade-off is economically viable.
...
This first-order linearization captures the dominant trend: accuracy erodes roughly in proportion to how far the current data distribution has drifted from the training distribution. The model breaks down for large shifts (where the relationship becomes nonlinear) and the specific divergence measure $D(\cdot \| \cdot)$ is left deliberately general (common choices include KL divergence, total variation distance, or Wasserstein distance, each with different sensitivity profiles). Despite these simplifications, the equation reveals three engineering levers for managing degradation:
...
3. **Monitor and respond to drift** ($D(P_t \| P_0)$)\index{Data Drift}: Continuous measurement of distribution divergence enables proactive retraining before accuracy falls below acceptable thresholds.
...
$$\text{Time}_{\text{total}} = \underbrace{ \frac{\text{Data} (D_{\text{vol}})}{\text{Bandwidth} (\text{BW})} }_{\text{The Data Term}} + \underbrace{ \frac{\text{Ops} (O)}{\text{Peak} (R_{\text{peak}}) \times \text{Efficiency} (\eta_{\text{hw}})} }_{\text{The Compute Term}} + \underbrace{ \text{Overhead} (L_{\text{lat}}) }_{\text{The Latency Term}}$$ {#eq-intro-iron-law}
...
$$T_{\text{pipelined}} = \max\left(\frac{D_{\text{vol}}}{BW}, \frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}}\right) + L_{\text{lat}}$$ {#eq-intro-iron-law-pipelined}
...
*   **Efficiency (η)**: ≈ `{python} GPT3Training.eta_base_pct_str` percent (typical for large-scale distributed training).
...
**The Systems Insight**: If we improve software efficiency (η) from `{python} GPT3Training.eta_base_pct_str` percent to `{python} GPT3Training.eta_opt_pct_str` percent through kernel fusion and better scheduling, training time drops to **`{python} GPT3Training.days_optimized_str` days**, saving nearly `{python} GPT3Training.days_saved_str` days of expensive compute time.
...
The **iron law** ($T \approx \frac{D_{\text{vol}}}{BW} + \frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}} + L_{\text{lat}}$) is the analytical backbone of this book. Before proceeding, verify you can manipulate its terms:
...
Abstract equations become concrete through concrete workloads. This textbook employs five recurring **Lighthouse Models**\index{Lighthouse Model!diagnostic tool} as diagnostic tools for the iron law. These canonical workloads serve as the Cast of Characters, our Systems Detectives, reappearing in every chapter to interrogate the iron law (the Silicon Contract between algorithm and machine).
...
Each archetype manifests different constraints along the D·A·M axes, ensuring that the principles developed throughout this text are tested against the diversity of real-world systems engineering challenges. Later in this chapter, these technical workloads are complemented with three deployment case studies, Waymo, FarmBeats, and AlphaFold, that illustrate how the same core challenges manifest in production systems under radically different constraints.
...
The top of the hierarchy transforms abstract systems into concrete engineering missions. Each mission inherits one of the four **System Archetypes** introduced in the Engineering Crux (@sec-introduction-engineering-crux) and pairs it with a Lighthouse Model. Throughout the book and its associated labs, we focus on these four **Application Scenarios**:
...
| **Mobile Assistant**\index{Mobile Assistant!smartphone constraints}  | Smartphone           | Llama-2-70B          | Thermal Throttling/RAM                     |
...
[^fn-ab-testing-validation]: **A/B Testing**: A statistical validation method that serves different model versions to distinct user segments, isolating the causal impact of a model change on business metrics. The "ease" the triggering sentence describes is cloud-specific: centralized traffic routing enables instant version switching and rollback. A typical experiment requires serving over 100,000 users to reliably detect a one percent change in a target metric, making A/B testing impractical on edge deployments with limited user pools. \index{A/B Testing!validation role}
...
Deployment Infrastructure\index{Deployment Infrastructure!pillar definition} (@sec-benchmarking, @sec-ml-operations) addresses system-related challenges around the training-serving divide and operational complexity. This pillar encompasses measuring and optimizing inference performance across deployment tiers, from cloud to edge devices. @sec-benchmarking covers inference metrics, latency analysis, and the MLPerf scenarios that characterize different deployment contexts. @sec-ml-operations covers A/B testing, staged rollouts, and operational playbooks for production systems.
...
For detailed guidance on reading paths, learning outcomes, prerequisites, and how to get the most from this textbook, refer to @sec-about.
...
Engineers assume algorithmic sophistication drives system performance, but this ignores the iron law (@sec-introduction-iron-law-ml-systems-c32a). A state-of-the-art vision transformer achieves 1–2 percent higher accuracy than ResNet-50 on ImageNet but requires 4$\times$ the FLOPs and 3$\times$ the memory bandwidth [@dosovitskiy2021image]. In production, a model that is one percent more accurate but violates latency requirements has effectively zero utility. Google's analysis found that only five percent of production ML code is the model itself; the remaining 95 percent is data pipelines, serving infrastructure, and monitoring [@sculley2015hidden]. A well-engineered system with a simpler model consistently outperforms a state-of-the-art architecture lacking robust infrastructure.
...
* **Five pillars require integration**: ML systems engineering encompasses Data Engineering, Training Systems, Deployment Infrastructure, Operations and Monitoring, and Ethics and Governance. Teams lacking expertise in any pillar face 60–70 percent project failure rates.
