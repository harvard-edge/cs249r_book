...
# │          AIMomentStats.accelerator_fp16_tflop_per_s_str,
...
class AIMomentStats:
    ...
    accelerator_fp16_tflop_per_s_str = fmt_flop_rate(accelerator_tflops, unit=TFLOPs / second, precision=0, commas=False)
    cpu_fp32_tflop_per_s_str = fmt_flop_rate(cpu_tflops, unit=TFLOPs / second, precision=0, commas=False)
```
...
# │ How: Compare an illustrative 25,000 A100-class GPU, 90-day run with the
# │      mlsysim GPT-4-class A100-GPU-day reference anchor.
# │
# │ Imports: mlsysim.fmt (fmt, fmt_count), mlsysim.core.constants
# │          (GPT4 public-estimate references, Models.Language.GPT4.training_gpu_days, MILLION)
# │ Exports: GPT4ScaleEstimate.a100_class_gpu_count_str, training_days_str,
# │          a100_gpu_days_million_str
# └─────────────────────────────────────────────────────────────────────────────
...
class GPT4ScaleEstimate:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(a100_gpu_days_m > 1.0, "GPT-4-class public estimate should be million-A100-GPU-day scale.")
    check(0.8 <= estimate_ratio <= 1.1, "Illustrative 25k A100-class GPU × 90 day estimate should be near the mlsysim anchor.")
...
@Tbl-ai-evolution-performance provides quantitative validation of this principle. Examine the rightmost column carefully: as computational resources grew from rule evaluation to CPU-era feature pipelines, multi-GPU training, and eventually large-scale distributed training, performance improved from amateur-level to superhuman. GPT-4's exact training configuration was not disclosed, but common public estimates place GPT-4-class runs in the same regime as `{python} GPT4ScaleEstimate.a100_class_gpu_count_str` A100-class GPUs for roughly `{python} GPT4ScaleEstimate.training_days_str`. The mlsysim reference anchor is about `{python} GPT4ScaleEstimate.a100_gpu_days_million_str`, so this value is a scale anchor rather than an official benchmark result.
...
| **Modern Deep Learning (2023)**  | Foundation models              | MMLU benchmark                | 86.4 percent (GPT-4)                                                 | Not disclosed; public-estimate regime of about `{python} GPT4ScaleEstimate.a100_gpu_days_million_str` (same scale as `{python} GPT4ScaleEstimate.a100_class_gpu_count_str` A100-class GPUs for `{python} GPT4ScaleEstimate.training_days_str`) |
...
# │ Imports: mlsysim.fmt (typed prose formatters)
# │ Exports: ResNet50DamExample.fp16_weight_mb_str,
# │          memory_traffic_mb_str, a100_hbm_tb_per_s_str,
# │          memory_time_us_str, resnet_gflop_str,
# │          a100_peak_tflop_per_s_str, compute_time_us_str,
# │          a100_ridge_flops_per_byte_str,
# │          workload_ai_flops_per_byte_str
# └─────────────────────────────────────────────────────────────────────────────
...
from mlsysim.core.units import MB, TB, GFLOPs, TFLOPs, flop, byte, microsecond, second

# ┌── LEGO ───────────────────────────────────────────────
class ResNet50DamExample:
    """Batch-size-one ResNet-50 roofline anchor for the D.A.M. taxonomy."""
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    profile = Engine.solve(m, h, batch_size=1, precision="fp16", efficiency=1.0)

    fp16_weight = profile.memory_footprint.to(MB)
    a100_hbm = profile.peak_bw_actual.to(TB / second)
    memory_traffic = profile.latency_memory * profile.peak_bw_actual
    memory_time = profile.latency_memory.to(microsecond)
    resnet_flops = m.inference_flops.to(GFLOPs)
    a100_peak = profile.peak_flops_actual.to(TFLOPs / second)
    compute_time = profile.latency_compute.to(microsecond)
    a100_ridge = (a100_peak / a100_hbm).to(flop / byte)
    workload_ai = profile.arithmetic_intensity.to(flop / byte)
    memory_compute_ratio = (memory_time / compute_time).to("").magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(45 <= fp16_weight.to(MB).magnitude <= 60, f"ResNet-50 FP16 weights should be about 50 MB, got {fp16_weight.to(MB).magnitude:.1f} MB.")
    check(
        abs(memory_traffic.to(MB).magnitude - (fp16_weight * 1.1).to(MB).magnitude) < 1,
        f"Batch-one memory traffic should be weights plus about 10 percent activations, got {memory_traffic.to(MB).magnitude:.1f} MB.",
    )
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    fp16_weight_mb_str = fmt_memory(profile.memory_footprint, unit=MB, precision=1, commas=False)
    memory_traffic_mb_str = fmt_memory(memory_traffic, unit=MB, precision=1, commas=False)
    a100_hbm_tb_per_s_str = fmt_bandwidth(profile.peak_bw_actual, unit=TB / second, precision=1, commas=False)
    memory_time_us_str = fmt_time(profile.latency_memory, unit=microsecond, precision=1, commas=False)
    resnet_gflop_str = fmt_flops(resnet_flops, unit=GFLOPs, precision=1, commas=False)
    a100_peak_tflop_per_s_str = fmt_flop_rate(profile.peak_flops_actual, unit=TFLOPs / second, precision=0, commas=False)
    compute_time_us_str = fmt_time(profile.latency_compute, unit=microsecond, precision=1, commas=False)
...
1.  **Significance (quantitative)**: The diagnostic power is concrete even before detailed hardware arithmetic enters the story. If the spam filter misses a new phishing campaign because the training set never contained that tactic, the binding axis is Data. If the training examples are adequate but the model cannot express the pattern, the binding axis is Algorithm. If both are adequate but the service cannot classify messages quickly enough during a traffic spike, the binding axis is Machine. A ResNet-50 inference run at batch size one on an A100 makes that Machine diagnosis quantitative: FP16 weights alone are `{python} ResNet50DamExample.fp16_weight_mb_str`, and loading about `{python} ResNet50DamExample.memory_traffic_mb_str` across `{python} ResNet50DamExample.a100_hbm_tb_per_s_str` of HBM takes about `{python} ResNet50DamExample.memory_time_us_str`, while `{python} ResNet50DamExample.resnet_gflop_str` at a `{python} ResNet50DamExample.a100_peak_tflop_per_s_str` tensor-core peak takes about `{python} ResNet50DamExample.compute_time_us_str`. The workload's arithmetic intensity is `{python} ResNet50DamExample.workload_ai_flops_per_byte_str`, below the A100 FP16 ridge point of `{python} ResNet50DamExample.a100_ridge_flops_per_byte_str`, so buying peak FLOP/s alone does not help until data reuse, bandwidth, or utilization improves.
...
1.  **Hardware (The Silicon)**: The physical foundation (The Engine). This layer defines the raw capabilities: $R_{\text{peak}}$, $\text{BW}$, and memory capacity. Later chapters instantiate this layer with concrete hardware twins.
...
This hierarchy ensures that when we build a lab or a case study, engineers are not starting from scratch, but rather inheriting the constraints of a deployment paradigm and applying a scenario workload to a specific mission. For instance, the Smart Doorbell scenario (@sec-introduction-deployment-case-studies-636f) inherits the TinyML paradigm, uses the Wake Vision model, and operates on microcontroller-class hardware. This structured approach allows us to reason about the "Physics of ML" across any application domain.
...
# │ Exports: DeploymentSystems.mem_span_math, DeploymentSystems.compute_span_math
# └─────────────────────────────────────────────────────────────────────────────
...
The multi-order-of-magnitude span across these four paradigms is not merely a technical curiosity; it translates directly into cost. A model that fits comfortably in a data-center accelerator's memory cannot run unchanged on a microcontroller-class device, and bridging that gap requires engineering trade-offs at every tier of the D·A·M taxonomy. Data quality, algorithmic efficiency, and hardware capability interact through a single economic constraint that systems engineers must optimize.
...
# │ Exports: GPT3Training.accelerator_name_str,
# │          GPT3Training.num_accelerators_str, GPT3Training.eta_base_pct_str,
# │          GPT3Training.eta_opt_pct_str, GPT3Training.days_initial_str,
# │          GPT3Training.days_optimized_str, GPT3Training.days_saved_str,
# │          GPT3Training.peak_tflop_per_s_per_accelerator_str, GPT3Training.time_formula_math,
...
class GPT3Training:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Text strings
    accelerator_name_str = MarkdownStr(h_accelerator.name)
...
**Problem**: What is the training time for a GPT-3 class model on a cluster of `{python} GPT3Training.num_accelerators_str` built from `{python} GPT3Training.accelerator_name_str` GPUs?
...
*   **Scale $(N_{\text{accel}})$**: `{python} GPT3Training.num_accelerators_str` (`{python} GPT3Training.accelerator_name_str` GPUs)
...
# │          mlsysim.fmt (fmt_count, check)
# │ Exports: GPT4TrainingCostRecap.accelerator_days_million_str
# └─────────────────────────────────────────────────────────────────────────────
