...
[^fn-microcontrollers-tinyml]: [offset=-30mm] **Microcontrollers**: Single-chip computers with kilobytes of memory and milliwatts of power budget. For TinyML, memory and energy determine feasibility before model quality. \index{Microcontroller!TinyML substrate}
...
[^fn-latency-responsiveness]: [offset=-30mm] **Latency**\index{Latency!etymology}\index{Latency!edge deployment}: From Latin *latere* ("to lie hidden"), delay is invisible until it causes failure. In autonomous braking, every millisecond adds roughly 3 cm of stopping distance, making $L_{\text{lat}}$ the edge constraint.
