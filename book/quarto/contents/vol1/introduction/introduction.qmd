...
# │
# │ Imports: mlsysim (Models.Language.GPT3.training_energy_mwh,
# │          Scenarios.EnergyAnchors.USHouseholdAnnualElectricity),
...
class GPT3TrainingEnergy:
    ...
    household_mwh_year = Scenarios.EnergyAnchors.USHouseholdAnnualElectricity
...
from mlsysim.fmt import fmt_int, fmt, fmt_qty, fmt_qty_int, check, fmt_time
from mlsysim.core.units import MB, TB, GFLOPs, TFLOPs, flop, byte, US, microsecond, second
...
class ResNet50DamExample:
    ...
    weight = p.memory_footprint.to(MB)
    a100_hbm = p.peak_bw_actual.to(TB / second)
    memory_traffic = p.latency_memory * p.peak_bw_actual
    memory_time = p.latency_memory.to(microsecond)

    resnet_flops = m.inference_flops.to(GFLOPs)
    a100_peak = p.peak_flops_actual.to(TFLOPs / second)
    compute_time = p.latency_compute.to(microsecond)

    a100_ridge = (a100_peak / a100_hbm).to(flop / byte)
    workload_ai = p.arithmetic_intensity.to(flop / byte)
    memory_compute_ratio = (memory_time / compute_time).to("").magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(45 <= weight.to(MB).magnitude <= 60, f"ResNet-50 FP16 weights should be about 50 MB, got {weight.to(MB).magnitude:.1f} MB.")
    check(
        abs(memory_traffic.to(MB).magnitude - (weight * 1.1).to(MB).magnitude) < 1,
    ...
    resnet_gflops_str = fmt_qty(resnet_flops, GFLOPs, precision=1, commas=False)
    ...
    a100_ridge_str = fmt_qty_int(a100_ridge, flop / byte, commas=False, unit_label="FLOP/byte")
    workload_ai_str = fmt_qty_int(workload_ai, flop / byte, commas=False, unit_label="FLOP/byte")
```
...
class GPT3Training:
    ...
    peak_tflops_str = fmt_qty(peak_flops_rate, TFLOPs / second, precision=0, commas=False, unit_label="TFLOP/s")
    ...
    time_value_math = fmt_math(
        rf"\approx \frac{{{_ops_coeff} \times 10^{{{ops_exp_value}}}}}{{{num_gpus} \times {peak_rate_value:.0f} \times 10^{{12}} \times {eta_base}}}"
    )
