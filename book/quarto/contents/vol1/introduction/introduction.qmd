...
[^fn-paradigm-shift-kuhn]: **Paradigm Shift**: Thomas Kuhn's original formulation described scientific paradigm shifts as discontinuous replacements, not incremental refinements. The rule-to-data transition qualifies: the new paradigm does not improve rule-writing but replaces the foundational assumption that an expert can explicitly program a solution, forcing a complete stack replacement from deterministic testing to statistical monitoring, from code versioning to data versioning, and from logic debugging to distribution analysis. \index{Paradigm Shift!Kuhn definition}
...
[^fn-turing-outputs]: **Alan Turing**: A British mathematician whose 1950 paper "Computing Machinery and Intelligence" posed the foundational question: *can machines think?* His "Imitation Game" (the Turing Test) shifted the focus from internal philosophical mechanisms to observable system **Outputs**, an engineering-first perspective that still defines how we evaluate AI: we measure what a system *does* (accuracy, latency, throughput), not what it *is*. \index{Turing, Alan!Imitation Game}
...
[^fn-mainframes-scale]: **Mainframes**: These room-sized computers operated on a non-interactive, batch-processing model that made the iterative development required for complex AI prohibitively slow. Severe memory and compute constraints forced developers to implement narrowly defined, brittle logic. The flagship IBM System/360 (1964), for instance, offered just 1 MB of memory—insufficient for anything beyond the most rudimentary symbolic reasoning or single-layer neural networks. \index{Mainframes!historical context}
...
[^fn-samuel-ml-coined]: **Machine learning**: Coined by Arthur Samuel in 1959 to define an approach that directly challenged the symbolic method of hand-crafting logical rules. His checkers program learned to improve not from programmed instructions, but by evaluating outcomes from playing thousands of games against itself. This proved a machine could acquire expertise directly from raw data, bypassing the symbolic bottleneck of needing human experts to first formalize all the rules of a domain. \index{Samuel, Arthur!machine learning coined}

[^fn-bobrow-student]: **STUDENT**: A 1964 program that parsed English word problems into formal algebraic equations, exemplifying the Symbolic AI approach mentioned in the text. This system revealed the paradigm's core scaling failure: every new input variation or phrasing required adding a new, hand-coded rule. This caused system complexity to grow far faster than its actual problem-solving capability, making such systems brittle and unmaintainable at any significant scale. \index{Bobrow, Daniel!STUDENT}
...
[^fn-ee-heritage-stochastic]: **EE Heritage**: Electrical engineers have historically built deterministic reliability from stochastic components by designing noise margins into every circuit. AI Engineering inherits this rigor but faces an inverted problem: validating a fundamentally uncertain learned function against uncertain real-world inputs, where the "noise" is not thermal but statistical and the margin must be learned from data. \index{EE Heritage!stochastic systems}

[^fn-computer-engineering-lineage]: **Computer Engineering**: A discipline that emerged in the early 1970s when neither Electrical Engineering nor Computer Science alone could address the integrated challenges of building reliable computers. ML systems engineering follows this lineage, requiring practitioners who can reason simultaneously about algorithmic complexity, memory hierarchy constraints, and data pipeline reliability. \index{Computer Engineering!historical lineage}
...
[^fn-deterministic-testing]: **Deterministic Execution**: Traditional software follows the same computational path for a given input, producing bit-for-bit identical outputs. This allows automated test suites to validate correctness by simple comparison against a known-good result. A single bit-flip deviation is sufficient to unambiguously signal a regression failure. \index{Deterministic Execution!vs. stochastic}

[^fn-ci-cd-ml]: **CI/CD (Continuous Integration/Deployment)**: In the traditional context, CI deterministically validates fixed logic against discrete test cases, providing a binary pass/fail signal. For ML systems, this is insufficient because the validation target is a statistical pattern, not a logical absolute. A pipeline can verify code integrity while deploying a model that has silently degraded by >5% in a key metric due to data drift, forcing a shift from integration tests to ongoing validation. \index{CI/CD!ML implementation}
...
[^fn-benchmark-reference]: **Benchmark**: The term's origin as a surveyor's static reference mark explains why it fails to model dynamic production systems. The benchmark's fixed workload inherently omits the sources of degradation cited—data drift from slang, network latency, and subgroup variance—which collectively account for the 12-16 percentage point drop from the 94% lab score to 78-82% in production. \index{Benchmark!etymology}
