...
The three axes are simple to name, but their intersections create a much richer design space. Data is the fuel that supplies evidence, Algorithm is the blueprint that turns evidence into behavior, and Machine is the engine that executes that behavior. @Fig-ai-triad maps how engineering questions emerge wherever those axes overlap.

::: {#fig-ai-triad fig-env="figure" fig-pos="htb" fig-cap="**The D·A·M Intersection Landscape**: Data, Algorithm, and Machine begin as distinct diagnostic lenses, but practical ML systems live at their boundaries. Pairwise intersections ask what a system can learn from, how information moves, and how computation executes efficiently. Their center is ML systems engineering, where all three constraints must be balanced together." fig-alt="Three-circle Venn diagram labeled Data, Algorithm, and Machine. Data lists storage formats, data quality, distribution properties, and labeling. Algorithm lists loss functions, architectures, optimization mathematics, and gradients. Machine lists silicon, memory hierarchy, peak FLOP/s, and power envelope. The Data–Algorithm overlap is titled What to Learn From, the Data–Machine overlap is titled How to Move Information, and the Algorithm–Machine overlap is titled How to Execute Efficiently. The central overlap is labeled ML systems engineering."}
...
Read the figure from the outer regions inward. Data and Algorithm determine *what the system can learn from*. Data and Machine determine *how information moves*. Algorithm and Machine determine *how computation executes efficiently*. At the center, all three questions must be answered together. That center is the discipline of ML systems engineering.

The techniques named in the figure are a preview, not a checklist to memorize. They show why a local choice rarely remains local: selecting data changes the work presented to the algorithm, choosing an architecture changes memory and compute demands, and choosing a machine changes which algorithms and data paths are practical. The recurring diagnostic step throughout this book is to locate the binding region before choosing an optimization.
