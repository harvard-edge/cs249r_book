...
Machine learning systems have a *physics*. Data must move through memory hierarchies governed by bandwidth limits. Arithmetic must execute on silicon governed by power budgets. Predictions must arrive within latency windows governed by the speed of light. *When* a traditional program misbehaves, engineers trace the bug to specific lines of code; *when* a machine learning system misbehaves, there is often no bug to find—the code executes correctly, but the learned behavior is wrong because the system's behavior is defined by data it has seen rather than code someone wrote. This distinction only scratches the surface: ML systems must simultaneously manage the statistical uncertainty inherent in learned behavior and the physical constraints of executing that behavior on real hardware. The engineering principles that made traditional software reliable remain necessary but are no longer sufficient. To address this, we must cement a systematic framework for **AI Engineering**—one grounded in the physics of computation, where decisions made at the algorithm level impact the full stack from distributed system orchestration down to the silicon circuits, and conversely, where hardware constraints shape model design. Understanding this physics is what transforms ML practitioners into ML *engineers*: practitioners who can predict how systems will behave, diagnose why they fail, and reason quantitatively about the trade-offs that govern every layer of the stack.

::: {.callout-tip title="Learning Objectives"}

- Explain the Data · Algorithm · Machine (D·A·M) taxonomy and apply it to diagnose ML system bottlenecks
...
: **AI Performance Evolution Across Paradigms**: Each paradigm transition correlates with increased computational scale rather than algorithmic sophistication. Performance improved from amateur-level expert systems (2000 Elo) to superhuman foundation models (86.4% MMLU), while computational requirements grew from single CPUs to `{python} gpt4_gpu_m_str` million A100 GPU-days. Training time initially increased (hours to days) but later decreased as distributed systems enabled parallelization. {#tbl-ai-evolution-performance}
...
The D·A·M taxonomy introduced earlier\index{D·A·M taxonomy!definition}\index{D·A·M taxonomy!diagnostic framework} provides a diagnostic framework for understanding these interdependencies: **Data** is the fuel, **Algorithm** is the blueprint, and **Machine** is the engine. Without any one component, the others remain theoretical.
...
**The Systems View**: While researchers optimize for *accuracy*, systems engineers optimize for **Samples per Dollar**. This metric unifies the three axes of the D·A·M taxonomy into a single constraint equation:
...
Because this failure mode is silent, we cannot rely on crash logs to detect it. We must rely on math. Just as Patterson and Hennessy's Iron Law [@patterson2021hardware] decomposed CPU performance into fundamental components, we can decompose ML system degradation into its constituent factors. The **Degradation Equation**\index{Degradation Equation!drift measurement} captures how model performance evolves over time:
...
The additive form assumes sequential execution; in practice, systems can **overlap** these terms, transforming the sum into a max:

$$T_{pipelined} = \max\left(\frac{D_{vol}}{BW}, \frac{O}{R_{peak} \cdot \eta}\right) + L_{lat}$$
...
#| echo: false
#| fig-cap: "**Algorithmic Efficiency Trajectory.** Training efficiency factor relative to AlexNet (2012 baseline) for ImageNet classification. Each point represents a model architecture that achieves comparable accuracy with fewer computational resources. The trajectory from AlexNet (1x) through VGG, ResNet, MobileNet, and ShuffleNet to EfficientNet (`{python} algo_efficiency_max_str`x) demonstrates that algorithmic innovation has delivered a `{python} algo_efficiency_max_str`-fold reduction in required compute over eight years, independent of hardware improvements."
...
Which efficiency dimensions to prioritize depends heavily on deployment context: cloud systems optimize for throughput while edge devices optimize for power. Before examining this deployment spectrum, however, we must formalize the discipline that manages these complex trade-offs.
...
Modern ML systems operate through specialized lifecycles and deployment patterns that shape every engineering decision.
