...
[^fn-proxy-variables]: **Proxy Variables**: Features that correlate with protected attributes without directly encoding them. In ML systems, proxies emerge through statistical patterns: ZIP codes correlate with race due to residential segregation; first names correlate with gender and ethnicity; healthcare utilization correlates with socioeconomic status. Removing protected attributes from training data is insufficient because models learn these correlations from remaining features. Systems engineering implications: fairness requires either adversarial debiasing to prevent proxy learning, fairness constraints during optimization, or post-hoc threshold adjustment per demographic group.
...
Distribution shift explains why models degrade over time. But there is a second mechanism for silent failure that can occur even when the data distribution is stable: misalignment between the metric the model optimizes and the outcome the organization actually values.
...
Several mitigation approaches exist, each with distinct tradeoffs. Threshold adjustment lowers the approval threshold for Group B to equalize TPR but may increase false positives for that group. Reweighting increases the weight of Group B samples during training to give the model stronger signal about this population but may reduce overall accuracy. Adversarial debiasing trains with an adversary that prevents the model from learning group membership but adds training complexity.[^fn-adversarial-debiasing] The choice among these approaches requires stakeholder input about which tradeoffs are acceptable in the specific application context.

[^fn-adversarial-debiasing]: **Adversarial Debiasing**: An in-processing fairness technique introduced by Zhang et al. (2018) that simultaneously trains a predictor and an adversary network. The predictor learns to maximize task accuracy while the adversary attempts to predict protected attributes from the predictor's outputs. During backpropagation, gradients from the adversary are reversed, encouraging the predictor to learn representations that conceal group membership. The technique supports both demographic parity (adversary sees only predictions) and equalized odds (adversary also sees true labels). Trade-offs: adds 20-50% training time, may reduce accuracy by 1-3%, and requires careful hyperparameter tuning for stable training.
...
[^fn-lime-shap]: **LIME and SHAP**: Two dominant post-hoc explainability methods with different computational trade-offs. LIME (Local Interpretable Model-agnostic Explanations) by Ribeiro et al. (2016) fits a simple interpretable model around each prediction, offering faster computation but potentially inconsistent explanations. SHAP (SHapley Additive exPlanations) by Lundberg and Lee (2017) computes feature contributions using game-theoretic Shapley values, providing mathematically consistent explanations but with exponential worst-case complexity in the number of features. KernelSHAP approximations reduce this cost. Systems implication: SHAP may add 10-100x inference latency for explanation generation, making LIME preferable for real-time applications despite its theoretical limitations.
