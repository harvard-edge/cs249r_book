...
**The Failure**: A ProPublica investigation [@angwin2016] revealed that while the system was "calibrated" (a score of seven meant the same probability of re-offending for any group), its error rates were skewed:
...
[^fn-fairness-metric-incompatibility]: **Fairness Metric Incompatibility**: The measured disparities are a direct consequence of an impossibility theorem proving that multiple fairness metrics cannot be satisfied simultaneously when group base rates differ [@chouldechova2017]. This forces an explicit trade-off: optimizing for one metric, like equal opportunity, will degrade another, such as predictive parity. A system designer must therefore choose which fairness guarantee to violate, as it is mathematically impossible to satisfy all three. \index{Fairness Metrics!impossibility theorem}
...
**The Context**: Researchers at Mount Sinai Hospital trained a neural network to detect pneumonia in chest X-rays [@rajkomar2019]. The model achieved superhuman accuracy on the test set.
