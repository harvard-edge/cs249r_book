...
Amazon's recruiting tool achieved exactly what it was designed to achieve—and then someone discovered it was penalizing women. By every technical metric, the system worked: low error rates, fast inference, consistent predictions. The model had correctly identified a statistical pattern in the training data: historically successful hires were predominantly male. It optimized perfectly for its objective. The objective was the problem. The full story, detailed below, reveals how optimization can succeed while systems fail.

If **MLOps** (@sec-machine-learning-operations-mlops), the monitoring and retraining infrastructure examined in the previous chapter, is the control loop for *reliability*, then **Responsible Engineering**\index{Responsible Engineering!safety control loop} is the control loop for *safety*.\index{Safety!responsible engineering} Where MLOps monitors system health and triggers retraining when performance degrades, responsible engineering monitors *outcome quality* and triggers intervention when systems cause harm. This distinction matters because a model can achieve excellent accuracy, indeed can optimize flawlessly for its stated objective, and still cause systematic harm. Standard ML metrics detected no failure in Amazon's system because, by every technical measure, the system was working. The failure was not a bug in the code; it was a flaw in the specification. In systems engineering terms, the system passed *verification* (it met its stated requirements) but failed *validation* (it did not meet the user's true needs).
...
As visualized in @fig-fairness-frontier, this tradeoff forms a Pareto frontier[^fn-pareto-fairness]—and responsible engineering requires quantifying where stakeholders should position their system along it [@kleinberg2016inherent].

[^fn-pareto-fairness]: **Pareto Frontier**: As introduced in @sec-model-compression, the Pareto frontier represents all solutions where improving one objective necessarily worsens another. For ML fairness, this means configurations where you cannot increase fairness without sacrificing accuracy (or vice versa). The frontier makes explicit that fairness is a design constraint requiring stakeholder input on acceptable tradeoffs.

The following notebook illustrates how to quantify this tradeoff in a separate hiring scenario (distinct from the loan approval example above, with different disparity magnitudes to illustrate a different point).
...
# Note: gpu_rate_value, inf_gpu_rate_value defined in inference-cost-calc
...
The fallacies above share a common thread: they treat responsibility as something that can be bolted on rather than built in. Data governance provides the infrastructure that makes responsible practices enforceable at scale.
