...
# inf_tco_3yr, train_tco_3yr, inf_carbon_3yr remain as module-level variables
# and are referenced directly by TCOSummary in the next cell.
```
...
1.  **Energy Consumption**: `{python} CarbonScaleCalc.train_energy_mwh_str` MWh = 1,300,000 kWh.
2.  **Carbon Intensity**: The average US grid emits $\approx$ **`{python} CarbonScaleCalc.carbon_intensity_str` kg CO2 per kWh**.
3.  **Total Emissions**: 1,300,000$\times$ `{python} CarbonScaleCalc.carbon_intensity_str` = **`{python} CarbonScaleCalc.total_emissions_kg_str` kg CO₂** (`{python} CarbonScaleCalc.total_emissions_tons_str` metric tons).
