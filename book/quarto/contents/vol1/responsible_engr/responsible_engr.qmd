...
**The Context**: COMPAS is a risk assessment tool used in US courtrooms to predict re-offending. Judges use these scores to inform bail and sentencing decisions.
...
*   **False Positives**: Black defendants who *did not* re-offend were incorrectly flagged as high-risk at nearly twice the rate of White defendants (44.9% vs. 23.5%).
*   **False Negatives**: White defendants who *did* re-offend were incorrectly labeled as low-risk far more often than Black defendants (47.7% vs. 28.0%).
...
These cases reveal that conventional engineering success (accurate predictions, optimized metrics) can coexist with profound system failures. Before examining how to prevent such failures, reflect on your own approach to responsible design.
...
***Distribution Shift*** is the violation of the **Stationarity Assumption** ($P_{train} \neq P_{deploy}$). It is the fundamental failure mode of machine learning systems, requiring architectures that favor **Robustness** over pure **Accuracy** and operations that prioritize **Adaptation** over static deployment.
:::
...
When harm occurs, engineers need a diagnostic framework to identify the root cause. The AI Triad introduced in @sec-introduction provides exactly this structure through the *DAM taxonomy*.
...
# Gender Shades disparity analysis
error_light_male = 0.8    # %
error_dark_female = 34.7  # %
disparity_fold = error_dark_female / error_light_male
accuracy_light_male = 100 - error_light_male
accuracy_dark_female = 100 - error_dark_female

disparity_str = f"{disparity_fold:.0f}"
acc_light_str = f"{accuracy_light_male:.1f}"
acc_dark_str = f"{accuracy_dark_female:.1f}"
```

Disaggregated evaluation revealed what aggregate accuracy scores concealed. Systems reporting high overall accuracy simultaneously achieved error rates as low as `{python} error_light_male`% for light-skinned males and as high as `{python} error_dark_female`% for dark-skinned females (corresponding to accuracies of `{python} acc_light_str`% and `{python} acc_dark_str`% respectively). The aggregate metric provided no indication of this `{python} disparity_str`-fold disparity in error rates.
...
This engineering-centered practice, which we call **Responsible AI Engineering**, cannot be delegated exclusively to ethics boards or legal departments. These groups provide essential oversight but lack the technical access required to identify problems early in the development process.
...
# Statistics of representation sampling
repr_target_images = 1000
repr_group_fraction = 0.01
repr_random_total = int(repr_target_images / repr_group_fraction)
repr_multiplier = int(repr_random_total / repr_target_images)
repr_random_total_str = f"{repr_random_total:,}"
repr_multiplier_str = f"{repr_multiplier}"
```
...
$$ N_{total} = \frac{1,000}{0.01} = `{python} repr_random_total_str` \text{ images} $$
...
| **Considerations** | demographic representation. Not validated for high-stakes applications (medical diagnosis, security screening). Performance may degrade on images from underrepresented regions. |
...
# Confusion matrix values from tables above
# Group A (n=10,000)
a_tp, a_fn = 4_500, 500
a_fp, a_tn = 1_000, 4_000
...
# Demographic parity: approval rate = (TP + FP) / total
a_approval_pct = (a_tp + a_fp) / a_total * 100
b_approval_pct = (b_tp + b_fp) / b_total * 100
dp_disparity_pp = a_approval_pct - b_approval_pct

# Equal opportunity: TPR = TP / (TP + FN)
a_tpr_pct = a_tp / (a_tp + a_fn) * 100
b_tpr_pct = b_tp / (b_tp + b_fn) * 100
tpr_disparity_pp = a_tpr_pct - b_tpr_pct

# Equalized odds: FPR = FP / (FP + TN)
...
# Format strings
a_approval_str = f"{a_approval_pct:.0f}"
b_approval_str = f"{b_approval_pct:.0f}"
dp_disparity_str = f"{dp_disparity_pp:.0f}"
a_tpr_str = f"{a_tpr_pct:.0f}"
b_tpr_str = f"{b_tpr_pct:.0f}"
tpr_disparity_str = f"{tpr_disparity_pp:.0f}"
a_fpr_str = f"{a_fpr_pct:.0f}"
b_fpr_str = f"{b_fpr_pct:.0f}"
a_fnr_str = f"{a_fnr_pct:.0f}"
b_fnr_str = f"{b_fnr_pct:.0f}"
```
...
Equal opportunity requires equal true positive rates among qualified applicants. Group A achieves a TPR of 4,500 / (4,500 + 500) = `{python} a_tpr_str`%, meaning `{python} a_tpr_str`% of applicants who would repay receive approval. Group B achieves only 600 / (600 + 400) = `{python} b_tpr_str`% TPR. This `{python} tpr_disparity_str` percentage point disparity means qualified applicants from Group B face substantially higher rejection rates than equally qualified applicants from Group A.

Equalized odds requires both equal true positive rates and equal false positive rates. Group A shows an FPR of 1,000 / (1,000 + 4,000) = `{python} a_fpr_str`%, and Group B shows 200 / (200 + 800) = `{python} b_fpr_str`%. While false positive rates are equal, the true positive rate disparity means equalized odds is violated.
...
def compute_fairness_metrics(confusion_matrix):
    tp, fp, tn, fn = (confusion_matrix[k] for k in ["TP", "FP", "TN", "FN"])
    total = tp + fp + tn + fn
    return {
        "approval_rate": (tp + fp) / total,         # Demographic parity
        "tpr": tp / (tp + fn) if (tp + fn) else 0,  # Equal opportunity
        "fpr": fp / (fp + tn) if (fp + tn) else 0,  # Equalized odds (with TPR)
    }
...
for metric in ["approval_rate", "tpr", "fpr"]:
    ...
    if disparity > FAIRNESS_THRESHOLD:  # e.g., 0.05 for high-stakes
...
Several mitigation approaches exist, each with distinct tradeoffs. Threshold adjustment lowers the approval threshold for Group B to equalize TPR but may increase false positives for that group. Reweighting increases the weight of Group B samples during training to give the model stronger signal about this population but may reduce overall accuracy. Adversarial debiasing trains with an adversary that prevents the model from learning group membership but adds training complexity.[^fn-adversarial-debiasing] The choice among these approaches requires stakeholder input about which tradeoffs are acceptable in the specific application context. How, then, should engineers present these tradeoffs to stakeholders? The answer lies in making the tradeoffs explicit and quantifiable.
...
The techniques from earlier chapters directly serve responsibility goals. Quantization (@sec-model-compression) reduces compute by 2–4× with minimal accuracy impact. Pruning removes 50–90% of parameters. Knowledge distillation typically achieves 5–20× compression while retaining 90–95% of the original accuracy. Hardware acceleration (@sec-ai-acceleration) achieves 10–100× better energy efficiency than general-purpose processors.
...
# Training costs
gpu_rate = 4.00  # $/GPU-hour
data_prep_hrs = 100
hyperparam_hrs = 500
train_hrs = 200

data_prep_cost = data_prep_hrs * gpu_rate
hyperparam_cost = hyperparam_hrs * gpu_rate
train_cost = train_hrs * gpu_rate
total_train_cost = data_prep_cost + hyperparam_cost + train_cost

# Inference costs
users_daily = 10_000_000
recs_per_user = 20
inferences_daily = users_daily * recs_per_user
inference_ms = 10
gpu_seconds_daily = inferences_daily * inference_ms / 1000
gpus_needed = gpu_seconds_daily / (24 * 3600)
gpu_inf_rate = 2.50  # $/GPU-hour
annual_inf_cost = gpus_needed * 24 * 365 * gpu_inf_rate

# 3-year lifecycle
retrain_quarters = 12  # quarterly over 3 years
lifecycle_train = total_train_cost * (retrain_quarters / 1)  # ~$10K approximate
# Actually: quarterly retraining = 12 retrains × ~$800 per retrain ≈ $10K
# The text says "$10,000" for training over 3 years — let's compute it properly
retrains_3yr = 12
retrain_cost_each = total_train_cost  # full retrain each quarter
lifecycle_train_cost = retrain_cost_each * (retrains_3yr / 4)  # ~$10K approximate
# The text simplifies: quarterly retrain at ~$800 each = $9,600 ≈ $10,000
lifecycle_inf_cost = annual_inf_cost * 3
inf_train_ratio = lifecycle_inf_cost / 10_000  # using the rounded $10K

data_prep_str = f"{data_prep_cost:,.0f}"
hyperparam_str = f"{hyperparam_cost:,.0f}"
train_cost_str = f"{train_cost:,.0f}"
total_train_str = f"{total_train_cost:,.0f}"
inferences_m = f"{inferences_daily / 1e6:.0f}"
gpus_str = f"{gpus_needed:.0f}"
annual_inf_str = f"{annual_inf_cost:,.0f}"
lifecycle_inf_str = f"{lifecycle_inf_cost / 1e6:.1f}"
ratio_str = f"{inf_train_ratio:.0f}"
```

Consider a concrete example of a recommendation system serving 10 million users daily. Training costs appear substantial: data preparation consumes 100 GPU-hours at approximately $4 per hour ($`{python} data_prep_str`), hyperparameter search across multiple configurations requires 500 GPU-hours ($`{python} hyperparam_str`), and the final training run uses 200 GPU-hours ($`{python} train_cost_str`). Total training cost reaches approximately $`{python} total_train_str`.

Inference costs dominate. With 10 million users each receiving 20 recommendations per day, the system serves `{python} inferences_m` million inferences daily. Assuming 10 milliseconds per inference on GPU hardware, the system requires approximately `{python} gpus_str` GPUs running continuously. At $2.50 per GPU-hour, annual GPU costs reach $`{python} annual_inf_str`.

Over a three-year operational period, quarterly retraining produces total training costs of approximately $10,000, while inference costs over the same period total $`{python} lifecycle_inf_str` million. The `{python} ratio_str`:1 ratio between inference and training costs is typical for production systems and has significant implications for engineering priorities.
...
gpu_rate = 4.0
inf_gpu_rate = 2.50
carbon_per_gpu_hr = 0.16 # kg

# Training inputs
t_data_prep_hrs = 100
t_hparam_exp = 50
t_hparam_cost_per_exp = 40
t_final_hrs = 200
t_cycles = 12
t_years = 3
t_cycles_per_year = t_cycles // t_years

# Training (1 cycle)
t_data_prep = t_data_prep_hrs * gpu_rate
t_hparam = t_hparam_exp * t_hparam_cost_per_exp
t_final = t_final_hrs * gpu_rate
t_subtotal = t_data_prep + t_hparam + t_final

# Training carbon (per cycle)
t_data_prep_carbon = t_data_prep_hrs * carbon_per_gpu_hr
t_hparam_hrs = t_hparam / gpu_rate  # convert cost back to hours
t_hparam_carbon = t_hparam_hrs * carbon_per_gpu_hr
t_final_carbon = t_final_hrs * carbon_per_gpu_hr
t_subtotal_carbon = t_data_prep_carbon + t_hparam_carbon + t_final_carbon

# Training totals
t_total = t_subtotal * t_cycles
t_total_carbon = t_subtotal_carbon * t_cycles

# Inference inputs
i_users = 10_000_000
i_queries_per_user = 20
i_daily_q = i_users * i_queries_per_user
i_latency = 0.01  # seconds

# Inference (3 years)
i_gpu_sec_day = i_daily_q * i_latency
i_gpu_hr_day = i_gpu_sec_day / 3600
i_daily_carbon = i_gpu_hr_day * carbon_per_gpu_hr
i_annual_cost = i_gpu_hr_day * 365 * inf_gpu_rate
i_annual_carbon = i_gpu_hr_day * 365 * carbon_per_gpu_hr
i_total = i_annual_cost * 3
i_carbon = i_annual_carbon * 3

# Operations inputs (annual)
o_monitor_annual = 50000
o_oncall_annual = 100000
o_incident_annual = 20000

# Operations (3 years)
o_monitor = o_monitor_annual * 3
o_oncall = o_oncall_annual * 3
o_incident = o_incident_annual * 3
o_total = o_monitor + o_oncall + o_incident

# Summary
total_tco = t_total + i_total + o_total
total_carbon = t_total_carbon + i_carbon
p_train = t_total / total_tco * 100
p_inf = i_total / total_tco * 100
p_ops = o_total / total_tco * 100

# Training table strings
t_data_prep_calc_str = f"{t_data_prep_hrs} GPU-hr × ${gpu_rate:.0f} = ${t_data_prep:,.0f}"
t_data_prep_carbon_str = f"{t_data_prep_carbon:.0f} kg"
t_hparam_calc_str = f"{t_hparam_exp} × ${t_hparam_cost_per_exp} = ${t_hparam:,.0f}"
t_hparam_carbon_str = f"{t_hparam_carbon:.0f} kg"
t_final_calc_str = f"{t_final_hrs} GPU-hr × ${gpu_rate:.0f} = ${t_final:,.0f}"
t_final_carbon_str = f"{t_final_carbon:.0f} kg"
t_subtotal_str = f"${t_subtotal:,.0f}"
t_subtotal_carbon_str = f"{t_subtotal_carbon:.0f} kg"
t_cycles_calc_str = f"{t_cycles_per_year}/year × {t_years} years = {t_cycles}"
t_total_str = f"${t_total:,.0f}"
t_total_carbon_str = f"{t_total_carbon:,.0f} kg"

# Inference table strings
i_daily_q_calc_str = f"{i_users/1e6:.0f}M × {i_queries_per_user} = {i_daily_q/1e6:.0f}M"
i_gpu_sec_calc_str = f"{i_daily_q/1e6:.0f}M × {i_latency} s = {i_gpu_sec_day/1e6:.0f}M sec"
i_gpu_hr_day_str = f"{i_gpu_hr_day:.0f} GPU-hr"
i_daily_carbon_str = f"{i_daily_carbon:.0f} kg"
i_annual_calc_str = f"{i_gpu_hr_day:.0f} × 365 × ${inf_gpu_rate:.2f} = ${i_annual_cost/1e3:.0f}K"
i_annual_carbon_str = f"{i_annual_carbon:,.0f} kg"
i_total_str = f"${i_total/1e6:.2f}M"
i_carbon_str = f"{i_carbon:,.0f} kg"

# Operations table strings
o_monitor_annual_str = f"${o_monitor_annual/1e3:.0f}K"
o_monitor_str = f"${o_monitor/1e3:.0f}K"
o_oncall_annual_str = f"${o_oncall_annual/1e3:.0f}K"
o_oncall_str = f"${o_oncall/1e3:.0f}K"
o_incident_annual_str = f"${o_incident_annual/1e3:.0f}K"
o_incident_str = f"${o_incident/1e3:.0f}K"
o_total_str = f"${o_total/1e3:.0f}K"

# Summary table strings
t_total_k_str = f"${t_total/1e3:.0f}K"
t_total_carbon_tons_str = f"{t_total_carbon/1000:.1f} tons"
i_carbon_tons_str = f"{i_carbon/1000:.1f} tons"
total_tco_str = f"${total_tco/1e6:.2f}M"
total_carbon_tons_str = f"~{total_carbon/1000:.0f} tons"

# Caption strings
gpu_rate_str = f"${gpu_rate:.0f}"
t_cycles_str = f"{t_cycles}"
p_train_str = f"{p_train:.0f}"
p_inf_str = f"{p_inf:.0f}"
p_ops_str = f"{p_ops:.0f}"
i_daily_q_m_str = f"{i_daily_q/1e6:.0f}"
i_latency_ms_str = f"{i_latency*1000:.0f}"
i_annual_k_str = f"${i_annual_cost/1e3:.0f}K"
i_users_m_str = f"{i_users/1e6:.0f}"
```
...
| **Retraining frequency** | cycles/year × years | `{python} t_cycles_calc_str` | `{python} t_cycles` |
...
# TCO summary derived values (from tco-calc above)
inf_train_ratio = i_total / t_total
carbon_inference_tons = i_carbon / 1000
quant_reduction_pct = 0.20
quant_savings_k = i_total * quant_reduction_pct / 1000
quant_carbon_tons = carbon_inference_tons * quant_reduction_pct

inf_train_ratio_str = f"{inf_train_ratio:.0f}"
quant_savings_str = f"{quant_savings_k:.0f}"
quant_carbon_str = f"{quant_carbon_tons:.0f}"
```
...
The fallacies examined above often stem from inadequate governance infrastructure. Without systematic data management, even well-intentioned teams cannot implement responsible engineering practices consistently. Understanding these failure modes clarifies why governance is not bureaucratic overhead but engineering necessity.
...
* **Correctness is insufficient**: A model can achieve 95% accuracy while systematically disadvantaging specific populations. Aggregate metrics obscure disparities that disaggregated evaluation reveals.
...
* **Efficiency and responsibility align**: A 4× more efficient model uses 4× less energy, costs 4× less, and enables 4× more organizations to deploy. Inefficient systems impose unnecessary resource costs and limit accessibility.
...
* **Silent bias requires proactive detection**: Systems causing harm continue operating without alerts. Outcome distributions across demographic groups must be monitored, not just aggregate accuracy.
