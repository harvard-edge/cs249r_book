...
**The Quantification** (conceptual, assuming normalized metrics on a common scale):
...
: **Gender Shades Facial Recognition Error Rates**: Disaggregated evaluation reveals that aggregate accuracy metrics conceal severe performance disparities. Systems that appear highly accurate overall show error rates varying by more than `{python} disparity_str`× across demographic groups. Worst-case results across systems studied; source: @buolamwini2018gender. {#tbl-gender-shades-results}
...
: **Confusion Matrix for Group A (Majority)**: Loan approval outcomes for `{python} a_total_str` applicants from the majority demographic group. The 90% true positive rate (4,500 approved of 5,000 qualified) and 20% false positive rate establish the baseline for fairness comparison. {#tbl-confusion-group-a}
...
: **Confusion Matrix for Group B (Minority)**: Loan approval outcomes for `{python} b_total_str` applicants from the minority demographic group. The 60% true positive rate (600 approved of 1,000 qualified) reveals a 30 percentage point disparity compared to Group A, indicating the model applies stricter criteria to minority applicants. {#tbl-confusion-group-b}
...
: **Edge Deployment Constraints**: Power and latency requirements across four deployment contexts. Smartphones allow `{python} smart_power_str` and `{python} smart_latency_str` latency for photo enhancement and voice assistants. IoT sensors operate at `{python} iot_power_str` with `{python} iot_latency_str` tolerance for anomaly detection. Embedded cameras require `{python} cam_power_str` at `{python} cam_latency_str` (30 FPS) for real-time object detection. Wearables budget `{python} wear_power_str` with `{python} wear_latency_str` latency for health monitoring. These concrete constraints transform abstract efficiency discussions into engineering requirements. {#tbl-edge-deployment-constraints}
...
: **Operational Cost Calculation**: Operational costs include monitoring infrastructure (`{python} o_monitor_annual_str`/year), on-call engineering at 0.5 FTE (`{python} o_oncall_annual_str`/year), and incident response reserves (`{python} o_incident_annual_str`/year). The `{python} o_total_str` three-year total represents `{python} p_ops_str`% of TCO and often surprises teams focused primarily on compute costs. These estimates represent minimum staffing; production systems at this scale typically require 2–5× more engineering support. These expenses persist regardless of model performance and grow with system complexity. {#tbl-tco-operations}
...
##### Fallacy: *Responsible AI is primarily a legal compliance issue.* {.unnumbered}
