...
3.  **Total Emissions**: $1,300,000 \times `{python} carbon_intensity_str` = \mathbf{`{python} total_emissions_kg_str` \text{ kg CO2}}$ (`{python} total_emissions_tons_str` metric tons).
4.  **Comparison**: A typical passenger car emits $\approx$ `{python} car_annual_tons_str` metric tons of CO2 per year.
