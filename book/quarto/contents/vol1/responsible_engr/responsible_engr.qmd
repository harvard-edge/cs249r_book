...
The Amazon recruiting tool case illustrates this gap. In 2014, Amazon developed an AI system to automate resume screening for technical positions, training it on historical hiring data\index{Bias!historical data encoding} spanning ten years of resumes submitted to the company. By 2015, the company discovered the system exhibited gender bias\index{Bias!gender discrimination} in candidate ratings [@dastin2018].
...
***Distribution shift***\index{Distribution Shift!definition} is the deployed-model violation of the stationarity assumption\index{Stationarity Assumption!distribution shift} $(P_0 \neq P_t)$ that underpins supervised learning. It is the umbrella term for a family of drift types: data drift\index{Data Drift!definition} (see @sec-ml-operations) occurs when $p(x)$ shifts while $p(y \mid x)$ remains stable; concept drift\index{Concept Drift!definition} occurs when $p(y \mid x)$ itself shifts.
