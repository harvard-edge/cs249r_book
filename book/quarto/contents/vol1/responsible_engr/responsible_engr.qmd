...
\noindent
![](images/png/cover_responsible_engr_blueprint.png){fig-alt="Isometric governed ML system chamber with fairness checks, safety rails, audit records, privacy boundaries, and incident response paths around a deployed pipeline."}
...
class TestingConstraintAnchor:
    ...
    minority_pct_str = fmt_percent(minority_pct / 100, precision=0, commas=False, style='prose')
...
class GenderShadesDisparity:
    ...
    error_light_female_str = fmt_percent(err_light_female / 100, precision=1, commas=False, style='symbol')
    error_dark_male_str = fmt_percent(round(int(err_dark_male)) / 100, precision=0, commas=False, style='symbol')
...
| **Dark-skinned females**  | `{python} GenderShadesDisparity.error_dark_female_tbl_str` |       `{python} GenderShadesDisparity.disparity_factor_mult_str` higher        |
...
class LoanFairness:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a_approval_tbl_str = fmt_percent(a_app_pct / 100, precision=0, commas=False, style='symbol')
    b_approval_tbl_str = fmt_percent(b_app_pct / 100, precision=0, commas=False, style='symbol')
    dp_disparity_str = fmt_pp(dp_disparity, precision=0, attributive=True)
    dp_disparity_tbl_str = fmt_pp(dp_disparity, precision=0, attributive=True, style="symbol")
    a_tpr_str = fmt_percent(a_tpr_pct / 100, precision=0, commas=False, style='prose')
    a_tpr_tbl_str = fmt_percent(a_tpr_pct / 100, precision=0, commas=False, style='symbol')
    b_tpr_str = fmt_percent(b_tpr_pct / 100, precision=0, commas=False, style='prose')
    b_tpr_tbl_str = fmt_percent(b_tpr_pct / 100, precision=0, commas=False, style='symbol')
    tpr_disparity_str = fmt_pp(tpr_disparity, precision=0, attributive=True)
    tpr_disparity_tbl_str = fmt_pp(tpr_disparity, precision=0, attributive=True, style="symbol")
    a_fpr_str = fmt_percent(a_fpr_pct / 100, precision=0, commas=False, style='prose')
    a_fpr_tbl_str = fmt_percent(a_fpr_pct / 100, precision=0, commas=False, style='symbol')
    b_fpr_tbl_str = fmt_percent(b_fpr_pct / 100, precision=0, commas=False, style='symbol')
    ...
    a_approval_eq = fmt_math(rf"({a_tp:,} + {a_fp:,}) / {a_total:,} = {a_app_pct:.0f}\%")
    b_approval_eq = fmt_math(rf"({b_tp:,} + {b_fp:,}) / {b_total:,} = {b_app_pct:.0f}\%")
    a_tpr_eq = fmt_math(rf"{a_tp:,} / ({a_tp:,} + {a_fn:,}) = {a_tpr_pct:.0f}\%")
    b_tpr_eq = fmt_math(rf"{b_tp:,} / ({b_tp:,} + {b_fn:,}) = {b_tpr_pct:.0f}\%")
    a_fpr_eq = fmt_math(rf"{a_fp:,} / ({a_fp:,} + {a_tn:,}) = {a_fpr_pct:.0f}\%")
    b_fpr_eq = fmt_math(rf"{b_fp:,} / ({b_fp:,} + {b_tn:,}) = {b_fpr_pct:.0f}\%")
    fpr_disparity_str = fmt_pp(fpr_disparity, precision=0, attributive=True, style="symbol")
...
**Demographic parity**\index{Demographic Parity!definition}\index{Fairness Metrics!demographic parity} requires equal approval rates across groups. Group A receives approval at a rate of `{python} LoanFairness.a_approval_eq`, while Group B receives approval at `{python} LoanFairness.b_approval_eq`. The `{python} LoanFairness.dp_disparity_str` disparity indicates unequal treatment in approval decisions.

**Equal opportunity**\index{Equal Opportunity!definition}\index{Fairness Metrics!equal opportunity} requires equal true positive rates\index{True Positive Rate!fairness computation} among qualified applicants. Group A achieves a TPR of `{python} LoanFairness.a_tpr_eq`, meaning `{python} LoanFairness.a_tpr_str` of applicants who would repay receive approval. Group B achieves only `{python} LoanFairness.b_tpr_eq`. This `{python} LoanFairness.tpr_disparity_str` disparity means qualified applicants from Group B face substantially higher rejection rates than equally qualified applicants from Group A.

**Equalized odds**\index{Equalized Odds!definition}\index{Fairness Metrics!equalized odds}[^fn-equalized-odds-postprocessing] requires both equal true positive rates and equal false positive rates. Group A shows an FPR of `{python} LoanFairness.a_fpr_eq`, and Group B shows `{python} LoanFairness.b_fpr_eq`. While false positive rates are equal, the true positive rate disparity means equalized odds is violated.
...
class TCOCalc:
    ...
    p_total_str = fmt_percent(p_total / 100, precision=0, commas=False, style='symbol')
...
| **Operations** |  `{python} TCOCalc.o_total_usd_str`  |  `{python} TCOCalc.p_ops_tbl_str`  |                     -                     |
...
* **Efficiency is a social constraint**: A 4$\times$ more efficient model uses 4$\times$ less energy, costs 4$\times$ less, and broadens who can deploy it. Because inference dominates total cost by `{python} ResponsibleTcoRecap.inf_train_ratio_str`:1 over training, per-query optimization is responsible engineering.
