...
| **(Compute Beast)**   | of minority groups in ImageNet)         | demographic group                    | light-skinned males, 65% on             |
|                       |                                         |                                      | dark-skinned females [@buolamwini2018]  |
+-----------------------+-----------------------------------------+--------------------------------------+-----------------------------------------+
...
| **(Sparse Scatter)**  | (popular items get more data)           | categories; **supplier diversity**   | same content to similar users, reducing |
...
| **(Tiny Constraint)** | (trained on clean audio, deployed       | environment and speaker accent       | accented speech; wake-word triggers on  |
|                       | in noisy real-world environments)       |                                      | TV audio in some languages              |
+-----------------------+-----------------------------------------+--------------------------------------+-----------------------------------------+
...
# Training (1 cycle)
t_data_prep = 100 * gpu_rate
t_hparam = 50 * 40 # 50 experiments at $40
t_final = 200 * gpu_rate
t_subtotal = t_data_prep + t_hparam + t_final
t_cycles = 12
t_total = t_subtotal * t_cycles
t_carbon = t_total / gpu_rate * carbon_per_gpu_hr

# Inference (3 years)
i_daily_q = 200e6
i_latency = 0.01
i_gpu_hr_day = (i_daily_q * i_latency) / 3600
i_annual_cost = i_gpu_hr_day * 365 * inf_gpu_rate
i_total = i_annual_cost * 3
i_carbon = (i_gpu_hr_day * 365 * 3) * carbon_per_gpu_hr
...
+---------------------------------+-------------------------------+------------------------+---------------------+
| **Subtotal per training cycle** |                               | **`{python} t_subtotal_str`** | **128 kg**          |
+---------------------------------+-------------------------------+------------------------+---------------------+
| **Retraining frequency**        | cycles/year × years           | 4/year × 3 years = 12  | 12                  |
+---------------------------------+-------------------------------+------------------------+---------------------+
| **Total training cost**         | subtotal x cycles             | **`{python} t_total_str`** | **1,536 kg**        |
+---------------------------------+-------------------------------+------------------------+---------------------+
...
+---------------------------+----------------------+---------------------------+---------------------+
| **3-year inference cost** | annual x 3           | **`{python} i_total_str`** | **97,410 kg**       |
+---------------------------+----------------------+---------------------------+---------------------+
...
+:===============+================:+===============:+==================:+
| **Training**   | `{python} f"${t_total/1e3:.0f}K"` | `{python} f"{p_train:.0f}%"` | 1.5 tons          |
+----------------+-----------------+----------------+-------------------+
| **Inference**  | `{python} i_total_str` | `{python} f"{p_inf:.0f}%"` | 97.4 tons         |
+----------------+-----------------+----------------+-------------------+
| **Operations** | `{python} o_total_str` | `{python} f"{p_ops:.0f}%"` | -                 |
+----------------+-----------------+----------------+-------------------+
| **Total TCO**  | **`{python} total_tco_str`** | 100%           | **~99 tons**      |
+----------------+-----------------+----------------+-------------------+

: **Total Cost of Ownership Summary**: Three-year TCO of $2.07M breaks down as: training $38K (2%), inference $1.52M (74%), and operations $510K (24%). The `{python} inf_train_ratio_str`:1 ratio between inference and training costs is typical for production systems serving 10 million daily users. A 20% reduction in inference latency through quantization would save $`{python} quant_savings_str`K and approximately `{python} quant_carbon_str` tons of CO2, easily justifying the optimization engineering investment. {#tbl-tco-summary}
...
We have journeyed from the bit-level physics of quantization to the societal-level physics of fairness. In @sec-conclusion, we assemble these pieces into a coherent philosophy of engineering excellence, synthesizing the principles established throughout this volume.
