...
::: {.column-margin}
![](images/svg/vol1_responsible_engr_margin_001.svg){width="100%" fig-alt="Margin illustration showing Random sampling versus targeted stratified evaluation for a 1 percent subgroup."}
...
::: {.column-margin}
![](images/svg/vol1_responsible_engr_margin_002.svg){width="100%" fig-alt="Margin illustration showing Automation reliability rises while human vigilance falls."}
...
class GDPRReviewLoad:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    daily_decisions_str = fmt_count(daily_decisions, scale="M", precision=0, commas=False)
...
class EdgeEfficiencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    smart_power_str   = fmt_power(smart_power, unit=watt, precision=0, commas=False)
    smart_latency_str = fmt_latency(smart_latency, unit=ms, precision=0, commas=False)
    iot_power_str     = fmt_power(iot_power, unit=milliwatt, precision=0, commas=False)
    iot_latency_str   = fmt_time(iot_latency, second, precision=0, commas=False, style='word')
    cam_power_str     = fmt_power(cam_power, unit=watt, precision=0, commas=False)
    cam_latency_str   = fmt_latency(cam_latency, unit=ms, precision=0, commas=False)
    wear_power_str    = fmt_power(wear_power, unit=milliwatt, precision=0, commas=False)
    wear_latency_str  = fmt_latency(wear_latency, unit=ms, precision=0, commas=False)

    mv2_params_str  = fmt_params(mv2_params, scale='M', precision=1, commas=False)
    mv2_power_str   = fmt_power(mv2_power, unit=watt, precision=1, commas=False)
    mv2_latency_str = fmt_latency(mv2_latency, unit=ms, precision=0, commas=False)

    eff_params_str  = fmt_params(eff_params, scale='M', precision=1, commas=False)
    eff_power_str   = fmt_power(eff_power, unit=watt, precision=1, commas=False)
    eff_latency_str = fmt_latency(eff_latency, unit=ms, precision=0, commas=False)

    rn50_params_str  = fmt_params(rn50_params, scale='M', precision=1, commas=False)
    rn50_power_str   = fmt_power(rn50_power, unit=watt, precision=1, commas=False)
    rn50_latency_str = fmt_latency(rn50_latency, unit=ms, precision=0, commas=False)

    tiny_params_str  = fmt_params(tiny_params, scale='K', precision=0, commas=False)
    tiny_power_str   = fmt_power(tiny_power, unit=milliwatt, precision=0, commas=False)
...
from mlsysim.fmt import fmt, fmt_count, fmt_usd, fmt_time, fmt_int, check
...
class InferenceCostCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    data_prep_str       = fmt_usd(data_prep_cost, precision=0, commas=True)
    hyperparam_str      = fmt_usd(hyperparam_cost, precision=0, commas=True)
    train_cost_str      = fmt_usd(train_cost, precision=0, commas=True)
    total_train_str     = fmt_usd(total_train_cost, precision=0, commas=True)
    lifecycle_train_str = fmt_usd(lifecycle_train_cost, precision=0, commas=True)
    inferences_m_str    = fmt_count(inferences_daily_value, scale="M", precision=0, commas=False)
    gpus_str            = fmt_count(gpus_needed, precision=1, commas=False, label="GPU", allow_fractional=True)
    annual_inf_str      = fmt_usd(annual_inf_cost, precision=0, commas=True)
    lifecycle_inf_str   = fmt_usd(lifecycle_inf_cost, precision=1, commas=False, scale="M")
    ratio_str           = fmt_int(round(inf_train_ratio_scalar), commas=False)
    ...
    users_daily_m_str   = fmt_count(users_daily_value, scale="M", precision=0, commas=False)
...
Consider a concrete example of a recommendation system serving `{python} InferenceCostCalc.users_daily_m_str` users daily. Training costs appear considerable: data preparation consumes `{python} InferenceCostCalc.data_prep_hrs_str` at approximately `{python} InferenceCostCalc.gpu_rate_input_str` (`{python} InferenceCostCalc.data_prep_str`), hyperparameter search across multiple configurations requires `{python} InferenceCostCalc.hyperparam_hrs_str` (`{python} InferenceCostCalc.hyperparam_str`), and the final training run uses `{python} InferenceCostCalc.train_hrs_str` (`{python} InferenceCostCalc.train_cost_str`). Total training cost reaches approximately `{python} InferenceCostCalc.total_train_str`.

Inference costs dominate. With `{python} InferenceCostCalc.users_daily_m_str` users each receiving `{python} InferenceCostCalc.recs_per_user_str` recommendations per day, the system serves `{python} InferenceCostCalc.inferences_m_str` inferences daily. Assuming `{python} InferenceCostCalc.inference_ms_str` per inference on GPU hardware, the system requires approximately `{python} InferenceCostCalc.gpus_str` running continuously. At `{python} InferenceCostCalc.gpu_inf_rate_str`, annual GPU costs reach `{python} InferenceCostCalc.annual_inf_str`.

Over a three-year operational period, quarterly retraining produces total training costs of approximately `{python} InferenceCostCalc.lifecycle_train_str`, while inference costs over the same period total `{python} InferenceCostCalc.lifecycle_inf_str`. The `{python} InferenceCostCalc.ratio_str`:1 ratio between inference and training costs is typical for production systems, directing optimization effort toward inference latency and serving efficiency rather than training speed.
...
class CarbonFactor:
    ...
    carbon_per_gpu_hr_str = fmt_emissions(carbon_per_gpu_hr, unit=kg, precision=2, commas=False)
```
...
*   **Conversion factor**: (`{python} CarbonFactor.gpu_power_kw_str` $\times$ 1 hour) $\times$ `{python} CarbonFactor.carbon_intensity_str` = `{python} CarbonFactor.carbon_per_gpu_hr_str` CO~2~eq per GPU-hour.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_count, fmt_emissions, fmt_int, fmt_usd, check, MarkdownStr, fmt_percent, fmt_qty, fmt_qty_int
from mlsysim import Infrastructure
from mlsysim.core.constants import (
    THOUSAND,
    MILLION,
...
class TCOCalc:
    ...
    t_subtotal_str = fmt_usd(train_cost_cycle, precision=0, commas=True)
    t_total_str = fmt_usd(train_tco_3yr, precision=0, commas=True)
    t_total_k_str = fmt_usd((round(train_tco_3yr.to(USD).magnitude/THOUSAND)) * THOUSAND * USD, commas=False, scale="K")
    ...
    t_data_prep_carbon_str = fmt_emissions(t_data_prep_time * carbon_per_gpu_hour, unit=kg, precision=0, commas=False)
    t_hparam_carbon_str = fmt_emissions(hparam_gpu_time * carbon_per_gpu_hour, unit=kg, precision=0, commas=False)
    t_final_carbon_str = fmt_emissions(t_final_time * carbon_per_gpu_hour, unit=kg, precision=0, commas=False)
    t_subtotal_carbon_str = fmt_emissions(train_carbon_cycle, unit=kg, precision=0, commas=False)
    t_total_carbon_str = fmt_emissions(train_carbon_3yr, unit=kg, precision=0, commas=True)
    t_total_carbon_tons_str = fmt_qty(train_carbon_3yr, metric_ton, precision=1, commas=False, unit_label="tons")
    ...
    i_daily_carbon_str = fmt_emissions(inf_carbon_day, unit=kg, precision=1, commas=False)
    ...
    i_annual_carbon_str = fmt_emissions(inf_carbon_3yr/3, unit=kg, precision=1, commas=True)
    i_total_str = fmt_usd(inf_tco_3yr, precision=2, commas=False, scale="M")
    i_carbon_str = fmt_emissions(inf_carbon_3yr, unit=kg, precision=1, commas=True)
    i_carbon_tons_str = fmt_qty(inf_carbon_3yr, metric_ton, precision=1, commas=False, unit_label="tons")
    i_daily_queries_m_str = fmt_count(inf_daily_total, scale="M", precision=0, commas=False)
    i_latency_ms_str = fmt_qty(i_latency, ms, precision=0, commas=False)
    i_annual_cost_k_str = fmt_usd((round(inf_cost_year.to(USD).magnitude / THOUSAND)) * THOUSAND * USD, commas=False, scale="K")
    i_total_m_str = fmt_usd(inf_tco_3yr, precision=2, commas=False, scale="M")

    # Operations table strings — currency values use fmt() prefixes/suffixes.
    o_monitor_annual_str = fmt_usd(o_monitor_yr, precision=0, commas=False, scale="K")
    o_monitor_str = fmt_usd(o_monitor_yr*3, precision=0, commas=False, scale="K")
    o_oncall_annual_str = fmt_usd(o_oncall_yr, precision=0, commas=False, scale="K")
    o_oncall_str = fmt_usd(o_oncall_yr*3, precision=0, commas=False, scale="K")
    o_incident_annual_str = fmt_usd(o_incident_yr, precision=0, commas=False, scale="K")
    o_incident_str = fmt_usd(o_incident_yr*3, precision=0, commas=False, scale="K")
    o_total_str = fmt_usd(o_total_3yr, precision=0, commas=False, scale="K")
    o_monitor_annual_k_str = fmt_usd(o_monitor_yr.to(USD).magnitude, precision=0, commas=False, scale="K", per="year")
    o_oncall_annual_k_str = fmt_usd(o_oncall_yr.to(USD).magnitude, precision=0, commas=False, scale="K", per="year")
    o_incident_annual_k_str = fmt_usd(o_incident_yr.to(USD).magnitude, precision=0, commas=False, scale="K", per="year")
    o_total_k_str = fmt_usd(o_total_3yr, precision=0, commas=False, scale="K")
    ...
    total_carbon_tons_str = fmt_qty_int(total_carbon_kg, metric_ton, commas=False, approx=True, unit_label="tons")
...
: **Training Cost Calculation**: Training costs accumulate through initial development (`{python} TCOCalc.t_subtotal_str` per cycle) and quarterly retraining over a three-year operational period. Data preparation, hyperparameter search, and final training consume GPU hours at `{python} TCOCalc.train_gpu_rate_str`, totaling `{python} TCOCalc.t_total_str` across `{python} TCOCalc.t_cycles_str` training cycles. Despite appearing substantial, training represents only `{python} TCOCalc.p_train_str` of total cost of ownership. {#tbl-tco-training tbl-colwidths="[25,32,23,20]"}
...
: **Operational Cost Calculation**: Operational costs include monitoring infrastructure (`{python} TCOCalc.o_monitor_annual_k_str`), on-call engineering at 0.5 FTE (`{python} TCOCalc.o_oncall_annual_k_str`), and incident response reserves (`{python} TCOCalc.o_incident_annual_k_str`). The `{python} TCOCalc.o_total_k_str` three-year total represents `{python} TCOCalc.p_ops_str` of TCO and often surprises teams focused primarily on compute costs. These estimates represent minimum staffing; production systems at this scale typically require 2--5$\times$ more engineering support. These expenses persist regardless of model performance and grow with system complexity. {#tbl-tco-operations tbl-colwidths="[34,33,33]"}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_usd, check, fmt_percent, fmt_qty_int
...
class TCOSummary:
    ...
    quant_carbon_str = fmt_qty_int(savings_carbon, metric_ton, commas=False)
...
: **Total Cost of Ownership Summary**: Three-year TCO breakdown: training, inference, and operations costs. The `{python} TCOSummary.inf_train_ratio_str`:1 ratio between inference and training costs is typical for production systems serving millions of daily users. A `{python} TCOSummary.quant_reduction_pct_str` reduction in inference latency through quantization saves about `{python} TCOSummary.quant_savings_str` and `{python} TCOSummary.quant_carbon_str` of CO~2~, easily justifying the optimization engineering investment. {#tbl-tco-summary tbl-colwidths="[25,25,25,25]"}
...
from mlsysim.fmt import fmt, fmt_carbon_intensity, fmt_emissions, fmt_energy, fmt_int, check, fmt_qty_int
...
class CarbonScaleCalc:
    ...
    total_emissions_kg_str   = fmt_emissions(total_emissions, unit=kg, precision=0, commas=True)
...
::: {.column-margin}
![](images/svg/vol1_responsible_engr_margin_003.svg){width="100%" fig-alt="Margin illustration showing Model-training emissions versus one passenger-car year."}
...
The key insight unifying these tools is that translating responsibility concerns into measurable properties makes them tractable.\index{Measurable Properties!responsibility requirements} "Fairness gap <5 percent across groups" is actionable; "be fair" is not. This translation extends beyond fairness: efficiency becomes carbon accounting and TCO analysis, where a `{python} ResponsibleTcoRecap.quant_reduction_pct_str` latency reduction through quantization saves `{python} ResponsibleTcoRecap.quant_savings_str` and eliminates `{python} ResponsibleTcoRecap.quant_carbon_str` of CO~2~. Documentation becomes model cards with explicit intended use and known limitations. Governance becomes access control, lineage tracking, and audit infrastructure that makes compliance demonstrable rather than aspirational. At every level, the same pattern holds: abstract ethical obligations become concrete engineering requirements that can be specified, tested, monitored, and enforced.
