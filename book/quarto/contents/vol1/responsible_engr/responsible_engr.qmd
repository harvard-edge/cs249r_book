...
+:================================+:==============================+========================:+=====================:+
| **Initial data preparation**    | hours x rate                  | 100 GPU-hrs x $4 = $400 | 16 kg                |
+---------------------------------+-------------------------------+-------------------------+----------------------+
| **Hyperparameter search**       | experiments x cost/experiment | 50 x $40 = $2,000       | 80 kg                |
+---------------------------------+-------------------------------+-------------------------+----------------------+
| **Final training**              | hours x rate                  | 200 GPU-hrs x $4 = $800 | 32 kg                |
+---------------------------------+-------------------------------+-------------------------+----------------------+
| **Subtotal per training cycle** |                               | **$3,200**              | **128 kg**           |
+---------------------------------+-------------------------------+-------------------------+----------------------+
| **Retraining frequency**        | cycles/year x years           | 4/year x 3 years = 12   | 12                   |
+---------------------------------+-------------------------------+-------------------------+----------------------+
...
+---------------------------+----------------------+---------------------------+----------------------+
| **GPU-hours/day**         | seconds / 3600       | 556 GPU-hrs               | 89 kg                |
+---------------------------+----------------------+---------------------------+----------------------+
| **Annual GPU cost**       | hours x 365 x rate   | 556 x 365 x $2.50 = $507K | 32,470 kg            |
+---------------------------+----------------------+---------------------------+----------------------+
...
Engineers assume high overall accuracy indicates the system works well for all users. In production, aggregate metrics conceal severe subgroup disparities. The Gender Shades study demonstrated commercial facial recognition systems reporting 95% overall accuracy while achieving 99.2% accuracy for light-skinned males versus 65.3% accuracy for dark-skinned females (a 40x error rate disparity from 0.8% to 34.7%). Similarly, the loan approval example in @sec-responsible-engineering-fairness-worked-example showed systems with strong aggregate performance exhibiting 30 percentage point TPR gaps between demographic groups, meaning qualified minority applicants faced rejection rates 3x higher than equally qualified majority applicants. These disparities can persist for months before detection because traditional monitoring tracks only aggregate metrics. Disaggregated evaluation across relevant populations must be implemented before deployment with automated alerts when subgroup performance disparities exceed predefined thresholds (commonly 1.25x error rate ratio or 5 percentage point TPR difference for high-stakes applications).
