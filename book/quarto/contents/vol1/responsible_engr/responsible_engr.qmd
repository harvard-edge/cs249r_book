...
[^fn-proxy-variable-bias]: **Proxy Variable**: From Latin *procurator* (agent, substitute), "proxy" entered statistics to denote a variable standing in for an unmeasurable quantity. The etymology explains the ML failure mode: any feature can silently act as an agent for a protected attribute. Detection is computationally intractable because any feature combination can serve as a proxy, making continuous per-group outcome monitoring the only reliable defense. \index{Proxy Variable!bias detection}
...
[^fn-compas-recidivism]: **COMPAS (Correctional Offender Management Profiling for Alternative Sanctions)**: The system achieved its technical objective of calibration, but this choice made disparate error rates mathematically inevitable because recidivism base rates differed between populations. This trade-off between a narrow engineering goal and broader fairness values resulted in Black defendants being falsely flagged as high-risk at nearly twice the rate of white defendants. \index{COMPAS!impossibility theorem}
...
***Distribution Shift***\index{Distribution Shift!definition} is the violation of the **Stationarity Assumption** ($P_{train} \neq P_{deploy}$) that underpins all supervised learning.

1.  **Significance (Quantitative):** It is the fundamental failure mode of ML systems, where accuracy erodes in proportion to the **Distributional Divergence** ($D(P_{train} \| P_{deploy})$), regardless of code quality.
2.  **Distinction (Durable):** Unlike **Model Error** (which is a learning failure), Distribution Shift is an **Environmental Failure**: the model's learned mapping is no longer representative of the current reality.
3.  **Common Pitfall:** A frequent misconception is that "Data Drift" and "Distribution Shift" are different. In reality, **Data Drift** is the specific *operational* manifestation of Distribution Shift, which requires continuous adaptation through **MLOps** loops.

:::

The stationarity assumption[^fn-stationarity-drift] underpins all supervised learning: training and deployment distributions must match.

[^fn-stationarity-drift]: **Stationarity Assumption**: This assumption is a mathematical requirement of statistical learning, which presumes that training and deployment data are drawn from an identical and unchanging probability distribution ($P_{train} = P_{deploy}$). Violating this assumption causes "distribution drift," where a model's predictive power silently degrades because its learned patterns no longer match the new reality. This failure is often unequal: a model's accuracy on a minority subgroup can drop by over 30 percentage points while aggregate metrics barely change, masking the harm. \index{Stationarity!drift and fairness}
...
[^fn-goodhart-proxy]: **Goodhart's Law**: Named after British economist Charles Goodhart, who articulated the principle in a 1975 paper on monetary policy; Marilyn Strathern later generalized it: "When a measure becomes a target, it ceases to be a good measure." ML systems are uniquely susceptible because gradient descent optimizes proxies at a scale and speed no human decision-maker can match, decoupling the proxy from the goal orders of magnitude faster than manual processes ever could. \index{Goodhart's Law!proxy decoupling}
...
[^fn-gender-shades-audit]: **Gender Shades**: A 2018 study by Joy Buolamwini and Timnit Gebru (MIT Media Lab) that audited facial recognition systems from Microsoft, IBM, and Face++ using the Fitzpatrick skin type scale to create a balanced benchmark with equal representation across gender and skin tone. The study's lasting systems contribution was methodological: it established disaggregated evaluation as the standard for algorithmic auditing, demonstrating that a single aggregate accuracy number can conceal 43$\times$ error rate disparities across intersectional subgroups. \index{Gender Shades!disaggregated evaluation}
...
***Responsible AI Engineering***\index{Responsible AI Engineering!definition} is the engineering practice of imposing **Safety Constraints** on **Stochastic Systems**.

1.  **Significance (Quantitative):** It treats Fairness, Privacy, and Robustness as **System Invariants** that must be enforced through testing, monitoring, and architectural guardrails.
2.  **Distinction (Durable):** Unlike **AI Ethics** (which focuses on **Aspiration**), Responsible AI Engineering focuses on **Implementation**: it translates abstract values into measurable, testable properties.
3.  **Common Pitfall:** A frequent misconception is that responsibility is "added" at the end of development. In reality, it is a **Lifecycle Constraint**: the most critical decisions about fairness and privacy occur during **Problem Definition** and **Data Collection**.
...
This framework parallels aviation pre-flight checklists, where pilots follow every item without exception to ensure comprehensive coverage of critical concerns despite time pressure.\index{Checklist Manifesto!systematic verification} Production ML deployments require equivalent discipline and rigorous verification.[^fn-checklist-discipline] Checklists ensure teams ask the right questions; documentation standards ensure the answers persist and travel with the model.

[^fn-checklist-discipline]: **Checklist Discipline**: Atul Gawande's surgical checklist research proved that externalizing checks into a repeatable process counters the tendency to skip steps under pressure. For ML deployments, this ensures non-obvious concerns like fairness and safety are rigorously verified, preventing regressions that familiarity and deadlines can mask. This simple discipline reduced surgical mortality by 47%, quantifying the high cost of verification failures. \index{Checklist Discipline!deployment verification}
...
[^fn-model-card-transparency]: **Model Cards**: The "nutrition label" metaphor (Mitchell et al., 2019) explains the design: just as food labels disclose ingredients consumers cannot detect, model cards disclose capabilities, limitations, and intended uses that downstream users cannot infer from accuracy numbers alone. The companion "Datasheets for Datasets" (Gebru et al., 2018) applies the same principle to training data. Together these frameworks prevent the scope creep that turns a photo-organization model into a security-screening tool without re-validation. \index{Model Card!documentation standard}
...
Three standard fairness metrics\index{Fairness Metrics!confusion matrix analysis} computed from the confusion matrices[^fn-confusion-matrix-fairness] in @tbl-confusion-group-a and @tbl-confusion-group-b\index{Confusion Matrix!fairness computation} reveal significant disparities.[^fn-fairness-metric-incompatibility]

[^fn-confusion-matrix-fairness]: **Confusion Matrix**: Its four cells—True/False Positives/Negatives—provide the raw counts for computing the fairness metrics referenced here. The critical operation is computing separate matrices per demographic group, which transforms an aggregate view into a disaggregated analysis. This reveals fairness violations invisible in the overall metrics; for instance, the Gender Shades audit found error rate disparities between groups exceeding 34% using this method. \index{Confusion Matrix!fairness decomposition}
...
[^fn-demographic-parity-limitation]: **Demographic Parity**: This metric enforces the equal positive prediction rates calculated for each group, considering only the fraction of approvals (`(TP+FP)/Total`) while ignoring ground-truth correctness. When underlying base rates of qualification differ, enforcing this parity is mathematically incompatible with achieving equal error rates (e.g., Equalized Odds). A model may be forced to increase incorrect approvals (false positives) for one group simply to close the observed `{python} dp_disparity_str` percentage point gap, making this an irreducible trade-off against overall accuracy. \index{Demographic Parity!limitation}

Demographic parity[^fn-demographic-parity-limitation]\index{Fairness Metrics!demographic parity} requires equal approval rates across groups. Group A receives approval at a rate of (`{python} a_tp_str` + `{python} a_fp_str`) / `{python} a_total_str` = `{python} a_approval_str`%, while Group B receives approval at (`{python} b_tp_str` + `{python} b_fp_str`) / `{python} b_total_str` = `{python} b_approval_str`%. The `{python} dp_disparity_str` percentage point disparity indicates unequal treatment in approval decisions.
...
Equalized odds[^fn-equalized-odds-postprocessing]\index{Fairness Metrics!equalized odds} requires both equal true positive rates and equal false positive rates. Group A shows an FPR of `{python} a_fp_str` / (`{python} a_fp_str` + `{python} a_tn_str`) = `{python} a_fpr_str`%, and Group B shows `{python} b_fp_str` / (`{python} b_fp_str` + `{python} b_tn_str`) = `{python} b_fpr_str`%. While false positive rates are equal, the true positive rate disparity means equalized odds is violated.
...
Several mitigation approaches exist, each with distinct trade-offs. Threshold adjustment[^fn-threshold-adjustment-fairness]\index{Threshold Adjustment!fairness mitigation} lowers the approval threshold for Group B to equalize TPR but may increase false positives for that group. Reweighting[^fn-reweighting-bias]\index{Reweighting!bias mitigation} increases the weight of Group B samples during training to give the model stronger signal about this population but may reduce overall accuracy. Adversarial debiasing\index{Adversarial Debiasing!fairness constraints} trains with an adversary that prevents the model from learning group membership but adds training complexity.[^fn-adversarial-debiasing-cost] The choice among these approaches requires stakeholder input about which trade-offs are acceptable in the specific application context. How, then, should engineers present these trade-offs to stakeholders? The answer lies in making the trade-offs explicit and quantifiable.

[^fn-threshold-adjustment-fairness]: **Threshold Adjustment**: This post-processing technique directly manipulates classification outputs without retraining the model. To equalize the True Positive Rate (TPR) as the context sentence describes, the classification threshold for the disadvantaged group is lowered, which necessarily makes the model more permissive for that group. This action directly increases the group's False Positive Rate (FPR), creating a non-negotiable trade-off where a 5% gain in TPR might require tolerating a >20% increase in false positives. \index{Threshold Adjustment!fairness trade-off}
...
[^fn-adversarial-debiasing-cost]: **Adversarial Debiasing**: This technique adds training complexity by introducing a second "adversary" model that learns to predict protected group membership from the primary model's outputs. The primary model is then forced via gradient reversal to create representations that hide this information, a process that adds 20-50% to training time and can reduce overall accuracy by 1-3%. \index{Adversarial Debiasing!training cost}
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
hire_value_k_str    = FairnessPrice.hire_value_k_str
bad_hire_cost_k_str = FairnessPrice.bad_hire_cost_k_str
fp_increase_str     = FairnessPrice.fp_increase_str
utility_loss_str    = FairnessPrice.utility_loss_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
smart_power_str   = EdgeEfficiencyCalc.smart_power_str
smart_latency_str = EdgeEfficiencyCalc.smart_latency_str
iot_power_str     = EdgeEfficiencyCalc.iot_power_str
iot_latency_str   = EdgeEfficiencyCalc.iot_latency_str
cam_power_str     = EdgeEfficiencyCalc.cam_power_str
cam_latency_str   = EdgeEfficiencyCalc.cam_latency_str
wear_power_str    = EdgeEfficiencyCalc.wear_power_str
wear_latency_str  = EdgeEfficiencyCalc.wear_latency_str
mv2_params_str    = EdgeEfficiencyCalc.mv2_params_str
mv2_power_str     = EdgeEfficiencyCalc.mv2_power_str
mv2_latency_str   = EdgeEfficiencyCalc.mv2_latency_str
eff_params_str    = EdgeEfficiencyCalc.eff_params_str
eff_power_str     = EdgeEfficiencyCalc.eff_power_str
eff_latency_str   = EdgeEfficiencyCalc.eff_latency_str
rn50_params_str   = EdgeEfficiencyCalc.rn50_params_str
rn50_power_str    = EdgeEfficiencyCalc.rn50_power_str
rn50_latency_str  = EdgeEfficiencyCalc.rn50_latency_str
tiny_params_str   = EdgeEfficiencyCalc.tiny_params_str
tiny_power_str    = EdgeEfficiencyCalc.tiny_power_str
tiny_latency_str  = EdgeEfficiencyCalc.tiny_latency_str
```
...
[^fn-tco-inference-dominance]: **Total Cost of Ownership (TCO)**: Originated by Gartner Group in the 1980s to capture full lifecycle IT costs beyond purchase price. For ML systems, TCO reveals that training costs---the focus of most research publications---are typically dwarfed 10--1,000$\times$ by inference, monitoring, and operational costs in production. A responsible engineering perspective extends TCO further to include externalities: carbon emissions, fairness remediation costs, and regulatory compliance overhead that traditional cloud invoices never reflect. \index{TCO!inference dominance}
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
data_prep_str       = InferenceCostCalc.data_prep_str
hyperparam_str      = InferenceCostCalc.hyperparam_str
train_cost_str      = InferenceCostCalc.train_cost_str
total_train_str     = InferenceCostCalc.total_train_str
lifecycle_train_str = InferenceCostCalc.lifecycle_train_str
inferences_m        = InferenceCostCalc.inferences_m
gpus_str            = InferenceCostCalc.gpus_str
annual_inf_str      = InferenceCostCalc.annual_inf_str
lifecycle_inf_str   = InferenceCostCalc.lifecycle_inf_str
ratio_str           = InferenceCostCalc.ratio_str
gpu_rate_input_str  = InferenceCostCalc.gpu_rate_input_str
data_prep_hrs_str   = InferenceCostCalc.data_prep_hrs_str
hyperparam_hrs_str  = InferenceCostCalc.hyperparam_hrs_str
train_hrs_str       = InferenceCostCalc.train_hrs_str
users_daily_m_str   = InferenceCostCalc.users_daily_m_str
recs_per_user_str   = InferenceCostCalc.recs_per_user_str
inference_ms_str    = InferenceCostCalc.inference_ms_str
gpu_inf_rate_str    = InferenceCostCalc.gpu_inf_rate_str
```
...
[^fn-ml-roi-heuristic]: **ML Return on Investment**: Industry experience suggests most ML projects never reach production; of those deployed, many fail to justify infrastructure, maintenance, and technical debt costs against business value delivered. The responsible engineering implication: a simple heuristic or logistic regression that achieves 90% of a deep model's accuracy at 1% of the cost often represents the correct engineering decision, especially when the 10$\times$ TCO difference compounds over years of production serving. \index{ML ROI!heuristic baseline}
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
train_energy_mwh_str     = CarbonScaleCalc.train_energy_mwh_str
carbon_intensity_str     = CarbonScaleCalc.carbon_intensity_str
total_emissions_kg_str   = CarbonScaleCalc.total_emissions_kg_str
total_emissions_tons_str = CarbonScaleCalc.total_emissions_tons_str
cars_eq_str              = CarbonScaleCalc.cars_eq_str
car_annual_tons_str      = CarbonScaleCalc.car_annual_tons_str
```
...
[^fn-audit-trail-scale]: **Audit Trail**: Accountability is achieved by logging every data access, which in an ML system includes every prediction, feature lookup, and training run. The resulting firehose of events creates a distinct systems challenge, forcing a design focused on extreme write-throughput and long-term immutable storage. This scale dictates the architecture, as a large platform may log over 50 billion events daily to remain compliant with regulations like HIPAA, which mandates a six-year retention period. \index{Audit Trail!storage scale}
