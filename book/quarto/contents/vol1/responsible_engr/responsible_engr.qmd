...
: **Confusion Matrix for Group A (Majority)**: Loan approval outcomes for `{python} LoanFairness.a_total_str` applicants from the majority demographic group in this constructed scenario. The `{python} LoanFairness.a_tpr_str` true positive rate (`{python} LoanFairness.a_tp_str` approved of `{python} LoanFairness.a_qualified_str` qualified) and `{python} LoanFairness.a_fpr_str` false positive rate establish the baseline for fairness comparison. {#tbl-confusion-group-a}
...
: **Confusion Matrix for Group B (Minority)**: Loan approval outcomes for `{python} LoanFairness.b_total_str` applicants from the minority demographic group in the same constructed scenario. The `{python} LoanFairness.b_tpr_str` true positive rate (`{python} LoanFairness.b_tp_str` approved of `{python} LoanFairness.b_qualified_str` qualified) reveals a `{python} LoanFairness.tpr_disparity_str` disparity compared with Group A; the confusion matrices alone do not identify its cause. {#tbl-confusion-group-b}
...
::: {#fig-fairness-threshold fig-env="figure" fig-pos="htb" fig-cap="**Threshold Effects on Subgroup Outcomes**: Two candidate classification thresholds intersect different score distributions for Subgroups A and B. Circles mark positive outcomes (loan repayment), plus markers mark negative outcomes (default), and color distinguishes subgroup. Each dashed line is labeled with the overall accuracy it produces, not with a score value. Moving to the higher-accuracy boundary lifts overall accuracy from 75 percent to 81.25 percent, but the whole gain falls in Subgroup A, which becomes perfectly separated, while Subgroup B goes from one misclassification to three." fig-alt="Two horizontal rows labeled Subgroup A and Subgroup B contain colored circle and plus markers at different positions. Dashed vertical lines labeled 75 percent and 81.25 percent cross both rows. Circles denote positive outcomes, plus signs denote negative outcomes, and color identifies subgroup."}
...
::: {#fig-data-governance-pillars fig-env="figure" fig-pos="htb" fig-cap="**Data Governance Framework**: Eight operating elements surround Data Governance as a central hub: policies, organization, security, operations, data quality and master data, sourcing, catalogs, and shared analytic definitions. These are organizational functions rather than the four technical domains introduced above: catalogs, shared definitions, and sourcing exist to make security, privacy, compliance, and lineage enforceable rather than aspirational. Adapted from a composite of industry data-governance frameworks." fig-alt="Circular diagram with 'Data Governance' at center, surrounded by eight topical elements: Organization, Policies, Data catalogs, Data & analytic definitions, Data Sourcing, Data quality & master Data, Data Operations, Data Security."}
