...
1.  **Significance (Quantitative):** Accuracy degradation is measurable against divergence. Empirical studies of production recommendation and NLP models find that when Jensen-Shannon divergence $\mathcal{D}_{\text{JS}}(P_{\text{train}} \| P_{\text{deploy}}) > 0.1$, observed accuracy drops exceed five percent relative; when $\mathcal{D}_{\text{JS}} > 0.3$, degradation typically exceeds 15–30 percent—sufficient to invalidate a production system that passed predeployment evaluation. This degradation occurs regardless of code quality, because the model is correct given its training distribution; the environment changed, not the code.
...
**The Physics**: You can equalize TPRs by adjusting the classification threshold $(\tau_{\text{cls}})$ for the disadvantaged group.
...
2.  If the value of a successful hire is `{python} FairnessPrice.hire_value_k_str` and the cost of a bad hire is `{python} FairnessPrice.bad_hire_cost_k_str`:
    - $\text{Utility Loss} = (\text{Utility of Correct Hires}) - (\text{Cost of Extra False Positives})$.
...
1.  **Energy Consumption**: \(`{python} CarbonScaleCalc.train_energy_mwh_str` \text{ MWh} = `{python} fmt(CarbonScaleCalc.train_energy_kwh, commas=True)` \text{ kWh}\).
2.  **Carbon Intensity**\index{Carbon Intensity!definition}: The average US grid emits \(\approx `{python} CarbonScaleCalc.carbon_intensity_str` \text{ kg CO2 per kWh}\).
3.  **Total Emissions**: \(`{python} fmt(CarbonScaleCalc.train_energy_kwh, commas=True)` \times `{python} CarbonScaleCalc.carbon_intensity_str` = `{python} CarbonScaleCalc.total_emissions_kg_str` \text{ kg CO2}\) (`{python} CarbonScaleCalc.total_emissions_tons_str` metric tons).
