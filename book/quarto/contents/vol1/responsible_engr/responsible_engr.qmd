...
class FairnessPrice:
    ...
    utility_loss_pct         = utility_loss_fraction.to("").magnitude * 100.0
    ...
    utility_loss_str    = fmt_percent(utility_loss_fraction, precision=1, commas=False, style='prose')  # computed from assumptions above
...
1.  Closing the `{python} FairnessPrice.tpr_gap_pp_str` TPR gap requires accepting a `{python} FairnessPrice.fp_increase_pp_str` increase in False Positives for the disadvantaged group.
...
**Systems insight**: The "Price of Fairness"\index{Price of Fairness!utility trade-off} in this system is a `{python} FairnessPrice.utility_loss_str` utility tax under the stated assumptions, a system constraint, not a bug. The exact figure depends on the base rate of qualified applicants and the disadvantaged group's share of the pool; the engineer's job is to present the Pareto frontier to stakeholders so they can choose the Utility/Fairness trade-off that aligns with organizational values.
...
# │
# │ Imports: mlsysim (Infrastructure.Pricing.Cloud), mlsysim.core.constants (MILLION, SEC_PER_HOUR, hour, ms)
...
class TCOCalc:
    ...
    t_data_prep_calc_str = MarkdownStr(f"{t_data_prep_hrs:.0f} GPU-hr × \\${train_gpu_rate_per_hour:.0f} = \\${(t_data_prep_time * train_gpu_rate).to(USD).magnitude:,.0f}")
    t_hparam_calc_str = MarkdownStr(f"{t_hparam_exps} × \\${t_hparam_cost_exp_usd:.0f} = \\${(t_hparam_exps * t_hparam_cost_exp).to(USD).magnitude:,.0f}")
    t_final_calc_str = MarkdownStr(f"{t_final_hrs:.0f} GPU-hr × \\${train_gpu_rate_per_hour:.0f} = \\${(t_final_time * train_gpu_rate).to(USD).magnitude:,.0f}")
    ...
    i_gpu_sec_calc_str = MarkdownStr(f"{inf_daily_total/MILLION:.0f}M × {i_latency.to(second).magnitude} s = {inf_daily_total * i_latency.to(second).magnitude/MILLION:.1f}M sec")
    ...
    i_annual_calc_str = MarkdownStr(f"{inf_gpu_hours_day:.0f} × {DAYS_PER_YEAR} × \\${inf_gpu_rate_per_hour:.2f} = \\${inf_cost_year.to(USD).magnitude / THOUSAND:.0f}K")
...
from mlsysim.core.constants import THOUSAND, USD
from mlsysim.core.units import metric_ton
...
A common fallacy is this: *Responsibility can be addressed after the system achieves technical objectives.*
...
A recurring pitfall is this: *Relying on aggregate metrics to assess fairness.*
...
A common fallacy is this: *Removing sensitive attributes from training data eliminates bias.*
...
A recurring pitfall is this: *Treating documentation as sufficient accountability.*
...
A common fallacy is this: *Responsible AI is primarily a legal compliance issue.*
...
A recurring pitfall is this: *Measuring the environmental impact of training but not inference.*
...
from mlsysim import Infrastructure
from mlsysim.core.constants import DAYS_PER_YEAR, MILLION, THOUSAND, hour, ms, USD
from mlsysim.core.units import metric_ton
from mlsysim.fmt import fmt_ratio, fmt_usd, check, fmt_percent, fmt_qty_int
