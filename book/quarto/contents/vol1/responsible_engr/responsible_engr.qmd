...
class RepresentationStats:
    ...
    repr_equation_md = Markdown(f"$$ N_{{\\rm total }} = {target_imgs:,} \\text{{ images}} $$")
```
...
: **Fairness Risk by ML Archetype**: Fairness risks vary by archetype's data source and deployment context. {#tbl-fairness-archetype tbl-colwidths="[20,30,20,30]"}
...
1.  **Energy Consumption**: `{python} CarbonScaleCalc.train_energy_mwh_str`  MWh = `{python} fmt(CarbonScaleCalc.train_energy_kwh, commas=True)` \text{ kWh}.
