...
class TestingConstraintAnchor:
    ...
    # Custom "×" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    multiplier_str = fmt_multiple(data_multiplier, precision=0, commas=False)
...
class GenderShadesDisparity:
    ...
    disparity_factor_str = fmt_multiple(disparity_fold, precision=1, commas=False)
    disparity_light_female_factor_str = fmt_multiple(disparity_light_female, precision=1, commas=False)
    disparity_dark_male_factor_str = fmt_multiple(int(disparity_dark_male), precision=0, commas=False)
...
| **Light-skinned females** | `{python} GenderShadesDisparity.error_light_female_str` | `{python} GenderShadesDisparity.disparity_light_female_factor_str`$\times$ higher |
| **Dark-skinned males**    |   `{python} GenderShadesDisparity.error_dark_male_str`  |   `{python} GenderShadesDisparity.disparity_dark_male_factor_str`$\times$ higher  |
| **Dark-skinned females**  |  `{python} GenderShadesDisparity.error_dark_female_str` |        `{python} GenderShadesDisparity.disparity_factor_str`$\times$ higher       |
...
class RepresentationStats:
    ...
    repr_multiplier_str = fmt_multiple(int(multiplier), commas=False, precision=0)
...
from mlsysim.fmt import fmt, fmt_count, check, fmt_percent
...
class EdgeEfficiencyCalc:
    ...
    tiny_wear_power_margin_factor_str = fmt_multiple(tiny_wear_power_margin, precision=0, commas=False)
    mv2_wear_power_overage_factor_str = fmt_multiple(mv2_wear_power_overage, precision=1, commas=False)
...
For the wearable budget in @tbl-edge-deployment-constraints, the TinyML model leaves a `{python} EdgeEfficiencyCalc.tiny_wear_power_margin_factor_str`$\times$ power margin, while MobileNetV2 exceeds the same power budget by `{python} EdgeEfficiencyCalc.mv2_wear_power_overage_factor_str`$\times$ before accounting for sustained thermals.
...
class FairnessPitfallAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tpr_disparity_str = fmt_pp(tpr_disparity, precision=0, attributive=True)
    fnr_ratio_str = fmt_multiple(fnr_ratio, precision=0, commas=False)
```
