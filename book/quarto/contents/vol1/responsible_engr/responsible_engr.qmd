...
class FairnessPrice:
    ...
    utility_loss_str    = fmt_percent(utility_loss_pct/100, precision=1, commas=False, style='prose')   # computed from assumptions above
...
class EdgeEfficiencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # Unit-backed W/ms/mW values use fmt_qty; count scales and prose-singular "second" remain explicit.
...
class InferenceCostCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    data_prep_str       = fmt_usd(data_prep_cost_value, precision=0, commas=True)
    hyperparam_str      = fmt_usd(hyperparam_cost_value, precision=0, commas=True)
    train_cost_str      = fmt_usd(train_cost_value, precision=0, commas=True)
    total_train_str     = fmt_usd(total_train_cost_value, precision=0, commas=True)
    lifecycle_train_str = fmt_usd(lifecycle_train_cost_value, precision=0, commas=True)
    inferences_m_str    = fmt(inferences_daily_value/MILLION, precision=0, commas=False)
    gpus_str            = fmt_count(gpus_needed_value, precision=1, commas=False, label="GPU", allow_fractional=True)
    annual_inf_str      = fmt_usd(round(annual_inf_cost_value), commas=True)
    lifecycle_inf_str   = fmt_usd(lifecycle_inf_cost_value, precision=1, commas=False, scale="M")
    ratio_str           = fmt_int(round(inf_train_ratio_value), commas=False)
    gpu_rate_input_str  = fmt_usd(gpu_rate_value, precision=0, commas=False, per="hour")
    data_prep_hrs_str   = fmt_count(data_prep_hrs_value, precision=0, label="GPU-hour", plural_label="GPU-hours")
    hyperparam_hrs_str  = fmt_count(hyperparam_hrs_value, precision=0, label="GPU-hour", plural_label="GPU-hours")
    train_hrs_str       = fmt_count(train_hrs_value, precision=0, label="GPU-hour", plural_label="GPU-hours")
    ...
    inference_ms_str    = fmt_time(inference_ms_value, "millisecond", precision=0, commas=False, style="word")
    gpu_inf_rate_str    = fmt_usd(gpu_inf_rate_value, precision=2, commas=False, per="GPU-hour")
```
...
class CarbonFactor:
    ...
    gpu_power_kw_str = fmt_qty(gpu_power, ureg.kilowatt, precision=1, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_count, fmt_int, fmt_usd, check, MarkdownStr, fmt_percent, fmt_qty
from mlsysim import Infrastructure
from mlsysim.core.constants import (
    THOUSAND,
    MILLION,
...
class TCOCalc:
    ...
    check(abs(carbon_per_gpu_hr - 0.16) < 1e-9, "TCO carbon factor should match the scenario baseline.")
    ...
    t_total_k_str = fmt_usd((round(train_tco_3yr/THOUSAND)) * THOUSAND, commas=False, scale="K")
    ...
    t_data_prep_carbon_str = fmt_qty(t_data_prep_hrs * carbon_per_gpu_hr * ureg.kilogram, ureg.kilogram, precision=0, commas=False)
    t_hparam_carbon_str = fmt_qty((t_hparam_exps * t_hparam_cost_exp/train_gpu_rate) * carbon_per_gpu_hr * ureg.kilogram, ureg.kilogram, precision=0, commas=False)
    t_final_carbon_str = fmt_qty(t_final_hrs * carbon_per_gpu_hr * ureg.kilogram, ureg.kilogram, precision=0, commas=False)
    t_subtotal_carbon_str = fmt_qty(train_carbon_cycle * ureg.kilogram, ureg.kilogram, precision=0, commas=False)
    t_total_carbon_str = fmt_qty(train_carbon_3yr * ureg.kilogram, ureg.kilogram, precision=0, commas=True)
    t_total_carbon_tons_str = fmt_qty((train_carbon_3yr/THOUSAND) * ureg.metric_ton, ureg.metric_ton, precision=1, commas=False, unit_label="tons")
    ...
    i_daily_carbon_str = fmt_qty(inf_carbon_day * ureg.kilogram, ureg.kilogram, precision=1, commas=False)
    ...
    i_annual_carbon_str = fmt_qty((inf_carbon_3yr/3) * ureg.kilogram, ureg.kilogram, precision=1, commas=True)
    i_total_str = fmt_usd(inf_tco_3yr, precision=2, commas=False, scale="M")
    i_carbon_str = fmt_qty(inf_carbon_3yr * ureg.kilogram, ureg.kilogram, precision=1, commas=True)
    i_carbon_tons_str = fmt_qty((inf_carbon_3yr/THOUSAND) * ureg.metric_ton, ureg.metric_ton, precision=1, commas=False, unit_label="tons")
    ...
    i_annual_cost_k_str = fmt_usd((round(inf_cost_day * DAYS_PER_YEAR / THOUSAND)) * THOUSAND, commas=False, scale="K")
    ...
    o_monitor_annual_k_str = fmt_usd(o_monitor_yr, precision=0, commas=False, scale="K", per="year")
    o_oncall_annual_k_str = fmt_usd(o_oncall_yr, precision=0, commas=False, scale="K", per="year")
    o_incident_annual_k_str = fmt_usd(o_incident_yr, precision=0, commas=False, scale="K", per="year")
    ...
    total_carbon_tons_str = fmt_qty(round(total_carbon_kg/THOUSAND) * ureg.metric_ton, ureg.metric_ton, precision=0, commas=False, approx=True, unit_label="tons")
...
from mlsysim.core.constants import THOUSAND, ureg
...
class TCOSummary:
    ...
    quant_savings_str = fmt_usd((round(savings_dollars / THOUSAND)) * THOUSAND, commas=False, scale="K")
...
: **Total Cost of Ownership Summary**: Three-year TCO breakdown: training, inference, and operations costs. The `{python} TCOSummary.inf_train_ratio_str`:1 ratio between inference and training costs is typical for production systems serving millions of daily users. A `{python} TCOSummary.quant_reduction_pct_str` reduction in inference latency through quantization saves about `{python} TCOSummary.quant_savings_str` and `{python} TCOSummary.quant_carbon_str` metric tons of CO~2~, easily justifying the optimization engineering investment. {#tbl-tco-summary tbl-colwidths="[25,25,25,25]"}
...
class CarbonScaleCalc:
    ...
    train_energy_mwh_q  = train_energy_mwh * ureg.megawatt_hour
    train_energy_kwh_q  = train_energy_kwh * ureg.kilowatt_hour
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    train_energy_mwh_str  = fmt_qty(train_energy_mwh_q, ureg.megawatt_hour, precision=0, commas=True)
    train_energy_kwh_str  = fmt_qty(train_energy_kwh_q, ureg.kilowatt_hour, precision=0, commas=True)
...
class ResponsibleTcoRecap:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(round(inf_tco_3yr / train_tco_3yr) >= 40, "Inference should round to at least 40x training TCO in this scenario.")
    ...
    quant_reduction_pct_str = fmt_percent(round(int(quant_reduction_pct * 100))/100, precision=0, commas=False, style='prose')
    quant_savings_str = fmt_usd((round(savings_dollars / THOUSAND)) * THOUSAND, commas=False, scale="K")
    quant_carbon_str = fmt_int(round(savings_carbon_kg / THOUSAND), commas=False)
```
...
The key insight unifying these tools is that translating responsibility concerns into measurable properties makes them tractable.\index{Measurable Properties!responsibility requirements} "Fairness gap <5 percent across groups" is actionable; "be fair" is not. This translation extends beyond fairness: efficiency becomes carbon accounting and TCO analysis, where a `{python} ResponsibleTcoRecap.quant_reduction_pct_str` latency reduction through quantization saves `{python} ResponsibleTcoRecap.quant_savings_str` and eliminates `{python} ResponsibleTcoRecap.quant_carbon_str` metric tons of CO~2~. Documentation becomes model cards with explicit intended use and known limitations. Governance becomes access control, lineage tracking, and audit infrastructure that makes compliance demonstrable rather than aspirational. At every level, the same pattern holds: abstract ethical obligations become concrete engineering requirements that can be specified, tested, monitored, and enforced.
