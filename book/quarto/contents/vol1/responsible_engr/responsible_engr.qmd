...
class TCOCalc:
    ...
    t_data_prep_carbon_str = fmt_qty(t_data_prep_hrs * carbon_per_gpu_hr * kg, kg, precision=0, commas=False)
    t_hparam_carbon_str = fmt_qty((t_hparam_exps * t_hparam_cost_exp/train_gpu_rate) * carbon_per_gpu_hr * kg, kg, precision=0, commas=False)
    t_final_carbon_str = fmt_qty(t_final_hrs * carbon_per_gpu_hr * kg, kg, precision=0, commas=False)
    t_subtotal_carbon_str = fmt_qty(train_carbon_cycle * kg, kg, precision=0, commas=False)
    t_total_carbon_str = fmt_qty(train_carbon_3yr * kg, kg, precision=0, commas=True)
    t_total_carbon_tons_str = fmt_qty((train_carbon_3yr/THOUSAND) * metric_ton, metric_ton, precision=1, commas=False, unit_label="tons")
    ...
    i_daily_carbon_str = fmt_qty(inf_carbon_day * kg, kg, precision=1, commas=False)
    ...
    i_annual_carbon_str = fmt_qty((inf_carbon_3yr/3) * kg, kg, precision=1, commas=True)
    i_total_str = fmt_usd(inf_tco_3yr, precision=2, commas=False, scale="M")
    i_carbon_str = fmt_qty(inf_carbon_3yr * kg, kg, precision=1, commas=True)
    i_carbon_tons_str = fmt_qty((inf_carbon_3yr/THOUSAND) * metric_ton, metric_ton, precision=1, commas=False, unit_label="tons")
    ...
    total_carbon_tons_str = fmt_qty(round(total_carbon_kg/THOUSAND) * metric_ton, metric_ton, precision=0, commas=False, approx=True, unit_label="tons")
...
from mlsysim.core.units import MWh, kWh
...
class CarbonScaleCalc:
    ...
    train_energy_kwh = train_energy_kwh_q.magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    train_energy_mwh_str  = fmt_qty(train_energy, MWh, precision=0, commas=True)
    train_energy_kwh_str  = fmt_qty(train_energy_kwh_q, kWh, precision=0, commas=True)
...
3.  **Total emissions**: `{python} CarbonScaleCalc.train_energy_kwh_str` $\times$ `{python} CarbonScaleCalc.carbon_intensity_str` = `{python} CarbonScaleCalc.total_emissions_kg_str` CO~2~ (`{python} CarbonScaleCalc.total_emissions_tons_str` metric tons).
