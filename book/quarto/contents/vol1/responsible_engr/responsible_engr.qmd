...
[^fn-proxy-variable-bias]: **Proxy Variable**: A feature that correlates with a protected attribute without encoding it directly. Amazon's failure illustrates the core difficulty: removing explicit gender indicators did not help because college names, activity descriptions, and career gap patterns reconstructed gender signal from combinations the engineers never anticipated. Detection is computationally intractable because *any* feature combination can serve as a proxy, making continuous per-group outcome monitoring---not feature removal---the only reliable defense. \index{Proxy Variable!bias detection}
