...
class TCOCalc:
    ...
    t_data_prep_calc_str = MarkdownStr(
        f"{t_data_prep_hrs:.0f} GPU-hr × \\${train_gpu_rate_per_hour:.0f} = \\${(t_data_prep_time * train_gpu_rate).to(USD).magnitude:,.0f}"
    )
    t_hparam_calc_str = MarkdownStr(
        f"{t_hparam_exps} × \\${t_hparam_cost_exp_usd:.0f} = \\${(t_hparam_exps * t_hparam_cost_exp).to(USD).magnitude:,.0f}"
    )
    t_final_calc_str = MarkdownStr(
        f"{t_final_hrs:.0f} GPU-hr × \\${train_gpu_rate_per_hour:.0f} = \\${(t_final_time * train_gpu_rate).to(USD).magnitude:,.0f}"
    )
    ...
    i_gpu_sec_calc_str = MarkdownStr(
        f"{inf_daily_total/MILLION:.0f}M × {i_latency.to(second).magnitude} s = {inf_daily_total * i_latency.to(second).magnitude/MILLION:.1f}M sec"
    )
    ...
    i_annual_calc_str = MarkdownStr(
        f"{inf_gpu_hours_day:.0f} × {DAYS_PER_YEAR} × \\${inf_gpu_rate_per_hour:.2f} = \\${inf_cost_year.to(USD).magnitude / THOUSAND:.0f}K"
    )
...
| **Total TCO**  | **`{python} TCOCalc.total_tco_str`** |              100%              | **`{python} TCOCalc.total_carbon_t_str`** |
