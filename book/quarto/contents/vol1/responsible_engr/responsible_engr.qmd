...
class GenderShadesDisparity:
    ...
    if disparity_fold < 40:
        raise ValueError(f"Narrative broken: Disparity ({disparity_fold:.1f}x) is too low.")
...
class RepresentationStats:
    ...
    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if multiplier != 100:
        raise ValueError(f"Math broken: Multiplier should be 100.")
...
class LoanFairness:
    ...
    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if tpr_disparity < 25:
        raise ValueError(f"Narrative broken: TPR Disparity ({tpr_disparity:.1f}%) is too low.")
...
inf_gpu_hours_day = (inf_daily_total * i_latency_s) / 3600.0
...
# 3. INVARIANTS
if inf_tco_3yr < train_tco_3yr * 5:
    raise ValueError(f"Narrative broken: Inference TCO doesn't dominate Training.")

# 4. OUTPUTS (Formatting)
users_daily_m_str = f"{i_users // 1_000_000}"
recs_per_user_str = f"{i_recs_per_user}"
inference_ms_str = f"{int(i_latency_s * 1000)}"
inferences_m = f"{inf_daily_total // 1_000_000}"
gpus_str = fmt(inf_gpu_hours_day / 24.0, precision=0, commas=True)
...
lifecycle_inf_str = fmt(inf_tco_3yr / 1e6, precision=1, commas=False)
...
i_daily_q_calc_str = f"{i_users/1e6:.0f}M × {i_recs_per_user} = {inf_daily_total/1e6:.0f}M"
i_gpu_sec_calc_str = f"{inf_daily_total/1e6:.0f}M × {i_latency_s} s = {inf_daily_total * i_latency_s / 1e6:.1f}M sec"
...
i_total_str = f"${inf_tco_3yr/1e6:.2f}M"
...
class TCOSummary:
    ...
    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    if savings_dollars < 100_000:
        raise ValueError(f"Narrative broken: Savings (${savings_dollars:,.0f}) too small to justify optimization.")
