...
The following plot visualizes this **Fairness-Accuracy Pareto Frontier**. It demonstrates that while perfect fairness (zero disparity) often requires a significant drop in accuracy, a "Sweet Spot" typically exists where large fairness gains can be achieved with minimal accuracy loss.
...
Disaggregated evaluation revealed what aggregate accuracy scores concealed. Systems reporting high overall accuracy simultaneously achieved error rates as low as 0.8% for light-skinned males and as high as 34.7% for dark-skinned females (corresponding to accuracies of 99.2% and 65.3% respectively). The aggregate metric provided no indication of this 43-fold disparity in error rates.
...
$$ N_{total} = \frac{1,000}{0.01} = 100,000 \text{ images} $$
...
**The Insight**: Relying on "natural distribution" data for fairness is physically impossible at scale. You effectively need 100$\times$ more data to validate the minority group than the majority group. Fairness requires **intentional data engineering**, not just more data.
:::
