...
The Amazon recruiting tool case illustrates this gap. The technical implementation was sound: trained on a decade of historical hiring data\index{Bias!historical data encoding}, the model optimized faithfully for the objective it was given, but those historical patterns encoded gender bias\index{Bias!gender discrimination} that the system reproduced in its candidate ratings [@dastin2018].
...
**Context**: COMPAS\index{COMPAS!recidivism algorithm audit} is a risk assessment tool\index{Risk Assessment!criminal justice} used in US courtrooms to predict re-offending. Judges use these scores to inform bail and sentencing decisions.

**Failure mode**: A ProPublica investigation [@angwin2016] revealed that while the system was "calibrated" (a score of seven meant the same probability of re-offending for any group), its error rates were skewed:
...
**Systems lesson**: The system optimized for *Calibration*\index{Calibration!fairness impossibility theorem}: a given score corresponded to the same observed re-offense probability across groups. It violated *Equalized Odds*: false positive and false negative rates did not match across groups. Mathematically, it is impossible to satisfy both simultaneously when base rates differ between groups (the "Impossibility Theorem of Fairness"\index{Fairness Metrics!impossibility theorem}). This algorithmic bias\index{Bias!algorithmic} shows why engineering responsibility requires explicitly choosing which fairness constraint matters for the domain; in criminal justice, false positives (wrongly jailing someone) are typically considered worse than false negatives. The worked fairness-metric section later computes these rates directly from confusion matrices.
...
[^fn-compas-recidivism]: **COMPAS (Correctional Offender Management Profiling for Alternative Sanctions)**\index{COMPAS!impossibility theorem}: COMPAS achieved calibration (a given score meant the same re-offense probability for any group), but because recidivism base rates differed between populations, that choice made disparate error rates *mathematically inevitable*. No amount of testing for calibration would have surfaced this failure; the harm was encoded in the objective itself.
...
This sepsis scenario illustrates a class of failure that traditional engineering is poorly equipped to handle. Traditional software fails *loudly*. A null pointer exception crashes the program, a network timeout returns an error code. These visible failures enable rapid detection and response. In contrast, ML systems fail *silently*\index{Silent Failure!detection challenges} because degraded predictions look like normal predictions. The primary mechanism behind this silent degradation is *distribution shift*.

::: {#dfn-responsible-engr-distribution-shift .callout-definition title="Distribution shift"}

***Distribution shift***\index{Distribution Shift!responsibility lens}, introduced in @sec-introduction and operationalized for drift detection in @sec-ml-operations, is the deployed-model violation of the stationarity assumption\index{Stationarity Assumption!distribution shift} $(P_0 \neq P_t)$ that underpins supervised learning. Recalled here for its responsibility consequences, it is the umbrella term for a family of drift types: data drift\index{Data Drift!definition} occurs when $p(x)$ shifts while $p(y \mid x)$ remains stable; concept drift\index{Concept Drift!definition} occurs when $p(y \mid x)$ itself shifts.

1.  **Significance (quantitative)**: Accuracy degradation can be measured against divergence statistics such as Jensen-Shannon divergence $\mathcal{D}_{\text{JS}}(P_t \lVert P_0)$, a bounded and symmetric distribution-distance measure used for the same drift-monitoring purpose as PSI and KL divergence in @sec-ml-operations-data-quality-monitoring-c6b6. Useful alert thresholds must be calibrated empirically for each task, representation, label process, and deployment environment. A $\mathcal{D}_{\text{JS}}$ value of 0.1 may be harmless for one feature space and severe for another. This degradation occurs regardless of code quality, because the model is correct given its training distribution; the environment changed, not the code.
2.  **Observation**: Unlike model error\index{Model Error!definition} (which is a learning failure caused by the algorithm or data quality at training time), distribution shift is an environmental failure\index{Environmental Failure!distribution shift}: the model's learned mapping was correct at training time but is no longer representative of current reality.
3.  **Pitfall**: A frequent misconception is that "data drift" and "distribution shift" are different concepts at the same level of the hierarchy. Distribution shift is the umbrella; data drift and concept drift are its two distinct subtypes. A system can experience data drift without concept drift (the inputs change, but the relationship holds), or concept drift without data drift (inputs are stable, but the correct output changes).
...
**Math** (conceptual, assuming normalized metrics on a common scale): @eq-alignment-gap captures the gap:
...
Each documented success shares the same structural move: a vague responsibility goal becomes an engineering constraint that can be specified, tested, communicated, and, when necessary, used to stop deployment. Following the findings of Gender Shades, a 2018 audit that exposed severe error-rate disparities in commercial facial recognition [@buolamwini2018gender], Microsoft invested in improving facial recognition\index{Facial Recognition!demographic disparities} performance across demographic groups\index{Bias!mitigation strategies}. Targeted data collection, model changes, and systematic disaggregated evaluation gave the team an explicit error-rate target, and Microsoft reduced error rates for darker-skinned subjects by up to 20 times, bringing error rates below 2 percent for all demographic groups [@raji2019]. The company published these improvements transparently, enabling external verification and preserving enterprise trust while competitors faced regulatory scrutiny and contract cancellations.

Twitter's automatic image cropping system\index{Image Cropping!racial bias} shows the same discipline under a different constraint. In 2020, users discovered racial bias in which faces appeared in preview thumbnails. Twitter characterized the problem quantitatively, published results for external review, and then removed automatic cropping entirely after determining that no technical solution could guarantee equitable outcomes across all contexts [@yee2021]. In that case, responsible engineering meant recognizing that the safe system design was feature removal\index{Feature Removal!principled design choice}, not a better threshold.

Differential privacy\index{Differential Privacy!mathematical guarantees} makes the pattern formal: a privacy requirement becomes a mathematical guarantee rather than a policy aspiration [@dwork2008].[^fn-differential-privacy-tradeoff] Systems that use differential privacy must calibrate noise to balance utility against privacy, track privacy budget across repeated analyses, and document the chosen privacy parameters. Spotify applied the same conversion at the user-interface layer, exposing why songs were recommended and giving users direct controls over recommendation behavior; transparency\index{Algorithmic Transparency!user controls} became a product mechanism rather than a compliance label.
...
# │
# │ Imports: mlsysim.book (fmt), mlsysim (Literature.Fairness — Gender Shades error rates)
# │ Exports: error_*_str, disparity_*_str, acc_*_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Literature
from mlsysim.fmt import fmt, fmt_int, fmt_multiple, check, MarkdownStr, fmt_percent
...
class TestingConstraintAnchor:
    ...
    minority_pct_str = fmt_percent(minority_pct / 100, precision=0, commas=False, style='symbol')
...
class GenderShadesDisparity:
    ...
    error_light_female_str = fmt_percent(err_light_female / 100, precision=1, commas=False, style='prose')
    error_dark_male_str = fmt_percent(round(int(err_dark_male)) / 100, precision=0, commas=False, style='prose')
    ...
    disparity_dark_male_factor_mult_str = fmt_multiple(int(disparity_dark_male), precision=0, commas=False)
...
| **Light-skinned females** | `{python} GenderShadesDisparity.error_light_female_str` | `{python} GenderShadesDisparity.disparity_light_female_factor_mult_str` higher |
| **Dark-skinned males**    |   `{python} GenderShadesDisparity.error_dark_male_str`  |  `{python} GenderShadesDisparity.disparity_dark_male_factor_mult_str` higher   |
| **Dark-skinned females**  |  `{python} GenderShadesDisparity.error_dark_female_str` |       `{python} GenderShadesDisparity.disparity_factor_mult_str` higher        |

: **Gender Shades Facial Recognition Error Rates**: Disaggregated evaluation reveals that aggregate accuracy metrics conceal severe performance disparities. Systems that appear highly accurate overall show error rates varying by more than `{python} GenderShadesDisparity.disparity_factor_mult_str` across demographic groups. Worst-case results across systems studied; source: @buolamwini2018gender. {#tbl-gender-shades-results tbl-colwidths="[34,33,33]"}
...
2.  **Observation**: Unlike AI ethics (which articulates aspirational values), responsible AI engineering translates those values into measurable, testable invariants that can be verified through automated testing and continuous monitoring, using the same lifecycle practices that enforce latency SLOs.
3.  **Pitfall**: A frequent misconception is that responsibility is "added" at the end of development. The constraints imposed on the Data axis (what data can be collected) propagate forward to constrain the Algorithm axis (what biases will be encoded) and the Machine axis (what audit trails must be kept), making late-stage remediation structurally impossible.

:::

By the time a system reaches legal or ethics review, architectural decisions have already constrained the space of possible fairness interventions. A review conducted before deployment can identify problems but faces limited remediation options: if the team trained the model without fairness constraints, if the architecture cannot support interpretability requirements, or if the data pipeline lacks the demographic attributes needed for monitoring, the review can only recommend accepting or rejecting the existing system, not repairing it. Engineering involvement from project inception enables *proactive design* rather than *reactive assessment*.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_int, MarkdownStr, fmt_percent, fmt_count, fmt_pp
...
**Stratified Sampling**\index{Stratified Sampling!fairness evaluation}: Specifically targeting this group (for example, via active learning or community outreach) requires only `{python} RepresentationStats.repr_equation_math`
...
::: {.column-margin}
![](images/svg/vol1_responsible_engr_margin_001.svg){width="100%" fig-alt="Random sampling versus targeted stratified evaluation for a 1 percent subgroup."}
...
::: {.column-margin}
![](images/svg/vol1_responsible_engr_margin_002.svg){width="100%" fig-alt="Automation reliability rises while human vigilance falls."}
...
: **Example Model Card: MobileNetV2 for Edge Deployment**: Abstract model card categories translate to practical documentation that guides responsible deployment decisions. {#tbl-model-card-example tbl-colwidths="[22,78]"}
...
The disaggregated evaluation that exposed the Gender Shades disparities (@sec-responsible-engineering-testing-challenge-77b0) generalizes into a standing testing discipline. Aggregate performance metrics mask disparities across user populations, the **Flaw of Averages**\index{Flaw of Averages} [@savage2009flaw]; responsible testing requires disaggregated evaluation that examines performance for each relevant subgroup.
...
|:------------------------------|:------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **ResNet-50 (Compute Beast)** | Training data bias (underrepresentation of minority groups in ImageNet)                         | Disaggregated accuracy by demographic group                                        | Gender Shades: `{python} GenderShadesDisparity.acc_light_pct_str` accuracy on light-skinned males, `{python} GenderShadesDisparity.acc_dark_pct_str` on dark-skinned females [@buolamwini2018gender] |
...
**Systems insight**: Fairness evaluation must match the archetype's failure mode. Vision models require demographic stratification of accuracy; large language models (LLMs) require toxicity and stereotype probing; recommendation systems require exposure audits; TinyML requires acoustic environment diversity testing. The Lighthouse keyword spotting (KWS) system introduced in @sec-ml-systems as the Tiny Constraint lighthouse faces exactly this challenge for its DS-CNN, a depthwise-separable convolutional neural network (CNN): trained on clean studio audio, it must perform equitably across accents, background noise levels, and speaker demographics in production homes—a governance challenge we examine in @sec-responsible-engineering-data-governance-compliance-bd1a.
...
Tool choice matters only after the team has named the fairness metric, subgroup slices, and alert thresholds. Open-source libraries such as Fairlearn [@bird2020fairlearn], AI Fairness 360 [@bellamy2019], and Google's What-If Tool lower the implementation cost of disaggregated and intersectional evaluation, but a library can only compute the metric the engineer asks for. It cannot decide which subgroup definition matters, which disparity threshold should page the team, or which fairness constraint is appropriate for the deployment context.
...
# │ Exports: a_*_str, b_*_str, dp_disparity_str, tpr_disparity_str, fpr_disparity_str,
# │          majority_acc_str, overall_acc_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, fmt_percent, fmt_pp
...
class LoanFairness:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a_approval_str = fmt_percent(a_app_pct / 100, precision=0, commas=False, style='prose')
    b_approval_str = fmt_percent(b_app_pct / 100, precision=0, commas=False, style='prose')
    dp_disparity_str = fmt_pp(dp_disparity, precision=0, attributive=True)
    a_tpr_str = fmt_percent(a_tpr_pct / 100, precision=0, commas=False, style='prose')
    b_tpr_str = fmt_percent(b_tpr_pct / 100, precision=0, commas=False, style='prose')
    tpr_disparity_str = fmt_pp(tpr_disparity, precision=0, attributive=True)
    a_fpr_str = fmt_percent(a_fpr_pct / 100, precision=0, commas=False, style='prose')
    b_fpr_str = fmt_percent(b_fpr_pct / 100, precision=0, commas=False, style='prose')
    a_fnr_str = fmt_percent(a_fnr_pct / 100, precision=0, commas=False, style='prose')
    b_fnr_str = fmt_percent(b_fnr_pct / 100, precision=0, commas=False, style='prose')
    ...
    fpr_disparity_str = fmt_pp(fpr_disparity, precision=0, attributive=True)
    majority_acc_str = fmt_percent(majority_acc_pct / 100, precision=0, commas=False, style='prose')
...
: **Confusion Matrix for Group A (Majority)**: Loan approval outcomes for `{python} LoanFairness.a_total_str` applicants from the majority demographic group. The `{python} LoanFairness.a_tpr_str` true positive rate (`{python} LoanFairness.a_tp_str` approved of `{python} LoanFairness.a_qualified_str` qualified) and `{python} LoanFairness.a_fpr_str` false positive rate establish the baseline for fairness comparison. {#tbl-confusion-group-a}
...
: **Confusion Matrix for Group B (Minority)**: Loan approval outcomes for `{python} LoanFairness.b_total_str` applicants from the minority demographic group. The `{python} LoanFairness.b_tpr_str` true positive rate (`{python} LoanFairness.b_tp_str` approved of `{python} LoanFairness.b_qualified_str` qualified) reveals a `{python} LoanFairness.tpr_disparity_str` disparity compared to Group A, indicating the model applies stricter criteria to minority applicants. {#tbl-confusion-group-b}
...
Equalized odds\index{Fairness Metrics!equalized odds}[^fn-equalized-odds-postprocessing] requires both equal true positive rates and equal false positive rates. Group A shows an FPR of \(`{python} LoanFairness.a_fp_str` / (`{python} LoanFairness.a_fp_str` + `{python} LoanFairness.a_tn_str`) = `{python} LoanFairness.a_fpr_str`\), and Group B shows \(`{python} LoanFairness.b_fp_str` / (`{python} LoanFairness.b_fp_str` + `{python} LoanFairness.b_tn_str`) = `{python} LoanFairness.b_fpr_str`\). While false positive rates are equal, the true positive rate disparity means equalized odds is violated.
...
| **False Positive Rate** |   `{python} LoanFairness.a_fpr_str`    |   `{python} LoanFairness.b_fpr_str`    | `{python} LoanFairness.fpr_disparity_str` |
...
# │
# │ Imports: mlsysim.book (fmt)
# │ Exports: hire_value_k_str, bad_hire_cost_k_str, fp_increase_pp_str, utility_loss_str
# └─────────────────────────────────────────────────────────────────────────────

from mlsysim.fmt import fmt, fmt_usd, check, MarkdownStr, fmt_percent, fmt_pp
...
class FairnessPrice:
    ...
    utility_loss_fraction = loss_fraction_within_group * minority_group_share
    utility_loss_pct = utility_loss_fraction * 100.0

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(net_change_per_applicant < 0 * USD, "The fairness tax only exists when extra bad-hire cost exceeds extra hire value")
    check(5.0 < utility_loss_pct < 7.0, f"Aggregate utility tax {utility_loss_pct:.1f}% no longer matches the stated assumptions.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # Custom "$" prefix / " percent" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    ...
    utility_loss_str = fmt_percent(utility_loss_fraction, precision=0, commas=False, style='prose')  # computed from assumptions above
...
1.  Closing the `{python} FairnessPrice.tpr_gap_pp_str` TPR gap requires accepting a `{python} FairnessPrice.fp_increase_pp_str` increase in False Positives for the disadvantaged group: near the original threshold most remaining candidates do not meet the bar, so each additional qualified hire admits several unqualified ones.
2.  If the value of a successful hire is `{python} FairnessPrice.hire_value_k_str` and the cost of a bad hire is `{python} FairnessPrice.bad_hire_cost_k_str`, both sides of the intervention must be counted:
    ...
    - Under the assumption that the disadvantaged group is `{python} FairnessPrice.minority_share_str` of the applicant pool and that the base rate of qualified applicants is `{python} FairnessPrice.positive_base_rate_str`, the added bad-hire cost outweighs the added hire value and the aggregate utility loss is `{python} FairnessPrice.utility_loss_str`.

**Systems insight**: The "Price of Fairness"\index{Price of Fairness!utility trade-off} in this system is a `{python} FairnessPrice.utility_loss_str` utility tax under the stated assumptions, a system constraint, not a bug. The tax is not automatic: when a TPR gap reflects a miscalibrated threshold, closing it can even raise net utility. It appears precisely when the baseline threshold is already near the utility optimum, so the marginal admits skew unqualified; the engineer's job is to present the Pareto frontier to stakeholders so they can choose the Utility/Fairness trade-off that aligns with organizational values.
...
A loan applicant denied credit by an algorithmic system has a right to know *why*, not in aggregate statistical terms but in terms specific to her application. Explainability\index{Explainability!definition and purposes}[^fn-explainability-interpretability] provides this capability: it enables human oversight of automated decisions, supports debugging when problems emerge, and satisfies regulatory requirements for decision transparency\index{Transparency!regulatory requirements}.

[^fn-explainability-interpretability]: **Explainability vs. Interpretability**\index{Explainability!architecture trade-off}: *Interpretability* is an intrinsic model property---the degree to which a human can understand internal mechanics (linear regression is interpretable; a 100-layer network is not). *Explainability* is a post-hoc capability added without changing the model (LIME, SHAP). The systems implication: interpretable models constrain architecture selection (simpler models, fewer features), while post-hoc explanations add a separate computation path whose cost depends on the method, model, and serving workflow. Regulations like the EU AI Act demand "meaningful information about the logic involved" without specifying which approach, leaving the latency-vs.-architecture trade-off to engineering teams.
...
::: {#ws-responsible-engr-hospital-shortcut .callout-war-story title="The hospital shortcut (2018)"}
...
**Failure mode**: Models that appeared strong on internal data performed worse on external hospitals. The problem was not simply architecture quality: hospital-specific prevalence, acquisition patterns, and dataset artifacts created shortcuts that did not transfer cleanly across sites.
...
The explainability requirements outlined earlier carry the force of law, not merely of engineering best practice. The EU AI Act, which entered into force on August 1, 2024 and applies in phases, imposes documentation, transparency, human-oversight, and risk-management obligations for high-risk systems [@euaiact2024]. US regulators also require adverse action notices in lending contexts, including when algorithmic tools contribute to credit decisions [@cfpb2022adverseactionalgorithms]. Regulations transform explainability from a design choice into a compliance requirement with concrete penalties for failure, making the technical mechanisms just described prerequisites for legal operation.
...
# │
# │ Imports: mlsysim.core.units (MILLION), mlsysim.fmt (fmt, check)
...
[^fn-gdpr-automated-decisions]: **GDPR (General Data Protection Regulation) Articles 15 and 22**\index{GDPR!Article 22 requirements}: Article 22 restricts certain solely automated decisions with legal or similarly significant effects and, for specified exceptions, requires safeguards including human intervention, the ability to express a point of view, and the ability to contest the decision. Article 15(1)(h) contains the access right to meaningful information about the logic involved in such automated decision-making. The European Data Protection Board's guidance emphasizes that required human oversight must be substantive and not merely a "rubber-stamping" exercise. A system making `{python} GDPRReviewLoad.daily_decisions_str` daily decisions with a `{python} GDPRReviewLoad.review_rate_str` error rate requiring substantive review would generate `{python} GDPRReviewLoad.review_cases_str`, an operational load that is untenable without built-in summarization and audit tools.

US sectoral law reaches the same capabilities through domain-specific evidence requirements. These regulations are less unified than the EU AI Act, but they force the same engineering posture through domain-specific duties. In the credit example, the Equal Credit Opportunity Act (ECOA) and its implementing Regulation B require specific reasons for adverse credit decisions even when complex algorithms contribute to the decision [@cfpb2022adverseactionalgorithms]. If consumer-report information or credit scores influence the decision, the Fair Credit Reporting Act (FCRA) adds notice obligations; if the same scoring machinery is used for housing, the Fair Housing Act (FHA) adds a discrimination-prohibition constraint. The technical consequence is practical rather than abstract: the system must connect model scores to reason codes, preserve the data sources used in the decision, support review, and monitor subgroup outcomes rather than relying on aggregate accuracy alone.

Healthcare regulations, including the Health Insurance Portability and Accountability Act (HIPAA)[^fn-hipaa-ml-constraints] and FDA guidance, impose the same pattern with different artifacts: protected-health-information controls, validation records, audit logs, and incident response. Employment systems likewise require evidence that automated screening does not reproduce discriminatory hiring practices. Across domains, the task is to translate each obligation into a concrete capability: explanation, human review, lineage, access control, deletion, monitoring, or incident response. The deployment checkpoint that follows is therefore not a US-sectoral checklist; it is the common production contract implied by the regulatory landscape.

[^fn-hipaa-ml-constraints]: **HIPAA (Health Insurance Portability and Accountability Act)**\index{HIPAA!ML system constraints}: Enacted 1996, with Privacy Rule (2003) and Security Rule (2005) establishing standards for protected health information. ML-specific constraints are stringent: training data containing PHI must be de-identified, model outputs that could re-identify patients may constitute PHI themselves, and audit logs must be retained for six years. Penalties reach \$50,000 per violation with \$1.5 million annual maximums per category---sufficient to make a single poorly governed ML pipeline an existential financial risk for a healthcare startup.
...
from mlsysim.fmt import fmt, check, MarkdownStr, fmt_latency, fmt_params, fmt_power, fmt_time
...
class EdgeEfficiencyCalc:
    ...
    cam_fps_str = MarkdownStr(str(round(cam_fps)))
...
| **Embedded Camera**    |  `{python} EdgeEfficiencyCalc.cam_power_w_str`  | `{python} EdgeEfficiencyCalc.cam_fps_str` FPS (`{python} EdgeEfficiencyCalc.cam_latency_str`) | Real-time object detection, surveillance    |
...
: **Edge Deployment Constraints**: Power and latency requirements across four deployment contexts. Smartphones allow `{python} EdgeEfficiencyCalc.smart_power_w_str` and `{python} EdgeEfficiencyCalc.smart_latency_str` latency for photo enhancement and voice assistants. IoT sensors operate at `{python} EdgeEfficiencyCalc.iot_power_mw_str` with `{python} EdgeEfficiencyCalc.iot_latency_s_str` tolerance for anomaly detection. Embedded cameras require `{python} EdgeEfficiencyCalc.cam_power_w_str` at `{python} EdgeEfficiencyCalc.cam_latency_str` (`{python} EdgeEfficiencyCalc.cam_fps_str` FPS) for real-time object detection. Wearables budget `{python} EdgeEfficiencyCalc.wear_power_mw_str` with `{python} EdgeEfficiencyCalc.wear_latency_str` latency for health monitoring. These concrete constraints transform abstract efficiency discussions into engineering requirements. {#tbl-edge-deployment-constraints tbl-colwidths="[17,16,24,43]"}
...
# │
# │ Imports: mlsysim (Infrastructure.Pricing.Cloud), mlsysim.core.units (MILLION, SEC_PER_HOUR, hour, ms)
...
class InferenceCostCalc:
    ...
    inference_latency = 10 * ms  # scenario assumption: inference latency
...
Inference costs dominate. With `{python} InferenceCostCalc.users_daily_m_str` users each receiving `{python} InferenceCostCalc.recs_per_user_str` recommendations per day, the system serves `{python} InferenceCostCalc.inferences_m_str` inferences daily. Assuming `{python} InferenceCostCalc.inference_ms_str` per inference on GPU hardware, the system requires approximately `{python} InferenceCostCalc.gpus_str` running continuously. At `{python} InferenceCostCalc.gpu_inf_rate_str`, annual GPU costs reach `{python} InferenceCostCalc.annual_inf_usd_str`.

Over a three-year operational period, quarterly retraining produces total training costs of approximately `{python} InferenceCostCalc.lifecycle_train_usd_str`, while inference costs over the same period total `{python} InferenceCostCalc.lifecycle_inf_usd_str`. The `{python} InferenceCostCalc.ratio_str`:1 ratio between inference and training costs is typical for production systems, directing optimization effort toward inference latency and serving efficiency rather than training speed.
...
# │
# │ Imports: mlsysim (Hardware.Cloud.A100.tdp), mlsysim.core.units (Infrastructure.Pricing.Fleet.CarbonPerGpuHr.rate, watt, ureg)
# │          mlsysim.fmt (fmt, check)
# │ Exports: CarbonFactor.gpu_power_w_str, CarbonFactor.gpu_power_kw_str,
# │          CarbonFactor.carbon_intensity_str, CarbonFactor.carbon_per_gpu_hr_kg_str
# │
...
These TCO examples share three baseline assumptions:
...
*   **Conversion factor**: (`{python} CarbonFactor.gpu_power_kw_str` $\times$ `{python} CarbonFactor.gpu_hour_str`) $\times$ `{python} CarbonFactor.carbon_intensity_str` = `{python} CarbonFactor.carbon_per_gpu_hr_kg_str` CO~2~eq per GPU-hour.
...
# │ Exports: TCOCalc.t_data_prep_calc_str, TCOCalc.t_data_prep_carbon_kg_str,
# │          TCOCalc.t_hparam_calc_str, TCOCalc.t_hparam_carbon_kg_str,
# │          TCOCalc.t_final_calc_str, TCOCalc.t_final_carbon_kg_str,
# │          TCOCalc.t_subtotal_usd_str, TCOCalc.t_subtotal_carbon_kg_str,
# │          TCOCalc.t_cycles_calc_str, TCOCalc.t_cycles_str,
# │          TCOCalc.t_total_usd_str, TCOCalc.t_total_carbon_kg_str,
# │          TCOCalc.i_daily_q_calc_str, TCOCalc.i_gpu_sec_calc_str,
# │          TCOCalc.i_gpu_hr_day_str, TCOCalc.i_daily_carbon_kg_str,
# │          TCOCalc.i_annual_calc_str, TCOCalc.i_annual_carbon_kg_str,
# │          TCOCalc.i_total_usd_str, TCOCalc.i_carbon_kg_str,
# │          TCOCalc.o_monitor_annual_usd_str, TCOCalc.o_monitor_usd_str,
# │          TCOCalc.o_oncall_annual_usd_str, TCOCalc.o_oncall_usd_str,
# │          TCOCalc.o_incident_annual_usd_str, TCOCalc.o_incident_usd_str,
# │          TCOCalc.o_total_usd_str, TCOCalc.total_tco_str,
# │          TCOCalc.total_carbon_t_str, TCOCalc.i_carbon_t_str,
# │          TCOCalc.t_total_carbon_t_str, TCOCalc.t_total_k_usd_str,
# │          TCOCalc.p_train_str, TCOCalc.p_inf_str, TCOCalc.p_ops_str, TCOCalc.p_total_str,
# │          TCOCalc.i_daily_queries_m_str, TCOCalc.i_latency_ms_str,
# │          TCOCalc.i_annual_cost_k_str, TCOCalc.i_total_m_usd_str,
# │          TCOCalc.o_monitor_annual_k_usd_str, TCOCalc.o_oncall_annual_k_usd_str,
# │          TCOCalc.o_incident_annual_k_usd_str, TCOCalc.o_total_k_usd_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_count, fmt_emissions, fmt_int, fmt_usd, check, MarkdownStr, fmt_percent, fmt_time
from mlsysim import Infrastructure
from mlsysim.core.units import (
    THOUSAND,
    MILLION,
...
class TCOCalc:
    ...
    i_users = 10_000_000
    i_recs_per_user = 20
    i_latency = 10 * ms  # scenario assumption
    ...
    t_data_prep_calc_str = MarkdownStr(
        f"{t_data_prep_hrs:.0f} GPU-hr $\times$ \\${train_gpu_rate_per_hour:.0f} = \\${(t_data_prep_time * train_gpu_rate).to(USD).magnitude:,.0f}"
    )
    t_hparam_calc_str = MarkdownStr(
        f"{t_hparam_exps} $\times$ \\${t_hparam_cost_exp_usd:.0f} = \\${(t_hparam_exps * t_hparam_cost_exp).to(USD).magnitude:,.0f}"
    )
    t_final_calc_str = MarkdownStr(
        f"{t_final_hrs:.0f} GPU-hr $\times$ \\${train_gpu_rate_per_hour:.0f} = \\${(t_final_time * train_gpu_rate).to(USD).magnitude:,.0f}"
    )
    t_cycles_calc_str = MarkdownStr(f"{t_cycles_3yr // 3}/year $\times$ 3 years = {t_cycles_3yr}")
    t_subtotal_usd_str = fmt_usd(train_cost_cycle, precision=0, commas=True)
    t_total_usd_str = fmt_usd(train_tco_3yr, precision=0, commas=True)
    ...
    i_daily_q_calc_str = MarkdownStr(f"{i_users/MILLION:.0f}M $\times$ {i_recs_per_user} = {inf_daily_total/MILLION:.0f}M")
    i_gpu_sec_calc_str = MarkdownStr(
        f"{inf_daily_total/MILLION:.0f}M $\times$ {i_latency.to(second).magnitude} s = {inf_daily_total * i_latency.to(second).magnitude/MILLION:.1f}M sec"
    )
    ...
    i_annual_calc_str = MarkdownStr(
        f"{inf_gpu_hours_day:.0f} $\times$ {DAYS_PER_YEAR} $\times$ \\${inf_gpu_rate_per_hour:.2f} = \\${inf_cost_year.to(USD).magnitude / THOUSAND:.0f}K"
    )
    ...
    o_monitor_usd_str = fmt_usd(o_monitor_yr * 3, precision=0, commas=False, scale="K")
    o_oncall_annual_usd_str = fmt_usd(o_oncall_yr, precision=0, commas=False, scale="K")
    o_oncall_usd_str = fmt_usd(o_oncall_yr * 3, precision=0, commas=False, scale="K")
    o_incident_annual_usd_str = fmt_usd(o_incident_yr, precision=0, commas=False, scale="K")
    o_incident_usd_str = fmt_usd(o_incident_yr * 3, precision=0, commas=False, scale="K")
    ...
    p_total_str = fmt_percent(p_total / 100, precision=1, commas=False, style='prose')
...
: **Inference Cost Calculation**: Inference costs scale with query volume: `{python} TCOCalc.i_daily_queries_m_str` daily queries at `{python} TCOCalc.i_latency_ms_str` each require `{python} TCOCalc.i_gpu_hr_day_str` daily, totaling `{python} TCOCalc.i_annual_cost_k_str` annually and `{python} TCOCalc.i_total_m_usd_str` over three years. At `{python} TCOCalc.p_inf_str` of total cost, inference dominates for high-traffic systems and justifies aggressive per-query optimization through quantization, pruning, and efficient serving. {#tbl-tco-inference tbl-colwidths="[25,27,27,21]"}
...
: **Operational Cost Calculation**: Operational costs include monitoring infrastructure (`{python} TCOCalc.o_monitor_annual_k_usd_str`), on-call engineering at 0.5 FTE (`{python} TCOCalc.o_oncall_annual_k_usd_str`), and incident response reserves (`{python} TCOCalc.o_incident_annual_k_usd_str`). The `{python} TCOCalc.o_total_k_usd_str` three-year total represents `{python} TCOCalc.p_ops_str` of TCO and often surprises teams focused primarily on compute costs. These estimates represent minimum staffing; production systems at this scale typically require 2--5$\times$ more engineering support. These expenses persist regardless of model performance and grow with system complexity. {#tbl-tco-operations tbl-colwidths="[34,33,33]"}
...
# │          quant_carbon_tonnes_str, quant_reduction_pct_str,
# │          training_time_reduction_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
class TCOSummary:
    ...
    carbon_train_ratio_str = fmt_ratio(round(carbon_train_ratio), precision=0, commas=False)
...
|:---------------|:------------------------------------:|:----------------------------------:|:-----------------------------------------:|
| **Training**   | `{python} TCOCalc.t_total_k_usd_str` |   `{python} TCOCalc.p_train_str`   |  `{python} TCOCalc.t_total_carbon_t_str`  |
| **Inference**  |  `{python} TCOCalc.i_total_usd_str`  |    `{python} TCOCalc.p_inf_str`    |     `{python} TCOCalc.i_carbon_t_str`     |
| **Operations** |  `{python} TCOCalc.o_total_usd_str`  |    `{python} TCOCalc.p_ops_str`    |                     -                     |
| **Total TCO**  | **`{python} TCOCalc.total_tco_str`** | **`{python} TCOCalc.p_total_str`** | **`{python} TCOCalc.total_carbon_t_str`** |

: **Total Cost of Ownership Summary**: Three-year TCO breakdown: training, inference, and operations costs. The `{python} TCOSummary.inf_train_ratio_str`:1 ratio between inference and training costs is typical for production systems serving millions of daily users. A `{python} TCOSummary.quant_reduction_pct_str` reduction in inference latency through quantization saves about `{python} TCOSummary.quant_savings_str` and `{python} TCOSummary.quant_carbon_tonnes_str` of CO~2~, easily justifying the optimization engineering investment. {#tbl-tco-summary tbl-colwidths="[25,25,25,25]"}
...
- [ ] **Inference dominance**: Can you explain why a `{python} TCOSummary.quant_reduction_pct_str` inference latency reduction delivers more savings than a `{python} TCOSummary.training_time_reduction_pct_str` training time reduction for a production system serving millions of users?
...
2.  **Carbon intensity**\index{Carbon Intensity!definition}: The average US grid emits $\approx$ `{python} CarbonScaleCalc.carbon_intensity_g_kwh_str` CO~2~ (`{python} CarbonScaleCalc.carbon_intensity_str`).
...
4.  **Comparison**: A typical passenger car emits ≈ `{python} CarbonScaleCalc.car_annual_tons_str` CO~2~ per year.

**Systems insight**: Under the training-energy scenario above, the carbon footprint is equivalent to the annual emissions of `{python} CarbonScaleCalc.cars_eq_str` cars. At this scale, efficiency transforms from a technical preference into a responsibility constraint. Every 1 percent improvement in the efficiency $(\eta_{\text{hw}})$ of the training pipeline removes the equivalent of about `{python} CarbonScaleCalc.one_percent_improvement_cars_str`' annual emissions from the calculation.

::: {.column-margin}
```{=latex}
\vspace*{-38mm}
```
![](images/svg/vol1_responsible_engr_margin_003.svg){width="100%" fig-alt="Model-training emissions versus one passenger-car year."}
...
Recommendation and targeting models are the primary consumers of user data at scale, which means a governance failure in the data pipeline is simultaneously a failure in the ML system's data ingestion and validation infrastructure. In January 2023, Ireland's Data Protection Commission fined Meta Ireland for Facebook and Instagram legal-basis violations under GDPR—the "personalized advertising" engine at the center of those violations is exactly this kind of system, and the fine is a reminder that responsible engineering must produce evidence of compliance, not only intentions. Governance is the enforcement layer that makes accountability possible: fairness metrics, model cards, and impact assessments only matter at scale if the system can prove what data it used, who accessed it, which legal basis allowed processing, and whether deletion or contestability rights were honored.
...
::: {#pri-responsible-engr-compliance-engineering .callout-principle title="Compliance as Engineering Constraint"}
...
[^fn-data-lineage-compliance]: **Data Lineage Systems**\index{Data Lineage!GDPR compliance}: Apache Atlas and LinkedIn's DataHub capture metadata about data flows automatically from pipeline execution logs, creating directed graphs where nodes are datasets and edges are transformations. GDPR Article 30 requires detailed records of all processing activities, making automated lineage tracking essential: when a user invokes the right to erasure\index{Right to Erasure!data lineage requirement}, the lineage graph identifies every derived artifact---features, embeddings, trained models---that must be removed or retrained, a task that is infeasible manually at production scale.
...
::: {.column-margin}
![](images/svg/vol1_responsible_engr_margin_004.svg){width="100%" fig-alt="Margin illustration showing Append-only audit volume grows with retained decisions."}
...
# │ Exports: FairnessPitfallAnchor.tpr_disparity_str, .fnr_ratio_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │ Exports: ResponsibleTcoRecap.p_train_str, p_inf_str,
# │          carbon_train_ratio_str, quant_reduction_pct_str,
# │          quant_savings_str, quant_carbon_tonnes_str
from mlsysim import Infrastructure
...
Public discourse focuses on the carbon cost of training runs, and engineers naturally follow this framing when assessing environmental responsibility. The TCO analysis in @sec-responsible-engineering-total-cost-ownership-35c1 reveals why this focus is misplaced: inference-to-training cost ratios can exceed 40:1 over a model's operational lifetime. A model trained once but served millions of times daily has its environmental footprint dominated by inference, not training. For the recommendation system analyzed in @tbl-tco-summary, training accounts for just `{python} ResponsibleTcoRecap.p_train_str` of three-year costs while inference accounts for `{python} ResponsibleTcoRecap.p_inf_str`. The carbon ratio is even more lopsided in this example, with inference emitting about `{python} ResponsibleTcoRecap.carbon_train_ratio_str` times as much CO~2~ as training. Engineers who optimize training efficiency while ignoring per-query inference costs address the smaller term in a lopsided equation, leaving the dominant source of environmental impact unexamined.

**Fallacy**: *Model weights are exempt from data governance and deletion requests.*

Teams often assume that once training data has been compiled into model weights, the data is gone and compliance obligations no longer apply. This assumption is wrong on both technical and legal grounds. Models can memorize training data: membership inference attacks (@sec-responsible-engineering-technical-privacy-protection-methods-4a74) demonstrate that an attacker can determine with high confidence whether a specific record appeared in the training set, meaning the model itself constitutes a form of data retention. When a user invokes the right to erasure under GDPR, a system that deletes the source records but keeps a model trained on those records has not fulfilled the obligation, because the weights encode the data's statistical signature and can leak it. The correct engineering response requires retraining from scratch or applying machine unlearning techniques, and data lineage systems must track which user data contributed to which model versions so that triggered deletions propagate through the artifact graph, not only through the raw storage layer. Teams that treat model artifacts as outside the data governance perimeter create a compliance gap that grows with every deployment and cannot be closed after the fact.\index{Data Governance!model artifact compliance}\index{Model Weights!data governance obligations}
...
The key insight unifying these tools is that translating responsibility concerns into measurable properties\index{Measurable Property!responsibility requirements} makes them tractable. "Fairness gap <5 percent across groups" is actionable; "be fair" is not. This translation extends beyond fairness: efficiency becomes carbon accounting and TCO analysis, where a `{python} ResponsibleTcoRecap.quant_reduction_pct_str` latency reduction through quantization saves `{python} ResponsibleTcoRecap.quant_savings_str` and eliminates `{python} ResponsibleTcoRecap.quant_carbon_tonnes_str` of CO~2~. Documentation becomes model cards with explicit intended use and known limitations. Governance becomes access control, lineage tracking, and audit infrastructure that makes compliance demonstrable rather than aspirational. At every level, the same pattern holds: abstract ethical obligations become concrete engineering requirements that can be specified, tested, monitored, and enforced.
...
* **Correctness is insufficient**: A model can achieve 95 percent accuracy while showing `{python} GenderShadesDisparity.disparity_factor_mult_str` error rate disparities across demographic groups. Aggregate metrics conceal failures that disaggregated, intersectional evaluation reveals.
...
* **Efficiency–responsibility alignment**: A 4$\times$ more efficient model\index{Efficiency!responsibility alignment} uses 4$\times$ less energy, costs 4$\times$ less, and enables 4$\times$ more organizations to deploy. Inference costs dominate TCO by `{python} TCOSummary.inf_train_ratio_str`:1 over training, making per-query optimization the highest-leverage responsibility intervention.
...
* **Governance as infrastructure**: Data lineage, audit trails, access controls, and privacy-preserving techniques must be built into pipelines from inception. Regulations like GDPR impose specific technical capabilities, including erasure workflows, meaningful information about automated decision logic, and substantive human review where required, that cannot be retrofitted.
