...
- **Model performance metrics** (requiring ground truth, available with delay): sensitivity (target >90%, alert if 7-day rolling average drops below 88%), specificity (target >80%, alert if drops below 78%), and subgroup performance (alert if any demographic drops >5% below baseline).
...
- **Operational metrics**: inference latency (P95 <50ms, alert if >100ms), throughput (alert if queue depth >50 images), and error rate (alert if >0.1% of requests fail).
