...
[]{#sec-ai-development-workflow-quantifying-ml-lifecycle-bd69}
...
The sensitivity and specificity targets from the audit example above must be achieved across diverse patient populations, camera equipment, and image quality conditions typical in resource-limited settings.
...
## Data Collection and Preparation {#sec-ai-development-workflow-data-collection-preparation-stage-ae99}
...
Problem definition decisions shape data requirements in the DR example. The multi-dimensional success criteria established (accuracy across diverse populations, hardware efficiency, and regulatory compliance) demand a data collection strategy that goes beyond typical computer vision datasets. Moreover, not all data contributes equally to learning — @sec-data-selection shows that strategically selecting training examples can match the accuracy of the full dataset at a fraction of the compute cost, a principle that becomes critical when iteration velocity determines project success.
...
As the Bandwidth vs. Compute exercise above quantified, this data volume makes cloud-only processing infeasible; the architectural conclusion is edge deployment using specialized hardware such as NVIDIA Jetson[^fn-nvidia-jetson]. Local preprocessing reduces bandwidth requirements by orders of magnitude but requires materially more local computation, shaping both model optimization strategies and deployment hardware requirements.
...
## Model Development and Training {#sec-ai-development-workflow-model-development-training-stage-d901}
...
Optimizing for clinical performance alone proves insufficient. Edge deployment constraints from the data collection phase impose additional requirements: the model must run efficiently on resource-limited hardware while maintaining inference speeds compatible with clinical workflows. Improvements in one dimension often come at the cost of others—the Operations ($O$) term and the Overhead ($L_{lat}$) term from the Iron Law pull in opposite directions. @sec-dnn-architectures explores model capacity, while @sec-ml-system-architecture discusses deployment feasibility, and the fundamental tension between them drives architectural decisions. Systematic application of quantization, pruning, and knowledge distillation[^fn-model-compression-workflow] techniques can bridge the gap, meeting deployment requirements while aiming to preserve clinical utility.
...
Development begins when data scientists collaborate with domain experts (like ophthalmologists in medical imaging) to identify characteristics indicative of target conditions. This interdisciplinary approach ensures that model architectures capture clinically relevant features while meeting the computational constraints identified during data collection.
...
At the deployment and integration stage, trained models integrate into production systems and workflows. In Iron Law terms, this stage focuses on minimizing the **Overhead ($L_{lat}$)** term through efficient serving infrastructure, and the binding constraint varies by archetype: ResNet-50-class workloads optimize batch size for throughput, DLRM-class workloads enforce strict SLA latency, and TinyML-class workloads operate under sub-milliwatt energy budgets (@tbl-lighthouse-workflow-comparison). Deployment requires addressing practical challenges such as system compatibility, scalability, and operational constraints. Successful integration makes the model's predictions accurate and actionable in real-world settings where resource limitations and workflow disruptions pose barriers. @sec-machine-learning-operations-mlops covers the operational aspects of deployment and maintenance.
...
Deployment proceeds through phases that progressively expose the system to real-world complexity because each phase catches different failure modes. Simulated environments catch integration issues before any real users are affected. Pilot sites reveal real-world variability invisible in simulation: equipment differences, operator skill levels, patient population diversity. Full deployment exposes scale effects that pilot sites cannot replicate: network contention, storage bottlenecks, and rare edge cases that appear only at volume. Teams begin with simulated environments replicating target constraints, then deploy to pilot sites for controlled real-world testing, gathering feedback from clinical staff before scaling to full deployment.
...
Proactive maintenance becomes essential: predictive models identify potential problems from operational patterns, while continuous learning pipelines retrain on new data. Production insights inform refined problem definitions, data quality improvements, and architectural enhancements. The feedback arrows in @fig-ml-lifecycle capture this dynamic, closing the loop that distinguishes ML systems from traditional linear development. These dynamics are not isolated phenomena—they reflect deeper structural patterns that we now synthesize.

## Systems Thinking Principles {#sec-ai-development-workflow-integrating-systems-thinking-principles-24c0}

The complete lifecycle from problem definition through monitoring reveals deeper patterns that unified every stage. The DR case study was not just a sequence of technical decisions; it revealed four fundamental systems thinking principles that appeared repeatedly. Constraint propagation surfaced when bandwidth constraints drove the edge deployment decision. Multi-scale feedback emerged in monitoring designs spanning minute-level quality checks to quarterly capacity reviews. Emergent complexity appeared when system-wide demographic biases proved invisible to single-site monitoring. Resource optimization manifested when accuracy improvements created cascading deployment costs.
...
ML workflows introduce counterintuitive complexities that lead teams to apply familiar software patterns to fundamentally different problems. These fallacies and pitfalls capture errors that waste development cycles, cause production failures, and create technical debt that compounds as systems scale.
