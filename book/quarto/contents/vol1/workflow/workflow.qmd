...
Machine learning systems fail not because of poor algorithms, but because of poor integration between development stages. Understanding how data decisions affect model choices, how model choices constrain deployment, and how deployment experiences reshape data strategies is the difference between laboratory success and production impact. This chapter introduces the ML workflow as the governing methodology that transforms isolated technical components into coherent production systems.
...
Abstract lifecycle stages become concrete when examined through a single, complete development journey. Diabetic retinopathy screening provides an ideal pedagogical vehicle: the problem is medically significant yet technically accessible, the development history is well documented, and the deployment challenges illustrate every systems thinking principle we have introduced.
...
[^fn-hyperparameter-tuning]: **Hyperparameter Optimization Complexity**: Modern deep learning models can expose dozens of hyperparameters (learning rate, batch size, regularization, architecture choices), creating extremely large search spaces. In practice, finding good configurations can require substantial experimentation and compute budgets. Random search often outperforms grid search, while Bayesian optimization and population-based training can reduce the number of required trials by focusing search on promising regions of the space.
...
Efficient hyperparameter optimization dramatically reduces these naive multiplicative costs. Random search achieves comparable results with 20 to 60 runs rather than exhaustive grid search, Bayesian optimization learns which configurations are promising and focuses search accordingly, and early stopping terminates unpromising runs after 10 to 20 percent of training completes. Modern hyperparameter optimization thus reduces 150 naive runs to 30 to 50 actual runs while achieving similar or better results. Teams that invest in optimization infrastructure early recover the investment within the first few experiment cycles.
...
Initial deployment often highlights several areas where systems fail to meet real-world needs. Clinics with equipment older than 5 years or images with resolution below 1024x1024 pixels show 15-25% accuracy decreases. Monitoring systems detect performance drops in specific subgroups. Patients with proliferative diabetic retinopathy show 18% accuracy reduction. Images with significant cataracts show 22% sensitivity loss for elderly patients over 65. These blind spots, invisible during laboratory validation but critical in clinical practice[^fn-deployment-reality-gap], inform maintenance strategies including targeted data collection and architectural improvements.
