...
***The Constraint Propagation Principle***\index{Constraint Propagation Principle!exponential cost of late discovery} states that the cost of a constraint discovered at lifecycle stage $N$ grows exponentially relative to the stage where it should have been defined, as @eq-correction-cost formalizes:
...
* **Constraint propagation is bidirectional and exponential**: Deployment constraints (latency, memory) flow backward to model selection; data constraints (volume, quality) flow forward to architecture choices. A constraint discovered at stage $N$ costs $2^{N-1}$ times more to fix than if caught at stage 1.
