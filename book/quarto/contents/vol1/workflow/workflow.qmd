...
By the end of this chapter, you should be able to achieve the following learning objectives:

::: {.callout-tip title="Learning Objectives"}

- Map the six core ML lifecycle stages and their feedback-driven relationships as the organizing framework for subsequent chapters
...
- Assess how problem definition choices cascade through data collection, model development, and deployment
- Use the workflow framework to anticipate how changes in one stage affect the entire system

:::

## Systematic Framework for ML Development {#sec-ai-development-workflow-systematic-framework-ml-development-7a1b}

Building an ML system requires coordinating many moving parts. The chapters ahead cover data engineering (@sec-data-engineering-ml), model training (@sec-ai-training), software frameworks (@sec-ai-frameworks), hardware acceleration (@sec-ai-acceleration), model optimization (@sec-model-compression), deployment and serving (@sec-model-serving-systems), and operational monitoring (@sec-machine-learning-operations-mlops). Each of these topics involves its own engineering challenges, and decisions made in one area propagate to all others. Without a systematic framework to organize these interdependencies, teams work in silos, optimizing components in isolation and discovering integration failures only at the end. This chapter provides that framework: the **Workflow**, an engineering control system that orchestrates the complete development lifecycle.

The need for such a framework becomes clear when we consider what we already know. The **AI Triad** introduced in @sec-introduction (Data, Algorithm, and Machine, or **DAM**) identifies the three pillars of any ML system, while the **Physical Constraints** and deployment paradigms explored in @sec-ml-system-architecture (Cloud, Edge, Mobile, TinyML) determine where and how systems operate. Together, these concepts tell us *what* the components are and *where* they run. What remains is the question of *how* to coordinate them, and that is the transition from **Model Researcher** to **Systems Engineer**. A researcher optimizes individual elements: a better architecture, a cleaner dataset, a faster accelerator. A systems engineer orchestrates these elements into production systems that reliably deliver value. The workflow framework provides this orchestration logic.
...
This compounding cost of slow iteration creates what we call the *iteration tax*, which the following exercise quantifies.

::: {.callout-notebook title="The Iteration Tax"}
**Problem**: You are building a diabetic retinopathy screening model for deployment in rural clinics. You must choose between a large ensemble trained on high-resolution fundus images (training time: 1 week, accuracy: 95%) and a lightweight model suitable for edge deployment on clinic hardware (training time: 1 hour, accuracy: 90%). Which approach yields a better screening system in 6 months?
...
* **Iteration speed significantly influences success**: Teams that reduce experiment cycles from days to hours compound learning advantages. Successful teams prioritize iteration velocity alongside model accuracy.
...
The workflow framework presented here transforms ML development from ad-hoc experimentation into disciplined engineering practice. By understanding how data pipelines and model development interact through feedback loops, teams can anticipate integration challenges, allocate resources effectively, and avoid the cascading failures that derail most ML projects. The constraint propagation principle—where late-stage discoveries create exponential cost multipliers—emphasizes why systematic workflow management is not bureaucratic overhead but essential risk mitigation.
