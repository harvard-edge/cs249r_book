...
# MobileNetV2 Specs
# Assuming FP32 (4 bytes per param) for the size estimate in the text
mobilenet_size_mb = (MOBILENETV2_PARAMS * 4 * byte).to(MB).magnitude
mobilenet_flops_m = MOBILENETV2_FLOPs.to(MFLOPs).magnitude

mobilenet_size_str = f"{mobilenet_size_mb:.0f}"
mobilenet_flops_str = f"{mobilenet_flops_m:.0f}"
...
cost_nvme_str = f"{cost_nvme_gb_mo:.2f}"
cost_s3_str = f"{cost_s3_gb_mo:.3f}"
...
ML systems are not assembled from independent parts; they are *integrated wholes where decisions in one phase propagate to all others*. Data collection strategies constrain model architectures. Model choices determine deployment requirements. Monitoring insights reshape data pipelines. Without a systematic workflow that makes these couplings explicit, teams optimize stages in isolation—achieving 95% accuracy in a notebook while the deployment target has half the required memory, or building sophisticated training pipelines for a data distribution that diverges from production reality. The workflow exists because a decision made during problem definition determines whether the system can be deployed six months later, and late-stage failures almost always trace back to early-stage assumptions that no one questioned.
...
These deployment constraints interact with the four Workload Archetypes that @sec-ml-system-architecture introduced to characterize distinct computational patterns. Each archetype names a different resource bottleneck that dominates engineering decisions: *Compute Beast* workloads are limited by raw arithmetic throughput (FLOPs); *Bandwidth Hog* workloads are limited by memory bandwidth (GB/s); *Sparse Scatter* workloads are limited by data infrastructure for massive embedding tables; and *Tiny Constraint* workloads are limited by physical size, power, and memory capacity. The following summary previews how each archetype translates into workflow requirements:
...
The deployment paradigm is only one dimension of difference. The *process* of ML development itself differs fundamentally from traditional software. ML systems evolve through **iterative experimentation** rather than linear implementation. Teams hypothesize model architecture choices, experiment through training and validation, analyze performance metrics, and iterate based on findings. This scientific methodology emerged from academic research labs in the 1990s-2000s but became essential for production ML when Google, Facebook, and others discovered that empirical validation outperformed theoretical predictions in complex, real-world systems. In Software 2.0[^fn-software-2-0] (the paradigm where neural networks learn rules from data rather than having them explicitly programmed), where **Data is Source Code**, the workflow is the "Compiler" that translates raw observations into operational logic. This empirical, data-centric approach requires specialized workflow methodologies that accommodate uncertainty, coordinate parallel development streams, and establish continuous improvement mechanisms.

[^fn-software-2-0]: **Software 2.0**: See @sec-introduction for the full comparison between Software 1.0 and Software 2.0 [@karpathy2017software]. The paradigm explains why data engineering consumes 60-80% of ML project effort and why workflow systems must treat data as a first-class artifact.
...
Understanding this lifecycle requires a systems thinking\index{Systems Thinking!principles in ML}[^fn-systems-thinking] approach—analyzing how a system's parts interrelate rather than treating them in isolation. The patterns that emerge from this analysis (formalized in @sec-ai-development-workflow-integrating-systems-thinking-principles-24c0 and explored throughout this chapter using diabetic retinopathy screening as a case study) provide the analytical framework for understanding *why* ML systems require integrated engineering approaches rather than sequential component optimization.
...
**Iteration cycles** characterize successful ML projects. Return to @fig-ml-lifecycle and notice the feedback loops driving these iterations—each arrow represents a path that teams traverse repeatedly. Production-ready ML systems typically require 4-8 complete iteration cycles, where each cycle may revisit multiple stages. Understanding what triggers these iterations guides resource allocation:
...
This compounding cost of slow iteration creates what we call the *iteration tax*\index{ML Lifecycle!iteration tax}, quantified in the following exercise.
...
# PURPOSE
# =============================================================================
# Purpose: Illustrate the iteration tax for slow vs fast training cycles.
# Used in: Callout "The Iteration Tax".

# =============================================================================
# INPUT
# =============================================================================
weeks_in_6mo_value = 26
hours_per_week_value = 168

large_train_time_str = "1 week"
large_accuracy_value = 95
large_gain_per_iter_value = 0.5

small_train_time_str = "1 hour"
small_accuracy_value = 90
small_gain_per_iter_value = 0.1
small_potential_iters_value = 100  # illustrative number of iterations possible

# =============================================================================
# PROCESS
# =============================================================================
small_model_experiments_value = weeks_in_6mo_value * hours_per_week_value
large_final_value = large_accuracy_value + (weeks_in_6mo_value * large_gain_per_iter_value)
small_final_value = small_accuracy_value + (small_potential_iters_value * small_gain_per_iter_value)

# =============================================================================
# OUTPUT
# =============================================================================
weeks_in_6mo_str = fmt(weeks_in_6mo_value, precision=0, commas=False)
hours_per_week_str = fmt(hours_per_week_value, precision=0, commas=False)
small_model_experiments_str = f"{small_model_experiments_value:,}"

large_accuracy_str = fmt(large_accuracy_value, precision=0, commas=False)
small_accuracy_str = fmt(small_accuracy_value, precision=0, commas=False)
large_gain_str = fmt(large_gain_per_iter_value, precision=1, commas=False)
small_gain_str = fmt(small_gain_per_iter_value, precision=1, commas=False)
small_potential_iters_str = fmt(small_potential_iters_value, precision=0, commas=False)

large_final_str = fmt(large_final_value, precision=0, commas=False)
small_final_str = fmt(small_final_value, precision=0, commas=False)
```
...
These distinctions translate directly into the structured six-stage framework that organizes how ML projects unfold, each stage presenting unique challenges that traditional software methodologies cannot adequately address.
...
**Cost Analysis**: Proceeding without deployment paradigm selection risks discovering at stage 5 (Deployment) that the target is Edge deployment with <100ms latency and <500MB memory. By the Constraint Propagation Principle, this would cost `{python} constraint_math` the effort of resolving it now at stage 1.
...
# PURPOSE
# =============================================================================
# Purpose: Compare raw image uploads vs edge summaries for bandwidth load.
# Used in: Callout "Bandwidth vs. Compute".

# =============================================================================
# INPUT
# =============================================================================
bw_patients_per_day_value = 100
bw_photos_per_patient_value = 10
bw_mb_per_photo_value = 5

bw_upload_mbps_value = 2  # Mbps
bw_clinic_hours_value = 8

bw_summary_kb_per_patient_value = 10
...
# OUTPUT
# =============================================================================
bw_patients_str = fmt(bw_patients_per_day_value, precision=0, commas=False)
bw_photos_str = fmt(bw_photos_per_patient_value, precision=0, commas=False)
bw_mb_per_photo_str = fmt(bw_mb_per_photo_value, precision=0, commas=False)
bw_daily_mb_str = f"{bw_daily_data_mb_value:,}"
bw_daily_gb_str = fmt(bw_daily_data_gb_value, precision=0, commas=False)
bw_upload_mbps_str = fmt(bw_upload_mbps_value, precision=0, commas=False)
bw_upload_mbs_str = f"{bw_upload_mbs_value}"
bw_upload_sec_str = fmt(bw_upload_time_sec_value, precision=0, commas=True)
bw_upload_hours_str = fmt(bw_upload_time_hours_value, precision=1, commas=False)
bw_clinic_hours_str = fmt(bw_clinic_hours_value, precision=0, commas=False)
bw_bandwidth_pct_str = fmt(bw_bandwidth_pct_value, precision=0, commas=False)
bw_summary_kb_str = fmt(bw_summary_kb_per_patient_value, precision=0, commas=False)
bw_reduction_str = f"{bw_reduction_factor_value:,}"
```
...
[^fn-tiered-storage]: **Tiered Storage**: A storage architecture that places data on different storage media based on access frequency and performance requirements. The cost-performance gap between storage tiers is substantial: high-performance NVMe SSDs deliver 500,000+ IOPS (cloud block storage like AWS EBS gp2 costs approximately $`{python} cost_nvme_str`/GB/month), while object storage like S3 costs approximately $`{python} cost_s3_str`/GB/month but with 100-200ms latency. For ML workloads, the hot tier feeds training loops requiring sustained sequential reads at 1-10 GB/s, while cold archival storage holds audit trails and historical datasets accessed only during investigations or regulatory reviews.
...
@fig-mlops-leverage charts this scaling dynamic, contrasting manual workflows against automated platforms to quantify the value of the infrastructure investments discussed later in @sec-machine-learning-operations-mlops. The axes are relative units intended to show shape, not absolute throughput.
...
# PURPOSE
# =============================================================================
# Purpose: Plot MLOps leverage vs manual workflows as team size scales.
# Used in: Figure "The MLOps Leverage".
...
# DATA
# =============================================================================
team_size_value = np.linspace(1, 20, 100)
velocity_manual_value = 5 * team_size_value * np.exp(-0.05 * team_size_value)
velocity_platform_value = 5 * team_size_value ** 1.1
...
# PURPOSE
# =============================================================================
# Purpose: Compare cloud vs edge economics for DR deployment.
# Used in: Callout "Cloud vs. Edge Deployment Economics".

# =============================================================================
# INPUT
# =============================================================================
n_clinics_value = 500
patients_per_day_value = 50
days_per_year_value = 365
cloud_cost_per_image_value = 0.01
image_size_mb_value = 5  # MB per retinal image

cloud_network_value = 45_000  # approximate network costs

edge_cost_per_unit_value = 500
edge_annual_maint_value = 25_000
edge_inference_cost_value = 0.001  # $ per image (electricity only)

cloud_latency_risk_ms_value = 200  # latency that breaks clinical workflow
edge_latency_benefit_ms_value = 50  # edge latency advantage
...
# OUTPUT
# =============================================================================
n_clinics_str = f"{n_clinics_value:,}"
patients_per_day_str = fmt(patients_per_day_value, precision=0, commas=False)
days_per_year_str = fmt(days_per_year_value, precision=0, commas=False)
cloud_cost_per_image_str = fmt(cloud_cost_per_image_value, precision=2, commas=False)
cloud_network_str = f"{cloud_network_value:,}"
edge_cost_per_unit_str = f"{edge_cost_per_unit_value:,}"
image_size_mb_str = fmt(image_size_mb_value, precision=0, commas=False)
edge_inference_cost_str = f"{edge_inference_cost_value}"
cloud_latency_risk_str = fmt(cloud_latency_risk_ms_value, precision=0, commas=False)
edge_latency_benefit_str = fmt(edge_latency_benefit_ms_value, precision=0, commas=False)

cloud_annual_str = fmt(cloud_annual_value, precision=0, commas=True)
cloud_total_str = fmt(cloud_total_value, precision=0, commas=True)
edge_capex_str = fmt(edge_capex_value, precision=0, commas=True)
edge_maintenance_str = f"{edge_annual_maint_value:,}"
payback_str = fmt(payback_years_value, precision=0, commas=False)
```
...
The hierarchy matters: operational metrics catch immediate problems, proxy metrics catch model issues within hours, and performance metrics catch accuracy degradation within weeks.
...
Having traced the complete lifecycle from problem definition through monitoring, we can now step back and identify the deeper patterns that unified every stage. The DR case study was not just a sequence of technical decisions; it revealed four fundamental systems thinking principles that appeared repeatedly. Constraint propagation surfaced when bandwidth constraints drove the edge deployment decision. Multi-scale feedback emerged in monitoring designs spanning minute-level quality checks to quarterly capacity reviews. Emergent complexity appeared when system-wide demographic biases proved invisible to single-site monitoring. Resource optimization manifested when accuracy improvements created cascading deployment costs.
...
***The Constraint Propagation Principle***\index{Constraint Propagation Principle!exponential cost of late discovery} states that the cost of a constraint discovered at lifecycle stage $N$ grows exponentially relative to the stage where it should have been defined:
