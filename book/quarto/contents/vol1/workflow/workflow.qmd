...
The deployment paradigm selected during problem definition — Cloud, Edge, Mobile, or TinyML (@sec-ml-systems) — fundamentally shapes every subsequent workflow stage. This is not a late-stage implementation detail but a first-order constraint that determines what you can build.

Recall that @sec-ml-systems introduced four Workload Archetypes characterizing distinct computational patterns: Compute Beasts are compute-bound workloads like training ResNet, Bandwidth Hogs are memory-bound workloads like LLM inference, Sparse Scatter workloads involve irregular memory access patterns like recommendation systems, and Tiny Constraints operate within extreme resource limits like microcontrollers. Each archetype imposes distinct workflow constraints:
...
These quantitative realities, specifically the 60-80% data allocation, 4-8 iteration cycles, and exponential cost escalation, explain why techniques that work for traditional software fail for ML. The numbers above are not merely interesting statistics; they represent fundamental departures from the assumptions underlying traditional software engineering. To understand what makes ML development different, we must examine these differences systematically.

The metrics reveal why traditional software methodologies fail for ML: a 60-80% data time allocation, 4-8 iteration cycles, and $2^{N-1}$ cost escalation for late discoveries are foreign to waterfall or even standard agile processes. Machine learning requires specialized lifecycle approaches because ML development differs fundamentally from traditional software engineering. Traditional lifecycles consist of sequential phases: requirements gathering, system design, implementation, testing, and deployment [@royce1970managing][^fn-waterfall-model]. Each phase produces specific artifacts that serve as inputs to subsequent phases. In financial software development, the requirements phase produces detailed specifications for transaction processing, security protocols, and regulatory compliance. These specifications translate directly into system behavior through explicit programming. This deterministic approach contrasts sharply with the probabilistic nature of ML systems that @sec-introduction introduced.
...
The Stage Interface Specification provides the contract; now we need to see how these contracts play out in practice. A well-chosen case study demonstrates every aspect of lifecycle complexity because real systems face simultaneous constraints that abstract examples cannot capture: regulatory requirements collide with edge deployment limitations, data quality variations challenge model robustness, and workflow integration determines whether technical success translates to operational value.
...
Different data access patterns demand different storage solutions. Teams typically implement **tiered storage architectures**:
...
### Systems Artifacts: Beyond Model Weights

A common failure mode is treating the trained model weights as the sole output of this stage. In a mature ML systems workflow, the deliverable is a **reproducible system artifact**. This includes:
...
Four systems thinking patterns emerge from our analysis. Constraint propagation shows how early decisions shape later stages. Multi-scale feedback reveals how systems adapt across different timescales. Emergent complexity explains how system-wide behaviors differ from component behaviors. Resource optimization demonstrates how trade-offs create interdependencies. These patterns provide the analytical framework for understanding how the technical chapters ahead interconnect, showing why specialized approaches to data engineering, frameworks, training, and operations collectively enable integrated systems that individual optimizations cannot achieve.
