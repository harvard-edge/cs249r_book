...
- **When data won’t move**: Start with @tbl-data-gravity and the transfer-time equation in @sec-system-foundations-data-engineering-foundations-9ba2.
- **When the GPU is starving**: Use @tbl-serialization-cost and the layout discussion in @sec-system-foundations-row-vs-columnar-formats-1f97.
- **When pipelines explode in cost**: Use the primitives in @sec-system-foundations-algebra-data-7b3d, especially join-induced shuffles.
- **When “average looks fine” but users complain**: Use @sec-system-foundations-distributions-long-tail-37cc and session-level tail probability.
- **When accuracy drifts silently**: Use @sec-system-foundations-measuring-drift-divergence-0525 to compare full distributions.
...
# --- Inputs (Data Gravity) ---
tb_value = (1 * TB).to(byte).magnitude
pb_value = (1 * PB).to(byte).magnitude

bw_1g_value = (1 * Gbps).to('bit/second').magnitude
bw_10g_value = (10 * Gbps).to('bit/second').magnitude
bw_100g_value = (100 * Gbps).to('bit/second').magnitude
...
The transfer time for moving a dataset is simply $T = \text{Data Volume} / \text{Bandwidth}$ (for the large volumes here, latency is negligible; the full equation appears in @sec-system-foundations-bandwidth-vs-latency-e155). @tbl-data-gravity illustrates the sobering reality:
...
[^fn-serialization]: **Serialization**: From Latin *serialis* (forming a series). The process of converting in-memory data structures into a byte stream for storage or transmission, and the reverse (*deserialization*). In ML pipelines, the choice of serialization format can dominate end-to-end training time; @sec-data-engineering-ml covers pipeline design strategies that minimize this overhead.
...
: **Serialization Overhead.** Zero-copy formats like Arrow are 10–100× faster than row-based text formats because they align directly with internal memory structures. {#tbl-serialization-cost}
...
## Probability and Statistics {#sec-system-foundations-probability-statistics-systems-a2cb}
...
In the "Information Roofline" model (@sec-appendix-dam), we treat data quality as a physical constraint similar to bandwidth. Information theory provides the units for this constraint. Three concepts are central:
