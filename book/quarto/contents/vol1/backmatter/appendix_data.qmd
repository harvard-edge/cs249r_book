...
::: {#fig-row-vs-col fig-env="figure" fig-pos="htb" fig-cap="**Storage Layouts**: Row-oriented formats pack data together by record (good for transactions). Column-oriented formats pack data by feature (good for analytics)." fig-alt="Diagram contrasting a row store with a column store. The row store lays out the ID, Age, and Loc fields of record 1 side by side, then the same three fields of record 2, and a scan reads all fields. The column store lays out the same six values as three field rows, ID, Age, and Loc, each holding both records, with the Age row highlighted because a scan reads only that field."}
```{.tikz}
...
1.  **Selection ($\sigma$)**: Filtering rows (for example, `WHERE age > 30`).
    *   **Cost**: $\mathcal{O}(\log N + K)$ if an index lookup returns $K$ rows; $\mathcal{O}(N)$ for a full scan.
...
3.  **Join ($\bowtie$)**: Combining tables. The most expensive operation.
    ...
    *   **Broadcast Join**: One small table is sent to all workers.
        *   **Cost**: $S_{\text{tbl}}(W-1)$ bytes; the $S_{\text{tbl}}$-byte table must fit on each of $W$ workers.
...
KL divergence[^fn-kl] measures the expected extra coding cost of approximating $P_t$ with $P_0$ [@kullback1951]:
