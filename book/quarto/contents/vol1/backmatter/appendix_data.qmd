...
This foundations appendix is a reference for quantitative reasoning about the data axis of ML systems. It collects reusable calculations and compact statistical tools for tracing data movement, representation, pipeline cost, distribution tails, and drift, so readers can connect observed symptoms to measurable constraints before choosing an intervention. The main chapters can then treat data as a systems constraint without pausing to rederive every transfer, representation, or distribution calculation.
...
Data gravity[^fn-data-gravity] is a metaphor grounded in transfer-time calculations. Unlike compute, which gets faster every year, the speed of light is fixed and network bandwidth is a finite resource. When datasets grow large enough, their practical inertia grows—moving them can cost more time and energy than moving computation to where the data already lives.
...
The ideal line-rate transfer time is $T = D_{\text{vol}} / \text{BW}$ (for the large volumes here, latency is negligible; the full equation appears in @sec-appdx-machine-foundations-bandwidth-vs-latency-5320). @Tbl-data-gravity gives lower bounds; protocol overhead and endpoint limits increase them:
...
The choice of file format determines the "physics" of how the data is read. Row-oriented formats such as CSV and JSON store data record-by-record, so reading the `age` field still requires parsing each row. This suits appended logs but not analytics on selected features. Parquet stores compressed column chunks on disk, while Arrow provides a columnar in-memory and IPC format. Parquet readers can use *projection pushdown*\index{Projection pushdown} to fetch selected column chunks, and both formats support *vectorized processing*\index{Vectorized processing!columnar data}. Compare the two arrangements in @fig-row-vs-col to see why columnar access avoids scanning unnecessary bytes.

::: {#fig-row-vs-col fig-env="figure" fig-pos="htb" fig-cap="**Storage Layouts**: Row-oriented formats pack data together by record (good for transactions). Column-oriented formats pack data by feature (good for analytics)." fig-alt="Diagram contrasting row and column stores. The row store groups ID, Age, and Loc values by record and reads every column. The column store groups values by column and reads only Age."}
```{.tikz}
...
For $N_{\text{session}} = 100$ independent requests, each with a 0.01 chance of being slow, the probability of at least one P99-tail event is `{python} TailLatency.p_slow_eq`.

Computed: $\Pr(\text{Slow})$ = `{python} TailLatency.p_slow_pct_str`.
Under this model, a 100-request session is more likely than not to include an event beyond P99. Correlated slowdowns can change this probability [@dean2013; @decandia2007dynamo].
...
@Eq-appdx-data-kl-divergence expresses KL divergence[^fn-kl] as the expected extra coding cost of approximating $P_t$ with $P_0$ [@kullback1951; @cover2006].
...
Shifted logits: $z - a =$ `{python} LogSumExp.shifted_math`

Exponentials: `{python} LogSumExp.stable_exp0_eq`, `{python} LogSumExp.stable_exp1_eq`, `{python} LogSumExp.stable_exp2_eq`
...
2. Your model's average latency is 50 ms, but P99 is 800 ms. Assuming independent requests, what is the probability that a fifty-request session includes a request beyond P99? Does average latency describe user experience under this model?
