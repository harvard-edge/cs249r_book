...
# │ Exports: csv_speed_mb_per_s_str, parquet_speed_mb_per_s_str, csv_cycles_str,
# │          parquet_cycles_str, proto_speed_mb_per_s_str, proto_cycles_str
# └─────────────────────────────────────────────────────────────────────────────
...
class SerializationCost:
    ...
    parquet_speed_mb_per_s_str = fmt_qty(parquet_speed, MB/second, precision=0, commas=True, lower_bound=True)
...
|:------------------|:-------------------------------------------------------:|:-----------------------------------------------:|:--------------------|
| **CSV/JSON**      |   `{python} SerializationCost.csv_speed_mb_per_s_str`   |   `{python} SerializationCost.csv_cycles_str`   | Debugging only      |
| **Protobuf**      |  `{python} SerializationCost.proto_speed_mb_per_s_str`  |  `{python} SerializationCost.proto_cycles_str`  | RPC/Messages        |
| **Parquet/Arrow** | `{python} SerializationCost.parquet_speed_mb_per_s_str` | `{python} SerializationCost.parquet_cycles_str` | High-Scale Training |
...
2.  **Projection ($\pi$)**: Selecting columns (for example, `SELECT age`).
    *   *Cost*: Free in columnar formats (only read relevant blocks). In row formats, the entire `{python} DataAlgebra.row_size_kb_str` row must be read just to extract the `{python} DataAlgebra.int_size_bytes_value_str`-byte integer, wasting `{python} DataAlgebra.waste_pct_str` of I/O bandwidth.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_count, check, MarkdownStr
