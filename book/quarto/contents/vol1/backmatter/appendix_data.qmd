...
**Column-Oriented (Parquet, Arrow)**: Data is stored column-by-column. To read the `age` column, the disk head seeks to that column's block and reads it sequentially. This enables **projection pushdown**\index{Projection pushdown} (reading only the bytes required) and **vectorized processing**\index{Vectorized processing!columnar data} (single instruction, multiple data (SIMD) operations on columns). Compare the two arrangements side by side in @fig-row-vs-col to see why columnar access avoids scanning unnecessary bytes.
...
# │ Exports: p_slow_eq, p_slow_pct_str, p_slow_pct_math_str
# └─────────────────────────────────────────────────────────────────────────────
...
class TailLatency:
    ...
    p_slow_pct_math_str = fmt(p_slow * 100, precision=1, commas=False)
    p_slow_eq = fmt_math(
        f"\\Pr(\\text{{Slow}}) = 1 - ({p_fast})^{{{n_requests}}} "
        f"\\approx 1 - {p_all_fast_str} = {p_slow_pct_math_str}\\%"
    )
```
...
**Information Density**\index{Information Density!definition} is the amount of useful signal per unit of storage. High-quality data has high information density; noisy data has low density.

**Signal-to-Noise Ratio (SNR)**\index{Signal-to-Noise Ratio!definition} is the ratio of useful information to irrelevant variance. In ML, training on low-SNR data is like trying to learn a pattern from static---the system hits the information roofline where adding more compute yields no improvement.
