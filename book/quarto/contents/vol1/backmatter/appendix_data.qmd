...
class SerializationCost:
    ...
    csv_speed_str = fmt_qty(csv_speed, MB/second, precision=0, commas=False, approx=True)
    parquet_speed_str = fmt_qty(parquet_speed, MB/second, precision=0, commas=True, lower_bound=True)
    csv_cycles_str = MarkdownStr("High")
    parquet_cycles_str = MarkdownStr("Low")
    proto_speed_str = fmt_qty(proto_speed, MB/second, precision=0, commas=False, approx=True)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import KB, TB, byte, BYTES_INT32
...
If a user session involves $N_{\text{req}} = 100$ requests (common for a web page load or chat session), the probability of experiencing *at least one* P99 latency spike is `{python} TailLatency.p_slow_eq`.
