...
# Data Gravity: Transfer time = Volume / Bandwidth
...
BW_1G = 1e9      # 1 Gbps
BW_10G = 10e9    # 10 Gbps
BW_100G = 100e9  # 100 Gbps

# Compute transfer times
t_1tb_1g = format_time(transfer_time(TB, BW_1G))           # ~2.2 Hours
t_1tb_10g = format_time(transfer_time(TB, BW_10G))         # ~13 Minutes
t_1tb_100g = format_time(transfer_time(TB, BW_100G))       # ~1.3 Minutes
t_100tb_1g = format_time(transfer_time(100*TB, BW_1G))     # ~9 Days
t_100tb_10g = format_time(transfer_time(100*TB, BW_10G))   # ~22 Hours
t_100tb_100g = format_time(transfer_time(100*TB, BW_100G)) # ~2.2 Hours
t_1pb_1g = format_time(transfer_time(PB, BW_1G))           # ~3 Months
t_1pb_10g = format_time(transfer_time(PB, BW_10G))         # ~9 Days
t_1pb_100g = format_time(transfer_time(PB, BW_100G))       # ~22 Hours
```
...
**Transfer Time Equation**: $T = \text{Data Volume} / \text{Bandwidth}$
...
::: {.callout-note title="Systems Insight: The Accelerator Starvation Problem"}

The choice of file format determines whether your system is **I/O Bound** or **Compute Bound**. In row-oriented formats (CSV/JSON), the CPU must parse text strings into floating-point numbers—a task that is often 10–100x slower than the accelerator's ability to process those numbers. Using columnar formats like Parquet or Arrow allows for **zero-copy reads** and **projection pushdown**, ensuring that the "Data Movement" term in the Iron Law does not become a bottleneck that leaves your expensive accelerators idling.
...
*   **Entropy ($H$)**: The average information content (uncertainty) in a distribution. A uniform distribution has maximum entropy (maximum uncertainty).
...
**The Reality**: Engineers often optimize their accelerator kernels but leave data loading as JSON or uncompressed CSV. Parsing text formats is extremely CPU-intensive. Your accelerator will idle while your CPU struggles to parse strings into floats.
:::
