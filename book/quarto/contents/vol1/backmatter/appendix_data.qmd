...
| **Data Volume** | **1 Gbps  \mbox{(Standard WAN)}**       | **10 Gbps \mbox{(High-End WAN)}**        | **100 Gbps \mbox{(Direct Connect)}**    | **Truck \mbox{(Illustrative)}**          |
|:----------------|:---------------------------------------:|:----------------------------------------:|:---------------------------------------:|:----------------------------------------:|
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \tikzset{
    ...
    minimum height=8mm, minimum width=24mm, align=center, font=\small\usefont{T1}{phv}{m}{n}},
...
If a user session involves $N_{\text{session}} = 100$ requests (common for a web page load or chat session), the probability of experiencing *at least one* P99 latency spike is `{python} TailLatency.p_slow_eq`.

Computed: $\Pr(\text{Slow})$ = `{python} TailLatency.p_slow_pct_str`.
For a heavy user ($N_{\text{session}}$ is large), the "tail" latency becomes their *median* experience. This is why large production systems optimize for high-percentile latency such as P99.9 or P99.99 [@dean2013; @decandia2007dynamo].
...
Since PSI = `{python} KLDrift.psi_str` < `{python} PsiThreshold.psi_threshold_str`, this drift does not yet cross the manual-review threshold. The right response is to keep the feature on the monitoring dashboard and look for corroborating drift signals rather than retraining from this statistic alone.

:::

Because KL divergence is asymmetric ($\mathcal{D}_{\text{KL}}(P_0 \lVert P_t) \neq \mathcal{D}_{\text{KL}}(P_t \lVert P_0)$), practitioners often use PSI, a symmetric metric derived from KL divergence that is easier to threshold. A PSI above `{python} PsiThreshold.psi_threshold_str` typically triggers manual review; retraining depends on the severity of the shift and whether model-quality metrics move with it.
...
::: {#exmp-appendix-data-log-sum-exp-action .callout-example title="Log-sum-exp in action"}

**Setup**: A three-class classifier outputs logits $z =$ `{python} LogSumExp.z_math`.
