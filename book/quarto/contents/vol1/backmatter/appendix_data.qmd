...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import TB, PB, Gbps, byte, BITS_PER_BYTE, SECONDS_PER_MINUTE, SEC_PER_HOUR, SEC_PER_DAY, DAYS_PER_MONTH
...
class TailLatency:
    ...
    p_slow_eq = fmt_math(f"\\Pr(\\text{{Slow}}) = 1 - ({p_fast})^{{{n_requests}}} " f"\\approx 1 - {p_all_fast_str} = {p_slow_pct_value_str}\\%")
```
