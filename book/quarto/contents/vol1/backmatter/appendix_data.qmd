...
t_1tb_1g    = DataGravity.t_1tb_1g_str
t_1tb_10g   = DataGravity.t_1tb_10g_str
t_1tb_100g  = DataGravity.t_1tb_100g_str
t_100tb_1g  = DataGravity.t_100tb_1g_str
t_100tb_10g = DataGravity.t_100tb_10g_str
t_100tb_100g= DataGravity.t_100tb_100g_str
t_1pb_1g    = DataGravity.t_1pb_1g_str
t_1pb_10g   = DataGravity.t_1pb_10g_str
t_1pb_100g  = DataGravity.t_1pb_100g_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────
csv_speed_str = SerializationCost.csv_speed_str
parquet_speed_str = SerializationCost.parquet_speed_str
csv_cycles_str = SerializationCost.csv_cycles_str
parquet_cycles_str = SerializationCost.parquet_cycles_str
proto_speed_str = SerializationCost.proto_speed_str
proto_cycles_str = SerializationCost.proto_cycles_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────
row_size_kb = DataAlgebra.row_size_kb_str
int_size_bytes = DataAlgebra.int_size_bytes_str
waste_pct_str = DataAlgebra.waste_pct_str
join_table_tb_str = DataAlgebra.join_table_tb_str
join_network_tb_str = DataAlgebra.join_network_tb_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────
n_users_str = LongTailScale.n_users_str
n_unhappy_str = LongTailScale.n_unhappy_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────
p_slow_eq = TailLatency.p_slow_eq
p_slow_pct_str = TailLatency.p_slow_pct_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────
psi_threshold = DriftThresholds.psi_threshold_str
logit_val = DriftThresholds.logit_val_str
```
...
class KLDrift:
    ...
    P_str = [_f2(p) for p in P]
    Q_str = [_f2(q) for q in Q]
    lr_str = [_f3(l) for l in lr]

    kl_pq_t_str = [_f3(t) for t in kl_pq_t]
    kl_pq_str = _f3(kl_pq)

    kl_qp_t_str = [_f3(t) for t in kl_qp_t]
    kl_qp_str = _f3(kl_qp)

    diff_str = [_f2(d) for d in diffs]
    psi_t_str = [_f3(t) for t in psi_t]
    psi_str = _f3(psi)
    ...
    kl_qp_calc = md_math(
        f"= {Q_str[0]} \\times ({_f3(-lr[0])}) "
        f"+ {Q_str[1]} \\times {lr_str[0]} "
        f"+ {Q_str[2]} \\times {_f3(abs(lr[2]))} "
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────
kl_scenario_str = KLDrift.scenario_str
kl_P_vec_str = KLDrift.P_vec_str
kl_Q_vec_str = KLDrift.Q_vec_str
kl_pq_formula = KLDrift.kl_pq_formula
kl_pq_calc = KLDrift.kl_pq_calc
kl_qp_formula = KLDrift.kl_qp_formula
kl_qp_calc = KLDrift.kl_qp_calc
kl_psi_eq = KLDrift.psi_eq
kl_psi_str = KLDrift.psi_str
```

::: {.callout-notebook title="Worked Example: KL Divergence for Drift Detection"}

**Scenario**: A sentiment classifier was trained on data where `{python} kl_scenario_str`.

Training distribution $P$: `{python} kl_P_vec_str`. Serving distribution $Q$: `{python} kl_Q_vec_str`.

`{python} kl_pq_formula`

`{python} kl_pq_calc` nats

`{python} kl_qp_formula`

`{python} kl_qp_calc` nats

Notice the asymmetry: $D_{KL}(P||Q) \neq D_{KL}(Q||P)$. The Population Stability Index (PSI) symmetrizes this:

`{python} kl_psi_eq`

Since PSI = `{python} kl_psi_str` < `{python} psi_threshold`, this drift is noticeable but does not yet trigger a retraining alert. However, monitoring should increase in frequency.
...
class LogSumExp:
    ...
    # Stable version: shift by max
    a = max(z)
    shifted = [zi - a for zi in z]
    stable_exp = [math.exp(s) for s in shifted]
    stable_sum = sum(stable_exp)
    lse = a + math.log(stable_sum)

    # Softmax from stable exponentials
    softmax = [e / stable_sum for e in stable_exp]
    ...
    # Shifted logits
    shifted_str = f"[{shifted[0]}, {shifted[1]}, {shifted[2]}]"
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────
lse_z_str = LogSumExp.z_str
lse_z0 = LogSumExp.z[0]
lse_z1 = LogSumExp.z[1]
lse_z2 = LogSumExp.z[2]
lse_naive0_str = LogSumExp.naive0_str
lse_naive1_str = LogSumExp.naive1_str
lse_naive2_str = LogSumExp.naive2_str
lse_a_str = LogSumExp.a_str
lse_shifted_str = LogSumExp.shifted_str
lse_shifted0 = LogSumExp.shifted[0]
lse_shifted1 = LogSumExp.shifted[1]
lse_shifted2 = LogSumExp.shifted[2]
lse_sexp0_str = LogSumExp.sexp0_str
lse_sexp1_str = LogSumExp.sexp1_str
lse_sexp2_str = LogSumExp.sexp2_str
lse_sum_str = LogSumExp.sum_str
lse_result_str = LogSumExp.lse_str
lse_sm0_str = LogSumExp.sm0_str
lse_sm1_str = LogSumExp.sm1_str
lse_sm2_str = LogSumExp.sm2_str
```

::: {.callout-notebook title="Worked Example: Log-Sum-Exp in Action"}

**The setup**: A 3-class classifier outputs logits $z = `{python} lse_z_str`$.

**Without the trick (naive softmax)**:

$e^{`{python} lse_z0`} \approx `{python} lse_naive0_str`$, $e^{`{python} lse_z1`} \approx `{python} lse_naive1_str`$, $e^{`{python} lse_z2`} \approx `{python} lse_naive2_str`$
...
**With the trick**: Subtract $a = \max(z) = `{python} lse_a_str`$:

$z - a = `{python} lse_shifted_str`$

$e^{`{python} lse_shifted0`} \approx `{python} lse_sexp0_str`$, $e^{`{python} lse_shifted1`} \approx `{python} lse_sexp1_str`$, $e^{`{python} lse_shifted2`} = `{python} lse_sexp2_str`$

Sum = `{python} lse_sum_str`. LogSumExp = $`{python} lse_a_str` + \log(`{python} lse_sum_str`) = `{python} lse_result_str`$.

Softmax: $[`{python} lse_sexp0_str`/`{python} lse_sum_str`,\; `{python} lse_sexp1_str`/`{python} lse_sum_str`,\; `{python} lse_sexp2_str`/`{python} lse_sum_str`] = [`{python} lse_sm0_str`,\; `{python} lse_sm1_str`,\; `{python} lse_sm2_str`]$
