...
+:==========================+:=========================================================+:===================================================================+
| **Performance Models**    | Determine if a workload is memory-bound or compute-bound | @sec-ai-acceleration, @sec-ai-training, @sec-model-serving-systems |
| **Memory Hierarchy**      | Optimize data movement and cache usage                   | @sec-ai-acceleration, @sec-model-compression                       |
| **Data Engineering**      | Choose storage formats and design reliable pipelines     | @sec-data-engineering-ml                                           |
| **System Statistics**     | Measure data drift and monitor tail latencies            | @sec-data-engineering-ml, @sec-machine-learning-operations-mlops   |
...
| **Computational Graphs**  | Understand compiler and hardware optimizations           | @sec-ai-frameworks, @sec-ai-acceleration                           |
+---------------------------+----------------------------------------------------------+--------------------------------------------------------------------+
...
Parallelization is the primary tool for scaling ML, but its limits depend on *how* you scale.
...
*   With $n=8$, speedup is $\frac{1}{0.05 + 0.95/8} \approx 5.9\times$.
...
*   With $n=1$, speedup is 1.
*   With $n=8$, Scaled Speedup is $8 - 0.05(7) = 8 - 0.35 = 7.65\times$.
*   With $n=1000$, Scaled Speedup is $1000 - 0.05(999) \approx 950\times$.
...
**Example**: To sustain 1,000 queries per second (QPS) with 50ms average latency, your system must support $1000 \times 0.05 = 50$ concurrent requests.
...
We use **Einstein summation** notation throughout this book because it makes complex operations explicit.^[Implemented as `torch.einsum` in PyTorch and `np.einsum` in NumPy. It provides a unified DSL for dot products, outer products, transposes, and matrix multiplications.] Matrix multiplication $C = AB$ becomes:
...
**Example**: A vocabulary embedding matrix of size $100,000 \times 10,000$ (1 billion parameters).
*   **Dense (FP32)**: $10^9 \times 4$ bytes = **4 GB**.
*   **Sparse (1% density)**: Storing only non-zeros requires roughly $10^7 \times (4 \text{ bytes value} + 4 \text{ bytes index}) \approx$ **80 MB**.
*   *Result*: A 50x reduction in memory footprint, fitting a model that would otherwise OOM (Out of Memory).
...
**Memory Implication**: Cached activations dominate memory usage. Storing activations for a 100-layer network consumes huge memory, motivating **Gradient Checkpointing** [@chen2016training] (recomputing activations to save memory).^[Gradient checkpointing trades compute for memory. Instead of storing all activations, you store only a subset (checkpoints) and recompute the missing ones during the backward pass. This reduces memory usage from $O(N)$ to $O(\sqrt{N})$ at the cost of $33\%$ more compute.]
