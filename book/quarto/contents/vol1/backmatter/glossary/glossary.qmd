...
**AlexNet**
: A groundbreaking convolutional neural network architecture that won the 2012 ImageNet challenge, reducing error rates from 26 percent to sixteen percent and sparking the deep learning renaissance.
...
: The fundamental computational operation in neural networks that dominates training time, accounting for 60–90 percent of computation in typical models.
...
: A quantitative framework decomposing ML system performance into three terms: Data (limited by bandwidth), Compute (limited by FLOPS), and Latency (limited by overhead). Formulated as $T = D_{\text{vol}}/\text{BW} + O/(R_{\text{peak}} \cdot \eta) + L_{\text{lat}}$.
...
**l0-norm constraint**
: A regularization technique that counts the number of nonzero parameters in a model, used in structured pruning to directly control model sparsity by penalizing the number of active weights.
...
: The disparity between technical optimization success (for example, high benchmark accuracy) and responsible deployment outcomes (for example, fairness and safety in production).
...
**transfer learning**
: A machine learning technique that leverages knowledge gained from pretrained models on related tasks, allowing faster training and better performance on new tasks with limited data by reusing learned features and representations
...
**warp**
: A group of threads (typically thirty-two on NVIDIA GPUs) that execute the same instruction in lock-step; the fundamental unit of scheduling and execution on GPUs.
