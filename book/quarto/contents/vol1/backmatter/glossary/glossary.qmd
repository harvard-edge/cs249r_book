...
**data term**
: The component of the Iron Law representing the physical cost of moving bits ($D_{vol}/BW$), often limited by memory or network bandwidth.
  *Appears in: @sec-introduction-to-ml-systems*
...
**expert systems**
: An AI paradigm that relies on hard-coded rules and knowledge bases curated by human experts, limited by the knowledge acquisition bottleneck.
  *Appears in: @sec-introduction-to-ml-systems*
...
: A storage performance metric measuring the number of read/write operations a device can handle per second, critical for random access workloads like training data loading.
  *Appears in: @sec-data-engineering-ml*
...
: A quantitative framework decomposing total task time into Data, Compute, and Overhead terms: $T \approx \frac{D_{vol}}{BW} + \frac{O}{R_{peak} \cdot \eta} + L_{lat}$.
...
: The concept that lifecycle stages are engineering levers used to optimize the variables in the Iron Law of ML Systems ($T = \frac{D_{vol}}{BW} + \frac{O}{R_{peak} \cdot \eta} + L_{lat}$).
  *Appears in: @sec-ai-development-workflow*
...
**pipeline jungle**
: A complex, unmanageable web of data processing scripts and intermediate files that evolves when data pipelines are not engineered with the same rigor as production code.
  *Appears in: @sec-data-engineering-ml*
...
: A model parallelism strategy where the layers of a model are partitioned across multiple devices (stages), and micro-batches are pipelined through them to improve utilization.
  *Appears in: @sec-ai-training*
...
: An AI paradigm that uses probability and statistics to learn patterns from data, traditionally relying on manual feature engineering.
  *Appears in: @sec-introduction-to-ml-systems*
