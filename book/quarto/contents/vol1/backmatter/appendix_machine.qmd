...
**Example**: Training a **1B parameter** model on **20B tokens** using **1 A100** (312 TFLOPS) at **40% utilization**.
...
Fetching a 32-bit value from DRAM costs roughly **1000$\times$ more energy** than performing a floating-point operation on it (e.g., ~640 pJ vs ~1 pJ). This "Energy Wall" means that maximizing **arithmetic intensity** (doing many ops per loaded byte) is the only way to be energy efficient.
::::
