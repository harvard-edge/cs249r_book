...
#| label: amdahl-setup
#| echo: false
# Input constants for Amdahl's Law prose references
s_value = 0.05
p_value = 1 - s_value
n_8_value = 8
s_pct_str = "5"
...
#| echo: false
# Input constants for Gustafson's Law prose references
s_g_pct_str = "5"
s_g_str = "0.05"
```

**Example**: Using the same `{python} s_g_pct_str`% serial overhead ($s=0.05$):
...
**Implication for Memory**: This tells you exactly how to size your inference worker pools. If serving one request requires 1 GB of temporary memory (KV cache, activations), handling `{python} L_concurrent_str` concurrent requests requires `{python} L_concurrent_str` GB of memory. If your accelerator only has 24 GB, you are physically limited to 24 concurrent requests. Your maximum throughput is capped at $L/W = 24 / 0.05 = 480$ QPS, regardless of how many requests arrive.
...
While latency tells us how long we wait for the *first* byte, bandwidth tells us how many bytes follow. Use these constants for back-of-the-envelope "Roofline" calculations. These represent the "standard units of compute" for the current era of machine learning.
...
#| echo: false
# Input constants for bandwidth-latency prose references
bw_gbps_str = "10"
ping_ms_str = "10"
data_kb_str = "1"
```
