...
This appendix is designed as a reference. When you are stuck, use it to turn a vague symptom ("it’s slow") into a specific constraint ("memory bound at batch size one") and then choose the lever that can actually move.
...
To see Amdahl's Law in action, suppose `{python} AmdahlSetup.s_pct_str` percent of your training step is serial overhead (e.g., Python GIL, kernel launch latency) and `{python} AmdahlSetup.p_pct_str` percent is parallelizable matrix math:
...
::: {.callout-perspective title="Why This Matters"}
Your production model runs at fifty QPS in FP32 but your target is 200 QPS. Switching to INT8 could get you there, but will accuracy suffer? Understanding numerical formats lets you make this trade-off quantitatively rather than hoping for the best.
:::
