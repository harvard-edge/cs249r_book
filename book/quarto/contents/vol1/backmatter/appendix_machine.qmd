...
# │ How: Scalar extraction via .to(unit).magnitude; ratios remain stable across
...
class NumbersToKnow:
    ...
    register_latency_ns = LATENCY_REGISTER_REF.to(nanosecond).magnitude
    hbm_vs_register = int(LATENCY_HBM3.to(nanosecond).magnitude / register_latency_ns)
    ssd_vs_register = int(round((LATENCY_NVME_SSD.to(nanosecond).magnitude / register_latency_ns) / 100_000) * 100_000)
    network_vs_local = int(LATENCY_INFINIBAND.to(nanosecond).magnitude / LATENCY_HBM3.to(nanosecond).magnitude)
    ...
    lat_l1_ns = int(LATENCY_L1_REGISTER.to(nanosecond).magnitude)
    lat_l2_ns = int(LATENCY_L2_CACHE.to(nanosecond).magnitude)
    lat_hbm_ns = int(LATENCY_HBM3.to(nanosecond).magnitude)
    lat_pcie_ns = int(LATENCY_PCIE_GEN5.to(nanosecond).magnitude)
    lat_ib_ns = int(LATENCY_INFINIBAND.to(nanosecond).magnitude)
    lat_ssd_ns = int(LATENCY_NVME_SSD.to(nanosecond).magnitude)
...
class AmdahlSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    s_pct_str = fmt_percent(s_pct_value/100, precision=0, commas=False, style='prose')
    p_pct_str = fmt_percent(p_pct_value/100, precision=0, commas=False, style='prose')
...
class GustafsonSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    s_g_pct_str = fmt_percent(s_g_pct_value/100, precision=0, commas=False, style='prose')
...
class GustafsonExample:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(gustafson_8_value > 1, "8-way Gustafson speedup should exceed serial speedup.")
    check(gustafson_1000_value > gustafson_8_value, "Larger n should increase scaled speedup.")
...
class TrainingTimeRef:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    T_seconds_str = fmt_time(t_seconds_value, 'second', precision=1, commas=True, style='word')
    T_minutes_str = fmt_time(t_minutes_value, 'minute', precision=1, commas=False, style='word')
    ...
    p_params_str = fmt_count(p_params, scale='B', precision=0, commas=False)
    d_tokens_str = fmt_count(d_tokens, scale='B', precision=0, commas=False)
...
class LittlesLawExample:
    ...
    w_latency_ms_str = fmt_time(round(w_latency_s_value * THOUSAND), 'millisecond', precision=0, commas=False)
...
class ArchReferenceTables:
    ...
    h100_bw_str = fmt_qty(h100_bw, TB/second, precision=2, commas=False, unit_label="TB")
    h100_cap_str = fmt_qty(h100_cap, GB, precision=0, commas=False)
    h100_nvlink_str = fmt_qty(h100_nvlink, GB/second, precision=0, commas=False, unit_label="GB")
    ...
    tpuv5_bw_str = fmt_qty(tpuv5_bw, TB/second, precision=2, commas=False, unit_label="TB")
...
class BandwidthLatencySetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    bw_gbps_str = fmt_qty(Systems.Fabrics.Ethernet_10G.bandwidth, Gbps, precision=0, commas=False, unit_label="Gb/s")
    ping_ms_str = fmt_time(ping_ms_value, 'millisecond', precision=0, commas=False)
