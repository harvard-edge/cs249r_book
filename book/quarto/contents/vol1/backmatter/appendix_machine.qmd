...
- **Diagnose the dominant ceiling**: Use the Roofline Model in @sec-machine-foundations-roofline-model-2529 to decide whether the workload is compute bound or memory bound.
- **Reason about scaling limits**: Use Amdahl's and Gustafson's Laws in @sec-machine-foundations-amdahls-law-gustafsons-law-b741 to understand why adding accelerators may not reduce time-to-train.
...
# │          lat_ssd_ns, bw_hbm_h100, bw_pcie5, bw_dram, bw_ib, bw_nvme,
...
class NumbersToKnow:
    ...
    bw_ib = int(INFINIBAND_NDR_BW.m_as(GB/second))
...
The following three numbers are critical to memorize:

1. **~600$\times$ energy ratio**: DRAM access costs ~`{python} NumbersToKnow.dram_vs_compute`$\times$ more energy than an FP16 FLOP. This is why arithmetic intensity is everything.

2. **16 bytes/parameter for training**: Model weights (2B FP16) + gradients (2B FP16) + master weights (4B FP32) + optimizer states for Adaptive Moment Estimation (Adam) at 8B. That totals 16 bytes per parameter, so a 7B model needs 112 GB just to start training.
...
| FP32 vs. FP16 energy         |   ~`{python} NumbersToKnow.fp32_vs_fp16`$\times$  | Narrower arithmetic reduces switching and datapath energy |
...
[^fn-horowitz-energy-app]: **Energy hierarchy source**: Energy numbers from Horowitz's classic "Computing's Energy Problem" (ISSCC 2014, 45nm process) [@horowitz2014]. While absolute values scale with process node, the *ratios* between memory access and compute remain remarkably stable because wire capacitance (distance) dominates.
...
|:-----------------------------------------------|:---------------------------------------------------------:|:----------------------------------|
| Accelerator memory (HBM) vs. register          |     ~`{python} NumbersToKnow.hbm_vs_l1`$\times$ slower    | On-chip vs. off-chip              |
| SSD vs. register                               |     ~`{python} NumbersToKnow.ssd_vs_l1`$\times$ slower    | Electrical vs. mechanical/flash   |
...
: **The Latency Hierarchy**: Each level costs roughly 10--100$\times$ more than the one above it. {#tbl-memory-ratios-ref tbl-colwidths="[47,23,30]"}
...
| Data center vs. edge compute  | ~`{python} NumbersToKnow.dc_mobile_ratio`$\times$ |   Compute per watt$\times$ power budget    |

: **Scaling Rules**: These are arithmetic, not hardware-specific. Training memory includes FP16 weights (2B), FP16 gradients (2B), FP32 master weights (4B), and Adam optimizer states (8B for momentum + variance). {#tbl-scaling-rules-ref tbl-colwidths="[33,33,33]"}
...
| InfiniBand (network) |  ~`{python} NumbersToKnow.lat_ib_ns` ns  |    `{python} NumbersToKnow.bw_ib` GB/s    |
...
|:-----------------------|:-----------------------------------------------:|:-------------------------------------------------------:|:-----:|
| Data center GPU (H100) | `{python} NumbersToKnow.flops_h100_fp16` TFLOPS | `{python} NumbersToKnow.flops_h100_fp8` TOPS (FP8 peak) | 700 W |
...
: **Compute Reference (c. 2024)**: Using the current FP16 and mobile INT8 constants, data-center peak throughput is about `{python} NumbersToKnow.dc_mobile_ratio`$\times$ the mobile reference; exact ratios depend on precision mode and workload. {#tbl-compute-current-ref tbl-colwidths="[33,20,32,15]"}
...
: **Arithmetic Intensity Thresholds (c. 2024)**: Most inference workloads are <10 ops/byte—firmly memory bound. {#tbl-ridge-current-ref tbl-colwidths="[33,33,34]"}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=1.0]
  ...
  \draw[Axis] (0,0) -- (6,0) node[right,text=TextBlack,font=\footnotesize\usefont{T1}{phv}{m}{n}] {Arithmetic Intensity (Ops/Byte)};
...
class AppendixMachineSetup:
    ...
    a100_bw_tb = fmt(A100_MEM_BW, "TB/s", precision=2)
...
**General matrix multiply (GEMM)**: For two square matrices of size `{python} AppendixMachineSetup.n_gemm` by `{python} AppendixMachineSetup.n_gemm`, arithmetic intensity is approximately `{python} AppendixMachineSetup.gemm_intensity` FLOP/byte. Since `{python} AppendixMachineSetup.gemm_intensity` > `{python} AppendixMachineSetup.ridge_point`, this operation is compute bound. You are using the hardware efficiently.

**ReLU (Element-wise)**: For a square tensor of size `{python} AppendixMachineSetup.n_gemm` by `{python} AppendixMachineSetup.n_gemm`, intensity is approximately `{python} AppendixMachineSetup.relu_intensity` op/byte. Since `{python} AppendixMachineSetup.relu_intensity` ≪ `{python} AppendixMachineSetup.ridge_point`, this operation is severely memory bound, achieving only about `{python} AppendixMachineSetup.relu_util_str` percent of peak TFLOP/s. The hardware is mostly waiting for data.
...
$$ T [s] = \underbrace{ \frac{D_{\text{vol}} [\text{Bytes}]}{\text{BW} [\text{Bytes/s}]} }_{\text{Seconds}} + \underbrace{ \frac{O [\text{FLOPs}]}{R_{\text{peak}} [\text{FLOPs/s}] \cdot \eta_{\text{hw}} [1]} }_{\text{Seconds}} + \underbrace{ L_{\text{lat}} [s] }_{\text{Seconds}} $$
...
The fundamental limits of scaling across multiple devices are the subject of @sec-machine-foundations-amdahls-law-gustafsons-law-b741.
...
To see Amdahl's Law in action, suppose `{python} AmdahlSetup.s_pct_str` percent of a training step is serial overhead (for example, Python global interpreter lock (GIL), kernel launch latency) and `{python} AmdahlSetup.p_pct_str` percent is parallelizable matrix math:
...
Using the same `{python} GustafsonSetup.s_g_pct_str` percent serial overhead ($s$ = `{python} GustafsonSetup.s_g_str`), Gustafson's Law tells a very different story:
...
class TrainingTimeRef:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    T_seconds_str = fmt(t_seconds_value, precision=0, commas=True)
...
*   **$U$**: Model FLOPs Utilization (MFU), typically 30 percent–50 percent.

**Example**: Training a **`{python} TrainingTimeRef.p_params_str` parameter** model on **`{python} TrainingTimeRef.d_tokens_str` tokens** using **`{python} TrainingTimeRef.n_gpus_str` A100** (`{python} AppendixMachineSetup.a100_fp16` TFLOPS) at **`{python} TrainingTimeRef.u_mfu_pct_str` percent utilization**.
...
For capacity planning in inference systems, **Little's Law** [@little1961] relates concurrency ($N_{\text{req}}$), arrival rate ($\lambda$), and latency ($T_{\text{lat}}$):[^fn-little]
...
This directly determines how to size inference worker pools. If serving one request requires `{python} LittlesLawExample.mem_per_req_gb_str` GB of temporary memory (KV cache, activations), handling `{python} LittlesLawExample.L_concurrent_str` concurrent requests requires `{python} LittlesLawExample.L_concurrent_str` GB of memory. If the accelerator only has `{python} LittlesLawExample.gpu_mem_gb_str` GB, the system is physically limited to `{python} LittlesLawExample.max_concurrent_str` concurrent requests. Maximum throughput is capped at `{python} LittlesLawExample.eq_max_throughput`, regardless of how many requests arrive.
...
| **Memory Bandwidth** |        `{python} AppendixMachineSetup.h100_bw` TB/s       |                 `{python} AppendixMachineSetup.tpuv5_bw` TB/s                 | The "Width of the Pipe" ($\text{BW}$) |
...
| **Interconnect**     | `{python} AppendixMachineSetup.h100_nvlink` GB/s (NVLink) | `{python} AppendixMachineSetup.tpuv5_ici` GB/s (Inter-Chip Interconnect, ICI) | Determines Model Parallelism Scaling  |
...
Asymmetric quantization handles distributions that are not centered (common after ReLU, which produces only nonnegative values) by shifting the range before scaling. If $x_{\min}$ is the minimum of the range and $\alpha$ is the range width ($x_{\max} - x_{\min}$):
$$ x_{\text{int}} = \text{round}\left(\frac{x - x_{\min}}{\alpha} \times 255\right) $$
...
With the full toolkit assembled---reference numbers, performance models, architectural constraints, and numerical trade-offs---the next section catalogs fallacies and pitfalls that violate the physical and architectural principles covered earlier.
...
- **Little's Law** ($N_{\text{req}} = \lambda T_{\text{lat}}$) directly sizes inference infrastructure: concurrency, memory, and maximum throughput are all linked by this simple identity.
