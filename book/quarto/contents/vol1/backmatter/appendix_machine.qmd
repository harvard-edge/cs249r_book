...
: **Speed of Light Reference**: Light in fiber travels ~200 km/ms. These latencies are physics—no optimization can reduce them. {#tbl-speed-of-light-ref}
...
: **Scaling Rules**: These are arithmetic, not hardware-specific. Training memory includes FP16 weights (2B), FP16 gradients (2B), FP32 master weights (4B), and Adam optimizer states (8B for momentum + variance). {#tbl-scaling-rules-ref}
...
: **Compute Reference (c. 2024)**: Using the current FP16 and mobile INT8 constants, data-center peak throughput is about `{python} NumbersToKnow.dc_mobile_ratio`$\times$ the mobile reference; exact ratios depend on precision mode and workload. {#tbl-compute-current-ref}
