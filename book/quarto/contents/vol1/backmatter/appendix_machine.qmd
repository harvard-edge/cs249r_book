...
- **Sanity-check feasibility**: Start with @sec-numbers-to-know for order-of-magnitude numbers.
- **Diagnose the dominant ceiling**: Use the Roofline Model in @sec-system-foundations-roofline-model-5f7c to decide whether you are compute-bound or memory-bound.
...
- **Choose the right precision**: Use @sec-system-foundations-floatingpoint-format-comparison-c861 to reason about FP32 vs. BF16/FP16 vs. INT8 as a systems trade-off.
- **Cross-reference for depth**: When you want the full narrative, jump back to @sec-ai-acceleration, @sec-ai-training, and @sec-model-serving-systems.

## Numbers to Know {#sec-numbers-to-know}
...
@tbl-energy-ratios-ref quantifies the energy cost of data movement versus computation—the fundamental reason why arithmetic intensity dominates ML performance optimization.
...
@tbl-memory-ratios-ref shows how each level of the memory hierarchy costs roughly 10–100× more latency than the one above it.
...
@tbl-scaling-rules-ref collects the arithmetic relationships that govern memory and compute requirements for training and inference.
...
[^fn-training-memory]: The 16 bytes/parameter rule assumes mixed-precision training with Adam. ZeRO optimization can reduce per-GPU memory by sharding optimizer states across GPUs, but the total memory across all GPUs remains ~16× parameters.

### Latency Budgets: The Non-Negotiables {#sec-latency-budgets-ref .unnumbered}
...
## Physics of Computing {#sec-system-foundations-physics-computing-b6a4}
...
This is the reality of Large Language Models. We don't use 1,000 GPUs to train GPT-4 on a laptop-sized dataset in milliseconds; we use them to train on a dataset 1,000× larger in reasonable time.
...
class TrainingTimeRef:
    ...
    d_tokens = 20 * BILLION
    n_gpus = 1
    x_flops = A100_FLOPS_FP16_TENSOR.to(TFLOPs / second).magnitude
    u_mfu = 0.40
    ...
    check(t_seconds > 0, "Training time must be positive.")

    # ┌── 4. OUTPUTS (Formatting) ──────────────────────────────────────────────
    T_seconds_str = fmt(t_seconds, precision=0, commas=False)
    T_minutes_str = fmt(t_minutes, precision=0, commas=False)
    T_days_str = fmt(t_days, precision=0, commas=False)
...
To see this in practice, consider sustaining `{python} lambda_qps_str` queries per second (QPS) with `{python} w_latency_ms_str` ms average latency. The law tells us the system must support `{python} lambda_qps_raw_str` × `{python} w_latency_s_str` = `{python} L_concurrent_str` concurrent requests.
...
::: {#fig-memory-hierarchy fig-env="figure" fig-pos="htb" fig-cap="**The Memory Hierarchy**: Performance depends on data proximity. Accessing HBM is ~100× slower than registers; accessing SSD is ~100,000× slower." fig-alt="Pyramid showing Registers at top, followed by Cache, HBM/DRAM, and Storage at bottom."}
```{.tikz}
...
For deep learning, FP32 often hurts performance without improving convergence. It consumes 2× memory bandwidth and energy compared to BF16. Since neural networks are resilient to noise, the extra mantissa bits in FP32 are often modeling random variance rather than signal.
...
- **Energy dominates**: Moving data costs ~600× more energy than computing on it. Arithmetic intensity—the ratio of compute to data movement—is the single most important metric for ML workload performance.
...
- **Numerical precision** is a systems lever, not just a modeling choice. BF16 matches FP32's dynamic range at half the memory cost; INT8 quantization can deliver 2–4× inference speedup with careful calibration.
...
- **Training system design**: @sec-ai-training
- **Serving system design**: @sec-model-serving-systems
- **Framework internals and kernels**: @sec-ai-frameworks
