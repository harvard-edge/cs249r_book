...
For capacity planning in inference systems, **Little's Law** [@little1961proof] relates concurrency ($N_{req}$), arrival rate ($\lambda$), and latency ($T_{lat}$):[^fn-little]
...
- **Little's Law** ($N_{req} = \lambda T_{lat}$) directly sizes inference infrastructure: concurrency, memory, and maximum throughput are all linked by this simple identity.
