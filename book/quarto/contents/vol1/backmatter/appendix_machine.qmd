...
: Light in fiber travels ~`{python} speed_of_light_km_ms` km/ms. These latencies are physics—no optimization can reduce them. {#tbl-speed-of-light-ref}
...
: **The Energy Wall.** Moving data costs ~`{python} dram_vs_compute`× more energy than computing on it. This ratio is physics, not engineering.[^fn-horowitz-energy] {#tbl-energy-ratios-ref}
...
: **Compute Reference (c. 2024).** Datacenter is ~`{python} dc_mobile_ratio`× more powerful than mobile—this ratio persists across generations. {#tbl-compute-current-ref}
