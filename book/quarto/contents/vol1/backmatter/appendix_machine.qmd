...
@tbl-memory-ratios-ref shows how each level of the memory hierarchy costs roughly 10$\times$--100$\times$ more latency than the one above it.
...
: **The Latency Hierarchy.** Each level costs roughly 10$\times$--100$\times$ more than the one above it. {#tbl-memory-ratios-ref}
...
- **Numerical precision** is a systems lever, not just a modeling choice. BF16 matches FP32's dynamic range at half the memory cost; INT8 quantization can deliver 2$\times$--4$\times$ inference speedup with careful calibration.
