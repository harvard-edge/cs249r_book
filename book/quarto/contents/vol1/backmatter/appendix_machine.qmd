...
No matter how many accelerators you buy, you cannot make this fixed workload run faster than `{python} amdahl_inf_str`x.
...
*   With $n=1$, speedup is 1.
*   With $n=8$, Scaled Speedup is $8 - 0.05(7) = 8 - 0.35$ = `{python} gustafson_8_str`$\times$.
...
The computed result: **`{python} T_seconds_str` seconds** ($\approx$ `{python} T_minutes_str` minutes, or about 11 days).
...
*   **Latency-Bound (1KB Packet)**:
    *   Transmission: 1KB / 10Gbps $\approx$ `{python} tx_time_us_str` $\mu$s.
    *   Total Time $\approx$ 10ms + `{python} tx_time_us_str`$\mu$s $\approx$ 10ms.
