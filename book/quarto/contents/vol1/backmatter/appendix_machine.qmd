...
To see Amdahl's Law in action, suppose `{python} AmdahlSetup.s_pct_str` percent of your training step is serial overhead (for example, Python GIL, kernel launch latency) and `{python} AmdahlSetup.p_pct_str` percent is parallelizable matrix math:
...
*   **Energy Efficiency**: INT8 operations are significantly more energy-efficient than floating-point equivalents because they utilize simpler integer ALUs and require less silicon area. Moving to INT8 for inference is the primary lever for deploying large language models (LLMs) on battery-constrained edge devices.
