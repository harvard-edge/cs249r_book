...
**ReLU (Element-wise)**: For a $`{python} n_gemm` \times `{python} n_gemm`$ tensor, intensity is approximately $`{python} relu_intensity`$ op/byte. Since $`{python} relu_intensity` \ll `{python} ridge_point`$, this operation is severely memory-bound, achieving only about $`{python} relu_util_str`\%$ of peak TFLOP/s. The hardware is mostly waiting for data.
