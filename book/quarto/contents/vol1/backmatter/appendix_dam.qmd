...
## Antipatterns {#sec-dam-taxonomy-antipatterns-72e1}
...
This system is **Memory-bound** (a **Data** bottleneck). At batch size one, each layer performs a matrix-vector multiply (GEMV) rather than a matrix-matrix multiply (GEMM). The model's `{python} DAMTaxonomy.ex2_params_str` parameters (~`{python} DAMTaxonomy.ex2_model_size_gb_str` GB in FP16) must be loaded from HBM for every forward pass, but each loaded weight is used for only a single input vector—yielding very low arithmetic intensity. The GPU's compute units sit idle waiting for memory transfers, which is why utilization is only `{python} DAMTaxonomy.ex2_util_str` percent.
...
##### Exercise four: *Antipattern detection* {.unnumbered}
