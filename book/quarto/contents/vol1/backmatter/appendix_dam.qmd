...
The accelerator is not the binding resource. The **Data** component cannot supply the **Machine** fast enough, so the workload is I/O bound even though the visible symptom is slow training. The useful intervention is therefore upstream of the model: optimize the extract, transform, load (ETL) path by moving from raw JPEGs with heavy CPU decoding to TFRecords or WebDataset with sequential reads, increasing data loader parallelism, and prefetching batches into accelerator memory.

### Case 2: The latency cliff (Algorithm) {#sec-appdx-dam-taxonomy-case-2-latency-cliff-algorithm-8ee2}

A real-time recommendation service fails to meet a 20 ms latency service-level agreement (SLA). Accelerator utilization is low and the batch size is one, so the signal does not point to a saturated chip. It points to an **Algorithm** that is too deep for the sequential deadline.

Adding more hardware does not remove the serial layer path that dominates latency. The effective fixes change the algorithmic work itself: quantization reduces memory fetch time by moving to INT8 where accuracy allows, pruning removes redundant heads or channels, and knowledge distillation trains a smaller student model to approximate the larger model's behavior.

### Case 3: The compute wall (Machine) {#sec-appdx-dam-taxonomy-case-3-compute-wall-machine-3a0a}

Accelerator utilization is pinned at 99 percent, memory bandwidth remains unsaturated, and training is stable but takes three weeks. In this case, the data path is doing its job: the system has kept the accelerator fed, and the workload is compute bound.
