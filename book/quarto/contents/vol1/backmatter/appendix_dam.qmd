...
A system that runs slowly rarely announces why. One symptom—an idle accelerator—can stem from a starved input pipeline (Data), an algorithm with too little arithmetic intensity (Algorithm), or a saturated memory bus (Machine). Structuring performance analysis around the D·A·M taxonomy ensures that napkin calculations isolate the true binding constraint before optimization begins. This appendix serves as the practical field guide for diagnosing single-node bottlenecks.
