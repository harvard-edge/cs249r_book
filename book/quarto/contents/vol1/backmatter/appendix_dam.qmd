...
This appendix provides a compact diagnostic framework—**Data · Algorithm · Machine (D·A·M)**\index{D·A·M Taxonomy!appendix refresher}—and shows how to map symptoms and measurements to the term of the iron law that dominates. It serves as a “first response” checklist before committing to deeper optimization.
...
The additive iron law represents **sequential execution**\index{Sequential execution!D·A·M coordination}---the worst case where Data, Algorithm, and Machine take turns. Skilled systems engineering, however, transforms the sum into a max:
...
*   **Symptom**: A team provisions a large A100 GPU instance to speed up training, but training time hardly improves. `nvidia-smi` shows GPU utilization fluctuating between 10 percent and 40 percent.
*   **Diagnosis**: The accelerator is not the binding resource. The data path cannot supply the machine fast enough, so the workload is I/O bound even though the visible symptom is slow training.
*   **Fix**: The useful intervention is upstream of the model: optimize the extract, transform, load (ETL) path by moving from raw JPEGs with heavy CPU decoding to TFRecords or WebDataset with sequential reads, increasing data loader parallelism, and prefetching batches into accelerator memory.

### Case 2: The latency cliff (Algorithm) {#sec-appdx-dam-taxonomy-case-2-latency-cliff-algorithm-8ee2}

*   **Symptom**: A real-time recommendation service fails to meet a 20 ms latency service-level agreement (SLA). Accelerator utilization is low and the batch size is one.
*   **Diagnosis**: The signal does not point to a saturated chip. It points to an algorithm that is too deep for the sequential deadline.
*   **Fix**: Adding more hardware does not remove the serial layer path that dominates latency. The effective fixes change the algorithmic work itself: quantization reduces memory fetch time by moving to INT8 where accuracy allows, pruning removes redundant heads or channels, and knowledge distillation trains a smaller student model to approximate the larger model's behavior.

### Case 3: The compute wall (Machine) {#sec-appdx-dam-taxonomy-case-3-compute-wall-machine-3a0a}

*   **Symptom**: Accelerator utilization is pinned at 99 percent, memory bandwidth remains unsaturated, and training is stable but takes three weeks.
*   **Diagnosis**: The data path is doing its job: the system has kept the accelerator fed, and the workload is compute bound.
*   **Fix**: The next step has to change the machine term in the iron law. The team can scale up to a newer accelerator generation, scale out by distributing training across multiple accelerators through data parallelism, or lower precision from FP32/TF32 to BF16 where numerically safe. On NVIDIA Tensor Core paths, BF16 peak throughput is typically about 2$\times$ TF32 peak, with realized speedup depending on kernels and bottlenecks.
