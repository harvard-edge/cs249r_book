...
Consider a transformer model with 7B parameters performing inference with batch size 1. The model requires 14 TFLOPs per forward pass. On an H100 GPU (`{python} h100_fp16_tflops_str` TFLOPs peak FP16 Tensor Core), the measured latency is 50ms. Calculate the achieved utilization ($\eta$). Is this system Data-bound, Algorithm-bound, or Machine-bound? Justify your answer.

*Answer*: First, calculate achieved throughput:
$$\text{Achieved FLOP/s} = \frac{14 \text{ TFLOPs}}{0.050 \text{ s}} = 280 \text{ TFLOP/s}$$
