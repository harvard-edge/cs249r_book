...
|:--------------|:------------------|:----------------------------------------------------------:|:-----------------:|:-----------------:|
| **Data**      | **I/O Overhead**  |  $\dfrac{\text{Data Wait Time}}{\text{Total Step Time}}$   |      $>$ 10%      |       $<$ 1%      |
| **Algorithm** | **Active Params** |    $\dfrac{\text{Nonzero Params}}{\text{Total Params}}$    |    100% (Dense)   |  $<$ 50% (Sparse) |
| **Machine**   | **MFU**           | $\dfrac{\text{Achieved model FLOP/s}}{\text{Peak FLOP/s}}$ |      $<$ 30%      |      $>$ 50%      |
...
[^fn-scaling-laws]: **Scaling Laws**: Empirical relationships, typically power laws of the form $\mathcal{L}(x) \propto x^{-\alpha}$, that predict model loss as a function of dataset size, parameter count, or compute budget. @kaplan2020scaling studied these relationships for neural language models at OpenAI; @hoffmann2022chinchilla later refined the compute-optimal training trade-off with the Chinchilla result. @Sec-model-training discusses scaling laws in detail.
