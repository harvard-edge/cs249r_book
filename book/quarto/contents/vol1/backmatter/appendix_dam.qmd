...
from physx.formatting import fmt, md

# =============================================================================
# PURPOSE
# =============================================================================
# Purpose: Compute Iron Law utilization example
# Used in: DAM exercise

# =============================================================================
# INPUT
# =============================================================================
ex2_flops_per_pass_value = 14  # TFLOPs
ex2_latency_s_value = 0.050  # 50ms
h100_fp16_tflops_value = int(H100_FLOPS_FP16_TENSOR.to(TFLOPs / second).magnitude)

# =============================================================================
# PROCESS
# =============================================================================
ex2_achieved_tflops_value = ex2_flops_per_pass_value / ex2_latency_s_value
ex2_utilization_value = ex2_achieved_tflops_value / h100_fp16_tflops_value * 100

# =============================================================================
# OUTPUT
# =============================================================================
# Input constant strings (for prose)
ex2_flops_per_pass_str = f"{ex2_flops_per_pass_value}"
ex2_latency_ms_str = f"{int(ex2_latency_s_value * 1000)}"
ex2_latency_s_str = f"{ex2_latency_s_value:.3f}"

# Computed result strings
ex2_achieved_str = fmt(ex2_achieved_tflops_value, precision=0, commas=False)
ex2_util_str = fmt(ex2_utilization_value, precision=1, commas=False)
h100_fp16_tflops_str = f"{h100_fp16_tflops_value}"

# LaTeX equations as Markdown objects (inline Python inside $$...$$ won't render)
ex2_achieved_eq = md(
    f"$$\\text{{Achieved FLOP/s}} = \\frac{{{ex2_flops_per_pass_value} \\text{{ TFLOPs}}}}"
    f"{{0.050 \\text{{ s}}}} = {ex2_achieved_str} \\text{{ TFLOP/s}}$$"
)
ex2_util_eq = md(
    f"$$\\eta = \\frac{{{ex2_achieved_str}}}{{{h100_fp16_tflops_value}}} \\approx 28.3\\%$$"
)
```
...
**The systems engineer's job is to make the components run in parallel, not in series.** This is D·A·M Coordination:
...
*   **If **Arithmetic Intensity** $<$ 100 Ops/Byte**: You are likely **Memory Bound** (Data/Machine boundary). This threshold is approximate for current-generation accelerators; compute your hardware's specific ridge point ($R_{peak}/BW$) for a precise boundary.
...
Once you identify the bottleneck, this table tells you what to do—and what NOT to do:
...
## Case Studies: D·A·M in the Wild
...
The D·A·M taxonomy provides a systematic framework for diagnosing ML systems bottlenecks. Each axis maps to a distinct physical constraint: Data is bounded by bandwidth, Algorithm by total operations, and Machine by peak throughput. The **Iron Law** equation ($\text{Time} = \frac{D_{vol}}{BW} + \frac{O}{R_{peak} \cdot \eta} + L_{lat}$) quantifies these constraints, transforming performance debugging from guesswork into engineering. Use **Arithmetic Intensity** to determine the Data/Machine boundary, and the D·A·M Scorecard to evaluate system maturity.

::: {.callout-takeaways}

* **Every bottleneck lives in one of three places**: Data (information flow), Algorithm (mathematical logic), or Machine (physical execution). The D·A·M taxonomy provides MECE categories for systematic diagnosis.
...
* **Diagnose before you optimize**: The D·A·M Scorecard (I/O Overhead, Active Params, MFU) provides quantitative thresholds for system health. Measure first, then act.
...
A production image classification service runs on an A100 GPU. The `nvidia-smi` output shows 25% GPU utilization, while `iotop` reveals the disk is saturated at 100%. Which D·A·M axis is the bottleneck? What are two specific optimizations you would recommend?
...
This system is **Memory-bound** (a **Data** bottleneck). At batch size 1, each layer performs a matrix-vector multiply (GEMV) rather than a matrix-matrix multiply (GEMM). The model's 7B parameters (~14 GB in FP16) must be loaded from HBM for every forward pass, but each loaded weight is used for only a single input vector—yielding very low arithmetic intensity. The GPU's compute units sit idle waiting for memory transfers, which is why utilization is only `{python} ex2_util_str`%.
...
A colleague proposes upgrading from 4× A100 GPUs to 8× H100 GPUs because training is "too slow." Before approving the $200K hardware purchase, what three diagnostic questions would you ask? Map each question to the D·A·M axis it investigates.
