...
*   **Observation**: A team provisions a large A100 GPU instance to speed up training, but training time hardly improves. `nvidia-smi` shows GPU utilization fluctuating between 10 percent and 40 percent.
...
*   **Observation**: A real-time recommendation service fails to meet a 20 ms latency service-level agreement (SLA). Accelerator utilization is low and the batch size is one.
...
*   **Observation**: Accelerator utilization is pinned at 99 percent, memory bandwidth remains unsaturated, and training is stable but takes three weeks.
