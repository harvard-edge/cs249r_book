...
[^fn-mece]: **MECE (Mutually Exclusive, Collectively Exhaustive)**\index{Mutually Exclusive, Collectively Exhaustive (MECE)}: A classification principle from management consulting requiring that categories do not overlap and together cover every possibility. D·A·M is not strictly MECE because its diagnostic axes overlap. It uses the categories as first-pass lenses, so the most useful diagnosis may name a dominant axis together with a boundary effect such as queueing, runtime overhead, or memory-bound execution.
