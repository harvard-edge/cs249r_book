...
A system that runs slowly rarely announces why. One symptom, an idle accelerator, can trace to a starved input pipeline, an algorithm with too little arithmetic per byte, or a machine whose memory bandwidth is already saturated. This appendix turns that ambiguity into a procedure, connecting symptoms and measurements to the axis that actually binds—data, algorithm, or machine. It assumes the three axes introduced in the opening chapters and the iron law that quantifies them. Where the chapters develop each axis in depth, this appendix is the field guide for telling them apart.
