...
* **Anti-patterns waste engineering cycles**: Buying faster GPUs for slow data loaders, changing architectures for I/O bottlenecks, or writing CUDA kernels before profiling are common traps.
...
$$\text{Achieved FLOP/s} = \frac{14 \times 10^{12} \text{ FLOPs}}{0.050 \text{ s}} = `{python} ex2_achieved_str` \text{ TFLOP/s}$$

Then calculate utilization:
$$\eta = \frac{`{python} ex2_achieved_str`}{`{python} h100_fp16_tflops`} = `{python} ex2_util_str`\%$$
