...
A team provisions a large A100 GPU instance to speed up training, but training time hardly improves. `nvidia-smi` shows GPU utilization fluctuating between ten percent and 40 percent.
...
Once a hypothesis exists (for example, "the workload appears Machine-bound"), evidence is needed to confirm it. Abstract concepts must be measured with concrete utilities. @tbl-dam-tooling connects the theoretical components to the specific Linux and Python profiling tools you should reach for.
