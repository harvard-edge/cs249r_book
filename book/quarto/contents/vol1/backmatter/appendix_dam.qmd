...
This is not a model or hardware problem. You must optimize the ETL pipeline: move from raw JPEGs (CPU decoding heavy) to TFRecords or WebDataset (sequential reads), increase the number of data loader workers, and prefetch batches to GPU memory.

### Case 2: The Latency Cliff (Algorithm)

#### Symptom {.unnumbered}

Your real-time recommendation system fails to meet the 20ms latency SLA (Service Level Agreement). The GPU utilization is low, and the batch size is 1.
