...
This equation transforms performance debugging from a qualitative guessing game into a quantitative engineering problem. Every bottleneck hides in one of these terms. If your system is slow, it is because you are moving too much data ($D_{\text{vol}}$), lacking bandwidth ($\text{BW}$), executing too many operations ($O$), or failing to use your hardware's peak capability ($\eta$). The levers below map specific optimizations to the variable they improve.
...
This system is **Memory-bound** (a **Data** bottleneck). At batch size one, each layer performs a matrix-vector multiply (GEMV) rather than a general matrix multiply (GEMM). The model's `{python} DAMTaxonomy.ex2_params_str` parameters (~`{python} DAMTaxonomy.ex2_model_size_gb_str` GB in FP16) must be loaded from HBM for every forward pass, but each loaded weight is used for only a single input vector—yielding very low arithmetic intensity. The GPU's compute units sit idle waiting for memory transfers, which is why utilization is only `{python} DAMTaxonomy.ex2_util_str` percent.
