...
from mlsysim.fmt import fmt_int, fmt, fmt_count, fmt_memory, fmt_percent, fmt_qty
...
class SparseEmbeddingExample:
    ...
    total_elements_str = fmt_count(total_elements, scale="B", precision=0, commas=False)
    dense_gb_str = fmt_qty(dense_bytes_q, GB, precision=0, commas=False)
    nonzeros_str = fmt_count(nonzeros, scale="M", precision=0, commas=False)
    ...
    bytes_int32_str = fmt_memory(BYTES_INT32, unit=byte, precision=0, commas=False)
...
from mlsysim.fmt import fmt_int, fmt, fmt_memory, MarkdownStr
...
$$
`{python} GPT2TrainingMem.layers_str` \times `{python} GPT2TrainingMem.act_factor_str` \times `{python} GPT2TrainingMem.batch_small_str` \times `{python} GPT2TrainingMem.seq_len_str` \times `{python} GPT2TrainingMem.d_str` \times `{python} GPT2TrainingMem.bf16_str` \text{ bytes } \approx `{python} GPT2TrainingMem.act_small_gb_str`
$$
