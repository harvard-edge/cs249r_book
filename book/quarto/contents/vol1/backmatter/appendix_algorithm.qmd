...
class GemmIntensityExample:
    ...
    gemm_intensity_eq = fmt_math(
        rf"\text{{Intensity}} = \frac{{O}}{{D_{{\text{{vol}}}}}} = \frac{{2n^3}}{{3n^2 \times {bytes_fp16_value_str}}} = \frac{{n}}{{3}} \text{{ FLOP/byte}}"
    )
```
...
*   **Dense (FP32)**: `{python} SparseEmbeddingExample.total_elements_str` $\times$ $10^{9}$ $\times$ `{python} SparseEmbeddingExample.bytes_fp32_value_str` bytes = `{python} SparseEmbeddingExample.dense_gb_str`.
*   **Sparse** (`{python} SparseEmbeddingExample.sparsity_pct_str` density): Storing only nonzeros requires roughly `{python} SparseEmbeddingExample.nonzeros_str` $\times$ $10^{6}$ $\times$ (`{python} SparseEmbeddingExample.bytes_fp32_value_str` bytes value + `{python} SparseEmbeddingExample.bytes_int32_value_str` bytes index) ≈ `{python} SparseEmbeddingExample.sparse_mb_str`.
...
# │ Exports: gpt2_P_str, gpt2_layers_str, gpt2_d_str, gpt2_bf16_bytes_value_str,
# │          gpt2_weights_gb_str, gpt2_grads_gb_str, gpt2_optim_bytes_value_str,
...
$$
`{python} GPT2TrainingMem.layers_str` \times `{python} GPT2TrainingMem.act_factor_str` \times `{python} GPT2TrainingMem.batch_small_str` \times `{python} GPT2TrainingMem.seq_len_str` \times `{python} GPT2TrainingMem.d_str` \times `{python} GPT2TrainingMem.bf16_bytes_value_str` \text{ bytes } \approx `{python} GPT2TrainingMem.act_small_gb_str`
$$
