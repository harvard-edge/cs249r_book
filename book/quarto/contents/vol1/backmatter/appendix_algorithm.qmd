...
n_small_str = AlgorithmFoundations.n_small_str
small_intensity_str = AlgorithmFoundations.small_intensity_str
small_efficiency_pct_str = AlgorithmFoundations.small_efficiency_pct_str
a100_ridge_str = AlgorithmFoundations.a100_ridge_str
sparsity_pct_str = AlgorithmFoundations.sparsity_pct_str
reduction_factor_str = AlgorithmFoundations.reduction_factor_str
vocab_size_str = AlgorithmFoundations.vocab_size_str
embed_dim_str = AlgorithmFoundations.embed_dim_str
total_elements_str = AlgorithmFoundations.total_elements_str
dense_gb_str = AlgorithmFoundations.dense_gb_str
nonzeros_str = AlgorithmFoundations.nonzeros_str
sparse_mb_str = AlgorithmFoundations.sparse_mb_str
bytes_fp16_str = AlgorithmFoundations.bytes_fp16_str
bytes_fp32_str = AlgorithmFoundations.bytes_fp32_str
bytes_int32_str = AlgorithmFoundations.bytes_int32_str
optimizer_overhead_str = AlgorithmFoundations.optimizer_overhead_str
gemm_intensity_eq = AlgorithmFoundations.gemm_intensity_eq
```
...
- **Small matrices are inefficient**: A `{python} n_small_str`$\times$ `{python} n_small_str` GEMM has intensity `{python} n_small_str`/3 ≈ `{python} small_intensity_str` FLOP/byte, well below the ridge point (`{python} a100_ridge_str`), achieving only ~`{python} small_efficiency_pct_str`% of peak throughput.
...
*   **Dense (FP32)**: `{python} total_elements_str`$\times$ $10^{9}$$\times$ `{python} bytes_fp32_str` bytes = **`{python} dense_gb_str` GB**.
*   **Sparse (`{python} sparsity_pct_str`% density)**: Storing only non-zeros requires roughly `{python} nonzeros_str`$\times$ $10^{6}$$\times$ (`{python} bytes_fp32_str` bytes value + `{python} bytes_int32_str` bytes index) ≈ **`{python} sparse_mb_str` MB**.
*   *Result*: A `{python} reduction_factor_str`$\times$ reduction in memory footprint, fitting a model that would otherwise OOM (Out of Memory).
...
class GPT2TrainingMem:
    ...
    # Per-layer activation: 2 * B * S * d * bytes_bf16 (approximate)
    act_small_bytes = layers * 2 * batch_small * seq_len * d * bf16
    act_small_gb = act_small_bytes / 1e9
    total_small_gb = model_state_gb + act_small_gb

    act_large_bytes = layers * 2 * batch_large * seq_len * d * bf16
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
gpt2_P_str = GPT2TrainingMem.P_str
gpt2_layers_str = GPT2TrainingMem.layers_str
gpt2_d_str = GPT2TrainingMem.d_str
gpt2_bf16_str = GPT2TrainingMem.bf16_str
gpt2_weights_gb_str = GPT2TrainingMem.weights_gb_str
gpt2_grads_gb_str = GPT2TrainingMem.grads_gb_str
gpt2_optim_bytes_str = GPT2TrainingMem.optim_bytes_str
gpt2_optim_gb_str = GPT2TrainingMem.optim_gb_str
gpt2_model_state_gb_str = GPT2TrainingMem.model_state_gb_str
gpt2_accel_gb_str = GPT2TrainingMem.accel_gb_str
gpt2_remaining_gb_str = GPT2TrainingMem.remaining_gb_str
gpt2_batch_small_str = GPT2TrainingMem.batch_small_str
gpt2_seq_len_str = GPT2TrainingMem.seq_len_str
gpt2_act_small_gb_str = GPT2TrainingMem.act_small_gb_str
gpt2_total_small_gb_str = GPT2TrainingMem.total_small_gb_str
gpt2_batch_large_str = GPT2TrainingMem.batch_large_str
gpt2_act_large_gb_str = GPT2TrainingMem.act_large_gb_str
```

::: {.callout-notebook title="Worked Example: GPT-2 (1.5B) Training Memory"}

**The model**: GPT-2 XL has $P = `{python} gpt2_P_str`$ parameters, `{python} gpt2_layers_str` layers, hidden dimension $d = `{python} gpt2_d_str`$.

**Model state (fixed per step)**:

- Weights (BF16): $`{python} gpt2_P_str` \times `{python} gpt2_bf16_str`$ bytes = `{python} gpt2_weights_gb_str` GB
- Gradients (BF16): $`{python} gpt2_P_str` \times `{python} gpt2_bf16_str`$ bytes = `{python} gpt2_grads_gb_str` GB
- Optimizer (FP32 master + momentum + variance): $`{python} gpt2_P_str` \times `{python} gpt2_optim_bytes_str`$ bytes = `{python} gpt2_optim_gb_str` GB
- **Total model state: `{python} gpt2_model_state_gb_str` GB** — fits on an A100/H100 (`{python} gpt2_accel_gb_str` GB), but leaves only `{python} gpt2_remaining_gb_str` GB for activations.

**Activations (scale with batch)**:

Per-layer activation memory for a transformer is approximately $2 \times B \times S \times d$ bytes (in BF16), where $B$ is batch size and $S$ is sequence length. With `{python} gpt2_layers_str` layers, batch size `{python} gpt2_batch_small_str`, and sequence length `{python} gpt2_seq_len_str`:

$`{python} gpt2_layers_str` \times 2 \times `{python} gpt2_batch_small_str` \times `{python} gpt2_seq_len_str` \times `{python} gpt2_d_str` \times `{python} gpt2_bf16_str`$ bytes $\approx$ `{python} gpt2_act_small_gb_str` GB

**Total: `{python} gpt2_total_small_gb_str` GB** — fits on one `{python} gpt2_accel_gb_str` GB accelerator. However, increase the batch to `{python} gpt2_batch_large_str` and activations grow to `{python} gpt2_act_large_gb_str` GB, exceeding the remaining capacity. This is the threshold where gradient checkpointing (trading ~33% more compute for $O(\sqrt{L})$ activation memory) becomes necessary.
