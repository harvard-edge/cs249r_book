...
# Sparse matrix example: 100,000 × 10,000 embedding matrix
vocab_size = 100_000
embed_dim = 10_000
total_elements = vocab_size * embed_dim  # 1 billion
bytes_per_fp32 = 4
dense_bytes = total_elements * bytes_per_fp32
dense_gb = dense_bytes / 1e9  # 4 GB

# Sparse at 1% density
sparsity_pct = 1
nonzeros = int(total_elements * sparsity_pct / 100)  # 10 million
# CSR format: value (4 bytes) + column index (4 bytes) per nonzero
sparse_bytes = nonzeros * (4 + 4)  # ~80 MB
sparse_mb = sparse_bytes / 1e6
reduction_factor = int(dense_bytes / sparse_bytes)  # 50x

# 64x64 GEMM efficiency example
# Arithmetic intensity = n/3 for n×n matrices
n_small = 64
small_intensity = n_small / 3  # ~21 FLOP/byte
# Ridge point for A100: ~156 FLOP/byte (from appendix_machine)
a100_ridge = int(A100_FLOPS_FP16_TENSOR.to(TFLOPs / second).magnitude /
                 A100_MEM_BW.to(TB / second).magnitude)
# When memory-bound, efficiency ≈ intensity / ridge_point
small_efficiency_pct = small_intensity / a100_ridge * 100  # ~14%
...
vocab_size_str = f"{vocab_size:,}"
embed_dim_str = f"{embed_dim:,}"
total_elements_str = f"{total_elements/1e9:.0f}"
dense_gb_str = f"{dense_gb:.0f}"
nonzeros_str = f"{nonzeros/1e6:.0f}"
sparse_mb_str = f"{sparse_mb:.0f}"
```
...
We use **Einstein summation** notation throughout this book because it makes complex operations explicit.[^fn-einsum] Matrix multiplication $C = AB$ becomes:

[^fn-einsum]: **Einstein Summation**: Implemented as `torch.einsum` in PyTorch and `np.einsum` in NumPy. It provides a unified DSL for dot products, outer products, transposes, and matrix multiplications.
...
- **Small matrices are inefficient**: A `{python} n_small`×`{python} n_small` GEMM has intensity `{python} n_small`/3 ≈ `{python} small_intensity_str` FLOP/byte, well below the ridge point (`{python} a100_ridge`), achieving only ~`{python} small_efficiency_pct_str`% of peak throughput.
...
*   **Sparse (`{python} sparsity_pct`% density)**: Storing only non-zeros requires roughly `{python} nonzeros_str` × 10⁶ × (4 bytes value + 4 bytes index) ≈ **`{python} sparse_mb_str` MB**.
*   *Result*: A `{python} reduction_factor`× reduction in memory footprint, fitting a model that would otherwise OOM (Out of Memory).
...
| **Attention (Single Head)** | $(B, S, d_{model})$    |             $4 \times d_{model}^2$ |       $2 S^2 d_{model} + 8 S d_{model}^2$ |
| **LayerNorm**               | $(B, S, d_{model})$    |               $2 \times d_{model}$ |          $O(B \times S \times d_{model})$ |

: **Deep Learning Tensor Primitives**: Summary of shapes, parameters, and FLOP counts. Note: $B$ is batch size, $S$ is sequence length, $K$ is kernel size. The Attention FLOPs include QKV projections and the $S^2$ attention matrix interactions. {#tbl-tensor-op-ref}
...
**The Activation Explosion**: While weights are fixed ($O(P)$), activations grow linearly with **Batch Size** and **Sequence Length**. For Large Language Models, activations can be 10-50x larger than the weights. This is why techniques like Gradient Checkpointing (recomputing activations) and FlashAttention (tiling attention) are mandatory, not optional.

**Memory Implication**: Cached activations dominate memory usage. Storing activations for a 100-layer network consumes huge memory, motivating **Gradient Checkpointing** [@chen2016training] (recomputing activations to save memory).[^fn-grad-checkpoint]
