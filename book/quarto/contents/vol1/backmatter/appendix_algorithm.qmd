...
A profile showing low utilization is often read as a hardware problem when the cause is algorithmic: the shape of a matrix multiply, a layout that breaks contiguity, or activations held for the backward pass. This appendix collects the models and derivations that make such causes measurable, covering matrix multiplication, tensor shapes and layouts, sparse storage, differentiation, and training memory. It assumes familiarity with linear algebra and with the iron law introduced early in the book. The chapters can then reason about system behavior without rederiving the mechanics underneath it.
