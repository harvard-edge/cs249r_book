...
We use Einstein summation[^fn-einsum] notation throughout this book because it makes complex operations explicit (implemented as `torch.einsum` in PyTorch and `np.einsum` in NumPy). Matrix multiplication $\mathbf{C} = \mathbf{A}\mathbf{B}$ becomes:
...
* **GEMM arithmetic intensity**: Under the ideal $\beta=0$ compulsory-traffic model it scales as $n/3$ for square FP16 matrices ($n/4$ when $C$ must also be read), so small matrices tend to be memory bound. Larger GEMMs or fused work and tile-aligned dimensions can improve throughput.
* **Sparse storage formats**: They reduce memory once density falls below the approximately `{python} SparseBreakEvenRecap.csr_break_even_density_pct_str` metadata break-even point for CSR with FP32 values and INT32 indices when row-pointer overhead is small. Useful sparse-kernel speedups often require substantially higher sparsity; the threshold depends on structure, kernel, and hardware.
* **Tensor layout bugs are silent**: Tensor shapes, strides, and broadcasting are the source of many subtle bugs: a noncontiguous tensor can silently degrade kernel performance, and an incorrect broadcast can produce a result of the wrong shape without raising an error.
* **Training memory is activation-sensitive**: Training memory is dominated by four components—weights, gradients, optimizer states, and activations—and the last of these grows with batch size and sequence length and can exceed the model's weight footprint.
* **Backpropagation trades memory for compute**: Backpropagation's efficiency comes from computing all parameter gradients in a single backward pass, but it requires forward-pass activations to be available during backward, creating a fundamental memory–compute trade-off that gradient checkpointing partially resolves.
