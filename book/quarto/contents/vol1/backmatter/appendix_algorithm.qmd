...
- **Power-of-two dimensions help**: Hardware tensor cores are optimized for specific tile sizes (typically $16 \times 16$ or $32 \times 32$). Dimensions that align with these sizes avoid padding overhead.
...
To see the trade-off concretely, consider a vocabulary embedding matrix of size `{python} AlgorithmFoundations.vocab_size_str` $\times$ `{python} AlgorithmFoundations.embed_dim_str` (`{python} AlgorithmFoundations.total_elements_str` billion parameters):
