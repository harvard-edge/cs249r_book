...
- **Small matrices are inefficient**: A $`{python} n_small` \times `{python} n_small`$ GEMM has intensity $`{python} n_small`/3 \approx `{python} int(small_intensity)$` FLOP/byte, well below the ridge point ($`{python} a100_ridge`$), achieving only ~$`{python} int(small_efficiency_pct)`\%$ of peak throughput.
...
**Example**: A vocabulary embedding matrix of size $`{python} f"{vocab_size:,}"` \times `{python} f"{embed_dim:,}"`$ (`{python} f"{total_elements/1e9:.0f}"` billion parameters).
*   **Dense (FP32)**: $`{python} f"{total_elements/1e9:.0f}"` \times 10^9 \times 4$ bytes = **`{python} f"{dense_gb:.0f}"` GB**.
*   **Sparse (`{python} sparsity_pct`% density)**: Storing only non-zeros requires roughly $`{python} f"{nonzeros/1e6:.0f}"` \times 10^6 \times (4 \text{ bytes value} + 4 \text{ bytes index}) \approx$ **`{python} f"{sparse_mb:.0f}"` MB**.
*   *Result*: A `{python} reduction_factor`x reduction in memory footprint, fitting a model that would otherwise OOM (Out of Memory).
...
+-------------------+------------------------+------------------------------------+-------------------------------------------+
| **Attention**     | $(B, S, d_{model})$    | $4 \times d_{model}^2$             | $2 \times S^2 \times d_{model} +$         |
| **(Single Head)** |                        |                                    | $8 \times S \times d_{model}^2$           |
+-------------------+------------------------+------------------------------------+-------------------------------------------+
