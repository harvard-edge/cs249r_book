...
**Total: ~48 GB** — fits on one 80 GB accelerator. However, increase the batch to 32 and activations grow to ~96 GB, exceeding the remaining capacity. This is the threshold where gradient checkpointing (trading ~33% more compute for $O(\sqrt{L})$ activation memory) becomes necessary.
