...
- **Small matrices are inefficient**: A `{python} n_small`×`{python} n_small` GEMM has intensity `{python} n_small`/3 ≈ `{python} int(small_intensity)` FLOP/byte, well below the ridge point (`{python} a100_ridge`), achieving only ~`{python} int(small_efficiency_pct)`% of peak throughput.
...
**Example**: A vocabulary embedding matrix of size `{python} f"{vocab_size:,}"`×`{python} f"{embed_dim:,}"` (`{python} f"{total_elements/1e9:.0f}"` billion parameters).

*   **Dense (FP32)**: `{python} f"{total_elements/1e9:.0f}"` × 10⁹ × 4 bytes = **`{python} f"{dense_gb:.0f}"` GB**.
*   **Sparse (`{python} sparsity_pct`% density)**: Storing only non-zeros requires roughly `{python} f"{nonzeros/1e6:.0f}"` × 10⁶ × (4 bytes value + 4 bytes index) ≈ **`{python} f"{sparse_mb:.0f}"` MB**.
