...
This geometric interpretation is why dot products appear everywhere in modern architectures. In attention mechanisms, query ($\mathbf{Q}$) and key ($\mathbf{K}$) vectors are dot-produced to compute a similarity score that determines how much each token attends to every other token. The resulting attention weights are then used to form a weighted combination of value ($\mathbf{V}$) vectors—making the dot product the foundation of the transformer's ability to model long-range dependencies.
...
This ideal bound assumes $\beta=0$: read $\mathbf{A}$ and $\mathbf{B}$, then write $\mathbf{C}$. Reading the old $\mathbf{C}$ when $\beta \ne 0$ changes it to $n/4$ FLOP/byte. This explains several important phenomena:
...
CSR is essential for recommendation systems (sparse embedding tables) and pruned models. For an $M{\times}N$ matrix with $\text{nnz}$ nonzeros, CSR uses $\mathcal{O}(\text{nnz} + M)$ storage instead of $\mathcal{O}(M \times N)$; when $\text{nnz}$ is large relative to $M$, this is often summarized as $\mathcal{O}(\text{nnz})$.
...
::: {#fig-backprop-graph fig-env="figure" fig-pos="htb" fig-cap="**Backpropagation Computational Graph**: A two-layer network showing the forward pass (gray arrows) and backward pass (red dashed arrows). Each node caches values during the forward pass that are reused during the backward pass." fig-alt="A computational graph with four nodes labeled x, h, y, and a script L for the loss, connected left to right. Solid gray arrows show the forward pass with weights W1 and W2. Dashed red arrows curve backward showing gradient flow with partial derivative notation."}
```{.tikz}
...
1. A model has 300 million parameters and is trained with Adam in mixed precision (FP16 weights, FP32 optimizer states). Without activations, how many GB of memory do the weights, gradients, and optimizer states require?
...
* **GEMM arithmetic intensity**: Under the ideal $\beta=0$ compulsory-traffic model it scales as $n/3$ for square FP16 matrices ($n/4$ when $\mathbf{C}$ must also be read), so small matrices tend to be memory bound. Larger GEMMs or fused work and tile-aligned dimensions can improve throughput.
