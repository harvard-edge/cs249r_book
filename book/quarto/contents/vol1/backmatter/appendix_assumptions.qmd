...
# │ How:     Scalar arithmetic from mlsys.constants.
# │
...
napkin_h100_tflops_str = NapkinMath.h100_tflops_str
napkin_h100_bw_str = NapkinMath.h100_bw_str
napkin_ridge_str = NapkinMath.ridge_str
napkin_gemm_n_str = NapkinMath.gemm_n_str
napkin_gemm_intensity_str = NapkinMath.gemm_intensity_str
napkin_model_b_str = NapkinMath.model_b_str
napkin_model_b_digit_str = f"{NapkinMath.model_params_b}"
napkin_bytes_per_param_str = NapkinMath.bytes_per_param_str
napkin_train_mem_gb_str = NapkinMath.train_mem_gb_str
napkin_accel_mem_str = NapkinMath.accel_mem_str
napkin_a100_tdp_str = NapkinMath.a100_tdp_str
napkin_gpt3_ops_str = NapkinMath.gpt3_ops_str
napkin_gpt3_days_str = NapkinMath.gpt3_days_str
napkin_elec_rate_str = NapkinMath.elec_rate_str
napkin_a100_kw_str = NapkinMath.a100_kw_str
napkin_elec_cost_str = NapkinMath.elec_cost_str
```
...
**Is this workload compute-bound or memory-bound?** Divide peak FLOPS by memory bandwidth to get the *ridge point* (@sec-machine-foundations-roofline-model-2529). For an H100: `{python} napkin_h100_tflops_str` TFLOPS / `{python} napkin_h100_bw_str` TB/s $\approx$ `{python} napkin_ridge_str` FLOP/byte. Any operation with arithmetic intensity above `{python} napkin_ridge_str` FLOP/byte is compute-bound; below it, memory-bound. A large GEMM with $n=`{python} napkin_gemm_n_str`$ has intensity $n/3 \approx `{python} napkin_gemm_intensity_str`$ FLOP/byte (compute-bound). A single-token autoregressive decode has intensity $\approx 1$ FLOP/byte (deeply memory-bound).

**How much memory does training a `{python} napkin_model_b_str` model require?** Mixed-precision Adam stores 2 bytes (BF16 weights) + 2 bytes (gradients) + 12 bytes (FP32 master weights + momentum + variance) = `{python} napkin_bytes_per_param_str` bytes per parameter. For `{python} napkin_model_b_str` parameters: $`{python} napkin_model_b_digit_str` \times 10^9 \times `{python} napkin_bytes_per_param_str`$ bytes = `{python} napkin_train_mem_gb_str` GB. An A100 or H100 has `{python} napkin_accel_mem_str` GB of HBM, so the model state alone exceeds a single accelerator—before accounting for activations.

**How much energy does one GPT-3 training run cost?** An A100 draws `{python} napkin_a100_tdp_str` W at TDP. GPT-3 training used roughly $`{python} napkin_gpt3_ops_str`$ FLOPS across `{python} napkin_gpt3_days_str` accelerator-days. At \$`{python} napkin_elec_rate_str`/kWh: `{python} napkin_gpt3_days_str` accelerator-days$\times$ 24 h/day$\times$ `{python} napkin_a100_kw_str` kW$\times$ \$`{python} napkin_elec_rate_str`/kWh $\approx$ `{python} napkin_elec_cost_str` in electricity alone—a small fraction of the total cost, which is dominated by accelerator amortization.
...
: **NVIDIA A100 (Ampere)**: The A100 is the most commonly cited accelerator in the book's training examples. Its 80 GB HBM2e capacity and TF32 tensor cores set the reference point for memory-capacity and compute-intensity calculations. {#tbl-assumptions-a100}
...
: **Google TPU v4**: The TPU v4's BF16 throughput and memory bandwidth are used when comparing ASIC-based training economics against accelerator-based alternatives. {#tbl-assumptions-tpuv4}
...
: **CPU, Mobile NPU, and Edge Device Specs**: These constants ground the edge and mobile ML examples. The contrast between mobile NPU throughput and datacenter accelerator throughput illustrates why deployment target shapes every design decision. {#tbl-assumptions-cpu-mobile}
...
| **GPT4_TRAINING_ACCELERATOR_DAYS** | `{python} GPT4_TRAINING_GPU_DAYS_val` | `{python} GPT4_TRAINING_GPU_DAYS_unit` |
...
: **Energy per Operation and Access**: These constants quantify the energy hierarchy from register file through DRAM and across precision formats. The 200$\times$ gap between a register read and a DRAM access explains why data reuse (tiling, fusion) dominates ML kernel optimization. {#tbl-assumptions-energy}
