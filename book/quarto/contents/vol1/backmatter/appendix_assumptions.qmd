...
This appendix lists the physical constants, hardware specifications, and economic assumptions used to calculate the quantitative examples. These values are defined in the book's source code (`physx/constants.py`) so derivations remain consistent across chapters and can be updated in one place as hardware generations change.

## How to Use This Appendix {.unnumbered}

Use this appendix as a **reference table** when you want to verify a calculation, swap in an alternative assumption (e.g., different bandwidth or power), or audit which constants a back-of-the-envelope estimate depends on.
