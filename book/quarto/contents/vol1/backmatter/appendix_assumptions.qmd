...
Because hardware generations change faster than textbooks, these tables also serve as a change log of sorts: updating a single value in `physx/constants.py` propagates the change to every calculation in every chapter automatically.
...
Finally, the constants file defines base and derived units so that all dimensional analysis in the book uses a consistent unit system via the `pint` library. These are not assumptions per se, but they ensure that every computed value carries its units and that unit-conversion errors are caught automatically.
