...
# │
# │ Why: Generates formatted value/unit pairs for all physical constants used
# │      in the book. Enables readers to verify napkin math calculations and
# │      understand the assumptions underlying quantitative examples. Provides
# │      a single source of truth for all hardware specs and physical constants.
# │
# │ Imports: physx.constants (all constants), pint
...
With hardware specifications established, the next question is: what workloads run on this hardware? The following tables capture the model architectures whose parameter counts, FLOP budgets, and training costs appear in the book's calculations.
...
Energy-per-operation and energy-per-access constants come from semiconductor process measurements (primarily 45 nm and 7 nm nodes) and are used in the book's energy-efficiency and sustainability analyses. The hierarchy from register access through DRAM illustrates why the memory wall is fundamentally an energy wall.
...
Distributed training and multi-GPU serving are bottlenecked by interconnect bandwidth as often as by compute. These constants capture the bandwidths for GPU-to-GPU links (NVLink), cross-node fabrics (InfiniBand), host buses (PCIe), storage (NVMe), datacenter Ethernet, and the speed-of-light floor that sets the minimum latency for any network hop.
...
Cost estimates throughout the book depend on electricity and cloud pricing assumptions. These are order-of-magnitude reference values; actual prices vary by region, provider, and contract terms, but the ratios between them are more stable than the absolute numbers.
...
When reasoning about production ML systems, it helps to have concrete scale anchors for "how much traffic does a large service actually handle?" These constants provide order-of-magnitude reference points drawn from public disclosures.
...
The constants file defines base and derived units so that all dimensional analysis in the book uses a consistent unit system via the `pint` library. These are not assumptions per se, but they ensure that every computed value carries its units and that unit-conversion errors are caught automatically.
...
- Every quantitative example in this book traces back to a specific constant in `physx/constants.py`. This appendix exposes all of those constants so you can audit, verify, or update the numbers that underpin the book's reasoning.
