...
class FoundationCostAmortization:
    ...
    check(crossover_tasks_value == 12, f"Expected 12 crossover tasks, got {crossover_tasks_value}")
...
class BreakevenCalc:
    ...
    check(abs(roi_pct - 150.0) < 1.0, f"Expected 150% ROI, got {roi_pct}")
...
class DistributedOverheadCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(40 * minute < t_total_overhead < 50 * minute, f"Expected ~44.5 min total overhead, got {t_total_overhead}")
...
class FpFallacyCalc:
    ...
    check(abs(synthetic_acc_drop_pp - 12.0) < 0.1, f"Expected 12 pp accuracy drop, got {synthetic_acc_drop_pp}")
