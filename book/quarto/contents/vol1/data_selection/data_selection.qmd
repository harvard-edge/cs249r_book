...
To make this concrete, consider training a model in the GPT-2\index{GPT-2!data selection}/Llama\index{Llama!data selection} Lighthouse family from @sec-network-architectures-lighthouse-roster-model-biographies-a763, which spans the autoregressive large language model (LLM)\index{LLM} family from GPT-2's 1.5 billion parameters to Llama's 7-70 billion parameter range, here using a 70-billion-parameter language model.
...
**Scenario**: ImageNet and CIFAR-10 classification models evaluated on newly collected test sets (ImageNetV2) experienced 3 to 15 percent accuracy drops [@recht2019imagenet].
