...
class ComputeDataGap:
    ...
    sustained_mfu_pct_str = fmt(sustained_mfu * 100, precision=0, commas=False, suffix=" percent")
    cluster_cost_m_str = fmt(cluster_cost/c.MILLION, precision=1, commas=False)
```

The compute budget (`{python} ComputeDataGap.h100_count_str` H100 GPUs for `{python} ComputeDataGap.training_months_str`) represents roughly USD `{python} ComputeDataGap.cluster_cost_m_str` million at the chapter's cloud-training price anchor and can process about `{python} ComputeDataGap.tokens_capacity_str` tokens at `{python} ComputeDataGap.sustained_mfu_pct_str` sustained MFU. Estimates of quality- and repetition-adjusted public human-generated text are on the order of `{python} ComputeDataGap.public_text_stock_str` tokens, far larger than a single curated web corpus. The practical bottleneck is narrower: how much of that stock is accessible, legally usable, high quality, deduplicated, and useful for the target distribution. If a team has only a `{python} ComputeDataGap.filtered_corpus_tokens_str`-token filtered corpus ready for use, the compute budget can already process it roughly `{python} ComputeDataGap.corpus_reuse_ratio_str`$\times$ over. The team faces three options: train on the same data for multiple epochs (diminishing returns after epochs 2--3), lower quality thresholds to include more data (which can degrade model quality), or invest in data selection through better filtering, curriculum design, and synthetic augmentation to extract more learning from each token. The third option is increasingly the dominant approach.
