...
To make this concrete, consider training a model in the **GPT-2/Llama Lighthouse** family (@sec-network-architectures), which spans the autoregressive large language model (LLM) family from GPT-2's 1.5B parameters to Llama's 7B--70B range, here using a `{python} ComputeDataGap.llama_params_str` parameter language model:
