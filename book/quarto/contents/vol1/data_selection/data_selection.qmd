...
| **Synthetic Data**      |                                                                                                 Potentially large | Bounded by generator quality (risk of model collapse) |
...
This makes data selection multiplicatively valuable in the iron law\index{Iron Law!multiplicative savings from data selection}: when all three optimization layers act on the same bottleneck, a 2$\times$ reduction in dataset size with 2$\times$ fewer operations per sample and 2$\times$ higher effective throughput yields `{python} IronLawSavings.combined_factor_mult_str` total cost reduction, not 6$\times$.
...
Before a single gradient is computed, significant efficiency gains are available by removing low-value samples from the dataset. Static pruning\index{Static Pruning!pretraining filtration} and pretraining filtration reduce total computation without affecting, and sometimes improving, final model accuracy, all without modifying the training loop or model architecture.
...
Low-information filtering applies domain-specific heuristics to remove samples that lack sufficient signal for learning. For text corpora, this often means removing high-perplexity\index{Perplexity!filtering, low-information text removal} garbled text and, in some pipelines, very low-perplexity boilerplate or repetitive text. For image datasets, filtering targets blurry, corrupted, or near-uniform samples that provide little visual information.
...
A common fallacy is this: *Data is the new oil, so more is always better.*
...
A recurring pitfall is this: *Replacing real-data validation with synthetic-only training data.*
...
A common fallacy is this: *Data selection is just data cleaning.*
...
A recurring pitfall is this: *Treating data selection as a budget-only tactic.*
...
A common fallacy is this: *Selection overhead is too small to change training economics.*
...
A recurring pitfall is this: *Pruning rare classes into oblivion.*
...
A common fallacy is this: *Deduplicating training data is enough to make evaluation reliable.*
...
A recurring pitfall is this: *Active learning without considering annotation latency.*
...
A common fallacy is this: *If a technique works on ImageNet, it will work on my dataset.*
...
A recurring pitfall is this: *Optimizing data selection metrics instead of deployment metrics.*
