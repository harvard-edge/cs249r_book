...
[^fn-stable-diffusion-synthesis]: [offset=-20mm] **Stable Diffusion**: Performs iterative denoising in a compressed latent space, enabling targeted text-to-image training examples. Its cost is seconds per image rather than near-free geometric transforms, so it is useful when novelty matters more than per-sample throughput. \index{Stable Diffusion!generation cost}
