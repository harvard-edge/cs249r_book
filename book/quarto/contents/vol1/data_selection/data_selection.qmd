...
class ActiveLearningRoi:
    ...
    cost_per_label_value_str = fmt_usd(
    ...
    cost_per_label_str = f"{cost_per_label_value_str}/label"

    # For math expressions: $5/\text{label}
    cost_per_label_math_str = f"{cost_per_label_value_str}/\\text{{label}}"
    ...
    baseline_calc_math = fmt_math(
        f"{budget_str} \\div {cost_per_label_math_str} = {n_random_str}"
    )

    cost_active_calc_math = fmt_math(
        f"{n_active_str} \\times {cost_per_label_math_str} = {cost_active_str}"
    )

    training_work_calc_math = fmt_math(
        f"{n_random_str} \\div {n_active_str} = {speedup_mult_str}"
    )

    savings_calc_math = fmt_math(
        f"{budget_str} - {cost_active_str} = {cost_saving_str}"
    )
```
