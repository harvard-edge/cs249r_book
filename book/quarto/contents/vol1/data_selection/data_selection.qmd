...
class CostBreakdown:
    ...
    check(
        abs(train_compute_cost.to(USD).magnitude - 768.0) < 1e-9,
        f"Training should be 192 GPU-hours x $4.00/h = $768, got {train_compute_cost.to(USD).magnitude:.2f}.",
    )
    check(
...
| **Training (`{python} CostBreakdown.train_epoch_count_str` in `{python} CostBreakdown.train_duration_str` on `{python} CostBreakdown.train_gpu_count_str`)** | `{python} CostBreakdown.train_gpu_hours_str` $\times$ `{python} CostBreakdown.cb_gpu_rate_str`                                                         |                         `{python} CostBreakdown.c_train_usd_str`                         |
...
**Systems insight**: Acquisition and labeling can dwarf compute cost in supervised vision workloads, as they do under these assumptions, where a single day on eight accelerators accounts for well under 1 percent of the total. A longer or larger training campaign shifts that balance, so a complete ROI calculation must price both categories rather than assuming which one dominates.
