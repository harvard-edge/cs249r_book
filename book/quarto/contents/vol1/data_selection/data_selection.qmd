...
| **3. Synthetic Generation** | On-demand        | Augmentation, Generative Models, Distillation         | 2$\times$--10$\times$ effective data expansion |
...
| **Active Learning**   | Inference on unlabeled pool + human-in-the-loop latency | 2$\times$--10$\times$ reduction in labeling budget for same acc. |
...
* **The Selection Inequality gates every technique**: $T_{\text{selection}} + T_{\text{train}}(subset) < T_{\text{train}}(full)$. Selection overhead should stay below ten percent of training time. Proxy models and cached embeddings keep $T_{\text{selection}}$ low; expensive selection algorithms can consume all the savings they promise.
