...
_Why can a carefully selected 10 percent of a dataset match the accuracy of the full 100 percent?_

The highest-impact optimization in machine learning operates upstream, before a single gradient is computed: on the data itself. Naive scaling assumes data is homogeneous, that every sample contributes equally to learning, but reality differs dramatically: in large-scale datasets, a tiny fraction of examples provides the majority of the gradient signal while the vast majority are redundant, noisy, or misaligned with the target distribution. This heterogeneity is not a statistical artifact but a systems optimization opportunity: data engineering established that data is the source code of ML systems, and data selection\index{Data Selection!systems optimization rationale} recognizes that not all source code is equally valuable. Where compressing models and accelerating hardware speed up the execution of work, data selection reduces the core workload itself, so a training run that takes a week on the full dataset might take a day on a strategically selected subset, and that savings compounds through every iteration of the development cycle, from faster experimentation to quicker response to distribution drift to lower barriers for teams with limited compute budgets. The shift is paradigmatic: from accumulating data as a massive liability to curating it as a precise resource, where every sample earns its place by contributing learning signal that no other sample provides. That curation is a direct application of Data-Algorithm co-design: the dataset shaped deliberately to raise the statistical efficiency of the learning process.
...
For decades, the dominant strategy was straightforward: more data, better models. Scaling laws\index{Scaling Laws!data-compute asymmetry}[^fn-scaling-laws-origin] [@kaplan2020scaling; @hoffmann2022chinchilla] confirmed that model performance improves predictably with dataset size, and teams responded rationally by scraping more web pages, labeling more images, and generating more synthetic examples. A critical asymmetry has since emerged. Accelerator fleets can expand usable compute faster than the supply of novel, high-quality human-generated text and images. Much of the easily accessible public web has already been incorporated into large training corpora, and expert labeling capacity grows slowly. This asymmetry, which this chapter calls the **Data Wall**\index{Data Wall!definition}[^fn-data-wall-scaling], has inverted the optimization priority from "get more data" to "get more from existing data." This chapter develops that inversion into an engineering discipline: the static and dynamic methods that choose which examples to train on, the self-supervised and synthetic approaches that manufacture value when labeled data runs short, and the cost models and decision frameworks that determine when selection is worth the effort.
...
GPU compute scales roughly `{python} ScalingAsymmetry.gpu_growth_mult_str` every `{python} ScalingAsymmetry.gpu_period_years_str`, while high-quality labeled data grows far more slowly (@tbl-scaling-asymmetry).
...
[^fn-data-wall-scaling]: **Data Wall**\index{Data Wall!scaling constraint}: Unlike compute (which scales with capital expenditure) or algorithms (which improve through research), the stock of high-quality human-generated text grows slowly. Epoch AI projections estimated that high-quality language data could be exhausted within one to two decades at then-current consumption rates [@villalobos2022will], making data the first ML resource in this scaling story to face a supply constraint rather than merely an economic one. This constraint directly affects the total operations $(O)$ term: when quality data becomes scarce, additional compute yields diminishing returns regardless of hardware throughput.

Trace the trend line in @fig-running-out-of-human-data: several foundation-model training datasets have grown toward the estimated stock of high-quality public text, with projections suggesting that unrestricted reuse of public human-generated text faces a finite supply\index{Data Wall!data exhaustion projections}. The exact exhaustion timeline depends on corpus definition, licensing, filtering, repetition policy, and synthetic-data practice; the durable lesson is that accessible high-quality data can become the binding resource even when compute remains available.

::: {#fig-running-out-of-human-data fig-env="figure" fig-pos="htb" fig-cap="**Dataset Growth Approaching Limits**: Several foundation-model datasets have grown toward estimates of the total stock of high-quality public text. The projection should be read as a dated constraint scenario rather than a fixed forecast: data selection, synthetic generation, multimodal learning, licensing, and filtering policy all change the usable supply." fig-alt="Line chart of dataset size in tokens (10^9 to 10^15 on a log y-axis) vs. year (2012 to 2028). A blue line shows exponential data growth with markers for GPT-3, Chinchilla, Llama 2, and Llama 3. An orange band marks the high-quality public text stock."}
```{.tikz}
...
**Consequence**: In regimes where accelerator budgets outrun accessible high-quality data, the system becomes compute-rich and data-constrained.
...
2.  **Observation**: Unlike data engineering, which focuses on the cleanliness and consistency of data, data selection focuses on the informativeness and diversity of the samples.
3.  **Pitfall**: A frequent misconception is that more data is always better. In reality, it is the quality of the samples that matters: adding 10$\times$ more low-quality data may yield less accuracy than 1.1$\times$ carefully selected, high-quality data.

:::

To make this concrete, consider training a model in the GPT-2/Llama Lighthouse family from @sec-network-architectures-lighthouse-roster-model-biographies-a763, which spans the autoregressive large language model (LLM) family from GPT-2's `{python} ComputeDataGap.gpt2_params_b_str` parameters to Llama's `{python} ComputeDataGap.llama_min_n_str`-`{python} ComputeDataGap.llama_max_n_str` billion parameter range, here using a `{python} ComputeDataGap.llm_params_b_str`-parameter language model:
...
In this textbook, we adopt a data-selection *systems framing*\index{Data Selection!systems vs. ML framing} that asks instead how to reduce the total cost of achieving target performance across the entire ML lifecycle. The shift moves attention from accuracy curves to resource consumption, as @tbl-ml-vs-systems-framing illustrates.
...
The Information-Compute Ratio is not constant; it follows a law of diminishing returns. We define the **ICR Frontier**\index{Information-Compute Ratio!frontier} as the point where the marginal learning signal from additional data drops toward zero.
...
# │
# │ Imports: mlsysim.core.units (Models.Vision.ResNet50.inference_flops, GFLOPs, Datasets.ImageNet.training_examples)
...
The first stage of the pipeline acts entirely before training begins, removing low-value samples so that fewer of them ever reach the model. Static pruning\index{Static Pruning!pretraining filtration} and pretraining filtration reduce total computation without affecting, and sometimes improving, final model accuracy, all without modifying the training loop or model architecture.
...
**The physics of noise**: This estimate explains why one clean sample provides as much learning signal as `{python} QualityMultiplier.ratio_str` noisy ones.
...
1.  **Clean data**: Convergence rate is typically $\mathcal{O}(1/D)$. Halving the error requires 2$\times$ data.
2.  **Noisy data**: Convergence rate drops to $\mathcal{O}(1/\sqrt{D})$. Halving the error requires 4$\times$ data.
...
[^fn-coreset-geometry]: **Coreset (Core Set)**\index{Coreset!etymology}\index{Coreset!geometry origins}: The method's guarantee comes from computational geometry, where a small subset of points is proven to approximate the geometric properties of the full set within a $(1 + \varepsilon)$ factor. In machine learning, this preserves the statistical structure of the data, ensuring the loss landscape of a model trained on the coreset approximates the true loss landscape. This provable bound on error is the critical distinction from random downsampling, which offers no such guarantee: it is what lets a training run trade most of the dataset for a fraction of the compute without risking unbounded accuracy loss.
...
: **Coreset Selection Algorithm Comparison**: $D =$ dataset size, $K =$ coreset size. The fundamental trade-off is selection quality vs. computational cost: gradient-based methods (GraNd, EL2N, Forgetting) outperform geometry-based methods ($k$-Center, Herding) because they use training dynamics to identify decision-boundary samples, but this advantage requires proxy model training as an upfront investment. {#tbl-coreset-comparison .striped .hover tbl-colwidths="[10,20,20,25,25]"}
...
While coreset selection identifies which samples to keep based on their informativeness, a complementary approach targets what to remove: exact and near-duplicates. Deduplication\index{Deduplication!data selection optimization} can provide immediate efficiency gains, especially for byte-for-byte duplicates, and requires no model training. This makes it one of the most accessible optimizations in data selection, but near-duplicate thresholds must be validated so the pipeline does not remove useful distributional signal.
...
Near-duplicate detection addresses the more subtle problem of semantically redundant content that differs at the byte level. For text, MinHash\index{MinHash!near-duplicate detection}[^fn-minhash-dedup] with Locality-Sensitive Hashing\index{Locality-Sensitive Hashing!near-duplicate detection}[^fn-lsh-dedup] (LSH) approximates Jaccard similarity[^fn-jaccard-dedup] efficiently, detecting paraphrased or lightly edited content. The core idea is to create compact "fingerprints" of each document such that similar documents produce similar fingerprints with high probability, enabling fast approximate similarity detection without comparing every document pair.
...
For images, perceptual hashing\index{Perceptual Hashing!image deduplication} produces signatures robust to minor transformations like resizing and compression, identifying visually identical images stored in different formats. Embedding-based similarity\index{Deduplication!embedding-based} offers the highest-fidelity detection by computing dense representations (CLIP[^fn-clip-dedup] [@radford2021learning] for images, sentence transformers for text) and clustering similar items, though this approach incurs higher computational overhead.
...
Low-information filtering applies domain-specific heuristics to remove samples that lack sufficient signal for learning. For text corpora, this often means removing high-perplexity\index{Perplexity!filtering, low-information text removal} garbled text (perplexity is a language model's measure of how surprising a text is, so high values flag incoherent strings) and, in some pipelines, very low-perplexity boilerplate or repetitive text. For image datasets, filtering targets blurry, corrupted, or near-uniform samples that provide little visual information.
...
Curriculum learning delivers `{python} CurriculumBenchmarks.cifar10_speedup_str` fewer training epochs on CIFAR-10 and `{python} CurriculumBenchmarks.cifar100_speedup_str` on CIFAR-100 (@tbl-curriculum-benchmarks).
...
# │
# │ Imports: mlsysim.core.units (Datasets.CIFAR10), mlsysim.book (fmt)
...
**Context**: FixMatch\index{FixMatch} [@sohn2020fixmatch] combines pseudo-labeling with consistency regularization to achieve high label efficiency (@tbl-fixmatch-cifar10). With only `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_labels_str` labeled samples (`{python} FixmatchLabelEfficiency.labels_per_class_250_str` per class), FixMatch achieves `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_acc_str` accuracy, within `{python} FixmatchLabelEfficiency.acc_loss_str` points of full supervision using `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_efficiency_factor_mult_str` fewer labels. The technique works by generating pseudo-labels on weakly augmented unlabeled images (only when model confidence exceeds 0.95), then training to predict these labels on strongly augmented versions of the same images.

**Systems insight**: Semi-supervised learning trades labeled data for unlabeled data and compute. On CIFAR-10, training FixMatch requires ~`{python} FixmatchLabelEfficiency.fixmatch_compute_mult_str` more compute than supervised training (processing `{python} FixmatchLabelEfficiency.cifar10_full_labels_k_str` unlabeled samples per epoch). Two baselines are in play. Against full supervision, the `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_labels_str`-label result in @tbl-fixmatch-cifar10 shows `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_efficiency_factor_mult_str` fewer labels. The ROI calculation below instead compares against a `{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_labels_str`-label supervised baseline, where FixMatch uses `{python} FixmatchLabelEfficiency.roi_label_efficiency_mult_str` fewer purchased labels; the resulting total-cost trade-off assumes labels cost `{python} FixmatchLabelEfficiency.cost_label_str` each and GPU hours cost `{python} FixmatchLabelEfficiency.cost_gpu_hr_str`:
...
: **FixMatch Label Efficiency on CIFAR-10**: With `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_labels_str` labels (`{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_frac_pct_str` of the dataset), FixMatch achieves within `{python} FixmatchLabelEfficiency.acc_loss_str` points of full supervision, demonstrating `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_efficiency_factor_mult_str` label efficiency. {#tbl-fixmatch-cifar10 .striped .hover tbl-colwidths="[25,25,25,25]"}
...
: **Cost Amortization in Foundation Model Fine-Tuning**: Pretraining costs are paid once and amortized across all downstream tasks; fine-tuning costs scale with task count but remain small per task. This asymmetry explains why fine-tuning is attractive when representations transfer: the marginal cost of each new task can drop by 20$\times$ or more compared to training from scratch. {#tbl-cost-amortization .striped .hover}
...
# │
# │ Imports: mlsysim.book (fmt), mlsysim.core.units (MILLION, THOUSAND)
...
# │
# │ Imports: mlsysim.core.units (KB, MB, second); mlsysim.fmt (check, fmt)
...
Together, augmentation, generative synthesis, and distillation complete the third stage of our data selection pipeline. Where static pruning removes redundancy and dynamic selection focuses compute on high-value samples, synthetic generation fills gaps by creating samples that never existed.
...
The stacked bars in @fig-selection-inequality demonstrate this trade-off: efficient selection (center) saves 55 percent of total compute in the figure's normalized example, while expensive selection (right) consumes all the savings in overhead.\index{Selection Inequality!overhead trade-off}
...
# │
# │ Imports: mlsysim.core.units (THOUSAND); mlsysim.fmt (check, fmt)
...
Data echoing\index{Data Echoing!amortizing I/O costs}[^fn-data-echoing-pipeline] [@choi2020dataechoing] offers an elegant solution to this CPU-GPU imbalance. The technique reuses batches of data multiple times before fetching new samples, effectively trading sample diversity for GPU utilization. When the data pipeline (reading, decoding, augmenting) is slower than GPU processing, the GPU idles waiting for data.
...
from mlsysim.fmt import fmt_percent, fmt_ratio, check, fmt_time, fmt_rate, fmt, MarkdownStr
...
class DataEchoingRoi:
    ...
    epochs_images_math = MarkdownStr(f"${echo_epoch_count} \\times {imagenet_size.to(count).magnitude / 1e6:.2f}\\text{{M}}$")
```
...
class CostBreakdown:
    ...
    storage_str = MarkdownStr(f"{storage_size.to(GB).magnitude:.0f} GB × {storage_duration.to(ureg.month).magnitude:.0f} months")
    train_desc_str = MarkdownStr(f"{train_epoch_count} epochs × {train_gpu_count} A100s × {train_duration.to(hour).magnitude:.0f} h")
```
...
: **ImageNet-scale training cost breakdown**: Itemized acquisition, labeling, storage, and compute costs for an ImageNet-scale supervised training run, showing that data costs dominate compute by a wide margin in this regime. {#tbl-data-selection-imagenet-cost-breakdown tbl-colwidths="[44,23,33]"}
...
The challenge lies in quantifying both sides accurately. Deduplication offers the lowest-risk entry point and active learning the highest potential savings, with coreset selection and augmentation occupying the middle ground (@tbl-roi-profiles).
...
| **Per-run savings (20% less data)**       |               `{python} DeduplicationAmortization.savings_per_run_str` |
...
# │ Exports: timing strings and coreset scenario strings
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
**Scenario**: Select a `{python} DistributedOverheadCalc.coreset_pct_str` coreset from ImageNet (`{python} DistributedOverheadCalc.imagenet_images_m_str` images) using `{python} DistributedOverheadCalc.workers_str` A100 workers, one GPU per worker.
...
2. **Deduplication phase** (distributed): Coordinator builds FAISS index, workers query for near-duplicates → remove `{python} DistributedOverheadCalc.dedup_pct_str` duplicates
3. **Scoring phase** (parallel): Each worker computes EL2N scores on its deduplicated shard using proxy model
4. **Selection phase** (centralized): Coordinator collects top-`{python} DistributedOverheadCalc.local_top_pct_str` scores from each worker, re-ranks globally, selects final `{python} DistributedOverheadCalc.coreset_pct_str`
...
The pipeline in @fig-optimization-stack reveals why data selection occupies a strategic position: it sits at the head of the optimization stack. Reducing the dataset by 50 percent through intelligent selection does not just halve data processing time; it halves the training compute, which in turn can produce a model that needs less aggressive simplification and less machine overprovisioning. Each downstream stage inherits the efficiency gains or quality losses from upstream decisions.

The multiplicative effect means that *every FLOP saved in data processing is a FLOP that never needs to be executed, simplified, or accelerated.* Conversely, poor data selection that degrades model quality forces downstream stages to compensate, whether through longer training, less aggressive simplification, or over-provisioned hardware.
...
The cross-layer stack makes a measurement framework\index{Measurement Framework!data selection metrics} unavoidable: every data selection technique claims to improve efficiency, but only rigorous measurement separates real savings from shifted costs or hidden quality loss.

### Core metrics {#sec-data-selection-core-metrics-32e9}

The core metrics connect sample reduction to model quality instead of reporting accuracy alone. They measure learning per sample, performance-per-data\index{Performance-Per-Data!metric definition}, and compression at a target accuracy.
...
# │
# │ Imports: math; mlsysim.core.units (Models.Language.GPT3.training_tokens, Models.Language.GPT3.parameters,
...
The benchmarking chapter later generalizes these ideas into broader model-and-data evaluation protocols; here, the point is narrower: a curated dataset must be validated against the task distribution, not only against its reduction ratio.
...
**Failure mode**: The Berkeley team built new CIFAR-10 and ImageNet test sets using the original collection methodology. Models that performed extremely well on the standard test sets dropped on the newly collected examples.
...
Data selection principles apply to all five Lighthouse Models, though the dominant constraint determines the technique: compute-bound ResNet-50 benefits from coreset selection, memory-bound GPT-2/Llama from deduplication, and the tiny KWS model from augmentation and synthesis (@tbl-data-selection-lighthouse-priorities).
...
# │
# │ Imports: mlsysim.fmt, mlsysim.core.units (MILLION, THOUSAND)
...
class FpFallacyCalc:
    ...
    data_scale_mult_str = fmt_multiple(data_scale_value, precision=0, commas=False)
...
# │
# │ Imports: mlsysim.fmt, mlsysim.core.units (MILLION)
