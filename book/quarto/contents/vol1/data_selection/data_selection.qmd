...
::: {#fig-optimization-triad fig-env="figure" fig-pos="htb" fig-cap="**The Optimization Triad**: Machine learning performance relies on three pillars: Algorithms (models), Machine (hardware/software), and Data Selection. While algorithms and machines have traditionally received the most attention, optimizing data selection (Input Optimization) offers a third, independent lever for scaling performance." fig-alt="Triangular diagram with three nodes: Algorithms (Model) at top, Machine (Hardware) at bottom-left, Data Selection at bottom-right. Arrows connect all three; edge labels read Compute Bound and Sample Efficiency. ML Performance appears at the center."}
```{.tikz}
\scalebox{0.8}{%
...
::: {#fig-data-selection-pipeline fig-cap="**The Data Selection Pipeline**: A structured approach to increasing data value. Raw data is first pruned to remove redundancy (Static Pruning, pre-training), then dynamically chosen during training (Dynamic Selection), and finally augmented on demand (Synthetic Gen). Each stage increases the Information-Compute Ratio (ICR)." fig-alt="Flow diagram: Raw Data -> 1. Static Pruning (pre-training) -> 2. Dynamic Selection (during training) -> 3. Synthetic Gen (on-demand) -> High ICR Model, with arrows indicating the flow between stages."}
```{.tikz}
...
::: {#fig-optimization-stack fig-env="figure" fig-pos="htb" fig-cap="**The Optimization Stack**: The complete pipeline from raw data to deployed system, showing how optimizations at each stage propagate downstream. Data artifacts (green-bordered boxes) flow through processing stages (orange-bordered boxes). Optimizations early in the pipeline, particularly data selection, have multiplicative effects because they reduce the workload for all subsequent stages." fig-alt="Pipeline diagram with two rows. Top row: Raw Data -> Data Selection -> Curated Data -> Training -> Model. Bottom row: Model -> Compression -> Compact Model -> Hardware -> Deployed System. Green borders mark data artifacts; orange borders mark processing stages."}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
\tikzset{
