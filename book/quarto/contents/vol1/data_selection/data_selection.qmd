...
# │ Exports: gpu_growth_str, gpu_period_years_str, web_data_growth_str, etc.
# └─────────────────────────────────────────────────────────────────────────────
...
class ScalingAsymmetry:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpu_growth_str = fmt_multiple(gpu_growth_factor, precision=0, commas=False)
    ...
    web_data_growth_str = fmt_multiple(web_growth_factor, precision=0, commas=False)
    web_data_period_years_str = fmt_time(round(web_period_years), 'year', precision=0, commas=False, style='word')

    label_data_growth_str = fmt_multiple(label_growth_factor, precision=1, commas=False)
...
|:------------------------|---------------------------------------------------------------------------------------------------------------------:|:------------------------------------------------------|
| **GPU Compute**         |               ~`{python} ScalingAsymmetry.gpu_growth_str`$\times$ / `{python} ScalingAsymmetry.gpu_period_years_str` | Hardware vendors deliver reliable exponential gains   |
| **Training Data (Web)** |     ~`{python} ScalingAsymmetry.web_data_growth_str`$\times$ / `{python} ScalingAsymmetry.web_data_period_years_str` | High-quality web text is finite; much already scraped |
| **Labeled Data**        | ~`{python} ScalingAsymmetry.label_data_growth_str`$\times$ / `{python} ScalingAsymmetry.label_data_period_years_str` | Human annotation throughput is inherently bounded     |
...
class ComputeDataGap:
    ...
    corpus_reuse_ratio_str = fmt_multiple(corpus_reuse_ratio, precision=1, commas=False)
...
class IronLawSavings:
    ...
    combined_factor_str = fmt_multiple(total_speedup, precision=0, commas=False)
```
...
class IcrCoresetComparison:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    resnet50_fwd_gflops_str = fmt_flops(resnet50_fwd_flops, unit=GFLOPs, precision=1)
    resnet50_fwdbwd_gflops_str = fmt_flops(resnet50_fwdbwd_flops, unit=GFLOPs, precision=1)
    ...
    icr_ratio_str = fmt_multiple(icr_ratio_value, precision=1, commas=False)
...
- Model: ResNet-50 Lighthouse (~`{python} IcrCoresetComparison.resnet50_fwd_gflops_str` per forward pass, roughly `{python} IcrCoresetComparison.resnet50_fwdbwd_gflops_str` for a forward plus backward training step, depending on implementation)
- One epoch: `{python} IcrCoresetComparison.imagenet_size_str` $\times$ `{python} IcrCoresetComparison.resnet50_fwdbwd_gflops_str` = `{python} IcrCoresetComparison.full_epoch_flops_str`
...
- Compute: `{python} IcrCoresetComparison.coreset_size_str` $\times$ `{python} IcrCoresetComparison.resnet50_fwdbwd_gflops_str` = `{python} IcrCoresetComparison.coreset_flops_str`
...
class QualityMultiplier:
    ...
    ratio_str = fmt_multiple(ratio, precision=0, commas=False)
```
...
class ActiveLearningRoi:
    ...
    speedup_str = fmt_multiple(speedup_value, precision=0, commas=False)
    cost_saving_str = fmt_usd(cost_saving, precision=2, commas=False, scale="Million")
    over_budget_ratio_str = fmt_multiple(over_budget_ratio_value, precision=0, commas=False)
...
from mlsysim.fmt import check, fmt, fmt_time_range, MarkdownStr, fmt_usd_range
...
class ClinicalLabelingEconomics:
    ...
    full_weeks_range_str = MarkdownStr(f"{fmt(full_weeks_low, precision=1, commas=False)}–{fmt(full_weeks_high, precision=0, commas=False)}")
...
class FixmatchLabelEfficiency:
    ...
    cost_reduction_str = fmt_multiple(cost_reduction, precision=1, commas=False)
    acc_loss_str = fmt(acc_loss, precision=1, commas=False)
    fixmatch_compute_multiplier_str = fmt_multiple(fixmatch_compute_multiplier_value, precision=0, commas=False)
    ...
    cifar10_fixmatch_4k_efficiency_factor_str = fmt_multiple(cifar10_fixmatch_4k_eff, precision=1, commas=False)
    cifar10_fixmatch_250_efficiency_factor_str = fmt_multiple(cifar10_fixmatch_250_eff, precision=0, commas=False)
    cifar10_fixmatch_40_efficiency_factor_str = fmt_multiple(cifar10_fixmatch_40_eff, precision=0, commas=False)
...
| **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_labels_str` (8 percent)**    | FixMatch         |  `{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_acc_str` |  **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_efficiency_factor_str`$\times$ more efficient** |
| **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_labels_str` (0.5 percent)** | FixMatch         | `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_acc_str` | **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_efficiency_factor_str`$\times$ more efficient** |
| **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_40_labels_str` (0.08 percent)** | FixMatch         |  `{python} FixmatchLabelEfficiency.cifar10_fixmatch_40_acc_str` |  **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_40_efficiency_factor_str`$\times$ more efficient** |
...
# │ Exports: label_cost_reduction_factor_str, marginal_compute_reduction_str, etc.
# └─────────────────────────────────────────────────────────────────────────────
...
class FoundationCostAmortization:
    ...
    marginal_compute_reduction_str = fmt_multiple(marginal_compute_reduction, precision=0, commas=False)
    crossover_tasks_str = fmt(crossover_tasks_value, precision=1, commas=False)
    label_cost_reduction_factor_str = fmt_multiple(label_cost_reduction, precision=0, commas=True)
```
...
# │          .seed_cost_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, MarkdownStr, fmt_percent, fmt_usd_range
...
class KwsDataSelectionCalc:
    ...
    augmentation_multiplier_str = fmt_multiple(augmentation_multiplier, precision=0, commas=False)
...
# │          .hdd_random_mb_s_str, .hdd_penalty_factor_str,
# │          .sata_seq_mb_s_str, .sata_iops_str, .sata_random_mb_s_str,
# │          .sata_penalty_factor_str, .nvme_seq_mb_s_str, .nvme_iops_str,
# │          .nvme_random_mb_s_str, .nvme_penalty_factor_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Systems
from mlsysim.core.constants import KB, MB, second
from mlsysim.fmt import fmt_int, check, fmt, fmt_count, fmt_bandwidth
...
class RandomAccessPenalty:
    ...
    # OUTPUT
    hdd_seq_mb_s_str = fmt_bandwidth(hdd_seq_q, unit=MB/second, precision=0, commas=False)
    hdd_iops_str = fmt(hdd_iops, precision=0, commas=False)
    hdd_random_mb_s_str = fmt_bandwidth(hdd_random_q, unit=MB/second, precision=1, commas=False)
    hdd_penalty_factor_str = fmt_multiple(round(hdd_penalty), precision=0, commas=False)

    sata_seq_mb_s_str = fmt_bandwidth(sata_seq_q, unit=MB/second, precision=0, commas=False)
    sata_iops_str = fmt_count(sata_iops, scale='K', commas=False)
    sata_random_mb_s_str = fmt_bandwidth(sata_random_q, unit=MB/second, precision=0, commas=False)
    sata_penalty_factor_str = fmt_multiple(sata_penalty, precision=1, commas=False)

    nvme_seq_mb_s_str = fmt_bandwidth(nvme_seq_q, unit=MB/second, precision=0, commas=True)
    nvme_iops_str = fmt_count(nvme_iops, scale='K', commas=False)
    nvme_random_mb_s_str = fmt_bandwidth(nvme_random_q, unit=MB/second, precision=0, commas=True)
    nvme_penalty_factor_str = fmt_multiple(nvme_penalty, precision=2, commas=False)
```

| **Storage Tier** |             **Sequential Throughput**             |             **Random I/O (IOPS)**             |            **Random Throughput (approx)**            |                       **Random Penalty**                       |
|:-----------------|:-------------------------------------------------:|:---------------------------------------------:|:----------------------------------------------------:|:--------------------------------------------------------------:|
| **HDD (7.2k)**   |  ~`{python} RandomAccessPenalty.hdd_seq_mb_s_str` |  ~`{python} RandomAccessPenalty.hdd_iops_str` | ~`{python} RandomAccessPenalty.hdd_random_mb_s_str`  | `{python} RandomAccessPenalty.hdd_penalty_factor_str`$\times$  |
| **SATA SSD**     | ~`{python} RandomAccessPenalty.sata_seq_mb_s_str` | ~`{python} RandomAccessPenalty.sata_iops_str` | ~`{python} RandomAccessPenalty.sata_random_mb_s_str` | `{python} RandomAccessPenalty.sata_penalty_factor_str`$\times$ |
| **NVMe SSD**     | ~`{python} RandomAccessPenalty.nvme_seq_mb_s_str` | ~`{python} RandomAccessPenalty.nvme_iops_str` | ~`{python} RandomAccessPenalty.nvme_random_mb_s_str` | `{python} RandomAccessPenalty.nvme_penalty_factor_str`$\times$ |
...
class DataEchoingRoi:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pipeline_throughput_str = fmt_rate(pipeline_throughput, "images/s", precision=0, commas=False)
    gpu_throughput_str = fmt_rate(gpu_throughput, "images/s", precision=0, commas=False)
    ...
    effective_throughput_str = fmt_rate(echo_throughput, "images/s", precision=0, commas=False)
```
...
- Data pipeline throughput: `{python} DataEchoingRoi.pipeline_throughput_str` (reading, decoding, augmenting on CPU)
- GPU training throughput: `{python} DataEchoingRoi.gpu_throughput_str` (forward + backward pass)
...
- Effective throughput: `{python} DataEchoingRoi.pipeline_throughput_str` (limited by data pipeline)
...
- Effective throughput: `{python} DataEchoingRoi.effective_throughput_str` (still below GPU capacity)
...
class FpFallacyCalc:
    ...
    curated_cost_ratio_str = fmt_multiple(curated_cost_ratio_value, precision=0, commas=False)
...
class FpPitfallCalc:
    ...
    selection_overhead_ratio_str = fmt_multiple(selection_overhead_ratio_value, precision=0, commas=False)
