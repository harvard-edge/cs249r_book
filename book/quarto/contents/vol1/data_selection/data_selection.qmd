...
For decades, the dominant strategy was simple. More data produced better models. Scaling laws\index{Scaling Laws!data-compute asymmetry}[^fn-scaling-laws-origin] [@kaplan2020scaling; @hoffmann2022chinchilla] confirmed that model performance improves predictably with dataset size, and teams responded by collecting and generating more examples. A critical asymmetry has since emerged. Accelerator fleets can expand usable compute faster than the supply of novel, high-quality human-generated text and images. Much of the easily accessible public web has already been incorporated into large training corpora, and expert labeling capacity grows slowly. This asymmetry is the **Data Wall**\index{Data Wall!definition}[^fn-data-wall-scaling], and it has shifted the priority from collecting more data to extracting more value from existing data. The sections that follow develop the selection methods and cost models needed to make that choice.
...
On CIFAR-10, Paul et al. report that EL2N pruning removed half of the training data without reducing accuracy [@paul2021deep]. On ImageNet-1K, Sorscher et al. report that a self-supervised prototype metric discarded 20 percent of the data without sacrificing performance [@ganguli2022]. The pattern extends to language modeling, where web-scraped corpora like The Pile[^fn-the-pile-diversity] and C4[^fn-c4-filtering] contain enough duplicate and templated content to make deduplication a systems issue. In datasets studied by Lee et al., approximate near-duplicate removal affected 3.04 percent of C4 and 13.63 percent of RealNews, and deduplicated training reduced memorization while preserving or improving perplexity [@lee2022].
...
::: {#fig-coreset-selection fig-env="figure" fig-pos="htb" fig-cap="**Illustrative Uncertainty-Based Selection**: Random sampling appears on the left. The illustrative strategy on the right prioritizes samples near the current decision boundary; other coreset objectives may prioritize geometric coverage instead." fig-alt="Two illustrative scatter plots with a diagonal decision boundary. The left plot shows randomly selected points, while the right highlights points near the boundary as selected."}
...
**Insight**: Random sampling may miss these rare failures. A hypothetical deployment can use active learning to send low-confidence predictions to human reviewers. If the model sees a statue and predicts "Person (51 percent)", that sample can be flagged for labeling.

**Systems lesson**: Active learning can focus labeling on uncertain or rare cases, but the gain over random collection must be measured for the deployment distribution.
...
[^fn-back-translation-nlp]: **Back-Translation**: In the method described by Sennrich et al., target-language monolingual text is translated into the source language, and the synthetic source is paired with the unchanged target for machine-translation training [@sennrich2016backtranslation]. The systems trade-off is latency because each synthetic source requires translation-model inference. Pipelines can precompute these examples offline rather than generating them in the data loader. \index{Back-Translation!pipeline latency}

Rather than hand-designing these augmentation policies, AutoAugment[^fn-autoaugment-search] uses reinforcement learning to discover augmentation strategies for specific datasets, while RandAugment[^fn-randaugment-simplicity] simplifies this by sampling from a fixed set of transformations. The MobileNetV2 lighthouse illustrates why augmentation policy must be validated jointly with the model and deployment task.

[^fn-autoaugment-search]: **AutoAugment**: Treats augmentation policy design as a reinforcement learning search problem: a controller selects operations (rotate, translate, shear, equalize), their magnitudes, and application probabilities to maximize validation accuracy [@cubuk2019]. The original ImageNet search used approximately 15,000 GPU-hours, motivating later methods that reduce the search space. \index{AutoAugment!search cost}
...
**Context**: The keyword spotting (KWS) DS-CNN lighthouse from @sec-network-architectures-lighthouse-roster-model-biographies-a763 represents the extreme end of data selection challenges. Consider an illustrative wake-word detector ("Hey Device") for a microcontroller with 256 KB SRAM (see @sec-ml-systems-tinyml-ubiquitous-sensing-scale-a67b for hardware constraints), a ~200 KB quantized-weight budget before runtime buffers, and a target of `{python} KwsDataSelectionCalc.target_samples_plus_str` labeled audio samples.

**Insight**:

- Recording `{python} KwsDataSelectionCalc.target_samples_str` real utterances may require many speakers for diversity
- At the scenario's `{python} KwsDataSelectionCalc.cost_per_sample_range_str`, recording costs `{python} KwsDataSelectionCalc.full_cost_range_str` total
...
**Systems insight**: When the target model is tiny, the data selection challenge can shift from reducing a large corpus to creating sufficient task coverage. In this scenario, `{python} KwsDataSelectionCalc.seed_samples_str` real recordings produce `{python} KwsDataSelectionCalc.target_samples_str`+ training examples after augmentation and noise injection. The real recording count is `{python} KwsDataSelectionCalc.seed_cost_pct_str` of the target example count, but deployment robustness still requires validation on real audio.
...
Equivalently, selection must satisfy $T_{\text{selection}} < T_{\text{train}}(D_{\text{total}}) - T_{\text{train}}(D_{\text{subset}})$. The allowable overhead therefore depends on how much training work the subset actually saves; no fixed percentage applies across workloads.
...
**Systems insight**: Selection time must remain smaller than the training time saved by using the subset. A lower overhead leaves more margin, but the selection inequality rather than a universal percentage determines positive ROI.
...
Echoing can introduce correlations when the same example appears within or across nearby batches, so implementations must preserve adequate shuffling and validate the target metric. Choi et al. did not observe a negative batch-normalization interaction in their ResNet-50 or SSD experiments, but that result does not guarantee the same behavior for every workload [@choi2020dataechoing].
...
: **Illustrative ImageNet-Scale Cost Breakdown**: Itemized acquisition, labeling, storage, and compute costs under the stated scenario assumptions. Data costs dominate this particular calculation because licensing and annotation are included. {#tbl-data-selection-imagenet-cost-breakdown tbl-colwidths="[44,23,33]"}

```{=latex}
\endgroup
```

**Systems insight**: Acquisition and labeling can exceed compute cost in supervised vision workloads, as they do under these assumptions. A complete ROI calculation must include both rather than assuming either category dominates.
...
: **ROI Profiles for Data Selection Techniques**: The table identifies costs and possible savings. No technique guarantees positive ROI; each must satisfy the selection inequality under measured workload conditions. {#tbl-roi-profiles .striped .hover tbl-colwidths="[18,40,44]"}
...
**Illustrative result** (modeled for an 8$\times$ A100 cluster):
...
**Systems insight**: In this illustrative timing scenario, the `{python} DistributedOverheadCalc.t_total_overhead_min_str` selection overhead pays for itself once full training exceeds about 1.25 hours under the assumed 10$\times$ speedup. The same arithmetic must be recomputed from measured selection and training times for the target workload.
...
[^fn-chinchilla-scaling]: **Chinchilla**: Chinchilla is a 70-billion-parameter language model trained on 1.4 trillion tokens. In the experiments reported by Hoffmann et al. [@hoffmann2022chinchilla], it outperformed the larger GPT-3 on most evaluated tasks under a similar training-compute budget. The fitted compute-optimal allocation scaled parameters and tokens at roughly equal rates within the study's experimental regime; it is not a universal prescription for every architecture, dataset, or training recipe. \index{Chinchilla!compute-optimal scaling}
...
::: {#cs-data-selection-benchmark-replication-gap .callout-case-study title="The benchmark replication gap"}
...
A sophisticated coreset algorithm requiring `{python} FpPitfallCalc.selection_time_bad_hours_str` to select samples for a `{python} FpPitfallCalc.training_time_hours_str` training run has `{python} FpPitfallCalc.selection_overhead_ratio_mult_str` overhead, yielding negative ROI in this scenario. @Sec-data-selection-selection-bottleneck-4d00 establishes the Selection Inequality: $T_{\text{selection}} + T_{\text{train}}(\text{subset}) < T_{\text{train}}(\text{full})$. Use lightweight proxy models or cached embeddings when they preserve selection quality. In the illustrative comparison, proxy-based EL2N scoring completes in `{python} FpPitfallCalc.selection_time_good_min_str` (`{python} FpPitfallCalc.selection_overhead_good_pct_str` overhead) and satisfies the inequality.
