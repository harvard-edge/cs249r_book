...
- Select data-selection investments using return on investment (ROI), amortization, and compute-optimal frontier diagnostics
...
**EL2N coreset** (a training-dynamics score developed in @sec-data-selection-coreset-selection-algorithms-2c74; `{python} IcrCoresetComparison.coreset_pct_str` of data):
...
Each algorithm in @tbl-coreset-comparison occupies a different point in the ICR framework's compute-vs.-information trade-off. @Fig-coreset-selection illustrates one uncertainty-based strategy rather than every coreset method. Random sampling appears on the left, while the right panel concentrates the selection budget near the illustrated decision boundary. Geometry-based methods may instead prioritize coverage across the embedding space.
...
Near-duplicate detection addresses content that differs at the byte level while retaining substantial token- or character-shingle overlap. For text, MinHash\index{MinHash!near-duplicate detection}[^fn-minhash-jaccard-dedup] with locality-sensitive hashing (LSH)\index{Locality-Sensitive Hashing!near-duplicate detection}[^fn-lsh-hashing-dedup] approximates Jaccard similarity[^fn-jaccard-similarity-dedup] efficiently. It can detect lightly edited content with high overlap, but it does not establish semantic paraphrase equivalence.

[^fn-jaccard-similarity-dedup]: **Jaccard Similarity**\index{Jaccard Similarity!deduplication}: Defined as $|A \cap B| / |A \cup B|$, ranging from 0 (disjoint) to 1 (identical). For deduplication, the metric compares sets of shingles across documents of different lengths. The threshold must be calibrated for the shingle definition, corpus, and downstream quality because no universal cutoff separates duplicates from legitimately related documents.
...
[^fn-randaugment-simple-search]: **RandAugment**\index{RandAugment!hyperparameter simplicity}: Collapses AutoAugment's policy search to two hyperparameters: transformation count $N$ and shared magnitude $M$ [@cubuk2020]. That small search space matches or exceeds AutoAugment on the reported benchmarks at far lower search cost, making it practical when 15,000 GPU-hours of policy search cannot be justified.
...
[^fn-domain-gap-shift]: **Domain Gap**\index{Domain Gap!training-serving skew}: The statistical divergence between synthetic and real data distributions, measurable with metrics such as maximum mean discrepancy or Fréchet inception distance. For ML systems, the gap becomes training-serving skew: a simulator-trained model can validate well on synthetic data while failing silently on real deployment data.
...
Systems can mitigate this penalty with proxy models, cached embeddings, and indexed retrieval. A smaller proxy can reduce scoring cost only if its ranking transfers to the target model. Lower-precision inference may reduce that cost further when the runtime, model, and required ranking fidelity support it. Embedding indices such as FAISS\index{Vector Index!embedding-based selection}[^fn-faiss-similarity-search] replace some full scans with exact or approximate retrieval whose cost and recall depend on the index configuration. These approaches decouple selection from target-model training so each can be measured independently.
...
Echoing can introduce correlations when the same example appears within or across nearby batches, so implementations must preserve adequate shuffling and validate the target metric. @choi2020dataechoing did not observe a negative batch-normalization interaction in their ResNet-50 or Single Shot MultiBox Detector experiments, but that result does not guarantee the same behavior for every workload.
...
Rather than comparing at a single point, the **Area Under the Learning Curve (AULC)**\index{Area Under the Learning Curve!definition} integrates performance across all dataset sizes:
$$
...
For coreset methods, the **Data Compression Ratio (DCR)**\index{Data Compression Ratio!definition} measures how much data reduction is achieved at a target accuracy:
$$
...
Within the fitted Chinchilla regime, compute-optimal model parameters and training tokens grew at roughly equal rates.[^fn-chinchilla-token-ratio] Combining that empirical fit with the dense-transformer compute approximation, we derive $D_{\text{opt}} \propto \sqrt{C}$, so doubling compute corresponds to about $\sqrt{2} - 1$, or `{python} ChinchillaDiagnostic.doubling_token_growth_pct_str`, more tokens under those assumptions. The commonly quoted `{python} ChinchillaDiagnostic.tokens_per_param_opt_str` tokens per parameter is a useful reference point from that study, not a universal optimum.
