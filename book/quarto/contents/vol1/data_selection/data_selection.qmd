...
class ScalingAsymmetry:
    ...
    # Custom "years" suffix → MarkdownStr per recipe Rule 1 escape-hatch
...
class ComputeDataGap:
    ...
    cluster_cost = (
        ...
        * c.DAYS_PER_MONTH
        * c.HOURS_PER_DAY
        * Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as(c.USD / c.hour)
    )
    ...
    tokens_capacity_str = fmt(tokens_capacity/c.TRILLION, precision=1, commas=False) + "T"
    filtered_corpus_tokens_str = fmt(filtered_corpus_tokens/c.TRILLION, precision=0, commas=False) + "T"
    public_text_stock_str = fmt(public_text_stock/c.TRILLION, precision=0, commas=False) + "T"
...
class IcrCoresetComparison:
    ...
    # Custom "K" suffix on a divided count → MarkdownStr escape-hatch
...
class FixmatchLabelEfficiency:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    supervised_label_cost_str = fmt(supervised_label_cost, precision=0, commas=True, prefix="$")
    supervised_total_str = fmt(supervised_total, precision=0, commas=True, prefix="$")
    fixmatch_label_cost_str = fmt(fixmatch_label_cost, precision=0, commas=True, prefix="$")
    fixmatch_total_str = fmt(fixmatch_total, precision=0, commas=True, prefix="$")
    ...
    supervised_compute_cost_str = fmt(supervised_compute_cost, precision=0, commas=False, prefix="$")
    fixmatch_compute_cost_str = fmt(fixmatch_compute_cost, precision=0, commas=False, prefix="$")
```
...
class FoundationCostAmortization:
    ...
    cost_per_label_str = fmt(cost_per_label, precision=0, prefix="$", commas=False)
    labels_per_task_scratch_str = fmt(labels_per_task_scratch, precision=0, commas=True)
    # Currency prefix + M/K suffix → MarkdownStr per recipe Rule 1 escape-hatch
    label_cost_scratch_total_str = fmt(label_cost_scratch_total / MILLION, precision=0, commas=False, prefix="$", suffix="M")
    ...
    label_cost_finetune_total_str = fmt(label_cost_finetune_total / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    ...
    marginal_compute_reduction_str = fmt(marginal_compute_reduction, precision=0, commas=False, suffix="×")
    crossover_tasks_str = fmt(crossover_tasks_value, precision=1, commas=False)
    label_cost_drop_str = fmt(label_cost_reduction, precision=0, commas=True, suffix="×")
```
...
The return on investment is substantial for labeling, marginal per-task compute, and time to deployment: labeling costs drop by `{python} FoundationCostAmortization.label_cost_drop_str` (from `{python} FoundationCostAmortization.label_cost_scratch_total_str` to `{python} FoundationCostAmortization.label_cost_finetune_total_str`), per-task marginal compute decreases by `{python} FoundationCostAmortization.marginal_compute_reduction_str`, and time to deployment accelerates by 20--50× per task. Total compute becomes favorable only after the pretraining cost is amortized across enough downstream tasks.
...
class RandomAccessPenalty:
    ...
    sata_iops_str = fmt(sata_iops / THOUSAND, precision=0, commas=False) + "k"
    ...
    nvme_iops_str = fmt(nvme_iops / THOUSAND, precision=0, commas=False) + "k"
...
class CostBreakdown:
    ...
    c_raw_str = fmt(c_raw_value, precision=0, prefix="$")
    c_label_str = fmt(c_label_value, precision=0, prefix="$")
    c_store_str = fmt(c_store_value, precision=0, commas=False, prefix="$")
    c_train_str = fmt(c_train_value, precision=0, prefix="$")
    c_total_str = fmt(c_total_value, precision=0, prefix="$")
...
class FpFallacyCalc:
    ...
    curated_size_str = fmt(curated_size_value / THOUSAND, precision=0) + "K"
