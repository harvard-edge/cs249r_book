...
[^fn-the-pile-diversity]: **The Pile**: An 825 GB English text corpus aggregating 22 sub-datasets (PubMed, ArXiv, GitHub, Project Gutenberg, CommonCrawl, Stack Exchange, Wikipedia, USPTO patents). Its design philosophy prioritizes *diversity* over scale, combining curated sources covering different domains and writing styles rather than simply crawling more web pages. For data selection, The Pile established a key systems insight: duplicate-heavy web scrapes produce diminishing returns per token, while diverse multi-source curation yields higher ICR at the same dataset size. \index{The Pile!data diversity}
...
[^fn-randaugment-simplicity]: **RandAugment**: Achieves this simplification by replacing AutoAugment's reinforcement learning search with random sampling from a fixed set of transformations. This proves an expensive, dataset-specific search is often unnecessary, as the randomized approach reduces the policy discovery cost from 15,000 GPU-hours to a simple grid search while maintaining equivalent model performance. \index{RandAugment!hyperparameter simplicity}
...
[^fn-data-echoing-pipeline]: **Data Echoing**: This technique resolves the CPU-GPU imbalance by having the GPU re-process the same batch of data 2-4 times, converting idle cycles into useful training. This accepts a minor reduction in sample diversity to prevent the GPU from idling, which wastes 100% of available compute cycles when the data pipeline is the bottleneck. \index{Data Echoing!GPU utilization}
