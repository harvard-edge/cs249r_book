...
Each algorithm in @tbl-coreset-comparison occupies a different point in the ICR framework's compute-vs.-information trade-off, determining how the selection budget is spent to maximize learning signal per FLOP. @Fig-coreset-selection makes the core insight behind coreset methods concrete. Compare the two panels: random sampling (left) selects points uniformly across the feature space, capturing many samples deep within class regions where the model is already confident. Coreset selection (right) concentrates the selection budget on samples near the decision boundary, the dashed diagonal that separates the two classes, where the model's predictions are most uncertain. The yellow band is the uncertainty margin straddling that boundary, and these boundary samples are precisely where additional training provides the most learning signal.
