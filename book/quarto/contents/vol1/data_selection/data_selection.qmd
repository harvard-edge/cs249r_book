...
class ScalingAsymmetry:
    ...
    gpu_period_str = fmt_time(round(gpu_period_years), 'year', precision=0, commas=False, style='word')

    web_data_growth_str = fmt(web_growth_factor, precision=0, commas=False)
    web_data_period_str = fmt_time(round(web_period_years), 'year', precision=0, commas=False, style='word')

    label_data_growth_str = fmt(label_growth_factor, precision=1, commas=False)
    label_data_period_str = fmt_time(round(label_period_years), 'year', precision=0, commas=False, style='word')
```
...
class IcrCoresetComparison:
    ...
    acc_gain_random_str = fmt_pp(acc_gain_random_value, precision=0)
    acc_gain_coreset_str = fmt_pp(acc_gain_coreset_value, precision=1)
    acc_diff_str = fmt_pp(acc_diff_value, precision=1)
...
class CoresetPractice:
    ...
    n_train_images_str = fmt_count(
        n_train_images_value, scale="M", precision=0, scale_style="word"
    )
...
class FoundationCostAmortization:
    ...
    cost_scratch_per_task_str = fmt_count(cost_scratch_per_task_value, precision=0, commas=True, label="GPU-hour", plural_label="GPU-hours")
    cost_scratch_total_str = fmt_count(cost_scratch_total_value, precision=0, commas=True, label="GPU-hour", plural_label="GPU-hours")
    cost_foundation_total_str = fmt_count(cost_foundation_total_value, precision=0, commas=True, label="GPU-hour", plural_label="GPU-hours")
    ...
    cost_finetune_value_str = fmt_count(cost_finetune_value, precision=0, commas=True, label="GPU-hour", plural_label="GPU-hours")
    cost_pretrain_value_str = fmt_count(cost_pretrain_value, precision=0, commas=True, label="GPU-hour", plural_label="GPU-hours")
...
class SelectionBottleneckAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dataset_size_str = fmt_count(dataset_size_m * MILLION, scale='M', precision=0, commas=False)
    scoring_time_str = fmt_time(scoring_time_hrs, 'hour', precision=1, commas=False, style='word')
...
class SelectionInequalityCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    score_a_str = fmt_time(score_a_sec_value, 'second', precision=0, commas=True)
    score_a_hrs_str = fmt_time(score_a_sec_value / SEC_PER_HOUR, 'hour', precision=1, commas=False)
    train_a_str = fmt_time(train_a_sec_value, 'second', precision=0, commas=True)
    train_a_hrs_str = fmt_time(round(train_a_sec_value / SEC_PER_HOUR), 'hour', precision=0, commas=False)
    total_a_hrs_str = fmt_time(round(total_a_hrs_value), 'hour', precision=0, commas=False)
    score_b_str = fmt_time(score_b_sec_value, 'second', precision=0, commas=True)
    score_b_hrs_str = fmt_time(score_b_hrs_value, 'hour', precision=1, commas=False)
    total_b_hrs_str = fmt_time(round(total_b_hrs_value), 'hour', precision=0, commas=False)
    baseline_str = fmt_time(baseline_sec_value, 'second', precision=0, commas=True)
    baseline_hrs_str = fmt_time(round(baseline_hrs_value), 'hour', precision=0, commas=False)
    savings_a_str = fmt_time(round(savings_a_hrs_value), 'hour', precision=0, commas=False)
    savings_a_pct_str = fmt_percent(savings_a_pct_value/100, precision=0, commas=False, style='symbol')
    savings_b_str = fmt_time(round(savings_b_hrs_value), 'hour', precision=0, commas=False)
    savings_b_pct_str = fmt_percent(savings_b_pct_value/100, precision=1, commas=False, style='symbol')
    b_beats_a_str = fmt_time(round(b_beats_a_hrs_value), 'hour', precision=0, commas=False)
    trap_total_str = fmt_time(round(trap_total_hrs_value), 'hour', precision=0, commas=False)
    ...
    n_coreset_str = fmt_count(n_coreset_value, scale='K', precision=0, commas=False)
    n_epochs_str = fmt(n_epochs_value, precision=0, commas=False)
    coreset_pct_str = fmt_percent(coreset_fraction_value, precision=0, commas=False, style='prose')
    resnet50_time_per_image_str = fmt_time(resnet50_time_per_image_value, 'second', precision=2, commas=False)
    resnet18_time_per_image_str = fmt_time(resnet18_time_per_image_value, 'second', precision=3, commas=False)
    trap_sel_hrs_str = fmt_time(trap_sel_hrs_value, 'hour', precision=0, commas=False)
```
...
from mlsysim.fmt import fmt_int,  check, fmt, fmt_count, fmt_qty

class RandomAccessPenalty:
    # LOAD
    random_read_q = 4 * KB
    random_read_kb = random_read_q.m_as(KB)
    hdd_seq_q = 150 * MB/second
    hdd_seq_mb_s = hdd_seq_q.m_as(MB/second)
    hdd_iops = 80
    sata_seq_q = 550 * MB/second
    sata_seq_mb_s = sata_seq_q.m_as(MB/second)
    sata_iops = 10_000
    nvme_seq_q = 3_500 * MB/second
    nvme_seq_mb_s = nvme_seq_q.m_as(MB/second)
    nvme_iops = 500_000
    ...
    hdd_random_mb_s = hdd_random_q.m_as(MB/second)
    sata_random_mb_s = sata_random_q.m_as(MB/second)
    nvme_random_mb_s = nvme_random_q.m_as(MB/second)
    ...
    # OUTPUT
    hdd_seq_mb_s_str = fmt_qty(hdd_seq_q, MB/second, precision=0, commas=False, unit_label="MB")
    hdd_iops_str = fmt(hdd_iops, precision=0, commas=False)
    hdd_random_mb_s_str = fmt_qty(hdd_random_q, MB/second, precision=1, commas=False, unit_label="MB")
    hdd_penalty_str = fmt_int(hdd_penalty, -2, commas=False)

    sata_seq_mb_s_str = fmt_qty(sata_seq_q, MB/second, precision=0, commas=False, unit_label="MB")
    sata_iops_str = fmt_count(sata_iops, scale='K', precision=0, commas=False)
    sata_random_mb_s_str = fmt_qty(sata_random_q, MB/second, precision=0, commas=False, unit_label="MB")
    sata_penalty_str = fmt(sata_penalty, precision=1, commas=False)

    nvme_seq_mb_s_str = fmt_qty(nvme_seq_q, MB/second, precision=0, commas=True, unit_label="MB")
    nvme_iops_str = fmt_count(nvme_iops, scale='K', precision=0, commas=False)
    nvme_random_mb_s_str = fmt_qty(nvme_random_q, MB/second, precision=0, commas=True, unit_label="MB")
...
class DataEchoingRoi:
    ...
    no_echo_sec_str = fmt_time(no_echo_sec_value, 'second', precision=0, commas=True, style='word')
    no_echo_hrs_str = fmt_time(no_echo_hrs_value, 'hour', precision=1, commas=False, style='word')
    gpu_util_str = fmt_percent(round(gpu_util_no_echo_value)/100, precision=0, commas=False, style='symbol')
    echo_sec_str = fmt_time(echo_sec_value, 'second', precision=0, commas=True, style='word')
    echo_hrs_str = fmt_time(echo_hrs_value, 'hour', precision=1, commas=False, style='word')
...
class DistributedOverheadCalc:
    ...
    t_embed_str = fmt_time(t_embed_value, 'minute', precision=0, commas=False, style='word')
    t_dedup_str = fmt_time(t_dedup_value, 'minute', precision=0, commas=False, style='word')
    t_score_str = fmt_time(t_score_value, 'minute', precision=0, commas=False, style='word')
    t_select_str = fmt_time(t_select_value, 'minute', precision=0, commas=False, style='word')
    t_total_overhead_str = fmt_time(t_total_overhead_value, 'minute', precision=0, commas=False, style='word')
```
...
class FpFallacyCalc:
    ...
    acc_gain_10x_str = fmt_pp(acc_gain_10x_value, precision=0)
...
class FpPitfallCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    selection_time_bad_str = fmt_time(selection_time_bad_value, 'hour', precision=0, commas=False, style='word')
    training_time_str = fmt_time(
        training_time_value,
        "hour",
        precision=0,
        commas=False,
    ...
    full_training_time_str = fmt_time(full_training_time_value, 'hour', precision=0, commas=False, style='word')
    selection_overhead_ratio_str = fmt(selection_overhead_ratio_value, precision=0, commas=False)
    selection_time_good_str = fmt_time(selection_time_good_value * 60, 'minute', precision=0, commas=False, style='word')
