...
Practitioners may view data selection as relevant only for TinyML or budget-limited teams. It applies wherever high-quality examples, not raw volume, constrain learning. A `{python} FpFallacyCalc.efficiency_gain_pct_str` efficiency gain on a `{python} FpFallacyCalc.training_run_cost_str` training run saves `{python} FpFallacyCalc.savings_str` under the scenario assumptions. The data wall (@fig-running-out-of-human-data) can become especially visible at large scale, when compute capacity outruns accessible, legally usable, target-relevant data. @Sec-data-selection-amortization-across-training-runs-f6b8 shows why reuse can make selection infrastructure economical even for well-funded teams.
...
Aggressive selection without group constraints can remove rare classes because an average-loss objective gives them little weight. In a `{python} FpPitfallCalc.total_samples_str` dataset with `{python} FpPitfallCalc.rare_class_pct_str` rare-class samples (`{python} FpPitfallCalc.rare_class_count_str` examples), an unstratified `{python} FpPitfallCalc.fp_coreset_pct_str` subset retains only `{python} FpPitfallCalc.expected_rare_in_coreset_str` such examples in expectation, below the scenario's `{python} FpPitfallCalc.min_samples_threshold_str`-sample target. **Importance Sampling**\index{Importance Sampling!gradient variance} can reweight rare samples, but large weights can inflate gradient variance. @Sec-data-selection-coreset-selection-algorithms-2c74 therefore recommends minimum representation by class or deployment-relevant slice before optimizing the remaining selection budget.
