...
: **Scaling Asymmetry in ML Resources**: In the illustrated scaling regime, compute grows faster than high-quality data supply, creating a compute-to-data imbalance that makes data selection valuable. {#tbl-scaling-asymmetry .striped .hover tbl-colwidths="[20,20,60]" tbl-colwidths="[20,20,60]"}
...
: **Coreset Selection Algorithm Comparison**: $D =$ dataset size, $K =$ coreset size. Geometry-based and training-dynamics methods optimize different proxies, so their quality ranking depends on the dataset, representation, target model, and scoring budget. {#tbl-coreset-comparison .striped .hover tbl-colwidths="[10,20,20,25,25]" tbl-colwidths="[10,20,20,25,25]"}
...
: **Illustrative Curriculum Epoch Reductions**: The table applies one target-accuracy rule to hypothetical baseline and curriculum epoch counts. It demonstrates the arithmetic but does not establish a ranking among datasets or methods. {#tbl-curriculum-benchmarks .striped .hover tbl-colwidths="[20,20,20,20,20]" tbl-colwidths="[20,20,20,20,20]"}
...
class ActiveLearningRoi:
    ...
    baseline_calc_math = fmt_math(f"{budget_str} \\div {cost_per_label_str} = {n_random_str}")
    cost_active_calc_math = fmt_math(f"{n_active_str} \\times {cost_per_label_str} = {cost_active_str}")
...
: **Self-Supervised Pretext Tasks by Modality**: Each task extracts supervision from data structure rather than human labels, enabling pretraining on large-scale unlabeled corpora. {#tbl-self-supervised-tasks .striped .hover tbl-colwidths="[30,30,40]" tbl-colwidths="[30,30,40]"}
...
: **Three-Stage Data Selection Pipeline**: The ranges illustrate possible outcomes rather than portable expectations. Actual gains depend on the dataset, model, selection method, and target metric. {#tbl-data-selection .striped .hover tbl-colwidths="[23,21,28,28]" tbl-colwidths="[23,21,28,28]"}
...
: **Total Cost of Training Data**: The four cost components span the full data lifecycle. Here $D$ is the total sample count, $D_{\text{labeled}}$ is the labeled subset, $D_{\text{vol,store}}$ is stored byte volume, $T_{\text{months}}$ is retention time, $N_{\text{epochs}}$ is the number of training passes, and $O_{\text{sample}}$ is per-sample training work. The ranges are scenario anchors rather than universal market prices. {#tbl-cost-components .striped .hover tbl-colwidths="[25,30,45]" tbl-colwidths="[25,30,45]"}
...
: **ROI Profiles for Data Selection Techniques**: The table identifies costs and possible savings. No technique guarantees positive ROI; each must satisfy the selection inequality under measured workload conditions. {#tbl-roi-profiles .striped .hover tbl-colwidths="[18,39,43]" tbl-colwidths="[18,40,44]"}
...
**Systems lesson**: Distributed coreset selection decouples dataset pruning scale from single-node memory capacity, as illustrated in the selection pipeline architecture (@fig-distributed-coreset-architecture). Sharding embedding and scoring phases across GPU workers amortizes selection overhead over parallel cluster bandwidth.
...
**Systems insight**: FLOPs are *not* latency. A 99 percent reduction in operations can yield a 0 percent reduction in time if the remaining operations are memory bound or cache-inefficient. Optimization must target the hardware's binding bottleneck rather than an abstract metric alone [@hoefler2021sparsity].
...
Take a scenario in which the full run takes `{python} FpPitfallCalc.full_training_time_hours_str` and training on the coreset takes `{python} FpPitfallCalc.training_time_hours_str`, so selection has to buy back its cost out of the hours saved. A sophisticated coreset algorithm requiring `{python} FpPitfallCalc.selection_time_bad_hours_str` to do that selection spends `{python} FpPitfallCalc.selection_overhead_ratio_mult_str` the training run it enables and more than the full run it was meant to avoid, yielding negative ROI. @Eq-selection-inequality defines the boundary. Use lightweight proxy models or cached embeddings when they preserve selection quality. In the illustrative comparison, proxy-based EL2N scoring completes in `{python} FpPitfallCalc.selection_time_good_min_str`, or `{python} FpPitfallCalc.selection_overhead_good_pct_str` of that full-run baseline, and satisfies the inequality.
