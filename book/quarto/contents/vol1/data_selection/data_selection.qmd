...
[^fn-data-wall-scaling]: **Data Wall**\index{Data Wall!scaling constraint}: Unlike compute (which scales with capital expenditure) or algorithms (which improve through research), the stock of high-quality human-generated text grows slowly. Epoch AI's 2022 projections estimated that, under the paper's modeled consumption assumptions, the stock of high-quality language data could be exhausted around 2026, while the far larger stock of public human-generated text could be exhausted between 2026 and 2032 [@villalobos2022will]. The point for systems design is not a fixed calendar deadline; it is that data can become a supply constraint rather than merely an economic one. This constraint directly affects the total operations $(O)$ term: when quality data becomes scarce, additional compute yields diminishing returns regardless of hardware throughput.
...
**Problem**: A hospital team has `{python} ActiveLearningRoi.n_unlabeled_str` unlabeled scans, a `{python} ActiveLearningRoi.cost_per_label_str` specialist-label cost, a `{python} ActiveLearningRoi.budget_str` budget, and `{python} ActiveLearningRoi.deadline_months_str`.
...
- **Training work**: The selected set has `{python} ActiveLearningRoi.speedup_mult_str` fewer examples per epoch than the budgeted random baseline.
- **Active learning outcome**: Matching `{python} ActiveLearningRoi.n_random_str` random scans requires empirical validation.

**Systems insight**: Against the budget-matched random baseline, selection saves `{python} ActiveLearningRoi.cost_saving_str`; accuracy still requires empirical validation.
...
Active learning optimizes which samples to label but still requires human annotation for every selected example. **Semi-supervised learning**\index{Semi-Supervised Learning!definition} takes a more aggressive approach: rather than asking which samples to label, it asks whether we can extract learning signal from unlabeled data directly. It uses a small set of labeled examples to guide learning on a much larger unlabeled pool, typically achieving over 90 percent of fully supervised accuracy with only a small fraction of the labels.
...
[^fn-randaugment-simplicity]: **RandAugment**: Collapses AutoAugment's policy search to two hyperparameters: transformation count $N$ and shared magnitude $M$ [@cubuk2020]. That small search space recovers most AutoAugment gains at negligible cost, making it practical when 15,000 GPU-hours of policy search cannot be justified. \index{RandAugment!hyperparameter simplicity}
...
[^fn-stable-diffusion-synthesis]: **Stable Diffusion**: Performs iterative denoising in a compressed latent space, enabling targeted text-to-image examples. Its cost is seconds per image rather than near-free geometric transforms, so it fits cases where novelty matters more than per-sample throughput. \index{Stable Diffusion!generation cost}
...
[^fn-domain-gap-shift]: **Domain Gap**: The statistical divergence between synthetic and real data distributions, measurable with metrics such as Maximum Mean Discrepancy (MMD) or Frechet Inception Distance (FID). For ML systems, the gap becomes training-serving skew: a simulator-trained model can validate well on synthetic data while failing silently on real deployment data. \index{Domain Gap!training-serving skew}
...
[^fn-domain-randomization-sim]: **Domain Randomization**: Makes synthetic data deliberately varied by randomizing textures, colors, lighting, and physical properties. The goal is not photorealism; it is enough variation that the real world becomes one more sample from the training distribution, shifting the cost bottleneck from rendering fidelity to coverage. \index{Domain Randomization!rendering trade-off}
...
class CostBreakdown:
    """ImageNet-scale training cost breakdown: data costs (~99%) dominate compute (~1%)."""
    ...
    gpu_hour_cost = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate  # registry GPU-hour rate
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    train_gpu_hours = train_gpu_count * train_duration
    train_compute_cost = (train_gpu_hours * gpu_hour_cost).to(USD)
    ...
    train_gpu_hours_str = fmt_count(
        train_gpu_count * train_duration.to(hour).magnitude,
        label="GPU-hour",
...
| **Training (`{python} CostBreakdown.train_epoch_count_str` in `{python} CostBreakdown.train_duration_str` on `{python} CostBreakdown.train_gpu_count_str`)** | `{python} CostBreakdown.train_gpu_hours_str` |                         `{python} CostBreakdown.c_train_usd_str`                         |
...
\begin{gather*}
\text{Random labeling cost = `{python} BreakevenCalc.be_n_random_str` $\times$ `{python} BreakevenCalc.cost_label_str` = `{python} BreakevenCalc.cost_random_total_str`}
\\
\text{Active learning cost = `{python} BreakevenCalc.be_n_active_str` $\times$ `{python} BreakevenCalc.cost_label_str` + `{python} BreakevenCalc.n_rounds_str` rounds $\times$ `{python} BreakevenCalc.cost_inference_str` = `{python} BreakevenCalc.cost_active_total_str`}
\\
\text{ROI = (`{python} BreakevenCalc.cost_random_total_str` − `{python} BreakevenCalc.cost_active_total_str`) / `{python} BreakevenCalc.cost_active_total_str` $\times$ 100 = `{python} BreakevenCalc.roi_pct_math`}
\end{gather*}
...
With a fixed worker count, data selection reduces communication overhead by reducing gradient updates per epoch. Fewer samples mean fewer synchronization points, and communication costs often dominate at large worker counts. Relative to a weak-scaling baseline, where each worker's workload stays approximately constant as the cluster grows, data selection can preserve accuracy with less total data than the nominal trajectory. This matters when data collection rather than compute is the bottleneck. Even within straightforward data parallelism, smaller curated datasets reduce per-worker shard sizes, potentially improving cache utilization and reducing I/O stalls on each node.
...
# │ Show: Semi-log axes; green frontier curve; orange/red/purple points; labels.
