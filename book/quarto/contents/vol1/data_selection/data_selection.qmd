...
To make this concrete, consider training a model in the GPT-2\index{GPT-2!data selection}/Llama\index{Llama!data selection} Lighthouse family from @sec-network-architectures-lighthouse-roster-model-biographies-a763, which spans the autoregressive LLM\index{LLM} family from GPT-2's 1.5 billion parameters to Llama's 7-70 billion parameter range, here using a 70-billion-parameter language model.
...
**EL2N coreset** (Error L2-Norm, a training-dynamics score developed in @sec-data-selection-coreset-selection-algorithms-2c74; `{python} IcrCoresetComparison.coreset_pct_str` of data):
...
```{.tikz}
\scalebox{0.85}{\begin{tikzpicture}[font=\small\sffamily]
\tikzset{
...
The figure tracks a different axis from the cost notebook in \ref{nbk-data-selection-active-learning-roi}. It illustrates label efficiency, while the notebook calculates labeling dollars under separate assumptions. Active learning also incurs scoring, acquisition, and repeated-retraining costs, so end-to-end savings can be smaller than the label-count gap.
...
**Scenario**: An embedded KWS system targets deployment on a 256 KB SRAM microcontroller with a `{python} KwsDataSelectionCalc.target_samples_plus_str` audio sample requirement.
...
\draw[Arrow](D2)--node[right, pos=0.25, font=\scriptsize\sffamily] {Yes}(D3);
...
The hardware bottleneck analysis in @sec-data-selection-hardware-acceleration-cf40 assumes single-machine training. The interactions become more complex when scaling to multiple machines, because data selection affects different parallelism strategies in distinct ways.
...
The earlier subsections in this section examined pairwise interactions, but production systems apply all these optimizations together. Trace the full optimization stack\index{Optimization Stack!multiplicative effects} in @fig-optimization-stack, from data to deployment: each stage in this pipeline amplifies or attenuates the effects of others.
...
The cross-layer stack makes a measurement framework\index{Measurement Framework!data selection metrics} unavoidable: every data selection technique claims to improve efficiency, but only rigorous measurement separates real savings from shifted costs or hidden quality loss. The core metrics in @sec-data-selection-core-metrics-32e9 tie sample reduction to accuracy, cost, and deployment coverage so that a smaller dataset is judged by what it preserves, rather than by what it removes alone.
