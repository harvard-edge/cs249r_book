...
Data selection asks a deceptively simple question with profound engineering consequences: given a clean, well-engineered dataset, which examples contribute the most learning per unit of compute cost? The preceding chapter on data engineering (@sec-data-engineering-ml) established the infrastructure for collecting, cleaning, and preparing data, producing pipelines that ingest raw signals and yield well-governed, versioned datasets ready for training. That chapter ensured data *quality* through correct labels, consistent schemas, and clean records. Data selection optimizes data *value* by extracting maximum learning from minimum samples, directly shrinking the **Total Operations ($O$)** term in the **Iron Law** (@sec-silicon-contract). The distinction matters: quality asks *whether* data is correct, while value asks *whether* correct data is worth the compute spent processing it.

\index{Scaling Laws!data-compute asymmetry}
For decades, the dominant strategy was straightforward: more data, better models. Scaling laws [@kaplan2020scaling; @hoffmann2022training] confirmed that model performance improves predictably with dataset size, and teams responded rationally by scraping more web pages, labeling more images, and generating more synthetic examples. A critical asymmetry has since emerged. Hardware acceleration (@sec-ai-acceleration) has outpaced the growth of high-quality data. GPU compute capacity has increased faster than traditional Moore's Law projections—architectural innovations like Tensor Cores and reduced-precision arithmetic stack on top of process-node gains, yielding effective throughput improvements faster than every two years—while the supply of novel, high-quality human-generated text and images grows at roughly 2$\times$ every five years (@tbl-scaling-asymmetry). The internet has already been scraped. Domain experts cannot label faster. This asymmetry, which researchers call the **Data Wall**\index{Data Wall!definition}[^fn-data-wall] [@villalobos2022will], has inverted the optimization priority from "get more data" to "get more from existing data."
...
# │
# │ Why: Quantifies the growth rate gap between compute capacity (10x/3yr) and
# │      high-quality data availability (2x/5yr). This asymmetry drives the
# │      entire chapter's premise: data selection matters because we're now
# │      compute-rich and data-poor.
# │
...
# --- Outputs (Unicode strings for table - use × not \times in tables) ---
gpu_growth_str = "10×"                                          # e.g. "10×"
gpu_period_str = "3 years"                                      # growth period
web_data_growth_str = "2×"                                      # e.g. "2×"
web_data_period_str = "5 years"                                 # growth period
label_data_growth_str = "1.5×"                                  # e.g. "1.5×"
label_data_period_str = "5 years"                               # growth period
```
...
The gap between what compute can process and what quality data exists is widening, necessitating the **Continuous Training** loops of **MLOps** (@sec-machine-learning-operations-mlops) to maintain relevance and making intelligent data selection increasingly critical.
...
\index{D·A·M Taxonomy!data selection as first pillar}
...
# │
# │ Why: Makes the compute-data asymmetry concrete with a real scenario. A team
# │      with 10,000 H100s can process 10T tokens, but only 5T high-quality
# │      tokens exist. This 2x gap forces data selection over naive scaling.
# │
# │ Imports: physx.formatting (fmt)
...
# --- Inputs (training scenario assumptions) ---
llama_params_value = 70e9                                       # 70B parameter model
h100_count_value = 10000                                        # GPUs available
training_months_value = 3                                       # training duration
tokens_capacity_value = 10e12                                   # compute can process
tokens_available_value = 5e12                                   # quality data exists

# --- Process ---
compute_gap_value = tokens_capacity_value / tokens_available_value

# --- Outputs (formatted strings for prose) ---
llama_params_str = fmt(llama_params_value, precision=0, unit="B")   # e.g. "70 B"
h100_count_str = fmt(h100_count_value, precision=0, commas=True)    # e.g. "10,000"
tokens_capacity_str = fmt(tokens_capacity_value, precision=0, unit="T")    # e.g. "10 T"
tokens_available_str = fmt(tokens_available_value, precision=0, unit="T")  # e.g. "5 T"
compute_gap_str = fmt(compute_gap_value, precision=0)               # e.g. "2"
```

\index{H100!compute capacity example}
...
### Systems Perspective {#sec-data-selection-data-selection-systems-problem-d857}
...
The systems framing reveals optimization opportunities invisible to the ML framing. To see *why*, consider *how* data selection interacts with the Iron Law introduced in @sec-silicon-contract.
...
# │
# │ Why: Demonstrates that data selection savings are multiplicative with other
# │      optimizations. A 2x dataset reduction + 2x model compression + 2x HW
# │      acceleration = 8x total savings (not 6x additive). This shows why
# │      data selection belongs at the head of the optimization pipeline.
# │
# │ Imports: physx.formatting (fmt)
...
# --- Inputs (training scenario assumptions) ---
training_cost_m_value = 100                                     # $M training budget
dataset_reduction_pct_value = 50                                # % data pruned
data_selection_factor_value = 2                                 # 2x from data selection
model_compression_factor_value = 2                              # 2x from compression
hardware_accel_factor_value = 2                                 # 2x from HW accel

# --- Process (multiplicative savings) ---
...
# --- Outputs (formatted strings for prose) ---
training_cost_m_str = fmt(training_cost_m_value, precision=0, commas=False)     # e.g. "100"
dataset_reduction_pct_str = fmt(dataset_reduction_pct_value, precision=0, commas=False)  # e.g. "50"
compute_savings_m_str = fmt(compute_savings_m_value, precision=0, commas=False) # e.g. "50"
combined_factor_str = fmt(combined_factor_value, precision=0, commas=False)     # e.g. "8"
```
...
These compute savings cascade through the entire infrastructure stack. Large datasets consume petabytes of storage and saturate network bandwidth during distributed training; deduplication and coreset selection reduce storage costs while eliminating I/O bottlenecks that can idle expensive GPU clusters. The savings extend to labeling economics as well: expert labeling costs (\$5--100+ per sample in domains like medical imaging) often exceed compute costs, and active learning and semi-supervised methods function as cost engineering tools that reduce labeling budgets by 10--100$\times$. The environmental implications compound these benefits further---training a large language model can emit hundreds of tons of CO₂, making data selection the most direct lever for Green AI: halving the dataset halves training energy with no accuracy trade-off if done correctly. Perhaps most importantly, smaller curated datasets enable faster iteration velocity. A team that can iterate in hours rather than days has a compounding advantage in model development.
...
# │
# │ Why: Makes the Information-Compute Ratio concrete by comparing random
# │      sampling vs. EL2N coreset selection on ResNet-50/ImageNet. The coreset
# │      achieves 1.8x higher ICR by focusing on decision boundary samples,
# │      demonstrating why data selection is not just about less data but
# │      about more learning per FLOP.
# │
# │ Imports: physx.constants (RESNET50_FLOPs, GFLOPs, IMAGENET_IMAGES)
...
imagenet_size_value = IMAGENET_IMAGES.magnitude                 # ~1.28M images
...
# --- Process (compute ICR for both strategies) ---
...
# --- Outputs (formatted strings for prose) ---
resnet50_fwd_gflops_str = fmt(RESNET50_FLOPs.to(GFLOPs), precision=1)           # e.g. "8.2"
resnet50_fwdbwd_gflops_str = fmt((RESNET50_FLOPs * 2).to(GFLOPs), precision=1)  # e.g. "16.4"
full_epoch_flops_str = f"{full_epoch_flops_value:.2e}"          # e.g. "2.10e+19"
icr_random_str = f"{icr_random_value:.1e}"                      # e.g. "2.4e-19"
imagenet_size_str = fmt(imagenet_size_value / 1e6, precision=2, unit="M")       # e.g. "1.28 M"
...
\index{EL2N!coreset selection example}
\index{ResNet-50!coreset selection benchmark}
\index{ImageNet!coreset selection benchmark}
::: {.callout-example title="Computing ICR: Coresets"}
**Scenario**: Training our **ResNet-50 Lighthouse model** (@sec-dnn-architectures) on ImageNet for one epoch. We compare random batch selection versus EL2N-based coreset selection (EL2N, or Error L2-Norm, scores each sample by how uncertain the model's prediction is; it is defined formally in @sec-data-selection-coreset-selection-algorithms-a520). ResNet-50's compute-bound nature (high **arithmetic intensity**; see @sec-system-foundations-roofline-model-5f7c for how the Roofline Model determines this classification) makes it an ideal candidate for data selection optimization: reducing dataset size directly reduces training FLOPs with minimal I/O impact.
...
## Static Pruning {#sec-data-selection-static-data-pruning-pretraining-filtration-d6e6}
...
\index{CIFAR-10!coreset pruning results}
\index{ImageNet!pruning results}
\index{The Pile!deduplication redundancy}
\index{C4!deduplication redundancy}
...
# │
# │ Why: Shows that label noise has a quadratic penalty on data requirements.
# │      To reach 1% error, clean data needs ~100 samples while noisy data
# │      needs ~10,000 (100x more). This makes data cleaning a "100x compute
# │      accelerator" — a dramatic quantification of quality vs quantity.
# │
# │ Imports: physx.formatting (fmt)
...
# --- Inputs (convergence rate assumptions) ---
epsilon_value = 0.01                                            # target error = 1%
n_clean_value = 100                                             # samples at O(1/N)
n_noisy_value = 10000                                           # samples at O(1/√N)

# --- Process ---
ratio_value = n_noisy_value / n_clean_value

# --- Outputs (formatted strings for prose) ---
epsilon_str = fmt(epsilon_value, precision=2, commas=False)     # e.g. "0.01"
epsilon_pct_str = fmt(epsilon_value * 100, precision=0, commas=False)  # e.g. "1"
n_clean_str = fmt(n_clean_value, precision=0, commas=False)     # e.g. "100"
n_noisy_str = fmt(n_noisy_value, precision=0, commas=True)      # e.g. "10,000"
ratio_str = fmt(ratio_value, precision=0, commas=False)         # e.g. "100"
```
...
\index{Facility Location|see{k-Center Algorithm}}
...
# │
# │ Why: Shows a practical workflow for 10x data reduction. Starting with 1M
# │      images, a 5-epoch proxy model computes EL2N scores, selecting the top
# │      100K high-uncertainty samples. This concrete recipe makes coreset
# │      selection implementable, not just theoretical.
# │
# │ Imports: physx.formatting (fmt)
...
# --- Outputs (formatted strings for prose) ---
n_train_images_str = fmt(n_train_images_value / 1e6, precision=0, unit=" million")  # e.g. "1 million"
...
[^fn-jaccard]: **Jaccard Similarity**\index{Jaccard Similarity!etymology}: Named after Paul Jaccard (1868--1944), a Swiss botanist who introduced the coefficient in 1901 to compare plant species distributions. Defined as $|A \cap B| / |A \cup B|$, it ranges from 0 (no overlap) to 1 (identical sets). It became a cornerstone of information retrieval because it naturally handles "bag of words" comparisons regardless of document length. MinHash [@broder1997resemblance] provides an efficient probabilistic approximation, enabling web-scale deduplication.
...
\index{CLIP!embedding-based deduplication}
...
\index{GPT-3!deduplication benefits}
\index{LLaMA!deduplication benefits}
...
\index{DLRM!embedding deduplication}
...
\index{Cleanlab!label error detection}
...
# │
# │ Why: Quantifies curriculum learning speedups across standard benchmarks.
# │      Gains are inversely proportional to dataset quality: CIFAR-10 (highly
# │      redundant) sees 23% speedup while ImageNet (well-curated) sees only
# │      11%. This calibrates practitioner expectations by dataset type.
# │
# │ Imports: physx.formatting (fmt)
...
# │
# │ Why: Makes the economic case for active learning concrete. With 1M scans at
...
# --- Outputs (formatted strings for prose) ---
n_unlabeled_str = fmt(n_unlabeled_value / 1e6, precision=0, unit=" Million")    # e.g. "1 Million"
...
cost_saving_str = fmt(cost_saving_value / 1e6, precision=2, unit=" Million")    # e.g. "4.75 Million"
```
...
# │
# │ Why: Visualizes the 4x data efficiency advantage of active learning vs
# │      random sampling. The gap between curves represents compute savings:
# │      active learning reaches 90% accuracy at ~2,500 samples while random
# │      needs ~9,000. Illustrative curves emphasize qualitative advantage.
# │
# │ Imports: numpy, physx.viz
...
\index{FixMatch!consistency plus pseudo-labeling}
...
# │
# │ Why: Demonstrates semi-supervised learning's label efficiency. FixMatch
# │      achieves 94.9% accuracy with just 250 labels (0.5% of data), showing
# │      200x label efficiency. The cost analysis shows that trading labels for
# │      compute (5x more training) still saves 8x total cost when labels are
# │      expensive. This quantifies when semi-supervised is economically viable.
# │
# │ Imports: physx.formatting (fmt)
...
\index{CLIP!multimodal alignment}
...
# │
# │ Why: Quantifies why fine-tuning dominates modern ML practice. Training 10
# │      tasks from scratch costs 100K labels × $1 = $1M + 10K GPU-hours. Fine-
# │      tuning costs 1K labels × $1 = $10K + 10.5K GPU-hours (including
# │      pre-training). The 100x label reduction and 20x marginal compute drop
# │      explain the foundation model paradigm shift.
# │
# │ Imports: physx.formatting (fmt)
...
# │
# │ Why: Provides data for the bar chart visualization comparing train-from-
# │      scratch (10 × 1,000 = 10,000 GPU-hrs) vs foundation model (10,000 pre-
# │      train + 10 × 50 = 10,500 GPU-hrs). This cell duplicates values from
# │      foundation-cost-calc for proximity to the TikZ figure.
# │
# │ Imports: (none)
...
\index{SimCLR!contrastive learning}
\index{MoCo!momentum contrastive learning}
\index{He, Kaiming!MoCo}
...
### Foundation Model Paradigm {#sec-data-selection-1000-multiplier-foundation-model-paradigm-7866}

\index{Self-supervised Learning!1000x labeled data multiplier}
...
## Synthetic Data Generation {#sec-data-selection-synthetic-data-generation-augmentation-f3c5}
...
\index{AutoAugment!learned augmentation policy}
\index{RandAugment!simplified augmentation}
...
## Decision Framework {#sec-data-selection-technique-summary-0ee8}

\index{Data Selection!decision framework}
...
## Selection Engineering {#sec-data-selection-engineering-data-selection-systems-7aef}
...
# │
# │ Why: Makes the selection inequality concrete with a 1M image scenario.
# │      Compares full-model scoring (2.8 hrs overhead) vs proxy scoring (0.6
# │      hrs). Both beat the 278 hr baseline, but proxy saves an extra 2.2 hrs.
# │      The "trap" scenario (50 hr selection with 7B model) shows how expensive
# │      selection can consume all efficiency gains.
# │
# │ Imports: physx.formatting (fmt)
...
score_a_hrs_str = fmt(score_a_sec_value/3600, precision=1, commas=False)  # e.g. "2.8"
train_a_str = fmt(train_a_sec_value, precision=0, commas=True)          # e.g. "100,000"
train_a_hrs_str = fmt(train_a_sec_value/3600, precision=1, commas=False)  # e.g. "27.8"
...
score_b_hrs_str = fmt(score_b_sec_value/3600, precision=1, commas=False)  # e.g. "0.6"
...
# │
# │ Why: Formalizes the selection inequality with epoch-normalized costs.
# │      Shows that one-shot selection (1 epoch) + 10 epochs subset training
# │      = 11 epoch cost → 9x speedup. But iterative selection (100 epochs)
# │      + 10 epochs = 110 epoch cost → slower than baseline! This derivation
# │      explains why proxy models and caching are essential.
# │
# │ Imports: physx.formatting (fmt)
...
# │
# │ Why: Visualizes the selection inequality with stacked bars. Baseline (100)
# │      vs efficient selection (5 overhead + 40 training = 45, 55% savings)
# │      vs expensive selection (60 overhead + 40 = 100, no savings). Shows
# │      that selection overhead can negate all efficiency gains if too high.
# │
# │ Imports: numpy, physx.viz
# │ Exports: (figure output only)
# └─────────────────────────────────────────────────────────────────────────────
...
\index{FAISS!embedding index for selection}
...
\index{WebDataset!shard-based data loading}
...
# │
# │ Why: Demonstrates when data echoing provides positive ROI. With a 300
# │      images/sec pipeline and 800 images/sec GPU, the GPU idles 63% of the
# │      time. Echo factor 2 doubles effective throughput (still below GPU
# │      capacity), cutting training time roughly in half. This quantifies the
# │      CPU-GPU imbalance that makes echoing valuable.
# │
# │ Imports: physx.formatting (fmt)
# │ Exports: pipeline_throughput_str, ratio_str, idle_pct_str, echo_hrs_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.formatting import fmt
...
ratio_str = fmt(ratio_r_value, precision=2, commas=False)       # e.g. "2.67"
...
## Cost Modeling {#sec-data-selection-cost-modeling-economics-b702}
...
# │
# │ Why: Shows that data costs (acquisition + labeling + storage) often exceed
# │      compute costs for supervised learning. In this ImageNet scenario,
# │      data is 70% of total cost vs 30% for compute. This explains why data
# │      selection has high ROI: reducing data cost by 50% saves more than
# │      halving compute budget.
# │
# │ Imports: physx.formatting (fmt - for n_labels_str)
...
n_labels_str = fmt(n_labels_value / 1e6, precision=1, unit="M") # e.g. "1.2 M"
cost_per_label_str = fmt(cost_per_label_value, precision=2, commas=False)           # e.g. "0.05"
...
### ROI Framework for Data Selection Techniques {#sec-data-selection-roi-framework}
...
# │
# │ Why: Calculates when active learning pays for itself. With $10/label and
...
# │
# │ Imports: physx.formatting (fmt)
# │ Exports: cost_random_total_str, cost_active_total_str, roi_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.formatting import fmt
...
n_random_str = fmt(n_random, precision=0, commas=True)          # e.g. "5,000"
n_active_str = fmt(n_active, precision=0, commas=True)          # e.g. "2,000"
...
### Amortization across Training Runs {#sec-data-selection-amortization-time-value-data-selection-26c5}
...
# │
# │ Why: Shows how infrastructure investment ROI depends on reuse. A $55K
# │      deduplication pipeline that saves $10K/run is a net loss after 1 run
# │      (-82%) but highly profitable after 50 runs (+809%). This explains why
# │      teams should invest in data infrastructure proportional to expected
# │      training frequency.
# │
# │ Imports: physx.formatting (fmt)
...
## Distributed Selection {#sec-data-selection-distributed-data-selection-25a8}
...
# │
# │ Why: Quantifies the total overhead for distributed coreset selection on
...
# │      Since ImageNet training is ~24 hours, coreset selection has clear
# │      positive ROI for this scale.
# │
# │ Imports: (none)
...
## Cross-Layer Interactions {#sec-data-selection-interactions-optimizations-02bc}
...
### Model Compression {#sec-data-selection-model-compression}
...
### Hardware Acceleration {#sec-data-selection-hardware-acceleration}
...
### Distributed Training {#sec-data-selection-distributed-training}
...
## Measurement Framework {#sec-data-selection-measuring-data-selection-7957}
...
# │
# │ Why: Visualizes why data selection matters: efficient selection reaches the
# │      accuracy plateau faster than random sampling. The efficiency gap (red
# │      arrow) at 40% dataset size shows the compute savings available through
# │      smarter data curation. Exponential saturation curves model real
# │      learning dynamics.
# │
# │ Imports: numpy, physx.viz
# │ Exports: (figure output only)
...
\index{DataPerf!benchmarking initiative}
For a comprehensive treatment of data selection metrics and benchmarking methodologies, including how initiatives like DataPerf are standardizing evaluation protocols, see @sec-benchmarking-ai.
...
\index{Data Selection!fallacies and pitfalls}
...
# │
# │ Why: Transforms qualitative assertions into concrete numbers that students
# │      can internalize and apply. Each claim needs at least one quantified
# │      threshold or worked example.
# │
# │ Imports: physx.formatting (fmt)
...
# Fallacy 1
data_1m_str = fmt(data_1m_value / 1e6, precision=0, unit="M")
data_10m_str = fmt(data_10m_value / 1e6, precision=0, unit="M")
acc_gain_10x_str = fmt(acc_gain_10x_value, precision=0, commas=False)
curated_size_str = fmt(curated_size_value / 1e3, precision=0, unit="K")
raw_size_str = fmt(raw_size_value / 1e6, precision=0, unit="M")
...
# Fallacy 4
training_run_cost_str = fmt(training_run_cost_value / 1e6, precision=0, unit="M")
efficiency_gain_pct_str = fmt(efficiency_gain_pct_value, precision=0, commas=False)
savings_str = fmt(savings_value / 1e6, precision=0, unit="M")
...
# Pitfall 2
total_samples_str = fmt(total_samples_value / 1e6, precision=0, unit="M")
...
coreset_pct_str = fmt(coreset_pct_value, precision=0, commas=False)
...
A sophisticated coreset algorithm requiring `{python} selection_time_bad_str` hours to select samples for a `{python} training_time_str`-hour training run has `{python} selection_overhead_ratio_str`$\times$ overhead, yielding negative ROI. @sec-data-selection-selection-bottleneck-f1d4 establishes the Selection Inequality: $T_{selection} + T_{train}(subset) < T_{train}(full)$. If full training takes `{python} full_training_time_str` hours, selection must remain under 10% of that time. Use lightweight proxy models (ResNet-18 for 5 epochs instead of ResNet-50 for 100) or cached embeddings: proxy-based EL2N scoring completes in `{python} selection_time_good_str` minutes (`{python} selection_overhead_good_pct_str`% overhead), satisfying the inequality while achieving comparable selection quality.

**Pitfall:** *Pruning rare classes into oblivion.*

Aggressive coreset selection removes rare classes entirely because they contribute little to average loss. In a `{python} total_samples_str` dataset with `{python} rare_class_pct_str`% rare class samples (`{python} rare_class_count_str` examples), a `{python} coreset_pct_str`% coreset using uniform importance sampling retains only `{python} expected_rare_in_coreset_str` rare examples on average, below the `{python} min_samples_threshold_str`-sample minimum needed for reliable learning. @sec-data-selection-coreset-selection-algorithms-a520 recommends stratified selection: set minimum samples per class before applying pruning, ensuring rare classes retain sufficient representation. Production models failing on rare cases despite excellent average accuracy often trace to this pitfall.

**Pitfall:** *Training on deduplicated data while evaluating on duplicated test sets.*

Web-scraped datasets contain `{python} test_contamination_pct_str`% train-test overlap on average. Deduplicating only training data inflates apparent test accuracy to `{python} inflated_acc_str`% when true generalization is `{python} true_acc_str`%, a `{python} inflation_gap_str`-point gap that masks real-world performance. @sec-data-selection-data-deduplication-9035 emphasizes joint deduplication: hash both train and test sets, removing any sample appearing in both. As noted in that section, GPT-3 and LLaMA training studies confirm that deduplicated data improves both efficiency and generalization. Teams observing accuracy drops after deduplication are measuring improved generalization, not degradation.

**Pitfall:** *Active learning without considering annotation latency.*

Active learning theory assumes instant oracle responses. @sec-data-selection-active-learning-humanintheloop-a9fa notes that expert labels require days or weeks in practice. With `{python} al_latency_days_str`-day annotation latency, a model trained for `{python} model_drift_epochs_value` epochs between query rounds may have drifted significantly: samples selected as uncertain become irrelevant as the decision boundary shifts. Select larger batches (`{python} batch_size_large_str` vs `{python} batch_size_small_str` samples) to amortize latency and use diversity sampling to hedge against model drift. @sec-data-selection-breakeven-analysis-ec3a shows that active learning ROI depends critically on matching batch size to annotation turnaround time.
