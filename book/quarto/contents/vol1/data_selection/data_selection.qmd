...
::: {#fig-active-learning-loop fig-cap="**Active Learning Loop**: A selection strategy queries uncertain or informative samples from an unlabeled pool, sends them to a human oracle, adds their labels to the training set, and updates the model before the next round. The loop aims to increase information gained per label; the gain must be measured for the task and oracle." fig-alt="Five-box cycle: Unlabeled Pool points right to Selection Strategy with Query, then down to Oracle (Human) with Uncertainty, down to Training Set with Labels, left to Model with Train, and back up to Unlabeled Pool with a dashed Update arrow."}
```{.tikz}
...
::: {#fig-active-learning-multiplier fig-env="figure" fig-pos="htb" fig-cap="**The Active Learning Multiplier**: Model accuracy is plotted against labeled samples on a logarithmic axis. In this illustrative scenario, active learning (green solid) reaches the same accuracy with fewer labels than random sampling (gray dashed); the arrow marks an approximately 4× label-efficiency gap near 90 percent accuracy." fig-alt="Line chart with labeled samples from 100 to 10,000 on a logarithmic x-axis and accuracy on the y-axis. A green active-learning curve rises above a gray dashed random-sampling curve, the area between them is shaded green, and a horizontal arrow near 90 percent accuracy is labeled 4x Data Efficiency."}
...
For a selection strategy to be systems-efficient, selection plus subset training must cost less than training on the full dataset. @Eq-selection-inequality expresses this break-even condition, the **Selection Inequality**\index{Selection Inequality!definition}.
...
The stacked bars in @fig-selection-inequality demonstrate this trade-off: efficient selection (center) saves 55 percent of total time in the figure's normalized example, while expensive selection (right) consumes all the savings in overhead.\index{Selection Inequality!overhead trade-off}
...
**Setup**: @Fig-distributed-coreset-architecture separates parallel shard-local scoring from the coordinator’s global merge; workers send local scores upward before the coordinator chooses the final coreset.
