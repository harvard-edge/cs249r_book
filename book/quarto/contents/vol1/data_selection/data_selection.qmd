...
**Scenario**: Given 1 million training images, the goal is to select a 100k coreset (10 percent) using EL2N scoring.
...
**Scenario**: Select a 10 percent coreset from ImageNet (1.2M images) using 8 workers with 4 GPUs each.
...
* **The Selection Inequality gates every technique**: $T_{selection} + T_{train}(subset) < T_{train}(full)$. Selection overhead should stay below 10 percent of training time. Proxy models and cached embeddings keep $T_{selection}$ low; expensive selection algorithms can consume all the savings they promise.
