...
Data selection asks a deceptively simple question with profound engineering consequences: given a clean, well-engineered dataset, which examples contribute the most learning per unit of compute cost? The data engineering infrastructure (@sec-data-engineering) established the infrastructure for collecting, cleaning, and preparing data, producing pipelines that ingest raw signals and yield well-governed, versioned datasets ready for training. That chapter ensured data *quality* through correct labels, consistent schemas, and clean records. Data selection optimizes data *value* by extracting maximum learning from minimum samples, directly shrinking the total operations $(O)$ term in the iron law\index{Iron Law!total operations reduction via data selection} (Principle \ref{pri-iron-law}). The distinction matters: quality asks whether data is correct, while value asks whether correct data is worth the compute spent processing it.
...
::: {.column-margin}
![](images/svg/data_selection_scaling_saturation.svg){width="100%" fig-alt="A performance curve that rises steeply at first and then flattens into a near-horizontal plateau, with a dot marking the bend where gains start diminishing. A flat baseline runs underneath. Early data buys large gains; later data buys almost none."}

*Diminishing returns: early data buys large gains, later data almost none.*
:::
...
class ScalingAsymmetry:
    ...
    gpu_period_str = fmt_int(gpu_period_years, commas=False, suffix=" years")

    web_data_growth_str = fmt(web_growth_factor, precision=0, commas=False)
    web_data_period_str = fmt_int(web_period_years, commas=False, suffix=" years")

    label_data_growth_str = fmt(label_growth_factor, precision=1, commas=False)
    label_data_period_str = fmt_int(label_period_years, commas=False, suffix=" years")
```
...
|:------------------------|---------------------------------------------------------------------------------------------------------------:|:------------------------------------------------------|
| **GPU Compute**         |               ~`{python} ScalingAsymmetry.gpu_growth_str`$\times$ / `{python} ScalingAsymmetry.gpu_period_str` | Hardware vendors deliver reliable exponential gains   |
| **Training Data (Web)** |     ~`{python} ScalingAsymmetry.web_data_growth_str`$\times$ / `{python} ScalingAsymmetry.web_data_period_str` | High-quality web text is finite; much already scraped |
| **Labeled Data**        | ~`{python} ScalingAsymmetry.label_data_growth_str`$\times$ / `{python} ScalingAsymmetry.label_data_period_str` | Human annotation throughput is inherently bounded     |
...
from mlsysim.fmt import check, fmt, fmt_usd, fmt_percent

class ComputeDataGap:
    # LOAD
    h100_count = Systems.Clusters.Training_10K.total_accelerators
    months = 3
    filtered_corpus_tokens = 5e12
    public_text_stock = 300e12
    sustained_mfu = calibration.REFERENCE_MFU_SUSTAINED
    training_flops_per_token = 6 * Models.Language.Llama2_70B.parameters.m_as(c.param)
    seconds = months * c.DAYS_PER_MONTH * c.HOURS_PER_DAY * c.SEC_PER_HOUR

    # EXECUTE
    total_flops = (
        h100_count
        * Hardware.Cloud.H100.compute.peak_flops.m_as(c.flop / c.second)
        * seconds
        * sustained_mfu
    )
    tokens_capacity = total_flops / training_flops_per_token
    corpus_reuse_ratio = tokens_capacity/filtered_corpus_tokens
    cluster_cost = (
        h100_count
        * months
        * c.DAYS_PER_MONTH
        * c.HOURS_PER_DAY
        * Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as("dollar / hour")
    )
...
class IronLawSavings:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_cost_m_str = fmt_usd(budget_m, precision=0, commas=False, suffix="M")
    dataset_reduction_pct_str = fmt(data_pruning_pct, precision=0, commas=False, suffix=" percent")
    compute_savings_m_str = fmt_usd(compute_savings_m, precision=0, commas=False, suffix="M")
...
# │ Exports: imagenet_size_str, icr_ratio_str, IcrCoresetComparison.coreset_pct_str, etc.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.fmt import fmt_percent, fmt, check, fmt_sci, MarkdownStr

class IcrCoresetComparison:
    """Compare learning-per-FLOP for random sampling vs. coreset selection."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    imagenet_size_value = Datasets.ImageNet.training_examples.m_as('count')
    ...
    resnet50_fwdbwd_gflops_value = (m_resnet.inference_flops * 3).m_as(GFLOPs)
    ...
    resnet50_fwdbwd_gflops_str = fmt((m_resnet.inference_flops * 3).to(GFLOPs), precision=1)
    full_epoch_flops_str = MarkdownStr(str(fmt_sci(full_epoch_flops_value, precision=2)) + " FLOPs")
    icr_random_str = fmt_sci(icr_random_value, precision=1)
    imagenet_size_str = fmt(imagenet_size_value/MILLION, precision=2, suffix="M")
    # Custom "K" suffix on a divided count uses fmt() suffix discipline.
    coreset_size_str = fmt(coreset_size_value / THOUSAND, precision=1, commas=False, suffix="K")
    coreset_flops_str = MarkdownStr(str(fmt_sci(coreset_flops_value, precision=1)) + " FLOPs")
    icr_coreset_str = fmt_sci(icr_coreset_value, precision=1)
    icr_ratio_str = fmt(icr_ratio_value, precision=1, commas=False)
    acc_gain_random_str = fmt(acc_gain_random_value, precision=0, commas=False, suffix=" percentage points")
    acc_gain_coreset_str = fmt(acc_gain_coreset_value, precision=1, commas=False, suffix=" percentage points")
    acc_diff_str = fmt(acc_diff_value, precision=1, commas=False, suffix=" percentage points")
    coreset_pct_str = fmt(coreset_fraction_value * 100, precision=0, commas=False, suffix=" percent")
    acc_perf_pct_str = fmt(acc_gain_coreset_value / acc_gain_random_value * 100, precision=0, commas=False, suffix=" percent")
```
...
- Model: ResNet-50 Lighthouse (~`{python} IcrCoresetComparison.resnet50_fwd_gflops_str` GFLOPs per forward pass, roughly `{python} IcrCoresetComparison.resnet50_fwdbwd_gflops_str` GFLOPs for a forward plus backward training step, depending on implementation)
- One epoch: `{python} IcrCoresetComparison.imagenet_size_str` $\times$ `{python} IcrCoresetComparison.resnet50_fwdbwd_gflops_str` GFLOPs = `{python} IcrCoresetComparison.full_epoch_flops_str`
...
- Compute: `{python} IcrCoresetComparison.coreset_size_str` $\times$ `{python} IcrCoresetComparison.resnet50_fwdbwd_gflops_str` GFLOPs = `{python} IcrCoresetComparison.coreset_flops_str`
...
**Systems insight**: The coreset achieves `{python} IcrCoresetComparison.icr_ratio_str`$\times$ higher ICR, nearly twice the learning per FLOP, by eliminating low-information "easy" samples that contribute little to the decision boundary. The `{python} IcrCoresetComparison.acc_diff_str` accuracy difference is often acceptable given the `{python} IcrCoresetComparison.coreset_pct_str` compute savings.
...
class QualityMultiplier:
    ...
    epsilon_pct_str = fmt(epsilon * 100, precision=0, commas=False, suffix=" percent")
...
::: {.column-margin}
![](images/svg/vol1_data_selection_margin_001.svg){width="100%" fig-alt="Two bars side by side: a short bar labeled clean (one sample) next to a bar about 100 times taller labeled noisy, showing how many noisy samples equal one clean one."}
...
**Systems insight**: Cleaning the dataset (removing label noise) is a `{python} QualityMultiplier.ratio_str`$\times$ compute accelerator.
...
**Coreset selection**\index{Coreset!definition}\index{Static Pruning!coreset selection}[^fn-coreset-geometry] answers this question by identifying a small subset of data that preserves the statistical properties of the entire dataset.
...
class CoresetPractice:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(n_coreset_value > 0, "Coreset must be nonempty.")
    check(coreset_fraction_value < 1.0, "Coreset fraction must be less than 1.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_train_images_str = fmt(n_train_images_value/MILLION, precision=0, suffix=" million")
    coreset_fraction_pct_str = fmt(coreset_fraction_value * 100, precision=0, commas=False, suffix=" percent")
...
class CurriculumBenchmarks:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cifar10_speedup_str = fmt(cifar10_speedup_pct, precision=1, commas=False, suffix="%")
    cifar100_speedup_str = fmt(cifar100_speedup_pct, precision=1, commas=False, suffix="%")
    imagenet_speedup_str = fmt(imagenet_speedup_pct, precision=1, commas=False, suffix="%")
    mentornet_speedup_str = fmt(mentornet_speedup_pct, precision=1, commas=False, suffix="%")
...
class ActiveLearningRoi:
    ...
    over_budget_ratio_value = cost_all_value / budget_value
    ...
    n_random_pct_str = fmt(n_random_pct_value, precision=0, commas=False, suffix=" percent")
    n_active_str = fmt(n_active_value, precision=0, commas=True)
    cost_active_str = fmt(cost_active_value, precision=0, commas=True)
    cost_active_pct_str = fmt(cost_active_pct_value, precision=0, commas=False, suffix=" percent")
    ...
    deadline_months_str = fmt(deadline_months_value, precision=0, commas=False, suffix=" month")
```

::: {#nbk-data-selection-active-learning-roi .callout-notebook title="The active learning ROI"}

**Problem**: A hospital team is building a medical diagnostic AI. The pool contains `{python} ActiveLearningRoi.n_unlabeled_str` unlabeled scans. A specialist doctor charges \$`{python} ActiveLearningRoi.cost_per_label_str` to label one scan. The team has a budget of \$`{python} ActiveLearningRoi.budget_str` and a deadline of `{python} ActiveLearningRoi.deadline_months_str`.

**Scenario A: Naive Labeling**

1.  **Cost**: Labeling all `{python} ActiveLearningRoi.n_unlabeled_str` scans would cost \$`{python} ActiveLearningRoi.cost_all_str` (`{python} ActiveLearningRoi.over_budget_ratio_str`$\times$ over budget).
...
class ClinicalLabelingEconomics:
    ...
    hourly_rate_range_str = MarkdownStr(f"{hourly_rate_low}–{hourly_rate_high}")
    labeled_hours_range_str = MarkdownStr(f"{fmt(labeled_hours_low_display, precision=0, commas=False)}–{fmt(labeled_hours_high_display, precision=0, commas=False)} hours")
    labeled_cost_range_str = f"{fmt(labeled_cost_low_display, precision=0, commas=True)}–{fmt(labeled_cost_high_display, precision=0, commas=True)}"
    full_hours_range_str = f"{fmt(full_hours_low, precision=0, commas=True)}–{fmt(full_hours_high, precision=0, commas=True)}"
    full_weeks_range_str = f"{fmt(full_weeks_low, precision=1, commas=False)}–{fmt(full_weeks_high, precision=0, commas=False)}"
    full_cost_range_str = f"{fmt(full_cost_low_display, precision=0, commas=True)}–{fmt(full_cost_high_display, precision=0, commas=True)}"
```
...
class FixmatchLabelEfficiency:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    cifar10_full_labels = int(Datasets.CIFAR10.training_examples.m_as(count))
    ...
    supervised_total_str = fmt_usd(supervised_total, precision=0, commas=True)
    fixmatch_label_cost_str = fmt_usd(fixmatch_label_cost, precision=0, commas=True)
    fixmatch_total_str = fmt_usd(fixmatch_total, precision=0, commas=True)
    cost_reduction_str = fmt(cost_reduction, precision=1, commas=False)
    ...
    cifar10_full_acc_str = fmt(cifar10_full_acc, precision=1, commas=False, suffix="%")
    cifar10_fixmatch_4k_acc_str = fmt(cifar10_fixmatch_4k_acc, precision=1, commas=False, suffix="%")
    cifar10_fixmatch_250_acc_str = fmt(cifar10_fixmatch_250_acc, precision=1, commas=False, suffix="%")
    cifar10_fixmatch_40_acc_str = fmt(cifar10_fixmatch_40_acc, precision=1, commas=False, suffix="%")
    cifar10_fixmatch_4k_eff_str = fmt(cifar10_fixmatch_4k_eff, precision=1, commas=False)
    cifar10_fixmatch_250_eff_str = fmt(cifar10_fixmatch_250_eff, precision=0, commas=False)
    cifar10_fixmatch_40_eff_str = fmt(cifar10_fixmatch_40_eff, precision=0, commas=False)
...
**FixMatch**\index{FixMatch} [@sohn2020fixmatch] combines pseudo-labeling with consistency regularization to achieve high label efficiency (@tbl-fixmatch-cifar10). With only `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_labels_str` labeled samples (`{python} FixmatchLabelEfficiency.labels_per_class_250_str` per class), FixMatch achieves `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_acc_str` accuracy, within `{python} FixmatchLabelEfficiency.acc_loss_str` points of full supervision using `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_eff_str`$\times$ fewer labels. The technique works by generating pseudo-labels on weakly augmented unlabeled images (only when model confidence exceeds 0.95), then training to predict these labels on strongly augmented versions of the same images.

**Systems insight**: Semi-supervised learning trades labeled data for unlabeled data and compute. On CIFAR-10, training FixMatch requires ~`{python} FixmatchLabelEfficiency.fixmatch_compute_multiplier_str`$\times$ more compute than supervised training (processing 50K unlabeled samples per epoch). Two baselines are in play. Against full supervision, the 250-label result in @tbl-fixmatch-cifar10 shows 200$\times$ fewer labels. The ROI calculation below instead compares against a 4,000-label supervised baseline, where FixMatch uses 16$\times$ fewer purchased labels; the resulting total-cost trade-off assumes labels cost \$1 each and GPU hours cost \$0.50:

- Supervised (`{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_labels_str` labels): `{python} FixmatchLabelEfficiency.supervised_label_cost_str` labeling + `{python} FixmatchLabelEfficiency.supervised_compute_cost_str` compute = `{python} FixmatchLabelEfficiency.supervised_total_str`
- FixMatch (`{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_labels_str` labels): `{python} FixmatchLabelEfficiency.fixmatch_label_cost_str` labeling + `{python} FixmatchLabelEfficiency.fixmatch_compute_cost_str` compute = `{python} FixmatchLabelEfficiency.fixmatch_total_str`
...
| **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_labels_str` (8 percent)**    | FixMatch         |  `{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_acc_str` |  **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_eff_str`$\times$ more efficient** |
| **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_labels_str` (0.5 percent)** | FixMatch         | `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_acc_str` | **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_eff_str`$\times$ more efficient** |
| **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_40_labels_str` (0.08 percent)** | FixMatch         |  `{python} FixmatchLabelEfficiency.cifar10_fixmatch_40_acc_str` |  **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_40_eff_str`$\times$ more efficient** |
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt, fmt_usd, check, MarkdownStr
...
class FoundationCostAmortization:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_tasks_str = fmt(n_tasks_value, precision=0, commas=False)
    cost_per_label_str = fmt_usd(cost_per_label, precision=0, commas=False)
    ...
    label_cost_scratch_total_str = fmt_usd(label_cost_scratch_total / MILLION, precision=0, commas=False, suffix="M")
    cost_scratch_per_task_str = fmt(cost_scratch_per_task_value, precision=0, commas=True, suffix=" GPU-hours")
    cost_scratch_total_str = fmt(cost_scratch_total_value, precision=0, commas=True, suffix=" GPU-hours")
    cost_foundation_total_str = fmt(cost_foundation_total_value, precision=0, commas=True, suffix=" GPU-hours")
    labels_per_task_finetune_str = fmt(labels_per_task_finetune, precision=0, commas=True)
    label_cost_finetune_total_str = fmt_usd(label_cost_finetune_total / THOUSAND, precision=0, commas=False, suffix="K")
    cost_finetune_value_str = fmt(cost_finetune_value, precision=0, commas=True, suffix=" GPU-hours")
    cost_pretrain_value_str = fmt(cost_pretrain_value, precision=0, commas=True, suffix=" GPU-hours")
...
The return on investment is substantial for labeling, marginal per-task compute, and time to deployment: labeling costs drop by `{python} FoundationCostAmortization.label_cost_drop_str`$\times$ overall (from `{python} FoundationCostAmortization.label_cost_scratch_total_str` to `{python} FoundationCostAmortization.label_cost_finetune_total_str`), per-task marginal compute decreases by `{python} FoundationCostAmortization.marginal_compute_reduction_str`$\times$, and time to deployment accelerates by 20--50$\times$ per task. Total compute becomes favorable only after the pretraining cost is amortized across enough downstream tasks.
...
# │          .cost_per_sample_range_str, .augmentation_multiplier_str,
# │          .seed_cost_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, MarkdownStr
...
class KwsDataSelectionCalc:
    ...
    utterances_per_speaker_str = fmt(utterances_per_speaker, precision=0, commas=False)
    ...
    full_cost_range_str = f"{fmt(full_cost_low / THOUSAND, precision=0, commas=False)}–{fmt(full_cost_high / THOUSAND, precision=0, commas=False)}K"
    cost_per_sample_range_str = MarkdownStr(f"{cost_per_sample_low}–{cost_per_sample_high}")
    augmentation_multiplier_str = fmt(augmentation_multiplier, precision=0, commas=False)
    seed_cost_pct_str = fmt(seed_cost_pct, precision=0, commas=False, suffix=" percent")
```
...
- Professional recording costs \$`{python} KwsDataSelectionCalc.cost_per_sample_range_str` per sample (\$`{python} KwsDataSelectionCalc.full_cost_range_str` total)
...
1. **Seed data** (`{python} KwsDataSelectionCalc.seed_samples_str` samples): Record `{python} KwsDataSelectionCalc.speakers_str` speakers with `{python} KwsDataSelectionCalc.utterances_per_speaker_str` utterances each in controlled conditions
2. **Augmentation** (`{python} KwsDataSelectionCalc.augmented_samples_str` samples): Apply pitch shift, time stretch, speed variation to `{python} KwsDataSelectionCalc.augmentation_multiplier_str`$\times$ the seed data
...
# │          .scoring_time_str, .coreset_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
class SelectionBottleneckAnchor:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(scoring_time_hrs > 2, "Full-model scoring should be hours, not minutes.")
    check(coreset_pct == 10, "Worked example assumes a 10% coreset.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dataset_size_str = fmt(dataset_size_m, precision=0, commas=False, suffix="M")
    scoring_time_str = fmt(scoring_time_hrs, precision=1, commas=False, suffix=" hours")
    coreset_pct_str = fmt(coreset_pct, precision=0, commas=False, suffix="%")
```

Dynamic data selection introduces a new bottleneck: selection latency\index{Latency!selection, dynamic selection bottleneck}. In standard training, the data loader reads the next batch sequentially. In active learning or curriculum learning, the system must evaluate a selection function $f(x)$ over a large candidate pool to determine the next batch. Concretely, scoring a `{python} SelectionBottleneckAnchor.dataset_size_str` dataset with a large model can take `{python} SelectionBottleneckAnchor.scoring_time_str`, potentially negating the savings from a `{python} SelectionBottleneckAnchor.coreset_pct_str` coreset if not performed with a smaller proxy model.
...
class SelectionInequalityCalc:
    ...
    coreset_fraction_value = n_coreset_value/n_images_value
    ...
    score_b_hrs_value = score_b_sec_value / SEC_PER_HOUR
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    score_a_str = fmt(score_a_sec_value, precision=0, commas=True, suffix=" s")
    score_a_hrs_str = fmt(score_a_sec_value / SEC_PER_HOUR, precision=1, commas=False, suffix=" h")
    train_a_str = fmt(train_a_sec_value, precision=0, commas=True, suffix=" s")
    train_a_hrs_str = fmt_int(round(train_a_sec_value / SEC_PER_HOUR), commas=False, suffix=" h")
    total_a_hrs_str = fmt_int(round(total_a_hrs_value), commas=False, suffix=" h")
    score_b_str = fmt(score_b_sec_value, precision=0, commas=True, suffix=" s")
    score_b_hrs_str = fmt(score_b_hrs_value, precision=1, commas=False, suffix=" h")
    total_b_hrs_str = fmt_int(round(total_b_hrs_value), commas=False, suffix=" h")
    baseline_str = fmt(baseline_sec_value, precision=0, commas=True, suffix=" s")
    baseline_hrs_str = fmt_int(round(baseline_hrs_value), commas=False, suffix=" h")
    savings_a_str = fmt_int(round(savings_a_hrs_value), commas=False, suffix=" h")
    savings_a_pct_str = fmt(savings_a_pct_value, precision=0, commas=False, suffix="%")
    savings_b_str = fmt_int(round(savings_b_hrs_value), commas=False, suffix=" h")
    savings_b_pct_str = fmt(savings_b_pct_value, precision=1, commas=False, suffix="%")
    b_beats_a_str = fmt_int(round(b_beats_a_hrs_value), commas=False, suffix=" h")
    trap_total_str = fmt_int(round(trap_total_hrs_value), commas=False, suffix=" h")
    trap_pct_str = fmt(trap_overhead_pct_value, precision=0, commas=False, suffix="%")
    n_images_str = fmt(n_images_value / MILLION, precision=0, commas=False, suffix="M")
    n_coreset_str = fmt(n_coreset_value / THOUSAND, precision=0, commas=False, suffix="k")
    n_epochs_str = fmt(n_epochs_value, precision=0, commas=False)
    coreset_pct_str = fmt(coreset_fraction_value * 100, precision=0, commas=False, suffix=" percent")
    resnet50_time_per_image_str = fmt(resnet50_time_per_image_value, precision=2, commas=False, suffix=" s")
    resnet18_time_per_image_str = fmt(resnet18_time_per_image_value, precision=3, commas=False, suffix=" s")
    trap_sel_hrs_str = fmt(trap_sel_hrs_value, precision=0, commas=False, suffix=" h")
```
...
- Score all `{python} SelectionInequalityCalc.n_images_str` with the target ResNet-50: `{python} SelectionInequalityCalc.n_images_str` $\times$ `{python} SelectionInequalityCalc.resnet50_time_per_image_str` = `{python} SelectionInequalityCalc.score_a_str` (`{python} SelectionInequalityCalc.score_a_hrs_str`)
- Train on `{python} SelectionInequalityCalc.n_coreset_str` coreset for `{python} SelectionInequalityCalc.n_epochs_str` epochs: `{python} SelectionInequalityCalc.n_coreset_str` $\times$ `{python} SelectionInequalityCalc.n_epochs_str` $\times$ `{python} SelectionInequalityCalc.resnet50_time_per_image_str` = `{python} SelectionInequalityCalc.train_a_str` (`{python} SelectionInequalityCalc.train_a_hrs_str`)
- **Total**: `{python} SelectionInequalityCalc.total_a_hrs_str`

**Option B: Proxy Model Selection**

- Score all `{python} SelectionInequalityCalc.n_images_str` with a small proxy (ResNet-18): `{python} SelectionInequalityCalc.n_images_str` $\times$ `{python} SelectionInequalityCalc.resnet18_time_per_image_str` = `{python} SelectionInequalityCalc.score_b_str` (`{python} SelectionInequalityCalc.score_b_hrs_str`)
- Train on `{python} SelectionInequalityCalc.n_coreset_str` coreset for `{python} SelectionInequalityCalc.n_epochs_str` epochs: `{python} SelectionInequalityCalc.train_a_str` (`{python} SelectionInequalityCalc.train_a_hrs_str`)
- **Total**: `{python} SelectionInequalityCalc.total_b_hrs_str`

**Baseline: No Selection**

- Train on the full `{python} SelectionInequalityCalc.n_images_str` dataset for `{python} SelectionInequalityCalc.n_epochs_str` epochs: `{python} SelectionInequalityCalc.n_images_str` $\times$ `{python} SelectionInequalityCalc.n_epochs_str` $\times$ `{python} SelectionInequalityCalc.resnet50_time_per_image_str` = `{python} SelectionInequalityCalc.baseline_str` (`{python} SelectionInequalityCalc.baseline_hrs_str`)

**Analysis**:

- Option A saves `{python} SelectionInequalityCalc.savings_a_str` vs. baseline (`{python} SelectionInequalityCalc.savings_a_pct_str` reduction) ✓
- Option B saves `{python} SelectionInequalityCalc.savings_b_str` vs. baseline (`{python} SelectionInequalityCalc.savings_b_pct_str` reduction) ✓
- Option B beats Option A by `{python} SelectionInequalityCalc.b_beats_a_str`. Proxy selection yields better ROI.

**Trap**: If selection required `{python} SelectionInequalityCalc.trap_sel_hrs_str` (for example, running a 7-billion-parameter model), the total would reach `{python} SelectionInequalityCalc.trap_total_str`, still better than baseline, but the selection overhead equals `{python} SelectionInequalityCalc.trap_pct_str` of the remaining net savings.
...
# │
# │ Imports: mlsysim.core.constants (THOUSAND); mlsysim.fmt (check, fmt)
# │ Exports: RandomAccessPenalty.hdd_seq_mb_s_str, .hdd_iops_str,
# │          .hdd_random_mb_s_str, .hdd_penalty_str,
# │          .sata_seq_mb_s_str, .sata_iops_str, .sata_random_mb_s_str,
# │          .sata_penalty_str, .nvme_seq_mb_s_str, .nvme_iops_str,
# │          .nvme_random_mb_s_str, .nvme_penalty_str
# └─────────────────────────────────────────────────────────────────────────────
...
class RandomAccessPenalty:
    ...
    # EXECUTE
    hdd_random_mb_s = hdd_iops * random_read_kb / THOUSAND
    sata_random_mb_s = sata_iops * random_read_kb / THOUSAND
    nvme_random_mb_s = nvme_iops * random_read_kb / THOUSAND
    hdd_penalty = hdd_seq_mb_s / hdd_random_mb_s
    sata_penalty = sata_seq_mb_s / sata_random_mb_s
    nvme_penalty = nvme_seq_mb_s / nvme_random_mb_s

    # GUARD
    check(hdd_penalty > sata_penalty > nvme_penalty, "Random penalty should shrink as storage latency improves.")
    check(nvme_random_mb_s < nvme_seq_mb_s, "Random NVMe throughput should remain below sequential throughput.")

    # OUTPUT
    hdd_seq_mb_s_str = fmt(hdd_seq_mb_s, precision=0, commas=False, suffix=" MB")
    hdd_iops_str = fmt(hdd_iops, precision=0, commas=False)
    hdd_random_mb_s_str = fmt(hdd_random_mb_s, precision=1, commas=False, suffix=" MB")
    hdd_penalty_str = fmt_int(hdd_penalty, -2, commas=False)

    sata_seq_mb_s_str = fmt(sata_seq_mb_s, precision=0, commas=False, suffix=" MB")
    sata_iops_str = fmt(sata_iops / THOUSAND, precision=0, commas=False, suffix="k")
    sata_random_mb_s_str = fmt(sata_random_mb_s, precision=0, commas=False, suffix=" MB")
    sata_penalty_str = fmt(sata_penalty, precision=1, commas=False)

    nvme_seq_mb_s_str = fmt(nvme_seq_mb_s, precision=0, commas=True, suffix=" MB")
    nvme_iops_str = fmt(nvme_iops / THOUSAND, precision=0, commas=False, suffix="k")
    nvme_random_mb_s_str = fmt(nvme_random_mb_s, precision=0, commas=True, suffix=" MB")
    nvme_penalty_str = fmt(nvme_penalty, precision=2, commas=False)
```

| **Storage Tier** |              **Sequential Throughput**              |             **Random I/O (IOPS)**             |             **Random Throughput (approx)**             |                    **Random Penalty**                   |
|:-----------------|:---------------------------------------------------:|:---------------------------------------------:|:------------------------------------------------------:|:-------------------------------------------------------:|
| **HDD (7.2k)**   |  ~`{python} RandomAccessPenalty.hdd_seq_mb_s_str`/s |  ~`{python} RandomAccessPenalty.hdd_iops_str` | ~`{python} RandomAccessPenalty.hdd_random_mb_s_str`/s  |  `{python} RandomAccessPenalty.hdd_penalty_str`$\times$ |
| **SATA SSD**     | ~`{python} RandomAccessPenalty.sata_seq_mb_s_str`/s | ~`{python} RandomAccessPenalty.sata_iops_str` | ~`{python} RandomAccessPenalty.sata_random_mb_s_str`/s | `{python} RandomAccessPenalty.sata_penalty_str`$\times$ |
| **NVMe SSD**     | ~`{python} RandomAccessPenalty.nvme_seq_mb_s_str`/s | ~`{python} RandomAccessPenalty.nvme_iops_str` | ~`{python} RandomAccessPenalty.nvme_random_mb_s_str`/s | `{python} RandomAccessPenalty.nvme_penalty_str`$\times$ |
...
::: {.column-margin}
![](images/svg/vol1_data_selection_margin_003.svg){width="100%" fig-alt="A curve that rises with echo factor then flattens at a marked knee dot at e equals R; the region past the knee, the over-echo zone, is shaded red."}
...
class DataEchoingRoi:
    ...
    idle_pct_str = fmt(gpu_idle_pct_value, precision=1, commas=False, suffix=" percent")
    no_echo_sec_str = fmt(no_echo_sec_value, precision=0, commas=True, suffix=" seconds")
    no_echo_hrs_str = fmt(no_echo_hrs_value, precision=1, commas=False, suffix=" hours")
    gpu_util_str = fmt_int(gpu_util_no_echo_value, commas=False, suffix="%")
    echo_sec_str = fmt(echo_sec_value, precision=0, commas=True, suffix=" seconds")
    echo_hrs_str = fmt(echo_hrs_value, precision=1, commas=False, suffix=" hours")
...
- Data pipeline throughput: `{python} DataEchoingRoi.pipeline_throughput_str`/second (reading, decoding, augmenting on CPU)
- GPU training throughput: `{python} DataEchoingRoi.gpu_throughput_str`/second (forward + backward pass)
...
- Effective throughput: `{python} DataEchoingRoi.pipeline_throughput_str`/second (limited by data pipeline)
- Training time for 90 epochs: $90 \times 1.28\text{M}$ / `{python} DataEchoingRoi.pipeline_throughput_str` = `{python} DataEchoingRoi.no_echo_sec_str` (`{python} DataEchoingRoi.no_echo_hrs_str`)
...
- Effective throughput: `{python} DataEchoingRoi.effective_throughput_str`/second (still below GPU capacity)
- Unique images per second: `{python} DataEchoingRoi.pipeline_throughput_str` (unchanged)
- Training time: $90 \times 1.28\text{M}$ / `{python} DataEchoingRoi.effective_throughput_str` = `{python} DataEchoingRoi.echo_sec_str` (`{python} DataEchoingRoi.echo_hrs_str`) if echoed data is equally valuable
...
class CostBreakdown:
    ...
    c_raw_str = fmt_usd(c_raw_value, precision=0)
    c_label_str = fmt_usd(c_label_value, precision=0)
    c_store_str = fmt_usd(c_store_value, precision=0, commas=False)
    c_train_str = fmt_usd(c_train_value, precision=0)
    c_total_str = fmt_usd(c_total_value, precision=0)
    p_data_str = fmt(p_data_value, precision=1, commas=False, suffix="%")
    p_compute_str = fmt(p_compute_value, precision=1, commas=False, suffix="%")
    n_labels_str = fmt(n_labels_value/MILLION, precision=1, suffix="M")
    cb_cost_per_label_str = fmt_usd(cost_per_label_value, precision=2, commas=False)
    # Mixed prose + math symbol (×) → MarkdownStr escape-hatch
    storage_str = MarkdownStr(f"{storage_gb} GB × {storage_months} months")
    train_desc_str = MarkdownStr(f"{train_epochs} epochs × {train_gpus} A100s × {train_hours} h")
```
...
|:-----------------------------------------------------------------------------------------------------------|:-----------------|:--------------------------------------------------------------------------------------------:|
| **Raw data (`{python} CostBreakdown.n_labels_str`)**                                                       | Licensed dataset |                              `{python} CostBreakdown.c_raw_str`                              |
| **Labels (`{python} CostBreakdown.n_labels_str` $\times$ `{python} CostBreakdown.cb_cost_per_label_str`)** | Crowd annotation |                             `{python} CostBreakdown.c_label_str`                             |
...
class BreakevenCalc:
    ...
    roi_pct_str = fmt(roi_pct, precision=1, commas=False, suffix=" percent")
```
...
class DeduplicationAmortization:
    ...
    roi_1_str = fmt(rois[0], precision=1, commas=False, suffix="%")
    roi_5_str = fmt(rois[1], precision=1, commas=False, suffix="%")
    roi_10_str = fmt(rois[2], precision=1, commas=False, suffix="%")
    roi_50_str = fmt(rois[3], precision=1, commas=False, suffix="%")
```
...
class DistributedOverheadCalc:
    ...
    t_embed_str = fmt(t_embed_value, precision=0, commas=False, suffix=" minutes")
    t_dedup_str = fmt(t_dedup_value, precision=0, commas=False, suffix=" minutes")
    t_score_str = fmt(t_score_value, precision=0, commas=False, suffix=" minutes")
    t_select_str = fmt(t_select_value, precision=0, commas=False, suffix=" minutes")
    t_total_overhead_str = fmt(t_total_overhead_value, precision=0, commas=False, suffix=" minutes")
```
...
class ChinchillaDiagnostic:
    ...
    llama2_70b_training_tokens = 2.0 * c.TRILLION
    compute_multiplier = 2.0

    # EXECUTE
    gpt3_dn_ratio = Models.Language.GPT3.training_tokens.m_as(count) / m_gpt3.parameters.m_as(param)
    llama2_dn_ratio = llama2_70b_training_tokens / m_llama2.parameters.m_as(param)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tokens_per_param_opt_str = fmt(tokens_per_param_opt, precision=0, commas=False)
    starvation_ratio_str = fmt(starvation_ratio, precision=0, commas=False)
    diminishing_ratio_str = fmt(diminishing_ratio, precision=0, commas=False)
    gpt3_dn_ratio_str = fmt(gpt3_dn_ratio, precision=1, commas=False)
    llama2_dn_ratio_str = fmt(llama2_dn_ratio, precision=1, commas=False)
    doubling_token_growth_pct_str = fmt(doubling_token_growth_pct, precision=1, commas=False, suffix=" percent")
```
...
class FpFallacyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    data_1m_str = fmt(data_1m_value / MILLION, precision=0, suffix="M")
    data_10m_str = fmt(data_10m_value / MILLION, precision=0, suffix="M")
    acc_gain_10x_str = fmt(acc_gain_10x_value, precision=0, commas=False, suffix=" percentage points")
    curated_size_str = fmt(curated_size_value / THOUSAND, precision=0, suffix="K")
    raw_size_str = fmt(raw_size_value / MILLION, precision=0, suffix="M")
    curated_accuracy_str = fmt(curated_accuracy_value, precision=0, commas=False, suffix=" percent")
    raw_accuracy_str = fmt(raw_accuracy_value, precision=0, commas=False, suffix=" percent")
    curated_cost_ratio_str = fmt(curated_cost_ratio_value, precision=0, commas=False)
    synthetic_gen1_acc_str = fmt(synthetic_gen1_acc_value, precision=0, commas=False, suffix=" percent")
    synthetic_gen5_acc_str = fmt(synthetic_gen5_acc_value, precision=0, commas=False, suffix=" percent")
    synthetic_acc_drop_str = fmt(synthetic_acc_drop_value, precision=0, commas=False)
    optimal_synthetic_mix_str = MarkdownStr(f"{optimal_synthetic_mix_min_value}–{optimal_synthetic_mix_max_value} percent")
    training_run_cost_str = fmt(training_run_cost_value / MILLION, precision=0, suffix="M")
    efficiency_gain_pct_str = fmt(efficiency_gain_pct_value, precision=0, commas=False, suffix=" percent")
    savings_str = fmt(savings_value / MILLION, precision=0, suffix="M")
    cifar10_coreset_pct_str = fmt(cifar10_coreset_pct_value, precision=0, commas=False, suffix=" percent")
    cifar10_acc_retained_str = fmt(cifar10_acc_retained_value, precision=0, commas=False, suffix=" percent")
    imagenet_acc_retained_str = fmt(imagenet_acc_retained_value, precision=0, commas=False, suffix=" percent")
    medical_acc_retained_str = fmt(medical_acc_retained_value, precision=0, commas=False, suffix=" percent")
```

**Fallacy**: *Data is the new oil, so more is always better.*

Engineers assume linear returns from data scaling: 10$\times$ more data should yield proportional accuracy gains. In reality, the ICR framework (@sec-data-selection-informationcompute-ratio-8c0b) reveals severe diminishing returns. Scaling from `{python} FpFallacyCalc.data_1m_str` to `{python} FpFallacyCalc.data_10m_str` samples typically yields only `{python} FpFallacyCalc.acc_gain_10x_str` of accuracy gain while incurring 10$\times$ the compute cost. @Tbl-scaling-asymmetry quantifies the asymmetry: GPU compute grows 10$\times$ every three years while high-quality data grows only 2$\times$ every five years. A curated `{python} FpFallacyCalc.curated_size_str` dataset achieving `{python} FpFallacyCalc.curated_accuracy_str` accuracy often outperforms a raw `{python} FpFallacyCalc.raw_size_str` dataset at `{python} FpFallacyCalc.raw_accuracy_str`, despite `{python} FpFallacyCalc.curated_cost_ratio_str`$\times$ fewer samples. Teams that blindly scale data budgets waste compute on redundant samples that contribute near-zero gradient signal.

**Fallacy**: *Synthetic data can completely replace real data.*

Engineers assume generative models produce unlimited training data at marginal cost. Synthetic-only training can fail through two different mechanisms. First, @sec-data-selection-bridging-domain-gap-100b and @tbl-synthetic-mix show the domain-gap problem: generated data can diverge from the real deployment distribution, causing the learned decision boundary in @fig-domain-gap to misclassify real-world inputs. Second, recursive training on model-generated data can cause model collapse: accuracy degrades from `{python} FpFallacyCalc.synthetic_gen1_acc_str` to `{python} FpFallacyCalc.synthetic_gen5_acc_str` after five generations of training on model-generated data, a `{python} FpFallacyCalc.synthetic_acc_drop_str`-point drop. Optimal mixes are typically `{python} FpFallacyCalc.optimal_synthetic_mix_str` synthetic, with the remainder real data; pure synthetic training fails catastrophically on deployment distributions.
...
class FpPitfallCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    selection_time_bad_value = 10
    training_time_value = 2
    full_training_time_value = 8
    selection_time_good_value = 0.5
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    selection_time_bad_str = fmt(selection_time_bad_value, precision=0, commas=False, suffix=" hours")
    training_time_str = fmt(training_time_value, precision=0, commas=False, suffix="-hour")
    full_training_time_str = fmt(full_training_time_value, precision=0, commas=False, suffix=" hours")
    selection_overhead_ratio_str = fmt(selection_overhead_ratio_value, precision=0, commas=False)
    selection_time_good_str = fmt(selection_time_good_value * 60, precision=0, commas=False, suffix=" minutes")
    selection_overhead_good_pct_str = fmt(selection_overhead_good_pct_value, precision=1, commas=False, suffix=" percent")
    total_samples_str = fmt(total_samples_value / MILLION, precision=0, suffix="M")
    rare_class_pct_str = fmt(rare_class_pct_value, precision=1, commas=False, suffix=" percent")
    rare_class_count_str = fmt(rare_class_count_value, precision=0, commas=True)
    fp_coreset_pct_str = fmt(coreset_pct_value, precision=0, commas=False, suffix=" percent")
    ...
    coreset_size_pct_str = fmt(coreset_size_pct_value, precision=0, commas=False, suffix=" percent")
    rare_class_acc_str = fmt(rare_class_acc_value, precision=0, commas=False, suffix=" percent")
    majority_class_acc_str = fmt(majority_class_acc_value, precision=0, commas=False, suffix=" percent")
```

**Pitfall**: *Optimizing selection without measuring selection overhead.*

A sophisticated coreset algorithm requiring `{python} FpPitfallCalc.selection_time_bad_str` to select samples for a `{python} FpPitfallCalc.training_time_str` training run has `{python} FpPitfallCalc.selection_overhead_ratio_str`$\times$ overhead, yielding negative ROI. @Sec-data-selection-selection-bottleneck-4d00 establishes the Selection Inequality: $T_{\text{selection}} + T_{\text{train}}(\text{subset}) < T_{\text{train}}(\text{full})$. If full training takes `{python} FpPitfallCalc.full_training_time_str`, selection must remain under 10 percent of that time. Use lightweight proxy models (ResNet-18 for five epochs instead of ResNet-50 for 100) or cached embeddings: proxy-based EL2N scoring completes in `{python} FpPitfallCalc.selection_time_good_str` (`{python} FpPitfallCalc.selection_overhead_good_pct_str` overhead), satisfying the inequality while achieving comparable selection quality.
