...
class ComputeDataGap:
    ...
    training_flops_per_token = 6 * Models.Language.Llama2_70B.parameters.to(c.param).magnitude
    seconds = months * c.DAYS_PER_MONTH * c.HOURS_PER_DAY * c.SEC_PER_HOUR

    # EXECUTE
    total_flops = (
        h100_count
        * Hardware.Cloud.H100.compute.peak_flops.to(c.flop / c.second).magnitude
        * seconds
        * sustained_mfu
    )
    ...
    cluster_cost = (
        ...
        * c.DAYS_PER_MONTH
        * c.HOURS_PER_DAY
        * Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.to(USD / hour).magnitude
    )
...
from mlsysim.fmt import fmt_percent, fmt, check, fmt_sci, fmt_sci_qty, fmt_count

class IcrCoresetComparison:
    """Compare learning-per-FLOP for random sampling vs. coreset selection."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    imagenet_size_value = Datasets.ImageNet.training_examples.to("").magnitude
    ...
    resnet50_fwdbwd_gflops_value = (m_resnet.inference_flops * 3).to(GFLOPs).magnitude
    ...
    full_epoch_flops_str = fmt_sci_qty(full_epoch_flops_value * flop, flop, precision=2, unit_label="FLOPs")
    ...
    coreset_flops_str = fmt_sci_qty(coreset_flops_value * flop, flop, precision=1, unit_label="FLOPs")
...
from mlsysim.fmt import check, fmt, fmt_time_range, MarkdownStr
...
class RandomAccessPenalty:
    # LOAD
    random_read_q = 4 * KB
    random_read_kb = random_read_q.to(KB).magnitude
    hdd_seq_q = 150 * (MB / second)
    hdd_seq_mb_s = hdd_seq_q.to(MB/second).magnitude
    hdd_iops = 80
    sata_seq_q = 550 * (MB / second)
    sata_seq_mb_s = sata_seq_q.to(MB/second).magnitude
    sata_iops = 10_000
    nvme_seq_q = 3_500 * (MB / second)
    nvme_seq_mb_s = nvme_seq_q.to(MB/second).magnitude
    nvme_iops = 500_000
    ...
    hdd_random_mb_s = hdd_random_q.to(MB/second).magnitude
    sata_random_mb_s = sata_random_q.to(MB/second).magnitude
    nvme_random_mb_s = nvme_random_q.to(MB/second).magnitude
...
class ChinchillaDiagnostic:
    ...
    # EXECUTE
    gpt3_dn_ratio = Models.Language.GPT3.training_tokens.to(count).magnitude / m_gpt3.parameters.to(param).magnitude
    llama2_dn_ratio = llama2_70b_training_tokens / m_llama2.parameters.to(param).magnitude
