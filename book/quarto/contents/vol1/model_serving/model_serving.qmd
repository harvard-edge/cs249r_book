...
class MobileServingCalc:
    ...
    fps_str = fmt_int(round(1000 / m_total_ms.to(ms).magnitude), commas=False)
...
| **Total**              | **`{python} MobileServingCalc.m_total_ms_str`** | **`{python} MobileServingCalc.m_total_mj_str`** | `{python} MobileServingCalc.fps_str` FPS sustained |
...
1. **Energy Budget**\index{Energy!budget, mobile inference}: Each inference depletes battery. In the modeled pipeline, `{python} MobileServingCalc.m_total_mj_str` at `{python} MobileServingCalc.fps_str` FPS is about `{python} MobileServingCalc.sustained_mw_str` for the inference path alone; camera, display, ISP, and OS overhead add to that total in a full photo app. The optimization target shifts from throughput to energy-per-inference.
