...
class BlackFridayCalc:
    ...
    bf_spike_factor_str = fmt_multiple(bf_spike_factor_value, precision=0, commas=False)
    bf_collapse_latency_s_str = fmt(bf_collapse_latency_s_value, precision=0, commas=False, suffix=" s")
    bf_full_util_pct_value = 100
    bf_full_util_pct_str = fmt_percent(bf_full_util_pct_value/100, precision=0, commas=False, style='symbol')
```
...
**The event**: On Black Friday, traffic spikes `{python} BlackFridayCalc.bf_spike_factor_str`$\times$ to `{python} BlackFridayCalc.bf_qps_spike_str`.
...
class CostLatencyCalc:
    ...
    cost_increase_str = fmt_percent(cost_increase_pct_value/100, precision=0, commas=False, style='symbol', max_ratio=4)
    cost_ratio_str = fmt_multiple(cost_ratio_value, precision=0, commas=False)
```
...
**Trade-off**: Reducing latency from `{python} CostLatencyCalc.latency_b_ms_str` to `{python} CostLatencyCalc.latency_a_ms_str` increases the hardware bill by `{python} CostLatencyCalc.cost_increase_str`. Engineers must quantify whether that speedup generates enough business value to justify the `{python} CostLatencyCalc.cost_ratio_str`$\times$ cost increase.
...
class SerializationEfficiency:
    ...
    efficiency_gain_str = fmt_multiple(efficiency_gain, precision=0, commas=False)
```
...
class LatencyBudgetCalc:
    ...
    preprocess_pct_str = fmt_percent(preprocess_pct_value/100, precision=1, commas=False, style='symbol')
    tensorrt_preprocess_pct_str = fmt_percent(tensorrt_preprocess_pct_value/100, precision=1, commas=False, style='symbol')
```
...
class AmdahlServingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    non_model_pct_str = fmt_percent(non_model_pct_value/100, precision=1, commas=False, style='symbol')
    optimized_total_str = fmt(optimized_total_ms_value, precision=1, commas=False, suffix=" ms")
    amdahl_speedup_str = fmt_multiple(amdahl_speedup_value, precision=1, commas=False)
```
...
class ServingTaxMicrosecondsCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    micro_tax_min_pct_str = fmt_percent(micro_tax_min_pct/100, precision=1, commas=False, style='symbol')
    micro_tax_max_pct_str = fmt_percent(micro_tax_max_pct/100, precision=0, commas=False, style='symbol')
```
...
class AdaptiveResolutionCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    adaptive_throughput_improvement_str = fmt_multiple(adaptive_throughput_improvement_value, precision=1, commas=False)
    adaptive_accuracy_retention_str = fmt_percent(adaptive_accuracy_retention_value/100, precision=1, commas=False, style='symbol')
```
...
class BatchingTax:
    ...
    penalty_ratio_str = fmt_multiple(round(penalty_ratio), precision=0, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt_int, fmt, fmt_math, check, fmt_percent
...
class CapacityPlanningCalc:
    ...
    cp_gpus_ceil_str = fmt_multiple(cp_gpus_ceil_value, precision=0, commas=False)
    ...
    cp_util_after_fail_str = fmt_percent(cp_util_after_fail_value/100, precision=1, commas=False, style='symbol')
    ...
    cp_rho_safe_pct_str = fmt_percent(cp_rho_safe_value, precision=0, commas=False, style='symbol')
    ...
    cp_headroom_pct_str = fmt_percent(cp_headroom_pct_value/100, precision=0, commas=False, style='symbol')
    cp_headroom_multiplier_str = fmt_multiple(cp_headroom_value, precision=1, commas=False)
```
...
GPUs needed = `{python} CapacityPlanningCalc.cp_mu_required_str` / `{python} CapacityPlanningCalc.cp_v100_throughput_str` = `{python} CapacityPlanningCalc.cp_gpus_raw_str` → `{python} CapacityPlanningCalc.cp_gpus_ceil_str`$\times$

**Step 4**: Add headroom for variance.
Production systems add `{python} CapacityPlanningCalc.cp_headroom_pct_str` headroom for traffic spikes and variance: final count = `{python} CapacityPlanningCalc.cp_gpus_ceil_str`$\times$ `{python} CapacityPlanningCalc.cp_headroom_multiplier_str`$\times$ = `{python} CapacityPlanningCalc.cp_final_raw_str`, rounded up to `{python} CapacityPlanningCalc.cp_final_ceil_str`.
...
class BatchThroughputCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    throughput_ratio_str = fmt_multiple(throughput_ratio_value, precision=1, commas=False)
...
class BatchingBudgetCalc:
    ...
    budget_pct_str = fmt_percent(budget_pct_value/100, precision=0, commas=False, style='symbol')
```
...
class BatchingAnalysisCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    throughput_increase_str = fmt_multiple(throughput_increase_value, precision=1, commas=False)
    ...
    il_eff_b1_str = fmt_percent(il_eff_b1_pct_value/100, precision=0, commas=False, style='symbol')
    il_eff_b32_str = fmt_percent(il_eff_b32_pct_value/100, precision=0, commas=False, style='symbol')
    il_threshold_str = fmt_percent(il_threshold_pct_value/100, precision=0, commas=False, style='symbol')
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int,  MarkdownStr, check, fmt, fmt_percent
...
class BatchingOptimization:
    ...
    throughput_gain_pct_str = fmt_percent(tput_gain/100, precision=1, commas=False, style='symbol')
...
class SloViolationCalc:
    ...
    p99_ratio_str = fmt_multiple(p99_to_mean_value, precision=2, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, fmt_qty, check, fmt_percent
...
class CarbonCostCalc:
    ...
    cc_low_util_str = fmt_percent(cc_low_util_pct_value/100, precision=0, commas=False, style='symbol')
...
class PrecisionTradeoffCalc:
    ...
    pt_fp32_acc_str = fmt_percent(pt_fp32_acc_value/100, precision=2, commas=False, style='symbol')
    ...
    pt_fp16_acc_str = fmt_percent(pt_fp16_acc_value/100, precision=2, commas=False, style='symbol')
    ...
    pt_int8_ptq_acc_str = fmt_percent(pt_int8_ptq_acc_value/100, precision=2, commas=False, style='symbol')
    pt_int8_qat_acc_str = fmt_percent(pt_int8_qat_acc_value/100, precision=2, commas=False, style='symbol')
...
class FallacyLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fl_utilization_high_pct_str = fmt_percent(fl_utilization_high_value, precision=0, commas=False, style='symbol')
    ...
    fl_utilization_new_pct_str = fmt_percent(fl_utilization_new_value, precision=0, commas=False, style='symbol')
    ...
    fl_queuing_improvement_str = fmt_multiple(fl_queuing_improvement_value, precision=1, commas=False)
    ...
    fl_system_speedup_str = fmt_multiple(fl_system_speedup_value, precision=1, commas=False)
    fl_model_speedup_str = fmt_multiple(fl_model_speedup_value, precision=1, commas=False)
```
...
class FallacyUtilizationCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fu_util_high_pct_str = fmt_percent(fu_util_high_value, precision=0, commas=False, style='symbol')
    fu_util_mod_pct_str = fmt_percent(fu_util_mod_value, precision=0, commas=False, style='symbol')
    fu_wait_high_factor_str = fmt_multiple(fu_wait_high_factor_value, precision=0, commas=False)
    fu_total_high_factor_str = fmt_multiple(1/(1-fu_util_high_value), precision=0, commas=False)
    fu_cost_reduction_str = fmt_percent(fu_cost_reduction_pct_value/100, precision=1, commas=False, style='symbol')
    fu_latency_increase_str = fmt_multiple(fu_latency_increase_factor_value, precision=0, commas=False)
...
# │          fs_resize_drop_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import MarkdownStr, fmt, fmt_percent
...
class FallacySkewCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fs_val_acc_str = fmt_percent(fs_val_acc_value/100, precision=0, commas=False, style='symbol')
    fs_prod_acc_str = fmt_percent(fs_prod_acc_value/100, precision=0, commas=False, style='symbol')
...
class TailLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tl_util_pct_str = fmt_percent(tl_util_value, precision=0, commas=False, style='symbol')
    ...
    tl_gap_str = fmt_multiple(tl_gap_value, precision=1, commas=False)
```
...
class FallacyBatchingCalc:
    ...
    fb_padding_waste_min_str = fmt_percent(fb_padding_waste_min_pct_value/100, precision=0, commas=False, style='symbol')
    fb_padding_waste_max_str = fmt_percent(fb_padding_waste_max_pct_value/100, precision=0, commas=False, style='symbol')
```
...
class FallacyCalibrationCalc:
    ...
    fc_imagenet_acc_str = fmt_percent(fc_imagenet_acc_value/100, precision=1, commas=False, style='symbol')
    fc_ood_acc_str = fmt_percent(fc_ood_acc_value/100, precision=1, commas=False, style='symbol')
```
