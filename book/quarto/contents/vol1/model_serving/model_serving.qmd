...
class CapacityPlanningCalc:
    ...
    cp_mu_required_req_per_s_math_fragment = cp_mu_required_req_per_s_str.replace(
    " req/s", r"\,\mathrm{req}/\mathrm{s}"
    )
    cp_peak_qps_math_fragment = cp_peak_qps_str.replace(
    " QPS", r"\,\mathrm{QPS}"
    )
    ...
    f"\\mu_{{\\text{{required}}}} = "
    f"{cp_peak_qps_math_fragment} / {cp_rho_safe_str} = "
    f"{cp_mu_required_req_per_s_math_fragment}"
)
