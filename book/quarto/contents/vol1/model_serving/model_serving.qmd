...
::: {#fig-tail-latency-explosion fig-env="figure" fig-pos="htb" fig-cap="**The Tail Latency Explosion**: Request latency vs. system utilization (ρ). While mean latency (blue) remains moderate, tail latency (red, p99) explodes once utilization passes the knee at ~70 percent. This uses a simple M/M/1 approximation (p99 ≈ 4.6$\times$ mean), so the curve is illustrative rather than workload-specific." fig-alt="Line plot showing latency growing with utilization. Blue line (mean) rises gradually then steeply. Red line (Tail, p99) curves upward sharply at 70 percent utilization. Shaded regions indicate safe zone and danger zone."}
...
The model\index{Serving!cloud/data center}\index{Microservice!model serving} runs as a standalone service (microservice), the deployment paradigm @sec-ml-systems-cloud-ml-maximizing-computational-power-a338 characterized as trading latency for virtually unlimited compute. The primary interface is the network (HTTP/gRPC), so the binding constraints are network bandwidth and serialization cost before the request even reaches the accelerator. Data-center hardware such as NVIDIA GPUs (V100, A100, H100), Google Tensor Processing Units (TPUs), and AWS Inferentia supports high-throughput batching and concurrency, but cold start can still stretch from seconds to minutes because container startup, model loading, and warmup sit outside the steady-state inference path.\index{Cold Start!cloud serving}
...
# │ Exports: serial_floats_str, json_size_str, json_parse_us_str, protobuf_size_str,
# │          protobuf_parse_us_str, requests_per_sec_str, efficiency_gain_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: (none)
# │ Exports: l_jpeg_ms_str, l_resize_ms_str, l_norm_ms_str, l_transfer_ms_str, l_inf_ms_str,
# │          l_post_ms_str, l_total_ms_str, p_jpeg_str, p_resize_str, p_norm_str,
...
# │          tensorrt_inference_ms_str, model_10x_ms_str, total_latency_ms_str,
...
| **Inference**      | **ResNet-50 forward pass**\index{ResNet-50!serving spectrum}\index{ResNet-50!latency budget} |  **`{python} LatencyTableCalc.l_inf_ms_str`**  | **`{python} LatencyTableCalc.p_inf_str`**  |
...
# │
# │ Imports: (none)
# │ Exports: dlrm_input_ms_str, dlrm_embed_ms_str, dlrm_mlp_ms_str, dlrm_post_ms_str,
# │          dlrm_total_ms_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.book (fmt)
# │ Exports: non_model_pct_str, optimized_total_ms_str, amdahl_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_percent, fmt_time
...
# │ Exports: r1_str, r2_str, theoretical_slowdown_str, measured_slowdown_str
# └─────────────────────────────────────────────────────────────────────────────
...
class ResolutionScalingCalc:
    ...
    theoretical_slowdown_str = fmt_multiple(theoretical_slowdown_value, precision=0, commas=False)
    measured_slowdown_str = fmt_multiple(measured_slowdown_factor_value, precision=1, commas=False)
```

Doubling resolution from `{python} ResolutionScalingCalc.r1_str` to `{python} ResolutionScalingCalc.r2_str` theoretically yields `{python} ResolutionScalingCalc.theoretical_slowdown_str`$\times$ slowdown (measured: `{python} ResolutionScalingCalc.measured_slowdown_str`$\times$ due to fixed overhead amortization). However, at high resolutions, models move toward memory-bandwidth sensitivity as activation tensors exceed cache capacity, with arithmetic intensity decreasing toward the roofline ridge point.
...
# │
# │ Imports: mlsysim.book (fmt)
# │ Exports: act_*_mb_str, ai_*_str, ridge_point_flops_per_byte_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_arithmetic_intensity, fmt_memory
...
class ResolutionBottleneckCalc:
    ...
    ridge_point_flops_per_byte_str = fmt_arithmetic_intensity(ridge_point_value, commas=False)
```
...
| **$640{\times}640$** | `{python} ResolutionBottleneckCalc.act_640_mb_str` | `{python} ResolutionBottleneckCalc.ai_640_flops_per_byte_str` |   Near ridge   |
...
# │
# │ Imports: mlsysim.core.constants, mlsysim.book
# │ Exports: littles_lambda_qps_str, littles_w_ms_str, littles_w_s_str, littles_l_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int,  MarkdownStr, check, fmt, fmt_time
...
* **Throughput target $(\lambda)$**: `{python} CapacityPlanning.littles_lambda_qps_str`.
* **Latency target $(T_{\text{lat}})$**: `{python} CapacityPlanning.littles_w_ms_str` (`{python} CapacityPlanning.littles_w_s_str`).

**Math**:

$N_{\text{req}}$ = `{python} CapacityPlanning.littles_lambda_qps_str` $\times$ `{python} CapacityPlanning.littles_w_s_str` = `{python} CapacityPlanning.littles_l_str` concurrent requests
...
class CapacityPlanningCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cp_mu_required_req_s_str = fmt_rate(cp_mu_required_value, 'req/s', precision=1, commas=False)
    ...
    cp_n_plus_2_min_gpus_str = fmt_count(
        cp_n_plus_2_min_value, precision=0, commas=False, label="GPU"
    )
    cp_n_plus_2_headroom_gpus_str = fmt_count(
        cp_n_plus_2_headroom_value, precision=0, commas=False, label="GPU"
    )
    ...
    cp_mu_required_math = fmt_math(
        ...
        f"= {cp_mu_required_req_s_str} \\text{{ requests/s}}"
    )
...
GPUs needed = `{python} CapacityPlanningCalc.cp_mu_required_req_s_str` / `{python} CapacityPlanningCalc.cp_v100_throughput_str` = `{python} CapacityPlanningCalc.cp_gpus_raw_str` → `{python} CapacityPlanningCalc.cp_gpus_ceil_str` GPUs
...
# │          batch32_total_ms_str, batch1_inference_total_ms_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.book (fmt)
# │ Exports: avg_wait_ms_str, budget_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_percent, fmt_time
...
# │
# │ Imports: mlsysim.core.constants (ms, second), mlsysim.book (fmt)
# │ Exports: throughput_*, b1_*, b32_*, il_*, T_window_ms_str,
...
|:--------------:|:---------------------------------------------:|:--------------------------------------------------:|:--------------------------------------------------:|:--------------:|
|       1        |  `{python} BatchingAnalysisCalc.st_b1_ms_str` | `{python} BatchingAnalysisCalc.b1_latency_ms_str`  | `{python} BatchingAnalysisCalc.b1_throughput_str`  |      Low       |
|       4        |  `{python} BatchingAnalysisCalc.st_b4_ms_str` |    `{python} BatchingAnalysisCalc.tl_b4_ms_str`    |     `{python} BatchingAnalysisCalc.tp_b4_str`      |    Moderate    |
|       8        |  `{python} BatchingAnalysisCalc.st_b8_ms_str` |    `{python} BatchingAnalysisCalc.tl_b8_ms_str`    |     `{python} BatchingAnalysisCalc.tp_b8_str`      |      Good      |
|       16       | `{python} BatchingAnalysisCalc.st_b16_ms_str` |   `{python} BatchingAnalysisCalc.tl_b16_ms_str`    |     `{python} BatchingAnalysisCalc.tp_b16_str`     |      High      |
|       32       | `{python} BatchingAnalysisCalc.st_b32_ms_str` | `{python} BatchingAnalysisCalc.b32_latency_ms_str` | `{python} BatchingAnalysisCalc.b32_throughput_str` |    Maximum     |

: **Batching Throughput Analysis**: ResNet-50 throughput on V100 with `{python} BatchingAnalysisCalc.T_window_ms_str` batching window. Throughput increases `{python} BatchingAnalysisCalc.throughput_increase_str`$\times$ from batch size one to 32 (`{python} BatchingAnalysisCalc.b1_throughput_str` to `{python} BatchingAnalysisCalc.b32_throughput_str`), but total latency more than doubles (`{python} BatchingAnalysisCalc.b1_latency_ms_str` to `{python} BatchingAnalysisCalc.b32_latency_ms_str`). The optimal configuration depends on whether the latency SLO or throughput target is the binding constraint. {#tbl-batching-throughput tbl-colwidths="[20,20,20,20,20]"}
...
# │ Exports: qps_str, T_slo_ms_str, mean_wait_ms_str, mean_batch_str,
# │          mean_service_ms_str, mean_latency_ms_str, p99_service_ms_str,
# │          p99_latency_ms_str, p99_ratio_str, p99_batch_str
# └─────────────────────────────────────────────────────────────────────────────
...
where $B_{\text{p99}}$ is the 99th percentile batch size. For $\lambda$ = `{python} SloViolationCalc.qps_str` and $T_{\text{window}}$ = `{python} SloViolationCalc.T_slo_ms_str`:
...
# │ Exports: cc_concurrent_str, cc_tokens_req_tokens_s_str, cc_total_tokens_tokens_s_str,
# │          cc_host_overhead_w_str, cc_total_power_w_str, cc_joules_token_j_str,
# │          cc_response_tokens_str, cc_response_joules_j_str, cc_smartphone_j_str,
# │          cc_boiling_j_str, cc_low_util_str, cc_idle_power_w_str, cc_low_util_joules_j_str
# └─────────────────────────────────────────────────────────────────────────────
...
class CarbonCostCalc:
    ...
    cc_tokens_req_tokens_s_str = fmt_rate(cc_tokens_per_sec_req_value, 'tokens/s', precision=0, commas=False)
    cc_total_tokens_tokens_s_str = fmt_rate(round(cc_total_tokens_sec.to(count / second).magnitude), 'tokens/s', precision=0, commas=False)
    ...
    cc_joules_token_j_str = fmt_qty(cc_joules_per_token, joule, precision=2, commas=False, per='token')
...
1.  **Throughput**: `{python} CarbonCostCalc.cc_concurrent_str` concurrent requests $\times$ `{python} CarbonCostCalc.cc_tokens_req_tokens_s_str` per request ≈ `{python} CarbonCostCalc.cc_total_tokens_tokens_s_str`.
2.  **Power**: `{python} CarbonCostH100Tdp.h100_tdp_str` W (GPU) + `{python} CarbonCostCalc.cc_host_overhead_w_str` (Host/Overhead) = `{python} CarbonCostCalc.cc_total_power_w_str`.
3.  **Energy per Token**:

    `{python} CarbonCostCalc.cc_total_power_w_str` / `{python} CarbonCostCalc.cc_total_tokens_tokens_s_str` ≈ `{python} CarbonCostCalc.cc_joules_token_j_str`

**Systems insight**: A typical `{python} CarbonCostCalc.cc_response_tokens_str`-token response consumes ≈ `{python} CarbonCostCalc.cc_response_joules_j_str`.
...
*The engineering lever*: The primary way to reduce J/token is to *increase hardware utilization* and *eliminate redundant compute*. If the GPU sits at `{python} CarbonCostCalc.cc_low_util_str` utilization due to poor batching, the "Idle Power" is still ~`{python} CarbonCostCalc.cc_idle_power_w_str`, causing the energy-per-token to skyrocket to >`{python} CarbonCostCalc.cc_low_util_joules_j_str`. Furthermore, architectural optimizations like prefix caching skip the energy-intensive prefill phase for shared context, directly reducing the energy footprint of RAG and chat applications. MLOps is not just about speed; it is about sustainability through efficiency.
...
When latency or hardware cost binds more tightly than portability, TensorRT\index{Inference!engine, specialized}\index{TensorRT!GPU optimization}[^fn-tensorrt-gpu-serving] (NVIDIA GPUs), OpenVINO[^fn-openvino-serving]\index{OpenVINO!Intel optimization} (Intel hardware), and similar engines optimize specifically for their target hardware [@nvidia2024tensorrt; @chen2018tvm]. They apply aggressive optimizations that framework-native runtimes cannot safely perform:
...
class RuntimeComparisonCalc:
    ...
    rt_pytorch_speedup_str = fmt_multiple(rt_pytorch_speedup_value, precision=0, commas=False)
    rt_torchscript_speedup_str = fmt_multiple(rt_torchscript_speedup_value, precision=1, commas=False)
    rt_onnx_speedup_str = fmt_multiple(rt_onnx_speedup_value, precision=1, commas=False)
    rt_trt_fp32_speedup_str = fmt_multiple(round(rt_trt_fp32_speedup_value), commas=False, precision=0)
    rt_trt_fp16_speedup_str = fmt_multiple(rt_trt_fp16_speedup_value, precision=1, commas=False)
    rt_trt_int8_speedup_str = fmt_multiple(rt_trt_int8_speedup_value, precision=1, commas=False)
```
...
class PrecisionTradeoffCalc:
    ...
    pt_int8_speedup_str = fmt_multiple(pt_int8_speedup_value, precision=1, commas=False)
    pt_fp16_speedup_str = fmt_multiple(pt_fp16_speedup_value, precision=0, commas=False)
    pt_int8_vs_fp16_speedup_str = fmt_multiple(pt_int8_vs_fp16_speedup_value, precision=1, commas=False)
...
At that lower layer, node-level optimization identifies which boundary is wasting time on this machine. The answer may be the computation graph\index{Graph Compilation!optimization}, CPU execution when GPUs are absent, bytes moving from disk to memory, or gaps visible only in a profiling trace. The goal is not to apply every optimization, but to match the measured bottleneck to the smallest intervention that removes it.
...
Tools\index{Timeline Profiling!serving optimization} like PyTorch Profiler\index{Framework Profiler!timeline analysis} or NVIDIA Nsight Systems (nsys)\index{GPU Profiler!timeline analysis} generate a timeline trace. This visualization reveals the exact sequence of events on the CPU and GPU. A useful trace reading is bottleneck-first: each visible artifact points to a different class of fix.

1.  **Gaps in the GPU timeline**: Empty spaces in the GPU bar indicate idle hardware. This usually means the GPU is waiting for the CPU (preprocessing bottleneck) or disk (data loading).
...
3.  **Host-to-device transfers**: `MemcpyHtoD` (Host to Device) blocks reveal input movement. The diagnostic distinction is whether those transfers overlap with computation or block it.
...
class CostAnalysisCalc:
    ...
    ca_v100_price_increase_str = fmt_multiple(ca_v100_price_increase_value, precision=1, commas=False)
```
...
# │
# │ Imports: numpy (np), book.tools.figures.style (book_style), mlsysim.constants (BYTES_FP16, byte, GB)
# │ Exports: (figure only, no prose variables)
# └─────────────────────────────────────────────────────────────────────────────
...
# │ Exports: LlmCaseStudyHwSpecs.h100_mem_gb_str, LlmCaseStudyHwSpecs.a100_bw_tbs_str,
...
*   **Hardware**: 1$\times$ NVIDIA H100 SXM5 GPU (`{python} LlmCaseStudyHwSpecs.h100_mem_gb_str` HBM3, `{python} LlmCaseStudyH100BwRecap.h100_bw_tbs_str` bandwidth).
...
Every token generation pays a massive "energy tax" to move the model's logic from HBM into compute registers. For comparison, on an A100 `{python} LlmCaseStudyHwSpecs.a100_mem_gb_str` (`{python} LlmCaseStudyHwSpecs.a100_bw_tbs_str` HBM2e), an 8-billion-parameter Llama 3 model (`{python} LlmCaseStudyHwSpecs.case_study_model_weight_gb_str` INT4) generates tokens at $\approx$ `{python} LlmCaseStudyHwSpecs.a100_token_ms_str` per token. Adding more *compute cores* yields *zero* latency improvement; only *faster memory* (Physics) or *smaller models* (Algorithm) can speed up generation.
...
# │          req_time_s_str, prefill_tokens_per_s_tokens_s_str,
# │          prefill_req_per_s_req_s_str, decode_req_capacity_req_s_str,
...
class LlmServingCalc:
    ...
    prefill_tokens_per_s_tokens_s_str = fmt_rate(prefill_tokens_per_s_value, 'tokens/s', precision=0)
    prefill_req_per_s_req_s_str = fmt_rate(prefill_req_per_s_value, 'req/s', precision=0, commas=False)
    decode_req_capacity_req_s_str = fmt_rate(round(decode_req_capacity_value), 'req/s', precision=0, commas=False)
...
*   Prefill-limited admissions: `{python} LlmServingCalc.prefill_tokens_per_s_tokens_s_str` divided by `{python} LlmServingCalc.prompt_tokens_str` = `{python} LlmServingCalc.prefill_req_per_s_req_s_str`. This is below the KV-residency limit of `{python} LlmServingCalc.concurrent_batch_str` / `{python} LlmServingCalc.req_time_s_str` ≈ `{python} LlmServingCalc.decode_req_capacity_req_s_str`, so full-request throughput is `{python} LlmServingCalc.prefill_req_per_s_req_s_str` $\times$ `{python} LlmServingCalc.sec_per_hr_str` $\times$ `{python} LlmServingCalc.tokens_per_req_str` ≈ `{python} LlmServingCalc.tokens_per_hour_m_str`.
...
# │
# │ Imports: mlsysim.book (fmt)
# │ Exports: cs_new_instances_str, cs_compile_time_s_str, cs_aggregate_cold_str,
# │          cs_parallel_warm_s_str, cs_steady_latency_ms_str, cs_cold_latency_ms_str,
# │          cs_cold_multiplier_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_count, fmt_time
...
class FallacyColdstartCalc:
    ...
    cs_cold_multiplier_str = fmt_multiple(cs_cold_multiplier_value, precision=0, commas=False)
```

Engineers optimize steady-state latency\index{Cold Start!bursty traffic impact} assuming most requests hit warm instances. In production, cold starts compound during the events that matter most: traffic spikes requiring scale-up, deployments rolling out new versions, and recovery from instance failures. @Sec-model-serving-model-loading-initialization-cc5a details the anatomy of cold start: TensorRT compilation alone takes `{python} FallacyColdstartCalc.cs_compile_time_s_str` per instance. During a traffic spike requiring `{python} FallacyColdstartCalc.cs_new_instances_str` new instances, aggregate cold-start work reaches `{python} FallacyColdstartCalc.cs_aggregate_cold_str`; if the instances warm in parallel, new capacity becomes useful after about `{python} FallacyColdstartCalc.cs_parallel_warm_s_str`. Worse, requests hitting cold instances experience `{python} FallacyColdstartCalc.cs_cold_latency_ms_str` latency vs. `{python} FallacyColdstartCalc.cs_steady_latency_ms_str` steady-state, a `{python} FallacyColdstartCalc.cs_cold_multiplier_str`$\times$ degradation that violates SLOs precisely when traffic is highest. Systems ignoring cold start meet SLOs during steady state but fail during scale-up events and deployment windows when reliability matters most.
