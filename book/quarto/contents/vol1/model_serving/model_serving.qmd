...
The `{python} CapacityPlanningCalc.cp_headroom_pct_str` headroom addresses traffic variance, but production systems also need fault tolerance. With `{python} CapacityPlanningCalc.cp_final_ceil_str` GPUs, losing one leaves `{python} CapacityPlanningCalc.cp_gpus_after_fail_str` GPUs handling `{python} CapacityPlanningCalc.cp_peak_qps_str`. The postfailure utilization is (`{python} CapacityPlanningCalc.cp_peak_qps_str` / `{python} CapacityPlanningCalc.cp_v100_throughput_str`) / `{python} CapacityPlanningCalc.cp_gpus_after_fail_str` = `{python} CapacityPlanningCalc.cp_util_after_fail_str`.

This remains well below the `{python} CapacityPlanningCalc.cp_rho_safe_pct_str` safe utilization threshold, confirming N+1 redundancy is satisfied. For stricter fault tolerance requirements, N+2 redundancy (tolerating two simultaneous failures) would require `{python} CapacityPlanningCalc.cp_n_plus_2_min_str` GPUs under the same safe-utilization threshold, or about `{python} CapacityPlanningCalc.cp_n_plus_2_headroom_str` GPUs if the `{python} CapacityPlanningCalc.cp_headroom_pct_str` headroom must remain after two simultaneous failures.
...
class TrafficAdaptiveBatchingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    qps_100_str = fmt(rows[0][0], precision=0, suffix=" QPS")
    window_100_ms_str = fmt((rows[0][1] / (1 * ms).m_as(second)), precision=1, commas=False, suffix=" ms")
    avg_batch_100_str = fmt(rows[0][2], precision=1, commas=False)
    latency_100_ms_str = fmt(rows[0][3], precision=1, commas=False, suffix=" ms")

    qps_500_str = fmt(rows[1][0], precision=0, suffix=" QPS")
    window_500_ms_str = fmt((rows[1][1] / (1 * ms).m_as(second)), precision=1, commas=False, suffix=" ms")
    avg_batch_500_str = fmt(rows[1][2], precision=1, commas=False)
    latency_500_ms_str = fmt(rows[1][3], precision=1, commas=False, suffix=" ms")

    qps_1000_str = fmt(rows[2][0], precision=0, suffix=" QPS")
    window_1000_ms_str = fmt((rows[2][1] / (1 * ms).m_as(second)), precision=0, commas=False, suffix=" ms")
    avg_batch_1000_str = fmt(rows[2][2], precision=0, commas=False)
    latency_1000_ms_str = fmt(rows[2][3], precision=0, commas=False, suffix=" ms")

    qps_5000_str = fmt(rows[3][0], precision=0, suffix=" QPS")
    window_5000_ms_str = fmt((rows[3][1] / (1 * ms).m_as(second)), precision=1, commas=False, suffix=" ms")
    avg_batch_5000_str = fmt(rows[3][2], precision=1, commas=False)
    latency_5000_ms_str = fmt(rows[3][3], precision=1, commas=False, suffix=" ms")
```
...
class RuntimeComparisonCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    rt_pytorch_ms_str = fmt(rt_pytorch_ms_value, precision=1, commas=False, suffix=" ms")
    rt_torchscript_ms_str = fmt(rt_torchscript_ms_value, precision=1, commas=False, suffix=" ms")
    rt_onnx_ms_str = fmt(rt_onnx_ms_value, precision=1, commas=False, suffix=" ms")
    rt_trt_fp32_ms_str = fmt(rt_trt_fp32_ms_value, precision=1, commas=False, suffix=" ms")
    rt_trt_fp16_ms_str = fmt(rt_trt_fp16_ms_value, precision=1, commas=False, suffix=" ms")
    rt_trt_int8_ms_str = fmt(rt_trt_int8_ms_value, precision=1, commas=False, suffix=" ms")
