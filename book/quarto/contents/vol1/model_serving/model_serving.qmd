...
class CapacityPlanningCalc:
    ...
    cp_rho_safe_math = fmt_math(f"\\rho \\leq 1 - ({mm1_p99_factor_str} \\times {cp_service_ms_str})/" f"{cp_p99_target_ms_str} = {cp_rho_safe_str}")
    cp_mu_required_math = fmt_math(
        f"\\mu_{{\\text{{required}}}} = {cp_peak_qps_str} / {cp_rho_safe_str} " f"= {cp_mu_required_req_per_s_str} \\text{{ requests/s}}"
    )
...
- [ ] **Headroom estimate**: To hold p99 under 50 ms at 2,000 requests per second, estimate the average in-flight request count $N_{\text{req}} = \lambda T_{\text{lat}}$ implies, and how fast that margin shrinks as $\rho$ passes 0.7.
