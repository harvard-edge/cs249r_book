...
**Scenario**: Loading a 5 GB Stable Diffusion model:

*   **Pickle (`torch.load`)**: ~15 seconds. High CPU usage.
*   **Safetensors**: ~0.5 seconds. Near-zero CPU usage.

**Systems insight**: By using `mmap` and formats like `safetensors`, loading speed becomes limited only by the disk's read speed (for example, 3 GB/s for NVMe), rather than CPU parsing overhead. For autoscaling serving fleets, serialization format becomes part of the latency budget.
...
**Systems lesson**: Profiling is an iterative measurement loop: capture a representative trace, identify the dominant idle block, change one bottleneck, and verify with a new trace. The optimization is credible only when the timeline changes in the expected direction.
