...
**Systems lesson**: High average throughput does not prevent queueing collapse under traffic surges. Maintaining tail latency SLOs requires proactive load shedding, dynamic model degradation, and autoscaling before utilization hits queueing knees.
...
**Systems lesson**: Model architectures cheap to train can prove unviable to serve at fleet scale. Production serving infrastructure requires hardware-software co-design to balance tail latency, batch size, and recurring operational TCO.
...
Concretely, for a target arrival rate of `{python} CapacityPlanningAnchor.qps_str` and a mean response time of `{python} CapacityPlanningAnchor.mean_latency_ms_str`, Little's Law translates that pair into the average in-system population; it does not set a minimum GPU batch size or a hard activation-memory floor. The worked example below carries out that calculation.
...
class LlmCaseStudyHwSpecs:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_mem_gb_str = fmt_memory_capacity(h_a100.memory.capacity, unit=GB, commas=False)
    a100_bw_tb_per_s_str = fmt_bandwidth(a100_bw, unit=TB / second, precision=2, commas=False)
    h100_mem_gb_str = fmt_memory_capacity(h_h100.memory.capacity, unit=GB, commas=False)
...
class LlmServingCalc:
    ...
    kv_cache_gb_str = fmt_memory_capacity(kv_cache_reserved, unit=GB, precision=1, commas=False)
