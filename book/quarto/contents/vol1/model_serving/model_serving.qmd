...
2.  **Distinction**: Unlike average median latency ($\text{p50}$), which hides variance and long-tail outliers, a Service Level Objective (**SLO**)\index{SLO!vs. SLI vs. SLA} couples a hard deadline (e.g. $<50\text{ ms}$) with a high completion quantile ($\text{p99}$ or $\text{p99.9}$). System performance is measured by Service Level Indicators (**SLI**\index{SLI!definition}, the actual monitored percentile value); when $\text{SLI} > \text{SLO}$, the system violates its internal objective, and exceeding contractual thresholds triggers Service Level Agreement (**SLA**\index{SLA!contractual penalty}) financial penalties. Tail latency spikes often stem from GPU queue delays, batch formation waiting, or host-to-device data copies.
...
Concretely, if we target `{python} CapacityPlanningAnchor.qps_str` and measure or estimate a mean response time of `{python} CapacityPlanningAnchor.mean_latency_ms_str`, Little's Law lets us translate that pair into the average in-system population; it does not set a minimum GPU batch size or a hard activation-memory floor. The worked example below carries out that calculation.
