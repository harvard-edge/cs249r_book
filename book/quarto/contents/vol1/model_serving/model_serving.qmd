...
Binary formats like Protocol Buffers[^fn-protobuf-serialization] (Protobuf) or FlatBuffers[^fn-flatbuffers-zerocopy] eliminate this bloat by designing the wire format to map directly to in-memory data structures. Native float arrays transmit as compact IEEE 754 bytes with no ASCII conversion and no Base64 wrapper. This enables "zero-copy" deserialization in optimal cases, where the network buffer can be used directly without allocating new memory.
...
Serving optimizes tail latency under load. You can answer the first question from the latency budget just introduced; the other two preview analyses developed later in this chapter.
...
- [ ] **Queuing theory**: Can you anticipate why latency spikes nonlinearly as utilization approaches 100 percent? @Sec-model-serving-queuing-theory-tail-latency-29a6 derives it.
- [ ] **Batching trade-offs**: Can you anticipate why a larger batch raises throughput in images per second yet can degrade latency in milliseconds per request? @Sec-model-serving-throughput-optimization-18d1 works it through.
...
# │
# │ Imports: mlsysim.core.units, mlsysim.book
# │ Exports: qps_str, qps_display_str, slo_ms_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, MarkdownStr, check, fmt, fmt_time
...
The next CPU boundary is locality. On multi-socket servers[^fn-numa-cpu-serving], accessing memory attached to a different CPU socket (NUMA\index{NUMA!memory locality}) adds significant latency. An inference server must therefore be NUMA-aware: threads\index{Thread Pinning!CPU affinity} should be pinned to specific cores, and the model weights and input buffers those threads touch should be allocated on the same socket. The reason NUMA is more damaging for ML inference than for a typical microservice is that ML model weights—hundreds of megabytes for a mid-sized network, gigabytes for a large language model—massively exceed the capacity of a CPU's L3 cache. Because every inference pass must read the full weight tensor, the working set never fits in cache, producing guaranteed L3 cache thrashing and forcing constant fetches from main RAM. A general-purpose microservice may hold its hot data in L3 and take only rare NUMA penalties; an ML inference workload takes a NUMA penalty on nearly every weight access, making the remote-memory latency tax persistent rather than occasional.
