...
**Scenario**: A GPU serving node has high P99 latency even though average utilization looks acceptable. A trace of twenty-five warm requests shows large blank regions in the GPU timeline between short kernels.

**System**: The trace indicates that CPU-side request preparation and kernel launch overhead are starving the accelerator. The team targets the largest visible idle gap, applies one specific change such as operator fusion or pinned host memory, then captures another trace to see whether that gap shrank.

**Lesson**: Profiling is an iterative measurement loop: capture a representative trace, identify the dominant idle block, change one bottleneck, and verify with a new trace. The optimization is credible only when the timeline changes in the expected direction.
