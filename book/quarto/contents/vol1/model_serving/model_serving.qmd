...
# │          cc_boiling_j_str, cc_low_util_pct_str, cc_idle_power_w_str, cc_low_util_joules_j_str
# └─────────────────────────────────────────────────────────────────────────────
...
class CarbonCostCalc:
    ...
    cc_low_util_pct_str = fmt_percent(cc_low_util_pct_value/100, precision=0, commas=False, style='symbol')
...
**Problem**: How much energy does an H100-backed chat service spend per generated token and per `{python} CarbonCostCalc.cc_response_tokens_str` response?

As LLMs scale, joules per token becomes a first-class operational metric alongside latency. For an H100 GPU (`{python} CarbonCostH100Tdp.h100_tdp_str` W TDP), the energy footprint follows from throughput and power:
