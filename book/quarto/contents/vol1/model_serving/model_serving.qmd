...
Just-in-time compilation\index{JIT Compilation!runtime optimization}, such as `torch.compile`, waits until the graph is first executed and can specialize to the shapes it observes. That specialization is useful for variable traffic, but it moves compiler work into the serving path and creates a cold-request latency spike. Ahead-of-time compilation\index{Ahead-of-Time Compilation!predeployment}, such as `torch.export` or TensorRT `trtexec`, performs the compiler work before deployment. It gives the service a fixed graph and avoids startup compilation latency, at the cost of defining all dynamic shapes explicitly or compiling multiple profiles.
...
# │ Exports: model_size_str, pickle_load_s_str, safetensors_load_s_str,
...
class SafetensorsLoadingSpeed:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    model_size_str = fmt_memory(model_size, unit=GB, precision=0, commas=False)
...
Loading a `{python} SafetensorsLoadingSpeed.model_size_str` Stable Diffusion model through Pickle (`torch.load`) can take about `{python} SafetensorsLoadingSpeed.pickle_load_s_str` while consuming substantial CPU time because Python has to reconstruct objects before tensors are usable. The same weights stored with Safetensors can load in about `{python} SafetensorsLoadingSpeed.safetensors_load_s_str` with near-zero CPU usage, a `{python} SafetensorsLoadingSpeed.load_speedup_mult_str` improvement in this example. By using `mmap` and formats like `safetensors`, loading speed becomes limited mainly by the disk's read speed, for example `{python} SafetensorsLoadingSpeed.nvme_bw_gb_per_s_str` for local Gen3 NVMe, rather than by CPU parsing overhead.
...
Run a warmup, capture a trace of ten to fifty representative requests, and open the trace in Chrome Tracing, Nsight, or the runtime's profiler. The largest gap or longest block identifies the first intervention: fusion for kernel-launch fragmentation, pinning for CPU locality, zero-copy loading for startup, or scheduling changes for host-device stalls. After applying the fix, capture the trace again. The loop ends only when the targeted gap has actually disappeared.
