...
: **The Serving Tax Bill**: A representative breakdown of noninference latency sources. While individual components like serialization seem small ($<1$ ms), they compound. In a 5 ms inference service, this "tax" can consume a substantial share of the latency budget. The primary engineering goal is to reduce these costs through architectural choices like binary protocols, persistent connections, and zero-copy data paths. {#tbl-serving-tax}
...
[^fn-mm1-erlang-serving]: **M/M/1 Queue**\index{M/M/1 Queue!wait time formula}\index{M/M/1 Queue!Erlang origin}: Queuing theory originated with Agner Krarup Erlang's 1909 analysis of the Copenhagen Telephone Exchange, where call arrivals genuinely were memoryless (Poisson). The M/M/1 model's exponential service time assumption fit telephony well but overpredicts service-time variance for many fixed-shape ML inference workloads. This mismatch is useful for intuition: M/M/1 gives a conservative mean-delay picture compared with a deterministic-service model such as M/D/1, so capacity planning based on it tends to preserve more headroom.
...
When multiple models or multiple instances of the same model must run concurrently on a single GPU, the hardware must partition resources between them. NVIDIA's Multi-Instance GPU[^fn-mig-multimodel]\index{MIG!hardware isolation} technology enables hardware-level isolation, dividing supported data-center GPUs into independent GPU instances with dedicated memory and compute resources [@nvidiaMigUserGuide]. For older GPUs such as V100 or T4, CUDA stream scheduling provides time-multiplexed sharing without hardware isolation.
...
: **ResNet-50 batching sweep**: Scenario values for per-image compute, throughput, and GPU utilization across batch sizes one through thirty-two on a V100-class deployment. Throughput grows `{python} BatchThroughputCalc.throughput_ratio_mult_str` from batch one to batch thirty-two as GPU utilization climbs from `{python} BatchThroughputCalc.gpu_util_str[0]` to `{python} BatchThroughputCalc.gpu_util_str[4]`, while pure inference time stretches from `{python} BatchThroughputCalc.batch1_inference_total_ms_str` to `{python} BatchThroughputCalc.batch32_inference_ms_str`. {#tbl-model-serving-batching-throughput-tradeoff}
...
Language model serving introduces challenges beyond the batching and memory principles established here. The key-value cache that stores attention context scales with sequence length and batch size, often exceeding the model weights themselves in memory consumption. Techniques like speculative decoding\index{Speculative Decoding!latency reduction}\index{Speculative Decoding!draft model verification} use small draft models to propose multiple tokens that the target model verifies in parallel, reducing latency when the draft model has high acceptance rate [@leviathan2023speculative]. Weight-only quantization (such as INT4 weights with FP16 activations) is especially relevant for memory-bandwidth-bound LLM inference [@lin2023awq].
...
[^fn-simd-cpu-serving]: **SIMD (Single Instruction, Multiple Data)**\index{SIMD!CPU inference}: From Michael Flynn's 1966 taxonomy of computer architectures, SIMD enables one instruction to operate on multiple data elements simultaneously. Intel's AVX-512 processes 512 bits (16 floats) per instruction; AMX extends this to matrix tile operations. For CPU inference, SIMD exploitation is the primary optimization lever: naive scalar matrix multiplication leaves most vector hardware unused, while optimized kernels can approach a much higher fraction of the hardware's theoretical peak---a gap that determines whether CPU-only serving is economically viable.
...
* **Fast models reveal pipeline taxes**: Once inference falls into single-digit milliseconds, image decode, tokenization, and other preprocessing can consume a large share of total latency. The binding optimization becomes the request path, not the neural network kernel.
