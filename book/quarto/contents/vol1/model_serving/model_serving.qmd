...
# │
# │ Imports: mlsysim.book (fmt), mlsysim (Hardware.Cloud.V100_PCIe / V100 SXM2),
# │          mlsysim.core.units (TFLOP, second, GB, flop, byte)
# │ Exports: act_*_mb_str, ai_*_str, ridge_point_flops_per_byte_str,
# │          v100_pcie_fp32_str, v100_sxm2_fp32_str, v100_bw_str
# └─────────────────────────────────────────────────────────────────────────────
...
class ResolutionBottleneckCalc:
    ...
    ridge_point_flops_per_byte_str = fmt_arithmetic_intensity(ridge_point_value, precision=0, commas=False)
...
**Math**: Little's Law gives $N_{\text{req}} = \lambda T_{\text{lat}}$, so concurrency equals throughput multiplied by latency (@sec-appdx-machine-foundations-littles-law-21a3 derives the law).
...
# │
# │ Imports: mlsysim.fmt, mlsysim.core.units (ms)
# │ Exports: rho_str, latency_multiple_str, example_ms_str (lists), service_ms_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt_time, MarkdownStr, check
...
class UtilizationLatency:
    ...
    latency_multiple_str = [MarkdownStr(f"{m:.1f}$\\times$") for m in latency_multiple_value]
...
|         **Utilization $(\rho)$**         |                  **Latency Multiple**                 | **Example (`{python} UtilizationLatency.service_ms_str` service)** |
|:----------------------------------------:|:-----------------------------------------------------:|:------------------------------------------------------------------:|
| `{python} UtilizationLatency.rho_str[0]` | `{python} UtilizationLatency.latency_multiple_str[0]` |          `{python} UtilizationLatency.example_ms_str[0]`           |
| `{python} UtilizationLatency.rho_str[1]` | `{python} UtilizationLatency.latency_multiple_str[1]` |          `{python} UtilizationLatency.example_ms_str[1]`           |
| `{python} UtilizationLatency.rho_str[2]` | `{python} UtilizationLatency.latency_multiple_str[2]` |          `{python} UtilizationLatency.example_ms_str[2]`           |
| `{python} UtilizationLatency.rho_str[3]` | `{python} UtilizationLatency.latency_multiple_str[3]` |          `{python} UtilizationLatency.example_ms_str[3]`           |
| `{python} UtilizationLatency.rho_str[4]` | `{python} UtilizationLatency.latency_multiple_str[4]` |          `{python} UtilizationLatency.example_ms_str[4]`           |

: **Utilization-Latency Relationship**: Average time in system (wait + service) as a multiple of service time for an M/M/1 queue. At `{python} UtilizationLatency.rho_str[0]` utilization, time in system is `{python} UtilizationLatency.latency_multiple_str[0]` service time; at `{python} UtilizationLatency.rho_str[3]`, it reaches `{python} UtilizationLatency.latency_multiple_str[3]`. This nonlinear growth explains why systems that perform well at moderate load suddenly violate SLOs when traffic increases: moving from 80 percent to 90 percent utilization doubles latency. {#tbl-utilization-latency}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_time, fmt_rate, fmt_percent, MarkdownStr, check
...
class BatchThroughputCalc:
    ...
    inference_ms_str = [MarkdownStr(f"{inf:.1f} ms") for inf in inference_ms_value]
    per_image_ms_str = [MarkdownStr(f"{p:.1f} ms") for p in per_image_ms_value]
...
: **ResNet-50 batching sweep**: Per-image compute, throughput, and GPU utilization across batch sizes one through thirty-two on a V100. Throughput grows `{python} BatchThroughputCalc.throughput_ratio_mult_str` from batch one to batch thirty-two as GPU utilization climbs from `{python} BatchThroughputCalc.gpu_util_str[0]` to `{python} BatchThroughputCalc.gpu_util_str[4]`, while pure inference time stretches from `{python} BatchThroughputCalc.batch1_inference_total_ms_str` to `{python} BatchThroughputCalc.batch32_inference_ms_str`. {#tbl-model-serving-batching-throughput-tradeoff}
...
# │
# │ Imports: math, mlsysim.fmt, mlsysim.core.units (ms)
# │ Exports: qps_str, mean_str, std_str, p0_str, p_ge_str (lists), window_ms_str
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt, fmt_percent, fmt_time, MarkdownStr, check
from mlsysim.core.units import ms
...
class BatchVariability:
    ...
    mean_str = [MarkdownStr(f"{m:.1f}") for m in mean_value]
    std_str = [MarkdownStr(f"{s:.1f}") for s in std_value]
...
: **Batch Size Variability**: At low traffic, batching windows frequently contain zero requests (wasted GPU cycles). At moderate traffic, batch sizes fluctuate significantly around the mean. High traffic provides more stable batching, and the probability of batches exceeding twice the mean size decreases as traffic grows (from `{python} BatchVariability.p_ge_str[0]` at `{python} BatchVariability.qps_str[0]` QPS to `{python} BatchVariability.p_ge_str[3]` at `{python} BatchVariability.qps_str[3]` QPS), reflecting the law of large numbers. {#tbl-batch-variability tbl-colwidths="[24,18,18,20,20]"}
...
# │
# │ Imports: mlsysim.fmt, mlsysim.core.units (ms)
# │ Exports: window_str, max_batch_str, avg_ms_str, p99_ms_str, throughput_str
# │          (lists), throughput_gain_str, p99_ratio_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
class ParetoBatching:
    ...
    throughput_gain_str = fmt_percent(round(throughput_gain_value * 100) / 100, precision=0, commas=False, style='symbol')
...
# │          mlsysim (Models.Language.Llama3_70B: layers, hidden_dim, heads, kv_heads)
# │ Exports: (figure only, no prose variables)
# └─────────────────────────────────────────────────────────────────────────────
