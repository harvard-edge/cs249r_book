...
At that lower layer, node-level optimization identifies which boundary is wasting time on this machine. A serving node exposes four recurring boundaries: the computation graph\index{Graph Compilation!optimization} has to become kernels, CPU execution has to exploit the processor rather than scalar Python, model bytes have to move from disk into memory during cold starts, and the host has to keep the accelerator scheduled without gaps. These are not independent checklist items. They are places where a measured trace can explain why a request path is slower than the model benchmark promised.
...
The systems choice is where to pay compilation cost: JIT pays it during serving and risks a first-request latency spike, while AOT pays it before deployment and requires tighter control over accepted input shapes.
...
# │
# │ Goal: Keep the model-size, load-time, and storage-bandwidth values in the
...
# │ How: Use the MLSysIM Stable Diffusion v1.5 model entry, scenario load-time
# │      assumptions, and the Gen3 NVMe storage registry to compute the speedup.
# │
# │ Imports: mlsysim public API (units, Systems registry, fmt helpers, checks)
# │ Exports: model_size_gb_str, pickle_load_s_str, safetensors_load_s_str,
...
class SafetensorsLoadingSpeed:
    ...
    pickle_load = 15 * second  # scenario assumption: object reconstruction path
    safetensors_load = 0.5 * second  # scenario assumption: memory-mapped tensor path
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    model_size_gb_str = fmt_memory(model_size, unit=GB, precision=0, commas=False)
    pickle_load_s_str = fmt_time(pickle_load, second, precision=0, commas=False)
    safetensors_load_s_str = fmt_time(safetensors_load, second, precision=1, commas=False)
...
**Scenario**: A cold-start replica has to load a `{python} SafetensorsLoadingSpeed.model_size_gb_str` Stable Diffusion model before it can absorb traffic.
...
**Scenario**: A serving node has high P99 latency even though average accelerator utilization looks acceptable.
