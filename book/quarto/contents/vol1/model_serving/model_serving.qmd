...
[^fn-inference-server-serving]: **Inference Server**: TensorFlow Serving established the architectural pattern of separating the serving engine from the model logic, treating the model as a resource to be managed rather than as an executable script. This separation allows the server to implement scheduling policies like dynamic batching, where incoming requests are queued and grouped to maximize hardware throughput. Executing requests in batches can improve GPU utilization by over 70% compared to a naive, single-request-per-execution approach. \index{Inference Server!architecture}
...
[^fn-nchw-nhwc-serving]: **NCHW and NHWC (Tensor Memory Layouts)**: NCHW stores channel data contiguously to accelerate GPU convolutions, while NHWC's interleaved format is more efficient for CPU-based operations. Because the layout is an implicit convention, a mismatch during the server's request transformation is a common source of silent bugs. The model will not raise an error but will instead misinterpret height data as channel data, causing accuracy to drop to near-zero. \index{NCHW!tensor layout}
...
[^fn-protobuf-serialization]: **Protocol Buffers (Protobuf)**: Google's binary serialization format (developed internally circa 2001, open-sourced 2008). The binary encoding is 3--10$\times$ more compact than JSON and 20--100$\times$ faster to parse because the schema eliminates runtime type checking. For ML serving, this parsing speed difference directly reduces the serialization tax in the latency budget, though Protobuf still requires a deserialization step unlike FlatBuffers. \index{Protocol Buffers!serialization}
...
[^fn-rest-serving]: **REST (Representational State Transfer)**: As an architectural style, REST's reliance on HTTP/1.1 and JSON ensures universal support, making it a default for public web APIs. This design choice creates a direct trade-off between broad compatibility and the performance required for ML workloads. The overhead from repeated TCP handshakes and text-based JSON serialization can introduce 10--100$\times$ more latency for numerical tensor data than optimized binary protocols. \index{REST!serving overhead}

[^fn-grpc-inference]: **gRPC (gRPC Remote Procedure Call)**: Open-sourced by Google in February 2015, gRPC evolved from Stubby, Google's internal RPC framework handling tens of billions of calls per second since approximately 2001. The combination of HTTP/2 multiplexing and Protocol Buffers binary serialization achieves roughly 10$\times$ lower serialization overhead than REST/JSON, making it the de facto standard for latency-sensitive ML inference APIs where serialization cost is a significant fraction of the latency budget. \index{gRPC!inference protocol}
...
[^fn-tail-latency-serving]: **Tail Latency**: The heavy-tailed distribution of ML inference latency, where factors like cache misses and garbage collection create extreme outliers invisible to averages. Engineering teams focus on percentile targets (p99) because user satisfaction tracks this worst-case experience, not the mean; early studies at Google and Amazon confirmed that an additional 100ms of latency cost ~1% of sales. \index{Tail Latency!revenue impact}
...
[^fn-dali-preprocessing]: **NVIDIA DALI (Data Loading Library)**: Released in 2018, DALI moves image preprocessing (decoding, resizing, normalization) from the CPU to the GPU. The key insight: when inference runs on optimized accelerators, CPU-based JPEG decoding (3--5 ms) dwarfs the model forward pass ($<$1 ms), making preprocessing the Amdahl's Law bottleneck. DALI's GPU-accelerated pipeline achieves 2--6$\times$ end-to-end speedup and reduces training-serving skew by providing identical preprocessing implementations across both pipelines. \index{DALI!GPU preprocessing}
...
[^fn-littles-law-serving]: **Little's Law**: Proven by John D.C. Little in 1961, this theorem holds for *any* stable queuing system regardless of arrival patterns, service distributions, or scheduling policies---a generality that makes it one of operations research's most powerful results. For ML serving, it enables capacity planning from observable metrics: measuring queue depth ($L$) and arrival rate ($\lambda$) directly yields average latency ($W$) without instrumenting individual requests, providing the minimum concurrency slots a node must support to meet its SLO. \index{Little's Law!capacity planning}
...
[^fn-kendall-notation-serving]: **Kendall Notation**: The "M" signifies Markovian (random) service times, while "D" signifies Deterministic (constant) times, which is more typical for ML inference. The text intentionally uses the less-accurate M/M/1 model because its mathematical simplicity yields conservative wait time estimates—approximately double those of an M/D/1 queue—thus ensuring capacity plans err on the side of meeting SLOs. \index{Kendall Notation!queuing models}
...
[^fn-hedging-tail-tolerance]: **Hedging**: This strategy makes an offsetting bet against tail latency by issuing a redundant request. The specific trade-off for ML serving is that non-interruptible GPU kernels force the losing duplicate request to run to completion, wasting one full inference cycle. The 95th percentile rule of thumb quantifies this trade-off: increasing system load by only 5% to dramatically reduce P99 latency. \index{Hedging!tail tolerance}
...
[^fn-warmup-coldstart]: **Warmup**: This step executes initial inferences to trigger lazy hardware initialization, where runtimes like CUDA defer resource allocation and kernel compilation until first use. Populating caches and compiling kernels before serving traffic prevents the first user request from bearing this one-time setup cost, which can make initial latency over 100x slower than the steady-state average. \index{Warmup!cold start mitigation}
...
[^fn-mig-multimodel]: **MIG (Multi-Instance GPU)**: This hardware-level isolation is achieved by partitioning a GPU’s physical resources (streaming multiprocessors, memory controllers) into fully independent slices. This prevents the "noisy neighbor" problem inherent to software time-slicing, where one runaway model can disrupt others and violate performance SLOs. The primary trade-off is fixed granularity; an A100 GPU can be divided into a maximum of 7 predefined instances, not arbitrary resource amounts. \index{MIG!hardware isolation}
...
[^fn-batch-serving-tradeoff]: **Batching**: Grouping requests amortizes the high fixed cost of a single GPU kernel launch across multiple inputs, explaining the 6.4x throughput gain observed. This creates the core serving trade-off: higher throughput vs. increased per-request latency, as each input must wait for the batch to assemble. While training batches can contain thousands of samples, serving batches rarely exceed 8-32 before this added latency violates user-facing SLOs. \index{Batching!etymology}
...
[^fn-pareto-frontier-serving]: **Pareto Frontier**: Named after Italian economist Vilfredo Pareto (1848--1923), the frontier describes the set of solutions where no objective can be improved without worsening another. In ML serving, the frontier maps the throughput-latency trade-off: each batching configuration represents a point where gaining throughput requires sacrificing latency. Operating *below* the frontier means the configuration is suboptimal (better throughput at the same latency exists); operating *on* the frontier means every improvement requires an explicit trade-off decision. \index{Pareto Frontier!throughput-latency}
...
[^fn-vllm-paging]: **vLLM (Virtual LLM)**: Its core innovation, PagedAttention, applies operating system virtual memory concepts to manage the GPU's KV cache, solving the fragmentation problem that limits static batching. By storing the KV cache in non-contiguous memory "pages", memory from a completed sequence can be immediately reused for a new request. This technique boosts memory utilization from a typical 60% to over 90%, directly enabling the throughput gains of continuous batching. \index{vLLM!virtual memory analogy}

[^fn-tensorrt-llm-serving]: **TensorRT-LLM**: NVIDIA's open-source library (released October 2023) extending TensorRT's graph compilation to LLMs with domain-specific optimizations: in-flight batching (continuous batching), paged KV cache management, multi-GPU tensor parallelism, and custom attention kernels. The critical systems trade-off is portability: TensorRT-LLM engines are compiled for a specific GPU architecture, so deploying across V100, A100, and H100 requires separate builds, adding complexity to heterogeneous serving fleets. \index{TensorRT-LLM!LLM serving}
...
[^fn-poisson-serving]: **Poisson Process**: The key property for serving is that the variance of arrivals equals the mean, creating a critical gap between the *expected* batch size from @eq-poisson-batch and the highly variable *actual* batch sizes. This means that while a rate ($\lambda$) of 200 req/s and a 10ms window ($T$) yield an expected batch of 2, in practice 16% of those batches will be empty, wasting GPU cycles. \index{Poisson Process!serving arrivals}
...
[^fn-autoregressive-serving]: **Autoregressive generation**: The model's output at step *t* becomes part of the input for step *t+1*, creating a strict sequential dependency. This serial bottleneck prevents the parallel processing of output tokens that is standard in traditional model serving. This forces inference to be memory-bandwidth-bound rather than compute-bound, as the entire model must be read from memory for every single token generated. \index{Autoregressive!serial bottleneck}
...
[^fn-ttft-tpot-llm]: **TTFT (Time To First Token) and TPOT (Time Per Output Token)**: They capture distinct user experiences because a user perceives two different phases: the initial, compute-bound "thinking" delay before a response begins (TTFT) and the subsequent, memory-bandwidth-bound speed at which the response is "typed" out (TPOT). A system can thus feel responsive with a low TTFT (<200ms) but frustratingly slow if its TPOT is high, as the generation speed is ultimately gated by the hardware's memory bandwidth, not just its peak FLOPs. \index{TTFT!TPOT!LLM metrics}
...
[^fn-beam-search-serving]: **Beam Search**: It avoids the irreversible mistakes of greedy decoding by maintaining multiple candidate sequences (the "beam") instead of one. This directly creates the computational trade-off mentioned; each sequence in the beam requires its own KV cache, so memory costs scale linearly with beam width. A beam width of 5 thus requires 5x the KV cache memory of greedy decoding. \index{Beam Search!memory cost}

[^fn-nucleus-sampling-serving]: **Nucleus Sampling (Top-p)**: The "nucleus" metaphor refers to the core of the probability distribution: rather than sampling from a fixed number of top tokens (top-k), nucleus sampling dynamically selects the smallest set whose cumulative probability exceeds a threshold *p* (typically 0.9--0.95). This adapts to distribution shape: when the model is confident, few tokens are sampled; when uncertain, more diversity is allowed. For serving, nucleus sampling adds negligible compute overhead compared to beam search, making it the dominant strategy for latency-sensitive generative applications. \index{Nucleus Sampling!adaptive diversity}
...
[^fn-kv-cache-serving]: **KV Cache (Key-Value Cache)**: During autoregressive generation, previously computed Key and Value vectors remain valid for all future tokens---only the new token's Query vector changes. The KV cache stores these projections to avoid redundant recomputation, reducing generation cost from quadratic to linear in sequence length. The trade-off is memory: for a 70B model with 80 layers in FP16, the KV cache consumes ~1.3 MB per token per request, meaning a batch of 32 requests at 8,000 tokens requires ~330 GB---far exceeding the model weights themselves and making KV cache management the central memory challenge of LLM serving. \index{KV Cache!memory scaling}
...
[^fn-openvino-serving]: **OpenVINO (Open Visual Inference and Neural network Optimization)**: Intel's inference toolkit (released May 2018) maps neural network operations onto Intel-specific instruction sets: AVX-512 and AMX on Xeon CPUs, integrated GPU execution via oneAPI, and Movidius VPUs for edge deployment. For organizations running Intel infrastructure, OpenVINO achieves 2--5$\times$ speedup over framework-native CPU inference through hardware-specific kernel implementations and INT8 calibration---making CPU-only serving economically viable for models under ~500M parameters. \index{OpenVINO!Intel inference}

[^fn-tensorrt-gpu-serving]: **TensorRT (Tensor Runtime)**: TensorRT performs these aggressive optimizations by rebuilding the computation graph for a *specific* GPU target (e.g., an H100) during a dedicated build phase. This compilation destroys portability—the engine is locked to that single GPU architecture—a safety trade-off a general framework cannot make. The resulting specialized engine, however, can achieve a 2–5$\times$ speedup over the original framework model. \index{TensorRT!GPU optimization}
...
[^fn-simd-cpu-serving]: **SIMD (Single Instruction, Multiple Data)**: This processor feature executes a single instruction across a vector of data, such as 16 floating-point numbers in one AVX-512 operation. Standard Python interpreters cannot generate these vector instructions, issuing one instruction per data point. This creates a performance chasm: a vanilla implementation might achieve less than 5% of a CPU's peak capabilities, while a specialized runtime that compiles operators directly to SIMD kernels can exceed 80% utilization. \index{SIMD!CPU inference}
...
[^fn-numa-cpu-serving]: **NUMA (Non-Uniform Memory Access)**: A memory architecture where access latency depends on physical proximity of processor to memory bank. In a two-socket server, local memory access takes ~80 ns while remote memory (attached to the other socket) takes ~130 ns---a 60% penalty. For ML inference, NUMA-unaware memory allocation causes model weights to reside on the "wrong" socket, adding 40--60% latency overhead to every weight access. This penalty is invisible in profiling tools that report only CPU cycles, making it a silent performance killer in multi-socket serving deployments. \index{NUMA!inference latency}
...
[^fn-safetensors-loading]: **Safetensors**: By restricting its design to raw-byte buffers with a minimal header, the format allows the operating system to memory-map the file directly into a process's address space, avoiding CPU-bound deserialization. This zero-copy mechanism delivers 30-100x faster loading than pickle. For an autoscaling service, this speed difference—e.g., 0.5s vs 15s—determines whether a new replica can come online fast enough to absorb a traffic spike before SLOs are violated. \index{Safetensors!fast loading}
...
[^fn-awq-quantization]: **AWQ (Activation-Aware Weight Quantization)**: AWQ makes 4-bit quantization possible by selectively protecting the most salient weights while aggressively compressing the rest. For Llama-3-8B, this reduces the memory requirement by nearly 4x, enabling it to run on commodity GPUs that could not otherwise fit the model. The resulting accuracy degradation is typically less than 0.5%. \index{AWQ!serving efficiency}
