...
class CostLatencyCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    gpu_cost_per_hour_value = 4.0 * (USD / hour)  # scenario assumption: GPU rental cost
...
from mlsysim.fmt import MarkdownStr, check, fmt, fmt_time, fmt_qty, fmt_rate, fmt_memory, fmt_energy
...
class ResNetServingSpectrum:
    ...
    check(mobile_energy_cpu >= mobile_energy_npu * 3,
    ...
    tiny_sram_kb_str = fmt_memory(s_tiny.ram, unit=KB, precision=1, commas=True)
...
class ResolutionScalingCalc:
    ...
    measured_slowdown_str = fmt(measured_slowdown_factor_value, precision=1, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_qty, fmt_memory
...
class ResolutionBottleneckCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    act_224 = 12.5 * MB                   # scenario assumption: 224×224 activation size
    act_384 = 36.8 * MB                   # scenario assumption: 384×384 activation size
    act_512 = 65.5 * MB                   # scenario assumption: 512×512 activation size
    act_640 = 102.4 * MB                  # scenario assumption: 640×640 activation size

    ai_224_value = 85 * (flop / byte)     # scenario assumption: 224×224 arithmetic intensity
    ai_384_value = 49 * (flop / byte)     # scenario assumption: 384×384 arithmetic intensity
    ai_512_value = 28 * (flop / byte)     # scenario assumption: 512×512 arithmetic intensity
    ai_640_value = 18 * (flop / byte)     # scenario assumption: 640×640 arithmetic intensity

    ridge_point_value = 16 * (flop / byte)  # scenario assumption: V100 ridge point
    ...
    ai_224_str = fmt_qty(ai_224_value, flop / byte, precision=0, commas=False, unit_label="FLOP/byte")
    ai_384_str = fmt_qty(ai_384_value, flop / byte, precision=0, commas=False, unit_label="FLOP/byte")
    ai_512_str = fmt_qty(ai_512_value, flop / byte, precision=0, commas=False, unit_label="FLOP/byte")
    ai_640_str = fmt_qty(ai_640_value, flop / byte, precision=0, commas=False, unit_label="FLOP/byte")

    ridge_point_str = fmt_qty(ridge_point_value, flop / byte, precision=0, commas=False, unit_label="FLOP/byte")
```
...
|:---------------------|:--------------------------------------------------:|:----------------------------------------------:|:--------------:|
| **$224{\times}224$** | `{python} ResolutionBottleneckCalc.act_224_mb_str` | `{python} ResolutionBottleneckCalc.ai_224_str` |    Compute     |
| **$384{\times}384$** | `{python} ResolutionBottleneckCalc.act_384_mb_str` | `{python} ResolutionBottleneckCalc.ai_384_str` |    Compute     |
| **$512{\times}512$** | `{python} ResolutionBottleneckCalc.act_512_mb_str` | `{python} ResolutionBottleneckCalc.ai_512_str` |    Compute     |
| **$640{\times}640$** | `{python} ResolutionBottleneckCalc.act_640_mb_str` | `{python} ResolutionBottleneckCalc.ai_640_str` |   Near ridge   |
...
class AdaptiveResolutionCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    adaptive_throughput_improvement_str = fmt_multiple(adaptive_throughput_improvement_factor_value, precision=1, commas=False)
    adaptive_accuracy_retention_str = fmt_percent(adaptive_accuracy_retention_pct_value/100, precision=1, commas=False, style='symbol')
```
...
class CapacityPlanningCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cp_mu_required_str = fmt_rate(cp_mu_required_value, 'req/s', precision=1, commas=False)
    ...
    cp_headroom_multiplier_str = fmt_multiple(cp_headroom_factor_value, precision=1, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_math, fmt_time, fmt_memory, fmt_bandwidth
...
class ModelSwapCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    model_size = 10 * GB                  # scenario assumption: model size
    ...
    pcie_bw_gbs_str = fmt_bandwidth(pcie_bw, unit=GB / second, precision=0, commas=False)
    model_swap_ms_str = fmt_time(model_swap, 'millisecond', precision=1, commas=False)
    model_size_for_latex = model_size.to(GB).magnitude
    pcie_bw_for_latex = pcie_bw.to(GB / second).magnitude
    model_swap_for_latex = model_swap.to(ms).magnitude
    swap_calc_math = fmt_math(
        f"T_{{\\text{{load}}}} = {model_size_for_latex:g} \\text{{ GB}} "
        f"/ {pcie_bw_for_latex:g} \\text{{ GB/s}} "
        f"\\approx {model_swap_for_latex:.1f} \\text{{ ms}}"
    )
```
...
class BatchThroughputCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    batch1_throughput_value = 200 * (count / second)  # scenario assumption: batch-1 throughput
    batch32_throughput_value = 1280 * (count / second)  # scenario assumption: batch-32 throughput
    batch32_inference_ms_value = 25.0 * ms  # scenario assumption: batch-32 inference time
    batch_window_ms_value = 10.0 * ms     # scenario assumption: batching window
    batch1_inference_total_ms_value = 5.0 * ms  # scenario assumption: batch-1 total latency
...
class PracticalConfigCalc:
    ...
    pc_predicted_throughput_value = (
        pc_config_batch_count_value * count / pc_peak_service_ms_value * pc_throughput_efficiency_factor_value
    ).to(count / second)
...
class TrafficAdaptiveBatchingCalc:
    ...
    window_5000_ms_str = fmt_time(rows[3][1], 'millisecond', precision=1, commas=False)
...
from mlsysim.fmt import fmt, fmt_int, fmt_time, fmt_memory, fmt_power, fmt_energy, check
class MobileServingCalc:
    ...
    m_total_mj_display = round(m_total_mj.to(mJ).magnitude) * mJ
    ...
    m_total_mj_str = fmt_energy(m_total_mj_display, unit=mJ, precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, fmt_qty, check, fmt_percent, fmt_rate, fmt_power, fmt_energy
...
class CarbonCostCalc:
    ...
    cc_host_overhead_str = fmt_power(cc_host_overhead, unit=watt, precision=0, commas=False)
    cc_total_power_str = fmt_power(cc_total_power, unit=watt, precision=0, commas=False)
    ...
    cc_response_joules_str = fmt_energy(cc_response_joules, unit=joule, precision=1, commas=False)
    cc_smartphone_str = fmt_energy(ENERGY_SMARTPHONE_CHARGE_J, unit=joule, precision=0)
    cc_boiling_str = fmt_energy(ENERGY_BOILING_WATER_J, unit=joule, precision=0)
    cc_low_util_str = fmt_percent(cc_low_util_pct_value/100, precision=0, commas=False, style='symbol')
    cc_idle_power_str = fmt_power(cc_idle_power, unit=watt, precision=0, commas=False)
...
class CostAnalysisCalc:
    ...
    ca_cpu_throughput_str = fmt_rate(ca_cpu_throughput_value, 'img/s', precision=0, commas=False)
    ca_cpu_cpm_str = fmt_usd(ca_cpu_cpm_value, precision=2, commas=False)
    ca_t4_cost_str = fmt_usd(ca_t4_cost_value * hour, precision=2, commas=False)
    ca_t4_throughput_str = fmt_rate(ca_t4_throughput_value, 'img/s', precision=0, commas=False)
    ca_t4_cpm_str = fmt_usd(ca_t4_cpm_value, precision=2, commas=False)
    ca_v100_cost_str = fmt_usd(ca_v100_cost_value * hour, precision=2, commas=False)
    ca_v100_throughput_str = fmt_rate(ca_v100_throughput_value, 'img/s', precision=0)
    ca_v100_cpm_str = fmt_usd(ca_v100_cpm_value, precision=2, commas=False)
...
|:--------------------------|:--------------------------------------------:|:--------------------------------------------------:|:-------------------------------------------:|
| **c5.xlarge (CPU)**       | `{python} CostAnalysisCalc.ca_cpu_cost_str`  | `{python} CostAnalysisCalc.ca_cpu_throughput_str`  |  `{python} CostAnalysisCalc.ca_cpu_cpm_str` |
| **g4dn.xlarge (T4 GPU)**  |  `{python} CostAnalysisCalc.ca_t4_cost_str`  |  `{python} CostAnalysisCalc.ca_t4_throughput_str`  |  `{python} CostAnalysisCalc.ca_t4_cpm_str`  |
| **p3.2xlarge (V100 GPU)** | `{python} CostAnalysisCalc.ca_v100_cost_str` | `{python} CostAnalysisCalc.ca_v100_throughput_str` | `{python} CostAnalysisCalc.ca_v100_cpm_str` |
...
from mlsysim.fmt import fmt, fmt_qty, fmt_time, fmt_memory, fmt_bandwidth
from mlsysim.physics import transfer_time
...
class LlmCaseStudyHwSpecs:
    ...
    a100_bw_tbs_str = fmt_bandwidth(a100_bw, unit=TB / second, precision=2, commas=False)
...
# │          cost_per_million_tokens_usd_str, remaining_vram_gb_str
# └─────────────────────────────────────────────────────────────────────────────
...
For an H100 SXM5 instance at approximately `{python} LlmServingCalc.hourly_cost_str` (specialized cloud providers; major hyperscaler on-demand rates were in the \$2--\$13 per hour range in 2026):
...
*   **Cost per million tokens**: `{python} LlmServingCalc.hourly_cost_str` / `{python} LlmServingCalc.tokens_per_hour_m_str` ≈ `{python} LlmServingCalc.cost_per_million_tokens_usd_str`.
...
class FallacySkewCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fs_val_acc_str = fmt_percent(fs_val_acc_pct_value/100, precision=0, commas=False, style='symbol')
    fs_prod_acc_str = fmt_percent(fs_prod_acc_pct_value/100, precision=0, commas=False, style='symbol')
...
class FallacyCalibrationCalc:
    ...
    fc_imagenet_acc_str = fmt_percent(fc_imagenet_acc_pct_value/100, precision=1, commas=False, style='symbol')
    fc_ood_acc_str = fmt_percent(fc_ood_acc_pct_value/100, precision=1, commas=False, style='symbol')
```
