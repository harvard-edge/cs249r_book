...
Third, model management: inference servers manage the lifecycle of models, including loading weights into VRAM, managing versioning, and completing warmup inferences before exposing the model to live traffic. Among these components, the scheduler deserves special attention because it embodies the core serving trade-off between throughput and latency.
...
::: {.column-margin}
![](images/svg/vol1_model_serving_margin_001.svg){width="100%" fig-alt="A low flat curve that bends sharply upward past a marked knee; a dashed 50 ms threshold line sits far below where the model-load latency lands, with the region above shaded red."}
...
::: {.column-margin}
![](images/svg/vol1_model_serving_margin_003.svg){width="100%" fig-alt="Two diverging strokes against rising QPS: a stroke for batch size climbing upward while a second stroke for the batching window falls, the gap between them shaded."}
