...
* **LLM serving is memory-bandwidth bound**: Token generation reads the model weights from VRAM per token, making decode latency primarily limited by memory bandwidth rather than compute when batching does not change the bound. KV cache management via PagedAttention and continuous batching is a major throughput lever, with reported and scenario-dependent gains over naive serving.
