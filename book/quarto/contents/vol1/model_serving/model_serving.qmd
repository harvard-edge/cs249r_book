...
1.  **Batch Formation Delay ($W_{form}$)**\index{Batching!batch formation delay, definition}: The time the first request in a batch waits for the last request to arrive.
2.  **Inference Inflation ($W_{inf}$)**\index{Inference Inflation!definition}: The increase in execution time when the GPU processes $B$ samples instead of 1.
