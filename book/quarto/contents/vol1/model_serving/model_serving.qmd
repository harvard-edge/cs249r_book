...
%
\node[Pre,below=0.99 of R1](R11){Pre1};
...
Comparing a conservative batching window against an aggressive one isolates how the window choice trades wait time, inference budget, and throughput. @Tbl-batching-windows lays the two configurations side by side.
...
: **Batching window trade-off**: How a conservative versus aggressive batching window trades average wait, inference budget, batch size, and throughput at fixed arrival rate. {#tbl-batching-windows tbl-colwidths="[28,36,36]"}
