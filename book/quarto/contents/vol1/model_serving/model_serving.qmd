...
In an illustrative cold-start scenario, loading a `{python} SafetensorsLoadingSpeed.model_size_gb_str` Stable Diffusion model through Pickle (`torch.load`) takes `{python} SafetensorsLoadingSpeed.pickle_load_s_str` while consuming substantial CPU time because Python has to reconstruct objects before tensors are usable. The same weights stored with Safetensors load in `{python} SafetensorsLoadingSpeed.safetensors_load_s_str` in this scenario, a `{python} SafetensorsLoadingSpeed.load_speedup_mult_str` improvement. By using `mmap` and formats like `safetensors`, loading speed becomes limited mainly by the disk's read speed, for example `{python} SafetensorsLoadingSpeed.nvme_bw_gb_per_s_str` for local Gen3 NVMe, rather than by CPU parsing overhead.
