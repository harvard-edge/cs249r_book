...
: **Batch Size Variability**: At low traffic, batching windows frequently contain zero requests (wasted GPU cycles). At moderate traffic, batch sizes fluctuate significantly around the mean. High traffic provides more stable batching, and the probability of batches reaching at least twice the mean size decreases as traffic grows (from `{python} BatchVariability.p_ge_str[0]` at `{python} BatchVariability.qps_str[0]` QPS to `{python} BatchVariability.p_ge_str[3]` at `{python} BatchVariability.qps_str[3]` QPS), reflecting the law of large numbers. {#tbl-batch-variability tbl-colwidths="[24,18,18,20,20]"}
