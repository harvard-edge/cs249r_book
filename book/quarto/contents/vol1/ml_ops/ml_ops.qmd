...
# │
# │ Imports: mlsysim.ReferenceStats; mlsysim.fmt formatters and check
...
from mlsysim.fmt import check, fmt_int, fmt_percent, fmt_pp, fmt_percent_range
...
class OuraValidationGap:
    ...
    participants = int(study.Participants)
    recording_nights = int(study.RecordingNights)
    recording_hours = int(study.RecordingHours)
    accel_only_accuracy = float(study.AccelOnlyAccuracy)
    enhanced_accuracy = float(study.EnhancedAccuracy)
    scorer_agreement_low = float(study.PsgScorerAgreementLow)
    scorer_agreement_high = float(study.PsgScorerAgreementHigh)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    participants_str = fmt_int(participants, commas=False)
    recording_nights_str = fmt_int(recording_nights, commas=False)
    recording_hours_str = fmt_int(recording_hours, commas=True)
...
To overcome performance limitations, the Oura team constructed a diverse dataset grounded in clinical standards through a study involving `{python} OuraValidationGap.participants_str` participants from three continents. Each participant wore the Oura Ring while simultaneously undergoing PSG, enabling high-fidelity labeled data that aligned wearable sensor data with validated sleep annotations.

The study yielded `{python} OuraValidationGap.recording_nights_str` nights of data and over `{python} OuraValidationGap.recording_hours_str` hours of time-synchronized recordings, capturing physiological diversity and variability in environmental and behavioral factors critical for generalizing across a real-world user base.
...
# │ How: LEGO class OuraModelAccuracy; load case-study anchors from ReferenceStats.
# │
# │ Imports: mlsysim.ReferenceStats; mlsysim.fmt (fmt_percent)
...
# │ How: LEGO class OuraOperationalAccuracy; load case-study anchors from ReferenceStats.
# │
# │ Imports: mlsysim.ReferenceStats; mlsysim.fmt (fmt_percent)
...
::: {#exmp-ml-ops-oura-ring-principles-summary .callout-example title="Oura Ring edge operations"}

**Context**: Oura's sleep-stage pipeline operates in a wearable edge environment, where battery limits, limited telemetry, and clinical validation requirements constrain the operational loop.

**Insight**: The standard MLOps principles still apply, but each principle changes shape. Reproducibility means versioning wearable and PSG datasets, preprocessing code, feature definitions, and hyperparameters. Consistency means aligning PSG-based training preprocessing with on-device inference. Observable degradation means tracking duty cycle, battery impact, inference failures, confidence, and anomaly rates through privacy-preserving telemetry.

**Systems lesson**: Edge constraints force graceful degradation to be designed before deployment. Battery-aware retraining triggers, OTA update infrastructure, and tiered fallback from multisensor models to accelerometer-only models let the system trade accuracy for resource safety without turning every degradation event into an incident.
...
::: {#exmp-ml-ops-clinaiops-principles-summary .callout-example title="ClinAIOps clinical operations"}

**Context**: ClinAIOps applies operational ML principles to clinical environments, where model outputs affect patient care and must pass through human, regulatory, and workflow controls.

**Insight**: The operational loop must preserve provenance, clinician authority, and regulatory accountability. Recommendations are logged with input data, model version, confidence, and clinician decision; validation and deployment are separated; and automated updates operate only within clinician-approved bounds.

**Systems lesson**: In clinical ML operations, human-in-the-loop control is not an exception to automation; it is part of the control system. Conservative recommendations, uncertainty flags, cohort-specific monitoring, and clinician override provide the safety mechanism that keeps automation aligned with patient risk.
...
# │ How: LEGO class OuraCaseRecap; load case-study anchors from ReferenceStats.
# │
# │ Imports: mlsysim.ReferenceStats; mlsysim.fmt (fmt_percent)
