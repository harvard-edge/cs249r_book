...
```{python}
#| echo: false
#| label: monitoring-archetype-table
# ┌─────────────────────────────────────────────────────────────────────────────
# │ MONITORING STRATEGY BY ARCHETYPE
# ├─────────────────────────────────────────────────────────────────────────────
# │ Goal: Keep the operational monitoring table tied to canonical workload
# │       archetypes instead of repeating model/archetype labels in prose.
# │ How: Scenarios.Archetypes.Workload entries point to Models.* workloads.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Scenarios as BookScenarios
from mlsysim.fmt import MarkdownStr

class MonitoringArchetypeTable:
    """Display fields for workload-specific monitoring strategy."""
    ...
    resnet_drift_str = MarkdownStr(resnet.dominant_drift_pattern)
    resnet_metric_str = MarkdownStr(resnet.monitoring_metric)
    resnet_trigger_str = MarkdownStr(resnet.retraining_trigger)
    gpt2_label_str = MarkdownStr(f"{gpt2.workload_label} ({gpt2.archetype})")
    gpt2_drift_str = MarkdownStr(gpt2.dominant_drift_pattern)
    gpt2_metric_str = MarkdownStr(gpt2.monitoring_metric)
    gpt2_trigger_str = MarkdownStr(gpt2.retraining_trigger)
    dlrm_label_str = MarkdownStr(f"{dlrm.workload_label} ({dlrm.archetype})")
    dlrm_drift_str = MarkdownStr(dlrm.dominant_drift_pattern)
    dlrm_metric_str = MarkdownStr(dlrm.monitoring_metric)
    dlrm_trigger_str = MarkdownStr(dlrm.retraining_trigger)
    kws_label_str = MarkdownStr(f"{kws.workload_label} ({kws.archetype})")
    kws_drift_str = MarkdownStr(kws.dominant_drift_pattern)
    kws_metric_str = MarkdownStr(kws.monitoring_metric)
    kws_trigger_str = MarkdownStr(kws.retraining_trigger)
```

| **Archetype**                                            | **Dominant Drift Pattern**                           | **Primary Monitoring Metric**                         | **Typical Retraining Trigger**                         |
|:---------------------------------------------------------|:-----------------------------------------------------|:------------------------------------------------------|:-------------------------------------------------------|
| **`{python} MonitoringArchetypeTable.resnet_label_str`** | `{python} MonitoringArchetypeTable.resnet_drift_str` | `{python} MonitoringArchetypeTable.resnet_metric_str` | `{python} MonitoringArchetypeTable.resnet_trigger_str` |
| **`{python} MonitoringArchetypeTable.gpt2_label_str`**   | `{python} MonitoringArchetypeTable.gpt2_drift_str`   | `{python} MonitoringArchetypeTable.gpt2_metric_str`   | `{python} MonitoringArchetypeTable.gpt2_trigger_str`   |
| **`{python} MonitoringArchetypeTable.dlrm_label_str`**   | `{python} MonitoringArchetypeTable.dlrm_drift_str`   | `{python} MonitoringArchetypeTable.dlrm_metric_str`   | `{python} MonitoringArchetypeTable.dlrm_trigger_str`   |
| **`{python} MonitoringArchetypeTable.kws_label_str`**    | `{python} MonitoringArchetypeTable.kws_drift_str`    | `{python} MonitoringArchetypeTable.kws_metric_str`    | `{python} MonitoringArchetypeTable.kws_trigger_str`    |
...
class SilentFailureCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    annual_revenue_str = fmt_usd(annual_revenue // MILLION, commas=False, suffix="M")
...
class MonitoringBudget:
    ...
    ingestion_cost_per_m_str = fmt_usd(ingestion_cost_per_m, precision=2, commas=False, suffix="/million")
    bytes_per_point_str = fmt(bytes_per_point, precision=0, commas=False)
    storage_cost_per_gb_str = fmt_usd(storage_cost_per_gb, precision=0, commas=False, suffix="/GB")
    ...
    query_cost_per_str = fmt_usd(query_cost_per, precision=2, commas=False, suffix="/query")
    dp_m_str = fmt(datapoints_mo_m, precision=1, commas=False)
    ingest_str = fmt_usd(ingestion_cost, precision=1, commas=False, suffix="/month")
    storage_gb_str = fmt(storage_gb, precision=1, commas=False, suffix=" GB")
    storage_cost_str = fmt_usd(storage_cost, precision=2, commas=False, suffix="/month")
    queries_str = fmt(queries_mo, precision=0, commas=True)
    query_cost_str = fmt_usd(query_cost, commas=False, suffix="/month")
    total_str = fmt_usd(total_cost, precision=1, commas=False, suffix="/month")
```
...
class SingleModelROI:
    ...
    incident_cost_str = fmt_usd(incident_cost // THOUSAND, precision=0, commas=False, suffix="K")
    hours_saved_monthly_str = fmt(hours_saved_monthly, precision=0, commas=False, suffix=" hours")
    hourly_cost_str = fmt_usd(hourly_cost, precision=0, commas=False, suffix="/hr")
    incident_savings_k_str = fmt_usd(incident_savings // THOUSAND, precision=0, commas=False, suffix="K")
    time_savings_k_str = fmt_usd(time_savings // THOUSAND, precision=0, commas=False, suffix="K")
    mlops_investment_k_str = fmt_usd(mlops_investment // THOUSAND, precision=0, commas=False, suffix="K")
    mlops_investment_k_year_str = fmt_usd(mlops_investment // THOUSAND, precision=0, commas=False, suffix="K/year")
...
class OuraValidationGap:
    ...
    scorer_low_pct_str = fmt(scorer_agreement_low * 100, precision=0, commas=False, suffix=" percent")
