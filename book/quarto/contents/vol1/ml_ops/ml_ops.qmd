...
**Systems insight**: Use dynamic sampling. Sample `{python} ObservabilitySampling.success_sample_pct_str` of successful requests but `{python} ObservabilitySampling.error_sample_pct_str` of errors. Use high-frequency (`{python} ObservabilitySampling.high_seconds_str`) monitoring only for aggregate counters (like error rate), but low-frequency (`{python} ObservabilitySampling.low_seconds_str`) for high-cardinality data (like user-level distribution sketches). @Sec-ml-operations-data-quality-monitoring-c6b6 provides worked examples for budgeting monitoring infrastructure.
