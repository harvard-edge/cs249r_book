...
1.  **Significance**: It represents a violation of the i.i.d. assumption (independent and identically distributed), causing accuracy to erode monotonically with the distributional divergence $(\mathcal{D}(P_t \lVert P_0))$, empirically modeled as $\text{Accuracy}(t) \approx \text{Accuracy}_0 - \lambda \cdot \mathcal{D}(P_t \lVert P_0)$ with $\lambda$ fit per deployment. Because $p(y \mid x)$ is unchanged, retraining on fresh $p(x)$ data can recover performance (when the new input distribution overlaps the original support), unlike concept drift, where the label relationship must also be re-learned.
