...
class KVCacheFootprint:
    ...
    check(2.5 * GB <= kv_cache_one <= 4 * GB,
          f"Expected 2.5-4 GB per active sequence, got {kv_cache_one.to(GB).magnitude:.2f} GB.")
...
class ObservabilitySampling:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(round(high_bytes_per_second.to(GB/second).magnitude, 1) == 1.0,
          f"Expected ~1 GB/s, got {high_bytes_per_second.to(GB/second).magnitude:.2f} GB/s.")
    check(round(low_bytes_per_second.to(MB/second).magnitude, 1) == 16.7,
...
- Data points per month: `{python} MonitoringBudget.n_variants_str` $\times$ `{python} MonitoringBudget.metrics_per_variant_str` $\times$ (`{python} MonitoringBudget.samples_per_min_str` $\times 60 \times 24 \times$ `{python} MonitoringBudget.days_str`) = `{python} MonitoringBudget.dp_m_str` million
- Cost at `{python} MonitoringBudget.ingestion_cost_per_m_str`: `{python} MonitoringBudget.ingest_usd_str`

**Storage**:

- At `{python} MonitoringBudget.bytes_per_point_value_str` bytes/point compressed: `{python} MonitoringBudget.dp_m_str` million $\times$ `{python} MonitoringBudget.bytes_per_point_value_str` bytes = `{python} MonitoringBudget.storage_gb_str`
...
class SliceAnalysisCalc:
    ...
    overall_accuracy_pct_str = fmt_percent(
        round(overall_accuracy) / 100, precision=0, commas=False, style='prose'
    )
```
...
A common fallacy is this: *MLOps is just applying traditional DevOps practices to machine learning models.*
...
A recurring pitfall is this: *Treating model deployment as a one-time event rather than an ongoing process.*
...
A common fallacy is this: *Automated retraining ensures optimal model performance without human oversight.*
...
A recurring pitfall is this: *Focusing on technical infrastructure while neglecting organizational and process alignment.*
...
A common fallacy is this: *Training and serving environments automatically remain consistent once pipelines are established.*
...
A recurring pitfall is this: *Assuming comprehensive monitoring prevents all production incidents.*
...
A common fallacy is this: *Accuracy is the first production signal to monitor.*
...
A recurring pitfall is this: *Alerting only after accuracy degradation becomes visible.*
