...
**Context**: Uber's Michelangelo platform\index{Michelangelo!Uber ML platform} established the production feature-store pattern\index{Feature Store!Uber case study} [@hermann2017michelangelo], serving thousands of deployed models at over $10^5\text{ QPS}$ across ride pricing, ETA prediction, and fraud detection.

**Mechanism**: Data scientists originally computed offline features in Spark, while software engineers reimplemented the same logic in Java for online serving. Dual implementations caused training-serving skew $\Delta \text{accuracy}$ and latency spikes ($L_{\text{lat}} > 50\text{ ms}$). Michelangelo moved feature storage into a shared system serving batch training via Hive and low-latency online inference via Cassandra ($L_{\text{fetch}} \le 10\text{ ms}$), compiling feature definitions once for both paths to decouple memory footprint $D_{\text{vol}}$ from serving latency $L_{\text{lat}}$.

**Systems lesson**: Feature stores preserve semantic parity by enforcing a single feature contract across training and serving. Point-in-time correctness\index{Point-in-Time Correctness!feature retrieval} requires timestamped historical feature snapshots and as-of joins to eliminate data leakage, ensuring feature fetch latency $L_{\text{fetch}}$ stays bounded by memory bandwidth $D_{\text{vol}}/\text{BW}$.
...
**Context**: TensorFlow Extended (TFX)\index{TensorFlow Extended!production pipelines} emerged from Google's production ML infrastructure [@baylor2017], processing petabyte-scale daily telemetry ($D_{\text{vol}} > 10^{15}\text{ bytes}$) across hundreds of production models.

**Mechanism**: Before TFX, ad hoc pipelines created unvalidated data handoffs and non-reproducible training runs. TFX standardized workflow steps via Apache Beam and C++ execution engines through versioned components (ExampleGen, StatisticsGen, SchemaGen, ExampleValidator, Transform, Trainer, Evaluator, Pusher), bounding validation compute overhead $O/(R_{\text{peak}} \cdot \eta_{\text{hw}})$ and gating releases on strict statistical contracts.

**Systems lesson**: Production ML pipelines require immutable artifact discipline, not just DAG task orchestration. TFX ensures every pipeline step produces versioned artifacts with ML Metadata (MLMD), allowing root-cause analysis to trace production accuracy loss back to exact dataset snapshots $D_{\text{vol}}$ and training configurations.
...
The decision to retrain a model\index{Retraining!economics, cost-benefit optimization} balances the cost of accuracy decay against retraining expense. When outcome data support a locally exponential fit, the fitted decay rate gives a measurable timescale[^fn-entropy-model-decay]. Its **Half-Life**\index{Half-Life!model decay}[^fn-half-life-model] describes when fitted accuracy falls to half its initial value, but it does not determine the economically optimal retraining interval. The calculation below distinguishes the fitted half-life from the economically optimal retraining interval.
...
**Mechanism**: The deployment repurposed a binary feature flag that activated legacy Power Peg code on the un-updated eighth server. Because the cumulative-share counter had been relocated in prior refactoring, the stale server entered an un-throttled loop, transmitting parent order requests at $\sim 1,400\text{ orders/sec}$ ($T_{\text{comm}} < 1\text{ ms}$). In 45 minutes, the router executed 4 million transactions (440 million shares), incurring a \$460 million loss because execution latency $L_{\text{lat}}$ outstripped monitoring loop synchronization time $T_{\text{sync}}$.

**Systems lesson**: Automated deployment is a physical control problem where configuration drift across heterogeneous nodes causes catastrophic failure. ML deployments share this exact vulnerability: a model registry version mismatch, un-synchronized feature schemas ($T_{\text{sync}}$ delay), or partial canary routing can flood production with corrupted inference requests before telemetry alerts fire.
...
**Context**: A large streaming recommendation service\index{Model Monitoring!recommendation systems} serving $10^5\text{ QPS}$ with a strict $L_{\text{lat}} \le 50\text{ ms}$ P99 inference SLA must monitor ranking quality as user viewing patterns and content catalogs evolve.

**Mechanism**: Traditional infrastructure telemetry (CPU/GPU utilization, HTTP error rates) stays green even when model predictions degrade under concept drift. The monitoring architecture combines statistical process control\index{Statistical Process Control!anomaly detection}, cohort-based subpopulation tracking, counterfactual evaluation, and online interleaving experiments. This multi-layered telemetry filters high-cardinality feature payload volume $D_{\text{vol}}$ against memory bandwidth capacity $\text{BW}$, catching localized cohort decay before aggregate CTR drops.

**Systems lesson**: Monitoring telemetry is effective only when the signal matches the failure mode. High-throughput recommendation engines require cohort-level and counterfactual signals because global metrics mask regional or subpopulation accuracy degradation.
