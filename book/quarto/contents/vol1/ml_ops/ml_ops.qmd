...
[^fn-mlops-operational-gap]: **MLOps (Machine Learning Operations)**: The discipline that makes invisible failures visible by bridging the gap between model development and sustained production health. Google's 2015 "Hidden Technical Debt" paper revealed that the majority of ML production failures stem not from algorithmic weakness but from operational blind spots: unmonitored feature drift, brittle pipelines, and unclear ownership. Without infrastructure that tracks statistical health as data distributions evolve, every deployed model is on a countdown to silent failure, degrading accuracy with no error signal. \index{MLOps!emergence}

[^fn-devops-determinism]: **DevOps**: Built on a determinism assumption that ML systems violate by design. DevOps monitoring answers binary questions (is the server up? is latency acceptable?) because identical code with identical inputs produces identical outputs. ML systems break this contract: changing data distributions alter model behavior without any code change, so a system can report perfect uptime while silently degrading predictions for weeks. \index{DevOps!determinism assumption}
...
[^fn-reproducibility-ml]: **Reproducibility Failure**: Occurs because bit-for-bit output alignment in ML requires synchronizing not just code, but the entire stack of data, dependencies, and hardware. A single version mismatch can silently degrade model accuracy by several percentage points—a statistical failure invisible to the uptime and latency checks that traditional DevOps relies on. \index{Reproducibility!crisis}
...
\index{Technical Debt!ML system complexity}The silent failure modes established earlier manifest concretely as technical debt [@sculley2015hidden]: data changes, model interactions, and evolving requirements cause gradual degradation that compounds over time.
 Unlike code bugs that trigger stack traces, these failures accumulate invisibly across multiple system components, demanding engineering approaches designed specifically for probabilistic systems. Originally proposed in software engineering in the 1990s[^fn-tech-debt-ml], the technical debt metaphor compares shortcuts in implementation to financial debt, trading short-term velocity for ongoing interest payments in maintenance, refactoring, and systemic risk. In ML, this debt extends beyond code to include "hidden" costs unique to statistical modeling and data dependencies. Systematic evaluation rubrics, such as the ML Test Score [@breck2020ml], provide frameworks for quantifying this debt and assessing production readiness across data, model, and infrastructure components.

[^fn-tech-debt-ml]: **Technical Debt**: A metaphor for the long-term costs of short-term implementation shortcuts. Unlike traditional code debt, ML debt accrues silently in data and model dependencies, causing gradual performance degradation that conventional unit tests or code reviews cannot detect. The ML Test Score rubric makes this debt explicit, where a system scoring below 0.8 on its 1.0 scale is considered to have high-interest debt and is not production-ready. \index{Technical Debt!ML compounding}
...
[^fn-training-serving-skew-ops]: **Training-Serving Skew**: Occurs when feature logic is implemented differently for training (e.g., a data scientist's Python script) and serving (e.g., an engineer's Java service). These seemingly minor inconsistencies, such as calculating session length with wall-clock versus processing time, mean the model trains on one data reality and serves on another, causing silent accuracy degradation of 5--15%. \index{Training-Serving Skew!accuracy impact}
...
[^fn-github-actions-ml-latency]: **GitHub Actions**: As a general-purpose orchestrator, it connects version control events to execution logic but lacks native ML infrastructure. For ML workflows, this forces a handoff architecture where Actions delegates heavy jobs to specialized platforms like Kubeflow. This delegation itself creates a latency floor, adding 30-60 seconds of overhead per job before the actual ML task starts. \index{CI/CD!ML pipeline latency}
...
[^fn-canary-deploy-ml]: **Canary Deployment**: This strategy validates a new model by routing a small fraction of live traffic (e.g., 1-5%) to it, acting like a "canary in a coal mine" for production health. This controlled exposure is essential for ML systems, where a "failure" is often a statistical degradation in model performance, not a deterministic crash. A successful validation requires routing enough traffic—often thousands of inferences—to achieve the statistical power needed to confirm the new model is safe to roll out. \index{Canary Deployment!statistical validation}

[^fn-blue-green-deploy-ml]: **Blue-Green Deployment**: This strategy enables the safe validation described by maintaining two identical production environments: "blue" (stable) and "green" (candidate). The critical advantage for robust rollback is that reverting to the previous model is not a redeployment but a simple traffic switch. This switch takes seconds, minimizing disruption to near-zero compared to the hours a full rollback might otherwise require. \index{Blue-Green Deployment!rollback speed}
...
[^fn-tfserving-throughput]: **TensorFlow Serving**: Its design is tightly coupled with TensorFlow's SavedModel format, providing a highly optimized, native path from training to production. This specialization is the central operational trade-off: choosing its high-performance path for TensorFlow models—often exceeding 100,000 QPS per core for simple models—sacrifices the multi-framework flexibility of servers like Triton or the container-native scaling of KServe. \index{TensorFlow Serving!throughput}

[^fn-triton-serving]: **NVIDIA Triton Inference Server**: Its core mechanism is a framework-agnostic C++ backend that can load models from any source (TensorFlow, PyTorch, ONNX) via a standardized API. This directly enables the "multi-framework GPU serving" context, as its key operational capability is concurrently executing heterogeneous models on a single GPU. This model multiplexing maximizes accelerator utilization and avoids dedicated infrastructure per model type, supporting over 100 model instances on a single node. \index{Triton!multi-model serving}

[^fn-kserve-autoscale]: **KServe (formerly KFServing)**: As a Kubernetes-native framework, KServe uses Custom Resource Definitions (CRDs) to directly leverage Kubernetes' orchestration for serverless inference. This native integration enables its primary operational advantage mentioned in the text: scaling model pods down to zero when idle to eliminate the cost of unused compute, a critical feature for managing thousands of intermittently-used models. The direct trade-off for this cost efficiency is a "cold start" latency penalty on the first request that can exceed 5 seconds, making it a poor fit for applications with strict sub-second latency requirements. \index{KServe!autoscaling}
...
[^fn-bulkhead-ml-isolation]: **Bulkhead Pattern**: Inspired by the watertight compartments in a ship's hull, this pattern partitions system resources to contain failures within isolated zones. For isolating experimental models, a bulkhead dedicates a fixed compute and memory budget—often just 10–20% of total capacity—to the new version. This resource partition ensures that a catastrophic failure in the experiment, such as a memory leak, cannot exhaust all available resources and cause a system-wide production outage. \index{Bulkhead!ML resource isolation}

[^fn-byzantine-ml]: **Byzantine Fault Tolerance**: In ML systems, the classic Byzantine failure model shifts from arbitrary node failures to "semantic failures"—models producing plausible but incorrect predictions that pass health checks. Unlike a system crash, these semantic failures do not trigger availability-focused circuit breakers, silently corrupting application outcomes. Mitigating this risk with ensemble-based voting requires at least a 3x increase in computational overhead to guard against a single faulty model, echoing the classic $3f+1$ node requirement. \index{Byzantine Fault!ML plausible failures}
...
[^fn-cross-validation-ops]: **Five-Fold Cross-Validation**: This guards against the 79% accuracy being a statistical fluke from a single "lucky" data split. While the average accuracy is 79%, the performance across individual folds might range from 72--85%; this variance, not the average, determines if a model truly meets the clinical benchmark or risks degrading a deployed system. \index{Cross-Validation!edge ML}
