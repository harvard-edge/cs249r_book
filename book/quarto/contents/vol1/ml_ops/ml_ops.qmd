...
2.  **Distinction**: Unlike model decay\index{Model Decay!definition} (which implies internal failure, where the algorithm or code degraded), data drift is an external force (market shifts, sensor aging, user behavior change) that invalidates the model's learned mapping without any engineering error.
