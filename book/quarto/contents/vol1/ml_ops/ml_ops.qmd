...
***Machine Learning Operations (MLOps)***\index{MLOps!definition} is the engineering discipline that closes the feedback loop between model behavior and data reality by automating retraining, validation, and deployment in response to measurable production drift. [@kreuzberger2023machine]
...
$$T^* \approx \sqrt{\frac{2 \times {python} RetrainingInterval.retrain_cost_str}{({python} RetrainingInterval.Q_str) \times ({python} RetrainingInterval.V_str) \times ({python} RetrainingInterval.A0_str) \times ({python} RetrainingInterval.lam_str)}} \approx \sqrt{\frac{{python} RetrainingInterval.numerator_str}{{python} RetrainingInterval.denominator_str}} \approx {python} RetrainingInterval.T_star_str \text{ days}$$
...
Production deployment requires frameworks that handle model packaging, versioning, and integration with serving infrastructure. Tools like MLflow and model registries manage these deployment artifacts [@chen2020developments], while serving-specific frameworks (detailed in @sec-model-serving) handle the runtime optimization and scaling requirements.
...
: **ML Test Score Checklist.** A practical rubric for assessing ML system production readiness. Each test scores 0 (not implemented), 0.5 (partially implemented), or one (fully automated). Systems scoring below five require significant investment before production deployment; scores above ten indicate mature operational practices. Based on @breck2017ml. {#tbl-ml-test-score}
