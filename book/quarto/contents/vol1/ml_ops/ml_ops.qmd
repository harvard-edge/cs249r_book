...
class DriftDetectionDelay:
    ...
    drop_pct_str = fmt_pp(drop * 100, precision=0, commas=False, style="symbol")
...
3.  **Low-traffic case**: If the model only processes `{python} DriftDetectionDelay.requests_per_day_low_str`, detecting the same `{python} DriftDetectionDelay.drop_pct_str` drift takes `{python} DriftDetectionDelay.days_needed_low_str`.
...
**Scenario**: @Tbl-ml-ops-principle-mapping lays out how the Oura Ring and ClinAIOps environments implement the five foundational MLOps principles. Each case study demonstrates that domain constraints (edge hardware, clinical regulation) reshape *how* principles are implemented without changing *which* principles matter.
