...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, fmt_bandwidth, fmt_memory, fmt_percent, fmt_ratio, fmt_time, fmt_rate
...
class ObservabilitySampling:
    ...
    sampling_interval_ratio_str = fmt_ratio(sampling_ratio, precision=0, commas=False)
...
Both data volumes follow from the same two-step arithmetic. At full fidelity (`{python} ObservabilitySampling.high_seconds_str` sampling), every request's telemetry enters the stream: `{python} ObservabilitySampling.request_rate_m_req_per_s_str` $\times$ `{python} ObservabilitySampling.telemetry_kb_str` of telemetry per request = `{python} ObservabilitySampling.high_gb_per_s_str`. Stretching the interval to `{python} ObservabilitySampling.low_seconds_str` spreads that same stream over a window `{python} ObservabilitySampling.sampling_ratio_mult_str` longer, so the sampled volume is `{python} ObservabilitySampling.high_gb_per_s_str` ÷ `{python} ObservabilitySampling.sampling_interval_ratio_str` $\approx$ `{python} ObservabilitySampling.low_mb_per_s_str`. @Tbl-observability-sampling-cost places the two regimes side by side with their cost impact.
