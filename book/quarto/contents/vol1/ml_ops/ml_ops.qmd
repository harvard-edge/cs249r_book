...
**Scenario**: Consider a fraud detection model with the parameters in @tbl-retraining-parameters that captures the high query volume and rapid drift rate characteristic of financial fraud detection.

**Result**:
`{python} RetrainingInterval.T_star_eq`

**Systems insight**: Daily retraining is economically optimal for this high-volume, high-stakes fraud detection scenario because staleness cost accumulates faster than retraining cost.
...
**Pitfall**: *Alerting only after accuracy degradation becomes visible.*

An accuracy-only alert closes the loop too late because it waits for the user-facing metric to absorb the upstream problem. A stronger monitoring plan treats schema changes, feature freshness, distribution drift, and calibration movement as leading indicators, then routes those alerts to the same response machinery that handles accuracy regressions. The operational goal is not to replace accuracy monitoring, but to make accuracy the confirmation signal rather than the first sign of trouble.
