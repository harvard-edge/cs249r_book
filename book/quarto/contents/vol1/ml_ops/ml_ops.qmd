...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_usd, fmt, check, MarkdownStr
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_usd, fmt, check, MarkdownStr, fmt_percent
...
**Systems insight**: A single silent failure costs `{python} SilentFailureCost.loss_diff_str` more without MLOps. If the system experiences 3–`{python} SilentFailureCost.incidents_per_year_str` such incidents a year (typical for high-drift domains), MLOps saves nearly `{python} SilentFailureCost.annual_savings_str` (at `{python} SilentFailureCost.incidents_per_year_str` incidents). The "expensive" infrastructure pays for itself by reducing the *time-to-detection* (TTD) for the silent failures defined in the verification invariant.
...
# │ Exports: slo_p99_ms_str, network_ms_str, inference_ms_str, end_to_end_speedup_str, etc.
# └─────────────────────────────────────────────────────────────────────────────
...
class LatencyBudget:
    ...
    model_speedup_str = fmt_multiple(model_speedup, precision=0, commas=False)
    new_total_after_model_speedup_ms_str = fmt_time(new_total_after_model_speedup, 'millisecond', precision=1, commas=False)
    end_to_end_speedup_str = fmt_multiple(end_to_end_speedup, precision=1, commas=False)
```
...
For a `{python} CostPerInference.hourly_gpu_cost_str` A100 instance processing `{python} CostPerInference.inferences_per_hour_str` inferences/hour, cost is `{python} CostPerInference.cost_per_1k_inferences_usd_str`. Track this metric over time; increases indicate efficiency degradation requiring investigation.
...
# │          mlsysim.fmt (fmt, check)
# │ Exports: ObservabilitySampling.request_rate_m_req_s_str,
...
class ObservabilitySampling:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    request_rate_m_req_s_str = fmt_rate(requests_per_second.to(1/second).magnitude, "req/s", scale="M", precision=0, commas=False)
    ...
    high_gb_s_str = fmt_bandwidth(high_bytes_per_second, unit=GB/second, precision=0, commas=False)
    low_mb_s_str = fmt_bandwidth(low_bytes_per_second, unit=MB/second, precision=1, commas=False)
    sampling_ratio_str = fmt_multiple(sampling_ratio, precision=0, commas=False)
...
@Tbl-observability-sampling-cost contrasts the data-volume swing between high- and low-frequency sampling regimes at `{python} ObservabilitySampling.request_rate_m_req_s_str`:

|                      **Sampling**                     | **Granularity** | **Data Volume (`{python} ObservabilitySampling.request_rate_m_req_s_str`)** | **Cost Impact**                   |
|:-----------------------------------------------------:|:----------------|:---------------------------------------------------------------------------:|:----------------------------------|
...
: **Sampling Frequency vs. Observability Cost**: Roughly `{python} ObservabilitySampling.sampling_ratio_str`$\times$ data-volume swing between `{python} ObservabilitySampling.high_seconds_str` and `{python} ObservabilitySampling.low_seconds_str` sampling at `{python} ObservabilitySampling.request_rate_m_req_s_str`, assuming about `{python} ObservabilitySampling.telemetry_kb_str` of telemetry per request. {#tbl-observability-sampling-cost tbl-colwidths="[14,28,28,30]"}
...
from mlsysim.fmt import fmt_int, fmt_usd, fmt, check, fmt_memory, fmt_time
