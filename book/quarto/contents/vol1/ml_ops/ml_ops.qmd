...
**Failure mode**: The new code repurposed an old flag (`SMARS`). On the seven updated servers, this worked correctly. On the eighth server running old code, activating `SMARS` triggered obsolete Power Peg functionality whose stopping logic had been moved, causing it to emit repeated child orders. In forty-five minutes, the defective router generated millions of erroneous orders, accumulated an unintended multi-billion-dollar portfolio, and ultimately cost Knight more than \$460 million. The company needed emergency financing within days.
