...
*   **Break-even point**: `{python} AutomationROI.mc_pipeline_hours_str` engineering hours $\div$ `{python} AutomationROI.mc_manual_hours_str` per week = `{python} AutomationROI.mc_breakeven_weeks_str`.
...
**Context**: TensorFlow Extended (TFX)\index{TensorFlow Extended!production pipelines} emerged from Google's production ML infrastructure, providing reusable components for data validation, transformation, training, model analysis, and deployment [@baylor2017].
...
Because KL divergence is asymmetric and becomes infinite when $p(x)>0$ where $q(x)=0$, operational monitors must preserve direction and apply an explicit support policy, such as smoothing previously unseen bins.
...
ML monitoring is layered, not monolithic. The prompts below test whether each symptom can be traced to the responsible layer.
