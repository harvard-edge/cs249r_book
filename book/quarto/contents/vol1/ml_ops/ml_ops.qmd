...
MLOps\index{MLOps!DevOps comparison} builds on DevOps\index{DevOps!MLOps extension} but addresses the specific demands of ML system development and deployment [@kreuzberger2023; @amershi2019]. Traditional CI/CD can usually reason about code, configuration, tests, and infrastructure as the primary release objects. Machine learning systems add data distributions, learned parameters, evaluation slices, and monitoring feedback loops, so a release can become stale even when the code has not changed.
...
: **Monitoring strategy by workload archetype**: Illustrative starting points for monitoring strategy. Real thresholds must be calibrated to the deployment's label delay, traffic volume, business risk, and alert-fatigue budget. {#tbl-monitoring-archetype-strategy tbl-colwidths="[19,27,23,31]"}

**Systems insight**: Ground truth availability determines monitoring strategy. ResNet-50 (image classification) can use explicit labels; GPT-2 relies on proxy metrics (perplexity); Deep Learning Recommendation Model (DLRM) uses implicit feedback (clicks); DS-CNN, a depthwise-separable convolutional neural network (CNN), monitors operational metrics (energy, false positives). The illustrative retraining cadence spans roughly two orders of magnitude, from daily recommendation updates to much slower embedded-device updates.
...
The feature store is, in systems terms, the engineering mechanism that enforces the training-serving skew law: by centralizing feature definitions and serving them through a shared path, it reduces the pipeline divergence that otherwise causes silent production accuracy loss.
...
::: {#lst-feature-store-consistency lst-cap="**Feature Store Consistency**: Unified feature retrieval reduces training-serving skew by ensuring both pipelines access identical feature computations."}
...
By computing `session_length` once in the feature pipeline, training and serving see identical values. Centralized feature stores also support feature reuse and metadata tracking, which makes skew easier to detect and correct when a feature definition changes [@hermann2017michelangelo; @feast2019].
...
*Reference: [@hermann2017michelangelo]*
...
Teams view deployment as a terminal milestone\index{MLOps Pitfalls!one-time deployment} analogous to shipping software releases, but models degrade continuously due to data drift and distribution shift. @Sec-ml-operations-data-quality-monitoring-c6b6 establishes PSI as one useful distribution-shift signal whose thresholds must be calibrated to the feature and business risk. A fraud detection model can move from below the warning threshold to above the review threshold within months, turning initially acceptable accuracy into material degradation. The optimal retraining interval follows $T^* \approx \sqrt{\frac{2C}{Q \cdot V \cdot \text{Accuracy}_0 \cdot \gamma}}$ from @sec-ml-operations-quantitative-retraining-economics-1579, where high-volume systems require more frequent retraining than low-drift domains. Production ML requires continuous monitoring of feature distributions, performance metrics, and automated retraining triggers throughout the operational lifecycle.
