...
# │
# │ Imports: mlsysim.book (fmt)
# │ Exports: incidents_prevented_str, avg_incident_cost_str, monitoring_roi_factor_value_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_ratio, fmt_usd, check
...
class MonitoringROI:
    ...
    monitoring_roi_factor_value_str = fmt_ratio(monitoring_roi, precision=0, commas=False)
```
...
$$
\text{ROI} = \frac{`{python} MonitoringROI.incidents_prevented_str` \times `{python} MonitoringROI.avg_incident_cost_str`}{`{python} MonitoringROI.annual_monitoring_cost_str`} = `{python} MonitoringROI.monitoring_roi_factor_value_str`\times
$$
...
# │
# │ Imports: mlsysim.book (fmt)
# │ Exports: incidents_avoided_str, incident_savings_k_str, single_model_roi_factor_value_str,
# │          mlops_investment_k_str, etc.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_ratio, fmt_time, fmt_usd, check
...
class SingleModelROI:
    ...
    single_model_roi_factor_value_str = fmt_ratio(single_model_roi, precision=1, commas=False)
```
...
$$
\text{ROI} = \frac{`{python} SingleModelROI.incident_savings_k_str` + `{python} SingleModelROI.time_savings_k_str`}{`{python} SingleModelROI.mlops_investment_k_str`} = `{python} SingleModelROI.single_model_roi_factor_value_str`\times
$$

The `{python} SingleModelROI.single_model_roi_factor_value_str`\times return means the investment is not justified by tooling elegance; it is justified because the model is expensive enough that preventing incidents and shortening deployments outweigh the annual platform spend.
