...
class AutomationROI:
    ...
    mc_breakeven_str = fmt_time(breakeven_weeks, "week", precision=0, commas=False, style="word")
    mc_time_horizon_str = fmt_time(time_horizon_years, "year", precision=0, commas=False, style="word")
...
class RetrainingInterval:
    ...
    V_str = fmt_usd(V, precision=2, commas=False, per="query")
...
*   **Utility** $(V)$: `{python} RetrainingInterval.V_str` for a unit accuracy change.
...
class LatencyBudget:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    slo_p99_str = fmt_time(slo_p99, 'millisecond', precision=0, commas=False)
    network_str = fmt_time(network, 'millisecond', precision=0, commas=False)
    feature_fetch_str = fmt_time(feature_fetch, 'millisecond', precision=0, commas=False)
    request_parse_str = fmt_time(request_parse, 'millisecond', precision=0, commas=False)
    inference_str = fmt_time(inference, 'millisecond', precision=0, commas=False)
    post_proc_str = fmt_time(post_proc, 'millisecond', precision=0, commas=False)
    serialization_str = fmt_time(serialization, 'millisecond', precision=0, commas=False)
    non_inference_str = fmt_time(non_inference, 'millisecond', precision=0, commas=False)
    ...
    new_total_after_model_speedup_str = fmt_time(new_total_after_model_speedup, 'millisecond', precision=1, commas=False)
...
class ObservabilitySampling:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    request_rate_m_str = fmt_rate(requests_per_second.to(1/second).magnitude, "req/s", scale="M", precision=0, commas=False)
...
@Tbl-observability-sampling-cost contrasts the data-volume swing between high- and low-frequency sampling regimes at `{python} ObservabilitySampling.request_rate_m_str`:

|                      **Sampling**                     | **Granularity** | **Data Volume (`{python} ObservabilitySampling.request_rate_m_str`)** | **Cost Impact**                   |
|:-----------------------------------------------------:|:----------------|:---------------------------------------------------------------------:|:----------------------------------|
...
class MonitoringBudget:
    ...
    ingest_str = fmt_usd(ingestion_cost, precision=1, commas=False, per="month")
    ...
    total_str = fmt_usd(total_cost, precision=1, commas=False, per="month")
```
...
- Cost at `{python} MonitoringBudget.ingestion_cost_per_m_str`: **`{python} MonitoringBudget.ingest_str`**
...
**Total**: ~`{python} MonitoringBudget.total_str` for a single ML node
