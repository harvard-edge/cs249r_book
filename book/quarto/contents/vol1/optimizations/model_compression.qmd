...
The preceding chapter on data efficiency (@sec-data-efficiency) examined how to extract maximum learning signal from training data through techniques like active learning, data augmentation, and dataset curation. This chapter turns to the complementary challenge of model efficiency: transforming trained models into deployable systems that meet resource constraints. While data efficiency optimizes what enters the training pipeline, model compression optimizes what emerges from it. Together, these approaches address the full efficiency spectrum from training inputs to deployment outputs.

The efficiency framework from @sec-introduction established three pillars of ML system efficiency: algorithmic, compute, and data. This chapter provides the engineering techniques for the first two pillars, reducing model size and accelerating inference through systematic optimization of model representation, numerical precision, and architectural design. The resource gap motivating these techniques is substantial. State-of-the-art language models may require hundreds of gigabytes for parameter storage [@brown2020gpt3], while mobile devices often provide only a few gigabytes of memory. This disparity extends to computational throughput, energy consumption, and latency. Production systems must balance these conflicting objectives: real-time applications impose strict latency bounds, while mobile deployments require energy efficiency.
