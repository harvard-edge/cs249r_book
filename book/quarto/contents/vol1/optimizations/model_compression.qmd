...
[^fn-flashattention]: **FlashAttention**: Memory-efficient attention algorithm that reduces HBM memory traffic from O(n^2) to O(n) by computing attention in tiles that fit in SRAM. Achieves 2-4× speedup on GPT-2 with 1K context and enables 64K context windows that would otherwise exceed memory capacity. The key insight is that attention matrices need not be fully materialized; intermediate results can be accumulated in fast on-chip memory.
