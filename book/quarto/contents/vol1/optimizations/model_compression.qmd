...
These three dimensions of optimization are not independent choices but complementary tools. Production deployments typically combine pruning to reduce computation, quantization to accelerate execution, and architecture search to optimize the underlying structure. Hardware-aware design closes the loop by aligning these choices with tensor core formats, SIMD widths, and memory hierarchies on the target platform. The deployment contexts introduced earlier illustrate how these techniques compose: mobile applications might prioritize quantization for power efficiency, while cloud deployments might favor structural pruning for throughput. Understanding these interactions enables practitioners to select and sequence techniques that match their specific deployment constraints.
...
* **PTQ is a strong baseline**: INT8 post-training quantization requires no retraining and delivers 4× compression. Use QAT or distillation when baseline accuracy is insufficient for the application.
