...
# PURPOSE
# =============================================================================
# Purpose: Chapter-wide constants for model compression calculations
# Used in: Multiple sections, tables, and callouts in this chapter
...
energy_add_fp32_pj_value = 0.9
energy_add_fp16_pj_value = 0.4
energy_add_int32_pj_value = 0.1
energy_add_int8_pj_value = 0.03
energy_mul_fp32_pj_value = ENERGY_FLOP_FP32_PJ.magnitude

# INT8 vs FP32 energy ratio
int8_fp32_energy_ratio_value = energy_mul_fp32_pj_value / energy_add_int8_pj_value
...
llm_175b_params_value = GPT3_PARAMS.magnitude / 1e9
llm_175b_mem_fp16_gb_value = llm_175b_params_value * 2

# Device memory constraints
smartphone_ram_gb_value = 6
mcu_ram_kb_value = 512
...
gpt3_training_flops_str = f"3.14×10^{gpt3_training_flops_exp_value}"
```
...
# PURPOSE
# =============================================================================
# Purpose: Plot quantization free-lunch curve
# Used in: Figure "The Quantization Free Lunch"
...
# PURPOSE
# =============================================================================
# Purpose: Quantization speedup example for 7B LLM
# Used in: Callout "The Quantization Speedup"
...
params_b_value = 7
bytes_fp16_value = 2
bytes_int4_value = 0.5
device_ram_gb_value = 16
mem_bw_gbs_value = 50
kv_cache_gb_value = 1
...
# PROCESS
# =============================================================================
fp16_size_gb_value = params_b_value * bytes_fp16_value
fp16_total_gb_value = fp16_size_gb_value + kv_cache_gb_value
fp16_latency_ms_value = fp16_size_gb_value / mem_bw_gbs_value * 1000
fp16_toks_per_sec_value = 1000 / fp16_latency_ms_value

int4_size_gb_value = params_b_value * bytes_int4_value
int4_latency_ms_value = int4_size_gb_value / mem_bw_gbs_value * 1000
int4_toks_per_sec_value = 1000 / int4_latency_ms_value
speedup_value = int4_toks_per_sec_value / fp16_toks_per_sec_value

# =============================================================================
# OUTPUT
# =============================================================================
params_b_str = fmt(params_b_value, precision=0, commas=False)
billion_exp_str = "10^9"
bytes_fp16_str = fmt(bytes_fp16_value, precision=0, commas=False)
bytes_int4_str = fmt(bytes_int4_value, precision=0, commas=False)
device_ram_gb_str = fmt(device_ram_gb_value, precision=0, commas=False)
mem_bw_gbs_str = fmt(mem_bw_gbs_value, precision=0, commas=False)
kv_cache_gb_str = fmt(kv_cache_gb_value, precision=0, commas=False)

fp16_size_str = fmt(fp16_size_gb_value, precision=0, commas=False)
fp16_total_str = fmt(fp16_total_gb_value, precision=0, commas=False)
fp16_latency_str = fmt(fp16_latency_ms_value, precision=0, commas=False)
fp16_toks_str = fmt(fp16_toks_per_sec_value, precision=1, commas=False)
int4_size_str = fmt(int4_size_gb_value, precision=1, commas=False)
int4_latency_str = fmt(int4_latency_ms_value, precision=0, commas=False)
int4_toks_str = fmt(int4_toks_per_sec_value, precision=0, commas=False)
...
1.  **Model Size**: `{python} params_b_str` × `{python} billion_exp_str` params × `{python} bytes_fp16_str` bytes = `{python} fp16_size_str` GB.
...
2.  **New Size**: `{python} params_b_str` × `{python} billion_exp_str` × `{python} bytes_int4_str` = `{python} int4_size_str` GB.
...
from physx.constants import GiB, MiB, KiB, MB, KB, byte

# =============================================================================
# PURPOSE
# =============================================================================
# Purpose: Compare model memory requirements to device capacities
# Used in: Table "Model vs Device" and deployment gap discussion

# =============================================================================
# INPUT
# =============================================================================
cloud_mem_value = 100 * GiB
mobile_mem_value = 8 * GiB
tiny_mem_value = 512 * KiB

dlrm_mem_value = 100 * GiB
...
def get_ratio(model_mem, device_mem):
    ratio = (model_mem / device_mem).magnitude
    if ratio < 1:
        return "ok"
...
# OUTPUT
# =============================================================================
dlrm_str = f"{dlrm_mem_value.to(GiB).magnitude:.0f} GB"
...
# PURPOSE
# =============================================================================
# Purpose: Compare memory footprint for full vs low-rank factorization
# Used in: Callout "The Bandwidth-Compute Trade-off"
...
# PROCESS
# =============================================================================
full_bytes_value = mat_dim_value * mat_dim_value * bytes_fp32_value
full_mb_value = full_bytes_value / (1024**2)

factored_bytes_value = (mat_dim_value * rank_k_value + rank_k_value * mat_dim_value) * bytes_fp32_value
factored_mb_value = factored_bytes_value / (1024**2)

data_reduction_value = full_mb_value / factored_mb_value

# =============================================================================
# OUTPUT
# =============================================================================
full_mb_str = fmt(full_mb_value, precision=0, commas=False)
factored_mb_str = fmt(factored_mb_value, precision=0, commas=False)
data_reduction_str = fmt(data_reduction_value, precision=0, commas=False)
mat_dim_str = fmt(mat_dim_value, precision=0, commas=False)
rank_k_str = fmt(rank_k_value, precision=0, commas=False)
```

::: {.callout-notebook title="The Bandwidth-Compute Trade-off"}
**Reducing the Memory Pressure**: Low-rank factorization illustrates a classic systems trade-off: **trading computation for bandwidth reduction**. Storing a `{python} mat_dim_str` × `{python} mat_dim_str` matrix requires `{python} full_mb_str` MB (at FP32). Fetching this matrix for a single inference is a massive memory bandwidth hit, especially when limited by physical memory bandwidth constraints.

If we factorize it with rank k = `{python} rank_k_str`, we store two matrices (`{python} mat_dim_str` × `{python} rank_k_str` and `{python} rank_k_str` × `{python} mat_dim_str`), totaling only `{python} factored_mb_str` MB, a **`{python} data_reduction_str` x reduction in data movement**. While the number of floating-point operations (FLOPs) actually *increases* slightly because we perform two smaller matrix multiplies instead of one large one, the system speedup is often dramatic. By reducing data movement by `{python} data_reduction_str`×, we allow the processor to spend more time computing and less time waiting for memory.
:::
...
# PURPOSE
# =============================================================================
# Purpose: Compute quantization storage savings for LLM deployment
# Used in: Callout "Quantization Savings"

# =============================================================================
# INPUT
# =============================================================================
llm_params_b_value = 8
fp16_bytes_value = 2
int4_bytes_value = 0.5

# =============================================================================
# PROCESS
# =============================================================================
params_count_value = llm_params_b_value * 1e9
fp16_size_gb_value = params_count_value * fp16_bytes_value / 1e9
int4_size_gb_value = params_count_value * int4_bytes_value / 1e9
compression_ratio_value = fp16_size_gb_value / int4_size_gb_value

# =============================================================================
# OUTPUT
# =============================================================================
llm_params_b_str = fmt(llm_params_b_value, precision=0, commas=False)
billion_exp_llm_str = "10^9"
fp16_bytes_str = fmt(fp16_bytes_value, precision=0, commas=False)
int4_bytes_str = fmt(int4_bytes_value, precision=0, commas=False)
fp16_size_gb_str = fmt(fp16_size_gb_value, precision=0, commas=False)
int4_size_gb_str = fmt(int4_size_gb_value, precision=0, commas=False)
compression_ratio_str = fmt(compression_ratio_value, precision=0, commas=False)
```
...
- **Size**: `{python} llm_params_b_str` × `{python} billion_exp_llm_str` × `{python} fp16_bytes_str` bytes (16-bit) = `{python} fp16_size_gb_str` GB
- **Hardware Req**: Requires 24GB GPU (e.g., A10G, 3090, 4090).

**INT4 (4-bit Quantization)**

- **Size**: `{python} llm_params_b_str` × `{python} billion_exp_llm_str` × `{python} int4_bytes_str` bytes (4-bit) = `{python} int4_size_gb_str` GB
...
# PURPOSE
# =============================================================================
# Purpose: Compute SIMD throughput gain moving from FP32 to INT8
# Used in: Callout "The SIMD Multiplier"

# =============================================================================
# INPUT
# =============================================================================
simd_register_bits_value = 512
simd_fp32_bits_value = 32
simd_int8_bits_value = 8

# =============================================================================
# PROCESS
# =============================================================================
simd_fp32_ops_value = simd_register_bits_value // simd_fp32_bits_value
simd_int8_ops_value = simd_register_bits_value // simd_int8_bits_value
simd_gain_value = simd_int8_ops_value // simd_fp32_ops_value

# =============================================================================
# OUTPUT
# =============================================================================
simd_fp32_str = f"{simd_fp32_ops_value}"
simd_int8_str = f"{simd_int8_ops_value}"
simd_gain_str = f"{simd_gain_value}"
simd_register_bits_str = fmt(simd_register_bits_value, precision=0, commas=False)
simd_fp32_bits_str = fmt(simd_fp32_bits_value, precision=0, commas=False)
simd_int8_bits_str = fmt(simd_int8_bits_value, precision=0, commas=False)
```
...
Throughput Gain = INT8 Ops/Cycle / FP32 Ops/Cycle = `{python} simd_int8_str` / `{python} simd_fp32_str` = `{python} simd_gain_str`×
...
1. **Tensor Core Throughput**: NVIDIA A100 delivers `{python} a100_tflops_fp16_str` TFLOPS for FP16 vs `{python} a100_tflops_int8_str` TOPS for INT8, a `{python} a100_int8_speedup_str` x peak throughput increase.
...
[^fn-depthwise-separable-efficiency]: **Depthwise Separable Convolutions**: Factorizes standard convolution into depthwise (per-channel) and pointwise (1×1) operations, reducing computation by 8-9x. MobileNetV2 achieves 72% ImageNet accuracy with `{python} mobilenetv2_mflops_str` M FLOPs vs. ResNet-50's 76% with `{python} resnet_gflops_str` B FLOPs (13.7x fewer operations). Enables real-time inference on mobile devices.
...
# PURPOSE
# =============================================================================
# Purpose: Quantify memory transfer reduction from Conv-BN-ReLU fusion
# Used in: Operator fusion discussion
...
# PURPOSE
# =============================================================================
# Purpose: Estimate traffic reduction from element-wise fusion
# Used in: Element-wise fusion performance discussion

# =============================================================================
# INPUT
# =============================================================================
tensor_mb_value = 1
unfused_traffic_mb_value = 8  # 8 memory transfers × 1 MB
fused_traffic_mb_value = 3    # 3 memory transfers × 1 MB

# =============================================================================
# PROCESS
# =============================================================================
ew_reduction_value = unfused_traffic_mb_value / fused_traffic_mb_value
saved_traffic_mb_value = unfused_traffic_mb_value - fused_traffic_mb_value
saved_us_value = saved_traffic_mb_value / (v100_bw_gbs_value * 1000) * 1e6

# =============================================================================
# OUTPUT
# =============================================================================
ew_reduction_str = fmt(ew_reduction_value, precision=2, commas=False)
saved_us_str = fmt(saved_us_value, precision=0, commas=False)
```

For tensors of size 1 MB, unfused execution performs 8 MB of memory traffic. Fused execution reduces this to 3 MB (`{python} ew_reduction_str`× reduction). On a GPU with `{python} v100_bw_gbs_str` GB/s memory bandwidth, this saves approximately `{python} saved_us_str` microseconds per operation, accumulating to milliseconds of latency reduction over thousands of operations in a full model.
...
On a V100 GPU with `{python} v100_bw_gbs_str` GB/s HBM bandwidth and `{python} v100_tflops_fp32_str` TFLOPS FP32 compute, the unfused sequence takes approximately 8 microseconds (memory-bound), while the fused version takes 4.3 microseconds (1.86× speedup). The speedup comes entirely from reducing memory traffic, as the compute remains identical.
