...
Throughput Gain = INT8 elements/inst / FP32 elements/inst = `{python} SIMDThroughput.simd_int8_str`/`{python} SIMDThroughput.simd_fp32_str` = `{python} SIMDThroughput.simd_gain_str`$\times$

**The Systems Conclusion**: Quantization delivers up to **`{python} SIMDThroughput.simd_gain_str`$\times$ speedup** on compute-bound layers from vector packing alone, on hardware whose INT8 vector instructions have comparable throughput to FP32, even before considering memory bandwidth savings.
