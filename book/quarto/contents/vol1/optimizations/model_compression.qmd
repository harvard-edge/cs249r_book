...
With the deployment context established, we now examine each optimization dimension in depth, beginning with structural methods that modify *what* computations occur, then precision techniques that reduce numerical fidelity, and finally architectural approaches that improve execution efficiency.
...
Structural optimization addresses the first dimension of our framework — **Efficient Model Representation** — by modifying *what* the model computes. Modern neural networks are heavily overparameterized[^fn-gradient-checkpointing], often containing far more parameters than are strictly necessary to solve a task. This redundancy is not a design flaw; it is essential for training, where over-capacity helps optimization navigate complex loss landscapes. But at deployment time, that surplus becomes pure waste: unnecessary memory, unnecessary computation, unnecessary energy.
...
The three pruning approaches — unstructured, structured, and dynamic — represent fundamentally different positions on the regularity-versus-compression trade-off. Unstructured pruning achieves the highest compression ratios because it can remove any individual weight, but the resulting irregular sparsity patterns are difficult for hardware to exploit: accelerators optimized for dense matrix operations[^fn-flops] cannot skip individual zero values without specialized sparse execution kernels. Structured pruning sacrifices some compression potential by removing entire channels, filters, or layers, but the resulting dense sub-network runs efficiently on commodity hardware without any sparse computation support. Dynamic pruning offers the most flexibility by adapting pruning decisions to each input at runtime, but this flexibility comes at the cost of implementation complexity and computational overhead for real-time pruning decisions. @tbl-pruning formalizes these comparisons across the dimensions that matter most for deployment.
...
##### Efficiency Gains and Trade-offs {#sec-model-compression-efficiency-gains-3a1d}

[]{#sec-model-compression-memory-model-compression-310d}
[]{#sec-model-compression-computation-inference-speed-71a6}
[]{#sec-model-compression-deployment-system-considerations-be9a}
[]{#sec-model-compression-distillation-tradeoffs-8b4e}
...
In practice, the choice between PTQ and QAT follows a simple decision rule: start with PTQ and measure accuracy on your validation set. If accuracy meets the production threshold, ship it — the engineering cost of QAT is not justified. If PTQ falls short, particularly for transformer-based architectures where attention mechanisms amplify quantization noise, invest in QAT to recover the gap. A hybrid approach, starting with PTQ calibration and applying QAT fine-tuning only for accuracy-critical layers, often provides the best balance.
...
Model compression is not a bag of tricks but an engineering discipline built on three complementary dimensions: *structural optimization* determines what the model computes, *precision optimization* determines how precisely it computes, and *architectural optimization* determines how efficiently those computations execute on physical hardware. The most important lesson of this chapter is that these dimensions compose multiplicatively. Pruning alone might achieve 2× compression; quantization alone might achieve 4×; but pruning, distillation, and quantization applied together can achieve 16× — as BERT's compression from 440MB to 28MB demonstrates. The second lesson is equally important: theoretical compression ratios lie. A 4× reduction in parameters translates to 4× latency improvement only when the optimization aligns with the hardware's execution model. Unstructured sparsity on hardware that lacks sparse kernels achieves almost nothing; INT8 quantization on hardware without INT8 units achieves even less. Profile on target hardware, not paper metrics.
