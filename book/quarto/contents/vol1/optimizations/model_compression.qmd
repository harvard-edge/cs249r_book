...
# │
# │ Why: Centralizes hardware specs, energy constants, and model parameters
# │      so that prose references stay consistent and updatable from a single
# │      source of truth (physx.constants).
# │
...
[^fn-nas]: **Neural Architecture Search (NAS)**\index{NAS!etymology}: Pioneered by Barret Zoph and Quoc V. Le at Google Brain [@zoph2017neural], who used reinforcement learning to *learn* the architecture itself. The initial cost was staggering: 800 GPUs for 28 days. This expense catalyzed efficient NAS: weight-sharing approaches (ENAS, 2018) reduced search cost by 1000×, while hardware-aware NAS [@tan2019mnasnet] incorporated latency targets directly into the search objective. The resulting architectures---EfficientNet, MobileNetV3, NASNet---consistently outperform hand-designed models, demonstrating that architectural inductive biases (see @sec-dnn-architectures) can themselves be learned.
...
The **Energy-Movement Invariant** ($E_{move} \gg E_{compute}$) means that in the physics of silicon, **bits represent energy** (see @sec-system-foundations-numerical-representations-7b2f for a detailed comparison of FP32 vs. INT8 energy costs).
...
# │
# │ Why: Visualizes the "Free Lunch Zone" where reducing precision from FP32
# │      to INT8 incurs <1% accuracy loss, contrasted with the "Cliff" below
# │      4 bits. Builds intuition for why INT8 quantization is nearly free.
# │
# │ Imports: numpy (np), physx (viz)
...
# │
# │ Why: Demonstrates the concrete bandwidth-driven speedup from INT4
# │      quantization on a 7B LLM. Shows that quantization is not just
# │      about fitting the model but yields a linear speedup because LLM
# │      generation is bandwidth-bound.
# │
# │ Imports: physx.formatting (fmt), physx.constants (BYTES_FP16, BYTES_INT4, byte)
...
# --- Inputs (7B LLM deployment scenario) ---
params_b_value = 7
...
# --- Process (FP16 vs INT4 comparison) ---
...
Optimization requirements vary dramatically across deployment contexts. What matters for cloud inference differs sharply from mobile or embedded systems. @tbl-deployment-scenarios summarizes the key constraints across deployment environments.
...
# │
# │ Why: Quantifies the gap between model memory requirements and device
# │      capacities across cloud, mobile, and TinyML targets. Shows that
# │      even aggressively optimized models exceed TinyML constraints,
# │      motivating the compression techniques in the rest of the chapter.
# │
# │ Imports: physx.constants (GB, GiB, MiB, KiB, MB, KB, byte,
...
# │          dscnn_tiny_str
# └─────────────────────────────────────────────────────────────────────────────
from physx.constants import (GB, GiB, MiB, KiB, MB, KB, byte,
...
# │
# │ Why: Compares the memory footprint of a full weight matrix vs. its
# │      low-rank factorization, showing how rank-k SVD reduces storage
# │      by ~16× for a 4096×4096 matrix with rank 128.
# │
...
# --- Inputs (matrix dimensions and precision) ---
mat_dim_value = 4096
rank_k_value = 128
bytes_fp32_value = 4

# --- Process (full vs factored storage) ---
...
# │
# │ Why: Computes the concrete storage savings from FP16→INT4 quantization
# │      for an 8B parameter LLM, showing how 4× compression enables
# │      deployment on commodity hardware (8GB GPU vs. 24GB GPU).
# │
# │ Imports: physx.formatting (fmt), physx.constants (BYTES_FP16, BYTES_INT4, byte, GB)
...
# │
# │ Why: Demonstrates why INT8 is 4× faster than FP32 on the same processor
# │      by computing how many operations fit in a fixed-width SIMD register.
# │      Connects quantization savings to hardware parallelism.
# │
# │ Imports: physx.formatting (fmt),
# │          physx.constants (SIMD_REGISTER_BITS, FP32_BITS, INT8_BITS)
...
Another technique is activation checkpointing\index{Activation Checkpointing}\index{Memory Optimization!activation checkpointing}[^fn-activation-checkpointing-compression], especially beneficial during training. Standard backpropagation requires storing all forward activations for the backward pass, which creates significant memory overhead for large models. Activation checkpointing reduces memory consumption by storing only a subset of activations and recomputing the remainder when needed.

[^fn-activation-checkpointing-compression]: **Activation Checkpointing**: Also called gradient checkpointing. Reduces memory usage by 20--50% in large transformers with only 15--20% training time overhead by recomputing activations instead of storing them. See @sec-ai-training for the full etymology and systems analysis.
...
# │
# │ Why: Quantifies three fusion benefits: intermediate memory elimination
# │      (Conv-BN-ReLU saves ~1.5 MB per layer), bandwidth reduction (~55%
# │      fewer memory transfers), and kernel launch overhead reduction
# │      (159→53 kernels). Grounds the abstract fusion concept in concrete
# │      ResNet-50 and BERT measurements.
# │
# │ Imports: physx.formatting (fmt)
...
# │
# │ Why: Quantifies the 3× memory transfer reduction from fusing Conv-BN-ReLU
# │      (6 transfers → 2), and renders the intermediate traffic as a LaTeX
# │      math expression for inline display.
# │
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ ELEMENT-WISE FUSION CALCULATION
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Element-wise fusion performance discussion, after
# │          @lst-elementwise-fusion
# │
# │ Why: Quantifies the 2.67× memory traffic reduction from fusing four
# │      element-wise operations into one kernel (8 MB → 3 MB), and
# │      estimates the time saved on a V100 GPU. Also renders GEMM
# │      intermediate size as LaTeX for the next subsection.
# │
# │ Imports: physx.formatting (fmt, md_math)
# │ Exports: ew_reduction_str, saved_us_str, gemm_mem_md
# │ Depends: v100_bw_gbs_value (from compression-setup),
# │          gemm_intermediate_mb_str (from fusion-calc)
# └─────────────────────────────────────────────────────────────────────────────
from physx.formatting import fmt, md_math

# --- Inputs (element-wise fusion scenario) ---
tensor_mb_value = 1
