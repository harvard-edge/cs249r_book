...
We have optimized the **Data** in the previous chapter (@sec-data-selection), maximizing the information density of our training set. Now we turn our attention to the **Algorithm**: the model artifact itself.

Part II established how to build ML systems—architectures, frameworks, and training pipelines that transform data into capable models. This chapter addresses what happens next: optimizing the *outputs* of training to transform capable models into deployable ones that meet real-world constraints.
...
The scale of this renegotiation makes model optimization an engineering discipline, not a collection of ad hoc tricks. A 175 billion parameter model consumes over 300 GB in FP16 representation alone, yet a smartphone provides 6 GB of RAM and a microcontroller offers 512 KB. Bridging five orders of magnitude between what training produces and what deployment permits requires systematic methods with predictable trade-offs, not trial and error. Every optimization technique removes something from the model, whether redundant parameters, numerical precision, or architectural complexity, and the engineer must understand exactly what is lost, what is preserved, and how these losses compose when techniques are combined.

This chapter organizes these techniques along three complementary dimensions: *structural* optimization (pruning away unnecessary parameters, distilling knowledge into smaller architectures, or searching for inherently efficient designs), *precision* optimization (reducing the numerical bit-width of weights and activations), and *hardware-level* optimization (fusing operations and exploiting sparsity to match the target silicon). These dimensions are not alternatives but layers in an optimization stack. A practitioner deploying ResNet-50 to a mobile device might prune 50% of its filters, quantize the remaining weights to INT8, and fuse batch normalization into convolution, with each technique compounding the gains of the others.
...
|                           | **(Runtime)**  | **(Weights)** | **(~100 GB)**| **(~8 GB)**  | **(~512 KB)**|
+:==========================+===============:+==============:+:============:+:============:+:============:+
| **ResNet-50**             | 100 MB         | 98 MB         | ✓            | ✓            | ✗ (200×)     |
+---------------------------+----------------+---------------+--------------+--------------+--------------+
| **MobileNetV2**           | 14 MB          | 14 MB         | ✓            | ✓            | ✗ (27×)      |
+---------------------------+----------------+---------------+--------------+--------------+--------------+
| **MobileNetV2 (INT8)**    | 3.5 MB         | 3.5 MB        | ✓            | ✓            | ✗ (7×)       |
+---------------------------+----------------+---------------+--------------+--------------+--------------+
| **DS-CNN (KWS)**          | 500 KB         | 500 KB        | ✓            | ✓            | ✓            |
+---------------------------+----------------+---------------+--------------+--------------+--------------+
...
**Pruning Strategies.** Beyond the broad categories of unstructured, structured, and dynamic pruning, different pruning workflows can impact model efficiency and accuracy retention. Two widely used pruning strategies are iterative pruning and one-shot pruning, each with its own benefits and trade-offs.
...
1. What is the key difference between structured and unstructured pruning in terms of hardware efficiency? Consider how each interacts with GPU and TPU execution patterns.
2. Why does knowledge distillation typically preserve accuracy better than aggressive pruning? Think about what information each method retains from the original model.
3. When would you choose Neural Architecture Search over manual architecture design? Consider the trade-offs in computational cost, design space coverage, and hardware-specific optimization.
:::
...
Beyond storage savings, quantization also accelerates computation through hardware parallelism. The following calculation illustrates why lower-precision formats execute faster.

::: {.callout-notebook title="Napkin Math: The SIMD Multiplier"}
...
2.  **FP32 Capacity**: $512 / 32 = 16$ operations per cycle.
3.  **INT8 Capacity**: $512 / 8 = 64$ operations per cycle.
...
@fig-quantization reveals how the quantization error distribution differs across numerical formats, with each format introducing varying levels of quantization noise that directly influence model accuracy and stability. Understanding this noise is essential, but practitioners ultimately care about end-to-end speedup. The magnitude of that speedup depends on whether a workload is compute-bound or memory-bound.
...
: **Quantization Trade-Offs**: Post-training quantization, quantization-aware training, and dynamic quantization represent distinct approaches to model compression. ✓ = present, ✗ = absent, △ = input-dependent. {#tbl-quantization_methods}
...
1. Why does INT8 quantization provide roughly 4x memory reduction but potentially more than 4x energy reduction? Consider the energy cost of floating-point versus integer arithmetic units.
2. What is the key advantage of quantization-aware training (QAT) over post-training quantization (PTQ), and when is the extra training cost justified?
3. For memory-bandwidth-bound LLM inference, why does weight quantization provide nearly linear speedup with bit-width reduction? Think about which resource is the bottleneck during autoregressive generation.
:::
...
**Adaptive Computation.** While operator fusion optimizes memory access patterns for fixed computation graphs, adaptive computation methods address a different inefficiency: conventional models apply uniform processing to all inputs regardless of complexity, wasting resources on simple cases and increasing power consumption. Dynamic computation allows models to skip layers or operations for simple inputs while engaging deeper networks for complex cases, optimizing computational efficiency, energy consumption, and latency while preserving predictive performance. This capability proves essential for resource-constrained hardware in mobile devices, embedded systems, and autonomous vehicles where real-time processing is critical. Dynamic schemes enable models to selectively reduce computation when inputs are simple, preserving resources while maintaining predictive performance.
...
* **Profile on target hardware—paper metrics mislead**: FLOPs and parameter count often mispredict real-world performance. A 2× FLOP reduction may yield only 1.2× speedup.
