...
The chapter proceeds through the three layers of the optimization stack from software to hardware. We begin with the **optimization framework** that positions these layers and the **deployment context** that determines which optimizations matter for a given target. We then examine **structural optimization**—pruning, knowledge distillation, and neural architecture search—that removes redundancy from the model itself. Next comes **quantization and precision optimization**, where reducing numerical bit-width from FP32 to INT8 or below yields multiplicative gains in both memory and throughput. We then turn to **architectural efficiency** techniques—operator fusion, sparsity exploitation, and graph-level transformations—that bridge the gap between compressed models and the hardware that executes them. The chapter closes with practical guidance on **technique selection**, **optimization strategies** for composing multiple techniques, and **measurement** methodologies for evaluating the resulting efficiency gains.
...
from physx.constants import BYTES_FP16, BYTES_INT4, byte
...
# OUTPUT
# =============================================================================
params_b_str = fmt(params_b_value, precision=0, commas=False)      # e.g. "7"
bytes_fp16_str = fmt(bytes_fp16_value, precision=0, commas=False)  # e.g. "2"
bytes_int4_str = fmt(bytes_int4_value, precision=1, commas=False)  # e.g. "0.5"
device_ram_gb_str = fmt(device_ram_gb_value, precision=0, commas=False)  # e.g. "16"
mem_bw_gbs_str = fmt(mem_bw_gbs_value, precision=0, commas=False)  # e.g. "50"
kv_cache_gb_str = fmt(kv_cache_gb_value, precision=0, commas=False)  # e.g. "1"

fp16_size_str = fmt(fp16_size_gb_value, precision=0, commas=False)    # e.g. "14"
fp16_total_str = fmt(fp16_total_gb_value, precision=0, commas=False)  # e.g. "15"
fp16_latency_str = fmt(fp16_latency_ms_value, precision=0, commas=False)  # e.g. "280"
fp16_toks_str = fmt(fp16_toks_per_sec_value, precision=1, commas=False)   # e.g. "3.6"
int4_size_str = fmt(int4_size_gb_value, precision=1, commas=False)    # e.g. "3.5"
int4_latency_str = fmt(int4_latency_ms_value, precision=0, commas=False)  # e.g. "70"
int4_toks_str = fmt(int4_toks_per_sec_value, precision=0, commas=False)   # e.g. "14"
speedup_str = fmt(speedup_value, precision=0, commas=False)           # e.g. "4"
```
...
The deployment gap table below quantifies this mismatch using the Lighthouse models from @sec-ml-system-architecture. The gap between model requirements and device capabilities explains *why* compression is not optional for resource-constrained deployment: without it, the models simply cannot run.
...
## Structural Optimization {#sec-model-compression-structural-model-optimization-methods-8ed4}
...
[]{#sec-model-compression-distillation-process-2f08}
...
##### Distillation Mathematics {#sec-model-compression-distillation-mathematics-0c2b}
...
##### Efficiency Gains {#sec-model-compression-efficiency-gains-3a1d}
...
##### Trade-offs {#sec-model-compression-distillation-tradeoffs-8b4e}
...
#| echo: false
from physx.formatting import fmt
from physx.constants import BYTES_FP16, BYTES_INT4, byte, GB
...
[]{#sec-model-compression-comparing-factorization-approaches-5c7d}
...
##### Cell-Based Search Spaces {#sec-model-compression-cell-based-search-spaces-a3f1}
...
##### Hardware-Aware Search Spaces {#sec-model-compression-hardware-aware-search-spaces-b2e4}
...
[]{#sec-model-compression-nas-in-practice-d4f2}
...
## Quantization and Precision {#sec-model-compression-quantization-precision-optimization-31d3}
...
#| echo: false
from physx.formatting import fmt
from physx.constants import SIMD_REGISTER_BITS, FP32_BITS, INT8_BITS
...
# OUTPUT
# =============================================================================
llm_params_b_str = fmt(llm_params_b_value, precision=0, commas=False)  # e.g. "8"
fp16_bytes_str = fmt(fp16_bytes_value, precision=0, commas=False)      # e.g. "2"
int4_bytes_str = fmt(int4_bytes_value, precision=1, commas=False)      # e.g. "0.5"
fp16_size_gb_str = fmt(fp16_size_gb_value, precision=0, commas=False)  # e.g. "16"
int4_size_gb_str = fmt(int4_size_gb_value, precision=0, commas=False)  # e.g. "4"
compression_ratio_str = fmt(compression_ratio_value, precision=0, commas=False)  # e.g. "4"
```
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: "Napkin Math: Calculating Scale and Zero-Point" callout
# │
# │ Why: Demonstrates the affine quantization formula derivation.
# │      Shows how to map a floating-point range [alpha, beta] to an integer
# │      range [0, 255] (UINT8).
# │
...
##### Granularity {#sec-model-compression-quantization-granularity-7f3a}
...
## Architectural Efficiency {#sec-model-compression-architectural-efficiency-techniques-daa6}
...
[]{#sec-model-compression-hardware-acceleration-sparse-operations-d609}
...
## Implementation Tools {#sec-model-compression-implementation-tools-software-frameworks-e939}
...
[]{#sec-model-compression-optimization-visualization-tools-e1a3}
...
# │
# │ Why: Provides quantitative examples for common optimization misconceptions.
# │      Shows how techniques interact non-linearly (pruning + quantization),
# │      accuracy degradation at extreme bit widths, and speedup gaps.
# │
# │ Imports: physx.formatting (fmt)
...
# --- Inputs (common optimization metrics) ---
int8_size_reduction = 4                  # FP32 to INT8 = 4x size reduction
bert_fp32_mb = 440                       # BERT-base FP32 size in MB
pruning_target_pct = 70                  # target pruning percentage
param_removal_pct = 40                   # aggressive parameter removal
resnet_fp32_acc = 76.15                  # ResNet-50 FP32 ImageNet accuracy
resnet_int8_acc = 76.1                   # ResNet-50 INT8 accuracy
resnet_binary_acc = 51                   # ResNet-50 binary accuracy
dequant_overhead_low = 10                # dequantization overhead low %
dequant_overhead_high = 20               # dequantization overhead high %
...
Teams achieve `{python} int8_size_reduction_str`x model size reduction through INT8 quantization and expect `{python} int8_size_reduction_str`x memory savings in deployment. In practice, runtime overhead erodes compression gains. Dequantization kernels add `{python} dequant_overhead_str`% latency overhead converting INT8 weights back to FP16. Pruned models with irregular sparsity achieve only 12% latency reduction despite `{python} param_removal_str`% parameter removal because hardware cannot skip zeroed weights efficiently. As @sec-model-compression-profiling-opportunity-analysis-ad25 demonstrates, a BERT model pruned to 50% sparsity and quantized to INT8 achieves `{python} actual_speedup_str`% end-to-end speedup rather than the expected `{python} expected_speedup_str`x, because unstructured sparsity creates irregular memory access. Production workflows must profile *deployed* latency on target hardware, not extrapolate from compression ratios.
