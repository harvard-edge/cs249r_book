...
**The Extreme Constraint**: Our **Keyword Spotting (KWS) Lighthouse** (@sec-dnn-architectures) lives here. Running on a microcontroller with 256KB of SRAM means "standard" compression is not enough.
...
We have compressed the model's logic, shaving off every unnecessary bit. Logic, however, must eventually run on physics. We turn next to @sec-ai-acceleration, where we explore how GPUs, TPUs, and NPUs are designed to exploit these optimizations and execute compressed models at maximum throughput.
