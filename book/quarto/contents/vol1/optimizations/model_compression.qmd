...
resnet_str = f"{resnet_mem.to(MiB).magnitude:.0f} MiB"
mobilenet_str = f"{mobilenet_mem.to(MiB).magnitude:.0f} MiB"
mobilenet_int8_str = f"{mobilenet_int8_mem.to(MiB).magnitude:.1f} MiB"
dscnn_str = f"{dscnn_mem.to(KiB).magnitude:.0f} KiB"

cloud_cap = f"~{cloud_mem.to(GiB).magnitude:.0f} GiB"
mobile_cap = f"~{mobile_mem.to(GiB).magnitude:.0f} GiB"
tiny_cap = f"~{tiny_mem.to(KiB).magnitude:.0f} KiB"
```

+------------------------+-------------------------------+-------------------------------+----------------------------+-----------------------------+--------------------------------+
| **Model**              | **Memory**                    | **Storage**                   | **Cloud**                  | **Mobile**                  | **TinyML**                     |
|                        | **(Runtime)**                 | **(Weights)**                 | **(`{python} cloud_cap`)** | **(`{python} mobile_cap`)** | **(`{python} tiny_cap`)**      |
+:=======================+:==============================+:==============================+:===========================+:============================+:===============================+
...
**Reducing the Memory Pressure**: Low-rank factorization illustrates a classic systems trade-off: **trading computation for bandwidth reduction**. Storing a $4096 \times 4096$ matrix requires `{python} full_mb_str`MiB (at FP32). Fetching this matrix for a single inference is a massive memory bandwidth hit, especially when limited by physical memory bandwidth constraints.

If we factorize it with rank $k=128$, we store two matrices ($4096 \times 128$ and $128 \times 4096$), totaling only `{python} factored_mb_str`MiB, a **`{python} data_reduction_str`x reduction in data movement**. While the number of floating-point operations (FLOPs) actually *increases* slightly because we perform two smaller matrix multiplies instead of one large one, the system speedup is often dramatic. By reducing data movement by `{python} data_reduction_str`×, we allow the processor to spend more time computing and less time waiting for memory.
:::
...
|                  |                     |                     | High (QAT)        |                         |                        |
+------------------+---------------------+---------------------+-------------------+-------------------------+------------------------+
| **Distillation** | Reduce Size         | Low-Moderate        | High (student     | Low                     | Creating smaller,      |
