...
[^fn-overparameterization-scale]: **Overparameterization**: @zhang2021 demonstrated that networks large enough to fit ImageNet can also memorize completely random labels --- proving they have vastly more capacity than any real task demands. Concretely, ResNet-50's 25.6M parameters can be pruned by 90 percent with less than two percent accuracy loss, implying that fewer than 3M parameters carry the task-relevant information. This roughly 10$\times$ redundancy ratio is not an edge case but a structural property of gradient-based training, and it is what makes the entire compression discipline viable. \index{Overparameterization!compression rationale}
