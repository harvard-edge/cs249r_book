...
***Model Compression***\index{Model Compression!definition} is the systematic renegotiation of the **Silicon Contract**, transforming research models into efficient deployment artifacts.
...
[^fn-nas-search-cost]: **Neural Architecture Search (NAS)**: Pioneered by Zoph and Le at Google Brain (2017), who used reinforcement learning to *learn* the architecture itself at a staggering cost: 800 GPUs for 28 days (22,400 GPU-hours). This expense catalyzed efficient NAS --- weight-sharing approaches reduced search cost by 1,000$\times$, and hardware-aware variants incorporated measured latency directly into the search objective. The resulting architectures (EfficientNet, MobileNetV3) consistently outperform hand-designed models, demonstrating that the search budget is a one-time systems cost amortized across every deployment. \index{NAS!search cost}
...
Pruning[^fn-pruning-history] directly addresses memory efficiency constraints by eliminating redundant parameters. Because neural networks carry far more weights than any single task demands (as established above), we can remove a significant fraction without substantial performance degradation. The central questions are *what* to prune (individual weights versus entire structures), *how* to decide what is expendable (magnitude, gradients, or activations), and *when* to prune (after training, during training, or even at initialization). As we will explore in @sec-hardware-acceleration, specialized hardware can further exploit the resulting sparse structures.

[^fn-pruning-history]: **Pruning**: The core decision is *how* to identify expendable parameters, with methods ranging from simple magnitude cutoffs to the complex second-derivative analysis of Yann LeCun's "Optimal Brain Damage" (1989). The choice of *when* to prune—after, during, or at initialization—creates a system-level trade-off between implementation complexity and final model efficiency. This became viable only after research confirmed that over 90% of parameters could be removed from large networks without accuracy loss, turning the *what*—individual weights versus entire structures—into a critical optimization target. \index{Pruning!etymology}
...
Where unstructured pruning removes individual weights, structured pruning [@li2017pruning][^fn-structured-pruning-hardware] eliminates entire computational units: neurons, filters, channels, or layers. This approach produces smaller dense models that map directly to modern machine learning accelerators. Because the resulting architecture remains fully dense, structured pruning leads to more efficient inference on general-purpose hardware than unstructured pruning, which requires specialized execution kernels to exploit its sparse weight matrices.

[^fn-structured-pruning-hardware]: **Structured Pruning**: By removing entire computational units like filters or channels, the underlying weight matrices are reduced in size but remain dense and regularly shaped. This structural regularity allows the smaller model to execute on standard hardware using highly-optimized GEMM operations, bypassing the need for the specialized sparse kernels required by unstructured pruning. For example, channel pruning a MobileNetV2 can yield a 3.2x inference speedup on ARM CPUs while retaining 96.5% of the original accuracy. \index{Structured Pruning!hardware compatibility}
...
These trade-offs become concrete when examining real-world deployments. Several high-profile models have successfully integrated pruning to optimize performance. MobileNet, designed for mobile and embedded applications, has been pruned to reduce inference latency while preserving accuracy [@howard2017mobilenets]. Concretely, removing `{python} mobilenet_pruning_pct_str` of near-zero weights in a MobileNet can reduce its size from `{python} mobilenet_original_size_str` to `{python} mobilenet_pruned_size_str` with less than 1% accuracy loss. BERT[^fn-bert-pruning-scale], a widely used transformer model for natural language processing, has undergone structured pruning of attention heads and intermediate layers to create efficient versions such as DistilBERT and TinyBERT, which retain much of the original performance while reducing computational overhead [@sanh2019distilbert].
 In computer vision, EfficientNet[^fn-efficientnet-channel-pruning] has been pruned to remove unnecessary filters, optimizing it for deployment in resource-constrained environments [@tan2019efficientnet].

[^fn-bert-pruning-scale]: **BERT Pruning**: This strategy is successful because BERT's architecture is massively redundant, allowing entire attention heads and layers to be removed with minimal impact. This trade-off is exploited to create efficient models like DistilBERT, where a 40% parameter reduction (from 110M to 67M) results in only a 1.2% drop in GLUE benchmark scores. \index{BERT!pruning redundancy}

[^fn-efficientnet-channel-pruning]: **EfficientNet Pruning**: The model's balanced architecture, a product of Neural Architecture Search, ensures no single dimension is a bottleneck, making it highly resilient to pruning. This allows for aggressively removing entire channels (filters) to reduce computational load for devices like smartphones. For example, pruning 70% of channels in EfficientNet-B0 results in a 2.8x inference speedup on a Pixel 4, with only a minor 1.3 percentage point drop in accuracy. \index{EfficientNet!pruning}
...
[^fn-nas-hardware-aware]: **Hardware-Aware NAS**: Architecture search that optimizes for *measured* target-hardware latency rather than proxy metrics like FLOPs. MnasNet (Google, 2019) incorporated actual latency into the search objective, finding architectures with 1.8$\times$ speedup over MobileNetV2 at higher accuracy. The key systems insight: optimal architectures differ significantly between mobile CPUs, GPUs, and TPUs because each platform has different memory hierarchies and parallelism constraints --- a FLOPs-optimal architecture can be latency-suboptimal on specific hardware. \index{NAS!hardware-aware}
...
[^fn-quantization-shannon]: **Quantization**: The conversion between numerical precisions (e.g., FP32 to INT8) is an application of Claude Shannon's work on representing continuous signals with discrete values. This mapping introduces error—reducing the number of unique values from over 4 billion (FP32) to just 256 (INT8). Neural networks, however, are uniquely robust to this information loss, enabling a >4x reduction in memory footprint for stored weights and activations. \index{Quantization!etymology}
...
[^fn-int8-energy-deployment]: **INT8 Energy Impact**: An instruction's total energy cost is dominated by data movement, as accessing off-chip DRAM is orders of magnitude more energy-intensive than the computation itself. Quantization to lower-precision formats like INT8 directly attacks this overhead by reducing the volume of data that must be stored and moved. This translates directly to system-level power savings; for example, quantizing MobileNetV2 from FP32 to INT8 reduces total energy per inference by 6.6x. \index{Quantization!energy impact}
...
[^fn-awq-salient-weights]: **Activation-aware Weight Quantization (AWQ)**: The core insight is that model performance is disproportionately sensitive to the ~1% of weights paired with the highest-magnitude activations. By selectively protecting only these salient weights, the remaining 99% of parameters can be aggressively quantized to 4-bits, directly attacking the memory bandwidth bottleneck of a model like the GPT-2/Llama Lighthouse. This trade-off enables a nearly 4x reduction in memory traffic (e.g., from 14 GB to 3.5 GB on an A100), delivering a proportional speedup for bandwidth-bound inference. \index{AWQ!bandwidth reduction}
...
[^fn-ste-gradient-trick]: **Straight-Through Estimator (STE)**: A gradient approximation technique that assigns a derivative of one to the rounding function, enabling backpropagation through otherwise non-differentiable operations. This shortcut is essential because the true gradient is zero almost everywhere, which would otherwise obstruct learning by blocking gradient flow to the full-precision weights. This allows the model to be "aware" of quantization during training, producing INT8-compatible models with minimal accuracy loss. \index{STE!quantization-aware training}
...
Activation checkpointing\index{Activation Checkpointing}\index{Memory Optimization!activation checkpointing}[^fn-checkpointing-compression-budget] complements feature reuse by trading computation for memory during training. As established in @sec-model-training, this technique stores only a subset of forward-pass activations and recomputes the rest during backpropagation, reducing peak memory from $\mathcal{O}(A_{\text{total}})$ to $\mathcal{O}\big(\sqrt{A_{\text{total}}}\big)$. In the compression context, checkpointing enables training of larger models within fixed memory budgets, which in turn provides more capacity for subsequent pruning or distillation to exploit.

[^fn-checkpointing-compression-budget]: **Activation Checkpointing**: Enables the compute-for-memory trade-off by recomputing activations during backpropagation instead of storing them all. For compression, this allows a much larger "teacher" model to be trained within a fixed memory budget, creating more capacity for subsequent pruning or distillation to exploit. This trade-off reduces peak activation memory quadratically—from $\mathcal{O}(N)$ to $\mathcal{O}(\sqrt{N})$—at the cost of roughly 30% more training computation. \index{Activation Checkpointing!compression pipeline}
...
By reducing $C_{\text{in}}$ using $1\times 1$ convolutions, SqueezeNet[^fn-squeezenet-fire-modules] reduces the number of parameters, achieving a 50$\times$ reduction in parameter count compared to AlexNet while maintaining similar performance. This method is well-suited for edge devices that have strict memory and storage constraints.

[^fn-squeezenet-fire-modules]: **SqueezeNet**: The "fire module" implements this exact strategy, using a 1x1 convolution "squeeze" layer to aggressively reduce input channels before they are fed to a more expensive 3x3 "expand" layer. This is the direct mechanism for the claimed 50$\times$ parameter reduction over AlexNet, shrinking the model size from 240 MB to under 5 MB. This efficiency proved that high accuracy was possible without a massive parameter budget, enabling complex models to run on memory-constrained edge devices. \index{SqueezeNet!fire modules}
