...
The preceding chapter on data efficiency (@sec-data-efficiency) examined how to optimize the "Source Code" of our ML system. This chapter turns to the challenge of optimizing the **Binary**: transforming the trained model into a streamlined representation that fits the physical constraints of the hardware.

As established in the **Silicon Contract** (@sec-build-principles), every architecture makes assumptions about hardware capability. Model compression is the process of renegotiating that contract to minimize the **Data Movement** and **Compute** terms of the **Iron Law**. While data efficiency optimizes what enters the training pipeline, model compression optimizes what emerges from it.
