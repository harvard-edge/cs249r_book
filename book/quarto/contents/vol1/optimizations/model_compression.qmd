...
A 7-billion parameter language model requires 14 GB just to store its weights in FP16. Your deployment target is a smartphone with 6 GB of RAM shared across the operating system, applications, and your model. *The math does not work.* No amount of clever engineering changes this arithmetic: 14 GB cannot fit in 6 GB. Yet users expect the model to run: responsively, offline, without draining their battery in an hour. The gap between what training produces and what deployment permits is not a minor inconvenience but a five-orders-of-magnitude chasm that defines the field of model compression.
...
The scale of this renegotiation makes model optimization an engineering discipline, not a collection of ad hoc tricks. A 175 billion parameter model consumes over 300 GB in FP16 representation alone, yet a smartphone provides 6 GB of RAM and a microcontroller offers 512 KB. Bridging six orders of magnitude between what training produces and what deployment permits requires systematic methods with predictable trade-offs, not trial and error. Every optimization technique removes something from the model, whether redundant parameters, numerical precision, or architectural complexity, and the engineer must understand exactly what is lost, what is preserved, and how these losses compose when techniques are combined.
...
1.  **Memory Energy ($D$)**: Fetching a 32-bit float from DRAM costs $\approx$ **`{python} energy_dram` pJ**. Fetching an 8-bit integer costs $\approx$ **`{python} energy_dram_per_byte` pJ**.
2.  **Compute Energy ($Ops$)**: A 32-bit FLOP costs $\approx$ **`{python} energy_flop_fp32` pJ**. An 8-bit integer OP costs $\approx$ **`{python} energy_flop_int8` pJ**.
...
1.  **Model Size**: $`{python} params_b_str` \times 10^9 \text{ params} \times `{python} bytes_fp16_str` \text{ bytes} = `{python} fp16_size_str` \text{ GB}$.
...
- **Size**: $`{python} llm_params_b_str` \times 10^9 \times `{python} fp16_bytes_str` \text{ bytes (16-bit)} = `{python} fp16_size_gb_str` \text{ GB}$
- **Hardware Req**: Requires 24GB GPU (e.g., A10G, 3090, 4090).

**INT4 (4-bit Quantization)**

- **Size**: $`{python} llm_params_b_str` \times 10^9 \times `{python} int4_bytes_str` \text{ bytes (4-bit)} = `{python} int4_size_gb_str` \text{ GB}$
