...
\index{Model Compression!deployability gap}
...
\index{ResNet-50!lighthouse model for compression}
\index{MobileNetV2!lighthouse model for compression}
\index{DS-CNN!keyword spotter lighthouse}
...
\index{Structured Pruning|see{Pruning}}
...
Dynamic pruning presents several advantages over its static counterpart. By allowing models to adapt to different workloads, it improves efficiency while maintaining accuracy across a wider range of inputs. Where static pruning risks over-pruning and permanently degrading performance, dynamic pruning provides a mechanism for selectively reactivating parameters when they prove necessary for a particular input. The cost of this flexibility is additional computational overhead, as pruning decisions must be made in real time during training or inference. This complexity makes dynamic pruning harder to integrate into standard machine learning pipelines, requiring the kind of sophisticated production deployment strategies and monitoring frameworks covered in @sec-machine-learning-operations-mlops.
...
\index{DistilBERT!performance metrics}
...
\index{CP Decomposition|see{Tensor Decomposition}}
\index{Tucker Decomposition|see{Tensor Decomposition}}
...
\index{TensorFlow Lite!PTQ support}
\index{ONNX Runtime!PTQ support}
...
\index{Batch Normalization!QAT interaction}
...
\index{Compound Scaling!formula}
...
feat_map_mb_value = conv_channels_value * conv_spatial_value * conv_spatial_value * bytes_per_element_value / 1e6  # SI MB
...
##### Element-wise Operation Fusion {.unnumbered}

\index{Operator Fusion!element-wise}Element-wise operations (addition, multiplication, activation functions) have extremely low arithmetic intensity, making them severely memory-bound. @lst-elementwise-fusion demonstrates how combining these operations reduces memory traffic:
...
##### Matrix Multiply-Add Fusion {.unnumbered}

Linear layers followed by bias addition and activation appear throughout transformers and MLPs. @lst-gemm-fusion illustrates the fusion pattern:
...
\index{BERT!operator fusion benefits}
...
##### Attention Pattern Fusion {.unnumbered}

\index{Operator Fusion!attention (FlashAttention)}Transformer attention mechanisms involve multiple operations that benefit from fusion. @lst-attention-fusion compares unfused attention with FlashAttention's tile-based approach:
...
\index{TensorFlow!Grappler optimizer}
\index{PyTorch!TorchScript fusion}
...
\index{TVM!compiler-level fusion}
\index{XLA!compiler-level fusion}
\index{TensorRT!runtime fusion}
