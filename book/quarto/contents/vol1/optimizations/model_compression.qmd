...
_How do we bridge the gap between research models that achieve state-of-the-art accuracy and production systems that must operate within tight constraints on memory, latency, energy, and cost?_

Consider running a large language model on a device with limited memory. In its native precision format, the model may consume all available memory for weights alone, leaving no room for the operating system, runtime caches, or your application. At typical memory bandwidth, loading those weights for each token becomes prohibitively slow. Reducing numerical precision shrinks the model, reduces memory bandwidth requirements proportionally, and can transform an unusable system into a responsive product. This illustrates the discipline of model optimization: the systematic transformation of trained models to meet deployment constraints while preserving task performance. Pruning removes redundant computations. Quantization reduces numerical precision. Knowledge distillation transfers capability to efficient architectures. Neural architecture search automates the discovery of optimized designs. Each technique addresses a different bottleneck, and the most effective deployments combine them strategically, navigating the fundamental tension between model capability and the physical constraints that determine whether systems can actually run where they need to run.
...
## Framework Application and Navigation {#sec-model-compression-framework-application-navigation-03d4}
...
### Mapping Constraints {#sec-model-compression-mapping-constraints-021d}

Understanding how system constraints map to optimization dimensions provides a framework before examining specific techniques. When facing deployment challenges, this mapping guides practitioners toward the most relevant approaches. Memory bandwidth limitations indicate focus areas in model representation and numerical precision optimizations. Latency bottlenecks suggest model representation and architectural efficiency techniques.
...
This systematic mapping builds on the efficiency principles established in @sec-introduction. Here we focus specifically on model-level optimizations that implement these efficiency principles through concrete techniques. Although each system constraint primarily aligns with one or more optimization dimensions, the relationships are not strictly one-to-one. Many optimization techniques affect multiple constraints simultaneously. Structuring model optimization along these three dimensions allows practitioners to analyze trade-offs more effectively and select optimizations that best align with deployment requirements.

### Technique Selection Guide {#sec-model-compression-technique-selection-guide}

Given the breadth of optimization techniques, practitioners need a systematic approach to technique selection. The following decision framework guides initial technique selection based on primary deployment constraints:
...
The rest of this chapter provides the technical depth needed to execute these strategies effectively. Return to this guide after understanding individual techniques to plan your optimization pipeline.
...
Despite its challenges, dynamic pruning proves useful in edge computing and efficient AI contexts discussed in @sec-introduction, where resource constraints and real-time efficiency requirements vary across different inputs. Practical considerations and trade-offs involved in choosing the right pruning method for a given machine learning system are explored.
...
##### Factorization Efficiency and Challenges {#sec-model-compression-factorization-efficiency-challenges-0a6a}
...
##### Training Mathematics {#sec-model-compression-training-mathematics-f502}
...
##### Tensor Decomposition Applications {#sec-model-compression-tensor-decomposition-applications-a0ac}
...
##### TD Trade-offs and Challenges {#sec-model-compression-td-tradeoffs-challenges-8c9b}
...
##### LRMF vs. TD {#sec-model-compression-lrmf-vs-td-2d74}
...
The techniques explored so far—pruning, distillation, and factorization—all optimize *existing* model architectures. But what if the architecture itself is suboptimal for efficiency? Rather than retrofitting efficiency onto human-designed networks, Neural Architecture Search automates the discovery of architectures that are efficient *by construction*.
...
#### Model Efficiency Encoding {#sec-model-compression-model-efficiency-encoding-b2a6}
...
###### Calibration Methods {#sec-model-compression-calibration-methods-9cce}
...
###### Layerwise Quantization {#sec-model-compression-layerwise-quantization-65f9}
...
###### Groupwise Quantization {#sec-model-compression-groupwise-quantization-b2f7}
...
###### Channelwise Quantization {#sec-model-compression-channelwise-quantization-9b31}
...
###### Sub-channelwise Quantization {#sec-model-compression-subchannelwise-quantization-680c}
...
We have now covered two optimization dimensions: structural optimization (pruning, distillation, NAS) determines *what* to compute, and precision optimization (quantization) determines *how precisely* to compute it. But here is a sobering reality: you can prune 50% of parameters and quantize to INT8, yet see only a 20% latency improvement on actual hardware.

Why? Because theoretical FLOPs and actual execution time are not the same thing. Sparse matrices stored in dense format waste memory bandwidth. Operations that could run in parallel execute sequentially. Simple inputs receive the same computational budget as complex ones. The gap between "optimized on paper" and "optimized in practice" is the domain of our third and final optimization dimension: **architectural efficiency**.

This dimension ensures that your structural and precision optimizations actually translate into real-world speedups by aligning computation patterns with hardware capabilities.
...
##### Training and Optimization Difficulties {#sec-model-compression-training-optimization-difficulties-8a87}
...
##### Overhead and Latency Variability {#sec-model-compression-overhead-latency-variability-8d30}
...
##### Hardware Execution Inefficiencies {#sec-model-compression-hardware-execution-inefficiencies-2415}
...
##### Generalization and Robustness {#sec-model-compression-generalization-robustness-db87}
...
##### Evaluation and Benchmarking {#sec-model-compression-evaluation-benchmarking-4235}
...
##### GPUs and Sparse Operations {#sec-model-compression-gpus-sparse-operations-1826}
...
##### TPUs and Sparse Optimization {#sec-model-compression-tpus-sparse-optimization-1c41}
...
##### FPGAs and Sparse Computations {#sec-model-compression-fpgas-sparse-computations-5933}
...
##### Memory and Energy Optimization {#sec-model-compression-memory-energy-optimization-d8dc}
...
The reduction in the number of computations and memory accesses directly translates into energy savings[^fn-sparse-energy-savings]. Sparse operations require fewer arithmetic operations and fewer memory fetches, leading to a decrease in the energy consumption required for both training and inference. This energy efficiency proves important for applications that run on edge devices, where power constraints are critical, as explored in @sec-introduction.
...
##### Future: Hardware and Sparse Networks {#sec-model-compression-future-hardware-sparse-networks-c0f6}
...
##### Sparsity and Pruning {#sec-model-compression-sparsity-pruning-15e1}
...
Pruning-generated sparse patterns must align with underlying hardware architecture to achieve computational savings [@gale2019state]. Structured pruning proves effective for hardware optimization.

##### Sparsity and Quantization {#sec-model-compression-sparsity-quantization-d451}

Combining sparsity and quantization yields significant reductions in memory usage and computation, but presents unique challenges [@nagel2021white]. Unstructured sparsity exacerbates low-precision weight processing challenges on hardware lacking efficient support for irregular sparse matrices. GPUs and TPUs amplify sparse matrix acceleration when combined with low-precision arithmetic, while CPUs struggle with combined overhead [@zhang2021learning].

##### Sparsity and Model Design {#sec-model-compression-sparsity-model-design-324e}
...
##### Sparsity and Optimization Challenges {#sec-model-compression-sparsity-optimization-challenges-47a0}
...
### AutoML Optimizations {#sec-model-compression-automl-optimizations-dbbf}
...
### Optimization Strategies {#sec-model-compression-optimization-strategies-c725}
...
### AutoML Optimization Challenges {#sec-model-compression-automl-optimization-challenges-be63}
...
The consistency provided by these built-in APIs proves valuable when working across different model architectures. The standardized interfaces ensure that optimization techniques are applied uniformly, reducing the risk of implementation errors or inconsistencies that could arise from custom solutions. This standardization helps maintain reliable and reproducible results across different projects and teams.
...
### Optimization Process Visualization {#sec-model-compression-optimization-process-visualization-c381}
...
#### Visualizing Quantization Effects {#sec-model-compression-visualizing-quantization-effects-3373}
...
![**Convolutional Kernel Weights**: Color mapping exposes patterns within learned convolutional filters, indicating feature detectors for edges, textures, or specific shapes within input images. Analyzing these weight distributions helps practitioners understand what features a neural network prioritizes and diagnose potential issues like dead or saturated filters—important for model calibration and performance optimization. Source: [@alexnet2012].](images/jpeg/color-mapping_9f68f817.jpeg){#fig-color-mapping width=70% fig-alt="Grid of 96 small color images showing AlexNet first-layer convolutional kernels. Patterns include oriented edges, color blobs, and Gabor-like filters learned from ImageNet training."}
...
#### Visualizing Sparsity Patterns {#sec-model-compression-visualizing-sparsity-patterns-bf17}
...
Model optimization bridges theoretical machine learning advances and practical deployment realities, where computational constraints, memory limitations, and energy efficiency requirements demand sophisticated engineering solutions. Combined with the data efficiency techniques from the preceding chapter, these model-centric optimizations complete the efficiency toolkit for machine learning practitioners: data efficiency maximizes learning from available examples, while model compression minimizes resources required for deployment. The core tension between model accuracy and resource efficiency drives a rich ecosystem of optimization techniques that operate across multiple dimensions simultaneously. Rather than simply reducing model size or complexity, modern optimization approaches strategically reorganize model representations, numerical precision, and computational patterns to preserve capabilities while dramatically improving efficiency characteristics.
...
* Model optimization operates across three dimensions: *structural* (what to compute), *precision* (how precisely), and *architectural* (how efficiently). The best results combine techniques across all three—BERT's 16× compression (440MB → 28MB) exemplifies this through pruning + distillation + quantization.
* No single technique is universally optimal. Match your approach to your primary constraint: quantization for memory/bandwidth, structured pruning for latency, distillation for maximum accuracy preservation.

**Practical Guidance:**

* Start with PTQ quantization (INT8 or INT4)—it requires no retraining, delivers 2-4× compression, and works well for most models. Only invest in QAT or distillation if PTQ accuracy is insufficient.
* For LLM deployment, weight-only quantization (INT4 weights, FP16 activations) is the dominant technique because generation is memory-bandwidth bound.
* Structured pruning delivers latency benefits on commodity hardware; unstructured pruning often requires specialized sparse accelerators to realize speedups.
* When combining techniques, follow this order: architecture changes → pruning → distillation → quantization. Each step optimizes what remains from the previous step.

**Systems Insight:**

* Hardware-aware optimization is not optional—a technique that theoretically reduces FLOPs may provide no speedup if it creates irregular memory access patterns or fails to saturate computational units.
* Profile *before* and *after* optimization on your target hardware. Paper metrics (FLOPs, parameter count) often mislead about real-world performance.
:::
...
Model optimization transforms computational requirements, but realizing these gains in practice requires hardware that can exploit the optimizations. An INT8 model delivers no speedup on hardware without INT8 tensor units; a sparse model provides no benefit without sparse matrix support. The next chapter (@sec-ai-acceleration) examines how specialized hardware accelerators—GPUs, TPUs, and custom AI chips—transform optimized models into high-performance deployments. Understanding this interplay between model optimization and hardware capabilities is essential for making informed decisions that maximize performance across the complete system stack.
