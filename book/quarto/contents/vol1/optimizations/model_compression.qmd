...
According to the **Iron Law** ($T = \frac{D_{vol}}{BW} + \frac{O}{R_{peak} \cdot \eta} + L_{lat}$), reducing the bit-width of a weight has a quadratic effect on efficiency:
...
@fig-quantization-free-lunch visualizes this robustness, showing the "Free Lunch" where reducing precision has minimal impact on accuracy—until you hit the "Cliff."
...
Before examining deployment scenarios, we must understand these three fundamental dimensions of model optimization: structural modifications that change the network topology, precision adjustments that alter numerical representation, and architectural innovations that redesign computation patterns. The following sections detail each dimension; here we preview how they combine in practice.
...
The optimization challenge lies in balancing competing constraints[^fn-pareto-frontier], a direct manifestation of the Conservation of Complexity thesis from @sec-part-foundations: you cannot destroy complexity, only move it. Pruning moves complexity from parameters to the hardware's ability to exploit sparse patterns. Quantization moves complexity from numerical precision to calibration effort and potential accuracy loss. Knowledge distillation moves complexity from inference compute to training compute. Aggressive compression risks accuracy degradation that renders models unreliable for production use, while insufficient optimization leaves models too large or slow for target deployment environments. Selecting appropriate techniques requires understanding where complexity should reside for your specific deployment target.
...
This leads to **knowledge distillation**, a fundamentally different approach. Rather than modifying an existing model's weights, distillation trains a new, compact "student" model to mimic the behavior of a larger "teacher" model. The student inherits the teacher's learned knowledge without inheriting its computational overhead.
...
This brings us to the second dimension of our optimization framework: *how precisely* should those computations be performed? A 32-bit floating-point number uses 4 bytes of memory and requires expensive floating-point arithmetic. An 8-bit integer uses 1 byte and enables fast integer math. For many models, this 4x reduction in precision translates directly to 4x reduction in memory bandwidth, and since LLM inference is bandwidth-bound, this means 4x faster token generation. The accuracy cost is often less than 1%.
...
Beyond storage savings, quantization also accelerates computation through hardware parallelism. The following calculation illustrates the impact of *the SIMD multiplier*.
...
Structural methods determine *what* to compute; precision methods determine *how precisely* to compute it. The final optimization dimension addresses how efficiently these computations execute on real hardware, closing the gap between theoretical complexity reduction and actual deployment performance.
...
This fusion pattern[^fn-flashattention-compression] achieves 2-4× speedup on long sequences by dramatically reducing memory bandwidth requirements, as detailed in @sec-ai-training-flash-attention-ioaware-attention-optimization-3da0.
...
Adaptive inference refers to a model's ability to dynamically adjust computational effort during inference based on input complexity. Unlike earlier approaches that rely on predefined exit points or discrete layer skipping, adaptive inference continuously modulates computational depth and resource allocation based on real-time confidence and task complexity [@yang2020resolution]. Instead of committing to a fixed computational path, models dynamically allocate layers, operations, or specialized computations based on intermediate assessments of the input.

Fast Neural Networks (FNNs) exemplify this approach, adjusting the number of active layers based on real-time complexity estimation. If an input is straightforward, only a subset of layers is activated; if early layers produce low-confidence outputs, additional layers refine the prediction [@wu2019fast]. A related approach, dynamic layer scaling, progressively increases computational depth based on uncertainty estimates. This technique is useful for fine-grained classification tasks where some inputs require only coarse-grained processing while others need deeper feature extraction [@wang2021glam].
