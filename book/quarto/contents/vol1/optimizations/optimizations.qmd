...
The optimization process operates through three interconnected dimensions that bridge software algorithms and hardware execution, as illustrated in @fig-3-sections. These dimensions and their relationships provide the conceptual foundation for all techniques explored in this chapter.
...
Efficiency requirements manifest differently across deployment contexts. In large-scale cloud ML settings, optimizing models helps minimize training time, computational cost, and power consumption [@dean2018new]. Edge ML[^fn-edge-ml-definition] requires models to run with limited compute resources, which necessitates optimizations that reduce memory footprint and computational complexity. Mobile ML introduces additional constraints, such as battery life and real-time responsiveness. Tiny ML[^fn-tiny-ml-definition] pushes efficiency to the extreme, requiring models to fit within the memory and processing limits of ultra-low-power devices [@banbury2020benchmarking].

[^fn-edge-ml-definition]: **Edge ML**: Local device inference introduced in @sec-ml-systems. Edge deployments constrain models by local compute, memory, and power budgets, making optimization essential rather than optional.

[^fn-tiny-ml-definition]: **TinyML**: Ultra-low-power ML on microcontroller-class devices, introduced in @sec-ml-systems. TinyML models are typically constrained to kilobytes-to-megabytes of parameters, requiring aggressive optimization techniques.
...
Production systems typically follow established patterns rather than random technique exploration. Quick deployment approaches apply post-training modifications that require minimal code changes. These can yield meaningful compression with modest accuracy impact on some models and tasks [@gholami2021survey; @nagel2021white]. Production-grade optimization often combines multiple techniques sequentially: reducing parameters, recovering accuracy through training refinement, then applying quantization. Realized gains and timelines depend strongly on the model family, hardware target, and quality requirements. Extreme constraint scenarios, such as microcontroller-class deployments, typically require architectural choices from the start. These include automated architecture discovery and low-precision inference, and can demand sustained engineering effort.

Model optimization represents a systems engineering challenge rather than a universal solution. Optimization benefits depend heavily on target hardware and software stack. The same quantization technique can yield very different speedups across processors and runtimes [@jacob2018quantization; @krishnamoorthi2018quantizing]. Accuracy preservation varies by model architecture and task. Some vision models tolerate aggressive optimization more effectively than some language models. Optimization requires iterative measurement rather than single application. System-level bottlenecks may limit benefits when data preprocessing or network I/O dominate latency, rendering model optimization less impactful. System-wide profiling before optimization investment remains essential, as detailed in the Strategy and Implementation section.

**Amdahl's Law and Optimization Limits.** A fundamental principle governs the maximum speedup achievable through any optimization. Amdahl's Law states that the overall speedup from improving one component is limited by the fraction of time that component consumes. If model inference represents only 30% of end-to-end latency, with preprocessing, network I/O, and postprocessing consuming the remaining 70%, then even infinitely fast inference yields at most 1.43x speedup (1/0.70):

$$\text{Speedup}_{\text{max}} = \frac{1}{(1-f) + \frac{f}{s}}$$

where $f$ is the fraction of execution time affected by the optimization and $s$ is the speedup factor for that fraction. For $f = 0.3$ and $s = \infty$, the maximum speedup approaches $1/(1-0.3) = 1.43\times$.

::: {.callout-notebook title="Engineering Calculation: The Optimization Ceiling" collapse="true"}
**Scenario**: You optimize a model kernel to be **10$\times$ faster** ($s=10$).
**Context**: This kernel accounts for **60%** of total end-to-end latency ($f=0.6$). Pre/post-processing consume the other 40%.

**Calculation**
$$ \text{Speedup} = \frac{1}{(1-f) + \frac{f}{s}} = \frac{1}{(1-0.6) + \frac{0.6}{10}} = \frac{1}{0.4 + 0.06} = \frac{1}{0.46} \approx \mathbf{2.17\times} $$

**Insight**: A 10x kernel improvement yields only ~2.2x system speedup. To achieve 5x system speedup, you must optimize the non-model code (increasing $f$) or optimize the other components.
:::

This mathematical limit explains why aggressive model optimization sometimes yields disappointing end-to-end improvements: the optimized component may not dominate total execution time. Effective optimization requires first identifying which components consume the largest time fractions, then targeting those components for improvement. The profiling techniques in @sec-benchmarking-ai provide the measurement foundation for this analysis.
...
The first dimension, model representation optimization, focuses on eliminating redundancy in the structure of machine learning models. Large models often contain excessive parameters[^fn-overparameterization] that contribute little to overall performance but significantly increase memory footprint and computational cost. Optimizing model representation involves techniques that remove unnecessary components while maintaining predictive accuracy. Pruning, knowledge distillation, and automated architecture search methods refine model structures to balance efficiency and accuracy. These optimizations primarily impact how models are designed at an algorithmic level.

### Numerical Precision {#sec-model-optimizations-numerical-precision-a93d}

While representation techniques modify what computations are performed, precision optimization changes how those computations are executed by reducing the numerical fidelity of weights, activations, and arithmetic operations. The second dimension, numerical precision optimization, addresses how numerical values are represented and processed within machine learning models. Quantization techniques map high-precision weights and activations to lower-bit representations, enabling efficient execution on hardware accelerators such as GPUs, TPUs, and specialized AI chips (@sec-ai-acceleration). Mixed-precision training[^fn-mixed-precision] dynamically adjusts precision levels during training to balance efficiency and accuracy.

[^fn-mixed-precision]: **Mixed-Precision Training**: Uses reduced precision (commonly FP16 or BF16) for many operations while keeping select computations in FP32 for numerical stability. On supported hardware, mixed precision often reduces memory footprint by about 2× for affected tensors and can improve throughput by a factor on the order of \(1.5\times\) to \(3\times\), with the exact speedup depending on the model and accelerator.

Numerical precision optimization enables significant computational cost reductions while maintaining acceptable accuracy levels, providing sophisticated model access in resource-constrained environments.
...
[^fn-sparsity-def]: **Sparsity**: Percentage of zero-valued parameters or activations in a model. Sparsity can reduce memory footprint and compute *if* the software stack and hardware can exploit sparse structure; otherwise, “theoretical” sparsity may not translate into speedups. Many models also exhibit activation sparsity (e.g., due to ReLU-like nonlinearities), which can sometimes be leveraged by specialized kernels.

Architectural efficiency involves techniques that exploit sparsity in both model weights and activations. It factorizes large computational components into more efficient forms[^fn-matrix-factorization] and dynamically adjusts computation based on input complexity.

[^fn-matrix-factorization]: **Matrix Factorization**: Approximates a large weight matrix as a product of smaller matrices (a low-rank approximation), reducing parameter count and potentially improving runtime efficiency. The achievable accuracy retention and speedup depend on the chosen rank, model architecture, and target hardware.
...
[^fn-lora]: **LoRA (Low-Rank Adaptation)**: Fine-tuning technique that freezes pretrained weights and adds small trainable low-rank matrices. This can reduce the number of trainable parameters by orders of magnitude relative to full fine-tuning (often \(10^2\) to \(10^4\)× fewer trainable parameters, depending on rank and where adapters are inserted), and can reduce optimizer-state memory and compute, with outcomes depending on the base model and adaptation target.

### Three-Dimensional Optimization Framework {#sec-model-optimizations-threedimensional-optimization-framework-a60e}

The interconnected nature of this three-dimensional framework emerges when examining technique interactions. Pruning primarily addresses model representation but also affects architectural efficiency by reducing inference operations. Quantization focuses on numerical precision but impacts memory footprint and execution efficiency. These interdependencies enable optimal optimization combinations.
...
[^fn-operator-fusion]: **Operator Fusion**: Graph-level optimization that combines multiple operations into fewer kernels, reducing intermediate memory traffic and kernel-launch overhead. Fusion can improve throughput and latency on memory-bound workloads, with improvements often ranging from modest (single-digit percent) to substantial (tens of percent) depending on operator mix and runtime.
...
Each dimension requires detailed examination. We begin with model representation optimization, encompassing techniques that modify neural network structure and parameters to eliminate redundancy while preserving accuracy.
...
[^fn-overparameterization]: **Overparameterization**: Networks with 10-100× more parameters than theoretically necessary. GPT-3's 175B parameters could compress to 1-10B maintaining 95% performance [@dettmers2022llm], yet overparameterization enables faster convergence through smoother loss landscapes (lottery ticket hypothesis) and implicit regularization from SGD. This paradox drives compression research while explaining training dynamics.
...
Pruning enables models to become smaller, faster, and more efficient without requiring architectural redesign. By eliminating redundancy, pruning directly addresses the memory, computation, and scalability constraints of machine learning systems for deployment across diverse hardware platforms.
...
[^fn-tf-model-optimization]: **TensorFlow Model Optimization Toolkit**: Production-ready optimization suite providing quantization (4× size reduction), pruning (90% sparsity), and weight clustering. Integrates with TFLite for seamless mobile deployment. Powers YouTube, Gmail, and Google Photos models across 4B+ devices. Quantization-aware training maintains <1% accuracy loss while enabling INT8 inference.
...
[^fn-tensorrt-optimization]: **TensorRT**: NVIDIA's inference optimizer that applies graph optimizations and hardware-specific tuning to quantized models. Performance characteristics depend on model architecture and hardware, as examined in @sec-ai-acceleration.
...
[^fn-kl-divergence]: **Kullback-Leibler (KL) Divergence**: Information-theoretic measure of distribution difference, introduced by Kullback & Leibler [@kullback1951information]. KL(P||Q) = Σ P(x) log(P(x)/Q(x)). In knowledge distillation, KL divergence of 0.1-2.0 nats indicates good teacher-student alignment; values >3.0 suggest architecture mismatch requiring temperature adjustment or capacity increase.
...
Finally, distillation helps compress large models into smaller, more efficient versions without major performance loss. This compression capability reduces the environmental impact of model deployment while maintaining performance standards. Training a small model from scratch often results in lower accuracy because the model lacks the capacity to learn the complex representations that a larger network can capture. However, by using the knowledge of a well-trained teacher, the student can reach a higher accuracy than it would have on its own, making it a more practical choice for real-world ML deployments, particularly in edge computing, mobile applications, and other resource-constrained environments explored in @sec-efficient-ai.
...
**Reducing the Memory Pressure**: Low-rank factorization illustrates a classic systems trade-off: **trading computation for bandwidth reduction**. Consider a weight matrix of size $4096 \times 4096$. Storing it requires 64MB (at FP32). Fetching this matrix for a single inference is a massive memory bandwidth hit. If we factorize it with rank $k=128$, we now store two matrices ($4096 \times 128$ and $128 \times 4096$), totaling only 4MB—a **16× reduction in bandwidth requirements**. While the number of floating-point operations actually *increases* slightly because we now perform two smaller matrix multiplies instead of one large one, the system speedup can be dramatic because most large model inferences are **memory-bound**. By reducing the data movement by 16×, we allow the processor to stay productive instead of waiting for data.
:::
...
Emerging numeric formats seek to balance the trade-off between efficiency and accuracy. TF32 [@nvidia_tf32], introduced by NVIDIA for Ampere GPUs, modifies FP32 by reducing the mantissa size while maintaining the exponent width, allowing for faster computations with minimal precision loss.

 Similarly, FP8, which is gaining adoption in AI accelerators, provides an even lower-precision floating-point alternative while retaining a structure that aligns well with machine learning workloads [@micikevicius2022fp8]. Alternative formats such as Posit [@posit_2021], Flexpoint [@flexpoint_2017], and BF16ALT [@arm_bf16alt] are also being explored for their potential advantages in numerical stability and hardware adaptability.

The efficiency of numeric encoding is further influenced by how data is stored and accessed in memory. AI accelerators optimize memory hierarchies to maximize the benefits of reduced-precision formats, using specialized hardware such as tensor cores, matrix multiply units (MMUs), and vector processing engines to accelerate lower-precision computations. On these platforms, data alignment, memory tiling, and compression techniques play an important role in ensuring that reduced-precision computations deliver tangible performance gains.
...
As illustrated in @fig-quantization-granularity, the range for Filter 1 is significantly smaller than that for Filter 3, demonstrating the variation in the magnitude of values across different filters. The precision with which the clipping range [$\alpha$, $\beta$] is determined for the weights becomes an important factor in effective quantization. This variability in ranges is a key reason why different quantization strategies, based on granularity, are employed.
...
The computational graph for a quantized convolution layer during QAT contains fake quantization nodes for both weights and activations:
...
Modern deep learning frameworks implement fake quantization as custom operators with forward and backward functions:
...
Scale factor adaptation during QAT training is crucial for convergence. Static scales computed once at initialization may become suboptimal as weights evolve during training. Dynamic scale updates track distribution changes:
...
Batch normalization interactions with QAT require special consideration. Batch normalization layers compute running statistics during training for use at inference. For quantized models, these statistics must reflect the distribution of fake-quantized activations, not full-precision values:
...
**Convolution-BatchNorm-ReLU Fusion**: This ubiquitous pattern appears in nearly every modern CNN architecture. Executing separately requires three memory round-trips:
...
Fused execution combines all three operations into a single kernel:
...
**Element-wise Operation Fusion**: Element-wise operations (addition, multiplication, activation functions) have extremely low arithmetic intensity, making them severely memory-bound. Consider a residual connection with scaling:
...
**Matrix Multiply-Add Fusion (GEMM Fusion)**: Linear layers followed by bias addition and activation appear throughout transformers and MLPs. The unfused pattern:
...
Fused GEMM combines bias addition and activation directly into the matrix multiply kernel:
...
**Attention Pattern Fusion**: Transformer attention mechanisms involve multiple operations that benefit from fusion:
...
FlashAttention represents an advanced fusion pattern that reorganizes attention computation to minimize memory traffic [@dao2022flashattention]:
...
This integration of hardware optimization libraries with machine learning frameworks enables developers to effectively implement the optimization techniques covered in this chapter while ensuring optimal adaptation to target hardware, supporting the deployment strategies detailed in @sec-serving and @sec-ml-operations.
...
These trade-offs enable systematic technique selection (@tbl-optimization-comparison). Pruning works best when sparse computation hardware is available and when reducing floating-point operations is critical. Quantization provides the most versatile approach with broad hardware support, making it ideal for diverse deployment scenarios. Knowledge distillation requires significant computational investment but produces consistently high-quality compressed models, making it valuable when accuracy preservation is paramount.
