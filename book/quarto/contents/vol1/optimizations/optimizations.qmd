...
[^fn-edge-ml-definition]: **Edge ML**: Computing paradigm where ML inference occurs on local devices (smartphones, IoT sensors, autonomous vehicles) rather than cloud servers. Edge deployments can reduce end-to-end latency by avoiding round-trip networking, but they constrain models by local compute, memory, and power budgets.

[^fn-tiny-ml-definition]: **TinyML**: Ultra-low-power ML systems running on microcontroller-class devices. TinyML deployments typically operate under *very tight* power and memory budgets (often milliwatts and kilobytes-to-megabytes), enabling always-on sensing and inference in smart sensors and wearables. A useful scale anchor is that TinyML models are often constrained to \(\approx 10^4\) to \(10^6\) bytes of parameters, not gigabytes.
...
[^fn-tvm-compiler]: **TVM (Tensor Virtual Machine)**: Apache's ML compiler with hardware-agnostic optimizations and auto-tuning. Achieves 1.2-2.8× speedup over vendor libraries on ARM, 1.5-3.2× on mobile GPUs through operator fusion, layout optimization, and automated kernel tuning. Enables deployment across diverse hardware (CPUs, GPUs, FPGAs, accelerators) from a single model definition.
