...
Optimization operates through three interconnected dimensions that bridge software algorithms and hardware execution, as illustrated in @fig-3-sections.
...
Pruning directly addresses the **Memory Wall** constraint (@sec-ai-acceleration-understanding-ai-memory-wall-3ea9) by eliminating redundant parameters. Modern neural networks are heavily overparameterized, often containing far more weights than are strictly necessary to solve a task. This redundancy allows us to remove a significant fraction of weights without substantial performance degradation.
...
where $\|\hat{W}\|_0$ is the **L0-norm** (the count of non-zero parameters). Since minimizing the L0-norm is NP-hard, we use heuristics like **magnitude-based pruning**, illustrated in @lst-pruning_example. By removing weights with small absolute values, we transform a dense weight matrix into a sparse representation (@fig-sparse-matrix).
...
@fig-kd-overview illustrates the distillation workflow. The student model is trained to minimize a combination of two loss functions:
...
The computational graph for a quantized convolution layer during QAT contains fake quantization nodes for both weights and activations. @lst-qat-conv-forward details this structure:
...
Modern deep learning frameworks implement fake quantization as custom operators with forward and backward functions. @lst-fake-quantize-autograd provides a reference implementation:
...
Scale factor adaptation during QAT training is crucial for convergence. Static scales computed once at initialization may become suboptimal as weights evolve during training. @lst-qat-scale-adaptation implements dynamic scale updates that track distribution changes:
...
**Convolution-BatchNorm-ReLU Fusion**: This ubiquitous pattern appears in nearly every modern CNN architecture. Executing separately requires three memory round-trips. @lst-conv-bn-relu-fusion contrasts the unfused and fused approaches:
...
**Element-wise Operation Fusion**: Element-wise operations (addition, multiplication, activation functions) have extremely low arithmetic intensity, making them severely memory-bound. @lst-elementwise-fusion presents a residual connection with scaling:
...
**Matrix Multiply-Add Fusion (GEMM Fusion)**: Linear layers followed by bias addition and activation appear throughout transformers and MLPs. @lst-gemm-fusion compares the unfused and fused patterns:
...
**Attention Pattern Fusion**: Transformer attention mechanisms involve multiple operations that benefit from fusion. @lst-attention-fusion highlights the FlashAttention approach:
...
The fusion process typically involves graph pattern matching. @lst-graph-pattern-matching outlines a basic identification algorithm:
