...
The DR system faces a sharp training challenge: achieve expert-level diagnostic accuracy with finite labeled-data and optimization budgets. **Transfer learning**\index{Transfer Learning!definition}[^fn-transfer-learning-efficiency] addresses this constraint by adapting a model pretrained on a source task to a target task. The ILSVRC2012 training subset contains `{python} ImageNetCorpus.pretrain_images_str` labeled images [@imagenet_website]. Reusing learned representations can improve target-task optimization and generalization relative to random initialization, with the benefit depending on source-target similarity, model capacity, and required performance [@yosinski2014transferable].

[^fn-transfer-learning-efficiency]: **Transfer Learning**\index{Transfer Learning!compute efficiency}: Reuses representations learned from a source task. Depending on source-target similarity, model capacity, and required performance, fine-tuning can improve target-task optimization and generalization relative to random initialization [@yosinski2014transferable]. It does not reduce inference operations for the same architecture.
