...
[^fn-systems-thinking-ml]: **Systems Thinking**: An analytical discipline that models how a system's components interact over time rather than treating them in isolation. ML systems demand this approach because the lifecycle stages described here are coupled through shared state: changing a single feature in the data pipeline can shift model accuracy, alter memory footprint, and invalidate deployment latency guarantees simultaneously. Component-level analysis misses these cross-stage failures entirely, which is why the lifecycle requires integrated orchestration rather than sequential handoffs. \index{Systems Thinking!ML engineering}
...
[^fn-healthcare-ai-deployment-gap]: **Healthcare AI Deployment Gap**: Over 80% of healthcare AI projects with strong laboratory accuracy fail to reach clinical deployment. The gap explains why the DR system's expert-level AUC did not translate directly to clinic adoption: success depends less on model accuracy and more on integration with clinical workflows, data infrastructure in resource-limited settings, and regulatory clearance pathways. For ML systems engineers, this means deployment constraints, not model metrics, are the binding bottleneck. \index{Healthcare AI!deployment gap}
...
[^fn-scaling-ml-overhead]: **ML Scaling Overhead**: The cross-disciplinary collaboration described here is necessary because ML constraint discovery scales non-linearly. Each additional stakeholder (clinicians, administrators, regulators) introduces requirements that interact multiplicatively: an ophthalmologist's sensitivity threshold constrains model architecture, which constrains hardware selection, which constrains per-clinic cost. Without systematic coordination, teams discover these interacting constraints sequentially rather than simultaneously, triggering the exponential cost escalation the Constraint Propagation Principle quantifies. \index{ML Scaling!operational overhead}
...
[^fn-ablation-component-isolation]: **Ablation Studies**: Named for surgical tissue removal, ablation studies systematically disable individual components to isolate their contribution to performance. The rigor matters because a 0.5% accuracy difference can determine whether the DR model meets its clinical sensitivity threshold. Without ablation, a team cannot distinguish a genuine architectural improvement from noise introduced by a different random seed, wasting iteration cycles on phantom gains. \index{Ablation Studies!component isolation}

[^fn-ab-testing-production]: **A/B Testing in ML**: Compares a new model (B) against the production baseline (A) on live, randomly segmented traffic to isolate the model's causal impact from confounding variables. The sample size requirement is the critical systems constraint: reliably detecting the 0.5% sensitivity lift that determines clinical acceptability requires millions of patient interactions, meaning the test duration scales inversely with traffic volume. For a low-throughput DR screening deployment, reaching statistical significance (p < 0.05) can take weeks, directly gating iteration velocity. \index{A/B Testing!ML deployment}
...
[^fn-calibration-clinical-trust]: **Calibration**: A calibrated model's predicted probabilities match observed frequencies: 80% confidence should correspond to 80% correctness. Calibration is distinct from accuracy, and the systems consequence for the DR system is severe: clinicians use confidence scores for triage decisions, so a miscalibrated model that assigns 90% confidence to uncertain cases misdirects clinical workflows more dangerously than a less accurate but well-calibrated alternative. Platt scaling and temperature scaling correct calibration post-training. \index{Calibration!clinical trust}
...
[^fn-ml-debt-entanglement]: **ML Technical Debt**: ML systems accumulate debt faster than traditional software through three mechanisms: *entanglement* (changing one feature affects all others because the model learned joint distributions), *hidden feedback loops* (predictions influence future training data), and *undeclared consumers* (downstream systems depending on outputs without contracts). Since ML code represents less than 5% of a production system, the remaining 95% of configuration, pipelines, and infrastructure is where this debt compounds silently, explaining why late-discovered constraints propagate so expensively. \index{Technical Debt!ML entanglement}
