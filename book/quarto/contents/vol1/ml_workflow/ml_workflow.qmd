...
class DataScientistTime:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    cleaning_str = fmt(cleaning_pct, precision=0, commas=False, suffix="%")
    collection_str = fmt(collection_pct, precision=0, commas=False, suffix="%")
    data_work_str = fmt(data_work_pct, precision=0, commas=False, suffix="%")
    model_focused_str = fmt(model_focused_pct, precision=0, commas=False, suffix="%")
```
...
class IterationTax:
    ...
    weeks_span_str = fmt(weeks_total, precision=0, commas=False, suffix=" weeks")
    ...
    large_acc_str = fmt(large_start_acc, precision=0, commas=False, suffix="%")
    large_gain_str = fmt(large_gain_per_iter, precision=2, commas=False, suffix=" percentage points")
    large_final_str = fmt(large_final_acc, precision=1, commas=False, suffix="%")

    small_time_str = MarkdownStr("1 hour")
    small_acc_str = fmt(small_start_acc, precision=0, commas=False, suffix="%")
    small_gain_str = fmt(small_gain_per_iter, precision=1, commas=False, suffix=" percentage points")
    small_iters_str = fmt(small_effective_iters, precision=0, commas=False)
    small_uncapped_str = fmt(small_uncapped_acc, precision=0, commas=False, suffix="%")
    small_final_str = fmt(small_final_acc, precision=0, commas=False, suffix="%")

    # Logic check helper for text
    small_total_capacity_str = fmt(weeks_total * hours_per_week, precision=0, commas=True)
    acc_ceiling_pct_str = fmt(99, precision=0, commas=False, suffix="%")
```

::: {#nbk-ml-workflow-iteration-tax .callout-notebook title="The iteration tax"}

**Problem**: A diabetic retinopathy (DR) screening system for rural clinics must choose between a large ensemble trained on high-resolution fundus images (training time: `{python} IterationTax.large_time_str`, accuracy: `{python} IterationTax.large_acc_str`) and a lightweight model suitable for edge deployment on clinic hardware (training time: `{python} IterationTax.small_time_str`, accuracy: `{python} IterationTax.small_acc_str`). Which approach yields a better screening system in six months?
...
class MobilenetSpecs:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    mobilenet_size_mb = (Models.Vision.MobileNetV2.parameters * BYTES_FP32).m_as(MB)
    mobilenet_flops_m = Models.Vision.MobileNetV2.inference_flops.m_as(MFLOPs)
...
The binding constraint differs dramatically across workload archetypes, causing each lifecycle stage to optimize different iron law terms. Whether a workload is memory-bound or compute-bound is set by its arithmetic intensity; @sec-appdx-machine-foundations-roofline-model-2529 develops the roofline model that makes this regime boundary precise, so the archetype labels below can be read off the ratio of compute to bytes moved rather than asserted. @Tbl-lighthouse-workflow-comparison shows *how* the same workflow stages manifest for three recurring workload archetypes the book returns to throughout (catalogued in @tbl-lighthouse-examples): a compute-bound vision model (ResNet-50), a memory-bound recommender (DLRM), and an energy-bound keyword spotter:
...
class ConstraintPropagation:
    ...
    stage5_latency_ms = 100
    stage5_memory_mb = 500
    jetson_latency_ms = 50
    jetson_model_mb = 200
    resnet_variant_mb = 200
    ...
    rework_weeks_str = MarkdownStr(
        f"{fmt(rework_weeks_min, precision=0, commas=False)}--"
        f"{fmt(rework_weeks_max, precision=0, commas=False, suffix=' weeks')}"
    )
    sensitivity_target_pct_str = fmt(sensitivity_target_pct, precision=0, commas=False, suffix="%")
    specificity_target_pct_str = fmt(specificity_target_pct, precision=0, commas=False, suffix="%")
    stage5_latency_ms_str = fmt(stage5_latency_ms, precision=0, commas=False, suffix=" ms")
    stage5_memory_mb_str = fmt(stage5_memory_mb, precision=0, commas=False, suffix=" MB")
    jetson_latency_ms_str = fmt(jetson_latency_ms, precision=0, commas=False, suffix=" ms")
    jetson_model_mb_str = fmt(jetson_model_mb, precision=0, commas=False, suffix=" MB")
    resnet_variant_mb_str = fmt(resnet_variant_mb, precision=0, commas=False, suffix=" MB")
```
...
class BandwidthCompute:
    ...
    bw_mb_per_photo_str = fmt(mb_per_photo, precision=0, commas=False, suffix=" MB/photo")
    bw_daily_mb_str = fmt(daily_mb, precision=0, suffix=" MB")
    bw_daily_gb_str = fmt(daily_gb, precision=1, commas=False, suffix=" GB/day")
    bw_upload_mbps_str = fmt(uplink_mbps, precision=0, commas=False, suffix=" Mb/s")
    bw_upload_mbs_str = fmt(uplink_mbs, precision=2, commas=False, suffix=" MB/s")
    bw_upload_sec_str = fmt(upload_seconds, precision=0, suffix=" s")
    bw_upload_hours_str = fmt_int(round(upload_hours), commas=False, suffix=" h")
    bw_clinic_hours_str = fmt(clinic_hours, precision=0, commas=False, suffix=" h")
    bw_bandwidth_pct_str = fmt(saturation_pct, precision=1, commas=False, suffix="%")
    bw_summary_kb_str = fmt(summary_kb, precision=0, commas=False, suffix=" KB/patient")
    bw_reduction_str = fmt(reduction_factor, precision=0, suffix="×")
```
...
1.  **Daily data**: `{python} BandwidthCompute.bw_patients_str` patients $\times$ `{python} BandwidthCompute.bw_photos_str` photos $\times$ `{python} BandwidthCompute.bw_mb_per_photo_str` = `{python} BandwidthCompute.bw_daily_gb_str`.
2.  **Upload time**: The uplink is `{python} BandwidthCompute.bw_upload_mbps_str`, or `{python} BandwidthCompute.bw_upload_mbs_str`. Thus `{python} BandwidthCompute.bw_daily_mb_str` / `{python} BandwidthCompute.bw_upload_mbs_str` = `{python} BandwidthCompute.bw_upload_sec_str` ≈ `{python} BandwidthCompute.bw_upload_hours_str`.
...
**Systems insight**: A Cloud-only architecture is too "expensive" in terms of bandwidth. Moving to the edge requires uploading only *detection summaries* (`{python} BandwidthCompute.bw_summary_kb_str`), reducing bandwidth usage by `{python} BandwidthCompute.bw_reduction_str`.
...
# │
# │ Imports: mlsysim.core.constants (Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate, Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate, USD, GB, ureg)
...
class StorageCosts:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    cost_nvme_gb_mo = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.m_as(USD/GB/ureg.month)
    cost_s3_gb_mo   = Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate.m_as(USD/GB/ureg.month)
...
Optimizing for clinical performance alone is not enough. Edge deployment constraints from the data collection phase impose additional requirements: the model must run efficiently on resource-limited hardware while maintaining inference speeds compatible with clinical workflows. Improvements in one dimension often come at the cost of others: the Operations $(O)$ term, the model byte footprint that contributes to $D_{\text{vol}}$, and fixed serving overhead $(L_{\text{lat}})$ can pull in different directions. @Sec-network-architectures explores model capacity, while @sec-ml-systems discusses deployment feasibility, and the inherent tension between them drives architectural decisions. Systematic application of quantization, pruning, and knowledge distillation[^fn-compression-iterative-workflow] techniques can bridge the gap, meeting deployment requirements while aiming to preserve clinical utility.

[^fn-compression-iterative-workflow]: **Model Compression Pipeline**: Quantization, pruning, and distillation each alter Operations $(O)$, model byte movement $(D_{\text{vol}})$, and sometimes fixed serving overhead $(L_{\text{lat}})$ differently, so bridging the gap between research accuracy and edge deployment requires an iterative "compress-validate-adjust" loop. Finding a model that fits in device memory while preserving clinical sensitivity typically requires 3--5 iterations, because each compression step can silently degrade accuracy below the 90 percent sensitivity threshold in ways invisible until the full validation suite runs. \index{Model Compression!iterative workflow}
...
***Model Validation***\index{Model Validation!definition} is the gate that determines whether a trained model is safe to deploy by testing it against the full constraint surface of the deployment environment: latency SLOs, fairness thresholds, cost budgets, and robustness under distribution shift.

1.  **Significance (quantitative)**: Validation adds dimensions that test-set accuracy ignores. A model achieving 95 percent accuracy on a static test set may violate a 100 ms p99 latency SLO, exceed a fairness constraint of $\varepsilon \leq 5$ percent demographic parity gap, or degrade by 10+ percent under input perturbation $\|\delta\|_\infty \leq 0.01$. Each unchecked dimension is a deployment risk that compounds silently in production.
2.  **Distinction (durable)**: Unlike model evaluation (which measures accuracy on a held-out i.i.d. test set drawn from the same distribution as training), model validation tests the model against the full constraint surface of the deployment environment: latency, throughput, fairness, robustness, and cost.
3.  **Common pitfall**: A frequent misconception is that validation is "one more test." In reality, it is a multi-dimensional gate: a model can pass the accuracy test and still fail validation because it violates latency, fairness, or cost constraints that accuracy alone does not measure.
...
class DeploymentEconomics:
    ...
    image_size_mb = 5.0      # matches retinal scan size in Bandwidth vs. Compute exercise
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(image_size_mb == 5.0, "Deployment image size must match the bandwidth exercise (5 MB/photo).")
    ...
    image_size_mb_str = fmt(image_size_mb, precision=0, commas=False, suffix=" MB")
    edge_inference_cost_str = fmt(edge_inf_cost, precision=3, commas=False)
    cloud_latency_risk_str = fmt(cloud_latency_ms, precision=0, commas=False, suffix=" ms+")
    edge_latency_benefit_str = fmt(edge_latency_ms, precision=0, commas=False, suffix=" ms")
...
- Plus: Network costs for uploading `{python} DeploymentEconomics.image_size_mb_str` = ~USD `{python} DeploymentEconomics.cloud_network_str`/year
- **Total**: ~USD `{python} DeploymentEconomics.cloud_total_str`/year operational cost
- Risk: `{python} DeploymentEconomics.cloud_latency_risk_str` breaks clinical workflow; connectivity outages halt screening
...
*The lifecycle cost crossover between cloud and edge deployment.*
:::
...
class MonitoringThresholds:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sens_target_str = fmt(sens_target, precision=0, commas=False, suffix="%")
    spec_target_str = fmt(spec_target, precision=0, commas=False, suffix="%")
    sens_alert_str = fmt(sens_alert, precision=0, commas=False, suffix="%")
    spec_alert_str = fmt(spec_alert, precision=0, commas=False, suffix="%")
    latency_p95_target_str = fmt(latency_p95_target, precision=0, commas=False, suffix=" ms")
    latency_alert_str = fmt(latency_alert, precision=0, commas=False, suffix=" ms")
```
...
- **Operational metrics**: Inference latency (P95 <`{python} MonitoringThresholds.latency_p95_target_str`, alert if >`{python} MonitoringThresholds.latency_alert_str`), throughput (alert if queue depth >50 images), and error rate (alert if >0.1 percent of requests fail).
...
***The Constraint Propagation Principle***\index{Constraint Propagation Principle!definition} states that a constraint discovered at a late lifecycle stage propagates backward through all earlier stages, and the cost of correction grows roughly exponentially with the number of stages traversed.

1.  **Significance (quantitative)**: A latency SLO of 100 ms discovered at deployment (stage 5) propagates backward to constrain model size (stage 3: algorithm complexity $O$), which constrains dataset requirements (stage 2: data volume $D$), which constrains problem definition (stage 1: what accuracy is achievable). Each backward step multiplies the rework cost because decisions at each stage were made without the constraint that is now imposed. Within the iron law, a deployment constraint on $L_{\text{lat}}$ or $R_{\text{peak}}$ redefines the feasible region for $O$, $D_{\text{vol}}$, and $\eta_{\text{hw}}$ at every earlier stage.
...
*Feedback loops span roughly five orders of magnitude, from minute-level operational fixes to quarterly architectural review.*
:::
...
**Takeaway**: Define demographic and deployment constraints before collecting data.
