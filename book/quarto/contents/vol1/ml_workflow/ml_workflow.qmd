...
Orchestrating machine learning requires managing data evolution in lockstep with model updates. In @fig-ml-lifecycle, the horizontal layout separates the top data pipeline from the bottom model development pipeline, with curved return vectors illustrating how operational insights and data defects cycle back into earlier stages.
...
Practitioner surveys reveal a stark asymmetry between where research attention centers and where engineering hours are spent, with data collection and preparation consuming `{python} DataScientistTime.data_work_str` of practitioner time. In @fig-ds-time, a pie chart breaks down reported primary time sinks, contrasting the dominant data collection and preparation slices against the narrow share occupied by model architecture and algorithm tuning.
...
These differences alter how lifecycle stages interact. While conventional software engineering relies on production feedback to refine code and requirements, ML systems introduce feedback loops where operational data reshapes training distributions, drift triggers automated retraining, and runtime errors expose dataset blind spots. @Tbl-sw-ml-cycles contrasts traditional software and ML engineering across six core lifecycle dimensions, highlighting how data evolution redefines testing, deployment, and ongoing maintenance.[^fn-data-versioning-drift]
...
The complete machine learning lifecycle distills into six core stages laid out across two functional rows. In @fig-lifecycle-overview, trace the top row from problem formulation through model evaluation, and follow the bottom row as deployment feeds monitoring signals back into upstream data collection.
...
::: {#fig-ml-lifecycle-feedback fig-env="figure" fig-pos="htb" fig-cap="**Feedback Paths Across Lifecycle Stages**: Multi-stage feedback pathways demonstrate how downstream discoveries force iterative upstream corrections. Upstream data quality and distribution gaps detected during evaluation trigger targeted collection and augmentation, while operational constraints and performance drift in production propagate backward to force model re-architecture, retraining, or pipeline reconfiguration." fig-alt="Diagram with six boxes: data collection, preparation, training, evaluation, deployment, monitoring. Labeled feedback arrows show data gaps, validation issues, performance insights, and deployment constraints flowing between stages."}
