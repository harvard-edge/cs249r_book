...
Understanding the ML lifecycle\index{ML Lifecycle!time allocation across stages} conceptually is necessary but insufficient for engineering decisions. Quantitative characterization reveals *where* effort and compute actually go in ML projects, exposing which stages bottleneck development and *where* optimization investments yield the highest returns.
...
[^fn-locality-violation-cost]: **Locality of Reference**: Formalized by @denning1968 as the principle governing virtual memory design. The cost of violating locality is quantitative even though the exact constants depend on the hardware generation: cache hits, DRAM accesses, SSD reads, and network reads differ by orders of magnitude. Random shuffling of multi-terabyte datasets during each training epoch can trigger the expensive end of that hierarchy, explaining why ML data loaders use explicit batching, buffering, and prefetching rather than relying only on OS page-cache heuristics. \index{Locality of Reference!ML workload violation}
...
These distinctions translate directly into the structured six-stage framework that organizes how ML projects unfold.
...
Where traditional software follows requirements through implementation to testing, ML systems demand a different organizational structure: one that preserves feedback from deployment back into data, training, and evaluation rather than ending at release. The six-stage framework that follows captures this loop.
...
**Savings**: `{python} ConstraintPropagation.rework_cycles_str` iteration cycles avoided, approximately `{python} ConstraintPropagation.rework_weeks_str` of rework prevented.
...
Each stage catches different failure modes. Offline evaluation catches algorithmic issues, shadow mode catches integration issues, canary deployment catches scaling issues, and A/B testing catches user-facing issues. Teams should plan for this staged validation workflow from the beginning, as retrofitting staged rollout to an already-deployed system proves more difficult than building it into the original deployment architecture.
...
Three patterns define the engineering discipline of the ML lifecycle:

1. **Data work dominates**: Practitioner surveys and production experience both place data collection, cleaning, labeling, validation, and preparation at the top of where engineering time goes. Model development, despite receiving the most attention, is only one part of the lifecycle. Plan accordingly.
