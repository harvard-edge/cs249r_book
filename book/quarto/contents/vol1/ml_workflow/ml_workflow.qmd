...
class IterationTax:
    ...
    large_time_str = fmt_time(large_cycle_time_hours * hour, week, precision=0, commas=False, style="word")
    ...
    small_time_str = fmt_time(small_cycle_time_hours * hour, hour, precision=0, commas=False, style="word")
...
class MobilenetSpecs:
    ...
    mobilenet_flops_str = fmt_qty(mobilenet_flops_q, MFLOPs, precision=0, commas=False)
```
...
from mlsysim.fmt import check, fmt, fmt_count_range, fmt_math, fmt_percent, fmt_qty, fmt_time, fmt_time_range
...
class BandwidthCompute:
    ...
    mb_per_photo_q = 5.0 * MB  # scenario assumption
...
class StorageCosts:
    ...
    cost_ratio_str = fmt_multiple(cost_ratio, precision=1, commas=False)
```
...
class DeploymentEconomics:
    ...
    image_size_q = BandwidthCompute.mb_per_photo_q
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(image_size_q == BandwidthCompute.mb_per_photo_q, "Deployment image size must match the bandwidth exercise.")
    ...
    cloud_network_str = fmt_usd(cloud_network_cost_q, precision=0, per="year")
    edge_cost_per_unit_str = fmt_usd(edge_unit_cost_q, precision=0)
    ...
    edge_latency_benefit_str = fmt_time(edge_latency_q, millisecond, precision=0, commas=False)

    cloud_annual_str = fmt_usd(cloud_annual_inf_q, precision=0, commas=True, per="year")
    cloud_total_str = fmt_usd(cloud_total_year_q, precision=0, commas=True, per="year")
    edge_capex_str = fmt_usd(edge_capex_q, precision=0, commas=True)
    edge_maintenance_str = fmt_usd(edge_maint_cost_q, precision=0, per="year")
    edge_inference_annual_str = fmt_usd(edge_inference_annual_q, precision=0, commas=True, per="year")
    edge_opex_year_str = fmt_usd(edge_opex_year_q, precision=0, commas=True, per="year")
    payback_str = fmt_time(payback_q, year, precision=1, commas=False, style="word", approx=True)
```
...
class MonitoringThresholds:
    ...
    latency_p95_target_str = fmt_time(latency_p95_target_q, millisecond, precision=0, commas=False)
    latency_alert_str = fmt_time(latency_alert_q, millisecond, precision=0, commas=False)
```
