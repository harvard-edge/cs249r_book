...
**Problem**: A production model processes one million images per month. Should you deploy on Cloud (AWS Lambda) or Edge (on-premise server)?
...
- Annual cost: negligible operational, ~USD `{python} DeploymentEconomics.edge_maintenance_str` maintenance
- **Total: USD `{python} DeploymentEconomics.edge_capex_str` upfront + ~USD `{python} DeploymentEconomics.edge_maintenance_str`/year**
- Benefit: <`{python} DeploymentEconomics.edge_latency_benefit_str` ms latency; works offline; no per-inference cost
