...
::: {#fig-eye-dr fig-env="figure" fig-pos="htb" fig-cap="**Retinal Hemorrhages**: Healthy and diseased retinal images show the visual distinction that the DR screening model must learn. The diseased image marks a hemorrhage. Source: Google." fig-alt="Two side-by-side retinal fundus images labeled A. HEALTHY and B. DISEASED. An arrow labeled Hemorrhages points to a small lesion in the diseased retina."}
...
::: {#fig-mlops-returns fig-env="figure" fig-pos="htb" fig-cap="**Illustrative Workflow Automation Scenario**: Assumed curves show a manual workflow rising and then declining as coordination costs grow, while a shared-platform curve continues rising. These functions are illustrative, not empirical estimates." fig-alt="Illustrative line chart comparing assumed experimentation velocity with team size. The manual-workflow curve eventually declines, while the shared-platform curve continues rising."}
