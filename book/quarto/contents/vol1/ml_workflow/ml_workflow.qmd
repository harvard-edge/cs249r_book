...
class BandwidthCompute:
    ...
    bw_reduction_str = fmt_multiple(reduction_factor, precision=0, commas=True)
```
...
**Systems insight**: A Cloud-only architecture is too "expensive" in terms of bandwidth. Moving to the edge requires uploading only *detection summaries* (`{python} BandwidthCompute.bw_summary_kb_str`), reducing bandwidth usage by `{python} BandwidthCompute.bw_reduction_str`$\times$.
...
# │
# │ Imports: mlsysim.core.constants (Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate, Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate, dollar, GB, ureg)
...
class StorageCosts:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    cost_nvme_gb_mo = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.m_as("dollar / GB / month")
    cost_s3_gb_mo   = Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate.m_as("dollar / GB / month")
...
class DeploymentEconomics:
    ...
    cloud_cost_per_image_str = fmt_usd(cloud_inf_cost, precision=2, commas=False, suffix="/image")
    cloud_network_str = fmt_usd(cloud_network_cost, precision=0, suffix="/year")
    edge_cost_per_unit_str = fmt_usd(edge_unit_cost, precision=0)
    image_size_mb_str = fmt(image_size_mb, precision=0, commas=False, suffix=" MB")
    edge_inference_cost_str = fmt_usd(edge_inf_cost, precision=3, commas=False, suffix="/image")
    ...
    cloud_annual_str = fmt_usd(cloud_annual_inf, precision=0, commas=True, suffix="/year")
    cloud_total_str = fmt_usd(cloud_total_year, precision=0, commas=True, suffix="/year")
    edge_capex_str = fmt_usd(edge_capex, precision=0, commas=True)
    edge_maintenance_str = fmt_usd(edge_maint_cost, precision=0, suffix="/year")
    edge_inference_annual_str = fmt_usd(total_images_year * edge_inf_cost, precision=0, commas=True, suffix="/year")
    edge_opex_year_str = fmt_usd(edge_opex_year, precision=0, commas=True, suffix="/year")
...
- Annual cost: `{python} DeploymentEconomics.n_clinics_str` clinics $\times$ `{python} DeploymentEconomics.patients_per_day_str` patients/day $\times$ `{python} DeploymentEconomics.billable_images_per_patient_str` billable image/patient $\times$ `{python} DeploymentEconomics.days_per_year_str` days $\times$ `{python} DeploymentEconomics.cloud_cost_per_image_str` = `{python} DeploymentEconomics.cloud_annual_str`
- Plus: Network costs for uploading `{python} DeploymentEconomics.image_size_mb_str` = ~`{python} DeploymentEconomics.cloud_network_str`
- **Total**: ~`{python} DeploymentEconomics.cloud_total_str` operational cost
...
- One-time hardware: `{python} DeploymentEconomics.n_clinics_str` $\times$ `{python} DeploymentEconomics.edge_cost_per_unit_str` = `{python} DeploymentEconomics.edge_capex_str` capital expense
- Inference cost: ~`{python} DeploymentEconomics.edge_inference_cost_str` (electricity only)
- Annual cost: ~`{python} DeploymentEconomics.edge_maintenance_str` maintenance + ~`{python} DeploymentEconomics.edge_inference_annual_str` inference electricity = ~`{python} DeploymentEconomics.edge_opex_year_str`
