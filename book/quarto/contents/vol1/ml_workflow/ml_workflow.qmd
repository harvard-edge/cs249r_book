...
class IterationTax:
    ...
    large_gain_str = fmt(large_gain_per_iter, precision=2, commas=False)
...
**Systems insight**: If each iteration improves accuracy by `{python} IterationTax.small_gain_str` percent on average, the small model reaches: `{python} IterationTax.small_acc_str` percent + (`{python} IterationTax.small_iters_str` $\times$ `{python} IterationTax.small_gain_str` percent) = 100 percent before applying the 99 percent ceiling, so it renders as `{python} IterationTax.small_final_str` percent. The large model reaches: `{python} IterationTax.large_acc_str` percent + (`{python} IterationTax.weeks_str` $\times$ `{python} IterationTax.large_gain_str` percent) = 98.9 percent, approximately `{python} IterationTax.large_final_str` percent. Even assuming we use only a fraction of the theoretical capacity (for example, 100 effective iterations out of thousands possible), the compound effect dominates. In practice, the small model's rapid iteration enables discovering better architectures, data augmentations, and hyperparameters.
...
The binding constraint differs dramatically across workload archetypes, causing each lifecycle stage to optimize different iron law terms. @tbl-lighthouse-workflow-comparison shows *how* the same workflow stages manifest for three of the five Lighthouse Models defined in @tbl-lighthouse-examples:
...
class BandwidthCompute:
    ...
    bw_daily_gb_str = fmt(daily_gb, precision=1, commas=False)
...
class DeploymentEconomics:
    ...
    monthly_images_str = f"{round(total_images_year / 12 / 1000) * 1000:,.0f}"
    ...
    payback_str = fmt(payback_years, precision=1, commas=False)
```

::: {.callout-notebook title="Cloud vs. edge deployment economics"}

**Problem**: A production model processes about `{python} DeploymentEconomics.monthly_images_str` billable screening images per month across 500 clinics, assuming one processed image per patient after local selection and quality checks. Should the deployment use Cloud inference (AWS Lambda) or Edge inference on an on-premise server?
...
***The Constraint Propagation Principle***\index{Constraint Propagation Principle!definition} states that constraints discovered late in the lifecycle ($N$) incur an exponential cost relative to catching them during Stage 1 problem definition ($\text{Correction Cost} \approx 2^{(N-1)} \times \text{Base Effort}$).

1.  **Significance (quantitative)**: It dictates that system design must proceed end-to-end. Within the iron law, a constraint on $R_{\text{peak}}$ at deployment (Stage 5) propagates backward to redefine the Stage 1 requirements and the downstream data volume $(D_{\text{vol}})$ and algorithm complexity $(O)$ that those requirements permit.
...
* **The lifecycle governs the iron law**: Problem definition sets the constraints, data collection determines $D_{\text{vol}}$, model development defines $O$, deployment minimizes $L_{\text{lat}}$, and monitoring feeds violations back into re-optimization.
