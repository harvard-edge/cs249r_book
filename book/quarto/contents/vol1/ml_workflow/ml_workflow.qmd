...
**Diagnosis**: The output contract lacks deployment paradigm selection and resource constraints (latency and memory targets). Attempting to deploy late in the lifecycle exposes violations (e.g., edge device memory limit <`{python} ConstraintPropagation.jetson_model_mb_str`), incurring `{python} ConstraintPropagation.cost_factor_mult_str` the cost compared to early correction.
