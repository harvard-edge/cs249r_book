...
class DataScientistTime:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    cleaning_str = fmt(cleaning_pct, precision=0, commas=False)
    collection_str = fmt(collection_pct, precision=0, commas=False)
    data_work_str = fmt(data_work_pct, precision=0, commas=False)
    model_focused_str = fmt(model_focused_pct, precision=0, commas=False)
```

Time allocation across stages follows a consistent pattern: data work often dominates the effort. In the 2016 CrowdFlower data-science survey, respondents reported spending `{python} DataScientistTime.cleaning_str` percent of their time cleaning and organizing data and another `{python} DataScientistTime.collection_str` percent collecting datasets [@crowdflower2016data]. Together, these two data-centered activities account for `{python} DataScientistTime.data_work_str` percent of reported practitioner effort in that survey. That survey measures practitioner time use rather than a universal project-level law, but it captures the practical lesson that data preparation can dominate ML engineering effort. Model development and training (the focus of @sec-model-training), despite receiving the most research attention, is only one part of the lifecycle; deployment, integration, and initial monitoring setup add their own engineering burden. This distribution surprises teams accustomed to traditional software where implementation dominates. In ML projects, the "source code" is the data, and preparing that source code is a primary engineering activity. For a concrete breakdown, turn to @fig-ds-time—data cleaning and organizing alone accounts for `{python} DataScientistTime.cleaning_str` percent of reported practitioner effort.

::: {#fig-ds-time fig-env="figure" fig-pos="htb" fig-cap="**Data Scientist Time Allocation**: Data preparation consumes up to `{python} DataScientistTime.cleaning_str` percent of data science effort, with data collection accounting for an additional `{python} DataScientistTime.collection_str` percent. Model-focused activities such as pattern mining, training set construction, and algorithm refinement together represent roughly `{python} DataScientistTime.model_focused_str` percent of total time. Source: CrowdFlower 2016 Data Science Report." fig-alt="Pie chart showing data scientist time allocation: 60 percent cleaning and organizing data, 19 percent collecting datasets, 9 percent mining for patterns, 5 percent building training sets, 4 percent refining algorithms, 3 percent other tasks."}
...
class IterationTax:
    ...
    large_time_str = MarkdownStr("1 week")
    ...
    large_final_str = fmt(large_final_acc, precision=1, commas=False)

    small_time_str = MarkdownStr("1 hour")
    ...
    small_uncapped_str = fmt(small_uncapped_acc, precision=0, commas=False)
...
**Systems insight**: If each iteration improves accuracy by `{python} IterationTax.small_gain_str` percent on average, the small model reaches: `{python} IterationTax.small_acc_str` percent + (`{python} IterationTax.small_iters_str` $\times$ `{python} IterationTax.small_gain_str` percent) = `{python} IterationTax.small_uncapped_str` percent before applying the 99 percent ceiling, so it renders as `{python} IterationTax.small_final_str` percent. The large model reaches: `{python} IterationTax.large_acc_str` percent + (`{python} IterationTax.weeks_str` $\times$ `{python} IterationTax.large_gain_str` percent) = `{python} IterationTax.large_final_str` percent. Even assuming we use only a fraction of the theoretical capacity (for example, 100 effective iterations out of thousands possible), the compound effect dominates. In practice, the small model's rapid iteration enables discovering better architectures, data augmentations, and hyperparameters.
...
class MobilenetSpecs:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(13 <= mobilenet_size_mb <= 15, f"Unexpected MobileNetV2 size: {mobilenet_size_mb:.1f} MB.")
    check(250 <= mobilenet_flops_m <= 350, f"Unexpected MobileNetV2 FLOPs: {mobilenet_flops_m:.1f} MFLOPs.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mobilenet_size_str = fmt(mobilenet_size_mb, precision=0, commas=False)
    mobilenet_flops_str = fmt(mobilenet_flops_m, precision=0, commas=False)
```
...
@fig-lifecycle-overview presents a linear narrative, but experienced practitioners recognize that these stages interconnect closely. Each stage corresponds to specific terms in the performance equation, and this mapping reveals a workflow-level version of the iron law: decisions made during data collection constrain what is achievable during model development, which in turn determines deployment requirements. The stage-by-stage mapping connects the lifecycle to the iron law of ML systems defined in @sec-introduction-iron-law-ml-systems-c32a:
...
class ConstraintPropagation:
    ...
    stage_monitoring = 6                                      # Monitoring stage
    ...
    monitoring_cost_factor = 2 ** (stage_monitoring - stage_definition)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cost_factor_str = fmt(cost_factor, precision=0, commas=False)
    constraint_math = fmt_math(f"2^{{{stage_deployment - stage_definition}}} = {cost_factor}\\times")
    rework_cycles_str = MarkdownStr(f"{avoided_cycles_min}--{avoided_cycles_max}")
    rework_weeks_str = MarkdownStr(f"{rework_weeks_min}--{rework_weeks_max}")
```
...
**Time saved**: `{python} ConstraintPropagation.rework_cycles_str` iteration cycles avoided, approximately `{python} ConstraintPropagation.rework_weeks_str` weeks of rework prevented.
...
# │
# │ Imports: mlsysim.core.constants (MB, GB, KB), mlsysim.fmt (fmt, check)
...
class BandwidthCompute:
    ...
    original_kb = (daily_mb * MB).m_as(KB)
    ...
    check(round(daily_mb) == 7500, f"Daily image volume changed unexpectedly: {daily_mb:.1f} MB.")
    ...
    bw_daily_mb_str = fmt(daily_mb, precision=0)
    ...
    bw_upload_sec_str = fmt(upload_seconds, precision=0)
    ...
    bw_reduction_str = fmt(reduction_factor, precision=0)
```
...
class StorageCosts:
    ...
    cost_ratio = cost_nvme_gb_mo / cost_s3_gb_mo

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(4.0 <= cost_ratio <= 5.0, f"Expected roughly 4x storage gap, got {cost_ratio:.1f}x.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cost_nvme_str = fmt(cost_nvme_gb_mo, precision=2, commas=False)
    cost_s3_str   = fmt(cost_s3_gb_mo, precision=3, commas=False)
    cost_ratio_str = fmt(cost_ratio, precision=1, commas=False)
```
...
\draw[LineA](B4.west)-|node[Txt,above,pos=0.25]{Validation Issues\index{Validation Issues!workflow diagram}}(B1);
...
class HyperparameterGrid:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_hyperparams_str = fmt(n_hyperparams, precision=0, commas=False)
    values_per_param_str = fmt(values_per_param, precision=0, commas=False)
...
\index{Model Validation!callout alias}
...
# │          DeploymentEconomics.edge_inference_annual_str,
...
class DeploymentEconomics:
    ...
    image_size_mb = 5.0      # matches BandwidthCompute.mb_per_photo
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(image_size_mb == BandwidthCompute.mb_per_photo, "Deployment image size must match the bandwidth exercise.")
    check(edge_capex > cloud_total_year, "Edge deployment should have larger upfront cost than one year of cloud OpEx.")
    check(annual_savings > 0, "Edge OpEx must be lower than cloud OpEx for payback.")
    ...
    monthly_images_str = fmt(round(total_images_year / 12 / THOUSAND) * THOUSAND, precision=0)
    cloud_cost_per_image_str = fmt(cloud_inf_cost, precision=2, commas=False)
    cloud_network_str = fmt(cloud_network_cost, precision=0)
    edge_cost_per_unit_str = fmt(edge_unit_cost, precision=0)
    image_size_mb_str = fmt(image_size_mb, precision=0, commas=False)
    edge_inference_cost_str = fmt(edge_inf_cost, precision=3, commas=False)
    ...
    edge_maintenance_str = fmt(edge_maint_cost, precision=0)
...
# │          MonitoringThresholds.latency_p95_target_str,
# │          MonitoringThresholds.latency_alert_str
# └─────────────────────────────────────────────────────────────────────────────
...
class MonitoringThresholds:
    ...
    latency_alert    = latency_p95_target * latency_alert_multiplier
    ...
    check(latency_alert == 100, f"Latency alert threshold should be 100 ms, got {latency_alert}.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sens_target_str = fmt(sens_target, precision=0, commas=False)
    spec_target_str = fmt(spec_target, precision=0, commas=False)
    sens_alert_str = fmt(sens_alert, precision=0, commas=False)
    spec_alert_str = fmt(spec_alert, precision=0, commas=False)
    latency_p95_target_str = fmt(latency_p95_target, precision=0, commas=False)
    latency_alert_str = fmt(latency_alert, precision=0, commas=False)
```
...
- **Model performance metrics** (requiring ground truth, available with delay): sensitivity (target >`{python} MonitoringThresholds.sens_target_str` percent, alert if seven-day rolling average drops below `{python} MonitoringThresholds.sens_alert_str` percent), specificity (target >`{python} MonitoringThresholds.spec_target_str` percent, alert if it drops below `{python} MonitoringThresholds.spec_alert_str` percent), and subgroup performance (alert if any demographic drops > 5 percent below baseline).
...
- **Operational metrics**: Inference latency (P95 <`{python} MonitoringThresholds.latency_p95_target_str` ms, alert if >`{python} MonitoringThresholds.latency_alert_str` ms), throughput (alert if queue depth >50 images), and error rate (alert if >0.1 percent of requests fail).
...
# │ Exports: FallaciesConstraintPropagation.constraint_math, .cost_factor_str
# └─────────────────────────────────────────────────────────────────────────────
