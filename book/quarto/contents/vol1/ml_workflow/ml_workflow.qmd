...
class MobilenetSpecs:
    ...
    mobilenet_size_mb = mobilenet_size_q.m_as(MB)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mobilenet_size_str = fmt_qty(mobilenet_size_q, MB, precision=0, commas=False)
    mobilenet_flops_str = fmt_qty(Models.Vision.MobileNetV2.inference_flops, MFLOPs, precision=0, commas=False)
```
...
from mlsysim.fmt import check, fmt, fmt_math, fmt_percent, fmt_qty, fmt_time, MarkdownStr
...
class ConstraintPropagation:
    ...
    stage5_memory_mb = stage5_memory_q.m_as(MB)
    jetson_latency_ms = 50
    jetson_model_q = 200 * MB
    jetson_model_mb = jetson_model_q.m_as(MB)
    resnet_variant_q = 200 * MB
    resnet_variant_mb = resnet_variant_q.m_as(MB)
    ...
    rework_weeks_str = MarkdownStr(
        ...
        f"{fmt_time(rework_weeks_max, 'week', precision=0, commas=False, style='word')}"
    )
    ...
    stage5_latency_ms_str = fmt_time(stage5_latency_ms, 'millisecond', precision=0, commas=False)
    stage5_memory_mb_str = fmt_qty(stage5_memory_q, MB, precision=0, commas=False)
    jetson_latency_ms_str = fmt_time(jetson_latency_ms, 'millisecond', precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_multiple, fmt_percent, fmt_qty, fmt_time
...
class BandwidthCompute:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # Step 1: Data Volume
    daily_q = patients_day * photos_per_patient * mb_per_photo * MB
    daily_mb = daily_q.m_as(MB)
    daily_gb = daily_q.m_as(GB)

    # Step 2: Transmission Time
    uplink_q = uplink_mbps * ureg.megabit/second
    uplink_mbs_q = uplink_q.to(MB/second)
    uplink_mbs = uplink_mbs_q.m_as(MB/second)
    ...
    bw_mb_per_photo_str = fmt_qty(mb_per_photo * MB, MB, precision=0, commas=False, per="photo")
    ...
    bw_upload_mbps_str = fmt_qty(uplink_q, ureg.megabit/second, precision=0, commas=False, unit_label="Mb/s")
    bw_upload_mbs_str = fmt_qty(uplink_mbs_q, MB/second, precision=2, commas=False)
    bw_upload_sec_str = fmt_time(upload_seconds, 'second', precision=0, commas=True)
    bw_upload_hours_str = fmt_time(round(upload_hours), 'hour', precision=0, commas=False)
    bw_clinic_hours_str = fmt_time(clinic_hours, 'hour', precision=0, commas=False)
    bw_bandwidth_pct_str = fmt_percent(saturation_pct/100, precision=1, commas=False, style='symbol')
    bw_summary_kb_str = fmt_qty(summary_kb * KB, KB, precision=0, commas=False, per="patient")
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_int, fmt_time, fmt_usd, fmt_qty
...
class DeploymentEconomics:
    ...
    image_size_q = 5.0 * MB  # matches retinal scan size in Bandwidth vs. Compute exercise
    image_size_mb = image_size_q.m_as(MB)
    ...
    cloud_cost_per_image_str = fmt_usd(cloud_inf_cost, precision=2, commas=False, per="image")
    cloud_network_str = fmt_usd(cloud_network_cost, precision=0, per="year")
    ...
    edge_inference_cost_str = fmt_usd(edge_inf_cost, precision=3, commas=False, per="image")
    cloud_latency_risk_str = fmt_time(
        cloud_latency_ms, "millisecond", precision=0, commas=False, marker="+"
    )
    edge_latency_benefit_str = fmt_time(edge_latency_ms, 'millisecond', precision=0, commas=False)

    cloud_annual_str = fmt_usd(cloud_annual_inf, precision=0, commas=True, per="year")
    cloud_total_str = fmt_usd(cloud_total_year, precision=0, commas=True, per="year")
    edge_capex_str = fmt_usd(edge_capex, precision=0, commas=True)
    edge_maintenance_str = fmt_usd(edge_maint_cost, precision=0, per="year")
    edge_inference_annual_str = fmt_usd(total_images_year * edge_inf_cost, precision=0, commas=True, per="year")
    edge_opex_year_str = fmt_usd(edge_opex_year, precision=0, commas=True, per="year")
...
class MonitoringThresholds:
    ...
    latency_p95_target_str = fmt_time(latency_p95_target, 'millisecond', precision=0, commas=False)
    latency_alert_str = fmt_time(latency_alert, 'millisecond', precision=0, commas=False)
```
