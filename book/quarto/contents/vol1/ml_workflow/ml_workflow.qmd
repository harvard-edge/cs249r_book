...
To make these stages concrete, consider *how* they apply to MobileNetV2, one of the mobile Lighthouse Models introduced earlier. For MobileNetV2, Problem Definition establishes tight constraints: about `{python} MobilenetSpecs.mobilenet_size_mb_str` model size, about `{python} MobilenetSpecs.mobilenet_mflop_str`, and real-time inference on mobile-class hardware. Data Collection must account for on-device preprocessing limitations. Model Development must choose an architecture that can meet the size and compute budget; @sec-network-architectures explains the architectural mechanisms later. Evaluation validates both accuracy and latency on target devices. Deployment then tests whether the model actually fits the device's memory and power envelope. Monitoring tracks performance across diverse device populations. Each stage's decisions propagate through subsequent stages, and the workflow framework makes these dependencies explicit. A DR screening model optimized for rural clinic deployment faces analogous pressures: limited device memory, strict power budgets, and the need for real-time inference without reliable connectivity. These shared constraints are why we use DR as the chapter's running case study.
...
The binding constraint differs dramatically across workload archetypes, causing each lifecycle stage to optimize different iron law terms. Some workloads spend most of their time on arithmetic, some are limited by how quickly data can be moved, and some are constrained first by device memory or energy. @Tbl-lighthouse-workflow-comparison shows *how* the same workflow stages manifest for three recurring workload archetypes the book returns to throughout: a high-throughput vision workload, an interactive recommendation workload, and an always-on keyword spotter:

| **Stage**    | **High-throughput vision**                                                                  | **Interactive recommendation**                                                                | **Always-on keyword spotting**\index{Keyword Spotting!energy constraint}                |
|:-------------|:--------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------|
| **Data Eng** | Keep image batches ready so the training job waits on computation rather than data loading. | Keep user and item records fresh enough that stale data does not dominate prediction quality. | Curate short audio clips aggressively because storage and memory budgets are tiny.      |
| **Training** | Reduce idle accelerator time by coordinating preprocessing, batching, and model execution.  | Control data movement and irregular lookup patterns that can dominate training cost.          | Search for the smallest model family that still recognizes the trigger phrase reliably. |
| **Deploy**   | Use batching to improve throughput when latency is secondary to cost.                       | Meet strict interactive latency targets while keeping features fresh.                         | Stay within an always-on energy budget so the device does not drain its battery.        |

: **Workflow Variations by Workload**: The same lifecycle stages target different iron law terms depending on the workload's binding constraint. Vision training often emphasizes useful computation per unit time, recommendation workloads often emphasize data movement and freshness, and tiny audio models are strictly bound by energy and memory capacity. {#tbl-lighthouse-workflow-comparison tbl-colwidths="[9,30,31,30]"}
...
As the Bandwidth vs. Compute exercise quantified, this data volume makes cloud-only processing infeasible. The architectural conclusion is edge deployment using a clinic-local accelerator[^fn-edge-clinic-device-tradeoff]. Local preprocessing reduces bandwidth requirements by orders of magnitude but demands correspondingly more local computation, forcing a trade-off: simpler models that run on constrained hardware, or more powerful edge devices that increase per-clinic costs.

[^fn-edge-clinic-device-tradeoff]: **Edge Clinic Device**: A clinic-local device must provide enough compute for image preprocessing and inference while staying within the clinic's power, memory, and cost budgets. This hardware choice imposes the exact trade-off described: the tight memory budget and power envelope constrain model complexity. Developers are forced to reduce model size and computation until the system fits within those limits, making model size a direct function of the per-clinic hardware cost. \index{Edge Deployment!clinic device trade-off}
...
\index{Privacy-Preserving ML!workflow constraints}
Privacy constraints impose a similar architectural decision. Patient privacy regulations often motivate training workflows that keep raw clinic data local while sharing only approved updates or summaries. This approach adds complexity to both data collection workflows and model training infrastructure but often proves necessary for regulatory approval and clinical adoption.
...
1.  **Significance (quantitative)**: Validation adds dimensions that test-set accuracy ignores. A model achieving 95 percent accuracy on a static test set may miss the clinic's latency target, underperform for a patient subgroup, or degrade under common image-quality problems such as blur and glare. Each unchecked dimension is a deployment risk that compounds silently in production.
...
A model that passes every validation test in the lab still faces its hardest exam when it meets the real world. Consider the DR system: a validated model must now run on tablets in rural clinics with intermittent connectivity, integrate with hospital information systems it was never tested against, and produce results that clinicians trust enough to act on, all within latency budgets that leave no room for cloud round-trips. Deployment is where the abstract constraints specified during problem definition become concrete engineering requirements. In iron law terms, this stage focuses on minimizing the Overhead $(L_{\text{lat}})$ term through efficient serving infrastructure, and the binding constraint varies by archetype: high-throughput vision workloads use batching to improve throughput, interactive recommendation workloads enforce strict latency targets, and tiny audio workloads operate under severe energy budgets (@tbl-lighthouse-workflow-comparison). @Sec-ml-operations covers the operational aspects of deployment and maintenance in depth.
...
**Option B: Edge Deployment (Clinic Device)**\index{Edge Deployment!bandwidth constraints}\index{Edge Deployment!economics}
...
A DR screening system, where missed diagnoses cause blindness, demands real-time monitoring, not periodic offline evaluations. Teams establish quantitative performance thresholds for latency, accuracy, and data distribution stability. As detailed in @sec-ml-operations, lightweight statistical drift tests[^fn-statistical-drift-tests] trigger automated responses ranging from on-call alerts to retraining workflows.

[^fn-statistical-drift-tests]: **Statistical Drift Tests**: Lightweight monitoring checks compare recent input or prediction distributions against a trusted baseline. They are computationally cheap enough for real-time monitoring, which is the critical systems property: detecting drift days or weeks before it degrades model accuracy allows proactive retraining rather than reactive incident response. @sec-ml-operations covers named drift detection pipelines in depth. \index{Data Drift!statistical tests}
