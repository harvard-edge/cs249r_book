...
*   **Stage 5 Discovery**: Discovering the same constraint during Deployment means the model you built is too big, the data you collected is too high-res, and your evaluation metrics were wrong. You must redo Stages 2, 3, and 4. Concretely, a constraint mismatch discovered at Stage 5 triggers a `{python} cost_factor_str` $\times$ cost multiplier compared to catching it at the definition stage. (Cost: `{python} cost_factor_str` $\times$)
