...
**The Engineering Conclusion**: A Cloud-only architecture is too "expensive" in terms of bandwidth. Moving to the edge requires uploading only *detection summaries* (~`{python} bw_summary_kb_str` KB/patient), reducing bandwidth usage by `{python} bw_reduction_str`$\times$.
:::
