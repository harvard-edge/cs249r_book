...
class DeploymentEconomics:
    ...
    monthly_images_str = f"{round(total_images_year / 12 / THOUSAND) * THOUSAND:,.0f}"  # round to nearest thousand
...
**Problem**: A production model processes about `{python} DeploymentEconomics.monthly_images_str` billable screening images per month across 500 clinics, assuming one processed image per patient after local selection and quality checks. Should the deployment use Cloud inference (AWS Lambda) or Edge inference on an on-premises server?
...
2.  **Distinction (durable)**: Unlike modular decomposition, which encourages isolation, this principle mandates global optimization: a "local maximum" in model accuracy may lead to a "global minimum" in system feasibility.
