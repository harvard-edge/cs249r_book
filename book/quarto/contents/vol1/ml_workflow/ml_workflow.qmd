...
Storage tiers are another place where the data pipeline either preserves or erodes downstream iteration velocity. Different data access patterns demand different storage solutions, so teams typically implement tiered storage architectures\index{Tiered Storage!hot warm cold architecture}[^fn-tiered-storage-ml]:
