...
class MobilenetSpecs:
    ...
    mobilenet_size_mb = mobilenet_size_q.to(MB).magnitude
    mobilenet_flops_m = Models.Vision.MobileNetV2.inference_flops.to(MFLOPs).magnitude
...
from mlsysim.fmt import check, fmt, fmt_math, fmt_percent, fmt_qty, fmt_time, fmt_time_range, MarkdownStr
...
from mlsysim.core.units import MB, GB, KB, megabit, second
...
class BandwidthCompute:
    ...
    daily_mb = daily_q.to(MB).magnitude
    daily_gb = daily_q.to(GB).magnitude

    # Step 2: Transmission Time
    uplink_q = uplink_mbps * megabit / second
    uplink_mbs_q = uplink_q.to(MB / second)
    uplink_mbs = uplink_mbs_q.to(MB / second).magnitude
    upload_seconds = (daily_q / uplink_mbs_q).to(second).magnitude
    ...
    original_kb = daily_q.to(KB).magnitude
    ...
    bw_upload_mbps_str = fmt_qty(uplink_q, megabit / second, precision=0, commas=False, unit_label="Mb/s")
...
from mlsysim.core.units import USD, GB, ureg
...
class StorageCosts:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    cost_nvme_gb_mo = nvme_rate.to(USD / GB / ureg.month).magnitude
    cost_s3_gb_mo = s3_rate.to(USD / GB / ureg.month).magnitude
...
class DeploymentEconomics:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(image_size_q.to(MB).magnitude == 5.0, "Deployment image size must match the bandwidth exercise (5 MB/photo).")
