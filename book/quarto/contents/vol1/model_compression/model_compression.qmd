...
On a V100 GPU with `{python} V100Specs.v100_bw_gb_per_s_str` HBM bandwidth and `{python} V100Specs.v100_fp32_tflop_per_s_str` FP32 compute, the unfused sequence takes approximately `{python} FusionCalc.unfused_time_us_str` (memory bound), while the fused version takes approximately `{python} FusionCalc.fused_time_us_str` (`{python} FusionCalc.fusion_speedup_mult_str` speedup). The speedup comes entirely from reducing memory traffic, as the compute remains identical. Fusion effectiveness varies by workload: memory-bound operations benefit most, while compute-bound operations see minimal improvement.
