...
The toolchain boundary is therefore clear: framework APIs create and calibrate the compressed model, while hardware optimization libraries adapt that model to the execution substrate. The benchmarking and MLOps chapters later show how to validate and operate that exported artifact; within this chapter, visualization\index{Visualization!quantization error histograms}\index{Visualization!activation visualizations}\index{Visualization!sparsity heat maps} tools answer whether the optimization damaged the model, rather than only whether it shrank.
