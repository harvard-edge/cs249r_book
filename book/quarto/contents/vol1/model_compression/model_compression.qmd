...
**Low-Rank Matrix Factorization (LRMF)**\index{Low-Rank Factorization!definition}\index{Model Compression!low-rank factorization} approximates weight matrices with lower-rank representations. Given a matrix $\mathbf{A} \in \mathbb{R}^{m \times n}$, LRMF finds matrices $\mathbf{U} \in \mathbb{R}^{m \times k}$ and $\mathbf{V} \in \mathbb{R}^{n \times k}$ such that:
$$
...
To see why this matters, study @fig-matrix-factorization: the matrix $\mathbf{M}$ can be approximated by the product of matrices $\mathbf{U}_k$ and $\mathbf{V}_k^T$ (as in Low-Rank Adaptation). For intuition, most fully connected layers in networks are stored as a projection matrix $\mathbf{M}$, which requires $m{\times}n$ parameters to be loaded during computation. However, by decomposing and approximating it as the product of two lower-rank matrices, we only need to store $m \times k + k \times n$ parameters in terms of storage. Applying the factors directly costs $\mathcal{O}(k(m+n))$ for a vector input, instead of $\mathcal{O}(mn)$ for the original dense matrix; explicitly forming the dense product $\mathbf{U}\mathbf{V}$ would cost $\mathcal{O}(mkn)$ and should be avoided in inference [@Denton2014].
