...
class QuantizationEnergyPhysics:
    ...
    energy_int8_fetch_pj_str = fmt_qty(energy_int8_fetch, pJ, precision=0, commas=False)
...
1.  **Memory movement $(D_{\text{vol}} \times E_{\text{move}})$**: Fetching a 32-bit float from DRAM costs ≈ `{python} QuantizationEnergyPhysics.energy_dram_pj_str`. Fetching one 8-bit weight costs ≈ `{python} QuantizationEnergyPhysics.energy_int8_fetch_pj_str`.
2.  **Compute work $(O \times E_{\text{compute}})$**: A 32-bit multiply-add costs ≈ `{python} QuantizationEnergyPhysics.energy_flop_fp32_pj_str`. An INT8 multiply-add costs ≈ `{python} QuantizationEnergyPhysics.energy_op_int8_pj_str`.
...
class ResNet50Int8Metrics:
    ...
    latency_speedup_mult_str = fmt_multiple(latency_speedup, precision=2, commas=False)
...
| **V100 latency**      | `{python} ResNet50Int8Metrics.fp32_latency_ms_str`        | `{python} ResNet50Int8Metrics.int8_latency_ms_str`                                                            | GPU latency improves when the runtime maps INT8 to fast kernels.      |
