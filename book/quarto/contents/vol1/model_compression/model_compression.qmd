...
::: {#fig-structured-unstructured fig-env="figure" fig-pos="htb" fig-cap="**Unstructured vs. Structured Pruning**: Unstructured pruning (left) removes individual connections, while structured pruning (middle and right) removes entire neurons or convolutional kernels. Source: [@qi2021]." fig-alt="Three-panel diagram. Left shows unstructured pruning with dashed connections in a neural network. Middle and right show structured pruning: fully connected network with pruned neurons and CNN with pruned filters shown as dashed squares."}
...
::: {#fig-winning-ticket fig-env="figure" fig-pos="htb" fig-cap="**Lottery Ticket Iteration Cycle**: A dense network is trained to convergence, the smallest-magnitude weights are pruned, and the surviving weights are reset to their original initialization. Repeating this cycle progressively identifies a sparse subnetwork labeled as the winning ticket." fig-alt="Cyclic flowchart with four stages: dense network, train to convergence, prune smallest weights, reset remaining weights to initial values. Arrows form iterative loop that progressively identifies winning ticket subnetwork."}
...
To see why this matters, study @fig-matrix-factorization. The matrix $\mathbf{M}$ can be approximated by the product of matrices $\mathbf{U}_k$ and $\mathbf{V}_k^T$. For intuition, most fully connected layers in networks are stored as a projection matrix $\mathbf{M}$, which requires $m{\times}n$ parameters to be loaded during computation. By decomposing and approximating it as the product of two lower-rank matrices, we need to store only $m \times k + k \times n$ parameters. Applying the factors directly costs $\mathcal{O}(k(m+n))$ for a vector input instead of $\mathcal{O}(mn)$ for the original dense matrix; explicitly forming the dense product $\mathbf{U}\mathbf{V}$ would cost $\mathcal{O}(mkn)$ and should be avoided in inference [@Denton2014].
...
::: {#fig-tensor-decomposition fig-env="figure" fig-pos="htb" fig-cap="**Tensor Decomposition**: A 3D tensor with dimensions $M{\times}N{\times}T$ is decomposed into a sum of rank-one components, each formed by the outer product of three factor vectors (U, V, W). This extends low-rank matrix factorization to multi-dimensional data, reducing storage and computation for convolutional layers. Source: [@gholami2022]." fig-alt="A three-dimensional tensor labeled y with dimensions M by N by T beside factor matrices U, X, and V; highlighted vectors u i, x i, and v i correspond to the highlighted tensor entry y i j t."}
...
::: {#fig-quantization-free-lunch fig-env="figure" fig-pos="htb" fig-cap="**Illustrative Quantization Regimes**: Two illustrative accuracy curves remain nearly flat from FP32 through INT8, then decline at lower bit widths; green shading marks the low-loss region and red shading marks the cliff." fig-alt="Line chart of accuracy vs. precision (bits). Blue line (CNN) stays flat to INT8 then drops at INT4 and INT3. Red line (transformer) holds higher and drops later, at INT3. Green shaded area marks the Free Lunch Zone of minimal loss."}
...
::: {#fig-ptq-calibration fig-env="figure" fig-pos="htb" fig-cap="**Post-Training Quantization**: Calibration data and a pretrained model feed a calibration stage, whose ranges feed the quantization stage to produce a quantized model." fig-alt="Vertical flowchart with four boxes connected by arrows. Pretrained model and calibration data feed into calibration step, which feeds into quantization step, producing final quantized model output."}
...
::: {#fig-calibration-ranges fig-env="figure" fig-pos="htb" fig-cap="**Calibration Range Selection**: The left mapping uses the symmetric real range from -1 to 1 and integer range from -127 to 127 with zero aligned; the right uses the asymmetric real range from -0.5 to 1.5 with a shifted zero point." fig-alt="Two side-by-side mapping diagrams. Left shows symmetric calibration with range from – 1 to 1 mapping to – 127 to 127 with zero aligned. Right shows asymmetric calibration with range – 0.5 to 1.5 mapping with shifted zero point."}
...
::: {#fig-switch-transformer fig-env="figure" fig-pos="htb" fig-cap="**Conditional Computation**: The left panel locates the Switching FFN Layer within a transformer block. The right panel shows routers assigning each token to one of four feedforward experts before the outputs rejoin the transformer path. Source: [@fedus2022switch]." fig-alt="Two-part diagram. Left shows switch transformer block with self-attention, add-normalize, switching FFN layer, and add-normalize. Right shows expanded view with router selecting one of four FFN experts per token based on probability."}
...
Quantization error histograms\index{Visualization!quantization error histograms} reveal whether quantization errors are Gaussian or contain problematic outliers. Activation visualizations\index{Visualization!activation visualizations} help detect overflow and saturation issues. @Fig-color-mapping schematically groups first-layer convolutional kernels by visual pattern. Near-zero kernels or filters with consistently negligible activations—not merely uniform-looking filters—are pruning candidates, so the diagnostic must combine weight visualization with measured activation behavior. TensorFlow's Quantization Debugger, PyTorch's FX Graph Mode, and TensorRT Inspector provide these capabilities.

::: {#fig-color-mapping fig-env="figure" fig-pos="htb" fig-cap="**Convolutional Kernel Weights**: Variation across the first four rows contrasts with the nearly uniform final row, but this contrast is a visual diagnostic cue rather than proof that those filters are inactive." fig-alt="Grid of 40 stylized first-layer convolutional filters (8 columns by 5 rows), with rows labeled oriented edges, color blobs, frequency patterns, mixed features (center-surround), and dead filters; the last row appears nearly uniform."}
...
::: {#fig-sparse-heat-map fig-env="figure" fig-pos="htb" fig-cap="**Sparsity Distribution**: This schematic heat map uses darker shades for blocks with more removed weights, illustrating how pruning can affect layers nonuniformly." fig-alt="Heatmap visualization of a pruned neural network with weight matrix blocks. Darker regions indicate higher sparsity where more weights have been removed. Lighter regions show retained weights."}
