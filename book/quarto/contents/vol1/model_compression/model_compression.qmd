...
A higher $T_{\text{distill}}$ (typically 3 to 5) flattens logit variance before applying softmax, transforming hard step-like probabilities into smooth distributions that expose **dark knowledge**\index{Dark Knowledge!class correlations}—the subtle inter-class similarity correlations (e.g. why an image of a cat resembles a dog far more than a truck). The total loss\index{Knowledge Distillation!loss function} $\mathcal{L}_{\text{distill}}$ balances standard cross-entropy\index{Cross-Entropy Loss!student loss} with the KL divergence:
$$
