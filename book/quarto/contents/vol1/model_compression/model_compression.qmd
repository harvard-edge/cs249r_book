...
::: {#fig-structured-unstructured fig-env="figure" fig-pos="htb" fig-cap="**Unstructured vs. Structured Pruning**: The regularity-compression trade-off across pruning granularities. Unstructured pruning (left) removes arbitrary individual connections, yielding high parameter compression but irregular memory access; structured pruning of neurons (middle) or convolutional channels (right) produces smaller, dense tensors that accelerate execution on standard hardware without sparse kernel support. Source: [@qi2021]." fig-alt="Three-panel diagram. Left shows unstructured pruning with dashed connections in a neural network. Middle and right show structured pruning: fully connected network with pruned neurons and CNN with pruned filters shown as dashed squares."}
...
::: {#fig-winning-ticket fig-env="figure" fig-pos="htb" fig-cap="**Lottery Ticket Iteration Cycle**: Iterative magnitude pruning isolates sparse subnetworks that can match dense accuracy when trained in isolation. The critical mechanism is resetting surviving weights to their initial state $\boldsymbol{\theta}_0$ rather than re-randomizing them, demonstrating that initialization lottery tickets are embedded within the original unpruned network." fig-alt="Cyclic flowchart with four stages: dense network, train to convergence, prune smallest weights, reset remaining weights to initial values. Arrows form iterative loop that progressively identifies winning ticket subnetwork."}
...
::: {#fig-quantization-impact fig-env="figure" fig-pos="htb" fig-cap="**Illustrative Quantization Scenario**: Quantizing from FP32 to INT8 yields compound deployment benefits across diverse vision architectures. Halving bit width fourfold reduces memory footprint (less DRAM/SRAM traffic) and lowers inference latency by packing more operands into hardware SIMD registers and integer execution units." fig-alt="Two stacked bar charts comparing FP32 and INT8. Left chart shows inference time in milliseconds for Inception_v3, MobileNet_v1, and ResNet_v2; right chart shows model size in megabytes. Orange segments mark FP32 values, blue segments mark INT8 values; INT8 is smaller and faster across all models."}
...
Post-training quantization relies on representative calibration data to determine optimal tensor clipping boundaries. Follow the step-by-step pipeline in @fig-ptq-calibration and the layer-by-layer calibration procedure in @algo-ptq-calibration, observing how activation distribution observers feed fixed scale metadata into quantized runtime execution.
...
Sharing a single quantization scale across an entire layer introduces severe clipping noise when channel weight distributions vary. Observe the filter range variations across the rows of @fig-quantization-granularity, contrasting shared layerwise clipping limits against independent per-channel scales.
...
Sparse Mixture-of-Experts architectures replace monolithic feedforward networks with dynamically routed sub-networks. Compare the transformer block structure on the left of @fig-switch-transformer with the expanded gating router on the right, observing how each input token activates only a single expert FFN block.
...
::: {#fig-color-mapping fig-env="figure" fig-pos="htb" fig-cap="**First-Layer Convolutional Filter Patterns**: Visual inspection of initial $8\times5$ convolutional kernels categorizes feature extractors into Gabor-like edge detectors, chromatic blobs, frequency textures, and uniform 'dead' filters. Filters showing uniform low variance represent inactive parameters that contribute negligible information to downstream layers and are prime candidates for channel pruning." fig-alt="Grid of 40 stylized first-layer convolutional filters (8 columns by 5 rows), with rows labeled oriented edges, color blobs, frequency patterns, mixed features (center-surround), and dead filters; the last row appears nearly uniform."}
