...
2.  **Distinction**: Unlike post-training compression methods such as pruning and quantization, neural architecture search (NAS) discovers efficient architectures from scratch by exploring a design space. Here, NAS is treated as a related structural optimization technique: it changes the representation before training rather than compressing a finished model post hoc.
...
[^fn-nas-search-cost]: **Neural Architecture Search (NAS)**\index{NAS!search cost}: @zoph2017neural at Google Brain used reinforcement learning to learn the architecture itself at a cost of `{python} NASCostCalc.gpu_days_str` (`{python} NASCostCalc.gpus_str` for `{python} NASCostCalc.days_str`), equivalent to `{python} NASCostCalc.gpu_hours_str`. Weight-sharing approaches such as Efficient Neural Architecture Search later reduced search cost by roughly `{python} NASCostCalc.weight_sharing_reduction_mult_str` by sharing parameters across candidate architectures [@pham2018efficient]. Hardware-aware NAS and scaling methods then made the search output practical for deployable architecture families such as EfficientNet and MobileNetV3 [@tan2019efficientnet; @howard2019].
...
| **Common Methods**            | Singular Value Decomposition (SVD), Alternating Least Squares          | CP Decomposition, Tucker Decomposition, Tensor-Train                                                         |
...
\pgfplotsset{
  mybarstyle/.style={
   ...
   /pgf/number format/assume math mode=true,
    font=\fontsize{6pt}{6}\selectfont\sffamily, anchor=center},
    %
...
\begin{axis}[mybarstyle,
   ...
   /pgf/number format/assume math mode=true,
    font=\fontsize{6pt}{6}\selectfont\sffamily, anchor=center}] coordinates {
        ({Inception\_v3},500)
        ({MobileNet\_v1},30)
        ({ResNet\_v2},70)};
\end{axis}
...
\begin{axis}[
  ...
  ylabel={\bfseries\sffamily $p(x)\times$ Error},
...
% ------------------------------------------------------------
\node[align=left,text=GreenD, font=\sffamily\bfseries,
...
font=\sffamily\bfseries]{Outlier tail\\
...
**Ternarization**\index{Ternarization!definition} extends binarization by allowing three values (-1, 0, +1), providing additional representational flexibility [@Zhu2017]. The zero value also enables sparsity. Both techniques require gradient approximation methods like the straight-through estimator to handle nondifferentiable quantization operations during training [@bengio2013estimating], with QAT integration helping mitigate accuracy loss [@Choi2019].
...
```{.tikz}
\scalebox{0.57}{\begin{tikzpicture}[line join=round,font=\sffamily]
\tikzset{%
helvetica/.style={align=flush center,font=\small\sffamily},
Line/.style={line width=1.0pt,black!50,text=black},
}
