...
::: {#fig-tensor-decomposition fig-env="figure" fig-pos="htb" fig-cap="**Tensor Decomposition**: The three-dimensional tensor is approximated by the factor matrices U, X, and V, and the highlighted vectors show how a single tensor entry is rebuilt from one slice of each factor. Extending low-rank approximation from two dimensions to three is what carries the storage and computation argument into convolutional layers. Source: [@gholami2022]." fig-alt="A three-dimensional tensor labeled y with dimensions M by N by T beside factor matrices U, X, and V; highlighted vectors u i, x i, and v i correspond to the highlighted tensor entry y i j t."}
...
::: {#fig-quantization-roadmap fig-env="figure" fig-pos="htb" fig-cap="**Quantization Complexity Roadmap**: Three progressive tiers of quantization techniques, from foundational approaches suitable for quick deployment to research frontier methods for extreme resource constraints, Effort rises monotonically across the tiers but accuracy loss does not: the production tier's retraining buys back what the foundation tier gives away, 0.2 to 1 percent against 0.5 to 2 percent, before the research frontier spends 2 to 10 percent on extreme bit widths." fig-alt="Tiered diagram with three levels. Foundation tier includes PTQ and basic INT8. Production tier adds QAT and mixed precision. Research frontier tier shows INT4, binary, and ternary quantization with icons for increasing complexity."}
...
::: {.column-margin}
![](images/svg/vol1_model_compression_margin_003.svg){width="100%" fig-alt="A speedup curve stays flat across most of the sparsity range, then climbs steeply at the right. A dashed red vertical line labeled 95 percent marks a red dot on the curve at the point labeled payoff."}
...
: **Optimization Technique Trade-Offs**: Comparison of the three major optimization approaches across key performance dimensions, highlighting how each technique addresses different constraints and deployment scenarios. Read the Hardware Dependency column first, because it decides whether the other columns matter. Pruning and quantization both cut size or latency at low training cost but each demands device support, sparse kernels for one and INT8 units for the other, while distillation is the only row that runs anywhere and the only one that charges a full training run up front. {#tbl-optimization-comparison tbl-colwidths="[13,13,16,20,16,22]"}
...
::: {#fig-color-mapping fig-env="figure" fig-pos="htb" fig-cap="**Convolutional Kernel Weights**: Five rows of first-layer filters group by learned pattern, ending in the near-uniform row the figure labels dead filters. That label names a hypothesis rather than a measurement, since a uniform-looking filter becomes a pruning candidate only after activation statistics confirm it stays near zero on real inputs." fig-alt="Grid of 40 stylized first-layer convolutional filters (8 columns by 5 rows), with rows labeled oriented edges, color blobs, frequency patterns, mixed features (center-surround), and dead filters; the last row appears nearly uniform."}
