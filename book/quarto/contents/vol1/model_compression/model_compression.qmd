...
Concrete systems keep those trade-offs measurable: ResNet-50 and MobileNetV2 (our Lighthouse Models from @sec-network-architectures-lighthouse-roster-model-biographies-a763) for vision workloads, transformer-based language models for sequence tasks, Deep Learning Recommendation Model (DLRM) for recommendation memory pressure [@naumov2019deep], and the DS-CNN, a depthwise-separable convolutional neural network (CNN), as the keyword spotter for TinyML deployment [@zhang2017hello]. Reusing these models lets us compare techniques under consistent conditions, making the trade-offs between accuracy, latency, memory, and energy tangible rather than abstract.
