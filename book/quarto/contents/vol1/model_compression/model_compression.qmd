...
::: {.column-margin}
![](images/svg/vol1_model_compression_margin_001.svg){width="100%" fig-alt="Margin illustration showing 175B FP16 weights, phone RAM, and MCU RAM."}

*Shows why frontier model weights cannot fit on phones or microcontrollers without compression.*
:::
...
**Reducing the Memory Pressure**: Low-rank factorization illustrates a classic systems trade-off: **trading computation for bandwidth reduction**. Storing a `{python} LowRankFactorization.mat_dim_str` by `{python} LowRankFactorization.mat_dim_str` matrix requires `{python} LowRankFactorization.full_mb_str` (at FP32). Fetching this matrix for a single inference is a massive memory bandwidth hit, especially when limited by physical memory bandwidth constraints.
...
**The energy and storage constraint**: Our **Smart Doorbell Lighthouse** operates at the opposite extreme of the iron law from DLRM. A typical TinyML budget is roughly 512 KB, where the purpose-built 500 KB DS-CNN keyword spotter fits. A stricter smart-doorbell microcontroller with 256 KB SRAM leaves closer to 100 KB for model weights after runtime buffers, audio windows, and feature extraction state, motivating more aggressive INT4 or binary compression.
...
Once quantized, inference is performed using integer arithmetic, which is significantly more efficient than floating-point operations on most hardware platforms [@gholami2022].
...
::: {.column-margin}
![](images/svg/vol1_model_compression_margin_002.svg){width="100%" fig-alt="Margin illustration showing Unfused versus fused memory-transfer count."}

*Shows operator fusion reducing memory transfers while leaving arithmetic unchanged.*
:::
...
$$
where $\mathbf{1}_{\{(\mathbf{W}_{\text{sparse}})_{ij} = 0\}}$ is an indicator function that yields one if entry $(i,j)$ is zero and 0 otherwise, and $\Vert \cdot \Vert_0$ represents the L0 norm\index{Sparsity!L0 norm}, which counts the number of nonzero elements.
...
::: {.column-margin}
![](images/svg/vol1_model_compression_margin_003.svg){width="100%" fig-alt="Margin illustration showing Unstructured sparsity payoff threshold near 90 to 95 percent."}

*Shows that unstructured sparsity needs a high enough zero fraction before overhead pays off.*
:::
...
::: {.column-margin}
![](images/svg/vol1_model_compression_margin_004.svg){width="100%" fig-alt="Margin illustration showing Model inference as 20 percent and other pipeline work as 80 percent."}

*Shows model compression hitting an end-to-end ceiling when non-model work dominates.*
:::
...
class ResNet50Int8Metrics:
    ...
    fp32_latency_ms_str = fmt(fp32_latency_ms, precision=1, commas=False, suffix=" ms")
    ...
    cpu_speedup_str = fmt(cpu_speedup, precision=1, commas=False, suffix="x")
    top1_drop_str = fmt(top1_drop, precision=1, commas=False)
    latency_speedup_str = fmt(latency_speedup, precision=1, commas=False, suffix="x")
    size_reduction_str = fmt(size_reduction, precision=0, commas=False, suffix="x")
    energy_improvement_str = fmt(energy_improvement, precision=1, commas=False, suffix="x")
```

When quantizing ResNet-50 from FP32 to INT8, baseline metrics show Top-1 accuracy of `{python} ResNet50Int8Metrics.fp32_top1_str`, inference latency on V100 of `{python} ResNet50Int8Metrics.fp32_latency_ms_str`, model size of `{python} ResNet50Int8Metrics.fp32_size_mb_str`, and energy per inference of `{python} ResNet50Int8Metrics.fp32_energy_j_str`. Postquantization metrics reveal Top-1 accuracy of `{python} ResNet50Int8Metrics.int8_top1_str` (`{python} ResNet50Int8Metrics.top1_drop_str` percentage-point degradation), inference latency of `{python} ResNet50Int8Metrics.int8_latency_ms_str` (`{python} ResNet50Int8Metrics.latency_speedup_str` speedup), model size of `{python} ResNet50Int8Metrics.int8_size_mb_str` (`{python} ResNet50Int8Metrics.size_reduction_str` reduction), and energy per inference of `{python} ResNet50Int8Metrics.int8_energy_j_str` (`{python} ResNet50Int8Metrics.energy_improvement_str` improvement). Additional analysis shows per-class accuracy degradation ranging from `{python} ResNet50Int8Metrics.per_class_min_drop_str` to `{python} ResNet50Int8Metrics.per_class_max_drop_str` percentage points with highest impact on fine-grained categories, calibration error increasing from `{python} ResNet50Int8Metrics.calibration_error_fp32_str` to `{python} ResNet50Int8Metrics.calibration_error_int8_str`, and INT8 quantization providing `{python} ResNet50Int8Metrics.latency_speedup_str` speedup on GPU but only `{python} ResNet50Int8Metrics.cpu_speedup_str` on CPU, demonstrating hardware-dependent gains.
