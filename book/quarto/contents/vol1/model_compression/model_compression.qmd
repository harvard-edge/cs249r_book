...
::: {#fig-quantization-free-lunch fig-env="figure" fig-pos="htb" fig-cap="**The Quantization Free Lunch**: Model accuracy vs. Bit-width. Most models exhibit a 'Free Lunch' plateau where reducing precision from FP32 to INT8 yields <1 percent accuracy loss. This robustness collapses at the 'Quantization Cliff' (typically 3--4 bits). Curves are illustrative and meant to show qualitative behavior." fig-alt="Line chart of accuracy vs. precision (bits). Blue line (CNN) stays flat to INT8 then drops at INT4 and INT3. Red line (transformer) holds higher and drops later, at INT3. Green shaded area marks the Free Lunch Zone of minimal loss."}
