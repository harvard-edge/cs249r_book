...
class CompressionDeploymentScale:
    ...
    smartphone_ram_str = fmt_int(round(smartphone_ram_gb), commas=False)
    mcu_ram_str = fmt(mcu_ram_kb, precision=1, commas=False)
```
...
class CompressionIronLawQuant:
    ...
    llm_175b_str = fmt(llm_175b_params, precision=0, commas=False)
    llm_175b_mem_str = fmt(llm_175b_mem_fp16_gb, precision=0, commas=False)
    llm_175b_mem_int8_str = fmt(llm_175b_mem_int8_gb, precision=0, commas=False)
...
class QuantizationEnergyPhysics:
    ...
    baseline_battery_hours_str = fmt(1, precision=0, commas=False)
    int8_battery_hours_str = fmt(int8_energy_reduction, precision=0, commas=False)
```
...
class QuantizationSpeedup:
    ...
    device_ram_gb_str = fmt(device_ram_gb, precision=0, commas=False)
    mem_bw_gbs_str = fmt_int(mem_bw_gbs, commas=False, suffix=' GB/s')
    kv_cache_gb_str = fmt_int(kv_cache_gb, commas=False)

    fp16_size_str = fmt_int(fp16_size_gb, commas=False)
    fp16_total_str = fmt_int(fp16_total_gb, commas=False)
    fp16_latency_str = fmt_int(fp16_latency_ms, commas=False)
    fp16_toks_str = fmt(fp16_toks, precision=1, commas=False, suffix=' tokens/s')

    int4_size_str = fmt(int4_size_gb, precision=1, commas=False)
    int4_latency_str = fmt_int(int4_latency_ms, commas=False)
...
class LowRankFactorization:
    ...
    full_mb_str = fmt(full_mb, precision=1, commas=False)
...
class QuantizationMemoryAnchor:
    ...
    llm_7b_mem_str = fmt(llm_7b_mem_fp16_gb, precision=0, commas=False)
    smartphone_ram_str = fmt(smartphone_ram_gb, precision=0, commas=False)
```
...
class QuantizationSavings:
    ...
    fp16_size_gb_str = fmt_int(fp16_size_gb, commas=False)
    int4_size_gb_str = fmt_int(int4_size_gb, commas=False)
    compression_ratio_str = fmt_int(ratio, commas=False)
    gpu_mem_fp16_gb_str = fmt(gpu_mem_fp16_gb, precision=0, commas=False, suffix=' GPU')
    gpu_mem_int4_gb_str = fmt(gpu_mem_int4_gb, precision=0, commas=False, suffix=' GPU')
```
...
class A100Int8Speedup:
    ...
    a100_tops_int8_str = fmt(a100_tops_int8, precision=0, commas=False)
...
1. **Tensor Core throughput**: NVIDIA A100 delivers `{python} A100Int8Speedup.a100_tflops_fp16_str` for FP16 vs. `{python} A100Int8Speedup.a100_tops_int8_str` TOPS (trillion INT8 operations/s) for INT8, a `{python} A100Int8Speedup.a100_int8_speedup_str`$\times$ peak throughput increase.
...
class TheoryPracticeSpeedup:
    ...
    realized_fraction_pct_str = fmt(realized_fraction_pct, precision=0, commas=False)
```
...
class FusionCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    feat_map_kb_str = fmt(feat_map_mb * THOUSAND, precision=1, commas=False)
    weights_mb_str = fmt(weights_mb, precision=1, commas=False)
    bn_params_kb_str = fmt(bn_params_mb * THOUSAND, precision=0, commas=False)
    unfused_conv_mb_str = fmt_int(round(unfused_conv_mb), commas=False)
    ...
    bandwidth_reduction_pct_str = fmt(bandwidth_reduction_pct, precision=1, commas=False)
    ...
    saved_latency_ms_str = fmt(saved_latency_ms, precision=1, commas=False)
    unfused_time_us_str = fmt_int(round(unfused_time_us), commas=False)
...
On a V100 GPU with `{python} V100Specs.v100_bw_gbs_str` HBM bandwidth and `{python} V100Specs.v100_tflops_fp32_str` FP32 compute, the unfused sequence takes approximately `{python} FusionCalc.unfused_time_us_str` microseconds (memory bound), while the fused version takes approximately `{python} FusionCalc.fused_time_us_str` microseconds (`{python} FusionCalc.fusion_speedup_str`$\times$ speedup). The speedup comes entirely from reducing memory traffic, as the compute remains identical.
...
Fusion also reduces kernel launch overhead. Each CUDA kernel launch incurs approximately 5-10 microseconds of latency. For a ResNet-50 with fifty-three convolutional layers, unfused execution launches `{python} FusionCalc.kernels_unfused_str` kernels (Conv + BN + ReLU), while fused execution launches `{python} FusionCalc.kernels_fused_str` kernels, saving approximately `{python} FusionCalc.saved_latency_ms_str` millisecond from launch overhead alone.
...
class AmdahlCompression:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    model_fraction_pct_str = fmt(model_fraction * 100, precision=0, commas=False)
    non_model_pct_str = fmt(non_model_fraction * 100, precision=0, commas=False)
...
class FallaciesAnalysis:
    ...
    bert_fp32_mb_str = MarkdownStr("440")
    bert_int8_mb_str = MarkdownStr("110")
    pruning_target_str = MarkdownStr("70")
    param_removal_str = MarkdownStr("40")
    resnet_fp32_acc_str = MarkdownStr("76.2")
    resnet_int8_acc_str = MarkdownStr("76.1")
    resnet_binary_acc_str = MarkdownStr("51")
```
