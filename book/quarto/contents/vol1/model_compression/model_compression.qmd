...
**Mechanism**: An NVIDIA GTX 580 provided only 3 GB HBM capacity—insufficient to store the 240 MB model weights alongside FP32 activations ($M_{\text{act}}$) and optimizer states ($M_{\text{state}}$) for single-GPU training.

**Fix**: They partitioned feature maps across two 3 GB GTX 580 GPUs, restricting cross-GPU communication to layers 3 and 6 to satisfy memory limits ($M_{\text{peak}} \le 3\text{ GB}$).

**Impact**: The two-tower architecture achieved a winning top-5 error rate of 15.3 percent (a 10.8 percentage point margin), demonstrating that hardware memory capacity bounds shape network architecture.
