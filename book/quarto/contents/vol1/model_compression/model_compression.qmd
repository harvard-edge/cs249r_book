...
::: {.column-margin}
![](images/svg/vol1_model_compression_margin_001.svg){width="100%" fig-alt="Vertical log-scale ladder of three blue bars: a 175B FP16 weight footprint of 350 GB at top, a phone RAM bar of 8 GB below it, and a much shorter microcontroller RAM bar of 524 KB at the bottom."}
...
class ModelDeviceComparison:
    ...
    dscnn_mem = Models.Tiny.DS_CNN.size_in_bytes(BYTES_FP32)
    dscnn_int8_mem = Models.Tiny.DS_CNN.size_in_bytes(BYTES_INT8)
    ...
    dscnn_int8_tiny_ratio = _overflow_ratio(dscnn_int8_mem, tiny_mem)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    # DS-CNN misses TinyML at FP32 and fits at INT8—that misfit is the lesson
    assert dscnn_tiny_overflow > 1, "DS-CNN at FP32 must exceed the TinyML envelope."
    assert dscnn_int8_tiny_ratio < 1, "DS-CNN at INT8 must fit in TinyML device."
    ...
    dscnn_int8_kb_str = fmt_qty(dscnn_int8_mem, KB, precision=0, commas=False)
    dscnn_tiny_overflow_mult_str = fmt_multiple(dscnn_tiny_overflow, precision=1, commas=False)
...
| **Model**              |                   **Loaded Weights**                   |               **Storage** **(Weights)**                | **Cloud** **(`{python} ModelDeviceComparison.cloud_cap_gb_str`)** | **Mobile** **(`{python} ModelDeviceComparison.mobile_cap_gb_str`)** |    **TinyML** **(`{python} ModelDeviceComparison.tiny_cap_kb_str`)**     |
|:-----------------------|:------------------------------------------------------:|:------------------------------------------------------:|:-----------------------------------------------------------------:|:-------------------------------------------------------------------:|:------------------------------------------------------------------------:|
...
| **DS-CNN (KWS)**       |     `{python} ModelDeviceComparison.dscnn_kb_str`      |     `{python} ModelDeviceComparison.dscnn_kb_str`      |                                 ok                                |                                  ok                                 |    no (`{python} ModelDeviceComparison.dscnn_tiny_overflow_mult_str`)    |
| **DS-CNN (KWS, INT8)** |   `{python} ModelDeviceComparison.dscnn_int8_kb_str`   |   `{python} ModelDeviceComparison.dscnn_int8_kb_str`   |                                 ok                                |                                  ok                                 |                                    ok                                    |

: **The Deployment Gap**: Model memory requirements compared against typical device capacities. DLRM represents recommendation-model embedding pressure [@naumov2019deep]; DS-CNN represents the TinyML keyword-spotting case [@zhang2017hello]. Even MobileNetV2 quantized to INT8 exceeds the `{python} ModelDeviceComparison.tiny_cap_kb_str` TinyML envelope by `{python} ModelDeviceComparison.mobilenet_int8_tiny_ratio_mult_str`, while the purpose-built DS-CNN keyword spotter fits within it only after INT8 quantization. Numbers in parentheses show how many times the model exceeds device memory. {#tbl-model-vs-device tbl-colwidths="[20,15,15,15,15,20]"}
...
The goal of pruning is to find a sparse version of the model parameters $\hat{\mathbf{W}}$ that minimizes the increase in prediction error (loss) while satisfying a fixed parameter budget $k$. Framing this goal mathematically clarifies both the objective and why approximate solutions are necessary:
$$
\min_{\hat{\mathbf{W}}} \mathcal{L}(\hat{\mathbf{W}}) \quad \text{subject to} \quad \|\hat{\mathbf{W}}\|_0 \leq k
$$
where $\|\hat{\mathbf{W}}\|_0$ is the **L0-norm**\index{L0 Norm!parameter count} (the count of nonzero parameters). Since minimizing the L0-norm is NP-hard\index{NP-hard!pruning optimization}, we use heuristics[^fn-heuristic-pruning] like **magnitude-based pruning**\index{Magnitude-Based Pruning!definition}. @Lst-pruning_example demonstrates this approach, removing weights with small absolute values to transform a dense weight matrix into the sparse representation\index{Sparse Matrix!storage efficiency} visualized in @fig-sparse-matrix.
...
Formalizing this process, let $\mathbf{W} \in \mathbb{R}^{m \times n}$ represent a weight matrix in a given layer. Pruning removes a subset of weights by applying a binary mask\index{Pruning!binary mask} $\mathbf{M} \in \{0,1\}^{m \times n}$, yielding a pruned weight matrix:
$$
\hat{\mathbf{W}} = \mathbf{M} \odot \mathbf{W}
$$
where $\odot$ represents the element-wise Hadamard product\index{Hadamard Product!pruning mask}. The mask $\mathbf{M}$ is constructed based on a pruning criterion, typically weight magnitude. A common approach is magnitude-based pruning, which removes a fraction $\rho_{\text{sparse}}$ of the lowest-magnitude weights by defining a threshold $\delta_{\text{prune}}$ such that:
$$
M_{i,j} =
\begin{cases}
...
%%%%%%%%%%%%%%%%%%%%%%%
\begin{scope}[local bounding box=ROW1,shift={(0,0)}]
\Iteration{1,...,12}{3}{4}{0.995}{1,...,6}{13}{14}{1st Iteration}\Test{0.971}{0.992};
\end{scope}
...
[^fn-distilbert-deployment]: **DistilBERT**: The reported model has 66 million parameters versus BERT-Base's 110 million, is 40 percent smaller and 60 percent faster, and retains 97 percent of its language-understanding performance. Absolute memory and latency still depend on numerical format, batch size, sequence length, runtime, and hardware [@sanh2019distilbert]. \index{DistilBERT!deployment metrics}
...
**Low-Rank Matrix Factorization (LRMF)**\index{Low-Rank Factorization!definition}\index{Model Compression!low-rank factorization} approximates weight matrices with lower-rank representations. Given a matrix $\mathbf{A} \in \mathbb{R}^{m \times n}$, LRMF finds matrices $\mathbf{U} \in \mathbb{R}^{m \times k}$ and $\mathbf{V} \in \mathbb{R}^{k \times n}$ such that:
$$
\mathbf{A} \approx \mathbf{U}\mathbf{V}
$$
...
To see why this matters, study @fig-matrix-factorization: the matrix $\mathbf{M}$ can be approximated by the product of matrices $\mathbf{U}_k$ and $\mathbf{V}_k^T$. For intuition, most fully connected layers in networks are stored as a projection matrix $\mathbf{M}$, which requires $m{\times}n$ parameters to be loaded during computation. However, by decomposing and approximating it as the product of two lower-rank matrices, we only need to store $m \times k + k \times n$ parameters in terms of storage. Applying the factors directly costs $\mathcal{O}(k(m+n))$ for a vector input, instead of $\mathcal{O}(mn)$ for the original dense matrix; explicitly forming the dense product $\mathbf{U}\mathbf{V}$ would cost $\mathcal{O}(mkn)$ and should be avoided in inference [@Denton2014].
...
(B3)--(A3) (F3)--(E3);
%
\node[right=0.3 of $(G3)!0.5!(D3)$]{$W\in\mathbb{R}^{T\times R}$};
\end{scope}
...
%
\node[right=0.3 of $(G3)!0.5!(D3)$]{$W\in\mathbb{R}^{T\times R}$};
\node[RedLine,left=2pt of $(MB3)!0.9!(MA3)$](UI){$\boldsymbol{w_i}$};
\end{scope}
...
: **NAS Search Strategy Comparison**: Representative trade-offs between search efficiency, use cases, and limitations for different NAS approaches. Reinforcement learning offers unconstrained exploration at high cost, evolutionary methods exploit parallelism, and gradient-based or weight-sharing approaches achieve dramatic speedups with potential optimality trade-offs. The GPU-day figures reflect efficient modern implementations that use proxy tasks and reduced search spaces; the landmark early searches were one to two orders of magnitude more expensive [@zoph2017neural; @real2019; @liu2019darts; @pham2018efficient]. {#tbl-nas-strategies tbl-colwidths="[23,18,34,25]"}

Reinforcement learning\index{Reinforcement Learning!NAS application} based NAS\index{NAS!reinforcement learning strategy}\index{Neural Architecture Search|see{NAS}} treats architecture search as a sequential decision problem where a controller generates architectures and receives accuracy as reward. The controller (typically an LSTM) learns to propose better architectures over time through policy gradient optimization. This approach discovered high-performing architectures like NASNet\index{NASNet!RL-discovered architecture}, but its inner loop is expensive because every reward requires training a candidate architecture; @zoph2017neural evaluated roughly 12,800 candidate architectures, totaling 22,400 GPU-days. That cost explains why practical NAS work moved toward weight sharing and lower-cost performance predictors.

Evolutionary algorithms\index{Evolutionary Algorithms!NAS application}\index{NAS!evolutionary strategy} maintain a population of candidate architectures and iteratively apply mutations (changing operations, adding connections) and crossover (combining parent architectures) to generate offspring. Fitness-based selection retains high-performing architectures for the next generation, so useful components such as skip connections or depthwise separable convolutions can be recombined rather than rediscovered from scratch. AmoebaNet\index{AmoebaNet!evolutionary NAS} used evolution to achieve state-of-the-art results after 3,150 GPU-days [@real2019], showing both the value of population search and the continuing need for proxy tasks, weight sharing, or massive parallelism to control search cost.
...
The practical payoff of quantization becomes concrete in @fig-quantization-impact. Compare the orange (FP32) and blue (INT8) segments in each stacked bar to see the gains when moving from FP32 to INT8. Model size falls by exactly 4$\times$ for every architecture, because each weight shrinks from four bytes to one, while inference time falls by 4--8$\times$ depending on how well each architecture maps onto the target hardware's INT8 path. The pattern across architectures makes quantized models well suited for deployment in resource-constrained environments.

::: {#fig-quantization-impact fig-env="figure" fig-pos="htb" fig-cap="**Quantization Impact**: Moving from FP32 to INT8 reduces both inference time and model size. Model size falls by exactly 4$\times$ for all three architectures because the weights themselves shrink from four bytes to one, while the inference-time gain ranges from 4$\times$ to 8$\times$ with the quality of each architecture's INT8 mapping and the target hardware's INT8 acceleration." fig-alt="Two stacked bar charts comparing FP32 and INT8. Left chart shows inference time in milliseconds for Inception_v3, MobileNet_v1, and ResNet_v2; right chart shows model size in megabytes. Orange segments mark FP32 values, blue segments mark INT8 values; INT8 is smaller and faster across all models."}
...
- **Hardware requirement**: Fits comfortably on `{python} QuantizationSavings.gpu_mem_int4_gb_str` GPU (for example, RTX 3070, consumer laptops).
...
::: {#exmp-model-compression-scale-zero-point .callout-example title="Calculating scale and zero-point"}
...
class ResNet50Int8Metrics:
    ...
    # via TensorRT.  Suppress the fallback warning for FP32 base metrics.
    ...
    # Use CompressionModel to quantify INT8 gains
    ...
    latency_speedup_mult_str = fmt_multiple(latency_speedup, precision=0, commas=False)
    size_reduction_mult_str = fmt_multiple(size_reduction, precision=0, commas=False)
    energy_improvement_mult_str = fmt_multiple(energy_improvement, precision=0, commas=False)
```
...
class FallaciesAnalysis:
    ...
    resnet_fp32_acc_str = fmt_percent(0.761, precision=1, commas=False, style='prose')
    resnet_int8_acc_str = fmt_percent(0.758, precision=1, commas=False, style='prose')
