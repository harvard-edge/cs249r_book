...
The middle layer, efficient numerics representation, optimizes how numerical values are stored and processed. **Quantization**\index{Quantization!precision reduction} and mixed-precision\index{Mixed-Precision Training} training reduce the bit-width of weights and activations (for example, from 32-bit floating point to 8-bit integers), enabling faster execution and lower memory usage on specialized hardware.
...
::: {#exmp-model-compression-alexnet-2012 .callout-example title="AlexNet's two-GPU split (2012)"}
...
A mobile BERT pipeline\index{BERT!mobile deployment pipeline} makes that sequencing dependency concrete by combining pruning, distillation, and quantization under one deployment target.
