...
[^fn-nas-search-cost]: **Neural Architecture Search (NAS)**: @zoph2017neural at Google Brain used reinforcement learning to learn the architecture itself at a cost of `{python} NASCostCalc.gpu_days_str` (`{python} NASCostCalc.gpus_str` for `{python} NASCostCalc.days_str` days), equivalent to `{python} NASCostCalc.gpu_hours_str`. Weight-sharing approaches later reduced this by `{python} NASCostCalc.weight_sharing_reduction_mult_str`, transforming NAS from a data-center-scale experiment into a practical tool whose one-time search budget is amortized across every deployment of the resulting architecture (EfficientNet, MobileNetV3). \index{NAS!search cost}
...
| **DRAM Read**   | `{python} QuantizationEnergyPhysics.energy_dram_read_bits_str`-bit | **`{python} QuantizationEnergyPhysics.energy_dram_read_ratio_mult_str`** |

: **The Energy Hierarchy**: Relative energy cost of representative arithmetic and memory operations, normalized to an 8-bit integer add. A 32-bit float add costs `{python} QuantizationEnergyPhysics.energy_float_add_ratio_mult_str` more than its 8-bit integer counterpart, but a single `{python} QuantizationEnergyPhysics.energy_dram_read_bits_str`-bit DRAM read costs `{python} QuantizationEnergyPhysics.energy_dram_read_ratio_mult_str` more—more than four orders of magnitude above on-chip compute. This gap is why bit-width reductions that shrink data movement dominate the energy budget of inference far more than the arithmetic savings alone would suggest. {#tbl-model-compression-energy-hierarchy tbl-colwidths="[33,33,33]"}
...
# │          int4_toks_str, speedup_mult_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
...
def _get_ratio(model_mem, device_mem):
    ...
    return MarkdownStr(f"no ({fmt_multiple(ratio)})")
...
# │ Exports: simd_fp32_str, simd_int8_str, simd_gain_mult_str,
...
$\text{Throughput Gain} = \text{INT8 elements/inst} / \text{FP32 elements/inst}$ = `{python} SIMDThroughput.simd_int8_str`/`{python} SIMDThroughput.simd_fp32_str` = `{python} SIMDThroughput.simd_gain_mult_str`
...
1. **Tensor Core throughput**: NVIDIA A100 delivers `{python} A100Int8Speedup.a100_tflops_fp16_str` for FP16 vs. `{python} A100Int8Speedup.a100_tops_int8_str` (trillion INT8 operations/s) for INT8, a `{python} A100Int8Speedup.a100_int8_speedup_mult_str` peak throughput increase.
...
On a V100 GPU with `{python} V100Specs.v100_bw_gbs_str` HBM bandwidth and `{python} V100Specs.v100_tflops_fp32_str` FP32 compute, the unfused sequence takes approximately `{python} FusionCalc.unfused_time_us_str` (memory bound), while the fused version takes approximately `{python} FusionCalc.fused_time_us_str` (`{python} FusionCalc.fusion_speedup_mult_str` speedup). The speedup comes entirely from reducing memory traffic, as the compute remains identical.
