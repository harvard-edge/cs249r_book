...
::: {#fig-tensor-decomposition fig-env="figure" fig-pos="htb" fig-cap="**Tensor Decomposition**: A 3D tensor with dimensions $M{\times}N{\times}T$ is decomposed into a sum of rank-one components, each formed by the outer product of highlighted vectors from the U, X, and V factor matrices. This extends low-rank matrix factorization to multi-dimensional data, reducing storage and computation for convolutional layers. Source: [@gholami2022]." fig-alt="A three-dimensional tensor labeled y with dimensions M by N by T beside factor matrices U, X, and V; highlighted vectors u i, x i, and v i correspond to the highlighted tensor entry y i j t."}
...
::: {#fig-quantization-impact fig-env="figure" fig-pos="htb" fig-cap="**Illustrative Quantization Scenario**: Across all three model examples, the INT8 values are lower for both inference time and model size than their FP32 counterparts. The values illustrate the two directions of benefit, not measurements on one common platform." fig-alt="Two stacked bar charts comparing FP32 and INT8. Left chart shows inference time in milliseconds for Inception_v3, MobileNet_v1, and ResNet_v2; right chart shows model size in megabytes. Orange segments mark FP32 values, blue segments mark INT8 values; INT8 is smaller and faster across all models."}
...
::: {.column-margin}
![](images/svg/vol1_model_compression_margin_004.svg){width="100%" fig-alt="A horizontal stacked bar has a small blue segment labeled Model 20% and a wide gray segment labeled Other 80%; a bracket below is labeled max 1.25x."}
