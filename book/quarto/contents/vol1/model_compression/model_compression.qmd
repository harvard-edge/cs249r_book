...
[^fn-nas-search-cost]: **Neural Architecture Search (NAS)**: @zoph2017neural at Google Brain used reinforcement learning to learn the architecture itself at a cost of `{python} NASCostCalc.gpu_days_str` (`{python} NASCostCalc.gpus_str` for `{python} NASCostCalc.days_str` days), equivalent to `{python} NASCostCalc.gpu_hours_str`. Weight-sharing approaches later reduced this by `{python} NASCostCalc.weight_sharing_reduction_str`$\times$, transforming NAS from a data-center-scale experiment into a practical tool whose one-time search budget is amortized across every deployment of the resulting architecture (EfficientNet, MobileNetV3). \index{NAS!search cost}
...
class FusionCalc:
    ...
    unfused_time_us_str = fmt_time(round(unfused_time_us), "microsecond", precision=0, commas=False, style="word")
    fused_time_us_str = fmt_time(fused_time_us, "microsecond", precision=1, commas=False, style="word")
