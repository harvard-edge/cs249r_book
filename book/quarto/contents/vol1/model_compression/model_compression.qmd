...
class CompressionDeploymentScale:
    ...
    smartphone_ram = round(Platforms.Mobile.ram.to(GB).magnitude) * GB
    mcu_ram_kb = Platforms.Tiny.ram.to(KB).magnitude
    mcu_ram = mcu_ram_kb * KB

    check(llm_7b_mem_fp16 > smartphone_ram,
    ...
    smartphone_ram_gb_str = fmt_qty_int(smartphone_ram, GB, commas=False)
    mcu_ram_kb_str = fmt_qty(mcu_ram, KB, precision=1, commas=False)
```
...
The scale of this renegotiation makes model optimization an engineering discipline, not a collection of ad hoc tricks. A `{python} CompressionDeploymentScale.llm_175b_str` billion parameter model consumes over `{python} CompressionDeploymentScale.llm_175b_mem_gb_str` in FP16 representation alone, yet a smartphone provides `{python} CompressionDeploymentScale.smartphone_ram_gb_str` of RAM and a microcontroller offers `{python} CompressionDeploymentScale.mcu_ram_kb_str`. Bridging six orders of magnitude requires systematic methods with predictable trade-offs, not trial and error. Every optimization technique removes something from the model (redundant parameters, numerical precision, or architectural complexity), and the engineer must understand exactly what is lost, what is preserved, and how these losses compose when techniques are combined.
...
class CompressionIronLawQuant:
    ...
    llm_175b_fp16_int8_reduction_str = fmt_multiple(llm_175b_fp16_int8_reduction, precision=0, commas=False)
```
...
class NASCostCalc:
    ...
    weight_sharing_reduction_str = fmt_multiple(weight_sharing_reduction, precision=0, commas=True)
```
...
from mlsysim.fmt import fmt, check, fmt_energy_per_byte, fmt_qty, fmt_time
...
class QuantizationEnergyPhysics:
    ...
    energy_dram_pj_per_byte_str = fmt_energy_per_byte(energy_dram_per_byte, precision=0, commas=False)
    energy_flop_fp32_pj_str = fmt_qty(energy_flop_fp32, pJ, precision=1, commas=False)
    energy_op_int8_pj_str = fmt_qty(energy_op_int8, pJ, precision=1, commas=False)
    energy_float_add_ratio_str = fmt_multiple(energy_float_add_ratio, precision=0, commas=False)
    energy_dram_read_ratio_str = fmt_multiple(energy_dram_read_ratio, precision=1, commas=True)
    int8_energy_reduction_str = fmt_multiple(int8_energy_reduction, precision=0, commas=False)
    mem_reduction_ratio_str = fmt_multiple(bytes_fp32 / bytes_int8, precision=0, commas=False)
...
1.  **Memory movement $(D_{\text{vol}} \times E_{\text{move}})$**: Fetching a 32-bit float from DRAM costs ≈ `{python} QuantizationEnergyPhysics.energy_dram_pj_str`. Fetching an 8-bit integer costs ≈ `{python} QuantizationEnergyPhysics.energy_dram_pj_per_byte_str`.
2.  **Compute work $(O \times E_{\text{compute}})$**: A 32-bit FLOP costs ≈ `{python} QuantizationEnergyPhysics.energy_flop_fp32_pj_str`. An INT8 operation costs ≈ `{python} QuantizationEnergyPhysics.energy_op_int8_pj_str`.
...
# │          mem_bw_gbs_str, kv_cache_gb_str, fp16_size_gb_str, fp16_total_gb_str,
# │          fp16_latency_ms_str, fp16_toks_str, int4_size_gb_str, int4_latency_ms_str,
# │          int4_toks_str, speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.fmt import fmt_int,  fmt, fmt_qty, fmt_qty_int, check, fmt_time, fmt_rate
...
class QuantizationSpeedup:
    ...
    fp16_latency = fp16_size / mem_bw
    int4_latency = int4_size / mem_bw

    # Step 3: Throughput (tokens/s)
    fp16_toks = (1 / fp16_latency).to(1 / second).magnitude
    int4_toks = (1 / int4_latency).to(1 / second).magnitude
    ...
    mem_bw_gbs_str = fmt_qty_int(mem_bw, GB/second, commas=False)
    kv_cache_gb_str = fmt_qty_int(kv_cache, GB, commas=False)

    fp16_size_gb_str = fmt_qty_int(fp16_size, GB, commas=False)
    fp16_total_gb_str = fmt_qty_int(fp16_total, GB, commas=False)
    ...
    speedup_str = fmt_multiple(speedup, commas=False, precision=0)
```
...
1.  **Model size**: `{python} QuantizationSpeedup.params_b_str` $\times 10^9 \times$ `{python} QuantizationSpeedup.bytes_fp16_str` bytes = `{python} QuantizationSpeedup.fp16_size_gb_str`.
2.  **KV Cache**: Context window (4096 tokens) requires ≈ `{python} QuantizationSpeedup.kv_cache_gb_str`.
3.  **Total memory**: `{python} QuantizationSpeedup.fp16_size_gb_str` + `{python} QuantizationSpeedup.kv_cache_gb_str` = `{python} QuantizationSpeedup.fp16_total_gb_str`. This barely fits, leaving no room for OS or buffers.
4.  **Bandwidth cost**: Loading `{python} QuantizationSpeedup.fp16_size_gb_str` at `{python} QuantizationSpeedup.mem_bw_gbs_str` takes `{python} QuantizationSpeedup.fp16_latency_ms_str` per token. That is `{python} QuantizationSpeedup.fp16_toks_str`, too slow for chat.

**Fix (INT4)**:

1.  **Quantization**: Convert weights to INT4 (`{python} QuantizationSpeedup.bytes_int4_str` bytes).
2.  **New size**: `{python} QuantizationSpeedup.params_b_str` $\times 10^9 \times$ `{python} QuantizationSpeedup.bytes_int4_str` = `{python} QuantizationSpeedup.int4_size_gb_str`.
3.  **New speed**: Loading `{python} QuantizationSpeedup.int4_size_gb_str` takes `{python} QuantizationSpeedup.int4_latency_ms_str`. Speed jumps to `{python} QuantizationSpeedup.int4_toks_str`.

**Systems insight**: Quantization makes the model fit and provides a `{python} QuantizationSpeedup.speedup_str`$\times$ linear speedup because LLM generation is bandwidth bound.
...
**Business value**: Compression turned a nonviable prototype into a deployable feature. Without INT8 quantization, the product could not exist for the target market.
...
# │ Exports: dlrm_gb_str, gpt2_gb_str, resnet_mb_str, mobilenet_mb_str, mobilenet_int8_mb_str,
# │          dscnn_kb_str, cloud_cap_gb_str, mobile_cap_gb_str, tiny_cap_kb_str,
...
def _get_ratio(model_mem, device_mem):
    """Return 'ok' if model fits, else 'no (N×)' with how many times it overflows."""
    ratio = model_mem.to(byte).magnitude / device_mem.to(byte).magnitude
    if ratio < 1:
        return MarkdownStr("ok")
    return MarkdownStr(f"no ({fmt_multiple(ratio, precision=0)}$\\times$)")
...
class ModelDeviceComparison:
    ...
    resnet_mem = 100 * MB            # scenario assumption: ~25M params × 4 bytes
    ...
    dscnn_mem = 500 * KB             # scenario assumption
    ...
    resnet_mb_str = fmt_qty(resnet_mem, MB, precision=0, commas=False)
    mobilenet_mb_str = fmt_qty(mobilenet_mem, MB, precision=0, commas=False)
    ...
    mobile_cap_gb_str = fmt_qty_int(mobile_mem, GB, commas=False, approx=True)
    tiny_cap_kb_str = fmt_qty(tiny_mem, KB, precision=1, commas=False, approx=True)
    ...
    mobilenet_int8_tiny_ratio_str = fmt_multiple(mobilenet_int8_tiny_ratio, precision=1, commas=False)
...
|:-----------------------|:------------------------------------------------------:|:------------------------------------------------------:|:-----------------------------------------------------------------:|:-------------------------------------------------------------------:|:-----------------------------------------------------------------:|
| **DLRM**               |      `{python} ModelDeviceComparison.dlrm_gb_str`      |      `{python} ModelDeviceComparison.dlrm_gb_str`      |                                 ok                                |           `{python} ModelDeviceComparison.dlrm_mobile_str`          |           `{python} ModelDeviceComparison.dlrm_tiny_str`          |
| **GPT-2 XL**           |      `{python} ModelDeviceComparison.gpt2_gb_str`      |      `{python} ModelDeviceComparison.gpt2_gb_str`      |                                 ok                                |                                  ok                                 |           `{python} ModelDeviceComparison.gpt2_tiny_str`          |
| **ResNet-50**          |     `{python} ModelDeviceComparison.resnet_mb_str`     |     `{python} ModelDeviceComparison.resnet_mb_str`     |                                 ok                                |                                  ok                                 |          `{python} ModelDeviceComparison.resnet_tiny_str`         |
| **MobileNetV2**        |   `{python} ModelDeviceComparison.mobilenet_mb_str`    |   `{python} ModelDeviceComparison.mobilenet_mb_str`    |                                 ok                                |                                  ok                                 |        `{python} ModelDeviceComparison.mobilenet_tiny_str`        |
| **MobileNetV2 (INT8)** | `{python} ModelDeviceComparison.mobilenet_int8_mb_str` | `{python} ModelDeviceComparison.mobilenet_int8_mb_str` |                                 ok                                |                                  ok                                 |      `{python} ModelDeviceComparison.mobilenet_int8_tiny_str`     |
| **DS-CNN (KWS)**       |     `{python} ModelDeviceComparison.dscnn_kb_str`      |     `{python} ModelDeviceComparison.dscnn_kb_str`      |                                 ok                                |                                  ok                                 |          `{python} ModelDeviceComparison.dscnn_tiny_str`          |
...
# │
# │ Imports: mlsysim.fmt typed formatters and check
# │ Exports: MobileNetCompressionAnchor.pruning_pct_str, .original_size_mb_str, .pruned_size_mb_str
# └─────────────────────────────────────────────────────────────────────────────
...
Framework pruning is only useful when it produces a deployment artifact the runtime can exploit: a smaller dense model, a structured sparse model, or a sparse format with matching kernels. In PyTorch\index{Framework Toolkits!pruning support}, the `torch.nn.utils.prune` module provides a flexible interface for pruning individual layers or entire models. It supports unstructured pruning (for example, `l1_unstructured`) and structured pruning (for example, `ln_structured`), but the key systems detail is mask handling: the original parameters are preserved, and a binary mask is multiplied element-wise during the forward pass. To realize actual memory savings for deployment, these masks must be "permanently" applied using `prune.remove(module, 'weight')`.
...
A large language model achieves state-of-the-art accuracy on medical question-answering, but at hundreds of billions of parameters it cannot run on a hospital's on-premise server constrained to a single GPU. Pruning alone cannot bridge this gap because a sparse variant is insufficient; the target architecture needs to be fundamentally different. Knowledge distillation solves this problem by training a compact "student" model to replicate the large "teacher" model's behavior, achieving 90 percent or more of the teacher's accuracy at a fraction of the compute [@ba2014deep]. The key insight is that the teacher's predictions carry far more information than the raw training labels.
...
class LowRankFactorization:
    ...
    data_reduction_str = fmt_multiple(data_reduction, precision=0, commas=False)
```
...
@Fig-tensor-decomposition breaks down a 3D tensor into its factor matrices, showing how each rank-one component contributes to the reconstruction. The choice among decomposition methods depends on tensor order, target rank, and inference overhead:
...
[^fn-nas-hardware-aware]: [offset=-15mm] **Hardware-Aware NAS**: The key systems choice is what the evaluation stage measures. Hardware-unaware NAS optimizes FLOPs; hardware-aware NAS optimizes measured latency, which can diverge by 3--5$\times$ for the same FLOP count because memory access and operator support dominate. @tan2019 fed device latency back into the search, finding architectures 1.8$\times$ faster than MobileNetV2 at higher accuracy and exposing depthwise convolutions as memory-bound on mobile CPUs. \index{NAS!hardware-aware}
...
# │ Exports: QuantizationMemoryAnchor.llm_7b_str, .llm_7b_mem_gb_str,
# │          .smartphone_ram_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.fmt import fmt, fmt_qty, check
...
class QuantizationMemoryAnchor:
    ...
    llm_7b_mem_gb_str = fmt_qty(llm_7b_mem_fp16, GB, precision=0, commas=False)
    smartphone_ram_gb_str = fmt_qty(smartphone_ram, GB, precision=0, commas=False)
```

A `{python} QuantizationMemoryAnchor.llm_7b_str` billion parameter language model stored in FP16 consumes `{python} QuantizationMemoryAnchor.llm_7b_mem_gb_str`, yet users expect it to run on a smartphone with `{python} QuantizationMemoryAnchor.smartphone_ram_gb_str` of shared RAM. Structural optimization alone cannot bridge this gap: even aggressive pruning rarely exceeds 50--70 percent parameter reduction, leaving a model far too large for the target device. The remaining gains come from a different dimension entirely: reducing the number of bits used to represent each parameter. *Quantization*, the process of reducing numerical precision, offers one of the most impactful optimizations for deployment, because it trades bits for speed and efficiency with minimal accuracy loss.
...
class EnergyDividend:
    ...
    energy_dividend_factor_str = fmt_multiple(round(dividend), precision=0, commas=False)
    energy_dram_str = fmt(e_dram, precision=0, commas=False)
    dram_read_bits_str = fmt(dram_read_bits, precision=0, commas=False)
    dram_read_vs_int8_mac_str = fmt_multiple(dram_read_vs_int8_mac, precision=1, commas=True)
```
...
class QuantizationSavings:
    ...
    gpu_mem_fp16 = 24 * GB  # scenario requirement: 24 GB-class GPU for FP16 deployment
    gpu_mem_int4 = 8 * GB  # scenario requirement: 8 GB-class GPU for INT4 deployment
    bytes_fp16 = BYTES_FP16.to(byte).magnitude
    bytes_int4 = BYTES_INT4.to(byte).magnitude
    ...
    compression_ratio_str = fmt_multiple(ratio, commas=False, precision=0)
...
class SIMDThroughput:
    ...
    simd_gain_str = fmt_multiple(gain, precision=0, commas=False)
...
class A100Int8Speedup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_tflops_fp16_str = fmt_qty(a100_fp16, TFLOP/second, precision=0, commas=False)
    a100_tops_int8_str = fmt_qty(a100_int8, TOPS, precision=0, commas=False)
    a100_int8_speedup_str = fmt_multiple(a100_int8_speedup, precision=0, commas=False)
    bandwidth_bound_speedup_str = fmt_multiple(bandwidth_bound_speedup, precision=0, commas=False)
```
...
::: {.column-margin}
![](images/svg/model_compression_quantization_roofline.svg){width="100%" fig-alt="Mini roofline with one workload dot on the memory-bound slope labeled mem 2x and one workload dot on the compute-bound ceiling labeled comp 8x, showing that quantization speedup depends on bottleneck regime."}

*Quantization speedup depends on which side of the ridge you occupy.*
:::
...
class EnergyRatio:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    int8_fp32_energy_ratio_str = fmt_multiple(int8_fp32_energy_ratio, precision=0, commas=False)
```
...
Quantization granularity\index{Quantization!granularity} is the control knob that trades calibration overhead for accuracy: fewer shared ranges are cheaper, while more local ranges preserve more information when channels differ. The common choices are:
...
class TheoryPracticeSpeedup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    pruning_speedup_str = fmt_multiple(pruning_speedup, precision=0, commas=False)
    quant_speedup_str = fmt_multiple(quant_speedup, precision=0, commas=False)
    theoretical_speedup_str = fmt_multiple(theoretical_speedup, precision=0, commas=False)
    measured_speedup_str = fmt_multiple(measured_speedup, precision=1, commas=False)
...
class ConvFusionCalc:
    ...
    transfer_reduction_str = fmt_multiple(transfer_reduction, precision=0, commas=False)
...
class FusionCalc:
    ...
    fusion_speedup_str = fmt_multiple(fusion_speedup, precision=2, commas=False)
```
...
class V100Specs:
    v100_bw = Hardware.Cloud.V100.memory.bandwidth
    v100_tflops_fp32 = Hardware.Cloud.V100.compute.precision_flops["fp32"]

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    v100_bw_gbs_str = fmt_qty(v100_bw, GB/second, precision=0, commas=False)
    v100_tflops_fp32_str = fmt_qty(v100_tflops_fp32, TFLOP/second, precision=1, commas=False)
```
...
Low-rank approximation, covered earlier in @sec-model-compression-lowrank-factorization-955e, is the related dense-factor alternative. Unlike sparsity exploitation, which skips literal zero-valued weights, low-rank approximation replaces a dense matrix with two smaller dense factors that capture the most important information while discarding redundant components. For instance, a weight matrix of size $1000{\times}1000$ with one million parameters can be factorized into two smaller matrices, say $U$ (size $1000{\times}50$) and $V$ (size $50{\times}1000$), which results in only 100,000 parameters, much fewer than the original one million. This smaller representation retains the key features of the original matrix while reducing storage and computation when the factorized path executes directly [@Denton2014].

The deployment choice is therefore sparse skips vs. dense factors rather than a generic desire for fewer parameters. Low-rank approximations, particularly Singular Value Decomposition, compress weight matrices in neural networks by discarding low-energy singular values. Recommendation systems and natural language processing models use these approximations extensively to reduce computational complexity and memory usage without significant performance loss [@joulin2017].
...
A sparse pattern earns hardware speedup only when it is regular enough for kernels to predict. Two prominent formats make that regularity explicit: block sparse matrices\index{Sparsity!block sparse} and N:M sparsity patterns. Block sparse matrices isolate blocks of zero and nonzero dense submatrices so that operations on the large sparse matrix can be re-expressed as a smaller number of dense operations on submatrices. This structure supports more efficient storage of dense submatrices while maintaining shape compatibility for matrix or vector products. For example, @fig-block-sparse-gemm shows how NVIDIA's cuSPARSE [@nvidia_cusparse_block] library supports sparse block matrix operations and storage. Several other works, such as Monarch matrices\index{Sparsity!Monarch matrices} [@dao2022monarchexpressivestructuredmatrices], have built on this block-sparsity approach to strike an improved balance between matrix expressivity and compute/memory efficiency.
...
The same format-hardware contract explains why sparsity often disappoints in practice. @Tbl-sparsity-optimization summarizes the challenges that limit its practical effectiveness.
...
class AmdahlCompression:
    ...
    max_speedup_str = fmt_multiple(max_speedup, precision=2, commas=False)
```
...
# │          .per_class_drop_pp_range_str,
# │          .calibration_error_fp32_str, .calibration_error_int8_str,
# │          .cpu_speedup_str, .top1_drop_pp_str, .latency_speedup_str,
...
class ResNet50Int8Metrics:
    ...
    fp32_latency_ms_str = fmt_time(round(fp32_latency.to(ms).magnitude), 'millisecond', precision=0, commas=False)
    int8_latency_ms_str = fmt_time(int8_latency, 'millisecond', precision=1, commas=False)
    ...
    per_class_drop_pp_range_str = fmt_range(
        per_class_min_drop, per_class_max_drop, precision=1, commas=False,
    ...
    cpu_speedup_str = fmt_multiple(cpu_speedup, precision=1, commas=False)
    top1_drop_pp_str = fmt_pp(top1_drop, precision=1, commas=False, attributive=True)
    latency_speedup_str = fmt_multiple(latency_speedup, precision=1, commas=False)
    size_reduction_str = fmt_multiple(size_reduction, precision=0, commas=False)
    energy_improvement_str = fmt_multiple(energy_improvement, precision=1, commas=False)
```

When quantizing ResNet-50 from FP32 to INT8, baseline metrics show Top-1 accuracy of `{python} ResNet50Int8Metrics.fp32_top1_str`, inference latency on V100 of `{python} ResNet50Int8Metrics.fp32_latency_ms_str`, model size of `{python} ResNet50Int8Metrics.fp32_size_mb_str`, and energy per inference of `{python} ResNet50Int8Metrics.fp32_energy_j_str`. Postquantization metrics reveal Top-1 accuracy of `{python} ResNet50Int8Metrics.int8_top1_str` (`{python} ResNet50Int8Metrics.top1_drop_pp_str` degradation), inference latency of `{python} ResNet50Int8Metrics.int8_latency_ms_str` (`{python} ResNet50Int8Metrics.latency_speedup_str`$\times$ speedup), model size of `{python} ResNet50Int8Metrics.int8_size_mb_str` (`{python} ResNet50Int8Metrics.size_reduction_str`$\times$ reduction), and energy per inference of `{python} ResNet50Int8Metrics.int8_energy_j_str` (`{python} ResNet50Int8Metrics.energy_improvement_str`$\times$ improvement). Additional analysis shows per-class accuracy degradation of `{python} ResNet50Int8Metrics.per_class_drop_pp_range_str` with highest impact on fine-grained categories, calibration error increasing from `{python} ResNet50Int8Metrics.calibration_error_fp32_str` to `{python} ResNet50Int8Metrics.calibration_error_int8_str`, and INT8 quantization providing `{python} ResNet50Int8Metrics.latency_speedup_str`$\times$ speedup on GPU but only `{python} ResNet50Int8Metrics.cpu_speedup_str`$\times$ on CPU, demonstrating hardware-dependent gains.
...
Tool choice should follow the layer that owns the transformation. Framework APIs are useful while the model is still being trained, calibrated, or pruned; compiler and runtime tools become necessary when the optimized graph must match a specific accelerator. Across those layers, software infrastructure provides prebuilt modules for common optimization techniques, assists with hyperparameter tuning (pruning schedules, quantization bit-widths), manages accuracy-compression trade-offs through automated evaluation, and supports hardware compatibility through device-specific code generation.
...
At the model-development layer, framework APIs are most useful for transformations that need access to training state, calibration data, or weight tensors before export. TensorFlow, PyTorch, and MXNet provide comprehensive APIs that apply optimization techniques without requiring practitioners to implement complex algorithms from scratch. @Sec-ml-frameworks examines these frameworks in depth. Built-in optimizations enhance model efficiency while keeping the workflow reproducible across teams.
...
After the model leaves the training framework, hardware-specific libraries own the final translation step: converting a pruned, quantized, or fused graph into kernels the target platform can execute efficiently. Libraries like TensorRT, Accelerated Linear Algebra (XLA), OpenVINO, and TVM perform this translation for target platforms. @Sec-hardware-acceleration examines the hardware acceleration principles underlying how these tools exploit accelerator capabilities for pruned, quantized, and architecturally optimized models.
...
The toolchain boundary is therefore clear: framework APIs create and calibrate the compressed model, while hardware optimization libraries adapt that model to the execution substrate. @Sec-benchmarking and @sec-ml-operations detail deployment strategies that build on these optimization foundations.

After transformation and export, visualization\index{Visualization!quantization error histograms}\index{Visualization!activation visualizations}\index{Visualization!sparsity heat maps} tools answer whether the optimization damaged the model, rather than only whether it shrank.
...
class FallaciesAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    int8_size_reduction_str = fmt_multiple(round(quant_speedup), commas=False, precision=0)
    expected_speedup_str = fmt_multiple(combined_expected, precision=0, commas=False)
