...
The gap between model requirements and device capabilities explains why compression is not optional for resource-constrained deployment. @Tbl-model-vs-device quantifies this mismatch using the Lighthouse models from @sec-network-architectures-lighthouse-roster-model-biographies-a763: even MobileNetV2 at INT8 precision exceeds TinyML device memory by about `{python} ModelDeviceComparison.mobilenet_int8_tiny_ratio_mult_str`.
