...
To see why this matters, study @fig-matrix-factorization. The matrix $\mathbf{M}$ can be approximated by the product of matrices $\mathbf{U}_k$ and $\mathbf{V}_k^T$. For intuition, most fully connected layers in networks are stored as a projection matrix $\mathbf{M}$, which requires $m{\times}n$ parameters to be loaded during computation. By decomposing and approximating it as the product of two lower-rank matrices, we need to store only $m \times k + k \times n$ parameters. Applying the factors directly costs $\mathcal{O}(k(m+n))$ for a vector input instead of $\mathcal{O}(mn)$ for the original dense matrix; explicitly forming the dense product $\mathbf{U}\mathbf{V}^T$ would cost $\mathcal{O}(mkn)$ and should be avoided in inference [@Denton2014].
...
1.  **Significance**: For $N$ unfused operations over an $M$-byte tensor, global-memory traffic scales as $2NM$ because each operation reads and writes global memory. Fusion reduces this to approximately $2M$ by reading the inputs once, computing the sequence locally, and writing only the final output. The savings directly reduce the $D_{\text{vol}}/\text{BW}$ term and eliminate repeated kernel launch overhead.
