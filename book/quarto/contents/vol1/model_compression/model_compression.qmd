...
To understand *why* numerics matter so deeply, move from the algorithm to silicon-level energy and data movement\index{Iron Law!quantization impact}. At that level, each bit is both a storage choice and an energy cost.
...
Quantization[^fn-quantization-shannon] affects every neural network weight and activation stored at some numerical precision: FP32 (32 bits), FP16 (16 bits), INT8 (8 bits), or lower. The bit width therefore becomes a systems parameter, not just a numerical representation.
