...
class CompressionDeploymentScale:
    ...
    mcu_ram_kb_str = fmt_memory_capacity(Platforms.Tiny.ram, unit=KB, precision=0, commas=False)
```
...
**Fix**: They partitioned feature maps across two 3 GB GTX 580 GPUs, restricting cross-GPU communication to layers 3 and 6 to satisfy memory limits ($M_{\text{peak}} \le 3\text{ GB}$). The resulting two-tower architecture achieved a winning top-5 error rate of 15.3 percent, a 10.8 percentage point margin.
...
class QuantizationMemoryAnchor:
    ...
    llm_7b_mem_gb_str = fmt_qty(llm_7b_mem_fp16, GB, precision=0, commas=False)
    smartphone_ram_gb_str = fmt_memory_capacity(smartphone_ram, unit=GB, precision=0, commas=False)
```
...
class KwsQuantImperative:
    ...
    budget_kb_str = fmt_memory_capacity(tiny_ram, unit=KB, precision=0, commas=False)
```
