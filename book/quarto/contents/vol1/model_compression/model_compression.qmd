...
class QuantizationMathCalc:
    ...
    scale_fraction_math = fmt_math(rf"\frac{{{range_str}}}{{{steps_str}}}")
...
*   **Zero-point**: $z = \text{round}(-(\alpha) / s)$ = round(-(`{python} QuantizationMathCalc.alpha_str`)/`{python} QuantizationMathCalc.scale_fraction_math`) = round(`{python} QuantizationMathCalc.zp_quotient_str`) = `{python} QuantizationMathCalc.zero_point_str`.
...
# │ Show:    An 8x paper speedup (2x pruning x 4x INT8 quantization) collapses
# │          to about 1.5x on unmodified hardware.
