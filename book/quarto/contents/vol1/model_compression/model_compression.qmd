...
1. **Integer throughput path**: Dedicated INT8 matrix units can deliver about a `{python} A100Int8Speedup.a100_int8_speedup_mult_str` peak throughput increase over the FP16 path on the reference accelerator.
...
**Mechanism**: Stage one applies structured architectural pruning: removing `{python} BertCompression.attention_prune_pct_str` of attention heads, trimming `{python} BertCompression.ffn_trim_pct_str` of intermediate FFN dimensions, and reducing depth and hidden width to a compact BERT variant. The full structural recipe yields a `{python} BertCompression.parameter_reduction_pct_str` parameter reduction, with accuracy dropping from `{python} BertCompression.baseline_accuracy_pct_str` to `{python} BertCompression.pruned_accuracy_pct_str`. Stage two uses knowledge distillation from the original teacher to recover accuracy to `{python} BertCompression.distilled_accuracy_pct_str`. Stage three applies quantization-aware training with INT8 quantization, achieving `{python} BertCompression.quant_memory_reduction_mult_str` additional memory reduction with final accuracy of `{python} BertCompression.final_accuracy_pct_str`.

**Systems insight**: The combined impact is `{python} BertCompression.compression_ratio_mult_str` memory reduction (`{python} BertCompression.full_mb_str` to `{python} BertCompression.compressed_mb_str`), `{python} BertCompression.mobile_speedup_mult_str` inference speedup on mobile CPU, and only `{python} BertCompression.final_accuracy_loss_pct_str` final accuracy loss vs. `{python} BertCompression.wrong_order_loss_pct_str` if quantization had been applied before pruning. The ordering of compression techniques changes the final deployment trade-off.
