...
This inversion of priorities motivates a precise definition of the discipline this chapter addresses.
...
Generative models produce a stream of tokens rather than a single output tensor. This streaming nature requires a two-part metric system that reflects the internal state transition from "prefill" (processing input) to "decode" (generating output). The two key measures are *Time to First Token (TTFT)* and *Time Per Output Token (TPOT)*, which capture responsiveness and fluidity respectively.
