...
2.  **Memory Locking (`mlock`)**\index{Memory Locking!mlock}: Instructing the OS to lock the model weights and KV caches in physical RAM. This prevents the system from "paging out" model data to slow disk storage during periods of high memory pressure, ensuring consistent microsecond-scale access times.
...
s1_max_batch_value = 70 # Theoretical: floor((50 - 2.5 - 5) / 0.6) = 70
