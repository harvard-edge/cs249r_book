...
# Model Serving Systems {#sec-serving}
...
### The Scheduler: Where Throughput Meets Latency {#sec-serving-scheduler}
...
## The Request Lifecycle {#sec-serving-request-lifecycle}
...
#### The Killer Microseconds Problem {#sec-serving-killer-microseconds}
...
#### Deployment-Specific Resolution Decisions {#sec-serving-resolution-deployment}
...
#### Adaptive Resolution {#sec-serving-adaptive-resolution}
...
### The Systems Metric: Hardware Duty Cycle {#sec-serving-duty-cycle}
...
#### Output Formatting {#sec-serving-output-format}
...
## Queuing Theory and Tail Latency {#sec-serving-queuing-theory}
...
#### The Tail at Scale Problem {#sec-serving-tail-at-scale}
...
#### Hedged Requests {#sec-serving-hedged-requests}
...
#### Tied Requests {#sec-serving-tied-requests}
...
#### Canary Requests {#sec-serving-canary-requests}
...
#### Graceful Degradation {#sec-serving-graceful-degradation}
...
#### Admission Control {#sec-serving-admission-control}
...
#### Multi-Stream Execution {#sec-serving-multi-stream}
...
#### Model Swapping and Host Memory {#sec-serving-model-swapping}
...
::: {.callout-perspective title="Back-of-the-Napkin: The Batching Sweet Spot"}
...
#### Queue Waiting Time Analysis {#sec-serving-queue-wait}
...
#### Batch Size Distribution {#sec-serving-batch-distribution}
...
#### Throughput Maximization Strategy {#sec-serving-throughput-max}
...
#### Latency-Constrained Optimization {#sec-serving-latency-constrained}
...
#### SLO Violation Analysis {#sec-serving-slo-violations}
...
#### Adaptive Batching Windows {#sec-serving-adaptive-batching}
...
#### Practical Configuration Guidelines {#sec-serving-config-guidelines}
...
#### Server Traffic (Poisson Arrivals) {#sec-serving-poisson-traffic}
...
#### Streaming Traffic (Correlated Arrivals) {#sec-serving-streaming-traffic}
...
#### Single-User Traffic (Sequential Arrivals) {#sec-serving-single-user}
...
#### Mobile Serving Constraints {#sec-serving-mobile-constraints}
...
3. **Memory Pressure**: Mobile apps compete for limited shared memory. A 150MB model footprint is acceptable for a dedicated camera app but problematic when the model must coexist with other app components. Memory-mapped model loading (@sec-serving-loading-strategies) helps by loading pages on demand rather than requiring the full model in memory.
...
### Decoding Strategies {#sec-serving-decoding}
...
### Streaming Responses {#sec-serving-streaming}
...
### Memory and KV Cache {#sec-serving-memory-considerations}
...
The continuous batching and PagedAttention techniques covered in @sec-serving-continuous-batching address these challenges. Advanced techniques including prefix caching and speculative decoding are covered in **Volume II** (@sec-optimization-kv-cache).
...
#### Precision-Throughput Relationship {#sec-serving-precision-throughput}
...
#### Layer Sensitivity {#sec-serving-layer-sensitivity}
...
#### Calibration Requirements {#sec-serving-calibration}
...
#### Dynamic Precision Selection {#sec-serving-dynamic-precision}
...
#### Scaling Responsiveness {#sec-serving-scaling-responsiveness}
...
The serving foundations established here provide the infrastructure for operational deployment. We have optimized the single node for milliseconds. But a single node is fragile. What happens when the server crashes? When the model drifts? When we need to update the code without dropping a single request? In **Ops** (@sec-ml-operations), we scale our perspective from the single request to the system lifecycle, building the automated machinery that keeps the factory running 24/7.
