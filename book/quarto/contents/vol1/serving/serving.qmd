...
# │ How: List constants for FLOPS, BW, and TDP from mlsys.constants.
# │
...
# │ Show: That a 10x traffic spike causes system collapse, not just 10x latency.
...
*   Total tokens per hour: `{python} concurrent_batch_str` batch × (SEC_PER_HOUR s/hr / `{python} req_time_s_str` s/req) × `{python} tokens_per_req_str` tokens/req ≈ `{python} tokens_per_hour_m_str` million tokens/hour.
