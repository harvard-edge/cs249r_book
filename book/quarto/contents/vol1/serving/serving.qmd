...
If **Benchmarking** (@sec-benchmarking-ai) is the sensor of our deployment control loop, then **Serving** is the actuator—the high-performance execution of the Silicon Contract in the face of unpredictable user demand. In this phase, the **Iron Law** moves from the laboratory into the wild, where we must minimize **Overhead** while balancing the needs of individual users against the economics of throughput.
...
With the queuing fundamentals established, we turn to the prerequisites that queuing analysis assumes: models that are loaded, initialized, and producing accurate predictions. Two assumptions underlie the analysis from previous sections: first, that models are ready to process requests, and second, that the predictions they produce match what was validated during development. Production systems regularly violate both assumptions. Cold start latency can exceed inference time by orders of magnitude during scaling events. Subtle preprocessing differences between training and serving pipelines cause accuracy degradation that no amount of queuing optimization can address. This section examines the challenges that threaten these foundational assumptions.

### Training-Serving Skew {#sec-serving-skew}

The queuing theory and latency analysis from previous sections assume the model produces correct predictions. However, a critical problem in production ML systems threatens this assumption: the model that performed well during validation may silently degrade when deployed.
...
However, batching forces requests to wait...
...
The traffic patterns and batching strategies examined in the previous section assume models produce a single output per request. Large language models break this assumption: they generate tokens incrementally over hundreds or thousands of iterations, creating a fundamentally different latency profile. The p50, p95, and p99 metrics that govern classification serving apply differently when a single request takes 2 to 3 seconds to complete but must feel responsive throughout. This section examines how LLM serving adapts the foundational principles to streaming generation.

Unlike discriminative models that produce a single output tensor, large language models introduce serving challenges distinct from the classification and detection models examined earlier in this chapter. While the foundational principles of queuing theory, batching tradeoffs, and latency budgets apply universally, LLMs require additional metrics, different optimization strategies, and unique memory management techniques.

### Performance Metrics: TTFT and TPOT {#sec-serving-llm-metrics}

Unlike discriminative models that produce a single output tensor, generative models produce a stream of tokens. This streaming nature requires a two-part metric system that reflects the internal state transition from "prefill" (processing input) to "decode" (generating output).
