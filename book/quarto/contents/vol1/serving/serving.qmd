...
Memory management adds complexity to continuous batching. As sequences enter and exit the batch, the key-value cache that stores attention context must be dynamically allocated and freed. Without careful management, this dynamic allocation causes *memory fragmentation*: small unusable gaps accumulate between allocated regions, eventually preventing new sequences from starting even when total free memory appears sufficient.
...
Large language models introduce serving challenges distinct from the classification and detection models examined throughout this chapter. While the foundational principles—queuing theory, batching tradeoffs, latency budgets—apply universally, LLMs require additional metrics, different optimization strategies, and unique memory management techniques.
