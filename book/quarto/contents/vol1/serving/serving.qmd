...
Training and serving want opposite things. Training maximizes throughput by processing large batches over long durations, tolerating latency spikes that disappear into hour-long epochs. Serving minimizes latency by responding to individual requests within milliseconds, where a single slow response means a frustrated user. Training runs on expensive hardware for weeks, amortizing fixed costs across billions of examples. Serving runs continuously, where cost scales directly with traffic and inefficiency compounds into operational expense. Training can checkpoint and restart; serving must never fail visibly. This inversion explains why models that train beautifully often serve poorly: the batch sizes that maximized GPU utilization during training create unacceptable latency during serving; the checkpointing that provided safety during training becomes overhead during serving; the focus on convergence metrics during training ignored the preprocessing bottlenecks that dominate serving latency. Serving is where training's success meets production's constraints, and the skills that optimize one actively harm the other.
