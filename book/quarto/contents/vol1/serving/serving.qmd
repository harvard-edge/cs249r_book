...
The following plot visualizes this **Tail Latency Explosion**. It demonstrates why production systems must run at relatively low utilization (40-60%) to guarantee stable tail latency (p99).
...
**Domain-Specific Architectures (DSAs)**, like Google’s TPU or NVIDIA’s Tensor Cores, solve this by replacing complex instruction logic with massive arrays of simple Multiply-Accumulate (MAC) units. This specialization allows a DSA to achieve 10–100$\times$ higher **Arithmetic Intensity** (@principle-arithmetic-intensity) (the ratio of compute to memory access) even at the small batch sizes required for low-latency serving. Understanding this architectural advantage explains why hardware acceleration is not just a speedup, but a requirement for economically viable serving.
...
**Fallacy:** ***Faster model inference automatically means faster end-to-end serving.***

Engineers assume model inference dominates serving latency. In production, preprocessing and postprocessing often consume 45 to 70 percent of total request time when inference runs on optimized accelerators, as **the latency budget** (@principle-latency-budget) demonstrates. A team reducing inference from 5ms to 2ms achieves only 23 percent end-to-end improvement if preprocessing remains at 8ms. **Amdahl's Law** (@principle-amdahls-law) formalizes this: if preprocessing consumes 50 percent of latency, even infinitely fast inference yields at most 2× speedup. Teams that optimize inference without profiling the complete request path waste engineering effort while the actual bottleneck remains unaddressed.
...
**Fallacy:** ***Larger batch sizes always improve throughput.***
