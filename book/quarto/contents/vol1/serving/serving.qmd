...
#| echo: false
#| fig-cap: "**The Tail Latency Explosion.** Normalized Latency vs. System Utilization ($\\rho$). The Median Latency (Blue, p50) remains stable until ~80% utilization. However, the Tail Latency (Red, p99) explodes exponentially much earlier, passing the 'Knee' around 70%. This physics dictates why we cannot run production clusters at 100% utilization: the cost of efficiency is instability."
...
#| echo: false
#| fig-cap: "**Intelligence Deflation.** Cost per 1M tokens (USD) over time (Log Scale). The cost of token generation has collapsed by multiple orders of magnitude since the release of GPT-3. This trend—driven by algorithmic efficiency, hardware specialization, and intense competition—transforms the feasibility of deploying large-scale AI for automated workflows."
#| fig-alt: "Line plot showing token pricing collapsing from $20/M tokens in 2020 to <$0.20/M tokens in 2024. Log scale highlights the deflationary trend."
...
Managing this budget requires understanding where time actually goes in production systems. Before examining the request lifecycle in detail, verify your intuition about serving constraints.

::: {.callout-checkpoint title="Inference Constraints" collapse="false"}
...
+:===================+:===========================+===========:+===============:+
| **Input Parsing**  | Request parsing            | 0.5ms      | CPU            |
+--------------------+----------------------------+------------+----------------+
| **Embedding Look** | **Fetch 100+ dense vectors**| **6.0ms**  | **Memory BW**  |
+--------------------+----------------------------+------------+----------------+
| **Inference**      | MLP forward pass           | 1.5ms      | Compute        |
+--------------------+----------------------------+------------+----------------+
| **Postprocessing** | Ranking & Filtering        | 1.0ms      | CPU            |
+--------------------+----------------------------+------------+----------------+
| **Total**          |                            | **9.0ms**  |                |
+--------------------+----------------------------+------------+----------------+
...
Barroso, Patterson, and colleagues identified a critical gap in how systems handle latency at different time scales [@barroso2017attack]. Modern systems efficiently handle nanosecond-scale events (CPU cache access, DRAM reads) through hardware mechanisms like out-of-order execution, and millisecond-scale events (disk I/O, network calls) through software techniques like threading and asynchronous I/O. Microsecond-scale events, however, fall into an uncomfortable middle ground where neither approach works well.
...
**The Systems Conclusion**: By accepting a **`{python} latency_increase_str`x increase in latency** (`{python} f"{batch1_ms:.0f}"`ms $\rightarrow$ `{python} batch8_user_latency_str`ms), you have achieved nearly **`{python} latency_increase_str`x higher throughput** on the same hardware. As long as `{python} batch8_user_latency_str`ms is under your 20ms budget, this is "free" capacity. This trade-off is the fundamental lever of serving economics.
:::
...
Dynamic batching introduces a fundamental tension between throughput optimization and latency constraints. Understanding this tradeoff quantitatively enables systematic configuration decisions rather than trial-and-error tuning. To begin, consider why latency spikes occur at all:
...
***Time to First Token (TTFT)*** measures latency from request to first output token, governed by the compute-bound **Prefill Phase** (processing the full prompt). **Time Per Output Token (TPOT)** measures latency of each subsequent token, governed by the memory-bandwidth-bound **Decode Phase** (autoregressive KV cache lookups). This decomposition isolates the distinct hardware bottlenecks (**Compute** versus **Memory Bandwidth**), enabling targeted optimization of each phase.
...
*   $T_{\text{token}} \approx \frac{`{python} model_weight_gb` \text{ GB}}{`{python} h100_bw_tbs` \text{ TB/s}} \approx `{python} token_time_theoretical_ms` \text{ ms}$ (theoretical limit).
...
*   Total decode time = $`{python} decode_tokens` \text{ tokens} \times `{python} realized_tpot_ms` \text{ ms/token} = `{python} total_decode_s` \text{ seconds}$.
...
With 4-bit weights occupying `{python} model_weight_gb` GB, the remaining ~`{python} int(80 - model_weight_gb)` GB of VRAM is available for the **KV Cache**. Using **PagedAttention**, we can allocate this memory with near-zero fragmentation.

*   Each token requires approximately `{python} kv_per_token_mb` MB of KV cache (32 layers $\times$ 4096 dim $\times$ 2 vectors $\times$ 2-byte precision).
*   Total cache capacity $\approx \frac{`{python} kv_cache_gb` \text{ GB}}{`{python} kv_per_token_mb` \text{ MB/token}} \approx `{python} kv_capacity_tokens_str` \text{ tokens}$.
*   At `{python} f"{tokens_per_req:,}"` tokens per request (input + output), the GPU can handle a **concurrent batch size of ~`{python} concurrent_batch` requests**.

#### Unit Economics {#sec-model-serving-systems-unit-economics-b685}

For an H100 SXM5 instance at approximately \$`{python} f"{hourly_cost:.2f}"` per hour (specialized cloud providers; hyperscaler rates vary from \$2-13 per hour as of 2024):
*   Total tokens per hour = $`{python} concurrent_batch` \text{ batch} \times \frac{3600 \text{ s/hr}}{`{python} req_time_s_str` \text{ s/req}} \times `{python} f"{tokens_per_req:,}"` \text{ tokens/req} \approx `{python} tokens_per_hour_m` \text{ million tokens/hour}$.
*   **Cost per million tokens**: $\frac{\$`{python} f"{hourly_cost:.2f}"`}{`{python} tokens_per_hour_m`} \approx \mathbf{\$`{python} cost_per_m_tokens`}$.
