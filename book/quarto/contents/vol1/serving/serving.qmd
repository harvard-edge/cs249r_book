...
\noindent
![](images/png/serving.png){width=80%}
...
- Explain why serving optimization differs fundamentally from training optimization, focusing on latency constraints and request-level thinking

- Identify training-serving skew in preprocessing pipelines and implement strategies to maintain parity between training and production environments

- Apply appropriate batching strategies for different model types and latency requirements, understanding the fundamental tradeoff between throughput and individual request latency

- Implement postprocessing pipelines including decoding strategies for generative models and output formatting for production APIs

- Evaluate model loading strategies and memory management techniques for production serving systems

- Select appropriate serving runtimes and frameworks based on deployment constraints and performance requirements
...
This inversion of priorities has profound consequences. The benchmarking techniques from @sec-benchmarking-perf-analysis now target percentile latencies rather than aggregate throughput. The quantization methods from @sec-model-optimizations must be validated not just for accuracy preservation but for calibration with production traffic. The hardware acceleration from @sec-ai-acceleration must be configured for single-request responsiveness rather than batch throughput. Understanding these connections enables practitioners to apply earlier optimizations correctly in the serving context.

The serving systems examined here focus on single-machine deployment, establishing the foundational principles that govern all inference systems. These foundations prepare practitioners for distributed serving patterns, including load balancing across nodes and model sharding for models exceeding single-machine memory, covered in @sec-inference-at-scale.
...
Input resolution dramatically affects both preprocessing and inference latency, but the relationship differs depending on whether the system is compute-bound or memory-bound. Understanding this distinction (covered in depth in @sec-hw-acceleration) is essential for making informed resolution decisions.

For compute-bound models, throughput scales inversely with resolution squared:

$$\frac{T(r_2)}{T(r_1)} = \left(\frac{r_1}{r_2}\right)^2$$ {#eq-resolution-throughput}

Doubling resolution from 224 to 448 theoretically yields 4× slowdown (measured: 3.6× due to fixed overhead amortization). However, at high resolutions, models transition from compute-bound to memory-bound as activation tensors exceed cache capacity:
...
The most fundamental result in queuing theory is Little's Law, which relates three quantities in any stable system:
...
Sixty-three percent of user requests experience the slow tail. At 2,000 servers (common for large-scale services), even a 1-in-10,000 slow response rate means 18% of user requests hit the tail. This fan-out amplification explains why even rare performance hiccups become user-visible problems at scale, and why tail-tolerant techniques become essential for distributed inference systems covered in @sec-inference-at-scale.
...
The queuing analysis explains the capacity planning approach mentioned in @sec-serving-capacity-planning and connects to the MLPerf Server scenario from @sec-benchmarking, which measures throughput only for requests meeting the latency SLO. A system achieving 10,000 QPS but violating the SLO on 5% of requests reports only 9,500 valid QPS.
...
The optimal batching window balances waiting cost against throughput benefit:

$$T_{\text{optimal}} = \min\left(L - S, \sqrt{\frac{S}{\lambda}}\right)$$ {#eq-optimal-window}

where $L$ is the latency SLO and $S$ is the service time. A counterintuitive result: as traffic increases, the optimal window decreases while achieved batch sizes still grow.
...
In practice, GPU compute pipelines and Tensor Core alignment requirements limit achieved speedup to 2.5-3.5× for INT8 versus FP32. Tensor Cores require specific alignment: INT8 operations need tensor dimensions divisible by 16, while FP16 requires divisibility by 8 (see @sec-hw-acceleration for Tensor Core architecture details).
...
Not all layers tolerate reduced precision equally. Quantization error for a layer scales with weight magnitude and gradient sensitivity:
...
The traffic pattern analysis reveals how deployment context fundamentally shapes batching strategy and system design. Server workloads with Poisson arrivals optimize dynamic batching windows, autonomous vehicles with streaming sensor data require synchronized batch formation, and mobile applications with single-user patterns eliminate batching entirely. The MLPerf scenarios codify these patterns for standardized benchmarking, connecting the serving principles established here to the measurement frameworks explored in @sec-benchmarking. Precision selection and runtime optimization extend the quantization techniques from @sec-model-optimizations and Tensor Core capabilities from @sec-hw-acceleration into the serving domain, where calibration with representative production traffic becomes essential.
