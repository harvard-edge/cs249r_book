...
# PURPOSE
# =============================================================================
# Purpose: Chapter-wide serving constants and carbon cost calculations
# Used in: Multiple serving tables, callouts, and narrative sections
...
# Carbon cost of chat calculations
cc_concurrent_req_value = 114
cc_tokens_per_sec_req_value = 7.5
cc_total_tokens_sec_value = cc_concurrent_req_value * cc_tokens_per_sec_req_value
cc_host_overhead_w_value = 300
...
cc_response_tokens_value = 500
cc_response_joules_value = cc_joules_per_token_value * cc_response_tokens_value
cc_smartphone_joules_value = 40000
cc_boiling_water_joules_value = 100000
cc_low_util_pct_value = 10
cc_idle_power_w_value = 300
cc_low_util_joules_value = 10

# =============================================================================
# OUTPUT
# =============================================================================
v100_tflops_fp32 = f"{v100_tflops_fp32_value:.1f}"
v100_bw = f"{v100_bw_value:.0f}"
a100_bw_tbs = f"{a100_bw_tbs_value:.1f}"
h100_bw_tbs = f"{h100_bw_tbs_value:.2f}"
h100_mem = f"{h100_mem_value:.0f}"
h100_tdp = f"{h100_tdp_value:.0f}"

cc_concurrent_str = str(cc_concurrent_req_value)
cc_tokens_req_str = str(cc_tokens_per_sec_req_value)
cc_total_tokens_str = f"{cc_total_tokens_sec_value:.0f}"
cc_host_overhead_str = str(cc_host_overhead_w_value)
cc_total_power_str = f"{cc_total_power_w_value:.0f}"
cc_joules_token_str = f"{cc_joules_per_token_value:.2f}"
cc_response_tokens_str = str(cc_response_tokens_value)
cc_response_joules_str = f"{cc_response_joules_value:.0f}"
cc_smartphone_str = f"{cc_smartphone_joules_value:,}"
cc_boiling_str = f"{cc_boiling_water_joules_value:,}"
cc_low_util_str = str(cc_low_util_pct_value)
cc_idle_power_str = str(cc_idle_power_w_value)
cc_low_util_joules_str = str(cc_low_util_joules_value)
```

## The Serving Paradigm {#sec-model-serving-systems-serving-paradigm-9634}

Serving\index{Serving!production deployment}\index{Model Serving!paradigm shift} marks the transition from model development to production deployment, where the **Iron Law of ML Systems** (@sec-silicon-contract) undergoes a fundamental shift. Training (@sec-ai-training) optimized for throughput, processing massive datasets over days where the goal was samples per hour and latency spikes were absorbed invisibly. Serving must deliver predictions within milliseconds under unpredictable load. The **Latency term**\index{Latency!serving constraint} ($L_{lat}$), representing the irreducible overhead of request scheduling, network round-trips, and system orchestration, becomes the dominant constraint rather than a rounding error.

This inversion (from throughput to *latency*, from controlled conditions to *unpredictable traffic*, from offline to *real time*) defines the serving challenge. Prior chapters established techniques that must now be revalidated: @sec-benchmarking-ai measured performance under controlled conditions, but serving faces traffic patterns that no benchmark could anticipate; @sec-model-compression provided quantization methods that reduced model size, but serving must confirm those optimizations preserve accuracy under real traffic distributions. This fundamental shift in constraints creates what we call the *serving inversion*\index{Serving Inversion!throughput to latency}.
...
As shown in @fig-tail-latency-explosion, this physics dictates *why* production systems must run at relatively low utilization (40-60%) to guarantee stable tail latency\index{Tail Latency!utilization threshold} (p99).
...
# PURPOSE
# =============================================================================
# Purpose: Plot tail latency explosion vs utilization
# Used in: Figure "The Tail Latency Explosion"
...
# Ensure physx directory is in path
sys.path.append(os.path.abspath("physx"))
...
# PURPOSE
# =============================================================================
# Purpose: Plot token cost deflation over time
# Used in: Figure "Intelligence Deflation"
...
Serving systems must execute a complete inference pipeline under latency constraints, not just the neural network computation. A common misconception is that "inference time" equals "serving time"—in reality, the neural network is just one stage in a longer pipeline. @fig-serving-inference-pipeline illustrates this pipeline: raw inputs flow through preprocessing (traditional computing), neural network inference (deep learning), and postprocessing (traditional computing) before producing final outputs. Each stage contributes to total latency, and bottlenecks can occur anywhere in the pipeline. As we will see in @sec-model-serving-systems-latency-budget-ef40, preprocessing often dominates total latency when inference runs on optimized hardware.
...
The preceding examples explain *why* serving systems must maintain capacity headroom. But before diving into *how* to optimize inference latency, we must address a more fundamental question: *when* should predictions be computed at all? The first architectural decision in any serving system is whether predictions happen before or during user requests [@google2024staticdynamic]. This choice shapes system design, cost structure, and capability boundaries.
...
# PURPOSE
# =============================================================================
# Purpose: Compute static batch inference time for ResNet-50 example
# Used in: Static vs Dynamic inference narrative

# =============================================================================
# INPUT
# =============================================================================
n_photos_value = 10_000
inference_ms_value = 5
...
# OUTPUT
# =============================================================================
n_photos_str = f"{n_photos_value:,}"
inference_ms_str = f"{inference_ms_value}"
batch_total_s_str = f"{batch_total_s_value:.0f}"
```
...
# PURPOSE
# =============================================================================
# Purpose: Compare serving cost per million queries for latency tradeoff
# Used in: Callout "The Cost of Latency"

# =============================================================================
# INPUT
# =============================================================================
gpu_cost_per_hour_value = 4.0  # $/hour

# Scenario A: batch size 1
latency_a_ms_value = 5
throughput_a_rps_value = 200

# Scenario B: batch size 8
latency_b_ms_value = 10
throughput_b_rps_value = 800
...
# OUTPUT
# =============================================================================
gpu_cost_per_hour_str = f"{gpu_cost_per_hour_value:.0f}"
...
cost_a_str = f"{cost_per_million_a_value:.2f}"
cost_b_str = f"{cost_per_million_b_value:.2f}"
cost_increase_str = f"{cost_increase_pct_value:.0f}"
cost_ratio_str = f"{cost_ratio_value:.0f}"
```
...
The static-versus-dynamic decision is the first of several architectural choices that shape serving system design. Equally important is *where* the model executes, since deployment context fundamentally constrains every subsequent optimization.
...
**1. Networked Serving (Cloud/Datacenter)**\index{Serving!cloud/datacenter}\index{Microservice!model serving}
...
**2. Application-Embedded Serving (Mobile/Edge)**\index{Serving!mobile/edge}\index{Edge Inference!embedded serving}
...
**3. Bare-Metal Serving (TinyML)**\index{Serving!TinyML}\index{TinyML!bare-metal serving}
...
**The Request Pipeline.** Every request traverses a multi-stage pipeline designed to maximize hardware throughput while minimizing latency overhead, as @fig-server-anatomy illustrates.
...
# PURPOSE
# =============================================================================
# Purpose: Compare JSON vs Protobuf serialization overhead
# Used in: Callout "JSON vs Protobuf Serialization"

# =============================================================================
# INPUT
# =============================================================================
serial_floats_value = 1000
json_size_kb_value = 9
json_parse_us_value = 50
protobuf_size_kb_value = 4
protobuf_parse_us_value = 5
requests_per_sec_value = 10000
...
# OUTPUT
# =============================================================================
serial_floats_str = f"{serial_floats_value:,}"
json_size_str = str(json_size_kb_value)
json_parse_str = str(json_parse_us_value)
protobuf_size_str = str(protobuf_size_kb_value)
protobuf_parse_str = str(protobuf_parse_us_value)
requests_per_sec_str = f"{requests_per_sec_value:,}"
efficiency_gain_str = str(efficiency_gain_value)
```
...
***Latency Budget***\index{Latency Budget!SLA constraints}\index{SLA (Service Level Agreement)!latency budget} is the **Time Capital** allocated to a request, strictly bounded by the **End-to-End SLA**\index{End-to-End SLA|see{SLA}}. It acts as a zero-sum constraint system where any milliseconds consumed by **Serialization** or **Network Overhead** directly reduce the computational budget available for Model Inference.
...
# PURPOSE
# =============================================================================
# Purpose: Build latency budget breakdown table for ResNet-50 serving
# Used in: Callout "ResNet-50: Latency Budget Breakdown"
...
# =============================================================================
l_total_value = (
    l_jpeg_value + l_resize_value + l_norm_value + l_transfer_value + l_inf_value + l_post_value
)
...
# PURPOSE
# =============================================================================
# Purpose: Quantify preprocessing share of latency before and after TensorRT
# Used in: Latency distribution analysis narrative

# =============================================================================
# INPUT
# =============================================================================
jpeg_decode_ms_value = 3.0
resize_ms_value = 1.0
normalize_ms_value = 0.5
cpu_gpu_ms_value = 0.5
resnet_inference_ms_value = 5.0
postprocess_ms_value = 0.1
tensorrt_inference_ms_value = 2.0

# =============================================================================
# PROCESS
# =============================================================================
preprocess_ms_value = jpeg_decode_ms_value + resize_ms_value + normalize_ms_value
total_latency_ms_value = (
    preprocess_ms_value + cpu_gpu_ms_value + resnet_inference_ms_value + postprocess_ms_value
)
preprocess_pct_value = preprocess_ms_value / total_latency_ms_value * 100

tensorrt_total_ms_value = (
    preprocess_ms_value + cpu_gpu_ms_value + tensorrt_inference_ms_value + postprocess_ms_value
)
...
# OUTPUT
# =============================================================================
preprocess_ms_str = f"{preprocess_ms_value:.1f}"
cpu_gpu_ms_str = f"{cpu_gpu_ms_value:.1f}"
resnet_inference_ms_str = f"{resnet_inference_ms_value:.0f}"
tensorrt_inference_ms_str = f"{tensorrt_inference_ms_value:.0f}"
model_10x_ms_str = f"{resnet_inference_ms_value / 10:.1f}"
total_latency_str = f"{total_latency_ms_value:.1f}"
preprocess_pct_str = f"{preprocess_pct_value:.0f}"
tensorrt_preprocess_pct_str = f"{tensorrt_preprocess_pct_value:.0f}"
```
...
# PURPOSE
# =============================================================================
# Purpose: DLRM serving latency breakdown for IO-bound example
# Used in: Lighthouse example "DLRM Serving"

# =============================================================================
# INPUT
# =============================================================================
dlrm_input_ms_value = 0.5
dlrm_embed_ms_value = 6.0
dlrm_mlp_ms_value = 1.5
dlrm_post_ms_value = 1.0

# =============================================================================
# PROCESS
# =============================================================================
dlrm_total_ms_value = (
    dlrm_input_ms_value + dlrm_embed_ms_value + dlrm_mlp_ms_value + dlrm_post_ms_value
)
...
# PURPOSE
# =============================================================================
# Purpose: Quantify Amdahl's Law impact on end-to-end serving latency
# Used in: Callout "The Quantitative Approach to Serving"
...
Moving preprocessing to GPU\index{GPU Preprocessing!NVIDIA DALI} can reduce total latency by 6x in some pipelines by eliminating CPU-GPU data transfers between stages [@nvidia2024tritontutorial]. Effective optimization targets the largest time consumers first.
...
# PURPOSE
# =============================================================================
# Purpose: Compute theoretical resolution scaling slowdown
# Used in: Resolution and input size tradeoffs narrative
...
r1_str = f"{r1_value}"
r2_str = f"{r2_value}"
theoretical_str = f"{theoretical_slowdown_value:.0f}"
...
**Overlapping I/O and Compute.**\index{I/O Overlap!compute pipelining} @fig-serving-pipeline-timing contrasts serial execution with pipelined execution. In the serial case (A), each request must complete its entire lifecycle (Network $\rightarrow$ CPU Preprocessing $\rightarrow$ GPU Inference $\rightarrow$ Postprocessing) before the next request begins. Even with a fast GPU, the system throughput is limited by the slowest stage, and the GPU remains idle for more than 50% of the time.
...
Classification models output logits\index{Logits!classification output} or probabilities across classes. Converting these to predictions involves several potential steps including argmax selection\index{Argmax!prediction selection} that chooses the highest-probability class, thresholding that applies confidence thresholds before returning predictions, top-k extraction that returns multiple high-probability classes with scores, and calibration that adjusts raw probabilities to better reflect true likelihoods.
...
The request lifecycle analysis explains *where* time goes within a single request. Production systems, however, rarely process requests in isolation. They must handle many concurrent requests competing for finite resources. When requests arrive faster than they can be processed, queues form. Understanding *how* these queues behave explains *why* latency degrades under load and enables principled capacity planning. Queuing theory\index{Queuing Theory!serving systems} provides the mathematical framework for these predictions. These principles apply to web servers and ML inference alike, and explain the counterintuitive behavior that causes well-provisioned systems to violate latency SLOs when load increases modestly.
...
# PURPOSE
# =============================================================================
# Purpose: Apply Little's Law to compute concurrency from throughput and latency
# Used in: Callout "Little's Law"

# =============================================================================
# INPUT
# =============================================================================
littles_lambda_value = 1000
littles_w_value = 0.05
...
# OUTPUT
# =============================================================================
littles_lambda_str = f"{littles_lambda_value:,}"
littles_w_ms_str = str(littles_w_ms_value)
littles_w_str = str(littles_w_value)
littles_l_str = f"{littles_l_value:.0f}"
```
...
*   **Latency Target (W)**: `{python} littles_w_ms_str` ms (`{python} littles_w_str` s).
...
For traffic exceeding single-node capacity, production systems deploy multiple replicas behind a load balancer. The M/M/c queuing model\index{M/M/c Queue!multi-server} extends M/M/1 to c parallel servers, showing that multiple replicas\index{Replica!tail latency improvement} dramatically improve tail latency: the probability of all servers being simultaneously slow drops exponentially with server count. At c=4 replicas, p99 latency can be 3× lower than the single-server case at the same total throughput.
...
# PURPOSE
# =============================================================================
# Purpose: Capacity planning calculation for ResNet-50 serving
# Used in: Callout "ResNet-50 Capacity Planning"
...
cp_rho_safe_value = 0.72
cp_v100_throughput_value = 1143
cp_headroom_value = 1.3

cp_fp32_bits_value = 32
cp_int8_bits_value = 8
...
cp_util_after_fail_value = (
    (cp_peak_qps_value / cp_v100_throughput_value) / cp_gpus_after_fail_value * 100
)
...
# OUTPUT
# =============================================================================
cp_mu_required_str = f"{cp_mu_required_value:.0f}"
cp_gpus_raw_str = f"{cp_gpus_raw_value:.1f}"
cp_gpus_ceil_str = f"{cp_gpus_ceil_value}"
cp_final_raw_str = f"{cp_final_raw_value:.1f}"
cp_final_ceil_str = f"{cp_final_ceil_value}"
cp_gpus_after_fail_str = f"{cp_gpus_after_fail_value}"
cp_util_after_fail_str = f"{cp_util_after_fail_value:.1f}"

cp_peak_qps_str = f"{cp_peak_qps_value:,}"
cp_v100_throughput_str = f"{cp_v100_throughput_value:,}"
cp_rho_safe_str = f"{cp_rho_safe_value:.2f}"
...
This yields rho ≤ `{python} cp_rho_safe_str` (72% maximum utilization).
...
**Retry Storm Prevention**\index{Retry Storm!load shedding coordination}: A subtle failure mode occurs when all replicas are overloaded simultaneously. If the load balancer retries rejected requests at other replicas that are also overloaded, retry traffic amplifies the overload. Coordinated load shedding addresses this by sharing load information across replicas, enabling system-wide decisions about which requests to accept. When global load exceeds capacity, replicas collectively reject the same fraction of requests rather than each rejecting independently and triggering retries.
...
# PURPOSE
# =============================================================================
# Purpose: Compute cold start timeline components for ResNet-50 serving
# Used in: Callout "ResNet-50: Cold Start Timeline"
...
cs_s3_value = 4.0
cs_cuda_value = 0.4
cs_compile_value = 30.0
cs_warmup_value = 0.2
cs_runtime_overhead_value = 0.4
...
cs_s3_str = "3-5s"
cs_cuda_str = "0.3-0.5s"
cs_compile_str = "15-30s"
...
**Container Image Embedding**\index{Model Caching!container embedding}: Bundle model weights directly in the container image. This produces a single deployment artifact and eliminates network fetches at startup, but creates large images (often 10-50GB) that slow container pulls and consume registry storage. Best for models that rarely update.
...
**Node-Local SSD Cache**\index{SSD Cache!model loading}: Pre-populate local SSDs on inference nodes with frequently-used models. Provides fast loading (500MB/s+ for NVMe) without network dependency, but requires cache management to handle model updates and capacity limits. Best for high-traffic models where cold start latency is critical.
...
Strategies for multi-model serving include time-multiplexing\index{Time-Multiplexing!model swapping} that loads one model at a time and swaps based on request routing, memory sharing\index{Memory Sharing!GPU multi-model} where models share GPU memory to limit concurrent execution but enable more models, and model virtualization where frameworks like Triton manage model lifecycle by loading and unloading based on traffic patterns [@nvidia2024triton]. The choice depends on request patterns. If models receive traffic evenly, concurrent loading works. If traffic is bursty and model-specific, time-multiplexing with intelligent preloading reduces average latency while maximizing GPU utilization.
...
# PURPOSE
# =============================================================================
# Purpose: Compute model swap time from PCIe bandwidth
# Used in: Model swapping and host memory discussion

# =============================================================================
# INPUT
# =============================================================================
model_size_gb_value = 10
pcie_bw_gbs_value = PCIE_GEN4_BW.to(GB / second).magnitude
...
pcie_bw_gbs_str = f"{pcie_bw_gbs_value:.0f}"
model_swap_ms_str = f"{model_swap_ms_value:.0f}"
```
...
# PURPOSE
# =============================================================================
# Purpose: Compute batching throughput and latency comparison values
# Used in: Callout "ResNet-50 Batching Efficiency"

# =============================================================================
# INPUT
# =============================================================================
batch1_throughput_value = 200
batch32_throughput_value = 1280
batch32_inference_ms_value = 25.0
batch_window_ms_value = 10.0
batch1_inference_total_ms_value = 5.0
...
# OUTPUT
# =============================================================================
throughput_ratio_str = f"{throughput_ratio_value:.1f}"
batch_window_ms_str = f"{batch_window_ms_value:.0f}"
batch32_inference_ms_str = f"{batch32_inference_ms_value:.0f}"
batch32_total_str = f"{batch32_total_ms_value:.0f}"
batch1_inference_total_ms_str = f"{batch1_inference_total_ms_value:.0f}"
```
...
# PURPOSE
# =============================================================================
# Purpose: Compute batching sweet spot metrics for ResNet-50
# Used in: Callout "The Batching Sweet Spot"

# =============================================================================
# INPUT
# =============================================================================
batch1_ms_value = 5.0
batch1_imgs_value = 200
batch8_wait_ms_value = 5.0
batch8_inference_ms_value = 9.0
...
# OUTPUT
# =============================================================================
batch1_ms_str = f"{batch1_ms_value:.0f}"
batch1_imgs_str = f"{batch1_imgs_value}"
batch8_wait_ms_str = f"{batch8_wait_ms_value:.0f}"
batch8_inference_ms_str = f"{batch8_inference_ms_value:.0f}"
batch8_user_latency_str = f"{batch8_user_latency_ms_value:.0f}"
batch8_throughput_str = f"{batch8_throughput_value:.0f}"
latency_increase_str = f"{latency_increase_value:.0f}"
```
...
**The Systems Conclusion**: By accepting a **`{python} latency_increase_str`× increase in latency** (`{python} batch1_ms_str`ms → `{python} batch8_user_latency_str`ms), you have achieved nearly **`{python} latency_increase_str`× higher throughput** on the same hardware. As long as `{python} batch8_user_latency_str`ms is under your 20ms budget, this is "free" capacity. This trade-off is the fundamental lever of serving economics.
:::
...
# PURPOSE
# =============================================================================
# Purpose: Plot throughput-latency knee for batching
# Used in: Figure "The Throughput-Latency Knee"
...
Dynamic batching introduces a fundamental tension between throughput optimization and latency constraints. Understanding this tradeoff quantitatively enables systematic configuration decisions rather than trial-and-error tuning.
...
# PURPOSE
# =============================================================================
# Purpose: Compute batching window latency budget share
# Used in: Batching window latency budget analysis

# =============================================================================
# INPUT
# =============================================================================
batch_window_ms_value = 20
slo_ms_value = 50
inference_ms_value = 5
...
# OUTPUT
# =============================================================================
avg_wait_str = f"{avg_wait_ms_value:.0f}"
budget_pct_str = f"{budget_pct_value:.0f}"
```
...
# PURPOSE
# =============================================================================
# Purpose: Analyze batching throughput and efficiency vs batch size
# Used in: Batching throughput analysis table and Iron Law callout

# =============================================================================
# INPUT
# =============================================================================
T_window_value = 10.0
fixed_overhead_ms_value = 5.0
per_image_ms_value = 0.6

batch_sizes_value = [1, 4, 8, 16, 32]
il_overhead_ms_value = 5.0
il_compute_b1_ms_value = 0.6
il_compute_b32_ms_value = 19.2
il_threshold_pct_value = 10
...
il_eff_b1_pct_value = int(
    il_compute_b1_ms_value / (il_overhead_ms_value + il_compute_b1_ms_value) * 100
)
il_eff_b32_pct_value = int(
    il_compute_b32_ms_value / (il_overhead_ms_value + il_compute_b32_ms_value) * 100
)

# =============================================================================
# OUTPUT
# =============================================================================
throughput_increase_str = f"{throughput_increase_value:.1f}"
b1_throughput_str = f"{throughputs_value[1]:.0f}"
b32_throughput_str = f"{throughputs_value[32]:.0f}"
b1_latency_str = f"{latencies_value[1]:.1f}"
b32_latency_str = f"{latencies_value[32]:.1f}"

il_overhead_str = f"{il_overhead_ms_value:.0f}"
...
il_threshold_str = str(il_threshold_pct_value)
T_window_str = f"{T_window_value:.0f}"
```
...
: **Batching Throughput Analysis**: ResNet-50 throughput on V100 with `{python} T_window_str`ms batching window. Throughput increases `{python} throughput_increase_str`x from batch size 1 to 32 (`{python} b1_throughput_str` to `{python} b32_throughput_str` img/s), but total latency more than doubles (`{python} b1_latency_str`ms to `{python} b32_latency_str`ms). The optimal configuration depends on whether the latency SLO or throughput target is the binding constraint. {#tbl-batching-throughput}
...
# =============================================================================
qps_str = f"{qps_value}"
T_slo_str = f"{T_slo_value:.0f}"
mean_wait_str = f"{mean_wait_value:.0f}"
mean_batch_str = f"{mean_batch_value:.0f}"
mean_service_str = f"{mean_service_value:.1f}"
mean_latency_str = f"{mean_latency_value:.0f}"
p99_service_str = f"{p99_service_value:.1f}"
p99_latency_str = f"{p99_latency_value:.1f}"
p99_ratio_str = f"{p99_to_mean_value:.2f}"
...
Autoregressive models\index{Autoregressive Models!token generation} like language models generate outputs token by token, creating a batching challenge that differs from single-pass models like ResNet-50. Traditional batching processes all sequences in a batch for all generation steps, wasting compute when sequences complete at different times\index{Sequence Length Variability!batch waste} [@yu2022orca]. If one sequence in a batch of 8 finishes after 10 tokens while others need 100 tokens, 87.5 percent of the compute for that sequence slot is wasted. This inefficiency matters as LLMs dominate production inference.
...
Continuous batching represents the state of the art for LLM serving, yet not all deployment scenarios benefit from batching. The sophisticated techniques examined so far (from dynamic batching windows to PagedAttention) optimize for high-throughput server workloads. These techniques introduce complexity and latency overhead that may not be justified for all deployment contexts. A fundamental question remains: *when* does batching hurt rather than help?

**When Not to Batch.**\index{Batching!when to avoid} Some scenarios require single-request processing. Ultra-low latency requirements\index{Ultra-Low Latency!no batching} where p99 latency must stay under 10ms make any batching delay unacceptable. Highly variable request sizes where inputs vary dramatically in size cause batching to create padding overhead that wastes compute. Memory constraints where models already consume most GPU memory mean batch activations may cause out-of-memory errors.
...
**Streaming Traffic (Correlated Arrivals).**\index{Streaming Traffic!sensor synchronization} Autonomous vehicles, video analytics, and robotics systems receive inputs from multiple synchronized sensors. This scenario illustrates *multi-camera autonomous vehicle serving*.
...
**Single-User Traffic (Sequential Arrivals).**\index{Single-User Traffic!mobile serving}\index{SingleStream!MLPerf scenario} Mobile and embedded applications serve one user at a time, with requests arriving only after the previous result is consumed. We can analyze these constraints in *ResNet-50 mobile serving*.
...
**Streaming Responses.**\index{Streaming Responses!LLM serving}\index{Chunked HTTP!streaming tokens} Production LLM systems return tokens as they are produced rather than waiting for complete generation. This fundamentally changes the user experience: a 2-second total generation feels responsive when tokens stream continuously, but feels broken when users stare at a blank screen for 2 seconds. Streaming requires infrastructure support for chunked HTTP responses and client-side incremental rendering. The latency profile shifts accordingly: TTFT determines when output starts appearing (responsiveness), while TPOT determines the perceived generation speed (fluidity).
...
rt_pytorch_speedup_str = f"{rt_pytorch_speedup_value:.1f}"
rt_torchscript_speedup_str = f"{rt_torchscript_speedup_value:.1f}"
rt_onnx_speedup_str = f"{rt_onnx_speedup_value:.1f}"
rt_trt_fp32_speedup_str = f"{rt_trt_fp32_speedup_value:.1f}"
rt_trt_fp16_speedup_str = f"{rt_trt_fp16_speedup_value:.1f}"
rt_trt_int8_speedup_str = f"{rt_trt_int8_speedup_value:.1f}"
```
...
**Precision-Throughput Relationship.**\index{Precision!throughput tradeoff}\index{Quantization!serving precision} For memory-bandwidth-bound operations\index{Memory-Bandwidth Bound!precision impact}, reducing precision proportionally increases throughput by reducing data movement. @eq-precision-throughput quantifies the theoretical maximum speedup from precision reduction:

ThroughputINT8 / ThroughputFP32 = 32 / 8 = `{python} cp_precision_ratio_str`× (theoretical maximum) {#eq-precision-throughput}

In practice, GPU compute pipelines and Tensor Core alignment requirements limit achieved speedup to 2.5-3.5x for INT8 versus FP32. Tensor Cores\index{Tensor Cores!alignment requirements} require specific alignment: INT8 operations need tensor dimensions divisible by 16, while FP16 requires divisibility by 8. @sec-ai-acceleration provides the detailed Tensor Core architecture that explains these alignment constraints. The *precision tradeoffs* for a standard vision model illustrate how these theoretical limits manifest in practice.
...
**Layer Sensitivity.**\index{Quantization!layer sensitivity}\index{Layer Sensitivity!precision tolerance} Not all layers tolerate reduced precision equally. @eq-quant-error captures how quantization error for a layer scales with weight magnitude and gradient sensitivity:
...
**Calibration Requirements.**\index{Calibration!INT8 quantization}\index{Post-Training Quantization!calibration}\index{Calibration Dataset!representative traffic} Post-training quantization requires a calibration dataset to determine optimal scale factors for INT8 conversion. Production experience shows that calibration data must be representative of actual serving traffic, not just training data. Using ImageNet validation images to calibrate a model serving wildlife camera images resulted in 3.2% accuracy degradation in one production system.

**Dynamic Precision Selection.**\index{Dynamic Precision!adaptive quality} Advanced serving systems select precision per request based on runtime conditions. If the system is ahead of latency SLO, it uses higher precision for better accuracy. For low-confidence INT8 results, it recomputes at FP16. Different customer tiers may receive different precision levels. This pattern enables adaptive quality-latency tradeoffs while maximizing throughput during normal operation.
...
Inference engines like TensorRT were introduced in @sec-model-serving-systems-inference-runtime-selection-5eef. How do they achieve 2-5x speedups? The answer lies in **Graph Compilation**. Training computation graphs are dynamic and mutable, whereas serving graphs are static. This static nature allows compilers to perform aggressive optimizations that would be unsafe or too slow during training.
...
**Memory Planning**\index{Memory Planning!tensor allocation}: Since the graph structure is known, the compiler can pre-calculate the exact memory offsets for every tensor. This leads to the fundamental architectural choice of *JIT vs. AOT compilation*.
...
**Safetensors**\index{Safetensors!zero-copy loading}: A modern format designed specifically for fast loading. It stores tensors as raw bytes with a minimal JSON header. This allows for **zero-copy**\index{Zero-Copy!model loading} loading: the raw bytes on disk are mapped directly into the tensor's memory buffer.
...
**The Timeline View**\index{Timeline Profiling!serving optimization}: Tools like **PyTorch Profiler**\index{PyTorch Profiler!timeline analysis} or NVIDIA **Nsight Systems (nsys)**\index{Nsight Systems!GPU profiling} generate a timeline trace. This visualization reveals the exact sequence of events on the CPU and GPU.
...
The runtime selection, precision tuning, and node-level optimizations examined in the preceding sections collectively determine the fundamental unit of serving physics\index{Serving Economics!infrastructure costs}: the performance per inference. Production deployment requires translating these technical metrics into infrastructure decisions. Serving costs\index{Serving Costs!request volume scaling} scale with request volume, unlike training costs that scale with dataset size and model complexity [@zhang2019mark]. Cost structure analysis enables decisions that balance performance requirements against budget constraints.

**Cost Per Inference.**\index{Cost Per Inference!serving economics} Total serving cost decomposes into several components including compute time for GPU or CPU per inference, memory for accelerator memory required to hold model and activations, data transfer for network bandwidth for request and response payloads, and orchestration overhead for container runtime, load balancing, and monitoring. For GPU inference, compute time dominates when utilization is high. When utilization is low, memory cost dominates\index{GPU Utilization!cost economics} because the GPU is reserved but idle. We can apply this framework to a *ResNet-50 cost analysis*.
...
The GPU versus CPU decision establishes the cost per inference, but determining how much infrastructure to provision requires combining cost analysis with the queuing theory foundations from @sec-model-serving-systems-queuing-theory-tail-latency-29a6. Capacity planning\index{Capacity Planning!infrastructure sizing} translates latency requirements and traffic projections into infrastructure specifications. Key inputs include traffic patterns such as peak request rate, daily and weekly cycles, and growth projections, latency SLOs including p50, p95, and p99 targets, and model characteristics such as inference time distribution at various batch sizes. From these inputs, queuing theory determines required capacity [@harchol2013performance]. The equations developed in @sec-model-serving-systems-queuing-theory-tail-latency-29a6 provide the mathematical foundation where @eq-mm1-wait shows how latency scales with utilization, while @eq-p99-latency enables calculating p99 latency for capacity planning.
...
We begin with the bottleneck that dominates LLM serving costs: KV cache memory. @fig-kv-cache-growth visualizes the memory pressure that explains *why* long-context serving is memory-bound even on H100s.
...
# PURPOSE
# =============================================================================
# Purpose: Plot KV-cache memory growth vs context length
# Used in: Figure "The KV-Cache Explosion"
...
The model generates 256 tokens sequentially. This phase is memory-bandwidth bound\index{Memory Bandwidth!LLM bottleneck}\index{Decode Phase!sequential generation}; the system must read the entire 3.5 GB weight tensor from VRAM to generate a single token.
...
h100_bw_tb_str = f"{h100_bw_tb_value:.1f}"
token_time_theoretical_ms_str = f"{token_time_theoretical_ms_value:.0f}"
realized_tpot_ms_str = f"{realized_tpot_ms_value}"
decode_tokens_str = f"{decode_tokens_value}"
total_decode_s_str = f"{total_decode_s_value:.2f}"
...
req_time_s_str = f"{req_time_s_value:.2f}"
hourly_cost_str = f"{hourly_cost_value:.2f}"
tokens_per_hour_m_str = f"{tokens_per_hour_value / 1e6:.0f}"
cost_per_m_tokens_str = f"{cost_per_m_tokens_value:.3f}"
...
*   Accounting for kernel launch overhead and attention computation, realized Ttoken is approximately `{python} realized_tpot_ms_str` ms.
...
*   Each token requires approximately `{python} kv_per_token_mb_str` MB of KV cache (32 layers × 4096 dim × 2 vectors × 2-byte precision).
...
For an H100 SXM5 instance at approximately USD `{python} hourly_cost_str` per hour (specialized cloud providers; hyperscaler rates vary from USD 2-13 per hour as of 2024):
...
Serving inverts training priorities in unexpected ways\index{Serving!fallacies and pitfalls}. Intuitions from batch processing fail under latency constraints and variable load, causing wasted effort, violated SLOs, and silent accuracy degradation in production.
