...
Parts I through III built a complete foundation...
...
## Generative Inference Metrics {#sec-serving-generative-metrics}

Generative models (LLMs) introduce unique performance metrics distinct from traditional classification. Unlike discriminative models that produce a single output tensor, generative models produce a stream of tokens, requiring a more nuanced approach to latency measurement.

### Performance Metrics: TTFT and TPOT {#sec-serving-llm-metrics}

Measuring the performance of a generative model requires a two-part metric system that reflects the internal state transition from "prefill" (processing input) to "decode" (generating output).
...
::: {.callout-lighthouse title="Lighthouse Example: Latency Targets (GPT-2 / Llama)"}
...
### Memory Considerations {#sec-serving-memory-considerations}
...
Advanced techniques like **PagedAttention** and **Prefix Caching** are required to solve these challenges at production scale. These architectural optimizations are covered in detail in **Volume II: Optimization at Scale** (@sec-optimization-kv-cache).
