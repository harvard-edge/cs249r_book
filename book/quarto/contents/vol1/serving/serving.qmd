...
cc_tokens_req_str = fmt(cc_tokens_per_sec_req_value, precision=0, commas=False)
...
Applying the **D·A·M taxonomy** reveals how deployment fundamentally flips your engineering priorities:
...
# PURPOSE
# =============================================================================
# Purpose: Define constants for Black Friday traffic spike example
# Used in: Callout "The 'Black Friday' Traffic Spike"

# =============================================================================
# INPUT
# =============================================================================
bf_latency_ms_value = 50
bf_qps_normal_value = 1000
bf_qps_spike_value = 10000
bf_spike_factor_value = 10
bf_collapse_latency_s_value = 10

# =============================================================================
# OUTPUT
# =============================================================================
bf_latency_ms_str = f"{bf_latency_ms_value}"
bf_qps_normal_str = f"{bf_qps_normal_value:,}"
bf_qps_spike_str = f"{bf_qps_spike_value:,}"
bf_spike_factor_str = f"{bf_spike_factor_value}"
bf_collapse_latency_s_str = f"{bf_collapse_latency_s_value}"
```
...
ax.text(2021, 0.05, "Trend: ~10× Cheaper\nEvery 18 Months", color=COLORS['grid'], fontsize=9, style='italic')
plt.show()
```
...
Serving systems must execute a complete inference pipeline under latency constraints, not just the neural network computation. A common misconception is that "inference time" equals "serving time"—in reality, the neural network is just one stage in a longer pipeline. Follow the stages in @fig-serving-inference-pipeline from left to right: raw inputs pass through preprocessing (traditional computing), neural network inference (deep learning), and postprocessing (traditional computing) before producing final outputs. Notice that any of these stages—not just the neural network—can become the latency bottleneck. As we will see in @sec-model-serving-systems-latency-budget-ef40, preprocessing often dominates total latency when inference runs on optimized hardware.
...
dynamic_latency_budget_ms_value = 100
...
batch_total_s_str = fmt(batch_total_s_value, precision=0, commas=False)
dynamic_latency_budget_ms_str = f"{dynamic_latency_budget_ms_value}"
```
...
# PURPOSE
# =============================================================================
# Purpose: Define performance metrics for ResNet-50 across serving spectrum
# Used in: Callout "ResNet-50 Across the Serving Spectrum"

# =============================================================================
# INPUT
# =============================================================================
# Cloud (V100)
cloud_model_mb_value = 49
cloud_inf_b1_ms_value = 1.4
cloud_inf_b16_ms_value = 14
cloud_throughput_value = 1143
cloud_vram_gb_value = 2

# Mobile (Pixel 6 NPU)
mobile_model_mb_value = 25
mobile_inf_npu_ms_value = 12
mobile_inf_cpu_ms_value = 45
mobile_throughput_value = 80
mobile_mem_mb_value = 150
mobile_energy_npu_mj_value = 0.8
mobile_energy_cpu_mj_value = 4.2

# TinyML (Cortex-M7)
tiny_model_mb_value = 98
tiny_alt_mb_value = 1.4
tiny_inf_ms_value = 120
tiny_throughput_value = 8
tiny_arena_kb_value = 320
tiny_sram_kb_value = 512
tiny_energy_mj_value = 12

# =============================================================================
# OUTPUT
# =============================================================================
cloud_model_mb_str = f"{cloud_model_mb_value}"
cloud_inf_b1_ms_str = f"{cloud_inf_b1_ms_value}"
cloud_inf_b16_ms_str = f"{cloud_inf_b16_ms_value}"
cloud_throughput_str = f"{cloud_throughput_value:,}"
cloud_vram_gb_str = f"{cloud_vram_gb_value}"

mobile_model_mb_str = f"{mobile_model_mb_value}"
mobile_inf_npu_ms_str = f"{mobile_inf_npu_ms_value}"
mobile_inf_cpu_ms_str = f"{mobile_inf_cpu_ms_value}"
mobile_throughput_str = f"{mobile_throughput_value}"
mobile_mem_mb_str = f"{mobile_mem_mb_value}"
mobile_energy_npu_mj_str = f"{mobile_energy_npu_mj_value}"
mobile_energy_cpu_mj_str = f"{mobile_energy_cpu_mj_value}"

tiny_model_mb_str = f"{tiny_model_mb_value}"
tiny_alt_mb_str = f"{tiny_alt_mb_value}"
tiny_inf_ms_str = f"{tiny_inf_ms_value}"
tiny_throughput_str = f"{tiny_throughput_value}"
tiny_arena_kb_str = f"{tiny_arena_kb_value}"
tiny_sram_kb_str = f"{tiny_sram_kb_value}"
tiny_energy_mj_str = f"{tiny_energy_mj_value}"
```
...
**The Request Pipeline.** Every request traverses a multi-stage pipeline designed to maximize hardware throughput while minimizing latency overhead. Walk through the six stages in @fig-server-anatomy to see how each component absorbs a different source of complexity.
...
json_size_str = fmt(json_size_kb_value, precision=0, commas=False)
json_parse_str = fmt(json_parse_us_value, precision=0, commas=False)
protobuf_size_str = fmt(protobuf_size_kb_value, precision=0, commas=False)
protobuf_parse_str = fmt(protobuf_parse_us_value, precision=0, commas=False)
requests_per_sec_str = f"{requests_per_sec_value:,}"
efficiency_gain_str = fmt(efficiency_gain_value, precision=0, commas=False)
```
...
For dynamic inference systems\index{Latency Budget!optimization objectives}, the fundamental difference from training lies in optimization objectives. Training optimizes throughput by maximizing samples processed per hour over days of computation. A training job that processes 1000 samples per second is successful regardless of whether a few individual samples take longer than average. Serving inverts this priority, optimizing latency (introduced in @sec-ml-system-architecture) per request under strict time constraints. A serving system with 1000ms per-request latency has failed, even if it achieves excellent throughput.
...
# PURPOSE
# =============================================================================
# Purpose: Define activation sizes and arithmetic intensity for resolution bottleneck table
# Used in: Table "Resolution and Compute Bottleneck"

# =============================================================================
# INPUT
# =============================================================================
act_224_mb_value = 12.5
act_384_mb_value = 36.8
act_512_mb_value = 65.5
act_640_mb_value = 102.4
...
**Overlapping I/O and Compute.**\index{I/O Overlap!compute pipelining} Compare the two timing diagrams in @fig-serving-pipeline-timing. In the serial case (A), each request must complete its entire lifecycle (Network $\rightarrow$ CPU Preprocessing $\rightarrow$ GPU Inference $\rightarrow$ Postprocessing) before the next request begins—notice the grey idle gaps that leave the GPU unused for more than 50% of the time. Now look at the pipelined case (B), where those gaps disappear.
...
::: {.callout-note title="Notation Alert: L vs. Latency"}
...
littles_w_ms_str = fmt(littles_w_ms_value, precision=0, commas=False)
littles_w_str = fmt(littles_w_value, precision=0, commas=False)
littles_l_str = fmt(littles_l_value, precision=0, commas=False)
```
...
# M/M/1 p99 factor derivation
mm1_p99_factor_value = 4.6
mm1_rho_example_value = 0.7
...
- Configuration: $T = `{python} pc_config_window_ms_str`$ms, $b_{\text{max}} = `{python} pc_config_batch_str`$
...
The worked example in @sec-model-serving-systems-queuing-theory-tail-latency-29a6 demonstrates the complete capacity planning process by starting from a 50ms p99 SLO and 5,000 QPS target, deriving the safe utilization threshold of `{python} cp_rho_safe_pct_str` percent, calculating required service rate of `{python} cp_mu_required_comma_str` QPS, and determining GPU count with headroom of `{python} cp_final_ceil_str` V100s. Production systems typically provision for peak load plus 30 percent headroom, using auto-scaling to reduce costs during low-traffic periods while meeting latency objectives during peaks.
...
import numpy as np
from physx import viz
from physx.constants import BYTES_FP16
...
# OUTPUT
# =============================================================================
tail_mult_str = fmt(tail_multiplier_value, precision=0, commas=False)
```
