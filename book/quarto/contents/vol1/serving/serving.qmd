...
h100_tdp_val = H100_TDP.to(watt).magnitude
h100_tdp = f"{h100_tdp_val:.0f}"

# Carbon cost of chat calculations
cc_concurrent_req = 114        # concurrent requests
cc_tokens_per_sec_req = 7.5    # tokens/sec per request
cc_total_tokens_sec = cc_concurrent_req * cc_tokens_per_sec_req
cc_host_overhead_w = 300       # host/overhead watts
cc_total_power_w = h100_tdp_val + cc_host_overhead_w
cc_joules_per_token = cc_total_power_w / cc_total_tokens_sec
cc_response_tokens = 500       # typical response length
cc_response_joules = cc_joules_per_token * cc_response_tokens
cc_smartphone_joules = 40000   # smartphone charge
cc_boiling_water_joules = 100000  # boiling cup of water
cc_low_util_pct = 10           # low utilization
cc_idle_power_w = 300          # idle power
cc_low_util_joules = 10        # joules/token at low utilization

cc_concurrent_str = str(cc_concurrent_req)
cc_tokens_req_str = str(cc_tokens_per_sec_req)
cc_total_tokens_str = f"{cc_total_tokens_sec:.0f}"
cc_host_overhead_str = str(cc_host_overhead_w)
cc_total_power_str = f"{cc_total_power_w:.0f}"
cc_joules_token_str = f"{cc_joules_per_token:.2f}"
cc_response_tokens_str = str(cc_response_tokens)
cc_response_joules_str = f"{cc_response_joules:.0f}"
cc_smartphone_str = f"{cc_smartphone_joules:,}"
cc_boiling_str = f"{cc_boiling_water_joules:,}"
cc_low_util_str = str(cc_low_util_pct)
cc_idle_power_str = str(cc_idle_power_w)
cc_low_util_joules_str = str(cc_low_util_joules)
```

## The Serving Paradigm {#sec-model-serving-systems-serving-paradigm-9634}

Serving\index{Serving!production deployment}\index{Model Serving!paradigm shift} marks the transition from model development to production deployment, where the **Iron Law of ML Systems** (@sec-silicon-contract) undergoes a fundamental shift. Training (@sec-ai-training) optimized for throughput, processing massive datasets over days where the goal was samples per hour and latency spikes were absorbed invisibly. Serving must deliver predictions within milliseconds under unpredictable load. The **Latency term**\index{Latency!serving constraint} ($L_{lat}$), the irreducible overhead of request scheduling, network round-trips, and system orchestration, becomes the dominant constraint rather than a rounding error.
...
::: {.callout-notebook #notebook-cost-latency title="Napkin Math: The Cost of Latency"}
...
The preceding spectrum focused on *how* deployment context shapes serving constraints, from datacenter GPUs to microcontroller SRAM. But what happens when traffic exceeds what a single machine can handle? For cloud and datacenter deployments specifically—where multiple replicas serve the same model—an additional infrastructure layer becomes essential: the load balancer. Production serving systems place load balancers\index{Load Balancer!serving infrastructure} between clients and model servers, providing three essential functions for serving infrastructure.
...
While model optimization focuses on the mathematical artifact, model serving requires a specialized software architecture to manage high-frequency request streams and hardware utilization. An inference server\index{Inference Server!architecture}[^fn-inference-server] (such as NVIDIA Triton\index{Triton Inference Server}, TensorFlow Serving\index{TensorFlow Serving}, or TorchServe\index{TorchServe}) is not a simple wrapper around a model script; it is a high-performance scheduler that manages concurrency, memory, and data movement.
...
# JSON vs Protobuf serialization comparison
serial_floats = 1000       # floating point numbers in payload
json_size_kb = 9           # KB on wire
json_parse_us = 50         # μs to parse
protobuf_size_kb = 4       # KB on wire
protobuf_parse_us = 5      # μs to parse
requests_per_sec = 10000   # high-throughput system
efficiency_gain = json_parse_us // protobuf_parse_us

serial_floats_str = f"{serial_floats:,}"
json_size_str = str(json_size_kb)
json_parse_str = str(json_parse_us)
protobuf_size_str = str(protobuf_size_kb)
protobuf_parse_str = str(protobuf_parse_us)
requests_per_sec_str = f"{requests_per_sec:,}"
efficiency_gain_str = str(efficiency_gain)
```
...
*   **JSON**: Uses ~`{python} json_size_str` KB on the wire. Requires ~`{python} json_parse_str`$\mu$s to parse.
*   **Protobuf**: Uses ~`{python} protobuf_size_str` KB on the wire. Requires ~`{python} protobuf_parse_str`$\mu$s to parse.
...
The request lifecycle concludes with postprocessing\index{Postprocessing!logits to predictions}, the often-overlooked phase that transforms model outputs into actionable results. A neural network produces raw tensors—floating-point arrays that carry no inherent meaning to applications or users. A 0.95 probability becomes a confident "dog" label only after postprocessing converts it; a sequence of token IDs becomes readable text; a bounding box tensor becomes a highlighted region in an image. While often overlooked in system design, postprocessing significantly impacts both latency and the usefulness of predictions.
...
littles_lambda_str = f"{littles_lambda:,}"
littles_w_ms = int(littles_w * 1000)
littles_w_ms_str = str(littles_w_ms)
littles_w_str = str(littles_w)
littles_l_str = f"{littles_l:.0f}"
```

::: {.callout-notebook #notebook-littles-law title="Napkin Math: Little's Law"}
...
**The Calculation**:
$$ L = `{python} littles_lambda_str` \times `{python} littles_w_str` = \mathbf{`{python} littles_l_str` \text{ concurrent requests}} $$
...
Dean and Barroso's analysis reveals *why* tail latency\index{Tail at Scale!fan-out amplification} becomes critical as systems scale beyond single machines [@dean2013tail]. The key insight is that when requests fan out to multiple servers, the probability of experiencing at least one slow response grows rapidly with server count. This "tail at scale" effect makes individual server tail latency critical for overall system performance.
...
cp_peak_qps_str = f"{cp_peak_qps:,}"
cp_v100_throughput_str = f"{cp_v100_throughput:,}"
cp_rho_safe_str = f"{cp_rho_safe:.2f}"
...
::: {.callout-notebook title="Napkin Math: ResNet-50 Capacity Planning"}
...
This yields $\rho \leq `{python} cp_rho_safe_str`$ (72% maximum utilization).

#### Step 2: Calculate Required Service Rate {.unnumbered}

$$\mu_{\text{required}} = \frac{\lambda}{\rho_{\text{safe}}} = \frac{`{python} cp_peak_qps_str`}{`{python} cp_rho_safe_str`} = `{python} cp_mu_required_str` \text{ requests/second}$$

#### Step 3: Determine GPU Count {.unnumbered}

Single V100 throughput at batch=16: `{python} cp_v100_throughput_str` images/second

$$\text{GPUs needed} = \frac{`{python} cp_mu_required_str`}{`{python} cp_v100_throughput_str`} = `{python} cp_gpus_raw_str` \rightarrow `{python} cp_gpus_ceil_str` \text{ GPUs}$$
...
$$\text{Utilization after failure} = \frac{`{python} cp_peak_qps_str` / `{python} cp_v100_throughput_str`}{`{python} cp_gpus_after_fail_str`} = `{python} cp_util_after_fail_str`\%$$
...
# Pre-formatted strings
batch1_ms_str = f"{batch1_ms:.0f}"
batch8_wait_ms_str = f"{batch8_wait_ms:.0f}"
batch8_inference_ms_str = f"{batch8_inference_ms:.0f}"
...
1.  **Baseline (Batch 1)**: Inference = **`{python} batch1_ms_str` ms**. Throughput = **`{python} batch1_imgs` img/s**.
2.  **Optimized (Batch 8)**:
    ...
    - **User Latency**: $`{python} batch8_wait_ms_str` \text{ ms (wait)} + `{python} batch8_inference_ms_str` \text{ ms (compute)} = \mathbf{`{python} batch8_user_latency_str` \text{ ms}}$.
...
# Iron Law of Batching - overhead and compute times
il_overhead_ms = 5.0       # fixed overhead (kernel launch, etc.)
il_compute_b1_ms = 0.6     # compute time at batch 1
il_compute_b32_ms = 19.2   # compute time at batch 32
il_eff_b1_pct = int(il_compute_b1_ms / (il_overhead_ms + il_compute_b1_ms) * 100)
il_eff_b32_pct = int(il_compute_b32_ms / (il_overhead_ms + il_compute_b32_ms) * 100)
il_threshold_pct = 10

il_overhead_str = f"{il_overhead_ms:.0f}"
il_compute_b1_str = f"{il_compute_b1_ms}"
il_compute_b32_str = f"{il_compute_b32_ms}"
il_eff_b1_str = f"{il_eff_b1_pct}"
il_eff_b32_str = f"{il_eff_b32_pct}"
il_threshold_str = str(il_threshold_pct)
```
...
*   **Case 1 (Batch 1):** Overhead (`{python} il_overhead_str`ms) $\approx$ Compute (`{python} il_compute_b1_str`ms). Efficiency $\approx `{python} il_eff_b1_str`\%$. The GPU is mostly waiting.
*   **Case 2 (Batch 32):** Overhead (`{python} il_overhead_str`ms) $\ll$ Compute (`{python} il_compute_b32_str`ms). Efficiency $\approx `{python} il_eff_b32_str`\%$. The GPU is crunching numbers.
...
Autoregressive models\index{Autoregressive Models!token generation} like language models generate outputs token by token, creating a batching challenge that differs from single-pass models like ResNet-50. Traditional batching processes all sequences in a batch for all generation steps, wasting compute when sequences complete at different times\index{Sequence Length Variability!batch waste} [@yu2022orca]. If one sequence in a batch of 8 finishes after 10 tokens while others need 100 tokens, 87.5% of the compute for that sequence slot is wasted. This inefficiency matters as LLMs dominate production inference.

Continuous batching\index{Continuous Batching!LLM serving} (also called iteration-level batching) addresses this waste by allowing new requests to join a batch between generation steps and completed sequences to exit [@kwon2023vllm]. Rather than forming static batches that persist for the entire generation process, the system manages batch composition dynamically at each decoding iteration.
...
**Streaming Responses.**\index{Streaming Responses!LLM serving}\index{Chunked HTTP!streaming tokens} Rather than waiting for complete generation, production LLM systems return tokens as they are produced. This fundamentally changes the user experience: a 2-second total generation feels responsive when tokens stream continuously, but feels broken when users stare at a blank screen for 2 seconds. Streaming requires infrastructure support for chunked HTTP responses and client-side incremental rendering. The latency profile shifts accordingly: TTFT determines when output starts appearing (responsiveness), while TPOT determines the perceived generation speed (fluidity).
...
1.  **Throughput**: `{python} cc_concurrent_str` concurrent requests × `{python} cc_tokens_req_str` tokens/sec/req $\approx$ **`{python} cc_total_tokens_str` tokens/sec**.
2.  **Power**: `{python} h100_tdp`W (GPU) + `{python} cc_host_overhead_str`W (Host/Overhead) = **`{python} cc_total_power_str`W**.
3.  **Energy per Token**:

    $$\frac{`{python} cc_total_power_str` \text{ Joules/sec}}{`{python} cc_total_tokens_str` \text{ tokens/sec}} \approx \mathbf{`{python} cc_joules_token_str` \text{ Joules/token}}$$

**The Systems Conclusion**: A typical `{python} cc_response_tokens_str`-token response consumes $\approx \mathbf{`{python} cc_response_joules_str` \text{ Joules}}$.

- For comparison, charging a smartphone consumes $\approx `{python} cc_smartphone_str`$ Joules.
- Boiling a cup of water consumes $\approx `{python} cc_boiling_str`$ Joules.
...
# Runtime comparison for ResNet-50 on V100 (batch size 1)
rt_pytorch_ms = 8.5
rt_torchscript_ms = 6.2
rt_onnx_ms = 5.1
rt_trt_fp32_ms = 2.8
rt_trt_fp16_ms = 1.4
rt_trt_int8_ms = 0.9

# Speedups relative to PyTorch baseline
rt_pytorch_speedup = 1.0
rt_torchscript_speedup = rt_pytorch_ms / rt_torchscript_ms
rt_onnx_speedup = rt_pytorch_ms / rt_onnx_ms
rt_trt_fp32_speedup = rt_pytorch_ms / rt_trt_fp32_ms
rt_trt_fp16_speedup = rt_pytorch_ms / rt_trt_fp16_ms
rt_trt_int8_speedup = rt_pytorch_ms / rt_trt_int8_ms

rt_pytorch_ms_str = f"{rt_pytorch_ms}"
rt_torchscript_ms_str = f"{rt_torchscript_ms}"
rt_onnx_ms_str = f"{rt_onnx_ms}"
rt_trt_fp32_ms_str = f"{rt_trt_fp32_ms}"
rt_trt_fp16_ms_str = f"{rt_trt_fp16_ms}"
rt_trt_int8_ms_str = f"{rt_trt_int8_ms}"

rt_pytorch_speedup_str = f"{rt_pytorch_speedup:.1f}"
rt_torchscript_speedup_str = f"{rt_torchscript_speedup:.1f}"
rt_onnx_speedup_str = f"{rt_onnx_speedup:.1f}"
rt_trt_fp32_speedup_str = f"{rt_trt_fp32_speedup:.1f}"
rt_trt_fp16_speedup_str = f"{rt_trt_fp16_speedup:.1f}"
rt_trt_int8_speedup_str = f"{rt_trt_int8_speedup:.1f}"
```
...
While GPUs dominate the narrative, CPUs\index{CPU Inference!when to use} remain the workhorse for a vast number of inference workloads, particularly for smaller models, latency-insensitive batch jobs, or cost-constrained environments. Optimizing for the CPU requires a different mindset.
...
**The Pickle Problem**\index{Pickle!loading overhead}: The standard PyTorch `torch.load()` uses Python's `pickle` format. This is inefficient because it requires the CPU to unpickle objects one by one, copy them into memory, and then often copy them *again* to the GPU.
