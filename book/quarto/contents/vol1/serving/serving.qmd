...
This memory pressure, visualized in @fig-kv-cache-growth, explains why long-context serving is memory-bound even on H100s. The linear growth of the KV cache with sequence length forces a hard trade-off: to support longer contexts (32k+), we must reduce batch size, which in turn kills throughput efficiency.
