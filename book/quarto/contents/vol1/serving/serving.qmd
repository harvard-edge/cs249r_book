...
These consequences manifest concretely in how we measure and optimize. The benchmarking methodology from @sec-benchmarking-ai now targets percentile latencies rather than aggregate throughput. The quantization techniques from @sec-model-optimizations require validation not just for accuracy preservation but for calibration with production traffic, since quantization error that averages out over training batches may concentrate in specific request types. @sec-ai-acceleration's hardware acceleration strategies must be configured for single-request responsiveness rather than batch throughput, often leaving compute capacity idle to ensure low latency. Understanding these connections enables practitioners to apply earlier optimizations correctly in the serving context.
...
Monitoring in production detects skew that emerges over time. Feature drift detection compares serving distributions to training baselines using statistical distance measures [@breck2019data]. When drift exceeds thresholds, alerts trigger investigation before accuracy degrades noticeably. @tbl-skew-prevention organizes these strategies along a spectrum from prevention to detection, each with distinct operational tradeoffs.
...
Serving inverts training priorities in ways that violate intuition. What works for throughput-optimized batch processing fails under latency constraints and unpredictable load. These fallacies and pitfalls capture mistakes that waste engineering effort, violate SLOs, and silently degrade accuracy in production.

**Fallacy:** _Faster model inference automatically means faster end-to-end serving._

Engineers focus optimization on model inference because that is where ML expertise applies. In production, preprocessing and postprocessing often dominate the latency budget. @sec-serving-latency-budget demonstrates that preprocessing consumes 45 to 70 percent of total latency when inference runs on optimized accelerators. A team that reduces inference time from 5ms to 2ms through quantization achieves only 15 percent end-to-end improvement if preprocessing remains at 8ms. Amdahl's Law formalizes this principle: if preprocessing consumes 50 percent of latency, even infinitely fast inference can yield at most a 2× speedup. Teams that optimize the wrong bottleneck waste engineering months while latency improvements remain negligible.

**Pitfall:** _Running serving infrastructure at high utilization to maximize cost efficiency._

Teams provision for 90 percent utilization to minimize idle capacity and control costs. Under load, latency explodes nonlinearly. @eq-mm1-wait reveals that at 90 percent utilization, average wait time reaches ten times service time, and p99 latency becomes unacceptable for most SLOs. Moving from 70 percent to 90 percent utilization cuts costs by 22 percent but triples average latency. Teams that provision for average load rather than peak load find their systems violate SLOs precisely when traffic increases. Production systems typically target 60 to 70 percent utilization at peak load to maintain latency headroom.

**Fallacy:** _Training accuracy guarantees serving accuracy._

Teams assume that deploying identical model weights preserves validation set performance. In production, preprocessing differences silently degrade accuracy. @sec-serving-skew examines how training-serving skew causes models to behave differently despite using identical weights. Differences in preprocessing libraries (PIL versus OpenCV), numerical precision (float64 versus float32), or feature computation timing shift inputs outside the training distribution without triggering errors. A model achieving 95 percent accuracy in validation might drop to 90 percent in production from subtle preprocessing differences. This 5 percentage point degradation costs real business value, yet standard monitoring that checks for exceptions and latency violations fails to detect it. Prevention requires either identical preprocessing code paths or rigorous statistical monitoring of input distributions.

**Pitfall:** _Using average latency to evaluate serving system performance._

Engineers monitor average latency because it is simple to compute and trends smoothly. Averages hide the experience of the slowest requests, which determine user satisfaction and SLO compliance. A system with 10ms average latency might have 200ms p99 latency, meaning 1 percent of users experience twenty times worse performance. For a service handling 1 million requests per day, 10,000 users encounter unacceptable latency despite healthy average metrics. @sec-serving-tail-latency analyzes how fan-out amplification makes even rare slow responses visible to most users at scale. Production SLOs specify percentile targets such as p95 and p99 precisely because averages mask tail behavior. Systems that report only averages pass monitoring checks while violating user experience standards.

**Fallacy:** _Larger batch sizes always improve throughput._

Engineers maximize batch size to saturate GPU compute and minimize cost per inference. Beyond a certain point, throughput gains diminish while latency grows unboundedly. For ResNet-50 on V100, increasing batch size from 16 to 32 improves throughput by only 12 percent (1,143 to 1,280 images per second) while nearly doubling inference time from 14ms to 25ms. Batch sizes that exceed GPU memory cause out-of-memory failures, and highly variable input sizes create padding overhead that wastes compute. At batch size 64, memory fragmentation can reduce effective utilization to 60 percent despite the GPU appearing busy. The optimal batch size depends on latency SLOs, memory constraints, and traffic patterns. Systems that maximize batch size unconditionally violate latency requirements during production traffic.

**Pitfall:** _Calibrating quantized models with training data rather than production traffic._

Teams use training data for calibration because it is readily available and matches the distribution that produced validation accuracy. When production traffic differs, calibration on training data produces suboptimal scale factors. Post-training quantization requires calibration data to determine optimal scale factors for INT8 conversion, but using training data assumes production inputs match the training distribution. When production traffic differs through different image sources, lighting conditions, or user behavior, calibration on the wrong distribution degrades accuracy. One production system experienced 3.2 percent accuracy loss when calibrating with ImageNet validation images but serving wildlife camera images with different lighting and framing. @sec-model-optimizations establishes that quantization error scales with input activation range, so calibration on mismatched distributions produces incorrect scale factors. Effective calibration requires representative samples of actual serving traffic.

**Pitfall:** _Cold start latency only matters for the first request._

Engineers optimize steady-state latency because that is what most requests experience. In production, cold start affects any request that arrives after a period of inactivity, after model updates, or when auto-scaling adds new instances. In systems with bursty traffic or multiple model versions, cold starts can affect 10 to 30 percent of requests during scale-up events. A model requiring 500ms to load impacts not just the first user but every user who triggers a scale-up event or model reload. For ResNet-50 with TensorRT, cold start adds 30 seconds for compilation if the optimized engine is not cached. Production systems mitigate cold start through model preloading, keep-alive mechanisms that maintain warm instances, and gradual traffic shifting during deployments. Systems that ignore cold start meet latency SLOs during steady state but violate them whenever traffic patterns change.
