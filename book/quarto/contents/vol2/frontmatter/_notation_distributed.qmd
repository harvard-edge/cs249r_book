...
Fleet-scale ML introduces a second layer of notation for coordination, communication, and reliability. Some symbols defined here are reserved for forward use in later chapters (synchronization, repair, gradient sizing) and may not appear in every equation in this notation chapter; defining them centrally keeps the canonical form unambiguous when they do appear. The central equation is the **Distributed Step Time Law**:
...
### The \texorpdfstring{$\alpha$-$\beta$}{-} communication model {#sec-notation-alpha-beta}
...
| $\alpha$   | Learning rate    | Network Latency         | **Network latency** in $\alpha$-$\beta$ model. Context disambiguates.           |
