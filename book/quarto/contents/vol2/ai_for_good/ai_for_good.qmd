...
Beyond direct social services, AI systems are transforming the fundamental pace of scientific discovery itself. This represents a shift to the "Fourth Paradigm" of science: data-intensive scientific discovery.

*   **Biology**: AlphaFold solved the 50-year-old protein folding problem, predicting 3D structures for nearly all known proteins. This accelerates drug discovery and disease understanding by orders of magnitude.
*   **Physics**: Deep reinforcement learning controls nuclear fusion plasma in tokamaks, maintaining stability in regimes that traditional control theory cannot handle.
*   **Materials Science**: Graph neural networks predict properties of millions of crystal structures, identifying candidates for better batteries and solar cells without expensive physical synthesis.
...
Achieving ultra-low model sizes for social good applications requires systematic optimization pipelines that balance accuracy with resource constraints. Model optimization techniques including quantization (reducing numerical precision) [@gholami2021survey], pruning (removing unnecessary parameters) [@han2015learning], and knowledge distillation[^fn-knowledge-distillation] (training smaller models to mimic larger ones) [@hinton2015distilling] must be adapted and intensified for extreme resource limitations encountered in underserved environments.
...
This inverse relationship between need and infrastructure availability, quantified in @tbl-social_challenges, fundamentally distinguishes social good deployments from commercial scenarios in well-resourced environments.

**Compute Equity in Science**

This paradox creates a "compute divide" in scientific research. If training a state-of-the-art climate model requires 10,000 GPUs, then climate science becomes the exclusive domain of a few wealthy institutions.
*   **Access vs Capability**: Democratizing AI for science requires more than open-sourcing code; it requires architectures that run on available hardware.
*   **Distillation for Access**: Techniques like knowledge distillation allow elite labs to train massive "teacher" models and release efficient "student" models that researchers in resource-constrained regions can actually run and fine-tune.
*   **Inference-Optimized Science**: Ensuring that scientific breakthroughs (like protein folding predictions) can be served on commodity hardware is a prerequisite for global scientific equity.
...
::: {.callout-note title="Figure Placeholder: Resource Paradox" collapse="true"}
```{.tikz}
% TODO: Comparison bar chart or scale: Need vs Resources for Commercial vs Social Good
\node[draw, align=center] {Resource Paradox\nNeed > Resources vs Resources > Need};
```
**The Resource Paradox**. Visual comparison of resource availability vs. societal need. Panel A shows "Commercial AI": High resources (cloud, data, bandwidth) applied to problems with lower marginal utility. Panel B shows "AI for Good": Low resources (offline, sparse data, constrained compute) applied to problems with high marginal utility (e.g., disease diagnosis, famine prediction).
:::
...
Resource-constrained ML deployments require fundamentally different architectural approaches than simply scaling down cloud-based systems. Analysis of 50+ successful social impact deployments reveals four recurring design patterns that address specific combinations of computational, connectivity, and data constraints. These patterns provide systematic frameworks for navigating the trade-offs between functionality, resource consumption, and deployment complexity.

#### Hierarchical Processing Pattern {#sec-ai-good-hierarchical-processing-pattern-detail-9b33}

The Hierarchical Processing Pattern organizes computational workloads across multiple tiers based on available resources and latency requirements. This pattern proves most effective when deployments span heterogeneous infrastructure, from resource-rich cloud servers to constrained edge devices.

**Architecture**: Systems implement three distinct tiers. Edge devices (microcontrollers with 100-500 KB RAM) perform local data collection, basic preprocessing, and time-critical inference using quantized models (typically 50-200 KB). Regional nodes (single-board computers with 1-4 GB RAM) aggregate data from multiple edge devices, execute intermediate-complexity models (5-50 MB), and cache cloud model outputs for offline periods. Cloud infrastructure handles model training, complex inference requiring large models (100+ MB), and long-term data storage.

**Communication Optimization**: To minimize bandwidth consumption, edge devices transmit only feature vectors or prediction uncertainties rather than raw sensor data. A typical agricultural monitoring deployment reduces transmission from 2 MB of daily image data per sensor to 50 KB of extracted features, achieving 40× bandwidth reduction. Regional nodes implement intelligent caching, storing the most recent 7 days of cloud model predictions to enable continued operation during connectivity outages lasting up to one week.

**Quantitative Trade-offs**: Hierarchical architectures introduce latency costs from multi-tier communication. Local edge inference completes in 50-200 ms, regional processing adds 500-2000 ms network transit time, while cloud processing introduces 2-10 second end-to-end latency depending on connectivity quality. Systems must carefully allocate tasks to tiers based on latency sensitivity: medical triage requiring <1 second response times executes locally, while crop yield prediction tolerating 10+ second delays can leverage cloud resources.

#### Progressive Enhancement Pattern {#sec-ai-good-progressive-enhancement-pattern-detail-7c44}

The Progressive Enhancement Pattern implements multiple capability tiers that gracefully degrade based on available resources. Unlike hierarchical processing which distributes computation spatially across tiers, progressive enhancement adapts temporally to changing resource availability on a single device.

**Model Cascades**: Systems deploy multiple model versions spanning accuracy-efficiency trade-offs. A crop disease detection system might implement: (1) a 500 KB quantized CNN achieving 82% accuracy with 50 ms latency, (2) a 5 MB full-precision model achieving 90% accuracy with 200 ms latency, and (3) cloud access to a 100 MB ensemble achieving 95% accuracy with 5 second latency. The system selects appropriate models based on current battery level, network availability, and inference urgency.

**Adaptive Model Selection**: Decision logic balances multiple objectives simultaneously. When battery charge exceeds 70% and Wi-Fi connectivity is available, systems use high-accuracy cloud models. When battery drops below 30% or connectivity degrades to cellular (2G/3G), systems switch to lightweight on-device models. During critical battery states (<10% charge), systems may disable complex inference entirely, falling back to rule-based heuristics requiring minimal computation.

**Storage and Update Management**: Maintaining multiple model versions creates storage pressure on constrained devices. A practical implementation stores the smallest quantized model permanently (500 KB), caches one intermediate model (5 MB) that can be evicted under storage pressure, and accesses cloud models without local storage. Over-the-air model updates use differential compression, transmitting only changed model weights rather than complete model files, reducing update bandwidth by 60-80%.

#### Distributed Knowledge Pattern {#sec-ai-good-distributed-knowledge-pattern-detail-2f55}

The Distributed Knowledge Pattern enables peer-to-peer learning and coordination without centralized infrastructure. This pattern excels in scenarios with distributed data sources, intermittent connectivity, and privacy constraints preventing data centralization.

**Federated Learning Adaptation**: Standard federated learning protocols assume reliable coordinator-client communication and sufficient client resources for local training. Resource-constrained adaptations implement asynchronous updates (clients transmit when connectivity permits rather than synchronized rounds), compressed gradients (quantizing gradient updates from 32-bit to 8-bit reduces communication by 75%), and selective participation (only clients with sufficient battery and connectivity participate in each round).

**Peer Coordination**: When central coordination is unavailable, devices coordinate through gossip protocols. Each device maintains a local model and periodically exchanges model updates with nearby peers via Bluetooth or direct Wi-Fi. Convergence requires 3-5× more communication rounds than centralized coordination, but operates without infrastructure dependencies. A 100-device deployment achieves model consensus within 50-100 gossip rounds, completing over 2-4 hours with 2-minute exchange intervals.

**Privacy-Utility Trade-offs**: Differential privacy mechanisms add noise to model updates to prevent inference of individual data points. In resource-constrained settings, the computational overhead of cryptographic secure aggregation (30-50% increase in local computation) must be balanced against privacy requirements. Practical deployments often implement local differential privacy with ε = 3-5, providing reasonable privacy guarantees while keeping computational overhead under 10%.

#### Adaptive Resource Pattern {#sec-ai-good-adaptive-resource-pattern-detail-5e66}

The Adaptive Resource Pattern dynamically adjusts system behavior based on real-time resource availability. This pattern proves particularly effective for deployments with predictable resource cycles (solar power), intermittent connectivity, or variable workloads.

**Energy-Aware Scheduling**: Solar-powered systems concentrate intensive computation during peak sunlight hours (typically 10:00-14:00 local time) when power generation exceeds 80% of maximum capacity. Model training, batch inference, and data synchronization execute during high-power periods. During low-power periods (nighttime, cloudy weather), systems limit operation to essential monitoring and time-critical inference using the most efficient models.

**Dynamic Duty Cycling**: Sensor sampling rates adapt to energy availability. When battery charge exceeds 60%, agricultural sensors sample at 15-minute intervals. As charge drops to 30-60%, sampling reduces to hourly intervals. Below 30% charge, sensors enter minimal operation mode with 6-hour sampling intervals, sufficient to detect critical events while conserving energy for multi-day operation.

**Predictive Resource Management**: Systems learn resource availability patterns over time. A deployment tracking 30 days of solar generation and battery performance develops hour-by-hour energy availability forecasts. These predictions enable proactive scheduling: if forecasts predict insufficient energy for planned operations, systems defer non-critical tasks or reduce sampling rates in advance, preventing reactive shutdowns that might miss important events.

**Computational Load Balancing**: When multiple inference requests arrive simultaneously, adaptive schedulers prioritize based on criticality, resource availability, and deadline constraints. Medical diagnostic requests receive priority over routine monitoring, executing even under low battery conditions. Agricultural monitoring requests may be delayed until next high-power period if battery charge is critical.

These four patterns address distinct constraint combinations and frequently combine within deployments. A comprehensive agricultural monitoring system might use hierarchical processing for spatial distribution (edge-regional-cloud tiers), progressive enhancement for temporal adaptation (switching models based on resources), distributed knowledge for peer learning (gossip-based model updates), and adaptive resources for energy management (solar-aware scheduling). Selecting appropriate pattern combinations requires analyzing the specific constraints, requirements, and operational context of each deployment.
...
- **Consensus mechanisms**: Modified Raft protocols[^fn-raft-protocol] operating with 10-second heartbeat intervals accommodate network latency while detecting failures within 30-second windows
...
- **Active learning**[^fn-active-learning]: Strategically selecting informative examples for labeling

[^fn-active-learning]: **Active Learning**: A machine learning approach where the model identifies which unlabeled examples would be most informative to label next, rather than labeling data randomly. For social good deployments, this is particularly valuable because expert annotation time is scarce and expensive. An active learning system might identify 50 ambiguous crop images from thousands collected and request expert labeling only for those, achieving the same accuracy improvement as labeling 500 random images. This 10× reduction in labeling effort makes expert involvement feasible in resource-constrained contexts.
...
## Fallacies and Pitfalls {#sec-ai-good-fallacies-pitfalls}

AI for Good deployments involve unique sociotechnical challenges that generate persistent misconceptions. These fallacies and pitfalls capture errors that undermine impact even when technical implementation succeeds.
...
Impact depends on the entire sociotechnical system: user interface design, infrastructure availability, supply chain access, and behavior change adoption. Studies of mobile health interventions find that technical accuracy explains less than 30% of variance in health outcomes. The remaining 70% depends on factors invisible to model performance metrics.
...
Engineers from well-connected environments systematically underestimate connectivity constraints. Designing for "typical" 3G speeds ignores that target users may experience multi-day outages, sub-50kbps throughput, or costs of $0.10 per megabyte that make cloud-dependent systems economically unviable.

A survey of agricultural apps deployed in Sub-Saharan Africa found that 40% became effectively unusable within 6 months due to connectivity assumptions that held during urban pilots but failed in rural deployments. Successful systems assume zero connectivity as the baseline and treat network access as an enhancement.

**Fallacy: Local partners will handle deployment challenges.**

Technical teams often assume that local implementing partners will resolve the "non-technical" challenges of deployment: user training, hardware distribution, ongoing support, and community engagement. This assumption fails because these "non-technical" challenges are actually deeply technical, requiring specific expertise in user interface design for low-literacy populations, hardware ruggedization for harsh environments, and sustainable business models for ongoing operations.

Partnership success requires explicit technical collaboration on deployment challenges, not delegation of problems the development team does not understand.

**Pitfall: Optimizing for pilot success instead of sustainable scale.**

Pilot deployments receive intensive support: dedicated staff, frequent troubleshooting, motivated early-adopter users, and flexible timelines. These conditions systematically differ from scale deployments where support is thin, users are less engaged, and operational constraints are rigid.

A common pattern: pilots achieve 80% adoption rates with intensive support, organizations declare success and scale, scaled deployments achieve 15% adoption without intensive support, and projects are quietly abandoned. Sustainable deployments design for minimal-support operation from the beginning, treating pilot intensive support as a bug rather than a feature.

**Fallacy: Open-sourcing ensures sustainability.**

Publishing code under open licenses does not ensure that communities can maintain, adapt, or extend systems. Sustainability requires:

- Documentation accessible to local technical capacity levels
- Modular architecture enabling partial updates without full system expertise
- Training programs building local implementation capability
- Business models funding ongoing development and support

Open source is necessary but insufficient. Communities need open ecosystems including training, documentation, and support networks, not just open code.

**Pitfall: Ignoring local power dynamics.**

Technology deployments interact with existing social structures in ways that can amplify inequities. Agricultural apps may benefit larger farmers with smartphones while disadvantaging smaller farmers without devices. Health information systems may shift power from traditional community health structures to centralized authorities. Educational tools may favor students from families with technology experience.

Successful deployments include explicit equity analysis: Who gains access? Who loses relative status? How do benefits distribute across existing social divisions? Without this analysis, well-intentioned systems may widen rather than narrow gaps.

**Fallacy: Technical robustness prevents misuse.**

Building secure, accurate, privacy-preserving systems does not prevent harmful applications. Facial recognition systems designed for accessibility may be repurposed for surveillance. Location tracking designed for emergency response may enable stalking. Predictive systems designed for resource allocation may encode discriminatory patterns.

Responsible deployment requires anticipating misuse scenarios, implementing governance structures that constrain harmful applications, and maintaining ongoing monitoring for unintended consequences. Technical robustness addresses some failure modes while leaving others unaddressed.

**Pitfall: Measuring success at deployment rather than adoption.**

Organizations commonly declare success when systems are deployed, installed, or launched. Actual impact requires sustained adoption: users continuing to engage over months and years, not just during initial rollout.

A review of 50 AI for Good projects found that 80% claimed deployment success in initial reports, but only 25% demonstrated sustained adoption at 24-month follow-up. The 55% gap represents systems that achieved technical deployment without achieving impact. Success metrics should track 12-month and 24-month adoption, not launch statistics.

## Summary {#sec-ai-good-summary-2437}

AI for social good represents one of the most challenging yet rewarding applications of machine learning technology, requiring systems that operate effectively under severe resource constraints while delivering meaningful impact to underserved communities. Building AI for social impact is the ultimate test of trustworthiness. These systems must be responsible to the communities they serve, as explored in @sec-responsible-ai, while implementing the security and privacy frameworks from @sec-security-privacy against misuse in vulnerable contexts. They must demonstrate the robustness principles from @sec-robust-ai to handle unpredictable real-world conditions, and apply the sustainability practices from @sec-sustainable-ai to operate for years on limited resources. The design patterns presented in this chapter are, in essence, architectures for trustworthiness under constraint.
...
This chapter has focused on deploying existing ML capabilities under severe resource constraints, treating limitation as a deployment challenge to overcome. However, the patterns and techniques developed here (efficient architectures, federated learning, edge processing, adaptive computation) represent more than specialized solutions for underserved environments.

**Simulation Intelligence**

A key frontier for AI impact is **Simulation Intelligence**: using AI not just to perceive the world, but to simulate complex systems to guide decision-making.
*   **Weather & Climate**: AI surrogates simulate weather patterns 1000x faster than physics-based models, enabling hyper-local forecasting.
*   **Ecosystems**: Simulating the impact of conservation interventions on biodiversity before implementation.
*   **Policy**: Modeling economic outcomes of policy changes in "digital twin" societies.

This shift from *predictive* AI (what will happen?) to *prescriptive* AI (what should we do?) relies on the ability to run massive simulations efficiently—a challenge that demands the very resource-constrained engineering principles honed in this chapter.
