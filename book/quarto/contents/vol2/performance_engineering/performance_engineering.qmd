...
class MemoryWallScenario:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check, fmt_time, fmt_qty, fmt_qty_int
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp8_str = fmt_qty_int(flops_fp8, TFLOP/second, commas=False)
    h100_hbm_bw_str = fmt_qty(bw, TB/second, precision=2, commas=False)
    h100_ridge_fp8_str = fmt(ridge_fp8, precision=1, commas=False)
    energy_ratio_str = fmt_int(energy_ratio.m_as(''), commas=False)
    h100_mem_str = fmt_qty(mem, GiB, precision=0, commas=False, unit_label="GB")
    h100_tdp_str = fmt_int(tdp.m_as(watt), commas=False)
    register_total_mb_str = fmt_qty_int(sm_count * reg_per_sm, ureg.mebibyte, commas=False, unit_label="MB")
    l2_cache_mb_str = fmt_qty_int(l2_cache, MB, commas=False)
    hbm_latency_ns_str = fmt_time(round(hbm_latency_ns), 'nanosecond', precision=0, commas=False)
    hbm_energy_pj_str = fmt_qty_int(energy_dram, ureg.picojoule, commas=False, unit_label="pJ")
    sm_count_str = fmt_int(sm_count, commas=False)
    reg_per_sm_str = fmt_qty_int(reg_per_sm, ureg.kibibyte, commas=False, unit_label="KB")
    shared_mem_per_sm_str = fmt_qty_int(shared_per_sm, ureg.kibibyte, commas=False, unit_label="KB")
```
...
class RooflineRidgeCalc:
    ...
    a100_fp16_str = fmt_qty_int(h_a100.compute.peak_flops, TFLOP/second, commas=False)
    a100_hbm_bw_str = fmt_qty(h_a100.memory.bandwidth, TB/second, precision=1, commas=False)
    h100_fp8_str = fmt_qty_int(h_h100.compute.precision_flops["fp8"], TFLOP/second, commas=False)
    h100_hbm_bw_str = fmt_qty(h_h100.memory.bandwidth, TB/second, precision=2, commas=False)
```
...
class FusionTrafficCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tensor_mb_str = fmt_qty(tensor_bytes * byte, MB, precision=1, commas=False)
    materialized_mb_str = fmt_qty(materialized_bytes * byte, MB, precision=1, commas=False)
    traffic_mb_str = fmt_qty(traffic_bytes * byte, MB, precision=1, commas=False)
    traffic_gb_str = fmt_qty(traffic_bytes_all_layers * byte, GB, precision=1, commas=False)
...
class FlashAttentionSavings:
    ...
    naive_mb_str = fmt_qty(naive_attn_bytes_total * byte, MB, precision=1, commas=False)
    flash_mb_str = fmt_qty(flash_attn_bytes_total * byte, MB, precision=1, commas=False)
    ...
    naive_head_traffic_mb_str = fmt_qty(naive_attn_bytes_per_head * byte, MB, precision=1, commas=False)
    flash_head_traffic_mb_str = fmt_qty(flash_attn_bytes_per_head * byte, MB, precision=1, commas=False)
```
...
class KVCacheAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt_int, fmt, fmt_qty, fmt_qty_int
    kv_fp16_gb_str = fmt_qty(kv_cache_qty_f16, GB, precision=2, commas=False)
    kv_fp16_per_gpu_gb_str = fmt_qty(kv_cache_qty_f16 / tensor_parallel_degree, GB, precision=2, commas=False)
    kv_int8_total_gb_str = fmt_qty(kv_cache_qty_i8, GB, precision=2, commas=False)
    kv_int8_per_gpu_gb_str = fmt_qty(kv_cache_qty_i8 / tensor_parallel_degree, GB, precision=2, commas=False)
    ...
    avail_fp16_gb_str = fmt_qty_int(avail_fp16_gb * GB, GB, commas=False)
    weight_int4_gb_str = fmt_qty(weight_int4_shard_gb * GB, GB, precision=2, commas=False)
    avail_int8_gb_str = fmt_qty(avail_int8_gb * GB, GB, precision=2, commas=False)
    weight_fp16_shard_gb_str = fmt_qty_int(weight_fp16_shard_gb * GB, GB, commas=False)
    weight_fp16_total_gb_str = fmt_qty_int(weight_fp16_shard_gb * tensor_parallel_degree * GB, GB, commas=False)
    tensor_parallel_degree_str = fmt_int(tensor_parallel_degree, commas=False)
    gpu_total_gb_str = fmt_qty_int(Hardware.Cloud.H100.memory.capacity, GiB, commas=False, unit_label="GB")
...
class CompilationDividend:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    tpt_str = fmt_time(_tpt, 'millisecond', precision=2, commas=False)
    ...
    new_time_str = fmt_time(new_time_ms, 'millisecond', precision=2, commas=False)
...
class NvlinkH100:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    bw_gbps_str = fmt_qty_int(bw, GB/second, commas=False)
```
...
class OverlapCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    from mlsysim.fmt import fmt, fmt_int, MarkdownStr, fmt_time, fmt_qty, fmt_qty_int
    ...
    allreduce_ms_str = fmt_time(allreduce_time_s * THOUSAND, 'millisecond', precision=1, commas=False)
    backward_ms_str = fmt_time(backward_time_s * THOUSAND, 'millisecond', precision=1, commas=False)
    step_no_overlap_ms_str = fmt_time(step_no_overlap * THOUSAND, 'millisecond', precision=1, commas=False)
    step_overlap_ms_str = fmt_time(step_with_overlap * THOUSAND, 'millisecond', precision=1, commas=False)
    ...
    nvlink_bw_gbs_str = fmt_qty_int(nvlink_bw, GB/second, commas=False)
```
...
class MemoryWallProfilingRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_hbm_bw_str = fmt_qty(Hardware.Cloud.H100.memory.bandwidth, TB/second, precision=2, commas=False)
```
...
class LayerNormRoofline:
    ...
    achieved_tflops = achieved_flops.m_as(TFLOPs/second)
    achieved_bw = 2.8 * TB / second
    achieved_tbs = achieved_bw.m_as(TB/second)

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    arithmetic_intensity = achieved_tflops / achieved_tbs

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    achieved_tflops_str = fmt_qty_int(achieved_flops, TFLOPs/second, commas=False, unit_label="TFLOP/s")
    achieved_tbs_str = fmt_qty(achieved_bw, TB/second, precision=1, commas=False)
...
class ProfilerDetective:
    ...
    decode_traffic_gb = decode_traffic.m_as(GB)
    hidden_performance_pct = 53
    gemm_bandwidth = 2.8 * TB / second
    gemm_bandwidth_tbs = gemm_bandwidth.m_as(TB/second)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    observed_tps_str = fmt_rate(round(observed_tps), 'tokens/s', precision=0, commas=False)
    ceiling_tps_str = fmt_rate(round(ceiling_tps), 'tokens/s', precision=0, commas=False)
    ...
    gemm_bandwidth_tbs_str = fmt_qty(gemm_bandwidth, TB/second, precision=1, commas=False)
    ...
    kv_cache_bandwidth_tbs_str = fmt_qty(kv_cache_bandwidth, TB/second, precision=1, commas=False)
...
from mlsysim.fmt import fmt_int,  fmt, check, MarkdownStr, fmt_percent, fmt_time
...
class FleetEfficiencyCalc:
    ...
    fleet_t_step_ms_str = fmt_time(round(t_fleet_step_ms), 'millisecond', precision=0, commas=False)
