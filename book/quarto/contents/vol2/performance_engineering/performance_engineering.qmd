...
The energy implications of reduced precision extend beyond throughput. The Horowitz (2014) energy estimates[^fn-horowitz-energy] show that an FP32 multiply-accumulate costs approximately 3.7 pJ, while an INT8 operation costs approximately 0.2 pJ, an 18.5$\times$ reduction.

[^fn-horowitz-energy]: **Horowitz Energy Model**: From Mark Horowitz's 2014 ISSCC keynote "Computing's Energy Problem (and what we can do about it)," measured at 45 nm CMOS. These figures are widely cited but represent a specific process node; at modern 5 nm nodes, the absolute picojoules differ but the ratios between arithmetic and memory access energy have widened, making data movement an even larger fraction of total system power in current ML accelerators. \index{Horowitz Energy Model!precision engineering}

FP8 operations fall between these extremes. At datacenter scale, where thousands of GPUs run continuously, the cumulative energy savings from reduced precision are substantial. The performance benefit (higher throughput) and the energy benefit (lower cost per operation) compound: FP8 delivers twice the throughput at roughly half the energy per operation, resulting in approximately 4$\times$ improvement in energy efficiency for compute-bound workloads.
...
This guarantee distinguishes speculative decoding from every other optimization technique in this chapter. Operator fusion changes the order of floating-point operations, which can alter results at the level of floating-point rounding. Precision reduction explicitly sacrifices numerical precision for speed. Quantization approximates weights and activations. Graph compilation may apply algebraically equivalent but numerically different transformations. In each case, the differences are typically negligible, but they exist. Speculative decoding is provably lossless[^fn-speculative-lossless]: the output distribution is mathematically identical to standard autoregressive generation, regardless of the draft model's quality.

[^fn-speculative-lossless]: **Lossless Guarantee Scope**: The guarantee holds in expectation over the rejection sampling process for a fixed target model. In practice, the guarantee can be weakened by floating-point non-determinism: if the target model's logits differ by even one ULP between standalone decoding and batched verification (due to different GEMM tiling or reduction order), the accepted distribution shifts slightly. Systems requiring bit-exact reproducibility must enforce deterministic CUDA operations, which can reduce throughput by 5--15%. \index{Speculative Decoding!lossless guarantee}

A terrible draft model simply produces no speedup (all tokens are rejected), but the output is still correct.
...
***Mixture of Experts (MoE)***\index{Mixture of Experts!definition} is a sparse architecture that replaces dense layers with multiple parallel "expert" sub-networks and a router that selects a subset ($k$) of experts for each input.

1.  **Significance (Quantitative):** It decouples **Model Capacity** (total parameters) from **Compute Cost ($O$)**. For a model with $N$ experts and top-$k$ routing, only $k/N$ of the FFN parameters are activated per token, allowing for trillion-parameter reasoning capabilities with billion-parameter compute budgets.
2.  **Distinction (Durable):** Unlike **Dense Architectures** (where every parameter is used for every input), MoE is a **Conditional Computation** model that trades **Parameter Density** for **Arithmetic Intensity**.
3.  **Common Pitfall:** A frequent misconception is that MoE is "cheaper" than dense models. In reality, while FLOPs per token are lower, the **Memory Wall ($BW$)** and **Communication Overhead ($L_{lat}$)** are much higher because the full model state must be sharded across the cluster, requiring all-to-all communication for token routing.
