...
To understand why the memory wall exists, consider the physical structure of a GPU memory system. @Tbl-performance-engineering-memory-hierarchy summarizes the four levels by scale, access cost, and the optimization constraint each one imposes.
...
The metric choice in @tbl-performance-engineering-key-metrics reflects the fundamental bottleneck shift between training and inference: MFU is usually the right training-efficiency lens, while MBU is often the right inference-efficiency lens when decode is limited by HBM traffic.
...
Five patterns reveal the most diagnostic information from a Nsight Systems trace. @Tbl-performance-engineering-nsight-patterns maps each visual pattern to the bottleneck question it answers.
...
**Math**: Scaling tax is computed as $1 - \text{Fleet MFU}/\text{Local MFU}$, giving `{python} FleetEfficiencyCalc.fleet_scaling_tax_str`.
