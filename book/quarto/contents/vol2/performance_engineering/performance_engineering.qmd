...
::: {layout-narrow}
::: {.column-margin}
_Gemini Pro 3 Prompt: A visualization of a "Compiler for Intelligence." The image shows a complex neural network graph entering a glowing, crystalline prism (the compiler) and emerging as a streamlined, highly efficient beam of light (machine code). Inside the prism, gears represent kernel fusion, tiling, and memory management. The background is a circuit board schematic representing the hardware substrate. The style is technical and futuristic._
:::
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Chapter-wide setup for Performance Engineering.
# │
# │ Goal: Provide foundational hardware specs for roofline and bottleneck analysis.
# │ Show: H100, A100, B200 specs for memory bandwidth, compute, and capacity.
# │ How: Pull from mlsys.constants, format for inline use.
# │
# │ Imports: mlsys.constants (H100_*, A100_*, B200_*, ENERGY_*)
# │ Exports: h100_*, a100_*, b200_* formatted strings
# └─────────────────────────────────────────────────────────────────────────────

# ┌── 1. PARAMETERS (Inputs) ──────────────────────────────────────────────────

# H100 specs
h100_hbm_bw_raw = H100_MEM_BW
h100_fp16_raw = H100_FLOPS_FP16_TENSOR
h100_fp8_raw = H100_FLOPS_FP8_TENSOR
h100_mem_raw = H100_MEM_CAPACITY
h100_tdp_raw = H100_TDP

# A100 specs
a100_hbm_bw_raw = A100_MEM_BW
a100_fp16_raw = A100_FLOPS_FP16_TENSOR
a100_mem_raw = A100_MEM_CAPACITY

# B200 specs
b200_fp8_raw = B200_FLOPS_FP8_TENSOR
b200_hbm_bw_raw = B200_MEM_BW
b200_mem_raw = B200_MEM_CAPACITY
...
h100_ridge_fp16 = h100_fp16_raw / h100_hbm_bw_raw
h100_ridge_fp8 = h100_fp8_raw / h100_hbm_bw_raw
a100_ridge_fp16 = a100_fp16_raw / a100_hbm_bw_raw

# ┌── 3. INVARIANTS (Guardrails) ──────────────────────────────────────────────
check(h100_hbm_bw_raw.to(TB / second).magnitude > 3.0,
      "H100 HBM BW should exceed 3 TB/s")
check(h100_fp16_raw.to(TFLOPs / second).magnitude > 900,
      "H100 FP16 should exceed 900 TFLOPS")

# ┌── 4. OUTPUTS (Formatting) ─────────────────────────────────────────────────

# H100
h100_hbm_bw_str = fmt(h100_hbm_bw_raw, "TB/s", precision=2)
h100_hbm_bw_gbs_str = fmt(h100_hbm_bw_raw, "GB/s", precision=0)
h100_fp16_str = fmt(h100_fp16_raw, "TFLOPs/s", precision=0)
h100_fp8_str = fmt(h100_fp8_raw, "TFLOPs/s", precision=0)
h100_mem_str = fmt(h100_mem_raw, "GiB", precision=0)
h100_tdp_str = fmt(h100_tdp_raw, "W", precision=0)
h100_ridge_fp16_str = f"{h100_ridge_fp16.magnitude:.0f}"
h100_ridge_fp8_str = f"{h100_ridge_fp8.magnitude:.0f}"

# A100
a100_hbm_bw_str = fmt(a100_hbm_bw_raw, "TB/s", precision=0)
a100_hbm_bw_gbs_str = fmt(a100_hbm_bw_raw, "GB/s", precision=0)
a100_fp16_str = fmt(a100_fp16_raw, "TFLOPs/s", precision=0)
a100_ridge_fp16_str = f"{a100_ridge_fp16.magnitude:.0f}"

# B200
b200_fp8_str = fmt(b200_fp8_raw, "TFLOPs/s", precision=0)
b200_hbm_bw_str = fmt(b200_hbm_bw_raw, "TB/s", precision=0)
b200_mem_str = fmt(b200_mem_raw, "GiB", precision=0)

# Energy ratio
energy_ratio = energy_dram_pj / energy_sram_pj
energy_ratio_str = f"{energy_ratio.magnitude:.0f}"
```

## The Efficiency Frontier {#sec-performance-engineering-efficiency-frontier}
...
::: {.callout-definition title="Definition of Arithmetic Intensity"}

**Arithmetic Intensity** (AI) is the ratio of floating-point operations (FLOPs) performed to bytes of data moved from off-chip memory (typically HBM) for a given computation. It is measured in FLOPs per byte (FLOP/byte). A workload with high arithmetic intensity reuses each loaded byte many times; a workload with low arithmetic intensity loads data, uses it once, and discards it.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Roofline Model section — ridge point comparison across GPUs
# │
# │ Goal: Show how the ridge point shifts across hardware generations.
# │ Show: A100 vs H100 vs H100-FP8 ridge points.
# │ How: Compute P/B for each configuration.
# │
# │ Imports: mlsys.constants (A100_*, H100_*)
# │ Exports: ridge point strings for prose
# └─────────────────────────────────────────────────────────────────────────────
...
a100_ridge_val = a100_ridge_fp16.magnitude
h100_ridge_fp16_val = h100_ridge_fp16.magnitude
h100_ridge_fp8_val = h100_ridge_fp8.magnitude
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Roofline section — classify common ML ops by arithmetic intensity
# │
# │ Goal: Compute arithmetic intensity for GEMM, attention, element-wise ops.
# │ Show: Which ops are compute-bound vs memory-bound.
# │ How: FLOP counts and byte counts for canonical operations.
# │
# │ Imports: (none, standalone calculation)
# │ Exports: intensity values for prose
# └─────────────────────────────────────────────────────────────────────────────
...
Before examining optimization techniques, it is worth understanding why batch size is the most powerful, and most constrained, lever for performance. Increasing the batch size transforms the arithmetic intensity of every operation. For an LLM decode step, the arithmetic intensity scales linearly with batch size:
...
The memory wall analysis reveals that individual operations like GELU activations and layer normalization are deeply memory-bound. But these operations never execute in isolation. A transformer layer applies a linear projection, a GELU activation, and a layer normalization in sequence. When executed as separate GPU kernels, each operation writes its result to HBM and the next operation reads it back. These intermediate writes and reads are entirely wasteful: the output of GELU feeds directly into layer normalization, so the intermediate tensor never needs to leave the chip. **Operator Fusion** eliminates these redundant memory round-trips by combining multiple operations into a single GPU kernel that keeps intermediate results in fast on-chip SRAM.
...
Before examining FlashAttention, it is worth understanding an orthogonal technique for reducing the overhead term in the Iron Law: **CUDA Graphs**. While operator fusion combines multiple operations into fewer kernels, CUDA Graphs eliminate the CPU overhead of launching those kernels.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: FlashAttention section — quantify HBM traffic reduction
# │
# │ Goal: Compare naive attention HBM traffic vs FlashAttention tiled traffic.
# │ Show: Memory savings ratio for seq_len=8192, head_dim=128.
# │ How: Count bytes read/written for each approach.
# │
# │ Imports: (none, standalone)
# │ Exports: savings ratio string
# └─────────────────────────────────────────────────────────────────────────────
...
Reducing numerical precision is one of the most effective ways to improve performance in ML systems. Halving the precision of a weight from FP16 (2 bytes) to FP8 (1 byte) doubles the effective memory bandwidth, since the same physical HBM bus now delivers twice as many values per second. On hardware that natively supports lower-precision arithmetic (the H100's FP8 Tensor Cores deliver `{python} h100_fp8_str` TFLOPS versus `{python} h100_fp16_str` TFLOPS at FP16), the compute throughput doubles as well. Precision engineering is the art of finding the lowest precision that preserves model quality, then engineering the system to exploit it.
...
The quantization techniques described above (LLM.int8(), GPTQ, AWQ) are all **post-training quantization (PTQ)** methods: they quantize a pre-trained model without additional training. PTQ is attractive because it requires no training infrastructure, just a calibration dataset and a few hours of compute. For INT8 and INT4 weight-only quantization, PTQ typically preserves model quality within 1--2% on standard benchmarks.

**Quantization-aware training (QAT)** takes a different approach: it simulates quantization effects during training (or fine-tuning), allowing the model's weights to adapt to the quantization noise. During QAT, the forward pass applies quantization (round weights to INT4 or INT8 grid points), but the backward pass uses the straight-through estimator (STE) to pass gradients through the non-differentiable rounding operation. Over the course of training, the model learns weight configurations that are robust to quantization, often achieving quality indistinguishable from the full-precision model.

The trade-off between PTQ and QAT is engineering cost versus quality at aggressive quantization levels. At INT8 precision, PTQ (with techniques like AWQ or SmoothQuant) is typically sufficient, and QAT offers minimal additional quality. At INT4 precision, the gap widens: PTQ methods may lose 2--5% on reasoning benchmarks, while QAT can close most of this gap. At INT3 or INT2 precision (explored in research but rarely deployed in production), QAT becomes essential because the quantization noise from PTQ destroys model capability entirely.

The practical workflow in most production environments uses PTQ for initial deployment (because it is fast and requires no training infrastructure), then applies QAT only if the PTQ model fails to meet quality requirements at the target precision. This two-stage approach minimizes engineering effort while preserving the option of higher quality when needed.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: KV Cache Compression section
# │
# │ Goal: Compute KV cache size for a 70B model and show INT4 savings.
# │ Show: KV cache in FP16 vs INT4, and freed memory as extra batch capacity.
# │ How: 2 * num_layers * num_heads * head_dim * seq_len * bytes_per_elem.
# │
# │ Imports: (none, standalone)
# │ Exports: kv cache size strings
# └─────────────────────────────────────────────────────────────────────────────
...
The previous sections examined hand-crafted optimizations: FlashAttention's tiled algorithm, FP8's format engineering, and block-wise quantization's outlier handling. Each optimization was discovered by human experts who understood both the algorithm and the hardware deeply enough to find a better mapping between the two. The labor cost is enormous. FlashAttention required thousands of lines of handwritten CUDA code, deep knowledge of GPU memory hierarchy, and months of development. Most organizations cannot afford to write custom kernels for every operation in their model.

**Graph Compilation** automates a significant portion of this process: a compiler analyzes the entire computational graph, identifies fusion opportunities, selects optimized kernels, plans memory allocation, and generates efficient machine code for the target hardware. The rise of graph compilation in the 2022-2025 period reflects a maturation of ML systems engineering. In the early era, ML frameworks (TensorFlow 1.x, Theano) were built around static graphs with ahead-of-time compilation. PyTorch disrupted this with eager execution, which was easier to debug and develop with but left performance on the table. The current generation of compilers (torch.compile, XLA, TensorRT) attempts to combine the best of both: the development experience of eager execution with the optimization potential of static compilation.

### The Compilation Pipeline {#sec-performance-engineering-compilation-pipeline}

A graph compiler transforms a high-level model definition (Python code) into optimized hardware instructions through a multi-stage pipeline.

In the **graph capture** stage, the compiler traces the model's execution to construct a computational graph, a directed acyclic graph where nodes represent operations and edges represent tensor dependencies. Dynamic Python control flow (loops, conditionals) must be handled by either tracing through a representative execution path or by using compiler-specific annotations to mark dynamic dimensions.

During **graph-level optimization**, the compiler applies algebraic simplifications, common subexpression elimination, and operation rewriting. Constant folding evaluates operations on constant inputs at compile time. Dead code elimination removes operations whose outputs are never used. These standard compiler optimizations can reduce graph size by 10--30% before any hardware-specific work begins.

The **operator fusion** pass identifies sequences of operations that can be combined into single kernels. The compiler uses heuristics and cost models to decide which fusions are profitable. Fusing a GEMM with a subsequent bias addition and GELU activation is almost always beneficial. Fusing two large GEMMs that share an input is beneficial only if the resulting kernel fits within SRAM constraints.

**Memory planning** determines when to allocate and free tensors, and where to place them in the memory hierarchy. The compiler analyzes tensor lifetimes, identifies tensors that can share the same physical memory (because their lifetimes do not overlap), and schedules memory operations to minimize peak memory usage. This is analogous to register allocation in a traditional compiler, but for GPU global memory.

The impact of memory planning on peak memory consumption can be substantial. Without optimization, a 24-layer transformer may allocate separate buffers for each layer's activations, consuming memory proportional to the number of layers. With lifetime analysis, the compiler recognizes that layer $i$'s activations are no longer needed once layer $i+1$ has consumed them (in inference, where no backward pass requires them). The compiler can reuse the same physical memory buffer for alternating layers, reducing activation memory from $O(L)$ to $O(1)$, where $L$ is the number of layers. For a model where activations consume 10 GB per layer across 80 layers, this optimization reduces peak activation memory from 800 GB (impossible on any single GPU) to approximately 10--20 GB (comfortably within a single H100).

Memory planning also interacts with operator fusion in important ways. Fusing two operations eliminates the intermediate tensor between them, which both removes the HBM traffic (the fusion benefit) and removes the memory allocation for the intermediate (the memory planning benefit). The compiler must reason about both effects jointly to make profitable decisions.

**Kernel selection** maps each fused operation to a specific kernel implementation. The compiler maintains a library of pre-tuned kernels for different operation types, input shapes, and data types. For novel shapes or operations, the compiler may invoke an autotuning process that benchmarks multiple kernel variants and selects the fastest.
...
XLA (Accelerated Linear Algebra) is Google's graph compiler, used as the backend for JAX and TensorFlow. Unlike TorchInductor, which generates Triton code targeting NVIDIA GPUs, XLA generates HLO (High-Level Operations) intermediate representation that targets multiple backends, including Google TPUs, NVIDIA GPUs, and CPUs.

XLA's key strength is **whole-program compilation**. While `torch.compile` may insert graph breaks at dynamic control flow, XLA traces the entire computation as a single static graph. This enables global optimization: memory planning across the full forward and backward passes, cross-layer fusion, and layout optimization that reshapes tensors to match hardware preferences (e.g., TPU's preference for 128-element aligned dimensions).

For TPU hardware specifically, XLA performs optimizations unavailable on other platforms. It maps matrix multiplications onto the TPU's systolic array architecture, padding matrix dimensions to align with the 128 $\times$ 128 systolic array size. It schedules communication operations (for distributed training) to overlap with computation using the TPU's dedicated Inter-Core Interconnect (ICI). These hardware-specific optimizations are why JAX/TPU often achieves higher Model FLOPs Utilization (MFU) than PyTorch/GPU for equivalent workloads.

The trade-off for XLA's whole-program approach is compilation latency. Because XLA must trace and compile the entire program before execution begins, the initial compilation can take minutes for large models. Any change in input shape triggers a recompilation. In contrast, `torch.compile` with TorchDynamo compiles incrementally, one subgraph at a time, and can handle dynamic shapes (with reduced optimization). This compilation latency trade-off mirrors a broader theme in systems engineering: static analysis enables better optimization but requires more upfront cost and less runtime flexibility.

The choice between `torch.compile` and XLA often depends on the deployment context rather than the raw performance difference. Organizations using NVIDIA GPUs predominantly use PyTorch with `torch.compile` (or TensorRT for inference). Organizations using Google TPUs use JAX with XLA. The compilation framework is increasingly a function of the hardware ecosystem rather than the model architecture.

### TensorRT: Inference Optimization {#sec-performance-engineering-tensorrt}

NVIDIA TensorRT is a specialized inference compiler that applies aggressive optimizations not suitable for training. Because inference does not require gradient computation, TensorRT can:

- **Quantize selectively**: Apply INT8 or FP8 quantization to layers that tolerate it, keeping sensitive layers in FP16. TensorRT's calibration process benchmarks each layer's quantization sensitivity and produces a per-layer precision assignment.
- **Eliminate unused paths**: Remove training-only operations (dropout, batch normalization running statistics updates).
- **Optimize for static shapes**: When input shapes are known at compile time, TensorRT generates kernels tuned for exact dimensions, avoiding the overhead of dynamic shape handling.
- **Plan memory precisely**: With no gradient tensors needed, peak memory is substantially lower, allowing aggressive batch size or sequence length increases.

TensorRT applies layer fusion, kernel autotuning (benchmarking hundreds of kernel variants for each operation), and precision calibration (determining per-layer quantization sensitivity from a calibration dataset). The result is typically 2--5 $\times$ faster inference compared to the unoptimized framework model, with the exact speedup depending on model architecture and input shapes.

The trade-off for TensorRT's aggressive optimization is reduced flexibility. A TensorRT engine is compiled for a specific GPU architecture, input shape range, and precision configuration. Changing any of these requires recompilation. For serving systems where the model and hardware are fixed for months, this is acceptable. For research environments where models change daily, the compilation overhead is prohibitive, and `torch.compile` with its faster compilation is more appropriate.
...
Autoregressive LLM generation is inherently sequential: each token depends on all previous tokens. The roofline analysis in @sec-performance-engineering-memory-wall showed that single-token decode is deeply memory-bound, achieving less than 1% of peak FLOPS on an H100. Each step reads the entire weight matrix from HBM to produce one token, wasting massive compute capability. **Speculative Decoding** breaks this inefficiency by generating multiple tokens per large-model forward pass, using a small "draft model" to predict likely continuations and the large "target model" to verify them in parallel.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Speculative Decoding section
# │
# │ Goal: Compute expected speedup for different acceptance rates.
# │ Show: How acceptance probability and draft length affect throughput.
# │ How: E[accepted tokens] = sum of geometric probabilities.
# │
# │ Imports: (none, standalone)
# │ Exports: speedup strings for different scenarios
# └─────────────────────────────────────────────────────────────────────────────
...
def expected_tokens(alpha, k):
    ...
    # Sum of alpha^0, alpha^1, ..., alpha^k (we always get at least 1 from correction)
    total = sum(alpha**i for i in range(k + 1))
    return total

def speedup(alpha, k, t_target_norm, t_draft_total_norm):
    """Speedup over standard autoregressive."""
    et = expected_tokens(alpha, k)
    # Time per round = draft time + 1 target verify
    time_per_round = t_draft_total_norm + t_target_norm
    # Baseline: et tokens * t_target each
    baseline_time = et * t_target_norm
    return baseline_time / time_per_round

sp_high = speedup(alpha_high, K, t_target, t_draft_total)
sp_med = speedup(alpha_med, K, t_target, t_draft_total)
sp_low = speedup(alpha_low, K, t_target, t_draft_total)

et_high = expected_tokens(alpha_high, K)
et_med = expected_tokens(alpha_med, K)
et_low = expected_tokens(alpha_low, K)
...
Speculative decoding's resource implications extend beyond the compute and latency analysis above. The draft model requires its own GPU memory allocation: weights, KV cache, and activation memory. For a 1.5B draft model in FP16, the weights consume approximately 3 GB. If the draft model shares the target model's GPU (the common case), this 3 GB reduces the memory available for the target model's KV cache, slightly reducing the maximum batch size.

A more significant cost arises from the draft model's KV cache. If the draft model generates $K = 5$ tokens per round, it must maintain its own KV cache for the full sequence (including all previously generated tokens). For a long conversation with 4,000 tokens of context, the draft model's KV cache can consume several hundred megabytes. When serving many concurrent requests, each with its own draft KV cache, the aggregate memory pressure can be substantial.

Some implementations mitigate this by sharing architectural components between the draft and target models. If the draft model is a distilled version of the target (same vocabulary, same tokenizer, smaller hidden dimension), the embedding layers can be shared. If the draft model is the first $n$ layers of the target (self-speculation), no additional weight memory is needed at all, though the KV cache for the draft "model" overlaps with the first $n$ layers of the target's KV cache.
...
Scaling model parameters improves capability. Scaling inference compute degrades latency and economics. Conventional dense models force a direct trade-off: a model with $N$ parameters requires reading and computing with all $N$ parameters for every input. **Mixture of Experts (MoE)** decouples these two quantities by activating only a subset of parameters for each input token. A model with 400B total parameters might activate only 50B per token, achieving the capacity of a 400B model with the inference cost of a 50B model.
...
# │ MOE ECONOMICS
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: MoE section — compare dense vs MoE inference cost
# │
# │ Goal: Quantify memory and compute savings from sparse activation.
# │ Show: Dense 400B vs MoE with 50B active parameters.
# │ How: Weight memory and per-token FLOP comparison.
# │
# │ Imports: (none, standalone)
# │ Exports: comparison strings
# └─────────────────────────────────────────────────────────────────────────────
...
**Expert collapse** occurs when the router converges to sending most tokens to a small subset of experts, leaving the remaining experts undertrained. In extreme cases, a model with 64 experts may effectively use only 4--8, wasting the memory and compute allocated to the remaining experts. The auxiliary loss (@eq-moe-router) addresses this, but if the auxiliary loss coefficient $\alpha$ is too small, the quality-driven gradient overwhelms the load-balancing signal. If $\alpha$ is too large, the model prioritizes uniform routing over quality, degrading outputs. Finding the right balance typically requires hyperparameter sweeps.
...
**Distribution shift** between training and serving creates a more subtle problem. During training, the router sees a specific distribution of input tokens (determined by the training data). During serving, the input distribution may differ (different languages, topics, or styles). If the router's learned preferences are distribution-dependent, some experts may be overloaded or underutilized at serving time, even if load balancing was excellent during training. Monitoring expert utilization in production and retraining or fine-tuning the router on representative serving data addresses this gap.
...
In distributed ML systems, communication between GPUs (gradient synchronization, activation transfers, expert routing) and computation (GEMMs, attention, element-wise operations) are traditionally executed sequentially: compute a gradient, then communicate it, then compute the next gradient. This sequential execution wastes time because the GPU's compute units are idle during communication, and the network is idle during computation. **Communication-computation overlap** hides network latency by executing communication and computation concurrently on separate hardware resources.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Overlap section — quantify overlap benefit for data parallelism
# │
# │ Goal: Compare step time with and without gradient overlap.
# │ Show: Step time breakdown for a 7B model on 8 GPUs with NVLink.
# │ How: Compute gradient size, AllReduce time, backward time.
# │
# │ Imports: mlsys.constants (NVLINK_H100_BW, H100_FLOPS_FP16_TENSOR)
# │ Exports: timing strings for prose
# └─────────────────────────────────────────────────────────────────────────────
...
The mechanism enabling overlap on NVIDIA GPUs is **CUDA streams**. A CUDA stream is an ordered sequence of GPU operations (kernel launches, memory copies, NCCL collectives) that execute sequentially within the stream but can execute concurrently with operations in other streams. By placing compute kernels on one stream and communication operations on another, the GPU hardware scheduler can dispatch both simultaneously, provided they do not compete for the same resources.

In practice, achieving effective overlap requires attention to several details. First, the communication operation must be launched *before* the compute kernel it should overlap with, not after, because NCCL operations have their own launch overhead. Second, compute and communication should not contend for the same SM resources; modern NCCL implementations use dedicated SMs or copy engines for communication, but poorly configured systems may experience contention. Third, synchronization points (where one stream waits for another) must be minimized, as each synchronization serializes execution.
...
Performance engineering without measurement is guesswork. The techniques in this chapter, operator fusion, precision reduction, speculative decoding, and communication overlap, each target different bottlenecks. Applying the wrong optimization wastes engineering effort and can even degrade performance. **System Profiling** provides the diagnostic capability to identify the actual bottleneck, quantify its severity, and verify that an optimization addresses it.
...
## The Optimization Playbook {#sec-performance-engineering-playbook}

The techniques in this chapter form a coherent optimization stack, not a menu of independent options. Applying them in the right order matters, because each technique changes the bottleneck landscape, potentially making the next technique more or less valuable. This section provides a decision framework for selecting and sequencing optimizations.
...
::: {.callout-definition title="Definition of Model FLOPs Utilization"}

**Model FLOPs Utilization (MFU)** is the ratio of observed throughput (measured in FLOPs/second based on the model's theoretical FLOP count per sample) to the hardware's peak FLOP/second capacity. Unlike hardware FLOPs utilization, which counts all arithmetic operations including overhead, MFU counts only the FLOPs that directly contribute to the model's computation. MFU therefore captures both hardware efficiency and software overhead in a single metric.
...
To illustrate how the techniques in this chapter compose in practice, consider the task of optimizing a 70B parameter LLM for production serving. The target is a real-time chatbot application requiring time-to-first-token (TTFT) under 500 ms, inter-token latency (ITL) under 50 ms, and throughput of at least 1,000 tokens/second across the cluster. The model is deployed on a node of 8 H100 GPUs connected by NVLink.
...
::: {.callout-chapter-connection title="From Datacenter to Device"}

We have optimized the individual operations, memory flows, and algorithmic strategies that make ML efficient within the datacenter. These techniques assume abundant power, high-bandwidth interconnects, and terabytes of memory.

In **Edge Intelligence** (@sec-edge-intelligence), we confront the opposite environment: milliwatts instead of kilowatts, kilobytes instead of gigabytes, and intermittent connectivity instead of always-on networking. The performance engineering principles from this chapter do not disappear at the edge; they intensify. Precision engineering becomes not an optimization but a necessity. Operator fusion is not a speedup but a survival strategy. The physics of the memory wall is the same, but the margins are unforgiving.
