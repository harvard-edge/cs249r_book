...
# │ Exports: MemoryWallScenario.h100_fp8_tflops_str, MemoryWallScenario.h100_hbm_bw_tb_s_str,
...
class MemoryWallScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp8_tflops_str = fmt_qty_int(flops_fp8, TFLOP/second, commas=False)
    h100_hbm_bw_tb_s_str = fmt_qty(bw, TB/second, precision=2, commas=False)
    ...
    h100_mem_gb_str = fmt_qty(mem, GiB, precision=0, commas=False, unit_label="GB")
    h100_tdp_w_str = fmt_power(tdp, unit=watt, precision=0, commas=False)
    register_total_mib_str = fmt_qty_int(register_total, MiB, commas=False, unit_label="MB")
    ...
    reg_per_sm_kib_str = fmt_qty_int(reg_per_sm, KiB, commas=False, unit_label="KB")
    shared_mem_per_sm_kib_str = fmt_qty_int(shared_per_sm, KiB, commas=False, unit_label="KB")
```

An NVIDIA H100 delivers `{python} MemoryWallScenario.h100_fp8_tflops_str` of FP8 compute but only `{python} MemoryWallScenario.h100_hbm_bw_tb_s_str` of memory bandwidth. If every byte loaded from memory does fewer than `{python} MemoryWallScenario.h100_ridge_fp8_str` arithmetic operations, the compute units sit idle, starved for data. This gap between compute capability and memory delivery rate is the memory wall, and it defines the landscape within which all performance engineering operates.
...
class RooflineRidgeCalc:
    ...
    a100_fp16_tflops_str = fmt_qty_int(h_a100.compute.peak_flops, TFLOP/second, commas=False)
    a100_hbm_bw_tb_s_str = fmt_qty(h_a100.memory.bandwidth, TB/second, precision=1, commas=False)
    h100_fp8_tflops_str = fmt_qty_int(h_h100.compute.precision_flops["fp8"], TFLOP/second, commas=False)
    h100_hbm_bw_tb_s_str = fmt_qty(h_h100.memory.bandwidth, TB/second, precision=2, commas=False)
```

Any operation performing fewer than `{python} RooflineRidgeCalc.h100_fp16_ridge_str` floating-point operations per byte loaded is memory-bound on the H100 at FP16. At FP8 precision, where compute doubles to `{python} RooflineRidgeCalc.h100_fp8_tflops_str` and bandwidth remains `{python} RooflineRidgeCalc.h100_hbm_bw_tb_s_str`, the ridge point rises to approximately `{python} RooflineRidgeCalc.h100_fp8_ridge_str` FLOP/byte. The A100, with `{python} RooflineRidgeCalc.a100_fp16_tflops_str` and `{python} RooflineRidgeCalc.a100_hbm_bw_tb_s_str`, has a lower ridge point of approximately `{python} RooflineRidgeCalc.a100_ridge_str` FLOP/byte at FP16. Each hardware generation increases compute faster than bandwidth, pushing the ridge point higher and making more workloads memory-bound.
...
class RooflineDiagnostic:
    ...
    tps_tokens_s_str = fmt_rate(tokens_per_sec.to(1 / second).magnitude, "tokens/s", precision=1, commas=False)
```
...
New time per token: `{python} CompilationDividend.gemm_ms_str` + `{python} CompilationDividend.elem_ms_str` + `{python} CompilationDividend.launch_ms_str` = `{python} CompilationDividend.new_time_ms_str`, yielding approximately `{python} CompilationDividend.new_tps_str`/second.
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: Roofline diagnosis — LayerNorm profiling example.
# │ Exports: MemoryWallProfilingRecap.h100_hbm_bw_tb_s_str
class MemoryWallProfilingRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_hbm_bw_tb_s_str = fmt_qty(Hardware.Cloud.H100.memory.bandwidth, TB/second, precision=2, commas=False)
```
...
```
Consider a concrete example: a LayerNorm kernel profiled on an H100 reports `{python} LayerNormRoofline.achieved_tflops_str` of achieved compute and `{python} LayerNormRoofline.achieved_tbs_str` of achieved memory bandwidth. Its arithmetic intensity is `{python} LayerNormRoofline.achieved_tflops_str` / `{python} LayerNormRoofline.achieved_tbs_str` $\approx$ `{python} LayerNormRoofline.arithmetic_intensity_str` FLOP/byte. The H100's ridge point is approximately `{python} RooflineProfilingRecap.h100_fp16_ridge_str` FLOP/byte at FP16. Since `{python} LayerNormRoofline.arithmetic_intensity_str` is far below `{python} RooflineProfilingRecap.h100_fp16_ridge_str`, the kernel is strictly memory-bound. Its achieved `{python} LayerNormRoofline.achieved_tbs_str` (84 percent of the H100's `{python} MemoryWallProfilingRecap.h100_hbm_bw_tb_s_str` peak bandwidth) confirms it is operating near the physical limit of the memory subsystem. The diagnosis is clear: further FLOP-level optimizations will yield little gain; performance can only be improved materially by reducing data movement, either through fusion (eliminating the HBM round-trip) or precision reduction (halving the bytes per element).
...
class ProfilerDetective:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    observed_tps_tokens_s_str = fmt_rate(round(observed_tps), 'tokens/s', precision=0, commas=False)
    ceiling_tps_tokens_s_str = fmt_rate(round(ceiling_tps), 'tokens/s', precision=0, commas=False)
...
**Problem**: A 7B parameter LLM runs on a single H100 and shows `{python} ProfilerDetective.observed_tps_tokens_s_str` during autoregressive generation at batch size 1. The pure FP16 weight-read roofline is higher, but after budgeting roughly `{python} ProfilerDetective.decode_traffic_gb_str` of total per-token traffic for weights, KV-cache reads, sampling, synchronization, and launch overhead, the bandwidth-limited full-decode ceiling is approximately `{python} ProfilerDetective.ceiling_tps_tokens_s_str`. Where is the remaining `{python} ProfilerDetective.hidden_performance_pct_str` of realizable decode-step performance hiding?

**Investigation**:

Step 1: Kernel-level analysis. Run Nsight Compute on the dominant GEMM kernel. Result: achieves `{python} ProfilerDetective.gemm_bandwidth_tbs_str` effective bandwidth out of the H100's `{python} MemoryWallProfilingRecap.h100_hbm_bw_tb_s_str` peak. Efficiency: `{python} ProfilerDetective.gemm_efficiency_pct_str`. This kernel is performing well.
