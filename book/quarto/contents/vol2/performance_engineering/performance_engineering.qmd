...
|:---|:---|:---|:---|
| **Model FLOPs Utilization (MFU)** | Fraction of hardware peak FLOP/s productively used by model computation | Training efficiency across memory limits, launch overhead, communication wait, and software overhead | 40--60 percent is common in well-tuned LLM training; above 60 percent is excellent |
...
| **Model Bandwidth Utilization (MBU)**\index{Model Bandwidth Utilization} | Achieved memory bandwidth divided by hardware peak bandwidth | Memory-bound inference, especially decode workloads | An H100 decode step can show 2 percent MFU but 85 percent MBU, which indicates a memory-saturated and well-optimized path |
...
| **Gray failure detection**\index{Gray Failure Detection} | Distribution of step times across the fleet | Straggler nodes, such as one 10 percent slower node that can reduce synchronous data-parallel MFU by 10 percent |
