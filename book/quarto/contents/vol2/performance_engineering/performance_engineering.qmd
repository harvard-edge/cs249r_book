...
# │ How: Naive = Q/K/V + score + prob + output traffic; Flash = Q/K/V + output.
# │
...
class FlashAttentionSavings:
    ...
    # Read Q/K/V once, materialize S and P with write+read, then write O.
    naive_attn_bytes_per_head = (
        4 * seq_len * seq_len + 4 * seq_len * head_dim
    ) * bytes_per_elem
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    naive_mb_str = f"{naive_attn_bytes_per_head/MILLION:.0f}"
    flash_mb_str = f"{flash_attn_bytes_per_head/MILLION:.0f}"
...
The HBM traffic reduction is dramatic. For a sequence length of `{python} FlashAttentionSavings.head_n_str`, 32 heads, and head dimension `{python} FlashAttentionSavings.head_d_str` in FP16, the na\"ive attention moves approximately `{python} FlashAttentionSavings.naive_mb_str` MB of HBM traffic per head. FlashAttention moves approximately `{python} FlashAttentionSavings.flash_mb_str` MB per head. This is a `{python} FlashAttentionSavings.savings_str`$\times$ reduction in HBM traffic, translating directly into a proportional speedup for this memory-bound operation.
...
The na\"ive algorithm computes $S = QK^T$, a matrix of shape $8192 \times 8192$. In FP16, this score matrix occupies $8192 \times 8192 \times 2 = 128$ MB per head. After applying softmax, the result $P = \text{softmax}(S)$ is also $128$ MB. Computing $PV$ requires reading $P$ again. In total, the na\"ive algorithm reads $Q$, $K$, $V$ from HBM (6 MB), writes $S$ (128 MB), reads $S$ for softmax (128 MB), writes $P$ (128 MB), reads $P$ for the final multiply (128 MB), and writes $O$ (2 MB). The total HBM traffic is approximately 545 MB per head, dominated by the quadratic intermediates.
