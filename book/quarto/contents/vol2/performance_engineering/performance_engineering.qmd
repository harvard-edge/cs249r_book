...
class MemoryWallScenario:
    ...
    ridge_fp8 = (flops_fp8 / bw).to(flop/byte).magnitude
    ...
    reg_per_sm_kib = reg_per_sm.to(KiB).magnitude
    shared_per_sm_kib = shared_per_sm.to(KiB).magnitude
    register_total_mb = (sm_count * reg_per_sm).to(MiB).magnitude
    l2_cache_mb = l2_cache.to(MB).magnitude
    hbm_latency_ns = hbm_latency.to(nanosecond).magnitude
    hbm_energy_pj = energy_dram.to(pJ).magnitude
    ...
    energy_ratio_str = fmt_int(energy_ratio.to("").magnitude, commas=False)
    h100_mem_str = fmt_qty(mem, GiB, precision=0, commas=False, unit_label="GB")
    h100_tdp_str = fmt_int(tdp.to(watt).magnitude, commas=False)
    register_total_mb_str = fmt_qty_int(sm_count * reg_per_sm, MiB, commas=False, unit_label="MB")
    ...
    hbm_energy_pj_str = fmt_qty_int(energy_dram, pJ, commas=False, unit_label="pJ")
    sm_count_str = fmt_int(sm_count, commas=False)
    reg_per_sm_str = fmt_qty_int(reg_per_sm, KiB, commas=False, unit_label="KB")
    shared_mem_per_sm_str = fmt_qty_int(shared_per_sm, KiB, commas=False, unit_label="KB")
```
...
# │       FLOP/byte using .to(flop/byte).magnitude; guard ordering with check().
# │
...
class RooflineRidgeCalc:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    a100_ridge_val = h_a100.ridge_point().to(flop/byte).magnitude
    h100_ridge_fp16_val = h_h100.ridge_point().to(flop/byte).magnitude
    h100_ridge_fp8_val = h_h100.ridge_point("fp8").to(flop/byte).magnitude
...
class RooflineDiagnostic:
    ...
    params_per_gpu_B = Models.Language.Llama2_70B.parameters.to(Bparam).magnitude / tensor_parallel_degree
    bytes_fp16 = BYTES_FP16.to(byte).magnitude
    h100_bw_tbs = Hardware.Cloud.H100.memory.bandwidth.to(TB/second).magnitude  # H100 HBM3 bandwidth (TB/s)
...
class FusionTrafficCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    tensor_mb = (tensor_bytes * byte).to(MB).magnitude
    traffic_mb = (traffic_bytes * byte).to(MB).magnitude
...
class KVCacheAnalysis:
    ...
    gpu_total_gb = Hardware.Cloud.H100.memory.capacity.to(GiB).magnitude
    int4_bits = 4
    fp16_bits = 16
    ...
    kv_fp16_gb_val = kv_cache_qty_f16.to(GB).magnitude

    kv_cache_qty_i8 = calc_kv_cache_size(
        model.layers, model.kv_heads, model.hidden_dim // model.heads,
        seq_len, 1, bytes_per_elem=BYTES_INT8
    )
    kv_int8_gb_val = kv_cache_qty_i8.to(GB).magnitude
...
class NvlinkH100:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    bw_gbps = bw.to(GB/second).magnitude
...
class OverlapCalc:
    ...
    forward_flops = 2 * m.parameters.to(param).magnitude * tokens_per_gpu * ureg.flop
    ...
    gradient_qty = m.parameters.to(param).magnitude * 2 * ureg.byte
    ...
    allreduce_time_s = t_comm.to(second).magnitude
    ...
    backward_time_s = (backward_flops / (peak_flops * mfu)).to(second).magnitude

    # Step 4: Forward pass compute time
    forward_time_s = (forward_flops / (peak_flops * mfu)).to(second).magnitude
...
class RooflineProfilingRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp16_ridge_str = fmt(Hardware.Cloud.H100.ridge_point().to(flop/byte).magnitude, precision=1, commas=False)
```
...
class LayerNormRoofline:
    ...
    achieved_tflops = achieved_flops.to(TFLOPs/second).magnitude
    achieved_bw = 2.8 * (TB / second)
    achieved_tbs = achieved_bw.to(TB/second).magnitude
...
class ProfilerDetective:
    ...
    decode_traffic_gb = decode_traffic.to(GB).magnitude
    hidden_performance_pct = 53
    gemm_bandwidth = 2.8 * (TB / second)
    gemm_bandwidth_tbs = gemm_bandwidth.to(TB/second).magnitude
    ...
    kv_cache_bandwidth_tbs = kv_cache_bandwidth.to(TB/second).magnitude
...
class FleetEfficiencyCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    n_gpus = 128
    r_peak_per_gpu = Hardware.Cloud.H100.compute.peak_flops.to(TFLOPs/second).magnitude * TRILLION
    ...
    local_mfu = local_flops_per_step / (8 * r_peak_per_gpu * (t_local_step_ms * ms).to(second).magnitude)
    global_mfu = fleet_flops_per_step / (n_gpus * r_peak_per_gpu * (t_fleet_step_ms * ms).to(second).magnitude)
