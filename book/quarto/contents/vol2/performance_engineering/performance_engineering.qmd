...
PagedAttention and KV cache quantization interact multiplicatively. PagedAttention reduces memory *waste* (from fragmentation), while quantization reduces memory *usage* (from precision). Together, they increase the effective batch size by 8--16$\times$ compared to a baseline system with pre-allocated FP16 KV caches, fundamentally changing the economics of LLM serving.
