...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=stealth, scale=1.0]
  ...
  % Workload markers
  \fill[OrangeLine] (1.2,1.2) circle (4pt);
  \node[right, font=\scriptsize, OrangeLine] at (1.4,1.0) {LLM Decode (B=1)};

  \fill[OrangeLine] (1.8,1.8) circle (4pt);
  \node[right, font=\scriptsize, OrangeLine] at (2.0,1.6) {Element-wise};

  \fill[GreenLine] (7.5,5.5) circle (4pt);
  \node[below, font=\scriptsize, GreenLine] at (7.5,5.3) {Large GEMM};

  \fill[VioletLine] (3.8,3.8) circle (4pt);
  \node[right, font=\scriptsize, VioletLine] at (4.0,3.6) {Attention};
