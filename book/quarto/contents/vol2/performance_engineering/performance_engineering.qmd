...
# │
# │ Imports: mlsysim public API (Hardware, units, fmt helpers)
# │ Exports: MemoryWallOpening.h100_fp16_tflop_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
The multi-dimensional nature of this frontier makes optimization challenging. Five dimensions span the efficiency frontier:
...
# │
# │ Imports: mlsysim public API (Hardware, units, fmt helpers)
# │ Exports: WideningGap.{v100,a100,h100,b200}_{peak_tflop_per_s,bw_tb_per_s,ridge}_str,
# │          WideningGap.compute_growth_mult_str, WideningGap.bw_growth_mult_str,
# │          WideningGap.ridge_ratio_mult_str, WideningGap.v100_ridge_str,
# │          WideningGap.b200_ridge_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_magnitude, fmt_multiple, check
...
class WideningGap:
    ...
    ridge_ratio_mult_str = fmt_multiple(ridge_ratio, precision=1, commas=False)
```
...
# │          WorkloadIntensityCalc.gemm_intensity_table_str,
# │          WorkloadIntensityCalc.gemm_flops_billion_str,
# │          WorkloadIntensityCalc.gemm_bytes_mb_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim Literature registry + fmt helpers
# │ Exports: BatchMfuAnchors.mfu_b1_pct_str, .mfu_batched_pct_str,
# │          .cost_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
class RooflineDiagnostic:
    ...
    ai_eq_math = fmt_math(
        ...
        rf"{fmt(arith_intensity.magnitude, precision=0, commas=False)} \text{{ FLOP/byte}}"
    )
...
# │
# │ Imports: mlsysim public API (Hardware, units, fmt helpers)
# │ Exports: FlashAttentionTiling.{block_kb,total_kb,shared_kb,tiles_per_dim}_str
# └─────────────────────────────────────────────────────────────────────────────
...
class FlashAttentionTiling:
    ...
    seq_len = 8192  # sequence length (matches the materialized-state example above)
    bytes_per_elem = BYTES_FP16
    shared_per_sm = Hardware.Cloud.H100.memory.shared_memory_per_sm
...
# │
# │ Imports: mlsysim public API (Hardware, Models, units, fmt helpers)
# │ Exports: NvlinkH100.bw_gb_per_s_str, NvlinkH100.b1_vol_kb_str, NvlinkH100.b1_time_ns_str,
# │          NvlinkH100.b64_vol_mb_str, NvlinkH100.b64_time_us_str
# └─────────────────────────────────────────────────────────────────────────────
...
class NvlinkH100:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt, fmt_qty, fmt_qty_int, fmt_time, check
...
The roofline model from @sec-performance-engineering-roofline becomes a diagnostic tool when combined with profiling data. Diagnosis follows three steps:
...
Four patterns reveal the most diagnostic information from a Nsight Systems trace:
