...
h100_mem_str = fmt(PerformanceSetup.h100_mem, "GB", precision=0)
...
b200_mem_str = fmt(PerformanceSetup.b200_mem, "GB", precision=0)
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ WORKLOAD INTENSITY ANALYSIS (P.I.C.O.)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-performance-engineering-workload-placement — classify ML ops
# │
# │ Goal: Compute arithmetic intensity (FLOP/byte) for a 4096×4096 FP16 GEMM,
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ FLASHATTENTION MEMORY SAVINGS (P.I.C.O.)
# ├─────────────────────────────────────────────────────────────────────────────
...
#| label: kv-cache-size
# ┌─────────────────────────────────────────────────────────────────────────────
# │ KV CACHE MEMORY PER REQUEST (P.I.C.O.)
# ├─────────────────────────────────────────────────────────────────────────────
...
class KVCacheAnalysis:
    ...
    # FP16 case: 45GB available for KV
...
The first step is baseline measurement: measure end-to-end throughput (tokens/second for LLMs, samples/second for training) and collect a Nsight Systems trace. Compute the Model FLOPs Utilization (MFU) as defined above. An MFU below 30% indicates substantial optimization opportunity; above 50% is good; above 60% is excellent for large models.

The second step is roofline classification. For the dominant kernels, compute the arithmetic intensity and plot on the roofline. This immediately identifies whether the workload is compute-bound or memory-bound, which determines the entire optimization strategy.

The third step is selecting the primary bottleneck. The primary bottleneck falls into one of three categories, each with a different optimization path:
