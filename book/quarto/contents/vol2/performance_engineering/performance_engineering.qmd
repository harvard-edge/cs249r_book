...
class MemoryWallScenario:
    ...
    h100_ridge_fp8_str = fmt(ridge_fp8.to(flop/byte).magnitude, precision=1, commas=False)
    ...
    register_total_mb_str = fmt_qty_int(register_total, MiB, commas=False, unit_label="MB")
...
class RooflineRidgeCalc:
    ...
    a100_ridge_str = fmt(a100_ridge.to(flop/byte).magnitude, precision=1, commas=False)
    h100_fp16_ridge_str = fmt(h100_ridge_fp16.to(flop/byte).magnitude, precision=1, commas=False)
    h100_fp8_ridge_str = fmt(h100_ridge_fp8.to(flop/byte).magnitude, precision=1, commas=False)
...
from mlsysim.fmt import fmt_int, check, fmt_count, fmt_memory, fmt_rate, fmt_time
...
class RooflineDiagnostic:
    ...
    ai_str = fmt_int(arith_intensity.to(flop/byte).magnitude, commas=False)
    decode_ms_str = fmt_time(decode_time, millisecond, precision=1, commas=False)
    tps_str = fmt_rate(tokens_per_sec.to(1 / second).magnitude, "tokens/s", precision=1, commas=False)
```
...
class KVCacheAnalysis:
    ...
    gpu_total = 80 * GB  # scenario assumption: vendor-facing H100 memory budget
    int4_bits = 4
    fp16_bits = 16
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt_int, fmt, fmt_count, fmt_memory
    ...
    avail_fp16_gb_str = fmt_memory(avail_fp16, unit=GB, precision=0, commas=False)
    weight_int4_gb_str = fmt_memory(weight_int4_shard, unit=GB, precision=2, commas=False)
    avail_int8_gb_str = fmt_memory(avail_int8, unit=GB, precision=2, commas=False)
    weight_fp16_shard_gb_str = fmt_memory(weight_fp16_shard, unit=GB, precision=0, commas=False)
...
class RooflineSummaryRecap:
    ...
    h100_fp16_ridge_str = fmt(h100_fp16_ridge.to(flop/byte).magnitude, precision=1, commas=False)
```
