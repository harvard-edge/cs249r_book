...
\begin{scope}[local bounding box=WORK1]
  ...
  \node[Box] (B2) at (18:\ra) {Collect\\Data};
  \node[Box2] (B5) at (306:\ra) {Preprocess\\data};
  \node[Box2] (B4) at (234:\ra) {Train\\model};
  \node[Box2] (B3) at (162:\ra) {Evaluate};
\end{scope}

% AutoML Workflow
\begin{scope}[local bounding box=WORK2, shift={(8,0)}]
  \node[font=\bfseries, align=center] (title2) at (0, \ra+15mm) {AutoML\\workflow};

  \draw[Arrow] (117:\ra) arc[radius=\ra, start angle=117, end angle=450];

  \node[Box] (A1) at (90:\ra) {Define\\problem};
  \node[Box] (A2) at (330:\ra) {Collect\\data};
  \node[Box2] (A3) at (210:\ra) {AutoML};
\end{scope}
