...
Distributed, adaptive machine learning systems present fundamentally different threat landscapes than centralized training architectures. Contemporary machine learning systems operate across heterogeneous computational environments spanning edge devices, federated networks, and hybrid cloud deployments (@sec-edge-intelligence). This architectural evolution enables new capabilities in adaptive intelligence while introducing attack vectors and privacy vulnerabilities that traditional cybersecurity frameworks cannot adequately address.
...
Our defense stack proceeds bottom-up, from foundational hardware trust anchors through system-level monitoring to model-layer protections and finally data-layer privacy. The subsequent sections on hardware security foundations will establish the TEEs, secure boot, HSMs, and PUFs that anchor system trust. With those foundations in place, we now examine the highest layer: data privacy techniques that protect sensitive information at the point where it enters the ML pipeline. This organizational logic ensures that each defensive layer builds on the trust guarantees established by the layer below it.
...
Here, $\theta_{t}^{(k)}$ represents the model update from client $k$, $n_k$ the number of samples held by that client, and $n$ the total number of samples across all clients. This weighted aggregation allows the global model to learn from distributed data without direct access to it.

**Gradient Inversion Risks**

However, federated learning does not guarantee privacy by default. **Gradient inversion attacks** (such as Deep Leakage from Gradients) demonstrate that an adversary with access to a client's raw update can reconstruct the original training data (e.g., images or text) with high fidelity. The gradient vector contains information about the data that produced it.

This vulnerability drives the need for **Secure Aggregation** protocols [@bonawitz2017practical] and local differential privacy. Secure aggregation ensures the server only sees the *sum* of updates, never an individual update, mathematically preventing inversion of any single user's data.

FL reduces the exposure of raw data, but still leaks information through gradients [@zhu2019deep; @nasr2019comprehensive], motivating the use of DP, secure aggregation [@bonawitz2017practical], and hardware-based protections in federated settings.
