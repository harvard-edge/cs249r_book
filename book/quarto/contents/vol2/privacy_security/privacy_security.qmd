...
**Minimal Attack Surface**: Every exposed interface, stored artifact, and network endpoint represents potential attack surface. Defensive design minimizes unnecessary exposure by restricting API capabilities to essential functionality, limiting model output information through confidence score truncation, encrypting stored models and training data, isolating training infrastructure from inference systems, and implementing strict access controls with audit logging.
...
To achieve target $\epsilon = 3.0$, we must either: (1) reduce $\sigma$ to 0.7 (more noise, accuracy drops ~5%), (2) train for fewer epochs ($T = 25$, underfitting risk), or (3) increase batch size to $B = 8000$ (better sampling, requires more memory).
