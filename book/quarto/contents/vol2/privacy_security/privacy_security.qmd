...
The threat prioritization framework established in the previous section identified high-likelihood attacks such as model extraction and data poisoning, as well as high-impact attacks like hardware side-channels, each demanding different defensive investments. With this analytical foundation in place, we now examine the specific mechanisms of each major attack category, providing the technical understanding needed to implement appropriate defenses against the risks identified in our threat analysis.
...
While hardware vulnerabilities represent attacks targeting ML infrastructure, the threat landscape extends beyond systems as targets. A complete security analysis must also consider the inverse scenario: machine learning systems can themselves become instruments of attack. Understanding this dual nature, where ML serves as both target and weapon, is essential for comprehensive threat modeling and informs the defensive architectures we develop in subsequent sections.

The threats examined thus far, including model theft, data poisoning, adversarial attacks, and hardware vulnerabilities, all position machine learning systems as assets to protect. However, the same capabilities that make ML powerful for beneficial applications also enhance adversarial operations, transforming machine learning from passive target to active weapon. This section examines how attackers weaponize ML techniques, completing our threat model before we turn to defensive countermeasures.
