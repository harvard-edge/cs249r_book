...
::: {.callout-note title="Figure Placeholder: Attack Surface" collapse="true"}
```{.tikz}
% TODO: Diagram showing Attack Vectors: Data (Poisoning) -> Model (Backdoor/Theft) -> API (Adversarial/Inversion) -> Hardware (Side-channel)
\node[draw, align=center] {ML Attack Surface\nData | Model | API | Hardware};
```
**ML System Attack Surface**. Visualizing entry points for adversarial actions across the ML lifecycle. Training data can be poisoned (input layer); model parameters can be extracted or backdoored (model layer); inference APIs can be exploited with adversarial examples or evasion attacks (interface layer); and physical hardware can be targeted via side-channels (infrastructure layer).
:::
...
::: {.callout-note title="Figure Placeholder: Threat Prioritization Matrix" collapse="true"}
```{.tikz}
% TODO: Create a 2x2 matrix visualizing the Threat Prioritization Framework
% X-axis: Likelihood (Low -> High)
% Y-axis: Impact (Low -> High)
% Quadrants:
% - High/High: Data Poisoning (Critical)
% - High/Low: Model Extraction (Common)
% - Low/High: Hardware Side-Channel (Specialized)
% - Low/Low: Deployment Anomalies (Routine)
\node[draw, align=center] { [Threat Prioritization Matrix] };
```
A 2x2 matrix classifying threats by Likelihood and Impact, helping practitioners prioritize defenses.
:::
...
::: {.callout-example title="Worked Example: Protecting a Production Image Classification API"}
...
::: {.callout-note title="Figure Placeholder: Data Poisoning Optimization Loop" collapse="true"}
```{.tikz}
% TODO: Visualize the bilevel optimization problem of data poisoning
% Outer Loop: Attacker optimizes poisoning data D_p
% Inner Loop: System trains model f on D + D_p
% Flow: D_p -> Training -> Model f -> Validation Loss -> Update D_p
\node[draw, align=center] { [Data Poisoning Bilevel Optimization Diagram] };
```
A conceptual diagram illustrating data poisoning as a bilevel optimization problem, where the attacker optimizes inputs to maximize model error on a target set.
:::
...
::: {.callout-note title="Figure Placeholder: Adversarial Decision Boundary" collapse="true"}
```{.tikz}
% TODO: Visual representation of a decision boundary in feature space.
% Show:
% 1. A clean data point (e.g., "Stop Sign") correctly classified.
% 2. A decision boundary separating classes.
% 3. A perturbation vector pushing the point across the boundary.
% 4. The resulting adversarial example classified as "Speed Limit".
\node[draw, align=center] { [Adversarial Decision Boundary Diagram] };
```
A visual representation of how small perturbations can push a data point across a high-dimensional decision boundary, causing misclassification.
:::

### Case Study: Traffic Sign Attack {#sec-security-privacy-case-study-traffic-sign-attack-6e93}

::: {.column-margin}
![**DALL-E Prompt**: A split screen comparison. Left: A standard Stop Sign. Right: The same Stop Sign with a subtle, pixelated "static" overlay that looks like digital noise but forms a hidden pattern. The background is a digital matrix. Cyber-security theme.](placeholder.png)
:::
...
::: {.column-margin}
![**DALL-E Prompt**: An intricate, isometric technical illustration of a microchip processor, glowing with blue and red zones indicating secure vs vulnerable areas. A magnifying glass hovers over a specific logic gate, revealing a "bug" icon. High-tech, blueprint style.](placeholder.png)
:::
...
::: {.callout-note title="Figure Placeholder: Hardware Supply Chain Attack Surface" collapse="true"}
```{.tikz}
% TODO: Flowchart of the hardware supply chain showing attack points.
% Stages: Design -> Fabrication -> Assembly -> Distribution -> Deployment
% Attack Vectors at each stage:
% - Design: Unintentional Bugs / Specification Flaws
% - Fabrication: Trojan Insertion / Counterfeits
% - Assembly: Component Substitution
% - Distribution: Interdiction / Tampering
\node[draw, align=center] { [Hardware Supply Chain Attack Diagram] };
```
A flowchart illustrating the hardware supply chain stages and the specific security risks present at each phase.
:::
...
::: {.callout-note title="Figure Placeholder: Differential Privacy" collapse="true"}
```{.tikz}
% TODO: Visualization of D and D-prime distributions overlapping with epsilon bound
\node[draw, align=center] {Differential Privacy\nIndistinguishability};
```
**Differential Privacy Mechanism**. Conceptual diagram of the randomized response mechanism. An individuals true data point (x) is perturbed by noise (Laplace or Gaussian) before aggregation. The output distribution for dataset D is indistinguishable from dataset D (without x), preventing inference of x while preserving the aggregate statistical signal.
:::
...
::: {.callout-note title="Figure Placeholder: Federated Privacy" collapse="true"}
```{.tikz}
% TODO: Diagram of Secure Aggregation (Masking + Summation)
\node[draw, align=center] {Secure Aggregation\nMasked Updates -> Zero-Sum Noise};
```
**Secure Aggregation in Federated Learning**. Visualization of the secure update protocol. Devices compute local updates $\Delta\theta$. Before uploading, devices add pairwise masking noise such that the sum of noise across all devices is zero. The server receives only the masked updates, aggregates them (canceling the noise), and recovers the true global update without ever seeing an individual devices raw contribution.
:::
