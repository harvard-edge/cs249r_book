...
-   **Legitimate User Impact**: Top-5 accuracy preserved at 99.2% (vs 99.3% baseline). Latency increase: +2ms for monitoring, +0.5ms for perturbation = +`{python} total_defense_ms_str`ms total (`{python} overhead_pct_str`% overhead on 100ms inference)
