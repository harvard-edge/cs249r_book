...
The Laplace distribution has density $p(x) = \frac{\epsilon}{2\Delta f}\exp\left(-\frac{\epsilon|x|}{\Delta f}\right)$, with scale parameter $b = \Delta f/\epsilon$. The noise magnitude scales inversely with $\epsilon$: stronger privacy ($\epsilon = 0.1$) requires 10× more noise than moderate privacy ($\epsilon = 1.0$).
