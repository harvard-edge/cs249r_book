...
***Priority Inversion***\index{Priority Inversion!definition} is a scheduling pathology in which a high-priority task is forced to wait for a lower-priority task to release a shared resource, effectively reducing the high-priority task's progress rate to that of the low-priority one. In distributed ML clusters, priority inversion typically manifests when a low-priority job holding GPUs is starved of auxiliary resources (CPU or network bandwidth) by medium-priority jobs, preventing it from checkpointing and releasing the resources needed by the high-priority job.
...
Proper scheduling yields a **4.8x speedup** purely by respecting the network topology, with zero changes to the model code.
:::
...
```{python}
#| label: fig-topology-placement
#| fig-cap: "Topology Placement Impact: Reducing AllReduce latency by 4.8x through topology-aware scheduling (Rack/Rail optimization) compared to random placement for a 1GB gradient sync on 64 GPUs."
...
***Elastic Training***\index{Elastic Training!definition} is the capability of a distributed training job to dynamically adjust its worker count without restarting, enabling training to continue through node failures or resource reallocation. Elasticity requires **gradient accumulation** recalibration and **learning rate** adjustment whenever the worker count changes, trading additional synchronization complexity for fault tolerance and resource efficiency.
...
```{python}
#| label: fig-cost-idle-gpus
#| fig-cap: "**The Cost of Idle GPUs**. Annual wasted cost as a function of cluster size for different GPU utilization levels, assuming $2/GPU-hour (cloud equivalent). The shaded region between 30% and 70% utilization represents the typical operating range for production clusters. At 10,000 GPUs with 50% utilization, organizations waste $87.6M annually. The Microsoft Philly data point marks the published median cluster utilization of 52%. Data sources: Jeon et al. (ATC 2019), Bhatele et al. (ISC HPC 2023)."
#| fig-alt: "Line chart of annual wasted cost in millions versus cluster size with four utilization curves at 30, 50, 70, and 90 percent and shaded typical range."
#| echo: false
