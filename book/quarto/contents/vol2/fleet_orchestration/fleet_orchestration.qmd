...
# │          util_high_pct_str, a100_mem_gb_str, h100_mem_gb_str
# └─────────────────────────────────────────────────────────────────────────────
...
class ClusterEconomicsContext:
    ...
    a100_mem_gb_str = fmt_qty(a100_mem, GiB, precision=0, commas=False, unit_label="GB")
...
# │          nvlink_h100_gb_s_str, ib_ndr_gbs_str
...
class FleetFragmentation:
    ...
    nvlink_h100_gb_s_str = fmt_qty(h_h100.nvlink.bandwidth, GB/second, precision=0)
...
# │ Exports: ClusterEconomicsMigRecap.a100_mem_gb_str, h100_mem_gb_str
...
# │ Exports: FleetTopologyInterconnect.nvlink_a100_gb_s_str, nvlink_h100_gb_s_str,
# │          ib_ndr_gbps_str, ib_ndr_gbs_str
...
class FleetTopologyInterconnect:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_gb_s_str = fmt_qty(h_h100.nvlink.bandwidth, GB/second, precision=0)
    nvlink_a100_gb_s_str = fmt_qty(h_a100.nvlink.bandwidth, GB/second, precision=0)
    ib_ndr_gbps_str = fmt_qty(ib_ndr.bandwidth, Gbps, precision=0, unit_label="Gb/s")
    ib_ndr_gbs_str = fmt_qty(ib_ndr.bandwidth, GB/second, precision=0)
...
Between nodes within the same rack or **leaf switch domain**\index{Leaf Switch Domain}, communication traverses InfiniBand or RoCE at `{python} FleetTopologyInterconnect.ib_ndr_gbps_str` (NDR InfiniBand), roughly `{python} FleetTopologyInterconnect.ib_ndr_gbs_str` per link. This represents an order of magnitude reduction from NVLink bandwidth. Data parallelism, which requires AllReduce of gradients after each training step, operates efficiently at this level because the gradient synchronization pattern allows overlap between communication and the next iteration's forward pass. The latency penalty is modest (microseconds vs. nanoseconds), and the aggregate bandwidth is sufficient for gradient tensors that are typically smaller than the activation tensors exchanged in tensor parallelism.
...
- Tensor parallel groups: 32 groups of 8 GPUs, each confined to one 8-GPU node (NVLink: `{python} FleetTopologyInterconnect.nvlink_h100_gb_s_str`)
...
# │          es_rescale_hours_str, es_vs_wait_str, es_vs_imm_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.book (fmt, check)
# │ Exports: as_capacity_qps_str, as_detect_str, as_ready_str, as_excess_qps_str,
# │          as_violations_str, as_pred_trigger_str, as_pred_capacity_qps_str
# └─────────────────────────────────────────────────────────────────────────────
...
Consider a serving fleet with `{python} AutoscalingLag.as_replicas_str` replicas, each capable of `{python} AutoscalingLag.as_qps_per_replica_str` queries per second (QPS) while meeting the `{python} AutoscalingLag.as_slo_ms_str` SLO. Total capacity: `{python} AutoscalingLag.as_capacity_qps_str`.
...
class GpuSharingRoi:
    ...
    gs_excl_total_mem_gb_str = fmt_qty_int(excl_total_mem, GiB, unit_label="GB")
    ...
    gs_shared_mem_gb_str = fmt_qty(shared_mem_per_gpu_q, GiB, precision=0, unit_label="GB")
    ...
    gs_a100_mem_gb_str = fmt_qty(a100_mem, GiB, precision=0, commas=False, unit_label="GB")
    gs_small_footprint_gb_str = fmt_qty(small_footprint, GiB, precision=0, commas=False, unit_label="GB")
...
# │ Exports: FleetDebugNvlinkRecap.nvlink_a100_gb_s_str
from mlsysim.fmt import fmt_qty

class FleetDebugNvlinkRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_a100_gb_s_str = fmt_qty(Hardware.Cloud.A100.nvlink.bandwidth, GB/second, precision=0)
```

| **GPU Type**   | **Count** | **Memory**  | **Interconnect**                                               | **Nodes**                |
|:---------------|----------:|:------------|:---------------------------------------------------------------|:-------------------------|
| **A100-80 GB** |       400 | 80 GB HBM2e | NVLink (`{python} FleetDebugNvlinkRecap.nvlink_a100_gb_s_str`) | 50 nodes $\times$ 8 GPUs |
| **A100-40 GB** |       400 | 40 GB HBM2e | NVLink (`{python} FleetDebugNvlinkRecap.nvlink_a100_gb_s_str`) | 50 nodes $\times$ 8 GPUs |
