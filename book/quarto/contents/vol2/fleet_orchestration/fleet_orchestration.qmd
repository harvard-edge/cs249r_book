...
class FleetTopologyInterconnect:
    ...
    xfer_slowdown_str = fmt_multiple(xfer_slowdown, precision=0, commas=False)
```
...
class ElasticScaling:
    ...
    es_vs_imm_factor_str = fmt_multiple(elastic_vs_immediate, precision=1, commas=False)
```
...
The elastic strategy outperforms waiting by approximately `{python} ElasticScaling.es_vs_wait_pct_str` and the immediate small-scale run by nearly `{python} ElasticScaling.es_vs_imm_factor_str`$\times$. However, if the re-scaling cost exceeded `{python} ElasticScaling.es_rescale_break_even_h_str` (due to slow checkpoint transfer or compilation), Strategy 2 would become superior.
...
::: {.column-margin}
![](images/svg/vol2_fleet_orchestration_margin_004.svg){width="100%" fig-alt="Allocated, compute-active, and productive GPUs."}
