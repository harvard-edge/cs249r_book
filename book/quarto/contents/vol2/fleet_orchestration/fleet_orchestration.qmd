...
\noindent
![](images/png/cover_fleet_orchestration_blueprint_labeled_print.png){fig-alt="Scheduler control-plane blueprint where queued jobs are placed onto topology-aware accelerator capacity with preemption, recovery, and policy constraints."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_fleet_orchestration_blueprint_labeled.webp){fig-alt="Scheduler control-plane blueprint where queued jobs are placed onto topology-aware accelerator capacity with preemption, recovery, and policy constraints."}
:::
...
A thousand accelerators sitting idle waiting for scheduling decisions cost the same as a thousand accelerators computing useful work. At scale, the limiting factor shifts from having enough hardware to using it efficiently. Jobs wait in queues while resources sit idle, fragmentation leaves gaps too small for pending jobs, and deadlocks leave jobs holding partial resources while waiting for more. Orchestration extracts useful work from shared infrastructure by deciding which jobs run on which nodes, when to preempt, how to balance fairness against utilization, and how to keep the coordination mechanisms from becoming bottlenecks. Poor orchestration turns expensive hardware into waste; effective orchestration turns a collection of machines into a coherent computing resource. In C³ terms, orchestration deliberately pays a coordination tax, trading some peak utilization to prevent deadlock and fragmentation under load.
...
- Evaluate ML-specific schedulers using job duration, iteration progress, dominant-resource fairness, and adaptive resource-allocation signals
- Design fleet-level inference capacity, isolation, and placement policies using queue depth, tail latency, and GPU memory pressure
...
Partitioning computation, synchronizing it efficiently, and recovering from hardware failures solve the single-job scaling problem. Orchestration addresses the fleet-level challenge: deciding which jobs run, where they run, and how resources are shared among competing demands.

In the fleet stack (shown in the margin), orchestration operates at the distribution layer, but the infrastructure layer constrains its decisions. The scheduler must account for GPU heterogeneity, NVLink topology, rack power limits, and network bisection bandwidth. When scheduling goes wrong, debugging requires examining all four layers to identify whether the root cause is infrastructure (hardware constraints), distribution (algorithm limitations), serving (workload mismatch), or governance (policy misconfiguration). Orchestration connects physical infrastructure to distributed algorithms, translating resource requests into placements that respect both hardware topology and organizational policy.
...
Workload semantics, rather than an ML-versus-HPC boundary, determine scheduling flexibility. Rigid synchronous jobs in both domains require a fixed set of co-scheduled ranks, whereas malleable jobs can make progress across a declared range of resource counts. Gang scheduling[^fn-gang-allor-nothing] represents the critical constraint for synchronous distributed training: a job requiring 1,024 GPUs cannot make useful iteration progress with only 512. Every worker must participate in every collective, and a missing worker blocks all others. @Sec-appdx-comm-foundations-allreduce develops the ring and tree AllReduce cost model that makes this constraint physical: the collective stalls until the last worker arrives, so allocating $N$ GPUs but landing $N-1$ leaves the missing rank's peers idle at the synchronization point. A scheduler that partially allocates resources creates deadlocks where multiple rigid jobs each hold some GPUs while waiting for more, with none able to proceed. Malleable data-parallel training can instead vary its replica count at explicit reconfiguration boundaries, but tensor- and pipeline-parallel groups usually retain a rigid base allocation. The scheduler must therefore model each job's minimum viable gang and permitted resize points rather than infer rigidity from the workload's field.

If gang scheduling is the first hard constraint, topology awareness via **Placement Groups**\index{Placement Group!topology-aware scheduling}[^fn-placement-group-affinity] is the second: to support tensor parallelism, the scheduler must ensure GPUs are placed within the same high-bandwidth NVLink domain rather than scattered across the data center.
...
These requirements connect the scheduler directly to the checkpoint and recovery infrastructure from @sec-fault-tolerance-reliability: scheduling decisions about spare capacity, preemption policies, and elastic training support determine how quickly the system recovers from statistically expected failures. A scheduler that reserves spare capacity or supports elastic recovery can restore throughput much faster than one that requires every failed job to wait for a full fresh allocation and reload, but the correct reserve fraction and recovery time are workload- and fleet-specific parameters rather than universal constants.
...
- **Fairness**\index{Fairness}: The equitable distribution of resources across users or teams prevents a single large team from monopolizing the fleet. Dominant resource fairness [@ghodsi2011] equalizes each user's largest normalized share across resources such as GPUs, CPU, and memory.
...
- **Cost Efficiency**\index{Cost Efficiency}: Interruptible spot instances, off-peak scheduling, and tight packing reduce budget burn, but often increase failure rates or completion time.
...
Fairness requires an outcome metric, not only a quota rule. For job $j$, **slowdown** $S_j=(W_j+T_j)/T_j$ compares its queueing-plus-running time with its isolated runtime $T_j$; a one-hour job that waits nine hours has $S_j=10$, even if its team eventually receives its nominal GPU share. A scheduler should report the distribution of slowdown by user and job class, together with the age of the oldest pending feasible job. Dominant-share parity detects unequal resource allocation, slowdown exposes unequal delay, and a bounded maximum feasible wait detects starvation. These metrics separate a policy that is fair in allocated GPU-hours from one that is fair in the time users wait to obtain useful results.
...
Here, $\rho = \lambda_{\text{job}}/\mu$ represents cluster utilization, $1/\mu$ is the average job duration, and $C_s = \sigma \mu$ is the **Coefficient of Variation**\index{Coefficient of Variation!definition} of job duration. The right-hand side is therefore the waiting time expressed as a multiple of the average job duration.
...
#### Worked allocation: Dominant-resource fairness {#sec-fleet-orchestration-drf-worked-allocation}

Dominant-resource fairness (DRF)\index{Dominant-resource fairness!worked allocation} makes a multi-resource entitlement comparable by normalizing each tenant's allocation against fleet capacity and taking its largest share [@ghodsi2011]. Consider the deliberately small cluster in @tbl-fleet-orchestration-drf-allocation: 8 GPUs, 32 CPU cores, 256 GB of memory, and 80 Gb/s of network capacity. Tenant A's replica requires 1 GPU, 4 cores, 32 GB, and 10 Gb/s; Tenant B's distributed worker requires 2 GPUs, 2 cores, 16 GB, and 20 Gb/s.
...
For tenant $i$, define its dominant share as $d_i=\max_r(a_{i,r}/C_r)$, where $a_{i,r}$ is its allocation of resource $r$ and $C_r$ is fleet capacity for that resource. The calculation equalizes $d_i$, not every resource share. One A replica raises A's dominant share by $1/8$; one B worker raises B's by $2/8$. Progressive filling therefore grants roughly two A replicas for each B worker until both dominant shares reach $0.50$. This is a fair policy target, but it is not yet a feasible placement. If B's two workers form one four-GPU gang, the four GPUs must become available atomically; if tensor parallelism requires one NVLink domain, four free GPUs scattered across nodes do not satisfy the allocation; and if the job requires 80 GB HBM accelerators, four incompatible GPUs do not count at all. A production scheduler therefore applies capability, gang, and topology constraints to the feasible set, then uses DRF to choose fairly among allocations that can actually run. When no equal-share placement exists, the scheduler should report the constraint causing the deviation rather than silently presenting an infeasible entitlement as service delivered.
...
A third strategy, **over-subscription**\index{Over-subscription!definition}, admits more demand than a capacity dimension can sustain simultaneously, but only where the resource is divisible and the scheduler can bound interference. Average SM utilization does not create fractional whole GPUs: a job that reserves device memory, requires an exclusive CUDA context, or belongs to a rigid gang still occupies its assigned device while kernels are idle. Statistical multiplexing can be safe for explicitly shareable CPU, network, storage, or partitioned GPU capacity when tenants have measured, weakly correlated peaks and enforceable limits. It is unsafe to convert 70 percent average GPU utilization into a 1.4$\times$ device overcommit ratio. Whole-device capacity should instead be recovered through preemption, elastic resizing, or an isolation mechanism such as MIG or MPS whose memory, compute, and fault-sharing contract the scheduler models explicitly. @Fig-fragmentation-problem shows the separate packing problem: although 19 GPUs (roughly 30 percent of the cluster) sit idle, no single node has a contiguous block of eight, so a full-node job cannot be placed.
...
class GangSchedulingDeadlock:
    ...
    gs_n_gpus_str = fmt(n_total_gpus, precision=0)
    gs_n_jobs_str = fmt(n_jobs, precision=0, commas=False)
    gs_gpus_per_job_str = fmt(gpus_per_job, precision=0)
...
::: {#exmp-fleet-orchestration-physics-deadlock .callout-example title="The physics of deadlock"}

**Scenario**: A `{python} GangSchedulingDeadlock.gs_n_gpus_str`-GPU cluster receives `{python} GangSchedulingDeadlock.gs_n_jobs_str` large distributed training jobs requiring `{python} GangSchedulingDeadlock.gs_gpus_per_job_str` GPUs each. A naive scheduler allocates `{python} GangSchedulingDeadlock.gs_partial_gpus_str` GPUs to Team A and `{python} GangSchedulingDeadlock.gs_partial_gpus_str` GPUs to Team B.

**Diagnosis**: Neither job can proceed without its full node allocation, creating a hold-and-wait deadlock: Team A waits for Team B's GPUs while Team B waits for Team A. Aggregate throughput drops to zero while consuming `{python} GangSchedulingDeadlock.gs_waste_m_str`/hour in idle compute cost.

**Systems lesson**: Distributed training workloads require atomic all-or-nothing (gang) scheduling. Fleet orchestrators must enforce job-level admission boundaries so that jobs remain queued until the entire requested GPU quota is available simultaneously.

:::

The partial-allocation pattern is a **Hold-and-Wait Deadlock**\index{Deadlock!hold-and-wait}[^fn-deadlock-scheduling-cycle] in which two jobs each hold half the cluster and neither can proceed. The implementation guarantee is straightforward: for a job $J$ requesting $N$ GPUs, the scheduler must guarantee that either all $N$ GPUs are allocated atomically, or the job remains in the queue without holding any resources. This binary outcome eliminates deadlock by construction, since a job that holds no resources cannot block other jobs. Implementing this guarantee efficiently, however, is the challenge that defines much of cluster scheduling algorithm design.

[^fn-deadlock-scheduling-cycle]: **Hold-and-Wait Deadlock**\index{Deadlock!hold-and-wait}: One of the four classical conditions jointly sufficient for deadlock in resource-allocation systems. In GPU clusters, this condition is uniquely expensive: two jobs each holding 500 GPUs while waiting for 200 more strand 1,000 GPUs indefinitely, burning approximately \$2,000/hour. Gang scheduling eliminates hold-and-wait by construction, requiring atomic all-or-nothing allocation at the cost of reduced packing flexibility.

Naive gang scheduling is wasteful in predictable ways. If the cluster has 900 free GPUs and a job requests 1,024, the 900 GPUs sit idle until 124 more become available, potentially wasting thousands of GPU-hours. Backfill scheduling addresses this by identifying jobs in the queue that can fit within the 900 available GPUs without delaying the large job's expected start time, an idea popularized in the ANL/IBM SP scheduling system [@lifka1995anlibm]. A 128-GPU job with an estimated runtime of 2 hours can safely backfill if the 124 additional GPUs needed by the large job will remain unavailable for at least 2 hours and the backfill job is expected to finish before the large job's reservation. The backfilled job uses resources that would otherwise be idle, improving utilization without violating the large job's scheduling guarantee.
...
The cluster economics introduced at the start of this chapter set the stake; the gang-scheduling decision is where a scheduler first earns or burns it. Taking the same `{python} ClusterEconomicsIdleRecap.cluster_size_str`-GPU cluster:

- **Operating cost**: `{python} ClusterEconomicsIdleRecap.daily_cost_str`/day (`{python} ClusterEconomicsIdleRecap.annual_cost_str`/year)
- **Lower Utilization**\index{Lower Utilization!definition} (`{python} ClusterEconomicsIdleRecap.util_low_pct_str`): `{python} ClusterEconomicsIdleRecap.productive_low_str` productive, `{python} ClusterEconomicsIdleRecap.idle_low_str` idle = `{python} ClusterEconomicsIdleRecap.waste_low_str`/day wasted
- **Higher Utilization**\index{Higher Utilization!definition} (`{python} ClusterEconomicsIdleRecap.util_high_pct_str`): `{python} ClusterEconomicsIdleRecap.productive_high_str` productive, `{python} ClusterEconomicsIdleRecap.idle_high_str` idle = `{python} ClusterEconomicsIdleRecap.waste_high_str`/day wasted
...
Gang scheduling delivers this utilization gain by preventing deadlock. A single hold-and-wait deadlock can strand the whole cluster, erasing more than a full day of the gain in one event.
...
For large-scale fleets, exact placement search is computationally nontrivial. A cluster with 10,000 GPUs and 500 pending jobs has a state space exceeding $10^{15}$ possible allocations, making exhaustive allocation search intractable. By contrast, detecting a cycle in an already constructed wait-for graph is linear in the graph's vertices and edges. The practical difficulty is maintaining a complete, current global graph while resource state changes across a distributed control plane. Large-scale schedulers therefore supplement wait-for detection with mechanisms that preserve liveness under incomplete state. A **Lease-Based Resource**\index{Lease-Based Resource!definition} gives every allocation a time-to-live, allowing the scheduler to reclaim GPUs when a job stops renewing its claim. **Progress Monitoring**\index{Progress Monitoring!definition} then checks whether the job is actually advancing, using signals such as GPU utilization, step counters, or heartbeat timestamps to identify zombie allocations. When the scheduler must preempt a lower-priority job, **Holistic Preemption**\index{Holistic Preemption!definition} reserves the CPU, network, and storage bandwidth needed for that job to checkpoint and exit cleanly; otherwise, the victim can be too starved to release the accelerators. These strategies accept a nonzero rate of false positives to preserve cluster liveness.

### Heterogeneous gang scheduling {#sec-fleet-orchestration-heterogeneous-gang}

The traditional gang scheduling model assumes a homogeneous bulk synchronous parallel workload: $N$ identical workers executing the exact same compute graph. Some training workflows break this assumption because one logical job contains several roles with different resource profiles. In preference-training pipelines such as reinforcement learning from human feedback (RLHF), for example, one component may update the policy while another scores outputs or supplies reference behavior. The scheduler still sees one job, but the gang contains trainers, evaluators, and inference-style workers that need different GPU counts, memory footprints, and lifetimes.

::: {#psp-fleet-orchestration-routing-over-synchrony .callout-perspective title="Routing over synchrony"}

Heterogeneous gang scheduling\index{Heterogeneous Gang Scheduling} reframes this allocation. The scheduler must atomically allocate a diverse constellation of resources: high-HBM nodes optimized for memory-bound inference generation, alongside high-compute nodes for compute-bound backpropagation. The orchestration challenge shifts from static topological bin-packing to dynamic data routing. Activations and gradients are no longer just reduced within a homogeneous ring; instead, data must stream continuously between inference nodes generating rollouts and training nodes computing policy updates. If the heterogeneous gang allocation is not perfectly balanced across these distinct workload profiles, the entire cluster falls out of sync, leaving expensive accelerators starved for data.
...
The fair-share formula in @eq-fleet-orchestration-fair-share naturally deprioritizes heavy users while allowing burst access when resources are idle. A researcher who has consumed twice their fair share sees their priority halved, pushing new submissions behind colleagues who have used less. Conversely, a researcher who has used no resources recently receives maximum priority, allowing rapid access when they return to active work.
...
Native Kubernetes\index{Kubernetes!gpu allocation} lacks ML-aware scheduling, but extensions address this gap. GPU scheduling relies on **Device Plugins**\index{Device Plugin!definition} that expose accelerators as schedulable extended resources. The NVIDIA device plugin registers GPUs with the kubelet (the node-level agent), enabling pod specifications that request GPU resources declaratively through the standard Kubernetes resource model. @Lst-fleet-orchestration-k8s-gpu shows the resulting pod fragment.
...
Multi-Instance GPU (MIG)\index{MIG!orchestration}[^fn-mig-gpu-isolation] technology addresses this inefficiency by partitioning A100 (`{python} ClusterEconomicsMigRecap.a100_mem_gb_str`) and H100 (`{python} ClusterEconomicsMigRecap.h100_mem_gb_str`) GPUs into hardware-isolated instances. Unlike software-based GPU sharing, MIG dedicates memory, cache, and compute resources to each instance, preventing one workload from interfering with another and eliminating the noisy neighbor[^fn-noisy-neighbor-etymology] effect. The A100 MIG profiles\index{MIG!A100 profiles} in @tbl-fleet-orchestration-mig-profiles show the scheduling consequence: a single physical A100 becomes several fixed-size resources that the device plugin can expose as independently schedulable accelerators.

[^fn-mig-gpu-isolation]: **Multi-Instance GPU (MIG)**\index{MIG!GPU isolation}: Introduced with the A100 (2020), MIG partitions a single GPU into up to 7 hardware-isolated instances with dedicated memory controllers, L2 cache slices, and compute units [@NVIDIA2020]. The isolation is enforced at the hardware level, not by time-slicing, reducing noisy-neighbor interference relative to software-only sharing. The trade-off is rigidity: partition profiles cannot change without draining all workloads, making MIG poorly suited for clusters with rapidly shifting workload mixes.
...
Within a single node, GPUs communicate via NVLink\index{NVLink!topology-aware scheduling}, providing `{python} FleetTopologyInterconnect.nvlink_a100_gb_per_s_str` aggregate bidirectional bandwidth (300 GB/s per direction) per GPU on A100 systems and `{python} FleetTopologyInterconnect.nvlink_h100_gb_per_s_str` (450 GB/s per direction) on H100 systems. This high-bandwidth, low-latency interconnect enables efficient tensor parallelism, where matrix operations are split across GPUs that must exchange intermediate results (activations and gradients) at every transformer layer. The NVLink bandwidth is sufficient to overlap communication with computation for most model architectures, meaning tensor parallel operations can proceed at near-ideal throughput when confined to a single node.

Between nodes within the same rack or **Leaf Switch Domain**\index{Leaf Switch Domain!definition}, communication traverses InfiniBand or RoCE at `{python} FleetTopologyInterconnect.ib_ndr_gbit_per_s_str` (NDR InfiniBand), roughly `{python} FleetTopologyInterconnect.ib_ndr_gb_per_s_str` per link. This represents an order of magnitude reduction from NVLink bandwidth. Data parallelism, which requires AllReduce of gradients after each training step, operates efficiently at this level because bucketed gradient synchronization can overlap with the remaining backward computation. The latency penalty is modest (microseconds vs. nanoseconds), and the aggregate bandwidth is sufficient for gradient tensors that are typically smaller than the activation tensors exchanged in tensor parallelism.
...
The bandwidth gap shown in @fig-topology-placement-paths is the key constraint: the roughly 4$\times$ reduction from intra-rack leaf paths to cross-rack leaf-spine-leaf paths means that scattered placement forces tensor parallel groups to spend more time communicating than computing, a penalty that compounds at every transformer layer in the model. Topology-aware placement algorithms assign jobs to nodes that minimize communication cost, transforming the abstract bin packing problem into a graph optimization problem on the cluster's physical topology. The simplest approach uses a **Locality Cost**\index{Locality Cost!definition}, defined in @eq-fleet-orchestration-locality-score, that penalizes allocations spanning multiple topology domains:

$$ \text{Cost}_{\text{locality}} = \sum_{i < j} w(d(g_i, g_j)) $$ {#eq-fleet-orchestration-locality-score}
...
\EndFor
\State \Return the feasible slot of least $\text{Cost}_{\text{locality}}$ \Comment{topology-optimal, atomic}
\end{algorithmic}
\end{algorithm}
```
...
::: {#fig-topology-placement fig-env="figure" fig-pos="htb" fig-cap="**Topology Placement Impact**: AllReduce latency across four placement strategies (Random, Rack-Aware, Rail-Optimized, and Topology-Optimal) for a 1 GB gradient sync on 64 GPUs. Topology-aware scheduling reduces sync latency from 120 ms to 25 ms, yielding a 4.8$\times$ speedup." fig-alt="Bar chart comparing AllReduce latency for Random, Rack-Aware, Rail-Optimized, and Topology-Optimal placement. 4.8× speedup from Random to Topology-Optimal."}
...
Static configuration files like Slurm's `topology.conf` or Kubernetes node labels provide a baseline map of the cluster, but they represent the intended state, not the effective state. True **Topology Discovery**\index{Topology Discovery!definition} requires runtime introspection to validate that the physical hardware matches the architectural diagram. Libraries like NCCL bypass static configurations entirely, parsing the Linux virtual filesystem at `/sys/class/infiniband` to map the PCIe bus hierarchy, enumerate NVLink connections, and verify network interface card (NIC) proximity. This creates a ground-truth graph where edges represent verified, operational links rather than theoretical cables.
...
**Diagnosis**: A single leaf switch transceiver experienced high CRC error rates and packet retransmissions. Binary up/down probes passed, but degraded bisection bandwidth stalled AllReduce communication across the entire training job.
...
[^fn-taint-toleration-rule]: **Taints and Tolerations**\index{Taints and Tolerations!GPU isolation}: Kubernetes node-repulsion primitives used in ML fleets to isolate specialized hardware. By tainting A100 or H100 nodes, the orchestrator prevents general-purpose web microservices from consuming expensive GPU capacity, ensuring those nodes remain reserved exclusively for training jobs that declare explicit tolerations.
...
- **Elastic Range**\index{Elastic Scheduling!elastic range}: $[N_{\text{min}}, N_{\text{max}}]$ specifies the worker counts between which the job can operate.
- **Transition Cost**\index{Elastic Scheduling!transition cost}: The rendezvous cycle, typically 10 to 60 seconds, determines how frequently the scheduler can resize without the overhead negating the throughput gain.
- **Scaling Efficiency Curve**\index{Scaling Efficiency!elastic scheduling}: The throughput gained from each added worker determines whether assigning idle GPUs to this job is more productive than holding them for a pending gang-scheduled job.
...
**Elastic Scheduling**\index{Elastic Scheduling!definition} (min 32, max 128):
...
The most immediately valuable benefit is faster job start. An elastic job requesting 32 to 128 GPUs can start as soon as 32 GPUs are available, rather than waiting in the gang scheduling queue for 128 contiguous GPUs. In congested clusters where large-job queue times can be hours to days, starting immediately at reduced scale often completes the job sooner than waiting for full-scale allocation, a trade-off notebook \ref{nbk-fleet-orchestration-elastic-scaling-decision} works through quantitatively. The scheduler can compute it dynamically, choosing the start time and initial scale that minimizes expected total time (wait time plus execution time).
...
The utilization-to-dollars relationship established in @sec-fleet-orchestration-introduction becomes a scheduling signal rather than a complete financial model. The orchestrator consumes resource prices, reservation commitments, interruption risk, checkpoint cost, and job deadlines when it chooses whether to admit, delay, preempt, or resize work. @Sec-ops-scale owns the larger questions of capital planning, platform return, showback and chargeback, and lifecycle total cost of ownership.
...
::: {#fig-cost-idle-gpus fig-env="figure" fig-pos="htb" fig-cap="**The Cost of Idle GPUs**: Annual wasted cost as a function of cluster size for different GPU utilization levels, assuming \$2/GPU-hour (cloud equivalent). The shaded region between 30 percent and 70 percent is an illustrative operating range for the scenario analysis. At 10,000 GPUs with 50 percent utilization, the model yields \$87.6M in annual wasted capacity. The Microsoft Philly data point marks the published mean GPU utilization of 52 percent. Data sources: [@jeon2019analysis; @li2023]." fig-alt="Line chart of annual wasted cost in millions vs. cluster size with four utilization curves at 30, 50, 70, and 90 percent and shaded illustrative range."}
...
Checkpoint frequency optimization for spot instances differs from the standard reliability logic. The Young-Daly formula developed in @sec-fault-tolerance-young-daly optimizes for unpredicted hardware failures. Spot interruptions, however, come with a warning: 2 minutes on AWS, 30 seconds on GCP. A **Two-Tier Checkpointing**\index{Two-Tier Checkpointing!definition} approach combines a periodic checkpoint at the Young-Daly interval with a best-effort checkpoint triggered by the termination notice. The notice-triggered checkpoint reduces lost work only if it can finish within the warning window. When a large model's full checkpoint takes longer, the job must rely on an already-running asynchronous or incremental checkpoint and accept rollback to the last durable state.
...
Organizations that rely on cloud infrastructure (or that think about on-premise infrastructure in economic terms) balance three tiers of compute capacity, each offering a different trade-off between cost, availability, and commitment. @Tbl-cloud-capacity-reservation-tiers compares those tiers across cost, availability, interruption risk, and workload fit.

| **Capacity tier**      | **Cost and commitment**                                              | **Availability and interruption risk**                               | **Best-fit workloads**                                                                                           |
|:-----------------------|:---------------------------------------------------------------------|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------|
| **Reserved capacity**  | One- to three-year commitments, with 30–60% discounts vs. on-demand. | Guaranteed availability for the committed baseline.                  | Predictable continuous workloads: 24/7 inference, scheduled training, CI and regression testing, SLA-bound jobs. |
...
**Spot**\index{Spot!definition} (`{python} SpotTrainingEconomics.spot_discount_pct_str` discount, `{python} SpotTrainingEconomics.checkpoint_overhead_pct_str` checkpoint overhead, `{python} SpotTrainingEconomics.interruptions_per_day_str`/day with `{python} SpotTrainingEconomics.restart_min_str` restart):
...
These decisions require the scheduler to model both the time-value of computation (the organizational cost of a 2-hour delay in researcher productivity, product launch schedules, or competitive position) and the financial cost of different resource allocations. The time-value varies enormously by workload type: delaying a production model retrain by 2 hours might violate a service-level agreement (service level agreement (SLA)) and cost thousands in penalty fees, while delaying an exploratory experiment by 2 hours costs nothing but researcher patience.

Production systems typically define **Cost Policies**\index{Cost Policy!definition} per workload class that encode these trade-offs as configuration rather than requiring per-decision human judgment. Inference workloads are pinned to reserved capacity for guaranteed availability and predictable latency, since the cost of an inference outage (lost revenue, degraded user experience) far exceeds the premium of reserved pricing. Large training runs use a mix of reserved capacity (for the minimum viable allocation) and spot capacity (for elastic expansion), with automatic fallback to on-demand if spot is reclaimed and the job is within a configurable deadline. Exploratory workloads are restricted to spot to minimize cost, with the understanding that they may be interrupted and must tolerate variable completion times.

The scheduler does not implement the organization's financial accounting system. It consumes the resulting price and policy signals: the marginal price of each capacity tier, the reservation a project may claim, the interruption risk it may accept, and the deadline it must meet. @Sec-ml-operations-scale-cost-visibility-attribution-3f3a develops showback, chargeback, and attribution; this chapter uses those outputs only to rank feasible scheduling actions.

### Consuming total-cost signals {#sec-fleet-orchestration-tco}

Total cost of ownership (TCO) determines the price signals exposed to the scheduler, but it is not itself a scheduling mechanism. @Sec-ops-scale-fleet-economics develops build-vs.-buy analysis and lifecycle TCO. The orchestrator needs only the resulting per-resource opportunity costs and policy constraints, including whether capacity is owned, reserved, on demand, or interruptible.
...
**Gandiva**\index{Gandiva!definition} [@xiao2018gandiva] exploits iteration-boundary time slicing, enabling higher utilization through controlled oversubscription. A cluster with 100 GPUs might support 120 concurrent jobs if each spends 20 percent of its iteration time waiting for data, because the scheduler can interleave data loading from one job with GPU computation from another on the same device. The critical insight that makes this practical is that iteration boundaries provide natural preemption points where GPU state is minimal. Between iterations, only model weights and optimizer state reside on the GPU; the intermediate activations from forward and backward passes have been freed. This minimal state makes context switching cheap (seconds rather than minutes), unlike preempting mid-iteration when the full activation memory is in use.

Gandiva also implements **Grow-Shrink Elasticity**\index{Grow-Shrink Elasticity!definition} at a finer granularity than the framework-level elastic training discussed in @sec-fleet-orchestration-elastic-training. Gandiva automatically adjusts data parallelism degree based on real-time resource availability, using profiled iteration times to predict the throughput impact of adding or removing workers. When a high-priority job arrives, Gandiva shrinks lower-priority jobs by reducing their worker count rather than killing them entirely, preserving their progress while freeing resources. When resources become available again, it grows jobs back to their preferred parallelism degree. This fine-grained elasticity, informed by actual iteration-level profiling data, enables scheduling decisions that general-purpose systems cannot make because they lack visibility into job internals.

::: {#ws-fleet-orchestration-limping-switch .callout-war-story title="The limping switch (2021)"}

**Context**: In a 16,384 GPU distributed cluster, a foundation-model training job experienced severe throughput degradation despite healthy GPU execution metrics [@sigler2021openai_kubernetes].

**Mechanism**: A single transceiver on an inter-rack spine switch experienced elevated bit-error rates, degrading link throughput from 100 Gbps to 1 Gbps ($100\times$ bandwidth collapse). Standard ping/up health checks reported green, but AllReduce gradient synchronization $T_{\text{sync}}$ stalled across all 16,384 GPUs waiting for the straggling link.

**Impact**: Cluster-wide hardware efficiency $\eta_{\text{hw}}$ collapsed from 45 percent to 8 percent, wasting $>15,000$ GPU-hours per day until network-level telemetry flagged the degraded switch port.

**Fix**: Engineers implemented automated micro-benchmark probes and packet-error telemetry to automatically isolate and quarantine degraded switch ports before scheduling job allocation.

**Systems lesson**: Binary up/down health monitoring is insufficient for distributed scaling. Schedulers require continuous, performance-aware telemetry (switch counters, packet retransmission rates, collective benchmarks) to quarantine degraded network paths automatically.

:::

### Themis: Finish-time fairness {#sec-fleet-orchestration-themis}

Traditional fair-share scheduling treats all GPU-seconds equally, even when equal shares produce very different completion delays. **Themis**\index{Themis!definition} [@mahajan2020themis] instead measures fairness in normalized finish time: each job compares its completion time under contention with the completion time it would receive under an isolated fair share of the cluster.

Themis allocates resources to improve the worst finish-time fairness ratio across jobs. A job close to completion may receive more resources when a short delay would inflate its normalized finish time more than the same allocation change would affect another job. Waiting does not waste the GPU-hours already consumed; it delays when their result becomes available. The metric therefore compares completion delay, not sunk compute, and prevents a policy from appearing fair merely because it eventually equalizes GPU-seconds.
...
| **Themis**    | Completion delay defines fairness          | Normalized finish time |  Improve worst fairness  |      Low (job metadata)     |   Low (auction complexity)  |
...
The production decision is whether a specialized policy improves marginal allocation enough to justify its operational burden. This chapter evaluates that decision through queue delay, useful GPU-hours, preemption tax, and placement efficiency. @Sec-ops-scale owns the broader platform investment decision, including staffing, capital planning, lifecycle cost, and portfolio return.
...
**Mechanism**: Default Kubernetes scheduling placed pods incrementally without all-or-nothing constraints. When two large experiments requested cluster capacity, Kubernetes scheduled half of each job's pods across nodes.

**Impact**: Both training jobs stalled indefinitely while holding allocated GPUs, producing zero training progress and deadlocking cluster resources.

**Fix**: OpenAI developed custom Kubernetes gang-scheduling operators that enforce atomic all-or-nothing pod placement.
...
- **Minimum Replica Count**\index{Minimum Replica Count}: Keeping at least one warm replica above zero eliminates cold starts for the first burst of traffic but incurs a baseline cost even during truly idle periods.
- **Predictive Scaling**\index{Predictive Scaling}: Scaling from historical traffic patterns, such as diurnal cycles, day-of-week patterns, and known events, adds capacity before expected peaks rather than reacting after load arrives.
- **Prewarming**\index{Prewarming}: Loading model replicas into GPU memory on standby GPUs reduces activation from minutes of model loading to seconds of process initialization, but standby replicas consume GPU memory that cannot serve other workloads.
...
The fleet-level response to cold-start lag is capacity sizing and prewarming. The orchestrator keeps enough warm replicas or staged model state that newly placed capacity becomes ready before the forecast demand exceeds the existing envelope. Continuous batching, request admission, queueing, and load shedding determine how the serving plane uses that envelope while new replicas start; @sec-inference-scale owns those per-request mechanisms.
...
Predictive autoscaling decouples scaling actions from current load. Analyzing historical traffic patterns (diurnal cycles, day-of-week seasonality) and incorporating real-time leading indicators (for example, a surge in login requests often precedes a surge in inference queries) allows the scheduler to preprovision capacity before the demand arrives. Serving our 175B model with a 500 ms P99 SLO requires this anticipation. If the model takes 3 minutes to become ready, the predictive scaler must issue the scale-up command at least 4 minutes before the expected traffic ramp. This transforms the scaling problem from a control theory problem (reacting to error) to a forecasting problem (predicting the future).

Effective scaling policies must be **SLO-driven**\index{SLO-Driven Scaling!definition} rather than utilization-driven. Targeting a fixed utilization (for example, "keep GPU at 70 percent") is a proxy that often fails: a model might hit its latency SLO at 60 percent utilization due to memory bandwidth contention, or it might safely run at 90 percent utilization if the requests are compute-bound and uniform. An SLO-driven policy explicitly targets the metric that matters: "Scale up when P99 latency exceeds 400 ms (80 percent of the 500 ms target)." This approach automatically adapts to changes in workload characteristics. If a new model version is less efficient, the latency metric will rise faster, and the scaler will provision more replicas to maintain the SLO, without requiring manual tuning of utilization thresholds.

To prevent **Oscillation**\index{Oscillation!definition}, where the scaler rapidly adds and removes replicas during noisy traffic, production systems implement **Hysteresis**\index{Hysteresis!definition} and **Cooldown Periods**\index{Cooldown Period!definition}. A typical policy scales up aggressively (no stabilization window) to protect the SLO, but scales down conservatively (15-minute cooldown) to avoid thrashing. This asymmetry acknowledges that the cost of an unnecessary scale-up (wasted compute for 15 minutes) is far lower than the cost of a missed scale-up (SLO violation and user churn).
...
# │      accumulated excess demand from excess QPS × duration.
# │
# └─────────────────────────────────────────────────────────────────────────────
...
class AutoscalingLag:
    ...
    excess_requests = int(ramp_excess_requests + flat_excess_requests)
    ...
    check(excess_requests == 4725, f"Excess requests: expected 4725, got {excess_requests}")
    ...
    as_excess_requests_str = fmt(excess_requests, precision=0)
    ...
    as_load_time_min_str = fmt_time(round(load_time_sec / 60), 'minute', precision=0, commas=False, style='word')
```
...
[^fn-mps-gpu-sharing]: **CUDA MPS (Multi-Process Service)**\index{MPS!GPU sharing}: NVIDIA's client-server runtime multiplexes CUDA processes through shared GPU scheduling resources. Volta and newer clients receive distinct GPU virtual address spaces, while compute, cache, bandwidth, and device-memory capacity remain shared. Fatal faults are contained only to the subset of clients sharing the affected GPU through the same MPS server, so MPS is not equivalent to a hardware partition or full-device isolation [@nvidiaCudaMps].

At the CPU level, core pinning assigns specific CPU cores to inference pods, preventing the Linux scheduler from migrating processes between cores in ways that invalidate processor caches. For latency-sensitive workloads, isolating cores using `isolcpus` kernel parameters and `taskset` affinity removes OS scheduling jitter that manifests as random latency spikes. Combined with non-uniform memory access (NUMA)-aware[^fn-numa-node-inference] placement that ensures inference pods access memory through the nearest memory controller (avoiding remote NUMA access that adds 50 to 100 ns per access), this reduces P99 latency by 10 to 30 percent for sub-millisecond inference tasks. The principle is straightforward: inference latency is dominated by memory access patterns, and any source of memory access unpredictability, whether cache eviction from core migration, remote NUMA access, or TLB misses from address space switching, directly degrades tail latency.

[^fn-numa-node-inference]: **NUMA (Non-Uniform Memory Access)**\index{NUMA!inference latency}: Local memory access takes approximately 100 ns vs. 150 to 200 ns for remote-socket access, a 50 to 100 percent penalty. For ML inference preprocessing, placing CPU cores on the wrong NUMA domain relative to the GPU's PCIe connection adds this penalty to every host-device transfer, inflating P99 latency by 10 to 30 percent for sub-millisecond inference tasks. The fix is straightforward (CPU pinning and NUMA-aware pod scheduling) but often overlooked in Kubernetes deployments where the default scheduler ignores NUMA topology entirely.

The isolation techniques discussed here represent a spectrum from stronger resource partitioning (MIG) to flexible software sharing (MPS and time-slicing), and the choice depends on the trust boundary between colocated workloads. MIG does not by itself create a complete external-tenant security boundary. A multi-tenant platform must combine device partitioning with VM or container isolation, IOMMU configuration, driver and runtime hardening, authentication and authorization, and network and storage controls. Single-organization workloads with a narrower trust boundary can use MPS or time-slicing when weaker fault and performance isolation is acceptable.

### GPU sharing economics {#sec-fleet-orchestration-gpu-sharing}

The decision to co-locate multiple models on a single GPU defines the efficiency frontier of an inference fleet. This choice operates on a spectrum: at one end, **Exclusive Access**\index{Exclusive Access!definition} provides device-resource isolation but strands capacity; at the other, **Aggressive Sharing**\index{Aggressive Sharing!definition} (via MPS or time-slicing) maximizes utilization but risks latency interference. Exclusive device assignment still depends on the surrounding host, driver, identity, network, and storage controls for tenant security. The physics of the packing trade-off are dictated by memory fragmentation and compute contention.
...
class GpuSharingRoi:
    # ┌── 1. LOAD ──────────────────────────────────────────
    n_small = 100
    small_footprint = 26 * GB  # scenario assumption
    n_large = 10
...
At fleet scope, routing begins with a capacity contract: the scheduler must place and maintain each serving replica as an indivisible execution group, reserve enough headroom for its SLO, and expose current readiness and memory pressure to the serving plane. A 175B parameter model may make an eight-GPU tensor-parallel group, rather than one GPU or one process, the schedulable replica. The fleet orchestrator decides how many such groups exist, where they reside, and which lower-priority allocations may yield capacity when demand changes. @Sec-inference-scale owns the per-request policy within that envelope, including batching, KV-cache-aware routing, admission, traffic shifting, and tail-latency control.

### The training-serving resource boundary {#sec-fleet-orchestration-boundary}

The deepest utilization divide in the ML fleet is the wall between training and serving. Most organizations manage these as separate fiefdoms: a "Production Serving" cluster that must handle peak traffic (and sits 50 percent idle at night) and a "Research Training" cluster that is perpetually backlogged. This **Static Partitioning**\index{Static Partitioning!definition} is safe but economically inefficient. **Dynamic Sharing**\index{Dynamic Sharing!definition} unifies these pools, but it introduces a fundamental friction: the mode-switching cost.
...
Multi-tenancy and quotas provide the organizational and technical firewalls needed to prevent this degradation. The quota systems discussed here establish formal policies for resource allocation, borrowing, and reclamation, so fair access and high overall utilization become enforceable scheduler properties rather than managerial negotiation.
...
Hierarchical quotas resolve this tension by enabling departmental limits with sub-team flexibility and cross-team borrowing. @Eq-fleet-orchestration-quota bounds the effective quota for any team by both its own allocation and the parent department's capacity that sibling teams have not already claimed, where $U_{\text{allocated}}$ is the resources currently committed to each of the other teams in the department:
...
This borrowing mechanism is essential for achieving high utilization in multi-tenant clusters. Without borrowing, organizations face a painful choice between reserving enough quota to handle each team's peak demand, which leaves capacity idle when peaks do not coincide, and allocating less, which blocks teams during peak periods even when reclaimable capacity exists elsewhere. Borrowing provides a reclaimable entitlement subject to the platform's response-time objective, safety headroom, and current physical capacity. A team entitled to 500 GPUs can reclaim borrowed devices according to that contract, but the scheduler must not promise an immediate 500-GPU allocation when failures, nonpreemptible reservations, or recovery headroom reduce reclaimable capacity.
...
In a shared GPU cluster, static quotas create a burst-capacity paradox: Team A has a guarantee of 64 GPUs but needs 256 for a weekend-long training run, while Team B's guaranteed share sits idle. The solution separates a guaranteed quota from a maximum quota. Team A may borrow idle whole devices up to its maximum, but those devices remain part of physical cluster capacity rather than becoming virtual capacity. If Team B reclaims its guarantee, the scheduler preempts or shrinks Team A's opportunistic work.

Burst capacity expands aspirational maxima and fair-share weights, not simultaneous guarantees. The sum of guarantees that the platform promises to honor concurrently must fit reclaimable physical capacity after nonpreemptible reservations and safety headroom. Teams may advertise higher maximums or weights because borrowed jobs yield when guaranteed demand returns, but those values are not guarantees. The admission controller must distinguish guaranteed from borrowed allocations, and borrowed jobs face preemption first. Fractional sharing is a separate mechanism that requires an explicit MIG, MPS, or time-slicing contract.

To manage this safely, clusters must implement strict priority classes:

- **Production Serving**\index{Production Serving}: Priority 0 workloads are nonpreemptible.
- **Production Training**\index{Production Training}: Priority 1 workloads handle core model updates and are only preemptible by serving outages.
- **Interactive/Debug**\index{Interactive/Debug}: Priority 2 workloads get high scheduling precedence but short time limits.
- **Batch/Research**\index{Batch/Research}: Priority 3 workloads are fully preemptible and fault-tolerant.
...
The safe burst limit follows from reclaimability rather than average SM utilization. The scheduler must measure how quickly borrowed jobs can checkpoint, shrink, or terminate; reserve the I/O and network bandwidth required for that transition; and bound the amount of opportunistic capacity that can be reclaimed within the guarantee's response-time objective. A job that averages 70 percent SM activity may still reserve all device memory and require every rank in its gang, so the remaining 30 percent does not constitute an allocatable device fraction.
...
The distinction is operational, not merely financial. A team might be allocated 500 GPUs but use 350 (70 percent compute utilization) and productively use 280 (56 percent of allocation). The 44 percent gap tells the scheduler where capacity disappears, but it does not by itself prove that the job can share or release whole devices.
...
Attribution in a shared fleet presents a control problem. Every admitted job needs an owner, project, workload class, and preemption contract so the scheduler can apply quota and recovery policy consistently. Kubernetes labels and Slurm accounts carry this scheduling identity. @Sec-ops-scale develops how organizations convert the resulting usage records into showback, chargeback, portfolio accountability, and platform investment decisions.
...
In Kubernetes environments, namespace separation provides the fundamental isolation boundary. Each team operates within dedicated namespaces with role-based access control limiting visibility to their own resources. Role-based policies control who can submit jobs, view logs, access model artifacts, and modify scheduling policies within each namespace, providing organizational governance over cluster usage.
...
@Tbl-gpu-virtualization-isolation-spectrum compares GPU virtualization options across isolation strength, resource efficiency, and workload fit.
...
**Diagnosis**: Upstream data delays postpone training, but the team retains the reservation to ensure future availability. The GPUs sit idle for three weeks, accumulating `{python} QuotaHoardingCost.hoarding_cost_str` in capital waste while other teams' queues back up.

**Systems lesson**: Static quota reservations cause severe capacity hoarding. Fleet orchestrators must implement dynamic "use it or lose it" reclamation policies, automatically demoting idle reserved nodes (<20 percent utilization for 72h) to preemptible spot pools.
...
The communication bottleneck reveals itself through periodic, synchronized drops in SM utilization across all ranks in a distributed group. For the 175B model, synchronizing gradients for 350 GB of parameters requires massive bandwidth; if the network is undersized or congested, the GPUs spend more time waiting for AllReduce to complete than performing computation. The diagnostic metric is the step time ratio. If total iteration time exceeds 1.5$\times$ compute-only time (forward plus backward passes in isolation), non-compute overhead exceeds 0.5$\times$ compute time, which is material but not dominant. Under the simplified decomposition $T_{\text{total}} = T_{\text{compute}} + T_{\text{comm}}$, communication exceeds computation only when the ratio is greater than 2$\times$; profiling must also separate communication from input and runtime overhead. Remediation requires topology-aware placement to keep traffic on high-bandwidth switch tiers or algorithmic changes like gradient accumulation to increase the compute-to-communication ratio.
...
Fleet orchestration transforms raw data center capacity into productive ML infrastructure by governing how silicon, bandwidth, and state interact across time and space. In terms of the iron law of performance ($T \approx D_{\text{vol}}/\text{BW} + O/(R_{\text{peak}} \cdot \eta_{\text{hw}}) + L_{\text{lat}}$), the orchestrator is the system's central optimizer: placement strategies preserve fabric bandwidth ($\text{BW}$), multi-tenancy and quota policies drive realized hardware efficiency ($\eta_{\text{hw}}$), and distributed scheduling algorithms bound queue delay and coordination overhead ($L_{\text{lat}}$). The difference between a high-performing cluster and thousands of idle accelerators is not hardware capability, but the scheduler's ability to navigate the trade-offs between atomic consistency, fault tolerance, and placement topology under partial failures and bursty demand.

The chapter's unifying technical insight is that placement directly dictates physical performance. By aligning model parallelism with the physical network hierarchy, topology-aware scheduling prevents cross-rack bisection bottlenecks, protecting the $D_{\text{vol}}/\text{BW}$ communication term and increasing throughput by 15 to 30 percent. Domain-specific schedulers—such as Tiresias, Gandiva, Themis, and Pollux—exploit the structure of ML workloads (iterative progress, heavy-tailed runtimes, and predictable convergence scaling) to minimize the coordination and wait-time terms ($L_{\text{lat}}$), cutting average job completion time by 40 to 60 percent compared to general-purpose FIFO policies. Elastic training and spot management extend this control by trading worker continuity for economic flexibility, provided jobs can dynamically adapt without incurring unacceptable preemption tax.

Fairness is likewise a constrained allocation problem rather than a single quota percentage. DRF equalizes each tenant's largest normalized resource share, but gang size, topology locality, and accelerator compatibility determine which nominal allocations are feasible. Slowdown and maximum feasible wait then reveal whether equal resource shares translate into equitable access over time.

At serving time, the fleet scheduler preserves a capacity envelope rather than choosing individual request routes. It places indivisible replicas, maintains SLO headroom, and uses measured demand to move the training-serving boundary. @Sec-inference-scale develops request admission, batching, routing, KV-cache policy, and tail-latency control within that envelope. Multi-tenant isolation and fair-share quota governance remain orchestration concerns because they decide which workloads may share hardware and which allocations yield under pressure.

Ultimately, fleet orchestration is diagnostic and economic as much as algorithmic. Operational pathologies such as zombie jobs, data starvation, topology mismatch, and pool fragmentation are symptoms of policy-infrastructure misalignment rather than deficient hardware. When compute nodes are defined, networks wired, storage attached, fault tolerance integrated, and orchestrators aligned, the fleet transitions from a collection of isolated servers into a coherent, self-healing computing platform.

Topology-aware placement makes the C$^3$ exchange concrete. Co-locating a tensor-parallel gang raises useful compute efficiency by keeping its collectives on NVLink and reducing cross-rack communication, but the scheduler pays more coordination cost while it waits for a feasible contiguous allocation. The bottleneck can therefore move from collective bandwidth to queue delay or fragmentation. Whether the move is worthwhile depends on end-to-end job completion time, not iteration throughput alone.
...
* **Fairness is measured over feasible bundles**: DRF equalizes each tenant's dominant normalized share across GPUs, CPU, memory, and network. Gang, topology, and compatibility constraints must filter infeasible bundles first, while slowdown and maximum feasible wait test whether the resulting policy avoids delay and starvation.
...
* **Policies must evolve with infrastructure**: Scheduling algorithms\index{Scheduling Policy!infrastructure coupling} are only as effective as the policies they implement. When infrastructure changes (heterogeneous hardware additions, topology upgrades, workload mix shifts), policies must be re-evaluated and updated (principle \ref{pri-conservation-overhead}).

:::

The fleet now has compute, communication, storage, and the resilience to keep them running. It still needs a policy that decides what runs where. A scheduler that chases peak utilization can deadlock when large jobs compete for the same nodes, while a scheduler that guarantees progress may leave some capacity deliberately unused. The gap between theoretical and safely sustainable throughput is the price of coordination. At fleet scale, performance therefore depends not only on hardware speed but also on the policy that allocates it.

::: {.callout-chapter-connection title="From orchestration to optimization"}

With distributed execution established in @sec-distributed-training-systems and fault-tolerant checkpointing in @sec-fault-tolerance-reliability, multi-tenant resource management completes the operational foundation of the machine learning fleet. With the orchestrator placing workloads on the right hardware tiers, the next challenge is maximizing single-node and intra-step efficiency. @Sec-performance-engineering examines operator fusion, precision engineering, graph compilation, and execution scheduling. Orchestration allocates the right resources; performance engineering extracts peak throughput from them.
