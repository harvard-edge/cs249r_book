...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=latex]
  ...
  \begin{scope}[local bounding box=scopeA]
    ...
    \node[anchor=south west, text=RedLine, font=\bfseries] (titleA) at (0, 2.8) {A. Independent Scheduling (Deadlock)};

    % Job B (bottom)
    \node[JobBox, fill=OrangeL, minimum width=4cm] (jobB) at (1, 0.2) {Job B (Hold)};
    \draw[WaitArrow, OrangeLine] (jobB.east) -- ++(2,0) node[right, text=black] {Wait};

    % Job A (top)
    \node[JobBox, fill=BlueL, minimum width=4cm] (jobA) at (0, 1.25) {Job A (Hold)};
  ...
  \begin{scope}[shift={(0,-4.5)}, local bounding box=scopeB]
    ...
    \node[anchor=south west, text=GreenLine, font=\bfseries] (titleB) at (0, 2.8) {B. Gang Scheduling (Atomic)};

    % Job A
    \node[JobBox, fill=BlueL, minimum width=3cm, minimum height=2cm] (jobA_gang) at (0, 0.2) {Job A (All)};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85, transform shape, >=latex]
  \tikzset{
    ...
    nodebox/.style={draw=black!30, fill=black!3, rounded corners=6pt, line width=0.75pt,
                    minimum width=4.5cm, minimum height=2.5cm, anchor=south west},
  ...
  \begin{scope}[local bounding box=leftScope]
    \node[Title, text=GreenLine] (titleLeft) at (2.25, 3.2) {Optimal (Single Node)};

    % Node box
    \node[nodebox] (opt_node) at (0, 0) {};
    \node[font=\tiny, anchor=south] (node0_lbl) at (2.25, 0.1) {Node 0};
    ...
    \foreach \i in {4,...,7} {
      ...
      \node[gpu, fill=GreenL] (og\idx) at (\xpos, 1.1) {G\idx};
  ...
  \begin{scope}[shift={(6,0)}, local bounding box=rightScope]
    \node[Title, text=RedLine] (titleRight) at (2.25, 3.2) {Scattered (Two Nodes)};

    % Node A
    \node[nodebox, minimum width=4.5cm, minimum height=1.2cm] (sn_a) at (0, 1.3) {};
    \node[font=\tiny, anchor=south] at (2.25, 1.4) {Node 0};
    \node[switch, fill=GreenL, minimum width=1.2cm] (nvsw_a) at (2.25, 2.1) {NVSw};

    \foreach \i in {0,...,3} {
      \pgfmathsetmacro{\xpos}{0.6 + \i * 1.1}
      \node[gpu, fill=GreenL] (sg\i) at (\xpos, 1.7) {G\i};
      \draw[nvpath] (sg\i) -- (nvsw_a);
    }

    % Node B
    \node[nodebox, minimum width=4.5cm, minimum height=1.2cm] (sn_b) at (0, -0.2) {};
    \node[font=\tiny, anchor=south] at (2.25, -0.1) {Node 1};
    \node[switch, fill=GreenL, minimum width=1.2cm] (nvsw_b) at (2.25, 0.6) {NVSw};

    \foreach \i in {4,...,7} {
      \pgfmathsetmacro{\xpos}{0.6 + (\i-4) * 1.1}
      \pgfmathtruncatemacro{\idx}{\i}
      \node[gpu, fill=RedL] (sg\idx) at (\xpos, 0.2) {G\idx};
