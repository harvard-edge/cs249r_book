...
cluster_size_str = FleetSetup.cluster_size_str
daily_cost_str = FleetSetup.daily_cost_str
annual_cost_str = FleetSetup.annual_cost_str
total_gpus_str = FleetSetup.total_gpus_str
frag_pct_str = FleetSetup.frag_pct_str
nvlink_h100_str = FleetSetup.nvlink_h100_str
nvlink_a100_str = FleetSetup.nvlink_a100_str
ib_ndr_str = FleetSetup.ib_ndr_str
ib_ndr_gbs_str = FleetSetup.ib_ndr_gbs_str
spot_price_str = FleetSetup.spot_price_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
cluster_size_str = ClusterEconomicsContext.cluster_size_str
daily_cost_str = ClusterEconomicsContext.daily_cost_str
annual_cost_str = ClusterEconomicsContext.annual_cost_str
daily_waste_str = ClusterEconomicsContext.daily_waste_str
annual_waste_str = ClusterEconomicsContext.annual_waste_str
equivalent_gpus_str = ClusterEconomicsContext.equivalent_gpus_str
gpu_hour_cost_usd = ClusterEconomicsContext.gpu_hour_cost_usd
idle_fraction = ClusterEconomicsContext.idle_fraction
util_low = ClusterEconomicsContext.util_low
util_high = ClusterEconomicsContext.util_high
a100_mem_str = ClusterEconomicsContext.a100_mem_str
h100_mem_str = ClusterEconomicsContext.h100_mem_str
```
...
class QueuingTheory:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(wq_uniform == 4, f"Expected 4, got {wq_uniform}")
    check(wq_ml == 20, f"Expected 20, got {wq_ml}")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    wq_uniform_str = fmt(wq_uniform, precision=0)
    wq_ml_str = fmt(wq_ml, precision=0)

wq_uniform_str = QueuingTheory.wq_uniform_str
wq_ml_str = QueuingTheory.wq_ml_str
```
...
topo_speedup_str = TopologyPlacement.speedup_str
```
...
Proper scheduling yields a **`{python} topo_speedup_str`$\times$ speedup** purely by respecting the network topology, with zero changes to the model code.
:::
...
es_s1_str = ElasticScaling.es_s1_str
es_s2_str = ElasticScaling.es_s2_str
es_s3p1_str = ElasticScaling.es_s3p1_str
es_s3p2_str = ElasticScaling.es_s3p2_str
es_s3_str = ElasticScaling.es_s3_str
es_rescale_str = ElasticScaling.es_rescale_str
```
...
**Assumptions**: Throughput at 128 GPUs is 1.0 epoch/hour (baseline). Throughput at 256 GPUs is 1.8 epochs/hour (90 percent scaling efficiency). Throughput at 512 GPUs is 3.2 epochs/hour (80 percent scaling efficiency). Re-scaling cost is 10 minutes (`{python} es_rescale_str` hours) to checkpoint, re-shard, and restart.

**Strategy 1 -- Immediate start (no scaling)**: The job starts immediately with 128 GPUs and runs for the full 24 hours. Total work: $24 \times 1.0 =$ `{python} es_s1_str` epochs.

**Strategy 2 -- Wait for capacity (gang scheduling)**: The job waits in the queue for 4 hours until 512 GPUs become available, then runs for 20 hours. Total work: $20 \times 3.2 =$ `{python} es_s2_str` epochs.

**Strategy 3 -- Elastic scaling (step-up)**: The job starts immediately with 256 GPUs. After 4 hours, 256 more GPUs become available. The job pauses, scales up, and resumes. Phase 1: $4 \times 1.8 =$ `{python} es_s3p1_str` epochs. Phase 2: $(24 - 4 - `{python} es_rescale_str`) \times 3.2 \approx$ `{python} es_s3p2_str` epochs. Total work: $`{python} es_s3p1_str` + `{python} es_s3p2_str` =$ `{python} es_s3_str` epochs.
...
as_capacity_str = AutoscalingLag.as_capacity_str
as_detect_str = AutoscalingLag.as_detect_str
as_ready_str = AutoscalingLag.as_ready_str
as_excess_str = AutoscalingLag.as_excess_str
as_violations_str = AutoscalingLag.as_violations_str
as_pred_trigger_str = AutoscalingLag.as_pred_trigger_str
as_pred_capacity_str = AutoscalingLag.as_pred_capacity_str
```

::: {.callout-notebook title="The Autoscaling Lag"}

Consider a serving fleet with 10 replicas, each capable of 5 queries per second (QPS) while meeting the 500 ms SLO. Total capacity: `{python} as_capacity_str` QPS. <!-- lego-ok: replica count, QPS/replica, traffic shape, and load time are input assumptions -->

**Scenario**: Traffic ramps from 40 QPS to 80 QPS linearly over 60 seconds. Model load time: 3 minutes (180 seconds).

**Reactive scaling**: HPA detects overload at $t =$ `{python} as_detect_str` s (when traffic hits `{python} as_capacity_str` QPS). It requests 10 new replicas. These replicas become ready at $t =$ `{python} as_ready_str` s (`{python} as_detect_str` + 180). From $t =$ `{python} as_detect_str` s to $t =$ `{python} as_ready_str` s (3 minutes), demand exceeds capacity. At peak (80 QPS), `{python} as_excess_str` QPS are queued or dropped. Over this 3-minute window, approximately `{python} as_violations_str` requests (`{python} as_excess_str` QPS$\times$ 120 s average excess) violate the SLO.

**Predictive scaling**: The scaler forecasts the ramp and triggers scale-up at $t =$ `{python} as_pred_trigger_str` s. The 10 new replicas come online at $t = 0$ s, pushing capacity to `{python} as_pred_capacity_str` QPS just as the ramp begins. Zero SLO violations.
...
gs_excl_util_str = GpuSharingRoi.gs_excl_util_str
gs_excl_total_mem_str = GpuSharingRoi.gs_excl_total_mem_str
gs_excl_large_str = GpuSharingRoi.gs_excl_large_str
gs_excl_total_str = GpuSharingRoi.gs_excl_total_str
gs_shared_gpus_str = GpuSharingRoi.gs_shared_gpus_str
gs_shared_mem_str = GpuSharingRoi.gs_shared_mem_str
gs_shared_util_str = GpuSharingRoi.gs_shared_util_str
gs_shared_total_str = GpuSharingRoi.gs_shared_total_str
gs_saved_gpus_str = GpuSharingRoi.gs_saved_gpus_str
gs_saved_pct_str = GpuSharingRoi.gs_saved_pct_str
gs_annual_str = GpuSharingRoi.gs_annual_str
```
...
**Strategy A -- Exclusive access (one model per GPU)**: The 7B models require 100 A100 GPUs (80 GB each), achieving only `{python} gs_excl_util_str` percent memory utilization ($100\times26$ GB used out of `{python} gs_excl_total_mem_str` GB total). The 175B models require `{python} gs_excl_large_str` GPUs (10 models$\times$ 8 GPUs each for tensor parallelism). Total fleet: `{python} gs_excl_total_str` GPUs.

**Strategy B -- Mixed sharing (MPS for small, exclusive for large)**: Pack 2 models per A100 for the 7B models ($2\times26$ = `{python} gs_shared_mem_str` GB, well within 80 GB), requiring only `{python} gs_shared_gpus_str` GPUs at `{python} gs_shared_util_str` percent memory utilization. The 175B models remain unchanged at `{python} gs_excl_large_str` GPUs. Total fleet: `{python} gs_shared_total_str` GPUs.

Sharing reduces the fleet size by `{python} gs_saved_pct_str` percent (`{python} gs_saved_gpus_str` fewer GPUs). At \$2 per GPU-hour, this saves approximately \$`{python} gs_annual_str` annually in compute costs, purely through scheduling policy.
...
ds_static_active_str = DiurnalGpuShift.ds_static_active_str
ds_static_util_str = DiurnalGpuShift.ds_static_util_str
ds_dynamic_active_str = DiurnalGpuShift.ds_dynamic_active_str
ds_dynamic_util_str = DiurnalGpuShift.ds_dynamic_util_str
ds_reclaimed_str = DiurnalGpuShift.ds_reclaimed_str
ds_annual_str = DiurnalGpuShift.ds_annual_str
```
...
**Scenario A -- Static Partitioning**: The serving pool reserves 400 GPUs to handle peak demand, but average usage is only 150 GPUs (37.5 percent utilization). The training pool reserves 600 GPUs, and backlog ensures 100 percent utilization. Result: $150 + 600 =$ `{python} ds_static_active_str` active GPUs. Fleet utilization: **`{python} ds_static_util_str` percent**.

**Scenario B -- Dynamic Sharing**: Serving claims GPUs strictly as needed (average 150). Training reclaims unused serving GPUs (average 250), minus a safety buffer. The orchestrator shifts 200 GPUs between roles twice daily based on diurnal forecasts. Result: serving (150) + training ($600 +$ `{python} ds_reclaimed_str` reclaimed) $=$ `{python} ds_dynamic_active_str` active GPUs. Fleet utilization: **`{python} ds_dynamic_util_str` percent**.

Dynamic sharing recovers `{python} ds_reclaimed_str` GPUs of effective capacity. At \$2 per GPU-hour, this creates approximately \$`{python} ds_annual_str` million per year in value without purchasing a single additional accelerator.
