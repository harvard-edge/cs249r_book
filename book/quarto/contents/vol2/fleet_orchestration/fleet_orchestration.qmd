...
from mlsysim.core.constants import (
    ...
    GPT3_PARAMS,
    GB, TB, second, Gbps, TFLOPs, param,
...
# │
# │ Goal: Establish cluster economics, scale, and hardware baselines.
# │ Show: 10,000-GPU cluster; ~$480k/day operating cost; fragmentation metrics.
# │ How: GPU_Count * hourly_rate * 24; pint .m_as() for hardware specs.
# │
# │ Imports: mlsysim.core.constants, mlsysim.book
# │ Exports: cluster_size_str, daily_cost_str, total_gpus_str, ...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware
from mlsysim.core.constants import (
    NVLINK_H100_BW, NVLINK_A100_BW, INFINIBAND_NDR_BW,
    GB, second, Gbps, TRILLION, MILLION, THOUSAND, BITS_PER_BYTE
)
...
class FleetSetup:
    """
    Central namespace for Fleet Orchestration constants.
    """
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # 1. Scale and Economics
    cluster_size = 10000
    hourly_rate = 2.0 # USD/GPU-hour
    daily_cost = cluster_size * hourly_rate * 24
    annual_cost = daily_cost * 365

    # 2. Fragmentation Scenario (64 nodes * 8 GPUs)
    n_nodes = 64
    gpus_per_node = 8
    ...
    nvlink_h100_val = NVLINK_H100_BW.m_as(GB/second)
    nvlink_a100_val = NVLINK_A100_BW.m_as(GB/second)
    ib_ndr_gbps_val = INFINIBAND_NDR_BW.m_as('Gbps')
    ib_ndr_gbs_val = INFINIBAND_NDR_BW.m_as(GB/second)

    # OUTPUTS
    cluster_size_str = fmt(cluster_size, precision=0)
    daily_cost_str = f"${daily_cost/THOUSAND:.0f}K"
    annual_cost_str = f"${annual_cost/MILLION:.0f}M"
    total_gpus_str = fmt(total_gpus, precision=0)
    frag_pct_str = fmt(frag_pct, precision=0)
    nvlink_h100_str = fmt(nvlink_h100_val, precision=0)
    nvlink_a100_str = fmt(nvlink_a100_val, precision=0)
    ib_ndr_str = fmt(ib_ndr_gbps_val, precision=0)
    ib_ndr_gbs_str = fmt(ib_ndr_gbs_val, precision=0)
    spot_price_str = "0.50"
```
...
# │ How: GPU_Count * hourly_rate * 24.
# │
# │ Imports: mlsysim.core.constants (GPUS_PER_HOST, A100_MEM_CAPACITY, H100_MEM_CAPACITY,
# │          GiB), mlsysim.book (fmt, check)
# │ Exports: FleetSetup.cluster_size_str, FleetSetup.daily_cost_str, FleetSetup.annual_cost_str,
# │          daily_waste_str, annual_waste_str, equivalent_gpus_str,
# │          gpu_hour_cost_usd, idle_fraction, util_low, util_high,
# │          a100_mem_str, h100_mem_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import GPUS_PER_HOST, A100_MEM_CAPACITY, H100_MEM_CAPACITY, GiB, MILLION
from mlsysim.fmt import fmt, check
...
class ClusterEconomicsContext:
    ...
    daily_cost = cluster_size * gpu_hour_cost_usd * 24
    annual_cost_m = (daily_cost * 365) / MILLION
    daily_waste = daily_cost * idle_fraction
    annual_waste_m = (daily_waste * 365) / MILLION
    gpu_gain = cluster_size * (util_high - util_low) / util_low
...
# │ How: P(allocation) = (Free_Gpus/Req_Gpus)^N_Jobs. (Conceptual)
# │
...
class GangSchedulingDeadlock:
    ...
    n_total_gpus = 1024
    n_jobs = 2
    gpus_per_job = 1024 # Each job needs the full cluster
    # ┌── 2. EXECUTE ───────────────────────────────────────
    # If scheduler allocates partially (e.g. 256 to each), no job can run.
    # Total idle waste per hour if deadlock occurs:
    gpu_hour_cost = 2.0 # $/hr
    waste_per_hour = n_total_gpus * gpu_hour_cost
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(waste_per_hour == 2048, f"Waste ${waste_per_hour} unexpected")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    gs_n_gpus_str = f"{n_total_gpus}"
    gs_n_jobs_str = f"{n_jobs}"
    gs_waste_m_str = f"{waste_per_hour:,.0f}"

```

::: {.callout-notebook title="The physics of deadlock"}

**Problem**: A `{python} GangSchedulingDeadlock.gs_n_gpus_str`-GPU cluster receives two frontier training jobs from separate teams, each requiring the full cluster. A naive scheduler (nongang) allocates 512 GPUs to Team A and 512 to Team B, then waits for more GPUs to become available. What is the steady-state outcome?

**Math**:

1. **Team A Status**\index{Team A Status}: Holds 512, Needs 1,024. Progress = **0 percent**.
2. **Team B Status**\index{Team B Status}: Holds 512, Needs 1,024. Progress = **0 percent**.
3. **Cluster Status**\index{Cluster!status}: 1,024 GPUs allocated, 0 samples processed.
4. **Waste**\index{Waste}: `{python} GangSchedulingDeadlock.gs_waste_m_str` dollars per hour in idle electricity and capital.
...
# │ How: W_q_mult = rho/(1-rho) * (1+Cs^2)/2
# │
...
Here, $\rho = \lambda/\mu$ represents cluster utilization, $1/\mu$ is the average job duration, and $C_s = \sigma \mu$ is the **coefficient of variation** of job duration. The right-hand side is therefore the waiting time expressed as a multiple of the average job duration.
...
At 80 percent utilization $(\rho = 0.8)$: with an exponential workload $(C_s = 1)$, $W_q =$ `{python} QueuingTheory.wq_uniform_str` $\times$ the average job duration. With a typical ML workload $(C_s = 3)$, $W_q =$ `{python} QueuingTheory.wq_ml_str` $\times$ the average job duration. This explains the "utilization wall" in ML infrastructure. A web server cluster feels responsive at 80 percent load, but an ML cluster at the same utilization feels broken, with jobs languishing in the queue for days. The heavy tail of the service distribution acts as a latency multiplier, forcing operators to run ML clusters at lower utilization (often 60 to 70 percent) to maintain acceptable responsiveness for interactive users.
...
Consider a 64-node cluster with 8 GPUs per node, totaling `{python} FleetSetup.total_gpus_str` GPUs. If jobs request 6 GPUs each, each job occupies one full node but wastes 2 GPUs per node, reducing effective capacity to `{python} f"{100 - float(FleetSetup.frag_pct_str):.0f}"` percent. The remaining 2 GPUs per node cannot be combined across nodes because GPU workloads require local memory access. This **fragmentation** grows worse with heterogeneous job sizes: a mix of 1-GPU, 3-GPU, and 7-GPU jobs creates irregular gaps distributed across many nodes, where no single pending job can fit into any individual gap, yet the total free capacity would be sufficient if the gaps were contiguous.
...
The cost of poor scheduling compounds rapidly. Consider a `{python} ClusterEconomicsContext.cluster_size_str`-GPU cluster:

- **Operating cost**\index{Operating Cost}: \$`{python} ClusterEconomicsContext.daily_cost_str`/day (\$`{python} ClusterEconomicsContext.annual_cost_str`M/year)
- **At 60 percent utilization**\index{At 60 Percent Utilization}: 6,000 GPUs productive, 4,000 idle = \$192,000/day wasted
- **At 80 percent utilization**\index{At 80 Percent Utilization}: 8,000 GPUs productive, 2,000 idle = \$96,000/day wasted
- **Improvement value**\index{Improvement Value}: Moving from 60 percent to 80 percent saves \$96,000/day, or about \$35M/year
...
The traditional gang scheduling model assumes a homogeneous Bulk Synchronous Parallel (BSP) workload: $N$ identical workers executing the exact same compute graph. However, advanced training paradigms fundamentally break this assumption. Reinforcement Learning from Human Feedback (RLHF) requires running four models of different sizes simultaneously: an Actor model (training), a Critic model (training), a Reference model (inference), and a Reward model (inference).

::: {.callout-perspective title="Routing over synchrony"}
...
| **inference**\index{Inference} | 2$\times$ A10G | Ethernet         | Model serving      |
...
Finally, **Trackable Resources (TRES)** extend Slurm's accounting beyond simple CPU/memory tracking to arbitrary assets like GPU-hours, license tokens, or power budget. Organizations use TRES to implement project billing for expensive resources. By defining a TRES type `billing=gpu`, administrators can charge different rates for A100s vs. V100s, or track the consumption of specific software licenses. This granular accounting enables the implementation of fair-share policies based on actual economic cost rather than just core counts, ensuring that a team using 100 older GPUs does not deplete its budget as fast as a team using 100 flagship H100s.
...
| **MIG Profile** | **GPU Memory**\index{GPU Memory} | **SM Count** | **Typical Workload** |
|:----------------|:---------------------------------|:-------------|:---------------------|
...
The scheduling paradigms discussed earlier provide the infrastructure for resource allocation, but they treat GPUs as interchangeable units within a partition. The next section examines a critical refinement: how the *physical location* of allocated GPUs within the cluster's network topology affects training performance, and how topology-aware scheduling algorithms exploit this structure.
...
::: {.callout-notebook title="The topology placement impact"}
...
- Tensor parallel groups: 32 groups of 8 GPUs, each confined to one 8-GPU node (NVLink: `{python} FleetSetup.nvlink_h100_str` GB/s)
- Pipeline stages: 4 consecutive nodes within the same rack (1 switch hop, InfiniBand: `{python} FleetSetup.ib_ndr_gbs_str` GB/s)
...
1.  **Significance (quantitative)**: It maximizes the **System Duty Cycle** $(\eta_{\text{hw}})$ by allowing training to continue through node failures and by absorbing idle capacity in the cluster. It requires **Learning Rate Recalibration** and gradient accumulation adjustment to maintain mathematical consistency as the global batch size changes.
2.  **Distinction (durable)**: Unlike **Standard Fault Tolerance** (which relies on checkpoint-restart), Elastic Training is **Always-Live**\index{Always-Live}: the optimization loop never stops, it only changes its parallel width.
3.  **Common pitfall**: A frequent misconception is that elasticity is "automatic." In reality, it is a **Distributed Coordination Problem**\index{Distributed Coordination Problem}: the training framework, the data loader, and the orchestrator must all synchronize their state to ensure that no data samples are lost or double-counted during worker scaling.
...
In the traditional non-elastic case, when a node fails, the entire job must stop, synchronize all remaining workers, reload the most recent checkpoint, and restart with a new allocation that replaces the failed node. The restart overhead (checkpoint loading, optimizer state reconstruction, data pipeline warmup) wastes minutes to hours depending on model size and checkpoint storage bandwidth. During this time, all the surviving workers sit idle, waiting for the new allocation to be provisioned and the new worker to catch up. For a 1,024-GPU job where a single node (8 GPUs) fails, the remaining 1,016 GPUs are entirely unproductive during recovery.
...
The mathematical implications of elastic scaling interact with the optimization dynamics of stochastic gradient descent in ways that affect convergence. The effective batch size changes with worker count according to $B_{\text{effective}} = B_{\text{per\_worker}} \times N_{\text{workers}}$. Changing $N_{\text{workers}}$ mid-training alters the gradient noise level, which can affect convergence speed and final model quality. Two strategies handle this tension, each with distinct trade-offs.

The first approach, constant batch size scaling, adjusts $B_{\text{per\_worker}}$ inversely with $N_{\text{workers}}$ to maintain $B_{\text{effective}}$ at its original value. When workers are added, each worker processes fewer samples per step; when workers are removed, each processes more. This approach preserves the optimization dynamics exactly (the gradient noise level is unchanged), but changes per-worker compute and memory requirements. If $B_{\text{per\_worker}}$ becomes too small after aggressive scale-up, per-GPU utilization drops because the batch cannot saturate the GPU's compute units. If $B_{\text{per\_worker}}$ becomes too large after scale-down, it may exceed per-GPU memory capacity.

The alternative, adaptive batch size scaling, keeps $B_{\text{per\_worker}}$ constant and adjusts the learning rate using scaling rules like the linear scaling rule of @goyal2017accurate, which increases the learning rate proportionally with batch size. This approach accepts changed optimization dynamics (larger batch sizes produce lower-variance gradient estimates, which may require different learning rate schedules) in exchange for simpler worker management (each worker's compute and memory requirements remain constant regardless of group size). For many practical workloads, adaptive batch size with linear learning rate scaling produces equivalent convergence outcomes, but the interaction between batch size scaling and other training hyperparameters (warmup schedule, weight decay, momentum) requires careful tuning.

### Framework support {#sec-fleet-orchestration-elastic-frameworks}

TorchElastic (torchelastic), integrated into PyTorch's `torch.distributed.elastic` module, provides the runtime infrastructure for elastic training. Jobs specify minimum and maximum worker counts, and TorchElastic manages worker lifecycle, as @lst-fleet-orchestration-torchelastic illustrates:

::: {#lst-fleet-orchestration-torchelastic lst-cap="**TorchElastic Launch Configuration**\index{TorchElastic Launch Configuration}: Elastic training specification allowing a job to run with between 32 and 128 GPUs. The rendezvous backend coordinates worker group membership changes, while the framework handles gradient synchronization group reconstruction when workers join or leave."}
...
class ElasticScaling:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    es_s1_str = f"{s1_work:.1f}"
    es_s2_str = f"{s2_work:.1f}"
    es_s3p1_str = f"{s3_phase1:.1f}"
    es_s3p2_str = f"{s3_phase2:.1f}"
    es_s3_str = f"{s3_work:.1f}"
    es_rescale_str = f"{rescale_h:.2f}"
```

::: {.callout-notebook title="The elastic scaling decision"}

Consider a large language model training job with a target batch size that requires 512 A100s for peak efficiency. Three scheduling strategies are evaluated over a 24-hour window. <!-- lego-ok: throughputs and timing are input assumptions -->

**Assumptions**\index{Assumptions}: Throughput at 128 GPUs is 1.0 epoch/hour (baseline). Throughput at 256 GPUs is 1.8 epochs/hour (90 percent scaling efficiency). Throughput at 512 GPUs is 3.2 epochs/hour (80 percent scaling efficiency). Re-scaling cost is 10 minutes (`{python} ElasticScaling.es_rescale_str` hours) to checkpoint, re-shard, and restart.

**Strategy 1 -- Immediate start (no scaling)**: The job starts immediately with 128 GPUs and runs for the full 24 hours. Total work: $24 \times 1.0 =$ `{python} ElasticScaling.es_s1_str` epochs.

**Strategy 2 -- Wait for capacity (gang scheduling)**: The job waits in the queue for 4 hours until 512 GPUs become available, then runs for 20 hours. Total work: $20 \times 3.2 =$ `{python} ElasticScaling.es_s2_str` epochs.

**Strategy 3 -- Elastic scaling (step-up)**: The job starts immediately with 256 GPUs. After 4 hours, 256 more GPUs become available. The job pauses, scales up, and resumes. Phase 1: $4 \times 1.8 =$ `{python} ElasticScaling.es_s3p1_str` epochs. Phase 2: \((24 - 4 - `{python} ElasticScaling.es_rescale_str`) \times 3.2 \approx `{python} ElasticScaling.es_s3p2_str`\) epochs. Total work: `{python} ElasticScaling.es_s3p1_str` + `{python} ElasticScaling.es_s3p2_str` = `{python} ElasticScaling.es_s3_str` epochs.
...
from mlsysim.core.constants import HOURS_PER_YEAR, MILLION, THOUSAND
...
waste_10k_50 = 10000 * (1 - 0.50) * gpu_cost_per_hour * HOURS_PER_YEAR / MILLION
...
**On-demand**\index{On-Demand}: 512 GPUs $\times$ 336 hours $\times$ \$2.00/GPU-hour = **\$344,064**

**Spot** (65 percent discount, 5 percent checkpoint overhead, 1 interruption/day with 30 min restart):

- Base cost: 512 $\times$ 336 $\times$ \$0.70 = \$120,422
- Checkpoint overhead: 5 percent $\times$ 336 hours = 16.8 extra hours
...
**Savings**\index{Savings}: \$215,192 (63 percent), at the cost of approximately 7 percent longer wall-clock time.
...
**Tiresias**\index{Tiresias} [@gu2019tiresias] addresses the fundamental problem identified in @sec-fleet-orchestration-gang-scheduling: ML job durations are unpredictable, and backfill scheduling degrades when users cannot provide accurate runtime estimates. Users consistently overestimate or underestimate runtimes by 2--5$\times$, and the game-theoretic incentives encourage overestimation (to avoid being killed before completion), further degrading scheduler performance.
...
| **Scheduler**\index{Scheduler} | **Key Insight**                            | **Scheduling Signal** | **Optimization Target**  | **Overhead**                | **Production Readiness**    |
|:-------------------------------|:-------------------------------------------|:----------------------|:-------------------------|:----------------------------|:----------------------------|
...
Autoscaling for inference must also handle **cold start latency**\index{Cold Start Latency}, which creates a tension between cost efficiency and responsiveness that has no simple resolution. The "cold start" problem in serving is quantitatively more severe than in standard web services due to the sheer mass of model weights. Loading a large model into GPU memory takes 30 seconds to several minutes, depending on model size, quantization level, and storage bandwidth, comparing Non-Volatile Memory Express (NVMe) SSD against network storage. A 70B parameter model stored in FP16 occupies approximately 140 GB on disk and requires transferring all weights to GPU memory before the first inference request can be served. Loading a 70B parameter model (approx. 140 GB in FP16) into VRAM is bound by PCIe or NVLink bandwidth. Even at the per-direction PCIe Gen4 x16 speed of 32 GB/s, simple data transfer takes over 4 seconds; in practice, model initialization, graph compilation, and safety checks often extend this to 60–120 seconds. Even with NVMe SSDs delivering 7 GB/s read bandwidth, loading takes approximately 20 seconds; with network storage, it can exceed 2 minutes.
...
class AutoscalingLag:
    ...
    as_violations_str = f"{slo_violations:,}"
...
Consider a serving fleet with 10 replicas, each capable of 5 queries per second (QPS) while meeting the 500 ms SLO. Total capacity: `{python} AutoscalingLag.as_capacity_str` QPS. <!-- lego-ok: replica count, QPS/replica, traffic shape, and load time are input assumptions -->

**Scenario**: Traffic ramps from 40 QPS to 80 QPS linearly over 60 seconds. Model load time: 3 minutes (180 seconds).

**Reactive scaling**\index{Reactive Scaling}: HPA detects overload at $t =$ `{python} AutoscalingLag.as_detect_str` s (when traffic hits `{python} AutoscalingLag.as_capacity_str` QPS). It requests 10 new replicas. These replicas become ready at $t =$ `{python} AutoscalingLag.as_ready_str` s (`{python} AutoscalingLag.as_detect_str` + 180). From $t =$ `{python} AutoscalingLag.as_detect_str` s to $t =$ `{python} AutoscalingLag.as_ready_str` s, demand exceeds capacity. During the ramp, excess demand grows linearly from 0 to `{python} AutoscalingLag.as_excess_str` QPS; after the ramp reaches 80 QPS, the full `{python} AutoscalingLag.as_excess_str` QPS overload persists until the new replicas are ready. The area above capacity is approximately `{python} AutoscalingLag.as_violations_str` requests, all of which are queued or dropped and violate the SLO.

**Predictive scaling**: The scaler forecasts the ramp and triggers scale-up at $t =$ `{python} AutoscalingLag.as_pred_trigger_str` s. The 10 new replicas come online at $t = 0$ s, pushing capacity to `{python} AutoscalingLag.as_pred_capacity_str` QPS just as the ramp begins. Zero SLO violations.
...
from mlsysim.fmt import fmt, check
from mlsysim.core.constants import (
    HOURS_PER_YEAR, GPT3_PARAMS, param, BYTES_FP16, byte, GB,
    A100_MEM_CAPACITY, GiB,
)
...
class GpuSharingRoi:
    ...
    n_large = 10
    large_footprint_gb = (GPT3_PARAMS.m_as(param) * BYTES_FP16).m_as(GB)  # 175B FP16
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    gs_excl_util_str = fmt(excl_util_pct, precision=0)
    gs_excl_total_mem_str = f"{excl_total_mem_gb:,}"
    gs_excl_large_str = fmt(excl_large_gpus, precision=0)
    gs_excl_total_str = fmt(excl_total, precision=0)
    gs_shared_gpus_str = fmt(shared_small_gpus, precision=0)
    gs_shared_mem_str = fmt(shared_mem_per_gpu, precision=0)
    gs_shared_util_str = fmt(shared_util_pct, precision=0)
    gs_shared_total_str = fmt(shared_total, precision=0)
    gs_saved_gpus_str = fmt(saved_gpus, precision=0)
    gs_saved_pct_str = fmt(saved_pct, precision=0)
    gs_annual_str = f"{annual_savings:,.0f}"
```

::: {.callout-notebook title="GPU sharing ROI"}

Consider a fleet serving 100 distinct 7B models (26 GB footprint each) and 10 distinct 175B models (350 GB footprint each). <!-- lego-ok: model counts, footprints, and GPU specs are input assumptions -->

**Strategy A -- Exclusive access (one model per GPU)**: The 7B models require 100 A100 GPUs (80 GB each), achieving only `{python} GpuSharingRoi.gs_excl_util_str` percent memory utilization ($100 \times 26$ GB used out of `{python} GpuSharingRoi.gs_excl_total_mem_str` GB total). The 175B models require `{python} GpuSharingRoi.gs_excl_large_str` GPUs (10 models $\times$ 8 GPUs each for tensor parallelism). Total fleet: `{python} GpuSharingRoi.gs_excl_total_str` GPUs.

**Strategy B -- Mixed sharing (MPS for small, exclusive for large)**: Pack 2 models per A100 for the 7B models ($2 \times 26$ = `{python} GpuSharingRoi.gs_shared_mem_str` GB, well within 80 GB), requiring only `{python} GpuSharingRoi.gs_shared_gpus_str` GPUs at `{python} GpuSharingRoi.gs_shared_util_str` percent memory utilization. The 175B models remain unchanged at `{python} GpuSharingRoi.gs_excl_large_str` GPUs. Total fleet: `{python} GpuSharingRoi.gs_shared_total_str` GPUs.

Sharing reduces the fleet size by `{python} GpuSharingRoi.gs_saved_pct_str` percent (`{python} GpuSharingRoi.gs_saved_gpus_str` fewer GPUs). At \$2 per GPU-hour, this saves approximately \$`{python} GpuSharingRoi.gs_annual_str` annually in compute costs, purely through scheduling policy.

:::

### Model routing and traffic management {#sec-fleet-orchestration-model-routing}

Once resources are isolated, the orchestrator faces the challenge of directing inference queries to the correct model replicas. For a monolithic web service, simple round-robin load balancing suffices. For a 175B parameter language model served across thousands of GPUs, routing becomes a complex distributed systems problem where the "unit of serving" is no longer a single container but a **tensor parallel group**\index{Tensor!parallel group}, a collection of 8 GPUs that must function as a single logical entity. The load balancer cannot simply route a request to "GPU 12"; it must route to "TP Group 3," ensuring the request arrives at the exact moment the group is ready to process it.

Request routing strategies determine cluster-wide throughput. Naive **round-robin** distribution fails for generative workloads because request processing times vary by orders of magnitude based on output token length. A round-robin scheduler inevitably sends new requests to replicas backed up with long-generation tasks, causing tail latency to spike. **Least-outstanding-requests** (LOR) routing improves this by directing traffic to the idlest replicas. However, for large language models, **memory-aware routing**\index{Memory!aware routing} is superior. By tracking the KV cache occupancy of each replica, the router can send long-context requests to replicas with ample free memory and short requests to those near capacity, preventing fragmentation-induced out-of-memory errors and increasing effective batch sizes by 30 to 40 percent compared to blind routing.

Traffic shifting mechanisms like canary deployments and A/B testing are essential for safe model updates. When deploying Model v2, the orchestrator does not simply replace Model v1. Instead, it spins up v2 replicas alongside v1, creating a shadow fleet. The traffic manager routes 1 percent of live requests to v2 (often in "shadow mode," where the response is computed but discarded) to validate latency and error rates against v1. Only after statistical verification does the orchestrator gradually shifts weights (5 percent, 25 percent, 100 percent), draining v1 replicas only as v2 proves stable.

Multi-model serving complicates this further. A single cluster often hosts diverse models: a product recommendation model (10 ms P99 SLO, high throughput) alongside a coding assistant model (2,000 ms P99 SLO, bursty usage). Placing these on the same node invites interference; placing them on separate clusters strands capacity. The optimal strategy uses **latency-aware bin packing**\index{Latency!aware bin packing}: the orchestrator co-locates latency-critical models with batch-processing jobs (like embedding generation) that can be throttled instantly, ensuring high utilization without violating strict SLOs.
...
# │ How: active_gpus/fleet_size; annual_value = reclaimed × rate × hours/yr.
# │
# │ Imports: mlsysim.book (fmt, check), mlsysim.constants (HOURS_PER_YEAR)
# │ Exports: ds_* strings for callout
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
from mlsysim.core.constants import HOURS_PER_YEAR, MILLION
...
class DiurnalGpuShift:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(static_active == 750, f"Static: expected 750, got {static_active}")
    check(static_util_pct == 75, f"Static util: expected 75%, got {static_util_pct}")
    check(dynamic_active == 880, f"Dynamic: expected 880, got {dynamic_active}")
    check(dynamic_util_pct == 88, f"Dynamic util: expected 88%, got {dynamic_util_pct}")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ds_static_active_str = fmt(static_active, precision=0)
    ds_static_util_str = fmt(static_util_pct, precision=0)
    ds_dynamic_active_str = fmt(dynamic_active, precision=0)
    ds_dynamic_util_str = fmt(dynamic_util_pct, precision=0)
    ds_reclaimed_str = fmt(reclaimed, precision=0)
    ds_annual_str = f"{annual_value / MILLION:.1f}"
```

::: {.callout-notebook title="The diurnal GPU shift"}

Consider a fleet of 1,000 GPUs managed under two regimes: <!-- lego-ok: fleet size, serving/training splits are input assumptions -->

**Scenario A -- Static Partitioning**\index{Scenario!a -- static partitioning}: The serving pool reserves 400 GPUs to handle peak demand, but average usage is only 150 GPUs (37.5 percent utilization). The training pool reserves 600 GPUs, and backlog ensures 100 percent utilization. Result: $150 + 600 =$ `{python} DiurnalGpuShift.ds_static_active_str` active GPUs. Fleet utilization: **`{python} DiurnalGpuShift.ds_static_util_str` percent**.

**Scenario B -- Dynamic Sharing**\index{Scenario!b -- dynamic sharing}: Serving claims GPUs strictly as needed (average 150). Training reclaims unused serving GPUs (average 250), minus a 120-GPU safety buffer. The orchestrator shifts approximately `{python} DiurnalGpuShift.ds_reclaimed_str` GPUs between roles twice daily based on diurnal forecasts. Result: serving (150) + training \((600 + `{python} DiurnalGpuShift.ds_reclaimed_str`) = `{python} DiurnalGpuShift.ds_dynamic_active_str`\) active GPUs (the second training term is reclaimed serving capacity). Fleet utilization: **`{python} DiurnalGpuShift.ds_dynamic_util_str` percent**.

Dynamic sharing recovers `{python} DiurnalGpuShift.ds_reclaimed_str` GPUs of effective capacity. At \$2 per GPU-hour, this creates approximately \$`{python} DiurnalGpuShift.ds_annual_str` million per year in value without purchasing a single additional accelerator.
...
# │ How: productive_gain = cluster × (util_after − util_before);
# │      recovered_raw = productive_gain / util_before;
# │      annual_value = recovered_raw × $2/GPU-hr × 8,760 hr/yr.
# │
# │ Imports: (uses ClusterEconomicsContext.gpu_hour_cost_usd from fleet-orchestration-setup)
...
class DebugValueCalc:
    ...
    _gpu_rate          = ClusterEconomicsContext.gpu_hour_cost_usd
    ...
    check(abs(debug_productive_gain - 240) < 1e-9, f"Expected 240 productive GPUs, got {debug_productive_gain}")
    check(abs(debug_recovered - 400) < 1e-9, f"Expected 400 recovered GPUs, got {debug_recovered}")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    debug_productive_gain_str = f"{int(debug_productive_gain)}"
    debug_recovered_str = f"{int(debug_recovered)}"
    debug_annual_str    = f"{debug_annual / MILLION:.0f}"
```

::: {.callout-example title="Debugging low GPU utilization"}

**Problem**: A 1,000-GPU cluster reports 60 percent average utilization despite a full job queue with over 50 pending jobs. Engineering leadership expects greater than 80 percent utilization given the capital investment. The standard monitoring dashboards show plenty of idle GPUs, yet users complain about long queue times. Identify the binding constraint and the systematic diagnostic path.
...
**Infrastructure Layer Analysis**\index{Infrastructure Layer!analysis}: The cluster contains heterogeneous hardware acquired over three procurement cycles:

| **GPU Type**   | **Count** | **Memory**  | **Interconnect**    | **Nodes**                |
|:---------------|----------:|:------------|:--------------------|:-------------------------|
| **A100-80 GB** |       400 | 80 GB HBM2e | NVLink (`{python} FleetSetup.nvlink_a100_str` GB/s)   | 50 nodes $\times$ 8 GPUs |
| **A100-40 GB** |       400 | 40 GB HBM2e | NVLink (`{python} FleetSetup.nvlink_a100_str` GB/s)   | 50 nodes $\times$ 8 GPUs |
| **V100-32 GB** |       200 | 32 GB HBM2  | PCIe Gen3 (16 GB/s) | 50 nodes $\times$ 4 GPUs |

: **Cluster Hardware Inventory**\index{Cluster!hardware inventory}: Three procurement generations create distinct resource pools with different capabilities. A100 nodes support tensor parallelism via NVLink, while V100 nodes are limited to data parallelism. {#tbl-fleet-orchestration-debug-inventory}
...
**Diagnosis**\index{Diagnosis}: Multiple pathologies compound to create low utilization:
...
**Quantified Impact**\index{Quantified Impact}: After implementing tiered matching and backfill scheduling over a two-week validation period:
...
A simple back-of-envelope calculation quantifies what that recovered capacity is actually worth.

::: {.callout-notebook title="The value of policy debugging"}

**Problem**: A platform team manages a `{python} DebugValueCalc.debug_cluster`-GPU cluster. Initial monitoring shows 60 percent average utilization. After a week of policy debugging (capability-based scheduling, topology-aware placement, backfill tuning, and workload-hardware guidance), utilization increases to 84 percent. What is the financial value of that one week of engineering work?

**Math**:

1. **Productive Gain**\index{Productive Gain}: 1,000 GPUs $\times$ (0.84 - 0.60) = **`{python} DebugValueCalc.debug_productive_gain_str` fully utilized GPU-equivalents**.
2. **Equivalent Raw Hardware**\index{Equivalent Raw Hardware}: `{python} DebugValueCalc.debug_productive_gain_str` GPU-equivalents $\div$ 0.60 old utilization = **`{python} DebugValueCalc.debug_recovered_str` raw GPUs**.
3. **Annual Value**\index{Annual Value}: `{python} DebugValueCalc.debug_recovered_str` GPUs $\times$ \$2/hr $\times$ 8,760 hrs/year $\approx$ **\$`{python} DebugValueCalc.debug_annual_str` Million**.

**Systems insight**: Policy debugging is not a "maintenance task"; it is a **Capacity Generator**. Increasing utilization from 60 percent to 84 percent adds **`{python} DebugValueCalc.debug_productive_gain_str` fully utilized GPU-equivalents**, the same productive capacity that **`{python} DebugValueCalc.debug_recovered_str` new raw GPUs** would deliver if they ran at the old utilization (costing ~\$12 Million in CapEx). In the Machine Learning Fleet, policy and performance engineering are often the most cost-effective way to scale: making existing GPUs more productive costs less than buying more hardware.

:::

### Common utilization anti-patterns {#sec-fleet-orchestration-anti-patterns}

High-level metrics often mask deep inefficiencies. A cluster dashboard may report 85 percent allocation, but the productive utilization, cycles actually advancing model state, can be significantly lower. Debugging these gaps requires identifying specific signatures in the GPU telemetry that reveal the underlying bottleneck.
...
* **Distributed scheduling is fundamentally hard**\index{Distributed Scheduling Is Fundamentally Hard}: Cluster scheduling faces challenges (partial failures, network partitions, state inconsistency) that single-machine schedulers never encounter. The CAP theorem forces trade-offs between consistency and availability that shape every scheduling system's design.
* **Gang scheduling prevents deadlock but reduces flexibility**\index{Gang Scheduling!prevents deadlock but reduces flexibility}: Distributed training requires atomic resource allocation to prevent hold-and-wait deadlocks, but rigid gang scheduling wastes resources when combined with backfill timeouts and elastic training alternatives.
* **Topology determines performance**\index{Topology Determines Performance}: Where GPUs are placed within the cluster hierarchy matters as much as how many GPUs a job receives. NVLink vs. InfiniBand placement decisions can create 15 to 30 percent throughput differences for the same job on the same hardware.
* **ML-specific scheduling outperforms generic approaches**\index{ML-Specific Scheduling Outperforms Generic Approaches}: Exploiting workload characteristics (predictable resource needs, iterative computation, diminishing returns) enables 40 to 60 percent improvements in job completion time compared to general-purpose FIFO or fair-share policies.
...
* **Policies must evolve with infrastructure**\index{Policies Must Evolve with Infrastructure}: Scheduling algorithms are only as effective as the policies they implement. When infrastructure changes (heterogeneous hardware additions, topology upgrades, workload mix shifts), policies must be re-evaluated and updated.
