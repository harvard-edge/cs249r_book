...
**Problem**: A `{python} GangSchedulingDeadlock.gs_n_gpus_str`-GPU cluster receives two jobs from separate teams, each requiring 512 GPUs. A naive scheduler (non-gang) allocates 256 GPUs to Team A and 256 to Team B, then waits for more GPUs to become available. What is the steady-state outcome?
