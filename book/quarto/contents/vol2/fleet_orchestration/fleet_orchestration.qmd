...
class FleetSetup:
    ...
    ib_ndr_gbs_val = (INFINIBAND_NDR_BW / 8).m_as(GB/second)
...
class GangSchedulingDeadlock:
    ...
    n_total_gpus = 1024
    n_jobs = 2
    gpus_per_job = 512 # Each job needs half the cluster
    # ┌── 2. EXECUTE ───────────────────────────────────────
...
**Problem**: A `{python} GangSchedulingDeadlock.gs_n_gpus_str`-GPU cluster receives two jobs from separate teams, each requiring 512 GPUs. A naive scheduler (nongang) allocates 256 GPUs to Team A and 256 to Team B, then waits for more GPUs to become available. What is the steady-state outcome?

**Math**:

1. **Team A Status**\index{Team A Status}: Holds 256, Needs 512. Progress = **0 percent**.
2. **Team B Status**\index{Team B Status}: Holds 256, Needs 512. Progress = **0 percent**.
3. **Cluster Status**\index{Cluster!status}: 512 GPUs allocated, 0 samples processed.
...
- **At 60 percent utilization**\index{At 60 Percent Utilization}: 4,000 GPUs productive, 6,000 idle = \$288,000/day wasted
- **At 80 percent utilization**\index{At 80 Percent Utilization}: 8,000 GPUs productive, 2,000 idle = \$96,000/day wasted
- **Improvement value**\index{Improvement Value}: Moving from 60 percent to 80 percent saves \$192,000/day, or \$70M/year
...
The physical location of GPUs on the network switch fabric dramatically impacts collective performance. Consider an AllReduce operation across 8 GPUs: <!-- lego-ok: latencies are input assumptions -->

*   **Random placement**: Spread across 8 different racks $\rightarrow$ Traffic traverses core switches $\rightarrow$ **120 ms** latency.
...
class AutoscalingLag:
    ...
    slo_violations = int(excess_at_peak * avg_excess_sec)
    ...
    check(slo_violations == 3600, f"Violations: expected 3600, got {slo_violations}")
...
**Reactive scaling**\index{Reactive Scaling}: HPA detects overload at $t =$ `{python} AutoscalingLag.as_detect_str` s (when traffic hits `{python} AutoscalingLag.as_capacity_str` QPS). It requests 10 new replicas. These replicas become ready at $t =$ `{python} AutoscalingLag.as_ready_str` s (`{python} AutoscalingLag.as_detect_str` + 180). From $t =$ `{python} AutoscalingLag.as_detect_str` s to $t =$ `{python} AutoscalingLag.as_ready_str` s (3 minutes), demand exceeds capacity. At peak (80 QPS), `{python} AutoscalingLag.as_excess_str` QPS are queued or dropped. Over this 3-minute window, approximately `{python} AutoscalingLag.as_violations_str` requests (`{python} AutoscalingLag.as_excess_str` QPS $\times$ 120 s average excess) violate the SLO.
...
class DebugValueCalc:
    ...
    check(debug_recovered == 400, f"Expected 400 recovered GPUs, got {debug_recovered}")
...
::: {.callout-notebook title="The value of a profiler"}

**Problem**: A platform team manages a `{python} DebugValueCalc.debug_cluster`-GPU cluster. Initial monitoring shows 60 percent average utilization. After a week of systematic debugging (identifying stragglers, optimizing AllReduce buckets, and tuning data loaders), utilization increases to 84 percent. What is the financial value of that one week of engineering work?

**Math**:

1. **Capacity Recovery**\index{Capacity Recovery}: (0.84 - 0.60)/0.60 = **40 percent increase in productive throughput**.
2. **Recovered Hardware**\index{Recovered Hardware}: 40 percent of 1,000 GPUs = **`{python} DebugValueCalc.debug_recovered_str` GPUs**.
...
**Systems insight**: Debugging is not a "maintenance task"; it is a **Capacity Generator**. Increasing utilization from 60 percent to 84 percent has the same business impact as buying **400 new GPUs** (costing ~\$12 Million in CapEx). In the Machine Learning Fleet, performance engineering is often the most cost-effective way to scale: making existing GPUs 40 percent more productive costs less than buying 40 percent more hardware.
