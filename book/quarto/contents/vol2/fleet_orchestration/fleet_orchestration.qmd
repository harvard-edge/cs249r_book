...
class ClusterEconomicsContext:
    ...
    idle_pct_str = fmt(idle_fraction * 100, precision=0, commas=False, suffix=" percent")
    util_low_pct_str = fmt(util_low * 100, precision=0, commas=False, suffix=" percent")
    util_high_pct_str = fmt(util_high * 100, precision=0, commas=False, suffix=" percent")
...
class QueuingTheory:
    ...
    rho_pct_str = fmt(rho * 100, precision=0, commas=False, suffix=" percent")
```
...
class FleetFragmentation:
    ...
    effective_capacity_pct_str = fmt(100 - frag_pct, precision=0, commas=False, suffix=" percent")
...
class SpotTrainingEconomics:
    ...
    longer_pct_str = fmt(longer_pct, precision=1, commas=False, suffix=" percent")
...
- Total time: `{python} SpotTrainingEconomics.spot_run_hours_str` + `{python} SpotTrainingEconomics.checkpoint_extra_hours_str` hours + `{python} SpotTrainingEconomics.restart_total_hours_str` = `{python} SpotTrainingEconomics.spot_total_hours_str`
...
class AutoscalingLag:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    as_capacity_str = fmt(capacity, precision=0, suffix=" QPS")
    as_detect_str = fmt(detect_sec, precision=0)
    as_ready_str = fmt(ready_sec, precision=0)
    as_excess_str = fmt(excess_at_peak, precision=0, suffix=" QPS")
    as_violations_str = fmt(slo_violations, precision=0)
    as_pred_trigger_str = fmt(pred_trigger_sec, precision=0)
    as_pred_capacity_str = fmt(pred_capacity, precision=0, suffix=" QPS")
    as_load_time_str = fmt(load_time_sec, precision=0, commas=False, suffix=" seconds")
    as_traffic_end_str = fmt(traffic_end, precision=0, commas=False, suffix=" QPS")
    ...
    as_traffic_start_str = fmt(traffic_start, precision=0, commas=False, suffix=" QPS")
    as_ramp_sec_str = fmt(ramp_sec, precision=0, commas=False, suffix=" seconds")
    as_slo_ms_str = fmt(slo_ms, precision=0, commas=False, suffix=" ms")
    as_load_time_min_str = fmt(load_time_sec / 60, precision=0, commas=False, suffix=" minutes")
```
...
class DebugValueCalc:
    ...
    debug_util_before_pct_str = fmt(debug_util_before * 100, precision=0, commas=False, suffix=" percent")
    debug_target_util_pct_str = fmt(80, precision=0, commas=False, suffix=" percent")
```
