...
class ClusterEconomicsMigRecap:
    ...
    h100_mem_str = fmt(Hardware.Cloud.H100.memory.capacity.to(GiB).magnitude, precision=0, commas=False)
```
...
class FleetTopologyInterconnect:
    ...
    ib_ndr = Systems.Fabrics.InfiniBand_NDR
    nvlink_h100_val = h_h100.nvlink.bandwidth.to(GB/second).magnitude
    nvlink_a100_val = h_a100.nvlink.bandwidth.to(GB/second).magnitude
    ib_ndr_gbps_val = ib_ndr.bandwidth.to(Gbps).magnitude
    ib_ndr_gbs_val = ib_ndr.bandwidth.to(GB/second).magnitude
    activation = 1 * GB
    activation_gb = activation.to(GB).magnitude
    # ┌── 2. EXECUTE ───────────────────────────────────────
...
class GpuSharingRoi:
    ...
    small_footprint_gb = small_footprint.to(GB).magnitude
    n_large = 10
    large_footprint = Models.Language.GPT3.parameters * BYTES_FP16  # 175B FP16
    large_footprint_gb = large_footprint.to(GB).magnitude
    a100_mem = Hardware.Cloud.A100.memory.capacity
    a100_mem_gb = a100_mem.to(GiB).magnitude
    gpus_per_large = 8
    ...
    small_model_params_b = Models.Language.Llama2_7B.parameters.to(Bparam).magnitude
    ...
    gs_large_model_b_str = fmt_count(
        Models.Language.GPT3.parameters.to(param).magnitude,
        scale='B',
        precision=0,
        commas=False,
    )
