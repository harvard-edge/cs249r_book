...
[^fn-placement-groups]: **Placement Group**: A cloud-native abstraction (AWS, GCP) that requests instances be placed within a single high-bandwidth, low-latency network domain. For the ML Fleet, placement groups are the **Topology Contract**: they guarantee the bisection bandwidth ($BW_{bisect}$) required for AllReduce, preventing the "topology lottery" where nodes are scattered across different datacenter racks. \index{Placement Group!topology}
