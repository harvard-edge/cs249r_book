...
- **Reserved capacity**: One- to three-year commitments provide guaranteed availability at 30--60 percent discounts compared to on-demand pricing. Reserved capacity forms the baseline for predictable, continuous workloads such as 24/7 inference serving, regularly scheduled training, CI and regression testing, and any workload where interruption would violate service level agreements.
- **On-demand capacity**: Immediate availability at full price serves as the safety valve for workloads that cannot tolerate queuing or interruption. Urgent retraining, interactive debugging, and short-lived jobs that do not justify spot management belong here despite the cost premium.
- **Spot capacity**: Discounted interruptible capacity serves workloads that can tolerate restart, including hyperparameter sweeps, ablation studies, pretraining runs with robust checkpointing, and exploratory research. The discount can be substantial, but availability varies with cloud demand patterns.

The tiers give the scheduler a price ladder rather than a single pool of undifferentiated GPUs.
...
GPU virtualization options represent a spectrum of isolation strength vs. resource efficiency:

- **Time-slicing**: Software-based GPU sharing provides low isolation but high flexibility, making it suitable for trusted workloads from the same team that share a GPU for cost efficiency.
- **MIG**: Hardware-partitioned GPU sharing provides strong isolation with fixed partitions, making it suitable for multi-tenant inference where different customers' workloads share the same physical GPU.
- **Full device passthrough**: Exclusive access to one or more complete GPUs provides complete isolation at the cost of the lowest packing efficiency, making it suitable for training workloads that saturate GPU resources and cannot tolerate interference.

The choice depends on the trust boundary between colocated workloads and the performance sensitivity of each workload type.
