...
class ClusterEconomicsContext:
    ...
    a100_mem_str = fmt_qty(a100_mem, GiB, precision=0, commas=False, unit_label="GB")
    h100_mem_str = fmt_qty(h100_mem, GiB, precision=0, commas=False, unit_label="GB")
```
...
class FleetFragmentation:
    ...
    nvlink_h100_str = fmt_qty(h_h100.nvlink.bandwidth, GB/second, precision=0)
    ib_ndr_gbs_str = fmt_qty(ib_ndr.bandwidth, GB/second, precision=0)
```
...
class ClusterEconomicsMigRecap:
    from mlsysim import Hardware
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_str = fmt_qty(Hardware.Cloud.A100.memory.capacity, GiB, precision=0, commas=False, unit_label="GB")
...
from mlsysim.fmt import fmt, check, fmt_time, fmt_qty
class FleetTopologyInterconnect:
    ...
    activation = 1 * GB
    activation_gb = activation.m_as(GB)
    # ┌── 2. EXECUTE ───────────────────────────────────────
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_str = fmt_qty(h_h100.nvlink.bandwidth, GB/second, precision=0)
    nvlink_a100_str = fmt_qty(h_a100.nvlink.bandwidth, GB/second, precision=0)
    ib_ndr_str = fmt_qty(ib_ndr.bandwidth, Gbps, precision=0, unit_label="Gb/s")
    ib_ndr_gbs_str = fmt_qty(ib_ndr.bandwidth, GB/second, precision=0)
    activation_gb_str = fmt_qty(activation, GB, precision=0, commas=False)
    nvlink_xfer_ms_str = fmt_time(nvlink_xfer_ms, 'millisecond', precision=1, commas=False)
    ib_xfer_ms_str = fmt_time(ib_xfer_ms, 'millisecond', precision=0, commas=False)
...
class ElasticScaling:
    ...
    es_rescale_str = fmt_time(rescale_h, 'hour', precision=2, commas=False, style='word')
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, MarkdownStr, fmt_count, fmt_time
...
class AutoscalingLag:
    ...
    as_load_time_str = fmt_time(load_time_sec, 'second', precision=0, commas=False, style='word')
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_qty, fmt_qty_int, fmt, check, fmt_count

class GpuSharingRoi:
    # ┌── 1. LOAD ──────────────────────────────────────────
    n_small = 100
    small_footprint = 26 * GB
    small_footprint_gb = small_footprint.m_as(GB)
    n_large = 10
    large_footprint = Models.Language.GPT3.parameters * BYTES_FP16  # 175B FP16
    large_footprint_gb = large_footprint.m_as(GB)
    a100_mem = Hardware.Cloud.A100.memory.capacity
    a100_mem_gb = a100_mem.m_as(GiB)
    gpus_per_large = 8
    ...
    gs_excl_total_mem_str = fmt_qty_int(excl_total_mem, GiB, unit_label="GB")
    ...
    gs_shared_mem_str = fmt_qty(shared_mem_per_gpu_q, GB, precision=0)
    ...
    gs_a100_mem_str = fmt_qty(a100_mem, GiB, precision=0, commas=False, unit_label="GB")
    gs_small_footprint_str = fmt_qty(small_footprint, GB, precision=0, commas=False)
    ...
    gs_large_footprint_str = fmt_qty(large_footprint, GB, precision=0, commas=False)
...
class FleetDebugNvlinkRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_a100_str = fmt_qty(Hardware.Cloud.A100.nvlink.bandwidth, GB/second, precision=0)
```
