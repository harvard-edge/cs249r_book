...
- **Throughput**\index{Throughput}: The total useful work completed per unit time naturally favors large, long-running jobs that saturate hardware and minimize context-switching and data-movement overhead.
...
- **Latency**\index{Latency}: Rapid completion of short jobs maintains researcher velocity, but aggressive preemption can starve large training jobs.
...
**Scenario**: A multi-node foundation model training job exhibits a 30% throughput drop while static topology maps show full bandwidth and GPU hardware health checks report green.
...
**Systems lesson**: Static quota reservations cause severe capacity hoarding. Fleet orchestrators must implement dynamic "use it or lose it" reclamation policies, automatically demoting idle reserved nodes (<20% utilization for 72h) to preemptible spot pools.
