...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=latex]
  ...
  \tikzset{
    ...
    DeadlockLabel/.style={
      fill=RedL, draw=RedLine, line width=0.75pt,
      inner sep=4pt, font=\bfseries, rounded corners=2pt
    },
    SuccessLabel/.style={
      fill=GreenL, draw=GreenLine, line width=0.75pt,
      inner sep=4pt, font=\bfseries, rounded corners=2pt
    }
  }
  ...
  \begin{scope}[local bounding box=scopeA]
    % Axes
    \node (originA) at (0,0) {};
    \draw[AxisLine] (originA.center) -- ++(8, 0) node[right, text=black] {Time};
    \draw[AxisLine] (originA.center) -- ++(0, 2.5) node[above, text=black] {Cluster Nodes};

    \node[anchor=south west, text=RedLine, font=\bfseries, above=2.5cm of originA.west, anchor=west] (titleA) {A. Independent Scheduling (Deadlock)};

    % Job B (bottom)
    \node[JobBox, fill=OrangeL, minimum width=4cm, anchor=south west, above right=0.2cm and 1cm of originA.center] (jobB) {Job B (Hold)};
    \draw[WaitArrow, OrangeLine] (jobB.east) -- ++(2,0) node[right, text=black] {Wait};

    % Job A (top)
    \node[JobBox, fill=BlueL, minimum width=4cm, anchor=south west, above right=1.25cm and 0cm of originA.center] (jobA) {Job A (Hold)};
    \draw[WaitArrow, BlueLine] (jobA.east) -- ++(2,0) node[right, text=black] {Wait};

    % Deadlock Label
    \node[DeadlockLabel, right=0.5cm of jobA.south east, anchor=west] {DEADLOCK};
  \end{scope}

  % ---------------------------------------------------------
  % B. Gang Scheduling
  % ---------------------------------------------------------
  \begin{scope}[shift={(0,-4.5)}, local bounding box=scopeB]
    % Axes
    \node (originB) at (0,0) {};
    \draw[AxisLine] (originB.center) -- ++(8, 0) node[right, text=black] {Time};
    \draw[AxisLine] (originB.center) -- ++(0, 2.5) node[above, text=black] {Cluster Nodes};

    \node[anchor=south west, text=GreenLine, font=\bfseries, above=2.5cm of originB.west, anchor=west] (titleB) {B. Gang Scheduling (Atomic)};

    % Job A
    \node[JobBox, fill=BlueL, minimum width=3cm, minimum height=2cm, anchor=south west, above right=0.2cm and 0cm of originB.center] (jobA_gang) {Job A (All)};

    % Job B
    \node[JobBox, fill=OrangeL, minimum width=3cm, minimum height=2cm, right=1.5cm of jobA_gang.south east, anchor=south west] (jobB_gang) {Job B (All)};

    % Queue Arrow
    \draw[WaitArrow, black!50, <-, solid] (jobA_gang.east) -- (jobB_gang.west) node[midway, above, text=black] {Queue};

    % Success Label
    \node[SuccessLabel, below=0.5cm of jobB_gang.south, anchor=north] {SUCCESS};
  \end{scope}
\end{tikzpicture}
```
:::
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85, transform shape, >=latex]
  \usetikzlibrary{positioning, fit, backgrounds}
  \tikzset{
    gpu/.style={draw=black!60, fill=BlueL, line width=0.75pt, rounded corners=2pt,
                minimum width=0.6cm, minimum height=0.5cm, font=\tiny\bfseries, align=center},
    switch/.style={draw=black!60, fill=BrownL, line width=0.75pt, rounded corners=2pt,
                   minimum width=1.6cm, minimum height=0.4cm, font=\tiny\bfseries, align=center},
    nodebox/.style={draw=black!30, fill=black!3, rounded corners=6pt, line width=0.75pt,
                    inner sep=10pt},
    nvpath/.style={draw=GreenLine, line width=1.2pt},
    ibpath/.style={draw=RedLine, line width=1.2pt, dashed},
    Title/.style={font=\small\bfseries}
  }

  % === LEFT: Optimal Placement ===
  \begin{scope}[local bounding box=leftScope]
    % NVSwitch
    \node[switch, fill=GreenL] (nvsw) {NVSwitch};

    % GPUs (2 rows of 4)
    \node[gpu, fill=GreenL, below left=0.5cm and 0.5cm of nvsw] (og1) {G1};
    \node[gpu, fill=GreenL, left=0.2cm of og1] (og0) {G0};
    \node[gpu, fill=GreenL, right=0.2cm of og1] (og2) {G2};
    \node[gpu, fill=GreenL, right=0.2cm of og2] (og3) {G3};

    \node[gpu, fill=GreenL, below=0.2cm of og0] (og4) {G4};
    \node[gpu, fill=GreenL, below=0.2cm of og1] (og5) {G5};
    \node[gpu, fill=GreenL, below=0.2cm of og2] (og6) {G6};
    \node[gpu, fill=GreenL, below=0.2cm of og3] (og7) {G7};
    ...
    \begin{scope}[on background layer]
      \node[nodebox, fit=(nvsw) (og0) (og7)] (opt_node) {};
    \end{scope}
    \node[font=\tiny, above right=2pt and 2pt of opt_node.south west] {Node 0};

    \node[Title, text=GreenLine, above=0.5cm of opt_node.north] (titleLeft) {Optimal (Single Node)};

    % Bandwidth label
    \node[font=\scriptsize\bfseries, text=GreenLine, below=0.2cm of opt_node.south] {900 GB/s (NVLink)};
  \end{scope}

  % === RIGHT: Scattered Placement ===
  \begin{scope}[shift={(6,0)}, local bounding box=rightScope]
    % Node A
    ...
    \node[gpu, fill=GreenL, right=0.2cm of sg1] (sg2) {G2};
    \node[gpu, fill=GreenL, right=0.2cm of sg2] (sg3) {G3};
    ...
    \begin{scope}[on background layer]
      \node[nodebox, fit=(nvsw_a) (sg0) (sg3)] (sn_a) {};
    \end{scope}
    \node[font=\tiny, above right=2pt and 2pt of sn_a.south west] {Node 0};
    ...
    \node[gpu, fill=RedL, right=0.2cm of sg5] (sg6) {G6};
    \node[gpu, fill=RedL, right=0.2cm of sg6] (sg7) {G7};

    \foreach \i in {4,...,7} {
      \draw[nvpath] (sg\i) -- (nvsw_b);
    }

    \begin{scope}[on background layer]
      \node[nodebox, fit=(nvsw_b) (sg4) (sg7)] (sn_b) {};
    \end{scope}
    \node[font=\tiny, above right=2pt and 2pt of sn_b.south west] {Node 1};

    \node[Title, text=RedLine, above=0.5cm of sn_a.north] (titleRight) {Scattered (Two Nodes)};

    % Inter-node connection
    \draw[ibpath] (nvsw_a) -- (nvsw_b) node[midway, right, font=\scriptsize\bfseries, text=RedLine] {50 GB/s (IB)};

    % Bandwidth label
    \node[font=\scriptsize\bfseries, text=RedLine, below=0.2cm of sn_b.south] {50 GB/s cross-node (18$\times$ slower)};
  \end{scope}
\end{tikzpicture}
```
:::
...
This 40% improvement in effective cluster capacity equals approximately `{python} DebugValueCalc.debug_recovered_str` additional GPUs worth of productive work.

::: {.callout-notebook title="The Value of a Profiler"}

**Problem**: You manage a `{python} DebugValueCalc.debug_cluster`-GPU cluster. Your initial monitoring shows 60% average utilization. After a week of systematic debugging (identifying stragglers, optimizing AllReduce buckets, and tuning data loaders), your utilization increases to 84%. What is the financial value of that one week of engineering work?
