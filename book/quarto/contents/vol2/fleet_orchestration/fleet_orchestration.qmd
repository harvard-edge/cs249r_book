...
1. **Team A Status**: Holds 256, Needs 512. Progress = **0 percent**.
2. **Team B Status**: Holds 256, Needs 512. Progress = **0 percent**.
3. **Cluster Status**: 512 GPUs allocated, 0 samples processed.
...
- **Operating cost**: $`{python} ClusterEconomicsContext.daily_cost_str`/day ($`{python} ClusterEconomicsContext.annual_cost_str`M/year)
- **At 60 percent utilization**: 4,000 GPUs productive, 6,000 idle = \$288,000/day wasted
- **At 80 percent utilization**: 8,000 GPUs productive, 2,000 idle = \$96,000/day wasted
- **Improvement value**: Moving from 60 percent to 80 percent saves \$192,000/day, or \$70M/year
...
**Assumptions**: Throughput at 128 GPUs is 1.0 epoch/hour (baseline). Throughput at 256 GPUs is 1.8 epochs/hour (90 percent scaling efficiency). Throughput at 512 GPUs is 3.2 epochs/hour (80 percent scaling efficiency). Re-scaling cost is 10 minutes (`{python} ElasticScaling.es_rescale_str` hours) to checkpoint, re-shard, and restart.
...
**Savings**: \$215,192 (63 percent), at the cost of approximately 7 percent longer wall-clock time.
...
**Reactive scaling**: HPA detects overload at $t =$ `{python} AutoscalingLag.as_detect_str` s (when traffic hits `{python} AutoscalingLag.as_capacity_str` QPS). It requests 10 new replicas. These replicas become ready at $t =$ `{python} AutoscalingLag.as_ready_str` s (`{python} AutoscalingLag.as_detect_str` + 180). From $t =$ `{python} AutoscalingLag.as_detect_str` s to $t =$ `{python} AutoscalingLag.as_ready_str` s (3 minutes), demand exceeds capacity. At peak (80 QPS), `{python} AutoscalingLag.as_excess_str` QPS are queued or dropped. Over this 3-minute window, approximately `{python} AutoscalingLag.as_violations_str` requests (`{python} AutoscalingLag.as_excess_str` QPS$\times$ 120 s average excess) violate the SLO.
...
1. **Over-specification**: Model memory analysis reveals that 12 of the 15 large jobs actually require only 45 GB peak memory per GPU, well within A100-40 GB capacity. Users copied job templates specifying A100-80 GB without recalculating requirements.
2. **Pool fragmentation**: The strict homogeneity requirement means a 64-GPU job requesting "A100-80 GB" cannot use any A100-40 GB GPUs, even when 300 A100-40 GB GPUs sit idle.
...
1. **Capacity Recovery**: (0.84 - 0.60) / 0.60 = **40 percent increase in productive throughput**.
2. **Recovered Hardware**: 40 percent of 1,000 GPUs = **`{python} DebugValueCalc.debug_recovered_str` GPUs**.
3. **Annual Value**: `{python} DebugValueCalc.debug_recovered_str` GPUs $\times$ \$2/hr $\times$ 8,760 hrs/year $\approx$ **\$`{python} DebugValueCalc.debug_annual_str` Million**.
