...
- Calculate **fairness metrics**\index{Fairness Metrics} (**demographic parity**, **equalized odds**\index{Equalized Odds}, **equality of opportunity**\index{Equality of Opportunity!definition}) from confusion matrices and explain why **impossibility theorems** prove these metrics are mathematically incompatible
...
- Design monitoring infrastructure for detecting **distribution drift**\index{Drift!distribution}, **fairness degradation**\index{Fairness!degradation}, and performance disparities across demographic groups in production systems
...
1. **Base Accuracy**\index{Accuracy!base}: `{python} BiasTax.ab_base_acc_str`.
