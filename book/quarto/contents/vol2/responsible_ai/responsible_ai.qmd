...
2.  **Observation**: Unlike AI ethics (which defines normative principles about what systems should do), responsible AI engineering defines technical mechanisms that enforce those principles—bias detection algorithms, differential privacy implementations, audit trails, and architectural guardrails that make compliance measurable and verifiable rather than aspirational.
3.  **Pitfall**: A frequent misconception is that responsible AI is a final compliance review applied to a finished model. Responsible constraints that are not designed in from the data collection stage typically require fundamental retraining to fix: a model trained on biased labels cannot be fairly calibrated by post-hoc threshold adjustment alone, as the learned representations themselves encode the bias.
...
2.  **Observation**: Unlike Equalized Odds (which focuses on error rates like False Positives), Demographic Parity focuses only on the Final Prediction\index{Demographic Parity!final prediction}, ignoring the relationship between the prediction and the ground truth.
3.  **Pitfall**: A frequent misconception is that Demographic Parity ensures "fairness." In reality, it can force the model to sacrifice Calibration\index{Calibration}: to meet the parity constraint, the model may have to intentionally misclassify qualified individuals in one group or unqualified individuals in another.
...
- [ ] **Transparency vs. fairness**: Given a model that is fully transparent about its decision logic yet still disadvantages one demographic group, can you explain why transparency and fairness are separate principles and which one this system violates?
- [ ] **Lifecycle coverage**: Can you put the responsible AI lifecycle to work: for a fairness requirement, which phases (data collection, training, evaluation, deployment, monitoring) must act, and why post-hoc threshold adjustment alone is insufficient?

**Judging the trade-off**

- [ ] **Safeguard budget**: When an organization with a limited compute budget must choose between differential privacy, real-time fairness monitoring, and on-demand explanation, can you reason about which safeguard its deployment can actually afford, using the overhead figures the section established?
- [ ] **Distributed accountability**: Can you argue why accountability for a biased hiring model is distributed across data providers, interface designers, and the deploying institution rather than resting solely with the model developer?
...
- [ ] **Serving-path detection**: When a deployed credit model begins rejecting one ZIP code at twice the normal rate, can you explain why detection infrastructure must run as a serving-path capability rather than as a periodic offline audit?
- [ ] **Metric selection**: Given approval rates of 94 percent for one group and 68 percent for another, can you decide which fairness metric a monitoring dashboard should track when the groups have different base rates, and justify why demographic parity may be the wrong choice?

**From signal to action**

- [ ] **Detection vs. remediation**: Can you explain why the section insists that a fairness alert with no remediation owner is evidence without action, and name what the system must add to close that loop?
- [ ] **Constrained deployments**: For a federated deployment where demographic labels cannot be centralized, can you reason about why the slice-based detection shown here may be infeasible and what that implies for the choice of safeguard?
...
- [ ] **Automation bias**: Given a sepsis model that is highly accurate yet does not improve mortality because clinicians ignore its alerts, can you identify which sociotechnical failure mode this is and what interface change the section argues would counter it?
- [ ] **Paradox of reliability**: Can you explain why raising model accuracy from 90 to 99 percent can lower system-level safety, and why the response is to add friction rather than remove the human?

**Reasoning about loops and stakeholders**

- [ ] **Feedback loops**: For a predictive-policing or recommender system, can you trace how a feedback loop makes a model's original prediction self-fulfilling, and explain why a static test set fails to detect it?
- [ ] **Normative pluralism**: When medical efficacy, patient autonomy, privacy, and legal compliance conflict in the adolescent mental-health chatbot, can you justify why no algorithm resolves the conflict and what the engineering team must do before optimization begins?
...
| **False Positive Rate**       | FP divided by negatives           | 20 percent | 20 percent |   `{python} FairnessGapCalc.fpr_gap_pp_str`    |
