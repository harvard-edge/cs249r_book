...
class UnlearningCostAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt, fmt_percent_range, fmt_time, fmt_usd
...
**Baseline (full retraining)**: For a 175B-parameter model at GPT-3 scale, retraining requires `{python} UnlearningCostAnalysis.full_retrain_gpus_str` A100 GPUs for approximately `{python} UnlearningCostAnalysis.full_time_days_str` days at a cost of roughly `{python} UnlearningCostAnalysis.full_cost_m_str`.
...
**Representation-tax scenario**: For `{python} RepresentationTax.subgroup_count_str` underrepresented subgroups:
...
# │          shap_total_high_ms_str, shap_capacity_range_factor_str
# └─────────────────────────────────────────────────────────────────────────────
...
class ExplainabilityRetrofitCost:
    ...
    shap_capacity_range_factor_str = fmt_multiple_range(capacity_low, capacity_high, precision=(1, 0), commas=False)
```
