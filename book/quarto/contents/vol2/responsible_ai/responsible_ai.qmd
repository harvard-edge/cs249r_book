...
# │
# │ Imports: from mlsysim.fmt import check, fmt, fmt_math, fmt_percent
# │ Exports: FairnessWorkedExample.parity_gap_math, .tpr_gap_math,
# │          .tpr_a_pct_str, .tpr_b_pct_str, .fpr_a_pct_str, .fpr_b_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: from mlsysim.fmt import check, fmt, fmt_math
# │ Exports: FairnessMetricsTable.di_ratio_math, .spd_math, .eod_math, .aod_math,
# │          .di_ratio_str, .spd_str, .eod_str, .aod_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: from mlsysim.fmt import check, fmt, fmt_math, fmt_percent
# │ Exports: PpvGap.gap_math, .ppv_b_pct_str, .ppv_a_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_math, fmt_percent
...
# │
# │ Imports: from mlsysim.fmt import check, fmt_pp, fmt_percent
# │ Exports: CostOfFairness.tax_pp_str, .tax_relative_pct_str,
# │          .approval_a_pct_str, .approval_b_pct_str,
# │          .acc_unconstrained_pct_str, .acc_constrained_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: math; from mlsysim.fmt import check, fmt, fmt_math
# │ Exports: FairnessZTest.z_math
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import check, fmt, fmt_math
...
# │
# │ Imports: from mlsysim.fmt import check, fmt, fmt_math
# │ Exports: ConfusionMatrixAudit.ppr_a_math, .ppr_b_math, .parity_gap_str,
# │          .four_fifths_math, .tpr_a_math, .tpr_b_math, .tpr_gap_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_math
...
# │ How: signed percentage-point ranges per technique/column; Exact SHAP as a x-multiplier.
# │ Exports: RaiOverheadTable.<technique>_<column> range strings.
# └─────────────────────────────────────────────────────────────────────────────
...
class RaiOverheadTable:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────
    # Each cell is a signed percentage-point range (or a x-multiplier for Exact
    # SHAP); MarkdownStr marks the composed range string as render-ready.
    dp_accuracy_str = MarkdownStr(f"{int(_R.DpAccuracyLowPct)}% to {int(_R.DpAccuracyHighPct)}%")
    dp_training_str = MarkdownStr(f"+{int(_R.DpTrainingLowPct)}% to +{int(_R.DpTrainingHighPct)}%")
    dp_memory_str = MarkdownStr(f"+{int(_R.DpMemoryLowPct)}% to +{int(_R.DpMemoryHighPct)}%")
    fair_accuracy_str = MarkdownStr(f"{int(_R.FairnessAccuracyLowPct)}% to {int(_R.FairnessAccuracyHighPct)}%")
    fair_training_str = MarkdownStr(f"+{int(_R.FairnessTrainingLowPct)}% to +{int(_R.FairnessTrainingHighPct)}%")
    fair_memory_str = MarkdownStr(f"+{int(_R.FairnessMemoryLowPct)}% to +{int(_R.FairnessMemoryHighPct)}%")
    approx_inference_str = MarkdownStr(f"+{int(_R.ApproxShapInferenceLowPct)}% to +{int(_R.ApproxShapInferenceHighPct)}%")
    approx_memory_str = MarkdownStr(f"+{int(_R.ApproxShapMemoryLowPct)}% to +{int(_R.ApproxShapMemoryHighPct)}%")
    exact_inference_str = MarkdownStr(
        f"{fmt_multiple(int(_R.ExactShapInferenceLowX), precision=0)} to {fmt_multiple(int(_R.ExactShapInferenceHighX), precision=0, commas=True)}"
    )
    exact_memory_str = MarkdownStr(f"+{int(_R.ExactShapMemoryLowPct)}% to +{int(_R.ExactShapMemoryHighPct)}%")
    adv_accuracy_str = MarkdownStr(f"{int(_R.AdversarialAccuracyLowPct)}% to {int(_R.AdversarialAccuracyHighPct)}%")
    adv_training_str = MarkdownStr(f"+{int(_R.AdversarialTrainingLowPct)}% to +{int(_R.AdversarialTrainingHighPct)}%")
    adv_memory_str = MarkdownStr(f"+{int(_R.AdversarialMemoryLowPct)}% to +{int(_R.AdversarialMemoryHighPct)}%")
    fed_accuracy_str = MarkdownStr(f"{int(_R.FederatedAccuracyLowPct)}% to {int(_R.FederatedAccuracyHighPct)}%")
    fed_training_str = MarkdownStr(f"+{int(_R.FederatedTrainingLowPct)}% to +{int(_R.FederatedTrainingHighPct)}%")
    fed_memory_str = MarkdownStr(f"+{int(_R.FederatedMemoryLowPct)}% to +{int(_R.FederatedMemoryHighPct)}%")
```
...
| **Exact SHAP**                     |                      N/A                      |                      N/A                      | `{python} RaiOverheadTable.exact_inference_str`  | `{python} RaiOverheadTable.exact_memory_str`  |
...
# │
# │ Imports: from mlsysim.fmt import check, fmt_count, fmt_range
# │ Exports: ExplanationStorage.predictions_per_day_str, .monthly_predictions_str,
# │          .shap_bytes_range_str, .image_map_range_str, .image_log_tb_range_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: from mlsysim import GB, byte;
# │          from mlsysim.fmt import check, fmt, fmt_count, fmt_percent, fmt_qty
# │ Exports: MonitoringScale.qps_str, .inferences_str, .groups_str, .metrics_str,
# │          .events_str, .storage_gb_str, .alerts_str, .bytes_per_record_str,
# │          .sampling_pct_str, .false_alarm_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: from mlsysim.fmt import check, fmt, fmt_math, fmt_percent
# │ Exports: AutomationBias.fail_prob_math, .sensitivity_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_math, fmt_percent
