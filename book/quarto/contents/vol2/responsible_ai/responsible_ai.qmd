...
[^fn-machine-unlearning-cost]: **Machine Unlearning**: First formalized by Cao and Yang in 2015, this is the ability to remove specific training data influence from a model without full retraining. The naive approach (retrain from scratch) costs the same as the original training run. SISA (Sharded, Isolated, Sliced, and Aggregated) training partitions training into independent shards, reducing unlearning to retraining only the affected shard -- cutting unlearning time from hours to minutes at the cost of 2--5 percent accuracy degradation. For GDPR-compliant ML systems, unlearning latency becomes an SLA: deletion requests must be honored within defined timeframes. \index{Machine Unlearning!privacy cost}
