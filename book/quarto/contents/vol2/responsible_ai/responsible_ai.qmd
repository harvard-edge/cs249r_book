...
**Mechanism**: To keep serving latency beneath strict real-time SLAs ($L_{\text{lat}} < 100\text{ ms}$), the underwriting inference pipeline omitted inline post-hoc feature attribution. When customers called customer support to contest line-limit decisions, support agents had no feature-level attributions or counterfactual recourse pathways to explain model outputs.
