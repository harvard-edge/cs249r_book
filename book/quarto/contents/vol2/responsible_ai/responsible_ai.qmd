...
acc_unconstrained = FairnessTaxAnalysis.acc_unconstrained_str
acc_parity = FairnessTaxAnalysis.acc_parity_str
fairness_tax_pct = FairnessTaxAnalysis.fairness_tax_str
```

::: {.callout-notebook title="The Fairness Tax"}

**Problem**: You have a credit model with **`{python} acc_unconstrained`% accuracy**. Group A (majority) has a 20% default rate. Group B (minority) has a 40% default rate due to systemic factors. If you enforce **Demographic Parity** (equal approval rates), what happens to accuracy?
...
1.  **Unconstrained**: Model approves everyone with predicted default prob < 30%.
    *   Group A approval: 80%.
    *   Group B approval: 60%.
    *   Total Accuracy: **`{python} acc_unconstrained`%**.
2.  **Constrained (Parity)**: Must approve Group B at 80% rate.
    ...
    *   New Total Accuracy: **`{python} acc_parity`%**.

**The Systems Conclusion**: Fairness is not free. Enforcing parity cost **`{python} fairness_tax_pct`% accuracy** (a huge drop in credit scoring). This is the **Fairness Tax**, the explicit cost of correcting for historical bias.
:::
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=stealth]
  \tikzset{
    ...
    Description/.style={anchor=north, font=\scriptsize, text=black!70, text width=8cm, align=center}
  }

  % Nodes at vertices
  \node[NodeParity] (parity) {\textbf{Demographic}\\\textbf{Parity}\\$P(\hat{Y}|S)$};
  \node[NodeOdds, right=of parity] (odds) {\textbf{Equalized}\\\textbf{Odds}\\$P(\hat{Y}|S, Y)$};
  \node[NodeCalib, above=of $(parity)!0.5!(odds)$] (calib) {\textbf{Predictive}\\\textbf{Parity}\\$P(Y|\hat{Y}, S)$};
  ...
  \node[Description, below=1cm of $(parity)!0.5!(odds)$] {When base rates differ between groups ($P(Y=1|S=a) \neq P(Y=1|S=b)$), it is mathematically impossible to satisfy all three criteria simultaneously.};
...
\node[Box3, below=of B1] (B2) {Does it include\\ PII?};
\node[Box, right=of B2] (B22) {Proceed with preprocessing};
\node[Box3, below=of B2] (B3) {Was user\\ consent acquired?};
\node[Box, left=of B3] (B33) {Reject data or request consent};
\node[Box3, below=of B3] (B4) {Is log access encrypted?};
\node[Box, right=of B4] (B44) {Encrypt or secure logging infrastructure};
\node[Box3, below=of B4] (B5) {Is DP or LDP implemented?};
\node[Box, below=of B5] (B6) {Data eligible for model training};
\node[Box, left=of B5] (B55) {Add privacy protections (e.g., DP-SGD, LDP)};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=stealth]
  ...
  \node[stage, right=of data] (model) {\textbf{Model}\\Learns patterns};
  \node[stage, below=of model] (pred) {\textbf{Predictions}\\Biased outputs};
  \node[stage, below=of data] (feedback) {\textbf{Retraining Data}\\Reinforced bias};
  ...
  \node[mitigation, above left=of data] (m1) {Data Auditing};
  \draw[fix] (m1) -- (data.north west);

  \node[mitigation, above right=of model] (m2) {Fairness\\Constraints};
  \draw[fix] (m2) -- (model.north east);

  \node[mitigation, below right=of pred] (m3) {Output\\Monitoring};
  \draw[fix] (m3) -- (pred.south east);

  \node[mitigation, below left=of feedback] (m4) {Feedback\\Governance};
...
\tikzset{%
    ...
    plus/.style={scale=0.55,line width=1.5pt}
}

\newcommand{\fplus}[1][black]{%
    \tikz\draw[#1] (0,0) -- (1,0)(0.5,0.5) -- (0.5,-0.5);
}

% Nodes for Subgroups
\node[anchor=south west] (A1) at (0,0) {Subgroup A};
\node[anchor=south west] (B1) at (0,-1.25) {Subgroup B};
\node[anchor=south west] (A2) at (12,0) {};
\node[anchor=south west] (B2) at (12,-1.25) {};

% Lines for Subgroups
\draw[LineA] (A1.east) -- (A2.west);
\draw[LineA] (B1.east) -- (B2.west);

% Coordinates for percentages
\coordinate (D7G) at ($(A1.east)!0.3!(A2.west)$);
\coordinate (D8G) at ($(A1.east)!0.66!(A2.west)$);
\coordinate (D7D) at ($(B1.east)!0.3!(B2.west)$);
\coordinate (D8D) at ($(B1.east)!0.66!(B2.west)$);

% Percentage Lines
\draw[LineD] ($(D7G)+(0,1)$) node[above]{75\%} -- ($(D7D)+(0,-0.6)$);
\draw[LineD] ($(D8G)+(0,1)$) node[above]{81.25\%} -- ($(D8D)+(0,-0.6)$);

% Nodes for Subgroup A
\foreach \x in {0.17, 0.47, 0.53, 0.59} {
    \node[circB,yshift=4mm] at ($(A1.east)!{\x}!(A2.west)$) {};
}

\foreach \x in {0.705, 0.775, 0.83, 0.92} {
    \node[yshift=4mm] at ($(A1.east)!{\x}!(A2.west)$) {\fplus[BlueLine]};
}

% Nodes for Subgroup B
\foreach \x in {0.05, 0.11, 0.17, 0.405} {
    \node[circR,yshift=4mm] at ($(B1.east)!{\x}!(B2.west)$) {};
}

\foreach \x in {0.34, 0.47, 0.57, 0.88} {
    \node[yshift=4mm] at ($(B1.east)!{\x}!(B2.west)$) {\fplus[RedLine]};
}
...
\begin{tikzpicture}[line join=round,font=\small\usefont{T1}{phv}{m}{n}]
\tikzset{
    Box/.style={align=flush center, inner xsep=2pt, node distance=1.4, draw=OrangeLine, line width=0.75pt, rounded corners, fill=OrangeL, text width=50mm, minimum width=50mm, minimum height=18mm},
    planet/.style={circle, draw=OrangeLine, semithick, fill=OrangeL, line width=0.75pt, font=\usefont{T1}{phv}{m}{n}\bfseries, minimum size=24mm, inner sep=1mm, align=flush center},
    satelliteI/.style={circle, draw=none, semithick, node distance=1.6, inner sep=1pt, align=flush center, minimum size=28mm, minimum height=12mm},
    satellite/.style={circle, draw=none, semithick, fill=BlueL, text width=26mm, inner sep=1pt, align=flush center, minimum size=20mm, minimum height=12mm},
    ...
    mycylinder/.style={cylinder, shape border rotate=90, aspect=1.3, draw, fill=white, minimum width=25mm, minimum height=11mm, line width=0.75pt, node distance=-0.15}
}
...
\node[satelliteI, fill=BlueL, right=8 of S2] (S3) {};
...
\draw[LineA] (ML) -- (S23);
\draw[LineA] (S23) -- (S25);
\draw[LineA, dashed, RedLine, line width=1.0pt] (S24) to[bend right=15] node[right=5pt, pos=0.8] {Model Update} (S2);
\end{tikzpicture}
```
:::
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
dp_eps_strong = PrivacyPriceAnalysis.eps_strong
dp_eps_mod = PrivacyPriceAnalysis.eps_mod
dp_cost_strong_m = PrivacyPriceAnalysis.cost_strong_str
dp_acc_drop_strong = PrivacyPriceAnalysis.acc_drop_strong_str
dp_overhead_mod = PrivacyPriceAnalysis.overhead_mod_str
dp_acc_drop_mod = PrivacyPriceAnalysis.acc_drop_mod_str
```
...
**Strong Privacy ($\epsilon = `{python} dp_eps_strong`$):** Gradients are heavily clipped ($C = 1.0$, clipping 40% of updates) and noisy ($\sigma = 1.0$). The model requires 3$\times$ more epochs to converge. Training cost jumps from \$4.6M to approximately **\$`{python} dp_cost_strong_m`M**. Accuracy drops **`{python} dp_acc_drop_strong`%**.

**Moderate Privacy ($\epsilon = `{python} dp_eps_mod`$, Apple's standard for keyboard predictions):** Less noise ($\sigma = 0.5$). Training overhead is **`{python} dp_overhead_mod`%**. Accuracy drops **`{python} dp_acc_drop_mod`%**.
...
\begin{scope}[local bounding box=LEFT]
  ...
  \node[Box, below=of {B1.east}, anchor=west] (MO) {Model};
...
\begin{scope}[local bounding box=RIGHT, xshift=11cm]
  ...
  \draw[LineB, OrangeLine] ([yshift=-1mm]MO.352) -- ++(0,-0.81)
    node[below, align=center, black] {Can we remove all influence of\\ someone's data when they ask to \\ delete it, but avoid the full cost of\\ retraining from scratch?};
\end{scope}
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
full_retrain_cost_m = UnlearningCostAnalysis.full_cost_m
sisa_shards = UnlearningCostAnalysis.n_shards
sisa_cost_k_str = UnlearningCostAnalysis.sisa_cost_k_str
sisa_time_hr = UnlearningCostAnalysis.sisa_time_hr_str
```
...
**Baseline (Full Retraining):** For a 175B-parameter model at GPT-3 scale, retraining requires 1,024 A100 GPUs for approximately 34 days at a cost of roughly **\$`{python} full_retrain_cost_m` million**.

**The Engineering Fix (SISA):** Sharded, Isolated, Sliced, and Aggregated training partitions data into $K = `{python} sisa_shards`$ independent shards, training 100 sub-models. To delete one datum, retrain only the specific shard containing it (1% of data). New cost: **\$`{python} sisa_cost_k_str`{,}000**. Time: approximately **`{python} sisa_time_hr` hours**.
...
\tikzset{%
    ...
    Box/.style={align=flush center, inner xsep=2pt, draw=BlueLine, line width=0.75pt, fill=BlueL, text width=23mm, minimum width=24mm, minimum height=10mm},
...
\foreach \x[count=\i] in{0.1,0.26,0.42,0.58,0.74,0.89} {
    \node[anchor=south, below=0.1 of $(B1.east)!\x!(B2.west)$] (L\i) {Decision Trees};
    \ifnum\i=2 \node[anchor=south, below=0.1 of L\i] {Linear Regression}; \fi
    \ifnum\i=3 \node[anchor=south, below=0.1 of L\i] {Logistic Regression}; \fi
    \ifnum\i=4 \node[anchor=south, below=0.1 of L\i] {Random Forest}; \fi
    \ifnum\i=5 \node[anchor=south, below=0.1 of L\i] {Neural Network}; \fi
    \ifnum\i=6 \node[anchor=south, below=0.1 of L\i] {Convolution Neural Network}; \fi
}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=stealth]
  ...
  \node[block, draw=BlueLine, fill=BlueL, right=of Data] (Model) {\textbf{Model Learning}\\(Captures Correlations)};
  \node[block, draw=OrangeLine, fill=OrangeL, below=of Model] (Decision) {\textbf{Biased Decisions}\\(e.g., Target Specific Area)};
  \node[block, draw=BrownLine, fill=BrownL, below=of Data] (NewData) {\textbf{New Observations}\\(Confirms the Bias)};

  % Arrows
  \foreach \source/\dest in {Data/Model, Model/Decision, Decision/NewData, NewData/Data} {
...
\begin{tikzpicture}[line join=round,font=\small\usefont{T1}{phv}{m}{n}]
\tikzset{%
    elli1/.style={draw=BlueLine,line width=0.75pt,ellipse,inner sep=-2pt,fill=BlueL,align=center,text width=45mm, minimum height=33mm},
    elli2/.style={elli1,draw=GreenLine,fill=GreenL,text width=80mm, minimum height=55mm},
    elli3/.style={elli1,draw=OrangeLine,fill=OrangeL,text width=110mm, minimum height=75mm},
    elli4/.style={elli1,draw=VioletLine,fill=VioletL2,text width=120mm, minimum height=90mm},
    textblock/.style={align=center, anchor=north}
}

\begin{scope}[node distance=0pt]
    \node[elli4] (E4) {};
    \node[textblock, below=9pt of E4.north] {\textbf{GOVERNMENT REGULATION}};

    \node[elli3, below=of E4] (E3) {};
    \node[textblock, below=4pt of E3.north] {\textbf{INDUSTRY:} \\ \textbf{Trustworthy Certification:} \\ \textbf{External Reviews}};
    ...
    \node[elli2, below=of E3] (E2) {};
    \node[textblock, below=4pt of E2.north] {\textbf{ORGANIZATION:} \\ \textbf{Safety Culture:} \\ \textbf{Organizational Design}};
    ...
    \node[elli1, below=of E2] (E1) {};
    \node[textblock, below=4pt of E1.north] (TE1) {\textbf{TEAM:}\\ \textbf{Reliable Systems:}\\ \textbf{Software Engineering}};
...
\node[Box5,right=of B4](B5){\textbf{Proxy Reward:}\\ Maximize Clicks};
