...
::: {.callout-note title="Figure Placeholder: Fairness Impossibility" collapse="true"}
```{.tikz}
% TODO: Triangle diagram showing tradeoff between Parity, Odds, and Calibration
\node[draw, align=center] {Fairness Impossibility\nChoose Two};
```
**Fairness Impossibility Theorem**. Visualizing the mathematical conflict between fairness criteria. It is impossible to simultaneously satisfy Demographic Parity, Equalized Odds, and Calibration when base rates differ between groups. This forces engineers to make explicit normative choices about which fairness definition to prioritize based on context.
:::
...
However, threshold adjustment has limitations. If base rates differ substantially between groups (i.e., $P(Y=1 \mid S=a) \neq P(Y=1 \mid S=b)$), achieving one fairness criterion through thresholding will necessarily violate others due to the impossibility theorems. Furthermore, differential thresholds require access to sensitive attributes at inference time and raise concerns about explicit group-based treatment, which may itself be considered unfair or illegal in certain jurisdictions.
...
::: {.callout-note title="Figure Placeholder: Bias Feedback Loop" collapse="true"}
```{.tikz}
% TODO: Cycle diagram: Biased Data -> Model -> Biased Decisions -> New Biased Data
\node[draw, align=center] {Bias Feedback Loop\nSelf-Reinforcing Disparities};
```
...
Yet responsible AI principles remain abstract without concrete applications that demonstrate their implementation. The next chapter (@sec-ai-good) examines how ML systems can be designed specifically to benefit underserved communities and address societal challenges. AI for Good applications face the most demanding requirements: systems must operate under severe resource constraints while achieving high reliability, serve diverse populations while maintaining fairness, and demonstrate sustained impact beyond initial deployment. These applications provide the ultimate test of whether responsible AI principles translate into beneficial outcomes, revealing both the potential and the limitations of ML systems engineering for social impact.
