...
**The Systems Conclusion**: Fairness is not free. Enforcing parity cost **`{python} FairnessTaxAnalysis.fairness_tax_str` percent accuracy** (a huge drop in credit scoring). This is the **Fairness Tax**, the explicit cost of correcting for historical bias.
...
For example, a facial recognition system might have 99 percent accuracy for "Men" and 99 percent accuracy for "Light-Skinned People", but only 65 percent accuracy for "Dark-Skinned Women" [@buolamwini2018gender]. If the audit only checks Race and Gender separately, the model appears fair. This phenomenon, sometimes called **Fairness Gerrymandering**, requires evaluating model performance on intersectional subgroups (e.g., Race$\times$ Gender) to detect and mitigate compounded biases.
...
1.  **Demographic Parity:** Requires equal acceptance rates across groups (e.g., 50 percent men, 50 percent women hired). *Risk:* Can force rejection of qualified candidates if base rates differ.
...
2. **Fairness-Constrained Accuracy**: **`{python} BiasTax.ab_fair_acc_str`**.
...
::: {#fig-rlhf-pipeline fig-env="figure" fig-pos="htb" fig-cap="**RLHF Alignment Pipeline**. Five-stage process for aligning generative models: from a pre-trained Base Model through Supervised Fine-Tuning (SFT) on demonstrations, Human Preference collection, Reward Model training, and PPO Training to an Aligned Model. The figure also annotates label cost, compute overhead, and alignment tax, plus three RLHF failure modes (reward hacking, misaligned internal goals, power-seeking behavior)." fig-alt="Five-stage horizontal pipeline: Base Model -> Supervised Fine-Tuning -> Human Preferences -> Reward Training -> PPO Training -> Aligned Model. Trade-off annotations for label cost, compute, alignment tax, plus three listed RLHF failure modes."
...
::: {#fig-human-centered-ai fig-env="figure" fig-pos="htb" fig-cap="**Stakeholder Responsibility**: Responsibility for a deployed AI system extends outward through nested layers. From the AI system at the center, each surrounding ring (Development Team, Organization, Industry, Government / Society) enforces norms the inner layers cannot self-impose. External anchors (ISO standards and third-party audits; fairness testing, model cards, red-teaming; impact assessments and ethical review boards; GDPR, AI Act, and sector-specific law) bind these layers to practice. " fig-alt="Nested ellipses from center outward: AI System, Development Team, Organization, Industry, Government / Society. External anchors include ISO standards, fairness testing, impact assessments, and GDPR / AI Act."
