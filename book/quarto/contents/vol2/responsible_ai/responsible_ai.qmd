...
|:-------------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------------------------|:------------------------------------------------------------------------------------------|:---------------------------------------------:|:----------------------------------------------------------------------------------------------------------|
| Disparate impact ratio (four-fifths rule)\index{Disparate Impact Ratio!definition}\index{Four-Fifths Rule} [@feldman2015]                  | lower approval rate divided by higher approval rate | $\dfrac{\min_g \Pr(h=1 \mid g)}{\max_g \Pr(h=1 \mid g)}$                                  | `{python} FairnessMetricsTable.di_ratio_math` | a legally recognized screen is needed; $\text{DI} < 0.8$ flags disparate impact                           |
...
**Systems insight**: This illustrates the fundamental trade-off: achieving equal opportunity through threshold adjustment increases false positives and lowers precision for the group receiving the lower threshold. The threshold policy does not alter calibration of the underlying score model, but it can make positive decisions carry different predictive values across groups. The decision involves weighing opportunity equity against prediction reliability.
...
With `{python} FairnessZTest.z_math` (far exceeding critical value $z_{0.05/2} = 1.96$), we reject $H_0$ and conclude the demographic parity violation is statistically significant at $p < 0.001$. Similar tests can be constructed for equal opportunity and equalized odds by restricting to subpopulations where $Y=1$ or $Y=0$ respectively. Statistical significance does not imply practical significance; even statistically significant disparities may be acceptable if the magnitude is small. Conversely, large disparities in small samples may not reach statistical significance but still warrant intervention.
...
One of the core challenges in supporting privacy is the inherent tension between data utility and individual protection. Rich, high-resolution datasets can enhance model accuracy and adaptability but also heighten the risk of exposing sensitive information, particularly when datasets are aggregated or linked with external sources. For example, large language models trained on broad text corpora have been shown to memorize[^fn-model-memorization] specific strings that can later be retrieved through model queries or adversarial prompting [@carlini2021extracting].

[^fn-model-memorization]: **Model Memorization**\index{Model Memorization!privacy risk}: @carlini2021extracting demonstrated that GPT-2 could reproduce verbatim training strings, including contact information, through carefully crafted prompts. The systems implication is narrower but still serious: models that expose generative outputs to user-facing queries can create a privacy attack surface where the serving layer itself becomes a data exfiltration vector, requiring output filtering, rate limiting, and data-minimization controls as defense-in-depth measures.
...
**Answer**: Equality of opportunity directly matches this goal because it requires equal true positive rates among qualified candidates. Equalized odds would add a separate requirement that false positive rates also be equal. Demographic parity can force rejection of qualified candidates when base rates differ, while calibration only ensures that a score such as 0.8 has the same meaning across groups.
...
[^fn-api-security]: **API Security for ML**\index{API Security for ML}\index{API Security!ML serving}: ML serving endpoints face attacks absent from traditional APIs, including model extraction through repeated targeted queries and adversarial input injection. Rate limiting, anomaly detection, access controls, and input validation can reduce exposure, but the fundamental trade-off is that making models more accessible for legitimate explainability simultaneously increases the attack surface for model theft and adversarial probing.
...
class PrivacyPriceAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt, fmt_percent, fmt_usd
...
Training a next-word predictor on sensitive messages with **DP-SGD**\index{DP-SGD!training cost} (Differentially Private Stochastic Gradient Descent) reduces training-data extraction risk, but the protection is priced in compute and accuracy.

The privacy parameter $\epsilon$ bounds privacy loss; smaller values provide a stronger bound when the neighboring relation and $\delta$ are fixed. A reproducible DP-SGD result must also specify the dataset size, batch sampling scheme and rate, number of optimization steps, clipping norm, noise multiplier, neighboring relation, $\delta$, and privacy accountant. The values below are illustrative planning assumptions, not outputs derived by the chapter's sizing code.

**Illustrative stronger-privacy configuration** (assumed accountant result $\epsilon =$ `{python} PrivacyPriceAnalysis.eps_strong_str`): The scenario assumes gradients are clipped at $C = 1.0$, `{python} PrivacyPriceAnalysis.clip_fraction_pct_str` of updates are affected by clipping, and the noise multiplier is $\sigma = 1.0$. It further assumes 3$\times$ more epochs, a training-cost increase from `{python} PrivacyPriceAnalysis.base_cost_str` to approximately `{python} PrivacyPriceAnalysis.cost_strong_str`, and an accuracy drop of `{python} PrivacyPriceAnalysis.acc_drop_strong_str`.
...
**Illustrative weakest configuration** (assumed accountant result $\epsilon = 10$): The scenario assumes less noise and less than 1 percent accuracy loss, with a weaker formal privacy bound than the other two configurations under otherwise comparable accounting assumptions.
...
**Baseline (full retraining)**: In this sizing scenario, a 175B-parameter model at GPT-3 scale retrains on `{python} UnlearningCostAnalysis.full_retrain_gpus_str` NVIDIA A100 (80 GB SXM4) GPUs (assuming dense FP16 $R_{\text{peak}} = 312\text{ TFLOP/s}$ per accelerator at MFU $\eta_{\text{MFU}} \approx 32\%$) for approximately `{python} UnlearningCostAnalysis.full_time_days_str` at a cost of roughly `{python} UnlearningCostAnalysis.full_cost_m_str`.
...
**Mechanism**: To keep serving latency beneath strict real-time SLAs ($L_{\text{lat}} < 100\text{ ms}$), the underwriting inference pipeline omitted inline post-hoc feature attribution ($O/(R_{\text{peak}} \cdot \eta_{\text{hw}})$ across $2^n$ feature subsets). When customers called customer support to contest line-limit decisions, support agents had no feature-level attributions or counterfactual recourse pathways to explain model outputs.
...
class AutomationBias:
    ...
    alpha_range_pct_str = fmt_range(alpha_low * 100, alpha_high * 100, precision=0, commas=False, unit="percent")
...
**Trade-off**: Optimizing clinical efficacy via aggressive alert dispatch conflicts with patient autonomy and privacy protocols. Restricting data retention to protect user privacy limits offline retraining and explanation auditing, while routing high-confidence alerts to human counselors increases per-query serving cost and latency ($L_{\text{lat}}$).

**Mechanism**: The system encodes trade-offs into architectural constraints before model training: local differential privacy bounds input exposure, on-device thresholding gates emergency escalation, and strict SLA budgets ($L_{\text{lat}} < 200\text{ ms}$) cap human-in-the-loop review queues.
...
@Tbl-practitioner-decision-framework provides a practitioner decision framework\index{Practitioner Decision Framework} that maps these deployment contexts to primary principles, implementation priorities, and acceptable trade-offs so the decision can remain context-sensitive rather than universal.
...
While this method improves alignment over standard RL, it also introduces governance risks. Ngo [@ngo2022alignment] identifies three potential failure modes introduced by RLHF:
...
The worked example reports disaggregated fairness metrics\index{Disaggregated Fairness Metrics}, including false positive rate\index{False Positive Rate}, so the same prediction table can satisfy one criterion while failing others. Equalized odds would require both true positive rates and false positive rates to match; this table therefore shows the narrower case of false-positive-rate parity, not full equalized odds.
