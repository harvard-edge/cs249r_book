...
3. **Parity gap**: `{python} FairnessWorkedExample.parity_gap_math` (30 percentage point gap).
...
3. **Fairness tax**: Overall accuracy drops to `{python} CostOfFairness.acc_constrained_pct_str`, incurring a `{python} CostOfFairness.tax_pp_str` absolute accuracy penalty (`{python} CostOfFairness.tax_relative_pct_str` relative drop).
