...
**Baseline (full retraining)**: In this sizing scenario, a 175B-parameter model at GPT-3\index{GPT-3} scale retrains on `{python} UnlearningCostAnalysis.full_retrain_gpus_str` NVIDIA A100 (80-GB SXM4) GPUs (assuming dense FP16 $R_{\text{peak}} = 312\text{ TFLOP/s}$ per accelerator at MFU $\eta_{\text{MFU}} \approx 32\%$) for approximately `{python} UnlearningCostAnalysis.full_time_days_str` at a cost of roughly `{python} UnlearningCostAnalysis.full_cost_m_str`.
