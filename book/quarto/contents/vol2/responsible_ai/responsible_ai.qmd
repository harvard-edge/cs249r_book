...
**Systems insight**: Fairness is not free. Enforcing parity costs `{python} FairnessTaxAnalysis.fairness_tax_pp_str` of accuracy (about a `{python} FairnessTaxAnalysis.fairness_tax_relative_pct_str` relative accuracy drop in this credit-scoring scenario). This is the fairness tax, the explicit cost of correcting for historical bias.
...
class UnlearningCostAnalysis:
    ...
    from mlsysim.units import TFLOPs, second

    gpu_name_str = gpu.name
    gpu_mem_str = fmt_memory_capacity(gpu.memory.capacity, unit=GiB)
    gpu_rpeak_str = fmt_flop_rate(gpu.compute.peak_flops, unit=TFLOPs / second, precision=0, commas=False)
...
@Fig-adversarial-example demonstrates how a visually negligible perturbation can cause confident misclassification, showing how subtle changes produce disproportionately harmful effects in safety-critical applications.
...
[^fn-automation-bias]: **Automation Bias**\index{Bias!automation, human-AI interaction}: First studied in aviation in the 1990s, this is the paradox where humans defer to automated systems even when demonstrably incorrect -- and the effect intensifies as system accuracy increases. At 70--80 percent model accuracy, operators accept erroneous outputs at high rates when presented without uncertainty indicators. For ML serving systems, this means higher model accuracy can paradoxically reduce system-level safety by suppressing human oversight, requiring deliberate interface friction (uncertainty visualization, mandatory justification) that adds latency but preserves the human correction channel.
...
class AutomationBias:
    ...
    acc_low_pct_str = fmt_percent(acc_low_pct / 100, precision=0, commas=False, style='prose')
    acc_high_pct_str = fmt_percent(acc_high_pct / 100, precision=0, commas=False, style='prose')
    override_high_pct_str = fmt_percent(override_high_pct / 100, precision=0, commas=False, style='prose')
    override_low_pct_str = fmt_percent(override_low_pct / 100, precision=0, commas=False, style='prose')
```
...
: **Disaggregated Fairness Metrics**: A hypothetical loan approval system satisfies equalized false positive rates (`{python} FairnessGapCalc.fpr_gap_pp_attr_str` gap) but violates demographic parity (`{python} FairnessGapCalc.approval_gap_pp_attr_str` approval gap) and equal opportunity (`{python} FairnessGapCalc.tpr_gap_pp_attr_str` TPR gap). The metrics expose incompatible operational priorities, so production systems must make explicit choices about which fairness criterion to prioritize. {#tbl-responsible-ai-fairness-summary tbl-colwidths="[25,35,10,10,20]"}
