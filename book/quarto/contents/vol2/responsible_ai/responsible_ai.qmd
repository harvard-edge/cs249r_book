...
<!-- TODO(focused-followup): Items deferred from the codex deep-review
     ...
     - Non-protected callouts use direct address ("Use the section
       structure..." / "your immediate needs") at lines 125, 281, 1098,
       1522. Remove second-person from non-checkpoint/non-learning-
       objectives callouts.
     - Navigation-callout subject-verb disagreement at line 117:
       "Principles and Foundations ... defines" (plural label, singular
       verb); reword.
     - SHAP overhead claim drift (line ~1044): table reports +50 to
       +200 percent inference cost while surrounding prose states 50-
       to 1000-fold overhead or 200 ms to 5+ seconds. Normalize across
       exact SHAP, TreeSHAP, approximate SHAP, and asynchronous paths.
     - Listing has two adjacent string literals where a single docstring
       was intended (line ~1590).
     - `confusion_matrix(...).ravel()` without `labels=[0, 1]` (line
       ~1633): fails when a group window contains only one class.
     - Missing apostrophes at lines 190, 929, 1717, 2131: "the models
       general behavior", "a users data", "an individuals record",
       "the systems intended goals" should be possessive.
     - `images/svg/bias-loop.svg` reused for both `@fig-bias-amplification`
       (line ~907) and `@fig-bias-loop` (line ~2117); generate a
       distinct asset or consolidate.
     - FCC rural-broadband statistic uncited at line ~865; cite or
       remove.
     - Hyphen ranges in decision-framework table at line ~2427.
     - Compound forms drift at line ~180 (datacenter vs. data center,
       high-traffic, real-time, post hoc vs. post-hoc, group-specific);
       standardize.

     Note: raw `\ref{pri-...}` cross-references at lines 399 and 2113
     are correct per the vol1 + vol2 principle-reference convention;
     previously flagged for conversion but should stay as is. -->
...
**Engineering fix (SISA)**: Sharded, Isolated, Sliced, and Aggregated training partitions data into $K =$ `{python} UnlearningCostAnalysis.n_shards` independent shards, training 100 sub-models. To delete one datum, retrain only the specific shard containing it (1 percent of data). New cost: **\$`{python} UnlearningCostAnalysis.sisa_cost_k_str`{,}000**. Time: approximately **`{python} UnlearningCostAnalysis.sisa_time_hr_str` hours**.
...
Establishing effective organizational structures for responsible AI requires more than policy declarations. It demands operational mechanisms: designated roles with responsibility for ethical oversight, clearly defined escalation pathways, accountability for postdeployment monitoring, and incentives that reward teams for ethical foresight and system maintainability. In some organizations, this may take the form of Responsible AI committees, cross-functional review boards, or model risk teams that work alongside developers throughout the model lifecycle. In others, domain experts or user advocates may be embedded into product teams to anticipate downstream impacts and evaluate value tradeoffs in context.
