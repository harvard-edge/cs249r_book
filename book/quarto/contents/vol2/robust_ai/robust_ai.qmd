...
**Context**: In June 2015, Google Photos launched automatic image tagging across billions of user photos, serving $D_{\text{vol}} > 10^9$ images under real-time serving SLAs ($L_{\text{lat}} \le 100\text{ ms}$) [@kasperkevic2015googlephotos].
...
**Impact**: Google issued a public apology and, as an emergency hotfix, completely blocked the tags "gorilla," "chimpanzee," and "monkey" from the classification taxonomy—a blunt category-level suppression that persisted for years because the underlying model lacked verifiable safety boundaries.
...
class RobustAISetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt_int, fmt_params, fmt_qty

    gpt3_params_b_str = fmt_params(m.parameters, scale="B", precision=0, commas=False)
    d_vol_gb_str = fmt_int(int(d_vol_gb))
    a100_hbm_gb_str = fmt_int(a100_hbm_gb)
