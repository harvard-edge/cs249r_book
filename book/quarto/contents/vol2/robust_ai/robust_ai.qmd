...
\tikzset{%
    ...
    Line/.style={line width=1.2pt,BrownLine,text=black},
    Arr/.style={->, line width=1.2pt, VioletLine},
    cube/.style={cylinder, draw=BrownLine, line width=0.75pt, shape border rotate=90, aspect=1.8,inner ysep=0pt,
    ...
    cpuPin/.style={fill=BlueLine, inner sep=0pt},
    cpuPinV/.style={cpuPin, minimum width=3mm, minimum height=12mm},
    cpuPinH/.style={cpuPin, minimum width=12mm, minimum height=3mm}
}

\node[Box](B1){Decompress file size calculation};
\node[Box,below=1.1cm of B1](B2){Scale math.pow()};

\begin{scope}[local bounding box=CPU, below=of B2, yshift=-3.5cm, scale=0.7, every node/.append style={transform shape}]
    \node[fill=BlueLine,minimum width=56mm, minimum height=56mm, rounded corners=8pt,outer sep=2pt] (C1) {};
    \node[fill=white,minimum width=44mm, minimum height=44mm] (C2) {};
    \node[fill=BlueL,minimum width=39mm, minimum height=39mm, align=center,inner sep=0pt,font=\usefont{T1}{phv}{m}{n}\fontsize{8pt}{9}\selectfont] (C3) {Defective\\CPU};

    \foreach \x/\y in {0.11/1,0.26/2,0.41/3,0.56/4,0.71/5,0.85/6}{
        \node[cpuPinV,anchor=south](GO\y)at($(C1.north west)!\x!(C1.north east)$){};
        \node[cpuPinV,anchor=north](DO\y)at($(C1.south west)!\x!(C1.south east)$){};
        \node[cpuPinH,anchor=east](LE\y)at($(C1.north west)!\x!(C1.south west)$){};
        \node[cpuPinH,anchor=west](DE\y)at($(C1.north east)!\x!(C1.south east)$){};
    }
\end{scope}

\begin{scope}[local bounding box=CY1, right=3cm of B2]
    \node (CA1) [cube] {};
...
\begin{scope}[local bounding box=CY2, left=3cm of B2]
    \node (LCA1) [cube] {};
...
\draw[Arr] (LEV) -- ++(-1.5,0) coordinate (LS);
\draw[Arr] (DES) -- ++(1.5,0) coordinate (DS);

\scoped[on background layer]
\node[draw=VioletLine,inner xsep=6.5mm,inner ysep=6.5mm,outer sep=0pt, yshift=2mm,fill=none,fit=(CPU)(B1),line width=0.75pt] (BB1) {};
\node[below=3pt of BB1.north,anchor=north,helvetica] {Shuffle and merge};

\node[Box2,below left=0.5cm and 0.1cm of LS] (N2) {\textbf{2.} Compute (1.1)\textsuperscript{53}};
\node[Box2,below right=0.5cm and 0.1cm of DS,fill=BlueL,draw=BlueLine] (R3) {\textbf{3.} Result = 0};
\node[Box2,below=0.3cm of R3,text width=43mm] (N3) {\textbf{3.} Expected Result = 156.24};

\node[Box2,above=1cm of CY2] (N1) {\textbf{1.} Compute file size for decompression};
\node[Box2,above=1cm of CY1] (N4) {\textbf{4.} Write file to database if size $>$ 0};
\node[Box2,below=1cm of CY1] (N5) {\textbf{5.} Missing rows in DB};

\draw[Line,-latex] (N5) |- (CA3.before bottom);
\draw[Line,-latex] (N5.50) |- (CA4.6);
\draw[Line] (N3.west) -| ($(R3.south west) + (-0.5,-0.5)$) |- (R3.west);
...
gpt3_params_b = RobustAISetup.gpt3_params_b
v100_mem_bw = RobustAISetup.v100_mem_bw
```
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \tikzset{
    ...
    crossLine/.style={RedLine, line width=1.2pt},
    arrowLine/.style={->, line width=1.2pt, RedLine}
  }
  ...
  \node[expBit, right=of S] (E1) {0};
  \foreach \i [evaluate=\i as \prev using int(\i-1)] in {2,...,8} {
    \node[expBit, right=of E\prev] (E\i) {0};
  }
  \node[font=\tiny, above right=0.1cm and -0.2cm of E4] {Exponent (8 bits)};

  \node[mantBit, right=of E8] (M9) {1};
  \foreach \i [evaluate=\i as \prev using int(\i-1)] in {10,...,12} {
    \node[mantBit, right=of M\prev] (M\i) {1};
  }
  \node[right=0.2cm of M12] (Dots) {...};
  \node[font=\tiny, above right=0.1cm and -0.2cm of M10] {Mantissa (23 bits)};
  ...
  \node[anchor=north west, font=\scriptsize, text=BrownLine, below right=0.5cm and 0.5cm of E4f] {Exponent bit-flip causes order-of-magnitude explosion.};
...
\tikzset{
  ...
  BoxHeader/.style={Box, fill=GreenD, draw=GreenD, minimum height=10mm, text=white, font=\usefont{T1}{phv}{m}{n}\bfseries, inner ysep=2pt},
  BoxInput/.style={Box, fill=BlueL, draw=BlueLine},
  BoxInputHeader/.style={Box, fill=BlueLine, draw=BlueLine, minimum height=10mm, text=white, font=\usefont{T1}{phv}{m}{n}\bfseries, inner ysep=2pt},
  BoxEnv/.style={Box, fill=RedL, draw=RedLine},
  BoxEnvHeader/.style={Box, fill=RedL, draw=RedLine, minimum height=10mm, text=white, font=\usefont{T1}{phv}{m}{n}\bfseries, inner ysep=2pt},
  BoxFinal/.style={Box, draw=OrangeLine, fill=OrangeL, text width=138mm, minimum width=138mm, minimum height=10mm},
  Line/.style={line width=1.2pt, shorten <=1pt, shorten >=2pt}
}

\node[BoxFinal] (B0) {Robust AI System};

\node[BoxHeader, below=0.7cm of B0.south west, anchor=north west] (B11) {Software\\ Faults};
...
\node[BoxInputHeader, below=0.7cm of B0.south, anchor=north] (B22) {Input-Level\\ Attacks};
\node[BoxInput, below=0pt of B22.south, anchor=north, text depth=30mm, align=left] (B2) {\parskip=3pt%
...
\node[BoxEnvHeader, below=0.7cm of B0.south east, anchor=north east] (B33) {Environmental\\ Shifts};
\node[BoxEnv, below=0pt of B33.south, anchor=north, text depth=30mm, align=left] (B3) {\parskip=3pt%
...
\foreach \i/\col in {11/GreenD, 22/BlueLine, 33/RedLine}{
  \draw[Line, \col] (B0) -- (B\i.north);
}
\end{tikzpicture}
```
:::
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line join=round]
    \tikzset{
        basebox/.style={draw, thick, rounded corners, align=center, minimum height=5cm, minimum width=3.5cm, anchor=north},
        col1box/.style={basebox, BlueLine, fill=BlueL!5},
        col2box/.style={basebox, RedLine, fill=RedL!5},
        col3box/.style={basebox, GreenLine, fill=GreenL!5},
        titlebox/.style={draw, very thick, fill=gray!10, minimum width=10cm, minimum height=1cm, rounded corners, font=\Large\bfseries},
        header/.style={font=\bfseries, align=center},
        content/.style={align=left, font=\small, text width=3cm},
        arrow/.style={thick, ->, BrownLine}
    }

    % Root node
    \node[titlebox] (root) {Unified Robustness Framework};

    % Column 2 (Center)
    \node[col2box, below=1cm of root] (col2) {};
    \node[header, RedLine, below=0.2cm of col2.north] (col2h) {Data\\Poisoning};
    \node[content, below=0.2cm of col2h] (col2c) {
        \textbf{Attacks:}\\
        - Label Flipping\\
    ...
    % Column 1 (Left)
    \node[col1box, left=0.5cm of col2] (col1) {};
    \node[header, BlueLine, below=0.2cm of col1.north] (col1h) {Adversarial\\Manipulation};
    \node[content, below=0.2cm of col1h] (col1c) {
        \textbf{Attacks:}\\
        - FGSM\\
        - PGD\\
        - C\&W\\[0.3cm]
    ...
    % Column 3 (Right)
    \node[col3box, right=0.5cm of col2] (col3) {};
    \node[header, GreenLine, below=0.2cm of col3.north] (col3h) {Distribution\\Shift};
    \node[content, below=0.2cm of col3h] (col3c) {
        \textbf{Types:}\\
        - Covariate Shift\\
        - Concept Drift\\
        - Label Shift\\[0.3cm]
    ...
    % Arrows
    \foreach \col in {col1, col2, col3} {
        \draw[arrow] (root.south) -- (\col.north);
    }
\end{tikzpicture}
```
:::
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.8, node distance=1.5cm, positioning]
  \tikzset{
    axis/.style={->, line width=1.0pt, BrownLine},
    dist/.style={line width=1.2pt, domain=-1.5:1.5, samples=50},
    shifted/.style={dashed, line width=1.2pt, domain=-1.5:1.5, samples=50},
    box/.style={minimum width=1cm, anchor=south west, inner sep=0pt},
    label/.style={align=center, font=\footnotesize},
    bluebox/.style={fill=BlueL, box},
    redbox/.style={fill=RedL, opacity=0.7, box},
    blueLine/.style={line width=1.2pt, BlueLine},
    redLine/.style={dashed, line width=1.2pt, RedLine}
  }

  % 1. Covariate Shift
  \begin{scope}[local bounding box=covariate]
    \coordinate (origin1) at (0,0);
    \draw[axis] (origin1) -- ++(3.5,0) node[right] {$x$};
    \draw[axis] (origin1) -- ++(0,2.5) node[above] {$P(x)$};
    \draw[dist, BlueLine] plot (\x+1.2, {1.8*exp(-(\x)*(\x))});
    \draw[shifted, RedLine] plot (\x+2.2, {1.8*exp(-(\x)*(\x))});
    \node[label, below=0.5cm of origin1, anchor=north, xshift=1.75cm] {\textbf{Covariate Shift}\\$P(x)$ changes\\$P(y|x)$ constant};
  \end{scope}

  % 2. Label Shift
  \begin{scope}[local bounding box=labelshift, shift={([xshift=5cm]origin1)}]
    \coordinate (origin2) at (0,0);
    \draw[axis] (origin2) -- ++(3.5,0) node[right] {$y$};
    \draw[axis] (origin2) -- ++(0,2.5) node[above] {$P(y)$};
    
    \node[bluebox, minimum height=0.8cm] at ([xshift=0.5cm]origin2) {};
    \node[redbox, minimum height=1.5cm] at ([xshift=0.5cm]origin2) {};
    
    \node[bluebox, minimum height=1.8cm] at ([xshift=2.0cm]origin2) {};
    \node[redbox, minimum height=0.5cm] at ([xshift=2.0cm]origin2) {};
    
    \node[label, below=0.5cm of origin2, anchor=north, xshift=1.75cm] {\textbf{Label Shift}\\$P(y)$ changes\\$P(x|y)$ constant};
  \end{scope}

  % 3. Concept Drift
  \begin{scope}[local bounding box=conceptdrift, shift={([xshift=5cm]origin2)}]
    \coordinate (origin3) at (0,0);
    \draw[axis] (origin3) -- ++(3.5,0) node[right] {$x$};
    \draw[axis] (origin3) -- ++(0,2.5) node[above] {$y$};
    \draw[blueLine] ([xshift=0.5cm,yshift=0.5cm]origin3) -- ([xshift=3.0cm,yshift=2.0cm]origin3);
    \draw[redLine] ([xshift=0.5cm,yshift=2.0cm]origin3) -- ([xshift=3.0cm,yshift=0.5cm]origin3);
    \node[label, below=0.5cm of origin3, anchor=north, xshift=1.75cm] {\textbf{Concept Drift}\\$P(y|x)$ changes\\Relationship evolves};
  \end{scope}
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm, positioning]
  \tikzset{
    decision boundary/.style={line width=1.2pt, BlueLine},
    class label/.style={font=\bfseries},
    point/.style={circle, inner sep=2pt},
    shift vector/.style={->, line width=1.2pt, RedLine},
    distance constraint/.style={dashed, BrownLine, line width=1.0pt},
    label style/.style={font=\footnotesize}
  }

  % Clean Point as Origin
  \node[point, fill=GreenL, label={below:Training $x$}, anchor=center] (X) at (0,0) {};

  % Shifted Point
  \node[point, fill=RedL, label={above:Shifted $x'$}, anchor=center] (Xadv) [above right=1.2cm and 1.8cm of X] {};

  % Regions
  \node[class label] (classA) [left=1.5cm of X] {\textbf{Class A} (Stop)};
  \node[class label] (classB) [right=1.5cm of Xadv] {\textbf{Class B} (Speed)};

  % Decision Boundary (Curved)
  \begin{scope}
    \draw[decision boundary] plot [smooth, tension=1] coordinates {([xshift=-2cm,yshift=2.5cm]X) ([xshift=1cm,yshift=0.5cm]X) ([xshift=4cm,yshift=-1.5cm]X)};
    \node[BlueLine, font=\scriptsize, anchor=south west] at ($(Xadv.east) + (0.5cm, 0)$) {Decision Boundary};
  \end{scope}

  % Shift Vector
  \draw[shift vector] (X) -- (Xadv) node[midway, sloped, above, label style] {Distribution shift $\Delta$};

  % Distance constraint
  \begin{scope}
    \draw[distance constraint] (X) circle (2.2cm);
    \node[BrownLine, label style] at ($(X.south) + (0,-2.3cm)$) {Shift magnitude $|\|\Delta|\|$};
  \end{scope}
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
mmd_n_samples_str = DistributionShiftMetrics.mmd_n_samples_str
mmd_latency_ms_str = DistributionShiftMetrics.mmd_latency_ms_str
ks_n_samples_str = DistributionShiftMetrics.ks_n_samples_str
psi_warn_thresh_str = DistributionShiftMetrics.psi_warn_thresh_str
psi_critical_thresh_str = DistributionShiftMetrics.psi_critical_thresh_str
```

Statistical distance metrics quantify the degree of distribution shift by measuring differences between training and deployment data distributions. Maximum Mean Discrepancy (MMD) with RBF kernels ($\gamma = 1.0$) provides detection sensitivity of 0.85 for shifts with Cohen's $d > 0.5$, processing `{python} mmd_n_samples_str` samples in `{python} mmd_latency_ms_str`ms on modern hardware. Kolmogorov-Smirnov tests achieve 95% detection rates for univariate shifts with `{python} ks_n_samples_str`+ samples, but scale poorly to high-dimensional data. Population Stability Index (PSI)[^fn-psi-etymology] thresholds of `{python} psi_warn_thresh_str`–`{python} psi_critical_thresh_str` indicate significant shift requiring model investigation.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm, positioning]
\tikzset{
  basebox/.style={rectangle, draw, line width=0.75pt, minimum width=3cm, minimum height=1.5cm, align=center, anchor=center, rounded corners=2pt},
  dataset/.style={basebox, fill=GreenL, draw=GreenLine},
  poisoned/.style={basebox, fill=RedL, draw=RedLine},
  algo/.style={basebox, fill=OrangeL, draw=OrangeLine},
  model/.style={basebox, fill=BlueL, draw=BlueLine},
  arrow/.style={->, line width=1.2pt, BrownLine},
  container/.style={draw, line width=0.75pt, inner sep=0.5cm, rounded corners=4pt}
}

% Normal learning diagram
\begin{scope}[local bounding box=NORMAL]
  \node[dataset] (data1) {Clean\\Dataset};
  \node[algo, below=of data1] (algo1) {Learning\\Algorithm};
  \node[model, below=of algo1] (model1) {Clean\\Model};
  
  \draw[arrow] (data1) -- (algo1);
  \draw[arrow] (algo1) -- (model1);
  
  % Background box
  \begin{scope}[on background layer]
    \node[container, draw=BlueLine, fill=BlueL!20, fit=(data1)(model1)] (fit1) {};
  \end{scope}
  \node[above=0.2cm of fit1, BlueLine, font=\bfseries] {Normal Learning};
\end{scope}

% Poisoning attack diagram
\begin{scope}[local bounding box=POISONING, right=3cm of NORMAL]
  \node[dataset, minimum width=1.4cm] (data2_clean) {Clean\\Data};
  \node[poisoned, minimum width=1.4cm, right=0.2cm of data2_clean] (data2_poison) {Poisoned\\Data};
  
  % Create a coordinate in the middle of the two data boxes for alignment
  \coordinate (data2_mid) at ($(data2_clean.east)!0.5!(data2_poison.west)$);
  
  \node[algo, below=1.5cm of data2_mid] (algo2) {Learning\\Algorithm};
  \node[model, below=of algo2] (model2) {Compromised\\Model};
  
  \draw[arrow] (data2_clean.south) -- (algo2.north);
  \draw[arrow] (data2_poison.south) -- (algo2.north);
  \draw[arrow] (algo2) -- (model2);
  
  % Background box
  \begin{scope}[on background layer]
    \node[container, draw=RedLine, fill=RedL!20, fit=(data2_clean)(data2_poison)(model2)] (fit2) {};
  \end{scope}
  \node[above=0.2cm of fit2, RedLine, font=\bfseries] {Poisoning Attack};
\end{scope}
...
\tikzset{
    ...
    comp/.style={draw, minimum width=20mm, minimum height=12mm, inner sep=0pt, rounded corners, draw=BlueLine, fill=BlueL, line width=0.75pt},
    Line/.style={line width=1.0pt, VioletLine, text=black},
    DLine/.style={dashed, line width=1.0pt, VioletLine, text=black},
    person/.style={scale=0.5, line width=1.0pt},
    gear/.style={scale=0.1, every node/.append style={transform shape}, fill=RedL, even odd rule},
    data/.style={node distance=-1.7, scale=0.4, every node/.append style={transform shape}},
    legend/.style={font=\footnotesize\usefont{T1}{phv}{m}{n}}
}

% Cloud
\node[RedLine, cloud, cloud puffs=11.4, cloud ignores aspect, minimum width=40mm, minimum height=24mm, rotate=10, align=center, draw=BrownLine, line width=0.75pt] (cloud) {};

% Two Gears
\node[anchor=center] (GEAR2) at ($(cloud.300)+(0.5,1.05)$) {};
\begin{scope}[gear, shift={(0,0)}]
    \fill \gear{10}{1.9}{1.4}{10}{2}{0.6} coordinate (GER2);
\end{scope}
\begin{scope}[gear, shift={(-1.8,-2.08)}, scale=0.15]
    \fill \gear{10}{1.9}{1.4}{11}{2}{0.6} coordinate (GER2);
\end{scope}
\draw[Line, -latex] (GEAR2) -- ++(10:1.75) node[right, align=center, text=black] {Poisoned Model\\ Aggregation};

% Persons
\foreach \i in {1,2} {
    \begin{scope}[shift={(-0.9,-0.16)}, person, yshift={\i*0.3}]
        ...
        \draw[rounded corners=1.5mm, fill=GreenL] (top) to [out=-10,in=100] (right) to [bend left=15] (left) to [out=80,in=190] (top);
...
\draw[draw=BlueLine, line width=1.0pt, shorten <=-3mm, shorten >=-3mm] (DL) -- node[below=4pt] {Client 1} (DD);
\node[draw, GreenLine, inner sep=0pt] (CB1) at ($(COM.north west)!0.25!(COM.south west)+(0.3,0)$) {$\times$};
\node[draw, GreenLine, inner sep=0pt] (CB2) at ($(COM.north west)!0.6!(COM.south west)+(0.3,0)$) {$\times$};
\foreach \j in {1,2} {
    \draw[GreenLine, decoration={zigzag, segment length=4pt, amplitude=0.5pt}, decorate] ($(CB\j)+(0.3,0.05)$) -- ++(0:1.3);
    \draw[GreenLine, decoration={zigzag, segment length=4pt, amplitude=0.5pt}, decorate] ($(CB\j)+(0.3,-0.12)$) -- ++(0:1.0);
}

% Gear left
\node[anchor=center] (GEAR1) at ($(cloud.200)!0.1!(COM.110)+(-0.4,0)$) {};
\begin{scope}[gear]
    \fill \gear{10}{1.9}{1.4}{10}{2}{0.6} coordinate (GER1);
    \draw[-latex, shorten <=3mm] (GER1) -- ++(233:9);
\end{scope}

% Data left
\node[mycylinder, fill=RedL] (A) at ($(COM.east)+(0.7,0.3)$) {};
\scoped[on background layer]
...
% Display right
\node[comp] (COM2) at {(3,-4)} {};
\draw[draw=BlueLine, line width=1.0pt] ($(COM2.north west)!0.85!(COM2.south west)$) -- ($(COM2.north east)!0.85!(COM2.south east)$);
...
\draw[draw=BlueLine, line width=1.0pt, shorten <=-3mm, shorten >=-3mm] (DL2) -- node[below=4pt] {Client 2} (DD2);
...
\foreach \j in {1,2} {
    \draw[GreenLine, decoration={zigzag, segment length=4pt, amplitude=0.5pt}, decorate] ($(2CB\j)+(0.3,0.05)$) -- ++(0:1.3);
    \draw[GreenLine, decoration={zigzag, segment length=4pt, amplitude=0.5pt}, decorate] ($(2CB\j)+(0.3,-0.12)$) -- ++(0:1.0);
}
...
% Gear right
\node[anchor=center] (GEAR3) at ($(cloud.330)!0.25!(COM2.110)+(-0.6,0)$) {};
\begin{scope}[gear]
    \fill \gear{10}{1.9}{1.4}{10}{2}{0.6} coordinate (2GER1);
    \draw[-latex, shorten <=3mm] (2GER1) -- ++(300:9);
\end{scope}

% Data right
\node[mycylinder, fill=GreenL] (A) at ($(COM2.east)+(0.7,0.3)$) {};
\scoped[on background layer]
...
% Legend
% Persons
\node[below=3pt of (7.75,0.9), legend] {Owner on server};
\foreach \i in {1,2} {
    \begin{scope}[shift={(0.3,0.3)}, person, anchor=center]
        \coordinate (head-center) at (0,0);
        \coordinate (top) at ([yshift=-2mm]head-center);
        \coordinate (left) at ([yshift=-10mm,xshift=-7mm]head-center);
        \coordinate (right) at ([yshift=-10mm,xshift=7mm]head-center);
...
% Data legend 1
\node[below=3pt of (LPER)+(0,-1.7), legend] {Local Data};
\node[mycylinder, fill=GreenL] (A) at {(LPER)+(0,-1.7)} {};
\node[mycylinder, above=of A, fill=GreenL] (C) {};
\node[mycylinder, above=of C, fill=GreenL] (B) {};

% Gear legend
\node[below=3pt of (LSC2)+(0,-1.7), legend] {Poisoned Global Model};
\node[mycylinder, fill=RedL] (A) at {(LSC2)+(0,-1.7)} {};
\node[mycylinder, above=of A, fill=RedL] (C) {};
\node[mycylinder, above=of C, fill=RedL] (B) {};

% Data legend 2
\node[below=3pt of (LGEAR3)+(0,-2.05), legend] {Poisoned Local Data};
\node[mycylinder, fill=RedL] (A) at {(LGEAR3)+(0,-2.05)} {};
\node[mycylinder, above=of A, fill=RedL] (C) {};
\node[mycylinder, above=of C, fill=RedL] (B) {};

% Fitting
\scoped[on background layer]
\node[draw=BrownLine, inner xsep=3mm, inner ysep=2mm, fill=BrownL, fit=(PLD)(LPER), line width=0.75pt] (BB) {};
\end{tikzpicture}
```
:::
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
clean_acc_rn50 = RobustnessTaxAnalysis.clean_acc_str
robust_acc_rn50 = RobustnessTaxAnalysis.robust_acc_str
acc_drop_rn50 = RobustnessTaxAnalysis.acc_drop_str
```
...
1.  **Standard ResNet-50**: `{python} clean_acc_rn50`% Top-1 Accuracy on ImageNet.
2.  **Adversarially Trained ResNet-50** ($\epsilon=8/255$): ~`{python} robust_acc_rn50`% Top-1 Accuracy on Clean ImageNet.
...
```{.tikz}
\begin{tikzpicture}[line join=round, font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm]
    \colorlet{Green}{GreenL}
    \colorlet{Red}{RedL}
    \colorlet{Blue}{BlueL}
    \colorlet{Violet}{VioletL}
    ...
    \tikzset{
        doubleL/.style={-{Triangle[length=0.44cm,width=0.62cm]},line width=1.0pt,VioletLine,text=black},
        doubleLBlue/.style={-{Triangle[length=0.44cm,width=0.62cm]},line width=1.0pt,BlueLine,text=black},
        Line/.style={line width=1.0pt,VioletLine,text=black},
        DLine/.style={dashed,line width=1.0pt,VioletLine,text=black},
        cell/.style={draw=BrownLine, fill=GreenL, minimum width=6mm, minimum height=4mm, line width=0.75pt, inner sep=0pt, outer sep=0pt, anchor=center},
        cellB/.style={draw=BrownLine, fill=GreenL, minimum width=3mm, minimum height=2mm, line width=0.75pt, inner sep=0pt, outer sep=0pt, anchor=center},
        face/.style={fill=OrangeL, draw=BrownLine, line width=0.75pt},
        eye/.style={circle,inner sep=0pt,minimum width=2pt,minimum height=2pt,fill=black},
        eye-white/.style={ellipse,inner sep=0pt,minimum width=3pt,minimum height=2pt,fill=white},
        textnode/.style={align=center,text=black}
    }

    % Matrix
    \matrix (MAT) [matrix of nodes, nodes={cell}, row sep=0pt, column sep=0pt] {
        \node {}; & \node {}; & \node {}; & \node {}; & \node {}; & \node {}; \\
        \node {}; & \node {}; & \node {}; & \node {}; & \node {}; & \node {}; \\
        \node {}; & \node {}; & \node {}; & \node {}; & \node {}; & \node {}; \\
        \node {}; & \node {}; & \node {}; & \node {}; & \node {}; & \node {}; \\
    ...
    % Defender
    \begin{scope}[local bounding box=DEF, below=of MAT, yshift=-2.0cm, scale=0.7, line width=1.0pt]
        \coordinate (head-center) at (0,0);
        \draw[rounded corners=1.5mm,fill=GreenL] (0,-2) to [out=-10,in=100] (1.5,-2) to [bend left=15] (-1.5,-2) to [out=80,in=190] (0,-2);
        \fill[OrangeL] (head-center) circle (0.35);
        \node[eye] (OKO1) at ($(head-center)+(0.17,0.1)$) {};
        \node[eye] (OKO2) at ($(head-center)+(-0.17,0.1)$) {};
        \draw[] ($(OKO2)+(0,-0.2)$) to[bend right=40] ($(OKO1)+(0,-0.2)$);
    \end{scope}

    % Attacker
    \begin{scope}[local bounding box=ATA, left=of cell-1-6A, xshift=-3.5cm, scale=0.7, line width=1.0pt]
        \coordinate (HC) at (0,0);
        \draw[rounded corners=1.5mm,fill=black] (0,-2) to [out=-10,in=100] (1.5,-2) to [bend left=15] (-1.5,-2) to [out=80,in=190] (0,-2);
        \fill[black] (HC) circle (0.35);
        \node[eye-white] (OKO1) at ($(HC)+(0.17,0.1)$) {};
        \node[eye-white] (OKO2) at ($(HC)+(-0.17,0.1)$) {};
        \draw[white] ($(OKO2)+(0,-0.25)$) to[bend left=40] ($(OKO1)+(0,-0.25)$);
    \end{scope}

    % Poisoned Model
    \begin{scope}[local bounding box=POI, right=of cell-6-6A, xshift=3.0cm]
        \newcommand{\Depth}{1.8}
        ...
        \draw[face] (D2) -- (E2) -- (F2) -- (G2) -- cycle;
        \draw[face] (C2) -- (B2) -- (F2) -- (G2) -- (C2);
        \draw[face] (A2) -- (B2) -- (F2) -- (E2) -- cycle;
        \node[textnode] at ($(B2)!0.5!(G2)$) {Poisoned\\ Model};
    \end{scope}

    % Arrows and Labels
    \draw[doubleLBlue] ($(DEF.east)+(0.2,0)$) coordinate(LE) -| coordinate(DE) ($(POI.south)+(0,-0.5)$);
    \draw[doubleLBlue] ($(DEF.north)+(0,0.2)$) -- node[right=5pt,textnode] {Analyze\\ and clean} ($(MAT.south)+(0,-0.2)$);
    \node[below=0.1 of DEF] {\textbf{Defender}};
    \node[below=0.1 of ATA] {\textbf{Attacker}};
    \draw[doubleL] ($(ATA.east)+(0.2,0)$) -- node[below=5pt,textnode] {Malicious data\\ injection} ($(cell-1-6A.west)+(-0.2,0)$);
    \draw[doubleL] ($(cell-6-6A.east)+(0.2,0)$) -- node[below=5pt,textnode] {Train} ($(POI.west)+(-0.2,0)$);
    \node[font=\huge\usefont{T1}{phv}{m}{n}\bfseries,VioletLine] at (MAT.center) {R};
    \node[above=0.1 of MAT] {Item};
    \node[left=0.1 of MAT] {User};

    % Retrain Matrix
    \matrix (MAT1) [matrix of nodes, nodes={cellB}, row sep=0pt, column sep=0pt, scale=0.7, every node/.append style={transform shape}, above=0.5cm of LE] {
        \node {}; & \node {}; & \node {}; & \node {}; & \node {}; & \node {}; \\
        \node {}; & \node {}; & \node {}; & \node {}; & \node {}; & \node {}; \\
        \node {}; & \node {}; & \node {}; & \node {}; & \node {}; & \node {}; \\
        \node {}; & \node {}; & \node {}; & \node {}; & \node {}; & \node {}; \\
...
```{.tikz}
\begin{tikzpicture}[line join=round, font=\small\usefont{T1}{phv}{m}{n}, node distance=0.5cm]
\def\cellsize{6mm}
\def\cellheight{6mm}

\tikzset{
    Box/.style={draw=BrownLine, minimum width=\cellsize, minimum height=\cellheight, line width=0.75pt, inner sep=0pt, outer sep=0pt, anchor=center},
    Line/.style={dashed, line width=1.0pt, BrownLine},
    Label/.style={above=0.2cm, font=\small\usefont{T1}{phv}{m}{n}, text=black},
    Input/.style={fill=GreenL},
    Output/.style={fill=BlueL},
    Code/.style={fill=VioletL},
    Encoder/.style={RedLine, line width=1.0pt, decoration={brace, amplitude=9pt, mirror}, decorate},
    Decoder/.style={RedLine, line width=1.0pt, decoration={brace, amplitude=9pt}, decorate}
}

% Main structure
\matrix (SRE) [matrix of nodes, nodes={Box, Code}, row sep=0pt] {
    |[name=cell-1-1A]| \\
    |[name=cell-1-2A]| \\
    |[name=cell-1-3A]| \\
};
...
% Right structures
\matrix (SRED1) [matrix of nodes, nodes={Box, OrangeL}, row sep=0pt, right=of SRE] {
    |[name=cell-1-1A1]| \\ |[name=cell-1-2A1]| \\ |[name=cell-1-3A1]| \\
    |[name=cell-1-4A1]| \\ |[name=cell-1-5A1]| \\ |[name=cell-1-6A1]| \\
};

\matrix (SRED2) [matrix of nodes, nodes={Box, BlueL}, row sep=0pt, right=of SRED1] {
    |[name=cell-1-1A2]| \\ |[name=cell-1-2A2]| \\ |[name=cell-1-3A2]| \\ |[name=cell-1-4A2]| \\
    |[name=cell-1-5A2]| \\ |[name=cell-1-6A2]| \\ |[name=cell-1-7A2]| \\ |[name=cell-1-8A2]| \\
};

\matrix (SRED3) [matrix of nodes, nodes={Box, GreenL}, row sep=0pt, right=of SRED2] {
    |[name=cell-1-1A3]| \\ |[name=cell-1-2A3]| \\ |[name=cell-1-3A3]| \\ |[name=cell-1-4A3]| \\ |[name=cell-1-5A3]| \\
    |[name=cell-1-6A3]| \\ |[name=cell-1-7A3]| \\ |[name=cell-1-8A3]| \\ |[name=cell-1-9A3]| \\ |[name=cell-1-10A3]| \\
};

% Left structures
\matrix (LSRED1) [matrix of nodes, nodes={Box, OrangeL}, row sep=0pt, left=of SRE] {
    |[name=cell-1-1LA1]| \\ |[name=cell-1-2LA1]| \\ |[name=cell-1-3LA1]| \\
    |[name=cell-1-4LA1]| \\ |[name=cell-1-5LA1]| \\ |[name=cell-1-6LA1]| \\
};

\matrix (LSRED2) [matrix of nodes, nodes={Box, BlueL}, row sep=0pt, left=of LSRED1] {
    |[name=cell-1-1LA2]| \\ |[name=cell-1-2LA2]| \\ |[name=cell-1-3LA2]| \\ |[name=cell-1-4LA2]| \\
    |[name=cell-1-5LA2]| \\ |[name=cell-1-6LA2]| \\ |[name=cell-1-7LA2]| \\ |[name=cell-1-8LA2]| \\
};

\matrix (LSRED3) [matrix of nodes, nodes={Box, GreenL}, row sep=0pt, left=of LSRED2] {
    |[name=cell-1-1LA3]| \\ |[name=cell-1-2LA3]| \\ |[name=cell-1-3LA3]| \\ |[name=cell-1-4LA3]| \\ |[name=cell-1-5LA3]| \\
    |[name=cell-1-6LA3]| \\ |[name=cell-1-7LA3]| \\ |[name=cell-1-8LA3]| \\ |[name=cell-1-9LA3]| \\ |[name=cell-1-10LA3]| \\
};

% Labels
\node[Label] at (SRE.north) {Code};
\node[Label] at (SRED3.north) {Output};
\node[Label] at (LSRED3.north) {Input};

% Dashed lines
\foreach \i in {1, 2, 3} {
    \draw[Line] (cell-1-1LA3.north east) -- (cell-1-1A2.north west);
    \draw[Line] (cell-1-10LA3.south east) -- (cell-1-8A2.south west);
    \draw[Line] (cell-1-1LA3.north east) -- (cell-1-8A2.south west);
    \draw[Line] (cell-1-10LA3.south east) -- (cell-1-1A2.north west);
...
\coordinate (L2) at ($(cell-1-6LA1.south east) + (0, -0.5)$);
\coordinate (D1) at ($(cell-1-10A3.south east) + (0, -0.5)$);
\coordinate (D2) at ($(cell-1-6A1.south west) + (0, -0.5)$);

\draw[Encoder] (L1) -- node[below=9pt, text=black] {Encoder} (L1 -| L2);
\draw[Decoder] (D1) -- node[below=9pt, text=black] {Decoder} (D1 -| D2);
\end{tikzpicture}
```
:::
