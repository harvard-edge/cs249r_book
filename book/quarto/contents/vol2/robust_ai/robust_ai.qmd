...
Silent failures have already caused significant damage to production systems across cloud, edge, and embedded deployments.
...
::: {.column-margin}
![](images/svg/vol2_robust_ai_margin_001.svg){width="100%" fig-alt="Margin illustration showing One failing shard cascades through pipeline stages to a failed request."}
...
# │ Exports: gpt3_params_b_str, v100_mem_bw_gb_s_str, bits_per_second_math,
# │          base_error_rate_math, expected_errors_per_hour_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_math, fmt_qty, sci_latex
...
class RobustAISetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt, fmt_math, fmt_qty, sci_latex
    gpt3_params_b_str = fmt(gpt3_params_b_val, precision=0, commas=False)
    v100_mem_bw_gb_s_str = fmt_qty(h.memory.bandwidth, GB/second, precision=0, commas=False)
...
# │ How: N = logspace(0,5,500); Pr_at_least_one = 1-(1-p)^N; viz.setup_plot().
# │
...
# │ Exports: ap_std_passes_str, ap_n_pgd_steps_str, ap_slowdown_str,
# │          ap_cert_acc_str, ap_gap_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
class AdversarialPayback:
    ...
    ap_slowdown_str = fmt(slowdown, precision=0, commas=False)
    ap_cert_acc_str = fmt_percent(robust_acc, precision=0, commas=False, style='symbol')
    ap_gap_pct_str = fmt(accuracy_gap_pct, precision=0, commas=False)
...
1. **Forward/Backward Passes**: `{python} AdversarialPayback.ap_std_passes_str` (Standard) + `{python} AdversarialPayback.ap_n_pgd_steps_str` (Attack Generation) = `{python} AdversarialPayback.ap_slowdown_str` total passes.
...
***Concept Drift***\index{Concept Drift!definition} is the subtype of distribution shift (see @sec-robust-ai-distribution-shift-concept-drift-55e2) in which the statistical relationship $p(Y \mid X)$ changes over time, meaning the decision boundary itself becomes incorrect rather than merely the input distribution. Its sibling is data drift\index{Data Drift} (see @sec-ml-operations-scale-monitoring-scale-73c5), in which $p(X)$ changes while $p(Y \mid X)$ remains stable.
...
###### Conceptual foundation {#sec-robust-ai-conceptual-foundation-3a58}
...
###### Fast gradient sign method (FGSM) {#sec-robust-ai-fast-gradient-sign-method-fgsm-8258}

The **Fast Gradient Sign Method**\index{Fast Gradient Sign Method}[^fn-fgsm-attack] exemplifies the elegance and danger of gradient-based attacks. FGSM takes the conceptually simple approach of moving in the direction that most rapidly increases the model's prediction error.
...
- $\nabla_x \mathcal{L}(\theta, x, y)$: the gradient showing which input changes most increase prediction error
...
C&W attacks are especially difficult to detect because the perturbations are typically imperceptible to humans, and they often bypass many existing defenses. The attack can be formulated under various norm constraints (for example, $\ell_2$, $\ell_\infty$) depending on the desired properties of the adversarial perturbation.

The **Elastic Net Attack to DNNs (EAD)**\index{Elastic Net Attack to DNNs} incorporates elastic net regularization (a combination of $\ell_1$ and $\ell_2$ penalties) to generate adversarial examples with sparse perturbations. This can lead to minimal and localized changes in the input, which are harder to identify and filter. EAD is particularly useful in settings where perturbations need to be constrained in both magnitude and spatial extent.
...
::: {.column-margin}
![](images/svg/vol2_robust_ai_margin_002.svg){width="100%" fig-alt="Margin illustration showing One surrogate-crafted attack fans out to target models."}
...
In a striking demonstration of physical adversarial examples, researchers fooled an object detector into classifying a Stop sign as a Speed Limit 45 sign using only black and white stickers. Unlike digital perturbations invisible to humans, these "adversarial patches" were robust to changing distances (up to 30 feet), viewing angles, and lighting conditions. The attack achieved high misclassification rates during drive-by tests. This revealed a critical fragility in deep learning systems: high-confidence predictions can be manipulated by altering physical semantics in ways that are obvious to humans but catastrophic for the model's feature extractors.

:::

##### Attack taxonomy overview {#sec-robust-ai-attack-taxonomy-overview-a932}

The point of @Tbl-attack_types is defense selection, not vocabulary. The rows identify what the attacker uses to cross the decision boundary: gradients, optimization, surrogate transfer, or physical sensor manipulation. Each mechanism targets a specific relationship between the model's decision boundaries, the input data, and the attacker's objectives, producing incorrect predictions whose severity depends on the attack's sophistication and the defense measures in place.
...
::: {.column-margin}
![](images/svg/vol2_robust_ai_margin_003.svg){width="100%" fig-alt="Margin illustration showing Poisoning lifecycle: inject, learn, trigger."}
...
In physically deployed systems, attackers can manipulate data at the source by altering the environment captured by a sensor. A self-driving car that sees visual markers on a road sign during deployment is experiencing a physical adversarial attack at inference time. If fleet-learning systems harvest those altered road-sign images and include them in future training, the same sensor manipulation becomes data poisoning because the compromised examples enter the training set.
...
**Answer**: This is a **targeted attack**\index{Targeted Attack!definition}. Unlike availability attacks which aim to degrade overall model performance (often by flipping many labels), targeted poisoning seeks to induce specific errors while maintaining general accuracy to avoid detection.
...
class RobustnessTaxAnalysis:
    ...
    # acc_drop is a difference of two accuracies = percentage POINTS, not a
    # percent ratio; prose supplies "percentage points"/"accuracy points".
    acc_drop_str = fmt(acc_drop, precision=0, commas=False)
```
...
**Result**: Gaining robustness against rare adversarial attacks sacrifices `{python} RobustnessTaxAnalysis.acc_drop_str` percentage points of clean accuracy on normal inputs.
...
::: {#chk-robust-ai-defense-selection .callout-checkpoint title="Defense selection"}

Given your threat model and compute budget, select the optimal defense strategy for the following scenarios:

1.  **Production image classifier (evasion)**: Facing adversarial examples. **Recommendation**: Use adversarial training (Madry) or randomized smoothing, despite the inference latency cost.
2.  **Recommendation system (poisoning)**: Facing injection of fake user profiles. **Recommendation**: Use robust matrix factorization or trim outliers in the training data distribution.
3.  **Fraud detection (distribution shift)**: Facing evolving attack patterns. **Recommendation**: Use continuous monitoring with automated retraining triggers based on KS-test statistics, rather than static robustification.
...
Detecting adversarial examples before they reach the model forms the first line of defense only when the signal reaches a serving decision: reject, transform, route, or audit. Multiple techniques identify and flag suspicious inputs at different points in the inference pipeline.
...
The clean-accuracy cost depends on the threat model and defense. For strong ImageNet-scale adversarial training at $\epsilon = 8/255$, clean accuracy can drop by roughly `{python} RobustnessTaxAnalysis.acc_drop_str` accuracy points, as in the robustness-tax example above. Lighter defenses, smaller perturbation budgets, or randomized smoothing can impose smaller costs, often in the single digits to mid-teens, but the trade-off remains fundamental to the robust optimization objective. Model size often increases with robustness-enhancing architectural modifications such as wider networks or additional normalization layers that improve gradient stability.
...
Once adversarial examples are part of the training loop, deployment must coordinate robustness techniques with MLOps pipelines, monitoring strategies, and distributed training infrastructure that synchronizes updates across multiple nodes.

The remaining strategies spend the defense budget at different layers of the pipeline. Defensive distillation\index{Defensive Distillation} [@papernot2016] trains a second model (the student) to mimic the behavior of the original model (the teacher). The student trains on the soft labels produced by the teacher, which are less sensitive to small perturbations. Because the student learns to generalize better and is less sensitive to adversarial noise, using it for inference reduces the impact of adversarial perturbations.
...
Evaluating adversarial defenses closes the budget loop: the system must measure which attacks the defense actually covers and what performance it sacrifices under controlled attack conditions.
...
Standardized benchmarks provide common ground for evaluation. The MNIST-C [@mu2019mnistc], CIFAR-10-C, and ImageNet-C [@hendrycks2019benchmarking] datasets contain corrupted or perturbed versions of the original datasets for measuring robustness to common corruptions.
...
Anomaly detection is the first supply-chain control point: it asks whether a candidate training example belongs to the distribution that the pipeline intended to learn from. Statistical outlier detection methods identify data points that deviate significantly from the bulk of the data, operating on the assumption that poisoned instances are likely statistical outliers. Techniques such as the Z-score method\index{Z-Score Method!anomaly detection}, Tukey's method, and the Mahalanobis distance\index{Mahalanobis Distance!anomaly detection} measure the deviation of each data point from the dataset's central tendency. Data points that exceed a predefined threshold are flagged as potential outliers and considered suspicious for data poisoning.

Clustering-based methods group similar data points by features or attributes, operating on the assumption that poisoned instances form distinct clusters or lie far from normal data clusters. Algorithms like K-means, DBSCAN, and hierarchical clustering identify anomalous clusters or data points that do not belong to any cluster, treating these instances as potentially poisoned data.

Autoencoders are neural networks trained to reconstruct input data from a compressed representation (@fig-autoencoder). For anomaly detection, an autoencoder learns normal patterns from clean, unpoisoned data. At inference time, the system computes the reconstruction error for each data point. Data points with high reconstruction errors are flagged as abnormal and potentially poisoned because they do not conform to the learned normal patterns.\index{Autoencoder!anomaly detection}
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-robust-ai-robust-training-37d6
# │
...
# │
# │ Imports: mlsysim.fmt (check, fmt)
# │ Exports: normal_error_multiple_str, squared_loss_multiple_str,
# │          residual_gradient_multiple_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt
...
class HuberOutlierScaling:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt
    normal_error_multiple_str = fmt(normal_error_multiple, precision=0, commas=False)
    squared_loss_multiple_str = fmt(squared_loss_multiple, precision=0)
    residual_gradient_multiple_str = fmt(residual_gradient_multiple, precision=0, commas=False)
```
...
[^fn-huber-loss-robust]: **Huber Loss**: This piecewise function transitions from quadratic (MSE) to linear (MAE) at threshold $\delta$ (typically 1.0--1.5), capping gradient magnitude for extreme samples. In poisoned-data settings, Huber loss prevents a small number of malicious samples from generating outsized gradients that would dominate parameter updates---a property standard MSE lacks, since a single outlier with `{python} HuberOutlierScaling.normal_error_multiple_str`$\times$ normal error contributes `{python} HuberOutlierScaling.squared_loss_multiple_str`$\times$ squared-error loss and `{python} HuberOutlierScaling.residual_gradient_multiple_str`$\times$ residual-gradient magnitude. \index{Huber Loss!poisoning defense}
...
::: {.column-margin}
![](images/svg/vol2_robust_ai_margin_004.svg){width="100%" fig-alt="Margin illustration showing MSE curve versus Huber loss curve with gentler tail."}
...
# │ Exports: RobustnessTaxRecap.acc_drop_str
...
* **Robustness vs. accuracy**: Building resilient systems\index{Robustness!accuracy trade-off} requires an explicit trade-off. Strong ImageNet-scale adversarial training can cost roughly `{python} RobustnessTaxRecap.acc_drop_str` accuracy points of clean accuracy, while lighter threat models or randomized smoothing may cost single-digit to mid-teen points; compute costs still rise by 2--10$\times$ due to iterative attack generation or Monte Carlo sampling.
...
We have made our systems robust against the operational chaos of distribution drift, adversarial perturbation, and software faults. Yet a system that survives every failure while consuming a city's worth of power is not a viable long-term solution.
