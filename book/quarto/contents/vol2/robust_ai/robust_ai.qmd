...
::: {.column-margin}
![](images/svg/robust_ai_robustness_tax.svg){width="100%" fig-alt="Two columns comparing clean ImageNet accuracy. The standard model column (gray) reaches 76 percent; the adversarially trained model column (orange) reaches only 50 percent, a 26-point robustness tax."}
...
Across cloud, edge, and embedded environments, the same pattern recurs: ML systems fail silently, confidently, and catastrophically. These failures demonstrate why fault-tolerant design, rigorous testing, and robust system architectures are nonnegotiable for reliable operation.
...
# │ How: .m_as() from mlsysim.core.constants plus bit-rate arithmetic.
# │
# │ Imports: mlsysim.core.constants (*), mlsysim.fmt (*)
# │ Exports: gpt3_params_b_str, v100_mem_bw_str, bits_per_second_math,
# │          base_error_rate_math, expected_errors_per_hour_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_math, sci_latex
...
class RobustAISetup:
    ...
    base_error_rate = Monitoring.MemoryBitErrorRatePerBit

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    gpt3_params_b_val = m.parameters.m_as(param) / BILLION
    v100_bw_gbs_val = h.memory.bandwidth.m_as(GB/second)
    bits_per_second_val = h.memory.bandwidth.m_as(byte/second) * BITS_PER_BYTE
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt, fmt_math, sci_latex
    gpt3_params_b_str = fmt(gpt3_params_b_val, precision=0, commas=False)
    v100_mem_bw_str = fmt(v100_bw_gbs_val, precision=0, commas=False, suffix=' GB/s')
...
Robust AI systems must address three categories of challenges: environmental shifts, input-level attacks, and system-level faults. Software faults are a cross-cutting concern that can amplify or masquerade as any of the three. The **Three Pillars Framework**\index{Three Pillars Framework} in @fig-three-pillars-framework organizes these threats, each representing distinct but interconnected vulnerabilities that require complementary defense strategies:
...
class DistributionShiftMetrics:
    ...
    psi_warn = Monitoring.PsiWarnThreshold
    psi_critical = Monitoring.PsiCriticalThreshold
    ...
    mmd_latency_ms_str = fmt(mmd_latency_ms, precision=0, commas=False, suffix=' ms')
...
thresh_warning = Monitoring.PsiWarnThreshold
thresh_critical = Monitoring.PsiCriticalThreshold
...
@Tbl-attack_types summarizes the different categories of adversarial attacks: gradient-based attacks (FGSM, PGD, JSMA), optimization-based attacks (C&W, DeepFool [@moosavi-dezfooli2016], EAD), transfer-based attacks, and physical-world attacks (adversarial patches and objects). Each attack type exploits different model vulnerabilities and requires distinct defensive considerations.
...
The most direct defense against adversarial attacks is **adversarial training**\index{Adversarial Training}, which incorporates adversarial examples into the training process itself.
...
class RobustnessTaxAnalysis:
    ...
    # percent ratio; prose supplies "percentage points"/"accuracy points", so the
    # string is a bare number (matches RobustnessTaxRecap.acc_drop_str).
