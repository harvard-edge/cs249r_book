...
These principles extend beyond fault recovery to encompass systematic performance adaptation strategies that appear throughout ML system design. As @fig-robustness-taxonomy illustrates, detection strategies form the foundation for monitoring systems, graceful degradation guides fallback mechanisms when components fail, and adaptive response enables systems to evolve with changing conditions.
...
Imagine a hedge fund training a sentiment analysis model on financial tweets. If a rival firm coordinates a network of bots to systematically associate the word "growth" with negative sentiment during the training window, a newly deployed trading algorithm will aggressively short stocks on positive earnings reports. As @fig-adversarial-attack-injection illustrates, data poisoning attacks target the supply chain of machine learning, manipulating the raw material of intelligence before the model even begins to learn.
