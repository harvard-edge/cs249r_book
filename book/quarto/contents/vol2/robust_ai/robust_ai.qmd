...
- Explain how software faults can amplify or masquerade as all three categories using quantitative reliability metrics
...
The fleet stack clarifies why robustness is an infrastructure concern: silent failures propagate through every layer of the system, from input perturbations and distribution drift at the data layer to degraded predictions at the service level [@berkeley2024compound].

::: {#psp-robust-ai-fleet-stack-connection .callout-perspective title="Fleet stack connection"}

Robust AI sits in the Governance Layer of the fleet stack. Security and privacy (@sec-security-privacy) address malicious *external* threats; robustness addresses *operational* threats: distribution drift, adversarial perturbation, and software faults. A system that is secure but fragile is operationally useless; robustness engineering ensures that the fleet continues to function under perturbation and degraded conditions.
...
Despite these contextual differences, a robust ML system requires fault tolerance, error resilience, and sustained performance across all deployment environments, and those guarantees are not free. Error correction mechanisms consume 12--25 percent additional memory bandwidth, redundant processing increases energy consumption by 2--3$\times$, and continuous monitoring adds 5--15 percent computational overhead. These robustness measures also generate additional heat and exacerbate thermal management challenges that constrain deployment density and require enhanced cooling infrastructure. The robustness question is where the additional resource cost provides enough reliability value to justify itself.
...
Silent data corruption propagates across multiple layers of the application stack, resulting in data loss and application failures in large-scale distributed systems. In ML training, where gradient updates and parameter synchronization are critical, SDC can compromise model accuracy without triggering any alert. Meta's report and Dean's 2024 keynote suggest that SDC is not isolated to one provider. Jeff Dean, Chief Scientist at Google DeepMind and Google Research, highlighted these issues in AI hypercomputers[^fn-ai-hypercomputer-scale] during a keynote at MLSys 2024 [@dean2024mlsys] (@fig-sdc-robust-jeffdean).

[^fn-ai-hypercomputer-scale]: **AI Hypercomputer**\index{AI Hypercomputer!reliability at scale}: Purpose-built clusters of thousands of accelerators with high-bandwidth interconnects. As 2021--2024 cluster examples, Google's TPU v4 pods pack 4,096 chips delivering 1.1 exaFLOP/s, while NVIDIA DGX SuperPOD configurations scale to thousands of H100s. At these scales, the probability of at least one silent fault per hour approaches certainty (see @fig-silent-error-probability), transforming reliability from a component-level concern into an architectural constraint that shapes recovery, monitoring, and capacity planning.
...
In 1999, NASA's Mars Polar Lander\index{Mars Polar Lander!software failure} mission was lost after a likely software fault in its touchdown detection system [@nasa2000mpl]. The review board identified premature touchdown detection during landing-leg deployment as the most probable failure mode, which would have shut off the engines before the lander reached the surface. For remote missions where recovery is impossible, rigorous software validation and robust system design are prerequisites, not luxuries. As AI becomes more integral to space systems (@fig-nasa-example), the same rigor applies to any ML component in the decision loop.
...
When AI is applied in aviation, including tasks such as autonomous flight control and predictive maintenance, the robustness of embedded systems affects passenger safety.
...
# │
# │ Imports: mlsysim.core.units (THOUSAND)
...
```
The scale of large GPU clusters transforms per-device error rates into fleet-level events. @Fig-silent-error-probability illustrates this compounding effect: for a cluster of $N$ devices each with per-device silent data corruption probability $p$ per hour, the probability of at least one SDC event is $\Pr(\geq 1) = 1 - (1 - p)^N$. Meta's SDC report establishes that silent corruption occurs at observable fleet scale [@dixit2021silent]; the curve below uses an illustrative stress-test rate of $p$ = `{python} SilentErrorProbability.p_per_hr_math` per device-hour to show the compounding math. At that rate, a `{python} SilentErrorProbability.n_gpus_large_str`-device cluster is more likely than not to see an hourly silent error (`{python} SilentErrorProbability.p_total_large_str` probability), and the probability crosses 95 percent at about `{python} SilentErrorProbability.n_gpus_certain_str` devices.
...
- [ ] **Pillar classification**: Can you place each of these in the right pillar: a model losing accuracy as customer behavior evolves, an input crafted to force a misclassification, and a bit flip in an accelerator's HBM?
- [ ] **Software-fault mimicry**: A preprocessing bug suddenly shifts the feature statistics of every inference. Why is this hard to distinguish from genuine environmental shift, and which pillar actually owns it?

**Choosing the response**

- [ ] **Response selection**: Given the shared detection, graceful-degradation, and adaptive-response pattern, which response does an ECC-corrected single-bit memory error exercise, and which does a fired drift monitor exercise?
- [ ] **Fleet-scale effects**: At a fleet of tens of thousands of accelerators, why does a per-device silent-corruption rate that is negligible per device become an architectural constraint rather than a component-level concern?
...
# │
# │ Imports: mlsysim.core.units (THOUSAND, PSI thresholds)
...
Federated learning enables distributed adaptation across multiple deployment environments while preserving privacy. Representative FL deployments can involve 50--1,000 participants and 10--50 communication rounds, with each round requiring 10--100&nbsp;MB of parameter transmission depending on model size. Local training may require 5--20 epochs per round, with communication costs dominating when bandwidth falls below 1 Mbps. Differential privacy ($\epsilon = 1.0$--$8.0$) adds noise, and its utility cost must be measured for the application rather than assumed away.
...
The Population Stability Index quantifies how much a distribution has shifted between two time periods. It was originally developed for credit scoring validation and is widely applied in ML model monitoring [@yurdakul2020]. PSI measures the divergence between an expected (baseline) distribution $p_{\text{base}}$ and an actual (current) distribution $p_{\text{curr}}$ by computing a symmetric log-ratio difference across discretized bins.
...
- [ ] **Shift taxonomy**: Can you tell covariate shift, label shift, and concept drift apart by which probability the change touches, and explain why feature-statistics monitoring stays silent under pure concept drift?
- [ ] **Spurious correlation**: A cow-on-a-beach misclassification is not the world changing but the model having learned the wrong lesson. What is this failure called, and why does standard empirical risk minimization encourage it?

**Reasoning about thresholds and cost**

- [ ] **Threshold judgment**: A high-importance feature shows a population stability index in the major-shift band while precision and recall stay flat. Using the three-level framework, do you retrain, investigate, or keep monitoring, and why?
- [ ] **Cost asymmetry**: Why is concept drift structurally more expensive to remediate than data drift, even when the same volume of fresh data is available?
...
2.  **Observation**: Unlike Random Noise (which the model can learn to ignore), Adversarial Perturbations are Gradient-Directed\index{Adversarial Perturbation!gradient-directed}: they are specifically optimized to maximize the model's prediction error.
3.  **Pitfall**: A frequent misconception is that adversarial vulnerability is a "bug" to be patched. In reality, it is a structural vulnerability of many standard neural networks trained by empirical risk minimization; robust defense typically requires fundamental changes to the objective function (for example, adversarial training).
...
[^fn-white-box-robustness]: **White-Box Attack**\index{White-Box Attack!robustness bound}: Adversarial attack with complete model knowledge (architecture, weights, gradients), achieving near-100 percent success rates with methods like PGD and C&W. Though less realistic than black-box scenarios for deployed systems, white-box analysis establishes the worst-case robustness bound---the ceiling against which all practical defenses are measured. If a model survives PGD, it provides a quantitative floor for robustness under any weaker threat model.
...
[^fn-transferability-blackbox]: **Transferability**\index{Transferability!black-box attack}: The property, analyzed in @papernot2016transferability, that adversarial examples crafted for one model fool different architectures with 30--70 percent success rates. This transforms the threat model for deployed ML systems: attackers need not query the target model at all---they can train a substitute locally, craft attacks offline, and transfer them to production APIs. Ensemble adversarial training (training against perturbations from multiple surrogate models) partially mitigates transfer attacks at 3--5$\times$ the training cost.
...
**Mechanism**: Unlike digital perturbations invisible to humans, these adversarial patches were robust to changing distances (up to 30 feet), viewing angles, and lighting conditions. The attack achieved high misclassification rates during drive-by tests.
...
: **Adversarial Attack Categories**: Input perturbation attacks categorized by mechanism. {#tbl-attack_types tbl-colwidths="[15,18,67]"}

The defense cannot be chosen generically. Adversarial training raises the cost of gradient-based attacks, input transformation disrupts some small perturbations before inference, ensembles make transfer less reliable, and physical evaluation exposes failures that digital tests miss. The reason this defense budget matters is that adversarial attacks extend far beyond simple misclassification (@fig-adversarial-googlenet). These vulnerabilities create systemic risks across deployment domains.
...
Microsoft's Tay chatbot marks the transition from inference-time perturbations to poisoning through interaction. This was not a norm-bounded image perturbation; it was an online learning loop in which adversarial users shaped the system's future behavior. Within 24 hours of launch, coordinated users manipulated Tay's learning mechanisms to generate inappropriate and offensive content. The system lacked content filtering, user input validation, and behavioral monitoring, any one of which could have detected and prevented the exploitation. Systems that learn from user interactions require input validation, content filtering, and continuous behavioral monitoring as baseline safeguards.
...
2.  **Observation**: Unlike Adversarial Attacks (which occur at Inference Time), Data Poisoning occurs during Data Collection or Training, contaminating the model's learned mapping from the source.
3.  **Pitfall**: A frequent misconception is that poisoning can be "fixed" by more data. In reality, poisoning often exploits the Aggregation Property of training: adding more clean data may not "wash out" a carefully targeted backdoor that uses a unique trigger.
...
First formalized by @biggio2012poisoning[^fn-data-poisoning-attack], poisoning attacks\index{Data Poisoning!examples} alter existing training samples, introduce malicious examples, or interfere with the data collection pipeline (@fig-dirty-label-example) [see also @shan2024]. The consequences are especially severe in high-stakes domains like healthcare, where even small disruptions to training data can lead to dangerous misdiagnoses [@marulli2022].

[^fn-data-poisoning-attack]: **Data Poisoning**\index{Data Poisoning!training attack}: First formalized by @biggio2012poisoning, poisoning attacks inject malicious samples into training data to corrupt the learning process itself. Unlike adversarial examples that target inference (and can be mitigated at serving time), poisoning embeds vulnerabilities into model weights during training---making detection require auditing billions of training samples rather than filtering individual inference requests. At web-scale data collection, even 0.01 percent poisoned data can shift decision boundaries.
...
[^fn-backdoor-trigger-ml]: **Backdoor Attack**\index{Backdoor Attack!detection asymmetry}: Introduced by @gu2017badnets, backdoor attacks embed hidden triggers in training data that activate malicious behavior at inference when a specific pattern appears. Success rates exceed 99 percent while maintaining normal accuracy on clean inputs, making standard evaluation metrics useless for detection. The defense challenge is asymmetric: the attacker needs only a small trigger patch, while the defender must audit the entire training dataset or inspect high-dimensional activation space for spectral anomalies.
...
def adversarial_training_step(model, data, labels, epsilon=0.1):
    ...
    attack_data = data.detach().clone().requires_grad_(True)
    attack_logits = model(attack_data)
    attack_loss = F.cross_entropy(attack_logits, labels)
    input_grad = torch.autograd.grad(attack_loss, attack_data)[0]

    # Create adversarial perturbation and mix with clean data
    adv_data = attack_data + epsilon * input_grad.sign()
...
The implementation in @lst-adversarial-training generates adversarial examples on-the-fly during training by computing gradients with respect to a detached input clone, applying the sign function to extract the perturbation direction, and mixing the resulting adversarial examples with clean training data through `torch.cat`. Using `torch.autograd.grad` for the attack step avoids accumulating parameter gradients before the final training loss is computed. The `torch.clamp()` operation ensures pixel values remain valid, while the final concatenation doubles the effective batch size by combining clean and adversarial examples. This approach requires careful tuning of the perturbation budget $\epsilon$; optimized variants can reduce adversarial-training overhead by reusing gradient computations [@shafahi2019adversarial].
...
- [ ] **Worst-case evaluation**: Can you explain why a model that passes a white-box PGD test makes a worst-case robustness claim that a model evaluated only on saved digital images cannot?
- [ ] **Threat-model fit**: An API-facing classifier and a safety-critical road-sign detector face different attacker access. Which defense emphasis does each one need, and why does adversarial training alone not transfer between them?

**Reasoning about the budget**

- [ ] **Certified vs. empirical**: Certified randomized smoothing and empirical adversarial training both buy robustness. Why does the certified guarantee force smoothing into asynchronous auditing rather than the real-time serving path?
- [ ] **Defense budget**: For a low-risk component, why is input detection often the rational choice over building intrinsic robustness into the model weights, given the clean-accuracy tax adversarial training imposes?
...
* **Robustness vs. accuracy**\index{Robustness!accuracy trade-off}: Building resilient systems requires an explicit trade-off. Strong ImageNet-scale adversarial training can cost roughly `{python} RobustnessTaxRecap.acc_drop_pp_str` of clean accuracy, while lighter threat models or randomized smoothing may cost single-digit to mid-teen points; compute costs still rise by 2--10$\times$ due to iterative attack generation or Monte Carlo sampling.
* **Semantic robustness in LLMs**: Generative AI introduces a failure mode beyond misclassification: "hallucinations"\index{Robustness!LLM hallucination} where the model produces fluent but factually wrong output. Robustness at this level requires uncertainty quantification (UQ), using entropy and self-consistency to detect when a model is guessing rather than producing grounded text.
* **The drift detection arsenal**: Environmental shift is inevitable. Robust monitoring pipelines can use statistical distance metrics (MMD, PSI) to detect when the real world has diverged from the training assumptions, then route the finding to retraining, rollback, review, or additional evaluation before users report failures.
...
[^fn-lipschitz-robustness]: **Lipschitz Continuity**\index{Lipschitz Continuity!robustness}: A mathematical property that bounds how much a function's output changes relative to its input change ($\\|f(x) - f(x')\\| \\le K \\|x - x'\\|$). In robust AI, minimizing the Lipschitz Constant $(K)$ ensures the model's decision surface is "smooth" rather than "jagged," making it physically impossible for small adversarial perturbations to flip the model's prediction.
