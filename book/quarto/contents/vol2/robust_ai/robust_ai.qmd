...
This chapter has examined robustness across hardware faults, input-level attacks, environmental shifts, and software vulnerabilities. Each threat domain introduces misconceptions that can lead to inadequate defenses or misallocated engineering resources. The following fallacies and pitfalls, organized by theme, represent common errors that undermine robust system design.

### Foundational Misconceptions

**Fallacy: Security equals Robustness.**

A system can be perfectly secure (immune to unauthorized access) and adversarially robust (immune to PGD attacks) yet still be brittle. Security focuses on *malicious* agents; robustness focuses on *reliability* under uncertainty. A self-driving car might be unhackable but still crash because it was not robust to a new type of weather condition (natural distribution shift). Robustness requires preparing for the accidental and the random, not just the malicious.
