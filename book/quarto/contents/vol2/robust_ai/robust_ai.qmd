...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  % Corrupted Value
  \node[anchor=west, text=RedLine, below=0.4cm of CleanVal.west, anchor=west] (CorruptVal) {$\approx 1.2 \times 10^{19}$};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.7]
  ...
  \begin{scope}[shift={(6,0)}]
    ...
    \node[fill=BlueL, minimum width=1cm, minimum height=2cm, anchor=south west, inner sep=0pt] at (0.5,0) {};
    \node[fill=BlueL, minimum width=1cm, minimum height=1cm, anchor=south west, inner sep=0pt] at (2.5,0) {};
    \node[fill=RedL, opacity=0.5, minimum width=1cm, minimum height=1cm, anchor=south west, inner sep=0pt] at (0.5,0) {};
    \node[fill=RedL, opacity=0.5, minimum width=1cm, minimum height=2cm, anchor=south west, inner sep=0pt] at (2.5,0) {};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \draw[line width=1.0pt, BlueLine] plot [smooth, tension=1] coordinates {(-1,4) (2,2) (5,0)};
  \node[BlueLine, font=\scriptsize] at (4, 1.5) {Decision Boundary};
  ...
  % Clean Point
  \node[circle, fill=GreenL, inner sep=2pt, label={below:Training $x$}] (X) at (1, 1.5) {};

  % Shifted Point
  \node[circle, fill=RedL, inner sep=2pt, label={above:Shifted $x'$}] (Xadv) at (2.5, 2.5) {};

  % Shift Vector
  \draw[->, line width=1.0pt, RedLine] (X) -- (Xadv) node[midway, sloped, above, font=\tiny] {Distribution shift $\Delta$};

  % Distance constraint
  \draw[dashed, BrownLine] (X) circle (2.2);
  \node[BrownLine, font=\tiny] at (1, -0.8) {Shift magnitude $|\|\Delta|\|$};
...
\node[ellipsePE,fill=OrangeL](\picname-EL1)at(0,0.44){};
...
filllcolor=GreenL, Linewidth=0.75pt}};
}
...
\scoped[on background layer]
\node[draw=BlueLine,inner xsep=3mm,inner ysep=2mm,
fill=BlueL,fit=(BRAIN1)(7-CH1)(5-CH3),line width=0.75pt](BB1){};
\node[above=2ptof BB1,BlueLine]{Normal learning};
%
...
\begin{scope}[local bounding box=BRAIN1,shift={($(0,0)+(0,0)$)},
scale=1, every node/.append style={transform shape}]
\pic[shift={(0,0)}] at  (0,0){brain={scalefac=0.7,picname=1,filllcolor=BrownL, Linewidth=0.75pt,
  filllcirclecolor=BrownL,drawcircle=BrownLine}};
\end{scope}
%skull
...
\scoped[on background layer]
\node[draw=RedLine,inner xsep=3mm,inner ysep=2mm,
fill=RedL,fit=(BRAIN1)(7-CH1)(5-CH3),line width=0.75pt](BB1){};
\node[above=2ptof BB1,RedLine]{Poisoning attack};
\end{scope}
\end{tikzpicture}
...
\foreach \x in {1,...,\columns}{
    \foreach \y in {1,...,\rows}{
        %
        \node[draw=BrownLine, fill=GreenL, minimum width=\cellsize,
                    minimum height=\cellheight, line width=0.75pt] (cell-\x-\y\br) at (\x*\cellsize,-\y*\cellheight) {};
    }
}
%
\foreach \color [count=\x] in \rowone {
    \node[fill=\color,draw=BrownLine,line width=0.75pt, minimum size=\cellsize,
...
\end{scope}
%Poisoned Model
\begin{scope}[local bounding box=POI,line width=0.75pt,
...
\draw[fill=OrangeL] (D2) -- (E2) -- (F2) -- (G2) -- cycle;% Right Face
\draw[fill=OrangeL] (C2) -- (B2) -- (F2) -- (G2) -- (C2);% Front Face
\draw[fill=OrangeL] (A2) -- (B2) -- (F2) -- (E2) -- cycle;% Top Face
%
\node[align=center]at($(B2)!0.5!(G2)$){Poisoned\\ Model};
\end{scope}
%arrows
\draw[doubleL,BlueLine]($(DEF)+(0.7,0)$)coordinate(LE)-|coordinate(DE)($(POI)+(0,-1.2)$);
\draw[doubleL,BlueLine]($(DEF)+(0,0.7)$)--
...
\draw[doubleL,VioletLine]($(ATA)+(0.7,0)$)--
node[below=5pt,align=center,text=black]{Malicious data\\ injection}
++(0:3.3);
\draw[doubleL,VioletLine]($(cell-6-6A)!0.7!(cell-6-7A)+(0.6,0)$)--
...
\foreach \x in {1,...,\columns}{
    \foreach \y in {1,...,\rows}{
        %
        \node[draw=BrownLine, fill=VioletL, minimum width=\cellsize,
                    minimum height=\cellheight, line width=0.75pt] (cell-\x-\y\br) at (\x*\cellsize,-\y*\cellheight) {};
    }
}
\end{scope}
...
\foreach \x in {1,...,\columns}{
    \foreach \y in {1,...,\rows}{
        %
        \node[draw=BrownLine, fill=OrangeL, minimum width=\cellsize,
                    minimum height=\cellheight, line width=0.75pt] (cell-\x-\y\br) at (\x*\cellsize,-\y*\cellheight) {};
    }
}
\end{scope}
...
\foreach \x in {1,...,\columns}{
    \foreach \y in {1,...,\rows}{
        %
        \node[draw=BrownLine, fill=BlueL, minimum width=\cellsize,
                    minimum height=\cellheight, line width=0.75pt] (cell-\x-\y\br) at (\x*\cellsize,-\y*\cellheight) {};
    }
}
\end{scope}
...
%
\draw[RedLine,line width=1.0pt,decoration={brace,amplitude=9pt,mirror},decorate](L1)--
node[below=9pt]{Encoder}(L2);
\draw[RedLine,line width=1.0pt,decoration={brace,amplitude=9pt},decorate](D1)--
