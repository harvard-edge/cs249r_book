...
***Robust AI***\index{Robust AI!definition} is the engineering discipline of ensuring that machine learning systems maintain _performance_ and _reliability_ despite _system errors_, _malicious inputs_, and _environmental changes_ through systematic _fault detection_, _mitigation_, and _recovery_.
...
```{python}
#| label: fig-silent-error-probability
#| fig-cap: "**Silent Error Probability at Scale**. Probability of at least one silent data corruption event per hour as a function of cluster size, for three per-device error rates. At the rates reported by Meta (Dixit et al., 2021), which are orders of magnitude above traditional soft-error models, silent errors become effectively certain at cluster scales beyond a few thousand devices."
#| fig-alt: "Semilog plot showing three S-curves for per-device SDC rates of 1e-3, 1e-4, and 1e-5. All curves reach probability 1.0 as cluster size grows to 100000 GPUs."
#| echo: false
import numpy as np
...
::: {#fig-robustness-taxonomy fig-env="figure" fig-pos="htb" fig-cap="The Robustness Taxonomy. The three pillars of ML reliability: defending against attacks, ensuring data integrity, and handling distribution shifts."}
```{=latex}
\begin{tikzpicture}
    \usefont{T1}{phv}{m}{n}
    ...
    \node[BlueL, font=\bfseries] at (1.5, 3.5) {Adversarial\\Manipulation};
    ...
    \node[RedL, font=\bfseries] at (5.5, 3.5) {Data\\Poisoning};
    ...
    \node[GreenL, font=\bfseries] at (9.5, 3.5) {Distribution\\Shift};
...
***Concept Drift***\index{Concept Drift!definition} is the temporal change in the statistical relationship between input features and target labels ($P(Y|X)$ shifts over time), causing a model trained on historical data to degrade on current inputs. Unlike **data drift** (where $P(X)$ changes but $P(Y|X)$ remains stable), concept drift invalidates the learned mapping itself, requiring model retraining rather than input adaptation.
...
```{python}
#| label: fig-distribution-shift-detector
#| fig-cap: "The Distribution Shift Detector: Monitoring Population Stability Index (PSI) over a year. The model remains stable (Green Zone) until Week 20, drifts into the warning zone (Orange) due to evolving user behavior, and breaches the critical threshold (Red Zone) at Week 35. Automated retraining at Week 40 restores model stability."
import matplotlib.pyplot as plt
import numpy as np
...
***Adversarial Attack***\index{Adversarial Attack!definition} is a deliberate, mathematically crafted perturbation to model inputs designed to cause misclassification while remaining imperceptible to humans. Adversarial vulnerability is an inherent property of high-dimensional decision boundaries in neural networks, not a bug to be patched; the perturbation magnitude required for misclassification scales inversely with input dimensionality.
...
***Data Poisoning***\index{Data Poisoning!definition} is the corruption of training data to compromise model behavior at inference time, either by injecting malicious samples or modifying existing labels. Defending against poisoning requires **data validation** overhead that scales with dataset size, and no validation scheme can guarantee detection of all poisoning strategies without also rejecting legitimate edge cases.
