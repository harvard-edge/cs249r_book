...
The attacker's available information and perturbation budget determine which category of adversarial attack applies. The categories are not just vocabulary; each one tells the defender what the attacker can measure and what the defense must make expensive.
...
Here $x$ is the original input, $x_{\text{adv}}$ is the adversarial example, $\mathcal{L}(\theta, x, y)$ is the prediction loss, and $\nabla_x \mathcal{L}$ identifies the input changes that most increase that loss. The sign operation keeps only the direction of steepest ascent, while $\epsilon$ controls how much perturbation the attacker is allowed to add.
...
Source controls close the loop by reducing the probability that poisoned examples enter the training pipeline at all. Clear collection protocols, source-authenticity checks, and recurring quality assessments narrow the set of inputs a model is allowed to learn from.
