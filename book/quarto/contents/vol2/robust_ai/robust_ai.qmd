...
**Context**: In June 2015, Google Photos launched automatic image tagging across billions of user photos, serving $D_{\text{vol}} > 10^9$ images under real-time serving service level agreements (SLAs) ($L_{\text{lat}} \le 100\text{ ms}$) [@kasperkevic2015googlephotos].
...
@Sec-ml-operations-scale-drift-detection-793f introduced PSI as a cheap fleet-wide alerting signal, with its credit-scoring origin and the standard threshold bands. The full statistical machinery behind that signal determines how PSI is computed, what binning and smoothing choices govern its sensitivity, and how it combines with KL divergence and significance tests into a retraining decision. PSI measures the divergence between an expected (baseline) distribution $p_{\text{base}}$ and an actual (current) distribution $p_{\text{curr}}$ by computing a symmetric log-ratio difference across discretized bins.
