...
<!-- TODO(focused-followup): Items deferred from the codex deep-review
     pending-discussions walkthrough:

     - @fig-poisoning caption-vs-prose contradiction (line ~1866):
       caption describes online-learning incremental poisoning; prose
       at lines 1860--1864 and the figure alt text describe Nightshade-
       style concept poisoning of Stable Diffusion. Revise the caption
       to match the rendered figure (concept poisoning, dose-response
       grid).
     - Stale listing line references (line ~2030): prose cites
       implementation lines 2190, 2196, and 2199--2200 but the actual
       listing renders at lines 2008--2025. Either remove hard-coded
       source line numbers or update them to match the rendered
       listing.
     - Attack-taxonomy table single-cell concatenation (line ~1283):
       cells contain `FGSM PGD JSMA C&W` without separators; split
       into separate rows or use semicolon-delimited cells with
       explicit definitions.
     - Notebook callouts at lines 627, 1936, 1945 use second-person
       and rhetorical questions: convert similarly to the fix at
       line 618 (already fixed).
     - Informal figure-caption sources at lines 1225, 2056, 2209
       (`ivezic`, `li`, `dertat`, uppercase `HTTPS://`); replace with
       proper .bib citations or normalize source-attribution prose.
     - Uncited quantitative claims at lines 728, 744, 746 (nighttime
       degradation, weather mAP drops, fraud drift rates); cite or
       soften.
     - Spaced `---` triple-hyphens at lines 83 and 97; replace with
       em-dash or recast as full sentences. -->
...
::: {#fig-gradient-attack fig-env="figure" fig-pos="htb" fig-cap="**Adversarial Perturbations**: Gradient-based attacks generate subtle, intentionally crafted input noise with magnitude controlled by $\\epsilon$ that maximizes the loss function $\\mathcal{L}(\\theta, x, y)$ and causes misclassification by the model. These perturbations, imperceptible to humans, exploit model vulnerabilities by moving the input $x$ across the decision boundary. Source: [ivezic](HTTPS://defence.AI/AI-security/gradient-based-attacks/)." fig-alt="Diagram showing FGSM attack process: original input x, gradient computation, epsilon-scaled perturbation, and resulting adversarial example crossing decision boundary."}
...
**Systems conclusion**: To gain robustness against rare adversarial attacks, you sacrifice **`{python} RobustnessTaxAnalysis.acc_drop_str` percent accuracy** on normal inputs.
