...
# │ Show: 10K GPUs at Meta rates (1e-4/hr) gives P>0.6; ~30K gives P>0.95.
# │ How: P(at least one) = 1 - (1-p)^N.
# │
# │ Imports: mlsysim.core.constants (THOUSAND, MILLION)
# │ Exports: p_rate_meta_str, n_gpus_large_str, p_total_large_str,
# │          n_gpus_certain_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Goal: Plot P≥1 SDC) = 1-(1-p)^N vs N for p=1e-3, 1e-4, 1e-5.
...
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
# │ Plot P≥1 SDC) = 1-(1-p)^N vs N for p=1e-3, 1e-4, 1e-5.
import numpy as np
...
**Problem**: A model monitors a critical input feature. The baseline mean was `{python} ShiftConfidence.baseline_mean`. Over the last `{python} ShiftConfidence.ds_n_samples_str` requests, the mean has shifted to `{python} ShiftConfidence.current_mean`. The engineering question is whether this is a random fluctuation or a real distribution shift\index{Distribution Shift}.
...
3. **Statistical Significance**\index{Statistical Significance}: The shift is **approximately 5.3 standard errors** away from the mean.
...
The Population Stability Index quantifies how much a distribution has shifted between two time periods, originally developed for credit scoring validation and now widely applied in ML model monitoring [@yurdakul2020]. PSI measures the divergence between an expected (baseline) distribution $P$ and an actual (current) distribution $Q$ by computing the symmetric difference in log-odds across discretized bins.
...
@tbl-robust-ai-psi-thresholds collects the industry-standard PSI ranges and the recommended monitoring action at each tier:
...
: **PSI Interpretation Thresholds**: Industry-standard Population Stability Index ranges and the recommended monitoring action at each tier. Values above 0.25 reliably signal distribution shifts large enough to warrant retraining. {#tbl-robust-ai-psi-thresholds}

These thresholds derive from empirical studies showing that $\text{PSI} > 0.2$ correlates with statistically significant performance degradation $(p < 0.05)$ in most production systems [@yurdakul2020].
...
ax.axhspan(thresh_warning, thresh_critical, color='orange', alpha=0.1, label='Warning Zone (0.1 <= PSI < 0.25)')
...
| **$0.05 \le D_{\text{KL}} < 0.1$** | Moderate divergence    |
| **$D_{\text{KL}} \ge 0.1$**        | Significant divergence |
...
- $D_{\text{KL}} > 0.05$ on continuous features
...
::: {#fig-gradient-attack fig-env="figure" fig-pos="htb" fig-cap="**Adversarial Perturbations**: Gradient-based attacks generate subtle, intentionally crafted input noise with magnitude controlled by ε that maximizes the loss function 𝓛(θ, x, y) and causes misclassification by the model. These perturbations, imperceptible to humans, exploit model vulnerabilities by moving the input x across the decision boundary. Source: adapted from a gradient-based-attacks tutorial at defence.AI." fig-alt="Diagram showing FGSM attack process: original input x, gradient computation, epsilon-scaled perturbation, and resulting adversarial example crossing decision boundary."}
...
The Elastic Net Attack to DNNs (EAD) incorporates elastic net regularization (a combination of $\ell_1$ and $\ell_2$ penalties) to generate adversarial examples with sparse perturbations. This can lead to minimal and localized changes in the input, which are harder to identify and filter. EAD is particularly useful in settings where perturbations need to be constrained in both magnitude and spatial extent.
...
|:-----------------------|:------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Gradient-based**     | • Fast Gradient Sign Method (FGSM)<br>• Projected Gradient Descent (PGD)<br>• Jacobian-based Saliency Map Attack (JSMA) | • Perturbs input data by adding small noise in the gradient direction to maximize prediction error.<br>• Extends FGSM by iteratively applying the gradient update step for more refined adversarial examples.<br>• Identifies influential input features and perturbs them to create adversarial examples.       |
| **Optimization-based** | • Carlini and Wagner (C&W) Attack<br>• DeepFool<br>• Elastic Net Attack to DNNs (EAD)                                   | • Finds the smallest perturbation that causes misclassification while maintaining perceptual similarity.<br>• Iteratively computes minimal perturbation to cross decision boundary using linearization.<br>• Incorporates elastic net regularization to generate adversarial examples with sparse perturbations. |
...
| **Physical-world**     | • Adversarial Patches<br>• Adversarial Objects                                                                          | • Small, carefully designed patches placed on objects to fool object detection or classification models.<br>• Physical objects (for example, 3D-printed sculptures, modified road signs) crafted to deceive ML systems in real-world scenarios.                                                                  |
...
Defending against adversarial attacks often requires additional computational resources and can impact the overall system performance. Techniques like adversarial training, where models are trained on adversarial examples to improve robustness, can significantly increase training time and computational requirements [@bai2021]. Runtime detection and mitigation mechanisms, such as input preprocessing [@addepalli2020] or prediction consistency checks, introduce latency and affect the real-time performance of ML systems.
...
**Systems conclusion**: Gaining robustness against rare adversarial attacks sacrifices **`{python} RobustnessTaxAnalysis.acc_drop_str` percentage points of accuracy** on normal inputs.

*   **Why?** The model must learn to ignore "nonrobust features" (like high-frequency textures) that are predictive but brittle.
...
The implementation in @lst-adversarial-training generates adversarial examples on-the-fly during training by computing gradients with respect to input data via `requires_grad_` and the standard backward pass, applying the sign function to extract perturbation direction (`data.grad.sign()`), and mixing the resulting adversarial examples with clean training data through `torch.cat`. The `torch.clamp()` operation ensures pixel values remain valid, while the final concatenation doubles the effective batch size by combining clean and adversarial examples. This approach requires careful tuning of the perturbation budget $\epsilon$ and typically increases training time by 2--3$\times$ compared to standard training [@shafahi2019adversarial].
...
Standardized benchmarks provide common ground for evaluation. The MNIST-C [@mu2019mnistc], CIFAR-10-C, and ImageNet-C [@hendrycks2019benchmarking] datasets contain corrupted or perturbed versions of the original datasets, along with tools for generating adversarial attacks.
...
* **Robustness vs. Accuracy**\index{Robustness vs. Accuracy}: Building resilient systems requires an explicit trade-off. Strong ImageNet-scale adversarial training can cost roughly 26 percentage points of clean accuracy, while lighter threat models or randomized smoothing may cost single-digit to mid-teen points; compute costs still rise by 2--10$\times$ due to iterative attack generation or Monte Carlo sampling.
