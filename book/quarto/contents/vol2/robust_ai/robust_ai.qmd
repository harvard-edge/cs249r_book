...
[^fn-bayesian-nn]: **Bayesian Neural Networks**: Networks treating weights as probability distributions, enabling principled uncertainty quantification. Variational inference (Bayes by Backprop) and Monte Carlo dropout approximate intractable posteriors. Distinguishes aleatoric uncertainty (irreducible data noise) from epistemic uncertainty (model ignorance). Critical for safety-critical applications requiring reliable confidence estimates.
...
[^fn-continual-learning]: **Continual Learning**: A machine learning paradigm where models learn from a stream of data over time, adapting to new tasks or distributions without retraining from scratch. Unlike batch learning on static datasets, continual learning systems must balance acquiring new knowledge with preserving previously learned capabilities.
...
As introduced in our framework, software faults represent a cross-cutting concern that interacts with and amplifies every robustness challenge we have examined. While hardware faults corrupt physical computation, adversarial attacks exploit algorithmic boundaries, and environmental shifts challenge model generalization, software faults operate across all these dimensions: they arise from the complex ecosystems of frameworks, libraries, and deployment tools that make modern AI possible. These faults deserve dedicated attention because a single bug can manifest as any of the other three threat types, complicating diagnosis and defense.
