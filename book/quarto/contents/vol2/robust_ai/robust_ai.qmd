...
In distributed ML training, silent data corruption is qualitatively more destructive than in conventional data-processing systems: a flipped bit in a gradient tensor is amplified by the optimizer and broadcast to every worker via AllReduce, permanently poisoning the parameter state for all subsequent steps across the entire cluster. A dropped row in a database query is a localized data loss bounded to that query's output—a point fix. Where the database repair is a point fix, the ML remedy is often a full rollback to the last clean checkpoint and a restart of the training run. SDC can therefore compromise model accuracy without triggering any alert, and the blast radius scales with cluster size rather than remaining localized. Meta's report and Dean's 2024 keynote suggest that SDC is not isolated to one provider. Jeff Dean, Chief Scientist at Google DeepMind and Google Research, highlighted these issues in AI hypercomputers[^fn-ai-hypercomputer-scale] during a keynote at MLSys 2024 [@dean2024mlsys] (@fig-sdc-robust-jeffdean).
...
# │
# │ Imports: statistics (NormalDist), mlsysim.fmt (check, fmt, fmt_percent,
# │          MarkdownStr)
# │ Exports: baseline_mean_str, current_mean_str, ds_mean_diff_str,
# │          ds_std_error_str, ds_z_score_str, ds_n_samples_str,
# │          ds_p_value_str, ds_shift_detected_str, ds_noise_prob_percent_str
# └─────────────────────────────────────────────────────────────────────────────
...
class ShiftConfidence:
    ...
    # ("less than 0.1 percent") for the "Is the world changing?" callout.
...
# │
# │ Imports: mlsysim.fmt (check, fmt, fmt_math)
# │ Exports: <bucket>_base_str, <bucket>_curr_str, <bucket>_diff_str,
# │          <bucket>_logratio_str, <bucket>_contrib_str for usa/uk/germany/
# │          france/other; psi_total_str, psi_sum_math
# └─────────────────────────────────────────────────────────────────────────────
import math
...
# │
# │ Imports: mlsysim.fmt (check, fmt, fmt_math)
# │ Exports: kl_str, kl_worked_math, ks_crit_str, ks_crit_math
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import check, fmt, fmt_math
...
class ContinuousDriftAnalysis:
    ...
    kl_worked_math = fmt_math(
        ...
        r"\frac{1.1^2 + (4.2 - 4.5)^2}{2 \times 1.3^2} - 0.5 = " + kl_term1_str + " + " + kl_term2_str + " - 0.5 = " + kl_str
    )
...
The defense cannot be chosen generically. Adversarial training raises the cost of gradient-based attacks, input transformation disrupts some small perturbations before inference, ensembles make transfer less reliable, and physical evaluation exposes failures that digital tests miss. The reason this defense budget matters is that adversarial attacks extend far beyond the basic misclassification that @fig-adversarial-googlenet illustrates, where an imperceptible perturbation flips GoogLeNet's prediction on an otherwise unchanged image. That single-image failure is only the entry point; the same principle scales into physical, transferable, and systemic attacks that create risks across deployment domains.
