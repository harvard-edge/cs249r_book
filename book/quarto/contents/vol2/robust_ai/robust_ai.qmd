...
In February 2017, Amazon Web Services (AWS) experienced [a significant S3 outage](https://aws.amazon.com/message/41926/) due to human error during routine maintenance. An incorrectly entered command removed more server capacity than intended from S3 subsystems in US-East-1. S3 was unable to service requests while key subsystems restarted, and S3-dependent services such as EC2 launches, EBS snapshot-dependent volumes, and Lambda were affected.[^fn-aws-s3-outage] A single operational error cascaded through cloud infrastructure and disrupted services that treated regional object storage as an availability invariant rather than a dependency that could fail.
