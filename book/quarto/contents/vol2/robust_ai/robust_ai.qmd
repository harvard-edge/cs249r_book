...
This chapter's position in the book's organizing framework clarifies why robustness is an infrastructure concern: silent failures propagate through every layer of the system, from hardware faults at the physical level to degraded predictions at the service level.
...
Before examining the technical mechanisms of adversarial attacks, consider the engineering cost of defending against them. The *robustness tax* quantifies the accuracy penalty imposed by adversarial training.
...
::: {.callout-note title="Figure: Decision Boundary Under Distribution Shift" collapse="false"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
...
+----------------------------+-------------------+-------------------+-------------------+
| **Dimension**              | **Hardware**      | **FPGA**          | **Software**      |
|                            | **(Beam Testing)**| **(Emulation)**   | **(PyTorchFI)**   |
+:===========================+:=================:+:=================:+:=================:+
