...
3. **Utility Cost**: Accuracy on clean data drops from 95 percent to **`{python} AdversarialPayback.ap_cert_acc_str`**.
...
* **Robustness vs. Accuracy**: Building resilient systems requires an explicit trade-off. Adversarial training and certified defenses improve security but degrade clean accuracy by 10--15 percent and increase compute costs by 2--10$\times$ due to iterative attack generation or Monte Carlo sampling.
