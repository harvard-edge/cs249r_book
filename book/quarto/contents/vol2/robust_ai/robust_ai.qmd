...
Mean Time Between Failures (MTBF) quantifies the expected operational time between successive failures. For a system experiencing $N$ failures over total operational time $T$, MTBF is defined as:
...
The three pillars we have examined—hardware faults, input-level attacks, and environmental shifts—each target different aspects of AI systems. Yet these threats do not exist in isolation. They all operate within complex software ecosystems that can amplify, mask, or transform their effects in unexpected ways.

Consider how these interactions manifest in practice. A software bug in data preprocessing might create distribution shifts that expose model vulnerabilities invisible during training. An error in numerical computation might corrupt gradients in ways that resemble adversarial perturbations. A race condition in distributed training might cause weight corruption indistinguishable from hardware-induced bit flips. Understanding these interactions reveals why achieving robust AI requires attention not only to the threats we have examined, but also to the software infrastructure underlying modern ML systems.
...
The detection and prevention strategies we have examined focus on preventing errors during development and catching them in production. However, proactively understanding how systems behave under fault conditions requires deliberate fault injection: a systematic approach to stress-testing ML systems before deployment that complements the defensive techniques discussed above.
