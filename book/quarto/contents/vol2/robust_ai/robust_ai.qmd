...
::: {.callout-note title="Figure Placeholder: Bit Flip Impact" collapse="true"}
```{.tikz}
% TODO: Diagram showing IEEE 754 bit-flip in exponent leading to value explosion
\node[draw, align=center] {Bit Flip Impact\nValue Explosion in Exponent};
```
**Weight Corruption via Bit Flip**. Visualization of a floating-point numbers bit representation (sign, exponent, mantissa). A single bit flip in the exponent can change a weight from 0.001 to 10^38, causing a neurons activation to saturate and propagate massive errors through the network, leading to catastrophic misclassification.
:::
...
::: {.callout-note title="Figure Placeholder: Distribution Shift Types" collapse="true"}
```{.tikz}
% TODO: 3-panel diagram: Covariate Shift, Label Shift, Concept Drift
\node[draw, align=center] {Distribution Shift\nCovariate vs Label vs Concept};
```
**Types of Distribution Shift**. Comparison of Covariate Shift, Label Shift, and Concept Drift. Covariate shift (left) shows input features P(x) changing while P(y|x) remains constant. Label shift (center) shows target prevalence P(y) changing. Concept drift (right) shows the underlying decision boundary P(y|x) itself shifting over time, rendering the original model obsolete.
:::
...
::: {.callout-note title="Figure Placeholder: Distribution Shift Math" collapse="true"}
```{.tikz}
% TODO: 3-panel diagram showing P(x) change, P(y) change, and P(y|x) change
\node[draw, align=center] {Distribution Shift Types\nCovariate vs Label vs Concept};
```
**Types of Distribution Shift**. Visual comparison of Covariate Shift ($P(x)$ changes), Label Shift ($P(y)$ changes), and Concept Drift ($P(y|x)$ changes). Understanding the specific type of shift is crucial for selecting the correct adaptation strategy (e.g., re-weighting vs re-training).
:::
...
::: {.callout-note icon=false title="TODO: Figure - Distributed Training Race Condition"}
**Description**: A sequence diagram showing two worker nodes (Worker A and Worker B) sending gradient updates to a Parameter Server.
1. Worker A reads weights $W_t$.
2. Worker B reads weights $W_t$.
3. Worker A calculates gradients and updates to $W_{t+1}$.
4. Worker B calculates gradients based on stale $W_t$ and overwrites $W_{t+1}$ with its own update, effectively erasing Worker A's contribution.
**Caption**: Race conditions in asynchronous distributed training can lead to 'stale gradients' and lost updates, where valid learning progress is overwritten by concurrent processes.
:::
...
Building robust AI systems requires embedding robustness considerations throughout the development process, from initial design through deployment and maintenance. Critical applications in autonomous vehicles, medical devices, and infrastructure systems demand proactive approaches that anticipate failure modes and implement extensive safeguards. The challenge extends beyond individual components to encompass system-level interactions, requiring comprehensive strategies that ensure reliable operation under the diverse and evolving conditions encountered in real-world deployments.

The robustness mechanisms developed in this chapter enable systems to detect threats, degrade gracefully under stress, and adapt to changing conditions. Yet a system that functions correctly can still be unsustainable. Robust systems that consume excessive energy, require rare materials, or generate electronic waste at scale create different kinds of systemic risks. The next chapter, @sec-sustainable-ai, examines how to build systems that remain viable over time, balancing computational demands against environmental and economic constraints. Together, the security foundations from @sec-security-privacy, the reliability guarantees developed here, and the sustainability practices ahead complete the operational safety framework that Part IV establishes.
