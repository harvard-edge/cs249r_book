...
This chapter's position in the book's organizing framework, *the Fleet Stack*, clarifies why robustness is an infrastructure concern: silent failures propagate through every layer of the system, from hardware faults at the physical level to degraded predictions at the service level.

::: {.callout-note title="Connection: The Fleet Stack"}

We are deep in the **Governance Layer** of the Fleet Stack. In the previous chapter (@sec-security-privacy), we armored the system against malicious *external* threats. Now, we turn our armor inward and outward to face *operational* threats: hardware faults, chaotic data, and the inevitable entropy of the real world. A system that is secure but fragile is useless; this chapter ensures our fleet can take a hit and keep running.

:::

## Introduction to Robust AI Systems {#sec-robust-ai-introduction-robust-ai-systems-4671}

In the **Fleet Stack** (@sec-vol2-introduction), Robustness operates at the interface between the **Governance Layer** and the **Distribution Layer**. While Security (@sec-security-privacy) defends against malicious intent, Robustness defends against the chaotic reality of the world. It ensures that the system behaves reliably even when inputs drift, hardware glitches, or adversaries probe the decision boundary. If **Security** is the armor, **Robustness** is the immune system—detecting and neutralizing internal and external anomalies before they cause failure.
...
While hardware reliability determines the failure rate (MTBF), the optimal response strategy depends on the cost of recovery. The **Young-Daly formula**, derived in @sec-data-systems, balances checkpoint overhead against lost computation. For robustness engineering, the key insight is that increasing MTBF through hardware hardening (e.g., ECC memory) yields diminishing returns for checkpoint efficiency due to the square-root relationship in Young's formula. Systems must therefore balance investment in hardware reliability against investment in fast checkpointing infrastructure.
...
We have made our systems robust against the operational chaos of hardware faults, distribution drift, and adversarial perturbation. But a system that survives every failure yet consumes a city's worth of power is not a viable long-term solution.
