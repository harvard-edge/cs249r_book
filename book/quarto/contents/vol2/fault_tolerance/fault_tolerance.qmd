...
- [ ] A GPU produces a wrong result once after a cosmic-ray strike and then computes correctly for the rest of the run. Which of the three fault categories does this match, and why does it call for error correction codes rather than node replacement?
- [ ] How would you distinguish an **intermittent** fault from a **permanent** one when both can produce a stuck-at value, and which diagnostic signal separates them?
- [ ] Can you trace a single bit flip through the fault, error, failure chain, and explain at which stage error correction codes versus checkpoint-restart each intervene?
- [ ] Why does silent corruption sit apart from the other manifestations in @tbl-fault_types even when its underlying fault is an ordinary transient bit flip?
...
- [ ] Using the Young-Daly formula, estimate roughly how the optimal interval $\tau_{\text{opt}}$ shifts when a cluster doubles in size and its system MTBF therefore halves, with $T_{\text{write}}$ held fixed. Does the interval lengthen or shorten, and by about what factor?
- [ ] Why does writing checkpoints too frequently and writing them too rarely both raise the **checkpoint tax**, and which two costs is $\tau_{\text{opt}}$ balancing?
- [ ] If sharded checkpointing cuts $T_{\text{write}}$ by an order of magnitude, can you explain the direction in which the optimal interval moves and what that does to expected lost work per failure?
- [ ] Why does the Young-Daly interval depend on the square root of MTBF rather than scaling linearly with it?
...
- [ ] Given the heartbeat timeout rule $T_{\text{timeout}} = H + k\sigma_d$, explain what failure mode you trade away when you lower $k$ from 5 to 3, and which one you make worse.
- [ ] A single GPU fails in a 1,000-GPU job with surviving state intact. Would you choose a **warm restart** or a **cold restart**, and which two conditions would push you back toward the cold path?
- [ ] Why does a **hot spare** cut recovery latency relative to detect-then-restart, and what standing cost does the cluster pay to keep that spare available?
- [ ] When a node has slowed to a fraction of normal speed rather than crashing, what governs whether you wait it out or kill it and recover onto healthy hardware?
