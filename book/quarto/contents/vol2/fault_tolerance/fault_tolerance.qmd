...
As @fig-young-daly illustrates, the **Young-Daly formula**\index{Young-Daly Formula}[^fn-young-daly-history] identifies the "sweet spot" that minimizes total wasted work.
...
Transient faults are the most common category. @Fig-bit-flip illustrates the basic mechanism: a bit-flip error\index{Bit-Flip Error} occurs when a single bit in memory unexpectedly changes state, potentially altering critical data or computations in ways that cascade through neural network layers.
...
**Objective**: Find the optimal checkpoint interval $\tau_{\text{opt}}$ that minimizes wasted time.
...
$$
$$
\tau_{\text{opt}} = \sqrt{2 \cdot (2 \text{ min}) \cdot (180 \text{ min})} = \sqrt{720} \approx 26.8 \text{ minutes}
$$
