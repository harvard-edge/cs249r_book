...
2.  **Manufacturing Variances**: "Marginal" chips that pass initial QA may exhibit bit flips only under specific voltage/temperature conditions (for example, during the intense $di/dt$ swings of a backward pass).
...
- **Model state size**: 70B parameters$\times$ 2 bytes (BF16) = `{python} CheckpointDebug.weights_gb_str` GB for weights, plus `{python} CheckpointDebug.optimizer_gb_str` GB for Adam optimizer states (first and second moments), totaling approximately `{python} CheckpointDebug.total_ckpt_gb_str` GB per checkpoint
- **Storage system**: Shared NFS filer with 10 Gbps network attachment
- **Theoretical bandwidth**: 10 Gbps = `{python} CheckpointDebug.nfs_gbs_str` GB/s maximum throughput
- **Minimum write time**: `{python} CheckpointDebug.total_ckpt_gb_str` GB / `{python} CheckpointDebug.nfs_gbs_str` GB/s = `{python} CheckpointDebug.min_write_s_str` seconds (`{python} CheckpointDebug.min_write_min_str` minutes)
...
- **Observed behavior**: 10-minute writes instead of the `{python} CheckpointDebug.min_write_min_str`-minute theoretical minimum
...
3. **Validation**: New checkpoint overhead drops to 12 seconds (GPU-to-CPU copy), reducing training impact from 30 percent to under 1 percent
...
**Impact at scale**: With a 1,000-GPU cluster MTBF of 50 hours (from @tbl-cluster-mtbf-scaling), we experience $\sim$0.48 failures per day, losing $\sim$3 minutes daily—a modest 0.2 percent overhead. But at 10,000 GPUs with MTBF of 5 hours, we experience $\sim$4.8 failures per day, losing $\sim$31 minutes daily—a 2.1 percent overhead equivalent to wasting 210 GPU-hours every day. This is why recovery time optimization matters more as clusters grow.
