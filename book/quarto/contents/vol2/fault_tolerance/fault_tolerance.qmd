...
: **Checkpoint Overhead by Model Type**: Checkpoint sizes use the mixed-precision optimizer footprint for large LLMs (see Archetype A note above), not the full FP32 training-state total (`{python} FaultToleranceIntroRecap.gpt3_ckpt_tb_str`). Larger models require longer save times but also benefit from longer optimal checkpoint intervals. The percentage column reports save-only checkpoint write tax, not total Young-Daly waste; at the optimum, expected rework adds a comparable term. {#tbl-checkpoint-overhead-by-model}
