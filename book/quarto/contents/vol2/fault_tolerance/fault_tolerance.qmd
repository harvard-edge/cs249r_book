...
**The Systems Insight**: At this interval, the "Checkpoint Tax" (time spent saving + time spent re-computing) is minimized to approximately **`{python} YoungDalyOptimal.overhead_pct_str` percent**. Checkpointing every hour instead would increase failure risk, wasting nearly 10 percent of the cluster's capacity. As clusters scale, the optimal interval *must* shrink to keep up with the falling MTBF.
