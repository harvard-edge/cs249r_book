...
Quantitative reliability analysis replaces qualitative descriptions with precise mathematical frameworks. The GPU-only calculation in the opening reliability example supplies the scale intuition; a real training system then composes GPUs, HBM, links, power supplies, storage paths, and redundancy into one failure budget. Reliability engineering provides formal methods for calculating failure rates, predicting system availability, and designing fault tolerance strategies based on measurable metrics.
...
Without error-correcting-code (ECC) memory protection, large-scale training is impossible as corruption would occur every 21 seconds. ECC detects and corrects common bit errors, but residual multi-bit or escaped corruptions remain possible. With ECC providing a 100$\times$ reduction in soft error rates, the memory MTBF improves to 35 minutes, and the combined system MTBF becomes approximately 35 minutes. This quantitative analysis justifies checkpoint frequencies of 15--30 minutes for large-scale training.
...
**Problem**: A team training a `{python} CheckpointDebug.params_b_str` parameter model observes that checkpointing takes `{python} CheckpointDebug.observed_write_min_str` per checkpoint, far exceeding their expected `{python} CheckpointDebug.target_write_min_str`-minute target. Training throughput has dropped `{python} CheckpointDebug.overhead_pct_str` because the cluster sits idle during checkpoints. Diagnose the bottleneck and identify the recovery path.
...
A common fallacy is this: *Hardware failures are the main concern.*
...
A recurring pitfall is this: *Setting checkpoint interval by intuition.*
...
A common fallacy is this: *If each GPU is 99.99 percent reliable, a 10,000-GPU cluster is also 99.99 percent reliable.*
...
A recurring pitfall is this: *Using MTBF calculations as if failures were independent.*
...
A common fallacy is this: *Component MTTF values predict individual failure timing.*
...
A recurring pitfall is this: *Ignoring restart overhead in checkpoint planning.*
...
A common fallacy is this: *Every failure should be handled as a full restart.*
...
A recurring pitfall is this: *Assuming checkpoints are consistent without validating state.*
...
A common fallacy is this: *Fault tolerance can be tested only when failures happen.*
...
A recurring pitfall is this: *Using elastic training as a substitute for checkpointing.*
...
A common fallacy is this: *Overhead budgets are fixed fractions of training time.*
...
A recurring pitfall is this: *Adding GPUs without accounting for communication and recovery overhead.*
...
A common fallacy is this: *Silent data corruption is negligible because hardware has ECC.*
...
A recurring pitfall is this: *Relying only on hardware ECC instead of end-to-end validation.*
