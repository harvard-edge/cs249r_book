...
class CheckpointOverheadCalc:
    ...
    mtbf_hr_str = fmt_time(mtbf, hour, precision=0, commas=False)
...
| **Model Type**                   |                       **Checkpoint Size**                        | **Write Time (`{python} CheckpointOverheadCalc.write_bw_gb_per_s_str`)** | **Optimal Interval (`{python} CheckpointOverheadCalc.mtbf_hr_str` GPU-only MTBF)** |                           **Save Overhead**                           |
|:---------------------------------|:----------------------------------------------------------------:|:------------------------------------------------------------------------:|:----------------------------------------------------------------------------------:|:---------------------------------------------------------------------:|
...
| Recommendation | Embedding freshness, feature-store version                                                         | Favor freshness over full-state exactness with incremental checkpointing\index{Checkpointing!incremental}, tiered checkpointing\index{Checkpointing!tiered}, and embedding versioning\index{Embedding!versioning}.         |
...
A training-loss signature is useful because it maps a curve to a recovery decision. @Tbl-training-failure-response-map makes that mapping explicit: the curve is evidence, and the recovery action depends on what kind of system failure could have produced it.
