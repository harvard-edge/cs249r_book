...
# │   (@eq-young-daly, line ~1957), §Sharded Checkpointing (line ~2289),
...
# │
# │ Imports: mlsysim.core.constants (GPT3_PARAMS, param, byte, TB, GB, BILLION),
# │   mlsysim.book (fmt, sci)
...
from mlsysim.core.constants import *
from mlsysim.fmt import fmt, sci
...
class FaultToleranceSetup:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    # GPT-3 checkpoint byte layout (FP32 Adam, no mixed precision):
    #   FP32 weights (4 bytes) + Adam m (4 bytes FP32) + Adam v (4 bytes FP32)
    #   = 12 bytes/param
    bytes_full_ckpt = 12   # bytes per param: FP32 weights + Adam m + v
    ...
    check(gpt3_ckpt_bytes > gpt3_adam_bytes, "Full checkpoint must be larger than optimizer-only checkpoint")
...
[^fn-failure-rate-fits]: **Failure Rate ($\lambda$)**: Expressed in FITs (Failures In Time), where 1 FIT equals one failure per billion device-hours. A data center GPU with 50,000-hour MTBF has $\lambda = 20,000$ FITs, which seems negligible for one device but becomes dominant at fleet scale: a 10,000-GPU cluster accumulates 200,000,000 FITs from GPUs alone, translating to an expected GPU failure every 5 hours. \index{Failure Rate!FITs}
...
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
# │ Plot cluster \text{MTBF} = \text{MTBF}_component/N vs GPU count; show crossover
import numpy as np
...
class NinesReliability:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    downtime_min_year = (1 - single_gpu_success) * HOURS_PER_YEAR * 60
    ...
    availability_str = f"{availability_pct:.2f}"
    downtime_min_str = f"{int(downtime_min_year)}"
    survival_str = f"{single_gpu_success:.5f}"
    cluster_prob_str = f"{cluster_prob:.2f}"
    cluster_pct_str = f"{int(round(cluster_prob * 100))}"
```

::: {.callout-notebook title="The 9s of reliability"}

**Problem**: A cluster of **`{python} NinesReliability.n_gpus_str` GPUs** runs with each GPU at **`{python} NinesReliability.availability_str` percent** availability (only `{python} NinesReliability.downtime_min_str` minutes of downtime per year). What is the probability that the **entire cluster** is up at the same instant?

**Math**:

1.  **Single GPU Availability Probability** $(P_{\text{up}})$: A GPU with `{python} NinesReliability.availability_str` percent availability has an all-up probability $P_{\text{up}} =$ `{python} NinesReliability.survival_str` at a randomly chosen instant.
2.  **Cluster Availability Probability** $(P_{\text{cluster}})$: $P_{\text{cluster}} = (P_{\text{up}})^{N}$.
3.  **Calculation**: $(P_{\text{up}})^{N} \approx$ **`{python} NinesReliability.cluster_prob_str`**.

**Systems conclusion**: Even with `{python} NinesReliability.availability_str` percent reliable hardware, a `{python} NinesReliability.n_gpus_str`-GPU cluster has only a **`{python} NinesReliability.cluster_pct_str` percent chance** that every GPU is simultaneously available. Hardware reliability alone is insufficient; software must handle failures automatically.
...
For **series systems** (for example, a node where all 8 GPUs must work), failure of any component causes system failure. The system reliability $R_{\text{system}}$ is the product of individual component reliabilities:
...
$$ \lambda_{\text{cluster}} = 10,000 \times 2.708 \times 10^{-5} = 0.2708 \text{ failures/hour} $$

$$ \text{MTBF}_{\text{cluster}} = \frac{1}{0.2708} = 3.69 \text{ hours} $$

This result means the cluster experiences a failure approximately every 3.7 hours on average. Over a 24-hour period, the expected number of failures is 6.5. A training run lasting one week will experience approximately forty-five failures. Any training system operating at this scale must treat failure as a continuous condition, not an exceptional event.
...
The theoretical $1/N$ scaling in @tbl-cluster-mtbf-scaling is not merely a textbook exercise. @fig-published-failure-rates overlays published empirical measurements from Meta's production clusters against the theoretical curve, confirming that the mathematics accurately predict real-world failure rates. The @kokolis2025 study of Meta's Research SuperCluster measured MTTF values from 8-GPU allocations through 16,384-GPU jobs, and Meta's Llama 3 training report independently documented 419 failures across 54 days on 16,384 H100 GPUs. Both datasets converge on the same conclusion: at 16,384 GPUs, the cluster experiences a failure every 2 to 3 hours. This empirical validation transforms the theoretical argument from an abstract scaling law into a measured engineering constraint that determines checkpoint frequency, recovery architecture, and infrastructure investment.

::: {#fig-published-failure-rates fig-env="figure" fig-pos="htb" fig-cap="**Published Failure Rates from Production Clusters**: Measured MTTF values from Meta's RSC clusters [@kokolis2025] and Llama 3 training [@dubey2024llama] plotted against the theoretical 1/N curve. At 16,384 GPUs, both empirical measurements and theory predict failures every 2 to 3 hours, confirming that fault tolerance is a mathematical necessity, not an edge case. Data sources: Kokolis et al., arXiv:2410.21680; Meta, arXiv:2407.21783." fig-alt="Log-log scatter plot of MTTF in hours vs. GPU count with theoretical 1 over N curve and labeled data points from Meta RSC and Llama 3 training."}
...
# │
# │ Imports: numpy (np), matplotlib.pyplot (plt), mlsysim.core.viz (viz),
# │          mlsysim.constants (GPU_MTTF_HOURS)
# │ Exports: (figure only, no prose variables)
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
...
Transient failures are particularly insidious in ML training because they may not trigger explicit errors. A transient memory bit flip during gradient computation produces incorrect gradients that propagate through subsequent training steps. The model continues training but produces subtly degraded results. Studies of large-scale training runs have documented cases where transient hardware errors caused training to diverge after hundreds of hours, wasting substantial compute resources[^fn-silent-corruption-training] [@dixit2021silent; @schroeder2009].

[^fn-silent-corruption-training]: **Silent Data Corruption (SDC)**: Meta's fleet-wide analysis found approximately 0.1 percent of training runs exhibited anomalous loss trajectories traceable to hardware errors that triggered no explicit exceptions. Detection required statistical monitoring of loss curves rather than traditional error handling, fundamentally changing the debugging model: in ML training, the absence of an error message does not imply correct computation. \index{Silent Data Corruption!training detection}
...
: **Failure Domains in ML Infrastructure**\index{Failure Domains in ML Infrastructure}: Understanding failure domain boundaries enables placement of redundant components across independent domains, preventing correlated failures from defeating redundancy strategies. {#tbl-failure-domains}

::: {.callout-checkpoint title="Mapping failure domains"}
...
::: {.callout-perspective title="Three rules of failure at scale"}
...
2. **The optimal checkpoint interval is $\tau_{\text{opt}} = \sqrt{2 \times T_{\text{write}} \times \text{MTBF}}$.** The Young-Daly formula provides quantitative guidance for checkpoint frequency. This formula is derived in @sec-fault-tolerance-young-daly.
...
# │ V100 SPEC (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: §Quantitative fault rates (this section); fn-gpu-fault-rates-training
# │   footnote quotes V100 HBM2 bandwidth in load-bearing prose.
# │
# │ Goal: Plumb V100 HBM2 memory bandwidth from constants so the quoted spec
# │   tracks mlsysim's single source of truth.
# │ Show: v100_mem_bw_str="900" GB/s—inline in fn-gpu-fault-rates-training.
# │ How: V100_MEM_BW.m_as(GB/second), format with fmt.
# │
# │ Imports: mlsysim.core.constants (V100_MEM_BW, GB, second), mlsysim.fmt (fmt)
# │ Exports: V100Spec.v100_mem_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import V100_MEM_BW, GB, second
from mlsysim.fmt import fmt

class V100Spec:
    """V100 hardware spec plumbed from constants."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    mem_bw_gbs = V100_MEM_BW.m_as(GB/second)

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_mem_bw_str = fmt(mem_bw_gbs, precision=0)
```

#### Quantitative fault rates {#sec-ft-fault-analysis-performance-impact-fa37}

Advanced semiconductor processes exhibit dramatically higher soft error rates. Modern 7 nm processes experience approximately 1000$\times$ higher soft error rates compared to 65 nm nodes due to reduced node capacitance and charge collection efficiency [@baumann2005], translating to base error rates of approximately 1 error per $10^{14}$ operations. GPUs[^fn-gpu-fault-rates-training] exhibit 10--1000$\times$ higher transient error rates than CPUs, driven by thousands of simpler cores operating at aggressive voltage/frequency points with minimal per-core error protection. These fault rates translate into MTBF[^fn-mtbf-checkpoint] values that differ substantially across deployment contexts: cloud AI accelerators (V100, A100) achieve 50,000--100,000 hours under controlled data center conditions, while edge AI processors achieve 20,000--40,000 hours. In distributed training, these values compound: a cluster of 1,000 accelerators with individual MTBF of 50,000 hours experiences an expected failure every 50 hours, necessitating robust checkpointing.
...
[^fn-gpu-fault-rates-training]: **GPU Fault Rates**\index{GPU!fault rates}: GPUs exhibit 10--1,000$\times$ higher transient error rates than CPUs, driven by 5,000+ simpler cores (vs. 24--48 CPU cores) operating at aggressive voltage/frequency points with minimal per-core error protection. HBM2 memory in a V100 sustains `{python} V100Spec.v100_mem_bw_str` GB/s bandwidth with narrower ECC coverage than server DRAM, creating a larger silent corruption surface for the gradient and weight data that dominates ML workloads. \index{GPU!fault rate}
...
: **Fault Mitigation Strategies**\index{Fault Mitigation Strategies}: Software faults in ML systems require layered detection and mitigation techniques applied throughout the development lifecycle, from initial testing to ongoing monitoring, to ensure reliability and robustness. {#tbl-software-faults-summary-ft}
...
![](../robust_ai/images/png/regression_testing.png)
...
**Problem**: Calculate the probability that at least one GPU in a 100,000-GPU fleet experiences a silent ALU error during a single 2-second training step.

1. **Fleet Size**\index{Fleet Size}: 100,000 accelerators.
2. **Individual Risk**\index{Individual Risk}: $10^{-6}$ per hour (a conservative estimate for SDC).
...
**Question**\index{Question}: Why might a software-based fault injection tool underestimate the resilience of a system compared to physical beam testing?
**Answer**\index{Answer}: Software tools often miss **masking effects** at the circuit level. A bit flip in a hardware register might be corrected by ECC memory or masked by logical gates before it ever reaches the software layer. Software tools that inject faults directly into variables bypass these hardware-level natural defenses, potentially reporting a higher vulnerability than exists in reality.
...
1.  **Significance (quantitative)**: It minimizes the **Lost Work** after a system failure. Within the iron-law framework formalized in @sec-performance-engineering-efficiency-frontier, checkpointing creates an **I/O Overhead** that reduces the total training throughput $(\eta_{\text{hw}})$, with the optimal interval $(\tau_{\text{opt}})$ governed by the **Young-Daly Formula** $(\tau_{\text{opt}} = \sqrt{2 \cdot T_{\text{write}} \cdot \text{MTBF}})$.
...
[^fn-adam-state-memory]: **Adam Optimizer State**\index{Adam Optimizer!state}: Adaptive Moment Estimation (Adam) maintains per-parameter first-moment ($m$) and second-moment ($v$) estimates, tripling memory requirements compared to vanilla SGD (3$\times$: parameters + two state vectors). For a `{python} FaultToleranceSetup.gpt3_params_b`B parameter model, optimizer state alone reaches `{python} FaultToleranceSetup.gpt3_adam_tb` TB, which typically dominates checkpoint size and recovery time, making optimizer state the primary bottleneck in checkpoint I/O design. \index{Adam Optimizer!checkpoint memory}
...
The Young-Daly formula introduced in @sec-fault-tolerance-young-daly provides the optimal checkpoint interval: $\tau_{\text{opt}} = \sqrt{2 \times T_{\text{write}} \times \text{MTBF}}$. The failure analysis in this chapter enables the calculation of the MTBF term required by this formula.
...
*   **Mean Time Between Failures** $(\text{MTBF})$: At this scale, the cluster experiences a silent data corruption or node failure every **3 hours** (180 minutes).

**Objective**\index{Objective}: Find the optimal checkpoint interval $\tau$ that minimizes wasted time.
...
**Result**\index{Result}: The optimal strategy is to checkpoint roughly every **27 minutes**.

*   **Checkpointing every 10 mins**: Too much time spent writing to disk (20.0 percent save overhead, 22.8 percent total overhead).
*   **Checkpointing every 60 mins**: Too much work lost when the inevitable crash happens (16.7 percent expected rework, 20.0 percent total overhead).
...
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
# │ Plot checkpoint overhead, rework cost, total waste vs interval τ;
import numpy as np
import matplotlib.pyplot as plt
from mlsysim import viz
...
ax.plot(
    tau * 60,
    over_ckpt,
    label='Checkpoint Overhead ($T_{\text{write}}/\\tau$)',
    color=COLORS['BlueLine'],
    linestyle='--'
)
...
ax.plot(
    tau * 60,
    over_rework,
    label='Expected Rework ($\\tau / 2\\,\text{MTBF}$)',
    color=COLORS['RedLine'],
    linestyle='--'
)
...
#—Annotation ---
ax.annotate(
    f'$\\tau_{{opt}} \\approx {tau_opt*60:.0f}$ min',
...
For our 10,000-GPU cluster with calculated system MTBF of 3.69 hours (from @sec-fault-tolerance-reliability-reliability-worked-example-cluster-mtbf-calculation-9255) and checkpoint time of 21 seconds (from `{python} FaultToleranceSetup.gpt3_ckpt_tb` TB checkpoint at 100 GB/s), the Young-Daly formula computes the optimal interval.
...
# │ Show: tau_opt_min_str ≈ 12.4 min—inline in notebook steps.
# │ How: τ = sqrt(2 * T_save * MTBF); bare arithmetic for clarity.
# │
# │ Imports: mlsysim.book (check)
# │ Exports: yd_t_save_s_str, yd_mtbf_h_str, yd_tau_opt_min_str, yd_overhead_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check
import math
...
class YoungDalyOptimal:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    mtbf_s = mtbf_h * 3600
    tau_opt_s = math.sqrt(2 * t_save_s * mtbf_s)
    tau_opt_min = tau_opt_s / 60
    overhead_pct = (t_save_s/tau_opt_s + tau_opt_s / (2 * mtbf_s)) * 100
    hourly_interval_s = 3600
    hourly_overhead_pct = (t_save_s/hourly_interval_s + hourly_interval_s / (2 * mtbf_s)) * 100
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(10 < tau_opt_min < 15, f"Interval {tau_opt_min:.1f}m outside expected range")
    check(hourly_overhead_pct > overhead_pct, "Hourly checkpointing should waste more than optimal")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    t_save_s_str = f"{t_save_s:.0f}"
    mtbf_h_str = f"{mtbf_h:.2f}"
    tau_opt_min_str = f"{tau_opt_min:.1f}"
    overhead_pct_str = f"{overhead_pct:.1f}"
    hourly_overhead_pct_str = f"{hourly_overhead_pct:.1f}"

```

::: {.callout-notebook title="The Young-Daly optimal interval"}

**Problem**: A 10,000-GPU cluster has an MTBF of `{python} YoungDalyOptimal.mtbf_h_str` hours. A full model checkpoint takes `{python} YoungDalyOptimal.t_save_s_str` seconds to write. What is the optimal checkpoint frequency?

**Math**:
Apply the Young-Daly formula: $\tau_{\text{opt}} = \sqrt{2 \cdot T_{\text{write}} \cdot \text{MTBF}}$

1. **Convert to common units**: MTBF = `{python} YoungDalyOptimal.mtbf_h_str` $\times$ 3,600s = `{python} f"{3.69 * 3600:,.0f}"` seconds.
2. **Calculate**: $\tau_{\text{opt}} = \sqrt{2 \cdot 21 \cdot 13,284} = \sqrt{557,928} \approx$ `{python} f"{math.sqrt(2 * 21 * 13284):,.0f}"` seconds.
3. **Result**: $\tau_{\text{opt}} \approx$ **`{python} YoungDalyOptimal.tau_opt_min_str` minutes**.

**Systems insight**: At this interval, the "checkpoint tax" (time spent saving + time spent re-computing) is minimized to approximately **`{python} YoungDalyOptimal.overhead_pct_str` percent**. Checkpointing every hour instead would increase failure risk, wasting about `{python} YoungDalyOptimal.hourly_overhead_pct_str` percent of the cluster's capacity. As clusters scale, the optimal interval *must* shrink to keep up with the falling MTBF.
...
When assumptions are violated, the optimal interval may shift significantly. As a rule of thumb, if restart overhead significantly exceeds checkpoint time, use $\sqrt{2 \times (T_{\text{write}} + T_{\text{restart}}) \times \text{MTBF}}$ instead.
...
$$ O_{\text{ckpt}} = \frac{T_{\text{write}}}{\tau_{\text{opt}}} + \frac{T_{\text{pause}}}{\tau_{\text{opt}}} $$ {#eq-checkpoint-overhead}
...
| **ResNet-50**\index{ResNet-50}   |              100 MB |                   0.001 s |                                      6 s | 0.02 percent |
| **ViT-Large**                    |              1.2 GB |                   0.012 s |                                     21 s | 0.06 percent |

: **Checkpoint Overhead by Model Type**: Larger models with correspondingly larger checkpoints require longer save times but also benefit from longer optimal checkpoint intervals. The percentage overhead remains manageable (under 5 percent) for most configurations. {#tbl-checkpoint-overhead-by-model}
...
::: {.callout-war-story title="The checkpoint storm"}
...
# │   serialized_min_str="597" min—inline in the fleet stack diagnosis.
...
# │
# │ Imports: mlsysim.core.constants (LLAMA2_70B_PARAMS, Bparam, BYTES_FP16, byte)
...
class CheckpointDebug:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    model_params_b = LLAMA2_70B_PARAMS.m_as(Bparam)
    bytes_per_param = BYTES_FP16.m_as(byte)  # BF16
    nfs_gbps = 10
    n_nodes = 64
    overhead_pct = 30
    base_weeks = 2
    ...
    optimizer_gb = weights_gb * 4 # Adam m + v in FP32
...
- **Model state size**\index{Model State Size}: 70B parameters $\times$ 2 bytes (BF16) = `{python} CheckpointDebug.weights_gb_str` GB for weights, plus `{python} CheckpointDebug.optimizer_gb_str` GB for FP32 Adam optimizer states (first and second moments), totaling approximately `{python} CheckpointDebug.total_ckpt_gb_str` GB per checkpoint before gradients, master weights, metadata, or framework overhead
- **Storage system**\index{Storage!system}: Shared NFS filer with 10 Gbps network attachment
- **Theoretical bandwidth**\index{Bandwidth!theoretical}: 10 Gbps = `{python} CheckpointDebug.nfs_gbs_str` GB/s maximum throughput
- **Minimum write time**\index{Minimum Write Time}: `{python} CheckpointDebug.total_ckpt_gb_str` GB/`{python} CheckpointDebug.nfs_gbs_str` GB/s = `{python} CheckpointDebug.min_write_s_str` seconds (`{python} CheckpointDebug.min_write_min_str` minutes)
...
- **Write pattern**\index{Write Pattern}: All 64 nodes writing simultaneously to shared NFS
- **Observed behavior**\index{Observed Behavior}: 10-minute writes instead of the `{python} CheckpointDebug.min_write_min_str`-minute theoretical minimum

The Distribution Layer reveals contention: 64 nodes competing for 10 Gbps creates severe congestion. Each node effectively receives only `{python} CheckpointDebug.per_node_mbs_str` MB/s (10 Gbps/64 nodes), extending checkpoint time to `{python} CheckpointDebug.total_ckpt_gb_str` GB/`{python} CheckpointDebug.per_node_gbs_str` GB/s ≈ `{python} CheckpointDebug.serialized_min_str` minutes per node if writes were serialized. The observed 10 minutes reflects partial parallelism with significant queuing delays.

**Governance Layer Analysis (Control Plane)**

At the top layer, organizational constraints are considered:

- **SLA requirement**\index{SLA!requirement}: Training must complete within 2 weeks
- **Current trajectory**: 30 percent overhead extends training to `{python} CheckpointDebug.extended_weeks_str` weeks
...
1. **infrastructure layer**\index{Infrastructure Layer}: Install local Non-Volatile Memory Express (NVMe) staging drives (3.5 GB/s per node)
...
3. **Validation**\index{Validation}: New checkpoint overhead drops to 12 seconds (GPU-to-CPU copy), reducing training impact from 30 percent to under 1 percent
...
::: {.callout-perspective title="Checkpoint consistency models"}
...
[^fn-sharded-ckpt-io]: **Sharded Checkpointing**: Each worker saves only its local partition rather than gathering state to a single writer. For ZeRO-3 or FSDP with 1,000 workers training a `{python} FaultToleranceSetup.gpt3_params_b`B model, each worker writes approximately `{python} FaultToleranceSetup.gpt3_shard_gb` GB instead of one writer handling `{python} FaultToleranceSetup.gpt3_ckpt_tb` TB, parallelizing I/O across all nodes. The trade-off: recovery requires all shards to be present and consistent, making the checkpoint protocol more complex and the failure of any single shard's storage fatal to the entire checkpoint. \index{Sharded Checkpointing!I/O parallelism}

This approach enables efficient checkpointing even for massive models. A `{python} FaultToleranceSetup.gpt3_params_b`B parameter model with `{python} FaultToleranceSetup.gpt3_ckpt_tb` TB checkpoint distributed across 1,000 workers requires each worker to write only `{python} FaultToleranceSetup.gpt3_shard_gb` GB, achievable in seconds with local NVMe storage.
...
class RecoveryBudget:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    t_total_str = f"{t_total_min:.1f}"
```

::: {.callout-notebook title="The recovery time budget"}

Consider our 175B parameter model training on 1,000 GPUs. The checkpoint size is approximately `{python} FaultToleranceSetup.gpt3_ckpt_tb` TB (weights + Adam optimizer state). How much does a single failure event cost?
...
3. **$T_{\text{load}}$**: 21 seconds (this is the aggregate-shared-storage read of the full `{python} FaultToleranceSetup.gpt3_ckpt_tb` TB checkpoint at 100 GB/s aggregate; with sharded local-NVMe reads at 5 GB/s, each `{python} FaultToleranceSetup.gpt3_shard_gb` GB shard loads in well under one second)
4. **$T_{\text{warmup}}$**: 2 minutes (JIT kernel compilation, data pipeline buffer fill, TCP connection re-establishment)

**Total**\index{Total}: $T_{\text{recovery}} = T_{\text{detect}} + T_{\text{restart}} + T_{\text{load}} + T_{\text{warmup}} \approx$ **`{python} RecoveryBudget.t_total_str`** minutes per failure event.

**Impact at scale**\index{Impact at Scale}: With the GPU-only baseline from @tbl-cluster-mtbf-scaling, a 1,000-GPU cluster has MTBF of 50 hours and experiences about 0.48 failures per day, losing about 3 minutes daily, a modest 0.2 percent overhead. However, a 10,000-GPU cluster has GPU-only MTBF of 5 hours and experiences about 4.8 failures per day, losing about 31 minutes daily—a 2.1 percent overhead equivalent to wasting roughly 5,000 GPU-hours every day (31 min $\times$ 10,000 GPUs ÷ 60). This is why recovery time optimization matters more as clusters grow.
...
Warm restarts can reduce $T_{\text{load}}$ and $T_{\text{warmup}}$ to near-zero for 99.9 percent of the cluster, cutting total recovery time from minutes to seconds. For our 1,000-GPU example, a warm restart avoids reloading `{python} FaultToleranceSetup.gpt3_ckpt_tb` TB from storage, saving the 21-second $T_{\text{load}}$ and 2-minute $T_{\text{warmup}}$ for all but the replacement node.
...
1.  **Significance (quantitative)**: In a synchronous system (BSP), cluster throughput ($\eta_{\text{hw}}$) is bounded by the speed of the **Slowest Rank**. A single 10 percent performance drop on one node can reduce the effective compute capacity of thousands of nodes by 10 percent.
...
$$ T_{\text{step}} = \max(T_{\text{rank}_0}, T_{\text{rank}_1}, \dots, T_{\text{rank}_{N-1}}) + T_{\text{comm}} $$
...
class StragglerTax:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    n_gpus_str = fmt(n_gpus, precision=0)
    normal_step_str = f"{normal_step_s:.1f}"
    throttle_pct_str = f"{int(throttle_factor * 100)}"
    straggler_step_str = f"{straggler_step_s:.1f}"
    healthy_gpus_str = fmt(healthy_gpus, precision=0)
    effective_speed_str = f"{effective_speed:.1f}"
    straggler_frac_str = f"{straggler_frac_pct:.1f}"
    throughput_loss_str = f"{int(throughput_loss_pct)}"
    gpu_cost_str = f"{int(gpu_cost_per_hour)}"
    wasted_cost_str = fmt(wasted_cost_per_hour, precision=0)
```

::: {.callout-notebook title="The straggler tax"}

Consider our `{python} StragglerTax.n_gpus_str`-GPU cluster where a normal training iteration takes **`{python} StragglerTax.normal_step_str` second**. A single GPU enters a thermally throttled state, clocking down to `{python} StragglerTax.throttle_pct_str` percent speed, and now takes **`{python} StragglerTax.straggler_step_str` seconds** to complete its computation.
...
- **Normal step time**\index{Normal Step Time}: `{python} StragglerTax.normal_step_str` second
- **Straggler step time**\index{Straggler!step time}: `{python} StragglerTax.straggler_step_str` seconds
- **Effective cluster speed**\index{Effective Cluster Speed}: 1 step/`{python} StragglerTax.straggler_step_str`s = `{python} StragglerTax.effective_speed_str` steps/sec

A single failing device (`{python} StragglerTax.straggler_frac_str` percent of the hardware) has reduced the throughput of the entire cluster by **`{python} StragglerTax.throughput_loss_str` percent**. At \$`{python} StragglerTax.gpu_cost_str`/GPU-hour, this straggler wastes \$`{python} StragglerTax.wasted_cost_str`/hour in idle compute.

**Lesson**: It is mathematically optimal to treat a severe straggler as a hard failure. Detecting and killing a slow node to force a restart onto healthy hardware yields higher long-term throughput than tolerating the degradation. The break-even point: if a straggler slows the cluster by more than $T_{\text{recovery}}/\text{MTBF}$ (the fraction of time spent recovering from failures), replacing it immediately is cheaper than waiting.
...
Suppose a 1,024-GPU training job loses an 8-GPU node to a hardware fault, but the cluster has no spare nodes available. Without elastic training, the remaining 1,016 GPUs sit idle for hours waiting for a repair; elastic training instead allows the job to dynamically resize and continue with slightly less compute, breaking the rigid assumption of a fixed worker count. @fig-elastic-flow traces the recovery sequence that makes this possible: detect the failure, pause, rescale across the surviving workers, and resume.
...
As @fig-elastic-flow illustrates, elastic training converts what would otherwise be hard failures into graceful capacity adjustments: a lost worker reduces the active count rather than aborting the job, the running step adapts to variable resource allocations, preemption in shared clusters becomes a routine rebalancing event, and spot-instance availability changes drive scale-up and scale-down decisions without losing progress.
...
PyTorch Elastic (TorchElastic) provides elastic launch capabilities through `torchrun`. It supports membership changes through a rendezvous mechanism[^fn-rendezvous-elastic] where workers can join or leave and the training process adapts. It integrates with Kubernetes for automatic scaling.
...
DeepSpeed [@rasley2020] supports elastic training through integration with Azure ML and automatic checkpoint/restart mechanisms. The ZeRO optimizer can reshard checkpoints for different worker counts.
...
Horovod Elastic extends Horovod's [@sergeev2018horovod] data parallel training with elastic capabilities. Workers can join or leave during training, with automatic rank reassignment and gradient accumulation adjustment.
...
: **Elastic Training Framework Comparison**\index{Elastic Training Framework Comparison}: Modern frameworks provide varying levels of support for elastic training, with different approaches to automatic recovery and state management. {#tbl-elastic-training-comparison}
...
- **Image classification**\index{Image Classification}: Each image is classified independently
- **Object detection**\index{Object Detection}: Each frame is processed independently
- **Single-turn text classification**\index{Single-Turn Text Classification}: Each text snippet is classified without context
- **Embedding generation**\index{Embedding!generation}: Each input is embedded independently
...
- **Health checks**\index{Health Checks}: Failed replicas are removed from the load balancer
...
$$ A_{\text{system}} = 1 - (1 - A_{\text{single}})^R $$ {#eq-availability-redundancy}
...
::: {.callout-war-story title="The recommendation fallback"}

Resilience is not always about restoring the primary system; sometimes it is about graceful degradation. During a major data center outage, a leading e-commerce platform's complex deep learning recommendation engine (DLRM) became unavailable. The serving infrastructure automatically failed over to a simple "Top-n" popularity model, which had 1/100th the parameters and zero personalization. This fallback persisted for 3 hours before engineers noticed, because the revenue impact was only **2 percent**. This incident proved that complex models often fight for the "last mile" of accuracy, while simple heuristics provide the bulk of the utility.
...
Circuit breakers operate in three states: closed (normal operation), open (failing fast to prevent resource exhaustion), and half-open (probing for recovery). Examine @fig-circuit-breaker to understand the state transitions that enable this protective mechanism to both shield the system from cascading failures and automatically recover when conditions improve.\index{Open}\index{Half-Open}
...
Degradation triggers define conditions that activate degradation. @lst-degradation-triggers illustrates three common trigger conditions that progressively activate fallback mechanisms based on latency, error rate, and feature store health.\index{Progressive Degradation Triggers}
...
Structured logging uses structured formats such as JavaScript Object Notation (JSON) with consistent fields. @lst-structured-log-entry shows a typical error entry that captures both the failure context and the trace identifiers needed to correlate events across services.\index{Structured JSON Log Entry}
...
The case studies that follow draw on published Meta OPT-175B training logbooks, Google TPU operational disclosures, Netflix engineering blog posts, and Microsoft DeepSpeed documentation. Specific operational numbers (failure counts, MFU achieved, MTTR figures, overhead percentages) reflect what those public sources have reported; treat them as representative magnitudes for the engineering pattern rather than precision benchmarks reproducible from this prose alone. Production deployments illustrate how leading organizations implement fault tolerance in ML systems, threading the principles developed throughout this chapter. Meta's OPT-175B training run on a 992-GPU cluster provides a well-documented study in handling dozens of hardware failures over the run without aborting training.
...
- **Shared power domain**\index{Shared Power Domain}: UPS failure takes down entire rack
- **Shared switch**\index{Shared Switch}: Top-of-rack switch failure partitions all connected GPUs
- **Shared software**\index{Shared Software}: Bug triggered by specific input fails all replicas simultaneously
- **Thermal correlation**\index{Thermal Correlation}: Cooling failure causes clustered GPU throttling
...
Total restart time can be 3--5$\times$ checkpoint save time. A 5-minute checkpoint save followed by 20-minute restart means effective failure cost is 25 minutes, not 5 minutes. The modified Young-Daly formula should use $T_{\text{write}} + T_{\text{restart}}$ for the overhead term, significantly increasing optimal checkpoint intervals.
...
1. **Rank desynchronization**\index{Rank Desynchronization}: If rank 0 checkpoints iteration 1000 while rank 1 checkpoints iteration 1001, the checkpoint is inconsistent

2. **Partial writes**\index{Partial Writes}: Storage failure mid-checkpoint leaves incomplete shards
...
4. **In-flight gradients**\index{In-Flight Gradients}: AllReduce in progress during checkpoint may or may not be included
...
Elastic training adjusts parallelism degree when workers fail, continuing with reduced capacity, which appears to eliminate checkpoint-restart overhead. However, state consistency challenges remain: reducing from N to N-1 workers requires redistributing model shards, optimizer states, and data assignments consistently. Below some minimum viable size, training becomes infeasible (model does not fit, batch size too small), requiring checkpoint-restart regardless. Each removed worker reduces throughput; accumulated failures progressively degrade training speed until checkpoint-restart becomes preferable to continued degradation. If a failure was caused by a software bug triggered by specific data, the bug persists in remaining workers. Elastic training is complementary to checkpointing, not a replacement; the reduced checkpoint frequency still requires occasional checkpoints for catastrophic failures and training completion.

**Fallacy**: *Silent data corruption is rare in modern hardware.*

Modern GPUs and memory systems include extensive error correction (ECC, CRC, parity), creating the intuition that silent data corruption is negligible. Large-scale studies reveal otherwise [@dixit2021silent; @sridharan2015]: 0.01--0.1 percent of GPUs per year exhibit silent corruption not caught by ECC, and cosmic ray-induced bit flips occur at approximately 1 per GB-month. For a 10,000-GPU cluster, this means 1--10 silent GPU corruption events per year and hundreds of thousands of raw memory bit flips monthly before ECC masking. Silent corruption causes mysterious training anomalies: loss spikes attributed to "bad batches" may be hardware errors, gradient NaNs blamed on learning rates may be bit flips, and models failing to converge despite correct hyperparameters may have corrupted weights. Detection strategies include redundant computation (computing batches on multiple workers and comparing), gradient checksums (verifying AllReduce consistency), and statistical monitoring of gradient/activation distributions. Unlike detectable failures, silent corruption does not trigger errors; training "succeeds" but produces subtly broken models, requiring detection mechanisms as discussed in @sec-fault-tolerance-reliability-reliability-mlspecific-debugging-27d5.
...
* **Scale guarantees failure**\index{Scale Guarantees Failure}: A 10,000-GPU cluster will encounter hardware failures every few hours; software must treat failure as a normal state.
...
* **The Young-Daly formula governs checkpoint intervals**: The optimal checkpoint interval $\tau_{\text{opt}} = \sqrt{2 \times T_{\text{write}} \times \text{MTBF}}$ balances the cost of saving state against the cost of lost work, often landing in the tens-of-minutes range for frontier-scale training while remaining much shorter for small checkpoints.
* **Elasticity enables persistence**\index{Elasticity Enables Persistence}: Designing training jobs to be "elastic" (reconfiguring around lost or added nodes, as @fig-elastic-flow illustrates) is superior to simple restart-from-scratch strategies.
* **Stateful serving is the frontier**\index{Stateful Serving Is the Frontier}: For LLMs, the KV cache represents a massive serving state that must be replicated or migrated to prevent high-latency session restarts, with strategies illustrated in @fig-serving-redundancy.
