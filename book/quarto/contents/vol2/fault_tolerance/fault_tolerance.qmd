...
The case studies that follow draw on published Meta OPT-175B training logbooks, Google TPU operational disclosures, Netflix engineering blog posts, and Microsoft DeepSpeed documentation. Specific operational numbers (failure counts, MFU achieved, MTTR figures, overhead percentages) reflect what those public sources have reported; treat them as representative magnitudes for the engineering pattern rather than precision benchmarks reproducible from this prose alone. Production deployments illustrate how leading organizations implement fault tolerance in ML systems, threading the principles developed throughout this chapter. Meta's OPT-175B training run on a 992-node GPU cluster provides a well-documented study in handling dozens of hardware failures per week without aborting the run.
