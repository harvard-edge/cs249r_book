...
- Apply **graceful degradation** strategies including **model fallback**\index{Model Fallback}, **feature fallback**\index{Feature Fallback}, and **load shedding** to maintain partial service when full capability is unavailable
...
**Scenario**\index{Scenario}: A team is training Llama-3 on a cluster of 16,000 GPUs.
...
1. **$T_{\text{detect}}$**\index{$t_{\text{detect}}$}: 60 seconds (conservative heartbeat timeout with verification retries)
2. **$T_{\text{restart}}$**\index{$t_{\text{restart}}$}: 3 minutes (scheduler queue time + container launch + Python import overhead + NCCL initialization)
3. **$T_{\text{load}}$**\index{$t_{\text{load}}$}: 21 seconds (with sharded checkpointing, each of 1,000 workers reads only its `{python} FaultToleranceSetup.gpt3_shard_gb` GB shard at 5 GB/s from local NVMe staging)
4. **$T_{\text{warmup}}$**\index{$t_{\text{warmup}}$}: 2 minutes (JIT kernel compilation, data pipeline buffer fill, TCP connection re-establishment)
...
| **Framework**\index{Framework} | **Elastic Support** | **Automatic Recovery** | **State Resharding** | **Cluster Integration** |
|:-------------------------------|:--------------------|:-----------------------|:---------------------|:------------------------|
...
- **Incremental checkpointing**\index{Incremental Checkpointing}: Only save embeddings that changed since last checkpoint
- **Tiered checkpointing**\index{Tiered Checkpointing}: Frequent checkpoints for model parameters, infrequent for embeddings
...
2. **Secondary**\index{Secondary}: EfficientNet-B4 (19M params, 83 percent ImageNet top-1)

3. **Tertiary**\index{Tertiary}: MobileNet-V3-Large (5.4M params, 75 percent ImageNet top-1)
