...
class FaultToleranceCheckpointRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpt3_params_b_str = fmt(m.parameters.to(param).magnitude / BILLION, precision=0, commas=False)
    gpt3_ckpt_tb_str = fmt_qty(gpt3_ckpt_bytes, TB, precision=1, commas=False)
    gpt3_adam_tb_str = fmt_qty(gpt3_adam_bytes, TB, precision=1, commas=False)
    gpt3_shard_gb_str = fmt(gpt3_ckpt_bytes.to(GB).magnitude / n_workers, precision=1, commas=False)
...
class CheckpointDebug:
    ...
    bytes_per_param = 10
    total_ckpt_qty = m.parameters.to(param).magnitude * bytes_per_param * ureg.byte
    total_ckpt_gb_val = total_ckpt_qty.to(GB).magnitude

    # Use library formula for transfer time
    t_write = (total_ckpt_qty / nfs_bw)
    min_write_s_val = t_write.to(second).magnitude
    min_write_min_val = t_write.to(minute).magnitude

    per_node_shard_qty = total_ckpt_qty / n_nodes
    per_node_shard_gb_val = per_node_shard_qty.to(GB).magnitude

    # Staging over PCIe
    t_stage = (per_node_shard_qty / pcie_bw)
    t_stage_single = (total_ckpt_qty / pcie_bw)
    gpu_cpu_parallel_s_val = t_stage.to(second).magnitude
    gpu_cpu_single_path_s_val = t_stage_single.to(second).magnitude

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(total_ckpt_gb_val == 700, f"Expected 700 GB checkpoint, got {total_ckpt_gb_val}")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    weights_gb_str = fmt_qty(m.parameters.to(param).magnitude * 2 * ureg.byte, GB, precision=0, commas=False)
    optimizer_gb_str = fmt_qty(m.parameters.to(param).magnitude * 8 * ureg.byte, GB, precision=0, commas=False)
    ...
    params_b_str = fmt_count(m.parameters.to(param).magnitude, scale='B', precision=0, commas=False)
...
class FaultToleranceRecoveryRecap:
    ...
    n_workers = Systems.Clusters.Training_1K.total_accelerators
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    params_b_str = fmt(m.parameters.to(param).magnitude / BILLION, precision=0, commas=False)
    gpt3_ckpt_tb_str = fmt_qty(gpt3_ckpt_bytes, TB, precision=1, commas=False)
    gpt3_shard_gb_str = fmt_qty(gpt3_ckpt_bytes / n_workers, GB, precision=1, commas=False)
