...
Log levels should map incidents to escalation consistently across all components: DEBUG records diagnostic detail, INFO records normal operational events, WARN records conditions that may become failures, ERROR records failed operations, and FATAL records system-wide failures that require immediate intervention. Log aggregation centralizes those events for search and analysis. The particular backend matters less than the ability to query distributed logs using the same trace and request identifiers that appear in metrics and spans.
