...
# │
# │ Imports: mlsysim.core.constants (LLAMA2_70B_PARAMS, param, BYTES_FP16, byte)
...
class CheckpointDebug:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    model_params_b = LLAMA2_70B_PARAMS.m_as(param) / 1e9
...
Correlated failures change two quantities that the simple MTBF equation hides: incident frequency and blast radius. A cluster with 1000 independent GPUs each with 10,000-hour MTBF has a first-GPU-failure MTBF of 10 hours. If a shared power, cooling, or software hazard adds an incident rate 10 times larger than that independent first-failure rate, the effective incident MTBF drops to about 1 hour. Separately, a correlated incident may take out 10 GPUs at once, increasing recovery cost even if the independent GPU failure rate itself has not changed.
