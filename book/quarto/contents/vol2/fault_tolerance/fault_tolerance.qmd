...
**Transient faults**\index{Transient Fault} are temporary disruptions caused by external factors such as cosmic rays or electromagnetic interference [@ziegler1996]. Their danger is silent corruption: a rank may keep participating while sending a wrong gradient or activation. **Permanent faults**\index{Permanent Fault} represent irreversible damage from physical defects or component wear-out, such as stuck-at faults or device failures that require hardware replacement. Their danger is repeatability: retrying the same device reproduces the same bad computation or hard failure. **Intermittent faults**\index{Intermittent Fault} appear and disappear sporadically due to unstable conditions like loose connections, aging components, or thermal stress. Their danger is ambiguity: the job may pass validation during one run and fail under a slightly different load.
...
At the software level, the same decision tree continues in model-visible evidence. Runtime monitoring [@francalanza2017; @mahmoud2021] and anomaly detection, including statistical outlier detection and One-Class SVM methods [@chandola2009], flag gradients, losses, activations, or latencies that no longer match the expected distribution. Data-consistency checks validate that a model state or checkpoint still corresponds to trusted data [@lindholm2019], while heartbeat mechanisms [@kawazoe_aguilera1997] distinguish silent corruption from fail-stop node loss. Software-implemented fault tolerance techniques such as SWIFT [@reis2005swift], N-version programming, and Reed-Solomon error correction codes [@plank1997; @reed1960] add redundancy when hardware support is insufficient. Watchdog timers [@pont2002using] close the loop by triggering recovery when a task stops making progress.
