...
These manifestations encompass several distinct categories. Common transient fault types include Single Event Upsets (SEUs)[^fn-seu-rate] from cosmic rays and ionizing radiation, voltage fluctuations [@reddi2013resilient] from power supply instability, Electromagnetic Interference (EMI) from external electromagnetic fields, Electrostatic Discharge (ESD) from sudden static electricity flow, crosstalk from unintended signal coupling, ground bounce from simultaneous switching of multiple outputs, timing violations from signal timing constraint breaches, and soft errors in combinational logic [@mukherjee2005soft]. Understanding these fault types enables designing robust hardware systems that can mitigate their impact and ensure reliable operation.
...
These theoretical fault rates translate into practical reliability metrics that vary significantly with deployment environment and workload characteristics. Typical AI accelerators demonstrate Mean Time Between Failures (MTBF)[^fn-mtbf-checkpoint] values that differ substantially across deployment contexts:
...
- **NVM (Non-Volatile Memory)**: Emerging technologies like 3D XPoint with unique error patterns requiring specialized protection schemes[^fn-nvm-ml-persistence].

[^fn-nvm-ml-persistence]: **Non-Volatile Memory (NVM)**: Storage-class memory bridging DRAM and SSDs, with Intel's 3D XPoint (commercially launched 2017) providing 1,000$\times$ faster access than SSDs while retaining data through power cycles. For ML systems, NVM enables persistent model residency: checkpoint recovery reduces from minutes (reading terabytes from SSD) to microseconds (memory already mapped), fundamentally changing the cost calculus of the Young-Daly checkpoint interval. \index{NVM!checkpoint recovery}
...
Mitigating permanent faults requires integrated fault-tolerant design combining hardware redundancy and error-correcting codes [@kim2015bamboo] with software approaches like checkpoint and restart mechanisms[^fn-checkpoint-restart-training] [@egwutuoha2013survey].
...
Error detection codes are widely used to detect data storage and transmission errors [@hamming1950error][^fn-hamming-ecc-origin]. These codes add redundant bits to the original data, allowing the detection of bit errors. @fig-parity illustrates how parity checks provide a simple form of error detection: in a single-bit parity scheme, an extra bit is appended to each data word, making the number of 1s in the word even (even parity) or odd (odd parity), enabling immediate detection when a bit flip occurs.
...
When reading the data, the parity is checked, and if it does not match the expected value, an error is detected. More advanced error detection codes, such as cyclic redundancy checks (CRC)[^fn-crc-gradient-validation], calculate a checksum based on the data and append it to the message.
...
Hardware redundancy involves duplicating critical components and comparing their outputs to detect and mask faults [@sheaffer2007hardware]. Voting mechanisms, such as double modular redundancy (DMR)[^fn-dmr-detection] or triple modular redundancy (TMR)[^fn-tmr-correction], employ multiple instances of a component and compare their outputs to identify and mask faulty behavior [@arifeen2020approximate].
...
Tesla also incorporates redundancy mechanisms beyond DMR. For example, they use redundant power supplies, steering and braking systems, and diverse sensor suites[^fn-sensor-fusion-redundancy] (e.g., cameras, radar, and ultrasonic sensors) to provide multiple layers of fault tolerance.

[^fn-sensor-fusion-redundancy]: **Sensor Fusion**: Pioneered in 1980s military applications, sensor fusion combines heterogeneous sensor modalities (camera, radar, ultrasonic) so that the failure of any single sensor type does not blind the system. Tesla processes 8 cameras plus radar and ultrasonic sensors simultaneously, generating approximately 40 GB of sensor data per hour. The redundancy is the point: each modality covers the failure modes of the others, providing graceful degradation rather than catastrophic loss of perception. \index{Sensor Fusion!redundancy}
...
**Transition:** The practical question is how distributed training systems detect these faults and recover from them without losing days of computation.
...
1.  **Significance (Quantitative):** It minimizes the **Lost Work** after a system failure. Within the **Iron Law**, checkpointing creates an **I/O Overhead** that reduces the total training throughput ($\eta$), with the optimal interval ($T_{opt}$) governed by the **Young-Daly Formula** ($T_{opt} = \sqrt{2 \cdot T_{save} \cdot \text{MTBF}}$).
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  % Failure marker
  \node[font=\bfseries, text=RedLine] at (7.8, \bary+1.1) {\Large\ding{55}};
...
**PyTorch Elastic (TorchElastic)** [@li2020pytorch]: Provides elastic launch capabilities through `torchrun`. Supports membership changes through a rendezvous mechanism.[^fn-rendezvous-elastic] Workers can join or leave, and the training process adapts. Integrates with Kubernetes for automatic scaling.
...
**KV cache state**: The KV cache[^fn-kv-cache-serving-ft] can be substantial (gigabytes for long contexts across attention layers). Losing the KV cache requires regenerating all previous turns, which can take seconds to minutes.
...
1.  **Significance (Quantitative):** It allows the system to remain functional by trading **Model Accuracy** for lower resource requirements ($R_{peak}, BW$). For example, a system may fall back to a smaller model or use fewer features to maintain its **Inference SLO** ($L_{lat}$) during high load.
...
**NaN detection**:[^fn-nan-propagation-training] Because NaN values propagate silently through all downstream computations, early detection is essential. @lst-nan-detection shows a minimal check that catches corruption before it reaches users.
