...
gpt3_params_b = f"{GPT3_PARAMS.to(param).magnitude / 1e9:.0f}"
...
The operational layer reveals contention: 64 nodes competing for 10 Gbps creates severe congestion. Each node effectively receives only `{python} per_node_mbs_str` MB/s (10 Gbps / 64 nodes), extending checkpoint time to `{python} total_ckpt_gb_str` GB / `{python} per_node_gbs` GB/s $\approx$ `{python} serialized_min_str` minutes per node if writes were serialized. The observed 10 minutes reflects partial parallelism with significant queuing delays.
