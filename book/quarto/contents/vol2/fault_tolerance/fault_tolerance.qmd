...
![](images/svg/failure-spectrum.svg){width=100 percent}
:::
...
![](images/svg/young-daly-optimization.svg){width=100 percent}
:::
...
![](images/svg/checkpoint-tax.svg){width=100 percent}
:::
...
**Problem**\index{Problem}: You have a cluster of **`{python} NinesReliability.n_gpus_str` GPUs**. Each GPU has **`{python} NinesReliability.availability_str` percent** availability (only `{python} NinesReliability.downtime_min_str` minutes of downtime per year). What is the probability that the **entire cluster**\index{Entire Cluster} is up for **1 hour**?
...
*   **HBM2 Memory**\index{HBM2 Memory}: 250 FIT per Mb. Total Mb = $1,024 \times 80 \text{ GB} \times 8 \times 1,024 \approx 671 \text{M Mb}$. Total FIT $\approx$ 167B FIT $\rightarrow$ **MTBF $\approx$ 21 seconds**.
...
![](images/svg/sdc-propagation.svg){width=100 percent}
:::
...
![](images/svg/sdc-controller.svg){width=100 percent}
:::
...
[^fn-recsys-freshness-cost]: **RecSys Freshness**\index{RecSys Freshness}: Meta's DLRM infrastructure documents that embedding staleness measured in hours produces measurable degradation in recommendation relevance and engagement metrics. This inverts the typical fault tolerance priority: for recommendation systems, minimizing recovery time matters more than minimizing checkpoint overhead, because stale embeddings directly reduce revenue. \index{Recommendation Systems!freshness fault tolerance}
...
2. **The optimal checkpoint interval is $\sqrt{2 \times T_{\text{save}} \times \text{MTBF}}$.** The Young-Daly formula provides quantitative guidance for checkpoint frequency. This formula is derived in @sec-fault-tolerance-reliability-reliability-checkpoint-restart-fundamentals-0ac2.
...
![](./images/png/permanent_fault.png){width=70 percent}
...
![](./images/png/intermittent_fault.png){width=85 percent}
...
![](./images/png/intermittent_fault_dram.png){width=70 percent}
...
![](images/svg/tesla-dmr.svg){width=100 percent}
:::
...
![](images/svg/error-masking.svg){width=100 percent}
:::
...
- **Theoretical bandwidth**\index{Theoretical Bandwidth}: 10 Gbps = `{python} CheckpointDebug.nfs_gbs_str` GB/s maximum throughput
...
![](images/svg/distributed-checkpoint-architecture.svg){width=100 percent}
:::
...
: **Stateless vs. Stateful Serving Fault Tolerance**\index{Stateless vs. Stateful Serving Fault Tolerance}: Stateful serving introduces significant complexity in fault tolerance due to the need to preserve accumulated session state. {#tbl-stateless-stateful-comparison}
...
![](images/svg/observability-three-pillars.svg){width=100 percent}
:::
...
![](images/svg/training-failure-signatures.svg){width=100 percent}
:::
...
* **Checkpointing is the Baseline**\index{Checkpointing!is the baseline}: Synchronous checkpointing is simple but incurs high overhead; asynchronous approaches hide I/O latency at the cost of consistency complexity.
...
* **Training and Serving Require Different Strategies**\index{Training!and serving require different strategies}: Training fault tolerance is checkpoint-centric and tolerates minutes of recovery, while serving fault tolerance is state-migration-centric and demands sub-second failover.
