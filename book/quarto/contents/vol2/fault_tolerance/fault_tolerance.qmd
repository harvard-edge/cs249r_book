...
When reading the data, the parity is checked, and if it doesn't match the expected value, an error is detected. More advanced error detection codes, such as cyclic redundancy checks (CRC)[^fn-crc-gradient-validation], calculate a checksum based on the data and append it to the message.
