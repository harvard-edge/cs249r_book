...
The checkpoint and recovery strategies developed above require adaptation because workloads lose different kinds of state when they fail. The design question is therefore not simply how often to checkpoint, but which state variable would make a restart mathematically or economically wrong. LLMs protect optimizer state and curriculum position, recommendation systems protect embedding freshness, vision models protect augmentation and normalization state, and scientific workloads protect exploration or simulator state.

LLM training puts the greatest pressure on checkpoint I/O because optimizer state and long training duration make each restart expensive. Mixed-precision checkpoints reduce the bytes written for weights while preserving FP32 optimizer state where precision matters; ZeRO/FSDP sharding spreads writes across workers; asynchronous checkpointing moves serialization off the critical path; and incremental checkpointing records only changed state. These mechanisms solve different parts of the same bottleneck, so the durable requirement is that recovery preserve both optimizer state and the model's position in the data schedule. Curriculum learning, document position, and long-context schedules must be included in the checkpoint, because resuming with the wrong data order can change the optimization trajectory even when the weights reload correctly.

Recommendation training reverses the priority. A full snapshot of trillion-parameter embedding tables may be too large to write frequently, while stale embeddings can degrade revenue-sensitive ranking quality within hours. The checkpoint design therefore favors freshness over full-state exactness: **incremental checkpointing**\index{Checkpointing!incremental} records only embeddings that changed since the last checkpoint, **tiered checkpointing**\index{Checkpointing!tiered} saves dense parameters more often than sparse tables, and **embedding versioning**\index{Embedding!versioning} stores compact deltas that let recovery reconstruct recent user and item state. Because many RecSys models train continuously on streaming data, recovery must also record feature-store versions so that model state and training features remain semantically aligned after restart.

Vision workloads usually have smaller checkpoints, but reproducibility depends on state outside the model weights. Data augmentation seeds, current augmentation parameters, shuffling order, synchronized batch-normalization statistics, and progressive-resizing schedules all shape the training trajectory. A recovered vision run that reloads weights but changes augmentation or normalization state may still produce a different model, so checkpoint design must capture the stochastic and schedule state that controls the input stream.

Scientific and specialized workloads push this requirement even further because the ML model is often coupled to a search or simulation process. Protein-structure, molecular-design, and climate-modeling runs may need to remember which configurations have already been explored, which simulator state produced the current examples, and which random seeds support reproducible validation. Losing that domain state creates duplicate exploration or invalid comparisons, so recovery must treat the model, simulator, search frontier, and random state as one consistency unit.
...
Model-specific serving differences follow from what state is expensive to lose and how quickly the user needs a response. The extremes are LLM sessions that carry large live context, recommendation systems that depend on fresh user and item features, and vision services that are often stateless but sensitive to preprocessing and device health.
