...
**Problem**: You have a cluster of **`{python} NinesReliability.n_gpus_str` GPUs**. Each GPU has **`{python} NinesReliability.availability_str` percent** availability (only `{python} NinesReliability.downtime_min_str` minutes of downtime per year). What is the probability that the **entire cluster**\index{Cluster!entire} is up for **1 hour**?

**The Math**:

1.  **Single GPU Success Probability ($P_{1h}$)**: A GPU failing once per year (`{python} NinesReliability.hours_str` hours) has hourly survival probability $P_{1h} \approx 1 - 1/N_{\text{hours}}$ = `{python} NinesReliability.survival_str`.
2.  **Cluster Success Probability ($P_{cluster}$)**: $P_{cluster} = (P_{1h})^{N}$.
3.  **Calculation**\index{Calculation}: $(P_{1h})^{N}$ $\approx$ **`{python} NinesReliability.cluster_prob_str`**.
...
- **Adversarial corruption**\index{Adversarial Corruption}: Malicious actors intentionally inject incorrect gradients
...
::: {#fig-bathtub-curve fig-env="figure" fig-pos="htb" fig-cap="**The Bathtub Curve**. Hardware failure rates $\lambda(t)$ vary over time. (1) **Infant Mortality**\index{Infant Mortality}: High failure rate initially due to manufacturing defects. (2) **Useful Life**\index{Useful Life}: Constant, low failure rate where random failures dominate. (3) **Wear-Out**\index{Wear-Out}: Increasing failure rate as components age. Burn-in testing aims to filter out infant mortality failures before deployment." fig-alt="Line graph of failure rate vs. component age showing bathtub shape. Three phases: infant mortality with high decreasing rate, useful life with constant low rate, and wear-out with increasing rate. Vertical dashed line marks burn-in period."}
...
*   **Checkpointing every 10 mins**\index{Checkpointing!every 10 mins}: Too much time spent writing to disk ($17\%$ overhead).
*   **Checkpointing every 60 mins**\index{Checkpointing!every 60 mins}: Too much work lost when the inevitable crash happens ($15\%$ overhead).
*   **At 27 mins**\index{At 27 Mins}: You minimize the combined cost of overhead and recovery ($7\%$ total overhead).
...
ax.plot(
    tau * 60,
    over_ckpt,
    label='Checkpoint Overhead ($T_{write}/\\tau$)',
    color=COLORS['BlueLine'],
    linestyle='--'
)
...
ax.plot(
    tau * 60,
    over_rework,
    label='Expected Rework ($\\tau / 2\\,MTBF$)',
    color=COLORS['RedLine'],
    linestyle='--'
)
...
1. **Convert to common units**\index{Convert to Common Units}: MTBF = `{python} YoungDalyOptimal.mtbf_h_str` $\times$ 3,600s = `{python} f"{3.69 * 3600:,.0f}"` seconds.
2. **Calculate**\index{Calculate}: $\tau_{\text{opt}} = \sqrt{2 \cdot 21 \cdot 13,284} = \sqrt{557,928} \approx$ `{python} f"{math.sqrt(2 * 21 * 13284):,.0f}"` seconds.
...
: **Checkpoint Overhead by Model Type**\index{Checkpoint Overhead by Model Type}: Larger models with correspondingly larger checkpoints require longer save times but also benefit from longer optimal checkpoint intervals. The percentage overhead remains manageable (under 5 percent) for most configurations. {#tbl-checkpoint-overhead-by-model}
...
- **Checkpoint mode**\index{Checkpoint Mode}: Synchronous, all 512 GPUs pause and wait
...
- **Current trajectory**\index{Current Trajectory}: 30 percent overhead extends training to `{python} CheckpointDebug.extended_weeks_str` weeks
- **Budget impact**\index{Budget Impact}: Extended training incurs additional \$`{python} CheckpointDebug.extra_cost_k_str`K in compute costs
...
**Bounded Asynchronous**\index{Bounded Asynchronous}: Workers may be within $k$ steps of each other (typically $k=1–3$). The checkpoint manager tracks the "checkpoint wavefront" across workers. Recovery uses the earliest consistent cut across all shards. This trades perfect consistency for dramatically reduced synchronization overhead and is what production systems actually use.
...
4. **Checkpoint loading**\index{Checkpoint Loading}: Each worker reads its state shard from the distributed filesystem. For sharded checkpoints, each worker loads only its partition.
...
1.  **Significance (Quantitative):** In a synchronous system (BSP), cluster throughput ($\eta$) is bounded by the speed of the **Slowest Rank**. A single 10 percent performance drop on one node can reduce the effective compute capacity of thousands of nodes by 10 percent.
...
- **Automatic replacement**\index{Automatic Replacement}: Failed replicas are restarted or replaced
...
In **active-active replication**\index{Active-Active Replication} (@fig-serving-redundancy, left), all replicas actively serve requests.
...
In **active-passive replication**\index{Active-Passive Replication} (@fig-serving-redundancy, right), primary replicas serve requests while standby replicas remain idle but ready. Failure of a primary triggers failover to the standby. This approach provides simpler failover but wastes standby resources during normal operation.
...
::: {#fig-circuit-breaker fig-env="figure" fig-pos="htb" fig-cap="**Circuit Breaker States**. The circuit breaker protects the system from cascading failure. **Closed**\index{Closed}: Normal operation. **Open**\index{Open}: Error threshold exceeded; all requests fail fast to prevent resource exhaustion. **Half-Open**\index{Half-Open}: After a timeout, a limited number of requests are allowed through to probe the dependency's health. Success resets to Closed; failure returns to Open." fig-alt="State diagram with three circular nodes. Green Closed state transitions to red Open state on errors exceeding threshold. Open transitions to yellow Half-Open on timeout. Half-Open returns to Closed on success or back to Open on failure."}
...
::: {#lst-degradation-triggers lst-cap="**Progressive Degradation Triggers**\index{Progressive Degradation Triggers}: Conditions that activate graceful degradation based on tail latency, error rate, and feature store health. Each trigger activates a different fallback mechanism appropriate to the detected problem."}
...
Structured logging uses structured formats such as JavaScript Object Notation (JSON) with consistent fields. @lst-structured-log-entry shows a typical error entry that captures both the failure context and the trace identifiers needed to correlate events across services.

::: {#lst-structured-log-entry lst-cap="**Structured JSON Log Entry**\index{Structured JSON Log Entry}: A structured log entry for a GPU memory allocation failure, including trace and span identifiers for correlation with distributed traces and resource metrics for diagnosis."}
...
::: {#lst-nan-detection lst-cap="**NaN Detection**\index{NaN Detection}: Checking model outputs for NaN values prevents silent corruption from propagating to downstream consumers. Logging the input hash enables reproducibility during diagnosis."}
...
::: {#lst-timing-instrumentation lst-cap="**Operation-Level Timing**\index{Operation-Level Timing}: Context manager instrumentation attributes latency to individual pipeline stages, enabling straggler detection by comparing the same span across replicas."}
...
4. **Communication re-establishment**\index{Communication Re-Establishment}: NCCL ring topology reconstruction
