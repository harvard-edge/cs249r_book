...
**Problem**: A cluster of `{python} NinesReliability.n_gpus_str` runs with each GPU at `{python} NinesReliability.availability_str` availability (only `{python} NinesReliability.downtime_min_str` of downtime per year). What is the probability that the *entire cluster* is up at the same instant?
...
|:---------------------------------|--------------------------------------------------:|---------------------------------------------------:|------------------------------------------------------:|------------------------------------------------------:|
| **Archetype A (175B dense LLM)** |  `{python} CheckpointOverheadCalc.archA_size_str` |  `{python} CheckpointOverheadCalc.archA_write_str` |  `{python} CheckpointOverheadCalc.archA_interval_str` |  `{python} CheckpointOverheadCalc.archA_overhead_str` |
...
| **Archetype B (DLRM at Scale)**  |  `{python} CheckpointOverheadCalc.archB_size_str` |  `{python} CheckpointOverheadCalc.archB_write_str` |  `{python} CheckpointOverheadCalc.archB_interval_str` |  `{python} CheckpointOverheadCalc.archB_overhead_str` |
...
class RecoveryBudget:
    ...
    mtbf_1k_hrs_str = fmt(mtbf_1k_hrs, precision=0, commas=False)
    mtbf_10k_hrs_str = fmt(mtbf_10k_hrs, precision=0, commas=False)
    ...
    daily_loss_min_1k_str = fmt_int(round(daily_loss_min_1k), commas=False)
    daily_loss_min_10k_str = fmt_int(round(daily_loss_min_10k), commas=False)
    overhead_pct_1k_str = fmt(overhead_pct_1k, precision=1, commas=False)
    overhead_pct_10k_str = fmt(overhead_pct_10k, precision=1, commas=False)
...
Warm restarts can reduce $T_{\text{load}}$ and $T_{\text{warmup}}$ to near-zero for 99.9 percent of the cluster, cutting total recovery time from minutes to seconds. For our `{python} FaultToleranceRecoveryRecap.n_workers_str`-GPU example, a warm restart avoids reloading `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` from storage, saving the `{python} RecoveryBudget.t_load_s_str`-second $T_{\text{load}}$ and 2-minute $T_{\text{warmup}}$ for all but the replacement node.
...
class StragglerTax:
    ...
    throttle_pct_str = fmt_int(throttle_factor * 100, commas=False)
    ...
    straggler_frac_str = fmt(straggler_frac_pct, precision=1, commas=False)
...
Consider our `{python} StragglerTax.n_gpus_str`-GPU cluster where a normal training iteration takes `{python} StragglerTax.normal_step_str`. A single GPU enters a thermally throttled state, clocking down to `{python} StragglerTax.throttle_pct_str` speed, and now takes `{python} StragglerTax.straggler_step_str` to complete its computation.
...
- **Normal step time**: `{python} StragglerTax.normal_step_str`
- **Straggler step time**\index{Straggler!step time}: `{python} StragglerTax.straggler_step_str`
...
A single failing device (`{python} StragglerTax.straggler_frac_str` of the hardware) has reduced the throughput of the entire cluster by `{python} StragglerTax.throughput_loss_str`. At \$`{python} StragglerTax.gpu_cost_str`/GPU-hour, this straggler wastes \$`{python} StragglerTax.wasted_cost_str`/hour in idle compute.
