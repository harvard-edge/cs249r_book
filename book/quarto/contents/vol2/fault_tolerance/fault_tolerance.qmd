...
* **Elasticity enables Persistence**: Designing training jobs to be "elastic" (reconfiguring around lost or added nodes) is superior to simple restart-from-scratch strategies.
* **Stateful Serving is the Frontier**: For LLMs, the KV cache represents a massive serving state that must be replicated or migrated to prevent high-latency session restarts.
:::
