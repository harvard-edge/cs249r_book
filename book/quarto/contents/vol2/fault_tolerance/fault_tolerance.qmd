...
The cluster MTBF scaling\index{Cluster!mtbf scaling} in @tbl-cluster-mtbf-scaling isolates the GPU-only baseline across cluster sizes. The full-system calculation above is lower once PCIe, power, and network failure domains are included.
...
The memory bandwidth protection analysis\index{Memory Bandwidth!protection analysis} below shows the throughput tax that error protection can impose on different memory technologies.
