...
Silent hardware errors (e.g., bit flips in ALU logic) often fail to crash the process but corrupt the mathematical result. In training, this manifests as a sudden, catastrophic spike in the loss function.
*   **Symptom**: Loss jumps 10x-100x or becomes NaN/Inf instantly.
*   **Diagnosis**: This is rarely an algorithmic issue if hyperparameters were stable. It is a signature of hardware failure.
*   **Response**: Systems should automatically pause training, identify which rank contributed the corrupted gradient (using checksums or replay), and drain that node before restarting from the last good checkpoint.
