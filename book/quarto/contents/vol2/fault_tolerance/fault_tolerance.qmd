...
class MemBandwidthProtection:
    ...
    # ┌── 3. GUARD (Invariants) ────────────────────────────────────────────
    check(abs(ddr4_eff_bw.to(GB / second).magnitude - 44.8) < 0.01,
          f"DDR4 effective BW mismatch: {ddr4_eff_bw}")
    check(abs(hbm2_eff_bw.to(GB / second).magnitude - 787.5) < 0.01,
          f"HBM2 effective BW mismatch: {hbm2_eff_bw}")
    check(abs(hbm3_eff_bw.to(GB / second).magnitude - 1400) < 0.01,
          f"HBM3 effective BW mismatch: {hbm3_eff_bw}")
    check(abs(gddr6x_eff_bw.to(GB / second).magnitude - 760) < 0.01,
          f"GDDR6X effective BW mismatch: {gddr6x_eff_bw}")
...
# │
# │ Imports: mlsysim public API (Models, Systems, ReferenceStats, units, fmt helpers), math
# │ Exports: CheckpointOverheadCalc.{archA,dense20b,bert,archB,resnet,vision_transformer}_*_str,
# │          CheckpointOverheadCalc.write_bw_gb_per_s_str, mtbf_h_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
class CheckpointOverheadCalc:
    ...
    mtbf = (
        float(Systems.Reliability.Gpu.mttf_hours)
        / Systems.Clusters.Training_10K.total_accelerators
    ) * hour
    ...
    mtbf_h_str = fmt_time(mtbf, hour, precision=0, commas=False)
...
| **Model Type**                   |                       **Checkpoint Size**                        | **Write Time (`{python} CheckpointOverheadCalc.write_bw_gb_per_s_str`)** | **Optimal Interval (`{python} CheckpointOverheadCalc.mtbf_h_str` GPU-only MTBF)** |                           **Save Overhead**                           |
|:---------------------------------|:----------------------------------------------------------------:|:------------------------------------------------------------------------:|:---------------------------------------------------------------------------------:|:---------------------------------------------------------------------:|
...
class RecoveryBudget:
    ...
    t_restart = 3 * minute  # scenario assumption: queue, launch, import, NCCL setup
    aggregate_ckpt_bw = float(Systems.Reliability.Recovery.checkpoint_write_bw_gbs) * (GB / second)
    t_load = (_mem.total_memory / aggregate_ckpt_bw).to(second)
    t_warmup = 2 * minute  # scenario assumption: JIT, buffers, connections
    ...
    mtbf_1k = (float(Systems.Reliability.Gpu.mttf_hours) / n_gpus_1k) * hour
    mtbf_10k = (float(Systems.Reliability.Gpu.mttf_hours) / n_gpus_10k) * hour
