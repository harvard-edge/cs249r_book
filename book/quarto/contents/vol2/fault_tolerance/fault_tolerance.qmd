...
**Systems lesson**: Checkpoint overhead cannot be solved by algorithmic adjustments alone when physical I/O bandwidth is saturated. Implementing asynchronous NVMe staging (GPU $\to$ host CPU $\to$ local NVMe $\to$ background NFS) reduces critical-path pause time to under `{python} CheckpointDebug.residual_overhead_pct_str`.
