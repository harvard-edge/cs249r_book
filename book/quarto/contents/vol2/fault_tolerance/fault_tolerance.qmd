...
# ┌── EXPORTS (Bridge to Text) ───────────────────────────────
nines_n_gpus = NinesReliability.n_gpus_str
nines_availability = NinesReliability.availability_str
nines_downtime_min = NinesReliability.downtime_min_str
nines_hours = NinesReliability.hours_str
nines_survival = NinesReliability.survival_str
nines_cluster_prob = NinesReliability.cluster_prob_str
nines_cluster_pct = NinesReliability.cluster_pct_str
```

::: {.callout-notebook title="The 9s of Reliability"}
**Problem**: You have a cluster of **`{python} nines_n_gpus` GPUs**. Each GPU has **`{python} nines_availability`%** availability (only `{python} nines_downtime_min` minutes of downtime per year). What is the probability that the **entire cluster** is up for **1 hour**?

**The Math**:

1.  **Single GPU Success Probability ($P_{1h}$)**: A GPU failing once per year (`{python} nines_hours` hours) has hourly survival probability $P_{1h} \approx 1 - 1/N_{\text{hours}}$ = `{python} nines_survival`.
2.  **Cluster Success Probability ($P_{cluster}$)**: $P_{cluster} = (P_{1h})^{N}$.
3.  **Calculation**: $(P_{1h})^{N}$ $\approx$ **`{python} nines_cluster_prob`**.

**The Systems Conclusion**: Even with `{python} nines_availability`% reliable hardware, a `{python} nines_n_gpus`-GPU cluster has only a **`{python} nines_cluster_pct`% chance** of surviving a single hour without a failure. Hardware reliability alone is insufficient; software must handle failures automatically.
:::
...
# ┌── EXPORTS (Bridge to Text) ───────────────────────────────
recovery_total_min = RecoveryBudget.t_total_str
```
...
**Total**: $T_{\text{recovery}} = T_{\text{detect}} + T_{\text{restart}} + T_{\text{load}} + T_{\text{warmup}}$ $\approx$ **`{python} recovery_total_min`** minutes per failure event.
