...
class NinesReliability:
    ...
    availability_str = fmt(availability_pct, precision=2, commas=False)
    downtime_min_str = fmt(downtime_min_year, precision=1, commas=False)
    survival_str = fmt(single_gpu_success, precision=5, commas=False)
    cluster_prob_str = fmt(cluster_prob, precision=2, commas=False)
    cluster_pct_str = fmt_int(round(cluster_prob * 100), commas=False)
```
...
For **series systems** (for example, a node where all 8 GPUs must work), failure of any component causes system failure. The system reliability $R_{\text{system}}(t)$ is the product of individual component reliabilities:
...
For **parallel systems** providing redundancy (for example, active-active model replicas), failure occurs only when all redundant components fail simultaneously:
...
2. **The optimal checkpoint interval is $\tau_{\text{opt}} = \sqrt{2 \times T_{\text{write}} \times \text{MTBF}_{\text{system}}}$.** The Young-Daly formula provides quantitative guidance for checkpoint frequency. This formula is derived in @sec-fault-tolerance-young-daly.
...
*Shows silent-data-corruption risk accumulating until it is operationally expected.*
:::
...
*Shows checkpoint payload dominated by optimizer state, not just model weights.*
:::
...
- **Observed behavior**\index{Observed Behavior}: `{python} CheckpointDebug.observed_write_min_str`-minute writes instead of the `{python} CheckpointDebug.min_write_min_str`-minute theoretical minimum
...
*Shows a single straggler wasting healthy accelerator time at synchronization.*
:::
...
*Shows redundancy converting days of downtime into minutes or seconds.*
:::
...
Engineers read a GPU MTTF of `{python} FleetReliabilityRates.gpu_mttf_str` hours and plan around a five-year replacement cadence per device. MTTF is a statistical property of a population, not a prediction for any single component. A GPU with MTTF `{python} FleetReliabilityRates.gpu_mttf_str` hours might fail at 200 hours or last 100,000; the distribution is broad. What MTTF reliably predicts is the aggregate failure rate: in a fleet of `{python} FleetReliabilityRates.fleet_gpus_str` GPUs, the steady-state failure rate is approximately `{python} FleetReliabilityRates.fleet_rate_eq_math` failures per hour, or roughly one failure every `{python} FleetReliabilityRates.hours_between_str` hours. Fleet-scale reliability engineering relies on this statistical regularity to size automated recovery, spare-pool depth, and on-call rotations. Trying to predict which individual GPU will fail next is a category error and a waste of monitoring effort.
