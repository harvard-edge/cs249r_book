...
The distributed training systems examined in @sec-distributed-training-systems...
...
For our 10,000 GPU cluster with calculated system MTBF of 3.69 hours (from @sec-fault-tolerance-reliability-worked-example-cluster-mtbf-calculation-9255) and checkpoint time of 21 seconds (from 2.1 TB checkpoint at 100 GB/s), @eq-young-daly-applied computes the optimal interval:
...
We now turn from the logical to the physical. **Part II: Building the Machine Learning Fleet** begins with @sec-compute, where we examine the silicon, power, and cooling systems required to build the supercomputers that power these algorithms.
