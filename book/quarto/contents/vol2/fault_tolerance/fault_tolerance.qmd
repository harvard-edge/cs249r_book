...
| **Vision transformer class**          |            1–3 days           |        1–2 GB       |   Medium (augmentation state)   |    \$10–50K per day loss     |
...
At the software level, the same decision tree continues through model-visible evidence. Runtime monitoring [@francalanza2017; @mahmoud2021] and anomaly detection, including statistical outlier detection and One-Class SVM methods [@chandola2009], flag gradients, losses, activations, or latencies that no longer match the expected distribution. Data-consistency checks decide whether a model state or checkpoint still corresponds to trusted data [@lindholm2019], while heartbeat mechanisms [@kawazoe_aguilera1997] distinguish fail-stop node loss from slow progress. When hardware protection is insufficient, software-implemented redundancy such as SWIFT [@reis2005swift], N-version programming, and Reed-Solomon error correction codes [@plank1997; @reed1960] provides a second way to compare or reconstruct state. Watchdog timers [@pont2002using] close the loop by triggering recovery when a task stops making progress.
...
# │
# │ Imports: mlsysim public API (Models, Systems, units, fmt helpers), math
# │ Exports: CheckpointOverheadCalc.{archA,dense20b,bert,archB,resnet,vision_transformer}_*_str,
# │          CheckpointOverheadCalc.write_bw_gb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models, Systems
from mlsysim.core.units import BYTES_FP32, GB, MB, TB, byte, hour, second
from mlsysim.fmt import MarkdownStr, check, fmt_bandwidth, fmt_memory, fmt_percent, fmt_time
...
class CheckpointOverheadCalc:
    ...
    checkpoint_bytes_per_param = 12 * byte  # scenario assumption: mixed precision + optimizer
    archA_size = calc_checkpoint_size(
        Models.Language.GPT3.parameters,
        bytes_per_param=checkpoint_bytes_per_param,
    )
    dense20b_size = 240 * GB   # scenario assumption: 20B dense transformer checkpoint
    bert_size = Models.Language.BERT_Large.size_in_bytes(BYTES_FP32)
    archB_size = 4 * TB        # scenario assumption: embedding-heavy recommender checkpoint
    resnet_size = Models.Vision.ResNet50.size_in_bytes(BYTES_FP32)
    vision_transformer_size = 1.2 * GB  # scenario assumption: medium vision-transformer checkpoint

    write_bw = Systems.Reliability.Recovery.checkpoint_write_bw_gbs.value * (GB / second)
    mtbf = 5 * hour            # GPU-only MTBF
    ...
    def young_daly_interval(write_time):
        interval_s = math.sqrt((2 * write_time * mtbf).to(second**2).magnitude)
        return interval_s * second

    # Optimal interval = sqrt(2 * write_time * MTBF) [Young-Daly formula]
    archA_interval = young_daly_interval(archA_write)
    dense20b_interval = young_daly_interval(dense20b_write)
    bert_interval = young_daly_interval(bert_write)
    archB_interval = young_daly_interval(archB_write)
    resnet_interval = young_daly_interval(resnet_write)
    vision_transformer_interval = young_daly_interval(vision_transformer_write)
    ...
    vision_transformer_overhead_ratio = (
        vision_transformer_write / vision_transformer_interval
    ...
    resnet_ckpt_mb_str = fmt_memory(resnet_size, unit=MB, precision=0, commas=False)
    vision_transformer_ckpt_gb_str = fmt_memory(
        vision_transformer_size, unit=GB, precision=1, commas=False
    )

    write_bw_gb_per_s_str = fmt_bandwidth(write_bw, unit=GB / second, precision=0, commas=False)

    # -- Write times
    archA_write_s_str = fmt_time(archA_write, second, precision=0, commas=False)
    dense20b_write_s_str = fmt_time(dense20b_write, second, precision=1, commas=False)
    bert_write_s_str = fmt_time(bert_write, second, precision=3, commas=False)
    archB_write_s_str = fmt_time(archB_write, second, precision=0, commas=False)
    resnet_write_s_str = fmt_time(resnet_write, second, precision=3, commas=False)
    vision_transformer_write_s_str = fmt_time(
        vision_transformer_write, second, precision=3, commas=False
    )

    # -- Optimal intervals (min for >= 60s, s for < 60s)
    archA_interval_min_str = fmt_time(archA_interval, "minute", precision=1, commas=False)
    dense20b_interval_min_str = fmt_time(
        dense20b_interval, "minute", precision=1, commas=False
    )
    bert_interval_s_str = fmt_time(bert_interval, second, precision=0, commas=False)
    archB_interval_min_str = fmt_time(archB_interval, "minute", precision=0, commas=False)
    resnet_interval_s_str = fmt_time(resnet_interval, second, precision=0, commas=False)
    vision_transformer_interval_s_str = fmt_time(
        vision_transformer_interval, second, precision=0, commas=False
    )

    # -- Save overhead percentages
    archA_overhead_pct_str = fmt_percent(
        archA_overhead_ratio, precision=1, commas=False, style='prose'
    )
    dense20b_overhead_pct_str = fmt_percent(
        dense20b_overhead_ratio, precision=1, commas=False, style='prose'
    )
    bert_overhead_pct_str = fmt_percent(
        bert_overhead_ratio, precision=2, commas=False, style='prose'
    )
    archB_overhead_pct_str = fmt_percent(
        archB_overhead_ratio, precision=1, commas=False, style='prose'
    )
    resnet_overhead_pct_str = fmt_percent(
        resnet_overhead_ratio, precision=2, commas=False, style='prose'
    )
    vision_transformer_overhead_pct_str = fmt_percent(
        vision_transformer_overhead_ratio, precision=2, commas=False, style='prose'
    )
```

| **Model Type**                   |                       **Checkpoint Size**                        | **Write Time (`{python} CheckpointOverheadCalc.write_bw_gb_per_s_str`)** |               **Optimal Interval (5hr GPU-only MTBF)**              |                           **Save Overhead**                           |
|:---------------------------------|:----------------------------------------------------------------:|:------------------------------------------------------------------------:|:-------------------------------------------------------------------:|:---------------------------------------------------------------------:|
...
| **ResNet-50**\index{ResNet-50}   |       `{python} CheckpointOverheadCalc.resnet_ckpt_mb_str`       |           `{python} CheckpointOverheadCalc.resnet_write_s_str`           |       `{python} CheckpointOverheadCalc.resnet_interval_s_str`       |       `{python} CheckpointOverheadCalc.resnet_overhead_pct_str`       |
...
An image classification cascade makes the trade-off concrete. The primary path can run the highest-quality model while the service has enough accelerator headroom. If latency, memory pressure, or dependency failures make that path unsafe, the serving layer can route to a smaller model that preserves coarse classification quality at lower cost. If even that path cannot meet the SLO, the service can return cached labels, a coarse category, or "classification unavailable" rather than blocking the request indefinitely.\index{Fallback} For that ladder to be real rather than aspirational, the production system must keep multiple models deployed, route requests to the appropriate level, monitor fallback frequency, and measure the quality impact of each level.
...
A training-loss signature is useful because it maps a curve to a recovery decision. A loss spike followed by recovery typically indicates a transient data issue or numerical instability; the job may continue, but the incident should be investigated to rule out a systematic input problem. A spike followed by a plateau is more serious because the learning rate may be too high, a checkpoint may be corrupted, or a data bug may have changed the training objective, so rollback to an earlier checkpoint may be necessary. Gradual divergence is harder to catch with threshold alerts because silent data corruption, hardware error, or distributed training desynchronization can accumulate slowly. A hang without an error points instead to a deadlock in collective communication or a crashed worker blocking synchronization, which means timeout detection becomes the diagnostic mechanism.

#### Serving failures {#sec-fault-tolerance-reliability-reliability-serving-failures-3322}

Serving failures require the same symptom-to-action mapping under much tighter latency constraints. Latency spikes can arise from resource contention, garbage collection, cold caches, or model reloads; individual spikes may be transient, but repeated occurrences indicate a capacity or placement problem. Rising error rates point to dependency failure, data-format changes, or model bugs and require immediate investigation because errors compound rapidly across dependent services. Silent quality degradation is different: model drift, feature degradation, or data-pipeline faults erode predictions without raising error-rate alarms, so the serving system needs quality monitoring beyond standard operational metrics. Cascade failures occur when one failing component causes others to fail through timeout exhaustion, resource depletion, or error propagation, which makes circuit breakers and isolation boundaries part of the fault-tolerance design rather than an after-the-fact operations layer.
