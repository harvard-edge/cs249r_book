...
```{python}
#| echo: false
from mlsysim.fmt import fmt_int, fmt_count, fmt_time, fmt_qty, fmt_qty_int
...
class CheckpointDebug:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    weights_gb_str = fmt_qty(m.parameters.m_as(param) * 2 * ureg.byte, GB, precision=0, commas=False)
    optimizer_gb_str = fmt_qty(m.parameters.m_as(param) * 8 * ureg.byte, GB, precision=0, commas=False)
    ...
    nvme_bw_gbs_str = fmt_qty(nvme_bw_gbs * GB/second, GB/second, precision=1, commas=False)
...
class RecoveryBudget:
    ...
    aggregate_read_gbs_str = fmt_qty(aggregate_ckpt_bw, GB/second, precision=0, commas=False)
    shard_read_gbs_str = fmt_qty(shard_read_gbs * GB/second, GB/second, precision=0, commas=False)
