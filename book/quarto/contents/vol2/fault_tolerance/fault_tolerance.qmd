...
Because many software faults leave the pipeline running while corrupting gradients, mitigation must assign each lifecycle phase a detection job. The prompt-truncation bug from the opening example should not survive until production monitoring: a unit test should catch the tokenizer boundary, an integration test should catch the batch shape and metadata path, a regression test should protect representative prompts, and runtime monitoring should detect any unexpected truncation distribution that escapes development. @Tbl-software-faults-summary-ft organizes these gates by the kind of corruption they are designed to catch.
...
: **Fault Mitigation Strategies**\index{Software Fault!mitigation strategies}: Software faults in ML systems require layered gates that catch different corruption modes: tokenizer boundary errors, batch-shape drift, unsafe slicing, dependency incompatibility, and runtime anomalies. {#tbl-software-faults-summary-ft}

These safeguards matter only when each gate targets a concrete corruption path. A unit test that checks only whether the tokenizer runs is too weak; it must assert that system prompts, labels, masks, and sequence-length metadata survive preprocessing with the intended semantics. Integration tests then follow one batch through loading, sharding, collation, and model input construction, while regression tests preserve representative prompts and edge cases before they enter distributed execution (@fig-regression-testing-ft). Automated CI/CD pipelines (@fig-CI-CD-procedure-ft) make those checks release gates instead of optional habits, reducing the chance that a valid-looking preprocessing change silently alters the training objective.\index{Pipeline!ci/cd}\index{Regression Test Automation}
...
A degradation plan must name which service property will be sacrificed before an incident begins. Quality is the most visible lever: the system serves predictions from simpler, faster models, such as falling back from a sophisticated multi-tower recommender to a collaborative-filtering model. Latency can also become the sacrifice; under high load, batching more requests together preserves throughput and model quality while accepting slower responses.

Other degradation dimensions preserve a useful answer by narrowing what the system promises. Coverage degradation returns partial results, such as top-10 search results instead of top-100. Freshness degradation serves cached or stale results rather than real-time computation, as when news recommendations use hour-old candidates during a recommendation-service outage. Feature degradation keeps inference running with fewer inputs, such as using content features when user history is unavailable.
...
Load shedding protects the system by sacrificing selected requests before overload spreads to every request. Random shedding is the simplest policy: drop a fraction of incoming requests and preserve enough capacity for the rest. Its weakness is that it treats all requests as equally valuable. Priority-based shedding adds product judgment by serving premium users before free users, revenue-generating requests before analytics requests, and interactive traffic before background batch work.
...
Track degradation metrics including the percentage of requests served by fallback models, the percentage of features using defaults, the request drop rate from load shedding, and the quality difference between primary and fallback paths. Alerting thresholds should reflect duration and blast radius: degradation activation can trigger an informational alert, sustained degradation beyond 5 minutes can trigger a warning, severe degradation affecting over 50 percent of requests can trigger a critical alert, and degradation beyond 1 hour should escalate. After the event, postincident analysis should identify the root cause, evaluate fallback effectiveness, measure user and business impact, and define improvements that prevent recurrence.
