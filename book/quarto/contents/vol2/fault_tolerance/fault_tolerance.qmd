...
3.  **Calculation**: $(P_{1h})^{N}$ $\approx$ **`{python} NinesReliability.cluster_prob_str`**.
...
$$ C_{\text{total}} = C_{\text{compute}} + E[N_{\text{failures}}] \times C_{per\_failure} + C_{ft} $$ {#eq-ft-total-cost}

where $C_{\text{compute}}$ is the base compute cost, $E[N_{\text{failures}}]$ is the expected number of failures during training, $C_{per\_failure}$ is the cost per failure, and $C_{ft}$ is the cost of fault tolerance mechanisms. The cost per failure includes wasted compute plus overhead.
...
$$ \eta_{new} = \eta_{base} \times \sqrt{\frac{N_{\text{new}}}{N_{\text{base}}}} $$ {#eq-lr-scaling}
