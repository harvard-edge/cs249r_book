...
2. **The optimal checkpoint interval is $\tau_{\text{opt}} = \sqrt{2 \times T_{\text{write}} \times \text{MTBF}}$.** The Young-Daly formula provides quantitative guidance for checkpoint frequency. This formula is derived in @sec-fault-tolerance-reliability-reliability-checkpoint-restart-fundamentals-0ac2.
...
Serving fault tolerance operates under constraints that training never faces. While @sec-inference-scale examines distributed serving architectures comprehensively, the focus here is specifically on fault tolerance and reliability mechanisms for real-time inference.
...
The three-state cycle in @fig-circuit-breaker is what prevents a single failing dependency from consuming the entire system's connection pool: the open state fails fast, and the half-open state probes for recovery before restoring full traffic.
