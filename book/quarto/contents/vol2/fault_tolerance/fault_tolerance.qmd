...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_math, sci_latex, check
...
class FleetReliabilityRates:
    ...
    gpu_mttf_str = fmt(gpu_mttf_h, precision=0)
    fleet_gpus_str = fmt(fleet_gpus, precision=0)
    fleet_fit_str = fmt(fleet_fit_total, precision=0)
    hours_between_str = fmt(hours_between, precision=0, commas=False)
...
[^fn-poisson-failure]: **Poisson Process**: A statistical model for events occurring independently at a constant average rate. Reliability models assume hardware failures follow a Poisson distribution, leading to the exponential survival function $R(t) = e^{-\lambda t}$. This assumption simplifies fleet planning: the probability of at least one failure in a 10,000-component cluster is $1 - e^{-N \lambda t}$, making failure risk scale linearly with $N$ but exponentially with $t$. \index{Poisson Process!failure modeling}

[^fn-failure-rate-fits]: **Failure Rate ($\lambda$)**: Expressed in FITs (Failures In Time), where 1 FIT equals one failure per billion device-hours. A data center GPU with `{python} FleetReliabilityRates.gpu_mttf_str`-hour MTBF has `{python} FleetReliabilityRates.fit_eq_math`, corresponding to `{python} FleetReliabilityRates.lambda_eq_math` failures/hour. That seems negligible for one device but becomes dominant at fleet scale: a `{python} FleetReliabilityRates.fleet_gpus_str`-GPU cluster accumulates `{python} FleetReliabilityRates.fleet_fit_str` FITs from GPUs alone, translating to an expected GPU failure every `{python} FleetReliabilityRates.hours_between_str` hours. \index{Failure Rate!FITs}

$$ R_{\text{single}}(t) = e^{-\lambda t} $$ {#eq-single-component-reliability}

The mean time between failures (MTBF) for this component equals $1/\lambda$. Modern GPUs in data center environments exhibit MTBF values ranging from 40,000 to 100,000 hours depending on operating conditions, cooling effectiveness, and manufacturing variation.[^fn-gpu-mtbf-field] @sec-appdx-reliability-foundations-component-rates catalogs the canonical per-component MTTF values (H100, A100, TPU, PCIe, power, network) that anchor these $\lambda$ and FIT figures, so a reader can substitute the constant for any device and reproduce the rates used throughout this chapter.

[^fn-gpu-mtbf-field]: **GPU MTBF Variation**\index{GPU!mtbf variation}: NVIDIA specifies A100 MTBF at approximately 50,000 hours under controlled conditions, but fleet telemetry from Google (Tensor Processing Unit (TPU) pods) and Meta (GPUs) consistently shows field failure rates 20--50 percent higher than manufacturer specifications. The gap arises from thermal stress, power fluctuations, and memory-intensive ML workloads that push components beyond the steady-state assumptions in reliability models. \index{MTBF!GPU field variation}
...
The answer is given by the Young-Daly checkpoint law (Principle \ref{pri-young-daly}): the optimal checkpoint interval $\tau_{\text{opt}} = \sqrt{2 \cdot T_{\text{write}} \cdot \text{MTBF}_{\text{system}}}$ balances the cost of writing checkpoints against the expected cost of reworking lost progress. As cluster size grows, system MTBF drops, forcing more frequent checkpoints—demanding high-bandwidth storage to prevent I/O from dominating training time. @Sec-appdx-reliability-foundations-young-daly carries out the full derivation and the tighter second-order bounds, so a reader who wants the assumptions behind this square-root law, rather than the result alone, can follow the argument step by step.

As @fig-young-daly illustrates, the **Young-Daly formula**[^fn-young-daly-history] identifies the "sweet spot" that minimizes total wasted work.
...
The formula $\tau_{\text{opt}} = \sqrt{2 \cdot T_{\text{write}} \cdot \text{MTBF}_{\text{system}}}$ reveals a critical scaling property: as clusters grow larger ($\text{MTBF}_{\text{system}} \downarrow$), we must checkpoint more frequently. This, in turn, demands higher-bandwidth storage systems (@sec-data-storage) to keep $T_{\text{write}}$ small, otherwise the "checkpoint tax" will consume most of the cluster's compute capacity, as @fig-checkpoint-tax quantifies.
...
3.  **Calculation**: $(R_{\text{GPU}})^{N} \approx$ `{python} NinesReliability.cluster_prob_str`.
...
Complex ML systems comprise multiple components whose individual failure rates combine to determine overall system reliability. For **series systems**\index{Series Systems} (for example, a node where all 8 GPUs must work), failure of any component causes system failure. The system reliability $R_{\text{system}}(t)$ is the product of individual component reliabilities:
$$R_{\text{system}}(t) = \prod_{i=1}^{N} R_i(t)$$
The system MTBF for a series configuration is $\text{MTBF}_{\text{system}} = 1/\sum \lambda_i$. This reveals the scaling challenge: adding components reduces system MTBF linearly.

For **parallel systems**\index{Parallel Systems} providing redundancy (for example, active-active model replicas), failure occurs only when all redundant components fail simultaneously:
...
Consider training an Archetype A (GPT-4/Llama-3) 70B model (@sec-vol2-introduction-archetypes) on 1,024 A100 GPUs.
*   **GPU Logic**\index{GPU!logic}: 4,000 FIT per GPU $\times$ 1,024 = 4,096,000 FIT $\rightarrow$ MTBF $\approx$ 244 hours.
*   **HBM2 Memory**\index{Memory!hbm2}: 250 FIT per Mb. Total Mb = $1,024 \times 80 \text{ GB} \times 8 \times 1,024 \approx 671 \text{M Mb}$. Total FIT $\approx$ 167B FIT $\rightarrow$ MTBF $\approx$ 21 seconds.

Without ECC memory protection, large-scale training is impossible as corruption would occur every 21 seconds. With ECC providing a 100$\times$ reduction in soft error rates, the memory MTBF improves to 35 minutes, and the combined system MTBF becomes approximately 35 minutes. This quantitative analysis justifies checkpoint frequencies of 15--30 minutes for large-scale training.
...
# │
# │ Imports: mlsysim.constants (Systems), mlsysim.fmt (fmt, fmt_int, MarkdownStr, check)
# │ Exports: setup strs (fleet_gpus_str, {gpu,pcie,psu,nic}_mttf_str, gpus_per_node_str);
# │          7 *_eq_math display equations; mtbf_h_str, per_day_str, per_week_str
# └─────────────────────────────────────────────────────────────────────────────
...
class FailureCascadeExample:
    ...
    _gpu_d = gpu_mttf_str.replace(",", "{,}")
    _pcie_d = pcie_mttf_str.replace(",", "{,}")
    _psu_d = psu_mttf_str.replace(",", "{,}")
    _nic_d = nic_mttf_str.replace(",", "{,}")
    ...
    mtbf_eq_math = MarkdownStr(
        rf"$$ \text{{MTBF}}_{{\text{{system}}}} = \frac{{1}}{{{l_system_disp:.4f}}} = {mtbf_h:.2f} \text{{ hours}} $$")
    mtbf_h_str = fmt(mtbf_h, precision=1, commas=False)
    per_day_str = fmt(per_day, precision=1, commas=False)
    per_week_str = fmt_int(per_week)
```
...
- Individual GPU MTBF: `{python} FailureCascadeExample.gpu_mttf_str` hours
- Each GPU connected to host via PCIe (MTBF: `{python} FailureCascadeExample.pcie_mttf_str` hours)
- Each node contains `{python} FailureCascadeExample.gpus_per_node_str` GPUs with shared power supply (MTBF: `{python} FailureCascadeExample.psu_mttf_str` hours)
- Network infrastructure per node (NIC, cables): MTBF `{python} FailureCascadeExample.nic_mttf_str` hours
...
: **Cluster MTBF Scaling**\index{Cluster!mtbf scaling}: GPU-only mean time between failures decreases linearly with cluster size, transforming failures from rare events to continuous operating conditions at scale. Additional host, power, storage, and network failure domains reduce system-level MTBF further. {#tbl-cluster-mtbf-scaling}
...
ax.plot(n_theoretical, mtbf_theoretical, color=COLORS['grid'], linestyle='--',
        linewidth=1.5, label=f'Theoretical: {Systems.Reliability.Gpu.mttf_hours:,}h/N', zorder=1)
...
- **Network packet loss**\index{Network Packet Loss}: Momentary congestion causes dropped packets, but retransmission succeeds
- **Memory bit flips**\index{Memory!bit flips}: Cosmic ray induced single-event upsets[^fn-seu-rate] corrupt individual bits
...
- **Thermal throttling**\index{Thermal Throttling}: Temporary performance reduction due to temperature spikes
- **Software timeouts**\index{Software Timeouts}: Temporary resource contention causes operation delays
...
- **GPU hardware failure**\index{GPU!hardware failure}: Memory errors cause device to become unresponsive
- **Node crash**\index{Node!crash}: Operating system failure terminates all processes
- **Network partition**: Physical or logical disconnection isolates node from cluster
- **Storage failure**\index{Storage!failure}: Disk failure prevents checkpoint read/write operations
...
Detection time $T_{\text{detect}}$ typically involves heartbeat mechanisms where each worker periodically signals liveness to a coordinator. If no heartbeat arrives within the timeout period $T_{\text{timeout}}$, the coordinator declares failure. Setting $T_{\text{timeout}}$ requires balancing false positive rate against detection latency. False positives declare healthy workers failed due to transient delays, while slow detection wastes compute during the detection window.
...
where $k$ typically ranges from three to five to achieve low false positive rates while maintaining reasonable detection speed.

#### Byzantine failures {#sec-fault-tolerance-reliability-reliability-byzantine-failures-8a24}

Where a fail-stop component simply goes silent and is detected and replaced, a far more insidious class keeps running but produces incorrect results. A GPU that returns wrong gradients without throwing errors, a network that delivers corrupted packets that pass CRC checks, or a worker that computes different results for identical inputs all exemplify **Byzantine failures**\index{Byzantine Failures}, the most challenging class in distributed systems.
...
- [ ] Why is silent data corruption (SDC) more dangerous for ML training than a simple node crash?
- [ ] How does a **hot spare** redundancy strategy differ from a traditional **checkpoint-restart** approach in terms of recovery latency?
- [ ] In the Google case study, which metric was used as a "sanity checker" to identify potential hardware faults: GPU Temperature or Gradient Norm?
- [ ] True or False: Byzantine failures only occur when a malicious actor intentionally sabotages the training cluster.

:::

Byzantine failures include:

- **Silent data corruption**\index{Silent Data Corruption!mechanism}: Memory or computation errors produce wrong values without triggering errors
- **Numerical instability**\index{Numerical Instability}: Floating-point edge cases cause gradients to become not a number (NaN) or infinity
- **Determinism violations**\index{Determinism Violations}: Race conditions cause different workers to compute different results for identical inputs
...
- **Power supply failure**\index{Power Supply Failure}: All GPUs in a node lose power simultaneously
- **Network switch failure**\index{Network Switch Failure}: All nodes connected to the switch become unreachable
- **Cooling system failure**\index{Cooling!system failure}: Thermal shutdown affects multiple racks
- **Software bugs**\index{Software Bugs}: A bug in the CUDA driver crashes all processes using that driver version
- **Operator error**\index{Operator Error}: Misconfiguration affects entire cluster
...
: **Failure Domains in ML Infrastructure**\index{Failure Domain!ML infrastructure}: Understanding failure domain boundaries enables placement of redundant components across independent domains, preventing correlated failures from defeating redundancy strategies. {#tbl-failure-domains}
...
- [ ] Why does placing all 3 model replicas in the same Zone (for example, `us-east-1a`) fail to provide protection against a regional power outage?
- [ ] Which failure domain is uniquely addressed by **Staged Rollouts** rather than hardware redundancy?
- [ ] Can you explain the concept of **Hierarchical Containment**? If a Zone fails, does the corresponding Rack failure domain still matter for that event?
...
::: {#ws-fault-tolerance-google-tpuv4-routing .callout-war-story title="The supercomputer that rerouted around faults"}
...
**Failure**: At that scale, a failed machine, chip, ICI cable, or optical circuit switch was not merely a lost component. It could disrupt the topology that synchronous training jobs relied on for collective communication.
...
: **Failure Impact by Model Type**\index{Failure!model-type impact}: The cost of training failures varies dramatically by model type, driven by training duration, checkpoint overhead, and the value of accumulated training state. These differences demand model-specific fault tolerance strategies. {#tbl-failure-impact-by-model}
...
Large Language Models experience the highest absolute failure costs due to their extended training durations and the computational expense of each training hour. A frontier-scale training run consuming 25,000 GPUs at approximately \$2 per GPU-hour incurs \$1.2M in compute costs per day. A failure that loses 24 hours of training progress costs \$1.2M in wasted compute plus schedule delay. The checkpoint overhead spans a wide range: 70B-class dense LLM checkpoints can reach hundreds of gigabytes, and the 175B-parameter running example used later in this chapter reaches `{python} FaultToleranceIntroRecap.gpt3_ckpt_tb_str` once FP32 Adam optimizer state is included.
...
The temporal signature of a hardware fault determines the recovery response, so the taxonomy must serve detection and mitigation rather than naming alone. @Fig-fault-temporal-categories summarizes the three categories that matter operationally.
...
**Transient faults**\index{Transient Fault} are temporary disruptions caused by external factors such as cosmic rays or electromagnetic interference [@ziegler1996]; they cause incorrect computations without permanent hardware damage and can corrupt gradient updates during training or alter model weights during inference. **Permanent faults**\index{Permanent Fault} represent irreversible damage from physical defects or component wear-out, such as stuck-at faults or device failures that require hardware replacement; for long-running training jobs, they can mean days or weeks of lost computation. **Intermittent faults**\index{Intermittent Fault} appear and disappear sporadically due to unstable conditions like loose connections or aging components, causing nondeterministic behavior that compromises model validation and reproducibility.

### Transient faults {#sec-ft-transient-faults-1455}

Transient faults are the most common category. @Fig-bit-flip illustrates the basic mechanism: a single bit in memory unexpectedly changes state, potentially altering critical data or computations in ways that cascade through neural network layers.\index{Bit-Flip Error}
...
Transient faults encompass several distinct categories: Single Event Upsets (SEUs) from cosmic rays and ionizing radiation, voltage fluctuations [@reddi2013] from power supply instability, electromagnetic interference (EMI), electrostatic discharge (ESD), crosstalk, ground bounce, timing violations, and soft errors in combinational logic [@mukherjee2005].
...
[^fn-mtbf-checkpoint]: **MTBF (Mean Time Between Failures)**: Formalized by the U.S. military in MIL-HDBK-217 (1965), MTBF assumes exponential failure distributions during the useful-life period. For ML training, MTBF feeds directly into the Young-Daly formula: a cluster with 50,000-hour per-device MTBF and 1,000 devices has a system MTBF of 50 hours, demanding checkpoints every 1--2 hours to keep wasted compute below 1 percent of total training time. \index{MTBF!checkpoint frequency}

[^fn-gpu-fault-rates-training]: **GPU Fault Rates**\index{GPU!fault rates}: GPUs concentrate thousands of execution lanes and high-bandwidth memory stacks in a single package. ML accelerators also run sustained high-power kernels with large volumes of weight, activation, and gradient data in flight, so transient faults can affect both computation and communication state. \index{GPU!fault rate}
...
# │ How: Multiply base bandwidth by (1 - overhead fraction). GDDR6X has no ECC.
# │
# │ Imports: mlsysim.fmt (fmt, fmt_int, MarkdownStr, check)
# │ Exports: MemBandwidthProtection.ddr4_eff_str,
# │   MemBandwidthProtection.hbm2_eff_str,
# │   MemBandwidthProtection.hbm3_eff_str,
# │   MemBandwidthProtection.gddr6x_eff_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_int, MarkdownStr, check
...
class MemBandwidthProtection:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    # Base bandwidths (GB/s) — not in mlsysim constants; hardcoded from specs
    ddr4_base_gbps = 51.2
    hbm2_base_gbps = 900
    hbm3_base_gbps = 1600
    gddr6x_base_gbps = 760

    ecc_overhead = 0.125  # 12.5% ECC overhead

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    ddr4_eff = ddr4_base_gbps * (1 - ecc_overhead)
    hbm2_eff = hbm2_base_gbps * (1 - ecc_overhead)
    hbm3_eff = hbm3_base_gbps * (1 - ecc_overhead)
    gddr6x_eff = gddr6x_base_gbps  # no ECC

    # ┌── 3. GUARD (Invariants) ────────────────────────────────────────────
    check(abs(ddr4_eff - 44.8) < 0.01, f"DDR4 effective BW mismatch: {ddr4_eff}")
    check(abs(hbm2_eff - 787.5) < 0.01, f"HBM2 effective BW mismatch: {hbm2_eff}")
    check(abs(hbm3_eff - 1400) < 0.01, f"HBM3 effective BW mismatch: {hbm3_eff}")
    check(gddr6x_eff == 760, f"GDDR6X effective BW mismatch: {gddr6x_eff}")

    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────
    ddr4_eff_str = fmt(ddr4_eff, precision=1, commas=False)
    hbm2_eff_str = fmt(hbm2_eff, precision=1, commas=False)
    hbm3_eff_str = fmt(hbm3_eff, precision=0)
    gddr6x_eff_str = fmt_int(gddr6x_eff)
```

| **Memory Technology** | **Base Bandwidth** **(GB/s)** | **ECC Overhead** **(%)** |               **Effective** **Bandwidth (GB/s)** |
|:----------------------|------------------------------:|-------------------------:|-------------------------------------------------:|
| **DDR4-3200**         |                          51.2 |             12.5 percent |   `{python} MemBandwidthProtection.ddr4_eff_str` |
| **HBM2**              |                           900 |             12.5 percent |   `{python} MemBandwidthProtection.hbm2_eff_str` |
| **HBM3**              |                         1,600 |             12.5 percent |   `{python} MemBandwidthProtection.hbm3_eff_str` |
| **GDDR6X**            |                           760 |           Typically none | `{python} MemBandwidthProtection.gddr6x_eff_str` |

: **Memory Bandwidth Protection Analysis**\index{Memory Bandwidth!protection analysis}: Impact of ECC protection on effective memory bandwidth across different memory technologies used in ML accelerators. The bandwidth overhead directly affects training throughput for memory-bound workloads. {#tbl-memory-bandwidth-protection}
...
@Fig-transient-fault illustrates the physical mechanism: cosmic rays strike sensitive hardware areas like memory cells or transistors, inducing charge disturbances that alter stored or transmitted data.\index{Transient Fault!mechanism}
...
For ML systems, permanent faults in accelerator datapaths can cause gradient calculation errors and parameter corruption that persist until hardware replacement, requiring more sophisticated recovery strategies than transient faults demand [@he2023; @zhang2018]. Permanent faults in storage can compromise training datasets or saved checkpoints, so training systems also need checksum validation, replication, and checkpoint-restart recovery. Mitigating permanent faults requires integrated fault-tolerant design combining hardware redundancy and error-correcting codes [@kim2015] with checkpoint and restart mechanisms[^fn-checkpoint-restart-training] [@egwutuoha2013]. The Young-Daly formula, introduced in @sec-fault-tolerance-young-daly, balances checkpoint overhead against lost computation; the key insight is that increasing MTBF through hardware hardening yields diminishing returns due to the square-root relationship, so systems must balance investment in hardware reliability against investment in fast checkpointing infrastructure.

[^fn-checkpoint-restart-training]: **Checkpoint-restart**: Originated in 1960s mainframe batch processing, where restarting multi-hour jobs from scratch was prohibitively expensive. Modern distributed training checkpoints 100+ GB model states every 10--30 minutes; Google’s TPU pods achieve 99.9 percent training completion rates through coordinated checkpointing, reducing wasted computation from node failures to less than 1 percent of total training time. \index{Checkpoint-Restart!training completion}
...
For ML systems, intermittent faults in processing units or memory cause sporadic gradient computation errors that accumulate across iterations, degrading convergence without triggering explicit failures [@he2023; @rashid2015]. Mitigating these faults requires a multi-layered approach [@rashid2012]: runtime monitoring and anomaly detection at the software level, robust environmental controls and higher-quality components at the hardware level, and adaptive resource management (load balancing, dynamic scaling) to maintain operation during fault episodes.
...
[^fn-dmr-detection]: **DMR (Double Modular Redundancy)**: Duplicates computation and compares outputs to detect (but not correct) errors, at 100 percent silicon overhead vs. TMR's 200 percent. DMR detects 99.99 percent of transient faults in critical paths. Tesla's Full Self-Driving computer uses DMR across two independent SoCs, reflecting the design trade-off: DMR halves the hardware cost of TMR while requiring a safe fallback policy when outputs disagree. \index{DMR!detection trade-off}

[^fn-tmr-correction]: **TMR (Triple Modular Redundancy)**: Performs computation three times and takes a majority vote, enabling automatic single-fault correction at 200 percent hardware overhead. First applied in early fault-tolerant mainframes in the 1950s, TMR remains the standard for space-grade ML inference where cosmic radiation rates are orders of magnitude higher than at sea level, achieving error rates below $10^{-12}$ per operation. \index{TMR!majority voting}
...
At the software level, distributed ML systems employ runtime monitoring [@francalanza2017; @mahmoud2021], anomaly detection (statistical outlier detection, One-Class SVM [@chandola2009]), data-consistency checks for model validation [@lindholm2019], and heartbeat mechanisms [@kawazoe_aguilera1997] that detect node failures within configurable timeout periods. Software-implemented fault tolerance techniques such as SWIFT (Software Implemented Fault Tolerance) [@reis2005swift], N-version programming, and Reed-Solomon error correction codes [@plank1997; @reed1960] add redundancy at the software level, enabling detection and correction of errors without dedicated hardware. Watchdog timers [@pont2002using] monitor task execution and trigger recovery actions when systems become unresponsive.
...
Because many software faults leave the pipeline running while corrupting gradients, mitigation must assign each lifecycle phase a detection job. @Tbl-software-faults-summary-ft organizes those detection and mitigation approaches by lifecycle phase.
...
: **Fault Mitigation Strategies**\index{Software Fault!mitigation strategies}: Software faults in ML systems require layered detection and mitigation techniques applied throughout the development lifecycle, from initial testing to ongoing monitoring, to ensure reliability and robustness. {#tbl-software-faults-summary-ft}

These safeguards matter only when each gate targets a concrete corruption path. Systematic testing (unit, integration, and regression) catches faults before they enter distributed execution (@fig-regression-testing-ft). Automated CI/CD pipelines (@fig-CI-CD-procedure-ft) embed testing, validation, and monitoring directly into the software delivery process, with gates at each stage that reduce the risk of unnoticed regressions.\index{Pipeline!ci/cd}\index{Regression Test Automation}
...
**Systems insight**: In a 100k-GPU fleet, a silent error occurs every 18,000 steps (roughly every 10 hours). If the AllReduce path does not implement checksummed collectives or hash-and-verify gradients, model parameters silently accumulate corrupted contributions and drift within half a day. Robustness moves from being a *restart* problem to a *verification* problem: the fleet must perform redundant reductions or use parity-protected gradients to catch silent parameter corruption.
...
Hardware-based fault injection\index{Hardware-Based Fault Injection} is the calibration point for the experiment: it introduces faults into physical systems so software-level assumptions can be checked against real hardware behavior. While most error injection tools used in ML robustness research are software-based because of their speed and scalability, hardware-based approaches remain critical for grounding higher-level error models.
...
[^fn-fpga-fault-injection-ft]: **FPGA (Field-Programmable Gate Array)**: Reconfigurable hardware containing millions of programmable logic blocks. For fault injection, FPGAs provide bit-level targeting precision that software-based tools cannot match. \index{FPGA!fault injection}
...
**Answer**: Software tools often miss **masking effects**\index{Masking Effect} at the circuit level. A bit flip in a hardware register might be corrected by ECC memory or masked by logical gates before it ever reaches the software layer. Software tools that inject faults directly into variables bypass these hardware-level natural defenses, potentially reporting a higher vulnerability than exists in reality.
...
@Tbl-fault_types closes the loop from fault model to recovery choice. It compares transient, permanent, and intermittent faults across duration, persistence, causes, and ML system impact so the detection and mitigation strategy matches the fault category being tested.
...
1.  **Significance (quantitative)**: It minimizes the Lost Work after a system failure. Within the iron-law framework formalized in @sec-performance-engineering-efficiency-frontier, checkpointing creates an I/O Overhead that reduces effective hardware utilization $(\eta_{\text{hw}})$, with the optimal interval $(\tau_{\text{opt}})$ governed by the Young-Daly Formula $(\tau_{\text{opt}} = \sqrt{2 \cdot T_{\text{write}} \cdot \text{MTBF}_{\text{system}}})$.
...
class FaultToleranceCheckpointRecap:
    ...
    aggregate_ckpt_bw = 100 * (GB / second)
    ...
    aggregate_read_gbs_str = fmt_qty(aggregate_ckpt_bw, GB / second, precision=0, commas=False)
```

[^fn-adam-state-memory]: **Adam Optimizer State**\index{Adam Optimizer!state}: Adaptive Moment Estimation (Adam) maintains per-parameter first-moment ($m$) and second-moment ($v$) estimates, tripling memory requirements compared to vanilla SGD (3$\times$: parameters + two state vectors). For a `{python} FaultToleranceCheckpointRecap.gpt3_params_b_str` parameter model, optimizer state alone reaches `{python} FaultToleranceCheckpointRecap.gpt3_adam_tb_str`, which typically dominates checkpoint size and recovery time, making optimizer state the primary bottleneck in checkpoint I/O design. \index{Adam Optimizer!checkpoint memory}
...
The Young-Daly formula introduced in @sec-fault-tolerance-young-daly provides the optimal checkpoint interval: $\tau_{\text{opt}} = \sqrt{2 \times T_{\text{write}} \times \text{MTBF}_{\text{system}}}$. The failure analysis in this chapter enables the calculation of the system MTBF term required by this formula.

::: {#nbk-fault-tolerance-optimal-checkpoint-interval .callout-notebook title="Optimal checkpoint interval"}
...
*   **Checkpoint Cost** $(T_{\text{write}})$: It takes 2 minutes to save the model state to shared storage.
*   **Mean Time Between Failures** $(\text{MTBF}_{\text{system}})$: At this scale, the cluster experiences a silent data corruption or node failure every 3 hours (180 minutes).

**Objective**\index{Objective}: Find the optimal checkpoint interval $\tau_{\text{opt}}$ that minimizes wasted time.
...
**Result**: The optimal strategy is to checkpoint roughly every 27 minutes.

*   **Checkpointing every 10 mins**: Too much time spent writing to disk (20 percent save overhead, 22.8 percent total overhead).
*   **Checkpointing every 60 mins**: Too much work lost when the inevitable crash happens (16.7 percent expected rework, 20 percent total overhead).
*   **At 27 mins**: The combined cost of overhead and recovery is minimized ($\sim 15\%$ total overhead, with checkpoint cost and rework cost balanced at $\sim 7.5\%$ each).

:::

@Fig-checkpoint-tradeoff illustrates the fundamental trade-off: checkpointing too frequently wastes time saving state, while checkpointing too rarely wastes computation when failures occur. The Young-Daly formula identifies the optimal balance point.
...
ax.plot(
    tau * 60,
    over_ckpt,
    label='Checkpoint Overhead ($T_{\\text{write}}/\\tau_{\\text{ckpt}}$)',
    color=COLORS['BlueLine'],
    linestyle='--'
)
...
The U-shaped cost curve in @fig-young-daly visualizes this trade-off: checkpoint overhead decreases as intervals lengthen, while rework cost from failures increases linearly. The optimal point minimizes their sum.

For our 10,000-GPU cluster with calculated system MTBF of 3.69 hours (from @sec-fault-tolerance-reliability-reliability-worked-example-cluster-mtbf-calculation-9255) and checkpoint time of `{python} FaultToleranceCheckpointRecap.ckpt_write_s_str` (from `{python} FaultToleranceCheckpointRecap.gpt3_ckpt_tb_str` checkpoint at `{python} FaultToleranceCheckpointRecap.aggregate_read_gbs_str`), the Young-Daly formula computes the optimal interval.
...
# │
# │ Imports: mlsysim (constants, fmt, solver)
...
class YoungDalyOptimal:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    t_write = (_mem.total_memory / (100 * (GB / second))).to(second)
    tau_opt = (2 * t_write * mtbf) ** 0.5
    ...
    seconds_per_hour_str = fmt(SEC_PER_HOUR, precision=0, commas=False)
...
1. **Convert to common units**: $\text{MTBF}_{\text{system}} =$ `{python} YoungDalyOptimal.mtbf_h_str` $\times$ `{python} YoungDalyOptimal.seconds_per_hour_str` s/hr = `{python} YoungDalyOptimal.mtbf_s_str`.
...
This result demonstrates why failure analysis matters: without knowing the system MTBF, we cannot set checkpoint intervals rationally. With this interval, checkpoint overhead consumes approximately 2.8 percent of training time, but the estimate depends on assumptions that often fail in production.

::: {.callout-perspective title="Young-Daly formula assumptions"}
...
*   **Idle Resource**\index{Idle Resource}: 10,000 H100 GPUs.
*   **Pause Duration**\index{Pause Duration}: 2 minutes (typical for multi-TB checkpoints on shared storage).
*   **Wasted Compute**\index{Wasted Compute}: $10,000 \times \frac{2}{60} \approx 333$ GPU-hours.
*   **Financial Loss**\index{Financial Loss}: At ~\$3/GPU-hour, a single checkpoint costs \$1,000 in wasted idle time.

If checkpoints occur hourly, the system wastes \$24,000 per day waiting for storage I/O. This economic reality drives the aggressive adoption of asynchronous checkpointing strategies that move data movement off the critical path. @Tbl-checkpoint-overhead-by-model reveals how checkpoint characteristics vary dramatically by model architecture: larger models require longer write times but also benefit from correspondingly longer optimal intervals.
...
# │ How: write_time = size / bandwidth; tau_opt = sqrt(2 * write_time * MTBF);
# │      overhead = write_time / tau_opt * 100.
# │
# │ Imports: mlsysim.fmt (fmt, MarkdownStr, check), math
# │ Exports: CheckpointOverheadCalc.{archA,gpt35,bert,archB,resnet,vit}_*_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_int, MarkdownStr, check, fmt_time
import math


class CheckpointOverheadCalc:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────────
    # Checkpoint sizes in GB for each model archetype
    archA_size_gb = 2100       # 175B dense LLM: ~12 bytes/param (mixed precision + optimizer)
    gpt35_size_gb = 240        # 20B model
    bert_size_gb = 1.3         # BERT-Large
    archB_size_gb = 4000       # DLRM at scale (embedding-heavy)
    resnet_size_gb = 0.1       # ResNet-50 (100 MB)
    vit_size_gb = 1.2          # ViT-Large

    write_bw_gbs = 100         # Write bandwidth: 100 GB/s
    mtbf_hours = 5             # GPU-only MTBF: 5 hours
    mtbf_s = mtbf_hours * 3600  # = 18000 seconds

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────────
    # Write time = checkpoint_size / write_bandwidth
    archA_write_s = archA_size_gb / write_bw_gbs
    gpt35_write_s = gpt35_size_gb / write_bw_gbs
    bert_write_s = bert_size_gb / write_bw_gbs
    archB_write_s = archB_size_gb / write_bw_gbs
    resnet_write_s = resnet_size_gb / write_bw_gbs
    vit_write_s = vit_size_gb / write_bw_gbs

    # Optimal interval = sqrt(2 * write_time * MTBF) [Young-Daly formula]
    archA_interval_s = math.sqrt(2 * archA_write_s * mtbf_s)
    gpt35_interval_s = math.sqrt(2 * gpt35_write_s * mtbf_s)
    bert_interval_s = math.sqrt(2 * bert_write_s * mtbf_s)
    archB_interval_s = math.sqrt(2 * archB_write_s * mtbf_s)
    resnet_interval_s = math.sqrt(2 * resnet_write_s * mtbf_s)
    vit_interval_s = math.sqrt(2 * vit_write_s * mtbf_s)

    # Save overhead = write_time / optimal_interval * 100
    archA_overhead_pct = (archA_write_s / archA_interval_s) * 100
    gpt35_overhead_pct = (gpt35_write_s / gpt35_interval_s) * 100
    bert_overhead_pct = (bert_write_s / bert_interval_s) * 100
    archB_overhead_pct = (archB_write_s / archB_interval_s) * 100
    resnet_overhead_pct = (resnet_write_s / resnet_interval_s) * 100
    vit_overhead_pct = (vit_write_s / vit_interval_s) * 100

    # ┌── 3. GUARD (Invariants) ────────────────────────────────────────────────
    check(archA_interval_s > 0, "Archetype A interval must be positive")
    check(gpt35_interval_s > 0, "GPT-3.5 interval must be positive")
    check(bert_interval_s > 0, "BERT interval must be positive")
    check(archB_interval_s > 0, "Archetype B interval must be positive")
    check(resnet_interval_s > 0, "ResNet-50 interval must be positive")
    check(vit_interval_s > 0, "ViT-Large interval must be positive")
    check(archA_overhead_pct < 10, "Archetype A overhead must be < 10%")
    check(gpt35_overhead_pct < 10, "GPT-3.5 overhead must be < 10%")
    check(bert_overhead_pct < 10, "BERT overhead must be < 10%")
    check(archB_overhead_pct < 10, "Archetype B overhead must be < 10%")
    check(resnet_overhead_pct < 10, "ResNet-50 overhead must be < 10%")
    check(vit_overhead_pct < 10, "ViT-Large overhead must be < 10%")

    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────────
    # -- Checkpoint sizes
    archA_size_str = MarkdownStr("2.1 TB")
    gpt35_size_str = MarkdownStr("240 GB")
    bert_size_str = MarkdownStr("1.3 GB")
    archB_size_str = MarkdownStr("4 TB")
    resnet_size_str = MarkdownStr("100 MB")
    vit_size_str = MarkdownStr("1.2 GB")

    # -- Write times
    archA_write_s_str = fmt_time(archA_write_s, 'second', precision=0, commas=False)
    gpt35_write_s_str = fmt_time(gpt35_write_s, 'second', precision=1, commas=False)
    bert_write_s_str = fmt_time(bert_write_s, 'second', precision=3, commas=False)
    archB_write_s_str = fmt_time(archB_write_s, 'second', precision=0, commas=False)
    resnet_write_s_str = fmt_time(resnet_write_s, 'second', precision=3, commas=False)
    vit_write_s_str = fmt_time(vit_write_s, 'second', precision=3, commas=False)

    # -- Optimal intervals (min for >= 60s, s for < 60s)
    archA_interval_min_str = fmt_time(archA_interval_s / 60, 'minute', precision=1, commas=False)
    gpt35_interval_min_str = fmt_time(gpt35_interval_s / 60, 'minute', precision=1, commas=False)
    bert_interval_s_str = fmt_time(round(bert_interval_s), 'second', precision=0, commas=False)
    archB_interval_min_str = fmt_time(round(archB_interval_s / 60), 'minute', precision=0, commas=False)
    resnet_interval_s_str = fmt_time(round(resnet_interval_s), 'second', precision=0, commas=False)
    vit_interval_s_str = fmt_time(round(vit_interval_s), 'second', precision=0, commas=False)

    # -- Save overhead percentages
    archA_overhead_str = fmt_percent(archA_overhead_pct/100, precision=1, commas=False, style='prose')
    gpt35_overhead_str = fmt_percent(gpt35_overhead_pct/100, precision=1, commas=False, style='prose')
    bert_overhead_str = fmt_percent(bert_overhead_pct/100, precision=2, commas=False, style='prose')
    archB_overhead_str = fmt_percent(archB_overhead_pct/100, precision=1, commas=False, style='prose')
    resnet_overhead_str = fmt_percent(resnet_overhead_pct/100, precision=2, commas=False, style='prose')
    vit_overhead_str = fmt_percent(vit_overhead_pct/100, precision=2, commas=False, style='prose')
```

| **Model Type**                   |                               **Checkpoint Size** |                            **Write Time (100 GB/s)** |                 **Optimal Interval (5hr GPU-only MTBF)** |                                     **Save Overhead** |
|:---------------------------------|--------------------------------------------------:|-----------------------------------------------------:|---------------------------------------------------------:|------------------------------------------------------:|
| **Archetype A (175B dense LLM)** |  `{python} CheckpointOverheadCalc.archA_size_str` |  `{python} CheckpointOverheadCalc.archA_write_s_str` | `{python} CheckpointOverheadCalc.archA_interval_min_str` |  `{python} CheckpointOverheadCalc.archA_overhead_str` |
| **GPT-3.5 (20B)**                |  `{python} CheckpointOverheadCalc.gpt35_size_str` |  `{python} CheckpointOverheadCalc.gpt35_write_s_str` | `{python} CheckpointOverheadCalc.gpt35_interval_min_str` |  `{python} CheckpointOverheadCalc.gpt35_overhead_str` |
| **BERT-Large**                   |   `{python} CheckpointOverheadCalc.bert_size_str` |   `{python} CheckpointOverheadCalc.bert_write_s_str` |    `{python} CheckpointOverheadCalc.bert_interval_s_str` |   `{python} CheckpointOverheadCalc.bert_overhead_str` |
| **Archetype B (DLRM at Scale)**  |  `{python} CheckpointOverheadCalc.archB_size_str` |  `{python} CheckpointOverheadCalc.archB_write_s_str` | `{python} CheckpointOverheadCalc.archB_interval_min_str` |  `{python} CheckpointOverheadCalc.archB_overhead_str` |
| **ResNet-50**\index{ResNet-50}   | `{python} CheckpointOverheadCalc.resnet_size_str` | `{python} CheckpointOverheadCalc.resnet_write_s_str` |  `{python} CheckpointOverheadCalc.resnet_interval_s_str` | `{python} CheckpointOverheadCalc.resnet_overhead_str` |
| **ViT-Large**                    |    `{python} CheckpointOverheadCalc.vit_size_str` |    `{python} CheckpointOverheadCalc.vit_write_s_str` |     `{python} CheckpointOverheadCalc.vit_interval_s_str` |    `{python} CheckpointOverheadCalc.vit_overhead_str` |

: **Checkpoint Overhead by Model Type**: Larger models with correspondingly larger checkpoints require longer save times but also benefit from longer optimal checkpoint intervals. The percentage column reports save-only checkpoint write tax, not total Young-Daly waste; at the optimum, expected rework adds a comparable term. {#tbl-checkpoint-overhead-by-model}
...
# │ Show: total_ckpt_gb_str="700" GB, nfs_gbs_str="1.25" GB/s,
# │   min_write_min_str="9.3" min, per_node_mbs_str="20" MB/s,
...
# │
# │ Imports: mlsysim.core.constants (Models.Language.Llama2_70B.parameters, Bparam, BYTES_FP16, byte)
# │ Exports: weights_gb_str, optimizer_gb_str, total_ckpt_gb_str, nfs_gbs_str,
# │   min_write_s_str, min_write_min_str, per_node_mbs_str,
...
class CheckpointDebug:
    ...
    total_ckpt_qty = params_count * bytes_per_param * byte
    ...
    nfs_gbs_str = fmt_qty(nfs_bw, GB/second, precision=2, commas=False)
    ...
    per_node_mbs_str = fmt_qty_int(nfs_bw / n_nodes, MB/second, commas=False)
    ...
    bytes_per_param_weights_str = fmt(2, precision=0, commas=False)
    ...
    target_write_min_str = fmt(target_write_min, precision=0, commas=False)
    ...
    nvme_bw_gbs_str = fmt_qty(nvme_bw, GB/second, precision=1, commas=False)
    pcie_gbs_str = fmt_qty(pcie_bw, GB/second, precision=0, commas=False)
...
A team training a `{python} CheckpointDebug.params_b_str` parameter model observes that checkpointing takes `{python} CheckpointDebug.observed_write_min_str` per checkpoint, far exceeding their expected `{python} CheckpointDebug.target_write_min_str`-minute target. Training throughput has dropped `{python} CheckpointDebug.overhead_pct_str` because the cluster sits idle during checkpoints. How can this be diagnosed and resolved?
...
The middle layer shifts the diagnosis from raw bandwidth to checkpoint coordination:
...
1. **infrastructure layer**\index{Infrastructure Layer}: Install local Non-Volatile Memory Express (NVMe) staging drives (`{python} CheckpointDebug.nvme_bw_gbs_str` per node)
2. **Distribution Layer**\index{Distribution Layer}: Implement asynchronous checkpointing:
   - Phase 1: GPU $\rightarrow$ CPU memory copy (fast, `{python} CheckpointDebug.pcie_gbs_str` PCIe)
   - Phase 2: CPU $\rightarrow$ local NVMe (`{python} CheckpointDebug.nvme_bw_gbs_str`, training resumes)
   - Phase 3: Background NVMe $\rightarrow$ NFS copy (overlapped with training)
3. **Validation**\index{Validation}: The critical-path data copy drops to approximately `{python} CheckpointDebug.gpu_cpu_parallel_s_str` if GPU-to-CPU staging proceeds in parallel across `{python} CheckpointDebug.n_nodes_str` (or `{python} CheckpointDebug.gpu_cpu_single_path_s_str` if serialized through one `{python} CheckpointDebug.pcie_gbs_str` path), reducing training impact from `{python} CheckpointDebug.overhead_pct_str` to under `{python} CheckpointDebug.residual_overhead_pct_str` once background NVMe and NFS copies are overlapped
...
Production systems use two-phase commit[^fn-two-phase-commit-checkpoint] to ensure correctness. Two-phase commit is a classic distributed systems protocol [@gray1978]:

[^fn-two-phase-commit-checkpoint]: **Two-Phase Commit (2PC)**: Formalized by @gray1978, 2PC ensures all participants either commit or abort atomically. The protocol's known weakness is blocking: if the coordinator fails between prepare and commit, participants wait indefinitely. For distributed checkpointing this blocking is acceptable because a stuck checkpoint can be timed out and retried, whereas the alternative (partial checkpoint writes) would leave the system with an unrecoverable inconsistent state. \index{Two-Phase Commit!checkpoint atomicity}

In the prepare phase, workers write to staging location and report success to coordinator. In the commit phase, coordinator atomically renames or commits all shards together. For example, the coordinator moves files from `staging/` to `checkpoints/`. If the coordinator fails between phases, workers detect during next heartbeat\index{Heartbeat} and rollback staged writes.
...
**Sharded checkpointing**[^fn-sharded-ckpt-io] [@rajbhandari2020] uses this distribution: each worker writes only its shard, dramatically reducing per-worker write volume. Recovery loads shards and redistributes state to workers based on the recovery configuration.

This approach enables efficient checkpointing even for massive models.
...
[^fn-sharded-ckpt-io]: **Sharded Checkpointing**: Each worker saves only its local partition rather than gathering state to a single writer. For ZeRO-3 or FSDP with `{python} FaultToleranceShardRecap.n_workers_str` workers training a `{python} FaultToleranceShardRecap.gpt3_params_b_str` model, each worker writes approximately `{python} FaultToleranceShardRecap.gpt3_shard_gb_str` instead of one writer handling `{python} FaultToleranceShardRecap.gpt3_ckpt_tb_str`, parallelizing I/O across all nodes. The trade-off: recovery requires all shards to be present and consistent, making the checkpoint protocol more complex and the failure of any single shard's storage fatal to the entire checkpoint. \index{Sharded Checkpointing!I/O parallelism}
...
::: {.callout-perspective title="Realistic failure detection latencies"}
...
from mlsysim.fmt import fmt, check, fmt_int, fmt_time, fmt_qty

class RecoveryBudget:
    # ┌── 1. LOAD ──────────────────────────────────────────
    from mlsysim.solvers import TrainingMemoryModel
    _mem = TrainingMemoryModel().solve(Models.Language.GPT3, Hardware.Cloud.A100, batch_size=1, precision="fp32")
    t_detect = 60 * second  # scenario assumption
    t_restart = 3 * minute
    aggregate_ckpt_bw = 100 * (GB / second)
    t_load = (_mem.total_memory / aggregate_ckpt_bw).to(second)
    t_warmup = 2 * minute
    shard_read_bw = 5 * (GB / second)
    n_gpus_1k = Systems.Clusters.Training_1K.total_accelerators
    n_gpus_10k = Systems.Clusters.Training_10K.total_accelerators
    failures_per_day_1k = 0.48
    failures_per_day_10k = 4.8
    mtbf_1k = 50 * hour  # scenario assumption
    mtbf_10k = 5 * hour  # scenario assumption
    ...
    aggregate_read_gbs_str = fmt_qty(aggregate_ckpt_bw, GB/second, precision=0, commas=False)
    shard_read_gbs_str = fmt_qty(shard_read_bw, GB/second, precision=0, commas=False)
    n_gpus_1k_str = fmt(n_gpus_1k, precision=0, commas=True)
    n_gpus_10k_str = fmt(n_gpus_10k, precision=0, commas=True)
    mtbf_1k_hrs_str = fmt_time(mtbf_1k, hour, precision=0, commas=False, style='word')
    mtbf_10k_hrs_str = fmt_time(mtbf_10k, hour, precision=0, commas=False, style='word')
    failures_per_day_1k_str = fmt(failures_per_day_1k, precision=2, commas=False)
    failures_per_day_10k_str = fmt(failures_per_day_10k, precision=1, commas=False)
    ...
    waste_gpu_hours_10k_str = fmt_int(round(waste_gpu_hours_10k), commas=True)
```
...
Consider our `{python} FaultToleranceRecoveryRecap.params_b_str` parameter model training on `{python} FaultToleranceRecoveryRecap.n_workers_str`. The checkpoint size is approximately `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` (weights + Adam optimizer state). How much does a single failure event cost?
...
3. **$T_{\text{load}}$**: `{python} RecoveryBudget.t_load_s_str` (this is the aggregate-shared-storage read of the full `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` checkpoint at `{python} RecoveryBudget.aggregate_read_gbs_str` aggregate; with sharded local-NVMe reads at `{python} RecoveryBudget.shard_read_gbs_str`, each `{python} FaultToleranceRecoveryRecap.gpt3_shard_gb_str` shard loads in well under one second)
...
**Impact at scale**\index{Recovery Time!scale impact}: With the GPU-only baseline from @tbl-cluster-mtbf-scaling, a `{python} RecoveryBudget.n_gpus_1k_str`-GPU cluster has MTBF of `{python} RecoveryBudget.mtbf_1k_hrs_str` and experiences about `{python} RecoveryBudget.failures_per_day_1k_str` failures per day, losing about `{python} RecoveryBudget.daily_loss_min_1k_str` daily, a modest `{python} RecoveryBudget.overhead_pct_1k_str` overhead. However, a `{python} RecoveryBudget.n_gpus_10k_str`-GPU cluster has GPU-only MTBF of `{python} RecoveryBudget.mtbf_10k_hrs_str` and experiences about `{python} RecoveryBudget.failures_per_day_10k_str` failures per day, losing about `{python} RecoveryBudget.daily_loss_min_10k_str` daily—a `{python} RecoveryBudget.overhead_pct_10k_str` overhead equivalent to wasting roughly `{python} RecoveryBudget.waste_gpu_hours_10k_str` GPU-hours every day (`{python} RecoveryBudget.daily_loss_min_10k_str` $\times$ `{python} RecoveryBudget.n_gpus_10k_str` ÷ 60). This is why recovery time optimization matters more as clusters grow.
...
class StragglerTax:
    ...
    wasted_cost_per_hour = (n_gpus * gpu_cost_per_hour
                            * (1 - normal_step_s/straggler_step_s))
...
However, killing a slow node and restarting the job implies waiting for a replacement to maintain the original GPU count. The previous sections assumed that recovery means restoring the *same* resource allocation: checkpoint, wait for a replacement, restart. Elastic recovery breaks that assumption by allowing the job to continue with *fewer* workers rather than idling the entire cluster until the original count is restored.
...
***Elastic Training***\index{Training!elastic, definition} is the capability of a distributed training job to dynamically adjust its worker count during execution without requiring a full restart.
...
2.  **Distinction (durable)**: Unlike checkpoint-restart recovery (which halts the job, waits for replacement hardware, and reloads state), elastic recovery is always-live: the optimization loop never stops, it only changes its parallel width.
...
Suppose a 1,024-GPU training job loses an 8-GPU node to a hardware fault, but the cluster has no spare nodes available. Under checkpoint-restart recovery, the remaining 1,016 GPUs sit idle for hours waiting for a repair. Elastic recovery instead allows the job to dynamically resize and continue with slightly less compute, breaking the rigid assumption of a fixed worker count. @Fig-elastic-flow traces the recovery sequence: detect the failure, pause, rescale across the surviving workers, and resume.
...
Hardware faults are not the only event that removes workers from a running job. Cloud providers offer preemptible ("spot") instances at 60 to 90 percent discounts compared to on-demand pricing, with the caveat that they can be reclaimed with little warning. From the training framework's perspective, a spot reclamation is indistinguishable from a hardware failure: workers vanish, and the job must decide whether to halt or adapt.
...
This recovery capability unlocks a significant cost advantage. Organizations can train on clusters of cheap, unreliable spot hardware because elastic recovery converts each preemption into a temporary throughput reduction rather than a fatal error. The cost savings (for example, \$0.60/hour vs. \$3.00/hour) far outweigh the efficiency loss from occasional resizing pauses, often reducing total training bills by more than 50 percent.
...
Ray Train, built on Ray's actor model, provides the most automated recovery path. Workers are Ray actors that can be dynamically added or removed, and Ray's distributed object store facilitates state redistribution to surviving workers without a full checkpoint reload.
...
@Tbl-elastic-training-comparison compares the recovery automation and state management approaches across these frameworks.
...
: **Elastic Recovery Framework Comparison**\index{Elastic Training!framework support}: Frameworks differ in how they detect worker failures and how much of the recovery sequence (group reconstruction, state redistribution, batch size adjustment) they automate. {#tbl-elastic-training-comparison}
...
The checkpoint and recovery strategies developed above require adaptation because each model type places a different state variable at risk: LLMs protect optimizer state and curriculum position, recommendation systems protect embedding freshness, vision models protect augmentation state, and scientific workloads protect exploration or simulator state.
...
The governing decision for LLMs is how to preserve optimizer and curriculum state without making checkpoint I/O the training bottleneck. Existing strategies attack different parts of that cost:
...
Recommendation models with trillion-parameter embedding tables present unique checkpoint challenges. Embedding tables can be multiple terabytes, making full checkpointing at high frequency impractical. The checkpoint design therefore favors freshness over full-state exactness:

- **Incremental checkpointing**\index{Checkpointing!incremental}: Only save embeddings that changed since last checkpoint
- **Tiered checkpointing**\index{Checkpointing!tiered}: Frequent checkpoints for model parameters, infrequent for embeddings
- **Embedding versioning**\index{Embedding!versioning}: Maintain embedding versions with efficient delta storage
...
Serving fault tolerance operates under constraints that training never faces. While @sec-inference-scale-health-checking-failover-070e examines serving health checks and failover in the broader inference architecture, the focus here is specifically on fault tolerance and reliability mechanisms for real-time inference.
...
The stateless pattern appears when the input itself carries all context:
...
The required mechanisms therefore target replica health and request routing rather than state preservation:

- **Redundant replicas**\index{Redundant Replicas}: Multiple copies of the model serve requests in parallel
- **Load balancing**\index{Load Balancing}: Requests are distributed across healthy replicas
- **Health checks**\index{Health Check}: Failed replicas are removed from the load balancer
...
Stateful serving appears wherever the current request depends on earlier requests. LLM conversations accumulate KV cache across turns, streaming speech recognition maintains context from previous audio, recommendation sessions accumulate user context, and interactive editing maintains document state across edits.

The fault-tolerance consequence is that failure loses quality-bearing state, not just serving capacity. KV cache loss requires reprocessing previous turns, session context loss forces users to repeat previous interactions, and accumulated user state loss degrades quality when context is unavailable.

The mitigation choice follows the size, update rate, and quality value of that state. Session affinity routes requests within a session to the same replica, state checkpointing periodically saves session state for recovery, state replication maintains copies for high availability, and graceful degradation allows service to continue with reduced quality if state is lost.
...
: **Stateless vs. Stateful Serving Fault Tolerance**\index{Fault Tolerance!stateless vs. stateful serving}: Stateful serving introduces significant complexity in fault tolerance due to the need to preserve accumulated session state. {#tbl-stateless-stateful-comparison}
...
Single replica availability: $A_{\text{single}} = 99\%$ (3.65 days of downtime per year)
...
In **active-active replication**\index{Replication!active-active replication} (@fig-serving-redundancy, left), all replicas actively serve requests.
...
In **active-passive replication**\index{Replication!active-passive replication} (@fig-serving-redundancy, right), primary replicas serve requests while standby replicas remain idle but ready. Failure of a primary triggers failover to the standby. This approach provides simpler failover but wastes standby resources during normal operation.

Geographic replication distributes replicas across geographic regions. This protects against regional failures (data center outage, regional network issues) but introduces latency for requests routed to distant regions.
...
Effective redundancy requires placing replicas in independent failure domains. Different machines tolerate individual machine failures. Different racks tolerate rack-level failures from power and ToR switch issues. Different availability zones tolerate data center section failures. Different regions tolerate entire data center failures.
...
For stateful serving, session affinity\index{Session Affinity} routes all requests within a session to the same replica:
...
State failover choices trade recovery speed against consistency and operational complexity. State loss accepts degraded quality on failover by regenerating state from scratch. State checkpointing periodically saves session state for recovery. State replication copies state to a standby replica. Distributed state stores session state in external stores like Redis or Memcached.
...
Model-specific serving differences follow from what state is expensive to lose and how quickly the user needs a response.

#### LLM serving fault tolerance {#sec-fault-tolerance-reliability-reliability-llm-serving-fault-tolerance-462b}

The binding state in LLM serving is conversational context, especially the KV cache:
...
LLM serving choices trade KV-cache recovery latency against memory and storage overhead. Accepting regeneration cost means regenerating KV cache from conversation history on failure. This approach is simple but can significantly increase latency for long conversations. KV cache checkpointing periodically saves KV cache state, enabling partial recovery but introducing storage overhead and latency for checkpointing. KV cache replication duplicates KV cache to standby, providing fast failover but doubling memory requirements. **Prefix caching**\index{Prefix Caching} stores common prefixes separately. System prompts and shared context are cached. On failure, common prefixes restore quickly. Only session-specific state requires regeneration.

Prompt caching\index{Prompt Caching} services like those offered by cloud providers store and reuse KV cache for common prefixes, reducing both cost and recovery time for failures.
...
The serving decision is how stale the system may become before it should block or degrade. Replicated feature stores span availability zones. Local caching stores frequently accessed features. Fallback to stale features accepts quality degradation. Default features activate when feature lookup fails.
...
1.  **Significance (quantitative)**: Graceful degradation converts total outage risk into a controlled quality reduction. A recommendation system that falls back from a 7B-parameter ranker to a collaborative-filtering model on GPU failure may reduce click-through rate by 8–15 percent but maintains 100 percent request availability vs. 0 percent during a hard failure. At \$1M/day in revenue dependent on recommendations, an 8 percent CTR reduction sustained for 10 minutes costs about \$560, while a 10-minute hard outage costs about \$7K total.
...
A degradation plan must name which service property will be sacrificed before an incident begins. The common dimensions are quality, latency, coverage, freshness, and feature completeness.

Quality degradation is the most visible availability lever: the system serves predictions from simpler, faster models. A recommendation system might fall back from a sophisticated multi-tower model to a simpler collaborative filtering model.

Latency degradation preserves model quality by accepting slower responses. Under high load, batching more requests together increases latency but maintains throughput.

Coverage degradation preserves a useful response by serving partial results rather than complete results. A search system might return top-10 results instead of top-100 when compute is constrained.

Freshness degradation preserves availability by serving cached or stale results rather than real-time computation. News recommendations might serve hour-old recommendations when the recommendation service is unavailable.

Feature degradation preserves inference by using fewer features when feature retrieval fails. A recommendation system might use only content features when user history is unavailable.
...
Model fallback turns quality into an explicit availability lever by maintaining multiple model versions with different resource requirements:

- **Primary model**\index{Primary Model}: Full capability, highest resource requirements
- **Secondary model**\index{Secondary Model}: Reduced capability, lower resource requirements
- **Tertiary model**\index{Tertiary Model}: Minimal capability, minimal resources
- **Static fallback**\index{Static Fallback}: Precomputed defaults, no inference required

When the primary model is unavailable or overloaded, the policy descends only as far as resource pressure requires, then returns upward when the primary path is healthy again.
...
2. **Secondary**\index{Secondary Model}: EfficientNet-B4 (19M params, 83 percent ImageNet top-1)

3. **Tertiary**\index{Tertiary Model}: MobileNet-V3-Large (5.4M params, 75 percent ImageNet top-1)

4. **Fallback**\index{Fallback}: Return "classification unavailable" or cached results

For that fallback path to be real rather than aspirational, the production system needs:
...
In production, recommendation and ranking systems routinely fall back from a complex deep learning model to a lightweight backup---a top-n popularity list, a linear ranker, or a cached response---when the primary serving stack becomes unavailable. Complex models often fight for the last mile of accuracy while simple heuristics provide the bulk of the utility, so the fallback path must be monitored as a first-class quality signal rather than treated as a silent failover; otherwise the product appears healthy while quality is silently traded for availability.

#### Feature fallback {#sec-fault-tolerance-reliability-reliability-feature-fallback-0730}

Feature fallback controls how much input quality the model may lose before the request must be blocked. When feature retrieval fails, use default or computed fallback values:
...
Prioritizing features by importance to prediction quality allows systems to degrade gracefully, as @tbl-feature-degradation-tiers summarizes:
...
Load shedding protects the system by sacrificing selected requests before overload spreads to every request:
...
Admission control\index{Admission Control} rate-limits at system entry points. It rejects requests that would exceed capacity rather than accepting and degrading all requests.
...
Implementation begins with health signals that choose when to enter and leave each degradation level. Track request latency percentiles at p50, p95, and p99. Monitor error rates by error type. Measure resource utilization for CPU, GPU, and memory. Watch queue depths and wait times.

Degradation triggers define conditions that activate degradation. @Lst-degradation-triggers illustrates three common trigger conditions that progressively activate fallback mechanisms based on latency, error rate, and feature store health.\index{Graceful Degradation!triggers}
...
Degradation should increase progressively as conditions worsen rather than switch at one cliff. The system can raise the fallback percentage gradually as load increases.

Recovery needs the same control discipline. Hysteresis prevents oscillation, and sustained improvement should be required before traffic returns to higher-capability paths.
...
Alerting thresholds should reflect duration and blast radius. Degradation activation triggers an informational alert, sustained degradation beyond 5 minutes triggers a warning alert, severe degradation affecting over 50 percent of requests triggers a critical alert, and extended degradation beyond 1 hour triggers escalation.

After degradation events, postincident analysis should identify the root cause, evaluate fallback effectiveness, measure user and business impact, and define improvements that prevent recurrence.
...
[^fn-heisenbug-ml]: **Heisenbug**: Coined in hacker culture circa 1983, formalized by Jim Gray in his 1985 analysis of computer failures, as a pun on Heisenberg's uncertainty principle. The bug disappears under observation because adding instrumentation changes timing. In distributed ML training, this is especially pernicious: inserting gradient logging alters NCCL collective timing, masking the very race condition that caused the silent corruption. \index{Heisenbug!distributed debugging}

Partial failures turn that nondeterminism into a recovery problem. Unlike single-machine systems where failures are typically total, distributed systems experience partial failures where some components fail while others continue, and the interaction between working and failed components produces complex failure modes.

Scale then removes manual inspection as a viable debugging method. With thousands of components, automated tools must filter relevant information from massive telemetry streams and identify the few signals that explain the incident.

ML systems add model behavior to the same diagnostic problem. Silent accuracy degradation produces wrong results without errors, numerical issues like NaN and infinity propagate through computation, data-dependent bugs manifest only for specific inputs, and learning causes expected behavior changes that can resemble bugs.

### Observability pillars {#sec-fault-tolerance-reliability-reliability-observability-pillars-0e45}

Observability is useful only when it connects a symptom to an action: shed load, restart a replica, roll back a checkpoint, or degrade gracefully. The three signal types in @fig-observability-three-pillars supply different parts of that decision: metrics, logs, and traces.
...
Metrics are numerical measurements collected over time. In ML fleets, they span infrastructure signals such as CPU and GPU utilization, memory allocation, network bandwidth, and storage I/O; application signals such as request rate, latency percentiles, error counts, queue depths, and cache hit rates; and ML-specific signals such as inference latency by model, batch utilization, feature retrieval latency, and prediction distributions for drift detection. Because metrics are cheap to aggregate over time, they support real-time dashboards, threshold-based alerting, capacity planning, and performance analysis.

#### Logs {#sec-fault-tolerance-reliability-reliability-logs-9906}

Logs answer what happened at a specific point in the recovery path.

Structured logging uses structured formats such as JavaScript Object Notation (JSON) with consistent fields. @Lst-structured-log-entry shows a typical error entry that captures both the failure context and the trace identifiers needed to correlate events across services.\index{Structured Logging!JSON log entry}
...
Log levels should map incidents to escalation consistently across all components:
...
Log aggregation centralizes logs from all components for search and analysis. The particular backend matters less than the ability to search distributed logs with the same trace and request identifiers used by metrics and spans.

#### Traces {#sec-fault-tolerance-reliability-reliability-traces-ffc5}

Traces show where latency or failure propagated across distributed components.

Distributed tracing relies on three concepts that preserve request context across services:

- **Trace**\index{Trace}: End-to-end journey of a request
- **Span**\index{Span}: Single operation within a trace
- **Context propagation**\index{Context Propagation}: Passing trace context between services
...
OpenTelemetry provides a standard API for distributed tracing\index{Distributed Tracing}. Backend systems like Jaeger, Zipkin, or cloud tracing services store and visualize traces. The durable requirement is correlation: the same trace context must connect metrics, logs, and spans across the request path.
...
NaN detection[^fn-nan-propagation-training] is essential because NaN values propagate silently through all downstream computations. @Lst-nan-detection shows a minimal check that catches corruption before it reaches users.\index{NaN Detection}

[^fn-nan-propagation-training]: **NaN Propagation**: IEEE 754 specifies that any arithmetic operation involving NaN produces NaN, meaning a single NaN in one gradient computation silently corrupts every downstream parameter update. Common triggers in ML training include division by zero in normalization layers (batch norm with zero variance), log of nonpositive values, and FP16 overflow to infinity. Without per-step NaN detection, an entire training run can produce a model full of NaN weights before any monitoring alarm fires. \index{NaN Propagation!training corruption}
...
ML bugs often originate in data, and because silent corruption and distribution shift surface as valid-looking but wrong inputs, validation at the pipeline boundary is the first line of defense: the pipeline checks inputs for expected format, shape, value ranges, required fields, and encoding before they reach the model. Teams then track feature distributions over time so that distribution shift (feature values drifting), missing features (null rates rising), and outliers (extreme values appearing) are caught as they emerge rather than after they have degraded the model. Finally, data-debugging instrumentation traces transformations end to end: logging intermediate shapes and statistics, validating that each transform produces the expected result, and comparing pipeline outputs against known-good data.

#### Straggler detection and analysis {#sec-fault-tolerance-reliability-reliability-straggler-detection-analysis-6925}

Straggler diagnosis asks which component is slow enough to become a failure-equivalent bottleneck.

Timing instrumentation measures time for each operation, enabling latency attribution across pipeline stages. @Lst-timing-instrumentation wraps operations in context managers that emit per-span timing metrics, making it straightforward to compare performance across replicas.\index{Operation-Level Timing}
...
The observability pillars above enable detection of recurring failure patterns that experience with large-scale ML systems has identified. @Fig-training-failure-signatures catalogs the most common training loss signatures, each requiring a different diagnostic and recovery response.
...
**Metrics**\index{Metric!observability} reveal the symptom: p99 latency jumped from 45 ms to 800 ms at 14:32, with error rate increasing from 0.1 percent to 15 percent.

**Traces**\index{Trace!observability} isolate the bottleneck: traces show the feature-service span consuming 700 ms instead of the normal 12 ms. The model-inference span remains normal at 40 ms.

**Logs**\index{Logs!observability} identify the root cause: feature-service logs show repeated connection timeouts to the user embedding cache at 14:31, followed by cache miss storms as requests bypass the failed cache and hit the embedding database directly.
...
Production case studies show how failure detection, state preservation, and observability are orchestrated together in production at scale.
...
Industry experience from large-scale ML systems points to a broader failure mix:

- Hardware failures
- Software bugs
- Configuration errors
- Resource exhaustion
- Unknown or cross-layer causes
...
class FleetReliabilityFallacyRates:
    ...
    check(abs(hours_between * fleet_rate_per_h - 1) < 1e-9,
          "interval must be reciprocal of fleet rate")
...
Engineers read a GPU MTTF of `{python} FleetReliabilityFallacyRates.gpu_mttf_str` hours and plan around a five-year replacement cadence per device. MTTF is a statistical property of a population, not a prediction for any single component. A GPU with MTTF `{python} FleetReliabilityFallacyRates.gpu_mttf_str` hours might fail at 200 hours or last 100,000; the distribution is broad. What MTTF reliably predicts is the aggregate failure rate: in a fleet of `{python} FleetReliabilityFallacyRates.fleet_gpus_str` GPUs, the steady-state failure rate is approximately `{python} FleetReliabilityFallacyRates.fleet_rate_eq_math` failures per hour, or roughly one failure every `{python} FleetReliabilityFallacyRates.hours_between_str` hours. Fleet-scale reliability engineering relies on this statistical regularity to size automated recovery, spare-pool depth, and on-call rotations. Trying to predict which individual GPU will fail next is a category error and a waste of monitoring effort.
...
**Fallacy**: *Adding more GPUs always speeds up training, even after accounting for communication.*
...
* **Scale guarantees failure**: A 10,000-GPU cluster\index{Failure!inevitability at scale} will encounter hardware failures every few hours; software must treat failure as a normal state.
...
* **The Young-Daly formula governs checkpoint intervals**: The optimal checkpoint interval $\tau_{\text{opt}} = \sqrt{2 \times T_{\text{write}} \times \text{MTBF}_{\text{system}}}$ balances the cost of saving state against the cost of lost work, often landing in the tens-of-minutes range for frontier-scale training while remaining much shorter for small checkpoints.
...
* **Stateful serving is the frontier**: For LLMs, the KV cache\index{Stateful Serving!KV cache migration} represents a massive serving state that must be replicated or migrated to prevent high-latency session restarts, with strategy trade-offs summarized in @tbl-stateful-failover.
...
* **Production scale validates the theory**: Meta's OPT-175B\index{Fault Tolerance!production evidence} experienced over 35 significant hardware failures during training, Google's TPUv4 supercomputer reached 99.98 percent availability by rerouting around hardware failures, and Netflix's chaos engineering exposed resilience gaps, confirming that fault tolerance must be continuously exercised rather than assumed.
