...
Volume 1 established serving replicas, routing, readiness checks, load balancing, KV cache, and graceful degradation as serving-system building blocks. Serving fault tolerance asks how those mechanisms behave when replicas, GPUs, or state stores fail under live latency budgets. @Sec-inference-scale-health-checking-failover-070e later broadens the same mechanisms into a full distributed inference architecture; the focus here is specifically on fault tolerance and reliability for real-time inference.
