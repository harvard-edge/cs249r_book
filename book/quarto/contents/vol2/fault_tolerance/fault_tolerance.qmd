...
# │
# │ Imports: mlsysim public API (Systems, fmt, fmt_int, MarkdownStr, check)
# │ Exports: gpu_mttf_hrs_str; n0..n6_str; mtbf0..mtbf6_str; fpd0..fpd6_str
# └─────────────────────────────────────────────────────────────────────────────
...
def _mtbf_h(v):
    ...
    return fmt_int(int(round(v))) if float(v).is_integer() else fmt(v, precision=1, commas=False)

def _mtbf_cell(v):
    # show the days parenthetical only where the cluster MTBF is at least a day
    hrs = _mtbf_h(v)
    if v >= 24.0:
        return MarkdownStr(f"{hrs} hrs ({fmt_int(round(v / 24.0))} days)")
    return MarkdownStr(f"{hrs} hrs")
...
class ClusterMtbfScaling:
    ...
    # ┌── 3. GUARD (Invariants) ────────────────────
    check(cluster_mtbf_h[0] > cluster_mtbf_h[-1] > 0, "cluster MTBF must fall with size and stay positive")
    check(failures_per_day[-1] > failures_per_day[0] > 0, "failures/day must rise with size")
    check(abs(cluster_mtbf_h[3] - 50.0) < 1e-9, "1,000-GPU row anchors at 50 h cluster MTBF")

    # ┌── 4. OUTPUT (Formatting) ───────────────────
    gpu_mttf_hrs_str = MarkdownStr(f"{fmt_int(int(gpu_mttf_h))} hrs")
...
ML infrastructure teams apply this model directly to cluster scheduling and fleet admission. These teams do not place new accelerator pods directly into production training queues after provisioning. Instead, operators run intensive synthetic workloads—sustained matrix-multiply loops at peak TFLOPS utilization—for 24 to 72 hours to force any infant-mortality defects to surface. Operators repair or replace an accelerator that fails during this burn-in phase before it ever touches a high-value training run; an accelerator that survives enters the useful-life period where the constant-rate exponential model applies. This discipline matters because the cost asymmetry is stark: catching an infant-mortality failure during a burn-in session costs a few hours of idle accelerator time, while the same failure during a multi-week foundation model pretraining run can invalidate days of gradient accumulation and force a checkpoint rollback. Scheduling policy reflects this asymmetry: cluster operators typically assign fresh or recently-repaired nodes to short exploratory jobs or inference serving first, not to months-long pretraining jobs, until they have accumulated enough operating hours to leave the infant-mortality risk window.
...
# │       model so the "no headroom" claim tracks the model size and byte budget.
# │ Show: Parameter count and the aggregate per-node training-state footprint.
...
# │
# │ Imports: mlsysim public API (Models, memory_from_params, fmt_params, fmt_qty)
# │ Exports: params_b_str, footprint_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.physics import memory_from_params
from mlsysim.fmt import fmt_params, fmt_qty, check
...
class SDCCollective:
    ...
    check(exposure_gpu_hr > 0, "exposure must be positive")
    # ┌── 4. OUTPUT ────────────────────────────────────────
...
# │
# │ Imports: mlsysim public API (Systems, GB, TB, fmt_qty, fmt_count, check)
# │ Exports: storm_workers_str, storm_shard_gb_str, storm_payload_tb_str
# └─────────────────────────────────────────────────────────────────────────────
...
**Mechanism**: For the `{python} CheckpointOverheadCalc.archA_ckpt_tb_str` mixed-precision optimizer footprint in the Archetype A row above—not the `{python} CheckpointOverheadCalc.archA_fp32_tb_str` full FP32 total—`{python} CheckpointOverheadCalc.archA_workers_str` would each write roughly `{python} CheckpointOverheadCalc.archA_shard_gb_str` of state. The same mechanism becomes catastrophic in embedding-heavy recommendation or trillion-parameter mixture-of-experts jobs: `{python} CheckpointStorm.storm_workers_str`, each holding a `{python} CheckpointStorm.storm_shard_gb_str` shard of model or embedding state, can flood the network fabric with `{python} CheckpointStorm.storm_payload_tb_str` of simultaneous writes.
...
The Distribution Layer reveals the shared-storage bottleneck. With `{python} CheckpointDebug.n_nodes_str` competing for 10 Gbps, each node effectively receives only `{python} CheckpointDebug.per_node_mb_per_s_str`, the 10 Gbps link split evenly across all `{python} CheckpointDebug.n_nodes_str`. If the checkpoint is sharded evenly, each node writes about `{python} CheckpointDebug.per_node_shard_gb_str`, so the write still lands near `{python} CheckpointDebug.per_node_write_min_str`. The observed `{python} CheckpointDebug.observed_write_min_str` is therefore close to the bandwidth floor, not something more simultaneous NFS writers can solve.
...
# │       4-hour-MTBF cluster so the "intuition is wrong" point cannot drift.
...
# │       minutes for display.
# │
# │ Imports: mlsysim public API (fmt_int, fmt_math, MarkdownStr, check)
# │ Exports: tau_opt_fast_eq_math, tau_opt_slow_min_str
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt_int, fmt_math, MarkdownStr, check

class YoungDalyIntuitionFallacy:
    # ┌── 1. LOAD (Constants) ──────────────────────
    # scenario assumptions: a hypothetical 1,000-GPU cluster posed for this pitfall
    mtbf_min = 4 * 60  # 4-hour cluster MTBF expressed in minutes
