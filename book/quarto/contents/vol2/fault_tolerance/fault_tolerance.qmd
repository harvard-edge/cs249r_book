...
```{python}
#| echo: false
from mlsysim.fmt import fmt_int, fmt_count, fmt_time, fmt_qty, fmt_qty_int, fmt_params
...
class CheckpointDebug:
    ...
    nfs_bw = 10 * Gbps
    pcie_bw = 32 * (GB / second)
    nvme_bw = 3.5 * (GB / second)
    n_nodes = 64
    n_gpus_ckpt = 512
...
class RecoveryBudget:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    t_total_str = fmt(t_total.to(minute).magnitude, precision=1, commas=False)
    ...
    t_load_s_str = fmt_int(round(t_load.to(second).magnitude), commas=False)
