...
### Checkpoint coordination patterns {#sec-fault-tolerance-reliability-reliability-distributed-checkpointing-f670}
...
The dimensions in @tbl-graceful-degradation-dimensions become operational only when the service binds each sacrificed property to a specific fallback mechanism. The strategy ladder starts with model fallback when the primary inference path is too expensive or unavailable, then moves to feature fallback when input quality degrades, load shedding when demand exceeds capacity, and quality-of-service degradation when the system must preserve the highest-value requests first.
