...
class ClusterMtbfScaling:
    ...
    # ┌── 3. GUARD (Invariants) ────────────────────
    check(abs(cluster_mtbf_h[0] - 500.0) < 1e-9 and abs(cluster_mtbf_h[-1] - 5.0) < 1e-9, "100-GPU MTBF must be 500h and 10,000-GPU MTBF must be 5h")
    check(abs(failures_per_day[-1] - 4.8) < 0.01, "10,000-GPU failure rate must be 4.8 failures/day")
...
class SDCCollective:
    ...
    check(abs(exposure_gpu_hr - 409600.0) < 0.1, f"Expected 409,600 GPU-hours exposure, got {exposure_gpu_hr}")
    # ┌── 4. OUTPUT ────────────────────────────────────────
...
class CheckpointOverheadCalc:
    ...
    # ┌── 3. GUARD (Invariants) ────────────────────────────────────────────────
    check(10200 * second < archA_interval < 10400 * second, "Archetype A interval should be ~171 minutes")
    check(1800 * second < dense20b_interval < 1900 * second, "20B dense transformer interval should be ~30.6 minutes")
    check(370 * second < bert_interval < 390 * second, "BERT interval should be ~6.3 minutes")
    check(120 * second < archB_interval < 130 * second, "Archetype B interval should be ~2.1 minutes")
    check(370 * second < resnet_interval < 390 * second, "ResNet-50 interval should be ~6.3 minutes")
    check(810 * second < vision_transformer_interval < 830 * second, "Vision transformer interval should be ~13.7 minutes")
...
class RecoveryBudget:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(abs(t_total.to(minute).magnitude - 24.6) < 1.0, f"Expected ~24.6 min total recovery time, got {t_total.to(minute).magnitude:.1f}")
...
class StragglerTax:
    ...
    check(abs(throughput_loss_pct - 15.0) < 0.01, f"Expected 15% throughput loss, got {throughput_loss_pct}")
    check(abs(wasted_cost_per_hour - 307.2) < 0.1, f"Expected $307.20/hr wasted cost, got {wasted_cost_per_hour}")
