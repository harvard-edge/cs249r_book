...
Defending against correlated failures requires understanding failure domains and ensuring redundancy spans independent failure domains (see @fig-failure-domains). @tbl-failure-domains lists common failure domains in ML infrastructure and their typical impact scope.
...
::: {#fig-failure-domains fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
...
The failure taxonomy above classifies failure types and domains, answering WHAT KIND of failures occur. Equally important for designing fault tolerance is understanding WHEN in a component's lifetime failures are most likely to occur. Hardware failure rates are not constant over component lifetime. The bathtub curve, a well-established model in reliability engineering, describes how failure rates vary across three distinct phases (see @fig-bathtub-curve):
...
::: {#fig-bathtub-curve fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \begin{axis}[
...
The optimal investment in fault tolerance balances these costs. For small-scale training on a few GPUs where failures are rare, minimal fault tolerance may be cost-optimal. Infrequent checkpoints and manual recovery suffice. For large-scale training on thousands of GPUs where failures occur multiple times daily, extensive fault tolerance provides positive return on investment. Frequent checkpoints, automatic recovery, and elastic training become essential (visualized in @fig-young-daly).
...
::: {.callout-example title="Engineering Calculation: Optimal Checkpoint Interval"}
...
::: {#fig-young-daly fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \begin{axis}[
    width=10cm, height=6cm,
...
\end{tikzpicture}
```
**Optimal Checkpoint Interval**. The Young-Daly formula identifies the "sweet spot" for checkpoint frequency. The total wasted time (Y-axis) is the sum of checkpoint overhead (linear increase) and rework due to failure (exponential decay). The minimum point represents the optimal interval $\tau$.
:::

For our 10,000 GPU cluster with calculated system MTBF of 3.69 hours (from @sec-cluster-mtbf-calculation) and checkpoint time of 21 seconds (from 2.1 TB checkpoint at 100 GB/s), the optimal interval is calculated using @eq-young-daly-applied:

$$ T_{opt} = \sqrt{2 \times 21s \times 3.69hr \times 3600s/hr} \approx 12.4 \text{ minutes} $$ {#eq-young-daly-applied}

This result demonstrates why failure analysis matters: without knowing the system MTBF, we cannot set checkpoint intervals rationally. With this interval, checkpoint overhead consumes approximately 2.8% of training time.
...
::: {#fig-elastic-flow fig-env="figure" fig-pos="htb"}
```{.tikz}
...
::: {#fig-serving-redundancy fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
...
Circuit breakers operate in three states (see @fig-circuit-breaker). In the closed state, normal operation proceeds and requests pass through. In the open state, downstream is failing and requests fail immediately without attempting downstream call. In the half-open state, the breaker tests recovery by allowing limited requests through to test if downstream recovered.

::: {#fig-circuit-breaker fig-env="figure" fig-pos="htb"}
```{.tikz}
...
Engineers often treat all failures as "node crashed, restart from checkpoint." This oversimplification leads to inefficient recovery strategies. Different failures require different responses:

*   **Transient Failures (Network Blips, Thermal Throttling)**:
    *   *Cause*: Congestion, temporary heat spikes.
    *   *Impact*: Training stalls or slows down.
    *   *Response*: **Retry/Pause**, not Restart. Checkpointing is not needed; the state is still in memory.
*   **Permanent Failures (GPU Death, Node Crash)**:
    *   *Cause*: Hardware aging, manufacturing defects.
    *   *Impact*: State in VRAM/RAM is lost.
    *   *Response*: **Restart from Checkpoint**. Requires migration to a new node.
*   **Silent Corruption (Bit Flips, ECC Errors)**:
    *   *Cause*: Cosmic rays, voltage instability.
    *   *Impact*: Checkpoint is valid *format* but contains poisoned weights.
    *   *Response*: **Rollback to Previous Checkpoint**. Requires maintaining *history* of checkpoints, not just the latest one.
*   **OOM / Resource Exhaustion**:
    *   *Cause*: Memory fragmentation, activation spikes.
    *   *Impact*: Process termination.
    *   *Response*: **Reconfiguration**. Restarting on the same config will just crash again. Requires offloading or model parallelism adjustments.

Treating a silent corruption like a transient failure (ignoring it) poisons the model. Treating a transient network blip like a permanent failure (restarting) wastes hours of re-initialization time.
...
::: {.callout-note title="Cross-Reference: Handling Corruption"}
This chapter focuses on **detecting** silent corruption to trigger system recovery (restarting the job). For a deep dive into **algorithmic resilience**—designing models that are naturally robust to noise and bit errors without crashing—see @sec-robust-ai.
:::
...
* **Elasticity enables Persistence**: Designing training jobs to be "elastic" (reconfiguring around lost or added nodes (@fig-elastic-flow)) is superior to simple restart-from-scratch strategies.
* **Stateful Serving is the Frontier**: For LLMs, the KV cache represents a massive serving state that must be replicated or migrated to prevent high-latency session restarts (see @fig-serving-redundancy).
:::

With the algorithmic foundations of distributed training established—parallelism strategies, communication patterns, and fault tolerance mechanisms—we have completed our study of how models are built at scale.

However, a trained model is only potential energy. To create value, it must be deployed. We now turn our attention to the serving layer, where constraints shift from throughput to latency, and from batch processing to real-time interaction. **Part III: Deployment at Scale** begins with @sec-inference-at-scale, examining how we transform massive, static model artifacts into low-latency global services that serve billions of users.
