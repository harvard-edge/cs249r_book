...
![](images/svg/failure-spectrum.svg){width=100%}
:::
...
![](images/svg/young-daly-optimization.svg){width=100%}
:::
...
![](images/svg/checkpoint-tax.svg){width=100%}
:::
...
![](images/svg/sdc-propagation.svg){width=100%}
:::
...
![](images/svg/sdc-controller.svg){width=100%}
:::
...
[^fn-mtbf-checkpoint]: **MTBF (Mean Time Between Failures)**: Formalized by the U.S. military in MIL-HDBK-217 (1965), MTBF assumes exponential failure distributions during the useful-life period. For ML training, MTBF feeds directly into the Young-Daly formula: a cluster with 50,000-hour per-device MTBF and 1,000 devices has a system MTBF of 50 hours, demanding checkpoints every 1--2 hours to keep wasted compute below 1% of total training time. \index{MTBF!checkpoint frequency}
...
**The Systems Insight**: At this interval, the "Checkpoint Tax" (time spent saving + time spent re-computing) is minimized to approximately **`{python} YoungDalyOptimal.overhead_pct_str`%**. Checkpointing every hour instead would increase failure risk, wasting nearly 10% of the cluster's capacity. As clusters scale, the optimal interval *must* shrink to keep up with the falling MTBF.
...
Elastic training doubles as an economic lever that can reduce training costs by 50% or more. Cloud providers offer preemptible ("spot") instances at 60–90% discounts compared to on-demand pricing, with the caveat that they can be reclaimed with little warning.
