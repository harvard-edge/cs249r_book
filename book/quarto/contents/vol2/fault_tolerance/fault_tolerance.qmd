...
::: {.column-margin}
![](images/svg/fault_tolerance_mtbf_ladder.svg){width="100%" fig-alt="Ladder of three blue bars showing system mean time between failures collapsing as fleet size grows: 1 GPU at 50,000 hours, 1,000 GPUs at 50 hours, 10,000 GPUs at 5 hours."}
...
class NinesReliability:
    ...
    availability_str = fmt(availability_pct, precision=2, commas=False, suffix=' percent')
    downtime_min_str = fmt(downtime_min_year, precision=1, commas=False, suffix=' minutes')
    survival_str = fmt(single_gpu_success, precision=4, commas=False)
    cluster_prob_str = fmt(cluster_prob, precision=2, commas=False)
    cluster_pct_str = fmt_int(round(cluster_prob * 100), commas=False, suffix=' percent')
```
...
```{python}
#| echo: false
#| label: failure-impact-archetype-labels
# ┌─────────────────────────────────────────────────────────────────────────────
# │ FAILURE IMPACT ARCHETYPE LABELS
# ├─────────────────────────────────────────────────────────────────────────────
# │ Goal: Use the same fleet archetype names as the rest of the book.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Scenarios as BookScenarios
from mlsysim.fmt import MarkdownStr

class FailureImpactArchetypeLabels:
    """Display labels for recurring fleet archetypes in fault-tolerance tables."""

    a_label_str = MarkdownStr(BookScenarios.Archetypes.Fleet.ArchetypeA.label)
    b_label_str = MarkdownStr(BookScenarios.Archetypes.Fleet.ArchetypeB.label)
```

| **Model Type**                                          | **Typical Training Duration** | **Checkpoint Size** | **State Sensitivity**           | **Failure Cost**             |
|:--------------------------------------------------------|:------------------------------|--------------------:|:--------------------------------|:-----------------------------|
| **`{python} FailureImpactArchetypeLabels.a_label_str`** | 2–4 weeks                     |          350–700 GB | High (position in curriculum)   | \$2–5M compute per 24hr loss |
| **Vision (ViT-Large)**                                  | 1–3 days                      |              1–2 GB | Medium (augmentation state)     | \$10–50K per day loss        |
| **`{python} FailureImpactArchetypeLabels.b_label_str`** | Continuous                    | 2–4 TB (embeddings) | Very High (embedding freshness) | Revenue impact per hour      |
...
class FaultToleranceIntroRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpt3_ckpt_tb_str = fmt(gpt3_ckpt_bytes.m_as(TB), precision=1, commas=False, suffix=' TB')
```
...
Hardware faults fall into three categories based on temporal characteristics, as @fig-fault-temporal-categories summarizes.
...
Systematic testing (unit, integration, and regression) forms the first line of defense (@fig-regression-testing-ft). Automated CI/CD pipelines (@fig-CI-CD-procedure-ft) embed testing, validation, and monitoring directly into the software delivery process, with gates at each stage that reduce the risk of unnoticed regressions.\index{Pipeline!ci/cd}\index{Regression Test Automation}
...
class SDCCollective:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    prob_str = fmt(p_step_sdc*100, precision=4, commas=False, suffix="%")
    n_gpus_str = fmt(n_gpus, precision=0, commas=True)
    training_step_s_str = fmt(training_step_s, precision=0, commas=False)
```

::: {#nbk-fault-tolerance-napkin-math-sdc-certainty .callout-notebook title="The SDC certainty"}

**Problem**: Calculate the probability that at least one GPU in a `{python} SDCCollective.n_gpus_str`-GPU fleet experiences a silent ALU error during a single `{python} SDCCollective.training_step_s_str`-second training step.
...
::: {.column-margin}
![](images/svg/vol2_fault_tolerance_margin_002.svg){width="100%" fig-alt="Margin illustration showing Checkpoint payload composition dominated by Adam state."}
...
class FaultToleranceCheckpointRecap:
    ...
    ckpt_write_s = (gpt3_ckpt_bytes / aggregate_ckpt_bw).to(second).magnitude
    check(34 < ckpt_write_s < 38, f"GPT-3 checkpoint write should be ~36 s, got {ckpt_write_s:.1f}")
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpt3_params_b_str = fmt(m.parameters.m_as(param) / BILLION, precision=0, commas=False)
    gpt3_ckpt_tb_str = fmt(gpt3_ckpt_bytes.m_as(TB), precision=1, commas=False, suffix=' TB')
    gpt3_adam_tb_str = fmt(gpt3_adam_bytes.m_as(TB), precision=1, commas=False, suffix=' TB')
    gpt3_shard_gb_str = fmt(gpt3_ckpt_bytes.m_as(GB) / n_workers, precision=1, commas=False)
    ckpt_write_s_str = fmt_int(round(ckpt_write_s), commas=False)
    aggregate_read_gbs_str = fmt(aggregate_ckpt_bw.m_as(GB / second), precision=0, commas=False, suffix=' GB/s')
```

[^fn-adam-state-memory]: **Adam Optimizer State**\index{Adam Optimizer!state}: Adaptive Moment Estimation (Adam) maintains per-parameter first-moment ($m$) and second-moment ($v$) estimates, tripling memory requirements compared to vanilla SGD (3$\times$: parameters + two state vectors). For a `{python} FaultToleranceCheckpointRecap.gpt3_params_b_str`B parameter model, optimizer state alone reaches `{python} FaultToleranceCheckpointRecap.gpt3_adam_tb_str`, which typically dominates checkpoint size and recovery time, making optimizer state the primary bottleneck in checkpoint I/O design. \index{Adam Optimizer!checkpoint memory}
...
For our 10,000-GPU cluster with calculated system MTBF of 3.69 hours (from @sec-fault-tolerance-reliability-reliability-worked-example-cluster-mtbf-calculation-9255) and checkpoint time of `{python} FaultToleranceCheckpointRecap.ckpt_write_s_str` s (from `{python} FaultToleranceCheckpointRecap.gpt3_ckpt_tb_str` checkpoint at `{python} FaultToleranceCheckpointRecap.aggregate_read_gbs_str`), the Young-Daly formula computes the optimal interval.
...
class YoungDalyOptimal:
    ...
    t_write_s = (_mem.total_memory / (100 * GB / second)).to(second).magnitude
    mtbf_h = 3.69
    # ┌── 2. EXECUTE ───────────────────────────────────────
    mtbf_s = mtbf_h * SEC_PER_HOUR
    radicand = 2 * t_write_s * mtbf_s
    tau_opt_s = math.sqrt(radicand)
    tau_opt_min = tau_opt_s / 60
    overhead_pct = (t_write_s/tau_opt_s + tau_opt_s / (2 * mtbf_s)) * 100
    hourly_interval_s = SEC_PER_HOUR
    hourly_overhead_pct = (t_write_s/hourly_interval_s + hourly_interval_s / (2 * mtbf_s)) * 100
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(14 < tau_opt_min < 18, f"Interval {tau_opt_min:.1f}m outside expected range")
    check(hourly_overhead_pct > overhead_pct, "Hourly checkpointing should waste more than optimal")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    t_write_s_str = fmt(t_write_s, precision=1, commas=False)
    mtbf_h_str = fmt(mtbf_h, precision=2, commas=False, suffix=' hours')
    mtbf_s_str = fmt(mtbf_s, precision=0)
    tau_opt_s_str = fmt(tau_opt_s, precision=1)
    tau_opt_min_str = fmt(tau_opt_min, precision=1, commas=False, suffix=' min')
    overhead_pct_str = fmt(overhead_pct, precision=1, commas=False, suffix=' percent')
    hourly_overhead_pct_str = fmt(hourly_overhead_pct, precision=1, commas=False, suffix=' percent')
...
**Problem**: A `{python} YoungDalyOptimal.n_gpus_str`-GPU cluster has an MTBF of `{python} YoungDalyOptimal.mtbf_h_str`. A full model checkpoint takes `{python} YoungDalyOptimal.t_write_s_str` s to write. What is the optimal checkpoint frequency?

**Math**:
Apply the Young-Daly formula: $\tau_{\text{opt}} = \sqrt{2 \cdot T_{\text{write}} \cdot \text{MTBF}_{\text{system}}}$

1. **Convert to common units**: $\text{MTBF}_{\text{system}} =$ `{python} YoungDalyOptimal.mtbf_h_str` $\times$ `{python} YoungDalyOptimal.seconds_per_hour_str` s/hr = `{python} YoungDalyOptimal.mtbf_s_str` s.
2. **Substitute and solve**: With $T_{\text{write}} =$ `{python} YoungDalyOptimal.t_write_s_str` s and $\text{MTBF} =$ `{python} YoungDalyOptimal.mtbf_s_str` s, the formula gives $\tau_{\text{opt}} \approx$ `{python} YoungDalyOptimal.tau_opt_s_str` s, or about `{python} YoungDalyOptimal.tau_opt_min_str`.
...
class CheckpointOverheadCalc:
    ...
    # -- Write times
    archA_write_str = fmt(archA_write_s, precision=0, commas=False, suffix=" s")
    gpt35_write_str = fmt(gpt35_write_s, precision=1, commas=False, suffix=" s")
    bert_write_str = fmt(bert_write_s, precision=3, commas=False, suffix=" s")
    archB_write_str = fmt(archB_write_s, precision=0, commas=False, suffix=" s")
    resnet_write_str = fmt(resnet_write_s, precision=3, commas=False, suffix=" s")
    vit_write_str = fmt(vit_write_s, precision=3, commas=False, suffix=" s")

    # -- Optimal intervals (min for >= 60s, s for < 60s)
    archA_interval_str = fmt(archA_interval_s / 60, precision=1, commas=False, suffix=" min")
    gpt35_interval_str = fmt(gpt35_interval_s / 60, precision=1, commas=False, suffix=" min")
    bert_interval_str = fmt_int(bert_interval_s, commas=False, suffix=" s")
    archB_interval_str = fmt_int(archB_interval_s / 60, commas=False, suffix=" min")
    resnet_interval_str = fmt_int(resnet_interval_s, commas=False, suffix=" s")
    vit_interval_str = fmt_int(vit_interval_s, commas=False, suffix=" s")

    # -- Save overhead percentages
    archA_overhead_str = fmt(archA_overhead_pct, precision=1, commas=False, suffix=" percent")
    gpt35_overhead_str = fmt(gpt35_overhead_pct, precision=1, commas=False, suffix=" percent")
    bert_overhead_str = fmt(bert_overhead_pct, precision=2, commas=False, suffix=" percent")
    archB_overhead_str = fmt(archB_overhead_pct, precision=1, commas=False, suffix=" percent")
    resnet_overhead_str = fmt(resnet_overhead_pct, precision=2, commas=False, suffix=" percent")
    vit_overhead_str = fmt(vit_overhead_pct, precision=2, commas=False, suffix=" percent")
```

| **Model Type**                                          |                               **Checkpoint Size** |                          **Write Time (100 GB/s)** |              **Optimal Interval (5hr GPU-only MTBF)** |                                     **Save Overhead** |
|:--------------------------------------------------------|--------------------------------------------------:|---------------------------------------------------:|------------------------------------------------------:|------------------------------------------------------:|
| **`{python} FailureImpactArchetypeLabels.a_label_str`** |  `{python} CheckpointOverheadCalc.archA_size_str` |  `{python} CheckpointOverheadCalc.archA_write_str` |  `{python} CheckpointOverheadCalc.archA_interval_str` |  `{python} CheckpointOverheadCalc.archA_overhead_str` |
| **GPT-3.5 (20B)**                                       |  `{python} CheckpointOverheadCalc.gpt35_size_str` |  `{python} CheckpointOverheadCalc.gpt35_write_str` |  `{python} CheckpointOverheadCalc.gpt35_interval_str` |  `{python} CheckpointOverheadCalc.gpt35_overhead_str` |
| **BERT-Large**                                          |   `{python} CheckpointOverheadCalc.bert_size_str` |   `{python} CheckpointOverheadCalc.bert_write_str` |   `{python} CheckpointOverheadCalc.bert_interval_str` |   `{python} CheckpointOverheadCalc.bert_overhead_str` |
| **`{python} FailureImpactArchetypeLabels.b_label_str`** |  `{python} CheckpointOverheadCalc.archB_size_str` |  `{python} CheckpointOverheadCalc.archB_write_str` |  `{python} CheckpointOverheadCalc.archB_interval_str` |  `{python} CheckpointOverheadCalc.archB_overhead_str` |
| **ResNet-50**\index{ResNet-50}                          | `{python} CheckpointOverheadCalc.resnet_size_str` | `{python} CheckpointOverheadCalc.resnet_write_str` | `{python} CheckpointOverheadCalc.resnet_interval_str` | `{python} CheckpointOverheadCalc.resnet_overhead_str` |
| **ViT-Large**                                           |    `{python} CheckpointOverheadCalc.vit_size_str` |    `{python} CheckpointOverheadCalc.vit_write_str` |    `{python} CheckpointOverheadCalc.vit_interval_str` |    `{python} CheckpointOverheadCalc.vit_overhead_str` |
...
class CheckpointDebug:
    ...
    nfs_bw = 10 * ureg.Gbps
    pcie_bw = 32 * ureg.GB / ureg.second
    nvme_bw_gbs = 3.5
    n_nodes = 64
    n_gpus_ckpt = 512
    ...
    bytes_per_param = 10
    total_ckpt_qty = m.parameters.m_as(param) * bytes_per_param * ureg.byte
    total_ckpt_gb_val = total_ckpt_qty.m_as(GB)

    # Use library formula for transfer time
    t_write = (total_ckpt_qty / nfs_bw)
    min_write_s_val = t_write.m_as("second")
    min_write_min_val = t_write.m_as("minute")

    per_node_shard_qty = total_ckpt_qty / n_nodes
    per_node_shard_gb_val = per_node_shard_qty.m_as(GB)

    # Staging over PCIe
    t_stage = (per_node_shard_qty / pcie_bw)
    t_stage_single = (total_ckpt_qty / pcie_bw)
    gpu_cpu_parallel_s_val = t_stage.m_as("second")
    gpu_cpu_single_path_s_val = t_stage_single.m_as("second")

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(total_ckpt_gb_val == 700, f"Expected 700 GB checkpoint, got {total_ckpt_gb_val}")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    weights_gb_str = fmt(m.parameters.m_as(param) * 2 / BILLION, precision=0, commas=False, suffix=' GB')
    optimizer_gb_str = fmt(m.parameters.m_as(param) * 8 / BILLION, precision=0, commas=False, suffix=' GB')
    total_ckpt_gb_str = fmt(total_ckpt_gb_val, precision=0, commas=False, suffix=' GB')
    nfs_gbs_str = fmt(nfs_bw.m_as(GB/second), precision=2, commas=False, suffix=' GB/s')
    min_write_s_str = fmt(min_write_s_val, precision=0, commas=False)
    min_write_min_str = fmt(min_write_min_val, precision=1, commas=False, suffix=' min')
    per_node_mbs_str = fmt_int(nfs_bw.m_as(MB/second) / n_nodes, commas=False, suffix=' MB/s')
    per_node_shard_gb_str = fmt(per_node_shard_gb_val, precision=1, commas=False, suffix=' GB')
    per_node_write_min_str = fmt(min_write_min_val, precision=1, commas=False, suffix=' min') # Aggregated BW bottleneck
    gpu_cpu_parallel_s_str = fmt(gpu_cpu_parallel_s_val, precision=2, commas=False)
    gpu_cpu_single_path_s_str = fmt(gpu_cpu_single_path_s_val, precision=1, commas=False)
    extended_weeks_str = fmt(base_weeks * (1 + overhead_pct / 100), precision=1, commas=False, suffix=' weeks')
    extra_cost_k_str = fmt(extra_cost_k, precision=0, commas=False)
    params_b_str = fmt(m.parameters.m_as(param) / BILLION, precision=0, commas=False, suffix="B")
    bytes_per_param_weights_str = fmt(2, precision=0, commas=False)
    n_gpus_str = fmt(n_gpus_ckpt, precision=0, commas=False, suffix=' GPUs')
    n_nodes_str = fmt(n_nodes, precision=0, commas=False, suffix=' nodes')
    observed_write_min_str = fmt(observed_write_min, precision=0, commas=False, suffix=' minutes')
    target_write_min_str = fmt(target_write_min, precision=0, commas=False)
    overhead_pct_str = fmt(overhead_pct, precision=0, commas=False, suffix=' percent')
    sla_weeks_str = fmt(base_weeks, precision=0, commas=False, suffix=' weeks')
    nvme_bw_gbs_str = fmt(nvme_bw_gbs, precision=1, commas=False, suffix=' GB/s')
    pcie_gbs_str = fmt(pcie_bw.m_as(GB/second), precision=0, commas=False, suffix=' GB/s')
    residual_overhead_pct_str = fmt(residual_overhead_pct, precision=0, commas=False, suffix=' percent')
```
...
- **Minimum write time**\index{Minimum Write Time}: `{python} CheckpointDebug.total_ckpt_gb_str` / `{python} CheckpointDebug.nfs_gbs_str` = `{python} CheckpointDebug.min_write_s_str` s (`{python} CheckpointDebug.min_write_min_str`)
...
- **Budget impact**: Extended training incurs additional \$`{python} CheckpointDebug.extra_cost_k_str`K in compute costs
...
3. **Validation**\index{Validation}: The critical-path data copy drops to approximately `{python} CheckpointDebug.gpu_cpu_parallel_s_str` s if GPU-to-CPU staging proceeds in parallel across `{python} CheckpointDebug.n_nodes_str` (or `{python} CheckpointDebug.gpu_cpu_single_path_s_str` s if serialized through one `{python} CheckpointDebug.pcie_gbs_str` path), reducing training impact from `{python} CheckpointDebug.overhead_pct_str` to under `{python} CheckpointDebug.residual_overhead_pct_str` once background NVMe and NFS copies are overlapped
...
class FaultToleranceShardRecap:
    ...
    gpt3_params_b_str = fmt(m.parameters.m_as(param) / BILLION, precision=0, commas=False)
    gpt3_ckpt_tb_str = fmt(gpt3_ckpt_bytes.m_as(TB), precision=1, commas=False, suffix=' TB')
    gpt3_shard_gb_str = fmt(gpt3_ckpt_bytes.m_as(GB) / n_workers, precision=1, commas=False, suffix=' GB')
    n_workers_str = fmt(n_workers, precision=0, commas=True)
```

[^fn-sharded-ckpt-io]: **Sharded Checkpointing**: Each worker saves only its local partition rather than gathering state to a single writer. For ZeRO-3 or FSDP with `{python} FaultToleranceShardRecap.n_workers_str` workers training a `{python} FaultToleranceShardRecap.gpt3_params_b_str`B model, each worker writes approximately `{python} FaultToleranceShardRecap.gpt3_shard_gb_str` instead of one writer handling `{python} FaultToleranceShardRecap.gpt3_ckpt_tb_str`, parallelizing I/O across all nodes. The trade-off: recovery requires all shards to be present and consistent, making the checkpoint protocol more complex and the failure of any single shard's storage fatal to the entire checkpoint. \index{Sharded Checkpointing!I/O parallelism}

This approach enables efficient checkpointing even for massive models. A `{python} FaultToleranceShardRecap.gpt3_params_b_str`B parameter model with a `{python} FaultToleranceShardRecap.gpt3_ckpt_tb_str` checkpoint distributed across `{python} FaultToleranceShardRecap.n_workers_str` workers requires each worker to write only `{python} FaultToleranceShardRecap.gpt3_shard_gb_str`, achievable in seconds with local NVMe storage.
...
class RecoveryBudget:
    ...
    t_detect_s = 60
    t_restart_min = 3
    aggregate_read_gbs = 100
    aggregate_ckpt_bw = aggregate_read_gbs * GB / second
    t_load_s = (_mem.total_memory / aggregate_ckpt_bw).to(second).magnitude
    t_warmup_min = 2
    shard_read_gbs = 5
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    t_detect_min = t_detect_s / 60
    t_load_min = t_load_s / 60
    t_total_min = t_detect_min + t_restart_min + t_load_min + t_warmup_min
    daily_loss_min_1k = failures_per_day_1k * t_total_min
    daily_loss_min_10k = failures_per_day_10k * t_total_min
    overhead_pct_1k = daily_loss_min_1k / (24 * 60) * 100
    overhead_pct_10k = daily_loss_min_10k / (24 * 60) * 100
    waste_gpu_hours_10k = daily_loss_min_10k * n_gpus_10k / 60

    # ┌── 3. GUARD ─────────────────────────────────────────
    check(t_total_min > 0, "Recovery time must be positive")
    check(t_total_min < 30, "Recovery time should be under 30 min")
    check(abs(daily_loss_min_1k - 3.0) < 0.5, f"1k daily loss ({daily_loss_min_1k:.1f} min)")
    check(34 < t_load_s < 38, f"Aggregate checkpoint load should be ~36 s, got {t_load_s:.1f}")
    check(abs(daily_loss_min_10k - 31.0) < 2.0, f"10k daily loss ({daily_loss_min_10k:.1f} min)")

    # ┌── 4. OUTPUT ────────────────────────────────────────
    t_total_str = fmt(t_total_min, precision=1, commas=False)
    t_detect_s_str = fmt(t_detect_s, precision=0, commas=False)
    t_restart_min_str = fmt(t_restart_min, precision=0, commas=False, suffix=' min')
    t_load_s_str = fmt_int(round(t_load_s), commas=False)
    t_warmup_min_str = fmt(t_warmup_min, precision=0, commas=False, suffix=' min')
    aggregate_read_gbs_str = fmt(aggregate_read_gbs, precision=0, commas=False, suffix=' GB/s')
    shard_read_gbs_str = fmt(shard_read_gbs, precision=0, commas=False, suffix=' GB/s')
    n_gpus_1k_str = fmt(n_gpus_1k, precision=0, commas=True)
    n_gpus_10k_str = fmt(n_gpus_10k, precision=0, commas=True)
    mtbf_1k_hrs_str = fmt(mtbf_1k_hrs, precision=0, commas=False, suffix=" hours")
    mtbf_10k_hrs_str = fmt(mtbf_10k_hrs, precision=0, commas=False, suffix=" hours")
    ...
    daily_loss_min_1k_str = fmt_int(round(daily_loss_min_1k), commas=False, suffix=" minutes")
    daily_loss_min_10k_str = fmt_int(round(daily_loss_min_10k), commas=False, suffix=" minutes")
    overhead_pct_1k_str = fmt(overhead_pct_1k, precision=1, commas=False, suffix=" percent")
    overhead_pct_10k_str = fmt(overhead_pct_10k, precision=1, commas=False, suffix=" percent")
...
class FaultToleranceRecoveryRecap:
    ...
    n_workers = Systems.Clusters.Training_1K.total_accelerators
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    params_b_str = fmt(m.parameters.m_as(param) / BILLION, precision=0, commas=False)
    gpt3_ckpt_tb_str = fmt(gpt3_ckpt_bytes.m_as(TB), precision=1, commas=False, suffix=' TB')
    gpt3_shard_gb_str = fmt(gpt3_ckpt_bytes.m_as(GB) / n_workers, precision=1, commas=False, suffix=' GB')
    n_workers_str = fmt(n_workers, precision=0, commas=True, suffix=' GPUs')
```

Consider our `{python} FaultToleranceRecoveryRecap.params_b_str`B parameter model training on `{python} FaultToleranceRecoveryRecap.n_workers_str`. The checkpoint size is approximately `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` (weights + Adam optimizer state). How much does a single failure event cost?

**The budget ($T_{\text{recovery}}$)**:

1. **$T_{\text{detect}}$**: `{python} RecoveryBudget.t_detect_s_str` s (conservative heartbeat timeout with verification retries)
...
**Total**: $T_{\text{recovery}} = T_{\text{detect}} + T_{\text{restart}} + T_{\text{load}} + T_{\text{warmup}} \approx$ `{python} RecoveryBudget.t_total_str` minutes per failure event.
...
Warm restarts can reduce $T_{\text{load}}$ and $T_{\text{warmup}}$ to near-zero for 99.9 percent of the cluster, cutting total recovery time from minutes to seconds. For the `{python} FaultToleranceRecoveryRecap.n_workers_str` example, a warm restart avoids reloading `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` from storage, saving the `{python} RecoveryBudget.t_load_s_str`-second $T_{\text{load}}$ and 2-minute $T_{\text{warmup}}$ for all but the replacement node.
...
class StragglerTax:
    ...
    gpu_cost_per_hour = Infrastructure.Pricing.Cloud.GpuTrainingUtilityScenarioPerHour.rate.m_as("dollar / hour")
    ...
    throttle_pct_str = fmt_int(throttle_factor * 100, commas=False, suffix=" percent")
    ...
    straggler_frac_str = fmt(straggler_frac_pct, precision=1, commas=False, suffix=" percent")
    throughput_loss_str = fmt_int(throughput_loss_pct, commas=False)
    gpu_cost_str = fmt_int(gpu_cost_per_hour, commas=False)
    wasted_cost_str = fmt(wasted_cost_per_hour, precision=0)
```
...
A single failing device (`{python} StragglerTax.straggler_frac_str` of the hardware) has reduced the throughput of the entire cluster by `{python} StragglerTax.throughput_loss_str` percent. At \$`{python} StragglerTax.gpu_cost_str`/GPU-hour, this straggler wastes \$`{python} StragglerTax.wasted_cost_str`/hour in idle compute.
...
The recovery adaptation mechanisms above (batch size adjustment, learning rate recalibration, data loader redistribution, state resharding) must be implemented by the training framework. Frameworks differ in how much of this recovery logic they automate and how they coordinate the surviving workers after a failure.
...
Recommendation models with trillion-parameter embedding tables present unique checkpoint challenges. Embedding tables can be multiple terabytes, making full checkpointing at high frequency impractical. Strategies include:
...
::: {.column-margin}
![](images/svg/vol2_fault_tolerance_margin_004.svg){width="100%" fig-alt="Margin illustration showing Yearly downtime drops from one replica to multiple replicas."}
...
After degradation events, conduct postincident analysis. Analyze root cause of degradation, effectiveness of fallback mechanisms, user impact and business impact, and improvements to prevent future degradation.
...
Effective distributed debugging requires three observability pillars, shown in @fig-observability-three-pillars: metrics, logs, and traces.
...
OpenTelemetry provides a standard API for distributed tracing\index{Distributed Tracing}. Backend systems like Jaeger, Zipkin, or cloud tracing services store and visualize traces.
...
The case studies that follow draw on published Meta OPT-175B training logbooks, Google's TPUv4 resiliency paper, Netflix's published chaos-engineering literature, and Microsoft DeepSpeed documentation. Specific operational numbers (failure counts, MFU achieved, availability percentages, and overhead percentages) reflect what those public sources have reported; treat them as representative magnitudes for the engineering pattern rather than precision benchmarks reproducible from this prose alone. Production deployments illustrate how leading organizations implement fault tolerance in ML systems, threading the principles developed throughout this chapter. Meta's OPT-175B training run on a 992-GPU cluster provides a well-documented study in handling dozens of hardware failures over the run without aborting training.
