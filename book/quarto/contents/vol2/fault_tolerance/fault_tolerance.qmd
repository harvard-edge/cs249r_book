...
: **Cluster MTBF Scaling**\index{Cluster!mtbf scaling}: System-level mean time between failures decreases linearly with cluster size, transforming failures from rare events to continuous operating conditions at scale. A training cluster sized for modern large language model (LLM) development (10,000+ GPUs) experiences multiple failures daily. {#tbl-cluster-mtbf-scaling}
...
**Impact at scale**\index{Impact at Scale}: With a 1,000-GPU cluster MTBF of 50 hours (from @tbl-cluster-mtbf-scaling), we experience about 0.48 failures per day, losing about 3 minutes daily—a modest 0.2 percent overhead. However, at 10,000 GPUs with MTBF of 5 hours, we experience about 4.8 failures per day, losing about 31 minutes daily—a 2.1 percent overhead equivalent to wasting roughly 5,000 GPU-hours every day (31 min $\times$ 10,000 GPUs ÷ 60). This is why recovery time optimization matters more as clusters grow.
...
The case studies that follow draw on published Meta OPT-175B training logbooks, Google TPU operational disclosures, Netflix engineering blog posts, and Microsoft DeepSpeed documentation. Specific operational numbers (failure counts, MFU achieved, MTTR figures, overhead percentages) reflect what those public sources have reported; treat them as representative magnitudes for the engineering pattern rather than precision benchmarks reproducible from this prose alone. Production deployments illustrate how leading organizations implement fault tolerance in ML systems, threading the principles developed throughout this chapter. Meta's OPT-175B training run on a 992-GPU cluster provides a well-documented study in handling dozens of hardware failures per week without aborting the run.
...
* **The Young-Daly formula governs checkpoint intervals**: The optimal checkpoint interval $\tau_{\text{opt}} = \sqrt{2 \times T_{\text{write}} \times \text{MTBF}}$ balances the cost of saving state against the cost of lost work, typically yielding intervals of 20 to 30 minutes for large-scale training clusters.
