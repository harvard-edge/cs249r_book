...
Byzantine failures represent the most challenging class: components continue operating but produce incorrect results. Named after the Byzantine Generals Problem[^fn-byzantine-generals] [@lamport1982byzantine] in distributed systems theory, these failures include:
...
Byzantine-resilient distributed training algorithms exist but impose significant overhead. Algorithms such as Krum [@blanchard2017machine] and coordinate-wise trimmed mean [@yin2018byzantine] compute aggregates that are robust to a bounded number of Byzantine workers, but they require more communication and computation than simple averaging.[^fn-byzantine-ml]
...
For large models, optimizer state often exceeds model size. Adam[^fn-adam-state] maintains two state tensors per parameter, so optimizer state equals twice the model parameter count. The two state tensors are $m$ and $v$. For a 175B parameter model at FP32 precision:
...
The Young-Daly formula [@young1974first; @daly2006higher] provides the optimal checkpoint interval that minimizes expected total time including both checkpointing overhead and failure recovery:
...
**Derivation sketch**: Let $T_c$ be the checkpoint interval. Expected wasted work per failure is $T_c/2$ plus recovery time. On average, failure occurs halfway through the interval. Checkpoint overhead per interval is $T_{save}/T_c$. Minimizing total expected time yields the square root relationship.
...
**Interpretation**: Optimal checkpoint interval is approximately 14.5 minutes. With this interval, checkpoint overhead consumes 2.4% of training time.

**Verification**: Expected wasted work per failure is 435 seconds. Expected number of failures per hour is 0.2. Expected wasted work per hour is 87 seconds. Checkpoint overhead per hour is 87 seconds. The optimal interval balances these equal contributions to overhead, confirming the formula's validity.
...
Snapshot data consumes additional memory until the write completes, creating memory pressure. The snapshot must represent a consistent point in training, requiring careful coordination. Background writes compete for storage bandwidth with data loading, causing I/O contention.

Modern frameworks like PyTorch's FSDP [@zhao2023pytorch] provide asynchronous checkpointing support through memory-efficient state snapshotting and background writes. DeepSpeed's checkpointing [@rasley2020deepspeed] similarly supports overlap between checkpoint writes and continued training.[^fn-async-checkpoint]
...
#### Checkpoint Storage Architecture {#sec-checkpoint-storage-architecture-ft}
...
Checkpoints can consume significant storage budget at scale. A 175B model training with 14-minute checkpoint intervals for 30 days generates:

$$\frac{30 \times 24 \times 60}{14} \times 2.1\text{ TB} = 6,480\text{ TB}$$

At $0.02/GB/month for object storage, this costs $130,000/month just for checkpoint storage. Production systems implement checkpoint versioning and garbage collection. They retain only recent checkpoints plus periodic snapshots for long-term recovery.
...
Traditional distributed training operates with a fixed number of workers. Failures require either waiting for replacement resources or restarting with fewer workers. Restarting with fewer workers requires checkpoint resharding. **Elastic training** enables dynamic scaling by adding or removing workers without stopping training.
...
This section develops fault tolerance mechanisms appropriate for serving's stringent requirements, building on single-machine serving fundamentals while addressing the scale and complexity of distributed serving systems.
...
Reliability engineering provides the quantitative tools for reasoning about failure. The exponential reliability model shows that system failure rate grows linearly with component count, making failures inevitable at scale. We presented this in @eq-system-reliability-n-components. The Young-Daly formula provides optimal checkpoint intervals that balance checkpoint overhead against expected lost work. We derived this in @eq-young-daly. These mathematical tools transform fault tolerance from intuition-based design to quantitative engineering.
