...
```{python}
#| label: frontiers-setup
#| echo: false
from mlsys.constants import *
from mlsys.formatting import fmt, sci

# Pre-format constants as strings for inline use
# --- Model parameter counts ---
gpt3_params_B = f"{int(GPT3_PARAMS.to(param).magnitude / BILLION)}"          # "175"
gpt3_params_label = f"{gpt3_params_B}B"                                   # "175B"
gpt3_params_billion = f"{gpt3_params_B} billion"                          # "175 billion"
gpt2_params_B = f"{GPT2_PARAMS.to(param).magnitude / BILLION:.1f}"           # "1.5"
gpt2_params_label = f"{gpt2_params_B}B"                                   # "1.5B"

# --- GPU specs ---
h100_tdp_w = f"{int(H100_TDP.to(watt).magnitude)}"                       # "700"

# --- TPU specs ---
tpuv4_bf16_tflops = f"{int(TPUV4_FLOPS_BF16.to(TFLOPs/second).magnitude)}"  # "275"
```

This chapter's position in the book's organizing framework clarifies how the constraints encountered at each layer converge to define the boundaries of what next-generation ML systems can achieve.
...
::: {.callout-example title="Inference Compute Multiplier"}
...
[^fn-chinchilla-laws]: **Chinchilla Scaling Laws**: Discovered by DeepMind [@hoffmann2022training], optimal model performance requires balanced scaling of parameters N and training tokens D following N ∝ D^0.74. Previous models were under-trained: GPT-3 (`{python} gpt3_params_label` parameters, 300B tokens) should have used 4.6 trillion tokens for optimal performance. Chinchilla (70B parameters, 1.4T tokens) outperformed GPT-3 despite being 2.5× smaller, proving data quality matters more than model size.
...
These requirements impose a permanent *alignment tax* on system performance, a cost that cannot be engineered away because it is intrinsic to operating safely.

::: {.callout-note title="The Alignment Tax: Cost of Safety"}
...
Multi-agent coordination provides engineering benefits (modularity, specialization, fault isolation) but introduces coordination costs (latency, complexity, failure modes). The trade-off is workload-specific: tasks requiring tight coordination between components may perform better with monolithic models, while tasks with natural decomposition may benefit from multi-agent approaches. The following checkpoint consolidates our analysis of engineering patterns before we turn to practical guidance.

::: {.callout-note title="Checkpoint: Engineering Patterns"}
...
[^fn-tpu]: **Tensor Processing Unit (TPU)**: Google's custom ASIC designed for neural network ML. TPU v4 (2021) delivers `{python} tpuv4_bf16_tflops` teraFLOPs for training with specialized matrix multiplication units. The key engineering question is not whether specialized hardware provides benefits (it does) but how to orchestrate heterogeneous hardware efficiently.
...
This section examines enduring design principles that guide frontier ML systems regardless of which specific technologies prevail. @tbl-frontier-mapping connects system challenges to foundational knowledge; the principles here transcend particular implementations.
...
**Fallacy:** ***Scaling solves all capability limitations.***
...
**Fallacy:** ***Compound systems are temporary workarounds.***
...
**Fallacy:** ***Frontier systems require entirely new engineering principles.***
...
**Pitfall:** ***Treating biological efficiency as achievable target.***
...
**Fallacy:** ***Inference-time compute makes training efficiency irrelevant.***
...
**Pitfall:** ***Planning based on extrapolated scaling laws.***
