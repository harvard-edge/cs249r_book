...
class CriticalMsgSize:
    ...
    beta_str = fmt_qty(beta, GB/second, precision=0, commas=False)
...
**Problem**: Consider a cluster using InfiniBand NDR 400 Gbps with $\alpha =$ `{python} CriticalMsgSize.alpha_str` $\mu\text{s}$ and $\beta =$ `{python} CriticalMsgSize.beta_str`. At what message size does optimizing for bandwidth start to matter more than optimizing for latency?
...
class LlamaBudgetCalc:
    ...
    nvlink_bw_str              = fmt_qty(nvlink_bw, GB/second, precision=0, commas=False)
    ib_bw_str                  = fmt_qty(ib_bw, GB/second, precision=0, commas=False)
```
...
**Latency Analysis** (InfiniBand NDR, $\alpha =$ `{python} MoeAllToAll.alpha_str` $\mu\text{s}$, $\beta =$ `{python} MoeAllToAll.beta_str`):
...
class RingVsTree:
    ...
    alpha_str = fmt(alpha_us, precision=0, commas=False)
    beta_str = fmt_qty(beta, GB/second, precision=0, commas=False)
    ring_lat_str = fmt_time(ring_lat, unit=microsecond, precision=0)
    tree_lat_str = fmt_time(tree_lat, unit=microsecond, precision=0, commas=False)
    ring_bw_str = fmt_time(ring_bw, unit=microsecond, precision=1, commas=False)
    tree_bw_str = fmt_time(tree_bw, unit=microsecond, precision=0)
    ring_total_str = fmt_time(ring_total, unit=microsecond, precision=1)
    tree_total_str = fmt_time(tree_total, unit=microsecond, precision=0)
    crossover_str = fmt_qty(crossover, MB, precision=1, commas=False)
...
**Problem**: A cluster synchronizes a `{python} RingVsTree.M_mb_str` buffer across `{python} RingVsTree.N_str` GPUs. The network has latency $\alpha =$ `{python} RingVsTree.alpha_str` $\mu\text{s}$ and bandwidth $\beta =$ `{python} RingVsTree.beta_str`. Which algorithm performs better: Ring or Tree?
