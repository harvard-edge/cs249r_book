...
class AllReduceCost:
    ...
    t_ring = calc_ring_allreduce_time(
        ...
        latency_s=alpha_us * microsecond
    )
...
**Problem**: Consider a cluster using InfiniBand NDR 400 Gbps with $\alpha =$ `{python} CriticalMsgSize.alpha_str` $\mu\text{s}$ and $\beta =$ `{python} CriticalMsgSize.beta_gb_s_str`. At what message size does optimizing for bandwidth start to matter more than optimizing for latency?
...
**Latency Analysis** (InfiniBand NDR, $\alpha =$ `{python} MoeAllToAll.alpha_str` $\mu\text{s}$, $\beta =$ `{python} MoeAllToAll.beta_gb_s_str`):
...
from mlsysim.fmt import check, fmt, fmt_percent, fmt_qty
...
**Problem**: A cluster synchronizes a `{python} RingVsTree.M_mb_str` buffer across `{python} RingVsTree.N_str` GPUs. The network has latency $\alpha =$ `{python} RingVsTree.alpha_str` $\mu\text{s}$ and bandwidth $\beta =$ `{python} RingVsTree.beta_gb_s_str`. Which algorithm performs better: Ring or Tree?

**Math**:

**Latency**:

1.  **Ring Latency**\index{Ring AllReduce!latency}: $2(N-1)\alpha = 2 \times 63 \times 10\ \mu\text{s}$ = `{python} RingVsTree.ring_lat_us_str` $\mu\text{s}$.
2.  **Tree Latency**\index{Tree AllReduce!latency}: $2(\log_2 N)\alpha = 2 \times 6 \times 10\ \mu\text{s}$ = `{python} RingVsTree.tree_lat_us_str` $\mu\text{s}$.

**Bandwidth** (note the difference):

3.  **Ring Bandwidth**\index{Ring AllReduce!bandwidth}: $2\frac{N-1}{N}\frac{M}{\beta} \approx 2 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = `{python} RingVsTree.ring_bw_us_str` $\mu\text{s}$ (optimal: each byte sent once).
4.  **Tree Bandwidth**\index{Tree AllReduce!bandwidth}: $2\log_2 N \cdot \frac{M}{\beta} = 12 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = `{python} RingVsTree.tree_bw_us_str` $\mu\text{s}$ (each level sends full message).
...
|:--------------|-----------------------------------------:|----------------------------------------:|-------------------------------------------:|
| **Ring**      | `{python} RingVsTree.ring_lat_us_str` μs | `{python} RingVsTree.ring_bw_us_str` μs | `{python} RingVsTree.ring_total_us_str` μs |
| **Tree**      | `{python} RingVsTree.tree_lat_us_str` μs | `{python} RingVsTree.tree_bw_us_str` μs | `{python} RingVsTree.tree_total_us_str` μs |
