...
\noindent
![](images/png/cover_communication_ops.png){fig-alt="Three communication topologies: parameter server (top) with nodes connecting through aggregators to central server, ring AllReduce (bottom left) with 8 GPUs in circular data flow, and all-to-all mesh (bottom right) with fully connected nodes." width=100 percent}
...
2.  **Bandwidth-Bound ($n \gg n$)**: For large messages (for example, LLM gradients, optimizer states), time is dominated by $n/\beta$. Optimization focuses on **compression** (FP8, Top-K sparsity), **algorithm choice** (Ring vs. Tree), and **link aggregation** (multi-rail NICs).
