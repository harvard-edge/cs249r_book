...
class RingVsTree:
    ...
    tree_win_margin_pct_str = fmt_int(tree_win_margin_pct, commas=False, suffix=" percent")
```
...
class HierarchicalAllreduceCalc:
    ...
    intra_rs_ms_str   = fmt_int(intra_rs_ms, commas=False, suffix=" ms")
    ...
    intra_ag_ms_str   = fmt_int(intra_ag_ms, commas=False, suffix=" ms")
...
class ErrorFeedbackTrace:
    ...
    naive_loss_pct_str = fmt(100, precision=0, commas=False, suffix=" percent")
```
...
```{python}
#| echo: false
#| label: collective-archetype-table
# ┌─────────────────────────────────────────────────────────────────────────────
# │ COLLECTIVE PATTERN BY ARCHETYPE
# ├─────────────────────────────────────────────────────────────────────────────
# │ Goal: Keep communication-pattern rows aligned to the canonical fleet
# │       archetypes in MLSysIM.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Scenarios as BookScenarios
from mlsysim.fmt import MarkdownStr

class CollectiveArchetypeTable:
    """Display fields for collective communication patterns by archetype."""
    ...
    a_collective_str = MarkdownStr(a.primary_communication)
    a_friction_str = MarkdownStr(a.dominant_friction)
    a_strategy_str = MarkdownStr(a.optimization_strategy)
    b_label_str = MarkdownStr(b.label)
    b_collective_str = MarkdownStr(b.primary_communication)
    b_friction_str = MarkdownStr(b.dominant_friction)
    b_strategy_str = MarkdownStr(b.optimization_strategy)
    c_label_str = MarkdownStr(c.label)
    c_collective_str = MarkdownStr(c.primary_communication)
    c_friction_str = MarkdownStr(c.dominant_friction)
    c_strategy_str = MarkdownStr(c.optimization_strategy)
```

| **Archetype**                                       | **Primary Collective**                               | **Dominant Friction**                              | **Optimization Strategy**                          |
|:----------------------------------------------------|:-----------------------------------------------------|:---------------------------------------------------|:---------------------------------------------------|
| **`{python} CollectiveArchetypeTable.a_label_str`** | `{python} CollectiveArchetypeTable.a_collective_str` | `{python} CollectiveArchetypeTable.a_friction_str` | `{python} CollectiveArchetypeTable.a_strategy_str` |
| **`{python} CollectiveArchetypeTable.b_label_str`** | `{python} CollectiveArchetypeTable.b_collective_str` | `{python} CollectiveArchetypeTable.b_friction_str` | `{python} CollectiveArchetypeTable.b_strategy_str` |
| **`{python} CollectiveArchetypeTable.c_label_str`** | `{python} CollectiveArchetypeTable.c_collective_str` | `{python} CollectiveArchetypeTable.c_friction_str` | `{python} CollectiveArchetypeTable.c_strategy_str` |
