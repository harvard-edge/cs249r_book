...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ COMMUNICATION SETUP — INTERCONNECT BANDWIDTH
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-communication-collective-operations — reliability introduction
# │
# │ Goal: Surface canonical interconnect bandwidth constants for inline prose.
# │ Show: nvlink_h100_bw_gbs, ib_ndr_bw_gbs in hardware fault exposure paragraph.
# │ How: fmt() on NVLINK_H100_BW, INFINIBAND_NDR_BW from mlsys.constants.
# │
...
ib_ndr_bw_gbs = f"{INFINIBAND_NDR_BW.m_as(ureg.bit / ureg.second) / 8 / BILLION:.0f}"
```
...
1.  **Significance (Quantitative):** It ensures mathematical equivalence to single-device training by ensuring all workers apply identical parameter updates. Within the **Iron Law**, it is the primary driver of the **Communication Overhead ($L_{lat}$)** and **Bandwidth ($BW$)** terms during training.
...
1.  **Significance (Quantitative):** They are the primary mechanisms for managing **Data Movement** across the fleet. Within the **Iron Law**, collective operations (e.g., AllReduce) determine the total **Communication Overhead ($L_{lat}$)** and whether training becomes **Bandwidth-Bound ($BW$)**.
2.  **Distinction (Durable):** Unlike **Point-to-Point Communication** (one-to-one), Collective Operations are **Group-Aware** (one-to-all or all-to-all), utilizing optimized algorithms (e.g., Ring, Tree) to minimize redundant transfers.
