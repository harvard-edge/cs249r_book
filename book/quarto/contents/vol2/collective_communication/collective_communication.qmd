...
2.  **Observation**: Unlike parameter-server approaches (where all workers send gradients to a centralized aggregator whose bandwidth scales as $\mathcal{O}(N)$), ring AllReduce distributes the communication across all workers so that each worker's per-step communication cost stays constant at $2 \times (N-1)/N \times M$ regardless of cluster size.
3.  **Pitfall**: A frequent misconception is that gradient synchronization overhead scales with the number of GPUs. In ring AllReduce, per-node communication volume is constant as the cluster grows—the bottleneck is the gradient volume per step (proportional to model size), not the number of participants.
...
At large scale (hundreds of billions of parameters across thousands of GPUs), gradient synchronization can dominate training step time, consuming 30--70 percent of wall-clock time unless aggressive optimization techniques are applied. The remainder of this chapter develops those techniques systematically.

### The physics of data movement {#sec-communication-collective-operations-physics-data-movement}

Before designing algorithms, we must understand the physical constraints governing data movement. Unlike software, which can be optimized almost indefinitely, communication is bound by the speed of light and by information-theoretic limits on reliable signaling over noisy channels.
...
# │
# │ Imports: mlsysim.book (check), mlsysim.core.units (MS_PER_SEC)
# │ Exports: gradient_size_gb_str, ring_bw_time_ms_str, ring_lat_time_ms_str,
# │          ring_total_ms_str, n_gpus_napkin_str, ring_bw_factor_value_str
# └─────────────────────────────────────────────────────────────────────────────
...
class AllReduceCost:
    ...
    gradient_qty = m.parameters * (2 * byte / param)

    # Use library formula for Ring AllReduce time
    # Note: calc_ring_allreduce_time handles both bandwidth and latency (alpha)
    t_ring = calc_ring_allreduce_time(message_bytes=gradient_qty, n_gpus=n_gpus, bandwidth_bytes_s=ib_ndr.bandwidth, latency_s=alpha)
    ring_total = t_ring.to(ms)

    # Break out terms for pedagogical display
    ring_bw_factor = 2 * (n_gpus - 1) / n_gpus
    ring_bw_time = (ring_bw_factor * gradient_qty / ib_bw.bandwidth).to(ms)
    ring_lat_time = (ring_total - ring_bw_time).to(ms)
    ...
    ring_bw_factor_value_str = fmt_ratio(ring_bw_factor, precision=2, commas=False)
...
from mlsysim.core.units import byte, ms, param
...
for n in gpu_counts_num:
    ...
    t_comp = p.latency_compute.to(ms).magnitude

    # Simple Ring AllReduce estimate for the scale
    from mlsysim.physics import calc_ring_allreduce_time

    grad_size = m.parameters * (2 * byte / param)
    ...
    t_comm = calc_ring_allreduce_time(grad_size, n, bw, 0 * ms).to(ms).magnitude
...
bars_compute = ax.barh(y_pos, compute_pct, height=bar_height, color=COLORS['GreenLine'], label='Compute', edgecolor='white', linewidth=0.5)
bars_comm = ax.barh(
    y_pos, comm_pct, height=bar_height, left=compute_pct, color=COLORS['OrangeLine'], label='Communication', edgecolor='white', linewidth=0.5
)
bars_sync = ax.barh(
    y_pos,
    sync_pct,
...
ax.annotate(
    'InfiniBand-limited\n(50 GB/s per port)', xy=(88, 2.5), xytext=(88, 2.0), fontsize=7, ha='center', color=COLORS['OrangeLine'], fontweight='bold'
)
...
2.  **Observation**: Unlike idealized throughput models that treat bandwidth as the sole communication cost, the α-β model reveals that $N$ small messages of size $n/N$ cost up to $N\times$ more than one large message of size $n$ when $n/N \ll n^*$, explaining why NCCL fuses small AllReduce calls and why MoE routing algorithms buffer tokens before launching collectives.
3.  **Pitfall**: A frequent misconception is that gradient compression always helps. If the compressed gradient size remains well above $n^*$, compression reduces the bandwidth term but leaves the latency term unchanged. Even shrinking a 70B model's gradient payload from 140 GB to 1.4 GB leaves the message firmly bandwidth-bound; it does not turn the operation into a low-latency exchange.
...
: **Interconnect Performance Parameters**: Typical latency and bandwidth values for data center interconnects. The critical size shows the crossover point where communication transitions from latency-bound to bandwidth-bound. {#tbl-interconnect-parameters}
...
from mlsysim.fmt import check, fmt_qty, fmt_time
...
- [ ] **Bound regime**: A message of 10 KB is being sent over a link where $\alpha=2\ \mu\text{s}$ and $\beta=10\ \text{GB/s}$. Is this message **Latency-Bound** or **Bandwidth-Bound**?
- [ ] **Software overhead**: If you reduce the "Software Overhead" ($\alpha$) by half, which type of parallelism benefits more: Data Parallelism or Tensor Parallelism?
- [ ] **Critical message size**: What happens to the critical message size ($n^* = \alpha \beta$) as you upgrade from 100 Gbps to 400 Gbps networking while keeping software latency constant?
- [ ] **NIC scaling**: True or False: In the bandwidth-bound regime, doubling the number of NICs per node effectively doubles the transmission speed for large tensors.
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: LogPOverlap.L_us_str, LogPOverlap.o_us_str,
# │          LogPOverlap.two_o_us_str, LogPOverlap.compute_us_str,
# │          LogPOverlap.effective_us_str
...
5.  **Exposed**: The `{python} LogPOverlap.two_o_us_str` of $o$ cannot overlap.
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.nvlink.bandwidth,
# │          Systems.Fabrics.InfiniBand_NDR.bandwidth, GB, second, ms)
...
# │          nvlink_bw_gb_per_s_str, ib_bw_gb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Models, Systems
from mlsysim.core.units import BILLION, GB, byte, microsecond, ms, param, second
from mlsysim.fmt import check, fmt, fmt_count, fmt_qty, fmt_time
...
class LlamaBudgetCalc:
    ...
    nvlink_bw = Hardware.Cloud.H100.nvlink.bandwidth  # H100 NVLink 4.0
    ...
    compute_per_step = 200 * ms  # scenario assumption: illustrative compute step

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # Hierarchical AllReduce—Step 1: Intra-node ReduceScatter
    intra_payload = gradient_bf16 * (gpus_per_node_llama - 1) / gpus_per_node_llama
    ...
    inter_ring_bw_time = (2 * (n_nodes_llama - 1) / n_nodes_llama * inter_payload / ib_bw).to(ms)
    inter_ring_lat_time = (2 * (n_nodes_llama - 1) * ib_alpha).to(ms)
    ...
    flat_ring_bw = (2 * (n_gpus_llama - 1) / n_gpus_llama * gradient_bf16 / ib_bw).to(ms)
    flat_ring_lat = (2 * (n_gpus_llama - 1) * ib_alpha).to(ms)
    flat_total = flat_ring_bw + flat_ring_lat

    ccr_hier = (hier_total_llama / compute_per_step).to_base_units().magnitude
    ccr_flat = (flat_total / compute_per_step).to_base_units().magnitude
    inter_ring_bw_time_display = round(inter_ring_bw_time.to(ms).magnitude, 2)
    flat_total_rounded = round(flat_total.to(ms).magnitude)
    ...
    intra_rs_time_ms_str = fmt_time(intra_rs_time, ms, precision=1, commas=False)
    inter_payload_gb_str = fmt_qty(inter_payload, GB, precision=1, commas=False)
    inter_ring_bw_time_ms_str = fmt_time(inter_ring_bw_time_display, ms, precision=1, commas=False)
    inter_ring_lat_time_ms_str = fmt_time(inter_ring_lat_time, ms, precision=2, commas=False)
    intra_ag_time_ms_str = fmt_time(intra_ag_time, ms, precision=1, commas=False)
    hier_total_llama_ms_str = fmt_time(hier_total_llama, ms, precision=1, commas=False)
    flat_total_ms_str = fmt_time(flat_total_rounded, ms, precision=0, commas=True)
    ...
    nvlink_bw_gb_per_s_str = fmt_qty(nvlink_bw, GB / second, precision=0, commas=False)
    ib_bw_gb_per_s_str = fmt_qty(ib_bw, GB / second, precision=0, commas=False)
```

Using BF16 gradients (a common practice that halves communication volume to `{python} LlamaBudgetCalc.gradient_bf16_gb_str`), the hierarchical AllReduce decomposes as follows:

1. **Intra-Node ReduceScatter** (NVLink at `{python} LlamaBudgetCalc.nvlink_bw_gb_per_s_str`): Each GPU exchanges 7/8 of the gradient locally. Time: `{python} LlamaBudgetCalc.intra_rs_time_ms_str`.
2. **Inter-Node Ring AllReduce** (InfiniBand NDR at `{python} LlamaBudgetCalc.ib_bw_gb_per_s_str`): Each GPU synchronizes only `{python} LlamaBudgetCalc.inter_payload_gb_str` across 16 nodes. Bandwidth time: `{python} LlamaBudgetCalc.inter_ring_bw_time_ms_str`. Latency time: `{python} LlamaBudgetCalc.inter_ring_lat_time_ms_str`.
...
2.  **Observation**: Unlike point-to-point communication (where one sender and one receiver exchange data independently), a collective operation coordinates the entire process group—every participant must invoke the collective before any can complete it, and the library guarantees a consistent result even when different processes contribute different data.
3.  **Pitfall**: A frequent misconception is that one collective algorithm suits all message sizes. Ring AllReduce achieves near-optimal bandwidth for large gradient tensors but has $\mathcal{O}(N)$ latency—for small messages like MoE routing decisions (~4 KB), a tree-based algorithm reduces latency to $\mathcal{O}(\log N)$ steps at the cost of suboptimal bandwidth utilization (the bandwidth-optimal recursive halving-doubling alternative is covered in @sec-communication-butterfly), requiring workload-specific algorithm selection rather than a single default.
...
[^fn-fsdp-memory]: **FSDP (Fully Sharded Data Parallel)**\index{FSDP!memory trade-off}: PyTorch's implementation of ZeRO-3 (Zero Redundancy Optimizer), which shards parameters, gradients, and optimizer states across $N$ accelerators, reducing per-GPU memory from $16P$ bytes under FP32 Adaptive Moment Estimation (Adam) to $16P/N$ bytes. The trade-off is communication frequency: FSDP issues $2N_{\text{layers}}$ collective operations per step (AllGather + ReduceScatter per layer) instead of data parallelism's single AllReduce, making it more sensitive to the $\alpha$ overhead.
...
class MoeAllToAll:
    ...
    bw_time = (data_per_transfer / beta).to(microsecond)
    bw_time_us_approx = round(bw_time.to(microsecond).magnitude)
    per_transfer = alpha + bw_time_us_approx * microsecond
    ...
    bw_time_approx_us_str = fmt_time(bw_time_us_approx, microsecond, precision=0, commas=False)
...
Each of the 7 transfers is a `{python} MoeAllToAll.data_per_transfer_kb_str` message. From the $\alpha$-$\beta$ model: $T_{\text{transfer}}$ = `{python} MoeAllToAll.alpha_us_str` + `{python} MoeAllToAll.bw_time_approx_us_str` = `{python} MoeAllToAll.per_transfer_us_str` per transfer.

If serialized: 7 $\times$ `{python} MoeAllToAll.per_transfer_us_str` = `{python} MoeAllToAll.serialized_us_str`. If all 7 transfers run in parallel (full-duplex, non-blocking): $\approx$ `{python} MoeAllToAll.parallel_us_str`.
...
As $N$ grows large, this approaches $2M/\beta$, which is independent of $N$. An algorithm that achieves this bound is called **bandwidth-optimal**\index{Bandwidth-Optimal}. Ring AllReduce achieves this bound exactly. Tree AllReduce does not. This distinction has practical consequences: on a 64-GPU cluster synchronizing a 1 GB gradient, the bandwidth difference between an optimal and a merely logarithmic algorithm amounts to several milliseconds per step, which accumulates to hours over a multi-day training run.
...
- [ ] **Step count**: In a ring of $N$ GPUs, how many **sequential steps**\index{Ring AllReduce!sequential steps} are required to complete the full AllReduce?
- [ ] **Bandwidth optimality**: Why is Ring AllReduce considered bandwidth-optimal? How many times does each byte of the total message $M$ traverse the network per GPU?
- [ ] **Straggler impact**: If one GPU in the ring is 50 percent slower than the others, what is the impact on the total completion time of the collective?
- [ ] **Send/receive duality**: True or False: In Ring AllReduce, every GPU is both a sender and a receiver in every step of the algorithm.
...
class RingVsTree:
    ...
    ring_lat = (2 * (N - 1) * alpha).to(microsecond)
    tree_lat = (2 * log2N * alpha).to(microsecond)
    ring_bw = (2 * (N - 1) / N * M / beta).to(microsecond)
    tree_bw = (2 * log2N * M / beta).to(microsecond)
    ring_total = ring_lat + ring_bw
    tree_total = tree_lat + tree_bw
    crossover = (N * alpha * beta / log2N).to(MB)
...
1.  **Ring Latency**\index{Ring AllReduce!latency}: $2(N-1)\alpha = 2 \times 63 \times 10\ \mu\text{s}$ = `{python} RingVsTree.ring_lat_us_str`.
2.  **Tree Latency**\index{Tree AllReduce!latency}: $2(\log_2 N)\alpha = 2 \times 6 \times 10\ \mu\text{s}$ = `{python} RingVsTree.tree_lat_us_str`.

**Bandwidth** (note the difference):

3.  **Ring Bandwidth**\index{Ring AllReduce!bandwidth}: $2\frac{N-1}{N}\frac{M}{\beta} \approx 2 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = `{python} RingVsTree.ring_bw_us_str` (optimal: each byte sent once).
4.  **Tree Bandwidth**\index{Tree AllReduce!bandwidth}: $2\log_2 N \cdot \frac{M}{\beta} = 12 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = `{python} RingVsTree.tree_bw_us_str` (each level sends full message).
...
- [ ] **Ring vs. Tree**: Can you explain why Ring AllReduce achieves bandwidth-optimal communication (approaching $2M/\beta$) while Tree AllReduce incurs $\mathcal{O}(\log N)$ bandwidth overhead?
- [ ] **Crossover point**: Given a 256-GPU cluster with $\alpha = 5\ \mu\text{s}$ and $\beta = 50\ \text{GB/s}$, can you calculate $M_{\text{crossover}}$ and determine whether a 10 MB gradient buffer should use Ring or Tree?
- [ ] **Scatter-reduce trace**: Can you trace through the Scatter-Reduce phase of Ring AllReduce for 3 GPUs with a 3-element vector and verify that each GPU ends with the correct partial sum?
- [ ] **Latency limit**: Can you explain why Ring AllReduce's $\mathcal{O}(N)$ latency makes it impractical for tensor parallelism (where message sizes are small and latency dominates)?

:::

[^fn-moe-sparsity]: **Mixture of Experts (MoE)**\index{Mixture of Experts!communication}: An architecture where each token activates only a subset of specialized subnetworks (experts), reducing per-token FLOPs while maintaining total model capacity. The systems trade-off is stark: MoE replaces the bandwidth-bound AllReduce of dense models with latency-bound AllToAll, shifting the communication bottleneck from $\beta$ to $\alpha$ and creating $\mathcal{O}(N^2)$ contention that limits practical cluster size.
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.nvlink.bandwidth, Systems.Fabrics.InfiniBand_NDR.bandwidth, GB, second)
# │ Exports: nvlink_h100_bw_gb_per_s_str, ib_ndr_bw_gb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
class CommInfraScenario:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    bandwidth_ratio = (nvlink_raw / ib_ndr_raw).to_base_units().magnitude

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check, fmt, fmt_qty
...
# │ Show: speedup_mult_str ≈ 6.3×—inline in callout insight sentence.
# │ How: Flat: T = 2M/β_IB; Hierarchical: T = T_rs + T_inter + T_ag with
# │      NVLink for intra-node phases and IB for inter-node phase.
# │
# │ Imports: mlsysim.core.units (MB, GB, second, ms, microsecond, Hardware.Cloud.H100.nvlink.bandwidth,
...
# │          hier_total_ms_str, speedup_mult_str, ib_bw_gb_per_s_str, nvlink_bw_gb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.units import GB, MB, microsecond, ms, second
from mlsysim import Hardware, Systems
from mlsysim.fmt import check, fmt, fmt_count, fmt_multiple, fmt_qty, fmt_time
...
class HierarchicalAllreduceCalc:
    ...
    nvlink_bw = Hardware.Cloud.H100.nvlink.bandwidth  # NVLink 4.0 (H100)
    ib_alpha = 3 * microsecond  # scenario assumption: alpha-beta base latency
    ...
    intra_data = (gradient * (gpus_per_node - 1) / gpus_per_node).to(MB)  # ~875 MB
    intra_rs = (intra_data / nvlink_bw).to(ms)

    inter_data = (gradient / gpus_per_node).to(MB)  # 125 MB
    inter_ring_factor = 2 * (n_nodes_har - 1) / n_nodes_har
    inter_bw = (inter_ring_factor * inter_data / ib_bw).to(ms)
    inter_lat = (2 * (n_nodes_har - 1) * ib_alpha).to(ms)
    ...
    intra_rs_rounded = round(intra_rs.to(ms).magnitude)
    intra_ag_rounded = round(intra_ag.to(ms).magnitude)
    ...
    intra_rs_ms_str = fmt_time(intra_rs_rounded, ms, precision=0, commas=False)
    inter_data_mb_str = fmt_qty(inter_data, MB, precision=0, commas=False)
    inter_time_ms_str = fmt_time(inter_time, ms, precision=1, commas=False)
    intra_ag_ms_str = fmt_time(intra_ag_rounded, ms, precision=0, commas=False)
...
1. **Intra-node ReduceScatter**: Each GPU sends `{python} HierarchicalAllreduceCalc.intra_data_mb_str` at `{python} HierarchicalAllreduceCalc.nvlink_bw_gb_per_s_str` → ~`{python} HierarchicalAllreduceCalc.intra_rs_ms_str`
2. **Inter-Node AllReduce**\index{AllReduce!inter-node}: Each GPU AllReduces a $1\ \text{GB}/8$ = `{python} HierarchicalAllreduceCalc.inter_data_mb_str` shard over InfiniBand; Ring moves roughly $2(N-1)/N$ times that payload, giving ~`{python} HierarchicalAllreduceCalc.inter_time_ms_str` at `{python} HierarchicalAllreduceCalc.ib_bw_gb_per_s_str`

   (Only 1/8 of the data crosses InfiniBand!)

3. **Intra-node AllGather**: Each GPU receives `{python} HierarchicalAllreduceCalc.intra_data_mb_str` at `{python} HierarchicalAllreduceCalc.nvlink_bw_gb_per_s_str` → ~`{python} HierarchicalAllreduceCalc.intra_ag_ms_str`
...
Communication libraries like NCCL perform **topology detection**\index{Topology Detection} at initialization, running graph search algorithms to discover the physical network structure and find high-bandwidth communication paths. This is critical because the same collective algorithm can differ by 2$\times$ or more in throughput depending on how logical ranks map to physical hardware.
...
**Scenario**: A collective communication library must determine whether another GPU sits on a local NVLink switch or 100 meters away across an InfiniBand fabric.

**Mechanism**: NVIDIA's Collective Communications Library (NCCL) performs a topology-detection phase at initialization to map these physical realities.

**Systems lesson**: Collective performance depends on matching the communication algorithm to the physical topology. Without topology discovery, a correct AllReduce can still route traffic through the wrong links and lose most of the available bandwidth.
...
2.  **Observation**: Unlike lossy compression (which permanently discards sub-threshold gradient components), error feedback is a delayed transmission strategy: the residual $e_t$ telescopes across steps so that $\frac{1}{K}\sum_{t=1}^{K} v_t \to \frac{1}{K}\sum_{t=1}^{K} g_t$ as $K \to \infty$, making the long-run average of transmitted gradients equal to the true gradient average.
3.  **Pitfall**: A frequent misconception is that error feedback is built into standard gradient compressors. PyTorch's built-in DDP gradient compression does not implement error feedback by default; without it, Top-k and 1-bit quantization introduce systematic bias that accumulates across steps, typically degrading final model accuracy by 1–3 percent on benchmark tasks compared to the feedback-corrected version.
...
class ErrorFeedbackTrace:
    ...
    e0 = 0.0
    v1_in = gradients[0] + e0
    v1 = round(v1_in)
    e1 = v1_in - v1
    v2_in = gradients[1] + e1
    v2 = round(v2_in)
    e2 = v2_in - v2
    v3_in = gradients[2] + e2
    v3 = round(v3_in)
    e3 = v3_in - v3
    v4_in = gradients[3] + e3
    v4 = round(v4_in)
    e4 = v4_in - v4
    v5_in = gradients[4] + e4
    v5 = round(v5_in)
    e5 = v5_in - v5
...
# │ Exports: cp_comm_time_ms_str, cp_ratio_str, cp_overhead_ms_str,
# │          cp_compressed_comm_ms_str, cp_total_new_ms_str, cp_speedup_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
class CompressionPayback:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cp_comm_time_ms_str = fmt_time(t_comm_ms, 'millisecond', precision=0, commas=False)
    ...
    cp_overhead_ms_str = fmt_time(t_overhead_ms, 'millisecond', precision=0, commas=False)
    cp_compressed_comm_ms_str = fmt_time(t_compressed_comm, 'millisecond', precision=0, commas=False)
    cp_total_new_ms_str = fmt_time(t_total_new, 'millisecond', precision=0, commas=False)
...
**Problem**: A training job synchronizes a `{python} CompressionPayback.cp_comm_time_ms_str` gradient buffer. The system implements 1-bit Adam, which reduces communication volume by 32$\times$ but adds `{python} CompressionPayback.cp_overhead_ms_str` of CPU/GPU overhead for the compression logic. If the effective network throughput improves by `{python} CompressionPayback.cp_throughput_factor_mult_str`, what is the actual communication speedup?
...
- [ ] **Error feedback**: Can you explain why 1-bit SGD without error feedback causes divergence, while 1-bit Adam with warmup converges? What is fundamentally different about how each method handles information loss?
- [ ] **Compression payoff**: Given a training run where the backward pass takes 800 ms and AllReduce takes 200 ms, would applying 4$\times$ gradient compression improve total step time? What if AllReduce takes 2000 ms?
- [ ] **Error accumulator**: Can you derive the steady-state behavior of the error accumulator $e_t$ when the true gradient is constant at $g = 0.3$ and the compression threshold is 0.5?
- [ ] **Top-k vs. quantization**: When would you choose Top-k sparsification over quantization? Consider the trade-off between compression ratio and the overhead of encoding sparse indices.
...
# │
# │ Imports: mlsysim.core.units (MB, ms)
...
$T_{\text{pipelined}} = T_{\text{backward}} + T_{\text{first layer comm}} + (N_{\text{layers}} - 1) \times T_{\text{exposed}}$ = `{python} OverlapBudgetCalc.with_overlap_ms_str`.
...
# │ Exports: CommInfraRecap.nvlink_h100_bw_gb_per_s_str, CommInfraRecap.ib_ndr_bw_gb_per_s_str
from mlsysim.core.units import GB, second
...
The communication traffic patterns differ fundamentally across the Lighthouse Archetypes. The compression and error-feedback mechanisms developed in this section also matter outside data centers: when devices are intermittent or bandwidth-poor, the dominant cost may be the ability to send any useful update at all rather than saturating a high-speed fabric.
