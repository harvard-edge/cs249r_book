...
class RingVsTree:
    ...
    crossover_mb = (N * alpha_us * 1e-6 * beta_gbs * 1e9 / 1e6) / log2N
    # ┌── 3. GUARD ─────────────────────────────────────────
...
2. **Inter-Node AllReduce**\index{AllReduce!inter-node}: Each GPU AllReduces a $1\ \text{GB}/8$ = `{python} HierarchicalAllreduceCalc.inter_data_mb_str` MB shard over InfiniBand; Ring moves roughly $2(N-1)/N$ times that payload, giving ~`{python} HierarchicalAllreduceCalc.inter_time_ms_str` ms at 50 GB/s
...
**Level 2 (Intra-Rack, InfiniBand)**: Ring AllReduce among 4 nodes on the same rack. Each GPU AllReduces a `{python} ThreeLevelNapkin.step2_payload_gb_str` GB rack-level payload; Ring traffic is roughly $2(N-1)/N$ times that payload at 50 GB/s. Time: `{python} ThreeLevelNapkin.step2_ring_bw_ms_str` ms.

**Level 3 (Cross-Rack, Spine)**: Ring AllReduce among 4 racks. Each GPU AllReduces only a `{python} ThreeLevelNapkin.step3_payload_gb_str` GB cross-rack payload; Ring traffic again applies the $2(N-1)/N$ multiplier at 25 GB/s. Time: `{python} ThreeLevelNapkin.step3_ring_bw_ms_str` ms.
