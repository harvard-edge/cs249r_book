...
class AllReduceCost:
    ...
    ib_bw_gbs_str = fmt_qty(ib_bw.bandwidth, GB/second, precision=0, commas=False)
...
**Problem**: A `{python} AllReduceCost.params_b_str` parameter model trains with data parallelism across `{python} AllReduceCost.n_gpus_napkin_str` connected by InfiniBand NDR (`{python} AllReduceCost.ib_bw_gbs_str` per port). Each GPU computes gradients in BF16 (2 bytes per parameter, standard for Llama-class training). How long does one AllReduce take?
...
Substituting: $T_{\text{bandwidth}}$ = `{python} AllReduceCost.ring_bw_factor_value_str` $\times$ `{python} AllReduceCost.gradient_size_gb_str` / `{python} AllReduceCost.ib_bw_gbs_str` $\approx$ `{python} AllReduceCost.ring_bw_time_ms_str`.
...
class CriticalMsgSize:
    ...
    beta_gb_s_str = fmt_qty(beta, GB/second, precision=0, commas=False)
...
**Problem**: Consider a cluster using InfiniBand NDR 400 Gbps with $\alpha =$ `{python} CriticalMsgSize.alpha_us_str` and $\beta =$ `{python} CriticalMsgSize.beta_gb_s_str`. At what message size does optimizing for bandwidth start to matter more than optimizing for latency?
...
# │          nvlink_bw_gb_s_str, ib_bw_gb_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
class LlamaBudgetCalc:
    ...
    nvlink_bw_gb_s_str              = fmt_qty(nvlink_bw, GB/second, precision=0, commas=False)
    ib_bw_gb_s_str                  = fmt_qty(ib_bw, GB/second, precision=0, commas=False)
```
...
1. **Intra-Node ReduceScatter** (NVLink at `{python} LlamaBudgetCalc.nvlink_bw_gb_s_str`): Each GPU exchanges 7/8 of the gradient locally. Time: `{python} LlamaBudgetCalc.intra_rs_time_ms_str`.
2. **Inter-Node Ring AllReduce** (InfiniBand NDR at `{python} LlamaBudgetCalc.ib_bw_gb_s_str`): Each GPU synchronizes only `{python} LlamaBudgetCalc.inter_payload_gb_str` across 16 nodes. Bandwidth time: `{python} LlamaBudgetCalc.inter_ring_bw_time_ms_str`. Latency time: `{python} LlamaBudgetCalc.inter_ring_lat_time_ms_str`.
...
**Latency Analysis** (InfiniBand NDR, $\alpha =$ `{python} MoeAllToAll.alpha_us_str`, $\beta =$ `{python} MoeAllToAll.beta_gb_s_str`):
...
**Problem**: A cluster synchronizes a `{python} RingVsTree.M_mb_str` buffer across `{python} RingVsTree.N_str` GPUs. The network has latency $\alpha =$ `{python} RingVsTree.alpha_us_str` and bandwidth $\beta =$ `{python} RingVsTree.beta_gb_s_str`. Which algorithm performs better: Ring or Tree?
...
class CommInfraScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_bw_gbs_str = fmt_qty(nvlink_raw, GB/second, precision=0, commas=False)
    ib_ndr_bw_gbs_str = fmt_qty(ib_ndr_raw, GB/second, precision=0, commas=False)
```
...
# │          hier_total_ms_str, speedup_mult_str, ib_bw_gbs_str, nvlink_bw_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
...
class HierarchicalAllreduceCalc:
    ...
    ib_bw_gbs_str     = fmt_qty(ib_bw, GB/second, precision=0, commas=False)
    nvlink_bw_gbs_str = fmt_qty(nvlink_bw, GB/second, precision=0, commas=False)
...
from mlsysim.core.constants import MB, ms
...
class OverlapBudgetCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check, fmt, fmt_count, fmt_percent, fmt_time, fmt_qty
...
class CommInfraRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvlink_h100_bw_gbs_str = fmt_qty(nvlink_bw, GB/second, precision=0, commas=False)
    ib_ndr_bw_gbs_str = fmt_qty(ib_bw, GB/second, precision=0, commas=False)
```
