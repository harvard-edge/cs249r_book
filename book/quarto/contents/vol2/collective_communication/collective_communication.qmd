...
Before designing algorithms, we must understand the physical constraints governing data movement. @Sec-network-fabrics-wire-link establishes the wire-level physics this chapter builds on: the speed of light sets a latency floor[^fn-speed-of-light-latency] (roughly 5 $\mu\text{s}$ per kilometer in fiber), the bandwidth-distance product limits how far a fast link can reach before it needs optics (PAM4 signaling and copper-vs-optics reach are developed in @sec-network-fabrics-pam4), and kernel-bypass transports such as RDMA and GPUDirect RDMA strip the per-message software tax down to a few microseconds (@sec-network-fabrics-rdma). The contribution this chapter adds on top of that wire-level model is the energy cost of moving a bit, which the collective algorithms must respect as firmly as latency and bandwidth.
...
# │ Exports: gradient_bf16_gb_str, hier_total_llama_ms_str, flat_total_ms_str
# └─────────────────────────────────────────────────────────────────────────────
...
The recursive halving-doubling pattern is also directly relevant to AllToAll operations in ML workloads. Sequence parallelism reconstructs activation shards through an AllGather before a local operation and redistributes the result via a ReduceScatter along the sequence dimension, and the inter-rank exchange schedule during those operations follows the same distance-doubling logic. Mixture of Experts routing, where each token is dispatched to a remote expert and the result is gathered back, similarly uses a fan-out pattern in which every GPU must exchange data with every other GPU. In both cases, the doubling distance schedule is exactly what creates the cross-boundary pairings that make topology misalignment costly: token dispatch in MoE must reach any of $N$ expert GPUs, and a butterfly-style schedule produces those long-distance crossings in the final round.
...
To attach ML intuition to these buffer sizes: a 1 MB message roughly corresponds to the gradients from a single feed-forward layer in a smaller transformer model, or to an MoE token-routing payload. A 10 MB message is a typical DDP gradient bucket from a single mid-size transformer layer. A 50 MB to several-hundred-MB buffer represents a fused DDP bucket covering multiple layers, while 140 GB is the full gradient tensor of a 70B-parameter model in BF16. This range matters because most real workloads operate somewhere between these points, and the crossover analysis above sets the algorithmic boundary that separates Tree-favored bucket-level operations from Ring-favored full-model synchronizations.
...
SHARP becomes especially relevant during strong scaling, the regime where a fixed model is trained across more and more accelerators without proportionally increasing batch size. As the per-accelerator batch shrinks to fill memory constraints, each gradient tensor stays large (it is still proportional to model size) but the backward pass per accelerator shortens, so communication dominates an increasing fraction of step time and the latency term $\alpha$ in the ring formula hits the bottleneck. In-switch aggregation cuts directly into that latency cost. Modern InfiniBand switch ASICs have also been explicitly updated to support ML-native datatypes, including FP16 and BF16, which confirms that SHARP is co-evolving with ML workload requirements rather than serving only the legacy HPC floating-point operations it was originally designed for. An ML system team choosing SHARP infrastructure for a scaling campaign should verify FP16/BF16 aggregation support in the specific switch generation, since earlier ASICs supported only FP32 and INT32.
