...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_count, fmt_multiple, fmt_params, fmt_qty, fmt_time
...
class AllReduceCost:
    ...
    ring_bw_factor_str = fmt_multiple(ring_bw_factor, precision=2, commas=False)
...
class CompressionPayback:
    ...
    cp_throughput_factor_str = fmt_multiple(ratio, precision=0, commas=False)
    ...
    cp_speedup_str = fmt_multiple(speedup, precision=1, commas=False)
```

::: {#nbk-collective-communication-payback-compression .callout-notebook title="The payback of compression"}

**Problem**: A training job synchronizes a `{python} CompressionPayback.cp_comm_time_ms_str` gradient buffer. The system implements 1-bit Adam, which reduces communication volume by 32$\times$ but adds `{python} CompressionPayback.cp_overhead_ms_str` of CPU/GPU overhead for the compression logic. If the effective network throughput improves by `{python} CompressionPayback.cp_throughput_factor_str`$\times$, what is the actual communication speedup?
