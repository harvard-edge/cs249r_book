...
1.  **Overlappable portion**\index{Overlappable Portion}: Network latency $L$ = `{python} LogPOverlap.L_str` $\mu\text{s}$ (data in flight while GPU computes).
...
4.  **Hidden**\index{Hidden}: All `{python} LogPOverlap.L_str` μs of $L$ can overlap with compute.
