...
The critical message size separates two distinct operating regimes. Below it, small messages such as MoE routing tokens or scalar reductions are latency-bound $(n < n^*)$: time is dominated by $\alpha$, so optimization focuses on fusion (batching small messages), topology (reducing hop count), and software-stack tuning (kernel bypass via RDMA). Above it, large messages such as LLM gradients or optimizer states are bandwidth-bound $(n \gg n^*)$: time is dominated by $n/\beta$, so optimization focuses on compression\index{Gradient Compression} (lower precision or sparsity), algorithm choice (Ring vs. Tree), and link aggregation\index{Link Aggregation} (multi-rail NICs). The distinction between latency-bound and bandwidth-bound communication is the diagnostic skill to practice.
...
The error-feedback trace in @tbl-error-feedback runs the same sequence with a residual accumulator $e_t$ that re-injects the discarded remainder into the next step. The notebook below works through both traces to show that the deferred information is conserved rather than lost.
...
NVIDIA Collective Communications Library (NCCL)[^fn-nccl-dominance] is central to many NVIDIA multi-GPU training deployments because it turns the collective algorithms above into GPU-resident data movement [@jeaugey2017nccl; @nvidiaNcclUserGuide]. Its adoption stems from three GPU-specific optimizations that MPI and Gloo cannot replicate without hardware vendor support. The first is kernel fusion: NCCL folds the reduction operator (sum, average) directly into the memory-copy kernel, so instead of copying data to a buffer, reducing, then copying results back, it performs the reduction during the transfer, eliminating intermediate memory traffic and maximizing HBM bandwidth utilization. The second is channel pipelining\index{Channel Pipelining}: NCCL opens multiple parallel communication channels to saturate all available network interfaces at once, so a DGX node with 8 NICs reaches 8$\times$ the bandwidth of a single-channel implementation by spreading the collective across every link. The third is GPUDirect RDMA: the network card reads directly from GPU memory over PCIe, bypassing the CPU entirely, where without it the data would traverse GPU → CPU memory → NIC → network, adding microseconds of latency and consuming CPU cycles. Together these explain why NCCL is usually the first backend to benchmark for NVIDIA GPU collectives before falling back to generic MPI implementations.
