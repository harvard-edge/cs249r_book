...
# │
# │ Imports: mlsysim (Models, Systems) + core.units + fmt
# │ Exports: GradSyncSignificance.* (_str / _math per displayed value)
# └─────────────────────────────────────────────────────────────────────────────
...
class LlamaBudgetCalc:
    ...
    flat_total_ms_str = fmt_int(flat_total, ms, commas=True, suffix=" ms")
```
...
# │
# │ Imports: mlsysim (Systems) + core.units + fmt
# │ Exports: NcclRealityGap.* (_str per table cell)
# └─────────────────────────────────────────────────────────────────────────────
...
class NcclRealityGap:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    predicted = [(alpha + n / beta) for n in sizes]
    ratios = [(m / p).to_base_units().magnitude for m, p in zip(measured, predicted)]
    ...
    # Integer-displayed cells use fmt_int (intentional rounding); fractional
    # cells use fmt_time at precision 1. Ratios carry a "~" prose prefix.
    pred_1kb_str = fmt_time(predicted[0], microsecond, precision=1, commas=False, approx=True)
    pred_64kb_str = fmt_time(predicted[1], microsecond, precision=1, commas=False, approx=True)
    pred_1mb_str = fmt_int(predicted[2], microsecond, commas=False, approx=True, suffix=" μs")
    pred_64mb_str = fmt_time(predicted[3], ms, precision=1, commas=False, approx=True)
    pred_1gb_str = fmt_int(predicted[4], ms, commas=False, approx=True, suffix=" ms")
    pred_10gb_str = fmt_int(predicted[5], ms, commas=False, approx=True, suffix=" ms")
    meas_1kb_str = fmt_int(measured[0], microsecond, commas=False, approx=True, suffix=" μs")
    meas_64kb_str = fmt_int(measured[1], microsecond, commas=False, approx=True, suffix=" μs")
    meas_1mb_str = fmt_int(measured[2], microsecond, commas=False, approx=True, suffix=" μs")
    meas_64mb_str = fmt_time(measured[3], ms, precision=1, commas=False, approx=True)
    meas_1gb_str = fmt_int(measured[4], ms, commas=False, approx=True, suffix=" ms")
    meas_10gb_str = fmt_int(measured[5], ms, commas=False, approx=True, suffix=" ms")
    ratio_1kb_str = fmt_multiple(ratios[0], precision=1)
    ratio_64kb_str = fmt_multiple(ratios[1], precision=1)
    ratio_1mb_str = fmt_multiple(ratios[2], precision=1)
    ratio_64mb_str = fmt_multiple(ratios[3], precision=1)
    ratio_1gb_str = fmt_multiple(ratios[4], precision=2)
    ratio_10gb_str = fmt_multiple(ratios[5], precision=2)
```

| **Message Size** |     **$\alpha$-$\beta$ Prediction**     |            **Measured NCCL**            |       **Ratio (Measured/Predicted)**      | **Explanation**                         |
|:-----------------|:---------------------------------------:|:---------------------------------------:|:-----------------------------------------:|:----------------------------------------|
| 1 KB             |  `{python} NcclRealityGap.pred_1kb_str` |  `{python} NcclRealityGap.meas_1kb_str` |  ~`{python} NcclRealityGap.ratio_1kb_str` | NCCL protocol setup dominates           |
| 64 KB            | `{python} NcclRealityGap.pred_64kb_str` | `{python} NcclRealityGap.meas_64kb_str` | ~`{python} NcclRealityGap.ratio_64kb_str` | Still latency-bound; NCCL overhead      |
| 1 MB             |  `{python} NcclRealityGap.pred_1mb_str` |  `{python} NcclRealityGap.meas_1mb_str` |  ~`{python} NcclRealityGap.ratio_1mb_str` | Transitioning to bandwidth-bound        |
| 64 MB            | `{python} NcclRealityGap.pred_64mb_str` | `{python} NcclRealityGap.meas_64mb_str` | ~`{python} NcclRealityGap.ratio_64mb_str` | NCCL approaches theoretical bandwidth   |
| 1 GB             |  `{python} NcclRealityGap.pred_1gb_str` |  `{python} NcclRealityGap.meas_1gb_str` |  ~`{python} NcclRealityGap.ratio_1gb_str` | Bandwidth-dominant; NCCL nearly optimal |
| 10 GB            | `{python} NcclRealityGap.pred_10gb_str` | `{python} NcclRealityGap.meas_10gb_str` | ~`{python} NcclRealityGap.ratio_10gb_str` | Large payloads saturate the wire        |
...
class CommInfraScenario:
    ...
    bandwidth_ratio_str = fmt_int(bandwidth_ratio, commas=False)
```
...
class HierarchicalAllreduceCalc:
    ...
    intra_rs_ms_str = fmt_int(intra_rs, ms, commas=False, suffix=" ms")
    ...
    intra_ag_ms_str = fmt_int(intra_ag, ms, commas=False, suffix=" ms")
...
class ErrorFeedbackTrace:
    ...
    # e_t entering step t (error-feedback table)
    e1_str, e2_str, e3_str, e4_str, e5_str = [_ef_dec(errors[t]) for t in range(5)]
    ...
    # e_{t+1} residual (error-feedback table)
    en1_str, en2_str, en3_str, en4_str, en5_str = [_ef_dec(errors[t + 1]) for t in range(5)]
