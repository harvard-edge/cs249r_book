...
class AllReduceCost:
    ...
    gradient_size_gb = gradient_qty.to(GB).magnitude
    ...
    ring_total_ms = t_ring.to(ms).magnitude

    # Break out terms for pedagogical display
    ring_bw_factor = 2 * (n_gpus - 1) / n_gpus
    ring_bw_time_ms = (ring_bw_factor * gradient_size_gb / ib_bw.bandwidth.to(GB/second).magnitude) * THOUSAND
    ...
    ring_bw_time_ms_str = fmt_time(ring_bw_time_ms, 'millisecond', precision=1, commas=True)
    ring_lat_time_ms_str = fmt_time(ring_lat_time_ms, 'millisecond', precision=1, commas=False)
    ring_total_ms_str = fmt_time(ring_total_ms, 'millisecond', precision=1, commas=True)
...
for n in gpu_counts_num:
    ...
    from mlsysim.physics import calc_ring_allreduce_time
    grad_size = m.parameters.to(param).magnitude * 2 * ureg.byte
    ...
    t_comm = calc_ring_allreduce_time(grad_size, n, bw, 0*ureg.ms).to(ms).magnitude
...
from mlsysim.fmt import check, fmt, fmt_qty
from mlsysim.core.constants import byte, GB, KB, second
...
class CriticalMsgSize:
    ...
    n_star_kb = round((n_star_bytes * byte).to(KB).magnitude)
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(n_star_kb == 100, f"n* = {n_star_kb} KB, expected 100")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    alpha_str = fmt(alpha_us, precision=0, commas=False)
    beta_str = fmt_qty(beta_gbs * GB/second, GB/second, precision=0, commas=False)
    n_star_kb_str = fmt_qty(n_star_bytes * byte, KB, precision=0, commas=False)
```
...
class LlamaBudgetCalc:
    ...
    nvlink_bw   = Hardware.Cloud.H100.nvlink.bandwidth.to(GB/second).magnitude         # GB/s (H100 NVLink 4.0)
    nvlink_alpha = 1.5                                    # us
    ib_bw        = Systems.Fabrics.InfiniBand_NDR.bandwidth.to(GB/second).magnitude        # GB/s per port (NDR 400 Gbps)
    ib_alpha     = 3                                      # us
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gradient_bf16_gb_str       = fmt_qty(gradient_bf16_gb * GB, GB, precision=0, commas=False)
    intra_rs_time_ms_str       = fmt_time(intra_rs_time_ms, 'millisecond', precision=1, commas=False)
    inter_payload_gb_str       = fmt_qty(inter_payload_gb * GB, GB, precision=1, commas=False)
    inter_ring_bw_time_ms_str  = fmt_time(inter_ring_bw_time_ms, 'millisecond', precision=1, commas=False)
    inter_ring_lat_time_ms_str = fmt_time(inter_ring_lat_time_ms, 'millisecond', precision=2, commas=False)
    intra_ag_time_ms_str       = fmt_time(intra_ag_time_ms, 'millisecond', precision=1, commas=False)
    hier_total_llama_ms_str    = fmt_time(hier_total_llama_ms, 'millisecond', precision=1, commas=False)
    flat_total_ms_str          = fmt_time(round(flat_total_ms), 'millisecond', precision=0, commas=True)
    ...
    nvlink_bw_str              = fmt_qty(nvlink_bw * GB/second, GB/second, precision=0, commas=False)
    ib_bw_str                  = fmt_qty(ib_bw * GB/second, GB/second, precision=0, commas=False)
```
...
from mlsysim.core.constants import GB, KB, MB, second
from mlsysim.fmt import check, fmt, fmt_qty
...
class MoeAllToAll:
    ...
    kb_per_token_str = fmt_qty(kb_per_token * KB, KB, precision=3, commas=False)
    tokens_per_expert_str = fmt(tokens_per_expert, precision=0, commas=False)
    data_per_transfer_kb_str = fmt_qty(data_per_transfer_kb * KB, KB, precision=3, commas=False)
    total_sent_mb_str = fmt_qty(total_sent_mb * MB, MB, precision=2, commas=False)
...
class RingVsTree:
    ...
    crossover_mb = (N * (alpha_us * US) * (beta_gbs * GB / second) / log2N).to(MB).magnitude
    ...
    crossover_str = fmt_qty(crossover_mb * MB, MB, precision=1, commas=False)
    M_mb_str = fmt_qty(M_mb * MB, MB, precision=0, commas=False)
...
class CommInfraScenario:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    nvlink_gbs = nvlink_raw.to(GB/second).magnitude
    ib_gbs = ib_ndr_raw.to(GB/second).magnitude
...
class HierarchicalAllreduceCalc:
    ...
    ib_bw_gbs      = Systems.Fabrics.InfiniBand_NDR.bandwidth.to(GB/second).magnitude        # InfiniBand NDR 400 Gbps per port
    nvlink_bw_gbs  = Hardware.Cloud.H100.nvlink.bandwidth.to(GB/second).magnitude            # NVLink 4.0 (H100)
    ib_alpha_us    = 3
    ...
    intra_rs_ms   = ((intra_data_mb * MB) / (nvlink_bw_gbs * GB / second)).to(ms).magnitude

    inter_data_mb = int(gradient_gb * THOUSAND / gpus_per_node)  # 125 MB
    inter_ring_factor = 2 * (n_nodes_har - 1) / n_nodes_har
    inter_bw_ms = ((inter_ring_factor * inter_data_mb * MB) / (ib_bw_gbs * GB / second)).to(ms).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    flat_time_ms_str  = fmt_time(flat_time_ms, 'millisecond', precision=0, commas=False)
    intra_data_mb_str = fmt_qty(intra_data_mb * MB, MB, precision=0, commas=False)
    intra_rs_ms_str   = fmt_time(round(intra_rs_ms), 'millisecond', precision=0, commas=False)
    inter_data_mb_str = fmt_qty(inter_data_mb * MB, MB, precision=0, commas=False)
    inter_time_ms_str = fmt_time(inter_time_ms, 'millisecond', precision=1, commas=False)
    intra_ag_ms_str   = fmt_time(round(intra_ag_ms), 'millisecond', precision=0, commas=False)
    hier_total_ms_str = fmt_time(hier_total_ms, 'millisecond', precision=1, commas=False)
    ib_bw_gbs_str     = fmt_qty(ib_bw_gbs * GB/second, GB/second, precision=0, commas=False)
    nvlink_bw_gbs_str = fmt_qty(nvlink_bw_gbs * GB/second, GB/second, precision=0, commas=False)
    ...
    flat_send_gb_str  = fmt_qty(flat_data_gb * GB, GB, precision=0, commas=False)
```
...
from mlsysim.core.constants import MB
...
class OverlapBudgetCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check, fmt, fmt_time, fmt_qty
    check(no_overlap_ms > with_overlap_ms, "No overlap should take longer than with overlap")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_backward_ms_str      = fmt_time(total_backward_ms, 'millisecond', precision=0, commas=False)
    allreduce_per_layer_ms_str = fmt_time(allreduce_per_layer_ms, 'millisecond', precision=1, commas=False)
    no_overlap_ms_str          = fmt_time(no_overlap_ms, 'millisecond', precision=1, commas=False)
    with_overlap_ms_str        = fmt_time(with_overlap_ms, 'millisecond', precision=1, commas=False)
    overlap_savings_pct_str    = fmt_percent(overlap_savings_pct/100, precision=1, commas=False, style='prose')
    exposed_per_layer_ms_str   = fmt_time(exposed_per_layer_ms, 'millisecond', precision=1, commas=False)
    n_layers_str               = fmt(n_layers_overlap, precision=0, commas=False)
    backward_per_layer_ms_str  = fmt_time(backward_per_layer_ms, 'millisecond', precision=0, commas=False)
    n_gpus_overlap_str         = fmt_count(n_gpus_overlap, commas=False, label="GPU")
    gradient_per_layer_mb_str  = fmt_qty(gradient_per_layer_mb_val * MB, MB, precision=0, commas=False)
    bucket_size_mb_str         = fmt_qty(bucket_size_mb * MB, MB, precision=0, commas=False)
...
class CommInfraRecap:
    ...
    ib_gbs = Systems.Fabrics.InfiniBand_NDR.bandwidth.to(GB/second).magnitude
    from mlsysim.fmt import fmt, fmt_qty
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvlink_h100_bw_gbs_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth, GB/second, precision=0, commas=False)
    ib_ndr_bw_gbs_str = fmt_qty(Systems.Fabrics.InfiniBand_NDR.bandwidth, GB/second, precision=0, commas=False)
```
