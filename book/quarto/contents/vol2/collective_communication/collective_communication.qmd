...
for n in gpu_counts_num:
    ...
    # Effective BW decreases at scale due to topology. Intra-node (n <= 8)
    # rides NVLink, not HBM (2026-06-10 audit: HBM bandwidth here understated
    # the 8-GPU comm share ~3.7x); beyond a node, the IB fabric binds.
    bw = h.nvlink.bandwidth if n <= 8 else Systems.Fabrics.InfiniBand_NDR.bandwidth
...
@Tbl-interconnect-parameters shows typical values for data center interconnects, and the critical-size column carries the load-bearing pattern: intra-node NVLink stays latency-bound only up to roughly a megabyte, whereas the inter-node fabrics cross over near 100 KB, so a message large enough to be bandwidth-bound inside a node can still be latency-bound once it travels between nodes.
...
# │
# │ Imports: mlsysim registries + calc_alpha_beta_crossover + fmt helpers
# │ Exports: InterconnectAlphaBeta.* (_str per table cell)
# └─────────────────────────────────────────────────────────────────────────────
...
class InterconnectAlphaBeta:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(nvlink_nstar > 1 * MB, "NVLink crossover should exceed 1 MB — intra-node stays latency-bound up to ~MB messages")
...
: **Interconnect Performance Parameters**: Effective per-message launch latency ($\alpha$, including the software stack), registry link bandwidth ($\beta$), and the computed crossover size $n^* = \alpha \cdot \beta$ at each row's $\alpha$ midpoint, where communication transitions from latency-bound to bandwidth-bound. {#tbl-interconnect-parameters}
