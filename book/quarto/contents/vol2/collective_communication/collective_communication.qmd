...
1.  **Ring Latency**\index{Ring Latency}: $2(N-1)\alpha = 2 \times 63 \times 10\ \mu$s = **`{python} RingVsTree.ring_lat_str` $\mu$s**.
2.  **Tree Latency**\index{Tree Latency}: $2(\log_2 N)\alpha = 2 \times 6 \times 10\ \mu$s = **`{python} RingVsTree.tree_lat_str` $\mu$s**.

*Bandwidth Terms (note the difference!):*

3.  **Ring Bandwidth**\index{Ring Bandwidth}: $2\frac{N-1}{N}\frac{M}{\beta} \approx 2 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = **`{python} RingVsTree.ring_bw_str` $\mu$s** (optimal: each byte sent once).
4.  **Tree Bandwidth**\index{Tree Bandwidth}: $2\log_2 N \cdot \frac{M}{\beta} = 12 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = **`{python} RingVsTree.tree_bw_str` $\mu$s** (each level sends full message).
...
- **Effective bandwidth**\index{Effective Bandwidth}: Limited by the slowest link = 50 GB/s (InfiniBand).
...
2. **Inter-Node AllReduce**\index{Inter-Node AllReduce}: Each GPU sends $1\ \text{GB}/8$ = `{python} HierarchicalAllreduceCalc.inter_data_mb_str` MB at 50 GB/s → ~`{python} HierarchicalAllreduceCalc.inter_time_ms_str` ms
