...
class AllReduceCost:
    ...
    ring_lat_time = (2 * (n_gpus - 1) * alpha).to(ms)
...
class LlamaBudgetCalc:
    ...
    inter_ring_total = calc_ring_allreduce_time(
        message_bytes=inter_payload, n_gpus=n_nodes_llama, bandwidth_bytes_s=ib_bw, latency_s=ib_alpha
    ).to(ms)
    ...
    flat_ring_total = calc_ring_allreduce_time(
        message_bytes=gradient_bf16, n_gpus=n_gpus_llama, bandwidth_bytes_s=ib_bw, latency_s=ib_alpha
    ).to(ms)
    flat_ring_bw = calc_ring_allreduce_time(
        message_bytes=gradient_bf16, n_gpus=n_gpus_llama, bandwidth_bytes_s=ib_bw, latency_s=0 * second
    ).to(ms)
    flat_ring_lat = (flat_ring_total - flat_ring_bw).to(ms)
...
class MoeAllToAll:
    ...
    bw_time = (calc_point_to_point_time(data_per_transfer, alpha, beta) - alpha).to(microsecond)
...
t_ring = np.array([
    calc_ring_allreduce_time(size * byte, N, bandwidth_bytes_sec * byte / second, latency).to(second).magnitude
    for size in message_sizes
])
t_tree = np.array([
    calc_tree_allreduce_time(size * byte, N, bandwidth_bytes_sec * byte / second, latency).to(second).magnitude
    for size in message_sizes
])
t_dbt = np.array([
...
from mlsysim.physics import calc_ring_allreduce_time, calc_tree_allreduce_time, calc_ring_tree_crossover_size
...
class RingVsTree:
    ...
    ring_lat = (ring_total - ring_bw).to(microsecond)
    ...
    tree_lat = (tree_total - tree_bw).to(microsecond)
...
class HierarchicalAllreduceCalc:
    ...
    flat_data = 2 * gradient
    flat_time = calc_ring_allreduce_time(
        message_bytes=gradient,
        n_gpus=n_nodes_har * gpus_per_node,
