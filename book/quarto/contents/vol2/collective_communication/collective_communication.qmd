...
# AllReduce communication cost for a Llama-class model
# Context: Napkin Math callout in Communication Fundamentals
llama_params_b = 70  # 70B parameters
bytes_per_param = 4  # FP32 gradients
gradient_size_gb = llama_params_b * bytes_per_param  # 280 GB
n_gpus_napkin = 64
ib_bw_per_port = 50  # GB/s NDR 400G per port
nics_per_gpu = 1  # single NIC per GPU

# Ring AllReduce bandwidth term: 2 * (N-1)/N * M / beta
ring_bw_factor = 2 * (n_gpus_napkin - 1) / n_gpus_napkin
ring_bw_time_s = ring_bw_factor * gradient_size_gb / ib_bw_per_port
ring_bw_time_ms = ring_bw_time_s * 1000

# Ring AllReduce latency term: 2*(N-1)*alpha
alpha_us = 3  # microseconds
ring_lat_time_us = 2 * (n_gpus_napkin - 1) * alpha_us
ring_lat_time_ms = ring_lat_time_us / 1000

# Total
ring_total_ms = ring_bw_time_ms + ring_lat_time_ms

# Compute time for one training step (approximate for context)
# H100 at ~1000 TFLOPS FP16, ~6 * params FLOPs per token, batch of ~4M tokens
flops_per_step = 6 * llama_params_b * 1e9 * 4096  # simplified single microbatch
h100_tflops = 1000
compute_time_ms = flops_per_step / (h100_tflops * 1e12) * 1000 / n_gpus_napkin
...
n_gpus_napkin_str = f"{n_gpus_napkin}"
...
$$T_{bandwidth} = 2 \cdot \frac{N-1}{N} \cdot \frac{M}{\beta}$$

Substituting: $T_{bandwidth}$ = `{python} ring_bw_factor_str` $\times$ 280 GB / 50 GB/s $\approx$ `{python} ring_bw_time_ms_str` ms.

**Step 3: Add the latency term.**

$$T_{latency} = 2(N-1) \cdot \alpha$$

Substituting: $T_{latency}$ = 2 $\times$ 63 $\times$ 3 $\mu$s = `{python} ring_lat_time_ms_str` ms.

**Step 4: Total communication time.**

Total: $T_{AllReduce} \approx$ `{python} ring_bw_time_ms_str` + `{python} ring_lat_time_ms_str` $\approx$ `{python} ring_total_ms_str` ms.
...
# Llama 70B communication budget analysis
# Context: Worked example in alpha-beta section

n_gpus_llama = 128
n_nodes_llama = 16
gpus_per_node_llama = 8
param_count_b = 70
gradient_fp32_gb = param_count_b * 4  # 280 GB in FP32
...
# Hierarchical AllReduce for BF16 gradients
# Step 1: Intra-node ReduceScatter
intra_payload_gb = gradient_bf16_gb * (gpus_per_node_llama - 1) / gpus_per_node_llama  # ~122.5 GB
intra_rs_time_ms = intra_payload_gb / nvlink_bw * 1000  # ms

# Step 2: Inter-node AllReduce (reduced payload)
inter_payload_gb = gradient_bf16_gb / gpus_per_node_llama  # 17.5 GB per GPU
# Ring among 16 nodes
...
# Typical H100 backward pass for 70B at batch=1 microbatch (rough)
# ~6 * 70B * 2048 (seq_len) * 2 (fwd+bwd) FLOPs per microbatch
# distributed across 128 GPUs at ~1000 TFLOPS each
compute_per_step_ms = 200  # approximate for illustration

# Comm-to-compute ratio
...
The table reveals two critical lessons. First, the alpha-beta model dramatically underestimates small-message latency because it accounts only for wire-level propagation, not the software stack overhead. For latency-sensitive operations (tensor parallelism AllReduce, MoE token routing), the effective alpha is 5--10 $\times$ higher than the physical wire latency. Second, for large messages the model is remarkably accurate, confirming that bandwidth is the binding constraint and that NCCL's internal optimizations (channel pipelining, kernel fusion) successfully saturate the available links.
...
$$T_{bandwidth}^{min} = \frac{2(N-1)}{N} \cdot \frac{M}{\beta}$$
...
$$ T_{butterfly} = 2\log_2 N \cdot \alpha + 2\frac{N-1}{N} \cdot \frac{M}{\beta} $$
...
In practice, NCCL uses Recursive Halving-Doubling as one of its internal algorithm choices for medium-sized messages on smaller GPU counts where it outperforms both Ring and Tree. For very large clusters, the non-local communication pattern creates contention on shared network links that erodes the theoretical advantage.
...
- [ ] Given a 256-GPU cluster with $\alpha = 5\ \mu\text{s}$ and $\beta = 50\ \text{GB/s}$, can you calculate $M_{crossover}$ and determine whether a 10 MB gradient buffer should use Ring or Tree?
...
# Three-level hierarchical AllReduce analysis
# Context: Napkin Math callout in Hierarchical AllReduce section

# Cluster: 4 racks, 4 nodes per rack, 8 GPUs per node = 128 GPUs
n_racks = 4
nodes_per_rack = 4
gpus_per_node_3l = 8
...
# Level 2: InfiniBand within rack (intra-rack, inter-node)
ib_bw_3l = 50  # GB/s per port
step2_payload_gb = gradient_3l_gb / gpus_per_node_3l  # 0.25 GB per GPU
# Ring among 4 nodes in a rack
step2_ring_bw_ms = 2 * (nodes_per_rack - 1) / nodes_per_rack * step2_payload_gb / ib_bw_3l * 1000

# Level 3: Spine switch between racks (cross-rack)
# Oversubscription ratio 2:1 reduces effective cross-rack BW
cross_rack_bw = 25  # GB/s effective (50/2 oversubscription)
step3_payload_gb = step2_payload_gb / nodes_per_rack  # further reduced by intra-rack reduction
step3_ring_bw_ms = 2 * (n_racks - 1) / n_racks * step3_payload_gb / cross_rack_bw * 1000

# Total with symmetric gather phases
total_3l_ms = 2 * step1_time_ms + 2 * step2_ring_bw_ms + step3_ring_bw_ms
# Flat baseline
flat_3l_ms = 2 * gradient_3l_gb / cross_rack_bw * 1000  # limited by slowest link
...
Hierarchical AllReduce reduces the *volume* of cross-node traffic, but the aggregation still requires multiple network round-trips. An alternative approach eliminates round-trips entirely by performing the reduction *inside the network switch itself*. NVIDIA's **Scalable Hierarchical Aggregation and Reduction Protocol (SHARP)**[^fn-sharp] implements this idea: instead of gradients traveling to a destination GPU for summation, the InfiniBand switch aggregates partial sums as data packets pass through it.

[^fn-sharp]: **SHARP (Scalable Hierarchical Aggregation and Reduction Protocol)**: NVIDIA's in-network computing technology, supported on Quantum InfiniBand switches. SHARP performs reduction operations (sum, min, max) in the switch ASIC, reducing the number of bytes traversing the network and eliminating multiple round-trips in tree-based collectives.
...
The practical impact is most pronounced for medium-sized messages (1--64 MB) where latency contributes meaningfully to total AllReduce time. For very large messages (greater than 1 GB), bandwidth dominates regardless of algorithm, and SHARP's latency reduction becomes proportionally smaller. For very small messages, SHARP's benefit is limited by the switch's own processing overhead. Production deployments at NVIDIA and Microsoft have reported 2--3 $\times$ AllReduce speedups for typical training gradient sizes when SHARP is enabled on Quantum InfiniBand switches.
...
**Why Rails Matter**: Without rail alignment, all 8 GPUs on a node might try to send to the same remote GPU simultaneously, creating 8 $\times$ contention on a single NIC. Rail-aligned routing ensures each NIC handles exactly 1/8 of the traffic, achieving full bisection bandwidth utilization.
...
The previous section attacked the communication bottleneck from the algorithm side: choosing the right collective, exploiting the bandwidth hierarchy, and routing traffic along optimal physical paths. These techniques reduce the *overhead* of moving data but do not change the *amount* of data to be moved. For a 70B parameter model producing 280 GB of FP32 gradients per step, even a perfectly hierarchical, rail-optimized AllReduce must still transfer those 280 GB across the network fabric. When the best algorithms and the fastest rails still leave communication as the dominant bottleneck, the remaining option is to reduce the size of the payload itself.
...
**Trade-off**: Each reduction in bit-width introduces quantization noise. This noise acts as a biased perturbation to the true gradient direction. For aggressive quantization (INT8 and especially 1-bit), the systematic bias can prevent convergence unless corrected by **Error Feedback** (discussed below).
...
The preceding sections developed three complementary strategies for taming communication cost: algorithmic efficiency (choosing the right collective), topological awareness (mapping the algorithm to the physical network), and payload reduction (gradient compression). Each strategy is a mathematical abstraction. Translating these abstractions into executable code that saturates real hardware requires communication *libraries* that implement collective operations with hardware-specific optimizations. The choice of library determines which algorithms are available, how efficiently they map to the physical network, and what level of hardware support (GPUDirect, NVLink, RDMA) is exploited. Three libraries dominate distributed ML communication, each targeting a different hardware and portability sweet spot.
...
The preceding sections address communication cost from three angles: algorithms (Ring, Tree, hierarchical) reduce the number of bytes each link must carry; topology-aware routing ensures those bytes flow without contention; and compression shrinks the bytes themselves. Yet all three approaches share a common assumption: communication and computation are sequential phases. The GPU computes gradients, then the network carries them, then the GPU resumes computing. In practice, this sequential model wastes GPU cycles. During communication, the GPU is idle (or performing only the non-overlappable overhead $o$ from the LogP model). During computation, the network is idle. **Pipelining** these phases is the final optimization that transforms a communication-bound training run into one where communication is hidden behind useful work.
...
Additionally, the first and last layers of the backward pass cannot overlap. The first layer to complete (the output layer) has no prior communication to overlap with. The last layer to communicate (the input layer) has no subsequent computation to overlap with. For very shallow models (2--3 layers), these boundary effects consume a significant fraction of the total time, limiting the achievable overlap to 50--60%. For deep models (dozens of layers or more), the boundary effects are negligible, and overlap can hide 90--95% of communication time.
...
# Communication-computation overlap budget
# Context: Quantitative example in overlap section
...
# Gradient size per layer (attention + FFN weights for a 7B model)
params_per_layer_m = 220  # million parameters
gradient_per_layer_mb = params_per_layer_m * 4  # FP32, in MB
gradient_per_layer_mb_val = 880  # MB

# AllReduce time per bucket (hierarchical, 64 GPUs)
bucket_size_mb = 100  # MB
allreduce_per_bucket_ms = 3  # ms (achievable with hierarchical on IB NDR)
buckets_per_layer = gradient_per_layer_mb_val / bucket_size_mb  # ~8.8
...
# With overlap: communication hidden behind next layer's backward
# Exposed communication = first layer's allreduce (no prior compute to hide it)
# + any excess where allreduce > backward per layer
exposed_per_layer_ms = max(0, allreduce_per_layer_ms - backward_per_layer_ms)
with_overlap_ms = total_backward_ms + allreduce_per_layer_ms + (n_layers_overlap - 1) * exposed_per_layer_ms
...
$T_{sequential} = T_{backward} + T_{comm}$ = `{python} total_backward_ms_str` ms + 32 $\times$ `{python} allreduce_per_layer_ms_str` ms = `{python} no_overlap_ms_str` ms.
...
$T_{pipelined} = T_{backward} + T_{first\_layer\_comm} + (N_{layers} - 1) \times T_{exposed}$ = `{python} with_overlap_ms_str` ms.
...
The key insight is that Archetype A and Archetype B, despite both operating at datacenter scale, face fundamentally different bottlenecks. Archetype A (dense LLMs) is bandwidth-bound and benefits from hierarchical AllReduce combined with communication-computation overlap. Archetype B (MoE) is latency-bound and benefits from low-latency switches and topology-aware AllToAll routing. Applying the wrong optimization to the wrong archetype wastes engineering effort without improving performance.
