...
from mlsysim.core.constants import GB, byte, microsecond, ms, param, second
...
class AllReduceCost:
    ...
    check(
        ring_total > 5000 * ms,
        f"AllReduce ({ring_total.to(ms).magnitude:.0f}ms) should be significant (>5s) for 70B BF16 model."
    )
...
from mlsysim.core.constants import byte, ms, param
...
from mlsysim.core.constants import GB, KB, microsecond, second
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.nvlink.bandwidth,
...
from mlsysim.core.constants import BILLION, GB, byte, microsecond, ms, param, second
...
from mlsysim.core.constants import BYTES_FP16, GB, KB, MB, microsecond, second
...
The algorithm splits the vector of size $M$ into $N$ chunks and then alternates communication and local reduction in two named phases: Scatter-Reduce\index{Ring AllReduce!scatter-reduce phase} followed by AllGather\index{Ring AllReduce!AllGather phase}. @Alg-ring-allreduce states the mechanics before the figure and trace make the same dataflow concrete.

::: {#alg-ring-allreduce name="Ring AllReduce"}

**Require**: $N$ ranks arranged in a logical ring; each rank $i$ starts with tensor $G_i$ of size $M$ bytes, partitioned into $N$ equal chunks.

**Output**: Every rank holds the reduced tensor $G=\sum_{i=0}^{N-1}G_i$.

1. Partition each local tensor into chunks of size $M/N$ and assign a ring order $0\to1\to\dots\to N-1\to0$.
2. For $N-1$ Scatter-Reduce steps, each rank sends one chunk or partial sum to its right neighbor, receives one from its left neighbor, and adds the received contribution into the matching local partial sum.
3. After Scatter-Reduce, each rank owns one fully reduced chunk of $G$.
4. For $N-1$ AllGather steps, each rank forwards fully reduced chunks around the same ring while storing the chunks it receives.
5. After AllGather, concatenate the $N$ reduced chunks so every rank holds the complete reduced tensor $G$.

**Systems cost**: Steps 2 and 4 perform $2(N-1)$ sequential send/receive rounds. Each rank moves $M/N$ bytes per round, for $2(N-1)M/N$ bytes sent per rank and the same amount received; the bandwidth term reaches the lower bound, but the latency term is $2(N-1)\alpha$.

:::

The data flow during the Scatter-Reduce phase is illustrated in @fig-ring-allreduce.
...
from mlsysim.core.constants import GB, MB, microsecond, second
import math
...
# │
# │ Imports: mlsysim.core.constants (MB, GB, second, ms, microsecond, Hardware.Cloud.H100.nvlink.bandwidth,
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import GB, MB, microsecond, ms, second
...
from mlsysim.core.constants import BILLION, MB, ms
...
# │
# │ Imports: mlsysim.core.constants (MB, ms)
...
class OverlapBudgetCalc:
    ...
    with_overlap = (
        total_backward
        + allreduce_per_layer
        + (n_layers_overlap - 1) * exposed_per_layer
    )
...
A common fallacy is this: *Bandwidth is the only metric that matters.*
...
A recurring pitfall is this: *Assuming AllReduce works for everything.*
...
A common fallacy is this: *Pipeline parallelism eliminates communication overhead.*
...
A recurring pitfall is this: *Using Ring AllReduce by default without checking message size and topology.*
...
A common fallacy is this: *Gradient compression is safe without error feedback.*
...
A recurring pitfall is this: *Evaluating gradient compression by communication speedup alone.*
...
A common fallacy is this: *Async collectives always hide latency.*
...
A recurring pitfall is this: *Silent data corruption in the network.*
...
A common fallacy is this: *Flat AllReduce is good enough for multi-node training.*
...
A recurring pitfall is this: *Using the ring AllReduce formula for intra-node communication.*
...
A common fallacy is this: *Rank-to-GPU mapping does not affect collective performance.*
...
A recurring pitfall is this: *Benchmarking collectives without recording placement context.*
