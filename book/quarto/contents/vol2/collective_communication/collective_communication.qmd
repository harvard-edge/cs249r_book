...
The error-feedback trace in @tbl-error-feedback runs the same sequence with a residual accumulator $e_t$ that re-injects the discarded remainder into the next step. The example in \ref{exmp-collective-communication-error-feedback-mechanism} works through both traces to show that the deferred information is conserved rather than lost.
