...
class MoeAllToAll:
    ...
    per_transfer_us_str = fmt(per_transfer.to(microsecond).magnitude, precision=0, commas=False)
    serialized_us_str = fmt(serialized.to(microsecond).magnitude, precision=0, commas=False)
    parallel_us_str = fmt(parallel.to(microsecond).magnitude, precision=0, commas=False)
...
class RingVsTree:
    ...
    alpha_str = fmt(alpha_us, precision=0, commas=False)
    beta_str = fmt_qty(beta, GB/second, precision=0, commas=False)
    ring_lat_str = fmt(ring_lat.to(microsecond).magnitude, precision=0)
    tree_lat_str = fmt(tree_lat.to(microsecond).magnitude, precision=0, commas=False)
    ring_bw_str = fmt(ring_bw.to(microsecond).magnitude, precision=1, commas=False)
    tree_bw_str = fmt(tree_bw.to(microsecond).magnitude, precision=0)
    ring_total_str = fmt(ring_total.to(microsecond).magnitude, precision=1)
    tree_total_str = fmt(tree_total.to(microsecond).magnitude, precision=0)
