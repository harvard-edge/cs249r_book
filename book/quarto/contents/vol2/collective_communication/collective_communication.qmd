...
2.  **Bandwidth-Bound** $(n \gg n^*)$: For large messages (for example, LLM gradients, optimizer states), time is dominated by $n/\beta$. Optimization focuses on **compression** \index{Gradient Compression} (FP8, Top-k sparsity), **algorithm choice** (Ring vs. Tree), and **link aggregation**\index{Link Aggregation} (multi-rail NICs).
...
The crossover point in @fig-allreduce-crossover, where Ring overtakes Tree, follows from setting $T_{\text{ring}} = T_{\text{tree}}$ and solving for $M$.
...
Quantization is the least disruptive compression lever when bandwidth is binding because it keeps every gradient element but reduces the bits used to represent it. Most gradients are computed in FP32 (32-bit floating point) or BF16 (16-bit brain float), so reducing bit-width directly reduces communication volume.

The useful question is how much gradient fidelity the optimizer can absorb. The progression from mild to aggressive quantization illustrates the trade-off between compression ratio and gradient fidelity:
