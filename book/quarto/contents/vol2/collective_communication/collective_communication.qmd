...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ COMMUNICATION INFRASTRUCTURE AND PERFORMANCE (P.I.C.O.)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-collective-communication — chapter-wide interconnect specs
# │
# │ Goal: Surface canonical interconnect bandwidth and latency constants.
...
class CommInfraSetup:
    """
    Namespace for global communication constants.
    Hierarchy:
    - Level 1: Interconnect Specs (Silicon Contract)
    - Level 4: Bandwidth Ratios (Physical Laws)
    """
    # ┌── 1. PARAMETERS (Inputs) ───────────────────────────────────────────────
    ...
    nvlink_gbs_val = nvlink_raw.m_as(GB/second)
    ib_gbs_val = (ib_ndr_raw / 8).m_as(GB/second)

    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    check(nvlink_gbs_val == 900, f"NVLink H100 should be 900 GB/s, got {nvlink_gbs_val}")
    check(ib_gbs_val == 50, f"InfiniBand NDR should be 50 GB/s, got {ib_gbs_val}")

    # ┌── 4. OUTPUTS (Formatting) ──────────────────────────────────────────────
    nvlink_h100_bw_gbs = f"{nvlink_gbs_val:.0f}"
    ib_ndr_bw_gbs = f"{ib_gbs_val:.0f}"

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
nvlink_h100_bw_gbs = CommInfraSetup.nvlink_h100_bw_gbs
ib_ndr_bw_gbs = CommInfraSetup.ib_ndr_bw_gbs
```
...
Substituting: $T_{latency}$ = 2$\times$ $63\times3$ $\mu$s = `{python} ring_lat_time_ms_str` ms.
