...
class CriticalMsgSize:
    ...
    n_star_kb = int(n_star_bytes / 1e3)
    # ┌── 3. GUARD ─────────────────────────────────────────
...
cms_alpha_str = CriticalMsgSize.alpha_str
cms_beta_str = CriticalMsgSize.beta_str
cms_n_star_str = CriticalMsgSize.n_star_kb_str
```

::: {.callout-notebook title="The Critical Message Size"}
**Problem**: Your cluster uses InfiniBand NDR 400G with $\alpha = `{python} cms_alpha_str`\ \mu s$ and $\beta = `{python} cms_beta_str`\ \text{GB/s}$. At what message size does optimizing for bandwidth start to matter more than optimizing for latency?

**The Math**:
$$n = \alpha \cdot \beta = 2 \times 10^{-6}\ \text{s} \times 50 \times 10^9\ \text{B/s} = `{python} cms_n_star_str`\ \text{KB}$$

**The Systems Insight**: Messages under `{python} cms_n_star_str` KB (like MoE tokens, pipeline activations) are **latency-bound**—buy lower-latency switches, reduce software overhead. Messages over `{python} cms_n_star_str` KB (like LLM gradients) are **bandwidth-bound**—buy more bandwidth, compress the data. Applying the wrong optimization wastes money without improving performance.
:::
...
lbw_alpha_str = LatencyVsBw.alpha_str
lbw_beta_str = LatencyVsBw.beta_str
lbw_bw_a_str = LatencyVsBw.bw_a_us_str
lbw_total_a_str = LatencyVsBw.total_a_us_str
lbw_lat_pct_a_str = LatencyVsBw.lat_pct_a_str
lbw_bw_b_str = LatencyVsBw.bw_b_us_str
lbw_bw_b_ms_str = LatencyVsBw.bw_b_ms_str
lbw_total_b_str = LatencyVsBw.total_b_us_str
lbw_lat_pct_b_str = LatencyVsBw.lat_pct_b_str
```

::: {.callout-notebook title="Latency vs. Bandwidth Dominance"}
**Problem**: You are synchronizing a 1 MB buffer versus a 1 GB buffer on InfiniBand NDR with $\alpha = `{python} lbw_alpha_str`\ \mu s$ and $\beta = `{python} lbw_beta_str`\ \text{GB/s}$. How does the bottleneck shift?

**Case A: 1 MB Message**

*   Bandwidth Time: $10^6 / (50 \times 10^{9})$ = `{python} lbw_bw_a_str` $\mu\text{s}$.
*   Latency Time: $2\ \mu s$.
*   **Total: `{python} lbw_total_a_str` μs**. Latency is `{python} lbw_lat_pct_a_str`% of total—still meaningful!

**Case B: 1 GB Message**

*   Bandwidth Time: $10^9 / (50 \times 10^{9})$ = `{python} lbw_bw_b_str` $\mu\text{s}$ = `{python} lbw_bw_b_ms_str` ms.
*   Latency Time: $2\ \mu s$.
*   **Total: `{python} lbw_total_b_str` μs**. Latency is `{python} lbw_lat_pct_b_str`%—completely negligible.
...
logp_L_str = LogPOverlap.L_str
logp_two_o_str = LogPOverlap.two_o_str
logp_compute_str = LogPOverlap.compute_str
logp_effective_str = LogPOverlap.effective_str
```

::: {.callout-notebook title="Can You Hide the Communication?"}
**Problem**: You want to overlap gradient AllReduce with the next layer's backward pass. The backward pass takes `{python} logp_compute_str` μs. The AllReduce has network latency $L = 100\ \mu s$ but processor overhead $o = 50\ \mu s$ to initiate and $o = 50\ \mu s$ to receive. Can you hide the communication?

**The Math**:

1.  **Overlappable portion**: Network latency $L$ = `{python} logp_L_str` $\mu\text{s}$ (data in flight while GPU computes).
2.  **Non-overlappable portion**: $2o$ = `{python} logp_two_o_str` $\mu\text{s}$ (GPU busy initiating/receiving).
3.  **Compute available**: `{python} logp_compute_str` $\mu\text{s}$.
4.  **Hidden**: All `{python} logp_L_str` μs of $L$ can overlap with compute.
5.  **Exposed**: The `{python} logp_two_o_str` μs of $o$ cannot overlap.

**Effective time**: $\max(`{python} logp_compute_str`, `{python} logp_L_str` + `{python} logp_two_o_str`)$ = `{python} logp_effective_str` $\mu\text{s}$ (communication hidden!).
...
moe_alpha_str = MoeAllToAll.alpha_str
moe_beta_str = MoeAllToAll.beta_str
moe_tpg_str = MoeAllToAll.tokens_per_gpu_str
moe_tpt_str = MoeAllToAll.tokens_per_transfer_str
moe_kbpt_str = MoeAllToAll.kb_per_token_str
moe_tpe_str = MoeAllToAll.tokens_per_expert_str
moe_dpxfer_str = MoeAllToAll.data_per_transfer_kb_str
moe_sent_str = MoeAllToAll.total_sent_mb_str
moe_bw_str = MoeAllToAll.bw_time_approx_str
moe_xfer_str = MoeAllToAll.per_transfer_us_str
moe_serial_str = MoeAllToAll.serialized_us_str
moe_par_str = MoeAllToAll.parallel_us_str
```

::: {.callout-notebook title="AllToAll for MoE Token Routing" collapse="false"}

**Problem**: A MoE model processes a batch of 4096 tokens across 8 GPUs (`{python} moe_tpg_str` tokens per GPU). Each token is a 2048-dimensional hidden state in BF16 (`{python} moe_kbpt_str` KB per token). The gating network assigns each token to exactly 1 of 64 experts (8 experts per GPU). Assuming uniform routing (each expert receives $4096/64$ = `{python} moe_tpe_str` tokens), how much data does each GPU send and receive?

**The Math**:

Each GPU holds `{python} moe_tpg_str` tokens that need to reach 64 different experts across 8 GPUs. With uniform routing, each GPU sends $512/8$ = `{python} moe_tpt_str` tokens to each of the other 7 GPUs (keeping `{python} moe_tpt_str` tokens local).

*   Data per GPU-to-GPU transfer: `{python} moe_tpt_str` tokens $\times$ `{python} moe_kbpt_str` KB/token = `{python} moe_dpxfer_str` KB
*   Total data sent per GPU: 7 $\times$ `{python} moe_dpxfer_str` KB = `{python} moe_sent_str` MB
*   Total data received per GPU: `{python} moe_sent_str` MB (symmetric)

**Latency Analysis (InfiniBand NDR, $\alpha = `{python} moe_alpha_str`\ \mu\text{s}$, $\beta = `{python} moe_beta_str`\ \text{GB/s}$)**:

Each of the 7 transfers is a `{python} moe_dpxfer_str` KB message. From the alpha-beta model: $T$ = 3 + `{python} moe_bw_str` = `{python} moe_xfer_str` $\mu\text{s}$ per transfer.

If serialized: 7 $\times$ `{python} moe_xfer_str` $\mu\text{s}$ = `{python} moe_serial_str` $\mu\text{s}$. If all 7 transfers run in parallel (full-duplex, non-blocking): $\approx$ `{python} moe_par_str` $\mu\text{s}$.
...
rt_steps_str = RingTrace.steps_str
rt_elem_str = RingTrace.elem_str
rt_total_str = RingTrace.total_str
```
...
**Total data movement per GPU**: In each of the $2(N-1)$ = `{python} rt_steps_str` steps, each GPU sends exactly $M/N$ = `{python} rt_elem_str` element. Total per GPU: `{python} rt_steps_str` $\times$ `{python} rt_elem_str` = `{python} rt_total_str` elements sent, `{python} rt_total_str` received.
...
class RingVsTree:
    ...
    check(abs(ring_bw_us - 200) < 1, f"Ring bw = {ring_bw_us}")
...
rvt_alpha_str = RingVsTree.alpha_str
rvt_beta_str = RingVsTree.beta_str
rvt_ring_lat_str = RingVsTree.ring_lat_str
rvt_tree_lat_str = RingVsTree.tree_lat_str
rvt_ring_bw_str = RingVsTree.ring_bw_str
rvt_tree_bw_str = RingVsTree.tree_bw_str
rvt_ring_total_str = RingVsTree.ring_total_str
rvt_tree_total_str = RingVsTree.tree_total_str
rvt_crossover_str = RingVsTree.crossover_str
```

::: {.callout-notebook title="The Ring vs. Tree Crossover"}
**Problem**: You are synchronizing a **1 MB** buffer across **64 GPUs**. The network has latency $\alpha = `{python} rvt_alpha_str`\ \mu s$ and bandwidth $\beta = `{python} rvt_beta_str`\ \text{GB/s}$. Should you use Ring or Tree?

**The Math**:

*Latency Terms:*

1.  **Ring Latency**: $2(N-1)\alpha = 2 \times 63 \times 10\ \mu s$ = $\mathbf{`{python} rvt_ring_lat_str`\ \mu s}$.
2.  **Tree Latency**: $2(\log_2 N)\alpha = 2 \times 6 \times 10\ \mu s$ = $\mathbf{`{python} rvt_tree_lat_str`\ \mu s}$.

*Bandwidth Terms (note the difference!):*

3.  **Ring Bandwidth**: $2\frac{N-1}{N}\frac{M}{\beta} \approx 2 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = $\mathbf{`{python} rvt_ring_bw_str`\ \mu s}$ (optimal—each byte sent once).
4.  **Tree Bandwidth**: $2\log_2 N \cdot \frac{M}{\beta} = 12 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = $\mathbf{`{python} rvt_tree_bw_str`\ \mu s}$ (each level sends full message).

**Total Time**:

| **Algorithm** | **Latency** | **Bandwidth** |    **Total** |
|:--------------|------------:|--------------:|-------------:|
| **Ring**      |    `{python} rvt_ring_lat_str` μs |        `{python} rvt_ring_bw_str` μs | **`{python} rvt_ring_total_str` μs** |
| **Tree**      |      `{python} rvt_tree_lat_str` μs |      `{python} rvt_tree_bw_str` μs | **`{python} rvt_tree_total_str` μs** |

**Tree wins**, but only by 10%. For this 1 MB message, we are near the crossover point.

**The Systems Insight**: Ring's latency penalty (10$\times$ worse than Tree) nearly balances Tree's bandwidth penalty (6$\times$ worse than Ring). The crossover formula predicts: $M_{\text{crossover}} = N \cdot \alpha \cdot \beta = 64 \times 10\ \mu s \times 10\ \text{GB/s}$ = `{python} rvt_crossover_str` MB. At 1 MB, we are below crossover—Tree wins. At 10 MB, Ring would dominate.
:::
...
- **Time**: $\approx 2 \times 1\ \text{GB} / 50\ \text{GB/s}$ = `{python} flat_time_ms_str` ms (bandwidth term dominates).
...
2. **Inter-Node AllReduce**: Each GPU sends $1\ \text{GB}/8$ = `{python} inter_data_mb_str` MB at 50 GB/s → ~`{python} inter_time_ms_str` ms
...
- **Total Time**: $\approx$ `{python} intra_rs_ms_str` + `{python} inter_time_ms_str` + `{python} intra_ag_ms_str` = `{python} hier_total_ms_str` ms
...
ef_cum_str = ErrorFeedbackTrace.cum_str
ef_error_str = ErrorFeedbackTrace.error_str
ef_true_str = ErrorFeedbackTrace.true_str
ef_naive_str = ErrorFeedbackTrace.naive_str
```
...
**Result**: After 5 steps, the system has transmitted **`{python} ef_naive_str`** but the true cumulative gradient is **`{python} ef_true_str`**. The parameter never updates—**100% of gradient information is lost**.
...
**Result**: After 5 steps, the system has transmitted **`{python} ef_cum_str`** with a remaining error of **`{python} ef_error_str`**. The true cumulative is **`{python} ef_true_str`**, so transmitted + error = $`{python} ef_cum_str` + (`{python} ef_error_str`)$ = `{python} ef_true_str` ✓
...
nap_grad_str = NapkinOverlap.gradient_gb_str
nap_comm_str = NapkinOverlap.comm_s_str
nap_compute_str = NapkinOverlap.compute_s_str
nap_pct_str = NapkinOverlap.overlap_pct_str
```
...
1.  **Gradient Size:** 70B params $\times$ 2 bytes (FP16) = `{python} nap_grad_str` GB.
2.  **Communication Time:** Using Ring AllReduce on NVLink (900 GB/s), time is $\approx \frac{2 \cdot `{python} nap_grad_str`\text{GB}}{900\text{GB/s}}$ = `{python} nap_comm_str` s.
3.  **Compute Time:** A typical forward/backward pass for a block of this size takes $\approx$ `{python} nap_compute_str` s.
4.  **Overlap Ratio:** `{python} nap_comm_str` s / `{python} nap_compute_str` s $\approx$ `{python} nap_pct_str`%.
Since communication takes only `{python} nap_pct_str`% of the compute time, it fits comfortably within the backward pass "overlap budget," meaning the network cost effectively disappears.
:::
