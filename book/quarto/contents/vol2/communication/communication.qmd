...
Production AllReduce implementations must handle node failures gracefully. At 1000+ GPU scale, hardware failures occur multiple times per day, making fault tolerance a critical design consideration rather than an edge case. The checkpoint storage infrastructure developed in @sec-storage enables the recovery mechanisms discussed here; @sec-fault-tolerance provides comprehensive treatment of distributed training fault tolerance. Here we focus on the communication-specific failure modes and recovery strategies.
...
For models like Switch Transformer [@fedus2022switch] with one MoE layer every other transformer block, and GPT-4 (rumored to have 8 experts), communication overhead accumulates across many layers.
...
accumulated = (
    gradient + error_buffer
)  # Include previously discarded gradients
compressed = top_k(
    accumulated, k
)  # Keep only k largest magnitude elements
error_buffer = accumulated - decompress(
    compressed
)  # Store discarded for next iteration
allreduce(
    compressed
)  # Synchronize compressed gradients across workers
```
:::
...
if all_gpus_nvlink_connected:
    ...
    use_hierarchical_allreduce(
        intra=nvlink, inter=ib
    )  # Exploit multi-level topology
...
Frameworks like Horovod [@sergeev2018horovod][^fn-horovod] use MPI for process coordination while using NCCL for actual GPU communication:
...
Megatron-LM [@shoeybi2019megatron][^fn-megatron], developed by NVIDIA, demonstrates how to orchestrate multiple parallelism strategies for training models exceeding a trillion parameters. Its communication architecture illustrates careful collective selection and topology-aware design.
...
- Model: PaLM [@chowdhery2022palm] 540B parameters
