...
*   **Latency ($\alpha$)**: The fixed cost to send a message, regardless of size. This includes software overhead (kernel launch), PCIe traversal, and network switching time. Typical values: $5\text{-}20$ μs.
*   **Bandwidth ($\beta$)**: The data transfer rate. This is the inverse of the time to send one byte. Typical values: $25\text{-}100$ GB/s.
...
Engineers assume uniform network performance across clusters. In production, collective algorithms synchronize at the speed of the slowest link: every worker must complete its step before the next begins. Ring AllReduce (@sec-ring-allreduce) on a 1024-GPU cluster with one degraded InfiniBand cable (200 Gbps to 10 Gbps, 20× slower from physical damage) operates at 10 Gbps effective bandwidth cluster-wide. For a 7B model with 14 GB gradients, AllReduce time increases from 0.28s to 5.6s per step, a 20× slowdown. Standard monitoring reports average bandwidth, masking individual link failures. A single faulty cable among thousands reduces cluster throughput by 10×, and aggregate dashboards will not reveal it.
