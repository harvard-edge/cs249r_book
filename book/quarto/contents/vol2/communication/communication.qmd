...
::: {.callout-note title="Connection: From Parallelism Strategies to Communication Primitives"}
The previous chapter (@sec-distributed-training) established *what* we want to parallelize: data parallelism replicates models across workers, tensor parallelism partitions layers within a model, and pipeline parallelism stages computation across model depth. Each strategy assumes workers can efficiently exchange gradients, activations, or parameters. This chapter examines *how* that exchange happens: the collective operations, algorithms, and optimizations that determine whether distributed training achieves near-linear scaling or collapses under communication overhead.
:::
...
The parallelism strategies introduced in @sec-distributed-training—data parallelism, model parallelism, and pipeline parallelism—all share a common assumption: workers can exchange data efficiently. That assumption hides what becomes the dominant engineering challenge at scale.
...
**Conclusion**: For large models, **bandwidth ($B$)** dominates. Latency ($L$) only matters for small message collectives (like MoE[^fn-moe] routing).

[^fn-moe]: **Mixture of Experts (MoE)**: An architecture where input tokens are routed to specialized subnetworks (experts) rather than processed by the entire model. Each token activates only a subset of experts, reducing computation while maintaining model capacity. GPT-4 and Mixtral use MoE architectures. The routing decision requires AlltoAll communication to send tokens to their assigned experts across GPUs.
:::
...
::: {.callout-perspective title="Back-of-the-Napkin: The Ring vs. Tree Crossover"}
...
| **DLRM[^fn-lighthouse]**  | AlltoAll                 | Embedding exchange                      |
| **MoE**                   | AlltoAll                 | Token routing                           |

[^fn-lighthouse]: **Lighthouse Examples**: DLRM (Deep Learning Recommendation Model) and MoE (Mixture of Experts) serve as canonical workloads throughout Volume II, representing recommendation systems and sparse expert architectures respectively. These "lighthouse" examples illuminate how different model architectures create distinct communication patterns. DLRM's massive embedding tables require AlltoAll for cross-worker embedding exchange, while MoE's dynamic token routing similarly requires AlltoAll to direct tokens to their assigned experts.
...
### Adaptive Routing and Congestion Control {#sec-adaptive-routing}
...
### Debugging Communication Performance
