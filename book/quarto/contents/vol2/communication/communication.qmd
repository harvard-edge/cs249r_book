...
Gradient compression addresses bandwidth limitations through algorithmic means, reducing the volume of data that must be transferred. Network topology addresses the same challenge through hardware means, determining the physical paths and bandwidth available for transfers. Understanding both enables engineers to optimize communication from both algorithmic and infrastructure perspectives.

Network topology determines how GPUs connect to each other and fundamentally shapes collective operation performance. A topology optimized for AllReduce may perform poorly for AlltoAll. Different ML workloads benefit from different network topologies, and understanding these trade-offs helps engineers select appropriate infrastructure or adapt algorithms to existing hardware.
...
SHARP has several limitations. It requires SHARP-capable switches, specifically the Mellanox Quantum series. The technology supports only limited reduction operations including sum, max, and min. Floating-point precision is constrained to FP16 and BF16 formats. The approach adds infrastructure cost and complexity to cluster deployments.
