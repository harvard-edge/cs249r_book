...
The algorithms developed here allow us to synchronize thousands of GPUs efficiently. However, these collective operations are only tools. To build truly massive models, we must orchestrate these patterns within complex training schedules.

The next chapter, @sec-distributed-training, applies these communication primitives to the high-level logic of **Data, Model, and Pipeline Parallelism**, enabling the training of models that exceed the memory of any single node.
