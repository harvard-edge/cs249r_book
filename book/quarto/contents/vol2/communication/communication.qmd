...
In a naive implementation, the training loop is sequential:
1.  Compute Forward Pass
2.  Compute Backward Pass (calculate all gradients)
3.  Synchronize Gradients (AllReduce)
4.  Update Parameters

This leaves the network idle during computation and the compute units idle during synchronization. **Communication-computation overlap** changes this by pipelining the backward pass.

*   **Layer-wise Triggering**: As the backward pass proceeds from the last layer to the first, gradients for the final layers become available immediately.
*   **Bucketing**: Frameworks like PyTorch DistributedDataParallel group these gradients into "buckets" (typically 25MB).
*   **Async AllReduce**: As soon as a bucket is full, an asynchronous AllReduce operation is launched for that bucket.
*   **Hiding Latency**: While the AllReduce for bucket $N$ travels over the network, the GPU continues computing gradients for bucket $N-1$.

If the computation time for a bucket exceeds its communication time, the network cost is effectively zero (fully hidden). If communication is slower, the training time becomes determined solely by the network speed. Tuning bucket sizes is a key optimization: too small, and latency overhead dominates; too large, and the pipeline stalls waiting for data.
...
Ring AllReduce[^fn-ring-history] arranges processes in a logical ring and pipelines communication to achieve optimal bandwidth utilization. Originally developed for MPI implementations, it became the standard for distributed deep learning after adoption by Baidu [@gibiansky2017baidu] in 2017.
...
**Hierarchical AlltoAll** mitigates this by decomposing the operation:
1.  **Intra-node Gather**: Each GPU collects data destined for other nodes into a local buffer.
2.  **Inter-node AlltoAll**: One GPU per node performs a coarse-grained exchange with other nodes.
3.  **Intra-node Scatter**: Data is distributed to the final destination GPUs within the node.
...
NVIDIA's SHARP[^fn-sharp] (Scalable Hierarchical Aggregation and Reduction Protocol) performs reduction operations in InfiniBand switches:
...
PyTorch's DistributedDataParallel[^fn-ddp] buckets gradients for communication:
