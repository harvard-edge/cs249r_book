...
The **tiered staging** strategy minimizes $T_{\text{write}}$ by writing in two phases. In the first phase, each node writes its checkpoint shard to local NVMe at full bandwidth. With four NVMe drives providing 14 GB/s aggregate, a per-node shard of `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB (for a 256-node cluster, each node saves roughly 7 GB) completes in under a second. The training pipeline can resume immediately after the local write completes.
