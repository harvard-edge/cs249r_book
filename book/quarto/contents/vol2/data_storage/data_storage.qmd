...
***I/O Wall***\index{I/O Wall!definition} is the storage-compute bandwidth mismatch that occurs when accelerator consumption rates exceed the sustainable throughput of the storage subsystem. In a modern training cluster, GPU HBM consumes data at `{python} h100_bw_tbs` TB/s per device, while the fastest local storage delivers single-digit GB/s. The I/O Wall manifests as *accelerator starvation*: idle compute cycles caused not by algorithmic limitations but by the physics of moving data from where it is stored to where it is needed. Unlike the classic "memory wall" (which describes the latency gap between processor and DRAM), the I/O Wall spans multiple tiers of the storage hierarchy and requires a combination of caching, prefetching, and format optimization to bridge.
...
```{python}
#| label: fig-storage-compute-chasm
#| fig-cap: "**The Storage-Compute Chasm**. GPU peak FP16 throughput (blue, left axis) and NVMe sequential read bandwidth (orange, right axis) from 2016 to 2024, both on logarithmic scales. GPU throughput has grown 236$\\times$ while storage bandwidth has grown only 4$\\times$ over the same period. The shaded region highlights the widening gap that data pipeline engineering must bridge through prefetching, caching, and format optimization."
#| fig-alt: "Dual-line log-scale plot showing GPU TFLOPS growing from 21 to 5000 and storage bandwidth growing from 3.5 to 14 GB/s between 2016 and 2024."
#| echo: false
import numpy as np
