...
1. **Per-node write to local NVMe** (4 drives at 3.5 GB/s each = 14 GB/s): $7 \text{ GB} \div 14 \text{ GB/s} = 0.5$ seconds.
...
4. **Training pause**\index{Training!pause}: only 0.5 seconds (the local NVMe write). The PFS copy overlaps with the next training iteration.
5. **Overhead**\index{Overhead}: 0.5 s pause every 600 s $\approx$ **0.08 percent** training time lost to checkpointing.
...
The **tiered staging** strategy minimizes $T_{\text{write}}$ by writing in two phases. In the first phase, each node writes its checkpoint shard to local NVMe at full bandwidth. With four NVMe drives providing 14 GB/s aggregate, a per-node shard of roughly 7 GB (a 256-node ZeRO-3 split of the `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB total checkpoint) completes in under a second. The training pipeline can resume immediately after the local write completes.
