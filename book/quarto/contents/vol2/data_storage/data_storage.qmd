...
# │          MILLION)
# │ Exports: NVMeTierCalcs.raw_bw_mb_s_str, .hdd_slowdown_factor_mult_str,
...
2.  **HDD Reality**: A 7200 RPM hard disk drive (HDD) delivers `{python} NVMeTierCalcs.hdd_iops_str` random IOPS\index{HDD!random IOPS bottleneck}. Sustaining `{python} NVMeTierCalcs.images_per_sec_str` requires `{python} NVMeTierCalcs.hdd_slowdown_factor_mult_str` more IOPS than the disk provides.
...
* **GPU Direct Storage eliminates CPU bottlenecks**: GDS bypasses the CPU in the data path, reducing per-transfer latency by `{python} GDSLatencyRecap.gds_speedup_mult_str` and freeing CPU cores for augmentation.
...
* **Economics drive tiering**: The `{python} EconRatiosRecap.tier_cost_ratio_mult_str` cost difference between local NVMe and object storage\index{Storage Tiering!cost drivers} (and the >3,000$\times$ span from HBM to archive) mandates a tiering strategy. Egress fees often exceed storage fees; design for total cost of data delivery, not storage cost alone.
