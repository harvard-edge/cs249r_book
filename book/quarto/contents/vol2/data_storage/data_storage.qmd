...
For **image training**, the picture changes dramatically. Using a common batch size of 256 images (ImageNet at $224\times224,$ roughly 150 KB/image), the aggregate bandwidth explodes:
...
1.  **Raw Bandwidth**: 1,000 images/s$\times$ 150 KB/image = **`{python} NVMeTierCalcs.raw_bw_str` MB/s**.
2.  **HDD Reality**: A 7200 RPM HDD delivers 100 random IOPS. Sustaining 1,000 images/s requires **`{python} NVMeTierCalcs.hdd_slowdown_factor`$\times$ more IOPS** than the disk provides.
...
1. **Remote Only**: `{python} LocalCacheRoi.lc_compute_ms_str` / (`{python} LocalCacheRoi.lc_compute_ms_str` + `{python} LocalCacheRoi.lc_remote_io_ms_str`) $\approx$ **`{python} LocalCacheRoi.lc_util_remote_str` percent**.
...
**Traditional path**: Each image requires a DMA from NVMe to DRAM, a `memcpy` from kernel to user space, and a PCIe transfer to GPU. At 64,000 images/second with `{python} GDSLatency.gds_trad_us_str` μs of CPU time per image, the CPU spends 7.68 seconds of CPU time per wall-clock second, consuming roughly 8 cores worth of processing just for data movement.

**GDS path**: Each image is DMA'd directly from NVMe to GPU. At `{python} GDSLatency.gds_bypass_us_str` μs of CPU time per image (for initiating the DMA), the CPU spends 1.92 seconds of CPU time per wall-clock second, freeing 6 cores for data augmentation.
...
3. **Per-node PFS bandwidth**: If all 256 nodes write simultaneously, each gets $1000/256 = 3.9$ GB/s. Per-node async copy: $4/3.9 = 1.03$ seconds.
4. **Training pause**: only 0.29 seconds (the local NVMe write). The PFS copy overlaps with the next training iteration.
5. **Overhead**: 0.29 s pause every 600 s = **0.05 percent** training time lost to checkpointing.
...
**Total data moved through the hierarchy**: approximately `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb` PB of checkpoint data plus 3 TB$\times$ epochs of training data reads. For a single-epoch language model training run, checkpoint I/O dominates data loading I/O by a factor of over 1,000.
...
1. **Raw Payload**: 1 TB.
2. **Provenance Overhead**: `{python} int(SyntheticStorage.provenance_metadata_overhead * 100)` percent extra for lineage hashes, generation logs, and reward-model scores.
...
*   **Economics drive tiering**: The `{python} EconRatios.tier_cost_ratio_str`$\times$ cost difference between HBM and archive storage mandates a tiering strategy. Egress fees often exceed storage fees; design for total cost of data delivery, not storage cost alone.
