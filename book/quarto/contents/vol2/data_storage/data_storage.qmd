...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \tikzset{
    Box/.style={draw=black!60, line width=0.75pt, rounded corners=1pt, align=center},
    Line/.style={->, line width=1.0pt},
    HBM/.style={fill=BlueL, draw=BlueLine, line width=0.75pt},
    DRAM/.style={fill=GreenL, draw=GreenLine, line width=0.75pt},
    NVMe/.style={fill=OrangeL, draw=OrangeLine, line width=0.75pt},
    DFS/.style={fill=RedL, draw=RedLine, line width=0.75pt},
    Object/.style={fill=VioletL2, draw=VioletLine, line width=0.75pt}
  }
  % Pyramid Layers
  \coordinate (T) at (0, 5);
  ...
  % HBM
  \filldraw[HBM] (0,5) -- (-0.8,4) -- (0.8,4) -- cycle;
  \node at (0, 4.3) {\textbf{GPU HBM}};

  % DRAM
  \filldraw[DRAM] (-0.8,4) -- (-1.6,3) -- (1.6,3) -- (0.8,4) -- cycle;
  \node at (0, 3.5) {Host DRAM};

  % NVMe
  \filldraw[NVMe] (-1.6,3) -- (-2.4,2) -- (2.4,2) -- (1.6,3) -- cycle;
  \node at (0, 2.5) {Local NVMe SSD};

  % DFS
  \filldraw[DFS] (2.4,2) -- (3.2,1) -- (-3.2,1) -- (-2.4,2) -- cycle;
  \node at (0, 1.5) {Parallel File System};

  % Object
  \filldraw[Object] (-3.2,1) -- (-4,0) -- (4,0) -- (3.2,1) -- cycle;
  \node at (0, 0.5) {Object Storage};

  % Annotations
  \draw[Line] (-4.5, 0.5) -- (-4.5, 4.5) node[midway, left, rotate=90] {Bandwidth};
  \draw[Line] (4.5, 4.5) -- (4.5, 0.5) node[midway, right, rotate=90] {Capacity};
\end{tikzpicture}
```
:::
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=stealth]
  ...
  \tikzset{
    Box/.style={draw, line width=0.75pt, minimum width=2cm, minimum height=0.6cm, align=center},
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=stealth]
  \usetikzlibrary{positioning}
  \tikzset{
    box/.style={draw=black!60, line width=0.75pt, rounded corners, minimum width=2.2cm, minimum height=0.8cm, align=center},
    nvme/.style={box, fill=OrangeL, draw=OrangeLine},
    dram/.style={box, fill=GreenL, draw=GreenLine},
    cpu/.style={box, fill=RedL, draw=RedLine},
    gpu/.style={box, fill=BlueL, draw=BlueLine},
    arrow/.style={->, line width=1.0pt},
    gdsarrow/.style={->, line width=1.0pt, color=BlueLine},
  ...
  \node[nvme, right=0.5cm of tradLabel] (nvme1) {NVMe};
  \node[dram, right=1.3cm of nvme1] (dram1) {Host\\DRAM};
  \node[cpu, right=1.3cm of dram1] (cpu1) {CPU\\Copy};
  \node[gpu, right=1.3cm of cpu1] (gpu1) {GPU\\HBM};
  ...
  % GDS path
  \node[label, below=1.2cm of tradLabel] (gdsLabel) {GDS};
  \node[nvme] (nvme2) at (gdsLabel -| nvme1) {NVMe};
  \node[gpu] (gpu2) at (gdsLabel -| gpu1) {GPU\\HBM};

  \draw[gdsarrow] (nvme2) -- node[above, font=\scriptsize] {Direct DMA (PCIe peer-to-peer)} (gpu2);
