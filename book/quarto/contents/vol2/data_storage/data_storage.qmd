...
\noindent
![](images/png/cover_storage.png){fig-alt="Distributed data storage and scalable storage systems for ML workloads." width=100 percent}
...
For **text training**\index{Text Training}, the demand is surprisingly low. With a typical batch size of 4,096 tokens per GPU and a 200 ms step time, the aggregate bandwidth is:
...
For **image training**\index{Image Training}, the picture changes dramatically. Using a common batch size of 256 images (ImageNet at $224\times224,$ roughly 150 KB/image), the aggregate bandwidth explodes:
...
1.  **Raw Bandwidth**\index{Raw Bandwidth}: 1,000 images/s$\times$ 150 KB/image = **`{python} NVMeTierCalcs.raw_bw_str` MB/s**.
2.  **HDD Reality**\index{HDD Reality}: A 7200 RPM HDD delivers 100 random IOPS. Sustaining 1,000 images/s requires **`{python} NVMeTierCalcs.hdd_slowdown_factor`$\times$ more IOPS** than the disk provides.
...
3.  **Common Pitfall:** A frequent misconception is that this is "fixed" by buying faster SSDs. In reality, it is a **Format Problem**\index{Format Problem}: the fix is to pack samples into large sequential containers (for example, TFRecord, Parquet) to **Amortize** metadata operations across thousands of samples.
...
\index{Data Stall Ratio!GPU utilization}**Scenario**\index{Scenario}: Your GPU processes a batch in `{python} PipelineCalcs.t_comp_stall_str` ms, but storage takes `{python} PipelineCalcs.t_io_stall_str` ms to deliver the next batch.
...
The cost of deep prefetching is memory: each buffered batch occupies host DRAM. A batch of 256 images at 224$\times$ $224\times3$ bytes (after decoding) occupies roughly 37 MB. A prefetch buffer of 8 such batches consumes 300 MB, which is negligible for a node with 512 GB of DRAM. For large-batch language model training where each batch contains millions of tokens, however, the prefetch buffer can grow to several gigabytes, competing with embedding tables and other DRAM consumers.
...
A `{python} StorageHierarchyAnalysis.gpt3_params_b`B parameter model with Adam optimizer generates checkpoints of approximately `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB. The checkpoint includes model weights (`350` GB in FP16), optimizer state (momentum and variance, `700` GB in FP32), learning rate scheduler state, random number generator state, and the current data loader position. Every GPU in the cluster saves its shard of the checkpoint simultaneously, creating a checkpoint storm that the storage system must absorb without disrupting ongoing training reads.
...
3. **Per-node PFS bandwidth**\index{Per-Node PFS Bandwidth}: If all 256 nodes write simultaneously, each gets $1000/256 = 3.9$ GB/s. Per-node async copy: $4/3.9 = 1.03$ seconds.
...
*   **HBM is the destination**\index{HBM Is the Destination}: Every lower tier exists to keep GPU HBM populated. The 300,000$\times$ bandwidth gap between HBM and object storage drives every design decision in the ML storage hierarchy.
...
*   **Pipelining hides average latency, not tail latency**\index{Pipelining Hides Average Latency, Not Tail Latency}: Prefetch buffers must be sized for P99 I/O latency, not average, to prevent accelerator stalls at scale. Buffer depth of $\lceil T_{I/O,p99} / T_{\text{compute}} \rceil$ is the minimum.
...
*   **Metadata, not bandwidth, is the first bottleneck**\index{Metadata, Not Bandwidth, Is the First Bottleneck}: The small file problem collapses parallel file system throughput to a fraction of rated capacity. Aggregate samples into large shards to reduce metadata operations by 10,000$\times$ or more.
*   **Checkpoint staging minimizes $T_{save}$**\index{Checkpoint Staging Minimizes $t_{save}$}: Write to local NVMe first, then replicate asynchronously to shared storage. This decouples checkpoint pause time from parallel file system performance.
