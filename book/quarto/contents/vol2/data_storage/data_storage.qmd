...
# │
# │ Imports: mlsys.constants (H100_MEM_BW, NVME_SEQUENTIAL_BW, GPT3_PARAMS,
...
[^fn-raid-storage]: **RAID (Redundant Array of Independent Disks)**: A 1988 Berkeley taxonomy of drive-combining strategies, each trading redundancy against bandwidth. ML training inverts the database-era default: RAID 0 (striping, no parity) maximizes sequential read throughput at the cost of zero fault tolerance, a safe trade-off[^fn-raid0-immutable] because training data is immutable and durably backed in object storage. Choosing RAID 5 or 6 instead wastes 15--25% of bandwidth on parity calculations that protect data already protected elsewhere. \index{RAID!ML storage}
...
$$ \text{Utilization} = \min\!\left(1,\;\frac{BW_{\text{storage}}}{N_{GPU} \times R_{consumption}}\right) $$
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \tikzset{
    Box/.style={draw=black!60, line width=0.75pt, rounded corners=1pt, align=center},
    Line/.style={->, line width=1.0pt},
    WallText/.style={anchor=west, text=RedLine},
    WallLine/.style={->, line width=1.0pt, color=RedLine},
    SeqPlot/.style={color=BlueLine, line width=1.0pt, mark=square},
    RandPlot/.style={color=RedLine, line width=1.0pt, mark=triangle}
  }
  \begin{axis}[
    ...
    xticklabels={4 KB, 16 KB, 64 KB, 256 KB, 1 MB},
    ymin=0, ymax=12,
    grid=major,
    legend pos=north west,
    legend style={draw=none, fill=none}
  ]
    \addplot[SeqPlot] coordinates {
      (4, 2) (16, 6) (64, 10) (256, 10.5) (1024, 10.8)
    };
    \addlegendentry{Sequential Read (ML Training)}
    \addplot[RandPlot] coordinates {
      (4, 0.2) (16, 0.5) (64, 1.2) (256, 2.8) (1024, 4.5)
    };
    \addlegendentry{Random Read (Database)}
    \node[WallText] at (axis cs: 64, 6) {The "I/O Wall"};
    \draw[WallLine] (axis cs: 64, 5.8) -- (axis cs: 64, 1.5);
  \end{axis}
\end{tikzpicture}
```
:::
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=stealth]
  ...
  \tikzset{
    ...
    HBM/.style={Layer, fill=BlueL, draw=BlueLine, trapezium stretches body},
    DRAM/.style={Layer, fill=GreenL, draw=GreenLine, trapezium stretches body},
    NVMe/.style={Layer, fill=OrangeL, draw=OrangeLine, trapezium stretches body},
    DFS/.style={Layer, fill=RedL, draw=RedLine, trapezium stretches body},
    Object/.style={Layer, fill=VioletL2, draw=VioletLine, trapezium stretches body}
  }

  % Pyramid Layers using nodes
  % HBM is the top, so we can make it a triangle by using regular polygon or just a trapezium with very small top width.
  % Actually, trapezium with top width 0 is a triangle. But TikZ trapezium doesn't easily allow top width 0.
  % We can just use trapezium stretches body and set minimum widths.
  \node[HBM, minimum width=1.6cm, trapezium left angle=60, trapezium right angle=120] (hbm) {\textbf{GPU HBM}};
  \node[DRAM, minimum width=3.2cm, below=0pt of hbm] (dram) {Host DRAM};
  \node[NVMe, minimum width=4.8cm, below=0pt of dram] (nvme) {Local NVMe SSD};
  \node[DFS, minimum width=6.4cm, below=0pt of nvme] (dfs) {Parallel File System};
  \node[Object, minimum width=8.0cm, below=0pt of dfs] (obj) {Object Storage};

  % Annotations
  \draw[Line] ([xshift=-0.5cm]obj.south west) -- ([xshift=-0.5cm]hbm.north west) node[midway, left, rotate=90] {Bandwidth};
  \draw[Line] ([xshift=0.5cm]hbm.north east) -- ([xshift=0.5cm]obj.south east) node[midway, right, rotate=90] {Capacity};
\end{tikzpicture}
```
:::
...
# │
# │ Imports: mlsys.constants (PCIE_GEN5_BW, NVLINK_H100_BW, GB, second)
...
# │          shard_GB / drive_BW.
# │
# │ Imports: mlsys.constants (NVME_SEQUENTIAL_BW, GB, second, BILLION, THOUSAND,
# │          MILLION)
# │ Exports: NVMeTierCalcs.raw_bw_str, .hdd_slowdown_factor,
# │          .ckpt_nvme_s_str, .ckpt_pfs_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import NVME_SEQUENTIAL_BW, GB, second, BILLION, THOUSAND, MILLION
from mlsys.formatting import check
...
2.  **HDD Reality**: A 7200 RPM HDD delivers 100 random IOPS. For 1,000 images/s, you need **`{python} NVMeTierCalcs.hdd_slowdown_factor`$\times$ more IOPS** than the disk provides.
...
1. **Remote Only**: `{python} LocalCacheRoi.lc_compute_ms_str` / (`{python} LocalCacheRoi.lc_compute_ms_str` + `{python} LocalCacheRoi.lc_remote_io_ms_str`) $\approx$ **`{python} LocalCacheRoi.lc_util_remote_str`%**.
...
**The Systems Insight**: Local storage is a **GPU Utilization Multiplier**. By adding a \$500 NVMe drive to a \$30,000 GPU node, you recover **15% of the GPU's capacity** that was previously wasted on I/O wait. Across a 1,000-GPU cluster, this is equivalent to adding **150 GPUs** for "free." In modern ML infrastructure, local NVMe is not auxiliary; it is the "Physical Buffer" that decouples expensive compute from unpredictable shared storage.
...
# │
# │ Imports: mlsys.constants (BILLION, THOUSAND)
# │          mlsys.formatting (fmt, check, md)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import BILLION, THOUSAND
from mlsys.formatting import check, md
import math
...
**The Systems Conclusion**: Even with pipelining, the accelerator is idle `{python} PipelineCalcs.data_stall_pct_str`% of the time. To eliminate the stall entirely, storage throughput must be reduced below 200 ms, either by using faster storage, deeper prefetch buffers, or more parallel I/O streams.
:::
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=stealth]
  ...
  \tikzset{
    Box/.style={draw=black!60, line width=0.75pt, rounded corners=1pt, minimum width=2cm, minimum height=0.6cm, align=center},
    LoadBox/.style={Box, fill=BlueL, draw=BlueLine},
    TrainBox/.style={Box, fill=GreenL, draw=GreenLine},
    Line/.style={->, line width=1.0pt, dashed, color=RedLine}
  }

  % Load nodes (CPU)
  \node[LoadBox] (loadN) {Load N};
  \node[LoadBox, right=0.2cm of loadN] (loadN1) {Load N+1};
  \node[LoadBox, right=0.2cm of loadN1] (loadN2) {Load N+2};

  % Train nodes (GPU)
  \node[TrainBox, below=0.4cm of loadN1] (trainN) {Train N};
  \node[TrainBox, right=0.2cm of trainN] (trainN1) {Train N+1};
  \node[TrainBox, right=0.2cm of trainN1] (trainN2) {Train N+2};

  % Connections
  \draw[Line] (loadN.south east) -- (trainN.north west);
  \draw[Line] (loadN1.south east) -- (trainN1.north west);
  \draw[Line] (loadN2.south east) -- (trainN2.north west);

  % Labels
  \node[left=0.5cm of loadN] (cpu) {CPU};
  \node at (cpu |- trainN) {GPU};

  \node[anchor=west, font=\footnotesize] at ([yshift=-0.5cm]loadN.south west |- trainN.south) {Time $\longrightarrow$};
\end{tikzpicture}
```
:::
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=stealth]
  \usetikzlibrary{positioning}
  \tikzset{
    Box/.style={draw=black!60, line width=0.75pt, rounded corners=1pt, minimum width=2.2cm, minimum height=0.8cm, align=center},
    NvmeBox/.style={Box, fill=OrangeL, draw=OrangeLine},
    DramBox/.style={Box, fill=GreenL, draw=GreenLine},
    CpuBox/.style={Box, fill=RedL, draw=RedLine},
    GpuBox/.style={Box, fill=BlueL, draw=BlueLine},
    Line/.style={->, line width=1.0pt},
    GdsLine/.style={->, line width=1.0pt, color=BlueLine},
    Label/.style={font=\small\bfseries},
    Ann/.style={font=\scriptsize, color=RedLine},
    GdsAnn/.style={font=\scriptsize, color=BlueLine}
  }

  \begin{scope}
    % Traditional path
    \node[Label] (tradLabel) {Traditional};
    \node[NvmeBox, right=0.5cm of tradLabel] (nvme1) {NVMe};
    \node[DramBox, right=1.3cm of nvme1] (dram1) {Host\\DRAM};
    \node[CpuBox, right=1.3cm of dram1] (cpu1) {CPU\\Copy};
    \node[GpuBox, right=1.3cm of cpu1] (gpu1) {GPU\\HBM};

    \draw[Line] (nvme1) -- node[above, font=\scriptsize] {DMA} (dram1);
    \draw[Line] (dram1) -- node[above, font=\scriptsize] {memcpy} (cpu1);
    \draw[Line] (cpu1) -- node[above, font=\scriptsize] {PCIe} (gpu1);
  \end{scope}

  \begin{scope}[yshift=-2cm]
    % GDS path
    \node[Label] (gdsLabel) at (tradLabel |- 0,0) {GDS};
    \node[NvmeBox] (nvme2) at (nvme1 |- 0,0) {NVMe};
    \node[GpuBox] (gpu2) at (gpu1 |- 0,0) {GPU\\HBM};

    \draw[GdsLine] (nvme2) -- node[above, font=\scriptsize] {Direct DMA (PCIe peer-to-peer)} (gpu2);
  \end{scope}

  % Latency annotations
  \path (nvme1) -- node[below=0.6cm, Ann] {$\sim$\,120\,\textmu s total} (gpu1);
  \path (nvme2) -- node[below=0.2cm, GdsAnn] {$\sim$\,30\,\textmu s total} (gpu2);
\end{tikzpicture}
```
:::
