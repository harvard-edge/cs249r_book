...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ STORAGE HIERARCHY AND PIPELINE ECONOMICS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-data-storage — storage hierarchy, I/O bottlenecks, and TCO
# │
# │ Goal: Quantify the 1000x+ gap between accelerator HBM and shared storage,
# │       derive required pipeline bandwidth, and model checkpoint storm impact.
# │ Show: ~3.35 TB/s HBM vs ~7 GB/s NVMe; ~20% data stall without pipelining;
# │       ~1 TB checkpoint save in <1s via tiered staging.
# │ How: 6NP pipeline eqn; max(T_comp, T_io) for stalls; tiered write latency.
# │
# │ Imports: mlsys.constants (H100_MEM_BW, GPT3_PARAMS, NVME_SEQUENTIAL_BW, ...)
# │ Exports: h100_bw_tbs, nvme_bw, gpt3_params_b, req_bw_imagenet,
# │          data_stall_pct_str, stall_pct_display_math, s3_annual_cost,
# │          ckpt_total_gb, ckpt_nvme_s, ckpt_pfs_s, tier_cost_ratio,
# │          egress_100tb_cost, prob_tail_all_str, fail_rate_pct_str, n_tail_servers_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import (
...
class StorageHierarchyAnalysis:
    ...
    ckpt_weights_gb = (gpt3_params * 2) / BILLION
    ckpt_optimizer_gb = (gpt3_params * 8) / BILLION
    ...
    ckpt_weights_gb_str = f"{ckpt_weights_gb:,.0f}"
    ckpt_optimizer_gb_str = f"{ckpt_optimizer_gb:,.0f}"
    ...
    prefetch_t_io_p99_str = f"{t_io_p99}"
    prefetch_t_compute_str = f"{t_compute}"
    prefetch_min_depth = f"{math.ceil(t_io_p99/t_compute)}"
    prefetch_safe_depth = f"{math.ceil(t_io_p99/t_compute) + 2}"
...
stall_pct_display_math = md(
    ...
    f"\\mathbf{{{StorageHierarchyAnalysis.data_stall_pct_str}\\%}}$$"
)
...
1.  **Raw Bandwidth**: 1,000 images/s$\times$ 150 KB/image = **`{python} StorageHierarchyAnalysis.raw_bw_str` MB/s**.
2.  **HDD Reality**: A 7200 RPM HDD delivers 100 random IOPS. For 1,000 images/s, you need **`{python} StorageHierarchyAnalysis.hdd_slowdown_factor`$\times$ more IOPS** than the disk provides.
...
The cost advantage of object storage is significant: at roughly \$0.02/GB/month, it is 750$\times$ cheaper per byte than HBM and 5$\times$ cheaper than local NVMe. For a 100 TB training dataset, object storage costs roughly \$`{python} StorageHierarchyAnalysis.s3_annual_str` per year, compared to \$`{python} StorageHierarchyAnalysis.nvme_annual_str` for local NVMe. This cost advantage makes object storage the natural home for the organization's training data lake, where all raw data, preprocessed datasets, and archived model artifacts reside permanently.
...
Return to our running example. Training on 256 GPUs with ImageNet-scale images, 200 ms iterations, and 80% target utilization requires **`{python} StorageHierarchyAnalysis.req_bw_imagenet_str` GB/s** of aggregate storage throughput. This bandwidth must be sustained continuously for the duration of training, which may last days or weeks. If at any point the storage system delivers less than `{python} StorageHierarchyAnalysis.req_bw_imagenet_str` GB/s, accelerators begin to idle. For a language model training on tokenized text (where each sample is small but the number of samples is enormous), the required bandwidth is lower per batch but the total data volume over the training run is much larger, shifting the bottleneck from peak bandwidth to sustained throughput over weeks.
...
\index{data stall ratio!GPU utilization}**Scenario**: Your GPU processes a batch in `{python} StorageHierarchyAnalysis.t_comp_stall_str` ms, but storage takes `{python} StorageHierarchyAnalysis.t_io_stall_str` ms to deliver the next batch.
...
**The Systems Conclusion**: Even with pipelining, the accelerator is idle `{python} StorageHierarchyAnalysis.data_stall_pct_str`% of the time. To eliminate the stall entirely, storage throughput must be reduced below 200 ms, either by using faster storage, deeper prefetch buffers, or more parallel I/O streams.
:::
...
The latency reduction from GDS matters most when the training pipeline is already optimized and the remaining bottleneck is the CPU's ability to mediate transfers. In a node with 8 GPUs, each running a data loader with 4 workers, the CPU must manage 32 concurrent I/O streams. At `{python} StorageHierarchyAnalysis.gds_trad_us_str` μs per transfer, the CPU spends significant time in interrupt handling and buffer management. GDS offloads this work to hardware DMA engines, freeing CPU cores for data augmentation and other preprocessing tasks.
...
**Traditional path**: Each image requires a DMA from NVMe to DRAM, a `memcpy` from kernel to user space, and a PCIe transfer to GPU. At 64,000 images/second with `{python} StorageHierarchyAnalysis.gds_trad_us_str` μs of CPU time per image, the CPU spends 7.68 seconds of CPU time per wall-clock second, consuming roughly 8 cores worth of processing just for data movement.

**GDS path**: Each image is DMA'd directly from NVMe to GPU. At `{python} StorageHierarchyAnalysis.gds_bypass_us_str` μs of CPU time per image (for initiating the DMA), the CPU spends 1.92 seconds of CPU time per wall-clock second, freeing 6 cores for data augmentation.
...
*   **GPU Direct Storage eliminates CPU bottlenecks**: GDS bypasses the CPU in the data path, reducing per-transfer latency by `{python} StorageHierarchyAnalysis.gds_speedup_str`$\times$ and freeing CPU cores for augmentation.
...
*   **Economics drive tiering**: The `{python} StorageHierarchyAnalysis.tier_cost_ratio_str`$\times$ cost difference between HBM and archive storage mandates a tiering strategy. Egress fees often exceed storage fees; design for total cost of data delivery, not storage cost alone.
:::
