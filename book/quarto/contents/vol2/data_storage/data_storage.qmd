...
class HbmMemoryBudget:
    ...
    # Percentages divide by the branded "80 GB" figure the table displays,
    # so a reader's arithmetic on the visible numbers checks out (2026-06-07:
    # dividing by the physical 80 GiB rendered 65.9-79.9% against rows that
    # visibly sum to 70.8-85.8% of the displayed capacity).
    hbm_capacity_branded = hbm_capacity * GB / GiB  # GiB magnitude under the branded decimal-GB label
    total_min_pct = (total_min / hbm_capacity_branded).to("").magnitude * 100
    total_max_pct = (total_max / hbm_capacity_branded).to("").magnitude * 100
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    hbm_capacity_gb_str = fmt_memory_capacity(hbm_capacity, unit=GiB)
...
class BuildVsBuyStorageEconomics:
    ...
    breakeven_range_str = fmt_range(breakeven_low_years, breakeven_high_years, unit="years", precision=1)
```
...
A `{python} CheckpointModelIntro.gpt3_params_b_str` parameter model with Adam optimizer\index{Adam Optimizer!state size} generates checkpoints of approximately `{python} CheckpointModelIntro.ckpt_total_gb_str`. The checkpoint includes `{python} CheckpointModelIntro.weights_fp16_gb_str` of FP16 model weights, `{python} CheckpointModelIntro.optimizer_state_gb_str` of FP32 optimizer state for momentum and variance, (the mixed-precision footprint convention used throughout this book: FP32 master weights are not separately persisted), learning rate scheduler state, random number generator state, and the current data loader position. Every GPU in the cluster saves its shard of the checkpoint simultaneously, creating a checkpoint storm that the storage system must absorb without disrupting ongoing training reads.
...
class CheckpointStormDefinition:
    ...
    # 2026-06-07: gradients dropped from the WRITE total — they are consumed
    # by the optimizer step and are not part of the durable checkpoint.
    zero3_total = weights + optimizer_state
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────────────────────
    check(abs(zero3_total.to(GB).magnitude - 980) < 1e-6, "70B checkpoint write should be 980 GB: FP16 weights + FP32 master + moments, no gradients")
...
1.  **Significance (quantitative)**: The storm magnitude scales as $T_{\text{write}} = R \times \text{per-replica state} / \text{BW}_{\text{fabric}}$, where $R$ is the number of replicas writing at once. For a `{python} CheckpointStormDefinition.params_b_str`-parameter model in FP16 (`{python} CheckpointStormDefinition.weights_gb_str` of weights) trained across `{python} CheckpointStormDefinition.replicas_str` with vanilla data parallelism, every replica holds a complete copy and a naive checkpoint in which each rank persists its FP16 weight copy generates `{python} CheckpointStormDefinition.naive_total_tb_str` of simultaneous writes. At 100 GB/s fabric bandwidth, that write takes roughly `{python} CheckpointStormDefinition.naive_write_s_str`, or over `{python} CheckpointStormDefinition.naive_write_min_str` of training stall per checkpoint event. ZeRO-3 sharding collapses the written state to roughly `{python} CheckpointStormDefinition.zero3_total_tb_str` total (`{python} CheckpointStormDefinition.weights_gb_str` weights + `{python} CheckpointStormDefinition.optimizer_state_gb_str` optimizer state; gradients are consumed by the update and are not persisted, split across `{python} CheckpointStormDefinition.replicas_str`), bringing $T_{\text{write}}$ down to roughly `{python} CheckpointStormDefinition.zero3_write_s_str`. This stall is the checkpoint $T_{\text{write}}$ term and can dwarf the compute time between checkpoints when the sharding optimization is omitted.
