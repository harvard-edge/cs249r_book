...
class StorageEconomics:
    ...
    req_bw_imagenet = (n_gpus * util_target * (img_size_kb / 1024 / 1024)) / iter_time_s # in GB/s
...
## Introduction: The Fuel Line {#sec-storage-fuel-line}
...
::: {.callout-definition title="Definition of I/O Wall"}

The **I/O Wall** is the storage-compute bandwidth mismatch that occurs when accelerator consumption rates exceed the sustainable throughput of the storage subsystem. In a modern training cluster, GPU HBM consumes data at `{python} h100_bw_tbs` TB/s per device, while the fastest local storage delivers single-digit GB/s. The I/O Wall manifests as *accelerator starvation*: idle compute cycles caused not by algorithmic limitations but by the physics of moving data from where it is stored to where it is needed. Unlike the classic "memory wall" (which describes the latency gap between processor and DRAM), the I/O Wall spans multiple tiers of the storage hierarchy and requires a combination of caching, prefetching, and format optimization to bridge.
:::
...
[^fn-iops]: **IOPS (Input/Output Operations Per Second)**: A storage performance metric counting discrete read or write operations. ML training data pipelines care about throughput (GB/s), not IOPS, because they read large batches sequentially.
...
::: {.callout-notebook title="Napkin Math: Text vs. Image Bandwidth"}
...
\index{NVMe!ML storage tier}When the working set exceeds host DRAM, the system falls to Tier 2: the local NVMe drives attached directly to the compute node. NVMe[^fn-nvme] provides a high-performance protocol designed specifically for solid-state drives, achieving `{python} nvme_bw` GB/s of sequential throughput per drive. With four drives in a RAID-0 configuration, a single node can sustain 14 GB/s of sequential reads, sufficient to stream a 3 TB dataset from local disk in under four minutes.

[^fn-nvme]: **NVMe (Non-Volatile Memory Express)**: A storage protocol designed specifically for SSDs, connecting directly to the CPU via PCIe lanes. NVMe eliminates the legacy AHCI command queue bottleneck, supporting 64K queues with 64K commands each, compared to AHCI's single queue of 32 commands. This parallelism is what enables multi-GB/s throughput.
...
1.  **Raw Bandwidth**: 1,000 images/s $\times$ 150 KB/image = **`{python} ImageNetBottleneck.raw_bw_str` MB/s**.
...
\index{Parallel File System!ML infrastructure}Beyond the single node, the workload requires a shared namespace where all workers can access the same datasets and where durable checkpoints are globally visible. This is the role of the **Parallel File System (PFS)**.[^fn-pfs]

[^fn-pfs]: **PFS (Parallel File System)**: The term encompasses a family of distributed file systems designed for high-performance computing, including Lustre, GPFS (IBM Spectrum Scale), BeeGFS, and WekaFS. The defining characteristic is that a single client can read from multiple storage servers simultaneously, aggregating their bandwidth into a single file read.

::: {.callout-definition title="Definition of Parallel File System"}

A **Parallel File System (PFS)** is a distributed storage architecture that stripes data across many storage servers to provide aggregate throughput exceeding what any single server could deliver. In ML infrastructure, parallel file systems like Lustre [@schwan2003lustre] and GPFS [@schmuck2002gpfs] provide the shared namespace where training data, checkpoints, and model artifacts are visible to every compute node in the cluster. The key distinction from network-attached storage (NAS) is that PFS clients read from *multiple servers simultaneously*, aggregating bandwidth in proportion to the number of servers.
:::

The architecture of a parallel file system separates two concerns that traditional file systems handle together. **Metadata Servers (MDS)** manage the namespace: file creation, directory listings, permission checks, and lock management. **Object Storage Servers (OSS)**[^fn-oss] manage the actual data blocks, each serving a stripe of every large file. When a client opens a 10 GB training shard, the MDS tells the client which OSS nodes hold which stripes, and the client reads from all of them in parallel. A Lustre[^fn-lustre] deployment with 100 OSS nodes, each providing 10 GB/s, delivers an aggregate 1 TB/s.
...
[^fn-oss]: **OSS (Object Storage Server)**: In the context of parallel file systems, an OSS is a server that manages one or more Object Storage Targets (OSTs), which are the physical storage devices holding file data. This usage of "object" predates and differs from cloud object storage (S3, GCS); in PFS terminology, an "object" is simply a chunk of file data managed by a target.

[^fn-lustre]: **Lustre**: A parallel file system originally developed at Carnegie Mellon University and released in 2003. The name is a portmanteau of "Linux" and "cluster." Lustre dominates HPC and ML infrastructure because its architecture naturally scales bandwidth with the number of OSS nodes. The world's largest Lustre deployments exceed 500 PB.

The separation between metadata and data paths is what enables the aggregate bandwidth that ML workloads demand. In GPFS[^fn-gpfs] (IBM Spectrum Scale), the architecture takes a slightly different approach: rather than separating MDS and OSS roles, GPFS distributes metadata across all nodes using a token-based distributed lock manager. Each node can serve both metadata and data, reducing the single-point-of-contention risk of a dedicated MDS. The tradeoff is greater complexity in the lock management protocol, which must ensure consistency across thousands of nodes.

[^fn-gpfs]: **GPFS (General Parallel File System)**: Developed by IBM Research in the late 1990s and now marketed as IBM Spectrum Scale. GPFS pioneered the use of distributed locking for parallel file access, enabling both large-scale scientific computing and, more recently, ML training workloads. Its token-based lock manager provides strong consistency guarantees that are essential for checkpoint reliability.
...
::: {.callout-definition title="Definition of the Small File Problem"}

**The small file problem** is a pathological I/O pattern where millions of individually small files (e.g., one JPEG per training sample) overwhelm the metadata server of a parallel file system, reducing effective throughput to a fraction of the theoretical bandwidth. The root cause is that each file requires its own metadata operations (`open()`, `stat()`, `close()`), and the MDS serializes these operations. The fix is to pack small files into large sequential containers such as TFRecord, WebDataset tar archives, or Parquet files, amortizing a single metadata operation across thousands of samples.
:::
...
Aggregation complements striping by reducing the number of files that the MDS must track. Small samples are bundled into large sequential files (such as WebDataset[^fn-webdataset] tar archives or TFRecord sequences) to amortize metadata costs across thousands of samples. A single `open()` on a 4 GB tar file gives access to 40,000 samples, reducing metadata load by 40,000 $\times$ compared to individual files.

[^fn-webdataset]: **WebDataset**: An open-source data format and loading library that stores training samples as standard POSIX tar archives. Each sample is a set of files (image, label, metadata) stored sequentially in the archive. The key insight is that tar archives can be read sequentially without random seeks, making them ideal for streaming from both parallel file systems and object storage. WebDataset archives are also valid HTTP byte-range targets, enabling direct streaming from cloud storage without local staging.
...
Object storage achieves "eleven nines"[^fn-nines] of durability (99.999999999%) through **erasure coding**[^fn-erasure] and geographic replication, making it the only tier where data loss is essentially impossible.

[^fn-erasure]: **Erasure Coding**: The theoretical foundation of modern erasure codes is Reed-Solomon coding, developed by Irving Reed and Gustave Solomon in 1960. Reed-Solomon codes are used not only in distributed storage but also in QR codes, CDs, DVDs, and deep-space communication. The mathematical elegance of the approach is that it provides the maximum possible fault tolerance for a given storage overhead, a property known as MDS (Maximum Distance Separable).

[^fn-nines]: **Eleven Nines Durability (99.999999999%)**: Statistically, for one million objects stored for ten million years, you would expect to lose a single object. This durability justifies the higher latency of object storage for archival data.

::: {.callout-definition title="Definition of Erasure Coding"}

**Erasure coding** is a data protection scheme that splits an object into $k$ data fragments and $m$ parity fragments (an "$k+m$" code), such that any $k$ of the $k+m$ fragments are sufficient to reconstruct the original object. For example, a 4+2 code stores 6 fragments; if any 2 are lost (due to disk failure, rack failure, or data center outage), the original data is fully recoverable. Unlike triple replication (which stores 3 copies at 3 $\times$ cost), erasure coding achieves comparable durability at only 1.5 $\times$ storage overhead. The tradeoff is higher CPU cost during reads (to decode fragments) and higher latency for small objects.
:::
...
$$\text{Stall \%} = \frac{250 - 200}{250} = \mathbf{`{python} data_stall_pct_str`\%}$$
...
```{python}
#| label: fig-data-stall-frontier
#| echo: false
#| fig-cap: "**The Data Stall Frontier**. GPU utilization follows a steep S-curve as a function of storage bandwidth. Below the stall threshold (~10 GB/s for a typical 8-GPU node), accelerators spend more time waiting for data than computing. The position of common storage technologies on this curve explains why tiered architectures are essential."
#| fig-alt: "S-curve plot showing GPU utilization versus storage bandwidth with labeled storage technologies from HDD at 0.5 GB/s to NVMe RAID at 25 GB/s, with a stall threshold at 10 GB/s."
...
If I/O at the 99th percentile takes `{python} PrefetchBuffer.t_io_p99_ms` ms and compute takes `{python} PrefetchBuffer.t_compute_ms` ms, then $D_{min} = $ `{python} prefetch_min_depth` batches, with a safety margin of `{python} prefetch_safe_depth`. In practice, data loaders like PyTorch's `DataLoader` use `prefetch_factor` and `num_workers` parameters to control this depth. Setting `prefetch_factor=2` with 4 workers creates a buffer of 8 batches, which is typically sufficient for NVMe-backed pipelines but may be inadequate for object-storage-backed pipelines where P99 latency can exceed 500 ms.
...
The traditional data path for loading training data follows three hops: storage $\to$ host DRAM $\to$ GPU HBM. Data is first read from NVMe into a kernel buffer in host DRAM (via a DMA transfer initiated by the NVMe controller), then copied to a user-space buffer (the data loader's tensor, via a `memcpy` that the CPU executes), and finally transferred to GPU memory via the PCIe bus (using a `cudaMemcpy` or `cudaMemcpyAsync` call that programs the GPU's DMA engine). Each hop adds latency and consumes CPU resources. The CPU must orchestrate every transfer, manage buffer allocation, and handle interrupts from both the storage device and the GPU. For a single transfer, this overhead is negligible. But when eight GPUs each demand thousands of small transfers per second, the aggregate CPU load for data movement alone can saturate multiple cores, leaving insufficient CPU capacity for the data augmentation that is also essential to the pipeline.[^fn-gds]

[^fn-gds]: **GDS (GPU Direct Storage)**: Part of NVIDIA's GPUDirect technology family, which also includes GPUDirect RDMA (allowing network adapters to write directly to GPU memory) and GPUDirect Peer-to-Peer (allowing GPUs to access each other's memory). GDS extends this philosophy to storage devices, using the cuFile API to manage DMA transfers from NVMe controllers to GPU memory without CPU intervention.

::: {.callout-definition title="Definition of GPU Direct Storage (GDS)"}

**GPU Direct Storage (GDS)** is a technology that enables a direct DMA path between NVMe storage devices and GPU memory, bypassing the CPU and host DRAM entirely. By eliminating the intermediate copy through system memory and the associated CPU overhead, GDS reduces data loading latency from roughly `{python} gds_trad_us` μs (CPU-mediated) to `{python} gds_bypass_us` μs (DMA bypass), a `{python} gds_speedup` $\times$ improvement. GDS is part of NVIDIA's GPUDirect family of technologies and requires both hardware support (a compatible NVMe controller and GPU) and software support (the cuFile API).
:::
...
::: {.callout-notebook title="Napkin Math: The Egress Tax"}
...
A `{python} gpt3_params_b`B parameter model with Adam optimizer generates checkpoints of approximately `{python} ckpt_total_gb` GB. The checkpoint includes model weights (`{python} CheckpointStorage.weights_gb_str` GB in FP16), optimizer state (momentum and variance, `{python} CheckpointStorage.optimizer_gb_str` GB in FP32), learning rate scheduler state, random number generator state, and the current data loader position. Every GPU in the cluster saves its shard of the checkpoint simultaneously, creating a checkpoint storm that the storage system must absorb without disrupting ongoing training reads.

::: {.callout-definition title="Definition of Checkpoint Storm"}

A **checkpoint storm** is a burst of synchronized write traffic that occurs when all nodes in a training cluster save model state simultaneously, saturating storage bandwidth and stalling training. In a 10,000-GPU cluster saving 175B-parameter checkpoints, a single checkpoint event can generate tens of terabytes of write traffic in seconds. Checkpoint storms are the dominant write-side stress event for parallel file systems and must be absorbed without degrading the concurrent read traffic that feeds the training pipeline.
:::

::: {.callout-notebook title="Napkin Math: The Checkpoint Storm"}
...
::: {.callout-notebook title="Napkin Math: The 175B Model's Storage Footprint"}
...
This chapter has detailed the physical foundation of the storage hierarchy, from the fastest accelerator memory to the slowest archive tier. But the *demand* placed on this hierarchy is dictated almost entirely by the distributed training strategy chosen for the model. Data parallelism replicates the model on every node, creating uniform read patterns but massive checkpoint redundancy as every node saves the same parameters. Pipeline parallelism creates strict sequential dependencies between stages, where a data loading stall in an early stage cascades down the pipeline, starving all downstream stages. Expert parallelism, as seen in Mixture of Experts (MoE) models, creates highly non-uniform access patterns where different accelerator groups require different subsets of the data at any given time. Each strategy imposes a unique signature on the storage system. Understanding and co-designing the interaction between parallelism strategy and storage hierarchy is a cornerstone of efficient ML systems design, and it is the central theme of @sec-distributed-training-systems.
