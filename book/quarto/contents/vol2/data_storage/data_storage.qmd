...
**Systems insight**: The pipeline must either use NVMe (~`{python} StorageHierarchyTable.nvme_latency_us_str` random access, three orders of magnitude faster than object storage) or convert the dataset to a sequential format (TFRecord/WebDataset) to achieve sequential throughput.
...
2.  **Observation**: Unlike Network Attached Storage (NAS), where every I/O request routes through a single server, a PFS client receives stripe location metadata from a dedicated Metadata Server (MDS) and then reads data directly from multiple OSS nodes in parallel: the MDS and OSS paths are architecturally separated, so data bandwidth scales with OSS count while metadata operations scale with MDS count.
3.  **Pitfall**: A frequent misconception is that a PFS has unlimited throughput if enough OSS nodes are added. In reality, a Lustre MDS handles roughly 100,000–300,000 metadata operations per second; at 10,000 workers each opening one small file, the MDS saturates in under 1 second and becomes the serialization point that idles the entire cluster regardless of how many OSS nodes are present.
...
2.  **Observation**: Unlike bulk data throughput (which measures bit-rate), the small file problem is a metadata latency $(L_{\text{lat}})$ issue: the bottleneck is the frequency of requests, not the size of the data.
3.  **Pitfall**: A frequent misconception is that this is "fixed" by buying faster SSDs. In reality, it is a Format Problem\index{Format Problem}: the fix is to pack samples into large sequential containers (for example, TFRecord\index{TFRecord}, Parquet) to Amortize metadata operations across thousands of samples.
...
2.  **Observation**: Unlike replication (which stores complete copies), erasure coding uses mathematical encoding (for example, Reed-Solomon) to distribute redundant information across different failure domains (disks, racks, or sites).
3.  **Pitfall**: A frequent misconception is that erasure coding is "free durability." In reality, it is a latency-compute trade-off\index{Latency!compute trade-off}: reconstructing data after a failure requires additional CPU cycles and increases the tail latency $(L_{\text{lat}})$ of the storage read.
...
class PipelineCalcs:
    ...
    tstep_seq_display_math = MarkdownStr(
        f"$$T_{{\\text{{step}}}} = T_{{\\text{{I/O}}}} + T_{{\\text{{compute}}}} = " f"{_io_ms:.0f} + {_comp_ms:.0f} = {_seq_ms:.0f} \\text{{ ms}}$$"
    )
    stall_seq_display_math = MarkdownStr(f"$$\\text{{Stall \\%}} = \\frac{{{_io_ms:.0f}}}{{{_seq_ms:.0f}}} = " f"{stall_seq_val:.1f}\\%$$")
    tstep_pipe_display_math = MarkdownStr(f"$$T_{{\\text{{step}}}} = \\max({_comp_ms:.0f}, {_io_ms:.0f}) = {_pipe_ms:.0f} \\text{{ ms}}$$")
```
...
2.  **Observation**: Unlike Traditional I/O\index{Traditional I/O}, where every byte must be processed by the CPU and stored in kernel buffers, GDS provides Direct Memory Access between the storage controller and the accelerator.
3.  **Pitfall**: A frequent misconception is that GDS makes all storage "faster." In reality, it only accelerates Local or RDMA-attached NVMe; it does not eliminate the physical latency of network-attached file systems or object storage.
...
class StorageIcebergCosts:
    ...
    egress_rate_gb_str = fmt_usd(egress_rate.to(USD / GB).magnitude, precision=2, commas=False, suffix="/GB")
```
...
class StreamVsStageEconomics:
    ...
    s3_rate_gb_mo_str = fmt_usd((s3_rate * (1 * GB) * (1 * ureg.month)).to(USD).magnitude, precision=3, commas=False, suffix="/GB/month")
    nvme_rate_gb_mo_str = fmt_usd((nvme_rate * (1 * GB) * (1 * ureg.month)).to(USD).magnitude, precision=2, commas=False, suffix="/GB/month")
    egress_rate_gb_str = fmt_usd(egress_rate.to(USD / GB).magnitude, precision=2, commas=False, suffix="/GB")
...
2. If each node writes to local NVMe at `{python} CheckpointStorageDesignPrompt.local_nvme_bw_gb_per_s_str`, what is $T_{\text{write}}$ for the local write?
3. If the parallel file system provides `{python} CheckpointStorageDesignPrompt.pfs_aggregate_tb_per_s_str` aggregate and all `{python} CheckpointStorageDesignPrompt.nodes_str` write simultaneously, what is the per-node write bandwidth, and how long does the async copy take?
