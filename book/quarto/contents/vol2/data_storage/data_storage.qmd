...
[^fn-raid-storage]: **RAID (Redundant Array of Independent Disks)**\index{RAID!ML storage}: A 1988 Berkeley taxonomy of drive-combining strategies, each trading redundancy against bandwidth [@patterson1988raid]. ML training inverts the database-era default: RAID 0 (striping, no parity) maximizes sequential read throughput at the cost of zero fault tolerance, a safe trade-off[^fn-raid0-immutable] because training data is immutable and durably backed in object storage. Choosing RAID 5 or 6 instead wastes bandwidth on parity calculations that protect data already protected elsewhere.
...
**Systems insight**: Even with a large number of shards, the birthday-problem approximation $1 - e^{-n^2/(2k)}$ yields a `{python} ShardContention.sc_collision_prob_str` chance of storage shard contention\index{Shard Contention!collision probability} with $n = 32$ workers and $k = 1{,}000$ shards. Such a collision creates a "hot spot" where multiple workers land on the same storage server in the same epoch. In a distributed fleet, the resulting tail latency\index{Tail Latency} can leave the cluster waiting for the GPUs that share a disk. Production data loaders reduce this effect through global shuffling and deterministic shard assignment that spread workers across the storage fabric.
...
```
The second inversion is **working set size**\index{Working Set Size}. Traditional applications exhibit temporal locality\index{Locality!temporal}: a web server repeatedly accesses popular pages, and a cache holding the top 10 percent of content serves 90 percent of requests. ML training datasets are accessed uniformly. Our `{python} StorageAccessPatternFacts.tokenized_text_tb_str` serialized corpus has no "popular" tokens; each is consumed once per epoch. A cache of any practical size holds only a tiny fraction of the dataset, and most samples are effectively "cold" when accessed. This lack of temporal locality makes traditional caching strategies much less effective. An LRU[^fn-lru-ml-failure] (Least Recently Used) cache can approach a 0 percent hit rate for a cyclic scan larger than the cache. With independently reshuffled epochs, cross-epoch hits are low but nonzero and depend on the cache fraction and sample order.

[^fn-lru-ml-failure]: **LRU (Least Recently Used)**\index{LRU!ML failure mode}: LRU is effective when recently accessed data is likely to be accessed again soon. A cyclic full-dataset scan larger than the cache can evict each sample before its next access and drive the hit rate toward zero. Independent reshuffling changes the boundary behavior because some samples retained from the end of one epoch may be reused before eviction in the next, so the hit rate depends on the cache-to-dataset ratio and ordering rather than being universally zero. Teams should measure this reuse before allocating DRAM that could otherwise serve as prefetch buffers.
...
**Problem**: How much aggregate storage bandwidth does a `{python} TextImageBandwidth.n_gpus_str`-GPU cluster require for text training versus image training?
...
4. **Collate**: The collation stage assembles individual samples into batches and places them in **pinned memory**\index{Pinned Memory!DMA transfer} for efficient DMA transfer to the accelerator.
...
Some augmentation pipelines have moved from CPU to GPU execution, using libraries like NVIDIA Data Loading Library (DALI)\index{DALI} to perform image decoding and augmentation on the accelerator itself. This approach eliminates the CPU augmentation bottleneck and reduces the host DRAM bandwidth demand, because compressed data (smaller) is transferred to the GPU instead of decoded data (larger). The trade-off is that augmentation on the GPU consumes HBM capacity and compute cycles that would otherwise be available for training. For *compute-bound* workloads (where the GPU is already saturated with matrix multiplications), GPU-based augmentation is counterproductive. For *I/O-bound* workloads (where the GPU waits for data), it can improve overall throughput by shifting work from the bottleneck (CPU) to the resource with spare capacity (GPU).
...
The architecture of a Lustre-style parallel file system separates two concerns that traditional file systems handle together [@schwan2003lustre]. **Metadata Servers (MDS)**\index{Metadata Server}\index{MDS} manage the namespace: file creation, directory listings, permission checks, and lock management. **Object Storage Servers (OSS)**\index{Object Storage Server}\index{OSS}[^fn-oss-disambiguation] manage the actual data blocks, each serving a stripe of every large file. When a client opens a 10 GB training shard, the MDS tells the client which OSS nodes hold which stripes, and the client reads from all of them in parallel. A Lustre[^fn-lustre-origin] deployment with 100 OSS nodes, each providing 10 GB/s, delivers an aggregate 1 TB/s.
...
**Diagnosis**: The file system delivers less than 1 percent of rated throughput. Millions of per-second `stat()` and `open()` calls overwhelm the metadata servers (MDS), choking data loaders despite abundant network and storage bandwidth.
...
**Scenario**: A large GPU cluster achieves low Model FLOPs Utilization (MFU) despite storage metrics reporting ample network and byte bandwidth.

**Diagnosis**: Profiling reveals that PyTorch data loaders issue individual `stat()` calls for millions of files across thousands of workers, overwhelming the Metadata Server (MDS) and leaving GPUs stalled on blocking file-open operations invisible to byte-bandwidth counters.
...
1.  **Significance**: It eliminates the "Bounce Buffer" through system memory. In the small-transfer model detailed in \ref{nbk-data-storage-cpu-bypass-dividend}, this reduces per-transfer latency by `{python} GDSLatency.gds_latency_reduction_pct_str` and makes the local transfer path `{python} GDSLatency.gds_speedup_mult_str` faster. It allows the GPU to saturate the NVMe link speed (for example, 7 GB/s) while reducing CPU utilization for I/O.
...
4. **Result**: Total footprint = `{python} SyntheticStorage.data_size_tb_str` $\times$ `{python} SyntheticStorage.footprint_factor_str` $\times$ `{python} SyntheticStorage.verification_passes_str` = `{python} SyntheticStorage.total_tb_str`.
