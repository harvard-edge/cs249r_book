...
::: {.column-margin}
![](images/svg/data_storage_dai_locator.svg){width="100%" fig-alt="Three side-by-side pills labeled D, A, and I. The I pill is filled blue and the D and A pills are gray, marking the Infrastructure axis as this chapter's focus."}
...
::: {.column-margin}
![](images/svg/data_storage_checkpoint_dominance.svg){width="100%" fig-alt="A two-rung ladder on a log scale: a tall blue rung for checkpoint writes at 7.56 PB towering over a tiny rung for per-epoch training data at 6 TB, showing checkpoints move over a thousand times more bytes than the dataset."}
...
class StorageAccessPatternFacts:
    ...
    tokenized_text_tb_str = fmt_int(round(tokenized_text_tb_val), commas=False, suffix=' TB')
    compressed_text_tb_str = fmt(compressed_text_tb_val, precision=0, commas=False, suffix=' TB')
    ckpt_total_gb_str = fmt_int(round(ckpt_total.m_as(GB)), suffix=' GB')
...
::: {.column-margin}
![](images/svg/vol2_data_storage_margin_001.svg){width="100%" fig-alt="Margin illustration showing Text training bandwidth versus image training bandwidth."}
...
class HbmModelBudget:
    ...
    # ┌── 3. GUARD (Invariants) ────────────────────
    check(abs(adam_bytes - bytes_fp32 * adam_states) < 1e-9, "Adam state bytes must equal FP32 * 2 states")
...
class NVMeTierCalcs:
    ...
    imagenet_images_m_str  = fmt(imagenet_images / MILLION, precision=2, commas=False, suffix="M")
...
The architecture of a parallel file system separates two concerns that traditional file systems handle together [@ghemawat2003]. **Metadata Servers (MDS)**\index{Metadata Server}\index{MDS} manage the namespace: file creation, directory listings, permission checks, and lock management. **Object Storage Servers (OSS)**\index{Object Storage Server}\index{OSS}[^fn-oss-disambiguation] manage the actual data blocks, each serving a stripe of every large file. When a client opens a 10 GB training shard, the MDS tells the client which OSS nodes hold which stripes, and the client reads from all of them in parallel. A Lustre[^fn-lustre-origin] deployment with 100 OSS nodes, each providing 10 GB/s, delivers an aggregate 1 TB/s.
...
class PipelineCalcs:
    ...
    data_stall_pct_str  = fmt(data_stall_pct_val, precision=0, commas=False, suffix=" percent")
    t_comp_stall_str    = fmt(t_comp_val, precision=0, commas=False, suffix=" ms")
    t_io_stall_str      = fmt(t_io_val, precision=0, commas=False, suffix=" ms")

    stall_pct_display_math = MarkdownStr(
        f"$$\\text{{Stall \\%}} = \\frac{{{t_io_val} - {t_comp_val}}}"
        f"{{{t_io_val}}} = "
        f"\\mathbf{{{data_stall_pct_val:.0f}\\%}}$$"
    )
```
...
::: {.column-margin}
![](images/svg/vol2_data_storage_margin_002.svg){width="100%" fig-alt="Margin illustration showing 500 ms P99 I/O delay spanning three 200 ms compute windows."}
...
class GDSLatency:
    ...
    gds_total_images_per_s_str = fmt(total_images_per_s, precision=0, suffix=" images/s")
    ...
    images_per_s_per_gpu_str = fmt(images_per_s_per_gpu, precision=0, commas=True, suffix=" images/s")
```
...
class EconRatios:
    ...
    cost_nvme_gb_mo = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.m_as("dollar / GB / month")
    cost_s3_gb_mo   = 0.02
    cost_egress_gb  = Infrastructure.Pricing.Cloud.EgressPerGB.rate.m_as("dollar / GB")
    dataset_size_tb = 100
...
class ObjectStorageCostRecap:
    ...
    nvme_annual_str = fmt(100 * THOUSAND * Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.m_as("dollar / GB / month") * 12, precision=0)
```
...
::: {.column-margin}
![](images/svg/vol2_data_storage_margin_003.svg){width="100%" fig-alt="Margin illustration showing Cloud storage cost bar dominated by repeated egress."}
...
class BuildVsBuyStorageEconomics:
    # ┌── 1. LOAD ─────────────────────────────────────────
    from mlsysim.fmt import check, fmt, fmt_usd, MarkdownStr
    capex_low = 3_000_000
    capex_high = 5_000_000
    annual_ops = 500_000
    cloud_cost_gb_month = 0.02
    cloud_egress_gb = Infrastructure.Pricing.Cloud.EgressPerGB.rate.m_as("dollar / GB")
    dataset_pb = 1
    reads_per_year = 10
    ...
    cloud_cost_gb_month_str = fmt_usd(cloud_cost_gb_month, precision=2, commas=False)
...
The cost structure described earlier motivates a **tiering strategy**\index{Tiering Strategy} that places data at the appropriate level of the hierarchy based on access frequency and latency requirements.
...
::: {.column-margin}
![](images/svg/data_storage_checkpoint_storm_write_time.svg){width="100%" fig-alt="Margin ladder comparing a naive checkpoint storm taking about 23.9 minutes with a ZeRO-3 sharded write taking about 11.2 seconds, annotated as a 128 times reduction."}

*Sharding collapses checkpoint-storm write time by two orders of magnitude.*
:::
...
This architectural pattern shifts the primary storage bottleneck from **sequential read bandwidth**\index{Sequential Read Bandwidth} to **random-access IOPS**\index{Random-Access IOPS}. Vector databases must traverse high-dimensional graph indexes to find approximate nearest neighbors across billions of embeddings. The performance is governed by a strict physical constraint: the **Recall vs. Latency vs. Capacity**\index{Recall vs. Latency vs. Capacity} trade-off. Maximizing recall requires traversing a larger portion of the index, driving up IOPS and latency. Disk-backed graph indexes such as DiskANN show how billion-scale search can trade DRAM footprint against SSD-backed graph traversal while targeting high recall and low query latency [@subramanya2019diskann].
...
class SyntheticStorage:
    ...
    total_tb_str = fmt(total_footprint_tb, precision=1, commas=False, suffix=" TB")
    ...
    provenance_overhead_pct_str = fmt(provenance_metadata_overhead * 100, precision=0, commas=False, suffix=" percent")
    footprint_multiplier_str = fmt(1 + provenance_metadata_overhead, precision=1, commas=False)
    data_size_tb_str = fmt(data_size_tb, precision=0, commas=False, suffix=" TB")
...
class EconRatiosRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tier_cost_ratio_str = fmt(Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.m_as("dollar / GB / month") / 0.02, precision=0, commas=False)
```
