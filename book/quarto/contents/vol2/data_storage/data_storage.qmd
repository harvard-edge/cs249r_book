...
class StorageFuelLineContext:
    ...
    training_tokens = Scenarios.StorageTrainingCorpus.TrainingTokens
    compressed_text = Scenarios.StorageTrainingCorpus.CompressedSource
    token_id_bytes = Scenarios.StorageTrainingCorpus.TokenIdBytes
    training_window = Scenarios.StorageTrainingCorpus.TrainingWindow
    checkpoint_interval = Scenarios.StorageTrainingCorpus.CheckpointInterval
    bytes_per_param_ckpt = Scenarios.StorageTrainingCorpus.CheckpointBytesPerParameter
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models, Scenarios
...
class CompleteDataPathCheckpoint:
    ...
    checkpoint_interval = Scenarios.StorageTrainingCorpus.CheckpointInterval
    tokenized_text = training_tokens * token_id_bytes
    ckpt_total = gpt3_params * Scenarios.StorageTrainingCorpus.CheckpointBytesPerParameter
    checkpoint_count_val = int((training_window / checkpoint_interval).to("").magnitude)
    ckpt_fleet_total = ckpt_total * checkpoint_count_val
    check(checkpoint_count_val == 4320, "30 days at 10-min checkpoint interval should be 4320")
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
