...
<!-- TODO(numerical-audit): Several quantitative claims in this chapter
     conflict with adjacent tables or each other and need a focused
     numerical-audit pass to reconcile:

     - Line ~442: HBM 300,000-fold bandwidth gap claim vs. ~33.5-fold from
       adjacent table values (HBM 3.35 TB/s and object 100 GB/s aggregate).
     - Line ~448: Storage pyramid caption tier values disagree with
       tbl-storage-hierarchy-merged (host DRAM 200 vs. 300 GB/s, archive
       cost \$0.004 vs. \$0.001 per GB, etc.).
     - Line ~912: Archive "20-fold cheaper" claim vs. stated prices
       (\$0.02 over \$0.004 is 5-fold, not 20-fold).
     - Line ~918: Two-year archive cost stated as \$960 but math yields
       \$9,600 for 100 TB at \$0.004 per GB-month over 24 months.
     - Line ~1474: Callout \$13,000-per-day idle cost vs. preceding
       \$9,600-per-day calculation for the same 20-point utilization drop
       on a 1,000-GPU cluster at \$2 per GPU-hour.
     - Line ~1572: ZeRO-3 70B per-node shard math (140 GB-per-node times
       1,000 nodes equals 140 TB of writes) is inconsistent with the
       FP16 70B total of 140 GB.
     - Line ~1580: 175B 256-node 4 GB shard vs. 1,750/256 = 6.8 GB implied.

     Each requires careful derivation against the chapter's running
     examples and storage table; recompute and standardize before next
     release. -->
...
The I/O wall is not static: it is widening. Between 2018 and 2025, advertised accelerator Tensor Core throughput grew sharply, but exact ratios depend on whether the comparison holds precision fixed or follows each generation's lowest supported training/inference precision. Over the same period, NVMe sequential bandwidth grew far more slowly, from roughly 3.5 GB/s to 7 GB/s per drive. The compute-to-storage bandwidth ratio has therefore worsened substantially. If this trend continues, the storage hierarchy must add new tiers (persistent memory, CXL-attached storage) or fundamentally change the data pipeline architecture (compute-near-storage, in-storage processing) to prevent the I/O wall from becoming the dominant constraint on training throughput. The history of computing suggests that the gap will continue to widen, because the economic incentives favor investing in faster compute (which directly reduces training time) over faster storage (which only indirectly improves utilization).
...
**Systems conclusion**: Even with pipelining, the accelerator is idle `{python} PipelineCalcs.data_stall_pct_str` percent of the time. To eliminate the stall entirely, storage throughput must be reduced below 200 ms, either by using faster storage, deeper prefetch buffers, or more parallel I/O streams.
...
**Systems conclusion**: Local NVMe caching saves \$282,000/year, enough to fund 3 additional H100 GPUs. The break-even point is 2 epochs: reading the data more than twice makes local caching pay for itself.
...
1.  **Significance (quantitative)**: The storm magnitude scales as $T_{\text{write}} = N \times \text{per-node-shard} / \text{BW}_{\text{fabric}}$. For a 70B-parameter model trained across 1,000 nodes with ZeRO-3 sharding (approximately 140 GB per node at FP16), a checkpoint generates 140 TB of simultaneous writes; at 100 GB/s fabric bandwidth, $T_{\text{write}} \approx 1{,}400$ seconds—over 23 minutes of training stall per checkpoint event. This adds directly to the $L_{\text{lat}}$ term and can dwarf the compute time between checkpoints.
...
2. If each node writes to local NVMe at 14 GB/s, what is $T_{\text{write}}$ for the local write?
