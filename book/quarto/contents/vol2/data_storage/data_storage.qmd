...
class GlacierCost:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    glacier_annual_str = fmt_usd(glacier_annual_cost, precision=0)
```
...
class ObjectStorageCostRecap:
    ...
    s3_annual_cost = dataset_size * (0.02 * USD / GB / ureg.month) * 12 * ureg.month
    nvme_annual_cost = dataset_size.to(TB) * Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate * 12 * ureg.month
    s3_annual_str = fmt_usd(s3_annual_cost, precision=0)
    nvme_annual_str = fmt_usd(nvme_annual_cost, precision=0)
```
...
class GlacierCostRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    dataset_size = 100 * THOUSAND * GB
    glacier_annual_cost = dataset_size * (0.004 * USD / GB / ureg.month) * 12 * ureg.month
    glacier_annual_str = fmt_usd(glacier_annual_cost, precision=0)
```
...
class EconRatiosRecap:
    ...
    tier_cost_ratio_str = fmt_multiple(
