...
class StorageFootprintTable:
    ...
    tokenized_text_tb_str = fmt_int(round(tokenized_text_tb_val), commas=False, suffix=" TB")
    ckpt_fleet_total_pb_str = fmt(ckpt_fleet_total_pb_val, precision=1, suffix=" PB")
    archive_lineage_tb_str = fmt(archive_lineage_tb_val, precision=1, commas=False, approx=True, suffix=" TB")
...
**Total data moved through the hierarchy**\index{Storage!total data movement}: approximately `{python} StorageFootprintTable.ckpt_fleet_total_pb_str` of checkpoint data plus `{python} StorageFootprintTable.tokenized_text_tb_str` $\times$ `{python} StorageFootprintTable.epochs_str` epochs of training data reads. For a single-epoch language model training run, checkpoint I/O dominates data loading I/O by a factor of over `{python} StorageFootprintTable.ckpt_io_dominance_factor_str`.
