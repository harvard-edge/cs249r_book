...
class DRAMInterconnect:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    pcie5_bw_gbs_str  = fmt_bandwidth(pcie5_bw, unit=GB / second, precision=0, commas=False)
...
class HashValidationCost:
    ...
    cpu_core_hours_str = fmt(core_hours, precision=1)
```
...
[^fn-hash-cost]: **Hash Cost (SHA-256)**: Verifying the integrity of a `{python} HashValidationCost.dataset_tb_str` dataset using SHA-256 consumes approximately `{python} HashValidationCost.cpu_core_hours_str` CPU-core hours under a pipeline-level throughput assumption of `{python} HashValidationCost.throughput_mb_s_str` per core. For a 30-day foundation model training run, this metadata validation step must be pipelined or performed during ingestion to avoid stalling the fleet's startup phase. \index{Cryptographic Hash!validation cost}
...
# │
# │ Imports: mlsysim.core.constants (microsecond, second); mlsysim.Systems
...
from mlsysim.core.constants import KB, microsecond, second
...
class GDSLatency:
    ...
    gds_speedup_str   = fmt_multiple(gds_speedup_val, precision=0, commas=False)
...
class ObjectStorageCostRecap:
    ...
    s3_annual_cost = (
        dataset_size
        * Infrastructure.Pricing.Storage.S3StandardLowPerTbMonth.rate
        * 12
        * ureg.month
    ).to(USD)
    nvme_annual_cost = (
...
class GlacierCostRecap:
    ...
    glacier_annual_cost = (
        dataset_size
        * Infrastructure.Pricing.Storage.GlacierStandardPerTbMonth.rate
        * 12
        * ureg.month
    ).to(USD)
    glacier_annual_usd_str = fmt_usd(glacier_annual_cost, precision=0)
...
The storage hierarchy so far has optimized the training fuel line: large shards, sequential reads, and checkpoint bursts. Retrieval-augmented inference adds a storage workload that belongs in the same chapter because it reverses the access pattern again. Instead of streaming sequential blobs via formats like TFRecord or Parquet, retrieval systems traverse high-dimensional vector graphs representing semantic embeddings.
...
As the fleet consumes the sum of human-generated text, it encounters the **Data Wall**\index{Data Wall}: the point where new high-quality training samples can only be generated by the fleet itself. This shift from *collecting* data to *synthesizing* data sends more than payload bytes through the hierarchy. A "synthetic fuel line" must also store the **provenance chain**\index{Provenance Chain} of every sample, so tiering and metadata design now carry the burden of preventing "model collapse\index{Model Collapse}" (where a model degrades by training on its own unverified errors).
