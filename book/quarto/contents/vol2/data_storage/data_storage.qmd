...
Accelerators can compute faster than storage can feed them. A high-end GPU processes data at terabytes per second internally, but individual Non-Volatile Memory Express (NVMe) drives deliver gigabytes to low tens of gigabytes per second, and distributed storage systems add latency that compounds into idle accelerators waiting for data to arrive. This mismatch is invisible in benchmarks that measure accelerator performance in isolation but dominates real workloads where training data must stream continuously, checkpoints must be saved reliably, and model weights must be loaded at serving time. The gap between what accelerators can *consume* and what storage can *deliver* shapes system architecture at every level: it forces careful attention to data formats, caching strategies, and pipeline design that would be unnecessary if storage kept pace with compute. Organizations that optimize accelerator utilization without addressing storage discover that their expensive hardware runs at a fraction of capacity because nobody planned for the data path. Data storage is a Compute-Communication co-design problem: the fastest Compute in the Fleet sits idle when the storage hierarchy cannot supply data at the rate the accelerators consume it.
...
- Explain how GPUDirect Storage bypasses the CPU to reduce data loading latency for accelerator-bound workloads
...
# │ Exports: StorageFuelLineContext.h100_bw_tb_per_s_str, .nvme_bw_gb_per_s_str,
# │          .ckpt_total_gb_str, .gpt3_params_b_str, .bw_ratio_mult_str,
...
class StorageFuelLineContext:
    ...
    bytes_per_param_ckpt = ReferenceStats.StorageTrainingCorpus.CheckpointBytesPerParameter
...
The previous two chapters built the engine and wired it together. @Sec-compute-infrastructure established that dense accelerator nodes can pack eight GPUs delivering petaFLOP/s of aggregate compute, and @sec-network-fabrics showed how InfiniBand fabrics connect thousands of such nodes at hundreds of gigabits per second. Within the fleet stack shown in @fig-fleet-stack, data storage\index{Data Storage} completes that physical foundation by providing the fuel supply: training data, model weights, optimizer state, and intermediate checkpoints staged at the right distance from the accelerator. An engine without fuel is expensive sculpture. This chapter asks *how* to deliver that fuel fast enough that 1,000 accelerators never starve.
...
A database administrator moving to an ML infrastructure team would find that ML storage workloads\index{Storage!workload inversion} invert nearly every storage design principle they relied on. To see *why*, return to our 175B model training on 1.5 trillion tokens. Each training epoch reads every token, in shuffled order, exactly once. The next epoch shuffles again and reads them all once more. There is no "hot data" in the traditional sense and no 80/20 rule where a small fraction of data accounts for most accesses. The standard storage optimizations fail precisely because they assume the opposite.
...
[^fn-raid-storage]: **RAID (Redundant Array of Independent Disks)**\index{RAID!ML storage}: A 1988 Berkeley taxonomy of drive-combining strategies, each trading redundancy against bandwidth. ML training inverts the database-era default: RAID 0 (striping, no parity) maximizes sequential read throughput at the cost of zero fault tolerance, a safe trade-off[^fn-raid0-immutable] because training data is immutable and durably backed in object storage. Choosing RAID 5 or 6 instead wastes 15–25 percent of bandwidth on parity calculations that protect data already protected elsewhere.

[^fn-raid0-immutable]: **RAID 0 Risk**\index{RAID 0!risk}\index{RAID 0!data safety}: RAID 0 provides no redundancy; if one drive in a 4-drive array fails, all data on that node's cache is lost. This is acceptable for immutable training data (re-stage from the parallel file system) or model weights (re-load from object storage), but it is a catastrophic choice for the model's only copy of un-checkpointed gradients or local logs.
...
Between 2016 and 2024, advertised accelerator Tensor Core throughput grew sharply, but exact ratios depend on whether the comparison holds precision fixed or follows each generation's lowest supported training/inference precision. Over the same period, NVMe sequential bandwidth grew far more slowly, from roughly 3.5 GB/s to 14 GB/s per drive. The compute-to-storage bandwidth ratio has therefore worsened substantially. If that pattern continues, the storage hierarchy must add new tiers (persistent memory, CXL-attached storage) or fundamentally change the data pipeline architecture (compute-near-storage, in-storage processing) to prevent the I/O wall from becoming the binding constraint on training throughput. The durable planning lesson is that compute improvements do not automatically carry the storage path with them.
...
ax1.semilogy(years_gpu, gpu_tflops, "o-", color=COLORS["BlueLine"], markersize=8, linewidth=2, label="GPU FP16 TFLOP/s", zorder=5)
...
ax2 = ax1.twinx()
ax2.semilogy(years_storage, storage_gbs, "s-", color=COLORS["OrangeLine"], markersize=8, linewidth=2, label="NVMe Seq. Read (GB/s)", zorder=5)
...
for i, lbl in enumerate(gpu_labels):
    ax1.annotate(lbl, (years_gpu[i], gpu_tflops[i]), textcoords="offset points", xytext=gpu_offsets[i], fontsize=8, color=COLORS["BlueLine"])
...
class StorageAccessPatternFacts:
    ...
    token_id_bytes = ReferenceStats.StorageTrainingCorpus.TokenIdBytes
    tokenized_text = training_tokens * token_id_bytes
    ckpt_total = gpt3_params * ReferenceStats.StorageTrainingCorpus.CheckpointBytesPerParameter
...
- [ ] **Database heuristics**: Based on the five inversions described earlier, which storage optimization strategies from the database world would be counterproductive for this workload? Name at least three.
- [ ] **Epoch reads**: If each training epoch reads the full 10 TB dataset once and the cluster trains for 100 epochs, what is the total data volume read? How does this compare to a typical database workload?
- [ ] **Checkpoint bandwidth**: The model saves a 500 GB checkpoint every 15 minutes. If checkpoint saves must complete within 30 seconds to minimize training disruption, what minimum write bandwidth is required?
- [ ] **Write duty cycle**: Given the write pattern (bursty checkpoints every 15 minutes), what percentage of time is the storage system in "write mode" vs. "read mode"?
...
# │ Exports: StorageHierarchyTable.h100_bw_tb_per_s_str,
# │          StorageHierarchyTable.nvme_latency_us_str,
# │          StorageHierarchyTable.host_dram_bw_gb_per_s_str
...
|:-------------------------|:--------------------:|:----------------------------------------------------------:|:-----------------------------------------------------:|:----------------:|
| **GPU HBM**              |        80 GB         |   `{python} StorageHierarchyTable.h100_bw_tb_per_s_str`    |                        ~100 ns                        |      ~15.00      |
| **Host DRAM**            |     512 GB–2 TB      | `{python} StorageHierarchyTable.host_dram_bw_gb_per_s_str` |                        ~100 ns                        |      ~3.00       |
| **Local NVMe SSD**       |       4–30 TB        |                         7–25 GB/s                          | ~`{python} StorageHierarchyTable.nvme_latency_us_str` |      ~0.10       |
...
# │          StoragePyramidContext.hbm_host_dram_ratio_mult_str,
# │          StoragePyramidContext.h100_bw_tb_per_s_str,
# │          StoragePyramidContext.host_dram_bw_gb_per_s_str,
# │          StoragePyramidContext.raid0_bw_gb_per_s_str,
# │          StoragePyramidContext.dram_nvme_ratio_mult_str
from mlsysim import Hardware
...
# │
# │ Imports: mlsysim.fmt (MarkdownStr, fmt, fmt_count, check)
# │ Exports: TextImageBandwidth.text_bw_display_math,
# │          TextImageBandwidth.image_bw_display_math,
# │          TextImageBandwidth.text_bw_mb_per_s_str,
# │          TextImageBandwidth.image_bw_gb_per_s_str,
# │          TextImageBandwidth.ratio_rounded_mult_str,
# │          TextImageBandwidth.image_size_kb_str
# └─────────────────────────────────────────────────────────────────────────────
...
::: {.column-margin}
![](images/svg/vol2_data_storage_margin_001.svg){width="100%" fig-alt="Text training bandwidth versus image training bandwidth."}
...
# │
# │ Imports: mlsysim (Models), mlsysim.core.units (BYTES_FP16, BYTES_FP32,
...
class HbmMemoryBudget:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    hbm_capacity_gb_str = fmt_memory_capacity(hbm_capacity, precision=0, commas=False)
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.interconnect.bandwidth, Hardware.Cloud.H100.nvlink.bandwidth, GB, second);
# │          mlsysim.fmt (fmt_bandwidth)
# │ Exports: DRAMInterconnect.pcie5_bw_gb_per_s_str, DRAMInterconnect.nvlink_bw_gb_per_s_str,
# │          DRAMInterconnect.pcie5_bidir_bw_gb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.units (Hardware.Tech.Storage.NvmeGen4.bandwidth, GB, second, BILLION, THOUSAND,
# │          MILLION)
# │ Exports: NVMeTierCalcs.raw_bw_mb_per_s_str, .hdd_slowdown_factor_mult_str,
# │          .hdd_util_ceiling_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
class NvmeTierContext:
    ...
    metadata_reduction_val = (training_tokens / tokenized_shard_count_val).to(count).magnitude
...
# │ Exports: ImageNetNvmeLatency.nvme_latency_us_str
from mlsysim import Hardware
...
# │          lc_local_io_ms_str, lc_util_gain_str, lc_cluster_gpus_str, lc_recovered_gpus_str
# └─────────────────────────────────────────────────────────────────────────────
...
class LocalCacheRoi:
    ...
    check(util_gain > 0, "Local caching must improve utilization")
    # ┌── 4. OUTPUT ────────────────────────────────────────
...
Beyond the single node, the workload requires a shared namespace where all workers can access the same datasets and where durable checkpoints are globally visible. This is the role of the parallel file system (PFS)\index{Parallel File System!ML infrastructure}.[^fn-pfs-architecture]
...
***Small File Problem***\index{Small File Problem!definition} is an ML data-loading pathology where millions of individually small files overwhelm the metadata server of a storage system.
...
- [ ] **OSS count**: If each Lustre OSS delivers 10 GB/s, how many OSS nodes are required?
- [ ] **Metadata cost**: If the dataset consists of 200 million images at 150 KB each, and each `open()` call takes 50 μs on the MDS, how long would it take to open all files sequentially? What is the practical implication?
- [ ] **Shard count**: If images are bundled into 4 GB tar files, how many metadata operations are needed to read the full dataset?
...
Object storage targets "eleven nines"[^fn-eleven-nines-durability] of durability (99.999999999 percent) through erasure coding[^fn-reed-solomon-erasure] and geographic replication. That target makes object storage the natural source-of-truth tier for training data and checkpoint archives, while still requiring versioning, lifecycle policy, access control, and recovery testing for operational failures outside the storage service's durability model.

[^fn-reed-solomon-erasure]: **Erasure Coding**: Built on Reed-Solomon codes\index{Erasure Coding!Reed-Solomon} [@reed1960], which use maximum-distance-separable coding to recover the original data from any sufficient subset of encoded fragments. For ML storage, the trade-off is concrete: a 4+2 erasure code provides the same durability as triple replication at 1.5$\times$ storage cost instead of 3$\times$, but reconstruction after a disk failure adds 10–50 ms of tail latency to reads, a penalty that compounds at scale when hundreds of data loader workers hit degraded stripes simultaneously.

[^fn-eleven-nines-durability]: **Eleven Nines (99.999999999 Percent Durability)**\index{Durability!eleven nines}: A design target of 11 nines annual durability corresponds to an expected loss probability of about $10^{-11}$ per object-year; for one million objects, that is about one expected object loss per 100,000 years. This extreme durability is why object storage is the only tier where training data and checkpoint archives can be treated as permanent, but the erasure coding and geographic replication that achieve it add 50–100 ms of read latency, making object storage unsuitable for real-time data pipeline access without prefetch buffering.
...
# │
# │ Imports: mlsysim.core.units (Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate)
# │ Exports: ObjectStorageCost.s3_annual_usd_str, .nvme_annual_usd_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim units and fmt_usd
# │ Exports: GlacierCost.glacier_annual_usd_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.units (BILLION, THOUSAND)
# │          mlsysim.fmt (fmt, check)
# │ Exports: PipelineCalcs.req_bw_imagenet_gb_per_s_str, PipelineCalcs.data_stall_pct_str,
# │          PipelineCalcs.t_comp_stall_str, PipelineCalcs.t_io_stall_str,
# │          PipelineCalcs.stall_pct_display_math,
# │          PipelineCalcs.tstep_seq_display_math, PipelineCalcs.stall_seq_display_math,
# │          PipelineCalcs.tstep_pipe_display_math
# └─────────────────────────────────────────────────────────────────────────────
...
class PipelineCalcs:
    ...
    stall_pct_display_math = fmt_display_math(
        f"\\text{{Stall \\%}} = \\frac{{{_io_ms:.0f} - {_comp_ms:.0f}}}" f"{{{_io_ms:.0f}}} = " f"\\mathbf{{{data_stall_pct_val:.0f}\\%}}"
    )
...
# │ Exports: PrefetchDepth.compute_ms_str, .batch_mb_str, .gpus_per_node_str,
# │          .nvme_p99_ms_str, .pfs_p99_ms_str, .obj_p99_ms_str,
# │          .nvme_depth_math, .pfs_depth_math, .obj_depth_str,
# │          .obj_safety_str, .obj_buf_gpu_mb_str, .obj_buf_node_gb_str,
# │          .min_buf_node_mb_str
import math
...
- [ ] **Checkpoint contention**: If the parallel file system delivers 800 GB/s aggregate, is it sufficient? What if 20 percent of bandwidth is consumed by checkpoint writes?
- [ ] **Prefetch depth**: The object storage backend has P99 latency of 300 ms. What minimum prefetch buffer depth (in batches) would absorb this variance?
- [ ] **Host memory**: Each decoded batch occupies 10 MB in host DRAM. What is the total prefetch buffer memory for the depth calculated in (3)?
...
# │ Exports: WorkerAllocation.gpus_per_node_str, .workers_str, .cpu_cores_str,
# │          .remaining_cores_str
from mlsysim import Systems
...
class WorkerAllocation:
    ...
    # ┌── 3. GUARD ────────────────────────────────────────
    check(remaining_cores > 0, "Worker count must leave cores for the runtime")
    # ┌── 4. OUTPUT ───────────────────────────────────────
    gpus_per_node_str = fmt(gpus_per_node, precision=0)
...
# │          HashValidationCost.throughput_mb_per_s_str,
# │          HashValidationCost.cpu_core_hours_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.units (microsecond, second); mlsysim.Systems
# │ Exports: GDSLatency.gds_trad_us_str, .gds_bypass_us_str,
# │          .gds_speedup_mult_str, .gds_latency_reduction_pct_str,
...
## GPUDirect Storage and the CPU Bypass {#sec-storage-gds}
...
# │
# │ Imports: mlsysim.core.units (Infrastructure.Pricing.Cloud.EgressPerGB.rate, MILLION,
# │          Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate)
# │ Exports: EconRatios.tier_cost_ratio_mult_str, EconRatios.egress_100tb_usd_str
# └─────────────────────────────────────────────────────────────────────────────
class EconRatios:
    # ┌── 1. LOAD ─────────────────────────────────────────
    cost_nvme_gb_mo = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.to(USD / (GB * ureg.month)).magnitude
...
# │ Exports: ObjectStorageCostRecap.s3_annual_usd_str, ObjectStorageCostRecap.nvme_annual_usd_str
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: @sec-storage-economics tier cost list
# │ Exports: GlacierCostRecap.glacier_annual_usd_str
...
# │ Exports: StorageIcebergCosts.egress_10epoch_usd_str, .get_request_usd_str,
# │          .storage_monthly_usd_str, .glacier_retrieval_usd_str,
# │          .glacier_egress_usd_str, .file_count_str, .epochs_str,
# │          .egress_rate_gb_str
from mlsysim import Infrastructure
from mlsysim.fmt import fmt, fmt_usd, fmt_count, check
...
# │ Exports: StorageCostExtremes.hbm_storage_usd_str, idle_daily_usd_str,
# │          idle_loss_daily_usd_str, idle_loss_run_usd_str
# └─────────────────────────────────────────────────────────────────────────────
...
- **Egress fees**\index{Egress Fee}: A representative cross-boundary egress charge of `{python} StorageIcebergCosts.egress_rate_gb_str` makes a 100 TB dataset read once per epoch across `{python} StorageIcebergCosts.epochs_str` epochs cost `{python} StorageIcebergCosts.egress_10epoch_usd_str` in egress alone.
...
- **Idle accelerator cost**\index{Idle Accelerator Cost}: If storage stalls reduce GPU utilization from 90 percent to 70 percent on a 1,000-GPU cluster at \$2/GPU/hour, the lost compute costs `{python} StorageCostExtremes.idle_loss_daily_usd_str`/day (20 percent of `{python} StorageCostExtremes.idle_daily_usd_str` daily spend), dwarfing any storage savings.
- **Retrieval fees**\index{Retrieval Fee}: Archive storage charges retrieval fees (\$0.02/GB for Glacier) in addition to egress. Restoring a 100 TB dataset from Glacier costs `{python} StorageIcebergCosts.glacier_retrieval_usd_str` in retrieval fees plus `{python} StorageIcebergCosts.glacier_egress_usd_str` in egress.
...
# │
# │ Imports: mlsysim registries (Infrastructure.Pricing) + fmt helpers
# │ Exports: StreamVsStageEconomics.* (_str exports listed in OUTPUT)
# └─────────────────────────────────────────────────────────────────────────────
...
# │ Exports: BuildVsBuyStorageEconomics.annual_ops_usd_str,
# │          BuildVsBuyStorageEconomics.cloud_cost_gb_month_str,
# │          BuildVsBuyStorageEconomics.cloud_storage_annual_usd_str,
# │          BuildVsBuyStorageEconomics.reads_per_year_str,
# │          BuildVsBuyStorageEconomics.cloud_egress_annual_usd_str,
# │          BuildVsBuyStorageEconomics.cloud_total_annual_usd_str,
...
The cost structure described earlier turns tiering into a placement policy: keep data only as close to the accelerator as its access frequency and latency requirement justify. A **tiering strategy**\index{Tiering Strategy} encodes that policy across the hierarchy.
...
# │ Exports: PfsTieringSavings.savings_usd_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_usd
...
While the six-tier hierarchy represents a common production baseline, memory and storage technologies can fill the bandwidth gaps between existing tiers, reshaping effective storage architecture for large-scale ML.
...
# │ Exports: CheckpointModelIntro.gpt3_params_b_str,
# │          CheckpointModelIntro.ckpt_total_gb_str,
# │          CheckpointModelIntro.weights_fp16_gb_str,
# │          CheckpointModelIntro.optimizer_state_gb_str
from mlsysim import Models, ReferenceStats
from mlsysim.core.units import BYTES_ADAM_STATE, BYTES_FP16, GB
...
class CheckpointModelIntro:
    ...
    optimizer_state = gpt3_params * BYTES_ADAM_STATE
    ckpt_total = gpt3_params * ReferenceStats.StorageTrainingCorpus.CheckpointBytesPerParameter
    check(weights_fp16 + optimizer_state == ckpt_total, "Checkpoint component bytes must sum to total.")
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpt3_params_b_str = fmt_params(gpt3_params, scale="B", precision=0, commas=False)
    ckpt_total_gb_str = fmt_memory(ckpt_total, unit=GB, precision=0, commas=True)
...
class CheckpointStormDefinition:
    ...
    gradients = memory_from_params(params, bytes_fp16 / param)
    master_weights = memory_from_params(params, bytes_fp32 / param)
    adam_moments = memory_from_params(params, 2 * bytes_fp32 / param)
    optimizer_state = master_weights + adam_moments
    per_replica_state = weights + gradients + optimizer_state
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────────────────────
    check(zero3_total.to(GB).magnitude == 1120, "70B sharded training-state total should be 1,120 GB with master weights")
    ...
    optimizer_state_gb_str = fmt_memory(optimizer_state, unit=GB, precision=0, commas=False)
...
1.  **Significance (quantitative)**: The storm magnitude scales as $T_{\text{write}} = R \times \text{per-replica state} / \text{BW}_{\text{fabric}}$, where $R$ is the number of replicas writing at once. For a `{python} CheckpointStormDefinition.params_b_str`-parameter model, each replica holds `{python} CheckpointStormDefinition.weights_gb_str` FP16 weights, `{python} CheckpointStormDefinition.gradients_gb_str` gradients, and `{python} CheckpointStormDefinition.optimizer_state_gb_str` optimizer state. With vanilla data parallelism across `{python} CheckpointStormDefinition.replicas_str`, a naive checkpoint of that full per-replica state generates `{python} CheckpointStormDefinition.naive_total_tb_str` of simultaneous writes. At 100 GB/s fabric bandwidth, that write takes roughly `{python} CheckpointStormDefinition.naive_write_s_str`, or over `{python} CheckpointStormDefinition.naive_write_min_str` of training stall per checkpoint event. ZeRO-3, an optimizer-state-sharding scheme in which each node stores only its $1/N$ slice of the state rather than a full replica, collapses the written state to roughly `{python} CheckpointStormDefinition.zero3_total_tb_str` total (`{python} CheckpointStormDefinition.weights_gb_str` weights + `{python} CheckpointStormDefinition.gradients_gb_str` gradients + `{python} CheckpointStormDefinition.optimizer_state_gb_str` optimizer state, split across `{python} CheckpointStormDefinition.replicas_str`), bringing $T_{\text{write}}$ down to roughly `{python} CheckpointStormDefinition.zero3_write_s_str`. This stall is the checkpoint $T_{\text{write}}$ term and can dwarf the compute time between checkpoints when the sharding optimization is omitted.
...
class CheckpointStormCalc:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    total_checkpoint = Models.Language.GPT3.parameters * (10 * (byte / param))
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────────────────────
    check(0.20 < local_write.to(second).magnitude < 0.30, "Local checkpoint pause should be about 0.25 s")
    check(1.7 < pfs_copy.to(second).magnitude < 1.8, "PFS async copy should be about 1.75 s")
...
class CheckpointTotalGbRecap:
    ckpt_total = Models.Language.GPT3.parameters * (10 * (byte / param))
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ckpt_total_gb_str = fmt_memory(ckpt_total, unit=GB, precision=0, commas=True)
```
...
- [ ] **Per-node state**: What is the per-node checkpoint size?
- [ ] **Local write**: If each node writes to local NVMe at `{python} CheckpointStorageDesignPrompt.local_nvme_bw_gb_per_s_str`, what is $T_{\text{write}}$ for the local write?
- [ ] **Async copy**: If the parallel file system provides `{python} CheckpointStorageDesignPrompt.pfs_aggregate_tb_per_s_str` aggregate and all `{python} CheckpointStorageDesignPrompt.nodes_str` write simultaneously, what is the per-node write bandwidth, and how long does the async copy take?
- [ ] **Tiered staging**: Why is tiered staging (local NVMe first, then async PFS copy) faster than writing directly to PFS?
...
class SyntheticStorage:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    total_footprint = data_size * (1 + provenance_metadata_overhead) * verification_passes
    amplification = (total_footprint / data_size).to("").magnitude
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(round(amplification, 1) == 4.2, f"Amp {amplification} unexpected")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    amplification_factor_mult_str = fmt_multiple(amplification, precision=1, commas=False)
    total_tb_str = fmt_memory(total_footprint, unit=TB, precision=1, commas=False)
    ...
    def plot(cls):
        ...
        return book_style.bar_compare(
            **{
                ...
                "values": [cls.data_size.to(TB).magnitude, cls.total_footprint.to(TB).magnitude],
...
4. **The amplification**: Total footprint = `{python} SyntheticStorage.data_size_tb_str` $\times$ `{python} SyntheticStorage.footprint_factor_str` $\times$ `{python} SyntheticStorage.verification_passes_str` = `{python} SyntheticStorage.total_tb_str`.

**Systems insight**: Synthetic data is *verified data*\index{Synthetic Data!verified data}, and verification has a `{python} SyntheticStorage.amplification_factor_mult_str` storage tax. In the Machine Learning Fleet, storage moves from being a simple bit-bucket to a *provenance engine*. A system that cannot store the *why* and *who* behind a synthetic token risks poisoning the future of the fleet with its own past mistakes.
...
# │ Exports: GDSLatencyRecap.gds_speedup_mult_str
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: @sec-storage-summary takeaways
# │ Exports: EconRatiosRecap.tier_cost_ratio_mult_str
...
* **Economics drive tiering**: The `{python} EconRatiosRecap.tier_cost_ratio_mult_str` cost difference between local NVMe and object storage (and the >3,000$\times$ span from HBM to archive) mandates a tiering strategy. Egress fees often exceed storage fees; design for total cost of data delivery, not storage cost alone.
