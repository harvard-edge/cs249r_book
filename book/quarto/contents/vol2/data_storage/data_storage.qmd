...
# │ Exports: (none)
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, MarkdownStr, fmt, fmt_qty, fmt_qty_int, check
```
...
from mlsysim.fmt import MarkdownStr, fmt, fmt_int, check, fmt_qty
...
class StorageFuelLineContext:
    ...
    token_id_bytes = BYTES_INT32.to(byte).magnitude
    training_days = 30
    ...
    ckpt_total_gb_val = (gpt3_params.to(param).magnitude * bytes_per_param_ckpt) / BILLION
    checkpoint_count_val = training_days * 24 * 60 // checkpoint_interval_min
    ckpt_fleet_total_pb_val = (ckpt_total_gb_val * checkpoint_count_val * GB).to(PB).magnitude
    tokenized_text_tb_val = (training_tokens * token_id_bytes * byte).to(TB).magnitude
    ...
    nvme_bw_str = fmt_qty(nvme_bw_raw, GB/second, precision=0, commas=False)
    bw_ratio_str = fmt(bw_ratio_val, precision=1)
    compressed_text_tb_str = fmt_qty(compressed_text_tb_val * TB, TB, precision=0, commas=False)
    tokenized_text_tb_str = fmt_qty_int(round(tokenized_text_tb_val) * TB, TB, commas=False)
    baseline_training_tb_str = fmt_qty_int(round(baseline_training_tb_val) * TB, TB, commas=False)
    ckpt_total_gb_str = fmt_qty(ckpt_total_gb_val * GB, GB, precision=0, commas=True)
    ckpt_fleet_total_pb_str = fmt_qty(ckpt_fleet_total_pb_val * PB, PB, precision=1, commas=True)
```
...
from mlsysim.fmt import fmt, check, fmt_qty, fmt_qty_int, fmt_int, fmt_time
...
class StorageAccessPatternFacts:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpt3_params_b_str = fmt(gpt3_params.to(param).magnitude / BILLION, precision=0, commas=False)
    tokenized_text_tb_str = fmt_qty_int(round(tokenized_text_tb_val) * TB, TB, commas=False)
    compressed_text_tb_str = fmt_qty(compressed_text_tb_val * TB, TB, precision=0, commas=False)
...
class StorageHierarchyTable:
    # ┌── 4. OUTPUT ───────────────────────────────────────
    h100_bw_tbs_str = fmt_qty(Hardware.Cloud.H100.memory.bandwidth, TB/second, precision=2, commas=False)
```
...
class StoragePyramidContext:
    ...
    compressed_text_tb_val = 3
    token_id_bytes = BYTES_INT32.to(byte).magnitude
    host_dram_bw_gbs_val = Hardware.Cloud.H100.memory.bandwidth.to(GB/second).magnitude
    h100_bw = Hardware.Cloud.H100.memory.bandwidth
    tokenized_text_tb_val = (training_tokens * token_id_bytes * byte).to(TB).magnitude
    hbm_host_dram_ratio_val = h100_bw.to(GB/second).magnitude / host_dram_bw_gbs_val
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    compressed_text_tb_str = fmt_qty(compressed_text_tb_val * TB, TB, precision=0, commas=False)
    tokenized_text_tb_str = fmt_qty_int(round(tokenized_text_tb_val) * TB, TB, commas=False)
    h100_bw_tbs_str = fmt_qty(h100_bw, TB/second, precision=2, commas=False)
    host_dram_bw_gbs_str = fmt_qty(host_dram_bw_gbs_val * GB / second, GB / second, precision=0, commas=False)
...
class TextImageBandwidth:
    ...
    token_bytes = BYTES_INT32.to(byte).magnitude
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    text_bw_mb_str = fmt_qty(text_bw_mb_s * MB / second, MB / second, precision=1, commas=False)
    image_bw_gb_str = fmt_qty(image_bw_gb_s * GB / second, GB / second, precision=1, commas=False)
...
class HbmTierBandwidth:
    ...
    hbm_host_dram_ratio_str = fmt_multiple(hbm_host_dram_ratio_val.to("").magnitude, precision=1, commas=False)
```
```{python}
#| echo: false
...
class HbmModelBudget:
    ...
    adam_bytes = BYTES_ADAM_STATE.to(byte).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────
    weights_fp16_gb_str = fmt_qty(weights_fp16_gb * GB, GB, precision=0, commas=True)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import MarkdownStr, check, fmt_percent_range, fmt_qty, fmt_qty_range
...
class HbmMemoryBudget:
    ...
    comm_min_gb = 2
    comm_max_gb = 4
    hbm_capacity_gb = Hardware.Cloud.H100.memory.capacity.to(GB).magnitude

    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    full_weight_gb = (params * BYTES_FP16.to(byte).magnitude * byte).to(GB).magnitude
    full_optimizer_gb = (params * 2 * BYTES_FP32.to(byte).magnitude * byte).to(GB).magnitude
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    weight_per_gpu_str = fmt_qty(weight_per_gpu_gb * GB, GB, precision=2, commas=False)
    optimizer_per_gpu_str = fmt_qty(optimizer_per_gpu_gb * GB, GB, precision=1, commas=False)
    gradient_per_gpu_str = fmt_qty(gradient_per_gpu_gb * GB, GB, precision=1, commas=False)
...
class DRAMInterconnect:
    pcie5_val  = Hardware.Cloud.H100.interconnect.bandwidth.to(GB/second).magnitude
    nvlink_val = Hardware.Cloud.H100.nvlink.bandwidth.to(GB/second).magnitude

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    pcie5_bw_gbs_str  = fmt(pcie5_val, precision=0, commas=False)
    nvlink_bw_gbs_str = fmt_qty(nvlink_val * GB / second, GB / second, precision=0, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_count, fmt_time, fmt_rate
...
class NVMeTierCalcs:
    ...
    ckpt_total_gb_val = (Models.Language.GPT3.parameters.to(param).magnitude * 10) / BILLION
    n_nodes           = 256
    ...
    ckpt_nvme_s_val  = node_shard_gb / (4 * nvme_bw_raw.to(GB/second).magnitude)
    ckpt_pfs_s_val   = node_shard_gb/pfs_node_bw.to(GB/second).magnitude
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    raw_bw_str             = fmt_qty(raw_bw_val * MB / second, MB / second, precision=0, commas=False)
    images_per_sec_str     = fmt_rate(images_per_sec, 'images/s', precision=0, commas=False)
    img_size_kb_str        = fmt_qty((img_size / THOUSAND) * KB, KB, precision=0, commas=False)
    imagenet_images_m_str  = fmt_count(imagenet_images, scale='M', precision=2, commas=False)
    imagenet_avg_kb_str    = fmt_qty((img_size / THOUSAND) * KB, KB, precision=0, commas=False)
...
class NvmeTierContext:
    ...
    local_nvme_capacity_tb_val = (LOCAL_NVME_DRIVES_PER_NODE_REF * nvme_drive_capacity).to(TB).magnitude
    ...
    multimodal_images_tb_str = fmt_qty(multimodal_images_tb * TB, TB, precision=0, commas=False)
    multimodal_text_tb_str = fmt_qty(multimodal_text_tb * TB, TB, precision=0, commas=False)
    multimodal_audio_tb_str = fmt_qty(multimodal_audio_tb * TB, TB, precision=0, commas=False)
    multimodal_total_tb_str = fmt_qty(multimodal_total_tb_val * TB, TB, precision=0, commas=False)
    multimodal_excess_tb_str = fmt_qty(multimodal_excess_tb_val * TB, TB, precision=1, commas=False)
    tokenized_shard_count_str = fmt(tokenized_shard_count_val, precision=0)
    tokenized_shard_gb_str = fmt_qty(tokenized_shard_gb_val * GB, GB, precision=0, commas=False)
    gpt3_params_b_str = fmt(gpt3_params.to(param).magnitude / BILLION, precision=0, commas=False)
    ckpt_total_gb_str = fmt_qty_int(ckpt_total, GB, commas=False)
```
...
class ObjectStreamContext:
    ...
    tokenized_shard_count_val = round(tokenized_text_tb_val * THOUSAND / tokenized_shard_gb_val)
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    text_bw_mb_str = fmt_qty((2048 * 4096 * token_id_bytes / 0.2 / MILLION) * MB / second, MB / second, precision=1, commas=False)
    tokenized_shard_count_str = fmt(tokenized_shard_count_val, precision=0)
    tokenized_shard_gb_str = fmt_qty(tokenized_shard_gb_val * GB, GB, precision=0, commas=False)
```
...
class PipelineCalcs:
    ...
    # ┌── 2. EXECUTE ──────────────────────────────────────
    t_step_s             = (t_step_ms * ms).to(second).magnitude
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    req_bw_imagenet_str = fmt_qty(req_bw_imagenet_gbs * GB / second, GB / second, precision=1, commas=False)
...
class NodeStagingContext:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tokenized_text_tb_str = fmt_qty_int(round(tokenized_text_tb_val) * TB, TB, commas=False)
    node_tokenized_gb_str = fmt_qty(node_tokenized_gb_val * GB, GB, precision=1, commas=False)
...
class HashValidationCost:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    dataset_tb_str = fmt_qty(dataset_tb * TB, TB, precision=0, commas=False)
    throughput_mb_s_str = fmt_qty(throughput_mb_s_per_core * MB / second, MB / second, precision=1, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int,  check, fmt, fmt_time, fmt_rate
...
class GDSLatency:
    ...
    images_per_s_per_gpu_str = fmt_rate(images_per_s_per_gpu, 'images/s', precision=0, commas=True)
```
...
class CompleteDataPathCheckpoint:
    ...
    checkpoint_interval_min = 10
    tokenized_text_tb_val = (training_tokens * token_id_bytes * byte).to(TB).magnitude
    ckpt_total_gb_val = (gpt3_params.to(param).magnitude * 10) / BILLION
    checkpoint_count_val = training_days * 24 * 60 // checkpoint_interval_min
    ckpt_fleet_total_pb_val = (ckpt_total_gb_val * checkpoint_count_val * GB).to(PB).magnitude
    check(checkpoint_count_val == 4320, "30 days at 10-min checkpoint interval should be 4320")
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
...
class EconRatios:
    ...
    cost_nvme_gb_mo = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.to(USD / GB / ureg.month).magnitude
    cost_s3_gb_mo   = 0.02
    cost_egress_gb  = Infrastructure.Pricing.Cloud.EgressPerGB.rate.to(USD / GB).magnitude
    dataset_size_tb = 100
...
class ObjectStorageCostRecap:
    ...
    nvme_annual_str = fmt(100 * THOUSAND * Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.to(USD / GB / ureg.month).magnitude * 12, precision=0)
```
...
class StorageEconomicsCorpus:
    # ┌── 4. OUTPUT ───────────────────────────────────────
    tokenized_text_tb_str = fmt_qty_int(1.5 * TRILLION * BYTES_INT32.to(byte).magnitude * byte, TB, commas=False)
```
...
class BuildVsBuyStorageEconomics:
    ...
    cloud_egress_gb = Infrastructure.Pricing.Cloud.EgressPerGB.rate.to(USD / GB).magnitude
    dataset_pb = 1
    reads_per_year = 10
...
from mlsysim.fmt import fmt, fmt_qty_int
class StorageTieringCorpus:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    compressed_text_tb_str = fmt_qty(3 * TB, TB, precision=0, commas=False)
    tokenized_text_tb_str = fmt_qty_int(1.5 * TRILLION * BYTES_INT32.to(byte).magnitude * byte, TB, commas=False)
```
...
class CheckpointStormDefinition:
    ...
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    params_b = Models.Language.Llama2_70B.parameters.to(Bparam).magnitude
    nodes = Systems.Clusters.Training_1K.total_accelerators
    bytes_fp16 = BYTES_FP16.to(byte).magnitude
    bytes_fp32 = BYTES_FP32.to(byte).magnitude
    fabric_bw_gbs = Systems.Fabrics.InfiniBand_XDR.bandwidth.to(GB/second).magnitude
    ...
    optimizer_state_gb_str = fmt_qty(optimizer_state_gb * GB, GB, precision=0, commas=False)
    zero3_total_tb_str = fmt_qty(zero3_total_tb * TB, TB, precision=2, commas=False)
    zero3_write_s_str = fmt_time(zero3_write_s, 'second', precision=1, commas=False, style='word')
    naive_total_tb_str = fmt_qty(naive_total_tb * TB, TB, precision=1, commas=False)
...
class CheckpointStormCalc:
    ...
    LOCAL_NVME_DRIVES_PER_NODE_REF = 4 * count   # pedagogical: NVMe drives per node
    nodes = 256
    local_drive_count = LOCAL_NVME_DRIVES_PER_NODE_REF.to("").magnitude
    local_drive_bw_gbs = Hardware.Tech.Storage.NvmeGen4.bandwidth.to(GB/second).magnitude
    pfs_aggregate_bw_gbs = 1000
    checkpoint_interval_s = 600
    checkpoint_interval_min = checkpoint_interval_s // 60

    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    total_checkpoint_gb = (Models.Language.GPT3.parameters.to(param).magnitude * 10) / BILLION
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    ckpt_total_gb_str = fmt_qty(total_checkpoint_gb * GB, GB, precision=0, commas=True)
    per_node_shard_gb_str = fmt_qty(per_node_shard_gb * GB, GB, precision=1, commas=False)
    local_bw_gbs_str = fmt_qty(local_bw_gbs * GB / second, GB / second, precision=0, commas=False)
    local_write_s_str = fmt_time(local_write_s, 'second', precision=2, commas=False, style='word')
    total_pfs_tb_str = fmt_qty(total_pfs_tb * TB, TB, precision=1, commas=False)
    pfs_copy_s_str = fmt_time(pfs_copy_s, 'second', precision=1, commas=False, style='word')
    per_node_pfs_bw_gbs_str = fmt_qty(per_node_pfs_bw_gbs * GB / second, GB / second, precision=1, commas=False)
    ...
    local_drive_bw_gbs_str = fmt_qty(local_drive_bw_gbs * GB / second, GB / second, precision=0, commas=False)
    nodes_str = fmt_count(nodes, commas=False, label="node")
    pfs_aggregate_bw_tbs_str = fmt_qty((pfs_aggregate_bw_gbs / THOUSAND) * TB / second, TB / second, precision=0, commas=False)
    checkpoint_interval_min_str = fmt_time(checkpoint_interval_min, 'minute', precision=0, commas=False, style='word')
    checkpoint_interval_s_str = fmt_time(checkpoint_interval_s, 'second', precision=0, commas=False, style='word')
    pfs_aggregate_bw_gbs_str = fmt_qty(pfs_aggregate_bw_gbs * GB / second, GB / second, precision=0, commas=False)
```
...
class CheckpointFleetRetention:
    ...
    checkpoint_count_str = fmt(checkpoint_count_val, precision=0)
    ckpt_fleet_total_pb_str = fmt_qty(ckpt_fleet_total_pb_val * PB, PB, precision=1, commas=True)
    retention_recent_tb_str = fmt_qty(retention_recent_tb_val * TB, TB, precision=2, commas=False)
...
class CheckpointStorageDesignPrompt:
    ...
    nodes_str = fmt_count(nodes, commas=True, label="node")
    checkpoint_interval_min_str = fmt_time(checkpoint_interval_min, 'minute', precision=0, commas=False, style='word')
    local_nvme_bw_gbs_str = fmt_qty(local_nvme_bw_gbs * GB / second, GB / second, precision=0, commas=False)
    pfs_aggregate_tbs_str = fmt_qty(pfs_aggregate_tbs * TB / second, TB / second, precision=0, commas=False)
```
...
class StorageFootprintTable:
    ...
    tokenized_text_tb_str = fmt_qty_int(round(tokenized_text_tb_val) * TB, TB, commas=False)
    ckpt_fleet_total_pb_str = fmt_qty(ckpt_fleet_total_pb_val * PB, PB, precision=1, commas=True)
    archive_lineage_tb_str = fmt_qty(archive_lineage_tb_val * TB, TB, precision=1, commas=False, approx=True)
...
class SyntheticStorage:
    ...
    total_tb_str = fmt_qty(total_footprint_tb * TB, TB, precision=1, commas=False)
    ...
    data_size_tb_str = fmt_qty(data_size_tb * TB, TB, precision=0, commas=False)
...
class EconRatiosRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tier_cost_ratio_str = fmt(Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.to(USD / GB / ureg.month).magnitude / 0.02, precision=0, commas=False)
```
