...
[^fn-raid0-immutable]: **RAID 0 Risk**\index{RAID 0!risk}\index{RAID 0!data safety}: RAID 0 provides no redundancy; if one drive in a 4-drive array fails, all data on that node's cache is lost. This is acceptable for immutable training data (re-stage from PFS) or model weights (re-load from Object), but it is a catastrophic choice for the model's only copy of un-checkpointed gradients or local logs.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import MarkdownStr, fmt, fmt_count, check, fmt_time, fmt_bandwidth
...
: **HBM Memory Budget for 175B-Parameter Training**: Per-GPU allocation when splitting a 175B model eight ways inside the node and sharding optimizer and gradient state across 256 nodes. The optimizer and gradient rows are per-GPU shards after both levels of partitioning. The resulting budget occupies most of an 80 GB HBM device, leaving limited buffer for the incoming data pipeline. Any delay in fetching the next batch from host memory risks starving the accelerator. {#tbl-hbm-memory-budget}
...
**Systems insight**: Local storage is a *GPU utilization multiplier*. In this scenario, adding a \$500 NVMe drive to a \$30,000 GPU node recovers 15 percent of the GPU's capacity that was previously wasted on I/O wait. Across a 1,000-GPU cluster, that utilization gain is equivalent to adding 150 GPUs worth of useful work without buying 150 more accelerators. In ML infrastructure, local NVMe is not auxiliary; it is the physical buffer that decouples expensive compute from unpredictable shared storage.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_time, fmt_memory, fmt_params
...
1.  **Significance (quantitative)**: The storm magnitude scales as $T_{\text{write}} = R \times \text{per-replica state} / \text{BW}_{\text{fabric}}$, where $R$ is the number of replicas writing at once. For a `{python} CheckpointStormDefinition.params_b_str`-parameter model in FP16 (`{python} CheckpointStormDefinition.weights_gb_str` of weights) trained across `{python} CheckpointStormDefinition.replicas_str` with vanilla data parallelism, every replica holds a complete copy and a naive checkpoint generates `{python} CheckpointStormDefinition.naive_total_tb_str` of simultaneous writes. At 100 GB/s fabric bandwidth, that write takes roughly `{python} CheckpointStormDefinition.naive_write_s_str`, or over `{python} CheckpointStormDefinition.naive_write_min_str` of training stall per checkpoint event. ZeRO-3 sharding collapses the written state to roughly `{python} CheckpointStormDefinition.zero3_total_tb_str` total (`{python} CheckpointStormDefinition.weights_gb_str` weights + `{python} CheckpointStormDefinition.gradients_gb_str` gradients + `{python} CheckpointStormDefinition.optimizer_state_gb_str` optimizer state, split across `{python} CheckpointStormDefinition.replicas_str`), bringing $T_{\text{write}}$ down to roughly `{python} CheckpointStormDefinition.zero3_write_s_str`. This stall is the checkpoint $T_{\text{write}}$ term and can dwarf the compute time between checkpoints when the sharding optimization is omitted.
...
Feature stores and model registries, the operational systems that manage feature serving and model versioning, are also built on top of this storage hierarchy. Their production architecture is an operations problem; the storage point is that they depend on the same tiering, durability, and latency boundaries introduced here. One adjacent workload does belong in this chapter before it turns to storage fallacies: retrieval infrastructure reverses the training access pattern and therefore stresses a different part of the hierarchy. Vector databases, specialized storage systems for approximate nearest-neighbor search in embedding spaces, serve the retrieval-augmented generation pipeline by making that reversed access pattern operational.
...
The total data moved through the hierarchy\index{Storage!total data movement} is approximately `{python} StorageFootprintTable.ckpt_fleet_total_pb_str` of checkpoint data plus `{python} StorageFootprintTable.tokenized_text_tb_str` $\times$ `{python} StorageFootprintTable.epochs_str` epochs of training data reads. For a single-epoch language model training run, checkpoint I/O dominates data loading I/O by a factor of over `{python} StorageFootprintTable.ckpt_io_dominance_factor_str`.
...
A common fallacy is this: *NVMe is fast enough to feed GPUs directly without pipelining.*
...
A recurring pitfall is this: *Relying on prefetching to hide object storage latency.*
...
A common fallacy is this: *Millions of small training files are harmless at scale.*
...
A recurring pitfall is this: *Ignoring egress costs when designing the storage architecture.*
...
A common fallacy is this: *Average write bandwidth is enough for checkpoint design.*
...
A recurring pitfall is this: *Assuming faster accelerators automatically improve training throughput.*
...
A common fallacy is this: *RAID configurations optimized for databases are suitable for ML training.*
...
A recurring pitfall is this: *Creating full copies for each dataset snapshot.*
...
A common fallacy is this: *Compression always helps because it reduces the amount of data to read.*
...
A recurring pitfall is this: *Choosing a compression format before profiling the pipeline bottleneck.*
