...
The same hierarchy can be analyzed as a cache\index{Cache!effective service time} rather than as six isolated devices. If a request hits the hot tier with probability $h$, taking $T_{\text{hot}}$, and otherwise falls through to a colder tier, taking $T_{\text{cold}}$, its mean service time is
...
Failures create a separate correctness question. **At-most-once processing**\index{Processing semantics!at-most-once} can lose an output because it does not replay uncertain work; **at-least-once processing**\index{Processing semantics!at-least-once} can duplicate an effect because it does. An effectively exactly-once outcome requires replayable inputs and idempotent or transactional output keyed by a stable event or transaction identifier [@akidau2015]. Transport delivery alone cannot provide this application-level guarantee.
...
* **HBM is the destination**: Every lower tier exists to keep GPU HBM populated (principle \ref{pri-storage-hierarchy}). The roughly 30$\times$ aggregate bandwidth gap between HBM and object storage (and the much larger per-client gap once a single instance pulls from a shared object endpoint) drives every design decision in the ML storage hierarchy.
...
* **Cache misses set effective throughput**: A high hot-tier hit rate is insufficient when cold misses are orders of magnitude slower. Use @eq-storage-cache-service-time as a mean baseline, then test tail latency and miss bursts against the prefetch budget.
...
* **Storage reliability has three contracts**: Durability governs whether bytes survive, availability whether the service responds, and consistency which version a read may observe. Replication alone does not make these guarantees interchangeable.
* **Pipeline correctness lives at commit boundaries**: Replay protects availability only when stable identifiers and idempotent or transactional writes prevent duplicate effects. Event-time watermarks then make the separate freshness-completeness trade-off explicit.
