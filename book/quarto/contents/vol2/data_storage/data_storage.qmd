...
The cost advantage of object storage is significant: at roughly \$0.02/GB/month, it is 750$\times$ cheaper per byte than HBM and 5$\times$ cheaper than local NVMe. For a 100 TB training dataset, object storage costs roughly `{python} ObjectStorageCost.s3_annual_usd_str` per year, compared to `{python} ObjectStorageCost.nvme_annual_usd_str` for local NVMe. This cost advantage makes object storage the natural home for the organization's training data lake, where all raw data, preprocessed datasets, and archived model artifacts reside permanently.
...
# │ Exports: EconRatios.tier_cost_ratio_str, EconRatios.egress_100tb_usd_str
# └─────────────────────────────────────────────────────────────────────────────
class EconRatios:
    # ┌── 1. LOAD ─────────────────────────────────────────
    cost_nvme_gb_mo = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.to(USD / (GB * ureg.month)).magnitude
