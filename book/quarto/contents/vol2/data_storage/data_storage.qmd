...
That batch lifecycle creates a "spill" dynamic down the hierarchy. When weights alone do not fit in one HBM pool, the system has only two basic choices: split the live weights across accelerators, or keep some state outside HBM and fetch it when needed. Distributed training later gives these choices formal names, including tensor parallelism and offloading, but the storage pressure is already visible from the numbers. For our 175B parameter model, FP16 weights occupy 350 GB, requiring at least five 80 GB H100 accelerators just for weight storage, with no room left for activations or optimizer state. Adam's FP32 momentum and variance add another 1.4 TB. Sharding this footprint across the cluster makes the run possible, but every shard creates more lower-tier traffic and more coordination. @Sec-distributed-training-systems examines those partitioning strategies in depth; here, the key point is that HBM scarcity is what forces the rest of the storage hierarchy to exist.

The razor-thin margin is a defining feature of large model training. @Tbl-hbm-memory-budget shows the HBM memory budget for training our 175B parameter model on a single H100 GPU with 80 GB of HBM, using an 8-way split of model weights inside the node and a ZeRO-3-style shard of optimizer and gradient state across 256 nodes. The later distributed-training chapter derives these schemes; this table uses them only to show the storage pressure HBM sees:
...
**Problem**\index{ImageNet!bottleneck analysis}: A ResNet-50 training job on ImageNet (`{python} NVMeTierCalcs.imagenet_images_m_str`, ~`{python} NVMeTierCalcs.imagenet_avg_kb_str` average) targets `{python} NVMeTierCalcs.images_per_s_str`. The question is whether to use individual JPEG files on an HDD or NVMe.

**Math**:

1.  **Raw Bandwidth**\index{ImageNet!raw bandwidth calculation}: `{python} NVMeTierCalcs.images_per_s_str` $\times$ `{python} NVMeTierCalcs.img_size_kb_str` = `{python} NVMeTierCalcs.raw_bw_mb_per_s_str`.
...
Feature stores and model registries, the operational systems that manage feature serving and model versioning, are built on top of this storage hierarchy, but their architecture belongs with production ML operations in @sec-ml-operations-scale-feature-store-operations-2a36 and @sec-ml-operations-scale-model-registries-scale-ccba. One adjacent workload does belong here before the chapter turns to storage fallacies: retrieval infrastructure reverses the training access pattern and therefore stresses a different part of the hierarchy.
...
As the fleet consumes the sum of human-generated text, it encounters the **Data Wall**\index{Data Wall!definition}: the point where new high-quality training samples can only be generated by the fleet itself. This shift from *collecting* data to *synthesizing* data sends more than payload bytes through the hierarchy. A "synthetic fuel line" must also store the **provenance chain**\index{Provenance Chain!definition} of every sample: which model generated it, which filters accepted it, which judge models verified it, and which downstream run consumed it. Without that lineage, the system risks **model collapse**\index{Model Collapse!definition}, where a model degrades by training on its own unverified errors.
