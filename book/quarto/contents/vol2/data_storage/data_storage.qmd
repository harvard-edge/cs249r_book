...
class HbmTierBandwidth:
    ...
    hbm_host_dram_ratio_str = fmt_multiple(hbm_host_dram_ratio_val.m_as(ureg.dimensionless), precision=1, commas=False)
```
```{python}
#| echo: false
...
class NVMeTierCalcs:
    ...
    hdd_slowdown_factor_str = fmt_multiple(hdd_slowdown_val, precision=0, commas=False)
...
::: {.column-margin}
![](images/svg/vol2_data_storage_margin_004.svg){width="100%" fig-alt="Margin illustration showing Prestage, on-demand, and streaming staging patterns."}

*Prestage, on-demand, and streaming each trade latency against cost.*
:::
