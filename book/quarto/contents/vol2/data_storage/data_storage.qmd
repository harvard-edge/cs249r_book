...
This consistency property makes object storage suitable for both training data source (the original dataset) and long-term checkpoint retention (the archival copy after local NVMe staging), provided the pipeline validates the service semantics it relies on. Vector databases, a specialized storage primitive for approximate nearest-neighbor search in embedding spaces, are covered in @sec-data-storage-retrieval-infrastructure, where they serve the retrieval-augmented generation pipeline.
...
Versioning turns the storage hierarchy into a reproducibility system. The fleet must know not only which bytes it read, but which preprocessing code, shuffle seed, schema, and integrity checks produced those bytes before the run began.
...
Feature stores and model registries, the operational systems that manage feature serving and model versioning, are also built on top of this storage hierarchy. Their production architecture is an operations problem; the storage point is that they depend on the same tiering, durability, and latency boundaries introduced here. One adjacent workload does belong in this chapter before it turns to storage fallacies: retrieval infrastructure reverses the training access pattern and therefore stresses a different part of the hierarchy.
...
* **ML workloads invert storage assumptions**\index{ML Storage!inverted assumptions}: Traditional caching, IOPS optimization, and small-write durability patterns all produce the wrong answer for ML training. Throughput (GB/s) matters more than IOPS. Small files are the enemy; aggregated shards are the solution.
...
* **Economics drive tiering**: The `{python} EconRatiosRecap.tier_cost_ratio_mult_str` cost difference between local NVMe and object storage (and the >3,000$\times$ span from HBM to archive) mandates a tiering strategy\index{Storage Tiering!cost drivers}. Egress fees often exceed storage fees; design for total cost of data delivery, not storage cost alone.
