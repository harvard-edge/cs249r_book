...
# │ Exports: StorageFuelLineContext.h100_bw_tbs_str, .nvme_bw_gb_s_str,
...
class StorageFuelLineContext:
    ...
    nvme_bw_gb_s_str = fmt_bandwidth(nvme_bw_raw, unit=GB / second, precision=0, commas=False)
...
# │          TextImageBandwidth.text_bw_mb_s_str,
# │          TextImageBandwidth.image_bw_gb_s_str,
...
class TextImageBandwidth:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    text_bw_mb_s_str = fmt_bandwidth(text_bw, unit=MB / second, precision=1, commas=False)
    image_bw_gb_s_str = fmt_bandwidth(image_bw, unit=GB / second, precision=1, commas=False)
...
|:--------------------------------------------|----------------------------------------------------:|
| Model Weights (FP16, tensor-parallel)       |    `{python} HbmMemoryBudget.weight_per_gpu_gb_str` |
| Optimizer State (ZeRO-3 partitioned)        | `{python} HbmMemoryBudget.optimizer_per_gpu_gb_str` |
| Activations (variable by sequence length)   |     `{python} HbmMemoryBudget.activation_range_str` |
| Gradient Buffers (FP16, ZeRO-3 partitioned) |  `{python} HbmMemoryBudget.gradient_per_gpu_gb_str` |
...
# │          MILLION)
# │ Exports: NVMeTierCalcs.raw_bw_mb_s_str, .hdd_slowdown_factor_str,
...
class NVMeTierCalcs:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    raw_bw_mb_s_str             = fmt_bandwidth(raw_bw, unit=MB / second, precision=0, commas=False)
...
class NvmeTierContext:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvme_bw_gb_s_str = fmt_bandwidth(nvme_per_drive_bw, unit=GB / second, precision=0, commas=False)
    tokenized_text_tb_str = fmt_memory(tokenized_text, unit=TB, precision=0, commas=False)
...
1.  **Raw Bandwidth**\index{ImageNet!raw bandwidth calculation}: `{python} NVMeTierCalcs.images_per_sec_str` $\times$ `{python} NVMeTierCalcs.img_size_kb_str` = `{python} NVMeTierCalcs.raw_bw_mb_s_str`.
...
# │ Exports: PipelineCalcs.req_bw_imagenet_gb_s_str, PipelineCalcs.data_stall_pct_str,
...
class PipelineCalcs:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    req_bw_imagenet_gb_s_str = fmt_bandwidth(req_bw_imagenet, unit=GB / second, precision=1, commas=False)
...
As a separate image-training example, consider 256 GPUs training with ImageNet-scale images, 200 ms iterations, and 80 percent target utilization. This configuration requires `{python} PipelineCalcs.req_bw_imagenet_gb_s_str` of aggregate storage throughput. The bandwidth must be sustained continuously for the duration of training, which may last days or weeks. If at any point the storage system delivers less than `{python} PipelineCalcs.req_bw_imagenet_gb_s_str`, accelerators begin to idle. For the chapter's 175B language-model running example, tokenized text has a much lower per-batch bandwidth demand, but the total data volume and checkpoint traffic over the run are much larger, shifting the bottleneck from peak training-data bandwidth to sustained throughput and checkpoint movement over weeks.
...
class ObjectStorageCostRecap:
    ...
    s3_annual_cost = dataset_size.to(TB) * Infrastructure.Pricing.Storage.S3StandardLowPerTbMonth.rate * 12 * ureg.month
...
class GlacierCostRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    dataset_size = 100 * THOUSAND * GB
    glacier_annual_cost = dataset_size.to(TB) * Infrastructure.Pricing.Storage.GlacierStandardPerTbMonth.rate * 12 * ureg.month
    glacier_annual_usd_str = fmt_usd(glacier_annual_cost, precision=0)
```
