...
class StorageFuelLineContext:
    ...
    compressed_text = 3 * TB  # scenario compressed source corpus
    token_id_bytes = BYTES_INT32
    training_days = 30
    checkpoint_interval_min = 10
    bytes_per_param_ckpt = 10 * (byte / param)  # scenario checkpoint bytes per parameter
    bw_ratio_val = (h100_bw / nvme_bw_raw).to("").magnitude
    ckpt_total = gpt3_params * bytes_per_param_ckpt
    ...
    nvme_bw_str = fmt_bandwidth(nvme_bw_raw, unit=GB / second, precision=0, commas=False)
...
class StoragePyramidContext:
    ...
    tokenized_text_tb_str = fmt_memory(tokenized_text, unit=TB, precision=0, commas=False)
    h100_bw_tbs_str = fmt_bandwidth(h100_bw, unit=TB / second, precision=2, commas=False)
    host_dram_bw_gbs_str = fmt_bandwidth(host_dram_bw, unit=GB / second, precision=0, commas=False)
    hbm_host_dram_ratio_str = fmt_multiple(hbm_host_dram_ratio_val, precision=1, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import MarkdownStr, fmt, check, fmt_time, fmt_bandwidth
...
class TextImageBandwidth:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    text_bw_mb_str = fmt_bandwidth(text_bw, unit=MB / second, precision=1, commas=False)
    image_bw_gb_str = fmt_bandwidth(image_bw, unit=GB / second, precision=1, commas=False)
...
class HbmModelBudget:
    ...
    weights_fp16 = params * bytes_fp16
    ...
    adam_eq_math = fmt_math(rf"{params.to(Bparam).magnitude:.0f} \times 10^9 \times {bytes_fp32.to(byte).magnitude:.0f} \times {adam_states} = {_adam_res}")
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import MarkdownStr, check, fmt_percent_range, fmt_qty, fmt_qty_range, fmt_memory
...
class HbmMemoryBudget:
    ...
    hbm_capacity = Hardware.Cloud.H100.memory.capacity
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    weight_per_gpu_str = fmt_memory(weight_per_gpu, unit=GB, precision=2, commas=False)
    optimizer_per_gpu_str = fmt_memory(optimizer_per_gpu, unit=GB, precision=1, commas=False)
    gradient_per_gpu_str = fmt_memory(gradient_per_gpu, unit=GB, precision=1, commas=False)
    activation_range_str = MarkdownStr(f"{activation_min.to(GB).magnitude:.0f}--{activation_max.to(GB).magnitude:.0f} GB")
    comm_range_str = MarkdownStr(f"{comm_min.to(GB).magnitude:.0f}--{comm_max.to(GB).magnitude:.0f} GB")
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_count, fmt_time, fmt_rate, fmt_memory, fmt_bandwidth

class NVMeTierCalcs:
    # ┌── 1. LOAD ─────────────────────────────────────────
    nvme_bw_raw    = Hardware.Tech.Storage.NvmeGen4.bandwidth
    pfs_node_bw    = 4.0 * (GB / second)  # scenario per-node PFS bandwidth assumption
    images_per_sec = 1000
    ...
    node_shard       = ckpt_total / n_nodes
    ckpt_nvme        = node_shard / (4 * nvme_bw_raw)
    ckpt_pfs         = node_shard / pfs_node_bw
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    raw_bw_str             = fmt_bandwidth(raw_bw, unit=MB / second, precision=0, commas=False)
    ...
    ckpt_nvme_s_str        = fmt_time(ckpt_nvme, second, precision=1, commas=False, style='word')
    ckpt_pfs_s_str         = fmt_time(ckpt_pfs, second, precision=1, commas=False, style='word')
```
...
from mlsysim import Hardware, Models
from mlsysim.engine.solver import TrainingMemoryModel
from mlsysim.fmt import fmt, check, fmt_qty, fmt_qty_int, fmt_int, fmt_time, fmt_memory, fmt_bandwidth
...
class NvmeTierContext:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvme_bw_str = fmt_bandwidth(nvme_per_drive_bw, unit=GB / second, precision=0, commas=False)
    tokenized_text_tb_str = fmt_memory(tokenized_text, unit=TB, precision=0, commas=False)
...
class PipelineCalcs:
    ...
    # ┌── 3. GUARD ────────────────────────────────────────
    check(req_bw_imagenet_gbs_val > 10,
          f"ImageNet aggregate BW should be high, got {req_bw_imagenet_gbs_val:.1f} GB/s")
    check(data_stall_pct_val == 20, "Data stall calculation mismatch")

    # ┌── 4. OUTPUT ───────────────────────────────────────
    req_bw_imagenet_str = fmt_bandwidth(req_bw_imagenet, unit=GB / second, precision=1, commas=False)
    ...
    stall_pct_display_math = MarkdownStr(
        f"$$\\text{{Stall \\%}} = \\frac{{{t_io.to(ms).magnitude:.0f} - {t_comp.to(ms).magnitude:.0f}}}"
        f"{{{t_io.to(ms).magnitude:.0f}}} = "
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_memory, fmt_bandwidth
...
class HashValidationCost:
    ...
    throughput_mb_s_str = fmt_bandwidth(throughput_per_core, unit=MB / second, precision=1, commas=False)
...
class CompleteDataPathCheckpoint:
    ...
    checkpoint_interval_min = 10
    tokenized_text = training_tokens * token_id_bytes
    ckpt_total = gpt3_params * (10 * (byte / param))
    checkpoint_count_val = training_days * 24 * 60 // checkpoint_interval_min
    ckpt_fleet_total = ckpt_total * checkpoint_count_val
    check(checkpoint_count_val == 4320, "30 days at 10-min checkpoint interval should be 4320")
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_time, fmt_memory
...
class CheckpointStormDefinition:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    weights = params * bytes_fp16
    gradients = params * bytes_fp16
    master_weights = params * bytes_fp32
    adam_moments = params * 2 * bytes_fp32
    optimizer_state = master_weights + adam_moments
    zero3_total = weights + gradients + optimizer_state
    zero3_write = zero3_total / fabric_bw
    naive_total = weights * nodes
    ...
    check(1420 < naive_write.to(second).magnitude < 1440, "Naive write at 100 GB/s should be about 1,430 s for Training_1K replicas")
...
class CheckpointStormCalc:
    ...
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    LOCAL_NVME_DRIVES_PER_NODE_REF = 4 * count   # pedagogical: NVMe drives per node
    nodes = 256  # scenario checkpoint node count
    local_drive_count = LOCAL_NVME_DRIVES_PER_NODE_REF.to("").magnitude
    local_drive_bw = Hardware.Tech.Storage.NvmeGen4.bandwidth
    pfs_aggregate_bw = 1000 * (GB / second)  # scenario aggregate PFS bandwidth assumption
    ...
    local_bw_gbs_str = fmt_bandwidth(local_bw, unit=GB / second, precision=0, commas=False)
    ...
    per_node_pfs_bw_gbs_str = fmt_bandwidth(per_node_pfs_bw, unit=GB / second, precision=1, commas=False)
    ...
    local_drive_bw_gbs_str = fmt_bandwidth(local_drive_bw, unit=GB / second, precision=0, commas=False)
    nodes_str = fmt_count(nodes, commas=False, label="node")
    pfs_aggregate_bw_tbs_str = fmt_bandwidth(pfs_aggregate_bw, unit=TB / second, precision=0, commas=False)
    ...
    pfs_aggregate_bw_gbs_str = fmt_bandwidth(pfs_aggregate_bw, unit=GB / second, precision=0, commas=False)
```
...
class CheckpointStorageDesignPrompt:
    ...
    nodes_str = fmt_count(nodes, commas=True, label="node")
    checkpoint_interval_min_str = fmt_time(checkpoint_interval_min, 'minute', precision=0, commas=False, style='word')
    local_nvme_bw_gbs_str = fmt_bandwidth(local_nvme_bw, unit=GB / second, precision=0, commas=False)
    pfs_aggregate_tbs_str = fmt_bandwidth(pfs_aggregate_bw, unit=TB / second, precision=0, commas=False)
```
...
from mlsysim.fmt import check, fmt, fmt_memory, fmt_percent
