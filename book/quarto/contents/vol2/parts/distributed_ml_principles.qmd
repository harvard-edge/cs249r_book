...
**Implication**: Scale is not an option; it is a requirement for capability. To achieve a 10$\times$ improvement in performance, you typically need a 100--1000$\times$ increase in compute. This exponential hunger drives the transition from single-server training to warehouse-scale clusters.
:::
...
**Implication**: Scaling is a race between parallelizable compute (which shrinks with $N$) and communication overhead (which grows or stays constant). To scale efficiently, algorithms must minimize $T_{\text{comm}}$ or maximize $T_{\text{overlap}}$ (for example, through gradient accumulation or communication hiding).
:::
