...
**Invariant**: Model loss ($\mathcal{L}$) improves as a power-law function of compute ($C$), dataset size ($D$), and parameters ($P$).
$$ \mathcal{L}(C) \propto C^{-\alpha} $$

**Implication**: Scale is not an option; it is a requirement for capability. Because loss improves sublinearly with compute, each equal-sized improvement requires disproportionately more compute; in practice this diminishing return drives the transition from single-server training to warehouse-scale clusters.
:::

::: {#pri-distributed-step-time .callout-principle title="The Distributed Step Time Law (Iron Law of Scale)"}
**Invariant**: The time to complete one training step is the sum of computation and non-overlapped communication.
$$ T_{\text{step}}(N) = \frac{T_{\text{compute}}}{N} + T_{\text{comm}}(N) - T_{\text{overlap}} $$

**Implication**: Scaling is a race between parallelizable compute (which shrinks with $N$) and communication overhead (which grows or stays constant). To scale efficiently, algorithms must reduce or amortize $T_{\text{comm}}$ (for example, through **Gradient Accumulation** or compression) and maximize $T_{\text{overlap}}$ through **Communication Hiding** and pipelined execution.
:::
...
Part II establishes how we make the Fleet compute:
