...
**Implication**: Small messages—for example, mixture-of-experts (MoE) routing metadata—are latency-bound; large messages (for example, gradients) are bandwidth-bound. Optimization strategies must match the regime: fuse small messages to amortize $\alpha$, compress large messages to reduce $n$.
...
**Design heuristic**: Distributed ML optimizations often relocate overhead among Compute, Communication, and Coordination (the C$^3$ taxonomy). Reducing one term can increase another, so designs must evaluate all three.
