...
If Volume 1 was about optimizing the single machine, Volume 2 is about coordinating the **Fleet**. These principles govern the physics of distributed learning—why adding more GPUs does not always yield linear speedups, and why communication is often the hidden bottleneck.

::: {.callout-perspective title="The Universal Scaling Law"}
...
::: {.callout-note icon=false title="The Distributed Step Time Law (Iron Law of Scale)"}
...
## Part I Roadmap: The Logic of Distribution

This section establishes the algorithmic foundations for the Machine Learning Fleet:

1.  **Distributed Training (@sec-distributed-training)**: The strategies (Data, Model, Pipeline) for partitioning the workload.
2.  **Communication (@sec-communication)**: The collective operations (All-Reduce, All-Gather) that bind the fleet together.
3.  **Fault Tolerance (@sec-fault-tolerance)**: The mechanisms for surviving the inevitable failures of scale.
