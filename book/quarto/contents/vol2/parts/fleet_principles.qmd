...
**The invariant**: Model parameter growth (10$\times$ per year) consistently outpaces GPU memory capacity growth (2$\times$ per year).
...
**The implication**: The trajectory from CPU to GPU to Tensor Processing Unit (TPU) to fixed-function ASIC is not a technology trend—it is a physical law. Each step trades programmability for efficiency, and the fleet architect must choose the right point on this curve for each workload.
:::
...
**The invariant**: Storage performance decreases and capacity increases as data moves further from the accelerator.

**The implication**: The systems engineer's task is to ensure that the *right data* is on the *right tier* at the *right time*. Data format choices, caching strategies, prefetch buffer sizing, and tiering policies all exist to manage movement upward through the hierarchy so that accelerators do not starve.
:::
...
**The implication**: Perfect linear scaling ($\eta_{\text{scaling}} = 1.0$) is a theoretical limit, not a practical target. Real systems achieve $\eta = 0.85$–$0.95$ at moderate scale and degrade further as $N$ grows. The gap between $\eta = 1.0$ and the achieved efficiency is the **communication tax**—the price of coordination.
:::
