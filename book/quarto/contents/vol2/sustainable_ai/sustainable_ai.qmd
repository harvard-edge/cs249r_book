...
::: {.callout-note title="Figure Placeholder: Carbon-Aware Scheduling" collapse="true"}
```{.tikz}
% TODO: Diagram showing Time-shifting (Night vs Day) and Geo-shifting (Coal Region vs Hydro Region)
\node[draw, align=center] {Carbon-Aware Scheduling\nTemporal vs Geographic Shifting};
```
**Carbon-Aware Workload Scheduling**. Conceptual diagram of shifting compute jobs across time and space. Panel A shows temporal shifting: delaying a training job until solar availability peaks at noon. Panel B shows geographic shifting: routing inference requests to a datacenter with low carbon intensity (e.g., hydro-powered) vs high intensity (coal-powered).
:::
