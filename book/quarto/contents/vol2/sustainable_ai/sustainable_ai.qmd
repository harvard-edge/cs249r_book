...
**Parameters**: `{python} LifecycleCarbonEstimate.n_gpus_str` H100 GPUs, `{python} LifecycleCarbonEstimate.training_days_str`, `{python} LifecycleCarbonEstimate.h_h100_tdp_w_str` TDP, PUE `{python} LifecycleCarbonEstimate.pue_str`, grid intensity `{python} LifecycleCarbonEstimate.grid_ci_str` g $\text{CO}_2$/kWh.
...
**Total**: `{python} LifecycleCarbonEstimate.operational_tonnes_str` + `{python} LifecycleCarbonEstimate.embodied_tonnes_str` $\approx$ `{python} LifecycleCarbonEstimate.total_tonnes_str` $\text{CO}_2$.
...
The geographic choice alone produces a `{python} TrainingEmissions.emissions_ratio_mult_str` difference in training emissions.
...
That sequence begins with memory. Microcontrollers operate with 64 KB to 2 MB SRAM, so sustainable TinyML starts with careful memory planning during model design.

The memory pass starts with peak activation analysis, not just parameter count.\index{Memory Analysis!layer-wise} A model that has small weights can still fail if one intermediate tensor exceeds the tensor arena, so TinyML runtimes rely on in-place operations,\index{In-Place Operations} tensor-arena planning,\index{Tensor Arena!optimization} and operator fusion\index{Operator Fusion} to reuse buffers and avoid fragmentation. Once the memory plan fits, the energy pass asks whether ordinary INT8 arithmetic is still too expensive for the deployment. On devices powered by solar, vibration, or RF harvesting, Binary Neural Networks (BNNs)\index{Binary Neural Networks} may be justified because XNOR-style operations\index{XNOR-Net Operations} replace multiply-accumulate work with bit operations. That trade is not free: BNNs often sacrifice accuracy, so they belong in applications where continuous sensing matters more than full-precision classification margins.

Automated architecture design becomes useful only after those constraints are explicit. MCUNet\index{MCUNet} jointly searches the network and inference schedule for memory-limited microcontrollers [@lin2020mcunet], Once-for-All Networks\index{Once-for-All Networks} amortize search by training a supernet from which device-specific subnetworks can be extracted [@Cai2020], and ProxylessNAS\index{ProxylessNAS} optimizes directly against hardware latency and energy [@cai2018proxylessnas]. These methods should not be read as a menu of sustainable techniques. They are responses to a specific failure: the hand-designed model cannot simultaneously satisfy SRAM, latency, and harvested-energy constraints. The lifecycle budget must record the resulting trade-offs because accuracy loss, extra search or distillation training, and rebound-driven deployment growth can erase nominal per-inference savings.
