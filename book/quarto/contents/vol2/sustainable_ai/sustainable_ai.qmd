...
class LifecycleCarbonEstimate:
    ...
    from mlsysim.core.solver import SustainabilityModel
...
# │       constant ENERGY_DRAM_PJ_PER_BYTE (160) per the single-source decision.
# │ Imports: mlsysim (Literature.Energy.Move*), mlsysim.core.constants
# │          (ENERGY_DRAM_PJ_PER_BYTE), mlsysim.fmt (fmt, check)
# │ Exports: reg_str, l1_str, l2_str, dram_str, nvme_str, net_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Literature
from mlsysim.core.constants import ENERGY_DRAM_PJ_PER_BYTE
from mlsysim.fmt import fmt, check
...
class MemoryHierarchyEnergy:
    ...
    dram = ENERGY_DRAM_PJ_PER_BYTE.m_as('picojoule/byte')
...
# │       ENERGY_DRAM_PJ_PER_BYTE (device). crossover = E_move/E_compute.
# │
# │ Imports: mlsysim (Literature.Energy.EffGpuPjFlop), mlsysim.core.constants
# │          (ENERGY_DRAM_PJ_PER_BYTE, BYTES_FP32), mlsysim.fmt (fmt, MarkdownStr, check)
# │ Exports: e_compute_str, e_move_str, crossover_eq_math, n_small_str,
# │          {ai,comp,mem}_{small,large,vec}_eq_math, vec_bytes_eq_math
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Literature
from mlsysim.core.constants import ENERGY_DRAM_PJ_PER_BYTE, BYTES_FP32
...
class MatMulEnergyAnalysis:
    ...
    e_move = ENERGY_DRAM_PJ_PER_BYTE.m_as('picojoule/byte')         # 160 (device)
...
::: {.column-margin}
![](images/png/sustainable_ai_inference_crossover.png){width="100%" fig-alt="Two curves over a model's service life: a flat blue training-emissions baseline and a rising red cumulative-inference curve that climbs steeply past it, crossing over so inference dominates total emissions."}
