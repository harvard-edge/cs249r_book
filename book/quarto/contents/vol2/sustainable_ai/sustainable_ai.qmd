...
The proliferation of machine learning systems has precipitated an environmental sustainability crisis that directly challenges the field's trajectory. Sustainability emerges as a core systems engineering discipline rather than an ancillary consideration, addressing the critical intersection between computational requirements and environmental stewardship.
...
The fundamental metric for computational energy efficiency is energy consumed per operation, typically measured in picojoules (pJ). For AI workloads, the most relevant metrics are energy per floating-point operation (pJ/FLOP) and energy per multiply-accumulate (pJ/MAC), where one MAC operation performs both a multiplication and addition, equivalent to two FLOPs.
...
Precision significantly affects energy per operation. INT8 integer arithmetic consumes approximately one-sixteenth the energy of FP32 floating-point at the same frequency and voltage, combining reduced datapath capacitance (4× from bit width) with lower voltage requirements (2× from larger noise margins) and simpler control logic (2× from reduced complexity).
...
Sustainable AI implementation faces a critical challenge known as Jevons Paradox[^fn-jevons-paradox]: efficiency improvements alone may inadvertently increase overall consumption by making AI more accessible and affordable. This phenomenon, known as the **Rebound Effect**, occurs when efficiency gains lower the cost of computation, enabling entirely new classes of applications (e.g., real-time video generation, pervasive ambient computing) that were previously economically infeasible.
...
A third approach, knowledge distillation[^fn-knowledge-distillation] [@hinton2015distilling], allows large AI models to transfer their learned knowledge to smaller, more efficient models. In this process, a large teacher model trains a smaller student model to approximate its predictions, enabling the student model to achieve competitive performance with significantly fewer parameters. DistilBERT exemplifies this technique, retaining 97% of the original BERT model's accuracy while using only 40% of its parameters and being 60% faster [@sanh2019distilbert]. Knowledge distillation techniques allow AI practitioners to deploy lightweight models that require less computational power while delivering high-quality predictions.
...
TinyML deployments face unique constraints beyond datacenter optimization: models must fit in kilobytes of SRAM, execute with microsecond latency, and consume milliwatts of power. These constraints require specialized optimization techniques that extend beyond standard pruning and quantization:

+---------------------------+---------------------------+----------------------------------+----------------------------------+
| **Technique**             | **Typical Accuracy**      | **Memory Reduction**             | **Energy Reduction**             |
|                           | **Impact**                |                                  |                                  |
+:==========================+:==========================+:=================================+:=================================+
| INT8 Quantization         | less than 1%              | 4x                               | 8-16x                            |
+---------------------------+---------------------------+----------------------------------+----------------------------------+
| INT4 Quantization         | 1-5%                      | 8x                               | 16-32x                           |
+---------------------------+---------------------------+----------------------------------+----------------------------------+
| Binary Neural Networks    | 5-15%                     | 32x                              | 50-100x                          |
+---------------------------+---------------------------+----------------------------------+----------------------------------+
| Structured Pruning        | 1-3%                      | 2-10x                            | 2-10x                            |
| (90% sparsity)            |                           |                                  |                                  |
+---------------------------+---------------------------+----------------------------------+----------------------------------+
...
: **TinyML Optimization Techniques**: Optimization techniques for microcontroller deployment trade accuracy for dramatic memory and energy reductions. INT8 quantization is the baseline for MCU inference; more aggressive techniques enable deployment on severely constrained devices. {#tbl-tinyml-optimization}
...
*MCU-Specific Quantization:* While datacenter quantization focuses on INT8 for inference acceleration, MCU deployments often push to INT4 or binary precision where supported:

- **CMSIS-NN kernels**: ARM's optimized neural network library for Cortex-M processors supports INT8 and INT16 operations
- **TensorFlow Lite Micro**: Provides INT8 quantization with per-channel calibration for MCU deployment
- **Post-training quantization**: Requires calibration dataset to determine optimal scaling factors for each layer
