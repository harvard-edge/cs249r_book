...
from mlsysim.fmt import fmt, fmt_qty, fmt_usd, check, MarkdownStr
...
class AutoPlacement:
    ...
    energy_mwh_str = fmt_qty(energy_mwh * ureg.megawatt_hour, ureg.megawatt_hour, precision=0, commas=True)
```
...
class LifecycleCarbonEstimate:
    ...
    gpu_tdp_kw_str = fmt_qty(gpu_tdp_kw * kilowatt, kilowatt, precision=1, commas=False)
    ...
    facility_power_kw_str = fmt_qty(facility_power_kw * kilowatt, kilowatt, precision=1, commas=True)
    facility_energy_kwh_str = fmt_qty_int(round(total_facility_energy_kwh) * ureg.kilowatt_hour, ureg.kilowatt_hour, commas=True)
...
class PueEfficiency:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    pue_it_mw_str = fmt_qty(p_it_mw * ureg.megawatt, ureg.megawatt, precision=0, commas=False)
    ...
    pue_savings_mwh_str = fmt_qty(annual_savings_mwh * ureg.megawatt_hour, ureg.megawatt_hour, precision=1, commas=True)
...
class TrainingEmissions:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_hours_str = fmt_time(training_hours, 'hour', precision=0, commas=True, style='word')
    ...
    gpu_energy_wh_str = fmt_qty(gpu_energy, ureg.watt_hour, precision=0, commas=True)
    gpu_energy_kwh_str = fmt_qty(gpu_energy_kwh * ureg.kilowatt_hour, ureg.kilowatt_hour, precision=1, commas=True)
    facility_energy_kwh_str = fmt_qty(facility_energy_kwh * ureg.kilowatt_hour, ureg.kilowatt_hour, precision=1, commas=True)
