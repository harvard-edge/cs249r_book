...
Contemporary machine learning applications operate at industrial scales, with environmental impact comparable to established heavy industries. Training a single large AI model can consume as much electricity as roughly `{python} Gpt3HouseholdEnergyAnchor.gpt3_household_years_str` US homes do in an entire year. When compute demand grows faster than hardware efficiency, the result is the **Sustainability Paradox**\index{Sustainability Paradox!definition} in artificial intelligence [@sevilla2022]. Sustainable AI treats that gap as a first-class systems problem rather than an externality to be managed after deployment.
...
**Systems lesson**: Carbon-aware placement internalizes environmental externalities into the fleet scheduling objective. The optimizer proves that high-renewables grids minimize true TCO when carbon penalties are factored into ML workload placement.
...
::: {.column-margin}
![](images/svg/vol2_sustainable_ai_margin_001.svg){width="100%" fig-alt="Carbon-emissions ladder for the same training job: Poland is about 8,200 tonnes CO2, Quebec about 200 tonnes, with the 40 times difference shown as an annotation rather than a third bar."}
...
[^fn-ai-compute-growth]: **Ai Compute Growth Rate**\index{Ai Compute Growth Rate!definition}\index{AI Compute Growth!doubling time}: The `{python} ComputeGrowthScenario.compute_growth_factor_mult_str` increase from 2012 to 2019 implies a doubling time of approximately `{python} ComputeGrowthScenario.compute_doubling_months_str`, roughly `{python} ComputeGrowthScenario.moore_speedup_mult_str` faster than Moore's Law's 2-year doubling. This divergence is one major driver of the energy wall: no physically realizable improvement in silicon efficiency can match a doubling cadence measured in months by process scaling alone, making algorithmic efficiency and carbon-aware scheduling central sustainability levers at scale.
...
[^fn-pue-evolution]: **Pue (power Usage Effectiveness)**\index{Pue (power Usage Effectiveness)!definition}: In the early 2000s, PUE values of 2.0-2.5 were common, meaning more power went to cooling than to computing [@thegreengrid2007pue]. Google's 2009 disclosure of PUE 1.21 proved that free-air cooling could halve data center overhead. The shift from PUE to CUE (Carbon Usage Effectiveness) and WUE (Water Usage Effectiveness) reflects a systems-level insight: optimizing watts alone is insufficient when water and carbon constraints bind independently.

### Training-scale energy concentration {#sec-sustainable-ai-training-scale-energy-concentration}

The grid, siting, and heat-reuse constraints become severe because large training campaigns concentrate energy demand into long, synchronized runs. OpenAI's GPT-3[^fn-gpt3-energy-scale] exemplifies this scale: its `{python} Gpt3HouseholdEnergyAnchor.gpt3_training_mwh_str` training run, the chapter's roughly `{python} Gpt3HouseholdEnergyAnchor.gpt3_household_years_str`-household-year anchor, reflects the computation required to train large language models on large datasets, with additional energy overhead from distributed-training communication[^fn-training-communication-overhead] [@maslej2023artificial].

[^fn-training-communication-overhead]: **Training Communication Overhead**\index{Training Communication Overhead!definition}: Distributed training adds 15–30 percent energy overhead beyond raw computation due to gradient synchronization and checkpointing across nodes. For large models requiring thousands of GPUs, this communication tax alone can consume more energy than the entire training run of a mid-scale model, making parallelism strategy selection a first-order sustainability decision.

[^fn-gpt3-energy-scale]: **Gpt-3 Energy Scale**\index{Gpt-3 Energy Scale!definition}\index{GPT-3!energy consumption}: GPT-3's 1,287 MWh training cost translates to roughly \$130,000 in US electricity and 552 metric tons of CO~2~ at average grid intensity. The energy-per-parameter ratio of approximately 7.35 MWh per billion parameters reveals the co-design opportunity: optimized architectures using mixed precision and sparsity achieve sub-1 MWh per billion parameters, a several-fold efficiency gain that compounds across large training runs.
...
from book.tools.figures.style import COLORS
...
- Selective, Event-Driven Activation: Rather than processing all information continuously, high-efficiency systems are asynchronous. They activate only small portions of the network at any time and consume energy only when actively processing changing signals.[^fn-event-driven-energy]
...
[^fn-event-driven-energy]: **Event-driven Computing**\index{Event-driven Computing!definition}: A paradigm where computation triggers only on input changes rather than continuous clock cycles. Neuromorphic chips like Intel's Loihi exploit this to achieve 100--1,000$\times$ energy reductions for temporal tasks (audio, video, sensor data) by drawing near-zero power when inputs are static. The trade-off: event-driven architectures sacrifice throughput on batch workloads where all data changes simultaneously.

The biological model points toward promising research directions for sustainable AI. Architectures that implement **Spiking Neural Networks (snns)**\index{Spiking Neural Networks (snns)!definition}, event-driven models that communicate through discrete spikes, or sparse activation patterns can achieve significant energy reductions by mimicking sparse communication models[^fn-snn-efficiency] [@prakash2023tinyml]. Local learning algorithms and self-supervised approaches offer additional pathways toward more sample-efficient and energy-conscious systems.

[^fn-snn-efficiency]: **Spiking Neural Networks (snns)**\index{Spiking Neural Networks (snns)!definition}: Third-generation neural networks that communicate through discrete spikes rather than continuous activations. SNNs process information only when spikes occur, achieving 10--100$\times$ energy savings on temporal data (audio, video, sensor streams). The sustainability trade-off: SNN training algorithms remain less mature than backpropagation for many benchmarked workloads, but hardware implementations like Intel Loihi 2 demonstrate the efficiency ceiling these architectures can approach.
...
[^fn-optimizer-memory-cost]: **Optimizer Memory as Energy Cost**\index{Optimizer Memory as Energy Cost!definition}: Adaptive Moment Estimation (Adam) stores per-parameter first and second moment estimates. For a 70B model in FP32, these two moment buffers total 560 GB; the 280 GB of parameters bring parameters plus moments to 840 GB, excluding gradients. Any memory comparison with plain SGD depends on whether gradients and momentum are included. The sustainability implication is direct: larger optimizer state means more HBM accesses per training step, and at 160 pJ/byte for DRAM, memory overhead can dominate the energy budget of parameter updates.

[^fn-carbon-intensity-variance]: **Carbon Intensity Variance**\index{Carbon Intensity Variance!definition}: Grid carbon intensity spans two orders of magnitude: coal at 820 g CO~2~/kWh vs. hydro at 10–30 g CO~2~/kWh. Critically, intensity also varies temporally: Texas fluctuates 10$\times$ within a single day based on wind generation. This dual geographic and temporal variance is what makes carbon-aware scheduling viable: combining the geographic lever (the 8 to 40 times range from @sec-sustainable-ai-scale-environmental-impact-ac9a) with temporal shifting pushes the achievable spread toward the high end of the 10 to 80 times span, so identical training runs can differ several-fold in emissions based solely on when and where they execute.
...
1. **Overhead Reduction**\index{Overhead Reduction!definition}: `{python} PueEfficiency.pue_old_str` - `{python} PueEfficiency.pue_new_str` = `{python} PueEfficiency.pue_diff_str`.
...
3. **Financial Savings**\index{Financial Savings!definition}: `{python} PueEfficiency.pue_savings_mwh_str` $\times$ `{python} PueEfficiency.electricity_rate_per_mwh_str` $\approx$ `{python} PueEfficiency.pue_savings_usd_str`.
...
The balance between computation and data movement determines whether energy consumption is compute-bound or memory-bound. @Eq-energy-arithmetic-intensity defines arithmetic intensity (AI), the ratio that determines which resource dominates energy consumption. In these equations, $O$ is the operation count in FLOPs, $D_{\text{vol}}$ is data volume in bytes, $E_{\text{compute}}$ is energy per FLOP, and $E_{\text{move}}$ is energy per byte moved:
...
1. Compute energy: `{python} TrainingEmissions.n_gpus_str` GPUs $\times$ `{python} A100PowerScenario.a100_tdp_w_str` $\times$ `{python} TrainingEmissions.training_hours_str` hours = `{python} TrainingEmissions.gpu_energy_kwh_str`.
...
Beyond computational demands, cooling accounts for 30--40 percent of data center energy consumption [@ebrahimi2014], as discussed in @sec-sustainable-ai-cooling.
...
[^fn-nas-carbon-cost]: **Neural Architecture Search (nas) Carbon Cost**\index{Neural Architecture Search (nas) Carbon Cost!definition}: The roughly 284,000 kg CO~2~e figure from @strubell2019 is the original Evolved Transformer NAS estimate, not the cost of training one final model. @patterson2021carbon later showed that the estimate drops sharply when the search's proxy-task setup, Google's TPU hardware, data-center efficiency, and grid mix are accounted for. The systems lesson is boundary discipline: search, tuning, and failed trials belong inside the accounting boundary, but reusable architecture search should be amortized across the models and domains that reuse the discovered design. This concern helped motivate efficient NAS work: weight sharing, continuous relaxations, and hardware-aware search reduce the search budget, so the meta-optimization of how architectures are searched is itself a sustainability lever [@elsken2019].
...
![](images/svg/model-carbonfootprint.svg)
:::
...
Scope 2 emissions\index{Scope 2 Emissions} represent indirect emissions from electricity purchased to power AI infrastructure. This is often the dominant category for owned facility operations, and it varies dramatically by geographic location and grid energy mix. As established in @sec-sustainable-ai-scale-environmental-impact-ac9a, this geographic lever sits in the 8 to 40 times range for representative region pairs; comparing the dirtiest coal grid against the cleanest hydro grid stretches it toward the high end of the 10 to 80 times span.
...
The **Ghg Protocol**\index{Ghg Protocol!definition}[^fn-ghg-protocol-standard] framework [@ghgprotocol2023] provides the standard categorization for these emissions, summarized in @fig-ghg-protocol. Three scopes provide an engineering classification checklist:

[^fn-ghg-protocol-standard]: **Ghg Protocol**\index{Ghg Protocol!definition}: Developed jointly by the World Resources Institute and WBCSD, this framework is used by over 90 percent of Fortune 500 companies reporting to CDP. Its three-scope taxonomy matters for ML systems because most AI carbon hides in Scope 3 (hardware manufacturing, cloud compute supply chains), which companies historically underreport by 50–70 percent compared to Scopes 1 and 2.
...
Audit the carbon footprint of a machine learning platform by classifying the following emission sources into Scope 1 (Direct), Scope 2 (Indirect Energy), or Scope 3 (Value Chain):

- [ ] **Backup generation**: Classify diesel burned by backup generators during a grid outage at an owned facility.
- [ ] **Purchased electricity**: Classify electricity purchased from the grid to power a leased NVIDIA H100 cluster.
...
Deploying `{python} GridQueue.cluster_gpus_str` and deploying the electrical substation required to power their IT load (`{python} GridQueue.cluster_power_mw_str`, after the `{python} GridQueue.cluster_gpu_power_mw_str` GPU subtotal is expanded for rack support power) run on very different clocks. On the silicon path, GPU supply chains are volatile, but typical enterprise lead times are `{python} GridQueue.gpu_lead_time_months_str`. On the infrastructure path, permitting, EPC (Engineering, Procurement, Construction), and grid connection for a new 10+ MW substation averages `{python} GridQueue.substation_lead_time_months_str`. Infrastructure therefore takes `{python} GridQueue.ratio_mult_str` longer to deploy than the accelerators themselves.
...
class RackPowerBudget:
    ...
    gpu_pct_str = fmt_percent(gpu_pct / 100, precision=0, commas=False, style='symbol')
    host_pct_str = fmt_percent(host_pct / 100, precision=0, commas=False, style='symbol')
    nvswitch_pct_str = fmt_percent(nvswitch_pct / 100, precision=0, commas=False, style='symbol')
    ib_pct_str = fmt_percent(ib_pct / 100, precision=0, commas=False, style='symbol')
    conversion_pct_str = fmt_percent(conversion_pct / 100, precision=0, commas=False, style='symbol')
    cooling_pct_str = fmt_percent(cooling_pct / 100, precision=0, commas=False, style='symbol')
...
The electricity consumed by a single large-model training campaign can become nonnegligible relative to the output of a power plant. A `{python} WindFarmEquivalent.facility_power_mw_str` training facility operating at full capacity for one year consumes `{python} WindFarmEquivalent.facility_energy_gwh_str`, which is comparable to the annual output of a `{python} WindFarmEquivalent.facility_power_mw_str` wind farm (at `{python} WindFarmEquivalent.capacity_factor_pct_str` capacity factor, a wind farm produces about `{python} WindFarmEquivalent.wind_energy_gwh_str` per year, so the training facility would require the equivalent of approximately `{python} WindFarmEquivalent.wind_equiv_str` large wind farms).
...
[^fn-pue-metric]: **Pue (power Usage Effectiveness)**\index{Pue (power Usage Effectiveness)!definition}: Defined by The Green Grid consortium in 2007 as $P_{\text{total}} / P_{\text{IT}}$. Google's fleet-wide PUE averages 1.10; the industry average is ~1.58. For a 10,000-GPU cluster consuming `{python} PueSavings.it_power_mw_str` of IT power after rack support load, reducing PUE from 1.58 to 1.10 saves `{python} PueSavings.diff_158_to_110_mw_str` of cooling overhead -- roughly `{python} PueSavings.savings_158_to_110_musd_str` per year in electricity and the equivalent of removing `{python} PueSavings.households_158_to_110_str` residential homes from the grid. ML-specific facilities with direct-to-chip liquid cooling have demonstrated PUE values of 1.03--1.08.
...
The prefill/decode distinction summarized in @tbl-prefill-decode extends beyond latency into energy efficiency. Recent analysis [@ma2026challenges] reveals that autoregressive generation is inherently energy-wasteful compared to batch processing because the two phases stress different hardware limits. During prefill\index{Inference!prefill phase}, high arithmetic intensity allows the GPU to perform thousands of operations for every byte read from memory, achieving near-peak energy efficiency in pJ/FLOP. During decode\index{Inference!decode phase}, the model must read the entire weight set from HBM to generate a single token; arithmetic intensity is low, so the compute units sit idle for much of the cycle.
...
**Systems insight**: Full fine-tuning is impossible within a reasonable daily battery budget. Sustainable on-device learning requires parameter-efficient fine-tuning (PEFT)\index{Parameter-Efficient Fine-Tuning} or sparse updates to reduce the energy cost per token by 100$\times$ or more.
...
class WakeWordPower:
    ...
    avg_power_eq = fmt_display_math(
        f"P_{{\\text{{average}}}} = {_vad_mw:g} + {speech_activity:g} \\times {_wake_mw:g} + "
        f"{wake_occurrence:g} \\times {_full_mw:g} \\times {full_model_duty_s:g} = {avg_power_mw_str}"
    )
```
...
| **Quantization**                                   | Bit width               | Lowers numerical precision so arithmetic units and memory transfers move fewer bits           | INT8 operations consume about 16$\times$ less energy than FP32, 4-bit operations can reach 64$\times$ reductions, and Q8BERT reduces BERT size by 4$\times$ with minimal degradation [@zafrir2019] |
...
Google's carbon-intelligent computing platform[^fn-carbon-scheduling-results] demonstrated this approach at scale, achieving a 40 percent reduction in carbon footprint under its global workload-shifting assumptions [@radovanovic2021carbonaware]. Within the broader energy-gap cascade of @fig-carbon-scheduling, carbon-aware scheduling is one of the systemic-stage steps, contributing a more conservative 1.3$\times$ average reduction for mixed production fleets, where only some jobs are deadline-tolerant enough to move across time or geography.
...
[^fn-carbon-scheduling-results]: **Carbon-aware Scheduling at Scale**\index{Carbon-aware Scheduling at Scale!definition}\index{Carbon-Aware Scheduling!results}: Google's carbon-aware data center study reports 15 percent carbon reduction through intra-region temporal shifting alone, and 40 percent globally by routing nonurgent batch compute across time zones to chase renewable peaks [@radovanovic2021carbonaware]. The key insight is that delay-tolerant workloads gain access to dramatically different grid carbon intensities without any model or infrastructure changes.
...
| **Mechanization**\index{Google Sustainability Framework!mechanization} | Run optimized cloud infrastructure at high utilization                                                | 1.4--2$\times$ energy reduction compared to conventional on-premise data centers                                               | Lower facility PUE than the industry-average baseline used in the 2021 study |
...
Engineers assume cloud providers operate efficiently and sustainably. In production, geographic region dominates all other factors through grid carbon intensity differences. Training a 7B model on `{python} TrainingEmissions.n_gpus_str` A100s for `{python} TrainingEmissions.training_days_str` produces `{python} TrainingEmissions.emissions_tons_str` CO~2~ on the US average grid (`{python} TrainingEmissions.grid_ci_g_per_kwh_str`) but only `{python} TrainingEmissions.quebec_emissions_kg_str` CO~2~ on Quebec's hydroelectric grid (`{python} TrainingEmissions.quebec_ci_g_per_kwh_str` operational), a roughly `{python} TrainingEmissions.emissions_ratio_mult_str` difference for this US-average-versus-Quebec pairing, which sits inside the 8 to 40 times geographic range established in @sec-sustainable-ai-scale-environmental-impact-ac9a. Coal-powered grids emit 800–1000 g CO~2~/kWh while well-managed hydroelectric sources emit 10–50 g CO~2~/kWh. As demonstrated in @sec-sustainable-ai-carbon-footprint-calculation-c2fe, teams that deploy to default cloud regions without checking grid carbon intensity waste a large multiple of the carbon budget necessary, turning "cloud sustainability" into a geographic lottery rather than an inherent advantage.
...
The lifecycle carbon footprint spans from the per-H100 embodied carbon quantified in @sec-sustainable-ai-embodied-carbon-assessment-9de0 to the thousands of megawatt-hours consumed during training. During autoregressive decode, accelerators can draw power while waiting on memory, which makes quantization and memory-optimized hardware important for both cloud and edge deployments. The Jevons rebound adds a demand constraint because efficiency gains do not reduce total impact when usage grows faster than per-query consumption falls.

Carbon budgets, power delivery, and cooling capacity impose measurable limits on fleet expansion. The Jevons Paradox shows why technical efficiency must be evaluated with demand growth: lower per-query cost can increase total consumption enough to offset the original savings. Sustainability therefore requires workload accounting and governance in addition to component-level optimization.
