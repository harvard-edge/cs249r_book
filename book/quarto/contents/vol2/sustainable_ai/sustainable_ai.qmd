...
start_chapter("vol2:sustainable_ai")
```
...
training_mwh_gpt3 = CarbonCostGPT3.training_mwh_str
grid_ci_us_kg = CarbonCostGPT3.grid_ci_us_kg
total_emissions_gpt3_kg_str = CarbonCostGPT3.total_emissions_kg_str
flight_ratio_gpt3_str = CarbonCostGPT3.flight_ratio_str
emissions_ratio_low_carbon_str = CarbonCostGPT3.emissions_ratio_str
```

::: {.callout-notebook title="The Carbon Cost of Training"}
**Problem**: You train a large model (GPT-3 size) consuming **`{python} training_mwh_gpt3` MWh**. How much CO2 did you emit, and how does that compare to a trans-Atlantic flight?

**The Math**:

1.  **Energy**: `{python} training_mwh_gpt3` MWh = 1,287,000 kWh.
2.  **Carbon Intensity (US Average)**: $\approx `{python} grid_ci_us_kg` \text{ kg CO}_2\text{/kWh}$.
3.  **Total Emissions**: $1,287,000 \times 0.429 \approx \mathbf{`{python} total_emissions_gpt3_kg_str` \text{ kg CO}_2}$.
4.  **Comparison**:
    *   One passenger, NY to London (round trip): $\approx 1,000 \text{ kg CO}_2$.
    *   **Ratio**: `{python} total_emissions_gpt3_kg_str` / 1,000 = \mathbf{`{python} flight_ratio_gpt3_str` \text{ flights}}$.

**The Systems Conclusion**: A single training run emits as much carbon as flying a Boeing 747 full of passengers across the Atlantic. **Optimization matters.** Moving this job to a hydro-powered region (0.02 kg/kWh) would reduce emissions by **`{python} emissions_ratio_low_carbon_str`x** to just ~25 flights.
:::
...
[^fn-dennard-scaling-end]: **Dennard Scaling**: Robert Dennard observed in 1974 that smaller transistors could operate at constant power density by reducing voltage proportionally. This ended around 2005 when leakage current made further voltage reduction impractical. The consequence for AI sustainability is direct: without Dennard scaling, each new process node no longer delivers proportional power savings, forcing the shift to specialized accelerators (GPUs, TPUs) that achieve efficiency through architectural parallelism rather than transistor physics. \index{Dennard Scaling!sustainability}
...
\tikzset{
    axis/.style={axis line style={draw=none}, grid=both, minor tick num=1, major grid style={Black}, minor grid style={GrayLine}},
    legend/.style={at={(0.15,0.9)}, anchor=north, fill=BrownL!40, draw=BrownLine, row sep=1.85pt, font=\footnotesize\usefont{T1}{phv}{m}{n}},
    plot style/.style={line width=2pt},
    red line/.style={mark=*, RedLine},
    blue line/.style={mark=triangle*, mark size=3pt, BlueLine},
    green line/.style={mark=square*, mark size=2.5pt, GreenLine},
    xlabel/.style={xlabel={Year}},
    ylabel/.style={ylabel={Electricity Usage (TWh)}}
}

\begin{axis}[axis, width=16cm, height=10cm, xmin=2009.5, xmax=2030, ymin=250, ymax=8300, 
    ...
    cycle multi list={RedLine,BlueLine,GreenLine\nextlist solid\nextlist mark=o,mark=none,mark=triangle,mark=none,mark=,mark=none}
]

\addplot+[red line] table[x=Date,y=Y, col sep=comma] {
Y,Date
500, 2010
510, 2012
520, 2014
...
\addplot+[blue line] table[x=Date,y=Y, col sep=comma] {
Y,Date
500, 2010
550, 2012
600, 2014
...
\addplot+[green line] table[x=Date,y=Y, col sep=comma] {
Y,Date
500, 2010
600, 2012
750, 2014
...
\node[fill=white, above=2pt of {(axis description cs:0.15,0.92)}, anchor=center]{\small\bfseries Scenario};
\end{axis}
\end{tikzpicture}
```
:::
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
energy_wall_gap_str = EnergyWallScenario.energy_wall_gap_str
battery_gain_pct = EnergyWallScenario.battery_gain_pct
grid_gain_pct = EnergyWallScenario.grid_gain_pct
```
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \tikzset{
    ...
    curve_steep/.style={line width=1.5pt, RedLine},
    curve_shallow/.style={line width=1.5pt, GreenLine},
    gap_line/.style={<->, line width=1.5pt, OrangeLine},
    anno_steep/.style={RedLine, font=\tiny\bfseries},
    anno_shallow/.style={GreenLine, font=\tiny\bfseries},
    anno_gap/.style={OrangeLine, rotate=90, font=\bfseries},
    year_label/.style={below},
    growth_label/.style={right}
  }
  ...
  % Axes
  \node[axis_line] (x-axis) at (0,0) {};
  \node[axis_line] (y-axis) at (0,5.5) {};
  \draw[axis_line] (x-axis.south west) -- (x-axis.east) node[growth_label] {Year};
  \draw[axis_line] (y-axis.north west) -- (y-axis.south) node[growth_label] {Growth (Log Scale)};

  % Compute Growth (Steep)
  \node[curve_steep] (compute_growth) at (0,0.5) {};
  \draw[curve_steep] (compute_growth) .. controls (2,1) and (5,4) .. (7,5) node[growth_label] {AI Compute Demand};
  \node[anno_steep] at (6, 4.5) {~350,000$\times$};

  % Grid/Battery Growth (Shallow)
  \node[curve_shallow] (grid_growth) at (0,0.5) {};
  \draw[curve_shallow] (grid_growth) -- (7,1.5) node[growth_label] {Grid/Battery Capacity};
  \node[anno_shallow] at (6, 1.0) {~20-80\%};

  % The Gap
  \draw[gap_line] (6.5, 1.4) -- (6.5, 4.7);
  \node[anno_gap] at (6.8, 3.0) {Sustainability Gap};

  % Ticks
  \node[year_label] at (0,0) {2012};
  \node[year_label] at (7,0) {2024};
...
Managing these transients requires **Energy Buffering**: using on-site battery arrays or massive capacitors to smooth the training iterations, ensuring the ML Fleet does not destabilize the local municipal power grid.
...
By treating heat as a **Byproduct** rather than a **Pollutant**, the fleet moves toward a "Circular Energy Economy."[^fn-pue-evolution]
...
3. **Sparsity and Sparse Interconnects**: In modern GPUs, the majority of energy is spent on data movement and global synchronization. High-efficiency systems use sparse representations where only 1-2% of parameters are active for any given task, reducing bandwidth and switching energy by 50-100$\times$.
...
These insights point toward promising research directions for sustainable AI. Architectures that implement **Spiking Neural Networks (SNNs)** or sparse activation patterns can achieve significant energy reductions by mimicking these sparse communication models [@prakash2023tinyml].[^fn-snn-efficiency] Similarly, local learning algorithms and self-supervised approaches offer pathways toward more sample-efficient and energy-conscious systems.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \tikzset{
    ...
    redbar/.style={bar, fill=RedL!70},
    orangebar/.style={bar, fill=OrangeL!70, minimum width=6.4cm},
    bluebar/.style={bar, fill=BlueL!70, minimum width=4.6cm},
    greenbar/.style={bar, fill=GreenL!70, minimum width=2.8cm}
  }
  ...
  % --- Bar 1: Current gap ---
  \node[redbar] (bar1) at (0, 0) {\hspace{0.15cm}350,000$\times$ energy gap};
  \node[label_left] at ([xshift=-0.15cm]bar1.west) {Current};

  % Intervention 1: Algorithmic
  \begin{scope}[xshift=6.4cm, yshift=-0.1cm]
    \draw[arrow, VioletLine] (0, 0) -- (0, -\vs+\bh+0.1);
    \node[intervention, VioletLine] at (0.2, -\vs/2+\bh/2+0.12) {Algorithmic Efficiency};
    \node[desc] at (0.2, -\vs/2+\bh/2-0.2) {Sparse models, MoE, pruning};
    \node[multiplier, VioletLine] at (-0.2, -\vs/2+\bh/2) {$\div$100$\times$};
  \end{scope}

  % --- Bar 2: After algorithmic ---
  \node[orangebar] (bar2) at (0, -\vs) {\hspace{0.15cm}3,500$\times$};
  \node[label_left] at ([xshift=-0.15cm]bar2.west) {After Algo.};

  % Intervention 2: Hardware
  \begin{scope}[xshift=4.6cm, yshift=-\vs-0.1cm]
    \draw[arrow, BlueLine] (0, 0) -- (0, -2*\vs+\bh+0.1);
    \node[intervention, BlueLine] at (0.2, -3*\vs/2+\bh/2+0.12) {Hardware Efficiency};
    \node[desc] at (0.2, -3*\vs/2+\bh/2-0.2) {Specialized accelerators, process nodes};
    \node[multiplier, BlueLine] at (-0.2, -3*\vs/2+\bh/2) {$\div$10$\times$};
  \end{scope}

  % --- Bar 3: After hardware ---
  \node[bluebar] (bar3) at (0, -2*\vs) {\hspace{0.15cm}350$\times$};
  \node[label_left] at ([xshift=-0.15cm]bar3.west) {After HW};

  % Intervention 3: Systemic
  \begin{scope}[xshift=2.8cm, yshift=-2*\vs-0.1cm]
    \draw[arrow, GreenLine] (0, 0) -- (0, -3*\vs+\bh+0.1);
    \node[intervention, GreenLine] at (0.2, -5*\vs/2+\bh/2+0.12) {Systemic Efficiency};
    \node[desc] at (0.2, -5*\vs/2+\bh/2-0.2) {Carbon-aware scheduling, renewables, heat reuse};
    \node[multiplier, GreenLine] at (-0.2, -5*\vs/2+\bh/2) {$\div$10$\times$};
  \end{scope}

  % --- Bar 4: Bridgeable gap ---
  \node[greenbar] (bar4) at (0, -3*\vs) {\hspace{0.15cm}35$\times$};
  \node[label_left] at ([xshift=-0.15cm]bar4.west) {Tractable};

  % Right brace showing combined reduction
  \draw[decorate, decoration={brace, amplitude=5pt, mirror}, line width=1.0pt]
    (10.3, \bh) -- (10.3, -3*\vs)
...
\tikzset{
  ...
  BoxOrange/.style={inner xsep=2pt,
...
% Concern Nodes
\node[BoxOrange,left=of G1](LG1){Inconclusive evidence};
\node[BoxOrange,below=of LG1](LG2){Inscrutable evidence};
\node[BoxOrange,below=of LG2](LG3){Misguided evidence};
\node[BoxOrange,below=of LG3](LG4){Unfair outcomes};

\node[BoxOrange,left=of G6](LG5){Transformative effects};
\node[BoxOrange,left=of G10](LG6){Traceability};
...
\foreach \x in {2,3,4,5,6}{
  \draw[link_line](LG1.west)--++(180:0.65)|-(LG\x);
}

\draw[brace_line] ([yshift=0mm,xshift=1mm]LG1.north east)--([yshift=0mm,xshift=1mm]LG3.south east);
\draw[brace_line] ([yshift=0mm,xshift=1mm]LG4.north east)--([yshift=0mm,xshift=1mm]LG5.south east);
...
# │ How: pulling H100_TDP from mlsys.constants.
# │
# │ Imports: mlsys.constants (H100_TDP, watt)
# │ Exports: h100_tdp_w
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import H100_TDP, watt
...
class GpuEmissionsScenario:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # (Direct constant pull)

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tdp_w = f"{h100_tdp.m_as(watt):.0f}"

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
h100_tdp_w = GpuEmissionsScenario.h100_tdp_w
```

[^fn-embodied-carbon-hardware]: **Embodied Carbon**: The CO₂ emitted during manufacturing, transport, and disposal before a device computes its first FLOP. A single H100 embodies 150-200 kg CO₂ from fabrication alone; at `{python} h100_tdp_w` W on the average U.S. grid, operational emissions match embodied carbon in roughly 1-2 years. As datacenters shift to renewables, embodied carbon's share of total lifetime emissions grows, potentially exceeding 30%, making hardware refresh cycles a first-order sustainability decision. \index{Embodied Carbon!hardware lifecycle}
...
The practical implications for AI systems follow directly from these physics. The quadratic voltage dependence means that reducing voltage from 1.0V to 0.8V decreases dynamic power by 36%, even before considering that lower voltages often enable frequency reduction with additional linear savings. This relationship explains why specialized AI accelerators operating at lower voltages but higher utilization can achieve order-of-magnitude efficiency improvements over general-purpose processors.
...
These facility-level metrics are not merely accounting tools; they identify where engineering intervention yields the greatest returns. The following case study demonstrates how ML-driven optimization of PUE translates directly into measurable energy savings.
...
The results demonstrated significant efficiency gains. When deployed in live data center environments, DeepMind's AI-driven cooling system reduced cooling energy consumption by 40%, leading to an overall 15% improvement in PUE[^fn-pue-deepmind] [@barroso2019datacenter; @evans2016deepmind]. For a facility operating at the industry-average PUE of 1.5 (@eq-pue), a 15% improvement reclaims a substantial fraction of the energy lost to cooling overhead. These improvements were achieved without additional hardware modifications, demonstrating the potential of software-driven optimizations to reduce AI's carbon footprint.

[^fn-pue-deepmind]: **PUE Optimization via ML**: Google's best facilities achieve PUE 1.08, meaning only 8% energy overhead for cooling and power distribution. DeepMind's reinforcement-learning controller reduced cooling energy by 40% by exploiting nonlinear interactions between chillers, pumps, and ambient conditions that rule-based systems miss. This is a rare positive feedback loop where AI improves the efficiency of the infrastructure that powers AI. \index{PUE!ML optimization}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  % Regions
  \node[region, low] (Quebec) {\textbf{Quebec}\\Hydro\\20g $CO_2$/kWh};
  \node[region, med, right=of Quebec] (Texas) {\textbf{Texas}\\Gas + Wind\\350g $CO_2$/kWh};
  \node[region, high, right=of Texas] (Poland) {\textbf{Poland}\\Coal\\800g $CO_2$/kWh};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9]
  ...
  % Axes
  \node[axis_line] (x-axis) at (0,0) {};
  \node[axis_line] (y-axis) at (0,0) {};
  \draw[->] (x-axis) -- (8,0) node[right] {Arithmetic Intensity (FLOP/Byte)};
  \draw[->] (y-axis) -- (0,5) node[above] {Energy Efficiency (FLOP/Joule)};
  ...
  % Ridge Point
  \node[dot, fill=black] (ridge) at (4,4) {};
  \draw[dashed] (4,0) -- (ridge);
  \node[below] at (4,0) {$AI_{crossover}$};

  % Regions
  \fill[region_mem] (0,0) -- (4,4) -- (4,0) -- cycle;
  \node[BlueLine, node_label] at (2.5, 1) {Data Movement Dominates};

  \fill[region_comp] (4,0) -- (4,4) -- (8,4) -- (8,0) -- cycle;
  \node[RedLine, node_label] at (6, 2) {Arithmetic Dominates};
...
# │
# │ Imports: mlsys.constants (A100_TDP, watt)
...
a100_tdp_w = TrainingEmissions.a100_tdp_w_str
```
...
# │ How: pulling A100_TDP from mlsys.constants.
# │
# │ Imports: mlsys.constants (A100_TDP, watt)
# │ Exports: a100_tdp_w
# └─────────────────────────────────────────────────────────────────────────────
...
class A100PowerScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_tdp_w = f"{a100_tdp.m_as(watt):.0f}"

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
a100_tdp_w = A100PowerScenario.a100_tdp_w
```
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line join=round]
  \tikzset{
    label_left/.style={left, align=right},
    label_right/.style={right, align=left},
    label_above/.style={above, font=\itshape},
    box/.style={draw, line width=0.75pt, fill opacity=0.3},
    embodied/.style={box, fill=EmbodiedColor!50},
    ops/.style={box, fill=OpsColor!50},
    value/.style={box, fill=ValueColor!40},
    node distance=1.5cm
  }
  ...
  % Input flows
  \node[embodied] (rawMaterials) at (0, 2.5) {\textbf{Raw Materials}\\Extraction \& Log.};
  \node[embodied] (chipFabrication) [below=of rawMaterials] {\textbf{Chip Fabrication}\\(Embodied Carbon)};
  \node[ops] (gridEnergy) [below=of chipFabrication] {\textbf{Grid Energy}\\(Coal/Gas/Renew.)};

  % Lifecycle phases
  \node[ops] (modelTraining) [right=3cm of rawMaterials] {\textbf{Model Training}};
  \node[ops] (inferenceServing) [below=of modelTraining] {\textbf{Inference / Serving}};

  % Output flow
  \node[value] (aiFleetIntelligence) [right=3cm of gridEnergy] {\textbf{AI Fleet Intelligence}\\(System Value)};

  % Labels
  \node[label_above, text=EmbodiedColor] at (1.5, 3) {Upstream};
  \node[label_above, text=OpsColor] at (4.5, 2.5) {Operational};
  \node[label_above, text=ValueColor] at (7.5, 2) {Downstream};
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
n_gpus_amort = EmbodiedCarbonAmort.n_gpus
t_job_hr = EmbodiedCarbonAmort.t_job_hr
op_carbon_dirty_kg = EmbodiedCarbonAmort.op_carbon_dirty_str
embodied_carbon_job_kg = EmbodiedCarbonAmort.embodied_carbon_job_str
op_carbon_clean_kg = EmbodiedCarbonAmort.op_carbon_clean_str
share_clean_pct_str = EmbodiedCarbonAmort.share_clean_pct_str
```
...
**Scenario**: Training a model for **`{python} t_job_hr` hours** on **`{python} n_gpus_amort` NVIDIA H100s**.
...
# │ How: pulling H100_TDP from mlsys.constants and converting to kW.
# │
# │ Imports: mlsys.constants (H100_TDP, watt)
# │ Exports: h100_tdp_kw
# └─────────────────────────────────────────────────────────────────────────────
...
class H100KwScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tdp_kw = f"{h100_tdp.m_as(watt) / 1000:.1f}"

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
h100_tdp_kw = H100KwScenario.h100_tdp_kw
```

*   **Operational**: 8 GPUs$\times$ `{python} h100_tdp_kw` kW$\times$ 10h = 56 kWh. At 0.4 kg/kWh (gas grid) = **`{python} op_carbon_dirty_kg` kg CO₂**.
...
*   **Amortization**: Lifetime = 3 years (26,280 hours).
    ...
    *   Job Cost = $0.046 \times 10 = \mathbf{`{python} embodied_carbon_job_kg` \text{ kg } CO_2}$.

**Conclusion**: For long-lived hardware in dirty grids, **electricity dominates** (`{python} op_carbon_dirty_kg` vs `{python} embodied_carbon_job_kg`). However, in **clean grids** (hydro, 0.02 kg/kWh), operational drops to **`{python} op_carbon_clean_kg` kg**, making embodied carbon a significant fraction (~`{python} share_clean_pct_str`%) of the total footprint.
:::
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \tikzset{
    label_pct/.style={font=\bfseries, align=center, text=white},
    label_desc/.style={align=left},
    label_desc_right/.style={anchor=east, align=right},
    pointer/.style={line width=1.0pt},
    box/.style={draw, line width=0.75pt, fill=white, minimum width=2cm, minimum height=1cm},
    arc/.style={draw, line width=0.75pt, fill=none}
  }

  \begin{scope}
    \fill[BlueLine] (0,0) -- (90:3) arc (90:-108:3) -- cycle;
    \fill[BlueL] (0,0) -- (-108:3) arc (-108:-198:3) -- cycle;
    \fill[RedLine] (0,0) -- (162:3) arc (162:108:3) -- cycle;
    \fill[RedL] (0,0) -- (108:3) arc (108:90:3) -- cycle;
    \fill[white] (0,0) circle (1.5);
  \end{scope}

  \node[label_pct] (op) at (-10:2.2) {55\%};
  \node[box, label_desc] (op_desc) at (3.2, 0) {\textbf{Operational}\\\small(Inference)};
  \draw[pointer] (10:2.8) -- (op_desc.west);

  \node[label_pct] (train) at (-153:2.2) {25\%};
  \node[box, label_desc_right] (train_desc) at (-3.2, -1.5) {\textbf{Operational}\\\small(Training)};
  \draw[pointer] (-153:2.8) -- (train_desc.east);

  \node[label_pct] (emb_man) at (135:2.2) {15\%};
  \node[box, label_desc_right] (emb_man_desc) at (-3.2, 2.1) {\textbf{Embodied}\\\small(Manufacturing)};
  \draw[pointer] (135:2.8) -- (emb_man_desc.east);

  \node[anchor=south, align=center] (end_life) at (0, 3.2) {\textbf{Embodied}\\\small(End-of-Life) \textbf{5\%}};
  \draw[pointer] (99:2.9) -- (end_life);
...
This programmatic approach allows teams to integrate total lifecycle carbon accounting directly into their orchestration dashboards. Calculating operational and embodied emissions for individual training runs, however, captures only one dimension of the problem. The macro-level patterns of how modern AI data centers consume resources at scale reveal additional constraints and optimization opportunities.
...
[^fn-datacenter-emissions-scale]: **Data Center Emissions Scale**: Data centers consume roughly 1% of global electricity, but including embodied carbon from hardware manufacturing pushes total emissions to approximately 2% of global CO₂, rivaling the aviation industry. The largest hyperscale facilities draw over 100 MW continuously, equivalent to powering 80,000 homes, and AI workloads are the fastest-growing segment of that demand. \index{Data Center!emissions scale}
...
Large-scale AI training inherently requires distributed systems coordination, creating additional energy overhead that compounds computational demands. The parallelism strategies examined in @sec-distributed-training-systems introduce network communication costs that can account for 20-40% of total energy consumption in large clusters.[^fn-parallelism-energy-overhead] This coordination across thousands of GPUs requires constant synchronization of computational updates and model parameters[^fn-gradient-sync-energy], generating data movement between nodes. This communication overhead scales poorly: doubling cluster size can increase networking energy consumption by 4$\times$ due to all-to-all communication patterns in gradient aggregation.

[^fn-parallelism-energy-overhead]: **Parallelism Energy Overhead**: Data, model, and pipeline parallelism each impose distinct communication patterns with different energy costs. Data parallelism broadcasts gradients (bandwidth-bound); model parallelism exchanges activations every layer (latency-bound); pipeline parallelism introduces bubble overhead (utilization-bound). GPT-3 combined all three, and the choice of parallelism strategy can swing total training energy by 20-40% for the same model. \index{Distributed Training!energy overhead}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
    \tikzset{
        benchmark/.style={fill=BlueL, draw=none, bar width=17pt},
        ...
        node near coord/.style={anchor=west, align=right, text=black, font=\sffamily},
        axis/.style={
    ...
    \begin{axis}[title={Common carbon footprint benchmarks}, title style={title}, axis]
        \addplot[benchmark] coordinates {
    ...
    \node[below=-3mm, align=center, font=\small\bfseries\usefont{T1}{phv}{m}{n}] at (current axis.north) {in kg of CO2 equivalent};
\end{tikzpicture}
```
:::
...
\tikzset{
    ...
    bar/.style={bar shift=0.5pt},
    legend/.style={at={(0.5,1.0)}, anchor=north, font=\small\usefont{T1}{phv}{m}{n}, draw=none},
    dashed line/.style={dashed, line width=1.0pt},
    axis line/.style={line width=1.0pt, -latex},
    y label/.style={anchor=south, rotate=90},
    legend entry/.style={above}
}
...
\begin{axis}[ 
    ...
    xticklabel style={align=right, rotate=90},
    tick label style={/pgf/number format/assume math mode=true},
    yticklabel style={font=\small\usefont{T1}{phv}{m}{n}, /pgf/number format/.cd, fixed, fixed zerofill, precision=2},
    symbolic x coords={LM, RM-1, RM-2, RM-3, RM-4, RM-5, BERT-NAS, Evolved Transformer, T5, Meena, GShard-600B, Switch Transformer, GPT-3},
]

    \addplot [fill=RedL, bar] coordinates {
        (LM, 0.06)
        (RM-1, 0.352)
        (RM-2, 0.243)
        (RM-3, 0.126)
    ...
    \addplot [fill=OrangeL, bar] coordinates {
        (LM, 0.115)
        (RM-1, 0.041)
        (RM-2, 0.026)
        (RM-3, 0.02)
    ...
    \addplot [fill=GreenL, bar] coordinates {
        (LM, 0)
        (RM-1, 0.497)
        (RM-2, 0.310)
        (RM-3, 0.265)
    ...
    \addplot [fill=BlueL, bar] coordinates {
        (LM, 0)
        (RM-1, 0)
        (RM-2, 0)
        (RM-3, 0)
    ...
    \node[y label] at (axis description cs:-0.07,0.9) {Millions};
\end{axis}

\begin{scope}
    \path[draw=dashed line] (axis cs:0.045,0) -- (axis cs:0.045,-0.65) coordinate(X1);
    \path[draw=dashed line] (axis cs:0.51,0) -- (axis cs:0.51,-0.65) coordinate(X2);
    \path[draw=dashed line] (axis cs:1.03,0) -- (axis cs:1.03,-0.65) coordinate(X3);
    \path[](X1) -- node[legend entry]{Facebook}(X2);
    \path[](X2) -- node[legend entry]{OSS Large-Scale ML Models}(X3);
...
[^fn-ghg-protocol-standard]: **GHG Protocol**: Developed jointly by the World Resources Institute and WBCSD, this framework is used by over 90% of Fortune 500 companies reporting to CDP. Its three-scope taxonomy matters for ML systems because most AI carbon hides in Scope 3 (hardware manufacturing, cloud compute supply chains), which companies historically underreport by 50-70% compared to Scopes 1 and 2. \index{GHG Protocol!scope taxonomy}
...
The intensive computational loads generate heat that cooling infrastructure must continuously remove, adding 30--40% to total energy requirements. Reducing this overhead requires co-optimization of hardware architecture, parallelism strategy, and algorithmic efficiency.
...
\tikzset{
  ...
  Box/.style={helvetica, anchor=south, inner xsep=2pt, draw=none, fill=BrownL, minimum width=30},
  InferenceLabel/.style={below=0.5 of $(B1.south)!0.5!(B2.south)$, anchor=north},
  TrainingLabel/.style={below=0.5 of $(B3.south)!0.5!(B4.south)$, anchor=north},
  BackgroundBox/.style={inner xsep=11, inner ysep=33, yshift=1mm, line width=0.75pt},
}

% First Diagram
\begin{scope}
  \node[Box, minimum height=80](B1){};
  \node[Box, minimum height=200](B2){};
  \node[Box, minimum height=175, fill=BrownL](B22){};
...
\begin{scope}[xshift=2cm]
  \node[Box, minimum height=20](B3){};
  \node[Box, minimum height=100](B4){};
  \node[Box, minimum height=80, fill=BrownL](B44){};
  \node[above=2pt of B3]{–1};
  \node[below=2pt of B44]{2025};
  \node[below=2pt of B3]{2017};
\end{scope}

\fill[fill=OliveL!10](B1.north east)--(B2.north west)--(B1.south east);
\node[InferenceLabel]{\textbf{Inference}};

\fill[fill=OliveL!10](B3.north east)--(B4.north west)--(B3.south east);
\node[TrainingLabel]{\textbf{Training}};

\begin{scope}[on background layer]
  \node[draw=VioletLine2, fill=VioletL2!50, BackgroundBox, fit=(B1)(B2)(B4)](BB1){};
...
\begin{scope}[xshift=10cm]
  ...
  \node[Box, minimum height=2mm, fill=BrownL](B1){};
  \node[Box, minimum height=90](B2){};
  \node[Box, minimum height=80, fill=BrownL](B22){};
...
\begin{scope}[xshift=12cm]
  \node[Box, minimum height=2mm, fill=BrownL](B3){};
  \node[Box, minimum height=31](B4){};
  \node[Box, minimum height=20, fill=BrownL](B44){};
...
\begin{scope}[on background layer]
  \node[draw=BlueLine, fill=BlueL!10, BackgroundBox, fit=(B0)(B2)(B4)](BB1){};
...
1.  **Phone Battery**: Typical capacity is $\sim$12.5 Wh (Watt-hours) $\approx$ 45,000 Joules.
2.  **Budget**: 5% of 45,000 J = **2,250 Joules**.
3.  **Training Cost**:
...
where $p_{\text{escalate}}$ is the probability of requiring cloud inference (typically 5--20% for well-designed cascades).
...
Semiconductor fabrication is an exceptionally water-intensive process. TSMC's fab in Arizona is projected to consume 34 million liters of water per day [@tsmc2023water][^fn-fab-water-scale], accounting for nearly 3% of the city's total water production. A single 300mm silicon wafer requires over 8,300 liters of water throughout the complete fabrication process. @fig-water_cycle illustrates the typical fab water cycle, where advanced recycling can reclaim 60--80% of water but still leaves a substantial consumption footprint.

[^fn-fab-water-scale]: **Semiconductor Water Scale**: TSMC's Arizona fab will consume 12 billion liters annually (37,000 Olympic pools), and advanced-node AI chips require 5-10$\times$ more water per die than older process nodes due to additional EUV and cleaning steps. This water dependency creates a direct sustainability constraint: fabs compete with municipal water supplies in drought-prone regions like Arizona and Taiwan, where semiconductor water demand can reach 3% of a city's total allocation. \index{Semiconductor Fabrication!water consumption}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm]
  ...
  % Nodes
  \node[stage] (Source) {Raw Water\\Source};
  \node[stage] (UPW) [right=of Source] {Ultra-Pure\\Water (UPW)};
  \node[stage] (Fab) [right=of UPW] {\textbf{Fab Process}\\Etching/Cleaning};
  \node[wasteStage] (Waste) [below=of Fab] {Wastewater\\Treatment};
  \node[stage] (Recycle) [left=of Waste] {Recycling\\Plant};
  \node[anchor=north] (Discharge) [below=of Waste] {Discharge};

  % Flows
  \draw[arrow] (Source) -- (UPW);
  \draw[arrow] (UPW) -- (Fab);
  \draw[arrow] (Fab) -- (Waste);
  \draw[arrow] (Waste) -- (Recycle);
  \draw[reclaimArrow] (Recycle) -- node[left] {Reclaim (60-80\%)} (UPW);
  \draw[dashedArrow] (Waste) -- (Discharge);
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line join=round]
 \tikzset{
    ...
    arrowbox/.style={signal,
        ...
        fill=BlueL!70,
        align=center},
    phase/.style = {arrowbox, fill=BlueL},
    phaseOrange/.style = {arrowbox, fill=OrangeL!80},
    phaseViolet/.style = {arrowbox, fill=VioletL2},
    phaseGreen/.style = {arrowbox, fill=GreenL},
    hand/.style = {line width=0.75pt, fill=OrangeL},
    factory/.style = {rectangle, draw, fill=BrownL, minimum height=15, minimum width=23, line width=1.0pt},
    mobile/.style = {rectangle, draw, minimum height=94, minimum width=47, rounded corners=6, line width=0.75pt, fill=RedL},
    component/.style = {comp, fill=BrownL!10},
    zigzag/.style = {GreenLine, decoration={zigzag, segment length=4pt, amplitude=0.5pt}, decorate}
}

\node[phase] (A1) {~~Design Phase};
\node[phaseOrange, right=of A1] (A2) {~~Manufacture Phase};
\node[phaseViolet, right=of A2] (A3) {~~Use Phase};
\node[phaseGreen, right=of A3] (A4) {~~Disposal Phase};

% Recycled
\begin{scope}[local bounding box=MOB, scale=0.7, every node/.append style={transform shape}, shift={($(A4.south)+(0,-2.05)$)}]
    \node[draw, fill=BrownL!70, minimum width=1.2cm, minimum height=1.2cm] (recycled) at (0,0) {};
    \draw[line width=2.5pt] (recycled.north west) -- (recycled.north east);
    \draw[line width=1.5pt] (recycled.south west) -- (recycled.south east);
\end{scope}

% Mobile
\begin{scope}[local bounding box=MOB, scale=0.4, every node/.append style={transform shape}, shift={($(A3.south)+(0,-2.45)$)}]
    \node[mobile] (R1) {};
    \node[rectangle, draw, minimum height=60, minimum width=32, line width=0.75pt, fill=white] (R2) at (0, -1.5) {};
    \node[circle, draw, minimum size=8, below= 2pt of R2, inner sep=0pt, line width=0.75pt, fill=BrownL] {};
    \node[rectangle, fill=black, minimum height=1, minimum width=20, above= 4pt of R2, inner sep=0pt, line width=0.75pt] {};
    % Hand
    \draw[hand] (0, -1.5) to[out=320,in=190] (0.5, -1.5) to[out=180,in=270, distance=15] (0, -1.5) -- (0, -2.5);
\end{scope}

% Factory
\begin{scope}[local bounding box=MOB, scale=1.4, every node/.append style={transform shape}, shift={($(A2.south)+(0,-1.05)$)}]
    \node[factory] (R1) {};
    \draw[fill=BrownL, line width=1.0pt] (R1.40) -- ++(110:0.2) -- ++(180:0.12) |- (R1.40);
    \draw[line width=2.5pt] (-0.8,-0.27) -- (0.55,-0.27);
    \foreach \x in{0.25,0.45,0.65} {
        \node[rectangle, fill=black, minimum height=2, minimum width=5, line width=0.75pt, inner sep=0pt] at ($(R1.north)!\x!(R1.south)$) {};
    }
\end{scope}
...
\begin{scope}[local bounding box=COMP, shift={($(A1.south)+(0,-1.05)$)}]
    ...
    \draw[zigzag] ($(CB1)+(0.3,0.05)$) -- ++(0:1.3);
    \draw[zigzag] ($(CB1)+(0.3,-0.12)$) -- ++(0:1.0);
    \draw[zigzag] ($(CB2)+(0.3,0.05)$) -- ++(0:1.3);
    \draw[zigzag] ($(CB2)+(0.3,-0.12)$) -- ++(0:1.0);
\end{scope}

% Pencil
\begin{scope}[rotate=300, scale=0.3, shift={($(COMP)+(-0.15,1.05)$)}]
    \fill[GreenL] (0,4) -- (0.4,4) -- (0.4,0) --(0.3,-0.15) -- (0.2,0) -- (0.1,-0.14) -- (0,0) -- cycle;
    \draw[color=yellow, line width=0.75pt] (0.2,4) -- (0.2,0);
    \fill[black] (0,3.5) -- (0.2,3.47) -- (0.4,3.5) -- (0.4,4) arc(30:150:0.23cm);
    \fill[BrownL!60] (0,0) -- (0.2,-0.8) node[coordinate,pos=0.75](a){} -- (0.4,0) node[coordinate,pos=0.25](b){} -- (0.3,-0.15) -- (0.2,0) -- (0.1,-0.14) -- cycle;
    \fill[GrayL] (a) -- (0.2,-0.8) -- (b) -- cycle;
\end{scope}

\node[above=0.9 of $(A2)!0.5!(A3)$] (LCA) {\textbf{Life Cycle Analysis}};
\node[above=0 of LCA] (AI) {AI System};
\end{tikzpicture}
```
:::
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
\tikzset{
    axis/.style={axis lines=left, axis line style={line width=1.0pt,-latex}},
    ylabel/.style={ylabel style={font=\footnotesize\usefont{T1}{phv}{m}{n}}},
    xticklabel/.style={align=right, font=\small\usefont{T1}{phv}{m}{n}},
    yticklabel/.style={font=\small\usefont{T1}{phv}{m}{n}, /pgf/number format/.cd, fixed, precision=1},
    nodes near coords/.style={/pgf/number format/assume math mode=true, font=\footnotesize\usefont{T1}{phv}{m}{n}, /pgf/number format/.cd, fixed, precision=2},
    grid style/.style={major grid style={dashed}},
    bar/.style={fill=RedL, bar width=7.5mm}
}

\begin{axis}[axis, ylabel=Connected devices in billions, ymin=0, ymax=34, width=136mm, height=65mm, scale only axis, xtick=data, ytick={0,5,10,15,20,25,30}, enlarge x limits=0.07, xticklabel, tick label style={/pgf/number format/assume math mode=true}, yticklabel, nodes near coords, grid=major]
\addplot[bar] coordinates {
    (2019, 8.6) (2020, 9.76) (2021, 11.28) (2022, 13.14) (2023, 15.14) (2024*, 17.8) (2025*, 19.08) (2026*, 21.09) (2027*, 23.14) (2028*, 25.21) (2029*, 27.31) (2030*, 29.42)
};
\end{axis}
\end{tikzpicture}
```
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
\tikzset{
  axis_line/.style={line width=1.0pt, -latex},
  curve_line/.style={line width=1.95pt, BlueLine},
  dashed_line/.style={dashed, line width=1.0pt},
  anno_text/.style={align=center, font=\footnotesize\usefont{T1}{phv}{m}{n}},
  fill_green/.style={fill=GreenL!60},
  fill_brown/.style={fill=BrownL!60},
  box/.style={draw, line width=0.75pt, fill=white},
  node_label/.style={anchor=center}
}

% Axes
\node[axis_line] (x-axis) at (0,0) {};
\node[axis_line] (y-axis) at (0,0) {};
\draw[axis_line] (x-axis) -- (9.0,0) coordinate(XT) node[below=1mm] {AI Usage};
\draw[axis_line] (y-axis) -- (0,5.2) coordinate(YT) node[below left=0mm and 3mm,align=right] {Cost of\\ AI Services};

% Curved line
\draw[curve_line, name path=G] (1,4.7) to[bend right=20] node[align=center,right=7mm,pos=0.3,text=black]{Demand Response \\ Curve for AI Usage} (8.5,1.9);
...
\draw[dashed_line] (X1) -- (T1) -- (T1-|YT) coordinate(Y2);
\draw[dashed_line] (X2) -- (T2) -- (T2-|YT) coordinate(Y1);
\node[node_label] at (Y2) [left] {A};
\node[node_label] at (Y1) [left] {B};
\draw[-latex] ($(Y2)!0.8!(Y2|-T1)$) -- node[anno_text,left]{50\% Drop\\ in Costs} ($(Y1)!0.8!(Y1|-T2)$);

% Consumption labels
\node[node_label] at (0,0) [below] {C};
\node[node_label] at (X1) [below] {C};
\node[node_label] at (X2) [below] {D};
\draw[-latex] ($(0,0)!0.8!(0,0|-Y2)$) -- ($(X1)!0.8!(X1|-Y2)$);
\draw[-latex] ($(X1)!0.8!(X1|-Y2)$) -- node[anno_text,below]{Consumption of tech. more than\\ doubles - total costs are higher} ($(X2)!0.8!(X2|-Y2)$);

% Background fills
\begin{scope}[on background layer]
  \fill[fill_green] (X1) rectangle (T2);
  \fill[fill_brown] (Y1) rectangle (T1);
\end{scope}

% Annotations
\node[anno_text] at ($(Y1)!0.5!(T1)$) {Savings  \\ from\\ reduced\\ AI costs};
\node[anno_text] at ($(X1)!0.5!(T2)$) {Savings are offset\\ by increased AI usage};
\end{tikzpicture}}
```
:::
...
[^fn-flops-vs-flops-energy]: **FLOPS vs. FLOPs**: FLOPS (all caps) measures rate (operations per second); FLOPs (lowercase s) measures count (total operations). The distinction matters for sustainability because energy scales with FLOPs (count), not FLOPS (rate). GPT-3 required $3.1 \times 10^{23}$ FLOPs total, and the energy cost per operation spans a `{python} efficiency_range_str`$\times$ range: CPUs at ~`{python} cpu_pj_per_flop` pJ/FLOP, GPUs at ~`{python} gpu_pj_per_flop`, TPUs at ~`{python} tpu_pj_per_flop`, and custom ASICs approaching `{python} asic_pj_per_flop` pJ/FLOP. \index{FLOPS!energy distinction}
...
[^fn-pruning-energy-impact]: **Pruning Energy Impact**: Structured pruning at 90% sparsity reduces inference energy by 2-10$\times$ because eliminated weights require neither storage nor computation, directly reducing both memory bandwidth and arithmetic. SparseGPT achieves 60% unstructured sparsity on LLMs with less than 1% accuracy loss, though realizing energy savings from unstructured sparsity requires hardware with native sparse execution support (e.g., NVIDIA's Sparse Tensor Cores). \index{Pruning!energy impact}
...
These TinyML-specific techniques enable sustainable AI deployment at billion-device scale: always-on sensor nodes achieving useful intelligence on harvested energy, eliminating the infrastructure, network, and power demands of cloud-dependent alternatives.
...
Algorithmic optimizations reduce per-operation energy, but the operational environment determines whether those savings translate to actual emissions reduction. Infrastructure-level innovations—renewable energy integration, carbon-aware workload scheduling, and AI-driven cooling optimization—address the physical context where computational efficiency gains are realized.
...
[^fn-pue-hyperscale-gap]: **PUE Gap**: The industry-average PUE of 1.67 means 40% of electricity powers cooling and infrastructure rather than computation, while Google's best facilities achieve 1.08 (only 7.4% overhead). For a 100 MW AI datacenter, this gap represents 59 MW of wasted power, enough to run 47,000 homes. Each 0.1 PUE improvement at hyperscale saves millions in annual electricity costs and hundreds of tons of CO₂. \index{PUE!hyperscale efficiency gap}
...
[^fn-carbon-scheduling-results]: **Carbon-Aware Scheduling at Scale**: Google's system achieved 15% carbon reduction through intra-region temporal shifting alone, and 40% globally by routing non-urgent batch training (70% of total workload) across time zones to chase renewable peaks. The key insight is that most training workloads are deadline-tolerant: a job that can accept a 6-hour delay gains access to dramatically different grid carbon intensities without any model or infrastructure changes. \index{Carbon-Aware Scheduling!results}
...
The cumulative impact of individual technical choices depends on systemic, industry-wide adoption. Without external pressure, market forces prioritize speed and scale over efficiency, requiring policy and regulatory frameworks to enforce sustainable practices globally.
...
end_chapter("vol2:sustainable_ai")
```
