...
1.  **Energy**: `{python} CarbonCostGPT3.training_mwh_str` MWh = 1,287,000 kWh.
2.  **Carbon Intensity (US Average)**: $\approx$ `{python} CarbonCostGPT3.grid_ci_us_kg` kg CO$_2$/kWh.
3.  **Total Emissions**: $1,287,000 \times 0.429 \approx$ **`{python} CarbonCostGPT3.total_emissions_kg_str` kg CO$_2$**.
4.  **Comparison**:
    *   One passenger, NY to London (round trip): $\approx 1,000 \text{ kg CO}_2$.
    *   **Ratio**: `{python} CarbonCostGPT3.total_emissions_kg_str` / 1,000 = **`{python} CarbonCostGPT3.flight_ratio_str` flights**.
...
RedLine] table[x=Date,y=Y, col sep=comma] {
Y,Date
500, 2010
510, 2012
520, 2014
...
\addplot+[mark=triangle*, mark size=3pt,BlueLine,
...
GreenLine] table[x=Date,y=Y, col sep=comma] {
Y,Date
500, 2010
600, 2012
750, 2014
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  % Axes
  \draw[axis_line] (0,0) -- (8,0) node[right] {Year};
  \draw[axis_line] (0,0) -- (0,5.5) node[above] {Growth (Log Scale)};

  % Compute Growth (Steep)
  \draw[curve_steep] (0,0.5) .. controls (2,1) and (5,4) .. (7,5) node[right] {AI Compute Demand};
  \node[anno_steep] at (6, 4.5) {~350,000$\times$};

  % Grid/Battery Growth (Shallow)
  \draw[curve_shallow] (0,0.5) -- (7,1.5) node[right] {Grid/Battery Capacity};
...
While AI logic follows the "Iron Law" of software optimization, energy follows the laws of chemistry and thermodynamics. Over the last 12 years, battery energy density has improved by only ~`{python} EnergyWallScenario.battery_gain_pct`%, and grid efficiency by ~`{python} EnergyWallScenario.grid_gain_pct`%. The `{python} EnergyWallScenario.energy_wall_gap_str`$\times$ gap between these two curves is the **Sustainability Wall**—the point where we can no longer "buy our way out" of the efficiency problem with more power.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  % --- Bar 1: Current gap ---
  \node[bar, fill=RedL!70, minimum width=10cm] (bar1) at (0, 0) {\hspace{0.15cm}350,000$\times$ energy gap};
  \node[label_left] at ([xshift=-0.15cm]bar1.west) {Current};

  % Intervention 1: Algorithmic
  \draw[arrow, VioletLine] (6.4, -0.1) -- (6.4, -\vs+\bh+0.1);
  \node[intervention, VioletLine] at (6.6, -\vs/2+\bh/2+0.12) {Algorithmic Efficiency};
  \node[desc] at (6.6, -\vs/2+\bh/2-0.2) {Sparse models, MoE, pruning};
  \node[multiplier, VioletLine] at (6.2, -\vs/2+\bh/2) {$\div$100$\times$};

  % --- Bar 2: After algorithmic ---
  \node[bar, fill=OrangeL!70, minimum width=6.4cm] (bar2) at (0, -\vs) {\hspace{0.15cm}3,500$\times$};
  \node[label_left] at ([xshift=-0.15cm]bar2.west) {After Algo.};

  % Intervention 2: Hardware
  \draw[arrow, BlueLine] (4.6, -\vs-0.1) -- (4.6, -2*\vs+\bh+0.1);
  \node[intervention, BlueLine] at (4.8, -3*\vs/2+\bh/2+0.12) {Hardware Efficiency};
  \node[desc] at (4.8, -3*\vs/2+\bh/2-0.2) {Specialized accelerators, process nodes};
  \node[multiplier, BlueLine] at (4.4, -3*\vs/2+\bh/2) {$\div$10$\times$};

  % --- Bar 3: After hardware ---
  \node[bar, fill=BlueL!70, minimum width=4.6cm] (bar3) at (0, -2*\vs) {\hspace{0.15cm}350$\times$};
  \node[label_left] at ([xshift=-0.15cm]bar3.west) {After HW};

  % Intervention 3: Systemic
  \draw[arrow, GreenLine] (2.8, -2*\vs-0.1) -- (2.8, -3*\vs+\bh+0.1);
  \node[intervention, GreenLine] at (3.0, -5*\vs/2+\bh/2+0.12) {Systemic Efficiency};
  \node[desc] at (3.0, -5*\vs/2+\bh/2-0.2) {Carbon-aware scheduling, renewables, heat reuse};
  \node[multiplier, GreenLine] at (2.6, -5*\vs/2+\bh/2) {$\div$10$\times$};

  % --- Bar 4: Bridgeable gap ---
  \node[bar, fill=GreenL!70, minimum width=2.8cm] (bar4) at (0, -3*\vs) {\hspace{0.15cm}35$\times$};
...
%
\node[BoxOrange,node distance=3.9,left =of G1](LG1){Inconclusive evidence};
...
%
\node[BoxOrange,node distance=3.9,left =of G6](LG5){Transformative effects};
\node[BoxOrange,node distance=3.9,left =of G10](LG6){Traceability};
...
node [BlueLine,midway,below=1mm] {};
%
\draw[brace_line]
...
1. **Overhead Reduction**: `{python} PueEfficiency.pue_old_str` - `{python} PueEfficiency.pue_new_str` = **0.48**.
2. **Annual Energy Savings**: $2.0 \text{ MW} \times 0.48 \times 8,760 \text{ hours} \approx$ **`{python} PueEfficiency.pue_savings_mwh_str` MWh**.
3. **Financial Savings**: `{python} PueEfficiency.pue_savings_mwh_str` MWh $\times$ \$70/MWh $\approx$ **\$`{python} PueEfficiency.pue_savings_usd_str`**.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line join=round]
  ...
  \node[label_left] at (0, 2.5) {\textbf{Raw Materials}\\Extraction \& Log.};

  \fill[EmbodiedColor!50] (0, 1.5) -- (3, 1.5) -- (3, 0.5) -- (0, 1) -- cycle;
  \node[label_left] at (0, 1.25) {\textbf{Chip Fabrication}\\(Embodied Carbon)};

  \fill[OpsColor!40] (0, 0) -- (3, 0.5) -- (3, -2.5) -- (0, -2) -- cycle;
  \node[label_left] at (0, -1) {\textbf{Grid Energy}\\(Coal/Gas/Renew.)};
  ...
  \node[label_right] at (9, 0.5) {\textbf{AI Fleet Intelligence}\\(System Value)};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \foreach \start/\end/\col in {90/-108/BlueLine, -108/-198/BlueL, 162/108/RedLine, 108/90/RedL} {
      \fill[\col] (0,0) -- (\start:3) arc (\start:\end:3) -- cycle;
  }
  \fill[white] (0,0) circle (1.5);

  \node[label_pct] at (-10:2.2) {55\%};
  \node[anchor=west, label_desc] at (3.2, 0) {\textbf{Operational}\\\small(Inference)};
  \draw[pointer] (10:2.8) -- (3.1, 0);

  \node[label_pct] at (-153:2.2) {25\%};
  \node[label_desc_right] at (-3.2, -1.5) {\textbf{Operational}\\\small(Training)};
  \draw[pointer] (-153:2.8) -- (-3.1, -1.5);

  \node[label_pct] at (135:2.2) {15\%};
  \node[label_desc_right] at (-3.2, 2.1) {\textbf{Embodied}\\\small(Manufacturing)};
  \draw[pointer] (135:2.8) -- (-3.1, 2.1);

  \node[anchor=south, align=center] at (0, 3.2) {\textbf{Embodied}\\\small(End-of-Life) \textbf{5\%}};
  \draw[pointer] (99:2.9) -- (0, 3.2);
...
%
  \addplot [fill=RedL, bar shift=0.5pt] coordinates {
                    (LM, 0.06)
                    (RM-1, 0.352)
                    (RM-2, 0.243)
                    (RM-3, 0.126)
...
\addlegendentry{Offline Training~~~~}
  ...
  \addplot [fill=OrangeL, bar shift=0.5pt] coordinates {
                    (LM, 0.115)
                    (RM-1, 0.041)
                    (RM-2, 0.026)
                    (RM-3, 0.02)
...
\addlegendentry{Online Training~~~~}
  \addplot [fill=GreenL, bar shift=0.5pt] coordinates {
                    (LM, 0)
                    (RM-1, 0.497)
                    (RM-2, 0.310)
                    (RM-3, 0.265)
...
\addlegendentry{Inference}
  \addplot [fill=BlueL, bar shift=0.5pt] coordinates {
                    (LM, 0)
                    (RM-1, 0)
                    (RM-2, 0)
                    (RM-3, 0)
...
\end{axis}
%
\draw[dashed, line width=1.0pt] ({rel axis cs:0.045,0}) --({rel axis cs:0.045,-0.65})coordinate(X1);
\draw[dashed, line width=1.0pt] ({rel axis cs:0.51,0}) -- ({rel axis cs:0.51,-0.65})coordinate(X2);
\draw[dashed, line width=1.0pt] ({rel axis cs:1.03,0}) -- ({rel axis cs:1.03,-0.65})coordinate(X3);
...
\begin{scope}
\node[Box](B1){};
\node[Box,minimum height=80,fill=BrownL](B11){};
...
\node[Box,minimum height=20,fill=BrownL](B3){};
...
%%%
\scoped[on background layer]
\node[draw=BlueLine,inner xsep=11,inner ysep=33,yshift=1mm,
           fill=BlueL!10,fit=(B0)(B2)(B4),line width=0.75pt](BB1){};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm]
  ...
  \node[below=of Waste] (Discharge) {Discharge};
  ...
  \draw[arrow, BlueLine] (Recycle) -- node[left] {Reclaim (60-80\%)} (UPW);
  \draw[arrow, dashed] (Waste) -- (Discharge);
...
\node[arrowbox, fill=OrangeL!80, right=of A1] (A2) {~~Manufacture Phase};
\node[arrowbox, fill=VioletL2, right=of A2] (A3) {~~Use Phase};
\node[arrowbox, fill=GreenL, right=of A3] (A4) {~~Disposal Phase};
%%%
%recycled
...
\draw[line width=1.5pt,fill=BrownL!70](A)--(B)--(C)--(D)--cycle;
...
\node[rectangle,draw,minimum height=94,minimum width=47,
            rounded corners=6,line width=0.75pt,fill=RedL](R1){};
\node[rectangle,draw,minimum height=60,minimum width=32,line width=0.75pt,fill=white](R2){};
...
%hand
\draw[line width=0.75pt,fill=OrangeL](D)--(G)to[out=320,in=190] (D1)%--++(180:0.05)
...
\node[rectangle,draw,minimum height=12,minimum width=23,line width=0.75pt,
            rounded corners=2.5,rotate=35,fill=OrangeL]at(P3)(PR2){};
\node[rectangle,draw,minimum height=12,minimum width=23,line width=0.75pt,
            rounded corners=2.5,rotate=35,fill=OrangeL]at(P4)(PR3){};
\node[rectangle,draw,minimum height=12,minimum width=19,line width=0.75pt,
            rounded corners=2.5,rotate=35,fill=OrangeL]at(P5)(PR4){};
\draw[line width=0.75pt,fill=OrangeL](P1)--(P11)to[out=355,in=5,distance=9] (P22)--(P2)--cycle;
%\fill[BlueL](D4)circle(1pt);
\end{scope}
%%%
%factory
\begin{scope}[local bounding box=MOB,scale=1.4, every node/.append style={transform shape},
                          shift={($(A2.south)+(0,-1.05)$)}]
\node[rectangle,draw,fill=BrownL,minimum height=15,minimum width=23,line width=1.0pt](R1){};
\draw[fill=BrownL,line width=1.0pt]($(R1.40)+(0,-0.01)$)--++(110:0.2)--++(180:0.12)|-($(R1.40)+(0,-0.01)$);
\draw[line width=1.0pt,fill=GreenL](-0.68,-0.27)--++(88:0.9)--++(0:0.15)--(-0.48,-0.27)--cycle;
\draw[line width=2.5pt](-0.8,-0.27)--(0.55,-0.27);

\foreach \x in{0.25,0.45,0.65}{
\node[rectangle,fill=black,minimum height=2,minimum width=5,line width=0.75pt,inner sep=0pt]
...
% Axses
\draw[axis_line] (0,0)--(9.0,0)coordinate(XT) node[below=1mm] {AI Usage};
\draw[axis_line] (0,0)--(0,5.2)coordinate(YT)
              node[below left=0mm and 3mm,align=right] {Cost of\\ AI Services};
% curved line
\draw[curve_line, name path=G] (1,4.7) to[bend right=20]
...
\draw[-latex]($(Y2)!0.8!(YY2)$)--
            node[anno_text,left]{50\% Drop\\ in Costs}($(Y1)!0.8!(YY1)$);
%
...
\draw[-latex]($(X1)!0.8!(XX1)$)--
            node[anno_text,below]{Consumption of tech. more than\\
