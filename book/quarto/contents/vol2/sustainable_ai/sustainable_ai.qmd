...
The biological model points toward promising research directions for sustainable AI. Architectures that implement **Spiking Neural Networks (SNNs)**\index{Spiking Neural Network}\index{SNN} or sparse activation patterns can achieve significant energy reductions by mimicking sparse communication models[^fn-snn-efficiency] [@prakash2023tinyml]. Local learning algorithms and self-supervised approaches offer additional pathways toward more sample-efficient and energy-conscious systems.
...
This is the Jevons paradox of AI (Principle \ref{pri-jevons-paradox}): making models 10$\times$ more efficient can increase total usage enough to erase or even exceed the expected energy savings. Sustainability strategies must therefore focus on **absolute limits**\index{Absolute Limits} (carbon budgets, renewable sourcing) rather than just rate efficiency (FLOP/s per watt).
