...
The grid, siting, and heat-reuse constraints above become severe because frontier training concentrates energy demand into long, synchronized campaigns. OpenAI's GPT-3 exemplifies this scale: training required 1,287 megawatt-hours of electricity, equivalent to powering roughly 122 US homes for an entire year[^fn-gpt3-energy-scale] [@maslej2023artificial]. This energy consumption reflects the computation required to train modern large language models on large datasets.[^fn-training-communication-overhead]
...
Development-time carbon tracking\index{Carbon Footprint!tracking} matters when it feeds placement and complexity decisions rather than becoming a report after the fact. @Lst-carbon-tracking demonstrates how the CodeCarbon library wraps model training to capture real-time emissions data, enabling data-driven sustainability decisions.
...
The purpose of such tracking is to make environmental cost visible while model complexity, region, and schedule are still adjustable, before the training run has already consumed its energy.
...
energy_joules = (
    ((end_energy - start_energy) * ureg.microjoule)
    .to(ureg.joule)
    .magnitude
)
    ...
    with torch.no_grad():
        for _ in range(num_iterations):
            ...
            power_readings.append(
                (power_mw * milliwatt).to(watt).magnitude
            )
...
For accurate energy measurement rather than instantaneous power sampling, integrate power readings over time or use NVIDIA's energy counter when available on data center GPUs. Edge devices and microcontrollers present fundamentally different measurement challenges: they lack built-in power counters, operate at milliwatt rather than kilowatt scales, and require external instrumentation for accurate energy profiling. As TinyML deployments expand to billions of devices, edge energy measurement becomes essential for comprehensive sustainability assessment.

#### Hardware power monitors for embedded systems {#sec-sustainable-ai-hardware-power-monitors-embedded-systems-c6c3}

Microcontrollers and edge processors require external current and voltage measurement to quantify energy consumption. The relevant decision is how much temporal resolution, rail attribution, and cost the workload justifies:
...
class PueSavings:
    ...
    gpt3_training = Models.Language.GPT3.training_energy_mwh
    ...
    household_mwh_per_year = gpt3_training / gpt3_households
...
For sub-watt TinyML deployments, MLPerf Tiny provides the same discipline at microcontroller scale. @Tbl-mlperf-tiny summarizes benchmark tasks and typical energy requirements spanning from sub-millijoule to multi-millijoule ranges. The measurement methodology requires external power monitors like those discussed in @sec-sustainable-ai-hardware-power-monitors-embedded-systems-c6c3 and specifies warm-up periods, measurement windows, and statistical reporting requirements so that tiny efficiency claims can be reproduced across submissions.
