...
***Sustainable AI***\index{Sustainable AI!definition} is the engineering discipline that elevates _environmental impact_ to a _first-class design constraint_ alongside traditional performance and cost objectives in machine learning systems development.
...
```{python}
#| label: fig-energy-gap
#| fig-cap: "**The Energy Gap**. US datacenter electricity consumption tripled from 58 TWh (2014) to 176 TWh (2023), driven primarily by AI workloads. LBNL projects a further doubling to 325 to 580 TWh by 2028. At the high end, datacenters would consume approximately 12% of US electricity, representing a physical constraint on AI scaling that no software optimization can overcome."
#| fig-alt: "Bar chart showing US datacenter electricity: 58 TWh in 2014, 176 TWh in 2023, projected 325 to 580 TWh by 2028. Bars colored blue for historical and red for projections."
#| echo: false
import numpy as np
import matplotlib.pyplot as plt
from mlsys import viz
...
***Jevons Paradox in AI***\index{Jevons Paradox!definition} is the phenomenon where improvements in ML model efficiency (FLOPs per inference, energy per token) lead to increased total resource consumption, because lower cost per query stimulates proportionally greater demand. Efficiency optimizations such as **quantization**, **pruning**, and **distillation** reduce per-inference cost but historically expand total inference volume, making aggregate energy consumption grow despite per-unit improvements.
