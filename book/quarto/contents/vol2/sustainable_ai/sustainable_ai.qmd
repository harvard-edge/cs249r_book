...
The third governance concern determines long-term viability: the resource constraints that govern whether systems remain economically and environmentally sustainable at scale. A model can be secure against adversaries and robust under distribution shift and still fail if it cannot be powered.
...
# │
# │ Imports: mlsysim.Models; mlsysim.core.units; mlsysim.fmt helpers
# │ Exports: model_params_b_str, train_flops_math, pue_str, efficiency_gflop_per_j_str
# └─────────────────────────────────────────────────────────────────────────────
...
3. Discuss whether this metric captures the true environmental cost, considering the difference between energy consumption (MWh) and carbon intensity, the grams of CO~2~ the grid emits per kilowatt-hour delivered (g CO~2~/kWh).
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.tdp, watt)
...
::: {#nbk-sustainable-ai-automated-carbon-aware-scheduling-tier-3-optimizer .callout-notebook title="Automated carbon-aware placement"}
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.tdp, kilowatt)
...
class LifecycleCarbonEstimate:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    res = SustainabilityModel().solve(fleet=fleet, datacenter=baseline_grid, duration_days=training_days, mfu=1.0)  # Peak training
...
# │
# │ Imports: mlsysim.fmt
# │ Exports: v_high_str, v_low_str, power_reduction_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, MarkdownStr, check
...
class VoltageScalingDynamicPower:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    # Fixed parallel voltage labels (1.0 V / 0.8 V); the load-bearing computed
    # value is the V^2 power reduction below.
    v_high_str = MarkdownStr(f"{v_high:.1f}")
    v_low_str = MarkdownStr(f"{v_low:.1f}")
...
# │          mlsysim.core.units (BYTES_FP32), mlsysim.fmt (fmt, MarkdownStr, check)
...
class MatMulEnergyAnalysis:
    ...
    comp_large_eq_math = MarkdownStr(rf"$2 \times 10^9 \times {e_compute:.0f} \text{{ pJ}} = {comp_large_pj/1e9:.0f}$")
    mem_large_eq_math = MarkdownStr(rf"$3 \times 10^6 \times {bytes_fp32:.0f} \times {e_move:.0f} \text{{ pJ}} = {mem_large_pj/1e9:.2f}$")
    vec_bytes_eq_math = MarkdownStr((rf"$3N_{{\text{{vec}}}} \times {bytes_fp32:.0f} = {vec_bytes:,.0f}$").replace(",", "{,}"))
...
The model must capture two terms. Consider a data center running on 100 percent renewable hydroelectric power. Its operational carbon emissions are effectively zero, but AI trained there is *not* carbon-free. Mining the silicon, manufacturing the GPUs, and pouring the concrete for the data center released thousands of tons of CO~2~ before the servers were ever turned on. A true carbon footprint calculation must account for both the energy consumed during operation and the "embodied carbon" emitted during construction.
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.A100.tdp, watt, hour, ureg, THOUSAND — module-wide)
...
# │
# │ Imports: mlsysim.Infrastructure; mlsysim.core.units; mlsysim.fmt
# │ Exports: low_eff_pct_str, high_eff_pct_str, rack_power_kw_str, gain_pct_str,
# │          per_rack_saved_kw_str, n_racks_str, facility_saved_kw_str,
# │          additional_nodes_str, additional_racks_str, lifecycle_years_str,
# │          rate_str, savings_usd_str
# └─────────────────────────────────────────────────────────────────────────────
...
class DistributionEfficiencySavings:
    ...
    rate_str = fmt_usd(rate.to("dollar/kWh").magnitude, precision=2, commas=False, suffix="/kWh")
...
- [ ] **Grid stress**: Why does synchronous ML training create more stress on the power grid than asynchronous web traffic?
- [ ] **Power ramp**: What is the "Power Ramp" problem, and how do supercapacitor banks address it?
- [ ] **Power budgeting**: If a node has `{python} H100TdpRackRecap.node_gpus_str` at `{python} H100TdpRackRecap.tdp_w_str` each, why is the total node power budgeted at ~`{python} H100TdpRackRecap.node_power_budget_kw_str` rather than `{python} H100TdpRackRecap.node_gpu_only_kw_str`?
- [ ] **AC-DC conversion**: True or False: Converting high-voltage AC to low-voltage DC at the rack level is more efficient than doing it at the server level.
...
# │ Show: 70 MW IT (100,000 × 700 W) → ~78 MW facility at the cluster PUE.
# │ How: IT = n_gpus × H100 TDP; facility = IT × effective_pue.
# │
# │ Imports: mlsysim.Hardware, Systems; mlsysim.core.units; mlsysim.fmt
# │ Exports: cluster_gpus_str, gpu_tdp_w_str, it_power_mw_str, facility_power_mw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Systems
from mlsysim.fmt import fmt, fmt_power, fmt_qty, check
from mlsysim.core.units import watt, MW
...
# │
# │ Imports: mlsysim.core.units; mlsysim.fmt
# │ Exports: facility_power_mw_str, facility_energy_gwh_str, capacity_factor_pct_str,
# │          wind_energy_gwh_str, wind_equiv_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.units, mlsysim.fmt helpers
# │ Exports: cp_j_str, delta_t_k_str, heat_load_kw_str, heat_load_w_str,
# │          mass_flow_kg_per_s_str, volumetric_flow_m3_per_s_str, cfm_str,
# │          air_density_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import ureg
from mlsysim.core.units import watt, kW, J, second
from mlsysim.fmt import fmt, fmt_int, fmt_qty, check

_kg = ureg.kilogram
_K = ureg.kelvin
_m3 = ureg.meter**3
_cfm_unit = ureg.feet**3 / ureg.minute

class AirCoolingMassFlow:
    # ┌── 1. LOAD ──────────────────────────────────────────
    # Physical / scenario constants (no canonical mlsysim home yet):
    cp = 1005 * J / (_kg * _K)  # specific heat capacity of air at constant pressure
    delta_t = 15 * _K  # inlet 20 C → outlet 35 C
    air_density = 1.2 * _kg / _m3  # sea-level dry air
    heat_load = 100 * kW  # rack heat load to remove

    # ┌── 2. EXECUTE ───────────────────────────────────────
    mass_flow = (heat_load / (cp * delta_t)).to(_kg / second)
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    cp_j_str = fmt(cp.magnitude, precision=0, commas=True)
    delta_t_k_str = fmt_qty(delta_t, unit=_K, precision=0, commas=False)
    delta_t_value_str = fmt(delta_t.magnitude, precision=0, commas=False)
    heat_load_kw_str = fmt_qty(heat_load, unit=kW, precision=0, commas=False)
    heat_load_w_str = fmt_int(int(heat_load.to(watt).magnitude))
    air_density_str = fmt_qty(air_density, unit=_kg / _m3, precision=1, commas=False)
    mass_flow_kg_per_s_str = fmt_qty(mass_flow, unit=_kg / second, precision=1, commas=False)
...
where $Q$ is the heat removed (watts), $\dot{m}$ is the mass flow rate (kg/s), $c_p$ is the specific heat capacity (J/kg/K), and $\Delta T$ is the temperature difference between outlet and inlet. For air with $c_p$ of `{python} AirCoolingMassFlow.cp_j_str` J/kg/K and a typical $\Delta T$ of `{python} AirCoolingMassFlow.delta_t_k_str` (inlet at 20 degrees C, outlet at 35 degrees C), removing `{python} AirCoolingMassFlow.heat_load_kw_str` requires a mass flow rate of `{python} AirCoolingMassFlow.heat_load_w_str` $\div$ (`{python} AirCoolingMassFlow.cp_j_str` $\times$ `{python} AirCoolingMassFlow.delta_t_value_str`) $\approx$ `{python} AirCoolingMassFlow.mass_flow_kg_per_s_str`. At sea-level air density of `{python} AirCoolingMassFlow.air_density_str`, this corresponds to a volumetric flow rate of `{python} AirCoolingMassFlow.volumetric_flow_m3_per_s_str`, or approximately `{python} AirCoolingMassFlow.cfm_str` CFM (cubic feet per minute). Moving this much air through the confined space of a server rack requires powerful fans that themselves consume substantial power.
...
2.  **Observation**: Unlike Computing Efficiency (which focuses on FLOPs per Watt), PUE focuses on Facility Efficiency\index{Efficiency!facility}: it captures how much energy is "wasted" before it even reaches the processor.
3.  **Pitfall**: A frequent misconception is that a low PUE means a "green" data center. In reality, PUE only measures Efficiency, not the Carbon Intensity of the energy source; a coal-powered data center can have a better PUE than a solar-powered one while having a much higher environmental impact.
...
# │
# │ Imports: mlsysim.Systems, Infrastructure; mlsysim.core.units; mlsysim.fmt
# │ Exports: failed_gpus_str, cluster_gpus_str, idle_gpus_str, repair_hours_str,
# │          gpu_hour_rate_str, direct_cost_str, cluster_cost_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Systems, Infrastructure
...
The economics of cooling reliability shift dramatically when moving from independent inference servers to tightly coupled training clusters. In a distributed training run using synchronous parallelism, a single rack failure halts the entire job. Consider a cooling failure that triggers a thermal shutdown of a `{python} StragglerFailureCost.failed_gpus_str`-GPU rack within a `{python} StragglerFailureCost.cluster_gpus_str`-GPU cluster. The direct hardware cost is negligible, but the opportunity cost is immense. If the repair time for a CDU pump is `{python} StragglerFailureCost.repair_hours_str`, the immediate loss of `{python} StragglerFailureCost.failed_gpus_str` GPUs at `{python} StragglerFailureCost.gpu_hour_rate_str` per GPU-hour is only `{python} StragglerFailureCost.direct_cost_str`. However, because the training algorithm requires all workers to proceed in lockstep, the remaining `{python} StragglerFailureCost.idle_gpus_str` GPUs also sit idle, burning electricity without making progress. This straggler effect inflates the cost to `{python} StragglerFailureCost.cluster_cost_str` in lost compute time. When adding the overhead of checkpoint retrieval and the rollback to the last saved state -- often losing 30 to 60 minutes of computation -- the total financial impact of a single cooling component failure easily exceeds \$200,000. The nonlinear scaling of failure costs makes N+1 redundancy in cooling loops a mathematical necessity for training economics.
...
# │
# │ Imports: mlsysim.Scenarios; mlsysim.core.units.watt;
...
class WakeWordPower:
    ...
    avg_power_eq = MarkdownStr(f"$$P_{{\\text{{average}}}} = 0.01 + 0.05 \\times 0.1 + " f"0.001 \\times 10 \\times 0.05 = {avg_power_mw_str}$$")
```
...
Carbon-aware scheduling is fundamentally a **load shifting software problem**\index{Load Shifting}\index{Load Shifting!software}. The scheduler queries real-time grid carbon intensity APIs (for example, ElectricityMap, WattTime) to execute two core operations:
...
* **Carbon-aware scheduling**: Geographic placement is a high-leverage sustainability choice when jobs can move. Moving a training job from a coal-powered grid to a hydro-powered one can reduce emissions by the geographic factor established in @sec-sustainable-ai-scale-environmental-impact-ac9a (8 to 40 times for representative region pairs) without changing a single line of code.
