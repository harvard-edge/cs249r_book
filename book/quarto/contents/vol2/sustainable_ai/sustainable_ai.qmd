...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_qty, fmt_energy, fmt_emissions, check
from mlsysim.physics.quantities import carbon_from_energy
...
class CarbonFrontier:
    ...
    energy_kwh_str = fmt(energy_kwh.magnitude, precision=0)
    quebec_g_str = fmt(quebec_carbon.to(gram).magnitude, precision=0)
    poland_g_str = fmt(poland_carbon.to(gram).magnitude, precision=0)
    ...
    intensity_quebec_str = fmt(intensity_quebec.to(gram / kWh).magnitude, precision=0, commas=False)
    intensity_poland_str = fmt(intensity_poland.to(gram / kWh).magnitude, precision=0, commas=False)
```
...
1. **Site A Emissions**: `{python} CarbonFrontier.energy_kwh_str` $\times$ `{python} CarbonFrontier.intensity_quebec_str` g/kWh = `{python} CarbonFrontier.quebec_g_str` g = `{python} CarbonFrontier.cf_quebec_tonnes_str` $\text{CO}_2$.
2. **Site B Emissions**: `{python} CarbonFrontier.energy_kwh_str` $\times$ `{python} CarbonFrontier.intensity_poland_str` g/kWh = `{python} CarbonFrontier.poland_g_str` g = `{python} CarbonFrontier.cf_poland_tonnes_str` $\text{CO}_2$.
...
class TrainingEmissions:
    ...
    emissions_kg = total_carbon.to(kilogram).magnitude
    emissions_tons = total_carbon.to(metric_ton).magnitude
    quebec_emissions_kg = quebec_carbon.to(kilogram).magnitude
...
class EmbodiedCarbonAmort:
    ...
    t_job = 10 * hour
    mfg_carbon_per_gpu = 164 * kilogram  # NVIDIA H100 product carbon footprint (even split)
    lifetime_years = 3
...
class TrainingEmbodiedRecap:
    ...
    n_gpus_str = fmt(n_gpus, precision=0, commas=False)
    training_days_str = fmt(training_days, precision=0, commas=False)
    embodied_training_kg_str = fmt_emissions(embodied_training_kg * kilogram, precision=1)
...
from mlsysim.core.units import MW, ureg
...
class GridQueue:
    ...
    queue_str = fmt_power(grid_queue_size_gw * ureg.gigawatt, precision=0, commas=False)
...
from mlsysim.core.units import watt, kilowatt, microsecond, ureg
...
class H100TdpRackRecap:
    ...
    power_rate_gws_str = fmt_qty(power_rate.to(ureg.gigawatt / second), ureg.gigawatt / second, precision=2, commas=False)
...
class RackPowerBudget:
    ...
    host_kw = 3.2 * kilowatt
    nvswitch_kw = 1.6 * kilowatt
    ib_kw = 0.8 * kilowatt
    conversion_kw = 2.8 * kilowatt
    cooling_kw = 2.7 * kilowatt
...
class PueSavings:
    ...
    savings_15_to_11_usd = saved_15_to_11.to(kWh).magnitude * electricity_rate.to(USD / kWh).magnitude
    savings_158_to_110_usd = saved_158_to_110.to(kWh).magnitude * electricity_rate.to(USD / kWh).magnitude
...
class H100TdpCoolingRecap:
    ...
    annual_savings_usd = savings.to(kilowatt).magnitude * hours_per_year.to(hour).magnitude * electricity_rate.to(USD / kWh).magnitude
    lifecycle_years = 3
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Scenarios
from mlsysim.fmt import fmt, fmt_energy, fmt_percent, check, fmt_count
...
class OnDeviceLearningEnergy:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    phone_battery_j = phone_battery.to(joule).magnitude
    ...
    model_params_b_str = fmt_count(model_params, scale='B', precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.units import milliwatt, watt, ureg
from mlsysim.fmt import fmt, fmt_power, check, MarkdownStr

class WakeWordPower:
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────────
    vad_power = 0.01 * milliwatt
    speech_activity = 0.05
    wake_detector_power = 0.1 * milliwatt
    wake_occurrence = 0.001
    full_model_power = 10 * milliwatt
    ...
    avg_power_uw = avg_power.to(ureg.microwatt).magnitude
