...
class ArchetypeATdp:
    ...
    cluster_accel_mw_str = fmt_qty(cluster_accel_mw * ureg.megawatt, ureg.megawatt, precision=1, commas=False)
```
...
class CarbonFrontier:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cf_energy_mwh_str = fmt_qty(energy_mwh * ureg.megawatt_hour, ureg.megawatt_hour, precision=0, commas=True)
...
from mlsysim.fmt import MarkdownStr, fmt, fmt_qty, check
...
class SustainableMobilePowerEnvelope:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sustained_range_str = MarkdownStr(
        f"{fmt(sustained_low_w, precision=0, commas=False)}–"
        f"{fmt_qty(sustained_high_w * watt, watt, precision=0, commas=False)}"
    )
    burst_range_str = MarkdownStr(
        f"{fmt(burst_low_w, precision=0, commas=False)}–"
        f"{fmt_qty(burst_high_w * watt, watt, precision=0, commas=False)}"
    )
```
