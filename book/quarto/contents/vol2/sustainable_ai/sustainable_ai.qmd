...
[^fn-pjmac-energy-hierarchy]: **pJ/FLOP and pJ/MAC**\index{Multiply-Accumulate}: Energy-efficiency specifications often mix floating-point and multiply-accumulate operations (where 1 MAC = 2 FLOPs), requiring conversion across unit conventions and precision levels. The simplified hierarchy used here aligns with later comparisons in this chapter: CPUs at ~100 pJ/FLOP, GPUs around 10 pJ/FLOP for dense tensor ops, TPUs around 1--2 pJ/FLOP, and custom low-precision ASICs approaching 0.1 pJ/op (see @tbl-appdx-assumptions-energy-hierarchy for exact energy per operation scaling across FP64 to INT4 data formats and 7nm, 5nm, and 3nm silicon nodes). Choosing the right hardware tier for a given workload can reduce energy consumption by 100--1,000$\times$ without any algorithmic changes.
