...
class CarbonCostGPT3:
    ...
    flight_co2 = Literature.Sustainability.TransatlanticRoundTripCo2Kg * kilogram
    ...
    total_emissions_kg_str = fmt_emissions(total_carbon.to(kilogram), unit=kilogram, precision=0, commas=True)
    ...
    kg_per_flight_str = fmt_emissions(flight_co2, precision=0, commas=False)
...
4.  **Comparison**:
    *   One passenger, NY to London (round trip): $\approx$ `{python} CarbonCostGPT3.kg_per_flight_str`.
...
class ArchetypeATdp:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h_h100_tdp_w_str = fmt_power(tdp, precision=0, commas=False)
    cluster_gpus_str = fmt(cluster_gpus, precision=0, commas=True)
    cluster_accel_mw_str = fmt_power(accelerator_power, precision=1, commas=False)
```
...
class CarbonFrontier:
    ...
    energy_kwh_str = fmt_energy(energy_kwh, unit=kWh, precision=0, commas=True)
    quebec_g_str = fmt_emissions(quebec_carbon, unit=gram, precision=0, commas=True)
    poland_g_str = fmt_emissions(poland_carbon, unit=gram, precision=0, commas=True)
    cf_quebec_tonnes_str = fmt_emissions(quebec_carbon.to(metric_ton), precision=0)
    cf_poland_tonnes_str = fmt_emissions(poland_carbon.to(metric_ton), precision=0)
    cf_ratio_str = fmt(ratio, precision=0, commas=False)
    intensity_quebec_str = fmt_carbon_intensity(intensity_quebec, precision=0, commas=False)
    intensity_poland_str = fmt_carbon_intensity(intensity_poland, precision=0, commas=False)
```
...
- **Site A (Quebec)**: Hydropower, `{python} CarbonFrontier.intensity_quebec_str` $\text{CO}_2$.
- **Site B (Poland)**: Coal-heavy, `{python} CarbonFrontier.intensity_poland_str` $\text{CO}_2$.
How does the location affect a model's carbon footprint?

**Math**:
Carbon = Energy $\times$ Grid Intensity.

1. **Site A Emissions**: `{python} CarbonFrontier.energy_kwh_str` $\times$ `{python} CarbonFrontier.intensity_quebec_str` = `{python} CarbonFrontier.quebec_g_str` = `{python} CarbonFrontier.cf_quebec_tonnes_str` $\text{CO}_2$.
2. **Site B Emissions**: `{python} CarbonFrontier.energy_kwh_str` $\times$ `{python} CarbonFrontier.intensity_poland_str` = `{python} CarbonFrontier.poland_g_str` = `{python} CarbonFrontier.cf_poland_tonnes_str` $\text{CO}_2$.
...
::: {.column-margin}
![](images/svg/vol2_sustainable_ai_margin_001.svg){width="100%" fig-alt="Margin illustration showing Quebec versus Poland carbon-intensity ladder."}
...
class LifecycleCarbonEstimate:
    ...
    facility_power = accelerator_power * DummyFleet.effective_pue
    ...
    h_h100_tdp_w_str = fmt_power(h_h100.tdp, precision=0, commas=False)
    ...
    operational_tonnes_str = fmt_emissions(operational_carbon, precision=1)
    embodied_kg_per_gpu_str = fmt(embodied_kg_per_gpu, precision=0, commas=False)
    embodied_tonnes_str = fmt_emissions(embodied_carbon, precision=1, commas=False)
    total_tonnes_str = fmt_emissions(total_carbon, precision=1)
```
...
class GpuEmissionsScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h_h100_tdp_w_str = fmt_power(h_h100_tdp, precision=0, commas=False)
```
...
class PueEfficiency:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    pue_it_mw_str = fmt_power(p_it, precision=0, commas=False)
    ...
    pue_savings_mwh_str = fmt_energy(annual_savings, precision=1, commas=True)
...
::: {.column-margin}
![](images/svg/vol2_sustainable_ai_margin_002.svg){width="100%" fig-alt="Margin illustration showing IT base plus infrastructure overhead from PUE 1.58 to 1.10."}
...
::: {.column-margin}
![](images/svg/sustainable_ai_energy_per_byte_ladder.svg){width="100%" fig-alt="Six-rung energy staircase, per-byte cost rising down the memory hierarchy on a log scale: register 0.1, L1 1, L2 5, DRAM 160, NVMe 1,000, network 10,000 pJ per byte, spanning five orders of magnitude."}
...
# │ Imports: mlsysim (Literature.Energy.Move*, Hardware.Tech.Memory.DRAM.energy_per_byte),
...
class MemoryHierarchyEnergy:
    ...
    dram = Hardware.Tech.Memory.DRAM.energy_per_byte.to(pJ / byte).magnitude
...
# │
# │ Imports: mlsysim (Literature.Energy.EffGpuPjFlop, Hardware.Tech.Memory.DRAM.energy_per_byte),
...
from mlsysim.core.units import byte, pJ
...
class TrainingEmissions:
    ...
    quebec_ci_str = fmt_carbon_intensity(quebec_ci, unit=gram / kWh, precision=0, commas=False)
    gpu_energy_wh_str = fmt_energy(gpu_energy, precision=1, commas=True)
    gpu_energy_kwh_str = fmt_energy(gpu_energy_kwh, precision=1, commas=True)
    facility_energy_kwh_str = fmt_energy(facility_energy_kwh, precision=1, commas=True)
    emissions_kg_str = fmt_emissions(total_carbon.to(kilogram), precision=1)
    emissions_tons_str = fmt_emissions(total_carbon.to(metric_ton), precision=1, commas=False)
    quebec_emissions_kg_str = fmt_emissions(quebec_carbon.to(kilogram), precision=1)
    emissions_ratio_str = fmt(emissions_ratio, precision=1, commas=False)
    a100_tdp_w_str = fmt_power(a100_tdp, precision=0, commas=False)
    embodied_kg_per_accelerator_str = fmt_emissions(embodied_carbon_per_accelerator, unit=kilogram, precision=0, commas=False)
...
- Emissions: `{python} TrainingEmissions.facility_energy_kwh_str` $\times$ `{python} TrainingEmissions.quebec_ci_str` g/kWh = `{python} TrainingEmissions.quebec_emissions_kg_str`
...
class EmbodiedCarbonAmort:
    ...
    op_energy_kwh_str = fmt_energy(op_energy.to(kWh), precision=0, commas=False)
    ci_dirty_str = fmt_qty(ci_dirty, kilogram / kWh, precision=3, commas=False)
    ci_clean_str = fmt_qty(ci_clean, kilogram / kWh, precision=3, commas=False)
    mfg_carbon_per_gpu_str = fmt_emissions(mfg_carbon_per_gpu, precision=0, commas=False)
    total_mfg_carbon_str = fmt_emissions(total_mfg_carbon, unit=kilogram, precision=0, commas=False)
    ...
    op_carbon_dirty_str = fmt_emissions(op_carbon_dirty.to(kilogram), precision=1, commas=False)
    op_carbon_clean_str = fmt_emissions(op_carbon_clean.to(kilogram), precision=1, commas=False)
    embodied_carbon_job_str = fmt_emissions(embodied_carbon_job.to(kilogram), precision=2, commas=False)
...
*   **Operational**: `{python} EmbodiedCarbonAmort.n_gpus_str` $\times$ `{python} EmbodiedCarbonAmort.h_h100_tdp_kw_str` $\times$ `{python} EmbodiedCarbonAmort.t_job_hr_str` = `{python} EmbodiedCarbonAmort.op_energy_kwh_str`. At `{python} EmbodiedCarbonAmort.ci_dirty_str` (US-average grid) = `{python} EmbodiedCarbonAmort.op_carbon_dirty_str`.
*   **Embodied**: `{python} EmbodiedCarbonAmort.n_gpus_str` $\times$ `{python} EmbodiedCarbonAmort.mfg_carbon_per_gpu_str` = `{python} EmbodiedCarbonAmort.total_mfg_carbon_str`.
*   **Amortization**: Lifetime = `{python} EmbodiedCarbonAmort.lifetime_years_str` (`{python} EmbodiedCarbonAmort.total_lifetime_hr_str`).
    *   Hourly "Rent" = `{python} EmbodiedCarbonAmort.total_mfg_carbon_str` / `{python} EmbodiedCarbonAmort.total_lifetime_hr_str` $\approx$ `{python} EmbodiedCarbonAmort.hourly_rent_kg_str`.
    *   Job Cost = `{python} EmbodiedCarbonAmort.hourly_rent_kg_str` $\times$ `{python} EmbodiedCarbonAmort.t_job_hr_str` = `{python} EmbodiedCarbonAmort.embodied_carbon_job_str`.

**Systems insight**: For long-lived hardware in dirty grids, electricity dominates (`{python} EmbodiedCarbonAmort.op_carbon_dirty_str` vs. `{python} EmbodiedCarbonAmort.embodied_carbon_job_str`). However, in clean grids (hydro, `{python} EmbodiedCarbonAmort.ci_clean_str`), operational drops to `{python} EmbodiedCarbonAmort.op_carbon_clean_str`, making embodied carbon a significant fraction (~`{python} EmbodiedCarbonAmort.share_clean_pct_str`) of the total footprint.
...
class TrainingEmbodiedRecap:
    ...
    n_gpus_str = fmt(n_gpus, precision=0, commas=False)
    training_days_str = fmt(training_days, precision=0, commas=False)
    embodied_training_kg_str = fmt_emissions(embodied_training_carbon, precision=1)
...
class InferenceLifecycleExample:
    ...
    daily_energy_kwh_str = fmt_energy(daily_energy, precision=0, commas=True)
    annual_energy_kwh_str = fmt_energy(annual_energy, precision=2, commas=True)
    grid_ci_kg_per_kwh_str = fmt_qty(grid_ci, kilogram / kWh, precision=3, commas=False)
    annual_emissions_kg_str = fmt_emissions(annual_carbon.to(kilogram), unit=kilogram, precision=0, commas=True)
    annual_emissions_tonnes_str = fmt_emissions(annual_carbon.to(metric_ton), precision=1)
    days_per_year_str = fmt(days_per_year, precision=0, commas=False)
...
class GridQueue:
    ...
    queue_str = fmt_power(grid_queue_size_gw * GW, precision=0, commas=False)
    ...
    cluster_power_mw_str = fmt_power(cluster_power, precision=0, commas=False)
```
...
class H100TdpRackRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_str = fmt_power(gpu_tdp, precision=0, commas=False)
    cluster_gpus_str = fmt(cluster_gpus, precision=0, commas=False)
    comm_phase_w_str = fmt_power(comm_phase, precision=0, commas=False)
    power_delta_w_str = fmt_power(power_delta, precision=0, commas=False)
...
class RackPowerBudget:
    ...
    gpu_tdp_w_str = fmt_power(gpu_tdp, precision=0, commas=False)
...
To appreciate the magnitude of these transients, consider a `{python} H100TdpRackRecap.cluster_gpus_str`-GPU cluster transitioning from communication phase (`{python} H100TdpRackRecap.comm_phase_w_str` per GPU average) to matrix multiplication phase (`{python} H100TdpRackRecap.tdp_str` per GPU). The power delta is `{python} H100TdpRackRecap.power_delta_w_str` $\times$ `{python} H100TdpRackRecap.cluster_gpus_str` = `{python} H100TdpRackRecap.power_delta_kw_str`, and this transition occurs in approximately `{python} H100TdpRackRecap.transition_us_str` microseconds. The rate of power change is therefore `{python} H100TdpRackRecap.power_delta_kw_str`/`{python} H100TdpRackRecap.transition_us_str` $\mu$s = `{python} H100TdpRackRecap.power_rate_gws_str`. No passive electrical component can respond at this rate; only energy storage devices (supercapacitors) positioned physically close to the load can absorb the transient before it propagates into the building's electrical distribution.
...
# │ Exports: SustainableCoolingRackPowerRecap.rack_power_str
...
class SustainableCoolingRackPowerRecap:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    node_gpu_power = Hardware.Cloud.H100.tdp * Systems.Nodes.DGX_H100.accelerators_per_node
    nodes_per_rack = 4
    non_gpu_power = 10.3 * kilowatt  # scenario assumption

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    rack_power = node_gpu_power * nodes_per_rack + non_gpu_power
...
class PueSavings:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    diff_15_to_11_mw_str = fmt_power(diff_15_to_11, precision=1, commas=False)
    diff_158_to_110_mw_str = fmt_power(diff_158_to_110, precision=1, commas=False)
...
Consider a `{python} H100TdpCoolingRecap.n_gpus_str`-GPU cluster of H100s at `{python} H100TdpCoolingRecap.tdp_str` each.
...
# │ How: Pull Scenarios.MobilePower anchors, validate ordering, format ranges.
# │
...
class SustainableMobilePowerEnvelope:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    sustained_low = Scenarios.MobilePower.MobileMlSustainedLow
    sustained_high = Scenarios.MobilePower.MobileMlSustainedHigh
    burst_low = Scenarios.MobilePower.MobileMlBurstLow
    burst_high = Scenarios.MobilePower.MobileMlBurstHigh
...
class OnDeviceLearningEnergy:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    phone_battery = Scenarios.PhoneBattery.EnergyWh
    ...
    joules_per_token_str = fmt_qty(joules_per_token, joule, precision=0, commas=False, per="token")
...
3.  **Training Cost**\index{On-Device Learning!training cost}:
    ...
    *   Total per token: `{python} OnDeviceLearningEnergy.train_nj_per_param_str` nJ/param $\times 10^9$ params = `{python} OnDeviceLearningEnergy.joules_per_token_str`.
4.  **Capacity**: `{python} OnDeviceLearningEnergy.budget_j_str` / `{python} OnDeviceLearningEnergy.joules_per_token_str` = `{python} OnDeviceLearningEnergy.token_capacity_str`.
...
::: {.column-margin}
![](images/svg/vol2_sustainable_ai_margin_004.svg){width="100%" fig-alt="Margin illustration showing INT32 add, FP32 multiply, and wireless transmitted bit energy."}
