...
$$P_{total} = P_{dynamic} + P_{static} = \alpha C V^2 f + V I_{\text{leak}}$$ {#eq-cmos-power}
...
$$E_{\text{total}} = (E_{CPU} + E_{GPU} + E_{\text{memory}} + E_{\text{network}}) \times PUE$$ {#eq-total-energy}
...
$$C_{\text{operational}} = E_{\text{total}} \times CI_{\text{grid}} \times PUE$$ {#eq-operational-carbon}

where $E_{\text{total}}$ is the energy consumed by IT equipment, $CI_{\text{grid}}$ is the carbon intensity of the electricity grid, and $PUE$ accounts for facility overhead. A concrete *training emissions calculation* illustrates this framework.
...
$$C_{embodied,daily} = \frac{C_{\text{manufacturing}}}{L_{lifetime} \times 365}$$ {#eq-embodied-daily}
