...
from mlsysim.fmt import fmt, fmt_time, fmt_power, fmt_energy, fmt_emissions, check
...
class TrainingEmissions:
    ...
    grid_ci_g_per_kwh_str = fmt(us_grid.carbon_intensity_g_kwh, precision=0, commas=False)
    quebec_ci_str = fmt(quebec_grid.carbon_intensity_g_kwh, precision=0, commas=False)
    ...
    embodied_kg_per_accelerator_str = fmt(embodied_carbon_per_accelerator.to(kilogram).magnitude, precision=0, commas=False)
    embodied_lifetime_years_str = fmt(embodied_lifetime_years, precision=0, commas=False)
    embodied_daily_kg_str = fmt(embodied_daily_carbon.to(kilogram).magnitude, precision=3, commas=False)
    embodied_training_kg_str = fmt(embodied_training_carbon.to(kilogram).magnitude, precision=1)
...
- Operational emissions: `{python} TrainingEmissions.facility_energy_kwh_str` $\times$ `{python} TrainingEmissions.grid_ci_g_per_kwh_str` g/kWh = `{python} TrainingEmissions.emissions_kg_str` = `{python} TrainingEmissions.emissions_tons_str`
...
class H100TdpRackRecap:
    ...
    transition_us_str = fmt(transition.to(microsecond).magnitude, precision=0, commas=False)
