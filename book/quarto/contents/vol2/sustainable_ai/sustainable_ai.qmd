...
**Scenario**: A team is planning a large training run requiring `{python} AutoPlacement.energy_mwh_str` of energy, choosing between three regions with different electricity prices and carbon intensities.

**Problem**: With an internal carbon tax of `{python} AutoPlacement.carbon_tax_str`, which region minimizes the true Total Cost of Ownership (TCO)\index{Total Cost of Ownership!carbon-aware placement}?
...
The biological model points toward promising research directions for sustainable AI. Architectures that implement **Spiking Neural Networks (SNNs)**\index{Spiking Neural Network}, event-driven models that communicate through discrete spikes, or sparse activation patterns can achieve significant energy reductions by mimicking sparse communication models[^fn-snn-efficiency] [@prakash2023tinyml]. Local learning algorithms and self-supervised approaches offer additional pathways toward more sample-efficient and energy-conscious systems.
...
Consider matrix multiplication $\mathbf{C} = \mathbf{A} \times \mathbf{B}$ for $N_{\text{mat}}{\times}N_{\text{mat}}$ matrices in FP32 precision on a GPU with the energy characteristics above.
...
- Bytes: $3N_{\text{mat}}^2 \times 4$ bytes (read matrices $\mathbf{A}$ and $\mathbf{B}$, write matrix $\mathbf{C}$, each FP32 = 4 bytes)
- Arithmetic intensity: $\text{AI} = \frac{2N_{\text{mat}}^3}{12N_{\text{mat}}^2} = \frac{N_{\text{mat}}}{6}$ FLOP/byte

**Step 2**: Determine the energy-limiting factor. @Tbl-matmul-energy-bottleneck-comparison compares the three regimes directly.
...
- **Setup**: Execute 100 or more inference iterations before measurement during the **warm-up period**\index{Warm-Up Period} to reach thermal equilibrium, as initial iterations may exhibit different power characteristics due to cache warming and voltage regulator settling.
- **Constraint**: Account for the **duty cycle**\index{Duty Cycle!accounting} by reporting both peak inference power and average power at realistic duty cycles, because edge devices typically operate with significant idle periods between inferences.
- **Isolation**: Disable or account for **peripheral**\index{Peripheral Isolation} power consumption, such as sensors, radios, and displays, when measuring model inference energy, because these can dominate total system power.
...
- IT energy after rack-profile support load: `{python} TrainingEmissions.it_energy_kwh_str`
- Facility PUE: `{python} TrainingEmissions.pue_str` (efficient hyperscale data center)
- Total facility energy: `{python} TrainingEmissions.it_energy_kwh_str` $\times$ `{python} TrainingEmissions.pue_str` = `{python} TrainingEmissions.facility_energy_kwh_str`
...
- Quebec grid intensity: `{python} TrainingEmissions.quebec_ci_g_per_kwh_str` (low-carbon grid)
- Emissions: `{python} TrainingEmissions.facility_energy_kwh_str` $\times$ `{python} TrainingEmissions.quebec_ci_g_per_kwh_str` = `{python} TrainingEmissions.quebec_emissions_kg_str`
...
[^fn-nas-carbon-cost]: **Neural Architecture Search (NAS) Carbon Cost**\index{Neural Architecture Search!carbon cost}: The roughly 284,000 kg CO~2~e figure from @strubell2019 is the original Evolved Transformer NAS estimate, not the cost of training one final model. @patterson2021carbon later showed that the estimate drops sharply when the search's proxy-task setup, Google's TPU hardware, data-center efficiency, and grid mix are accounted for. The systems lesson is boundary discipline: search, tuning, and failed trials belong inside the accounting boundary, but reusable architecture search should be amortized across the models and domains that reuse the discovered design. This concern helped motivate efficient NAS work: weight sharing, continuous relaxations, and hardware-aware search reduce the search budget, so the meta-optimization of *how* we search for architectures is itself a sustainability lever [@elsken2019].
...
The **GHG Protocol**\index{GHG Protocol}[^fn-ghg-protocol-standard] framework [@ghgprotocol2023] provides the standard categorization for these emissions, summarized in @fig-ghg-protocol. Three scopes provide an engineering classification checklist:
...
[^fn-pue-metric]: **PUE (Power Usage Effectiveness)**\index{PUE!efficiency metric}: Defined by The Green Grid consortium in 2007 as $P_{\text{total}} / P_{\text{IT}}$. Google's fleet-wide PUE averages 1.10; the industry average is ~1.58. For a 10,000-GPU cluster consuming `{python} PueSavings.it_power_mw_str` of IT power after rack support load, reducing PUE from 1.58 to 1.10 saves `{python} PueSavings.diff_158_to_110_mw_str` of cooling overhead -- roughly `{python} PueSavings.savings_158_to_110_musd_str` per year in electricity and the equivalent of removing `{python} PueSavings.households_158_to_110_str` residential homes from the grid. ML-specific facilities with direct-to-chip liquid cooling have demonstrated PUE values of 1.03--1.08.
...
- **Right-to-repair**\index{Right-to-Repair}: Legislative and regulatory movements push back against repair restrictions by emphasizing access to parts, tools, diagnostics, and service information [@ftc2021nixingfix].
...
[^fn-carbon-scheduling-results]: **Carbon-Aware Scheduling at Scale**\index{Carbon-Aware Scheduling!at scale}\index{Carbon-Aware Scheduling!results}: Google's carbon-aware data center study reports 15 percent carbon reduction through intra-region temporal shifting alone, and 40 percent globally by routing nonurgent batch compute across time zones to chase renewable peaks [@radovanovic2021carbonaware]. The key insight is that delay-tolerant workloads gain access to dramatically different grid carbon intensities without any model or infrastructure changes.
...
- **Measure first**\index{Sustainable AI!measurement first}: Tools like CarbonTracker and CodeCarbon track the emissions of training runs. Teams cannot improve what they do not measure, and establishing baseline metrics is essential for validating the effectiveness of optimization efforts [@anthony2020carbontracker; @schmidt2021codecarbon].
...
- **Optimize the model**\index{Sustainable AI!model optimization}: Avoid training the largest model possible by default. Pruning, quantization, and knowledge distillation find the smallest model that meets accuracy targets. A 90 percent accurate model requiring 10 percent of the resources often provides better real-world value than a 95 percent accurate model requiring full resources.
...
- **Select efficient hardware**\index{Sustainable AI!efficient hardware selection}: Energy-efficient accelerators, such as TPUs or specialized inference chips, reduce deployment costs. The full hardware lifecycle and workload-specific platform selection matter as much as raw throughput.
...
Beyond measurement mandates, governments are exploring direct restriction mechanisms. These include setting limits on computational power available for training large AI models, mirroring **Emissions Trading Systems (ETS)**\index{Emissions Trading System}[^fn-ets-compute-cap] used in environmental policy. Such "cap-and-trade" systems for compute would force organizations to operate within predefined energy budgets or procure additional capacity, creating a market for computational carbon credits. Carbon pricing can make electricity source and workload location economic scheduling variables. The EU **Carbon Border Adjustment Mechanism (CBAM)**\index{Carbon Border Adjustment Mechanism}, by contrast, applies to embedded emissions in specified imported goods and electricity; it does not generally price an AI service according to the regional grid where its computation ran.
...
Internal carbon pricing is another effective self-regulatory tool. Assigning a "shadow price" to carbon emissions integrates environmental costs directly into financial decision-making for AI projects, naturally prioritizing investments in energy-efficient hardware and low-emission models. Voluntary checklists and open-source tools promote accountability when they feed those same project decisions: projects like **CodeCarbon**\index{CodeCarbon} and **ML CO~2~ Impact**\index{ML CO2 Impact} provide frameworks that allow developers to estimate and track model carbon footprints directly within their workflows [@schmidt2021codecarbon; @lacoste2019quantifying].
