...
# Model parameters
gpt3_params_b = f"{GPT3_PARAMS.to(param).magnitude / 1e9:.0f}"  # "175"
```

This chapter's position in the book's organizing framework, *the Systems Sandwich*, clarifies why energy and environmental constraints are not external concerns but physical limits that bound what the entire system can achieve.
...
To appreciate the scale of the problem, consider the *carbon cost of training* a single frontier model.
...
[^fn-pue-metric]: PUE, established in @sec-compute, measures facility overhead relative to compute power. Google's best facilities achieve 1.08, demonstrating the optimization targets achievable through AI-driven efficiency improvements.
...
The following diagram illustrates how geographic optimization can dramatically reduce carbon emissions by choosing training locations strategically.
...
### Carbon Footprint Calculation {#sec-sustainable-ai-carbon-footprint-calculation-c2fe-c2fe}
...
- GPU energy: 64 GPUs times `{python} a100_tdp_w`W times 336h = 8,601,600 Wh = 8,602 kWh
...
#### Embodied Carbon Assessment {#sec-sustainable-ai-embodied-carbon-assessment-9de0-assessment-9de0}
...
*   **Operational**: 8 GPUs $\times$ `{python} h100_tdp_kw` kW $\times$ 10h = 56 kWh. At 0.4 kg/kWh (gas grid) = **22.4 kg $CO_2$**.
...
#### Lifecycle Carbon Accounting {#sec-sustainable-ai-lifecycle-carbon-accounting-99a1-accounting-99a1}
...
# Energy efficiency range across hardware
cpu_pj_per_flop = 100
gpu_pj_per_flop = 10
tpu_pj_per_flop = 1
...
[^fn-flops-vs-flops]: **FLOPS vs FLOPs**: FLOPS (all caps) = Floating-Point Operations Per Second (rate), while FLOPs (mixed case) = total Floating-Point Operations (count). GPT-3 training required 3.1×10²³ FLOPs total, executed on hardware capable of 1.25×10¹⁷ FLOPS. Energy efficiency varies dramatically across hardware: CPUs consume ~`{python} cpu_pj_per_flop` pJ/FLOP, GPUs achieve ~`{python} gpu_pj_per_flop` pJ/FLOP, TPUs reach ~`{python} tpu_pj_per_flop` pJ/FLOP, while specialized AI accelerators approach `{python} asic_pj_per_flop` pJ/FLOP—a `{python} efficiency_range_str`× efficiency range that defines sustainability opportunities.
