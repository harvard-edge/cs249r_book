...
When an engineer optimizes a database query to save one hundred milliseconds, it is considered standard performance tuning. When that same query is executed billions of times a day across a global data center, however, that one-hundred-millisecond savings translates to megawatts of electrical power and tons of avoided carbon emissions [@lacoste2019quantifying]. Sustainable AI ceases to be a theoretical ethical concern once we recognize that power density is the absolute physical ceiling on data center computational capacity; energy is the ultimate currency of machine learning.[^fn-joule-ml]
...
class MatMulEnergyAnalysis:
    ...
    vec_bytes_eq_math = fmt_math((rf"3N_{{\text{{vec}}}} \times {bytes_fp32:.0f} = {vec_bytes:,.0f}").replace(",", "{,}"))
...
# │ Exports: TrainingEmissions.n_gpus_str, training_days_str,
# │          emissions_tons_str, grid_ci_g_per_kwh_str,
# │          quebec_emissions_kg_str, quebec_ci_g_per_kwh_str,
# │          emissions_ratio_mult_str
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: EnergyHarvestingArea.solar_cell_area_cm2_str,
# │          EnergyHarvestingArea.kws_solar_area_cm2_str
...
class WakeWordPower:
    ...
    avg_power_eq = fmt_display_math(f"P_{{\\text{{average}}}} = 0.01 + 0.05 \\times 0.1 + " f"0.001 \\times 10 \\times 0.05 = {avg_power_mw_str}")
```
