...
class CarbonCostGPT3:
    ...
    emissions_ratio_str = fmt_multiple(emissions_ratio, precision=1, commas=False)
...
class CarbonFrontier:
    ...
    cf_ratio_str = fmt_multiple(ratio, precision=0, commas=False)
...
class ComputeGrowthScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    compute_growth_factor_str = fmt_multiple(compute_growth_factor, precision=0, commas=True)
    compute_doubling_months_str = fmt_time(compute_doubling_months, 'month', precision=1, commas=True, style='word')
    compute_annual_growth_str = fmt_multiple(compute_annual_growth, precision=1)
    moore_speedup_str = fmt_multiple(moore_speedup, precision=1, commas=True)
```
...
class EnergyWallScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    energy_wall_gap_str = fmt_multiple(energy_wall_gap, precision=1, commas=True)
...
The geographic choice alone produces a `{python} TrainingEmissions.emissions_ratio_str`$\times$ difference in training emissions.
...
class GridQueue:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ratio_str = fmt_multiple(lead_time_ratio, precision=0, commas=False)
...
class WakeWordPower:
    ...
    reduction_str = fmt_multiple(reduction, precision=1, commas=False)
...
The hierarchical approach achieves `{python} WakeWordPower.avg_power_uw_str` average power compared to 10 mW for always-active full inference, a `{python} WakeWordPower.reduction_str`$\times$ reduction enabling battery-powered voice assistants with multi-year operation.
...
class EnergyEfficiencyRange:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    efficiency_range_str = fmt_multiple(efficiency_range, precision=0, commas=False)
