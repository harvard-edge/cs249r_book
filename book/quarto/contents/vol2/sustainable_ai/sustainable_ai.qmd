...
In the **Systems Sandwich** (@sec-vol2-introduction), Sustainability is the **Endurance Layer**. While Security protects against malice and Robustness against chaos, Sustainability protects against **Exhaustion**. It ensures that the fleet can run for years without depleting its energy budget or the physical environment it depends on. If the **Iron Law** defines speed, Sustainability defines **Range**: how far we can go before running out of fuel (energy, capital, or carbon budget).
...
::: {.callout-perspective title="Back-of-the-Napkin: The Carbon Cost of Training"}
...
\end{tikzpicture}
```
: **Data Center Energy Projections**: Between 2010 and 2030, data center electricity usage is projected to increase sharply, particularly under worst-case scenarios where consumption could exceed 8,000 TWh by 2030 [@jones2018much]. The gap between best and worst scenarios—over 10× difference—demonstrates the critical importance of efficiency optimization at every layer of the systems stack.
:::
...
::: {.callout-perspective title="The Hidden Cost: Embodied Carbon Amortization"}
...
#| fig-alt: "Stacked bar chart showing data center power demand from 2015 to 2030. Dark blue bars show non-AI demand growing from 200 to 780 TWh. Light blue shows AI portion growing from 0 to 240 TWh. Gray line shows efficiency gains declining then stabilizing."
#| fig-align: center
#| echo: false
#| message: false
#| warning: false
...
![**GHG Emission Scopes**: Organizations categorize carbon emissions into scope 1 (direct), scope 2 (purchased energy), and scope 3 (value chain) to comprehensively assess their environmental impact and identify targeted reduction strategies for AI systems. Source: Ucircularise.](images/png/ghg_protocol.png){#fig-ghg-protocol fig-alt="Diagram showing three concentric emission scopes: Scope 1 for direct emissions from owned sources, Scope 2 for purchased energy emissions, and Scope 3 for value chain emissions including manufacturing and disposal."}
...
::: {#fig-mckinsey_analysis fig-env="figure" fig-pos="htb" fig-alt="Two grouped bar charts comparing 2017 to 2025 market projections. Datacenter inference doubles from 4-5 to 9-10 billion dollars. Edge inference grows from near zero to 4-4.5 billion dollars. Training markets grow more slowly."}
```{.tikz}
\begin{tikzpicture}
\tikzset{
...
\nWith the Machine Learning Fleet now built, optimized, protected, and sustainable, we have completed the core systems engineering curriculum. Yet the field does not stand still. New architectures are already pushing beyond the boundaries of single-model systems.\n\nThe final chapter, @sec-emerging-systems, looks beyond the current horizon. We will explore the systems challenges of **Compound AI**, **AGI infrastructure**, and the engineering requirements for supporting the next order of magnitude in intelligence.
