...
That geographic concentration creates environmental-justice risks that align with broader responsible AI frameworks.[^fn-environmental-justice-siting] At this point in the volume, the fairness claim is narrow and systems-specific: an ML fleet allocates benefits, electricity demand, water use, land pressure, and e-waste across different communities. Communities hosting AI infrastructure can bear disproportionate environmental burdens while having limited access to AI's economic benefits, so site selection becomes part of the engineering design space rather than a facilities afterthought.
...
The grid, siting, and heat-reuse constraints above become severe because large training campaigns concentrate energy demand into long, synchronized runs. OpenAI's GPT-3 exemplifies this scale: its `{python} Gpt3HouseholdEnergyAnchor.gpt3_training_mwh_str` training run, the chapter's roughly `{python} Gpt3HouseholdEnergyAnchor.gpt3_household_years_str`-household-year anchor, reflects the computation required to train large language models on large datasets [@maslej2023artificial].[^fn-gpt3-energy-scale][^fn-training-communication-overhead]
...
from mlsysim.core.units import MW, MWh, hour, kWh
...
class PueEfficiency:
    ...
    elec_price = 0.07 # $/kWh scenario input
    # ┌── 2. EXECUTE ───────────────────────────────────────
    diff = pue_old - pue_new
    annual_savings = (p_it * diff * HOURS_PER_YEAR * hour).to(MWh)
    annual_savings_usd = annual_savings.to(kWh).magnitude * elec_price
    # ┌── 3. GUARD ─────────────────────────────────────────
    ...
    elec_price_per_mwh_str = fmt_usd(elec_price * THOUSAND, precision=0, commas=False, per="MWh")
...
3. **Financial Savings**\index{PUE!financial savings}: `{python} PueEfficiency.pue_savings_mwh_str` $\times$ `{python} PueEfficiency.elec_price_per_mwh_str` $\approx$ `{python} PueEfficiency.pue_savings_usd_str`.
