...
Memory-aware optimization is the first constraint because microcontrollers operate with 64 KB to 2 MB SRAM, requiring careful memory planning during model design:
...
For devices powered by ambient energy harvesting (solar, vibration, RF), even INT8 inference may exceed available power budgets. Binary neural networks (BNNs) push quantization to its extreme by representing weights and activations as single bits. This directly enables the ultra-low-power operation required for the TinyML paradigms established in @sec-edge-intelligence.
...
Automated architecture design becomes useful when hand-designed models cannot fit the memory and energy envelope. Neural architecture search for TinyML finds efficient network structures for specific constraints:
...
For inference, the relevant benchmark quantity depends on the serving regime:
...
@Tbl-mlperf-tiny summarizes benchmark tasks and typical energy requirements spanning from sub-millijoule to multi-millijoule ranges. The MLPerf Tiny measurement methodology requires external power monitors (like those in @sec-sustainable-ai-edge-mobile-device-energy-measurement-3012) and specifies warm-up periods, measurement windows, and statistical reporting requirements to ensure reproducible results across submissions.
