...
class ArchetypeATdp:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h_h100_tdp_w_str = fmt_qty(tdp, watt, precision=0, commas=False)
...
class MemoryHierarchyEnergy:
    ...
    dram = Hardware.Tech.Memory.DRAM.energy_per_byte.m_as('picojoule/byte')
...
class MatMulEnergyAnalysis:
    ...
    e_move = Hardware.Tech.Memory.DRAM.energy_per_byte.m_as('picojoule/byte')  # 160 (device)
...
```{python}
#| echo: false
from mlsysim.fmt import fmt, fmt_count, fmt_power, fmt_qty, fmt_qty_int
from mlsysim.core.units import watt, kilowatt, microsecond, count
...
class H100TdpRackRecap:
    ...
    power_delta_kw_str = fmt_qty_int(cluster_power_delta.to(kilowatt), kilowatt, commas=False)
    ...
    node_power_budget_kw_str = fmt_qty(7.0 * kilowatt, kilowatt, precision=0, commas=False)
    node_gpus_str = fmt_count(Systems.Nodes.DGX_H100.accelerators_per_node, commas=False, label="GPU")
    node_gpu_only_kw_str = fmt_qty(Hardware.Cloud.H100.tdp * Systems.Nodes.DGX_H100.accelerators_per_node, kilowatt, precision=1, commas=False)
```
...
from mlsysim.core.constants import kilowatt
from mlsysim.fmt import fmt_qty, check

class SustainableCoolingRackPowerRecap:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    node_gpu_power_kw = (Hardware.Cloud.H100.tdp * Systems.Nodes.DGX_H100.accelerators_per_node).m_as(kilowatt)
    nodes_per_rack = 4
    non_gpu_power_kw = 10.3

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    rack_power_kw = node_gpu_power_kw * nodes_per_rack + non_gpu_power_kw

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(rack_power_kw > node_gpu_power_kw, "Rack power should exceed one node's GPU power.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    rack_power_str = fmt_qty(rack_power_kw * kilowatt, kilowatt, precision=1, commas=False)
```
...
class PueSavings:
    ...
    electricity_rate = Infrastructure.Pricing.OnPremises.ElectricityPerKwh.rate.m_as("dollar / kilowatt_hour")
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    diff_15_to_11_mw_str = fmt_qty(diff_15_to_11_mw * ureg.megawatt, ureg.megawatt, precision=1, commas=False)
    diff_158_to_110_mw_str = fmt_qty(diff_158_to_110_mw * ureg.megawatt, ureg.megawatt, precision=1, commas=False)
...
```{python}
#| echo: false
from mlsysim.fmt import fmt_qty, fmt_usd
# ┌── LEGO ───────────────────────────────────────────────
...
class H100TdpCoolingRecap:
    ...
    n_gpus_str = fmt(n_gpus, precision=0)
    it_power_kw_str = fmt_qty(it_power_kw * kilowatt, kilowatt, precision=1, commas=False)
    air_total_kw_str = fmt_qty(air_total_kw * kilowatt, kilowatt, precision=1, commas=False)
    air_cooling_kw_str = fmt_qty(air_cooling_kw * kilowatt, kilowatt, precision=1, commas=False)
    liquid_total_kw_str = fmt_qty(liquid_total_kw * kilowatt, kilowatt, precision=1, commas=False)
    liquid_cooling_kw_str = fmt_qty(liquid_cooling_kw * kilowatt, kilowatt, precision=1, commas=False)
    savings_kw_str = fmt_qty(savings_kw * kilowatt, kilowatt, precision=1, commas=False)
...
from mlsysim.core.constants import watt
...
class SustainableMobilePowerEnvelope:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    sustained_low_w = Scenarios.MobilePower.MobileMlSustainedLow.m_as(watt)
    sustained_high_w = Scenarios.MobilePower.MobileMlSustainedHigh.m_as(watt)
    burst_low_w = Scenarios.MobilePower.MobileMlBurstLow.m_as(watt)
    burst_high_w = Scenarios.MobilePower.MobileMlBurstHigh.m_as(watt)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(sustained_low_w < sustained_high_w < burst_low_w < burst_high_w,
          "Mobile sustained and burst power ranges should be ordered.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sustained_range_str = fmt_qty_range(
        sustained_low_w * watt,
        sustained_high_w * watt,
        watt,
        precision=0,
        commas=False,
    )
...
class OnDeviceLearningEnergy:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    phone_battery_wh = Scenarios.PhoneBattery.EnergyWh.m_as('Wh')
    joules_per_wh = 3600
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    phone_battery_wh_str = fmt_qty(phone_battery_wh * ureg.watt_hour, ureg.watt_hour, precision=0, commas=False)
