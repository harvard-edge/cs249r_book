...
Several strategies can facilitate this shift. Legislative and regulatory movements promoting the **right-to-repair**\index{Right-to-Repair} push back against repair restrictions by emphasizing access to parts, tools, diagnostics, and service information [@ftc2021nixingfix]. Modular AI hardware designs---allowing independent upgrade of accelerators, memory, or networking interfaces---prevent the need to discard entire systems when only one component is obsolete, following the principle demonstrated by companies like Framework in consumer laptops [@framework2022modular]. Extended software and firmware support cycles serve the same lifecycle goal: they keep usable hardware secure and operational for longer, delaying its entry into the e-waste stream [@forti2020ewaste]. Secondary-use programs can further amortize embodied carbon by moving older accelerators into research, batch, or lower-priority workloads instead of sending them directly to disposal.
...
The value of Google's case study is that it decomposes mitigation across the same layers this chapter has tracked: model, machine, mechanization, and map. To mitigate emissions from rapidly expanding AI workloads, Google engineers identified the "4 Ms," where systematic improvements collectively reduce the carbon footprint of machine learning [@patterson2022]:
...
1. **Measure First**\index{Sustainable AI!measurement first}: Tools like CarbonTracker and CodeCarbon track the emissions of training runs. Teams cannot improve what they do not measure, and establishing baseline metrics is essential for validating the effectiveness of optimization efforts [@anthony2020carbontracker; @schmidt2021codecarbon].
