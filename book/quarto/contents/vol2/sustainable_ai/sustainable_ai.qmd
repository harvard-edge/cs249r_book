...
[^fn-training-paradigms]: **Training Paradigms**: Distributed training strategies coordinating computation across systems. Data parallelism replicates models across workers processing different batches; model parallelism partitions layers across devices; pipeline parallelism chains stages for throughput. GPT-3 combined all three across thousands of GPUs, with parallelism strategy selection significantly impacting training efficiency and energy consumption.
...
Scope 2 emissions, which account for 60 to 75 percent of the total, represent indirect emissions from electricity purchased to power AI infrastructure. This dominant operational emission category varies dramatically by geographic location and grid energy mix. Training the same model in Quebec with hydro power versus West Virginia with coal power creates a 75× difference in carbon intensity.
...
[^fn-pruning-technique]: **Pruning Technique**: Removing unnecessary model components (weights, neurons, or layers) to improve efficiency while preserving accuracy. Unstructured pruning achieves 90%+ sparsity but requires specialized hardware; structured pruning removes entire filters for immediate speedups. LLMs like SparseGPT achieve 60% sparsity with <1% accuracy loss, reducing inference memory and compute proportionally.
