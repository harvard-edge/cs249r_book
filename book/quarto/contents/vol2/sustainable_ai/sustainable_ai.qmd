...
**Embodied**: Assume manufacturing footprint is $\approx 175 \text{kg } \text{CO}_2$ per H100 GPU. Amortized for 1 month of a 3-year cycle: $(2048 \times 175 \text{kg}) / 36 \text{ months} \approx 10 \text{ metric tons}$.
...
[^fn-embodied-carbon-hardware]: **Embodied Carbon**: The CO~2~ emitted during manufacturing, transport, and disposal before a device computes its first FLOP. A single H100 embodies 150–200 kg CO~2~ from fabrication alone; at `{python} GpuEmissionsScenario.h100_tdp_w` W on the average U.S. grid, continuous operation matches embodied carbon in roughly three to four weeks. As data centers shift to renewables, embodied carbon's share of total lifetime emissions grows, potentially exceeding 30 percent, making hardware refresh cycles a first-order sustainability decision. \index{Embodied Carbon!hardware lifecycle}
...
The units are liters per kilowatt-hour, with typical values ranging from 0.5 to 2.0 L/kWh depending on climate and cooling technology. A data center with WUE of 1.8 L/kWh training a model requiring 10,000 MWh would consume 18 million liters of water, equivalent to roughly 40--50 U.S. household-years of water use under a 380,000--450,000 L/year household baseline.
...
Arithmetic intensity measured in FLOPs per byte determines the dominant energy consumer. The energy model separates total energy from the roofline-style dominant term:

$$E_{\text{total}} = E_{\text{compute}} + E_{\text{memory}} = \text{FLOPs} \times e_{\text{flop}} + \text{Bytes} \times e_{\text{byte}}$$ {#eq-energy-total}

$$E_{\text{dominant}} = \max\left(E_{\text{compute}}, E_{\text{memory}}\right) = \max\left(\text{FLOPs} \times e_{\text{flop}}, \text{Bytes} \times e_{\text{byte}}\right)$$ {#eq-energy-roofline}

where $e_{\text{flop}}$ is energy per FLOP and $e_{\text{byte}}$ is energy per byte moved. The maximum term identifies the dominant bottleneck for roofline reasoning; it is not the full energy in balanced cases. @eq-ai-crossover defines the crossover arithmetic intensity\index{Crossover Arithmetic Intensity} where compute and memory energy balance:
...
Compared with the 7B/64-A100 training example above (4.4 metric tons), cumulative inference emissions exceed training emissions after less than 2 days of deployment at this scale. For frontier-scale training runs, the crossover can shift to weeks or months depending on query volume, per-query energy, and grid intensity.
...
: **Critical Materials for AI Hardware**\index{Critical Materials!critical materials for ai hardware}: Semiconductor manufacturing relies on specific materials, including silicon, gallium, germanium, indium, tantalum, cobalt, tungsten, copper, helium, and rare earth elements, that face increasing supply constraints and geopolitical risks, potentially impacting AI hardware production and innovation. The table details these materials, their applications in AI systems, and the associated supply vulnerabilities requiring proactive mitigation strategies. {#tbl-material_depletion}
...
Google's carbon-intelligent computing platform[^fn-carbon-scheduling-results] demonstrated this approach at scale, achieving a 40 percent reduction in carbon footprint under its global workload-shifting assumptions. The intervention cascade uses a more conservative 1.3$\times$ average reduction for mixed production fleets, where only some jobs are deadline-tolerant enough to move across time or geography.

::: {#fig-carbon-scheduling fig-env="figure" fig-pos="htb" fig-cap="**Carbon-Aware Workload Scheduling**: Step 5 of the intervention cascade uses a conservative 1.3$\times$ reduction for mixed fleets via temporal and geographic shifting." fig-alt="6-step intervention cascade highlighting step 5: carbon-aware scheduling with a conservative 1.3 times reduction."}
![](images/svg/intervention-cascade.svg)
:::
