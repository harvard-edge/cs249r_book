...
class CarbonCostGPT3:
    ...
    total_emissions_kg_str = fmt_emissions(total_carbon.to(kilogram), precision=0)
...
from mlsysim.core.units import kilowatt, kWh
...
class LifecycleCarbonEstimate:
    ...
    facility_energy_kwh_str = fmt_energy(res.total_energy_kwh, unit=kWh, precision=0, commas=True)
...
class TrainingEmissions:
    ...
    gpu_energy_wh_str = fmt_energy(gpu_energy, precision=0, commas=True)
...
class EmbodiedCarbonAmort:
    ...
    total_mfg_carbon_str = fmt_emissions(total_mfg_carbon, precision=0, commas=False)
...
class InferenceLifecycleExample:
    ...
    kwh_per_query_str = fmt_energy(kwh_per_query, precision=3, commas=False)
    daily_energy_kwh_str = fmt_energy(daily_energy, precision=0, commas=True)
    annual_energy_kwh_str = fmt_energy(annual_energy, precision=0, commas=True)
    grid_ci_kg_per_kwh_str = fmt_qty(grid_ci, kilogram / kWh, precision=3, commas=False)
    annual_emissions_kg_str = fmt_emissions(annual_carbon.to(kilogram), precision=0)
...
class RackPowerBudget:
    ...
    gpu_kw_str = fmt_power(gpu_kw, precision=1, commas=False)
    host_kw_str = fmt_power(host_kw, precision=1, commas=False)
    nvswitch_kw_str = fmt_power(nvswitch_kw, precision=1, commas=False)
    ib_kw_str = fmt_power(ib_kw, precision=1, commas=False)
    conversion_kw_str = fmt_power(conversion_kw, precision=1, commas=False)
    cooling_kw_str = fmt_power(cooling_kw, precision=1, commas=False)
    total_kw_str = fmt_power(total_kw, precision=1, commas=False)
