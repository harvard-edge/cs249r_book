...
# │ Exports: ArchetypeATdp.h100_tdp_w_str
# └─────────────────────────────────────────────────────────────────────────────
...
class ArchetypeATdp:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tdp_w_str = f"{tdp_w:.0f}"
```

::: {.callout-lighthouse title="Archetype A (GPT-4/Llama-3): The energy wall"}

**Archetype A (GPT-4)** is the primary driver of the industry's exponential energy growth. A single 25,000-GPU cluster drawing `{python} ArchetypeATdp.h100_tdp_w_str` W per chip requires 17.5 MW of continuous power for training. The constraint is physical, not financial: it is a **Grid Capacity** problem. Organizations operating Archetype A models are increasingly forced to build their own power infrastructure or relocate to regions with excess renewable energy, making Carbon-Aware Scheduling and Geographic Optimization as critical as learning rate tuning.
...
class CarbonFrontier:
    ...
    quebec_tonnes = (energy_kwh * intensity_quebec) / MILLION
    poland_tonnes = (energy_kwh * intensity_poland) / MILLION
...
Arithmetic intensity measured in FLOPs per byte determines the dominant energy consumer. @eq-energy-total expresses total energy as the sum of compute and memory contributions, while @eq-energy-roofline isolates the roofline-style dominant term:
...
energy_joules = ((end_energy - start_energy) * ureg.microjoule).m_as(ureg.joule)
...
# │
# │ Imports: mlsysim.core.constants (A100_TDP, watt, hour, ureg, THOUSAND — module-wide)
...
class TrainingEmissions:
    ...
    gpu_energy_kwh = (gpu_energy_wh * watt * hour).m_as(ureg.kilowatt_hour)
...
# │
# │ Imports: mlsysim.core.constants (H100_TDP, kilowatt)
# │ Exports: h100_tdp_kw
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import H100_TDP, kilowatt
...
class H100KwScenario:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    h100_tdp_kw_val = h100_tdp.m_as(kilowatt)
...
*   **Amortization**: Lifetime = 3 years (26,280 hours).
    ...
    *   Job Cost = $0.050 \times 10 =$ **`{python} EmbodiedCarbonAmort.embodied_carbon_job_str` kg CO~2~**.
...
For an H100 with 164 kg embodied carbon (per NVIDIA's product carbon footprint) and 4-year data center lifetime:

$$C_{\text{embodied,daily}} = \frac{164 \text{ kg}}{4 \times 365} \approx 0.112 \text{ kg/day}$$

Over a 14-day training run using 64 GPUs:

$$C_{\text{embodied,training}} = 64 \times 14 \times 0.112 \approx 101 \text{ kg } \text{CO}_2$$
