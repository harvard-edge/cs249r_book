...
# ┌── LEGO ───────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, fmt_power, fmt_energy, fmt_emissions, check
...
**Parameters**: `{python} LifecycleCarbonEstimate.n_gpus_str` H100 GPUs, `{python} LifecycleCarbonEstimate.training_days_str` days, `{python} LifecycleCarbonEstimate.h_h100_tdp_w_str` TDP, PUE `{python} LifecycleCarbonEstimate.pue_str`, grid intensity `{python} LifecycleCarbonEstimate.grid_ci_str` g $\text{CO}_2$/kWh.
...
The daily amortized embodied carbon is `{python} TrainingEmbodiedRecap.embodied_kg_per_accelerator_str` kg / (`{python} TrainingEmbodiedRecap.embodied_lifetime_years_str` $\times$ `{python} TrainingEmbodiedRecap.days_per_year_str`) $\approx$ `{python} TrainingEmbodiedRecap.embodied_daily_kg_str`/day.
...
To appreciate the magnitude of these transients, consider a `{python} H100TdpRackRecap.cluster_gpus_str`-GPU cluster transitioning from communication phase (`{python} H100TdpRackRecap.comm_phase_w_str` per GPU average) to matrix multiplication phase (`{python} H100TdpRackRecap.tdp_w_str` per GPU). The power delta is `{python} H100TdpRackRecap.power_delta_w_str` $\times$ `{python} H100TdpRackRecap.cluster_gpus_str` = `{python} H100TdpRackRecap.power_delta_kw_str`, and this transition occurs in approximately `{python} H100TdpRackRecap.transition_us_str`. The rate of power change is therefore `{python} H100TdpRackRecap.power_delta_kw_str`/`{python} H100TdpRackRecap.transition_us_str` = `{python} H100TdpRackRecap.power_rate_gws_s_str`. No passive electrical component can respond at this rate; only energy storage devices (supercapacitors) positioned physically close to the load can absorb the transient before it propagates into the building's electrical distribution.
...
3.  **Training Cost**\index{On-Device Learning!training cost}:
    ...
    *   Total per token: `{python} OnDeviceLearningEnergy.train_nj_per_param_str` nJ/param $\times 10^9$ params = `{python} OnDeviceLearningEnergy.joules_per_token_j_str`.
