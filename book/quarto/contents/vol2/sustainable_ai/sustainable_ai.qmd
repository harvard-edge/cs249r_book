...
Operations with arithmetic intensity below 10 FLOPs/byte are memory-bound, with data movement dominating energy consumption. Operations above this threshold are compute-bound.
...
Part IV has addressed the operational concerns that keep production ML systems safe: protecting them from security threats (@sec-security-privacy), ensuring reliability under adversarial conditions (@sec-robust-ai), and maintaining environmental and economic viability through sustainable practices. These chapters focus on the technical and operational dimensions of safety, but operational safety alone is insufficient. A system can be secure, robust, and sustainable while still causing harm through biased decisions, lack of transparency, or misaligned incentives. Part V shifts from operational safety to societal responsibility, beginning with @sec-responsible-ai, which examines the governance frameworks, fairness requirements, and accountability structures that ensure ML systems serve human values. The principles developed throughout Part IV provide the technical foundation; responsible AI provides the ethical framework that determines how these capabilities serve society.
