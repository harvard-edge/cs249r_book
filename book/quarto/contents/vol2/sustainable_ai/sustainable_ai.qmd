...
<!-- TODO(focused-followup + numerical-audit): Items deferred from the
     codex deep-review pending-discussions walkthrough:

     - Operational-carbon equation PUE double-count (line ~1256): line
       1236 defines E_total = component-energy times PUE, then line
       1256 writes C_operational = E_total times CI_grid times PUE,
       which double-counts facility overhead. Use distinct symbols
       (E_IT and E_facility) or remove the extra PUE factor.
     - Two near-duplicate energy-wall figures + narrative (lines ~373
       and ~638): @fig-energy-wall-quantitative and @fig-energy-wall
       tell the same 350,000-fold growth story against different
       ceilings. Keep one or make the second clearly answer a
       different question.
     - Fallacy grid-intensity numbers (line ~2848): claim is "21-fold
       difference" between 367 g/kWh and 34.5 g/kWh lifecycle, but
       367/34.5 = 10.6, not 21. The earlier worked example uses 429
       vs. 20 = 21-fold ratio. Reconcile against one consistent
       intensity model.
     - Embodied-carbon two-order-of-magnitude inconsistency (line
       ~2852 vs. lines ~1501--1506): 108 kg amortized at line 1501
       vs. 10.5 metric tons total at line 2852 for the same 64 H100s
       over 14 days. State whether the comparison uses full upfront
       manufacturing carbon or amortized job-level carbon, then use
       one basis consistently.
     - GPT-3 household-energy comparison drift across lines 85/91/304/
       665 (roughly 100/120/122/130 U.S. household-years); pick one
       rounded value.
     - OpenAI supercomputer reference uncited/undated (line ~1908):
       285,000 CPU cores + 10,000 GPUs is the 2020 Microsoft/OpenAI
       disclosure, not current frontier infrastructure. Qualify by
       date or replace with a current cited example.
     - "Noise Event", "Voltage Spike", "Voltage Sag", "Byproduct",
       "Pollutant", "Circular Energy Economy" capitalized in running
       prose at line ~653 without being defined terms; lowercase
       these as ordinary concept terms. -->
...
- Explain the sustainability paradox where AI compute growth (350,000$\times$ from 2012 to 2019) outpaces hardware efficiency gains
...
**Systems conclusion**: A single training run emits as much carbon as flying a Boeing 747 full of passengers across the Atlantic. **Optimization matters.** Moving this job to a hydro-powered region (0.02 kg/kWh) would reduce emissions by **`{python} CarbonCostGPT3.emissions_ratio_str`$\times$** to just ~25 flights.
...
- **Site A (Quebec)**: Hydropower, `{python} CarbonFrontier.intensity_quebec` g $\text{CO}_2$/kWh.
- **Site B (Poland)**: Coal-heavy, `{python} CarbonFrontier.intensity_poland` g $\text{CO}_2$/kWh.
How does the location affect your model's carbon footprint?

**Math**:
Carbon = Energy $\times$ Grid Intensity.

1. **Site A Emissions**\index{Site A Emissions}: 10,000,000 kWh $\times$ 20 g = 200,000,000 g = **`{python} CarbonFrontier.cf_quebec_tonnes_str` tonnes $\text{CO}_2$**.
2. **Site B Emissions**\index{Site B Emissions}: 10,000,000 kWh $\times$ 800 g = 8,000,000,000 g = **`{python} CarbonFrontier.cf_poland_tonnes_str` tonnes $\text{CO}_2$**.
...
**Parameters**: 2,048 H100 GPUs, 30 days, 700 W TDP, PUE 1.3, Grid intensity 400g $\text{CO}_2$/kWh.
...
$$\text{WUE} = \frac{W_{\text{annual\_water\_usage}}}{P_{\text{IT\_equipment\_energy}}}$$ {#eq-wue}
...
$$\text{AI} = \frac{\text{Total FLOPs}}{\text{Total Bytes Moved}}$$ {#eq-arithmetic-intensity}
...
$$\text{AI}_{\text{crossover}} = \frac{e_{\text{byte}}}{e_{\text{flop}}}$$ {#eq-ai-crossover}
...
- Arithmetic intensity: $\text{AI} = \frac{2N^3}{12N^2} = \frac{N}{6}$ FLOPs/byte
...
$$C_{\text{operational}} = E_{\text{total}} \times \text{CI}_{\text{grid}} \times \text{PUE}$$ {#eq-operational-carbon}

where $E_{\text{total}}$ is the energy consumed by IT equipment, $\text{CI}_{\text{grid}}$ is the carbon intensity of the electricity grid, and $\text{PUE}$ accounts for facility overhead. A concrete *training emissions calculation* illustrates this framework.
...
The embodied contribution of 108 kg represents approximately 2.4 percent of the operational emissions (4,428 kg) calculated above for the US average grid, and about 17 percent of total emissions if training occurred in Quebec's low-carbon grid.
...
[^fn-gradient-sync-energy]: **Gradient Synchronization Energy Cost**\index{Gradient Synchronization!energy cost}: Ring-allreduce scales communication linearly with message size but requires every node to participate, meaning one slow node wastes energy across the entire ring. At scale, gradient compression (1-2 bit quantization) can reduce network energy by 10--50$\times$ per synchronization step, but introduces statistical noise that may require additional training iterations, partially offsetting the savings. \index{Gradient Synchronization!energy cost}
...
**Systems conclusion**: Full fine-tuning is impossible within a reasonable daily battery budget. Sustainable on-device learning requires **PEFT (Parameter-Efficient Fine-Tuning)**\index{Parameter-Efficient Fine-Tuning} or sparse updates to reduce the energy cost per token by 100$\times$ or more.
...
**Trade-off**: Should you process data locally or send it to the cloud? The physics of energy consumption provides a clear answer based on the **Energy-to-Communication Ratio**.
...
This is the Jevons paradox of AI (Principle \ref{pri-jevons-paradox}): making models 10$\times$ more efficient can increase total usage enough to erase or even exceed the expected energy savings. Sustainability strategies must therefore focus on **absolute limits** (carbon budgets, renewable sourcing) rather than just rate efficiency (FLOPS/Watt).
...
$$\text{EDP} = E \times T = P \times T^2$$ {#eq-energy-delay-product}
...
* **Machine**\index{Machine}: The implementation of AI-specific hardware offers 2--5$\times$ improvements in performance per watt compared to general-purpose systems. Google's TPUs demonstrate 5--13$\times$ greater carbon efficiency relative to non-optimized GPUs.

* **Mechanization**\index{Mechanization}: Optimized cloud computing infrastructure with high utilization rates yields 1.4--2$\times$ energy reductions compared to conventional on-premise data centers. Google's facilities consistently exceed industry standards for PUE.

* **Map**\index{Map}: The strategic positioning of data centers in regions with low-carbon electricity supplies reduces gross emissions by 5--10$\times$. Google maintains real-time monitoring of renewable energy usage across its global infrastructure.
