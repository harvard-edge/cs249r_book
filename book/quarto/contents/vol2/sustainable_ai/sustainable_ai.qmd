...
2.  **Carbon Intensity (US Average)**: $\approx `{python} CarbonCostGPT3.grid_ci_us_kg` \text{ kg CO}_2\text{/kWh}$.
3.  **Total Emissions**: $1,287,000 \times 0.429 \approx \mathbf{`{python} CarbonCostGPT3.total_emissions_kg_str` \text{ kg CO}_2}$.
4.  **Comparison**:
    *   One passenger, NY to London (round trip): $\approx 1,000 \text{ kg CO}_2$.
    *   **Ratio**: `{python} CarbonCostGPT3.total_emissions_kg_str` / 1,000 = $\mathbf{`{python} CarbonCostGPT3.flight_ratio_str` \text{ flights}}$.
...
*   **Amortization**: Lifetime = 3 years (26,280 hours).
    ...
    *   Job Cost = $0.046 \times 10 = \mathbf{`{python} EmbodiedCarbonAmort.embodied_carbon_job_str` \text{ kg } CO_2}$.
