...
The D·A·M taxonomy describes the *workload*: the nouns of our system (the Data we have, the Algorithm we want to run, the Machine we bought). The C$^3$ taxonomy describes the *execution tax*: the verbs of the fleet (Computing the math, Communicating the results, Coordinating the state). The single-node D·A·M lens reviewed in @sec-appdx-dam-taxonomy optimizes the workload on one node; this volume projects that workload onto a distributed fleet. *When* we stretch the nouns across thousands of machines, we are forced to pay the verbs:
