...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, fmt_energy, fmt_memory, fmt_percent
...
class GPT3SyncTax:
    ...
    bw_eth = 100 * Gbps  # scenario assumption
...
Part I, the fleet core, begins with physical limits: no single server has enough memory, power, or cooling to train a frontier model. @Sec-compute-infrastructure therefore starts with the physics of high-density silicon, liquid cooling, and megawatt-scale power ramp rates. @Sec-network-fabrics then connects those machines through a high-bandwidth "Gradient Bus" that acts as the cluster-scale system bus. @Sec-data-storage completes the substrate by designing storage hierarchies that feed terabytes of data to accelerators without stalling the math.

Part II begins once the physical fleet exists and the coordination tax becomes unavoidable. Splitting math across machines creates synchronization bottlenecks and frequent failures, so @Sec-distributed-training-systems develops partitioning strategies for models with hundreds of billions to trillions of parameters, @Sec-collective-communication explains the AllReduce and AllGather algorithms that bind independent nodes into a coherent computer, and @Sec-fault-tolerance-reliability treats recovery speed as more important than individual-node uptime. @Sec-fleet-orchestration then manages the multi-tenant cluster itself, where gang scheduling prevents deadlocks and unified frameworks like **Ray**\index{Ray} [@moritz2018ray] enable flexible scaling across diverse hardware resources.

Part III shifts from training a model to serving it economically. Inference costs eventually dwarf training costs, so the central metric shifts from training throughput to latency, utilization, and lifetime serving cost. @Sec-performance-engineering closes the gap between hardware peak and actual throughput through kernel fusion and compilation; innovations like **FlashAttention**\index{FlashAttention} [@dao2022] demonstrate how I/O-aware kernels can improve performance 2--4$\times$ on memory-bound workloads. @Sec-inference-scale then turns those local optimizations into serving systems for millions of users, @Sec-edge-intelligence moves intelligence toward devices with milliwatt power budgets, and @Sec-ops-scale develops the control plane for monitoring health, drift, and performance across global deployments.

Part IV treats responsibility as an engineering layer rather than an afterword. At global scale, technical bugs become societal hazards, so @Sec-security-privacy defends the fleet against adversaries who seek to poison data or extract proprietary weights, @Sec-robust-ai studies how models survive non-i.i.d. open-world conditions, @Sec-sustainable-ai manages the energy wall and lifecycle carbon footprint of industrial-scale AI, and @Sec-responsible-ai connects technical capability to fairness, transparency, accountability, and governance.
...
A common fallacy is *focusing on algorithmic efficiency while ignoring hardware-system alignment*.
...
A common fallacy is *assuming efficiency optimizations always improve system performance across all metrics*.
...
A common fallacy is *assuming edge deployment efficiency requirements are simply scaled-down versions of cloud requirements*.
...
A common pitfall is *assuming scaling laws predict efficiency requirements linearly across all scales*.
