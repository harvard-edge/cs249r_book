...
# │ Exports: gpt4_gpus_str, gpt4_days_str, cluster_fail_day_str, mtbf_hours_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsys import Models, Applications, Systems
from mlsys.constants import GPU_MTTF_HOURS, hour, day
...
gpt4_gpus_str = Gpt4TrainingScenario.gpt4_gpus_str
gpt4_days_str = Gpt4TrainingScenario.gpt4_days_str
cluster_fail_day_str = Gpt4TrainingScenario.cluster_fail_day_str
mtbf_hours_str = Gpt4TrainingScenario.mtbf_hours_str
hw_name = Gpt4TrainingScenario.hw_name
```

Consider the training of GPT-4. It reportedly required approximately **`{python} gpt4_gpus_str` `{python} hw_name` GPUs** running for **`{python} gpt4_days_str` days** [@openai2023gpt4]. In a cluster of this size, the probability of failure ($P_{\text{fail}}$) becomes the dominant constraint.
...
# │
# │ Imports: mlsys.constants (GPT3_PARAMS, GPT3_TRAINING_OPS, param, BILLION),
# │          mlsys.formatting (sci, check)
# │ Exports: gpt3_params_b, gpt3_training_ops_sci, gpt3_gradient_gb
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import GPT3_PARAMS, GPT3_TRAINING_OPS, param, BILLION
from mlsys.formatting import sci, check
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
gpt3_params_b = Gpt3TrainingScenario.gpt3_params_b
gpt3_training_ops_sci = Gpt3TrainingScenario.gpt3_training_ops_sci
gpt3_gradient_gb = Gpt3TrainingScenario.gpt3_gradient_gb
```
...
# │ How: pulling constants from mlsys.constants.
# │
# │ Imports: mlsys.constants (INFINIBAND_HDR_BW, INFINIBAND_NDR_BW,
# │          Gbps, BITS_PER_BYTE)
# │ Exports: ib_hdr_gbps, ib_ndr_gbps, ib_hdr_gbs, ib_ndr_gbs
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import INFINIBAND_HDR_BW, INFINIBAND_NDR_BW, Gbps, BITS_PER_BYTE
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
ib_hdr_gbps = InfiniBandScenario.ib_hdr_gbps
ib_ndr_gbps = InfiniBandScenario.ib_ndr_gbps
ib_hdr_gbs = InfiniBandScenario.ib_hdr_gbs
ib_ndr_gbs = InfiniBandScenario.ib_ndr_gbs
```
...
# │ Exports: gpt3_params_b, gpt3_gradient_gb
# └─────────────────────────────────────────────────────────────────────────────
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
gpt3_params_b = Gpt3CommunicationScenario.gpt3_params_b
gpt3_gradient_gb = Gpt3CommunicationScenario.gpt3_gradient_gb
```
...
The rapid evolution in AI capabilities over the past decade exemplifies this scaling trajectory. GPT-1 (2018) contained 117 million parameters and demonstrated basic sentence completion capabilities. GPT-2 (2019) scaled to 1.5 billion parameters and achieved coherent paragraph generation.
...
# │ How: pulling constants from mlsys.constants.
# │
# │ Imports: mlsys.constants (GPT3_PARAMS, GPT3_TRAINING_OPS, A100_FLOPS_FP16_TENSOR,
# │          param, BILLION, TFLOPs, second), mlsys.formatting (sci, check)
# │ Exports: gpt3_params_b, gpt3_training_ops_sci, a100_fp16_tflops
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import GPT3_PARAMS, GPT3_TRAINING_OPS, A100_FLOPS_FP16_TENSOR, param, BILLION, TFLOPs, second
from mlsys.formatting import sci, check
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
gpt3_params_b = Gpt3StatsScenario.gpt3_params_b
gpt3_training_ops_sci = Gpt3StatsScenario.gpt3_training_ops_sci
...
\tikzset{
    ...
    legend/.style={fill=BrownL, draw=BrownLine, row sep=-0.5pt, font=\fontsize{6pt}{6}\selectfont\usefont{T1}{phv}{m}{n}},
    axis/.style={
        ...
        yticklabel style={xshift=1mm, font=\fontsize{7pt}{7}\selectfont\usefont{T1}{phv}{m}{n}, /pgf/number format/.cd, fixed, fixed zerofill, precision=1},
    ...
    myaxis/.style={
        ...
        height=67.2mm,
        cycle list={
            {BlueLine,mark=*,only marks,mark options={line width=1pt},mark size=1.75pt},
            {OrangeLine,mark=*,only marks,mark options={line width=1pt},mark size=1.75pt},
            {GreenLine,mark=*,only marks,mark options={line width=1pt},mark size=1.75pt},
            {RedLine,mark=*,only marks,mark options={line width=1pt},mark size=1.75pt},
            {VioletLine,mark=*,only marks,mark options={line width=1pt},mark size=1.75pt},
            {BrownLine,mark=*,only marks,mark options={line width=1pt},mark size=1.75pt},
            {BlueLine},
            {OrangeLine},
            {GreenLine},
            {RedLine},
...
\begin{axis}[myaxis, axis]
    \addplot+[] coordinates {(3.05e7,4.645)(3.05e7,4.48)(5.9e7,4.415)(1.14e8,4.34)(8.3e8,4.29)(2.3e10,4.28)};
    \addlegendentry{393.2K}
    
    \addplot+[] coordinates {(3.05e7,4.25)(5.9e7,4.1)(1.14e8,3.93)(2.2e8,3.867)(4.3e8,3.837)(8.3e8,3.8)(2.3e10,3.77)};
    \addlegendentry{3M}
    
    \addplot+[] coordinates {(3.05e7,4.25)(5.9e7,3.941)(1.14e8,3.735)(2.2e8,3.567)(4.3e8,3.415)(8.3e8,3.325)(2.3e10,3.27)};
    \addlegendentry{25M}
    
    \addplot+[] coordinates {(3.05e7,4.21)(5.9e7,3.941)(1.14e8,3.69)(2.2e8,3.472)(4.3e8,3.31)(8.3e8,3.12)(1.61e9,3.04)(2.3e10,2.97)};
    \addlegendentry{85M}
    
    \addplot+[] coordinates {(3.05e7,4.21)(5.9e7,3.941)(1.14e8,3.69)(2.2e8,3.46)(4.3e8,3.28)(8.3e8,3.01)(1.61e9,2.84)(2.3e10,2.62)};
    \addlegendentry{302M}
    
    \addplot+[] coordinates {(3.05e7,4.31)(5.9e7,3.941)(1.14e8,3.69)(2.2e8,3.46)(4.3e8,3.28)(8.3e8,3.05)(1.61e9,2.80)(2.3e10,2.42)};
    \addlegendentry{708M}
    
    % Approximation plots
    \addplot+[LineD,smooth] coordinates {(1.5e7,4.595)(3.05e7,4.47)(5.9e7,4.395)(1.14e8,4.35)(8.3e8,4.3)(3e10,4.290)};
    
    \addplot+[LineD,smooth] coordinates {(1.5e7,4.46)(3.05e7,4.25)(5.9e7,4.08)(1.14e8,3.96)(2.2e8,3.867)(4.3e8,3.814)(8.3e8,3.789)(3e10,3.756)};
    
    \addplot+[LineD,smooth] coordinates {(1.5e7,4.42)(3.05e7,4.17)(5.9e7,3.95)(1.14e8,3.75)(2.2e8,3.58)(4.3e8,3.444)(8.3e8,3.345)(3e9,3.253)(3e10,3.213)};
    
    \addplot+[LineD,smooth,samples=200] coordinates {(1.5e7,4.42)(3.05e7,4.17)(5.9e7,3.93)(1.14e8,3.7)(2.2e8,3.499)(4.3e8,3.32)(8.3e8,3.17)(1.61e9,3.064)(5e9,2.955)(1e10,2.92)(3e10,2.913)};
    
    \addplot+[LineD,smooth,samples=200] coordinates {(1.5e7,4.42)(3.05e7,4.17)(5.9e7,3.93)(1.14e8,3.7)(2.2e8,3.467)(4.3e8,3.25)(8.3e8,3.054)(1.61e9,2.89)(4e9,2.73)(1e10,2.64)(3e10,2.59)};
    
    \addplot+[LineD,smooth,samples=200] coordinates {(1.5e7,4.42)(3.05e7,4.17)(5.9e7,3.93)(1.14e8,3.7)(2.2e8,3.456)(4.3e8,3.223)(8.3e8,3.013)(1.61e9,2.82)(4e9,2.61)(1e10,2.47)(3e10,2.39)};
    
    \node[legend, anchor=south west] at (axis description cs:0.1,0.45) {Params};
\end{axis}
\end{tikzpicture}
```
:::
...
\tikzset{
  axis line/.style={line width=1.0pt},
  Line/.style={line width=1.2pt,RedLine},
  LineD/.style={line width=1.2pt,dashed},
  box/.style={draw, fill=white, minimum width=2cm, minimum height=1cm, anchor=center},
  label/.style={below right, text=black}
}
...
% Axis
\node[axis line] (O) at (0,0) {};
\node[axis line] (Y) at (0,\hi) {};
\draw[axis line] (O) -- node[below=3pt]{Compute} ++(\wi,0);
\draw[axis line] (O) -- node[above=3pt,midway,sloped]{Intelligence} (Y);

% Nodes
\node[box] (T1) at (2,0.88) {};
\node[box] (T2) [above=of T1] {};
\node[box] (T3) [above=of T2] {};
\node[box] (T4) [above=of T3] {};

% Connections
\draw[Line] (O) -- (T1) -- (T2) -- (T3) -- (T4);
\draw[Line,-latex] (O) -- ++(23:3.6) node[label]{Pre-training scaling};
\draw[BlueLine,-latex,LineD] (O) -- ++(23:7.0);
\draw[Line,-latex] (T2) -- ++(27:1.6) node[label]{Post-training scaling};
\draw[RedLine,-latex,LineD] (T2) -- ++(27:4.0);
\draw[Line,-latex] (T3) to [out=40,in=210] ($(T4)+(0.15,0.09)$) node[label,align=center]{Test-time scaling\\ "long thinking"};
\draw[RedLine,-latex,LineD] (T4) -- ++(29:2.0);

% Scaling laws label
\node[align=center,font=\normalsize\usefont{T1}{phv}{m}{n}] at ($(Y)+(0,-0.5)$) {From one to three\\ scaling laws};
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
gpt3_params_b = Gpt3OptimalScenario.gpt3_params_b
```
...
If the D·A·M taxonomy describes the *workload* — the nouns of our system (the Data we have, the Algorithm we want to run, the Machine we bought) — then the C$^3$ taxonomy describes the *execution tax* — the verbs of the fleet (Computing the math, Communicating the results, Coordinating the state).
...
: **The Framework Rosetta Stone**. A mapping of abstract distributed ML primitives to their implementations across major frameworks. Primitives are covered in depth in Part VI (Distribution) and Part VII (Deployment). {#tbl-framework-rosetta-stone}
