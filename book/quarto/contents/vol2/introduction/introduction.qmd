...
GPT-3 consumed an estimated `{python} Gpt3TrainingScenario.gpt3_training_ops_sci` FLOPS during training, requiring thousands of V100 GPUs running for weeks [@brown2020language]. PaLM trained on 6,144 TPU v4 chips for roughly 60 days, consuming approximately $10^{24}$ FLOPS [@chowdhery2022palm]. GPT-4 reportedly trained on approximately 25,000 A100 GPUs over 90--100 days [@openai2023gpt4]. This progression spans roughly seven orders of magnitude in training compute over a single decade, from roughly $10^{18}$ FLOPS for AlexNet to $10^{25}$ FLOPS for GPT-4 [@sevilla2022; @amodei2018ai].
...
This volume tracks three canonical workloads: Archetype A (GPT-4 and Llama-3 frontier language models), Archetype B (DLRM at planetary-scale recommendation), and Archetype C (Federated MobileNet on edge devices). The full roster, C$^3$ taxonomy mapping, and binding-constraint analysis appears at @tbl-vol2-lighthouse-archetypes and @sec-vol2-introduction-archetypes; systems perspectives continue as sidebars throughout, focusing on data-center physics, network topology, and distributed consistency.
...
| **Archetype B (DLRM at Scale)**                                             | Coordination (all-to-all)     | Shard 10 TB+ embedding tables across hundreds of nodes; process millions of QPS with <100 ms tail latency while managing $\mathcal{O}(N^2)$ all-to-all contention.                                       |
