...
This volume addresses the next frontier: taking those systems to production scale where they serve hundreds of millions of users, training models across tens of thousands of accelerators, and governing deployments that affect human lives. The organizing principles of Volume II are **scale**, **distribute**, and **govern**. These three imperatives define what changes when ML systems move from research prototypes to production infrastructure.

**Scale** addresses the infrastructure challenge. Production ML systems process datasets measured in petabytes[^fn-petabyte-scale-vol2], train models with hundreds of billions of parameters[^fn-parameter-scale], and serve predictions at rates exceeding millions of queries per second. Scaling is not simply doing more of the same; it requires fundamentally different architectural approaches, different failure modes to anticipate, and different operational practices to maintain.
...
[^fn-petabyte-scale-vol2]: **Petabyte-Scale Training Data**: Modern foundation models train on datasets that dwarf earlier ML systems. GPT-3 trained on approximately 570GB of filtered text [@brown2020language], while subsequent models use multi-petabyte corpora. Common Crawl, a primary data source for language models, generates approximately 100TB per monthly crawl with the cumulative archive containing petabytes of web content. LAION-5B, used for training image-text models, comprises 5.85 billion image-text pairs. From a systems perspective, datasets of this scale cannot fit on any single machine and require distributed storage architectures. Reading 1 petabyte at typical SSD speeds (500 MB/s) would take 23 days; production systems achieve this in hours through parallel reads across thousands of storage nodes, each contributing bandwidth to achieve aggregate throughput exceeding 100 GB/s.

[^fn-parameter-scale]: **Model Parameter Scale**: The parameter count of frontier models has grown exponentially. GPT-2 (2019) had 1.5 billion parameters; GPT-3 (2020) reached 175 billion; PaLM (2022) scaled to 540 billion [@chowdhery2022palm]; and unverified industry estimates suggest GPT-4 uses approximately 1.8 trillion parameters across a Mixture of Experts architecture, though OpenAI has not publicly disclosed the model's specifications. Each parameter requires 2 to 4 bytes of storage depending on precision, meaning a 175B parameter model requires 350 to 700GB just to store weights. Training requires additional memory for gradients and optimizer states, typically 12 to 20 bytes per parameter, pushing GPT-3 scale training to require approximately 2 to 3.5TB of aggregate GPU memory. No single accelerator provides this capacity; distributed training across hundreds to thousands of devices becomes mandatory.
...
Consider the trajectory of compute requirements. AlexNet (2012) trained on two GTX 580 GPUs for approximately 5 to 6 days [@krizhevsky2012imagenet]. BERT (2018) required 64 TPU chips for 4 days, roughly 6,144 chip hours [@devlin2018bert]. GPT-3 (2020) consumed an estimated 3.14×10²³ FLOPS during training, requiring thousands of V100 GPUs running for weeks [@brown2020language]. GPT-4 (2023) reportedly trained on approximately 25,000 A100 GPUs over 90 to 100 days [@openai2023gpt4]. This represents a 10,000 fold increase in training compute over a single decade.
...
Volume I established the foundations that this volume extends. If you are beginning with Volume II, this section provides essential context. If you completed Volume I, consider this a brief reminder before we proceed to advanced topics.
...
This volume organizes around the three imperatives, progressing from infrastructure foundations through distribution techniques to governance practices. @tbl-vol2-structure summarizes the four-part structure.
...
: **Volume II Structure**: The four parts progress from infrastructure foundations through distributed systems techniques to production challenges and responsible deployment. {#tbl-vol2-structure}
