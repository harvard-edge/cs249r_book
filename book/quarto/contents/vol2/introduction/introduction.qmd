...
class Gpt3TrainingScenario:
    ...
    gradient_gb = gradient_q.to(GB).magnitude
    ...
    gpt3_gradient_gb_str = fmt_qty(gradient_q, GB, precision=0, commas=False)
```
...
class GPT3SyncTax:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    params = Models.Language.GPT3.parameters.to(param).magnitude
    bw_ib = Systems.Fabrics.InfiniBand_HDR.bandwidth.to(byte/second).magnitude # HDR InfiniBand
    bw_eth = (100 * Gbps).to(byte/second).magnitude # 100 Gb/s Ethernet
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    sync_size_gb_str = fmt_qty(sync_size_bytes * byte, GB, precision=0, commas=True)
