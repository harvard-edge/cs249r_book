...
**Context**: Meta reports training Llama 3 405B on 16,384 H100 GPUs during a 54-day snapshot of pretraining [@dubey2024llama].
...
*   **Archetype B (DLRM at Scale)**: Deep Learning Recommendation Model (DLRM) workloads move from fitting embedding tables in local memory to **Embedding Sharding**\index{Embedding Sharding} across hundreds of nodes, so coordinating sparse feature routing, shard placement, and request scheduling becomes the bottleneck.
...
These archetypes turn the growth curve into an engineering question. The same increase in accelerator count can produce a network wall, a sparse-routing coordination bottleneck, or a per-device compute ceiling. Where the archetypes focus attention on the workload, a parallel set of systems lenses does the same for the substrate, directing attention to data-center physics, network topology, and distributed consistency. Combined with the rise of federated learning[^fn-federated-forward], that diversity has made systems engineering central to ML at scale. A sophisticated algorithm that cannot scale often provides less practical value than a simpler algorithm deployed efficiently across scalable infrastructure.
...
Empirical studies of large language models (LLMs) show that, for a fixed computational budget and within the studied scaling regime, balancing model size and dataset size (measured in tokens[^fn-tokens-bpe]) can minimize training loss.
...
The **CAP Theorem**\index{CAP Theorem}[^fn-cap-theorem-ml] establishes that an asynchronous distributed system subject to a network partition cannot guarantee both atomic **Consistency**\index{CAP Theorem!consistency} and **Availability**\index{CAP Theorem!availability}. **Partition Tolerance**\index{CAP Theorem!partition tolerance} describes the network condition under which that trade-off applies, not a third interchangeable property that a system can simply discard.

[^fn-cap-theorem-ml]: **CAP Theorem** (Brewer's Theorem): Brewer formulated the conjecture [@brewer2000], and Gilbert and Lynch proved that an asynchronous system cannot guarantee both atomic consistency and availability when the network may partition [@gilbert2002]. Synchronous and asynchronous training provide a useful analogy for this trade-off, but gradient freshness is not the atomic consistency property in the theorem.
