...
2.  **Distinction (durable)**: Unlike Traditional Clusters (for example, Spark, MapReduce) that manage independent, asynchronous jobs, an ML Fleet operates under Synchronous Tight Coupling\index{Synchronous Tight Coupling}: all workers must reach the training-step barrier before any can advance, so near-perfect reliability is required to maintain throughput.
