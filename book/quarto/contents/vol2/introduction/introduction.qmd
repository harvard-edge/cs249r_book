...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_sci_flops, check, fmt_memory
...
class InfiniBandStackRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvlink_h100_gbs_str = fmt_qty(nvlink_h100, GB/second, precision=0, commas=False)
    ib_ndr_gbps_str = fmt_qty(ndr, Gbps, precision=0, commas=False)
```

The stack architecture in @fig-vol2-system-scaling-regimes does not change when we scale: every ML system still has hardware, a system envelope, a workload, and a mission. *What* changes is the physics at each layer. Read the figure from bottom to top. At the bottom row, Hardware (NVLink at `{python} InfiniBandStackRecap.nvlink_h100_gbs_str` within one node) becomes Infrastructure (InfiniBand RDMA fabric spanning racks at `{python} InfiniBandStackRecap.ib_ndr_gbps_str` per link), and the bottleneck shifts from the memory wall to the bisection bandwidth wall. One row up, System Software (a single CUDA runtime managing PCIe DMA) becomes Distribution (NCCL and RDMA libraries coordinating thousands of processes across the fabric).
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, assert_qty_close, fmt_memory
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_flop_rate, fmt_sci_flops, check, fmt_qty
...
class Gpt3StatsScenario:
    ...
    gpt3_params_b_str = fmt(params_b, precision=0, commas=False)
    gpt3_training_ops_str = fmt_sci_flops(ops * flop)
    a100_fp16_tflops_str = fmt_flop_rate(a100_throughput_q, unit=TFLOPs/second, precision=0, commas=False)
```
