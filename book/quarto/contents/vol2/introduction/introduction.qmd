...
- Apply the **Systems Sandwich** framework to organize the layers of the Machine Learning Fleet (Physical, Operational, Societal)
...
class Vol2IntroSetup:
    ...
    num_gpus_gpt4 = 25000
    gpu_days_gpt4 = 90
    fail_rate_annual = 0.02 # 2% failure rate
...
Volume I focused on the "AI Moment"—the transition from code-centric to data-centric software. Volume II focuses on the **Scale Moment**\index{Scale Moment}: the physical and operational transformation that occurs when models grow nine orders of magnitude larger. Between 2012 and 2024, the compute required to train a state-of-the-art model increased from $10^{18}$ FLOPS (AlexNet) to $10^{26}$ FLOPS (estimated for GPT-5 class models). This is not just a difference in degree; it is a difference in kind.
...
Volume I introduced the **Iron Law of ML Systems** ($T \approx \text{Data} + \text{Compute} + \text{Overhead}$). At production scale, this law undergoes a transformation. We no longer care about the performance of a single machine; we care about the **Efficiency of the Fleet**\index{Distributed Efficiency}. The **Law of Distributed Efficiency**\index{Law of Distributed Efficiency!equation} formalizes this transition:
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: "The Coordination Tax" callout - Worked example
# │
# │ Goal: Quantify the impact of network bandwidth on scaling efficiency.
# │ Show: That slow networking (Ethernet) collapses efficiency for large models.
# │ How: Calculate T_sync vs T_compute for GPT-3 gradients over IB vs Ethernet.
# │
# │ Imports: mlsys.constants (GPT3_PARAMS, INFINIBAND_HDR_BW, ETHERNET_100G_BW)
...
class GPT3SyncTax:
    ...
    # ┌── 1. PARAMETERS (Inputs) ───────────────────────────────────────────────
    params = GPT3_PARAMS.magnitude
    bw_ib = (200 * Gbps).to('byte/second').magnitude # HDR InfiniBand
    bw_eth = (100 * Gbps).to('byte/second').magnitude # 100G Ethernet
...
Volume I introduced the **Verification Gap**\index{Verification Gap}: the impossibility of testing a high-dimensional model against every possible input. Volume II introduces a more physical challenge: the **Reliability Gap**\index{Reliability Gap!definition}.
...
The engineering lesson: **Failure is the common case.** At scale, we stop trying to prevent failure and start engineering for **Resilience**. We trade *uptime* for *recovery speed*. This shift in mindset—from "How do I keep it running?" to "How do I ensure it self-heals?"—is the defining challenge of Part IV.

## Communication Intensity (The CI Ratio) {#sec-vol2-introduction-ci-ratio}

\index{Communication Intensity}
If the **Iron Law** governs *how* a system executes, the **Communication-Computation Ratio**\index{CI Ratio} governs *where* it stalls. In Volume I, we used the **Roofline Model** to determine if a kernel was compute-bound or memory-bound. In Volume II, we elevate this analysis to the network.
...
Scale is not just "more of the same." Systems that work perfectly at modest scale exhibit qualitatively different behaviors at production scale. Understanding scaling laws and efficiency constraints sets the *requirements*, but fulfilling them requires a fundamental shift in architecture. *Why* do we move from single machines to the Machine Learning Fleet? The answer lies in the unique characteristics of ML workloads.

### ML Workloads Are Unique

*Why* do we need specialized infrastructure for machine learning? *Why* not simply use established distributed systems like Apache Spark or Hadoop? The answer lies in the fundamental characteristics of ML training.

Traditional distributed data processing optimizes for **throughput** on independent tasks. A MapReduce job processes terabytes of logs where each record is handled separately. *When* one node fails, only its local task needs re-execution.

Distributed ML training, by contrast, is **iterative, stateful, and tightly coupled**.

*   **Iterative**: The system repeats the same forward/backward pass millions of times.
*   **Stateful**: All workers must synchronize a shared state (the model parameters).
*   **Tightly Coupled**: Progress is blocked until *all* workers complete their update.

This profile creates different constraints. While a web server optimizes for request latency, an ML cluster must optimize for **all-to-all bandwidth**. A 10% performance drop on one node doesn't just reduce total throughput; in synchronous training, it slows down the entire fleet.
...
This ratio explains *why* distributed training systems optimize communication so aggressively. Gradient compression, kernel overlap, and hierarchical aggregation are not optimizations—they are requirements for scaling.
...
\begin{tikzpicture}[line join=round,font=\usefont{T1}{phv}{m}{n}\small, scale=0.9, transform shape]
  ...
  % Nodes with standard colors
  \node[part, draw=RedLine, fill=RedL] (p5) at (0, 6.4) {Part V: Responsible AI at Scale\\ Governance \& Society};
  \node[part, draw=OrangeLine, fill=OrangeL] (p4) at (0, 4.8) {Part IV: Production Concerns\\ Security, Robustness, Sustainability};
  \node[part, draw=GreenLine, fill=GreenL] (p3) at (0, 3.2) {Part III: Deployment at Scale\\ Inference \& Operations};
  \node[part, draw=BlueLine, fill=BlueL] (p2) at (0, 1.6) {Part II: The Machine Learning Fleet\\ Infrastructure \& Networking};
  \node[part, draw=BrownLine, fill=BrownL!40] (p1) at (0, 0) {Part I: Foundations of Scale\\ Algorithms \& Communication};
  ...
  \begin{scope}[on background layer]
    ...
    \draw[arrow, OrangeLine!40] (p3.north) -- (p4.south);
    \draw[arrow, RedLine!40] (p4.north) -- (p5.south);
  \end{scope}

  % Annotations
  \node[right=3.8cm of p1, text width=4cm, font=\footnotesize] {Atomic Constraints\\ (Power, Bandwidth)};
  \node[right=3.8cm of p3, text width=4cm, font=\footnotesize] {Operational Logic\\ (Distribution, Serving)};
  \node[right=3.8cm of p5, text width=4cm, font=\footnotesize] {Societal Objective\\ (Fairness, Security)};
...
\begin{tikzpicture}[line join=round,font=\usefont{T1}{phv}{m}{n}\small, scale=0.9, transform shape]
  ...
  % Nodes with standard colors
  \node[part, draw=BrownLine, fill=BrownL!40] (p1) at (0, 0) {Part I: Foundations of Scale};
  \node[part, draw=BlueLine, fill=BlueL, below=0.8cm of p1] (p2) {Part II: Building the ML Fleet};
  \node[part, draw=GreenLine, fill=GreenL, below=0.8cm of p2] (p3) {Part III: Deployment at Scale};
  \node[part, draw=OrangeLine, fill=OrangeL, below=0.8cm of p3] (p4) {Part IV: Production Concerns};
  \node[part, draw=RedLine, fill=RedL, below=0.8cm of p4] (p5) {Part V: Responsible AI at Scale};
...
This textbook organizes around the **Systems Sandwich**\index{Systems Sandwich!textbook structure}, progressing from the logical foundations through physical infrastructure to governance.

### Part I: Foundations of Scale
*Why* do we distribute? We establish the logical requirements of scale: **Distributed Training** algorithms, **Communication** primitives, and the mathematics of **Fault Tolerance**.

### Part II: Building the Machine Learning Fleet
*How* do we build it? We turn to the physical supercomputer: **Compute Infrastructure**, **Network Fabrics**, **Storage Hierarchies**, and the **Orchestration** logic that binds them.

### Part III: Deployment at Scale
*Where* does the model live? we move to the network edge: **Inference at Scale**, **Edge Intelligence**, and the operational hardening of **MLOps at Scale**.

### Part IV: Production Concerns
*When* do things break? We address the operational realities of the fleet: **Privacy & Security**, **Robust AI**, and the **Sustainability** of large-scale computation.

### Part V: Responsible AI at Scale
*Who* does the system serve? We reach the control plane: **Responsible AI**, **AI for Good**, and the emerging frontiers of **AGI Systems**.
...
We have established the *requirements* of scale: we know *why* we must distribute and what the physical costs will be. But how do we actually split the math across thousands of independent processors?

In **Distributed Training** (@sec-distributed-training-systems), we move from the "Why" of scale to the "How," examining the specific algorithms that allow a single massive model to live and learn across the Machine Learning Fleet.
