...
This volume is dedicated to the engineering of this fleet: mastering the physics, logic, and governance of machine learning at the limits of modern infrastructure. Between 2012 and the mid-2020s, public and third-party estimates place frontier training compute growth from roughly $10^{18}$ FLOPs for AlexNet to approaching $10^{25}$ FLOPs for leading frontier-class models, roughly seven orders of magnitude. The difference is qualitative, not merely quantitative.
...
@Fig-cluster-size-explosion reveals that cluster sizes have grown by roughly four orders of magnitude in just over a decade, from two GPUs for AlexNet to over 16,000 H100s for Llama 3. This exponential trajectory is the empirical foundation of the scale moment: each generation of frontier models demands exponentially larger fleets of accelerators. The curve alone, however, does not explain *which* constraint breaks first. Frontier language-model training, recommendation serving, and federated mobile learning all encounter fleet scale, but each stresses a different part of the system.
...
*   **Archetype A (GPT-4/Llama-3)**: Frontier language models move from memory bounds on one device to multi-node **Model Parallelism**\index{Model Parallelism} and **Pipeline Parallelism**\index{Pipeline Parallelism}, so communication becomes the bottleneck.
...
The Machine Learning Fleet, by contrast, operates under **Synchronous Tight Coupling**\index{Synchronous Tight Coupling}. While the parameter server architecture [@li2014] introduced ways to manage distributed state, modern frontier models often require even tighter synchronization to maintain performance.
