...
class Gpt4TrainingScenario:
    ...
    hours_per_year_str = fmt_int(hours_per_year, commas=True, suffix=" hours")
...
```{python}
#| echo: false
#| label: vol2-lighthouse-archetype-table
# ┌─────────────────────────────────────────────────────────────────────────────
# │ VOLUME II LIGHTHOUSE ARCHETYPES
# ├─────────────────────────────────────────────────────────────────────────────
# │ Goal: Render the fleet-scale A/B/C archetypes from MLSysIM.
# │ How: Scenarios.Archetypes.Fleet stores only the scenario composition and
# │      teaching metadata; model and system objects stay in their registries.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Scenarios as BookScenarios
from mlsysim.fmt import MarkdownStr

class Vol2LighthouseArchetypeTable:
    """Display fields for the canonical fleet-scale archetypes."""
    ...
    a_constraint_str = MarkdownStr(a.c3_constraint)
    a_challenge_str = MarkdownStr(a.fleet_challenge)
    b_label_str = MarkdownStr(b.label)
    b_constraint_str = MarkdownStr(b.c3_constraint)
    b_challenge_str = MarkdownStr(b.fleet_challenge)
    c_label_str = MarkdownStr(c.label)
    c_constraint_str = MarkdownStr(c.c3_constraint)
    c_challenge_str = MarkdownStr(c.fleet_challenge)
```

| **Archetype**                                                                                 | **C$^3$ Dominant Constraint**                            | **Fleet Challenge**                                     |
|:----------------------------------------------------------------------------------------------|:---------------------------------------------------------|:--------------------------------------------------------|
| **`{python} Vol2LighthouseArchetypeTable.a_label_str`**\index{Llama-3!lighthouse archetype}   | `{python} Vol2LighthouseArchetypeTable.a_constraint_str` | `{python} Vol2LighthouseArchetypeTable.a_challenge_str` |
| **`{python} Vol2LighthouseArchetypeTable.b_label_str`**                                       | `{python} Vol2LighthouseArchetypeTable.b_constraint_str` | `{python} Vol2LighthouseArchetypeTable.b_challenge_str` |
| **`{python} Vol2LighthouseArchetypeTable.c_label_str`**\index{MobileNet!lighthouse archetype} | `{python} Vol2LighthouseArchetypeTable.c_constraint_str` | `{python} Vol2LighthouseArchetypeTable.c_challenge_str` |
