...
\pgfplotsset{myaxis/.style={
  ...
  1000 sep={},
   ...
   legend style={fill=BrownL,draw=BrownLine,row sep=-0.5pt,
...
\def\wi{11}
\def\hl{5/7*\hi}
\draw[axis line](0,-1)coordinate(O)--node[below=3pt]{Training Data Set Size (Log-Scale)}(\wi,-1)coordinate(E);
\draw[axis line](0,-1)--node[above=3pt,midway,sloped]{Generalization Error (Log-Scale)}(0,\hi);
%
...
\path[axis line](A)--++(90:\hi)coordinate(LG1);
\path[axis line](B)--++(90:\hi)coordinate(LG2);

\draw[smooth,BlueLine,axis line](G1)--
...
\node[main, draw=BrownLine, fill=BrownL] (compute) at (90:4.2cm) {\normalsize\textbf{Compute} ($C_1$)\\[1pt]\scriptsize The Math};
...
\node[center node] (center) at (0,0) {\small\textbf{Conservation}\\\small\textbf{of Overhead}};

% Double-headed arrows with trade-off labels
\draw[arrow line, BrownLine] (compute) -- node[edgelbl, pos=0.35] {Bandwidth\\[-1pt]Bound} (comm);
\draw[arrow line, BlueLine] (comm) -- node[edgelbl, pos=0.5, below=2pt] {Synchronization\\[-1pt]Cost} (coord);
\draw[arrow line, GreenLine] (coord) -- node[edgelbl, pos=0.65] {Pipeline\\[-1pt]Bubbles} (compute);
...
\begin{tikzpicture}[line join=round,font=\usefont{T1}{phv}{m}{n}\small, scale=0.9, transform shape]
  ...
  \begin{scope}[on background layer]
    ...
    \draw[arrow, BlueLine!40] (p2.north) -- (p3.south);
    \draw[arrow, BlueLine!40] (p3.north) -- (p4.south);
  \end{scope}
...
\begin{tikzpicture}[line join=round,font=\usefont{T1}{phv}{m}{n}\small, scale=0.9, transform shape]
  ...
  \node[part, draw=BlueLine, fill=BlueL!25, text=BlueLine] (p1) {Part I: The Fleet};
  \node[part, draw=BlueLine, fill=BlueL!50, text=BlueLine, below=of p1] (p2) {Part II: Distributed ML};
  \node[part, draw=BlueLine, fill=BlueL!75, text=white, below=of p2] (p3) {Part III: Deployment at Scale};
  \node[part, draw=BlueLine, fill=BlueLine, text=white, below=of p3] (p4) {Part IV: The Responsible Fleet};
...
\node[circle,draw=\channelcolor,line width=\Linewidth,fill=\fillcolor,
...
\foreach \j in {1,2,3} {
  ...
  \pic at (-0.8,\y) {circles={channelcolor=RedLine,fillcolor=RedL,picname=1CD\j}};
}
\foreach \i in {1,...,4} {
  \pgfmathsetmacro{\y}{(2-\i)*0.43+0.7}
  \pic at (0,\y) {circles={channelcolor=RedLine,fillcolor=RedL, picname=2CD\i}};
}
\foreach \j in {1,2} {
  \pgfmathsetmacro{\y}{(1-\j)*0.43 + 0.7}
  \pic at (0.8,\y) {circles={channelcolor=RedLine,fillcolor=RedL,picname=3CD\j}};
}
\foreach \i in {1,2,3}{
...
\node[draw=GreenLine,fill=GreenL,line width=0.5pt,rounded corners=1pt,minimum width=1.6cm,minimum height=1.6cm,inner sep=0pt] at (0,0) {};
\foreach \y in {-0.4,0,0.4} {
  \node[draw=GreenLine,fill=white,line width=0.4pt,minimum width=1.2cm,minimum height=0.24cm,inner sep=0pt] at (0,\y) {};
  \node[circle,fill=GreenLine,minimum size=0.12cm,inner sep=0pt] at (-0.4,\y) {};
  \node[circle,fill=GreenLine,minimum size=0.12cm,inner sep=0pt] at (0.4,\y) {};
}
\end{scope}
...
\draw[OrangeLine,fill=OrangeL,line width=0.5pt] (0,0.6) ellipse (0.7 and 0.2);
\draw[OrangeLine,fill=OrangeL,line width=0.5pt] (-0.7,0.6) -- (-0.7,-0.2) arc (180:360:0.7 and 0.2) -- (0.7,0.6);
\draw[OrangeLine,fill=OrangeL,line width=0.5pt] (0,-0.2) ellipse (0.7 and 0.2);
\draw[OrangeLine,fill=OrangeL,line width=0.5pt] (0,-0.6) ellipse (0.5 and 0.15);
\draw[OrangeLine,fill=OrangeL,line width=0.5pt] (-0.5,-0.6) -- (-0.5,-1.2) arc (180:360:0.5 and 0.15) -- (0.5,-0.6);
\end{scope}
