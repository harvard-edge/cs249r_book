...
We begin by examining how compute requirements have evolved over the past decade, revealing why systems engineering has become central to AI advancement. This historical perspective sets the stage for understanding the three qualitative changes that emerge at production scale: communication dominance, routine failure, and the governance requirements that accompany societal impact.
...
[^fn-all-reduce]: **Ring All-Reduce**: The dominant collective communication algorithm for distributed training gradient synchronization [@patarasuk2009bandwidth]. In ring all-reduce, N workers arrange logically in a ring. Each worker sends 1/N of its gradient to its neighbor, which adds the received gradient to its own before forwarding. After N-1 steps, each worker has the sum of all gradients for 1/N of parameters. A second ring pass distributes the complete sum to all workers. The algorithm achieves optimal bandwidth utilization. Total data transferred equals 2(N-1)/N times the gradient size, approaching 2× regardless of worker count. For 1,000 workers with 700GB of gradients and 200 Gb/s links, theoretical completion time is approximately 2 × 700GB / 200 Gb/s = 56 seconds for the full reduce-scatter plus all-gather. Practical implementations achieve 60-80% of theoretical bandwidth, making communication a significant fraction of large-scale training time.
...
[^fn-gradient-compression]: **Gradient Compression**: Techniques that reduce the size of gradient data transmitted between workers during distributed training. Common approaches include sparsification (transmitting only the largest gradient values, with Top-K methods achieving 99% sparsity), quantization (reducing precision from 32-bit to 8-bit or lower), and error feedback (accumulating dropped gradients to prevent training divergence). Deep Gradient Compression [@lin2018deep] demonstrated that transmitting only 0.1% of gradients with error feedback achieves training accuracy comparable to uncompressed baselines while reducing communication by 270-600×. The trade-off is additional computation for compression and decompression, plus potential convergence slowdown from information loss. Modern implementations like PowerSGD use low-rank decomposition to balance compression ratio with computational overhead, achieving 10× compression with minimal accuracy impact on production workloads.
...
[^fn-checkpointing]: **Checkpointing in Distributed Training**: The practice of periodically saving model state (weights, optimizer state, training progress) to persistent storage, enabling recovery from failures without restarting from scratch. For frontier models, checkpoint sizes reach terabytes (GPT-3's checkpoint exceeds 700GB for weights alone; with optimizer state and training metadata, checkpoints can exceed 2TB). At scale, checkpointing itself becomes a systems challenge: writing 2TB every hour across 1,000 workers requires careful coordination to avoid I/O bottlenecks. Asynchronous checkpointing overlaps with training, while distributed checkpointing shards state across workers to parallelize writes. The checkpoint frequency trade-off balances storage costs and I/O overhead against maximum acceptable work loss during failure recovery. Meta's LLaMA 3 training checkpointed approximately every 1-2 hours to balance these concerns.
...
Scale forces distribution: no single machine provides the thousands of GPUs that frontier training requires, no single datacenter serves global user bases with acceptable latency, and no centralized system can collect the distributed data that edge devices generate. Yet distribution is not merely scale by another name. Coordinating computation across physically separated machines connected by finite bandwidth, non-zero latency networks creates fundamental constraints that exist even at modest scales. These constraints arise from the physics of information transfer, not from the quantity of computation, and no amount of engineering cleverness can eliminate them.
...
[^fn-differential-privacy]: **Differential Privacy**: A mathematical framework providing provable privacy guarantees by ensuring that query results are nearly identical whether or not any individual's data is included. The privacy guarantee is quantified by epsilon (lower values mean stronger privacy). In ML, differential privacy is typically achieved through gradient clipping (bounding individual contributions) and noise addition (obscuring individual influences). Apple uses differential privacy with epsilon around 2-8 for keyboard predictions, while Google's federated learning implementations target epsilon around 1-2 for sensitive applications. The trade-off is utility: stronger privacy requires more noise, degrading model accuracy. Differential privacy is explored in depth in @sec-security-privacy.
...
The challenges we have examined form an interconnected system where decisions in one domain cascade through others. Communication dominating computation, failure becoming routine, heterogeneity emerging unavoidably, coordination overhead limiting parallelism, edge constraints demanding new architectures, and societal impact requiring governance are not independent problems to solve in isolation. To reason systematically about these interconnections, we need organizing frameworks. This section introduces three such frameworks, showing how each helps structure the engineering decisions you will face throughout this textbook.
...
[^fn-data-parallelism]: **Data Parallelism**: The most common distributed training strategy, where each worker holds a complete copy of the model but processes different data batches. Workers compute gradients independently, then synchronize via collective operations (typically AllReduce) to ensure all copies remain identical. Data parallelism scales well when the model fits in single-device memory and communication bandwidth supports gradient synchronization. Effective batch size grows with worker count, requiring learning rate adjustments; GPT-3 training used data parallelism across hundreds of workers with careful hyperparameter tuning to maintain convergence.

[^fn-model-parallelism]: **Model Parallelism**: Distributes model parameters across multiple devices, enabling training of models too large for single-device memory. Tensor parallelism partitions individual layers (e.g., splitting large matrix multiplications across GPUs), while expert parallelism routes different inputs to different model subnetworks. Model parallelism introduces point-to-point communication dependencies between devices holding adjacent partitions, making high-bandwidth interconnects (NVLink, NVSwitch) critical for efficiency. Megatron-LM demonstrated tensor parallelism enabling 8.3 billion parameter models across 8 GPUs with 77% scaling efficiency.

[^fn-pipeline-parallelism]: **Pipeline Parallelism**: Partitions the model by layers across devices, with each device handling a subset of layers. Micro-batches flow through the pipeline, allowing devices to work concurrently on different batches at different stages. The challenge is pipeline bubbles (idle time at startup and completion) and memory overhead from storing activations for backward passes. GPipe and PipeDream demonstrated techniques to reduce bubble overhead from O(pipeline_depth) to O(1) through careful scheduling. Combined with data and tensor parallelism, pipeline parallelism enables training of trillion-parameter models like GPT-4 across thousands of devices.
...
[^fn-allgather]: **AllGather**: A collective operation where each worker contributes data and receives the concatenation of all workers' contributions. Unlike AllReduce (which combines values via summation), AllGather preserves each worker's distinct data. AllGather is essential for model parallelism: after computing partial outputs on sharded layers, workers must gather results to proceed. For N workers each holding M bytes, AllGather transfers N×M bytes total, with each worker receiving N×M bytes. Combined with ReduceScatter (the inverse operation that distributes a reduction), these primitives implement efficient gradient synchronization for models sharded across devices.
...
The six systems engineering principles introduced earlier provide a vision for building ML systems that matter. This textbook extends that vision to the scale at which most consequential ML systems operate.
