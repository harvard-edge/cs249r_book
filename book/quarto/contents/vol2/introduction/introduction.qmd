...
Consider the trajectory of compute requirements. AlexNet (2012) trained on two GTX 580 GPUs for approximately 5 to 6 days [@krizhevsky2012imagenet]. BERT (2018) required 64 TPU[^fn-tpu] chips for 4 days, roughly 6,144 chip hours [@devlin2018bert].
...
This ratio explains why distributed training systems optimize communication aggressively. Gradient compression[^fn-gradient-compression] reduces transfer volume by 10-100× at the cost of some accuracy [@lin2018deep]. Overlapping communication with computation hides transfer latency during the next batch's forward pass. Hierarchical aggregation reduces cross-rack traffic by combining gradients locally first. These optimizations, unnecessary at small scale, become essential at production scale.
...
This section establishes the organizing frameworks that structure ML systems engineering. These concepts provide essential context for the distributed systems challenges examined throughout this textbook.

**The AI Triad** provides the organizing framework for understanding ML systems. Every machine learning system comprises three interdependent components: data that guides behavior, algorithms that learn patterns, and computational infrastructure that enables both training and inference. These components exist in dynamic tension. Larger models require more data and compute. Larger datasets enable more sophisticated models but demand storage and processing infrastructure. At the scales examined in this textbook, these interdependencies intensify. Distributed training requires coordinating the AI Triad's components across thousands of machines rather than optimizing them on a single system.
