...
# │
# │ Imports: mlsysim.core.units (EFLOP, second), mlsysim.fmt
# │ Exports: TpuV4PodScenario.tpuv4_pod_chips_str,
# │          TpuV4PodScenario.tpuv4_pod_peak_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.units (Models.Language.GPT3.parameters, Models.Language.GPT3.training_ops,
# │          Hardware.Cloud.V100.compute.peak_flops, GB, param, BILLION,
# │          BYTES_FP32), mlsysim.book (sci, check)
# │ Exports: gpt3_params_b_str, gpt3_training_ops_str, gpt3_gradient_gb_str,
# │          gpt3_accelerators_str, gpt3_v100_peak_min_days_str
# └─────────────────────────────────────────────────────────────────────────────
from math import ceil
from mlsysim import Hardware
from mlsysim.fmt import fmt, fmt_count, fmt_sci_flops, check, fmt_memory, fmt_params, fmt_time, MarkdownStr
...
class Gpt3TrainingScenario:
    ...
    gpt3_v100_peak_min_days_str = MarkdownStr(f"at least {fmt_time(ceil(v100_peak_min_days_q.magnitude), day, precision=0, style='word')}")
```
...
| **GPT-3**                |   2020   |   `{python} Gpt3TrainingScenario.gpt3_accelerators_str` (run est.)  |     `{python} Gpt3TrainingScenario.gpt3_v100_peak_min_days_str`     | `{python} Gpt3TrainingScenario.gpt3_training_ops_str` |
...
The **Linear Scaling Fallacy**\index{Linear Scaling Fallacy} occurs when teams extrapolate resource requirements using power-law relationships without accounting for coordination overhead. $\mathcal{L}(X) = A_{\mathcal{L}} X^{-\alpha_{\text{scale}}} + C_{\mathcal{L}}$ works within validated ranges but fails at the boundaries.
