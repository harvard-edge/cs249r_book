...
Compute requirements have grown exponentially. AlexNet\index{AlexNet} trained on two GTX 580 GPUs for approximately 5--6 days [@krizhevsky2012imagenet]. BERT\index{BERT} required 64 Tensor Processing Unit (TPU)[^fn-tpu-systolic] chips for 4 days, roughly 6,144 chip hours [@devlin2019].
...
**Lighthouse Archetypes**\index{Lighthouse Archetypes!definition} are the canonical workloads that keep the scale moment concrete throughout this book. At this point, their role is to show that "more machines" is not one engineering problem: the dominant constraint depends on what must cross the fleet boundary. The full roster with the C$^3$ taxonomy mapping and binding-constraint analysis appears at @sec-vol2-introduction-archetypes.
...
The ML fleet demands the **Warehouse-Scale Computer (WSC)**\index{Warehouse-Scale Computer!definition}[^fn-wsc-barroso] perspective. In traditional computing, the data center is a building that houses many computers. In the ML fleet, the data center is the computer.
