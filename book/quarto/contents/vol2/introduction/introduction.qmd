...
::: {layout-narrow}
\noindent
![Volume 2 Introduction: Scale, Distribute, Govern](images/png/cover_introduction.png){#fig-intro-cover}

:::

@fig-intro-cover introduces the three imperatives that define advanced ML systems engineering: scale, distribution, and governance.
...
The history of machine learning is defined by scale. Each major capability leap has come not from algorithmic breakthroughs alone, but from the ability to apply computation at previously impossible scales. Understanding how compute requirements have evolved over the past decade reveals why systems engineering has become central to AI advancement and sets the stage for understanding the three qualitative changes that emerge at production scale: communication dominance, routine failure, and the governance requirements that accompany societal impact.
...
Scale is not just a larger version of small. Systems that work perfectly at modest scale exhibit qualitatively different behaviors at production scale. Understanding these transitions prepares you for the engineering challenges examined throughout this volume.
...
**The Systems Sandwich** (@fig-systems-sandwich) organizes the complexity of Volume II into three distinct layers.

::: {#fig-systems-sandwich fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  \node[draw, rounded corners=5pt, fill=Layer3, minimum width=8cm, minimum height=1.5cm, align=center] (l3) at (0, 3) {\textbf{Societal Layer (Governance)}\\Safety, Ethics, Regulation, Impact};
  ...
  \node[draw, rounded corners=5pt, fill=Layer1, minimum width=8cm, minimum height=1.5cm, align=center] (l1) at (0, -0.6) {\textbf{Physical Layer (Infrastructure)}\\Silicon, Power, Cooling, Network Physics};
...
\end{tikzpicture}
```
**The Systems Sandwich**. The organizing framework for Volume II. We build from the **Physical Layer** (constraints of atoms and energy) up to the **Operational Layer** (distributed logic and algorithms) and finally to the **Societal Layer** (human impact and governance). Engineering decisions at the bottom constrain possibilities at the top.
:::

1.  **The Physical Layer (Infrastructure)**: This is the foundation. It deals with the hard constraints of physics—power density, cooling, interconnect bandwidth, and silicon limits. This is where we build the "Machine Learning Fleet."
...
3.  **The Societal Layer (Governance)**: This is the interface with the human world. It ensures the system operates safely, legally, and ethically. It includes security, privacy, and responsible AI.

The textbook follows this layered progression, organized into five Parts that correspond to different tiers of the system stack (see @fig-vol2-roadmap).
...
@fig-vol2-ai-triad illustrates this triad and the interdependencies between its components.

![The AI Triad: Data, Algorithms, and Infrastructure.](images/png/placeholder.png){#fig-vol2-ai-triad}
...
To bridge the gap between abstract principles and concrete engineering, this textbook employs a longitudinal narrative strategy. Rather than using isolated, disconnected examples in each chapter, we will trace the engineering evolution of three distinct system archetypes. These archetypes represent the fundamental constraint regimes of modern ML systems: throughput-bound, latency-bound, and power/privacy-bound. By revisiting these same three systems across different chapters, you will see how the *same* physics of distribution manifests differently depending on the primary constraint.

### Archetype A: The Trillion-Parameter LLM (Throughput-Bound)
...
### Archetype B: The Global Real-Time Recommendation Engine (Latency-Bound)
...
### Archetype C: The Autonomous Health Monitor (Power/Privacy-Bound)
...
@tbl-vol2-structure summarizes this five-part structure.

::: {#fig-vol2-roadmap fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  \tikzset{
...
\end{tikzpicture}
```
**Volume 2 Roadmap**. The textbook structure follows the system lifecycle. **Part I** establishes the mathematical and physical foundations of scale. **Part II** applies these to build the distributed training "fleet". **Part III** covers the deployment of trained models to global users. **Part IV** addresses the operational hardening required for production. **Part V** elevates to the governance layer, ensuring systems are responsible and beneficial.
:::
...
| **II: Building the Machine Learning Fleet** | **Build**: Physical infrastructure and storage systems | Compute Infrastructure, Networking & Orchestration, Storage |
...
| **V: Responsible AI at Scale** | **Govern**: Ensuring beneficial outcomes at scale | Responsible AI, AI for Good, Frontiers |

: The five parts progress from algorithmic foundations through physical infrastructure and deployment to production concerns and responsible governance. {#tbl-vol2-structure}
...
With the algorithmic foundations established, we turn to the physical reality. The communication and reliability requirements defined in Part I must be satisfied by concrete hardware. The terabytes of gradient synchronization and the checkpointing demands require a massive, interconnected supercomputer. Part II examines the "Fleet" that executes modern ML workloads.

**Compute Infrastructure** examines the hardware units at the heart of the fleet: GPU clusters with NVLink interconnects, high-density power and cooling architectures, and the accelerator selection trade-offs that determine cost and performance.

**Cluster Networking and Orchestration** extends to the fabric that binds these nodes together. You will explore InfiniBand and RoCE networks, cluster topologies, and the scheduling systems like Slurm and Kubernetes that manage resource allocation for thousands of concurrent jobs.
...
The path forward begins with the physical foundations of scale: the datacenter infrastructure (@sec-infrastructure) and high-performance storage systems (@sec-storage) that provide the raw capacity for the Machine Learning Fleet. Once we have built the metal and the memory, we will then explore the distributed logic—communication patterns and training algorithms—that coordinates these resources into a single global engine.
