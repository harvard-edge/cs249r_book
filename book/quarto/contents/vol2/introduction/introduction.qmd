...
# GPU specs
a100_fp16_tflops = f"{A100_FLOPS_FP16_TENSOR.to(TFLOPs / second).magnitude:.0f}"
a100_mem_bw_gbs = f"{A100_MEM_BW.to(GB / second).magnitude:,.0f}"
a100_mem_bw_tbs = f"{A100_MEM_BW.to(TB / second).magnitude:.0f}"
h100_fp8_tflops = f"{H100_FLOPS_FP8_TENSOR.to(TFLOPs / second).magnitude:,.0f}"
v100_mem_bw_gbs = f"{V100_MEM_BW.to(GB / second).magnitude:.0f}"
...
ib_hdr_gbs = f"{INFINIBAND_HDR_BW.to(Gbps).magnitude / 8:.0f}"
ib_ndr_gbs = f"{INFINIBAND_NDR_BW.to(Gbps).magnitude / 8:.0f}"

# Model specs
gpt3_params_b = f"{GPT3_PARAMS.to(param).magnitude / 1e9:.0f}"
gpt3_training_ops_sci = sci(GPT3_TRAINING_OPS)
gpt3_gradient_gb = f"{GPT3_PARAMS.to(param).magnitude * 4 / 1e9:.0f}"
```
