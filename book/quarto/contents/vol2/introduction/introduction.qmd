...
Compute requirements have grown exponentially. AlexNet trained on two GTX 580 GPUs for approximately 5--6 days [@krizhevsky2017]. BERT required 64 Tensor Processing Unit (TPU)[^fn-tpu-systolic] chips for 4 days, roughly 6,144 chip hours [@devlin2018bert].
...
Optimal compute allocation follows from the scaling relationship $D \propto N^{0.74}$ [@borgeaud2022], which shows that dataset size $D$ and model size $N$ must grow in coordinated proportions for a fixed budget. As model size increases, the dataset should grow at roughly three-quarters the rate to maintain compute-optimal efficiency.
