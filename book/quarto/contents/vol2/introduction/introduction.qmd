...
The single-node D·A·M lens reviewed in @sec-appendix-systems-dam optimizes the workload on one node. This volume **projects** that D·A·M workload onto a distributed fleet. *When* we stretch the nouns across thousands of machines, we are forced to pay the verbs:
