...
**Problem**: You are managing a training run for a GPT-4 class model using `{python} Gpt4TrainingScenario.gpt4_gpus_str` GPUs. If each individual GPU has an annual failure rate of 8%, how often will your training job be interrupted by hardware failure?
...
GPT-3 (2020) expanded to `{python} Gpt3StatsScenario.gpt3_params_b` billion parameters and demonstrated sophisticated text generation across diverse domains. Each increase in model size brought substantially improved capabilities, but at exponentially increasing costs.
