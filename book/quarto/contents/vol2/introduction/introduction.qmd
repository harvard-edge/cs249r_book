...
## The Learning Journey {#sec-vol2-introduction-learning-journey}

Each Part of this textbook addresses a fundamental question that ML systems engineers face when building at scale.

**Part I: Foundations of Scale** — *What does the landscape of large-scale ML look like?*

Before building distributed systems, you must understand the physical substrate. What does a modern ML datacenter contain? How do storage systems handle petabyte-scale datasets? Part I surveys this landscape so you understand what infrastructure capabilities exist before learning to leverage them.

**Part II: Distributed Training** — *How do we train models too large for any single machine?*

When model size exceeds single-device memory, training must span multiple machines. What parallelism strategies partition work effectively? How do machines coordinate gradient updates? What happens when machines fail mid-training? Part II teaches the core techniques of distributed training.

**Part III: Deployment at Scale** — *How do we serve predictions to millions of users reliably?*

Training produces a model; deployment delivers value. How do serving systems maintain latency under variable load? Where does edge deployment make sense versus cloud? How do we operate ML systems in production? Part III addresses getting models into users' hands.

**Part IV: Production Concerns** — *How do we keep large-scale ML systems safe and sustainable?*

Scale amplifies risks. How do we protect user privacy when models memorize training data? How do we maintain reliability when adversaries actively attack? How do we reduce the environmental footprint of massive compute? Part IV addresses operational safety.

**Part V: Responsible AI at Scale** — *How do we ensure ML systems benefit humanity?*

Technical excellence is insufficient. How do we ensure systems operate fairly across populations? How do we govern AI decisions that affect human lives? What does beneficial AI look like in practice? Part V addresses societal responsibility.
