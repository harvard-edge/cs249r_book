...
1. **Non-blocking (1:1)**: $\text{BW}_{\text{bisect}} =$ `{python} BisectionScaling.bbs_gpus_per_side_value_str` $\times 50 \text{ GB/s} =$ `{python} BisectionScaling.bbs_bw_1_1_gb_per_s_str` per direction.
...
2. **Rail-optimized**: 1 traversal $\times$ `{python} RailLatency.rol_hop_lat_us_str` = `{python} RailLatency.rol_rail_lat_us_str`.
