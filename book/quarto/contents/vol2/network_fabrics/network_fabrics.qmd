...
## Introduction {#sec-network-fabrics-introduction}
...
Understanding these communication patterns is essential because the fabric's physical properties constrain which patterns are efficient and which become bottlenecks.
...
**RDMA** is a technology that allows a network adapter to transfer data directly between the memory of two computers without involving the operating system kernel or CPU. It provides **Kernel Bypass** (eliminating context switches) and **Zero-Copy** (eliminating memory-to-memory copies) capabilities.
:::
...
The **$\alpha$-$\beta$ model** (or Hockney model) predicts the time $T$ to send a message of size $n$ bytes as:
...
::: {#nb-allreduce-bottleneck .callout-notebook title="Napkin Math: When Does AllReduce Become the Bottleneck?"}
...
A **Fat-Tree** (or Clos) network is a multi-tier topology where aggregate bandwidth increases toward the root. It provides multiple equal-cost paths between any two nodes, ensuring the fabric can sustain full injection from all endpoints simultaneously without blocking.
:::
...
::: {#nb-bisection-bottleneck .callout-notebook title="Napkin Math: The Bisection Bottleneck"}
...
**PFC** (IEEE 802.1Qbb) is a link-level mechanism that prevents buffer overflow by sending PAUSE frames to the upstream sender. While it ensures losslessness, it can cause **congestion spreading** (where a pause cascades across the fabric) and **PFC storms** (deadlocks) that freeze all communication.
:::
