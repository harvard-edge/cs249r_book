...
class TransceiverDegradationRisk:
    ...
    no_degradation_prob_math = fmt_math(f"\\Pr(\\text{{no degradation}}) = (1 - 0.00001)^{{{total_transceivers_str}}} " f"\\approx {no_degradation_prob_str}")
    any_degradation_prob_math = fmt_math(f"\\Pr(\\text{{at least one degradation}}) = 1 - {no_degradation_prob_str} " f"= \\mathbf{{{any_degradation_prob_decimal_str}}}")
