...
***α-β Model (Hockney Model)***\index{Alpha-Beta Model!definition} is a linear cost model for network communication where the time to transfer a message of $n$ bytes is $T(n) = \alpha + n/\beta$.
...
t_comp_display_math = md(f"$$ T_{{\\text{{compute}}}} = {AllReduceBottleneck.t_comp_ms} \\text{{ ms}} $$")
t_ring_1b_math = md_math(f"T_{{\\text{{ring}}}} \\approx {AllReduceBottleneck.t_comm_1b_ms} \\text{{ ms}}")
comm_frac_display_math = md(f"$$ \\text{{Comm. fraction}} = {AllReduceBottleneck.comm_frac_1b_pct} \\% $$")
t_ring_70b_display_math = md(f"$$ T_{{\\text{{ring}}}} \\approx {AllReduceBottleneck.t_comm_70b_ms} \\text{{ ms}} $$")
```

**Setup**: A cluster of 1,024 H100 GPUs training a model with 1 billion parameters (4 GB of FP32 gradients). Each GPU computes at `{python} NetworkFabricsContextScenario.h100_tflops` TFLOPS. The network uses NDR InfiniBand (`{python} alpha_math` and `{python} beta_math` per link).
