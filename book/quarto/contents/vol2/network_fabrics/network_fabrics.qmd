...
As cluster sizes push past 100,000 accelerators, the physics tax of current interconnects (power, latency, and reliability) becomes the primary scaling bottleneck. The durable question is which constraint each new technology relaxes: lossless transport without PFC cascades, lower power per bit, fewer switching hops, or a more flexible compute-memory-network ratio.

The **Ultra Ethernet Consortium (UEC)**\index{Ultra Ethernet} targets the reliability side of this bottleneck by trying to make Ethernet behave more like an HPC fabric without inheriting RoCEv2's PFC fragility. With that constraint in view, UEC integrates HPC-native features directly into the standard: packet spraying across multiple paths, flexible ordered and unordered reliable delivery services, packet-trimming and link-layer-retry mechanisms, and new congestion-control schemes intended to reduce reliance on Priority Flow Control for AI/HPC traffic. The goal is to provide the losslessness of InfiniBand with the ubiquity and multi-vendor economics of Ethernet.
