...
# │
# │ Imports: mlsysim.core.units (Systems.Fabrics.InfiniBand_NDR.bandwidth, Hardware.Cloud.H100.nvlink.bandwidth,
# │           Hardware.Cloud.H100.compute.peak_flops, Hardware.Cloud.H100.memory.bandwidth,
# │           Gbps, GB, TB, second, TFLOPs)
# │ Exports: NetworkFabricsContextScenario.ib_ndr_gbit_per_s_str,
# │          NetworkFabricsContextScenario.ib_ndr_gb_per_s_str,
# │          NetworkFabricsContextScenario.nvlink_h100_gb_per_s_str,
# │          NetworkFabricsContextScenario.nvlink_to_ib_ratio_mult_str,
# │          NetworkFabricsContextScenario.h100_fp16_tflop_per_s_str,
# │          NetworkFabricsContextScenario.h100_mem_bw_tb_per_s_str
...
@Fig-bandwidth-hierarchy reveals that this cliff is not an artifact of one accelerator generation: the NVLink-to-InfiniBand ratio has remained between 12$\times$ and 25$\times$ across four GPU generations, despite absolute bandwidth improvements at every tier. The persistent cliff reflects fundamental physics: on-package interconnects (NVLink) operate over millimeters of copper, while inter-node links (InfiniBand) span meters of cable and must traverse switches. The ratio determines which parallelism strategies are efficient. Tensor parallelism, which requires continuous high-bandwidth exchange of activations, is viable within a node but impractical across nodes. Pipeline and data parallelism, which tolerate lower inter-node bandwidth, must carry the burden of cross-node communication. Every topology and protocol decision in this chapter attempts to minimize the impact of this hierarchy on collective communication performance.
...
ML training workloads are the complete opposite: synchronous, periodic, and dominated by a small number of massive, collective communication operations. This ML networking inversion\index{ML Networking!data-center workload inversion} reverses the core assumptions of traditional network design.
...
[^fn-bsp-valiant]: **BSP (Bulk Synchronous Parallel)**\index{Bulk Synchronous Parallel!origin}: Proposed by @valiant1990 as a "bridging model" between parallel hardware and software, analogous to von Neumann's model for sequential machines. ML training adopted BSP because it guarantees mathematical equivalence to single-device training at the cost of a global barrier per step, making the network fabric's tail latency the binding constraint on cluster throughput.
...
- **Level 4: Fabric Behavior** (@sec-network-fabrics-behavior). Congestion control mechanisms such as DCTCP-style Explicit Congestion Notification (ECN) response, DCQCN, and HPCC, along with adaptive routing and incast behavior, determine whether theoretical bandwidth translates to realized throughput [@alizadeh2010; @zhu2015; @li2019].
...
# │ Exports: IbNdrWireRecap.ib_ndr_gbit_per_s_str
...
1.  **Significance (quantitative)**: PAM4 enables 400 Gb/s and 800 Gb/s link speeds that sustain the $\text{BW}$ required for large-scale gradient synchronization. However, the reduced gap between voltage levels increases susceptibility to noise, requiring Forward Error Correction (FEC), typically Reed-Solomon RS(544,514), that adds 100–200 ns of irreducible processing latency per hop. Across a three-tier fat-tree (three hops each way), FEC alone contributes 600–1,200 ns of fixed latency to every packet, setting a hard floor on the fabric's fixed per-message startup latency, the quantity that @sec-network-fabrics-performance-model formalizes as the $\alpha$ term of the communication cost model.
2.  **Observation**: Unlike NRZ (Non-Return-to-Zero) binary signaling, which uses only two voltage levels and offers better noise margin but is limited to half the data rate per lane, PAM4 trades signal-to-noise ratio for bandwidth density—the same copper trace carries twice the data at the cost of requiring FEC at every port.
3.  **Pitfall**: A frequent misconception is that doubling bits per symbol is a free throughput gain. PAM4 links require FEC at both endpoints, making them fundamentally higher-latency than equivalent NRZ links; latency-sensitive collectives like AllReduce pay this FEC tax on every packet regardless of message size.
...
@Fig-pam4-vs-nrz shows the signaling trade-off directly: PAM4 doubles the bits per symbol relative to NRZ, but the smaller voltage gaps reduce noise margin and make FEC part of the link's latency floor.
...
The choice of physical medium determines how far a link can reach before latency, power, and cost reshape the cluster geometry. The three recurring categories serve different parts of that placement decision:
...
[^fn-fec-latency]: **FEC (Forward Error Correction)**\index{FEC!latency tax}: At 400G and 800G speeds, signal integrity is so fragile that the hardware must use FEC to reconstruct corrupted bits. This adds 100--200 ns per hop---a "latency tax" that is physically unavoidable. In a three-tier fat-tree, FEC alone can contribute over 1 $\mu$s to the round-trip time, making it a significant component of the $\alpha$ latency term.

The sequence is the cluster-geometry trade-off in miniature: copper is cheap and low latency but keeps accelerators close together, optical interconnects\index{Optical Interconnect} extend a row or pod at higher power and latency, and pluggable optics reaches the core only when the topology needs distance more than it needs minimum per-hop cost.
...
# │ How: 20,000 links and $500/link are scenario assumptions (optical-link
# │      unit cost is a registry gap, noted 2026-06 audit); power per link is
# │      Systems.SwitchFabric.OpticsPluggable400G.
# │
# │ Imports: mlsysim registries (Systems.SwitchFabric) + fmt helpers
# │ Exports: SpineOpticsCost.n_links_str, .link_power_w_str,
# │          .total_cost_musd_str, .total_power_kw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_power, fmt_usd, check

class SpineOpticsCost:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    n_links = 20_000  # scenario assumption: spine-layer links for a 10K-GPU fat-tree
    ...
    total_cost_musd_str = fmt_usd(total_cost.to(USD).magnitude / 1e6, precision=0, commas=False, suffix=" million")
...
In an ML fleet, distance is money. A 10,000-GPU cluster requires ~`{python} SpineOpticsCost.n_links_str` optical links at the spine layer alone. At \$500 each with `{python} SpineOpticsCost.link_power_w_str` per pluggable link, that represents `{python} SpineOpticsCost.total_cost_musd_str` in cabling and `{python} SpineOpticsCost.total_power_kw_str` of continuous power for transceivers alone. The cost dictates **cluster geometry**\index{Cluster!geometry}: architects pack accelerators as densely as possible (70–100 kW per rack) to maximize cheap copper and minimize expensive optics.
...
# │
# │ Imports: mlsysim registries (Models, Hardware, Systems) + fmt helpers
# │ Exports: SerDesPowerBudget.grad_gb_str, .n_links_str, .port_power_w_str,
# │          .interconnect_kw_str, .power_frac_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
***Remote Direct Memory Access (RDMA)***\index{RDMA!definition}\index{RDMA} is a networking technology used in ML training fabrics that allows one machine to read or write the memory of another machine directly, bypassing the operating system kernel and CPU of both endpoints by offloading transport processing to the network interface card.
...
2.  **Observation**: Unlike traditional TCP/IP, where the CPU processes every packet through the kernel network stack (consuming tens of CPU cores to saturate a 400 Gb/s link), RDMA offloads the entire transport to dedicated NIC hardware, freeing the CPU to orchestrate computation rather than move data.
3.  **Pitfall**: A frequent misconception is that RDMA works reliably on any Ethernet network. RDMA lacks TCP's retransmission logic; a single dropped packet can stall an entire 1,024-GPU AllReduce for 100–500 ms as the Go-Back-N recovery retransmits from the loss point. RDMA requires a lossless fabric—InfiniBand or Ethernet with Priority Flow Control—to operate correctly at scale.
...
# │ Exports: IbNdrTransportRecap.ib_ndr_gbit_per_s_str
...
GPU clusters commonly choose between two RDMA transport stacks, and the decision is a reliability-versus-operations trade-off: InfiniBand provides losslessness as a native fabric property, while RoCE preserves Ethernet economics and ecosystem breadth at the cost of more delicate congestion control.
...
# │
# │ Imports: mlsysim.core.units (Systems.Fabrics.InfiniBand_NDR.bandwidth, Gbps, MILLION, TRILLION)
...
# │          NetworkAlphaBeta.ib_beta_gb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
class NetworkAlphaBeta:
    ...
    large_speedup_str = fmt_multiple(round(large_speedup), precision=0, commas=False)
...
2. **Large message** (`{python} NetworkAlphaBeta.n_large_mb_str`):
   ...
   - **Result**: InfiniBand is `{python} NetworkAlphaBeta.large_speedup_str` faster purely due to higher $\beta$.
...
from mlsysim.fmt import fmt, fmt_count, fmt_int, fmt_math, fmt_params, fmt_percent, fmt_qty, fmt_time, sci_latex
...
# │
# │ Imports: mlsysim.core.units
# │ Exports: AllReduceBottleneck.alpha_math, AllReduceBottleneck.beta_math,
# │          AllReduceBottleneck.t_comp_display_math, AllReduceBottleneck.t_ring_small_math,
...
class AllReduceBottleneck:
    ...
    t_comm_small_display = f"{t_comm_small.to(ms).magnitude:.0f}"
    t_comm_70b_display = f"{t_comm_70b.to(ms).magnitude:.0f}"
    comm_frac_small_pct = f"{comm_frac_small * 100:.1f}"
    ...
    t_ring_small_math = fmt_math(f"T_{{\\text{{ring}}}} \\approx {t_comm_small_display} \\text{{ ms}}")
    comm_frac_display_math = MarkdownStr(f"$$ \\text{{Comm. fraction}} = {comm_frac_small_pct} \\% $$")
...
# │
# │ Imports: mlsysim registries (Systems) + physics + fmt helpers
# │ Exports: BisectionDefinitionExample.gpus_str, .link_gbit_per_s_str,
# │          .bisect_1_1_math, .bisect_4_1_tb_per_s_str, .slowdown_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
2.  **Observation**: Unlike oversubscribed fabrics common in web data centers, where upper-tier links are shared among many lower-tier nodes, a non-blocking fabric provides dedicated path capacity for every possible pairing of senders and receivers simultaneously.
3.  **Pitfall**: A frequent misconception is that non-blocking means zero congestion. Endpoint congestion (incast) can still occur if multiple senders simultaneously target the same receiver port, regardless of how much internal fabric capacity is available.

:::

@Fig-network-topologies turns these topology definitions into a bandwidth and cost comparison. Fat-trees build on Clos-style non-blocking network ideas to provide full $\text{BW}_{\text{bisect}}$ [@clos1953], rail-optimized networks trade some global flexibility for lower cost, and dragonflies reduce cabling while accepting workload-placement constraints [@kim2008]. This trade-off between guaranteed bandwidth and economic scalability drives every topology decision in large-scale ML clusters.
...
2.  **Observation**: Unlike a standard tree (where bandwidth at the root is a single bottleneck shared by all leaves), a fat-tree replaces each root with multiple spine switches whose combined uplink capacity matches the total edge bandwidth, eliminating the bottleneck.
3.  **Pitfall**: A frequent misconception is that fat-trees guarantee zero network cost. They require $\mathcal{O}(N \log N)$ switches and dense cabling: a non-blocking three-tier fat-tree at the 4,000-GPU scale needs hundreds of switches and tens of thousands of optical cables, costing \$20–100 million in switching hardware alone.
...
The fat-tree[^fn-fat-tree-clos] is a common default for ML clusters because a non-blocking design can provide full $\text{BW}_{\text{bisect}}$, a nonnegotiable requirement for the AllReduce collective, which demands simultaneous, all-to-all communication. @Fig-fat-tree-detail shows the resulting three-tier structure. The network is constructed in hierarchical tiers: **Leaf** switches (ToR) connect directly to servers, **Spine** switches interconnect all leaves within a locality domain known as a pod\index{Pod}, and **Core** switches bind multiple pods together.
...
# │
# │ Imports: mlsysim.core.units, mlsysim.book (check)
# │ Exports: bbs_n_gpus_str, bbs_gpus_per_side_value_str, bbs_gradient_gb_str,
# │          bbs_total_data_gb_str, bbs_bw_1_1_gb_per_s_str, bbs_bw_4_1_gb_per_s_str,
# │          bbs_slowdown_mult_str, bbs_time_1_1_ms_str, bbs_time_4_1_ms_str
# └─────────────────────────────────────────────────────────────────────────────
...
- [ ] **Radix and scale**: In a **Radix-64** two-tier fat-tree, what is the maximum number of GPUs you can connect without core switches?
- [ ] **Non-blocking ratio**: Why does a Non-blocking fabric require a 1:1 subscription ratio at every tier of the network?
- [ ] **Tier vs. latency**: If you move from a two-tier to a three-tier fat-tree, how does the **$\alpha$ (startup latency)** of a message change?
- [ ] **Aggregate vs. bisection**: Explain the difference between aggregate bandwidth and $\text{BW}_{\text{bisect}}$. Which one matters for AllReduce?
...
# │ Exports: rol_hop_lat_us_str, rol_speedup_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
Archetype A (GPT-4) is a primary driver for rail-optimized fabrics. Because it uses **3D Parallelism**\index{3D Parallelism}, it generates two distinct traffic patterns: (1) massive, bandwidth-hungry gradient averaging for data parallelism, and (2) high-frequency, latency-sensitive activation exchanges for tensor parallelism. The rail-optimized design ensures that data-parallel traffic traverses only a single switch hop between nodes, minimizing the latency that would otherwise stall the synchronous training loop.
...
- [ ] **Rail beneficiary**: Which dimension of 3D Parallelism (TP, PP, or DP) is the primary beneficiary of a **Rail-Optimized** design?
- [ ] **Switch hops**: Why does a rail-optimized network reduce the number of switch hops for gradient exchanges compared to a standard fat-tree?
- [ ] **Traffic isolation**: What is the "Traffic Isolation" benefit: why do we want all GPU 0s on one rail and all GPU 1s on another?
- [ ] **Core switch need**: True or False: A rail-optimized network eliminates the need for high-bandwidth core switches connecting different rails.
...
2.  **Observation**: Unlike asynchronous parallelism, which allows workers to proceed with stale weights from previous steps, BSP enforces a global state update at every barrier, providing mathematical equivalence to single-device training and predictable convergence behavior.
3.  **Pitfall**: A frequent misconception is that BSP is simply inefficient compared to async models. Asynchronous training often requires more total steps to converge because stale gradient updates introduce noise—in practice, BSP with careful straggler mitigation typically reaches the same loss in fewer wall-clock hours than async alternatives.
...
The BSP weakest-link property means that even a single dropped packet can trigger a retransmission timeout that stalls the global barrier for milliseconds, an eternity in a training loop where each compute step finishes in tens of milliseconds. RoCEv2 fabrics solve this with priority flow control (PFC)\index{Priority Flow Control}, operating in lossless mode: rather than allowing switch buffers to overflow and drop packets, the fabric uses a link-layer backpressure mechanism to pause upstream senders before any buffer saturates.
...
2.  **Observation**: Unlike standard IEEE 802.3 flow control, which pauses all traffic classes on a link, PFC operates per priority class, allowing latency-sensitive control messages to continue flowing while only pausing the congested gradient data queue.
3.  **Pitfall**: A frequent misconception is that PFC solves congestion. It transforms packet loss into congestion spreading: a backpressure cascade can freeze the entire fabric within 200 ms of a single faulty transceiver, because no mechanism limits how far PAUSE frames propagate across the network.
...
**Problem**: A `{python} PfcStormRisk.pfc_n_gpus_str`-GPU RoCE cluster is in operation. If the probability of a single transceiver degrading and triggering PFC pauses is just `{python} PfcStormRisk.pfc_p_degrade_str` per day, what is the chance of a cluster-wide "PFC Storm" occurring in a given day?
...
[^fn-dcqcn-congestion]: **DCQCN (Data Center Quantized Congestion Notification)**\index{DCQCN!congestion control}: Introduced at SIGCOMM 2015 by Microsoft and Mellanox, DCQCN was the first congestion control protocol designed specifically for large-scale RDMA fabrics. Its binary ECN feedback limits stable link utilization to 85--90 percent, meaning a 400 Gb/s link effectively delivers only 340--360 Gb/s of useful gradient bandwidth under contention.

[^fn-hpcc-telemetry]: **HPCC (High Precision Congestion Control)**\index{HPCC!congestion control}: Developed by Alibaba and presented at SIGCOMM 2019, HPCC replaced binary ECN with per-packet in-network telemetry, reducing flow completion times by up to 95 percent compared to DCQCN in incast-heavy workloads. The trade-off is hardware dependency: HPCC requires programmable switch ASICs capable of appending telemetry metadata at line rate, limiting deployment to clusters with newer switching silicon [@li2019].
...
[^fn-packet-spraying]: **Packet Spraying**\index{Packet Spraying!load balancing}: Unlike standard Ethernet (which hashes flows to single paths to avoid reordering), packet spraying sends individual packets of a single flow across all available paths. This maximizes $\text{BW}_{\text{bisect}}$ for AllReduce elephant flows. InfiniBand performs this natively in hardware; Ultra Ethernet Consortium (UEC) proposals add analogous mechanisms to Ethernet to reduce the ECMP collision problem.
...
# │
# │ Imports: mlsysim registries (Systems) + fmt helpers
# │ Exports: IncastBurst.n_senders_str, .sender_bw_gb_per_s_str, .offered_tb_per_s_str,
# │          .port_gbit_per_s_str, .buffer_mb_str, .buffer_hold_us_str,
# │          .oversub_mult_str, .net_fill_tb_per_s_str, .egress_gb_per_s_str,
# │          .overflow_us_str
# └─────────────────────────────────────────────────────────────────────────────
...
1. **Layer-staggering**\index{Incast!layer-staggering mitigation}: Triggering each gradient bucket's AllReduce as soon as backpropagation produces it. Final-layer gradients reduce while earlier layers are still computing, so communication is staggered instead of released as one burst.
...
3. **Quality of Service (QoS) classification**\index{Quality of Service!priority traffic}: Tagging gradient traffic with the highest service class so background storage or management traffic does not delay the synchronization path.
...
The final level of the stack is cluster design\index{Cluster Design}, where wires, transport, topology, and congestion control combine into a coherent system. The goal of cluster design is to provide an end-to-end Gradient Bus that makes thousands of distributed GPUs feel like a single machine. At this level, the abstraction layers collapse into concrete engineering decisions: which cables to buy, how to wire the racks, which protocol to deploy, and how to validate that the resulting fabric actually delivers the bandwidth the training job expects. Two representative large-scale architectures, one built on InfiniBand and one on Ethernet, illuminate the trade-offs that define ML infrastructure [@meta2024genaiInfrastructure].
...
# │ Exports: NetworkFabricsClusterRecap.nvlink_to_ib_ratio_mult_str,
# │          nvlink_h100_gb_per_s_str, ib_ndr_gbit_per_s_str
...
The NVIDIA DGX SuperPOD architecture connects DGX H100 nodes using a two-tier InfiniBand network, serving as a concrete implementation of the Gradient Bus concept. Each node acts as a dense compute island, with eight H100 GPUs connected via NVSwitch to provide `{python} NetworkFabricsClusterRecap.nvlink_h100_gb_per_s_str` of internal bandwidth. Externally, each GPU pairs with a ConnectX-7 NIC delivering `{python} NetworkFabricsClusterRecap.ib_ndr_gbit_per_s_str` of injection bandwidth. Across a standard scalable unit of `{python} IncastBurst.su_nodes_str` (`{python} IncastBurst.n_senders_gpus_str`), this yields an aggregate injection bandwidth of `{python} IncastBurst.offered_tb_per_s_str` (`{python} IncastBurst.n_senders_str` $\times$ `{python} IncastBurst.sender_bw_gb_per_s_str`), ensuring the fabric can ingest gradients as fast as the accelerators produce them.
...
# │ Exports: IbNdrVirtualizationRecap.ib_ndr_gbit_per_s_str
...
# │
# │ Imports: mlsysim registries (Systems, Infrastructure) + fmt helpers
# │ Exports: SilentWaste.n_gpus_str, .bw_loss_pct_str, .slowdown_lo_pct_str,
# │          .slowdown_hi_pct_str, .run_days_str, .waste_gpu_hours_range_str,
# │          .waste_cost_range_str, .gpu_hour_rate_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_count, fmt_percent, fmt_usd, fmt_range, fmt, check
...
class SilentWaste:
    ...
    slowdown_lo_pct_str = fmt_percent(slowdown_lo, precision=0, commas=False, style='number')
    slowdown_hi_pct_str = fmt_percent(slowdown_hi, precision=0, commas=False, style='prose')
...
# │
# │ Imports: mlsysim.core.units (Systems.Fabrics.InfiniBand_HDR.bandwidth, Gbps)
...
# │ Exports: IbNdrMonitoringRecap.ib_ndr_gbit_per_s_str, ib_ndr_gb_per_s_str
...
The first pressure point is reliability. The Ultra Ethernet Consortium (UEC)\index{Ultra Ethernet} targets that constraint by trying to make Ethernet behave more like an HPC fabric without inheriting RoCEv2's PFC fragility. Its proposed design combines packet spraying across multiple paths, ordered and unordered reliable delivery services, congestion-control changes, and local loss-repair mechanisms intended to reduce reliance on Priority Flow Control for AI/HPC traffic. The goal is to preserve Ethernet's ubiquity and multi-vendor economics while moving its failure behavior closer to InfiniBand's lossless fabric model. Specific mechanisms such as packet trimming or link-layer retry are implementation details of that broader direction, so they should be evaluated against the same durable criteria: whether the fabric can repair congestion locally before it becomes a synchronous-training straggler.
...
# │
# │ Imports: mlsysim.core.units, mlsysim.fmt (fmt, check)
# │ Exports: NextGenLinkDensity.xdr_port_gbit_per_s_str,
# │          NextGenLinkDensity.eth_800g_port_gbit_per_s_str,
# │          NextGenLinkDensity.eth_1_6t_port_tbit_per_s_str,
# │          NextGenLinkDensity.switch_capacity_tbit_per_s_str,
...
class NextGenLinkDensity:
    ...
    eth_800g_port_gbit_per_s_str = fmt_bandwidth(ETHERNET_800G_BW, unit=Gbps, precision=0, commas=False)
...
Higher port rates attack hop count as well as raw bandwidth. XDR InfiniBand pushes `{python} NextGenLinkDensity.xdr_port_gbit_per_s_str` per port and 800GbE pushes `{python} NextGenLinkDensity.eth_800g_port_gbit_per_s_str` per port, while Ethernet switch silicon with `{python} NextGenLinkDensity.eth_1_6t_port_tbit_per_s_str` ports pushes aggregate capacity to `{python} NextGenLinkDensity.switch_capacity_tbit_per_s_str`. A single `{python} NextGenLinkDensity.eth_1_6t_port_tbit_per_s_str` port delivers the bandwidth of `{python} NextGenLinkDensity.port_ratio_str` 400G ports. The increased density allows architects to flatten the topology: a cluster that previously required three tiers of switches may be served by two when radix, port rate, and placement constraints line up, halving the transceiver count and reducing tail latency by removing an entire hop of switching and FEC overhead.
...
That shift does not repeal the physics this chapter has developed. A composed virtual node still pays the same $\alpha$-$\beta$ costs on every cross-pool access, the same power-per-bit limits on the links that carry it, and the same observability requirements that keep an oversubscribed path from masquerading as slow accelerators. The durable lesson is the displacement of overhead: disaggregation relocates the synchronization backbone rather than eliminating it: whether a composable fabric works is decided by the same fabric properties that decide whether a fixed-node fabric works.
...
# │ Exports: IbBandwidthFallaciesRecap.ib_hdr_gbit_per_s_str,
# │          IbBandwidthFallaciesRecap.ib_ndr_gbit_per_s_str
...
# │ Exports: InfiniBandSpeedupScenario.hdr_bw_gb_per_s_str,
# │          InfiniBandSpeedupScenario.ndr_bw_gb_per_s_str,
