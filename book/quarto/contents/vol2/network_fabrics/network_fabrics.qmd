...
That shift does not repeal the physics this chapter has developed. A composed virtual node still pays the same $\alpha$-$\beta$ costs on every cross-pool access, the same power-per-bit limits on the links that carry it, and the same observability requirements that keep an oversubscribed path from masquerading as slow accelerators. The durable lesson is that disaggregation relocates the synchronization backbone rather than eliminating it: whether a composable fabric works is decided by the same fabric properties that decide whether a fixed-node fabric works.
