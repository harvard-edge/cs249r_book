...
class NetworkAlphaBeta:
    ...
    eth_beta_gbs = eth_beta.m_as(GB / second)
...
class AllReduceBottleneck:
    ...
    peak_tflops = peak_flops.m_as(TFLOPs/second)
    gpu_util = 0.50
...
class BisectionScaling:
    ...
    beta_gbs = beta.m_as(GB / second)
    gradient = 100 * GB
    gradient_gb = gradient.m_as(GB)
    # ┌── 2. EXECUTE ───────────────────────────────────────
...
class NetworkFabricsClusterRecap:
    ...
    ib_ndr_gbs = ib_ndr.m_as(GB/second)
...
class OpticalDividend:
    ...
    total_pluggable_kw = total_pluggable.m_as(kilowatt)
    total_cpo_kw = total_cpo.m_as(kilowatt)
...
class IbBandwidthFallaciesRecap:
    hdr_gbps = Systems.Fabrics.InfiniBand_HDR.bandwidth.m_as(Gbps)
    ndr_gbps = Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as(Gbps)
    check(round(hdr_gbps) == 200, "HDR bandwidth should be 200 Gb/s")
