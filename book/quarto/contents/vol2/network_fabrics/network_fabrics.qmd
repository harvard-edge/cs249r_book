...
#| fig-cap: "**The Bandwidth Hierarchy**. Bandwidth at four levels of the communication hierarchy across four GPU generations, on a logarithmic scale. While HBM and NVLink bandwidth have grown roughly 9$\\times$ and 6$\\times$ respectively from V100 to B200, InfiniBand has grown only 4$\\times$. The annotations show the NVLink-to-InfiniBand ratio for each generation, quantifying the cliff that distributed training must cross at every synchronization point."
#| fig-alt: "Grouped bar chart with log-scale y-axis showing bandwidth in GB per second at four hierarchy levels across four GPU generations from V100 to B200."
#| echo: false
import numpy as np
...
***PAM4 Signaling***\index{PAM4 Signaling!definition} is a signaling scheme that uses Pulse Amplitude Modulation with four voltage levels (00, 01, 10, 11) to encode two bits per symbol. While this doubles the data rate for a given symbol rate compared to binary NRZ, it reduces the signal-to-noise ratio (SNR) by approximately 9.5 dB, a factor of nearly 9 $\times$ in power terms.
:::
...
***Remote Direct Memory Access (RDMA)***\index{Remote Direct Memory Access!definition} is a technology that allows a network adapter to transfer data directly between the memory of two computers without involving the operating system kernel or CPU. It provides **Kernel Bypass** (eliminating context switches) and **Zero-Copy** (eliminating memory-to-memory copies) capabilities.
:::
...
::: {#fig-ib-roce-stack fig-env="figure" fig-pos="htb" fig-cap="**High-Performance Networking Stacks**. Comparison of InfiniBand and RoCE protocol stacks. Both expose the same Verbs API, but RoCE relies on Priority Flow Control (PFC) in the Ethernet layer to approximate InfiniBand's native lossless guarantees."}
```{.tikz}
...
***$\alpha$-$\beta$ Model***\index{alpha-beta Model!definition} (or Hockney model) is a predictive formula for the time $T$ to send a message of size $n$ bytes:
$$ T(n) = \alpha + \frac{n}{\beta} $$ {#eq-alpha-beta}
where **$\alpha$** is the latency (fixed overhead per message) and **$\beta$** is the bandwidth (peak transfer rate).
:::
...
```{python}
#| label: fig-alpha-beta-crossover
#| echo: false
#| fig-cap: "**The Alpha-Beta Crossover**. Transfer time as a function of message size for InfiniBand NDR and Ethernet RoCE. Small messages are latency-dominated (flat region), while large messages are bandwidth-dominated (linear region). The crossover point marks where investing in bandwidth begins to pay off more than reducing latency."
#| fig-alt: "Log-log plot showing transfer time versus message size for InfiniBand and Ethernet, with latency-dominated and bandwidth-dominated regions labeled."
...
***Bisection Bandwidth***\index{Bisection Bandwidth!definition} is the minimum total bandwidth across any cut that divides the network into two equal halves. It represents the worst-case aggregate throughput for global communication patterns like AllReduce. A network with *full bisection bandwidth* can sustain any permutation of source-destination pairs at line rate simultaneously.
...
***Non-blocking Fabric***\index{Non-blocking Fabric!definition} is a network topology in which any permutation of input-output port pairs can communicate simultaneously at full line rate without contention. In ML clusters, a non-blocking fabric ensures that AllReduce traffic from any subset of accelerators does not compete for shared links, eliminating topology-induced stragglers.
...
::: {#fig-topology-bisection fig-env="figure" fig-pos="htb" fig-cap="Topology Bisection Bandwidth Comparison. Fat-Tree architectures provide full bisection bandwidth, while other topologies trade this off for cost or latency."}
```{=latex}
\begin{tikzpicture}
    \usefont{T1}{phv}{m}{n}
...
***Fat-Tree***\index{Fat-Tree!definition} (or Clos) network is a multi-tier topology where aggregate bandwidth increases toward the root. It provides multiple equal-cost paths between any two nodes, ensuring the fabric can sustain full injection from all endpoints simultaneously without blocking.
:::
...
***Bulk Synchronous Parallel (BSP)***\index{Bulk Synchronous Parallel!definition} is a parallel computing model in which execution proceeds in discrete *supersteps*, each consisting of three phases: (1) local computation, (2) global communication (e.g., gradient exchange), and (3) a barrier synchronization that ensures all nodes complete before the next superstep begins. BSP is the implicit execution model of synchronous distributed training: no worker advances to the next iteration until every worker has contributed its gradients and received the updated parameters.
...
***Priority Flow Control (PFC)***\index{Priority Flow Control!definition} (IEEE 802.1Qbb) is a link-level mechanism that prevents buffer overflow by sending PAUSE frames to the upstream sender. While it ensures losslessness, it can cause **congestion spreading** (where a pause cascades across the fabric) and **PFC storms** (deadlocks) that freeze all communication.
:::
...
::: {#fig-congestion-cascade fig-env="figure" fig-pos="htb" fig-cap="PFC Pause Frame Propagation. Congestion at a tail switch (S4) triggers a backpressure cascade, freezing the entire communication path upstream."}
```{=latex}
\begin{tikzpicture}
    \usefont{T1}{phv}{m}{n}
    \definecolor{BlueL}{HTML}{006395}
...
[^fn-dcqcn]: **DCQCN (Data Center Quantized Congestion Notification)**: A congestion control protocol developed by Microsoft that combines ECN marking with rate-based flow control, designed specifically for RDMA over Converged Ethernet (RoCE) fabrics [@zhu2015congestion].

[^fn-hpcc]: **HPCC (High Precision Congestion Control)**: A protocol developed by Alibaba that uses in-network telemetry (INT) to achieve near-zero queue buildup, enabling higher link utilization than ECN-based schemes [@li2019hpcc].
...
***Incast***\index{Incast!definition} is a many-to-one traffic pattern in which multiple senders simultaneously transmit to a single receiver, overwhelming the receiver's switch port buffer and causing packet drops or flow-control backpressure. In ML training, incast occurs during the reduce phase of AllReduce when multiple workers send partial gradients to the same aggregation node.
...
::: {#fig-hierarchical-staircase fig-env="figure" fig-pos="htb" fig-cap="The Hierarchical Bandwidth Staircase. Communication bandwidth drops by orders of magnitude as the distance from the chip increases, dictating parallelism strategies."}
```{=latex}
\begin{tikzpicture}
    \usefont{T1}{phv}{m}{n}
    \definecolor{BlueL}{HTML}{006395}
