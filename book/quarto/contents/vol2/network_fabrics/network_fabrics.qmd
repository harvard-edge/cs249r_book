...
Assume each GPU processes a synthetic microbatch requiring $5 \times 10^{13}$ FLOPs, chosen to produce an approximately 100 ms compute phase for this bottleneck example. At `{python} AllReduceBottleneck.h100_fp16_tflop_per_s_str` with `{python} AllReduceBottleneck.gpu_util_pct_str` utilization:
