...
class FECLatency:
    ...
    ns_low = FEC_LATENCY_LOW.to(NS).magnitude
    ns_high = FEC_LATENCY_HIGH.to(NS).magnitude
    three_hops = 3
    round_trip_hops = 6
...
class AllReduceBottleneck:
    ...
    # Math strings
    alpha_math           = fmt_math(f"\\alpha = {FABRIC_ALPHA_NDR.to(US).magnitude:.1f} \\;\\mu\\text{{s}}")
...
class RailLatency:
    ...
    t_hop_us = FABRIC_HOP_LATENCY.to(US).magnitude # switch + wire latency
    n_nodes = 128
...
class OpticalDividend:
    ...
    cluster_pluggable_mw_str = fmt_qty(cluster_pluggable, ureg.megawatt, precision=2, commas=False, unit_label="MW")
    cluster_savings_mw_str = fmt_qty(cluster_savings, ureg.megawatt, precision=2, commas=False, unit_label="MW")
...
class NextGenLinkDensity:
    ...
    eth_1_6t_port_tbps_str = fmt_qty(ETHERNET_1P6T_BW, ureg.terabit / second, precision=1, commas=False, unit_label="Tb/s")
    switch_capacity_tbps_str = fmt_qty(SWITCH_ASIC_102T4_BW, ureg.terabit / second, precision=1, commas=False, unit_label="Tb/s")
...
class BandwidthFallacy:
    ...
    ib_ndr = Systems.Fabrics.InfiniBand_NDR
    FABRIC_ALPHA_NDR = Systems.SwitchFabric.AlphaNdr
    alpha_us = FABRIC_ALPHA_NDR.to(US).magnitude
    msg_size_kb = 10

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # T(n) = alpha + n/beta, computed via pint to keep units explicit.
    msg = msg_size_kb * ureg.KB
    t_hdr = alpha_us + (msg / ib_hdr.bandwidth).to(US).magnitude
    t_ndr = alpha_us + (msg / ib_ndr.bandwidth).to(US).magnitude
