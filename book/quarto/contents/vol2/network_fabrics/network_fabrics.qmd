...
The conventional node architecture bundles a fixed ratio of compute, memory, and networking in each server: 8 GPUs, 640 GB of HBM, 2 TB of DDR, and 8 InfiniBand HCAs in a DGX H100. This fixed bundling is suboptimal for workloads that need a different ratio. Training a 175B model needs more memory capacity per GPU than training a 7B model. Inference serving needs more network bandwidth per GPU (to handle request traffic) than training (which has predictable, batch-oriented communication). The fixed node architecture forces organizations to provision for the most demanding ratio across all workloads, wasting resources on workloads that need a different balance; disaggregated architecture\index{Architecture!disaggregated} tries to relax that fixed ratio.

CXL (Compute Express Link)\index{CXL} is one mechanism that makes this disaggregation plausible rather than just a scheduler abstraction. CXL 3.0 enables memory pooling\index{Memory!pooling}, allowing a processor in one chassis to access memory in another with load/store semantics rather than RDMA message passing. Memory pooling points toward architectures where the rigid distinction between intra-node NVLink domains and inter-node InfiniBand domains softens into a memory fabric, allowing the scheduler to compose virtual nodes with workload-specific resource ratios.
...
The systems value is resource-ratio matching rather than modularity for its own sake. Upgrading compute (replacing GPU modules with a newer generation) does not require replacing the memory or networking components. Scaling memory capacity (for larger models) does not require purchasing additional GPUs. Failure of a compute module does not take down the associated memory or networking resources.

Industry examples serve as partial steps toward the resource-ratio problem rather than as proof that full disaggregation has arrived. Research projects and specialized systems explore disaggregated designs for ML infrastructure. Intel's data-centric architecture strategy and AMD's MI300X (which combines GPU compute dies and CPU dies on the same package with a unified memory controller) represent partial steps toward disaggregation. Full disaggregation, where GPUs are separated from memory by a fabric rather than co-located on the same interposer, requires CXL-class bandwidth and latency to be competitive, which is why disaggregated architectures and CXL are deeply intertwined.

The deployment timeline is less important than the architectural dependency chain: disaggregated ML infrastructure depends on memory-fabric latency, silicon photonics for inter-pool communication, and software that can manage resource composition without hiding the resulting $\alpha$-$\beta$ costs. Infrastructure planners should track this direction because facilities built for fixed nodes may later host more composable systems. The durable lesson is that disaggregation shifts the synchronization backbone from a fixed node boundary to a composable fabric; the same $\alpha$-$\beta$ costs, power-per-bit limits, and observability requirements still decide whether it works.
