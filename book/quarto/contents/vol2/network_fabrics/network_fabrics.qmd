...
t_ib = np.array([
    calc_point_to_point_time(size * byte, alpha_ib_q, beta_ib_q).to(second).magnitude
    for size in n
])
t_eth = np.array([
    calc_point_to_point_time(size * byte, alpha_eth_q, beta_eth_q).to(second).magnitude
    for size in n
])
...
class BisectionWasteRecap:
    ...
    cluster_cost_m = 300
    comm_fraction = 0.30
    rel_throughput, throughput_loss = calc_oversubscription_effect(
        comm_fraction, oversub_ratio_b
    )
