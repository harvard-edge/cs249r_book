...
# │ How: .to(unit).magnitude on Pint constants; ratio via simple division.
# │
...
class NetworkFabricsContextScenario:
    ...
    ib_ndr_gbs_val = ib_ndr_raw.to(GB/second).magnitude
    nvlink_h100_gbs_val = nvlink_h100_raw.to(GB/second).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ib_ndr_gbps_str = fmt_qty(ib_ndr_raw, Gbps, precision=0, commas=False, unit_label="Gb/s")
    ...
    h100_tflops_str = fmt_qty(h100_flops_raw, TFLOP/second, precision=0, commas=False)
    h100_mem_bw_str = fmt_qty(h100_mem_bw_raw, TB/second, precision=2, commas=False)
```
...
class IbNdrWireRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ib_ndr_gbps_str = fmt_qty(Systems.Fabrics.InfiniBand_NDR.bandwidth, Gbps, precision=0, commas=False, unit_label="Gb/s")
```
...
class FECLatency:
    ...
    ns_low = FEC_LATENCY_LOW.to(nanosecond).magnitude
    ns_high = FEC_LATENCY_HIGH.to(nanosecond).magnitude
    three_hops = 3
    round_trip_hops = 6
    ...
    three_hop_low_str = fmt_time(three_hop_low, 'nanosecond', precision=0, commas=False)
    three_hop_high_str = fmt_time(three_hop_high, 'nanosecond', precision=0, commas=False)
    round_trip_low_str = fmt_time(round_trip_low, 'nanosecond', precision=0, commas=False)
    round_trip_high_str = fmt_time(round_trip_high, 'nanosecond', precision=0, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, fmt_math, fmt_time, fmt_qty
...
class NetworkAlphaBeta:
    ...
    alpha_ib = FABRIC_ALPHA_NDR.to(second).magnitude # 1.5 us (typical for IB NDR)
    beta_ib = Systems.Fabrics.InfiniBand_NDR.bandwidth.to(byte/second).magnitude # ~50 GB/s
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    n_cross_ib_kb_str = fmt_qty(n_cross_ib * byte, KB, precision=0, commas=False)
    t_small_ib_us_str = fmt_time(t_small_ib * MILLION, 'microsecond', precision=2, commas=False)
    small_bw_pct_str = fmt_percent(small_bw_term_pct/100, precision=1, commas=False, style='prose')
    t_large_ib_ms_str = fmt_time(t_large_ib * THOUSAND, 'millisecond', precision=1, commas=False)
    ib_alpha_us_str = fmt_time(alpha_ib * MILLION, 'microsecond', precision=1, commas=False)
    ib_beta_gbs_str = fmt_qty(beta_ib * byte / second, GB / second, precision=0, commas=False)
    n_small_kb_str = fmt_qty(n_small * byte, KB, precision=0, commas=False)
    n_large_mb_str = fmt_qty(n_large * byte, MB, precision=0, commas=False)
    n_demo_large_mb_str = fmt_qty(n_demo_large * byte, MB, precision=0, commas=False)
    n_large_grad_mb_str = fmt_qty(350 * MILLION * byte, MB, precision=0, commas=False)
    eth_alpha_us = 5.0
    eth_beta = 12.5 * (GB / second)
    eth_beta_gbs = eth_beta.to(GB / second).magnitude
    eth_alpha_us_str = fmt_time(eth_alpha_us, 'microsecond', precision=0, commas=False)
    eth_beta_gbs_str = fmt_qty(eth_beta, GB / second, precision=1, commas=False)
    doubled_beta_gbs_str = fmt_qty(2 * beta_ib * byte / second, GB / second, precision=0, commas=False)
```
...
# ┌── 2. ARRAYS ────────────────────────────────────────────────────────────────
alpha_ib = Systems.SwitchFabric.AlphaNdr.to(second).magnitude
beta_ib = Systems.Fabrics.InfiniBand_NDR.bandwidth.to(byte/second).magnitude
alpha_eth = Systems.SwitchFabric.AlphaRoce.to(second).magnitude
beta_eth = Systems.Fabrics.Ethernet_100G.bandwidth.to(byte/second).magnitude
...
# ┌── 3. RENDER ────────────────────────────────────────────────────────────────
beta_ib_gbs = (beta_ib * byte / second).to(GB / second).magnitude
beta_eth_gbs = (beta_eth * byte / second).to(GB / second).magnitude
...
#blue
ax.annotate(
    f'Crossover\n~{(n_cross_ib * byte).to(KB).magnitude:.0f} KB',
...
#red
ax.annotate(
    f'Crossover\n~{(n_cross_eth * byte).to(KB).magnitude:.0f} KB',
...
from mlsysim.fmt import fmt, fmt_count, fmt_int, fmt_math, fmt_percent, fmt_qty
# ┌─────────────────────────────────────────────────────────────────────────────
...
class AllReduceBottleneck:
    ...
    peak_tflops = peak_flops.to(TFLOPs/second).magnitude
    gpu_util = 0.50
    ...
    grad_1b = m_1b.parameters.to(param).magnitude * 4 * ureg.byte
    ...
    grad_70b = m_70b.parameters.to(param).magnitude * 4 * ureg.byte
    t_comm_70b = calc_ring_allreduce_time(grad_70b, num_gpus, ib_ndr.bandwidth, FABRIC_ALPHA_NDR)

    comm_frac_small = t_comm_1b.to(second).magnitude / (t_comp_s + t_comm_1b.to(second).magnitude)
    ...
    t_comm_1b_ms = f"{t_comm_1b.to(ms).magnitude:.0f}"
    t_comm_70b_ms = f"{t_comm_70b.to(ms).magnitude:.0f}"
    ...
    params_1b_billion_str = fmt_count(
        m_1b.parameters.to(param).magnitude,
        scale="B",
        precision=1,
        commas=False,
        scale_style="word",
    ...
    h100_tflops_str = fmt_qty(peak_flops, TFLOPs/second, precision=0, commas=False, unit_label="TFLOP/s")
    params_70b_billion_str = fmt_count(
        m_70b.parameters.to(param).magnitude,
        scale="B",
        precision=0,
        commas=False,
        scale_style="word",
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_qty, fmt, check, fmt_time
...
class BisectionScaling:
    ...
    beta_gbs = beta.to(GB / second).magnitude
    gradient = 100 * GB
    gradient_gb = gradient.to(GB).magnitude
    # ┌── 2. EXECUTE ───────────────────────────────────────
    ...
    bbs_time_1_1_ms_str = fmt_time(time_1_1 * THOUSAND, 'millisecond', precision=0, commas=True)
    bbs_time_4_1_ms_str = fmt_time(time_4_1 * THOUSAND, 'millisecond', precision=0, commas=True)
```
...
class RailLatency:
    ...
    t_hop_us = FABRIC_HOP_LATENCY.to(microsecond).magnitude # switch + wire latency
    n_nodes = 128
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    rol_hop_lat_us_str = fmt_time(t_hop_us, 'microsecond', precision=1, commas=False)
...
class NetworkFabricsClusterRecap:
    ...
    ib_ndr_gbs = ib_ndr.to(GB/second).magnitude
...
from mlsysim.fmt import fmt_int, fmt_qty, fmt, check
...
class OpticalDividend:
    ...
    n_ports = int(SWITCH_ASIC_51T2_BW.to(Gbps).magnitude / ETHERNET_400G_BW.to(Gbps).magnitude) # 51.2 Tb/s switch
    p_pluggable_w = OPTICS_POWER_PLUGGABLE_400G_W.to(watt).magnitude
    p_cpo_w = OPTICS_POWER_CPO_400G_W.to(watt).magnitude
    n_switches = 1000
    # ┌── 2. EXECUTE ───────────────────────────────────────
    total_pluggable = n_ports * OPTICS_POWER_PLUGGABLE_400G_W
    total_cpo = n_ports * OPTICS_POWER_CPO_400G_W
    total_pluggable_kw = total_pluggable.to(kW).magnitude
    total_cpo_kw = total_cpo.to(kW).magnitude
    ...
    h100_tdp_kw = Hardware.Cloud.H100.tdp.to(kW).magnitude
    ...
    cluster_pluggable_mw_str = fmt_qty(cluster_pluggable, MW, precision=2, commas=False, unit_label="MW")
    cluster_savings_mw_str = fmt_qty(cluster_savings, MW, precision=2, commas=False, unit_label="MW")
...
class NextGenLinkDensity:
    ...
    xdr_port_gbps_val = Systems.Fabrics.InfiniBand_XDR.bandwidth.to(Gbps).magnitude
    eth_800g_port_gbps_val = ETHERNET_800G_BW.to(Gbps).magnitude
    eth_1_6t_port_gbps_val = ETHERNET_1P6T_BW.to(Gbps).magnitude
    baseline_port_gbps_val = ETHERNET_400G_BW.to(Gbps).magnitude
    switch_capacity_tbps_val = SWITCH_ASIC_102T4_BW.to(Gbps).magnitude / THOUSAND
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    xdr_port_gbps_str = fmt_qty(Systems.Fabrics.InfiniBand_XDR.bandwidth, Gbps, precision=0, commas=False, unit_label="Gb/s")
    eth_800g_port_gbps_str = fmt_qty(ETHERNET_800G_BW, Gbps, precision=0, commas=False, unit_label="Gb/s")
    eth_1_6t_port_tbps_str = fmt_qty(ETHERNET_1P6T_BW, terabit / second, precision=1, commas=False, unit_label="Tb/s")
    switch_capacity_tbps_str = fmt_qty(SWITCH_ASIC_102T4_BW, terabit / second, precision=1, commas=False, unit_label="Tb/s")
...
# │ How: .to(Gbps).magnitude on the Systems.Fabrics.InfiniBand_HDR.bandwidth constant.
# │
...
class IbHdrReference:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    hdr_gbps = hdr_bw.to(Gbps).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ib_hdr_gbps_str = fmt_qty(hdr_bw, Gbps, precision=0, commas=False, unit_label="Gb/s")
```
...
class IbBandwidthFallaciesRecap:
    hdr_gbps = Systems.Fabrics.InfiniBand_HDR.bandwidth.to(Gbps).magnitude
    ndr_gbps = Systems.Fabrics.InfiniBand_NDR.bandwidth.to(Gbps).magnitude
    check(round(hdr_gbps) == 200, "HDR bandwidth should be 200 Gb/s")
    check(round(ndr_gbps) == 400, "NDR bandwidth should be 400 Gb/s")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ib_hdr_gbps_str = fmt_qty(Systems.Fabrics.InfiniBand_HDR.bandwidth, Gbps, precision=0, commas=False, unit_label="Gb/s")
    ib_ndr_gbps_str = fmt_qty(Systems.Fabrics.InfiniBand_NDR.bandwidth, Gbps, precision=0, commas=False, unit_label="Gb/s")
```
...
class BandwidthFallacy:
    ...
    ib_ndr = Systems.Fabrics.InfiniBand_NDR
    FABRIC_ALPHA_NDR = Systems.SwitchFabric.AlphaNdr
    alpha_us = FABRIC_ALPHA_NDR.to(microsecond).magnitude
    msg_size_kb = 10

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # T(n) = alpha + n/beta, computed via pint to keep units explicit.
    msg = msg_size_kb * ureg.KB
    t_hdr = alpha_us + (msg / ib_hdr.bandwidth).to(microsecond).magnitude
    t_ndr = alpha_us + (msg / ib_ndr.bandwidth).to(microsecond).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt, fmt_time, fmt_qty
    hdr_bw_str = fmt_qty(ib_hdr.bandwidth, Gbps, precision=0, commas=False, unit_label="Gb/s")
    ndr_bw_str = fmt_qty(ib_ndr.bandwidth, Gbps, precision=0, commas=False, unit_label="Gb/s")
    t_hdr_str = fmt_time(t_hdr, 'microsecond', precision=2, commas=False)
    t_ndr_str = fmt_time(t_ndr, 'microsecond', precision=2, commas=False)
...
class BisectionWasteRecap:
    ...
    total_bisec_bw_a = gpus_per_side * injection_bw.bandwidth.to(GB/second).magnitude
