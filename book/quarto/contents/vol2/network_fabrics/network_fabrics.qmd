...
1.  **Significance (quantitative)**: PAM4 enables 400 Gb/s and 800 Gb/s link speeds that sustain the $\text{BW}$ required for large-scale gradient synchronization. However, the reduced gap between voltage levels increases susceptibility to noise, requiring Forward Error Correction (FEC), typically Reed-Solomon RS(544,514), that adds about 100 ns of host-FEC processing latency per link in common high-speed Ethernet discussions [@anslow2016rsFec; @yin2022fecRequirements]. Across a three-tier fat-tree (three hops each way), FEC alone can contribute hundreds of nanoseconds of fixed latency to every packet, setting a hard floor on the fabric's fixed per-message startup latency, the quantity that @sec-network-fabrics-performance-model formalizes as the $\alpha$ term of the communication cost model.
...
- [ ] **Protocol fit**: Recommend a protocol and justify the choice under bandwidth-dominated large messages and latency-dominated small messages.
