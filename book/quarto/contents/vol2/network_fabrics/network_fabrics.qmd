...
A fat-tree built from switches with **radix**[^fn-switch-radix-density] $k$ supports $N = k^3/4$ hosts at three tiers, or $k^2/2$ at two tiers. In a two-tier network using radix-64 switches, a single pod can therefore support up to 2,048 GPUs, comfortably accommodating our 1,000-GPU reference cluster with only leaf and spine layers.
