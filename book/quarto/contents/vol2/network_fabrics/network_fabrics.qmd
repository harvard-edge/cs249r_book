...
**Problem**: A fabric designer is comparing InfiniBand NDR (`{python} NetworkAlphaBeta.ib_alpha_us_str` $\mu s$, `{python} NetworkAlphaBeta.ib_beta_gbs_str` GB/s) against a slower Ethernet baseline (5.0 $\mu s$, 12.5 GB/s). At what message size does the faster link's bandwidth advantage finally outweigh its higher cost?
...
**Problem**: A cluster designer is choosing between a "Non-blocking" (1:1) fat-tree and a "Cost-optimized" (4:1) spine for a `{python} BisectionScaling.bbs_n_gpus_str`-GPU cluster. How much slower will a `{python} BisectionScaling.bbs_gradient_gb_str` GB AllReduce be on the cheaper network?
...
**Problem**: A cluster has 1,024 accelerators across 128 nodes. Each node has `{python} NetworkFabricsContextScenario.ib_ndr_gbps` Gb/s (`{python} NetworkFabricsContextScenario.ib_ndr_gbs` GB/s) injection bandwidth. An AllReduce job requires full bisection bandwidth.
