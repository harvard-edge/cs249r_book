...
\noindent
![](images/png/cover_network_fabrics.png){fig-alt="Network interconnects and fabric topology linking compute nodes in a datacenter." width=100 percent}
...
- Analyze network topologies (**fat-tree**, **rail-optimized**\index{Rail-Optimized}, **dragonfly**) by computing **bisection bandwidth** and hop count for ML collective patterns
- Evaluate congestion control mechanisms (**PFC**\index{PFC}, **DCQCN**, **HPCC**) and their impact on tail latency during distributed training
...
To see *why* this ratio matters so viscerally, consider *what* happens during a single training step. @fig-bandwidth-cliff-gantt contrasts the timeline for an 8-GPU job communicating within a node vs. a 64-GPU job communicating across nodes.
...
: **ML Networking Inverts Traditional Datacenter Assumptions**\index{ML Networking Inverts Traditional Datacenter Assumptions}: *Where* web services require fairness for millions of independent flows, ML training requires a globally synchronized, lossless fabric optimized for a handful of massive collective operations. {#tbl-network-assumptions}
...
- **InfiniBand (IB)**[^fn-infiniband-origin]: A purpose-built HPC architecture designed for point-to-point, switched fabrics. It uses hardware-level **credit-based flow control**\index{Credit-Based Flow Control}: a sender must hold a credit (representing available buffer space at the receiver) before transmitting. IB is therefore inherently **lossless** at the link layer. It includes a **Subnet Manager (SM)** for global routing and **Virtual Lanes (VLs)** for traffic isolation.
...
**Problem**\index{Problem}: You are comparing InfiniBand NDR (`{python} NetworkAlphaBeta.ib_alpha_us_str` $\mu s$, `{python} NetworkAlphaBeta.ib_beta_gbs_str` GB/s) against a slower Ethernet baseline (5.0 $\mu s$, 12.5 GB/s). At what message size does the faster link's bandwidth advantage finally outweigh its higher cost?
...
1. **Standard Latency**\index{Standard Latency}: 2 hops $\times$ `{python} RailLatency.rol_hop_lat_us_str` $\mu s$ = **1.2 $\mu s$**.
...
class BisectionBottleneck:
    ...
    # In AllReduce, the total data crossing the bisection is num_nodes * gradient_size_gb
    total_data_crossing_bisection = num_nodes * gradient_size_gb
...
1. **Pluggable Architecture**\index{Pluggable Architecture}: 128 ports $\times$ `{python} int(OpticalDividend.p_pluggable_w)` W = **`{python} OpticalDividend.total_pluggable_kw_str` kW** for optics alone.
2. **CPO Architecture**\index{CPO Architecture}: 128 engines $\times$ `{python} int(OpticalDividend.p_cpo_w)` W = **1.28 kW**.
...
1. **Check GPU utilization**\index{Check GPU Utilization}: Rule out compute bottlenecks using `dcgmi` or `nvidia-smi`. If SM utilization is 100 percent, the network is not the bottleneck.
...
5. **Validate Physical Layer**\index{Validate Physical Layer}: Check symbol errors and CRC counts to identify failing transceivers or cables.
...
* **Topology Choice is Workload-Dependent**\index{Topology Choice Is Workload-Dependent}: Fat-trees provide flexibility; rail-optimized networks minimize latency for tensor parallelism; dragonflies reduce cabling costs for massive scale.
