...
| **AOC (Active Optical Cable)**\index{AOC} | 3 to 30 meters | Higher cost (~\$500) from permanently attached transceivers | Electrical/optical conversion plus Forward Error Correction (FEC)[^fn-fec-latency] adds hundreds of nanoseconds | Rack-to-rack or row-scale links |
...
|:---|:---|:---|:---|
| **InfiniBand (IB)**\index{InfiniBand}\index{IB|see{InfiniBand}}[^fn-infiniband-origin] | Purpose-built HPC switched fabric [@infiniband2000spec] with credit-based flow control, so losslessness is native at the link layer | Subnet Manager (SM)\index{Subnet Manager} and Virtual Lanes (VLs)\index{Virtual Lane} govern routing and traffic isolation | Dedicated training clusters where predictable tail latency outweighs Ethernet ecosystem flexibility |
