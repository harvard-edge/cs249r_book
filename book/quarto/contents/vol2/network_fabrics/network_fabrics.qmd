...
import numpy as np
from mlsys.constants import INFINIBAND_NDR_BW, ETH_100G_BW

alpha_ib = 1.5e-6
beta_ib = INFINIBAND_NDR_BW.m_as('byte/second')
alpha_eth = 5.0e-6
beta_eth = ETH_100G_BW.m_as('byte/second')
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm]
  ...
  \definecolor{SwitchColor}{RGB}{0,99,149}

  \tikzset{
    node/.style={draw=black!60, fill=NodeColor, thick, rounded corners=2pt, minimum width=5cm, minimum height=1.5cm},
    gpu/.style={draw=black!40, fill=GPUColor, thick, minimum width=0.8cm, minimum height=0.6cm, font=\tiny\bfseries},
    switch/.style={circle, draw=black!60, fill=SwitchColor, text=white, inner sep=2pt, font=\tiny\bfseries}
  }

  % Nodes
  \node[node] (N1) at (0, 0) {};
  \node[anchor=north west, font=\tiny\bfseries] at (N1.north west) {Node 1};
  \node[node] (N2) at (0, -3) {};
  \node[anchor=north west, font=\tiny\bfseries] at (N2.north west) {Node 2};

  % GPUs in Node 1
  \node[gpu] (G1_0) at (-1.8, 0) {GPU 0};
  \node[gpu] (G1_1) at (-0.6, 0) {GPU 1};
  \node[gpu] (G1_2) at (0.6, 0) {GPU 2};
  \node[gpu] (G1_3) at (1.8, 0) {GPU 3};

  % GPUs in Node 2
  \node[gpu] (G2_0) at (-1.8, -3) {GPU 0};
  \node[gpu] (G2_1) at (-0.6, -3) {GPU 1};
  \node[gpu] (G2_2) at (0.6, -3) {GPU 2};
  \node[gpu] (G2_3) at (1.8, -3) {GPU 3};

  % Rail Switches
  \node[switch] (S0) at (-1.8, 2) {R0};
  \node[switch] (S1) at (-0.6, 2) {R1};
  \node[switch] (S2) at (0.6, 2) {R2};
  \node[switch] (S3) at (1.8, 2) {R3};

  % Cabling
  \foreach \i in {0,1,2,3} {
    \draw[thick, BlueLine] (G1_\i) -- (S\i);
    \draw[thick, BlueLine] (G2_\i) -- (S\i);
  }

  % Annotation
  \draw[latex-latex, thick, OrangeLine] (G1_0) -- (G1_1) node[midway, below, font=\tiny, text=black] {NVLink};
...
\begin{tikzpicture}
    ...
    \definecolor{RedL}{HTML}{CB202D}
    \definecolor{GreenL}{HTML}{008F45}
    \foreach \x/\name/\t in {0/S1/t=3 ms, 3/S2/t=2 ms, 6/S3/t=1 ms, 9/S4/t=0} {
