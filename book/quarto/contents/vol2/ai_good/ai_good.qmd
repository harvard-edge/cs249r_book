...
To illustrate these optimization requirements, the optimization pipeline for the PlantVillage crop disease detection system demonstrates quantitative compression trade-offs. Starting with a ResNet-50 architecture [@he2016deep] at 100 MB achieving 91% accuracy, systematic optimization reduces model size by 31× while maintaining practical effectiveness:
...
Compression and quality adaptation trade data fidelity for bandwidth efficiency. Agricultural monitoring images captured at 2048×1536 resolution (3-5 MB per image) compress to 512×384 resolution with 70% JPEG quality (150-300 KB per image) for transmission, reducing bandwidth by 90% while preserving sufficient detail for crop disease detection. When bandwidth budget tightens further, systems reduce to 256×256 resolution (50-80 KB) or extract features locally and transmit only 5-10 KB feature vectors.
...
Communication costs dominate distributed processing performance, requiring careful optimization of inter-tier protocols. Parameter synchronization scales as O(model_size × participants), becoming prohibitive with large models and many edge nodes. Network bandwidth becomes the primary bottleneck for gradient aggregation rather than computational capacity. Sustainable distributed operation requires maintaining a 10:1 compute-to-communication ratio.
...
Expected information gain = I(y;x) × (1 - annotation_entropy)
...
For the medical example: $15 / 0.80 = 18.75 versus $33 / 0.90 = 36.67, making consensus 2× more cost-effective despite requiring more total annotation time.
...
**Annotation bandwidth optimization** reduces synchronization overhead by transmitting only essential data. For image-based tasks, systems send downsampled images (512×512 rather than full 2048×2048 resolution) for annotation, reducing bandwidth from 3-5 MB to 150-300 KB per image while preserving diagnostic information. When experts require full resolution, they flag cases for high-resolution synchronization during the next connectivity window.
