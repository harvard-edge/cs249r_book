...
We must move beyond viewing the datacenter as mere "housing" for servers. Following the **Warehouse-Scale Computer (WSC)** paradigm pioneered by Barroso, Dean, and Hölzle, we treat the entire facility—power, cooling, network, and compute—as a **single, unified machine**. In this machine:
*   **The Network** is the system bus (connecting distributed memory).
*   **The Cluster** is the multi-core processor (executing parallel instruction streams).
*   **The Power Plant** is the power supply (providing the thermodynamic potential for intelligence).

This shift is not semantic; it is architectural. Traditional datacenters optimize for *isolation* (multitenancy). ML datacenters optimize for *coupling* (gang scheduling). When training a 175B parameter model, 4,000 GPUs act as a single logical device. A failure in one cooling loop does not just "lose a node"; it stalls the entire "CPU."
...
Training a model is thermodynamically equivalent to reversing entropy (organizing weights). By the Second Law of Thermodynamics, this requires generating massive amounts of waste heat.
...
**The Network is the Bus:** In a warehouse-scale computer, the InfiniBand/Ethernet fabric is not "networking" in the traditional sense; it is the **System Bus** (like PCIe or NVLink) writ large.
*   **Packet Loss = Data Corruption.**
*   **Latency = Memory Access Time.**
*   **Bandwidth = Memory Throughput.**

When we optimize physical layout, we are literally **placing memory closer to compute** to minimize the speed-of-light latency of the "Gradient Bus."
:::
