...
\end{tikzpicture}
```
**Datacenter Power Distribution**: Hierarchical power delivery for ML infrastructure. Voltage steps down from utility-level 13.8 kV through 480V building distribution to 208V rack distribution and finally 12V DC at the component level. The 2N redundancy pattern at UPS and PDU tiers ensures training jobs survive single-component failures during multi-week runs. Each DGX H100 system draws 10.2 kW, requiring robust power infrastructure to support racks exceeding 40 kW total load.
:::
...
InfiniBand[^fn-infiniband] has emerged as the dominant interconnect for ML training clusters due to its RDMA (Remote Direct Memory Access)[^fn-rdma] capabilities and consistent low latency [@infiniband2000spec]. @fig-infiniband-stack illustrates the protocol stack architecture that enables direct memory-to-memory transfers without CPU involvement, reducing both latency and processor overhead through kernel bypass mechanisms.
...
\end{tikzpicture}
```
**Network Topologies for ML**: Three common interconnect architectures with distinct bandwidth-latency trade-offs. (A) Fat-Tree provides full bisection bandwidth (non-blocking) at the cost of 4-6 switch hops, ideal for AllReduce patterns in data parallelism. (B) Torus (TPU pods) connects only nearest neighbors with O(sqrt(N)) hop diameter, optimizing for structured tensor-parallel communication. (C) Rail-Optimized designs achieve 2-hop latency between corresponding GPUs across nodes, enabling efficient pipeline parallelism but sacrificing placement flexibility.
:::
...
[^fn-allreduce]: **AllReduce**: A collective communication operation that synchronizes gradients across all workers in distributed training. The Introduction established AllReduce as a key synchronization primitive. @sec-communication provides the detailed algorithmic analysis of ring, tree, and hybrid implementations that determine bandwidth efficiency.
...
Gradient compression techniques, including sparsification and quantization, can reduce bandwidth requirements by 10 to 100 times for some models, trading computation and potential accuracy impact for reduced network utilization. @sec-communication analyzes these compression algorithms alongside error compensation techniques that preserve convergence despite lossy gradient encoding.
...
GPU scheduling relies on device plugins that expose accelerators as extended resources. @lst-k8s-gpu-alloc-infra demonstrates how the NVIDIA device plugin registers GPUs with the kubelet through Kubernetes' extended resource mechanism, where a simple `nvidia.com/gpu: 4` specification requests four dedicated GPUs for a pod.
...
Locality-aware scheduling recognizes that communication topology matters for distributed training. A 64-GPU job performs better on 8 nodes of 8 GPUs each than 16 nodes of 4 GPUs, due to higher NVLink bandwidth within nodes. Recall from @sec-networking-ml that fat-tree topologies with 4-6 switch hops introduce latency that compounds across thousands of collective operations per training step. Advanced schedulers exploit this topology awareness by preferring allocations that share fewer switch hops, achieving 15 to 30 percent training throughput improvement from topology-aware placement.
...
Many organizations discover that their 512-GPU cluster trains models slower than a well-optimized 128-GPU deployment due to crossing this inflection point. @sec-distributed-training develops the hybrid parallelism strategies that address this limitation by combining tensor, pipeline, and data parallelism to minimize cross-node communication while maximizing compute utilization.
...
Software overhead adds latency that theoretical bandwidth calculations ignore. NCCL introduces 5-20 microseconds of software latency per collective operation. For small message sizes common in pipeline parallelism, this overhead dominates transfer time, reducing effective bandwidth by 50% or more. The crossover point where bandwidth dominates latency occurs at approximately 250 KB message size for typical InfiniBand configurations. Operators that fail to aggregate small tensors before collective operations waste substantial network capacity. @sec-communication formalizes these bandwidth-latency trade-offs and develops the message chunking and aggregation strategies that achieve near-optimal network utilization.
...
With the compute nodes defined, powered, cooled, and racked, we must now address the data they consume. A massive compute fleet is useless without a storage system capable of feeding it. @sec-storage examines the high-performance storage architectures required to keep these accelerators fed with data, developing the bandwidth calculations, consistency models, and checkpoint optimization strategies that translate infrastructure capacity into training throughput.
