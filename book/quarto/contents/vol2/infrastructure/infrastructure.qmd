...
- Compare network topologies using bisection bandwidth and latency trade-offs
...
- Design resource allocation policies for multi-tenant GPU clusters

:::

## Datacenter Architecture for ML Workloads {#sec-datacenter-architecture}

The scale transformation established in @sec-vol2-introduction, where frontier models require $10^{25}$ FLOPS across thousands of GPUs, creates infrastructure requirements far exceeding single machines or small clusters. Training a frontier model may require thousands of GPUs coordinated across multiple datacenters, each machine contributing to a unified computation that can span weeks or months. The transition from single-machine ML systems to distributed training fundamentally changes infrastructure requirements: single-node optimization focuses on GPU utilization, memory bandwidth, and efficient data loading, while production ML at scale demands purpose-built datacenters designed around the unique characteristics of ML workloads including massive power consumption, extreme heat density, and communication patterns that differ markedly from traditional cloud computing.

Managing such infrastructure demands expertise in datacenter design, high-bandwidth networking, and distributed systems orchestration. The infrastructure must satisfy competing requirements: computational efficiency alongside fault tolerance, coordination across thousands of machines without prohibitive communication overhead, and dynamic capacity provisioning that controls costs. These challenges have become central to machine learning advancement, as organizations cannot access frontier capabilities without mastering the physical and software systems that make large-scale computation possible.
