...
## The Warehouse-Scale Computer {#sec-datacenter-architecture}
...
In Volume I, we reframed Data Engineering as **Dataset Compilation**: the process of transforming raw observations into an optimized binary for learning. If the dataset is the code, then the datacenter is the computer that executes it.

We must move beyond viewing the datacenter as mere housing for servers. Following the **Warehouse-Scale Computer (WSC)** paradigm pioneered by Barroso, Dean, and Hölzle, we treat the entire facility (power, cooling, network, and compute) as a **single, unified machine**. In this machine, the network is the system bus connecting distributed memory, the cluster is the multi-core processor executing parallel instruction streams, and the power plant is the power supply providing the thermodynamic potential for intelligence.

This shift is not semantic; it is architectural. Traditional datacenters optimize for isolation (multitenancy), while ML datacenters optimize for coupling (gang scheduling). When training a 175B parameter model, 4,000 GPUs act as a single logical device. A failure in one cooling loop does not just lose a node; it stalls the entire CPU.

This chapter examines the physical foundation of this global machine. We transition from abstract algorithms to concrete engineering, analyzing how power density, thermal flux, and interconnect topology determine the execution speed of our dataset compiler.

### Physical Infrastructure Fundamentals

ML datacenters differ from traditional cloud facilities in three critical dimensions. First, power density per rack reaches 5 to 10 times higher than conventional levels. Second, cooling requirements demand liquid rather than air-based solutions (@fig-cooling-topology contrasts traditional air-cooled architectures with direct-to-chip liquid systems capable of managing 700W per GPU). Third, physical layout must optimize for high-bandwidth interconnects rather than flexible networking.

#### Power Delivery and Distribution

A single NVIDIA DGX H100 system consumes 10.2 kW at peak load. A rack containing four such systems requires over 40 kW, compared to 5 to 10 kW for traditional server racks. This power density[^fn-power-density] fundamentally changes power infrastructure design.
...
The H100 delivers approximately 3x the tensor TFLOPS of A100 at 1.75x the power. To derive the efficiency improvement, A100 achieves 312 TF / 400 W = 0.78 TF/W, while H100 achieves 990 TF / 700 W = 1.41 TF/W, yielding approximately 80% improvement in FLOPS/watt. Memory bandwidth increases proportionally, maintaining the compute-to-memory ratio critical for transformer models.
