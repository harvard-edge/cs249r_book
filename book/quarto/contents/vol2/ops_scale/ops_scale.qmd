...
::: {#psp-ops-scale-platform-superlinear-returns .callout-perspective title="Platform operations have superlinear returns"}
...
2.  Check whether the `{python} InfraPlanningExercise.power_mw_str` power budget is sufficient, assuming PUE `{python} InfraPlanningExercise.pue_str`, `{python} H100TdpExerciseRecap.tdp_w_str` per GPU, and `{python} InfraPlanningExercise.non_gpu_overhead_pct_str` overhead for non-GPU components.
