...
$$t_{\text{stage}} = \frac{n_{samples\_needed}}{r_{\text{requests}} \times p_{stage}}$$ {#eq-canary-duration}

where $t_{\text{stage}}$ is the duration required at a given percentage, $n_{samples\_needed}$ is the number of observations needed for statistical significance, $r_{\text{requests}}$ is the request rate, and $p_{stage}$ is the traffic percentage.
...
$$C_{\text{train}} = N_{GPU} \times T_{\text{hours}} \times R_{GPU/hr} \times PUE \times (1 + F_{\text{fail}})$$ {#eq-training-cost}
...
$$C_{\text{infer}} = \frac{Q_{\text{daily}} \times L_{avg}}{U_{GPU} \times B_{\text{eff}}} \times R_{GPU/hr} \times 24 \times 365$$ {#eq-inference-cost}
...
$$C_{\text{infer}} = Q_{\text{annual}} \times \frac{t_{\text{inference}}}{U_{GPU}} \times \frac{R_{GPU/hr}}{3600}$$
