...
Serving workloads bind on latency rather than batch flexibility. Demand fluctuates with time of day, events, and seasonality, but a live request cannot be preempted without user impact. Serving resource management therefore protects latency while avoiding overprovisioning: autoscaling adds horizontal capacity from request-rate, latency, or model-specific queue metrics; resource isolation prevents one model from consuming another model's budget; and cost optimization combines right-sized instances, reserved capacity for baseline demand, and spot instances only where overflow can tolerate interruption.
...
Tenants need isolation at every boundary where one team's workload, data, or cost can affect another team. Performance isolation uses resource limits, scheduling fairness, and network quality of service to prevent one workload from degrading another. Security isolation uses access controls, network segmentation, and encryption when teams work with different sensitivity levels. Cost isolation completes the boundary by making each team's usage attributable through metering and chargeback.
...
Serving costs scale with traffic, but ML serving cannot autoscale like a stateless web application. Large models may take seconds or minutes to load, so purely reactive scaling arrives after the latency spike; the platform needs predictive scale-up before anticipated demand. GPU memory also makes capacity discrete rather than fractional: a service often scales in jumps of 0, 1, 2, or 4 GPUs. Batching adds another coupling because waiting for a larger batch improves utilization but consumes latency budget. An autoscaling policy must therefore choose the least expensive capacity step that still protects the p99 latency SLO.

#### Right-sizing and instance selection {#sec-ml-operations-scale-rightsizing-instance-selection-fe66}

ML workloads have distinct compute profiles, so right-sizing starts from the binding constraint rather than the newest instance type. Memory-bound training with large embedding tables prioritizes GPU memory capacity and bandwidth. Compute-bound dense training maximizes sustained FLOP/s per dollar. Latency-sensitive serving minimizes cold start and single-request latency, while throughput-oriented serving maximizes requests per dollar through batching. Instance selection should therefore be benchmark-driven: compare cost per training step or cost per inference across instance types instead of assuming larger hardware is automatically more economical.
...
RAG chooses freshness and attribution over lower-cost inference. Updating a document index can change the system's answers without retraining the model, and retrieved passages provide a concrete source trail for generated claims. The cost moves into the serving path: each query may carry thousands of extra context tokens, increasing prefill attention work and growing the KV cache linearly with context length. Long contexts also introduce a quality risk because the model may underuse relevant evidence when the prompt contains too much retrieved material.
...
- **Leakage**\index{Leakage}: Using this value (10) to train a model predicting a click that happened at noon leaks future information. At noon, the user might have only had 4 clicks.
