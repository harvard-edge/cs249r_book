...
* **FinOps for ML**\index{FinOps!ML cost attribution}: GPU compute is the primary cost driver. Effective MLOps requires granular cost attribution (Cost-per-Inference) and opportunistic resource scheduling (Spot instances) to maintain economic viability.
...
::: {.callout-chapter-connection title="From operations to fleet orchestration"}

The operational machinery for managing ML systems at scale now includes platform economics, fleet monitoring, edge deployment, and FinOps governance. That machinery still needs a scheduler that turns shared capacity into completed work under placement, quota, and failure constraints. @Sec-fleet-orchestration addresses that control problem, showing how the fleet assigns work, protects isolation, and keeps capacity useful when many models compete for the same accelerators.
