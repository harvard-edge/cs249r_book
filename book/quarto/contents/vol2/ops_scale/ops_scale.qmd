...
# │ Exports: DebtAuditInputs.config_eq_str, .glue_eq_str, .monitoring_eq_str
...
class DebtAuditInputs:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    config_eq_str = MarkdownStr(
        f"{fmt(config_hours_per_deploy, precision=0)} engineer-hours per deployment "
        f"$\\times$ {fmt(deploys_per_year, precision=0)} deployments/year = {fmt(config_hours, precision=0)} hours"
    )
    glue_eq_str = MarkdownStr(
        f"{fmt(glue_hours_per_week, precision=0)} hours/week $\\times$ {fmt(weeks_per_year, precision=0)} weeks "
        f"= {fmt(glue_hours, precision=0)} hours"
    )
    monitoring_eq_str = MarkdownStr(
        f"Extended incident duration costs {fmt_usd(incident_cost_usd, scale='K')} per incident "
        f"$\\times$ {fmt(incidents_per_year, precision=0)} incidents/year = {fmt_usd(monitoring_cost, scale='K')}"
    )
```
...
# │ Exports: FleetCapexRecap.node_price_str, .cluster_gpus_str,
# │          .hardware_capex_m_str
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_math, MarkdownStr
...
class DeploymentSafety:
    ...
    ds_full_cutover_pct_str = fmt_percent(full_cutover_pct, precision=0, commas=False, style='symbol')
...
# │ Exports: CanaryDuration.t_1pct_eq_math, .t_5pct_eq_math
...
# │ Exports: AbSampleSize.per_variant_str, .total_str, .duration_str
...
# │ Exports: MultipleTestingFp.per_quarter_eq_math, .per_year_str
...
# │ Exports: NetworkEffectBias.measured_min_str, .measured_pct_str, ...
...
# │ Exports: TwoSigmaAlerts.daily_eq_math, .remaining_str
from mlsysim.fmt import fmt, fmt_math, fmt_count, check
...
class TwoSigmaAlerts:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(daily_alerts == 14_400 and remaining == 144, "Alert-volume drift")
    # ┌── 4. OUTPUT ────────────────────────────────────────
...
class DriftLatency:
    ...
    dl_drop_prose_str = fmt_percent(p1 - p2, precision=0, commas=False, style='prose')
...
# │ Exports: CostSpikeZScore.z_eq_math
...
# │ Exports: CostQualityTradeoff.medium_judgment_str, .large_judgment_str
from mlsysim.fmt import fmt_pp, fmt_multiple, MarkdownStr, check
...
class CostQualityTradeoff:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    medium_judgment_str = MarkdownStr(f"{fmt_pp(medium_gain_pp, style='prose')} for {fmt_multiple(medium_cost_mult, precision=0, commas=False)} cost")
    large_judgment_str = MarkdownStr(
        f"Additional {fmt_pp(large_gain_pp, style='prose')} for {fmt_multiple(large_cost_mult, precision=0, commas=False)} more"
    )
```
...
# │ Exports: TrainingCostExample.c_train_eq_math, .annual_str
from mlsysim.fmt import fmt, fmt_math, fmt_usd, check
...
class TrainingCostExample:
    ...
    n_gpus = 256
    days = 14
    gpu_hour_rate = 3.50  # illustrative cloud H100-hour rate for this scenario
    pue = 1.15
    fail_factor = 1.15  # 15% failure overhead
    retrains_per_year = 4
    # ┌── 2. EXECUTE ───────────────────────────────────────
    hours = days * 24
    c_train = n_gpus * hours * gpu_hour_rate * pue * fail_factor
    annual_cost = c_train * retrains_per_year
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(round(c_train) == 398_147, f"C_train {c_train} drift")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    c_train_eq_math = fmt_math(
        rf"C_{{\text{{train}}}} = {fmt(n_gpus, precision=0)} \times ({days} \times 24) "
        rf"\times {fmt_usd(gpu_hour_rate, precision=2, commas=False)} \times {pue} \times {fail_factor} "
...
|:---------------------|:----------:|:------------:|:-------------:|:--------:|:----------------:|
| **Daily users**      | 2$\times$  |      --      |     +100%     |  +120%   |       +45%       |
| **Model count**      | 2$\times$  |    +100%     |     +100%     |   +50%   |       +85%       |
| **Queries per user** | 2$\times$  |      --      |     +100%     |   +80%   |       +40%       |
| **Model size**       | 2$\times$  |    +150%     |     +120%     |   +30%   |       +55%       |
| **Retraining freq.** | 2$\times$  |    +100%     |       --      |   +40%   |       +25%       |
...
# │ Exports: OptimizationBreakeven.infer_savings_eq_str, .infer_breakeven_eq_math,
# │          .train_savings_eq_str, .train_breakeven_eq_math
from mlsysim.fmt import fmt, fmt_math, fmt_usd, MarkdownStr, check
...
class OptimizationBreakeven:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    infer_savings_eq_str = MarkdownStr(f"{fmt_usd(infer_monthly)} $\\times$ {fmt(infer_reduction, precision=2)} = {fmt_usd(infer_savings)}")
    ...
    train_savings_eq_str = MarkdownStr(f"{fmt_usd(train_monthly)} $\\times$ {fmt(train_reduction, precision=2)} = {fmt_usd(train_savings)}")
