...
Beyond these core requirements, the distinguishing feature of enterprise registries is explicit dependency tracking (see @fig-model-registry). When Model A consumes features computed by Model B, this relationship must be recorded and enforced.

::: {#fig-model-registry fig-env="figure" fig-pos="htb"}
```{.tikz}
...
The dependency graph (@fig-ecommerce-graph) reveals operational implications:

::: {#fig-ecommerce-graph fig-env="figure" fig-pos="htb"}
```{.tikz}
...
For models affecting users, fairness validation[^fn-fairness-validation] ensures equitable treatment across demographic groups [@hardt2016equality]. @eq-demographic-parity defines demographic parity, requiring similar prediction rates across groups, while @eq-equalized-odds requires similar accuracy across groups:
...
The operational challenges examined thus far assume a controlled datacenter environment. However, as established in @sec-edge-intelligence, the "Machine Learning Fleet" increasingly extends to millions of heterogeneous edge devices. Managing this distributed population introduces three distinct MLOps challenges.
...
While utilization metrics like @eq-platform-utilization capture resource busyness, they often fail to reflect true engineering value. A GPU spinning at 100% utilization on a hyperparameter tuning job that eventually fails due to a configuration error is "efficient" in terms of hardware but "wasteful" in terms of productivity. To address this, we introduce **ML Productivity Goodput (MPG)**, a comprehensive metric for fleet efficiency [@kumar2024machine].
...
**Training-Serving Skew** is the performance degradation that occurs when the data distribution or feature processing logic used during inference (serving) differs from that used during model training. It is a primary cause of silent model failure in production.
:::
...
Training data must use features as they existed at the time of each training example (as visualized in @fig-time-travel). Using current feature values to label historical events creates data leakage[^fn-data-leakage] that inflates offline metrics but fails in production.
...
::: {#fig-time-travel fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  % Timeline
...
As we transition to **Part IV: Production Concerns**, we shift from *how* to build the machine to *how* to protect it. The next chapter, @sec-security-privacy, examines the unique adversarial threats facing ML systems—from data poisoning to model inversion—and the privacy frameworks that determine how we can safely handle the data that fuels our fleet.
