...
$$t_{\text{stage}} = \frac{n_{samples\_needed}}{r_{\text{requests}} \times p_{\text{stage}}}$$ {#eq-canary-duration}

where $t_{\text{stage}}$ is the duration required at a given percentage, $n_{samples\_needed}$ is the number of observations needed for statistical significance, $r_{\text{requests}}$ is the request rate, and $p_{\text{stage}}$ is the traffic percentage.
...
$$C_{\text{train}} = N_{\text{GPU}} \times T_{\text{hours}} \times R_{GPU/hr} \times PUE \times (1 + F_{\text{fail}})$$ {#eq-training-cost}
...
$$C_{\text{infer}} = \frac{Q_{\text{daily}} \times L_{\text{avg}}}{U_{\text{GPU}} \times B_{\text{eff}}} \times R_{GPU/hr} \times 24 \times 365$$ {#eq-inference-cost}
...
$$C_{\text{infer}} = Q_{\text{annual}} \times \frac{t_{\text{inference}}}{U_{\text{GPU}}} \times \frac{R_{GPU/hr}}{3600}$$
...
$$C_{\text{storage}} = V_{\text{data}} \times R_{storage/GB} \times T_{\text{retention}}$$

where $V_{\text{data}}$ is data volume in GB, $R_{storage/GB}$ is the monthly storage rate, and $T_{\text{retention}}$ is retention period in months.

Egress costs scale with data transfer volume:
$$C_{\text{egress}} = V_{\text{transfer}} \times R_{egress/GB}$$
...
$$C_{\text{process}} = V_{\text{processed}} \times R_{process/GB}$$
...
$$\text{Staleness} = t_{\text{current}} - t_{feature\_update}$$ {#eq-feature-staleness}
