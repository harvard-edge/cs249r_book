...
[^fn-prometheus-fleet]: **Prometheus**: An open-source monitoring system (CNCF, 2012) using a **Pull-Based** model to scrape metrics from exporters. For ML fleets, Prometheus is the standard for tracking operational health (CPU/GPU utilization, request rates), but its time-series model is poorly suited for tracking high-dimensional distribution drift, necessitating a two-tier monitoring architecture. \index{Prometheus!monitoring}
