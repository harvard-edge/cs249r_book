...
# │
# │ Imports: mlsysim.book (check)
# │ Exports: md_toil_hours_str, md_low_toil_hours_str, md_savings_ratio_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
from mlsysim.fmt import fmt_rate, fmt_time, fmt_usd
# ┌─────────────────────────────────────────────────────────────────────────────
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: GpuEfficiencyTableRecap.v100_efficiency_tflop_per_s_per_w_str, GpuEfficiencyTableRecap.a100_efficiency_tflop_per_s_per_w_str, GpuEfficiencyTableRecap.h100_efficiency_tflop_per_s_per_w_str, GpuEfficiencyTableRecap.b200_efficiency_tflop_per_s_per_w_str, GpuEfficiencyTableRecap.b200_fp8_tflop_per_s_str, GpuEfficiencyTableRecap.b200_tdp_w_str
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: V100EfficiencyRecap.flops_fp16_tflop_per_s_str, V100EfficiencyRecap.tdp_w_str
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: A100EfficiencyRowRecap.flops_fp16_tflop_per_s_str, A100EfficiencyRowRecap.tdp_w_str
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: TensorCoreEfficiencyRecap.h100_fp8_tc_tflop_per_s_str
...
|:----------------|:----------------------------------------------------------------------:|:-------------------------------------------------:|:------------------------------------------------------------------------:|:-----------------------:|
| **V100 (2017)** |    `{python} V100EfficiencyRecap.flops_fp16_tflop_per_s_str` (FP16)    |      `{python} V100EfficiencyRecap.tdp_w_str`     | `{python} GpuEfficiencyTableRecap.v100_efficiency_tflop_per_s_per_w_str` |        1$\times$        |
| **A100 (2020)** |  `{python} A100EfficiencyRowRecap.flops_fp16_tflop_per_s_str` (FP16)   |    `{python} A100EfficiencyRowRecap.tdp_w_str`    | `{python} GpuEfficiencyTableRecap.a100_efficiency_tflop_per_s_per_w_str` |       1.9$\times$       |
| **H100 (2022)** | `{python} TensorCoreEfficiencyRecap.h100_fp8_tc_tflop_per_s_str` (FP8) |    `{python} H100TdpEfficiencyRecap.tdp_w_str`    | `{python} GpuEfficiencyTableRecap.h100_efficiency_tflop_per_s_per_w_str` |       6.8$\times$       |
| **B200 (2024)** |   `{python} GpuEfficiencyTableRecap.b200_fp8_tflop_per_s_str` (FP8)    | `{python} GpuEfficiencyTableRecap.b200_tdp_w_str` | `{python} GpuEfficiencyTableRecap.b200_efficiency_tflop_per_s_per_w_str` |       10.8$\times$      |
...
Node sizing then decides how many accelerators share a host and which memory tier carries optimizer state, activations, and data staging. For our `{python} InfraFrontierTcoNodeRecap.frontier_params_b_str` model, that points toward eight GPUs per node with tensor parallelism and roughly 2 TB of host DRAM for optimizer-state offload. Cluster sizing divides the total compute budget by sustained per-node throughput after MFU losses, then adds 5--10 percent for a maintenance pool and spares. Network design follows the dominant collective and validates expected scaling with @eq-distributed-training-scaling-efficiency. Power and cooling translate the IT load through PUE, using @tbl-rack-power as the per-rack sanity check. The final step is not a spreadsheet afterthought: the 3-year TCO and the procurement timeline determine whether the cluster can arrive in time to matter, especially when GPU supply and electrical work dominate lead time.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: NodeCapacityScenarioSecComputeTcoRecap.h100_fp8_tflop_per_s_str
...
# │
# │ Imports: mlsysim.book (check)
# │ Exports: ds_canary_pct_str, ds_risk_reduction_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
3. **Risk Mitigation**\index{Risk Mitigation}: 100/5 = `{python} DeploymentSafety.ds_risk_reduction_mult_str`.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, MarkdownStr, fmt_count, fmt_rate, fmt_time, fmt_usd, fmt_int
...
class SilentFailure:
    ...
    sf_sec_per_day_str = fmt_time(SEC_PER_HOUR * 24, "second", precision=0, commas=False, per="day")
    sf_total_requests_str = fmt_count(total_requests, commas=False, label="request")
...
1. **Total Requests**\index{Total Requests}: `{python} SilentFailure.sf_qps_str` $\times$ `{python} SilentFailure.sf_sec_per_day_str` = `{python} SilentFailure.sf_total_requests_str`.
2. **Lost Clicks**\index{Lost Clicks}: `{python} SilentFailure.sf_total_requests_str` $\times$ 0.005 = `{python} SilentFailure.sf_lost_clicks_str`.
...
At the lowest level, the platform owns allocation rather than ML workflow. It provides GPU capacity, storage volumes, network connectivity, and basic orchestration such as Kubernetes namespaces. Model teams still build the training code, serving stack, monitoring, and deployment path themselves. This is the right abstraction when workloads are unusual and teams have strong infrastructure expertise; it becomes costly when many teams repeat the same operational work.

#### Level 2: Container orchestration {#sec-ml-operations-scale-level-2-container-orchestration-ee8a}

The next boundary moves packaging and runtime concerns into the platform. Standardized container images, ML-aware Kubernetes integration, persistent volume management, and basic service-mesh support give teams a common execution substrate. Model teams still decide how experiments, training jobs, serving releases, and monitoring workflows fit together. The duplication tax falls, but operational correctness still depends heavily on each team.

#### Level 3: ML-aware scheduling {#sec-ml-operations-scale-level-3-mlaware-scheduling-4895}

At the third level, the platform starts owning ML-specific control loops. Training job scheduling understands GPU topology, hyperparameter tuning runs as shared infrastructure, distributed training coordination becomes repeatable, and model serving includes autoscaling rather than only a container target. Systems such as Kubeflow and Ray occupy this level. Model teams retain modeling judgment, but the platform absorbs the parts of ML operations that fail in similar ways across teams.

#### Level 4: Full platform {#sec-ml-operations-scale-level-4-full-platform-424c}

The highest level treats the ML lifecycle as a governed internal product. Integrated development environments, feature stores, experiment tracking, model registries, CI/CD, monitoring, alerting, cost attribution, and governance appear behind high-level APIs. Platforms such as Vertex AI, SageMaker, TFX[^fn-tfx-production] at Google [@baylor2017], and MLflow [@zaharia2018accelerating] represent this end of the spectrum. The trade-off is explicit: teams gain speed and consistency by giving up some low-level control, while the platform team accepts responsibility for policy, reliability, and economic visibility.
...
# │          total_scale_factor_mult_str, s_dom_str, p_dom_str
# └─────────────────────────────────────────────────────────────────────────────
...
The later `user_engagement_score` incident makes the mechanics concrete. A platform team changes the feature from a 30-day z-score to a 7-day min-max scale. The output may still be a floating-point number, so schema validation alone can pass while the feature's meaning changes completely. Versioning names the semantic change before it reaches production; lineage identifies every model trained or served with the old meaning; backfill recomputes historical values for the new meaning; and freshness and quality checks decide whether the rewritten history is safe to use. Without that chain, a feature migration becomes a silent multi-model regression.

#### Version schema {#sec-ml-operations-scale-version-schema-178c}

A version schema must separate the parts of a feature that change for different reasons. The definition version identifies the computation logic, the data version identifies the source snapshot or stream contract, and the schema version identifies the output type and shape. A change to any one of these can preserve API compatibility while changing model semantics, so dependent models declare the exact feature versions they consume. That declaration gives the platform a place to block unsafe updates before incompatible feature meanings reach production.

#### Lineage tracking {#sec-ml-operations-scale-lineage-tracking-1265}

Feature lineage records enough provenance to make that version declaration actionable. Each feature value needs a path back to the source tables and their versions, the transformation code, the computation timestamp and environment, and the quality metrics observed when the value was produced. Without that path, a bad prediction leaves the operator comparing symptoms. With it, the platform can trace unexpected feature behavior to its source, estimate the blast radius of an upstream data change, and reproduce historical values for audit or compliance.
...
The arithmetic behind a large recommender explains why feature stores become serving infrastructure rather than a metadata convenience. One billion daily recommendations with 100 features per recommendation produces 100 billion feature lookups per day. That average load is roughly 1.1 million lookups per second before peak traffic, which commonly reaches 5--10$\times$ the mean.
...
The investigation starts with the model symptom and then works backward through the data path. Model metrics confirm that CTR declined from 4.2 percent to 3.87 percent, but freshness checks show that features still arrive within the 1-hour SLO. That rules out a stale pipeline and points to semantic change. Distribution analysis then shows the `user_engagement_score` mean shifting from 0.42 to 0.31, a 26 percent decline, and lineage tracking ties the shift to an upstream pipeline version deployed five days earlier. The root cause is not model drift but a feature computation bug in the new pipeline version.\index{Model Metrics}\index{Data!data freshness}\index{Lineage Tracking}\index{Root Cause}
...
Incident response\index{Incident Response} procedures activate when quality degrades. Automated alerts page the on-call team with severity and affected systems, while circuit breakers prevent known-bad data from reaching models. The serving path falls back to last-known-good data or cached features, and bad batches are quarantined for forensic analysis. Root-cause analysis then repairs the pipeline rather than only clearing the current alert.
...
Feature freshness monitoring becomes computationally intensive at scale. For 10,000 features updated at different frequencies, continuous freshness checking generates substantial overhead, so efficient implementations reduce the number of checks without losing systemic signal. Sampling representative entities rather than every feature value is usually enough to detect systemic freshness failures. Pipeline-level aggregation is even cheaper when one pipeline updates hundreds of features, because the pipeline update timestamp captures the freshness state of the group. Threshold stratification reserves per-feature monitoring for revenue-critical features and uses aggregated monitoring for features whose staleness has lower immediate impact.
...
Technical infrastructure alone is insufficient for ML operations at scale. Organizational structure determines whether platform capabilities become shared infrastructure or another bottleneck. The central question is where an organization places invariants: deployment safety, observability, compliance, cost control, and feature consistency can either be enforced once by a platform or rediscovered differently by every model team.
...
The appropriate organizational pattern depends on where standardization creates more value than local autonomy. @Tbl-ops-scale-org-factors summarizes the decision pressure: higher model counts, stricter regulatory requirements, and immature infrastructure favor centralized platforms, while heterogeneous model portfolios and smaller organizations may benefit from distributed expertise:
...
The runbook below encodes that order for a recommendation engagement drop. The symptom establishes the user-visible failure, the diagnostic steps move from business metrics to shared feature infrastructure to recent model changes, and the decision tree keeps mitigation tied to evidence. Escalation thresholds matter because an ML incident can look deceptively quiet: latency may be normal while revenue or engagement falls.
...
The template below is useful only because each field forces a control decision. Duration and impact quantify severity; the timeline exposes detection and attribution latency; root causes name missing controls; and corrective actions assign ownership for the next platform change. In the recommendation incident, the most important entry is not the disk-full symptom but the absence of a freshness gate that would have caught the stalled feature pipeline before engagement metrics moved.
