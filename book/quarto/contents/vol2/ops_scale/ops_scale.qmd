...
class BuildVsBuyFigureScenario:
    ...
    cloud_monthly_str = fmt_usd(cloud_monthly_usd, precision=2, scale="M")
    cloud_horizon_str = fmt_usd(cloud_horizon_usd, precision=1, scale="M")
...
class TcoScenario:
    ...
    annual_elec_str = fmt_usd(annual_elec, precision=1)
    onprem_str = fmt_usd(onprem_node_annual, precision=1)
    util_pct_str = fmt(utilization*100, precision=0, commas=False)
...
class TenKGpuClusterTco:
    ...
    annual_electricity_str = fmt_usd(annual_electricity_usd, precision=1, scale="M")
    annual_staffing_str = fmt_usd(annual_staffing_usd, precision=0, scale="M")
    upfront_capex_str = fmt_usd(upfront_capex_usd, precision=1, scale="M")
    annual_opex_str = fmt_usd(annual_opex_usd, precision=1, scale="M")
    three_year_total_str = fmt_usd(three_year_total_usd, precision=1, scale="M")
    cloud_annual_str = fmt_usd(cloud_annual_usd, precision=1, scale="M")
    cloud_three_year_str = fmt_usd(cloud_three_year_usd, precision=1, scale="M")
...
- **Annual Cost**\index{Cost!annual cost}: `{python} TenKGpuClusterTco.gpus_str` $\times$ `{python} TenKGpuClusterTco.cloud_gpu_hour_usd_str` $\times$ `{python} TenKGpuClusterTco.hours_per_year_str` h $\times$ `{python} TenKGpuClusterTco.utilization_str` = `{python} TenKGpuClusterTco.cloud_annual_str`/year
...
class TcoComparison:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    s_training_str = fmt_usd(s_training, precision=0, per="month")
    s_inference_str = fmt_usd(s_inference, precision=0, per="month")
    s_data_str = fmt_usd(s_data, precision=0, per="month")
    s_iteration_str = fmt_usd(s_iteration, precision=0, per="month")
    s_total_str = fmt_usd(s_total, precision=0, per="month")

    p_training_str = fmt_usd(p_training, precision=0, per="month")
    p_inference_str = fmt_usd(p_inference, precision=0, per="month")
    p_data_str = fmt_usd(p_data, precision=0, per="month")
    p_iteration_str = fmt_usd(p_iteration, precision=0, per="month")
    p_total_str = fmt_usd(p_total, precision=0, per="month")
