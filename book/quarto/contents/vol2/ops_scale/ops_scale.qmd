...
In the **Fleet Stack** (@sec-vol2-introduction), MLOps is the **Control Plane**—the nervous system that monitors and corrects the behavior of the fleet. While the **Distribution Layer** (training) and **Serving Layer** (inference) execute the work, the Control Plane ensures they remain within safe operating parameters. If the **Iron Law** defines performance, MLOps defines **Reliability**: ensuring that $L_{step}$ remains stable over months of operation.
