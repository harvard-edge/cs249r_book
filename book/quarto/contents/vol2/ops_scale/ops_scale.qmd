...
class TrainingCapacityCost:
    ...
    gpu_hour_rate_str = fmt_usd(gpu_hour_rate, precision=0, commas=False, suffix="/GPU-hour")
    training_cost_m_str = fmt_usd(training_cost / MILLION, precision=2, commas=False, suffix="M")
```
...
class DeploymentRoi:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    roi_engineer_rate_str = fmt_usd(engineer_rate, precision=0, commas=False, suffix="/hr")
    ...
    weekly_savings_str = fmt_usd(weekly_savings, precision=0, commas=False, suffix="/week")
...
```{python}
#| echo: false
#| label: ops-scale-archetype-table
# ┌─────────────────────────────────────────────────────────────────────────────
# │ OPERATIONAL REQUIREMENTS BY ARCHETYPE
# ├─────────────────────────────────────────────────────────────────────────────
# │ Goal: Keep Archetype A/B operational cadence tied to MLSysIM.
# │ How: Non-archetype rows remain chapter-local examples; the recurring
# │      lighthouse rows come from Scenarios.Archetypes.Fleet.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Scenarios as BookScenarios
from mlsysim.fmt import MarkdownStr

class OpsScaleArchetypeTable:
    """Display fields for recurring fleet archetype operations."""

    a = BookScenarios.Archetypes.Fleet.ArchetypeA
    b = BookScenarios.Archetypes.Fleet.ArchetypeB

    a_label_str = MarkdownStr(a.label)
    a_update_str = MarkdownStr(a.update_cadence)
    a_deploy_str = MarkdownStr(a.deployment_pattern)
    a_risk_str = MarkdownStr(a.primary_risk)
    a_rollback_str = MarkdownStr(a.rollback_window)
    b_label_str = MarkdownStr(b.label)
    b_update_str = MarkdownStr(b.update_cadence)
    b_deploy_str = MarkdownStr(b.deployment_pattern)
    b_risk_str = MarkdownStr(b.primary_risk)
    b_rollback_str = MarkdownStr(b.rollback_window)
```

| **Model Type**                                    | **Update Frequency**                           | **Deployment Pattern**                         | **Primary Risk**                             | **Rollback Speed**                               |
|:--------------------------------------------------|:-----------------------------------------------|:-----------------------------------------------|:---------------------------------------------|:-------------------------------------------------|
| **`{python} OpsScaleArchetypeTable.a_label_str`** | `{python} OpsScaleArchetypeTable.a_update_str` | `{python} OpsScaleArchetypeTable.a_deploy_str` | `{python} OpsScaleArchetypeTable.a_risk_str` | `{python} OpsScaleArchetypeTable.a_rollback_str` |
| **`{python} OpsScaleArchetypeTable.b_label_str`** | `{python} OpsScaleArchetypeTable.b_update_str` | `{python} OpsScaleArchetypeTable.b_deploy_str` | `{python} OpsScaleArchetypeTable.b_risk_str` | `{python} OpsScaleArchetypeTable.b_rollback_str` |
...
class BuildVsBuyFigureScenario:
    ...
    horizon_months_str = fmt(horizon_months, precision=0, commas=False, suffix=" months")
    upfront_capex_str = fmt_usd(upfront_capex_usd / MILLION, precision=2, suffix="M")
    annual_opex_str = fmt_usd(annual_opex_usd / MILLION, precision=1, suffix="M")
    cloud_monthly_str = fmt_usd(cloud_monthly_usd / MILLION, precision=2, suffix="M")
    cloud_horizon_str = fmt_usd(cloud_horizon_usd / MILLION, precision=1, suffix="M")
...
class TcoScenario:
    ...
    util_pct_str = fmt(utilization*100, precision=0, commas=False)
    breakeven_util_str = fmt(breakeven, precision=1, commas=False, suffix=" percent")
```
...
class TenKGpuClusterTco:
    ...
    hardware_capex_str = fmt_usd(hardware_capex_usd / MILLION, precision=1, suffix="M")
    network_capex_str = fmt_usd(network_capex_usd / MILLION, precision=0, suffix="M")
    facility_capex_str = fmt_usd(facility_capex_usd / MILLION, precision=0, suffix="M")
    annual_electricity_str = fmt_usd(annual_electricity_usd / MILLION, precision=1, suffix="M")
    annual_staffing_str = fmt_usd(annual_staffing_usd / MILLION, precision=0, suffix="M")
    upfront_capex_str = fmt_usd(upfront_capex_usd / MILLION, precision=1, suffix="M")
    annual_opex_str = fmt_usd(annual_opex_usd / MILLION, precision=1, suffix="M")
    three_year_total_str = fmt_usd(three_year_total_usd / MILLION, precision=1, suffix="M")
    cloud_annual_str = fmt_usd(cloud_annual_usd / MILLION, precision=1, suffix="M")
    cloud_three_year_str = fmt_usd(cloud_three_year_usd / MILLION, precision=1, suffix="M")
    savings_str = fmt_usd(savings_usd / MILLION, precision=1, suffix="M")
    ...
    node_price_str = fmt_usd(node_price_usd, precision=0, commas=True, suffix="")
    ...
    cloud_gpu_hour_str = fmt_usd(cloud_gpu_hour_usd, precision=0, commas=False, suffix="/hr")
...
class TcoComparison:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    s_training_str = fmt_usd(s_training, precision=0, suffix="/month")
    s_inference_str = fmt_usd(s_inference, precision=0, suffix="/month")
    s_data_str = fmt_usd(s_data, precision=0, suffix="/month")
    s_iteration_str = fmt_usd(s_iteration, precision=0, suffix="/month")
    s_total_str = fmt_usd(s_total, precision=0, suffix="/month")

    p_training_str = fmt_usd(p_training, precision=0, suffix="/month")
    p_inference_str = fmt_usd(p_inference, precision=0, suffix="/month")
    p_data_str = fmt_usd(p_data, precision=0, suffix="/month")
    p_iteration_str = fmt_usd(p_iteration, precision=0, suffix="/month")
    p_total_str = fmt_usd(p_total, precision=0, suffix="/month")
