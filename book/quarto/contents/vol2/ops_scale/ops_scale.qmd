...
: **TCO Comparison: Startup vs. Production Company**. Cost structure shifts dramatically with scale. Startups are dominated by iteration costs (engineering salaries for experimentation), while production companies see inference costs dominate as serving volume grows. The 100 $\times$ user increase yields only 20 $\times$ TCO increase due to optimization effects, but note the superlinear 160 $\times$ scaling in data costs. {#tbl-tco-comparison}
