...
The management layer is the fleet stack's control plane\index{Control Plane}: the dashboard, steering, and maintenance system that keeps the physical and logical layers below from drifting apart. Parts I and II built the engine, and Part III deployed the service. This chapter explains the operational machinery that keeps the whole fleet running after deployment.
...
Capacity planning for large-scale ML is fundamentally an exercise in optimizing the economics of the GPU-hour. In the running example, a 30-day training run on a 256-node H100 cluster (`{python} TrainingCapacityCost.n_gpus_str`) at an illustrative market rate of `{python} TrainingCapacityCost.gpu_hour_rate_str` represents a direct investment of roughly `{python} TrainingCapacityCost.training_cost_m_str`. At this scale, the cost per training run $(C_{\text{run}})$ becomes a primary design lever that dictates the sizing of the entire fleet. Faster time-to-market pushes the planner toward more GPUs, but larger jobs also suffer diminishing parallel efficiency and a higher probability of hardware interruption. Total cost is therefore not merely a function of compute time; it must account for data staging, checkpointing, and the cost of recovery from interruptions (@sec-fault-tolerance-reliability).

The drive for efficiency forces capacity to behave like a dynamic resource rather than a static allocation. Organizations must decide when to buy additional permanent capacity and when to recover the same effective throughput through better orchestration or model optimization. The decision also has an energy dimension: powering thousands of GPUs for months consumes enough electricity that financial cost and carbon cost move together. Capacity planning therefore becomes a strategic lever that balances performance, budget, and responsible engineering, with the carbon accounting developed later in @sec-sustainable-ai.
...
|:-----------------------|:--------------------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------|:-------------------------------------------------------------------|:----------------------------------------------------------------------------------------|
| **Configuration debt** | Each model has custom YAML (YAML Ain't Markup Language) configuration files averaging 847 lines with no validation schema | 35 percent of deployment delays result from late config errors | Manual config review required for every deployment                 | 12 engineer-hours per deployment $\times$ 80 deployments/year = 960 hours               |
| **Pipeline glue code** | Data preprocessing uses 23 different scripts with 62 percent code duplication                                             | 12 engineer-hours per week debugging pipeline breaks           | No shared preprocessing library; each team implements custom logic | 12 hours/week $\times$ 52 weeks = 624 hours                                             |
...
While operational tooling saves engineering hours, the financial ledger of a large-scale AI lab is dominated by physical capital. Deployment practices cannot scale economically until the fleet's hardware costs are visible.
...
Third, all major providers offer **custom accelerator**\index{Custom Accelerator} options (purpose-built training and inference ASICs) alongside GPU instances. These custom chips apply the same generality-efficiency trade-off introduced at the silicon level: lower cost per operation for supported workloads, but reduced flexibility for nonstandard architectures. The choice between GPU and custom accelerator instances mirrors the on-premises accelerator selection decision.
...
At that point, the remaining planning sequence is:

1.  **Node sizing**: Decide how many accelerators share a host and which memory tier carries optimizer state, activations, and data staging. For our `{python} InfraFrontierTcoNodeRecap.frontier_params_b_str` model, that points toward eight GPUs per node with tensor parallelism and roughly 2 TB of host DRAM for optimizer-state offload.
2.  **Cluster sizing**: Divide the total compute budget by sustained per-node throughput after MFU losses, then add 5--10 percent for a maintenance pool and spares.
3.  **Network design**: Follow the dominant collective and validate expected scaling with @eq-distributed-training-scaling-efficiency.
4.  **Power and cooling**: Translate IT load through PUE, using @tbl-rack-power as the per-rack sanity check.
5.  **TCO and schedule**: Treat the 3-year TCO and the procurement timeline as part of the design, because the cluster must arrive in time to matter and GPU supply or electrical work often dominates lead time.
...
For models affecting users, fairness validation[^fn-fairness-validation-tradeoff] turns a policy commitment into a release gate [@hardt2016equality]. The platform cannot simply ask whether "the model is fair" because different contexts encode fairness differently. @Eq-demographic-parity expresses one operational choice: the probability of a positive prediction must differ by less than threshold $\epsilon$ between protected groups $a$ and $b$. @Eq-equalized-odds encodes a stricter choice: prediction behavior must be similar across groups after conditioning on the true outcome.
...
Application-layer drift detection depends on a healthy physical substrate. At fleet scale, a degraded NVLink connection or a thermally throttling GPU can masquerade as a software timeout.
...
Standard monitoring cannot detect semantic safety failures such as persuasive misinformation or skilled manipulation. Red teaming\index{Red Teaming} adds an adversarial testing channel: human evaluators or automated probes attempt jailbreaks\index{Jailbreak} before deployment, and a smaller continuous probe set verifies that production safety filters remain active. Sampling outputs for delayed human review closes the loop for failures that automated metrics do not observe.
...
A dashboard is an attention-routing mechanism: each view answers a different operational question and withholds detail until the reader needs it. @Tbl-ops-scale-dashboard-views shows the resulting progression: executive views show whether the business is impaired, portfolio views locate correlated failures across models, model views support an owner's diagnosis, and investigation views trace root cause.
...
Model teams specify what they need; the platform handles how to provide it and applies shared controls that keep local autonomy from creating fleet-wide risk. Resource quotas bind deployment requests to GPU and compute budgets, with approval required when a model exceeds its allocation. Security requirements attach controls to models that access sensitive data, while quality gates block deployment until validation succeeds. Deployment windows apply the same logic to time: high-risk releases should occur only when rollback and incident-response capacity are available.
...
Serving costs scale with traffic, but ML serving cannot autoscale like a stateless web application. Large models may take seconds or minutes to load, so purely reactive scaling arrives after the latency spike; the platform needs predictive scale-up before anticipated demand.

The capacity steps are also lumpy. GPU memory makes serving scale in jumps of 0, 1, 2, or 4 GPUs rather than smooth CPU fractions, and batching adds another coupling because waiting for a larger batch improves utilization but consumes latency budget. An autoscaling policy must therefore choose the least expensive capacity step that still protects the p99 latency SLO.

#### Right-sizing and instance selection {#sec-ml-operations-scale-rightsizing-instance-selection-fe66}

ML workloads have distinct compute profiles, so right-sizing starts from the binding constraint rather than the newest instance type. Memory-bound training with large embedding tables prioritizes GPU memory capacity and bandwidth. Compute-bound dense training maximizes sustained FLOP/s per dollar.
...
A critical operational decision for LLM platforms is how to inject domain knowledge. Retrieval-augmented generation\index{Retrieval-Augmented Generation} (RAG) retrieves documents at inference time and places them in the prompt [@lewis2020retrieval]. Fine-tuning, including supervised fine-tuning (SFT) and low-rank adaptation (LoRA), stores the adaptation in model weights or adapter weights. Both approaches can improve task behavior, but they move cost and failure risk to different places in the fleet.
...
- **Setup**: At midnight, the feature pipeline calculates `total_clicks_today` for User A as 10.
- **Failure**: Using this value (10) to train a model predicting a click that happened at noon creates leakage\index{Leakage} from future information. At noon, the user might have only had 4 clicks.
...
```{python}
#| echo: false
#| label: feature-store-scale-scenario
# ┌─────────────────────────────────────────────────────────────────────────────
# │ FEATURE STORE SCALE SCENARIO
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Feature-store scale, data-quality incident, and freshness examples
# │
# │ Goal: Keep recurring feature-store quantities in one scenario so storage,
# │       latency, quality, and incident-impact prose do not drift independently.
# │ Show: Feature-history volume, lookup rate, latency budget, completeness,
# │       CTR incident impact, and monitoring-sampling assumptions.
# │ How: Derive displayed values from one recommendation-platform scenario.
# │
# │ Imports: mlsysim.core.constants, mlsysim.core.units, mlsysim.fmt
# │ Exports: FeatureStoreScale.storage_users_str, .storage_features_str,
# │          .storage_retention_years_str, .storage_update_interval_hr_str,
# │          .storage_feature_values_str, .storage_bytes_per_value_str,
# │          .storage_total_pb_str, .daily_recommendations_str,
# │          .features_per_recommendation_str, .lookups_per_day_str,
# │          .avg_lookup_rate_str, .peak_traffic_range_mult_str,
# │          .recommendation_budget_ms_str, .feature_budget_range_ms_str,
# │          .network_overhead_range_ms_str, .store_lookup_range_ms_str,
# │          .expected_events_str, .delivered_events_str,
# │          .completeness_pct_str, .validation_failure_threshold_pct_str,
# │          .ctr_drop_pct_str, .incident_days_str, .ctr_before_pct_str,
# │          .ctr_after_pct_str, .freshness_slo_hr_str, .engagement_before_str,
# │          .engagement_after_str, .engagement_decline_pct_str,
# │          .weekly_revenue_usd_str, .revenue_impact_usd_str,
# │          .alert_detection_hr_str, .impact_reduction_pct_str,
# │          .feature_shift_threshold_pct_str, .freshness_feature_count_str,
# │          .freshness_sample_pct_str, .pipeline_feature_count_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MILLION, BILLION
from mlsysim.core.units import PB, byte
...
class FeatureStoreScale:
    """Recommendation feature-store scenario used across the scale discussion."""
    ...
    check(87 < (storage_total_bytes * byte).to(PB).magnitude < 88, "Storage should remain about 87 PB.")
    check(1.1 * MILLION < avg_lookups_per_s < 1.2 * MILLION, "Average lookup rate should be about 1.16M/s.")
    ...
    storage_bytes_per_value_str = fmt_memory(storage_bytes_per_value * byte, unit=byte, precision=0, commas=False)
    storage_total_pb_str = fmt_memory(storage_total_bytes * byte, unit=PB, precision=1, commas=False)
...
For `{python} FeatureStoreScale.storage_users_str`, `{python} FeatureStoreScale.storage_features_str`, `{python} FeatureStoreScale.storage_retention_years_str` retention, and updates every `{python} FeatureStoreScale.storage_update_interval_hr_str`, the history contains `{python} FeatureStoreScale.storage_feature_values_str`. At `{python} FeatureStoreScale.storage_bytes_per_value_str` per value, this represents approximately `{python} FeatureStoreScale.storage_total_pb_str` before compression. Efficient feature stores use compression, columnar storage, and retention policies to manage this scale.
...
The arithmetic behind a large recommender explains why feature stores become serving infrastructure rather than a metadata convenience. `{python} FeatureStoreScale.daily_recommendations_str` with `{python} FeatureStoreScale.features_per_recommendation_str` per recommendation produces `{python} FeatureStoreScale.lookups_per_day_str` per day. That average load is roughly `{python} FeatureStoreScale.avg_lookup_rate_str` before peak traffic, which commonly reaches `{python} FeatureStoreScale.peak_traffic_range_mult_str` the mean.

Feature retrieval must fit inside the overall latency budget because every extra lookup millisecond steals time from ranking. If the recommendation budget is `{python} FeatureStoreScale.recommendation_budget_ms_str`, the feature store may receive only `{python} FeatureStoreScale.feature_budget_range_ms_str`. Network overhead can consume `{python} FeatureStoreScale.network_overhead_range_ms_str` of that allocation, leaving roughly `{python} FeatureStoreScale.store_lookup_range_ms_str` for the store lookup itself. Meeting this budget requires in-memory storage, careful batching, and geographic placement close enough to the serving fleet that network latency does not dominate.
...
Data quality must be quantified in dimensions that map directly to prevention gates. Completeness measures whether expected records and fields are present. A daily data pipeline expected to produce `{python} FeatureStoreScale.expected_events_str` but delivering only `{python} FeatureStoreScale.delivered_events_str` is `{python} FeatureStoreScale.completeness_pct_str` complete, which usually indicates a pipeline failure or upstream data source issue rather than ordinary statistical variation. Consistency captures schema compliance and referential integrity: an `age` feature should fall between 0 and 120, while values such as 999 or -1 suggest sentinel leakage or data errors. Production systems often reject batches that fail more than `{python} FeatureStoreScale.validation_failure_threshold_pct_str` of validation rules because allowing corrupt data through is more expensive than delaying a batch.
...
A recommendation model's click-through rate (CTR) drops `{python} FeatureStoreScale.ctr_drop_pct_str` over `{python} FeatureStoreScale.incident_days_str`. Initial hypothesis focuses on model drift, but data quality investigation reveals the root cause.
...
The investigation starts with the model symptom and then works backward through the data path. Model metrics confirm that CTR declined from `{python} FeatureStoreScale.ctr_before_pct_str` to `{python} FeatureStoreScale.ctr_after_pct_str`, but freshness checks show that features still arrive within the `{python} FeatureStoreScale.freshness_slo_hr_str` SLO. That rules out a stale pipeline and points to semantic change. Distribution analysis then shows the `user_engagement_score` mean shifting from `{python} FeatureStoreScale.engagement_before_str` to `{python} FeatureStoreScale.engagement_after_str`, a `{python} FeatureStoreScale.engagement_decline_pct_str` decline, and lineage tracking ties the shift to an upstream pipeline version deployed `{python} FeatureStoreScale.incident_days_str` earlier. The root cause is not model drift but a feature computation bug in the new pipeline version.\index{Model Metrics}\index{Data!data freshness}\index{Lineage Tracking}\index{Root Cause}

The quantitative impact is the relative CTR loss multiplied by weekly revenue and the fraction of the week exposed: `{python} FeatureStoreScale.ctr_drop_pct_str` of `{python} FeatureStoreScale.weekly_revenue_usd_str`/week over `{python} FeatureStoreScale.incident_days_str` produces `{python} FeatureStoreScale.revenue_impact_usd_str` in lost revenue. Distribution monitoring with automated alerts would detect the shift within `{python} FeatureStoreScale.alert_detection_hr_str`, reducing impact by `{python} FeatureStoreScale.impact_reduction_pct_str`.

The resolution is to roll back the pipeline to the previous version, redeploy models with correctly computed features, and add a distribution validation gate to prevent future pipeline deployments with feature shifts exceeding the `{python} FeatureStoreScale.feature_shift_threshold_pct_str` threshold.
...
Feature freshness monitoring becomes computationally intensive at scale. For `{python} FeatureStoreScale.freshness_feature_count_str` updated at different frequencies, continuous freshness checking generates substantial overhead, so efficient implementations reduce the number of checks without losing systemic signal. Sampling `{python} FeatureStoreScale.freshness_sample_pct_str` of representative entities rather than every feature value is usually enough to detect systemic freshness failures. Pipeline-level aggregation is even cheaper when one pipeline updates `{python} FeatureStoreScale.pipeline_feature_count_str`, because the pipeline update timestamp captures the freshness state of the group. Threshold stratification reserves per-feature monitoring for revenue-critical features and uses aggregated monitoring for features whose staleness has lower immediate impact.
