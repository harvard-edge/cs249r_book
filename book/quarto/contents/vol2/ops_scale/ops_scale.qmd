...
Just as a single developer can compile code manually, a single data scientist can train a model manually. But compiling an operating system requires a build system (Make/Bazel). Similarly, training a fleet of models requires an **ML Build System** that manages dependencies, compilation steps (training), and linking (serving) of the entire codebase. This chapter is about building that Make/Bazel for AI.
...
$$Z = \frac{C_{current} - \mu}{\sigma}$$ {#eq-cost-zscore}
...
$$\Delta\% = \frac{C_{current} - C_{previous}}{C_{previous}} \times 100$$ {#eq-cost-pct-change}
