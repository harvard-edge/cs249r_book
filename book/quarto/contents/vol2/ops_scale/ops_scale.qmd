...
class MaintenanceDividend:
    ...
    md_low_toil_str = fmt_time(low_toil_h_per_month, 'hour', precision=0, commas=False, style='word')
    md_savings_ratio_str = fmt(ratio, precision=1, commas=False)
    md_maintenance_investment_str = fmt_time(maintenance_investment_h, 'hour', precision=0, commas=False, style='word')
...
2. **Proactive Path**\index{Proactive Path}: (`{python} MaintenanceDividend.md_maintenance_investment_str` investment + `{python} MaintenanceDividend.md_months_str` $\times$ `{python} MaintenanceDividend.md_low_toil_str`/month) $\times$ `{python} MaintenanceDividend.md_engineer_rate_str` = `{python} MaintenanceDividend.md_cost_proactive_str`.
...
class TenKGpuClusterTco:
    ...
    gpu_tdp_str = fmt_qty(gpu_tdp, watt, precision=0, commas=False)
...
class V100EfficiencyRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    flops_fp16_str = fmt_qty(Hardware.Cloud.V100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
...
class A100EfficiencyRowRecap:
    flops_fp16_str = fmt_qty(Hardware.Cloud.A100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
...
class TensorCoreEfficiencyRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp8_tc_str = fmt_qty(Hardware.Cloud.H100.compute.precision_flops['fp8'], TFLOPs/second, precision=0, commas=True, unit_label="TFLOP/s")
```
...
class RackPowerPlanningRecap:
    ...
    rack_power_str = fmt_qty(rack_power, kilowatt, precision=1, commas=False)
```
