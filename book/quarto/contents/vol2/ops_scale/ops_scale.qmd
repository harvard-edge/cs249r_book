...
2. **Latency analysis**: P99 latency increased from 50 ms to 125 ms. Each request now consumes roughly 2.5$\times$ the GPU-seconds.

3. **Model version check**: A new model version deployed at 2:00 AM used a larger ranking model and additional features for quality improvements.

4. **Resource utilization**: GPU utilization remained at 95 percent, but throughput (requests per second per GPU) dropped 60 percent.
...
The investigation starts with the model symptom and then works backward through the data path. Model metrics\index{Model Metrics} confirm that CTR declined from `{python} FeatureStoreScale.ctr_before_pct_str` to `{python} FeatureStoreScale.ctr_after_pct_str`, but freshness checks\index{Data!data freshness} show that features still arrive within the `{python} FeatureStoreScale.freshness_slo_hr_str` SLO. That rules out a stale pipeline and points to semantic change. Distribution analysis then shows the `user_engagement_score` mean shifting from `{python} FeatureStoreScale.engagement_before_str` to `{python} FeatureStoreScale.engagement_after_str`, a `{python} FeatureStoreScale.engagement_decline_pct_str` decline, and lineage tracking\index{Lineage Tracking} ties the shift to an upstream pipeline version deployed `{python} FeatureStoreScale.incident_days_str` earlier. The root cause is not model drift but a feature computation bug in the new pipeline version.
...
* **Ensembles are the unit of management**: In production (especially recommendation), the atomic unit is rarely a single model but an ensemble\index{Model Ensemble!management unit} of 10–50 components. Management must be "dependency-aware" to prevent upstream updates from breaking downstream consumers.
...
* **FinOps for AI**\index{FinOps!AI cost attribution}: GPU compute is the primary cost driver. Effective MLOps requires granular cost attribution (Cost-per-Inference) and opportunistic resource scheduling (Spot instances) to maintain economic viability.
