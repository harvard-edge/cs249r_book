...
from mlsysim.fmt import fmt_count, fmt_time
# ┌─────────────────────────────────────────────────────────────────────────────
...
class DebtPriority:
    ...
    config_savings_str = fmt_count(config_savings, scale='K', precision=0, commas=False)
...
class InfraFrontierTcoClusterRecap:
    ...
    node_price_usd = Infrastructure.Pricing.Capital.DgxH100NodeUsd.rate.m_as("dollar")
...
class BuildVsBuyFigureScenario:
    ...
    cloud_gpu_hour_usd = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as("dollar / hour")
    utilization = 0.80
    horizon_months = 30
...
class TcoScenario:
    ...
    amortization_years = 3
    utilization = 0.80
    electricity_rate = Infrastructure.Pricing.OnPremises.ElectricityPerKwh.rate.m_as("dollar / kilowatt_hour") # dollar/kWh
...
class PlanningThroughputScenario:
    ...
    h100_peak_tflops = h100_peak_flops.m_as(TFLOPs / second)
    ...
    cluster_sustained_pflops = cluster_sustained_flops.m_as(PFLOPs / second)
...
class InfraPlanningExercise:
    ...
    tokens_t = 1
    weeks = 4
    power = 2 * ureg.megawatt
    power_mw = power.m_as(ureg.megawatt)
    ...
    power_mw_str = fmt_qty(power, ureg.megawatt, precision=0, commas=False, unit_label="MW")
...
class CheckpointComputeTradeoff:
    ...
    write_bw_gbs = write_bw.m_as(GB / second)
    ...
    training_state_tb = training_state.m_as(TB)
    t_write_s = (training_state / write_bw).m_as(second)
