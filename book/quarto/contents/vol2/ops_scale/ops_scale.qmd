...
class TcoScenario:
    ...
    # On-prem: CapEx + OpEx
    node_power_kw = (h100_tdp * gpus_per_node).to(kW).magnitude * 1.2 # including system
...
class TenKGpuClusterTco:
    ...
    gpu_tdp_w = gpu_tdp.to(watt).magnitude
...
class PlanningThroughputScenario:
    ...
    h100_peak_tflops = h100_peak_flops.to(TFLOPs / second).magnitude
...
class InfraPlanningExercise:
    ...
    power_mw = power.to(MW).magnitude
...
class CheckpointComputeTradeoff:
    ...
    write_bw_gbs = write_bw.to(GB / second).magnitude
    ...
    training_state_tb = training_state.to(TB).magnitude
    t_write_s = (training_state / write_bw).to(second).magnitude
