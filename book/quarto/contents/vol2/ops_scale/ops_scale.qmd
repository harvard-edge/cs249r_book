...
::: {.column-margin}
![](images/svg/ops_scale_cross_model_blast.svg){width="100%" fig-alt="Blast-radius fan: one red source square on the left, the shared infrastructure fault, with arrows fanning out to five downstream model nodes on the right that all degrade together."}

*One shared-infrastructure fault degrades every model at once.*
:::
...
class GpuEfficiencyTableRecap:
    ...
    b200_fp8_tflops_str = fmt_qty(Hardware.Cloud.B200.compute.precision_flops['fp8'], TFLOP/second, precision=0, commas=False)
    b200_tdp_w_str = fmt_qty(Hardware.Cloud.B200.tdp, watt, precision=0, commas=False)
```
...
class TensorCoreEfficiencyRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp8_tc_tflops_str = fmt_qty(Hardware.Cloud.H100.compute.precision_flops['fp8'], TFLOPs/second, precision=0, commas=True, unit_label="TFLOP/s")
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_qty, fmt, fmt_sci, check, fmt_count, fmt_percent
...
class PlanningThroughputScenario:
    ...
    h100_peak_tflops_str = fmt_qty(h100_peak_flops, TFLOPs / second, precision=0, commas=False, unit_label="TFLOP/s")
    per_gpu_sustained_tflops_str = fmt_qty(per_gpu_sustained_flops, TFLOPs / second, precision=1, commas=False, unit_label="TFLOP/s")
    gpus_str = fmt_count(gpus, label="GPU")
    cluster_sustained_pflops_str = fmt_qty(cluster_sustained_flops, PFLOPs / second, precision=1, commas=False, unit_label="PFLOP/s")
...
class RackPowerPlanningRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    rack_gpu_power = Hardware.Cloud.H100.tdp * Systems.Nodes.DGX_H100.accelerators_per_node * 4
    rack_overhead_power = 10.3 * kilowatt
    rack_power = rack_gpu_power + rack_overhead_power
    rack_power_kw_str = fmt_qty(rack_power, kilowatt, precision=1, commas=False)
```
...
class InfraPlanningExercise:
    ...
    power_mw_str = fmt_qty(power, MW, precision=0, commas=False, unit_label="MW")
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, MarkdownStr, fmt_time, fmt_usd, fmt_int
...
::: {.column-margin}
![](images/svg/vol2_ops_scale_margin_002.svg){width="100%" fig-alt="Margin illustration showing Sample need rises sharply as detectable effect shrinks."}
...
::: {.column-margin}
![](images/svg/vol2_ops_scale_margin_003.svg){width="100%" fig-alt="Margin illustration showing False-alert probability saturates near one as test count grows."}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int,  check, fmt, MarkdownStr, fmt_time
import math
...
::: {.column-margin}
![](images/svg/vol2_ops_scale_margin_004.svg){width="100%" fig-alt="Margin illustration showing Delayed detection window versus automated monitoring alert window."}
