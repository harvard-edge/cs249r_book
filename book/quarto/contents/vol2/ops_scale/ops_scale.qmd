...
**Scenario**: A recommendation service processes incoming user event logs to update real-time feature vectors during an active user session.

**Mechanism**: Feature freshness latency is defined physically by event availability:
$$T_{\text{freshness}} = T_{\text{available}} - T_{\text{event}} = \frac{D_{\text{vol}}}{\text{BW}} + L_{\text{lat}}$$

**Trade-off**:

- **Batch pipeline (daily)**: Events aggregate overnight ($T_{\text{freshness}} \approx 12\text{--}24\text{ h}$), leaving session state static and causing users to abandon sessions before feed updates land.
- **Streaming pipeline (real-time)**: Events stream via Kafka/Flink into an online key-value store ($T_{\text{freshness}} \approx 1\text{--}5\text{ s}$), updating feature vectors before the subsequent request.

**Systems insight**: Replacing batch materialization with streaming ingestion ($T_{\text{freshness}} \approx 5\text{ s}$) yields a 10 to 20 percent lift in session engagement, justifying the added $D_{\text{vol}}/\text{BW}$ ingestion infrastructure costs.
...
| **Electricity**              |      OpEx      | 60--70% of OpEx   | At \$0.07/kWh, a `{python} InfraFrontierTcoClusterRecap.cluster_gpus_str`-GPU H100 cluster consuming 1 MW including cooling at PUE 1.1 costs about \$615,000 per year.      |
...
@Fig-ecommerce-graph traces this blast radius: a single change to the User Embedding model propagates through two direct consumers and four transitive downstream nodes, including the final business rules layer. Operational procedures must therefore address four coordination requirements:
...
**Scenario**: A high-throughput product ranking model ($25,000\text{ QPS}$) passes all offline validation gates ($L_{\text{lat}} < 12\text{ ms}$) but causes a 3.2 percent conversion rate drop post-deployment.

**Mechanism**: An upstream feature pipeline schema change causes a critical user affinity feature to return `null` for 8 percent of incoming traffic. The serving layer's fallback handler imputes `null` as `0.0`—a valid scalar within the model's feature space.

**Impact**: Because imputed values generate valid float outputs without raising exceptions, latency $L_{\text{lat}}$ and HTTP error rates remain nominal, allowing the silent failure to persist unalerted for 48 hours.

**Systems insight**: Syntactically valid feature imputations create semantically silent failures; production data validation gates must evaluate feature distribution shifts ($\Delta D_{\text{vol}}$) alongside system uptime and latency bounds.
...
Formally, standard A/B testing relies on the Stable Unit Treatment Value Assumption (SUTVA): user $i$'s outcome $Y_i(w)$ depends only on their own treatment assignment $w$, not on the treatments assigned to other users [@rubin1980randomization]. This assumption fails systematically in networked products and distributed ML systems, leading to biased effect estimates that can mislead deployment decisions [@hudgens2008toward; @eckles2017design].
...
- **Mechanism** (Direct network effects): User A's treatment directly influences user B's outcome through platform interactions. In a social feed ranking experiment, treatment users may share algorithmically optimized content with control users, biasing measured effects toward zero because control users partially receive the treatment through network propagation.
- **Mechanism** (Indirect network effects): Market-level mechanisms affect all users regardless of their individual treatment assignment. A ride-sharing pricing experiment that increases driver compensation in the treatment group can attract more drivers overall, causing control users to experience shorter wait times and making the measured treatment effect underestimate the true benefit.
- **Mechanism** (Spillover effects): Treatment effects spread across geographic or temporal boundaries. A local recommendation experiment can shift restaurant foot traffic and popularity signals in adjacent neighborhoods, changing recommendation quality for nearby control users.
...
Complementing Z-score analysis, **percentage change**\index{Percentage Change} detection captures sudden shifts regardless of historical variance, which makes it useful for catching step-function increases such as a misconfigured autoscaler that doubles instance count overnight:
...
Effective alerting requires calibrating thresholds to balance detection sensitivity against operational burden. Two common configurations are a **Z-score threshold**\index{Z-Score Threshold}, which alerts when $|Z| > 3$ under the 3-sigma rule, and a **percentage change threshold**\index{Percentage Change Threshold}, which alerts when $|\Delta\%| > 50\%$ day over day. The choice of thresholds determines false positive rates. For a normally distributed cost metric checked daily, a 3-sigma threshold produces:
...
**Impact**: Ingestion delays $D_{\text{vol}}/\text{BW}$ degrade click-through rates (CTR) by 5 to 15 percent, imposing a severe revenue penalty ("staleness tax").
...
**Context**: Meta's global infrastructure relied on a high-capacity backbone network connecting data centers globally, carrying internal control traffic, DNS requests, and synchronization signals across thousands of model-serving instances [@meta2021outage].

**Failure mode**: On October 4, 2021, a routine audit command intended to evaluate backbone capacity unintentionally severed all backbone links. This triggered an automated BGP routing withdrawal, dropping all authoritative DNS routes and driving communication latency $T_{\text{comm}} \to \infty$, which broke internal authentication and out-of-band management tools required for remote remediation.

**Consequence**: Facebook, Instagram, and WhatsApp experienced a 5.5-hour global outage affecting over 3.5 billion users, while synchronization time $T_{\text{sync}}$ for model-serving states collapsed due to unreachable feature stores and control registries.

**Systems lesson**: Platform-scale operations must treat the control plane as a hard dependency with a defined blast radius. ML platforms present identical failure risks at fleet scale: training schedulers, model registries, feature stores, and inference routers share the underlying auth, DNS, and network planes. When network partitions force $T_{\text{comm}} \to \infty$, recovery requires strict physical and logical isolation between the control plane and inference serving infrastructure.
...
**Scenario**: A core platform team alters the transformation logic of a shared `user_engagement_score` feature, changing its normalization from a 30-day z-score to a 7-day min-max scaling to capture rapid trend shifts.

**Mechanism**: The update mutates the shared feature store table in-place. Fourteen downstream models across five engineering teams consume this feature as an implicit interface without explicit schema versioning.

**Impact**: In-place feature mutation invalidates model expectations across all 14 consumers, causing up to a 12 percent drop in ranking accuracy and inducing 18 hours of cross-team debugging toil before the shared root cause is identified.

**Systems lesson**: Shared feature definitions are production interfaces that mandate immutable versioning (e.g., `user_engagement_score_v2`). In-place schema mutations break synchronization $T_{\text{sync}}$ across dependent model fleets.
