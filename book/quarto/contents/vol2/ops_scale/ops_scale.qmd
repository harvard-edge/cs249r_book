...
<!-- TODO(focused-followup): Items deferred from the codex deep-review
     ...
     - Inference-cost equation (line ~3265): the `24 x 365` factor
       double-counts time relative to the preceding Q_daily * L_avg
       term, overstating annual serving cost by 24x. Recheck dimensional
       analysis: either remove the `24` factor or redefine `Q` and
       `L_avg` with explicit units.
     - 3-sigma alerts weekly vs. daily mismatch (line ~2193): same setup
       yields different cadences; harmonize and distinguish once-per-day
       control checks from five-minute monitoring if both are intentional.
     - Non-protected callout-notebook second-person + Qs at lines 326,
       600, 725, 1396, 1493, 2401; convert similarly to line 152
       (already fixed).
     - Promotional register at line ~161 ("Sharing Dividend",
       "Infrastructure Multiplier", "for free", "secret"); rewrite in
       neutral systems prose.
     - Bold paragraph-starter callout at line ~3456 duplicating
       preceding sentence; merge or add distinct content.
     - Organizational-pattern bold-starter list at line ~3841
       ("Consistency", "Efficiency", "Bottleneck risk"); convert to
       table or connected paragraphs.
     - Organizational-pattern figure caption (line ~3768): make panel
       labels explicit between caption, alt text, and figure.
     - Worked example after selection table (line ~3933) redundant with
       preceding criteria; condense or fold into selection discussion.
     - Case-study overclaims at line ~3964 (Uber, Vertex AI, Netflix,
       Spotify): tighten to cited sources or recast as illustrative.
     - Netflix interleaving "dramatic" intensifier (line ~3984); replace
       with concise mechanism statement. -->
...
The severity of network effect bias depends on network structure and outcome correlation. For cluster-randomized experiments, a common design-effect approximation quantifies how within-cluster correlation inflates the variance of treatment effect estimates, directly determining the sample size increase required for equivalent statistical power:
