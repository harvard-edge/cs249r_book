...
Just as a single developer can compile code manually, a single data scientist can train a model manually. But compiling an operating system requires a build system (Make/Bazel). Similarly, training a fleet of models requires an **ML Build System**—a platform that manages the dependencies, compilation steps (training), and linking (serving) of the entire diverse codebase of "Data." This chapter is about building that Make/Bazel for AI.
