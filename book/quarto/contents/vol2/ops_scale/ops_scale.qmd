...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ PLATFORM ROI CALCULATION (P.I.C.O.)
# ├─────────────────────────────────────────────────────────────────────────────
...
$$C_{current} = 50 \times 40 \times 12 \times \$150 = \$`{python} annual_cost_before_str`$$

After platform (annual operational cost plus amortized platform cost):
$$C_{after} = 50 \times 10 \times 12 \times \$150 + \frac{\$2,000,000}{3} = \$900,000 + \$667,000 = \$`{python} annual_cost_after_str`$$
...
Capacity planning for large-scale ML is fundamentally an exercise in optimizing the economics of the GPU-hour. For our running example, a 30-day training run on a 256-node H100 cluster (2,048 GPUs) at a market rate of \$2.00 per GPU-hour represents a direct investment of roughly \$2.95 million. At this scale, the cost per training run ($C_{run}$) becomes a primary design lever that dictates the sizing of the entire fleet. We must balance the desire for faster time-to-market (scaling to more GPUs) against the diminishing returns of large-scale parallelism and the increased probability of hardware failure. Total cost is not merely a function of compute time; it must account for the overhead of data staging, checkpointing, and the inevitable cost of recovery from interruptions (@sec-fault-tolerance-reliability).
...
[^fn-ab-testing-power]: **A/B Testing Power**: The probability that an experiment correctly detects a true effect (-\beta$, typically set to 0.8). At fleet scale, the systems challenge is **Statistical Power**: if the expected improvement is small (e.g., 0.1%), the experiment may require millions of samples to reach statistical significance, consuming substantial serving resources ($C_{infer}$) for weeks. \index{A/B Testing!statistical power}
...
Batch pipelines with daily feature updates ignore the quantitative impact of staleness. The freshness formula $L_{freshness} = T_{available} - T_{event}$ shows daily batch processing yields $L_{freshness} \approx 12$-$24$ hours; streaming pipelines achieve $L_{freshness} \approx 1$-$5$ seconds. As @sec-ml-operations-scale-feature-store-operations-2a36 demonstrates, this improvement delivers 10-20% engagement lift, and for fraud detection, day-old features give adversaries a 24-hour exploitation window. A recommendation platform generating \$15M weekly with 10% from ML that improves 15% with real-time features gains \$225K weekly, easily covering streaming infrastructure costs.
