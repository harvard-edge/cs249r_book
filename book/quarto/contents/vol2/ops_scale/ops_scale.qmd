...
```{python}
#| label: fig-n-models-complexity
#| fig-cap: "The N-Models Complexity Explosion. Monitoring alerts grow linearly with model count, deployment coordination grows as O(N log N), and dependency conflicts grow quadratically as models share features and data sources. The total operational load crosses team capacity around 50 models, marking the transition from artisanal model management to platform-required operations."
...
```{python}
#| label: fig-platform-roi-threshold
#| fig-cap: "**The Platform ROI Threshold**. Platform return on investment as a function of model count, for two platform cost levels. A $2M/year platform breaks even at approximately 20 models, while a $5M/year enterprise platform requires roughly 50 models. Beyond break-even, ROI grows linearly — at 100 models, the $2M platform delivers 5x return."
#| fig-alt: "Line chart showing platform ROI versus model count with break-even points for two cost levels"
#| echo: false
import numpy as np
import matplotlib.pyplot as plt
...
***FinOps (Financial Operations) for ML***\index{FinOps!definition} is the discipline of treating ML compute cost as a first-class optimization variable alongside model accuracy and latency. FinOps bridges the fundamental tension between engineering velocity, which favors unlimited experimentation, and financial discipline, which demands accountability for every GPU-hour. By establishing visibility into spending, governance over resource allocation, and optimization feedback loops, FinOps enables teams to make cost-aware decisions without sacrificing model quality or development velocity.
...
***Total Cost of Ownership (TCO) for ML Systems***\index{Total Cost of Ownership!definition} is the complete economic accounting of developing, deploying, and operating machine learning capabilities across their full lifecycle. Unlike traditional IT systems where capital expenditure on hardware dominates, ML TCO is uniquely shaped by *inference compute* that grows with user adoption, making the serving phase the primary cost driver rather than development. This inversion means that a model's true cost is determined not by how expensive it was to train, but by how many users it serves and for how long.
...
```{python}
#| label: fig-tco-iceberg
#| fig-cap: "The TCO Iceberg: Total Cost of Ownership analysis for ML systems. While GPU compute and storage are the visible costs, the hidden operational costs---including engineering labor, maintenance, and compliance---often constitute the majority of the actual budget."
...
***Training-Serving Skew***\index{Training-Serving Skew!definition} is the divergence between the feature processing logic or data distribution used during model training and that used during inference (serving). It is a primary cause of silent model failure in production, often driven by code divergence between batch and real-time pipelines.
