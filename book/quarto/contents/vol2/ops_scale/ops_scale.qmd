...
n_monitors_fat_str = FalseAlarmTax.n_monitors_str
checks_per_day_fat_str = FalseAlarmTax.checks_per_day_str
check_interval_fat_str = FalseAlarmTax.check_interval_min_str
daily_alerts_fat_str = FalseAlarmTax.daily_alerts_str
specificity_val_fat_str = FalseAlarmTax.specificity_val_str
fpr_fat_str = FalseAlarmTax.fpr_str
fpr_pct_fat_str = FalseAlarmTax.fpr_pct_str
```
...
1.  **Total Monitors**: `{python} n_models_fat` models $\times$ `{python} n_metrics_fat` metrics = **`{python} n_monitors_fat_str` monitors**.
2.  **False Positive Rate**: $1 - `{python} specificity_val_fat_str` = `{python} fpr_fat_str`$ (`{python} fpr_pct_fat_str`%).
3.  **Checks per Day**: Assume checks every `{python} check_interval_fat_str` minutes (**`{python} checks_per_day_fat_str` checks/day**).
4.  **Daily False Alarms**: $`{python} n_monitors_fat_str` \times `{python} checks_per_day_fat_str` \times `{python} fpr_fat_str` \approx \mathbf{`{python} daily_alerts_fat_str` alerts/day}$.
