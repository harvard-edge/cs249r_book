...
Rollback should be automated based on metric degradation, as configured in @lst-rollback-triggers.\index{Automated Rollback Configuration}
...
A well-designed deployment API abstracts operational complexity. @lst-deployment-api illustrates a declarative specification that covers model selection, resource allocation, traffic management, and monitoring in a single configuration.\index{Declarative Deployment API}
...
Profiling across all ranks identifies stragglers, as shown in @lst-distributed-profiling.\index{Distributed Profiling}
