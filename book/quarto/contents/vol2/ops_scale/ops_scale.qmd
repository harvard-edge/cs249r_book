...
Feature stores implement **point-in-time joins**\index{Point-in-Time Join} that retrieve feature values as of specific timestamps, as shown in @lst-pit-joins.
...
OOM errors require the same per-rank view because aggregate memory usage hides the device that actually crossed the limit. @Lst-memory-debugging tracks memory across devices.
