...
annual_cost_before_str = PlatformEconomics.annual_cost_before_str
annual_cost_after_str = PlatformEconomics.annual_cost_after_str
```
...
$$C_{\text{current}} = 50 \times 40 \times 12 \times \$150 = \$`{python} annual_cost_before_str`$$

After platform (annual operational cost plus amortized platform cost):
$$C_{\text{after}} = 50 \times 10 \times 12 \times \$150 + \frac{\$2,000,000}{3} = \$900,000 + \$667,000 = \$`{python} annual_cost_after_str`$$
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  % Axes
  \draw[axis] (0,0) -- (8,0) node[right] {Maturity};
  \draw[axis] (0,0) -- (0,5) node[above] {Scale / Automation};

  % Steps
  \node[step box] (l0) at (0,0) {Level 0\\Manual};
  \node[step box, minimum height=2cm, right=of l0.south east] (l1) {Level 1\\Per-Model CI/CD};
  \node[step box, minimum height=3cm, right=of l1.south east] (l2) {Level 2\\Platform Ops};
  \node[step box, minimum height=4cm, right=of l2.south east] (l3) {Level 3\\Enterprise Gov};

  % Annotations
  \node[annotation, below=2mm of l0.south] {1-2 Models};
  \node[annotation, below=2mm of l1.south] {10-20 Models};
  \node[annotation, below=2mm of l2.south] {50-200 Models};
  \node[annotation, below=2mm of l3.south] {500+ Models};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  % Alert Overlay
  \node[alert box, fit=(Retrieval) (Ensemble)] (AlertBox) {};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \node[source, right=3cm of User] (Prod) {Product Data};

  % L1
  \node[l1, below=1cm of User] (UserEmb) {User\\Embeddings};
  \node[l1, below=1cm of Prod] (ProdEmb) {Product\\Embeddings};

  % L2
  \node[l2, below right=1cm and 0.5cm of UserEmb] (Retrieval) {Candidate\\Retrieval};
  \node[l2, right=2cm of Retrieval] (Price) {Price\\Sensitivity};

  % L3
  \node[l3, below right=1cm and 0cm of Retrieval, xshift=-1cm] (Ranking) {Ranking\\Model};

  % L4
  \node[l4, below=1cm of Ranking] (Logic) {Business Rules\\(Diversity)};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \node[gate, right=0.5cm of Data] (Gate1) {Schema\\Check};
  \node[stage, right=0.5cm of Gate1] (Train) {Training};
  \node[gate, right=0.5cm of Train] (Gate2) {Loss\\Check};
  \node[stage, right=0.5cm of Gate2] (Eval) {Evaluation};
  \node[gate, right=0.5cm of Eval] (Gate3) {Metric\\Check};
  \node[stage, right=0.5cm of Gate3] (Reg) {Registry};
  \node[gate, right=0.5cm of Reg] (Gate4) {Approval};
  \node[stage, right=0.5cm of Gate4] (Deploy) {Canary};
  ...
  % Feedback
  \draw[edge, dashed, RedLine] (Gate3) to[bend left=30] node[midway, above, font=\scriptsize] {Fail} (Train);
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \tikzset{
    ...
    edge/.style={->, line width=1.0pt},
    compare/.style={draw, dashed, inner sep=5pt, line width=0.75pt}
  }

  % Router
  \node[router] (Router) {Router};
  \node[left=1cm of Router] (Request) {Request};
  \draw[edge] (Request) -- (Router);

  % Production Path
  \node[model, fill=ProdColor, above right=0.5cm and 1.5cm of Router] (Prod) {Production Model};
  \draw[edge] (Router) |- (Prod);
  \node[right=1cm of Prod] (ProdResp) {Response (User)};
  \draw[edge] (Prod) -- (ProdResp);

  % Shadow Path
  \node[model, fill=ShadowColor, below right=0.5cm and 1.5cm of Router] (Shadow) {Shadow Model};
  \draw[edge, dashed] (Router) |- (Shadow);
  \node[right=1cm of Shadow] (ShadowResp) {Log (No User)};
  \draw[edge, dashed] (Shadow) -- (ShadowResp);

  % Async Comparison
  \node[compare, right=1.5cm of Router] (Compare) {Compare};
  \draw[edge, dotted] (Prod) -- (Compare);
  \draw[edge, dotted] (Shadow) -- (Compare);
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \tikzset{
    ...
    mixed box/.style={draw, fill=black!5, minimum width=2.2cm, minimum height=2cm, line width=0.75pt}
  }

  \begin{scope}[local bounding box=ab]
    % A/B Testing
    \node[title] (ABTitle) {A/B Testing};
    \node[below left=0.5cm and 0.5cm of ABTitle] (UserA) {User A};
    \node[below right=0.5cm and 0.5cm of ABTitle] (UserB) {User B};

    \node[item, fill=AColor, below=0.2cm of UserA] (ListA) {Model A List};
    \node[item, fill=BColor, below=0.2cm of UserB] (ListB) {Model B List};

    \node[font=\scriptsize, text=black!60, below=1cm of ABTitle] (ABDesc) {Split Traffic (Low Sensitivity)};
  \end{scope}
  ...
  \begin{scope}[local bounding box=interleave, xshift=7cm]
    ...
    \node[mixed box, below=1.5cm of AnyUser] (MixedBox) {};
    \node[anchor=north] at (MixedBox.north) {Mixed List};
    ...
    \node[font=\scriptsize, text=black!60, below=0.2cm of MixedBox] (IntDesc) {Blended (High Sensitivity)};
  \end{scope}
...
# │
# │ Imports: (none)
# │ Exports: n_models_fat, n_metrics_fat, specificity_fat, daily_alerts_fat_str
# └─────────────────────────────────────────────────────────────────────────────
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
n_models_fat = FalseAlarmTax.n_models
n_metrics_fat = FalseAlarmTax.n_metrics
specificity_fat = "3--sigma"
daily_alerts_fat_str = FalseAlarmTax.daily_alerts_str
```
...
1.  **Total Monitors**: `{python} n_models_fat` models $\times$ `{python} n_metrics_fat` metrics = **1,000 monitors**.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \definecolor{RedLine}{HTML}{DC3545}
  \definecolor{OrangeLine}{HTML}{FD7E14}

  \tikzset{
    pyramid/.style={draw, line width=0.75pt}
  }
  ...
  % Pyramid Layers
  \draw[pyramid, fill=L4Color] (A) -- (B) -- (C) -- cycle; % Base
  \draw[pyramid, fill=L3Color] (0.67,1) -- (7.33,1) -- (C) -- cycle;
  \draw[pyramid, fill=L2Color] (1.33,2) -- (6.67,2) -- (C) -- cycle;
  \draw[pyramid, fill=L1Color] (2.0,3) -- (6.0,3) -- (C) -- cycle;

  % Labels
  \node[align=center] at (4, 4) {\textbf{Business Metrics}\\(Revenue, Engagement)};
  \node[align=center] at (4, 2.5) {\textbf{Portfolio Metrics}\\(Search, Ads, RecSys)};
  \node[align=center] at (4, 1.5) {\textbf{Model Metrics}\\(Latency, Accuracy, Drift)};
  \node[align=center] at (4, 0.5) {\textbf{Infrastructure Metrics}\\(GPU Util, Network)};

  % Annotations
  \node[anchor=west, text=RedLine, font=\scriptsize] at (5.5, 4) {Alert Execs};
  \node[anchor=west, text=OrangeLine, font=\scriptsize] at (6.5, 2.5) {Alert Product Owners};
  \node[anchor=west, text=black!60, font=\scriptsize] at (7.2, 1.5) {Alert Model Owners};
  \node[anchor=west, text=black!60, font=\scriptsize] at (7.8, 0.5) {Alert Platform Team};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  % Timeline
  \draw[timeline] (0, 0) -- (10, 0) node[below] {Time applied};
  ...
  % Actual Clicks
  \draw[path, BlueLine] (1, 0.1) -- (5, 0.1) node[midway, above, font=\scriptsize] {Clicks: 4};
  \draw[path, BlueLine] (5, 0.1) -- (9, 0.1) node[midway, above, font=\scriptsize] {Clicks: +6};

  % Event
  \draw[event] (5, -1.5) -- (5, 1.5);
  \node[RedLine, font=\bfseries, rotate=90, anchor=south] at (5, 0.5) {Inference Event};

  % Leakage Path
  \draw[path, RedLine, ->] (9, 0.5) to[bend right=45] node[midway, above, font=\footnotesize] {Time Travel (Leakage)} (5.1, 0.5);
  \node[RedLine, align=center, font=\footnotesize, above=1cm of {(8,0)}] {Batch Job @ Midnight\\Total: 10};

  % Correct Path
  \draw[path, GreenLine, ->] (4.9, 0.5) -- (5, 0.5);
  \node[GreenLine, align=center, font=\footnotesize, left=0.1cm of {(5,0.5)}] {Point-in-Time\\Total: 4};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \definecolor{RedLine}{HTML}{DC3545}
  \definecolor{GreenLine}{HTML}{28A745}
  ...
  \begin{scope}[local bounding box=central]
    ...
    \node[box, fill=ModelColor, below left=1cm and -0.5cm of P1] (M1a) {M1};
    \node[box, fill=ModelColor, right=0.2cm of M1a] (M1b) {M2};
    \node[box, fill=ModelColor, right=0.2cm of M1b] (M1c) {M3};

    \draw[edge] (M1a) -- (P1);
    \draw[edge] (M1b) -- (P1);
    \draw[edge] (M1c) -- (P1);

    \node[font=\scriptsize, text=RedLine, below=0.5cm of M1b] {Risk: Bottleneck};
  \end{scope}

  \begin{scope}[local bounding box=embedded, xshift=5cm]
    % Embedded
    \node[title] (EmbTitle) {Embedded};

    \node[box, fill=ModelColor, below=0.5cm of EmbTitle, xshift=-1.5cm] (M2a) {M1};
    \node[box, fill=PlatformColor, minimum width=0.5cm, right=0cm of M2a] (P2a) {P};

    \node[box, fill=ModelColor, below=1.5cm of EmbTitle] (M2b) {M2};
    \node[box, fill=PlatformColor, minimum width=0.5cm, right=0cm of M2b] (P2b) {P};

    \node[box, fill=ModelColor, below=0.5cm of EmbTitle, xshift=1.5cm] (M2c) {M3};
    \node[box, fill=PlatformColor, minimum width=0.5cm, right=0cm of M2c] (P2c) {P};

    \node[font=\scriptsize, text=RedLine, below=0.5cm of M2b] {Risk: Fragmentation};
  \end{scope}

  \begin{scope}[local bounding box=hybrid, xshift=10cm]
    % Hybrid
    \node[title] (HybTitle) {Hybrid};
    \node[box, fill=PlatformColor, minimum width=3cm, below=0.5cm of HybTitle] (P3) {Core Platform};

    \node[box, fill=ModelColor, below left=1cm and -0.5cm of P3] (M3a) {M1};
    \node[box, fill=PlatformColor, minimum width=0.5cm, right=0cm of M3a] (P3a) {P};

    \node[box, fill=ModelColor, below right=1cm and -0.5cm of P3] (M3b) {M2};
    \node[box, fill=PlatformColor, minimum width=0.5cm, right=0cm of M3b] (P3b) {P};

    \draw[edge, dashed] (P3a) -- (P3);
    \draw[edge, dashed] (P3b) -- (P3);

    \node[font=\scriptsize, text=GreenLine, below=0.5cm of M3a, xshift=1cm] {Balanced};
  \end{scope}
