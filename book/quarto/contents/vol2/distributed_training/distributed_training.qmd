...
- **Linear throughput scaling**: Scaling ResNet-50 on ImageNet from 1 to 256 GPUs yields near-linear speedup for compute-bound models, because the gradient exchange is small relative to the compute time.
- **Unmodified architecture**: The framework wraps the model in a data-parallel container that intercepts backward-pass hooks to trigger gradient synchronization automatically.
- **High utilization**: Unlike model parallelism, there are no pipeline bubbles—all GPUs work on the forward and backward pass simultaneously.

Three hard ceilings limit these advantages:

- **The memory wall**\index{Memory Wall!data parallelism}: Every GPU must hold a full copy of the model parameters, gradients, and optimizer states; for a `{python} FrontierTrainingContext.frontier_params_b_str` parameter model, this demands more than 1 TB of memory per GPU, exceeding per-device HBM budgets without ZeRO sharding.
- **The bandwidth wall**: As $N$ grows, the AllReduce cost $\frac{2(N-1)}{N} \times \frac{M}{\text{BW}_{\text{net}}}$ eventually dominates, and for large language models gradient synchronization can consume more than 50 percent of the step time, collapsing efficiency.
- **The batch size trap**: Scaling to thousands of GPUs requires increasing the global batch size $(B_{\text{global}} = N \times B_{\text{local}})$, and eventually the critical batch size is reached, where adding more data per step yields diminishing returns in convergence.
...
The operational guarantees of BSP, SSP, and ASP established in @sec-distributed-training-systems-systems-synchronization-models-396f have a convergence cost, which this section sketches with stylized smooth-objective bounds. The staleness parameter $\tau_{\text{stale}}$ quantifies how many iterations behind a gradient may be when applied to parameters, and it is the variable that ties throughput to solution quality [@ho2013more; @dutta2018slow].
...
The second term represents the **staleness penalty**\index{Staleness Penalty}. For bounded staleness $s$, this penalty is controlled by keeping delay small and retuning the learning rate. Deployments that choose bounded staleness accept convergence degradation for throughput improvement on heterogeneous clusters.
...
Published systems such as Llama 3 405B use TP=8 within NVLink-connected H100 nodes [@dubey2024llama], and similar node-local tensor parallelism is a common design pattern for large LLM training [@jiang2024megascale].
...
- **Memory scaling**: This enables training of models that exceed single-device capacity; with 8-way tensor parallelism, the FP16 weight slice of a `{python} ModelParallelTradeoffsRecap.frontier_params_b_str` model fits within A100 HBM before optimizer state and activation budgets are counted.
- **Larger global batch sizes**: Splitting the model allows larger global batch sizes without out-of-memory errors, since each GPU processes a smaller parameter slice.
- **Natural topological mapping**: The approach maps directly to the physical structure of transformers, where attention heads split via tensor parallelism and layers split via pipeline parallelism.

The advantages come at the cost of three fundamental limitations:

- **Pipeline bubbles**: GPUs sit idle while filling and draining the pipeline; the bubble fraction is approximately $(p-1)/(m+p-1)$, where $p$ is pipeline stages and $m$ is microbatches, and achieving more than 90 percent efficiency requires $m \gg p$, which increases activation memory.
- **Communication intensity**: In tensor parallelism, two AllReduce operations execute per layer on the critical path, demanding extremely high-bandwidth, low-latency interconnects (NVLink) and typically preventing scaling beyond a single node (8 GPUs) before hitting the bandwidth wall.
- **Implementation complexity**: The strategy requires invasive changes to the model definition—replacing standard linear layers with column-parallel and row-parallel variants—unlike data parallelism, which wraps the model externally without modifying internals.
...
[^fn-critical-batch-size]: **Critical Batch Size**\index{Critical Batch Size!convergence limit}: The gradient-noise-scale view introduced by McCandlish et al. treats the largest useful batch size as a measurable statistic that varies by domain, model, optimizer, and training phase [@mccandlish2018empirical]. Scaling beyond that measured point improves hardware throughput but does not proportionally reduce the number of optimization steps needed to reach target quality, collapsing the scaling efficiency $(\eta_{\text{scaling}})$.
