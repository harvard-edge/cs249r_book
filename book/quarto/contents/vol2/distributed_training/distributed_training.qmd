...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int,  fmt_percent, fmt, check, MarkdownStr, fmt_time
import math

class DistTrainReliabilityFacts:
    # ┌── 1. LOAD (Constants) ──────────────────────────────
    a100_mem_gb = Hardware.Cloud.A100.memory.capacity.to(GiB).magnitude
    h100_mem_gb = Hardware.Cloud.H100.memory.capacity.to(GiB).magnitude
    nvlink_a100_gbs = Hardware.Cloud.A100.nvlink.bandwidth.to(GB/second).magnitude
    nvlink_h100_gbs = Hardware.Cloud.H100.nvlink.bandwidth.to(GB/second).magnitude
    ib_hdr_gbs = Systems.Fabrics.InfiniBand_HDR.bandwidth.to(GB/second).magnitude  # 200 Gbps -> 25 GB/s per port
    ib_ndr_gbs = Systems.Fabrics.InfiniBand_NDR.bandwidth.to(GB/second).magnitude  # 400 Gbps -> 50 GB/s per port
...
class DistTrainNvlinkIntro:
    ...
    nvlink_a100_gbs = Hardware.Cloud.A100.nvlink.bandwidth.to(GB/second).magnitude
    nvlink_h100_gbs = Hardware.Cloud.H100.nvlink.bandwidth.to(GB/second).magnitude
    ib_hdr_gbs = Systems.Fabrics.InfiniBand_HDR.bandwidth.to(GB/second).magnitude
    nvlink_hdr_ratio_low = nvlink_a100_gbs / ib_hdr_gbs
    nvlink_hdr_ratio_high = nvlink_h100_gbs / ib_hdr_gbs
    check(20 < nvlink_hdr_ratio_low < 30, "A100 NVLink/HDR ratio should be about 24x")
    nvlink_a100_str = fmt_qty(nvlink_a100_gbs * GB / second, GB / second, precision=0, commas=False)
    nvlink_h100_str = fmt_qty(nvlink_h100_gbs * GB / second, GB / second, precision=0, commas=False)
...
class HierarchicalAllReduceDebug:
    ...
    flat_ring_ms = t_flat.to(ms).magnitude
    ...
    hierarchical_total_ms = t_hier.to(ms).magnitude
    ...
    inter_node_ms = t_inter.to(ms).magnitude
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    flat_ring_ms_str = fmt_time(flat_ring_ms, 'millisecond', precision=1, commas=False)
    inter_node_ms_str = fmt_time(inter_node_ms, 'millisecond', precision=1, commas=False)
    hierarchical_total_ms_str = fmt_time(hierarchical_total_ms, 'millisecond', precision=1, commas=False)
    observed_ms_str = fmt_time(observed_ms, 'millisecond', precision=0, commas=False)
    residual_ms_str = fmt_time(residual_ms, 'millisecond', precision=1, commas=False)
    nvlink_a100_str = fmt_qty(nvlink_bw, GB/second, precision=0, commas=False)
    tensor_gb_str = fmt_qty(tensor_bytes, GB, precision=0, commas=False)
    ...
    ib_hdr_bw_str = fmt_qty(ib_hdr_bw, GB/second, precision=0, commas=False)
    flat_single_link_ms_str = fmt_time((tensor_bytes.to(GB).magnitude / ib_hdr_bw.to(GB/second).magnitude) * THOUSAND, 'millisecond', precision=0, commas=False)
...
class Scaling8GPU:
    ...
    gradient_tensor = Models.Language.GPT2.parameters.to(param).magnitude * fp16_gradient_bytes * ureg.byte
    t_comm = calc_ring_allreduce_time(gradient_tensor, gpus_per_node, nvlink_bw, 0*ureg.ms)
    comm_8gpu_ms_val = t_comm.to(ms).magnitude

    sync_size_gb = (gradient_tensor * (2 * (gpus_per_node - 1) / gpus_per_node)).to(GB).magnitude
    ...
    gpt2_sync_size_gb_str = fmt_qty(sync_size_gb * GB, GB, precision=1, commas=False)
    nvlink_h100_str = fmt_qty(nvlink_bw, GB/second, precision=0, commas=False)
    ...
    fits_mem_gb_str = fmt_qty(fits_mem_gb * GB, GB, precision=0, commas=False)
```
...
class Scaling32GPU:
    ...
    net_bw = NETWORK_10G_BW.to(GB/second).magnitude
    ...
    ib_hdr_str = fmt_qty(net_bw * GB / second, GB / second, precision=2, commas=False)
...
class DistTrainA100Memory:
    ...
    activation_headroom_gb_str = fmt_qty_int(round(activation_headroom_gb) * GB, GB, commas=False)
    a100_mem_str = fmt_qty(a100_mem_gb * GiB, GiB, precision=0, commas=False, unit_label="GB")
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_math, fmt_time

class CommScalingMath:
    """Formatted math strings for scaling efficiency and communication overhead analysis."""

    _ib_gbs = fmt(Systems.Fabrics.InfiniBand_NDR.bandwidth.to(GB/second).magnitude, precision=0, commas=False)
    _overlap_fraction = 0.75
    _gradient_size_gb = (Models.Language.GPT3.parameters.to(param).magnitude * BYTES_FP16.to(byte).magnitude * byte).to(GB).magnitude
    ...
    gpt3_params_b_str = fmt(Models.Language.GPT3.parameters.to(param).magnitude / BILLION, precision=0, commas=False)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gradient_gb_str = fmt_qty(_gradient_size_gb * GB, GB, precision=0, commas=False)
...
class ModelParallelMemoryFacts:
    # ┌── 1. LOAD ─────────────────────────────────────────
    params_b = Models.Language.GPT3.parameters.to(Bparam).magnitude
    n_gpus = 64
    bytes_weights = 2
    bytes_gradients = 2
    bytes_adam_state = 12
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    weight_gb_str = fmt_qty(weight_gb * GB, GB, precision=0, commas=False)
    weight_per_gpu_str = fmt_qty(weight_per_gpu_gb * GB, GB, precision=1, commas=False)
    full_state_gb_str = fmt_qty(full_state_gb * GB, GB, precision=0, commas=True)
    full_state_per_gpu_str = fmt_qty(full_state_per_gpu_gb * GB, GB, precision=1, commas=False)
```
...
class A100CapacityContext:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_str = fmt_qty(mem_gb * GiB, GiB, precision=0, commas=False, unit_label="GB")

class MemoryWallAnalysis:
    """ZeRO-3 memory wall analysis for 175B model on A100s."""

    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    params_b = Models.Language.GPT3.parameters.to(Bparam).magnitude  # GPT-3 175B
    bytes_weights = 2
    bytes_gradients = 2
    bytes_adam_state = 12
    n_gpus = 64
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    param_gb_str = fmt_qty(param_gb * GB, GB, precision=0, commas=False)
    gradient_gb_str = fmt_qty(gradient_gb * GB, GB, precision=0, commas=False)
    optim_gb_str = fmt_qty(optim_gb * GB, GB, precision=0, commas=True)
    total_static_gb_str = fmt_qty(total_static_gb * GB, GB, precision=0, commas=True)
    per_gpu_str = fmt_qty(per_gpu_static * GB, GB, precision=1, commas=False)
    activation_str = fmt_qty(activation_approx_gb * GB, GB, precision=0, commas=False)
    total_per_gpu_str = fmt_qty(total_per_gpu * GB, GB, precision=1, commas=False)
...
Consider our running example: the `{python} FrontierModelParallelPipelineRecap.frontier_params_b_str`B parameter model requires 350 GB of memory in FP16, exceeding the `{python} A100CapacityContext.a100_mem_str` capacity of a single A100 by a factor of four. Model parallelism addresses this **capacity wall**\index{Capacity Wall} by partitioning model weights across multiple devices, effectively stitching them into a single super-accelerator. Unlike data parallelism, where every GPU holds a full replica of the model and processes a unique fraction of the global batch, model parallelism requires each GPU to hold a *unique* fraction of the model and process the *same* data stream sequentially. With 8-way partitioning on A100s, BF16 weights alone occupy approximately 44 GB per GPU. That is only the weights budget: full training still needs activations, gradients, and optimizer state, so pure 8-way model parallelism is not sufficient by itself. Training requires additional sharding or offload for optimizer and gradient state, plus pipeline or hybrid parallelism to keep activation memory within capacity.
...
class TensorParallelBandwidth:
    ...
    ib_ndr_str = fmt_qty(Systems.Fabrics.InfiniBand_NDR.bandwidth, GB/second, precision=0, commas=False)
    pipeline_ib_lo_gbs = 25
    pipeline_ib_hi_gbs = 50
    pipeline_ib_lo_str = fmt_qty(pipeline_ib_lo_gbs * GB / second, GB / second, precision=0, commas=False)
    pipeline_ib_hi_str = fmt_qty(pipeline_ib_hi_gbs * GB / second, GB / second, precision=0, commas=False)
```
...
class FlashAttentionOverlapScenario:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    hbm_bw = hbm.to(TB/second).magnitude
    nvlink_bw = nvlink.to(GB/second).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_hbm_bw_str = fmt_qty(hbm_bw * TB / second, TB / second, precision=2, commas=False)
    nvlink_h100_str = fmt_qty(nvlink_bw * GB / second, GB / second, precision=0, commas=False)
```
...
class TensorParallel3DTraffic:
    ...
    activation_payload_mb_str = fmt_qty((activation_payload_bytes / MILLION) * MB, MB, precision=1, commas=False)
    full_forward_gb_str = fmt_qty(full_forward_gb * GB, GB, precision=1, commas=False)
    per_stage_forward_gb_str = fmt_qty(per_stage_forward_gb * GB, GB, precision=1, commas=False)
    per_stage_forward_backward_gb_str = fmt_qty(per_stage_forward_backward_gb * GB, GB, precision=1, commas=False)
```
...
class HybridMemoryAccounting:
    ...
    simplified_optimizer_bytes_per_param = 2 * BYTES_FP32.to(byte).magnitude
    full_adam_bytes_per_param = 3 * BYTES_FP32.to(byte).magnitude

    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    weights_gb = (params * BYTES_FP16.to(byte).magnitude * byte).to(GB).magnitude
    optimizer_simplified_gb = (params * simplified_optimizer_bytes_per_param * byte).to(GB).magnitude
    optimizer_full_adam_gb = (params * full_adam_bytes_per_param * byte).to(GB).magnitude
    ...
    weight_tp_slice_gb_str = fmt_qty((weights_gb / tensor_parallel) * GB, GB, precision=2, commas=False)
    optimizer_simplified_tb_str = fmt_qty((optimizer_simplified_gb / THOUSAND) * TB, TB, precision=1, commas=False)
    optimizer_full_adam_tb_str = fmt_qty((optimizer_full_adam_gb / THOUSAND) * TB, TB, precision=1, commas=False)
    tp_only_static_gb_str = fmt_qty(tp_only_static_gb * GB, GB, precision=1, commas=False)
    hybrid_static_gb_str = fmt_qty(hybrid_static_gb * GB, GB, precision=1, commas=False)
    full_adam_hybrid_static_gb_str = fmt_qty(full_adam_hybrid_static_gb * GB, GB, precision=1, commas=False)
...
class ThreeDParallelism:
    ...
    pp_h100 = 16
    dp_h100 = 128
    nvlink5_bw_tbs = Hardware.Cloud.B200.nvlink.bandwidth.to(TB/second).magnitude
    ...
    nvlink5_bw_str = fmt_qty(nvlink5_bw_tbs * TB / second, TB / second, precision=1, commas=False)
    ...
    die_to_die_bw_str = fmt_qty(10 * (TB / second), TB / second, precision=0, commas=False)
```
...
class DistTrainH100Capacity:
    ...
    policy_b = Models.Language.Llama2_70B.parameters.to(Bparam).magnitude
    reward_b = 13
    value_b = 13
    bytes_train = 16
    bytes_infer = 2
    ppo_total_mem = policy_b * bytes_train + policy_b * bytes_infer + reward_b * bytes_infer + value_b * bytes_train
    min_gpus_mem = math.ceil(ppo_total_mem / Hardware.Cloud.H100.memory.capacity.to(GiB).magnitude)
    ppo_total_mem_str = fmt_qty(ppo_total_mem * GB, GB, precision=0, commas=True)
...
class RLHFKVCache:
    ...
    kv_total_gb = (kv_cache_bytes * byte).to(GB).magnitude
    dense_cache_bytes = 2 * layers * hidden_dim * seq_len * batch_size * bytes_per_element
    dense_total_gb = (dense_cache_bytes * byte).to(GB).magnitude
    ...
    kv_total_gb_str = fmt_qty(kv_total_gb * GB, GB, precision=1, commas=False)
    dense_total_gb_str = fmt_qty(dense_total_gb * GB, GB, precision=1, commas=False)
```
...
class RLHFBudget:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    policy_b = Models.Language.Llama2_70B.parameters.to(Bparam).magnitude  # Llama-2 70B
    ref_b = Models.Language.Llama2_70B.parameters.to(Bparam).magnitude     # Llama-2 70B
    reward_b = 13
    value_b = 13
    bytes_train = 16
    bytes_infer = 2
    tp = 8
    pp = 4
    h100_mem = Hardware.Cloud.H100.memory.capacity.to(GiB).magnitude
    layers = Models.Language.Llama2_70B.layers
    kv_heads = Models.Language.Llama2_70B.kv_heads
    hidden_dim = Models.Language.Llama2_70B.hidden_dim
    attention_heads = Models.Language.Llama2_70B.heads
    ...
    head_dim = hidden_dim // attention_heads
    kv_width = kv_heads * head_dim
    kv_cache_bytes = 2 * layers * kv_width * seq_len * batch_size * BYTES_FP16.to(byte).magnitude
    kv_total_gb = (kv_cache_bytes * byte).to(GB).magnitude
    kv_total_gb_str = fmt_qty(kv_total_gb * GB, GB, precision=1, commas=False)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    policy_mem_str = fmt_qty(policy_mem * GB, GB, precision=0, commas=True)
    ref_mem_str = fmt_qty(ref_mem * GB, GB, precision=0, commas=False)
    n_gpus_str = fmt_count(n_gpus, commas=False, label="GPU")
    policy_per_gpu_str = fmt_qty(policy_per_gpu * GB, GB, precision=0, commas=False)
    ref_per_gpu_str = fmt_qty(ref_per_gpu * GB, GB, precision=1, commas=False)
    ref_per_gpu_sep_str = fmt_qty(ref_per_gpu_sep * GB, GB, precision=1, commas=False)
    reward_mem_str = fmt_qty(reward_mem * GB, GB, precision=0, commas=False)
    value_mem_str = fmt_qty(value_mem * GB, GB, precision=0, commas=False)
    ppo_total_mem_str = fmt_qty(ppo_total_mem * GB, GB, precision=0, commas=True)
    dpo_total_mem_str = fmt_qty(dpo_total_mem * GB, GB, precision=0, commas=True)
    dpo_reduction_pct_str = fmt_percent(dpo_reduction_pct, precision=1, commas=False, style='prose')
    static_per_gpu_str = fmt_qty(static_per_gpu * GB, GB, precision=1, commas=False)
    remaining_str = fmt_qty(remaining * GB, GB, precision=1, commas=False)
    kv_total_gb_str = fmt_qty(kv_total_gb * GB, GB, precision=1, commas=False)
    kv_per_gpu_str = fmt_qty(kv_per_gpu * GB, GB, precision=1, commas=False)
    remaining_kv_str = fmt_qty(remaining_after_kv * GB, GB, precision=1, commas=False)
...
class A100CapacityFallacies:
    mem_gb = Hardware.Cloud.A100.memory.capacity.to(GiB).magnitude
    params_b = Models.Language.GPT3.parameters.to(param).magnitude / BILLION
    check(mem_gb == 80, "A100 memory capacity should be 80 GB")
    a100_mem_str = fmt_qty(mem_gb * GiB, GiB, precision=0, commas=False, unit_label="GB")
    frontier_params_b_str = fmt(params_b, precision=0)
