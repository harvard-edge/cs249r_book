...
class DistTrainNvlinkIntro:
    ...
    nvlink_hdr_ratio_high = (nvlink_h100_bw / ib_hdr_bw).to_base_units().magnitude
    check(20 < nvlink_hdr_ratio_low < 30, "A100 NVLink/HDR ratio should be about 24x")
    nvlink_a100_str = fmt_bandwidth(nvlink_a100_bw, unit=GB / second, precision=0, commas=False)
    nvlink_h100_str = fmt_bandwidth(nvlink_h100_bw, unit=GB / second, precision=0, commas=False)
...
[^fn-nvlink-intranode]: **NVLink**: NVIDIA's point-to-point GPU interconnect delivers `{python} DistTrainNvlinkIntro.nvlink_a100_str`--`{python} DistTrainNvlinkIntro.nvlink_h100_str` bidirectional bandwidth, roughly `{python} DistTrainNvlinkIntro.nvlink_hdr_ratio_low_str`$\times$--`{python} DistTrainNvlinkIntro.nvlink_hdr_ratio_high_str`$\times$ InfiniBand HDR per port. This bandwidth gap is why tensor parallelism, which requires AllReduce on every layer, is confined to intra-node communication, while pipeline and data parallelism tolerate the slower inter-node fabric. \index{NVLink!distributed training}
...
class HierarchicalAllReduceDebug:
    ...
    nvlink_a100_str = fmt_bandwidth(nvlink_bw, unit=GB/second, precision=0, commas=False)
    ...
    ib_hdr_bw_str = fmt_bandwidth(ib_hdr_bw, unit=GB/second, precision=0, commas=False)
...
class Scaling8GPU:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    total_8gpu_str = fmt_time(total_8gpu_ms_val, 'millisecond', precision=1, commas=False)
    ...
    training_8gpu_str = fmt_time(training_hours_8gpu_val, 'hour', precision=1, commas=False, style='word')
    gpt2_sync_size_gb_str = fmt_memory(sync_size, unit=GB, precision=1, commas=False)
    nvlink_h100_str = fmt_bandwidth(nvlink_bw, unit=GB/second, precision=0, commas=False)
...
class Scaling32GPU:
    ...
    total_32gpu_str = fmt_time(total_32gpu_ms_val, 'millisecond', precision=1, commas=False)
    ...
    training_32gpu_str = fmt_time(training_32gpu_hours_val, 'hour', precision=1, commas=False, style='word')
    gpt2_sync_size_gb_str = fmt_memory(sync_size, unit=GB, precision=1, commas=False)
    inter_node_ms_str = fmt_time(inter_node_ms_val, 'millisecond', precision=0, commas=False)
    ib_hdr_str = fmt_bandwidth(net_bw, unit=GB / second, precision=2, commas=False)
...
**Problem**: Training a `{python} DistTrainA100Memory.params_b_str` parameter Llama-2 model in mixed precision requires `{python} DistTrainA100Memory.bytes_per_param_str` bytes per parameter for weights, gradients, and optimizer state. Does the full training state fit on a single A100-`{python} DistTrainA100Memory.a100_mem_str`, and how does ZeRO Stage 3 with `{python} DistTrainA100Memory.n_gpus_zero_str` change the per-device memory requirement?
...
**Baseline outcome**: out-of-memory (OOM)\index{Out-of-Memory}\index{OOM} on A100-`{python} DistTrainA100Memory.a100_mem_str`.
...
**Setup**\index{Setup}: Training a `{python} InfraFrontierPodSetupRecap.frontier_params_b_str` model on a DGX H100 cluster with `{python} InfraIbBwPodRecap.ib_bw_str` InfiniBand per GPU.
...
class DistTrainNvlinkScaling:
    ...
    nvlink_a100_str = fmt_qty(Hardware.Cloud.A100.nvlink.bandwidth, GB/second, precision=0, commas=False)
```
...
class ModelParallelMemoryFacts:
    ...
    weight_per_gpu_str = fmt_memory(weight_per_gpu_mem, unit=GB, precision=1, commas=False)
    full_state_gb_str = fmt_memory(full_state_mem, unit=GB, precision=0, commas=True)
    full_state_per_gpu_str = fmt_memory(full_state_per_gpu_mem, unit=GB, precision=1, commas=False)
```
...
class A100CapacityContext:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_str = fmt_qty(mem_capacity, GiB, precision=0, commas=False, unit_label="GB")
...
class MemoryWallAnalysis:
    ...
    per_gpu_str = fmt_memory(per_gpu_static, unit=GB, precision=1, commas=False)
    activation_str = fmt_memory(activation_approx, unit=GB, precision=0, commas=False)
    total_per_gpu_str = fmt_memory(total_per_gpu, unit=GB, precision=1, commas=False)
...
**Problem**: Training a `{python} FrontierModelParallelRecap.frontier_params_b_str` parameter model (like GPT-3) on NVIDIA A100s (`{python} A100CapacityContext.a100_mem_str`). Can Data Parallelism with ZeRO-3 handle this?
...
Consider our running example: the `{python} FrontierModelParallelPipelineRecap.frontier_params_b_str` parameter model requires 350 GB of memory in FP16, exceeding the `{python} A100CapacityContext.a100_mem_str` capacity of a single A100 by a factor of four. Model parallelism addresses this **capacity wall**\index{Capacity Wall} by partitioning model weights across multiple devices, effectively stitching them into a single super-accelerator. Unlike data parallelism, where every GPU holds a full replica of the model and processes a unique fraction of the global batch, model parallelism requires each GPU to hold a *unique* fraction of the model and process the *same* data stream sequentially. With 8-way partitioning on A100s, BF16 weights alone occupy approximately 44 GB per GPU. That is only the weights budget: full training still needs activations, gradients, and optimizer state, so pure 8-way model parallelism is not sufficient by itself. Training requires additional sharding or offload for optimizer and gradient state, plus pipeline or hybrid parallelism to keep activation memory within capacity.
...
class TensorParallelBandwidth:
    ...
    nvlink_h100_str = fmt_bandwidth(Hardware.Cloud.H100.nvlink.bandwidth, unit=GB / second, precision=0, commas=False)
    ib_ndr_str = fmt_bandwidth(Systems.Fabrics.InfiniBand_NDR.bandwidth, unit=GB / second, precision=0, commas=False)
    pipeline_ib_lo_bw = Systems.Fabrics.InfiniBand_HDR.bandwidth
    pipeline_ib_hi_bw = Systems.Fabrics.InfiniBand_NDR.bandwidth
    pipeline_ib_lo_str = fmt_bandwidth(pipeline_ib_lo_bw, unit=GB / second, precision=0, commas=False)
    pipeline_ib_hi_str = fmt_bandwidth(pipeline_ib_hi_bw, unit=GB / second, precision=0, commas=False)
```
...
class FlashAttentionOverlapScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_hbm_bw_str = fmt_bandwidth(hbm_bw, unit=TB/second, precision=2, commas=False)
    nvlink_h100_str = fmt_bandwidth(nvlink_bw, unit=GB/second, precision=0, commas=False)
```
...
class FlashAttentionHybridRecap:
    from mlsysim.core.constants import GB, second
    from mlsysim.fmt import fmt, fmt_qty
    nvlink_h100_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth, GB/second, precision=0, commas=False)
```
...
class ThreeDParallelism:
    ...
    nvlink5_bw_str = fmt_bandwidth(nvlink5_bw, unit=TB / second, precision=1, commas=False)
    ...
    die_to_die_bw_str = fmt_bandwidth(10 * (TB / second), unit=TB / second, precision=0, commas=False)
```
...
class DistTrainH100Capacity:
    ...
    ppo_total_mem_str = fmt_memory(ppo_total_mem, unit=GB, precision=0, commas=True)
...
class RLHFBudget:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    policy_mem_str = fmt_memory(policy_mem, unit=GB, precision=0, commas=True)
    ref_mem_str = fmt_memory(ref_mem, unit=GB, precision=0, commas=False)
    n_gpus_str = fmt_count(n_gpus, commas=False, label="GPU")
    policy_per_gpu_str = fmt_memory(policy_per_gpu, unit=GB, precision=0, commas=False)
    ref_per_gpu_str = fmt_memory(ref_per_gpu, unit=GB, precision=1, commas=False)
    ref_per_gpu_sep_str = fmt_memory(ref_per_gpu_sep, unit=GB, precision=1, commas=False)
    reward_mem_str = fmt_memory(reward_mem, unit=GB, precision=0, commas=False)
    value_mem_str = fmt_memory(value_mem, unit=GB, precision=0, commas=False)
    ppo_total_mem_str = fmt_memory(ppo_total_mem, unit=GB, precision=0, commas=True)
    dpo_total_mem_str = fmt_memory(dpo_total_mem, unit=GB, precision=0, commas=True)
    dpo_reduction_pct_str = fmt_percent(dpo_reduction_pct, precision=1, commas=False, style='prose')
    static_per_gpu_str = fmt_memory(static_per_gpu, unit=GB, precision=1, commas=False)
    remaining_str = fmt_memory(remaining, unit=GB, precision=1, commas=False)
    kv_total_gb_str = fmt_memory(kv_total, unit=GB, precision=1, commas=False)
    kv_per_gpu_str = fmt_memory(kv_per_gpu, unit=GB, precision=1, commas=False)
    remaining_kv_str = fmt_memory(remaining_after_kv, unit=GB, precision=1, commas=False)
...
class A100CapacityFallacies:
    ...
    a100_mem_str = fmt_qty(mem, GiB, precision=0, commas=False, unit_label="GB")
    frontier_params_b_str = fmt(params_b, precision=0)
```
