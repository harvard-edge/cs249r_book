...
a100_mem = DistTrainConstants.a100_mem_str
h100_mem = DistTrainConstants.h100_mem_str
nvlink_a100 = DistTrainConstants.nvlink_a100_str
nvlink_h100 = DistTrainConstants.nvlink_h100_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
batch_8_str = ScalingWorkers.batch_8_str
batch_64_str = ScalingWorkers.batch_64_str
exp_iter_8_str = ScalingWorkers.exp_iter_8_str
exp_iter_64_str = ScalingWorkers.exp_iter_64_str
speedup_8_str = ScalingWorkers.speedup_8_str
speedup_64_str = ScalingWorkers.speedup_64_str
speedup_ssp_str = ScalingWorkers.speedup_ssp_str
lr_ssp_str = ScalingWorkers.lr_ssp_str
base_iters_k_str = ScalingWorkers.base_iters_k_str
base_batch_str = ScalingWorkers.base_batch_str
n_workers_8_str = ScalingWorkers.n_workers_8_str
n_workers_64_str = ScalingWorkers.n_workers_64_str
ssp_staleness_str = ScalingWorkers.ssp_staleness_str
critical_batch_str = ScalingWorkers.critical_batch_str
```

::: {.callout-notebook title="Scaling from 8 to 64 Workers" collapse="false"}

**Setup**: Transformer model with 1.3B parameters, target perplexity 15.0, baseline training: `{python} base_iters_k_str` iterations on single GPU with $b =$ `{python} base_batch_str`.

**8 Workers (BSP)**

- Effective batch size: $B = 8 \times 32 =$ `{python} batch_8_str`
- Learning rate: $\eta =$ `{python} n_workers_8_str` $\times \eta_{\text{base}}$ (linear scaling with warmup)
- Expected iterations: $100K / 8 =$ `{python} exp_iter_8_str` iterations
- Convergence: Reaches target perplexity in 12.8K iterations (97% efficiency)
- Communication overhead: 15% (NVLink intra-node)
- Wall-clock speedup: $100K \times 1.0 / (12.8K \times 1.15) =$ `{python} speedup_8_str`$\times$

**64 Workers (BSP)**

- Effective batch size: $B = 64 \times 32 =$ `{python} batch_64_str`
- Learning rate ($N =$ `{python} n_workers_64_str` workers): $\eta = N \times \eta_{\text{base}}$ (if $B < B^*$) or $\eta = \sqrt{N} \times \eta_{\text{base}}$ (if $B > B^*$)
- Assuming $B^* \approx$ `{python} critical_batch_str` (below critical): Linear scaling applies
- Expected iterations: $100K / 64 =$ `{python} exp_iter_64_str` iterations
...
- Wall-clock speedup: $100K \times 1.0 / (1.72K \times 1.45) =$ `{python} speedup_64_str`$\times$

**`{python} n_workers_64_str` Workers (SSP, $s =$ `{python} ssp_staleness_str`)**

- Same effective batch size: $B =$ `{python} batch_64_str`
- Learning rate: $\eta' = \eta_{\text{BSP}} / \sqrt{1 + 4} \approx$ `{python} lr_ssp_str` $\times \eta_{\text{BSP}}$
...
- Wall-clock speedup: $100K \times 1.0 / (2.1K \times 1.25) =$ `{python} speedup_ssp_str`$\times$
...
# │ Exports: a100_mem, mwa_per_gpu_str, mwa_activation_str
# └─────────────────────────────────────────────────────────────────────────────
...
mwa_per_gpu_str = MemoryWallAnalysis.per_gpu_str
mwa_activation_str = MemoryWallAnalysis.activation_str
```
...
4.  **ZeRO-3 Sharding**: With 64 GPUs, per-GPU static memory = $2{,}450 / 64 \approx$ **`{python} mwa_per_gpu_str` GB**.
5.  **Activation Memory**: For sequence length 2048 and batch size 1, a 96-layer transformer generates $\approx$ **`{python} mwa_activation_str` GB** of activations per GPU.
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
policy_mem_str = RLHFBudget.policy_mem_str
ref_mem_str = RLHFBudget.ref_mem_str
rlhf_n_gpus_str = RLHFBudget.n_gpus_str
policy_per_gpu_str = RLHFBudget.policy_per_gpu_str
ref_per_gpu_str = RLHFBudget.ref_per_gpu_str
ref_per_gpu_sep_str = RLHFBudget.ref_per_gpu_sep_str
reward_mem_str = RLHFBudget.reward_mem_str
value_mem_str = RLHFBudget.value_mem_str
static_per_gpu_str = RLHFBudget.static_per_gpu_str
rlhf_remaining_str = RLHFBudget.remaining_str
kv_per_gpu_str = RLHFBudget.kv_per_gpu_str
rlhf_remaining_kv_str = RLHFBudget.remaining_kv_str
```
...
The policy model under 3D parallelism with TP=8 and PP=4 distributes its `{python} policy_mem_str` GB training state across `{python} rlhf_n_gpus_str` GPUs, yielding `{python} policy_per_gpu_str` GB per GPU. The reference model, requiring only `{python} ref_mem_str` GB for inference, can be sharded across a separate pool of 8 GPUs at `{python} ref_per_gpu_sep_str` GB each, or co-located with the policy GPUs at an additional `{python} ref_per_gpu_str` GB per GPU (`{python} ref_mem_str` / `{python} rlhf_n_gpus_str`). The 13B reward model adds `{python} reward_mem_str` GB shared across its pool. The 13B value model in training mode adds approximately `{python} value_mem_str` GB (13B $\times$ 16 bytes/param) across its pool.

Total static memory (co-located policy + reference on `{python} rlhf_n_gpus_str` GPUs): $`{python} policy_per_gpu_str` + `{python} ref_per_gpu_str` \approx$ `{python} static_per_gpu_str` GB per GPU, leaving $\sim$`{python} rlhf_remaining_str` GB for activations and KV cache. With generation-phase KV caches for 256 sequences of 1,024 tokens, each policy GPU must reserve an additional $172 / `{python} rlhf_n_gpus_str` \approx$ `{python} kv_per_gpu_str` GB for its KV cache shard. The remaining $\sim$`{python} rlhf_remaining_kv_str` GB constrains the micro-batch size during the training phase.

**DPO Memory Budget (same parallelism configuration)**

The policy model uses the same `{python} policy_per_gpu_str` GB per GPU. The reference model adds `{python} ref_per_gpu_str` GB per GPU (co-located). No reward model, no value model, no KV cache for generation.

Total static memory: $`{python} policy_per_gpu_str` + `{python} ref_per_gpu_str` =$ `{python} static_per_gpu_str` GB per GPU — identical to PPO's static footprint, but without the generation-phase KV cache burden. The full $\sim$`{python} rlhf_remaining_str` GB remaining is available for training activations, permitting 2$\times$ larger micro-batch sizes or eliminating the need for activation checkpointing that PPO requires.
