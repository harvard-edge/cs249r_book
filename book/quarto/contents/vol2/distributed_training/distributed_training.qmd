...
Understanding these basic requirements provides the foundation for examining the main approaches to distributed training. Data parallelism divides the training data across machines while each maintains a full model copy. Model parallelism splits the model itself across devices when parameters exceed single-device memory. Pipeline parallelism partitions models into sequential stages that process microbatches concurrently. Hybrid approaches integrate multiple strategies, enabling training at scales where any single approach would fail. The following sections examine each approach in detail, beginning with the efficiency metrics that govern all distributed training decisions.
...
The crossover point depends on message size. Tree AllReduce wins for small messages that are latency-dominated, while ring AllReduce wins for large gradients that are bandwidth-dominated [@patarasuk2009allreduce]. Modern implementations like NCCL use hierarchical algorithms that achieve tree latency within nodes using NVLink and ring bandwidth between nodes using InfiniBand.
:::
...
As models and datasets continue to expand, training and inference workloads must extend beyond single-server configurations. Google's TPU Pods[^fn-tpu-pod] represent a pioneering approach to this challenge, interconnecting hundreds of TPUs to function as a unified system [@jouppi2017tpu].
