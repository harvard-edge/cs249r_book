...
_Why does adding more machines to training eventually make it slower rather than faster?_

Distributed training appears simple: split the work across machines and combine the results. But communication costs grow with the number of machines while computation per machine shrinks, until synchronization overhead dominates and adding hardware provides no benefit or actively degrades performance. This scaling ceiling is not a bug to be fixed but a fundamental property of distributed systems: coordinating independent machines requires moving data between them, and data movement across networks is orders of magnitude slower than data movement within a chip. The art of distributed training is managing this tension—partitioning work to minimize communication, overlapping communication with computation to hide latency, choosing synchronization strategies that balance consistency against throughput. Without this understanding, organizations waste millions on hardware that sits idle waiting for gradients to arrive, or produce models that never converge because stale updates corrupted the optimization.
...
In the **Systems Sandwich** framework (@sec-vol2-introduction), Distributed Training represents the **Operational Layer**—the logic that partitions the mathematical workload across the physical fleet. While Volume 1 optimized the execution on a single device, Volume 2 solves the constraint satisfaction problem of mapping a model larger than any single memory bank onto a cluster with finite bandwidth.

Training frontier models requires computational resources...

::: {.callout-note title="Connection: The Systems Sandwich"}
In the **Systems Sandwich** framework (@fig-systems-sandwich in @sec-vol2-introduction), Distributed Training represents the **Operational Layer (Logic)**. We are defining *how* to split the math. The actual execution of these split workloads happens on the **Physical Layer (Infrastructure)**, which we will build in Part II. The algorithms defined here (Ring AllReduce, Tensor Parallelism) dictate the bandwidth requirements for the physical interconnects (NVLink, InfiniBand) discussed later. Throughout this chapter, we apply the Systems Sandwich methodology to diagnose distributed training bottlenecks by systematically examining Physical constraints, Operational choices, and Service metrics.
:::

The transition from single-machine to distributed training fundamentally changes optimization strategy and system complexity. Single-machine optimization focuses on efficiently utilizing available resources through techniques such as prefetching, mixed precision, and gradient accumulation. Distributed training extends systematic optimization to multiple machines, introducing coordination challenges that require new conceptual frameworks.

### Multi-Machine Training Requirements {#sec-distributed-training-multimachine-training-requirements}

Three concrete signals indicate when distributed training becomes necessary rather than merely beneficial. First, **memory exhaustion** occurs when model parameters, optimizer states, and activation storage exceed single-device capacity. We must distinguish between static memory, which includes parameters and optimizer states and remains constant per batch, and dynamic memory for activations, which scales with batch size and sequence length.

::: {.callout-definition title="Scaling Laws"}
**Scaling Laws** describe the predictable, power-law relationship between a model's performance (loss) and the scale of its compute, dataset size, and parameter count. These empirical laws provide the "physics" of deep learning, allowing engineers to extrapolate the resources required to achieve a target performance level before training begins.
:::

For long-context LLMs, activation memory often dominates. Techniques like **gradient checkpointing**[^fn-gradient-checkpointing] address this by discarding intermediate activations during the forward pass and recomputing them during the backward pass, trading 30% more compute for O(sqrt(n)) memory reduction. Even with these optimizations, models beyond 10-20 billion parameters typically exceed the 40-80GB HBM capacity of modern GPUs.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, z={(0.6,0.5)}, x={(1,0)}, y={(0,1)}, scale=1.3]
  ...
  \definecolor{PipeColor}{RGB}{205,92,92}  % IndianRed
  \definecolor{TensorColor}{RGB}{60,179,113}% MediumSeaGreen

  % Origin
  \coordinate (O) at (0,0,0);

  % Axes Arrows
  \draw[->, ultra thick, DataColor] (O) -- (4.5,0,0) node[anchor=north east, align=center, font=\bfseries] {Data Parallelism\\(Replica Count $d$)};
  \draw[->, ultra thick, PipeColor] (O) -- (0,3.5,0) node[anchor=east, align=right, font=\bfseries] {Pipeline Parallelism\\(Depth/Stages $p$)};
  \draw[->, ultra thick, TensorColor] (O) -- (0,0,3.5) node[anchor=south west, align=left, font=\bfseries] {Tensor Parallelism\\(Layer Width $t$)};
  ...
  \node[align=center, font=\footnotesize, text=DataColor] at (2.5, -0.5, 0) {constraint: Global Batch Size};
  \node[align=center, font=\footnotesize, text=PipeColor] at (-0.5, 2.5, 0) {constraint: Latency/Bubbles};
  \node[align=center, font=\footnotesize, text=TensorColor] at (-0.5, 0, 2.5) {constraint: Memory/NVLink};

\end{tikzpicture}
```
:::

### Engineering Trade-offs: Selecting a Parallelism Strategy {#sec-distributed-training-engineering-tradeoffs}
...
::: {.callout-perspective title="The Compiler Perspective: Distributed Training as Loop Transformations"}
...
[^nvlink]: NVLink provides 600 GB/s bidirectional bandwidth, enabling efficient gradient synchronization within nodes. @sec-infrastructure provides detailed coverage of GPU interconnect architectures, including NVLink topology design and bandwidth optimization strategies.
...
Before examining specific parallelism strategies, we must understand the quantitative metrics that govern distributed training efficiency. These metrics provide the foundation for making informed decisions about scaling strategies, hardware selection, and optimization approaches.
...
We can formalize the **Distributed Step Time** equation:

$$ T_{step}(N) = \frac{T_{compute}}{N} + T_{comm}(N) - T_{overlap} $$

Where:
*   $T_{compute}$ is the total computation required for the batch (ideally perfectly parallelizable).
*   $T_{comm}(N)$ is the communication overhead, which typically grows with $N$ (e.g., $2(N-1)/N \times M/B$ for Ring AllReduce).
*   $T_{overlap}$ is the portion of communication that can be hidden behind computation.
...
This equation reveals the **Scaling Wall**: as $N$ increases, the compute term ($T_{compute}/N$) shrinks, but the communication term ($T_{comm}$) remains constant or grows. Eventually, the denominator is dominated by communication, driving efficiency toward zero. This physics dictates the transition points between parallelism strategies: when Data Parallelism hits this wall (high $T_{comm}$ due to full gradient sync), we must switch to Model or Pipeline Parallelism to change the variables in the equation.
...
## Convergence Guarantees for Distributed Optimization {#sec-distributed-training-convergence-guarantees}
...
### Convergence Rate for Synchronous Data Parallel SGD {#sec-distributed-training-convergence-rate}
...
### Staleness Impact: BSP versus SSP versus ASP {#sec-distributed-training-staleness-impact}
...
**Gradient staleness** $\tau$ is the number of parameter updates that occurred between when a gradient was computed and when it is applied. In BSP, $\tau = 0$ always. In asynchronous SGD (ASP), $\tau$ can be arbitrarily large. Stale Synchronous Parallel (SSP) bounds staleness to $\tau \leq s$ for some threshold $s$.
:::
...
### Learning Rate Scaling Rules {#sec-distributed-training-lr-scaling}
...
### Critical Batch Size: When Does Parallelism Hurt? {#sec-distributed-training-critical-batch-size}
...
### Worked Example: Convergence Comparison for 8 versus 64 Workers {#sec-distributed-training-convergence-example}

To illustrate these concepts concretely, consider training a transformer language model with baseline batch size $b = 32$ per worker.

::: {.callout-notebook title="Worked Example: Scaling from 8 to 64 Workers" collapse="false"}
...
### Trade-off: Communication Cost versus Convergence Speed {#sec-distributed-training-comm-convergence-tradeoff}
...
Building on the efficiency characteristics outlined above, data parallelism represents the most straightforward distributed approach. It proves particularly effective in the linear scaling regime where communication overhead remains manageable.
...
::: {.callout-note title="Cross-Reference: Communication Algorithms"}
The specific algorithms for gradient synchronization—including **Ring AllReduce**, **Tree AllReduce**, and **Hierarchical AllReduce**—are analyzed in depth in @sec-communication. That chapter derives the bandwidth and latency bounds ($2N(D-1)/D$), explains how topology-aware algorithms exploit NVLink vs. InfiniBand, and details the gradient compression techniques used to reduce this overhead.
:::

::: {.callout-perspective title="Systems Sandwich Analysis: Debugging Slow Gradient Synchronization"}
...
The Systems Sandwich framework provides a systematic debugging methodology by examining each layer:
...
**Root Cause Diagnosis**: The mismatch between Physical and Operational layers reveals the problem. Your single ring correctly identifies InfiniBand as the bottleneck, but the observed 100ms (rather than 240ms) suggests NCCL is already using a hierarchical algorithm internally. The *remaining* gap comes from switch congestion, not algorithm choice.

**Solution**: Monitor InfiniBand switch port utilization to identify hot spots. Consider rail-optimized topology (@sec-infrastructure) or hierarchical AllReduce that explicitly partitions intra-node (NVLink) from inter-node (InfiniBand) communication. The 40ms remaining gap likely represents achievable optimization through better network provisioning rather than algorithm changes.

This analysis demonstrates how the Systems Sandwich layers interact: Physical constraints (bandwidth) bound Operational choices (algorithm), which manifest in Service metrics (latency). Debugging requires examining all three layers, not just tuning one in isolation.
...
::: {.callout-notebook title="Worked Example: GPT-2 Data Parallel Scaling (1→8→32 GPUs)" collapse="true"}
...
While data parallelism is effective for distributed training, it introduces several challenges that must be addressed to achieve efficient and scalable training systems. These challenges stem from the inherent trade-offs between computation and communication, as well as the limitations imposed by hardware and network infrastructures.
...
[^fn-infiniband]: InfiniBand provides 200 Gb/s per port with sub-microsecond latency, enabling efficient gradient synchronization across nodes via RDMA. @sec-infrastructure explores network topology design, including fat-tree and dragonfly architectures that optimize bisection bandwidth for AllReduce communication patterns.
...
::: {.callout-notebook title="Engineering Calculation: ZeRO Memory Savings" collapse="true"}
...
::: {.callout-perspective title="Back-of-the-Napkin: The Memory Wall of Scale"}
**Problem**: You want to train a **175 Billion parameter** model (like GPT-3) on NVIDIA A100s (80 GB). Can you use Data Parallelism with ZeRO-3?

**The Math**:
1.  **Parameter Storage**: 175B params $\times$ 2 bytes (FP16) = **350 GB**.
2.  **Optimizer State**: 175B params $\times$ 12 bytes (Adam FP32) = **2,100 GB**.
...
**The Systems Conclusion**: $38 \text{ GB} (Static) + 50 \text{ GB} (Dynamic) = \mathbf{88 \text{ GB}}$. This exceeds the 80 GB capacity of the A100. Even with full ZeRO-3 sharding, **pure Data Parallelism fails**. You *must* use **Tensor Parallelism** to split the layers themselves.
:::
...
This distribution method enables training of large-scale models. GPT-3, with 175 billion parameters, relies on model parallelism for training. Vision transformers processing high-resolution 16k × 16k pixel images use model parallelism to manage memory constraints. Mixture-of-Experts architectures use this approach to distribute their conditional computation paths across hardware.
...
Each device applies its chosen optimizer (such as Adam [@kingma2015adam] or AdaFactor) to update its portion of the model parameters. A device holding the first six transformer layers updates only those layers' weights and biases. This local parameter update eliminates the need for cross-device synchronization during the optimization step, reducing communication overhead.
...
Layer-wise partitioning assigns distinct model layers to separate computing devices. In transformer architectures, this translates to specific devices managing defined sets of attention and feed-forward blocks. @fig-layers-blocks demonstrates this partitioning for a 24-layer transformer: six consecutive blocks reside on each of four devices, with forward activations flowing left-to-right and backward gradients propagating right-to-left across the device boundaries.

::: {#fig-layers-blocks fig-env="figure" fig-pos="htb" fig-cap="**Layer-Wise Model Parallelism**. A 24-layer transformer distributed across four GPUs, with six consecutive transformer blocks assigned to each device. Forward activations (black arrows) flow left-to-right through device boundaries, while backward gradients (red arrows) propagate right-to-left during parameter updates. This partitioning reduces per-GPU memory from the full model size to 1/4, but introduces sequential dependencies where downstream devices wait for upstream computation to complete." fig-alt="24-layer transformer split across 4 devices: Blocks 1-6 on GPU 1, 7-12 on GPU 2, 13-18 on GPU 3, 19-24 on GPU 4. Black arrows show forward activation flow; red arrows show backward gradient propagation."}
```{.tikz}
\begin{tikzpicture}[font=\usefont{T1}{phv}{m}{n}\small]
...
### Expert Parallelism (Mixture of Experts) {#sec-distributed-training-expert-parallelism}
...
The primary advantage is decoupling model size from compute budget. A trillion-parameter MoE model might use only 10B parameters per token, enabling training on feasible hardware budgets. The constraint is the **All-to-All** communication, which is bandwidth-intensive and sensitive to load imbalance (if all tokens route to one expert, that device becomes a straggler). Advanced load balancing losses and capacity factors are required to ensure uniform expert utilization.
...
A further challenge is pipeline bubbles in pipeline parallelism. With $P$ pipeline stages and $M$ microbatches per training step, the pipeline experiences idle time at the start (fill) and end (drain) of the schedule.

::: {.callout-notebook title="Engineering Calculation: Pipeline Efficiency" collapse="true"}
**Scenario**: 8-stage pipeline ($P=8$), choosing micro-batch count ($M$).

**Formula (Bubble Fraction)**:
$$ \text{Overhead} = \frac{P - 1}{M} $$

**Case A: Low Latency ($M=4$)**
$$ \text{Overhead} = \frac{7}{4} = 175\% \text{ (Wait time > Compute time)} $$
$$ \text{Efficiency} = \frac{1}{1 + 1.75} \approx \mathbf{36\%} $$

**Case B: High Throughput ($M=32$)**
$$ \text{Overhead} = \frac{7}{32} \approx 22\% $$
$$ \text{Efficiency} = \frac{1}{1 + 0.22} \approx \mathbf{82\%} $$

**Trade-off**: Case B achieves high efficiency but requires storing activations for $32$ micro-batches, increasing memory usage by $8\times$ compared to Case A.
:::

Increasing $M$ reduces this overhead but increases the memory required to store activations for the pending backward passes. This trade-off between throughput with high $M$ and memory with low $M$ is the central optimization constraint in pipeline parallelism.

::: {.callout-note title="Figure: Pipeline Bubble Analysis" collapse="false"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.8]
  \definecolor{ForwardColor}{RGB}{200,220,255}
  \definecolor{BackwardColor}{RGB}{255,220,200}
  \definecolor{BubbleColor}{RGB}{240,240,240}

  \tikzset{
    box/.style={draw=black!70, thick, minimum width=1.2cm, minimum height=0.8cm, align=center, font=\scriptsize},
    fwd/.style={box, fill=ForwardColor},
    bwd/.style={box, fill=BackwardColor},
    bubble/.style={box, fill=BubbleColor, text=gray}
  }
  ...
  % GPU 0
  \node[fwd] at (0.6, 0) {$F_{0,0}$};
  \node[fwd] at (1.8, 0) {$F_{0,1}$};
  \node[bubble] at (3.0, 0) {...};
  \node[bubble] at (4.2, 0) {...};
  \node[bubble] at (5.4, 0) {...};
  \node[bwd] at (6.6, 0) {$B_{0,0}$};
  \node[bwd] at (7.8, 0) {$B_{0,1}$};

  % GPU 1
  \node[bubble] at (0.6, 1) {};
  \node[fwd] at (1.8, 1) {$F_{1,0}$};
  \node[fwd] at (3.0, 1) {$F_{1,1}$};
  \node[bubble] at (4.2, 1) {...};
  \node[bubble] at (5.4, 1) {...};
  \node[bwd] at (6.6, 1) {$B_{1,0}$};
  \node[bubble] at (7.8, 1) {};

  % GPU 2
  \node[bubble] at (0.6, 2) {}; \node[bubble] at (1.8, 2) {};
  \node[fwd] at (3.0, 2) {$F_{2,0}$};
  \node[fwd] at (4.2, 2) {$F_{2,1}$};
  \node[bubble] at (5.4, 2) {...};
  \node[bwd] at (6.6, 2) {$B_{2,0}$};

  % GPU 3
  \node[bubble] at (0.6, 3) {}; \node[bubble] at (1.8, 3) {}; \node[bubble] at (3.0, 3) {};
  \node[fwd] at (4.2, 3) {$F_{3,0}$};
  \node[bwd] at (5.4, 3) {$B_{3,0}$};

  % Bubble Braces
  \draw[decorate, decoration={brace, amplitude=5pt}, thick] (0,-0.6) -- (3.6,-0.6) node[midway, below, yshift=-5pt] {Pipeline Fill (Bubble)};
  \draw[decorate, decoration={brace, amplitude=5pt}, thick] (6,-0.6) -- (9.6,-0.6) node[midway, below, yshift=-5pt] {Pipeline Drain};

\end{tikzpicture}
```
**Pipeline Bubble Analysis**. A space-time diagram showing the "Fill" and "Drain" phases of a pipeline. Gray areas represent idle time (bubbles) where GPUs wait for dependencies. Synchronous pipeline parallelism (GPipe) results in a bubble overhead of $(P-1)/M$, where $P$ is the number of stages and $M$ is the number of microbatches.
:::

::: {.callout-perspective title="Systems Sandwich Analysis: Optimizing Pipeline Bubble Overhead"}

**Problem Statement**: Your 8-stage pipeline shows 50% bubble overhead when the formula predicts only 12.5% for your configuration. Where is the efficiency going?

Applying the Systems Sandwich framework to diagnose pipeline inefficiency:

**Physical Layer (Infrastructure)**:

- **Configuration**: 8 pipeline stages across 8 nodes
- **Inter-node bandwidth**: InfiniBand HDR at 200 Gb/s (25 GB/s)
- **Activation tensor size**: 128 MB per microbatch boundary
- **Transfer time**: $128 \text{ MB} / 25 \text{ GB/s} = 5.1\text{ms}$ per stage transition

**Operational Layer (Logic)**:

- **Microbatch count**: $M = 16$ microbatches
- **Expected bubble fraction**: $(P-1)/M = 7/16 = 43.75\%$
- **Computation time per stage**: 40ms per microbatch
- **Communication time**: 5.1ms per boundary crossing

**Service Layer (Measurement)**:

- **Observed efficiency**: 50% (matching bubble prediction plus overhead)
- **Stage utilization histogram**: Shows Stage 4 consistently 15% slower than others
- **Memory pressure**: Stage 4 reports 95% HBM utilization

**Root Cause Diagnosis**: The Physical-Operational mismatch reveals two issues:

1. **Insufficient microbatches**: With $M=16$, theoretical bubble overhead is 43.75%, already high
2. **Unbalanced stage computation**: Stage 4 (the FFN expansion layer with 4x hidden dimension) takes longer, creating additional stalls

**Validated Solution**:

1. **Increase microbatch count to $M=64$**: This reduces theoretical bubble to $(8-1)/64 = 10.9\%$
2. **Rebalance pipeline stages**: Move one transformer layer from Stage 4 to Stage 5 to equalize computation time
3. **Trade-off**: Increasing to $M=64$ requires 4x more activation memory storage. Verify this fits within HBM capacity or enable activation checkpointing for the additional microbatches

**Quantitative Impact**:

- Before: 50% bubble overhead → 50% efficiency
- After ($M=64$ + rebalancing): 10.9% bubble + 3% communication overhead → 86% efficiency
- Training time reduction: $1 - (0.50/0.86) = 42\%$ faster

This example demonstrates how the Systems Sandwich connects algorithmic choices (microbatch count) to physical constraints (memory capacity) through observable metrics (stage utilization). The diagnosis required examining all three layers: Physical revealed the memory constraint, Operational identified the suboptimal configuration, and Service metrics pinpointed the unbalanced stage.
...
::: {.callout-note title="Archetype A (Scaled Lighthouse): The Physics of 3D Parallelism"}
...
Hardware utilization improves substantially over single-strategy approaches. Pure model parallelism leaves devices idle during pipeline bubbles (20-30% of training time). Pure data parallelism leaves devices waiting during gradient synchronization (10-40% of training time). Hybrid approaches overlap these operations: while one pipeline stage computes, another synchronizes gradients within its data parallel group. Megatron-LM demonstrates approximately 50-55% MFU (Model FLOPS Utilization) on 1024 A100 GPUs using 3D parallelism [@narayanan_pipeline_parallelism_2021], compared to 30-40% MFU for pure data parallelism at similar scale. @fig-hybrid-mapping illustrates how these efficiency gains emerge from hierarchical communication structure: tensor parallelism confines high-frequency communication to NVLink within nodes, pipeline parallelism restricts cross-node traffic to activation transfers at stage boundaries, and data parallelism performs AllReduce across replica groups using InfiniBand.
...
::: {#fig-hybrid-mapping fig-env="figure" fig-pos="htb" fig-cap="**3D Parallelism Physical Mapping**. How Hybrid Parallelism maps to hardware topology. **Tensor Parallelism** uses high-bandwidth NVLink within a node to split layers. **Pipeline Parallelism** connects adjacent nodes via high-speed InfiniBand to form the model depth. **Data Parallelism** replicates this entire pipeline structure, synchronizing gradients via the inter-node network." fig-alt="Two-node cluster diagram. Within each node, 4 GPUs connect via NVLink for Tensor Parallelism. Between nodes, InfiniBand carries Pipeline Parallelism. Dashed box shows Data Parallel replica group."}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \definecolor{NodeColor}{RGB}{245,245,245}
  \definecolor{TPColor}{RGB}{176,196,222} % LightSteelBlue
  \definecolor{PPColor}{RGB}{255,182,193} % LightPink

  % Node 1
  \node[draw, fill=NodeColor, rounded corners=4pt, minimum width=4cm, minimum height=2.5cm, label={north:Node 1 (PP Stage 1)}] (N1) at (0,0) {};
  \node[draw, fill=TPColor, circle, label={below:G1}] (G11) at (-1.5, -0.5) {};
  \node[draw, fill=TPColor, circle, label={below:G2}] (G12) at (-0.5, -0.5) {};
  \node[draw, fill=TPColor, circle, label={below:G3}] (G13) at (0.5, -0.5) {};
  \node[draw, fill=TPColor, circle, label={below:G4}] (G14) at (1.5, -0.5) {};
  \draw[thick, <->, blue] (G11) -- (G12) -- (G13) -- (G14);
  \node[blue, font=\scriptsize] at (0, 0.2) {Tensor Parallel (NVLink)};

  % Node 2
  \node[draw, fill=NodeColor, rounded corners=4pt, minimum width=4cm, minimum height=2.5cm, label={north:Node 2 (PP Stage 2)}] (N2) at (6,0) {};
  \node[draw, fill=TPColor, circle, label={below:G1}] (G21) at (4.5, -0.5) {};
  \node[draw, fill=TPColor, circle, label={below:G2}] (G22) at (5.5, -0.5) {};
  \node[draw, fill=TPColor, circle, label={below:G3}] (G23) at (6.5, -0.5) {};
  \node[draw, fill=TPColor, circle, label={below:G4}] (G24) at (7.5, -0.5) {};
  \draw[thick, <->, blue] (G21) -- (G22) -- (G23) -- (G24);

  % Pipeline Arrow
  \draw[ultra thick, ->, orange] (N1) -- node[above, font=\bfseries] {Pipeline Parallel (InfiniBand)} (N2);

  % Replica Group (Data Parallel)
  \node[draw, dashed, inner sep=10pt, label={above:Replica 1}] (R1) at (3, 0) [minimum width=11cm, minimum height=3.5cm] {};

  \node[align=left, font=\footnotesize, anchor=west] at (-2, -3) {
    \textbf{Mapping Strategy:}\\
    1. \textcolor{blue}{Tensor Parallel}: Inside single Node (Highest BW)\\
    2. \textcolor{orange}{Pipeline Parallel}: Between adjacent Nodes (High BW)\\
    3. \textbf{Data Parallel}: Replicates this entire structure
  };
\end{tikzpicture}
```
:::
...
@tbl-parallelism-compare contrasts data, model, pipeline, and hybrid parallelism across six dimensions: memory requirements per device, communication overhead patterns, scalability characteristics, implementation complexity, and ideal use cases. The table reveals a fundamental trade-off: data parallelism offers implementation simplicity but high memory requirements, while model parallelism reduces per-device memory at the cost of complex coordination.
...
## Hardware Constraints on Parallelism {#sec-distributed-training-hardware-constraints}

While this chapter focuses on algorithmic strategies, the physical reality of hardware interconnects dictates which strategies are feasible. The bandwidth hierarchy of a cluster creates natural boundaries for different parallelism types.

**Intra-Node Bandwidth**: Modern GPU nodes provide extremely high bandwidth (600-900 GB/s) between devices via NVLink. This supports **Tensor Parallelism**, which requires frequent, high-volume communication (2 AllReduce operations per layer).

**Inter-Node Bandwidth**: Connectivity between nodes (InfiniBand/Ethernet) typically offers 25-100 GB/s, an order of magnitude slower than intra-node links. This constrains cross-node strategies to **Data Parallelism** (synchronizing gradients once per step) or **Pipeline Parallelism** (sending small activation tensors at stage boundaries).

**Memory Capacity**: The physical memory per device (40-80 GB) sets the hard limit for model shards.

A detailed analysis of these physical systems—including TPU Pods, SuperPODs, and wafer-scale integration—is provided in **Part II: Building the Machine Learning Fleet**. For the algorithm designer, the key takeaway is to match communication-heavy strategies (Tensor Parallelism) to high-bandwidth domains (Intra-node) and communication-light strategies (Pipeline/Data Parallelism) to lower-bandwidth domains (Inter-node).
...
**Fallacy:** _Linear speedup is achievable with sufficient engineering effort._

Amdahl's Law establishes hard limits: any sequential component bounds maximum speedup regardless of parallelism. In distributed training, gradient synchronization is inherently sequential since all gradients must be collected before any update proceeds. As @sec-distributed-training-efficiency-metrics demonstrates, the scaling efficiency equation $\text{Efficiency}(N) = 1/(1 + N(T_{comm}(N) - T_{overlap})/T_{compute})$ reveals how communication overhead dominates as $N$ increases. Even with perfect overlap and optimal algorithms, communication overhead grows with cluster size. For data parallelism, AllReduce time increases logarithmically with tree algorithms or linearly in the latency term with ring algorithms as GPU count grows. A 1000-GPU cluster will never train 1000x faster than a single GPU; achieving 500x speedup would be exceptional, and 100-200x is more typical for communication-heavy workloads. Organizations that budget projects assuming linear scaling inevitably miss deadlines and overspend on compute.

**Pitfall:** _Hyperparameters tuned on small clusters transfer directly to large-scale training._

Engineers tune hyperparameters on 8-GPU workstations then deploy to 256-GPU clusters expecting identical behavior. In production, convergence patterns change fundamentally with scale. The most critical hyperparameter is learning rate: as @sec-distributed-training-data-parallelism explains, batch size increases proportionally with GPU count in data parallelism, requiring learning rate adjustments. The "linear scaling rule"[^fn-linear-scaling] [@goyal2017accurate] suggests $\eta_{large} = \eta_{base} \times (B_{large}/B_{base})$, but this relationship holds only within bounds.
...
::: {.callout-note title="Figure: The 3D Parallelism Cube" collapse="false"}
```{.tikz}
\begin{tikzpicture}[scale=1.2, font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \node[anchor=west, font=\scriptsize, text=black!60] at (3, 2.5) {Total GPUs $N = d \times t \times p$};
\end{tikzpicture}
```
**The 3D Parallelism Space**. Distributed training for frontier models combines three orthogonal scaling axes. Data Parallelism ($d$) replicates the model to scale throughput. Tensor Parallelism ($t$) splits layers within a node to fit memory. Pipeline Parallelism ($p$) splits layers across nodes to reduce communication frequency.
:::

... (later in summary) ...

The parallelism strategies examined here determine how we coordinate learning across the Machine Learning Fleet. However, each of these strategies—whether splitting by data, layer, or tensor—creates specific, high-volume traffic patterns that can easily become the system bottleneck.

The next chapter, @sec-communication, examines the logical communication patterns and collective operations (AllReduce, AllGather) that coordinate these distributed shards and explores how to optimize the "Communication-Computation Ratio" that governs scaling efficiency.
