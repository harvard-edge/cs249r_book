...
**Deadlock from Worker Failure**: If any worker crashes during AllReduce, all other workers block indefinitely waiting for the missing contribution. Without timeout mechanisms, the entire training job hangs rather than failing cleanly. Production systems implement watchdog timers (typically 5-10 minutes) to detect and terminate stuck jobs.

**Deadlock from Gradient Mismatch**: If workers disagree on which tensors to synchronize (due to conditional computation paths or dynamic batching), AllReduce operations may block waiting for tensors that some workers never send. This commonly occurs with:

- Variable-length sequences in NLP models
- Dynamic computation graphs
- Mixture-of-experts with different routing decisions

**Straggler-Induced Delays**: The iteration time equals the slowest worker's time plus synchronization overhead. A single slow worker (due to thermal throttling, network congestion, or OS jitter) delays all workers, reducing cluster utilization. At 1000 GPUs with 1% probability of straggler per GPU per step, approximately 10 GPUs straggle every iteration.
...
**ZeRO-2** additionally shards gradients. Instead of AllReduce (which leaves full gradients on each GPU), ZeRO-2 uses ReduceScatter (each GPU receives $1/N$ of the reduced gradients). Memory savings: gradients reduced from $4N$ bytes/param to $4$ bytes/param total.

**ZeRO-3 / FSDP** shards parameters themselves. Each GPU stores only $1/N$ of the model. Before each layer's forward pass, parameters are gathered via AllGather; after backward pass, gradients are reduced via ReduceScatter, then parameters are discarded. This achieves maximum memory efficiency at the cost of additional communication.
...
- **Model fits in GPU memory with room for activations**: Use DDP (lower overhead)
- **Model barely fits or requires activation checkpointing**: Use FSDP ZeRO-2
- **Model parameters exceed single-GPU memory**: Use FSDP ZeRO-3
- **Training 70B+ models on 80GB GPUs**: Combine FSDP with tensor parallelism
...
**Megatron-Style Tensor Parallelism**: The key insight is that matrix multiplications can be partitioned in two ways:

**Column-parallel linear layers**: Weights are split along columns. For input $X$ and weight matrix $W = [W_1 | W_2]$ split across 2 GPUs:
...
**Row-parallel linear layers**: Weights are split along rows. For $W = \begin{bmatrix} W_1 \\ W_2 \end{bmatrix}$:
...
**Communication volume per transformer layer**: For sequence length $S$, hidden dimension $H$, and batch size $B$:
...
**Tensor parallelism scaling limits**: Beyond 8-way tensor parallelism, efficiency degrades rapidly because:
...
**Fallacy: Linear speedup is achievable with enough engineering effort.**

This belief persists despite theoretical impossibility. Amdahl's Law establishes hard limits: any sequential component bounds maximum speedup regardless of parallelism. In distributed training, gradient synchronization is inherently sequential since all gradients must be collected before any update proceeds.

Even with perfect overlap and optimal algorithms, communication overhead grows with cluster size. For data parallelism, AllReduce time increases logarithmically (tree) or linearly (ring latency term) with GPU count. A 1000-GPU cluster will never train 1000x faster than a single GPU; achieving 500x speedup would be exceptional, and 100-200x is more typical for communication-heavy workloads.

Organizations that budget projects assuming linear scaling inevitably miss deadlines and overspend on compute.

**Pitfall: Using the same hyperparameters at all scales.**
...
Data parallelism increases effective batch size proportionally with GPU count. But statistical efficiency (loss reduction per sample) decreases with batch size beyond model-specific thresholds. A 100K-sample batch may provide only 2x the gradient quality of a 10K-sample batch, not 10x.
...
**Pitfall: Ignoring the memory-compute trade-off in parallelism selection.**
...
**Fallacy: FSDP/ZeRO always improves training efficiency.**
...
**Pitfall: Benchmarking parallelism strategies on small models.**
