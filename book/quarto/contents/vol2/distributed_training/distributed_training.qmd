...
# │ Show:    "80" GB A100 memory and "600" GB/s NVLink H100 bandwidth.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, z={(0.6,0.5)}, x={(1,0)}, y={(0,1)}, scale=1.3]
  \tikzset{
    ...
    cube/.style={thick, gray!50, dashed},
    volume/.style={gray!10, opacity=0.5},
    label/.style={font=\bfseries, align=center},
    constraint/.style={align=center, font=\footnotesize},
    point/.style={circle, fill=black, inner sep=1.5pt},
    textBlue/.style={text=BlueLine},
    textRed/.style={text=RedLine},
    textGreen/.style={text=GreenLine}
  }
  ...
  % Axes Arrows
  \node[axis, BlueLine, label={[anchor=north east]Data Parallelism\\(Replica Count $d$)}] (xaxis) at (4.5,0,0) {};
  \node[axis, RedLine, label={[anchor=east]Pipeline Parallelism\\(Depth/Stages $p$)}] (yaxis) at (0,3.5,0) {};
  \node[axis, GreenLine, label={[anchor=south west]Tensor Parallelism\\(Layer Width $t$)}] (zaxis) at (0,0,3.5) {};

  % The "Cube" of a specific training configuration
  \begin{scope}[shift={(O)}]
    \draw[cube] (2,0,0) -- (2,2,0) -- (0,2,0);
    \draw[cube] (2,0,0) -- (2,0,2) -- (0,0,2);
    \draw[cube] (0,2,0) -- (0,2,2) -- (0,0,2);
    \draw[cube] (2,2,0) -- (2,2,2) -- (0,2,2);
    \draw[cube] (2,0,2) -- (2,2,2);
    \draw[cube] (0,2,2) -- (2,2,2);

    % Highlight the volume
    \fill[volume] (O) -- (2,0,0) -- (2,0,2) -- (0,0,2) -- cycle; % Bottom
    \fill[volume] (O) -- (0,2,0) -- (0,2,2) -- (0,0,2) -- cycle; % Side
    \fill[volume] (O) -- (2,0,0) -- (2,2,0) -- (0,2,0) -- cycle; % Back
  \end{scope}

  % Label the point
  \node[point, label={[anchor=west, xshift=2mm]Training Config $(d, p, t)$}] at (2,2,2) {};
  \node[anchor=north west, gray, font=\footnotesize] at (2,2,2) {Total Accelerators = $d \times p \times t$};

  % Contextual Constraints
  \node[constraint, textBlue] at (2.5, -0.5, 0) {constraint: Global Batch Size};
  \node[constraint, textRed] at (-0.5, 2.5, 0) {constraint: Latency/Bubbles};
  \node[constraint, textGreen] at (-0.5, 0, 2.5) {constraint: Memory/NVLink};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85, transform shape]
  ...
  \tikzset{
    ...
    shift/.style n args={2}{shift={(#1,#2)}}
  }

  % Root decision
  \node[decision=BlueLine] (d1) {Model fits in\\single GPU\\memory?};

  % YES branch
  \node[leaf=GreenLine, below left=of d1, shift={(-1.5,-0.5)}] (dp) {\textbf{Data Parallelism}\\Scale batch size};
  \node[hw, below=of dp, yshift=-2pt] {InfiniBand sufficient};
  \draw[arrow] (d1) -- node[yes, left, pos=0.3] {Yes} (dp);

  % NO branch -> second decision
  \node[decision=OrangeLine, below right=of d1, shift={(1.5,-0.5)}] (d2) {Model fits in\\single node\\(8 GPUs)?};
  \draw[arrow] (d1) -- node[no, right, pos=0.3] {No} (d2);

  % YES from d2: Tensor Parallel
  \node[leaf=BlueLine, below=of d2, shift={(0,-0.5)}] (tp) {\textbf{Tensor Parallelism}\\Split layers across GPUs};
  \node[hw, below=of tp, yshift=-2pt] {Requires NVLink};
  \draw[arrow] (d2) -- node[yes, left, pos=0.3] {Yes} (tp);

  % NO from d2 -> third decision
  \node[decision=RedLine, below=of d2, shift={(2,-0.5)}] (d3) {Sequential\\dependencies\\dominate?};
  \draw[arrow] (d2) -- node[no, right, pos=0.3] {No} (d3);

  % YES from d3: Pipeline Parallel
  \node[leaf=OrangeLine, below=of d3, shift={(0,-0.5)}] (pp) {\textbf{Pipeline Parallelism}\\Stage layers across nodes};
  \node[hw, below=of pp, yshift=-2pt] {Low BW (Ethernet OK)};
  \draw[arrow] (d3) -- node[yes, left, pos=0.3] {Yes} (pp);

  % NO from d3: 3D Hybrid
  \node[leaf=VioletLine, below=of d3, shift={(2,-0.5)}] (hybrid) {\textbf{3D Hybrid}\\TP + PP + DP};
  \node[hw, below=of hybrid, yshift=-2pt] {NVLink + IB + Ethernet};
...
3.  **Common Pitfall:** A frequent misconception is that distributed training scales linearly with node count. In practice, communication overhead grows with cluster size and the serial fraction of each step (Amdahl's Law): with 30% of a step's time spent on synchronization, the theoretical scaling ceiling is $1/0.30 \approx 3\times$ regardless of how many accelerators are added.
...
::: {.callout-principle title="Distributed Training Complexity"}
...
\tikzset{
    mycylinder/.style={cylinder, shape border rotate=90, aspect=1.3, draw, fill=white, minimum width=20mm, minimum height=9mm, line width=1pt},
    mycycle/.style={circle, draw=none, fill=red, minimum width=5mm},
    myline/.style={line width=1.15pt, draw=cyan},
    Box/.style={align=flush center, inner xsep=2pt, draw=RedLine, line width=0.75pt, fill=RedL!20, text width=22mm, minimum width=22mm, minimum height=8mm},
    Line/.style={line width=1.0pt, black!50},
    every node/.style={font=\usefont{T1}{phv}{m}{n}\small}
}
...
\draw[myline, dashed] (CO.south) -- ++(270:1) -| node[fill=white, align=center, pos=0.25] (COM) {Compare\\ predicted\\ label with\\ annotation} (ER.south);
...
\draw[myline, dashed, shorten >=3mm, shorten <=3mm] (B) -- node[fill=white] {Step 2 -- Compute gradients} (C);
\draw[myline, dashed, shorten >=3mm, pos=0.46] (C) -- node[fill=white] {Step 3 -- Update Parameters} (D);
\draw[myline, dashed, shorten >=3mm, pos=0.46] (B) -- node[fill=white] {Step 1 -- Predict a label} (A);
...
% Error and computation nodes (lower section)
...
\draw[myline, dashed] (DCO.north) -- ++(90:1) -| node[fill=white, align=center, pos=0.25] (DCOM) {Compare\\ predicted\\ label with\\ annotation} (DER.north);
...
\draw[myline, latex-, shorten <=3mm, shorten >=3mm] (DCL2) -| node[fill=white, pos=0.75] {Chunk} (SC1.south);

\node[below=0.2 of DSC2] {Forward pass};
\node[below=0.2 of DSC3] {Backward pass};

% Global calculations node
\coordinate (S1) at ($(3B1)!0.5!(4B1)$);
\coordinate (S2) at ($(D3B1)!0.5!(D4B1)$);
\coordinate (S) at ($(S1)!0.5!(S2)$);

\node[draw=none, fill=green!50!black!90, text=white, inner xsep=10pt, inner ysep=9pt, outer sep=5pt] (CGG) at (S) {\textbf{Calculate Global Gradients}};
...
3.  **Common Pitfall:** A frequent misconception is that data parallelism scales indefinitely with worker count. Scaling the effective batch size beyond the critical batch size (roughly 8,192 for ResNet-50, ~4M tokens for LLM pretraining) degrades statistical efficiency, requiring more training steps to reach target loss and eroding the throughput gains from adding more workers.
...
::: {.callout-principle title="Data Parallelism at Scale"}
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.2cm, positioning]

\tikzset{
  Line/.style={line width=0.75pt, black!50, text=black},
  Box/.style={inner xsep=2pt, line width=0.75pt, fill=VioletL2, draw=VioletLine2, text width=27mm, align=center, minimum height=9mm},
  Box2/.style={Box, draw=BlueLine, fill=BlueL, text width=21mm, minimum width=22mm},
  Text/.style={inner xsep=6pt, inner ysep=4pt, draw=none, fill=TextColor!80, font=\usefont{T1}{phv}{m}{n}\footnotesize, align=center, minimum width=22mm, minimum height=5mm},
  GradientBox/.style={Box, fill=GreenL, draw=GreenLine}
}

% GPU Nodes
\node[Box] (B1) {GPU 1\\Forward \& Backward Pass};
\node[Box, right=of B1] (B2) {GPU 2\\Forward \& Backward Pass};
\node[Box, right=of B2] (B3) {GPU 3\\Forward \& Backward Pass};
\node[Box, right=of B3] (B4) {GPU 4\\Forward \& Backward Pass};
...
% Gradient Nodes
\node[GradientBox, below=of $(B2)!0.5!(B3)$] (DB1) {Gradients GPU N};
\node[GradientBox, below=of DB1] (DB2) {Gradient Aggregation};
\node[GradientBox, below=of DB2] (DB3) {Model Update};

% Draw Arrows
\draw[Line, -latex] (GGB1) -- ++(270:1.4cm) -| (GB2);
\draw[Line, -latex] (GGB1) -- ++(270:1.4cm) -| (GB3);
\draw[Line, -latex] (GGB1) -- ++(270:1.4cm) -| (GB4);
\draw[Line, -latex] (GGB1) -- node[Text, pos=0.5, anchor=center] {Split into Non-Overlapping Subsets} ++(270:1.4cm) -| (GB1);
...
\draw[Line, -latex] (B3) -- ++(270:0.9cm) -| (DB1);
\draw[Line, -latex] (B2) -- ++(270:0.9cm) -| (DB1);
\draw[Line, -latex] (B1) -- ++(270:0.9cm) -| (DB1);
\draw[Line, -latex] (B4) -- ++(270:0.9cm) -| node[Text, pos=0.72] {Compute Gradients} (DB1);
...
\draw[Line, -latex] (GGB1.east) -- ++(0:6.8cm) |- node[Text, pos=0.8]{Next Mini-Batch} (DB3.east);
...
The backward pass mirrors this independence but introduces the system's primary bottleneck. As the GPU traverses the computation graph in reverse, it computes gradients for every parameter in the model. For a `{python} FrontierTrainingContext.frontier_params_b_str`B model in FP16, this generates a 350 GB gradient payload per GPU. While the computation itself requires zero communication, the resulting gradients represent a fractured view of the true loss surface—valid only for the local micro-batch. Before the optimizer step can occur, these local gradients must be aggregated across all $N$ GPUs to form a valid global gradient. This transition—from isolated, high-throughput compute to a massive, global synchronization event—defines the rhythm of data parallel training: long periods of silent, intense arithmetic punctuated by bursts of heavy network traffic.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85, transform shape]
  ...
  \tikzset{
     ...
     title_style/.style={anchor=south, font=\bfseries},
     note_style/.style={align=center, font=\footnotesize}
  }

  % A. Parameter Server
  \begin{scope}
    \node[title_style] (labelA) {A. Parameter Server};
    \node[ps_style, below=0.7cm of labelA] (ps) {PS};
    \node[node_style, below left=1.3cm and 0.5cm of ps] (w1) {W1};
    \node[node_style, right=0.6cm of w1] (w2) {W2};
    \node[node_style, right=0.6cm of w2] (w3) {W3};
    \node[node_style, right=0.6cm of w3] (w4) {W4};
    ...
    \node[note_style, below=0.8cm of ps] {Bottleneck at Central Node};
  \end{scope}

  % B. Ring AllReduce
  \begin{scope}[xshift=6.5cm]
    \node[title_style] (labelB) {B. Ring AllReduce};
    ...
    \node[note_style, below=2.2cm of labelB] {Bandwidth\\Optimal};
  \end{scope}

  % C. Tree AllReduce
  \begin{scope}[xshift=12.5cm]
    \node[title_style] (labelC) {C. Tree AllReduce};
    \node[node_style, below=0.7cm of labelC] (root) {R};
    \node[node_style, below left=1.2cm and 0.5cm of root] (l1) {L1};
    \node[node_style, below right=1.2cm and 0.5cm of root] (l2) {L2};
    \node[node_style, below left=1.1cm and 0.3cm of l1] (ll1) {1};
    \node[node_style, right=0.8cm of ll1] (ll2) {2};
    \node[node_style, right=0.8cm of l2] (lr1) {3};
    \node[node_style, right=0.8cm of lr1] (lr2) {4};
    ...
    \node[note_style, right=0.3cm of l2] {Low Latency\\$O(\log N)$};
  \end{scope}
...
class Scaling8GPU:
    ...
    speedup_8gpu_val = gpus_per_node * (single_gpu_step_s / (total_8gpu_ms_val / 1000))
    efficiency_8gpu_val = speedup_8gpu_val / gpus_per_node * 100
    training_hours_1gpu = 25
    training_hours_8gpu_val = training_hours_1gpu / (speedup_8gpu_val / gpus_per_node)
...
Training time: 25 hours ÷ (`{python} Scaling8GPU.speedup_8gpu_str` / 8) = **`{python} Scaling8GPU.training_8gpu_str` hours**
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import INFINIBAND_HDR_BW, GB, second, BILLION
...
class Scaling32GPU:
    ...
    speedup_32gpu_val = 32 * (single_gpu_step_s / (total_32gpu_ms_val / 1000))
    efficiency_32gpu_val = speedup_32gpu_val / 32 * 100
    training_32gpu_hours_val = training_hours_1gpu / (speedup_32gpu_val / 32)
...
class GradAccumScenario:
    ...
    ga_savings_val = Scaling32GPU.training_32gpu_hours_val * hourly_rate - ga_cost_val
    ga_savings_pct_val = ga_savings_val / (Scaling32GPU.training_32gpu_hours_val * hourly_rate) * 100
...
- Computation: `{python} Scaling32GPU.compute_32gpu_ms_str`ms (`{python} Scaling32GPU.compute_pct_str`% of time)
- Communication: `{python} Scaling32GPU.comm_32gpu_ms_str`ms (`{python} Scaling32GPU.comm_pct_str`% of time)
...
**Problem**: You are training GPT-2 on a commodity 10G network. A 32-GPU configuration was communication-bound, costing \$3,021 for a fixed number of samples. Can you achieve the same effective batch size on a single 8-GPU node using gradient accumulation more efficiently?
...
2. **Communication Overhead**: With 4-step accumulation, we AllReduce once every 4 steps.
   - Overhead = $5 \text{ ms} / (4 \times 180 \text{ ms}) \approx$ **`{python} GradAccumScenario.comm_overhead_pct_str`%**.
3. **Training Duration**: Total time reduces to **`{python} GradAccumScenario.ga_training_hours_str` hours**.
4. **Total Cost**: `{python} GradAccumScenario.ga_training_hours_str` hrs $\times$ \$128/hr = **\$`{python} GradAccumScenario.ga_cost_str`**.

**The Systems Insight**: Gradient accumulation saves **\$`{python} GradAccumScenario.ga_savings_str`** (`{python} GradAccumScenario.ga_savings_pct_str`%) by concentrating computation where bandwidth is abundant (NVLink within the node) and minimizing the frequency of synchronization. When your network is slow, don't scale out—scale the batch size locally.

:::

- Configuration: `{python} GradAccumScenario.n_gpus_ga` GPUs$\times$ batch `{python} GradAccumScenario.batch_per_gpu`$\times$ `{python} GradAccumScenario.accum_steps` accumulation steps = `{python} GradAccumScenario.effective_batch_str` effective batch
- Communication overhead: `{python} GradAccumScenario.comm_8gpu_ms`ms ÷ (`{python} GradAccumScenario.accum_steps`$\times$ `{python} GradAccumScenario.compute_8gpu_ms`ms) = `{python} GradAccumScenario.comm_overhead_pct_str`%
- Training time: `{python} GradAccumScenario.ga_training_hours_str` hours
- Cost: USD 128/hour$\times$ `{python} GradAccumScenario.ga_training_hours_str` hours = USD `{python} GradAccumScenario.ga_cost_str` vs. USD 3,021 for 32 GPUs
- Savings: USD `{python} GradAccumScenario.ga_savings_str` (`{python} GradAccumScenario.ga_savings_pct_str`% reduction) with only 1 hour longer training
...
Hardware selection critically impacts these scaling characteristics. NVIDIA DGX systems with NVLink achieve `{python} DistTrainConstants.nvlink_a100_str` GB/s bisection bandwidth, enabling 90% parallel efficiency up to 8 GPUs per node. Multi-node scaling requires InfiniBand networks, where EDR at 100 Gbps supports efficient training up to 64 nodes, while HDR at 200 Gbps enables scaling to 256+ nodes with greater than 70% efficiency.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-distributed-training-systems-systems-worked-example-convergence-comparison-8-versus-64-workers-57ce
# │
...
**Problem**: You want to train a **`{python} FrontierTrainingContext.frontier_params_b_str` Billion parameter** model (like GPT-3) on NVIDIA A100s (`{python} A100CapacityContext.a100_mem` GB). Can you use Data Parallelism with ZeRO-3?
...
**The Systems Conclusion**: 38 GB (Static) + 50 GB (Dynamic) = **88 GB**. This exceeds the `{python} A100CapacityContext.a100_mem` GB capacity of the A100. Even with full ZeRO-3 sharding, **pure Data Parallelism fails**. You *must* use **Tensor Parallelism** to split the layers themselves.
:::
...
This distribution method enables training of large-scale models. GPT-3, with `{python} FrontierTrainingContext.frontier_params_b_str` billion parameters, relies on model parallelism for training. Vision transformers processing high-resolution 16k$\times$ 16k pixel images use model parallelism to manage memory constraints. Mixture-of-Experts architectures use this approach to distribute their conditional computation paths across hardware.
...
- [ ] Can you identify two ways to split a Transformer layer: **Layer-based** versus **Tensor-based**?
...
Consider our running example: the `{python} FrontierTrainingContext.frontier_params_b_str`B parameter model requires 350 GB of memory in FP16, exceeding the `{python} A100CapacityContext.a100_mem` GB capacity of a single A100 by a factor of four. Model parallelism addresses this **capacity wall** by partitioning the model's state—parameters, gradients, and optimizer states—across multiple devices, effectively stitching them into a single super-accelerator. Unlike data parallelism, where every GPU holds a full replica of the model and processes a unique fraction of the global batch, model parallelism requires each GPU to hold a unique fraction of the model and process the *same* data stream sequentially. With 8-way partitioning on A100s, each GPU holds approximately 44 GB of parameters—a tight fit that leaves roughly 36 GB for activations and optimizer state.
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.8cm and 1.3cm, on grid, auto]
\tikzset{
  Block/.style={
    draw, line width=0.75pt, rounded corners, align=center, text width=19mm, minimum height=8mm,
    inner sep=2pt
  },
  BoxRed/.style={Block, fill=RedL, draw=RedLine},
  BoxOrange/.style={Block, fill=OrangeL, draw=OrangeLine},
  BoxGreen/.style={Block, fill=GreenL, draw=GreenLine},
  BoxBlue/.style={Block, fill=BlueL, draw=BlueLine},
  BoxViolet/.style={Block, fill=VioletL2, draw=VioletLine2},
  Line/.style={line width=1.0pt, black!50, -latex},
  BackBox/.style={
    draw=BackLine, fill=BackColor, line width=0.75pt, inner xsep=13pt, inner ysep=18pt
  }
}

\begin{scope}[start chain=going right, node distance=1.8cm]
  \node[BoxRed] (B1) {Blocks 1-6};
  \node[BoxOrange, right=of B1] (B2) {Blocks 7-12};
  \node[BoxGreen, right=of B2] (B3) {Blocks 13-18};
  \node[BoxBlue, right=of B3] (B4) {Blocks 19-24};
\end{scope}

\begin{scope}[node distance=1.3cm]
  \node[BoxViolet, below=of B1] (G1) {GPU 1};
  \node[BoxViolet, below=of B2] (G2) {GPU 2};
  \node[BoxViolet, below=of B3] (G3) {GPU 3};
  \node[BoxViolet, below=of B4] (G4) {GPU 4};
\end{scope}

\begin{scope}[on background layer, node distance=1mm]
  \node[BackBox, fit=(B1)(G1), yshift=6pt] (BB1) {};
  \node[below=of BB1.north, anchor=north] {Device 1};
  \node[BackBox, fit=(B2)(G2), yshift=6pt] (BB2) {};
  \node[below=of BB2.north, anchor=north] {Device 2};
  \node[BackBox, fit=(B3)(G3), yshift=6pt] (BB3) {};
  \node[below=of BB3.north, anchor=north] {Device 3};
  \node[BackBox, fit=(B4)(G4), yshift=6pt] (BB4) {};
  \node[below=of BB4.north, anchor=north] {Device 4};
\end{scope}
...
\foreach \i in {4,3,2} {
  \pgfmathtruncatemacro{\j}{\i - 1}
  \draw[red, Line] (B\i.230) to[out=230, in=300] (B\j.300);
}
\end{tikzpicture}
```
:::
...
# │ Show: bubble_fraction_pct ≈ 18% — inline in notebook result.
...
**Problem**: You are training a frontier model using **Pipeline Parallelism** across `{python} PipelineBubble.pb_stages_str` nodes. To hide the sequential delay, you split your batch into `{python} PipelineBubble.pb_microbatches_str` microbatches. What is your "Bubble Tax"—the fraction of GPU cycles lost to idle waiting?
...
```{.tikz}
\begin{tikzpicture}[
    every node/.style={font=\small\usefont{T1}{phv}{m}{n}, draw, minimum width=1cm, minimum height=0.7cm, align=center, outer sep=0, line width=0.75pt},
    fill0/.style={fill=RedL},
    fill1/.style={fill=BlueL},
    fill2/.style={fill=OrangeL},
    fill3/.style={fill=GreenL},
    update/.style={fill=Yellow!50},
    node distance=0.2cm and 0.5cm,
    on grid
]

% Forward Pass Layers
\begin{scope}[start chain=going right, node distance=0.5cm]
    \node[fill0, on chain] (F0_0) {$F_{0,0}$};
    \foreach \x in {1, 2, 3} {
        \node[fill0, on chain] (F0_\x) {$F_{0,\x}$};
    }
\end{scope}

\begin{scope}[start chain=going right, node distance=0.5cm, above=of F0_0]
    \node[fill1, on chain] (F1_0) {$F_{1,0}$};
    \foreach \x in {1, 2, 3} {
        \node[fill1, on chain] (F1_\x) {$F_{1,\x}$};
    }
\end{scope}

\begin{scope}[start chain=going right, node distance=0.5cm, above=of F1_0]
    \node[fill2, on chain] (F2_0) {$F_{2,0}$};
    \foreach \x in {1, 2, 3} {
        \node[fill2, on chain] (F2_\x) {$F_{2,\x}$};
    }
\end{scope}

\begin{scope}[start chain=going right, node distance=0.5cm, above=of F2_0]
    \node[fill3, on chain] (F3_0) {$F_{3,0}$};
    \foreach \x in {1, 2, 3} {
        \node[fill3, on chain] (F3_\x) {$F_{3,\x}$};
    }
\end{scope}

% Backward Pass Layers
\begin{scope}[start chain=going left, node distance=0.5cm, right=of F3_3]
    \node[fill3, on chain] (B3_3) {$B_{3,3}$};
    \foreach \x in {2, 1, 0} {
        \node[fill3, on chain] (B3_\x) {$B_{3,\x}$};
    }
\end{scope}

\begin{scope}[start chain=going left, node distance=0.5cm, below=of B3_2]
    \node[fill2, on chain] (B2_3) {$B_{2,3}$};
    \foreach \x in {2, 1, 0} {
        \node[fill2, on chain] (B2_\x) {$B_{2,\x}$};
    }
\end{scope}

\begin{scope}[start chain=going left, node distance=0.5cm, below=of B2_2]
    \node[fill1, on chain] (B1_3) {$B_{1,3}$};
    \foreach \x in {2, 1, 0} {
        \node[fill1, on chain] (B1_\x) {$B_{1,\x}$};
    }
\end{scope}

\begin{scope}[start chain=going left, node distance=0.5cm, below=of B1_2]
    \node[fill0, on chain] (B0_3) {$B_{0,3}$};
    \foreach \x in {2, 1, 0} {
        \node[fill0, on chain] (B0_\x) {$B_{0,\x}$};
    }
\end{scope}

% Update Nodes
\begin{scope}[start chain=going up, node distance=0.2cm, right=of B0_0]
    \node[update, on chain] (U0_0) {Update};
    \foreach \x in {1, 2, 3} {
        \node[update, on chain] (U0_\x) {Update};
    }
\end{scope}
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=0.5cm and 1cm]
  ...
  \tikzset{
    ...
    input_box/.style={draw, fill=gray!10, minimum width=2cm, minimum height=0.5cm, anchor=center},
    gpu_text/.style={below=0.2cm of X, text width=3cm, align=center}
  }

  % Column Parallel (Layer 1)
  \begin{scope}
    \node[anchor=south] at (0, 3.5) {\textbf{Column Parallel (e.g., QKV)}};
    \node[anchor=south] at (0, 3.2) {$W_A = [W_{A1} | W_{A2}]$};

    % Input X
    \node[input_box] (X) {$X$};

    % Split Matrix
    \node[split_v, below=of X, anchor=center] (W1) {$W_{A1}$};
    \node[split_v, right=0cm of W1] (W2) {$W_{A2}$};

    % GPUs Processing
    \node[gpu_text, below=0.7cm of W1] (GPU) {GPUs compute $Y_i = X W_{Ai}$};

    \draw[arrow] (X.south) -- (W1.north);
    \draw[arrow] (X.south) -- (W2.north);
  \end{scope}

  % Symbol
  \node[above=of X] (arrowSymbol) {\Huge $\rightarrow$};
  \node[align=center, font=\footnotesize] at (0, -3.5) {Activation\\Pass};

  % Row Parallel (Layer 2)
  \begin{scope}[xshift=5.5cm]
    \node[anchor=south] at (0, 3.5) {\textbf{Row Parallel (e.g., Output)}};
    \node[anchor=south] at (0, 3.2) {$W_B = [W_{B1} ; W_{B2}]$};

    % Inputs
    \node[draw, fill=SplitColor, minimum width=1cm, minimum height=0.5cm, anchor=center] (Y1) at (0, -1) {$Y_1$};
    \node[draw, fill=MatrixColor, minimum width=1cm, minimum height=0.5cm, right=1cm of Y1] (Y2) {$Y_2$};

    % Split Matrix
    \node[split_h, above=of Y1, anchor=center] (WB1) {$W_{B1}$};
    \node[split_h, below=0cm of WB1, fill=MatrixColor] (WB2) {$W_{B2}$};

    % Output
    \node[draw, minimum width=2cm, minimum height=0.5cm, above=2cm of WB1] (Z) {$Z = Y_1 W_{B1} + Y_2 W_{B2}$};

    \draw[arrow] (Y1.west) -- (WB1.south|-Y1.west);
    \draw[arrow] (Y2.east) -- (WB2.south|-Y2.east);
...
Designing the training topology for GPT-3 (175B parameters) requires balancing memory, bandwidth, and compute across three dimensions. A standard configuration uses **Tensor Parallelism (TP=8)** to shard layers within a single node, exploiting the 900 GB/s NVLink bandwidth. **Pipeline Parallelism (PP=16)** then splits the model depth across 16 nodes, tolerant of the slower inter-node bandwidth (InfiniBand) due to bubble overhead. Finally, **Data Parallelism (DP=128)** replicates this entire 16-node pipeline 128 times to scale throughput. The total hardware requirement is massive: $8 \text{ GPUs} \times 16 \text{ Stages} \times 128 \text{ Replicas} =$ `{python} ThreeDParallelism.total_gpus_h100_str` **GPUs**.
...
- [ ] If you have a total budget of 1,024 GPUs, what is the trade-off between increasing **DP** (Data Parallelism) versus increasing **PP** (Pipeline Parallelism)?
...
class RLHFBudget:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(policy_mem == 1120, f"policy memory ({policy_mem})")
    check(n_gpus == 32, f"GPU count ({n_gpus})")
    check(policy_per_gpu == 35.0, f"policy per GPU ({policy_per_gpu})")
    check(value_mem == 208, f"value mem ({value_mem})")
    check(abs(static_per_gpu - 39.4) < 0.1, f"static/GPU ({static_per_gpu:.1f})")
...
\tikzset{
  Line/.style={line width=1.0pt,black!50,text=black},
  Box/.style={inner xsep=2pt,node distance=16mm,
    draw=GreenLine, line width=0.75pt,
    fill=GreenL, text width=27mm, align=flush center,
    minimum width=27mm, minimum height=9mm, anchor=center},
  Box1/.style={Box,
    draw=RedLine, fill=RedL,
    text width=31mm, minimum width=32mm, minimum height=10mm},
  decision/.style={align=flush center, text width=42mm, diamond, aspect=2.2,
    node distance=18mm, inner xsep=-3pt, inner ysep=-2.95ex,
    fill=VioletL2, draw=VioletLine, anchor=center},
  startbox/.style={Box,fill=BlueL, draw=BlueLine},
  endbox/.style={Box,fill=BlueL, draw=BlueLine, node distance=15mm}
}

\node[startbox](G5B3){Start};
\node[decision,below=of G5B3](G4B3){Does\\ the model fit in a\\ single device?};
\node[decision,below=of G4B3](G3B3){Does\\ the dataset fit in a\\ single device?};
\node[decision,below=of G3B3](G2B3){Are\\ both constraints\\ significant?};
\node[Box1,below=of G2B3](G1B3){Is scaling the model\\ or data more critical?};
\node[decision,right=of G1B3](G1B4){Is\\ the dataset\\ very large?};

\node[Box,below=of G1B3](B1){Hybrid\\ Parallelism};
\node[Box,right=of B1](B2){Model\\ Parallelism};
\node[Box,right=of B2](B3){Data\\ Parallelism};
\node[Box,right=of B3,fill=RedL, draw=RedLine](B4){Single Device Optimization};

\node[endbox,below=of B2](DB2){End};

\begin{scope}[on background layer]
  \node[draw=BackLine,inner xsep=5mm,inner ysep=5mm,
    yshift=-1mm, fill=BackColor, fit=(B1)(B3), line width=0.75pt](BB){};
...
\draw[Line,-latex](G3B3.west)-|node[above,pos=0.25]{Yes}(B2.west);
...
# │ Exports: YoungDaly.t_opt_min_str, YoungDaly.loss_pct_str,
...
The Young-Daly formula establishes the optimal checkpoint interval as $T_{\text{opt}} = \sqrt{2 \times C \times MTBF}$, where $C$ is checkpoint time and MTBF is mean time between failures. For a 1024-GPU cluster with 4-hour MTBF and 5-minute checkpoint time, the optimal interval is approximately **`{python} YoungDaly.t_opt_min_str` minutes**. Checkpointing every 15 minutes "to be safe" wastes `{python} YoungDaly.loss_pct_str`% of compute time on unnecessary checkpoint overhead, while checkpointing every 2 hours risks losing significant work on failure. For larger models where checkpoint time increases to 15 minutes due to model size and storage bandwidth, the optimal interval shifts significantly. The cost of suboptimal checkpointing scales with cluster size: a 1024-GPU cluster loses approximately **USD `{python} YoungDaly.daily_savings_str` per day** to excessive checkpointing when using arbitrary intervals instead of the Young-Daly optimal.
...
\begin{tikzpicture}[scale=1.5, font=\small\usefont{T1}{phv}{m}{n}, line join=round]
  ...
  % Initial position of the 3D block
  \begin{scope}[start chain=going right, node distance=2.5cm]

    % Replicas (Data Parallel)
    \foreach \d in {0, 1} {
      \begin{scope}[start chain=going below, node distance=1.2cm, on chain]

        % Layers (Pipeline Parallel)
        \foreach \p in {0, 1} {
          \begin{scope}[start chain=going right, node distance=1.2cm, on chain]

            % Shards (Tensor Parallel)
            \foreach \t in {0, 1} {
              \node[gpu=BlueL] (data\d\p\t) [on chain] {\tiny GPU};
              \node[gpu=OrangeL] (tensor\d\p\t) [right=of data\d\p\t, on chain] {};
              \node[gpu=GreenL] (pipe\d\p\t) [above=of data\d\p\t, on chain] {};
            }
          \end{scope}
        }
        ...
        \draw[brace, GreenL]
          ([yshift=-0.1cm]data\d00.south west) --
          ([yshift=0.1cm]data\d10.north west)
          node[midway, left=8pt, label] {$p$ stages};

        % Bracket for Tensor
        \draw[brace, OrangeL]
          ([xshift=-0.1cm]data\d00.south west) --
          ([xshift=0.1cm]tensor\d00.south)
...
\begin{tikzpicture}[scale=1.2, font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \definecolor{BlueL}{RGB}{173,216,230}
  \definecolor{RedLine}{RGB}{255,0,0}
  \definecolor{RedL}{RGB}{240,128,128}
  \definecolor{GreenLine}{RGB}{0,128,0}
  \definecolor{GreenL}{RGB}{144,238,144}
  \definecolor{OrangeLine}{RGB}{255,165,0}
  \definecolor{OrangeL}{RGB}{255,222,173}

  \tikzset{
    cube/.style={thick,fill opacity=0.7},
    datalayer/.style={fill=DataColor!30},
    tensorlayer/.style={fill=TensorColor!30},
    pipelayer/.style={fill=PipeColor!30},
    axis/.style={->, thick},
    textnode/.style={font=\small\usefont{T1}{phv}{m}{n}, anchor=center},
  }

  % Axes
  \draw[axis] (0,0,0) -- (3,0,0) node[right] {Data Parallel ($d$)};
  \draw[axis] (0,0,0) -- (0,3,0) node[above] {Pipeline Parallel ($p$)};
  \draw[axis] (0,0,0) -- (0,0,3) node[below left] {Tensor Parallel ($t$)};

  % Create the cuboids
  % Data Parallel Cuboid
  \begin{scope}[canvas is zy plane at x=0]
    \node[cube, datalayer] (data) at (1, 1) [minimum width=2cm, minimum height=2cm] {};
  \end{scope}

  % Tensor Parallel Cuboid
  \begin{scope}[canvas is zx plane at y=2]
    \node[cube, tensorlayer] (tensor) at (1, 1) [minimum width=2cm, minimum height=2cm] {};
  \end{scope}

  % Pipeline Parallel Cuboid
  \begin{scope}[canvas is xy plane at z=2]
    \node[cube, pipelayer] (pipe) at (1, 1) [minimum width=2cm, minimum height=2cm] {};
  \end{scope}

  % Text nodes
  \node[textnode] at (1,1,2) {Model Replicas};
  \node[textnode, rotate=90] at (2.2,1,1) {Intra-layer};
  \node[textnode] at (1,2.2,1) {Inter-layer};
\end{tikzpicture}
```
