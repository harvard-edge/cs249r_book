...
::: {#fig-loop-transforms fig-env="figure" fig-pos="htb" fig-cap="**Distributed Strategies as Loop Transformations**: The logical training loop nests over epochs, batches, and layers. Distributed strategies are loop transformations applied at cluster scale: Data Parallelism unrolls the batch loop across devices; Tensor Parallelism vectorizes the per-layer matrix operations across devices; Pipeline Parallelism pipelines the layer sequence across devices." fig-alt="Two-block flowchart. Left block shows the logical training loop nested over epochs, batches, and layers. Right block shows the three physical implementations: Data Parallelism unrolls the batch loop, Tensor Parallelism vectorizes layer operations, Pipeline Parallelism pipelines the layer sequence. A heavy arrow connects logical to physical."}
...
**Root Cause Diagnosis**: NCCL is already using a hierarchical algorithm (100 ms versus the 240 ms flat-ring prediction confirms this). The remaining 20 ms gap between observed and modeled latency comes from switch congestion on a handful of inter-node uplinks, not from a suboptimal algorithm.

**Solution**: Monitor InfiniBand switch port utilization to identify hot spots. Consider rail-optimized topology (@sec-compute-infrastructure) or further tuning of the hierarchical AllReduce hand-off so it explicitly partitions intra-node (NVLink) from inter-node (InfiniBand) communication. The residual 20 ms gap likely represents achievable optimization through better network provisioning rather than algorithm changes.
...
::: {#fig-3d-parallelism-cube-summary fig-env="figure" fig-pos="htb" fig-cap="**3D Parallelism Strategy Space**: The three independent axes of distributed training parallelism. Data Parallelism ($d$) replicates models across workers. Pipeline Parallelism ($p$) partitions model layers vertically by depth (assigning layers to different stages). Tensor Parallelism ($t$) splits individual operations horizontally within each layer (assigning shards to different GPUs). A single training step coordinates $d \times p \times t$ GPUs, with tensor-parallel communication within layers, pipeline communication at microbatch stage boundaries, and data-parallel synchronization once per optimizer step. Archetype A occupies a coordinate $(d, t, p)$ in this cube to balance memory and bandwidth." fig-alt="Three-dimensional cube diagram with axes labeled Data Parallel, Pipeline Parallel, and Tensor Parallel. Colored planes show model replicas, intra-layer partitioning, and pipeline stages."}
