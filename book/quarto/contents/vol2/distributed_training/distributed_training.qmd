...
This equation reveals the **Scaling Wall**: as $N$ increases, the compute term ($T_{compute}/N$) shrinks, but the communication term ($T_{comm}$) remains constant or grows. Eventually, the denominator is dominated by communication, driving efficiency toward zero.

::: {.callout-perspective title="Systems Perspective: The Energy Tax of Scale"}
...
**The Limitations**:
*   **The Memory Wall**: Every GPU must hold a full copy of the model parameters, gradients, and optimizer states. For a 175B parameter model, this requires $>1$TB of memory per GPU, which is physically impossible on today's hardware without ZeRO sharding.
...
::: {.callout-note title="Archetype B (Scaled Lighthouse): Embedding Sharding"}
**Archetype B (The Global Recommendation Engine)** faces a different memory wall than GPT-4. While LLMs are bound by the *computation* of dense layers, Archetype B is bound by the *capacity* of **Embedding Tables**. 

A model serving billions of items can have embedding tables exceeding 10TB. This cannot fit in GPU memory, even with 3D parallelism. Instead, Archetype B uses **Embedding Sharding**: partitioning the tables across the CPU DRAM of hundreds of nodes. During the forward pass, the system performs **Sparse Lookups** across the network. The challenge here is not AllReduce throughput, but **All-to-All Latency** and the ability of the network to handle millions of small, unpredictable lookups simultaneously.
:::
...
**The Advantages**:
*   **Memory Scaling**: Enables training of models that exceed single-device memory. With 8-way tensor parallelism, a 175B model fits comfortably on A100s.
...
The parallelism strategies examined here define the *logical* traffic patterns of the Machine Learning Fleet. However, logic must eventually run on physics. Each strategy creates specific demands on the hardware:
*   **Tensor Parallelism** demands massive bisection bandwidth within a node (NVLink).
*   **Pipeline Parallelism** demands low-latency point-to-point links between nodes.
*   **Data Parallelism** demands high-throughput AllReduce across the entire cluster fabric.

In **Part II: Building the Machine Learning Fleet**, we will construct the physical supercomputer capable of satisfying these demands, starting with **Compute Infrastructure** (@sec-compute). But first, we must optimize the communication primitives themselves. The next chapter, @sec-communication-collective-operations, opens the black box of `dist.all_reduce` to reveal the ring and tree algorithms that make distributed consistency possible.
