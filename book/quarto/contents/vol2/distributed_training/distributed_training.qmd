...
The parallelism strategies examined here determine how we coordinate learning across the Machine Learning Fleet. However, orchestrating these complex schedules across thousands of nodes introduces a new problem: statistical inevitability. The more we split our model, the more points of failure we create.

The next chapter, @sec-fault-tolerance, examines the reliability mechanisms—checkpointing and elastic training—that allow these massive distributed jobs to finish despite inevitable hardware failures.
