...
class CommScalingMath:
    ...
    scaling_efficiency_str = fmt(scaling_efficiency * 100, precision=1, commas=False, suffix=" percent")
    ...
    overlap_pct_str = fmt(overlap_pct, precision=0, commas=False, suffix=" percent")
    scaling_efficiency_pct_str = fmt(scaling_efficiency * 100, precision=1, commas=False, suffix=" percent")
...
```{python}
#| echo: false
#| label: distributed-training-archetype-table
# ┌─────────────────────────────────────────────────────────────────────────────
# │ PARTITIONING STRATEGY BY ARCHETYPE
# ├─────────────────────────────────────────────────────────────────────────────
# │ Goal: Keep the A/B/C partitioning summary tied to Scenarios.Archetypes.Fleet.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Scenarios as BookScenarios
from mlsysim.fmt import MarkdownStr

class DistributedTrainingArchetypeTable:
    """Display fields for partitioning strategy by fleet archetype."""

    a = BookScenarios.Archetypes.Fleet.ArchetypeA
    b = BookScenarios.Archetypes.Fleet.ArchetypeB
    c = BookScenarios.Archetypes.Fleet.ArchetypeC

    a_label_str = MarkdownStr(a.label)
    a_strategy_str = MarkdownStr(a.partitioning_strategy)
    a_logic_str = MarkdownStr(
        f"Combine Tensor (width), Pipeline (depth), and Data (throughput) "
        f"to fit {DistTrainSummaryRecap.gpt4_mem_tb_str} of weights."
    )
    b_label_str = MarkdownStr(b.label)
    b_strategy_str = MarkdownStr(b.partitioning_strategy)
    b_logic_str = MarkdownStr(
        f"Partition massive {DistTrainSummaryRecap.dlrm_mem_tb_str}+ tables "
        "across a Parameter Server fleet; use sparse AllToAll updates."
    )
    c_label_str = MarkdownStr(c.label)
    c_strategy_str = MarkdownStr(c.partitioning_strategy)
    c_logic_str = MarkdownStr(c.partitioning_logic)
```

| **Archetype**                                                | **Primary Partitioning Strategy**                           | **The Logic**                                            |
|:-------------------------------------------------------------|:------------------------------------------------------------|:---------------------------------------------------------|
| **`{python} DistributedTrainingArchetypeTable.a_label_str`** | `{python} DistributedTrainingArchetypeTable.a_strategy_str` | `{python} DistributedTrainingArchetypeTable.a_logic_str` |
| **`{python} DistributedTrainingArchetypeTable.b_label_str`** | `{python} DistributedTrainingArchetypeTable.b_strategy_str` | `{python} DistributedTrainingArchetypeTable.b_logic_str` |
| **`{python} DistributedTrainingArchetypeTable.c_label_str`** | `{python} DistributedTrainingArchetypeTable.c_strategy_str` | `{python} DistributedTrainingArchetypeTable.c_logic_str` |
