...
Choosing the right parallelism strategy is not a matter of preference; it is a constraint satisfaction problem governed by model size ($M$), batch size ($B$), and interconnect bandwidth.
...
The parallelism strategies examined here determine how we coordinate learning across the Machine Learning Fleet. However, splitting computation is only half the battle; the pieces must talk to each other. The efficiency of every strategy we just discussed—Data, Tensor, and Pipeline Parallelism—depends entirely on the speed and latency of the underlying synchronization primitives.

The next chapter, @sec-communication, dives deep into the **Communication** layer, analyzing the collective operations (like AllReduce and AllGather) that bind our distributed logic into a coherent system.
