...
The low efficiency (`{python} CommScalingMath.scaling_efficiency_pct_str`) shows why naive data parallelism at this scale is insufficient. <!-- lego-ok-block: hierarchy-aware scaling efficiency range -->
Hierarchy-aware systems can reach 70--90 percent efficiency by combining data parallelism with tensor parallelism (which communicates over NVLink) and pipeline parallelism (which overlaps computation with communication).
<!-- end lego-ok-block -->
