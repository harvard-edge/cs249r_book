...
import numpy as np
# Ensure mlsys directory is in path
sys.path.append('../../../mlsys')
import viz
...
- Speedup: `{python} single_gpu_step_s_str`s ÷ `{python} total_8gpu_s_str`s = `{python} speedup_8gpu_str`× (not quite 8×)
...
- Speedup: `{python} single_gpu_step_s_str`s ÷ `{python} total_32gpu_s_str`s = `{python} speedup_32gpu_str`× faster → `{python} training_32gpu_str` hours
...
- Cost: USD 128/hour × `{python} ga_training_hours` hours = USD `{python} ga_cost_str` vs. USD 3,021 for 32 GPUs
