...
**Systems cost**: Steps 2 and 6 still pay fill and drain slots, so for fixed $p$ and $m$ the bubble remains approximately $(p-1)/(m+p-1)$. Steps 4 and 5 create the memory gain: interleaving backward work frees activations earlier than GPipe-style all-forward-then-all-backward scheduling, which lets the system raise $m$ until activation memory, recomputation, or the global-batch constraint becomes the next limit.
...
A common fallacy is this: *Linear speedup is achievable with sufficient engineering effort.*
...
A recurring pitfall is this: *Optimizing MFU without considering scaling efficiency.*
...
A common fallacy is this: *Hyperparameters tuned on small clusters transfer directly to large-scale training.*
...
A recurring pitfall is this: *Adding GPUs to data-parallel jobs without modeling communication.*
...
A common fallacy is this: *Memory capacity alone determines the parallelism strategy.*
...
A recurring pitfall is this: *Applying FSDP or ZeRO without measuring efficiency trade-offs.*
...
A common fallacy is this: *Parallelism overhead is roughly constant regardless of model size.*
...
A recurring pitfall is this: *Treating scaling efficiency as a property of the hardware alone.*
...
A common fallacy is this: *Gradient accumulation is free.*
...
A recurring pitfall is this: *Using fixed checkpoint intervals regardless of system characteristics.*
