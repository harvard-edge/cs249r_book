...
**Scenario**: An AllReduce of a `{python} HierarchicalAllReduceDebug.tensor_gb_str` gradient tensor across `{python} HierarchicalAllReduceDebug.nodes_str` ({python} HierarchicalAllReduceDebug.total_gpus_str` GPUs) takes `{python} HierarchicalAllReduceDebug.observed_ms_str`.
...
**Scenario**: An engineering team schedules a `{python} AutoParallelism.params_b_str` parameter model across `{python} AutoParallelism.gpu_count_str` H100 GPUs (80 GB HBM each), navigating the multi-dimensional 3D-parallelism search space (TP $\times$ PP $\times$ DP).

**Diagnosis**: Manual trial-and-error risks memory overflow (exceeding 80 GB HBM) or saturating NVLink. The `ParallelismOptimizer` evaluates all factorizations, selecting TP=`{python} AutoParallelism.opt_tp_str`, PP=`{python} AutoParallelism.opt_pp_value_str`, and DP=`{python} AutoParallelism.opt_dp_value_str` to maximize MFU without exceeding HBM capacity.
