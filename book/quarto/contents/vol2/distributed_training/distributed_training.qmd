...
Published large-batch regimes illustrate the scale rather than providing fixed thresholds:

- **ImageNet ResNet-50**: Goyal et al. stabilized global batch 8,192 with warmup; LARS extends ImageNet large-batch training further in some settings [@goyal2017accurate; @you2017large].
- **BERT-Large pretraining**: LAMB reports BERT training at 32K--64K batch sizes with layer-wise adaptive updates [@you2020large].
- **Other domains**: McCandlish et al. show useful batch-size limits ranging from tens of thousands in ImageNet-like settings to millions in reinforcement-learning workloads, so new frontier-language-model thresholds should be measured rather than copied [@mccandlish2018empirical].
