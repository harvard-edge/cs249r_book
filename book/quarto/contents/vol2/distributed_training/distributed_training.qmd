...
@Sec-compute-infrastructure established the accelerator hierarchy, @sec-network-fabrics wired nodes into a high-bandwidth fabric, and @sec-data-storage completed the infrastructure with pipelines that keep the fleet fed. With the physical foundation in place, the algorithmic challenge is splitting a single training job across this hardware.
...
# │
# │ Imports: mlsysim.physics.calc_activation_memory + fmt helpers
# │ Exports: Gpt3ActivationBudget.act_unchecked_tb_str,
# │          Gpt3ActivationBudget.act_checkpointed_gb_str
# └─────────────────────────────────────────────────────────────────────────────
...
- **Slower network path**: InfiniBand HDR at `{python} HierarchicalAllReduceDebug.ib_hdr_gbit_per_s_str` (`{python} HierarchicalAllReduceDebug.ib_hdr_bw_gb_per_s_str`) per port
...
from mlsysim.fmt import fmt, check, fmt_int, fmt_memory_capacity, fmt_params, fmt_qty_int, MarkdownStr
...
class DistTrainA100Memory:
    ...
    # The delimiter is assembled at runtime so no literal display-math delimiter
    # appears in the source text, which would confuse the index/math scanners.
    _dd = "$" + "$"
    ...
    total_state_display_math = MarkdownStr(f"{_dd} {_total_head} {_total_tail} {_dd}")
    ...
    zero3_per_gpu_display_math = MarkdownStr(f"{_dd} {_z3_body} {_dd}")
```
...
# │          ScalingWorkers.base_factor_str, ScalingWorkers.comm_8bsp_mult_str,
# │          ScalingWorkers.comm_64bsp_mult_str, ScalingWorkers.comm_ssp_mult_str
# └─────────────────────────────────────────────────────────────────────────────
import math
...
class ScalingWorkers:
    ...
    base_factor_str = MarkdownStr("1.0")
...
# │ Exports: pb_stages_str, pb_microbatches_str, pb_p_minus_1_str, pb_bubble_pct_str,
# │          pb_bubble_pct_round_str
# └─────────────────────────────────────────────────────────────────────────────
...
\vspace*{-4mm}
```
<!-- TODO(nvlink-convention 2026-06-10): regenerate SVG to show the per-direction cliff (NVLink 450 GB/s one-way versus NDR 50 GB/s per port, a factor of nine) to match the body; the baked-in annotation compares NVLink's 900 GB/s bidirectional total against a per-port rate, mixing conventions and showing a factor of eighteen. No matplotlib source in repo; flagged for regeneration. -->
![](images/svg/vol2_distributed_training_margin_004.svg){width="100%" fig-alt="Violet bandwidth ladder comparing NVLink with NDR InfiniBand, illustrating the order-of-magnitude bandwidth drop from intra-node to inter-node links."}
...
# │          DistTrainH100Capacity.policy_params_b_str,
# │          DistTrainH100Capacity.policy_train_gb_str,
# │          DistTrainH100Capacity.bytes_weights_str / bytes_grad_str / bytes_adam_str
from mlsysim import Hardware, Models
from mlsysim.core.units import GiB, Bparam, GB, byte, param, BYTES_FP16
from mlsysim.fmt import fmt, MarkdownStr, fmt_memory_capacity, fmt_qty, fmt_memory, fmt_params
from mlsysim.physics import memory_from_params
import math
...
1. The **policy model**\index{Policy Model} (the model being aligned) requires full training state: parameters, gradients, optimizer moments, and activations. For a `{python} DistTrainH100Capacity.policy_params_b_str` parameter model in mixed precision with Adam, this consumes approximately `{python} DistTrainH100Capacity.policy_params_b_str` $\times$ (`{python} DistTrainH100Capacity.bytes_weights_str` $+$ `{python} DistTrainH100Capacity.bytes_grad_str` $+$ `{python} DistTrainH100Capacity.bytes_adam_str`) = `{python} DistTrainH100Capacity.policy_train_gb_str` of memory—the same budget as standard pretraining.
