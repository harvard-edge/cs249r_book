...
Standard pretraining involves one model, one loss function, and one gradient stream. RLHF alignment, by contrast, orchestrates a system of models that interact during every training step. In Proximal Policy Optimization\index{Proximal Policy Optimization!model asymmetry} (PPO) [@schulman2017proximal], as used in InstructGPT-style RLHF systems [@ouyang2022instructgpt], four distinct models must operate in concert. @Tbl-rlhf-model-coordination-roles separates those models by role, execution mode, and memory burden.
