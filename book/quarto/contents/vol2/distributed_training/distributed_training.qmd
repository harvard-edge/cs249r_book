...
The parallelism strategies examined here determine how we coordinate learning across the Machine Learning Fleet. However, as we scale to thousands of devices, the probability of any single component failing becomes a statistical certainty. A system that halts entirely when one GPU fails cannot sustain weeks of training. The next chapter (@sec-fault-tolerance) examines how to build resilient systems that treat failure as a routine event rather than a catastrophic exception.
