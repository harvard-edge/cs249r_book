...
To maintain consistency across the distributed system, the gradients computed by each device must be synchronized. This coordination represents a distributed systems challenge in achieving global consensus while minimizing communication complexity. The ring all-reduce algorithm exemplifies this trade-off by organizing devices in a logical ring where each GPU communicates only with its neighbors. @fig-coll-comm contrasts three synchronization topologies: the bottlenecked parameter server architecture, the bandwidth-optimal ring AllReduce, and the low-latency tree AllReduce, each presenting distinct trade-offs between bandwidth utilization and latency scaling. The algorithm complexity is O(n) in communication rounds but requires sequential dependencies that can limit parallelism.
