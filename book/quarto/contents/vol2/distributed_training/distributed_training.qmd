...
```{python}
#| label: frontier-training-context
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ GPT-3 TRAINING CONTEXT (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
...
- Speedup: `{python} Scaling8GPU.single_gpu_step_s_str`s ÷ `{python} Scaling8GPU.total_8gpu_s_str`s = `{python} Scaling8GPU.speedup_8gpu_str`$\times$
- Parallel efficiency: `{python} Scaling8GPU.speedup_8gpu_str` ÷ 8 = `{python} Scaling8GPU.efficiency_8gpu_str`%
...
- Inter-node communication: `{python} Scaling8GPU.gpt2_sync_size_gb_str`GB @ `{python} Scaling32GPU.ib_hdr_gbs` GB/s (InfiniBand HDR) $\approx$ `{python} Scaling32GPU.inter_node_ms_str`ms
...
- Speedup: `{python} Scaling8GPU.single_gpu_step_s_str`s ÷ `{python} Scaling32GPU.total_32gpu_s_str`s = `{python} Scaling32GPU.speedup_32gpu_str`$\times$ faster → `{python} Scaling32GPU.training_32gpu_str` hours
- Parallel efficiency: `{python} Scaling32GPU.speedup_32gpu_str` ÷ 32 = `{python} Scaling32GPU.efficiency_32gpu_str`%
...
::: {.callout-definition title="AllReduce Communication Complexity"}
...
# │ Exports: batch_8_str, batch_64_str, exp_iter_8_str, exp_iter_64_str,
# │          speedup_8_str, speedup_64_str, speedup_ssp_str, lr_ssp_str,
# │          base_iters_k_str, base_batch_str, n_workers_8_str,
# │          n_workers_64_str, ssp_staleness_str, critical_batch_str
# └─────────────────────────────────────────────────────────────────────────────
import math
...
# │ Exports: DistTrainConstants.a100_mem_str, mwa_per_gpu_str, mwa_activation_str
# └─────────────────────────────────────────────────────────────────────────────
...
::: {.callout-definition title="Tensor vs. Pipeline Parallelism"}
...
# │ Exports: h100_hbm_bw_str, DistTrainConstants.nvlink_h100_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │ Exports: total_gpus_h100_str, nvlink5_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.formatting import check
...
# │
# │ Imports: mlsys.formatting (check)
# │ Exports: policy_mem_str, ref_mem_str, rlhf_n_gpus_str,
# │          policy_per_gpu_str, ref_per_gpu_str, ref_per_gpu_sep_str,
# │          reward_mem_str, value_mem_str, static_per_gpu_str,
# │          rlhf_remaining_str, kv_per_gpu_str, rlhf_remaining_kv_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.formatting import check
