...
::: {#fig-3d-parallelism-cube fig-env="figure" fig-pos="htb"}
```{.tikz}
...
::: {#fig-coll-comm fig-env="figure" fig-pos="htb"}
```{.tikz}
...
\end{tikzpicture}
```
**Gradient Synchronization Topologies**. Visual comparison of communication patterns. (A) **Parameter Server** uses a central node, creating bandwidth bottlenecks at the server. (B) **Ring AllReduce** distributes bandwidth evenly across all links but has linear latency scaling. (C) **Tree AllReduce** reduces latency to logarithmic time but may congest links near the root.
:::
...
::: {#fig-tensor-parallel-split fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
...
::: {#fig-hybrid-mapping fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
