...
Gradient synchronization is where independent SGD estimates become one update, so its cost determines whether data parallelism still behaves like a scaling strategy rather than a network benchmark. The immediate training requirement is simple: every replica must apply the same averaged gradient before it moves to the next step. The hard part is making that averaging fast enough. @Sec-collective-communication opens that box by deriving the ring, tree, and hierarchical collective algorithms that implement the averaging operation on real fabrics.
