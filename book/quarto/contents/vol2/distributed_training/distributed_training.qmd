...
class DistTrainSetup:
    ...
    h100_hbm_bw = f"{h100_hbm_bw.m_as(TB/second):.2f}"
...
h100_hbm_bw_str = DistTrainSetup.h100_hbm_bw
...
1.  **Significance (Quantitative):** It enables the training of models that exceed the **Memory Capacity** and **Compute Throughput** ($R_{peak}$) of a single device, reducing the total wall-clock time $T$ for a given $O$.
...
3.  **Common Pitfall:** A frequent misconception is that performance scales linearly with the number of devices ($N$). In reality, the **Communication Overhead ($L_{lat}$)** and **Bisection Bandwidth ($BW$)** eventually create a scaling ceiling (Amdahl's Law).
...
1.  **Significance (Quantitative):** It represents the **Synchronization Error** in distributed optimization. Increasing $\tau$ can improve **Throughput ($\eta$)** by reducing barrier waits ($L_{lat}$), but it typically degrades the **Rate of Convergence**, requiring more operations ($O$) to reach the same accuracy.
...
1.  **Significance (Quantitative):** It marks the transition point for **Parallel Scaling Efficiency**. Below $B^*$, increasing the batch size linearly improves the convergence per step. Above $B^*$, larger batches yield diminishing returns, requiring proportionally more samples ($D_{vol}$) to reach the same loss.
...
This overlaps communication with computation: while GPU $i$ computes attention using $K_j$/$V_j$, it simultaneously receives $K_{j+1}$/$V_{j+1}$ from the ring. If the compute time for one tile exceeds the communication time for transferring one tile over NVLink, the communication is fully hidden. On an H100 with `{python} h100_hbm_bw_str` TB/s HBM bandwidth and `{python} nvlink_h100` GB/s NVLink bandwidth, this overlap is achievable for typical tile sizes.
...
Hybrid parallelism is the only viable path for training frontier models (100B+ parameters), but it introduces a three-dimensional configuration space whose complexity compounds at each level.

Hybrid parallelism offers three principal advantages. Orthogonal scaling addresses all three bottlenecks simultaneously: tensor parallelism fits the layer in memory, pipeline parallelism fits the depth, and data parallelism scales the throughput. Hierarchical bandwidth matching maps logical communication patterns to physical hardware topology, with high-bandwidth NVLink handling tensor parallelism and medium-bandwidth InfiniBand handling pipeline and data parallelism. By overlapping pipeline stages with data parallel communication, hybrid systems can achieve 50-55% Model FLOPS Utilization (MFU) at scale, compared to less than 40% for pure strategies.

These advantages come with three limitations that scale with system size. Debugging requires tracing errors through a 3D coordinate system (data rank, pipeline stage, tensor slice), and a NaN in one microbatch can propagate backward and crash a different GPU in a different pipeline stage. Straggler amplification compounds this complexity: in a synchronous 3D mesh, a single slow GPU stalls the entire cluster, and the probability of a straggler event approaches 100% as $N \rightarrow 10,000$. The topology itself must be rigid and homogeneous, because mixing accelerator generations (A100s and H100s) or varying network bandwidths destroys efficiency.
