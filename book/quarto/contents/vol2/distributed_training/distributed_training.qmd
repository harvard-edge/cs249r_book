...
These factors interact, so efficiency cannot be inferred from model size or GPU count alone. The worked model below fixes the parameter count, tokens per update, accelerator count, effective bandwidth, MFU, and overlap before calculating one efficiency point. Changing any of those inputs requires recomputing the full step-time model.
...
The convergence behavior differs across these models in ways that directly affect training cost and solution quality. In Bulk Synchronous Parallel (BSP, $\tau_{\text{stale}}=0$), all workers compute on the same parameter version and aggregate before updating. Under the assumptions of the theorem above, the synchronized estimator inherits the $Nb$ minibatch guarantee.

Stale Synchronous Parallel (SSP, $\tau_{\text{stale}}\leq s$) allows workers to proceed up to $s$ clocks ahead of the slowest worker. Ho et al. prove an average-regret rate $\mathcal{O}(\sqrt{(s+1)N/T})$ for their SSP update sequence with $N$ workers and $T$ updates, assuming convex Lipschitz component functions, a bounded domain, and a decreasing step size [@ho2013more]. This result makes the staleness dependence explicit, but it is not interchangeable with the nonconvex stationarity theorem above.

Asynchronous SGD imposes no protocol-wide finite delay bound. Removing the barrier can improve update throughput, but a convergence rate requires additional assumptions about realized delays, the objective, and the step-size schedule. Dutta et al. analyze this as an error-runtime trade-off in which worker speed and gradient staleness jointly determine whether accepting a delayed update helps [@dutta2018slow]. Therefore neither a fixed square-of-average-delay penalty nor the complete loss of $N$-worker variance reduction follows from asynchronous execution alone.
...
|:----------|:------------------|:---------------------|:-------------------|
| **BSP** | $\tau_{\text{stale}}=0$ | Synchronous $Nb$-minibatch theorem above | Waits for the slowest worker |
...
: **Convergence Properties by Synchronization Model**: The synchronization protocol determines the delay model; the convergence rate follows only after the objective, gradient, delay, and step-size assumptions are fixed. {#tbl-convergence-comparison}
