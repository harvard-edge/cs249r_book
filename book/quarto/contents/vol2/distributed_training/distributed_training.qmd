...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85, transform shape]
  ...
  % Scope for YES branch
  \begin{scope}[shift={(-4.5,-2.2)}]
    \node[leaf=GreenLine] (dp) at (0,0) {\textbf{Data Parallelism}\\Scale batch size};
    \node[hw, below=of dp, yshift=-2pt] {InfiniBand sufficient};
  \end{scope}

  \draw[arrow] (d1) -- node[yes, left, pos=0.3] {Yes} (dp);

  % Scope for NO branch -> second decision
  \begin{scope}[shift={(3.5,-2.2)}]
    \node[decision=OrangeLine] (d2) {Model fits in\\single node\\(8 GPUs)?};
  \end{scope}

  \draw[arrow] (d1) -- node[no, right, pos=0.3] {No} (d2);

  % Scope for YES from d2: Tensor Parallel
  \begin{scope}[shift={(1,-2.6)}]
    \node[leaf=BlueLine, below=of d2] (tp) {\textbf{Tensor Parallelism}\\Split layers across GPUs};
    \node[hw, below=of tp, yshift=-2pt] {Requires NVLink};
  \end{scope}

  \draw[arrow] (d2) -- node[yes, left, pos=0.3] {Yes} (tp);

  % Scope for NO from d2 -> third decision
  \begin{scope}[shift={(3.5,-2.6)}]
    \node[decision=RedLine, below=of d2] (d3) {Sequential\\dependencies\\dominate?};
  \end{scope}

  \draw[arrow] (d2) -- node[no, right, pos=0.3] {No} (d3);

  % Scope for YES from d3: Pipeline Parallel
  \begin{scope}[shift={(0.5,-2.4)}]
    \node[leaf=OrangeLine, below=of d3] (pp) {\textbf{Pipeline Parallelism}\\Stage layers across nodes};
    \node[hw, below=of pp, yshift=-2pt] {Low BW (Ethernet OK)};
  \end{scope}

  \draw[arrow] (d3) -- node[yes, left, pos=0.3] {Yes} (pp);

  % Scope for NO from d3: 3D Hybrid
  \begin{scope}[shift={(3,-2.4)}]
    \node[leaf=VioletLine, below=of d3] (hybrid) {\textbf{3D Hybrid}\\TP + PP + DP};
...
% Batch Nodes
\node[Box2, above=1.06cm of B1] (GB1) {Batch 1};
\node[Box2, above=1.06cm of B2] (GB2) {Batch 2};
\node[Box2, above=1.06cm of B3] (GB3) {Batch 3};
\node[Box2, above=1.06cm of B4] (GB4) {Batch 4};

% Input Data Node
\node[Box2, above=1.8cm of $(GB2)!0.5!(GB3)$, fill=RedL, draw=RedLine] (GGB1) {Input Data};

% Gradient Nodes
\node[GradientBox, below=of $(B2)!0.5!(B3)$] (DB1) {Gradients GPU N};
\node[GradientBox, below=1.05cm of DB1] (DB2) {Gradient Aggregation};
\node[GradientBox, below=1.05cm of DB2] (DB3) {Model Update};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, xscale=1.5]
  ...
  % DDP (Replicated)
  \node[label=DDP, below] (ddp) at (0,2.7) {};
...
\begin{tikzpicture}[
  ...
  % Annotations
  \node[Annotation, anchor=south] at (axis cs:1000, 1.1) {Linear Scaling};
  \node[Annotation, anchor=south] at (axis cs:100000, 1.8) {Diminishing Returns};
  \node[Annotation, anchor=west, RedLine] at (axis cs:14000, 0.15) {$B*$};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \begin{scope}[every node/.style={label}]
    \addplot[point, BlueL] coordinates {(20, 98)};
    \node[anchor=west, BlueL] at (axis cs:22, 98) {BSP};

    \addplot[point, OrangeL] coordinates {(50, 85)};
    \node[anchor=west, OrangeL] at (axis cs:52, 85) {SSP ($s$=4)};

    \addplot[point, RedL] coordinates {(90, 65)};
    \node[anchor=west, RedL] at (axis cs:82, 60) {ASP};

    \addplot[point, GreenL] coordinates {(60, 92)};
    \node[anchor=south, GreenL] at (axis cs:60, 94) {Gradient Compression};

    \addplot[point, PurpleL] coordinates {(75, 78)};
    \node[anchor=north, PurpleL] at (axis cs:75, 76) {Local SGD};
  \end{scope}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \begin{scope}[every node/.style={Text}]
    \draw[Connector] (B1) -- node[pos=0.25] {Forward Pass} ++(0,12mm) -| (B2.120);
    \draw[ReverseConnector] (B1) -- node[pos=0.25] {Gradient Updates} ++(0,-12mm) -| (B2.240);

    \draw[Connector] (B2) -- node[pos=0.25] {Intermediate Data} ++(0,12mm) -| (B3.120);
    \draw[ReverseConnector] (B2) -- node[pos=0.25] {Gradient Updates} ++(0,-12mm) -| (B3.240);

    \draw[Connector] (B3) -- node[pos=0.25] {Intermediate Data} ++(0,12mm) -| (B4.120);
    \draw[ReverseConnector] (B3) -- node[pos=0.25] {Gradient Updates} ++(0,-12mm) -| (B4.240);

    \draw[Connector] (B4) -- node[pos=0.25] {Output} ++(0,12mm) -| (B5.120);
    \draw[ReverseConnector] (B4) -- node[pos=0.25] {Backward Pass} ++(0,-12mm) -| (B5.240);
  \end{scope}
\end{tikzpicture}
```
:::
...
% Forward Pass Layers
\node[fill0, anchor=south west] (F0_0) {$F_{0,0}$};
\foreach \x in {1, 2, 3} {
    \node[fill0, right=of F0_\the\numexpr\x-1\relax] (F0_\x) {$F_{0,\x}$};
}

\node[fill1, above=of F0_0] (F1_0) {$F_{1,0}$};
\foreach \x in {1, 2, 3} {
    \node[fill1, right=of F1_\the\numexpr\x-1\relax] (F1_\x) {$F_{1,\x}$};
}

\node[fill2, above=of F1_0] (F2_0) {$F_{2,0}$};
\foreach \x in {1, 2, 3} {
    \node[fill2, right=of F2_\the\numexpr\x-1\relax] (F2_\x) {$F_{2,\x}$};
}

\node[fill3, above=of F2_0] (F3_0) {$F_{3,0}$};
\foreach \x in {1, 2, 3} {
    \node[fill3, right=of F3_\the\numexpr\x-1\relax] (F3_\x) {$F_{3,\x}$};
}

% Backward Pass Layers
\node[fill3, right=of F3_3] (B3_3) {$B_{3,3}$};
\foreach \x in {2, 1, 0} {
    \node[fill3, right=of B3_\the\numexpr\x+1\relax] (B3_\x) {$B_{3,\x}$};
}

\node[fill2, below=of B3_2] (B2_3) {$B_{2,3}$};
\foreach \x in {2, 1, 0} {
    \node[fill2, right=of B2_\the\numexpr\x+1\relax] (B2_\x) {$B_{2,\x}$};
}

\node[fill1, below=of B2_2] (B1_3) {$B_{1,3}$};
\foreach \x in {2, 1, 0} {
    \node[fill1, right=of B1_\the\numexpr\x+1\relax] (B1_\x) {$B_{1,\x}$};
}

\node[fill0, below=of B1_2] (B0_3) {$B_{0,3}$};
\foreach \x in {2, 1, 0} {
    \node[fill0, right=of B0_\the\numexpr\x+1\relax] (B0_\x) {$B_{0,\x}$};
}

% Update Nodes
\node[update, right=of B0_0] (U0_0) {Update};
\node[update, above=of U0_0] (U0_1) {Update};
\node[update, above=of U0_1] (U0_2) {Update};
\node[update, above=of U0_2] (U0_3) {Update};
...
\node[decision,above=of B4](G1B4){Is\\ the dataset\\ very large?};
\node[Box1,above=of $(B2.north)!0.5!(B3.north)$](G1B3){Is scaling the model\\ or data more critical?};
...
\node[startbox,above=of G4B3](G5B3){Start};
...
\begin{tikzpicture}[scale=1.5, font=\small\usefont{T1}{phv}{m}{n}, line join=round]
  % Define colors
  \definecolor{DataColor}{RGB}{0,99,149}
  \definecolor{TensorColor}{RGB}{204,85,0}
  \definecolor{PipeColor}{RGB}{0,143,69}
  ...
  \begin{scope}[start chain=going right, node distance=2.5cm]
      ...
      \begin{scope}[start chain=going below, node distance=1.2cm, on chain]
        ...
        \foreach \p in {0, 1} {
          \begin{scope}[start chain=going right, node distance=1.2cm, on chain]
            ...
            \foreach \t in {0, 1} {
              \node[gpu=DataColor] (data\d\p\t) [on chain] {};
              \node[gpu=TensorColor] (tensor\d\p\t) [right=0 of data\d\p\t, on chain] {};
              \node[gpu=PipeColor] (pipe\d\p\t) [above=0 of data\d\p\t, on chain] {};

              \node at (data\d\p\t.center) {\tiny GPU};
            }
          \end{scope}
        }

        % Bracket for Pipeline
        \draw[decorate, decoration={brace, amplitude=5pt}, thick, PipeColor]
          ([yshift=-0.1cm]data\d00.south west) --
          ([yshift=0.1cm]data\d10.north west)
          node[midway, left=8pt, label] {$p$ stages};

        % Bracket for Tensor
        \draw[decorate, decoration={brace, amplitude=5pt}, thick, TensorColor]
    ...
    % Bracket for Data
    \draw[decorate, decoration={brace, mirror, amplitude=10pt}, thick, DataColor]
