...
In the **Systems Sandwich** framework (@sec-vol2-introduction), Distributed Training represents the **Operational Layer**—the logic that partitions the mathematical workload across the physical fleet. While Volume 1 optimized the execution on a single device, Volume 2 solves the constraint satisfaction problem of mapping a model larger than any single memory bank onto a cluster with finite bandwidth. This *connection to the Systems Sandwich* clarifies how the chapter's algorithms relate to the physical and service layers.

Training frontier models requires computational resources that exceed any single machine, placing distributed training squarely within the *Systems Sandwich* framework.
...
[^nvlink]: NVLink provides `{python} nvlink_a100` GB/s bidirectional bandwidth, enabling efficient gradient synchronization within nodes. @sec-compute provides detailed coverage of GPU interconnect architectures, including NVLink topology design and bandwidth optimization strategies.
...
The following plot, the **Scaling Tax** (@fig-scaling-tax), visualizes this divergence from ideal linear scaling. It shows how communication overhead ($r$) acts as a drag on performance, creating a "Communication Wall" where adding more GPUs yields diminishing returns.

```{python}
#| label: fig-scaling-tax
#| echo: false
#| fig-cap: "**The Scaling Tax.** Effective speedup vs. number of GPUs. While ideal scaling (dashed black) is linear, real-world systems pay a 'tax' for communication. Compute-bound models like ResNet (green) scale well because they have high arithmetic intensity. Bandwidth-bound models like GPT-3 (red) hit a 'Communication Wall' where adding more GPUs yields diminishing returns (efficiency < 50%)."
#| fig-alt: "Line chart of Speedup vs GPU Count (log-log). Green line (Compute Bound) stays close to the ideal linear diagonal. Red line (Bandwidth Bound) flattens out, showing diminishing returns as GPUs are added."
...
The following table summarizes the key trade-offs across synchronization models.
...
- Speedup: `{python} f"{single_gpu_step_s}"`s ÷ `{python} f"{total_8gpu_ms/1000}"`s = `{python} speedup_8gpu_str`× (not quite 8×)
...
- Speedup: `{python} f"{single_gpu_step_s}"`s ÷ `{python} f"{total_32gpu_ms/1000}"`s = `{python} speedup_32gpu_str`× faster → `{python} training_32gpu_str` hours
...
- Configuration: `{python} n_gpus_ga` GPUs × batch `{python} batch_per_gpu` × `{python} accum_steps` accumulation steps = `{python} effective_batch_str` effective batch
- Communication overhead: `{python} comm_8gpu_ms`ms ÷ (`{python} accum_steps` × `{python} compute_8gpu_ms`ms) = `{python} comm_overhead_pct_str`%
- Training time: `{python} ga_training_hours` hours
- Cost: \$128/hour × `{python} ga_training_hours` hours = \$`{python} ga_cost_str` vs. \$3,021 for 32 GPUs
- Savings: \$`{python} ga_savings_str` (`{python} ga_savings_pct_str`% reduction) with only 1 hour longer training
...
**The Systems Conclusion**: $38 \text{ GB} (Static) + 50 \text{ GB} (Dynamic) = \mathbf{88 \text{ GB}}$. This exceeds the `{python} a100_mem` GB capacity of the A100. Even with full ZeRO-3 sharding, **pure Data Parallelism fails**. You *must* use **Tensor Parallelism** to split the layers themselves.
:::
...
This distribution method enables training of large-scale models. GPT-3, with `{python} gpt3_params_b` billion parameters, relies on model parallelism for training. Vision transformers processing high-resolution 16k × 16k pixel images use model parallelism to manage memory constraints. Mixture-of-Experts architectures use this approach to distribute their conditional computation paths across hardware.
...
The preceding sections revealed a fundamental tension: data parallelism scales throughput but requires each GPU to hold the complete model, while model parallelism enables larger models but reduces throughput through sequential dependencies. For GPT-3 scale training, with `{python} gpt3_params_b`B parameters across 300B tokens, neither approach alone suffices [@narayanan_pipeline_parallelism_2021]. Data parallelism across 1024 GPUs would require each GPU to hold 700 GB of parameters. Pure model parallelism across 64 stages would create $(64-1)/32 \approx$ 98% pipeline bubble overhead with reasonable microbatch counts.
...
**Intra-Node Bandwidth**: Modern GPU nodes provide extremely high bandwidth (`{python} nvlink_a100`-`{python} nvlink_h100` GB/s) between devices via NVLink. This supports **Tensor Parallelism**, which requires frequent, high-volume communication (2 AllReduce operations per layer).
