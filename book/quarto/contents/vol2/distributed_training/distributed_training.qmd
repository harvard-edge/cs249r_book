...
2. **Communication Overhead**: With 4-step accumulation, we AllReduce once every 4 steps.
   - Overhead = $5 \text{ ms} / (4 \times 1{,}800 \text{ ms}) \approx$ **`{python} GradAccumScenario.comm_overhead_pct_str` percent**.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import A100_MEM_CAPACITY, GPT3_PARAMS, param
...
- Scaling degrades because per-GPU compute falls as tensor parallel degree grows, while collective latency and aggregate traffic rise; per-GPU ring AllReduce bytes approach 2$\times$ the activation tensor size rather than growing linearly without bound.
