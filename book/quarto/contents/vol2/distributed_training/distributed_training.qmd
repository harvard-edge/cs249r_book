...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Chapter-wide setup for Distributed Training.
# │
# │ Goal: Provide foundational specs for multi-node scaling analysis.
# │ Show: GPU memory limits and interconnect bandwidth for distributed clusters.
# │ How: Centralize A100/H100 specs and GPT-3 parameter counts.
# │
# │ Imports: mlsys.constants, mlsys.formatting
# │ Exports: a100_mem, h100_mem, nvlink_*, gpt3_params_b
# └─────────────────────────────────────────────────────────────────────────────
...
class DistTrainSetup:
    ...
    gpt3_params_b_val = gpt3_params_raw.to(Mparam).magnitude / THOUSAND

    # ┌── 3. INVARIANTS (Guardrails) ───────────────────────────────────────────
    check(gpt3_params_b_val == 175, f"GPT-3 should be 175B params, got {gpt3_params_b_val}")

    # ┌── 4. OUTPUTS (Formatting) ──────────────────────────────────────────────
    a100_mem = f"{a100_cap.to(GiB).magnitude:.0f}"
    h100_mem = f"{h100_cap.to(GiB).magnitude:.0f}"
    nvlink_a100 = f"{nvlink_a100_raw.to(GB/second).magnitude:.0f}"
    nvlink_h100 = f"{nvlink_h100_raw.to(GB/second).magnitude:.0f}"
