...
<!-- TODO(numerical-audit): The debugging walkthrough below states an
     observed 100 ms vs a 50 ms prediction, then computes a 120 ms naive
     transfer and 240 ms ring AllReduce, and concludes the observed 100 ms
     is "better than naive" with a 40 ms remaining gap. The arithmetic
     does not reconcile across the four numbers (50, 100, 120, 240).
     Pick one consistent baseline (either the 50 ms bandwidth prediction
     OR the 240 ms ring-AllReduce model) and recompute the remaining-gap
     analysis from there. -->
