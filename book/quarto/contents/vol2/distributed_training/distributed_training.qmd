...
class DistTrainNvlinkIntro:
    ...
    nvlink_hdr_ratio_high = (nvlink_h100_bw / ib_hdr_bw).to_base_units().magnitude
    check(20 < nvlink_hdr_ratio_low < 30, "A100 NVLink/HDR ratio should be about 24x")
    nvlink_a100_gb_s_str = fmt_bandwidth(nvlink_a100_bw, unit=GB / second, precision=0, commas=False)
    nvlink_h100_gb_s_str = fmt_bandwidth(nvlink_h100_bw, unit=GB / second, precision=0, commas=False)
    nvlink_hdr_ratio_low_str = fmt_multiple(nvlink_hdr_ratio_low, precision=0, commas=False)
    nvlink_hdr_ratio_high_str = fmt_multiple(nvlink_hdr_ratio_high, precision=0, commas=False)
```

Tensor parallelism across server racks connected by standard Ethernet will stall. The communication volume scales with batch size $B$ and layer count $N_L$. It requires the `{python} DistTrainNvlinkIntro.nvlink_a100_gb_s_str`--`{python} DistTrainNvlinkIntro.nvlink_h100_gb_s_str` throughput of NVLink. For cross-rack scaling, the design must switch to pipeline or data parallelism to respect the physics of the network.
...
[^fn-nvlink-intranode]: **NVLink**: NVIDIA's point-to-point GPU interconnect delivers `{python} DistTrainNvlinkIntro.nvlink_a100_gb_s_str`--`{python} DistTrainNvlinkIntro.nvlink_h100_gb_s_str` bidirectional bandwidth, roughly `{python} DistTrainNvlinkIntro.nvlink_hdr_ratio_low_str`$\times$--`{python} DistTrainNvlinkIntro.nvlink_hdr_ratio_high_str`$\times$ InfiniBand HDR per port. This bandwidth gap is why tensor parallelism, which requires AllReduce on every layer, is confined to intra-node communication, while pipeline and data parallelism tolerate the slower inter-node fabric. \index{NVLink!distributed training}
...
class HierarchicalAllReduceDebug:
    ...
    nvlink_a100_gb_s_str = fmt_bandwidth(nvlink_bw, unit=GB/second, precision=0, commas=False)
    ...
    ib_hdr_bw_gb_s_str = fmt_bandwidth(ib_hdr_bw, unit=GB/second, precision=0, commas=False)
...
class Scaling8GPU:
    ...
    speedup_8gpu_str = fmt_multiple(round(speedup_8gpu_val), commas=False, precision=0)
    ...
    nvlink_h100_gb_s_str = fmt_bandwidth(nvlink_bw, unit=GB/second, precision=0, commas=False)
...
- Gradient synchronization: `{python} Scaling8GPU.gpt2_sync_size_gb_str` @ `{python} Scaling8GPU.nvlink_h100_gb_s_str` (NVLink) $\approx$ `{python} Scaling8GPU.comm_8gpu_ms_str`
...
class Scaling32GPU:
    ...
    speedup_32gpu_str = fmt_multiple(speedup_32gpu_val, precision=1, commas=False)
    ...
    ib_hdr_gb_s_str = fmt_bandwidth(net_bw, unit=GB / second, precision=2, commas=False)
...
- Inter-node communication: `{python} Scaling32GPU.gpt2_sync_size_gb_str` @ `{python} Scaling32GPU.ib_hdr_gb_s_str` (10GbE) $\approx$ `{python} Scaling32GPU.inter_node_ms_str`
...
- Speedup (throughput): `{python} Scaling32GPU.speedup_32gpu_str`$\times$ faster → `{python} Scaling32GPU.training_32gpu_hours_str`
...
from mlsysim.fmt import fmt, check, fmt_int, fmt_memory_capacity, fmt_qty_int
...
class DistTrainA100Memory:
    ...
    bytes_per_param_str = fmt(bytes_per_param_val, precision=0, commas=False)
...
**Problem**: Training a `{python} DistTrainA100Memory.params_b_str` parameter Llama-2 model in mixed precision requires `{python} DistTrainA100Memory.bytes_per_param_str` bytes per parameter for weights, gradients, and optimizer state. Does the full training state fit on a single A100-`{python} DistTrainA100Memory.a100_mem_gb_str`, and how does ZeRO Stage 3 with `{python} DistTrainA100Memory.n_gpus_zero_str` change the per-device memory requirement?
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_count, fmt_params, fmt_percent, fmt_qty, fmt_math, fmt_time
...
class CommScalingMath:
    ...
    _ib_gbs_text = fmt(_ib_bw.to(GB / second).magnitude, precision=0, commas=False)
    _gradient_size_gb_text = fmt(_gradient_size.to(GB).magnitude, precision=0, commas=False)
    comm_time_formula_math = fmt_math(f"2 \\times (N-1)/N \\times {_gradient_size_gb_text} / {_ib_gbs_text}")
    comm_approx_math = fmt_math(f"2 \\times {_gradient_size_gb_text} / {_ib_gbs_text}")
    gpt3_params_b_str = fmt_params(Models.Language.GPT3.parameters)
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: CommScalingIbRecap.ib_bw_gb_s_str
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: InfraIbBwPodRecap.ib_bw_gbps_str
...
**Setup**\index{Setup}: Training a `{python} InfraFrontierPodSetupRecap.frontier_params_b_str` model on a DGX H100 cluster with `{python} InfraIbBwPodRecap.ib_bw_gbps_str` InfiniBand per GPU.
...
class DistTrainNvlinkScaling:
    ...
    nvlink_a100_gb_s_str = fmt_qty(Hardware.Cloud.A100.nvlink.bandwidth, GB/second, precision=0, commas=False)
```

Hardware selection critically impacts these scaling characteristics. NVIDIA DGX A100 systems provide `{python} DistTrainNvlinkScaling.nvlink_a100_gb_s_str` of bidirectional NVLink bandwidth per GPU, with aggregate NVSwitch bandwidth at the system level, enabling high parallel efficiency up to 8 GPUs per node. Multi-node scaling requires InfiniBand networks, where EDR at 100 Gbps supports efficient training up to 64 nodes, while HDR at 200 Gbps enables scaling to 256+ nodes with greater than 70 percent efficiency.
...
class ScalingWorkers:
    ...
    speedup_8_str = fmt_multiple(speedup_8, precision=1, commas=False)
    speedup_64_str = fmt_multiple(speedup_64, precision=1, commas=False)
    speedup_ssp_str = fmt_multiple(speedup_ssp, precision=1, commas=False)
    ...
    speedup_64_round_str = fmt_multiple(round(speedup_64), commas=False, precision=0)
...
class TensorParallelBandwidth:
    from mlsysim.core.constants import GB, second
    from mlsysim.fmt import fmt_bandwidth
    nvlink_h100_gb_s_str = fmt_bandwidth(Hardware.Cloud.H100.nvlink.bandwidth, unit=GB / second, precision=0, commas=False)
    ib_ndr_gb_s_str = fmt_bandwidth(Systems.Fabrics.InfiniBand_NDR.bandwidth, unit=GB / second, precision=0, commas=False)
    pipeline_ib_lo_bw = Systems.Fabrics.InfiniBand_HDR.bandwidth
    pipeline_ib_hi_bw = Systems.Fabrics.InfiniBand_NDR.bandwidth
    pipeline_ib_lo_gb_s_str = fmt_bandwidth(pipeline_ib_lo_bw, unit=GB / second, precision=0, commas=False)
    pipeline_ib_hi_gb_s_str = fmt_bandwidth(pipeline_ib_hi_bw, unit=GB / second, precision=0, commas=False)
```
...
3.  **Common pitfall**: A frequent misconception is that tensor parallelism can simply be scaled across nodes over InfiniBand. NVLink delivers approximately `{python} TensorParallelBandwidth.nvlink_h100_gb_s_str` bidirectional bandwidth; InfiniBand NDR delivers `{python} TensorParallelBandwidth.ib_ndr_gb_s_str`—an 18$\times$ gap. Running tensor parallelism across InfiniBand typically collapses MFU to single-digit percentages, making the communication overhead larger than the compute benefit of distributing the matrix multiplication.
...
# │ Imports: mlsysim.book (fmt)
# │ Exports: FlashAttentionOverlapScenario.h100_hbm_bw_tb_s_str,
# │          FlashAttentionOverlapScenario.nvlink_h100_gb_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
class FlashAttentionOverlapScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_hbm_bw_tb_s_str = fmt_bandwidth(hbm_bw, unit=TB/second, precision=2, commas=False)
    nvlink_h100_gb_s_str = fmt_bandwidth(nvlink_bw, unit=GB/second, precision=0, commas=False)
```

If the compute time for one tile exceeds the communication time for transferring one tile over NVLink, the communication is fully hidden. On an H100 with `{python} FlashAttentionOverlapScenario.h100_hbm_bw_tb_s_str` HBM bandwidth and `{python} FlashAttentionOverlapScenario.nvlink_h100_gb_s_str` NVLink bandwidth, this overlap is achievable for typical tile sizes.
...
class FlashAttentionHybridRecap:
    from mlsysim.core.constants import GB, second
    from mlsysim.fmt import fmt, fmt_qty
    nvlink_h100_gb_s_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth, GB/second, precision=0, commas=False)
```
...
from mlsysim.fmt import check, fmt, MarkdownStr, fmt_bandwidth
...
class ThreeDParallelism:
    ...
    nvlink5_bw_tb_s_str = fmt_bandwidth(nvlink5_bw, unit=TB / second, precision=1, commas=False)
    params_trillion_b_str = fmt(1, precision=0, commas=False)
...
# │          RLHFBudget.remaining_kv_gb_str, RLHFBudget.dpo_activation_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt_percent, fmt, fmt_memory
from mlsysim.physics import memory_from_params
...
class RLHFBudget:
    ...
    dpo_activation_ratio_str = fmt_multiple(dpo_activation_ratio, precision=2, commas=False)
    tp_str = fmt_count(tp, commas=False, label="GPU")
    pp_str = fmt(pp, precision=0, commas=False)
...
The policy model under 3D parallelism with TP=`{python} RLHFBudget.tp_str` and PP=`{python} RLHFBudget.pp_str` distributes its `{python} RLHFBudget.policy_mem_gb_str` training state across `{python} RLHFBudget.n_gpus_str`, yielding `{python} RLHFBudget.policy_per_gpu_gb_str` per GPU. The reference model, requiring only `{python} RLHFBudget.ref_mem_gb_str` for inference, can be sharded across a separate pool of `{python} RLHFBudget.ref_pool_gpus_str` at `{python} RLHFBudget.ref_per_gpu_sep_gb_str` each, or co-located with the policy GPUs at an additional `{python} RLHFBudget.ref_per_gpu_gb_str` per GPU (`{python} RLHFBudget.ref_mem_gb_str` / `{python} RLHFBudget.n_gpus_str`). The `{python} RLHFBudget.reward_b_str`B reward model adds `{python} RLHFBudget.reward_mem_gb_str` shared across its pool. The `{python} RLHFBudget.value_b_str`B value model in training mode adds approximately `{python} RLHFBudget.value_mem_gb_str` (`{python} RLHFBudget.value_b_str`B $\times$ `{python} RLHFBudget.bytes_train_str` bytes/param) across its pool.
...
from mlsysim.fmt import fmt, check, fmt_memory_capacity
...
* **Tensor parallelism is node-local**: Because it partitions matrix operations, it requires the `{python} DistTrainSummaryRecap.nvlink_a100_gb_s_str`-`{python} DistTrainSummaryRecap.nvlink_h100_gb_s_str` bandwidth of NVLink. Scaling it across racks on standard Ethernet will stall the fleet.
