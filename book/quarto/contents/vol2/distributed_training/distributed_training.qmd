...
* **Tensor parallelism belongs near NVLink**: Tensor parallelism splits matrix operations inside layers, so it needs the high-bandwidth intra-node fabric that A100 and H100 NVLink provide (principle \ref{pri-bandwidth-latency-tradeoff}). Stretching that traffic across racks turns a memory-capacity solution into a communication bottleneck.
