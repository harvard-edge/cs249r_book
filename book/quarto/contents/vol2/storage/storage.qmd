...
The storage architectures developed in this chapter complete our construction of the data substrate for the Machine Learning Fleet. We have now defined the compute nodes (@sec-infrastructure) and the data systems (@sec-storage) required to support distributed training.

However, isolated nodes are of limited use; they must communicate to synchronize learning. The next chapter (@sec-communication) examines the high-speed communication patterns and collective operations that bind these individual nodes into a single coherent training engine.

With the physical machine built and the data pipelines flowing, we turn our attention to the service layer. **Part III: Deployment at Scale** begins with @sec-inference-at-scale, examining how we transform trained models into low-latency global services.
