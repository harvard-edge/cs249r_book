...
The storage architectures examined here ensure that data is available when needed—whether for training throughput or serving latency. However, availability is not enough. In a fleet of thousands of nodes, failures are inevitable. A robust system must survive the loss of disks, servers, and even entire racks without losing data or halting progress.

The next chapter, @sec-fault-tolerance, examines the mechanisms that transform unreliable components into reliable systems, completing the infrastructure foundation of the Machine Learning Fleet.
