...
```{python}
#| label: checkpoint-interval-derived
#| echo: false
...
# Inputs
t_save = 10          # seconds
gpu_mtbf = 30_000    # hours per GPU
n_gpus = 1024
cluster_mtbf_hr = gpu_mtbf / n_gpus  # ~29.3 hours

# Young-Daly optimal interval
t_opt_s = math.sqrt(2 * t_save * cluster_mtbf_hr * 3600)
t_opt_min = t_opt_s / 60

# Derived efficiency values
work_lost_min = t_opt_min / 2
ckpt_overhead_pct = (t_save / t_opt_s) * 100
failure_loss_pct = (work_lost_min / (cluster_mtbf_hr * 60)) * 100
total_overhead_pct = ckpt_overhead_pct + failure_loss_pct

# Pre-format for inline use
t_opt_s_str = f"{t_opt_s:,.0f}"
t_opt_min_str = f"{t_opt_min:.0f}"
work_lost_min_str = f"{work_lost_min:.0f}"
ckpt_overhead_pct_str = f"{ckpt_overhead_pct:.1f}"
failure_loss_pct_str = f"{failure_loss_pct:.1f}"
total_overhead_pct_str = f"{total_overhead_pct:.1f}"
```
...
**Interpretation:** Checkpoint every `{python} t_opt_min_str` minutes. This balances the 10-second checkpoint overhead against the 29-hour MTBF.
...
With one failure per 29 hours and `{python} work_lost_min_str` minutes lost per failure:

$$\text{Loss rate} = \frac{12 \text{ min}}{29 \times 60 \text{ min}} = 0.7\%$$

Total overhead: `{python} ckpt_overhead_pct_str`% (checkpointing) + `{python} failure_loss_pct_str`% (failures) = `{python} total_overhead_pct_str`%
