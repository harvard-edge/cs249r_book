...
Understanding access patterns (@tbl-access-patterns) quantitatively enables storage system selection and capacity planning. ML workloads exhibit distinct patterns across different phases and model types (see @fig-access-patterns).

::: {#fig-access-patterns fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \begin{axis}[
...
::: {#fig-data-pipeline-vol2 fig-env="figure" fig-pos="htb"}
```{.tikz}
...
For most training runs, asynchronous checkpointing reduces overhead to near zero (as shown in @fig-async-checkpoint). The memory overhead (one additional copy of model state) is typically acceptable on systems with sufficient host memory.

::: {#fig-async-checkpoint fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9]
...
This formula minimizes expected wasted work, accounting for both checkpoint overhead and work lost to failures (as visualized in @fig-young-daly-storage). The MTBF[^fn-mtbf] for GPU clusters decreases inversely with cluster size, making optimal checkpoint intervals surprisingly short for large-scale training.

::: {#fig-young-daly-storage fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \begin{axis}[
    width=10cm, height=6cm,
...
A feature store has two primary components: the offline store for training and the online store for serving (see @fig-feature-store-architecture).

::: {#fig-feature-store-architecture fig-env="figure" fig-pos="htb"}
```{.tikz}
...
::: {#fig-pit-correctness fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  % Timeline
...
Point-in-time correctness requires that features used during training reflect only information available before the prediction event. This constraint (@fig-pit-correctness) is easy to state and deceptively difficult to enforce. Feature pipelines that aggregate over time windows, join multiple data sources, or depend on asynchronous updates can all violate point-in-time correctness in subtle ways.
...
* Storage bandwidth, not capacity, typically limits ML training throughput: systems must be designed to saturate accelerator memory bandwidth with training data (as shown in @fig-data-pipeline-vol2), requiring careful attention to prefetching, data format optimization, and parallel I/O
