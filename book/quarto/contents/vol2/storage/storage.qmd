...
In the **Systems Sandwich** (@sec-vol2-introduction), Storage is the **Fuel Line** of the Machine Learning Fleet. While compute nodes (@sec-infrastructure) provide the engine and networks (@sec-communication) provide the transmission, storage provides the fuel (data). If the **Iron Law** states that $L = (\text{Data} + \text{Compute}) / \text{Bandwidth}$, this chapter is about ensuring that the storage bandwidth term ($\text{Bandwidth}_{disk}$) never becomes the bottleneck.

The infrastructure foundations established in @sec-infrastructure provide the compute fabric...
...
: **Extended Memory Hierarchy for ML Systems**: GPU HBM delivers 3.35 TB/s at ~10 ns latency but costs $15/GB, while object storage offers unlimited capacity at $0.02/GB with 50 ms latency. This 300,000x bandwidth gap between HBM and storage drives the need for sophisticated prefetching and caching strategies. {#tbl-storage-hierarchy}

Looking at the bandwidth column, GPU HBM delivers 3.35 TB/s, roughly 17x faster than host DRAM and 130x faster than the fastest local NVMe drives. This disparity creates the central challenge of ML storage systems: keeping accelerators fed with data at rates that prevent them from idling.

### High Bandwidth Flash (HBF): Blurring Storage and Memory
...
::: {.callout-example title="LLM Training Bandwidth Requirements (GPT-3 Lighthouse)"}
...
::: {.callout-example title="Recommendation System Training Bandwidth (DLRM Lighthouse)"}
...
@tbl-consistency-requirements maps each storage tier to its appropriate consistency model: training data tolerates eventual consistency because samples are immutable and shuffled anyway, while checkpoints demand linearizability because partial writes cause catastrophic recovery failures.
...
### I/O Optimization for ML Data Pipelines {#sec-io-optimization}

The bandwidth calculations in @sec-storage-throughput-requirements establish what storage systems must deliver. This section examines how to achieve those targets through systematic I/O optimization, addressing the gap between raw storage capabilities and effective training throughput.

#### The I/O Bandwidth Requirement Equation {#sec-io-bandwidth-equation}
...
#### Sequential vs Random Access: The Performance Gap {#sec-sequential-vs-random}
...
#### Worked Example: ImageNet Data Loading Bottleneck Analysis {#sec-imagenet-io-analysis}

::: {.callout-example title="ImageNet I/O Bottleneck Analysis for ResNet-50"}
...
**Step 1: Calculate raw I/O requirement**
...
**Step 2: Account for access pattern**
...
**Step 3: Account for preprocessing overhead**
...
**Step 4: Calculate effective I/O requirement**
...
**Step 5: Format optimization impact**
...
#### Prefetching Benefit Quantification {#sec-prefetch-quantification}

Prefetching overlaps I/O latency with GPU computation, but its benefit depends on the relationship between I/O latency, preprocessing time, and batch time.
...
#### Data Format Performance Comparison {#sec-format-performance}
...
#### I/O Profiling and Diagnosis {#sec-io-profiling}
...
**Data loader wait time** measures how long the training loop blocks waiting for the next batch. PyTorch's DataLoader exposes this through profiling:
...
**Storage bandwidth utilization** indicates whether the storage system is saturated. On Linux, `iostat -x 1` shows per-device utilization:
...
**I/O wait percentage** in CPU statistics (`top`, `htop`) reveals when processes block on I/O:
...
::: {.callout-perspective title="Back-of-the-Napkin: The Checkpoint Tax"}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm]
  ...
  \draw[arrow] (OfflineStore) -- node[above, font=\scriptsize] {Historical\nFeatures} (Training);

  \draw[arrow] (StreamSource) -- (StreamProc);
  \draw[arrow] (StreamProc) -- (OnlineStore);
  \draw[arrow] (OnlineStore) -- node[above, font=\scriptsize] {Low Latency\nFeatures} (Serving);
...
**Erasure coding.** Colossus uses Reed-Solomon erasure coding [@reed1960polynomial] rather than simple replication. A typical configuration stores 9 data fragments plus 3 parity fragments, achieving durability equivalent to 3x replication while using only 1.33x storage. For petabyte-scale training data, this dramatically reduces storage costs.
...
The next chapter, @sec-orchestration, examines the software "brain" of the fleet: the orchestration and resource management systems (Slurm, Kubernetes) that coordinate thousands of jobs across these resources to ensure fairness and maximize utilization.
