...
1.  **Significance (Quantitative):** By minimizing the physical distance data travels, HBM provides the **Memory Bandwidth ($\text{BW}$)** required to feed massive arithmetic units ($R_{\text{peak}}$), achieving 10$\times$ to 100$\times$ higher throughput than conventional DDR memory.
...
1.  **Compute Time**: For 1 token, we compute $2 \times P \approx 1.4 \times 10^{11}$ FLOPs.
    $T_{\text{comp}} = 1.4 \times 10^{11} / 9.89 \times 10^{14} \approx \mathbf{0.14 \text{ ms}}$.
