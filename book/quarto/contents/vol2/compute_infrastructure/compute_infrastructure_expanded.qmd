...
from mlsys.constants import (
    ...
    TFLOPs, TB, GB, GiB, second, watt, Gbps, flop, byte
)
...
class InfraSetup:
    ...
    # Ridge points
    h100_r = (h100_flops / h100_bw).to(flop/byte).magnitude
    a100_r = (A100_FLOPS_FP16_TENSOR / A100_MEM_BW).to(flop/byte).magnitude

    h100_tflops_fp16 = f"{h100_flops.to(TFLOPs/second).magnitude:.0f}"
    h100_bw_tbs = f"{h100_bw.to(TB/second).magnitude:.2f}"
    h100_mem = f"{h100_cap.to(GiB).magnitude:.0f}"
    h100_tdp_str = f"{h100_tdp_raw.to(watt).magnitude:.0f}"
    h100_ridge = f"{h100_r:.0f}"
    
    a100_tflops_fp16 = f"{a100_flops.to(TFLOPs/second).magnitude:.0f}"
    a100_tdp_str = f"{a100_tdp.to(watt).magnitude:.0f}"
    
    b200_tflops_fp16 = f"{b200_flops.to(TFLOPs/second).magnitude:,.0f}"
    b200_tdp_str = f"{b200_tdp.to(watt).magnitude:,.0f}"
    
    v100_tflops_fp16 = f"{v100_flops.to(TFLOPs/second).magnitude:.0f}"
    v100_tdp_str = f"{v100_tdp.to(watt).magnitude:.0f}"
    ...
    # Efficiency
    h100_eff = h100_flops.to(TFLOPs/second).magnitude / h100_tdp_raw.to(watt).magnitude
    a100_eff = a100_flops.to(TFLOPs/second).magnitude / a100_tdp.to(watt).magnitude
    b200_eff = b200_flops.to(TFLOPs/second).magnitude / b200_tdp.to(watt).magnitude
    v100_eff = v100_flops.to(TFLOPs/second).magnitude / v100_tdp.to(watt).magnitude
...
***High Bandwidth Memory (HBM)*** is a high-speed computer RAM interface for 3D-stacked DRAM. It achieves **Bandwidth Density** by stacking memory dies vertically and connecting them directly to the processor via a **Silicon Interposer**. This physical proximity enables thousands of wide data paths, allowing the system to bridge the **Memory Wall** at the cost of higher manufacturing complexity and thermal concentration.
...
To determine whether an accelerator is being utilized effectively, we use a diagnostic framework that relates hardware limits to software requirements.

::: {.callout-definition title="Roofline Model"}

The ***Roofline Model*** is a graphical representation of a processor's performance limits, defining the **Ridge Point** where a workload transitions from being **Memory-Bound** to **Compute-Bound**. It quantifies the **Arithmetic Intensity** required to fully saturate the silicon's math units, providing a target for optimization in both hardware design and software implementation.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \draw[->, thick] (0,0) -- (0,5) node[above] {Log Bandwidth (GB/s)};
  
  % HBM
  \fill[BlueLine!20] (0.5,0) rectangle (1.5,4.5) node[midway, rotate=90, font=\bfseries] {HBM (3350)};
  \node[below] at (1,0) {On-Package};

  % NVLink
  \fill[BlueLine!40] (2.5,0) rectangle (3.5,3.5) node[midway, rotate=90, font=\bfseries] {NVLink (900)};
  \node[below] at (3,0) {Intra-Node};

  % IB/PCIe
  \fill[BlueLine!60] (4.5,0) rectangle (5.5,1.5) node[midway, rotate=90, font=\bfseries] {IB/PCIe (50-64)};
  \node[below] at (5,0) {Inter-Node};

  % Arrows showing the drop
  \draw[->, ultra thick, RedLine] (1.5, 4.0) -- (2.5, 3.5) node[midway, above, sloped] {~4$\times$ Drop};
  \draw[->, ultra thick, RedLine] (3.5, 3.0) -- (4.5, 1.5) node[midway, above, sloped] {~15$\times$ Drop};
...
***Power Usage Effectiveness (PUE)*** is the ratio of total energy used by a datacenter facility to the energy delivered to computing equipment. A PUE of 1.0 is the theoretical ideal where every watt powers a processor; higher values represent energy lost to **Cooling Overheads** and electrical distribution. In high-density ML clusters, PUE is optimized by moving from air to liquid cooling.
...
A ***Warehouse-Scale Computer (WSC)*** is a cluster of thousands of nodes designed to function as a single, massive computing entity. In a WSC, the **Network Fabric** replaces the system bus, **Distributed Storage** replaces the local disk, and the **Fleet Orchestrator** replaces the operating system. The design goal is to minimize **Bisection Bandwidth** bottlenecks so that any GPU can communicate with any other GPU as if they were in the same chassis.
