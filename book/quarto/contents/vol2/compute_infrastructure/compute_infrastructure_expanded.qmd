...
For an H100, the Ridge Point is `{python} InfraSetup.h100_ridge` FLOP/byte. If your operation has low intensity—such as the "Decode" phase of an LLM where you load massive weights but only compute for a single token—the system is memory-bound. If your operation has high intensity—such as the "Prefill" phase or training a CNN—the system is compute-bound.
