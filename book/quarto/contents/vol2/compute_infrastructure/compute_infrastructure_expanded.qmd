...
# │ How: Pull from mlsysim.core.constants; ridge via (flops/bw).to(flop/byte).magnitude;
# │       efficiency = flops.to(TFLOPs/s) / tdp.to(watt) for each generation.
# │
# │ Imports: mlsysim.core.constants (A100_FLOPS_FP16_TENSOR, A100_MEM_BW, A100_MEM_CAPACITY,
...
# │ Imports: mlsysim.book (fmt, sci, check)
...
### The workload pattern: GEMM dominance {#sec-compute-infrastructure-workload-pattern-gemm-dominance-3bef}

Modern deep learning is essentially a sequence of general matrix multiply (GEMM) operations. A single Transformer layer with a hidden dimension $d$ involves weight matrices of size $d{\times}d$ and activation tensors of $s{\times}d$, where $s$ is the sequence length. This workload presents a unique physical challenge: it requires high compute density ($O(d^3)$ operations) coupled with massive state ($O(d^2)$ parameters).
...
1.  **Significance (Quantitative):** By minimizing the physical distance data travels, HBM provides the **Memory Bandwidth ($\text{BW}$)** required to feed massive arithmetic units ($R_{\text{peak}}$), achieving 10$\times$--100$\times$ higher throughput than conventional DDR memory.
...
1.  **Significance (Quantitative):** It defines the **Ridge Point** where a system transitions from being **Bandwidth-Bound** ($\text{BW}$) to **Compute-Bound** ($R_{\text{peak}}$), allowing architects to predict attainable performance for any given operation.
...
3.  **Common Pitfall:** A frequent misconception is that the roofline is "static." In reality, it is **Precision-Dependent**: using lower precision (for example, INT8) raises the compute ceiling ($R_{\text{peak}}$) while the memory ceiling ($\text{BW}$) remains constant, shifting the ridge point significantly.
...
For an H100, the Ridge Point is `{python} InfraSetup.h100_ridge` FLOP/byte. If your operation has low intensity—such as the "Decode" phase of a large language model (LLM) where you load massive weights but only compute for a single token—the system is memory-bound. If your operation has high intensity—such as the "Prefill" phase or training a convolutional neural network (CNN)—the system is compute-bound.
...
1.  **Compute Time**: For 1 token, we compute $2 \times P \approx 1.4 \times 10^{11}$ FLOPs.
    $T_{\text{comp}} = \frac{1.4 \times 10^{11}}{9.89 \times 10^{14}} \approx \mathbf{0.14 \text{ ms}}$.
2.  **Memory Time**: Load 140 GB of weights.
    $T_{\text{mem}} = 140 \text{ GB} / 3350 \text{ GB/s} \approx \mathbf{41.8 \text{ ms}}$.

**The Systems Conclusion**:
The processor spends **99.7 percent of its time** waiting for memory. Doubling the TFLOPS would save only 0.07 ms; doubling the HBM bandwidth would save 20 ms. This illustrates why H100's primary advantage over A100 is not just the faster Tensor Cores, but the massive HBM throughput.
...
### The connectivity pattern: Bandwidth asymmetry {#sec-compute-infrastructure-connectivity-pattern-bandwidth-asymmetry-ae9a}

The defining characteristic of a node is its **Bandwidth Hierarchy**. As data moves from the silicon die to the board and finally to the network, it incurs a "Bandwidth Tax." As @fig-bandwidth-hierarchy-expanded illustrates, bandwidth drops by roughly an order of magnitude at each physical boundary.

::: {#fig-bandwidth-hierarchy-expanded fig-env="figure" fig-pos="htb" fig-cap="**The Bandwidth Hierarchy**. In a modern ML node, bandwidth drops by roughly an order of magnitude as data crosses physical boundaries. Tensor Parallelism is confined to the NVLink domain to avoid the 'InfiniBand Cliff'." fig-alt="Diagram of concentric ellipses showing bandwidth tiers. From the center out: HBM (3.35 TB/s), NVLink (900 GB/s), InfiniBand NDR (50 GB/s), and Storage (7 GB/s). Each boundary crossing represents a large drop in bandwidth."}
![](images/svg/bandwidth-hierarchy.svg)
:::

As @fig-bandwidth-hierarchy-expanded illustrates, this tax determines the **Distributed Strategy**. **Tensor Parallelism**, which requires synchronizing partial sums after every matrix multiplication, is physically confined to the high-bandwidth domain (NVLink). If we attempted to run Tensor Parallelism across nodes using standard networking, the GPUs would spend 90 percent of their time waiting for the "InfiniBand Cliff," rendering the extra compute useless.

### Dense node designs: DGX and HGX {#sec-compute-infrastructure-dense-node-designs-dgx-hgx-461f}

To minimize this tax, engineers build "Dense Nodes" like the NVIDIA DGX or HGX. These systems place 8 GPUs on a single board, cross-connected via **NVSwitch** fabric. This enables the 8 GPUs to act as one "Mega-GPU" with a shared memory pool. For a cluster of H100s, this node-level pool provides `{python} InfraSetup.h100_mem` GB of HBM, enough to hold the weights of most medium-scale LLMs entirely in resident memory.
...
### The power ramp: Megawatt transients {#sec-compute-infrastructure-power-ramp-megawatt-transients-7e9e}
...
### The thermodynamic limit: Liquid cooling {#sec-compute-infrastructure-thermodynamic-limit-liquid-cooling-de40}
...
1.  **Significance (Quantitative):** It measures the **Infrastructure Overhead** of the datacenter. A PUE of 1.0 is the theoretical ideal; a PUE of 1.10 means that for every 100 watts of IT load ($P_{\text{IT}}$), an additional 10 watts are required for cooling and power distribution.
...
3.  **Common Pitfall:** A frequent misconception is that WSC design is just "IT management." In reality, it is **Building-Level Computer Architecture**: the physical layout, power delivery, and cooling must be optimized as a single system to prevent the **Bisection Bandwidth ($\text{BW}$)** from becoming the binding constraint.

:::

### Case studies in fleet architecture {#sec-compute-infrastructure-case-studies-fleet-architecture-b80e}
...
*   **Google Tensor Processing Unit (TPU) Pod**: Optimized for **Transformers**. Uses a 3D-Torus interconnect built directly into the silicon, prioritizing high-speed neighbor communication for the predictable dataflows of LLMs.
...
### Economics: Total cost of ownership {#sec-compute-infrastructure-economics-total-cost-ownership-d7af}
...
$$\text{Break-even utilization} = \frac{\frac{C_{\text{capex}}}{Y_{\text{amortization}}} + C_{\text{opex}}}{C_{\text{cloud}} \times H_{\text{annual}}}$$

For a high-performance cluster, if the expected utilization is above 57 percent, buying on-premises infrastructure typically results in a lower TCO. If utilization is lower, the flexibility of the cloud outweighs the capital expense of ownership.
