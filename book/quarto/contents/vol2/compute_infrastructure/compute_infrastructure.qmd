...
Specialized accelerators reduce this tax. Moving along the spectrum from CPU to custom ASIC can devote more die area to arithmetic by removing general-purpose control logic. The progression is quantifiable: a CPU devotes roughly 5--10 percent of its die to arithmetic, a GPU devotes roughly 50--60 percent, a Tensor Processing Unit (TPU) devotes roughly 70--80 percent, and a purpose-built ASIC can devote over 90 percent of its die to the target computation.
...
[^fn-systolic-array-tpu]: **Systolic Array**\index{Systolic Array!etymology}: From Greek *systole* (contraction), the systolic-array model was introduced by H. T. Kung and Charles E. Leiserson's VLSI work and later framed by Kung as a class of architectures where data pulses through a grid of processing elements in a heart-like rhythm [@kung1979systolic; @kung1982]. The metaphor explains the design: each cell performs one MAC and passes the result to its neighbor, eliminating register-file round-trips. Google's TPU v1 deployed a $256{\times}256$ array (65,536 MACs), achieving 92 TOPS within 75 W by hardwiring this dataflow -- the architectural bet that made data center-scale inference economically viable [@jouppi2017].

::: {#exmp-compute-infrastructure-tpu-origin-story .callout-example title="The TPU origin story"}

**Context**: In 2013, Google engineers projected that if users spoke to their Android phones for just three minutes per day using voice search, the company would need to double its data center compute capacity to handle the inference load [@jouppi2017]. The cost was prohibitive.

**Insight**: Google's TPU analysis framed production neural-network inference as a dense-linear-algebra workload large enough that voice-search growth alone could have doubled serving capacity needs. This finding led directly to the TPU v1, a purpose-built inference accelerator with a $256{\times}256$ systolic array that could deliver 92 TOPS (INT8) within a 75 W power envelope. The first TPUs were deployed in 2015 and served production inference workloads including RankBrain and portions of Google Neural Machine Translation [@jouppi2017]; the AlphaGo match-play system is documented separately [@silver2016].
...
As @tbl-tpu-evolution illustrates, this comparison reveals a divergence in philosophy. GPU evolution is a race to pack more arithmetic and higher-precision Tensor Cores onto a single package, using chiplets (Blackwell) to overcome physical die-size limits. TPU evolution is a race to build more efficient warehouse-scale computers, where the individual chip's performance is secondary to the pod's collective bandwidth and reconfigurability [@barroso2019].
...
HBM solves this problem by changing the physical topology entirely, as @fig-hbm-architecture shows. Instead of routing signals *horizontally* across a PCB, HBM stacks multiple DRAM dies *vertically*, one on top of another, and connects them with **Through-Silicon Vias (TSVs)**\index{Through-Silicon Via!HBM stacking}[^fn-tsv-stacking]: microscopic copper pillars etched through the silicon substrate itself.
...
- **Energy per bit**\index{HBM!energy efficiency}: The transfer cost drops by an order of magnitude, from approximately 20 pJ/bit for DDR5 to approximately 2 pJ/bit for HBM, because shorter traces have lower capacitance and require less driving current.
...
class HbmB200Recap:
    ...
    capacity = Hardware.Cloud.B200.memory.capacity.to(GB)
    low_cost_per_gb = 10
    high_cost_per_gb = 15
    low_cost = capacity.magnitude * low_cost_per_gb
    high_cost = capacity.magnitude * high_cost_per_gb
    ...
    check(round(capacity.magnitude) == 180, "B200 capacity should match the 180 GB nameplate specification.")
    check(round(gpt3_weight_fraction * 100) == 51, "B200 should hold about 51 percent of GPT-3 FP16 weights.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    capacity_gb_str = fmt_memory_capacity(capacity, unit=GB, precision=0, commas=False)
...
As @tbl-bandwidth-hierarchy-compute shows, parallelism strategies must respect these boundaries. A simple synchronization calculation makes the bandwidth gaps concrete.
...
Consider the NVIDIA DGX H100 architecture\index{DGX}. Eight H100 GPUs sit on a single baseboard, each connected to four NVSwitch chips via 18 NVLink 4.0 lanes. The NVSwitch chips form a non-blocking crossbar[^fn-crossbar-nvswitch] through which any GPU can reach any other GPU. Each GPU's port budget is `{python} BandwidthHierarchyNvlinkRecap.nvlink_bw_gb_per_s_str` bidirectional, or `{python} BandwidthHierarchyNvlinkRecap.nvlink_per_direction_gb_per_s_str` in one direction, shared across that GPU's simultaneous transfers; it is not a separate full-bandwidth link for every GPU pair.
...
class TensorParallelAllReduce:
    ...
    pcie_per_allreduce_ms_str = fmt_time(pcie_per_allreduce, millisecond, precision=1, commas=False)
...
**Diagnosis**: Tensor parallelism requires collectives throughout the Transformer stack, not just *once* per training step. In this simplified scenario, a model with `{python} TensorParallelAllReduce.layers_str` layers generates `{python} TensorParallelAllReduce.layers_str` AllReduce operations per forward pass and another `{python} TensorParallelAllReduce.layers_str` during the backward pass, totaling `{python} TensorParallelAllReduce.sync_events_str` synchronization events per step. Including the ideal ring traffic factor, each PCIe AllReduce for a `{python} TensorParallelAllReduce.hidden_str`-dimensional hidden state takes approximately `{python} TensorParallelAllReduce.pcie_per_allreduce_ms_str`, accumulating to `{python} TensorParallelAllReduce.pcie_cumulative_ms_str` of pure communication time per step. With a `{python} TensorParallelAllReduce.compute_step_ms_str` compute-step anchor and no overlap, the accelerators spend roughly `{python} TensorParallelAllReduce.pcie_wait_pct_str` of combined time waiting for PCIe transfers. Switching to NVLink reduces the ideal lower bound to `{python} TensorParallelAllReduce.per_allreduce_ms_str` per AllReduce and `{python} TensorParallelAllReduce.cumulative_ms_str` total; computation then accounts for `{python} TensorParallelAllReduce.nvlink_compute_fraction_pct_str` of combined time before overlap.

**Systems lesson**: For communication-heavy tensor parallelism, NVLink cuts the ideal transfer-time floor by about an order of magnitude relative to PCIe, while achieved performance still depends on overlap, collective implementation, and contention.
<!-- end lego-ok-block -->
...
**Resolution**: Reduce the per-GPU microbatch size to lower peak activation memory, then use gradient accumulation\index{Gradient Accumulation} over 2--4 such microbatches before synchronization and the optimizer update to preserve the effective batch size. Gradient offloading\index{Gradient Offloading} stores inactive gradient shards in DDR5. With these techniques, the per-GPU HBM usage drops to approximately 70--75 GB, fitting within the 80 GB envelope with a small margin for NCCL communication buffers.
<!-- end lego-ok-block -->
...
::: {#cs-compute-infrastructure-ovh-strasbourg-fire .callout-case-study title="The data center that became the failure domain"}
...
Training our `{python} InfraFrontierStragglerRecap10.frontier_params_b_str` model on a single DGX H100 node (8 GPUs, roughly 16,000 TFLOP/s aggregate) would take several months, assuming we can fit the model at all. Reducing training time to weeks requires 100--1,000 nodes operating in concert. A pod is the cluster-scale unit that makes this possible: many racks under one high-speed fabric, power envelope, and cooling design. Pod design is the first point where topology, power, and failure domains become shared infrastructure rather than per-rack concerns.
...
Because the bottlenecks move rather than disappear, compute infrastructure teams often optimize the visible specification while missing the binding physical constraint. The fallacies below translate the chapter's node, rack, pod, and facility lessons into operational warnings.
...
class SummaryScaleScenario:
    ...
    nvlink_ib_gap_mult_str = fmt_multiple(
        (Hardware.Cloud.H100.nvlink.bandwidth_per_direction / Systems.Fabrics.InfiniBand_NDR.bandwidth).to_base_units().magnitude,
        precision=0,
        commas=False,
    )
```
