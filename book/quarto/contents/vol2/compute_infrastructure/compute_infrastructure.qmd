...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: TensorCoreTdpRecap.h100_fp16_tc_tflops_str, TensorCoreTdpRecap.h100_fp8_tc_tflops_str, TensorCoreTdpRecap.tc_vs_cuda_ratio_mult_str, TensorCoreTdpRecap.h100_fp32_cuda_tflops_str
...
1.  **Significance (quantitative)**: Tensor Cores provide the bulk of the H100's `{python} TensorCoreTdpRecap.h100_fp16_tc_tflops_str` FP16/BF16 peak (and ~`{python} TensorCoreTdpRecap.h100_fp8_tc_tflops_str` at FP8)—roughly `{python} TensorCoreTdpRecap.tc_vs_cuda_ratio_mult_str` the ~`{python} TensorCoreTdpRecap.h100_fp32_cuda_tflops_str` delivered by CUDA (vector) cores alone. However, this peak is only achievable for operations that decompose into tiled matrix multiplications. Layer normalization and softmax, which involve reductions and element-wise operations rather than matrix multiplications, run at approximately 3–8 percent of peak throughput on the same hardware.
...
# │ Exports: bs_hbm_bw_gb_s_str, bs_nvlink_bw_gb_s_str, bs_ib_bw_gb_s_str, bs_gap_nv_ib_mult_str
# └─────────────────────────────────────────────────────────────────────────────
