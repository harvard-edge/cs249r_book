...
**Scenario**: An engineering team evaluates hardware choices for serving (decode batch size 1) versus training (batch size `{python} RooflineInferenceMath.batch_size_str`) a `{python} InfraFrontierMemoryMxRecap.frontier_params_b_str` parameter model on an H100 GPU (ridge point `{python} RooflineCalloutRecap.h100_ridge_str` FLOP/byte).

**Diagnosis**: Single-token decode achieves arithmetic intensity $I =$ `{python} RooflineInferenceMath.inf_intensity_str` FLOP/byte $\ll$ `{python} RooflineCalloutRecap.h100_ridge_str`, bounding throughput to `{python} RooflineInferenceMath.inf_peak_pct_str` of peak FLOPs due to HBM bandwidth. Training batching increases intensity to $I =$ `{python} RooflineInferenceMath.train_intensity_str` FLOP/byte $\gg$ `{python} RooflineCalloutRecap.h100_ridge_str`, shifting the workload to compute-bound peak TFLOPs.
