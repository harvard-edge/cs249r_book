...
# │          die_area_mm2_str, heat_flux_str, stove_flux_str, stove_ratio_str
class H100TdpWallRecap:

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    tdp = Hardware.Cloud.H100.tdp
    die_area_mm2 = 814  # H100 SXM5 die area (~29x28 mm)
    stove_flux_w_cm2 = 10  # electric stovetop burner at high heat (reference)

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    tdp_w = tdp.to(watt).magnitude
    heat_flux_w_cm2 = tdp_w / (die_area_mm2 / 100)  # W per cm^2
    stove_ratio = heat_flux_w_cm2 / stove_flux_w_cm2

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_w_str = fmt_qty(tdp, watt, precision=0, commas=False)
    flops_fp16_tflop_per_s_str = fmt_qty(Hardware.Cloud.H100.compute.peak_flops, TFLOP / second, precision=0, commas=False)
    die_area_mm2_str = fmt_int(die_area_mm2)
    heat_flux_str = fmt_int(heat_flux_w_cm2)
    stove_flux_str = fmt_int(stove_flux_w_cm2)
    stove_ratio_str = fmt(stove_ratio, precision=1)
```
...
To appreciate the physical challenge, consider what `{python} H100TdpWallRecap.tdp_w_str` means in terms of heat flux. An H100 die measures approximately `{python} H100TdpWallRecap.die_area_mm2_str` mm$^2$ (roughly $29{\times}28$ mm). Dissipating `{python} H100TdpWallRecap.tdp_w_str` from this area produces a heat flux of approximately `{python} H100TdpWallRecap.heat_flux_str` W/cm$^2$. For comparison, the surface of an electric stovetop burner at high heat produces approximately `{python} H100TdpWallRecap.stove_flux_str` W/cm$^2$. An H100 die generates `{python} H100TdpWallRecap.stove_ratio_str`$\times$ the heat flux of a stove burner, concentrated on an area the size of a large postage stamp.
...
class TensorParallelAllReduce:
    ...
    shard_math = MarkdownStr(f"${hidden_disp}{{\\times}}{shard_disp}$")
    allreduce_formula_math = MarkdownStr(f"${microbatch} \\times {seq_disp} \\times {hidden_disp} \\times {bytes_fp16}$")
    ...
    cumulative_ms_str = fmt_int(cumulative.to("millisecond").magnitude)
```
...
Even with `{python} TensorParallelAllReduce.layers_str` layers (forward pass) and `{python} TensorParallelAllReduce.layers_str` layers (backward pass), the cumulative AllReduce time is about `{python} TensorParallelAllReduce.cumulative_ms_str` ms per training step. This is possible because the NVSwitch crossbar allows all 8 GPUs to communicate simultaneously without contention, unlike a ring topology where data must traverse multiple hops sequentially.
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: GpuHourEquivalence.ratio_a100_str, ratio_v100_str,
