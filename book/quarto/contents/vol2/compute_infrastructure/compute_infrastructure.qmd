...
class RooflineInferenceMath:
    ...
    inf_throughput_math = md_math(f"{InfraSetup.h100_bw} \\times 10^{{12}}")
...
For our `{python} InfraSetup.frontier_params_b`B model, training across 1,024 GPUs requires approximately 128 racks. Each rack dissipates `{python} RackPowerScenario.rack_power_str` kW as heat, the thermal output of a small industrial furnace. The aggregate power draw of the training cluster approaches 5 MW, enough to power a small town. Delivering this power reliably, converting it efficiently, and removing the resulting heat without allowing any component to exceed its thermal limit is a multi-disciplinary engineering challenge that spans electrical, mechanical, and civil engineering. A failure at any point in the power delivery chain, from the utility substation to the individual GPU voltage regulator, can halt the entire training run, wasting hours of computation and potentially corrupting the training state.
