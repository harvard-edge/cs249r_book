...
While **Archetype A (GPT-4)** is primarily throughput-bound (demanding more TFLOPS), **Archetype B (DLRM at Scale)**—the Deep Learning Recommendation Model workload—is primarily capacity-bound. A 10 TB embedding table cannot fit into the 80 GB HBM of a single H100, nor can it fit into the aggregate HBM of a single 8-GPU node (640 GB). This physical limit forces Archetype B to shard its state across hundreds of nodes, transforming a memory-access problem into a massive **All-to-All** network coordination problem.
