...
Our `{python} InfraFrontierNodeMemRecap.frontier_params_b_str` model needs 350 GB for its FP16 weights alone. A single H100 provides 80 GB of HBM, so we need at least five accelerators just to place the weights. An 8-H100 dense node\index{Node} can hold those weight shards, but it cannot hold the full Adam training state in HBM: optimizer moments, gradients, activations, and communication buffers push the requirement into the 2 TB range. This arithmetic forces us beyond the single die to the next physical level: the node, and it also forces the training system to combine tensor parallelism with optimizer sharding and memory offload.
...
# │          EmergingInfrastructureRecap.wse_sram_bw_pb_per_s_str,
...
class EmergingInfrastructureRecap:
    ...
    wse_sram_bw_pb_per_s_str = fmt_bandwidth(wse.memory.bandwidth, unit=PB / second, precision=0, commas=False)
...
At the opposite extreme from CXL, **wafer-scale integration**\index{Wafer-Scale Integration!definition} tries to make the cluster look like a chip. Cerebras Systems' CS-3 system, built around the WSE-3, exposes `{python} EmergingInfrastructureRecap.wse_sram_gb_str` of on-wafer SRAM and `{python} EmergingInfrastructureRecap.wse_sram_bw_pb_per_s_str` of local memory bandwidth. When a workload's active state fits this on-wafer memory, the usual chip-to-chip communication wall largely disappears: data moves across an on-die mesh instead of crossing package traces, board links, or a data-center fabric.

The new wall is capacity and injection bandwidth. Our `{python} EmergingInfrastructureRecap.frontier_params_b_str` running model needs `{python} EmergingInfrastructureRecap.frontier_weights_gb_str` for FP16 weights alone, far beyond the WSE's on-wafer SRAM. Cerebras addresses this class of workload through weight streaming, where external MemoryX units feed weights to the wafer over a `{python} EmergingInfrastructureRecap.wse_memoryx_bw_tb_per_s_str` path while the wafer executes the layer. That can be a good match for inference and for workloads whose computation can be staged cleanly, but training reintroduces harder scheduling questions because activations, gradients, and optimizer updates must be coordinated across forward and backward passes.
