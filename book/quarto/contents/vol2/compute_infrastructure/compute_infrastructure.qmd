...
Mixture-of-Experts\index{Mixture-of-Experts|see{MoE}}\index{MoE!infrastructure challenge} (MoE) and multi-modal models add a different constraint: routing and heterogeneity. MoE reduces per-token compute by activating only a subset of experts; Mixtral-8x7B [@jiang2024mixtral], for example, has `{python} MoETdpRecap.total_params_b_str` total parameters but only `{python} MoETdpRecap.active_params_b_str` active parameters per token because the router chooses `{python} MoETdpRecap.active_experts_count_str` from `{python} MoETdpRecap.experts_count_str`. That compute saving introduces a many-to-many token exchange pattern, the AllToAll collective examined in @sec-collective-communication, and load-balance risk because tokens must be routed to different expert placements; high-bisection fabrics and load-balancing mechanisms such as auxiliary losses or capacity factors determine whether the theoretical saving appears in the training trace. Multi-modal models combine text decode, vision processing, audio features, and sometimes video, each with different arithmetic intensity and batching behavior. These workloads favor platforms with flexible scheduling and strong bisection bandwidth rather than accelerators optimized for one regular dataflow.
...
#### Build-vs-buy TCO threshold {#sec-compute-infrastructure-build-vs-buy-tco}
