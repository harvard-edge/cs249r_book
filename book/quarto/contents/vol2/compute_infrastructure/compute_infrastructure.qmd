...
class TensorParallelAllReduce:
    ...
    pcie_per_allreduce = (allreduce_bytes / pcie_bw).to(millisecond)
    ...
    nvlink_utilization = (compute_step / (compute_step + cumulative)).to_base_units().magnitude
    ...
    nvlink_util_pct_str = fmt_percent(nvlink_utilization, precision=1, commas=False, style='prose')
```
...
**Diagnosis**: Tensor parallelism requires AllReduce after *every* Transformer layer, not just *once* per training step. A model with `{python} TensorParallelAllReduce.layers_str` layers generates `{python} TensorParallelAllReduce.layers_str` AllReduce operations per forward pass and another `{python} TensorParallelAllReduce.layers_str` during the backward pass, totaling `{python} TensorParallelAllReduce.sync_events_str` synchronization events per step. At PCIe bandwidth, each AllReduce for a `{python} TensorParallelAllReduce.hidden_str`-dimensional hidden state takes approximately `{python} TensorParallelAllReduce.pcie_per_allreduce_ms_str`, accumulating to `{python} TensorParallelAllReduce.pcie_cumulative_ms_str` of pure communication overhead per step. With a `{python} TensorParallelAllReduce.compute_step_ms_str` compute-step anchor, the accelerators spend roughly `{python} TensorParallelAllReduce.pcie_wait_pct_str` of their time waiting for PCIe transfers. Switching to NVLink reduces each AllReduce to `{python} TensorParallelAllReduce.per_allreduce_ms_str`, bringing total communication overhead to `{python} TensorParallelAllReduce.cumulative_ms_str` and restoring accelerator utilization to over `{python} TensorParallelAllReduce.nvlink_util_pct_str`.
