...
**Scenario**: An engineering team evaluates hardware choices for serving (decode batch size 1) versus training (batch size `{python} RooflineInferenceMath.batch_size_str`) a `{python} RooflineInferenceMath.params_b_str` parameter model on an H100 GPU (ridge point `{python} RooflineInferenceMath.h100_ridge_str` FLOP/byte).

**Diagnosis**: Single-token decode achieves arithmetic intensity $I =$ `{python} RooflineInferenceMath.inf_intensity_str` FLOP/byte $\ll$ `{python} RooflineInferenceMath.h100_ridge_str`, bounding throughput to `{python} RooflineInferenceMath.inf_peak_pct_str` of peak FLOPs due to HBM bandwidth. Training batching increases intensity to $I =$ `{python} RooflineInferenceMath.train_intensity_str` FLOP/byte $\gg$ `{python} RooflineInferenceMath.h100_ridge_str`, shifting the workload to compute-bound peak TFLOPs.
...
**Diagnosis**: Un-sharded optimizer state alone requires `{python} InfraFrontierNodeModelRecap.opt_state_gb_str` per GPU, causing per-GPU memory (`{python} InfraFrontierNodeModelRecap.total_per_gpu_gb_str`) to exceed the 80 GB HBM limit. Activation memory adds 100–200 GB per GPU without activation checkpointing.

**Systems lesson**: Sizing training memory requires managing weights, gradients, optimizer states, activations, and buffers holistically. Offloading optimizer states to host DDR5 and applying activation checkpointing reduces per-GPU HBM usage to 70–75 GB, fitting within the 80 GB envelope.
