...
from mlsys.constants import (
    ...
    Mparam, TFLOPs, flop, param, MFLOPs, GFLOPs
)
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Used across the chapter for hierarchy tables and TCO analysis.
# │
# │ Goal: Provide quantitative specs for hardware generations.
# │ Show: The scaling of compute density and the shift to liquid cooling.
# └─────────────────────────────────────────────────────────────────────────────
...
class InfraSetup:
    ...
    # Ridge points
    h100_ridge_val = (h100_flops / h100_bw).to(flop/byte).magnitude
    ...
    # Level 3: Rack
    nodes_per_rack = 4
    rack_power_kw = (h100_tdp * gpus_per_node * nodes_per_rack).to(kilowatt).magnitude * 1.2
    pue_efficient = 1.10

    # Economics
    dgx_h100_price = 350000 # USD
    electricity_rate = CLOUD_ELECTRICITY_PER_KWH.to(USD/(kilowatt*hour)).magnitude
    utilization = 0.80
    amortization_years = 3

    # Annual OpEx for 1 Node
    node_power_kw = (h100_tdp * gpus_per_node).to(kilowatt).magnitude * 1.2
    ...
    # Efficiency
    h100_tflops_per_watt = h100_flops.to(TFLOPs/second).magnitude / h100_tdp.to(watt).magnitude
    a100_tflops_per_watt = a100_flops.to(TFLOPs/second).magnitude / a100_tdp.to(watt).magnitude
    b200_tflops_per_watt = b200_flops.to(TFLOPs/second).magnitude / b200_tdp.to(watt).magnitude
    v100_tflops_per_watt = v100_flops.to(TFLOPs/second).magnitude / v100_tdp.to(watt).magnitude
...
class TokenLatency:
    ...
    t_mem_ms = (weight_vol / hbm_bw.to(byte/second)).to('millisecond').magnitude
    t_comp_ms = (ops_per_token / peak_tflops.to(flop/second)).to('millisecond').magnitude
...
h100_tflops = f"{InfraSetup.h100_flops.to(TFLOPs/second).magnitude:.0f}"
h100_bw = f"{InfraSetup.h100_bw.to(TB/second).magnitude:.2f}"
h100_ridge = f"{InfraSetup.h100_ridge_val:.0f}"
ddr_bw_str = f"{InfraSetup.ddr_bw.to(GB/second).magnitude:.0f}"
nvlink_bw_str = f"{InfraSetup.nvlink_bw.to(GB/second).magnitude:.0f}"
pcie_bw_str = f"{InfraSetup.pcie_bw.to(GB/second).magnitude:.0f}"
ib_bw_str = f"{InfraSetup.ib_bw.to(Gbps).magnitude:.0f}"
ib_bw_gbs = f"{InfraSetup.ib_bw.to(Gbps).magnitude / 8:.0f}"
node_hbm_cap = f"{InfraSetup.node_hbm_capacity.to(GiB).magnitude:.0f}"
...
gpt3_params_b = f"{GPT3_PARAMS.to(BILLION).magnitude:.0f}"
...
Suppose we want to train a `{python} gpt3_params_b`B-parameter language model. The arithmetic is straightforward: each training step requires roughly $6 \times `{python} gpt3_params_b` \times 10^9$ floating-point operations per token, and we need to process trillions of tokens. A modern server CPU can deliver perhaps 1--2 TFLOPS of matrix throughput. At that rate, training would take decades.
...
This question sits at the foundation of every fleet. The smallest physical unit of the machine learning fleet is the accelerator[^fn-accelerator], and every accelerator is a physical response to three fundamental constraints that limit computation:
...
[^fn-accelerator]: **Accelerator**: In the context of ML systems, an accelerator is any processor designed to execute matrix and tensor operations more efficiently than a general-purpose CPU. The term encompasses GPUs, TPUs, and custom ASICs. The key insight is that *all* accelerators trade generality for throughput by dedicating more silicon area to arithmetic units and less to control logic.
...
[^fn-systolic]: **Systolic Array**: The term was coined by H. T. Kung and Charles Leiserson in 1978, drawing an analogy to the rhythmic contraction of the heart (systole). In a systolic array, data pulses through a grid of processing elements in a regular, synchronized pattern, much as blood pulses through the circulatory system. Google's TPU v1 implemented a 256 $\times$ 256 systolic array, containing 65,536 MAC units, which remains one of the largest deployed systolic architectures.
...
% Labels
\node[below=0.2 of (0,-4.2)] {\textbf{300mm Silicon Wafer}};
\node[above right=0.1 and 0.1 of (5.5, 2.7)] {\textbf{Unified Compute Fabric}};
...
```{python}
#| label: fig-accelerator-efficiency-wall
#| fig-cap: "**The Accelerator Efficiency Wall**. FP16 throughput (blue, left axis) and power efficiency (green, right axis) for six generations of NVIDIA datacenter GPUs, both on logarithmic scales. Raw throughput has grown 236$\\times$ from the P100 to the B200, but efficiency (TFLOPS per watt) has grown only 70$\\times$ over the same period. The shaded region highlights the widening gap that the power grid, cooling systems, and datacenter infrastructure must absorb."
#| fig-alt: "Dual-axis log-scale plot showing GPU FP16 TFLOPS and TFLOPS per watt from 2016 to 2024 across six NVIDIA GPU generations."
#| echo: false
import numpy as np
...
[^fn-hbm]: **HBM (High Bandwidth Memory)**: First standardized by JEDEC in 2013 as a joint development between AMD and SK Hynix, HBM was originally designed for graphics cards. Its adoption by ML accelerators followed the realization that neural network workloads exhibit the same bandwidth-hungry, capacity-moderate access patterns as high-end graphics rendering. The HBM standard has since evolved through five generations (HBM, HBM2, HBM2e, HBM3, HBM3e), with each generation increasing both bandwidth and stack height.

::: {.callout-definition title="High Bandwidth Memory (HBM)"}

***High Bandwidth Memory (HBM)***\index{High Bandwidth Memory!definition} is a specialized DRAM architecture that uses 3D stacking of memory dies connected via **Through-Silicon Vias (TSVs)**. By placing the memory stack on the same silicon substrate as the processor, HBM minimizes the physical distance data must travel and maximizes the number of parallel data paths, achieving bandwidths 50 $\times$ greater than conventional DRAM.
...
HBM solves this problem by changing the physical topology entirely. Instead of routing signals horizontally across a PCB, HBM stacks multiple DRAM dies *vertically*, one on top of another, and connects them with **Through-Silicon Vias (TSVs)**[^fn-tsv]: microscopic copper pillars etched through the silicon substrate itself. This vertical stacking is a paradigm shift in memory architecture: rather than increasing bandwidth by pushing signals faster through long copper traces (the DDR approach, which has diminishing returns), HBM increases bandwidth by multiplying the number of parallel signal paths through extremely short vertical connections.
...
[^fn-tsv]: **TSV (Through-Silicon Via)**: A vertical electrical connection that passes through a silicon wafer or die. TSVs are typically 5--10 micrometers in diameter and 50--100 micrometers deep. The technology was originally developed for CMOS image sensors (the stacked sensors in smartphone cameras) before being adapted for memory stacking. Each HBM stack contains over 5,000 TSVs, providing the massive parallelism that enables terabyte-per-second bandwidths.
...
1. **Compute Time**: Each token requires $2 \times 70\text{B} = 1.4 \times 10^{11}$ floating-point operations. At `{python} h100_tflops` TFLOPS peak throughput, the arithmetic takes:
   $T_{comp} \approx \mathbf{`{python} TokenLatency.t_comp_str` \text{ ms}}$.
2. **Memory Time**: Loading 140 GB of weights from HBM at 3.35 TB/s takes:
   $T_{mem} \approx \mathbf{`{python} TokenLatency.t_mem_str` \text{ ms}}$.

**Conclusion**: The processor spends **`{python} TokenLatency.dominance_str`%** of its time waiting for data from memory. The arithmetic units are idle for almost the entire token generation. Even a hypothetical processor with *infinite* compute throughput would generate tokens only negligibly faster, because the memory transfer time dominates completely. This is why HBM bandwidth improvements deliver nearly linear speedups for inference workloads.
...
```{python}
#| label: fig-compute-memory-divergence
#| echo: false
#| fig-cap: "**The Compute-Memory Divergence**. GPU compute throughput (FP16 Tensor TFLOPS) has grown approximately 36x from V100 to B200, while HBM bandwidth has grown only 9x over the same period. The widening gap between these curves is the Memory Wall: each generation of accelerator becomes proportionally more starved for data."
#| fig-alt: "Dual-axis log-scale plot showing GPU compute growth at 36x versus memory bandwidth growth at 9x from 2017 to 2024, with diverging slopes illustrating the Memory Wall."
...
The token latency calculation above demonstrated that a 70B model on an H100 is overwhelmingly memory-bound. But how do we determine this systematically for *any* workload on *any* hardware? The **Roofline Model**[^fn-roofline], introduced by Williams, Waterman, and Patterson [@williams2009roofline], provides a visual and analytical framework that answers this question with a single number: the arithmetic intensity of the workload.

[^fn-roofline]: **Roofline Model**: Originally developed for multicore processors at UC Berkeley, the Roofline Model has become the standard diagnostic tool for understanding hardware utilization in high-performance computing. The model's power lies in its simplicity: it reduces the complex interaction between compute and memory to a single two-dimensional plot where any workload can be located at a glance.

::: {.callout-definition title="Arithmetic Intensity"}

***Arithmetic Intensity***\index{Arithmetic Intensity!definition} ($I$) is the ratio of floating-point operations performed to the number of bytes transferred from memory:

$$I = \frac{\text{FLOPs}}{\text{Bytes Transferred}}$$

It is measured in FLOP/byte and characterizes the *computational work per unit of data movement*. A high arithmetic intensity means the workload reuses data extensively; a low arithmetic intensity means each datum is used briefly and then discarded.
...
***Ridge Point***\index{Ridge Point!definition} is the arithmetic intensity at which the memory bandwidth ceiling meets the compute ceiling:

$$I_{\text{ridge}} = \frac{\text{Peak Compute (FLOP/s)}}{\text{Memory BW (byte/s)}}$$

For an H100, $I_{\text{ridge}} \approx$ `{python} h100_ridge` FLOP/byte. Workloads with $I < I_{\text{ridge}}$ are **memory-bound**; workloads with $I > I_{\text{ridge}}$ are **compute-bound**.
...
If the point lies below either line, the workload is not fully utilizing the available resource. This gap indicates an optimization opportunity in the software: kernel inefficiency, poor memory access patterns, or excessive synchronization. Closing this gap is the province of kernel engineering and communication optimization, topics examined in @sec-distributed-training-systems.
...
```{python}
#| label: fig-roofline-landscape
#| echo: false
#| fig-cap: "**The Roofline Landscape: NVIDIA H100**. The roofline is defined by two ceilings: memory bandwidth (diagonal) and peak compute (horizontal), meeting at the ridge point (~591 FLOP/byte). ML workloads span the full range: LLM decode at batch size 1 is deeply memory-bound, while LLM training at large batch sizes is compute-bound. The position of each workload determines whether faster memory or faster compute would improve performance."
#| fig-alt: "Log-log roofline plot for H100 showing memory-bound and compute-bound regions with five ML workloads plotted from LLM decode at 1 FLOP per byte to LLM training at 2000 FLOP per byte."
import matplotlib.pyplot as plt
import numpy as np
...
**Inference (Decode, Batch Size 1)**: Each token loads 350 GB of FP16 weights and performs $2 \times `{python} gpt3_params_b` \times 10^9 = 350 \times 10^9$ FLOPs. Arithmetic intensity: $I = 350 \times 10^9 / 350 \times 10^9 = 1.0$ FLOP/byte. Since $1.0 \ll `{python} h100_ridge`$ (the ridge point), the workload is deeply memory-bound. The achievable throughput is $`{python} h100_bw` \times 10^{12} \times 1.0 = 3.35$ TFLOPS, which is less than 0.2% of the H100's peak `{python} h100_tflops` TFLOPS. No amount of additional compute will help; only more memory bandwidth improves throughput.

**Training (Forward Pass, Batch Size 2048)**: The same weight tensor is multiplied by a batch of 2048 activation vectors simultaneously, turning matrix-vector operations into matrix-matrix operations. The FLOPs increase by 2048 $\times$ while the weight loading remains constant: $I = 2048 \times 1.0 = 2{,}048$ FLOP/byte. Since $2{,}048 \gg `{python} h100_ridge`$, the workload is compute-bound. The achievable throughput is now limited by the peak compute ceiling of `{python} h100_tflops` TFLOPS, and the memory bandwidth is irrelevant. Adding more TFLOPS (through a newer GPU generation) directly improves performance.
...
The Roofline Model tells us *whether* a workload can reach peak compute. But what determines that peak in the first place? The answer lies in the specialized arithmetic units that occupy the majority of a modern accelerator's die area: **Tensor Cores**[^fn-tensor-core] on NVIDIA GPUs and **Matrix Multiply Units (MXUs)** on Google TPUs.

[^fn-tensor-core]: **Tensor Core**: Introduced with NVIDIA's Volta architecture in 2017, Tensor Cores are specialized hardware units that perform a fused multiply-accumulate operation on small matrices (originally 4 $\times$ 4 $\times$ 4 in FP16) in a single clock cycle. Each subsequent GPU generation has expanded the supported matrix dimensions and precisions. The H100's fourth-generation Tensor Cores support FP8, FP16, BF16, TF32, and INT8 precisions with matrix dimensions up to 16 $\times$ 16 $\times$ 16.
...
***Tensor Core***\index{Tensor Core!definition} is a specialized hardware unit within an NVIDIA GPU that performs a fused matrix-multiply-accumulate (MMA) operation on small tile matrices in a single clock cycle: $D = A \times B + C$. Each Tensor Core computes the result for a small tile (e.g., 16 $\times$ 16 $\times$ 16 for FP16 on H100), producing thousands of multiply-accumulate results simultaneously. The key distinction from traditional SIMD/SIMT execution is that Tensor Cores operate on *matrices*, not individual elements or vectors, enabling significantly higher throughput for structured mathematical operations.
...
***Pipeline Bubble***\index{Pipeline Bubble!definition} is the idle time in pipeline-parallel training caused by stages waiting for inputs from upstream stages during the fill and drain phases of each micro-batch cycle. When a model is divided into $p$ pipeline stages processing $m$ micro-batches, the bubble fraction is approximately $(p-1)/m$, representing the fraction of total compute time wasted on stage idle periods. Reducing the bubble requires increasing the micro-batch count $m$, which in turn increases peak memory consumption because more intermediate activations must be stored simultaneously. This memory-efficiency trade-off is a central design tension in pipeline-parallel systems, examined in detail in @sec-distributed-training-systems.
...
***Model FLOPS Utilization (MFU)***\index{Model FLOPS Utilization!definition} is the ratio of the floating-point operations actually performed on useful model computation to the hardware's theoretical peak throughput:

$$\text{MFU} = \frac{\text{Achieved FLOPS}}{\text{Peak FLOPS}}$$

MFU captures real-world efficiency by accounting for all sources of overhead: memory stalls, pipeline bubbles, communication pauses, and software inefficiency. Peak specifications are theoretical maximums that assume perfect scheduling and zero idle time. A production training run achieving 40--60% MFU is considered excellent; the gap to 100% reflects the irreducible costs of data movement, synchronization, and control overhead in real systems.
...
Every floating-point operation dissipates energy as heat. A Tensor Core performing 4,096 MACs per cycle at 2 GHz generates heat proportional to the switching activity of billions of transistors. The **Thermal Design Power (TDP)**[^fn-tdp] of an accelerator is the maximum sustained heat dissipation that the cooling system must handle, and it has become the single most consequential specification in fleet design.

[^fn-tdp]: **TDP (Thermal Design Power)**: Often misunderstood as "power consumption," TDP actually specifies the maximum thermal load (in watts) that the cooling solution must be able to remove. In practice, accelerators can briefly exceed their TDP rating during power spikes (a phenomenon called transient power overshoot), which complicates power delivery and cooling design at rack scale.

::: {.callout-definition title="Thermal Design Power (TDP)"}

***Thermal Design Power (TDP)***\index{Thermal Design Power!definition} is the maximum sustained thermal load (in watts) that a processor's cooling solution must be designed to remove. TDP is *not* the same as peak power consumption: an accelerator can briefly exceed its TDP rating during transient spikes (a phenomenon called power overshoot), and it may consume less than its TDP under lighter workloads. TDP establishes the steady-state thermal contract between the chip and the cooling system.

:::

The trajectory of TDP tells the story of a physical law reaching its limits. For three decades, **Dennard scaling**[^fn-dennard] allowed chip designers to shrink transistors while maintaining constant power density: smaller transistors required lower voltage, so packing more transistors into the same area did not increase heat output per unit area. This scaling broke down around 2006 when transistor dimensions reached the point where quantum effects (subthreshold leakage, gate oxide tunneling) prevented further voltage reduction. Since then, each generation of accelerator has increased power consumption roughly in proportion to its performance gains. The era of "free" performance improvements through process shrinks is over; every gain in throughput now comes with a proportional gain in power consumption and heat generation.

[^fn-dennard]: **Dennard Scaling**: Named after Robert Dennard, who observed in 1974 that as transistors shrink, their power density remains constant because voltage and current scale proportionally with their dimensions. The breakdown of Dennard scaling after 2006 is sometimes called the "power wall" and is a primary driver of the shift from frequency scaling to parallelism in computer architecture. For ML accelerators, the post-Dennard era means that every generation of increased performance comes with increased power, which must be physically removed as heat.
...
***Node***\index{Node!definition} is a single physical server chassis containing multiple accelerators (typically 8), connected by a high-speed intra-node interconnect, and backed by substantial host memory. The node is the fundamental failure domain (if one component fails, the entire node is typically lost) and the basic scheduling unit for fleet orchestrators.
...
The economic argument for multi-accelerator nodes is equally compelling. Consider the alternative: building a single accelerator with enough HBM to hold a 175B model (350 GB in FP16). This would require approximately 4--5 HBM stacks at current capacities, with a total interposer area exceeding 2,000 mm$^2$. The manufacturing cost of such a package would be prohibitive (yield decreases exponentially with area), and the power delivery to a single chip with enough Tensor Cores to utilize all that bandwidth would exceed any practical cooling solution. By distributing the computation across 8 accelerators connected by NVLink, the node achieves the aggregate memory capacity and compute throughput of this hypothetical super-chip while remaining within manufacturable and coolable boundaries.
...
: **The Bandwidth Hierarchy**. Each physical boundary introduces an order-of-magnitude bandwidth cliff. These cliffs are not engineering failures to be optimized away; they reflect fundamental differences in the physics of each interconnect medium. The cliffs dictate model partitioning: Tensor Parallelism, which requires AllReduce after every layer, is strictly confined to the intra-node domain. {#tbl-bandwidth-hierarchy}

::: {.callout-definition title="The Bandwidth Hierarchy"}

***Bandwidth Hierarchy***\index{Bandwidth Hierarchy!definition} is the physical ordering of interconnect bandwidths from fastest to slowest: on-chip SRAM (tens of TB/s), device memory such as HBM (single-digit TB/s), intra-node links such as NVLink (hundreds of GB/s), inter-node fabric such as InfiniBand or RoCE (tens of GB/s), and wide-area network (single-digit GB/s). Each tier drops by roughly one order of magnitude. This hierarchy is a consequence of immutable physics: shorter signal paths over denser wiring achieve higher bandwidth at lower energy per bit, and each physical boundary (die edge, package edge, chassis boundary, rack boundary) introduces a step change in distance, medium, and protocol overhead. System design must respect this hierarchy; parallelism strategies that place high-bandwidth communication on slow tiers will idle the arithmetic units.
...
[^fn-serdes]: **SerDes (Serializer/Deserializer)**: A pair of circuits that converts parallel data to a serial bit stream for transmission over a high-speed link, and reconverts it at the receiving end. Modern SerDes in NVLink and InfiniBand operate at 112 Gbps per lane (PAM-4 signaling), with each link bundling many lanes. The SerDes is the atomic building block of all high-speed chip-to-chip communication.
...
Consider the NVIDIA DGX H100 architecture. Eight H100 GPUs sit on a single baseboard, each connected to four NVSwitch chips via 18 NVLink 4.0 lanes. The NVSwitch chips form a non-blocking crossbar[^fn-crossbar]: any GPU can communicate with any other GPU at the full `{python} nvlink_bw_str` GB/s bidirectional bandwidth simultaneously, without contention. This is the equivalent of giving every pair of GPUs their own private highway, rather than forcing them to share a single road.

[^fn-crossbar]: **Crossbar Switch**: A switch topology that provides a dedicated path between every input-output pair simultaneously. An $N \times N$ crossbar has $N^2$ crosspoints and achieves full bisection bandwidth (every input can send to a distinct output at full rate simultaneously). NVSwitch implements a 3-stage crossbar using multiple switch chips, each with 64 NVLink ports.
...
1. **Intra-Node (NVLink)**: Bandwidth = `{python} nvlink_bw_str` GB/s.
   $T_{intra} = 1\text{ GB} / `{python} nvlink_bw_str`\text{ GB/s} \approx \mathbf{1.1 \text{ ms}}$.
2. **Inter-Node (InfiniBand NDR)**: Bandwidth = `{python} ib_bw_gbs` GB/s.
   $T_{inter} = 1\text{ GB} / `{python} ib_bw_gbs`\text{ GB/s} \approx \mathbf{20 \text{ ms}}$.
...
***Rack***\index{Rack!definition} is a standard 42U (1.9 m) enclosure that physically concentrates multiple compute nodes, along with their power distribution, cooling infrastructure, and network switches. In ML infrastructure, the rack is the fundamental unit of power delivery (served by a Power Distribution Unit, or PDU) and the boundary at which cooling technology transitions from air to liquid.
...
This is the origin of the **Power Usage Effectiveness (PUE)**[^fn-pue] metric: a PUE of 1.5 means that for every watt consumed by compute, an additional 0.5 watts is consumed by cooling and power distribution overhead.
...
[^fn-pue]: **PUE (Power Usage Effectiveness)**: Defined by The Green Grid consortium in 2007 as the ratio of total facility power to IT equipment power: $\text{PUE} = P_{\text{total}} / P_{\text{IT}}$. A PUE of 1.0 represents the theoretical minimum where all power goes to computation. Google's fleet-wide PUE averages 1.10; the industry average for traditional datacenters is approximately 1.58. For ML-specific facilities with liquid cooling, PUE values of 1.03--1.08 have been demonstrated.

::: {.callout-definition title="Power Usage Effectiveness (PUE)"}

***Power Usage Effectiveness (PUE)***\index{Power Usage Effectiveness!definition} is the ratio of total facility power consumption to the power consumed by IT equipment:

$$\text{PUE} = \frac{P_{\text{facility}}}{P_{\text{IT}}}$$

A PUE of 1.0 means all power goes to computation (impossible in practice). A PUE of 2.0 means the facility uses as much power for cooling, lighting, and distribution as it does for computing. Modern liquid-cooled ML facilities target PUE of 1.05--1.10, meaning 90--95% of all power is used for computation.
...
Training a `{python} gpt3_params_b`B model on this cluster produces approximately 1.3 EFLOPS (exaFLOPS) of sustained throughput. This sounds enormous until one considers that the training requires $6 \times `{python} gpt3_params_b` \times 10^9 \times 300 \times 10^9 \approx 3.15 \times 10^{23}$ total floating-point operations (assuming 300 billion training tokens). At 1.3 EFLOPS, this takes approximately $3.15 \times 10^{23} / 1.3 \times 10^{18} \approx 242{,}000$ seconds, or roughly 67 hours, under idealized conditions.
...
1. **Total FLOPs**: Using the approximation $6 \times N \times P$, where $N = `{python} gpt3_params_b` \times 10^9$ parameters and $P = 300 \times 10^9$ tokens:
   $6 \times `{python} gpt3_params_b` \times 10^9 \times 300 \times 10^9 = 3.15 \times 10^{23}$ FLOPs

2. **Cluster Throughput**: 8,192 H100 GPUs at `{python} h100_tflops` TFLOPS peak, operating at 45% MFU:
   $8{,}192 \times `{python} h100_tflops` \times 10^{12} \times 0.45 \approx 7.3$ EFLOPS sustained
...
***Warehouse-Scale Computer (WSC)***[^fn-wsc]\index{Warehouse-Scale Computer!definition} is a cluster of thousands of compute nodes designed and operated as a single computing entity. In a WSC, the network fabric replaces the system bus, distributed storage replaces the local disk, and a fleet orchestrator replaces the operating system. The term was introduced by Barroso, Clidaras, and Holzle to describe the shift from treating a datacenter as a collection of independent servers to treating it as a *single machine* whose components happen to be connected by wires rather than traces on a PCB [@barroso2018datacenter].

:::

[^fn-wsc]: **WSC (Warehouse-Scale Computer)**: The concept was formalized by Luiz Andre Barroso and Urs Holzle at Google in their 2009 monograph. The key insight is that at Google's scale, the datacenter *is* the computer, and traditional computer architecture principles (memory hierarchies, interconnect topologies, fault tolerance) must be applied at the building level rather than the chip level. This perspective has become the dominant design philosophy for ML training infrastructure.
...
***Scaling Efficiency***\index{Scaling Efficiency!definition} ($\eta$) is a measure of whether adding compute nodes produces proportional speedup. Given a baseline training time $T_1$ on a single node and an observed training time $T_N$ on $N$ nodes, scaling efficiency is $\eta = T_1 / (N \times T_N)$. A value of $\eta = 1.0$ represents perfect linear scaling; real distributed training systems typically achieve $\eta = 0.70\text{--}0.95$ due to communication overhead, pipeline bubbles, and load imbalances. Scaling efficiency is the single most important diagnostic for cluster productivity: a 10,000-GPU cluster at $\eta = 0.4$ delivers the same useful throughput as a 4,000-GPU cluster at $\eta = 1.0$, wasting 60% of the hardware investment.

:::

For data-parallel training of our `{python} gpt3_params_b`B model, the communication cost per step is dominated by the AllReduce of 350 GB of gradients. Using ring-AllReduce over InfiniBand at `{python} ib_bw_gbs` GB/s effective bandwidth, the communication time is approximately $2 \times (N-1)/N \times 350 \text{ GB} / (`{python} ib_bw_gbs` \text{ GB/s})$, which for large $N$ approaches $2 \times 350 / `{python} ib_bw_gbs` \approx 14$ seconds. If the compute time per step is 20 seconds, the total step time is 34 seconds, yielding a scaling efficiency of $20/34 \approx 0.59$. In practice, communication can be overlapped with the backward pass (sending gradients for early layers while computing gradients for later layers), which recovers much of this loss, achieving 70--90% scaling efficiency for well-optimized training systems.
...
- **Compute per step** (assuming batch size 2M tokens, 6 FLOPs per parameter per token):
  $C = 6 \times `{python} gpt3_params_b` \times 10^9 \times 2 \times 10^6 = 2.1 \times 10^{18}$ FLOPs

- **Per-GPU compute time** on 1,024 GPUs, each at `{python} h100_tflops` TFLOPS (50% utilization):
  $T_{compute} = 2.1 \times 10^{18} / (1024 \times `{python} h100_tflops` \times 10^{12} \times 0.5) \approx 2.1$ seconds

- **AllReduce time** for 350 GB of gradients using ring-AllReduce with overlap:
  $T_{comm} \approx 0.5 \times 350 / `{python} ib_bw_gbs` \approx 3.5$ seconds (with 50% overlap)
...
***Hierarchy-Aware Parallelism***\index{Hierarchy-Aware Parallelism!definition} is the practice of mapping different parallelism strategies to the bandwidth tiers in the cluster hierarchy so that the most communication-intensive strategies operate over the fastest links. In the standard configuration, tensor parallelism (which requires AllReduce after every layer) is confined to NVLink-connected accelerators within a single node; pipeline parallelism (which exchanges activations between stages once per micro-batch) spans nodes within a rack or pod over InfiniBand; and data parallelism (which synchronizes gradients once per training step) extends across the full cluster. The optimal parallelism mapping mirrors the physical bandwidth hierarchy, and deviating from this mapping wastes accelerator time on communication stalls.
...
The DGX SuperPOD is designed for organizations that run diverse workloads: large language models one week, vision models the next, and scientific simulations on weekends. Its network uses a **fat-tree** topology[^fn-fat-tree] built from InfiniBand NDR switches, providing high-bandwidth, any-to-any connectivity with low hop counts. In a fat-tree, the aggregate bandwidth at each level of the tree matches the bandwidth of the level below, so no bottleneck exists regardless of which pairs of nodes are communicating. A typical SuperPOD configuration connects 32 DGX H100 nodes (256 GPUs) through a two-tier InfiniBand fabric, with larger installations using a three-tier fat-tree to scale to thousands of GPUs.

[^fn-fat-tree]: **Fat-Tree**: A network topology proposed by Charles Leiserson in 1985, where the bandwidth of links increases at higher levels of the tree to prevent congestion. In a non-blocking fat-tree, every node can communicate with every other node at full link bandwidth simultaneously. The fat-tree is the dominant topology for general-purpose HPC and ML clusters because it supports arbitrary communication patterns without hotspots. The practical constraint is cost: the number of switches grows as $O(N^{3/2})$ for $N$ endpoints in a three-tier fat-tree.
...
Google's TPU pods take a fundamentally different approach to scaling. Instead of connecting chips through external switches, each TPU chip has direct **Inter-Chip Interconnect (ICI)** links built into the silicon, connecting it to its neighbors in a 3D torus[^fn-torus] topology. This eliminates the switch as a separate component, reducing cost and latency at the expense of the universal any-to-any connectivity that NVSwitch provides.

[^fn-torus]: **3D Torus**: A network topology where nodes are arranged in a three-dimensional grid, with the edges of each dimension connected to form rings. The torus uses no switches, eliminating a major cost component, but provides lower bisection bandwidth than a fat-tree for non-local communication patterns. @sec-network-fabrics analyzes the bandwidth and latency properties of torus topologies in detail.
...
Having mapped the complete physical stack from transistor to warehouse, we now turn to the question that ultimately determines whether any of this infrastructure gets built: *what does it cost?*
...
[^fn-tco]: **TCO (Total Cost of Ownership)**: A financial framework that captures both the upfront capital expenditure (CapEx) and the ongoing operational expenditure (OpEx) of a system over its entire lifecycle. Originally developed in the IT industry by Gartner in the 1980s, TCO analysis has become critical for ML infrastructure decisions because the operational costs (primarily power) can dominate the hardware acquisition cost.
...
Cloud providers charge by the GPU-hour. At approximately $4.00 per H100-hour, a single 8-GPU node running at `{python} f"{InfraSetup.utilization*100:.0f}"`% utilization costs `{python} f"${cloud_node_annual_str}"` per year. An on-premises DGX H100 node costs approximately $350,000 to purchase. Amortized over three years and combined with electricity costs of `{python} f"${annual_elec_cost_str}"` per node per year, the total annual on-premises cost is approximately `{python} f"${onprem_node_annual_str}"` per node. On-premises infrastructure becomes favorable when sustained utilization exceeds roughly `{python} breakeven_util`%.

Several factors complicate this simple comparison. Cloud pricing is highly dynamic, with multiple pricing tiers that can significantly shift the economics. Reserved instances (1--3 year commitments) reduce the effective hourly rate by 40--60%, narrowing the gap between cloud and on-premises costs. For a 3-year commitment, the effective cloud cost for our 8-GPU node drops from `{python} f"${cloud_node_annual_str}"` to approximately $100,000--140,000 per year, which approaches the on-premises cost.
...
This is one reason why the cloud is often more cost-effective for organizations with bursty workloads: they pay only for the GPU-hours they use, and the cloud provider ensures the hardware is utilized during the gaps between one customer's training runs. The cloud provider amortizes the hardware across multiple customers, achieving 70-90% utilization even when each individual customer uses the hardware only intermittently.
...
The most significant near-term change to the node architecture is **Compute Express Link (CXL)**[^fn-cxl], a cache-coherent interconnect built on the PCIe physical layer that enables CPUs and accelerators to share a unified, coherent memory space. CXL addresses a fundamental limitation of the current architecture: the rigid boundary between GPU HBM and host DRAM.

[^fn-cxl]: **CXL (Compute Express Link)**: An open industry standard developed by the CXL Consortium (founded by Intel in 2019) that enables processors, accelerators, and memory devices to share a coherent memory space. CXL uses the PCIe 5.0 and 6.0 physical layer but adds three sub-protocols: CXL.io (PCIe-compatible I/O), CXL.cache (allows the accelerator to cache host memory coherently), and CXL.mem (allows the host to access device-attached memory). CXL 3.0 (ratified in 2023) adds memory pooling and fabric capabilities.
