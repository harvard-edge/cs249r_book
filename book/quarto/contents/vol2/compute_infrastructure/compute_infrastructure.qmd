...
- **ZeRO Stage 3 (Full Sharding)**: The optimizer states, gradients, and parameters are sharded across all GPUs participating in data parallelism. With 8-way data parallelism within a node, each GPU holds only 1/8 of each component, reducing per-GPU memory to roughly 263 GB of equivalent storage (2,100 GB total state divided by 8). This still exceeds 80 GB per GPU, requiring the combination with other techniques.
