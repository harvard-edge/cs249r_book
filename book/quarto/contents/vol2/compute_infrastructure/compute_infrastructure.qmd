...
class V100SpectrumRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    flops_fp16_str = fmt_qty(h_v100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
    tdp_str = fmt_qty(h_v100.tdp, watt, precision=0, commas=False)
```
...
class A100Specs:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    flops_fp16_str = fmt_flop_rate(flops_fp16_tc, unit=TFLOP / second, precision=0, commas=False)
    mem_bw_str = fmt_bandwidth(mem_bw, unit=GB / second, precision=0, commas=False)
    mem_bw_tbs_str = fmt_bandwidth(mem_bw, unit=TB / second, precision=2, commas=False)
    tdp_str = fmt_power(tdp, unit=watt, precision=0, commas=False)
    nvlink_str = fmt_bandwidth(nvlink, unit=GB / second, precision=0, commas=False)
```
...
class V100NvlinkRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_str = fmt_qty(Hardware.Cloud.V100.nvlink.bandwidth, GB/second, precision=0, commas=False)
```
...
class B200PerformanceScenario:
    ...
    nvlink_str = fmt_bandwidth(nvlink, unit=GB / second, precision=0, commas=True)
```
...
class B200NvlinkRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_str = fmt_qty(Hardware.Cloud.B200.nvlink.bandwidth, GB/second, precision=0, commas=False)
```
...
class MemoryBandwidthIntroRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_bw_str = fmt_qty(Hardware.Cloud.H100.memory.bandwidth, TB/second, precision=2, commas=False)
```
...
class MemoryBandwidthCacheRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_l2_str = fmt_qty(Hardware.Cloud.H100.memory.l2_cache, 'megabyte', precision=0, commas=False)
    h100_mem_str = fmt_qty(Hardware.Cloud.H100.memory.capacity, GiB, precision=0, commas=False, unit_label="GB")
    h100_bw_str = fmt_qty(Hardware.Cloud.H100.memory.bandwidth, TB/second, precision=2, commas=False)
```

Further down is the `{python} MemoryBandwidthCacheRecap.h100_l2_str` L2 cache (~12 TB/s), and finally the `{python} MemoryBandwidthCacheRecap.h100_mem_str` of HBM3 at `{python} MemoryBandwidthCacheRecap.h100_bw_str`.
...
class V100MemoryWallRecap:
    h_v100 = Hardware.Cloud.V100
    mem_bw_str = fmt_qty(h_v100.memory.bandwidth, GB/second, precision=0, commas=False)
    flops_fp16_str = fmt_qty(h_v100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
...
class MemoryBandwidthPitfallRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_bw_str = fmt_qty(Hardware.Cloud.H100.memory.bandwidth, TB/second, precision=2, commas=False)
    h100_flops_fp16_str = fmt_qty(Hardware.Cloud.H100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
```
...
class MemoryBandwidthDdrRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ddr_bw_str = fmt_qty(Hardware.Tech.Storage.HostDram.bandwidth, GB/second, precision=0, commas=False)
```
...
Loading `{python} TokenLatencyScenario.weight_gb_str` of weights from HBM at `{python} MemoryBandwidthLatencyRecap.h100_bw_str` takes `{python} TokenLatencyScenario.t_mem_math`.
...
class RooflineCaptionRecap:
    ...
    h100_bw_str = fmt_qty(h_h100.memory.bandwidth, TB/second, precision=2, commas=False)
...
class RooflineInferenceMath:
    ...
    h100_bw_str = fmt_bandwidth(h100_bw, unit=TB / second, precision=2, commas=False)
...
Consider our `{python} InfraFrontierMemoryMxRecap.frontier_params_b_str` model on an H100 with `{python} RooflineInferenceMath.h100_tflops_str` peak compute and `{python} RooflineInferenceMath.h100_bw_str` memory bandwidth. The ridge point is `{python} RooflineCalloutRecap.h100_ridge_str` FLOP/byte.
...
class TensorCoreTdpRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp16_tc_str = fmt_flop_rate(Hardware.Cloud.H100.compute.peak_flops, unit=TFLOPs/second, precision=0, commas=False)
    h100_fp8_tc_str = fmt_flop_rate(Hardware.Cloud.H100.compute.precision_flops['fp8'], unit=TFLOPs/second, precision=0, commas=True)
    tc_vs_cuda_ratio_str = fmt(64.0, precision=0, commas=False)
    h100_fp32_cuda_str = fmt_flop_rate(Hardware.Cloud.H100.compute.peak_flops / 64, unit=TFLOPs/second, precision=1, commas=False)
```
...
Memory stalls occur when the Tensor Cores complete their current tile multiplication before the next tile has been loaded from HBM. Even with the H100's `{python} MemoryBandwidthTensorRecap.h100_bw_str` HBM bandwidth, the Tensor Cores can consume data faster than HBM can deliver it for certain matrix dimensions, creating idle cycles while the arithmetic units wait for data.
...
class H100TdpWallRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_str = fmt_qty(Hardware.Cloud.H100.tdp, watt, precision=0, commas=False)
    flops_fp16_str = fmt_qty(Hardware.Cloud.H100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
```
...
class V100DennardRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_str = fmt_qty(Hardware.Cloud.V100.tdp, watt, precision=0, commas=False)
```
...
class A100TdpTrajectoryRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_str = fmt_qty(Hardware.Cloud.A100.tdp, watt, precision=0, commas=False)
```
...
class RackPowerTdpRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    rack_power_str = fmt_qty(Hardware.Cloud.H100.tdp * Systems.Nodes.DGX_H100.accelerators_per_node * 4 + 10.3 * kilowatt, kilowatt, precision=1, commas=False)
```
...
class GpuEfficiencyTrajectoryRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_ef_str = fmt_qty(Hardware.Cloud.V100.compute.peak_flops / Hardware.Cloud.V100.tdp, TFLOPs / second / watt, precision=2, commas=False, unit_label="TFLOP/s")
    a100_ef_str = fmt_qty(Hardware.Cloud.A100.compute.peak_flops / Hardware.Cloud.A100.tdp, TFLOPs / second / watt, precision=2, commas=False, unit_label="TFLOP/s")
    h100_ef_str = fmt_qty(Hardware.Cloud.H100.compute.precision_flops["fp8"] / Hardware.Cloud.H100.tdp, TFLOPs / second / watt, precision=2, commas=False, unit_label="TFLOP/s")
    b200_ef_str = fmt_qty(Hardware.Cloud.B200.compute.precision_flops["fp8"] / Hardware.Cloud.B200.tdp, TFLOPs / second / watt, precision=2, commas=False, unit_label="TFLOP/s")
```
...
class H100NvlinkNodeRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth, GB/second, precision=0, commas=False)
```
...
class BandwidthHierarchyTableRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_bw_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth, GB/second, precision=0, commas=False)
    pcie_bw_str = fmt_qty(Hardware.Cloud.H100.interconnect.bandwidth, GB/second, precision=0, commas=False)
    ib_bw_str = fmt_qty(Systems.Fabrics.InfiniBand_NDR.bandwidth, GB/second, precision=0, commas=False)
```
...
class BandwidthStaircase:
    ...
    bs_hbm_bw_str = fmt_bandwidth(hbm_bw, unit=GB / second, precision=0, commas=True)
    bs_nvlink_bw_str = fmt_bandwidth(nvlink_bw, unit=GB / second, precision=0, commas=False)
    bs_ib_bw_str = fmt_bandwidth(ib_bw, unit=GB / second, precision=0, commas=False)
...
While NVLink provides a high-bandwidth freeway for GPU-to-GPU communication, **PCIe Gen5**\index{PCIe Gen5!node hierarchy} serves as the universal glue connecting the heterogeneous components of the node. It links GPUs to the host CPU, GPUs to InfiniBand Host Channel Adapters (HCAs), the host CPU to NVMe storage, and the host CPU to system DRAM. Understanding this topology is critical because PCIe bandwidth -- though substantial at ~`{python} BandwidthHierarchyPcieRecap.pcie_bw_str` per direction (128 GB/s bidirectional) per x16 link -- is often the bottleneck for operations that cross the accelerator boundary, such as data loading, checkpointing, and inter-node gradient synchronization.
...
Consider the NVIDIA DGX H100 architecture. Eight H100 GPUs sit on a single baseboard, each connected to four NVSwitch chips via 18 NVLink 4.0 lanes. The NVSwitch chips form a non-blocking crossbar[^fn-crossbar-nvswitch]: any GPU can communicate with any other GPU at the full `{python} BandwidthHierarchyNvlinkRecap.nvlink_bw_str` bidirectional bandwidth simultaneously, without contention. This is the equivalent of giving every pair of GPUs their own private highway, rather than forcing them to share a single road.
...
class NodeCapacityDenseRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    node_hbm_cap_str = fmt_qty(Hardware.Cloud.H100.memory.capacity * Systems.Nodes.DGX_H100.accelerators_per_node, GiB, precision=0, commas=False, unit_label="GB")
...
class NodeReliabilityScenario:
    ...
    node_mtbf_str = fmt_time(composite_node_mtbf_hours, 'hour', precision=0, commas=True, style='word')
    ...
    recovery_low_str = fmt_time(recovery_low_min, 'minute', precision=0, commas=False, style='word')
    recovery_high_str = fmt_time(recovery_high_min, 'minute', precision=0, commas=False, style='word')
...
class NodeCapacityTrainMemRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    node_hbm_cap_str = fmt_qty(node_hbm, GiB, precision=0, commas=False, unit_label="GB")
...
class H100TdpRackRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_str = fmt_power(gpu_tdp, unit=watt, precision=0, commas=False)
    ...
    power_rate_gws_str = fmt_qty(power_rate, GW / second, precision=2, commas=False)
...
For our `{python} InfraFrontierRackPowerRecap.frontier_params_b_str` model, training across 1,024 GPUs requires approximately 32 racks (4 nodes of 8 GPUs each per rack). Each rack dissipates `{python} RackPowerRackRecap.rack_power_str` as heat, the thermal output of a small industrial furnace. The aggregate facility-relevant power draw of the training cluster is approximately 1.1 MW, enough to power several hundred homes. Delivering this power reliably, converting it efficiently, and removing the resulting heat without allowing any component to exceed its thermal limit is a multi-disciplinary engineering challenge that spans electrical, mechanical, and civil engineering. A failure at any point in the power delivery chain, from the utility substation to the individual GPU voltage regulator, can halt the entire training run, wasting hours of computation and potentially corrupting the training state.
...
class InfraIbBwRackRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ib_bw_str = fmt_qty(Systems.Fabrics.InfiniBand_NDR.bandwidth, Gbps, precision=0, commas=False, unit_label="Gb/s")
```
...
class SummaryScaleScenario:
    ...
    h100_tflops_str = fmt_flop_rate(flops, unit=TFLOP / second, precision=0, commas=False)
    h100_bw_str = fmt_bandwidth(bw, unit=TB / second, precision=2, commas=False)
    h100_tdp_str = fmt_power(tdp, unit=watt, precision=0, commas=False)
    rack_power_str = fmt_power(rack_power, unit=kilowatt, precision=1, commas=False)
```
