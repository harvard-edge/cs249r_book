...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt, fmt_math, fmt_params, fmt_memory, fmt_bandwidth, fmt_flop_rate
...
class RooflineInferenceMath:
    ...
    params_b_str = fmt(params_count / BILLION, precision=0, commas=False)
    h100_bw_value_str = fmt(h100_bw.to(TB / second).magnitude, precision=2, commas=False)
    ...
    inf_weight_gb_value_str = fmt(inf_weight.to(GB).magnitude, precision=0, commas=True)
    inf_intensity_str = fmt(inf_intensity, precision=0, commas=False)
    inf_throughput_value_str = fmt(inf_throughput.to(TFLOP / second).magnitude, precision=2, commas=False)
    ...
    inf_intensity_math = fmt_math(f"\\frac{{2 \\times {params_b_str} \\times 10^9}}{{{inf_weight_gb_value_str} \\times 10^9}} = {inf_intensity_str}")
    inf_throughput_math = fmt_math(f"{h100_bw_value_str} \\times {inf_intensity_str} = {inf_throughput_value_str}")
...
class InfraFrontierNodeBudgetRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_params_b_str = fmt(params.to(param).magnitude / BILLION, precision=0, commas=False)
...
class PodTrainingMath:
    ...
    h100_tflops_math_str = fmt(h100_fp8_flops.to(TFLOP / second).magnitude, precision=0, commas=False)
    ...
    cluster_eflops_str = fmt(cluster_throughput.to(EFLOP / second).magnitude, precision=2, commas=False)
...
class WscThroughput:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    wsc_fp16_eflops_str = fmt(wsc_fp16.to(EFLOP / second).magnitude, precision=2, commas=False)
    wsc_fp8_eflops_str = fmt(wsc_fp8.to(EFLOP / second).magnitude, precision=1, commas=False)
