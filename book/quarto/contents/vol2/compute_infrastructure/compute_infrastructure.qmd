...
- Explain how power density and thermal limits drive the rack's transition from air to liquid cooling
...
# │ Exports: ReticleLimitRecap.reticle_limit_mm2_str,
# │          ReticleLimitRecap.b200_equivalent_area_mm2_str
...
# │
# │ Imports: mlsysim registries (Hardware.Cloud.TPUv5p) + fmt helpers
# │ Exports: TpuV5pRecap.peak_bf16_str, .hbm_gb_str, .hbm_bw_gb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
| **TPU v5p**    |   2023   |     SparseCore, HBM2e      | `{python} TpuV5pRecap.peak_bf16_str` | `{python} TpuV5pRecap.hbm_gb_str` | 1,200 GB/s |
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: HbmPackagingAreaRecap.interposer_area_mm2_str,
# │          HbmPackagingAreaRecap.hbm_stack_area_mm2_str
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: TrillionModelFloor.weight_tb_str, weight_gb_str, b200_bw_tbs_str,
# │          token_b200_ms_str, hbm4_bw_tbs_str, token_hbm4_ms_str,
# │          model_params_t_str, hbm4_low_tbs_n_str, hbm4_tbs_n_str
...
# │ Exports: H200Comparison.bw_gain_mult_str, .cap_gain_mult_str, .speedup_mult_str
# └─────────────────────────────────────────────────────────────────────────────
import mlsysim
...
The cascade shown in @fig-power-path reveals why power delivery is an infrastructure problem, not merely an electrical one: these conversion losses, combined with the overhead of removing heat, determine the facility's total power draw, and the 10--40 kW concentrated at the rack PDU-to-VRM transition dictates the cooling architecture for the entire facility. That cooling overhead is captured by a single facility ratio, Power Usage Effectiveness (PUE), which @eq-pue defines and develops; here it is enough to know that some fraction of every watt drawn from the grid is spent moving heat rather than computing.
...
# │          H100TdpWallRecap.die_area_mm2_str, H100TdpWallRecap.h100_heat_flux_w_per_cm2_str,
# │          H100TdpWallRecap.stove_heat_flux_w_per_cm2_str, H100TdpWallRecap.stove_ratio_mult_str
...
# │ Exports: FutureTdpScenario.future_tdp_w_str, .gpus_per_node_str, .node_gpu_power_kw_str
# └─────────────────────────────────────────────────────────────────────────────
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: Fp8CostEquivalence.cluster_gpus_str, gpus_per_node_str,
# │          node_price_str, total_musd_str
class Fp8CostEquivalence:
    ...
    total_musd_str = fmt_usd(total_usd.magnitude / 1e6, precision=0)
```
...
```{python}
#| echo: false
from mlsysim.fmt import fmt_int, fmt_time, MarkdownStr

# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: TensorParallelAllReduce.hidden_str, tp_gpus_str, model_params_b_str,
# │          shard_math, allreduce_formula_math, allreduce_mb_str,
# │          per_allreduce_ms_str, layers_str, cumulative_ms_str
class TensorParallelAllReduce:
    ...
    per_allreduce = (allreduce_bytes / nvlink_bw).to("millisecond")
    cumulative = 2 * layers * per_allreduce
    ...
    per_allreduce_ms_str = fmt_time(per_allreduce, "millisecond", precision=1, commas=False)
...
The AllReduce for a single layer's output involves exchanging approximately `{python} TensorParallelAllReduce.allreduce_formula_math` bytes (FP16) across the NVLink fabric. For a typical microbatch size of `{python} TensorParallelAllReduce.microbatch_str` sequences at `{python} TensorParallelAllReduce.seq_str` tokens, this is roughly `{python} TensorParallelAllReduce.allreduce_mb_str` MB per AllReduce, which completes in about `{python} TensorParallelAllReduce.per_allreduce_ms_str` at `{python} BandwidthHierarchyNvlinkRecap.nvlink_bw_gb_per_s_str`.

Even with `{python} TensorParallelAllReduce.layers_str` layers (forward pass) and `{python} TensorParallelAllReduce.layers_str` layers (backward pass), the cumulative AllReduce time is about `{python} TensorParallelAllReduce.cumulative_ms_str` per training step. This is possible because the NVSwitch crossbar allows all 8 GPUs to communicate simultaneously without contention, unlike a ring topology where data must traverse multiple hops sequentially.
...
**Diagnosis**: Tensor parallelism requires AllReduce after *every* Transformer layer, not just *once* per training step. A model with ninety-six layers generates ninety-six AllReduce operations per forward pass and another ninety-six during the backward pass, totaling 192 synchronization events per step. At PCIe bandwidth, each AllReduce for a 12,288-dimensional hidden state takes approximately 4 ms, accumulating to 768 ms of pure communication overhead per step. The compute per step is roughly 200 ms. The accelerators were spending nearly 80 percent of their time waiting for PCIe transfers. Switching to NVLink reduced each AllReduce to 0.2 ms, bringing total communication overhead to 38 ms and restoring accelerator utilization to over 80 percent.
...
# │          .hbm_per_gpu_gb_str, .node_hbm_gb_str, .host_dram_tb_str,
# │          .nvme_bw_gb_per_s_str, .nvme_latency_us_str
class MemoryBandwidthNodeTableRecap:
    ...
    gpus_per_node_str = fmt(gpus_per_node, precision=0, commas=False)
    node_hbm_gb_str = fmt_memory_capacity(node_hbm, unit=GiB, precision=0, commas=False)
    host_dram_tb_str = fmt_qty(host_dram, TB, precision=0, commas=False)
...
|:----------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|:-----------------------------------------------------------------------------:|:-------------------------------------------------------------:|:---------------------:|
| **GPU HBM3**    | `{python} MemoryBandwidthNodeTableRecap.hbm_per_gpu_gb_str` $\times$ `{python} MemoryBandwidthNodeTableRecap.gpus_per_node_str` = `{python} MemoryBandwidthNodeTableRecap.node_hbm_gb_str` |     `{python} MemoryBandwidthNodeTableRecap.h100_bw_tb_per_s_str` per GPU     |                             <1 μs                             |   Active computation  |
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: RackPowerRackRecap.rack_power_kw_str, .node_power_kw_str,
# │          .racks_for_1k_str, .cluster_power_mw_str
...
For our `{python} InfraFrontierRackPowerRecap.frontier_params_b_str` model, training across 1,024 GPUs requires `{python} RackPowerRackRecap.racks_for_1k_str` racks (4 nodes of 8 GPUs each per rack). Each rack dissipates `{python} RackPowerRackRecap.rack_power_kw_str` as heat, the thermal output of a small industrial furnace. The aggregate facility-relevant power draw of the training cluster is approximately `{python} RackPowerRackRecap.cluster_power_mw_str`, enough to power several hundred homes. Delivering this power reliably, converting it efficiently, and removing the resulting heat without allowing any component to exceed its thermal limit is a multi-disciplinary engineering challenge that spans electrical, mechanical, and civil engineering. A failure at any point in the power delivery chain, from the utility substation to the individual GPU voltage regulator, can halt the entire training run, wasting hours of computation and potentially corrupting the training state.
...
The concept of the Warehouse-Scale Computer, articulated by Barroso and Holzle at Google, reframes the data center from a *room full of computers* to a *single computer that happens to fill a room*. This reframing has profound implications for physical design.
...
The second consequence is that *failure is a physical certainty, not an exceptional event*. A single GPU has a mean time between failures (MTBF)\index{MTBF} of approximately 50,000 hours (the canonical GPU MTTF anchor `Systems.Reliability.Gpu.mttf_hours` used in @sec-fault-tolerance-reliability), but reliability degrades linearly with component count: a cluster of 10,000 such GPUs experiences a failure roughly once every five hours. @Sec-appdx-reliability-foundations-mtbf-cascade derives why the fleet-level rate is simply the per-component lifetime divided by the component count. This is a building-level property of the physical machine, and it is why a WSC must be designed from the start so that individual component failures degrade rather than halt the running workload, with dual-path power feeds, redundant switch fabrics, and persistent checkpoint storage at every level.
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: GpuHourEquivalence.ratio_a100_mult_str, ratio_v100_mult_str,
# │          v100_hours_str, equiv_h100_hours_str
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: NetworkUpgradeStepTime.ib_hdr_str, ib_ndr_str, compute_s_str,
# │          comm_s_str, total_before_str, total_after_str, improvement_pct_str
...
* **TCO, not CapEx**: The total cost of ownership includes electricity, facility construction, networking, and staffing, not just the purchase price, so ownership pays off only above a utilization threshold that the purchase price alone never reveals. @Sec-ops-scale-fleet-economics works the build-vs-buy breakeven and the full fleet TCO where the fleet is operated and paid for.
