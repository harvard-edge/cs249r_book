...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ INFRASTRUCTURE SPECIFICATIONS AND HIERARCHY (P.I.C.O.)
# ├─────────────────────────────────────────────────────────────────────────────
...
class InfraSetup:
    ...
    Hierarchy:
    - Level 1: Accelerator Specs (The Silicon Contract)
    - Level 2: Node (The First Aggregation)
    - Level 3: Rack (The Infrastructure Unit)
    - Level 4: Ratios (The Physical Laws)
    """
    ...
    # Level 3: Rack
    nodes_per_rack = 4 # Typical DGX-H100 rack density
    pue_efficient = 1.10

    # Economics
    dgx_h100_price = 350000 # USD (est.)
    ...
    # Level 4: Ratios & Physics
    h100_ridge_val = (h100_flops / h100_bw).m_as(flop/byte)
    node_hbm_capacity = h100_cap * gpus_per_node

    # TCO Logic
    node_power_kw = (h100_tdp * gpus_per_node).m_as(kilowatt) * 1.2 # Including CPU/sys
    rack_power_kw_val = node_power_kw * nodes_per_rack
    annual_elec_cost_val = node_power_kw * SEC_PER_YEAR.m_as(hour) * utilization * electricity_rate * pue_efficient

    cloud_node_annual_val = cloud_h100_hourly * gpus_per_node * SEC_PER_YEAR.m_as(hour) * utilization
    onprem_node_annual_capex = dgx_h100_price / amortization_years
    onprem_node_total_annual = onprem_node_annual_capex + annual_elec_cost_val

    # Efficiency Ratios (Level 4)
    ...
    check(h100_ridge_val > 500, f"H100 ridge point should be high (>500), got {h100_ridge_val:.1f}")
    check(h100_tflops_per_watt > v100_tflops_per_watt, "H100 must be more efficient than V100")
    check(onprem_node_total_annual < cloud_node_annual_val, "On-prem should be cheaper at 80% util")

    # ┌── 4. OUTPUTS (Formatting) ──────────────────────────────────────────────
    h100_tflops = f"{h100_flops.m_as(TFLOPs/second):.0f}"
    h100_bw_str = f"{h100_bw.m_as(TB/second):.2f}"
    h100_ridge_str = f"{h100_ridge_val:.0f}"
    ddr_bw_str = f"{ddr_bw.m_as(GB/second):.0f}"
    nvlink_bw_str = f"{nvlink_bw.m_as(GB/second):.0f}"
    pcie_bw_str = f"{pcie_bw.m_as(GB/second):.0f}"
    ib_bw_str = f"{ib_bw.m_as(Gbps):.0f}"
    ib_bw_gbs_str = f"{ib_bw.m_as(GB/second):.0f}"
    node_hbm_cap_str = f"{node_hbm_capacity.m_as(GB):.0f}"
    rack_power_str = f"{rack_power_kw_val:.0f}"
    h100_ef_str = f"{h100_tflops_per_watt:.2f}"
    a100_ef_str = f"{a100_tflops_per_watt:.2f}"
    b200_ef_str = f"{b200_tflops_per_watt:.2f}"
    v100_ef_str = f"{v100_tflops_per_watt:.2f}"
    annual_elec_cost_str = fmt(annual_elec_cost_val, precision=0)
    onprem_node_annual_str = fmt(onprem_node_total_annual, precision=0)
    cloud_node_annual_str = fmt(cloud_node_annual_val, precision=0)
...
h100_bw = InfraSetup.h100_bw_str
h100_ridge = InfraSetup.h100_ridge_str
ddr_bw_str = InfraSetup.ddr_bw_str
nvlink_bw_str = InfraSetup.nvlink_bw_str
pcie_bw_str = InfraSetup.pcie_bw_str
ib_bw_str = InfraSetup.ib_bw_str
ib_bw_gbs = InfraSetup.ib_bw_gbs_str
node_hbm_cap = InfraSetup.node_hbm_cap_str
rack_power_str = InfraSetup.rack_power_str
h100_ef = InfraSetup.h100_ef_str
a100_ef = InfraSetup.a100_ef_str
b200_ef = InfraSetup.b200_ef_str
v100_ef = InfraSetup.v100_ef_str
annual_elec_cost_str = InfraSetup.annual_elec_cost_str
onprem_node_annual_str = InfraSetup.onprem_node_annual_str
cloud_node_annual_str = InfraSetup.cloud_node_annual_str
util_pct_str = InfraSetup.util_pct_str
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TRAINING TIME FOR 175B MODEL (P.I.C.O.)
# ├─────────────────────────────────────────────────────────────────────────────
