...
#| output: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ INFRASTRUCTURE SPECIFICATIONS AND HIERARCHY (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-compute-introduction — chapter-wide hardware specs and math.
# │
# │ Goal: Establish foundational parameters for accelerators and interconnects.
# │ Show: Baseline performance metrics for H100, GPT-3, and communication.
...
# │ Exports: h100_tflops, h100_bw, gpt3_params_b, frontier_params_b,
# │          nvlink_bw_str, ib_bw_gbs, ib_bw_str, t_mem_str, t_comp_str, ...
# └─────────────────────────────────────────────────────────────────────────────
...
class InfraSetup:
    """
    Central namespace for Compute Infrastructure constants.
    """
    # Hardware
    ...
    nvlink_bw_str = f"{nvlink_bw_val:.0f}"
    ib_bw_gbs = f"{ib_bw_gbs_val:.0f}"
    ib_bw_str = f"{ib_bw_gbps_val:.0f}"
    ddr_bw_str = "100" # Approximate for DRAM
    t_mem_str = f"{t_mem_ms:.1f}"
    t_comp_str = f"{t_comp_ms:.2f}"
    dominance_str = f"{dominance:.1f}"

# Math Blocks for Intro
flops_per_token_math = md_math(f"6 \\times {InfraSetup.gpt3_params_b} \\times 10^9")
t_comp_math = md_math(f"T_{{\\text{{comp}}}} \\approx \\mathbf{{{InfraSetup.t_comp_str} \\text{{ ms}}}}")
t_mem_math = md_math(f"T_{{\\text{{mem}}}} \\approx \\mathbf{{{InfraSetup.t_mem_str} \\text{{ ms}}}}")
...
t_intra_math = md_math(f"T_{{\\text{{intra}}}} = 1\\text{{ GB}} / {InfraSetup.nvlink_bw_str}\\text{{ GB/s}} \\approx \\mathbf{{1.1 \\text{{ ms}}}}")
t_inter_math = md_math(f"T_{{\\text{{inter}}}} = 1\\text{{ GB}} / {InfraSetup.ib_bw_gbs}\\text{{ GB/s}} \\approx \\mathbf{{20 \\text{{ ms}}}}")
...
comm_time_formula_math = md_math(f"2 \\times (N-1)/N \\times 350 / {InfraSetup.ib_bw_gbs}")
comm_approx_math = md_math(f"2 \\times 350 / {InfraSetup.ib_bw_gbs}")
```
