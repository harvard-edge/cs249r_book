...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85, transform shape, node distance=0.9cm and 0.6cm]
  \usetikzlibrary{positioning, decorations.pathreplacing}
  \tikzset{
    cell/.style 2 args={draw=#1, fill=#2, rounded corners=2pt, minimum width=3.4cm,
                 minimum height=1.1cm, align=center, font=\scriptsize, line width=0.75pt},
    levelbox/.style={draw=BrownLine, fill=BrownL!40, rounded corners=2pt,
                     minimum width=1.8cm, minimum height=1.1cm, align=center,
                     font=\small\bfseries, line width=0.75pt},
    wallhdr/.style={font=\small\bfseries, align=center},
    cascade/.style={-{Triangle[width=5pt,length=4pt]}, line width=1.0pt, black!40}
  }

  % Row 1: Accelerator
  \node[levelbox] (L1) {Accelerator};
  \node[cell={BlueLine}{BlueL}, right=of L1] (m1) {HBM bandwidth\\bottleneck\\(3.35 TB/s)};
  \node[cell={OrangeLine}{OrangeL}, right=of m1] (p1) {700 W TDP\\per chip};
  \node[cell={GreenLine}{GreenL}, right=of p1] (c1) {On-die SRAM\\capacity limit};

  % Column headers
  \node[wallhdr, text=BlueLine, above=0.3cm of m1] {Memory Wall};
  \node[wallhdr, text=OrangeLine, above=0.3cm of p1] {Power Wall};
  \node[wallhdr, text=GreenLine, above=0.3cm of c1] {Communication Wall};

  % Row 2: Node
  \node[levelbox, below=of L1] (L2) {Node\\(8 GPUs)};
  \node[cell={BlueLine}{BlueL}, below=of m1] (m2) {640 GB aggregate\\HBM capacity\\$<$ model size};
  \node[cell={OrangeLine}{OrangeL}, below=of p1] (p2) {5.6 kW compute\\forces liquid\\cooling};
  \node[cell={GreenLine}{GreenL}, below=of c1] (c2) {NVLink 900 GB/s\\intra-node only};

  % Row 3: Rack
  \node[levelbox, below=of L2] (L3) {Rack\\(4 nodes)};
  \node[cell={BlueLine}{BlueL}, below=of m2] (m3) {Host DRAM\\needed for\\optimizer state};
  \node[cell={OrangeLine}{OrangeL}, below=of p2] (p3) {40+ kW rack\\power delivery\\and cooling};
  \node[cell={GreenLine}{GreenL}, below=of c2] (c3) {PCIe between\\nodes: 64 GB/s};

  % Row 4: Pod
  \node[levelbox, below=of L3] (L4) {Pod\\(1000+ GPUs)};
  \node[cell={BlueLine}{BlueL}, below=of m3] (m4) {Distributed\\storage: PFS\\and object};
  \node[cell={OrangeLine}{OrangeL}, below=of p3] (p4) {Megawatt facility\\PUE $\approx$ 1.1--1.3};
  \node[cell={GreenLine}{GreenL}, below=of c3] (c4) {InfiniBand\\50 GB/s per link\\dominates cost};
  ...
  \draw[RedLine, line width=1.2pt, decorate, decoration={brace, amplitude=6pt, mirror}]
    ([xshift=0.3cm, yshift=-0.2cm]c4.south east) -- ([xshift=0.3cm, yshift=0.2cm]c1.north east)
...
```{.tikz}
\begin{tikzpicture}[line join=round, font=\small\usefont{T1}{phv}{m}{n}, scale=0.8]
\tikzset{
  core/.style={rectangle, draw=BlueLine, fill=BlueL, minimum size=3.5mm, line width=0.75pt, rounded corners=0.5pt},
  defect/.style={rectangle, draw=RedLine, fill=RedL, minimum size=3.5mm, line width=0.75pt, rounded corners=0.5pt},
  link/.style={line width=1.0pt, black!30},
  route/.style={line width=1.2pt, OrangeLine, -{Triangle[length=3pt, width=4pt]}}
}

% Draw the Wafer (circular boundary)
\node[circle, draw=black!10, fill=black!2, line width=1.5pt, minimum size=8.4cm, inner sep=0] (wafer) {};
\node[circle, draw=black!20, line width=0.8pt, minimum size=8.3cm, inner sep=0] at (wafer.center) {};
...
\node[defect, right=of c21] {};
\node[defect, below=of c3-3] {};

% Example routing path
\draw[route] (c-2-2) -- (c-2-1) -- (c-1-1) -- (c0-1) -- (c1-1) -- (c1-2) -- (c2-2) -- (c3-2);

% Zoom inset
\coordinate[right=1.3cm of wafer.east] (zoomcenter);
\begin{scope}[shift={(zoomcenter)}, yshift=1.5cm, scale=1.2]
  \node[draw=black!20, fill=white, rounded corners=3pt, minimum size=2.4cm, line width=0.75pt] (zoombox) {};
  \foreach \x in {-1,0,1} {
    \foreach \y in {-1,0,1} {
  ...
  \node[font=\fontsize{6pt}{6}\selectfont, below=0.05cm of z0-1] {Inter-core Links (PB/s)};
\end{scope}

% Labels
\node[below=0.2cm of wafer.south] {\textbf{300mm Silicon Wafer}};
\node[above=0.2cm of zoombox.north] {\textbf{Unified Compute Fabric}};
...
**Arithmetic Intensity** ($I$) is defined and analyzed at @sec-hw-acceleration-arithmetic-intensity (Volume I) and applied to fleet-scale performance analysis at @sec-performance-engineering-roofline. Briefly: $I = \text{FLOP}/\text{byte}$ — the ratio of computation to memory traffic that determines whether a workload is bandwidth-bound or compute-bound on the Roofline Model.
:::
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.2cm]
  \tikzset{
    stage/.style 2 args={draw=#1, fill=#2, line width=0.75pt, rounded corners=2pt, align=center, minimum width=2.8cm, minimum height=1.0cm},
    flow/.style={-{Triangle[width=5pt,length=4pt]}, line width=1.2pt, black!60},
    annotation/.style={anchor=west, text=#1}
  }

  % Nodes
  \node[stage={BrownLine}{BrownL}] (Sub) {Grid\\Substation\\(115 kV+)};
  \node[stage={BlueLine}{BlueL}, below=of Sub] (UPS) {Facility UPS\\\& Transformer\\(480 V)};
  \node[stage={OrangeLine}{OrangeL}, below=of UPS] (PDU) {Rack PDU\\(208--415 V)};
  \node[stage={OrangeLine}{OrangeL}, below=of PDU] (PSU) {Server PSU\\(12 V / 48 V)};
  \node[stage={RedLine}{RedL}, below=of PSU] (VRM) {On-Board VRM\\(0.8--1.2 V)};
  \node[stage={RedLine}{RedL}, below=of VRM] (Die) {\textbf{GPU Die}\\{(1,000+ Amps)}};
  ...
  % Annotations
  \node[annotation=BlueLine, right=0.5cm of UPS] {\textbf{Facility Level}};
  \node[annotation=OrangeLine, right=0.5cm of PSU] {\textbf{Rack Level}};
  \node[annotation=RedLine, right=0.5cm of Die] {\textbf{Silicon Level}};
...
This density, however, makes power delivery and cooling exponentially harder, creating the engineering challenges that define the rack and pod levels of the hierarchy. The paradox of modern ML infrastructure is that computational efficiency demands physical proximity, while thermal management demands physical separation. Every rack design is a negotiation between these opposing forces.
...
For our `{python} InfraSetup.frontier_params_b`B model, the training memory breaks down as follows:
...
This orchestration is complex but essential: without it, training our `{python} InfraSetup.frontier_params_b`B model on currently available hardware would be impossible. @sec-distributed-training-systems examines these memory optimization strategies and their interaction with parallelism in full detail.
...
This chapter has traced the physical infrastructure of machine learning from the transistor to the datacenter, using the running example of training a `{python} InfraSetup.frontier_params_b`B-parameter model to ground each concept in quantitative reality. The infrastructure stack is not a collection of independent components but an integrated system where decisions at each level constrain and enable choices at every other level. The accelerator's TDP determines the rack's cooling requirements. The rack's power density determines the pod's physical layout. The pod's network topology determines the achievable scaling efficiency. The scaling efficiency, in turn, determines the economics that make the entire enterprise viable or futile.
