...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-compute-infrastructure-evolution-gpu-architectures-566b—accelerator evolution. Plumbs the
...
# │ Show: ~4,500 TFLOPS FP8 at ~1,000 W.
# │ How: pulling B200_FLOPS_FP8_TENSOR and B200_TDP from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (B200_FLOPS_FP8_TENSOR, B200_TDP, TFLOPs,
# │                                  second, watt)
# │ Exports: B200PerformanceScenario.fp8_tflops_str,
# │          B200PerformanceScenario.tdp_w_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    B200_FLOPS_FP8_TENSOR, B200_TDP, TFLOPs, second, watt,
)
from mlsysim.fmt import check
...
class B200PerformanceScenario:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    fp8_tflops = B200_FLOPS_FP8_TENSOR.m_as(TFLOPs / second)
    tdp_w = B200_TDP.m_as(watt)

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # (none — direct extraction)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(fp8_tflops > 4000, "B200 should exceed 4,000 TFLOPS FP8")
    check(tdp_w == 1000, "B200 TDP should be 1,000 W")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    fp8_tflops_str = f"{fp8_tflops:,.0f}"
    tdp_w_str = f"{tdp_w:.0f}"
```

The **Blackwell** architecture (2024) continued this trajectory with the B200, which pairs two GPU dies in a single package via a high-bandwidth chip-to-chip link, effectively creating a "dual-die GPU" that delivers about `{python} B200PerformanceScenario.fp8_tflops_str` TFLOPS of dense FP8 Tensor Core throughput at `{python} B200PerformanceScenario.tdp_w_str` W. FP16/BF16 comparisons should use the lower precision-specific vendor figure rather than the FP8 peak.
...
| **Blackwell** (B200) |   2024   | Dual-die, NVLink 5      | `{python} B200PerformanceScenario.fp8_tflops_str` TFLOPS FP8 | `{python} B200PerformanceScenario.tdp_w_str` W |   1,800 GB/s  |
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-compute-hbm—CPU vs GPU bandwidth
# │
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-compute-roofline—arithmetic intensity
# │
...
# │                                  H100_FLOPS_FP32_CUDA, TFLOPs, second)
# │ Exports: h100_fp16_tc_str, h100_fp8_tc_str, h100_fp32_cuda_str,
# │          tc_vs_cuda_ratio_str, mfu_example_math
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    H100_FLOPS_FP16_TENSOR, H100_FLOPS_FP8_TENSOR,
    H100_FLOPS_FP32_CUDA, TFLOPs, second,
)
from mlsysim.fmt import fmt, check, md_math
...
class TensorCoreH100Specs:
    ...
    fp32_cuda_tflops = H100_FLOPS_FP32_CUDA.m_as(TFLOPs / second)
    ...
    # Pre-build the MFU-example math expression with the constant-driven
    # H100 FP16 peak so the numerator and denominator stay in sync if
    # H100_FLOPS_FP16_TENSOR ever changes. Branded result (~33%) is the
    # pedagogical anchor; the inputs are constants.
    mfu_example_math = md_math(
        f"\\text{{MFU}} = 6 \\times 7 \\times 10^9 / "
        f"({h100_fp16_tc_str} \\times 10^{{12}} \\times 0.13)"
    )
```
...
1.  **Significance (quantitative)**: $\text{MFU} = C_{\text{model}} / (R_{\text{peak}} \cdot T_{\text{step}})$, where $C_{\text{model}} \approx 6P$ FLOPs for a $P$-parameter transformer. For a 7B-parameter model on H100 ($R_{\text{peak}} =$ `{python} TensorCoreH100Specs.h100_fp16_tc_str` TFLOPS FP16) with a step time of 0.13 seconds: `{python} TensorCoreH100Specs.mfu_example_math` $\approx 33\%$. A practical ceiling of 55–65 percent applies even in highly optimized implementations; the gap represents inescapable memory stalls ($\text{BW}$ bottleneck), AllReduce overhead $(L_{\text{lat}})$, and pipeline bubbles.
...
The numbers are stark. The V100 (2017) operated at `{python} V100Specs.tdp_str` W TDP. The A100 (2020) increased to `{python} A100Specs.tdp_str` W. The H100 (2022) reached `{python} H100Specs.tdp_str` W. The B200 (2024) draws `{python} B200PerformanceScenario.tdp_w_str` W. In seven years, TDP has more than tripled, growing at approximately 18 percent per year.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-compute-tdp—thermal density
# │
...
class GpuEfficiencyScenario:
    ...
    b200_fp8_tflops_str = f"{b200_low_precision.m_as(TFLOPs/second):.0f}"
    b200_tdp_w_str = f"{b200_tdp.m_as(watt):.0f}"
```
...
| **B200 (2024)** |                       `{python} GpuEfficiencyScenario.b200_fp8_tflops_str` (FP8) |        `{python} GpuEfficiencyScenario.b200_tdp_w_str` | `{python} GpuEfficiencyScenario.b200_ef_str` |       10.8$\times$      |
