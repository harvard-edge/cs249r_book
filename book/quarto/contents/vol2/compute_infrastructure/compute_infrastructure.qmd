...
1. **Compute Time**\index{Compute!compute time}: Each token requires $2 \times 70\text{B} = 1.4 \times 10^{11}$ floating-point operations. At `{python} InfraSetup.h100_tflops` TFLOPS FP16/BF16 peak throughput, the arithmetic takes:
...
class GpuEfficiencyScenario:
    ...
    h100_tdp = H100_TDP
    b200_f16 = B200_FLOPS_FP8_TENSOR
    b200_tdp = B200_TDP
...
- **Memory wall dominance**\index{Memory Wall!dominance}: For single-request LLM inference, memory bandwidth determines latency because the arithmetic completes in microseconds while data transfer takes milliseconds. HBM costs 3--5$\times$ more per GB than DDR but provides more than an order of magnitude higher bandwidth. The energy cost of moving data (20 pJ/byte) exceeds the energy cost of computing on it (1 pJ/MAC) by 20$\times$.
