...
class A100Specs:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    flops_fp16_tflops_str = fmt_flop_rate(flops_fp16_tc, unit=TFLOP / second, precision=0, commas=False)
    mem_bw_gb_s_str = fmt_bandwidth(mem_bw, unit=GB / second, precision=0, commas=False)
    mem_bw_tbs_str = fmt_bandwidth(mem_bw, unit=TB / second, precision=2, commas=False)
    tdp_w_str = fmt_power(tdp, unit=watt, precision=0, commas=False)
    nvlink_gb_s_str = fmt_bandwidth(nvlink, unit=GB / second, precision=0, commas=False)
```
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: V100NvlinkRecap.nvlink_gb_s_str
class V100NvlinkRecap:

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_gb_s_str = fmt_qty(Hardware.Cloud.V100.nvlink.bandwidth, GB/second, precision=0, commas=False)
```

The Ampere architecture (2020) expanded Tensor Core support to additional data types (TF32, BF16, INT8, INT4), reflecting the growing importance of mixed-precision training and quantized inference. The A100 also introduced Multi-Instance GPU (MIG), which partitions a single GPU into up to seven isolated instances, enabling efficient sharing of expensive hardware across multiple inference workloads. The most consequential change for infrastructure was Ampere's third-generation NVLink, which doubled bidirectional bandwidth to `{python} A100Specs.nvlink_gb_s_str` per GPU from Volta's `{python} V100NvlinkRecap.nvlink_gb_s_str`, directly addressing the communication wall for tensor parallelism.

```{python}
#| echo: false
#| label: h100-generation-specs
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: H100Specs.flops_fp16_tflops_str, H100Specs.tdp_w_str, H100Specs.nvlink_gb_s_str
from mlsysim.fmt import check, fmt_bandwidth, fmt_flop_rate, fmt_power
...
The Hopper architecture (2022) added the Transformer Engine, which dynamically selects between FP8 and FP16 precision on a per-layer basis, doubling the effective throughput for Transformer models without requiring manual precision tuning. Hopper also introduced NVLink 4.0 at `{python} H100Specs.nvlink_gb_s_str` per GPU and the NVLink Switch, enabling NVLink connectivity beyond the 8-GPU boundary.
...
# │          B200PerformanceScenario.nvlink_gb_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt_bandwidth, fmt_flop_rate, fmt_power
...
class B200PerformanceScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    fp8_tflops_str = fmt_flop_rate(fp8_flops, unit=TFLOP / second, precision=0, commas=True)
    tdp_w_str = fmt_power(tdp, unit=watt, precision=0, commas=False)
    nvlink_gb_s_str = fmt_bandwidth(nvlink, unit=GB / second, precision=0, commas=True)
```
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: B200NvlinkRecap.nvlink_gb_s_str
class B200NvlinkRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_gb_s_str = fmt_qty(Hardware.Cloud.B200.nvlink.bandwidth, GB/second, precision=0, commas=False)
```

Blackwell also introduced fifth-generation NVLink at `{python} B200NvlinkRecap.nvlink_gb_s_str` per GPU, doubling the intra-node bandwidth again. The die-to-die link within the B200 package operates at 10 TB/s, fast enough that the two dies appear as a single logical GPU to the software, with no performance penalty for operations that span the die boundary.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: V100TableRecap.flops_fp16_tflops_str, V100TableRecap.tdp_w_str, V100TableRecap.nvlink_gb_s_str
...
|:---------------------|:--------:|:------------------------|:-----------------------------------------------------:|:--------------------------------------------:|:--------------------------------------------------:|
| **Volta** (V100)     |   2017   | First Tensor Core       |    `{python} V100TableRecap.flops_fp16_tflops_str`    |     `{python} V100TableRecap.tdp_w_str`      |     `{python} V100TableRecap.nvlink_gb_s_str`      |
| **Ampere** (A100)    |   2020   | Multi-precision, MIG    |       `{python} A100Specs.flops_fp16_tflops_str`      |        `{python} A100Specs.tdp_w_str`        |        `{python} A100Specs.nvlink_gb_s_str`        |
| **Hopper** (H100)    |   2022   | Transformer Engine, FP8 |   `{python} H100PerformanceScenario.h100_tflops_str`  |        `{python} H100Specs.tdp_w_str`        |        `{python} H100Specs.nvlink_gb_s_str`        |
| **Blackwell** (B200) |   2024   | Dual-die, NVLink 5      | `{python} B200PerformanceScenario.fp8_tflops_str` FP8 | `{python} B200PerformanceScenario.tdp_w_str` | `{python} B200PerformanceScenario.nvlink_gb_s_str` |

: **NVIDIA GPU Architecture Evolution**: Each generation introduces architectural features targeted at the dominant ML workload pattern of its era. Efficiency rises substantially but unevenly across generations, while NVLink bandwidth increases from 300 GB/s on V100 to `{python} B200PerformanceScenario.nvlink_gb_s_str` on B200, with a smaller 1.5$\times$ A100-to-H100 step between the larger doubling jumps. {#tbl-gpu-evolution}
...
# │          MemoryBandwidthCacheRecap.h100_bw_tb_s_str
...
Further down is the `{python} MemoryBandwidthCacheRecap.h100_l2_bytes_str` L2 cache (~12 TB/s), and finally the `{python} MemoryBandwidthCacheRecap.h100_mem_gb_str` of HBM3 at `{python} MemoryBandwidthCacheRecap.h100_bw_tb_s_str`.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: V100MemoryWallRecap.mem_bw_gb_s_str, V100MemoryWallRecap.flops_fp16_tflops_str
class V100MemoryWallRecap:
    h_v100 = Hardware.Cloud.V100
    mem_bw_gb_s_str = fmt_qty(h_v100.memory.bandwidth, GB/second, precision=0, commas=False)
    flops_fp16_tflops_str = fmt_qty(h_v100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthPitfallRecap.h100_bw_tb_s_str, MemoryBandwidthPitfallRecap.h100_flops_fp16_tflops_str
class MemoryBandwidthPitfallRecap:

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_bw_tb_s_str = fmt_qty(Hardware.Cloud.H100.memory.bandwidth, TB/second, precision=2, commas=False)
    h100_flops_fp16_tflops_str = fmt_qty(Hardware.Cloud.H100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
```

3.  **Common pitfall**: A frequent misconception is that HBM solves the memory wall. HBM moves the wall rather than eliminates it: Tensor Core throughput $(R_{\text{peak}})$ has scaled faster than HBM bandwidth across generations (from `{python} V100MemoryWallRecap.mem_bw_gb_s_str` with `{python} V100MemoryWallRecap.flops_fp16_tflops_str` FP16 in V100 to `{python} MemoryBandwidthPitfallRecap.h100_bw_tb_s_str` with `{python} MemoryBandwidthPitfallRecap.h100_flops_fp16_tflops_str` FP16 in H100), so the ridge point continues rising and more operations remain memory-bound despite HBM improvements.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthDdrRecap.ddr_bw_gb_s_str
class MemoryBandwidthDdrRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ddr_bw_gb_s_str = fmt_qty(Hardware.Tech.Storage.HostDram.bandwidth, GB/second, precision=0, commas=False)
```

Traditional DDR memory connects to the processor through pins on the edge of a printed circuit board (PCB). Each DIMM communicates over a 64-bit bus, and even with multiple channels (8 channels is typical for a high-end server CPU), a modern server tops out at roughly `{python} MemoryBandwidthDdrRecap.ddr_bw_gb_s_str` of aggregate memory bandwidth.
...
| **Bandwidth**\index{HBM!bandwidth}      | ~`{python} MemoryBandwidthDdrRecap.ddr_bw_gb_s_str` | **~`{python} MemoryBandwidthPitfallRecap.h100_bw_tb_s_str`** |    **~17$\times$ Faster**    |
...
class A100HbmQuantRecap:
    ...
    mem_bw_tbs_str = fmt_bandwidth(a100_bw, unit=TB / second, precision=2, commas=False)
    a100_token_ms_str = fmt_time(a100_token, millisecond, precision=1, commas=False)
    h100_bw_tbs_str = fmt_bandwidth(h100_bw, unit=TB / second, precision=2, commas=False)
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthHbmRecap.h100_bw_tb_s_str
...
class TokenLatencyScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt_flop_rate(h_h100.compute.peak_flops, unit=TFLOP / second, precision=0, commas=False)
...
1. **Compute Time**\index{Latency!compute time}: Each token requires $2 \times 70 \times 10^9 = 1.4 \times 10^{11}$ floating-point operations. At `{python} TokenLatencyScenario.h100_tflops_str` of low-precision Tensor Core peak throughput, the arithmetic takes `{python} TokenLatencyScenario.t_comp_math`.
```{python}
#| echo: false
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthLatencyRecap.h100_bw_tb_s_str
...
Loading `{python} TokenLatencyScenario.weight_gb_str` of weights from HBM at `{python} MemoryBandwidthLatencyRecap.h100_bw_tb_s_str` takes `{python} TokenLatencyScenario.t_mem_math`.
...
class H200Comparison:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    bw_gain_str = fmt_multiple((h200.memory.bandwidth/h_h100.memory.bandwidth).to_base_units().magnitude, precision=1, commas=True)
    cap_gain_str = fmt_multiple((h200.memory.capacity/h_h100.memory.capacity).to_base_units().magnitude, precision=1, commas=True)

    itl_h100 = res_h100.itl
    itl_h200 = res_h200.itl
    speedup = (itl_h100 / itl_h200).to_base_units().magnitude
    speedup_str = fmt_multiple(speedup, precision=1, commas=True)
```
...
To understand *why* the "memory wall" is the primary constraint for modern large language models (LLMs)\index{LLM!inference, memory bottleneck}, we compare the NVIDIA H100 against its successor, the H200\index{NVIDIA!H200, memory wall}. While both chips share the same compute cores (same peak TFLOP/s), the H200 provides `{python} H200Comparison.bw_gain_str`$\times$ higher memory bandwidth and `{python} H200Comparison.cap_gain_str`$\times$ more HBM capacity.

**Systems insight**: For LLM decoding, the H200 is `{python} H200Comparison.speedup_str`$\times$ faster than the H100 despite having identical compute power. This proves that for large-scale autoregressive models, the "Wall" is the memory interface, not the arithmetic logic.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthRooflineFnRecap.h100_bw_tb_s_str, MemoryBandwidthRooflineFnRecap.h100_flops_fp16_tflops_str
...
class RooflineLandscapeMath:
    ...
    train_gap_str = fmt_multiple(train_gap, precision=0, commas=True)
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: RooflineCaptionRecap.h100_tflops_str, RooflineCaptionRecap.h100_bw_tb_s_str, RooflineCaptionRecap.h100_ridge_str
class RooflineCaptionRecap:
    h_h100 = Hardware.Cloud.H100

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt_qty(h_h100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
    h100_bw_tb_s_str = fmt_qty(h_h100.memory.bandwidth, TB/second, precision=2, commas=False)
...
The roofline in @fig-roofline-landscape is defined by two ceilings: memory bandwidth (diagonal) and peak compute (horizontal), meeting at the FP16 ridge point (~`{python} RooflineCaptionRecap.h100_ridge_str` FLOP/byte for `{python} RooflineCaptionRecap.h100_tflops_str` over `{python} RooflineCaptionRecap.h100_bw_tb_s_str`). ML workloads span the full range: LLM decode at batch size 1 is deeply memory-bound, while LLM training at large batch sizes is compute-bound. The position of each workload determines whether faster memory or faster compute would improve performance.
...
class RooflineInferenceMath:
    ...
    h100_bw_tb_s_str = fmt_bandwidth(h100_bw, unit=TB / second, precision=2, commas=False)
    h100_tflops_str = fmt_flop_rate(h100_peak, unit=TFLOP / second, precision=0, commas=False)
...
class RooflineCalloutRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt_qty(Hardware.Cloud.H100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
    h100_ridge_str = fmt_arithmetic_intensity((Hardware.Cloud.H100.compute.peak_flops/Hardware.Cloud.H100.memory.bandwidth), unit=flop/byte, precision=1, commas=False)
```

Consider our `{python} InfraFrontierMemoryMxRecap.frontier_params_b_str` model on an H100 with `{python} RooflineInferenceMath.h100_tflops_str` peak compute and `{python} RooflineInferenceMath.h100_bw_tb_s_str` memory bandwidth. The ridge point is `{python} RooflineCalloutRecap.h100_ridge_str` FLOP/byte.
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: TensorCoreTdpRecap.h100_fp16_tc_tflops_str, TensorCoreTdpRecap.h100_fp8_tc_tflops_str, TensorCoreTdpRecap.tc_vs_cuda_ratio_str, TensorCoreTdpRecap.h100_fp32_cuda_tflops_str
class TensorCoreTdpRecap:

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp16_tc_tflops_str = fmt_flop_rate(Hardware.Cloud.H100.compute.peak_flops, unit=TFLOPs/second, precision=0, commas=False)
    h100_fp8_tc_tflops_str = fmt_flop_rate(Hardware.Cloud.H100.compute.precision_flops['fp8'], unit=TFLOPs/second, precision=0, commas=True)
    tc_vs_cuda_ratio_str = fmt_multiple(64.0, precision=0, commas=False)
    h100_fp32_cuda_tflops_str = fmt_flop_rate(Hardware.Cloud.H100.compute.peak_flops / 64, unit=TFLOPs/second, precision=1, commas=False)
```

1.  **Significance (quantitative)**: Tensor Cores provide the bulk of the H100's `{python} TensorCoreTdpRecap.h100_fp16_tc_tflops_str` FP16/BF16 peak (and ~`{python} TensorCoreTdpRecap.h100_fp8_tc_tflops_str` at FP8)—roughly `{python} TensorCoreTdpRecap.tc_vs_cuda_ratio_str`$\times$ the ~`{python} TensorCoreTdpRecap.h100_fp32_cuda_tflops_str` delivered by CUDA (vector) cores alone. However, this peak is only achievable for operations that decompose into tiled matrix multiplications. Layer normalization and softmax, which involve reductions and element-wise operations rather than matrix multiplications, run at approximately 3–8 percent of peak throughput on the same hardware.
...
The practical implication for fleet design is that peak TFLOP/s specifications are precision-dependent. The H100 delivers `{python} TensorCoreTdpRecap.h100_fp16_tc_tflops_str` in FP16 but twice that (`{python} TensorCoreTdpRecap.h100_fp8_tc_tflops_str`) in FP8. A fleet designed for FP8 training effectively has twice the compute density of the same fleet running FP16, with no additional hardware. This makes precision engineering a first-class optimization lever for infrastructure planners, not just a model accuracy concern.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthPipelineRecap.h100_bw_tb_s_str
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthTensorRecap.h100_bw_tb_s_str
...
Memory stalls occur when the Tensor Cores complete their current tile multiplication before the next tile has been loaded from HBM. Even with the H100's `{python} MemoryBandwidthTensorRecap.h100_bw_tb_s_str` HBM bandwidth, the Tensor Cores can consume data faster than HBM can deliver it for certain matrix dimensions, creating idle cycles while the arithmetic units wait for data.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: H100TdpWallRecap.tdp_w_str, H100TdpWallRecap.flops_fp16_tflops_str
class H100TdpWallRecap:

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_w_str = fmt_qty(Hardware.Cloud.H100.tdp, watt, precision=0, commas=False)
    flops_fp16_tflops_str = fmt_qty(Hardware.Cloud.H100.compute.peak_flops, TFLOP/second, precision=0, commas=False)
```
...
1.  **Significance (quantitative)**: The H100 SXM5 operates at `{python} H100TdpWallRecap.tdp_w_str` TDP. When a liquid cooling system can only sustain 500 W of heat removal (inadequate cooling), the GPU firmware reduces clock frequency via thermal throttling, dropping $R_{\text{peak}}$ from `{python} H100TdpWallRecap.flops_fp16_tflops_str` to roughly 712 TFLOP/s FP16, a 28 percent reduction in training throughput that propagates directly into longer wall-clock training time and higher cost per model.
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: GpuEfficiencyTrajectoryRecap.v100_ef_tflops_per_w_str, GpuEfficiencyTrajectoryRecap.a100_ef_tflops_per_w_str, GpuEfficiencyTrajectoryRecap.h100_ef_tflops_per_w_str, GpuEfficiencyTrajectoryRecap.b200_ef_tflops_per_w_str
class GpuEfficiencyTrajectoryRecap:

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_ef_tflops_per_w_str = fmt_compute_efficiency(Hardware.Cloud.V100.compute.peak_flops / Hardware.Cloud.V100.tdp, unit=TFLOPs / second / watt, precision=2, commas=False)
    a100_ef_tflops_per_w_str = fmt_compute_efficiency(Hardware.Cloud.A100.compute.peak_flops / Hardware.Cloud.A100.tdp, unit=TFLOPs / second / watt, precision=2, commas=False)
    h100_ef_tflops_per_w_str = fmt_compute_efficiency(Hardware.Cloud.H100.compute.precision_flops["fp8"] / Hardware.Cloud.H100.tdp, unit=TFLOPs / second / watt, precision=2, commas=False)
    b200_ef_tflops_per_w_str = fmt_compute_efficiency(Hardware.Cloud.B200.compute.precision_flops["fp8"] / Hardware.Cloud.B200.tdp, unit=TFLOPs / second / watt, precision=2, commas=False)
```

The power efficiency trajectory provides a partial counterweight to rising TDP. While absolute power has increased, the advertised low-precision TFLOP/s delivered per watt has improved dramatically: from `{python} GpuEfficiencyTrajectoryRecap.v100_ef_tflops_per_w_str` per watt (V100 FP16) to `{python} GpuEfficiencyTrajectoryRecap.h100_ef_tflops_per_w_str` per watt (H100 FP8) to `{python} GpuEfficiencyTrajectoryRecap.b200_ef_tflops_per_w_str` per watt (B200 FP8). These are not apples-to-apples FP16 numbers; the precision mode is part of the hardware trend. For a fixed compute budget that can use the newer precision modes, each generation requires fewer chips and therefore less total power. Frontier models, however, grow faster than efficiency improves, so the absolute power demand of training the latest model increases with every generation. This is the treadmill of infrastructure: efficiency gains buy time, but scale growth consumes it.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: RooflineTdpRecap.h100_bw_tb_s_str
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: TensorCorePrecisionRecap.h100_fp16_tc_tflops_str, TensorCorePrecisionRecap.h100_fp8_tc_tflops_str
...
| **BF16**\index{BF16} |    16    |      8       |      7       | `{python} TensorCorePrecisionRecap.h100_fp16_tc_tflops_str` | Training, fine-tuning            |
| **FP16**\index{FP16} |    16    |      5       |      10      | `{python} TensorCorePrecisionRecap.h100_fp16_tc_tflops_str` | Training with loss scaling       |
| **FP8 (E4M3)**       |    8     |      4       |      3       |  `{python} TensorCorePrecisionRecap.h100_fp8_tc_tflops_str` | Forward pass, weights            |
| **FP8 (E5M2)**       |    8     |      5       |      2       |  `{python} TensorCorePrecisionRecap.h100_fp8_tc_tflops_str` | Backward pass, gradients         |
| **INT8**\index{INT8} |    8     |     N/A      |     N/A      |  `{python} TensorCorePrecisionRecap.h100_fp8_tc_tflops_str` | Post-training quantization       |
...
The infrastructure implications are substantial. A fleet designed for BF16 training (the current standard for most large models) delivers `{python} TensorCorePrecisionRecap.h100_fp16_tc_tflops_str` per GPU on an H100. The same fleet running FP8 training delivers `{python} TensorCorePrecisionRecap.h100_fp8_tc_tflops_str` per GPU, effectively doubling the compute density without additional hardware. For a 10,000-GPU cluster priced at roughly \$350,000 per 8-GPU node, the difference between BF16 and FP8 training is equivalent to adding another 10,000 GPUs, or roughly \$440 million in node hardware.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: H100NvlinkNodeRecap.nvlink_gb_s_str
class H100NvlinkNodeRecap:

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_gb_s_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth, GB/second, precision=0, commas=False)
```
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: BandwidthHierarchyTableRecap.nvlink_bw_gb_s_str, BandwidthHierarchyTableRecap.pcie_bw_gb_s_str, BandwidthHierarchyTableRecap.ib_bw_gb_s_str
class BandwidthHierarchyTableRecap:

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_bw_gb_s_str = fmt_qty(Hardware.Cloud.H100.nvlink.bandwidth, GB/second, precision=0, commas=False)
    pcie_bw_gb_s_str = fmt_qty(Hardware.Cloud.H100.interconnect.bandwidth, GB/second, precision=0, commas=False)
    ib_bw_gb_s_str = fmt_qty(Systems.Fabrics.InfiniBand_NDR.bandwidth, GB/second, precision=0, commas=False)
```
...
| **Intra-Node**      | NVLink/ICI         | **~`{python} BandwidthHierarchyTableRecap.nvlink_bw_gb_s_str`** |           ~1 μs            | Node (8--16 Chips) |
| **Intra-Node (IO)** | PCIe Gen5 x16      |  **~`{python} BandwidthHierarchyTableRecap.pcie_bw_gb_s_str`**  |           ~2 μs            | CPU-GPU, NIC-GPU   |
| **Inter-Node**      | InfiniBand NDR     |   **~`{python} BandwidthHierarchyTableRecap.ib_bw_gb_s_str`**   |         ~5--10 μs          | Pod (Thousands)    |
...
# │ Exports: bs_hbm_bw_gb_s_str, bs_nvlink_bw_gb_s_str, bs_ib_bw_gb_s_str, bs_gap_nv_ib_str
# └─────────────────────────────────────────────────────────────────────────────
...
class BandwidthStaircase:
    ...
    bs_hbm_bw_gb_s_str = fmt_bandwidth(hbm_bw, unit=GB / second, precision=0, commas=True)
    bs_nvlink_bw_gb_s_str = fmt_bandwidth(nvlink_bw, unit=GB / second, precision=0, commas=False)
    bs_ib_bw_gb_s_str = fmt_bandwidth(ib_bw, unit=GB / second, precision=0, commas=False)
    bs_gap_nv_ib_str = fmt_multiple(gap_nv_ib, precision=0, commas=False)
```
...
1. **HBM (Intra-Chip)**: `{python} BandwidthStaircase.transfer_gb_str` / `{python} BandwidthStaircase.bs_hbm_bw_gb_s_str` $\approx$ `{python} BandwidthStaircase.hbm_ms_str`.
2. **NVLink (Intra-Node)**: `{python} BandwidthStaircase.transfer_gb_str` / `{python} BandwidthStaircase.bs_nvlink_bw_gb_s_str` $\approx$ `{python} BandwidthStaircase.nvlink_ms_str`.
3. **InfiniBand (Inter-Node)**: `{python} BandwidthStaircase.transfer_gb_str` / `{python} BandwidthStaircase.bs_ib_bw_gb_s_str` $\approx$ `{python} BandwidthStaircase.ib_ms_str`.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: BandwidthHierarchyPcieRecap.pcie_bw_gb_s_str
...
While NVLink provides a high-bandwidth freeway for GPU-to-GPU communication, **PCIe Gen5**\index{PCIe Gen5!node hierarchy} serves as the universal glue connecting the heterogeneous components of the node. It links GPUs to the host CPU, GPUs to InfiniBand Host Channel Adapters (HCAs), the host CPU to NVMe storage, and the host CPU to system DRAM. Understanding this topology is critical because PCIe bandwidth -- though substantial at ~`{python} BandwidthHierarchyPcieRecap.pcie_bw_gb_s_str` per direction (128 GB/s bidirectional) per x16 link -- is often the bottleneck for operations that cross the accelerator boundary, such as data loading, checkpointing, and inter-node gradient synchronization.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: BandwidthHierarchyNvlinkRecap.nvlink_bw_gb_s_str
...
Consider the NVIDIA DGX H100 architecture. Eight H100 GPUs sit on a single baseboard, each connected to four NVSwitch chips via 18 NVLink 4.0 lanes. The NVSwitch chips form a non-blocking crossbar[^fn-crossbar-nvswitch]: any GPU can communicate with any other GPU at the full `{python} BandwidthHierarchyNvlinkRecap.nvlink_bw_gb_s_str` bidirectional bandwidth simultaneously, without contention. This is the equivalent of giving every pair of GPUs their own private highway, rather than forcing them to share a single road.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: NodeCapacityDenseRecap.node_hbm_cap_gb_str, NodeCapacityDenseRecap.h100_tflops_str
class NodeCapacityDenseRecap:
    ...
    h100_tflops_str = fmt_qty(Hardware.Cloud.H100.compute.precision_flops['fp8'], TFLOP/second, precision=0, commas=False)
```

The result is that the 8 GPUs within a DGX H100 can be treated as a single logical device with `{python} NodeCapacityDenseRecap.node_hbm_cap_gb_str` of aggregate HBM and a combined compute throughput of 8 $\times$ `{python} NodeCapacityDenseRecap.h100_tflops_str`. This abstraction is critical for software: the training framework can partition a model across the 8 GPUs using tensor parallelism as if they were a single larger processor, with the NVSwitch fabric making the partitioning nearly transparent from a performance perspective.
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: BandwidthHierarchyPcieExampleRecap.pcie_bw_gb_s_str
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: H100NvlinkGaudiRecap.nvlink_gb_s_str
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, fmt_percent, check, fmt_time
...
|:---------------------------------------|:-------------:|:-----------------------------------------------------------:|:--------------------------------------------------------------------------------:|
| **Model Weights**\index{Model Weights} |      FP16     | `{python} InfraFrontierNodeTableRecap.bytes_fp16_str` bytes |              `{python} InfraFrontierNodeTableRecap.weights_gb_str`               |
| **Gradients**                          |      FP16     | `{python} InfraFrontierNodeTableRecap.bytes_fp16_str` bytes |             `{python} InfraFrontierNodeTableRecap.gradients_gb_str`              |
| **Optimizer (Adam $m$)**               |      FP32     | `{python} InfraFrontierNodeTableRecap.bytes_fp32_str` bytes |            `{python} InfraFrontierNodeTableRecap.optimizer_m_gb_str`             |
| **Optimizer (Adam $v$)**               |      FP32     | `{python} InfraFrontierNodeTableRecap.bytes_fp32_str` bytes |            `{python} InfraFrontierNodeTableRecap.optimizer_v_gb_str`             |
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: BandwidthHierarchyPcieOffloadRecap.pcie_bw_gb_s_str
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthNodeTableRecap.h100_bw_tb_s_str
...
|:----------------|:-------------------------:|:-----------------------------------------------------------------:|:-----------:|:----------------------|
| **GPU HBM3**    | 80 GB $\times$ 8 = 640 GB | `{python} MemoryBandwidthNodeTableRecap.h100_bw_tb_s_str` per GPU |    <1 μs    | Active computation    |
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: BandwidthHierarchyNvlinkPodRecap.nvlink_bw_gb_s_str
...
1. **Intra-Node (NVLink)**: Bandwidth = `{python} BandwidthHierarchyNvlinkPodRecap.nvlink_bw_gb_s_str`.
   `{python} InterconnectCommMath.t_intra_math`
```{python}
#| echo: false
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: BandwidthHierarchyPodRecap.ib_bw_gb_s_str
...
2. **Inter-Node (InfiniBand NDR)**: Bandwidth = `{python} BandwidthHierarchyPodRecap.ib_bw_gb_s_str`.
...
# │          H100TdpRackRecap.power_rate_gws_s_str
class H100TdpRackRecap:
    ...
    power_rate_gws_s_str = fmt_qty(power_rate, GW / second, precision=2, commas=False)
...
class PodTrainingMath:
    ...
    h100_tflops_math_str = fmt_flop_rate(h100_fp8_flops, unit=TFLOP / second, precision=0, commas=False)
    h100_tflops_str = fmt_flop_rate(h100_fp8_flops, unit=TFLOP / second, precision=0, commas=False)
    ...
    cluster_throughput_math = fmt_math(
        f"{n_gpus_fmt} \\times {h100_tflops_math_str} \\times 10^{{12}} \\times {mfu}"
    )
    cluster_throughput = n_gpus * h100_fp8_flops * mfu
    cluster_eflops_str = fmt_flop_rate(cluster_throughput, unit=EFLOP / second, precision=2, commas=False)
...
class WscThroughput:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    wsc_fp16_eflops_str = fmt_flop_rate(wsc_fp16, unit=EFLOP / second, precision=2, commas=False)
    wsc_fp8_eflops_str = fmt_flop_rate(wsc_fp8, unit=EFLOP / second, precision=1, commas=False)
    wsc_inject_tbs_str = fmt_bandwidth(wsc_inject, unit=TB / second, precision=0, commas=False)
    wsc_oversub_tbs_str = fmt_bandwidth(wsc_oversub, unit=TB / second, precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, fmt_bandwidth, fmt_memory, fmt_percent, fmt_time
...
class CheckpointOverheadBudget:
    ...
    ckpt_required_bw_gbs_str           = fmt_bandwidth(required_bw, unit=GB / second, precision=1, commas=False)
    ckpt_reference_write_s_str         = fmt_time(reference_write, second, precision=0, commas=False, style='word')
    ckpt_reference_bw_gbs_str          = fmt_bandwidth(reference_bw, unit=GB / second, precision=1, commas=False)
    ...
    ckpt_sustained_bw_gbs_str          = fmt_bandwidth(sustained_bw, unit=GB / second, precision=1, commas=False)
    ckpt_run_days_str                  = fmt_time(run_days, 'day', precision=0, commas=False, style='word')
    ckpt_run_hours_per_day_str         = fmt_time(HOURS_PER_DAY, 'hour', precision=0, commas=False, style='word')
    ckpt_per_hour_str                  = fmt(60 // cadence_min_value, precision=0, commas=False)
```
...
class CheckpointComputeTradeoff:
    ...
    write_bw_gbs_str = fmt_bandwidth(write_bw, unit=GB / second, precision=0, commas=False)
...
from mlsysim.fmt import fmt_qty
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthFallaciesRecap.h100_bw_tb_s_str
...
class A100FallaciesRecap:
    ...
    speedup_str = fmt_multiple(speedup, precision=1, commas=False)
```
...
class A100UniformFleetRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mem_bw_tbs_str = fmt_qty(Hardware.Cloud.A100.memory.bandwidth, TB/second, precision=2, commas=False)
```
...
# │ Exports: SummaryScaleScenario.gpt3_params_b_str, h100_tflops_str, h100_bw_tb_s_str, rack_power_kw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
class SummaryScaleScenario:
    ...
    h100_tflops_str = fmt_flop_rate(flops, unit=TFLOP / second, precision=0, commas=False)
    h100_bw_tb_s_str = fmt_bandwidth(bw, unit=TB / second, precision=2, commas=False)
