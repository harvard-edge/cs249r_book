...
1.  **Significance (quantitative)**: The H100 SXM5 operates at 700 W TDP. When a liquid cooling system can only sustain 500 W of heat removal (inadequate cooling), the GPU firmware reduces clock frequency via thermal throttling—dropping $R_{\text{peak}}$ from 989 to roughly 712 TFLOPS FP16, a 28 percent reduction in training throughput that propagates directly into longer wall-clock training time and higher cost per model.
...
| **BF16**\index{BF16} |    16    |      8       |      7       |          989          | Training, fine-tuning            |
| **FP16**\index{FP16} |    16    |      5       |      10      |          989          | Training with loss scaling       |
| **FP8 (E4M3)**       |    8     |      4       |      3       |         1,979         | Forward pass, weights            |
| **FP8 (E5M2)**       |    8     |      5       |      2       |         1,979         | Backward pass, gradients         |
| **INT8**\index{INT8} |    8     |     N/A      |     N/A      |         1,979         | Post-training quantization       |
...
class WscThroughput:
    ...
    wsc_fp16_eflops = n_gpus * h100_fp16_tflops / 1e6     # TFLOPS → EFLOPS
    wsc_fp8_eflops = n_gpus * h100_fp8_tflops / 1e6
    wsc_inject_tbs = n_gpus * per_gpu_inject_gbs / 1000   # GB/s → TB/s
...
|:----------------|---------------------------------:|------------:|:--------------------------------------------:|:-----------------------:|
| **V100 (2017)** |                       125 (FP16) |         300 | `{python} GpuEfficiencyScenario.v100_ef_str` |       1.0$\times$       |
| **A100 (2020)** |                       312 (FP16) |         400 | `{python} GpuEfficiencyScenario.a100_ef_str` |       1.9$\times$       |
| **H100 (2022)** |                       1979 (FP8) |         700 | `{python} GpuEfficiencyScenario.h100_ef_str` |       6.8$\times$       |
