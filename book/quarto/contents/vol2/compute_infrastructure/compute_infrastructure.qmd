...
Gaudi takes a different route by integrating RDMA-capable Ethernet into the accelerator and using a more network-centric design for communication. The Gaudi 2 profile exposes `{python} AlternativeNodeArchitectureRecap.gaudi2_roce_gb_per_s_str` of aggregate RoCE bandwidth through 24 attached 100 GbE ports, while the newer Gaudi 3-class accelerator still exposes `{python} AlternativeNodeArchitectureRecap.gaudi3_hbm_gb_str` of HBM. The question is whether simplifying the fabric and using Ethernet consistently across node and pod is worth giving up the very high local exchange bandwidth of a switch-rich NVLink design. That trade can be attractive when data parallelism, inference serving, or cluster cost dominates, and less attractive when tensor-parallel collectives sit inside every layer.
