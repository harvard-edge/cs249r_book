...
The second principle is that **failure is routine, not exceptional**. A cluster of 10,000 GPUs, each with a mean time between failures (MTBF) of approximately 50,000 hours (the canonical `GPU_MTTF_HOURS` figure used in @sec-fault-tolerance), will experience a GPU failure roughly once every five hours on average.
