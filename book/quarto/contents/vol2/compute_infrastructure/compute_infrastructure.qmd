...
## Introduction: The Physics of Scale {#sec-compute-introduction}
...
| **Feature** | **CPU** | **GPU** (H100) | **TPU** (v5p) | **Custom ASIC** |
|:---|:---:|:---:|:---:|:---:|
| **Arithmetic Core** | Scalar/Vector | Tensor Core | Systolic Array | Fixed Dataflow |
| **Execution** | Instruction | SIMT | Data-Driven | Hardwired |
| **Memory Control** | Cache Hierarchy | L1/L2 + HBM | Managed Scratchpad | Explicit Mesh |
| **Flexibility** | **Extreme** | High (CUDA) | Moderate (XLA) | Low |
| **Efficiency** | Low | High | Very High | **Extreme** |

: **The Accelerator Spectrum**. As we move from left to right, we trade general-purpose programmability for compute density and power efficiency. The optimal choice depends on the workload's regularity, expected lifetime, and scale of deployment. {#tbl-accelerator-comparison}
...
::: {#nb-token-latency .callout-notebook title="Napkin Math: The Physics of Token Latency"}
...
**Arithmetic Intensity** ($I$) is the ratio of floating-point operations performed to the number of bytes transferred from memory:
...
The **Ridge Point** is the arithmetic intensity at which the memory bandwidth ceiling meets the compute ceiling:
...
**Model FLOPS Utilization (MFU)** is the ratio of the floating-point operations actually performed on useful model computation to the hardware's theoretical peak throughput:
...
::: {.callout-notebook title="Napkin Math: The Energy Cost of Data Movement"}
...
A **Node** is a single physical server chassis containing multiple accelerators (typically 8), connected by a high-speed intra-node interconnect, and backed by substantial host memory. The node is the fundamental failure domain (if one component fails, the entire node is typically lost) and the basic scheduling unit for fleet orchestrators.
...
::: {#nb-comm-overhead .callout-notebook title="Napkin Math: The Cost of Crossing the Cliff"}
...
**Power Usage Effectiveness (PUE)** is the ratio of total facility power consumption to the power consumed by IT equipment:
...
::: {.callout-notebook title="Napkin Math: The Cooling Tax"}
...
::: {.callout-notebook title="Napkin Math: Training Time for 175B"}
...
::: {.callout-notebook title="Napkin Math: Scaling Efficiency for a 175B Model"}
...
::: {.callout-notebook title="Napkin Math: The 10,000-GPU Cluster"}
