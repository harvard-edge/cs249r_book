...
# │       ~640 GB HBM; cloud ~$280k/yr vs on-prem ~$166k/yr at 80% utilization.
...
# │          t_mem_str, t_comp_str, dominance_str, util_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
class TokenLatency:
    ...
    t_mem_ms_val = (weight_vol / hbm_bw).to('millisecond').magnitude
    t_comp_ms_val = (ops_per_token / peak_tflops).to('millisecond').magnitude

    mem_dominance_val = (t_mem_ms_val / (t_mem_ms_val + t_comp_ms_val)) * 100

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(mem_dominance_val > 90, f"Decode should be memory-dominant (>90%), got {mem_dominance_val:.1f}%")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    t_mem_str = f"{t_mem_ms_val:.1f}"
    t_comp_str = f"{t_comp_ms_val:.2f}"
    dominance_str = f"{mem_dominance_val:.1f}"

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
t_mem_str = TokenLatency.t_mem_str
t_comp_str = TokenLatency.t_comp_str
dominance_str = TokenLatency.dominance_str
...
```{python}
#| label: gpt3-scale-scenario
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ GPT-3 INFRASTRUCTURE SCALE (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-compute-introduction
# │
