...
class GpuEfficiencyTrajectoryRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_ef_str = fmt_qty((Hardware.Cloud.V100.compute.peak_flops.m_as(TFLOPs/second)/Hardware.Cloud.V100.tdp.m_as(watt)) * TFLOP / second, TFLOP / second, precision=2, commas=False)
    a100_ef_str = fmt_qty((Hardware.Cloud.A100.compute.peak_flops.m_as(TFLOPs/second)/Hardware.Cloud.A100.tdp.m_as(watt)) * TFLOP / second, TFLOP / second, precision=2, commas=False)
    h100_ef_str = fmt_qty((Hardware.Cloud.H100.compute.precision_flops['fp8'].m_as(TFLOPs/second)/Hardware.Cloud.H100.tdp.m_as(watt)) * TFLOP / second, TFLOP / second, precision=2, commas=False)
    b200_ef_str = fmt_qty((Hardware.Cloud.B200.compute.precision_flops['fp8'].m_as(TFLOPs/second)/Hardware.Cloud.B200.tdp.m_as(watt)) * TFLOP / second, TFLOP / second, precision=2, commas=False)
```
...
class TcoSummaryBreakevenRecap:
    cloud_h100_hourly = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as("dollar / hour")
    gpus_per_node = Systems.Nodes.DGX_H100.accelerators_per_node
    dgx_h100_price = Infrastructure.Pricing.Capital.DgxH100NodeUsd.rate.m_as("dollar")
    amortization_years = 3
    facility_pue = Infrastructure.FacilityCooling.StateOfArt.pue
    onprem_elec = Infrastructure.Pricing.OnPremises.ElectricityPerKwh.rate.m_as("dollar / kilowatt_hour")
