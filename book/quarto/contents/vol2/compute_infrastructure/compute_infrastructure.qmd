...
Machine learning systems have a physical reality that transcends code. While the algorithms for training large models may be public, the ability to execute them at scale is gated by the physics of infrastructure. Every training run is a race against thermal limits, power delivery stability, and the crushing cost of data movement. When algorithms are the primary bottleneck, progress can happen anywhere. At large scale, the constraint shifts toward who can build and operate the physical systems that make scale possible. Building an ML fleet requires engineering across four physical levels: the **Accelerator**\index{Accelerator!fleet stack} (silicon physics), the **Node**\index{Node!fleet stack} (interconnect density), the **Rack**\index{Rack!fleet stack} (thermal management), and the **Pod**\index{Pod!fleet stack} (warehouse-scale networking). At each level, the goal is the same: to minimize the time data spends in transit and maximize the time it spends in computation. The economics are equally unforgiving: a large GPU cluster represents staggering capital expenditure and megawatts of continuous power. Infrastructure becomes a participation boundary for organizations working at the largest scales. This chapter maps that physical stack, from the stacking of memory dies to the stabilization of grid-scale power ramps. The infrastructure is the physical substrate that defines the absolute speed-of-light limits for all Compute and Communication in the Fleet.
...
[^fn-systolic-array-tpu]: **Systolic Array**\index{Systolic Array!etymology}: From Greek *systole* (contraction) -- coined by H. T. Kung and Leiserson in 1978 because data pulses through a grid of processing elements in a heart-like rhythm. The metaphor explains the design: each cell performs one MAC and passes the result to its neighbor, eliminating register-file round-trips. Google's TPU v1 deployed a $256{\times}256$ array (65,536 MACs), achieving 92 TOPS within 75 W by hardwiring this dataflow -- the architectural bet that made data center-scale inference economically viable.

::: {#exmp-compute-infrastructure-tpu-origin-story .callout-example title="The TPU origin story"}

**Context**: In 2013, Google engineers projected that if users spoke to their Android phones for just three minutes per day using voice search, the company would need to double its data center compute capacity to handle the inference load. The cost was prohibitive.

**Observation**: Profiling showed that over 90 percent of inference cycles were spent on matrix multiplications in neural network models. This observation led directly to the TPU v1, a purpose-built inference accelerator with a $256{\times}256$ systolic array that could deliver 92 TOPS (INT8) within a 75 W power envelope. The first TPUs were deployed in 2015 and served production inference workloads including RankBrain, portions of Google Neural Machine Translation, and AlphaGo match play [@silver2017; @jouppi2017].
...
First, the maximum die size is capped by the photolithographic reticle limit (the 858 mm$^2$ single-exposure ceiling examined in @sec-compute-infrastructure-evolution-gpu-architectures-566b), which limits the number of Tensor Cores, SMs, and HBM stacks a monolithic GPU can integrate. Chiplet designs bypass this limit by placing multiple dies on a single package, with the B200's dual-die design effectively creating a 1,600 mm$^2$ equivalent processor.
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.compute.precision_flops["fp8"], TFLOPs, second)
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.B200.compute.precision_flops["fp8"], Hardware.Cloud.B200.tdp,
...
The planning implication is that a data center built for accelerators must anticipate refresh cycles before the first rack arrives. Hardware refresh planning is therefore an integral part of the initial procurement decision, not an afterthought.
...
The accelerator's arithmetic engine is extraordinarily powerful, capable of nearly 2,000 TFLOP/s on the H100. Yet raw arithmetic throughput is meaningless if the data cannot reach the compute units fast enough. The fundamental bottleneck that limits every accelerator's effective performance is the memory wall.
...
A single HBM stack bonds 8--12 DRAM dies vertically, threading thousands of TSVs through each die to form a 1024-bit-wide interface per stack, compared to 64 bits for a DDR5 channel. This 16$\times$ wider interface, combined with higher per-pin signaling rates, is the source of HBM's bandwidth advantage. Because the vias travel *through* the silicon rather than *across* a PCB, the signal path shrinks from centimeters to tens of micrometers, roughly a 1000$\times$ reduction, and the whole stack sits on the same silicon interposer as the processor die so data travels from DRAM cell to arithmetic unit in nanoseconds rather than the tens of nanoseconds required by off-package DDR.

The shortened distance has three direct physical benefits. First, the energy per bit transferred\index{HBM!energy efficiency} drops by an order of magnitude (from approximately 20 pJ/bit for DDR5 to approximately 2 pJ/bit for HBM) because shorter traces have lower capacitance and require less driving current. Second, the latency decreases because electrical signals propagate through silicon at roughly two thirds the speed of light, and the shorter path means proportionally shorter propagation time. Third, the reduced capacitance allows higher signaling frequencies without the sophisticated equalization circuits required by long PCB traces, enabling the high per-pin data rates that complement the wide bus width.

[^fn-tsv-stacking]: **TSV (Through-Silicon Via)**\index{TSV!3D stacking}: A vertical copper pillar, 5--10 micrometers in diameter, etched through a silicon die to connect stacked layers. Originally developed for CMOS image sensors in smartphone cameras, TSVs enabled HBM by replacing centimeters of PCB trace with tens of micrometers of vertical silicon -- a 1,000$\times$ reduction in signal path that drops energy per bit from ~20 pJ (DDR5) to ~2 pJ (HBM), making terabyte-per-second bandwidth economically feasible within an accelerator's power budget.
...
- [ ] **HBM vs. DDR5**: Why does HBM achieve higher bandwidth than DDR5 despite having a lower clock frequency?
- [ ] **Generality tax**: What is the "generality tax" of DDR memory, and how does vertical stacking (TSVs) eliminate it?
- [ ] **Capacity overflow**: If an H100 has 80 GB of HBM and a 70B parameter model requires 140 GB for weights (FP16), where must the remaining 60 GB reside?
- [ ] **Bandwidth not capacity**: True or False: HBM's primary benefit is increasing the total memory capacity available to the GPU.
...
HBM4-class packages projected in the 12--16 TB/s aggregate range illustrate the planning pressure created by trillion-parameter-scale models. A 1T-parameter model requires roughly 2 TB of weight storage in FP16. At B200-class HBM3e bandwidths (about 8 TB/s), serving such a model at batch size 1 would produce tokens at 2,000 GB/8 TB/s = 250 ms per token, far too slow for interactive applications. Even at 16 TB/s, the bandwidth floor remains roughly 125 ms per token before batching, quantization, or tensor-parallel sharding. This calculation suggests that trillion-parameter models require aggressive quantization and multi-accelerator tensor parallelism even for single-request inference. The co-evolution of model scale and memory technology continues to drive infrastructure requirements.
...
# │
# │ Imports: matplotlib.pyplot (plt), mlsysim.core.units
...
2.  **Observation**: Unlike Peak FLOP/s\index{Peak FLOP/s} (which only describes the horizontal ceiling), the ridge point describes the Balance of the architecture. A rising ridge point over hardware generations indicates that compute is growing faster than bandwidth, making utilization harder.
3.  **Pitfall**: A frequent misconception is that all GPUs have the same ridge point. In reality, it varies by Precision: because $R_{\text{peak}}$ is higher for INT8 than FP32 while $\text{BW}$ is constant, the ridge point for INT8 is much higher, requiring more data reuse to saturate the hardware.
...
# │
# │ Imports: mlsysim.core.units (BYTES_FP16, TFLOPs, TB, GB, second, byte);
...
class RooflineInferenceMath:
    ...
    inf_intensity_value_str = fmt(inf_intensity, precision=0, commas=False)
    ...
    train_intensity_value_str = fmt(train_intensity, precision=0, commas=True)
    ...
    inf_intensity_math = fmt_math(
        f"\\frac{{2 \\times {params_b_value_str} \\times 10^9}}{{{inf_weight_gb_value_str} \\times 10^9}} = {inf_intensity_value_str}"
    )
    inf_throughput_math = fmt_math(f"{h100_bw_tb_s_value_str} \\times {inf_intensity_value_str} = {inf_throughput_tflop_s_value_str}")
    train_intensity_math = fmt_math(f"{batch_size_str} \\times {inf_intensity_value_str} = {train_intensity_value_str}")
```
...
Consider our `{python} InfraFrontierMemoryMxRecap.frontier_params_b_str` model on an H100 with `{python} RooflineInferenceMath.h100_fp16_tflop_per_s_str` peak compute and `{python} RooflineInferenceMath.h100_bw_tb_per_s_str` memory bandwidth. The ridge point is `{python} RooflineCalloutRecap.h100_ridge_str`.
...
A team is serving a 13B-parameter model at batch size 1 on an H100 and observing 25 ms per token.

1.  Estimate the arithmetic intensity of the workload.
2.  Predict whether upgrading to a B200 with 2.3$\times$ the peak TFLOP/s will achieve the expected 2.3$\times$ speedup.
3.  Identify the alternative upgrade that would be more effective.
...
class TensorCoreTdpRecap:
    ...
    _fp32_cuda = _h100.compute.precision_flops["fp32_cuda"]
...
2.  **Observation**: Unlike general-purpose CUDA cores (which execute one floating-point operation per clock per lane in a SIMT pipeline), Tensor Cores execute 512 multiply-accumulate operations per clock per warp on matrix tiles—trading the arbitrary per-element programmability of CUDA cores for the specific throughput of matrix arithmetic.
3.  **Pitfall**: A frequent misconception is that all GPU operations benefit from Tensor Cores. Only operations that map to the $\mathbf{Y} = \mathbf{A} \times \mathbf{B} + \mathbf{C}$ tile computation (dense linear layers, attention score computation, convolutions) engage Tensor Cores; element-wise activations, layer norm, and embedding lookups fall back to CUDA cores and can run 10--30$\times$ slower per FLOP than Tensor Core operations.
...
The cascade shown in @fig-power-path reveals why power delivery is an infrastructure problem, not merely an electrical one: these conversion losses, combined with cooling overhead measured by PUE (Power Usage Effectiveness), determine the facility's total power draw, and the 10--40 kW concentrated at the rack PDU-to-VRM transition dictates the cooling architecture for the entire facility. PUE expresses that overhead as a single ratio, total facility power divided by the power delivered to the compute hardware, so a PUE of 1.5 means the facility draws 1.5 watts from the grid for every watt that reaches the accelerators.
...
2.  **Observation**: Unlike peak power consumption (the instantaneous maximum during a single-instruction burst), TDP specifies the steady-state thermal budget: the long-term average that determines whether training can run continuously at rated performance or must throttle.
3.  **Pitfall**: A frequent misconception is that TDP is a power limit for the software. TDP is a cooling requirement: the processor exceeds its rated speed only if the cooling system removes heat at least as fast as the chip generates it; insufficient cooling causes hardware throttling silently—no error is raised, training simply slows down and MFU drops without obvious cause.
...
[^fn-dennard-scaling]: **Dennard Scaling**\index{Dennard Scaling!power wall}: Named after @dennard1974 at IBM, who observed that shrinking transistors could maintain constant power density because voltage and current scale with dimensions. The breakdown around 2006 -- when subthreshold leakage and gate-oxide tunneling prevented further voltage reduction below ~28 nm -- is the direct cause of the V100-to-B200 TDP trajectory (`{python} V100DennardRecap.tdp_w_str` to 1,000 W). For ML infrastructure, the post-Dennard era means every TFLOP/s gain arrives with a proportional watt increase, making cooling and power delivery co-equal design constraints with the silicon itself.
...
class GpuEfficiencyTrajectoryRecap:
    ...
    v100_efficiency_tflop_per_s_per_w_str = fmt_compute_efficiency(
        Hardware.Cloud.V100.compute.peak_flops / Hardware.Cloud.V100.tdp, unit=TFLOPs / second / watt, precision=2, commas=False
    )
    a100_efficiency_tflop_per_s_per_w_str = fmt_compute_efficiency(
        Hardware.Cloud.A100.compute.peak_flops / Hardware.Cloud.A100.tdp, unit=TFLOPs / second / watt, precision=2, commas=False
    )
    h100_efficiency_tflop_per_s_per_w_str = fmt_compute_efficiency(
        Hardware.Cloud.H100.compute.precision_flops["fp8"] / Hardware.Cloud.H100.tdp, unit=TFLOPs / second / watt, precision=2, commas=False
    )
    b200_efficiency_tflop_per_s_per_w_str = fmt_compute_efficiency(
        Hardware.Cloud.B200.compute.precision_flops["fp8"] / Hardware.Cloud.B200.tdp, unit=TFLOPs / second / watt, precision=2, commas=False
    )
```
...
# │          InfraFrontierTdpRecap.diffusion_steps_count_str
...
# │ Exports: MoETdpRecap.{total_params_b_str,active_params_b_str,
# │          experts_count_str,active_experts_count_str}
class MoETdpRecap:
...
1.  Use the Roofline Model to determine the arithmetic intensity of each workload on an H100.
2.  Decide whether training and inference should use the same hardware or different hardware.
3.  Calculate the 2-year inference cost difference between H100s (\$4.00/GPU-hour) and A100s (\$2.00/GPU-hour), accounting for throughput at the workload's arithmetic intensity.
...
The defining characteristic of multi-accelerator systems is not the *number of chips* but the *speed at which they can exchange data*. Data movement speed drops by orders of magnitude as it crosses physical boundaries. Each boundary represents a different physical medium, a different connector technology, and a different set of engineering constraints. Understanding this bandwidth hierarchy\index{Bandwidth Hierarchy} is essential because it dictates *where* in the hierarchy each type of parallelism can operate efficiently.
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.memory.bandwidth, Hardware.Cloud.H100.nvlink,
...
***Bandwidth Hierarchy***\index{Bandwidth Hierarchy!definition} is the physical ordering of data transfer rates across ML system boundaries, from on-chip SRAM (fastest) to the wide-area network (slowest).
...
2.  **Observation**: Unlike Idealized Networking Models\index{Idealized Networking Models}, the Bandwidth Hierarchy captures the Physical Cost of Distance\index{Physical Cost of Distance}: shorter signal paths over denser wiring achieve higher throughput at lower energy per bit.
3.  **Pitfall**: A frequent misconception is that all cluster communication is equal. In reality, parallelism strategies must be Hierarchy-Aware\index{Hierarchy-Aware Parallelism}: placing high-frequency synchronization (for example, Tensor Parallelism) on a slow tier will rapidly idle the compute units $(R_{\text{peak}})$, collapsing scaling efficiency $(\eta_{\text{scaling}})$.
...
[^fn-serdes-signaling]: **SerDes (Serializer/Deserializer)**\index{SerDes!signaling}: From Latin *serialis* (in a row) and *de-* (reverse). A circuit pair that converts parallel data to a serial bit stream for transmission and back again. Modern high-speed links use PAM-4 signaling (4-level pulse amplitude modulation, encoding 2 bits per symbol) at very high lane rates, and the transceiver energy becomes a material part of node power. This is why dense interconnects such as NVLink, InfiniBand, and PCIe must be treated as power and cooling loads, not just as bandwidth specifications.
...
**Scenario**: Early users of multi-GPU nodes often attempted tensor parallelism over PCIe, expecting the `{python} BandwidthHierarchyPcieExampleRecap.pcie_bw_gb_per_s_str` bandwidth to suffice.
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: AlternativeNodeArchitectureRecap.frontier_params_b_str,
# │          AlternativeNodeArchitectureRecap.h100_hbm_gb_str,
# │          AlternativeNodeArchitectureRecap.h100_nvlink_gb_per_s_str,
# │          AlternativeNodeArchitectureRecap.tpu_v5p_ici_gb_per_s_str,
# │          AlternativeNodeArchitectureRecap.mi300x_hbm_gb_str,
# │          AlternativeNodeArchitectureRecap.mi300x_node_hbm_gb_str,
# │          AlternativeNodeArchitectureRecap.mi300x_hbm_bw_tb_per_s_str,
# │          AlternativeNodeArchitectureRecap.gaudi3_hbm_gb_str,
# │          AlternativeNodeArchitectureRecap.gaudi2_roce_gb_per_s_str,
# │          AlternativeNodeArchitectureRecap.frontier_weights_gb_str
...
Other systems move that boundary. TPU pods use the Inter-Chip Interconnect (ICI) and a mesh-oriented topology instead of making a central switch the defining component of the node; the TPU v5p profile in MLSysIM models ICI at `{python} AlternativeNodeArchitectureRecap.tpu_v5p_ici_gb_per_s_str`. The mesh can reduce switch cost and expose a regular structure to the compiler, but it also makes placement more visible: the runtime must respect neighbor relationships rather than assuming that every chip pair has the same communication path. That is a reasonable trade when the workload is regular enough for the compiler and collective libraries to map communication onto the topology.

The MI300X shifts the pressure point from local switching toward memory capacity. Its package exposes `{python} AlternativeNodeArchitectureRecap.mi300x_hbm_gb_str` of HBM at `{python} AlternativeNodeArchitectureRecap.mi300x_hbm_bw_tb_per_s_str`, compared with `{python} AlternativeNodeArchitectureRecap.h100_hbm_gb_str` on an H100-class accelerator. An eight-accelerator MI300X node would therefore provide `{python} AlternativeNodeArchitectureRecap.mi300x_node_hbm_gb_str` of aggregate HBM, enough to hold the `{python} AlternativeNodeArchitectureRecap.frontier_weights_gb_str` FP16 weight tensor for our `{python} AlternativeNodeArchitectureRecap.frontier_params_b_str` running model. That does not eliminate the optimizer, gradient, activation, and fragmentation problem introduced in @sec-compute-node, but it changes how often the system must spill state into slower memory tiers and how much microbatch headroom remains before offload becomes mandatory.
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: InfraFrontierNodeWeightsRecap.total_footprint_tb_str
...
During the forward pass of layer $\ell$, the manager simultaneously prefetches the weights for layer $\ell+1$ from DDR5 to HBM (if they have been offloaded) and evicts the weights for layer $\ell-2$ from HBM back to DDR5 (if memory is tight).
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.nvlink, Systems.Fabrics.InfiniBand_NDR.bandwidth, GB, second),
...
class InterconnectCommMath:
    ...
    t_intra_math = fmt_math(
        f"T_{{\\text{{intra}}}} = {gradient_buffer_value}\\text{{ GB}} / {_nvlink_bw_val:.0f}\\text{{ GB/s}} \\approx \\mathbf{{1.1 \\text{{ ms}}}}"
    )
    t_inter_math = fmt_math(
        f"T_{{\\text{{inter}}}} = {gradient_buffer_value}\\text{{ GB}} / {_ib_bw_gbs_val:.0f}\\text{{ GB/s}} \\approx \\mathbf{{20 \\text{{ ms}}}}"
    )
...
A standard 42U server rack\index{Rack} in a traditional data center draws 5--10 kW and can be cooled by room-temperature air pushed through perforated floor tiles. Now place four DGX H100 nodes in that same rack: 32 GPUs, each drawing `{python} H100TdpRackRecap.tdp_w_str`, plus host CPUs, memory, networking, power conversion losses, and cooling overhead. The rack power reaches `{python} RackPowerRackRecap.rack_power_kw_str`, an order of magnitude beyond what traditional data center infrastructure was designed to deliver or cool. At this density, the engineering constraints shift from silicon and signal integrity to power delivery\index{Power Delivery} and thermodynamics. The rack is where the power wall and the laws of heat transfer\index{Cooling} become the dominant design forces.
...
2.  **Observation**: Unlike a node (the compute unit), a rack is the infrastructure unit: it is the physical level at which cooling manifolds, power distribution, and the ToR switch are integrated—making the rack the minimum replaceable and serviceable unit for facility operations, not the node.
3.  **Pitfall**: A frequent misconception is that rack placement is a logistical afterthought. Nodes within the same rack share a ToR switch and have single-hop connectivity; nodes in different racks cross at least two switch hops. Placing all pipeline-parallel stages of a training job within the same rack minimizes inter-stage latency and reduces inter-rack fabric load.

:::

### Cooling architectures {#sec-compute-rack-cooling}
...
The mainstream answer for large clusters is **direct-to-chip liquid cooling**\index{Cooling!direct-to-chip}, which replaces the air gap between the silicon and the coolant with a far more conductive path. A sealed metal **cold plate**\index{Cooling!cold plate} bolts directly onto the accelerator die through the thermal interface material, and a liquid coolant (typically treated water or a water-glycol mix) flows through microchannels milled into the plate, absorbing heat directly at the source. Because water has roughly four times the heat capacity of air by mass and about a thousand times the density, a modest coolant flow carries away heat that would require an impractical torrent of air. The warmed coolant returns to a **coolant distribution unit (CDU)**\index{Coolant Distribution Unit}, a rack-level or row-level appliance that pumps coolant through the cold plates and exchanges the absorbed heat into the facility's chilled-water loop through a liquid-to-liquid heat exchanger. The CDU isolates the clean, low-particulate coolant that touches the chips from the facility water, and it is the component a fleet operator monitors for flow rate, pressure, and leaks. Direct-to-chip loops cool the highest-power components (the accelerators and often the switch ASICs) while leaving lower-power parts such as DIMMs and power supplies on residual airflow, which is why most production liquid-cooled racks are hybrids rather than fully liquid.

Pushing the principle to its limit, **immersion cooling**\index{Cooling!immersion} submerges the entire server board in a bath of dielectric (electrically non-conductive) fluid, eliminating cold plates and air paths altogether. Single-phase immersion circulates a warmed liquid to an external heat exchanger, much like direct-to-chip but with the whole board as the contact surface. Two-phase immersion goes further: the fluid is chosen to boil at the chip's operating temperature, so heat is removed as latent heat of vaporization when the fluid boils at the hot surface and releases it when the vapor condenses on a cooled coil above the bath. Two-phase immersion offers the highest heat-removal density of any approach but adds fluid-cost, containment, and serviceability complications that have kept it rare in production, making it the extreme point of the cooling spectrum rather than the mainstream choice. The cooling-tax calculation later in this section quantifies why the cluster pays for liquid cooling continuously, in facility power, every hour it runs.
...
1.  Calculate the total power draw, including cooling overhead.
2.  Identify whether power capacity or cooling capacity is the binding constraint.
3.  Specify what modifications would be needed to support the full deployment.

:::

The checkpoint's PUE calculation has an economic consequence: cooling overhead is paid every hour the fleet runs.

```{python}
#| echo: false
#| label: rack-cooling-tax
# ┌─────────────────────────────────────────────────────────────────────────────
# │ RACK COOLING TAX (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-compute-rack—PUE and rack-level cooling economics.
# │ Goal: Quantify the facility-power and electricity-cost effect of air vs.
# │       liquid cooling for the 1K training-cluster reference.
# │ Scope: chapter-anchor — rack cooling section and build/buy recap share exports.
# │ How: rack IT power × (PUE_air - PUE_liquid), using registry-backed rack
# │      profile, cooling profiles, and electricity-price values.
# │ Exports: RackCoolingTax.cluster_gpus_str,
# │          RackCoolingTax.cluster_racks_str,
# │          RackCoolingTax.rack_it_power_kw_str,
# │          RackCoolingTax.h100_tdp_w_str,
# │          RackCoolingTax.air_pue_str,
# │          RackCoolingTax.liquid_pue_str,
# │          RackCoolingTax.air_overhead_pct_str,
# │          RackCoolingTax.liquid_overhead_pct_str,
# │          RackCoolingTax.it_power_kw_str,
# │          RackCoolingTax.air_total_kw_str,
# │          RackCoolingTax.liquid_total_kw_str,
# │          RackCoolingTax.savings_kw_str,
# │          RackCoolingTax.electricity_rate_usd_str,
# │          RackCoolingTax.annual_savings_usd_str,
# │          RackCoolingTax.three_year_savings_usd_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Infrastructure, Systems
from mlsysim.fmt import check, fmt, fmt_count, fmt_percent, fmt_power, fmt_usd

class RackCoolingTax:
    """Cooling overhead for the registry-backed 1K H100 training cluster."""

    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    fleet = Systems.Clusters.Training_1K
    rack = Systems.Racks.DGX_H100_4Node
    n_gpus = fleet.total_accelerators
    n_racks = fleet.count / rack.nodes_per_rack
    gpu_tdp = Hardware.Cloud.H100.tdp
    air_pue = Infrastructure.FacilityCooling.SimpleAir.pue
    liquid_pue = Infrastructure.FacilityCooling.StateOfArt.pue
    electricity_rate = Infrastructure.Pricing.OnPremises.ElectricityPerKwh.rate
    lifecycle_years = 3  # scenario assumption: hardware lifecycle for the TCO comparison

    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    rack_it_power = rack.total_power
    it_power = n_racks * rack_it_power
    air_total = it_power * air_pue
    liquid_total = it_power * liquid_pue
    savings = air_total - liquid_total
    annual_savings_usd = (savings * HOURS_PER_YEAR * hour * electricity_rate).to(USD)
    three_year_savings_usd = annual_savings_usd * lifecycle_years

    # ┌── 3. GUARD ─────────────────────────────────────────────────────────
    check(n_gpus == 1024, "Training_1K should represent the 1,024-GPU reference cluster")
    check(n_racks == 32, "Training_1K should occupy 32 four-node racks")
    check(air_pue > liquid_pue, "Air-cooled PUE should exceed liquid-cooled PUE")
    check(1050 < it_power.to(kW).magnitude < 1090, "Fleet IT power should match 32 × 33.5 kW racks")
    check(420 < savings.to(kW).magnitude < 440, "Cooling savings should scale with fleet IT power")

    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────
    cluster_gpus_str = fmt_count(n_gpus, label="GPU")
    cluster_racks_str = fmt_count(n_racks, label="rack")
    rack_gpus_str = fmt_count(rack.accelerator_count, label="GPU")
    rack_it_power_kw_str = fmt_power(rack_it_power, unit=kilowatt, precision=1, commas=False)
    h100_tdp_w_str = fmt_power(gpu_tdp, unit=watt, precision=0, commas=False)
    air_pue_str = fmt(air_pue, precision=2, commas=False)
    liquid_pue_str = fmt(liquid_pue, precision=2, commas=False)
    air_overhead_pct_str = fmt_percent(air_pue - 1, precision=0, commas=False, style='prose')
    liquid_overhead_pct_str = fmt_percent(liquid_pue - 1, precision=0, commas=False, style='prose')
    it_power_kw_str = fmt_power(it_power, unit=kilowatt, precision=0, commas=False)
    air_total_kw_str = fmt_power(air_total, unit=kilowatt, precision=0, commas=False)
    ...
    electricity_rate_usd_str = fmt_usd(electricity_rate, precision=2, commas=False, per="kWh")
    annual_savings_usd_str = fmt_usd(annual_savings_usd, scale="K", precision=0, commas=False)
    three_year_savings_usd_str = fmt_usd(three_year_savings_usd, scale="K", precision=0, commas=False)
```

::: {#nbk-compute-infrastructure-cooling-tax .callout-notebook title="The cooling tax"}

For the chapter's `{python} RackCoolingTax.cluster_gpus_str` reference cluster, each H100 dissipates `{python} RackCoolingTax.h100_tdp_w_str`.

1.  **IT power**\index{IT Power}: `{python} RackCoolingTax.cluster_racks_str` $\times$ `{python} RackCoolingTax.rack_it_power_kw_str` each (`{python} RackCoolingTax.rack_gpus_str` plus host CPUs, memory, and networking) = `{python} RackCoolingTax.it_power_kw_str`.
2.  **Air-cooled facility**: PUE `{python} RackCoolingTax.air_pue_str` adds `{python} RackCoolingTax.air_overhead_pct_str` overhead, raising facility draw to `{python} RackCoolingTax.air_total_kw_str`.
3.  **Liquid-cooled facility**: PUE `{python} RackCoolingTax.liquid_pue_str` adds `{python} RackCoolingTax.liquid_overhead_pct_str` overhead, raising facility draw to `{python} RackCoolingTax.liquid_total_kw_str`.

**Systems insight**: The cooling choice changes continuous power by `{python} RackCoolingTax.savings_kw_str`. At `{python} RackCoolingTax.electricity_rate_usd_str`, that saves about `{python} RackCoolingTax.annual_savings_usd_str` per year, or `{python} RackCoolingTax.three_year_savings_usd_str` over a three-year hardware lifecycle. The thermodynamic requirement and the economic argument point in the same direction: high-density AI racks should be designed for liquid cooling from the start.
...
A pod is a cluster-scale unit of many racks under one high-speed fabric, power envelope, and cooling design. Pod design is the first point where topology, power, and failure domains become shared infrastructure rather than per-rack concerns.
...
Training our `{python} InfraFrontierPodIntroRecap.frontier_params_b_str` model on a single DGX H100 node (8 GPUs, roughly 16,000 TFLOP/s aggregate) would take several months, assuming we can fit the model at all. Reducing training time to weeks requires 100--1,000 nodes operating in concert, and doing so demands that these nodes be wired together into a network fast enough to keep gradient synchronization from becoming the bottleneck. This is the engineering challenge of the pod\index{Pod}: aggregating hundreds of racks into a single, coordinated computing system where the network fabric serves the same role that the system bus serves within a single machine.
...
# │
# │ Imports: mlsysim.core.units (Models.Language.GPT3.parameters, Hardware.Cloud.H100.compute.precision_flops["fp8"], ...)
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.compute.peak_flops,
...
The second principle is that failure is routine, not exceptional. A cluster of 10,000 GPUs, each with a mean time between failures (MTBF)\index{MTBF} of approximately 50,000 hours (the canonical GPU MTTF anchor `Systems.Reliability.Gpu.mttf_hours` used in @sec-fault-tolerance-reliability), will experience a GPU failure roughly once every five hours on average. @Sec-appdx-reliability-foundations-mtbf-cascade derives why this cluster-level rate follows from the per-component lifetimes: reliability degrades linearly with component count at each level, so a node MTBF divided by node count yields the fleet-level interval between failures.
...
# │ Exports: CheckpointOverheadBudget.ckpt_model_weights_gb_str,
# │          .ckpt_optimizer_states_gb_str, .ckpt_total_tb_str,
...
class CheckpointOverheadBudget:
    ...
    # ┌── 2. EXECUTE ──────────────────────────────────────
    checkpoint_model_weights = model.size_in_bytes(BYTES_FP16)
    checkpoint_optimizer_states = model.parameters * BYTES_FP32 * 2
    checkpoint = checkpoint_model_weights + checkpoint_optimizer_states
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    ckpt_model_weights_gb_str = fmt_memory(checkpoint_model_weights, unit=GB, precision=0, commas=False)
    ckpt_optimizer_states_gb_str = fmt_memory(checkpoint_optimizer_states, unit=GB, precision=0, commas=True)
...
Checkpointing also creates a significant *storage* requirement that demands its own dedicated infrastructure. A full checkpoint for our `{python} InfraFrontierStragglerRecap11.frontier_params_b_str` model includes `{python} CheckpointOverheadBudget.ckpt_model_weights_gb_str` of model weights, `{python} CheckpointOverheadBudget.ckpt_optimizer_states_gb_str` of optimizer state, and the random number generator state needed to resume training deterministically. The total is approximately `{python} CheckpointOverheadBudget.ckpt_total_tb_str` per checkpoint.

The overhead budget turns that footprint into a bandwidth requirement. At a cadence of `{python} CheckpointOverheadBudget.ckpt_cadence_min_str`, a write that takes `{python} CheckpointOverheadBudget.ckpt_reference_write_s_str` would consume `{python} CheckpointOverheadBudget.ckpt_reference_overhead_pct_str` of training time, not `{python} CheckpointOverheadBudget.ckpt_target_overhead_pct_str`. To keep checkpoint overhead below `{python} CheckpointOverheadBudget.ckpt_target_overhead_pct_str`, the write must complete in about `{python} CheckpointOverheadBudget.ckpt_allowed_write_s_str`, demanding a sustained aggregate write bandwidth of approximately `{python} CheckpointOverheadBudget.ckpt_required_bw_gb_per_s_str`. A write target of `{python} CheckpointOverheadBudget.ckpt_reference_write_s_str` corresponds to approximately `{python} CheckpointOverheadBudget.ckpt_reference_bw_gb_per_s_str` and only fits a `{python} CheckpointOverheadBudget.ckpt_target_overhead_pct_str` budget when checkpoints are spaced at least `{python} CheckpointOverheadBudget.ckpt_reference_min_cadence_min_str` apart. Standard NFS or object storage implementations collapse under this "thundering herd" pattern, where all GPUs transition from compute to I/O simultaneously. The solution requires a parallel file system that stripes each write across many drives at once, so the aggregate write bandwidth scales with the number of drives rather than the speed of any single one. A failure to architect for this burst bandwidth results in "I/O jitter," where training hangs unpredictably for minutes as the checkpoint write saturates the storage backend, effectively burning millions of dollars in idle GPU cycles. If checkpoints are taken every `{python} CheckpointOverheadBudget.ckpt_cadence_min_str` during a training run of `{python} CheckpointOverheadBudget.ckpt_run_days_str`, the system must write:
...
Because failure is routine at pod scale, the monitoring system\index{Monitoring!fleet} is not an operational convenience but a prerequisite for productive training. A 10,000-GPU cluster experiences hardware failures roughly once every five hours, and the difference between a 10-minute recovery and a multi-hour disruption depends on whether the operations team can detect degradation before it becomes failure. Observability\index{Observability} is itself a distributed system, collecting telemetry from every GPU, switch, and cooling component in the fleet.
...
Fleet operators have learned, often through costly experience, that proactive maintenance can reduce the impact of hardware failures on training productivity. The three pillars of proactive maintenance are: **predictive diagnostics**\index{Predictive Diagnostics} (using models trained on historical telemetry to predict component failures before they occur), **scheduled burn-in testing**\index{Scheduled Burn-In Testing} (running benchmark workloads on newly installed nodes before assigning production work), and **rolling maintenance windows**\index{Rolling Maintenance Window} (cycling a small fraction of nodes through health checks without reducing available capacity). The systems point is not a universal utilization number; it is that proactive maintenance converts surprise outages into scheduled capacity management.
...
class CheckpointComputeTradeoff:
    ...
    mtbf_system_hr = calc_mtbf_cluster(Systems.Reliability.Gpu.mttf_hours, num_gpus)
    mtbf_system_s = mtbf_system_hr.to(second)
    tau_opt_s = calc_young_daly_interval(t_write, mtbf_system_s)
    overhead_pct = (t_write / tau_opt_s).to("").magnitude * 100

    # ┌── 3. GUARD ─────────────────────────────────────────
    check(abs(t_write_value - 21) < 1, "Checkpoint write time should be about 21 s")
    check(800 < tau_opt_s.to(second).magnitude < 900, "Optimal checkpoint interval should be about 14 min")
...
The choice of network topology is a direct reflection of the dominant communication patterns in the distributed training workload. A fat-tree topology offers full bisection bandwidth and non-blocking any-to-any communication, making it theoretically ideal for algorithms like AllReduce and AllToAll. However, the cost of the upper-tier spine switches makes it expensive at scale. A torus or mesh topology provides excellent nearest-neighbor bandwidth at a fraction of the switch cost but suffers severe congestion penalties for global traffic patterns. A rail-optimized topology\index{Topology!rail-optimized} physically wires the network so that each GPU rank connects to a dedicated leaf switch, maximizing the efficiency of structured AllReduce within parallelism groups. For our `{python} InfraFrontierStragglerRecap17.frontier_params_b_str` model configured with TP-8, PP-4, and DP-32, the rail-optimized topology is the natural match: TP-8 means 8-way tensor parallelism inside a node, PP-4 means four pipeline stages, and DP-32 means 32 data-parallel replicas. The bandwidth-heavy tensor parallelism traffic is entirely contained within the NVLink domain of a single node, the pipeline parallelism traffic flows between adjacent nodes requiring only moderate point-to-point bandwidth, and the data-parallel gradient reduction happens across the full fleet but with reduced volume because the TP-8 $\times$ PP-4 partition leaves each data-parallel group synchronizing a 1/32 model shard. The rail-optimized design physically matches this communication structure, placing the nodes that communicate most frequently on the same switch.

#### NVIDIA DGX SuperPOD {#sec-compute-infrastructure-nvidia-dgx-superpod-b9e2}

The DGX SuperPOD\index{SuperPOD} is designed for organizations that run diverse workloads: large language models one week, vision models the next, and scientific simulations on weekends. Its network uses a **fat-tree**\index{Fat-Tree!DGX SuperPOD} topology[^fn-fat-tree-topology] built from InfiniBand NDR switches, providing high-bandwidth, any-to-any connectivity with low hop counts. In a fat-tree, the aggregate bandwidth at each level of the tree matches the bandwidth of the level below, so no bottleneck exists regardless of which pairs of nodes are communicating. A typical SuperPOD configuration connects 32 DGX H100 nodes (256 GPUs) through a two-tier InfiniBand fabric, with larger installations using a three-tier fat-tree to scale to thousands of GPUs.

[^fn-fat-tree-topology]: **Fat-Tree**\index{Fat-Tree!topology}: Proposed by @leiserson1985 at MIT, named because links grow "fatter" (higher bandwidth) at each level of the tree hierarchy, preventing upper-tier congestion. A non-blocking fat-tree gives every node full bisection bandwidth for arbitrary traffic patterns, which makes it common in ML clusters running diverse AllReduce and AllToAll workloads. The cost constraint is switch count: $\mathcal{O}(N^{3/2})$ switches for $N$ endpoints in a three-tier design, making the network fabric 10--15 percent of total cluster cost at scale.
...
: **WSC Architecture Comparison**: The choice of pod architecture is not a matter of preference but of workload economics. NVIDIA prioritizes generality because its customers run diverse workloads. Google TPU pods prioritize dataflow throughput for regular dense models. Meta's Grand Teton design prioritizes embedding capacity for recommendation and ranking workloads. {#tbl-pod-comparison}

#### Meta Grand Teton {#sec-compute-infrastructure-meta-grand-teton-4d4d}
...
The hybrid approach can achieve substantially better cost efficiency than a GPU-only design when embedding capacity dominates. In a GPU-only design, the embedding table would consume most of the HBM capacity, leaving little room for the activations and gradients of the dense layers. The expensive Tensor Cores would sit idle during the memory-bound embedding lookup phase, which can represent a large fraction of inference time for recommendation models.
...
Meta's network architecture for these systems uses RoCE (RDMA over Converged Ethernet) rather than InfiniBand, reflecting a preference for supply chain diversity and cost over raw per-port performance. This can be acceptable when recommendation workloads involve less inter-node synchronization than dense LLM training. The choice also reflects Meta's scale: operating large fleets across multiple data centers, Meta benefits from the commodity economics and multi-vendor supply chain of Ethernet switches compared to InfiniBand's more concentrated ecosystem. @Sec-network-fabrics examines the RoCE vs. InfiniBand trade-off in detail, including the protocol-level differences that affect latency and congestion behavior.
...
Regulatory and geopolitical boundaries can dictate site selection as rigidly as physics. Export controls on high-performance compute act as a hard filter: a cluster designed for 175B-parameter model training may require accelerator classes that are legally restricted from export to certain nations, rendering otherwise power-rich geographies viable only for less restricted workloads or older hardware. Data sovereignty laws like the European Union's General Data Protection Regulation (GDPR) or local residency mandates create a parallel fragmentation, compelling organizations to keep training data, model weights, or serving traffic within specific legal jurisdictions. This regulatory layer can force a suboptimal thermodynamic choice, requiring facilities in expensive, power-constrained regions to satisfy compliance rather than efficiency.

The resulting **power alley**\index{Power alley!site selection} phenomenon concentrates large capacity in a few specific zones: Northern Virginia (Ashburn) for connectivity, the Pacific Northwest for cheap hydroelectric power, and the Nordic countries for free cooling and renewable energy. This geographic clustering creates its own constraint: local grids in these hubs can face multi-year waits for new substation capacity, and land prices can rise dramatically. As prime zones saturate, operators explore unconventional "brownfield" sites -- retired aluminum smelters, defunct coal plants, and industrial parks with existing high-voltage transmission -- where power capacity exists but fiber and cooling infrastructure must be built from scratch.

Water scarcity is the final volatile constraint. A 10 MW training facility using standard evaporative cooling towers can consume well over 100 million liters of water annually, depending on climate and cooling design. In water-stressed regions like the American Southwest or parts of the Middle East, this consumption can place data centers in direct competition with municipal and agricultural needs, leading to permitting limits or moratoriums. Operators may adopt closed-loop liquid cooling or fully adiabatic dry cooler systems, which reduce water consumption to near zero but increase capital intensity and may impose a PUE penalty compared to evaporative systems in hot climates.
...
The practical consequence is that infrastructure decisions must be made 18--30 months before the infrastructure is needed. Given that GPU architecture generations can span roughly two years, the infrastructure team is often selecting a facility design for hardware that does not yet exist, based on preliminary specifications from the manufacturer. To mitigate this temporal mismatch, experienced infrastructure teams design facilities with **headroom**\index{Headroom!facility design} for later generations: oversizing the electrical infrastructure by 30--50 percent, designing the cooling plant for higher heat densities than the initial deployment requires, and specifying flexible rack layouts. The additional upfront cost of this headroom (typically 10--20 percent of facility CapEx) is justified by the ability to upgrade compute hardware without reconstructing the facility.

The construction timeline is governed by a **critical path**\index{Critical Path} that aligns two disparate workstreams: the facility track (site selection, power provisioning, building shell, cooling) and the hardware track (silicon allocation, server assembly, network integration). While GPU procurement often captures headlines, the true bottleneck can be the electrical substation, which may require 18--24 months for permitting and transformer delivery -- a timeline that cannot always be compressed by spending more. This creates a high-stakes synchronization problem. An organization that secures a 20,000-GPU allocation (\$500M+) before its facility is ready risks depreciating allocated silicon in a warehouse for months. Conversely, completing a \$200M data center shell only to wait 6--12 months for an accelerator allocation leaves massive capital assets idle.
...
These logistical contortions are driven by the **time value of compute**\index{Time value of compute}. For large-model development, the cost of delay can exceed the interest on capital because delayed compute postpones experiments, product launches, or downstream capabilities. If a model is projected to generate \$10 million per month in value -- whether through direct inference revenue or downstream product capabilities -- a three-month construction delay represents a \$30 million loss, often exceeding the cost of the facility's electrical infrastructure entirely. This calculus can justify aggressive expediting measures, such as paying premiums for prefabricated modular data centers or leasing temporary colocation space to bridge the gap between silicon delivery and facility readiness.

#### Build-vs-buy total cost of ownership (TCO) threshold {#sec-compute-infrastructure-build-vs-buy-tco}

The same timing problem shapes the build-vs-buy decision. Cloud rental converts scarce accelerators into an operating expense and is attractive when demand is bursty, uncertain, or urgent. Owning a cluster converts the same capacity into capital expense and only wins when utilization is high enough to amortize the node, power, cooling, networking, staffing, and facility risk over the hardware lifecycle.

```{python}
#| echo: false
#| label: compute-infrastructure-build-buy-tco
# ┌─────────────────────────────────────────────────────────────────────────────
# │ BUILD-VS-BUY TCO THRESHOLD (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-compute-infrastructure-build-vs-buy-tco and chapter summary.
# │ Goal: Estimate the utilization threshold where owning a DGX H100 node beats
# │       renting equivalent cloud H100 hours under a simplified model.
# │ Scope: chapter-anchor — TCO section and chapter summary share breakeven inputs.
# │ How: annualized node CapEx / (cloud annual cost at 100% utilization -
# │      on-prem electricity cost at 100% utilization).
# │ Exports: BuildBuyTcoThreshold.node_gpus_str,
# │          BuildBuyTcoThreshold.node_price_usd_str,
# │          BuildBuyTcoThreshold.amortization_years_str,
# │          BuildBuyTcoThreshold.cloud_h100_hourly_usd_str,
# │          BuildBuyTcoThreshold.electricity_rate_usd_str,
# │          BuildBuyTcoThreshold.breakeven_util_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Infrastructure, Systems
from mlsysim.fmt import check, fmt, fmt_count, fmt_percent, fmt_usd

class BuildBuyTcoThreshold:
    """Simplified per-node build-vs-buy utilization threshold."""
    ...
    amortization_years = 3  # scenario assumption: simplified node amortization window
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    annual_cloud_cost = cloud_h100_hourly * gpus_per_node * HOURS_PER_YEAR
    annual_power_cost = node_power.to(kW).magnitude * HOURS_PER_YEAR * onprem_elec * facility_pue
    annual_capex = dgx_h100_price / amortization_years
    breakeven_util = annual_capex / (annual_cloud_cost - annual_power_cost)

    # ┌── 3. GUARD ─────────────────────────────────────────────────────────
    check(0 < breakeven_util < 1, "Breakeven utilization must be a fraction")
    check(annual_cloud_cost > annual_power_cost, "Cloud cost must exceed on-prem power cost")

    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────
    node_gpus_str = fmt_count(gpus_per_node, label="GPU")
    node_price_usd_str = fmt_usd(dgx_h100_price, scale="M", precision=1, commas=False)
    amortization_years_str = fmt(amortization_years, precision=0, commas=False)
    cloud_h100_hourly_usd_str = fmt_usd(cloud_h100_hourly, precision=0, commas=False, per="GPU-hour")
    electricity_rate_usd_str = fmt_usd(onprem_elec, precision=2, commas=False, per="kWh")
    breakeven_util_pct_str = fmt_percent(breakeven_util, precision=1, commas=False, style='prose')
```

This simplified threshold deliberately omits facility construction, network fabric, staffing, financing, and resale value so the direction of the trade-off is visible. A DGX H100 node with `{python} BuildBuyTcoThreshold.node_gpus_str`, costing about `{python} BuildBuyTcoThreshold.node_price_usd_str` and amortized over `{python} BuildBuyTcoThreshold.amortization_years_str` years, becomes cheaper than renting equivalent cloud H100 capacity at `{python} BuildBuyTcoThreshold.cloud_h100_hourly_usd_str` when sustained utilization exceeds roughly `{python} BuildBuyTcoThreshold.breakeven_util_pct_str` of full-time use. The comparison is annualized: amortized hardware plus facility electricity versus hourly cloud rental at the same utilization, before the omitted fleet-level costs are added. The engineering lesson is not that this threshold is universal; it is that utilization, power price (`{python} BuildBuyTcoThreshold.electricity_rate_usd_str` here), and facility readiness determine whether ownership turns into leverage or idle capital.

These physical and logistical constraints define warehouse-scale computer design. New technologies do not remove the walls; they move them. The technologies below matter because they change where the memory, communication, or power constraint binds.
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: EmergingInfrastructureRecap.frontier_params_b_str,
# │          EmergingInfrastructureRecap.frontier_weights_gb_str,
# │          EmergingInfrastructureRecap.optimizer_state_gb_str,
# │          EmergingInfrastructureRecap.h100_hbm_bw_tb_per_s_str,
# │          EmergingInfrastructureRecap.cxl_bw_gb_per_s_str,
# │          EmergingInfrastructureRecap.cxl_vs_hbm_gap_mult_str,
# │          EmergingInfrastructureRecap.wse_sram_gb_str,
# │          EmergingInfrastructureRecap.wse_memoryx_bw_tb_per_s_str
...
Compute infrastructure mistakes usually come from optimizing the visible specification while ignoring the binding physical constraint. The fallacies below translate the chapter's node, rack, pod, and facility lessons into operational warnings.
...
**Pitfall**: *Rejecting liquid cooling on budget grounds without modeling operating cost.*

Organizations often reject liquid cooling based on the upfront capital cost (10--15 percent higher than air-cooled facilities) without performing a TCO analysis. The Rack section's cooling-tax calculation showed that liquid cooling saves `{python} RackCoolingTax.savings_kw_str` of continuous power for the `{python} RackCoolingTax.cluster_gpus_str` reference cluster, translating to about `{python} RackCoolingTax.annual_savings_usd_str` per year in electricity savings. Over a three-year lifecycle, the savings reach `{python} RackCoolingTax.three_year_savings_usd_str`, which can exceed the incremental capital cost of the liquid cooling infrastructure. At larger scales, the savings multiply proportionally, making liquid cooling both thermodynamically necessary and economically advantageous.
...
# │ How: pulling constants from mlsysim.core.units.
# │
# │ Imports: mlsysim.core.units (Models.Language.GPT3.parameters, Hardware.Cloud.H100.compute.precision_flops["fp8"], Hardware.Cloud.H100.memory.bandwidth,
...
* **TCO, not CapEx**: The total cost of ownership includes electricity, facility construction, networking, and staffing, not just the purchase price. Under the simplified per-node hardware-plus-electricity model in @sec-compute-infrastructure-build-vs-buy-tco, owning becomes favorable above approximately `{python} BuildBuyTcoThreshold.breakeven_util_pct_str` sustained utilization; adding facility, network, and staffing costs to a full fleet TCO raises that threshold further.
