...
# │ How: IT power × (PUE_air - PUE_liquid), using registry-backed GPU,
# │      cooling-profile, and electricity-price values.
...
class RackCoolingTax:
    ...
    check(280 < savings.to(kW).magnitude < 290, "Cooling savings should be about 287 kW")
...
1.  **IT power**\index{IT Power}: `{python} RackCoolingTax.cluster_gpus_str` $\times$ `{python} RackCoolingTax.h100_tdp_w_str` = `{python} RackCoolingTax.it_power_kw_str`.
...
class PodTrainingMath:
    ...
    pod_flops_math = fmt_math(f"6 \\times {gpt3_params_b_value_str} \\times {gpt3_tokens_b_value_str} \\times 10^{{9}}")
...
class BuildBuyTcoThreshold:
    ...
    node_power = Hardware.Cloud.H100.tdp * gpus_per_node * 1.2  # scenario assumption: 20% non-GPU node overhead
...
This simplified threshold deliberately omits facility construction, network fabric, staffing, financing, and resale value so the direction of the trade-off is visible. A DGX H100 node with `{python} BuildBuyTcoThreshold.node_gpus_str`, costing about `{python} BuildBuyTcoThreshold.node_price_usd_str` and amortized over `{python} BuildBuyTcoThreshold.amortization_years_str` years, becomes cheaper than renting equivalent cloud H100 capacity at `{python} BuildBuyTcoThreshold.cloud_h100_hourly_usd_str` when sustained utilization exceeds roughly `{python} BuildBuyTcoThreshold.breakeven_util_pct_str`, before the omitted fleet-level costs are added. The engineering lesson is not that this threshold is universal; it is that utilization, power price (`{python} BuildBuyTcoThreshold.electricity_rate_usd_str` here), and facility readiness determine whether ownership turns into leverage or idle capital.
