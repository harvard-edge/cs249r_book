...
Google's TPU trajectory follows a different path, focusing on distributed efficiency and XLA compiler integration. While GPUs emphasize peak per-chip throughput and software flexibility (CUDA), the TPU is designed from the beginning as a "Pod-scale" resource. Every TPU chip is born as part of a larger cluster, with dedicated inter-chip interconnect (ICI) links that bypass the host CPU entirely. The TPU v1 (2015) was a dedicated inference chip with a $256{\times}256$ systolic array, delivering 92 TOPS (INT8) for the matrix operations that dominated Google's production inference workloads. TPU v2 (2017) and TPU v3 (2018) then shifted the architecture toward training by adding bfloat16 (BF16), avoiding the dynamic range problems of standard FP16, and scaling the pod concept to 1,024 chips and 100+ PFLOP/s of aggregate compute. TPU v4 (2021) extended the same pod-first logic into the network itself, adding an optical circuit switch (OCS) so the physical topology could be reconfigured for different workloads while raising per-chip BF16 throughput to 275 TFLOP/s.
...
Gaudi takes a different route by integrating RDMA-capable Ethernet into the accelerator and using a more network-centric design for communication. The point is not that it lacks memory capacity; a Gaudi 3-class accelerator still exposes `{python} AlternativeNodeArchitectureRecap.gaudi3_hbm_gb_str` of HBM. The question is whether simplifying the fabric and using Ethernet consistently across node and pod is worth giving up the very high local exchange bandwidth of a switch-rich NVLink design. That trade can be attractive when data parallelism, inference serving, or cluster cost dominates, and less attractive when tensor-parallel collectives sit inside every layer.
...
Underpinning both directions is the evolution of **advanced packaging**\index{Advanced Packaging}. Packaging moves the boundary inside the accelerator package rather than all the way out to the rack or all the way in to a single wafer. Current **2.5D interposers**\index{2.5D interposers} mount compute dies and HBM stacks on a shared silicon substrate, allowing chiplet-based accelerators to behave like a single device for most software. The next step, **3D stacking**\index{3D stacking}, places SRAM caches or logic vertically above one another, shortening data paths further. The wall that appears next is thermal: dense vertical integration makes it harder to remove heat from buried layers. The fleet-design consequence is that future accelerators may demand liquid cooling, tighter power transients, and more conservative rack layouts before their architectural advantages can be realized.
...
The ML fleet is built through a series of progressive responses to physical constraints. Matrix multiplication workloads first demanded specialized arithmetic units, producing Tensor Cores and systolic arrays that trade general-purpose flexibility for orders-of-magnitude throughput gains. Weight loading during inference then exposed the memory wall, driving the development of HBM with 3D-stacked dies and through-silicon vias that deliver roughly 17$\times$ the bandwidth of high-end DDR5 in the H100 comparison, and tens of times more bandwidth in newer HBM generations. Once model sizes exceeded single-chip memory capacity, multi-accelerator nodes connected by NVLink became necessary, creating the bandwidth hierarchy that dictates parallelism strategy.

The same constraint cascade continues beyond the server. Synchronous training across dense nodes creates power transients that stress data center electrical infrastructure, which is why large clusters require supercapacitor banks and fast-response UPS systems. Rack power densities exceeding 100 kW force the transition from air cooling to direct-to-chip liquid cooling, reducing the cooling tax from 50 percent to under 10 percent of IT power. Each level is therefore not an independent component choice but the engineering consequence of the level below it.
