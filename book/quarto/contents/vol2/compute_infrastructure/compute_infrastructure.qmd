...
\noindent
![](images/png/cover_infrastructure.png){fig-alt="Isometric datacenter visualization with rows of server racks connected by glowing blue network paths, cooling infrastructure below, and monitoring dashboards displaying utilization graphs above." width=100 percent}
...
![](images/svg/accelerator-spectrum.svg){width=100 percent}
:::
...
| **Architecture**\index{Architecture} | **Year** | **Key Innovation**      | **Peak Tensor** | **TDP** | **NVLink BW** |
|:-------------------------------------|:--------:|:------------------------|:---------------:|:-------:|:-------------:|
...
The **TPU v5p** (2023) represents Google's most capable training accelerator. It features 459 TFLOPS of BF16 compute and 95 GB of HBM, but its true advantage lies in its interconnect: 1,600 GB/s of ICI bandwidth, optimized for the massive AllReduce operations required by billion-parameter models.
...
[^fn-register-file-bandwidth]: **Register File Bandwidth**\index{Register File Bandwidth}: In an H100 GPU, the register file provides over 300 TB/s of aggregate bandwidth to the CUDA cores. This is ~100$\times$ higher than HBM3 bandwidth, explaining why tiling is mandatory: any operand not in a register during execution forces the arithmetic units to wait 100+ clock cycles for data, collapsing utilization. \index{Register File!bandwidth}
...
While **Archetype A (GPT-4)** is primarily throughput-bound (demanding more TFLOPS), **Archetype B (DLRM at Scale)** is primarily capacity-bound. A 10 TB embedding table cannot fit into the 80 GB HBM of a single H100, nor can it fit into the aggregate HBM of a single 8-GPU node (640 GB). This physical limit forces Archetype B to shard its state across hundreds of nodes, transforming a memory-access problem into a massive **All-to-All** network coordination problem.
...
![](images/svg/tensor-core-tiling.svg){width=100 percent}
:::
...
![](images/svg/hbm-architecture.svg){width=100 percent}
:::
...
| **Metric**\index{Metric}       |                 **Host DRAM** (DDR5)                |             **Accelerator HBM** (HBM3e)              |     **Scaling Factor**    |
|:-------------------------------|:---------------------------------------------------:|:----------------------------------------------------:|:-------------------------:|
...
**Scenario**\index{Scenario}: Generating one token for Llama-3 70B (140 GB weights in FP16) on an NVIDIA H100.

1. **Compute Time**\index{Compute Time}: Each token requires $2 \times 70\text{B} = 1.4 \times 10^{11}$ floating-point operations. At `{python} H100PerformanceScenario.h100_tflops` TFLOPS peak throughput, the arithmetic takes:
...
$$ \text{Achievable FLOPS} = \min\left(R_{\text{peak}},\ \text{BW} \times I\right) $$ {#eq-roofline}
...
A Tensor Core, by contrast, performs a complete matrix-multiply-accumulate (MMA) of the form $D = A \times B + C$ on small tile matrices in a single cycle. On the H100, each Tensor Core computes a 16$\times$ $16\times16$ MMA per cycle, producing 4,096 multiply-accumulate results simultaneously. This is equivalent to 8$\times$ what an entire CPU can produce per cycle, concentrated in a single hardware unit that occupies a tiny fraction of the chip area.
...
1.  **Significance (Quantitative):** $\text{MFU} = C_{\text{model}} / (R_{\text{peak}} \cdot T_{\text{step}})$, where $C_{\text{model}} \approx 6P$ FLOPs for a $P$-parameter transformer. For a 7B-parameter model on H100 ($R_{\text{peak}} = 312$ TFLOPS) with a step time of 0.4 seconds: $\text{MFU} = 6 \times 7\times10^9 / (312\times10^{12} \times 0.4) \approx 34\%$. A practical ceiling of 55–65 percent applies even in highly optimized implementations; the gap represents inescapable memory stalls ($\text{BW}$ bottleneck), AllReduce overhead ($L_{\text{lat}}$), and pipeline bubbles.
...
::: {#fig-node-topology-comparison fig-env="figure" fig-pos="htb" fig-cap="**Intra-Node GPU Topology Comparison**. Three approaches to wiring GPUs inside a node. (Left) **Ring**\index{Ring}: sequential pass-through, ~50 GB/s per link, latency scales with distance. (Center) **Full Mesh**\index{Full Mesh}: all-to-all direct links, N-1 links per GPU, impractical beyond ~8 GPUs. (Right) **NVSwitch Crossbar**\index{NVSwitch Crossbar}: non-blocking fabric with 112 GB/s per NVLink and 900 GB/s aggregate, any-GPU-to-any-GPU in one hop, enabling tensor-parallel AllReduce within a node." fig-alt="Three panels comparing intra-node GPU topologies. Left: Ring with 8 GPUs and arrows around the loop. Center: Full Mesh showing all-to-all links between 8 GPUs. Right: 8 GPUs connected through a central NVSwitch crossbar fabric."}
![](images/svg/node-topology-comparison.svg){width=100 percent}
:::
...
![](images/svg/bandwidth-hierarchy.svg){width=100 percent}
:::
...
**Problem**\index{Problem}: You are synchronizing a 10 GB buffer. How much does the physical "Layer" of the fleet affect your transfer time?
...
|:----------------------------------|:--------------:|:-------------------:|
| **GPU compute ($32\times700$ W)** |      22.4      |      67 percent     |
...
2.  **Distinction (Durable):** Unlike **Computing Efficiency** (which focuses on FLOPs per Watt), PUE focuses on **Facility Efficiency**\index{Facility Efficiency}: it captures how much energy is "wasted" before it even reaches the processor.
...
![](images/svg/cooling-comparison.svg){width=100 percent}
:::
...
- **IT Power**\index{IT Power}: $1,000\times700$ W = 700 kW
...
![](images/svg/pod-network-topology.svg){width=100 percent}
:::
...
2. **Cluster Throughput**\index{Cluster Throughput}: 8,192 H100 GPUs at `{python} NodeCapacityScenario.h100_tflops` TFLOPS peak, operating at 45 percent MFU:
...
***Warehouse-Scale Computer (WSC)***\index{Warehouse-Scale Computer!definition} is a building-scale computing system in which thousands of servers are operated as a single coherent machine—with the network fabric serving as the system bus, distributed storage as the disk subsystem, and a cluster orchestrator as the operating system—enabling training workloads that would be physically impossible on any single machine.
...
- **Checkpoints per run**\index{Checkpoints Per Run}: (14 days$\times$ 24 hours$\times$ 6 per hour) = 2,016 checkpoints
- **Total checkpoint data**\index{Total Checkpoint Data}: $2,016\times1.75$ TB = 3,528 TB
...
3.  **Common Pitfall:** A frequent misconception is that any model can be sharded across any number of nodes. In reality, if the **Hierarchy Mapping**\index{Hierarchy Mapping} is wrong (for example, sharding a large tensor across a slow inter-rack link), the communication time will dwarf the compute time, making the scale-out useless.
...
| **Optimization**\index{Optimization} | Flexibility         | **Dataflow Throughput** | Embedding Memory          |
...
![](images/svg/tco-build-vs-buy.svg){width=100 percent}
:::
...
- **Facility CapEx**\index{Facility CapEx}: ~\$75M (liquid-cooled datacenter hall)
- **Annual Electricity**\index{Annual Electricity}: 10,000 GPUs$\times$ 700 W$\times$ PUE $1.1\times8,760$ h$\times$ \$0.07/kWh = \$4.7M/year
- **Annual Staffing**\index{Annual Staffing}: ~\$5M/year
- **3-Year Total**\index{3-Year Total}: \$537.5M + 3$\times$ \$9.7M = **~\$567M**

**Cloud (3-year rental at 80 percent utilization)**:

- **Annual Cost**\index{Annual Cost}: 10,000 GPUs$\times$ \$4.00/hr$\times$ 8,760 h$\times$ 0.80 = \$280M/year
- **3-Year Total**: **~\$840M**

**Break-Even**\index{Break-Even}: On-premises saves approximately \$273M over three years at 80 percent utilization. The savings disappear if utilization drops below ~40 percent, because the on-premises hardware still incurs facility and staffing costs regardless of load.
...
- **PUE as Efficiency Metric**\index{PUE As Efficiency Metric}: Liquid cooling achieves PUE of 1.05--1.10, delivering 90--95 percent of facility power to computation, compared to 50--65 percent for air-cooled facilities. The transition from air to liquid cooling is a thermodynamic necessity, not an optional upgrade, for modern ML racks.
- **Failure at Scale**\index{Failure at Scale}: A 10,000-GPU cluster experiences roughly one GPU failure per hour. Checkpointing strategy is a first-order performance concern that must be designed into the system from the start.
- **TCO, Not CapEx**\index{TCO, Not CapEx}: The total cost of ownership includes electricity (which can equal hardware cost over a 3-year lifecycle), facility construction, and staffing. On-premises infrastructure becomes cost-effective only above approximately `{python} TcoScenario.breakeven_util` percent sustained utilization.
