...
class TpuV5pRecap:
    ...
    hbm_gb_str = fmt_memory_capacity(v5p.memory.capacity, unit=GB, precision=0, commas=False)
...
class MemoryBandwidthCacheRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_l2_bytes_str = fmt_memory_capacity(Hardware.Cloud.H100.memory.l2_cache, unit=MB, precision=0, commas=False)
    h100_mem_gb_str = fmt_memory_capacity(Hardware.Cloud.H100.memory.capacity, unit=GB, precision=0, commas=False)
    h100_bw_tb_per_s_str = fmt_qty(Hardware.Cloud.H100.memory.bandwidth, TB / second, precision=2, commas=False)
```
...
**Scenario**: An engineering team evaluates hardware choices for serving (decode batch size 1) versus training (batch size `{python} RooflineInferenceMath.batch_size_str`) a `{python} RooflineInferenceMath.params_b_str` parameter model on an H100 GPU (ridge point `{python} RooflineInferenceMath.h100_ridge_str`).
...
class InfraFrontierNodeBudgetRecap:
    ...
    gpu_hbm_gb_str = fmt_memory_capacity(gpu_hbm, unit=GB, precision=0, commas=False)
    node_hbm_gb_str = fmt_memory_capacity(node_hbm, unit=GB, precision=0, commas=False)
```
...
class NodeCapacityDenseRecap:
    ...
    node_hbm_cap_gb_str = fmt_memory_capacity(
        Hardware.Cloud.H100.memory.capacity * Systems.Nodes.DGX_H100.accelerators_per_node, unit=GB, precision=0, commas=False
    )
...
class AlternativeNodeArchitectureRecap:
    ...
    h100_hbm_gb_str = fmt_memory_capacity(h100.memory.capacity, unit=GB, precision=0, commas=False)
    ...
    mi300x_hbm_gb_str = fmt_memory_capacity(mi300x.memory.capacity, unit=GB, precision=0, commas=False)
    mi300x_node_hbm_gb_str = fmt_memory_capacity(mi300x_node_hbm, unit=GB, precision=0, commas=True)
    mi300x_hbm_bw_tb_per_s_str = fmt_bandwidth(mi300x.memory.bandwidth, unit=TB / second, precision=1, commas=False)
    gaudi3_hbm_gb_str = fmt_memory_capacity(gaudi3.memory.capacity, unit=GB, precision=0, commas=False)
...
class NodeCapacityTrainMemRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    node_hbm_cap_gb_str = fmt_memory_capacity(node_hbm, unit=GB, precision=0, commas=False)
    host_dram_tb_str = fmt_memory_capacity(host_dram, unit=TB, precision=0, commas=False)
    weights_gb_str = fmt_memory(weights, unit=GB, precision=0, commas=True)
    gradients_gb_str = fmt_memory(gradients, unit=GB, precision=0, commas=True)
    optimizer_gb_str = fmt_memory(optimizer, unit=GB, precision=0, commas=True)
...
class MemoryBandwidthNodeTableRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_bw_tb_per_s_str = fmt_qty(Hardware.Cloud.H100.memory.bandwidth, TB / second, precision=2, commas=False)
    hbm_per_gpu_gb_str = fmt_memory_capacity(hbm_per_gpu, unit=GB, precision=0, commas=False)
    gpus_per_node_str = fmt(gpus_per_node, precision=0, commas=False)
    node_hbm_gb_str = fmt_memory_capacity(node_hbm, unit=GB, precision=0, commas=False)
    host_dram_tb_str = fmt_memory_capacity(host_dram, unit=TB, precision=0, commas=False)
...
class EmergingInfrastructureRecap:
    ...
    wse_sram_gb_str = fmt_memory_capacity(wse.memory.capacity, unit=GB, precision=0, commas=False)
...
class A100FallaciesRecap:
    ...
    option_b_hbm_gb_str = fmt_memory_capacity(option_b_hbm, unit=GB, precision=0, commas=False)
