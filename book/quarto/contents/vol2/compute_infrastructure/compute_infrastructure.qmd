...
# │ Exports: (none)
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_sci, fmt_qty, fmt_qty_int, check, fmt_math
```
...
class InfraSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_params_b_str = fmt(m_frontier.parameters.to(param).magnitude / BILLION, precision=0, commas=False)
```
...
class InfraFrontierAccelRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_params_b_str = fmt(Models.Language.GPT3.parameters.to(param).magnitude / BILLION, precision=0, commas=False)
```
...
class A100Specs:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    flops_fp16_tflops = flops_fp16_tc.to(TFLOPs / second).magnitude
    mem_bw_gbs = mem_bw.to(GB / second).magnitude
    mem_bw_tbs = mem_bw.to(TB / second).magnitude
    tdp_w = tdp.to(watt).magnitude
    nvlink_gbs = nvlink.to(GB / second).magnitude
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    flops_fp16_str = fmt_qty(flops_fp16_tflops * TFLOP / second, TFLOP / second, precision=0, commas=False)
    mem_bw_str = fmt_qty(mem_bw_gbs * GB / second, GB / second, precision=0, commas=False)
    mem_bw_tbs_str = fmt_qty(mem_bw_tbs * TB / second, TB / second, precision=2, commas=False)
    tdp_str = fmt_qty(tdp_w * watt, watt, precision=0, commas=False)
    nvlink_str = fmt_qty(nvlink_gbs * GB / second, GB / second, precision=0, commas=False)
```
...
class H100PerformanceScenario:
    ...
    h100_tf = flops.to(TFLOPs/second).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt_qty(h100_tf * TFLOP / second, TFLOP / second, precision=0, commas=False)
```
...
class B200PerformanceScenario:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    fp8_tflops = Hardware.Cloud.B200.compute.precision_flops["fp8"].to(TFLOPs / second).magnitude
    tdp_w = Hardware.Cloud.B200.tdp.to(watt).magnitude
    nvlink_gbs = Hardware.Cloud.B200.nvlink.bandwidth.to(GB / second).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    fp8_tflops_str = fmt_qty(fp8_tflops * TFLOP / second, TFLOP / second, precision=0, commas=True)
    tdp_w_str = fmt_qty(tdp_w * watt, watt, precision=0, commas=False)
    nvlink_str = fmt_qty(nvlink_gbs * GB / second, GB / second, precision=0, commas=True)
```
...
class A100HbmQuantRecap:
    # ┌── 1. LOAD ──────────────────────────────────────────
    params = Models.Language.Llama3_70B.parameters.to(param).magnitude
    weights_gb = (params * BYTES_FP16.to(byte).magnitude * byte).to(GB).magnitude
    ...
    b200_bw_tbs = Hardware.Cloud.B200.memory.bandwidth.to(TB/second).magnitude
    ...
    weights_gb_str = fmt_qty_int(weights_gb * GB, GB, commas=False)
    mem_bw_tbs_str = fmt_qty(a100_bw_tbs * TB / second, TB / second, precision=2, commas=False)
    a100_token_ms_str = fmt_time(a100_token_ms, 'millisecond', precision=1, commas=False)
    h100_bw_tbs_str = fmt_qty(h100_bw_tbs * TB / second, TB / second, precision=2, commas=False)
    h100_token_ms_str = fmt_time(h100_token_ms, 'millisecond', precision=1, commas=False)
    b200_bw_tbs_str = fmt_qty(b200_bw_tbs * TB / second, TB / second, precision=0, commas=False)
    b200_token_ms_str = fmt_time(b200_token_ms, 'millisecond', precision=1, commas=False)
```
...
from mlsysim.fmt import fmt, check, fmt_math, fmt_qty
...
class TokenLatencyScenario:
    ...
    weight_gb_str = fmt_qty(weight_vol, GB, precision=0, commas=False)
...
class RooflineLandscapeMath:
    ...
    # ┌── 1. LOAD ─────────────────────────────────────────────
    h_h100 = Hardware.Cloud.H100
    peak_tflops = h_h100.compute.peak_flops.to(TFLOPs/second).magnitude
    bandwidth_tbs = h_h100.memory.bandwidth.to(TB/second).magnitude
...
class RooflineInferenceMath:
    ...
    weight_bytes_per_param = BYTES_FP16.to(byte).magnitude
    batch_size = 2048
    h_h100 = Hardware.Cloud.H100
    h100_bw_tbs = h_h100.memory.bandwidth.to(TB/second).magnitude
    h100_peak_tflops = h_h100.compute.peak_flops.to(TFLOPs/second).magnitude
    ridge = (h_h100.compute.peak_flops / h_h100.memory.bandwidth).to(flop/byte).magnitude
    params_b_str = fmt(params / BILLION, precision=0, commas=False)
    h100_bw_str = fmt(h100_bw_tbs, precision=2, commas=False)
    h100_tflops_str = fmt_qty(h100_peak_tflops * TFLOP / second, TFLOP / second, precision=0, commas=False)
    h100_ridge_str = fmt(ridge, precision=1, commas=False)

    # ┌── 2. EXECUTE ──────────────────────────────────────────
    inf_flops = 2 * params
    inf_weight_bytes = weight_bytes_per_param * params
    inf_weight_gb = (inf_weight_bytes * byte).to(GB).magnitude
...
Consider our `{python} InfraFrontierMemoryMxRecap.frontier_params_b_str`B model on an H100 with `{python} RooflineInferenceMath.h100_tflops_str` peak compute and `{python} RooflineInferenceMath.h100_bw_str` TB/s memory bandwidth. The ridge point is `{python} RooflineCalloutRecap.h100_ridge_str` FLOP/byte.
...
class TensorCoreTdpRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp16_tc_str = fmt(Hardware.Cloud.H100.compute.peak_flops.to(TFLOPs/second).magnitude, precision=0, commas=False)
    h100_fp8_tc_str = fmt(Hardware.Cloud.H100.compute.precision_flops['fp8'].to(TFLOPs/second).magnitude, precision=0, commas=True)
    tc_vs_cuda_ratio_str = fmt(64.0, precision=0, commas=False)
    h100_fp32_cuda_str = fmt_int(Hardware.Cloud.H100.compute.peak_flops.to(TFLOPs/second).magnitude/64, commas=False)
```
...
class GpuEfficiencyTrajectoryRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_ef_str = fmt_qty((Hardware.Cloud.V100.compute.peak_flops.to(TFLOPs/second).magnitude/Hardware.Cloud.V100.tdp.to(watt).magnitude) * TFLOP / second, TFLOP / second, precision=2, commas=False)
    a100_ef_str = fmt_qty((Hardware.Cloud.A100.compute.peak_flops.to(TFLOPs/second).magnitude/Hardware.Cloud.A100.tdp.to(watt).magnitude) * TFLOP / second, TFLOP / second, precision=2, commas=False)
    h100_ef_str = fmt_qty((Hardware.Cloud.H100.compute.precision_flops['fp8'].to(TFLOPs/second).magnitude/Hardware.Cloud.H100.tdp.to(watt).magnitude) * TFLOP / second, TFLOP / second, precision=2, commas=False)
    b200_ef_str = fmt_qty((Hardware.Cloud.B200.compute.precision_flops['fp8'].to(TFLOPs/second).magnitude/Hardware.Cloud.B200.tdp.to(watt).magnitude) * TFLOP / second, TFLOP / second, precision=2, commas=False)
```
...
class InfraFrontierNodeBudgetRecap:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    weights_gb = (params * BYTES_FP16.to(byte).magnitude * byte).to(GB).magnitude
    optimizer_gb = (params * BYTES_FP32.to(byte).magnitude * 2 * byte).to(GB).magnitude
    gradients_gb = weights_gb
    total_static_gb = weights_gb + optimizer_gb + gradients_gb
    gpu_hbm_gb = Hardware.Cloud.H100.memory.capacity.to(GiB).magnitude
    ...
    weights_gb_str = fmt_qty(weights_gb * GB, GB, precision=0, commas=True)
    optimizer_gb_str = fmt_qty(optimizer_gb * GB, GB, precision=0, commas=True)
    gradients_gb_str = fmt_qty(gradients_gb * GB, GB, precision=0, commas=True)
    total_static_tb_str = fmt_qty((total_static_gb / THOUSAND) * TB, TB, precision=1, commas=False)
    gpu_hbm_gb_str = fmt_qty(gpu_hbm_gb * GB, GB, precision=0, commas=False)
    node_hbm_gb_str = fmt_qty(node_hbm_gb * GB, GB, precision=0, commas=False)
```
...
class BandwidthStaircase:
    # ┌── 1. LOAD ──────────────────────────────────────────
    hbm_bw_gbs = Hardware.Cloud.H100.memory.bandwidth.to(GB/second).magnitude           # H100 HBM3
    nvlink_bw_gbs = Hardware.Cloud.H100.nvlink.bandwidth.to(GB/second).magnitude      # H100 NVLink
    ib_bw_gbs = Systems.Fabrics.InfiniBand_NDR.bandwidth.to(GB/second).magnitude # 400G NDR InfiniBand
    transfer_gb = 10
    # ┌── 2. EXECUTE ───────────────────────────────────────
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    transfer_gb_str = fmt_qty(transfer_gb * GB, GB, precision=0, commas=False)
    hbm_ms_str = fmt_time(round(hbm_ms), 'millisecond', precision=0, commas=False)
    nvlink_ms_str = fmt_time(round(nvlink_ms), 'millisecond', precision=0, commas=False)
    ib_ms_str = fmt_time(ib_ms, 'millisecond', precision=0, commas=False)
    bs_hbm_bw_str = fmt_qty(hbm_bw_gbs * GB / second, GB / second, precision=0, commas=True)
    bs_nvlink_bw_str = fmt_qty(nvlink_bw_gbs * GB / second, GB / second, precision=0, commas=False)
    bs_ib_bw_str = fmt_qty(ib_bw_gbs * GB / second, GB / second, precision=0, commas=False)
...
class InfraFrontierNodeTpRecap:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    activation_mb = microbatch * seq_len * hidden_dim * BYTES_FP16.to(byte).magnitude / MILLION
    gradient_allreduce_gb = (params * BYTES_FP16.to(byte).magnitude * byte).to(GB).magnitude
    ...
    activation_mb_str = fmt_qty_int(activation_mb * MB, MB, commas=False)
    gradient_allreduce_gb_str = fmt_qty(gradient_allreduce_gb * GB, GB, precision=0, commas=False)
```
...
class InfraFrontierNodeTableRecap:
    ...
    weights_gb = (params * BYTES_FP16.to(byte).magnitude * byte).to(GB).magnitude
    gradients_gb = weights_gb
    optimizer_m_gb = (params * BYTES_FP32.to(byte).magnitude * byte).to(GB).magnitude
    ...
    activation_range_gb_str = fmt_range(activation_min_gb, activation_max_gb, precision=0, commas=False)
    total_range_tb_str = fmt_range(total_min_tb, total_max_tb, precision=1, commas=False)
```
...
class InfraFrontierNodeWeightsRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_params_b_str = fmt(params / BILLION, precision=0, commas=False)
    weights_gb_str = fmt_qty(weights_gb * GB, GB, precision=0, commas=False)
    optimizer_gb_str = fmt_qty(optimizer_gb * GB, GB, precision=0, commas=True)
    gradients_gb_str = fmt_qty(gradients_gb * GB, GB, precision=0, commas=False)
    activation_range_gb_str = f"{fmt(activation_min_gb, precision=0, commas=False)}–{fmt(activation_max_gb, precision=0, commas=False)} GB"
    total_footprint_tb_str = f"{fmt(total_footprint_min_tb, precision=1, commas=False)}–{fmt(total_footprint_max_tb, precision=1, commas=False)} TB"
```
...
class NodeCapacityTrainMemRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    node_hbm_cap_str = fmt_qty(node_hbm_gb * GB, GB, precision=0, commas=False)
    host_dram_tb_str = fmt_qty(host_dram_tb * TB, TB, precision=0, commas=False)
    weights_gb_str = fmt_qty(weights_gb * GB, GB, precision=0, commas=True)
    gradients_gb_str = fmt_qty(gradients_gb * GB, GB, precision=0, commas=True)
    optimizer_gb_str = fmt_qty(optimizer_gb * GB, GB, precision=0, commas=True)
    total_dp_gb_str = fmt_qty(total_dp_gb * GB, GB, precision=0, commas=True)
    gpu_hbm_gb_str = fmt_qty(gpu_hbm_gb * GB, GB, precision=0, commas=False)
    zero3_per_gpu_gb_str = fmt_qty(zero3_per_gpu_gb * GB, GB, precision=1, commas=False)
    tp_weight_shard_gb_str = fmt_qty(tp_weight_shard_gb * GB, GB, precision=2, commas=False)
    optimizer_per_gpu_gb_str = fmt_qty(optimizer_per_gpu_gb * GB, GB, precision=1, commas=False)
    optimizer_per_tp_group_gb_str = fmt_qty(optimizer_per_tp_group_gb * GB, GB, precision=1, commas=False)
    weight_plus_optimizer_gb_str = fmt_qty(weight_plus_optimizer_gb * GB, GB, precision=1, commas=False)
...
class InfraFrontierNodeModelRecap:
    ...
    h100_mem_gb = Hardware.Cloud.H100.memory.capacity.to(GiB).magnitude
    host_ddr_tb = 2
    weight_gb = (Models.Language.GPT3.parameters * BYTES_FP16).to(GB).magnitude
    ...
    h100_mem_gb_str = fmt_qty(h100_mem_gb * GB, GB, precision=0, commas=False)
    total_hbm_gb_str = fmt_qty(h100_mem_gb * node_gpus * GB, GB, precision=0, commas=False)
    host_ddr_tb_str = fmt_qty(host_ddr_tb * TB, TB, precision=0, commas=False)
    weight_per_gpu_gb_str = fmt_qty(weight_per_gpu_gb * GB, GB, precision=2, commas=False)
    opt_state_gb_str = fmt_qty(opt_state_gb * GB, GB, precision=0, commas=False)
    act_gb_str = fmt_qty(act_gb * GB, GB, precision=0, commas=False)
    grad_gb_str = fmt_qty(grad_gb * GB, GB, precision=2, commas=False)
    buf_gb_str = fmt_qty(buf_gb * GB, GB, precision=0, commas=False)
    total_per_gpu_gb_str = fmt_qty_int(total_per_gpu_gb * GB, GB, commas=False)
    optimizer_pool_gb_str = fmt_qty(optimizer_pool_gb * GB, GB, precision=0, commas=True)
```
...
class InterconnectCommMath:
    ...
    gradient_buffer_gb_str = fmt_qty(gradient_buffer_gb * GB, GB, precision=0, commas=False)
```
...
class H100TdpRackRecap:
    # ┌── 1. LOAD ──────────────────────────────────────────
    gpu_tdp_w = Hardware.Cloud.H100.tdp.to(watt).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_str = fmt_qty(gpu_tdp_w * watt, watt, precision=0, commas=False)
    ...
    power_rate_gws_str = fmt_qty(power_rate_gws * ureg.gigawatt / second, ureg.gigawatt / second, precision=2, commas=False)
...
class PodTrainingMath:
    ...
    h100_fp8_tflops = Hardware.Cloud.H100.compute.precision_flops['fp8'].to(TFLOPs/second).magnitude
    h100_tflops_str = fmt_qty(h100_fp8_tflops * TFLOP / second, TFLOP / second, precision=0, commas=False)
...
class WscThroughput:
    ...
    per_gpu_inject_gbs = Systems.Fabrics.InfiniBand_NDR.bandwidth.to(GB/second).magnitude
    oversub_factor = 4                 # 4:1 fabric oversubscription scenario

    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    h100_fp16_tflops = Hardware.Cloud.H100.compute.peak_flops.to(TFLOPs / second).magnitude
    h100_fp8_tflops = Hardware.Cloud.H100.compute.precision_flops["fp8"].to(TFLOPs / second).magnitude
    ...
    wsc_inject_tbs_str = fmt_qty(wsc_inject_tbs * TB / second, TB / second, precision=0, commas=False)
    wsc_oversub_tbs_str = fmt_qty(wsc_oversub_tbs * TB / second, TB / second, precision=0, commas=False)
...
class CheckpointOverheadBudget:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    ckpt_total_tb_str                  = fmt_qty(checkpoint_tb * TB, TB, precision=2, commas=False)
    ckpt_cadence_min_str               = fmt_time(cadence_min, 'minute', precision=0, commas=False, style='word')
    ckpt_target_overhead_pct_str       = fmt_percent(target_overhead_pct/100, precision=0, commas=False, style='prose')
    ckpt_allowed_write_s_str           = fmt_time(allowed_write_s, 'second', precision=0, commas=False, style='word')
    ckpt_required_bw_gbs_str           = fmt_qty(required_bw_gbs * GB / second, GB / second, precision=1, commas=False)
    ckpt_reference_write_s_str         = fmt_time(reference_write_s, 'second', precision=0, commas=False, style='word')
    ckpt_reference_bw_gbs_str          = fmt_qty(reference_bw_gbs * GB / second, GB / second, precision=1, commas=False)
    ckpt_reference_overhead_pct_str    = fmt_percent(reference_overhead_pct/100, precision=0, commas=False, style='prose')
    ckpt_reference_min_cadence_min_str = fmt_time(reference_min_cadence_min, 'minute', precision=0, commas=False, style='word')
    ckpt_run_count_str                 = fmt(checkpoints_per_run, precision=0)
    ckpt_run_total_tb_str              = fmt_qty(run_total_tb * TB, TB, precision=0, commas=True)
    ckpt_recent_retention_tb_str       = fmt_qty(recent_retention_tb * TB, TB, precision=1, commas=False)
    ckpt_sustained_bw_gbs_str          = fmt_qty(sustained_bw_gbs * GB / second, GB / second, precision=1, commas=False)
...
class CheckpointComputeTradeoff:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_params_b_str = fmt(params / BILLION, precision=0, commas=False)
    training_state_tb_str = fmt_qty_int(training_state_tb * TB, TB, commas=False)
    write_bw_gbs_str = fmt_qty(write_bw_gbs * GB / second, GB / second, precision=0, commas=False)
    t_write_s_str = fmt_time(t_write_s, 'second', precision=0, commas=False, style='word')
...
class A100FallaciesRecap:
    ...
    option_b_hbm_gb = Hardware.Cloud.H100.memory.capacity.to(GiB).magnitude
    a100_bw_tbs = Hardware.Cloud.A100.memory.bandwidth.to(TB/second).magnitude
    h100_bw_tbs = Hardware.Cloud.H100.memory.bandwidth.to(TB/second).magnitude

    # ┌── 2. EXECUTE ───────────────────────────────────────
    weights_fp16_gb = (params * BYTES_FP16.to(byte).magnitude * byte).to(GB).magnitude
    weights_int8_gb = (params * BYTES_INT8.to(byte).magnitude * byte).to(GB).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mem_bw_tbs_str = fmt_qty(a100_bw_tbs * TB / second, TB / second, precision=2, commas=False)
    option_a_hbm_gb_str = fmt_qty(option_a_hbm_gb * GB, GB, precision=0, commas=False)
    weights_fp16_gb_str = fmt_qty_int(weights_fp16_gb * GB, GB, commas=False)
    option_a_lat_ms_str = fmt_time(option_a_lat_ms, 'millisecond', precision=1, commas=False)
    option_b_hbm_gb_str = fmt_qty(option_b_hbm_gb * GB, GB, precision=0, commas=False)
    weights_int8_gb_str = fmt_qty_int(weights_int8_gb * GB, GB, commas=False)
    option_b_lat_ms_str = fmt_time(round(option_b_lat_ms), 'millisecond', precision=0, commas=False)
...
class SummaryScaleScenario:
    ...
    gpu_power = (tdp * gpus_per_node).to(kW).magnitude * 4 # 4 nodes per rack
    ...
    h100_tflops_str = fmt_qty(flops, TFLOP/second, precision=0, commas=False)
    h100_bw_str = fmt_qty(bw, TB/second, precision=2, commas=False)
    h100_tdp_str = fmt_qty(tdp, watt, precision=0, commas=False)
    rack_power_str = fmt_qty(rack_power * kilowatt, kilowatt, precision=1, commas=False)
```
...
class TcoSummaryBreakevenRecap:
    ...
    node_power_kw = (Hardware.Cloud.H100.tdp * Systems.Nodes.DGX_H100.accelerators_per_node).to(kW).magnitude * 1.2
    denominator = (
