...
h100_tflops = InfraSetup.h100_tflops
h100_bw = InfraSetup.h100_bw
h100_ridge = InfraSetup.h100_ridge
gpt3_params_b = InfraSetup.gpt3_params_b
frontier_params_b = InfraSetup.frontier_params_b
nvlink_bw_str = InfraSetup.nvlink_bw_str
ib_bw_gbs = InfraSetup.ib_bw_gbs
ib_bw_str = InfraSetup.ib_bw_str
ddr_bw_str = InfraSetup.ddr_bw_str
t_mem_str = InfraSetup.t_mem_str
t_comp_str = InfraSetup.t_comp_str
dominance_str = InfraSetup.dominance_str
...
inf_flops_math = md_math(f"2 \\times {gpt3_params_b} \\times 10^9")
inf_throughput_math = md_math(f"{h100_bw} \\times 10^{{12}}")
...
pod_flops_math = md_math(f"6 \\times {gpt3_params_b} \\times 10^{{23}}")
total_flops_math = md_math(f"6 \\times {gpt3_params_b} \\times 10^{{23}}")
n_params_math = md_math(f"N = {gpt3_params_b} \\times 10^9")
compute_per_step_math = md_math(f"C = 6 \\times {gpt3_params_b} \\times 10^{{18}}")
t_compute_math = md_math(f"T_{{\\text{{compute}}}} \\approx 2.1")
t_comm_math = md_math(f"T_{{\\text{{comm}}}} \\approx 3.5")
cluster_throughput_math = md_math(f"8{{,}}192 \\times {h100_tflops} \\times 10^{{12}} \\times 0.45")
comm_time_formula_math = md_math(f"2 \\times (N-1)/N \\times 350 / {ib_bw_gbs}")
comm_approx_math = md_math(f"2 \\times 350 / {ib_bw_gbs}")
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
h100_tflops = H100PerformanceScenario.h100_tflops
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
ddr_bw_str = MemoryBandwidthScenario.ddr_bw_str
h100_bw = MemoryBandwidthScenario.h100_bw
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
gpt3_params_b = RooflineScenario.gpt3_params_b
h100_tflops = RooflineScenario.h100_tflops
h100_bw = RooflineScenario.h100_bw
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
rack_power_str = RackPowerScenario.rack_power_str
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
v100_ef = GpuEfficiencyScenario.v100_ef_str
a100_ef = GpuEfficiencyScenario.a100_ef_str
h100_ef = GpuEfficiencyScenario.h100_ef_str
b200_ef = GpuEfficiencyScenario.b200_ef_str
```
...
# │          GB, second)
# │ Exports: nvlink_bw_str, pcie_bw_str, ib_bw_gbs
# └─────────────────────────────────────────────────────────────────────────────
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
nvlink_bw_str = BandwidthHierarchyScenario.nvlink_bw_str
pcie_bw_str = BandwidthHierarchyScenario.pcie_bw_str
ib_bw_gbs = BandwidthHierarchyScenario.ib_bw_gbs
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
node_hbm_cap = NodeCapacityScenario.node_hbm_cap
h100_tflops = NodeCapacityScenario.h100_tflops
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
t_physics_hr_str = TrainingTime175B.t_physics_hr_str
t_comm_hr_str = TrainingTime175B.t_comm_hr_str
t_bubble_hr_str = TrainingTime175B.t_bubble_hr_str
t_ckpt_hr_str = TrainingTime175B.t_ckpt_hr_str
t_fail_hr_str = TrainingTime175B.t_fail_hr_str
t_maint_hr_str = TrainingTime175B.t_maint_hr_str
t_total_week_str = TrainingTime175B.t_total_week_str
```
...
3. **Idealized Training Time** (the "physics limit"):
   `{python} total_flops_math` / `{python} cluster_throughput_math` $\times 10^{18}$ $\approx$ **`{python} t_physics_hr_str` hours**

4. **Real-World Multipliers**:
   - Communication overhead (scaling efficiency $\approx 0.85$): $\div 0.85 \rightarrow$ **`{python} t_comm_hr_str` hours**
   - Pipeline bubbles ($\approx 5\%$): $\times$ $1.05$ $\rightarrow$ **`{python} t_bubble_hr_str` hours**
   - Checkpoint overhead ($\approx 3\%$): $\times$ $1.03$ $\rightarrow$ **`{python} t_ckpt_hr_str` hours**
   - Hardware failures and restarts ($\approx 10\%$ of wall time): $\times$ $1.10$ $\rightarrow$ **`{python} t_fail_hr_str` hours**
   - Maintenance windows ($\approx 5\%$): $\times$ $1.05$ $\rightarrow$ **`{python} t_maint_hr_str` hours**

**Total**: ~18 hours (system minimum) $\rightarrow$ 2--3 days (typical operations) $\rightarrow$ **`{python} t_total_week_str` weeks** (conservative schedule with operational margin). The gap between the 12-hour physics limit and the practical `{python} t_total_week_str` week schedule is a factor of 15--25$\times$. Recovering this lost time is the domain of distributed systems engineering -- the subject of @sec-distributed-training-systems.
...
util_pct_str = TcoScenario.util_pct_str
breakeven_util = TcoScenario.breakeven_util
```
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
gpt3_params_b = SummaryScaleScenario.gpt3_params_b
h100_tflops = SummaryScaleScenario.h100_tflops
h100_bw = SummaryScaleScenario.h100_bw
