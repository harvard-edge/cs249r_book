...
| **Bandwidth**       | ~`{python} MemoryBandwidthScenario.ddr_bw_str` GB/s | **~`{python} MemoryBandwidthScenario.h100_bw` TB/s** |   **~50$\times$ Faster**  |
...
1. **Compute Time**: Each token requires $2 \times 70\text{B} = 1.4 \times 10^{11}$ floating-point operations. At `{python} H100PerformanceScenario.h100_tflops` TFLOPS peak throughput, the arithmetic takes:
...
1.  **Significance (Quantitative):** It defines the **Hardware Efficiency Threshold**. Workloads with an intensity below the ridge point are **Bandwidth-Bound** ($BW$), while those above are **Compute-Bound** ($R_{\text{peak}}$).
...
3.  **Common Pitfall:** A frequent misconception is that all cluster communication is equal. In reality, parallelism strategies must be **Hierarchy-Aware**: placing high-frequency synchronization (for example, Tensor Parallelism) on a slow tier will rapidly idle the compute units ($R_{\text{peak}}$), collapsing system efficiency ($\eta$).
...
The total exceeds the 80 GB HBM capacity. **Solution**: offload optimizer states to host DDR5. The 2 TB of DDR5 can hold the full 1,400 GB of optimizer state with room to spare.
...
- **IT Power**: $1,000\times700$ W = 700 kW
...
1. **Total FLOPs**: Using the approximation $6 \times N \times P$, where `{python} PodTrainingMath.n_params_math` and $P = 300 \times 10^9$ tokens:
...
**Setup**: Training a `{python} InfraSetup.frontier_params_b`B model on a DGX H100 cluster with `{python} InfraSetup.ib_bw_str` Gbps InfiniBand per GPU.
...
- **Annual Electricity**: 10,000 GPUs$\times$ 700 W$\times$ PUE $1.1\times8,760$ h$\times$ \$0.07/kWh = \$4.7M/year
...
**Break-Even**: On-premises saves approximately \$273M over three years at 80 percent utilization. The savings disappear if utilization drops below ~40 percent, because the on-premises hardware still incurs facility and staffing costs regardless of load.
