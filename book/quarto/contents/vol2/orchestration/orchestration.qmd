...
# Orchestration and Resource Management {#sec-orchestration-resource-management-resource-management}
...
## Introduction {#sec-orchestration-resource-management-resource-management-introduction-ed23}

Orchestration decisions span multiple layers of the system stack, from physical hardware topology to high-level scheduling policy. Understanding where orchestration fits in this stack clarifies both its responsibilities and its constraints. The *connection to the Systems Sandwich* framework makes this layered relationship explicit.
...
## The Scheduler's Challenge {#sec-orchestration-resource-management-resource-management-schedulers-challenge-a316}

Cluster schedulers must solve complex optimization problems under tight constraints. This section examines the core algorithmic challenges: fitting heterogeneous workloads onto fixed node configurations and ensuring that distributed training jobs can start atomically without causing deadlocks.

### Bin Packing at Scale {#sec-orchestration-resource-management-resource-management-bin-packing-scale-91d2}
...
### Gang Scheduling {#sec-orchestration-resource-management-resource-management-gang-scheduling-424b}
...
## Orchestration Paradigms {#sec-orchestration-resource-management-resource-management-orchestration-paradigms-e21b}

Two dominant philosophies have emerged for cluster orchestration: the batch-oriented model from high-performance computing and the service-oriented model from cloud infrastructure. Each brings distinct trade-offs for ML workloads.

### Slurm: The HPC Heritage {#sec-orchestration-resource-management-resource-management-slurm-hpc-heritage-ffb4}
...
### Kubernetes: The Cloud Native Standard {#sec-orchestration-resource-management-resource-management-kubernetes-cloud-native-standard-16fa}
...
## Resource Efficiency {#sec-orchestration-resource-management-resource-management-resource-efficiency-dec4}

Maximizing cluster utilization while maintaining fairness across users requires sophisticated resource management policies. This section covers quota systems for multi-tenant environments and preemption mechanisms that balance interactive and batch workloads.

### Multi-Tenancy and Quotas {#sec-orchestration-resource-management-resource-management-multitenancy-quotas-66bf}
...
### Preemption and Priority {#sec-orchestration-resource-management-resource-management-preemption-priority-c7ed}
...
### Why Distributed Scheduling is Hard {#sec-orchestration-resource-management-resource-management-distributed-scheduling-hard-0b85}
...
## Batch Scheduling for Training {#sec-orchestration-resource-management-resource-management-batch-scheduling-training-bb5b}
...
#### Slurm for HPC-Style ML {#sec-orchestration-resource-management-resource-management-slurm-hpcstyle-ml-1804}
...
#### Kubernetes for ML Workloads {#sec-orchestration-resource-management-resource-management-kubernetes-ml-workloads-f83e}
...
#### Custom ML Schedulers {#sec-orchestration-resource-management-resource-management-custom-ml-schedulers-a33f}
...
Locality-aware scheduling recognizes that communication topology matters for distributed training. A 64-GPU job performs better on 8 nodes of 8 GPUs each than 16 nodes of 4 GPUs, due to higher NVLink bandwidth within nodes. Advanced schedulers consider the fat-tree topology discussed in @sec-networking-networking-largescale-ml-adca, preferring allocations that share fewer switch hops. Experiments show 15 to 30 percent training throughput improvement from topology-aware placement.

Despite advances in scheduling algorithms, real clusters routinely underperform their theoretical utilization targets. Diagnosing why requires examining the full system stack.
...
```{python}
#| label: cluster-value-calc
#| echo: false

# Cluster utilization value recovery
cluster_gpus = 1000  # approximate total cluster size
utilization_improvement_pct = 40
effective_gpus_recovered = cluster_gpus * utilization_improvement_pct / 100
gpu_hour_cost = 2  # dollars
hours_per_year = 8760
annual_value = effective_gpus_recovered * gpu_hour_cost * hours_per_year
annual_value_m = annual_value / 1e6

effective_gpus_str = f"{int(effective_gpus_recovered)}"
annual_value_m_str = f"{annual_value_m:.0f}"
```

This 40% improvement in effective cluster capacity equals approximately `{python} effective_gpus_str` additional GPUs worth of productive work. At $2 per GPU-hour, this represents \$`{python} annual_value_m_str` million in annual recovered value, achieved through policy changes requiring zero additional hardware investment.
...
## Online Serving Resource Management {#sec-orchestration-resource-management-resource-management-online-serving-resource-management-d5e6}
...
#### Autoscaling for Inference {#sec-orchestration-resource-management-resource-management-autoscaling-inference-554c}
...
LLM inference requires specialized scaling due to the key-value cache[^fn-kv-cache]. A 70B parameter model serving long-context requests may require 80GB+ of GPU memory for KV cache alone, even with PagedAttention optimizations (detailed in @sec-inference-scale-kv-cache-management-04ed). Scaling decisions must account for both request rate and context length distribution.
...
#### Resource Isolation {#sec-orchestration-resource-management-resource-management-resource-isolation-0a3d}
...
## Multi-Tenancy Considerations {#sec-orchestration-resource-management-resource-management-multitenancy-considerations-a540}
...
#### Quota Management {#sec-orchestration-resource-management-resource-management-quota-management-d922}
...
#### Security Isolation {#sec-orchestration-resource-management-resource-management-security-isolation-59c0}
...
Consider these essential insights from this chapter:
...
**Part III: Deployment at Scale** begins with @sec-inference-scale, where we examine the challenges of serving these models to millions of users with low latency and high throughput.
