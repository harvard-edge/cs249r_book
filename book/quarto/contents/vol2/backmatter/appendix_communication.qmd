...
- **When pipeline bubbles dominate**: Use @sec-comm-foundations-pipeline-arithmetic to compute bubble fraction and determine the required microbatch count.
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ COMMUNICATION FOUNDATIONS—MASTER COMPUTATION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: PERSISTENT—All values used throughout the Communication Foundations
# │          appendix: α-β model table, ring vs. tree AllReduce worked example,
# │          pipeline bubble table, gradient compression break-even, and all prose.
# │
# │ Goal: Provide all communication cost model constants in one place.
# │ How: pint Quantities from mlsysim.core.constants; calc_* AllReduce/Pipeline from formulas.py.
# │
# │ Imports: mlsysim.core.constants (*), mlsysim.formulas (calc_*), mlsysim.book (*)
# │ Exports: CF = CommFoundations (accessed as CF.attribute)
# └─────────────────────────────────────────────────────────────────────────────
...
from mlsysim.core.constants import (
    ...
    GB, KB, byte, second, NS, BILLION, MILLION, THOUSAND,
    BYTES_FP32, BYTES_FP16, MICROSECOND,
    ureg,
)
...
class CommFoundations:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    # Interconnect Specs (Level 1)
    ib_ndr_alpha_us = IB_NDR_LATENCY_US
    ib_ndr_beta_gbs = INFINIBAND_NDR_BW_GBS
    ...
    nvlink_h100_bw_gbs = int(NVLINK_H100_BW.m_as(GB/second))
    pcie_gen5_bw_gbs = int(PCIE_GEN5_BW.m_as(GB/second))
    nvlink_latency_ns = int(LATENCY_NVLINK.m_as(NS))
    pcie_latency_ns = int(LATENCY_PCIE_GEN5.m_as(NS))

    # Collective Params (Level 2)
    ring_n_gpus = 256
    ring_msg_bytes = BILLION # 1 GB

    # Pipeline Params (Level 3)
    p_stages = 8
    m_microbatches = 16
    v_interleave = 4
    ...
    # Step 1: Level 4: Ratios & Efficiency
    ib_ndr_alpha_s = (ib_ndr_alpha_us * MICROSECOND).m_as(second)
    ib_ndr_beta_bs = (ib_ndr_beta_gbs * GB / second).m_as(byte / second)
    crossover_kb_val = (ib_ndr_alpha_s * ib_ndr_beta_bs * byte).m_as(KB)

    # Step 2: AllReduce Comparison (formulas return Quantity; use magnitude for scalar ms)
    ring_time_s = calc_ring_allreduce_time(ring_msg_bytes, ring_n_gpus, ib_ndr_beta_bs, ib_ndr_alpha_s)
    tree_time_s = calc_tree_allreduce_time(ring_msg_bytes, ring_n_gpus, ib_ndr_beta_bs, ib_ndr_alpha_s)
    ring_time_ms = ring_time_s.m_as(ureg.ms)
    tree_time_ms = tree_time_s.m_as(ureg.ms)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # (Pre-computed formatted strings for appendix tables)
    ...
    crossover_kb_str = f"{round(crossover_kb_val)}"
    ...
    ring_msg_gb_str = f"{(ring_msg_bytes * byte).m_as(GB):.0f}"

    bubble_std_pct = f"{bubble_std * 100:.1f}"
    bubble_interleaved_pct = f"{bubble_interleaved * 100:.1f}"
    bubble_p4_m8_pct = f"{bubble_p4_m8 * 100:.1f}"
    bubble_p4_m16_pct = f"{bubble_p4_m16 * 100:.1f}"
    bubble_p4_m32_pct = f"{bubble_p4_m32 * 100:.1f}"
    bubble_p4_m64_pct = f"{bubble_p4_m64 * 100:.1f}"
    bubble_p8_m8_pct = f"{bubble_p8_m8 * 100:.1f}"
    bubble_p8_m16_pct = f"{bubble_p8_m16 * 100:.1f}"
    bubble_p8_m32_pct = f"{bubble_p8_m32 * 100:.1f}"
    bubble_p8_m64_pct = f"{bubble_p8_m64 * 100:.1f}"

    compress_ratio_str = f"{int(compress_ratio)}"
    compress_overhead_ms_str = fmt(compress_overhead_ms, precision=1, commas=False)
    uncompressed_ms_str = fmt(ring_time_ms, precision=1, commas=False)
...
Estimating *how* long a gradient synchronization will take across `{python} CF.ring_n_str` GPUs must happen before committing to a cluster topology. The $\alpha$-$\beta$ model is the "Roofline for networks": it gives a first-order prediction of transfer time from just two hardware parameters and the message size.
:::
...
The **$\alpha$-$\beta$ model**, given by @eq-alpha-beta, predicts the time to transfer a message of $M$ bytes:
...
The model decomposes every transfer into exactly two costs: a per-message tax ($\alpha$) paid regardless of size, and a per-byte tax ($M/\beta$) that scales linearly with the payload. This decomposition drives all the algorithm selection decisions in @sec-comm-foundations-algorithm-selection.
...
|:------------------------------|:-----------------------------------------|:---------------------------------------|:---------------------------------|
| **NVLink 4.0 (H100)**         | `{python} CF.nvlink_lat_str` ns          | `{python} CF.nvlink_bw_str` GB/s       | Intra-node GPU-to-GPU            |
| **PCIe Gen5**                 | `{python} CF.pcie_lat_str` ns            | `{python} CF.pcie_bw_str` GB/s         | CPU-GPU, NIC-GPU                 |
| **InfiniBand NDR (400 Gbps)** | `{python} CF.ib_ndr_alpha_us_str` $\mu$s | `{python} CF.ib_ndr_beta_gbs_str` GB/s | Inter-node (high-end clusters)   |
| **InfiniBand HDR (200 Gbps)** | `{python} CF.ib_hdr_alpha_us_str` $\mu$s | `{python} CF.ib_hdr_beta_gbs_str` GB/s | Inter-node (previous generation) |
| **RoCE v2 (100 GbE)**         | `{python} CF.roce_alpha_us_str` $\mu$s   | `{python} CF.roce_beta_gbs_str` GB/s   | Inter-node (Ethernet clusters)   |
| **TCP/IP (Ethernet)**         | `{python} CF.tcp_alpha_us_str` $\mu$s    | Varies                                 | Control plane, non-RDMA fallback |
...
**Setup**: InfiniBand NDR has $\alpha$ = `{python} CF.ib_ndr_alpha_us_str` $\mu$s and $\beta$ = `{python} CF.ib_ndr_beta_gbs_str` GB/s.

**Question**: At what message size does the bandwidth term equal the latency term?

**Calculation**:

$M_{\text{cross}} = \alpha \times \beta =$ `{python} CF.ib_ndr_alpha_us_str` $\times 10^{-6}$ s $\times$ `{python} CF.ib_ndr_beta_gbs_str` $\times 10^{9}$ B/s $=$ `{python} CF.crossover_kb_str` KB

**Interpretation**: Messages smaller than `{python} CF.crossover_kb_str` KB are latency-dominated on IB NDR. Gradient tensors from a single layer of a large model are typically 10--100 MB---well above this threshold. Control messages, heartbeats, and barrier synchronizations, however, are well below it.

**Implication**: This is why NCCL and Gloo batch small gradient tensors into larger "buckets" before launching AllReduce. Sending each tensor individually would pay the `{python} CF.ib_ndr_alpha_us_str` $\mu$s startup cost hundreds of times per iteration instead of a handful.
...
Tree AllReduce, expressed in @eq-tree-allreduce:
...
**Setup**: `{python} CF.ring_n_str` H100 GPUs connected via InfiniBand NDR ($\alpha$ = `{python} CF.ib_ndr_alpha_us_str` $\mu$s, $\beta$ = `{python} CF.ib_ndr_beta_gbs_str` GB/s). The gradient tensor is `{python} CF.ring_msg_gb_str` GB (a 500M-parameter model in FP16).

**Question**: *How* long does AllReduce take with ring vs. tree?

**Ring AllReduce**:

$T_{\text{ring}} \approx$ **`{python} CF.ring_time_ms_str` ms**

**Tree AllReduce**:

$T_{\text{tree}} \approx$ **`{python} CF.tree_time_ms_str` ms**

**Interpretation**: For this 1 GB message, ring AllReduce takes `{python} CF.ring_time_ms_str` ms while tree takes `{python} CF.tree_time_ms_str` ms---the ring is significantly faster because it distributes the bandwidth load across all links. The tree's logarithmic latency advantage is negligible compared to its bandwidth penalty for large messages.
...
|:------------------------------------------------------|:---------------------------------------|:-------------------------------------|:---------------------------------------------------|
| **Large messages ($M > 10 \times M_{\text{cross}}$)** | Best---optimal bandwidth utilization   | Poor---sends $M$ at every tree level | Good---near-optimal bandwidth, $O(\log N)$ latency |
| **Small messages ($M < M_{\text{cross}}/10$)**        | Poor---$2(N-1)$ startup costs dominate | Best---$\log_2 N$ startup costs      | Good---$O(\log N)$ startup costs                   |
| **Medium messages ($M \approx M_{\text{cross}}$)**    | Acceptable                             | Acceptable                           | Best---balances both terms                         |
...
Modern clusters have a two-level topology: GPUs within a node are connected by NVLink (`{python} CF.nvlink_bw_str` GB/s), while nodes are connected by InfiniBand (`{python} CF.ib_ndr_beta_gbs_str` GB/s). Hierarchical AllReduce exploits this asymmetry in three phases:

1. **Intra-node reduce** (NVLink): Each node reduces its 8 local GPUs to a single result using NVLink's `{python} CF.nvlink_bw_str` GB/s bandwidth. This is fast because NVLink is 18$\times$ faster than InfiniBand NDR.
...
Rule of thumb: Keeping bubble overhead below 20 percent requires $M \geq 4(P - 1)$, which means $M \geq 4P$ for practical purposes.
...
|:--------------------------|:--------------------------------|:---------------------------------|:---------------------------------|:---------------------------------|
| **$P$ = 4**               | `{python} CF.bubble_p4_m8_pct`% | `{python} CF.bubble_p4_m16_pct`% | `{python} CF.bubble_p4_m32_pct`% | `{python} CF.bubble_p4_m64_pct`% |
| **$P$ = 8**               | `{python} CF.bubble_p8_m8_pct`% | `{python} CF.bubble_p8_m16_pct`% | `{python} CF.bubble_p8_m32_pct`% | `{python} CF.bubble_p8_m64_pct`% |
...
The denominator grows by a factor of $V$ compared to @eq-pipeline-bubble. For $P$ = 8, $M$ = 16, and $V$ = 4, the bubble fraction drops from `{python} CF.bubble_std_pct` percent to `{python} CF.bubble_interleaved_pct` percent---a dramatic improvement that comes at the cost of more frequent, smaller communication between stages.
...
Compression is beneficial when $T_{\text{compressed}} < T_{\text{comm}}(M)$. Rearranging this inequality yields the break-even condition in @eq-compression-breakeven:
...
**Setup**: Ring AllReduce of a `{python} CF.ring_msg_gb_str` GB gradient across `{python} CF.ring_n_str` H100 GPUs on InfiniBand NDR, taking `{python} CF.uncompressed_ms_str` ms uncompressed. A Top-k sparsification scheme achieves `{python} CF.compress_ratio_str`$\times$ compression with `{python} CF.compress_overhead_ms_str` ms total encode + decode overhead.

**Question**: Does compression reduce the total AllReduce time?

**Calculation**:

- Uncompressed AllReduce: `{python} CF.uncompressed_ms_str` ms
- Compressed communication: `{python} CF.compressed_comm_ms_str` ms
- Total with compression: `{python} CF.compressed_total_ms_str` ms
- Speedup: `{python} CF.compress_speedup_str`$\times$

**Interpretation**: Compression delivers a `{python} CF.compress_speedup_str`$\times$ speedup in communication time. The `{python} CF.compress_overhead_ms_str` ms codec overhead is a small fraction of the `{python} CF.uncompressed_ms_str` ms saved.
...
For ring AllReduce, the bandwidth term is $2(N-1)/N \cdot M/\beta$, which does scale inversely with $\beta$. The latency term $2(N-1) \cdot \alpha$, however, is independent of bandwidth. At 256 GPUs with small messages, the latency term can dominate, and doubling bandwidth yields negligible improvement. Always check the current regime before investing in faster interconnects.
...
Pipeline parallelism replaces AllReduce with point-to-point activation transfers between adjacent stages, which are indeed cheaper. It introduces, however, bubble overhead that is mathematically unavoidable: with $P$ stages, at least $(P-1)/(P-1+M)$ of GPU-time is idle. For deep pipelines ($P \geq 8$), this overhead requires $M \geq 32$ microbatches to stay below 20 percent---a constraint that affects batch size, memory, and convergence.
...
- **Pipeline bubble fraction** $b = (P-1)/(P-1+M)$ is the fundamental overhead of pipeline parallelism. The practical rule $M \geq 4P$ keeps bubbles below 20 percent, and interleaved scheduling with $V$ virtual stages reduces bubbles by a factor of $V$.
...
- Every communication cost model in this appendix derives from two hardware parameters ($\alpha$ and $\beta$) and two workload parameters ($M$ and $N$). Measuring these four numbers for a cluster yields a complete first-order prediction of distributed training overhead.
