...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ COMMUNICATION FOUNDATIONS — MASTER COMPUTATION
# ├─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Goal: Provide all communication cost model constants in one place for the
# │       quantitative reference appendix on collective operations and pipeline overhead.
# │ Show: See individual section prose for formatted values. This cell provides
# │       the physics; string attributes convert to display-ready form.
# │ How: pint Quantities from mlsys.constants; calc_ring_allreduce_time,
# │      calc_tree_allreduce_time, calc_pipeline_bubble from formulas.py.
# │
# │ Imports: mlsys.constants (INFINIBAND_NDR_BW_GBS, IB_NDR_LATENCY_US, NVLINK_H100_BW, …)
# │          mlsys.formulas (calc_ring_allreduce_time, calc_tree_allreduce_time, calc_pipeline_bubble)
# │          mlsys.formatting (fmt, check, md_math)
# │ Exports: CF = CommFoundations (accessed as CF.attribute), plus module-level aliases below
# └─────────────────────────────────────────────────────────────────────────────
...
class CommFoundations:
    ...
    ring_time_ms = ring_time_s.m_as(ureg.millisecond)

    # Tree AllReduce same scenario
    tree_time_s = calc_tree_allreduce_time(
        ring_msg_bytes, ring_n_gpus, ib_ndr_beta_bs, ib_ndr_alpha_s
    )
    tree_time_ms = tree_time_s.m_as(ureg.millisecond)
