...
# │ How: pint Quantities from mlsys.constants; calc_* AllReduce/Pipeline from formulas.py.
# │
...
$$M_{\text{cross}} = \alpha \times \beta = `{python} CF.ib_ndr_alpha_us_str` \times 10^{-6} \text{ s} \times `{python} CF.ib_ndr_beta_gbs_str` \times 10^{9} \text{ B/s} = `{python} CF.crossover_kb_str` \text{ KB}$$
...
$$T_{\text{ring}} \approx \mathbf{`{python} CF.ring_time_ms_str` \text{ ms}}$$

**Tree AllReduce**:

$$T_{\text{tree}} \approx \mathbf{`{python} CF.tree_time_ms_str` \text{ ms}}$$
