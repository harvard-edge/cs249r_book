...
+:====================+:==================================+:============================+:==================================+
| **NVLink 4.0**      | `{python} nvlink_lat_str` ns      | `{python} nvlink_bw_str`    | Intra-node GPU-to-GPU             |
| (H100)              |                                   | GB/s                        |                                   |
+---------------------+-----------------------------------+-----------------------------+-----------------------------------+
| **PCIe Gen5**       | `{python} pcie_lat_str` ns        | `{python} pcie_bw_str`      | CPU-GPU, NIC-GPU                  |
|                     |                                   | GB/s                        |                                   |
+---------------------+-----------------------------------+-----------------------------+-----------------------------------+
| **InfiniBand NDR**  | `{python} ib_ndr_alpha_us_str`    | `{python}                   | Inter-node (high-end clusters)    |
| (400 Gbps)          | $\mu$s                            | ib_ndr_beta_gbs_str` GB/s   |                                   |
+---------------------+-----------------------------------+-----------------------------+-----------------------------------+
| **InfiniBand HDR**  | `{python} ib_hdr_alpha_us_str`    | `{python}                   | Inter-node (previous generation)  |
| (200 Gbps)          | $\mu$s                            | ib_hdr_beta_gbs_str` GB/s   |                                   |
+---------------------+-----------------------------------+-----------------------------+-----------------------------------+
| **RoCE v2**         | `{python} roce_alpha_us_str`      | `{python}                   | Inter-node (Ethernet clusters)    |
| (100 GbE)           | $\mu$s                            | roce_beta_gbs_str` GB/s     |                                   |
+---------------------+-----------------------------------+-----------------------------+-----------------------------------+
| **TCP/IP**          | `{python} tcp_alpha_us_str`       | Varies                      | Control plane, non-RDMA fallback  |
| (Ethernet)          | $\mu$s                            |                             |                                   |
+---------------------+-----------------------------------+-----------------------------+-----------------------------------+
...
+:====================+:============================================+:==========================+:=============================+
| **AllReduce**       | $2 \cdot \frac{N-1}{N} \cdot \frac{M}      | $2(N-1) \cdot \alpha$     | Data-parallel gradient sync  |
| (Ring)              | {\beta}$                                    |                           |                              |
+---------------------+---------------------------------------------+---------------------------+------------------------------+
| **AllReduce**       | $2\log_2 N \cdot \frac{M}{\beta}$           | $2\log_2 N \cdot \alpha$  | Small-message sync           |
| (Tree)              |                                             |                           |                              |
+---------------------+---------------------------------------------+---------------------------+------------------------------+
| **AllGather**       | $\frac{N-1}{N} \cdot \frac{M}{\beta}$       | $(N-1) \cdot \alpha$      | FSDP parameter               |
| (Ring)              |                                             |                           | reconstruction               |
+---------------------+---------------------------------------------+---------------------------+------------------------------+
| **ReduceScatter**   | $\frac{N-1}{N} \cdot \frac{M}{\beta}$       | $(N-1) \cdot \alpha$      | FSDP / ZeRO gradient         |
| (Ring)              |                                             |                           | sharding                     |
+---------------------+---------------------------------------------+---------------------------+------------------------------+
| **AllToAll**        | $\frac{N-1}{N} \cdot \frac{M}{\beta}$       | $(N-1) \cdot \alpha$      | MoE expert routing           |
| (Ring)              |                                             |                           |                              |
+---------------------+---------------------------------------------+---------------------------+------------------------------+
| **Broadcast**       | $\frac{M}{\beta}$                           | $\log_2 N \cdot \alpha$   | Weight distribution,         |
| (Tree)              |                                             |                           | config sync                  |
+---------------------+---------------------------------------------+---------------------------+------------------------------+
...
+:==================+:=============+:=============+:=============+:=============+
| **$P$ = 4**       | `{python}    | `{python}    | ---          | ---          |
|                   | bubble_4_8   | bubble_4_16  |              |              |
|                   | _pct`%       | _pct`%       |              |              |
+-------------------+--------------+--------------+--------------+--------------+
| **$P$ = 8**       | ---          | `{python}    | `{python}    | `{python}    |
|                   |              | bubble_8_16  | bubble_8_32  | bubble_8_64  |
|                   |              | _pct`%       | _pct`%       | _pct`%       |
+-------------------+--------------+--------------+--------------+--------------+
