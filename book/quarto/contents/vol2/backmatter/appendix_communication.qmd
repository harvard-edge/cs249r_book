...
CF = CommFoundations
ib_ndr_alpha_us_str = CF.ib_ndr_alpha_us_str
ib_ndr_beta_gbs_str = CF.ib_ndr_beta_gbs_str
ib_hdr_alpha_us_str = CF.ib_hdr_alpha_us_str
ib_hdr_beta_gbs_str = CF.ib_hdr_beta_gbs_str
roce_alpha_us_str = CF.roce_alpha_us_str
roce_beta_gbs_str = CF.roce_beta_gbs_str
tcp_alpha_us_str = CF.tcp_alpha_us_str
nvlink_bw_str = CF.nvlink_bw_str
pcie_bw_str = CF.pcie_bw_str
nvlink_lat_str = CF.nvlink_lat_str
pcie_lat_str = CF.pcie_lat_str
crossover_kb_str = CF.crossover_kb_str
ring_n_str = CF.ring_n_str
ring_time_ms_str = CF.ring_time_ms_str
tree_time_ms_str = CF.tree_time_ms_str
ring_msg_gb_str = CF.ring_msg_gb_str
bubble_std_pct = CF.bubble_std_pct
bubble_interleaved_pct = CF.bubble_interleaved_pct
compress_ratio_str = CF.compress_ratio_str
compress_overhead_ms_str = CF.compress_overhead_ms_str
uncompressed_ms_str = CF.uncompressed_ms_str
compressed_comm_ms_str = CF.compressed_comm_ms_str
compressed_total_ms_str = CF.compressed_total_ms_str
compress_speedup_str = CF.compress_speedup_str
```

## The $\alpha$-$\beta$ Communication Model {#sec-comm-foundations-alpha-beta}

::: {.callout-perspective title="Why This Matters"}
You need to estimate how long a gradient synchronization will take across `{python} ring_n_str` GPUs before you commit to a cluster topology. The $\alpha$-$\beta$ model is the "Roofline for networks": it gives you a first-order prediction of transfer time from just two hardware parameters and the message size.
:::
...
$$M_{\text{cross}} = \alpha \times \beta = `{python} ib_ndr_alpha_us_str` \times 10^{-6} \text{ s} \times `{python} ib_ndr_beta_gbs_str` \times 10^{9} \text{ B/s} = `{python} crossover_kb_str` \text{ KB}$$
...
$$T_{\text{ring}} \approx \mathbf{`{python} ring_time_ms_str` \text{ ms}}$$

**Tree AllReduce**:

$$T_{\text{tree}} \approx \mathbf{`{python} tree_time_ms_str` \text{ ms}}$$

**Interpretation**: For this 1 GB message, ring AllReduce takes `{python} ring_time_ms_str` ms while tree takes `{python} tree_time_ms_str` ms---the ring is significantly faster because it distributes the bandwidth load across all links. The tree's logarithmic latency advantage is negligible compared to its bandwidth penalty for large messages.
...
|:--------------------------|:---------------------------|:---------------------------|:---------------------------|:---------------------------|
| **$P$ = 4**               | `{python} bubble_std_pct`% | `{python} bubble_std_pct`% | ---                        | ---                        |
| **$P$ = 8**               | ---                        | `{python} bubble_std_pct`% | `{python} bubble_std_pct`% | `{python} bubble_std_pct`% |
...
The denominator grows by a factor of $V$ compared to @eq-pipeline-bubble. For $P$ = 8, $M$ = 16, and $V$ = 4, the bubble fraction drops from `{python} bubble_std_pct`% to `{python} bubble_interleaved_pct`%---a dramatic improvement that comes at the cost of more frequent, smaller communication between stages.
...
- Compressed communication: `{python} compressed_comm_ms_str` ms
- Total with compression: `{python} compressed_total_ms_str` ms
- Speedup: `{python} compress_speedup_str`$\times$
