...
You need to estimate *how* long a gradient synchronization will take across `{python} CF.ring_n_str` GPUs before you commit to a cluster topology. The $\alpha$-$\beta$ model is the "Roofline for networks": it gives you a first-order prediction of transfer time from just two hardware parameters and the message size.
:::
...
Rule of thumb: To keep bubble overhead below 20 percent, you need $M \geq 4(P - 1)$, which means $M \geq 4P$ for practical purposes.
