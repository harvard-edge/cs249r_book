...
Training introduces a unique memory challenge known as the **Activation Wave**. During the forward pass, every intermediate activation must be stored in memory to be used later during the backward pass to compute gradients. This creates a memory footprint that scales linearly with model depth and batch size, often exceeding the storage available for model weights by 3$\times$--5$\times$. This "memory wall" is the primary driver for distributed strategies like **Activation Checkpointing** and **ZeRO**.
