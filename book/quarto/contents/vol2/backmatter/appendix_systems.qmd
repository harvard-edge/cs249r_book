...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \tikzset{
    layer/.style={draw=black!60, thick, rounded corners=1pt, minimum width=3.5cm, minimum height=0.6cm, align=center}
  }

  % Single Machine
  \begin{scope}
    \node[anchor=south] at (1.75, 3.5) {\textbf{A. Single-Machine Stack}};
    \node[layer, fill=red!10] (S4) at (1.75, 3) {Application};
    \node[layer, fill=orange!10] (S3) at (1.75, 2.3) {ML Framework};
    \node[layer, fill=blue!10] (S2) at (1.75, 1.6) {Operating System};
    \node[layer, fill=gray!10] (S1) at (1.75, 0.9) {\textbf{Hardware (PCIe/HBM)}};
    \node[font=\tiny, text=red!80!black] at (1.75, 0.2) {\textit{Bottleneck: Memory Wall}};
  \end{scope}

  % The Fleet
  \begin{scope}[shift={(5,0)}]
    \node[anchor=south] at (1.75, 3.5) {\textbf{B. Fleet Stack}};
    \node[layer, fill=GreenLine!20] (F4) at (1.75, 3) {Governance};
    \node[layer, fill=accentcolor!40] (F3) at (1.75, 2.3) {Serving};
    \node[layer, fill=accentcolor!25] (F2) at (1.75, 1.6) {Distribution};
    \node[layer, fill=accentcolor!10] (F1) at (1.75, 0.9) {\textbf{Infra (Fabric/RDMA)}};
    \node[font=\tiny, text=accentcolor] at (1.75, 0.2) {\textit{Bottleneck: Bisection Wall}};
  \end{scope}
  
  \draw[->, ultra thick, gray!40] (3.6, 1.95) -- (4.9, 1.95) node[midway, above, font=\tiny, text=black] {Scaling};
