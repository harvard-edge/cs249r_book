...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, >=stealth]
  ...
  \begin{scope}[local bounding box=single]
    ...
    \node[layer, fill=BlueL, above=0.1cm of S1] (S2) {Operating System};
    \node[layer, fill=OrangeL, above=0.1cm of S2] (S3) {ML Framework};
    \node[layer, fill=RedL, above=0.1cm of S3] (S4) {Application};
  ...
  \begin{scope}[local bounding box=fleet]
    ...
    \node[layer, fill=BlueL, above=0.1cm of F1] (F2) {Distribution};
    \node[layer, fill=OrangeL, above=0.1cm of F2] (F3) {Serving};
    \node[layer, fill=RedL, above=0.1cm of F3] (F4) {Governance};

    \node[stacklabel, above=0.2cm of F4] (F_title) {B. Fleet Stack};
    \node[bottleneck, below=0.1cm of F1] (F_bot) {Bottleneck: Bisection Wall};
  \end{scope}

  \draw[->, line width=1.0pt, color=GreenLine] (single) -- (fleet) node[midway, above, font=\tiny, text=black] {Scaling};
