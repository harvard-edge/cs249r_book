...
# │ How: pint Quantities from mlsys.constants; calc_mtbf_* from formulas.py.
# │
...
# │ Show: gpu_mttf_yr, hbm_mttf_yr, nic_mttf_yr, etc. in table cells.
# │ How: Convert R (ReliabilityFoundations) hours to years via HOURS_PER_YEAR.
# │
# │ Imports: mlsys.constants (HOURS_PER_YEAR)
...
# Build display math in Python (vol1 pattern: no {python} inside math)
ckpt_gb_str = ckpt_175b_gb_str
bw_str = f"{R.ckpt_write_bw:.0f}"
delta_eq_math = md(
    f"$$\\delta = \\frac{{\\text{{Checkpoint Size}}}}{{\\text{{Write Bandwidth}}}} "
    f"= \\frac{{{ckpt_gb_str} \\text{{ GB}}}}{{{bw_str} \\text{{ GB/s}}}} "
    f"= {WorkedExampleYoungDaly.yd_delta_str} \\text{{ s}}$$"
)
tau_opt_eq_math = md(
    f"$$\\tau_\\text{{opt}} = \\sqrt{{2 \\times {WorkedExampleYoungDaly.yd_delta_str} \\text{{ s}} "
    f"\\times {WorkedExampleYoungDaly.yd_mtbf_h_str} \\text{{ h}} \\times 3{{,}}600 \\text{{ s/h}}}} "
    f"= {WorkedExampleYoungDaly.yd_tau_min_str} \\text{{ min}}$$"
)
```

::: {.callout-example title="Young-Daly: 175B Model on a 10,000-GPU Cluster"}

**Setup.** You are training a 175B-parameter model on a 10,000-GPU cluster. The cluster MTBF is `{python} WorkedExampleYoungDaly.yd_mtbf_h_str` hours (@tbl-mtbf-cluster). The checkpoint size is `{python} ckpt_175b_gb_str` GB, and your parallel storage system writes at `{python} f"{R.ckpt_write_bw:.0f}"` GB/s.

**Step 1: Checkpoint write time ($\delta$).**

`{python} delta_eq_math`

**Step 2: Apply the Young-Daly formula.**

`{python} tau_opt_eq_math`
...
# │ Show: t_detect_str, t_reschedule_str, t_reload_str, t_replay_str, t_total_str.
