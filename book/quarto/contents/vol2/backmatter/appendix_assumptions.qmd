...
# │ Exports: fleet_n_small_str, fleet_mttf_str, fleet_fail_hr_str,
# │          fleet_fail_day_str, fleet_n_large_str, fleet_fail_large_str,
# │          fleet_model_str, fleet_model_params_str, fleet_bf16_str,
# │          fleet_grad_gb_str, fleet_ib_bw_str, fleet_allreduce_str,
# │          fleet_n_carbon_str, fleet_tdp_str, fleet_pue_str,
# │          fleet_facility_mw_str, fleet_carbon_q_str, fleet_co2_q_str,
# │          fleet_carbon_p_str, fleet_co2_p_str, fleet_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
...
fleet_n_small_str = FleetNapkin.n_small_str
fleet_mttf_str = FleetNapkin.mttf_str
fleet_fail_hr_str = FleetNapkin.fail_hr_str
fleet_fail_day_str = FleetNapkin.fail_day_str
fleet_n_large_str = FleetNapkin.n_large_str
fleet_fail_large_str = FleetNapkin.fail_large_str
fleet_model_str = FleetNapkin.model_str
fleet_model_params_str = f"{FleetNapkin.model_params_b}"
fleet_bf16_str = FleetNapkin.bf16_str
fleet_grad_gb_str = FleetNapkin.grad_gb_str
fleet_ib_bw_str = FleetNapkin.ib_bw_str
fleet_allreduce_str = FleetNapkin.allreduce_str
fleet_n_carbon_str = FleetNapkin.n_carbon_str
fleet_tdp_str = FleetNapkin.tdp_str
fleet_pue_str = FleetNapkin.pue_str
fleet_facility_mw_str = FleetNapkin.facility_mw_str
fleet_carbon_q_str = FleetNapkin.carbon_q_str
fleet_co2_q_str = FleetNapkin.co2_q_str
fleet_carbon_p_str = FleetNapkin.carbon_p_str
fleet_co2_p_str = FleetNapkin.co2_p_str
fleet_ratio_str = FleetNapkin.ratio_str
```
...
**How many failures per day should you expect?** A cluster of `{python} fleet_n_small_str` GPUs with each GPU having an MTTF of `{python} fleet_mttf_str` hours experiences GPU failures at a rate of `{python} fleet_n_small_str` / `{python} fleet_mttf_str` $\approx$ `{python} fleet_fail_hr_str` failures per hour, or roughly `{python} fleet_fail_day_str` GPU failures per day. This estimate does not include NIC, PSU, or cable failures, which collectively double the rate. At `{python} fleet_n_large_str` GPUs, expect approximately `{python} fleet_fail_large_str` GPU failures per day --- failure becomes a continuous background process, not an exceptional event.

**How long does an AllReduce of a `{python} fleet_model_str` model take?** With BF16 parameters, the gradient payload is $`{python} fleet_model_params_str` \times 10^9 \times `{python} fleet_bf16_str`$ bytes = `{python} fleet_grad_gb_str` GB. Over InfiniBand NDR (`{python} fleet_ib_bw_str` GB/s effective per port), the ring AllReduce time is approximately $2 \times$ `{python} fleet_grad_gb_str` GB / `{python} fleet_ib_bw_str` GB/s $\approx$ `{python} fleet_allreduce_str` seconds --- a significant fraction of a training step, which is why pipeline and tensor parallelism exist to shrink the gradient volume each rank must communicate.

**What is the carbon footprint of a `{python} fleet_n_carbon_str` GPU training run?** Each H100 draws `{python} fleet_tdp_str` W at TDP. With PUE of `{python} fleet_pue_str` (best-in-class air cooled), total facility power is $`{python} fleet_n_carbon_str`\times`{python} fleet_tdp_str`$ W$\times$ `{python} fleet_pue_str` = `{python} fleet_facility_mw_str` MW. Running in Quebec (`{python} fleet_carbon_q_str` gCO$_2$/kWh): `{python} fleet_facility_mw_str` MW$\times$ 720 h/month$\times$ `{python} fleet_carbon_q_str` g/kWh = `{python} fleet_co2_q_str` tonnes CO$_2$ per month. The same run in Poland (`{python} fleet_carbon_p_str` gCO$_2$/kWh) emits `{python} fleet_co2_p_str` tonnes --- a `{python} fleet_ratio_str`$\times$ difference driven entirely by grid carbon intensity.
