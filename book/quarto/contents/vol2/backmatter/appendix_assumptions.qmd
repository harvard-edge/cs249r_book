...
# │ Exports: FleetNapkin.n_small_str, FleetNapkin.mttf_str,
# │          FleetNapkin.fail_hr_str, FleetNapkin.fail_day_str,
# │          FleetNapkin.n_large_str, FleetNapkin.fail_large_str,
# │          FleetNapkin.model_str, FleetNapkin.model_params_b_str,
# │          FleetNapkin.bf16_str, FleetNapkin.grad_gb_str,
# │          FleetNapkin.ib_bw_str, FleetNapkin.allreduce_str,
# │          FleetNapkin.n_carbon_str, FleetNapkin.tdp_str,
# │          FleetNapkin.pue_str, FleetNapkin.facility_mw_str,
# │          FleetNapkin.carbon_q_str, FleetNapkin.co2_q_str,
# │          FleetNapkin.carbon_p_str, FleetNapkin.co2_p_str,
# │          FleetNapkin.ratio_str
# └─────────────────────────────────────────────────────────────────────────────
...
**How many failures per day should you expect?** A cluster of `{python} FleetNapkin.n_small_str` GPUs with each GPU having an MTTF of `{python} FleetNapkin.mttf_str` hours experiences GPU failures at a rate of `{python} FleetNapkin.n_small_str` / `{python} FleetNapkin.mttf_str` $\approx$ `{python} FleetNapkin.fail_hr_str` failures per hour, or roughly `{python} FleetNapkin.fail_day_str` GPU failures per day. This estimate does not include NIC, PSU, or cable failures, which collectively double the rate. At `{python} FleetNapkin.n_large_str` GPUs, expect approximately `{python} FleetNapkin.fail_large_str` GPU failures per day --- failure becomes a continuous background process, not an exceptional event.

**How long does an AllReduce of a `{python} FleetNapkin.model_str` model take?** With BF16 parameters, the gradient payload is $`{python} FleetNapkin.model_params_b_str` \times 10^9 \times `{python} FleetNapkin.bf16_str`$ bytes = `{python} FleetNapkin.grad_gb_str` GB. Over InfiniBand NDR (`{python} FleetNapkin.ib_bw_str` GB/s effective per port), the ring AllReduce time is approximately $2 \times$ `{python} FleetNapkin.grad_gb_str` GB / `{python} FleetNapkin.ib_bw_str` GB/s $\approx$ `{python} FleetNapkin.allreduce_str` seconds --- a significant fraction of a training step, which is why pipeline and tensor parallelism exist to shrink the gradient volume each rank must communicate.

**What is the carbon footprint of a `{python} FleetNapkin.n_carbon_str` GPU training run?** Each H100 draws `{python} FleetNapkin.tdp_str` W at TDP. With PUE of `{python} FleetNapkin.pue_str` (best-in-class air cooled), total facility power is $`{python} FleetNapkin.n_carbon_str`\times`{python} FleetNapkin.tdp_str`$ W$\times$ `{python} FleetNapkin.pue_str` = `{python} FleetNapkin.facility_mw_str` MW. Running in Quebec (`{python} FleetNapkin.carbon_q_str` gCO$_2$/kWh): `{python} FleetNapkin.facility_mw_str` MW$\times$ 720 h/month$\times$ `{python} FleetNapkin.carbon_q_str` g/kWh = `{python} FleetNapkin.co2_q_str` tonnes CO$_2$ per month. The same run in Poland (`{python} FleetNapkin.carbon_p_str` gCO$_2$/kWh) emits `{python} FleetNapkin.co2_p_str` tonnes --- a `{python} FleetNapkin.ratio_str`$\times$ difference driven entirely by grid carbon intensity.
