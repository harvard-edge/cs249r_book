...
Serving a model at scale is governed by the mathematics of waiting lines and the physics of the key-value cache. Queuing models and KV-cache arithmetic provide the quantitative foundation for choosing batch sizes and predicting tail latencies under stochastic request arrival patterns. This appendix develops the analytical serving models that the book's inference and operations chapters draw on, establishing how compute capacity, memory bandwidth, and queue depth combine into a single serving constraint.
