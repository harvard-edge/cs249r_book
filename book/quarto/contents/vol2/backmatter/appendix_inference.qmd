...
2. **Batch size 8 achieves stability**\index{Batch Size!8 achieves stability}: At `{python} GPT3ServingResults.b8_util_pct_str` utilization, the system is stable but queuing delays contribute significantly to latency (`{python} GPT3ServingResults.b8_wait_ms_str`), causing queuing latency to explode near saturation (@fig-queuing-hockey-stick).
...
4. **Diminishing returns** beyond $B=32$\index{Batching!diminishing returns}: Further batch size increases would reduce utilization but memory constraints prevent exploration.
...
**Verification using Little's Law**\index{Little's Law!verification using} [@little1961]: $Q_{\text{req}} = \lambda_{\text{arr}} \cdot T_{\text{lat}}$
...
class AppdxKVCacheSizing:
    ...
    kv_gb_per_req = kv_bytes_per_req / (1024**3)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    kv_gb_per_req_str = f"{kv_gb_per_req:.2f}"
    max_batch_size_str = str(max_batch_size)
    remaining_mem_str = str(remaining_mem)
    total_hbm_str = str(total_hbm)
    weights_mem_str = str(weights_mem)
```

For the 175B GPT-3 model ($N_L=96$, $d=12288$, FP16 $s_{\text{elem}}=2$ bytes) at sequence length $S=2048$, each request requires:

$$M_{\text{KV/req}} = 2 \times 96 \times 12288 \times 2048 \times 2 \approx \{python} AppdxKVCacheSizing.kv_gb_per_req_str \text{ GB/request}$$

Across 8$\times$ A100 80 GB GPUs (`{python} AppdxKVCacheSizing.total_hbm_str` GB total HBM), storing model weights in FP16 consumes `{python} AppdxKVCacheSizing.weights_mem_str` GB, leaving `{python} AppdxKVCacheSizing.remaining_mem_str` GB for activation and KV cache memory. Unmanaged contiguous allocation limits the maximum concurrent batch size to $B_{\text{max}} \le \{python} AppdxKVCacheSizing.remaining_mem_str`/\{python} AppdxKVCacheSizing.kv_gb_per_req_str \approx \{python} AppdxKVCacheSizing.max_batch_size_str$ requests before running out of memory (OOM), demonstrating how KV cache capacity bounds serving throughput.
