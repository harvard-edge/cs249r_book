...
The taxonomy maps directly to the **Iron Law of ML Systems**, introduced in @sec-vol2-introduction-notation. @tbl-dam-components-ref summarizes the role, primary physical constraint, and core optimization pathway for each axis.
...
| **Machine (M)**   | **Physics** (The Engine)   | Throughput ($R_{peak}$) | Hardware Acceleration                              |

: **D·A·M Axis Reference.** Each axis maps to a distinct physical constraint and a high-leverage optimization strategy. Start diagnosis here: identify which constraint is binding, then follow the optimization lever. {#tbl-dam-components-ref}
...
$$ T_{sequential} = \frac{D_{vol}}{BW} + \frac{O}{R_{peak} \cdot \eta} + L_{lat} \quad \xrightarrow{	ext{overlap}} \quad T_{pipelined} = \max\left(\frac{D_{vol}}{BW}, \frac{O}{R_{peak} \cdot \eta}ight) + L_{lat} $$
...
: **D·A·M Overlap Techniques.** Each technique allows one D·A·M axis to execute while another is in flight, converting the Iron Law's additive terms into overlapped terms. {#tbl-dam-overlap}
...
[^fn-arith-intensity]: **Arithmetic Intensity**: The ratio of floating-point operations to bytes transferred (FLOPs/byte). It determines whether a workload is memory-bound or compute-bound by comparison against the hardware's *ridge point* ($R_{peak}/BW$).
...
: **What Works vs. What's Wasted.** Optimizing the wrong term yields exactly zero improvement. A memory-bound model will not speed up from a faster GPU; the GPU will simply idle faster while waiting for memory. {#tbl-bottleneck-actions}
...
: **D·A·M Tooling Map.** Profiling utilities for diagnosing bottlenecks along each D·A·M axis. {#tbl-dam-tooling}

## D·A·M Scorecard {#sec-dam-taxonomy-dam-scorecard}

Use the efficiency ratios in @tbl-dam-scorecard to grade your system's performance against its theoretical limit. This "Report Card" is anchored by **MFU**[^fn-mfu-scorecard]—the ratio of achieved model FLOPs to the hardware's theoretical peak FLOPs.
...
: **The D·A·M Efficiency Rubric.** Use these three numbers to characterize single-machine maturity. {#tbl-dam-scorecard}
