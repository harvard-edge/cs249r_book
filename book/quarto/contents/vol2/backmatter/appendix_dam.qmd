...
This appendix provides a compact diagnostic framework—**Data · Algorithm · Machine (D·A·M)**\index{D·A·M Taxonomy!appendix refresher}—and shows how to map single-machine symptoms and measurements to the term of the iron law that dominates. It serves as a foundational checklist before fleet-scale optimization.
...
class DAMTaxonomy:
    ...
    ex2_latency_s = 0.050
    h100_fp16_tflops_peak = h.compute.peak_flops.to(TFLOPs/second).magnitude
    h100_hbm_tb_s = h.memory.bandwidth.to(TB/second).magnitude
    ...
    ex2_flops_per_pass = calc_transformer_decode_flops(ex2_params).to(TFLOPs).magnitude
    ex2_achieved_tflops = ex2_flops_per_pass/ex2_latency_s
    ex2_util = (ex2_achieved_tflops/h100_fp16_tflops_peak) * 100
    ex2_model_size_gb = (ex2_params * 2 * ureg.byte).to(GB).magnitude
...
The additive iron law represents **sequential execution**\index{Sequential execution!D·A·M coordination}—the worst case where Data, Algorithm, and Machine take turns. Skilled systems engineering transforms the sum into a max:
