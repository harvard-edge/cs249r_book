...
class DAMTaxonomy:
    ...
    ex2_util_eq = md(
        rf"$$\eta_{\text{hw}} = \frac{{{ex2_achieved_str}}}{{{int(h100_fp16_tflops_peak)}}} \approx {ex2_util_str}\%$$"
    )
    ...
    ex4_gpu_old_str = md(rf"{ex4_gpu_old_n}$\times$ A100")
    ex4_gpu_new_str = md(rf"{ex4_gpu_new_n}$\times$ H100")
...
$$ T_{sequential} = \frac{D_{\text{vol}}}{\text{BW}} + \frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}} + L_{\text{lat}} \quad \xrightarrow{\text{overlap}} \quad T_{pipelined} = \max\left(\frac{D_{\text{vol}}}{\text{BW}},\; \frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}}\right) + L_{\text{lat}} $$
