...
This appendix provides a compact diagnostic framework—**Data · Algorithm · Machine (D·A·M)**\index{Data Algorithm Machine}\index{D·A·M}—and shows how to map single-machine symptoms and measurements to the term of the iron law that dominates. It serves as a foundational checklist before fleet-scale optimization.
...
The additive iron law represents **sequential execution**\index{Sequential Execution}—the worst case where Data, Algorithm, and Machine take turns. Skilled systems engineering transforms the sum into a max:
...
This overlap view still leaves one practical question: whether a workload is waiting on bytes or on FLOPs. The boundary between Data\index{Data!D·A·M} (memory bound) and Machine (compute bound) is not arbitrary; it is defined mathematically by **Arithmetic Intensity**[^fn-intensity-v2] ($I$) of the workload.
