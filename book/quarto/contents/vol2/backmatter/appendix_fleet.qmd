...
class FleetFoundations:
    ...
    # Latency hierarchy (microseconds)
    ...
    # Key ratios
    nvlink_to_ib = int(nvlink_h100_bw / ib_ndr_bw)  # NVLink vs IB NDR
    ib_to_tcp_lat = int(tcp_lat / ib_ndr_lat)  # TCP vs IB latency ratio
    ...
    cl_mega = CLUSTER_MEGA_GPUS

    # MTBF calculations: single node
    node_mtbf_h = calc_mtbf_node(
        GPU_MTTF_HOURS, 8,        # 8 GPUs per node
        NIC_MTTF_HOURS, 8,        # 8 NICs per node
        PSU_MTTF_HOURS, 4         # 4 PSUs per node
    )

    # Cluster MTBF (GPU-only, simplified)
    ...
    oh_checkpoint = int(OVERHEAD_CHECKPOINT * 100)
    oh_failure = int(OVERHEAD_FAILURE_RECOVERY * 100)
    ...
    rack_ratio = round(rack_ai / rack_trad, 1)
    ...
    check(pfail_8192_24h > 0.9, "8K GPU cluster must have >90% failure prob in 24h")
...
This book builds directly on the foundational concepts established in the preceding book. The following provides a compact reference for the key ideas that reappear throughout the distributed systems chapters.

- **The Iron Law** ($T \approx D_{vol}/BW + O/(R_{peak} \cdot \eta) + L_{lat}$): Performance is bounded by data movement or compute. At fleet scale, the data movement term expands to include inter-node communication, not just memory bandwidth.
...
Just as the preceding book provided "Numbers Every ML Systems Engineer Should Know" (@sec-machine-foundations-numbers-know-b531), this section provides the fleet-scale counterparts. The single-machine numbers still apply within each node, but a new set of numbers governs the spaces *between* nodes. While absolute values evolve with hardware generations, the *ratios* between communication tiers and the *scaling behavior* of failure rates remain remarkably stable. **Memorize the ratios and scaling trends; use the specific numbers as sanity checks.**

::: {.callout-note title="Node-level numbers fleet engineers need"}
Fleet reasoning often depends on a few *node-level* numbers; the full single-machine reference is @sec-machine-foundations-numbers-know-b531. The subset that directly affects fleet design: **16 bytes per parameter** (checkpoint sizing, ZeRO partitioning, memory budgets); **NVLink vs HBM bandwidth** (intra-node parallelism placement); **peak FLOPS and HBM capacity** (MFU and effective FLOPS, batch and model sharding). This appendix's hardware table and Communication numbers above give the inter-node and current-generation values; use Vol I for the full memory hierarchy and roofline ridge points within a node.
:::
...
#| echo: false
# Formatting for @tbl-fleet-numbers-quick-ref only; values from FF.
_nvlink_ib = fmt(FF.nvlink_to_ib, precision=0, commas=False)
_ib_ndr_gbs = fmt(FF.ib_ndr_bw, precision=0, commas=False)
_ib_ndr_us = fmt(FF.ib_ndr_lat, precision=0, commas=False)
_eff_1024 = fmt(FF.eff_1024, precision=0, commas=False)
_eff_8192 = fmt(FF.eff_8192, precision=0, commas=False)
...
_air_limit = fmt(FF.air_limit, precision=0, commas=False)
_pue_liq = fmt(FF.pue_liquid, precision=2, commas=False)
_pue_typ = fmt(FF.pue_typical, precision=2, commas=False)
_h100_tdp = fmt(FF.h100_tdp, precision=0, commas=False)
```

| **Category**      | **Number**                                                                 | **Use** |
|:------------------|:---------------------------------------------------------------------------|:--------|
| **Communication** | NVLink ~`{python} _nvlink_ib`× IB NDR                                      | Parallelism boundary (in-node vs cross-node) |
| **Communication** | IB NDR ~`{python} _ib_ndr_gbs` GB/s, ~`{python} _ib_ndr_us` μs one-way     | Inter-node bandwidth and latency |
| **Computation**   | MFU `{python} _mfu_lo`--`{python} _mfu_hi`%, η ~`{python} _eff_1024`% @ 1K, ~`{python} _eff_8192`% @ 8K | Effective FLOPS and scaling sanity checks |
| **Coordination**  | MTBF 8K: ~`{python} _mtbf_8k_min` min; 100K: ~`{python} _mtbf_100k_min` min | Failure expectation and checkpoint cadence |
| **Coordination**  | 175B checkpoint ~`{python} _ckpt_175` GB (16 B/param)                      | Recovery and storage sizing |
| **Coordination**  | Goodput ~`{python} _goodput`% after overheads                              | Wall-clock utilization |
| **Power & sustainability** | AI rack ~`{python} _rack_ai` kW; air limit ~`{python} _air_limit` kW      | Cooling feasibility (liquid required above air limit) |
| **Power & sustainability** | PUE liquid ~`{python} _pue_liq`, typical ~`{python} _pue_typ`; H100 `{python} _h100_tdp` W → 10K GPUs ≈ 7 MW IT × PUE | Facility load and carbon (see @sec-fleet-foundations-thermal-power) |
...
# │ Show: nvlink_bw_str, ib_ndr_gbs, etc. in table cells and prose.
...
|:----------------------|:------------------------------|:--------------------------------------------|
| **NVLink 4.0 (H100)** | `{python} nvlink_bw_str` GB/s | Tensor/pipeline parallelism within a node   |
| **TPU v5p ICI**       | `{python} tpuv5_ici_str` GB/s | Intra-pod model parallelism (Google)        |
| **PCIe Gen5 x16**     | `{python} pcie5_bw_str` GB/s  | CPU-GPU data transfer, NIC attachment       |
| **IB XDR (800 Gbps)** | `{python} ib_xdr_bw_str` GB/s | Next-gen inter-node (2025+)                 |
| **IB NDR (400 Gbps)** | `{python} ib_ndr_bw_str` GB/s | Current inter-node standard for AI clusters |
| **IB HDR (200 Gbps)** | `{python} ib_hdr_bw_str` GB/s | Previous-gen inter-node                     |
| **RoCE v2 (100 GbE)** | `{python} roce_bw_str` GB/s   | Budget clusters, inference fleets           |

: **Communication Bandwidth Hierarchy**: Bandwidth drops by `{python} nvlink_to_ib_str`$\times$ crossing from intra-node (NVLink) to inter-node (InfiniBand). This ratio determines parallelism placement. {#tbl-fleet-bandwidth-hierarchy}
...
|:----------------------|:----------------------------------|:--------------------------------------|
| **InfiniBand NDR**    | ~`{python} ib_ndr_lat_str` $\mu$s | Low enough for synchronous AllReduce  |
| **InfiniBand HDR**    | ~`{python} ib_hdr_lat_str` $\mu$s | Adequate for most training topologies |
| **RoCE v2**           | ~`{python} roce_lat_str` $\mu$s   | Acceptable for data parallelism       |
| **TCP/IP (Ethernet)** | ~`{python} tcp_lat_str` $\mu$s    | Too slow for synchronous training     |
| **Cross-datacenter**  | ~40,000 $\mu$s (40 ms)            | Physics floor; async training only    |

: **Communication Latency Hierarchy**: Latency determines whether synchronous training is feasible. TCP/IP is `{python} fmt(FF.ib_to_tcp_lat, precision=0, commas=False)`$\times$ slower than InfiniBand NDR, making it unsuitable for gradient synchronization in large clusters. {#tbl-fleet-latency-hierarchy}
...
# │ Show: mtbf_256_str, mtbf_2048_str, etc. and per-day failure counts.
...
|:-----------------|:-------------------------------|:-----------------------------|:---------------------------|
| **256 GPUs**     | `{python} mtbf_256_str` hours  | `{python} mtbf_256_min_str`  | `{python} pfail_256_str`%  |
| **2,048 GPUs**   | `{python} mtbf_2048_str` hours | `{python} mtbf_2048_min_str` | `{python} pfail_2048_str`% |
| **8,192 GPUs**   | `{python} mtbf_8192_str` hours | `{python} mtbf_8192_min_str` | `{python} pfail_8192_str`% |
| **100,000 GPUs** | `{python} mtbf_100k_str` hours | `{python} mtbf_100k_min_str` | `{python} pfail_100k_str`% |

: **MTBF and Failure Probability by Cluster Size**: GPU-only failure model with per-GPU MTTF of `{python} fmt(GPU_MTTF_HOURS, precision=0)` hours. Real clusters also include NIC, PSU, cable, and switch failures, making these estimates conservative. The probability column uses $P(\geq 1) = 1 - e^{-T/\text{MTBF}}$ for $T = 24$ hours. {#tbl-fleet-mtbf}
...
# │ Show: ckpt_7b_str, ckpt_70b_str, ckpt_175b_str, ckpt_1t_str in table cells.
...
Fleet-scale capacity planning and sustainability reporting require a few power numbers that every fleet engineer should know. Vol I covers per-device energy ratios (e.g., DRAM vs compute); at fleet scale, the critical numbers are **rack power density**, **PUE**, and the **air-cooling limit**---they determine where you can build and what your facility load will be.
...
# │ Show: h100_flops_str, h100_bw_str, h100_cap_str, h100_tdp_str, b200_*, tpuv5_*.
...
tpuv5_bw_str = FF.tpuv5_bw_tbs
tpuv5_cap_str = fmt(FF.tpuv5_cap, precision=0)
tpuv5_ici_str2 = fmt(FF.tpuv5_ici, precision=0)
```

| **Spec**             | **NVIDIA H100 (SXM)**            | **NVIDIA B200**                  | **Google TPU v5p**                   |
|:---------------------|:---------------------------------|:---------------------------------|:-------------------------------------|
| **BF16/FP16 Peak**   | `{python} h100_flops_str` TFLOPS | `{python} b200_flops_str` TFLOPS | `{python} tpuv5_flops_str` TFLOPS    |
| **Memory Bandwidth** | `{python} h100_bw_str` TB/s      | `{python} b200_bw_str` TB/s      | `{python} tpuv5_bw_str` TB/s         |
| **HBM Capacity**     | `{python} h100_cap_str` GB       | `{python} b200_cap_str` GB       | `{python} tpuv5_cap_str` GB          |
| **Intra-Node Link**  | 900 GB/s (NVLink 4.0)            | 1,800 GB/s (NVLink 5.0)          | `{python} tpuv5_ici_str2` GB/s (ICI) |
| **TDP**              | `{python} h100_tdp_str` W        | `{python} b200_tdp_str` W        | ~400 W (estimated)                   |
...
The preceding book introduced Amdahl's Law for a single machine, where the serial fraction $s$ represents sequential code that cannot be parallelized (@sec-machine-foundations-amdahls-law-gustafsons-law-b741). At fleet scale, the "serial fraction" is not just sequential code---it includes every operation that forces all $N$ GPUs to wait:
...
|:----------------------------------------|:-----------------------------------------------------|:----------------------|:------------------------|
| **Data parallel, 7B model, 256 GPUs**   | `{python} ar_7b_ms_str` ms (AllReduce over IB NDR)   | ~50 ms (fwd+bwd)      | `{python} rho_7b_str`   |
| **Data parallel, 350M model, 256 GPUs** | `{python} ar_350m_ms_str` ms (AllReduce over IB NDR) | ~5 ms (fwd+bwd)       | `{python} rho_350m_str` |
| **Tensor parallel, 8 GPUs (NVLink)**    | ~0.02 ms (activation over NVLink)                    | ~1 ms (one layer)     | `{python} rho_tp_str`   |
...
The 7B model on 256 GPUs achieves $\rho =$ `{python} rho_7b_str`---communication takes about `{python} fmt(rho_7b * 100, precision=0)`% as long as computation, which can be partially overlapped. The 350M model on the same cluster has $\rho =$ `{python} rho_350m_str`---communication dominates, making this configuration communication-bound. The solution is either to use fewer GPUs (reduce $N$ in the AllReduce) or to increase the computation per step (larger batch size, gradient accumulation).

Tensor parallelism within a node achieves $\rho =$ `{python} rho_tp_str`, confirming that NVLink bandwidth is sufficient to keep intra-node parallelism compute-bound. This is the quantitative reason why tensor parallelism is confined within nodes while data parallelism spans across them.

### Weak Scaling at Fleet Scale {#sec-fleet-foundations-weak-scaling-fleet}

Amdahl's Law paints a pessimistic picture because it assumes a fixed problem size. In practice, fleet-scale training follows **weak scaling**: the problem size (tokens, data, model parameters) grows proportionally with the number of GPUs. Gustafson's Law (@sec-machine-foundations-weak-scaling-gustafsons-law-eb7e) captures this optimistic view.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-fleet-foundations-effective-flops — compound loss callout
# │
# │ Goal: Format peak and effective FLOPS; build display math (vol1 pattern:
# │       no {python} inside $...$ or $$...$$).
# │ Show: effective_eq_math display equation; peak_str, eff_str, etc. in prose.
# │ How: fmt() on FF values; build md() display math from formatted strings.
# │
# │ Imports: mlsys.formatting (fmt)
# │ Exports: peak_str, eff_str, eff_pct_str, goodput_pct_str, mfu_pct_str, scaling_pct_str, effective_eq_math
# └─────────────────────────────────────────────────────────────────────────────
...
goodput_fmt = fmt(FF.goodput_ratio, precision=2, commas=False)
effective_eq_math = md(
    f"$$= {peak_str} \\times {mfu_fmt} \\times {scaling_fmt} \\times {goodput_fmt} "
    f"\\approx {eff_str} \\text{{ TFLOPS}}$$"
)
```

::: {.callout-perspective title="The Compound Loss of Fleet Utilization"}

A 1,024-GPU H100 cluster has a peak aggregate throughput of `{python} peak_str` TFLOPS. After the three multiplicative losses, the effective throughput is:
...
Amdahl's Law caps speedup at $1/s$ for a fixed workload. At `{python} s_fleet_pct_str`% serial overhead, no fleet larger than ~`{python} max_speedup_str`$\times$ the baseline delivers meaningful additional speedup. Beyond the Amdahl limit, every additional GPU contributes nearly zero marginal throughput while increasing failure probability and communication overhead. The solution is weak scaling: grow the problem to match the hardware.
:::
...
A single GPU with a `{python} fmt(GPU_MTTF_HOURS, precision=0)`-hour MTTF seems extremely reliable---that is nearly 6 years. But at `{python} fmt(FF.cl_large, precision=0)` GPUs, the cluster MTBF drops to `{python} mtbf_8192_str` hours (`{python} mtbf_8192_min_str` minutes). The probability of at least one failure in a 24-hour training window is `{python} pfail_8192_str`%. Designing a training system without automated failure recovery at this scale guarantees that every long-running job will fail and require manual intervention.
:::
...
- **The 18$\times$ bandwidth gap**: NVLink provides ~`{python} nvlink_to_ib_str`$\times$ more bandwidth than InfiniBand. This invariant ratio determines parallelism placement: tensor parallelism within nodes, data parallelism across nodes.
- **Failure is certain at scale**: Cluster MTBF scales as $1/N$. At 8,192 GPUs, expect a failure every `{python} mtbf_8192_min_str` minutes. Fault tolerance is not optional---it is a prerequisite for completing any fleet-scale training job.
- **Compound utilization loss**: After MFU (~`{python} mfu_pct_str`%), scaling efficiency (~`{python} scaling_pct_str`%), and operational overhead (~`{python} goodput_pct_str`% goodput) compound, a 1,024-GPU cluster delivers ~`{python} eff_pct_str`% of peak FLOPS as useful work.
