...
Volume I's Machine Foundations appendix (@sec-machine-foundations) gave engineers the numbers for a single machine: memory hierarchy latencies, roofline ridge points, and precision trade-offs. But the moment a training job spans two machines, a new set of numbers takes over. Network latency replaces cache latency as the dominant concern. Component failure rates compound from negligible to inevitable. Communication overhead erodes the scaling efficiency that justifies buying more accelerators in the first place.

This appendix collects the reference numbers and compact models for fleet-scale reasoning. It begins with a comparison of the three system paradigms that underpin Volume II, then provides the "numbers every fleet engineer should know"---organized around the three axes of Communication, Computation, and Coordination (C$^3$). It concludes with the scaling physics and thermal constraints that govern cluster design.
...
### Volume I Recap {.unnumbered}

Volume II builds directly on the foundational concepts established in Volume I. The following provides a compact reference for the key ideas that reappear throughout the distributed systems chapters.
...
Volume I introduced Amdahl's Law for a single machine, where the serial fraction $s$ represents sequential code that cannot be parallelized (@sec-machine-foundations-amdahls-law-gustafsons-law-b741). At fleet scale, the "serial fraction" is not just sequential code---it includes every operation that forces all $N$ GPUs to wait:
...
check(rho_350m > rho_7b, "Smaller model should have higher comm ratio")
...
$$= `{python} peak_str` \times `{python} fmt(MFU_TRAINING_HIGH, precision=2, commas=False)` \times `{python} fmt(SCALING_EFF_1024GPU, precision=2, commas=False)` \times `{python} fmt(FF.goodput_ratio, precision=2, commas=False)` \approx `{python} eff_str` \text{ TFLOPS}$$
...
The shift from traditional datacenter workloads to AI training has created a **power density crisis**. @tbl-fleet-power-density quantifies the gap.
...
@tbl-fleet-pue compares PUE across datacenter generations and cooling technologies.
...
: **Power Usage Effectiveness (PUE)**: Lower is better. Liquid cooling achieves PUE near 1.0 because it removes heat directly from the chip without the intermediate step of heating air. The gap between legacy (1.58) and liquid-cooled (1.06) represents a 49% reduction in total facility power. {#tbl-fleet-pue}
