...
: **Overhead Budgets for Fleet-Scale Training**: These are fractions of wall-clock time. At 10,000+ GPUs, failure recovery dominates. The compound effect is additive: total goodput ratio $\approx 1.0 - (0.05 + 0.03 + 0.10 + 0.05) = 0.77 \approx 77$%. {#tbl-fleet-overhead-budgets}
...
The `{python} FleetAmdahlExample.s_fleet_pct_str` percent serial fraction above is optimistic. In practice, fleet-scale serial overhead accumulates from multiple sources: `{python} FF.oh_pipeline` percent pipeline bubbles + `{python} FF.oh_checkpoint` percent checkpointing + `{python} FF.oh_failure` percent failure recovery + `{python} FF.oh_maintenance` percent maintenance. These are not all strictly serial in the Amdahl sense---some overlap with computation---but they illustrate how quickly small per-category overheads compound into significant throughput loss.
:::
...
|:----------------------------------------|:------------------------------------------------------------------------|:----------------------|:-------------------------------------------|
| **Data parallel, 7B model, 256 GPUs**   | `{python} FleetCommCompRatio.ar_7b_ms_str` ms (AllReduce over IB NDR)   | ~5.0 s (fwd+bwd)      | `{python} FleetCommCompRatio.rho_7b_str`   |
| **Data parallel, 350M model, 256 GPUs** | `{python} FleetCommCompRatio.ar_350m_ms_str` ms (AllReduce over IB NDR) | ~10 ms (fwd+bwd)      | `{python} FleetCommCompRatio.rho_350m_str` |
