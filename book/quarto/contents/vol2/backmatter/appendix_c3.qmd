...
The **Communication**\index{C³ Taxonomy!communication} axis dominates. Each GPU computes efficiently (MFU is healthy), but more than half the step time is spent synchronizing gradients across the network. The system is **communication-bound**\index{Communication-bound workload}: adding more GPUs will make it worse, not better, because AllReduce time grows with participant count while per-GPU computation stays constant.
