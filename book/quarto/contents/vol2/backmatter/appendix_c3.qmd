...
In a distributed training cluster, "it's slow" is even less informative than on a single machine. A 4,096-GPU job can miss its throughput target because individual accelerators are underutilized (computation), because gradient synchronization saturates the network fabric (communication), or because checkpoint overhead and failure recovery consume too much wall-clock time (coordination). Without a taxonomy, teams buy more GPUs when they should be upgrading interconnects, or optimize kernels when the real problem is pipeline bubble overhead.

This appendix provides a compact diagnostic framework---**Computation, Communication, Coordination (C$^3$)**---and shows how to map fleet-scale symptoms and measurements to the term of the Fleet Law that dominates. It is the Volume II counterpart to the D·A·M Taxonomy (@sec-dam-taxonomy), extending the same diagnostic philosophy from a single machine to the distributed fleet. Use it as your "first response" checklist before committing to deeper fleet-scale optimization.
...
+:===================+:==============================+:==============================+:===========================================+
| **Data (D)**       | I/O bandwidth, disk to GPU    | **Communication ($C_2$)**     | Data moves across network, not just        |
|                    |                               |                               | memory hierarchy                           |
+--------------------+-------------------------------+-------------------------------+--------------------------------------------+
| **Algorithm (A)**  | FLOPs, model depth, ops count | **Computation ($C_1$)**       | Per-GPU utilization (MFU) still matters,   |
|                    |                               |                               | but scaling efficiency erodes it           |
+--------------------+-------------------------------+-------------------------------+--------------------------------------------+
| **Machine (M)**    | Peak FLOPS, hardware limits   | **Computation ($C_1$)**       | Fleet peak = $N \times$ single-GPU peak,   |
|                    |                               |                               | but compound losses reduce effective FLOPS |
+--------------------+-------------------------------+-------------------------------+--------------------------------------------+
| *(no equivalent)*  | *(overhead term $L_{lat}$)*   | **Coordination ($C_3$)**      | New axis: barriers, checkpoints, failure   |
|                    |                               |                               | recovery, scheduling---negligible on one   |
|                    |                               |                               | machine, dominant at 10K+ GPUs             |
+--------------------+-------------------------------+-------------------------------+--------------------------------------------+
...
The most important row in @tbl-c3-dam-mapping is the last one. On a single machine, the overhead term ($L_{lat}$) in the Iron Law is typically small---kernel launch latency, Python dispatch, synchronization barriers. At fleet scale, Coordination becomes an axis in its own right: checkpoint writes, failure detection and recovery, pipeline bubble overhead, scheduler preemptions, and maintenance windows collectively consume a significant fraction of wall time. Coordination is the axis that Volume II exists to address.
...
+:===================+:==========================+:==============================+:==============================+:====================================+
| **Computation**    | Arithmetic throughput     | Low MFU, GPU utilization      | MFU (Model FLOPS Utilization) | Kernel optimization, mixed          |
| ($C_1$)            | ($R_{peak} \times \eta$)  | below 80%, poor per-GPU       |                               | precision, operator fusion          |
|                    |                           | performance                   |                               | (@sec-performance-engineering)      |
+--------------------+---------------------------+-------------------------------+-------------------------------+-------------------------------------+
| **Communication**  | Network bandwidth         | High AllReduce time, low      | Scaling efficiency ($\eta$),  | Gradient compression, overlap       |
| ($C_2$)            | ($BW_{net}$)              | scaling efficiency,           | communication fraction        | compute/communication, topology     |
|                    |                           | communication > 30% of step  | ($T_{comm}/T_{step}$)         | optimization                        |
|                    |                           |                               |                               | (@sec-collective-communication)     |
+--------------------+---------------------------+-------------------------------+-------------------------------+-------------------------------------+
| **Coordination**   | Synchronization overhead  | Low goodput ratio, frequent   | Goodput ratio                 | Async checkpointing, elastic        |
| ($C_3$)            | and failure recovery      | restarts, large pipeline      | ($T_{useful}/T_{wall}$)       | training, faster failure detection   |
|                    |                           | bubbles, scheduler churn      |                               | (@sec-fault-tolerance-reliability)  |
+--------------------+---------------------------+-------------------------------+-------------------------------+-------------------------------------+
...
+:=========================+:============================+:==========================================+:==========================================+
| **Computation-bound**    | $T_{\text{Computation}}$    | Better kernels, mixed precision, operator | More network bandwidth (GPUs are          |
|                          |                             | fusion, next-gen accelerators             | not waiting on the network)               |
+--------------------------+-----------------------------+-------------------------------------------+-------------------------------------------+
| **Communication-bound**  | $T_{\text{Communication}}$  | Gradient compression, compute-comm        | Faster GPUs (they will just idle faster   |
|                          |                             | overlap, hierarchical collectives,        | while waiting for the network)            |
|                          |                             | InfiniBand upgrade                        |                                           |
+--------------------------+-----------------------------+-------------------------------------------+-------------------------------------------+
| **Coordination-bound**   | $T_{\text{Coordination}}$   | Async checkpointing, elastic training,    | Neither faster GPUs nor faster network    |
|                          |                             | faster failure detection, fewer pipeline  | (the time is lost to overhead, not to     |
|                          |                             | stages                                    | data movement or arithmetic)              |
+--------------------------+-----------------------------+-------------------------------------------+-------------------------------------------+
...
+:==================================+:===================+:==========================================+:==================================+:==========================================+
| Low MFU despite fast network      | **Computation**    | Are Tensor Cores engaged? Is batch size   | Per-GPU kernel trace              | Enable mixed precision, increase          |
|                                   |                    | sufficient for arithmetic intensity?      | (Nsight/PyTorch Profiler)         | per-GPU batch size                        |
+-----------------------------------+--------------------+-------------------------------------------+-----------------------------------+-------------------------------------------+
| Throughput plateaus when adding   | **Communication**  | Does AllReduce time grow faster than       | NCCL trace, $\rho$ ratio          | Gradient compression, hierarchical        |
| GPUs                              |                    | computation shrinks?                      |                                   | collectives, overlap                      |
+-----------------------------------+--------------------+-------------------------------------------+-----------------------------------+-------------------------------------------+
| Frequent job restarts             | **Coordination**   | What is the cluster MTBF? Is detection    | Failure logs, MTBF calculation    | Elastic training, faster detection,       |
|                                   |                    | fast enough?                              |                                   | smaller blast radius                      |
+-----------------------------------+--------------------+-------------------------------------------+-----------------------------------+-------------------------------------------+
| High GPU-hours but slow progress  | **Coordination**   | What fraction of GPU-hours produce useful | Goodput ratio                     | Async checkpointing, reduce pipeline      |
|                                   |                    | training steps?                           | ($T_{useful}/T_{wall}$)           | stages, eliminate scheduler churn         |
+-----------------------------------+--------------------+-------------------------------------------+-----------------------------------+-------------------------------------------+
| Scaling efficiency drops with     | **Comm / Coord**   | Is the bottleneck network bandwidth or    | Separate $T_{comm}$ from          | If comm: compress or overlap.             |
| cluster size                      |                    | synchronization barriers?                 | $T_{coord}$                       | If coord: async methods                   |
+-----------------------------------+--------------------+-------------------------------------------+-----------------------------------+-------------------------------------------+
| Stragglers slow entire job        | **Comm $\cap$**    | Is one node consistently last to reach    | Per-node step time histogram      | Straggler detection + replacement,        |
|                                   | **Coord**          | the AllReduce barrier?                    |                                   | bounded staleness, backup workers         |
+-----------------------------------+--------------------+-------------------------------------------+-----------------------------------+-------------------------------------------+
...
+:===================+:==============================+:==============================+:==============================+
| **Computation**    | MFU, kernel utilization       | PyTorch Profiler              | Nsight Compute (per-kernel    |
|                    |                               | (TensorBoard plugin)          | roofline analysis)            |
+--------------------+-------------------------------+-------------------------------+-------------------------------+
| **Communication**  | AllReduce time, $\rho$ ratio  | NCCL debug logs               | Nsight Systems (timeline),    |
|                    |                               | (`NCCL_DEBUG=INFO`)           | `ibstat` / `perfquery` (IB)  |
+--------------------+-------------------------------+-------------------------------+-------------------------------+
| **Coordination**   | Goodput ratio, restart count  | Cluster scheduler logs        | Custom goodput dashboards     |
|                    |                               | (Slurm, K8s event logs)      | (e.g., Google ML Goodput)     |
+--------------------+-------------------------------+-------------------------------+-------------------------------+
...
The C$^3$ Scorecard grades your fleet's efficiency against known thresholds, paralleling the D·A·M Scorecard (@sec-dam-taxonomy). @tbl-c3-scorecard defines the three metrics that characterize fleet health.
...
$$\text{Effective} = `{python} peak_str` \times `{python} fmt(MFU_TRAINING_HIGH, precision=2, commas=False)` \times `{python} fmt(SCALING_EFF_8192GPU, precision=2, commas=False)` \times `{python} fmt(C3.goodput_all, precision=2, commas=False)` \approx `{python} eff_str` \text{ PFLOPS}$$
...
$$\text{Effective fraction} = 0.`{python} C3.mfu_high_str` \times 0.`{python} C3.eff_1024_str` \times 0.85 = `{python} fmt(MFU_TRAINING_HIGH * SCALING_EFF_1024GPU * 0.85 * 100, precision=1, commas=False)`\%$$

The C$^3$ tax is $1 / `{python} fmt(MFU_TRAINING_HIGH * SCALING_EFF_1024GPU * 0.85, precision=3, commas=False)` \approx `{python} fmt(1 / (MFU_TRAINING_HIGH * SCALING_EFF_1024GPU * 0.85), precision=1, commas=False)` \times$. To deliver $10^{24}$ effective FLOPS, you must provision $`{python} fmt(1 / (MFU_TRAINING_HIGH * SCALING_EFF_1024GPU * 0.85), precision=1, commas=False)` \times 10^{24} \approx `{python} fmt(1e24 / (MFU_TRAINING_HIGH * SCALING_EFF_1024GPU * 0.85) / 1e24, precision=1, commas=False)` \times 10^{24}$ raw peak FLOPS. This is the practical cost of the C$^3$ gap: scaling-law compute budgets must be inflated by the C$^3$ tax to account for real-world fleet overhead.
