...
class C3EffectiveFlops:
    ...
    effective_flops = calc_effective_flops(
        peak_flops, Literature.Training.MfuHigh, Literature.Scaling.Eff8192Gpu, goodput_all
    )
    c3_tax = (peak_flops / effective_flops).to("").magnitude
    eff_fraction = (effective_flops / peak_flops).to("").magnitude

    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    peak_pflops_str = fmt_qty(peak_flops, PFLOPs/second, precision=0, commas=True, unit_label="PFLOP/s")
    effective_pflops_str = fmt_qty(effective_flops, PFLOPs/second, precision=1, commas=False, unit_label="PFLOP/s")
