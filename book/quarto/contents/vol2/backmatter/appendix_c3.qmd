...
| ***(no equivalent)*** | *(overhead term $L_{lat}$)*   | **Coordination ($C_3$)**  | New axis: barriers, checkpoints, failure recovery, scheduling---negligible on one machine, dominant at 10K+ GPUs |
...
|:--------------------------|:-----------------------------------------------|:------------------------------------------------------------------------------|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------|
| **Computation ($C_1$)**   | Arithmetic throughput ($R_{peak} \times \eta$) | Low MFU, GPU utilization below 80%, poor per-GPU performance                  | MFU (Model FLOPS Utilization)                                             | Kernel optimization, mixed precision, operator fusion (@sec-performance-engineering)                       |
| **Communication ($C_2$)** | Network bandwidth ($BW_{net}$)                 | High AllReduce time, low scaling efficiency, communication > 30% of step      | Scaling efficiency ($\eta$), communication fraction ($T_{comm}/T_{step}$) | Gradient compression, overlap compute/communication, topology optimization (@sec-collective-communication) |
...
| **Scaling efficiency drops with cluster size** | **Comm / Coord**      | Is the bottleneck network bandwidth or synchronization barriers?             | Separate $T_{comm}$ from $T_{coord}$           | If comm: compress or overlap. If coord: async methods                  |
