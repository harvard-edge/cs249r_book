...
A `{python} C3CaseStudies.case2_n_gpus_str`-GPU data-parallel training run achieves `{python} C3CaseStudies.case2_mfu_pct_str` MFU on each individual GPU---good per-device efficiency. Yet scaling from `{python} C3CaseStudies.case2_baseline_gpus_str` GPUs to `{python} C3CaseStudies.case2_n_gpus_str` GPUs yields only `{python} C3CaseStudies.case2_observed_speedup_mult_str` speedup instead of the expected `{python} C3CaseStudies.case2_ideal_speedup_mult_str`, or `{python} C3CaseStudies.case2_scaling_efficiency_pct_str` scaling efficiency. NCCL profiling reveals that AllReduce consumes `{python} C3CaseStudies.case2_comm_pct_str` of every training step.
