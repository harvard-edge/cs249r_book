...
The fleet delivers `{python} C3.eff_fraction_pct_str`% of its peak capacity as useful training work. The **C$^3$ tax**---the ratio of peak to effective---is `{python} C3.c3_tax_str`$\times$: you need `{python} C3.c3_tax_str`$\times$ the raw hardware to achieve a given effective compute budget. Broken down by axis: Computation consumes a `{python} C3.mfu_pct_str`% factor (MFU), Communication consumes a `{python} C3.scaling_pct_str`% factor (scaling efficiency at this cluster size), and Coordination consumes a `{python} C3.goodput_pct_str`% factor (goodput ratio after pipeline bubbles, checkpoints, failures, and maintenance).
...
- **The C$^3$ tax is multiplicative**: Peak FLOPS$\times$ MFU$\times$ Scaling Efficiency$\times$ Goodput Ratio = Effective FLOPS. At 100,000 GPUs, expect only ~`{python} C3.eff_fraction_pct_str`% of peak.
