...
class DefenseOverheadAnalysis:
    ...
    daily_queries_str = fmt_count(daily_queries, scale='M', precision=0, commas=False)
    daily_users_str = fmt(daily_users, precision=0)
    training_cost_k_str = fmt_count(training_cost_usd, scale='K', precision=0, commas=False)
    ...
    attacker_daily_queries_str = fmt_count(attacker_daily_queries, scale='K', precision=0, commas=False)
    ...
    paid_tier_queries_str = fmt_count(paid_tier_queries, scale='K', precision=0, commas=False)
    alert_queries_per_day_str = fmt_count(alert_queries_per_day, scale='K', precision=0, commas=False)
    ...
    extraction_cost_k_str = fmt_count(extraction_cost_usd, scale='K', precision=0, commas=False)
...
class TEEMemoryFootprint:
    ...
    resnet18_params_m = resnet18_params.to(Mparam).magnitude
    resnet50_params_m = resnet50_params.to(Mparam).magnitude
    fp32_bytes_q = BYTES_FP32
    int8_bytes_q = BYTES_INT8
    fp32_bytes = fp32_bytes_q.to(byte).magnitude
    int8_bytes = int8_bytes_q.to(byte).magnitude
    sgx_epc_q = Hardware.Cloud.IntelSGX.memory.capacity
    sgx_epc_mb = sgx_epc_q.to(MB).magnitude
    sgx_latency_base_ms = Hardware.Cloud.IntelSGX.dispatch_tax.to(ms).magnitude
    ...
    small_model_limit_q = 10 * MB
    small_model_limit_mb = small_model_limit_q.to(MB).magnitude
    ...
    resnet18_fp32_mb = resnet18_fp32_q.to(MB).magnitude
    resnet18_int8_mb = resnet18_int8_q.to(MB).magnitude
    resnet50_fp32_mb = resnet50_fp32_q.to(MB).magnitude
    resnet50_int8_mb = resnet50_int8_q.to(MB).magnitude
    ...
    resnet50_fp32_epc_pct_str = fmt_percent(resnet50_fp32_epc_pct/100, precision=0, commas=False, style='prose')
    sgx_latency_base_ms_str = fmt_time(sgx_latency_base_ms, 'millisecond', precision=0, commas=False)
    sgx_latency_overflow_ms_str = fmt_time(sgx_latency_overflow_ms, 'millisecond', precision=0, commas=False)
...
::: {.column-margin}
![](images/svg/vol2_security_privacy_margin_001.svg){width="100%" fig-alt="Margin illustration showing SGX EPC capacity versus ResNet model sizes."}
...
class EncryptionOverhead:
    ...
    t_fhe_sec = (t_fhe_ms * ms).to(second).magnitude
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(t_fhe_sec == 200, f"FHE time {t_fhe_sec}s unexpected")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    eo_base_ms_str = fmt_time(t_inference_ms, 'millisecond', precision=0, commas=False)
    eo_aes_overhead_ms_str = fmt_time(overhead_aes_ms, 'millisecond', precision=1, commas=False)
    eo_aes_ms_str = fmt_time(t_aes, 'millisecond', precision=1, commas=False)
    eo_fhe_sec_str = fmt_time(t_fhe_sec, 'second', precision=0, commas=False, style='word')
