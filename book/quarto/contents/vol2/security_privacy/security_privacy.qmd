...
**The Implication**: Privacy is a budget, not a switch. Data cannot be "anonymized" once it is memorized by weights. Systems requiring strict privacy must implement **Differential Privacy** (DP) to quantify and cap the information leakage ($\epsilon$) per query, halting access when the budget is exhausted.
