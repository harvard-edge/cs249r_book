...
- Evaluate **hardware security primitives** (**TEEs**\index{TEEs}, **secure boot**\index{Secure Boot}, **HSMs**\index{HSMs}, **PUFs**) for ML workloads with quantitative overhead analysis
...
**Problem**\index{Problem}: Consider computing the average salary of **`{python} DPCostAnalysis.n_employees` employees** while guaranteeing privacy ($\epsilon=`{python} DPCostAnalysis.epsilon`). The salaries range from \$0 to \$`{python} DPCostAnalysis.sensitivity_str`. How much noise must the mechanism add?
...
![](images/svg/stuxnet.svg){width=100 percent}
...
![](images/svg/model-theft-types.svg){width=100 percent}
:::
...
**Scenario**\index{Scenario}: An autonomous vehicle's vision system misclassifies a stop sign as a speed limit sign due to a specially crafted sticker placed on the sign. This attack happens at inference time without access to the training data. This is an example of:
...
![](images/png/fault-injection_demonstrated_with_assembly_code.png){width=75 percent}
...
**Conclusion**\index{Conclusion}: Achieving $\epsilon=1.0$ requires adding noise nearly **5$\times$ larger** than the signal (gradient norm 1.0). This degrades accuracy by 5--10 percent unless the model trains for significantly longer or uses much larger batch sizes to average out the noise.
...
The analysis demonstrates the fundamental privacy-utility-computational resource tradeoff in production ML systems.
...
* **Differential Privacy is the Standard**\index{Differential Privacy Is the Standard}: Mathematical privacy guarantees (ε, δ) are superior to naive anonymization. Implementation requires careful budget management throughout the model lifecycle to prevent "privacy exhaustion."
