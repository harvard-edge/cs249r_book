...
Sweeping the bound across $\alpha$ (@tbl-security-privacy-dp-sgd-alpha-sweep) shows the slope and tail terms trading off, with a minimum near $\alpha \approx$ `{python} DPSGDBudget.alpha_opt_str`. Optimizing the displayed approximation gives $\epsilon \approx$ `{python} DPSGDBudget.eps_opt_str`, which satisfies the target of $\epsilon \leq$ `{python} DPSGDBudget.eps_target_str` with margin.
...
|:------------:|:------------------------------------:|:-----------------------------------:|:----------------------------------:|
|      8       | `{python} DPSGDBudget.slope_a8_str`  |  `{python} DPSGDBudget.tail_a8_str` | `{python} DPSGDBudget.eps_a8_str`  |
|      10      | `{python} DPSGDBudget.slope_a10_str` | `{python} DPSGDBudget.tail_a10_str` | `{python} DPSGDBudget.eps_a10_str` |
|      12      | `{python} DPSGDBudget.slope_a12_str` | `{python} DPSGDBudget.tail_a12_str` | `{python} DPSGDBudget.eps_a12_str` |

: **RDP-to-DP conversion across Rényi orders**: The privacy bound $\epsilon$ is the sum of a slope term that grows with $\alpha$ and a tail term that shrinks with $\alpha$; the released budget is the minimum over orders, near $\alpha \approx$ `{python} DPSGDBudget.alpha_opt_str`. {#tbl-security-privacy-dp-sgd-alpha-sweep}

The guarantee itself comes from clipping, calibrated noise, and the privacy accountant; the library call in @lst-security-privacy-dp-sgd-step is only an implementation path for those mechanism choices.
