...
* **Artifact provenance is a security control**: Signed datasets, dependency locks, model registry permissions, and deployment attestation form one release chain. A fleet cannot roll back or investigate a compromised model if the checkpoint cannot be tied to the data, code, and principal that produced it.
