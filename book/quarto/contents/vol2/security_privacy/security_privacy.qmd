...
# │ Exports: HsmGpuCrypto.throughput_ratio_str, hsm_cost_low_str, hsm_cost_high_str
# └─────────────────────────────────────────────────────────────────────────────
...
class GaussianNoiseScale:
    ...
    sigma_signal_ratio_str = fmt(sigma_signal_ratio, precision=1, commas=False)
...
*   **Strong Privacy** $(\epsilon =$ `{python} GaussianNoiseScale.eps_strong_str` $)$: $\sigma \approx$ `{python} GaussianNoiseScale.clip_norm_str` $\times \sqrt{2 \times$ `{python} GaussianNoiseScale.ln_term_str` $} /$ `{python} GaussianNoiseScale.eps_strong_str` $\approx$ `{python} GaussianNoiseScale.sigma_strong_str`
*   **Weak Privacy** $(\epsilon =$ `{python} GaussianNoiseScale.eps_weak_str` $)$: $\sigma \approx$ `{python} GaussianNoiseScale.clip_norm_str` $\times \sqrt{2 \times$ `{python} GaussianNoiseScale.ln_term_str` $} /$ `{python} GaussianNoiseScale.eps_weak_str` $\approx$ `{python} GaussianNoiseScale.sigma_weak_str`

**Systems insight**: Achieving $\epsilon=$ `{python} GaussianNoiseScale.eps_strong_str` requires adding noise nearly `{python} GaussianNoiseScale.sigma_signal_ratio_str`$\times$ larger than the signal (gradient norm `{python} GaussianNoiseScale.clip_norm_str`). This degrades accuracy by 5--10 percent unless the model trains for significantly longer or uses much larger batch sizes to average out the noise.
...
# │
# │ Imports: math, numpy, mlsysim (Datasets, units), mlsysim.fmt (fmt, check)
...
class PracticalDPBudget:
    ...
    n_records = int(Datasets.CIFAR10.training_examples.m_as(count))  # 50,000
...
1. Per-step RDP: $\varepsilon_{\text{step}}(\alpha) \approx \frac{q^2 \alpha}{2\sigma^2} = \frac{($ `{python} PracticalDPBudget.q_str` $)^2 \alpha}{2($ `{python} PracticalDPBudget.sigma_str` $)^2} \approx$ `{python} PracticalDPBudget.eps_step_coef_str` $\alpha$

2. Total RDP after `{python} PracticalDPBudget.k_steps_str` steps: $\varepsilon_{\text{total}}(\alpha) =$ `{python} PracticalDPBudget.k_steps_str` $\cdot$ `{python} PracticalDPBudget.eps_step_coef_str` $\alpha =$ `{python} PracticalDPBudget.eps_total_coef_str` $\alpha$
