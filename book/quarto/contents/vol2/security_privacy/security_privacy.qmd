...
For $k=$ `{python} OutputRoundingLeakage.top_k_str` on a `{python} OutputRoundingLeakage.class_count_value_str`-class problem, this reduces information leakage by $\log_2(1000/5) \approx$ `{python} OutputRoundingLeakage.topk_bits_str` per query. Legitimate users rarely need beyond top-5 predictions, making this a practical defense.
...
class DefenseOverheadAnalysis:
    ...
    perturbation_ms_str = fmt_time(perturbation_ms, 'millisecond', precision=1, commas=False)
...
Consider a production API serving a ResNet-50 image classifier (`{python} DefenseOverheadAnalysis.class_count_value_str` ImageNet classes) with `{python} DefenseOverheadAnalysis.daily_queries_str` daily queries from `{python} DefenseOverheadAnalysis.daily_users_str`.
...
- Extraction requirements: ~`{python} DefenseOverheadAnalysis.extraction_queries_m_str` queries for `{python} DefenseOverheadAnalysis.target_fidelity_pct_str` fidelity, or `{python} DefenseOverheadAnalysis.queries_per_class_str` for a `{python} DefenseOverheadAnalysis.class_count_value_str`-class classifier
- Without defenses: attacker queries `{python} DefenseOverheadAnalysis.attacker_queries_per_day_str` for `{python} DefenseOverheadAnalysis.extraction_days_str`, cost \$0 (free tier abuse)
...
1.  **Rate limiting**:
    - Free tier: `{python} DefenseOverheadAnalysis.free_tier_queries_per_day_str` (prevents sustained extraction)
    - Paid tier (`{python} DefenseOverheadAnalysis.paid_tier_price_usd_month_str`): `{python} DefenseOverheadAnalysis.paid_tier_queries_per_day_str`
...
2.  **Query monitoring**:
    ...
    - Alert on: >`{python} DefenseOverheadAnalysis.alert_queries_per_day_str` from single user, <`{python} DefenseOverheadAnalysis.alert_interquery_ms_str` median inter-query time, input entropy <threshold
...
4.  **Economic deterrent**:
    - Pricing: `{python} DefenseOverheadAnalysis.price_per_query_usd_str` for paid tier
    ...
    - Legitimate user cost: `{python} DefenseOverheadAnalysis.legit_queries_per_day_str` $\times$ `{python} DefenseOverheadAnalysis.billing_days_str` $\times$ `{python} DefenseOverheadAnalysis.price_per_query_usd_str` = `{python} DefenseOverheadAnalysis.legit_monthly_cost_usd_str` per month (acceptable)

**Quantitative impact**:

-   **Extraction fidelity**: Without defenses, `{python} DefenseOverheadAnalysis.output_accuracy_no_defense_pct_str` accuracy surrogate. With defenses, surrogate accuracy drops to `{python} DefenseOverheadAnalysis.output_accuracy_with_defense_pct_str`, an `{python} DefenseOverheadAnalysis.fidelity_drop_pp_str` reduction in this scenario.
...
-   **False positive rate**: `{python} DefenseOverheadAnalysis.false_positive_pct_str` of users flagged for manual review (`{python} DefenseOverheadAnalysis.daily_users_str` → `{python} DefenseOverheadAnalysis.false_positive_reviews_per_day_str`, manageable)
