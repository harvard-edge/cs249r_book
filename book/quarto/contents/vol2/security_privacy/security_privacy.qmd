...
![](images/svg/model-theft-types.svg){width=100%}
...
The most direct signal is query volume. Legitimate users typically exhibit bursty, irregular usage patterns, while extraction attacks require sustained high-volume querying. Tracking per-user query counts over sliding time windows provides a first-order detection mechanism. For an image classification API, a threshold might be:
...
Volume alone is insufficient because a sophisticated attacker can throttle query rates below detection thresholds. A stronger signal comes from the queries themselves: extraction attacks often use synthetic or systematically generated inputs that differ markedly from natural user queries. Detection systems quantify this difference by computing distributional divergence between a user's query distribution and the expected input distribution:
...
A third detection axis exploits the temporal regularity of automated tools. Extraction attacks exhibit distinctive timing signatures: low variance in inter-query intervals (compared to the high variance of human users), systematic rather than task-specific sampling patterns, and long uninterrupted sessions that differ from typical browsing behavior.
...
The simplest approach assigns fixed quotas based on authentication level and payment tier:
...
Fixed tiers raise the cost of extraction but treat all users within a tier identically, allowing a patient attacker with an enterprise account to extract freely. Adaptive rate limiting addresses this gap by dynamically adjusting limits based on the anomaly scores from the detection layer. Users with high anomaly scores face progressively stricter limits, while verified legitimate users receive relaxed quotas:
...
The most straightforward technique rounds output probabilities to coarser precision. Instead of returning full-precision logits or probabilities, the API truncates each class probability:
...
Rounding reduces precision per class, but the attacker still receives a probability for every class. A complementary defense restricts the number of classes returned, exposing only the top-$k$ predicted classes rather than the full probability distribution:
...
Both rounding and truncation are deterministic: given the same input, the API returns the same perturbed output, allowing an attacker to average out the perturbation over repeated queries. Adding calibrated random noise to each response breaks this determinism while preserving decision quality:
...
Noise injection can be formalized through differential privacy, which provides mathematically rigorous guarantees rather than ad-hoc perturbation. For a query $q$, the privatized response satisfies $(\epsilon, \delta)$-DP:
...
- Return only class predictions, not full probability distributions or logits
- Replace numeric probabilities with coarse confidence bins ("high", "medium", "low")
- Prevent access to hidden layer activations, attention weights, or embeddings
- Abstract away model version details that would enable targeted extraction
...
**Conclusion**: Achieving $\epsilon=1.0$ requires adding noise nearly **5$\times$ larger** than the signal (gradient norm 1.0). This degrades accuracy by 5--10% unless the model trains for significantly longer or uses much larger batch sizes to average out the noise.
...
The maximum acceptable accuracy degradation must be quantified before deployment. For safety-critical applications (medical diagnosis, autonomous vehicles), even 5% accuracy loss may be unacceptable. For recommendation systems or content personalization, 10--15% degradation might be tolerable given the privacy benefits. @tbl-dp-accuracy-tradeoff provides a starting point, followed by experiments on the specific task.
