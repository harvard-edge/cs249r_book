...
Privacy failures are not limited to raw records or explicit identifiers. The Strava heatmap incident shows how aggregated behavioral traces can reveal sensitive routines when the surrounding geography makes individuals and sites easy to infer.
...
The distinction between training-time and inference-time attacks is easiest to verify in a concrete deployment scenario. An autonomous vehicle's vision system misclassifies a stop sign as a speed limit sign due to a specially crafted sticker placed on the sign. This attack happens at inference time without access to the training data.
