...
\noindent
![](images/png/cover_security_privacy_blueprint_print.png){fig-alt="Security and privacy boundary chamber separating data, model weights, runtime, users, and audit logs into protected and untrusted zones."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_security_privacy_blueprint.webp){fig-alt="Security and privacy boundary chamber separating data, model weights, runtime, users, and audit logs into protected and untrusted zones."}
:::
...
The confidentiality-integrity-availability triad matters because each property breaks a different system contract: confidentiality protects data and model access, integrity protects the correctness of training and inference behavior, and availability protects the service from being disabled or degraded when it is needed.
...
2.  **Distinction**: Unlike confidentiality (which focuses on access control), privacy in ML focuses on inference risks\index{Inference!risks}: the ability of an observer to reconstruct sensitive training samples from the model's outputs or weights.
...
**Systems insight**: Protecting one outlier introduces a `{python} DPCostAnalysis.error_per_person_usd_str` error in the average. For a dataset of $n_{\text{records}}=$ `{python} DPCostAnalysis.n_small_str`, the error grows to `{python} DPCostAnalysis.error_small_usd_str`. Differential privacy can make estimates unstable for small $n_{\text{records}}$; it works best when the dataset is large enough that $1/n_{\text{records}}$ dampens the added noise. In an ML training context, the salary average maps directly to the gradient update computed across a mini-batch: the "sum" being protected is the sum of per-example gradients, the sensitivity is the maximum gradient norm any one training example can contribute (the clipping threshold in DP-SGD), and the injected noise scales by the same $\Delta f / \epsilon$ ratio. Protecting one user's highly anomalous gradient from disproportionately shifting a model's decision boundary during federated learning is mathematically identical to protecting one outlier salary from skewing the reported mean.
...
**Setup**: By querying a victim BERT-based API with 2 million carefully crafted inputs (costing roughly \$50 in query fees), they trained a student model that achieved more than 97 percent agreement with the victim on test tasks [@krishna2020thieves]. This model-stealing attack exploited the high-information signal returned by confidence scores and logits.
...
class DefenseOverheadAnalysis:
    ...
    legit_monthly_cost_usd = paid_tier_price_usd_month
    ...
    check(legit_monthly_cost_usd == paid_tier_price_usd_month, "Legitimate user should pay the flat paid tier")
...
The same inference-time framing extends to LLMs, but the attack surface changes because the model interface mixes instructions, content, retrieval context, and policy controls in one token stream [@gupta2023]. Tool-using systems add an additional boundary: model text can trigger external side effects. A secure LLM system therefore separates five objects that the model tends to collapse: instructions, untrusted content, retrieved documents, tool permissions, and secrets. Three recurring failure modes organize the space: prompt injection breaks the boundary between control and data, training data extraction exposes memorized records, and jailbreaking bypasses alignment controls.
...
**Scenario**: An autonomous vehicle's vision system misclassifies a stop sign as a speed limit sign due to a specially crafted sticker placed on the sign. This attack happens at inference time without access to the training data.
...
One of the most widely studied examples involves Advanced Encryption Standard (AES)[^fn-aes-standard] implementations, and its lessons transfer directly to ML workloads. While AES is mathematically secure, the physical process of computing its encryption functions leaks measurable signals---and the same principle holds for the ReLU activations, softmax normalizations, and weight-load sequences that constitute a forward pass.
...
Once immediate containment is in place, investigation and recovery can begin. This may include forensic analysis of input logs, parameter deltas between model versions, or memory snapshots from inference containers. Generative and tool-using systems add their own evidence requirements: investigators need prompts, retrieved documents, tool calls, policy-classifier decisions, and secret-access logs preserved with enough context to separate privacy breach, model-integrity failure, and ordinary model error. In regulated environments, organizations may also need to notify users or auditors, particularly if personal or safety-important data was affected.
...
[^fn-open-bootloaders]: **Open-Source Secure Boot Stacks**\index{Secure Boot!open-source stacks}: U-Boot [@uboot2026source] is the de facto bootloader for embedded Linux systems, including Raspberry Pi-class devices used in edge ML deployments; coreboot [@coreboot2026project] targets x86 server and workstation hardware. Both implement signature-verified boot chains, but the trust root remains the OEM key fused into silicon---so "open-source" applies to the bootloader code, not to the root-of-trust that anchors the ML model's execution environment.
...
# ┌─────────────────────────────────────────────────────────────────────────────
# │ GAUSSIAN-MECHANISM NOISE SCALE (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: #exmp-security-privacy-privacy-accuracy-tax + the sigma~5.72 prose.
# │
# │ Goal: Pin the (eps,delta)-DP Gaussian noise scale sigma >= C*sqrt(2 ln(1.25/
# │       delta))/eps for the privacy-accuracy-tax scenario, and the typical-ML
# │       coefficient sigma ~ 5.72 * sensitivity at delta = 1e-7.
# │ Show: strong eps=1 -> sigma~4.8, weak eps=10 -> sigma~0.48; coef~5.72.
# │ How: sigma = C * sqrt(2 ln(1.25/delta)) / eps.
# │
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt, fmt_math, fmt_multiple, check

class GaussianNoiseScale:
    """Gaussian-mechanism noise scale sigma for (eps, delta)-DP."""

    # ┌── 1. LOAD (scenario assumptions) ───────────────────────────────────
    clip_norm = 1.0  # gradient clipping norm C (= sensitivity Delta_2)
    delta_callout = 1e-5  # failure probability for the worked callout
    eps_strong = 1.0  # strong-privacy budget
    eps_weak = 10.0  # weak-privacy budget
    delta_typical = 1e-7  # typical-ML failure probability for the 5.72 coef

    # ┌── 2. EXECUTE (noise scale) ─────────────────────────────────────────
    ln_term = math.log(1.25 / delta_callout)  # ~11.74
    noise_factor = math.sqrt(2 * ln_term)  # sqrt(2 ln(1.25/delta))
    sigma_strong = clip_norm * noise_factor / eps_strong
    sigma_weak = clip_norm * noise_factor / eps_weak
    sigma_signal_ratio = sigma_strong / clip_norm  # noise vs gradient norm
    coef_typical = math.sqrt(2 * math.log(1.25 / delta_typical)) / eps_strong

    # ┌── 3. GUARD (anchors) ───────────────────────────────────────────────
    check(abs(sigma_strong - 4.8) < 0.1, f"strong sigma {sigma_strong}")
    check(abs(sigma_weak - 0.48) < 0.01, f"weak sigma {sigma_weak}")
    check(abs(coef_typical - 5.72) < 0.05, f"typical coef {coef_typical}")
    ...
    ln_term_str = fmt(ln_term, precision=1, commas=False)
    sigma_strong_str = fmt(sigma_strong, precision=1, commas=False)
    sigma_weak_str = fmt(sigma_weak, precision=2, commas=False)
    sigma_signal_ratio_mult_str = fmt_multiple(sigma_signal_ratio, precision=1, commas=False)
    coef_typical_str = fmt(coef_typical, precision=2, commas=False)
    sigma_strong_math = fmt_math(
        rf"\sigma \approx {clip_norm_str} \times \sqrt{{2 \times {ln_term_str}}} / {eps_strong_str} \approx {sigma_strong_str}"
    )
    sigma_weak_math = fmt_math(rf"\sigma \approx {clip_norm_str} \times \sqrt{{2 \times {ln_term_str}}} / {eps_weak_str} \approx {sigma_weak_str}")
```
...
*   **Gradient Norm Limit** $(C)$: `{python} GaussianNoiseScale.clip_norm_str`
*   **Failure Probability** $(\delta)$: $10^{-5}$

**Result** $(\sigma)$:

*   **Strong Privacy** $(\epsilon =$ `{python} GaussianNoiseScale.eps_strong_str` $)$: `{python} GaussianNoiseScale.sigma_strong_math`
*   **Weak Privacy** $(\epsilon =$ `{python} GaussianNoiseScale.eps_weak_str` $)$: `{python} GaussianNoiseScale.sigma_weak_math`

**Systems insight**: Achieving $\epsilon=$ `{python} GaussianNoiseScale.eps_strong_str` requires adding noise nearly `{python} GaussianNoiseScale.sigma_signal_ratio_mult_str` larger than the signal (gradient norm `{python} GaussianNoiseScale.clip_norm_str`). This degrades accuracy by 5--10 percent unless the model trains for significantly longer or uses much larger batch sizes to average out the noise.

:::

The noise scale must satisfy $\sigma \geq \frac{\Delta_2 f}{\epsilon}\sqrt{2\ln(1.25/\delta)}$ to achieve $(\epsilon, \delta)$-DP. For typical ML hyperparameters $(\epsilon = 1, \delta = 10^{-7})$, this requires $\sigma \approx$ `{python} GaussianNoiseScale.coef_typical_str` $\cdot \Delta_2 f$.
...
```{python}
#| echo: false
#| label: moments-accountant-calc
# ┌─────────────────────────────────────────────────────────────────────────────
# │ MOMENTS-ACCOUNTANT VS NAIVE COMPOSITION (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: ResNet-20/CIFAR-10 DP-SGD privacy-accounting comparison prose.
# │
# │ Goal: Pin the published moments-accountant epsilon and the naive-composition
# │       epsilon it improves on (the "~10x tighter" accounting result).
# │ Show: moments accountant eps ~2.3 vs naive composition eps ~23.
# │ How: naive = moments * tightness_factor (the reported accounting gain).
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check

class MomentsAccountant:
    """Moments-accountant vs naive-composition privacy budget (@abadi2016deep)."""

    # ┌── 1. LOAD (published result + scenario inputs) ─────────────────────
    clip_norm = 1.0  # gradient clipping norm C
    eps_moments = 2.3  # @abadi2016deep moments-accountant epsilon (delta=1e-5)
    tightness_factor = 10  # accounting gain over naive composition (~10x)

    # ┌── 2. EXECUTE (naive composition bound) ────────────────────────────
    eps_naive = eps_moments * tightness_factor

    # ┌── 3. GUARD (anchor) ────────────────────────────────────────────────
    check(abs(eps_naive - 23.0) < 0.05, f"naive eps {eps_naive}")

    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    ma_clip_norm_str = fmt(clip_norm, precision=0, commas=False)
    ma_eps_moments_str = fmt(eps_moments, precision=1, commas=False)
    ma_eps_naive_str = fmt(eps_naive, precision=0, commas=False)
```

For DP-SGD training a ResNet-20 on CIFAR-10 with 100 epochs, batch size 256, and clipping norm $C=$ `{python} MomentsAccountant.ma_clip_norm_str`, the moments accountant yields $\epsilon \approx$ `{python} MomentsAccountant.ma_eps_moments_str` (for $\delta=10^{-5}$), compared to $\epsilon \approx$ `{python} MomentsAccountant.ma_eps_naive_str` under naive composition. This tighter accounting makes private deep learning practically feasible.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: "Practical privacy budget example" prose worked example.
# │
# │ Goal: Pin the RDP accounting for CIFAR-10 DP-SGD: sampling rate, sensitivity,
# │       step count, per-step and total RDP coefficients, and the (eps,delta)
# │       conversion swept over the Renyi order alpha.
# │ Show: q=0.08, Delta_2=0.0005, k=750, per-step ~0.00189, total ~1.42, eps~9.5.
# │ How: q=B/n; eps_step = q^2/(2 sigma^2); eps(alpha)=total*alpha+ln(1/delta)/(alpha-1).
# │
# └─────────────────────────────────────────────────────────────────────────────
import math
import numpy as np
...
class PracticalDPBudget:
    """RDP accounting for a CIFAR-10 DP-SGD privacy budget."""
    ...
    alpha_lo = 4  # displayed Renyi orders bracketing the optimum
    alpha_hi = 3
    alpha_too_high = 10

    # ┌── 2. EXECUTE (RDP accounting) ──────────────────────────────────────
    q = batch_size / n_records  # subsampling rate
    sensitivity = 2 * clip_norm / batch_size  # Delta_2 = 2C/B
    k_steps = int(n_epochs * n_records / batch_size)
    eps_step_coef = q**2 / (2 * sigma**2)  # per-step RDP slope in alpha
    eps_total_coef = k_steps * eps_step_coef  # total RDP slope in alpha
    ln_inv_delta = math.log(1 / delta)
    _grid = np.linspace(1.1, 20, 400_000)
    _eps = eps_total_coef * _grid + ln_inv_delta / (_grid - 1)
    alpha_opt = float(_grid[_eps.argmin()])
    eps_opt = float(_eps.min())
    eps_alpha_too_high = eps_total_coef * alpha_too_high + ln_inv_delta / (alpha_too_high - 1)
    eps_alpha_lo = eps_total_coef * alpha_lo + ln_inv_delta / (alpha_lo - 1)
    eps_alpha_hi = eps_total_coef * alpha_hi + ln_inv_delta / (alpha_hi - 1)

    # ┌── 3. GUARD (anchors) ───────────────────────────────────────────────
    check(abs(q - 0.08) < 1e-6, f"q {q}")
    check(abs(sensitivity - 0.0005) < 1e-9, f"sensitivity {sensitivity}")
    check(k_steps == 750, f"k_steps {k_steps}")
    check(abs(eps_step_coef - 0.00189) < 1e-4, f"per-step {eps_step_coef}")
    check(abs(eps_total_coef - 1.42) < 1e-2, f"total {eps_total_coef}")
    check(abs(eps_opt - 9.5) < 0.2, f"eps_opt {eps_opt}")
    ...
    sensitivity_str = fmt(sensitivity, precision=4, commas=False)
    k_steps_str = fmt(k_steps, precision=0, commas=False)
    eps_step_coef_str = fmt(eps_step_coef, precision=5, commas=False)
    eps_total_coef_str = fmt(eps_total_coef, precision=2, commas=False)
    alpha_opt_str = fmt(alpha_opt, precision=1, commas=False)
    eps_opt_str = fmt(eps_opt, precision=1, commas=False)
    # alpha-sweep line components (slope, tail, total) at the three orders.
    slope_too_high_str = fmt(eps_total_coef * alpha_too_high, precision=1, commas=False)
    tail_too_high_str = fmt(ln_inv_delta / (alpha_too_high - 1), precision=2, commas=False)
    eps_too_high_str = fmt(eps_alpha_too_high, precision=2, commas=False)
    slope_lo_str = fmt(eps_total_coef * alpha_lo, precision=2, commas=False)
    tail_lo_str = fmt(ln_inv_delta / (alpha_lo - 1), precision=2, commas=False)
    eps_lo_str = fmt(eps_alpha_lo, precision=2, commas=False)
    slope_hi_str = fmt(eps_total_coef * alpha_hi, precision=2, commas=False)
    tail_hi_str = fmt(ln_inv_delta / (alpha_hi - 1), precision=2, commas=False)
    eps_hi_str = fmt(eps_alpha_hi, precision=2, commas=False)
    eps_step_math = fmt_math(
        rf"\varepsilon_{{\text{{step}}}}(\alpha) \approx "
        rf"\frac{{q^2 \alpha}}{{2\sigma^2}} = "
        rf"\frac{{({q_str})^2 \alpha}}{{2({sigma_str})^2}} "
        rf"\approx {eps_step_coef_str}\alpha"
    )
    eps_total_math = fmt_math(
        rf"\varepsilon_{{\text{{total}}}}(\alpha) = {k_steps_str} \times " rf"{eps_step_coef_str}\alpha = {eps_total_coef_str}\alpha"
    )
```
...
- Batch size $B =$ `{python} PracticalDPBudget.batch_size_str` (sampling rate $q = B/n_{\text{records}} =$ `{python} PracticalDPBudget.q_str`)
- Gradient clipping norm $C =$ `{python} PracticalDPBudget.clip_norm_str` (sensitivity $\Delta_2 = 2C/B =$ `{python} PracticalDPBudget.sensitivity_str`)
...
1. Per-step RDP: `{python} PracticalDPBudget.eps_step_math`

2. Total RDP after `{python} PracticalDPBudget.k_steps_str` steps: `{python} PracticalDPBudget.eps_total_math`

3. Converting to $(\epsilon, \delta)$-DP, we optimize over $\alpha$:
   - For $\alpha = 10$: $\epsilon =$ `{python} PracticalDPBudget.eps_total_coef_str` $\times 10 + \ln(10^5)/9 =$ `{python} PracticalDPBudget.slope_too_high_str` $+$ `{python} PracticalDPBudget.tail_too_high_str` $=$ `{python} PracticalDPBudget.eps_too_high_str` (too high)
   - For $\alpha = 4$: $\epsilon =$ `{python} PracticalDPBudget.eps_total_coef_str` $\times 4 + \ln(10^5)/3 =$ `{python} PracticalDPBudget.slope_lo_str` $+$ `{python} PracticalDPBudget.tail_lo_str` $=$ `{python} PracticalDPBudget.eps_lo_str`
   - For $\alpha = 3$: $\epsilon =$ `{python} PracticalDPBudget.eps_total_coef_str` $\times 3 + \ln(10^5)/2 =$ `{python} PracticalDPBudget.slope_hi_str` $+$ `{python} PracticalDPBudget.tail_hi_str` $=$ `{python} PracticalDPBudget.eps_hi_str`
   - Optimal $\alpha \approx$ `{python} PracticalDPBudget.alpha_opt_str` yields $\epsilon \approx$ `{python} PracticalDPBudget.eps_opt_str`

To reach the target $\epsilon =$ `{python} PracticalDPBudget.eps_target_str`, we must increase $\sigma$ substantially (more noise tightens privacy at the cost of roughly 5 percent accuracy drop), train for fewer epochs (e.g., $N_{\text{epochs}} = 25$) at the risk of underfitting, or reduce the sampling rate by decreasing batch size to $B = 2000$ $(q = 0.04)$ at the cost of more optimizer steps and noisier gradient estimates. These adjustments demonstrate the fundamental privacy-utility-computational resource trade-off[^fn-privacy-utility-tension] in production ML systems: differential privacy offers strong theoretical assurances, but tightening the privacy bound consumes accuracy and compute budget in measurable ways.
...
```{python}
#| echo: false
#| label: dp-accuracy-tradeoff-calc
# ┌─────────────────────────────────────────────────────────────────────────────
# │ DP-SGD ACCURACY TRADE-OFF TABLE (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @tbl-dp-accuracy-tradeoff
# │
# │ Goal: Compute the accuracy-point degradation (pp) of DP-SGD at eps=8 and
# │       eps=1 versus the non-private baseline for each benchmark row.
# │ Show: e.g. MNIST loses 1.1 pp at eps=8 and 2.6 pp at eps=1.
# │ How: delta = private_acc - nonprivate_acc, per cell.
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt_pp, check

# ┌── 1. LOAD (published accuracies, percent) ──────────────────────────────
# Source: @abadi2016deep, @bu2020gaussian, @de2022dp (delta = 1e-5).
_dp_base = dict(mnist=99.2, cifar10=93.5, imagenet=76.1, imdb=93.0)
_dp_eps8 = dict(mnist=98.1, cifar10=83.0, imagenet=47.8, imdb=88.5)
_dp_eps1 = dict(mnist=96.6, cifar10=67.0, imagenet=None, imdb=82.0)

# ┌── 2. EXECUTE (degradation in percentage points) ────────────────────────
_dp_eps8_pp = {k: round(_dp_eps8[k] - _dp_base[k], 1) for k in _dp_base}
_dp_eps1_pp = {k: (round(_dp_eps1[k] - _dp_base[k], 1) if _dp_eps1[k] is not None else None) for k in _dp_base}

# ┌── 3. GUARD (deltas negative, MNIST/CIFAR anchors) ──────────────────────
check(_dp_eps1_pp["mnist"] == -2.6, f"MNIST eps=1 loss {_dp_eps1_pp['mnist']} pp")
check(_dp_eps1_pp["cifar10"] == -26.5, f"CIFAR eps=1 loss {_dp_eps1_pp['cifar10']} pp")
check(
    all(_dp_eps8_pp[k] < 0 for k in _dp_base) and all(v < 0 for v in _dp_eps1_pp.values() if v is not None),
    "DP accuracy deltas must all be degradations",
)

# ┌── 4. OUTPUT (per-row strings for the table) ────────────────────────────
# Adaptive precision: integer-like values render without a dead ".0"
# (house style: "83%", not "83.0%"); fractional values keep one decimal.
def _dp_prec(_x):
    return 0 if abs(_x - round(_x)) < 1e-9 else 1

dpacc_mnist_base_str = fmt_percent(_dp_base["mnist"] / 100, precision=_dp_prec(_dp_base["mnist"]), commas=False, style="symbol")
dpacc_mnist_eps8_str = fmt_percent(_dp_eps8["mnist"] / 100, precision=_dp_prec(_dp_eps8["mnist"]), commas=False, style="symbol")
dpacc_mnist_eps8_pp_str = fmt_pp(_dp_eps8_pp["mnist"], precision=_dp_prec(_dp_eps8_pp["mnist"]), commas=False, style="symbol")
dpacc_mnist_eps1_str = fmt_percent(_dp_eps1["mnist"] / 100, precision=_dp_prec(_dp_eps1["mnist"]), commas=False, style="symbol")
dpacc_mnist_eps1_pp_str = fmt_pp(_dp_eps1_pp["mnist"], precision=_dp_prec(_dp_eps1_pp["mnist"]), commas=False, style="symbol")

dpacc_cifar10_base_str = fmt_percent(_dp_base["cifar10"] / 100, precision=_dp_prec(_dp_base["cifar10"]), commas=False, style="symbol")
dpacc_cifar10_eps8_str = fmt_percent(_dp_eps8["cifar10"] / 100, precision=_dp_prec(_dp_eps8["cifar10"]), commas=False, style="symbol")
dpacc_cifar10_eps8_pp_str = fmt_pp(_dp_eps8_pp["cifar10"], precision=_dp_prec(_dp_eps8_pp["cifar10"]), commas=False, style="symbol")
dpacc_cifar10_eps1_str = fmt_percent(_dp_eps1["cifar10"] / 100, precision=_dp_prec(_dp_eps1["cifar10"]), commas=False, style="symbol")
dpacc_cifar10_eps1_pp_str = fmt_pp(_dp_eps1_pp["cifar10"], precision=_dp_prec(_dp_eps1_pp["cifar10"]), commas=False, style="symbol")

dpacc_imagenet_base_str = fmt_percent(_dp_base["imagenet"] / 100, precision=_dp_prec(_dp_base["imagenet"]), commas=False, style="symbol")
dpacc_imagenet_eps8_str = fmt_percent(_dp_eps8["imagenet"] / 100, precision=_dp_prec(_dp_eps8["imagenet"]), commas=False, style="symbol")
dpacc_imagenet_eps8_pp_str = fmt_pp(_dp_eps8_pp["imagenet"], precision=_dp_prec(_dp_eps8_pp["imagenet"]), commas=False, style="symbol")

dpacc_imdb_base_str = fmt_percent(_dp_base["imdb"] / 100, precision=_dp_prec(_dp_base["imdb"]), commas=False, style="symbol")
dpacc_imdb_eps8_str = fmt_percent(_dp_eps8["imdb"] / 100, precision=_dp_prec(_dp_eps8["imdb"]), commas=False, style="symbol")
dpacc_imdb_eps8_pp_str = fmt_pp(_dp_eps8_pp["imdb"], precision=_dp_prec(_dp_eps8_pp["imdb"]), commas=False, style="symbol")
dpacc_imdb_eps1_str = fmt_percent(_dp_eps1["imdb"] / 100, precision=_dp_prec(_dp_eps1["imdb"]), commas=False, style="symbol")
dpacc_imdb_eps1_pp_str = fmt_pp(_dp_eps1_pp["imdb"], precision=_dp_prec(_dp_eps1_pp["imdb"]), commas=False, style="symbol")
```

| **Dataset**     | **Model** |        **Non-Private Acc.**        |                           **$\epsilon=8$ Acc.**                            |                          **$\epsilon=1$ Acc.**                           |
|:----------------|:---------:|:----------------------------------:|:--------------------------------------------------------------------------:|:------------------------------------------------------------------------:|
| **MNIST**       |    CNN    |  `{python} dpacc_mnist_base_str`   |    `{python} dpacc_mnist_eps8_str` (`{python} dpacc_mnist_eps8_pp_str`)    |   `{python} dpacc_mnist_eps1_str` (`{python} dpacc_mnist_eps1_pp_str`)   |
| **CIFAR-10**    | ResNet-18 | `{python} dpacc_cifar10_base_str`  |  `{python} dpacc_cifar10_eps8_str` (`{python} dpacc_cifar10_eps8_pp_str`)  | `{python} dpacc_cifar10_eps1_str` (`{python} dpacc_cifar10_eps1_pp_str`) |
| **ImageNet**    | ResNet-50 | `{python} dpacc_imagenet_base_str` | `{python} dpacc_imagenet_eps8_str` (`{python} dpacc_imagenet_eps8_pp_str`) |                            N/A (not feasible)                            |
| **IMDB (text)** | BERT-base |   `{python} dpacc_imdb_base_str`   |     `{python} dpacc_imdb_eps8_str` (`{python} dpacc_imdb_eps8_pp_str`)     |    `{python} dpacc_imdb_eps1_str` (`{python} dpacc_imdb_eps1_pp_str`)    |
...
# │
# │ Goal: Pin the RDP accounting for a sentiment-classifier DP-SGD run:
# │       per-step RDP coefficient, total over k steps, and the (eps,delta)
# │       conversion swept over the Renyi order alpha.
# │ Show: per-step ~0.000312*alpha, total ~0.156*alpha, optimal eps ~3.1.
# │ How: eps_step = q^2 / (2 sigma^2); eps(alpha) = total*alpha + ln(1/delta)/(alpha-1).
# │
# └─────────────────────────────────────────────────────────────────────────────
import math
import numpy as np
...
class DPSGDBudget:
    """RDP accounting for a DP-SGD sentiment-classifier privacy budget."""
    ...
    sigma = 0.8  # noise multiplier (typical starting point)
    ...
    eps_step_coef = q_sq / two_sigma_sq  # per-step RDP slope in alpha
    eps_total_coef = k_steps * eps_step_coef  # total RDP slope in alpha
    ln_inv_delta = math.log(1 / delta)
    _grid = np.linspace(1.1, 40, 400_000)
    _eps = eps_total_coef * _grid + ln_inv_delta / (_grid - 1)
    alpha_opt = float(_grid[_eps.argmin()])
    eps_opt = float(_eps.min())

    # ┌── 3. GUARD (anchors + feasibility) ─────────────────────────────────
    check(abs(eps_step_coef - 0.000312) < 1e-5, f"per-step {eps_step_coef}")
    check(abs(eps_total_coef - 0.156) < 1e-3, f"total {eps_total_coef}")
    check(eps_opt < eps_target, f"eps_opt {eps_opt} must meet target {eps_target}")
    ...
    q_str = fmt(q, precision=2, commas=False)
    k_steps_str = fmt(k_steps, precision=0, commas=False)
    eps_step_coef_str = fmt(eps_step_coef, precision=6, commas=False)
    eps_total_coef_str = fmt(eps_total_coef, precision=3, commas=False)
    alpha_opt_str = fmt(alpha_opt, precision=1, commas=False)
    eps_opt_str = fmt(eps_opt, precision=1, commas=False)
    nonprivate_accuracy_pct_str = fmt_percent(nonprivate_accuracy, precision=0, commas=False, style="prose")
    # Per-step RDP identity: q^2 alpha / (2 sigma^2) = num/den = coef*alpha.
    q_sq_str = fmt(q_sq, precision=4, commas=False)
    two_sigma_sq_str = fmt(two_sigma_sq, precision=2, commas=False)
    eps_step_math = fmt_math(
        rf"\varepsilon_{{\text{{step}}}}(\alpha) = \frac{{({q_str})^2 \alpha}}"
        rf"{{2({sigma_str})^2}} = \frac{{{q_sq_str}\alpha}}{{{two_sigma_sq_str}}} "
        rf"\approx {eps_step_coef_str}\alpha"
    )
    eps_total_math = fmt_math(
        rf"\varepsilon_{{\text{{total}}}}(\alpha) = {k_steps_str} \times " rf"{eps_step_coef_str}\alpha = {eps_total_coef_str}\alpha"
    )
    eps_convert_math = fmt_math(
        rf"\epsilon = \varepsilon_{{\text{{total}}}}(\alpha) + "
        rf"\frac{{\ln(1/\delta)}}{{\alpha - 1}} = {eps_total_coef_str}\alpha + "
        rf"\frac{{\ln(10^6)}}{{\alpha - 1}}"
    )

    # Per-alpha sweep: slope term (total_coef*alpha), tail term
    # (ln(1/delta)/(alpha-1)), and their sum (total epsilon).
    slope_a8_str = fmt(eps_total_coef * 8, precision=2, commas=False)
    slope_a10_str = fmt(eps_total_coef * 10, precision=2, commas=False)
    slope_a12_str = fmt(eps_total_coef * 12, precision=2, commas=False)
    tail_a8_str = fmt(ln_inv_delta / 7, precision=2, commas=False)
    tail_a10_str = fmt(ln_inv_delta / 9, precision=2, commas=False)
    tail_a12_str = fmt(ln_inv_delta / 11, precision=2, commas=False)
    eps_a8_str = fmt(eps_total_coef * 8 + ln_inv_delta / 7, precision=2, commas=False)
    eps_a10_str = fmt(eps_total_coef * 10 + ln_inv_delta / 9, precision=2, commas=False)
    eps_a12_str = fmt(eps_total_coef * 12 + ln_inv_delta / 11, precision=2, commas=False)
```

::: {#nbk-security-privacy-dp-sgd-privacy-budget-example .callout-notebook title="DP-SGD privacy budget example"}
...
- Batch size: $B =$ `{python} DPSGDBudget.batch_size_str` (sampling rate $q = B/n_{\text{records}} =$ `{python} DPSGDBudget.q_str`)
...
Using the Gaussian mechanism formula and RDP accounting, we need noise multiplier $\sigma$ such that after $k =$ `{python} DPSGDBudget.k_steps_str` iterations with sampling rate $q =$ `{python} DPSGDBudget.q_str`, the total privacy loss is $\epsilon \leq$ `{python} DPSGDBudget.eps_target_str`.
...
For $\sigma =$ `{python} DPSGDBudget.sigma_str` (a typical starting point):

`{python} DPSGDBudget.eps_step_math`

After `{python} DPSGDBudget.k_steps_str` steps: `{python} DPSGDBudget.eps_total_math`

**Step 3**: Convert RDP to DP.
Converting to $(\epsilon, \delta)$-DP by optimizing over $\alpha$:

`{python} DPSGDBudget.eps_convert_math`

We evaluate the bound at several $\alpha$ values:

- $\alpha = 8$: $\epsilon =$ `{python} DPSGDBudget.slope_a8_str` $+$ `{python} DPSGDBudget.tail_a8_str` $=$ `{python} DPSGDBudget.eps_a8_str`
- $\alpha = 10$: $\epsilon =$ `{python} DPSGDBudget.slope_a10_str` $+$ `{python} DPSGDBudget.tail_a10_str` $=$ `{python} DPSGDBudget.eps_a10_str`
- $\alpha = 12$: $\epsilon =$ `{python} DPSGDBudget.slope_a12_str` $+$ `{python} DPSGDBudget.tail_a12_str` $=$ `{python} DPSGDBudget.eps_a12_str`

Optimizing the displayed approximation gives $\alpha \approx$ `{python} DPSGDBudget.alpha_opt_str` and $\epsilon \approx$ `{python} DPSGDBudget.eps_opt_str`, which satisfies the target of $\epsilon \leq$ `{python} DPSGDBudget.eps_target_str` with margin.

**Step 4**: Expected accuracy impact.
With $\sigma =$ `{python} DPSGDBudget.sigma_str` and $\epsilon \approx$ `{python} DPSGDBudget.eps_opt_str` under this simplified accountant, expect a nontrivial accuracy degradation compared to nonprivate training. For a sentiment classifier achieving `{python} DPSGDBudget.nonprivate_accuracy_pct_str` accuracy without DP, the private version might land several points lower depending on model capacity, clipping norm, optimizer, and data distribution.
...
Adversarial defenses enter when the attacker is adaptive rather than accidental. Query monitoring and output limiting reduce the information available for extraction attacks, while robustness evaluation and red-team exercises test whether the model can be induced into unsafe behavior. Stronger mechanisms such as trusted execution environments or certified defenses [methods that provide formal robustness guarantees under a specified perturbation bound] should be selected only when the threat model justifies their latency, hardware, and operational cost. Governance then closes the loop: audit logs, compliance mappings, incident records, and reproducible control checks provide evidence that the deployed system still satisfies its privacy and security obligations after the model, data, and traffic change.
