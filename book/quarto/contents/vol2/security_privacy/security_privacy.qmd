...
::: {#cs-security-privacy-strava-heatmap .callout-case-study title="The heatmap that revealed routines"}
...
This type of theft often targets models deployed as services, where the model is exposed through an API or embedded in a user-facing application. Repeatedly querying the model and recording its responses allows an attacker to train their own model to mimic the behavior of the original. This process, often called model distillation[^fn-model-distillation] or knockoff modeling, allows attackers to achieve comparable functionality without access to the original model's proprietary internals [@orekondy2019].
...
Training generative models on real datasets and sampling new instances from the learned distribution allows organizations to create datasets that approximate the statistical properties of the original data without retaining identifiable details [@goncalves2020].
...
| **Financial ML**<br>(Fraud detection API)                         | Model theft (IP loss)<br>Adversarial evasion<br>Data poisoning                            | • Model encryption (AES-256) at rest<br>• HSM for cryptographic key management<br>• Adversarial training on projected-gradient perturbations<br>• Input validation + rate limiting (100 req/min)<br>• Output confidence monitoring                  | HSM adds \$10--50K capital cost;<br>rate limiting may impact legitimate high-frequency users                                                                                                        |
...
class PracticalDPBudget:
    ...
    eps_alt_epochs_coef = k_alt_epochs * eps_step_coef
    eps_alt_epochs = float((eps_alt_epochs_coef * _grid + ln_inv_delta / (_grid - 1)).min())
    q_alt_batch = alt_batch_size / n_records
    k_alt_batch = int(n_epochs * n_records / alt_batch_size)
    eps_alt_batch_coef = k_alt_batch * (q_alt_batch**2 / (2 * sigma**2))
    eps_alt_batch = float((eps_alt_batch_coef * _grid + ln_inv_delta / (_grid - 1)).min())
    ...
    check(eps_target < eps_alt_epochs < eps_opt, f"alt epochs epsilon {eps_alt_epochs}")
    check(eps_target < eps_alt_batch < eps_opt, f"alt batch epsilon {eps_alt_batch}")
    ...
    eps_alt_epochs_str = fmt(eps_alt_epochs, precision=1, commas=False)
    alt_batch_size_str = fmt(alt_batch_size, precision=0)
    alt_q_str = fmt(q_alt_batch, precision=2, commas=False)
    eps_alt_batch_str = fmt(eps_alt_batch, precision=1, commas=False)
...
3. Converting to $(\epsilon, \delta)$-DP means optimizing over $\alpha$. For $\alpha = 10$, $\epsilon =$ `{python} PracticalDPBudget.eps_total_coef_str` $\times 10 + \ln(10^5)/9 =$ `{python} PracticalDPBudget.slope_too_high_str` $+$ `{python} PracticalDPBudget.tail_too_high_str` $=$ `{python} PracticalDPBudget.eps_too_high_str`, which is too high. For $\alpha = 4$, $\epsilon =$ `{python} PracticalDPBudget.eps_total_coef_str` $\times 4 + \ln(10^5)/3 =$ `{python} PracticalDPBudget.slope_lo_str` $+$ `{python} PracticalDPBudget.tail_lo_str` $=$ `{python} PracticalDPBudget.eps_lo_str`. For $\alpha = 3$, $\epsilon =$ `{python} PracticalDPBudget.eps_total_coef_str` $\times 3 + \ln(10^5)/2 =$ `{python} PracticalDPBudget.slope_hi_str` $+$ `{python} PracticalDPBudget.tail_hi_str` $=$ `{python} PracticalDPBudget.eps_hi_str`. The optimum is $\alpha \approx$ `{python} PracticalDPBudget.alpha_opt_str`, yielding $\epsilon \approx$ `{python} PracticalDPBudget.eps_opt_str`.

The single-knob changes are partial remediations, not complete solutions under this accountant. Training for $N_{\text{epochs}} =$ `{python} PracticalDPBudget.alt_epochs_str` lowers the estimate to $\epsilon \approx$ `{python} PracticalDPBudget.eps_alt_epochs_str`, while reducing the sampling rate to $B =$ `{python} PracticalDPBudget.alt_batch_size_str` $(q =$ `{python} PracticalDPBudget.alt_q_str` $)$ lowers it to $\epsilon \approx$ `{python} PracticalDPBudget.eps_alt_batch_str`. Reaching the target $\epsilon =$ `{python} PracticalDPBudget.eps_target_str` therefore requires a stronger combination, typically increasing $\sigma$ substantially while accepting lower accuracy, fewer training passes, or smaller batches. These adjustments demonstrate the fundamental privacy-utility-computational resource trade-off[^fn-privacy-utility-tension] in production ML systems: differential privacy offers strong theoretical assurances, but tightening the privacy bound consumes accuracy and compute budget in measurable ways.
