...
In the **Fleet Stack** (@sec-vol2-introduction), Security is the **Governance Layer**. Having built the fleet (Part I), the distributed logic (Part II), and the serving infrastructure (Part III), we now focus on protection. This layer does not add new features; it wraps the entire stack in armor, ensuring that our powerful global machine cannot be hijacked, poisoned, or exploited by adversaries. If the **Iron Law** defines performance, Security defines **Integrity**: ensuring that $L_{step}$ produces the *correct* result, not a manipulated one.
