...
Many high-utility machine learning systems depend on personal data, institutional knowledge, or behavioral patterns, creating tension between utility and protection that determines societal acceptance. ML systems can learn from sensitive information and embed patterns into persistent models that inadvertently reveal private details, extending the risks beyond those of transient data processing. This capability creates systemic risks extending beyond individual privacy violations to threaten institutional trust, competitive advantages, and democratic governance. A high-performing model remains unused if it cannot be deployed without exposing sensitive data, if it cannot be trusted to resist adversarial manipulation, or if it cannot satisfy regulatory requirements that govern its intended domain. Privacy and security are not features to be added after the system works but prerequisites that determine whether the system can work at all in contexts where data sensitivity and adversarial risk are nonnegotiable constraints. In C³ terms, security and privacy change Compute, Communication, and Coordination contracts: throughput is no longer sufficient unless each boundary also preserves confidentiality, integrity, and accountable control.
...
[^fn-gdpr]: **GDPR (General Data Protection Regulation)**\index{GDPR!ML systems}: Applicable since 2018 and authorizing upper-tier fines up to 4 percent of total worldwide annual turnover, GDPR has levied billions of euros in penalties including €746 million against Amazon (2021). For ML systems, GDPR's erasure right creates a technical challenge: removing an individual's influence from trained model weights may require retraining or machine-unlearning techniques that approximate removal of a record's learned influence, both computationally expensive at scale.
...
::: {#exmp-security-privacy-privacy-accuracy-tax .callout-example title="One-shot Gaussian mechanism"}

**Scenario**: Release one vector-valued query with $\ell_2$ sensitivity $\Delta_2 f =$ `{python} GaussianNoiseScale.sensitivity_str`, target $\epsilon =$ `{python} GaussianNoiseScale.epsilon_str`, and $\delta =$ `{python} GaussianNoiseScale.delta_math`.

**Result**: The classic sufficient bound requires $\sigma \geq$ `{python} GaussianNoiseScale.sigma_min_str`.

**Systems insight**: This calculation applies to one Gaussian-mechanism release. It is not a DP-SGD privacy budget, because sampled training requires composition across all optimization steps.
...
[^fn-privacy-utility-tension]: **Privacy-Utility Tension**\index{Privacy!utility tension}\index{Privacy!utility, trade-off}: The privacy budget is a property of the composed randomized mechanisms applied to adjacent datasets. DP-SGD epochs contribute because they contain additional sampled Gaussian steps, and interactive DP queries contribute when they access the protected data. Post-processing an already released DP result does not consume additional budget. Once an allocation is exhausted, another data-dependent release cannot retain the same claimed bound unless the reported budget or mechanism is changed. This makes privacy accounting a first-class constraint in ML system design, alongside compute and memory budgets.
