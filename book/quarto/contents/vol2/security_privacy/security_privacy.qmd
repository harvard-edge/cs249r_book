...
The principle of **minimal attack surface**\index{Minimal Attack Surface} starts from the fact that every exposed interface, stored artifact, and network endpoint represents a potential attack surface. Minimizing unnecessary exposure reduces risk through five mechanisms:
...
The principle of **fail-safe defaults**\index{Fail-Safe Defaults} requires systems to fail in ways that preserve security rather than availability when attacks succeed or anomalies occur. Four defaults make that posture concrete:
...
- **Backdoor attacks**\index{Backdoor Attack}: Hidden triggers, often imperceptible patterns, elicit malicious behavior only when the trigger is present [@gu2017badnets].[^fn-backdoor-attacks]
...
Three recurring failure modes organize the space: prompt injection breaks the boundary between control and data [@perez2022ignore; @greshake2023not], training data extraction exposes memorized records [@carlini2021extracting], and jailbreaking bypasses alignment controls [@zou2023].
...
By training generative models on real datasets and sampling new instances from the learned distribution, organizations can create datasets that approximate the statistical properties of the original data without retaining identifiable details [@goncalves2020].
