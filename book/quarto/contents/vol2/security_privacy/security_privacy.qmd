...
4.  **Impact on Mean**: The noise added to the *sum* has magnitude $\approx `{python} DPCostAnalysis.noise_scale_str`.
...
class MultiTenantIsolation:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(overhead_pct == 15.0, f"Overhead {overhead_pct}% unexpected")
    # ┌── 4. OUTPUT ────────────────────────────────────────
...
1. **Throughput Loss**: `{python} MultiTenantIsolation.mt_peak_tokens_str` - `{python} MultiTenantIsolation.mt_secure_tokens_str` = **150 tokens/sec**.
2. **The Isolation Tax**: (150 / 1000) = **`{python} MultiTenantIsolation.mt_overhead_pct_str`%**.

**The Systems Insight**: Security is a **Capacity Drain**. Providing a "Secure Enclave" for your model costs **`{python} MultiTenantIsolation.mt_overhead_pct_str`%** of your GPU's raw throughput. In the Machine Learning Fleet, multi-tenancy is an economic necessity, but it is not free. You must decide if your data sensitivity justifies losing 15% of your fleet's total capacity. For most public-facing APIs, this "Tax" is the price of preventing one tenant's prompt from leaking into another's response.
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line cap=round,line join=round, node distance=2cm, every node/.style={transform shape}]
\tikzset{%
TxtL/.style = {font=\large\usefont{T1}{phv}{m}{n},text width=90mm,align=justify,anchor=north},
Line/.style={VioletLine, line width=1.0pt,shorten <=1pt,shorten >=2pt},
LineA/.style={VioletLine,line width=1.0pt,{-{Triangle[width=1.1*6pt,length=2.0*6pt]}},shorten <=3pt,shorten >=2pt},
ALine/.style={GrayLine, line width=1.0pt,{{Triangle[width=1.1*6pt,length=2*6pt]}-}},
Larrow/.style={fill=VioletLine, single arrow,  inner sep=2pt, single arrow head extend=3pt,
...
\coordinate(4BL)at($(EK.south west)+(0.95,0.3)$);
    ...
    \foreach \i in {0,...,4} {
      ...
      \begin{scope}
        ...
        \fill[GrayL]($(4BL)+(\x,0)$) rectangle ++(\w,\h*1);
...
\draw[line width=1.4pt](SM1)to [bend right=45](SM2);
\else
\node[draw=GreenLine,rounded corners=2pt,rectangle,minimum width=53,minimum height=30,draw=black,fill=black](EK)at(0,0.53){};
\pic[shift={(0,0)}] at  (EK){skull={scalefac=1.3,picname=1,filllcolor=white, filllcirclecolor=black,Linewidth=0.5pt}};
\fi
\fi
%
\draw[fill=\filllcolor!60!GrayL,line width=\Linewidth](-1.00,-0.1)--(1.0,-0.1)--(1.28,-0.6)--(-1.28,-0.6)--cycle;
\draw[fill=\filllcolor!60!GrayL,line width=\Linewidth](1.28,-0.6)--(-1.28,-0.6)arc[start angle=180, end angle=270, radius=4pt]--(1.14,-0.73)
arc[start angle=270, end angle=355, radius=4pt]--cycle;
\draw[fill=\filllcolor!30!GrayL,line width=\Linewidth](-0.95,-0.17)--(0.95,-0.17)--(1.03,-0.34)--(-1.03,-0.34)--cycle;
\draw[fill=\filllcolor!30!GrayL,line width=\Linewidth](-0.16,-0.52)--(0.16,-0.52)--(0.14,-0.42)--(-0.14,-0.42)--cycle;
\end{scope}
    }
  }
}
...
(0.45,-1.34)--(0.85,-1.48)--(0.61,-0.68)--(0.44,-0.1)--cycle;
 \draw[draw=none,postaction={very line width=1.0pt, line join=round, draw=white,fill=\filllcolor,
...
\foreach \i in {0,1,2} {
  \foreach \j in {0,1,2,3,4,5,6,7} {
    ...
    \foreach \c in \coloredcells {%
      \ifx\cellid\c
        \global\def\fillcolor{GreenLine!60}%
      \fi
    }
...
\node[draw=black,fill=\filllcolor!50!GrayL,anchor=north west,inner sep=0pt,
minimum width=5mm,minimum height=3.0mm](BX2)at(2BL){};
\coordinate(3BL)at($(BX2.south east)+(1mm,0)$);
\node[draw=black,fill=\filllcolor!70!GrayL,anchor=south west,inner sep=0pt,
minimum width=12mm,minimum height=3.0mm](BX3)at(3BL){};
\path[RedLine](BX3.north west)|-coordinate[pos=0.5](1E)(MB27.south east);
%display
\ifnum\Smile=1
...
\fi
\draw[fill=\filllcolor!40!GrayL](-0.2,-0.67)--(0.8,-0.67)--(0.72,-0.54)--(-0.14,-0.54)--cycle;
...
\node[draw=black,fill=\filllcirclecolor!60!GrayL,rectangle,inner sep=0pt,anchor=north,minimum width=10,minimum height=2]at(SR1){};
\draw[line width=2*\Linewidth](-0.72,-0.4)--coordinate[pos=0.5](SR2)(0.665,-0.4);
\node[draw=black,fill=\filllcirclecolor!60!GrayL,rectangle,inner sep=0pt,anchor=north,minimum width=10,minimum height=2]at(SR2){};
\draw[line width=2*\Linewidth](-0.72,-0.8)--coordinate[pos=0.5](SR3)(0.665,-0.8);
\node[draw=black,fill=\filllcirclecolor!60!GrayL,rectangle,inner sep=0pt,anchor=north,minimum width=10,minimum height=2]at(SR3){};
%
\end{scope}
    }
  }
...
%fire
\ifnum\Fire=1
\fill[fill=RedLine!70,line width=1.0pt](0,1.6)--(0.4,0.6)--(1.4,2)--(2.2,0.8)--(3.1,1.8)--(3,0.5)--(3.7,0.6)--(3.0,-1.2)--
...
\draw[black,line width=1.0pt]($(SRA1.south west)!\i!(SRA1.north west)$)--($(SRA1.south east)!\y!(SRA1.north east)$);
...
%
\node (LAPTOP1) [draw=none] at (0,0) {\pic[shift={(0,0)}] {laptop={scalefac=1.5,picname=1,drawcolor=GreenLine,Dual=0,Smile=0,
filllcolor=GreenL,Linewidth=1.0pt, filllcirclecolor=OrangeLine}};};
%
\node (USB1) [right=of LAPTOP1] {\pic[shift={(0,0)},rotate=320] {usb={scalefac=0.6,picname=1,drawcolor=OrangeLine,filllcirclecolor=white,filllcolor=VioletLine!GrayLine!,Linewidth=1.0pt,}};};
%
\node (TESTING1) [right=of USB1] {\pic[shift={(0,-0.5)}] {testing={scalefac=1,picname=1,drawcolor=OrangeLine,filllcolor=OrangeLine, Linewidth=1.0pt}};};
%
\node (LAPTOP2) [right=of TESTING1] {\pic[shift={(0,0)}] {laptop={scalefac=1.0,picname=2,drawcolor=RedLine,Dual=0,Smile=1,
filllcolor=RedL,Linewidth=1.0pt, filllcirclecolor=OrangeLine}};};
\tikzset{%
    LineZ/.style={-*,GreenL0!black,line width=1.0pt}
}
\node (LAPTOP3) [right=of LAPTOP2] {\pic[shift={(0,0)}] {laptop={scalefac=1.0,picname=3,drawcolor=RedLine,Dual=0,Smile=1,
filllcolor=OrangeLine!,Linewidth=1.0pt, filllcirclecolor=OrangeLine}};};
...
%
\node (LAPTOP4) [right=of LAPTOP3] {\pic[shift={(0,0)}] {laptop={scalefac=1.0,picname=4,drawcolor=RedLine,Dual=0,Smile=1,
filllcolor=BlueLine!,Linewidth=1.0pt, filllcirclecolor=OrangeLine}};};
%
\draw[LineZ](EKV4.east)--node[above,black]{\huge !}
node[below=3pt,black,fill=VioletL,circle,inner sep=1pt](CIRC1){\large 1}++(0:1.2);
%
\node (PLC1) [right=of LAPTOP4] {\pic[shift={(0,0)}] {plc={scalefac=1.2,picname=1,drawcolor=OrangeLine,filllcirclecolor=BlueLine, Smile=1,
...
\node[TxtL,below=6pt of BB1.south west,text width=110mm,anchor=north west](GT1){\textcolor{RedLine}{\textbf{1. Infection}}\\[0.35ex]
...
%
\path[RedLine](BB1.south east)-|coordinate[pos=0.5](T2)(EKV3.south);
\node[TxtL,below=6pt of T2.south,text width=80mm,xshift=-12mm](GT2){\textcolor{RedLine}{\textbf{2. Search}}\\[0.35ex]
...
\path[RedLine](BB1.south east)-|coordinate[pos=0.5](T3)(CIRC1);
\node[TxtL,below=6pt of T3.south,text width=67mm](GT3){\textcolor{RedLine}{\textbf{3. Update}}\\[0.35ex] If the system isn’t a target, Stuxnet does nothing;
...
\draw[BrownLine,line width=1.0pt]([yshift=-2mm]BOX1.south west)coordinate(LE)
--([yshift=-2mm]BOX1.south east)coordinate(DE);
%%%%%%%%%%%%%%%%%%%
%Row below
%%%%%%%%%%%%%%%%%%%
\node (LAPTOP5) [below=of LAPTOP1] {\pic[shift={(0,0)}] {laptop={scalefac=1.0,picname=5,drawcolor=RedLine,Dual=0,Smile=1,
filllcolor=GreenL,Linewidth=1.0pt, filllcirclecolor=OrangeLine}};};
%
\node (COPIER1) [right=of LAPTOP5] {\pic[shift={(0,0)}] {copier={scalefac=0.8,picname=1,drawcolor=BlueLine,
filllcolor=BlueL,Linewidth=1.0pt, filllcirclecolor=OrangeLine!}};};
%
\node (PLC2) [right=of COPIER1] {\pic[shift={(0,0)}] {plc={scalefac=1.2,picname=2,drawcolor=OrangeLine,filllcirclecolor=BlueLine, Smile=0,
filllcolor=OrangeLine,Linewidth=1.0pt,Level=0.42}};};
%%%%%%%%%%%%%%%
\node (LAPTOP6) [right=of PLC2] {\pic[shift={(0,0)}] {laptop={scalefac=1.0,picname=6,drawcolor=RedLine,Dual=1,Smile=1,
filllcolor=BrownL,Linewidth=1.0pt, filllcirclecolor=OrangeLine}};};
\draw[LineZ](EKV6.15)--++(0:0.7);
\draw[LineZ,RedLine](EKV6.345)--++(0:0.7);
%
\node (PLC3) [right=of LAPTOP6] {\pic[shift={(0,0)}] {plc={scalefac=1.2,picname=3,drawcolor=OrangeLine,filllcirclecolor=RedLine, Smile=0,
filllcolor=OrangeLine,Linewidth=1.0pt,Level=0.42}};};
\draw[LineZ,RedLine](R3.355)--++(0:0.9);
\draw[LineZ,-](R3.20)--++(0:0.35)--++(0,-0.5)
node[rectangle, fill=white,draw=GreenL0!black,minimum size=width=2mm,minimum height=4mm] {};
%
\node (SRAF1) [right=of PLC3] {\pic[shift={(0,0)}] {sraf={scalefac=0.4,picname=1,drawcolor=BrownLine,filllcolor=BrownLine!50!,Linewidth=0.5pt,Fire=0}};};
%%%%%%%%%%%%%%%
\node (LAPTOP7) [right=of SRAF1] {\pic[shift={(0,0)}] {laptop={scalefac=1.0,picname=7,drawcolor=RedLine,Dual=1,Smile=1,
filllcolor=RedLine,Linewidth=1.0pt, filllcirclecolor=OrangeLine}};};
%
\node (PLC4) [right=of LAPTOP7] {\pic[shift={(0,0)}] {plc={scalefac=1.2,picname=4,drawcolor=OrangeLine,filllcirclecolor=RedLine, Smile=0,
filllcolor=OrangeLine,Linewidth=1.0pt,Level=1}};};
%
\node (SRAF2) [right=of PLC4] {\pic[shift={(0,0)}] {sraf={scalefac=0.4,picname=1,drawcolor=BrownLine,filllcolor=BrownLine!50!,Linewidth=0.5pt,Fire=1}};};
%arrows
...
\node[Larrow,RedLine,rotate=180]at(2SR4){};
...
\node[TxtL,below=6pt of 2BB1.south west,text width=81mm,anchor=north west](DT1){\textcolor{RedLine}{\textbf{4. Compromise}}\\[0.35ex] The worm then compromises
...
%
\path[RedLine](2BB1.south east)-|coordinate[pos=0.5](2T2)(PLC3.south);
\node[TxtL,below=6pt of 2T2.south](DT2){\textcolor{RedLine}{\textbf{5. Control}}\\[0.35ex] In the beginning,
...
%
\path[RedLine](2BB1.south east)-|coordinate[pos=0.5](2T3)(PLC4.south);
\node[TxtL,below=6pt of 2T3.south,text width=80mm](DT3){\textcolor{RedLine}{\textbf{6. Deceive and destroy}}\\[0.35ex] Meanwhile,
it provides false feedback to outside controllers, ensuring that they won’t know what’s going wrong
until it’s too late to do anything about it.};
%
\path[RedLine](LE)--++(0,-6.7)coordinate(LE2)-|coordinate(DE2)(DE);
\pgfdeclarehorizontalshading{mygradient}{100bp}{
  color(0bp)=(GreenLine);
  color(50bp)=(RedLine)
}
\shade[shading=mygradient] (LE2) rectangle ($(DE2)+(0,-5mm)$);
\path[RedLine](LE)--++(0,9.5)coordinate(GLE2)-|coordinate(GDE2)(DE);
\fill[BrownLine!40] (GLE2) rectangle ($(GDE2)+(0,5mm)$);
\node[fill=white]at($([yshift=2.5mm]GLE2)!0.5!([yshift=2.5mm]GDE2)$){\large \bfseries HOW \textcolor{RedLine}{STUXNET} WORKED};
\draw[LineA,*-*,text=black,line width=1pt,shorten <=5pt,shorten >=5pt](EKV1.north)--++(0,1.85)-|
node[below=5pt,pos=0.1]{Update from source}
node[left=5pt,pos=0.85,black,fill=VioletL,circle,inner sep=1pt]{2}(EKV4.north);
\end{tikzpicture}
```
:::
...
```{.tikz}
\scalebox{0.85}{%
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape, line join=round, node distance=1cm and 2.2cm]
\tikzset{%
  ...
  Box3/.style={Box, draw=VioletLine, fill=VioletL}
}
...
\node[Box3, above left=0.5cm and 2cm of B1] (PL) {Privacy Leakage};
\node[Box3, above right=0.5cm and 2cm of B1] (DP) {Data Poisoning};

% Background and Lifecycle label
\begin{scope}[on background layer]
  \node[draw=GrayLine, inner xsep=10mm, inner ysep=4mm, yshift=2.5mm, fill=GrayL!60, fit=(B1)(B4), line width=0.75pt] (BB2) {};
  \node[below=4pt of BB2.north, inner sep=0pt, anchor=north] {Lifecycle};
\end{scope}
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line join=round, node distance=0.4cm, on grid]
\tikzset{%
  Line/.style={line width=1.0pt,GrayLine},
  ...
  Box3/.style={Box, draw=VioletLine, fill=VioletL, text width=42mm}
}
...
\begin{scope}[on background layer]
  \node[draw=GrayLine, inner xsep=10mm, minimum height=71.5mm,
  yshift=2.5mm, fill=GrayL!60, fit=(B1)(B5), line width=0.75pt] (BB2) {};
  \node[below=6pt of BB2.north, inner sep=0pt, anchor=north] {\textbf{Approximate Model Theft}};
\end{scope}

% Exact Model Theft
\node[Box3, right=5cm of B1] (RB1) {Access to model file or deployment artifact};
\node[Box3, right=5cm of B5] (RB4) {Use or resell\\ proprietary IP};
...
\begin{scope}[on background layer]
  \node[draw=GreenLine, inner xsep=10mm, minimum height=71mm,
  yshift=2.2mm, fill=GreenL, fit=(RB1)(RB4), line width=0.75pt] (BB2) {};
  \node[below=6pt of BB2.north, inner sep=0pt, anchor=north] {\textbf{Exact Model Theft}};
\end{scope}
...
-   **Legitimate User Impact**: Top-5 accuracy preserved at 99.2% (vs 99.3% baseline). Latency increase: +2 ms for monitoring, +0.5 ms for perturbation = +`{python} DefenseOverheadAnalysis.total_defense_ms_str`ms total (`{python} DefenseOverheadAnalysis.overhead_pct_str`% overhead on 100 ms inference)
...
::: {.callout-note title="Figure: Data Poisoning Optimization Loop"}
```{.tikz}
...
::: {.callout-note title="Figure: Adversarial Decision Boundary"}
```{.tikz}
...
::: {#fig-adversarial-stickers fig-env="figure" fig-pos="htb" fig-cap="**Adversarial Stickers**: Nearly imperceptible stickers can trick machine learning models into misclassifying stop signs as speed limit signs over 85% of the time. This emphasizes the vulnerability of ML systems to adversarial attacks." fig-alt="Photo of stop signs with small black and white stickers applied. Original signs appear normal to humans but cause ML classifiers to misread them as speed limit signs."}
...
```{.tikz}
\scalebox{0.65}{%
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line join=round, node distance=0.5cm, on grid]
\tikzset{%
  Line/.style={line width=1.0pt,GrayLine},
  ...
  Box2/.style={Box,draw=BrownLine,fill=BrownL},
  Box3/.style={Box, draw=VioletLine,fill=VioletL},
  Box4/.style={Box, draw=OrangeLine,fill=OrangeL,text width=43mm}
}

% Start Node
\node[Box4, name=B0] {Start: Identify Threat Type};

% Column 1
\node[Box, below=of B0, anchor=north] (B1) {Model Theft};
...
% Column 2
\node[Box2, right=1.75cm of B1] (SB1) {Data Poisoning};
...
% Column 3
\node[Box3, right=1.75cm of SB1] (RB1) {Adversarial Examples};
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85, transform shape, node distance=1cm]
  \tikzset{
    stage/.style={draw=GrayLine, line width=0.75pt, rounded corners, fill=white, minimum width=2.5cm, minimum height=1cm, align=center, font=\bfseries\small},
    arrow/.style={->, >=stealth, line width=1.0pt, GrayLine!80},
    attack/.style={text=RedLine!70!black, font=\scriptsize, align=center}
  }

  % Nodes
  \node[stage, fill=BlueL] (design) {Design};
  \node[stage, fill=OrangeL, right=of design] (fab) {Fabrication};
  \node[stage, fill=GreenL, right=of fab] (assembly) {Assembly};
  \node[stage, fill=VioletL, right=of assembly] (dist) {Distribution};
  \node[stage, fill=GrayL, right=of dist] (deploy) {Deployment};
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line join=round, node distance=1cm and 1cm]
\pgfplotsset{myaxis/.style={
    ...
    cycle list={
        {BlueLine,line width=1.75pt},
        {GreenLine,line width=1.75pt},
        {VioletLine,line width=1.75pt},
    }
}}

\tikzset{
    box/.style={line width=0.75pt, draw=BrownLine, rectangle},
    arrow/.style={-latex, line width=1.0pt},
    plotline/.style={line width=1.25pt}
}
...
\begin{scope}[local bounding box=G1]
    \begin{axis}[myaxis]
        ...
        \addplot+[] {7*exp(-x^2/2)};
        \node[box, minimum size=14mm, anchor=center] (B1) at (S) {};
    \end{axis}
\end{scope}

% Right graph
\begin{scope}[local bounding box=G2, right=of G1, xshift=1cm, scale=1.5]
    \begin{axis}[myaxis, width=5cm, height=5cm]
        \addplot+[domain=-0.75:0.75, plotline] {6.5*exp(-x^2/2)} node[pos=0.66, outer sep=0pt, inner sep=0pt] (T1) {};
        \addplot+[domain=-0.796:0.796, plotline] {6.75*exp(-x^2/2)} node[pos=0.63, outer sep=0pt, inner sep=0pt] (T2) {};
        \addplot+[domain=-0.84:0.84, plotline] {7*exp(-x^2/2)} node[pos=0.6, outer sep=0pt, inner sep=0pt] (T3) {};

        \draw[BlueLine, arrow] (T1) -- ++(0:9.5mm) node[right] {0000};
        \draw[GreenLine, arrow] (T2) -- ++(0:9.9mm) node[right] {1111};
        \draw[VioletLine, arrow] (T3) -- ++(0:10.4mm) node[right] {0101};
    \end{axis}
    \node[box, minimum height=48mm, minimum width=67mm, anchor=center] (B2) at (rel axis cs:0.7,0.6) {};
\end{scope}
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line join=round, node distance=2cm, on grid]
...
\node[Box, right=of B2] (B3) {Hardware Security Modules};
\node[Box, right=of B3] (B4) {Physical Unclonable Functions};

\begin{scope}[on background layer]
  \node[draw=GrayLine, inner xsep=6mm, inner ysep=6mm, yshift=2.5mm, fill=GrayL!60, fit=(B1)(B4), line width=0.75pt] (BB1) {};
  \node[below=6pt of BB1.north, inner sep=0pt, anchor=north] {\textbf{Hardware-Level Security}};
\end{scope}

% System-Level Security
\node[Box3, above=of B1] (2B1) {System Integrity Checks};
...
\begin{scope}[on background layer]
  \node[draw=BlueLine, inner xsep=6mm, inner ysep=6mm, yshift=2.5mm, fill=BlueL, fit=(2B1)(2B4), line width=0.75pt] (BB2) {};
  \node[below=6pt of BB2.north, inner sep=0pt, anchor=north] {\textbf{System-Level Security}};
\end{scope}

% Model-Level Security
\node[Box2, above=of 2B1] (3B1) {Model Encryption \& Serialization};
\node[Box2, right=of 3B1] (3B2) {Secure Model Design};
\node[Box2, right=of 3B2] (3B3) {Secure Deployment \& Access};

\begin{scope}[on background layer]
  \node[draw=GreenLine, inner xsep=6mm, inner ysep=6mm, yshift=2.5mm, fill=GreenL, fit=(3B1)(3B3), line width=0.75pt] (BB3) {};
  \node[below=6pt of BB3.north, inner sep=0pt, xshift=4mm, anchor=north] {\textbf{Model-Level Security}};
\end{scope}

% Data Privacy & Governance
\node[Box4, above left=of 3B2] (4B1) {Differential Privacy};
\node[Box4, above right=of 3B2] (4B2) {Federated Learning};
\node[Box4, above=of 4B1] (5B1) {Homomorphic Encryption};
\node[Box4, above=of 4B2] (5B2) {Synthetic Data Generation};

\begin{scope}[on background layer]
  \node[draw=RedLine, inner xsep=6mm, inner ysep=6mm, yshift=2.3mm, fill=VioletL, fit=(4B1)(5B2), line width=0.75pt] (BB4) {};
  \node[below=6pt of BB4.north, inner sep=0pt, anchor=north] {\textbf{Data Privacy \& Governance}};
\end{scope}
...
::: {.callout-note title="Figure: Secure Aggregation Protocol"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm and 2cm]

  \tikzset{
    clientnode/.style={draw=GrayLine, line width=0.75pt, circle, minimum size=1.2cm, fill=GrayL, align=center, font=\tiny},
    servernode/.style={draw=GrayLine, line width=0.75pt, fill=BlueL, rounded corners=2pt, minimum width=3cm, minimum height=1cm},
    arrow/.style={->, line width=1.0pt},
    maskarrow/.style={<->, dashed, OrangeLine, line width=1.0pt}
  }

  % Server
  \node[servernode] (Server) {\textbf{Server}\\Aggregate: $(x_A + r) + (x_B - r) = x_A + x_B$};

  % Clients
  \node[clientnode, above left=of Server] (C1) {\textbf{Client A}\\$x_A + r$};
  \node[clientnode, above right=of Server] (C2) {\textbf{Client B}\\$x_B - r$};

  % Masking arrow
  \draw[maskarrow] (C1) -- node[midway, above, font=\tiny] {Shared Mask $r$} (C2);

  % Upload arrows
  \draw[arrow] (C1) -- (Server);
  \draw[arrow] (C2) -- (Server);

  \node[anchor=west, font=\scriptsize, text=GrayLine, right=0.5cm of Server, align=left] {Masks cancel\\during summation};
...
2. **AES-256**: 20 ms + 0.5 ms = **`{python} EncryptionOverhead.eo_aes_ms_str` ms** (negligible tax).
3. **FHE**: 20 ms $\times$ `{python} EncryptionOverhead.eo_fhe_ratio_str` = **`{python} EncryptionOverhead.eo_fhe_sec_str` seconds**.
...
scale=0.9, every node/.append style={transform shape}, node distance=1cm and 1.5cm]
\tikzset{%
LineA/.style={line width=1.0pt,GrayLine,latex-latex},
Line/.style={BrownLine!60, {Triangle[width = 4pt, length = 4pt]}-{Triangle[width = 4pt, length = 4pt]}, line width = 1.2pt},
LineD/.style={line width=1.0pt,GrayLine,latex-,dashed},
...
Box4a/.style={Box,draw=BlueLine,fill=BlueL,anchor=west,minimum width=42mm, minimum height=48mm},
Box5/.style={Box,draw=BlueLine,fill=BlueL},
Box6/.style={Box,draw=OrangeLine,fill=OrangeL},
}
...
\node[Box3,below=1.35 of B4](B5){Secure Nonvolatile Storage};

\node[Box4,anchor=west](SEP)at($(B1.north east)!0.5!(B4.south east)+(1.25,0)$){Secure Enclave\\ Processor};
\node[Box,anchor=west,right=of SEP](MPE){Memory Protection\\Engine};
...
\node[Box4a,anchor=south west,above=of B1](AP){Application\\ Processor};
\node[Box5,anchor=south west,right=of AP](NAN){NAND flash controller};
\node[Box5,anchor=north west,below=of NAN](AES){AES engine};
...
yshift=-0.5mm,fill=VioletL,fit=(BB1)(AP)(MC),line width=0.75pt](BB2){};
\node[above=4pt of  BB2.south,inner sep=0pt,
anchor=south]{\textbf{System on chip}};
\end{scope}

\begin{scope}[on background layer]
\node[draw=GrayLine,inner xsep=5mm,inner ysep=4mm,
yshift=0mm,fill=GrayL!60,fit=(B1)(B4)(MPE),line width=0.75pt](BB3){};
...
channelcolor=RedLine!79!GrayLine, Linewidth=1.0pt}};
\end{scope}
\end{tikzpicture}
```
:::
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line join=round, node distance=0.5cm, positioning]
\tikzset{%
Line/.style={GrayLine,-{Triangle[width = 6pt, length = 6pt]}, line width=1.0pt,text=black},
Box/.style={inner xsep=2pt,inner ysep=6pt,
    draw=BlueLine,
    line width=0.75pt,
    fill=BlueL,
...
Box3/.style={draw=VioletLine,fill=VioletL, trapezium, trapezium left angle=70,
...
\node[Box3, below=0.6 of B6] (B7) {Success};
\node[Box, below=0.7 of B7] (B8) {Boot abort};

\node[Box, right=3 of B3] (B9) {Checking the CRC checksum of root fs image};
\node[Box, below=1 of B9] (B10) {Perform integrity measurement, verification, and storage of root fs image};
\node[Box3, below=1 of B10] (B11) {Success};
\node[Box, below=1 of B11] (B12) {Set boot parameters for kernel};
\node[Box, below=1 of B12] (B13) {Booting the kernel};
...
::: {.callout-note title="Differential Privacy Diagram"}
```{.tikz}
