...
- [ ] **Lifecycle stage**: Is the suspicious activity occurring during training, serving, or infrastructure operation?
- [ ] **Attack surface**: Which interface is the attacker exercising, and what does the query pattern reveal?
- [ ] **Defense mapping**: Which rate-limiting, monitoring, or output-control mechanisms should activate first?
...
- [ ] **Attack timing**: What distinguishes this failure from a training-time compromise?
- [ ] **Input mechanism**: How did the attacker change the model's observed input without changing the model itself?
- [ ] **Defense choice**: Which detection or robustness control belongs closest to the serving path?
...
- [ ] **Isolation boundary**: What threat model does a Trusted Execution Environment address in cloud deployment?
- [ ] **Layer separation**: Which risks remain outside the scope of a Trusted Execution Environment?
- [ ] **Deployment fit**: Where should attestation and runtime monitoring sit around the protected execution boundary?
...
# │ Exports: TeeOverheadSpecs.tz_cycles_low_str/_high_str, tz_us_low_str/_high_str,
# │          tz_power_low_str/_high_str, sgx_ctx_low_str/_high_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │ Exports: HsmGpuCrypto.throughput_ratio_mult_str, hsm_cost_low_str, hsm_cost_high_str
# └─────────────────────────────────────────────────────────────────────────────
...
- [ ] **Primitive matching**: Which of the four primitives in @tbl-hw-security-comparison most directly defends against fault injection on model weights, and which defends against side-channel extraction of those weights from the inference pipeline?
- [ ] **Edge defense stack**: For an edge inference deployment, which combination of primitives defends against the three primary threats listed in @tbl-defense-selection-framework: physical access, side-channel attacks, and model extraction?
- [ ] **Supply-chain gap**: Can you explain why supply chain compromise is the hardest hardware threat to defend against with the four primitives in @tbl-hw-security-comparison, and which row of @tbl-defense-selection-framework treats it as a primary threat?
- [ ] **TEE vs. attestation**: Given a fault injection attack and a counterfeit hardware attack against the same deployment, which is detectable at runtime by a TEE and which requires hardware attestation at boot?
...
# │
# │ Imports: math, mlsysim.fmt (fmt, fmt_math, check)
# │ Exports: gns_* clipping/delta inputs, ln-term, strong/weak sigma, 5.72 coef.
# └─────────────────────────────────────────────────────────────────────────────
import math
...
# │ Exports: ma_clip_norm_str, ma_eps_moments_str, ma_eps_naive_str.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
# │
# │ Imports: math, numpy, mlsysim (Datasets, units), mlsysim.fmt (fmt, fmt_math, check)
# │ Exports: pdp_* scenario inputs, coefficients, alpha-sweep epsilons.
# └─────────────────────────────────────────────────────────────────────────────
import math
import numpy as np
...
# │
# │ Imports: mlsysim.fmt (fmt_percent, fmt_pp, check)
# │ Exports: dpacc_<row>_{base,eps8,eps1}_str and dpacc_<row>_{eps8,eps1}_pp_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: math, numpy, mlsysim.fmt (fmt, fmt_math, check)
# │ Exports: dpsgd_* scenario inputs, coefficients, alpha-sweep, and the
# │          per-step / total RDP _math expressions.
# └─────────────────────────────────────────────────────────────────────────────
import math
import numpy as np
from mlsysim.fmt import fmt, fmt_math, check
...
**Scenario**: Train a sentiment classifier on `{python} DPSGDBudget.n_records_str` customer reviews with target privacy $(\epsilon, \delta) = ($ `{python} DPSGDBudget.eps_target_str` $, 10^{-6})$.
...
- Target: $\epsilon =$ `{python} DPSGDBudget.eps_target_str`, $\delta = 10^{-6}$
