...
***Security***\index{Security!definition} is the protection of ML system _data_, _models_, and _infrastructure_ from _unauthorized access_, _manipulation_, and _disruption_ through _defensive mechanisms_ spanning development, deployment, and operational environments.
...
***Privacy***\index{Privacy!definition} is the protection of _sensitive information_ from _unauthorized disclosure_, _inference_, and _misuse_ through methods that preserve _confidentiality_ and _control over data usage_ across ML system environments.
...
In 2010, the Stuxnet worm covertly infiltrated Iran's Natanz nuclear facility, not by attacking the mainframes, but by subtly altering the rotational speeds of the uranium centrifuges while reporting normal operations to the monitoring dashboard. This watershed moment proved that manipulating the subtle physical parameters of a system is far more devastating than simply crashing it. In the context of machine learning, an attacker does not need to crash your inference server; they only need to subtly manipulate your model's decision boundary to achieve their goals.
...
Comprehensive defense against such weaponization requires zero-trust edge security, leveraging hardware security modules and secure boot protocols to ensure device integrity. With these historical lessons in mind, we can move beyond reacting to past breaches and develop a systematic framework for anticipating and quantifying the specific threats targeting our machine learning infrastructure.

## Systematic Threat Analysis and Risk Assessment {#sec-security-privacy-systematic-threat-analysis-risk-assessment-3ef1}

How do you protect a system when the attack surface includes every image it will ever see and every word it will ever process? Traditional cybersecurity focuses on securing networks and authenticating users, but ML systems introduce entirely new attack surfaces: the training data, the model weights, and the inference API. Systematic threat analysis forces us to map these unique surfaces, asking not just "Who can access the server?" but "Who can influence the model's behavior?"
...
Identifying whether an anomalous query pattern is a benign user or an active extraction attack is the crux of modern threat assessment. Armed with a structured understanding of these threat models, we will now drill down into the mechanics of the specific attack vectors that adversaries use to compromise model integrity and confidentiality.

## Model-Specific Attack Vectors {#sec-security-privacy-modelspecific-attack-vectors-0575}

A stop sign with three strategically placed pieces of black tape is recognized by a human as a vandalized stop sign, but an autonomous vehicle's vision system might confidently classify it as a speed limit sign. This is not a software bug in the traditional sense; it is an adversarial example. We will now deconstruct the precise mathematical techniques attackers use to craft these specific exploits against deployed models.
...
[^fn-adversarial-examples]: **Adversarial Examples**: Adversarial examples are inputs crafted to deceive ML models, first discovered by Szegedy et al. in 2013. These attacks can fool state-of-the-art image classifiers with perturbations invisible to humans (changing <0.01% of pixel values), affecting 99%+ of deep learning models.

[^fn-data-poisoning]: **Data Poisoning Attacks**: Data poisoning is an attack technique where adversaries inject malicious data during training, first formally studied in 2012. Studies show that poisoning just 0.1% of training data can reduce model accuracy by 10-50%, making it a highly efficient attack vector against ML systems.
...
# │ Show: "+2.5ms total" and "3% overhead" — inline in quantitative impact list.
...
class DefenseOverhead:
    """Namespace for model-extraction defense latency overhead calculation."""
...
total_defense_ms_str = DefenseOverhead.total_defense_ms_str
overhead_pct_str = DefenseOverhead.overhead_pct_str
```
...
While hardware-level isolation via TEEs provides a critical foundation for defending proprietary models against compromised infrastructure, protecting the model is only half the battle. What happens when the model itself is weaponized? As we will explore next, the same powerful pattern recognition capabilities we deploy for defense can be turned against us when adversaries use machine learning as an attack tool.
...
Implementing secure multi-party computation and gradient compression establishes a robust architecture for collaborative training without exposing raw data. Yet, even if the training process is perfectly secure, the final published model may still inadvertently memorize and regurgitate the sensitive information it was trained on. To guarantee that a model's outputs cannot be reverse-engineered to reveal individual records, we must enforce a rigorous mathematical standard known as differential privacy.
...
| **(Mobile/IoT devices)**    | side-channel attacks, model extraction            | • ARM TrustZone or similar TEE<br>• Model quantization + obfuscation<br>• Encrypted model storage<br>• Anti-tampering hardware (PUF)                                        | (&lt;50MB); quantization required for large models; 15-30% power overhead from encryption                                          |
| **Cloud ML Training**       | Data poisoning,                                   | • Secure data pipelines (provenance tracking)                                                                                                                               | Training time increases 30-120% with DP;                                                                                           |
| **(Multi-tenant platform)** | backdoor injection, gradient leakage              | • Differential Privacy (DP-SGD, ε ≈ 1-10)<br>• Gradient verification and anomaly detection<br>• Secure aggregation (if federated)<br>• Model watermarking for IP protection | gradient verification adds 10-15% compute overhead; federated aggregation requires secure communication protocols                  |
| **Public-Facing LLM**       | Prompt injection,                                 | • Input sanitization (prompt filtering)                                                                                                                                     | Aggressive filtering may block 5-10% of                                                                                            |
| **(Chatbot/API)**           | data extraction (training leakage), abuse/overuse | • Output monitoring (PII detection)<br>• Rate limiting (per-user quotas)<br>• Response watermarking<br>• Confidence thresholding (abstention)                               | legitimate requests; response time increases 50-100ms for content filtering; watermarking may be detectable by sophisticated users |
...
| **training)**               | privacy compliance (GDPR)                         | • Differential Privacy (ε ≤ 1)<br>• Homomorphic Encryption (for sensitive ops)                                                                                              | compute cost; accuracy may degrade 5-15%; requires legal agreements for liability                                                  |
...
We have established the defensive perimeter of the ML fleet, protecting it from adversarial manipulation and unauthorized inference. But security addresses only intentional threats. A model that is perfectly protected from attackers can still fail catastrophically when the real world shifts beneath it.
