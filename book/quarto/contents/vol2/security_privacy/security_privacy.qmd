...
Understanding when and where different attacks occur in the ML lifecycle helps prioritize defenses and understand attacker motivations. @Fig-ml-lifecycle-threats visualizes these stages and the specialized techniques adversaries use at each point, from data collection through model deployment and inference. During data collection, attackers can inject malicious samples or manipulate labels, especially in federated learning or crowdsourced data scenarios where data sources are less controlled. During training, backdoor insertion attacks embed hidden behaviors that activate only under specific trigger conditions, while label manipulation attacks systematically corrupt the learning process. During deployment, model theft becomes more attractive because trained models are accessible through APIs, file downloads, or reverse engineering of mobile applications. During inference, adversarial attacks craft runtime inputs that fool deployed models into incorrect predictions while appearing normal to human observers.
