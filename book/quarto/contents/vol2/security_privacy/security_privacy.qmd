...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, node distance=0.5cm]
  \tikzset{
    ...
    layerGreen/.style={layer, fill=GreenL},
    layerOrange/.style={layer, fill=OrangeL},
    layerBlue/.style={layer, fill=BlueL},
    layerGray/.style={layer, fill=GrayL}
  }

  % Layers
  \node[layerGreen] (Api) {\textbf{API / Interface Layer}\\Adversarial Examples, Model Extraction,\\Membership Inference};
  \node[layerOrange, below=of Api] (Model) {\textbf{Model Layer}\\Backdoor Injection, Trojan Weights,\\Parameter Theft};
  \node[layerBlue, below=of Model] (Data) {\textbf{Data Layer}\\Poisoning, Label Manipulation,\\Data Leakage};
  \node[layerGray, below=of Data] (Hard) {\textbf{Hardware / Infrastructure Layer}\\Side-channel Attacks, Fault Injection,\\Supply Chain Compromise};
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line cap=round,line join=round]
\tikzset{%
    ...
    BoxStyle/.style={draw=black, fill=\filllcolor!10, rounded corners=2pt, minimum width=53, minimum height=30, line width=0.75pt}
}

% Main diagram structure
\node[BoxStyle] (laptop) {};
\node[BoxStyle, right=of laptop] (usb) {};
\node[BoxStyle, right=of usb] (testing) {};

% Draw arrows and connections
\draw[Larrow] (laptop.east) -- (usb.west);
\draw[Larrow] (usb.east) -- (testing.west);

% Text below first row
\node[draw=none, fit=(laptop)(usb)(testing)] (BB1) {};
\node[TxtL, below=6pt of BB1.south west] (GT1) {\textcolor{RedLine}{\textbf{1. Infection}}\\[0.35ex] Stuxnet enters a system via a USB stick and proceeds to infect all machines running Microsoft Windows. By brandishing a digital certificate that seems to show that it comes from a reliable company, the worm is able to evade automated-detection systems.};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \tikzset{
    point/.style={circle, inner sep=2pt, fill=Black},
    axis/.style={->, line width=1.0pt},
    quadrant/.style={dashed, GrayLine, line width=0.75pt},
    critical/.style={text=RedLine, font=\bfseries},
    common/.style={text=OrangeLine, font=\bfseries},
    specialized/.style={text=OrangeLine, font=\bfseries},
    routine/.style={text=GrayLine, font=\bfseries},
    data/.style={point, label={right:\tiny #1}}
  }

  % Axes
  \draw[axis] (0,0) -- (8,0) node[right] {Likelihood};
  \draw[axis] (0,0) -- (0,8) node[above] {Impact};

  % Quadrants
  \draw[quadrant] (4,0) -- (4,8);
  \draw[quadrant] (0,4) -- (0,8);
  \draw[quadrant] (0,4) -- (8,4);

  % Labels
  \node[critical] at (6, 6) {CRITICAL};
  \node[common] at (6, 2) {COMMON};
  \node[specialized] at (2, 6) {SPECIALIZED};
  \node[routine] at (2, 2) {ROUTINE};

  % Data points
  \begin{scope}[every node/.style={data}]
    \node[fill=RedLine] at (7, 7) {Data Poisoning};
    \node[fill=OrangeLine] at (7, 3) {Model Extraction};
    \node[fill=OrangeLine] at (1.5, 7) {Hardware Side-channel};
    \node[fill=GrayLine] at (3, 2) {Membership Inference};
    \node[fill=RedLine] at (7.5, 5) {Prompt Injection};
  \end{scope}
...
```{.tikz}
\scalebox{0.85}{%
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape, line join=round]
\tikzset{%
    Line/.style={line width=1.0pt, GrayLine},
    Box/.style={inner xsep=2pt, draw=GreenLine, line width=0.75pt, fill=GreenL, align=flush center, minimum width=28mm, minimum height=8mm},
    Box2/.style={Box, draw=BrownLine, fill=BrownL},
    Box3/.style={Box, draw=VioletLine, fill=VioletL},
    LifecycleLabel/.style={below=4pt of BB2.north, inner sep=0pt, anchor=north},
    Background/.style={draw=GrayLine, inner xsep=10mm, inner ysep=4mm, fill=GrayL!60, line width=0.75pt}
}
...
\begin{scope}[on background layer]
    \node[Background, fit=(B1)(B4)](BB2){};
    \node[LifecycleLabel]{Lifecycle};
\end{scope}
...
\tikzset{%
    Line/.style={line width=1.0pt, GrayLine},
    Box/.style={inner xsep=2pt, inner ysep=4pt, draw=GreenLine, line width=0.75pt, fill=GreenL, align=flush center, text width=38mm, minimum width=38mm, minimum height=8mm},
    Box2/.style={Box, draw=BrownLine, fill=BrownL},
    Box3/.style={Box, draw=VioletLine, fill=VioletL, text width=42mm},
    BackgroundBox/.style={draw=GrayLine, inner xsep=10mm, line width=0.75pt, fill=GrayL!60},
    BackgroundBoxGreen/.style={draw=GreenLine, inner xsep=10mm, line width=0.75pt, fill=GreenL}
}
...
\begin{scope}[on background layer]
    \node[BackgroundBox, fit=(B1)(B5), minimum height=71.5mm, yshift=2.5mm](BB1){};
\end{scope}
\node[below=6pt of BB1.north, inner sep=0pt, anchor=north]{\textbf{Approximate Model Theft}};
...
\begin{scope}[on background layer]
    \node[BackgroundBoxGreen, fit=(RB1)(RB4), minimum height=71mm, yshift=2.2mm](BB2){};
\end{scope}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm]
  ...
  \tikzset{
    ...
    outerloop/.style={block, fill=OrangeL},
    innerloop/.style={block, fill=BlueL},
    evaluation/.style={block, fill=GrayL},
    redarrow/.style={arrow, RedLine}
  }

  \begin{scope}[align=center]
    % Outer Loop (Attacker)
    \node[outerloop] (Attacker) {\textbf{Outer Loop: Attacker}\\Optimize Poisoning Data $D_p$};

    % Inner Loop (System)
    \node[innerloop, below=of Attacker] (System) {\textbf{Inner Loop: System}\\Train Model $f$ on $D \cup D_p$};

    % Evaluation
    \node[evaluation, below=of System] (Eval) {Compute Validation Loss\\$\mathcal{L}(f, D_{test})$};
  \end{scope}
  ...
  \draw[redarrow] (Eval.east) -- ++(1,0) |- node[pos=0.2, right] {Maximize $\mathcal{L}$ (Attacker)} (Attacker.east);
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  \tikzset{
    ...
    arrow/.style={->, line width=1.0pt},
    decisionBoundary/.style={line width=1.0pt, BlueLine!60},
    regionLabel/.style={font=\textbf},
    constraintLabel/.style={font=\tiny},
    dashedCircle/.style={dashed, GrayLine, line width=0.75pt},
    perturbationLabel/.style={font=\tiny},
    cleanPoint/.style={point, fill=GreenLine!60!black, label={below:Clean $x$}},
    adversarialPoint/.style={point, fill=RedLine, label={above:Adversarial $x'$}},
    decisionBoundaryLabel/.style={BlueLine, font=\scriptsize}
  }

  % Decision Boundary (Curved)
  \begin{scope}
    \draw[decisionBoundary] plot [smooth, tension=1] coordinates {(-1,4) (2,2) (5,0)};
    \node[decisionBoundaryLabel] at (4, 1.5) {Decision Boundary};
  \end{scope}

  % Regions
  \node[regionLabel] (classA) at (0, 1) {Class A (Stop Sign)};
  \node[regionLabel] (classB) at (4, 3) {Class B (Speed Limit)};

  % Clean Point
  \node[cleanPoint] (X) at (1, 1.5) {};

  % Adversarial Point
  \node[adversarialPoint] (Xadv) at (2.5, 2.5) {};

  % Perturbation Vector
  \draw[arrow, RedLine] (X) -- (Xadv) node[midway, sloped, above, perturbationLabel] {Perturbation $\delta$};

  % Distance constraint
  \draw[dashedCircle] (X) circle (2.2);
  \node[constraintLabel] at (1, -0.8) {Constraint $\|\delta\| \leq \epsilon$};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85, transform shape, node distance=1cm]
  ...
  % Arrows
  \foreach \start/\end in {design/fab, fab/assembly, assembly/dist, dist/deploy} {
    \draw[arrow] (\start.east) -- (\end.west);
  }

  % Attack Vectors (Above/Below)
  \node[attack, above=of design] {Unintentional Bugs\\Spec Flaws};
  \node[attack, below=of fab] {Trojan Insertion\\Counterfeits};
  \node[attack, above=of assembly] {Component\\Substitution};
  \node[attack, below=of dist] {Interdiction\\Tampering};
  \node[attack, above=of deploy] {Physical\\Access};
...
\begin{scope}[local bounding box=G2, shift={(11,1)}, scale=1.5]
    \begin{axis}[myaxis, width=5cm, height=5cm]
        \addplot+[domain=-0.75:0.75, line width=1.25pt] {6.5*exp(-x^2/2)} node[pos=0.66] (T1) {};
        \addplot+[domain=-0.796:0.796, line width=1.25pt] {6.75*exp(-x^2/2)} node[pos=0.63] (T2) {};
        \addplot+[domain=-0.84:0.84, line width=1.25pt] {7*exp(-x^2/2)} node[pos=0.6] (T3) {};
        
        \node[anchor=west] at (T1.east) {0000};
        \node[anchor=west] at (T2.east) {1111};
        \node[anchor=west] at (T3.east) {0101};
    \end{axis}
    \node[box, minimum height=48mm, minimum width=67mm] (B2) at (rel axis cs:0.7,0.6) {};
\end{scope}
...
\begin{scope}
    ...
    \draw[dashedline] (B1.north east) -- (B2.north east);
    ...
    \draw[dashedline] (B1.south east) -- (I);
\end{scope}
\end{tikzpicture}
```
:::
...
\tikzset{%
    ...
    BoxBackground/.style={inner xsep=6mm, inner ysep=6mm, line width=0.75pt}
}
...
\begin{scope}[on background layer]
    \node[draw=GrayLine, fill=GrayL!60, fit=(B1)(B4), BoxBackground](BB1){};
\end{scope}
\node[below=6pt of BB1.north, anchor=north]{\textbf{Hardware-Level Security}};

% System-Level Security
\node[Box3, above=2.0 of B1](2B1){System Integrity Checks};
...
\begin{scope}[on background layer]
    \node[draw=BlueLine, fill=BlueL, fit=(2B1)(2B4), BoxBackground](BB2){};
\end{scope}
\node[below=6pt of BB2.north, anchor=north]{\textbf{System-Level Security}};
...
\begin{scope}[on background layer]
    \node[draw=GreenLine, fill=GreenL, fit=(3B1)(3B3), BoxBackground](BB3){};
\end{scope}
\node[below=6pt of BB3.north, anchor=north]{\textbf{Model-Level Security}};

% Data Privacy & Governance
\node[Box4, above left=2 and 0.3 of 3B2](4B1){Differential Privacy};
\node[Box4, above right=1.9 and 0.3 of 3B2](4B2){Federated Learning};
\node[Box4, above=of 4B1](5B1){Homomorphic Encryption};
\node[Box4, above=of 4B2](5B2){Synthetic Data Generation};

\begin{scope}[on background layer]
    \node[draw=RedLine, fill=VioletL, fit=(4B1)(5B2), BoxBackground](BB4){};
\end{scope}
\node[below=6pt of BB4.north, anchor=north]{\textbf{Data Privacy \& Governance}};

% Connections
\foreach \start/\end in {B1/2B1, B2/2B1.310, B3/3B3.230, B4/3B3, 2B1/3B1, 2B2.120/3B2, 2B3.80/3B2, 3B2.120/4B1, 3B2.70/4B2, 4B1/5B1, 4B2/5B2} {
    \draw[Line,-latex] (\start) -- (\end);
}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm and 2cm]
  ...
  \tikzset{
    ...
    description/.style={anchor=west, font=\scriptsize, text=GrayLine, align=left}
  }
  ...
  \begin{scope}
    ...
    % Clients
    \node[clientnode] (C1) [above left=of Server] {\textbf{Client A}\\$x_A + r$};
    \node[clientnode] (C2) [above right=of Server] {\textbf{Client B}\\$x_B - r$};
  ...
  % Description
  \node[description] at ($(Server.east)+(0.5cm,0)$) {Masks cancel\\during summation};
...
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, line join=round, scale=0.9, every node/.append style={transform shape}]
\tikzset{%
    ...
    Box/.style={inner xsep=2pt, inner ysep=6pt, draw=GreenLine, line width=0.75pt, fill=GreenL, align=flush center, text width=28mm, minimum width=28mm, minimum height=10.5mm},
    Box2/.style={Box, draw=RedLine, fill=RedL},
    Box3/.style={Box, draw=VioletLine, fill=VioletL, text width=42mm, minimum width=42mm, minimum height=8mm},
...
\node[Box](B1) {TRNG};
\node[Box](B2) [below=of B1] {Secure Enclave AES Engine};
\node[Box](B3) [below=of B2] {PKA};
\node[Box2](B4) [below=of B3] {I2C bus};
\node[Box3](B5) [below=1.35 of B4] {Secure Nonvolatile Storage};

\node[Box4, anchor=south west](SEP) at ($(B1.north east)!0.5!(B4.south east)+(1.25,0)$) {Secure Enclave\\ Processor};
\node[Box, anchor=west](MPE) at ($(SEP.east)+(1.25,0)$) {Memory Protection\\ Engine};

\begin{scope}[on background layer]
    \node[draw=none, inner xsep=5mm, inner ysep=4mm, fill=none, fit=(B1)(B4)(MPE), line width=0.75pt](BB1) {};
\end{scope}
...
\begin{scope}[on background layer]
    \node[draw=RedLine, inner xsep=5mm, inner ysep=5mm, fill=VioletL, fit=(BB1)(AP)(MC), line width=0.75pt](BB2) {};
    \node[above=4pt of BB2.south, inner sep=0pt, anchor=south] {\textbf{System on chip}};
\end{scope}

\begin{scope}[on background layer]
    \node[draw=GrayLine, inner xsep=5mm, inner ysep=4mm, fill=GrayL!60, fit=(B1)(B4)(MPE), line width=0.75pt](BB3) {};
    \node[above=6pt of BB3.south, inner sep=0pt, anchor=south] {\textbf{Secure Enclave}};
\end{scope}

\begin{scope}[local bounding box=KEY1, shift={($(KEY)+(0.4,-0.30)$)}, scale=1, every node/.append style={transform shape}]
    \pic[shift={(0,0)}] at (0,0) {key={scalefac=0.4, picname=1, drawchannelcolor=none, channelcolor=RedLine!79!GrayLine, Linewidth=1.0pt}};
\end{scope}
\end{tikzpicture}
```
:::
...
\tikzset{%
    ...
    Box/.style={inner xsep=2pt, inner ysep=6pt, draw=BlueLine, line width=0.75pt, fill=BlueL, align=flush center, text width=58mm, minimum width=58mm, minimum height=10mm},
    Box2/.style={Box, draw=RedLine, fill=RedL, rounded corners=12pt},
    Box3/.style={draw=VioletLine, fill=VioletL, trapezium, trapezium left angle=70, diamond, minimum width=45mm, minimum height=15mm, text centered, inner sep=-2ex},
    Box4/.style={Box, anchor=west, minimum width=42mm, minimum height=48mm},
    Box5/.style={Box, draw=BlueLine, fill=BlueL},
    Box6/.style={Box, draw=OrangeLine, fill=OrangeL},
}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.8]
    ...
    % Define styles
    \tikzset{
        BlueLine/.style={color=blue, line width=1.0pt},
        RedLine/.style={color=red, line width=1.0pt},
        GrayL/.style={fill=gray, opacity=0.5},
        BoxStyle/.style={draw, line width=0.75pt},
        ArrowStyle/.style={->, line width=1.0pt},
        LabelStyle/.style={anchor=west}
    }

    % Curves (use -(\x)^2 so exponent is unambiguously negative; avoids PGF "Dimension too large")
    \draw[BlueLine] plot[domain=-4:4, samples=100] (\x, {3*exp(-(\x)^2/2)}); % Distribution for D
    \draw[RedLine] plot[domain=-4:4, samples=100] (\x, {2.5*exp(-((\x-0.5)^2)/2)}); % Distribution for D'
    ...
    % Labels
    \node[LabelStyle] (labelD) at (1.5, 2.5) {$\Pr[\mathcal{A}(D)]$};
    \node[LabelStyle] (labelDprime) at (2.0, 1.5) {$\Pr[\mathcal{A}(D')]$};

    % Epsilon bound
    \draw[ArrowStyle] (-0.5, 3.0) -- (-0.5, 2.45) node[midway, left] {$e^\epsilon$};

    % Description
    \node[anchor=north, font=\scriptsize, text=GrayL] at (0,-0.5) {Distributions must be within $e^\epsilon$ factor everywhere.};
