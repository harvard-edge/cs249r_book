...
class DefenseOverheadAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt, fmt_count, fmt_percent, fmt_pp, fmt_time, fmt_usd
    ...
    attacker_daily_queries_str = fmt_count(attacker_daily_queries, scale='K', commas=False)
    ...
    paid_tier_queries_str = fmt_count(paid_tier_queries, scale='K', commas=False)
    alert_queries_per_day_str = fmt_count(alert_queries_per_day, scale='K', commas=False)
...
1.  **Rate limiting**:
    ...
    - Paid tier (`{python} DefenseOverheadAnalysis.paid_tier_price_usd_month_str`): `{python} DefenseOverheadAnalysis.paid_tier_queries_str` queries per day
...
4.  **Economic deterrent**:
    ...
    - Legitimate user cost: `{python} DefenseOverheadAnalysis.legit_queries_per_day_str` queries per day $\times$ `{python} DefenseOverheadAnalysis.billing_days_str` days $\times$ `{python} DefenseOverheadAnalysis.price_per_query_usd_str` = `{python} DefenseOverheadAnalysis.legit_monthly_cost_usd_str` per month (acceptable)
