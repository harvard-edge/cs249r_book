...
# │
# │ Imports: mlsys.constants (H100_MEM_CAPACITY, H100_MEM_BW, A100_MEM_BW,
# │          NVLINK_H100_BW, GB, TB, second)
# │ Exports: h100_mem_gb, h100_mem_bw_tbs, a100_mem_bw_tbs, nvlink_h100_gbs
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import (
...
class InferenceSetup:
    ...
    h100_mem_gb = f"{H100_MEM_CAPACITY.m_as(GB):.0f}"       # "80"
    h100_mem_bw_tbs = f"{H100_MEM_BW.m_as(TB / second)}"     # "3.35"
    a100_mem_bw_tbs = f"{A100_MEM_BW.m_as(TB / second):.0f}" # "2"
    nvlink_h100_gbs = f"{NVLINK_H100_BW.m_as(GB/second):.0f}" # "900"

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
h100_mem_gb = InferenceSetup.h100_mem_gb
h100_mem_bw_tbs = InferenceSetup.h100_mem_bw_tbs
a100_mem_bw_tbs = InferenceSetup.a100_mem_bw_tbs
nvlink_h100_gbs = InferenceSetup.nvlink_h100_gbs
...
@sec-performance-engineering established how to maximize throughput on individual forward passes through operator fusion, precision engineering, and graph compilation. This chapter confronts the next challenge: what happens when a single optimized model must serve thousands of concurrent users across a globally distributed fleet? Single-machine model serving establishes foundational principles that this chapter extends to distributed systems. Single-machine inference optimization, including batching, caching, model optimization, and hardware acceleration, provides the building blocks. This chapter addresses when those techniques reach their limits and distributed approaches become necessary.
...
[^fn-ttft-tpot]: **TTFT vs. TPOT**: **Time to First Token** (prefill) determines the perceived responsiveness; **Time Per Output Token** (decode) determines the perceived reading speed. Humans read at 5--10 tokens/sec; if TPOT exceeds 50 ms, the user perceives the system as "stuttering," even if TTFT was fast. This dual-SLA requirement forces schedulers to prioritize decode tokens over new prefill prompts. \index{TTFT!responsiveness}\index{TPOT!reading speed}
...
[^fn-serialization-zerocopy]: **Zero-Copy Serialization**: FlatBuffers (Google, 2014) and Cap'n Proto access wire-format data in place, eliminating the parse-allocate-copy cycle of Protocol Buffers or JSON. For distributed inference with large activation tensors, this drops per-hop serialization overhead from milliseconds to microseconds, reclaiming latency budget that would otherwise consume 5--10% of a tight SLO. \index{Serialization!zero-copy}
...
$$L_{\text{total}} = L_{\text{compute}} + L_{\text{network}} + L_{\text{serialization}} + L_{\text{coordination}} + L_{\text{queuing}}$$ {#eq-serving-tax}
...
The serving tax quantified above consumes a fraction of the latency budget per request. The true economic impact of inference, however, emerges when we consider cost over a model's entire operational lifetime. A critical insight for infrastructure planning is that serving cost typically dominates training cost over a model's operational lifetime. This reversal from the training-centric view of model development has profound implications for where optimization effort should focus. Training is a one-time capital expenditure (CapEx); serving is a continuous operational expenditure (OpEx) that scales with user growth. To make this cost dominance concrete, consider a worked example that reveals the *serving cost multiplier* in practice.
...
class ServingCostSimple:
    ...
    annual_reqs_b = round(annual_reqs / 1e9)
    # ┌── 3. GUARD ─────────────────────────────────────────
...
# ┌── EXPORTS ─────────────────────────────────────────────────
annual_reqs_b_str = ServingCostSimple.annual_reqs_b_str
```
...
2.  **Inference Volume**: $1M \text{ users} \times 50 \text{ reqs} \times 365 \text{ days}$ $\approx$ `{python} annual_reqs_b_str` Billion requests/year.
3.  **Cost per Request**: Assume a 70B model on H100 costs $\approx \$0.001$ per request (input + output).
4.  **Annual Serving Cost**: $18 \times 10^9 \times \$0.001 = \mathbf{\$18{,}000{,}000}$.

**The Systems Conclusion**: Serving costs **9$\times$** more than training in just the first year. A 10% optimization in inference latency saves **\$1.8M**—paying for the entire training run almost immediately.
:::
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-inference-scale-economics — training vs serving costs
# │
...
# │
# │ Imports: mlsys.constants (BILLION, SEC_PER_DAY, SEC_PER_YEAR)
# │ Exports: lifetime_queries_str, lifetime_queries_latex_str, serving_cost_total_str,
# │          training_cost_total_str, leverage_ratio_str, qps_str,
# │          serve_duration_days_str, cost_per_million_str
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsys.constants import BILLION, SEC_PER_DAY, SEC_PER_YEAR, THOUSAND, MILLION
...
class InferenceEconomics:
    ...
    if lifetime_queries_val >= 1e9:
        exp = int(round(math.log10(lifetime_queries_val)))
        mantissa = lifetime_queries_val / 10**exp
        lifetime_queries_latex_str = f"{mantissa:.2f} \\times 10^{{{exp}}}"
    else:
        lifetime_queries_latex_str = f"{lifetime_queries_val:,.0f}"
    serving_cost_total_str = fmt(serving_total_cost_val, precision=0)
    training_cost_total_str = fmt(training_total_cost, precision=0)
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
lifetime_queries_str = InferenceEconomics.lifetime_queries_str
lifetime_queries_latex_str = InferenceEconomics.lifetime_queries_latex_str
serving_cost_total_str = InferenceEconomics.serving_cost_total_str
training_cost_total_str = InferenceEconomics.training_cost_total_str
leverage_ratio_str = InferenceEconomics.leverage_ratio_str
qps_str = InferenceEconomics.qps_str
serve_duration_days_str = InferenceEconomics.serve_duration_days_str
cost_per_million_str = InferenceEconomics.cost_per_million_str
```

::: {.callout-notebook title="The Serving Cost Multiplier" collapse="true"}
...
- **Total Training**: `{python} training_cost_total_str`

**Serving Cost ($C_{serve}$)**

- Deployment: `{python} serve_duration_days_str` days (constant updates, amortized)
- Traffic: `{python} qps_str` Queries Per Second (QPS)
- Efficiency: `{python} cost_per_million_str` dollars per million queries

**Lifetime Volume ($Q_{total}$)**
$$ Q_{total} = `{python} qps_str` \text{/s} \times 86,400 \text{ s/day} \times `{python} serve_duration_days_str` \text{ days} \approx \mathbf{`{python} lifetime_queries_latex_str` \text{ queries}} $$

**Total Serving Cost**
$$ C_{serve\_total} = `{python} lifetime_queries_latex_str` \times \$10^{-5} = \mathbf{`{python} serving_cost_total_str`} $$

**Conclusion**:
$$ \text{Ratio} = \frac{`{python} serving_cost_total_str`}{`{python} training_cost_total_str`} \approx \mathbf{`{python} leverage_ratio_str`\times} $$
Serving optimization leverage is `{python} leverage_ratio_str`$\times$ higher than training optimization. A 1% serving efficiency gain saves significant capital—far exceeding the entire training budget.
:::
...
::: {#fig-serving-cost-curve fig-env="figure" fig-pos="htb" fig-cap="**The Serving Cost Curve**. Cost per 1000 tokens decreases with scale and optimization. Single-GPU serving never reaches profitability at typical pricing. Multi-GPU batching breaks even at moderate volume. Fully optimized serving (INT4, continuous batching, speculative decoding) achieves profitability at much lower volumes, fundamentally changing the economics of inference." fig-alt="Log-log plot of cost per 1000 tokens versus daily volume for single-GPU, multi-GPU, and optimized serving with break-even line."}
```{python}
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
...
ax.text(1.2e7, typical_pricing * 1.05, 'Typical pricing (break-even)', fontsize=9, color='#333333')
...
The cost ratios in @tbl-cost-ratios tell a static story, but the *dynamics* of cost accumulation reveal an even more striking pattern. @fig-serving-cost-crossover plots cumulative cost over a 36-month deployment window for three representative scenarios: a small model with low traffic, a 70B LLM serving one million daily active users, and a recommendation system serving 100 million users. The crossover point, where cumulative serving cost overtakes training cost, varies by orders of magnitude across these scenarios. For recommendation systems, serving dominates within days; for high-traffic LLMs, within roughly six weeks. Only small models with expensive training and low traffic see training cost dominate for extended periods. This temporal view reinforces why inference optimization yields the highest return on engineering investment for any model that reaches production scale.

::: {#fig-serving-cost-crossover fig-env="figure" fig-pos="htb" fig-cap="**The Serving Cost Crossover**. Cumulative cost of training versus serving for three deployment scenarios. For high-traffic LLMs, serving cost exceeds training cost within weeks. For recommendation systems at 100M daily active users, serving dominates within days. Only low-traffic, expensive-to-train models see training cost dominate for extended periods." fig-alt="Area chart showing cumulative cost over 36 months for three scenarios with crossover points marked"}
```{python}
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
...
# │ How: cum_serve = train + serve_per_month*months; viz.setup_plot().
# │
...
|:-------------------|:-------------------|:-------------------|:------------------|
| **Recommendation** | Very high (80–90%) | &lt;10 ms P99      | Embedding lookup  |
| **Vision (CNN)**   | Moderate (5–10%)   | 20–100 ms          | Batch efficiency  |
| **LLM**            | Lower (1–5%)       | 100 ms–10s         | Memory bandwidth  |
...
These four levels form a *serving hierarchy*, each building on the one below.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \node[level, fill=RequestColor, minimum width=3cm] (L1) {\textbf{Request Level}\\Batching, Caching, Preprocessing};
  \node[level, fill=ReplicaColor, minimum width=4cm, above=0.2cm of L1] (L2) {\textbf{Replica Level}\\GPU Memory (KV Cache), Kernel Optimization};
  \node[level, fill=ServiceColor, minimum width=5cm, above=0.2cm of L2] (L3) {\textbf{Service Level}\\Load Balancing, Autoscaling, Routing};
  \node[level, fill=PlatformColor, above=0.2cm of L3] (L4) {\textbf{Platform Level}\\Multi-tenancy, Isolation, Cluster Efficiency};

  \node[anchor=west, font=\scriptsize, text=black!50, right=0.2cm of L4.east] {Scale: Cluster};
  \node[anchor=west, font=\scriptsize, text=black!50, right=0.2cm of L3.east] {Scale: Service};
  \node[anchor=west, font=\scriptsize, text=black!50, right=0.2cm of L2.east] {Scale: GPU};
  \node[anchor=west, font=\scriptsize, text=black!50, right=0.2cm of L1.east] {Scale: Logic};
...
### Batching Strategy: The Throughput-Latency Tradeoff {#sec-inference-scale-framework-categories-172a}
...
Multi-GPU tensor parallelism shards each layer's weight matrices across multiple GPUs connected by high-bandwidth interconnect (NVLink at `{python} nvlink_h100_gbs` GB/s on H100). Each GPU computes a partial result for every layer, and an all-reduce operation synchronizes the partial results before the next layer. This topology is necessary when model weights exceed single-device memory and provides latency reduction proportional to the parallelism degree, at the cost of communication overhead per layer. The model sharding section (@sec-inference-scale-model-sharding-inference-82b3) analyzes the communication overhead quantitatively.
...
In production, recommendation systems constitute 80–90% of inference requests at major technology companies, with vision models handling most of the remainder, and LLMs currently representing 1–5% of request volume (though growing rapidly). Despite this distribution, we present batching strategies in order of conceptual complexity: vision (straightforward batching), LLMs (continuous batching with KV cache), and recommendation (feature-parallel batching with distributed embedding). This pedagogical ordering builds understanding progressively, even though practitioners will most frequently encounter recommendation workloads first.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.8]
  ...
  \tikzset{
    req/.style={draw=BlueLine, fill=BlueL, line width=0.75pt, minimum width=0.8cm, minimum height=0.4cm, font=\tiny},
    group/.style={draw=black!50, dashed, rounded corners=2pt, line width=1.0pt, inner sep=10pt}
  }

  % Static Batching
  \node[anchor=west] (titleA) at (0, 3.5) {\textbf{A. Static Batching} (Vision)};
  \node[group, fill=BlueL!40, below=0.2cm of titleA.south west, anchor=north west, minimum width=4cm, minimum height=1cm] (groupA) {};
  \node[req, right=0.2cm of groupA.west] (r1) {R1};
  \node[req, right=0.2cm of r1] (r2) {R2};
  \node[req, right=0.2cm of r2] (r3) {R3};
  \node[req, right=0.2cm of r3] (r4) {R4};
  \node[anchor=west, font=\scriptsize, text=black!60, right=0.2cm of groupA.east] {Wait for full batch};

  % Continuous Batching
  \node[anchor=west, below=1.5cm of titleA.west] (titleB) {\textbf{B. Continuous Batching} (LLM)};
  \node[group, fill=OrangeL!40, below=0.2cm of titleB.south west, anchor=north west, minimum width=4cm, minimum height=1cm] (groupB) {};
  \node[req, fill=GreenL, draw=GreenLine, right=0.2cm of groupB.west] (r1b) {R1};
  \node[req, fill=GreenL, draw=GreenLine, right=0.2cm of r1b] (r2b) {R2};
  \node[req, fill=BlueL, draw=BlueLine, right=0.2cm of r2b] (r5) {R5};
  \node[req, fill=black!10, draw=black!50, right=0.2cm of r5] (r6) {R6};
  \node[anchor=west, font=\scriptsize, text=black!60, right=0.2cm of groupB.east] {Insert mid-generation};

  % Feature Parallel
  \node[anchor=west, right=4cm of titleA.east] (titleC) {\textbf{C. Feature-Parallel} (RecSys)};
  \node[group, fill=GreenL!40, below=0.2cm of titleC.south west, anchor=north west, minimum width=4cm, minimum height=1cm] (groupC) {};
  \node[req, right=0.2cm of groupC.west, yshift=0.2cm] (u1) {U1};
  \node[req, right=0.2cm of u1] (u2) {U2};
  \node[req, right=0.2cm of u2] (u3) {U3};
  \node[req, below=0.1cm of u1] (i1) {I1};
  \node[req, below=0.1cm of u2] (i2) {I2};
  \node[req, below=0.1cm of u3] (i3) {I3};
  \node[anchor=west, font=\scriptsize, text=black!60, right=0.2cm of groupC.east] {Shard by feature};
...
*   $T_{\text{fixed}}$: Costs paid once per batch (e.g., loading weights from HBM, kernel launch latency).
*   $T_{\text{variable}}$: Marginal cost of adding one request (e.g., compute time for that sample).
...
# ┌── EXPORTS ─────────────────────────────────────────────────
required_concurrency_str = LittlesLawExample.required_concurrency_str
replica_throughput_str = LittlesLawExample.replica_throughput_str
replicas_needed_str = LittlesLawExample.replicas_needed_str
total_concurrency_str = LittlesLawExample.total_concurrency_str
```

::: {.callout-notebook title="Little's Law for Inference" collapse="true"}
...
**Required Concurrency ($L$)**: $L = 1000 \times 0.1 =$ **`{python} required_concurrency_str`** concurrent requests

**Capacity Planning**:
If a single GPU replica handles batch size 8 with 80 ms latency:

1. Replica Throughput $= 8 / 0.08 =$ `{python} replica_throughput_str` req/s
2. Replicas Needed $= 1000 /$ `{python} replica_throughput_str` $=$ `{python} replicas_needed_str` replicas

**Verification**:
Total system concurrency $=$ `{python} replicas_needed_str` replicas $\times$ 8 batch $=$ `{python} total_concurrency_str`.
Wait! We needed `{python} required_concurrency_str`.
*Correction*: We must account for queue depth. With `{python} replicas_needed_str` replicas active processing `{python} total_concurrency_str` reqs, 20 reqs are in queues.
...
This notation reflects a long tradition of *queuing theory and performance* analysis in systems engineering.

::: {.callout-note title="Queuing Theory and Performance"}
...
class Gpt3Scenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpt3_params_b = f"{params.m_as(param) / BILLION:.0f}"

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
gpt3_params_b = Gpt3Scenario.gpt3_params_b
```

Consider serving a GPT-3 class model (`{python} gpt3_params_b`B parameters) with the following characteristics:
...
3. **Batch size 32 minimizes total latency**: Despite longer service time (66 ms vs 54 ms), the dramatic reduction in queue wait time (8.9 ms vs 42.3 ms) yields lower total latency.
...
class LittlesLawVerify:
    ...
    arrival_rate = 100
    batch_size = 32
    expected_total_ms = 91.5
    utilization = 0.206
    # ┌── 2. EXECUTE ───────────────────────────────────────
    L = arrival_rate * (expected_total_ms / 1000)
    in_service = batch_size * utilization
    in_queue = L - in_service
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(8 < L < 12, f"L should be ~9, got {L:.2f}")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    expected_total_str = f"{expected_total_ms}"

# ┌── EXPORTS ─────────────────────────────────────────────────
expected_total_str = LittlesLawVerify.expected_total_str
```

::: {.callout-notebook title="Applying Little's Law to Verify" collapse="true"}
**Verification using Little's Law**: $L = \lambda \cdot W$

At B=32 with $\lambda = 100$ req/s and $E[T] =$ `{python} expected_total_str` ms:
...
Production systems must select batch size to meet Service Level Objectives (SLOs), typically specified as latency percentiles (e.g., P99 latency < 200 ms). The following framework systematizes this decision:
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9]
  ...
  % Grid
  \draw[black!20, thin] (0,0) grid[step=1] (7.5,4.5);

  % Operating region (shaded)
  \fill[BlueL] (0.5,1.5) -- (0.5,4) -- (5,4) -- (5,1.5) -- cycle;

  % SLO boundary
  \draw[RedLine, thick, dashed] (5,0) -- (5,4.5) node[above, font=\scriptsize] {SLO};

  % Minimum throughput
  \draw[OrangeLine, thick, dashed] (0,1.5) -- (7.5,1.5) node[right, font=\scriptsize] {Min $\lambda$};

  % Trade-off curve (batch size increasing)
  \draw[BlueLine, very thick]
  ...
  % Optimal point
  \fill[GreenLine] (5,4) circle (3pt);
  \node[above right, font=\scriptsize, GreenLine] at (5,4) {Optimal $B$};
...
$$E[L_{\text{total}}] = E[L_{queue}] + L_{batch} + L_{inference}(B)$$ {#eq-dynamic-batch-latency}
...
**Tradeoff**: Option C achieves 25% better throughput (lower utilization) at the cost of higher average latency (22 ms vs 15 ms). Both meet the 50 ms P99 SLO.
...
1.  **Significance (Quantitative):** It maximizes the **System Throughput ($\eta$)** by eliminating the padding waste and head-of-line blocking inherent in static batching. It ensures the GPU remains saturated even when requests have widely varying sequence lengths.
2.  **Distinction (Durable):** Unlike **Static or Dynamic Batching** (which group requests at the *request* level), Continuous Batching operates at the **Iteration Level**, dynamically reshaping the compute tensor at each clock cycle.
3.  **Common Pitfall:** A frequent misconception is that Continuous Batching is "purely a scheduler change." In reality, it requires a **Dynamic Memory Manager** (like PagedAttention) because the KV caches for different requests grow and shrink at different rates, preventing static memory pre-allocation.
...
[^fn-orca-scheduling]: **Orca (Iteration-Level Scheduling)**: Developed at Seoul National University and FriendliAI (OSDI 2022), Orca demonstrated that scheduling at iteration granularity rather than request granularity allows sequences to enter and exit batches at each decode step. This eliminated the head-of-line blocking that wasted 50%+ of GPU compute in static batching, establishing the scheduling paradigm that vLLM and all modern LLM serving systems now adopt. \index{Orca!iteration-level scheduling}
...
Continuous batching (also called iteration-level batching) decouples batch membership from iteration boundaries. At each decode iteration, the system checks for completed sequences, removes completed sequences from the batch immediately, inserts waiting sequences into freed slots, and processes the reorganized batch for the next iteration. This technique is central to the *throughput-latency* tradeoff that defines large-scale LLM serving.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, xscale=0.8, yscale=0.7]
  ...
  \tikzset{
    req/.style={fill=BlueL, draw=BlueLine, line width=0.75pt, minimum height=0.8cm, anchor=south west, inner sep=0pt, align=center},
    idle/.style={fill=black!5, draw=black!30, dashed, line width=0.75pt, minimum height=0.8cm, anchor=south west, inner sep=0pt}
  }

  % Panel A: Static Batching
  \begin{scope}
    \node[anchor=west, RedLine] at (0, 4.5) {\textbf{A. Static Batching}};
    % Batch 1
    \node[req, minimum width=2cm] at (0, 3) {R1};
    \node[req, minimum width=5cm] at (0, 2) {R2};
    \node[req, minimum width=3cm] at (0, 1) {R3};
    % Idle regions
    \node[idle, minimum width=3cm] at (2, 3) {};
    \node[idle, minimum width=2cm] at (3, 1) {};
    % Vertical barrier
    \draw[thick, RedLine, dashed] (5, 0.5) -- (5, 4.2) node[above, font=\tiny] {Batch Barrier};

    % Batch 2 starts after barrier
    \node[req, minimum width=2.8cm] at (5.2, 3) {R4};
    \node at (2.5, -0.5) {Idle Compute};
  \end{scope}

  % Panel B: Continuous Batching
  \begin{scope}[shift={(10,0)}]
    \node[anchor=west, RedLine] at (0, 4.5) {\textbf{B. Continuous Batching}};
    % R1 ends at 2, R4 enters immediately
    \node[req, minimum width=2cm] at (0, 3) {R1};
    \node[req, fill=OrangeL, draw=OrangeLine, minimum width=2.9cm] at (2.1, 3) {R4};
    \node[req, fill=OrangeL, draw=OrangeLine, minimum width=2.9cm] at (5.1, 3) {R5};

    \node[req, minimum width=5cm] at (0, 2) {R2};
    \node[req, fill=GreenL, draw=GreenLine, minimum width=3.9cm] at (5.1, 2) {R6};

    \node[req, minimum width=3cm] at (0, 1) {R3};
    \node[req, fill=VioletL, draw=VioletLine, minimum width=3.9cm] at (3.1, 1) {R7};
...
These analytical gains translate directly into production systems. The following implementation study examines how *vLLM* realizes continuous batching through iteration-level scheduling, paged memory management, and preemption.

::: {.callout-note title="Continuous Batching in vLLM"}
...
# │
# │ Imports: (none)
# │ Exports: mean_output_str, total_batch_time_str, total_seq_iters_str,
# │   useful_seq_iters_str, r1_lat_str, r2_lat_str, r3_lat_str, r4_lat_str,
# │   trad_lat_str, cont_avg_lat_str, latency_red_str, r1_imp_str, r3_imp_str,
# │   r4_imp_str
# └─────────────────────────────────────────────────────────────────────────────
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
mean_output_str = BatchingWorkedExample.mean_output_str
total_batch_time_str = BatchingWorkedExample.total_batch_time_str
total_seq_iters_str = BatchingWorkedExample.total_seq_iters_str
useful_seq_iters_str = BatchingWorkedExample.useful_seq_iters_str
r1_lat_str = BatchingWorkedExample.r1_lat_str
r2_lat_str = BatchingWorkedExample.r2_lat_str
r3_lat_str = BatchingWorkedExample.r3_lat_str
r4_lat_str = BatchingWorkedExample.r4_lat_str
trad_lat_str = BatchingWorkedExample.trad_lat_str
cont_avg_lat_str = BatchingWorkedExample.cont_avg_lat_str
latency_red_str = BatchingWorkedExample.latency_red_str
r1_imp_str = BatchingWorkedExample.r1_imp_str
r3_imp_str = BatchingWorkedExample.r3_imp_str
r4_imp_str = BatchingWorkedExample.r4_imp_str
```
...
- Mean output length: (50 + 200 + 100 + 150) / 4 = `{python} mean_output_str` tokens
...
- Total batch time: $200\times20$ ms = `{python} total_batch_time_str` ms
- Request completion times:
  - R1 completes useful work at iteration 50, but waits until iteration 200 → latency = `{python} trad_lat_str` ms
  - R2 completes at iteration 200 → latency = `{python} trad_lat_str` ms
  - R3 completes useful work at iteration 100, but waits until iteration 200 → latency = `{python} trad_lat_str` ms
  - R4 completes useful work at iteration 150, but waits until iteration 200 → latency = `{python} trad_lat_str` ms

Waste calculation using @eq-waste-ratio:

$$W = 1 - \frac{125}{200} = 1 - 0.625 = 37.5\%$$

The GPU performs $4\times200$ = `{python} total_seq_iters_str` "sequence-iterations" but only `{python} useful_seq_iters_str` are useful.
...
- R1: $50\times20$ ms = `{python} r1_lat_str` ms (`{python} r1_imp_str`$\times$ improvement over traditional)
- R3: $100\times20$ ms = `{python} r3_lat_str` ms (`{python} r3_imp_str`$\times$ improvement)
- R4: $150\times20$ ms = `{python} r4_lat_str` ms (`{python} r4_imp_str`$\times$ improvement)
- R2: $200\times20$ ms = `{python} r2_lat_str` ms (no improvement for longest request)

Average latency comparison

- Traditional: `{python} trad_lat_str` ms (all requests)
- Continuous: (`{python} r1_lat_str` + `{python} r3_lat_str` + `{python} r4_lat_str` + `{python} r2_lat_str`) / 4 = `{python} cont_avg_lat_str` ms

This represents a **`{python} latency_red_str`% reduction in average latency**, exactly matching the waste ratio.

::: {.callout-note title="Traditional vs. Continuous Batching" collapse="false"}
```{.tikz}
...
For new LLM serving deployments, continuous batching frameworks like vLLM, TensorRT-LLM, or TGI provide the implementation complexity as a solved problem. The decision becomes whether to adopt these frameworks versus building custom serving infrastructure. For organizations with existing traditional batching systems, the migration cost must be weighed against the workload's output length variance using @tbl-continuous-batching-benefit.
...
**Problem**: An LLM serving system exhibits unexpectedly high tail latency. P50 latency is 100 ms and P95 is 180 ms, both within SLO, but P99 spikes to 500 ms against a 200 ms target. GPU utilization appears healthy at 85%. Where is the bottleneck?
...
# │ Show: "80" GB, "2" TB/s, "600" GB/s NVLink, "64" GB/s PCIe Gen5.
# │ How: pulling constants from mlsys.constants.
# │
# │ Imports: mlsys.constants (A100_MEM_CAPACITY, A100_MEM_BW, NVLINK_A100_BW,
# │          PCIE_GEN5_BW, GB, TB, second)
# │ Exports: a100_mem_gb, a100_mem_bw_tbs, nvlink_a100_gbs, pcie_gen5_gbs
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import A100_MEM_CAPACITY, A100_MEM_BW, NVLINK_A100_BW, PCIE_GEN5_BW, GB, TB, second
...
class A100HardwareScenario:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    a100_mem_gb = f"{A100_MEM_CAPACITY.m_as(GB):.0f}"       # "80"
    a100_mem_bw_tbs = f"{A100_MEM_BW.m_as(TB / second):.0f}" # "2"
    nvlink_a100_gbs = f"{NVLINK_A100_BW.m_as(GB / second):.0f}"  # "600"
    pcie_gen5_gbs = f"{PCIE_GEN5_BW.m_as(GB / second):.0f}"  # "64"

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
a100_mem_gb = A100HardwareScenario.a100_mem_gb
a100_mem_bw_tbs = A100HardwareScenario.a100_mem_bw_tbs
nvlink_a100_gbs = A100HardwareScenario.nvlink_a100_gbs
pcie_gen5_gbs = A100HardwareScenario.pcie_gen5_gbs
```

The system runs on 4 A100-`{python} a100_mem_gb`GB GPUs connected via PCIe Gen4 (`{python} pcie_gen5_gbs` GB/s per GPU) rather than NVLink. The server has a dual-socket CPU with NUMA topology. Memory bandwidth per GPU is `{python} a100_mem_bw_tbs` TB/s (HBM2e), adequate for decode operations. However, PCIe bandwidth limits tensor parallelism communication to `{python} pcie_gen5_gbs` GB/s versus NVLink's `{python} nvlink_a100_gbs` GB/s. For a batch requiring 100 MB activation transfers between GPUs, PCIe adds approximately 1.5 ms per synchronization point versus 0.17 ms for NVLink.

**Distribution Layer Analysis** (Algorithmic Behavior):

Dynamic batching is configured with max batch size 32 and timeout 50 ms. Examining the batch size distribution reveals the problem: 90% of batches contain 4 to 8 requests (explaining good P50/P95), but 5% of batches reach the full 32 requests. These large batches occur during traffic bursts and experience head-of-line blocking: short requests that would complete quickly must wait for long-sequence requests in the same batch to finish all their decode iterations.
...
This parallelization is natural when embeddings are sharded across servers: each embedding server handles lookups for its shard across all requests in the batch. To appreciate the scale at which this operates, consider *RecSys batching at Meta scale*.
...
**Batch accumulation window**: 1 ms
...
These adaptive principles are embodied in production serving infrastructure. The *NVIDIA Triton Inference Server* implements a configurable adaptive batching system that illustrates how SLO-aware and request-aware strategies operate in practice.

::: {.callout-note title="Adaptive Batching: Triton"}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.5cm]
  \definecolor{ProcessColor}{HTML}{D1E6F3}
  \definecolor{NetworkColor}{HTML}{FCE4CC}
  \definecolor{QueueColor}{HTML}{D4EFDF}
  \definecolor{RedLine}{HTML}{CB202D}
  \definecolor{VioletL}{HTML}{E6D4E5}
  ...
  \node[process, right=of batcher, fill=VioletL] (gpu) {Model\\Execution (GPU)};
  ...
  % Latency Sources
  \node[below=0.8cm of lb, font=\scriptsize, align=center, color=RedLine] {Parsing \&\\Routing Latency};
  \node[below=0.8cm of queue, font=\scriptsize, align=center, color=RedLine] {Queuing\\Delay};
  \node[below=0.8cm of batcher, font=\scriptsize, align=center, color=RedLine] {Batch\\Sync Time};
  \node[below=0.8cm of gpu, font=\scriptsize, align=center, color=RedLine] {Compute\\Latency};
...
3.  **Common Pitfall:** A frequent misconception is that the KV Cache is "fixed-size." In reality, it is **Dynamic and Fragmented**: because different requests have different lengths, the cache can cause massive memory waste (up to 60--80%) due to internal fragmentation if not managed by a virtual memory system.
...
# │ How: KV = 2*80*8*128*2 bytes/token; mb_per_token; viz.set_book_style().
# │
# │ Imports: numpy (np), matplotlib.pyplot (plt), mlsys.viz (viz)
# │ Exports: (figure only, no prose variables)
# └─────────────────────────────────────────────────────────────────────────────
...
ax.text(5, hbm_cap_gb + 2, 'HBM Capacity (80 GB)', color=COLORS['RedLine'], fontweight='bold')
...
This visualization reveals why the **Distribution Layer** must sometimes shard models that would otherwise fit on a single GPU. Sharding does not just reduce compute time; it provides the **Memory Headroom** needed to maintain high batch sizes for long-context requests. Without sharding, a 128K context request effectively "evicts" all other users from the GPU.
...
Internal fragmentation wastes memory within each allocation. Sequences shorter than the maximum allocation leave the unused portion idle. If maximum length is 4,096 but average output is 100 tokens, 97.5% of allocated memory is wasted.
...
***PagedAttention***\index{PagedAttention!definition} is a memory management technique that applies virtual memory principles to **KV Cache** allocation, storing attention states in non-contiguous, fixed-size physical blocks.

1.  **Significance (Quantitative):** It eliminates **Internal and External Fragmentation**, which can waste up to 80% of GPU memory. By allowing sequences to grow dynamically, it enables 2$\times$ to 4$\times$ higher **Concurrent Throughput ($\eta$)** on the same hardware.
2.  **Distinction (Durable):** Unlike **Contiguous Allocation** (which requires pre-reserving the maximum context length), PagedAttention uses a **Block Table** to map logical sequence indices to physical memory pages, allocating only what is currently used.
...
PagedAttention [@kwon2023vllm], introduced in vLLM, applies virtual memory concepts to KV cache management. Instead of contiguous allocation, the KV cache is divided into fixed-size pages (typically 16--256 tokens), and sequences are allocated pages on demand.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9]
  ...
  \tikzset{
    ...
    arrow/.style={->, >=stealth, thick, black!50!60}
  }
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  \tikzset{
    block/.style={draw=black!60, fill=BlockColor, minimum width=0.8cm, minimum height=0.6cm, font=\scriptsize, inner sep=0pt},
  ...
  \node[right=0.2cm of pb5, font=\scriptsize] {(System Prompt)};
  ...
  \node[font=\scriptsize, right=0.1cm of la1] (la_dots) {...};
  ...
  \node[font=\scriptsize, right=0.1cm of lb1] (lb_dots) {...};
  ...
  % Annotations
  \node[align=left, font=\scriptsize, anchor=west, right=2cm of la6] {Both requests point\\to same physical blocks\\for blocks 0-5};
...
# │ Show: kv_tb_no_cache_str="~2.6" TB (without), kv_tb_cached_str="~0.66" TB
# │   (with), memory_reduction_pct_str="75"%, concurrent_multiplier_str="4" —
...
# │
# │ Imports: (none)
# │ Exports: tokens_per_user_str, kv_tb_no_cache_str, total_tokens_cached_str,
# │   kv_tb_cached_str, memory_reduction_pct_str, concurrent_multiplier_str
# └─────────────────────────────────────────────────────────────────────────────
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
tokens_per_user_str = PrefixCachingAnalysis.tokens_per_user_str
kv_tb_no_cache_str = PrefixCachingAnalysis.kv_tb_no_cache_str
total_tokens_cached_str = PrefixCachingAnalysis.total_tokens_cached_str
kv_tb_cached_str = PrefixCachingAnalysis.kv_tb_cached_str
memory_reduction_pct_str = PrefixCachingAnalysis.memory_reduction_pct_str
concurrent_multiplier_str = PrefixCachingAnalysis.concurrent_multiplier_str
```
...
- KV cache per user: 2000 + 500 (avg response) = `{python} tokens_per_user_str` tokens
- Total KV cache: `{python} tokens_per_user_str`$\times$ 1000$\times$ 2$\times$ 80$\times$ $8192\times2$ = `{python} kv_tb_no_cache_str` TB

**With prefix caching**:

- Shared prefix: 2000 tokens (once)
- Unique per user: 500 tokens
- Total: ($2000\times1$) + ($500\times1000$) = `{python} total_tokens_cached_str` tokens
- Memory: `{python} total_tokens_cached_str`$\times$ 2$\times$ 80$\times$ $8192\times2$ = `{python} kv_tb_cached_str` TB

**Savings**: `{python} memory_reduction_pct_str`% reduction in KV cache memory, enabling `{python} concurrent_multiplier_str`$\times$ more concurrent users.
...
# │ Show: ~84.9 GB FP16 vs ~10.6 GB INT4 → 8x compression.
...
# ┌── LEGO ───────────────────────────────────────────────
from mlsys.constants import H100_MEM_CAPACITY, GB
class KVCacheCalc:
    ...
    h100_mem_gb = f"{h100_mem.m_as(GB):.0f}"
...
h100_mem_gb = KVCacheCalc.h100_mem_gb
```
...
This is a substantial fraction of the H100's `{python} h100_mem_gb` GB of HBM, consumed by a single request's KV cache alone. For a batch of 8 concurrent requests, the KV cache would require `{python} f"{kv_fp16_gb * 8:.0f}"` GB, exceeding single-GPU capacity. Reducing this footprint requires two complementary strategies: quantization and architectural optimization.
...
Compressing the KV cache to INT4 reduces it to approximately `{python} kv_int4_gb_str` GB per request, a `{python} kv_compression_str`$\times$ reduction. This freed memory directly translates into larger batch sizes and higher serving throughput. KV cache values exhibit different distributions than model weights: Keys have relatively uniform distributions and tolerate aggressive quantization (2-4 bits), while Values are more sensitive and require careful calibration (4-6 bits) or asymmetric precision like the **KIVI** scheme.

#### Grouped Query Attention (GQA) {#sec-inference-scale-gqa}

An architectural complement to quantization is **Grouped Query Attention (GQA)** [@ainslie2023gqa]. In standard Multi-Head Attention (MHA), every attention head has its own key and value projections. In GQA, multiple query heads share a single key-value head. A model with 32 query heads and 8 KV heads uses a group size of 4: every 4 query heads share one K and one V projection.
...
This is dramatically more manageable than the `{python} kv_fp16_gb_str` GB required by MHA. Combining GQA with INT8 KV cache quantization yields sub-gigabyte per-request cache sizes, enabling batch sizes of 50 or more on a single GPU. GQA has become the dominant architectural choice for inference-optimized LLMs (Llama 3, Mistral, Gemma) because its quality impact is typically less than 1% on standard benchmarks while enabling an order-of-magnitude increase in serving throughput.
...
class PrecisionDividend:
    ...
    int4_weight_per_gpu_str = f"{int4_weight_per_gpu}"
    avail_kv_int4_str = f"{avail_kv_int4}"

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
avail_kv_fp16_str = PrecisionDividend.avail_kv_fp16_str
int4_weight_per_gpu_str = PrecisionDividend.int4_weight_per_gpu_str
avail_kv_int4_str = PrecisionDividend.avail_kv_int4_str
fp16_weight_per_gpu = PrecisionDividend.fp16_weight_per_gpu
```

::: {.callout-notebook title="The Precision Dividend"}

**Problem**: You serve a 70B model on 4$\times$ H100 GPUs. Weights in FP16 consume 140 GB (35 GB/GPU). KV cache at FP16 consumes `{python} kv_fp16_gb_str` GB per request. How does quantizing weights to INT4 and KV cache to INT8 change the maximum batch size?

**Before optimization** (all FP16):

- Weights: 35 GB/GPU
- Available for KV cache: $80 -$ `{python} fp16_weight_per_gpu` $=$ `{python} avail_kv_fp16_str` GB/GPU
- KV cache per request: `{python} kv_fp16_gb_str` GB $\div$ 4 GPUs $\approx$ `{python} f"{kv_fp16_gb / 4:.1f}"` GB/GPU
- Maximum batch size: $\lfloor$ `{python} avail_kv_fp16_str` $/$ `{python} f"{kv_fp16_gb / 4:.1f}"` $\rfloor$ $\approx$ `{python} f"{int(45 / (kv_fp16_gb / 4))}"` requests

**After optimization** (INT4 weights, INT8 KV cache):

- Weights: `{python} fp16_weight_per_gpu` GB $\times$ (4/16) $=$ `{python} int4_weight_per_gpu_str` GB/GPU (INT4)
- Available for KV cache: $80 -$ `{python} int4_weight_per_gpu_str` $=$ `{python} avail_kv_int4_str` GB/GPU
- KV cache per request (INT8): `{python} f"{kv_fp16_gb / 2:.1f}"` GB $\div$ 4 $\approx$ `{python} f"{kv_fp16_gb / 2 / 4:.1f}"` GB/GPU
- Maximum batch size: $\lfloor$ `{python} avail_kv_int4_str` $/$ `{python} f"{kv_fp16_gb / 2 / 4:.1f}"` $\rfloor$ $\approx$ `{python} f"{int(float(PrecisionDividend.avail_kv_int4) / (kv_fp16_gb / 2 / 4))}"` requests
...
***Speculative Decoding***\index{Speculative Decoding!definition} is a latency optimization that uses a smaller **Draft Model** to predict multiple future tokens, which are then verified in parallel by the full **Target Model** in a single forward pass.

1.  **Significance (Quantitative):** It breaks the sequential bottleneck of autoregressive generation, reducing **Wall-Clock Latency ($L_{\text{lat}}$)** by a factor of 2$\times$ to 3$\times$ depending on the **Acceptance Rate** of the draft tokens. It trades extra compute ($O$) for reduced time.
...
3.  **Common Pitfall:** A frequent misconception is that Speculative Decoding "always speeds up" inference. In reality, it is a **Probabilistic Bet**: if the draft model is too inaccurate (low acceptance rate), the extra compute and overhead of verification can actually make the system *slower* than sequential decoding.
...
[^fn-speculative-cpu-analogy]: **Speculative Execution (CPU to LLM)**: Borrows from CPU branch prediction, where processors execute instructions ahead of confirmation and roll back on misprediction. The critical difference: CPU speculation is invisible to software, while LLM speculation requires explicit draft-verify logic and adds a second model's memory footprint to the serving budget. The trade-off is extra compute and memory for 2--3$\times$ latency reduction, but only when draft acceptance rates exceed roughly 70%. \index{Speculative Decoding!CPU analogy}

The speculative decoding algorithm proceeds in three phases:

1.  **Draft phase**: A small, fast model (the "draft model") generates $K$ candidate tokens autoregressively. Because the draft model is much smaller (e.g., 1B parameters versus 70B for the target), each draft step is fast. Generating $K$ draft tokens typically takes roughly the time of one or two target model decode steps.
2.  **Verification phase**: The target model processes all $K$ draft tokens *in parallel* in a single forward pass. Because this is a forward pass over $K$ known tokens (not autoregressive generation), the operation is a standard GEMM with much higher arithmetic intensity than batch-1 decode.
3.  **Acceptance phase**: Starting from the first draft token, the system compares the draft model's probability distribution with the target model's distribution. Continue accepting tokens until a draft token is rejected or all $K$ are accepted. On rejection, sample a corrected token from a modified distribution that accounts for the draft model's error.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  \tikzset{
     stepbox/.style={draw=black!50, rounded corners=2pt, minimum height=0.7cm, font=\scriptsize, align=center}
  }
  ...
  \node[right=0.2cm of t4, font=\scriptsize, align=left] {Accept 3\\Reject 1};
...
For Llama-70B with 4K context, KV cache is ~80 MB per sequence. At 100 Gbps, transfer takes 6.4 ms versus ~500 ms for prefill. Transfer is clearly better.
...
2.  **Distinction (Durable):** Unlike **Single-Pass Inference** (e.g., ImageNet), where the resource bottleneck is constant, LLM inference switches between these regimes at every request, requiring **Iteration-Level Scheduling** to maintain utilization.
...
This dichotomy creates a scheduling challenge: prefill operations are long-running and compute-intensive, while decode operations are short and bandwidth-limited.
 Mixing them in the same batch can cause interference. This creates the **Static Power Waste** problem analyzed in @sec-sustainable-ai, where low-utilization decode steps dominate energy consumption.
...
- **Decode pool**: Optimized for memory bandwidth (e.g., nodes with maximum HBM capacity or specialized High Bandwidth Flash as discussed in @sec-data-storage) to handle thousands of concurrent autoregressive streams.
...
KV Cache management enables a single GPU to handle many concurrent requests without crashing. What happens, however, when the model itself—like a 175-billion parameter foundation model—is simply too large to fit on a single GPU, let alone its KV Cache? The answer requires splitting the workload across multiple devices through model sharding for inference.
...
# │ Show: "80" GB.
# │ How: pulling H100_MEM_CAPACITY from mlsys.constants.
# │
# │ Imports: mlsys.constants (H100_MEM_CAPACITY, GB)
# │ Exports: h100_mem_gb
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import H100_MEM_CAPACITY, GB
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
h100_mem_gb = HardwareSetupSharding.h100_mem_gb
```

A 70-billion parameter model requires over 140 GB of VRAM just to hold its weights in 16-bit precision, making it physically impossible to load onto a standard 80 GB A100 GPU. To serve this model, we must slice its architecture across multiple GPUs that act together as a single logical replica. This requires adapting our distributed training parallelism techniques—like tensor and pipeline parallelism—specifically for low-latency inference.
...
This exceeds the `{python} h100_mem_gb`GB capacity of an H100 GPU, requiring at minimum 2-way sharding.
...
The multi-head attention computation naturally partitions across attention heads. For a model with $H$ attention heads distributed across $P$ devices, each device computes $H/P$ heads:
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  \node[above, font=\bfseries] at (gpu0_bg.north) {GPU 0};

  \node[gpu_bg] (gpu1_bg) at (4.25, -0.5) {};
  \node[above, font=\bfseries] at (gpu1_bg.north) {GPU 1};
...
$$T_{inference} = \frac{T_{\text{compute}}}{P} + 2 \times T_{\text{allreduce}}\left(\frac{A}{P}\right)$$ {#eq-tensor-parallel-time}

where $T_{\text{compute}}$ is the sequential compute time, $P$ is the parallelism degree, and $A$ is the activation size being reduced. The factor of 2 accounts for the two all-reduce operations per transformer layer (attention and feed-forward).
...
**Minimum sharding**: 2-way (140 GB / `{python} h100_mem_gb`GB per H100)
...
# │ How: Divide per-layer compute times; multiply by n_layers for full-model timing.
# │
# │ Imports: (none — pure Python arithmetic, no pint quantities)
# │ Exports: tp_speedup_str, tp_theoretical, n_layers, seq_total_ms_str,
# │   tp8_total_ms_str
# └─────────────────────────────────────────────────────────────────────────────
...
class TpSpeedupCalc:
    ...
    # Step 1: Full-model per-token timing
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
tp_speedup_str = TpSpeedupCalc.tp_speedup_str
tp_theoretical = TpSpeedupCalc.tp_theoretical
n_layers = TpSpeedupCalc.n_layers
seq_total_ms_str = TpSpeedupCalc.seq_total_ms_str
tp8_total_ms_str = TpSpeedupCalc.tp8_total_ms_str
nvlink_h100_gbs = TpSpeedupCalc.nvlink_h100_gbs
```
...
**For `{python} n_layers` layers**:

- Sequential: `{python} seq_total_ms_str`ms per token
- 8-way TP: `{python} tp8_total_ms_str`ms per token

The `{python} tp_speedup_str`$\times$ speedup (vs theoretical `{python} tp_theoretical`$\times$) reflects communication overhead. With `{python} nvlink_h100_gbs` GB/s NVLink bandwidth, each 8 MB activation all-reduce takes ~0.3 ms.
...
This architecture enables scaling the total parameter count while keeping the per-token compute constant. However, serving such models at scale requires **Expert Parallelism (EP)**, where different experts are hosted on different GPUs.
...
The trade-off is memory capacity: all experts must reside in memory even though only a fraction are active at any time. DeepSeek-V3's full model in FP16 requires approximately `{python} moe_mem_str` GB, necessitating distribution across many GPUs.

```{python}
#| label: moe-capacity-planning
#| echo: false
from mlsys.constants import H100_MEM_BW, GB, TB, second
from mlsys.formatting import check
...
# ┌── EXPORTS ─────────────────────────────────────────────────
moe_read_per_gpu_str = MoECapacityPlan.read_per_gpu_str
moe_t_read_ms_str = MoECapacityPlan.t_read_ms_str
moe_total_decode_ms_str = MoECapacityPlan.total_decode_ms_str
moe_tokens_per_sec_str = MoECapacityPlan.tokens_per_sec_str
```
...
2.  **Latency**: Each decode step reads 37B active parameters (37 GB). Distributed across 16 GPUs, each reads $\sim$`{python} moe_read_per_gpu_str` GB. At `{python} h100_mem_bw_tbs` TB/s HBM bandwidth, $t_{read} \approx$ `{python} moe_t_read_ms_str` ms. AllToAll routing adds $\sim$0.3 ms.
3.  **Floor**: Estimated decode latency $\approx$ `{python} moe_total_decode_ms_str` ms/token, or `{python} moe_tokens_per_sec_str` tokens/second at batch size 1.
...
1.  **Auxiliary loss**: An additional term in the training loss penalizes uneven expert utilization by rewarding uniform routing probabilities.
2.  **Capacity factor**: Each expert has a maximum number of tokens it will accept per batch (typically 1.25$\times$ the fair share). Tokens exceeding this are dropped or rerouted.
3.  **Expert buffering**: Tokens are buffered and processed in subsequent iterations if experts are at capacity, smoothing out load spikes at the cost of latency variance.
...
Mixture-of-Experts (MoE)[^fn-moe-serving] models present unique sharding challenges because computation is dynamically routed to different experts based on input. Popular models like Mixtral [@jiang2024mixtral] use MoE to achieve high capacity with lower inference cost.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  \tikzset{
    ...
    idlebox/.style={draw=black!70, fill=black!50!20, minimum height=0.8cm, minimum width=2cm, align=center},
...
Recommendation systems typically contain embedding tables that dwarf model weights in size. Meta's DLRM-scale models [@naumov2019dlrm] have embedding tables exceeding 100 TB, requiring sharding strategies distinct from tensor or pipeline parallelism.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85, transform shape]
  ...
  \node[draw, fill=RedL, font=\scriptsize] at (4, -0.5) {Replicated / Col-wise};
  \node[draw, fill=BlueL, font=\scriptsize] at (4, -1.8) {Row-wise Sharded};

\end{tikzpicture}
```
:::

These sharding strategies operate at massive scale in production. Meta's recommendation infrastructure provides a concrete example of how row-wise, column-wise, and hybrid approaches combine to serve trillion-entity embedding tables.

::: {.callout-note title="Embedding Sharding at Meta"}
...
This achieves 4$\times$ latency reduction (from TP) while handling models requiring 8-way sharding for memory.
...
$$T_{\text{allreduce}} = 2 \times \frac{(P-1)}{P} \times \frac{M}{B}$$ {#eq-allreduce-time}
...
$$T_{\text{alltoall}} = (P-1) \times \left(L + \frac{M/P}{B}\right)$$ {#eq-alltoall-time}

These equations make clear that the achievable bandwidth $B$ and latency $L$ of the underlying interconnect determine real-world sharding performance. The following *interconnect technology comparison* quantifies these differences across production hardware.
...
Activation size: 8 MB per all-reduce (batch=1, hidden=8192)

| **Interconnect**  | **AllReduce Time** | **% of 30 ms Layer** |
|:------------------|-------------------:|---------------------:|
| **NVLink**        |            0.02 ms |                0.07% |
| **InfiniBand**    |             0.7 ms |                 2.3% |
| **100G Ethernet** |             1.5 ms |                   5% |
...
[^fn-nvlink-bandwidth-inf]: **NVLink Bandwidth Evolution**: From 160 GB/s bidirectional (Pascal, 2016) to `{python} nvlink_h100_gbs` GB/s (Hopper, 2022) to 1.8 TB/s (Blackwell, 2024), a 10$\times$ improvement over three generations. This bandwidth growth is what made intra-node tensor parallelism across 8 GPUs practical with less than 5% communication overhead, directly determining the maximum model size servable without crossing the slower cross-node interconnect. \index{NVLink!bandwidth evolution}
...
A remarkable result in load balancing theory [@mitzenmacher2001power] shows that querying just two random servers before making a routing decision provides exponentially better load distribution than random assignment.[^fn-balls-bins-routing]
...
# │
# │ Imports: (none — pure Python arithmetic, no pint quantities)
# │ Exports: total_cap_str, h100_weight_str, a100_weight_str, h100_load_str,
# │   a100_load_str, h100_util_str, a100_util_str, equal_load_str,
# │   h100_util_equal_str, a100_util_equal_str
# └─────────────────────────────────────────────────────────────────────────────
...
# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
total_cap_str = HeterogeneousGpuCluster.total_cap_str
h100_weight_str = HeterogeneousGpuCluster.h100_weight_str
a100_weight_str = HeterogeneousGpuCluster.a100_weight_str
h100_load_str = HeterogeneousGpuCluster.h100_load_str
a100_load_str = HeterogeneousGpuCluster.a100_load_str
h100_util_str = HeterogeneousGpuCluster.h100_util_str
a100_util_str = HeterogeneousGpuCluster.a100_util_str
equal_load_str = HeterogeneousGpuCluster.equal_load_str
h100_util_equal_str = HeterogeneousGpuCluster.h100_util_equal_str
a100_util_equal_str = HeterogeneousGpuCluster.a100_util_equal_str
```
...
- Total capacity: $10\times1000$ + $20\times600$ = `{python} total_cap_str` QPS

**Target traffic**: 15,000 QPS

**Weighted assignment**:

- H100 weight: 1000 / 22000 = `{python} h100_weight_str`%
- A100 weight: 600 / 22000 = `{python} a100_weight_str`%

**Expected load per server**:

- H100: $15000\times0.045$ = `{python} h100_load_str` QPS (`{python} h100_util_str`% utilization)
- A100: $15000\times0.027$ = `{python} a100_load_str` QPS (`{python} a100_util_str`% utilization)
...
- Per-server load: 15000 / 30 = `{python} equal_load_str` QPS
- H100 utilization: `{python} h100_util_equal_str`% (underutilized)
- A100 utilization: `{python} a100_util_equal_str`% (overloaded, latency spikes)
...
**Consistent hashing**[^fn-consistent-hashing-kv] [@karger1997consistent] maps requests to servers based on a hash of the routing key (user ID, session ID):
...
Under tensor parallelism, each request is broadcast to all devices in the shard group. Each device processes its portion of each layer, and results are synchronized via all-reduce.
...
Under pipeline parallelism, each request flows through stages sequentially, with each stage forwarding to the next.
...
Under expert parallelism, each request is dispatched to devices hosting the selected experts based on the gating decision.
...
When multiple shard groups provide horizontal scaling, the load balancer routes to groups rather than individual devices:
...
::: {.callout-note title="Health Checks for GPU Inference"}
...
**Circuit breaker pattern**[^fn-circuit-breaker-serving] [@nygard2007releaseit]:
...
Imagine your company's critical customer-support chatbot running on the same GPU server as an intern's experimental computer vision script. If the intern's script accidentally monopolizes the PCIe bus or memory bandwidth, the customer-facing chatbot will begin timing out, violating SLAs. Multi-tenancy and isolation ensure that we can safely co-locate disparate workloads to save money without sacrificing the predictability of our flagship services.
...
GPU memory contention is the most severe: a tenant with unexpectedly long sequences can consume a disproportionate share of the KV cache pool. Consider three tenants sharing a 60 GB KV cache pool with equal 20 GB allocations. When one tenant begins issuing long-context requests, its allocation can swell to 45 GB, forcing evictions that reduce the other tenants from 200 concurrent sequences each to 75 -- a 62% batch size reduction that directly degrades their throughput. Network bandwidth saturation compounds this effect when a tenant streaming many large responses consumes the available egress capacity. GPU time-sharing between tenants introduces context-switching overhead and unpredictable latency variance. *Quantifying noisy neighbor impact* requires measuring these interference effects across all three dimensions simultaneously.
...
Hard quotas enforce strict limits, as shown in @lst-resource-quotas.
...
Soft quotas with fair sharing provide a more flexible alternative. Each tenant has a nominal quota (for example, 100 QPS), but when the cluster is underutilized (50% capacity), tenants can burst to 2$\times$ their quota. When the cluster saturates (90% capacity), quotas are enforced. This approach maximizes utilization during low-traffic periods while protecting tenants during contention.
...
The bulkhead pattern[^fn-bulkhead-isolation] [@nygard2007releaseit] physically isolates tenant workloads, preventing failures from propagating across tenants. Named after ship compartments that contain flooding to isolated sections.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  \node[anchor=west, below=2cm of gold_title.west] (std_title) {\textbf{Standard Tier} (Shared Pool)};
  ...
  % Impact Annotation
  \node[align=left, font=\scriptsize, color=FailColor!80!black, right=0.5cm of r4] {Failure constrained\\to Shared Pool};
  \node[align=left, font=\scriptsize, color=GoldColor!50!black, right=0.5cm of r1, yshift=-0.5cm] {Gold Tier\\unaffected};
...
A typical LLM API illustrates the three-tier bulkhead pattern. The enterprise tier runs on a dedicated GPU pool with hardware isolation and a 99.9% availability SLO -- no other tenant's traffic can affect it. The professional tier shares a GPU pool but receives 70% of that pool's capacity through resource quotas and can preempt the free tier when contention arises. The free tier receives the remaining 30% on a best-effort basis with rate limiting (for example, 10 QPS) and no SLO guarantee. This three-tier structure concentrates isolation cost where revenue justifies it while still achieving high utilization across the shared pool.
...
[^fn-mig-partitioning]: **MIG (Multi-Instance GPU)**: Introduced with Ampere (A100, 2020), MIG spatially partitions a single GPU into up to 7 isolated instances, each with dedicated compute SMs, memory bandwidth, and L2 cache. Unlike time-slicing, which shares resources and introduces latency variance of 2--5$\times$, MIG provides hardware-enforced isolation with predictable performance, making it the preferred mechanism for serving multiple small models on a single expensive datacenter GPU. \index{MIG!partitioning}

Model loading presents a subtler isolation challenge: loading a new model should not evict a running model from GPU memory. @lst-model-loading-isolation demonstrates priority-aware eviction logic that enforces this constraint.
...
A viral social media post drives a 10x spike in traffic to your generative AI app in under five minutes. If your infrastructure team relies on manual scaling or slow, CPU-based metrics to spin up new GPU instances, the service will collapse before the new replicas even finish downloading the model weights. Autoscaling for ML inference requires predictive, custom-metric strategies to handle these massive load dynamics.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, xscale=0.025, yscale=0.8]
  ...
  \tikzset{
    bar/.style={draw=black!70, minimum height=0.8cm, font=\tiny, inner sep=0pt, anchor=west}
  }
  ...
  % Connect total
  \draw[decorate, decoration={brace, amplitude=5pt, raise=5pt}] (prov.south west |- 0, 2) -- (warm.south east |- 0, 2) node[midway, above=10pt, font=\bfseries] {Total Cold Start: > 5 Minutes};
...
Event-driven scaling scales proactively for known events, as shown in @lst-event-driven-scaling.

::: {#lst-event-driven-scaling lst-cap="**Event-Driven Scaling**: Scheduled scaling rules that pre-provision capacity for product launches and recurring traffic spikes."}
```{.yaml}
scheduled_scaling:
...
::: {.callout-note title="Predictive Scaling for Traffic"}
...
The cost savings above depend on anticipating traffic before it arrives. Proactive provisioning avoids the SLO violations that reactive-only systems suffer during ramp-up periods.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.8]
  ...
  \node[RedLine, font=\tiny, rotate=90] at (5.1, 2.7) {SLO VIOLATION};
...
The cost of maintaining warm replicas is $\text{Pool size} \times \text{GPU cost/hour} \times \text{Idle fraction}$, creating a direct tradeoff between response speed and idle-capacity expense. Tiered warm pools resolve this tradeoff by maintaining replicas at different readiness levels, each with different activation latency and cost:
...
[^fn-spot-inference]: **Spot Instance Economics**: Cloud providers sell unused GPU capacity at 60--90% discounts with 30-second to 2-minute termination notice. For inference, termination destroys in-flight requests and KV cache state, requiring graceful drain logic and rapid request re-routing. The 60--90% savings justify this complexity for burst and best-effort traffic tiers but are unsuitable for SLO-critical paths where termination would violate latency guarantees. \index{Spot Instances!inference}
...
@lst-spot-termination illustrates graceful handling of spot termination within the two-minute warning window.
...
@lst-active-failover shows the active-active failover routing logic.
...
Deploying model updates across regions requires careful coordination. A phased rollout begins with a canary deployment to a single region (typically 1% of traffic), monitors for 1--4 hours, then progressively expands to additional regions. Each expansion point is a gate: if error rates exceed 0.1% or P99 latency exceeds the target, the rollout halts. Rollback switches the affected region back to the previous model version while preserving the new version for debugging. The key invariant is that no region proceeds to a new version until the prior region has demonstrated healthy metrics for a sufficient monitoring window.
...
Cost-aware routing directs latency-tolerant workloads (batch inference, background processing) to the cheapest available region, as shown in @lst-cost-aware-routing.
...
[^fn-hessian-quantization]: **Hessian Matrix in Quantization**: The Hessian $H = X^T X$ captures second-order sensitivity: weights with large diagonal entries have outsized impact on model outputs. GPTQ exploits this to quantize insensitive weights aggressively while preserving sensitive ones, achieving 3--4-bit quantization with less than 1% perplexity degradation. Without Hessian guidance, naive uniform quantization below 8-bit typically destroys model quality. \index{Hessian!quantization}
...
QuaRot achieves 4-bit weights AND 4-bit activations with quality competitive to GPTQ's 4-bit weights only. This enables approximately 8$\times$ memory reduction versus FP16.
...
@lst-vllm-quantization shows how vLLM loads a pre-quantized AWQ model.
...
@lst-tensorrt-quantization demonstrates the equivalent quantization workflow in TensorRT-LLM.
...
A 70B model with 4-bit weights requires approximately 35 GB, leaving 45 GB on an `{python} a100_mem_gb`GB A100 for KV cache. With FP16 KV cache at 2.5 MB per 1K tokens per sequence, this supports ~18 concurrent 1K-token sequences versus ~8 with FP16 weights.
...
This reduces embedding lookup latency from 50 ms (naive) to 2 ms through batching and locality optimization.

Instead of batching entire requests, Meta batches at the feature level. Each inference request triggers 5,000+ embedding lookups, but these lookups are batched across requests within a 1 ms window. This achieves 90%+ memory bandwidth utilization on embedding servers.
...
To meet this latency budget while evaluating thousands of documents, the system employs a *ranking cascade* that progressively narrows the candidate set through increasingly expensive models.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  % Down arrows
  \draw[->, ultra thick, black!50!60] (L0) -- (L1);
  \draw[->, ultra thick, black!50!60] (L1) -- (L2);
  \draw[->, ultra thick, black!50!60] (L2) -- (L3);

  \node[anchor=west, font=\scriptsize, text=red!80, right=0.2cm of L0] {Low Precision, High Recall};
  \node[anchor=west, font=\scriptsize, text=green!60!black, right=0.2cm of L3] {High Precision, Low Recall};
...
This achieves 100$\times$ cost reduction compared to running L3 on all candidates.
...
TikTok separates user understanding (online) from content understanding (offline) through a two-tower architecture with caching[^fn-two-tower-serving]:

[^fn-two-tower-serving]: **Two-Tower Architecture**: Encodes user features and item features through separate neural networks ("towers") into embeddings, then scores via dot product. The serving advantage: item embeddings can be pre-computed offline and cached, reducing online inference to the user tower forward pass plus a sub-millisecond vector similarity lookup. The trade-off is reduced model expressiveness from late interaction, since the towers cannot attend to each other's features during encoding. \index{Two-Tower Architecture!serving}
...
New video content is processed with different priorities:
...
These case studies reveal how deeply custom engineering is required to serve ML at scale. However, engineers attempting to replicate these architectures often fall victim to conventional wisdom that does not translate to GPU-bound workloads, leading us to the field's most common fallacies and pitfalls.
...
Queuing theory establishes that systems provisioned for average load violate SLOs during traffic peaks. At 80% average utilization, a modest 25% traffic spike pushes utilization above 100%, causing unbounded queue growth. As discussed in @sec-inference-scale-autoscaling-168d, the cold start problem exacerbates this: by the time new capacity is available (5+ minutes for GPU instances), SLO violations have already occurred. Capacity planning must account for peak load plus headroom, not average load.
...
Distributed inference introduces overhead absent from single-machine serving: network round-trips, serialization, load balancer decisions, and coordination for sharded models. This "serving tax" often consumes 10--30% of the latency budget. A team that achieves 70 ms model inference on a single GPU may be surprised when end-to-end latency reaches 100 ms in production due to these overheads. Latency budgets must explicitly account for distribution overhead, not just compute time.
...
# │ How: pulling constants from mlsys.constants.
# │
# │ Imports: mlsys.constants (H100_MEM_CAPACITY, H100_MEM_BW, GB, TB, second)
# │ Exports: h100_mem_gb, h100_mem_bw_tbs
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import H100_MEM_CAPACITY, H100_MEM_BW, GB, TB, second
