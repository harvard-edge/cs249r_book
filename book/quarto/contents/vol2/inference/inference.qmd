...
\noindent
![](images/png/cover_inference_at_scale.png){fig-alt="Distributed inference and model serving at scale." width=100 percent}
...
- Design batching strategies matched to model architectures, distinguishing **static batching**\index{Static Batching} for vision models from **continuous batching** for LLMs and **feature-parallel batching** for recommendation systems
...
- Compare model sharding strategies (**tensor parallelism**\index{Tensor Parallelism}, **pipeline parallelism**\index{Pipeline Parallelism}, **expert parallelism**\index{Expert Parallelism}, **embedding sharding**) by their communication patterns and latency-memory tradeoffs
- Analyze load balancing algorithms using **queuing theory**\index{Queuing Theory} to explain why **power-of-two-choices** achieves exponentially better performance than random assignment
...
- Design **autoscaling**\index{Autoscaling} policies that account for **GPU cold start** latency and balance cost optimization with SLO guarantees
...
|:---------------------------------|:-------------------------|---------------------:|:----------------------------|
| **Memory**\index{Memory}         | 80 GB (H100)             |      GPT-4 (400 GB+) | Tensor/pipeline parallelism |
...
**Problem**\index{Problem}: You spent **\$2 Million** training a 70B parameter model. You serve it to 1 million daily active users (DAU), each making 50 requests/day. Is training or serving the dominant cost over 1 year?

**The Math**:

1.  **Training Cost**\index{Training Cost}: \$2,000,000 (One-time).
2.  **Inference Volume**\index{Inference!volume}: $1M \text{ users} \times 50 \text{ reqs} \times 365 \text{ days}$ $\approx$ `{python} ServingCostSimple.annual_reqs_b_str` Billion requests/year.
3.  **Cost per Request**\index{Cost Per Request}: Assume a 70B model on H100 costs $\approx \$0.001$ per request (input + output).
4.  **Annual Serving Cost**\index{Annual Serving Cost}: $18 \times 10^9 \times \$0.001 = \mathbf{\$18{,}000{,}000}$.

**The Systems Conclusion**: Serving costs **9$\times$** more than training in just the first year. A 10 percent optimization in inference latency saves **\$1.8M**\index{\$1.8M}—paying for the entire training run almost immediately.
...
**Scenario**\index{Scenario}: Recommend-system model (DLRM).

**Training Cost ($C_{train}$)**

- Hardware: 1,000 GPU-hours @ \$3/hr = \$3,000
- Engineering Overhead (Data/Exp): 3$\times$ Hardware = \$9,000
- **Total Training**\index{Total Training}: `{python} InferenceEconomics.training_cost_total_str`
...
::: {#fig-batching-strategies fig-env="figure" fig-pos="htb" fig-cap="**Batching Strategies Compared.** Three timelines show how batching policies trade wait time for GPU utilization. (a) **No batching**\index{No Batching}: requests processed sequentially, GPU idle between them (around 40 percent utilization). (b) **Static batching**: requests wait until a full batch is assembled before dispatch, with padding for short requests (around 70 percent utilization). (c) **Dynamic batching**\index{Dynamic Batching}: assembles a batch within a short time window with adaptive size, reducing wait and lifting utilization (around 85 percent)." fig-alt="Three stacked timelines labeled (a) No Batching, (b) Static Batching, (c) Dynamic Batching. Each shows four requests R1-R4 and the corresponding GPU utilization bar (approximately 40, 70, and 85 percent respectively)."}
![](images/svg/batching-strategies.svg)
:::

For **vision models**\index{Vision Models}—convolutional neural networks (CNNs) and vision transformers (ViTs) processing fixed-size images—computation scales linearly with batch size while memory scales sub-linearly due to weight sharing. Larger batches improve GPU utilization with minimal overhead, making static or dynamic batching with large batch sizes optimal.
...
1.  **Small $B$**\index{Small $B$}: Throughput is dominated by $T_{\text{fixed}}$. The system is **latency-bound** (or overhead-bound). Increasing $B$ yields super-linear throughput gains.
2.  **Large $B$**\index{Large $B$}: As $B \to \infty$, the $T_{\text{fixed}}$ term becomes negligible. Throughput asymptotically approaches the hardware limit $1/T_{\text{variable}}$. The system becomes **compute-bound**\index{Compute-Bound} (or bandwidth-bound for LLMs).
...
**Concept**\index{Concept}: In any stable queuing system, the average number of requests in the system ($L$) equals the arrival rate ($\lambda$) multiplied by the average time a request spends in the system ($W$).
...
: **Batch Size Impact on GPT-3 Serving Performance**. Batch sizes below 8 cannot sustain 100 QPS (utilization exceeds 100 percent). The optimal operating point balances throughput against latency, with B=32 achieving lowest total latency despite higher service time. {#tbl-batch-size-comparison}
...
1. **Batch sizes 1-4 are unstable**\index{Batch!sizes 1-4 are unstable}: Utilization exceeds 100 percent, meaning the system cannot keep up with arrivals. Queues grow unboundedly.
...
**Verification using Little's Law**\index{Verification Using Little's Law}: $L = \lambda \cdot W$
...
With batch size `{python} LittlesLawVerify.batch_size` and utilization `{python} GPT3ServingResults.b32_util_pct` percent:
...
- **Arrival rate**\index{Arrival Rate}: 5,000 QPS
...
As models transition from "Fast Thinking" (instant pattern matching) to "Slow Thinking" (deliberative reasoning), the bottleneck shifts from **HBM Bandwidth** to **Test-Time Compute**. This is the **Logic Wall**\index{Logic Wall}: the realization that for complex problems, the fleet must scale compute *per request* proportional to the difficulty of the task, often through search or "Chain-of-Thought" (CoT) unrolling.
...
2. **Batch sizes are small**\index{Batch!sizes are small}: Few opportunities for slot reuse within a batch
...
2. **Differentiated batching**\index{Differentiated Batching}: Limit short-request batches to 8, medium to 16, long to 32
...
![](images/svg/feature-parallel-pipeline.svg){width=100 percent}
:::
...
2. **Dense feature processing**\index{Dense Feature Processing}: Transform and normalize dense features
...
**Batch accumulation window**\index{Batch!accumulation window}: 1 ms
**Requests per batch**\index{Requests Per Batch}: 10,000 (at 10M QPS)
**Lookups per shard per batch**\index{Lookups Per Shard Per Batch}: 50M
...
**Latency breakdown**\index{Latency!breakdown}:
...
**Pipeline stages**\index{Pipeline!stages}:
...
1.  **Significance (Quantitative):** It eliminates **Internal and External Fragmentation**\index{Internal and External Fragmentation}, which can waste up to 80 percent of GPU memory. By allowing sequences to grow dynamically, it enables 2$\times$ to 4$\times$ higher **Concurrent Throughput ($\eta$)** on the same hardware.
...
::: {#fig-paged-attention fig-env="figure" fig-pos="htb" fig-cap="**KV Cache Fragmentation and the PagedAttention Solution**. Top left: internal fragmentation wastes memory by pre-allocating for the maximum sequence length. Top right: external fragmentation leaves unusable memory gaps. Bottom: PagedAttention solves this by mapping contiguous logical pages of a sequence to non-contiguous physical memory blocks via a block table, allowing the system to fill fragmentation gaps with small blocks from any sequence." fig-alt="A three-part diagram on memory fragmentation. Top panels show internal and external fragmentation. The bottom panel shows the PagedAttention solution using a block table to map logical pages to scattered physical blocks, eliminating waste."}
![](images/svg/kv-cache-fragmentation.svg){width=100 percent}
:::
...
**Attention kernel modification**\index{Attention!kernel modification}:
...
# │   (with), memory_reduction_pct_str="75"%, concurrent_multiplier_str="4"—# │   inline in the "Prefix Caching at Scale" callout.
...
3.  **Acceptance phase**\index{Acceptance Phase}: Starting from the first draft token, the system compares the draft model's probability distribution with the target model's distribution. Continue accepting tokens until a draft token is rejected or all $K$ are accepted. On rejection, sample a corrected token from a modified distribution that accounts for the draft model's error. @fig-speculative-decoding-inf illustrates this three-phase process.
...
For Llama-70B with 4K context, KV cache is ~80 MB per sequence. At 100 Gbps, transfer takes 6.4 ms vs. ~500 ms for prefill. Transfer is clearly better.
...
- **Prefill pool**\index{Prefill Pool}: Optimized for compute intensity (for example, H100 nodes with high TFLOPS) using large batch sizes to maximize throughput.
- **Decode pool**\index{Decode Pool}: Optimized for memory bandwidth (for example, nodes with maximum HBM capacity or specialized High Bandwidth Flash as discussed in @sec-data-storage) to handle thousands of concurrent autoregressive streams.
...
**Chunk sizing**\index{Chunk Sizing}: Chunks are sized to complete in approximately the same time as one decode iteration (typically 10–50 ms). For a prefill throughput of 10,000 tokens/second, a 20 ms chunk processes 200 tokens.
...
**For `{python} TpSpeedupCalc.n_layers` layers**\index{For `{python} TpSpeedupCalc.n_layers` Layers}:
...
![](images/svg/pipeline-parallel-routing.svg){width=100 percent}
:::
...
**Communication pattern per token**\index{Communication Pattern Per Token}:
...
**Load balancing metrics**\index{Load Balancing!metrics}:
...
| **Metric**\index{Metric} | **Without Optimization** | **With Optimization** |
|:-------------------------|-------------------------:|----------------------:|
...
| **Interconnect**   | **Bandwidth**\index{Bandwidth} | **Latency** | **Use Case**           |
|:-------------------|-------------------------------:|------------:|:-----------------------|
...
- H100: $15000\times0.045$ = `{python} HeterogeneousGpuCluster.h100_load_str` QPS (`{python} HeterogeneousGpuCluster.h100_util_str` percent utilization)
- A100: $15000\times0.027$ = `{python} HeterogeneousGpuCluster.a100_load_str` QPS (`{python} HeterogeneousGpuCluster.a100_util_str` percent utilization)
...
- **Centralized counter**\index{Centralized Counter}: Single source of truth, potential bottleneck
...
- **Streaming inference**\index{Streaming Inference}: Model state from previous frames
...
**Timeout configuration**\index{Timeout Configuration}:
...
**Circuit breaker pattern**\index{Circuit Breaker Pattern}[^fn-circuit-breaker-serving] [@nygard2007releaseit]:
...
**Without circuit breaker**\index{Without Circuit Breaker}:
...
**With circuit breaker**\index{With Circuit Breaker}:
...
**Configuration for GPU inference**\index{Configuration for GPU Inference}:
...
- **Cost efficiency**\index{Cost Efficiency}: Sharing infrastructure across tenants improves utilization
...
- **Noisy neighbors**\index{Noisy Neighbors}: One tenant's burst traffic impacts others
...
::: {#lst-resource-quotas lst-cap="**Resource Quotas**\index{Resource Quotas}: Admission control that enforces per-tenant limits on concurrency, KV cache memory, and request rate."}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  \node[fill=white, inner sep=2pt, font=\scriptsize, align=center] at (8.0, 1.8) {\textbf{Complete Isolation}\\No Shared Resources};
...
::: {#lst-model-loading-isolation lst-cap="**Model Loading Isolation**\index{Model Loading!isolation}: Priority-aware memory management that prevents model eviction from violating tenant isolation constraints."}
...
::: {#lst-metric-based-scaling lst-cap="**Metric-Based Scaling**\index{Metric!based scaling}: Autoscaling policy driven by utilization thresholds with a cooldown period to prevent oscillation."}
...
**Current state**\index{Current State}: 10 replicas, 100 QPS each, 70 percent utilization

**Target state**\index{Target State}: 30 replicas for 3000 QPS

**Without pre-warming**\index{Without Pre-Warming}:
...
::: {#lst-predictive-scaling lst-cap="**Predictive Scaling**\index{Predictive Scaling}: Time-series demand forecasting combining daily seasonality, weekly seasonality, and trend estimation."}
...
::: {#lst-event-driven-scaling lst-cap="**Event-Driven Scaling**\index{Event-Driven Scaling}: Scheduled scaling rules that pre-provision capacity for product launches and recurring traffic spikes."}
...
**Cost comparison**\index{Cost Comparison}:
...
::: {#lst-spot-termination lst-cap="**Spot Termination Handling**\index{Spot Termination Handling}: Graceful shutdown sequence that drains in-flight requests, persists KV cache state, and deregisters from the load balancer."}
...
::: {#lst-active-failover lst-cap="**Active-Active Failover**\index{Active-Active Failover}: Global routing that falls back to the second-nearest healthy region on timeout or unavailability."}
...
- **Capacity spike**\index{Capacity Spike}: The receiving region sees sudden traffic increase. Pre-provision headroom (typically 30--50 percent over steady-state) or accept degraded latency during failover.
...
::: {#lst-cost-aware-routing lst-cap="**Cost-Aware Routing**\index{Cost-Aware Routing}: Priority-based routing that sends low-priority batch requests to the cheapest region while keeping interactive traffic local."}
...
The compensation step propagates quantization error to unquantized columns, where the Hessian-weighted update minimizes output deviation. This achieves $O(d_{row} \cdot d_{col}^2)$ complexity vs. $O(d_{row} \cdot d_{col}^3)$ for naive OBS.
...
|:---------------------|:------------------|:------------------|---------------------:|
| **FP16**             | Yes               | Yes               |                   1x |
| **BF16**\index{BF16} | Yes               | Yes               |                   1x |
| **INT8**\index{INT8} | Yes               | Yes               |                   2x |
| **FP8 (E4M3)**       | No                | Yes               |                   2x |
| **INT4**\index{INT4} | Via CUTLASS       | Native            |                   4x |
...
::: {#lst-vllm-quantization lst-cap="**vLLM Quantization**\index{Vllm Quantization}: Loading an AWQ-quantized model with automatic kernel selection and KV cache management."}
...
::: {#lst-tensorrt-quantization lst-cap="**TensorRT-LLM Quantization**\index{TensorRT!llm quantization}: Offline AWQ quantization with calibration data for optimized INT4 inference."}
...
A 70B model with 4-bit weights requires approximately 35 GB, leaving 45 GB on an `{python} A100HardwareScenario.a100_mem_gb`GB A100 for KV cache. With FP16 KV cache at 2.5 MB per 1K tokens per sequence, this supports ~18 concurrent 1K-token sequences vs. ~8 with FP16 weights.
...
: **Case Study Summary**\index{Case Study Summary}: Each system innovates on a core technique matched to its workload characteristics. {#tbl-case-studies-summary}
...
* **Batching is Not Universal**\index{Batching!is not universal}: There is no "optimal" batch size for all models. Vision models scale with batch size; recommendation models scale with feature-parallel shards; LLMs scale via continuous, iteration-level management.
