...
Disaggregated serving separates the prefill phase (processing the input prompt, which is compute-bound) from the decode phase (generating tokens one at a time, which is memory-bandwidth-bound) onto different hardware pools optimized for each workload profile. Prefill nodes use high-FLOP/s accelerators; decode nodes use high-bandwidth-memory configurations. This separation allows each phase to operate at its hardware's optimal operating point. The disaggregated serving section (@sec-inference-scale-disaggregated-serving-splitting-workload-b8f9) develops this architecture in detail.
...
**Scenario**: Meta-scale recommendation infrastructure serves `{python} RecsysLatencyBreakdown.platform_qps_str`, querying `{python} RecsysLatencyBreakdown.lookups_per_request_str` sparse embeddings per request across `{python} RecsysLatencyBreakdown.embedding_shards_str` shards.

**Diagnosis**: Single-threaded per-request embedding lookups fail at `{python} RecsysLatencyBreakdown.lookups_per_shard_per_second_million_str` lookups/sec per shard. Accumulating requests over a `{python} RecsysLatencyBreakdown.accumulation_window_ms_str` window forms batches of `{python} RecsysLatencyBreakdown.lookups_per_shard_per_batch_k_str` per shard, converting random memory reads into sequential accesses.
...
**Diagnosis**: Pure HBM allocation limits node concurrency to 50 active sequences. Evicting or dropping requests degrades user experience. Implementing a 3-tier memory hierarchy (HBM $\to$ CPU DRAM $\to$ NVMe SSD) allows swapping inactive sequence pages to CPU DRAM (1–5 ms swap latency) or NVMe (10–50 ms).
...
**Systems lesson**: 8-way Tensor Parallelism over NVLink achieves a 6.9$\times$ end-to-end speedup (prefill time reduced from `{python} TpSpeedupCalc.seq_total_ms_str` to `{python} TpSpeedupCalc.tp8_total_ms_str`). Sharding attention and FFN layers across intra-node NVLink interconnects buys latency by trading low-overhead NVLink transfers for compute acceleration.
...
**Systems lesson**: Predictive autoscaling converts diurnal time-series forecasts into scheduled pre-warming windows. Triggering scale-up actions 30 minutes before expected ramps ensures replicas are fully warmed before traffic arrives.
