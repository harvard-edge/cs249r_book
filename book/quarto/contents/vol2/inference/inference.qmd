...
2. **Batch size 8 achieves stability**: At `{python} GPT3ServingResults.b8_util_pct` percent utilization, the system is stable but queuing delays contribute significantly to latency (`{python} GPT3ServingResults.b8_wait_ms` ms), as @fig-queuing-hockey-stick illustrates.
...
3.  **Common Pitfall:** A frequent misconception is that the KV Cache is "fixed-size." In reality, it is **Dynamic and Fragmented**: because different requests have different lengths, the cache can cause massive memory waste (up to 60--80 percent) due to internal fragmentation if not managed by a virtual memory system.
...
1.  **Significance (Quantitative):** It eliminates **Internal and External Fragmentation**, which can waste up to 80 percent of GPU memory. By allowing sequences to grow dynamically, it enables 2$\times$ to 4$\times$ higher **Concurrent Throughput ($\eta$)** on the same hardware.
...
**Savings**: `{python} PrefixCachingAnalysis.memory_reduction_pct_str` percent reduction in KV cache memory, enabling `{python} PrefixCachingAnalysis.concurrent_multiplier_str`$\times$ more concurrent users.
...
![](images/svg/serving-pipeline.svg){width=100 percent}
:::
