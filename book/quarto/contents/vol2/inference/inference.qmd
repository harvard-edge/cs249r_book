...
2.  **Inference Volume**: $1M \text{ users} \times 50 \text{ reqs} \times 365 \text{ days} \approx 18 \text{ Billion requests/year}$.
...
2.  **Latency**: Each decode step reads 37B active parameters (37 GB). Distributed across 16 GPUs, each reads $\sim$2.3 GB. At `{python} h100_mem_bw_tbs` TB/s HBM bandwidth, $t_{read} \approx 0.69 \text{ ms}$. AllToAll routing adds $\sim$0.3 ms.
3.  **Floor**: Estimated decode latency $\approx 1.0 \text{ ms/token}$, or 1,000 tokens/second at batch size 1.
