...
# H100 specs
h100_mem_gb = f"{H100_MEM_CAPACITY.to(GiB).magnitude:.0f}"       # "80"
h100_mem_bw_tbs = f"{H100_MEM_BW.to(TB / second).magnitude}"     # "3.35"
h100_tdp_w = f"{H100_TDP.to(watt).magnitude:.0f}"                # "700"

# A100 specs
a100_mem_gb = f"{A100_MEM_CAPACITY.to(GiB).magnitude:.0f}"       # "80"
a100_mem_bw_tbs = f"{A100_MEM_BW.to(TB / second).magnitude:.0f}" # "2"
a100_mem_bw_gbs = f"{A100_MEM_BW.to(GB / second).magnitude:.0f}" # "2039"

# NVLink bandwidth
nvlink_h100_gbs = f"{NVLINK_H100_BW.to(GB / second).magnitude:.0f}"  # "900"
nvlink_a100_gbs = f"{NVLINK_A100_BW.to(GB / second).magnitude:.0f}"  # "600"

# PCIe bandwidth
pcie_gen4_gbs = f"{PCIE_GEN4_BW.to(GB / second).magnitude:.0f}"  # "32"
pcie_gen5_gbs = f"{PCIE_GEN5_BW.to(GB / second).magnitude:.0f}"  # "64"
...
output_lengths = [50, 200, 100, 150]  # R1, R2, R3, R4
...
# Prefix caching KV cache analysis
...
# Tensor parallelism speedup
seq_per_layer_ms = 30.0
tp8_per_layer_ms = 4.35
tp_speedup = seq_per_layer_ms / tp8_per_layer_ms
tp_theoretical = 8
tp_speedup_str = f"{tp_speedup:.1f}"

# Per-token timing for 80 layers
n_layers = 80
...
# Heterogeneous GPU cluster weighted routing example
n_h100 = 10
n_a100 = 20
h100_cap_qps = 1000
a100_cap_qps = 600
