...
The four levels form a *serving hierarchy* (@fig-serving-hierarchy), each building on the one below.

::: {#fig-serving-hierarchy fig-env="figure" fig-pos="htb" fig-cap="**The Serving Hierarchy.** Optimization occurs at four distinct levels. The Request Level focuses on minimizing per-request latency. The Replica Level maximizes single-instance throughput. The Service Level manages distribution across multiple replicas. The Platform Level handles efficient resource sharing across multiple services and tenants." fig-alt="Four-level hierarchy diagram: request level, replica level, service level, and platform level with optimization targets."}
...
::: {#fig-batching-strategies fig-env="figure" fig-pos="htb" fig-cap="**Batching Strategies Compared.** **Static batching** (A) waits for all slots to fill, creating latency bubbles. **Continuous batching** (B) allows requests to join and leave at each iteration, maximizing GPU utilization for LLMs. **Feature-parallel batching** (C) shards requests by feature type (e.g., User IDs vs. Item IDs), optimizing for distributed embedding lookup." fig-alt="Three-panel diagram comparing static, continuous, and feature-parallel batching strategies with GPU utilization."}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import A100_MEM_CAPACITY, A100_MEM_BW, NVLINK_A100_BW, PCIE_GEN4_BW, GB, TB, second
...
# ┌── LEGO ───────────────────────────────────────────────
from mlsysim.core.constants import H100_MEM_CAPACITY, GB, BYTES_FP16, BYTES_INT4, byte
class KVCacheCalc:
