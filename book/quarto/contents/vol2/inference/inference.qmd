...
: **Batch Size Impact on GPT-3 Serving Performance**. Batch sizes below 8 cannot sustain 100 QPS (utilization exceeds 100 percent). The optimal operating point balances throughput against latency, with B=16 slightly lower on total latency while B=32 provides more headroom and a lower queueing delay. {#tbl-batch-size-comparison}
...
class LittlesLawVerify:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(7 < L < 8, f"L should be ~7.5, got {L:.2f}")
    # ┌── 4. OUTPUT ────────────────────────────────────────
...
3. **Batch size 32 sharply reduces queueing delay**\index{Batch Size!32 reduces queueing delay}: Despite longer service time (`{python} GPT3ServingResults.b32_service_ms` ms vs. `{python} GPT3ServingResults.b8_service_ms` ms), the dramatic reduction in queue wait time (`{python} GPT3ServingResults.b32_wait_ms` ms vs. `{python} GPT3ServingResults.b8_wait_ms` ms) brings total latency down into the mid-70 ms range; batch size 16 is slightly lower on end-to-end latency in this toy model.
...
This matches our queue wait calculation: about 1 request waiting on average.
