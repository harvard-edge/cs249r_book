...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt_bandwidth, fmt_qty_int
...
class InferenceSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_mem_gb_str = fmt_qty_int(h100_mem, GiB, commas=False, unit_label="GB")
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt_bandwidth, fmt_memory, fmt_time, fmt_qty_int
...
class A100HardwareScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_gb_str = fmt_qty_int(mem, GiB, commas=False, unit_label="GB")
...
class RecsysLatencyBreakdown:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────
    total_str = fmt_time(total, millisecond, precision=1, commas=False)
```
...
| **Total**                                    | **`{python} RecsysLatencyBreakdown.total_str`** | Within 10 ms SLO            |
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt_memory, fmt_qty_int, MarkdownStr
...
class KVCacheCapacityEstimator:
    ...
    total_hbm = 640 * GB  # scenario budget
    weight = Models.Language.Llama2_70B.size_in_bytes(BYTES_FP16)
    ...
    available_kv_gb_str = fmt_memory(available_kv, unit=GB, precision=0, commas=False)
    ...
    h100_mem_gb_str = fmt_qty_int(Hardware.Cloud.H100.memory.capacity, GiB, commas=False, unit_label="GB")
    ...
    step2_cache_per_request_math = MarkdownStr(
        ...
        f"\\mathbf{{{kv_gb_per_request_display:.0f} \\text{{ GB/request}}}} $$"
    )
    step3_max_batch_math = MarkdownStr(
        f"$$ \\text{{Max Batch}} = \\left\\lfloor \\frac{{{available_kv_display:.0f}}}{{{kv_gb_per_request_display:.0f}}} \\right\\rfloor "
        f"= \\mathbf{{{max_batch}}} $$"
    )
```
...
class KVCacheCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt_int, check, MarkdownStr, fmt_memory, fmt_qty
...
class PrecisionDividend:
    ...
    fp16_weight_per_gpu_str = fmt_memory(fp16_weight_per_gpu, unit=GB, precision=0, commas=False)
    avail_kv_fp16_str = fmt_memory(avail_kv_fp16, unit=GB, precision=0, commas=False)
    int4_weight_per_gpu_str = fmt_memory(int4_weight_per_gpu, unit=GB, precision=2, commas=False)
    avail_kv_int4_str = fmt_memory(avail_kv_int4, unit=GB, precision=2, commas=False)
    kv_fp16_per_gpu_str = fmt_memory(kv_fp16_per_gpu, unit=GB, precision=1, commas=False)
    kv_int8_total_str = fmt_memory(kv_int8_total, unit=GB, precision=1, commas=False)
    kv_int8_per_gpu_str = fmt_memory(kv_int8_per_gpu, unit=GB, precision=1, commas=False)
...
- Available for KV cache: `{python} PrecisionDividend.h100_mem_gb_str` $-$ `{python} PrecisionDividend.fp16_weight_per_gpu_str` = `{python} PrecisionDividend.avail_kv_fp16_str`/GPU
...
- Available for KV cache: `{python} PrecisionDividend.h100_mem_gb_str` $-$ `{python} PrecisionDividend.int4_weight_per_gpu_str` = `{python} PrecisionDividend.avail_kv_int4_str`/GPU
- KV cache per request (INT8): `{python} PrecisionDividend.kv_int8_total_str` $\div$ `{python} PrecisionDividend.tensor_parallel_degree_str` $\approx$ `{python} PrecisionDividend.kv_int8_per_gpu_str`/GPU
...
class TpSpeedupCalc:
    ...
    hidden_dim = 8192
    weight_fp16 = Models.Language.Llama2_70B.size_in_bytes(BYTES_FP16)
...
class MoEEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dense_mem_str = fmt_memory(dense_mem_fp16, unit=GB, precision=0, commas=False)
    moe_mem_str = fmt_memory(moe_mem_fp16, unit=GB, precision=0, commas=False)
...
class MoECapacityPlan:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    read_per_gpu_str = fmt_memory(read_per_gpu, unit=GB, precision=1, commas=False)
    ...
    tokens_per_sec_str = fmt_rate(tokens_per_sec, "tokens/s", precision=0, commas=True)
    ...
    moe_active_params_str = fmt_params(moe_active_params, scale="B", precision=0, commas=False)
    ...
    h100_mem_gb_str = fmt_memory(h100_mem, unit=GB, precision=0, commas=False)
    moe_total_params_str = fmt_params(moe_total_params, scale="B", precision=0, commas=False)
...
**Problem**: A team deploys DeepSeek-V3\index{DeepSeek-V3} (`{python} MoECapacityPlan.moe_total_params_str` total parameters, `{python} MoECapacityPlan.moe_active_params_str` active per token) for a chatbot application. The model uses FP8 weights (1 byte per parameter). The cluster has `{python} MoECapacityPlan.node_gpus_str`-GPU nodes, each with `{python} MoECapacityPlan.node_gpus_str`$\times$ H100 GPUs (`{python} MoECapacityPlan.h100_mem_gb_str` HBM per GPU, `{python} MoECapacityPlan.node_capacity_gb_str` per node). How many nodes are needed, and what is the per-token decode latency?
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt, fmt_memory, fmt_qty_int
...
class QuantizedServingCapacity:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_mem_gb_str = fmt_qty_int(a100_mem, GiB, commas=False, unit_label="GB")
