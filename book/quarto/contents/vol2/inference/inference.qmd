...
## Scaling Inference Beyond Single Machines {#sec-inference-scale-scaling-inference-beyond-single-machines-54e6}

In the **Fleet Stack** (@sec-vol2-introduction), Inference is the **Serving Layer**—the interface where the model meets the user. While the **Distribution Layer** (training) optimizes for throughput over weeks, the Serving Layer must deliver predictions in milliseconds. If the **Iron Law** defines the speed limit of training, it defines the **deadline** of inference: $L_{inference} \le \text{SLO}$.
...
The batching, sharding, and load balancing techniques introduced above do not exist in isolation; they are implemented within serving frameworks that constrain and enable different optimizations. Before examining these techniques in depth, practitioners face an immediate practical decision: which serving infrastructure to build upon?

The choice of serving framework determines which optimizations are available, how models are deployed, and what performance characteristics are achievable. Understanding this landscape first provides essential context: when we discuss continuous batching in @sec-inference-scale-continuous-batching-llm-inference-f8e6, we will see how vLLM and TensorRT-LLM implement it differently. This section provides a systematic framework for selecting among the major options.
...
## Autoscaling {#sec-inference-scale-autoscaling-168d}
...
- **Decode pool**: Optimized for memory bandwidth (e.g., nodes with maximum HBM capacity or specialized High Bandwidth Flash as discussed in @sec-data-systems) to handle thousands of concurrent autoregressive streams.
...
::: {.callout-chapter-connection title="From Serving to Squeezing"}

We have established the architectural foundations for inference at scale: continuous batching, PagedAttention, model sharding, and load balancing across distributed serving fleets. These techniques manage the resources; the next question is how to use them more efficiently. @sec-performance-engineering examines the optimization techniques, from FlashAttention and kernel fusion to mixed-precision arithmetic and speculative decoding, that push the efficiency frontier of every GPU-hour and every watt spent on inference and training.

:::

We also saw that efficient serving requires specialized hardware. The "Decode" phase of LLM inference is memory-bandwidth bound, motivating the specialized inference accelerators we examined in @sec-compute-infrastructure.
...
While data center serving provides massive scale, many applications require inference to happen closer to the user—on smartphones, IoT devices, or local gateways. This introduces extreme constraints on power, memory, and connectivity.

The next chapter, @sec-edge-intelligence, examines how we push intelligence to the edge, adapting massive models to run on constrained devices through techniques like federated learning and on-device adaptation.
