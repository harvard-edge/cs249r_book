...
**The Systems Conclusion**: Serving costs **9x** more than training in just the first year. A 10% optimization in inference latency saves **$1.8M**—paying for the entire training run almost immediately.
:::
...
***Autoregressive Generation***\index{Autoregressive Generation!definition} is the sequential token-by-token decoding process in which each output token is conditioned on all previously generated tokens. This sequential dependency creates a fundamental **memory-bandwidth bottleneck**: every generation step must read the full model weights from HBM to produce a single token of output, leaving compute units severely underutilized and making decode latency proportional to model size divided by memory bandwidth rather than available FLOPs.

:::

::: {.callout-definition title="Continuous Batching"}

***Continuous Batching***\index{Continuous Batching!definition} is a scheduling strategy that decouples batch membership from iteration boundaries, allowing new requests to enter and completed requests to leave at every decode step. Continuous batching eliminates the padding waste and head-of-line blocking inherent in static batching, where all sequences must wait for the longest to finish, at the cost of more complex memory management (particularly for the **KV Cache**) and dynamic batch reshaping at each iteration.
...
***KV Cache***\index{KV Cache!definition} is the memory buffer that trades **memory capacity** for **redundant computation** by storing previously computed key and value attention vectors so they need not be recomputed at each **autoregressive generation** step. The cache reduces per-token computation from $O(t^2)$ to $O(t)$ but grows linearly with sequence length and batch size ($2 \times L \times H \times D$ per sequence), making it the dominant memory bottleneck in large language model serving and the primary constraint on concurrent request capacity.
...
***PagedAttention***\index{PagedAttention!definition} is a memory management technique that applies virtual memory principles to **KV Cache** allocation, storing attention states in non-contiguous fixed-size physical blocks mapped through a block table. PagedAttention eliminates both internal fragmentation (over-allocated pre-reserved memory) and external fragmentation (unusable gaps between allocations) at the cost of block-table indirection overhead per attention operation, enabling near-optimal memory utilization for variable-length sequences.
...
***Speculative Decoding***\index{Speculative Decoding!definition} is a latency optimization that uses a smaller **draft model** to predict multiple tokens speculatively, then verifies the entire draft in parallel with the full target model in a single forward pass. This converts the sequential bottleneck of **autoregressive generation** into a speculation-verification pattern, trading **additional compute** (draft model inference plus parallel verification) for **reduced wall-clock latency**, with the acceptance rate between draft and target models determining the realized speedup.
...
# Goal: Compute effective speedup ratio (standard_time / speculative_time) from
#   the TikZ diagram parameters, for the prose sentence immediately below.
...
***Prefill and Decode Phases***\index{Prefill and Decode!definition} are the two distinct computational regimes of LLM inference. The prefill phase processes the entire input prompt in parallel, is compute-bound, and exhibits high **arithmetic intensity** (100+ FLOPs/byte). The decode phase generates output tokens sequentially via **autoregressive generation**, is memory-bandwidth-bound, and exhibits low arithmetic intensity (1--10 FLOPs/byte). These opposing hardware utilization profiles within a single request create a fundamental scheduling challenge: resources optimized for one phase are necessarily underutilized during the other.
