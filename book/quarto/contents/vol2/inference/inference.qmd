...
Having established the framework landscape, we now turn to the first level of the serving hierarchy: request-level optimization through batching. The framework selection made in the previous section determines implementation details, but the underlying principles apply across frameworks.
...
This moves us from request-level optimization to replica-level concerns in the serving hierarchy. Unlike training sharding where throughput is the primary concern, inference sharding must carefully balance parallelization benefits against communication overhead within strict latency budgets.
...
Load balancing, examined in the previous section, distributes requests across replicas at the service level. Within each replica, however, LLM serving faces a distinctive memory management challenge that does not exist for other model types: the key-value (KV) cache.
