...
<!-- TODO(style-cleanup): Several plaintext labels in this chapter render
     as ordinary paragraphs instead of structured headings or bold
     lead-ins. Convert each cluster to either proper subheadings or the
     house bold lead-in pattern (**Label**: full sentence...):

     - Quantization section (~line 4991): "Key insight", "Algorithm",
       "Core technique", "Critical distinction".
     - Case-study sections (~line 5259): "Scale and requirements",
       "Architecture overview", "Key design decisions", "Lessons learned"
       across all four case studies.
     - Cross-cutting observations (~line 5545): "Hybrid architectures No
       system...", etc. Still need full-sentence rewrite for the entries
       beyond "Separation of concerns".

     Also queued: fallacy "More GPU memory always means more batch size..."
     (line ~5594) needs its missing explanatory paragraph; Fallacies-
     and-Pitfalls close hype (robust, fiercely responsive, We
     conclude at line ~5641) needs recast; range form audit at line
     ~5572 (4 to 5 requests, 10 to 30 percent). -->
...
**Service time model**: For the prefill phase (processing input prompt), service time follows @eq-service-time-batch:

$$S(B) = 50 + 0.5 \cdot B \text{ ms}$$

**Optimal batch size calculation**: Applying @eq-optimal-batch-approx:
...
# ┌── LEGO ───────────────────────────────────────────────
from mlsysim.core.constants import H100_MEM_CAPACITY, GiB, BYTES_FP16, BYTES_INT4, byte
class KVCacheCalc:
...
| **Decode**  | O(1) dense weight work plus O(context length) attention over KV cache per token | Weight and KV-cache reads              | Bandwidth      |      Large (100s) |
