...
A common misconception equates inference at scale with LLM serving. Large language models present distinctive challenges and attract attention, but they are only one part of production inference. In large consumer platforms, recommendation and ranking workloads\index{Recommendation System!inference demand} can dominate AI inference cycles and capacity [@gupta2020architectural; @hazelwood2018]. Vision and image processing\index{Vision Models!inference demand}, NLP and LLM workloads\index{LLM!inference demand}, fraud detection, ads, and other classification tasks fill out the remainder. @Tbl-inference-landscape compares these model types by serving pressure and optimization challenge. Recommendation binds on embedding lookup, vision on batch efficiency, LLMs on memory bandwidth, and speech on sequential decode. No single optimization serves all four.
...
|:-----------------------------|:-------------------------------:|:-------------------:|:------------------------:|
| **Recommendation/ranking**   | Very high in consumer platforms | $<10\text{ ms}$ P99 |     Embedding lookup     |
...
The distinction between throughput capacity and latency capacity is exactly what queuing theory formalizes; @sec-appdx-inference-scale-queuing-theory-batched-inference-bbf0 derives the M/D/1 wait-time distribution that determines how much headroom a serving SLO needs.
...
- **Batch service time**: `{python} DynamicBatchingResNet50.svc_b1_ms_str` at batch=1, `{python} DynamicBatchingResNet50.svc_b32_ms_str` total for batch=32
...
**Measured performance**: Across the workloads evaluated in the vLLM study, the complete vLLM system delivered 2--4$\times$ higher throughput than FasterTransformer and Orca at a comparable latency level [@kwon2023]. The gain varied with model size, sequence length, and decoding method, so this range is an empirical result for those configurations rather than a general law for continuous batching.
...
Evaluating the per-phase latency\index{Latency!breakdown} composition (@tbl-inference-recsys-latency-breakdown) highlights how execution time distributes across request routing, batch accumulation, embedding lookup\index{Embedding Lookup}, feature processing, and ranking:
...
def paged_attention(Q, page_table, physical_blocks, block_size):
    ...
    key_blocks = []
    value_blocks = []
    for logical_idx in range(len(page_table)):
        physical_idx = page_table[logical_idx]
        key_blocks.append(physical_blocks[physical_idx].K)
        value_blocks.append(physical_blocks[physical_idx].V)

    # Normalize over every logical token, not independently per block.
    K = concatenate(key_blocks, axis=-2)
    V = concatenate(value_blocks, axis=-2)
    attention_scores = Q @ K.T / sqrt(d)
    return softmax(attention_scores, axis=-1) @ V
```
...
Two-level AllToAll communication steps make expert parallelism uniquely sensitive to load imbalance (@fig-expert-parallel-routing): if the gating function concentrates tokens on a few experts, those devices become bottlenecks while others sit idle.
...
class ModelManager:
    def load_model(self, model_id, priority):
        ...
        if required_memory > available:
            memory_deficit = required_memory - available
            evictable = get_evictable_memory(priority)
            if memory_deficit > evictable:
                raise InsufficientMemoryError(
                    "Cannot load model without violating tenant isolation constraints"
                )
            evict_lower_priority_models(priority, memory_deficit)
...
The cost savings above depend on anticipating traffic before it arrives. Proactive provisioning avoids the SLO violations that reactive-only systems suffer during ramp-up periods (@fig-predictive-reactive-scaling).
...
- Latency target: Time-to-first-token (TTFT) $<2\text{s}$, throughput varies by model
...
Serving economically under fixed SLOs requires understanding the prefill/decode split, KV-cache capacity, and the interaction between batching and tail latency. Continuous batching can raise utilization when output lengths vary, PagedAttention can recover memory lost to fragmentation, and prefill/decode disaggregation can lower per-token cost when the workload justifies separate hardware pools. These techniques keep serving cost from growing faster than useful demand.
