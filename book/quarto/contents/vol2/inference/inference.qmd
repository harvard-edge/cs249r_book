...
- [ ] **Cross-node overhead**: Identify the distribution overheads that can make sharding across machines worse than staying within a single node.
...
- [ ] **Speculative decoding level**: Identify the hierarchy level used when **Speculative Decoding** reduces token latency for a single request.
...
- [ ] **Quantization impact**: Identify which dimension (batching, memory, scheduling, topology, state) is primarily affected by the **outlier features** problem in quantization.
