...
Find minimum batch size $B_{\text{min}}$ such that $\mu_{eff}(B_{\text{min}}) > \lambda$:
...
where $E[L_{queue}]$ is the expected queuing delay, $L_{batch}$ is the batch formation delay (up to $T_{\text{window}}$), and $L_{inference}(B)$ is the inference time for batch size $B$. A concrete example of *dynamic batching for ResNet-50* illustrates how these components interact in practice.
...
$$C_{\text{traditional}} = n \times L_{max} \times c_{\text{decode}}$$

where $L_{max} = \max_i(L_i)$ and $c_{\text{decode}}$ is the compute cost per decode iteration per sequence. However, the useful compute is only:

$$C_{\text{useful}} = \sum_{i=1}^{n} L_i \times c_{\text{decode}}$$

The **waste ratio** quantifies the inefficiency:

$$W = 1 - \frac{C_{\text{useful}}}{C_{\text{traditional}}} = 1 - \frac{\sum_{i=1}^{n} L_i}{n \times L_{max}} = 1 - \frac{\bar{L}}{L_{max}}$$ {#eq-waste-ratio}
...
$$T_{\text{window}} = \min\left(T_{\text{max}}, \frac{B_{\text{target}}}{\lambda_{current}}\right)$$
...
$$ M_{KV} = 2 \times n_{\text{layers}} \times n_{\text{heads}} \times d_{\text{head}} \times s_{\text{elem}} $$
$$ \text{Total Memory} = M_{\text{weights}} + (\text{Batch} \times \text{Context} \times M_{KV}) $$
...
2.  **Latency**: Each decode step reads 37B active parameters (37 GB). Distributed across 16 GPUs, each reads $\sim$`{python} MoECapacityPlan.read_per_gpu_str` GB. At `{python} InferenceSetup.h100_mem_bw_tbs` TB/s HBM bandwidth, $t_{\text{read}} \approx$ `{python} MoECapacityPlan.t_read_ms_str` ms. AllToAll routing adds $\sim$0.3 ms.
