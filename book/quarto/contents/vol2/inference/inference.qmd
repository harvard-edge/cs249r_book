...
**Verification using Little's Law**\index{Little's Law!verification using}: $L = \lambda \cdot W$
...
**Without circuit breaker**\index{Circuit Breaker!without}:
...
**With circuit breaker**\index{Circuit Breaker!with}:
...
**Configuration for GPU inference**\index{Inference!configuration for gpu}:
...
::: {#lst-event-driven-scaling lst-cap="**Event-Driven Scaling**\index{Scaling!event-driven}: Scheduled scaling rules that pre-provision capacity for product launches and recurring traffic spikes."}
...
::: {#lst-vllm-quantization lst-cap="**vLLM Quantization**\index{Quantization!vllm}: Loading an AWQ-quantized model with automatic kernel selection and KV cache management."}
...
[^fn-two-tower-serving]: **Two-Tower Architecture**: Encodes user features and item features through separate neural networks ("towers") into embeddings, then scores via dot product. The serving advantage: item embeddings can be pre-computed offline and cached, reducing online inference to the user tower forward pass plus a sub-millisecond vector similarity lookup. The trade-off is reduced model expressiveness from late interaction, since the towers cannot attend to each other's features during encoding. \index{Architecture!two-tower, serving}
