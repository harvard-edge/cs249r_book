...
Tenant 3 bursts to `{python} NoisyNeighborQuotaScenario.tenant_burst_qps_str` (5$\times$ baseline). Without isolation, @tbl-inference-noisy-neighbor-no-isolation records how the burst cascades into SLO violations for all ten tenants on the shared pool:
...
**Contrast --- with per-tenant quotas enabled** (@tbl-inference-noisy-neighbor-quotas): impact contained to the offending tenant alone.
