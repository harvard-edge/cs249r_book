...
2.  **Inference Volume**: $1M \text{ users} \times 50 \text{ reqs} \times 365 \text{ days}$ $\approx$ `{python} ServingCostSimple.annual_reqs_b_str` Billion requests/year.
...
- **Total Training**: `{python} InferenceEconomics.training_cost_total_str`
...
$$ Q_{total} = `{python} InferenceEconomics.qps_str` \text{/s} \times 86,400 \text{ s/day} \times `{python} InferenceEconomics.serve_duration_days_str` \text{ days} \approx \mathbf{`{python} InferenceEconomics.lifetime_queries_latex_str` \text{ queries}} $$

**Total Serving Cost**
$$ C_{serve\_total} = `{python} InferenceEconomics.lifetime_queries_latex_str` \times \$10^{-5} = \mathbf{`{python} InferenceEconomics.serving_cost_total_str`} $$

**Conclusion**:
$$ \text{Ratio} = \frac{`{python} InferenceEconomics.serving_cost_total_str`}{`{python} InferenceEconomics.training_cost_total_str`} \approx \mathbf{`{python} InferenceEconomics.leverage_ratio_str`\times} $$
Serving optimization leverage is `{python} InferenceEconomics.leverage_ratio_str`$\times$ higher than training optimization. A 1% serving efficiency gain saves significant capital—far exceeding the entire training budget.
:::
...
::: {.callout-note title="Figure: The Serving Hierarchy"}
```{.tikz}
...
::: {.callout-note title="Figure: Batching Strategies Compared"}
```{.tikz}
...
**Required Concurrency ($L$)**: $L = 1000 \times 0.1 =$ **`{python} LittlesLawExample.required_concurrency_str`** concurrent requests
...
Consider serving a GPT-3 class model (`{python} Gpt3Scenario.gpt3_params_b`B parameters) with the following characteristics:
...
class LittlesLawVerify:
    ...
    arrival_rate = 100
    batch_size = 32
    expected_total_ms = GPT3ServingResults.b32_total_ms
    utilization = GPT3ServingResults.b32_util_pct / 100
    # ┌── 2. EXECUTE ───────────────────────────────────────
...
At B=32 with $\lambda = 100$ req/s and $E[T] =$ `{python} LittlesLawVerify.expected_total_str` ms:

$$L = 100 \times `{python} expected_total_ms / 1000` = `{python} LittlesLawVerify.l_str` \text{ requests in system}$$

With batch size `{python} LittlesLawVerify.batch_size` and utilization `{python} GPT3ServingResults.b32_util_pct` %:

- Expected requests in service: `{python} LittlesLawVerify.batch_size` $\times$ `{python} LittlesLawVerify.utilization` = `{python} LittlesLawVerify.in_service_str`
...
- Total batch time: $200\times20$ ms = `{python} BatchingWorkedExample.total_batch_time_str` ms
...
The GPU performs $4\times200$ = `{python} BatchingWorkedExample.total_seq_iters_str` "sequence-iterations" but only `{python} BatchingWorkedExample.useful_seq_iters_str` are useful.
...
- R1: $50\times20$ ms = `{python} BatchingWorkedExample.r1_lat_str` ms (`{python} BatchingWorkedExample.r1_imp_str`$\times$ improvement over traditional)
- R3: $100\times20$ ms = `{python} BatchingWorkedExample.r3_lat_str` ms (`{python} BatchingWorkedExample.r3_imp_str`$\times$ improvement)
- R4: $150\times20$ ms = `{python} BatchingWorkedExample.r4_lat_str` ms (`{python} BatchingWorkedExample.r4_imp_str`$\times$ improvement)
- R2: $200\times20$ ms = `{python} BatchingWorkedExample.r2_lat_str` ms (no improvement for longest request)
...
This represents a **`{python} BatchingWorkedExample.latency_red_str`% reduction in average latency**, exactly matching the waste ratio.
...
# │ Exports: a100_mem_gb, InferenceSetup.a100_mem_bw_tbs, nvlink_a100_gbs, pcie_gen5_gbs
# └─────────────────────────────────────────────────────────────────────────────
...
The system runs on 4 A100-`{python} A100HardwareScenario.a100_mem_gb`GB GPUs connected via PCIe Gen4 (`{python} A100HardwareScenario.pcie_gen5_gbs` GB/s per GPU) rather than NVLink. The server has a dual-socket CPU with NUMA topology. Memory bandwidth per GPU is `{python} A100HardwareScenario.a100_mem_bw_tbs` TB/s (HBM2e), adequate for decode operations. However, PCIe bandwidth limits tensor parallelism communication to `{python} A100HardwareScenario.pcie_gen5_gbs` GB/s versus NVLink's `{python} A100HardwareScenario.nvlink_a100_gbs` GB/s. For a batch requiring 100 MB activation transfers between GPUs, PCIe adds approximately 1.5 ms per synchronization point versus 0.17 ms for NVLink.
...
- Total KV cache: `{python} PrefixCachingAnalysis.tokens_per_user_str`$\times$ 1000$\times$ 2$\times$ 80$\times$ $8192\times2$ = `{python} PrefixCachingAnalysis.kv_tb_no_cache_str` TB

**With prefix caching**:

- Shared prefix: 2000 tokens (once)
- Unique per user: 500 tokens
- Total: ($2000\times1$) + ($500\times1000$) = `{python} PrefixCachingAnalysis.total_tokens_cached_str` tokens
- Memory: `{python} PrefixCachingAnalysis.total_tokens_cached_str`$\times$ 2$\times$ 80$\times$ $8192\times2$ = `{python} PrefixCachingAnalysis.kv_tb_cached_str` TB

**Savings**: `{python} PrefixCachingAnalysis.memory_reduction_pct_str`% reduction in KV cache memory, enabling `{python} PrefixCachingAnalysis.concurrent_multiplier_str`$\times$ more concurrent users.
...
This is a substantial fraction of the H100's `{python} KVCacheCalc.h100_mem_gb` GB of HBM, consumed by a single request's KV cache alone. For a batch of 8 concurrent requests, the KV cache would require `{python} f"{KVCacheCalc.kv_fp16_gb * 8:.0f}"` GB, exceeding single-GPU capacity. Reducing this footprint requires two complementary strategies: quantization and architectural optimization.
...
**Problem**: You serve a 70B model on 4$\times$ H100 GPUs. Weights in FP16 consume 140 GB (35 GB/GPU). KV cache at FP16 consumes `{python} KVCacheCalc.kv_fp16_gb_str` GB per request. How does quantizing weights to INT4 and KV cache to INT8 change the maximum batch size?
...
# │ Exports: KVCacheCalc.h100_mem_gb
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import H100_MEM_CAPACITY, GB
...
This exceeds the `{python} HardwareSetupSharding.h100_mem_gb`GB capacity of an H100 GPU, requiring at minimum 2-way sharding.
...
**For `{python} TpSpeedupCalc.n_layers` layers**:

- Sequential: `{python} TpSpeedupCalc.seq_total_ms_str`ms per token
- 8-way TP: `{python} TpSpeedupCalc.tp8_total_ms_str`ms per token

The `{python} TpSpeedupCalc.tp_speedup_str`$\times$ speedup (vs theoretical `{python} TpSpeedupCalc.tp_theoretical`$\times$) reflects communication overhead. With `{python} TpSpeedupCalc.nvlink_h100_gbs` GB/s NVLink bandwidth, each 8 MB activation all-reduce takes ~0.3 ms.
...
2.  **Latency**: Each decode step reads 37B active parameters (37 GB). Distributed across 16 GPUs, each reads $\sim$`{python} MoECapacityPlan.read_per_gpu_str` GB. At `{python} InferenceSetup.h100_mem_bw_tbs` TB/s HBM bandwidth, $t_{read} \approx$ `{python} MoECapacityPlan.t_read_ms_str` ms. AllToAll routing adds $\sim$0.3 ms.
...
- Total capacity: $10\times1000$ + $20\times600$ = `{python} HeterogeneousGpuCluster.total_cap_str` QPS
...
- H100: $15000\times0.045$ = `{python} HeterogeneousGpuCluster.h100_load_str` QPS (`{python} HeterogeneousGpuCluster.h100_util_str`% utilization)
- A100: $15000\times0.027$ = `{python} HeterogeneousGpuCluster.a100_load_str` QPS (`{python} HeterogeneousGpuCluster.a100_util_str`% utilization)
...
- A100 utilization: `{python} HeterogeneousGpuCluster.a100_util_equal_str`% (overloaded, latency spikes)
...
::: {.callout-note title="Figure: Predictive vs. Reactive Scaling"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.8]
  \definecolor{BlueLine}{HTML}{006395}
  \definecolor{BlueL}{HTML}{D1E6F3}
...
A 70B model with 4-bit weights requires approximately 35 GB, leaving 45 GB on an `{python} A100HardwareScenario.a100_mem_gb`GB A100 for KV cache. With FP16 KV cache at 2.5 MB per 1K tokens per sequence, this supports ~18 concurrent 1K-token sequences versus ~8 with FP16 weights.
...
::: {.callout-note title="Figure: Ranking Cascade Architecture"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
