...
The four levels form a *serving hierarchy*, each building on the one below.
...
Batching efficiency depends on how computation scales with batch size relative to how memory and communication scale. Different model architectures exhibit different scaling relationships, requiring different batching strategies.
...
1. **Vision Model** ($T_{fixed}=2 ms$, $T_{var}=1 ms$):
...
2. **LLM Decode** ($T_{fixed}=40 ms$, $T_{var}=0.5 ms$):
...
2. **Batch size 8 achieves stability**: At `{python} GPT3ServingResults.b8_util_pct`% utilization, the system is stable but queuing delays contribute significantly to latency (`{python} GPT3ServingResults.b8_wait_ms` ms), as @fig-queuing-hockey-stick illustrates.
...
```{python}
#| label: batching-optimizer-example
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ BATCHING OPTIMIZER (TIER 3)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Callout "Automated SLA Resolution"
# │
# │ Goal: Demonstrate the Tier 3 Optimizer finding the maximum viable batch size.
# │ Show: The optimizer balances P99 SLA against throughput for Llama-2-70B.
# │ How: Invoke mlsysim.BatchingOptimizer.
# │
# │ Imports: mlsysim (BatchingOptimizer, Models, Hardware)
# │ Exports: AutoBatching.qps_str, AutoBatching.sla_ms_str,
# │          AutoBatching.opt_batch_str, AutoBatching.opt_p99_str,
# │          AutoBatching.opt_util_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import BatchingOptimizer, Models, Hardware
from mlsysim.fmt import fmt_percent, fmt, check

# ┌── LEGO ───────────────────────────────────────────────
class AutoBatching:
    """
    Namespace for automated Batch Size search.
    Scenario: Finding the optimal batch size for Llama-2-70B under a strict SLA.
    """
    ...
    qps_load = 10.0
    sla_ms = 1000.0  # 1.5 seconds P99
    seq_len = 32

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    optimizer = BatchingOptimizer()

    # Solve for optimal batch size that meets the SLA
    res = optimizer.solve(
        model, hardware,
        seq_len=seq_len,
        arrival_rate_qps=qps_load,
        sla_latency_ms=sla_ms,
        max_search_batch=64
    )

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(res.is_feasible, "Optimizer must find a feasible batch size for this scenario.")
    check(res.p99_latency.m_as('ms') <= sla_ms, "Selected batch size must respect SLA.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    qps_str = f"{qps_load:.0f}"
    sla_ms_str = f"{sla_ms:.0f}"
    opt_batch_str = f"{res.best_batch_size}"
    opt_p99_str = f"{res.p99_latency.m_as('ms'):.0f}"
    opt_util_str = fmt_percent(res.best_config.get('utilization', 0.0), precision=1)

```

::: {.callout-notebook title="Automated SLA Resolution (Tier 3 Optimizer)"}

**Problem**: A production endpoint serving **Llama-2-70B** on a single H100 GPU receives **`{python} AutoBatching.qps_str` queries per second (QPS)**. The Service Level Agreement (SLA) requires that 99% of requests complete in under **`{python} AutoBatching.sla_ms_str` ms**. What is the maximum batch size the system can sustain without violating this queuing SLA?

**The Solution**: Instead of manually plotting throughput-latency tradeoff curves or solving complex M/M/c queuing approximations, we invoke a **Tier 3 Optimizer** (the `BatchingOptimizer` in our physics engine) to automatically invert the queuing models.

**The Result**: The optimizer sweeps the design space of continuous batching configurations and identifies the optimal operating point:

*   **Optimal Batch Size ($B$)**: **`{python} AutoBatching.opt_batch_str`**
*   **Projected P99 Latency**: **`{python} AutoBatching.opt_p99_str` ms** (Safely under the `{python} AutoBatching.sla_ms_str` ms SLA)
*   **System Utilization**: **`{python} AutoBatching.opt_util_str`**

**The Systems Insight**: The optimizer proves that a batch size of `{python} AutoBatching.opt_batch_str` perfectly balances the high fixed overhead of the LLM decode step with the strict latency SLA. If the batch size were smaller, the GPU would be severely underutilized, causing queues to back up indefinitely under the 5 QPS load. If the batch size were larger, the raw computation time per batch would exceed the SLA even if the queue was entirely empty. The "Square Root Law" heuristic is valuable for intuition, but an optimizer is required for production synthesis.
...
As @fig-inference-lifecycle shows, non-compute stages (serialization, queuing, routing) can consume a substantial fraction of the latency budget, meaning batching strategy selection must account for the full pipeline, not GPU execution time alone. The following decision framework guides strategy selection:
...
*Why* can we not simply increase the batch size to maximize throughput in LLM serving? We hit the **KV Cache Wall**. As @fig-kv-cache-wall shows, model weights represent a fixed "Static Tax" on GPU memory, while the KV cache grows linearly with both batch size and sequence length.
...
KV Cache management enables a single GPU to handle many concurrent requests without crashing. When the model itself exceeds single-GPU capacity, as a 175-billion parameter foundation model does, fitting even the weights requires splitting the workload across multiple devices through model sharding for inference.

## Dynamic State & Agentic Workflows {#sec-inference-dynamic-state}

As machine learning systems transition from monolithic pattern matchers to **Agentic Workflows** (chains, loops, and tool-use), the serving architecture undergoes its final transformation. In this regime, the system is no longer serving a single request; it is managing a **Stateful reasoning session** that may span multiple specialized models, external tools, and retrieval steps. This transition introduces two new "Walls": the **State Wall** and the **Logic Wall**.

### The Physics of State Migration

In a monolithic LLM call, the KV-cache is generated and consumed on a single node. In an agentic workflow, a session might start on a large "General Reasoning" model (e.g., Llama-3-405B) and then hand off to a smaller "Tool-Specialized" model (e.g., Llama-3-8B) to execute a coding task.

This requires **State Migration**\index{State Migration}: moving the accumulated KV-cache—the "short-term memory" of the reasoning process—across the network fabric. The latency of this handoff ($L_{\text{mig}}$) is governed by the size of the state and the bisection bandwidth:

$$ L_{\text{mig}} = \frac{\text{KV-Cache Size}}{\text{Bisection BW}} $$

For large context windows (128k+ tokens), the KV-cache can exceed 10--20 GB. Moving this state over a standard 100G Ethernet link adds hundreds of milliseconds of delay, violating interactive reasoning SLAs.

```{python}
#| label: state-migration-calc
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ STATE MIGRATION PHYSICS (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-inference-dynamic-state — the cost of agentic handoffs
# │
# │ Goal: Quantify the latency of moving reasoning state across nodes.
# │ Show: ~10 GB KV-cache takes ~200ms on 400G fabric vs ~800ms on 100G.
# │ How: Latency = Size / BW.
# │
# │ Imports: mlsysim.core.constants (LLAMA3_8B_PARAMS, GB, Gbps, BITS_PER_BYTE)
# │ Exports: StateMigration.cache_size_gb_str, StateMigration.eth_lat_ms_str,
# │          StateMigration.ib_lat_ms_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import LLAMA3_8B_PARAMS, GB, Gbps, BITS_PER_BYTE
from mlsysim.fmt import fmt, check

class StateMigration:
    """Calculates state handoff latency for agentic workflows."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    # A 128k context KV-cache for a 70B model is ~10.2 GB (FP16)
    kv_cache_gb = 10.2
    bw_100g = 100 * Gbps
    bw_400g = 400 * Gbps

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # Bytes to Bits: 10.2 * 8 = 81.6 Gb
    bits = kv_cache_gb * GB.m_as('byte') * BITS_PER_BYTE

    t_100g_ms = (bits / bw_100g.m_as('bit/second')) * 1000
    t_400g_ms = (bits / bw_400g.m_as('bit/second')) * 1000

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(t_100g_ms > 500, f"100G migration should be slow (~800ms), got {t_100g_ms:.1f}")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cache_size_gb_str = f"{kv_cache_gb:.1f}"
    eth_lat_ms_str = fmt(t_100g_ms, precision=0)
    ib_lat_ms_str = fmt(t_400g_ms, precision=0)
```

::: {.callout-notebook title="State Migration Latency"}

**Problem**: An agentic workflow hands off a **`{python} StateMigration.cache_size_gb_str` GB** KV-cache (128k context) between two inference nodes. *How* does the network choice impact the "handoff pause" the user experiences?

**The Math**:

1.  **100G Ethernet**: $L_{\text{mig}} \approx$ **`{python} StateMigration.eth_lat_ms_str` ms**. The user experiences a nearly 1-second lag every time the agent switches models.
2.  **400G InfiniBand**: $L_{\text{mig}} \approx$ **`{python} StateMigration.ib_lat_ms_str` ms**. The handoff is perceived as a minor stutter, within the range of standard inter-token latency.

**The Systems Insight**: In the agentic era, the **Network Wall** is the primary bottleneck for **Reasoning Continuity**. If the bisection bandwidth is low, the fleet is forced to be "Sticky"—pinning sessions to single nodes and sacrificing utilization—or pay a massive "Handoff Tax." High-bandwidth fabrics are no longer just for training; they are the backplane of distributed reasoning.

:::

### The Reasoning-to-Compute Ratio

The final invariant of inference at scale is the **Reasoning-to-Compute Ratio ($R:C$)**. We distinguish between two regimes of intelligence:

1.  **Fast Execution ($R:C \approx 1$):** Every token generated is a final output. This is the regime of standard chat and search.
2.  **Slow Reasoning ($R:C \gg 1$):** For every output token, the model executes multiple internal reasoning steps, searches, or tool calls.

At production scale, agentic systems shift the fleet from a **Throughput Factory** to a **Search Engine**. The binding constraint moves from memory bandwidth (loading weights) to **Logic Throughput** (how many reasoning steps can the fleet execute per megawatt). Mastering this shift requires moving from simple token-based SLAs to **Goal-based SLAs**, where the system is optimized to find the correct answer with the minimum total energy, regardless of the number of intermediate tokens generated.
...
A foundational result in load balancing theory [@mitzenmacher2001power] shows that querying just two random servers before making a routing decision provides exponentially better load distribution than random assignment.[^fn-balls-bins-routing]
...
::: {#fig-pipeline-parallel-routing fig-env="figure" fig-pos="htb" fig-cap="**Pipeline Parallelism Request Flow**. (a) A single request flows sequentially through four stages—no latency benefit from parallelism. (b) Multiple requests fill the pipeline: once steady state is reached, throughput scales with the number of stages P, while per-request latency remains approximately constant." fig-alt="Two-part diagram. Top: single request through four stage boxes labeled by layer ranges with GPU labels below. Bottom: Gantt-style grid showing four GPU rows with requests offset by one stage, illustrating pipeline fill and steady-state throughput."}
...
The two paths in @fig-edge-cache-routing highlight the order-of-magnitude latency difference: cache hits return in under 10 ms, while cache misses incur the full backend round-trip. Effectiveness depends on request repeatability, as the following table quantifies:
...
Time-of-day routing can reduce costs by 20--40% for batch workloads while maintaining SLOs for interactive traffic.
