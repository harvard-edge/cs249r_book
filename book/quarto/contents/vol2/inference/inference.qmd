...
[^fn-consistent-hashing-kv]: **Consistent Hashing**: A ring-based load distribution algorithm originally deployed at Akamai for CDN routing. With $N$ evenly weighted servers and $K$ keys, adding one server remaps about $K/(N+1)$ keys to the new owner, while removing one remaps the departing server's roughly $K/N$ keys. For LLM services that deliberately retain cross-turn KV state, this limited remapping preserves more cache locality during autoscaling than a full rehash. Stateless services do not need session affinity, although prefix-aware routing can still improve reuse.
