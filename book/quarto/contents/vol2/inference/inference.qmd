...
The three choices in @tbl-dynamic-batching-resnet50-policy differ only in how much latency budget they convert into batch size:

| **Policy**  | **Batching window**                                 | **Expected batch**                                   | **Per-request compute**                             | **Utilization**                                  | **Latency outcome**                                               |
|:------------|:----------------------------------------------------|:-----------------------------------------------------|:----------------------------------------------------|:-------------------------------------------------|:------------------------------------------------------------------|
| No batching | 0 ms                                                | 1 request                                            | `{python} DynamicBatchingResNet50.svc_b1_ms_str`    | `{python} DynamicBatchingResNet50.rho_a_eq_math` | Overloaded; cannot meet demand                                    |
| Dynamic B   | `{python} DynamicBatchingResNet50.twindow_b_ms_str` | `{python} DynamicBatchingResNet50.eb_b_str` requests | `{python} DynamicBatchingResNet50.per_req_b_ms_str` | `{python} DynamicBatchingResNet50.rho_b_eq_math` | ~`{python} DynamicBatchingResNet50.lat_b_ms_str` mean, ~30 ms P99 |
| Dynamic C   | `{python} DynamicBatchingResNet50.twindow_c_ms_str` | `{python} DynamicBatchingResNet50.eb_c_str` requests | `{python} DynamicBatchingResNet50.per_req_c_ms_str` | `{python} DynamicBatchingResNet50.rho_c_eq_math` | ~`{python} DynamicBatchingResNet50.lat_c_ms_str` mean, ~42 ms P99 |

: **Dynamic batching policy comparison**: Option C achieves about `{python} DynamicBatchingResNet50.capacity_gain_pct_str` higher per-replica capacity, or `{python} DynamicBatchingResNet50.util_drop_pct_str` lower utilization, at the cost of higher average latency. Both dynamic batching policies meet the `{python} DynamicBatchingResNet50.latency_slo_ms_str` P99 SLO. {#tbl-dynamic-batching-resnet50-policy}
...
**Problem**: Quantify how one tenant's burst changes latency for every tenant in an unprotected shared pool.
