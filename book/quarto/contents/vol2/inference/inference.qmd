...
The techniques in this chapter enable inference systems that scale from single machines to global datacenter deployments. However, the Machine Learning Fleet does not stop at the datacenter door. The next chapter, @sec-edge-intelligence, examines the other end of the deployment spectrum: inference at the edge, where devices with limited compute, memory, and power must still deliver predictions reliably. We will see how the compression and optimization patterns established here are pushed to their physical limits on sensors, smartphones, and autonomous robots.
