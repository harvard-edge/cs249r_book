...
# ┌─────────────────────────────────────────────────────────────────────────────
# │ INFERENCE CHAPTER SETUP
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Chapter-wide registry — values used across §When Single-Machine
# │   Serving Is Insufficient, §KV Cache Compression, §Tensor Parallelism,
# │   §Speculative Decoding, §Load Balancing, footnote fn-hbm3-bandwidth, fn-nvlink-bandwidth-inf.
# │
# │ Goal: Pre-compute H100/A100 memory, bandwidth, NVLink, PCIe, and GPT-3
# │   parameter strings for inline use throughout the chapter.
# │ Show: "80" GB, "3.35" TB/s, "900" GB/s, etc. — inline in prose and callouts.
# │ How: Extract pint Quantity magnitudes with .m_as(unit) for fmt() string formatting.
# │
# │ Imports: mlsys.constants (H100_MEM_CAPACITY, H100_MEM_BW, H100_TDP,
# │   A100_MEM_CAPACITY, A100_MEM_BW, NVLINK_H100_BW, NVLINK_A100_BW,
# │   PCIE_GEN4_BW, PCIE_GEN5_BW, GPT3_PARAMS, GiB, TB, GB, watt, second, param),
# │   mlsys.formatting (fmt, sci)
# │ Exports: h100_mem_gb, h100_mem_bw_tbs, h100_tdp_w, a100_mem_gb,
# │   a100_mem_bw_tbs, a100_mem_bw_gbs, nvlink_h100_gbs, nvlink_a100_gbs,
# │   pcie_gen4_gbs, pcie_gen5_gbs, gpt3_params_b
# │ Note: PERSISTENT — h100_mem_gb used in §Memory Exhaustion (line ~90),
# │   §KV Cache Compression (line ~2500), §Tensor Parallelism (line ~2935);
# │   nvlink_h100_gbs used in §Tensor Parallelism (line ~2973) and fn-nvlink-bandwidth-inf.
# └─────────────────────────────────────────────────────────────────────────────
...
class InferenceSetup:
    """Pre-compute H100/A100 memory, bandwidth, NVLink, PCIe, and GPT-3 strings for inline use."""

    # ┌── 1. PARAMETERS (Inputs) ──────────────────────────────────────────────
    # All inputs come from mlsys.constants — no magic numbers here.

    # ┌── 2. CALCULATION (The Physics) ────────────────────────────────────────
    # H100 specs
    h100_mem_gb = f"{H100_MEM_CAPACITY.m_as(GiB):.0f}"       # "80"
    h100_mem_bw_tbs = f"{H100_MEM_BW.m_as(TB / second)}"     # "3.35"
    h100_tdp_w = f"{H100_TDP.m_as(watt):.0f}"                # "700"

    # A100 specs
    a100_mem_gb = f"{A100_MEM_CAPACITY.m_as(GiB):.0f}"       # "80"
    a100_mem_bw_tbs = f"{A100_MEM_BW.m_as(TB / second):.0f}" # "2"
    a100_mem_bw_gbs = f"{A100_MEM_BW.m_as(GB / second):.0f}" # "2039"
    ...
    # PCIe bandwidth
    pcie_gen4_gbs = f"{PCIE_GEN4_BW.m_as(GB / second):.0f}"  # "32"
    pcie_gen5_gbs = f"{PCIE_GEN5_BW.m_as(GB / second):.0f}"  # "64"

    # GPT-3
    gpt3_params_b = f"{GPT3_PARAMS.m_as(param) / BILLION:.0f}"   # "175"

    # ┌── 3. INVARIANTS (Guardrails) ──────────────────────────────────────────
    # No numeric invariants needed — values are direct pint lookups.

    # ┌── 4. OUTPUTS (Formatting) ─────────────────────────────────────────────
    # All exports are string variables (see Exports above).

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
h100_mem_gb = InferenceSetup.h100_mem_gb
h100_mem_bw_tbs = InferenceSetup.h100_mem_bw_tbs
h100_tdp_w = InferenceSetup.h100_tdp_w
a100_mem_gb = InferenceSetup.a100_mem_gb
a100_mem_bw_tbs = InferenceSetup.a100_mem_bw_tbs
a100_mem_bw_gbs = InferenceSetup.a100_mem_bw_gbs
nvlink_h100_gbs = InferenceSetup.nvlink_h100_gbs
nvlink_a100_gbs = InferenceSetup.nvlink_a100_gbs
pcie_gen4_gbs = InferenceSetup.pcie_gen4_gbs
pcie_gen5_gbs = InferenceSetup.pcie_gen5_gbs
gpt3_params_b = InferenceSetup.gpt3_params_b
```
...
Serving optimization leverage is 500$\times$ higher than training optimization. A 1% serving efficiency gain saves \$63k—5$\times$ the entire training budget.
:::
...
## Serving Framework Selection {#sec-inference-scale-serving-framework-selection-45d3}

If your team wants to deploy a new vision-language model, do you write a custom Flask wrapper around PyTorch, rely on NVIDIA's Triton Inference Server, or adopt an LLM-specific engine like vLLM? The framework you select will dictate your latency ceiling, your memory overhead, and the specific batching algorithms available to you. Understanding this framework landscape is the prerequisite to all further inference scaling.

### Framework Categories {#sec-inference-scale-framework-categories-172a}
...
### Framework Selection Criteria {#sec-inference-scale-framework-selection-criteria-6c20}
...
### vLLM Architecture {#sec-inference-scale-vllm-architecture-9b84}

vLLM[^fn-vllm-etymology] [@kwon2023vllm] has emerged as the leading open-source LLM serving framework due to its PagedAttention innovation. Understanding its architecture illustrates key LLM serving principles.
...
The core innovations in vLLM include PagedAttention (virtual memory for KV cache, covered in @sec-inference-scale-pagedattention-3c94), continuous batching (add/remove requests mid-generation), optimized attention kernels (FlashAttention [@dao2022flashattention] integration), and tensor parallelism (automatic model sharding across GPUs).
...
@lst-inference-deployment-example shows a typical vLLM deployment configuration.

::: {#lst-inference-deployment-example lst-cap="**vLLM Deployment**: Initialization with automatic GPU detection, tensor parallelism, and batch inference."}
```{.python}
...
### TensorRT-LLM Architecture {#sec-inference-scale-tensorrtllm-architecture-2794}
...
The build and deployment workflow in @lst-tensorrt-build-workflow illustrates the three-step process of converting, optimizing, and deploying a model.

::: {#lst-tensorrt-build-workflow lst-cap="**TensorRT-LLM Build Workflow**: Converting a model checkpoint, building an optimized engine, and deploying with Triton."}
```{.bash}
...
### Triton Inference Server {#sec-inference-scale-triton-inference-server-f0ba}
...
### Framework Selection Decision Tree {#sec-inference-scale-framework-selection-decision-tree-734b}

@fig-framework-decision-tree systematizes the selection process. Starting from the model's generation pattern, each decision node narrows the candidate set based on hardware constraints and operational requirements.

::: {#fig-framework-decision-tree fig-env="figure" fig-pos="htb" fig-cap="**Framework Selection Decision Tree**: Model type, throughput requirements, serving complexity, and target hardware systematically narrow the framework choice from seven candidates to one. Diamond nodes represent decision points; green leaves indicate recommended frameworks for each path." fig-alt="Decision tree with four diamond decision nodes branching to seven green leaf nodes showing TensorRT-LLM, vLLM, Triton, TensorRT, OpenVINO, and ONNX Runtime as framework choices."}
```{.tikz}
\scalebox{0.82}{%
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  \tikzset{
    start/.style={rectangle, draw=black!70, fill=black!5, thick,
      rounded corners=3pt, align=center,
      font=\small\bfseries\usefont{T1}{phv}{m}{n}},
    dec/.style={diamond, draw=#1, fill=#1!12, thick, align=center,
      inner sep=1pt, font=\scriptsize\usefont{T1}{phv}{m}{n}, aspect=2},
    leaf/.style={rectangle, draw=#1, fill=#1!12, thick,
      rounded corners=3pt, align=center,
      font=\scriptsize\bfseries\usefont{T1}{phv}{m}{n},
      minimum width=1.5cm, minimum height=0.5cm},
    arr/.style={->, >=stealth, thick, black!50},
    lbl/.style={font=\tiny\usefont{T1}{phv}{m}{n}, fill=white, inner sep=1pt}
  }

  % Level 0: Start
  \node[start] (s) at (5, 0) {New Model to Deploy};

  % Level 1: LLM decision
  \node[dec=BlueLine] (d1) at (5, -2.2) {LLM\\(autoregressive)?};
  \draw[arr] (s) -- (d1);

  % Level 2a: Throughput critical? (Yes to LLM)
  \node[dec=BlueLine] (d2) at (1.5, -5) {Max throughput\\critical?};
  \draw[arr] (d1) -- node[lbl, above left] {Yes} (d2);

  % Level 2b: Multi-model? (No to LLM)
  \node[dec=OrangeLine] (d3) at (8.5, -5) {Multi-model\\serving?};
  \draw[arr] (d1) -- node[lbl, above right] {No} (d3);

  % Level 3: Leaves from throughput decision
  \node[leaf=GreenLine] (trt) at (0, -7.8) {TensorRT-LLM};
  \draw[arr] (d2) -- node[lbl, left] {Yes} (trt);
  \node[font=\tiny\itshape, text=gray] at (0, -8.35) {NVIDIA optimized};

  \node[leaf=GreenLine] (vllm) at (3, -7.8) {vLLM};
  \draw[arr] (d2) -- node[lbl, right] {No} (vllm);
  \node[font=\tiny\itshape, text=gray] at (3, -8.35) {Open-source, flexible};

  % Level 3: Leaf from multi-model Yes
  \node[leaf=GreenLine] (triton) at (6.5, -7.8) {Triton};
  \draw[arr] (d3) -- node[lbl, left] {Yes} (triton);
  \node[font=\tiny\itshape, text=gray] at (6.5, -8.35) {Multi-framework};

  % Level 3: Hardware target decision (multi-model No)
  \node[dec=VioletLine] (d4) at (10.5, -7.8) {Target\\hardware?};
  \draw[arr] (d3) -- node[lbl, right] {No} (d4);

  % Level 4: Leaves from hardware target
  \node[leaf=GreenLine] (trt2) at (8.5, -10.5) {TensorRT};
  \draw[arr] (d4) -- (trt2);
  \node[font=\tiny\itshape, text=gray] at (8.5, -11.05) {NVIDIA GPU};

  \node[leaf=GreenLine] (ovino) at (10.5, -10.5) {OpenVINO};
  \draw[arr] (d4) -- (ovino);
  \node[font=\tiny\itshape, text=gray] at (10.5, -11.05) {Intel CPU/GPU};

  \node[leaf=GreenLine] (onnx) at (12.5, -10.5) {ONNX Runtime};
  \draw[arr] (d4) -- (onnx);
  \node[font=\tiny\itshape, text=gray] at (12.5, -11.05) {Cross-platform};
...
### Framework Performance Benchmarking {#sec-inference-scale-framework-performance-benchmarking-5352}

When evaluating frameworks, benchmark on representative workloads:

@lst-llm-benchmark-methodology outlines the standard benchmark parameters for LLM evaluation.

::: {#lst-llm-benchmark-methodology lst-cap="**LLM Benchmark Methodology**: Standard parameters covering input/output lengths, batch sizes, concurrency, and key latency and throughput metrics."}
```{.python}
...
### Migration and Integration Considerations {#sec-inference-scale-migration-integration-considerations-04a0}
...
### Orchestration Platforms for Production Serving {#sec-inference-scale-orchestration-platforms-production-serving-9dd8}

Individual inference runtimes (vLLM, TensorRT-LLM, Triton) handle the mechanics of efficient inference on a single node or small cluster. Production systems require an orchestration layer that manages service composition, autoscaling, traffic management, fault tolerance, and multi-tenancy. Platforms such as Ray Serve [@moritz2018ray] provide this orchestration for Python-native ML workloads at scale.
...
@lst-vllm-ray-serve demonstrates a production-ready vLLM deployment with Ray Serve.

::: {#lst-vllm-ray-serve lst-cap="**vLLM with Ray Serve**: Autoscaling LLM deployment using Ray actor pools and vLLM inference replicas."}
```{.python}
...
1. **Memory**: Llama-70B requires ~140GB in FP16. With 4-bit quantization (AWQ), this drops to ~35GB, fitting on a single H100 (`{python} h100_mem_gb`GB) with room for KV cache.
...
**Recommendation**: Option B (vLLM + Ray Serve) provides the best balance. At 500 QPS, the 30% throughput advantage of TensorRT-LLM does not justify the deployment complexity. Start with vLLM; migrate to TensorRT-LLM only if traffic grows beyond 2,000 QPS. @lst-framework-recommendation shows the recommended production configuration.

::: {#lst-framework-recommendation lst-cap="**Framework Recommendation**: Production configuration for vLLM with Ray Serve autoscaling on a customer support workload."}
```{.python}
...
While selecting the right framework provides the structural foundation, the framework itself is merely a vessel for processing data. To actually achieve high throughput and maximize our GPU investment, we must now configure how this framework groups incoming traffic using advanced batching strategies at scale.
...
::: {.callout-definition title="Autoregressive Generation"}

***Autoregressive Generation***\index{Autoregressive Generation!definition} is the sequential token-by-token decoding process where each output token is conditioned on all previously generated tokens.

1.  **Significance (Quantitative):** It creates a fundamental **Memory-Bandwidth Bottleneck**. Within the **Iron Law**, each step must read the full model weights from HBM to produce a single token, making decode latency proportional to $\frac{D_{vol}}{BW}$. This leads to extremely low **Arithmetic Intensity** and low system utilization ($\eta$).
2.  **Distinction (Durable):** Unlike **Feedforward Architectures** (e.g., CNNs), which process all outputs in a single pass, Autoregressive models have a **Sequential Dependency** ($L_{lat}$) where the compute depth scales with the number of tokens generated.
3.  **Common Pitfall:** A frequent misconception is that decode speed is limited by FLOPs ($R_{peak}$). In reality, for almost all LLMs, the GPUs are "waiting for data" rather than "doing math," meaning a faster GPU with the same bandwidth will not speed up generation.
...
::: {.callout-note title="Archetype A: Throughput-Latency"}
...
class BatchingWorkedExample:
    """Quantify latency difference between traditional and continuous batching for 4 LLM requests."""
    ...
    # Continuous batching latencies (each request: output_length * decode_time)
    ...
    r4_improvement = round(trad_latency_ms / r4_latency_ms, 2)

    # ┌── 3. INVARIANTS (Guardrails) ──────────────────────────────────────────
    assert cont_avg_latency_ms <= trad_latency_ms, "Continuous batching avg must be <= traditional"
    assert 0 < latency_reduction_pct < 100, "Reduction must be between 0% and 100%"
...
- Total batch time: 200$\times$ 20 ms = `{python} total_batch_time_str` ms
...
The GPU performs 4$\times$ 200 = `{python} total_seq_iters_str` "sequence-iterations" but only `{python} useful_seq_iters_str` are useful.
...
- R1: 50$\times$ 20 ms = `{python} r1_lat_str` ms (`{python} r1_imp_str`$\times$ improvement over traditional)
- R3: 100$\times$ 20 ms = `{python} r3_lat_str` ms (`{python} r3_imp_str`$\times$ improvement)
- R4: 150$\times$ 20 ms = `{python} r4_lat_str` ms (`{python} r4_imp_str`$\times$ improvement)
- R2: 200$\times$ 20 ms = `{python} r2_lat_str` ms (no improvement for longest request)
...
The root cause is not the hardware (Infrastructure Layer) but the scheduling policy (Distribution Layer). Head-of-line blocking occurs when the scheduler forms large batches from bursty arrivals without considering request heterogeneity. The 500ms P99 corresponds to large batches where short requests wait for long-sequence completions.
...
After implementing these changes, P99 dropped to 185ms. The Infrastructure Layer was adequate; the problem was entirely in the Distribution Layer's scheduling logic.
...
# │       batch 1/2/4/8; show 80GB HBM limit; OOM zone.
...
class PrefixCachingCalc:
    ...
    kv_tb_no_cache = kv_bytes_no_cache / 1e12
    ...
    kv_tb_cached = kv_bytes_cached / 1e12

    # Savings
    memory_reduction_pct = (1 - kv_tb_cached / kv_tb_no_cache) * 100
    concurrent_multiplier = kv_tb_no_cache / kv_tb_cached

    # ┌── 3. INVARIANTS (Guardrails) ──────────────────────────────────────────
    assert kv_tb_cached < kv_tb_no_cache, "Cached must be smaller than uncached"
    assert 0 < memory_reduction_pct < 100, "Reduction must be between 0% and 100%"

    # ┌── 4. OUTPUTS (Formatting) ─────────────────────────────────────────────
    tokens_per_user_str = f"{tokens_per_user:,}"
    kv_tb_no_cache_str = f"{kv_tb_no_cache:.1f}"
    total_tokens_cached_str = f"{total_tokens_cached:,}"
    kv_tb_cached_str = f"{kv_tb_cached:.1f}"
    memory_reduction_pct_str = f"{memory_reduction_pct:.0f}"
    concurrent_multiplier_str = f"{concurrent_multiplier:.0f}"

# ┌── EXPORTS (Bridge to Text) ─────────────────────────────────────────────────
tokens_per_user_str = PrefixCachingCalc.tokens_per_user_str
kv_tb_no_cache_str = PrefixCachingCalc.kv_tb_no_cache_str
total_tokens_cached_str = PrefixCachingCalc.total_tokens_cached_str
kv_tb_cached_str = PrefixCachingCalc.kv_tb_cached_str
memory_reduction_pct_str = PrefixCachingCalc.memory_reduction_pct_str
concurrent_multiplier_str = PrefixCachingCalc.concurrent_multiplier_str
```
...
- Total KV cache: `{python} tokens_per_user_str`$\times$ 1000$\times$ 2$\times$ 80$\times$ 8192$\times$ 2 = `{python} kv_tb_no_cache_str` TB

**With prefix caching**:

- Shared prefix: 2000 tokens (once)
- Unique per user: 500 tokens
- Total: (2000$\times$ 1) + (500$\times$ 1000) = `{python} total_tokens_cached_str` tokens
- Memory: `{python} total_tokens_cached_str`$\times$ 2$\times$ 80$\times$ 8192$\times$ 2 = `{python} kv_tb_cached_str` TB
...
### KV Cache Compression {#sec-inference-scale-kv-cache-compression-943a}
...
While weight quantization reduces model storage, the KV cache dominates memory consumption for long-context LLM serving. At 32K+ token context lengths, KV cache can exceed model weights in memory usage. For a 70B model with 32K context in FP16, a single sequence consumes 86GB, exceeding the `{python} h100_mem_gb`GB H100 capacity.

This reveals a critical property: KV cache values exhibit different distributions than model weights, enabling targeted quantization strategies. Keys have relatively uniform distributions and tolerate aggressive quantization (2-4 bits), while Values are more sensitive and require careful calibration (4-6 bits).

**KIVI (Key-Value cache quantization for Inference)** quantizes keys and values asymmetrically based on their sensitivity:
...
**Integration with PagedAttention**: Quantized KV cache requires quantization-aware block management. With FP8 KV cache, the per-sequence memory drops from 2.5MB to 1.25MB per 1K tokens, doubling maximum concurrent sequences.
...
**Grouped-query attention (GQA)**[^fn-gqa-cache-tradeoff] [@ainslie2023gqa]: Architectural change that reduces KV cache by sharing key-value heads:

[^fn-gqa-cache-tradeoff]: **GQA (Grouped-Query Attention)**: Interpolates between MHA (one KV head per query head) and MQA (one KV head total). GQA with 8 KV heads for 64 query heads achieves 8$\times$ KV cache reduction versus MHA while preserving model quality, unlike MQA which can degrade accuracy. For serving, this 8$\times$ reduction directly multiplies feasible batch size on the same GPU, making GQA the dominant architectural choice for inference-optimized LLMs like Llama 2 and Mistral. \index{GQA!cache trade-off}
...
1.  **Significance (Quantitative):** It breaks the sequential bottleneck of autoregressive generation, reducing **Wall-Clock Latency ($L_{lat}$)** by a factor of $2\times$ to $3\times$ depending on the **Acceptance Rate** of the draft tokens. It trades extra compute ($O$) for reduced time.
...
Autoregressive generation is inherently sequential: each token depends on previous tokens. Speculative decoding[^fn-speculative-cpu-analogy] [@leviathan2023speculative; @chen2023accelerating] breaks this bottleneck by using a smaller draft model to predict multiple tokens, then verifying them in parallel with the target model.
...
**Why this works**: The draft model is much smaller (7B vs 70B) and can generate $k$ tokens in the time the target model generates 1 token. Verification is cheap because the target model can process all $k$ tokens in parallel (like prefill). The following *speculative decoding example* traces this process step by step.
...
```{python}
#| echo: false
#| label: speculative-speedup-calc
# ┌─────────────────────────────────────────────────────────────────────────────
# │ SPECULATIVE DECODING SPEEDUP
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Speculative decoding TikZ diagram — prose sentence immediately below
# │
# │ Goal: Compute effective speedup ratio (standard_time / speculative_time).
# │ Show: standard_time, speculative_time, spec_speedup_str in prose.
# │ How: Values mirror TikZ brace annotations; ratio = standard/speculative.
# │
# │ Imports: (none — diagram parameters)
# │ Exports: standard_time, speculative_time, spec_speedup_str
# └─────────────────────────────────────────────────────────────────────────────

# Speculative decoding speedup (values mirror the TikZ brace annotations above)
standard_time = 43  # arbitrary units (ms or tokens)
speculative_time = 30
spec_speedup = standard_time / speculative_time
spec_speedup_str = f"{spec_speedup:.2f}"
```

**Effective speedup**: `{python} standard_time`/`{python} speculative_time` = `{python} spec_speedup_str`x

**Factors affecting speedup**:

| **Acceptance Rate** |              **Speedup** |
|:--------------------|-------------------------:|
| **90% (easy text)** |            2.5–3$\times$ |
| **70% (typical)**   |            1.5–2$\times$ |
| **50% (hard text)** |          1.2–1.5$\times$ |
| **30% (very hard)** | &lt;1$\times$ (overhead) |
...
1.  **Significance (Quantitative):** The **Prefill Phase** (processing the prompt) is **Compute-Bound ($R_{peak}$)** with high arithmetic intensity, while the **Decode Phase** (generating tokens) is **Bandwidth-Bound ($BW$)** with extremely low arithmetic intensity. This mismatch means a single request's efficiency ($\eta$) varies wildly during its lifecycle.
...
3.  **Common Pitfall:** A frequent misconception is that both phases should be batched together naively. In reality, because they have opposing hardware requirements, mixing them in the same batch without **Chunked Prefill** or similar techniques can lead to significant queuing delays ($L_{lat}$) for decoding tokens.
...
**Prefill phase**: Process the entire input prompt in parallel. Computation scales with prompt length. Memory access pattern is compute-bound (high arithmetic intensity[^fn-arithmetic-intensity-serving]).
...
KV Cache management enables a single GPU to handle many concurrent requests without crashing. But what happens when the model itself—like a 175-billion parameter foundation model—is simply too large to fit on a single GPU, let alone its KV Cache? We must split the workload across multiple devices using model sharding for inference.
...
**Minimum sharding**: 2-way (140GB / `{python} h100_mem_gb`GB per H100)
...
class TpSpeedupCalc:
    ...
    # ┌── 3. INVARIANTS (Guardrails) ──────────────────────────────────────────
    assert tp_speedup < tp_theoretical, "Realized speedup must be less than theoretical (communication overhead)"
...
The `{python} tp_speedup_str`$\times$ speedup (vs theoretical `{python} tp_theoretical`$\times$) reflects communication overhead. With `{python} nvlink_h100_gbs` GB/s NVLink bandwidth, each 8MB activation all-reduce takes ~0.3ms.
...
::: {.callout-note title="Archetype B: The Tail at Scale"}
...
- Total capacity: 10$\times$ 1000 + 20$\times$ 600 = `{python} total_cap_str` QPS
...
- H100: 15000$\times$ 0.045 = `{python} h100_load_str` QPS (`{python} h100_util_str`% utilization)
- A100: 15000$\times$ 0.027 = `{python} a100_load_str` QPS (`{python} a100_util_str`% utilization)
...
Efficient routing assumes that our inference servers own the underlying hardware. But in enterprise environments, our critical production models are often running on the exact same physical clusters as experimental models and developer endpoints. To prevent a rogue developer query from crashing our flagship service, we must enforce strict multi-tenancy and resource isolation.
...
**Compute interference**: GPU time-sharing between tenants introduces context-switching overhead and unpredictable latency. *Quantifying noisy neighbor impact* requires measuring these interference effects across all three dimensions simultaneously.
...
In multi-tenant deployments, bulkhead boundaries typically align with service tiers, each with distinct resource guarantees.
...
- **MIG (Multi-Instance GPU)**[^fn-mig-partitioning]: Hardware partitioning of A100/H100 into isolated GPU instances
...
**Model loading isolation**: Loading one model should not evict another from GPU memory. @lst-model-loading-isolation demonstrates priority-aware eviction logic.
...
**Alerting thresholds** should cover SLO violations, quota exhaustion, and noisy neighbor detection, as in @lst-alerting-thresholds.

::: {#lst-alerting-thresholds lst-cap="**Alerting Thresholds**: Per-tenant alert rules for SLO violations, quota exhaustion, and noisy neighbor detection."}
```{.yaml}
alerts:
...
**Provisioning time** ($T_{provision}$): Acquiring a GPU instance takes 30 seconds to several minutes depending on cloud provider and GPU type.[^fn-cold-start-gpu]
...
Warmup typically requires 10-30 dummy inferences, adding 5-30 seconds. A detailed *cold start timeline for Llama-70B* shows how these phases accumulate in practice.
...
**Time-series forecasting** uses historical patterns to predict future demand. @lst-predictive-scaling shows a simplified forecasting function combining daily and weekly seasonality with trend estimation.
...
A 70B model with 4-bit weights requires approximately 35GB, leaving 45GB on an `{python} a100_mem_gb`GB A100 for KV cache. With FP16 KV cache at 2.5MB per 1K tokens per sequence, this supports ~18 concurrent 1K-token sequences versus ~8 with FP16 weights.
...
**Custom TPU infrastructure**: Google runs ranking models on TPUs[^fn-tpu-inference] optimized for transformer inference. TPU pods provide:
...
**Two-tower architecture with caching**[^fn-two-tower-serving]: TikTok separates user understanding (online) from content understanding (offline):
