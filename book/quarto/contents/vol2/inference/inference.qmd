...
: **Training vs. Serving Cost Ratios**: High-QPS applications like recommendation systems show extreme serving cost dominance over training (up to 1000$\times$), whereas lower-volume LLM APIs and analytics exhibit more moderate ratios (10--100$\times$). {#tbl-cost-ratios}
...
The implementation study in \ref{exmp-inference-continuous-batching-vllm} examines how vLLM realizes continuous batching through iteration-level scheduling, paged memory management, preemption, throughput, and GPU utilization.
...
The capacity-management techniques in @sec-inference-scale-pagedattention-3c94 and @sec-inference-scale-prefix-caching-8e8b tolerate the cache; compression shrinks it. The calculation here uses a 64-KV-head multi-head attention baseline so the grouped query attention subsection can isolate the head-count reduction as a separate architectural lever. For a 70B parameter model with 80 layers, 64 heads, head dimension 128, and sequence length 4096 in FP16, the KV cache requires:
...
: **Single vs. Multi-Tenant Tradeoffs**: Multi-tenant deployments double GPU utilization and cut per-request cost but introduce noisy-neighbor risks that demand strict isolation controls. {#tbl-tenancy-comparison}
...
::: {#lst-resource-quotas lst-cap="**Resource Quotas**: Admission control enforcement that evaluates per-tenant limits on concurrency, KV-cache memory allocation, and request rate before admitting requests."}
...
\begin{tikzpicture}[font=\small\sffamily, scale=0.9, transform shape]
  ...
  \node[fill=white, inner sep=2pt, font=\scriptsize\sffamily, align=center] at (8.0, 1.8) {\textbf{Complete Isolation}\\No Shared Resources};
...
The cost savings demonstrated in \ref{exmp-inference-predictive-scaling-traffic} depend on anticipating traffic before it arrives. Proactive provisioning avoids the SLO violations that reactive-only systems suffer during ramp-up periods (@fig-predictive-reactive-scaling).
...
::: {#lst-active-failover lst-cap="**Active-Active Failover**: Global routing implementation that attempts the primary regional endpoint with a strict timeout before falling back to the secondary healthy region."}
...
Search ranking deployments can run ranking models on TPUs[^fn-tpu-inference] optimized for transformer inference. TPU pods provide:
