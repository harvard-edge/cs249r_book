...
The contrast in @fig-continuous-batching-comparison motivates the throughput analysis: static batching leaves GPUs idle for the duration of the longest sequence in each batch, while continuous batching eliminates these idle gaps by inserting new requests at every iteration boundary. Continuous batching's dynamic batch management maintains high GPU utilization regardless of sequence length variance. The throughput improvement depends on sequence length distribution. For a distribution with coefficient of variation $\text{CV} = \sigma / \mu$, the gain is approximately @eq-continuous-batching-gain:
