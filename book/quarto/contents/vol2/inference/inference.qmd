...
- Quantify why serving cost can dominate training cost over a model's lifetime using the total cost of serving equation
...
- Compare model sharding strategies (tensor parallelism, pipeline parallelism, expert parallelism, embedding sharding) by their communication patterns and latency-memory trade-offs
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.memory.capacity, Hardware.Cloud.H100.memory.bandwidth, Hardware.Cloud.A100.memory.bandwidth,
...
[^fn-load-balancing-inference]: **Load Balancing (Inference)**\index{Load Balancing!inference}: GPU inference requests range from 10 ms to 30+ seconds with high variance, unlike web requests that complete in uniform milliseconds. This variance invalidates round-robin and random assignment, forcing queue-depth-aware routing strategies that add monitoring overhead but prevent tail-latency blowups across heterogeneous GPU fleets.
...
Building such a service requires three linked decisions: when distribution becomes necessary, how the architecture preserves latency bounds under varying load, and how resource utilization stays high enough that serving cost does not erode the value the model creates.
...
The perceived responsiveness of interactive systems depends on both **Time to First Token (TTFT)**\index{Time to First Token}[^fn-ttft-tpot] and **Time Per Output Token (TPOT)**\index{Time Per Output Token}.

[^fn-ttft-tpot]: **TTFT vs. TPOT**\index{Time to First Token!vs. time per output token}: **Time to First Token**\index{Time to First Token!responsiveness} (prefill) determines the perceived responsiveness; **Time Per Output Token**\index{Time Per Output Token!reading speed} (decode) determines the perceived reading speed. Humans read at roughly 5--10 tokens/sec; TPOT above roughly 100--200 ms can begin to feel slower than natural reading, while many products target lower TPOT for smooth streaming. This dual service level agreement (SLA) requirement forces schedulers to prioritize decode tokens over new prefill prompts.
...
- [ ] **Shard vs. replicate**: A 13B model (26 GB weights) fits on a single A100. If the throughput requirements double, should you use model sharding or horizontal replication?
- [ ] **Latency floor**: For a real-time speech-to-text service, the primary bottleneck is **inter-token latency**. Does adding more GPUs through data-parallel replication reduce this latency?
- [ ] **Mandatory sharding**: Why is **Model Sharding** mandatory for a 175B model on 80 GB GPUs, regardless of the request volume?
- [ ] **Cross-node overhead**: Identify the trade-off: what distribution overheads might make sharding across machines worse than staying within a single node?
...
# │
# │ Imports: mlsysim public API (DAYS_PER_YEAR, BILLION, units, fmt helpers)
# │ Exports: annual_reqs_calc_math, annual_reqs_b_str, annual_serving_cost_str,
# │          serving_training_ratio_mult_str, efficiency_savings_str,
# │          training_cost_str, dau_str, reqs_per_day_str, cost_per_request_usd_str,
# │          efficiency_gain_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.units (BILLION, SEC_PER_DAY, SEC_PER_YEAR, THOUSAND, MILLION)
...
A common misconception frames inference at scale as synonymous with LLM serving. Large language models present distinctive challenges and attract attention, but they are only one part of production inference volume. Appropriate technique selection requires understanding the full inference landscape. In large consumer platforms, recommendation and ranking workloads\index{Recommendation System!request volume} can account for most inference requests; vision and image processing\index{Vision Models!request volume}, NLP and LLM workloads\index{LLM!request volume}, fraud detection, ads, and other classification tasks fill out the remainder. @Tbl-inference-landscape breaks down these model types, request volumes, and optimization challenges.

Recommendation systems often dominate high-volume consumer inference because they serve predictions for every user interaction. Every page load, scroll, or click triggers inference. A user browsing an e-commerce site might generate 100 recommendation requests in a single session. In contrast, LLM queries typically require explicit user action and occur less frequently.
...
- [ ] **Memory level**: Which level of the hierarchy is responsible for managing GPU memory waste to increase concurrent request capacity?
- [ ] **Speculative decoding level**: If you implement **Speculative Decoding** to reduce token latency for a single request, at which level are you operating?
- [ ] **Load balancing**: Load-balancing algorithms (like Power-of-Two-Choices) operate at the Service level. What is their primary optimization target?
- [ ] **Isolation level**: True or False: Multi-tenancy and resource isolation are primarily Request-level concerns.
...
[^fn-vllm-etymology]: **vLLM (Virtual LLM)**\index{vLLM!etymology}: The name signals its core design: applying OS-style virtual memory to KV cache management, decoupling logical sequence addresses from physical GPU memory. Developed at UC Berkeley (2023), vLLM achieved 2--4$\times$ throughput over FasterTransformer and up to 24$\times$ over HuggingFace Transformers by eliminating the 60--80 percent memory fragmentation that capped batch sizes in prior serving systems.
...
1.  **Significance (quantitative)**: It reduces per-token attention compute from $O(n^2)$ to $O(n)$ in the sequence length, but its footprint grows linearly with both context length and batch size, often consuming more HBM than the model weights themselves and becoming the binding constraint on serving capacity. @Sec-appdx-inference-scale-kv-cache-fundamentals-2ccd derives the fundamental sizing math.
2.  **Observation**: Unlike model weights (which are shared across all requests and statically allocated), the KV cache is request-private and dynamically sized; each concurrent user pays a separate, sequence-length-proportional memory tax that the scheduler must budget for.
3.  **Pitfall**: A frequent misconception is that reducing model size proportionally reduces serving memory. In practice, the KV cache dominates HBM at long contexts, so a quantized 70B model can still be capacity-bound at batch sizes the weights alone would easily admit.
...
- [ ] **LLM vs. vision**: Why is **preemptive scheduling** more critical for LLMs than for vision models?
- [ ] **Stateful workloads**: For which workload type is **stateful serving** (requiring sticky routing) a fundamental requirement?
- [ ] **RecSys standard**: Explain why **feature-parallel batching** is the standard for recommendation systems but not for LLMs.
- [ ] **Quantization impact**: Which dimension (batching, memory, scheduling, topology, state) is primarily affected by the **outlier features** problem in quantization?
...
In large consumer platforms, recommendation systems can constitute the majority of inference requests, with vision, language, ranking, fraud, ads, and other classification workloads sharing the remainder. Despite this distribution, we present batching strategies in order of conceptual complexity: vision (straightforward batching), LLMs (continuous batching with KV cache), and recommendation (feature-parallel batching with distributed embedding). This pedagogical ordering builds understanding progressively, even though practitioners may frequently encounter recommendation workloads first.
...
For recommendation systems\index{Recommendation System}, the bottleneck is often embedding lookup rather than dense computation. Batching strategies must optimize for parallel embedding access patterns rather than matrix multiplication throughput.
...
# │ Show:    Required concurrency, per-replica throughput, replicas needed, total
# │          system concurrency, and the queue shortfall.
...
# │
# │ Imports: mlsysim public API (second, millisecond units, fmt helpers)
# │ Exports: target_throughput_str, latency_slo_ms_str, latency_slo_s_str,
# │          required_concurrency_math, required_concurrency_str,
# │          replica_throughput_calc_math, replica_throughput_str,
# │          replicas_needed_str, batch_size_str, replica_latency_ms_str,
# │          total_concurrency_calc_math, total_concurrency_str, queue_shortfall_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt_time, fmt_math
...
class LittlesLawExample:
    ...
    queue_shortfall_str = fmt_int(queue_shortfall, commas=False)
...
2. Replicas Needed = `{python} LittlesLawExample.target_throughput_str` divided by `{python} LittlesLawExample.replica_throughput_str` = `{python} LittlesLawExample.replicas_needed_str` replicas

**Verification**:
Total system concurrency $=$ `{python} LittlesLawExample.total_concurrency_calc_math` `{python} LittlesLawExample.total_concurrency_str`, short of the `{python} LittlesLawExample.required_concurrency_str` required. The shortfall is the queue: with `{python} LittlesLawExample.replicas_needed_str` replicas actively processing `{python} LittlesLawExample.total_concurrency_str` requests, the remaining `{python} LittlesLawExample.queue_shortfall_str` wait in queues, and that queue wait must fit inside the SLO.
...
# │ How:     Utilization ρ = λ·T_svc; expected batch E[B] = λ·T_window; per-request
# │          compute = inference_time / batch; capacity gain and utilization drop.
# │
# │ Imports: mlsysim public API (units, fmt helpers)
# │ Exports: arrival_qps_str, replicas_str, per_replica_qps_str, latency_slo_ms_str,
# │          svc_b1_ms_str, svc_b32_ms_str, rho_a_eq_math, eb_b_eq_math, eb_c_eq_math,
# │          per_req_b_ms_str, per_req_c_ms_str, rho_b_eq_math, rho_c_eq_math,
# │          bmax_b_str, twindow_b_ms_str, bmax_c_str, twindow_c_ms_str,
# │          inf_b_ms_str, inf_c_ms_str, eb_b_str, eb_c_str,
# │          capacity_gain_pct_str, util_drop_pct_str, lat_c_ms_str, lat_b_ms_str
# └─────────────────────────────────────────────────────────────────────────────
...
class DynamicBatchingResNet50:
    ...
    svc_b32_ms = 25  # scenario assumption: per-image time at batch=32
    ...
    rho_a_eq_math = fmt_math(
        rf"\rho = \lambda_{{\text{{arr}}}} \times T_{{\text{{svc}}}} "
    ...
    rho_b_eq_math = fmt_math(rf"\rho = {int(per_replica_qps)} \times {round(per_req_b_ms / 1000, 4)} " rf"= {fmt_ratio(rho_b, precision=1)}")
    rho_c_eq_math = fmt_math(rf"\rho = {int(per_replica_qps)} \times {round(per_req_c_ms / 1000, 4)} " rf"= {fmt_ratio(rho_c, precision=1)}")
...
- **Arrival rate**: `{python} DynamicBatchingResNet50.arrival_qps_str`
- **Latency SLO**: `{python} DynamicBatchingResNet50.latency_slo_ms_str` P99
- **Per-image inference time**: `{python} DynamicBatchingResNet50.svc_b1_ms_str` at batch=1, `{python} DynamicBatchingResNet50.svc_b32_ms_str` at batch=32
- **Number of replicas**: `{python} DynamicBatchingResNet50.replicas_str` (each handling `{python} DynamicBatchingResNet50.per_replica_qps_str`)

For a single replica with Poisson arrivals at $\lambda_{\text{arr}} =$ `{python} DynamicBatchingResNet50.per_replica_qps_str`:

**Option A: No batching (batch=1)**

- Service time: `{python} DynamicBatchingResNet50.svc_b1_ms_str` per request
- Utilization: `{python} DynamicBatchingResNet50.rho_a_eq_math` (impossible, system is overloaded)

This configuration *cannot* meet demand. Batching is required.

**Option B: Dynamic batching** with $B_{\text{max}}=$ `{python} DynamicBatchingResNet50.bmax_b_str`, $T_{\text{window}}=$ `{python} DynamicBatchingResNet50.twindow_b_ms_str`

Expected requests per window: `{python} DynamicBatchingResNet50.eb_b_eq_math`

With `{python} DynamicBatchingResNet50.eb_b_str` requests per batch:

- Inference time: approximately `{python} DynamicBatchingResNet50.inf_b_ms_str` (interpolating between batch=1 and batch=32)
- Per-request compute: `{python} DynamicBatchingResNet50.inf_b_ms_str`/`{python} DynamicBatchingResNet50.eb_b_str` = `{python} DynamicBatchingResNet50.per_req_b_ms_str`
- Maximum batch delay: `{python} DynamicBatchingResNet50.twindow_b_ms_str`
- Expected total latency: ~`{python} DynamicBatchingResNet50.lat_b_ms_str` mean, ~30 ms P99

Utilization: `{python} DynamicBatchingResNet50.rho_b_eq_math` (sustainable)

**Option C: Dynamic batching** with $B_{\text{max}}=$ `{python} DynamicBatchingResNet50.bmax_c_str`, $T_{\text{window}}=$ `{python} DynamicBatchingResNet50.twindow_c_ms_str`

Expected requests per window: `{python} DynamicBatchingResNet50.eb_c_eq_math`

With `{python} DynamicBatchingResNet50.eb_c_str` requests per batch:

- Inference time: approximately `{python} DynamicBatchingResNet50.inf_c_ms_str`
- Per-request compute: `{python} DynamicBatchingResNet50.inf_c_ms_str`/`{python} DynamicBatchingResNet50.eb_c_str` = `{python} DynamicBatchingResNet50.per_req_c_ms_str`
- Maximum batch delay: `{python} DynamicBatchingResNet50.twindow_c_ms_str`
- Expected total latency: ~`{python} DynamicBatchingResNet50.lat_c_ms_str` mean, ~42 ms P99

Utilization: `{python} DynamicBatchingResNet50.rho_c_eq_math` (comfortable)

**Trade-off**: Option C achieves about `{python} DynamicBatchingResNet50.capacity_gain_pct_str` higher per-replica capacity, or `{python} DynamicBatchingResNet50.util_drop_pct_str` lower utilization, at the cost of higher average latency (`{python} DynamicBatchingResNet50.lat_c_ms_str` vs. `{python} DynamicBatchingResNet50.lat_b_ms_str`). Both meet the `{python} DynamicBatchingResNet50.latency_slo_ms_str` P99 SLO.
...
2.  **Observation**: Unlike static or dynamic batching (which group requests at the request level), continuous batching operates at the Iteration Level\index{Continuous Batching!iteration-level scheduling}, dynamically reshaping the compute tensor at each clock cycle.
3.  **Pitfall**: A frequent misconception is that continuous batching is "purely a scheduler change." In reality, it requires a Dynamic Memory Manager (like PagedAttention) because the KV caches for different requests grow and shrink at different rates, preventing static memory preallocation.
...
The analytical gains translate directly into production systems. The following implementation study examines how vLLM realizes continuous batching through iteration-level scheduling, paged memory management, preemption, throughput, and GPU utilization\index{GPU Utilization}.
...
# │   BatchingWorkedExample.total_iterations_str, BatchingWorkedExample.max_output_str,
# │   BatchingWorkedExample.decode_time_ms_str, BatchingWorkedExample.mean_output_eq_math,
# │   BatchingWorkedExample.total_batch_time_calc_math,
# │   BatchingWorkedExample.total_seq_iters_calc_math, BatchingWorkedExample.waste_eq_math,
# │   BatchingWorkedExample.r{1,2,3,4}_lat_calc_math
# └─────────────────────────────────────────────────────────────────────────────
...
class BatchingWorkedExample:
    ...
    waste_eq_math = fmt_math(
        rf"W = 1 - \frac{{{int(mean_output)}}}{{{max_output}}} = 1 - {waste_ratio} = "
        + str(fmt_percent(latency_reduction_pct / 100, precision=1, commas=False, style="symbol"))
    )
...
The result is a `{python} BatchingWorkedExample.latency_red_str` reduction in average latency, exactly matching the waste ratio and realizing the slot-reuse pattern of @fig-continuous-batching-comparison.
...
# │ Exports: ContinuousBatchingBenefit.*_cv_str, *_tokens_k_str,
# │          *_waste_pct_str, *_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt, fmt_percent, fmt_count
...
class ContinuousBatchingBenefit:
    ...
    code_tokens_k_str = fmt_count(code_k * THOUSAND, scale="K", precision=1, commas=False)
    ...
    chat_tokens_k_str = fmt_count(chat_k * THOUSAND, scale="K", precision=0, commas=False)
    ...
    text_tokens_k_str = fmt_count(text_k * THOUSAND, scale="K", precision=1, commas=False)
    ...
    creative_tokens_k_str = fmt_count(creative_k * THOUSAND, scale="K", precision=0, commas=False)
    ...
    rag_tokens_k_str = fmt_count(rag_k * THOUSAND, scale="K", precision=1, commas=False)
...
|:----------------------------|:----------------------------------------------------:|:----------------------------------------------------------:|:-----------------------------------------------------------:|:--------------------------------------------------------------:|
| **Code completion**         |   `{python} ContinuousBatchingBenefit.code_cv_str`   |   `{python} ContinuousBatchingBenefit.code_tokens_k_str`   |   `{python} ContinuousBatchingBenefit.code_waste_pct_str`   |   `{python} ContinuousBatchingBenefit.code_speedup_mult_str`   |
| **Chat (short responses)**  |   `{python} ContinuousBatchingBenefit.chat_cv_str`   |   `{python} ContinuousBatchingBenefit.chat_tokens_k_str`   |   `{python} ContinuousBatchingBenefit.chat_waste_pct_str`   |   `{python} ContinuousBatchingBenefit.chat_speedup_mult_str`   |
| **General text generation** |   `{python} ContinuousBatchingBenefit.text_cv_str`   |   `{python} ContinuousBatchingBenefit.text_tokens_k_str`   |   `{python} ContinuousBatchingBenefit.text_waste_pct_str`   |   `{python} ContinuousBatchingBenefit.text_speedup_mult_str`   |
| **Creative writing**        | `{python} ContinuousBatchingBenefit.creative_cv_str` | `{python} ContinuousBatchingBenefit.creative_tokens_k_str` | `{python} ContinuousBatchingBenefit.creative_waste_pct_str` | `{python} ContinuousBatchingBenefit.creative_speedup_mult_str` |
| **RAG with variable docs**  |   `{python} ContinuousBatchingBenefit.rag_cv_str`    |   `{python} ContinuousBatchingBenefit.rag_tokens_k_str`    |    `{python} ContinuousBatchingBenefit.rag_waste_pct_str`   |   `{python} ContinuousBatchingBenefit.rag_speedup_mult_str`    |
...
The systems pattern is straightforward: continuous batching is most valuable under four conditions:
...
Conversely, continuous batching provides minimal benefit under three conditions:
...
# │          A100HardwareScenario.* latency and batching scenario strings
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
**Scenario**: An LLM serving system exhibits unexpectedly high tail latency. P50 latency is `{python} A100HardwareScenario.p50_latency_ms_str` and P95 is `{python} A100HardwareScenario.p95_latency_ms_str`, both within SLO, but P99 spikes to `{python} A100HardwareScenario.p99_latency_ms_str` against a `{python} A100HardwareScenario.p99_target_ms_str` target. GPU utilization appears healthy at `{python} A100HardwareScenario.gpu_utilization_pct_str`.

**Setup**: The system runs on `{python} A100HardwareScenario.num_gpus_str` A100-`{python} A100HardwareScenario.a100_mem_gb_str` GPUs connected via PCIe Gen4 (`{python} A100HardwareScenario.pcie_gen4_gb_per_s_str` per GPU) rather than NVLink. Memory bandwidth per GPU is `{python} A100HardwareScenario.a100_mem_bw_tb_per_s_str` (HBM2e), adequate for decode operations, but PCIe limits tensor-parallel communication. For a batch requiring `{python} A100HardwareScenario.activation_transfer_mb_str` activation transfers between GPUs, raw bandwidth math gives approximately `{python} A100HardwareScenario.pcie_transfer_ms_str` per synchronization point over PCIe versus `{python} A100HardwareScenario.nvlink_transfer_ms_str` over NVLink. The scheduling policy is the real problem: dynamic request-level batching uses max batch size `{python} A100HardwareScenario.max_batch_size_str` and timeout `{python} A100HardwareScenario.batch_timeout_ms_str`, and the largest `{python} A100HardwareScenario.affected_batch_share_pct_str` of batches experience head-of-line blocking because FIFO batching holds short requests behind long-sequence requests.
...
1. **Request classification**\index{Request Classification}: Tag requests by expected output length (short: under `{python} A100HardwareScenario.short_token_threshold_str` tokens, medium: `{python} A100HardwareScenario.short_token_threshold_str` to `{python} A100HardwareScenario.medium_token_threshold_str` tokens, long: over `{python} A100HardwareScenario.medium_token_threshold_str` tokens) based on prompt patterns or user-provided hints
2. **Differentiated batching**\index{Batching!differentiated}: Limit short-request batches to `{python} A100HardwareScenario.short_batch_size_str`, medium to `{python} A100HardwareScenario.medium_batch_size_str`, long to `{python} A100HardwareScenario.long_batch_size_str`
...
After implementing these changes, P99 dropped to `{python} A100HardwareScenario.p99_after_ms_str`. High average GPU utilization can mask head-of-line blocking that affects only a small percentage of requests but drives P99; @sec-inference-scale-multitenancy-isolation-688c develops the full isolation framework for keeping one workload's tail from degrading another.
...
# │
# │ Imports: mlsysim
# │ Exports: RecsysLatencyBreakdown.total_ms_str plus callout scale strings.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import *
...
class RecsysLatencyBreakdown:
    ...
    candidate_items_str = fmt_count(candidate_items, precision=0, commas=False, label="item")
    lookups_per_item_str = fmt_count(lookups_per_item, precision=0, commas=False, label="embedding lookup")
    lookups_per_request_str = fmt_count(lookups_per_request, precision=0, commas=True, label="embedding lookup")
    embedding_table_size_tb_str = fmt_memory(embedding_table_size, unit=TB, precision=0, commas=False)
    ...
    requests_per_batch_str = fmt_count(requests_per_batch, precision=0, commas=True, label="request")
...
Consider Meta's recommendation infrastructure serving `{python} RecsysLatencyBreakdown.platform_qps_str` across the platform:

**Request characteristics**:

- Each request queries approximately `{python} RecsysLatencyBreakdown.candidate_items_str` (candidate ranking)
- Each item requires `{python} RecsysLatencyBreakdown.lookups_per_item_str` (user features, item features, cross features)
- Total per request: `{python} RecsysLatencyBreakdown.lookups_per_request_str`
- Embedding table size: `{python} RecsysLatencyBreakdown.embedding_table_size_tb_str` across `{python} RecsysLatencyBreakdown.embedding_shards_str`

**Batching strategy**:

With `{python} RecsysLatencyBreakdown.platform_qps_str` and `{python} RecsysLatencyBreakdown.embedding_shards_str`, each shard receives:

$$\text{Lookups per shard} = \frac{\text{QPS} \times \text{lookups/request}}{\text{shards}}$$

In this scenario, the rate is `{python} RecsysLatencyBreakdown.lookups_per_shard_per_second_million_str` per second per shard.

Single-threaded processing cannot sustain that rate. Instead, the system uses a batch accumulation window\index{Batching!batch, accumulation window} of `{python} RecsysLatencyBreakdown.accumulation_window_ms_str`, which yields batches of `{python} RecsysLatencyBreakdown.requests_per_batch_str` at `{python} RecsysLatencyBreakdown.platform_qps_str` and about `{python} RecsysLatencyBreakdown.lookups_per_shard_per_batch_k_str` per shard per batch.

Each embedding shard processes `{python} RecsysLatencyBreakdown.lookups_per_shard_per_batch_k_str` in a batched operation, achieving high memory bandwidth utilization through sequential memory access patterns.

As @tbl-inference-recsys-latency-breakdown shows, the per-phase latency\index{Latency!breakdown} composition includes request routing, batch accumulation, embedding lookup\index{Embedding Lookup}, feature processing, and ranking:
...
: **Latency breakdown for a recommender request**: Per-phase contribution to end-to-end latency for a single recommender request at the request volume and shard count in the batching example. {#tbl-inference-recsys-latency-breakdown}
...
@Tbl-inference-streaming-speech-pipeline traces the per-stage latency\index{Pipeline!stages}, including feature extraction\index{Feature Extraction}, that the pipeline must hit to stay within the 100 ms budget:
...
- [ ] **Static vs. continuous**: A vision service has highly uniform input sizes and predictable traffic. Which is more appropriate: **static batching** or **continuous batching**?
- [ ] **Waste ratio**: For an LLM serving chat requests, why does the "Waste Ratio" ($1 - \bar{S}/S_{\text{max}}$) collapse to zero when moving from static to continuous batching?
...
- [ ] **RecSys bottleneck**: In **feature-parallel batching** for RecSys, which component of the fleet stack acts as the primary bottleneck: the GPU's Tensor Cores or the distributed Embedding Store's memory bandwidth?
- [ ] **KV-cache budget**: For a 7B model serving 2,048-token contexts, estimate the KV-cache memory per request and roughly how many concurrent requests fit in 40 GB of free HBM, and why that capacity, not compute, usually caps the achievable batch size.
...
As @fig-inference-lifecycle shows, noncompute stages (serialization, queuing, routing) can consume a substantial fraction of the latency budget, meaning batching strategy selection must account for the full pipeline, not GPU execution time alone. A decision tree guides strategy selection.
...
# │          KVCacheCapacityEstimator.context_len_str,
...
class KVCacheCapacityEstimator:
    ...
    context_len_str = fmt_count(context_tokens, commas=True)
...
1.  **Significance (quantitative)**: It eliminates Internal and External Fragmentation\index{Internal and External Fragmentation}, which can waste 60--80 percent of the memory allocated to the KV cache under contiguous preallocation. By allowing sequences to grow dynamically, it enables 2--4$\times$ higher Concurrent Throughput $(X)$ on the same hardware.
2.  **Observation**: Unlike Contiguous Allocation (which requires prereserving the maximum context length), PagedAttention uses a Block Table to map logical sequence indices to physical memory pages, allocating only what is currently used.
3.  **Pitfall**: A frequent misconception is that PagedAttention speeds up *individual token math*. In reality, it is a Capacity Optimization\index{Optimization!capacity}: it improves *system-level efficiency* by allowing larger batch sizes, though it adds a small Indirection Overhead $(L_{\text{lat}})$ for the pointer lookups.
...
PagedAttention [@kwon2023], introduced in vLLM, applies virtual memory concepts to KV cache management. Instead of contiguous allocation\index{Memory Allocation!contiguous}, the KV cache is divided into fixed-size pages (typically 16--256 tokens), and sequences are allocated pages on demand.
...
**Systems insight**: The throughput impact\index{PagedAttention!throughput impact} of gather operations is small compared to the memory savings; @tbl-inference-pagedattention-throughput shows the 2.5--4$\times$ throughput improvement coming from fitting more concurrent sequences in the same memory.
...
# │   PrefixCachingAnalysis.system_prompt_tokens_str,
# │   PrefixCachingAnalysis.concurrent_users_str,
# │   PrefixCachingAnalysis.avg_response_tokens_str,
# │   PrefixCachingAnalysis.tokens_per_user_eq_math,
# │   PrefixCachingAnalysis.kv_no_cache_factors_math,
# │   PrefixCachingAnalysis.total_tokens_cached_eq_math,
# │   PrefixCachingAnalysis.kv_cached_factors_math
# └─────────────────────────────────────────────────────────────────────────────
...
**Result**: Prefix caching yields a `{python} PrefixCachingAnalysis.memory_reduction_pct_str` memory reduction\index{Prefix Caching!memory savings} in KV cache, enabling `{python} PrefixCachingAnalysis.concurrent_mult_str` more concurrent users.

**Systems insight**: Prefix caching pays off when the prefix hit rate\index{Prefix Caching!hit rate} is high because many requests share a long prefix. @Tbl-inference-prefix-caching-workloads contrasts hit rates and memory savings across workloads, showing where the technique changes serving capacity and where it does not.
...
class KVCacheCalc:
    ...
    kv_fp16_formula_math = MarkdownStr(
        rf"2 \times {kv_layers} \times {kv_heads} \times {kv_head_dim} \times {kv_seq_len} \times 2 " rf"\approx {kv_fp16.to(GB).magnitude:.1f}"
    )
...
@Sec-performance-engineering-speculative established speculative decoding as an algorithmic optimization: a small draft model proposes $K$ tokens, the target model verifies them in one parallel forward pass, and rejection sampling guarantees the emitted tokens follow the target model's distribution exactly, so the technique is lossless and bounded by the draft acceptance rate $\alpha_{\text{acc}}$. The serving question is different. The mechanism is a knob the scheduler turns per endpoint, and turning it on consumes resources that the rest of the fleet is already competing for. This section treats speculation as a serving policy: when the scheduler should enable it, how its memory cost interacts with KV-cache admission, and what it does to the SLA the endpoint is bound by.
...
The first concern is memory. The draft model is not free state: it requires its own GPU allocation for weights, and each in-flight request that uses speculation reserves a small additional KV cache for the draft model alongside the target's. Pairing a 7B draft model with a 70B target adds roughly 14 GB of weights per replica plus per-request draft cache. This *speculation tax*\index{Speculative Decoding!speculation tax} competes directly with the KV-cache admission budget developed in @sec-inference-scale-kv-cache-wall: every gigabyte the draft model holds is a gigabyte the admission controller cannot allocate to a concurrent request. Speculation therefore shrinks the maximum batch size the node can admit, trading aggregate throughput for the latency of individual requests. The admission controller and the speculation policy cannot be tuned independently; enabling speculation lowers the concurrency ceiling that the KV-cache wall already imposes.
...
@Sec-inference-scale-model-sharding-inference-82b3 develops the tensor and pipeline parallelism strategies that distribute a model across devices; here we note the consequence each one carries for KV-cache management:
...
2.  **Observation**: Unlike Single-Pass Inference (for example, ImageNet), where the resource bottleneck is constant, LLM inference switches between these regimes at every request, requiring Iteration-Level Scheduling to maintain utilization.
3.  **Pitfall**: A frequent misconception is that both phases should be batched together naively. In reality, because they have opposing hardware requirements, mixing them in the same batch without Chunked Prefill or similar techniques can lead to significant queuing delays $(L_{\text{lat}})$ for decoding tokens.

:::

The prefill phase processes the entire input prompt in parallel. Computation scales with prompt length, and the memory access pattern\index{Memory Access} is compute-bound with high arithmetic intensity[^fn-arithmetic-intensity-serving].

[^fn-arithmetic-intensity-serving]: **Arithmetic Intensity (Prefill vs. Decode)**\index{Arithmetic Intensity!serving}: Measured in FLOP/byte of memory accessed, this ratio determines whether a workload is compute-bound or bandwidth-bound. Prefill achieves 100+ FLOP/byte (compute-bound); decode achieves 1--10 FLOP/byte (bandwidth-bound). This 10--100$\times$ gap is why mixing the two phases in a single batch wastes either compute or bandwidth, motivating disaggregated serving architectures.
...
**Chunked prefill**\index{Chunked Prefill!latency}[^fn-chunked-prefill-latency] addresses this by breaking long prompts into fixed-size chunks that interleave with decode operations:
...
A 70-billion-parameter model requires over 140 GB of VRAM just to hold its weights in 16-bit precision, making it physically impossible to load onto a single 80 GB GPU. To serve this model, we must slice its architecture across multiple GPUs that act together as a single logical replica. Adapting distributed training parallelism techniques, specifically tensor and pipeline parallelism, for low-latency inference introduces a distinct set of constraints.
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.memory.capacity, GiB)
...
# │   TpSpeedupCalc.mem_70b_eq_math, TpSpeedupCalc.{att,ff}_{seq,tp}_ms_str,
# │   TpSpeedupCalc.allreduce_tp_ms_str, TpSpeedupCalc.{att,ff}_speedup_mult_str,
# │   TpSpeedupCalc.seq_per_layer_ms_str, TpSpeedupCalc.tp8_per_layer_ms_str,
# │   TpSpeedupCalc.h100_mem_gb_str
# └─────────────────────────────────────────────────────────────────────────────
...
class TpSpeedupCalc:
    ...
    mem_70b_eq_math = fmt_math(
        rf"\text{{Memory}}_{{\text{{70B}}}} = {int(parameters_b)} \times 10^9 \times " rf"{bytes_per_element} = {int(weight_fp16_gb_val)}\text{{ GB}}"
    )
...
- Parameters: `{python} TpSpeedupCalc.parameters_b_str`
...
`{python} TpSpeedupCalc.mem_70b_eq_math`

**Minimum sharding**: 2-way (`{python} TpSpeedupCalc.weight_fp16_gb_str`/`{python} TpSpeedupCalc.h100_mem_gb_str` per H100)
...
|:--------------------------|:-------------------------------------------------:|:-------------------------------------------------:|:------------------------------------------------:|
| **Attention compute**     |      `{python} TpSpeedupCalc.att_seq_ms_str`      |       `{python} TpSpeedupCalc.att_tp_ms_str`      |  `{python} TpSpeedupCalc.att_speedup_mult_str`   |
| **AllReduce (attention)** |                        0 ms                       |    `{python} TpSpeedupCalc.allreduce_tp_ms_str`   |                       N/A                        |
| **Feed-forward compute**  |       `{python} TpSpeedupCalc.ff_seq_ms_str`      |       `{python} TpSpeedupCalc.ff_tp_ms_str`       |   `{python} TpSpeedupCalc.ff_speedup_mult_str`   |
| **AllReduce (FF)**        |                        0 ms                       |    `{python} TpSpeedupCalc.allreduce_tp_ms_str`   |                       N/A                        |
| **Total per layer**       | **`{python} TpSpeedupCalc.seq_per_layer_ms_str`** | **`{python} TpSpeedupCalc.tp8_per_layer_ms_str`** | **`{python} TpSpeedupCalc.tp_speedup_mult_str`** |
...
: **Pipeline vs. Tensor Parallelism**: Each strategy has distinct trade-offs in latency, throughput, and implementation complexity. {#tbl-pipeline-tensor-comparison}
...
This AllToAll pattern creates a critical system challenge: load balancing. If the router consistently sends more tokens to certain experts than others, those experts' GPUs become bottlenecks while other GPUs sit idle. Three mechanisms address load imbalance:
...
Router behavior directly impacts both system performance and model quality. Some failures are training-shaped problems that become visible during serving, while others are runtime overload conditions the serving system must mitigate directly:

-   **Expert collapse**\index{Expert Collapse}: A training-time router failure in which the router converges to a small subset of experts, leaving others undertrained. The served model then behaves like a small dense model with a massive memory footprint.
-   **Routing instability**\index{Routing Instability}: A training-time failure in which assignments oscillate, preventing experts from specialized learning.
-   **Distribution shift**\index{Distribution Shift}: A serving-time workload mismatch in which a router trained on formal text may overload specific "grammar experts" when served colloquial chat data, creating hotspots.
-   **Token dropping**\index{Token Dropping}: A serving-time overload behavior in which capacity-limited experts drop tokens, skipping their computation and relying solely on the residual connection, which degrades output quality.
...
[^fn-moe-serving]: **MoE (Mixture-of-Experts) Serving Trade-Off**\index{MoE!serving}: Only a subset of experts activate per token (typically 2 of 8--16), keeping per-token FLOPs low while total parameter count reaches hundreds of billions. The serving constraint: all expert weights must reside in memory even though most are idle on any given token, inflating memory footprint 4--8$\times$ relative to the active compute. This forces expert parallelism across devices with AllToAll communication at every MoE layer.
...
# │
# │ Imports: mlsysim.core.units (BYTES_INT8, byte), mlsysim.fmt (check, fmt)
...
# │
# │ Imports: mlsysim.core.units (Systems.Nodes.DGX_H100.accelerators_per_node,
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @tbl-inference-interconnect-specs inside
# │          @nbk-inference-interconnect-technology-comparison.
# │
...
# │
# │ Imports: mlsysim registries (Hardware, Systems.Fabrics) + fmt helpers
# │ Exports: InterconnectSpecs.* (_str per table cell)
# └─────────────────────────────────────────────────────────────────────────────
...
Communication overhead depends heavily on the interconnect technology. @Tbl-inference-interconnect-specs records the physical bandwidth\index{Interconnect!bandwidth}, latency\index{Interconnect!latency}, and intended scope of each candidate interconnect:
...
[^fn-balls-bins-routing]: **Balls-into-Bins (Power of Two Choices)**\index{Power of Two Choices!routing}: With random placement, maximum bin load is $\Theta(\log R / \log \log R)$; with two random choices and greedy selection, it drops to $\Theta(\log \log R)$. For a 1,000-server inference fleet, this reduces worst-case queue depth from roughly 5 to roughly 2, cutting P99 tail latency by nearly half while adding only the overhead of a single extra queue-length probe per request.

The procedure is deliberately small, but the load signal must match the serving workload. For a homogeneous vision fleet, current queue length may be enough. For an LLM fleet, active tokens, estimated remaining decode work, and KV-cache pressure better approximate the work a replica has already accepted. @Algo-power-two-choices-routing states a representative routing loop: probe two replicas, compare normalized work, and route without polling the whole fleet. @Eq-two-choices-max-queue formalizes why this small change matters, reducing maximum queue length from $\mathcal{O}(\log R / \log \log R)$ to $\mathcal{O}(\log \log R)$:
...
- Total capacity is `{python} HeterogeneousGpuCluster.total_cap_qps_str`; target traffic is `{python} HeterogeneousGpuCluster.target_qps_str`.

At target traffic of `{python} HeterogeneousGpuCluster.target_qps_str`, weighted assignment\index{Weighted Assignment} gives each H100 a weight of `{python} HeterogeneousGpuCluster.h100_weight_pct_str` and each A100 a weight of `{python} HeterogeneousGpuCluster.a100_weight_pct_str`. The expected per-server load\index{Expected Load Per Server} becomes `{python} HeterogeneousGpuCluster.h100_load_qps_str` for H100s and `{python} HeterogeneousGpuCluster.a100_load_qps_str` for A100s, leaving the two server types at `{python} HeterogeneousGpuCluster.h100_util_pct_str` and `{python} HeterogeneousGpuCluster.a100_util_pct_str` utilization.
...
Many inference workloads maintain state that benefits from routing affinity\index{Routing Affinity}. LLM conversations\index{LLM!conversations} reuse KV cache from previous turns, recommendation sessions\index{Recommendation Sessions} carry user context and recent interactions, and streaming inference\index{Inference!streaming} may depend on model state from previous frames. For these workloads, routing the same user or session to the same server improves performance by avoiding cache misses and state reconstruction.
...
The important properties are determinism\index{Consistent Hashing!determinism}, minimal disruption\index{Consistent Hashing!minimal disruption}, and balance. The same key always routes to the same server, adding or removing servers remaps only $K/N_{\text{servers}}$ keys on average, and virtual nodes distribute load evenly enough to prevent one physical server from owning a disproportionate key range. For LLM serving, the most direct payoff is avoiding unnecessary KV-cache rebuilds across conversation turns.
...
**Implementation with virtual nodes**\index{Consistent Hashing!virtual nodes}:
...
**Handling server failures**\index{Consistent Hashing!failure handling}: When Server A fails, its 100 virtual nodes drop from the ring and the affected requests reroute to the next server clockwise, so only $\sim 1/N_{\text{servers}}$ of all requests are touched.
...
The progression shows clear trade-offs:
...
A saturated LLM fleet under KV cache pressure has relief valves that generic backpressure cannot provide: the serving system can evict older conversational turns from the KV cache to reduce context window and free memory, disable speculative decoding to reclaim the GPU memory its draft model occupies, or route new requests to a smaller quantized fallback model until the primary queue drains. Each of these responses trades output quality or latency for continued availability, which is often preferable to rejection from the user's perspective. The system designer must specify which degradation modes are acceptable at each pressure tier, because the correct trade-off depends on the application—a real-time assistant tolerates context truncation better than silent quality degradation.
...
# │
# │ Imports: mlsysim.fmt (check, fmt, MarkdownStr)
# │ Exports: NoisyNeighborImpact.*_str, NoisyNeighborQuotaScenario.*_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt, MarkdownStr, fmt_percent, fmt_rate, fmt_time

class NoisyNeighborImpact:
    """Shared baseline for the unprotected-pool noisy-neighbor walkthrough."""
    ...
    # Bare numbers for table cells whose column header already reads "QPS"
    tenant_base_qps_num_str = fmt_int(tenant_base_qps, commas=False)
    tenant_burst_qps_num_str = fmt_int(tenant_burst_qps, commas=False)
...
class NoisyNeighborQuotaScenario:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    slo_ms = NoisyNeighborImpact.slo_ms
    ...
    protected_p99_ms_str = fmt_time(protected_p99_ms, 'millisecond', precision=0, commas=False)
    burst_p99_ms_str = fmt_time(burst_p99_ms, 'millisecond', precision=0, commas=False)
...
- Each tenant: `{python} NoisyNeighborImpact.tenant_base_qps_str`, `{python} NoisyNeighborImpact.slo_ms_str` P99 latency
- GPU utilization: `{python} NoisyNeighborImpact.gpu_util_pct_str`
- All SLOs met

Tenant 3 bursts to `{python} NoisyNeighborImpact.tenant_burst_qps_str` (5$\times$ baseline). Without per-tenant quotas, the burst cascades into SLO violations for all ten tenants on the shared pool (@tbl-inference-noisy-neighbor-no-isolation):

| **Tenant**       |                           **QPS**                           | **P99 Latency** | **SLO Status** |
|:-----------------|:-----------------------------------------------------------:|:---------------:|:--------------:|
| **Tenant 1**     |    `{python} NoisyNeighborImpact.tenant_base_qps_num_str`   |      25 ms      |    Violated    |
| **Tenant 2**     |    `{python} NoisyNeighborImpact.tenant_base_qps_num_str`   |      28 ms      |    Violated    |
| **Tenant 3**     |   `{python} NoisyNeighborImpact.tenant_burst_qps_num_str`   |      45 ms      |    Violated    |
| **Tenants 4-10** | `{python} NoisyNeighborImpact.tenant_base_qps_num_str` each |     22-30 ms    |    Violated    |

: **Noisy neighbor without isolation (unprotected pool)**: All tenants violate their latency SLO when one tenant bursts. {#tbl-inference-noisy-neighbor-no-isolation tbl-colwidths="[26,20,24,30]"}

:::

::: {#nbk-inference-noisy-neighbor-quotas .callout-notebook title="Noisy neighbor with per-tenant quotas"}

Enabling per-tenant quotas contains the impact to the offending tenant alone (@tbl-inference-noisy-neighbor-quotas):

| **Tenant**       |                              **QPS (actual)**                              |                      **P99 Latency**                       |                             **SLO Status**                             |
|:-----------------|:--------------------------------------------------------------------------:|:----------------------------------------------------------:|:----------------------------------------------------------------------:|
| **Tenant 1**     |           `{python} NoisyNeighborImpact.tenant_base_qps_num_str`           | `{python} NoisyNeighborQuotaScenario.protected_p99_ms_str` |       `{python} NoisyNeighborQuotaScenario.protected_status_str`       |
| **Tenant 2**     |           `{python} NoisyNeighborImpact.tenant_base_qps_num_str`           | `{python} NoisyNeighborQuotaScenario.protected_p99_ms_str` |       `{python} NoisyNeighborQuotaScenario.protected_status_str`       |
...
| **Tenants 4-10** |        `{python} NoisyNeighborImpact.tenant_base_qps_num_str` each         | `{python} NoisyNeighborQuotaScenario.protected_p99_ms_str` |       `{python} NoisyNeighborQuotaScenario.protected_status_str`       |
...
Resource quotas\index{Resource Quotas} limit what each tenant can consume, preventing any single tenant from monopolizing shared resources.
...
When total demand exceeds capacity, max-min fairness allocates resources to maximize the minimum allocation across tenants. The algorithm first gives each tenant an equal share, then redistributes unused capacity from tenants whose demand falls below their equal share. Raw request counts are an inadequate unit for this calculation because, as @sec-inference-scale-multitenancy-isolation-688c establishes, equal request counts are not equal load—a tenant issuing long-context summarization requests consumes far more KV cache and decode compute than a tenant issuing short classification queries. Token generation rate\index{Token Generation Rate} (tokens/sec) serves as the appropriate bounding metric, capturing both throughput and memory pressure. For three tenants with demands of 30,000, 20,000, and 80,000 tokens/sec competing for a GPU pool capable of 100,000 tokens/sec, max-min fairness yields allocations of 30,000, 20,000, and 50,000—each tenant receives up to its demand or its fair share, whichever is less, with excess capacity redistributed proportionally.
...
[^fn-mig-partitioning]: **MIG (Multi-Instance GPU)**\index{MIG!partitioning}: Introduced with NVIDIA's A100 [@NVIDIA2020], MIG spatially partitions a single GPU into up to 7 isolated instances, each with dedicated compute SMs, memory bandwidth, and L2 cache. Unlike time-slicing, which shares resources and introduces latency variance of 2--5$\times$, MIG provides hardware-enforced isolation with predictable performance, making it a strong mechanism for serving multiple small models on a single expensive data center GPU when partition sizes match model requirements.

Model loading\index{Model Loading!isolation} presents a subtler isolation challenge: loading a new model should not evict a running model from GPU memory. @Lst-model-loading-isolation demonstrates priority-aware eviction logic that enforces this constraint.
...
A common approach is horizontal scaling\index{Scaling!horizontal}: adding or removing model replicas to adjust aggregate throughput. Total capacity scales linearly as $\text{Capacity} = \text{Replicas} \times \text{Per-replica throughput}$, but each new replica requires provisioning time measured in minutes.
...
The fastest response comes from batch size scaling\index{Scaling!batch size}, which adjusts how many requests the system processes simultaneously. Increasing batch size trades higher per-request latency for greater throughput, with no provisioning delay. This makes batch size the first knob to turn during a traffic spike, buying time for horizontal scaling to take effect. These scaling dimension trade-offs\index{Scaling!dimension tradeoffs} are summarized in @tbl-scaling-dimensions.
...
[^fn-cold-start-gpu]: **Cold Start (GPU vs. Serverless)**\index{Cold Start!GPU inference}: Serverless functions cold-start in 100 ms--1 s; GPU inference cold-starts in 1--10 minutes due to GPU allocation, model weight transfer, and CUDA context initialization. This 100$\times$ gap means reactive autoscaling alone cannot absorb traffic spikes, making predictive scaling and warm pools essential design requirements for any GPU serving fleet with latency SLOs.
...
@Lst-metric-based-scaling shows a typical metric-based scaling\index{Scaling!metric-based} configuration.
...
Event-driven scaling\index{Scaling!event-driven} scales proactively for known events, as shown in @lst-event-driven-scaling.
...
Each component offers distinct optimization opportunities. Detection speed improves by collecting metrics at 1-second intervals rather than 60-second intervals, at the cost of noisier signals and higher metric volume. Decision speed improves by precomputing scaling plans for predicted scenarios so that when a trigger fires, the system executes a precomputed plan rather than computing one from scratch. Provisioning speed improves most dramatically through warm pools, which eliminate the provisioning phase entirely for anticipated demand. Warmup speed\index{Cold Start!warmup} improves through precompiled inference engines that skip JIT compilation, reducing the final phase from 30 seconds to under 5 seconds.

### Spot and preemptible instances {#sec-inference-scale-spot-preemptible-instances-c969}

Cloud providers may offer discounted GPU instances[^fn-spot-inference] that can be reclaimed with short notice, so the scheduling decision is whether the workload can drain, reroute, or recompute without violating its SLO. @Tbl-gpu-instance-types lays out the three GPU instance classes and the workloads each suits best:

[^fn-spot-inference]: **Spot Instance Economics**\index{Spot Instance Economics}\index{Spot Instances!inference}: Cloud providers have sold unused GPU capacity at large discounts, sometimes with 30-second to 2-minute termination notice. For inference, termination destroys in-flight requests and KV cache state, requiring graceful drain logic and rapid request re-routing. The savings can justify this complexity for burst and best-effort traffic tiers but are unsuitable for SLO-critical paths where termination would violate latency guarantees.
...
@Lst-spot-termination illustrates graceful handling of spot termination\index{Spot Instances!termination handling} within the two-minute warning window.
...
The two paths in @fig-edge-cache-routing highlight the order-of-magnitude latency difference: cache hits return in under 10 ms, while cache misses incur the full backend round-trip. Effectiveness depends on request repeatability across workloads such as autocomplete, FAQ chatbots, open-ended chat, and code generation\index{Code Generation}, as @tbl-edge-cache-hit-rates quantifies:
...
When a region becomes unavailable, traffic must reroute to healthy regions. @Lst-active-failover shows active-active failover\index{Failover!active-active} routing logic that falls back to the next healthy region after timeout or unavailability.
...
Stateful LLM serving makes failover harder than ordinary HTTP routing because the receiving region must absorb both traffic and lost session state:
...
Cost-aware routing\index{Cost-Aware Routing} directs latency-tolerant workloads (batch inference, background processing) to the cheapest available region, as shown in @lst-cost-aware-routing.
...
[^fn-gptq-column-wise]: **GPTQ column-wise update**\index{GPTQ!column-wise update}: GPTQ factors the inverse Hessian $H^{-1}$ via Cholesky decomposition so the compensation becomes a stable triangular update: after rounding column $q$, the residual error is propagated to columns $q{+}1{:}$ weighted by $[H^{-1}][:,q{+}1{:}] / [H^{-1}]_{qq}$. The factorization avoids recomputing a full inverse per column, giving $\mathcal{O}(d_{\text{row}} \cdot d_{\text{col}}^2)$ work instead of the naive $\mathcal{O}(d_{\text{row}} \cdot d_{\text{col}}^3)$. Per-group scaling (group size 128) gives outlier channels a finer step than a single per-matrix scale.
...
@Tbl-quarot-perplexity compares perplexity across quantization methods and precision\index{Precision} choices on LLaMA-2-70B:
...
The quantization method only pays off when the deployment hardware accelerates the chosen precision\index{Precision}. Different accelerators support formats such as BF16\index{BF16}, INT8\index{INT8}, and INT4\index{INT4} with varying performance multipliers.
...
@Tbl-quantization-deployment-configurations summarizes how the major quantization\index{Quantization} configurations map to deployment targets and framework support:
...
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.A100.memory.capacity, BYTES_INT4,
...
# │
# │ Imports: mlsysim public API (fmt helpers)
# │ Exports: cost_fp16_8_str, cost_awq_4_str, cost_awq_2_str
# └─────────────────────────────────────────────────────────────────────────────
...
The central design decision is progressive refinement: rather than running one expensive model on all candidates, Google uses a ranking cascade summarized in @tbl-google-ranking-cascade:
...
Search ranking deployments can run ranking models on Tensor Processing Units (TPUs)[^fn-tpu-inference] optimized for transformer inference. TPU pods provide:

[^fn-tpu-inference]: **TPU for Inference**\index{TPU!inference}: TPU systolic arrays provide deterministic, low-variance matrix execution for ranking-style inference. GPU latency can vary with batch composition, while TPU-style execution can deliver consistent per-request timing for strict search SLOs where P99 latency directly affects revenue.
...
The misconception, reinforced by LLM-centric discourse, leads to over-focus on LLM-specific techniques while ignoring the broader inference landscape. By request volume, recommendation systems can constitute the majority of production inference at large consumer platforms, with vision, language, fraud, ads, and other models comprising the remainder. A practitioner who only understands continuous batching and KV cache management is unprepared for the feature-parallel batching and embedding sharding that dominate high-volume recommendation inference. Technique selection must match the actual workload.
...
# │ How: pulling constants from mlsysim.core.units.
# │
# │ Imports: mlsysim.core.units (Hardware.Cloud.H100.memory.capacity, Hardware.Cloud.H100.memory.bandwidth, GiB, TB, second)
# │ Exports: InferenceBottleneckHardware.h100_mem_gb_str, InferenceBottleneckHardware.h100_mem_bw_tb_per_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
* **The inference wall**: Serving cost can dominate training cost by 100$\times$ or more over a model's lifetime, making inference efficiency a primary driver of ML economics.
...
* **Power of two choices**: Simple, randomized load balancing achieves exponentially better queue balance ($\mathcal{O}(\log \log R)$) with minimal overhead, making it a useful primitive for large-scale deployment.
...
The cost that decides an inference system is not the one that makes the news. A frontier model's training bill is paid once; its serving bill is paid on every request, for as long as the model lives, which is why a single percent of serving throughput can outweigh the entire cost of training it. What makes that percent so hard to win is a physical fact about generation: it is autoregressive, produced one token at a time, and each token re-reads the model's full weights from memory, so decode is bound by bandwidth rather than arithmetic and the accelerator's FLOPs sit half-idle. Almost every technique in this chapter is a way to claw back that idle silicon, because at serving scale the saving is collected a billion times over.
