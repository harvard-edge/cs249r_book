...
In the **Systems Sandwich** (@sec-vol2-introduction), Inference is the **Service Layer**—the interface where the model meets the user. While the **Operational Layer** (training) optimizes for throughput over weeks, the Service Layer must deliver predictions in milliseconds. If the **Iron Law** defines the speed limit of training, it defines the **deadline** of inference: $L_{inference} \le \text{SLO}$.
...
For a batched inference system with batch size $B$, service time becomes a function of batch size. Let $S(B)$ denote the time to process a batch of $B$ requests. From the physics of batching (@sec-inference-scale-batching-strategies-scale-6733-differences), we model this as:
...
Even with modern frameworks, diagnosing tail latency anomalies requires systematic investigation across the system stack. The following case study applies the Systems Sandwich methodology to a real-world P99 latency regression.
...
After computing local attention, an AllReduce operation (covered in @sec-communication-collective-operations) combines results across devices, adding communication overhead proportional to the activation size divided by the interconnect bandwidth.
...
We also saw that efficient serving requires specialized hardware. The "Decode" phase of LLM inference is memory-bandwidth bound, motivating the specialized inference accelerators we examined in @sec-compute.
