...
The engineering goal is to find the maximum $B$ such that $L(B) \le \text{SLO}$. This formulation explains why vision models (high $T_{variable}$) saturate at smaller batches than LLMs (high $T_{fixed}$ due to weight loading), requiring different tuning strategies.
...
The choice of batching strategy depends on model characteristics, traffic patterns, and latency requirements (see @fig-inference-lifecycle). The following decision framework guides selection:

::: {#fig-inference-lifecycle fig-env="figure" fig-pos="htb"}
```{.tikz}
...
The communication pattern for tensor-parallel inference (as shown in @fig-tensor-parallel-flow) follows:

::: {#fig-tensor-parallel-flow fig-env="figure" fig-pos="htb"}
```{.tikz}
...
For inference, pipeline parallelism creates bubbles differently than in training (see @fig-pipeline-bubbles):

::: {#fig-pipeline-bubbles fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
...
**Expert parallelism** (@fig-moe-routing) distributes experts across devices, with each device hosting $E/P$ experts:
...
The choice of embedding sharding strategy depends on lookup patterns and communication overhead (see @fig-embedding-sharding and @tbl-embedding-sharding).
...
::: {#fig-embedding-sharding fig-env="figure" fig-pos="htb"}
```{.tikz}
...
**Prefix caching** (@fig-prefix-caching) shares KV cache entries across requests with common prefixes:

::: {#fig-prefix-caching fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
...
Speculative decoding accelerates generation by using a small model to draft tokens which a larger model verifies in parallel (see @fig-speculative-decoding).

**Scenario**: Generating "The quick brown fox jumps"

::: {#fig-speculative-decoding fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
...
**Deployment-level bulkheads** (@fig-bulkhead-patterns) dedicate replicas to specific tenants or tenant groups.

::: {#fig-bulkhead-patterns fig-env="figure" fig-pos="htb"}
```{.tikz}
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
...
Cold start latency represents a significant challenge for serverless inference, with model loading often dominating the total startup time (see @fig-cold-start-breakdown).
...
::: {#fig-cold-start-breakdown fig-env="figure" fig-pos="htb"}
```{.tikz}
...
However, not all inference happens in the datacenter. For applications requiring ultra-low latency, privacy, or offline operation, we must push intelligence to the edge.

The next chapter, @sec-edge-intelligence, examines how to deploy these massive models on constrained devices, from smartphones to microcontrollers, completing the deployment spectrum.
