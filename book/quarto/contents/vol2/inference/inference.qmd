...
class LittlesLawVerify:
    ...
    expected_total_s_str = f"{expected_total_ms / 1000:.3f}"
...
: **Continuous Batching Throughput on Llama-2 70B (8x A100)**: Tokens-per-second and GPU utilization for static, dynamic, and continuous batching of a Llama-2 70B serving workload on an 8x A100 node. Continuous batching reaches 1,200 tokens/s at 92 percent GPU utilization, roughly 3x the static baseline of 400 tokens/s at 45 percent, by inserting new sequences at every iteration boundary instead of waiting for the longest in-flight sequence to finish. {#tbl-inference-continuous-batching-throughput}
