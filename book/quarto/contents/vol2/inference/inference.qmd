...
from mlsysim.fmt import fmt_int, fmt_usd, check, fmt
...
class ServingCostSimple:
    ...
    efficiency_savings_str = fmt_usd(efficiency_savings/ MILLION, precision=1, commas=False, suffix="M")
    ...
    efficiency_gain_pct_str = fmt_int(efficiency_gain * 100, commas=False, suffix=" percent")
```
...
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt_int, fmt_usd, fmt, check, fmt_math, MarkdownStr
...
class LittlesLawExample:
    ...
    latency_slo_ms_str = fmt_int(latency_slo_s * THOUSAND, commas=False, suffix=" ms")
...
class LogicScaling:
    ...
    time_s_str = fmt(t_reasoning_s, precision=1, commas=False, suffix=" seconds")
...
class ContinuousBatchingBenefit:
    ...
    code_waste_pct_str = fmt(code_waste_pct, precision=1, commas=False, suffix=" percent")
    ...
    chat_waste_pct_str = fmt(chat_waste_pct, precision=1, commas=False, suffix=" percent")
    ...
    text_waste_pct_str = fmt_int(text_waste_pct, commas=False, suffix=" percent")
    ...
    creative_waste_pct_str = fmt(creative_waste_pct, precision=1, commas=False, suffix=" percent")
    ...
    rag_waste_pct_str = fmt(rag_waste_pct, precision=1, commas=False, suffix=" percent")
...
class KVCacheCapacityEstimator:
    ...
    total_hbm_gb = Systems.Nodes.DGX_H100.accelerators_per_node * Hardware.Cloud.H100.memory.capacity.m_as(GiB)
    weight_gb = _model.size_in_bytes(BYTES_FP16).m_as(GB)
    system_gb = 20
...
class KVCacheCalc:
    ...
    gqa_kv_heads = _model.kv_heads or _model.heads
...
class TpSpeedupCalc:
    ...
    weight_fp16_gb = _model.size_in_bytes(BYTES_FP16).m_as(GB)
    parameters_b = _model.parameters.m_as(Bparam)
    activation_mb = 8
...
class MoECapacityPlan:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    read_per_gpu_str = fmt(read_per_gpu_gb, precision=1, commas=False, suffix=" GB")
    t_read_ms_str = fmt(t_read_ms, precision=2, commas=False, suffix=" ms")
    total_decode_ms_str = fmt(total_decode_ms, precision=1, commas=False, suffix=" ms")
    tokens_per_sec_str = fmt_int(tokens_per_sec)
    total_weight_gb_str = fmt_int(total_weight_gb, commas=False, suffix=" GB")
    node_capacity_gb_str = fmt_int(node_capacity_gb, commas=False, suffix=" GB")
    nodes_needed_str = fmt_int(nodes_needed, commas=False)
    cluster_capacity_gb_str = fmt_int(cluster_capacity_gb, suffix=" GB")
    active_mem_gb_str = fmt_int(active_mem_gb, commas=False, suffix=" GB")
    moe_active_params_b_str = fmt_int(moe_active_params_b, commas=False)
    n_gpus_str = fmt_int(n_gpus, commas=False, suffix=" GPUs")
    alltoall_overhead_ms_str = fmt(alltoall_overhead_ms, precision=1, commas=False, suffix=" ms")
    h100_bw_tbs_str = fmt(h100_bw_tbs, precision=2, commas=False, suffix=" TB/s")
    h100_mem_gb_str = fmt_int(h100_mem_gb, commas=False, suffix=" GB")
...
class Mixtral8x7BSpec:
    ...
    total_params_b_str = fmt_int(_model.parameters.m_as(Bparam), commas=False)
    active_params_b_str = fmt_int(_model.active_parameters.m_as(Bparam), commas=False)
