...
Single-threaded processing cannot sustain that rate. Instead, the system uses a batch accumulation window\index{Batching!batch, accumulation window} of `{python} RecsysLatencyBreakdown.accumulation_window_ms_str`, which yields batches of `{python} RecsysLatencyBreakdown.requests_per_batch_str` at `{python} RecsysLatencyBreakdown.platform_qps_str` and about `{python} RecsysLatencyBreakdown.lookups_per_shard_per_batch_k_str` lookups per shard per batch.

Each embedding shard processes about `{python} RecsysLatencyBreakdown.lookups_per_shard_per_batch_k_str` in a batched operation, achieving high memory bandwidth utilization through sequential memory access patterns.
