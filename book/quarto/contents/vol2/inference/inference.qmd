...
# │          GB, TB, second)
# │ Exports: h100_mem_gb, h100_mem_bw_tbs, a100_mem_bw_tbs
# └─────────────────────────────────────────────────────────────────────────────
from mlsys.constants import H100_MEM_CAPACITY, H100_MEM_BW, A100_MEM_BW, GB, TB, second
...
class BatchingWorkedExample:
    ...
    # Traditional: all requests wait for max
    trad_latency_ms = max_output * decode_time_ms

    # Continuous: average of individual latencies
...
class PrefixCachingAnalysis:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # Without prefix caching: every user stores full system_prompt + response tokens
    ...
    # With prefix caching: system prompt stored once; only response tokens per user
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  \node[stepbox, fill=VerifyColor, minimum width=1.5cm] at (1, 3.7) {Gen T1};
  \node[stepbox, fill=VerifyColor, minimum width=1.5cm] at (2.6, 3.7) {Gen T2};
  \node[stepbox, fill=VerifyColor, minimum width=1.5cm] at (4.2, 3.7) {Gen T3};
  \node[stepbox, fill=VerifyColor, minimum width=1.5cm] at (5.8, 3.7) {Gen T4};
  ...
  % Verify Step
  \node[stepbox, fill=VerifyColor, minimum width=2.0cm] (verify) at (3.5, 1.0) {Target Model\\Verify [T1..T4]};
  \node[below, font=\tiny] at (verify.south) {Parallel Forward Pass};

  % Outcome
  \node[stepbox, fill=AcceptColor, minimum width=0.5cm] at (5.0, 1.8) {T1};
  \node[stepbox, fill=AcceptColor, minimum width=0.5cm] at (5.6, 1.8) {T2};
  \node[stepbox, fill=AcceptColor, minimum width=0.5cm] at (6.2, 1.8) {T3};
  \node[stepbox, fill=RejectColor, minimum width=0.5cm] at (6.8, 1.8) {T4};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  % GPU Columns
  \draw[fill=GPUColor, draw=none] (-2, -0.5) rectangle (1.5, 6);
...
class HeterogeneousGpuCluster:
    ...
    # Weighted routing: each server receives weight × total_qps
    ...
    # Equal routing: each server receives target_qps / total_servers
