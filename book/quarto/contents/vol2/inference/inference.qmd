...
- R1: 50 $\times$ 20 ms = `{python} r1_lat_str` ms (`{python} r1_imp_str`x improvement over traditional)
- R3: 100 $\times$ 20 ms = `{python} r3_lat_str` ms (`{python} r3_imp_str`x improvement)
- R4: 150 $\times$ 20 ms = `{python} r4_lat_str` ms (`{python} r4_imp_str`x improvement)
...
**Savings**: `{python} memory_reduction_pct_str`% reduction in KV cache memory, enabling `{python} concurrent_multiplier_str`x more concurrent users.
...
The `{python} tp_speedup_str`x speedup (vs theoretical `{python} tp_theoretical`x) reflects communication overhead. With `{python} nvlink_h100_gbs` GB/s NVLink bandwidth, each 8MB activation all-reduce takes ~0.3ms.
