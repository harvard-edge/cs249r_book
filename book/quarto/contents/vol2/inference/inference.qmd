...
[^fn-bulkhead-service-isolation]: [offset=-30mm] **Bulkhead Pattern**\index{Bulkhead Pattern!isolation}: Named after ship compartments that contain flooding to isolated sections, which failed on the Titanic when water cascaded over incomplete hull dividers. The lesson for multi-tenant inference: shared GPU pools with soft quotas provide only partial isolation, as a single tenant's burst can exhaust memory or saturate bandwidth for all others. Effective bulkheads require dedicated hardware resources per critical tenant.
...
Search ranking deployments can run ranking models on TPUs[^fn-tpu-serving-accelerator] optimized for transformer inference. TPU pods provide:

[^fn-tpu-serving-accelerator]: [offset=-10mm] **TPU (Tensor Processing Unit) for Inference**\index{TPU!inference}: TPU systolic arrays provide deterministic, low-variance matrix execution for ranking-style inference. GPU latency can vary with batch composition, while TPU-style execution can deliver consistent per-request timing for strict search SLOs where P99 latency directly affects revenue.
