...
The contrast between training and inference optimization extends beyond the throughput-latency distinction introduced in Volume I. At scale, this inversion manifests in system architecture, resource allocation, and operational priorities as compared in @tbl-training-inference-inversion.
...
Vision models represent the simplest batching case because inputs have uniform size (after preprocessing) and computation follows a predictable pattern. The foundations from Volume I apply directly, with scale introducing considerations of batch formation across multiple replicas.
...
This section assumes familiarity with quantization fundamentals covered in Volume I. Here we focus on production inference-specific quantization challenges: post-training methods designed for LLM deployment, hardware-deployment co-design, and the interaction between quantization and serving system design.
:::
