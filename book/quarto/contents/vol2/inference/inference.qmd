...
The optimization techniques in this chapter organize into a **serving hierarchy**\index{Serving!hierarchy}, analogous to the memory hierarchy in computer architecture. Request-level optimizations affect individual request processing through batching, caching, and preprocessing, targeting per-request latency. Replica-level optimizations work within a single model instance, where GPU utilization, memory management, and model optimization determine single-replica throughput. Service-level techniques span multiple replicas of the same model through load balancing, request routing, and autoscaling, while platform-level techniques coordinate across services and tenants through resource allocation, multi-tenancy, scheduling, and cluster management.
...
The system runs on 4 A100-`{python} A100HardwareScenario.a100_mem_gb_str` connected via PCIe Gen4 (`{python} A100HardwareScenario.pcie_gen4_gb_per_s_str` per GPU) rather than NVLink. The server has a dual-socket CPU with NUMA topology. Memory bandwidth per GPU is `{python} A100HardwareScenario.a100_mem_bw_tb_per_s_str` (HBM2e), adequate for decode operations. However, PCIe bandwidth limits tensor parallelism communication to `{python} A100HardwareScenario.pcie_gen4_gb_per_s_str` vs. NVLink's `{python} A100HardwareScenario.nvlink_a100_gb_per_s_str`. For a batch requiring `{python} A100HardwareScenario.activation_transfer_mb_str` activation transfers between GPUs, raw bandwidth math gives approximately `{python} A100HardwareScenario.pcie_transfer_ms_str` per synchronization point over PCIe vs. `{python} A100HardwareScenario.nvlink_transfer_ms_str` over NVLink.
...
***PagedAttention***\index{PagedAttention!block table}\index{PagedAttention!definition} is an LLM serving memory management technique that applies virtual memory principles to KV Cache allocation, storing attention states in noncontiguous, fixed-size physical blocks.
...
**Savings**\index{Savings}: `{python} PrefixCachingAnalysis.memory_reduction_pct_str` reduction in KV cache memory, enabling `{python} PrefixCachingAnalysis.concurrent_mult_str` more concurrent users.
...
The algorithm is deliberately small: select two servers uniformly at random, query both for current queue length, and route to the server with the shorter queue. @Eq-two-choices-max-queue formalizes why this small change matters, reducing maximum queue length from $\mathcal{O}(\log R / \log \log R)$ to $\mathcal{O}(\log \log R)$:
...
# │ Show: total_cap_qps_str="22,000 QPS", h100_util_pct_str="68"%,
# │   a100_util_pct_str="68"%; a100_util_equal_pct_str="83"%
...
Consider a cluster with `{python} HeterogeneousGpuCluster.n_h100_str` H100 GPUs at `{python} HeterogeneousGpuCluster.h100_cap_qps_str` each and `{python} HeterogeneousGpuCluster.n_a100_str` A100 GPUs at `{python} HeterogeneousGpuCluster.a100_cap_qps_str` each, for total capacity of `{python} HeterogeneousGpuCluster.total_cap_qps_str`. At target traffic of `{python} HeterogeneousGpuCluster.target_qps_str`, weighted assignment gives each H100 a weight of `{python} HeterogeneousGpuCluster.h100_weight_pct_str` and each A100 a weight of `{python} HeterogeneousGpuCluster.a100_weight_pct_str`.\index{Target Traffic}\index{Weighted Assignment}

The expected per-server load then becomes `{python} HeterogeneousGpuCluster.h100_load_qps_str` for H100s and `{python} HeterogeneousGpuCluster.a100_load_qps_str` for A100s, leaving the two server types at `{python} HeterogeneousGpuCluster.h100_util_pct_str` and `{python} HeterogeneousGpuCluster.a100_util_pct_str` utilization. Equal distribution would instead send `{python} HeterogeneousGpuCluster.equal_load_qps_str` to every server, leaving H100s underutilized at `{python} HeterogeneousGpuCluster.h100_util_equal_pct_str` and A100s with reduced latency headroom at `{python} HeterogeneousGpuCluster.a100_util_equal_pct_str`.\index{Expected Load Per Server}
...
Least-connections reduces P99 by 38 percent in this example, and combining it with two choices provides additional improvement.
...
Consider an LLM serving system where each user's conversation maintains KV-cache state. Without affinity, the first message may route to Server A and build a KV cache, while the next message randomly routes to Server B and forces a cache rebuild. A 500 ms rebuild penalty across a 10-turn conversation wastes 4.5 seconds on recomputation rather than generation.\index{Without Affinity}

With consistent hashing, the user ID maps to Server A, all messages from that user route to the same server, and the KV cache is reused across turns. Rebuilds occur only when the server assignment changes or the cache is evicted. In practice, each physical server is represented by many virtual nodes on the hash ring, which improves balance despite server heterogeneity.\index{With Consistent Hashing}\index{Implementation with Virtual Nodes}
