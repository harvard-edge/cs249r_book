...
Training a model is a one-time expense; serving it is perpetual. A model trained once for millions of dollars may serve billions of requests over its lifetime, each request incurring compute, memory, and energy costs that accumulate into operational expenses far exceeding the original training investment. The serving cost dominance law\index{Serving Cost Dominance Law} (Principle \ref{pri-serving-cost-dominance}) captures this economic reality: over a successful model's lifetime, Inference OpEx exceeds Training CapEx by orders of magnitude, and optimization efforts should be disproportionately focused on the inference path, even when they increase training cost or complexity. Optimizations that shave milliseconds from inference latency or percentage points from GPU utilization compound across billions of requests into substantial savings or costs. Performance engineering developed the optimization toolkit: fusion, precision, compilation, speculative decoding, and this chapter applies those tools to build serving systems that handle millions of requests under strict latency budgets. Reliability requirements intensify because downtime during serving means lost revenue and broken user experiences, not merely delayed experiments, and the architecture that was optimal for training (large batches, high throughput, tolerance for latency variance) becomes financially unsustainable or user-hostile at serving time. Inference at scale is where ML systems succeed or fail economically.
...
class InferenceSetup:
    ...
    h100_mem_bw_tbs_str = fmt_bandwidth(h100_bw, unit=TB / second, precision=2, commas=False)
    a100_mem_bw_tbs_str = fmt_bandwidth(a100_bw, unit=TB / second, precision=1, commas=False)
    nvlink_h100_gbs_str = fmt_bandwidth(nvlink, unit=GB / second, precision=0, commas=False)
```
...
class ServingCostSimple:
    ...
    serving_training_ratio_str = fmt_multiple(serving_training_ratio, precision=1, commas=False)
...
from mlsysim.fmt import fmt_int, fmt_usd, fmt, fmt_count, check, fmt_math, MarkdownStr
...
class InferenceEconomics:
    ...
    leverage_ratio_str = fmt_multiple(leverage_ratio, precision=1, commas=False)
    qps_str = fmt_int(qps)
...
class InferenceTopologyRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_gbs_str = fmt_bandwidth(nvlink_bw, unit=GB / second, precision=0, commas=False)
```
...
class BatchingWorkedExample:
    ...
    r1_imp_str = fmt_multiple(r1_improvement, precision=0, commas=False)
    r3_imp_str = fmt_multiple(r3_improvement, precision=0, commas=False)
    r4_imp_str = fmt_multiple(r4_improvement, precision=2, commas=False)
```
...
- R1: $50 \times 20$ ms = `{python} BatchingWorkedExample.r1_lat_ms_str` (`{python} BatchingWorkedExample.r1_imp_str`$\times$ improvement over traditional)
...
class ContinuousBatchingBenefit:
    ...
    code_speedup_str = fmt_multiple(code_speedup, precision=1, commas=False)
    ...
    chat_speedup_str = fmt_multiple(chat_speedup, precision=1, commas=False)
    ...
    text_speedup_str = fmt_multiple(text_speedup, commas=False, precision=0)
    ...
    creative_speedup_str = fmt_multiple(creative_speedup, precision=1, commas=False)
    ...
    rag_speedup_str = fmt_multiple(rag_speedup, precision=1, commas=False)
```
...
class A100HardwareScenario:
    ...
    a100_mem_bw_tbs_str = fmt_bandwidth(bw, unit=TB / second, precision=1, commas=False)
    nvlink_a100_gbs_str = fmt_bandwidth(nvlink, unit=GB / second, precision=0, commas=False)
    pcie_gen4_gbs_str = fmt_bandwidth(pcie, unit=GB / second, precision=0, commas=False)
...
class KVCacheCapacityEstimator:
    ...
    # Scalar boundary: the registry stores H100 as 80 GiB; this worked
    # example keeps the vendor-labeled 80 GB budget used by the figure and
    # surrounding prose math.
    h100_vendor_capacity = h100_capacity.to(GiB) * (GB / GiB)
...
class PrefixCachingAnalysis:
    ...
    concurrent_multiplier_str = fmt_multiple(round(concurrent_multiplier_val), commas=False, precision=0)
```
...
**Prefix hit rate**\index{Prefix Hit Rate} determines effectiveness; @tbl-inference-prefix-caching-workloads contrasts hit rates and memory savings across workloads, showing where prefix caching pays off and where it does not.
...
class KVCacheCalc:
    ...
    kv_compression_str = fmt_multiple(kv_compression, precision=0, commas=False)
    h100_mem_gb_str = fmt_memory_capacity(h100_mem, unit=GiB, precision=0, commas=False)
...
class PrecisionDividend:
    ...
    h100_capacity = Hardware.Cloud.H100.memory.capacity
    # Scalar boundary: mlsysim stores H100 as 80 GiB; this worked example keeps
    # the vendor-labeled 80 GB budget used by the surrounding prose math.
    h100_vendor_capacity = h100_capacity.to(GiB).magnitude
    h100_mem = h100_vendor_capacity * GB
...
class SpeculativeSpeedup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sp_high_str = fmt_multiple(sp_high, precision=1, commas=False)
    sp_med_str = fmt_multiple(sp_med, precision=1, commas=False)
    sp_low_str = fmt_multiple(sp_low, precision=1, commas=False)
...
class TpSpeedupCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tp_speedup_str = fmt_multiple(tp_speedup, precision=1, commas=False)
    ...
    tp_theoretical_str = fmt_multiple(tp_theoretical, precision=0, commas=False)
...
class MoEEconomics:
    ...
    compute_ratio_str = fmt_multiple(compute_ratio, precision=1, commas=False)
    bw_ratio_str = fmt_multiple(bw_ratio, precision=1, commas=False)
```

The performance advantages are striking. During autoregressive decode at batch size 1, the dominant cost is reading weights from HBM. A dense 400B model in FP16 reads `{python} MoEEconomics.dense_mem_gb_str` per step. DeepSeek-V3, despite having more total parameters, reads only the 37B active parameters per step (approximately 74 GB in FP16), a `{python} MoEEconomics.bw_ratio_str`$\times$ reduction in per-token bandwidth. The compute savings are proportional: `{python} MoEEconomics.compute_ratio_str`$\times$ fewer FLOPs per token.
...
class MoECapacityPlan:
    ...
    node_gpus = Systems.Nodes.DGX_H100.accelerators_per_node
    h100_capacity = Hardware.Cloud.H100.memory.capacity
    # Scalar boundary: keep the vendor-labeled 80 GB capacity used in this
    # capacity-planning example while sourcing the value from mlsysim.
    h100_vendor_capacity = h100_capacity.to(GiB).magnitude
    h100_mem = h100_vendor_capacity * GB
    n_gpus = 16
    ...
    tokens_per_sec_tokens_s_str = fmt_rate(tokens_per_sec, "tokens/s", precision=0, commas=True)
...
2.  **Latency**: Each decode step reads `{python} MoECapacityPlan.moe_active_params_b_str` active parameters (`{python} MoECapacityPlan.active_mem_gb_str`). Distributed across `{python} MoECapacityPlan.n_gpus_str`, each reads about `{python} MoECapacityPlan.read_per_gpu_gb_str`. At `{python} MoECapacityPlan.h100_bw_tbs_str` HBM bandwidth, $t_{\text{read}} \approx$ `{python} MoECapacityPlan.t_read_ms_str`. AllToAll routing adds about `{python} MoECapacityPlan.alltoall_overhead_ms_str`.
3.  **Floor**: Estimated decode latency $\approx$ `{python} MoECapacityPlan.total_decode_ms_str`/token, or `{python} MoECapacityPlan.tokens_per_sec_tokens_s_str` at batch size 1.
...
class Mixtral8x7BSpec:
    ...
    total_params_b_str = fmt_int(_model.parameters.to(Bparam).magnitude, commas=False)
    active_params_b_str = fmt_int(_model.active_parameters.to(Bparam).magnitude, commas=False)
...
- Total parameters: `{python} Mixtral8x7BSpec.total_params_b_str`B (but only ~`{python} Mixtral8x7BSpec.active_params_b_str`B active per token)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_percent, fmt_rate
...
class HeterogeneousGpuCluster:
    ...
    h100_weight_str = fmt_percent(h100_weight, precision=1, commas=False, style='prose')
    a100_weight_str = fmt_percent(a100_weight, precision=1, commas=False, style='prose')
    ...
    h100_util_str = fmt_percent(h100_util/100, precision=1, commas=False, style='prose')
    a100_util_str = fmt_percent(a100_util/100, precision=1, commas=False, style='prose')
...
- H100: $15000 \times (1000/22000)$ = `{python} HeterogeneousGpuCluster.h100_load_qps_str` (`{python} HeterogeneousGpuCluster.h100_util_str` utilization)
- A100: $15000 \times (600/22000)$ = `{python} HeterogeneousGpuCluster.a100_load_qps_str` (`{python} HeterogeneousGpuCluster.a100_util_str` utilization)
...
- H100 utilization: `{python} HeterogeneousGpuCluster.h100_util_equal_pct_str` (underutilized)
- A100 utilization: `{python} HeterogeneousGpuCluster.a100_util_equal_pct_str` (high utilization, reduced latency headroom)
...
For long-running requests (common in LLM serving), least-connections significantly outperforms round-robin because it accounts for current load rather than just historical assignments. The challenge is maintaining accurate connection counts in a distributed system. Options include:
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt, MarkdownStr, fmt_percent, fmt_time
...
The three patterns form a decision ladder. Regional replicas pay the highest deployment cost but minimize RTT and isolate regional failures. Edge caching pays operational complexity only when queries repeat. Cross-region sharding is a last resort for capacity shortages because inter-region latency overwhelms per-stage compute.
...
GPTQ [@frantar2023gptq] is the weight-only choice when deployment needs 4-bit compression after training and can afford calibration data. It uses Hessian-based[^fn-hessian-quantization] error compensation, adjusting remaining weights to compensate for errors introduced by quantization rather than quantizing all weights independently.
...
Production serving frameworks matter because quantized weights reduce cost only when the runtime selects kernels and memory managers that preserve the theoretical savings.
...
The final selection matches the method to the binding constraint. Deployment constraints determine the appropriate quantization method:
...
4. Is the model compute-bound or memory-bound?
   - Compute-bound (prefill): W8A8 provides 2× speedup
   - Memory-bound (decode): W4A16 provides 4× memory bandwidth
```
...
class RankingCascadeCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    final_eval_reduction_str = fmt_multiple(final_eval_reduction, precision=0, commas=True)
    ...
    cost_reduction_rounded_str = fmt_multiple(cost_reduction_rounded, precision=0, commas=True)
```
