...
Three distinct signals indicate when distributed inference becomes necessary rather than merely optional, as summarized in @tbl-distribution-triggers.
...
The total serving tax often consumes 10-30% of the latency budget in distributed systems as shown in @eq-serving-tax:
...
The total cost of operating a model comprises training cost (a one-time expense) and serving cost (an ongoing expense) as defined in @eq-total-cost:
...
The cost dominance ratio varies by application as shown in @tbl-cost-ratios:
...
A critical misconception in current discourse frames inference at scale as synonymous with LLM serving. While large language models present distinctive challenges and attract significant attention, they represent a small fraction of production inference volume. The full inference landscape summarized in @tbl-inference-landscape is essential for appropriate technique selection.
...
Each level has distinct optimization levers as summarized in @tbl-serving-hierarchy:
...
**LLM-specialized servers** optimize specifically for autoregressive generation[^fn-autoregressive]:
...
3. **Optimized attention kernels**: FlashAttention [@dao2022flashattention] integration
...
The core insight is that batching efficiency depends on how computation scales with batch size relative to how memory and communication scale. Different model architectures exhibit different scaling relationships, requiring different batching strategies as summarized in @tbl-batching-by-model.
...
LLM inference consists of two distinct phases with different computational characteristics, requiring different batching strategies within the same request as summarized in @tbl-prefill-decode:
...
Model sharding for inference is driven by two distinct requirements as summarized in @tbl-sharding-triggers:
...
**Latency requirements**: Even when a model fits in memory, sharding can reduce latency by parallelizing computation. The potential speedup depends on the parallelization efficiency as shown in @eq-parallel-time:
...
Tensor parallelism[^fn-megatron] [@shoeybi2019megatron] distributes individual layers across multiple devices, enabling parallel computation within each layer. For transformer models, the primary target is the attention mechanism and feed-forward layers, which contain the majority of computation.

[^fn-megatron]: **Megatron-LM**: NVIDIA's framework for training and serving large transformer models, introduced in 2019. Megatron pioneered the column-row partitioning scheme for tensor parallelism that minimizes communication: splitting the first linear layer by columns and the second by rows requires only one AllReduce per transformer block rather than two.
...
After computing local attention, an all-reduce[^fn-allreduce] operation combines results across devices.

[^fn-allreduce]: **AllReduce**: A collective communication primitive that combines values from all processes and distributes the result back to all processes. For tensor parallelism, AllReduce sums partial attention outputs across devices, adding communication overhead proportional to the activation size divided by the interconnect bandwidth.
...
: **Pipeline vs Tensor Parallelism**: Each strategy has distinct tradeoffs in latency, throughput, and implementation complexity as compared in @tbl-pipeline-tensor-comparison.
...
The choice of embedding sharding strategy depends on lookup patterns and communication overhead as compared in @tbl-embedding-sharding.
...
**AllReduce** (tensor parallelism): Combines data from all devices, with result available on all devices as shown in @eq-allreduce-time.
...
**Point-to-point** (pipeline parallelism): Sends data from one device to the next as calculated by @eq-p2p-time.

$$T_{p2p} = L + \frac{M}{B}$$ {#eq-p2p-time}

where $L$ is the network latency and $M/B$ is the transfer time.

**AllToAll** (expert parallelism): Exchanges data where each device sends different data to each other device as defined in @eq-alltoall-time.
...
The choice of sharding strategy depends on model architecture and deployment priorities as summarized in @tbl-sharding-selection.
...
This simple modification reduces maximum queue length from $O(\log n / \log \log n)$ to $O(\log \log n)$ as shown in @eq-two-choices-max-queue:
...
The KV cache stores these computed key and value vectors, reducing generation to $O(t)$ per token. However, this memory savings comes at the cost of storing past context as calculated by @eq-kv-cache-size:
...
Quantization reduces numerical precision of model weights and activations, decreasing memory footprint and increasing throughput. The fundamentals include post-training quantization (PTQ), which quantizes after training without retraining; quantization-aware training (QAT), which simulates quantization during training for better accuracy; and precision format tradeoffs between INT8, INT4, and mixed precision schemes. Serving at scale introduces distinct challenges beyond these fundamentals. Models must be quantized after training without access to training data. Quantization must preserve quality across diverse inputs. Hardware deployment targets vary from datacenter GPUs to edge accelerators. This section examines quantization techniques specifically designed for production inference.
