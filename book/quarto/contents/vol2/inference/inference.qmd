...
- Total batch time: 200 × 20 ms = `{python} total_batch_time_str` ms
...
The GPU performs 4 × 200 = `{python} total_seq_iters_str` "sequence-iterations" but only `{python} useful_seq_iters_str` are useful.
...
- R1: 50 × 20 ms = `{python} r1_lat_str` ms (`{python} r1_imp_str`x improvement over traditional)
- R3: 100 × 20 ms = `{python} r3_lat_str` ms (`{python} r3_imp_str`x improvement)
- R4: 150 × 20 ms = `{python} r4_lat_str` ms (`{python} r4_imp_str`x improvement)
- R2: 200 × 20 ms = `{python} r2_lat_str` ms (no improvement for longest request)
...
- Total KV cache: `{python} tokens_per_user_str` × 1000 × 2 × 80 × 8192 × 2 = `{python} kv_tb_no_cache_str` TB

**With prefix caching**:

- Shared prefix: 2000 tokens (once)
- Unique per user: 500 tokens
- Total: (2000 × 1) + (500 × 1000) = `{python} total_tokens_cached_str` tokens
- Memory: `{python} total_tokens_cached_str` × 2 × 80 × 8192 × 2 = `{python} kv_tb_cached_str` TB
...
- Sequential: `{python} f"{seq_total_ms:,.0f}"`ms per token
- 8-way TP: `{python} f"{tp8_total_ms:,.0f}"`ms per token
...
- Total capacity: 10×1000 + 20×600 = `{python} total_cap_str` QPS
...
- H100: 15000 × 0.045 = `{python} h100_load_str` QPS (`{python} h100_util_str`% utilization)
- A100: 15000 × 0.027 = `{python} a100_load_str` QPS (`{python} a100_util_str`% utilization)
