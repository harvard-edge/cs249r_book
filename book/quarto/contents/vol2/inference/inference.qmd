...
[^fn-dlrm]: **Deep Learning Recommendation Model (DLRM)**: Meta's reference architecture for large-scale recommendation, introduced in 2019. DLRM separates dense features (processed by MLPs) from sparse features (processed via embedding tables), enabling the hybrid CPU-GPU architectures that characterize modern recommendation serving.
...
|--------------|-----------|---------|----------|
| NVLink (H100)[^fn-nvlink] | 900 GB/s | 1μs | Intra-node TP |
...
Under these realistic conditions, uninformed strategies perform poorly. The maximum queue length under random assignment follows[^fn-theta-notation]:

$$E[\text{max queue}] = \Theta\left(\frac{\log n}{\log \log n}\right)$$ {#eq-random-max-queue}

[^fn-theta-notation]: The $\Theta$ notation indicates asymptotic tight bounds: the queue length is both $O(\log n / \log \log n)$ and $\Omega(\log n / \log \log n)$, meaning this characterization is asymptotically exact rather than just an upper bound.
