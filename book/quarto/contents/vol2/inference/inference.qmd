...
from mlsysim.fmt import fmt_int, fmt_usd, check, fmt, fmt_percent, fmt_count
...
2.  **Metric**: Annual inference volume is $10^6 \text{ users} \times 50 \text{ reqs} \times 365 \text{ days} \approx$ `{python} ServingCostSimple.annual_reqs_b_str` requests/year.
...
class LittlesLawExample:
    ...
    replica_latency_ms_str = fmt_time(replica_latency_s * THOUSAND, 'millisecond', precision=0, commas=False)
```
...
class TpSpeedupCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt_int, check, fmt, fmt_bandwidth, fmt_memory, fmt_time, fmt_params
...
class RankingCascadeCost:
    ...
    l3_all_s_str = fmt_time(l3_all_s, second, precision=0, commas=False)
