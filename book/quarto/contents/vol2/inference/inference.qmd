...
class InferenceSetup:
    ...
    h100_mem_gb_val = mem_h_h100.to(GiB).magnitude
    h100_bw_tbs_val = bw_h_h100.to(TB/second).magnitude
    a100_bw_tbs_val = bw_a100.to(TB/second).magnitude
    nvlink_gbs_val = nvlink.to(GB/second).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_mem_gb_str = fmt_qty_int(h100_mem_gb_val * GiB, GiB, commas=False, unit_label="GB")
    h100_mem_bw_tbs_str = fmt_qty(h100_bw_tbs_val * TB / second, TB / second, precision=2, commas=False)
    a100_mem_bw_tbs_str = fmt_qty(a100_bw_tbs_val * TB / second, TB / second, precision=1, commas=False)
...
class ServingCostSimple:
    ...
    cost_per_request_str = fmt_usd(cost_per_request, precision=3, commas=False)
...
class InferenceEconomics:
    ...
    cost_per_gpu_hour_str = fmt_usd(cost_per_gpu_hour, commas=False)
...
class InferenceTopologyRecap:
    nvlink_gbs = Hardware.Cloud.H100.nvlink.bandwidth.to(GB/second).magnitude
    from mlsysim.fmt import fmt_int, fmt, fmt_qty_int
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_gbs_str = fmt_qty_int(nvlink_gbs * GB / second, GB / second, commas=False)
```
...
class LogicScaling:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    t_reasoning_s = (t_std_ms * reasoning_steps * ms).to(second).magnitude
    ...
    time_s_str = fmt_time(t_reasoning_s, 'second', precision=1, commas=False, style='word')
...
class BatchingWorkedExample:
    ...
    total_batch_time_str = fmt_time(round(total_batch_time_ms), 'millisecond', precision=0, commas=True)
    total_seq_iters_str = fmt_int(total_seq_iters)
    useful_seq_iters_str = fmt_int(useful_seq_iters)
    r1_lat_str = fmt_time(round(r1_latency_ms), 'millisecond', precision=0, commas=True)
    r2_lat_str = fmt_int(r2_latency_ms)
    r3_lat_str = fmt_int(r3_latency_ms)
    r4_lat_str = fmt_int(r4_latency_ms)
    trad_lat_str = fmt_time(round(trad_latency_ms), 'millisecond', precision=0, commas=True)
...
class A100HardwareScenario:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    a100_mem_gb_val = mem.to(GiB).magnitude
    a100_mem_bw_tbs_val = bw.to(TB/second).magnitude
    nvlink_a100_gbs_val = nvlink.to(GB/second).magnitude
    pcie_gen4_gbs_val = pcie.to(GB/second).magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_gb_str = fmt_qty_int(a100_mem_gb_val * GiB, GiB, commas=False, unit_label="GB")
    a100_mem_bw_tbs_str = fmt_qty(a100_mem_bw_tbs_val * TB / second, TB / second, precision=1, commas=False)
    nvlink_a100_gbs_str = fmt_qty_int(nvlink_a100_gbs_val * GB / second, GB / second, commas=False)
    pcie_gen4_gbs_str = fmt_qty_int(pcie_gen4_gbs_val * GB / second, GB / second, commas=False)
    activation_transfer_mb_str = fmt_qty_int(activation_transfer_mb * MB, MB, commas=False)
    pcie_transfer_ms_str = fmt_time(pcie_transfer_ms_val, 'millisecond', precision=1, commas=False)
    nvlink_transfer_ms_str = fmt_time(nvlink_transfer_ms_val, 'millisecond', precision=2, commas=False)
```
...
::: {.column-margin}
![](images/svg/inference_kv_cache_ladder.svg){width="100%" fig-alt="Three stacked blue bars on a log scale spanning six orders of magnitude. A single token uses 0.33 MB, a 128K request uses 43 GB, and HBM capacity is 480 GB."}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt, fmt_qty_int, MarkdownStr
...
class KVCacheCapacityEstimator:
    ...
    kv_mb_per_token = kv_cache_one_token.to(MB).magnitude

    kv_cache_full = calc_kv_cache_size(n_layers, n_kv_heads, d_head, context_tokens, 1, bytes_per_elem)
    kv_gb_per_request = kv_cache_full.to(GB).magnitude
    ...
    available_kv_gb_str = fmt_qty_int(available_kv_gb * GB, GB, commas=False)
    max_batch_str = fmt_int(max_batch, commas=False)
    weight_gb_str = fmt_qty_int(weight_gb * GB, GB, commas=False)
    total_hbm_gb_str = fmt_qty_int(total_hbm_gb * GB, GB, commas=False)
    system_gb_str = fmt_qty_int(system_gb * GB, GB, commas=False)
    node_gpus_str = fmt_int(Systems.Nodes.DGX_H100.accelerators_per_node, commas=False)
    h100_mem_gb_str = fmt_int(Hardware.Cloud.H100.memory.capacity.to(GiB).magnitude, commas=False)
...
class PrefixCachingAnalysis:
    ...
    kv_tb_no_cache_str = fmt_qty(kv_tb_no_cache_val * TB, TB, precision=1, commas=False)
    total_tokens_cached_str = fmt_int(total_tokens_cached)
    kv_tb_cached_str = fmt_qty(kv_tb_cached_val * TB, TB, precision=1, commas=False)
...
class KVCacheCalc:
    ...
    kv_head_dim = 128
    kv_seq_len = 4096
    kv_bytes_fp16 = BYTES_FP16.to(byte).magnitude
    kv_bytes_int4 = BYTES_INT4.to(byte).magnitude
    ...
    kv_fp16_gb = (kv_fp16_bytes * byte).to(GB).magnitude
    kv_int4_gb = (kv_int4_bytes * byte).to(GB).magnitude
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt_int, check, MarkdownStr, fmt, fmt_math, fmt_qty
    ...
    kv_fp16_gb_str = fmt_qty(kv_fp16_gb * GB, GB, precision=1, commas=False)
    kv_int4_gb_str = fmt_qty(kv_int4_gb * GB, GB, precision=1, commas=False)
    ...
    batch8_kv_fp16_gb_str = fmt_qty(batch8_kv_fp16_gb * GB, GB, precision=1, commas=False)
    gqa_kv_fp16_gb_str = fmt_qty(gqa_kv_fp16_gb * GB, GB, precision=1, commas=False)
    gqa_kv_heads = 8
...
from mlsysim.fmt import fmt_int, check, fmt, fmt_qty, fmt_qty_int
...
class PrecisionDividend:
    ...
    # so .to(GiB).magnitude gives the 80 that prose anchors to.
    ...
    kv_head_dim = 128
    kv_seq_len = 4096
    kv_fp16_gb = (2 * kv_layers * kv_heads * kv_head_dim * kv_seq_len * BYTES_FP16.to(byte).magnitude * byte).to(GB).magnitude
    ...
    fp16_weight_per_gpu_str = fmt_qty_int(fp16_weight_per_gpu * GB, GB, commas=False)
    ...
    kv_fp16_per_gpu_str = fmt_qty(kv_fp16_per_gpu * GB, GB, precision=1, commas=False)
    ...
    fp16_weight_total_gb_str = fmt_qty_int(fp16_weight_total_gb * GB, GB, commas=False)
...
*Decode is memory-bound; batching is the only lever toward the ridge.*
:::
...
::: {.column-margin}
![](images/svg/vol2_inference_margin_001.svg){width="100%" fig-alt="Margin illustration showing Speculative decoding speedup threshold around 70 percent acceptance."}
...
class TpSpeedupCalc:
    ...
    nvlink_h100_gbs_val = nvlink_bw.to(GB/second).magnitude

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt_int, check, fmt, fmt_qty_int, fmt_time
    ...
    nvlink_h100_gbs_str = fmt_qty_int(nvlink_h100_gbs_val * GB / second, GB / second, commas=False)
    hidden_dim_str = fmt_int(hidden_dim)
    weight_fp16_gb_str = fmt_qty_int(weight_fp16_gb * GB, GB, commas=False)
...
class MoEEconomics:
    ...
    dense_params_b = 400
    moe_total_params_b = _moe.parameters.to(Bparam).magnitude
    moe_active_params_b = _moe.active_parameters.to(Bparam).magnitude
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt_int, check, fmt, fmt_qty_int
    check(compute_ratio > 5, f"MoE should give >5x compute reduction, got {compute_ratio:.1f}x")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dense_mem_str = fmt_qty_int(dense_mem_fp16 * GB, GB, commas=False)
    moe_mem_str = fmt_qty_int(moe_mem_fp16 * GB, GB, commas=False)
...
::: {.column-margin}
![](images/svg/vol2_inference_margin_002.svg){width="100%" fig-alt="Margin illustration showing MoE resident memory, dense resident memory, and MoE active read."}
...
from mlsysim.fmt import fmt_int, check, fmt, fmt_qty, fmt_qty_int, fmt_time
...
class MoECapacityPlan:
    ...
    moe_total_params_b = _moe.parameters.to(Bparam).magnitude
    moe_active_params_b = _moe.active_parameters.to(Bparam).magnitude
    bytes_fp8 = BYTES_INT8.to(byte).magnitude  # FP8 = 1 byte
    ...
    h100_bw_gbs = Hardware.Cloud.H100.memory.bandwidth.to(GB/second).magnitude
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    read_per_gpu_str = fmt_qty(read_per_gpu_gb * GB, GB, precision=1, commas=False)
    t_read_ms_str = fmt_time(t_read_ms, 'millisecond', precision=2, commas=False)
    total_decode_ms_str = fmt_time(total_decode_ms, 'millisecond', precision=1, commas=False)
    tokens_per_sec_str = fmt_int(tokens_per_sec)
    total_weight_gb_str = fmt_qty_int(total_weight_gb * GB, GB, commas=False)
    node_capacity_gb_str = fmt_qty_int(node_capacity_gb * GB, GB, commas=False)
    nodes_needed_str = fmt_int(nodes_needed, commas=False)
    cluster_capacity_gb_str = fmt_qty_int(cluster_capacity_gb * GB, GB)
    active_mem_gb_str = fmt_qty_int(active_mem_gb * GB, GB, commas=False)
    moe_active_params_b_str = fmt_int(moe_active_params_b, commas=False)
    n_gpus_str = fmt_count(n_gpus, commas=False, label="GPU")
    alltoall_overhead_ms_str = fmt_time(alltoall_overhead_ms, 'millisecond', precision=1, commas=False)
    h100_bw_tbs_str = fmt_qty(h100_bw_tbs * TB / second, TB / second, precision=2, commas=False)
    h100_mem_gb_str = fmt_qty_int(h100_mem_gb * GiB, GiB, commas=False, unit_label="GB")
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt, fmt_percent, fmt_qty_int
...
class MetaEmbeddingScaleCheck:
    ...
    entity_count_t = 10
    min_dimension = 128
    bytes_per_dimension_int8 = BYTES_INT8.to(byte).magnitude
...
class AllReduceInterconnect:
    ...
    message_mb = 8
    layer_budget_ms = 30
    nvlink_bw_gbs = Hardware.Cloud.H100.nvlink.bandwidth.to(GB/second).magnitude
    ib_hdr_bw_gbs = Systems.Fabrics.InfiniBand_HDR.bandwidth.to(GB/second).magnitude
    eth_100g_bw_gbs = NETWORK_100G_BW.to(GB/second).magnitude
    ...
    message_mb_str = fmt_qty_int(message_mb * MB, MB, commas=False)
...
class HeterogeneousGpuCluster:
    ...
    h100_load_str = fmt_rate(h100_load, "QPS", precision=1, commas=False)
    a100_load_str = fmt_rate(a100_load, "QPS", precision=1, commas=False)
...
*One tenant's burst eats shared KV cache and starves the rest.*
:::
...
class QuantizedServingCapacity:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_mem_gb_str = fmt_qty_int(a100_mem_gb * GiB, GiB, commas=False, unit_label="GB")
    weight_int4_gb_str = fmt_qty_int(weight_int4_gb * GB, GB, commas=False)
    kv_headroom_gb_str = fmt_qty_int(kv_headroom_gb * GB, GB, commas=False)
    kv_per_1k_sequence_gb_str = fmt_qty(kv_per_1k_sequence_gb * GB, GB, precision=1, commas=False)
...
class FinalSummaryHardware:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    h100_mem_gb_val = h100_mem.to(GiB).magnitude
    h100_bw_tbs_val = h100_bw.to(TB/second).magnitude
