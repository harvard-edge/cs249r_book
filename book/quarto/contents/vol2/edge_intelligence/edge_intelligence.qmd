...
\begin{scope}[local bounding box=MOBILE2, right=1.5cm of MOBILE1, anchor=west, yshift=-1.52cm]
  \foreach \i/\sf in {1/0.6, 2/0.7, 3/0.8, 4/0.9, 5/1} {
    \pic[shift={(0,0)}] at ({-\i*0.17}, {0.4*\i}) {mobile={scalefac=\sf, picname=2\i}};
  }
\end{scope}

\begin{scope}[local bounding box=MOBILE3, right=1.5cm of MOBILE2, anchor=west]
  \foreach \i/\sf in {1/0.6, 2/0.7, 3/0.8, 4/0.9, 5/1} {
    \pic[shift={(0,0)}] at ({-\i*0.02}, {0.4*\i}) {mobile={scalefac=\sf, picname=3\i}};
  }
\end{scope}

\begin{scope}[local bounding box=MOBILE4, right=1.5cm of MOBILE3, anchor=west]
  \foreach \i/\sf in {1/0.6, 2/0.7, 3/0.8, 4/0.9, 5/1} {
    \pic[shift={(0,0)}] at ({-\i*0.17}, {0.4*\i}) {mobile={scalefac=\sf, picname=4\i}};
  }
\end{scope}

\begin{scope}[local bounding box=MOBILE5, right=1.5cm of MOBILE4, anchor=west]
  \foreach \i/\sf in {1/0.6, 2/0.7, 3/0.8, 4/0.9, 5/1} {
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85]
  ...
  % Axes
  \draw[Axis] (0,0) -- (8,0) node[right] {Local Epochs $E$};
  \draw[Axis] (0,0) -- (0,5) node[above] {Rounds to $\epsilon$-optimal};
  ...
  % Axis labels
  \node[Label, below] at (1,0) {1};
  \node[Label, below] at (4,0) {5};
  \node[Label, below] at (7,0) {10};
  \node[Label, left] at (0,1) {100};
  \node[Label, left] at (0,2.5) {500};
  \node[Label, left] at (0,4) {1000};
\end{tikzpicture}
```
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  % Secret Exchange
  \draw[<->, dashed, line width=1.0pt, draw=OrangeLine] (clientA) -- node[above, font=\scriptsize, align=center, text=black] {Shared Secret Mask\\$M_{AB}$} (clientB);
  ...
  % Server
  \path (masA) -- (masB) node[midway, below=1.5cm] (mid) {};
  \node[ServerBox] (server) at (mid) {Aggregation Server};
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}]
  ...
  % Pyramid tiers using nodes would be complex, but we can use relative coordinates
  \coordinate (top) at (0, 3);
  \coordinate (tier1_left) at (-1, 3);
  \coordinate (tier1_right) at (1, 3);
  \coordinate (tier2_left) at (-1.5, 2);
  \coordinate (tier2_right) at (1.5, 2);
  \coordinate (tier3_left) at (-2.5, 1);
  \coordinate (tier3_right) at (2.5, 1);
  \coordinate (bottom_left) at (-3.5, 0);
  \coordinate (bottom_right) at (3.5, 0);

  \draw[tier, fill=BlueL, draw=BlueLine] (tier1_left) -- (tier1_right) -- (tier2_right) -- (tier2_left) -- cycle;
  \draw[tier, fill=GreenL, draw=GreenLine] (tier2_left) -- (tier2_right) -- (tier3_right) -- (tier3_left) -- cycle;
  \draw[tier, fill=OrangeL, draw=OrangeLine] (tier3_left) -- (tier3_right) -- (bottom_right) -- (bottom_left) -- cycle;

  % Labels
  \node[label] (l1) at (0, 2.5) {\textbf{Flagship Devices}};
  \node[sublabel, below=0.1cm of l1] {LoRA Rank-32 + Full Replay};

  \node[label] (l2) at (0, 1.5) {\textbf{Mid-Range Devices}};
  \node[sublabel, below=0.1cm of l2] {LoRA Rank-8 + Feature Replay};

  \node[label] (l3) at (0, 0.5) {\textbf{Budget / IoT Devices}};
