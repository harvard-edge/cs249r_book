...
class EdgeNpuSpeedup:
    ...
    e_cpu = p_cpu.energy
    e_npu = e_cpu / 60.0
    e_cpu_j = e_cpu.m_as(joule)
    e_npu_j = e_npu.m_as(joule)
...
class EdgeMobilenetSizing:
    ...
    alpha10_mb = alpha10.m_as(MB)
    alpha05_total_mb = alpha05_total.m_as(MB)
    alpha05_feature_mb = alpha05_feature.m_as(MB)
    ...
    stm32_hidden_kb = stm32_hidden.m_as(KB)
    ...
    model_budget = 4 * MB
    model_budget_mb = model_budget.m_as(MB)
...
class EdgeMobilePowerEnvelope:
    ...
    sustained_low_w = sustained_low.m_as(watt)
    sustained_high_w = sustained_high.m_as(watt)
    burst_low_w = burst_low.m_as(watt)
    burst_high_w = burst_high.m_as(watt)
    ...
    sustained_range_str = MarkdownStr(
        ...
        f"{fmt_qty(sustained_high, watt, precision=0, commas=False)}"
    )
    burst_range_str = MarkdownStr(
        f"{fmt(burst_low_w, precision=0, commas=False)}–"
        f"{fmt_qty(burst_high, watt, precision=0, commas=False)}"
    )
```
...
class EdgeFedPowerEnvelopeRecap:
    ...
    sustained_low_w = sustained_low.m_as(watt)
    sustained_high_w = sustained_high.m_as(watt)
    background_low_mw = background_low.m_as(milliwatt)
    background_high_mw = background_high.m_as(milliwatt)
    ...
    background_range_str = MarkdownStr(
        f"{fmt(background_low_mw, precision=0, commas=False)}–"
        f"{fmt_qty(background_high, milliwatt, precision=0, commas=False)}"
    )
```
...
class EdgeAdaptationPowerMetrics:
    ...
    background_energy_per_hour_str = MarkdownStr(
        f"{fmt(background_low_kj_per_hour, precision=1, commas=False)}–"
        f"{fmt_qty(background_high_kj_per_hour * ureg.kilojoule, ureg.kilojoule, precision=1, commas=False, unit_label='kJ per hour')}"
    )
```
