...
[^fn-non-iid-federated]: **Non-IID (Not Independent and Identically Distributed)**\index{Non-IID!federated learning}: Data where samples are not drawn from a single common distribution, the default condition in federated learning where each device reflects a single user's patterns. Non-IID label and feature distributions can sharply degrade federated averaging compared with IID baselines [@zhao2018federated], forcing systems to adopt techniques like local adaptation layers or clustered aggregation to maintain convergence.
...
[^fn-gradient-sparsification-topk]: **Gradient Sparsification**\index{Gradient!sparsification, top-k}: Transmits only a selected subset of gradients by magnitude, exploiting the observation that many gradient elements are near-zero or less useful for a given update. Local gradient accumulation stores the untransmitted residuals until they grow large enough to send, a pattern used in sparsified SGD and deep gradient compression [@stich2018sparsified; @lin2018deep]. The trade-off: aggressive sparsification can bias updates toward frequently active parameters, potentially reducing personalization for rare user patterns.
...
[^fn-cloudlet-hybrid-training]: **Cloudlet**\index{Cloudlet!hybrid training}: A small-scale compute tier located close to users, often near access networks or enterprise sites, provides lower-latency offload than a distant cloud region. This intermediate tier enables hybrid on-device learning: time-sensitive inference runs locally while aggregation, validation, or heavier adaptation work can offload nearby when the network and trust model allow it. The trade-off is deployment complexity: cloudlets add an orchestration layer that must decide per-update whether to compute locally, offload nearby, or defer to cloud.
