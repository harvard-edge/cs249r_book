...
::: {.callout-note title="Figure Placeholder: Memory Amplification" collapse="true"}
```{.tikz}
% TODO: Bar chart comparing memory usage: Inference (Weights+Activations) vs Training (Weights+Activations+Gradients+Optimizer)
\node[draw, align=center] {Memory Amplification\nInference vs Training};
```
**Training Memory Amplification**. Visualizing the explosion of memory requirements when moving from inference to training. Inference requires only weights and a single activation map. Training requires weights, *all* activation maps (for backprop), gradients, and optimizer state. This 3-5x increase creates the primary bottleneck for on-device learning.
:::
...
Real-world applications demonstrate both the potential and challenges of on-device learning, from keyword spotting systems that adapt to user voices to recommendation engines that personalize without transmitting user data. As machine learning expands into mobile, embedded, and wearable environments, the ability to learn locally while maintaining efficiency and reliability becomes essential for intelligent systems that operate seamlessly across diverse deployment contexts.

Part III has examined deployment at scale across the full spectrum: from centralized inference systems serving millions of requests per second (@sec-inference-at-scale) to the distributed edge deployments explored in this chapter. Deploying systems, however, is only the beginning. Scaling inference, whether in datacenters or at the edge, creates operational complexity that compounds with every deployment decision. Model versions proliferate across environments. Device fleets require coordinated updates. Performance regressions must be detected before users notice. The next chapter, @sec-ops-scale, addresses how organizations manage this complexity through systematic operational practices: continuous integration pipelines, monitoring infrastructure, and incident response frameworks that keep distributed ML systems healthy at scale. The deployment architectures established in Part III provide the foundation; ML Operations provides the discipline that sustains them.
