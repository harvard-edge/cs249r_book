...
This chapter examines what happens when ML systems move beyond datacenters to the billions of devices at the network edge. The distributed training techniques from Part II assumed high bandwidth interconnects; edge devices communicate over cellular networks with intermittent connectivity. The inference systems from @sec-inference-at-scale assumed GPU clusters with terabytes of memory; edge devices operate with megabytes under strict power budgets. The fault tolerance mechanisms from @sec-fault-tolerance assumed centralized checkpointing; edge systems must maintain progress across devices that appear and disappear unpredictably. These inversions define a fundamentally different engineering challenge.
