...
As @fig-training-memory-amplifier shows, this amplification manifests as a 4--12$\times$ memory footprint increase across model scales.

::: {#fig-training-memory-amplifier fig-env="figure" fig-pos="htb" fig-cap="**The Training Memory Amplifier**. Comparison of memory requirements for Inference vs. Training across model scales (1M to 100M parameters). Training requires storing gradients, optimizer states, and activations, resulting in a 4--12$\times$ memory footprint increase." fig-alt="Grouped bar chart comparing inference (weights only) vs. training (weights, gradients, optimizer state, activations) for 1M, 10M, and 100M parameter models. Amplification labels show a 4 to 12 times increase."}
