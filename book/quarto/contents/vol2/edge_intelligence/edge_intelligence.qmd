...
This battery calculation is the local edge-learning version of a broader energy constraint: training must fit the user's energy budget, not only the model's accuracy target. @Sec-sustainable-ai later develops the full sustainability accounting for edge fine-tuning, including the energy tradeoff between local computation and communication.
...
These reliability threats demand systematic approaches that keep local adaptation bounded even when devices are faulty, malicious, or operating under environmental shift. The local lesson is that robust aggregation, drift checks, and conservative update policies are part of the edge-learning architecture, not optional add-ons after deployment. @Sec-robust-ai later develops the broader robustness framework, while @sec-security-privacy develops the cryptographic and privacy mechanisms, such as secure aggregation and differential privacy, that protect self-learning systems at scale.
