...
from mlsysim.core.constants import joule, ureg
...
class EdgeNpuSpeedup:
    ...
    t_cpu_ms = p_cpu.latency.to(ms).magnitude
    t_npu_ms = t_cpu_ms / 20.0
    e_cpu = p_cpu.energy
    e_npu = e_cpu / 60.0
    e_cpu_j = e_cpu.to(joule).magnitude
    e_npu_j = e_npu.to(joule).magnitude
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    npu_cpu_lat_ms_str = fmt_time(t_cpu_ms, 'millisecond', precision=3, commas=False)
    npu_lat_ms_str = fmt_time(t_npu_ms, 'millisecond', precision=3, commas=False)
...
::: {.column-margin}
![](images/svg/vol2_edge_intelligence_margin_001.svg){width="100%" fig-alt="Margin illustration showing Keyboard app memory budget versus one local gradient update."}
...
class EdgeSmartphoneRamRecap:
    smartphone_ram_gb = int(Platforms.Mobile.ram.to(GB).magnitude)
...
class EdgeMobilenetSizing:
    ...
    alpha10_mb = alpha10.to(MB).magnitude
    alpha05_total_mb = alpha05_total.to(MB).magnitude
    alpha05_feature_mb = alpha05_feature.to(MB).magnitude
    ...
    stm32_hidden_kb = stm32_hidden.to(KB).magnitude
    ...
    model_budget = 4 * MB
    model_budget_mb = model_budget.to(MB).magnitude
...
class EdgeMobilePowerEnvelope:
    ...
    sustained_low_w = sustained_low.to(watt).magnitude
    sustained_high_w = sustained_high.to(watt).magnitude
    burst_low_w = burst_low.to(watt).magnitude
    burst_high_w = burst_high.to(watt).magnitude
...
class EdgeMobileNpuRecap:
    mobile_npu_tops = int(Hardware.Mobile.iPhone15Pro.compute.peak_flops.to(TOPS).magnitude)
...
class EdgeH100BwRecap:
    h100_mem_bw_gbs = int(Hardware.Cloud.H100.memory.bandwidth.to(GB/second).magnitude)
    h100_mem_bw = h100_mem_bw_gbs * GB / second
    from mlsysim.fmt import fmt_qty_int
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
...
from mlsysim.fmt import check, fmt, fmt_percent, fmt_qty, fmt_qty_int, fmt_time
from mlsysim import Scenarios
...
class EdgeBattery:
    ...
    battery = Scenarios.PhoneBattery.EnergyWh  # Modern flagship smartphone battery
    ...
    battery_energy_wh_str = fmt_qty(energy, Wh, precision=2, commas=False, unit_label="Wh")
    battery_drain_pct_str = fmt_percent(round(drain_pct)/100, precision=0, commas=False, style='prose')
    battery_total_wh_str = fmt_qty_int(battery, Wh, commas=False, unit_label="Wh")
```
...
class StorageWall:
    ...
    model_params_m = 10
    n_contexts = 10
    bytes_per_param = BYTES_FP32.to(byte).magnitude # FP32 for training/fine-tuning
    ...
    size_full_mb = size_full.to(MB).magnitude
    size_adapter_kb = size_adapter.to(KB).magnitude
    size_adapter_mb = size_adapter.to(MB).magnitude
    total_full_mb = total_full.to(MB).magnitude
    total_adapter_mb = total_adapter.to(MB).magnitude
...
::: {.column-margin}
![](images/svg/vol2_edge_intelligence_margin_002.svg){width="100%" fig-alt="Margin illustration showing Full per-context copy versus rank-64 adapter."}
...
::: {.column-margin}
![](images/svg/vol2_edge_intelligence_margin_003.svg){width="100%" fig-alt="Margin illustration showing New-task performance rises while old-task accuracy falls."}
...
class FederatedSavings:
    ...
    raw_data_mb = raw_data.to(MB).magnitude
    ...
    update_mb = update.to(MB).magnitude
...
::: {.column-margin}
![](images/svg/vol2_edge_intelligence_margin_004.svg){width="100%" fig-alt="Margin illustration showing Weekly raw image upload versus compressed model update."}
...
class EdgeFedNpuRecap:
    ...
    mobile_npu_tops = Hardware.Mobile.iPhone15Pro.compute.peak_flops.to(TOPS).magnitude
    iot_tops = 0.03 * TOPS
...
class EdgeFedPowerEnvelopeRecap:
    ...
    sustained_low = Scenarios.MobilePower.MobileMlSustainedLow
    sustained_high = Scenarios.MobilePower.MobileMlSustainedHigh
    background_low = Scenarios.MobilePower.BackgroundAdaptationLow
    background_high = Scenarios.MobilePower.BackgroundAdaptationHigh
    sustained_low_w = sustained_low.to(watt).magnitude
    sustained_high_w = sustained_high.to(watt).magnitude
    background_low_mw = background_low.to(mW).magnitude
    background_high_mw = background_high.to(mW).magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(sustained_low_w < sustained_high_w, "Sustained mobile power range should be ordered.")
    check(background_low_mw < background_high_mw, "Background mobile power range should be ordered.")
...
from mlsysim import Scenarios
from mlsysim.core.constants import milliwatt, SEC_PER_HOUR, THOUSAND, ureg
from mlsysim.fmt import check, fmt_qty_range
...
class EdgeAdaptationPowerMetrics:
    ...
    background_low_kj_per_hour = background_low_mw / THOUSAND * SEC_PER_HOUR / THOUSAND
    background_high_kj_per_hour = background_high_mw / THOUSAND * SEC_PER_HOUR / THOUSAND
    ...
    background_energy_per_hour_str = fmt_qty_range(
        background_low_kj_per_hour * ureg.kilojoule,
        background_high_kj_per_hour * ureg.kilojoule,
        ureg.kilojoule,
        precision=1,
        commas=False,
