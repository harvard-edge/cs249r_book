...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  ...
  % Clients
  \node[draw, fill=ClientColor, minimum width=2cm, minimum height=1cm] (clientA) at (0, 4) {Client A\\$W_A$};
  \node[draw, fill=ClientColor, minimum width=2cm, minimum height=1cm] (clientB) at (6, 4) {Client B\\$W_B$};

  % Secret Exchange
  \draw[<->, dashed, thick, SecretColor] (clientA) -- node[above, font=\scriptsize, align=center] {Shared Secret Mask\\$M_{AB}$} (clientB);

  % Mask Application
  \node[draw, dashed, fill=white, below=0.5cm of clientA] (masA) {$W_A + M_{AB}$};
