...
# │ How: CPU baseline from SingleNodeModel; NPU ratios are asserted chapter anchors.
# │
# └─────────────────────────────────────────────────────────────────────────────
...
y = W x + b
$$
where $\mathbf{W} \in \mathbb{R}^{m \times n}$ is the weight matrix, $b \in \mathbb{R}^m$ is the bias vector, and $x \in \mathbb{R}^n$ is the input. In full training, gradients are computed for both $W$ and $b$. In bias-only adaptation, we constrain:
$$
...
::: {#cs-edge-intelligence-tesla-autopilot-2016 .callout-case-study title="The edge model with a human fallback"}
