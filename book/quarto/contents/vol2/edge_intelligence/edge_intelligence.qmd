...
These scenarios require on-device learning, where models train and adapt directly on the devices where they operate.[^fn-a11-bionic-ondevice] Machine learning then moves from centralized training to distributed learning across heterogeneous devices, each operating under local constraints. Within the fleet stack, this pushes computation toward the limits of device power, memory, and thermal capacity.
...
[^fn-lora-parameter-efficiency]: **LoRA**\index{LoRA!parameter efficiency}: @hu2021lora learns rank-$r$ decomposition matrices $\mathbf{A}$ and $\mathbf{B}$ instead of updating full weight matrices, reducing trainable parameters by 100--10,000$\times$ while maintaining much of the adaptation quality. For on-device learning, this compression is decisive, but it affects adapter state rather than the full model footprint: a model with `{python} EdgeLoraParameterEfficiency.base_model_params_b_str` parameters has about a `{python} EdgeLoraParameterEfficiency.fp16_weight_footprint_gb_str` FP16 weight footprint before gradients, optimizer state, and activations, while rank `{python} EdgeAdaptationTierProfile.mid_tier_lora_rank_str` LoRA can reduce the trainable adapter parameters to tens or hundreds of megabytes. Practical phone deployment still requires storing, quantizing, or offloading the frozen base model.
...
**Scenario**: Large-scale mobile keyboard systems (e.g., Gboard) deployed across hundreds of millions of mobile devices serve as a canonical pattern for federated on-device adaptation [@mcmahan2017communication].

**Mechanism**: Next-word prediction models adapt locally while keeping raw keystrokes on device. Training runs strictly when the device is idle, charging, and connected to unmetered Wi-Fi, keeping memory overhead $M_{\text{peak}} < 50\text{ MB}$ to prevent OS process eviction. Quantized, compressed update vectors reduce network transfer $D_{\text{vol}} / \text{BW}_{\text{net}}$ before transmission to secure aggregation servers.

**Systems lesson**: On-device learning is viable only when local resource constraints ($M_{\text{peak}}$, $E_{\text{battery}}$), communication payload ($D_{\text{vol}} / \text{BW}$), and privacy guarantees are engineered together. The model update is an end-to-end systems protocol, not just a local SGD step.
...
2. **One Federated Uplink Round**\index{Federated Learning!model updates}: A compressed model update (5M params) is `{python} FederatedSavings.fs_update_mb_str`.
3. **One-Round Uplink Reduction**\index{Bandwidth!reduction}: `{python} FederatedSavings.fs_raw_data_mb_str`/`{python} FederatedSavings.fs_update_mb_str` = `{python} FederatedSavings.fs_reduction_mult_str`.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, node distance=1.2cm]
  ...
  \node[DecisionBlock, right=1.0cm of Arbiter, font=\scriptsize\usefont{T1}{phv}{m}{n}] (Logic) {Rollback gate\\$C_{\text{local}} < C_{\text{base}} - \epsilon$};
...
**Context**: A 2015 Tesla Model S operating with Mobileye EyeQ3/NVIDIA Mobile SoC hardware performed perception and control decisions locally under strict real-time constraints ($L_{\text{lat}} \le 50\text{ ms}$ for safety loop execution), relying on human supervision [@ntsb2017tesla].
...
**Impact**: The NTSB post-mortem identified driver overreliance and missing attention safeguards as root causes, prompting fleet-wide sensor upgrades (Radar/Vision fusion with $10\times$ local compute) and mandatory hands-on-wheel monitoring.
