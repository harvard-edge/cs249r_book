...
Traditional machine learning systems rely on a clear division of labor between model training and inference: centralized environments with high-performance compute resources and large-scale datasets handle training, while client devices receive the trained models and operate in a static inference-only mode. The centralized paradigm works well when data collection is feasible, connectivity is reliable, and a single global model can serve all users, but it breaks down when data is user-specific, behavior is dynamic, or connectivity is intermittent.
...
# │          stm32_hidden_*_str
from mlsysim import BYTES_FP32, Datasets, KB, MB, Models, byte, param
from mlsysim.fmt import (
    check,
    fmt_int,
    fmt_math,
...
class EdgeMobilenetSizing:
    ...
    bytes_per_channel = 1 * byte  # scenario assumption: uint8 RGB input tensor
...
Full-model fine-tuning is typically infeasible on edge platforms. Localized update strategies, including bias-only optimization, residual adapters, and lightweight task-specific heads, keep model specialization within resource constraints while mitigating the risks of overfitting or instability.
...
These layers only work as a system when the integration policy protects against capability mismatch, component failure, resource conflicts, and untested emergent behavior:
...
Heterogeneity is not background variation; it determines which devices can train, which can validate, and which can only contribute lightweight signals. Federated and on-device ML systems span smartphones, wearables, IoT sensors, and microcontrollers across hardware capabilities, software stacks, network connectivity, and power availability. Unlike cloud-based systems, where environments can be standardized and controlled, edge deployments encounter a wide distribution of configurations that directly shape algorithm design, resource scheduling, and model rollout.
