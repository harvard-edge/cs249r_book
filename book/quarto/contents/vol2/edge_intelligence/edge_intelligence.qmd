...
[^fn-lora-parameter-efficiency]: **LoRA**: @hu2021lora learns rank-$r$ decomposition matrices $A$ and $B$ instead of updating full weight matrices, reducing trainable parameters by 100--10,000$\times$ while maintaining much of the adaptation quality. For on-device learning, this compression is decisive, but it affects adapter state rather than the full model footprint: a model with `{python} EdgeLoraParameterEfficiency.base_model_params_b_str` parameters has about a `{python} EdgeLoraParameterEfficiency.fp16_weight_footprint_gb_str` FP16 weight footprint before gradients, optimizer state, and activations, while rank `{python} EdgeAdaptationTierProfile.mid_tier_lora_rank_str` LoRA can reduce the trainable adapter parameters to tens or hundreds of megabytes. Practical phone deployment still requires storing, quantizing, or offloading the frozen base model. \index{LoRA!parameter efficiency}
...
### Data-efficient continual adaptation {#sec-edge-intelligence-bioinspired-learning-efficiency-55ad}
...
These layers only work as a system when the integration policy protects against capability mismatch, component failure, resource conflicts, and untested emergent behavior. @Tbl-edge-integration-controls maps each system risk to the control that keeps a tiered deployment from becoming a collection of disconnected techniques.

| **Integration risk** | **Control**                                                              | **Why it matters**                                                                                                                                   |
|:---------------------|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------|
| Capability mismatch  | Hierarchical capability matching\index{Hierarchical Capability Matching} | Deploys more sophisticated techniques on capable devices while preserving basic functionality across the full device spectrum.                       |
| Component failure    | Graceful degradation\index{Graceful Degradation}                         | Keeps local adaptation useful when connectivity is poor or battery constraints force the system into a minimal adaptation mode.                      |
...
| Emergent behavior    | Performance validation\index{Validation!performance}                     | Tests combinations of device tier, network state, and adaptation policy because integrated systems fail in ways individual techniques do not expose. |

: **Edge-Learning Integration Controls**\index{Edge Learning!integration controls}: A tiered edge-learning deployment needs explicit controls for capability matching, graceful degradation, resource conflicts, and validation across device and network combinations. {#tbl-edge-integration-controls}
...
[^fn-arm-cortex-heterogeneity]: **ARM Cortex Spectrum**\index{ARM Cortex Spectrum}: Representative endpoints span nearly six orders of magnitude, from Cortex-M-class devices around `{python} EdgeDeviceSpectrum.tiny_clock_mhz_str`, `{python} EdgeDeviceSpectrum.tiny_ram_kb_str` RAM, and `{python} EdgeDeviceSpectrum.sensor_power_uw_str` power to mobile-class devices around `{python} EdgeDeviceSpectrum.mobile_clock_ghz_str`, `{python} EdgeDeviceSpectrum.flagship_ram_gb_str` RAM, and `{python} EdgeDeviceSpectrum.phone_power_w_str` [@warden2020tinyml; @lai2018cmsis]. This diversity forces federated learning to tier algorithms by device class: quantized inference on the smallest microcontrollers, lightweight bias-only updates on larger embedded or wearable tiers, and fuller adapter or backpropagation loops only on mobile-class SoCs. A single federated round must accommodate these tiers simultaneously without biasing the global model toward any one tier. \index{ARM Cortex!device heterogeneity}
