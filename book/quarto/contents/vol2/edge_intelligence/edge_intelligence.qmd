...
The interaction between local epochs and communication rounds creates a fundamental design trade-off, visualized in @fig-comm-compute-tradeoff. More local epochs reduce communication frequency but increase client drift, while fewer local epochs maintain tighter synchronization at higher communication cost.

::: {#fig-comm-compute-tradeoff fig-env="figure" fig-pos="htb" fig-cap="**Communication-Computation Trade-off.** For IID data, increasing local epochs consistently reduces total communication with minimal penalty. For non-IID data, client drift causes convergence degradation beyond an optimal point (typically $E = 2$-$5$), making aggressive local computation counterproductive. System designers must estimate data heterogeneity to select appropriate operating points." fig-alt="Plot of rounds to convergence vs local epochs showing flat IID curve and rising non-IID curve with sweet spot annotation."}
...
The shadow validation approach described above operates as a continuous comparison pipeline running on each device (@fig-shadow-validation), where incoming data flows through both a frozen baseline and the locally adapted model before an arbiter decides whether to accept or roll back adaptations.
