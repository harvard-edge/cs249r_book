...
class EdgeDeviceSpectrum:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tiny_ram_kb_str = fmt_memory_capacity(tiny_ram, unit=KB, precision=0, commas=False)
    mcu_sram_kb_str = fmt_memory_capacity(mcu_sram, unit=KB, precision=0, commas=False)
    ...
    flagship_ram_gb_str = fmt_memory_capacity(flagship_ram, unit=GB, precision=0, commas=False)
...
class EdgeAdaptationTierProfile:
    ...
    budget_phone_ram_gb_str = fmt_memory_capacity(budget_phone_ram, unit=GB, precision=0, commas=False)
    budget_phone_available_low_gb_str = fmt_memory_capacity(budget_phone_available_low, unit=GB, precision=0, commas=False)
    budget_phone_available_high_gb_str = fmt_memory_capacity(budget_phone_available_high, unit=GB, precision=0, commas=False)
    budget_device_memory_limit_gb_str = fmt_memory_capacity(budget_device_memory_limit, unit=GB, precision=0, commas=False)
    budget_keyboard_phone_ram_gb_str = fmt_memory_capacity(budget_keyboard_phone_ram, unit=GB, precision=0, commas=False)
    iot_memory_low_mb_str = fmt_memory_capacity(iot_memory_low, unit=MB, precision=0, commas=False)
    iot_memory_high_gb_str = fmt_memory_capacity(iot_memory_high, unit=GB, precision=0, commas=False)
    ...
    budget_replay_buffer_mb_str = fmt_memory_capacity(budget_replay_buffer, unit=MB, precision=0, commas=False)
    flagship_replay_buffer_mb_str = fmt_memory_capacity(flagship_replay_buffer, unit=MB, precision=0, commas=False)
    ...
    federated_lora_update_mb_str = fmt_memory_capacity(federated_lora_update, unit=MB, precision=0, commas=False)
```
...
[^fn-a11-bionic-ondevice]: **A11 Bionic (2017)**\index{A11 Bionic!definition}: Apple's first SoC with a dedicated Neural Engine, rated up to 600 billion operations per second for machine-learning inference workloads. The systems consequence was establishing the mobile neural processing unit (NPU) as a distinct power domain: on-device adaptation becomes more practical when inference-optimized silicon frees thermal headroom for short local training or personalization bursts.
...
[^fn-non-iid-federated]: **Non-IID (Not Independent and Identically Distributed)**\index{Non-IID (Not Independent and Identically Distributed)!definition}: Data where samples are not drawn from a single common distribution, the default condition in federated learning where each device reflects a single user's patterns. Non-IID label and feature distributions can sharply degrade federated averaging compared with IID baselines [@zhao2018federated], forcing systems to adopt techniques like local adaptation layers or clustered aggregation to maintain convergence.
...
[^fn-lora-parameter-efficiency]: **LoRA**\index{LoRA!definition}: @hu2021lora learns rank-$r$ decomposition matrices $\mathbf{A}$ and $\mathbf{B}$ instead of updating full weight matrices, reducing trainable parameters by 100--10,000$\times$ while maintaining much of the adaptation quality. For on-device learning, this compression is decisive, but it affects adapter state rather than the full model footprint: a model with `{python} EdgeLoraParameterEfficiency.base_model_params_b_str` parameters has about a `{python} EdgeLoraParameterEfficiency.fp16_weight_footprint_gb_str` FP16 weight footprint before gradients, optimizer state, and activations, while rank `{python} EdgeAdaptationTierProfile.mid_tier_lora_rank_str` LoRA can reduce the trainable adapter parameters to tens or hundreds of megabytes. Practical phone deployment still requires storing, quantizing, or offloading the frozen base model.
...
[^fn-tinyml-market-scale]: **TinyML Scale**\index{TinyML Scale!definition}\index{TinyML!market scale}: Over 100 billion microcontrollers ship annually, but fewer than 1 percent support on-device learning due to memory ($<$256 KB) and power ($<$1 mW) constraints. The gap between deployed hardware and ML-capable hardware defines the engineering challenge: enabling experience replay and adaptation on chips costing under \$1 requires algorithms designed for kilobytes, not gigabytes.
...
[^fn-wireless-bandwidth-edge]: **Wireless Upload Asymmetry**\index{Wireless Upload Asymmetry!definition}: LTE uploads average 5--10 Mbps vs. 50+ Mbps downloads, creating the asymmetric bottleneck that dominates federated learning design. Transmitting a 50 MB model update consumes approximately 100 mAh (2--3 percent of battery) and takes 40--80 seconds over LTE. Low-power protocols are far worse: LoRaWAN maxes at 50 kbps with 1 percent duty cycle, making uncompressed updates physically impossible. This asymmetry is why gradient compression is not optional but architecturally required.
...
[^fn-gradient-sparsification-topk]: **Gradient Sparsification**\index{Gradient Sparsification!definition}: Transmits only the top 1--10 percent of gradients by magnitude, exploiting the observation that most gradient elements are near-zero and contribute minimally to convergence. Local gradient accumulation stores the untransmitted residuals until they grow large enough to send, achieving 10--100$\times$ compression while preserving training quality, a pattern used in sparsified SGD and deep gradient compression [@stich2018sparsified; @lin2018deep]. The trade-off: aggressive sparsification can bias updates toward frequently active parameters, potentially reducing personalization for rare user patterns.
...
[^fn-cloudlet-hybrid-training]: **Cloudlet**\index{Cloudlet!definition}: A small-scale compute tier located close to users, often near access networks or enterprise sites, provides lower-latency offload than a distant cloud region. This intermediate tier enables hybrid on-device learning: time-sensitive inference runs locally while aggregation, validation, or heavier adaptation work can offload nearby when the network and trust model allow it. The trade-off is deployment complexity: cloudlets add an orchestration layer that must decide per-update whether to compute locally, offload nearby, or defer to cloud.
