...
3.  **Pitfall**: A frequent misconception is that on-device learning requires training from scratch. In reality, it is almost always On-Device Adaptation\index{On-Device Adaptation}: fine-tuning a small subset of a pretrained base model's parameters to fit the specific data distribution of a local environment.
...
- [ ] **Training memory blowup**: Why does on-device training typically require 4$\times$ to 12$\times$ more memory than on-device inference for the same model?
- [ ] **Starved NPU**: How can a fast NPU still sit idle if the training pipeline cannot keep data and activations moving within the device's memory budget?
- [ ] **Backward-pass OOM**: If a smartphone has `{python} EdgeAdaptationTierProfile.smartphone_ram_gb_str` of RAM, why might a 100M parameter model still trigger out-of-memory errors during the backward pass?
- [ ] **Checkpointing energy**: True or False: Gradient checkpointing reduces the total energy consumed by on-device training.
...
class StorageWall:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    size_full = memory_from_params(model_params_m * Mparam, bytes_per_param * byte / param)
    size_adapter = memory_from_params(adapter_params * param, bytes_per_param * byte / param)
...
- [ ] **Adapter switching**: Why is Adapter Switching more storage-efficient than maintaining separate full-model copies for different user contexts?
- [ ] **Adapter size**: How does the size of a **rank-64 adapter** compare to a standard 10M parameter vision model?
- [ ] **Residual vs. LoRA**: In which deployment tier (Tier 1, 2, or 3) are **Residual Adapters** preferred over LoRA, and why?
- [ ] **Adapter bandwidth**: True or False: Transmitting adapter weights in federated learning consumes more bandwidth than transmitting the entire model.
...
Let a neural network be defined by parameters $\theta = \{\theta_1, \theta_2, \ldots, \theta_{N_L}\}$ across $N_L$ layers. In standard fine-tuning, we compute gradients and perform updates on all parameters:
$$
\theta_i \leftarrow \theta_i - \eta \frac{\partial \mathcal{L}}{\partial \theta_i}, \quad \text{for } i = 1, \ldots, N_L
$$

In task-adaptive sparse updates, we select a small subset $\mathcal{S} \subset \{1, \ldots, N_L\}$ such that only parameters in $\mathcal{S}$ are updated:
$$
\theta_i \leftarrow
\begin{cases}
...
2.  **Privacy**: With a strict privacy budget of $\varepsilon < 5$, how much noise must be injected, and how does that impact convergence time?
