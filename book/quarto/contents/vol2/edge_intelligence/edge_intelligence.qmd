...
::: {#fig-ondevice-pretraining fig-env="figure" fig-pos="htb" fig-cap="**On-Device Pre-Training and Selective Fine-Tuning**: A two-stage pipeline for IoT adaptation. Left: offline meta-training of the full neural network on generic cloud data. Center: on-device constraints (data availability, compute, memory) feed an importance-ranking step that chooses which layers and channels to update. Right: only the selected layers (highlighted) are fine-tuned on-device, keeping the rest of the network frozen." fig-alt="Two-stage pipeline. Left: offline pre-training with full neural network. Center: device constraints (data, compute, memory) feed into importance ranking. Right: selected layers highlighted in green for on-device training."}
