...
Part II established the infrastructure for distributed ML at scale: datacenter architectures optimized for GPU clusters (@sec-infrastructure), storage systems engineered for petabyte datasets (@sec-storage), distributed training across thousands of accelerators (@sec-distributed-training), communication patterns that coordinate gradient synchronization (@sec-communication), and fault tolerance mechanisms that handle inevitable failures (@sec-fault-tolerance). The preceding chapter (@sec-inference-at-scale) then examined how to serve predictions from this infrastructure, addressing load balancing, model sharding, and autoscaling for centralized inference systems. These chapters share a common assumption: controlled environments where computational resources are abundant, network connectivity is reliable, and system behavior is predictable.
...
On-device learning constraints fall into three critical dimensions that define the efficiency framework: model compression requirements (algorithmic efficiency), sparse and non-uniform data characteristics (data efficiency), and severely limited computational resources (compute efficiency). These three dimensions form an interconnected constraint space that defines the feasible region for on-device learning systems, with each dimension imposing distinct limitations that influence algorithmic choices, system architecture, and deployment strategies.
...
Standard efficiency constraints (quantization, pruning, knowledge distillation) apply to both inference and training, but training amplifies each constraint dimension by 3-10×. @tbl-training-amplification quantifies how training workloads intensify the challenges of model efficiency, optimization techniques, and hardware acceleration.
...
Memory consumption during training is not static; it fluctuates dynamically, reaching a maximum during the backward pass when activations, gradients, and optimizer states must coexist. This **peak memory usage** determines whether a model can be trained on a device. A model that averages 50MB usage might spike to 200MB during backpropagation, causing an Out-Of-Memory (OOM) crash on a device with 128MB RAM. Techniques like **gradient checkpointing** mitigate this by discarding intermediate activations during the forward pass and recomputing them on-demand during the backward pass, effectively trading increased computation (20-30%) for a dramatic reduction in peak memory (often 3-4x).
