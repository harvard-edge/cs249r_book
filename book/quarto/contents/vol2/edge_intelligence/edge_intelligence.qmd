...
[^fn-neural-engine-evolution]: **Apple Neural Engine**\index{Apple Neural Engine}: From the A11 (0.6 TOPS) to the A17 Pro (`{python} EdgeIntelligenceSetup.mobile_npu_tops` TOPS), an approximately 83$\times$ improvement in six years. The systems consequence: fine-tuning a MobileNet classifier takes approximately 2 seconds on the Neural Engine vs. 45 seconds on CPU, while consuming only approximately 500 mW additional power. This roughly 23$\times$ speedup at minimal power cost is what makes on-device adaptation thermally feasible during normal phone usage. \index{Neural Engine!on-device training}
