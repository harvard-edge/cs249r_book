...
<!-- TODO(focused-followup): Items deferred from the codex deep-review
     ...
     - @fig-... Local Only vs Centralized Cloud A/B/C region labels
       inconsistent between figure and prose (line ~308).
     - Constraint-solution mapping table missing the Federated
       Coordination pillar row (line ~972); only Resource Amplification
       and Information Scarcity are covered.
     - Convergence-bound math F(theta) vs. theta-star (lines ~2255 and
       2271): heterogeneity multiplier breaks the beta=0 -> IID-rate
       sanity check. Use standard comparator theta-star and express
       heterogeneity as an additive penalty term.
     - Client scheduling repetition (lines ~2383 vs. ~2385): stratified
       sampling and adaptive client selection re-stated in adjacent
       paragraphs; merge.
     - Performance-metrics paragraph (line ~2976): comma splices and
       overlong sentences; split or convert to compact list.
     - Non-protected callout-notebook second-person + Qs at lines 160,
       921, 1024, 1819; convert to declarative scenarios.
     - Adaptation-strategy table caption vs. row contents mismatch
       (line ~1605): caption names "full finetuning" which is not a
       row; rows are Bias-Only Updates, Residual Adapters, Sparse Layer
       Updates.
     - "Worked Example: Federated Learning with 100 Clients" plaintext
       label (~line 2279): convert to subsection or worked-example
       callout.
     - "$E = 2$-$5$" math en-dash inside math fragments (~line 2309). -->
