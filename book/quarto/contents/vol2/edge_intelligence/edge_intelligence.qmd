...
Data center ML systems assume a controlled environment where computational resources are abundant, network connectivity is reliable, and system behavior is predictable. Centralized inference (@sec-inference-scale) is the clearest case of that assumption, and the edge breaks it.
...
1.  **Significance (quantitative)**: It enables Hyper-Personalization and autonomous operation under severe resource constraints. Within the iron law\index{Iron Law}, on-device learning must minimize the total energy consumed per update, because every gradient step draws on limited battery power and competes with other system tasks for the available compute throughput $(R_{\text{peak}})$.
...
The compression techniques that make these small updates possible (gradient quantization, sparsification, and top-$k$ selection with error accumulation) are the same ones established in @sec-edge-intelligence-bandwidthaware-update-compression-7730; here they serve the participation argument rather than the bandwidth argument, because shrinking the update is what widens the eligible device pool.
...
A billion-user predictive text model that learns from live keyboard inputs can be poisoned if a coordinated group of malicious users repeatedly types a specific slur and those local updates enter the global federated aggregate. The shift to on-device learning exposes the system to unprecedented security and robustness threats, and engineering mitigations must actively defend against both adversarial attacks and the natural statistical drift of the real world. The strongest single mitigation, and the chapter's central payoff, is to stop treating the three pillars as separate techniques: this section first shows how to integrate model adaptation, data efficiency, and federated coordination by device tier, then turns to the heterogeneity, observability, resource, and failure challenges that integration leaves open.
