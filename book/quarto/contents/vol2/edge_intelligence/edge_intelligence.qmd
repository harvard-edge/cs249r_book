...
[^fn-non-iid-federated]: **Non-IID (Not Independent and Identically Distributed)**\index{Non-IID Data!definition}: Data where samples are not drawn from a single common distribution, the default condition in federated learning where each device reflects a single user's patterns. Non-IID label and feature distributions can sharply degrade federated averaging compared with IID baselines [@zhao2018federated], forcing systems to adopt techniques like local adaptation layers or clustered aggregation to maintain convergence.
