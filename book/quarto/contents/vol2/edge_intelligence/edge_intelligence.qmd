...
# │
# │ Exports: EdgeDeviceSpectrum.tiny_ram_kb_str,
# │          EdgeDeviceSpectrum.mcu_sram_kb_str,
# │          EdgeDeviceSpectrum.mcu_sram_high_mb_str,
# │          EdgeDeviceSpectrum.arduino_flash_mb_str,
# │          EdgeDeviceSpectrum.flagship_ram_gb_str,
# │          EdgeDeviceSpectrum.memory_ratio_mult_str,
# │          EdgeDeviceSpectrum.tiny_clock_mhz_str,
# │          EdgeDeviceSpectrum.mobile_clock_ghz_str,
# │          EdgeDeviceSpectrum.tiny_cpu_mips_str,
# │          EdgeDeviceSpectrum.mobile_cpu_mips_str,
# │          EdgeDeviceSpectrum.cpu_throughput_ratio_mult_str,
# │          EdgeDeviceSpectrum.sensor_power_uw_str,
# │          EdgeDeviceSpectrum.phone_power_w_str,
# │          EdgeDeviceSpectrum.power_ratio_mult_str,
# │          EdgeDeviceSpectrum.iot_compute_tops_str
# └─────────────────────────────────────────────────────────────────────────────
...
class EdgeDeviceSpectrum:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(tiny_ram < mcu_sram < mcu_sram_high < flagship_ram,
          "Device memory endpoints should be ordered.")
    check(50_000 < memory_ratio < 70_000, f"MCU-to-phone memory ratio drifted: {memory_ratio:.0f}x")
    check(500_000 <= endpoint_memory_ratio <= 600_000,
          f"Tiny-to-phone memory ratio drifted: {endpoint_memory_ratio:.0f}x")
    check(9_999 <= cpu_throughput_ratio <= 10_001,
          f"Tiny-to-mobile CPU throughput ratio drifted: {cpu_throughput_ratio:.0f}x")
    ...
    mcu_sram_high_mb_str = fmt_memory_capacity(mcu_sram_high, unit=MB, precision=0, commas=False)
    arduino_flash_mb_str = fmt_memory_capacity(arduino_flash, unit=MB, precision=0, commas=False)
    ...
    power_ratio_mult_str = fmt_multiple(power_ratio, precision=0, commas=True)
...
# │
# │ Exports: EdgeAdaptationTierProfile.wearable_system_memory_mb_str,
# │          EdgeAdaptationTierProfile.tiny_wearable_ram_mb_str,
# │          EdgeAdaptationTierProfile.budget_phone_ram_gb_str,
# │          EdgeAdaptationTierProfile.budget_phone_available_low_gb_str,
# │          EdgeAdaptationTierProfile.budget_phone_available_high_gb_str,
# │          EdgeAdaptationTierProfile.budget_device_memory_limit_gb_str,
# │          EdgeAdaptationTierProfile.budget_keyboard_phone_ram_gb_str,
# │          EdgeAdaptationTierProfile.iot_memory_low_mb_str,
# │          EdgeAdaptationTierProfile.iot_memory_high_gb_str,
# │          EdgeAdaptationTierProfile.keyboard_gradient_memory_low_mb_str,
# │          EdgeAdaptationTierProfile.keyboard_gradient_memory_high_mb_str,
# │          EdgeAdaptationTierProfile.keyboard_background_budget_low_mb_str,
# │          EdgeAdaptationTierProfile.keyboard_background_budget_high_mb_str,
# │          EdgeAdaptationTierProfile.keyboard_memory_share_pct_str,
# │          EdgeAdaptationTierProfile.fleet_devices_million_str,
# │          EdgeAdaptationTierProfile.flagship_share_pct_str,
# │          EdgeAdaptationTierProfile.mid_tier_share_pct_str,
# │          EdgeAdaptationTierProfile.budget_share_pct_str,
# │          EdgeAdaptationTierProfile.flagship_lora_rank_str,
# │          EdgeAdaptationTierProfile.mid_tier_lora_rank_str,
# │          EdgeAdaptationTierProfile.budget_replay_buffer_mb_str,
# │          EdgeAdaptationTierProfile.flagship_replay_buffer_mb_str,
# │          EdgeAdaptationTierProfile.few_shot_interactions_range_str,
# │          EdgeAdaptationTierProfile.federated_lora_update_mb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import (
    check,
    fmt_count,
...
class EdgeAdaptationTierProfile:
    ...
    keyboard_memory_share = (
        keyboard_gradient_example / keyboard_background_budget_example
    ).to("").magnitude

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(tiny_wearable_ram < wearable_memory < smartphone_ram,
          "Wearable and phone memory envelopes should be ordered.")
    check(budget_phone_available_low < budget_phone_available_high <= budget_phone_ram,
    ...
    check(mid_tier_lora_rank < flagship_lora_rank,
          "Flagship LoRA rank should exceed mid-tier LoRA rank.")
    check(budget_replay_buffer < flagship_replay_buffer,
          "Replay buffers should grow with device capability.")
    check(few_shot_interactions_low < few_shot_interactions_high,
          "Few-shot interaction range should be ordered.")
    check(keyboard_gradient_memory_low < keyboard_gradient_example < keyboard_gradient_memory_high,
          "Keyboard gradient example should sit inside its memory range.")
    check(keyboard_background_budget_low < keyboard_background_budget_example <= keyboard_background_budget_high,
          "Keyboard background-budget example should sit inside its memory range.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    wearable_system_memory_mb_str = fmt_memory_capacity(wearable_memory, unit=MB, precision=0, commas=False)
    tiny_wearable_ram_mb_str = fmt_memory_capacity(tiny_wearable_ram, unit=MB, precision=0, commas=False)
    ...
    iot_memory_low_mb_str = fmt_memory(iot_memory_low, unit=MB, precision=0, commas=False)
    ...
    fleet_devices_million_str = fmt_count(
        fleet_devices, scale="million", scale_style="word", precision=0, label="device"
    )
    ...
    budget_replay_buffer_mb_str = fmt_memory(budget_replay_buffer, unit=MB, precision=0, commas=False)
    flagship_replay_buffer_mb_str = fmt_memory(flagship_replay_buffer, unit=MB, precision=0, commas=False)
    few_shot_interactions_range_str = fmt_count_range(
        few_shot_interactions_low, few_shot_interactions_high, label="interaction", commas=False
    )
    federated_lora_update_mb_str = fmt_memory(federated_lora_update, unit=MB, precision=0, commas=False)
```

## The Edge Learning Paradigm {#sec-edge-intelligence-distributed-learning-paradigm-shift-883d}

Imagine a voice assistant trained on millions of generic audio samples in a cloud data center. When deployed, it struggles to understand a user's heavy regional accent or specific household vocabulary. If the model cannot adapt locally on the device itself, it remains permanently frozen and becomes less useful as local usage diverges from the training distribution. The edge learning paradigm shifts ML from a *centralized factory model* to a *decentralized, continuously adapting network*.

Data center ML systems assume controlled environments where computational resources are abundant, network connectivity is reliable, and system behavior is predictable. Compute infrastructure (@sec-compute-infrastructure), storage systems (@sec-data-storage), distributed training (@sec-distributed-training-systems), collective communication (@sec-collective-communication), fault tolerance (@sec-fault-tolerance-reliability), and centralized inference (@sec-inference-scale) all rely on that assumption in different ways.
...
[^fn-a11-bionic-ondevice]: **A11 Bionic (2017)**\index{A11 Bionic!on-device learning}: Apple's first SoC with a dedicated Neural Engine, rated up to 600 billion operations per second for machine-learning inference workloads. The systems consequence was establishing the mobile NPU as a distinct power domain: on-device adaptation becomes more practical when inference-optimized silicon frees thermal headroom for short local training or personalization bursts.
...
# │          mlsysim.core.constants.THOUSAND; mlsysim.fmt.fmt, fmt_int, check
# │ Exports: npu_cpu_lat_ms_str, npu_lat_ms_str, npu_speedup_mult_str,
# │          npu_energy_gain_mult_str, npu_cpu_energy_mj_str,
# │          npu_energy_mj_str
# └─────────────────────────────────────────────────────────────────────────────
...
class EdgeNpuSpeedup:
    ...
    npu_lat_ms_str = fmt_time(t_npu, millisecond, precision=3, commas=False)
    npu_cpu_energy_mj_str = fmt_qty(e_cpu, mJ, precision=1, commas=False)
    npu_energy_mj_str = fmt_qty(e_npu, mJ, precision=3, commas=False)
...
1. **Latency Speedup**\index{Latency!speedup}: `{python} EdgeNpuSpeedup.npu_cpu_lat_ms_str`/`{python} EdgeNpuSpeedup.npu_lat_ms_str` ≈ `{python} EdgeNpuSpeedup.npu_speedup_mult_str`.
2. **Energy Efficiency**\index{Energy Efficiency}: `{python} EdgeNpuSpeedup.npu_cpu_energy_mj_str` (CPU)/`{python} EdgeNpuSpeedup.npu_energy_mj_str` (NPU) ≈ `{python} EdgeNpuSpeedup.npu_energy_gain_mult_str`.

**Systems insight**: Specialized hardware makes the model *feasible*, not merely faster. A `{python} EdgeNpuSpeedup.npu_energy_gain_mult_str` energy gain translates into `{python} EdgeNpuSpeedup.npu_energy_gain_mult_str` more inferences on the same battery charge. On the edge, the CPU is for coordination, but the NPU is for survival. Falling back to the CPU when the NPU target is missed does more than cost performance: it typically renders the application unusable through rapid battery drain and thermal throttling.
...
2.  **Observation**: Unlike Edge Inference\index{Edge Inference}, which only executes a fixed model, on-device learning involves a Local Optimization Loop (forward and backward passes) that requires significantly more memory and compute.
3.  **Pitfall**: A frequent misconception is that on-device learning requires training from scratch. In reality, it is almost always On-Device Adaptation\index{On-Device Adaptation}: fine-tuning a small subset of a pretrained "Teacher" model to fit the specific data distribution of a local environment.
...
Cloud-based fine-tuning with privacy controls enables personalization through centralized adaptation with appropriate privacy safeguards. User data is processed in batches during off-peak hours using privacy-preserving techniques such as differential privacy[^fn-dp-federated] or federated analytics, which computes aggregate statistics across devices without training a shared model. When paired with secure aggregation, where the server observes only an aggregate rather than individual device contributions, or with differential privacy, federated analytics can reduce exposure of per-device records. This approach often achieves superior accuracy compared to resource-constrained on-device updates while maintaining acceptable privacy properties for many applications.

[^fn-dp-federated]: **Differential Privacy (DP)**\index{Differential Privacy!federated learning}: A mathematical framework that bounds information leakage by adding calibrated noise to computations. The key parameter $\varepsilon$ controls the privacy-utility trade-off: typical federated deployments use $\varepsilon = 1$--$8$, where smaller values provide stronger privacy but reduce model accuracy by 1--5 percent and increase communication overhead by 2--10$\times$. This noise tax is the fundamental cost of privacy-preserving on-device learning: every gradient update becomes noisier, requiring more communication rounds to converge.
...
Mobile input prediction is a well-documented example of on-device learning. In systems such as smartphone keyboards, predictive text and autocorrect features benefit substantially from continuous local adaptation. User typing patterns are highly personalized and evolve dynamically, making centralized static models insufficient for high-quality user experience. On-device learning allows language models to fine-tune their predictions directly on the device, achieving personalization while maintaining data locality. Google's Gboard[^fn-gboard-federated-scale], for instance, employs federated learning[^fn-federated-metaphor] to improve shared models across a large population of users while keeping raw data local to each device [@hard2018federated].

[^fn-gboard-federated-scale]: **Gboard (2017)**\index{Gboard!federated learning}: One of the first widely reported commercial federated learning deployments at mobile scale. Published reports showed that keyboard models could improve from aggregated on-device updates while raw typed text remained local. The systems consequence is durable: compressed updates and privacy mechanisms make cross-fleet learning possible over constrained mobile networks without centralizing raw keystrokes.

@Fig-ondevice-gboard demonstrates how different prediction strategies\index{On-Device Prediction Strategy} enable local adaptation in real time. Next-word prediction suggests likely continuations based on prior text, while Smart Compose uses on-the-fly rescoring to offer dynamic completions. These techniques demonstrate the sophistication of local inference mechanisms.
...
[^fn-non-iid-federated]: **Non-IID (Not Independent and Identically Distributed)**\index{Non-IID!federated learning}: Data where samples are not drawn from a single common distribution, the default condition in federated learning where each device reflects a single user's patterns. Non-IID data causes federated averaging to diverge: experiments show accuracy drops of 10--50 percent compared to IID baselines when label distributions are skewed across clients, forcing systems to adopt techniques like local adaptation layers or clustered aggregation to maintain convergence.
...
Three efficiency dimensions define the on-device learning design space: algorithmic efficiency (how many parameters must change), compute efficiency (how many operations per update), and data efficiency (how many examples are needed). These dimensions interact *multiplicatively* rather than *additively*, creating a constrained optimization problem far more challenging than inference-only deployment. Quantization shrinks bytes per weight, pruning removes unnecessary parameters, and knowledge distillation transfers behavior into a smaller architecture; on-device learning turns these compression tools from optional optimizations into baseline feasibility requirements.
...
Standard efficiency constraints apply to both inference and training. These constraints include quantization, pruning, and knowledge distillation. However, training amplifies memory footprint\index{Memory Footprint}, memory bandwidth\index{Memory Bandwidth}, and hardware utilization\index{Hardware Utilization} pressures in ways that are specific to local adaptation. @Tbl-training-amplification quantifies how compute operations grow 2--3$\times$ and energy consumption can balloon 10--50$\times$, while @fig-training-memory-amplifier shows a representative 9$\times$ Adam-style memory budget within that broader range.
...
- [ ] How can a fast NPU still sit idle if the training pipeline cannot keep data and activations moving within the device's memory budget?
- [ ] If a smartphone has `{python} EdgeAdaptationTierProfile.smartphone_ram_gb_str` of RAM, why might a 100M parameter model still trigger out-of-memory errors during the backward pass?
...
# │ Exports: mobilenetv2_*_str, imagenet_rgb_*,
# │          stm32_hidden_*_str
...
class EdgeMobilenetSizing:
    ...
    check(alpha10 > alpha05_total > alpha05_feature,
          "MobileNetV2 storage variants should shrink in the expected order")
    check(imagenet_width_px == 224 and imagenet_height_px == 224 and imagenet_channels == 3,
          "ImageNet RGB input shape should remain 224x224x3.")
    check(0.55 <= imagenet_rgb_sram_pct <= 0.60,
          f"ImageNet RGB SRAM share drifted: {imagenet_rgb_sram_pct:.2f}.")
    ...
    imagenet_rgb_shape_math = fmt_math(
        rf"{imagenet_width_px}{{\times}}{imagenet_height_px}{{\times}}{imagenet_channels}"
    )
...
# │ Exports: EdgeMobilePowerEnvelope.sustained_range_str,
# │          EdgeMobilePowerEnvelope.burst_range_str
# └─────────────────────────────────────────────────────────────────────────────
...
class EdgeMobilePowerEnvelope:
    ...
    check(sustained_low < sustained_high < burst_low < burst_high,
...
# │ Exports: EdgeH100BwRecap.h100_mem_bw_gb_per_s_str
...
While mobile NPUs deliver impressive TOPS, the "memory wall" that @sec-compute-memory-wall examines becomes an impassable barrier for large-scale models on edge devices. The same bandwidth wall that limits training updates is easiest to see in mobile language-model decode: if weights and KV state cannot stream through memory fast enough, no advertised TOPS figure can rescue the workload. Recent analysis [@ma2026challenges] highlights that autoregressive "Decode" operations are strictly bandwidth-bound.

The quantitative disparity is stark: data-center HBM3 on an H100 provides `{python} EdgeH100BwRecap.h100_mem_bw_gb_per_s_str`, while flagship mobile systems such as A17 Pro or Snapdragon 8 Gen 3 devices sit near 64--100 GB/s of LPDDR5X bandwidth.
...
Edge accelerator marketing emphasizes peak compute throughput (TOPS), but for autoregressive workloads the relevant ceiling is set by memory bandwidth, not arithmetic. Each generated token requires streaming the model's weights from memory to the compute units, so the iron law's Data term $(D_{\text{vol}} / \text{BW})$ dominates the Compute term whenever weights exceed on-chip cache, which is the case for billion-parameter models on mobile-class silicon.
...
A practical corollary: when ranking edge accelerator options for transformer decoding, the figure of merit is the GB/s a chip can sustain to its on-package memory, not the peak TOPS on its spec sheet. The two can diverge by orders of magnitude on mobile-class silicon.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MINUTES_PER_HOUR, hour, ureg, watt
...
[^fn-gradient-checkpoint-edge]: **Gradient Checkpointing**\index{Gradient Checkpointing!edge memory}: Trades computation for memory by recomputing intermediate activations during the backward pass instead of storing them, reducing memory requirements by 50--80 percent at the cost of 20--30 percent additional compute. On edge devices where memory is the binding constraint and compute cycles are relatively cheap, this trade-off is almost always favorable: it can make the difference between a model that fits in `{python} EdgeAdaptationTierProfile.budget_keyboard_phone_ram_gb_str` RAM and one that requires `{python} EdgeAdaptationTierProfile.smartphone_ram_gb_str`.
...
# │
# │ Imports: mlsysim.book (check)
# │ Exports: ls_full_mb_str, ls_adapter_kb_str, ls_savings_ratio_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
class StorageWall:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    size_full = model_params_m * MILLION * bytes_per_param
    size_adapter = adapter_params * bytes_per_param
...
3. **Storage Savings**\index{Storage!savings}: `{python} StorageWall.ls_total_full_mb_str`/`{python} StorageWall.ls_total_adapter_mb_str` $\approx$ `{python} StorageWall.ls_total_savings_ratio_mult_str` total device storage reduction.
4. **Per-Context Efficiency**\index{Adapter!per-context efficiency}: `{python} StorageWall.ls_full_mb_str`/`{python} StorageWall.ls_adapter_mb_str` = `{python} StorageWall.ls_savings_ratio_mult_str`.
...
@Lst-bias-adaptation makes the constraint explicit: all convolutional and fully connected weights remain frozen, while only bias terms adapt to local data.
...
# │ Exports: EdgeTinyTlRecap.tinytl_param_reduction_mult_str
...
# │ Exports: EdgeLoraParameterEfficiency.base_model_params_b_str,
# │          EdgeLoraParameterEfficiency.fp16_weight_footprint_gb_str
...
class EdgeLoraParameterEfficiency:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(6.5 <= base_model_params.to(Bparam).magnitude <= 7.5,
          "LoRA footprint example should remain 7B-parameter class.")
    check(13 <= fp16_weight_footprint.to(GB).magnitude <= 15,
          "FP16 footprint should remain about 14 GB.")
    # ┌── 4. OUTPUT ────────────────────────────────────────
...
[^fn-lora-parameter-efficiency]: **LoRA**\index{LoRA!parameter efficiency}: @hu2021lora learns rank-$r$ decomposition matrices $A$ and $B$ instead of updating full weight matrices, reducing trainable parameters by 100--10,000$\times$ while maintaining much of the adaptation quality. For on-device learning, this compression is decisive, but it affects adapter state rather than the full model footprint: a model with `{python} EdgeLoraParameterEfficiency.base_model_params_b_str` parameters has about a `{python} EdgeLoraParameterEfficiency.fp16_weight_footprint_gb_str` FP16 weight footprint before gradients, optimizer state, and activations, while rank `{python} EdgeAdaptationTierProfile.mid_tier_lora_rank_str` LoRA can reduce the trainable adapter parameters to tens or hundreds of megabytes. Practical phone deployment still requires storing, quantizing, or offloading the frozen base model.
...
A concrete example is TinyTrain, a method designed to allow rapid adaptation on-device [@kwon2024tinytrain]. TinyTrain pretrains a model along with meta-gradients, gradients computed during offline preparation to estimate which layers are worth updating under future device constraints. At runtime, the system dynamically selects layers to update based on task characteristics and available resources.
...
Together, these techniques turn data scarcity into an allocation problem: spend labels, memory, compute, and privacy budget where they produce the most reliable local signal.
...
[^fn-hey-siri-power-budget]: **"Hey Siri" Power Budget**\index{Hey Siri!power constraint}: The always-on detector consumes $<$1 mW while monitoring audio continuously, using a compact acoustic model on a low-power always-on processor rather than the main application cores. The constraint envelope is extreme: $<$100 ms detection latency, $<$0.1 false activations per hour, and $>$95 percent true positive rate across accents and noise conditions. Few-shot personalization must improve accuracy within this fixed power and model-size budget, meaning adaptation can tune classifier weights but cannot expand the model.
...
[^fn-tinyml-market-scale]: **TinyML Scale**\index{TinyML!scale}\index{TinyML!market scale}: Over 100 billion microcontrollers ship annually, but fewer than 1 percent support on-device learning due to memory ($<$256 KB) and power ($<$1 mW) constraints. The gap between deployed hardware and ML-capable hardware defines the engineering challenge: enabling experience replay and adaptation on chips costing under \$1 requires algorithms designed for kilobytes, not gigabytes.
...
2.  **Observation**: Unlike Centralized Training\index{Training!centralized}, where data is moved to the compute, federated learning moves the Compute to the Data\index{Compute to the Data}, ensuring that raw information never leaves the device.
3.  **Pitfall**: A frequent misconception is that federated learning is "inherently private." In reality, model updates themselves can leak information through Gradient Inversion attacks, requiring additional protections like differential privacy or secure aggregation.
...
# │
# │ Imports: mlsysim.book (check)
# │ Exports: fs_raw_data_mb_str, fs_update_mb_str, fs_reduction_mult_str
# └─────────────────────────────────────────────────────────────────────────────
...
This shift aligns closely with mobile, edge, and embedded systems because it preserves data locality while still improving the shared model. However, the privacy and personalization benefits are real only if the system also handles client variability, communication efficiency, and non-i.i.d. data distributions[^fn-non-iid-convergence] through specialized protocols and coordination mechanisms.
...
# │ Exports: fc_* formatted strings for the federated-convergence worked example.
# └─────────────────────────────────────────────────────────────────────────────
...
You are architecting a federated learning system for a fleet of 10 million mobile devices. The data is highly non-IID (users have distinct, clustered typing patterns), and the network environment is constrained (1-10 Mbps). Before choosing optimizations for this scenario, diagnose the bottleneck locally: constrained bandwidth points to communication-bound mitigations, while non-IID heterogeneity points to coordination-bound mitigations because client drift can dominate convergence even when each round is cheap.

Consider three design decisions:

1.  **Aggregation**: Do you use plain FedAvg for simplicity, reduce local epochs to limit client drift, or add personalization/clustering when one global model no longer fits the population?
...
The compression decision separates into three levers, each changing a different part of the communication bottleneck: model compression reduces bits per update, selective sharing reduces which parameters travel, and architectural partitioning keeps private state local.
...
All of these approaches operate within the FedAvg aggregation protocol (@sec-edge-intelligence-federated-aggregation-protocols-5d37): compression changes what each client transmits, not how the server combines updates.
...
# │          EdgeFedNpuRecap.iot_compute_tops_str,
# │          EdgeFedNpuRecap.compute_ratio_mult_str
...
# │ Exports: EdgeFedPowerEnvelopeRecap.background_range_str,
# │          EdgeFedPowerEnvelopeRecap.sustained_range_str
from mlsysim import ReferenceStats
...
# │ Exports: brain_tops_per_w_str, brain_vs_npu_low_str, brain_vs_npu_high_mult_str
...
Following this systematic flowchart yields a functional deployment, but exposing an adapting ML model to the real world surfaces failure modes that static inference does not. Models can drift, malicious actors can attempt to poison the aggregated gradients, and diverse hardware can expose edge-case bugs. The system must anticipate and harden against these engineering challenges.

## Engineering Challenges and Mitigations {#sec-edge-intelligence-systems-integration-production-deployment-c6bb}

A billion-user predictive text model that learns from live keyboard inputs can be poisoned if a coordinated group of malicious users repeatedly types a specific slur and those local updates enter the global federated aggregate. The shift to on-device learning exposes the system to unprecedented security and robustness threats. Engineering mitigations must actively defend against both adversarial attacks and the natural statistical drift of the real world.

A concrete way to bound these risks is to integrate the chapter's three pillars by device tier rather than deploy one adaptation policy everywhere. Consider a production voice assistant deployment across `{python} EdgeAdaptationTierProfile.fleet_devices_million_str`, where each layer constrains what the others are allowed to do.
...
The data efficiency layer implements adaptive strategies across the entire device population while respecting individual resource constraints. All devices implement experience replay[^fn-replay-forgetting], but with device-appropriate buffer sizes, `{python} EdgeAdaptationTierProfile.budget_replay_buffer_mb_str` on budget devices vs. `{python} EdgeAdaptationTierProfile.flagship_replay_buffer_mb_str` on flagship models, ensuring that memory-constrained devices can still benefit from replay-based learning. Few-shot learning enables rapid adaptation to new users within their first `{python} EdgeAdaptationTierProfile.few_shot_interactions_range_str`, reducing the cold-start problem[^fn-cold-start-personalization] that plagues systems requiring extensive training data. Streaming updates accommodate continuous voice pattern evolution as users' speaking styles naturally change over time or as they use the assistant in new acoustic environments.

[^fn-replay-forgetting]: **Experience Replay**\index{Experience Replay!forgetting prevention}: Borrowed from reinforcement learning, stores past input-output pairs in a buffer and interleaves them with new training data to prevent catastrophic forgetting. Memory-efficient implementations store compressed embeddings (64--512 dimensions) rather than raw inputs, reducing per-example storage from MB to KB. The buffer size directly determines forgetting rate: too small and old tasks degrade, too large and it competes with model weights for scarce edge memory.

[^fn-cold-start-personalization]: **Cold-Start Problem**\index{Cold-Start Problem}\index{Cold-Start Problem!personalization}: New users or devices have no local data for personalization, reducing initial engagement by 30--50 percent compared to personalized users. Meta-learned initializations (MAML) or privacy-preserving population statistics can bootstrap personalization, but the fundamental trade-off remains: faster cold-start requires more information from the global model, which may not represent the new user's distribution. Few-shot adaptation from `{python} EdgeAdaptationTierProfile.few_shot_interactions_range_str` offers one balance of speed and local relevance.

The federated coordination layer orchestrates privacy-preserving collaboration across the device population. Devices participate in federated training rounds opportunistically based on connectivity status and battery level, ensuring that coordination does not degrade user experience. LoRA adapters aggregate efficiently with just `{python} EdgeAdaptationTierProfile.federated_lora_update_mb_str` per update compared to `{python} EdgeLoraParameterEfficiency.fp16_weight_footprint_gb_str` for full model synchronization, making federated learning practical over mobile networks. Privacy-preserving aggregation protocols ensure that individual voice patterns never leave devices while still enabling population-scale improvements in accent recognition and language understanding that benefit all users.
...
Hardware tiers still shape the execution path. ARM Cortex-M-class devices, mobile-class A-series CPUs, and phones with dedicated NPUs[^fn-arm-cortex-heterogeneity] may all participate in the same federated population, but each tier supports a different adaptation envelope. The system must therefore assign model formats, training algorithms, validation duties, and update frequencies by device class rather than treating the fleet as uniform.

[^fn-arm-cortex-heterogeneity]: **ARM Cortex Spectrum**\index{ARM Cortex Spectrum}\index{ARM Cortex!device heterogeneity}: Representative endpoints span nearly six orders of magnitude, from Cortex-M-class devices around `{python} EdgeDeviceSpectrum.tiny_clock_mhz_str`, `{python} EdgeDeviceSpectrum.tiny_ram_kb_str` RAM, and `{python} EdgeDeviceSpectrum.sensor_power_uw_str` power to mobile-class devices around `{python} EdgeDeviceSpectrum.mobile_clock_ghz_str`, `{python} EdgeDeviceSpectrum.flagship_ram_gb_str` RAM, and `{python} EdgeDeviceSpectrum.phone_power_w_str` [@warden2020tinyml; @lai2018cmsis]. This diversity forces federated learning to tier algorithms by device class: quantized inference on the smallest microcontrollers, lightweight bias-only updates on larger embedded or wearable tiers, and fuller adapter or backpropagation loops only on mobile-class SoCs. A single federated round must accommodate these tiers simultaneously without biasing the global model toward any one tier.
...
# │ Exports: EdgeAdaptationPowerMetrics.background_range_str,
# │          EdgeAdaptationPowerMetrics.background_energy_per_hour_str
from mlsysim import ReferenceStats
from mlsysim.core.constants import milliwatt, hour, kJ
from mlsysim.fmt import check, fmt_qty_range
...
Beyond traditional inference metrics, adaptive systems require measures that explain whether local learning is worth the resource cost. Adaptation efficiency is the first test: how much accuracy improves per training sample consumed. A system that gains 2 percent accuracy from 100 local samples is more useful on a device than one that needs 500 samples for the same improvement, because fewer samples mean faster personalization, less local storage, and fewer training windows competing with user-facing work.

The second test is whether the improvement fits inside the device envelope. Memory-constrained convergence evaluates validation loss under a fixed RAM budget, such as "convergence within 512 KB training footprint," so methods can be compared across microcontrollers, wearables, and smartphones. Energy-per-update measures millijoules consumed per gradient step; background adaptation commonly budgets `{python} EdgeAdaptationPowerMetrics.background_range_str`, translating to roughly `{python} EdgeAdaptationPowerMetrics.background_energy_per_hour_str` of adaptation before noticeably affecting battery life. Time-to-adaptation extends the same idea to scheduling: it measures wall-clock time from new data to measurable improvement, including waiting for idleness, charging status, and thermal headroom rather than only raw accelerator throughput.

The third test is whether personalization improves the right behavior without damaging the base model. Per-user performance delta compares the adapted model against the global baseline on user-specific holdout data, and deployments typically need statistically significant gains, often above 2 percent accuracy, before accepting the compute and energy overhead. Personalization-privacy trade-off measures accuracy gain per unit of local data exposure, which matters most for health, location, or personal-communication workloads. Catastrophic forgetting rate measures degradation on the original task after local adaptation; acceptable thresholds depend on the domain, but many systems target less than 5 percent accuracy loss on the original task so personalization does not erase general capability.

When devices coordinate through the federated protocols examined in @sec-edge-intelligence-federated-learning-6e7e, the metrics shift from one device to the population. Communication efficiency measures accuracy improvement per byte transmitted, capturing whether gradient compression and selective updates make mobile deployment practical; compressed-update designs often target 10--100$\times$ compression while retaining at least 95 percent of uncompressed accuracy. Straggler impact measures convergence delay caused by slow or unreliable devices, while aggregation quality tracks global model performance as participation rate changes. Together, these metrics reveal the minimum viable participation threshold, commonly 10--20 percent of devices per round, below which federated learning becomes unstable.
...
These on-device learning challenges\index{On-Device Learning!challenge} are not isolated; they interact in ways that influence the viability of different adaptation strategies. @Tbl-ondevice-challenges synthesizes these interconnected issues, mapping each challenge category to its root cause and system-level implications for on-device learning deployments.
...
A common fallacy is this: *On-device learning provides the same adaptation capabilities as cloud-based training.*
...
A recurring pitfall is this: *Assuming federated learning automatically preserves privacy without additional safeguards.*
...
A common fallacy is this: *Resource-constrained adaptation always produces better personalized models than generic models.*
...
A recurring pitfall is this: *Underestimating the complexity of coordinating learning across distributed edge systems.*
...
* **Device heterogeneity is the default**\index{Device Heterogeneity!edge}: Unlike uniform data center racks, the edge is a fragmented ecosystem of microcontrollers and SoCs. Federated learning must account for "stragglers" and participation bias generated by these physical disparities.
