...
# │ Exports: npu_cpu_lat_ms_str, npu_lat_ms_str, npu_speedup_str,
# │          npu_energy_gain_str, npu_cpu_energy_mj_str,
# │          npu_energy_mj_str
# └─────────────────────────────────────────────────────────────────────────────
...
class EdgeNpuSpeedup:
    ...
    npu_speedup_str = fmt_multiple(speedup, commas=False, precision=0)
    npu_energy_gain_str = fmt_multiple(energy_gain, commas=False, precision=0)
```
...
class StorageWall:
    ...
    ls_savings_ratio_str = fmt_multiple(round(per_context_savings), commas=False, precision=0)
    ...
    ls_total_savings_ratio_str = fmt_multiple(total_storage_savings, precision=1, commas=False)
```
...
class EdgeTinyTlRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tinytl_param_reduction_str = fmt_multiple(tinytl_param_reduction, commas=False, precision=0)
```
...
class FederatedSavings:
    ...
    fs_reduction_str = fmt_multiple(reduction, commas=False, precision=0)
```
...
class FederatedConvergence:
    ...
    fc_round_increase_factor_str = fmt_multiple(round_increase, precision=1, commas=False)
    fc_round_reduction_str = fmt_multiple(round_reduction, precision=2, commas=False)
    fc_local_compute_ratio_str = fmt_multiple(local_compute_ratio, precision=1, commas=False)
    fc_client_epoch_reduction_str = fmt_multiple(client_epoch_reduction, precision=1, commas=False)
```
...
class EdgeFedNpuRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mobile_npu_tops_str = fmt_ops_rate(mobile_npu_rate, unit=TOPS, precision=0, commas=False)
    compute_ratio_str = fmt_multiple(compute_ratio, precision=0, commas=True)
```
...
class EdgeBrainEfficiencyRecap:
    ...
    brain_vs_npu_high_str = fmt_multiple(brain_vs_npu_high, precision=0, commas=False)
```
