...
class EdgeDeviceSpectrum:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(tiny_ram < mcu_sram < flagship_ram, "Device memory endpoints should be ordered.")
    ...
    tiny_cpu_mips_str = fmt_int(tiny_cpu_mips, commas=False, suffix=" MIPS", approx=True)
    mobile_cpu_mips_str = fmt_int(mobile_cpu_mips, commas=True, suffix=" MIPS", approx=True)
    ...
    iot_compute_tops_str = fmt_qty(iot_compute, unit=TOPS, precision=2, commas=False)
```
...
class EdgeFedNpuRecap:
    from mlsysim.fmt import check, fmt_multiple, fmt_ops_rate, fmt_qty
    mobile_npu_rate = Hardware.Mobile.iPhone15Pro.compute.peak_flops
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mobile_npu_tops_str = fmt_ops_rate(mobile_npu_rate, unit=TOPS, precision=0, commas=False)
    iot_compute_tops_str = fmt_qty(iot_tops, unit=TOPS, precision=2, commas=False)
...
A concrete way to bound these risks is to integrate the chapter's three pillars by device tier rather than deploy one adaptation policy everywhere. Consider a production voice assistant deployment across 50 million heterogeneous devices, where each layer constrains what the others are allowed to do.
