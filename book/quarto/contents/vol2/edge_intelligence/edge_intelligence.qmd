...
::: {#fig-centralized-vs-decentralized fig-env="figure" fig-pos="htb" fig-cap="**Training Memory Amplification**. Inference requires only the model weights and a single layer's activations. Training requires storing *all* intermediate activations (for gradient computation), the gradients themselves, and the optimizer state (momentum/variance). This 3–5$\times$ increase in memory footprint is the primary bottleneck for on-device learning."}
```{.tikz}
