...
::: {#fig-centralized-vs-decentralized fig-env="figure" fig-pos="htb" fig-cap="**Training Memory Amplification**. Inference requires only the model weights and a single layer's activations. Training requires storing *all* intermediate activations (for gradient computation), the gradients themselves, and the optimizer state (momentum/variance). This 3–5$\times$ increase in memory footprint is the primary bottleneck for on-device learning." fig-alt="Three-region diagram labeled A, B, C. Region A: single phone with circular training loop. Region B: five phones sending data to stacked layers. Region C: five phones exchanging updates through cloud and aggregation nodes."}
```{.tikz}
...
However, deploying a model—whether to a GPU cluster or a billion smartphones—is only the beginning. Scaling these services creates operational complexity that grows exponentially with every model version, device type, and deployment region. How do we ensure a model update doesn't silently fail on 10% of budget Android phones? How do we monitor drift across a million private, local datasets?
