...
<!-- TODO(focused-followup): Items deferred from the codex deep-review
     ...
     - Compound capability law callout (line ~177): bold paragraph
       starters Observation, Principle, Implication, and rhetorical
       Q opener; convert to ordinary prose or a compact table and
       replace the question with a declarative transition.
     - Capability ~ Model_IQ x Tools x Context x Planning^N formula at
       line ~184: introduces a speculative composite without
       definitions, units, derivation, or citation. Either define
       explicitly as a qualitative heuristic and remove the equation
       styling, or add definitions and supporting evidence.
     - Notebook callout rhetorical Qs at lines 226 and 277; recast as
       declarative scenario statements followed by the calculation
       target.
     - 1,000 TFLOPS hardcoded at line ~364 vs. the chapter's
       H100_FLOPS_FP16_TENSOR constant (989 TFLOPS); either render the
       per-GPU TFLOPS from Python or mark the arithmetic as
       approximate throughout.
     - Author signature appears before takeaways at line ~388; move
       the signature after the takeaways callout or remove from the
       chapter body so the closing flow goes synthesis -> takeaways
       -> signature.
     - Time-sensitive phrases at line ~173 ("today's challenges",
       "the next decade", "already here") age quickly; anchor to
       concrete conditions or publication-era context.
     - Manifesto-style closing at line ~386 ("Build systems that
       scale. Build systems that endure. Build systems that serve
       humanity well."); retain the idea in declarative prose or
       reserve the imperative cadence for a preface or dedication. -->
...
**Observation**: Moore's Law (transistor density) provided the first wave of exponential growth. Neural scaling laws (data/compute) provided the second. Both are now flattening against physical power limits. Where does the next 100$\times$ improvement come from?

**Principle**:
> *When individual model scaling saturates, system capability scales with the **complexity of orchestration**.*

**Implication**:
The future belongs to **compound AI systems**\index{Compound AI Systems}: architectures that combine multiple specialized models, retrieval systems, and reasoning agents into a coherent whole. The "unit of compute" is no longer a FLOP; it is a **reasoning chain**\index{Reasoning Chain}.
$$ \text{Capability} \propto \text{Model}_{\text{IQ}} \times (\text{Tools} + \text{Context} + \text{Planning})^N $$
...
*   **Ops/sec**\index{Ops/sec}: $25,000 \times 1,000 \text{ TFLOPS} =$ **`{python} FermiEstimate.machine_ops_sci` FLOPS**.
