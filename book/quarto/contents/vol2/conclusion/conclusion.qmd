...
[^fn-tail-effects]: **Tail Latency**: At the 99th percentile, a request touching 100 servers has a 63 percent chance of hitting at least one slow server, dominating distributed inference response times (see @sec-inference-scale).
...
[^fn-gradient-compression]: **Gradient Compression**: Techniques that reduce communication volume 10--100$\times$ through sparsification, quantization, and error feedback, enabling bandwidth-limited distributed and federated training (see @sec-collective-communication).\index{Gradient Compression}
...
[^fn-elastic-training]: **Elastic Training**: A scheduling pattern that enables dynamic worker membership; jobs continue with remaining workers after failures and seamlessly incorporate new resources when available (see @sec-fault-tolerance-reliability).
...
[^fn-lifecycle-assessment]: **Lifecycle Assessment**: A method that evaluates environmental impact across a system's entire lifespan, including embodied carbon in hardware manufacturing (often 20--50 percent of total impact; see @sec-sustainable-ai).\index{Lifecycle Assessment}
