...
%   - columns=2: Two-column index layout for space efficiency
%   - columnseprule=true: Vertical line between columns
\usepackage{imakeidx}
\makeindex[intoc=true, columns=2, columnseprule=true]
\indexsetup{level=\small}

% =============================================================================
% FIGURE MANIFEST GENERATION FOR MIT PRESS
% =============================================================================
% Captures figure metadata during PDF compilation:
%   - Label (fig-xxx) for traceability to source code
%   - Figure number as assigned by LaTeX
%   - Page number
%   - Caption text exactly as rendered
%
% Output: figure_manifest.txt with format:
%   FIG|{label}|{number}|{page}|{caption}
%
% Alt text is captured separately by Lua filter (Quarto strips it before LaTeX)
% Post-render script merges both files into final FIGURE_LIST_VOL{N}.md
%
\newwrite\figmanifest
\AtBeginDocument{%
  \immediate\openout\figmanifest=figure_manifest.txt%
}
\AtEndDocument{%
  \immediate\closeout\figmanifest%
}

% Storage for current figure's label
\makeatletter
\newcommand{\currentfiglabel}{}

% Hook into \label to capture figure labels
\let\origlabel\label
\renewcommand{\label}[1]{%
  \ifx\@captype\undefined\else
    \def\@captype@test{figure}%
    \ifx\@captype\@captype@test
      \gdef\currentfiglabel{#1}%
    \fi
  \fi
  \origlabel{#1}%
}

% Hook into \caption to log figure with its label
...
\renewcommand{\caption}[2][]{%
  ...
  \ifx\@captype\undefined\else
    \def\@captype@test{figure}%
    \ifx\@captype\@captype@test
      % Write: FIG|label|number|page|caption
      \immediate\write\figmanifest{FIG|\currentfiglabel|\thefigure|\thepage|#2}%
      \gdef\currentfiglabel{}% Reset for next figure
    \fi
  \fi
}
\makeatother
