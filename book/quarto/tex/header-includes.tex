...
% the \sidenote marginpars. All 121 figure calls are plain \marginnote{...}
% (no optional args), so this signature is safe.
% The body of a margin figure contains blank lines (\par), so the redefinition
% MUST be \long or LaTeX fails with "Paragraph ended before \marginnote was
% complete". We also avoid scanning for a trailing optional offset (that
% lookahead reads past \par too). Plain-TeX with \@ifnextchar handles the
% leading optional [left-text] (unused here) without either hazard.
...
\long\def\marginnote{\@ifnextchar[\mfx@mn@opt\mfx@mn@plain}
\long\def\mfx@mn@opt[#1]#2{\mfx@mn@place{#2}}
\long\def\mfx@mn@plain#1{\mfx@mn@place{#1}}
\long\def\mfx@mn@place#1{\ifinner\mfxOrigMarginnote{#1}\else\marginpar{#1}\fi}
\makeatother
...
\def\mfx@tbl@grabcap#1{\gdef\mfx@tbl@cap{#1}}
\def\mfx@tbl@grablbl#1{\gdef\mfx@tbl@lbl{#1}}
\long\def\mfx@tbl@gobble#1\endlastfoot{}
\def\mfx@tbl@begin[#1]#2{%
  \gdef\mfx@tbl@cap{}\gdef\mfx@tbl@lbl{}%
  \let\caption\mfx@tbl@grabcap
  \let\label\mfx@tbl@grablbl
...
\def\mfx@tbl@end{%
  \bottomrule
  \end{tabular}\par
  \ifx\mfx@tbl@cap\@empty\else
    \nopagebreak\smallskip
    {\footnotesize\captionof{table}{\mfx@tbl@cap}%
      \ifx\mfx@tbl@lbl\@empty\else\mlsys@orig@label{\mfx@tbl@lbl}\fi}\par
  \fi
}
