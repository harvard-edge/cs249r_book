...
% 8-layer fleet stack with data bar for chapter margin figures
\newcommand{\mlfleetstack}[9]{
    \begin{tikzpicture}
        % Stack layers: 8 layers at 1.0cm height each
        % Total height: 8.0cm (layers) + margins

        % Layer 8: Governance (top)
        \stacklayer{7.0}{Governance}{#8}{icon_fleet_governance}

        % Layer 7: Security
        \stacklayer{6.0}{Security}{#7}{icon_fleet_security}

        % Layer 6: Operations
        \stacklayer{5.0}{Operations}{#6}{icon_fleet_ops}

        % Layer 5: Serving
        \stacklayer{4.0}{Serving}{#5}{icon_fleet_serve}

        % Layer 4: Orchestration
        \stacklayer{3.0}{Orchestration}{#4}{icon_fleet_orch}

        % Layer 3: Communication
        \stacklayer{2.0}{Communication}{#3}{icon_fleet_comm}

        % Layer 2: Parallelism
        \stacklayer{1.0}{Parallelism}{#2}{icon_fleet_parallel}

        % Layer 1: Infrastructure (bottom)
        \stacklayer{0.0}{Infrastructure}{#1}{icon_fleet_infra}

        % --- Data: vertical bar with uniform fill (same pattern as \mlsysstack) ---
        \pgfmathtruncatemacro{\ddata}{#9}
        \ifnum\ddata=0
            \def\databarcol{black!5}
        \else
            \def\databarcol{crimson!\ddata}
        ...
        \fill[\databarcol, rounded corners=1.5pt]
            (2.85, -0.1) rectangle (3.15, 8.10);

        % Horizontal connectors ("wires") from each layer into the data bar
        % Layer order bottom-to-top: INFRA(#1), PARA(#2), COMM(#3), ORCH(#4),
        %                            SERVE(#5), OPS(#6), SEC(#7), GOV(#8)
        \foreach \layerint/\ypos in {#1/0.5, #2/1.5, #3/2.5, #4/3.5, #5/4.5, #6/5.5, #7/6.5, #8/7.5} {
            \pgfmathtruncatemacro{\lint}{\layerint}
            \ifnum\lint=0
                \def\wirecol{black!8}
            \else
...
% Full-size fleet stack with subtitles and data bar for introductory use
\newcommand{\mlfleetstackfull}[9]{
    \begin{tikzpicture}
        % Stack layers with 1.1cm height

        % Layer 8: Governance
        \stacklayerfull{7.70}{Governance}{Fairness, sustainability, policy}{#8}{icon_fleet_governance}

        % Layer 7: Security
        \stacklayerfull{6.60}{Security}{TEEs, differential privacy, defense}{#7}{icon_fleet_security}

        % Layer 6: Operations
        \stacklayerfull{5.50}{Operations}{Monitoring, CI/CD, platform eng.}{#6}{icon_fleet_ops}

        % Layer 5: Serving
        \stacklayerfull{4.40}{Serving}{Batching, routing, KV cache}{#5}{icon_fleet_serve}

        % Layer 4: Orchestration
        \stacklayerfull{3.30}{Orchestration}{Schedulers, Slurm, Kubernetes}{#4}{icon_fleet_orch}

        % Layer 3: Communication
        \stacklayerfull{2.20}{Communication}{AllReduce, AllGather, AllToAll}{#3}{icon_fleet_comm}

        % Layer 2: Parallelism
        \stacklayerfull{1.10}{Parallelism}{Data, tensor, pipeline, expert}{#2}{icon_fleet_parallel}

        % Layer 1: Infrastructure
        \stacklayerfull{0.0}{Infrastructure}{Accelerators, power, cooling}{#1}{icon_fleet_infra}

        % --- Data: uniform color bar (same logic as \mlfleetstack) ---
        \pgfmathtruncatemacro{\ddata}{#9}
        \ifnum\ddata=0
            \def\databarcol{black!5}
        \else
        ...
        \fill[\databarcol, rounded corners=2pt]
            (4.45, -0.15) rectangle (4.75, 8.95);

        % Horizontal connectors
        \foreach \layerint/\ypos in {#1/0.55, #2/1.65, #3/2.75, #4/3.85, #5/4.95, #6/6.05, #7/7.15, #8/8.25} {
            \pgfmathtruncatemacro{\lint}{\layerint}
            \ifnum\lint=0
                \def\wirecol{black!8}
            \else
