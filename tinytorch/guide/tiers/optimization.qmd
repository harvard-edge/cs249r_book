...
- `simd_matmul`: a C++ matrix multiply with cache blocking, SIMD inner loops, and OpenMP threads, compiled on first use.
- `triton_fused_gelu`: a Triton GPU kernel that fuses a bias add and GELU into one pass, for NVIDIA GPUs.
- `mps_matmul`: a matrix multiply on Apple silicon through Metal Performance Shaders, using PyTorch's MPS backend.

Each one checks for its hardware or toolchain (`has_simd_support()`, `has_triton_support()`, `has_mps_support()`) and falls back to NumPy when it is missing, so no module depends on them and nothing is graded on them. The book's final chapter, *The Modern Stack: Compilers, Triton, and Custom Silicon*, explains what they do and what production compilers do instead.
