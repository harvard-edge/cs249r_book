---
title: "Architecture Tier (Modules 09-13)"
---
...
::: {#fig-architecture-deps}
![](../assets/images/svg/tiers-architecture-dependencies.svg){fig-alt="Architecture tier dependency diagram. Foundation box at top branches into two tracks: Vision track (M09 Convolutions leading to Computer Vision outcome) and Language track (M10 Tokenization -> M11 Embeddings -> M12 Attention -> M13 Transformers leading to Language Models outcome)."}

**Architecture Module Flow.** Two parallel tracks branch from Foundation: vision (Convolutions) and language (Tokenization through Transformers).
:::
...
**Historical impact**: This module enables **Milestone 04 (1998 CNN Revolution)** - reaching 65–75% accuracy on CIFAR-10 with YOUR implementations.
...
::: {#fig-architecture-milestones fig-env="figure" fig-pos="htb" fig-cap="**Architecture milestones.** The CNN milestone requires modules 01 through 09 and uses TinyDigits with an optional CIFAR-10 path. The Transformer milestone requires modules 01 through 08 and 11 through 13 for sequence-learning experiments." fig-alt="The CNN milestone requires modules 01 through 09 and uses TinyDigits with an optional CIFAR-10 path. The Transformer milestone requires modules 01 through 08 and 11 through 13 for sequence-learning experiments."}
...
- **Milestone 04 (1998)**: Build CNNs that reach 65–75% accuracy on CIFAR-10 (color images)
...
2. **Use**: Train on real data (TinyDigits, with an optional CIFAR-10 path)
...
```bash
tito milestone run 04   # a LeNet-style CNN on TinyDigits; --part 2 scales to CIFAR-10
```
...
```bash
tito milestone run 05   # reversal, copy, and mixed sequence tasks
```
