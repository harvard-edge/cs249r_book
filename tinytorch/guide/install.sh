...
# Configuration
# ============================================================================
# These can be configured via CLI flags or environment variables:
...
TINYTORCH_DEFAULT_VERSION="0.1.17"  # TINYTORCH_INSTALL_VERSION
...
fetch_latest_version() {
    # Skip if already set via environment variable or CLI flag
    ...
    if [ "$BRANCH" != "main" ]; then
        ...
        TINYTORCH_VERSION="dev"
        return 0
    fi

    # For main branch, try tags API first (single source of truth for releases)
    if command_exists curl; then
        local response
...
check_existing_directory() {
    if [ -d "$INSTALL_DIR" ]; then
        if [ -f "$INSTALL_DIR/pyproject.toml" ] || [ -d "$INSTALL_DIR/.tito" ]; then
            print_error "Directory '$INSTALL_DIR' already exists and contains a TinyTorch workspace."
            echo ""
            echo -e "  ${BOLD}To update without losing your exercise notebooks or progress:${NC}"
            echo -e "    ${CYAN}cd $INSTALL_DIR${NC}"
            if [ -f "$INSTALL_DIR/.venv/bin/activate" ]; then
                echo -e "    ${CYAN}source .venv/bin/activate${NC}"
            elif [ -f "$INSTALL_DIR/.venv/Scripts/activate" ]; then
                echo -e "    ${CYAN}source .venv/Scripts/activate${NC}"
            fi
            echo -e "    ${CYAN}tito update${NC}"
            echo ""
            echo -e "  ${BOLD}To install fresh in a separate location:${NC}"
            echo -e "    ${CYAN}curl -sSL mlsysbook.ai/tinytorch/install.sh | bash -s -- --dir ./${INSTALL_DIR}-fresh${NC}"
            echo ""
            echo -e "  ${DIM}(Do not run 'rm -rf $INSTALL_DIR' if you have completed exercises in modules/!)${NC}"
        else
            print_error "Directory '$INSTALL_DIR' already exists."
            echo "  Please choose a different location with --dir <path> or remove the empty directory."
        fi
        exit 1
    fi
}
    ...
    if [ $clone_status -eq 124 ]; then
        ...
        if [ -f "$clone_err_file" ] && grep -qi "Remote branch.*not found" "$clone_err_file"; then
            ...
            if [ -f "$clone_err_file" ] && [ -s "$clone_err_file" ]; then
                echo -e "  ${DIM}$(cat "$clone_err_file")${NC}"
            fi
        fi
        exit 1
    fi
...
Usage:
  install.sh [OPTIONS]
  curl -sSL mlsysbook.ai/tinytorch/install.sh | bash -s -- [OPTIONS]
...
Examples:
  ...
  # Install from a specific branch:
  curl -sSL mlsysbook.ai/tinytorch/install.sh | bash -s -- --branch dev

  # Non-interactive installation to a custom directory:
  curl -sSL mlsysbook.ai/tinytorch/install.sh | bash -s -- --dev --dir ./my-tinytorch -y
EOF
}
        ...
        case "$1" in
            ...
            -b|--branch)
                if [ -n "${2:-}" ] && [ "${2:0:1}" != "-" ]; then
                    ...
                    echo -e "${RED}✗${NC} Option '$1' requires a branch name argument."
                    exit 1
                fi
                ;;
            --branch=*)
            ...
            -d|--dir|--directory)
                if [ -n "${2:-}" ] && [ "${2:0:1}" != "-" ]; then
                    ...
                    echo -e "${RED}✗${NC} Option '$1' requires a directory path argument."
                    exit 1
                fi
                ;;
            --dir=*|--directory=*)
            ...
            -v|--version)
                if [ -n "${2:-}" ] && [ "${2:0:1}" != "-" ]; then
                    ...
                    echo -e "${RED}✗${NC} Option '$1' requires a version argument."
                    exit 1
                fi
                ;;
            --version=*)
...
main() {
    # Parse CLI flags (e.g. --dev, --branch, --dir, -y, --help)
    parse_args "$@"
