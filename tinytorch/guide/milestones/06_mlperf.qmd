...
:::{.callout-note title="🏎️ Kernel 1: Dense Matrix Multiply (Module 17 Vectorization)"}
- **Baseline (3 Nested Interpreter Loops):** 5.30 ms
- **Accelerated (Hardware SIMD / BLAS):** 0.02 ms
- **Empirical Speedup:** **>200× FASTER ⚡**
- **Mechanism:** `vectorized_matmul` replaces interpreter loop overhead with contiguous vector instructions and hardware SIMD execution.
:::

:::{.callout-note title="⚡ Kernel 2: Spatial Convolution Lowering (Module 17 im2col)"}
- **Baseline (7 Nested Interpreter Loops):** 1.79 ms
- **Accelerated (im2col Patch GEMM):** 0.06 ms
- **Empirical Speedup:** **>30× FASTER ⚡**
- **Mechanism:** `im2col_conv2d` lowers sliding spatial convolution loops into a single contiguous BLAS matrix multiplication.
:::

:::{.callout-note title="💾 Kernel 3: Autoregressive Memoization (Module 18 KV-Cache)"}
- **Baseline (O(N²) Prefix Recomputation):** 5.86 ms
- **Accelerated (O(1) Step KV-Cached):** 4.14 ms
- **Empirical Speedup:** **~1.5× FASTER ⚡**
- **Mechanism:** `KVCache` memoizes previous Key/Value attention tensors, turning quadratic $O(N^2)$ token generation into linear $O(N)$ operations while preserving exact numerical output parity.
:::

@tbl-milestone-06-acceleration-scorecard summarizes the measured hardware speedups across these three critical workloads.

| Kernel / Workload | Baseline (Unoptimized) | Accelerated (TinyTorch) | Empirical Speedup |
|:---|---:|---:|:---:|
| **Dense GEMM (32×32)** | 5.30 ms (loops) | 0.02 ms (SIMD) | **>200× FASTER ⚡** |
| **2D Conv (4-ch, 8×8)** | 1.79 ms (loops) | 0.06 ms (im2col) | **>30× FASTER ⚡** |
| **Autoregressive Decode (16 tokens)** | 5.86 ms (recompute) | 4.14 ms (cached) | **~1.5× FASTER ⚡** |

: **Optimization Tier Acceleration Scorecard.** {#tbl-milestone-06-acceleration-scorecard}
