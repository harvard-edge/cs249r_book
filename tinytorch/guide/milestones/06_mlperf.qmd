...
### Part 1: Architectural Triad MLPerf Scorecard

@tbl-milestone-06-triad-scorecard presents the multi-objective evaluation of the architectural triad across candidates.

| Architecture | Candidate | Memory | Latency | Quality | Primary Bottleneck | Status |
|:---|:---|---:|---:|:---|:---|:---|
| **DigitMLP** | Baseline FP32 | 9,640 B | 0.023 ms | 86.5% Acc | Parameter-bound (Dense) | ● Dominated |
| **DigitMLP** | INT8 Quantized | 2,442 B | 0.023 ms | 86.5% Acc | Parameter-bound (Dense) | ★ **Pareto** |
| **DigitMLP** | 50% Pruned | 9,640 B | 0.023 ms | 86.0% Acc | Parameter-bound (Dense) | ● Dominated |
| **SimpleCNN** | Baseline FP32 | 2,664 B | 1.636 ms | 91.5% Acc | Compute-bound (Spatial) | ★ **Pareto** |
| **SimpleCNN** | INT8 Quantized | 714 B | 1.636 ms | 91.0% Acc | Compute-bound (Spatial) | ★ **Pareto** |
| **SimpleCNN** | 50% Pruned | 2,664 B | 1.636 ms | 90.5% Acc | Compute-bound (Spatial) | ● Dominated |
| **TinyGPT** | Baseline FP32 | 113,152 B | 0.960 ms | 4.2 PPL | Prefix Recompute $O(N^2)$ | ● Dominated |
| **TinyGPT** | INT8 Quantized | 28,584 B | 0.960 ms | 4.3 PPL | Bandwidth-bound | ● Dominated |
| **TinyGPT** | KV-Cached (Mod 18) | 113,152 B | 0.219 ms | 4.2 PPL | $O(1)$ Step Decoding | ★ **Pareto** |
| **TinyGPT** | Full Stack (Quant+Cache) | 28,584 B | 0.219 ms | 4.3 PPL | Production Serving | ★ **Pareto** |

: **Architectural Triad MLPerf Scorecard across independent optimization candidates.** {#tbl-milestone-06-triad-scorecard}

### Pareto Frontier & Trade-Off Curve

The benchmark evaluates each candidate using `pareto_frontier` from Module 19. A configuration is **Pareto-optimal** ($\bigstar$) if no other configuration achieves strictly better metrics in one dimension without sacrificing another.

```
       Memory (KB) vs Latency (ms) Trade-Off Curve
   KB ┌──────────────────────────────────────────────────────┐
127.1 │                                                      │
      │                                                      │
      │      ★                    ●                          │
 ...
 12.7 │                                                      │
  ...
  0.0 └──────────────────────────────────────────────────────┘
      0.0                                          1.9 ms
      Legend: ★ Pareto-optimal frontier    ● Dominated candidate
```

Notice the key insight: **Pareto frontiers are architecture-specific**:
- On `DigitMLP`, INT8 Quantization strictly dominates baseline FP32 because it yields $3.95\times$ compression with zero degradation in accuracy.
- On `SimpleCNN`, Baseline FP32 and INT8 Quantization form the frontier: FP32 preserves maximum top-line accuracy (91.5%), while INT8 offers ultra-compact footprint (714 B) at 91.0% accuracy.
- On `TinyGPT`, the KV-Cached and Full Stack (Quant + Cache) configurations form the frontier: caching removes the quadratic latency penalty of prefix recomputation, and quantization compresses the weight memory footprint by $3.96\times$.
