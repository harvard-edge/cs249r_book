---
title: "Milestone 02: The XOR Crisis (1969)"
---
...
::: {#fig-m02-xor-space-folding fig-env="figure" fig-pos="htb" fig-cap="**The XOR crisis and non-linear space folding.** Left: Diagonally paired classes cannot be separated by any single straight line ($w_1 x_1 + w_2 x_2 + b = 0$), capping single-layer accuracy at 75%. Right: Adding a hidden layer with non-linear activation (ReLU) transforms the coordinate space ($[h_1, h_2]$), folding the input so that a simple linear decision boundary achieves 100% accuracy." fig-alt="Diagram comparing single-layer perceptron failing on XOR with two-layer MLP folding coordinate space to achieve 100% accuracy."}

![](../assets/images/diagrams/m02_xor_space_folding.svg)
...
The crisis needs Modules 01–03. The multi-layer solution needs Modules 01–08, and `tito` runs it as the first part of Milestone 03:
...
tito milestone run 03 --part 1   # the solution: one hidden layer reaches 100%
```
...
| 02 (Multi-Layer, run as Milestone 03 part 1) | 2 | 0.0053 after 500 epochs | 100% by epoch 100 | Hidden layers solve it |
...
You check the code. Everything's correct. Your Linear layer works. Your autograd computes gradients. Your optimizer updates weights. But accuracy tops out at 75%.
...
The companion book's chapter *Milestone I: From Perceptrons to Rumelhart's MLP* covers Milestones 01 through 03 together. Read it in [*TinyTorch: From Tensors to Transformers*](https://mlsysbook.ai/tinytorch/assets/downloads/TinyTorch-Book.pdf) (PDF) after you have run the milestone.
