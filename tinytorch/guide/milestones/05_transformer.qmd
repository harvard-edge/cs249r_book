---
title: "Milestone 05: The Transformer Era (2017)"
---
...
- How attention lets every position read every other position directly
...
Before running, ensure you have completed Modules 01–08 and 11–13. You can check your progress:
...
@tbl-milestone-05-expected-results records the accuracy and runtime you should expect to see.

| Script | Task | Success Criteria | Time |
|--------|------|------------------|------|
| `01_vaswani_attention.py` | Reversal / copy / mixed | Passes at 95% / 95% / 90%; measured 97–99% / 100% / 99.0–99.7% | A few seconds (2, 2, and 4 epochs) |

: **Expected success criteria and runtime for the Transformer milestone.** {#tbl-milestone-05-expected-results}
...
Reversal is awkward for a model that reads left to right through one hidden state, because the first output depends on the last input. Attention lets every output position read any input position directly:
...
|-----------|-------------|--------------|
| `Embedding` | Module 11 | YOUR learned token representations |
| `PositionalEncoding` | Module 11 | YOUR position encodings |
...
The companion book's chapter *Milestone II: Convolution, Attention, and Generated Text* covers Milestones 04 and 05, and why fixed permutations are easy for attention while prefix-conditioned routing is not. Read it in [*TinyTorch: From Tensors to Transformers*](https://mlsysbook.ai/tinytorch/assets/downloads/TinyTorch-Book.pdf) (PDF) after you have run the milestone.
