---
title: "Milestone 01: The Perceptron (1958)"
---
...
2. **The Limitation** — random weights produce arbitrary predictions

::: {#fig-m01-perceptron-architecture fig-env="figure" fig-pos="htb" fig-cap="**The 1958 Perceptron forward pass and the learning dilemma.** The forward path passes input features through your Linear layer and Sigmoid activation to classify points. Without a learning loop (loss, autograd, and optimizer), weights remain random, demonstrating why learning infrastructure is essential for intelligence." fig-alt="Diagram showing input features passing through Linear layer, Sigmoid activation, and classification, contrasted with the missing feedback loop of untrained weights."}

![](../assets/images/diagrams/m01_perceptron_architecture.svg)
...
|--------|----------|---------------|
| 01 (Forward Only) | Whatever the random weights give (0% in the run we measured) | The model has not learned anything |
...
You'll run a single forward-only script using YOUR Linear layer and YOUR sigmoid. Its accuracy is whatever the random weights happen to give, sometimes near zero and occasionally high by luck, because nothing has been learned yet.
