---
title: "Progress & Data Management"
---
...
::: {#fig-tito-progress-tracking fig-env="figure" fig-pos="htb" fig-cap="**One store, two views.** Completing a module writes to `.tito/progress.json`. TITO reads that same file to report which of the 20 modules are built and which milestones their prerequisites have unlocked." fig-alt="Completing a module writes to the .tito/progress.json file, which tito module status and tito milestone status both read."}
...
**A**: Yes. `tito module reset NN` regenerates `modules/NN_name/<short>.ipynb` from
`src/`, which overwrites whatever you had written in that notebook. `tito module
reset --all` does it for all twenty. There is no automatic backup.

Copy the notebook somewhere safe first if you want to keep your work:

```bash
cp modules/03_layers/layers.ipynb ~/layers-backup.ipynb
tito module reset 03
```

What reset does *not* touch: your `src/` files and the exported package under
`tinytorch/`.
...
### Q: How do I see what I have completed?

**A**: Run `tito module status`. `.tito/progress.json` records which modules are
started and completed, plus a single `last_updated` stamp; it does not record a
date per module. `.tito/milestones.json` does record per-milestone completion
dates.

### Q: Does TinyTorch back my work up automatically?

**A**: No. Nothing in `tito` writes a backup of your notebooks. `tito system
update` copies `progress.json`, `milestones.json`, and `config.json` into
`.tito/backups/` before it updates, and that is the only thing that directory
ever holds. Your implementations are not in it. Use git, or copy the notebook by
hand, before running anything destructive.
...
<a href="modules.qmd" style="display: inline-block; background: #007bff; color: white; padding: 0.75rem 1.5rem; border-radius: 0.25rem; text-decoration: none; font-weight: 500; margin-right: 1rem;">Module Workflow →</a>
