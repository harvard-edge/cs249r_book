...
::: {#fig-tito-test-hierarchy fig-env="figure" fig-pos="htb" fig-cap="**Tests at increasing scope.** Inline and component checks provide focused feedback. Integration, regression, CLI, and end-to-end tests cover interactions and user workflows. Run full release validation in an isolated checkout." fig-alt="Inline and component checks provide focused feedback. Integration, regression, CLI, and end-to-end tests cover interactions and user workflows. Run full release validation in an isolated checkout."}
...
:::{.callout-warning}
`--inline` exports each module from `src/` before testing it, which rewrites
the notebooks under `modules/` and discards your edits. `tito dev` says the
same of `tito dev export`: "This OVERWRITES student notebooks!". It is a
developer command; students should use `tito module test`.
:::

```bash
# Run inline tests for modules 01 through 06 (not module 06 alone)
...
**Example**: Testing that `Tensor` from Module 01 works correctly with `Linear` from Module 03.
...
**Requirements**: Every module a milestone touches must be exported first. Modules 01-13 export to `tinytorch/core/`, Modules 14-19 export to `tinytorch/perf/`, and Module 20 exports to `tinytorch/olympics.py`.
...
:::{.callout-warning}
This is **DESTRUCTIVE** and it deletes student work. It will:

1. Delete every module directory under `modules/`, including your notebooks
2. Delete the whole `.tito/` directory, losing all progress tracking
3. Clean `tinytorch/core/`
4. Export each module from `src/` from scratch
5. Execute all milestones

It does **not** run the unit, integration, end-to-end, CLI, or milestone
pytest suites: `--release` is an alias for `--user-journey`, and the unit
phase is suppressed while a user-journey run is active. Use `--all` for the
test suites.

Copy anything under `modules/` that you want to keep before running it.
:::
...
- **[Module Workflow](workflow.qmd)** - How modules build progressively
- **[Milestone System](../milestones/index.qmd)** - Understanding historical milestones
