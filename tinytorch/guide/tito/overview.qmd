...
<div class="responsive-cards responsive-cards-three">
...
# Stage TinyTorch notebooks with tiered scaffolding
tito nbgrader generate 01 --tier student      # Default: 41 core archetypes
tito nbgrader generate 01 --tier challenge    # Unscaffolded: full build
tito nbgrader generate 01 --tier instructor   # Full reference solutions

# Create student release notebooks with nbgrader
...
Tito stages TinyTorch notebooks and normalizes nbgrader metadata across three pedagogical tiers (`--tier student`, `--tier challenge`, `--tier instructor`). In the student tier, 41 core architectural archetypes are left for student implementation, while 195 secondary/boilerplate solution blocks remain scaffolded with `role="scaffold"`. Solution stripping is marker-driven: only content inside `### BEGIN SOLUTION` / `### END SOLUTION` is handed to nbgrader as removable content.
