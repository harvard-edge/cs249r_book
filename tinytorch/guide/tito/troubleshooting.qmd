...
```bash
ls -la tinytorch/core/layers.py
...
**Step 4: Test import**:
```python
python -c "from tinytorch.core.layers import Linear; print(Linear)"
```
...
def some_function():
 from tinytorch.core.tensor import Tensor # Local import
 ...
```
...
**Option 1: Reset the module** — note that this also regenerates
`modules/05_dataloader/dataloader.ipynb` from `src/` and discards what you wrote
in it, so copy the notebook first if you want to keep it:

```bash
cp modules/05_dataloader/dataloader.ipynb ~/dataloader-backup.ipynb
tito module reset 05
```

**Option 2: Delete the progress file**. It is rebuilt the next time a command
writes progress, such as `tito module complete`; no command recreates it on
demand, and `tito module status` will report 0/20 until then:

```bash
rm .tito/progress.json
tito module status   # shows 0/20 until the next completion is recorded
```
...
tito --verbose module complete 03

# See detailed error traces
python -m pdb milestones/03_1986_mlp/01_rumelhart_tinydigits.py
```
...
*Most issues have simple fixes. Start with `tito system health` and read the error message carefully. Do keep your own copy of a notebook before running anything that regenerates it: `tito module reset`, `tito dev export`, and `tito dev test --inline` all rewrite `modules/` from `src/`.*
