...
A complete reference of all `tito` commands, subcommands, options, and environment variables. For a practical walkthrough of the daily student routine, see the [Student Workflow](workflow.qmd).
...
- **Options**:
  - `--force`, `-f`: Re-run full setup even if already initialized.
...
- **Options**:
  - `--no-browser`: Start the Jupyter server without automatically opening a browser window.
...
- **Options**:
  ...
  - `--non-interactive`: Skip interactive pauses or prompts (for automated evaluation).
...
### `tito system jupyter`
Manages the background Jupyter notebook server:
- `tito system jupyter start`: Launch server in background.
- `tito system jupyter stop`: Stop running server.
- `tito system jupyter status`: Check server status and port.
