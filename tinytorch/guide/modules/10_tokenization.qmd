...
BPE training scales as **O(n × m)**: each of the m merges makes one pass to count pairs and one pass to apply the chosen merge, so cost grows linearly in corpus size for a fixed merge budget, not quadratically.
