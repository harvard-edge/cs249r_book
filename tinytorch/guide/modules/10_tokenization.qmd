...
<a href="../assets/slides/10_tokenization.pdf" download><i class="fa-solid fa-download"></i> Download</a>
<a href="#" onclick="slideFullscreen('10_tokenization'); return false;" class="secondary"><i class="fa-solid fa-expand"></i> Fullscreen</a>
</div>
</div>
...
::: {#fig-10_tokenization-diag-1 fig-env="figure" fig-pos="htb" fig-cap="**Learned BPE merges.** Training counts words, constructs a character vocabulary, and learns frequent adjacent-pair merges. Encoding applies those merges in order within each word and maps tokens to IDs. Merges are applied in learned order within each word; token IDs depend on the trained vocabulary. The alphabet comes from the corpus. This implementation uses characters and an unknown-token fallback." fig-alt="Training counts words, constructs a character vocabulary, and learns frequent adjacent-pair merges. Encoding applies those merges in order within each word and maps tokens to IDs."}
...
@tbl-10-tokenization-implementation-roadmap lays out the implementation in order, one part at a time.
...
: **Implementation roadmap for the Tokenizer classes.** {#tbl-10-tokenization-implementation-roadmap}
...
@tbl-10-tokenization-char-api lists the methods you will implement on this class.
...
: **Methods on the CharTokenizer class.** {#tbl-10-tokenization-char-api}
...
@tbl-10-tokenization-bpe-api lists the methods you will implement on this class.
...
: **Methods on the BPETokenizer class.** {#tbl-10-tokenization-bpe-api}

**Helper Methods:**

@tbl-10-tokenization-bpe-helpers lists the methods you will implement on this class.
...
: **Internal helper methods used by the BPETokenizer.** {#tbl-10-tokenization-bpe-helpers}
...
@tbl-10-tokenization-utilities lists the utility helpers that accompany the tokenizers.
...
: **Utility functions for tokenizer creation and analysis.** {#tbl-10-tokenization-utilities}
...
```{python}
#| label: lst-10-tokenization-build-vocab
#| lst-cap: "**`CharTokenizer.build_vocab()` collects unique characters, reserves index 0 for `<UNK>`, and builds the forward and reverse ID maps.**"
#| eval: false
...
```{python}
#| label: lst-10-tokenization-bpe-train
#| lst-cap: "**BPE training loop: count all adjacent pairs, merge the most frequent, repeat until the vocabulary reaches its target size.**"
#| eval: false
...
```{python}
#| label: lst-10-tokenization-bpe-encode
#| lst-cap: "**`BPETokenizer.encode()` splits words, applies the learned merge rules, and looks up integer IDs, falling back to `<UNK>` for unseen tokens.**"
#| eval: false
def encode(self, text: str) -> List[int]:
...
```{python}
#| label: lst-10-tokenization-bpe-decode
#| lst-cap: "**`BPETokenizer.decode()` reverses encoding: look up token strings, join them, and convert `</w>` markers back into whitespace.**"
#| eval: false
def decode(self, tokens: List[int]) -> str:
    ...
    # Clean up whitespace without corrupting literal marker text
...
BPE is slower due to merge rule application. Training BPE scales approximately O(n × m) where n is corpus size (or unique vocabulary entries) and m is the number of merges. Each merge iteration requires counting pairs across the word table, then updating token sequences. For a 10,000-word corpus learning 5,000 merges, this takes seconds to minutes depending on implementation.
...
@tbl-10-tokenization-complexity summarises the complexity and speed of each stage.
...
: **Complexity and speed of character and BPE tokenization stages.** {#tbl-10-tokenization-complexity}
...
@tbl-10-tokenization-production-models places your implementation in context against production models.
...
: **Vocabulary size and tokenizer strategy in production LLMs.** {#tbl-10-tokenization-production-models}
...
@tbl-10-tokenization-vs-huggingface places your implementation side by side with the production reference for direct comparison.

| Feature | Your Implementation | Hugging Face Tokenizers |
|---------|---------------------|-------------------------|
| **Language** | Pure Python | Rust (compiled to native code) |
| **Speed** | 1-10 ms/sentence | 0.01-0.1 ms/sentence (100$\times$ faster) |
...
: **Feature comparison between TinyTorch tokenizers and Hugging Face Tokenizers.** {#tbl-10-tokenization-vs-huggingface}
...
- **GPT-3 training**: Processing 300 billion tokens required careful vocabulary selection. Switching to character tokenization would have inflated sequence lengths by 3--4$\times$, multiplying attention cost by 9--16$\times$ — adding millions of dollars to a single training run.
...
:::{.callout-tip title="Check Your Understanding — Tokenization"}
Before moving on, verify you can articulate each of the following:

- [ ] The memory/sequence-length trade-off: BPE's 50K-vocab embedding table (tens of MB) versus character-level's O(n²) attention bloat from long sequences.
- [ ] Why BPE training is O(n² × m) while trained BPE *encoding* is near-linear, and why production tokenizers run in Rust rather than Python.
...
BPE is **16$\times$ faster** at attention. This is why modern models use subword tokenization despite the larger embedding table.
:::
...
- **Compute**: 36 attention computations vs 400 (~11$\times$ fewer)
...
- **Sequence processing**: 4.0$\times$ shorter sequences → 16$\times$ faster attention (quadratic scaling)
...
**Trade-off**: BPE vocabulary is ~100$\times$ larger (10K tokens vs 100), pushing embedding memory from ~200 KB to ~20 MB. The trade favors BPE heavily once you stack multiple transformer layers and attention dominates the cost.
:::
...
- **10,000 words**: ~10,000 ms = 10 seconds (100$\times$ longer, from 10² scaling)
- **100,000 words**: ~1,000,000 ms = 1,000 seconds $\approx$ **16.7 minutes** (10,000$\times$ longer)
...
- Use optimized implementations (Rust, C++) that are 100--1,000$\times$ faster
...
- **Textbook**: ["Speech and Language Processing"](https://web.stanford.edu/~jurafsky/slp3/) by Jurafsky & Martin - Chapter 2 covers tokenization, including Unicode handling and language-specific issues
...
@tbl-10-tokenization-downstream-usage traces how this module is reused by later parts of the curriculum.
