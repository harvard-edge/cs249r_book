...
<a href="../assets/slides/14_profiling.pdf" download><i class="fa-solid fa-download"></i> Download</a>
<a href="#" onclick="slideFullscreen('14_profiling'); return false;" class="secondary"><i class="fa-solid fa-expand"></i> Fullscreen</a>
</div>
</div>
...
- Quantization: FP32 → INT8 for 4$\times$ compression
...
@tbl-14-profiling-implementation-roadmap lays out the implementation in order, one part at a time.
...
: **Implementation roadmap for the Profiler class and its measurement methods.** {#tbl-14-profiling-implementation-roadmap}
...
@tbl-14-profiling-core-methods lists the methods in this group.
...
: **Core measurement methods on the Profiler class.** {#tbl-14-profiling-core-methods}

### Analysis Methods

@tbl-14-profiling-analysis-methods lists the methods in this group.
...
: **Higher-level analysis methods on the Profiler class.** {#tbl-14-profiling-analysis-methods}

### Utility Functions

@tbl-14-profiling-utility-functions lists the methods in this group.
...
: **Utility functions for quick profiling and weight analysis.** {#tbl-14-profiling-utility-functions}
...
```{python}
#| label: lst-14-measure-latency
#| lst-cap: "**measure_latency with warmup and median**. Runs warmup iterations to reach steady state, then times a fixed number of forward passes and returns the median to reject OS-noise outliers."
#| eval: false
...
```{python}
#| label: lst-14-measure-memory
#| lst-cap: "**measure_memory breakdown**. Uses tracemalloc to trace allocations during a forward pass, accounting for baseline memory, parameters, and input/output activations."
#| eval: false
def measure_memory(self, model, input_shape: Tuple[int, ...]) -> Dict[str, float]:
    ...
    try:
        ...
        # Count actual input and output buffers
...
:::{.callout-note title="Systems Implication: Reading the Roofline to Pick the Right Optimization"}
...
@tbl-14-profiling-vs-pytorch places your implementation side by side with the production reference for direct comparison.
...
: **Feature comparison between TinyTorch Profiler and PyTorch profiling tools.** {#tbl-14-profiling-vs-pytorch}
...
In each case the profiler did not invent the optimization — it told the engineer which optimization was worth implementing. A single session tells you where the time and memory actually go, which is the prerequisite for any speedup you can defend. The instrument earns its keep on the first run.
...
According to the **Roofline model**, the hardware ridge point is:
$$I^* = \frac{\text{Peak Compute}}{\text{Memory Bandwidth}} = \frac{100\text{ GFLOP/s}}{50\text{ GB/s}} = 2.0\text{ FLOP/byte}$$

If the system saturates memory bandwidth at 50 GB/s while executing 5 GFLOP/s, the workload's operational intensity is:
$$I = \frac{5\text{ GFLOP/s}}{50\text{ GB/s}} = 0.1\text{ FLOP/byte}$$

Because $I = 0.1 \ll I^* = 2.0$, this workload is squarely **memory-bound**. The chip can perform 100 GFLOP/s but is starved for data, spending most cycles stalled on DRAM transfers.

The right optimization strategy is to cut memory traffic — kernel fusion, tiling for cache locality, or quantization. Reducing FLOPs will not improve latency when compute units are already waiting on memory.
:::
...
Total: 500 + 500 + 500 + 500 = **2,000 MB (2 GB)** — 4$\times$ the raw model size, just to train.
...
- **Training memory is dominated by optimizer state, not weights:** Adam multiplies the model footprint ~4$\times$ before activations, which is why optimizer sharding (ZeRO/FSDP) exists.
...
- **Tool**: [PyTorch Profiler](https://docs.pytorch.org/tutorials/recipes/recipes/profiler_recipe.html) - Production profiling with GPU support
...
@tbl-14-profiling-downstream-usage traces how this module is reused by later parts of the curriculum.
