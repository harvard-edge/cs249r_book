...
<a href="../assets/slides/20_capstone.pdf" download><i class="fa-solid fa-download"></i> Download</a>
<a href="#" onclick="slideFullscreen('20_capstone'); return false;" class="secondary"><i class="fa-solid fa-expand"></i> Fullscreen</a>
</div>
</div>
...
@tbl-20-capstone-implementation-roadmap lays out the implementation in order, one part at a time.
...
: **Implementation roadmap for the capstone benchmarking artifact.** {#tbl-20-capstone-implementation-roadmap}
...
The properties you expose are summarised in @tbl-20-capstone-report-properties.
...
: **Data properties stored on the BenchmarkReport class.** {#tbl-20-capstone-report-properties}

### BenchmarkReport Methods

@tbl-20-capstone-report-methods lists the methods on the report class.

| Method | Signature | Description |
|--------|-----------|-------------|
| `benchmark_model` | `benchmark_model(model, X_test, y_test, num_runs=100) -> dict` | Measures accuracy, latency (mean, std, median), throughput, memory |
| `measure_latency` | `measure_latency(model, X_batch, num_runs=100) -> List[float]` | Single-sample latency timing with warmup |
| `measure_memory` | `measure_memory(model) -> float` | Model parameter memory footprint in MB |

: **Core methods on the BenchmarkReport class.** {#tbl-20-capstone-report-methods}

### Torch Olympics Competition Harness

The `tinytorch.olympics` module provides rules and submission tools for classroom competitions:

@tbl-20-capstone-olympics-api lists the competition events and submission helpers.

| Symbol | Signature / Values | Description |
|--------|---------------------|-------------|
| `OlympicEvent` | `LATENCY_SPRINT`, `MEMORY_CHALLENGE`, `ACCURACY_CONTEST`, `EXTREME_PUSH` | Enum defining competitive challenge categories |
| `qualifies_event` | `qualifies_event(metrics: Dict[str, float], event: OlympicEvent) -> bool` | Checks whether candidate metrics satisfy competition threshold constraints |
| `generate_submission` | `generate_submission(baseline_report, optimized_report=None, student_name="Student", event=None) -> dict` | Creates standardized submission payload comparing baseline vs candidate |
| `save_submission` | `save_submission(submission, filepath="submission.json") -> str` | Serializes and writes submission JSON to disk |
| `validate_submission_schema` | `validate_submission_schema(submission: dict) -> bool` | Validates JSON schema integrity and required fields |

: **Torch Olympics competition harness API.** {#tbl-20-capstone-olympics-api}
...
Each optimization from Modules 14–18 attacks a specific term, and Module 19 verifies the combined effect:
...
**Latency** measures per-sample speed: how long does it take to process one input? This matters for real-time applications where users wait for results. Your implementation measures latency by timing single-sample inference with warmup and `precise_timer`:

```python
# Latency: time for ONE sample after untimed warmup
for _ in range(5):
...
for _ in range(num_runs):
    with precise_timer() as timer:
        _ = model.forward(X_test[:1])
    latencies.append(timer.elapsed * 1000)  # in ms
```
...
**Throughput** measures batch processing capacity: how many total inputs can the system process per second? This is the metric for offline jobs grinding through millions of examples. Critically, throughput is **timed separately on the full batch**, not derived from single-sample latency:

```python
# Throughput: time the WHOLE test batch together
...
# Samples processed per second across the batch
throughput_samples_per_sec = batch_size / max(np.median(batch_seconds), 1e-9)
```

At 10ms per single sample, computing `1000 / 10 = 100` samples/second would severely undercount actual throughput if the model processes a batch of 32 samples in 50ms (achieving $32 / 0.050 = 640$ samples/second!). Conversely, running large batches increases throughput at the expense of per-request latency. Deriving throughput via `1000 / avg_latency` is an anti-pattern because it erases this fundamental hardware trade-off.
...
:::{.callout-note title="Systems Implication: Hardware Utilization and Little's Law"}
...
**Fast + Small** means aggressive optimization. Quantizing to INT8 models a 4x smaller footprint; whether it speeds up inference is something you measure, but typically costs 1-2% accuracy. Pruning 50% of weights zeroes them without shrinking the arrays, so measured memory is unchanged, but may lose another 1-2% accuracy. You've traded accuracy for efficiency.
...
```{python}
#| label: lst-20-capstone-schema-automation
#| lst-cap: "**Schema-validated submissions unlock automated aggregation, leaderboards, and regression detection**."
#| eval: false
...
```{python}
#| label: lst-20-capstone-trap-isolation
#| lst-cap: "**Isolating the timed region from setup and post-processing**."
#| eval: false
...
@tbl-20-capstone-vs-industry places your implementation side by side with the production reference for direct comparison.
...
: **Feature comparison between TinyTorch capstone benchmarking and industry systems.** {#tbl-20-capstone-vs-industry}
...
:::{.callout-tip title="Check Your Understanding — Capstone"}
...
- [ ] How end-to-end inference complexity decomposes across attention, FFN, and decode terms, which optimization from Modules 14–18 attacks each, and how Module 19 validates the combined effect.
