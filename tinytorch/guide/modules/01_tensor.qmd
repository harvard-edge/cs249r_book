...
def __add__(self, other):
    ...
    if isinstance(other, Tensor):
        return Add.apply(self, _as_tensor(other))  # dispatches through Function.apply
    else:
...
::: {#fig-01_tensor-diag-2 fig-env="figure" fig-pos="htb" fig-cap="**Broadcasting reuses a vector across rows.** Adding rows [1, 2, 3] and [4, 5, 6] to vector [10, 20, 30] produces [11, 22, 33] and [14, 25, 36]. Trailing axes must match or have size one; the result has new storage." fig-alt="Adding rows [1, 2, 3] and [4, 5, 6] to vector [10, 20, 30] produces [11, 22, 33] and [14, 25, 36]. Trailing axes must match or have size one; the result has new storage."}
...
When you reshape or transpose an array in NumPy or PyTorch, does it allocate new memory or create a view? In pure NumPy and PyTorch, reshaping and transposing create *views* that share memory with the source (an $O(1)$ metadata update).

However, in **TinyTorch**, `Tensor.__init__` enforces an explicit systems invariant: **all constructed Tensors own independent storage**. Every operation result returned by `Function.apply` passes through `np.array(..., dtype=np.float32)`, creating an isolated copy:

```python
x = Tensor([1, 2, 3, 4])
y = x.reshape(2, 2)  # TinyTorch wraps the NumPy view in fresh, copied storage
y.data[0, 0] = 99    # In TinyTorch, x is NOT modified!
print(x.data[0, 0])  # Still 1.0
```

This deliberate design choice eliminates tricky mutation bugs where modifying an intermediate activation accidentally corrupts an earlier layer or invalidates tape recording in autograd.

@tbl-01-tensor-view-vs-copy contrasts the memory and time cost of native view operations against TinyTorch's constructor copy.

| Operation | NumPy / PyTorch Native | TinyTorch `Tensor` | Time Complexity |
|-----------|------------------------|--------------------|-----------------|
| `reshape()` | View (Shared buffer) | Fresh Allocation (Copied buffer) | $O(n)$ copy on wrap |
| `transpose()` | View (Strided swap) | Fresh Allocation (Copied buffer) | $O(n)$ copy on wrap |
| `+ - * /` | New allocation | New allocation | $O(n)$ |

: **Memory and time cost of native views versus TinyTorch copying operations.** {#tbl-01-tensor-view-vs-copy}
...
```{python}
#| label: lst-01-tensor-matmul
#| lst-cap: "**Matrix multiplication with explicit loops for the 2D case and shape validation.**"
#| eval: false
...
```{python}
#| label: lst-01-tensor-reshape
#| lst-cap: "**Reshape with dimension inference via `-1` and element-count validation.**"
#| eval: false
...
To the math, `reshape` or `transpose` just change dimensions. To the hardware, they manipulate *strides*. A tensor's data is stored as a flat, 1D contiguous block of bytes in RAM. In pure NumPy a `reshape` is an O(1) metadata update and the buffer is untouched. TinyTorch's `reshape` dispatches through `Function.apply`, whose `Tensor` constructor copies, so here it costs O(n) and you get an owned buffer.
...
::: {#fig-01_tensor-diag-3 fig-env="figure" fig-pos="htb" fig-cap="**The TinyTorch execution stack.** Python Tensor expressions dispatch through Function.apply to NumPy operations on CPU. New Tensor results copy their data; the linked BLAS library may accelerate matrix multiplication." fig-alt="Python Tensor expressions dispatch through Function.apply to NumPy operations on CPU. New Tensor results copy their data; the linked BLAS library may accelerate matrix multiplication."}
...
- **Views vs. copies is a systems decision:** pure NumPy views share memory while TinyTorch constructor copies guarantee isolated storage; mixing them up causes either memory blowup or silent mutation.
