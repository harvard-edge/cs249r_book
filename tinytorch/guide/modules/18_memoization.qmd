...
| `generation` | `generation() -> Iterator[KVCache]` | Context manager establishing cached inference scope |
...
### Helper Functions and Stand-ins
...
| `CachedAttention` | `CachedAttention(attention, cache_obj, layer_idx)` | Stand-in attention layer routing tokens through cache |
| `_cached_generate` | `_cached_generate(model, prompt_tokens, max_new_tokens, temperature) -> Tensor` | Fast autoregressive generation using KV cache |

: **Helper functions and classes for enabling and using the KV cache.** {#tbl-18-memoization-helper-functions}
...
def update(self, layer_idx: int, key: Tensor, value: Tensor) -> None:
    ...
    if self.seq_pos >= self.max_seq_len:
        raise ValueError(f"KV cache full: seq_pos {self.seq_pos} >= max_seq_len {self.max_seq_len}")
...
### Scoped Dispatch with CachedAttention

To integrate KV caching into a transformer model without modifying the underlying architecture or breaking training, TinyTorch uses the `CachedAttention` stand-in layer. When `enable_kv_cache(model)` is called, it replaces each block's attention layer with a `CachedAttention` wrapper:

- **Inside `with cache.generation():`**: Single new tokens (`seq_len == 1`) route through `_cached_generation_step`. Only the new token's Q, K, V projections are computed; new K and V are appended to the cache in $O(1)$ time, and attention is evaluated across the accumulated prefix in $O(S)$ time.
- **Outside the scope**: Full sequence prompts or training passes bypass the cache completely and execute standard multi-head attention.
