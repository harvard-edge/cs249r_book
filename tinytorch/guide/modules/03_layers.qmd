...
::: {#fig-03_layers-diag-1 fig-env="figure" fig-pos="htb" fig-cap="**A shared layer interface.** Linear and Dropout inherit `forward()` and `parameters()` from `Layer`, while Sequential provides the same interface to chain them into networks. All three provide the interface; Sequential composes layers into a network." fig-alt="Diagram showing the Layer base class interface shared across Linear, Dropout, and Sequential."}
