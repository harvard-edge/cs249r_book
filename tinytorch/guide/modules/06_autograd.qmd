...
<a href="../assets/slides/06_autograd.pdf" download><i class="fa-solid fa-download"></i> Download</a>
<a href="#" onclick="slideFullscreen('06_autograd'); return false;" class="secondary"><i class="fa-solid fa-expand"></i> Fullscreen</a>
</div>
</div>
...
@tbl-06-autograd-implementation-roadmap lays out the implementation in order, one part at a time.

| Part | What You'll Implement | Key Concept |
|------|----------------------|-------------|
| 1 | `Function` base class | Storing inputs and recording graph history |
| 2 | `Add.backward`, `Mul.backward`, `MatMul.backward` | Operation-specific gradient rules and broadcasting reduction |
| 3 | `backward()` method on Tensor | Reverse-mode topological differentiation |
| 4 | Gradient mode toggles (`no_grad`, `is_grad_enabled`) | Disabling graph construction during evaluation |
| 5 | Integration tests | Multi-layer gradient flow and numerical gradient verification |

: **Implementation roadmap for the reverse-mode autograd engine.** {#tbl-06-autograd-implementation-roadmap}
...
Base class for all differentiable operations. Every operation (addition, multiplication, etc.) inherits from `Function` and implements forward and backward gradient computation rules.

### Core Operation Gradient Rules

@tbl-06-autograd-function-classes lists the core operations and the gradient rule implemented in their `backward()` methods.

| Operation | Gradient Rule | Implementation Notes |
|-----------|---------------|----------------------|
| `Add.backward` | $\partial(a+b)/\partial a = 1, \; \partial(a+b)/\partial b = 1$ | Distributes upstream gradient, reduces broadcasted axes |
| `Sub.backward` | $\partial(a-b)/\partial a = 1, \; \partial(a-b)/\partial b = -1$ | Negates second operand gradient, handles broadcasting |
| `Mul.backward` | $\partial(a \cdot b)/\partial a = b, \; \partial(a \cdot b)/\partial b = a$ | Scales by opposite operand data, reduces broadcasting |
| `Div.backward` | $\partial(a/b)/\partial a = 1/b, \; \partial(a/b)/\partial b = -a/b^2$ | Reciprocal and quotient derivatives |
| `MatMul.backward` | $\partial(A B)/\partial A = G B^T, \; \partial(A B)/\partial B = A^T G$ | 2D/batched matrix transposition and multiplication |
| `Sum.backward` | $\partial \sum(x)/\partial x_i = 1$ | Broadcasts scalar gradient back to input shape |
| `Reshape.backward`| $\partial \text{reshape}(x)/\partial x = \text{reshape}(G, x.\text{shape})$ | Reshapes gradient back to original input shape |
| `Permute.backward`| Inverts permutation axes | Applies inverse transposition to gradient array |

: **Core differentiable operations and their gradient rules.** {#tbl-06-autograd-function-classes}

**Additional Backward Implementations:** The implementation also equips activations (`ReLUFunction`, `SigmoidFunction`, `TanhFunction`, `GELUFunction`, `SoftmaxFunction`), losses (`MSEFunction`, `BinaryCrossEntropyFunction`, `CrossEntropyFunction`, `LogSoftmax`), and tensor operations (`Slice`, `MaskedFill`) with backward methods following the same pattern.

### Enhanced Tensor Methods

Your implementation adds these methods to the Tensor class:

@tbl-06-autograd-tensor-methods lists the new methods autograd adds to the `Tensor` class.

| Method | Signature | Description |
|--------|-----------|-------------|
| `backward` | `backward(gradient=None, retain_graph=False) -> None` | Compute gradients via topological backpropagation |
| `zero_grad` | `zero_grad() -> None` | Reset gradients to None (frees gradient memory) |

: **Methods added to the Tensor class for autograd.** {#tbl-06-autograd-tensor-methods}

### Gradient Tracking Modes

@tbl-06-autograd-global-helpers lists the helpers that manage gradient tracking state.

| Function | Signature | Description |
|----------|-----------|-------------|
| `no_grad` | `no_grad() -> ContextManager` | Context manager disabling graph tracking during evaluation |
| `is_grad_enabled` | `is_grad_enabled() -> bool` | Check if autograd recording is currently enabled |

: **Helper utilities for gradient tracking modes.** {#tbl-06-autograd-global-helpers}
...
```{python}
#| label: lst-06-autograd-add-backward
#| lst-cap: "**`Add.backward` retrieves stored inputs and distributes incoming gradients with broadcasting reduction.**"
#| eval: false
@method_of(Add)
def backward(self, grad_output):
    ...
    if isinstance(a, Tensor) and a.requires_grad:
        grad_a = _reduce_broadcast_grad(grad_output, a.data.shape)

    # Gradient for second input
    if isinstance(b, Tensor) and b.requires_grad:
        grad_b = _reduce_broadcast_grad(grad_output, b.data.shape)

    return grad_a, grad_b
```

When you compute `z = x + y`, your enhanced Tensor class automatically creates an `Add` node and attaches it to `z`:

```python
result = x.data + y.data
result_tensor = Tensor(result)
result_tensor._grad_fn = node  # Track operation node containing inputs (x, y)
```
...
Each backward function does exactly one thing: multiply the incoming gradient by its own local derivative. It knows nothing about the rest of the graph — and it doesn't need to. Here's how `Mul.backward` implements this:

The code in @lst-06-autograd-mul-backward makes this concrete.

```{python}
#| label: lst-06-autograd-mul-backward
#| lst-cap: "**`Mul.backward` applies the product rule with NumPy broadcasting and reduction.**"
#| eval: false
@method_of(Mul)
def backward(self, grad_output):
    """
...
```{python}
#| label: lst-06-autograd-tensor-backward
#| lst-cap: "**`Tensor.backward()` sorts the graph topologically, then makes one pass that accumulates into each `.grad` and hands every parent its share.**"
#| eval: false
def backward(self, gradient=None, retain_graph=False):
    ...
    # Step 1: iterative topological sort via DFS
    topo_order = []
    seen = set()
    stack = [(self, False)]
    while stack:
    ...
    # Step 2: seed the output, then accumulate in topological order
    ...
    for tensor in topo_order:
        ...
        for parent, parent_grad in zip(fn.inputs, parent_grads):
            if not isinstance(parent, Tensor) or parent_grad is None or not parent.requires_grad:
                continue
...
The implementation shows this memory overhead clearly in the MatMul operation:

The code in @lst-06-autograd-matmul-backward makes this concrete.

```{python}
#| label: lst-06-autograd-matmul-backward
#| lst-cap: "**`MatMul.backward` turns one forward matmul into two backward matmuls, the source of the 2x backward-to-forward FLOP ratio.**"
#| eval: false
@method_of(MatMul)
def backward(self, grad_output):
    """
...
@tbl-06-autograd-vs-pytorch places your implementation side by side with the production reference for direct comparison.
...
: **Feature comparison between TinyTorch autograd and PyTorch autograd.** {#tbl-06-autograd-vs-pytorch}
...
:::{.callout-tip title="Check Your Understanding — Autograd"}
...
- **PyTorch: An Imperative Style, High-Performance Deep Learning Library** - Paszke et al. (2019). Describes PyTorch's autograd implementation and design philosophy. Shows how imperative programming (define-by-run) enables dynamic computation graphs.
  - **Systems Implication:** By introducing dynamic computation graphs, PyTorch mandated that memory allocation and operator dispatch overhead occur strictly at runtime (eager execution). This paradigm shift required highly optimized C++ backends to aggressively hide Python's interpretive latency. [NeurIPS 2019](https://arxiv.org/abs/1912.01703)

### Additional Resources

- **Textbook**: ["Deep Learning"](https://www.deeplearningbook.org/) by Goodfellow, Bengio, and Courville - Chapter 6 covers backpropagation and computational graphs with excellent visualizations
...
@tbl-06-autograd-downstream-usage traces how this module is reused by later parts of the curriculum.
