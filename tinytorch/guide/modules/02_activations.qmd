...
<a href="../assets/slides/02_activations.pdf" download><i class="fa-solid fa-download"></i> Download</a>
<a href="#" onclick="slideFullscreen('02_activations'); return false;" class="secondary"><i class="fa-solid fa-expand"></i> Fullscreen</a>
</div>
</div>
...
As illustrated in @fig-02_activations-diag-1, each activation function defines a distinct transfer curve: elementwise functions preserve shape while Softmax normalizes across a designated axis. @tbl-02-activations-implementation-roadmap lays out the implementation in order, one part at a time.
...
This section provides a quick reference for the activation classes you'll build. Each activation follows a clean two-tier design: a `Function` subclass executing array operations, wrapped by a callable layer object implementing `parameters(self) -> []`.
...
class ActivationNameFunction(Function):
    def forward(self, x: np.ndarray) -> np.ndarray:
        # Mathematical kernel operating on raw NumPy arrays
        pass

class ActivationName:
    def parameters(self) -> list:
        return []  # Activations own no trainable parameters
...
@tbl-02-activations-function-summary gives the mathematical form, output range, and typical use case of each.
...
```{python}
#| label: lst-02-activations-sigmoid
#| lst-cap: "**Numerically stable Sigmoid using bounded non-positive exponents.**"
#| eval: false
class SigmoidFunction(Function):
    def forward(self, x: np.ndarray) -> np.ndarray:
        # Both branches use bounded exponential z = exp(-|x|), guaranteed <= 0
        z = np.exp(-np.abs(x))
...
Notice how the two branches share the identical bounded term $z = \exp(-\lvert x\rvert)$. Because $-\lvert x\rvert \le 0$ for all $x$, $\exp(-\lvert x\rvert) \in (0, 1]$, making overflow structurally impossible without needing arbitrary hard clipping.
...
The implementation in @lst-02-activations-gelu uses the fast sigmoid approximation from your module.

```{python}
#| label: lst-02-activations-gelu
#| lst-cap: "**GELU using the fast sigmoid approximation, reusing the Sigmoid implementation from this module.**"
#| eval: false
...
```{python}
#| label: lst-02-activations-softmax
#| lst-cap: "**Softmax with max-subtraction and safe masking for numerical stability.**"
#| eval: false
class SoftmaxFunction(Function):
    dim = -1

    def forward(self, x: np.ndarray) -> np.ndarray:
        # Subtract maximum per slice for numerical stability
        x_max = np.max(x, axis=self.dim, keepdims=True)
        # Handle fully masked rows (e.g., -inf in causal attention)
        ...
        # Protect against division by zero on masked slices
        safe_sum = np.where(exp_sum == 0, 1.0, exp_sum)
        return exp_values / safe_sum
...
- Sigmoid/Tanh: 3–4$\times$ (one exponential per element)
- GELU: 4–5$\times$ (exponential plus the approximation polynomial)
...
@tbl-02-activations-relative-cost compares their cost against the ReLU baseline.
...
@tbl-02-activations-vs-pytorch places your implementation side by side with the production reference for direct comparison.

| Feature | Your Implementation | PyTorch |
|---------|---------------------|---------|
| **Backend** | NumPy (Python/C) | C++/CUDA kernels |
| **Speed** | 1× (CPU baseline) | 10-100× faster (GPU) |
| **Numerical Stability** | ✓ Max subtraction & safe masking (Softmax), nonpositive exp (Sigmoid) | ✓ Same techniques |
...
- **Image classification**: ResNet-50 has 49 convolutional layers, each followed by ReLU. A batch of 256 images at $224{\times}224$ resolution executes roughly **12 billion ReLU operations** per batch.
...
GELU is approximately **4--5$\times$ slower** than ReLU due to exponential computation in the sigmoid approximation.
...
**With max subtraction**: First compute `x_shifted = x - max(x) = [1000, 1001, 1002] - 1002 = [-2, -1, 0]`, then compute `exp([-2, -1, 0]) ≈ [0.1353, 0.3679, 1.0]` where all exponents are nonpositive and immune to overflow.
...
- **Blog**: [Understanding Activation Functions](https://mlu-explain.github.io/neural-networks/) - Amazon's MLU visual explanation of ReLU
