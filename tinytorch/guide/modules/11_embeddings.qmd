...
<a href="../assets/slides/11_embeddings.pdf" download><i class="fa-solid fa-download"></i> Download</a>
<a href="#" onclick="slideFullscreen('11_embeddings'); return false;" class="secondary"><i class="fa-solid fa-expand"></i> Fullscreen</a>
</div>
</div>
...
@tbl-11-embeddings-implementation-roadmap lays out the implementation in order, one part at a time.
...
: **Implementation roadmap for the embedding classes.** {#tbl-11-embeddings-implementation-roadmap}
...
@tbl-11-embeddings-token-api lists the methods on this class.
...
: **Core methods on the Embedding class.** {#tbl-11-embeddings-token-api}
...
@tbl-11-embeddings-positional-api lists the methods on this class.
...
: **Core methods on the PositionalEncoding class.** {#tbl-11-embeddings-positional-api}
...
@tbl-11-embeddings-combined-api lists the methods on this class.
...
: **Core methods on the combined token+position Embedding wrapper.** {#tbl-11-embeddings-combined-api}
...
```{python}
#| label: lst-11-embedding-forward
#| lst-cap: "**Embedding forward pass**. Gathers embedding rows via EmbeddingFunction.apply, recording operations for autograd."
#| eval: false
def forward(self, indices: Tensor) -> Tensor:
    ...
    # Perform embedding lookup through EmbeddingFunction for autograd tracking
    return EmbeddingFunction.apply(self.weight, indices=indices.data.astype(int))
```

Under the hood, `EmbeddingFunction.forward` performs `weight[self.indices]`. NumPy's fancy indexing pulls the rows in a single gather operation, broadcasting cleanly over batched inputs of any shape. During the backward pass, `EmbeddingFunction.backward` uses `np.add.at` to scatter incoming gradients back into only the accessed rows of the weight matrix.
...
```{python}
#| label: lst-11-positional-forward
#| lst-cap: "**PositionalEncoding forward pass**. Slices position embeddings to current sequence length and reshapes to add batch dimension while preserving autograd tracking."
#| eval: false
def forward(self, x: Tensor, start_pos: int = 0) -> Tensor:
    """Add positional encodings to input embeddings."""
    batch_size, seq_len, embed_dim = x.shape

    # Slice position embeddings for current sequence segment
    # start_pos is 0 during whole-sequence training; during KV-cached generation,
    # it corresponds to the position of the newly generated token
    pos_embeddings = self.position_embeddings[start_pos:start_pos + seq_len]

    # Reshape to (1, seq_len, embed_dim) for batch broadcasting — preserves autograd!
    pos_embeddings_batched = pos_embeddings.reshape(1, seq_len, embed_dim)

    # Element-wise addition: gradients flow through both x and position_embeddings
    return x + pos_embeddings_batched
```
...
```{python}
#| label: lst-11-sinusoidal-pe
#| lst-cap: "**Sinusoidal positional encoding construction**. Builds a fixed position matrix from trigonometric functions at geometrically spaced frequencies."
#| eval: false
...
@tbl-11-embeddings-production-sizes places your implementation in context against production models.
...
: **Vocabulary size, embedding dimension, and memory cost across production LLMs.** {#tbl-11-embeddings-production-sizes}
...
# Option 2: Use sinusoidal, then extend `max_seq_len` (the table is still allocated, and running past it raises)
embed_layer = EmbeddingLayer(vocab_size=50000, embed_dim=512,
...
@tbl-11-embeddings-vs-pytorch places your implementation side by side with the production reference for direct comparison.
...
: **Feature comparison between TinyTorch embeddings and torch.nn.Embedding.** {#tbl-11-embeddings-vs-pytorch}
...
- **Blog Post**: ["The Illustrated Word2Vec"](https://jalammar.github.io/illustrated-word2vec/) by Jay Alammar - Visual explanation of learned word embeddings and semantic relationships
...
- **Paper**: ["GloVe: Global Vectors for Word Representation"](https://nlp.stanford.edu/pubs/glove.pdf) - Pennington et al. (2014) - Alternative embedding approach based on co-occurrence statistics
