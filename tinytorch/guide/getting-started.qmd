...
The milestone executes your implementations to recreate Rosenblatt's 1958 Perceptron:

![An excerpt of a typical run of `tito milestone run perceptron` after completing Modules 01 through 03. The weights are random and unseeded, so your points, weights, and accuracy will differ, and now and then a run guesses right by luck; the dim lines mark output left out of the figure.](assets/images/screenshots/milestone_01_run.svg){#fig-milestone-01-terminal fig-alt="Terminal output of tito milestone run perceptron: the prerequisite check passes for Modules 01 to 03, the forward pass with random weights gets every prediction wrong, and the diagnosis says the model is guessing."}

You're recreating ML history with your own code. *By Module 19, you'll benchmark your own framework using MLPerf's measurement discipline.*
...
| 01-08 + 11-13 | Transformer Era (2017) | Structured sequence tasks with attention |
| 01-08 + 11-19 | MLPerf Benchmarks (2018) | Production optimization pipeline |
...
|------|---------|-------|---------------|
| **Foundation** | 01-08 | Core ML infrastructure (tensors, dataloader, autograd, training) | 33-49 hours |
| **Architecture** | 09-13 | Neural architectures (CNNs, transformers) | 26-36 hours |
| **Optimization** | 14-19 | Production optimization (profiling, quantization) | 25-37 hours |
| **Capstone** | 20 | TinyTorch Olympics Competition | 6-8 hours |

: **TinyTorch module tiers with scope and time estimate.** {#tbl-getting-started-module-tiers}

**Total: 90-130 hours**, the sum of the per-module estimates. At 6 hours a week that is a two-semester pace; at 12, one semester.
...
TinyTorch is designed for both self-paced learners and university classrooms (such as Harvard CS249r). Instructors and teaching assistants can use the built-in `tito nbgrader` suite to stage assignments, customize implementation scaffolding, generate student releases, and autograde submissions.
...
|:---|:---|:---|:---|
| **Student** *(Default)* | `--tier student` | **47 core archetypes** (189 repetitive helper blocks scaffolded) | Standard university courses, 14-week semesters, balanced homework loads |
| **Challenge** | `--tier challenge` | Regions marked `role="challenge"` *(currently unannotated: no `role="challenge"` region exists yet, so `tito nbgrader generate --tier challenge` refuses rather than emit the full reference)* | Advanced graduate seminars, systems hackathons, honors tracks |
...
# 2. Stage student assignment with the 47 core archetypes
