...
:::{.callout-warning title="💾 Systems Implication: The Hardware Lottery"}
...
lr1_original = f"{lr1_original:,}"
lr1_k = f"{lr1_k}"
lr1_u = f"{lr1_m * lr1_k:,}"
lr1_s = f"{lr1_k:,}"
lr1_v = f"{lr1_k * lr1_n:,}"
lr1_compressed = f"{lr1_compressed:,}"
lr1_ratio = f"{lr1_ratio:.2f}x"
lr1_reduction_pct = f"{lr1_reduction_pct:.0f}%"
...
lr2_original = f"{lr2_original:,}"
lr2_k = f"{lr2_k}"
lr2_u = f"{lr2_m * lr2_k:,}"
lr2_s = f"{lr2_k:,}"
lr2_v = f"{lr2_k * lr2_n:,}"
lr2_compressed = f"{lr2_compressed:,}"
lr2_ratio = f"{lr2_ratio:.1f}x"
lr2_reduction_pct = f"{lr2_reduction_pct:.0f}%"
```
...
The core idea is matrix factorization. Instead of storing a full (512, 256) weight matrix with `{python} lr1_original` parameters, you decompose it into smaller factors that capture the essential structure:
...
- Original: 512 × 256 = `{python} lr1_original` parameters
- Compressed: (512 × `{python} lr1_k`) + `{python} lr1_s` + (`{python} lr1_k` × 256) = `{python} lr1_compressed` parameters
- Compression ratio: `{python} lr1_ratio` (`{python} lr1_reduction_pct` reduction)

The compression ratio improves with larger matrices. For a (1024, 1024) matrix at rank_ratio=0.1:
- Original: `{python} lr2_original` parameters
- Compressed: (1024 × `{python} lr2_k`) + `{python} lr2_s` + (`{python} lr2_k` × 1024) = `{python} lr2_compressed` parameters
- Compression ratio: `{python} lr2_ratio` (`{python} lr2_reduction_pct` reduction)
...
deploy_model_mb = f"{deploy_model_mb} MB"
deploy_sparse_mb = f"{deploy_sparse_mb:.0f} MB"
```
...
A `{python} deploy_model_mb` model pruned to 90% sparsity becomes `{python} deploy_sparse_mb` with sparse storage, fitting mobile constraints. The same model distilled to a 1MB student runs 10x faster, meeting latency requirements. These aren't theoretical gains; they're necessary for deployment.
...
q1_total = f"{q1_total:,}"
q1_active = f"{q1_active:,}"
q1_zeroed = f"{q1_zeroed:,}"
...
q2_final_active = f"{q2_final_active_pct:.0f}%"
q2_final_sparse = f"{q2_final_sparse_pct:.0f}%"
...
q3_compression = f"{q3_compression:.0f}x"
q3_speedup = f"{q3_speedup:.0f}x"
q3_acc_loss = f"{q3_acc_loss:.0f}%"
...
q4_original = f"{q4_original:,}"
q4_u_params = f"{q4_u_params:,}"
q4_s_params = f"{q4_s_params:,}"
q4_v_params = f"{q4_v_params:,}"
q4_compressed = f"{q4_compressed:,}"
q4_ratio = f"~{q4_ratio:.0f}x"
q4_savings_mb = f"{q4_savings_mb:.1f} MB"
```
...
Total parameters: 512 × 256 = **`{python} q1_total`**

After 80% pruning: 20% remain active = `{python} q1_total` × 0.2 = **`{python} q1_active` active weights**

Zeroed weights: `{python} q1_total` × 0.8 = **`{python} q1_zeroed` zeros**
...
- Final active weights ≈ 10% × 50% = **`{python} q2_final_active` active → `{python} q2_final_sparse` sparse**
...
**Compression ratio**: 100M / 10M = **`{python} q3_compression` smaller**

**Speedup**: 500ms / 50ms = **`{python} q3_speedup` faster**

**Accuracy loss**: 95% - 92% = **`{python} q3_acc_loss` degradation**

Why speedup matches compression: Student has 10x fewer parameters, so 10x fewer operations. Linear scaling!

Is this good? **Yes** - `{python} q3_compression` compression with only `{python} q3_acc_loss` accuracy loss is excellent for mobile deployment.
:::
...
Original: 1000 × 1000 = **`{python} q4_original` parameters**

SVD decomposition: W ≈ U @ S @ V
- U: (1000, 100) = `{python} q4_u_params` parameters
- S: (100,) = `{python} q4_s_params` parameters (diagonal)
- V: (100, 1000) = `{python} q4_v_params` parameters

Compressed: `{python} q4_u_params` + `{python} q4_s_params` + `{python} q4_v_params` = **`{python} q4_compressed` parameters**

Compression ratio: `{python} q4_original` / `{python} q4_compressed` = **`{python} q4_ratio` reduction**

Memory savings: (`{python} q4_original` - `{python} q4_compressed`) × 4 bytes = **`{python} q4_savings_mb` saved** (float32)
:::
