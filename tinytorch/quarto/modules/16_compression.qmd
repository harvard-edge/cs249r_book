...
When you zero channel `i` in layer `l`, you don't just zero some weights — you eliminate a column of forward connections, a column of gradient computation, and the channel's slot in activation memory. That is what turns sparsity into speed:

1. **Memory coalescing** — the remaining dense channels live in contiguous memory.
2. **SIMD execution** — CPUs and GPUs process the surviving channels in parallel without masking.
3. **No indexing overhead** — no sparse format needed; you can literally rebuild a smaller dense matrix.
4. **Cache efficiency** — dense access patterns keep spatial locality.

The trade-off is honest: structured pruning maxes out around 30–50% sparsity, well below the 80–90% magnitude pruning can hit. But on real GPUs and accelerators, structured sparsity delivers 2–3x speedup out of the box, while unstructured sparsity needs custom kernels to deliver anything at all.
