...
:::{.callout-warning title="💾 Systems Implication: CPU/GPU Sync Overhead"}
...
epoch_iters_10k = f"{epoch_iters_10k:,}"
...
An iteration processes one batch: forward pass, backward pass, optimizer step. If your dataset has 10,000 samples and batch size is 32, one epoch requires `{python} epoch_iters_10k` iterations (10,000 ÷ 32, rounded up). Training a model to convergence typically requires dozens or hundreds of epochs, meaning tens of thousands of iterations.

The mathematics is straightforward but the implications are significant. Training ImageNet with 1.2 million images, batch size 256, for 90 epochs requires `{python} imagenet_total_iters` iterations (1,200,000 ÷ 256 × 90). At 250ms per iteration, that's `{python} imagenet_hours` hours of compute. Understanding this arithmetic helps you estimate training costs and debug slow convergence.
...
cosine_lr_0 = f"{lr_epoch_0:.3f}"
cosine_lr_25 = f"{lr_epoch_25:.3f}"
cosine_lr_50 = f"{lr_epoch_50:.3f}"
cosine_lr_75 = f"{lr_epoch_75:.3f}"
cosine_lr_100 = f"{lr_epoch_100:.3f}"
```

Visualizing the schedule for `max_lr=0.1`, `min_lr=0.01`, `total_epochs=100`:

Epoch   0:  `{python} cosine_lr_0` (aggressive learning)
Epoch  25:  `{python} cosine_lr_25` (still fast)
Epoch  50:  `{python} cosine_lr_50` (slowing down)
Epoch  75:  `{python} cosine_lr_75` (fine-tuning)
Epoch 100:  `{python} cosine_lr_100` (stable convergence)
...
clip_norm_prose = f"{clip_norm_prose:.0f}"
clip_coef_prose = f"{clip_coef_prose:.5f}"
clip_g1_prose = f"{clipped_prose[0]:.3f}"
clip_g2_prose = f"{clipped_prose[1]:.3f}"
clip_g3_prose = f"{clipped_prose[2]:.3f}"
```
...
Consider gradients [100, 200, 50] with global norm √(100² + 200² + 50²) = `{python} clip_norm_prose`. With max_norm=1.0, we compute clip_coef = 1.0 / `{python} clip_norm_prose` = `{python} clip_coef_prose` and scale all gradients: [`{python} clip_g1_prose`, `{python} clip_g2_prose`, `{python} clip_g3_prose`]. The new norm is exactly 1.0, but the relative magnitudes are preserved (the second gradient is still twice the first).
...
d_sq_L = d**2 * L
comp_d_sq_L = f"{d_sq_L:,}"

batch_ops = d_sq_L * B
batch_ops_M = batch_ops / 1e6
comp_batch_ops = f"{batch_ops_M:.1f}"

iters_per_epoch = math.ceil(N / B)
comp_iters_per_epoch = f"{iters_per_epoch:,}"

total_iters = iters_per_epoch * E
comp_total_iters = f"{total_iters:,}"

total_ops = total_iters * batch_ops
total_ops_B = total_ops / 1e9
comp_total_ops_B = f"{total_ops_B:.0f}"

cpu_seconds = total_ops / 1e9
cpu_minutes = cpu_seconds / 60
comp_cpu_seconds = f"{cpu_seconds:.0f}"
comp_cpu_minutes = f"{cpu_minutes:.0f}"

gpu_seconds = total_ops / 1e12
comp_gpu_seconds = f"{gpu_seconds:.1f}"
```
...
d² × L = 512² × 2 = `{python} comp_d_sq_L` operations per sample
Batch operations = `{python} comp_d_sq_L` × 32 = `{python} comp_batch_ops` million ops
Iterations per epoch = 10,000 / 32 = `{python} comp_iters_per_epoch`
Total iterations = `{python} comp_iters_per_epoch` × 100 = `{python} comp_total_iters`
Total operations = `{python} comp_total_iters` × `{python} comp_batch_ops`M = `{python} comp_total_ops_B` billion operations

At 1 billion operations per second (typical CPU), that's `{python} comp_cpu_seconds` seconds (`{python} comp_cpu_minutes` minutes). This arithmetic explains why GPUs matter: a GPU at 1 trillion ops/second (1000× faster) completes this in `{python} comp_gpu_seconds` seconds.
...
imagenet_total_iters = imagenet_iters_per_epoch * 90
imagenet_hours = imagenet_total_iters * 0.250 / 3600
prod_imagenet_hours = f"{imagenet_hours:.0f}"
```
...
- **ImageNet training**: 1.2 million images, 90 epochs standard. At 250ms per iteration (batch size 256), that's **`{python} prod_imagenet_hours` hours** on one GPU. Learning rate scheduling is the difference between 75% accuracy (poor) and 76.5% accuracy (state-of-the-art).
...
q1_param_mb = f"{param_mb:.0f}"

adam_moments_mb = num_params * 2 * bytes_per_param / 1e6
q1_adam_moments_mb = f"{adam_moments_mb:.0f}"

adam_total_mb = param_mb + param_mb + adam_moments_mb  # params + grads + 2 moments
q1_adam_total_mb = f"{adam_total_mb:.0f}"

sgd_momentum_mb = param_mb  # one momentum buffer
sgd_total_mb = param_mb + param_mb + sgd_momentum_mb  # params + grads + momentum
q1_sgd_total_mb = f"{sgd_total_mb:.0f}"

mem_diff_pct = (adam_total_mb - sgd_total_mb) / sgd_total_mb * 100
q1_mem_diff_pct = f"{mem_diff_pct:.0f}"
```

:::{.callout-note collapse="true" title="Answer"}

**Adam optimizer:**
- Parameters: 10M × 4 bytes = **`{python} q1_param_mb` MB**
- Gradients: 10M × 4 bytes = **`{python} q1_param_mb` MB**
- Adam state (two moments): 10M × 2 × 4 bytes = **`{python} q1_adam_moments_mb` MB**
- **Total: `{python} q1_adam_total_mb` MB** (4× parameter size)

**SGD with momentum:**
- Parameters: 10M × 4 bytes = **`{python} q1_param_mb` MB**
- Gradients: 10M × 4 bytes = **`{python} q1_param_mb` MB**
- Momentum buffer: 10M × 4 bytes = **`{python} q1_param_mb` MB**
- **Total: `{python} q1_sgd_total_mb` MB** (3× parameter size)

**Key insight:** Optimizer choice affects memory by `{python} q1_mem_diff_pct`%. For large models near GPU memory limits, SGD may be the only option.
:::
...
q4_total_every_epoch_gb = f"{total_every_epoch_gb:,}"

last_n = 3
best = 1
milestones = 3  # every 25 epochs: 25, 50, 75
smart_total_gb = (last_n + best + milestones) * ckpt_size_gb
q4_smart_total_gb = f"{smart_total_gb:,}"
```

You're training for 100 epochs. Each checkpoint is 1 GB. Checkpointing every epoch creates `{python} q4_total_every_epoch_gb` GB of storage. Checkpointing every 10 epochs risks losing 10 epochs of work if training crashes. Design a checkpointing strategy that balances fault tolerance and storage costs.
...
**Total storage: `{python} q4_smart_total_gb` GB** (vs `{python} q4_total_every_epoch_gb` GB for every epoch)
...
q5_global_norm = f"{global_norm_q5:.0f}"

scale_factor_q5 = 1.0 / global_norm_q5
q5_scale_factor = f"{scale_factor_q5:.4f}"

clipped_q5 = [g * scale_factor_q5 for g in grads_q5]
q5_clipped_g1 = f"{clipped_q5[0]:.2f}"
q5_clipped_g2 = f"{clipped_q5[1]:.2f}"
q5_clipped_g3 = f"{clipped_q5[2]:.2f}"

verify_norm_q5 = math.sqrt(sum(g**2 for g in clipped_q5))
q5_verify_norm = f"{verify_norm_q5:.1f}"
```

Two training runs: (A) clips each gradient individually to max 1.0, (B) clips by global norm (max_norm=1.0). Both encounter gradients [50, 100, 5] with global norm √(50² + 100² + 5²) ≈ `{python} q5_global_norm`. What are the clipped gradients in each case? Which preserves gradient direction better?
...
- Original: [50, 100, 5], global norm ≈ `{python} q5_global_norm`
- Scale factor: 1.0 / `{python} q5_global_norm` ≈ `{python} q5_scale_factor`
- Clipped: [`{python} q5_clipped_g1`, `{python} q5_clipped_g2`, `{python} q5_clipped_g3`]
- New global norm: **`{python} q5_verify_norm`** (exactly max_norm)
...
**Verification:** √(`{python} q5_clipped_g1`² + `{python} q5_clipped_g2`² + `{python} q5_clipped_g3`²) ≈ `{python} q5_verify_norm` ✓
:::
