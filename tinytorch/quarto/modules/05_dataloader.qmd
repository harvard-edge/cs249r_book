...
<a href="../assets/slides/05_dataloader.pdf" download><i class="fa-solid fa-download"></i> Download</a>
<a href="#" onclick="slideFullscreen('05_dataloader'); return false;" class="secondary"><i class="fa-solid fa-expand"></i> Fullscreen</a>
</div>
</div>
...
As illustrated in @fig-05_dataloader-diag-1, the data pipeline transforms individual raw samples into training-ready batches through dataset indexing, shuffling, and batch collation. @tbl-05-dataloader-implementation-roadmap lays out the implementation in order, one part at a time.
...
This section provides a quick reference for the dataset and dataloader components you'll build. Use it as your cheat sheet while implementing and debugging.
...
- `*tensors`: One or more `Tensor` objects with identical first dimension (length)
- All tensors must have matching size along dimension 0 (number of samples)
...
len(dataset)       # 3
dataset[0]         # (Tensor([1.0, 2.0]), Tensor(0))
```
...
Wraps a dataset to provide batched, optionally shuffled iteration.

**Constructor Arguments:**

- `dataset`: Any object implementing the `Dataset` interface (`__len__` and `__getitem__`)
- `batch_size`: Positive integer specifying number of samples per batch
- `shuffle`: Boolean (default `False`). If `True`, data is shuffled before each epoch

**Core Methods:**

@tbl-05-dataloader-dataloader-api lists the methods on the DataLoader class.

| Method | Returns | Description |
|--------|---------|-------------|
| `__len__()` | `int` | Total number of batches (ceiling division: `ceil(N / B)`) |
...
```python
RandomHorizontalFlip(p=0.5, layout=None)
RandomCrop(size, padding=4, layout=None)
Compose(transforms)
```
...
```{python}
#| label: lst-05-dataloader-collate
#| lst-cap: "**`_collate_batch` stacking per-position samples into contiguous batch tensors.**"
#| eval: false
...
```{python}
#| label: lst-05-dataloader-iter
#| lst-cap: "**DataLoader `__iter__` permuting indices and yielding batches lazily via a generator.**"
#| eval: false
...
Python's iterator protocol enables `for batch in dataloader` syntax. When Python encounters this loop, it first calls `dataloader.__iter__()` to get an iterator object. As seen in @lst-05-dataloader-iter, your `__iter__` method is a generator function (contains `yield`), so Python automatically creates a generator that produces values lazily.
...
| **Drop Last** | Always keeps partial final batch | Configurable (`drop_last=True/False`) |
