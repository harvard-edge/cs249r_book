...
bert_mb = f"{bert_mb:.0f} MB"
gpt2_gb = f"{gpt2_gb:.1f} GB"
gpt3_gb = f"{gpt3_gb:.0f} GB"

# Quantized BERT (INT8 = 1 byte per param)
bert_int8_mb = bert_params * 1 / 1024**2
bert_int8_mb = f"{bert_int8_mb:.0f} MB"
```

Modern neural networks face a memory wall problem. A BERT model requires `{python} bert_mb`, GPT-2 needs `{python} gpt2_gb`, and GPT-3 demands `{python} gpt3_gb`, yet mobile devices have only 4-8 GB of RAM. The culprit? Every parameter uses 4 bytes of FP32 precision, representing values with 32-bit accuracy when 8 bits often suffice. Quantization solves this by converting FP32 weights to INT8, achieving 4× memory reduction with less than 1% accuracy loss.
...
q1_fp32_mb = f"{q1_fp32_bytes / 1024**2:.2f} MB"
q1_int8_bytes = f"{q1_int8_bytes:,}"
q1_int8_mb = f"{q1_int8_bytes / 1024**2:.2f} MB"
q1_savings_mb = f"{q1_savings / 1024**2:.2f} MB"
...
q2_scale = f"{q2_scale:.6f}"
q2_max_error = f"±{q2_max_error:.6f}"
q2_snr = f"{q2_snr:.0f} dB"
...
q4_fp32_mb = f"{q4_fp32_mb:.0f} MB"
q4_int8_mb = f"{q4_int8_mb:.0f} MB"
q4_fp32_time = f"{q4_fp32_time:.1f} seconds"
q4_int8_time = f"{q4_int8_time:.2f} seconds"
q4_time_saved = f"{q4_fp32_time - q4_int8_time:.1f}s"

# Q5: SIMD register capacity
simd_bits = 512
fp32_per_reg = simd_bits // 32
int8_per_reg = simd_bits // 8
q5_fp32 = f"{fp32_per_reg}"
q5_int8 = f"{int8_per_reg}"
q5_ratio = f"{int8_per_reg // fp32_per_reg}×"
```
...
**Parameter count:**
- Layer 1: (784 × 256) + 256 = `{python} q1_l1`
- Layer 2: (256 × 128) + 128 = `{python} q1_l2`
- Layer 3: (128 × 10) + 10 = `{python} q1_l3`
- **Total: `{python} q1_total` parameters**

**Memory usage:**
- FP32: `{python} q1_total` × 4 bytes = **`{python} q1_fp32_bytes` bytes ≈ `{python} q1_fp32_mb`**
- INT8: `{python} q1_total` × 1 byte = **`{python} q1_int8_bytes` bytes ≈ `{python} q1_int8_mb`**
- **Savings: `{python} q1_savings_mb` (75% reduction, 4× compression)**
...
**Quantization error:**
- Range: 0.5 - (-0.5) = 1.0
- Scale: 1.0 / 255 = **`{python} q2_scale`**
- Max error: scale / 2 = **`{python} q2_max_error`** (half step size)

**Signal-to-noise ratio:**
- SNR = 20 × log₁₀(signal_range / quantization_step)
- SNR = 20 × log₁₀(1.0 / `{python} q2_scale`)
- SNR = 20 × log₁₀(255)
- SNR ≈ **`{python} q2_snr`**
...
**Loading time:**
- FP32 size: 100M × 4 bytes = `{python} q4_fp32_mb`
- INT8 size: 100M × 1 byte = `{python} q4_int8_mb`
- FP32 load time: `{python} q4_fp32_mb` / 500 MB/s = **`{python} q4_fp32_time`**
- INT8 load time: `{python} q4_int8_mb` / 500 MB/s = **`{python} q4_int8_time`**
- **Speedup: 4× faster loading**

**User experience impact:**
- Mobile app launch: `{python} q4_fp32_time` → `{python} q4_int8_time` (**`{python} q4_time_saved` faster startup**)
- Cloud inference: `{python} q4_fp32_time` latency → `{python} q4_int8_time` latency (**4× better throughput**)
- Model updates: `{python} q4_fp32_mb` download → `{python} q4_int8_mb` download (**75% less data usage**)
...
**SIMD capacity:**
- 512-bit register with FP32: 512 / 32 = **`{python} q5_fp32` values**
- 512-bit register with INT8: 512 / 8 = **`{python} q5_int8` values**
- **Theoretical speedup: `{python} q5_int8`/`{python} q5_fp32` = `{python} q5_ratio`**
