...
The forward pass dequantizes weights on the fly, runs an FP32 matmul, and returns FP32 outputs. This is the *teaching* implementation: every step is visible. Production runtimes skip the round trip and execute the matmul directly in INT8 using GEMM kernels (FBGEMM, QNNPACK, cuDNN's INT8 path), which is where the speedup actually comes from — memory savings are free, but compute speedup requires INT8-native arithmetic.
...
- **Mobile**: A phone has 6–8 GB of RAM shared across every app. A quantized BERT (105 MB) fits without disturbing the rest of the system; the FP32 version (420 MB) thrashes memory and gets killed by the OS.
- **Edge**: IoT boards often ship with 512 MB of RAM. Quantization is what makes on-device inference possible at all for privacy-sensitive workloads (medical devices, security cameras).
- **Data center**: Serving at 1000 req/s needs multiple model replicas per GPU. 4× compression means 4× the replicas per GPU, which means roughly 75% off the serving bill.
- **Battery**: INT8 ops cost 2–4× less energy than FP32 on mobile silicon. The model that runs on your phone is also the one that drains your battery — quantization buys back hours.
