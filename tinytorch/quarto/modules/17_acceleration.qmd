...
Consider an ideal compiled kernel processing 1,000,000 float32 elements. A simplified staged model transfers 16 full arrays, while the fused model transfers two. How much traffic does the model save? This is not the traffic measured by TinyTorch’s grouped NumPy expression.
...
The simplified model saves 56,000,000 bytes (about 53.4 MiB). Actual runtime depends on allocation, cache reuse, arithmetic, and the implementation; traffic alone does not establish end-to-end throughput.
:::
