...
:::{.callout-warning title="💾 Systems Implication: Activation Pinning"}
...
mem_x_bytes = f"{mem_x_bytes:,}"
mem_x_kb = f"{mem_x_kb:,.0f}"

mem_W_bytes = 512 * 768 * 4
mem_W_kb = mem_W_bytes / 1024
mem_W_bytes = f"{mem_W_bytes:,}"
mem_W_kb = f"{mem_W_kb:,.0f}"

mem_grad_x_kb = mem_x_kb
mem_grad_x_bytes = f"{mem_x_bytes:,}"
mem_grad_x_kb = f"{mem_grad_x_kb:,.0f}"

mem_grad_W_kb = mem_W_kb
mem_grad_W_bytes = f"{mem_W_bytes:,}"
mem_grad_W_kb = f"{mem_grad_W_kb:,.0f}"

mem_grad_b_bytes = 768 * 4
mem_grad_b_kb = mem_grad_b_bytes / 1024
mem_grad_b_bytes = f"{mem_grad_b_bytes:,}"
mem_grad_b_kb = f"{mem_grad_b_kb:,.0f}"

mem_total_bytes = mem_x_bytes + mem_W_bytes + mem_x_bytes + mem_W_bytes + mem_grad_b_bytes
mem_total_mb = mem_total_bytes / 1024**2
mem_total_mb = f"{mem_total_mb:.1f}"
```

For a batch of 32 samples through a (512, 768) linear layer, the memory breakdown is:

Forward storage:
  x: 32 × 512 × 4 bytes = `{python} mem_x_kb` KB
  W: 512 × 768 × 4 bytes = `{python} mem_W_kb` KB

Backward storage:
  grad_x: 32 × 512 × 4 bytes = `{python} mem_grad_x_kb` KB
  grad_W: 512 × 768 × 4 bytes = `{python} mem_grad_W_kb` KB
  grad_b: 768 × 4 bytes = `{python} mem_grad_b_kb` KB

Total: ~`{python} mem_total_mb` MB for one layer (2× parameter size + activation size)
...
q1_total_kb = f"{q1_total_kb:.1f}"
q1_total_mb = f"{q1_total_mb:.2f}"
```

:::{.callout-note collapse="true" title="Answer"}

Layer 1 input: 64 × 784 × 4 bytes = `{python} q1_l1_kb` KB
Layer 2 input: 64 × 512 × 4 bytes = `{python} q1_l2_kb` KB
Layer 3 input: 64 × 256 × 4 bytes = `{python} q1_l3_kb` KB
Layer 4 input: 64 × 128 × 4 bytes = `{python} q1_l4_kb` KB
Layer 5 input: 64 × 10 × 4 bytes = `{python} q1_l5_kb` KB

**Total: ~`{python} q1_total_kb` KB = `{python} q1_total_mb` MB**
...
q3_model_gb = f"{q3_model_gb:.0f}"
q3_grad_gb = f"{q3_grad_gb:.0f}"
q3_opt_gb = f"{q3_opt_gb:.0f}"
q3_total_gb = f"{q3_total_gb:.0f}"
```

:::{.callout-note collapse="true" title="Answer"}

Model parameters: 1B × 4 bytes = `{python} q3_model_gb` GB
Gradients: 1B × 4 bytes = `{python} q3_grad_gb` GB
Optimizer state (Adam): 1B × 8 bytes = `{python} q3_opt_gb` GB (momentum + variance)

**Total framework overhead: `{python} q3_total_gb` GB**
...
q4_batch_values = f"{q4_batch_values / 1e6:.1f}M"
q4_batch_bytes = f"{q4_batch_bytes:,}"
q4_batch_mb = f"{q4_batch_mb:.1f}"
```
...
This is why PyTorch defaults requires_grad=False for new tensors and requires explicit opt-in for parameters. For a batch of 32 images with 3×224×224 pixels each, tracking gradients wastes `{python} q4_batch_values` values × 4 bytes = `{python} q4_batch_mb` MB for the batch!
:::
...
:::
ne, or clone the repository for persistent local work.
:::
ocal work.
:::
