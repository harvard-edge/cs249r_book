...
Warmup is non-negotiable. The first few runs are slower because of cold CPU caches, Python interpreter warmup, and NumPy initialization. Ten or more warmup iterations push the system into steady state before any measurement counts.
...
The single most useful piece of profiling data is whether a workload is compute-bound or memory-bound. That answer dictates which optimization is worth your time.

**Compute-bound** — limited by arithmetic throughput. The chip can't multiply fast enough to keep up with the memory feeding it. Fix it with better algorithms, vectorization, or fewer FLOPs.

**Memory-bound** — limited by data movement. The chip can multiply faster than memory can deliver operands. Fix it with cache locality, data layout, fused kernels, or reduced precision.

Your profiler distinguishes the two by comparing achieved GFLOP/s to peak. Low achieved GFLOP/s on capable hardware means memory-bound; high achieved GFLOP/s near the peak means compute-bound. Pick the wrong category and your optimization will be a no-op.
