...
@tbl-02-activations-implementation-roadmap lays out the implementation in order, one part at a time.
...
#| lst-cap: "**Numerically stable Sigmoid using a piecewise formula to avoid overflow on either side of zero.**"
#| eval: false
class Sigmoid:
    def forward(self, x: Tensor) -> Tensor:
...
#| lst-cap: "**Softmax with max-subtraction for numerical stability (the log-sum-exp trick).**"
#| eval: false
class Softmax:
