...
<a href="../assets/slides/18_kvcache.pdf" download><i class="fa-solid fa-download"></i> Download</a>
<a href="#" onclick="slideFullscreen('18_memoization'); return false;" class="secondary"><i class="fa-solid fa-expand"></i> Fullscreen</a>
</div>
</div>
...
Memoization is the opposite move: a single, narrow optimization that targets one structural property of autoregressive generation — that past keys and values never change. You spend O(n) memory to avoid repeated K,V projections. The cache is valid when earlier tokens retain the same causal context and model parameters; changing either requires a reset.
...
@tbl-18-memoization-implementation-roadmap lays out the implementation in order, one part at a time.

| Step | What You'll Implement | Key Concept |
|------|----------------------|-------------|
| 1 | `KVCache.__init__()` | Pre-allocated cache storage per layer |
| 2 | `KVCache.update()` | Append one token without copying the existing prefix |
| 3 | `KVCache.get()` | Retrieve the valid prefix; Tensor construction copies it |
| 4 | `enable_kv_cache()` | Non-invasive model enhancement |
| 5 | Performance analysis | Measure speedup and memory usage |

: **Implementation roadmap for the KV cache data structure.** {#tbl-18-memoization-implementation-roadmap}
...
@tbl-18-memoization-cache-methods lists the methods and helpers you will implement.
...
: **Core methods on the KVCache class.** {#tbl-18-memoization-cache-methods}

### Helper Functions

@tbl-18-memoization-helper-functions lists the methods and helpers you will implement.
...
: **Helper functions for enabling and disabling the KV cache.** {#tbl-18-memoization-helper-functions}
...
Memoization trades memory for speed by storing computation results for reuse. When a function is called with the same inputs repeatedly, computing the result once and caching it eliminates redundant work. This trade-off helps when the saved computation outweighs cache access costs and the cache fits the memory budget.
...
```{python}
#| label: lst-18-memoization-update
#| lst-cap: "**O(1) KV cache append into pre-allocated per-layer storage**."
#| eval: false
...
This update writes one token into pre-allocated storage. Its cost is constant in sequence length for fixed batch size, head count, and head dimension. It copies the new K,V values without resizing or copying the existing prefix. The use of `.data` accesses the underlying NumPy array directly, avoiding gradient tracking overhead since caching is inference-only.
...
```{python}
#| label: lst-18-memoization-reset
#| lst-cap: "**Cache reset clearing per-layer K and V storage between sequences**."
#| eval: false
def reset(self) -> None:
...
Example (GPT-2 Small):
  ...
  Memory = 2 × 12 × 12 × 1024 × 64 × 4 = 75,497,472 bytes ≈ 72 MiB
```

For a 125M-parameter model (500 MB of weights), that cache adds about 15% memory overhead — modest, until you weigh it against the compute it saves.
...
@tbl-18-memoization-cache-scaling shows how KV-cache savings scale with sequence length.

| Sequence Length | Allocated Cache | K,V Projection Reduction | End-to-End Speedup |
|-----------------|--------------|-------------------|-------------------|
| 10 tokens | 72 MiB | 5.5x | Measure on target workload |
| 50 tokens | 72 MiB | 25.5x | Measure on target workload |
| 100 tokens | 72 MiB | 50.5x | Measure on target workload |
| 500 tokens | 72 MiB | 250.5x | Measure on target workload |

: **KV-cache memory and compute savings as sequence length grows.** {#tbl-18-memoization-cache-scaling}

The table assumes one sequence, the float32 architecture above, and pre-allocation for 1024 tokens. Projection counts describe a growing prefix of lengths 1 through N. They are not timing predictions: attention, prefix copies, Python overhead, and sampling also contribute to runtime. Small NumPy workloads can run slower with caching.
...
**Fix**: Always advance the cache position after processing a complete token through all layers. Manual model forwards must also run inside `with cache.generation():` and pass `start_pos=cache.seq_pos`; reset before starting a new prompt:
...
```python
# The helper resets and opens the cache context for each independent prompt.
output1 = _cached_generate(model, prompt1, 20, 0.0, cache)
output2 = _cached_generate(model, prompt2, 20, 0.0, cache)

# For manual forwards, call cache.reset() before entering cache.generation().
```
...
@tbl-18-memoization-vs-pytorch places your implementation side by side with the production reference for direct comparison.
...
: **Feature comparison between TinyTorch KV cache and PyTorch transformers.** {#tbl-18-memoization-vs-pytorch}
...
# Trace a prompt token by token; prompt_tokens is a nonempty list of IDs.
cache.reset()
with cache.generation():
    for token_id in prompt_tokens:
        input_token = Tensor(np.array([[token_id]]))
        logits = model.forward(input_token, start_pos=cache.seq_pos)
        cache.advance()  # Advance once after all layers process this token.

    # Greedy next token; continue the same pattern to decode more tokens.
    next_token = int(np.argmax(logits.data[0, -1, :]))

# Ordinary forwards outside the context do not read cached history.
```
...
KV caching avoids projecting previous tokens again, but each request retains its own K,V history. Longer contexts and larger batches therefore increase memory demand. Production serving systems balance that storage against latency and throughput requirements.

Measure both time and memory on the target workload. TinyTorch exposes the mechanism on NumPy arrays; its timings do not establish the cost, latency, or user capacity of a production service.

## Check Your Understanding

:::{.callout-tip title="Check Your Understanding — Memoization"}
Before moving on, verify you can articulate each of the following:

- [ ] Why the KV cache dominates inference memory at long context, and how PagedAttention allocates it in blocks to reduce wasted capacity.
...
- [ ] The memory-compute trade-off: when trading O(n) memory for reduced projection work improves measured inference performance.
...
Memory: 50,331,648 × 4 bytes = 201,326,592 bytes ≈ **192 MiB**
...
This is the reduction in K,V projection work; measured generation speedup also depends on attention and cache overhead.
:::
...
**Analysis**: A measured 12x latency improvement with 15% extra memory can be worthwhile if that memory fits the deployment budget. It does not imply 12x more concurrent users: each additional sequence needs its own cache, and throughput also depends on batching and hardware utilization.
:::
...
The fraction of reused K,V pairs approaches 100% for long sequences. Attention still reads those pairs, so this fraction alone does not predict runtime.
:::
...
- **O(n) cache storage reduces K,V projection work from O(n²) to O(n):** at n = 100 tokens that's a 50× reduction in K,V work; at n = 1000 it's 500×.
...
- **The trade-off depends on invariance:** reuse cached projections only while their inputs and model parameters remain unchanged.

**Coming next:** Module 19 measures whether saved projection work produces an end-to-end speedup on the chosen workload.
...
- **Blog**: ["The Illustrated Transformer"](https://jalammar.github.io/illustrated-transformer/) by Jay Alammar - Visual explanation of attention mechanisms that benefit from caching
...
@tbl-18-memoization-downstream-usage traces how this module is reused by later parts of the curriculum.
