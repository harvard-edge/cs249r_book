...
overview_without_cache = f"{overview_without:,}"
overview_with_cache = f"{overview_with:,}"
overview_speedup = f"{overview_without // overview_with}x"
...
ckpt_stored = f"{ckpt_stored}"
ckpt_recomputed = f"{ckpt_recomputed}"
ckpt_reduction = f"{ckpt_reduction}x"
...
gpt2_cache_bytes = f"{gpt2_cache_bytes:,}"
gpt2_cache_mb = f"{gpt2_cache_mb:.0f}"
gpt2_overhead_pct = f"{gpt2_overhead_pct:.0f}%"
...
prod_total_cache_mb = f"{prod_total_cache_mb:,}"
prod_total_gb = f"{prod_total_gb:.2f}"
...
q1_per_tensor = f"{q1_elements_per_tensor:,}"
q1_per_layer = f"{q1_elements_per_layer:,}"
q1_total_elements = f"{q1_total_elements:,}"
q1_total_bytes = f"{q1_total_bytes:,}"
q1_total_mb = f"{q1_total_mb:.0f}"
...
q2_without = f"{q2_without:,}"
q2_with = f"{q2_with:,}"
q2_reduction = f"{q2_reduction:.1f}x"

# === Q3: Memory-Compute Trade-off ===
q3_cache_mb = 300
q3_model_mb = 2000
q3_overhead_pct = q3_cache_mb / q3_model_mb * 100
q3_overhead_pct = f"{q3_overhead_pct:.0f}%"
...
q5_total_mb = f"{q5_total_mb:,}"
q5_avail_gb = f"{q5_avail_mb / 1000:.0f}"
q5_max_seqs = f"{q5_max_seqs:,}"
```
...
- Without caching: 1 + 2 + 3 + ... + 100 = `{python} overview_without_cache` K,V computations
- With caching: `{python} overview_with_cache` K,V computations (one per token)
- Speedup: `{python} overview_speedup` reduction in K,V computation alone
...
- With checkpointing every 12 layers: Store `{python} ckpt_stored` sets, recompute `{python} ckpt_recomputed` sets during backward
- Memory reduction: `{python} ckpt_reduction` decrease
...
S = 1024 tokens
D = 64 dimensions
Memory = 2 × 12 × 12 × 1024 × 64 × 4 = `{python} gpt2_cache_bytes` bytes ≈ `{python} gpt2_cache_mb` MB

For a model with 125 million parameters (500 MB), the cache adds `{python} gpt2_overhead_pct` memory overhead. This seems significant until you consider the computational savings.
...
For N = 100 tokens, caching provides `{python} compute_red_100` reduction in K,V computation. For N = 1000 tokens, the reduction is `{python} compute_red_1000`. The speedup grows with sequence length, making the memory trade-off increasingly favorable for longer generation.

| Sequence Length | Cache Memory | Compute Reduction | Effective Speedup |
|-----------------|--------------|-------------------|-------------------|
| 10 tokens | `{python} gpt2_cache_mb` MB | `{python} table_red_10` | 3-5x |
| 50 tokens | `{python} gpt2_cache_mb` MB | `{python} table_red_50` | 8-12x |
| 100 tokens | `{python} gpt2_cache_mb` MB | `{python} table_red_100` | 10-15x |
| 500 tokens | `{python} gpt2_cache_mb` MB | `{python} table_red_500` | 12-20x |
...
- 10 concurrent users: `{python} prod_total_cache_mb` MB cache + 500 MB model = `{python} prod_total_gb` GB total
...
Elements per tensor: 4 × 8 × 1024 × 64 = `{python} q1_per_tensor`

Each layer has 2 tensors (K and V): 2 × `{python} q1_per_tensor` = `{python} q1_per_layer` elements per layer

Total across 12 layers: 12 × `{python} q1_per_layer` = `{python} q1_total_elements` elements

Memory: `{python} q1_total_elements` × 4 bytes = `{python} q1_total_bytes` bytes ≈ **`{python} q1_total_mb` MB**
...
**Without caching**: 1 + 2 + 3 + ... + 200 = 200 × 201 / 2 = **`{python} q2_without` computations**

**With caching**: `{python} q2_with` computations (one per token)

**Reduction**: `{python} q2_without` / `{python} q2_with` = **`{python} q2_reduction` fewer K,V computations**
...
**Memory overhead**: 300 MB / 2000 MB = `{python} q3_overhead_pct` increase

**Speedup**: 12x faster generation

**Analysis**:
- Cost: `{python} q3_overhead_pct` more memory
...
**Cache hit rate**: 49/50 = **`{python} q4_hit_50`**

As generation continues:
- Token 100: 99/100 = `{python} q4_hit_100` hit rate
- Token 500: 499/500 = `{python} q4_hit_500` hit rate
...
**batch_size=8**: 75 MB × 8 = **`{python} q5_total_mb` MB**
...
- Model: 2 GB
- Available for cache: `{python} q5_avail_gb` GB
- Max batch size: `{python} q5_avail_gb` GB / 75 MB ≈ `{python} q5_max_seqs` sequences
