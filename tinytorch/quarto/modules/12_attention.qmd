...
Think of a Python dict. A hard lookup is `value = table[key]` — exact match, return one entry. Attention is the soft version: compare your query to *every* key, get a similarity score for each, turn those scores into weights with softmax, and return the weighted sum of *all* values. Nothing is ever "not found" — every position contributes a little, and the most-similar key contributes the most.

The twist that makes it work for language is that Q, K, and V are not given to the model. They are three separate linear projections of the same input embedding, so the model *learns* what each token should ask for, what it should advertise, and what it should hand over. Training discovers, end-to-end, that one set of projections might emphasize syntactic role while another emphasizes semantic content.

Here is how your implementation creates the three roles:
