...
<a href="../assets/slides/12_attention.pdf" download><i class="fa-solid fa-download"></i> Download</a>
<a href="#" onclick="slideFullscreen('12_attention'); return false;" class="secondary"><i class="fa-solid fa-expand"></i> Fullscreen</a>
</div>
</div>
...
::: {#fig-12_attention-diag-1 fig-env="figure" fig-pos="htb" fig-cap="**Attention Mechanism**: Information retrieval paradigm where similarity scores between Queries and Keys determine how much of each Value is retrieved." fig-alt="Three input tensors (Query, Key, Value) flow into a dot-product block that computes QK^T scores, feeding a softmax normalization stage that weights the Value tensor to produce the attended output."}
...
: **Listing 12.2 — Scaled dot-product score matrix**. Computes `QK^T / √d_k`, producing the O(N²) scores tensor that dominates attention memory. {#lst-12-scaled-scores}
