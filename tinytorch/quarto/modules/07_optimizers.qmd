...
#| lst-cap: "**Full Adam `step()`: first and second moment updates, bias correction, and the per-parameter adaptive step `lr * m_hat / (sqrt(v_hat) + eps)`.**"
#| eval: false
def step(self):
...
#| lst-cap: "**AdamW update: the adaptive Adam step runs on pure gradients, then weight decay shrinks parameters separately by `(1 - lr * weight_decay)`.**"
#| eval: false
