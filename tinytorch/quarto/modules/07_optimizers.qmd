...
:::{.callout-warning title="💾 Systems Implication: Optimizer State Memory"}
...
scale_param_gb = f"{scale_param_gb:.1f} GB"
scale_state_tb = f"{scale_state_tb:.2f} TB"
scale_multiplier = f"{scale_multiplier}x"
```

To appreciate optimizer importance, consider production training scenarios:

- **Large language models (175B parameters)**: Optimizer state alone consumes **`{python} scale_state_tb`** with Adam (`{python} scale_multiplier` x `{python} scale_param_gb` parameters), requiring multi-GPU state sharding
...
q1_param_gb = f"{q1_param_gb:.2f} GB"
q1_adam_state_gb = f"{q1_adam_state_gb:.2f} GB"
q1_total_adam_gb = f"{q1_total_adam_gb:.2f} GB"
q1_sgd_state_gb = f"{q1_sgd_state_gb:.2f} GB"
q1_total_sgd_gb = f"{q1_total_sgd_gb:.2f} GB"
q1_diff_gb = f"{q1_diff_gb:.2f} GB"
```
...
q2_adam_time = f"{q2_adam_time:,.0f}"
q2_sgd_time = f"{q2_sgd_time:,.0f}"
q2_speedup = f"{q2_speedup:.2f}x"
```
...
q3_corr_step1 = f"{q3_corr_step1:.1f}"
q3_corr_step10 = f"{q3_corr_step10:.3f}"
q3_corr_step100 = f"{q3_corr_step100:.4f}"
q3_mult_step1 = f"{q3_mult_step1:.0f}x"
q3_mult_step10 = f"{q3_mult_step10:.2f}x"
q3_mult_step100 = f"{q3_mult_step100:.1f}x"
```
...
**Parameters:** 10B x 4 bytes = **`{python} q1_param_gb`**

**Adam state:** 2 buffers (m, v) = 2 x `{python} q1_param_gb` = **`{python} q1_adam_state_gb`**
**Total with Adam:** `{python} q1_param_gb` (params) + `{python} q1_adam_state_gb` (state) = **`{python} q1_total_adam_gb`**

**SGD with momentum:** 1 buffer (velocity) = **`{python} q1_sgd_state_gb`**
**Total with SGD:** `{python} q1_param_gb` (params) + `{python} q1_sgd_state_gb` (state) = **`{python} q1_total_sgd_gb`**

**Difference:** Adam uses **`{python} q1_diff_gb` more** than SGD (50% increase). This might force you to use fewer GPUs or implement optimizer state sharding.
:::
...
**Adam:** 100,000 steps x 1.2 = **`{python} q2_adam_time` time units**
**SGD:** 200,000 steps x 1.0 = **`{python} q2_sgd_time` time units**

**Adam finishes `{python} q2_speedup` faster** despite higher per-step cost. The convergence advantage (2x fewer steps) outweighs the computational overhead (1.2x slower steps).
...
In Adam, bias correction divides first moment by (1 - β₁^t). At step 1 with β₁=0.9, this correction factor is `{python} q3_corr_step1`. At step 10, it's `{python} q3_corr_step10`. How does this affect early vs late training?

:::{.callout-note collapse="true" title="Answer"}

**Step 1:** Divide by `{python} q3_corr_step1` = multiply by **`{python} q3_mult_step1`** (huge correction)
**Step 10:** Divide by `{python} q3_corr_step10` = multiply by **`{python} q3_mult_step10`** (moderate correction)
**Step 100:** Divide by `{python} q3_corr_step100` ≈ multiply by **`{python} q3_mult_step100`** (negligible correction)
