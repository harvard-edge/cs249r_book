...
All three losses collapse a batch of per-sample errors to a single scalar by taking the mean. The choice of reduction quietly shapes both loss magnitude and gradient scale, so it's worth understanding even though we only ship `mean` here.
