...
<a href="../assets/slides/04_losses.pdf" download><i class="fa-solid fa-download"></i> Download</a>
<a href="#" onclick="slideFullscreen('04_losses'); return false;" class="secondary"><i class="fa-solid fa-expand"></i> Fullscreen</a>
</div>
</div>
...
::: {#fig-04_losses-diag-1 fig-env="figure" fig-pos="htb" fig-cap="**Stable cross-entropy on large logits.** For logits [1000, 1002, 1001] and target class 1, direct float32 exponentiation overflows. Shifting by the maximum gives [-2, 0, -1], and log-sum-exp yields a finite scalar loss of approximately 0.4076." fig-alt="Two calculation paths share logits of shape (1, 3) and a target of shape (1,). The naive path produces infinite exponentials and NaN. The stable path shifts by 1002, sums exponentials to 1.5032, and obtains scalar loss 0.4076. Displayed decimals are rounded."}
...
As illustrated in @fig-04_losses-diag-1, naive loss evaluations can overflow to infinity and produce `nan` when exponentiating large logits, while shifting by the maximum guarantees bounded, stable loss values. @tbl-04-losses-implementation-roadmap lays out the implementation in order, one part at a time.
...
: **Implementation roadmap for the three loss functions.** {#tbl-04-losses-implementation-roadmap}
...
This section provides a quick reference for the loss functions you'll build. Each loss is a callable class with a `forward()` method that takes predictions and targets and returns a scalar `Tensor`.

### Summary Table

@tbl-04-losses-function-summary summarizes the math, output range, and typical use case of each loss function.

| Loss Function | Mathematical Form | Target Type | Use Case |
|---------------|-------------------|-------------|----------|
| `MSELoss` | $\frac{1}{N} \sum (\hat{y} - y)^2$ | Continuous values | Regression |
| `CrossEntropyLoss` | $-\frac{1}{N} \sum \log \hat{y}_{\text{target}}$ | Integer class indices | Multi-class classification |
| `BinaryCrossEntropyLoss` | $-\frac{1}{N} \sum [y \log \hat{y} + (1-y) \log(1-\hat{y})]$ | Binary labels (0 or 1) | Binary classification |

: **Mathematical form, target type, and use case for each loss function.** {#tbl-04-losses-function-summary}
...
@tbl-04-losses-input-output-shapes pins down the shape contract each loss expects.
...
| BinaryCrossEntropy | `(N,)` or `(N, D)` probabilities³ | Same as predictions (labels in [0, 1]) | `()` scalar |

: **Input and output shape contracts for each loss function.** {#tbl-04-losses-input-output-shapes}
...
```{python}
#| label: lst-04-losses-mse
#| lst-cap: "**Mean squared error forward pass: defensive validation, subtract, square, average.**"
#| eval: false
...
class MSELoss:
    ...
    def forward(self, predictions: Tensor, targets: Tensor) -> Tensor:
        """Compute the loss through its operation, recording graph history for autograd."""
...
```{python}
#| label: lst-04-losses-cross-entropy
#| lst-cap: "**CrossEntropyLoss forward pass fusing log-softmax with target-class selection.**"
#| eval: false
class CrossEntropyFunction(Function):
    ...
    def forward(self, logits, targets):
        ...
        # Return negative mean (cross-entropy is negative log-likelihood)
        return -np.mean(selected_log_probs)
...
Binary cross-entropy is the natural choice when decisions are independent: predicting spam vs. not spam, detecting disease presence, or performing multi-label classification where each sample can belong to multiple categories simultaneously. Unlike multi-class cross-entropy, BCE expects probabilities in $[0, 1]$ (after applying a sigmoid activation) rather than raw unbounded logits.

The code in @lst-04-losses-bce makes this concrete.

```{python}
#| label: lst-04-losses-bce
#| lst-cap: "**BinaryCrossEntropyLoss forward pass with defensive validation and probability clamping.**"
#| eval: false
class BinaryCrossEntropyFunction(Function):
    ...
    def forward(self, predictions, targets):
        ...
        # Step 2: Compute binary cross-entropy per sample
...
```{python}
#| label: lst-04-losses-log-softmax
#| lst-cap: "**Numerically stable log-softmax via the log-sum-exp shift.**"
#| eval: false
...
:::{.callout-note title="Systems Implication: Parallel Reductions & Atomic Locks"}
...
**Error**: `ValueError: CrossEntropyLoss target index out of range: [5]`

This happens when your target class indices exceed the number of classes in your logits. If you have 3 classes (indices 0, 1, 2) but your targets contain index 5, TinyTorch's defensive validation catches the out-of-bounds index before NumPy can silently select the wrong class or raise an unhandled exception.
...
@tbl-04-losses-vs-pytorch places your implementation side by side with the production reference for direct comparison.
...
: **Feature comparison between TinyTorch losses and PyTorch equivalents.** {#tbl-04-losses-vs-pytorch}
...
- **Textbook**: ["Deep Learning"](https://www.deeplearningbook.org/) by Goodfellow, Bengio, and Courville - Chapter 5 covers loss functions and maximum likelihood
