...
scale_savings = f"${scale_savings / 1_000_000:.0f} million"
```

Production ML systems operate at scales where small performance differences compound into massive resource consumption:

- **Cost**: A data center running 10,000 GPUs 24/7 consumes $50 million in electricity annually. Reducing latency 10% saves `{python} scale_savings` per year.
...
q1_bl_margin = f"{q1_bl_margin:.2f}"
q1_bl_ci_lo = f"{q1_bl_ci_lo:.2f}"
q1_bl_ci_hi = f"{q1_bl_ci_hi:.2f}"
q1_opt_margin = f"{q1_opt_margin:.2f}"
q1_opt_ci_lo = f"{q1_opt_ci_lo:.2f}"
q1_opt_ci_hi = f"{q1_opt_ci_hi:.2f}"
q1_mean_diff = f"{q1_mean_diff:.1f}"
...
q2_sqrt_n = f"{q2_sqrt_n:.2f}"
q2_n_raw = f"{q2_n:.1f}"
q2_n_ceil = f"{q2_n_ceil}"
...
q3_nw_mean = f"{q3_nw_mean:.1f}ms"
q3_w_mean = f"{q3_w_mean:.2f}ms"
q3_latency_diff = f"{q3_latency_diff:.2f}ms"
q3_latency_pct = f"{q3_latency_pct:.0f}%"
q3_var_reduction = f"{q3_var_reduction:.0f}%"
...
q5_total_op_us = f"{q5_total_op_us:,}"
q5_total_op_ms = f"{q5_total_op_ms:.0f}ms"
q5_total_overhead_us = f"{q5_total_overhead_us:,}"
q5_total_overhead_ms = f"{q5_total_overhead_ms:.0f}ms"
q5_total_measured_ms = f"{q5_total_measured_ms:.0f}ms"
q5_overhead_pct = f"{q5_overhead_pct:.2f}%"
```
...
Baseline: CI = mean ± 1.96 * (std / sqrt(n)) = 12.5 ± 1.96 * (1.2 / sqrt(10)) = 12.5 ± `{python} q1_bl_margin` = [`{python} q1_bl_ci_lo`, `{python} q1_bl_ci_hi`]

Optimized: CI = 11.8 ± 1.96 * (1.5 / sqrt(10)) = 11.8 ± `{python} q1_opt_margin` = [`{python} q1_opt_ci_lo`, `{python} q1_opt_ci_hi`]

**Result**: The confidence intervals OVERLAP (baseline goes as low as `{python} q1_bl_ci_lo`, optimized goes as high as `{python} q1_opt_ci_hi`). This means the difference is **NOT statistically significant** at the 95% confidence level. You cannot confidently claim the optimized model is faster.

**Lesson**: Always compute confidence intervals. A `{python} q1_mean_diff`ms difference in means might seem meaningful, but with these variances and sample sizes, it could be random noise.
:::
...
sqrt(n) = 1.96 * 2.0 / 0.5 = `{python} q2_sqrt_n`

n = `{python} q2_sqrt_n`² = **`{python} q2_n_raw` ≈ `{python} q2_n_ceil` measurements**
...
**Without warmup:**
- Mean = (15.2 + 12.1 + 10.8 + 10.5 + 10.6 + 10.4) / 6 = **`{python} q3_nw_mean`**
- Std = 1.8ms (high variance due to warmup effects)

**With warmup:**
- Mean = (10.5 + 10.6 + 10.4 + 10.7 + 10.5 + 10.6) / 6 = **`{python} q3_w_mean`**
- Std = 0.1ms (low variance, stable measurements)

**Impact:**
- Latency reduced: `{python} q3_nw_mean` - `{python} q3_w_mean` = **`{python} q3_latency_diff` (`{python} q3_latency_pct` reduction)**
- Variance reduced: 1.8 → 0.1ms = **`{python} q3_var_reduction` reduction in noise**
...
**Total true operation time**: 50μs × 1000 = `{python} q5_total_op_us`μs = `{python} q5_total_op_ms`

**Total timer overhead**: 1μs × 1000 = `{python} q5_total_overhead_us`μs = `{python} q5_total_overhead_ms`

**Total measured time**: `{python} q5_total_op_ms` + `{python} q5_total_overhead_ms` = `{python} q5_total_measured_ms`

**Overhead percentage**: (`{python} q5_total_overhead_ms` / `{python} q5_total_measured_ms`) × 100% = **`{python} q5_overhead_pct`**
