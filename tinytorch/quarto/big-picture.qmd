...
::: {#fig-module-flow}
![](assets/images/svg/big-picture-module-flow.svg){fig-alt="The TinyTorch stack, four horizontal bands stacked bottom-up. Foundation (01-08) is the bottom band with eight pill-shaped modules (Tensor, Activations, Layers, Losses, DataLoader, Autograd, Optimizers, Training). Architecture (09-13) sits above it with CNNs on the left (vision) and a Tokenization, Embeddings, Attention, Transformers chain on the right (language). Optimization (14-19) is above that with Profiling, Quantization, Compression, Acceleration, Memoization, and Benchmarking. Capstone (20) crowns the stack as an orange-accent band with an MIT-red pill label. Upward arrows between bands show that each layer depends on the one below."}

**The TinyTorch Stack.** Four layers stacked bottom-up. Foundation (01-08) builds the core numerical machinery, Architecture (09-13) adds models for vision and language, Optimization (14-19) makes the system fast, small, and measurable, and Capstone (20) runs everything end-to-end as an MLPerf-style benchmark submission. Each layer depends on the one below.
:::
