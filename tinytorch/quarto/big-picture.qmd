...
::: {#fig-module-flow}
![](assets/images/svg/big-picture-module-flow.svg){fig-alt="Three-tier architecture using a neutral palette with an orange accent on Capstone. Foundation tier (01-08) stacks Tensor -> Activations -> Layers -> Losses -> DataLoader -> Autograd -> Optimizers -> Training. Architecture tier (09-13) shows CNNs alongside a Tokenization -> Embeddings -> Attention -> Transformers chain. Optimization tier (14-19) shows Profiling fanning out to Quantization, Compression, Acceleration, and Memoization, all converging on Benchmarking. Red cross-tier arrows route through inter-panel gutters: DataLoader -> CNNs and Tokenization, CNNs and Transformers -> Profiling, and Benchmarking -> Capstone (highlighted in orange)."}

**TinyTorch Module Flow.** The 20 modules progress through three tiers: Foundation builds core ML primitives, Architecture applies them to vision and language tasks, and Optimization makes systems production-ready. Capstone (orange) is the culmination where all three tiers meet.
:::
