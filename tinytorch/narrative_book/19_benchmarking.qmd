...
Chapter 18 removed repeated projections with a KV cache, but each decode step still reads a growing prefix. A speedup therefore needs a stated prompt length and generation length. Chapters 15 through 17 also distinguished measured behavior from modeled savings in storage or execution cost. A ratio of two timings is easy to compute. Establishing that it measures the same workload before and after the change requires more care. This chapter follows that evidence from individual clock readings to a comparison another reader can check.

A useful comparison follows a small measurement discipline. Run the model a few times before you start the clock, because the first run may pay for setup that later runs do not. Run it many times after, because one timing is one draw from a noisy distribution. Report the distribution rather than a single number, because the users who notice latency are the ones who hit the slow runs. And attach an accuracy check, because any accuracy loss must stay within the comparison's stated tolerance. The protocol in @fig-benchmarking-pipeline organizes those habits. TinyTorch's benchmark classes implement the measurement and scoring paths, and the chapter closes with a pass/fail check modeled on the industry benchmark that turned the same habits into rules.

![Fix the workload and model mode, warm up, and retain repeated timings. Report the distribution alongside quality checks, keeping storage estimates distinct from measured allocations.](assets/images/diagrams/19_benchmarking-diag-1.svg){#fig-benchmarking-pipeline}
...
The harness supplies the repeated observations needed to compare models. For a Chapter 18 decoder, its usefulness depends on the caller holding the prefix and generation lengths fixed. It automates repetition; the caller still decides which workload to repeat and how many observations the claim needs.
...
Constructing the container checks that the hand arithmetic agrees with the implementation.
...
The MLPerf-style path adds predictions and labels to the timing loop. It validates their shapes and values, counts agreements, and checks both accuracy and $P_{99}$ against local thresholds. Synthetic labels cannot pass that local check, and supplied labels still need independent provenance. Chapter 20 uses these measurement habits to compare combinations of optimizations under its own event policy. Preserve the baseline, control input shape and model state, check quality on the same data, and compare the same named statistic before combining another mechanism.
...
2. `run_standard_benchmark` checks the latency bound at $P_{99}$. Compare that policy with a bound on the mean. Construct a list of 100 latencies for which the two rules disagree, one where the mean passes and the tail fails and one the other way around, and argue which verdict a user of the model would agree with.
3. Using the formula from the sample-size box, compute how many runs are needed to have a 95 percent chance of observing at least one event above $P_{99.9}$. Then measure how long that many runs of `ToyModel` would take and decide whether $P_{99.9}$ is a statistic this harness can support with this sample.
4. Benchmark two models named `candidate`, each returning a different `evaluate(dataset)` accuracy. Inspect `benchmark.model_names` and predict which key holds each score. Then repeat with unnamed models and verify that optimization-comparison results still contain both identities.
5. Change the scoring trace to return ten scores for the binary task, then to return a NaN, then to supply a fractional label. Predict where each run fails and check that none returns a compliance verdict. Explain why labels copied from the model's own predictions would pass the numeric checks but fail to measure task accuracy.
