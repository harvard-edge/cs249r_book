...
@sec-memoization removed repeated projections with a KV cache, but each decode step still reads a growing prefix. A speedup therefore needs a stated prompt length and generation length. @sec-quantization through @sec-acceleration also distinguished measured behavior from modeled savings in storage or execution cost. A ratio of two timings is easy to compute. Establishing that it measures the same workload before and after the change requires more care. This chapter follows that evidence from individual clock readings to a comparison another reader can check.
...
::: {.column-margin}
<img src="assets/images/diagrams/19_benchmarking-margin-latency.svg" width="100%"/>
...
::: {#chk-benchmarking-cuda-synchronize .callout-warning title="Sanity check: the asynchronous CUDA execution timing pitfall"}
...
t1 = time.perf_counter()
```
Or record timestamps directly on device streams via `torch.cuda.Event(enable_timing=True)`.
:::
