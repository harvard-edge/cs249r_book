...
A weight of $1.0$ with gradient $2.0$ becomes $0.8$ when we subtract one tenth of the gradient. That is already a complete optimizer step. The gradient produced by `loss.backward()` supplies the direction; the optimizer chooses the size of the change and writes the new value into the parameter. Parameters that did not participate in the recorded computation can still have `grad=None` and must be skipped.

The difficulty appears when the same learning rate serves weights with different gradient scales. A step that is safe in one direction can diverge in another. Momentum stores previous gradients so consistent directions accumulate. Adam uses gradient history to scale each coordinate's update. AdamW separates weight shrinkage from that adaptive scaling. Following these update paths will show what an optimizer reads, what it changes, and what it must remember for the next batch.
...
The listings are extracted by symbol name from Module 07 (`src/07_optimizers/07_optimizers.py`); the listing checker detects drift from that source. Adam and AdamW define their moment-update and step methods in separate exercise cells. The `@method_of(AdamW)` decorator attaches a function to the named class, as Module 06 does for backward methods. The excerpts below follow those cells in execution order. SGD's checkpoint helpers are omitted from its listing.
:::
...
Output: `t=1: m=0.05000 v=0.00025000 theta=0.8990` and `t=2: m=0.09500 v=0.00049975 theta=0.7981`.

After either call, the weight has changed, the two raw moment arrays and their age remain available for the next call, and the assigned gradient is still present until `zero_grad()` clears it. The normalized update and the decayed intermediate weight are temporary values. The model continues to hold the same `Tensor`; its next forward pass reads the new array.
...
For $7\times10^9$ scalar weights, the four-array float32 model gives $7\times10^9\times16=112\text{ GB}$. Weights alone occupy 28 GB, gradients another 28 GB, and the two moments 56 GB. These are calculated storage totals, not measured peak memory. A different precision or a scheme that distributes arrays across devices changes the accounting and must be stated explicitly.
:::
