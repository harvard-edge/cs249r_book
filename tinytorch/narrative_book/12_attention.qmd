...
Changing *machine* to *engine* in "the machine crashed because it overheated" does not change the embedding row for *it*. The token ID and position of *it* stay the same, and the lookup from @sec-embeddings cannot read neighboring rows. A useful representation needs that missing interaction. This chapter builds a computation that lets one row select and combine information from other rows.

The mechanism is **scaled dot-product attention**, and the layer that packages it is `MultiHeadAttention`. We build both from the tensor operations already available, trace a two-token example by hand, and identify the contract that lets the next chapter stack these layers. The production bridge then explains how the same arithmetic can avoid storing its largest intermediate.

---

## The Problem

After embedding lookup a sequence is a matrix $X$ with one row per token and $D$ columns. The `Linear` and activation layers in a token-wise network process those rows independently. The convolution from chapter 9 mixed neighboring spatial positions through a fixed window, but this sequence computation needs weights that can change with the contents of the entire input.
...
The listings in this section are extracted from named symbols in Module 12 (`src/12_attention/12_attention.py`). The listing-generation check detects changes to registered source excerpts; the surrounding commentary explains how those excerpts compose. Module 12 exposes three helpers, `scaled_dot_product_attention(Q, K, V, mask)`, and a `MultiHeadAttention(embed_dim, num_heads)` class whose methods are shown one at a time below.
:::
...
The helper contains two operations from chapter 1. `transpose(-2, -1)` swaps the last two axes and preserves the leading axes. For a single head, `(S, d_k)` becomes `(d_k, S)`, making `Q.matmul(K_t)` a product with shape `(S, S)`. For a batch of heads, `(B, H, S, d_k)` becomes `(B, H, d_k, S)`, and the product has shape `(B, H, S, S)`. This batched path uses `np.matmul`. The returned transpose owns its buffer in TinyTorch; it does not expose a shared view of `K`.
