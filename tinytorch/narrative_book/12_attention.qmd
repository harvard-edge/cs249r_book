...
The mechanism is **scaled dot-product attention**, and the layer that packages it is `MultiHeadAttention`. We build both from the tensor operations already available, trace a two-token example by hand, and identify the contract that lets @sec-transformers stack these layers. The production bridge then explains how the same arithmetic can avoid storing its largest intermediate.
