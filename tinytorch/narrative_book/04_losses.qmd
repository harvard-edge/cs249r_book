...
::: {.column-margin}
**Source Code Mapping**: Every listing in this section is extracted directly from Module 04 (`src/04_losses/04_losses.py`). `MSELoss` computes mean squared error, `CrossEntropyLoss` uses stabilized `log_softmax` with index gathering, and `BinaryCrossEntropyLoss` uses probability clipping.
:::
