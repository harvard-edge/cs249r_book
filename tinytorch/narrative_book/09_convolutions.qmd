...
For the image in @fig-conv-topology, the first output is the sum $1+2+3+5+6+7+9+10+11=54$. Moving the same all-ones filter one column right produces 63. The input window changes; the nine filter weights do not.
...
The listings in this section are extracted from Module 09 (`src/09_convolutions/09_convolutions.py`) by name when the book is built, so a renamed method in the module fails the build instead of drifting from the chapter. Module 09 computes convolution, max pooling, and average pooling with explicit loops and gives each operation its own backward method; it does not implement im2col, so that section's listing is a model, marked as such, and the worked trace checks it against the module.
:::
...
In @lst-conv-pool-trace, clearing parameter gradients prevents the two objectives from accumulating into the same buffers. Recomputing `conv(X)` is separately necessary because the earlier `Y.backward(...)` released its operation records. This distinction is the graph-lifetime rule from the autograd chapter in a complete spatial chain. The array-only im2col model can check forward values, but it does not create a TinyTorch graph and cannot replace this training path.
