...
Milestone III defined the evidence an optimization needs: the candidate's measured quality, storage, and execution time against a fixed baseline. Its source walk also exposed incomplete comparisons that must be repaired before claiming a speedup. Every optimization on that scorecard was one you wrote by hand in pure Python, a fused GELU in Chapter 17, a tiled matrix multiply, a preallocated KV cache, and the milestone closed by asking what production compilers and custom silicon do to move those numbers further than pure Python can. This chapter answers that question in the form a practitioner meets it. When I call `torch.compile(model)`, or open a repository and find a Triton kernel where I expected PyTorch code, what is happening, and why did production need it when TinyTorch got by without it?

The short answer is that production systems do three things TinyTorch does not, and each is a response to a cost that TinyTorch can afford to ignore at its size. They capture the program as a graph before running it. They compile that graph into a small number of fused kernels, one GPU launch doing the work of several, instead of dispatching one operation at a time. And they run the heaviest of those kernels, matrix multiplication, on hardware whose wiring matches the shape of the computation. None of the three introduces new mathematics. Graph capture exposes opportunities across operations; fusion and specialized hardware can then remove dispatch overhead or data movement.
...
Chapter 17 wrote the eight steps of a GELU as one expression, the shape a compiler needs to run them as a single pass with every intermediate held in a register, and it noted beside the listing that a bias add folds into the same expression at no extra traffic. On paper that fix cut the traffic ninefold, and it took one function. The trouble is how many times a real model needs the same fix, and who writes each one. Consider the smallest possible version of an unfused chain, three pointwise operations on a four-element vector, in plain Python.
...
with $MN$ multiply-adds happening every cycle once the array is full. The array is fast because its wiring is the matrix multiply. The array directly executes multiply-accumulate operations. A convolution can be lowered to matrix products; a softmax needs other operations and therefore other execution units. The trade is specialization, together with the buffering and scheduling needed to keep the array supplied.

## The Code

There is no TinyTorch module to walk, so I build the two middle layers of @fig-modern-stack here as the smallest Python that shows each step. Everything runs on a laptop except the Triton kernel, which is here because it is what the compiler actually emits and you will meet it in real repositories.

### An eager runtime that counts its memory traffic

Before a compiler can remove a memory pass, we need to see the passes, and TinyTorch cannot show them to us. Chapter 17 timed its unfused and fused GELU and got 160 ms against 159 ms, because NumPy dispatches every operator as its own pass over memory either way and the intermediates never surface in Python. What a framework needs here is the ledger a transformer block runs up. The MLP in every block pushes its activation through a bias add, a GELU, and a scale or dropout, three pointwise operations on a $B \times 4d$ array; at a batch of 256 tokens and $d = 1024$ that is a million floats, 4 MB, read and written three times over, 24 MB of traffic where the mathematics needs 8. NumPy pays that and says nothing. So the first listing is a caricature of how TinyTorch and eager PyTorch run, a library of pointwise operations and an interpreter that executes them one at a time, kept small enough that every pass and every launch can be counted by hand. Each operation takes the current value, the element index, and one argument, so a bias add can look up `b[i]` while a scale takes a scalar. The interpreter keeps the count as it goes.
...
The `program` list is already an IR, three operations on named values, and `run_eager` is an interpreter for it. Notice that the list comprehension inside the loop materializes a fresh `buf` every iteration. That is the intermediate tensor that fusion will eliminate, and the line `moves += 2 * len(x)` is the ledger entry NumPy never showed you. PyTorch calls this eager mode, and every `POINTWISE[op]` call stands in for one trip through its dispatcher and, on a GPU, one kernel launch.

### The fusion pass

The Problem put the price of a hand-written fused kernel at days of specialist time, and a model needs dozens of them. A transformer block alone has the bias-and-GELU chain in its MLP, the scale-then-mask chain in front of every softmax, and the residual add feeding every layer norm, each a different sequence of operations with its own fused loop, and once the backward pass of each chain is counted too, since training needs those kernels as well, the total runs past what any team writes by hand. Worse, before anyone can write those kernels, someone has to find every chain by reading the model. That is the part a compiler takes over first. Once the program exists as data, the way `program` did above, finding the chains is one walk over the list. Consecutive pointwise operations join a group; anything that is not pointwise, a matrix multiply for example, closes the group and stands alone. The compiler proper is that walk, written out.
...
Run on a five-operation program with a matrix multiply in the middle, it prints three groups, `fused ['add', 'gelu']`, then `matmul W`, then `fused ['scale', 'gelu']`. The grouping pass proposes three execution groups instead of five. Each two-operation fused group has one internal intermediate that a fused executor could avoid. Nothing has executed yet, so grouping alone has removed no allocations. Notice that nothing about the model had to be read by a person; the chains fell out of the data. Real compilers add a great deal on top of this, such as checking that shapes match, deciding whether a reduction like softmax can join a group, and folding a pointwise epilogue into the matrix multiply itself, but the decision they make is this decision, and in PyTorch 2 the component that makes it is TorchInductor's scheduler.

### The fused executor

A fused group is a promise, and so far nothing keeps it. Hand the group back to `run_eager` and it would allocate a fresh list per operation and pay the same $2kn$ moves as before, so grouping alone buys no traffic at all. The framework needs a second interpreter that runs a whole group as one pass, and the way to get one is to turn the loop inside out. Instead of one loop per operation over all elements, run one loop over elements with the whole chain inside it. Each value then goes through every operation while it sits in a local variable, and it touches memory twice, once in and once out, however long the chain is.

```python
def run_fused(x, chain):
    """One kernel for the whole chain. Models one pass over activations; Python locals stand in for registers."""
    out = []
...
The two Python runs produce identical values, and their modeled activation traffic differs. The counters do not measure physical memory transfers, and a Python local does not guarantee register residency. The local variable `v` is the register residency Chapter 17 talked about, and the outer `for i` loop is what a GPU parallelizes across thousands of threads. When PyTorch 2 compiles such a group, the kernel it generates has a name like `triton_poi_fused_add_gelu_0`, `poi` for pointwise, and its body is this inner loop.

### The same kernel in Triton

`run_fused` has the right shape, but a Python loop on a CPU is not where a framework runs it. The kernel has to run on the GPU, and until recently the standard way to get it there was to write it in CUDA C++. That is where the days-per-kernel cost in The Problem comes from. A CUDA programmer thinks one thread at a time and manages by hand everything the threads share, and for the decade after AlexNet that hand management was the job. It was a specialist's job, and no compiler could take it over, because CUDA shows a compiler one thread's view and the hard decisions are about what the threads do together. Triton, the Python-embedded language PyTorch 2 uses for its GPU kernels, moves the unit of thought up one level. The programmer says what one block of elements does, and the compiler owns the threads. The following hand-written Triton example expresses the same bias-and-GELU group; it is not a captured compiler output. This listing needs a GPU, CUDA, and the `triton` package, so it is not run in this chapter, and it is not TinyTorch code. Read it against `run_fused` above.
...
Three things to notice. The outer `for i` loop from `run_fused` is gone; in its place, `tl.program_id` says which block of `BLOCK_SIZE` elements this copy of the kernel owns, and the hardware runs thousands of copies at once. The pointer arithmetic `x_ptr + offsets` is the flat-buffer addressing from Chapter 1, the same stride arithmetic you wrote for `Tensor`, applied to GPU memory. And the middle of the kernel is straight-line arithmetic on values that never touch memory between `tl.load` and `tl.store`. What is missing is the point. There is no code here for which threads share a memory transaction, how data lands in the fast on-chip memory, or how the 32-thread warps from Chapter 4 are scheduled; the Triton compiler decides all of that from this block-level description, and that is the whole difference from CUDA. Launching it is one more line on the host, `fused_bias_gelu_kernel[(triton.cdiv(n, 1024),)](x, b, y, n, BLOCK_SIZE=1024)`, which asks for as many blocks as it takes to cover `n` elements.

### A systolic array in twelve lines

Fusion fixes the pointwise operations, and they were never where the FLOPs are. Nearly every multiply in a transformer belongs to a matrix product, the linear layers of Chapter 3 and the attention scores of Chapter 12, and the roofline puts those on the compute-bound side where fusion has nothing to remove. What hurts a matrix multiply is the operand traffic inside it. The Problem counted that traffic at $2n^3$ operand reads for $n^3$ multiply-adds on a general-purpose core, two fetches per multiply, and the tiled loop from Chapter 17 trims the reads by the tile size while leaving the pattern in place, because a core must still fetch each operand from a register file or a cache before it can use it. No software change removes that fetch; fetching is what a load-store core does. The question is whether hardware can pass each operand from one multiplier straight to the next, so that a value read from memory once gets used $n$ times before it is dropped, and the answer from The Idea is the output-stationary systolic array. The bottom layer of the figure is hardware, but its behavior is easy to simulate. The listing below has one accumulator per output and steps through clock cycles, applying the delay rule $k = t - i - j$.
...
The two inner loops are not sequential work in the real chip; they are the $M \times N$ cells all acting in the same clock tick. Only the outer loop over `t` is time. Reading it that way, the listing says exactly what the hardware does. At each tick, every cell that has an operand pair multiplies and accumulates, and the whole product is done after $M + N + K - 2$ ticks. Notice that `A[i][k]` and `B[k][j]` are indexed but never copied; in the chip those reads are the values arriving from the neighboring cell, which is why each input is injected once at the boundary of this ideal array. The Python simulation itself indexes the lists repeatedly; its execution is not a memory-traffic measurement. Scale the grid from $2 \times 2$ to $128 \times 128$ and this is the matrix unit inside a TPU.

## A Worked Trace

Both mechanisms trace by hand in a few lines. Start with the fusion. Feeding $x = (1.0, -2.0, 0.5, 3.0)$ and a bias of $0.1$ through add, GELU, and a scale of $2$ gives the values in @tbl-fusion-trace. I checked the first row with a calculator: $z = 1.1$, the GELU inner term is $0.7979 \times (1.1 + 0.044715 \times 1.331) = 0.9252$, $\tanh(0.9252) = 0.7284$, and $y = 0.5 \times 1.1 \times 1.7284 = 0.9506$.
