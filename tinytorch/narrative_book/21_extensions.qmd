...
::: {.column-margin}
![](assets/images/diagrams/21_framework-you-are-here.svg){width="100%" fig-alt="TinyTorch execution datapath blueprint highlighting Chapter 21 Extensions as the active subsystem."}

*Framework Blueprint: Chapter 21 reaches beyond the educational interpreter to examine graph compilers, custom GPU kernels, and TPU systolic arrays.*
:::

A four-element vector passes through a bias add, GELU, and a scale. The result needs four output values, yet an eager implementation also writes two intermediate vectors. @sec-acceleration exposed this gap between the arithmetic and the arrays that execute it. @sec-milestone-3 established the evidence needed to call an alternative better: compare the actual candidate with a fixed baseline, check quality, count the stated storage format, and measure elapsed time. This chapter carries that discipline beyond TinyTorch. We will follow a small program through a grouping pass and a fused executor, then trace a matrix product through an ideal hardware array.
...
::: {.column-margin}
![](assets/images/diagrams/21_extensions-margin-systolic.svg){width="100%"}
*2D systolic array: wavefront dataflow through multiply-accumulate cells.*
:::
...
(\ref{nbk-extensions-systolic-array-latency}) with the first tick numbered zero in the code. Thus the final tick has index $T-1$. The cells fill and drain at different times; short products may never have every cell active simultaneously. Each output accumulator remains in its cell until completion. Real arrays have fixed dimensions, so larger products require tiling, buffering, and additional transfers. The specialization also has a limit: multiply-accumulate cells do not by themselves compute operations such as exponentiation or softmax.

::: {#nbk-extensions-systolic-array-latency .callout-napkin-math title="Napkin math: systolic array cycle count and throughput scaling"}
For an $M \times K \times N$ matrix multiplication executed on an ideal $M \times N$ output-stationary 2D systolic array:

- **Latency Formula**: The total execution latency is $T = M + N + K - 2$ cycles. For square matrices where $M = N = K$, this simplifies to $T = 3N - 2 \approx 3N$ cycles.
- **Propagation Wavefront**: The first product $a_{00} b_{00}$ arrives at cell $(0,0)$ at cycle $t=0$. The final product $a_{(M-1)(K-1)} b_{(K-1)(N-1)}$ reaches the bottom-right cell $(M-1, N-1)$ at cycle $T-1$, incurring an input delay of $(M-1) + (N-1)$ cycles plus $K$ computation steps.
- **Arithmetic Throughput**: The array performs $2MNK$ arithmetic operations (or $MNK$ multiply-accumulate ops) in $M + N + K - 2$ cycles. As $K \to \infty$ with continuous streaming, the peak throughput reaches $MN$ MACs per cycle—100% compute unit utilization across all cells.
:::

## The Code

::: {.column-margin}
![](assets/images/diagrams/21_ext-margin-source.svg){width="100%" fig-alt="Source code mapping card for Chapter 21 Extensions showing file path, exported package, and tested primitives."}

*Source Code Mapping: Chapter 21 presents architectural models of computational graph capture, Triton GPU execution, and 2D systolic array hardware.*
:::
...
The hardware comparison is similarly specific. Google's documented TPU designs include matrix units organized as systolic arrays alongside other execution units. Array dimensions differ across generations; a particular grid size is not the definition of a TPU. The [TPU architecture documentation](https://docs.cloud.google.com/tpu/docs/system-architecture-tpu-vm) describes the matrix units and surrounding storage. Our simulation models an ideal arrival schedule (\ref{psp-extensions-tpu-vs-tensor-cores}), not those devices' complete arithmetic formats, instruction sets, or memory systems.

::: {#psp-extensions-tpu-vs-tensor-cores .callout-production-bridge title="Production bridge: Google TPU matrix units vs. NVIDIA Tensor Cores"}
Industrial AI acceleration balances hardware specialization, memory bandwidth, and programming flexibility across competing architectural philosophies:

- **Google TPU (Matrix Multiply Unit / MXU)**: A dedicated 2D systolic array (e.g., $128 \times 128$ in TPU v4/v5p, capable of 16,384 MACs per cycle in BF16). Weights or outputs remain stationary on-chip while activation data streams across neighbor-to-neighbor interconnects without reading/writing intermediate results back to registers or SRAM caches.
- **NVIDIA Tensor Cores (Warp-Level Matrix Engine)**: Rather than an autonomous systolic grid, Tensor Cores operate as synchronous warp instructions (e.g., `mma.sync`, `wgmma`) tightly coupled to streaming multiprocessor (SM) register files and shared memory. Threads cooperatively load tile fragments, compute small matrix products ($16 \times 8 \times 16$ or $64 \times 128 \times 16$), and accumulate into registers.
- **Architectural Trade-Off**: Systolic arrays achieve superior energy efficiency and silicon density for large, dense, statically shaped GEMMs by eliminating register file read/write power. Tensor Cores offer higher flexibility for irregular shapes, dynamic control flow, fused activations, and sparse or unstructured workloads.
:::
