...
The first batch might contain rows 7, 8, 1, and 5 of a dataset. Each row has two features, so the model needs one $(4, 2)$ input tensor and four matching labels. Fetching those rows separately does not yet produce that tensor. Something must choose the indices, retrieve each feature-label pair, and copy corresponding fields into batches.

After chapter 4, the layers and losses can consume those batches. A `Dataset` answers "how many samples, and give me sample $i$." A shuffle chooses the visiting order. A `DataLoader` walks that order in groups of $B$ samples, the batch size, and stacks each group. These pieces keep the stored data, the order of visits, and the batching mechanism separate, so a new data source does not require a new training loop.

@Fig-dataloader-pipeline follows the synchronous TinyTorch path. One request for a batch chooses indices, fetches samples, and stacks them before returning to the caller. The training-loop box is the destination that chapter 8 will build; this chapter establishes the batches it receives.
...
The listings in this section are extracted from Module 05 (`src/05_dataloader/05_dataloader.py`) by name when the book is built, so a renamed class in the module fails the build instead of drifting from the chapter. The module also carries image augmentations (`RandomCrop`, `RandomHorizontalFlip`, `Compose`); the lookup example below shows where they enter the same call chain.
:::

The listings use the copying `Tensor` constructor from @sec-tensors. Reading a row and constructing a batch both create fresh input tensors for the model. These copies do not connect the batch back to its source tensor for differentiation; chapter 6 will distinguish data preparation from recorded model operations. Read the code in the order one request executes: `__iter__` chooses indices, `TensorDataset.__getitem__` returns samples, and `_collate_batch` groups and stacks their fields.
...
The last piece owns the order. Chapter 8's loop is going to read `for epoch in range(E): for x, y in loader:` and expect two things from that inner line, a fresh shuffle when enabled and one batch at a time.
...
Two things to notice. First, `__iter__` is a generator, so the index list is rebuilt and the permutation drawn fresh every time a `for` loop starts over the loader; that is what "shuffle once per epoch" means in code, and it is how chapter 8 can request a fresh shuffle on each pass through its outer loop. Creating the generator does not execute its body; the first `next` request creates and optionally shuffles the indices. Second, a batch is never materialized before it is needed. The generator retains its index list and current sample list while suspended at `yield`; the caller holds the returned batch. The next request resumes the loop and prepares the next batch synchronously. There is no background work between yields, and the caller can increase memory use by retaining old batches. `random.shuffle` mutates the index list, leaving the dataset's stored tensors in place. It consumes Python's random-generator state, so reusing a seed requires resetting that state before iteration. A completed pass visits every index once; stopping early consumes only the requested batches. An empty dataset yields no batches, and construction rejects a nonpositive or noninteger batch size.
...
Sample 3 is a fresh tensor of shape $(2,)$ holding row index 3 (the fourth row), and its label is a scalar tensor. Without shuffling, the loader reproduces the sorted-batch problem from the opening; the label rows of the three batches are `[0, 0, 0, 0]`, `[0, 1, 1, 1]`, and `[1, 1]`, and `len(loader)` is $\lceil 10/4 \rceil = 3$.
