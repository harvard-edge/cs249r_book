...
The loader in @sec-dataloader supplies a batch, the layers turn it into predictions, and the loss reduces the prediction errors to one number. That number does not yet tell the optimizer how to change a weight. Even the small computation `y = x * x; L = y + x` illustrates the missing information. At `x = 3`, the result is `L = 12`; the result alone does not record that `x` contributed twice through the multiply and once through the addition.

A derivative measures how a small input change affects an output. Here replacing $x$ by $3+\delta$ changes $L$ from $12$ to $12+7\delta+\delta^2$. For small $\delta$, the change is approximately seven times the input change, so the derivative at $x=3$ is $7$. For a loss with many weights, the **gradient** collects one such derivative per weight, holding the others fixed. Training needs those derivatives at the current weights after each new forward pass.

Automatic differentiation computes them by keeping a record of each operation as it runs. In @fig-autograd-dag, the addition sends a contribution of $1$ directly to $x$, and the multiply sends two contributions of $3$. Their sum is $7$. The chapter follows those records from `Function.apply` into `Tensor.backward`, first for individual operations and then for a small matrix network. The arithmetic rules supply local derivatives; the engine makes sure every contribution arrives before an intermediate passes its gradient onward.
...
The listings in this chapter are extracted from Module 06 (`src/06_autograd/06_autograd.py`), and the `Function` class from Module 01 (`src/01_tensor/01_tensor.py`), by the book's build. The extraction removes student scaffolding, including TODO and HINT blocks and grading markers. These excerpts show the reference operations and traversal used by `loss.backward()`.
:::

The tensor implementation in chapter 1 left three fields on every `Tensor` with nothing to do: `requires_grad`, `grad`, and `_grad_fn`. It also routed every operation through one method, `Function.apply`, which computed a result and forgot how. This chapter is where all four earn their keep.

### An operation that remembers its inputs

Consider the entry point for the opening multiply. `x * x` calls `Tensor.__mul__`, which delegates to `Mul.apply(x, x)`. Chapter 1 supplied `Function.apply` as the common path for every operation. It constructs a fresh `Mul` node whose `inputs` tuple contains two references to the same tensor. Calling `forward` with the inputs' NumPy arrays produces the value $9$.
...
What each node needs in order to send a gradient back to its inputs is the other half of the operation, the `backward` that chapter 1 left raising `NotImplementedError`. The rest of this section writes those halves, one class at a time, on the same classes that already implement `forward`.
...
Running the real module on the same inputs reproduces the table (@lst-autograd-network-trace). Importing `tinytorch.core.autograd` is what completes the operation classes with the backward halves above; before that import, `backward()` raises.
...
The accumulation rule can be checked with a two-line experiment, and it is worth running because `x + x` is the smallest version of a residual connection, where a tensor is added back to something computed from it. The same tensor is both inputs of one `Add` node, so the walker adds to its `pending` entry twice in @lst-autograd-shared-input.
...
3. **Gradient checking.** Write a function that compares `W.grad` from `backward()` against the finite-difference estimate from The Problem for a random $\mathbf{x}$ and $\mathbf{W}$. Keep TinyTorch's analytical gradient in float32. Separately evaluate the NumPy finite-difference function with float32 and float64 arrays for `eps` in $\{10^{-1}, 10^{-2}, \dots, 10^{-6}\}$, and plot each error against the analytical matrix formula. Casting data before constructing a TinyTorch `Tensor` does not create a float64 engine. Keep pre-activations away from zero so the perturbation does not cross ReLU's corner, and explain why the float32 error rises again as `eps` shrinks.
4. **Broadcast and lifetime contracts.** Use the `(2, 3)` bias example with upstream gradient `[[1, 2, 3], [4, 5, 6]]`. Predict both input gradients, then call `backward` with that explicit seed. Repeat with bias shape `(1, 3)` and explain why the values agree but the shapes differ. Recompute the forward pass under `no_grad()` and inspect `requires_grad` and `_grad_fn`. Explain why a later backward call cannot recover operations that were never recorded.
