...
The approximation is off by at most about $0.02$ from the exact function, in the corner near $x = -0.75$, and the production section says why anyone tolerates that. Notice that the call is `SigmoidFunction().forward`, the array-level method, not `Sigmoid()` on a tensor. Inside an operation everything is NumPy; tensors and graph records exist only at the boundary that `apply` guards.
...
| Non-contiguous input | NumPy walks the strides; the result is a fresh row-major array | Walk the strides directly, no copy |
