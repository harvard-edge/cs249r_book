...
Casting the weights `[0.0123, -0.0871, 0.0456, -0.0012]` to integers produces four zeros. Each weight could fit in a byte, but the layer would lose every contribution from its input. Chapter 14 gave us a reason to try reducing those bytes: a linear layer processing one input row performs only about two arithmetic operations per weight. When weight traffic limits its speed, a smaller representation can help. The representation must preserve useful differences between weights.
...
The path in @fig-quantization-geometry separates the numerical mapping from its storage format. There are 256 signed INT8 codes, with 255 steps between the endpoints.
...
The class should first agree with the four weights traced by hand. The codes, the scale, and the zero point should match the displayed codes and metadata to the shown precision, and the two forwards should land within a few thousandths of the exact product. Chapter 3's `Linear` stores its weight as inputs by outputs, so $W$ goes in transposed.
