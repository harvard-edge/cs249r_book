...
@sec-transformers ended with `TinyGPT.forward`, which takes a sequence of token IDs and returns one row of logits per position. That is a model that can score continuations, not one that writes them, and nothing in @sec-convolutions through @sec-transformers said where its training labels come from. This milestone answers the question a practitioner asks the first time they finish a model definition: how do I train this thing on plain text, and how do I get text back out of it?
...
::: {.column-margin}
**Source Code Mapping:**
Module 13 (`src/13_transformers/13_transformers.py`) is the transformer students build in the TinyTorch course, and its `GPT` class (`TinyGPT`) carries `generate` and `_sample_next_token`. The sampler, generation loop, window builder, and training run are book code exercising `TinyGPT`, `BPETokenizer`, `CrossEntropyLoss`, and `AdamW`.
:::
...
The course's integration script for these modules, `01_vaswani_attention.py`, does not train on prose. It builds an `AttentionTransformer` from the @sec-embeddings through @sec-transformers pieces (a Post-LN block with a ReLU feed-forward layer and no causal mask, since its tasks need every position to see every other) and puts the attention mechanism through three tests: reverse a six-letter sequence, copy it, and do either depending on a prefix token. Its pass thresholds are 95 percent sequence accuracy on reversal, 95 percent on copying, and 90 percent on the mixed task. Its training step, quoted from the script, is the same reshape this milestone just used, with the fraction of completely correct sequences counted beside the loss and @sec-dataloader's `DataLoader` handing out the batches.

{{< include _listings/milestone_02/train_epoch.qmd >}}

The script builds its optimizer list from each component's `parameters()`, including the learned positional table. That table can now both receive gradients and be updated, unlike @sec-embeddings's intentional fixed sinusoidal table. Checking this registration boundary matters even when a task score passes: a model can learn through its other weights while an omitted component remains unchanged.
