...
![The positive-temperature generation loop. Each pass recomputes the retained token prefix, then samples from its last logit row and appends the selected ID.](assets/images/diagrams/milestone_02-diag-1.svg){#fig-milestone2-loop}
...
Behind these decoding choices sits a question this book has not yet answered. `TinyGPT` outputs a row of $V$ logits at every position. A classifier in Milestone I had one label per image. What is the label for position 7 of a sentence?
...
Now the Part I loop gets a language model in the middle. Every piece is already built: `BPETokenizer` (Chapter 10) turns the corpus into one token stream, `make_windows` turns the stream into pairs, `TinyGPT` (Chapter 13) turns a batch of windows into $[B, S, V]$ logits, and `CrossEntropyLoss` (Chapter 4) and `AdamW` (Chapter 7) do what they did in Milestone I. This final block uses the imports and helpers defined above.
...
The following calculation reconstructs the probability columns before the random draw. Its assertions check the largest probabilities at the settings used above.
...
2. `sample_next_token` filters by top-$k$ before scaling by temperature. Swap the order in a copy and show, on the logits from @tbl-sampling-shaping-trace with positive $T$, that both probability vectors agree within floating-point tolerance. Then explain in one sentence why the two orders could differ if `top_k` selected by probability threshold rather than by count.
