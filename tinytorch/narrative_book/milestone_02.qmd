...
Module 13 (`src/13_transformers/13_transformers.py`) is the transformer students build in the TinyTorch course, and its `GPT` class, which this book calls `TinyGPT`, carries a `generate` method and a `_sample_next_token` helper. Both are quoted below, and both sample with temperature only, with no top-$k$ and no cropping of the context. The course's integration script for this part of the book, `milestones/05_2017_transformer/01_vaswani_attention.py`, trains a small transformer on sequence reversal and copying tasks rather than on free text, and its training step is quoted from the script. The sampler, the generation loop, the window builder, and the training run are book code, labelled as such; they call the real `TinyGPT`, `BPETokenizer`, `CrossEntropyLoss`, and `AdamW`, and the training run was measured.
:::
