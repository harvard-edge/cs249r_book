...
For the three inputs `[-1, 0, 1]`, the GELU activation returns approximately `[-0.15881, 0, 0.84119]`. Module 17 computes these values in two ways. One keeps seven intermediate `Tensor` objects until the function returns; the other writes the calculation as one NumPy expression. Both produce the same answer within floating-point tolerance, but they allocate and retain different arrays. After chapter 16 examined weight representation, this chapter follows the temporary data created during a forward computation.

The first mechanism compares those allocation choices. The second, **cache tiling**, partitions a matrix multiplication into small blocks and accumulates their products. Both make execution costs visible without changing the intended mathematical function. Actual timing depends on arithmetic, data movement, and execution overhead. After tracing the NumPy implementations, we introduce **kernel fusion**, a production technique that compiles several operations into one traversal of the data. @Fig-kernel-fusion separates the three execution paths used in the module.
...
Our model counts requested data separately from measured time. A logical read of an array is an operation's need for its values; it may be served from a cache. An allocated array occupies storage, but allocation alone does not tell us how many bytes crossed a hardware boundary. Chapter 14's arithmetic intensity divides floating-point work by a stated byte count. Here every such count needs its assumptions attached.
...
At $x=1$, the result is approximately $0.84119$, matching the tanh GELU computation. @Tbl-fusion-trace accounts for eighteen logical array passes, or 360 bytes, before Tensor copies. A hypothetical implementation that reads each input once and writes each output once accounts for 40 bytes. The ratio of nine compares two models, not the measured traffic of the two NumPy helpers. @Lst-acceleration-gelu-ledger reproduces the values and checks the output against `fused_gelu`.
...
![](assets/images/diagrams/17_tiled-matmul-accumulation.svg){width=5in}
:::
...
Chapter 18 moves from data reused inside one operation to results reused across calls. During text generation, earlier keys and values can remain valid when one new token arrives. A cache makes that lifetime explicit and introduces a new obligation: reuse only state that belongs to the current generation.

---

## Exercises

1. **Trace the partial tile.** Change `+=` to `=` in a local copy of the blocked loop. Predict the result for the $(2,3)$ by $(3,2)$ example at tile sizes 2 and 4, then run it. Explain why one size detects the bug and the other does not.

2. **Follow ownership.** Call both GELU helpers with a Tensor that requires gradients. Check the returned shape, `requires_grad`, and `np.shares_memory` with the input. Modify one output element and verify that the input stays unchanged. Explain which constructor establishes that ownership boundary.

3. **Count and measure separately.** Reproduce the GELU operation ledger for three inputs. Add the logical read and write required by each Tensor constructor copy in `unfused_gelu`. Then time both helpers on several larger sizes. Explain why dividing your modeled byte count by elapsed time does not measure hardware memory bandwidth.

4. **Size the tile.** For a cache budget of 32,768 bytes, check which of $T=16,32,64$ satisfy the three-block storage estimate. Compare the actual tiled result with `vectorized_matmul` for a $(13,7)$ by $(7,5)$ product at each size. Then time a larger fixed workload and explain why fitting the estimate alone does not identify the fastest size.
