...
Chapter 15 mapped each weight onto an integer grid and calculated the storage a packed INT8 representation would need. TinyTorch still holds those codes in float32 arrays, and the matrices themselves have not changed shape. A $4096 \times 4096$ layer still has 16.8 million cells, and every one of them is read and multiplied on every forward pass, whether or not the value in it matters.

Consider a $2 \times 3$ float32 weight matrix. Its six entries occupy 24 bytes. Setting three entries to zero leaves all 24 bytes allocated; deleting one output column leaves a $2 \times 2$ matrix occupying 16 bytes. The prediction may change in either case. Compression therefore requires two separate checks: what representation the next forward pass consumes, and how its output differs from the original.

**Magnitude pruning** zeros individual weights, while Module 16's **structured pruning** zeros whole output columns and their biases. Neither changes dense array dimensions. **Low-rank factorization** returns smaller SVD factors, which require a new forward path before they replace a layer. The book supplies two short extensions that perform these structural changes. **Knowledge distillation** then supplies a differentiable objective for training a smaller student from fixed teacher predictions; it provides a way to pursue accuracy recovery, not a guarantee of it. @Fig-compression-spectrum distinguishes the reference representations before we follow their implementation.
...
Module 16 defaults to $T = 3$ and $\alpha = 0.7$. Both terms average over examples after summing over classes, so duplicating a batch leaves the objective unchanged. The soft term here omits the optional temperature-squared multiplier; changing temperature therefore changes both the target distribution and its gradient scale. The training loop still uses Chapter 8's forward, backward, and update sequence, with the teacher held fixed.
...
The listings in this chapter are extracted from Module 16 (`src/16_compression/16_compression.py`), which the package exports as `tinytorch.perf.compression`: `measure_sparsity`, `magnitude_prune`, `structured_prune`, `low_rank_approximate`, and `KnowledgeDistillation`. Two short functions are book code and are labeled as such where they appear. `prune_neurons` performs the shape change that the module's `structured_prune` deliberately stops short of, and `factorize_linear` turns the truncated SVD the module returns into two `Linear` layers.
:::
...
The loss has a soft term and a hard term. The soft term is the KL divergence from the teacher's tempered softmax to the student's, the hard term is Chapter 4's cross-entropy, and $\alpha$ mixes them. The module packages the two models and the two constants in a class whose only real method is the loss.
...
Use one input, two output classes, and explicitly chosen weights so that the trace has no random initialization hidden in it. The student begins with equal logits and therefore assigns each class probability one half. The teacher favors class zero, which also matches the label. Constructing the optimizer before the first student forward pass matters in TinyTorch: the optimizer enables gradient tracking on the student's parameters, and Chapter 6 records dependencies when the forward operations run.
...
The invariant to carry into Chapter 17 is that an optimization claim must name the representation, its consumer, and the comparison being measured. Reducing weights addresses only part of a forward pass. Chapter 17 examines the intermediate activations between operations and whether changing their computation order or lifetime can reduce work without altering the intended result.
