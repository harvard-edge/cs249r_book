...
A candidate model returns its answer twice as fast, but changes one correct prediction into an error. Another candidate zeros half its weights, yet its arrays occupy exactly as many bytes as before. Neither a speedup nor a sparsity percentage tells us whether to keep the change. Chapter 19 supplied the timing and scoring mechanisms. This chapter follows their results through Module 20's report, comparison, validation, and classroom eligibility checks.

The central experiment preserves a baseline, transforms a copy, and evaluates both on the same inputs. A small classifier makes every prediction and stored byte visible. A separate cost model then explains why the speedups of several transformations cannot usually be multiplied, and a cached transformer experiment tests the same reporting discipline when the workload has persistent state. The model predicts costs; the executable comparisons supply evidence about the artifact that actually runs.
...
Each of the four levers acts on one of the three quantities, and that is the whole reason the model has three terms rather than one. INT8 quantization (Chapter 15) shrinks $Q$, low-rank factorization (Chapter 16) shrinks matrix storage and arithmetic but adds intermediate arrays and products, kernel fusion (Chapter 17) shrinks $n_{\text{ops}}$ here, although a fused implementation can also reduce intermediate-memory traffic, and the KV cache (Chapter 18) changes how $F$ grows with the number of tokens already generated. The Code says how much, lever by lever, next to the line that implements each one.
...
Module 20 (`src/20_capstone/20_capstone.py`) is the NumPy-backed benchmarking and submission workflow students build at the end of the TinyTorch course. It provides a `BenchmarkReport` that measures a model's parameter count, size, accuracy, and latency, and a `generate_submission` that compares a baseline report against an optimized one and records the speedup, compression ratio, and accuracy change in a JSON file. Those two pieces are quoted from the module below. It also owns `OlympicEvent` and `qualifies_event`, which apply classroom eligibility rules to measured report fields. The cost model is separate book code and runs without `Tensor` or `GPT`. Its switches do not transform a TinyTorch model. The classifier trace uses the actual submission path; the generation trace uses workload-specific metrics because classification accuracy does not describe logit equivalence.
:::
...
The candidate is a deep copy. Zeroing its second output weight changes the scores to `[[2, 0], [1, 0], [3, 0], [1, 0]]`. It now predicts `[0, 0, 0, 0]` and gets two of four labels right. The array shapes and byte counts are unchanged. These are exact consequences of the transformation; its timing remains something to measure.
...
`benchmark_model` receives `(4, 2)` scores, reduces the class axis with `argmax`, and counts label agreements over four rows. Its returned metrics describe each model separately. `generate_submission` then returns storage ratio 1.0 and accuracy delta −0.5, while the speedup uses the two measured median latencies. Five repeats keep this trace short; they do not establish a stable performance result.

The final two assertions answer different questions. The file passes validation because its fields and derived comparisons agree. The candidate fails the latency event because its accuracy is below 0.85, regardless of its speed. A correct reporting pipeline can reject an optimization; that is the intended result here. After deciding which candidate to retain, `save_submission(submission, path)` records the already validated dictionary.
...
[^fn-cuda-graphs-bridge]: A [CUDA graph](https://docs.nvidia.com/cuda/cuda-programming-guide/04-special-topics/cuda-graphs.html) records GPU operations and their dependencies for later replay. Replay can reduce repeated host dispatch work; it does not eliminate the recorded computation or guarantee that every decode workload has a fixed replayable shape.
...
2. **Check the record.** Copy the valid submission, change `improvements["compression_ratio"]` to 2.0, and predict the validator's result. Then remove the median field from only one report and generate the submission again. Verify which latency statistic the speedup now compares.
