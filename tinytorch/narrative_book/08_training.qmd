...
A weight starts at $1$, and its gradient is also $1$. With learning rate $0.1$, the optimizer moves it to $0.9$. On the next batch, `backward()` adds the new gradient to the old buffer unless the loop clears it. The optimizer from @sec-optimizers can implement its update rule correctly and still receive the wrong gradient. The training engine must establish which forward and backward computations belong to each update.

`Trainer` assembles the components built so far into one call path. It keeps parameters fixed while a batch produces a loss and gradients, then lets the optimizer update them and clears the gradient buffers. Several batches can contribute to the same update if their contributions are weighted by sample count. The boundaries in @fig-training-pipeline determine where scheduling, clipping, evaluation, and checkpointing attach to this loop.
