...
::: {#fig-03_layers-diag-1 fig-env="figure" fig-pos="htb" fig-cap="**TinyTorch Layer System**: Inheritance and composition of neural network building blocks." fig-alt="Diagram showing the Layer base class, Linear and Dropout subclasses, and the Sequential container."}
...
mem_weight_bytes = f"{mem_weight_bytes:,}"
mem_weight_kb = f"{mem_weight_kb:.0f}"

mem_bias_bytes = 256 * 4
mem_bias_kb = mem_bias_bytes / 1024
mem_bias_bytes = f"{mem_bias_bytes:,}"
mem_bias_kb = f"{mem_bias_kb:.0f}"

mem_total_kb = (mem_weight_bytes + mem_bias_bytes) / 1024
mem_total_kb = f"{mem_total_kb:.0f}"

# Activation memory for batch=32
mem_input_bytes = 32 * 784 * 4
mem_input_kb = mem_input_bytes / 1024
mem_input_bytes = f"{mem_input_bytes:,}"
mem_input_kb = f"{mem_input_kb:.0f}"

mem_output_bytes = 32 * 256 * 4
mem_output_kb = mem_output_bytes / 1024
mem_output_bytes = f"{mem_output_bytes:,}"
mem_output_kb = f"{mem_output_kb:.0f}"
...
flops_l2 = f"{flops_l2:,}"
flops_l3 = f"{flops_l3:,}"
flops_total = f"{flops_total / 1e6:.1f}"
```
...
Weights: 784 × 256 × 4 = `{python} mem_weight_bytes` bytes ≈ `{python} mem_weight_kb` KB
Bias:    256 × 4 = `{python} mem_bias_bytes` bytes ≈ `{python} mem_bias_kb` KB
Total:   ≈ `{python} mem_total_kb` KB

Activation memory depends on batch size. For batch size 32 and the same layer:

Input:   32 × 784 × 4 = `{python} mem_input_bytes` bytes ≈ `{python} mem_input_kb` KB
Output:  32 × 256 × 4 = `{python} mem_output_bytes` bytes ≈ `{python} mem_output_kb` KB
...
Layer 1: 32 × 784 × 256 = `{python} flops_l1` FLOPs
Layer 2: 32 × 256 × 128 = `{python} flops_l2` FLOPs
Layer 3: 32 × 128 × 10  = `{python} flops_l3` FLOPs
Total:   ≈ `{python} flops_total` million FLOPs per forward pass
...
q1_orig_params = f"{q1_orig_params:,}"
q1_doubled_params = f"{q1_doubled_params:,}"

q1_orig_weights = 784 * 256
q1_doubled_weights = 784 * 512
q1_orig_weights = f"{q1_orig_weights:,}"
q1_doubled_weights = f"{q1_doubled_weights:,}"

q1_orig_bytes = q1_orig_params * 4
q1_orig_kb = q1_orig_bytes / 1024
q1_orig_bytes = f"{q1_orig_bytes:,}"
q1_orig_kb = f"{q1_orig_kb:.0f}"

q1_doubled_bytes = q1_doubled_params * 4
q1_doubled_mb = q1_doubled_bytes / 1024**2
q1_doubled_bytes = f"{q1_doubled_bytes:,}"
q1_doubled_mb = f"{q1_doubled_mb:.2f}"
```
...
q2_mem_bytes = f"{q2_mem_bytes:,}"
q2_mem_kb = f"{q2_mem_kb:.0f}"

# Activation memory for batch size 32
q2_act_bytes = 32 * (784 + 256 + 128 + 10) * 4
q2_act_kb = q2_act_bytes / 1024
q2_act_kb = f"{q2_act_kb:.0f}"
```

:::{.callout-note collapse="true" title="Answer"}

**Layer 1**: 784 × 256 + 256 = `{python} q2_l1` parameters
**Layer 2**: 256 × 128 + 128 = `{python} q2_l2` parameters
**Layer 3**: 128 × 10 + 10 = `{python} q2_l3` parameters
...
q4_matmul = f"{q4_matmul:,}"
q4_bias = f"{q4_bias:,}"
q4_ratio = f"{q4_ratio:.0f}"
```
...
- **Matmul**: 32 × 784 × 256 = `{python} q4_matmul` operations
- **Bias**: 32 × 256 = `{python} q4_bias` operations

Matrix multiply dominates by ~`{python} q4_ratio`x. This is why optimizing matmul (using BLAS, GPU kernels) is critical for neural network performance.
:::
