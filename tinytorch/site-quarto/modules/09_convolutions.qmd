...
stride1_out = f"{stride1_out}"
stride2_out = f"{stride2_out}"
```
...
output_size = (224 + 2×1 - 3) / 1 + 1 = `{python} stride1_out`

For the same input with stride=2:

output_size = (224 + 2×1 - 3) / 2 + 1 = `{python} stride2_out`
...
pool_before_mb = f"{pool_before_bytes / 1024**2:.1f} MB"
pool_after_mb = f"{pool_after_bytes / 1024**2:.1f} MB"
pool_saved_mb = f"{pool_saved_bytes / 1024**2:.1f} MB"
```

A 2×2 max pooling with stride=2 divides spatial dimensions by 2, reducing memory and computation by 4×. For a 224×224×64 feature map (`{python} pool_before_mb`), pooling produces 112×112×64 (`{python} pool_after_mb`), saving `{python} pool_saved_mb`.
...
H_out = (224 + 2×1 - 3) / 1 + 1 = `{python} ex1_h`
W_out = (224 + 2×1 - 3) / 1 + 1 = `{python} ex1_h`
Output: (32, 64, `{python} ex1_h`, `{python} ex1_h`)

MaxPool2d(kernel_size=2, stride=2):
H_out = (224 + 0 - 2) / 2 + 1 = `{python} ex2_h`
W_out = (224 + 0 - 2) / 2 + 1 = `{python} ex2_h`
Output: (32, 64, `{python} ex2_h`, `{python} ex2_h`)

Conv2d(64, 128, kernel_size=3, padding=0, stride=2):
H_out = (112 + 0 - 3) / 2 + 1 = `{python} ex3_h`
W_out = (112 + 0 - 3) / 2 + 1 = `{python} ex3_h`
Output: (32, 128, `{python} ex3_h`, `{python} ex3_h`)
...
complexity_hw = f"{complexity_hw:,}"
complexity_kernel = f"{complexity_kernel}"
complexity_ops = f"{complexity_ops:,}"
complexity_approx = f"{complexity_ops / 1e9:.1f}"

# 7x7 vs 3x3 kernel ratio
kernel_ratio = (7 * 7) / (3 * 3)
kernel_ratio = f"{kernel_ratio:.1f}"

# Memory for (32, 64, 224, 224) float32 tensor
mem_bytes = 32 * 64 * 224 * 224 * 4
tensor_mem_mb = f"{mem_bytes / 1024**2:.0f} MB"
```
...
This is why kernel size matters enormously. A 7×7 kernel requires (7×7)/(3×3) = `{python} kernel_ratio`× more computation than 3×3. Modern architectures favor stacking multiple 3×3 convolutions instead of using large kernels.
...
32 × 64 × 224 × 224 × 4 bytes = `{python} tensor_mem_mb`
...
dim_err_result = f"{dim_err_result}"
```

**Example**:

Input: 224×224, kernel=3, padding=0, stride=2
output_size = (224 + 0 - 3) // 2 + 1 = 221 // 2 + 1 = 110 + 1 = `{python} dim_err_result`
...
overflow_gb = f"{overflow_bytes / 1024**3:.1f} GB"
```

Large feature maps consume enormous memory. A batch of 64 images at 224×224×64 channels = `{python} overflow_gb` for a single layer's output. Deep networks with many layers can exceed GPU memory.
...
q2_dense_input = f"{q2_dense_input:,}"
q2_dense_params = f"{q2_dense_params:,}"
q2_ratio = f"{q2_ratio:,}"
...
q3_h = f"{q3_h}"
q3_hw = f"{q3_hw:,}"
q3_kernel_block = f"{q3_kernel_block}"
q3_ops = f"{q3_ops:,}"
q3_approx = f"{q3_ops / 1e9:.1f}"

# Q4: Conv2d(3, 256, k=7, s=2, p=3) on (64, 3, 224, 224)
q4_h = (224 + 2 * 3 - 7) // 2 + 1
q4_mem_bytes = 64 * 256 * q4_h * q4_h * 4
q4_h = f"{q4_h}"
q4_mem_bytes = f"{q4_mem_bytes:,}"
q4_mem_mb = f"{q4_mem_bytes / 1024**2:.0f} MB"
...
H_out = (128 + 2×2 - 5) / 2 + 1 = (128 + 4 - 5) / 2 + 1 = 127 / 2 + 1 = 63 + 1 = `{python} q1_h`
W_out = (128 + 2×2 - 5) / 2 + 1 = `{python} q1_h`

Output shape: **(32, 64, `{python} q1_h`, `{python} q1_h`)**
...
Weight: 64 × 3 × 3 × 3 = `{python} q2_weight` parameters
Bias: `{python} q2_bias` parameters
Total: `{python} q2_total` parameters

Compare this to a fully connected layer for 224×224 RGB images:

Dense(224×224×3, 64) = `{python} q2_dense_input` × 64 = `{python} q2_dense_params` parameters!

Convolution achieves **`{python} q2_ratio`× fewer parameters** through parameter sharing!
:::
...
H_out = (56 + 2×1 - 3) / 1 + 1 = `{python} q3_h`
W_out = (56 + 2×1 - 3) / 1 + 1 = `{python} q3_h`

Then total operations:

16 × 128 × `{python} q3_h` × `{python} q3_h` × 64 × 3 × 3
= 16 × 128 × `{python} q3_hw` × `{python} q3_kernel_block`
= `{python} q3_ops` operations
≈ `{python} q3_approx` billion operations per forward pass!
...
H_out = (224 + 2×3 - 7) / 2 + 1 = (224 + 6 - 7) / 2 + 1 = 223 / 2 + 1 = 111 + 1 = `{python} q4_h`
W_out = `{python} q4_h`

Output shape: (64, 256, `{python} q4_h`, `{python} q4_h`)

Memory (float32 = 4 bytes):

64 × 256 × `{python} q4_h` × `{python} q4_h` × 4 = `{python} q4_mem_bytes` bytes
≈ `{python} q4_mem_mb` for a single layer's output!
...
Layer 1 - Conv(3×3, stride=1): RF = `{python} q5_rf1`
Layer 2 - MaxPool(2×2, stride=2): RF = `{python} q5_rf1` + (2-1)×1 = `{python} q5_rf2`
Layer 3 - Conv(3×3, stride=1): RF = `{python} q5_rf2` + (3-1)×2 = `{python} q5_rf3`  (stride accumulates)
Layer 4 - Conv(3×3, stride=1): RF = `{python} q5_rf3` + (3-1)×2 = `{python} q5_rf4`

**Receptive field = `{python} q5_rf4`×`{python} q5_rf4`**

Each neuron in the final layer sees a `{python} q5_rf4`×`{python} q5_rf4` region of the original input. This is why stacking layers with stride/pooling is crucial: it grows the receptive field so deeper layers can detect larger patterns.
