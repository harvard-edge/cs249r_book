...
lt_throughput = f"{lt_throughput:.0f}"

lt_batch_size = 32
lt_batch_time_ms = 50
lt_batch_throughput = lt_batch_size * 1000 / lt_batch_time_ms
lt_batch_throughput = f"{lt_batch_throughput:.0f}"
```
...
If latency is 10ms per sample, throughput is 1000ms / 10ms = `{python} lt_throughput` samples/second. But this assumes processing samples one at a time. In practice, batching increases throughput significantly while adding latency. Processing a batch of 32 samples might take 50ms total, giving `{python} lt_batch_throughput` samples/second throughput but 50ms per-request latency.
...
scale_seconds_saved = f"{scale_seconds_saved:,.0f}"
scale_days_saved = f"{scale_days_saved:.2f}"
scale_cost_reduction = f"{scale_cost_reduction_pct:.0f}%"
...
scale_pipeline_savings = f"${scale_pipeline_savings:,.0f}"
```

To appreciate why professional benchmarking matters, consider the scale of production ML systems:

- **Model serving**: A recommendation system handles 10 million requests/day. If you reduce latency from 20ms to 10ms, you save `{python} scale_seconds_saved` seconds of compute daily = `{python} scale_days_saved` days of compute per day = `{python} scale_cost_reduction` cost reduction.
- **Training efficiency**: Training a large language model costs $1 million in GPU time. Profiling reveals 60% of time is spent in data loading. Optimizing the data pipeline saves `{python} scale_pipeline_savings`.
...
q1_fp32_bytes = f"{q1_fp32_bytes:,}"
q1_fp32_mb = f"{q1_fp32_mb:.2f}"
q1_int8_bytes = f"{q1_int8_bytes:,}"
q1_int8_mb = f"{q1_int8_mb:.2f}"
q1_savings_mb = f"{q1_savings_mb:.2f}"
q1_reduction_pct = f"{q1_reduction_pct:.0f}%"
q1_compression = f"{q1_compression:.1f}x"
```

:::{.callout-note collapse="true" title="Answer"}

FP32: 5,000,000 parameters x 4 bytes = `{python} q1_fp32_bytes` bytes = **`{python} q1_fp32_mb` MB**

INT8: 5,000,000 parameters x 1 byte = `{python} q1_int8_bytes` bytes = **`{python} q1_int8_mb` MB**

Savings: `{python} q1_fp32_mb` MB - `{python} q1_int8_mb` MB = **`{python} q1_savings_mb` MB** (`{python} q1_reduction_pct` reduction)

Compression ratio: `{python} q1_fp32_mb` MB / `{python} q1_int8_mb` MB = **`{python} q1_compression`**
...
q2_variance_ratio = f"{q2_variance_ratio:.0f}x"
q2_a_lo = f"{q2_a_lo:.1f}"
q2_a_hi = f"{q2_a_hi:.1f}"
q2_b_lo = f"{q2_b_lo:.1f}"
q2_b_hi = f"{q2_b_hi:.1f}"
```
...
Same mean latency (10.0ms) but Model A has `{python} q2_variance_ratio` lower variance (0.3ms vs 3.0ms std).

Model A's latency range: ~`{python} q2_a_lo`-`{python} q2_a_hi`ms (95% confidence: +/- 2 std)
Model B's latency range: ~`{python} q2_b_lo`-`{python} q2_b_hi`ms (95% confidence: +/- 2 std)
...
q3_amortized = f"{q3_amortized:.1f}"
q3_fixed = f"{q3_fixed:.1f}"
q3_var_total = f"{q3_var_total:.1f}"
q3_var_per_sample = f"{q3_var_per_sample:.1f}"
q3_batch1_check = f"{q3_batch1_check:.1f}"
```

Measuring latency with batch_size=32 gives 100ms total. Can you claim 100ms / 32 = `{python} q3_amortized`ms per-sample latency?
...
Example reality:
- Batch=32: 100ms total → `{python} q3_amortized`ms per sample (amortized)
- Batch=1: 8ms total → 8ms per sample (actual)

**Why the discrepancy?**
- Fixed overhead: `{python} q3_fixed`ms (data transfer, setup)
- Variable cost: `{python} q3_var_total`ms / 32 = `{python} q3_var_per_sample`ms per sample
- At batch=1: `{python} q3_fixed`ms fixed + `{python} q3_var_per_sample`ms variable = `{python} q3_batch1_check`ms
...
q4_speedup = f"{q4_speedup:.1f}x"
q4_times_faster = f"{q4_speedup:.0f}"
q4_optimized_rps = f"{q4_optimized_rps:.0f}"
q4_optimized_cost = f"${q4_optimized_cost:.0f}"
q4_headroom = f"{q4_headroom:.0f}%"
```
...
Speedup = baseline_latency / optimized_latency = 20ms / 5ms = **`{python} q4_speedup`**

**What it means:**
- Optimized model is **`{python} q4_times_faster` times faster**
- Processes same input in 1/`{python} q4_times_faster` the time
- Can handle `{python} q4_times_faster`x more traffic with same hardware

**Real-world impact:**
- If baseline served 100 requests/sec, optimized serves `{python} q4_optimized_rps` requests/sec
- If baseline cost $1000/month in compute, optimized costs `{python} q4_optimized_cost`/month
- If baseline met latency SLA at 60% utilization, optimized has `{python} q4_headroom` headroom
...
:::{.callout-note title="Congratulations: You've Completed TinyTorch!"}
