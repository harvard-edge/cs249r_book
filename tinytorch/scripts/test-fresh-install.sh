...
BRANCH="main"
CI_MODE=false
REPO="${GITHUB_REPOSITORY:-harvard-edge/cs249r_book}"
...
curl -fsSL "https://raw.githubusercontent.com/__REPO__/${BRANCH}/tinytorch/guide/install.sh" -o /tmp/install.sh || {
    echo "✗ Failed to download install script for branch: $BRANCH"
    echo "  URL: https://raw.githubusercontent.com/__REPO__/${BRANCH}/tinytorch/guide/install.sh"
    echo "  Hint: Does the branch '${BRANCH}' exist and contain tinytorch/guide/install.sh?"
    exit 1
}
bash /tmp/install.sh
...
TEST_SCRIPT=$(build_test_script | sed -e "s|__BRANCH__|$BRANCH|g" -e "s|__REPO__|$REPO|g")
