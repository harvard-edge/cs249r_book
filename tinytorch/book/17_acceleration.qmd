...
5. **Recompute instead of saving.** `Im2colConv2dFunction` keeps the patch matrix from forward to backward. Change it to save only the input and rebuild `cols` in `backward`. Check that the gradients still match `Conv2d`, then measure the time per training step and the peak memory of both versions on a layer large enough for the difference to show. State which you would choose for a deep CNN on a device with little memory.
