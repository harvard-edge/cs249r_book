...
From a framework and systems standpoint, **operator fusion** and **tiling** are the two standard answers, one reducing intermediate arrays and the other reusing data while it is still close to the processor:
1. **Eliminating memory round-trips**: Combining sequential elementwise operations into one pass, as a compiled kernel would, so intermediates need not be written out and read back.
2. **Reuse before eviction**: Keeping values in registers or cache while they are still needed, which a compiled kernel or a tiled loop can arrange and a sequence of NumPy calls cannot guarantee.
3. **The memory traffic ledger**: Counting logical array reads and writes, which for the eight-operation GELU come to nine times those of an ideal single pass. That ratio compares two models; it is not the measured traffic of either NumPy helper.

This chapter writes a GELU expressed with fewer temporary Tensors and a tiled matrix multiply in NumPy, then times both. The names describe how each computation is expressed; only the timing says what changed.
...
`im2col_conv2d` is the convolution in three steps. It builds the patch matrix, multiplies it by the transposed filter matrix with the `vectorized_matmul` from earlier in the module, and moves the channel axis from last to second. It reads `.data` and wraps the result in a new `Tensor`, like the other helpers in this chapter, so it computes Module 09's forward pass but records no backward path. Training through it would need col2im, which scatters each patch gradient back into the pixels it was copied from.
...
`im2col` copies $K^2$ strided slices into a patch matrix whose columns follow the order in which a filter flattens, and `im2col_conv2d` turns Module 09's seven loops into one call to `vectorized_matmul`. It reproduces `Conv2d` exactly, pays for it with a patch matrix about $K^2$ times the input, and like the other helpers here stops at the forward pass.
...
5. **Write col2im.** `im2col_conv2d` has no backward pass. Given the gradient with respect to its output, write the gradient with respect to the weights as one matrix multiply, and the gradient with respect to the patch matrix as another. Then write `col2im`, which adds each row of the patch gradient back into the pixels it came from, and check the result against Module 09's `Conv2d.backward` on a small input. Explain why col2im is an accumulation rather than a copy.
