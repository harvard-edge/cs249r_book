# Layers: Parameters, State, and Initialization {#sec-layers}
...
![**A shared layer interface**: Linear owns parameters, Dropout changes behavior with the training argument, and Sequential composes their forward calls and collects parameters.](assets/images/diagrams/03_layers-diag-1.svg){#fig-layers-container width="100%"}
...
::: {#chk-milestone-01 .callout-checkpoint title="Milestone 01 Unlocked: The 1958 Perceptron"}
With `Tensor` (Module 01), activations (`Sigmoid`, Module 02), and `Linear` layers (Module 03) complete, you have built the entire forward execution engine. You can now run your first historical milestone from the terminal:

```bash
tito milestone run 01
```

This milestone executes Frank Rosenblatt's 1958 single-layer perceptron on linearly separable 2D data using random weights. You will observe that while the forward pass computes without error, accuracy hovers around 50%—pure chance. The fundamental takeaway: **a forward pass alone is not intelligence**. Building model architectures is straightforward; making them learn requires loss functions, automatic differentiation, and optimizers (@sec-losses through @sec-training). See @sec-milestone-1 (Part 1) for the full historical analysis.
:::
