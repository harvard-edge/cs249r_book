...
1. **State management and parameter discovery**: How to hierarchically register child parameters (`parameters()`) so optimizers can locate every trainable weight exactly once.
...
3. **Variance-preserving initialization**: How to scale initial weights by fan-in (LeCun's rule, which this module uses, and its He and Xavier relatives) so that activation variance neither grows nor shrinks from one layer to the next at initialization.
