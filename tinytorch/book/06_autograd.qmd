...
3. **Memory lifetime and mutation safety**: Accumulate gradients into leaf parameters (`param.grad += ...`), release the graph's links once backward has used them so intermediates can be reclaimed, and keep code from mutating a saved tensor in place before backward reads it. TinyTorch does not detect such a mutation; the chapter shows what it corrupts.
