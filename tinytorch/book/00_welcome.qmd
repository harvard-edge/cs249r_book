...
1. **Part I: The Core Engine (Chapters 01–08)** builds the execution foundation: strided tensor storage, non-linear activations, parameter containers, loss functions, mini-batch loaders, reverse-mode autograd, and AdamW optimization.
2. **Part II: Deep Architectures (Chapters 09–13)** constructs the generative transformer: subword byte-pair encoding (BPE), learned positional embeddings, scaled dot-product causal multi-head self-attention, and stacked Pre-LN transformer decoder blocks, culminating in training TinyGPT to generate coherent language from scratch.
3. **Part III: Systems and Acceleration (Chapters 14–20)** subjects that exact same TinyGPT model to production systems engineering: profiling its memory-bandwidth roofline, quantizing its weights to simulated INT8, distilling its representations, fusing its normalization and projection operators, equipping it with a Key-Value cache to eliminate $O(S^2)$ decoding recomputation, and evaluating it under strict competitive benchmark scoring gates.
...
| **05** | **The Transformer Era (2017)** | 01–13 | Multi-head attention sequence reasoning (copy, reversal) and autoregressive natural language generation with TinyGPT. | @sec-milestone-2 (Parts 2 & 3) |
| **06** | **The Optimization Olympics (2018)** | 01–20 | Systems benchmarking across profiling, INT8 quantization, weight pruning, and Key-Value cache generation speedup. | @sec-milestone-3 |
