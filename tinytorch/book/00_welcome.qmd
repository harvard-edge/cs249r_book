...
: **The TinyTorch Roadmap**: Modules 01 through 20 map to source directories under `tinytorch/src/`; milestones integrate earlier modules, and @sec-extensions supplies a production comparison. {#tbl-pedagogical-roadmap tbl-colwidths="[22,36,42]"}
...
3. **The Code and Worked Trace**: At the narrowest point of the hourglass, we inspect the exact source implementation extracted from the repository and trace a small, hand-verifiable numerical example with concrete numbers.
4. **From TinyTorch to Production**: We expand wide again, examining how production engines (PyTorch ATen, cuDNN, CUTLASS, Triton, vLLM) scale the concept across distributed clusters and specialized silicon.

The chapter summary establishes what later chapters rely on, while exercises ask the reader to predict behavior or test edge cases.
