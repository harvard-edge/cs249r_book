...
### Culminating in TinyGPT: The Unifying Workload
...
For this reason, **the entire TinyTorch curriculum is architected to culminate in TinyGPT**:

1. **Part I: The Core Engine (Chapters 01–08, Milestone I)** builds the execution foundation: strided tensor storage, non-linear activations, parameter containers, loss functions, mini-batch loaders, reverse-mode autograd, and AdamW optimization, tested by reconstructing the Perceptron, XOR, and MLP milestones.
2. **Part II: Deep Architectures (Chapters 09–13, Milestone II)** constructs spatial and sequential building blocks. Milestone 04 validates spatial features with CNNs. Milestone 05 validates the basic transformer mechanism on synthetic sequence routing challenges. Milestone 07 culminates Part II by training TinyGPT from scratch on Shakespeare, establishing the generative language foundation of ChatGPT and highlighting its $O(S^2)$ prefix recomputation bottleneck.
3. **Part III: Systems and Acceleration (Chapters 14–20, Milestone III)** subjects that exact TinyGPT model to systematic performance engineering: profiling its memory-bandwidth roofline, quantizing its projection weights to INT8, pruning redundant weights, fusing operations, and eliminating prefix recomputation with a Key-Value cache.
4. **Milestone III: The MLPerf Optimization Olympics (Milestone 06)** closes the journey by evaluating TinyGPT under MLPerf-inspired benchmarking discipline, verifying bit-for-bit logit equivalence, certifying generation speedup, and charting the Pareto frontier across accuracy, memory, and latency.

Every abstraction introduced along the journey earns its place by directly serving the construction, training, or acceleration of this flagship model.

### The Milestone Progression: Recreating Deep Learning History

Alongside the modular chapters, the repository provides seven capstone milestone workloads. Each milestone recreates a pivotal historical breakthrough using exclusively the code you have built up to that point. @Tbl-milestones-curriculum details the progression, the required modules, and how the repository workloads map to the book's three synthesis milestone chapters.

| Milestone ID | Historical Era | Required Modules | Core Investigation | Book Synthesis Chapter |
| :--- | :--- | :--- | :--- | :--- |
| **01** | **The Perceptron (1958)** | 01–03 | Forward pass with random weights on linearly separable data; demonstrates that architecture alone without training produces coin-flip accuracy. | @sec-milestone-1 (Part 1) |
| **02** | **The XOR Crisis (1969)** | 01–03 | Minsky and Papert's linear separability proof; proves that no single-layer hyperplane can separate XOR coordinates. | @sec-milestone-1 (Part 2) |
| **03** | **The MLP Revival (1986)** | 01–08 | Rumelhart et al.'s backpropagation; hidden units fold the plane to solve XOR with 100% accuracy and scale to TinyDigits. | @sec-milestone-1 (Part 3) |
| **04** | **The CNN Revolution (1998)** | 01–09 | LeNet spatial parameter sharing and pooling; evaluates parameter count reduction versus loop arithmetic on images. | @sec-milestone-2 (Part 1) |
| **05** | **The Transformer Era (2017)** | 01–08, 11–13 | Basic multi-head attention sequence reasoning (reversal, copy, mixed prefix routing) in AttentionTransformer; validates non-recurrent sequence routing. | @sec-milestone-2 (Part 2) |
| **07** | **The Generative LLM (2020–2022)** | 01–08, 10–13 | Brown et al. (GPT-3) & ChatGPT; trains TinyGPT from scratch on Shakespeare with teacher forcing and causal masking, generating coherent text via autoregressive sampling. | @sec-milestone-2 (Part 3) |
| **06** | **The MLPerf Benchmark (2018)** | 01–20 | Systems benchmarking across DigitMLP model compression and TinyGPT Key-Value cache generation speedup under multi-objective Pareto scoring gates. | @sec-milestone-3 |

: **The Seven Milestones of TinyTorch**: Chronological breakthroughs in machine learning history and their alignment with the book's milestone chapters. {#tbl-milestones-curriculum tbl-colwidths="[12,18,16,36,18]"}
