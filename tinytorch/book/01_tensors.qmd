...
1. **Input Data (Sensory Coordinates)**: How real-world phenomena enter the network. An image batch is a 4D tensor of shape $(B, C, H, W)$—batch size $B$, color channels $C$ (such as RGB), height $H$, and width $W$. A batch of natural language text is a 3D tensor of shape $(B, S, D)$—batch size $B$, sequence length $S$ tokens, and embedding hidden dimension $D$.
...
Crucially, this copy allocates an **independent memory buffer** (`np.shares_memory(At.data, A.data) == False`), but NumPy's `np.array` constructor preserves the strided access pattern of the view. The strides of `At.data` remain `(4, 12)` in bytes—column-major order. The tensor owns its memory, yet `At.data.flags["C_CONTIGUOUS"]` is `False`. To arrange the physical elements sequentially in row-major order, the student must call `At.contiguous()`, producing `Buffer C`.
