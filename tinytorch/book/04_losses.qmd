# Losses: Cross-Entropy and Numerical Stability {#sec-losses}
...
In PyTorch, computing cross-entropy on multi-class logits is a single stable function call:

```python
import torch
import torch.nn.functional as F

# Logits with large values that would overflow naive exponentials
logits = torch.tensor([[2.0, 1.0, 0.1], [1000.0, 1001.0, 1002.0]])
targets = torch.tensor([0, 2])

loss = F.cross_entropy(logits, targets)
print("Stable scalar loss:", loss.item())  # 0.4076, no NaN or Inf!
```

Underneath that single call, the runtime protects computation from the hazards of IEEE 754 floating-point arithmetic:
1. **The LogSumExp max-subtraction shift:** In raw float32, $e^{1000}$ immediately overflows to `+inf`, causing division to produce `nan`. The runtime secretly subtracts the row maximum ($\max_j z_j$) before exponentiation, bounding all exponents to $\le 0$ so they never overflow.
2. **Operator fusion across softmax and log-likelihood:** Rather than materializing separate intermediate probability tensors through Softmax and then passing them to a logarithm, the kernel fuses them into a single pass to eliminate intermediate memory traffic.
3. **Analytic gradient simplification:** Fusing the operations simplifies the backward gradient algebraically to $(p_i - y_i)$, avoiding catastrophic cancellation when computing derivatives.

This chapter constructs TinyTorch's loss library, defusing the numerical traps of cross-entropy to provide stable scalar objectives that @sec-autograd and @sec-optimizers turn into parameter updates. Before building the stable implementation, we must inspect how textbook mathematical recipes immediately fail in float32.
...
![Stable cross-entropy on large logits. For `[1000, 1002, 1001]` and target class 1, direct float32 exponentiation overflows. Subtracting the maximum gives `[-2, 0, -1]` and a finite loss of approximately 0.4076.](assets/images/diagrams/04_losses-diag-1.svg){#fig-loss-stability width="100%" fig-alt="Two paths for logits [1000, 1002, 1001] with target 1. Direct exponentiation gives infinities and NaN loss. Shifting by 1002 gives exponentials [0.1353, 1, 0.3679], sum 1.5032, and target log-probability minus 0.4076. Negation returns scalar loss 0.4076."}
