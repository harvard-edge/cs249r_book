...
In PyTorch, applying magnitude-based pruning is an effortless one-liner:
...
# Prune 50% of connections with smallest magnitude
prune.l1_unstructured(layer, name="weight", amount=0.5)

print("Sparsity ratio:", (layer.weight == 0).float().mean().item())  # 0.5
print("Tensor storage shape:", layer.weight.shape)                   # (128, 128)
```

Underneath that simple call, the systems reality of compression reveals critical nuances:
1. **The dense array illusion:** A pruned tensor with 50% zeros in a dense float32 array still occupies exactly the same memory footprint and performs the same number of arithmetic multiplications. Dense GEMM kernels do not check for zeros before multiplying.
2. **Mask indirection overhead:** Pruning frameworks maintain auxiliary binary masks alongside original weights, temporarily *increasing* memory consumption until weights are pruned permanently.
3. **Structured vs. low-rank dimension reduction:** To achieve actual speedups without specialized sparse hardware, compression must either prune whole structured channels or factorize weight matrices via low-rank SVD ($W \approx U V$), physically reducing matrix dimensions and arithmetic counts.

This chapter implements TinyTorch's compression tools (pruning, SVD factorization, and knowledge distillation) and measures what each one changes. Before building these techniques, we must inspect the systems trap of dense zeroed weights.
...
![](assets/images/diagrams/16_compression-diag-1.svg){width="100%"}
:::
...
![](assets/images/diagrams/16_neuron-removal.svg){width="100%"}
:::
