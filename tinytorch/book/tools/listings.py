...
A `path:` ending in .c, .cc, .cpp, or .h quotes a C or C++ function by name
instead: from its signature line to the matching closing brace, fenced as cpp.
...
def find_node(tree: ast.Module, symbol: str):
    ...
    # Top level, or one level into an `if` that guards an optional import
    # (the Triton kernel exists only `if _HAS_TRITON:`).
    candidates = list(tree.body)
    candidates += [n for node in tree.body if isinstance(node, ast.If) for n in node.body]
...
def extract_c(path: str, symbol: str) -> str:
    """A C/C++ function definition, from the line naming it through its closing brace."""
    source = BOOK.parent / path
    if not source.exists():
        raise FileNotFoundError(f"no source at {source}")
    ...
    raise KeyError(f"{symbol}: unbalanced braces")


def extract(module: str, symbol: str, elide=(), keep_scaffold=False, doc="first", path: str = None) -> str:
    if path is not None and path.endswith(C_SUFFIXES):
        return extract_c(path, symbol)
    ...
    # A method, or a def nested under an `if`, is shown at column zero.
...
def render(entry) -> str:
    ...
    lang = "cpp" if entry.get("path", "").endswith(C_SUFFIXES) else "python"
    if "highlight" in entry:
        fence = f"```{{.{lang} source-line-numbers=\"{entry['highlight']}\"}}\n"
    else:
        fence = f"```{lang}\n"
    return header + fence + code + "```\n"
