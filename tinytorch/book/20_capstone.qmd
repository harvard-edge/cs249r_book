...
1. **A decode-step cost model**: Predicting how INT8 quantization, low-rank factorization, kernel fusion, and the KV cache each change one TinyGPT decode step, and why multiplying each lever's solo speedup can mispredict what the levers do together.
2. **Measured trade-offs**: Benchmarking a baseline and a candidate model for accuracy, stored size, and latency, and placing candidates on the trade-off curve among the three.
3. **Verification and scorecard reporting**: Checking a candidate's outputs and accuracy against the baseline before its speed or size counts, then writing the comparison into a submission report.

This chapter combines Part III's measurements and models into one report, with each claimed gain traced to the lever that produced it.
