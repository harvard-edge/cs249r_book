...
Real-world machine learning systems engineering is never about applying a single optimization in isolation; it is an exercise in multi-objective trade-offs. An optimization that doubles inference speed is unacceptable if it drops accuracy below the task's floor, and a compression technique that reduces storage can still lose if it adds latency. The systems engineer must synthesize multiple optimizations into a coherent, balanced production pipeline.

In production serving engines, optimizing an end-to-end generative pipeline composes multiple acceleration levers into a single loop:

```python
import torch

# The end-to-end optimized generation pipeline:
# Tokenization -> Embedding -> Quantized Transformer Blocks -> KV-Cached Autoregression
def generate_tokens(model, prompt_ids, max_new_tokens=20):
    input_ids = prompt_ids
    past_key_values = None
    for _ in range(max_new_tokens):
        # Forward pass using cached past key-values and quantized weights
        outputs = model(input_ids, past_key_values=past_key_values, use_cache=True)
        next_token = outputs.logits[:, -1, :].argmax(dim=-1, keepdim=True)
        input_ids = next_token  # Feed only the single newest token
        past_key_values = outputs.past_key_values
    return input_ids
```

Underneath that end-to-end generation loop, the systems engineer must navigate complex trade-offs and non-linear interactions:
1. **The Amdahl's Law bottleneck:** Individual optimizations (INT8 quantization, kernel fusion, and KV caching) do not multiply independently. Once the KV cache eliminates redundant attention projections, remaining runtime is dominated by MLP blocks, and optimizing memory traffic buys smaller incremental gains.
2. **Quality regression bounds:** Compressing parameters or applying low-rank SVD perturbs floating-point numerical outputs. Any speedup claim is invalid unless candidate model accuracy is strictly evaluated against the baseline ground truth.
3. **The Pareto trade-off surface:** A systems deployment scorecard must balance accuracy, persistent memory footprint, and latency percentiles (P50, P99), identifying configurations that lie on the optimal trade-off frontier.

This chapter combines Part III's measurements and models into one checkable report, tracing each claimed gain to the architectural lever that produced it. Before synthesizing our candidate models, we must inspect the reporting traps that arise when isolated speedups are combined naively.

![**An optimization needs a comparison**: Preserve the baseline and workload, transform a candidate, recheck quality, and measure its actual arrays and execution before recording the comparison.](assets/images/diagrams/20_capstone-diag-1.svg){#fig-capstone-stack width="100%" fig-alt="Baseline model and workload lead to a transformed candidate and a quality and timing check. Lower panels compare against the same baseline and record measured evidence."}
...
Most of the initial estimate is the assumed dispatch term. Reducing weight bytes alone barely changes it.[^fn-dispatch-bottleneck-waterfall] Fusion lowers that term, which makes the remaining row-dependent work more consequential. Caching then removes repeated projections and reduces the linear activation traffic to one row, while adding context-dependent reads of stored keys and values.

[^fn-dispatch-bottleneck-waterfall]: **Bottleneck migration in optimization stacks**: When an initial baseline is severely dispatch-bound ($n_{\text{ops}} \cdot c \gg \text{compute, memory}$), reducing memory traffic via INT8 quantization yields an apparent $1.00\times$ speedup. It is only after kernel fusion eliminates over 80% of dispatch overhead that memory bus bandwidth becomes the primary constraint, allowing INT8 and KV caching to unlock their full theoretical throughput gains.
...
[^fn-cuda-graphs-bridge]: **CUDA Graphs and dispatch elimination**: Standard PyTorch execution submits individual CUDA kernel launches over PCIe driver queues, incurring $5\text{--}15\,\mu\text{s}$ CPU overhead per operation. CUDA Graphs record an entire sequence of kernel launches, memory copies, and stream dependencies into a single executable graph. Subsequent executions launch the entire graph with a single CPU instruction, slashing launch overhead to $<1\,\mu\text{s}$. Because graph nodes reference fixed device memory addresses and grid dimensions, dynamic input shapes require bucketed padding or multiple pre-captured graphs.
...
::: {#chk-milestone-06 .callout-checkpoint title="Milestone 06 Unlocked: The Optimization Olympics"}
With the benchmarking ledger and evaluation harness complete (Modules 01–20), you can enter the final systems arena:

```bash
tito milestone run 06
```

This milestone deploys the complete systems optimization stack—profiling bottleneck diagnosis, simulated INT8 quantization, weight pruning, and Key-Value cache generation speedup—against strict competitive scoring gates. Turn to @sec-milestone-3 to inspect the Pareto frontier across accuracy, memory footprint, and latency.
:::
...
- Reddi et al. (2020), [MLPerf inference benchmark](https://arxiv.org/abs/1911.02549), *ISCA*. Accuracy floors that an optimized submission must still meet, the rule this chapter's events apply.
- Banbury et al. (2021), [MLPerf Tiny benchmark](https://arxiv.org/abs/2106.07597), *NeurIPS*. Emphasizes how constraints at the edge demand strict optimization discipline, tying perfectly into this capstone's multi-objective evaluation.
- Sculley et al. (2015), [Hidden technical debt in machine learning systems](https://proceedings.neurips.cc/paper/2015/hash/86df7dcfd896fcaf2674f757a2463eba-Abstract.html), *NeurIPS*. The costs a deployed system carries beyond its model.
