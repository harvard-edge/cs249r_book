...
From a systems and framework perspective, an optimizer is a manager of per-parameter state. While automatic differentiation (@sec-autograd) computes raw gradient vectors, the optimizer must translate those derivatives into stable parameter trajectories:
...
3. **Stable parameter identity**: Replace each parameter's values while keeping the same `Tensor` object, so the model and the optimizer continue to refer to one shared parameter.
