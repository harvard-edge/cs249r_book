...
From a systems and framework perspective, the data pipeline sets a floor on step time: an accelerator that finishes its arithmetic before the next batch arrives sits idle. An ML framework must address three core architectural responsibilities:
...
3. **Epoch orchestration**: Draw a fresh permutation of sample indices every epoch, reproducible when the caller seeds Python's `random` module, and deliver the final partial batch when the dataset size is not a multiple of the batch size.
