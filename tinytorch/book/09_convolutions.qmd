...
3. **Autograd gradient routing**: Accumulating gradients into shared kernel weights from every position they touched, and scattering each output gradient back onto the input pixels its window covered.

This chapter expands TinyTorch into spatial representation learning. It implements 2D convolution and max pooling as direct loops inside the autograd framework, then lowers the convolution to a matrix multiply with `im2col` in a model that the loop version checks; @sec-acceleration later builds that lowering into the package.
...
![](assets/images/diagrams/09_convolution-backward-route.svg){width=5in}
:::
...
4. **Trace pooling boundaries.** Fill a `(1, 1, 4, 6)` tensor with `np.arange(24)`. Predict the outputs of `MaxPool2d((2, 3))` and `AvgPool2d((2, 3))`, then run them. Repeat with stride `(1, 2)` and explain which inputs participate more than once. For an all-negative input, compare max pooling with padding 1 against an incorrect version padded with zeros. Check one overlapping window's gradient and explain why backward accumulates rather than assigns.

5. **Follow normalization state.** Run `BatchNorm2d(1)` on `[[[[0, 1], [2, 3]]]]`. Predict the normalized output and updated running statistics. Save the running arrays, switch to `eval()`, and run a different input. Verify that the arrays remain unchanged, then compare the input gradient in training and evaluation modes. Explain why one image still supplies multiple values to each channel's statistics.
