...
This chapter expands TinyTorch into spatial representation learning. It implements 2D convolution and max pooling as direct loops inside the autograd framework, then lowers the convolution to a matrix multiply with `im2col` in a book-only model that the loop version checks.
