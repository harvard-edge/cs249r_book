...
1. **Gradient zeroing (`zero_grad()`)**: Clearing accumulated gradients from the previous step, which backward would otherwise add to without warning.
2. **Forward evaluation**: Pipelining batch inputs through model layers to compute predictions and scalar losses.
3. **Backward tape execution**: Traversing the autograd DAG to populate parameter `.grad` fields, then releasing the graph so intermediate activations can be reclaimed.
4. **Gradient clipping and optimization**: Bounding global gradient $L_2$ norms to prevent numerical divergence, followed by the optimizer's parameter updates.
...
3. **Check the next update.** Use a deterministic one-weight model and fixed batches with `AdamW`, saving after two updates. Construct an identical fresh model and an optimizer with different initial hyperparameters, load the checkpoint, and compare the next update against uninterrupted training. Enable clipping before saving. Check the moments, per-parameter update counts, restored hyperparameters, and clipping threshold. Explain which additional state would be needed if the loader shuffled samples or the model used dropout.

4. **Trace the accumulation boundary.** Repeat the unequal-batch example with `accumulation_steps=2` and `4`, recording the weight, gradients, and counters. Explain why both give the same update. Then use `accumulation_steps=1` and predict why the final weight changes. Set the learning rate to zero and verify that the reported sample-weighted loss is independent of the grouping.

5. **Choose the metric.** Evaluate the probabilities `[[0.9], [0.1]]` against binary targets `[[1], [0]]`, then evaluate the same arrays with MSELoss. Predict both returned accuracy values. Explain why changing output shape alone cannot choose the task, and design a test for a two-output regression model.
