...
In PyTorch, looking up token embeddings and injecting positional information is achieved in a few expressive lines:
...
pos_embed = nn.Embedding(512, d_model)  # Maximum context length 512

token_ids = torch.tensor([[42, 108, 999]])  # 1 sequence of 3 tokens
positions = torch.arange(3).unsqueeze(0)

# Combine token semantic features with positional coordinates
x = token_embed(token_ids) + pos_embed(positions)
print("Representation tensor shape:", x.shape)  # (1, 3, 64)
```

Underneath that simple addition, the runtime manages several low-level memory and gradient dynamics:
1. **Direct table gathering over one-hot matrix multiplication:** Rather than materializing massive, sparse one-hot matrices containing millions of zeros and executing bloated matrix multiplies, the embedding layer performs an efficient row-gather operation directly into physical DRAM.
2. **Positional coordinate injection:** Because attention mechanisms treat inputs as an unordered set, the runtime adds positional vectors directly into token representations, ensuring that the model can distinguish *"dog bites man"* from *"man bites dog"*.
3. **Scatter-add gradient accumulation:** In the backward pass, when identical token IDs appear multiple times within a batch, their gradients must be accumulated into shared table rows using an unbuffered scatter-add (`np.add.at`), preventing subsequent occurrences from overwriting earlier derivatives.

This chapter constructs TinyTorch's embedding subsystem, converting discrete token sequences into continuous representation tensors $(B, S, D)$ ready for self-attention in @sec-attention. Before building the embedding table, we must inspect why feeding raw integers fails and why one-hot matrices are computationally prohibitive.

![The embedding pipeline. A token ID selects one row of the embedding table; the row for that token's position is added; the sum is the vector the rest of the network sees.](assets/images/diagrams/11_embeddings-diag-1.svg){#fig-embeddings-pipeline width="100%"}
...
![The memory footprint of embedding matrices grows linearly with vocabulary size. Modern LLMs frequently hit hundreds of megabytes in FP32 format just for their embedding table.](assets/images/generated_plots/chapter_11_embedding_size.pdf){#fig-embed-size}
...
Calling `layer(tokens, start_pos)` enters `EmbeddingLayer.__call__`, then the attached `emblayer_forward`. A one-dimensional `(S,)` input is temporarily reshaped to `(1, S)`; a batched `(B, S)` input keeps its shape. The token lookup returns `(B, S, D)`. Optional scaling multiplies those token values by $\sqrt{D}$ before position addition; the default is `False`.[^fn-embed-sqrt-scaling] The selected position strategy then returns the same shape, and an input that lacked a batch axis has that temporary axis removed. Thus the caller gets `(S, D)` or `(B, S, D)` to match its input.

[^fn-embed-sqrt-scaling]: In the original Transformer (Vaswani et al., 2017), token embeddings are initialized with variance $1/D$, giving each token vector an expected $L_2$ norm of approximately $1.0$. In contrast, sinusoidal positional encoding rows have a fixed Euclidean norm of $\sqrt{D/2}$. Multiplying the token vectors by $\sqrt{D}$ elevates their norm to approximately $\sqrt{D}$, preventing the deterministic positional signal from dominating the semantic token representations upon addition.
...
| Rotary (RoPE) | Rotates query and key vectors inside attention | None for the rotation | Formula can extend; model limits remain | RoFormer, LLaMA, Mistral, Gemma |
| ALiBi (Press et al., 2022) | Static slope penalty added to attention logits | None | Linear penalty extrapolates | MPT, BLOOM |
...
::: {.column-margin}
**Core Invariant: RoPE Shift Invariance**\
$$\langle R_m q, R_n k \rangle = q^T R_{n-m} k$$\
*Rotates 2D coordinate subspaces by $m\theta$; attention dot product depends strictly on relative offset $(n - m)$.*
:::

![**Rotary Position Embedding (RoPE) 2D subspace rotation**: Instead of adding static position vectors to inputs, RoPE rotates 2D coordinate slices of query ($m$) and key ($n$) vectors by angle multiples $m\theta$ and $n\theta$. The resulting attention inner product preserves relative distance $(n - m)$, enabling length generalization.](assets/images/diagrams/11_rope-phase-clock.svg){#fig-rope-phase-clock width="100%" fig-alt="Diagram of 2D complex subspace circles showing query rotated by m*theta and key rotated by n*theta, with invariant relative distance dot product formula."}
