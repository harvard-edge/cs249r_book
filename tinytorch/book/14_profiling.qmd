...
1. **Parameter and memory accounting**: Counting parameter bytes, measuring the new allocations a forward pass makes, and estimating what gradients and optimizer state would add.
2. **FLOP arithmetic and operational intensity**: Counting floating-point operations from layer shapes and dividing by bytes moved to get operational intensity.
3. **The Roofline model**: Placing an operation's intensity against a machine's two ceilings to bound its attainable speed and to ask whether it is **memory-bandwidth bound** (limited by memory bus throughput) or **compute-bound** (limited by arithmetic units). The profiler's own label is a heuristic, and the chapter says where it can mislead.
