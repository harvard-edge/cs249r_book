...
3. **Padding mask.** Sequences in a batch are padded to a common length. Write `padding_mask(length, seq_len)` that returns, for one nonempty sequence of true length $\ell$, a ones-and-zeros mask in Module 12's convention forbidding every key $j \ge \ell$, combine it with the causal mask by elementwise multiplication, and check that the weight rows still sum to one and that padded keys receive exactly zero.

4. **Mask and gradient contract.** Set `requires_grad=True` on the trace values and use a mask that allows only key 0 for both queries. Before calling `out.sum().backward()`, predict both rows of `V.grad`; then check them. Change the mask to an all-zero row and explain why the call raises rather than returning a uniform distribution. Finally replace a forbidden score with a large positive finite number before masking and verify that its attention weight stays zero.
