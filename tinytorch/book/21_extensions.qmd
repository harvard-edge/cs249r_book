...
Every module so far bottoms out in NumPy. When TinyTorch multiplies two matrices, it hands the arrays to `np.matmul`, and a library someone else wrote does the arithmetic. This chapter goes one level down. TinyTorch ships three optional extensions in `tinytorch.extensions`, each a small piece of code that runs a kernel outside NumPy:

1. **C++ through `ctypes`**: a tiled matrix multiply and a fused bias-plus-GELU, compiled on your machine the first time you call them.
2. **Triton**: the same fused bias-plus-GELU written as a GPU kernel for NVIDIA hardware.
3. **PyTorch MPS**: a matrix multiply handed to the GPU of an Apple M-series chip.

None of the twenty modules depends on these extensions, and each falls back to NumPy when its compiler, library, or hardware is missing (@fig-extension-paths). The C++ kernels need only a compiler. The two GPU paths need PyTorch, which TinyTorch itself never imports, so you install it separately to try them.

The measurements in this chapter come from one machine, an Apple M5 Max laptop. The C++ library there is built single-threaded, because Apple's compiler ships without the OpenMP runtime the kernel would use to spread work across cores. Your absolute numbers will differ. The ratios between them are what the chapter is about.

![**Four ways to run an operation on a TinyTorch Tensor.** The modules use NumPy. The extensions reach the CPU's vector units directly through compiled C++, or move the data to a GPU and back.](assets/images/diagrams/21_extension-paths.svg){#fig-extension-paths fig-alt="A TinyTorch Tensor at the top branches to four routes: NumPy, C++ through ctypes, PyTorch MPS, and Triton. NumPy and C++ run on the CPU, MPS on the Apple GPU, and Triton on an NVIDIA GPU. The two GPU routes copy data in and out. Each extension falls back to NumPy."}
...
Each CPU core has vector registers that hold several numbers and apply one instruction to all of them. On the ARM cores measured here a NEON register holds four `float32` values, and one fused multiply-add instruction updates all four. A C++ compiler emits these instructions without being asked when a loop's iterations are independent and walk memory in order. The kernel's innermost statement, `c_row[j] += a_ik * b_row[j]`, is written for exactly that. It walks two rows from left to right with one fixed scalar `a_ik`. Compiling the same file with the vectorizer switched off isolates the effect, which comes to a factor of 3.4 (@tbl-extensions-gemm), close to the four lanes of the register.
...
1. **Turn on the cores.** Install an OpenMP runtime (`brew install libomp` on macOS; GCC on Linux includes one), delete `libtinytorch_simd.so`, and call `simd_build_info()` to confirm the thread count. Time `simd_matmul` at $n = 1024$ with `OMP_NUM_THREADS` set to 1, 2, 4, and the number of cores. Predict the speedup before you run it, then explain why it falls short of linear.

2. **Tile the registers.** Rewrite the inner loops of `tinytorch_cpp_gemm` so that each step computes a 4 × 16 block of `C` held in local variables for the whole `k` tile, and store it once at the end. Count the loads and stores per multiply-add before and after the change, then measure. Keep the regression tests passing.

3. **Measure the vector tangent.** Build the library with and without `-fveclib=Accelerate` (on Linux, find the GCC or Clang flags that give a vector `tanh`) and time `simd_fused_bias_gelu` at three array sizes. Then time `x + bias` alone at the same sizes and decide, from your own numbers, whether the fused kernel is limited by memory or by arithmetic.

4. **Give it a gradient.** Write the derivative of bias-plus-GELU as a second C++ kernel, wrap both kernels in a `Function` from @sec-autograd, and check the gradient against a finite-difference estimate. State which values the backward kernel needs from the forward pass, and whether it must recompute them.

5. **Find the break-even point.** With PyTorch installed, time `mps_matmul`, or `triton_fused_gelu` on an NVIDIA GPU, against NumPy for sizes from 64 to 4096, with and without the copies. Plot both curves, mark where they cross, and compare the crossing with the napkin math.
