...
Part II built two architectures on the Part I engine. Convolution shares one small filter across every position of an image. Attention lets every position of a sequence decide, from the data, which other positions to read. This milestone puts each to work, measures what it bought, and then turns the transformer into a text generator.

It runs in three parts.

1. **A convolutional network on TinyDigits.** The course's milestone 04 trains a LeNet-style network on the 8×8 digits that @sec-milestone-1's MLP learned. We compare the two networks at the same training budget and measure what convolution costs in this implementation.
2. **Attention on structured sequence tasks.** The course's milestone 05 trains a small attention model to reverse a sequence, copy it, or pick between the two from a prefix token. Each task has exactly one right answer, so the score is exact.
3. **Generating text with TinyGPT.** Open-ended generation has many acceptable answers. It needs the next-token objective, a sampling loop, and the temperature and top-$k$ controls that shape the loop's choices. This is the model Part III profiles, compresses, and caches.

Parts one and two run with `tito milestone run 04` and `tito milestone run 05`. Part three uses @sec-transformers's `GPT`, which this book calls TinyGPT, and its training and sampling code appears in this chapter.
...
Scoring uses `np.argmax`, and here the argmax is exactly right. Reversal has one correct output, so the most probable token is the answer and sampling would only add errors. The rest of this chapter is about the opposite case: open-ended generation, where many continuations are acceptable and always taking the argmax is the first thing that goes wrong.
...
1. Rerun @tbl-m2-cnn-vs-mlp's comparison with one change: shrink the MLP's hidden layer until its parameter count brackets the CNN's 810 (ten and eleven hidden units give 760 and 835). Predict each network's 100-epoch accuracy before training. Then compute the first-layer weights each design would need on 32×32 color input, and say at what input size you would expect the CNN to lead.

2. In the attention script, skip the positional encoding in `AttentionTransformer.forward` so the model sees letters but no positions. Predict which of the three tasks can still pass and which cannot, and justify the prediction from how self-attention treats a permutation of its inputs. Then run it.

3. Run `shaped_probs` from the worked trace at $T = 0.1$ and $T = 10$ on the same logits and confirm the two limits: the first column should be within 0.01 percent of $[100, 0, 0, 0, 0]$ and the second flatter than the $T = 1$ distribution. Continue increasing $T$ until every probability is within one percentage point of uniform. At what $T$ does token A first drop below 50 percent?

4. `sample_next_token` filters by top-$k$ before scaling by temperature. Swap the order in a copy and show, on the logits from @tbl-sampling-shaping-trace with positive $T$, that both probability vectors agree within floating-point tolerance. Then explain in one sentence why the two orders could differ if `top_k` selected by probability threshold rather than by count.

5. Modify `make_windows` to stride by `seq_len` instead of by one, so that windows do not overlap. For a stream of 1,000 tokens and `seq_len=32`, how many training windows does each version produce, and how many times does each token appear as a target under each? Compare both with sampling windows at random offsets from one stored token stream. Which scheme allocates duplicated token arrays, and how does each choose which contexts receive training?
