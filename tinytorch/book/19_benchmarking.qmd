...
From a framework and systems standpoint, a trustworthy benchmarking harness needs a few controls:
1. **Warmup**: Running untimed iterations first, so one-time costs such as cold caches, first allocations, and (in runtimes that have one) just-in-time compilation are not counted as steady-state latency.
2. **High-resolution monotonic timing**: Measuring with a monotonic clock (`time.perf_counter`)[^fn-perf-counter-monotonic] and, on asynchronous hardware, waiting for queued work to finish before reading it.
3. **Statistical distribution reporting**: Reporting the spread as well as the mean, with standard deviation and latency percentiles (P50, P90, P99), alongside throughput in samples per second.

[^fn-perf-counter-monotonic]: **Monotonic clocks versus wall-clock time**: Standard Python `time.time()` reads the operating system's calendar wall clock, which is subject to Network Time Protocol (NTP) slews, clock synchronization jumps, and leap seconds; an NTP correction during a benchmark run can yield negative or falsely inflated durations. In contrast, `time.perf_counter()` accesses a monotonic hardware counter (such as the invariant Time Stamp Counter `RDTSC` on x86-64 or `CNTVCT_EL0` on ARM64) that is guaranteed never to move backward and offers nanosecond-level resolution undisturbed by system clock changes.
