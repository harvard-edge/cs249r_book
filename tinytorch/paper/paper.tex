...
\begin{abstract}
Machine learning systems engineering requires understanding the internals of machine learning frameworks, including why optimizers consume memory, when complexity becomes prohibitive, and how to navigate trade-offs among accuracy, latency, and memory. Yet current machine learning education separates algorithms from systems. Students learn gradient descent without measuring memory and attention mechanisms without profiling computational cost. This divide leaves graduates unable to debug production failures, widening the gap between machine learning research and real-world deployment. We present TinyTorch, a twenty-module curriculum in which students implement the core components of PyTorch, including tensors, automatic differentiation, optimizers, and neural networks, entirely in pure Python. The curriculum is organized around three pedagogical patterns that address this gap. First, progressive disclosure gradually reveals complexity over time. For example, gradient tracking exists from Module 01 but is only activated in Module 06. Second, systems-first integration embeds memory profiling and systems reasoning from the start rather than treating them as advanced topics. Third, historical milestone validation guides students to recreate major machine learning breakthroughs, from the Perceptron in 1958 to modern Transformers, using exclusively student-built code. The curriculum requires no GPU, only 4 GB of RAM, and a standard laptop, making machine learning education globally accessible. The goal is to prepare the next generation of \emph{AI engineers}, practitioners who understand not only what machine learning systems do, but why they work and how to make them scale. The complete open-source infrastructure is available at \texttt{mlsysbook.ai/tinytorch}.
\end{abstract}
