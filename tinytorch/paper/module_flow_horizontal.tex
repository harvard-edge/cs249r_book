...
\node[optimization, below right=0.4cm and 0.6cm of Prof] (Memo) {17 Memo.};
\node[optimization, right=0.4cm of Memo] (Accel) {18 Accel.};
...
\caption{\textbf{Module Dependency Graph.} TinyTorch's 20 modules form a directed acyclic graph with two architectural paths. Foundation modules (blue, M01--07) build core infrastructure sequentially. At DataLoader (M08), students choose: the \emph{Vision} path (M09) builds CNNs for spatial processing; the \emph{Language} path (M10--13) builds tokenization through transformers. Both paths converge at Profiling (M14), then branch into parallel optimization tracks---\emph{Size} (quantization, compression) and \emph{Speed} (memoization, acceleration)---before final convergence at Benchmarking (M19) and Capstone (M20).}
