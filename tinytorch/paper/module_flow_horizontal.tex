...
\draw[arr] (Bench) -- (Cap);
    ...
    % === TIER LABELS ===
    % Define a common height relative to the Language path (which is the lowest consistent baseline)
    % Architecture Label - Centered over Architecture section
    \node[font=\scriptsize\bfseries\sffamily, purple!60!black] (ArchLabel) at ($(Tok)!0.5!(Trans) + (0, 2.5)$) {ARCHITECTURE (09-13)};
    
    % Foundation Label - Aligned with ArchLabel, centered over Foundation section
    \node[font=\scriptsize\bfseries\sffamily, blue!60!black] at (ArchLabel -| $(T)!0.5!(Loss)$) {FOUNDATION (01-08)};
    
    % Optimization Label - Aligned with ArchLabel, centered over Optimization section
    \node[font=\scriptsize\bfseries\sffamily, orange!60!black] at (ArchLabel -| $(Prof)!0.5!(Bench)$) {OPTIMIZATION (14-19)};
