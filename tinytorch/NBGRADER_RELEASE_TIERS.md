...
| Module | Evidence of understanding | Student core strips | Keep as scaffold | Challenge direction |
|---|---|---|---|---|
| 09 Convolutions | Spatial layers compute feature maps with kernels, padding, pooling, and channel layout. | `conv2d-class`: output shape, padding, convolution loops, forward; `maxpool2d-class`: output shape, pool loops, forward | AvgPool after MaxPool, BatchNorm as scaffold unless explicitly taught, spatial analysis, `simple-cnn` glue | vectorized convolution (im2col is built in Module 17), cache-friendly layout |
...
| 17 Acceleration | Runtime optimization improves arithmetic intensity and reduces memory traffic. | `vectorized-matmul`, `fused-gelu`, `tiled-matmul`, `im2col`, `im2col-conv2d`, `col2im`, `im2col-conv2d-function` | `unfused-gelu` baseline, analysis helpers, profiler demo | tile tuning, cache behavior, fusion benchmarks, recomputing cols in backward |
