...
|---|---|---|---|---|
| 01 Tensor | Tensors hold data, transform shapes, and implement array math. | `tensor-class`: `__init__`; `ops-arithmetic`: `Add.forward`; `ops-matmul`: `MatMul.forward` | imports, representation helpers, memory helpers, simple properties | memory layout, broadcasting edge cases, operation fusion |
| 02 Activations | Nonlinearities transform tensors and require numerical care. | `sigmoid-impl`, `relu-impl`, `softmax-impl` (all `forward`) | setup, `tanh-impl`, `gelu-impl` after one or two patterns are learned | stable approximations, vectorized kernels, fused activation paths |
| 03 Layers | Layers own parameters and define reusable forward transforms. | `linear-layer`: `forward`; `dropout-layer`: `forward` | imports, `layer-base`, repr methods, analysis helpers | initialization experiments, dropout variants, memory/perf analysis |
| 04 Losses | Losses define the training objective and must be numerically stable. | `mse-loss`: `forward`; `cross-entropy-loss`: `forward` | setup, `binary-cross-entropy-loss` as a related pattern, analysis helpers | stability stress tests, reduction modes, mixed precision behavior |
| 05 DataLoader | Data pipelines batch, shuffle, and collate samples into training-ready tensors. | `dataloader-implementation`: `__iter__` | abstract `dataset-implementation`, most augmentation helpers, integration demo | augmentation policy, streaming, prefetching, memory pressure |
| 06 Autograd | Reverse-mode AD propagates gradients through a computation graph. | `matmul-backward`, `relu-backward`, `mse-backward` (all `backward`) | repeated backward operators after the pattern is learned, `function-base`, `enable-autograd`, no-grad context, helper glue | graph memory, checkpointing, more complete operator coverage |
| 07 Optimizers | Optimizers turn gradients into parameter updates with state. | `sgd-optimizer`: `step`; `adam-update-moments`: `_update_moments`; `adam-step`: `step` | AdamW variants after Adam is learned, analysis helpers | optimizer comparisons, scheduler interactions, convergence diagnostics |
| 08 Training | Training loops orchestrate batches, loss, backward pass, optimizer updates, and validation. | `trainer-train-epoch`: `train_epoch` | `trainer-init`, checkpoint save/load, complete integration example | gradient accumulation strategies, fault tolerance, throughput tuning |

### Architecture Tier

| Module | Evidence of understanding | Student core strips | Keep as scaffold | Challenge direction |
|---|---|---|---|---|
| 09 Convolutions | Spatial layers compute feature maps with kernels, padding, pooling, and channel layout. | `conv2d-class`: `_convolve_loops` and `forward` | AvgPool after MaxPool, BatchNorm as scaffold unless explicitly taught, spatial analysis, `simple-cnn` glue | vectorized convolution (im2col is built in Module 17), cache-friendly layout |
| 10 Tokenization | Text becomes stable integer sequences through vocabulary and merge rules. | `bpe-count-pairs`: `_count_byte_pairs`; `bpe-merge-pair`: `_merge_pair` | base tokenizer interface, decode variants, tokenization utility wrappers | vocabulary trade-offs, compression ratios, tokenizer benchmarks |
| 11 Embeddings | Discrete IDs become learned vectors plus positional structure. | `embedding-backward`: `backward`; `embedding-init`: `forward`; `posenc-sinusoidal-table`: `_compute_sinusoidal_table` | repr/parameter boilerplate, helper constructor, repeated positional wrapper logic | sparse update efficiency, memory footprint, tied embeddings |
| 12 Attention | Attention computes scores, masks them, normalizes them, and composes values across heads. | `attn-scaled-dot-product`: `scaled_dot_product_attention`; `multihead-attention`: `forward` | imports and parameters boilerplate | attention memory, causal masking variants, fused QKV paths |
| 13 Transformers | Transformers compose normalization, attention, MLPs, residuals, and generation. | `layer-norm`: `forward`; `transformer-block`: `forward` | constructor boilerplate, analysis helpers, integration demo | KV-cache integration, generation speed, parameter scaling experiments |
...
| Module | Evidence of understanding | Student core strips | Keep as scaffold | Challenge direction |
|---|---|---|---|---|
| 14 Profiling | Performance work starts with measurement, not guessing. | `count-linear-flops`; `arithmetic-intensity`; `analyze-bottleneck` | quick profile helper, weight distribution helper, advanced report text | profiler overhead reduction, more accurate memory models, hardware counters |
| 15 Quantization | Precision reduction trades accuracy, memory, and speed through scale/zero-point calibration. | `quantize_int8`; `dequantize_int8`; `quantized_linear` (`calibrate`, `forward`, `memory_usage`); `measure_layer_bytes`; `analyze_model_sizes` | model traversal helpers, export wrapper, reflection prompts with stripped answers | per-channel quantization, dynamic quantization, accuracy-speed frontier |
| 16 Compression | Model size can be reduced through sparsity, rank, and distillation. | `magnitude-prune`: `magnitude_prune` | `structured-prune`, `compress-model-comprehensive`, profiler demo | structured sparsity kernels, pruning schedules, accuracy recovery |
| 17 Acceleration | Runtime optimization improves arithmetic intensity and reduces memory traffic. | `fused-gelu`; `tiled-matmul`; `im2col`; `im2col-conv2d`; `col2im`; `im2col-conv2d-function` (`forward`, `backward`) | `unfused-gelu` baseline, analysis helpers, profiler demo | tile tuning, cache behavior, fusion benchmarks, recomputing cols in backward |
| 18 Memoization | KV-cache trades memory for faster autoregressive generation. | `kvcache-class`: `update` and `get` | wrapper integration in `kv-enable-cache`, analysis prompts | eviction policies, batching, long-context memory management |
| 19 Benchmarking | Optimization claims need reproducible latency, accuracy, memory, and comparison metrics. | `timer-context`: `precise_timer`; `benchmark-latency`: `run_latency_benchmark`; `pareto-frontier`: `pareto_frontier` | plotting, report formatting, MLPerf-style compliance boilerplate | statistical rigor, leaderboard scoring, energy estimation |
| 20 Capstone | Students package a working system and measure improvements. | `toy-model`: `SimpleMLP.__init__` and `SimpleMLP.forward` | example workflow, schema glue | optimization workflow, leaderboard track, final competition tasks |
