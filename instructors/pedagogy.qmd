...
- **The Science:** When a student predicts "10$\times$ GPU upgrade = 10$\times$ speedup" and the instruments reveal only 3.5$\times$, that prediction error signal primes the brain to update its mental model of Amdahl's Law.
...
- **Part A (Calibration):** Single-variable interactions. Students learn to isolate one physical concept (e.g., just the $D_{\text{vol}}$ term of the Iron Law). Heavy scaffolding, narrow parameter ranges.
...
**Part B → C Transition:** When students finish Part B, say: "Now you know the tradeoffs. Here's a real constraint: 50 ms latency SLA, 256 KB memory budget. Design the best system you can."
:::
...
- **The Standard:** Instead of "I made it smaller," students must write: "I reduced the $D_{\text{vol}}$ term via INT4 quantization to fit within the 256 KB SRAM constraint, accepting a 1.2% accuracy reduction."
...
The **Iron Law of ML Systems** ($T \approx \frac{D_{\text{vol}}}{\text{BW}} + \frac{O}{R_{\text{peak}} \cdot \eta} + L_{\text{lat}}$) is the analytical backbone of the entire curriculum.
...
::: {.callout-tip}
## Discussion Prompt
Use this prompt after any optimization lab: "We just saw a 3.5$\times$ speedup from [technique]. Which Iron Law term changed? Did any other term get worse? Would this technique work on a different hardware platform? Why or why not?"
:::
...
| Student finishes early | "Can you find a configuration that is 2$\times$ better than your current best? What's the theoretical limit?" |
