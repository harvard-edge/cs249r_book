...
% Title moved down to avoid clutter with neural nodes - increased size
\node[anchor=center,font={\fontsize{42pt}{46}\selectfont\bfseries}] at ([xshift=12cm,yshift=-15cm]current page.north west) {\color{crimson}\MakeUppercase{#1}};
% Simple elegant accent line below title - moved lower
\draw[crimson,opacity=0.6,line width=1pt] ([xshift=8cm,yshift=-18.5cm]current page.north west) -- ([xshift=16cm,yshift=-18.5cm]current page.north west);
% Subtitle/description below the line
\node[anchor=center,font={\fontsize{12pt}{14}\selectfont}] at ([xshift=12cm,yshift=-20cm]current page.north west) {\color{crimson!70}Introduction • Context • Foundation};
