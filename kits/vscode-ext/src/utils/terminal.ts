...
const TERMINAL_NAME = 'Kits';
const STATE_HISTORY_KEY = 'kits.commandHistory';
...
/** Run a command in the dedicated Kits terminal */
...
export function rerunLastCommand(): void {
  ...
  if (!last) {
    vscode.window.showWarningMessage('Kits: no previous command to rerun.');
    return;
  }
  rerunCommand(last);
}

/** Show the Kits terminal */
