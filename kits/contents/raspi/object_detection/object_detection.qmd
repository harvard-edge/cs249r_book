...
Once we have defined our Machine Learning project goal, the next and most crucial step is collecting the dataset. We can use a phone, the Raspi, or a mix to create the raw dataset (with no labels). Let's use the simple web app on our Raspberry Pi to view the `QVGA ($320{\times}240$)` captured images in a browser.
...
- **Pre-processing** consists of resizing the individual images. In our case, the images were pre-processed on Roboflow, to $320{\times}320$, so let's keep it.  The resize will not matter here because the images are already squared. If you upload a rectangular image, squash it (squared form, without cropping). Afterward, you could define if the images are converted from RGB to Grayscale or not.
...
def visualize_detections(
    ...
    # Set the figure size to $12{\times}8$ inches
