...
Note that the trained model (an Int8 MobileNet V2 with a size of 320KB) can be downloaded for further use or even analysis, for example, using [Netron](https://github.com/lutzroeder/netron). Note that the model uses images of size $224{\times}224{\times}3$ as its Input Tensor. In the next step, we will use different hyperparameters on the Edge Impulse Studio.
...
Camera → JPEG Image → RGB888 Conversion → Resize to $96{\times}96$ →
...
- **Frame Buffer Size**: Defines the raw image size ($320{\times}240$ pixels)
...
3. Resizes the image to match the model's input size ($96{\times}96$ pixels)
