...
![Curriculum progression and causal dependency spine of the Stochastic Computer across the four-volume ML systems series. Volume III treats the foundation model as an unprivileged, stochastic coprocessor operating under zero ambient authority ($A=0$), organizing the runtime into an accountable computer that progresses causally from a single model invocation to distributed multi-agent fleet operations.](books/vol3/18_conclusion/images/svg/mlsys_curriculum_arc.svg){#fig-subsystem-dependency-tree width="100%"}
