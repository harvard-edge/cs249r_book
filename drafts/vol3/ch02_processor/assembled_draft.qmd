...
Crucially, the neural core does not emit human-readable text or discrete token IDs directly. Instead, its final unembedding layer projects internal hidden activation vectors $\mathbf{h} \in \mathbb{R}^{d_{\text{model}}}$ into raw, unnormalized *logit vectors* $\mathbf{z} \in \mathbb{R}^{|\mathcal{V}|}$, where $|\mathcal{V}|$ represents the total vocabulary cardinality (often ranging from $32{,}000$ to over $128{,}000$ tokens). A logit is an unscaled real number representing the model's raw affinity for each possible vocabulary token. Only when the inference service daemon applies softmax normalization does this vector map onto the *probability simplex* $\Delta^{|\mathcal{V}|-1}$—a formal mathematical space where every token probability is non-negative and all probabilities sum to exactly one:
