...
Crucially, each probe is executed as an independent atomic invocation ($H=1$); orchestrating multiple probes across an iterative trajectory is the domain of Chapter 3 ("Inference-Time Deliberation"). Under equal total compute and token budgets, decomposed probes bound prefill overhead $\Delta M$, minimize decode step counts $\Delta K$, and eliminate the catastrophic truncation failure mode of full-file re-emission, maximizing verified task yield.

::: {.callout-note title="Worked Example: Sizing the Token Overhead Tax Across Edit Paradigms"}

**Problem**:
Consider an agent runtime tasked with applying a 5-line bug fix to a 500-line Python module (`parser.py`). The runtime executes on an unquantized 70B parameter model (`Meta-Llama-3-70B-Instruct` in 16-bit BF16 precision, $|\Theta| = 140\text{ GB}$) served across a dual NVIDIA H100 SXM5 node ($TP=2$, aggregate memory bandwidth $\beta = 6{,}700\text{ GB/s}$). Operating at a standard sustained memory bus efficiency of $\eta_{\text{bw}} = 0.80$, effective bandwidth is $\beta_{\text{eff}} = 6{,}700 \times 0.80 = 5{,}360\text{ GB/s}$. Shuttling the $140\text{ GB}$ parameter tensor across the bus yields an average single-step decode latency of $T_{\text{step}} = 140 / 5{,}360 \approx 26.1\text{ ms} \approx 26.0\text{ ms}$ (a single-stream throughput of approximately $38.5\text{ tokens/s}$).
...
- **Decode Latency ($T_{\text{decode}}$):**
  $$T_{\text{decode}} = 75\text{ tokens} \times 0.026\text{ s/token} \approx 1.95\text{ s} \approx 2.0\text{ s}$$
- **Accelerator Memory Traffic ($D_{\text{HBM}}$):**
  $$D_{\text{HBM}} = 75 \times 140\text{ GB} = 10{,}500\text{ GB} = 10.5\text{ TB}$$
- **SLA & Invariant Assessment:** Optimal operating point. The diff consumes less than $4\%$ of the token budget ($75 \ll 2{,}048$) and completes in approximately two seconds ($1.95\text{ s} \ll 60\text{ s}$). Compared to full-file re-emission, targeted search/replace achieves a **$33\times$ reduction in decode latency** ($65.0\text{ s} / 1.95\text{ s} \approx 33.3\times$) and eliminates **$339.5\text{ TB}$ of wasteful GPU memory bus traffic**.
