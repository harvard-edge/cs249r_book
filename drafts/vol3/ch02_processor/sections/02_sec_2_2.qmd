...
For $L$ transformer layers, Key-Value heads $n_{\text{kv}}$, and head dimension $d_{\text{head}}$, the total channel dimension per token is $H_{\text{kv}} = n_{\text{kv}} \cdot d_{\text{head}}$. In modern Grouped-Query Attention (GQA), $n_{\text{kv}} \ll n_{\text{heads}}$ (for example, 8 KV heads shared among 64 Query heads, slashing KV cache memory by $8\times$ compared to standard multi-head attention). Storing a Key vector $\mathbf{k} \in \mathbb{R}^{H_{\text{kv}}}$ and a Value vector $\mathbf{v} \in \mathbb{R}^{H_{\text{kv}}}$ per layer yields $2 \cdot L \cdot H_{\text{kv}}$ scalars. At scalar byte-width $b$ ($b=2$ for 16-bit BF16/FP16; $b=1$ for 8-bit FP8), the single-token memory footprint is:
