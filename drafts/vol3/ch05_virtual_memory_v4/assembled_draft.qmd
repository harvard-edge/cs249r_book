...
::: {.column-margin}
![Tensor geometry of key and value projections stored in accelerator high-bandwidth memory across transformer layers.](images/svg/fig-vol3-kv-geometry.svg){#fig-vol3-kv-geometry}
:::
...
::: {.column-margin}
![Contiguous Memory Fragmentation](images/svg/fig-vol3-kv-fragmentation.svg){#fig-vol3-kv-fragmentation}
...
::: {.column-margin}
![PagedAttention virtual block table mapping and copy-on-write branching.](images/svg/vol3-pagedattention-mapping.svg){#fig-vol3-pagedattention-mapping}
:::
...
![Break-even wait time regimes comparing resident retention, PCIe swapping, and recomputation across sequence length and tool pause duration](images/svg/fig-vol3-swapping-tradeoff.svg){#fig-vol3-swapping-tradeoff}
...
2. **Asynchronous DMA Despatch:** The runtime issues non-blocking copy calls (`cudaMemcpyAsync`) targeting a dedicated bidirectional CUDA copy stream. Because modern accelerator architectures provide independent copy engines that run concurrently with the primary tensor computation engines, this transfer proceeds in the background without stealing cycles from active decode batches running on adjacent streams.
