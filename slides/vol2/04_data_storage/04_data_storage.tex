...
\note{[2 min] Walk through objectives. Emphasize that this chapter is about the
invisible bottleneck: storage. Ask: ``Has anyone profiled a training job and found
the GPU waiting for data?''}
...
\note{[3 min] Open with the central tension. The previous chapters built the engine
(compute) and wired it (network). Without fuel (data), the engine is expensive sculpture.
Ask: ``If an H100 consumes data at 3.35 TB/s from HBM, how fast can a single NVMe feed it?''}
...
\note{[2 min] Introduce the running example that threads through every section.
These numbers will recur. Common error: students underestimate checkpoint volume.}
...
\note{[3 min] Key conceptual slide. A DBA moving to ML infra would get everything wrong.
Walk through each row. The critical inversion: ML needs throughput (GB/s), not IOPS.
Ask: ``Why does RAID 5 waste bandwidth for ML?''}
...
\note{[2 min] Metadata, not bandwidth, is the first bottleneck at scale.
One JPEG per image collapses the PFS metadata server.
The fix: aggregate into shards. Ask: ``Why do WebDataset and TFRecord exist?''}
...
\note{[3 min] The pyramid extends the classic memory hierarchy. Each tier trades bandwidth
for capacity. The 300,000x gap is the number to remember.
Walk through: HBM (fast, tiny, expensive) to Archive (slow, huge, cheap).}
...
\note{[3 min] Walk through the bandwidth ratios. Key insight: you cannot stream through
the hierarchy in real time. HBM empties in 50 ms but takes 400 ms to refill from DRAM.
Each tier must prefetch for the tier above it.}
...
\note{[2 min] HBM is the only tier where computation happens. Capacity is the constraint,
not bandwidth. 175B model needs 2 TB but one H100 has only 80 GB.
85--98\% of HBM is occupied; less than 15\% is prefetch buffer.}
...
\note{[2 min] Prediction exercise. Students estimate before reveal.
The 2,400x difference surprises most students.
Give 60 seconds to compute, then ask 2--3 answers.}
...
\note{[2 min] Reveal the answer. Text: 160 MB/s (trivial). Image: 393 GB/s
(needs a parallel file system). The 2,400x gap drives hierarchy design.
Text is volume-heavy but bandwidth-light.}
...
\note{[3 min] Walk through each variable. Key insight: none of the four levers is free.
Reducing GPUs costs throughput. Lowering utilization wastes hardware.
Smaller batches may hurt convergence. Slower iterations extend training.}
...
\note{[2 min] Diagnostic metric. Below 2\% = healthy. 2--10\% = marginal.
Above 10\% = clear bottleneck wasting compute budget.
The economic consequence: 5\% stall on 1000 GPUs wastes \$16.8K/week.}
...
\note{[3 min] Show how pipelining overlaps CPU loading with GPU compute.
Key caveat: pipelining hides average latency, not tail latency.
Buffer depth must absorb P99, not P50. Ask: ``What happens during a P99.9 spike?''}
...
\note{[1 min] Answer: HBM 3.35 TB/s vs NVMe 7 GB/s; pipelining/prefetching.}
...
\vspace{0.6cm}
{\large What is the 500$\\times$ storage-compute gap,\\and what technique hides it?}
...
\note{[3 min] Give students 90 seconds. Answer: Buffer depth = ceil(250/200) = 2 batches.
But for P99 (500 ms), depth = ceil(500/200) = 3 batches.
The P99 answer is the correct engineering answer.}
...
\note{[3 min] Traditional path: 3 hops, CPU involved at every step. GDS: 1 hop,
direct DMA from NVMe to GPU memory. Walk through both paths in the diagram.
Key benefit: frees 6 CPU cores for augmentation.}
...
\note{[2 min] Walk through the worked example. 64,000 images/sec total.
Traditional: 8 cores consumed by data copying. GDS: 2 cores.
Caveat: GDS only helps local/RDMA NVMe, not network FS or object storage.}
...
\note{[3 min] The visible cost (per-GB storage price) is the tip. Below the waterline:
egress fees, IOPS charges, and most importantly idle GPU cost.
A 100 TB dataset: storage is only 7\% of total data delivery cost.}
...
\note{[2 min] Walk through the egress tax calculation step by step.
Local NVMe caching saves \$282K/year. Break-even: 2 epochs.
If short: just show the bottom line comparison.}
...
\note{[2 min] Four tiers mapped to access patterns. Automated lifecycle policies
move data between tiers. Manual tiering leads to over- or under-provisioning.}
...
\note{[3 min] Turn-and-talk. No single right answer. The point: tiering depends on
access frequency. Single-epoch LLM training may stream; multi-epoch vision training
must cache. Cold-call 2--3 pairs.}
...
\note{[3 min] Checkpoints are the most demanding write workload. All 256 nodes write
simultaneously, creating a burst. The checkpoint storm can saturate PFS bandwidth
for minutes. Key concept: stage to NVMe, replicate async.}
...
\note{[2 min] Trace data through its full lifecycle. Training data: Object $\to$ NVMe
$\to$ DRAM $\to$ HBM. Checkpoints: HBM $\to$ NVMe $\to$ PFS $\to$ Object.
Every copy consumes bandwidth. Minimize copies on the critical path.}
...
The NVMe fallacy is the most common: 7 GB/s sounds fast but is 500x slower than HBM.}
...
\note{[2 min] Operational pitfalls. The egress cost pitfall is the most expensive.
If short: cover the first two.}
...
500x gap, 300,000x hierarchy span, 4.5 PB checkpoints, \$282K savings.}
...
\note{[1 min] Point students to key resources.}
...
\note{[1 min] Forward hook. Infrastructure is complete: compute, network, storage.
The next challenge is algorithmic: how to split a single training job across thousands
of resources. Data, tensor, pipeline, and expert parallelism.}
