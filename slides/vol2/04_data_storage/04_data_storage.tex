...
\begin{columns}[T]
  \begin{column}{0.55\textwidth}
    ...
    \begin{itemize}\setlength\itemsep{0pt}
      ...
      \item \textcolor{datastroke}{\textbf{Storage}}: GB/s per drive; TB/s only after striping
    \end{itemize}
...
% -- NARRATE: Walk through the table. ``3 TB training data. 1,750 GB per checkpoint
% (350 GB FP16 weights + 1,400 GB Adam optimizer state = 10 bytes/param × 175B params).
But look at total checkpoint volume: ~7.6 PB over 30 days. That DWARFS the training
data by over 1,267x. The surprise: checkpoint I/O, not data loading, is the dominant
...
IF SHORT: State the ~7.6 PB checkpoint number and the ~1,267x ratio.
}
...
\begin{tabular}{@{}lr@{}}
  ...
  Single checkpoint (full)              & 1,750 GB \\
  All checkpoints (30 days, 10 min)     & $\sim$7.6 PB \\
  \bottomrule
\end{tabular}
}
...
\vspace{0.2cm}
\begin{mlsyscard}{errorstroke}
{\footnotesize \textbf{Surprise}: Checkpoint I/O ($\sim$7.6 PB) dwarfs training data reads (3 TB) by over 1,267$\times$. Checkpoint storage, not data loading, is the dominant storage workload for frontier models.}
\end{mlsyscard}
...
\note{
[0.5 min]
% -- NARRATE: Read the equation aloud. ``Required bandwidth equals number of GPUs
times target utilization times batch size in bytes, divided by iteration time.
Every lever costs something: fewer GPUs loses throughput, lower utilization wastes
hardware, smaller batches hurt convergence.''
% -- FLEX: [CORE] The pipeline equation is the central sizing formula of this chapter.
}
...
\mlsysfocus{The Prefetch Depth Equation}{%
$Q_{\min} = \left\lceil \dfrac{T_{\text{I/O, p99}}}{T_{\text{compute}}} \right\rceil$%
}
\note{
[1 min]
% -- LINK: Pipelining hides average latency; this equation names the engineering
formula for sizing the buffer depth to hide P99 tail latency.

% -- NARRATE: ``This is the chapter's second named formula. Q\_min is the minimum
number of batches that must be prefetched simultaneously. T\_IO\_p99 is the
99th-percentile I/O latency for one batch from whatever storage tier you are using.
T\_compute is the accelerator compute time per batch. If T\_IO\_p99 is 500 ms and
T\_compute is 200 ms, Q\_min = ceil(500/200) = 3 batches. If you only prefetch 2,
you WILL stall.''

% -- WARN: Students size for average latency, not P99. At 10,000 workers, P99.9
events happen every few minutes. Size for the tail.

% -- FLEX: [CORE] This formula is the design tool for every buffer-sizing decision.
}
...
\note{
[1 min]
% -- LINK: Pipeline equation + prefetch depth equation together give students two
named tools. The Quick Check tests the 500x gap and the technique that hides it.
...
Use the prefetch depth equation to size the buffer: Q\_min = ceil(T\_IO\_p99 / T\_compute).
...
% -- NARRATE: ``All 256 nodes write simultaneously --- a checkpoint storm. 1,750 GB
...
Emphasize: ``Checkpoint I/O (~7.6 PB) dwarfs training data I/O (3 TB per epoch)
by ~1,267x. For frontier models, checkpoint strategy has more impact on storage
design than training data pipeline optimization.''

% -- FLEX: [CORE] The complete data path closes the chapter's narrative arc.
IF SHORT: State the ~7.6 PB vs 3 TB contrast and the checkpoint dominance insight.
}
...
\begin{tabular}{@{}lll@{}}
  ...
  \textbf{Checkpoint} & HBM $\to$ NVMe (local, fast)              & 1,750 GB each \\
  \textbf{Replicate} & NVMe $\to$ PFS (async, background)         & 1,750 GB each \\
...
\vspace{0.15cm}
\begin{mlsyscard}{crimson}
{\scriptsize \textbf{Counterintuitive}: Checkpoint I/O ($\sim$7.6 PB) dwarfs training data I/O (3 TB per epoch). For frontier models, \textbf{checkpoint staging strategy} has more impact on storage design than training data pipeline optimization.}
\end{mlsyscard}
...
\begin{frame}{War Story: The Backup That Had Not Been Restored (2017)}
\note{
[2 min]
% -- LINK: Async checkpointing shows HOW to write checkpoints; this story shows
why you must also prove you can RESTORE them.

% -- NARRATE: ``January 31, 2017. A GitLab SRE working a database replication
incident accidentally wiped the production PostgreSQL data directory, believing
it was the secondary replica. GitLab depended on FIVE layered recovery paths:
S3 pg\_dump, LVM snapshots, Azure disk snapshots, PostgreSQL replication, and
WAL archiving. Every single one failed or was absent.'' Walk through each: ``S3
dumps: silently empty due to a PostgreSQL 9.2/9.6 version mismatch. Azure
snapshots: never enabled for database servers. Replication: the very procedure
that had broken. LVM snapshots: six hours stale. WAL archiving: never
implemented. Result: six hours of commits unrecoverable. GitLab livestreamed
the recovery to 5,000 concurrent viewers and restored from a stale staging
snapshot after 18 hours.''

% -- ENGAGE: ``What is the systems lesson for checkpoint design?''
Expected: Storage reliability is a RESTORE property, not a backup property.
Any checkpoint that has never been restore-tested does not count.

% -- WARN: Students (and teams) assume that writing a checkpoint means they
have a checkpoint. The GitLab incident proves otherwise: all five backup
systems existed but none had been validated end-to-end under incident conditions.
Correct framing: ``A checkpoint only counts if you can prove you can restore it.''

% -- FLEX: [CORE] Anchors the entire checkpoint section with a memorable
production failure. IF SHORT: State the core lesson in one sentence and move on.
}

\small
\begin{columns}[T]
  \begin{column}{0.55\textwidth}
    \textbf{Context}: GitLab.com, Jan 31, 2017. A database replication incident
    led to accidental deletion of the production PostgreSQL data directory.

    \vspace{0.1cm}
    \textbf{Five layered recovery paths --- all failed:}
    {\footnotesize
    \begin{itemize}\setlength\itemsep{0pt}
      \item S3 pg\_dump: silently empty (PostgreSQL version mismatch)
      \item Azure disk snapshots: never enabled for DB servers
      \item LVM snapshots: 6 hours stale
      \item WAL archiving: never implemented
      \item Replication: the broken procedure itself
    \end{itemize}
    }
  \end{column}
  \begin{column}{0.42\textwidth}
    \vspace{0.1cm}
    \begin{mlsyscard}{errorstroke}
    \textbf{Consequence}\\[0.05cm]
    {\footnotesize 6 hours of data unrecoverable.\\
    18 hours of downtime.\\
    5,000 live viewers on YouTube.\\[0.1cm]
    \textbf{Recovery}: stale staging snapshot.}
    \end{mlsyscard}

    \vspace{0.15cm}
    \mlsysalert{Systems lesson:}{Storage reliability is a \emph{restore} property, not a backup property. A checkpoint only counts if you can prove you can restore it.}
  \end{column}
\end{columns}
...
\note{
[1 min]
% -- NARRATE: First three operational mistakes. The egress pitfall is the most expensive:
``100 TB times 10 reads times \$0.09/GB = \$90,000 in egress, far exceeding
the \$27,600 annual storage cost. Stage locally.''
% -- FLEX: [CORE] IF SHORT: Cover the small files and egress pitfalls; skip checkpoint.
}

\small
\textbf{Pitfall:} \textit{Using millions of small files for training data.}\\
{\footnotesize Metadata operations serialize on the PFS metadata server. At 10,000 workers, metadata saturates in $<$1 sec. Aggregate into shards.}

\vspace{0.15cm}
\textbf{Pitfall:} \textit{Ignoring egress costs.}\\
{\footnotesize 100 TB $\times$ 10 reads $\times$ \$0.09/GB = \$90,000 in egress, far exceeding annual storage cost (\$27,600). Stage locally.}

\vspace{0.15cm}
\textbf{Pitfall:} \textit{Designing checkpoint storage for average write performance.}\\
{\footnotesize Checkpoints burst at 10$\times$ steady-state. PFS must be provisioned for peak, not average.}
...
\note{
[1 min]
% -- NARRATE: The accelerator upgrade pitfall connects to Ch2: ``Upgrading A100 to H100
without upgrading storage INCREASES the stall ratio.'' The versioning pitfall is the most
economically surprising: ``Teams create a full copy of a 10 TB dataset for every version.
Content-addressable storage (DVC, lakeFS) reduces per-version overhead to only what changed
--- typically 500 GB, a 20x reduction.''
% -- FLEX: [CORE] IF SHORT: State the accelerator audit rule and the versioning ratio.
}

\small
\textbf{Pitfall:} \textit{Assuming faster accelerators automatically improve throughput.}\\
{\footnotesize Upgrading A100 $\to$ H100 without upgrading storage \emph{increases} the data stall ratio. Every accelerator upgrade requires a storage audit.}

\vspace{0.15cm}
\textbf{Pitfall:} \textit{Full-copy-per-version dataset management.}\\
{\footnotesize Copying a 10 TB dataset for each experiment snapshot wastes 10 TB/version. Content-addressable tools (DVC, lakeFS) share unchanged files: $\sim$500 GB per version --- a 20$\times$ reduction.}
...
% -- NARRATE: ``Did you capture: 500x gap, 300,000x hierarchy span, ~7.6 PB checkpoints,
...
\begin{itemize}\setlength\itemsep{0pt}
  ...
  \item \textbf{Checkpoint staging}: Write NVMe, replicate async. 175B model: $\sim$7.6 PB checkpoints in 30 days.
\end{itemize}
...
\vspace{0.15cm}
{\footnotesize \textbf{175B model checkpoint}: 1,750 GB (350 GB weights + 1,400 GB Adam state). At 10-min intervals over 30 days: $\sim$7.6 PB total I/O.}
