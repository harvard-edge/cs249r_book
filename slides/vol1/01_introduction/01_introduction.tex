...
\centering
\safeimg[width=0.80\textwidth,height=4.5cm,keepaspectratio]{monitoring-dashboard.pdf}
...
% --- Layout: IMAGE TOP + table below ---
\centering
\safeimg[width=0.88\textwidth,height=3.8cm,keepaspectratio]{deployment-spectrum.pdf}
...
\vspace{0.1cm}
\mlsysalert{Gap:}{$\sim\!10^{5}\!\times$ memory, $\sim\!10^{6}\!\times$ compute. Each tier requires \textbf{fundamental redesign}.}
...
\centering
\safeimg[width=0.85\textwidth,height=3.5cm,keepaspectratio]{amdahls-pipeline.pdf}
...
\vspace{0.1cm}
\mlsysalert{Amdahl's Law:}{A 3$\times$ speedup on 35\% of the pipeline yields only 23\% end-to-end.}
