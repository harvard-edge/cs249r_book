...
\begin{columns}[T]
  \begin{column}{0.48\textwidth}
    \safeimg[width=\textwidth,height=5.5cm,keepaspectratio]{ai-eras-timeline.pdf}
  \end{column}
  \begin{column}{0.48\textwidth}
    \renewcommand{\arraystretch}{1.2}
    {\footnotesize
    \begin{tabular}{@{}lp{3.5cm}@{}}
      \toprule
      \textbf{Era} & \textbf{Bottleneck} \\
      \midrule
      Symbolic AI & Logic cannot capture ambiguity \\
      Expert Sys. & Knowledge acquisition does not scale \\
      Statistical ML & Feature engineering is manual \\
      Deep Learning & Compute and data scale \\
      \bottomrule
    \end{tabular}
    }
    \vspace{0.2cm}
...
\begin{columns}[T]
  \begin{column}{0.55\textwidth}
    ...
    \vspace{0.1cm}
    \begin{mlsyscard}{datastroke}
    {\footnotesize\textbf{AlexNet (2012):} CNNs' parallel ops aligned with GPU hardware. Error: 26.2\% $\to$ 15.3\% --- a 42\% relative improvement.}
    \end{mlsyscard}
  \end{column}
  \begin{column}{0.42\textwidth}
    \safeimg{nvidia-a100.png}
  \end{column}
\end{columns}
...
\begin{columns}[T]
  \begin{column}{0.48\textwidth}
    \safeimg[width=\textwidth,height=5.5cm,keepaspectratio]{iron-law-visual.pdf}
  \end{column}
  \begin{column}{0.48\textwidth}
    Each term resolves to \textbf{seconds}:

    \vspace{0.15cm}
    \textcolor{datastroke}{\textbf{Data}} $\dfrac{D_{\text{vol}}}{BW}$\\[0.05cm]
    {\scriptsize $\frac{\text{Bytes}}{\text{Bytes/s}} = \text{s}$ \quad Moving bits through memory}

    \vspace{0.1cm}
    \textcolor{computestroke}{\textbf{Compute}} $\dfrac{O}{R_{\text{peak}} \cdot \eta}$\\[0.05cm]
    {\scriptsize $\frac{\text{FLOPs}}{\text{FLOPs/s}} = \text{s}$ \quad Arithmetic ($\eta \leq 0.7$)}

    \vspace{0.1cm}
    \textcolor{errorstroke}{\textbf{Overhead}} $L_{\text{lat}}$\\[0.05cm]
    {\scriptsize Irreducible orchestration tax}

    \vspace{0.1cm}
    The \textbf{slowest term dominates}.
  \end{column}
\end{columns}
...
\begin{columns}[T]
  \begin{column}{0.48\textwidth}
    \safeimg[width=\textwidth,height=5.5cm,keepaspectratio]{silent-failure.pdf}
  \end{column}
  \begin{column}{0.48\textwidth}
    \textbf{Traditional software:}
    \begin{itemize}\setlength\itemsep{0pt}
      \item Crashes produce stack traces
      \item Deterministic: same input $\to$ same output
      \item Failure is \emph{loud}
    \end{itemize}

    \vspace{0.1cm}
    \textbf{ML systems:}
    \begin{itemize}\setlength\itemsep{0pt}
      \item Degrade 10--15\% over months
      \item Stochastic outputs
      \item Failure is \emph{silent}
    \end{itemize}
...
\begin{columns}[T]
  \begin{column}{0.48\textwidth}
    \safeimg[width=\textwidth,height=5.5cm,keepaspectratio]{monitoring-dashboard.pdf}
  \end{column}
  \begin{column}{0.48\textwidth}
    \textbf{Not a pipeline --- a loop:}

    \vspace{0.15cm}
    {\footnotesize
    \begin{enumerate}\setlength\itemsep{0pt}
      \item Collect \& curate data
      \item Train \& validate model
      \item Deploy to production
      \item Monitor for degradation
      \item \textcolor{errorstroke}{\textbf{Retrain}} when drift exceeds threshold
    \end{enumerate}
    }

    \vspace{0.15cm}
    \begin{mlsyscard}{routingstroke}
    {\footnotesize A/B testing in ML takes \textbf{4--6 weeks} (vs.\ 2--3 days for traditional software) because statistical significance requires sustained observation.}
    \end{mlsyscard}
  \end{column}
\end{columns}
...
\begin{columns}[T]
  \begin{column}{0.48\textwidth}
    \safeimg[width=\textwidth,height=5.5cm,keepaspectratio]{deployment-spectrum.pdf}
  \end{column}
  \begin{column}{0.48\textwidth}
    ...
    \vspace{0.15cm}
    \mlsysalert{Gap:}{$\sim\!10^{5}\!\times$ memory, $\sim\!10^{6}\!\times$ compute.}

    \vspace{0.05cm}
    Each tier requires a \textbf{fundamental redesign}.
  \end{column}
\end{columns}
...
\begin{columns}[T]
  \begin{column}{0.48\textwidth}
    \safeimg[width=\textwidth,height=5.5cm,keepaspectratio]{amdahls-pipeline.pdf}
  \end{column}
  \begin{column}{0.48\textwidth}
    \textbf{A team speeds up inference 3$\times$:}
    ...
    \begin{tabular}{@{}lrr@{}}
      ...
      Inference      & 45 ms & 15 ms \\
      Postprocessing & 25 ms & 25 ms \\
      \midrule
      \textbf{Total} & 130 ms & 100 ms \\
      \bottomrule
    \end{tabular}
    }

    \vspace{0.15cm}
    Expected: 67\% improvement\\
    Actual: \textbf{23\% improvement}
  \end{column}
\end{columns}
...
{\footnotesize Recommendation system: 85\% $\to$ 79.2\% in 6 months (Degradation Equation). Fraud detection: 5--10\% decay per quarter.}

\vspace{0.15cm}
\textbf{Fallacy:} \textit{ML systems can be verified by testing.}\\
{\footnotesize Input space: $256^{150{,}528}$ configurations. Test set: 50{,}000. The verification gap is effectively infinite.}
...
{\footnotesize A 3$\times$ inference speedup (45$\to$15 ms) yields only \textbf{23\%} end-to-end improvement when preprocessing (60 ms) and postprocessing (25 ms) dominate. \textbf{Amdahl's Law governs the whole pipeline.}}

\vspace{0.2cm}
\textbf{Pitfall:} \textit{Treating ML systems as traditional software that happens to include a model.}\\
{\footnotesize ML systems degrade 10--15\% over 3--6 months silently. Unit tests are insufficient; \textbf{monitoring infrastructure} is required from day one.}

\vspace{0.2cm}
\textbf{Pitfall:} \textit{Assuming ML expertise alone is sufficient.}\\
{\footnotesize Teams with strong ML but weak systems achieve only \textbf{10--20\%} of throughput targets. The five disciplines require integrated expertise.}
