...
\note{
% [2 min]
% -- LINK: This chapter opens with the statistic that 60--80\% of ML effort
% is data-related (computed from the chapter's own LEGO cell). This slide
% sets the learning contract: the chapter dives into that 60--80\% of effort.
%
...
\begin{enumerate}
  ...
  \item Implement \textbf{training-serving consistency} through idempotent transformations and \textbf{drift detection}
  \item Build \textbf{data labeling systems} balancing accuracy, throughput, cost, and quality control
  \item Evaluate \textbf{storage architectures} and file formats for ML workloads, including \textbf{feature store} concepts
...
\begin{frame}{Data Gravity: Move Compute, Not Data}
\note{
% [3 min]
% -- LINK: The Energy-Movement Invariant showed that moving bits costs
% 100--1,000$\times$ more than computing. Data Gravity extends that
% principle to between-datacenter movement: when datasets reach PB scale,
% the cost of physically moving the data exceeds the cost of running
% compute on it.
%
% -- NARRATE: ``The chapter defines data gravity as movement cost: a function
% of volume and bandwidth. A 1 PB dataset on a 10 Gbps link takes days of
% wire time and costs $\sim$\$90K in egress fees at cloud rates. That single
% number changes every architectural decision: instead of moving data to the
% compute, you move the compute to the data. This is why Lakehouse architectures
% (Spark on the storage node) exist and why cloud vendors colocate storage
% and compute in the same availability zone.''
%
% -- ENGAGE: ``If training the model takes 8 GPU-days and egress costs
% \$90K, which is faster and cheaper---moving the data or running the compute
% on-premise where the data already lives?'' Give 15 seconds.
% Expected: run compute where the data lives---moving the data is slower
% and more expensive. This is the engineering implication of data gravity.
%
% -- WARN: Students assume egress cost is a minor line item. Correct framing:
% at PB scale, egress fees dominate the budget. The wire cost is the cheap
% part---the engineering cost to re-validate pipelines, migrate schemas, and
% synchronize downstream services adds 6--9 months of human time on top.
% IF STUCK: ``1 PB = 1,000 GB. At \$0.09/GB egress, what is the bill?''
% (\$90,000).
%
% -- FLEX: [CORE] Named chapter principle with quantitative anchor.
% IF SHORT: State the \$90K/1 PB figure and the `move compute to data' rule.
}

\small
\begin{columns}[T]
  \begin{column}{0.52\textwidth}
    \textbf{Data Gravity = Movement Cost}

    \vspace{0.15cm}
    {\footnotesize
    A 1 PB dataset on a 10 Gbps link:
    \begin{itemize}\setlength\itemsep{0pt}
      \item \textbf{Wire time:} $\sim$days (physics, fixed)
      \item \textbf{Egress cost:} $\sim$\$90K (at \$0.09/GB)
      \item \textbf{Engineering time:} 6--9 months\\
      {\scriptsize (pipeline re-validation, schema migration, lineage)}
    \end{itemize}
    }

    \vspace{0.1cm}
    \begin{mlsyscard}{datastroke}
    {\footnotesize \textbf{Rule:} For PB-scale data, \textbf{move compute to data}.\\
    For GB-scale data, \textbf{move data to compute}.}
    \end{mlsyscard}
  \end{column}
  \begin{column}{0.44\textwidth}
    ...
    \begin{tabular}{@{}ll@{}}
      \toprule
      \textbf{Scale} & \textbf{Architecture} \\
      \midrule
      GB dataset & Move data to GPU \\
      TB dataset & Colocate storage/GPU \\
      PB dataset & Lakehouse: Spark on storage \\
      PB+ mesh  & Decentralized Data Mesh \\
      \bottomrule
    \end{tabular}
    }
    \vspace{0.1cm}
    \begin{mlsyscard}{errorstroke}
    {\footnotesize \textbf{Fallacy:} ``Moving a PB is just a bandwidth problem.''\\[0.05cm]
    Wire cost $\ll$ engineering cost.}
    \end{mlsyscard}
  \end{column}
\end{columns}
...
\note{
% [3 min]
% -- LINK: Data Gravity showed the between-datacenter movement cost.
% The Feeding Problem shows the within-datacenter I/O bottleneck that
% starves GPUs even when data is nearby.
%
...
\begin{columns}[T]
  \begin{column}{0.48\textwidth}
    \safeimg[width=\textwidth,height=4.8cm,keepaspectratio]{ch04-feeding-problem.pdf}
  \end{column}
  \begin{column}{0.48\textwidth}
    \textbf{The mismatch:}
    ...
    40K images/sec $\to$ needs 23 GB/s

    \vspace{0.08cm}
    \textcolor{errorstroke}{\textbf{Standard cloud disk}}\\
    250 MB/s actual throughput

    \vspace{0.1cm}
    \begin{mlsyscard}{errorstroke}
    {\footnotesize \textbf{Feeding Tax:} $>$98\% GPU idle time.\\[0.03cm]
    Iron Law: $T_{\text{step}} = \max(T_{\text{compute}}, T_{\text{io}})$\\[0.03cm]
    The \textcolor{datastroke}{Data Term} dominates.}
    \end{mlsyscard}
    \vspace{0.06cm}
    \begin{mlsyscard}{datastroke}
    {\footnotesize \textbf{Formal limit:}\\[0.03cm]
    $\text{Throughput} = \min(\text{Compute}, \text{Supply Rate})$}
    \end{mlsyscard}
  \end{column}
\end{columns}
...
\begin{frame}{Lighthouse: DLRM vs.\ KWS --- Scalability Pillar (1/2)}
\note{
% [2 min]
% -- LINK: The Four Pillars framework said every pipeline decision is
% evaluated against all four pillars simultaneously. This slide grounds
% the Scalability pillar in two concrete workload archetypes.
%
% -- NARRATE: ``The chapter designates DLRM as the scalability lighthouse.
% Recommendation systems have TB/PB-scale embedding tables---every user ID
% and product ID maps to a dense vector. The bottleneck is not compute; it
% is memory capacity (tables exceed single-GPU memory) and sparse-access
% bandwidth (billions of random lookups per second). Contrast with KWS: a
% 23 million sample corpus with a compute/quality bottleneck. Same Four
% Pillars framework, completely different binding constraint. Students who
% see only KWS will misdiagnose DLRM systems.''
%
% -- ENGAGE: ``A recommendation team wants to improve throughput. They buy
% more A100 GPUs. Does this help?'' Give 10 seconds.
% Expected: probably not---if the bottleneck is sparse memory access and
% embedding table size, more FLOPs do not solve it. They need sharding and
% higher-bandwidth memory.
%
% -- WARN: Students assume compute-bound is the universal bottleneck.
% Correct framing: at PB scale with high-cardinality categoricals, the
% constraint shifts to memory capacity and access pattern, not arithmetic
% intensity.
%
% -- FLEX: [OPTIONAL] Lighthouse anchor for the Scalability pillar.
% IF SHORT: State the contrast: KWS = compute/quality bound; DLRM =
% memory capacity/sparse-access bound. (30 seconds)
}

\scriptsize
\begin{center}
\small\textbf{Same Four Pillars. Different binding constraint.}
\end{center}
...
\begin{tabular}{@{}lp{3.5cm}p{3.5cm}@{}}
  \toprule
  & \textbf{KWS (our lighthouse)} & \textbf{DLRM (recommendation)} \\
  \midrule
  \textbf{Data scale} & 23M samples, 736 GB & Billion+ users/items, TB/PB \\
  \textbf{Pillar stress} & Quality + compute & Scalability = memory capacity \\
  \textbf{Bottleneck} & Labeling throughput & Sparse embedding lookups \\
  \textbf{Architecture} & Streaming pipeline & Embedding table sharding \\
  \textbf{Fix} & AI-assisted labeling & Higher-BW memory, sharding \\
  \bottomrule
\end{tabular}

\end{frame}

\begin{frame}{Lighthouse: DLRM vs.\ KWS --- Scalability Pillar (2/2)}
\note{
% [1 min]
% -- LINK: The comparison table showed the two archetypes side by side.
% This slide draws out the systems lesson.
%
% -- NARRATE: ``The key insight: same Four Pillars framework, completely
% different binding constraint. For DLRM, more GPUs do not help because
% the bottleneck is memory capacity and sparse-access bandwidth. This is
% why understanding the binding pillar before purchasing hardware matters.''
%
% -- ENGAGE: ``If the DLRM team buys higher-bandwidth HBM3 memory instead
% of more GPU FLOPs, which Iron Law term improves?''
% Expected: the Data Term ($D_{\text{vol}}/BW$)---higher BW reduces the
% sparse embedding lookup cost that is the binding constraint.
%
% -- FLEX: [OPTIONAL] Systems lesson for the Scalability pillar.
% IF SHORT: Read the insight aloud and move on. (20 seconds)
}

\centering
\vspace{0.5cm}
\mlsysinsight{Systems lesson:}{For recommendation systems, the Scalability pillar binds first---TB-scale embedding tables exceed single-GPU memory and stress sparse-access bandwidth, not compute.}

\vspace{0.4cm}
\begin{mlsyscard}{datastroke}
{\footnotesize \textbf{Diagnostic question:} When a team says ``our training is slow,'' ask \emph{which} Iron Law term dominates before recommending a hardware upgrade.}
\end{mlsyscard}
...
%
% -- NARRATE: ``Four misconceptions (the chapter lists four). First: `more
% data always helps'---beyond a threshold, test loss follows a power law.
% 10$\times$ more data often reduces error by less than 1 pp. Second:
% `moving a PB to the cloud is just a bandwidth problem'---the wire cost
% (3 weeks at 10 Gbps) is the cheap part; re-validating pipelines and
% migrating schemas takes 6--9 months of human time. This is data gravity
% in action. Third: `high training accuracy = production ready'---
% training-serving skew, drift, and coverage gaps cause 99\% validation
% accuracy to fail in deployment. Fourth: `synthetic replaces real
% data'---synthetic inherits generator biases.''
%
% -- FLEX: [CORE] Corrects the most common data engineering misconceptions.
% IF SHORT: Cover the first and third fallacies.
}

\footnotesize
\textbf{Fallacy:} \textit{More data always improves model performance.}\\
Beyond a threshold, test loss follows a power law: 10$\times$ more data often reduces error by $<$1 percentage point, while costs scale linearly. Smart selection outperforms naive accumulation.

\vspace{0.1cm}
\textbf{Fallacy:} \textit{Moving a petabyte to the cloud is just a bandwidth problem.}\\
Wire time at 10 Gbps: $\sim$3 weeks (physics). Engineering cost---pipeline re-validation, schema migration, lineage updates: \textbf{6--9 months} of human time. Data gravity makes compute-to-data the right architecture.

\vspace{0.1cm}
\textbf{Fallacy:} \textit{High training accuracy indicates production readiness.}\\
Training-serving skew, distribution drift, and coverage gaps cause 99\% validation accuracy to fail in deployment.

\vspace{0.1cm}
\textbf{Fallacy:} \textit{Synthetic data can fully replace real-world collection.}\\
Synthetic data inherits generator biases. A KWS system trained on synthesized speech fails on real accents and background noise. Combine real data for coverage with synthetic for scale.

\end{frame}

\begin{frame}{Pitfalls (1/2)}
\note{
% [1 min]
% -- LINK: Fallacies named misconceptions. Pitfalls name operational mistakes.
%
% -- NARRATE: ``First two pitfalls. Treating preprocessing as one-time---
% drift makes it continuous. Ignoring serialization cost---teams optimize
% GPU kernels while loading JSON/CSV; text formats are 10$\times$+ slower to
% decode than Parquet/Arrow, so the CPU becomes the bottleneck and the H100
% idles at single-digit utilization.''
%
% -- FLEX: [CORE] Operational lessons.
% IF SHORT: Name both pitfalls and continue.
}

\small
\textbf{Pitfall:} \textit{Treating data preprocessing as a one-time task.}\\[0.1cm]
{\footnotesize Data distributions drift continuously. A pipeline validated at launch degrades silently. Production systems require continuous monitoring (PSI, KL divergence) and automated retraining triggers.}

\vspace{0.3cm}
\textbf{Pitfall:} \textit{Ignoring data serialization cost.}\\[0.1cm]
{\footnotesize Teams optimize GPU kernels while loading JSON or CSV. Text formats are $\geq$10$\times$ slower to decode than Parquet/Arrow---the CPU becomes the bottleneck and an H100 idles at single-digit TFLOP/s. Fix: convert to Parquet once; every downstream job reads columnar.}

\end{frame}

\begin{frame}{Pitfalls (2/2)}
\note{
% [1 min]
% -- LINK: Two more operational pitfalls.
%
% -- NARRATE: ``Third: ignoring training-serving skew until deployment---
% feature computation differences are the leading cause of failures. Fourth:
% neglecting data versioning---without snapshots, debugging takes 2--4$\times$
% longer.''
%
% -- FLEX: [CORE] Operational lessons, continued.
% IF SHORT: Cover training-serving skew pitfall; versioning can be brief.
}

\small
\textbf{Pitfall:} \textit{Ignoring training-serving skew until deployment.}\\[0.1cm]
{\footnotesize Feature computation differences are the leading cause of ML deployment failures. Feature stores and consistency contracts should be architectural requirements from day one.}

\vspace{0.3cm}
\textbf{Pitfall:} \textit{Neglecting data versioning until model debugging requires it.}\\[0.1cm]
{\footnotesize Without versioned snapshots, teams cannot determine whether regression stems from labeling changes, schema errors, or genuine drift. Organizations that defer versioning report 2--4$\times$ longer debugging cycles.}
