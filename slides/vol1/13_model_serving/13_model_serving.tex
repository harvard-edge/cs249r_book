...
Key surprise: preprocessing (resize, normalize) takes 4.3 ms and CPU$\to$GPU
transfer takes 0.5 ms --- together nearly matching inference itself. The model
gets less than 50\% of the total budget.
...
\begin{tabular}{@{}llrr@{}}
  ...
  \textcolor{routingstroke}{Preprocess} & Resize, normalize, to-tensor & \textbf{4.3 ms} & \textbf{43\%} \\
  \textcolor{computestroke}{CPU$\to$GPU} & Transfer tensor via PCIe & 0.5 ms & 5\% \\
...
linearly with sequence length AND batch size. For Llama-70B with grouped-query
attention, each token needs about 0.3 MB of KV cache. At batch 32 and 8K context,
the cache is about 86 GB. Models without GQA can be several times larger.
Key number: always compute KV cache from layers, KV heads, head dimension, and precision.}
...
\begin{columns}[T]
  \begin{column}{0.52\textwidth}
    ...
    \begin{tabular}{@{}lrr@{}}
      \toprule
      \textbf{Scenario} & \textbf{KV Cache} & \textbf{vs.\ FP16 weights} \\
      \midrule
      Llama-70B, batch 1, 640 tok & 210 MB & 0.002$\times$ \\
      Llama-70B, batch 8, 8K tok & 21 GB & 0.15$\times$ \\
      Llama-70B, batch 32, 8K tok & \textbf{86 GB} & \textbf{0.6$\times$} \\
      \bottomrule
    \end{tabular}
    }

    \vspace{0.15cm}
    {\footnotesize\mlsysalert{OOM risk:}{At batch 32 + 8K context, KV cache alone can overflow an 80\,GB GPU once weights and runtime overhead are included. Without GQA, it can exceed the weights.}}
  \end{column}
...
\begin{frame}{War Story: The Inference Tax at Facebook (1/2)}
\note{[1 min]
% -- LINK: What prior concept connects to this slide
The previous slide showed that serving costs can drop 24$\times$ with the right
stack. This slide grounds that economic imperative in a real fleet at
billion-user scale, from the chapter's only formal war-story callout.

% -- NARRATE: What to SAY while showing this slide
``Kim Hazelwood's team at Facebook described a production ML system in 2018
that ran across every user-facing surface---News Feed, Ads, Search, image
understanding, face recognition. Inference reached tens of trillions of
operations per day. Training cost you once; serving cost you forever.
The key lesson: they had to co-design models, accelerators, memory, and
serving platforms together---not treat serving as an afterthought.''

% -- ENGAGE: Specific question, prediction, or task for THIS slide
Ask: ``If training a model costs \$1M once, but serving it costs
\$500K per month, what does the lifetime cost look like after two years?''
Give 20 seconds, then cold-call. [Expected: training is a rounding error
by month 6---serving dominates lifecycle cost.]

% -- WARN: What students will get wrong on THIS topic
Common error: students think ``if a model is good, deployment takes care
of itself.'' Correct framing: production scale forces co-design of model
architecture, hardware, and serving platform. You cannot optimize serving
in isolation.

% -- FLEX: [CORE] First half of war story.
IF SHORT: State the headline numbers (tens of trillions of ops/day) and
the systems lesson, then advance to the next slide.
}

\footnotesize
\textbf{Hazelwood et al. (2018): ML at Facebook's Fleet Scale}

\vspace{0.2cm}
\begin{mlsyscard}{errorstroke}
\textbf{Context}\\[0.08cm]
{\small News Feed, Ads, Search, image understanding, face recognition --- every user-facing surface.\\[0.05cm]
Inference: \textbf{tens of trillions of ops/day} under tail-latency targets.\\[0.05cm]
A one-hour-old ranking model measurably degraded News Feed quality.}
\end{mlsyscard}

\vspace{0.2cm}
\begin{mlsyscard}{datastroke}
\textbf{Systems lesson}\\[0.08cm]
{\small Training creates the model; serving pays the \textbf{recurring bill}. Co-design models, accelerators, memory systems, and serving platforms --- never treat serving as an afterthought.}
\end{mlsyscard}

\end{frame}

\begin{frame}{War Story: The Inference Tax at Facebook (2/2)}
\note{[1 min]
% -- LINK: What prior concept connects to this slide
The prior slide established the scale: tens of trillions of ops/day.
This slide shows that the cost pressure never relents --- intelligence
deflation makes serving efficiency a permanent business constraint.

% -- NARRATE: What to SAY while showing this slide
Point to the intelligence deflation card: ``Public API token prices have
collapsed from \$20 per million tokens in 2020 to under \$0.10 in
2025---a roughly 5.8$\times$ drop per 18-month period. Every efficiency
gain opens new applications (Jevons Paradox); the serving engineer's job
never ends.''

% -- ENGAGE: Specific question, prediction, or task for THIS slide
Ask: ``What does Jevons Paradox predict about AI demand as token prices
fall?'' [Expected: cheaper tokens increase total AI usage faster than
any single model's efficiency improves.]

% -- FLEX: [CORE] Ties serving economics to the intelligence deflation trend.
IF AHEAD: ``What specific model-architecture choice would you change to
reduce the inference tax?'' (Expected: smaller embeddings, quantization,
reduced request rate via caching.)
IF SHORT: State the headline numbers and advance.
}

\footnotesize
\textbf{Intelligence Deflation: The Cost Curve Never Stops}

\vspace{0.1cm}
\begin{mlsyscard}{routingstroke}
\textbf{Public API token prices (log scale)}\\[0.05cm]
{\footnotesize
\begin{itemize}\setlength\itemsep{0pt}
  \item 2020: \textbf{\$20} / M tokens (GPT-3)
  \item 2023: \textbf{\$2} / M tokens
  \item 2025: \textbf{$<$\$0.10} / M tokens
\end{itemize}
\vspace{0.03cm}
$\approx$\textbf{5.8$\times$ per 18 months} --- Every efficiency gain opens new applications (Jevons Paradox); the serving engineer's job never ends.}
\end{mlsyscard}

\vspace{0.1cm}
\mlsysalert{Economic law:}{Training is a one-time cost. Serving is the recurring bill. At fleet scale, serving efficiency determines financial viability.}
...
\vspace{0.08cm}
\textbf{Pitfall:} \textit{Cold start latency only matters for the first request.}\\
During traffic spikes, new instances must warm up simultaneously. TensorRT compilation alone takes 30--120 s per instance; 10 new instances warming in parallel means every request to cold replicas sees 10--100$\times$ steady-state latency precisely when traffic is highest.
...
\begin{itemize}\setlength\itemsep{0pt}
  ...
  \item \textbf{Serving is the recurring bill}: Training is a one-time cost; serving pays forever. Intelligence deflation ($\approx$5.8$\times$/18 months) means margins tighten continuously --- serving efficiency is a business imperative.
\end{itemize}
...
\mlsysref{Hazelwood+18}{Hazelwood et al. ``Applied Machine Learning at Facebook: A Datacenter Infrastructure Perspective.'' HPCA 2018.}
