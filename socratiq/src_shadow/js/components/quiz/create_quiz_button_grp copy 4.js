...
export function createButtonGroup(shadowRoot) {
    ...
    thumbsDownButton.addEventListener('click', async (e) => {
        ...
        const isCurrentlySelected = thumbsDownButton.dataset.selected === 'true';
        
        // Toggle selected state
        thumbsDownButton.dataset.selected = (!isCurrentlySelected).toString();
        ...
        try {
            // Send negative score if selecting, positive score if unselecting
            await submitScore(questionId, isCurrentlySelected, chapterId, sectionId, difficulty);
            
            // Show appropriate popover message
            if (!isCurrentlySelected) {
            ...
            thumbsDownButton.dataset.selected = isCurrentlySelected.toString();
            thumbsDownButton.classList.toggle('text-red-500');
