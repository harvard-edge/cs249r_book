---
title: "Object Detection with Nicla Vision and EI FOMO model"
format: 
  html: 
    self-contained: true
editor: visual
...
This is a continuation of **CV on Nicla Vision**, now developing **Object Detection** on microcontrollers.
...
![](Images_4_1/img_1.png){fig-align="center"}

But what happens when the cat jumps near to the wine glass? The model still only recognizes the predominant category on the image, the tabby cat:

![](Images_4_1/img_2.png){fig-align="center"}

And what happens if there is not a dominant category on the image?

![](Images_4_1/img_3.png){fig-align="center"}

The model identifies the above image as completely wrong (an "ashcan" may be due to the color tonalities). The model used in all previous examples is the Mobilinet, trained with a large dataset, the Imagenet.
...
![](Images_4_1/img_4.png){fig-align="center"}
...
In this Hands-On exercise, we will explore the use of FOMO with Object Detection and only enter a little detail about the model itself. To understand more about how the model works, you can go into the [official FOMO announcement](https://www.edgeimpulse.com/blog/announcing-fomo-faster-objects-more-objects) by Edge Impulse, where Louis Moreau and Mat Kelcey explain in detail how it works.
...
![](Images_4_1/proj_goal.png){fig-align="center" width="752"}
...
![](Images_4_1/samples.png){fig-align="center"}
...
We can use the Edge Impulse Studio, the OpenMV IDE, or even your phone or other device for the image capture. Here, we will use again the OpenMV IDE for that.
...
![](Images_4_1/data_folder.png){fig-align="center"}Edge impulse suggests that the objects should be of similar size and not overlap for better performance. This is OK in an industrial facility, where the camera should be fixed, keeping the same distance from the objects to be detected. Despite that, we will also try with mixed sizes and positions to see the result.
...
On your Project Dashboard, go down and on **Project info** and select **Boulding boxes (object detection)** and Nicla Vision as your Target Device:

![](Images_4_1/img_7.png)

On Studio, go to the `Data acquisition` tab, and on the `UPLOAD DATA` section, upload from your computer files captured.

![](Images_4_1/img_8.png)

> You can leave for the Studio to split your data automatically between Train and Test or do it manually.

![](Images_4_1/img_9.png)

All the individual not labeled images (51) were uploaded but still need to be labeled appropriately before use as a dataset in the project. The Studio has a tool for that purpose, which you can find in the link `Labeling queue (51)`.
...
Ordinary objects can quickly be identified and labeled using an existing library of pre-trained object detection models from YOLOv5 (trained with the COCO dataset). But once, in our case, the objects are not part of COCO datasets, we should select the option of `tracking objects`. With this option, drawing bounding boxes and labeling the images in one frame, the objects will be tracked from frame to frame, partially labeling the new ones.
...
![](Images_4_1/img_10.png)

Continuous with this process until the queue is empty. At the end, all images should have the objects labeled as those samples below:

![](Images_4_1/img_11.png)

Next, review the labeled samples on the `Data acquisition` tab. In the case you realize that one of the labels was wrong, you can edit it using the *three dots* menu after the sample name:

![](Images_4_1/img_12.png)

You will be guided to replace the wrong label, correcting the dataset.

![](Images_4_1/img_13.png)

## The Impulse Design

In this phase, we should define how to:

-   **Pre-processing** consists of resizing the individual images from `320 x 240` to `96 x 96` and squashing them (squared form, without crop)---Posteriorly, the images are converted from RGB to Grayscale.

-   **Design a Model,** in this case, "Object Detection."

![](Images_4_1/img_14.png)
...
![](Images_4_1/img_16.png)

The feature explorer shows that all samples show a good separation after the feature generation.
...
For validation during training, will be spared 20% of the dataset (*validation_dataset*). For the remaining 80% (*train_dataset*), we will apply Data Augmentation, which randomly will flip, change the size and brightness of the image, and crop them, artificially increasing the number of samples on the dataset for training.

As a result, the model ends with practically 1.00 in F1 score, with a similar result when using the Test data.

> Note that FOMO automatically added a 3rd label background to the two previously defined (*box* and *wheel*).

![](Images_4_1/img_19.png)
...
Once Edge Impulse officially supports the Nicla Vision, let's connect it to the Studio. For that, follow the steps:
...
![](images_2/media/image17.png){width="4.416666666666667in" height="1.5520833333333333in"}
...
![](Images_4_1/img_20.png)

Once connected, you can use the Nicla to capture actual images to be tested by the trained model on Edge Impulse Studio.

![](Images_4_1/img_21.png)

One thing to be noted is that the model can produce some false positives and negatives, which can be minimized by defining a proper Confidence Threshold (use the `Three dots` menu). Try with 0.8 or more.

## Deploying the Model

Select OpenMV Firmware on the Deploy Tab and press \[Build\].

![](Images_4_1/img_22.png)
...
![](Images_4_1/img_24.png)

You will find a ZIP file on your computer from the Studio. Open it:

![](Images_4_1/img_23.png)Load the .bin file to your board:

![](Images_4_1/img_25.png)
...
Before running the script, let's change a few lines. Note that you can leave the window definition as 240 x 240 and the camera capturing images in a QVGA/RGB. The captured image will be pre-processed by the FW deployed from Edge Impulse
...
![](Images_4_1/img_27.png)

Note that the frames per second rate is around 8 fps (similar to what we got with the Image Classification project). This happens because FOMO is cleverly built over a CNN model, not with an object detection model like the SSD MobileNet. For example, when running a MobileNetV2 SSD FPN-Lite 320x320 model on a Raspberry Pi 4, the latency is around 5 times higger (around 1.5 fps)
