...
(e.g. `python3 bindery/tools/audit/scan.py --scope vol1`).

Relationship to `./binder check refs`:

  The existing `binder check refs` (specifically the `--scope orphans`
  sub-scope, see bindery/cli/commands/validate.py `_run_unreferenced_labels`)
...
# matching the LABEL_REF_PATTERN convention in bindery/cli/commands/validate.py.
...
def scan_corpus(root: Path, scope: str = "both") -> list[Issue]:
    ...
    This is the entrypoint ``bindery/tools/audit/scan.py`` calls for corpus-wide
