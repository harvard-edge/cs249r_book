...
1. ``bindery/tools/audit/scan.py --scope vol1|vol2`` ledger counts
...
def main() -> None:
    ...
    for scope in ("vol1", "vol2"):
        ...
        rc, out = run(
            ["python3", "bindery/tools/audit/scan.py", "--scope", scope, "-v",
    ...
    for module in EXPECTED_SELF_TESTS:
        ...
        rc, out = run(
            ["python3", f"bindery/tools/audit/checks/{module}.py"],
            log,
            env={"PYTHONPATH": str(REPO / "bindery/tools")},
        )
