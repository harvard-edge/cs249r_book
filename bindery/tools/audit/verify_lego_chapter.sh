...
python3 bindery/tools/audit/chapter_html_verify.py "--$VOL" "$CH" 2>&1 | tee -a "$LOG"
./binder reset html 2>&1 | tee -a "$LOG"

python3 bindery/tools/audit/fmt/audit_lego_cells.py --chapter "$VOL/$CH" 2>&1 | tee -a "$LOG"
cp "$REPO/book/tools/audit/artifacts/lego_cells_verify_report.json" "$CELLS_SNAP"
python3 bindery/tools/audit/fmt/audit_lego_rendered_prose.py --chapter "$VOL/$CH" 2>&1 | tee -a "$LOG"
cp "$REPO/book/tools/audit/artifacts/lego_rendered_prose_audit.json" "$PROSE_SNAP"

COHERENCE_RC=0
if command -v gemini >/dev/null 2>&1; then
  python3 bindery/tools/audit/lego_prose_coherence.py --chapter "$VOL/$CH" \
...
cells_path = Path(os.environ.get("LEGO_VERIFY_CELLS_SNAP", "")) or (
    repo / f"bindery/tools/audit/artifacts/lego_chapter_reports/{vol}_{ch}_cells.json"
)
prose_path = Path(os.environ.get("LEGO_VERIFY_PROSE_SNAP", "")) or (
    repo / f"bindery/tools/audit/artifacts/lego_chapter_reports/{vol}_{ch}_prose.json"
)
ledger_path = repo / "bindery/tools/audit/artifacts/chapter_html_audit.json"
coh_path = repo / f"bindery/tools/audit/artifacts/lego_prose_coherence_{vol}_{ch}.json"
report_md = repo / f"bindery/tools/audit/artifacts/lego_chapter_reports/{vol}_{ch}_rendered_prose.md"
cert_path = repo / f"bindery/tools/audit/artifacts/lego_chapter_reports/{vol}_{ch}_certificate.md"
progress = repo / "bindery/tools/audit/artifacts/lego_chapter_progress.md"
