# bindery/tools/audit — Pass 15 Audit-Fix-Verify Loop
...
```bash
python3 bindery/tools/audit/check_registry_migration_build.py
```
...
python3 bindery/tools/audit/scan.py --scope vol1 -v

# Reproduce pre-Pass-16 behavior (all 75 FPs report as open)
python3 bindery/tools/audit/scan.py --scope vol1 --no-accept-list -v

# Use a different accept-list file (e.g. a draft to iterate on)
python3 bindery/tools/audit/scan.py --scope vol1 --accept-list /tmp/draft.json
```
...
```bash
python3 bindery/tools/audit/scan.py --scope vol2 --verbose
python3 bindery/tools/audit/scan.py --scope vol1 --output vol1-ledger.json --verbose
```
...
```bash
python3 bindery/tools/audit/fix_script_lane.py \
...
python3 bindery/tools/audit/loop.py --scope vol2 --dry-run --verbose
...
$ python3 bindery/tools/audit/scan.py --scope vol2 -v
...
Reproduce with `python3 bindery/tools/audit/scan.py --scope vol1 -v`.
Run the detector self-tests with:

```bash
PYTHONPATH=bindery/tools python3 bindery/tools/audit/checks/h3_titlecase.py
PYTHONPATH=bindery/tools python3 bindery/tools/audit/checks/concept_term_capitalization.py
