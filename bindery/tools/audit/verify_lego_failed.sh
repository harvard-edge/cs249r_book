...
#
# Usage (repo root):
#   ./bindery/tools/audit/verify_lego_failed.sh
#   ./bindery/tools/audit/verify_lego_failed.sh bindery/tools/audit/artifacts/lego_chapter_failures.txt
set -euo pipefail
...
while IFS= read -r line; do
  ...
  ch="${line#*/}"
  echo "=== retry $vol/$ch ==="
  if ./bindery/tools/audit/verify_lego_chapter.sh "$vol" "$ch"; then
    echo "OK $line"
  else
