...
ARTIFACTS = REPO / "bindery/tools/audit/artifacts"
...
def run_prose_layer(vol: str) -> LayerResult:
    ...
    script = REPO / "bindery/tools/audit/fmt/audit_prose.py"
...
def run_static_html_layer(vol: str, html_dir: Path) -> LayerResult:
    ...
    audit_html = REPO / "bindery/tools/audit/fmt/audit_html.py"
...
def run_lego_html_layer(vol: str) -> LayerResult:
    ...
    proc = subprocess.run(
        [sys.executable, str(REPO / "bindery/tools/audit/fmt/audit_lego_html.py"), "--report", str(report_path)],
...
def run_playwright_layer(vol: str, artifact_dir: Path) -> LayerResult:
    ...
    proc = subprocess.run(
        ["uv", "run", "--with", "playwright", "python3",
         str(REPO / "bindery/tools/audit/render_playwright_verify.py"),
