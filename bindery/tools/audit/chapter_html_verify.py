...
Usage (repo root)::
    ...
    python3 bindery/tools/audit/chapter_html_verify.py --list
    python3 bindery/tools/audit/chapter_html_verify.py --vol1 introduction
    python3 bindery/tools/audit/chapter_html_verify.py --vol1 --all
    python3 bindery/tools/audit/chapter_html_verify.py --report

Ledger: bindery/tools/audit/artifacts/chapter_html_audit.json
Table:   bindery/tools/audit/artifacts/chapter_html_audit.md
"""
...
AUDIT_HTML = REPO_ROOT / "bindery/tools/audit/fmt/audit_html.py"
LEDGER_JSON = REPO_ROOT / "bindery/tools/audit/artifacts/chapter_html_audit.json"
LEDGER_MD = REPO_ROOT / "bindery/tools/audit/artifacts/chapter_html_audit.md"
...
def _lego_focal(qmd: Path) -> tuple[bool, str]:
    proc = subprocess.run(
        [sys.executable, str(REPO_ROOT / "bindery/tools/audit/lego_focal_verify.py"), str(qmd)],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
...
def _write_markdown_table(ledger: dict) -> None:
    ...
    lines.extend([
        ...
        "",
        "Re-run one chapter: `python3 bindery/tools/audit/chapter_html_verify.py --vol1 training`",
        "",
    ])
