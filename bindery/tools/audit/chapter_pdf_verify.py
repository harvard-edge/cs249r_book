...
Usage (repo root)::
    ...
    python3 bindery/tools/audit/chapter_pdf_verify.py --list
    python3 bindery/tools/audit/chapter_pdf_verify.py --vol1 introduction
    python3 bindery/tools/audit/chapter_pdf_verify.py --vol1 --all
    python3 bindery/tools/audit/chapter_pdf_verify.py --report

Ledger: bindery/tools/audit/artifacts/chapter_pdf_audit.json
Table:   bindery/tools/audit/artifacts/chapter_pdf_audit.md
...
LEDGER_JSON = REPO_ROOT / "bindery/tools/audit/artifacts/chapter_pdf_audit.json"
LEDGER_MD = REPO_ROOT / "bindery/tools/audit/artifacts/chapter_pdf_audit.md"
    ...
    try:
        proc = subprocess.run(
            [sys.executable, "-W", "ignore::UserWarning",
             str(REPO_ROOT / "bindery/tools/audit/fmt/audit_prose.py"), str(qmd)],
            cwd=REPO_ROOT,
            env=env,
            capture_output=True,
            text=True,
...
def _registry_scan(qmd: Path) -> tuple[bool, str]:
    proc = subprocess.run(
        [sys.executable, str(REPO_ROOT / "bindery/tools/audit/book_check_registry_sources.py"), str(qmd)],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
