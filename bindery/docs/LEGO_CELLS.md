...
fmt/notation audit lane (`bindery/tools/audit/fmt/README.md`), the agent verify
...
- `bindery/tools/audit/fmt/README.md` — spurious `.0` / fmt precision workflow
...
- `bindery/tools/audit/book_check_registry_sources.py` — legacy alias / constant import gate
...
python3 bindery/tools/audit/lego_focal_verify.py books/vol1 books/vol2
python3 bindery/tools/audit/fmt/audit_fmt_usage.py --root books
...
`bindery/tools/audit/artifacts/lego_audit_ranked_chapters.md`.
...
python3 bindery/tools/audit/book_check_lego_prose_units.py "$CH"
python3 bindery/tools/audit/fmt/fmt_prose_contract.py "$CH"
python3 bindery/tools/audit/audit_math_canonical.py "$CH"
./binder check math --scope canonical --path "$CH"
python3 bindery/tools/scripts/maintenance/validate_inline_refs.py --path "$CH"
```
...
See `bindery/tools/audit/artifacts/lego_closed_prose_audit.md`.
...
```bash
PYTHONPATH=mlsysim python3 bindery/tools/audit/fmt/audit_prose.py "$CH" --flagged-only
```
...
```bash
python3 bindery/tools/audit/chapter_html_verify.py --vol1 training   # build + full lane
python3 bindery/tools/audit/chapter_html_verify.py --report
...
`bindery/tools/audit/artifacts/lego_chapter_reports/`.

```bash
./bindery/tools/audit/verify_lego_chapter.sh vol1 introduction
./bindery/tools/audit/verify_lego_chapter.sh vol2 network_fabrics
```
...
```bash
./bindery/tools/audit/run_all_lego_chapters.sh
# failures → bindery/tools/audit/artifacts/lego_chapter_failures.txt
# progress  → bindery/tools/audit/artifacts/lego_chapter_progress.md
```

### Phase 5 — HTML spot checks

```bash
python3 bindery/tools/audit/fmt/audit_html.py books/_build/html-audit/vol1/introduction.html
```
...
```bash
./bindery/tools/audit/verify_lego_pipeline.sh   # or /precheck before push
```

Sign-off template: `bindery/tools/audit/artifacts/lego_audit_signoff.md`.
