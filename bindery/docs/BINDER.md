...
|----------------|-------------------------|
| **Preferred** | `bindery/cli/checks/*.py` — imported by `validate.py` |
| **Shared CLI primitives** | `bindery/cli/core/*.py` — reusable command logic such as bibliography fixes and artifact cleanup |
| **Inline** | `bindery/cli/commands/validate.py` — small regex/graph checks |
| **Transitional** | importlib/subprocess into `bindery/tools/` (being migrated) |

Scripts under `bindery/tools/` are not the public API. Pre-commit and CI call Binder subcommands, not scripts directly.
...
| Audience | Document |
|----------|----------|
| Authors & daily use | This file (`bindery/docs/BINDER.md`) — command reference |
| Check/fix implementation | [`bindery/cli/README.md`](../cli/README.md) — architecture, adding scopes, EPUB layers |
| Scope registry (code) | `bindery/cli/commands/validate.py` → `GROUPS` dict |
...
The binder lives at `binder` (Python entry point → `bindery/cli/`). Ensure it is executable:
...
3. **`./binder check bib`** — curated bibliography gate:
   ...
   Baselines: `bindery/tools/bib_lint_baseline.json` and
   `bindery/tools/bib_lint_style_baseline.json`.
...
**Note:** `publish` is not a Binder subcommand. Release publishing uses GitHub Actions and scripts under `bindery/tools/scripts/publish/`.
...
Release publishing is **not** a Binder subcommand. Use GitHub Actions and scripts under `bindery/tools/scripts/publish/`. The sections below describe historical `publish` behavior and may be outdated — see your team's release runbook.
...
- [`bindery/cli/README.md`](../cli/README.md) — check/fix architecture, adding scopes, EPUB layers
