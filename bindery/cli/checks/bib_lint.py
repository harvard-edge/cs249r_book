...
Usage:
    python3 bindery/tools/bib_lint.py <file.bib> [--check|--fix|--report]
    python3 bindery/tools/bib_lint.py --all [--check|--fix|--report]
    ./binder check bib
...
Integration points:
    ...
    - Compatibility CLI: `bindery/tools/bib_lint.py` is a thin wrapper for
        ...
        json.dumps(
            {
                "description": (
                    ...
                    "python3 bindery/tools/bib_lint.py --all --baseline"
                ),
        ...
        json.dumps(
            {
                "description": (
                    ...
                    "current debt via: python3 bindery/tools/bib_lint.py --all "
                    "--style-baseline"
                ),
...
class Field:
    ...
    def formatted(self, indent: str = "  ", align_col: int = 16) -> str:
        ...
        (the shared book references.bib is brace-quoted) and the existing
...
def _split_bibtex_names(value: str) -> list[str]:
    """Split a BibTeX name list on top-level `and` separators."""
    names: list[str] = []
    current: list[str] = []
    depth = 0
    i = 0
...
def _is_empty_bibtex_name(value: str) -> bool:
    """True when a BibTeX name segment is empty after wrappers are removed."""
    cleaned = value.strip().strip("{}").strip()
    return not cleaned


def _unsupported_bib_field_chars(value: str) -> list[str]:
    """Return field characters that are fragile in production PDF bibliography builds."""
    bad: list[str] = []
    for ch in value:
        if ch in "\n\r\t":
            continue
        category = unicodedata.category(ch)
        # Hard fail control/private/surrogate codepoints and emoji-like
        # supplementary symbols. Ordinary accented names are allowed.
        if category.startswith("C") or (ord(ch) >= 0x1F000 and category == "So"):
            if ch not in bad:
                bad.append(ch)
    return bad
...
def validate_entry(entry: Entry) -> list[Violation]:
    ...
    for f in entry.fields:
        ...
        bad_chars = _unsupported_bib_field_chars(f.value)
        if bad_chars:
            escaped = ", ".join(f"U+{ord(ch):04X}" for ch in bad_chars)
            v.append(Violation(
                entry.key, entry.start_line, "error",
                "unsupported-field-glyph",
                f"field `{f.name}` contains production-fragile glyph(s) "
                f"{escaped}; remove the glyph or replace it with plain text "
                f"so BibLaTeX/PDF builds are deterministic",
            ))
    ...
    a = entry.get("author")
    if a:
        names = _split_bibtex_names(a.value)
        if any(_is_empty_bibtex_name(name) for name in names):
            v.append(Violation(
                entry.key, entry.start_line, "error",
                "author-empty-name-segment",
                "author list contains an empty `and` segment; biber treats "
                "this as a malformed name and may skip the entry",
            ))
...
def main() -> int:
    ...
    mode.add_argument("--style-baseline", action="store_true",
                      help="regenerate the style allow-list from current "
                      "warning/info violations")
    ...
    mode_str = (
        ...
        "style-baseline" if args.style_baseline else
    ...
    all_violations_for_style_baseline: list[tuple[str, Violation]] = []
    ...
    for path in targets:
        ...
        fp_str = baseline_file_key(path)
        if args.baseline:
            for v in violations:
                all_violations_for_baseline.append((fp_str, v))
        if args.style_baseline:
            for v in violations:
                all_violations_for_style_baseline.append((fp_str, v))
    ...
    if args.style_baseline:
        save_style_baseline(all_violations_for_style_baseline)
        n_style = sum(
            1 for _, v in all_violations_for_style_baseline
            if v.severity in STYLE_BASELINE_SEVERITIES
        )
        print(
            f"\nStyle baseline written to {STYLE_ALLOW_LIST_PATH.name}: "
            f"{n_style} warning/info violations grandfathered"
        )
        return 0
