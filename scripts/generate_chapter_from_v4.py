#!/usr/bin/env python3
"""
Generate Chapter from V4 Master Outline (ML Systems Stance) (Multi-Call Systems Engineering Engine)

Authoritative chapter generation pipeline for Volume III: Agentic Machine Learning Systems.
...
MASTER_OUTLINE_V4_PATH = REPO_ROOT / "books" / "vol3" / "MASTER_TEXTBOOK_OUTLINE_V4_MLSYS.md"
...
def parse_chapter_v4(chapter_num: str, outline_path: Path = MASTER_OUTLINE_V4_PATH) -> ChapterManifest:
    """Parse a single chapter's blueprint from MASTER_TEXTBOOK_OUTLINE_V4_MLSYS.md."""
...
4. THE SALTZER & KAASHOEK SYSTEMS FOUNDATIONS:
   - Modularity & Layering: The foundation model is an unprivileged neural inference engine; the agent runtime is the host supervisor.
   ...
   - Protection & Least Privilege: Zero Ambient Authority ($A=0$). Candidate proposals are held in memory escrow; tools are mediated RPC endpoints (Ch 07); untrusted executions are isolated inside sandboxes (Ch 08).
...
def compose_step_prompt(
    ...
    macro_map = (
        "MACRO ARCHITECTURE OF VOLUME III (AGENTIC MACHINE LEARNING SYSTEMS):\n"
    ...
    if 1 <= step_idx <= len(manifest.sections):
        ...
        if is_sec1:
            ...
            task_parts.append(
                ...
                f"{negative_scope}\n"
                f"**Exact Section Specification from Master Outline V4:**\n{cleaned_spec}\n\n"
        ...
        task_parts.append(
            ...
            f"Target Depth: Rigorous and thorough analysis of the Fallacies and Pitfalls established in Outline V4.\n\n"
            f"**Heading & Anchor:** `## Fallacies and Pitfalls {{#sec-vol3-{manifest.slug}-fallacies}}`\n\n"
            f"**Exact Specification from Master Outline V4:**\n{manifest.fallacies_raw}\n\n"
...
def run_full_chapter(
    chapter_num: str,
    outline_path: Path = MASTER_OUTLINE_V4_PATH,
    ...
    manifest = parse_chapter_v4(chapter_num, outline_path=outline_path)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Multi-Call Chapter Generation Engine from Master Outline V4."
    )
    ...
    parser.add_argument("--outline", type=str, default=str(MASTER_OUTLINE_V4_PATH), help="Path to master outline file.")
    ...
    try:
        manifest = parse_chapter_v4(num, outline_path=outline_path)
    except Exception as e:
