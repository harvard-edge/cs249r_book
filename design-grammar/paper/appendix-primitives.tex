% Reference index for the primitive catalog. The paper switches to one-column
% layout immediately before this file so the appendix table can occupy a clean
% final page without a stranded heading.

\begin{table}[!t]
\centering
\caption{\textbf{Appendix: the 90 primitives of the ML Systems Design Grammar.} Each primitive lists its catalog number, symbol, full name, abstraction layer, and information-processing role. This table is the complete reference index; the main text refers to primitives by symbol (e.g., \texttt{At} for Attention) and occasionally by catalog number. The visual catalog appears in \Cref{fig:primitive-catalog}.}
\label{tab:appendix-primitives}
\footnotesize
