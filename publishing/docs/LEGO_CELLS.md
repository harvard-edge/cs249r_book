...
fmt/notation audit lane (`publishing/tools/audit/fmt/README.md`), the agent verify
playbook (`.claude/rules/lego-verify.md`), and pre-commit checks
(`lego-dead-code`, `./binder check math --scope canonical`).
...
4. Re-run `fmt/audit_prose.py` and `./binder check math --scope canonical` on that chapter.
...
- `publishing/tools/audit/fmt/README.md` — spurious `.0` / fmt precision workflow
- `./binder check math --scope canonical` — static fmt and suffix lint
- `./binder check code --scope lego-prose-literals` — walkthrough prose must not hardcode computed operands
- `./binder check refs --scope inline-python` — chapter exec validation
- `publishing/tools/audit/book_check_registry_sources.py` — legacy alias / constant import gate
...
python3 publishing/tools/audit/lego_focal_verify.py books/vol1 books/vol2
python3 publishing/tools/audit/fmt/audit_fmt_usage.py --root books
...
`publishing/tools/audit/artifacts/lego_audit_ranked_chapters.md`.

### Phase 1 — Gates (per chapter or corpus)

```bash
CH=books/vol1/introduction/introduction.qmd
./binder check code --scope lego-dead-code --path "$CH"
./binder check code --scope lego-prose-units --path "$CH"
python3 publishing/tools/audit/book_check_lego_prose_units.py "$CH"
python3 publishing/tools/audit/fmt/fmt_prose_contract.py "$CH"
python3 publishing/tools/audit/audit_math_canonical.py "$CH"
./binder check math --scope canonical --path "$CH"
python3 publishing/tools/scripts/maintenance/validate_inline_refs.py --path "$CH"
```
...
See `publishing/tools/audit/artifacts/lego_closed_prose_audit.md`.
...
```bash
PYTHONPATH=mlsysim python3 publishing/tools/audit/fmt/audit_prose.py "$CH" --flagged-only
```
...
```bash
python3 publishing/tools/audit/chapter_html_verify.py --vol1 training   # build + full lane
...
`publishing/tools/audit/artifacts/lego_chapter_reports/`.

```bash
./publishing/tools/audit/verify_lego_chapter.sh vol1 introduction
./publishing/tools/audit/verify_lego_chapter.sh vol2 network_fabrics
```
...
```bash
./publishing/tools/audit/run_all_lego_chapters.sh
# failures → publishing/tools/audit/artifacts/lego_chapter_failures.txt
# progress  → publishing/tools/audit/artifacts/lego_chapter_progress.md
```

### Phase 5 — HTML spot checks

```bash
python3 publishing/tools/audit/fmt/audit_html.py books/_build/html-audit/vol1/introduction.html
```
...
```bash
./publishing/tools/audit/verify_lego_pipeline.sh   # or /precheck before push
```

Sign-off template: `publishing/tools/audit/artifacts/lego_audit_signoff.md`.
