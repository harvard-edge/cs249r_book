...
**Two volumes. One curriculum. The physics of AI engineering.**

The goal of this work is to provide a resource for educators and learners seeking to understand the principles and practices of machine learning systems. This curriculum treats machine learning systems not as a collection of software frameworks, but as a discipline grounded in physical and mathematical invariants. Whether on a single microcontroller or a warehouse-scale fleet, ML systems are constrained by the same fundamental physics: the speed of light, the memory wall, the energy cost of data movement, and the statistical uncertainty of learned behavior. By focusing on these invariants, we provide a foundation that remains stable even as the frameworks and models themselves evolve.
...
- **The Fleet** (Part I): Master the physical infrastructure, from data center architecture and accelerators to high-performance networking and storage.
...
- **Computer Systems**: Familiarity with memory hierarchies, CPU/accelerator architecture basics, and operating systems concepts will enhance understanding of hardware acceleration and deployment chapters. Students without this background can still succeed but may need to reference supplementary materials.
- **Machine Learning Basics**: Prior exposure to supervised learning concepts (classification, regression), neural network fundamentals, and common architectures (convolutional neural networks (CNNs), recurrent neural networks (RNNs)) is helpful but not strictly required. @Sec-neural-computation provides essential background for those new to these topics.
...
**Human-in-the-loop.** Many ML system components involve humans: data labeling, quality assessment, governance decisions, fairness audits. These are not peripheral concerns—they are engineering constraints with quantifiable costs, throughput limits, and quality trade-offs. Amdahl's Law applies to human bottlenecks just as it does to computational ones.
...
::: {#fig-bloom fig-env="figure" fig-pos="htb" fig-cap="**Bloom's Taxonomy**: The 2021 edition organizes learning objectives from foundational knowledge to advanced creative thinking." fig-alt="Bloom's Taxonomy (2021 edition)."}
![](images/png/bloom_revised_taxonomy.png)
:::
...
1. **Part V: The Fleet**
   *Build the physical computer.* Master the physical layer of ML systems: data center architecture, specialized accelerators (Tensor Processing Units (TPUs), H100s), high-bandwidth networking (NVLink, InfiniBand), and massive-scale storage.
...
Distributed algorithms need physical hardware. This Part builds the "supercomputer" required for modern ML, examining the data center architecture, specialized accelerators (TPUs, H100s, B200s), high-bandwidth interconnects, and massive-scale storage systems that fuel the training fleet.
...
Training is only half the battle. This Part addresses the challenges of serving models at scale, from low-latency data center inference using paged attention and continuous batching to resource-constrained edge intelligence, and the MLOps practices needed to manage massive production fleets.
...
**Computer Science Students**: ML systems extend familiar concepts into new domains. If you have worked with algorithms and data structures, think of ML as learning algorithms that automatically optimize themselves based on data patterns rather than following fixed instructions.
...
We also build on the excellent work of experts in the field, fostering a collaborative learning ecosystem where knowledge is shared, extended, and collectively advanced.
