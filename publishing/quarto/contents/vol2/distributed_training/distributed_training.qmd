...
::: {#fig-tensor-parallel-split fig-env="figure" fig-pos="htb" fig-cap="**Tensor parallelism: matrix partitioning**: The first linear layer (for example, QKV) is split column-wise as $[W_1 | W_2]$, and the second (for example, the output projection) is split row-wise as $[W_1 ; W_2]$. This Megatron-LM arrangement passes the intermediate result directly between the two splits and requires one AllReduce after the second layer to sum partial outputs." fig-alt="Two-panel diagram of tensor parallelism. Left: Column Parallel splits weight matrix into columns, GPUs compute independently. Right: Row Parallel splits by rows, requires AllReduce to sum partial results."}
