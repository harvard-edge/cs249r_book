...
\noindent
![](images/png/cover_distributed_training_blueprint_labeled_print.png){fig-alt="Isometric distributed training map showing a large workload partitioned across accelerators along data, tensor, and pipeline axes with synchronization paths."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_distributed_training_blueprint_labeled.webp){fig-alt="Isometric distributed training map showing a large workload partitioned across accelerators along data, tensor, and pipeline axes with synchronization paths."}
:::
...
Before optimizing algorithms, the physical constraints of the Machine Learning Fleet must be understood. The performance of any distributed training job is governed by the fleet law (principle \ref{pri-distributed-step-time}), introduced in @sec-vol2-introduction-fleet-law, which decomposes the per-step time:

$$ T_{\text{step}}(N) = \frac{T_{\text{compute}}}{N} + T_{\text{comm}}(N) + T_{\text{sync}}(N) - T_{\text{overlap}} $$

The critical term is the **Communication-Computation Ratio**\index{Communication-Computation Ratio!definition}, $\rho = T_{\text{comm}}(N)/(T_{\text{compute}}/N)$. This ratio determines whether a cluster behaves as a supercomputer or a collection of idling heaters.
...
The third signal is **Dataset Scale**\index{Dataset Scale}. When training data reaches multiple terabytes, as occurs in large-scale vision or language modeling tasks, a single machine is no longer the natural unit of storage or input throughput. Distributed training then becomes a way to feed the model as much as a way to hold it. These three signals converge on the same structural limit. @Fig-memory-wall-trend traces it directly: device memory grows, model demand grows faster, and the crossing point is where distribution stops being an optimization and becomes a precondition.
...
# Read data
data = pd.read_csv("data/memory_wall_trends.csv")
...
Distribution changes the optimization problem by adding costs that do not exist inside one machine. Three complexity dimensions decide whether a parallelism strategy is viable, because any one of them can become the binding ceiling. **Communication Overhead**\index{Communication Overhead} is the cost of synchronizing gradients: for a model with $P$ parameters distributed across $N$ devices, all-reduce operations must transfer approximately $2P(N-1)/N$ gradient values per step, multiplied by the bytes per gradient element, and on commodity networks this can dominate computation time. **Fault Tolerance**\index{Fault Tolerance} grows harder as the cluster grows, because the expected number of failures per unit time rises linearly with cluster size while the probability that the entire cluster survives an interval without any failure decays exponentially; if a `{python} DistTrainReliabilityFacts.reliability_node_count_str`-node cluster has `{python} DistTrainReliabilityFacts.node_hourly_survival_pct_str` per-node hourly survival, the cluster-level failure probability is `{python} DistTrainReliabilityFacts.cluster_failure_prob_pct_str` per hour, corresponding to a mean time between failures (MTBF) of about `{python} DistTrainReliabilityFacts.cluster_mtbf_hours_str`. @Sec-appdx-reliability-foundations-mtbf-cascade derives this cascade from per-node survival to cluster MTBF and works the same calculation through a full example, so the reader can reproduce why MTBF collapses as the cluster grows. **Algorithmic Stability**\index{Algorithmic Stability} closes the set, because large batch sizes from data parallelism affect convergence behavior, requiring learning rate scaling and warmup strategies that single-machine training does not require [@goyal2017accurate]. Together, these costs explain why distributed training is a constraint satisfaction problem rather than a hardware multiplication exercise.
...
::: {#fig-3d-parallelism-cube fig-env="figure" fig-pos="htb" fig-cap="**The 3D parallelism cube**: The cube visualizes three distinct partitioning axes: data parallelism (replicating the model), pipeline parallelism (splitting depth), and tensor parallelism (splitting layers). Large-model training occupies a feasible point $(d, p, t)$ only when divisibility, memory, batch, topology, and failure-domain constraints are satisfied together." fig-alt="3D coordinate system with three axes: Data Parallelism (replica count d), Pipeline Parallelism (depth p), and Tensor Parallelism (width t). Dashed cube marks a feasible configuration point with Total Accelerators equals d times p times t."}

![](images/svg/3d-parallelism-cube.svg)

:::

@Fig-3d-parallelism-cube shows that the total accelerator count is $N_{\text{total}} = d \times p \times t$. The axes represent distinct mathematical partitions, but they are not independent configuration knobs. Tensor and pipeline degrees must divide the model structure, the data-parallel degree and microbatch schedule must produce a viable global batch, every partition must fit memory, and each communication group must map onto an adequate topology and acceptable failure domain. Training systems therefore select a jointly feasible coordinate rather than tuning $d$, $p$, and $t$ in isolation.
...
::: {#fig-parallelism-decision-tree fig-env="figure" fig-pos="htb" fig-cap="**Parallelism strategy decision tree**: Starting from the model's memory requirements, the tree follows the constraints that determine which parallelism strategies are physically feasible. The critical branches compare model memory with single-GPU and single-node capacity; leaf nodes identify the resulting strategy and dominant hardware constraint." fig-alt="Decision tree flowchart. Root asks if model fits in one GPU. Yes branch leads to data parallelism. No branch asks if model fits in one node, leading to tensor or pipeline parallelism with hardware annotations."}

![](images/svg/parallelism-decision-tree.svg)

:::

The decision tree reveals that parallelism strategy selection is not a preference but a consequence of physical constraints. This is a constraint-level preview: the communication-volume formulas in @tbl-parallelism-tradeoffs name what each strategy moves, but their full meaning depends on the mechanics of tensor, pipeline, and hybrid parallelism developed over the rest of the chapter. @Sec-distributed-training-systems-systems-parallelism-strategy-comparison-d92a completes the filter once those mechanics are in place. The next question is *how* these constraints shape the mechanics of a distributed training step on a real cluster.
...
2.  **Distinction**: Unlike single-node multi-threading (which shares a coherent physical memory space across cores), distributed training partitions state across disjoint accelerator memories, requiring explicit collective communication primitives over interconnect fabrics to synchronize optimization state.
...
Despite this complexity, the core workflow is mechanically straightforward; the engineering challenge is making it fast and reliable at scale. The recurring cost is the gradient synchronization that aggregates results across devices, an overhead that compounds as systems scale, as @sec-distributed-training-systems-systems-distributed-training-efficiency-metrics-9488 quantifies.
...
2.  **Distinction**: Unlike model and pipeline parallelism (which partition the network's layers or operations across accelerators), data parallelism replicates the entire model across all workers and partitions only the training dataset, requiring gradient synchronization after each backward pass.
...
Consider a model with parameters $\theta$ training on a dataset $D$. The loss function for a single data point $x_i$ is $\mathcal{L}(\theta, x_i)$. In standard stochastic gradient descent (SGD) with batch size $B$, the gradient update for a minibatch is:
$$
...
For unequal local batch sizes, the combined-batch gradient is the weighted average $g_{\text{global}} = (1/|B_{\text{total}}|)\sum_k |B_k|g_k$. The equivalence shows why data parallelism maintains the statistical properties of SGD training: sampling the intended global batch across devices, computing local gradient sums independently, and normalizing by the aggregate denominator reproduces its estimator. The reduction step uses an AllReduce over local gradient sums and valid-item counts; @sec-appdx-comm-foundations-allreduce derives the ring and tree AllReduce cost models that set the price of this synchronization, so the reader can predict when it overtakes the local compute saved by parallelism.
...
The implementation details matter because the SGD equivalence only holds when each phase preserves the intended aggregate sampling and weighting, complete local gradient contributions, and a single synchronized update. The concrete workflow therefore traces the path from assigning local batches to synchronizing the computed gradients. Consider @fig-dist-train-data-parallelism: it traces the complete workflow from batch assignment through gradient aggregation, showing how each GPU processes its assigned batch before synchronization brings all gradients together for parameter updates.

::: {#fig-dist-train-data-parallelism fig-env="figure" fig-pos="htb" fig-cap="**Data parallelism implementation pipeline**: The five-stage workflow for data-parallel training: (1) sample or partition the intended global batch, (2) assign local batches to GPUs, (3) compute forward and backward passes independently, (4) reduce weighted gradient contributions, and (5) update parameters uniformly across all devices. This approach contrasts with model parallelism, where the model itself is partitioned rather than replicated." fig-alt="Flowchart showing a five-stage data-parallel workflow. Input data splits into four batches assigned to GPUs 0–3; each GPU performs a forward and backward pass, gradients synchronize and aggregate, and each model replica receives the same update."}

![](images/svg/data-parallel-flow.svg)

:::

The critical synchronization point is stage 4 (@fig-dist-train-data-parallelism): AllReduce must complete before any GPU can update parameters, making gradient communication the dominant bottleneck as the device count grows.
...
class Gpt3ActivationBudget:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    layers = 96  # GPT-3 architecture (Models.Language.GPT3)
    seq_len = 2048
...
**Scenario**: An AllReduce of a `{python} HierarchicalAllReduceDebug.tensor_gb_str` gradient tensor across `{python} HierarchicalAllReduceDebug.nodes_str` (`{python} HierarchicalAllReduceDebug.total_gpus_str`) takes `{python} HierarchicalAllReduceDebug.observed_ms_str`.

**Diagnosis**: Latency falls well below the `{python} HierarchicalAllReduceDebug.flat_ring_ms_str` flat-ring bound, confirming NCCL uses a hierarchical algorithm. However, switch counters reveal inter-node InfiniBand links run at only `{python} HierarchicalAllReduceDebug.ib_util_pct_str` utilization due to uplink congestion.

**Systems lesson**: Debugging collective communication requires evaluating physical link utilization against algorithmic bounds. The gap between `{python} HierarchicalAllReduceDebug.hierarchical_total_ms_str` theoretical and `{python} HierarchicalAllReduceDebug.observed_ms_str` observed latency stems from network fabric uplink contention, requiring rail-optimized topologies rather than algorithm changes.

:::

Stepping back from that specific cluster to the design space, @fig-coll-comm contrasts three high-level synchronization topologies: the bandwidth-optimal ring AllReduce, the centralized parameter server, and the fully connected all-to-all mesh. In dense synchronous data-parallel settings, ring AllReduce can avoid a single reducer bottleneck by distributing traffic evenly across participating links, as Baidu's implementation illustrates [@gibiansky2017baidu].

::: {#fig-coll-comm fig-env="figure" fig-pos="htb" fig-cap="**Gradient synchronization topologies**: The panels compare three communication patterns. (a) Ring AllReduce distributes bandwidth evenly across links but has linear latency scaling. (b) A parameter server centralizes traffic and creates a server bottleneck. (c) An All-to-All mesh completes an exchange in one communication round, but its link count grows quadratically, making the topology impractical beyond small groups." fig-alt="Three communication topology diagrams. (a) Ring AllReduce with 4 GPUs in circular topology. (b) Parameter Server with central PS node connected to 4 workers creating bottleneck. (c) All-to-All mesh with 4 GPUs fully connected by 6 links."}

![](images/svg/gradient-sync-topologies.svg)

:::

The trade-off visible in @fig-coll-comm is between *bandwidth* and *latency*: a ring spreads bandwidth evenly but adds a latency step per hop, while an all-to-all mesh collapses the latency path to constant rounds at the cost of a link count that grows quadratically with the node count. High-performance libraries such as NCCL select among these topologies automatically based on message size and cluster topology. @Sec-collective-communication formalizes the latency complexity of each topology and derives when a ring beats the alternatives.

#### Synchronization models {#sec-distributed-training-systems-systems-synchronization-models-396f}

Distributed training systems operate under explicit synchronization models that govern when workers observe each other's updates. The choice of model determines whether the system guarantees mathematical equivalence to single-device training or trades consistency for throughput. The baseline model, bulk synchronous parallel (BSP)[^fn-bsp-synchronous-barrier] [@valiant1990], requires all workers to complete their local computation in forward and backward passes, synchronize gradients through a barrier with AllReduce, and then simultaneously update parameters.

[^fn-bsp-synchronous-barrier]: **Bulk synchronous parallel (BSP)**\index{BSP!synchronization}: Introduced by @valiant1990 as a "bridging model" between hardware and software for parallel computation. BSP divides work into supersteps (compute, communicate, barrier), giving every worker a consistent superstep boundary. The cost: iteration time equals the slowest worker's time, and at 1,000 GPUs with 1 percent straggler probability per device, roughly 10 GPUs straggle every step, making the barrier increasingly expensive.

BSP provides synchronous update semantics: every worker computes from the same parameter version and reaches the next update boundary together. With the same global batch and reduction, this matches the corresponding synchronous single-device update mathematically. It does not by itself guarantee bitwise reproducibility. Floating-point reduction order, nondeterministic kernels, data-order state, and failure recovery can still change the result. The synchronization cost remains that the slowest worker determines iteration time, creating the straggler\index{Straggler!synchronization delay} problem.

Stale synchronous parallel (SSP)\index{Stale Synchronous Parallel!bounded staleness} relaxes this constraint by allowing workers to proceed up to $s$ iterations ahead of the slowest worker before blocking. This bounds staleness while reducing synchronization delays. SSP requires careful learning rate tuning since workers compute gradients on slightly different parameter versions. The bounded staleness guarantee provides a middle ground between BSP's strong consistency and fully asynchronous approaches [@ho2013more].

Asynchronous SGD (ASP)\index{Asynchronous SGD!staleness penalty} eliminates synchronization barriers entirely as workers update parameters independently. This maximizes hardware utilization but introduces gradient staleness that can degrade convergence. The operational guarantee determines how much convergence risk the system accepts; @sec-distributed-training-systems-systems-staleness-impact-bsp-vs-ssp-vs-asp-e153 develops the convergence rates, the staleness penalty, and the compensation techniques each model requires.
...
: **Synchronization Model Trade-offs**: BSP, SSP, and asynchronous SGD trade consistency for throughput along a spectrum. BSP preserves synchronous update boundaries but pays for the slowest worker; SSP unlocks throughput under bounded staleness; async maximizes throughput at the cost of convergence guarantees. Reproducibility requires a separate deterministic-execution contract. {#tbl-sync-models}

The same trade-off becomes clearer when the schedules are placed on a timeline.

::: {#fig-sync-model-timeline fig-env="figure" fig-pos="htb" fig-cap="**Distributed synchronization models**: The timelines compare three synchronization strategies. (A) Bulk synchronous parallel (BSP) forces all workers to wait at a global barrier every step. (B) Stale synchronous parallel (SSP) allows workers to proceed up to $s$ steps ahead of the slowest worker. (C) Asynchronous SGD eliminates barriers, increasing throughput at the cost of gradient staleness." fig-alt="Timeline diagram with three panels. Top: BSP with global barriers. Middle: SSP with bounded staleness slack. Bottom: Asynchronous with independent workers and overlapping steps."}
...
Data parallelism is a common starting strategy for a reason: it scales throughput linearly with device count, provided the model fits in memory and communication is not the bottleneck. However, it hits a hard ceiling defined by the communication-computation ratio: once gradient exchange dominates useful computation, more workers mainly add synchronization work [@ben-nun2019].

Data parallelism offers three principal advantages. Throughput scales linearly for compute-bound models: scaling ResNet-50\index{ResNet-50} on ImageNet from 1 to 256 GPUs yields near-linear speedup because the gradient exchange is small relative to the compute time. The model architecture also remains unchanged; the framework wraps the model in a data-parallel container that intercepts backward-pass hooks to trigger gradient synchronization automatically. Utilization stays high because, unlike model parallelism, there are no pipeline bubbles: all GPUs work on the forward and backward pass simultaneously.

Three hard ceilings limit these advantages. The memory wall\index{Memory Wall!data parallelism} requires every GPU to hold a full copy of the model parameters, gradients, and optimizer states; for a `{python} FrontierTrainingContext.frontier_params_b_str`-parameter model, this demands more than 1 TB of memory per GPU, exceeding per-device HBM budgets without ZeRO sharding. The bandwidth wall emerges as $N$ grows: the AllReduce cost $\frac{2(N-1)}{N} \times \frac{M}{\text{BW}_{\text{net}}}$ eventually dominates, and for large language models gradient synchronization can consume more than 50 percent of the step time, collapsing efficiency. The batch size trap compounds the problem because scaling to thousands of GPUs requires increasing the global batch size $(B_{\text{global}} = N \times B_{\text{local}})$, and eventually the critical batch size is reached, where adding more data per step yields diminishing returns in convergence.
...
```{python}
#| echo: false
from mlsysim.fmt import fmt, check, fmt_int, fmt_usd, MarkdownStr, fmt_time, fmt_math
...
2. **Communication overhead**: With `{python} GradAccumScenario.accum_steps_str`-step accumulation, we AllReduce once every `{python} GradAccumScenario.accum_steps_str` steps.
   - Overhead = `{python} GradAccumScenario.comm_8gpu_ms_str` / (`{python} GradAccumScenario.accum_steps_str` $\times$ `{python} GradAccumScenario.compute_8gpu_ms_str`) $\approx$ `{python} GradAccumScenario.comm_overhead_pct_math`.
...
from mlsysim.core.units import GiB, GB, byte, param
from mlsysim.fmt import fmt, check, fmt_int, fmt_display_math, fmt_memory_capacity, fmt_params, fmt_qty_int
...
The choice between FSDP and DDP depends on model size and memory constraints. When the model fits in GPU memory with room for activations, DDP usually wins because it avoids the repeated AllGather work. As memory pressure rises, ZeRO-2 becomes attractive because it shards gradients and optimizer state while leaving parameters replicated; once parameters themselves exceed single-GPU memory, ZeRO-3/FSDP becomes necessary even though it puts AllGather on the critical path. For training 70B+ models on `{python} DistTrainA100Memory.a100_mem_gb_str`, FSDP typically has to combine with tensor parallelism rather than replace it.
...
2.  **Distinction**: Unlike single-node efficiency (which captures local bottlenecks like $\text{BW}$), scaling efficiency captures the cluster-level overhead of communication time $(T_{\text{comm}}(N))$ and synchronization.
...
class CommScalingMath:
    ...
    compute_per_step_math = fmt_math(
        f"O_{{\\text{{step}}}} = 6 \\times {gpt3_params_b_value_str} \\times 10^9 \\times 2 \\times 10^6 \\approx 2.1 \\times 10^{{18}}"
    )
...
However, even for large models, scaling does not continue indefinitely. There exists a **Scaling Cliff**\index{Scaling Cliff} beyond which marginal speedup no longer justifies the added devices. Under the single explicit model used here (a `{python} InfraFrontierStragglerRecap14.frontier_params_b_str` model, `{python} CommScalingMath.batch_tokens_m_str` per update, `{python} CommScalingMath.n_gpus_str`, `{python} CommScalingMath.mfu_pct_str` MFU, and `{python} CommScalingMath.overlap_pct_str` communication overlap) naive data parallelism reaches only `{python} CommScalingMath.scaling_efficiency_pct_str`. The example therefore does not imply a universal 1,024--4,096-GPU efficient region or an 8,192-GPU cliff. Evaluating another cluster size requires recomputing per-device compute time, collective latency and bandwidth cost, overlap, and topology. The model architecture and cluster size must be co-designed to keep those terms in balance.
...
**Setup**: Training a `{python} InfraFrontierPodSetupRecap.frontier_params_b_str` model on a DGX H100 cluster with `{python} InfraIbBwPodRecap.ib_bw_gbit_per_s_str` InfiniBand per GPU.
...
Just as the Iron Law of Processor Performance governs single-thread execution, distributed training is governed by an extended version of Amdahl's Law\index{Amdahl's Law}\index{Amdahl, Gene|seealso{Amdahl's Law}} that explicitly accounts for communication overhead. The time to complete one training step on $N$ devices is not simply $T_{\text{single}} / N$, but is constrained by the sequential nature of synchronization. @Sec-appdx-fleet-foundations-amdahls-fleet derives the speedup ceiling at fleet scale and works it through a concrete example; the essential consequence here is that a fixed synchronization fraction caps speedup no matter how many accelerators are added.
...
The fleet law (principle \ref{pri-distributed-step-time}) maps directly onto the iron law's variables, separating compute, bandwidth, and coordination into additive terms:
...
For the update $\theta_{k+1}=\theta_k-\eta g_k$ with constant step size $0<\eta\leq 1/L_s$, after $K$ synchronized updates,

$$
\frac{1}{K}\sum_{k=0}^{K-1}\mathbb{E}\!\left[\|\nabla\mathcal{L}(\theta_k)\|^2\right]
\leq
\underbrace{\frac{2\bigl(\mathcal{L}(\theta_0)-\mathcal{L}_{\inf}\bigr)}{\eta K}}_{\text{optimization term}}
+
\underbrace{\frac{\eta L_s\sigma^2}{Nb}}_{\text{stochastic-variance term}}.
$$
...
The theorem isolates the benefit that follows from the assumptions. With equal local batch size $b$, synchronous data-parallel SGD computes the same minibatch estimator as one worker using batch size $Nb$ for that update, and the estimator variance decreases as $1/(Nb)$. It does not say that every increase in batch size proportionally reduces time to a target loss; learning-rate limits and critical-batch-size effects still apply. This distinction is the **Statistical Efficiency**\index{Statistical Efficiency!distributed training} of distributed training, separate from hardware efficiency.
...
1.  **Significance**: It represents the synchronization error in distributed optimization. Increasing $\tau_{\text{stale}}$ can improve throughput by reducing barrier waits $(T_{\text{sync}}(N))$, but it typically degrades the rate of convergence, requiring more operations $(O)$ to reach the same accuracy.
...
The convergence behavior differs across these models in ways that directly affect training cost and solution quality. In BSP ($\tau_{\text{stale}}=0$), all workers compute on the same parameter version and aggregate before updating. Under the assumptions of theorem \ref{thrm-distributed-training-convergence-rate-distributed-sgd}, the synchronized estimator inherits the $Nb$ minibatch guarantee.

SSP ($\tau_{\text{stale}}\leq s$) allows workers to proceed up to $s$ clocks ahead of the slowest worker. @ho2013more prove an average-regret rate $\mathcal{O}(\sqrt{(s+1)N/T})$ for their SSP update sequence with $N$ workers and $T$ updates, assuming convex Lipschitz component functions, a bounded domain, and a decreasing step size. This result makes the staleness dependence explicit, but it is not interchangeable with the nonconvex stationarity theorem \ref{thrm-distributed-training-convergence-rate-distributed-sgd}.
...
|:----------|:------------------------------|:----------------------------------------------------------------------------------------------------|:-----------------------------------------------------|
| **BSP**   | $\tau_{\text{stale}}=0$       | Synchronous $Nb$-minibatch theorem \ref{thrm-distributed-training-convergence-rate-distributed-sgd} | Waits for the slowest worker                         |
| **SSP**   | $\tau_{\text{stale}}\leq s$   | $\mathcal{O}(\sqrt{(s+1)N/T})$ average regret under convex assumptions [@ho2013more]                | Bounds delay while reducing some waits               |
...
This rule is justified by approximating one large-batch step at learning rate $k\eta$ as $k$ consecutive small-batch steps each at learning rate $\eta$. With batch size $kB$, the gradient variance decreases by factor $k$, so the larger step is well-conditioned; the approximation holds when parameters do not move much during those $k$ small-batch steps. The rule is empirical rather than universal: @goyal2017accurate validated it for ResNet-50/ImageNet at global batch size 8,192 with gradual warmup, while layer-wise adaptive rate scaling (LARS) and layer-wise adaptive moments based optimizer (LAMB) extend large-batch recipes through layer-wise adaptation for ImageNet and BERT-style pretraining [@you2017large; @you2020large]. A **Warmup Period**\index{Learning Rate Warmup} that increases the learning rate linearly from $\eta_{\text{base}}$ to $k \cdot \eta_{\text{base}}$ over the first $W$ iterations lets the model reach a region of the loss landscape where large learning rates are stable:
...
For extreme batch sizes (32K--1M), LARS [@you2017large] and its Adam variant LAMB [@you2020large] automatically adjust learning rates per layer based on the ratio of weight norm to gradient norm:
...
***Critical batch size***\index{Critical Batch Size!definition} $(B^{*})$ is the distributed-training batch size at which the gradient noise scale is large enough that further batch growth yields diminishing returns in sample efficiency [@mccandlish2018empirical; @Shallue2019measuring].
...
- [ ] **Critical batch size**: calculate the critical batch size $(B_{\text{crit}})$, where the gradient noise scale equals the batch size.
...
- Learning rate: $\eta' = \eta_{\text{BSP}} / \sqrt{1 + s}$ with $s =$ `{python} ScalingWorkers.ssp_staleness_str`, giving $\eta' \approx$ `{python} ScalingWorkers.lr_ssp_str` $\times \eta_{\text{BSP}}$
...
Several techniques occupy different positions on this trade-off curve. Gradient compression\index{Gradient Compression!bandwidth reduction} reduces communication volume by transmitting compressed gradient information through quantization, sparsification, or low-rank approximation. Examples include sign-based updates [@bernstein2018signsgd], Deep Gradient Compression [@lin2018deep], PowerSGD [@vogels2020powersgd], quantized SGD with convergence analysis [@alistarh2017qsgd], and empirical sparse gradient dropping for neural machine translation [@aji2017].
...
1.  **Parameter storage**: `{python} FrontierModelParallelRecap.frontier_params_b_str` params $\times$ `{python} MemoryWallAnalysis.bytes_fp16_str` bytes (FP16) = `{python} MemoryWallAnalysis.param_gb_str`.
2.  **Gradients and optimizer state**: Gradients add `{python} FrontierModelParallelRecap.frontier_params_b_str` params $\times$ `{python} MemoryWallAnalysis.bytes_fp16_str` bytes = `{python} MemoryWallAnalysis.gradient_gb_str`, while Adam FP32 master weights, momentum, and variance add `{python} FrontierModelParallelRecap.frontier_params_b_str` params $\times$ `{python} MemoryWallAnalysis.bytes_adam_str` bytes = `{python} MemoryWallAnalysis.optim_gb_str`.
...
5.  **Activation memory**: For sequence length 2048 and batch size 1, the canonical full-checkpointing strategy stores $\approx$ `{python} MemoryWallAnalysis.activation_gb_str` of activations per GPU.

**Systems insight**: `{python} MemoryWallAnalysis.per_gpu_gb_str` (static) + `{python} MemoryWallAnalysis.activation_gb_str` (dynamic) = `{python} MemoryWallAnalysis.total_per_gpu_gb_str`. Under this simplified accounting, ZeRO-3 plus full activation checkpointing fits within the `{python} A100CapacityContext.a100_mem_gb_str` A100 capacity, leaving material headroom for framework buffers and transient layer gathers. If those additional allocations exceed the remaining HBM, the design can reduce the micro-batch, offload state, increase the sharding degree, or add tensor or pipeline parallelism; tensor parallelism is not the only recourse.
...
Once the model itself is the object being partitioned, the implementation problem is placement: every cut must save enough memory to justify the activation transfer and idle time it introduces. @Fig-dist-model-parallelism captures this bidirectional data flow: input data propagates forward through sequentially assigned model partitions while gradients flow backward to update parameters, with intermediate results transferring across device boundaries at each stage.

::: {#fig-dist-model-parallelism fig-env="figure" fig-pos="htb" fig-cap="**Model parallelism data flow**: Model partitions execute sequentially across three devices: input data flows forward through each partition (top path), while gradients propagate backward during the update phase (bottom path). Each device holds a distinct model slice, so intermediate activations and gradients cross device boundaries. The design fits models larger than one device's memory at the cost of sequential dependencies that reduce utilization." fig-alt="Linear flow diagram showing model parallelism: Input Data flows through Model Parts 1-3 on Devices 1-3 to Predictions via forward pass arrows above, with gradient update arrows returning below."}
...
::: {#fig-dist-layers-blocks fig-env="figure" fig-pos="htb" fig-cap="**Layer-wise model parallelism**: The upper path assigns four consecutive transformer blocks to each of four GPUs, with activation checkpoints at device boundaries. The lower schedule shows forward work in blue, backward work in orange, and hatched pipeline bubbles. Partitioning reduces the weight footprint per GPU to one-quarter of the model, but sequential dependencies leave downstream devices waiting during pipeline fill." fig-alt="A 16-layer transformer is split across four GPUs, with layers 1–4, 5–8, 9–12, and 13–16 assigned to successive devices. Below, a timeline shows staggered blue forward passes, orange backward passes, and hatched idle pipeline bubbles."}
...
Pipeline parallelism extends layer-wise partitioning by introducing microbatching to reduce device idle time. Instead of waiting for an entire batch to sequentially pass through all devices, the computation is divided into smaller segments called microbatches, with overlapping execution across pipeline stages (@fig-pipeline-parallelism). In this schedule, each row represents a device processing model layers across microbatches, with forward activations flowing downstream and backward gradients propagating upstream. The overlap reduces, but does not eliminate, idle pipeline slots.

::: {#fig-pipeline-parallelism fig-env="figure" fig-pos="htb" fig-cap="**Pipeline parallelism schedule**: A 4-stage pipeline processes 4 microbatches through forward passes ($F_{i,j}$) and backward passes ($B_{i,j}$) over time. Rows represent pipeline stages, and columns represent time steps. Staggering the work lets one stage process a microbatch while another handles an earlier or later microbatch, reducing idle time without eliminating the fill, drain, and dependency bubbles visible in the schedule." fig-alt="Pipeline schedule grid showing four stages processing four microbatches. Forward passes stagger diagonally across time, backward passes follow in reverse order, and blank cells show pipeline bubbles during fill, drain, and dependencies."}
...
3.  **Common pitfall**: A frequent misconception is that adding pipeline stages always improves throughput. Each additional stage increases the pipeline bubble fraction $(p-1)/(m+p-1)$ unless the number of microbatches $m$ scales proportionally, causing hardware to idle and erasing the benefit of deeper partitioning.
...
3. **Bubble fraction**: $(p-1) / (p-1+m) =$ `{python} PipelineBubble.pb_p_minus_1_str` $/$ (`{python} PipelineBubble.pb_p_minus_1_str` $+$ `{python} PipelineBubble.pb_microbatches_str`) $\approx$ `{python} PipelineBubble.pb_bubble_pct_str`.

**Systems insight**: Pipeline parallelism is a *utilization-memory trade-off*. Reducing the bubble from `{python} PipelineBubble.pb_bubble_pct_round_str` to 5 percent requires more microbatches $(m)$, which consume additional activation memory on each GPU. In the machine learning fleet, depth does not scale for free; the cluster pays a `{python} PipelineBubble.pb_bubble_pct_str` capacity tax just to keep the stages coordinated. This is why techniques like interleaved pipelining are essential: they chop the bubble into smaller pieces to recover that lost `{python} PipelineBubble.pb_bubble_pct_round_str` of fleet capacity.

:::

GPipe[^fn-gpipe-pipeline-schedule] [@huang2019gpipe] introduced synchronous pipeline parallelism with micro-batch accumulation, while PipeDream [@narayanan2019] developed asynchronous approaches with weight stashing. Modern systems employ **1F1B (One-Forward-One-Backward)**\index{1F1B Scheduling!memory efficiency}[^fn-one-forward-backward] scheduling to reclaim activation memory earlier; that lower memory pressure makes larger microbatch counts practical, and the larger $m$ is what shrinks the bubble term.

[^fn-one-forward-backward]: **1F1B (one-forward-one-backward) scheduling**\index{1F1B Scheduling!memory efficiency}: Unlike GPipe (which processes all forward passes before any backward passes), 1F1B interleaves them. This reduces the peak activation memory footprint from $\mathcal{O}(m \times p)$ to $\mathcal{O}(p)$, where $m$ is the number of micro-batches and $p$ is the number of pipeline stages. This memory reclamation is what enables the massive micro-batch counts $(m \gg p)$ required to keep the pipeline bubble small.

[^fn-gpipe-pipeline-schedule]: **GPipe**\index{GPipe!pipeline parallelism}: Published by Google in 2019, GPipe introduced synchronous micro-batch pipelining that trained a 557M-parameter AmoebaNet across 8 Tensor Processing Units (TPUs)\index{TPU}\index{Tensor Processing Unit|see{TPU}} with near-linear scaling. The key trade-off GPipe exposed: the pipeline bubble fraction $(p-1)/(m+p-1)$ means that with $p=4$ stages and $m=4$ micro-batches, 43 percent of compute is wasted in idle time -- driving systems design toward schedules that reduce activation residency and make larger micro-batch counts feasible.

At the schedule level, 1F1B is a warmup, steady-state, and drain loop. @Algo-one-forward-one-backward-pipeline makes the contract explicit: each stage alternates forward work for newer microbatches with backward work for older microbatches once gradients arrive, so activations can be released before the whole batch has completed.
...
Pipeline parallelism, examined in @sec-distributed-training-systems-systems-pipeline-parallelism-8748, addresses device idle time by overlapping microbatch processing across stages. Each device holds complete layers and processes them sequentially, with communication only at stage boundaries when activations transfer between devices. This approach tolerates moderate interconnect bandwidth because communication occurs infrequently, once per layer boundary per microbatch. However, pipeline parallelism cannot help when individual layers themselves exceed device memory, or when the communication pattern within layers benefits from a different granularity than layer boundaries.
...
\vspace*{-4mm}
```
![](images/svg/vol2_distributed_training_margin_004.svg){width="100%" fig-alt="Violet bandwidth ladder comparing per-direction NVLink bandwidth with NDR InfiniBand, illustrating the order-of-magnitude bandwidth drop from intra-node to inter-node links."}

*Tensor parallelism needs NVLink bandwidth; pipeline parallelism tolerates slower fabric.*
:::

Megatron-style tensor parallelism[^fn-megatron-tensor-parallelism] [@shoeybi2019megatron] partitions matrix multiplications in two ways. Examine @fig-tensor-parallel-split: column-parallel splitting divides weight matrices along columns for QKV projections, allowing independent computation across GPUs, while row-parallel splitting divides along rows for output layers, requiring AllReduce to combine partial sums at the end of each block.
...
Each GPU computes its partition independently, and the outputs are concatenated without communication when the next operation is row-parallel. The row-parallel half of the pattern splits weights along rows, $W = \begin{bmatrix} W_1 \\ W_2 \end{bmatrix}$:
...
::: {#fig-tensor-parallel-split fig-env="figure" fig-pos="htb" fig-cap="**Matrix partitioning under tensor parallelism**: The first linear layer (for example, QKV) is split column-wise as $[W_1 | W_2]$, and the second (for example, the output projection) is split row-wise as $[W_1 ; W_2]$. This Megatron-LM arrangement passes the intermediate result directly between the two splits and requires one AllReduce after the second layer to sum partial outputs." fig-alt="Two-panel diagram of tensor parallelism. Left: Column Parallel splits weight matrix into columns, GPUs compute independently. Right: Row Parallel splits by rows, requires AllReduce to sum partial results."}

![](images/svg/tensor-parallel-split.svg)

:::

The column-then-row arrangement shown in @fig-tensor-parallel-split is the key design insight. Pairing a column-parallel layer with a row-parallel layer allows the intermediate activations to flow directly between them without communication, confining synchronization to the end of the pair. Megatron applies that pattern twice in each transformer block. The QKV projection is column-parallel and its attention output projection is row-parallel, then the first feed-forward layer is column-parallel and the second feed-forward layer is row-parallel. The design places AllReduce operations strategically, one after attention and one after the feed-forward network, for two AllReduce operations per transformer layer. Communication volume per transformer layer depends on sequence length $S$, hidden dimension $d_{\text{model}}$, tensor-parallel degree $t$, and batch size $B$. The activation payload is:
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Ring Attention discussion in @sec-distributed-training-systems-systems-tensor-parallelism-d76e
# │
...
While AllReduce dominates dense model training, the **Parameter Server (PS)**\index{Parameter Server!architecture} architecture, formalized by @li2014, remains common for recommendation systems and other sparse workloads. A parameter server architecture separates workers (who compute gradients) from servers (who store parameters and apply updates).
...
However, for **Recommendation Systems**\index{Recommendation System} (specifically DLRM) the model parameters are dominated by massive embedding tables (often 10 TB+) that cannot fit on any single GPU. Furthermore, the updates are sparse: a batch of users interacts with only a tiny fraction (e.g., 0.001 percent) of the items.

In this sparse regime, the parameter server avoids dense synchronization by moving only the rows that a batch touches:

1.  **Embedding Sharding**\index{Embedding Sharding}: The massive tables are partitioned across the PS fleet (often CPU nodes with massive DRAM).
...
The **Sparse Pull/Sparse Push**\index{Sparse Pull}\index{Sparse Push} pattern avoids the bandwidth bottleneck of dense AllReduce. Implementations such as TorchRec or Meta's hierarchical sharding place "hot" embeddings on GPUs and "cold" embeddings on CPU PS nodes, creating a tiered memory hierarchy for model parameters.

### Expert parallelism (mixture of experts) {#sec-distributed-training-systems-systems-expert-parallelism-mixture-experts-bc45}

While tensor parallelism splits dense layers across devices, expert parallelism\index{Expert Parallelism!conditional computation} enables scaling model capacity (parameters) without increasing compute cost (FLOPs) by using conditional computation. In a mixture-of-experts (MoE)\index{MoE!routing} architecture [@shazeer2017outrageously], the feed-forward network of each transformer block is replaced by a set of $E$ "experts" (independent feed-forward networks). For each token, a gating network selects a small subset (typically top-1 or top-2) of experts to process it.
...
::: {#fig-moe-all-to-all-routing fig-env="figure" fig-pos="htb" fig-cap="**Mixture-of-experts (MoE) AllToAll routing**: Expert parallelism requires a unique \"All-to-All\" communication pattern. Tokens from a local batch are routed by a gating network to experts sharded across the cluster. This involves a global shuffle where every GPU sends tokens to and receives tokens from potentially every other GPU, stressing the cluster's bisection bandwidth." fig-alt="Diagram showing tokens being routed from a gating network to four different experts sharded across multiple GPUs, then returning to the original sequence order."}
...
The sparse communication pattern distinguishes recommendation and MoE workloads (Archetype B (DLRM at Scale)) from dense LLM training (Archetype A (GPT-4/Llama-3)) (@sec-vol2-introduction-archetypes).
...
To see why the combination matters, consider three parallelism configurations for training the `{python} FrontierHybridRecap.frontier_params_b_str` model on a 1,024-GPU cluster organized as 128 nodes of 8 GPUs each. The progression from pure data parallelism through tensor and pipeline parallelism makes the efficiency gain concrete.

Configuration A: Pure Data Parallelism (DP-1024) is a communication counterfactual rather than a feasible placement for this model. The FP16 weights alone occupy 350 GB, so an A100 or H100 cannot replicate them; sharding only optimizer states does not change that fact. If replication were possible, all 1,024 ranks would form one data-parallel group, and each rank would participate in an AllReduce of the 350 GB gradient tensor across the InfiniBand fabric. The 37.5 percent calculation in @sec-distributed-training-systems-systems-distributed-training-efficiency-metrics-9488 isolates that communication cost.
...
Training a `{python} FrontierHybridRecap.frontier_params_b_str`-parameter model requires **3D parallelism**\index{3D Parallelism!definition}: the coordinated composition of data, pipeline, and tensor parallelism across thousands of devices.[^fn-three-dimensional-parallelism] This approach does not merely sum the benefits of individual parallelism strategies; it composes them geometrically to match the physical topology of the hardware.

[^fn-three-dimensional-parallelism]: **3D parallelism**\index{3D Parallelism!etymology}: Named after the three orthogonal axes of decomposition: (1) Data Parallelism (batch), (2) Pipeline Parallelism (depth), and (3) Tensor Parallelism (layer width). Organizations visualize their training fleets as a 3D grid $(d, p, t)$, where the product $N_{\text{total}} = d \times p \times t$ equals the total GPU count. This geometric perspective is essential for balancing the tiered bandwidth constraints of high-bandwidth clusters.
...
# │ Show: The optimizer converges on TP=8, PP=2, DP=512 for 175B on 8K GPUs.
...
from mlsysim.fmt import fmt_percent, fmt, check, fmt_memory_capacity, fmt_qty, fmt_params
...
::: {#exmp-distributed-training-automated-design-space-search-tier-3-optimizer .callout-example title="Automated design space search (tier 3 optimizer)"}

**Scenario**: An engineering team schedules a `{python} AutoParallelism.params_b_str`-parameter model across `{python} AutoParallelism.gpu_count_str` H100 GPUs (`{python} AutoParallelism.hbm_gb_str` HBM each), navigating the multi-dimensional 3D-parallelism search space (TP $\times$ PP $\times$ DP).

**Diagnosis**: Manual trial-and-error risks memory overflow (exceeding `{python} AutoParallelism.hbm_gb_str` HBM) or saturating NVLink. The `ParallelismOptimizer` evaluates all factorizations, selecting TP=`{python} AutoParallelism.opt_tp_str`, PP=`{python} AutoParallelism.opt_pp_value_str`, and DP=`{python} AutoParallelism.opt_dp_value_str` to maximize MFU without exceeding HBM capacity.

**Systems lesson**: Automated design-space search formalizes empirical heuristics: tensor parallelism should be confined within single-node NVLink bounds (TP=`{python} AutoParallelism.opt_tp_str`), while pipeline parallelism (PP=`{python} AutoParallelism.opt_pp_value_str`) should be minimized to avoid pipeline bubble overhead while satisfying memory constraints, achieving `{python} AutoParallelism.opt_mfu_str` projected MFU.
...
The MFU values need a historical utilization baseline to show how published systems approached 50 percent utilization. @Fig-mfu-progression traces the evolution of MFU\index{Model FLOPs Utilization!distributed training} across published training systems from 2020 to 2024. The progression from GPT-3's 21 percent MFU to PaLM's 46 percent MFU reflects not improvements in raw hardware speed but advances in the parallelism strategies, communication overlap techniques, and scheduling optimizations discussed throughout this chapter. The plateau near 40--46 percent reveals that the theoretical ceiling imposed by communication overhead, pipeline bubbles, and memory management remains formidable even in highly optimized hybrid-parallel systems. Notably, Meta's Llama 3 training at 16,384 H100 GPUs achieved slightly lower MFU (41 percent) than the same model at 8,192 GPUs (43 percent), confirming that the scaling tax described in @sec-distributed-training-systems-systems-physics-scaling-amdahls-law-communication-4d7f is not merely theoretical but measurable in large published runs.

::: {#fig-mfu-progression fig-env="figure" fig-pos="htb" fig-cap="**The MFU progression**: Model FLOPs Utilization across published training systems, ordered chronologically. MFU doubled from 21 percent (GPT-3, 2020) to 46 percent (PaLM, 2022) through advances in hybrid parallelism and communication overlap, then plateaued near 40--45 percent for large-scale runs. The Llama 3 data points illustrate the scaling tax: MFU drops from 43 percent at 8,192 GPUs to 41 percent at 16,384 GPUs as communication overhead grows with cluster size. Data sources: @chowdhery2022palm, @dubey2024llama." fig-alt="Horizontal bar chart of MFU percentages for six training systems from 2020 to 2024 showing progression from 21 percent to 46 percent with a plateau near 41 to 43 percent."}
...
from mlsysim.fmt import fmt, fmt_count, fmt_count_range, fmt_memory_capacity, fmt_qty, fmt_memory, fmt_params
from mlsysim.physics import memory_from_params
import math
...
class DistTrainH100Capacity:
    ...
    min_gpus_practical_low = 64
    min_gpus_practical_high = 128
    min_gpus_practical_str = fmt_count_range(
        min_gpus_practical_low,
        min_gpus_practical_high,
        commas=False,
        label="GPU",
    )
...
Standard pretraining involves one model, one loss function, and one gradient stream. RLHF alignment, by contrast, orchestrates a system of models that interact during every training step. In PPO\index{Proximal Policy Optimization!model asymmetry} [@schulman2017proximal], as used in InstructGPT-style RLHF systems [@ouyang2022instructgpt], four distinct models must operate in concert. @Tbl-rlhf-model-coordination-roles separates those models by role, execution mode, and memory burden.

| **Model**                                  | **Role in PPO-style RLHF**                                                        | **Execution mode**                                                | **Memory burden**                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|:-------------------------------------------|:----------------------------------------------------------------------------------|:------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Policy Model**\index{Policy Model}       | The model being aligned and optimized.                                            | Training mode with gradients, optimizer moments, and activations. | For a `{python} DistTrainH100Capacity.policy_params_b_str`-parameter model in mixed precision with Adam, memory is approximately `{python} DistTrainH100Capacity.policy_params_b_str` $\times$ (`{python} DistTrainH100Capacity.bytes_weights_str` $+$ `{python} DistTrainH100Capacity.bytes_grad_str` $+$ `{python} DistTrainH100Capacity.bytes_adam_str`) = `{python} DistTrainH100Capacity.policy_train_gb_str`, the same budget as standard pretraining. |
| **Reference Model**\index{Reference Model} | A frozen copy of the pretrained policy that supplies the KL-divergence penalty.   | Inference mode only.                                              | FP16/BF16 parameters without gradients or optimizer state; a 70B model needs about 70B $\times$ 2 = 140 GB, roughly 8$\times$ less than the training configuration.                                                                                                                                                                                                                                                                                          |
| **Reward Model**\index{Reward Model}       | A preference-trained scorer that produces scalar rewards for generated sequences. | Inference mode only.                                              | Often smaller than the policy; a 13B reward model requires approximately 26 GB in FP16, but it must process every generated sequence.                                                                                                                                                                                                                                                                                                                        |
| **Value Model**\index{Value Model}         | The PPO critic that estimates expected future reward.                             | Training mode, either separate or sharing a policy backbone.      | A full-size separate critic can add another `{python} DistTrainH100Capacity.policy_train_gb_str`; smaller models or shared-backbone designs reduce this to 200--400 GB.                                                                                                                                                                                                                                                                                      |
...
class RLHFBudget:
    ...
    ppo_min_gpus_str = fmt_count_range(64, 128, commas=False, label="GPU")
    dpo_min_gpus_str = fmt_count_range(32, 64, commas=False, label="GPU")
    ppo_mfu_range_str = fmt_percent_range(ppo_mfu_lo / 100, ppo_mfu_hi / 100, precision=0, style="symbol")
    dpo_mfu_range_str = fmt_percent_range(dpo_mfu_lo / 100, dpo_mfu_hi / 100, precision=0, style="symbol")
```
...
| **Minimum GPUs (memory)**  |       `{python} RLHFBudget.ppo_min_gpus_str`      |       `{python} RLHFBudget.dpo_min_gpus_str`      |
| **Generation phase**       |             Yes (sequential, BW-bound)            |                        None                       |
| **Effective training MFU** |      `{python} RLHFBudget.ppo_mfu_range_str`      |      `{python} RLHFBudget.dpo_mfu_range_str`      |
...
| **Tensor parallelism**                               | One layer or tensor operation exceeds local capacity   | Reduce or gather partial layer results      | Keep tightly coupled partitions on fast local links        | Layer latency stays below activation handoff cost    |
| **Pipeline Parallelism**\index{Pipeline Parallelism} | Layer stack fits only when split by stage              | Activations and gradients between stages    | Balance stages and live microbatch activations             | Bubble and peak activation memory meet budget        |
...
| Hybrid parallelism\index{Hybrid Parallelism}         | More than one constraint binds at once                 | Multiple collectives on coupled axes        | Satisfy divisibility, batch, topology, and failure domains | MFU improves within memory and network budgets       |
...
At the data-parallel layer, the simplest distributed training abstraction wraps a model so that the framework automatically replicates it across available accelerators, splits each batch, and averages gradients after the backward pass. Historically, parameter-server systems organized this work with server nodes that store shared parameters while workers push and pull updates [@li2014]. As @sec-distributed-training-systems-systems-parameter-servers-embedding-sharding-8821 established, that design concentrates gradient bandwidth at the server tier, so dense synchronous training favors decentralized collectives.

Large-scale data parallelism removes the central reducer by assigning every worker a symmetric role in AllReduce. The training framework defines process groups, gradient buckets, and the point in backpropagation when each reduction becomes ready; the collective runtime then maps those reductions onto ring, tree, or hierarchical algorithms. This chapter owns the training-side contract: bucket order and overlap determine which communication remains exposed on the step's critical path. @Sec-collective-communication owns the algorithms and fabric cost models, while @sec-performance-engineering owns the measurement workflow for proving that overlap improved rather than merely moved the bottleneck.
...
The primitives compose into communication schedules specific to each strategy. Data parallelism ordinarily reduces gradient buckets once per optimizer step. Full-shard FSDP with resharding invokes parameter AllGathers around layers and ReduceScatter during backward, while tensor parallelism places reductions or gathers inside transformer blocks. The training design therefore determines message size, readiness order, and frequency; @sec-collective-communication determines how the collective moves those messages, and @sec-performance-engineering determines how to measure the resulting critical path. *No framework abstraction changes the underlying physics*: distributed-training efficiency ultimately depends on interconnect bandwidth, memory capacity, synchronization latency, and the useful overlap admitted by the computation graph.
...
Engineers tune hyperparameters on 8-GPU workstations then deploy to 256-GPU clusters expecting identical behavior. At scale, convergence patterns change fundamentally. The most critical hyperparameter is learning rate: as @sec-distributed-training-systems-systems-data-parallelism-6132 explains, batch size increases proportionally with GPU count in data parallelism, requiring learning rate adjustments. The "linear scaling rule"[^fn-linear-scaling-efficiency] [@goyal2017accurate] suggests $\eta_{\text{large}} = \eta_{\text{base}} \times (B_{\text{large}}/B_{\text{base}})$, but this relationship holds only within bounds. As models scale, they eventually encounter the critical batch size,[^fn-critical-batch-size] where adding more data per step yields diminishing returns in convergence.

[^fn-critical-batch-size]: **Critical batch size**\index{Critical Batch Size!convergence limit}: The gradient-noise-scale view introduced by @mccandlish2018empirical treats the largest useful batch size as a measurable statistic that varies by domain, model, optimizer, and training phase. Scaling beyond that measured point improves hardware throughput but does not proportionally reduce the number of optimization steps needed to reach target quality, collapsing the scaling efficiency $(\eta_{\text{scaling}})$.

[^fn-linear-scaling-efficiency]: **Linear scaling rule**\index{Linear Scaling Rule!convergence}: Established by @goyal2017accurate at Facebook AI Research, who trained ResNet-50 on ImageNet in one hour across 256 GPUs with a batch size of 8,192 while matching small-batch accuracy. The rule -- multiply learning rate by $k$ when batch size increases by $k$ -- works only while the large-batch approximation remains valid. Above the workload's measured critical-batch-size regime, gradient noise drops below the useful signal scale and additional parallelism yields diminishing convergence returns [@mccandlish2018empirical; @Shallue2019measuring].
...
Engineers checkpoint distributed training "every hour" or "every 1000 steps" based on intuition rather than analysis. @Sec-fault-tolerance-reliability derives the Young-Daly checkpoint law\index{Young-Daly Checkpoint Law!optimal interval} (principle \ref{pri-young-daly}); the training lesson here is that checkpoint cadence is not a constant. It depends on the mathematical relationship between checkpoint write cost and failure rate.
...
class YoungDaly:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # T_opt = sqrt(2 * C * MTBF), shared helper keeps this formula canonical.
    t_opt = calc_young_daly_interval(t_write_min * minute, cluster_mtbf_hr * hour)
    t_opt_min_val = t_opt.to(minute).magnitude
    t_opt_hr_val = t_opt.to(hour).magnitude

    # Overhead: (C/T_opt) + (T_opt / 2 * MTBF)
    # (Simplified Young-Daly first-order approximation)
    ckpt_overhead = t_write_min / t_opt_min_val
    # Both operands in hours: T_opt/2 divided by the cluster MTBF.
    rework_overhead = (t_opt_hr_val / 2) / cluster_mtbf_hr
    ...
    safe_overhead = (t_write_min / safe_interval_min) + (safe_interval_min / (2 * 60 * cluster_mtbf_hr))
...
The 3D Parallelism Cube (@fig-3d-parallelism-cube-summary) is a useful framework for scaling large models. Data parallelism unrolls the outer loop of training to scale throughput; tensor parallelism vectorizes the inner loops of matrix multiplication to fit memory; and pipeline parallelism stages sequential layers to reduce communication frequency. Together, these strategies map models larger than any single memory bank onto a fleet of accelerators with finite bandwidth.

::: {#fig-3d-parallelism-cube-summary fig-env="figure" fig-pos="htb" fig-cap="**3D parallelism strategy space**: An 8-GPU configuration fills a $(d,p,t)$ coordinate grid that combines data parallelism (replicas), pipeline parallelism (layer stages), and tensor parallelism (within-layer splits). One training step coordinates $d \times p \times t$ GPUs, with communication occurring within layers, at microbatch stage boundaries, and once per optimizer step, respectively." fig-alt="Flat 8-GPU coordinate-grid layout with each GPU labeled by its (d, p, t) coordinate showing how the three parallelism axes combine in a concrete configuration."}
...
from mlsysim import Models
from mlsysim.core.units import GB, second
from mlsysim.fmt import fmt, MarkdownStr, fmt_qty, fmt_memory
...
| **Archetype C (Federated MobileNet)** |         Federated Learning        | The same logic extends to coordinating on-device MobileNet updates across unreliable, privacy-constrained edge devices; @sec-edge-intelligence develops this regime. |

: **Partitioning strategy by archetype**: The operating point in the 3D-parallelism cube shifts with the dominant bottleneck. {#tbl-partitioning-archetypes}
...
* Sharding buys memory with messages: ZeRO and FSDP make 100B+ parameter models feasible by partitioning optimizer state, gradients, and parameters (principle \ref{pri-memory-capacity-gap}). The price is a stricter communication schedule of ReduceScatter and AllGather operations that must be overlapped or hidden.
...
* Pipeline parallelism trades bytes for bubbles: Layer staging reduces communication frequency, but fill and drain slots leave accelerators idle (principle \ref{pri-distributed-step-time}). Microbatching with $m \gg p$ is the mechanism that turns model depth into throughput rather than pipeline slack.
* The 3D cube is a hardware map: Real frontier training combines data, tensor, pipeline, expert, and sharded parallelism because no single axis fits the model and the fleet (principle \ref{pri-universal-scaling}). Logical groups must map to HBM, NVLink, InfiniBand, and failure domains together.

:::

Beneath the cube and its three axes lies a single trade. No way of splitting a model makes the underlying work smaller; each only moves it, turning a memory limit into a communication cost or a communication cost into idle time. Data, tensor, and pipeline parallelism spend communication to win the memory headroom no single accelerator has, and coordination is the tax paid to keep the split consistent. This is the displacement of overhead (principle \ref{pri-conservation-overhead}), the law the rest of the volume turns on: the execution tax of scale cannot be removed, only relocated among compute, communication, and coordination. The cube is the map of where the tax can be sent; the engineering is choosing the destination that binds least.

::: {.callout-chapter-connection title="From logic to traffic"}

Splitting the mathematics defines the *logical* traffic patterns of the Machine Learning Fleet. These patterns eventually reach the *physical* wires of the data center. @Sec-collective-communication opens the black box of the collective operations (AllReduce, AllToAll) that preserve consistency, moving from what must be sent to how it is routed through rings, trees, and rails.
