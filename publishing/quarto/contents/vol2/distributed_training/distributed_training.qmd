...
::: {#fig-3d-parallelism-cube fig-env="figure" fig-pos="htb" fig-cap="**The 3D Parallelism Cube**: A conceptual visualization of three distinct partitioning axes: Data Parallelism (replicating the model), Pipeline Parallelism (splitting depth), and Tensor Parallelism (splitting layers). Large-model training occupies a feasible point $(d, p, t)$ within this cube only when divisibility, memory, batch, topology, and failure-domain constraints are satisfied together." fig-alt="3D coordinate system with three axes: Data Parallelism (replica count d), Pipeline Parallelism (depth p), and Tensor Parallelism (width t). Dashed cube marks a feasible configuration point with Total Accelerators equals d times p times t."}
