...
::: {#fig-perf-eng-memory-wall fig-env="figure" fig-pos="htb" fig-cap="**The ridge point moves right**: Peak compute and memory bandwidth normalized to the V100. Compute grows about twice as fast as bandwidth, which pushes the roofline ridge point toward higher arithmetic intensity and leaves more kernels memory-bound each generation. Specifications come from the MLSysIM hardware registry." fig-alt="Line chart normalized to the V100. The compute line rises to 18 times and the bandwidth line to 8.9 times, so the ratio that sets the roofline ridge point doubles."}
...
data = pd.read_csv("data/memory_wall_trend.csv")
base = data.iloc[0]
