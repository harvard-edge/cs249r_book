...
::: {#fig-model-registry fig-env="figure" fig-pos="htb" fig-cap="**Dependency-aware model registry**: The diagram shows a registry that tracks both *artifacts* and the *graph* of dependencies between models. An update to the 'User Embedding Model' triggers alerts or automated retraining for dependent 'Ranking' and 'Retrieval' models, preventing silent downstream failures." fig-alt="Dependency graph with User Embedding v3.2 at top connected by dashed arrows to three downstream models: Retrieval, Ranking, and Ensemble. Red alert box indicates update triggers auto-retrain."}
