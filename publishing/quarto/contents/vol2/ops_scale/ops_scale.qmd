...
- Evaluate monitoring hierarchies and multidimensional reliability budgets using signal quality, burn rate, ownership, and release consequences
- Assemble an end-to-end evaluation contract with reproducible evidence, acceptance authority, and invalidation rules
...
The drive for efficiency forces capacity to behave like a dynamic resource rather than a static allocation. Organizations must decide when to buy additional permanent capacity and when to recover the same effective throughput through better orchestration or model optimization. The decision also has an energy dimension: powering thousands of GPUs for months consumes enough electricity that financial cost and carbon cost move together. Capacity planning therefore becomes a strategic lever that balances performance, budget, energy use, and responsible engineering in the same decision. Cost visibility also sharpens the case for paying down technical debt: the same metrics that reveal runaway training costs expose the hidden cost of unversioned data, brittle pipelines, and manual toil. The planning horizon is short because the workload itself is not stationary: the compute behind each successive landmark model in @fig-compute-scaling grows faster than any procurement cycle, so a fleet sized for today's flagship is undersized before it is depreciated.

::: {#fig-compute-scaling fig-env="figure" fig-pos="htb" fig-cap="**Historical training compute scaling**: Total training compute for landmark models from 2012 to 2024, spanning roughly eight orders of magnitude along a near-straight line on a log axis. Capacity planning inherits that slope, which is why reserved fleets are sized against a moving target rather than a fixed workload." fig-alt="Log-scale scatter of total training compute in FLOPs by year from AlexNet in 2012 to Llama-3-400B in 2024, with a fitted exponential trend line the points track closely."}
...
data = pd.read_csv("data/training_compute_flops.csv")
data = data.sort_values(by="Year")

plt.style.use('seaborn-v0_8-whitegrid')
fig, ax, COLORS, plt = viz.setup_plot(figsize=(9, 5.5))

ax.scatter(data['Year'], data['FLOPs'], color='#2980b9', s=80, alpha=0.8, zorder=5)
...
ax.set_yscale('log')
ax.set_title("Training compute scaling (FLOPs)", fontsize=14, fontweight='bold')
ax.set_xlabel("Year")
ax.set_ylabel("Total Compute (FLOPs)")
ax.set_xlim(2011.5, 2025.5)

# Fit log-linear trend
z = np.polyfit(data['Year'], np.log10(data['FLOPs']), 1)
p = np.poly1d(z)
x_trend = np.linspace(2011.5, 2025, 100)
ax.plot(x_trend, 10**p(x_trend), color='#e74c3c', linestyle='--', alpha=0.5, label='Exponential Trend')

plt.legend(loc='lower right', frameon=True, facecolor='white', framealpha=0.95, edgecolor='gray', fancybox=True)
...
![](images/svg/_model-dependency-graph.svg)
