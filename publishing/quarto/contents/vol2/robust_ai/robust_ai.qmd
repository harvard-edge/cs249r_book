...
::: {#fig-adversarial-robustness-trend fig-env="figure" fig-pos="htb" fig-cap="**Adversarial robustness vs. standard accuracy**: Historical progress on CIFAR-10 classification against $\ell_\infty$ ($\epsilon=8/255$) perturbations. While standard test accuracy (top curve) rapidly saturated above 95 percent, robust accuracy (bottom curve) has required fundamental advances in adversarial training and generated data to progress, demonstrating the persistent gap between average-case and worst-case performance. Data from RobustBench." fig-alt="Two-line chart from 2017 to 2024 comparing standard accuracy, which rises from about eighty-seven to ninety-five percent, against robust accuracy under an L-infinity attack, which rises from about forty-six to seventy-eight percent. The persistent vertical gap is the robustness deficit."}
...
data = pd.read_csv("data/cifar10_robustness_trend.csv")
...
ax.plot(data["Year"], data["Robust_Accuracy"], marker="s", linewidth=2, label="Robust Accuracy ($\ell_\infty$)")
