...
\noindent
![](images/png/cover_inference_blueprint_labeled_print.png){fig-alt="Global inference blueprint where requests route through regional gateways, batching systems, cache pages, model shards, and fleet serving workers."}
:::

::: {.content-visible unless-format="pdf"}
![](images/webp/cover_inference_blueprint_labeled.webp){fig-alt="Global inference blueprint where requests route through regional gateways, batching systems, cache pages, model shards, and fleet serving workers."}
:::
...
class InferenceSetup:
    ...
    gpt4_weights_tb_str = fmt_qty(gpt4_weights_fp16, TB, precision=1, commas=False, approx=True)
```
...
The first signal is memory exhaustion, which occurs when model parameters, key-value caches, or embedding tables exceed single-device capacity. A single NVIDIA H100 GPU provides `{python} InferenceSetup.h100_mem_gb_str` of HBM3[^fn-hbm3-memory-bandwidth] memory; @sec-appdx-fleet-assumption-provenance records the provenance of this capacity figure and the other canonical hardware constants used throughout the analysis that follows. GPT-4-class models dwarf that capacity: the public ~`{python} InferenceSetup.gpt4_params_t_str`-parameter mixture-of-experts estimate implies `{python} InferenceSetup.gpt4_weights_tb_str` for weights alone in FP16 precision, forcing distribution across multiple GPUs regardless of throughput requirements. Recommendation systems with trillion-parameter embedding tables face similar constraints: Meta's DLRM architecture[^fn-dlrm-recommendation-scale] stores embedding tables that require multiple terabytes of memory.
...
[^fn-dlrm-recommendation-scale]: **DLRM (Deep Learning Recommendation Model)**\index{DLRM!scale}: Meta's 2019 reference architecture separates dense features -- GPU-bound multilayer perceptrons (MLPs) -- from sparse features (CPU-bound embedding lookups), creating a hybrid serving topology [@naumov2019deep]. Production recommendation characterizations show why this structure matters for serving: embedding-heavy recommendation models create memory-access and capacity constraints that differ from dense convolutional neural network (CNN), recurrent neural network (RNN), or large language model (LLM) inference [@gupta2020architectural].
...
|:---------------|:---------------------------:|:----------------------------------------------------------:|:---------------------------:|
| **Memory**     |         80 GB (H100)        | GPT-4 (`{python} InferenceSetup.gpt4_weights_tb_str` FP16) | Tensor/pipeline parallelism |
...
Memory, throughput, and latency triggers all become user-visible once an interactive endpoint starts streaming. **Time to First Token (TTFT)**\index{Time to First Token}[^fn-ttft-tpot-metrics] determines when the user sees a response begin, while **Time Per Output Token (TPOT)**\index{Time Per Output Token} determines whether the rest of the response arrives at a readable pace; this is why inference reverses many of the optimization priorities from training.

[^fn-ttft-tpot-metrics]: **TTFT versus TPOT**\index{Time to First Token!vs. time per output token}: Time to first token measures the initial wait through queueing and prefill; time per output token measures the cadence of decode after streaming begins. A serving policy must protect both rather than hiding a slow stream behind a fast first response.
...
### Serving cost can dominate training cost {#sec-inference-scale-serving-cost-dominates-training-cost-576a}
...
# │
# │ Goal:    Show that annual serving (OpEx) dominates one-time training (CapEx).
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_usd, check, fmt, fmt_percent, fmt_count, fmt_math
...
**Systems insight**: Serving costs `{python} ServingCostSimple.serving_training_ratio_mult_str` more than training in the first year alone. A `{python} ServingCostSimple.efficiency_gain_pct_str` serving-cost or latency-linked efficiency improvement saves about `{python} ServingCostSimple.efficiency_savings_str` in the first year, nearly paying for the original training run.
...
: **Illustrative training vs. serving cost ratios**: Scenario ranges show how request volume, model cost, and service lifetime can make recurring serving expense exceed one-time training expense. These ranges are sensitivity assumptions, not representative prices; a deployment must substitute its measured traffic and accounting boundary. {#tbl-cost-ratios}
...
# ┌── 3. RENDER ────────────────────────────────────────────────────────────────
ax.plot(months, cum_a_serve, color=COLORS["BlueLine"], linewidth=2.5, label="A: Small model, low traffic (\\$50K train, \\$5K/mo serve)")
ax.plot(months, cum_b_serve, color=COLORS["OrangeLine"], linewidth=2.5, label="B: 70B LLM, 1M DAU (\\$2M train, \\$1.5M/mo serve)")
ax.plot(months, cum_c_serve, color=COLORS["RedLine"], linewidth=2.5, label="C: RecSys, 100M DAU (\\$12K train, \\$250K/mo serve)")
...
ax.annotate(
    ...
    va="bottom",
    xytext=(16, 0.25),
    arrowprops=dict(arrowstyle="->", color=COLORS["BlueLine"], lw=0.75),
)
...
A common misconception equates inference at scale with LLM serving. Large language models present distinctive challenges and attract attention, but they are only one part of production inference. In large consumer platforms, recommendation and ranking workloads can dominate AI inference cycles and capacity [@gupta2020architectural; @hazelwood2018]. Vision and image processing, NLP and LLM workloads, fraud detection, ads, and other classification tasks fill out the remainder. @Tbl-inference-landscape compares these model types by serving pressure and optimization challenge. Recommendation binds on embedding lookup, vision on batch efficiency, LLMs on memory bandwidth, and speech on sequential decode. No single optimization serves all four.
...
The distribution has direct implications for technique selection. Recommendation systems have driven important production inference innovations: dynamic batching, embedding sharding, feature store architectures, and low-latency serving were all developed primarily for recommendation workloads [@naumov2019deep; @gupta2020architectural]. LLM-specific techniques like continuous batching and KV cache management address a different slice of production inference. Text-to-image systems such as DALL-E provide one multimodal model example [@ramesh2021zero], but multimodal serving volume and latency targets remain product-specific.
...
Autoregressive language models break this assumption. Each request generates a different number of output tokens, so static batching forces all requests to wait for the longest generation in the batch and can leave accelerator capacity idle after shorter requests finish. Continuous batching solves this by allowing requests to enter and exit the batch at each decoding step, an iteration-level scheduling approach introduced by Orca [@yu2022orca]. When one request finishes generation, a new request immediately fills its slot. Systems like vLLM[^fn-vllm-etymology-paging] combine this scheduling style with PagedAttention-based KV-cache management, achieving 2--4$\times$ throughput gains over prior LLM serving systems by reducing memory waste [@kwon2023].

[^fn-vllm-etymology-paging]: **vLLM and virtual memory**\index{vLLM!etymology}: The name signals its operating-system analogy: logical KV-cache pages are decoupled from their physical locations in GPU memory.
...
1.  **Significance**: It reduces per-token attention compute from $O(n^2)$ to $O(n)$ in the sequence length, but its footprint grows linearly with both context length and batch size, often consuming more HBM than the model weights themselves and becoming the binding constraint on serving capacity. @Sec-appdx-inference-scale-kv-cache-fundamentals-2ccd derives the fundamental sizing math.
...
Multi-GPU tensor parallelism shards each layer's weight matrices across multiple GPUs connected by high-bandwidth interconnect (NVLink at `{python} InferenceTopologyRecap.nvlink_h100_gb_per_s_str` bidirectional [450 GB/s per direction] on H100). Each GPU computes a partial result for every layer, and an all-reduce operation synchronizes the partial results before the next layer. This topology is necessary when model weights exceed single-device memory and provides latency reduction proportional to the parallelism degree, at the cost of communication overhead per layer. The model sharding section (@sec-inference-scale-model-sharding-inference-82b3) analyzes the communication overhead quantitatively.

Disaggregated serving separates the prefill phase (processing the input prompt, which is compute-bound) from the decode phase (generating tokens one at a time, which is memory-bandwidth-bound) onto different hardware pools optimized for each workload profile. Prefill nodes use high-FLOP/s accelerators; decode nodes use high-bandwidth-memory configurations. Separation removes prefill/decode interference and permits independent parallelism and capacity plans, but it also places a state transfer on the critical path [@zhong2024distserve]. Chunked prefill provides the contrasting colocated approach: it bounds interference without paying a network transfer [@agrawal2024sarathi].

The disaggregation decision therefore depends on a quantitative contract, not on the phase labels alone. If prefill produces $S_{\mathrm{KV}}$ bytes of attention state and the usable inter-pool bandwidth is $B_{\mathrm{net}}$, then the handoff consumes at least

$$T_{\mathrm{handoff}} = L_{\mathrm{net}} + \frac{S_{\mathrm{KV}}}{B_{\mathrm{net}}},$$ {#eq-prefill-decode-handoff}

where $L_{\mathrm{net}}$ includes transfer setup and queueing. @Eq-prefill-decode-handoff makes placement part of the service contract: the prefill pool must sustain the admitted prompt-token rate, the decode pool must sustain the admitted output-token rate, and both pools need headroom for the request-length distribution. The useful capacity metric is **SLO goodput**\index{Goodput!SLO-constrained}: the request arrival rate for which TTFT and TPOT both remain within their targets. Disaggregation earns its transfer tax only when eliminating phase interference and balancing the two pools raises SLO goodput. Otherwise, colocated chunked prefill is the simpler design.
...
An autoregressive request is stateful while it is generating because its KV cache belongs to that in-flight sequence. Cross-request statefulness is optional. An API may send the full conversation history with every turn and rebuild KV state during prefill, making any replica eligible for the next request. A serving platform may instead retain a completed turn's KV state as a session cache, reducing repeated prefill at the cost of replica affinity, cache lifetime management, and recovery complexity. Sticky routing is an optimization for that retained cache, not a correctness requirement of autoregressive models.

The scaling divide is therefore an interface and cache-policy decision. Every live generation needs request-scoped state, but the service decides whether state survives the request boundary. Stateless cross-turn service pays repeated prefill and fails over cheaply; session-cached service saves prefill work but must drain, migrate, or rebuild cached state during scaling and failure. The architecture should choose affinity only when measured cache-reuse value exceeds the resulting imbalance and recovery costs.
...
In large consumer platforms, recommendation systems can constitute the majority of AI inference cycles or capacity pressure, with vision, language, ranking, fraud, ads, and other classification workloads sharing the remainder [@gupta2020architectural; @hazelwood2018]. Despite this distribution, batching strategies are presented in order of conceptual complexity: vision (straightforward batching), LLMs (continuous batching with KV cache), and recommendation (feature-parallel batching with distributed embedding). This ordering builds understanding progressively, even though production systems frequently encounter recommendation workloads first. The taxonomy that follows matches batching strategies to model characteristics, providing quantitative analysis of when each approach applies and what performance to expect.
...
![](images/svg/batching-strategies.svg){width="100%"}
:::
...
For recommendation systems, the bottleneck is often embedding lookup rather than dense computation. Batching strategies must optimize for parallel embedding access patterns rather than matrix multiplication throughput. Decode therefore does not inherit the headline compute number. @Fig-gpu-memory-wall separates the two growth rates, and a token-by-token decode step tracks the slower one, which is why the same generational upgrade that doubles prefill throughput can leave decode nearly unchanged.

::: {#fig-gpu-memory-wall fig-env="figure" fig-pos="htb" fig-cap="**Decode inherits the bandwidth deficit**: Peak compute and memory bandwidth for four accelerator generations, normalized to the V100. Compute rises 18$\times$ against bandwidth's 8.9$\times$, and because decode reads the full weight set once per token it advances at the lower of the two rates. Specifications come from the MLSysIM hardware registry." fig-alt="Line chart normalized to the V100 showing peak compute reaching 18 times and memory bandwidth 8.9 times by the B200, the gap that bounds decode throughput."}
...
data = pd.read_csv("data/gpu_memory_wall.csv")
base = data.iloc[0]
...
1. **Vision Model** $(T_{\text{fixed}} = 2\text{ ms}, T_{\text{var}} = 1\text{ ms})$:
   - Knee: $B \approx 2\text{ ms}/1\text{ ms} =$ `{python} BatchingEfficiency.bec_vision_knee_str`.
...
2. **LLM Decode** $(T_{\text{fixed}} = 40\text{ ms}, T_{\text{var}} = 0.5\text{ ms})$:
   - Knee: $B \approx 40\text{ ms}/0.5\text{ ms} =$ `{python} BatchingEfficiency.bec_llm_knee_str`.
...
Translating these batch-level equations into system-wide capacity planning requires a classical result from queuing theory: **Little's Law**\index{Little's Law!capacity planning},[^fn-littles-law-queuing] which relates concurrency, throughput, and latency in any stable system. Across model architectures, @tbl-batching-by-model shows that batching choice follows the binding bottleneck: compute for vision models, memory capacity or bandwidth for LLM prefill and decode, embedding lookup for recommendation systems, and real-time latency for speech.

[^fn-littles-law-queuing]: **Little's Law**: From queuing theory, $Q_{\text{req}} = \lambda_{\text{arr}} T_{\text{lat}}$, so concurrency equals arrival rate times latency. For a serving fleet, this defines the capacity envelope: if a GPU can handle 32 concurrent requests and each takes 100 ms, the maximum arrival rate is 320 req/s. Beyond this, queues grow rapidly as utilization approaches 1, and tail latency $(L_{\text{lat}})$ explodes.
...
: **Batching strategy by model type**: Each model class requires a distinct batching policy (from static/dynamic in vision to continuous in LLM decode and feature-parallel in recommendation) matched to its dominant hardware bottleneck. {#tbl-batching-by-model}
...
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt_int, check, fmt_time, fmt_math, fmt_percent
...
class LittlesLawExample:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    required_concurrency_math = fmt_math(rf"Q_{{\text{{req}}}} = {target_throughput} \times {latency_slo_s} =")
...
- **Latency SLO** $(T_{\text{lat}})$: `{python} LittlesLawExample.latency_slo_ms_str` (`{python} LittlesLawExample.latency_slo_s_str`)

**Required Concurrency** $(Q_{\text{req}})$: `{python} LittlesLawExample.required_concurrency_math` `{python} LittlesLawExample.required_concurrency_str` concurrent requests
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, fmt_percent, fmt_time, fmt_rate, fmt_math, fmt_ratio, check
...
class DynamicBatchingResNet50:
    ...
    svc_b32_ms = 25  # scenario assumption: total batch service time at batch=32
    ...
    per_replica_qps_str = fmt_rate(round(per_replica_qps), "QPS")
    ...
    svc_b32_ms_str = fmt_time(svc_b32_ms * millisecond, millisecond, precision=0, commas=False)
    bmax_b_str = fmt_int(bmax_b, commas=False)
    bmax_c_str = fmt_int(bmax_c, commas=False)
    ...
    rho_a_eq_math = fmt_math(
        rf"\rho_{{\text{{serv}}}} = \lambda_{{\text{{arr}}}} \times T_{{\text{{svc}}}} "
        rf"= {int(per_replica_qps)} \times {svc_b1_s} = {fmt_ratio(rho_a, precision=1)}"
    )
    eb_b_eq_math = fmt_math(
        rf"E[B] = \lambda_{{\text{{arr}}}} \times T_{{\text{{window}}}} " rf"= {int(per_replica_qps)} \times {twindow_b_s} = {round(eb_b)}"
    )
    eb_c_eq_math = fmt_math(rf"E[B] = {int(per_replica_qps)} \times {twindow_c_s} = {round(eb_c)}")
    rho_b_eq_math = fmt_math(
        rf"\rho_{{\text{{serv}}}} = {int(per_replica_qps)} \times {round(per_req_b_ms / 1000, 4)} " rf"= {fmt_ratio(rho_b, precision=1)}"
    )
    rho_c_eq_math = fmt_math(
        rf"\rho_{{\text{{serv}}}} = {int(per_replica_qps)} \times {round(per_req_c_ms / 1000, 4)} " rf"= {fmt_ratio(rho_c, precision=1)}"
    )
    capacity_gain_pct_str = fmt_percent(round(capacity_gain * 100) / 100, precision=0, commas=False, style="prose")
    util_drop_pct_str = fmt_percent(util_drop, precision=0, commas=False, style="prose")
    lat_b_ms_str = fmt_time(15 * millisecond, millisecond, precision=0, commas=False)
    lat_c_ms_str = fmt_time(22 * millisecond, millisecond, precision=0, commas=False)
```

::: {#exmp-dynamic-batching-resnet50 .callout-example title="Dynamic batching for ResNet-50"}

**Scenario**: A vision classification service receives requests at `{python} DynamicBatchingResNet50.arrival_qps_str` with a `{python} DynamicBatchingResNet50.latency_slo_ms_str` P99 latency SLO across `{python} DynamicBatchingResNet50.replicas_str` replicas.

**Diagnosis**: Serving requests individually (no batching) overloads the system (`{python} DynamicBatchingResNet50.rho_a_eq_math` utilization). Implementing dynamic batching windows (Option B: `{python} DynamicBatchingResNet50.twindow_b_ms_str`, Option C: `{python} DynamicBatchingResNet50.twindow_c_ms_str`) packs requests to lower per-request compute time, achieving up to `{python} DynamicBatchingResNet50.capacity_gain_pct_str` higher per-replica capacity within the latency SLO.
...
Autoregressive language models present a unique batching challenge that static and dynamic approaches handle poorly. The Orca system[^fn-orca-iteration-scheduling] [@yu2022orca] exposes the limitation. Traditional batching forces all sequences in a batch to complete before any new sequences can join, wasting compute when sequences finish at different times.

[^fn-orca-iteration-scheduling]: **Orca and iteration-level scheduling**\index{Orca!iteration-level scheduling}: Orca's terminology emphasizes that admission and eviction decisions occur between decode iterations, not only when an entire request-level batch completes [@yu2022orca].
...
\begin{algorithmic}
\Require waiting queue $Q$; active set $A$; KV-cache page allocator; active-token capacity $C_{\text{tok}}$; batched decode kernel
...
\While{the service is running}
    ...
    \State run one batched decode kernel over all sequences in $A$
    ...
    \If{HBM pressure exceeds the policy threshold}
        \State preempt low-priority sequences by paging their KV-cache to CPU DRAM
    \EndIf
...
The contrast in @fig-continuous-batching-comparison motivates the throughput analysis: static batching leaves slots idle until the longest sequence in a batch finishes, while continuous batching can refill those slots at iteration boundaries. Output-length variance creates the opportunity, but it does not determine the throughput gain by itself. The realized gain also depends on batch capacity, the full length distribution, request arrivals, KV-cache admission, prefill scheduling, and scheduler overhead. Consequently, there is no workload-independent gain formula based only on the coefficient of variation (CV), $\text{CV} = \sigma / \mu$; serving teams must compare policies under the same arrival trace, latency objective, and memory budget.

The implementation study in example \ref{exmp-inference-continuous-batching-vllm} examines how vLLM realizes continuous batching through iteration-level scheduling, paged memory management, preemption, throughput, and GPU utilization.
...
$$W = 1 - \frac{O_{\text{useful}}}{O_{\text{traditional}}} = 1 - \frac{\sum_{i=1}^{B} S_i}{B \times S_{\text{max}}} = 1 - \frac{\bar{S}}{S_{\text{max}}}$$ {#eq-waste-ratio}
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, fmt_percent, MarkdownStr, fmt_time, fmt_math
...
class BatchingWorkedExample:
    ...
    waste_eq_math = fmt_math(rf"W = 1 - \frac{{{int(mean_output)}}}{{{max_output}}} = 1 - {waste_ratio} = " + str(latency_reduction_pct_math_str))
...
The GPU performs `{python} BatchingWorkedExample.total_seq_iters_calc_math` `{python} BatchingWorkedExample.total_seq_iters_str` "sequence-iterations" but only `{python} BatchingWorkedExample.useful_seq_iters_str` are useful.
...
**Result**: Continuous batching reduces average latency by `{python} BatchingWorkedExample.latency_red_str`, exactly matching the waste ratio.

**Systems insight**: Continuous batching does not make the longest request faster; it prevents shorter requests from occupying finished slots while they wait for that longest request. The gain comes from reusing slots at iteration boundaries, which is the slot-reuse pattern shown in @fig-continuous-batching-comparison.
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt, fmt_percent, fmt_multiple
...
class A100HardwareScenario:
    ...
    a100_mem_bw_tb_per_s_str = fmt_bandwidth(bw, unit=TB / second, precision=2, commas=False)
...
**Scenario**: An LLM serving system exhibits high tail latency: P50 = `{python} A100HardwareScenario.p50_latency_ms_str` and P95 = `{python} A100HardwareScenario.p95_latency_ms_str` maintain SLO, but P99 spikes to $L_{\text{lat}} =$ `{python} A100HardwareScenario.p99_latency_ms_str` against a `{python} A100HardwareScenario.p99_target_ms_str` target despite GPU utilization $\eta_{\text{hw}} =$ `{python} A100HardwareScenario.gpu_utilization_pct_str`.

**Diagnosis**: The system runs across `{python} A100HardwareScenario.num_gpus_str` A100 GPUs linked via PCIe Gen4 (`{python} A100HardwareScenario.pcie_gen4_gb_per_s_str` per GPU) rather than NVLink. For `{python} A100HardwareScenario.activation_transfer_mb_str` tensor-parallel transfers, inter-GPU communication latency is $T_{\text{comm}} \approx$ `{python} A100HardwareScenario.pcie_transfer_ms_str` over PCIe versus `{python} A100HardwareScenario.nvlink_transfer_ms_str` over NVLink. The scheduling policy uses max batch size `{python} A100HardwareScenario.max_batch_size_str` and timeout `{python} A100HardwareScenario.batch_timeout_ms_str`, causing `{python} A100HardwareScenario.affected_batch_share_pct_str` of batches to experience head-of-line queuing delay $T_{\text{queue}}$: short prompt requests wait behind long-sequence generations.
...
This dropped P99 latency to $L_{\text{lat}} =$ `{python} A100HardwareScenario.p99_after_ms_str`.
...
Recommendation batching starts by grouping work around embedding locality rather than request shape. @Fig-feature-parallel-pipeline shows how the request is split into feature-specific paths before the dense ranking head recombines the retrieved representations.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @tbl-inference-recsys-latency-breakdown and RecSys batching callout.
# │
...
class RecsysLatencyBreakdown:
    ...
    platform_qps = 10_000_000  # scenario assumption: Meta-scale serving rate
    ...
    embedding_table_size = 100 * TB  # scenario assumption: sharded embedding corpus
...
**Scenario**: Meta-scale recommendation infrastructure serves `{python} RecsysLatencyBreakdown.platform_qps_str`, querying `{python} RecsysLatencyBreakdown.lookups_per_request_str` per request across `{python} RecsysLatencyBreakdown.embedding_shards_str`.

**Diagnosis**: Single-threaded per-request embedding lookups fail at `{python} RecsysLatencyBreakdown.lookups_per_shard_per_second_million_str`/sec per shard. Accumulating requests over a `{python} RecsysLatencyBreakdown.accumulation_window_ms_str` window forms batches of `{python} RecsysLatencyBreakdown.lookups_per_shard_per_batch_k_str` per shard, converting random memory reads into sequential accesses.

**Systems lesson**: Recommendation batching is organized around sparse feature locality rather than dense tensor shapes. Batching lookups at the embedding shard level maximizes memory bandwidth utilization within a `{python} RecsysLatencyBreakdown.total_ms_str` end-to-end latency budget.
...
- [ ] **Waste ratio**: explain why the "waste ratio" ($1 - \bar{S}/S_{\text{max}}$) collapses to zero when an LLM serving chat requests moves from static to continuous batching.
...
Noncompute stages (serialization, queuing, routing) consume a substantial fraction of the total latency budget (@fig-inference-lifecycle), requiring batching strategy selection to account for the full end-to-end pipeline, not GPU execution time alone. A decision tree guides strategy selection.
...
: **Batching strategy parameters**: Key configurable parameters (max batch size, queue timeout, iteration limits) tuned across static, dynamic, continuous, feature-parallel, and streaming strategies to optimize throughput vs. latency. {#tbl-batching-parameters}
...
Increasing the batch size to maximize throughput in LLM serving is bounded by the KV cache wall. Model weights represent a fixed "static tax" on GPU memory, while the KV cache grows linearly with both batch size and sequence length (@fig-kv-cache-wall).

::: {#fig-kv-cache-wall fig-env="figure" fig-pos="H" fig-cap="**The KV cache wall**: Total GPU memory usage for a 4-bit quantized 70B model (35 GB weights) on an 80 GB H100. While the model fits easily at small context lengths, the KV cache (sloped lines) eventually consumes all remaining HBM. At 128K context, a batch size of 2 is physically impossible on a single GPU (35 GB + 84 GB > 80 GB), forcing the system to either reduce batch size (killing throughput) or shard the model." fig-alt="Memory usage plot. Horizontal dashed line at 80 GB shows capacity. Solid blue region at bottom is 35 GB weights. Sloped lines for Batch 1, 2, 4, and 8 show memory rising with sequence length. Batch 8 hits the 80 GB line early (~16K tokens)."}
...
# ┌── 4. DECORATE ──────────────────────────────────────────────────────────────
ax.text(5, hbm_cap_gb + 2, 'HBM Capacity (80 GB)', color=COLORS['RedLine'], fontsize=9, fontweight='bold')
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, check, fmt_memory, fmt_memory_capacity, fmt_qty_int, fmt_count, MarkdownStr
...
class KVCacheCapacityEstimator:
    ...
    h100_mem_gb_str = fmt_memory_capacity(h100_vendor_capacity, unit=GiB, precision=0, commas=False)
...
1.  **Significance**: It eliminates internal and external fragmentation, which can waste 60--80 percent of the memory allocated to the KV cache under contiguous preallocation. Allowing sequences to grow dynamically enables 2--4$\times$ higher concurrent throughput $(X)$ on the same hardware.
...
3.  **Common pitfall**: A frequent misconception is that PagedAttention speeds up *individual token math*. In reality, it is a capacity optimization: it improves *system-level efficiency* by allowing larger batch sizes, though it adds a small indirection overhead $(L_{\text{lat}})$ for the pointer lookups.
...
**Scenario**: An LLM inference server allocates KV cache memory contiguously for incoming sequences based on maximum sequence length (e.g., 4,096 tokens).
...
\begin{tikzpicture}[font=\small\sffamily, scale=0.9, transform shape]
  ...
  \tikzset{
    block/.style={draw=black!60, fill=BlockColor, minimum width=0.8cm, minimum height=0.6cm, font=\scriptsize\sffamily, inner sep=0pt},
  ...
  \node[right=0.2cm of pb5, font=\scriptsize\sffamily] {(System Prompt)};
  ...
  \node[font=\scriptsize\sffamily, right=0.1cm of la1] (la_dots) {...};
  ...
  \node[font=\scriptsize\sffamily, right=0.1cm of lb1] (lb_dots) {...};
  ...
  % Annotations
  \node[align=left, font=\scriptsize\sffamily, anchor=west, right=2cm of la6] {Both requests point\\to same physical blocks\\for blocks 0-5};
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt_count, fmt_int, fmt_memory, fmt_percent, fmt_math
...
**Scenario**: A chatbot service uses a `{python} PrefixCachingAnalysis.system_prompt_tokens_str`-token system prompt and serves `{python} PrefixCachingAnalysis.concurrent_users_str` concurrent users.
...
**Result**: Prefix caching yields a `{python} PrefixCachingAnalysis.memory_reduction_pct_str` memory reduction in KV cache, enabling `{python} PrefixCachingAnalysis.concurrent_mult_str` more concurrent users.
...
The capacity-management techniques in @sec-inference-scale-pagedattention-3c94 and @sec-inference-scale-prefix-caching-8e8b tolerate the cache; compression shrinks it. The calculation here uses a 64-KV-head multi-head attention (MHA) baseline so the grouped query attention subsection can isolate the head-count reduction as a separate architectural lever. For a 70B parameter model with 80 layers, 64 heads, head dimension 128, and sequence length 4096 in FP16, the KV cache requires:
...
class KVCacheCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt_int, check, fmt_display_math, fmt_memory, fmt_memory_capacity, fmt_qty
...
Speculative decoding was established as an algorithmic optimization in @sec-performance-engineering-speculative: a small draft model proposes $K$ tokens, the target model verifies them in one parallel forward pass, and rejection sampling guarantees the emitted tokens follow the target model's distribution exactly, so the technique is lossless and bounded by the draft acceptance rate $\alpha_{\text{acc}}$. The serving question is different. Speculation becomes a resource-admission policy under service level agreement (SLA) pressure: the scheduler must decide when to enable it, how its memory cost interacts with KV-cache admission, and what it does to the SLA the endpoint is bound by.
...
#### The speculation tax and KV-cache admission {#sec-inference-scale-speculative-admission}

The first concern is memory. The draft model is not free state: it requires its own GPU allocation for weights, and each in-flight request that uses speculation reserves a small additional KV cache for the draft model alongside the target's. Pairing a 7B draft model with a 70B target adds roughly 14 GB of weights per replica plus per-request draft cache. This *speculation tax* competes directly with the KV-cache admission budget developed in @sec-inference-scale-kv-cache-wall: every gigabyte the draft model holds is a gigabyte the admission controller cannot allocate to a concurrent request. Speculation therefore shrinks the maximum batch size the node can admit, trading aggregate throughput for the latency of individual requests. The admission controller and the speculation policy cannot be tuned independently; enabling speculation lowers the concurrency ceiling that the KV-cache wall already imposes.

#### The latency-throughput tension under an SLA {#sec-inference-scale-speculative-sla}
...
**Scenario**: A multi-tenant LLM serving system experiences KV-cache HBM exhaustion (80 GB limit) during peak concurrency.
...
**Scenario**: An LLM inference server receives a mix of long input prompts (10k+ tokens) and interactive short decode requests [@agrawal2024sarathi].
...
# │ Show: "80" GB.
# │ How: pulling Hardware.Cloud.H100.memory.capacity from mlsysim.core.units.
# │
# └─────────────────────────────────────────────────────────────────────────────
...
$$M_{\text{weights}} = P \times \frac{b}{8} \text{ bytes}$$ {#eq-weight-memory}
...
$$T_{\text{parallel}} = \frac{T_{\text{compute}}}{N} + T_{\text{comm}}(N) + T_{\text{sync}}(N) - T_{\text{overlap}}$$ {#eq-parallel-time}
...
: **Sharding triggers**: Model memory, KV-cache capacity, embedding table size, and forward-pass latency limits dictate minimum sharding degrees and candidate distribution strategies. {#tbl-sharding-triggers}
...
\begin{tikzpicture}[font=\small\sffamily, scale=0.9, transform shape]
  ...
  \node[above, font=\bfseries\sffamily\bfseries] at (gpu0_bg.north) {GPU 0};

  \node[gpu_bg] (gpu1_bg) at (4.25, -0.5) {};
  \node[above, font=\bfseries\sffamily\bfseries] at (gpu1_bg.north) {GPU 1};

  % Attention Block
  \node[anchor=center] (att_title) at (2, 5.5) {\textbf{Attention Layer}};

  \node[opbox] (att0) at (-0.25, 4.5) {Heads 1..H/2};
  \node[opbox] (att1) at (4.25, 4.5) {Heads H/2+1..H};
...
class TpSpeedupCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    _model = Models.Language.Llama2_70B
    # @tbl-inference-tp8-layer-breakdown per-layer phase timings (1 GPU vs 8-way TP):
    ...
    att_speedup = att_seq_ms / att_tp_ms
    ff_speedup = ff_seq_ms / ff_tp_ms
    ...
    weight_fp16_gb_val = weight_fp16.to(GB).magnitude

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt_int, check, fmt, fmt_bandwidth, fmt_memory, fmt_memory_capacity, fmt_time, fmt_params, fmt_math
    ...
    # Weight-memory equation: params (B) × 10^9 × bytes/element = weight (GB)
    mem_70b_eq_math = fmt_math(
        rf"M_{{\text{{70B}}}} = {int(parameters_b)} \times 10^9 \times " rf"{bytes_per_element} = {int(weight_fp16_gb_val)}\text{{ GB}}"
    )
    # Per-layer phase breakdown for @tbl-inference-tp8-layer-breakdown
    ...
    att_speedup_mult_str = fmt_multiple(att_speedup, precision=0, commas=False)
    ff_speedup_mult_str = fmt_multiple(ff_speedup, precision=0, commas=False)
...
**Diagnosis**: Fitting the model on 2 GPUs (`{python} TpSpeedupCalc.weight_fp16_gb_str`/80 GB) suffers from compute bottlenecks (`{python} TpSpeedupCalc.seq_per_layer_ms_str` per layer). Applying 8-way tensor parallelism shards attention and feed-forward layers across all 8 GPUs, reducing per-layer execution to `{python} TpSpeedupCalc.tp8_per_layer_ms_str` despite AllReduce communication overhead.

**Systems lesson**: 8-way tensor parallelism over NVLink achieves a 6.9$\times$ end-to-end speedup (prefill time reduced from `{python} TpSpeedupCalc.seq_total_ms_str` to `{python} TpSpeedupCalc.tp8_total_ms_str`). Sharding attention and feed-forward layers across intra-node NVLink interconnects buys latency by trading low-overhead NVLink transfers for compute acceleration.
...
: **Pipeline vs. tensor parallelism**: Key trade-offs between layer-internal tensor splitting (reduces single-request latency, requires high-bandwidth NVLink) and inter-layer pipeline staging (maximizes batch throughput, introduces pipeline bubbles). {#tbl-pipeline-tensor-comparison}
...
The MoE architecture enables scaling the total parameter count while keeping the per-token compute constant. However, serving such models at scale requires expert parallelism\index{Expert Parallelism}, where different experts are hosted on different GPUs.
...
\begin{tikzpicture}[font=\small\sffamily, scale=0.9, transform shape]
  ...
  % Interconnect
  \node[text=CommColor!80!black, font=\bfseries\sffamily\bfseries] at (2.5, 1.5) {All-to-All Dispatch};
  ...
  \node[text=CommColor!80!black, font=\bfseries\sffamily\bfseries] at (6.5, 2.5) {All-to-All Gather};
...
- **Auxiliary Loss**\index{Auxiliary Loss}: An additional term in the training loss penalizes uneven expert utilization by rewarding uniform routing probabilities.
- **Capacity Factor**\index{Capacity Factor}: Each expert has a maximum number of tokens it will accept per batch (typically 1.25$\times$ the fair share). Tokens exceeding this are dropped or rerouted.
- **Expert Buffering**\index{Expert Buffering}: Tokens are buffered and processed in subsequent iterations if experts are at capacity, smoothing out load spikes at the cost of latency variance.
...
[^fn-moe-expert-serving]: **MoE (Mixture-of-Experts) Serving Trade-Off**\index{MoE!serving}: Only a subset of experts activate per token (typically 2 of 8--16), keeping per-token FLOPs low while total parameter count reaches hundreds of billions. The serving constraint: all expert weights must reside in memory even though most are idle on any given token, inflating memory footprint 4--8$\times$ relative to the active compute. This forces expert parallelism across devices with AllToAll communication at every MoE layer.
...
**Scenario**: Mixtral-8x7B (`{python} Mixtral8x7BSpec.total_params_b_str` total parameters, ~`{python} Mixtral8x7BSpec.active_params_b_str` active per token) is served across a 4-device GPU cluster using top-`{python} Mixtral8x7BSpec.active_experts_str` routing.
...
The trade-off is locality against load balance. Row-wise sharding keeps each embedding vector whole on one device, which localizes the lookup but needs an AllToAll gather to collect vectors scattered by entity ID; column-wise sharding parallelizes every lookup across devices at the cost of an AllGather; and hybrid sharding splits the difference, replicating hot embeddings column-wise while row-sharding the cold tail. @Fig-embedding-sharding shows the three layouts, where the row-wise "network gather" is the AllToAll collective named in @tbl-embedding-sharding.

::: {#fig-embedding-sharding fig-env="figure" fig-pos="H" fig-cap="**Embedding sharding strategies**: Row-wise sharding places complete embedding vectors on specific servers based on entity ID, requiring a network gather for lookup. Column-wise sharding splits each vector across all servers, allowing parallel local lookups followed by an AllGather, which is efficient for popular \"hot\" embeddings. Hybrid sharding combines these approaches, using column sharding for hot items and row sharding for the \"cold\" long tail to balance load and memory." fig-alt="Three-panel diagram. A: Row-wise sharding splits users across servers by ID. B: Column-wise sharding splits embedding dimensions across servers. C: Hybrid combines both, with hot items column-sharded and cold items row-sharded."}

```{.tikz}
\begin{tikzpicture}[font=\small\sffamily, scale=0.85, transform shape]
  \definecolor{TableColor}{HTML}{D1E6F3}
  \definecolor{ShardColor}{HTML}{E6D4E5}

  % A. Row-Wise
  \node[anchor=west, font=\bfseries\sffamily\bfseries] at (0, 4.5) {A. Row-Wise Sharding};
  ...
  \node[right, font=\scriptsize\sffamily] at (3.5, 3.75) {Svr 1: Users 0..K};

  % Server 2
  \draw[fill=ShardColor] (2.5, 2.5) rectangle (3.5, 3.0);
  \node[right, font=\scriptsize\sffamily] at (3.5, 2.75) {Svr 2: Users K..U};

  \node[font=\scriptsize\sffamily, align=center] at (1.7, 2.0) {Partition by ID\\(Network Gather)};

  % B. Column-Wise
  \node[anchor=west, font=\bfseries\sffamily\bfseries] at (6, 4.5) {B. Column-Wise Sharding};
  ...
  \node[above, font=\scriptsize\sffamily] at (8.75, 4.0) {Svr 1};

  % Server 2
  \draw[fill=ShardColor] (9.1, 2.5) rectangle (9.6, 4.0);
  \node[above, font=\scriptsize\sffamily] at (9.35, 4.0) {Svr 2};

  \node[font=\scriptsize\sffamily, align=center] at (7.7, 2.0) {Partition Vector\\(AllGather)};

  % C. Hybrid
  \node[anchor=west, font=\bfseries\sffamily\bfseries] at (0, 0.5) {C. Hybrid Sharding};

  \draw[fill=RedL] (0, -1.0) rectangle (1.5, 0);
  \node[font=\scriptsize\sffamily] at (0.75, -0.5) {Hot (Top 1\%)};

  \draw[fill=BlueL] (0, -2.5) rectangle (1.5, -1.1);
  \node[font=\scriptsize\sffamily] at (0.75, -1.8) {Cold (Tail)};

  \draw[->, thick] (1.7, -1.25) -- (2.7, -1.25);

  \node[draw, fill=RedL, font=\scriptsize\sffamily] at (4, -0.5) {Replicated/Col-wise};
  \node[draw, fill=BlueL, font=\scriptsize\sffamily] at (4, -1.8) {Row-wise Sharded};
...
::: {#lhs-inference-embedding-sharding-at-meta .callout-lighthouse title="Archetype B (DLRM at Scale): Embedding sharding at Meta"}
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: "Archetype B (DLRM at Scale): Embedding sharding at Meta" lighthouse callout.
# │
...
where $n$ is the point-to-point payload size, $\alpha$ is the network latency, and $n/\beta$ is the transfer time. @Eq-alltoall-time captures the more complex AllToAll communication for expert parallelism, where each device exchanges distinct data with every other device.
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_bandwidth, fmt_time, check
...
NVLink bandwidth[^fn-nvlink-bandwidth-inf] has evolved significantly over GPU generations.
...
[^fn-nvlink-bandwidth-inf]: **NVLink bandwidth evolution**: Each NVLink generation raises the local communication ceiling, but the realized tensor-parallel overhead still depends on message size, collective implementation, topology, and how much computation can overlap the transfer.
...
Archetype B (DLRM at Scale) relies on $P_{99} \le 100\text{ ms}$ performance boundaries. Because high-throughput serving aggregates $10^7$ QPS, $10^{-4}$ tail latency events occur $1{,}000$ times per second. This necessitates two-choice routing to suppress the queuing delay $T_{\text{queue}}$ that otherwise dominates physical compute $T_{\text{compute}}$.
...
::: {.column-margin}
![](images/svg/vol2_inference_margin_003.svg){width="100%" fig-alt="Two trend lines: a red random-assignment tail curve rises sharply above a flatter blue two-choice routing curve, with the gap shaded red."}
...
Under these realistic conditions, uninformed strategies perform poorly because unlucky assignments accumulate in the tail. The maximum queue length under random assignment follows the classical balls-into-bins result [@mitzenmacher2001], shown in @eq-random-max-queue:
...
[^fn-balls-bins-routing]: **Balls into bins (power of two choices)**\index{Power of Two Choices!routing}: With random placement, maximum bin load is $\Theta(\log R / \log \log R)$; choosing the less-loaded of two random bins reduces it to $\Theta(\log \log R)$. Serving gains depend on how accurately the probed queue length represents remaining work.

The procedure is deliberately small, but the load signal must match the serving workload. For a homogeneous vision fleet, current queue length may be enough. For an LLM fleet, active tokens, estimated remaining decode work, and KV-cache pressure better approximate the work a replica has already accepted. @Algo-power-two-choices-routing states a representative routing loop: probe two replicas, compare normalized work, and route without polling the whole fleet.
...
The two probes and the one state update per request are $\mathcal{O}(1)$ coordination rather than an all-replica scan, and the tail-latency win materializes only when the load signal reflects true work: in LLM serving, request count alone can hide a 10-token request behind a 500-token one, while active-token and KV-cache signals expose the risk. The queue-length bound in @eq-two-choices-max-queue formalizes why this small change matters, reducing maximum queue length from $\mathcal{O}(\log R / \log \log R)$ to $\mathcal{O}(\log \log R)$:
...
**Scenario**: An LLM serving fleet handles requests with highly variable output sequence lengths (500 ms for 10 tokens vs. 25 seconds for 500 tokens).
...
[^fn-consistent-hashing-kv]: **Consistent hashing**: The ring-based algorithm was developed for large-scale distributed caching at Akamai [@karger1997]. Its limited-remapping property later made it useful wherever membership changes should preserve cache locality.
...
The important properties are determinism, limited disruption, and balance. For fixed ring membership, the same key routes to the same server. A membership change remaps only the affected intervals: adding a server moves about a $1/(N+1)$ fraction of keys, while removing one moves its roughly $1/N$ share. Virtual nodes make those fractions approximate by smoothing uneven hash ranges across physical servers. For session-cached LLM serving, the payoff is increasing the probability that a subsequent turn reaches a replica with reusable state. Affinity cannot prevent repeated prefill after overload routing, failure, eviction, or membership remapping, and a cache hit is harmful if the selected replica is too overloaded to meet the request deadline.
...
**Diagnosis**: In this scenario, un-affinitized routing sends subsequent conversation turns to different replicas, forcing every turn to rebuild the KV cache from scratch (500 ms penalty, wasting 4.5s over a 10-turn conversation). With fixed membership, consistent hashing maps a user ID deterministically to one replica and retains most mappings when membership changes.
...
Under expert parallelism, each request is dispatched to the devices hosting its selected experts based on the gating decision. Two AllToAll communication steps bookend the expert compute (@fig-expert-parallel-routing).
...
**Scenario**: GPU inference servers pass standard process-level HTTP 200 checks while failing silently on real requests due to memory pressure or un-warmed model state.
...
The choice of load balancing algorithm has quantitative impact on system performance. Consider a system with 100 servers, 10,000 QPS, and variable request sizes (CV = 0.5). @Tbl-lb-comparison quantifies the latency and overhead trade-offs:
...
[^fn-circuit-breaker-serving]: **Circuit breaker**\index{Circuit Breaker!serving}: The name comes from the electrical safety device that interrupts current before an overloaded circuit causes wider damage.
...
```{python}
#| echo: false
from mlsysim.fmt import fmt_count, fmt_multiple, fmt_percent, fmt_rate, fmt_time
...
class NoisyNeighborImpact:
    tenant_base_qps_str = fmt_rate(100, "QPS")
    slo_ms_str = fmt_time(50 * millisecond, millisecond, precision=0, commas=False)
    tenant_burst_qps_str = fmt_rate(500, "QPS")

class NoisyNeighborQuotaScenario:
    tenant_throttled_qps_str = fmt_rate(120, "QPS")
    protected_p99_ms_str = fmt_time(15 * millisecond, millisecond, precision=0, commas=False)
```

::: {#exmp-inference-cascading-failure-prevention .callout-example title="Cascading failure prevention"}

**Scenario**: A single GPU inference server experiences thermal throttling (latency spiking from 50 ms to 500 ms) in a multi-replica serving fleet.
...
**Systems lesson**: Circuit breakers isolate degraded GPU replicas before retry traffic causes fleet-wide failure. Opening the breaker when error or latency thresholds are breached (`{python} CircuitBreakerServingProfile.error_threshold_pct_str` error rate) fails fast and preserves capacity across remaining replicas.
...
::: {.column-margin}
![](images/svg/vol2_inference_margin_004.svg){width="100%" fig-alt="A red tenant workload fans gray arrows to five blue peer workloads, showing one source degrading many shared dependents."}

*One tenant's burst can degrade every workload sharing the pool.*
:::

The same sharing creates the isolation problem and multi-tenancy trade-offs that @tbl-tenancy-comparison frames: noisy neighbors can make one tenant's burst degrade others, resource contention spans GPU memory, network bandwidth, and CPU cycles, security boundaries must protect tenant data, and SLO complexity increases when tenants have different requirements.
...
: **Single vs. multi-tenant trade-offs**: Multi-tenant deployments double GPU utilization and cut per-request cost but introduce noisy-neighbor risks that demand strict isolation controls. {#tbl-tenancy-comparison}
...
**Scenario**: An inference platform serves 10 tenants on shared H100 GPUs at `{python} NoisyNeighborImpact.tenant_base_qps_str` each under a `{python} NoisyNeighborImpact.slo_ms_str` P99 SLO.

**Diagnosis**: Tenant 3 bursts to `{python} NoisyNeighborImpact.tenant_burst_qps_str` (5$\times$ baseline). Without isolation, KV cache memory and compute contention cascade into P99 latency violations (22–45 ms) for all 10 tenants on the shared pool.
...
::: {#exmp-inference-noisy-neighbor-quotas .callout-example title="Noisy neighbor with per-tenant quotas"}

**Scenario**: Tenant 3 bursts to `{python} NoisyNeighborImpact.tenant_burst_qps_str` on a shared inference cluster with per-tenant resource quotas enabled.

**Diagnosis**: Quota enforcement throttles Tenant 3 to `{python} NoisyNeighborQuotaScenario.tenant_throttled_qps_str` (violating only Tenant 3's SLO), while Tenants 1–2 and 4–10 maintain normal `{python} NoisyNeighborQuotaScenario.protected_p99_ms_str` latency.
...
::: {#lst-resource-quotas lst-cap="**Multidimensional Resource Quotas**: Admission control evaluates per-tenant concurrency, KV-cache allocation, request rate, and input/output token budgets before admitting work."}
...
When total demand exceeds capacity, max-min fairness allocates resources to maximize the minimum allocation across tenants. The algorithm first gives each tenant an equal share, then redistributes unused capacity from tenants whose demand falls below their equal share. Raw request counts are an inadequate unit for this calculation because equal request counts are not equal load: a tenant issuing long-context summarization requests consumes far more prefill compute and KV capacity than a tenant issuing short classification queries. Token rates provide useful accounting units, but input and output tokens must remain separate because they stress different phases. Memory reservation and concurrency require additional limits. For three tenants with output demands of 30,000, 20,000, and 80,000 tokens/sec competing for a decode pool capable of 100,000 tokens/sec, max-min fairness yields allocations of 30,000, 20,000, and 50,000. That allocation protects decode bandwidth; a separate input-token budget protects the prefill pool.

### Priority scheduling {#sec-inference-scale-priority-scheduling-3e37}

When tenants have different SLO requirements, priority scheduling ensures that high-priority requests receive resources first. @Tbl-priority-scheduling-classes shows that preemption rights and resource guarantees move together down the three classes: critical traffic both preempts any lower class and reserves capacity, while best-effort never preempts and gets no guarantee, so an SLO tier maps directly onto a position in the preemption ladder.
...
Preemption extends this principle to requests already in flight, but priority does not make state movement free. At a token boundary, the scheduler may stop issuing decode work to a lower-priority request while retaining its KV pages, swap those pages to host memory, discard them for later recomputation, or reject the request if its deadline can no longer be met. Retention works when HBM rather than compute is plentiful; swapping works only when transfer time fits the deadline and does not congest other traffic; recomputation works when repeated prefill costs less than migration. The victim policy must account for remaining service time, state size, and deadline slack, or aggressive preemption can consume more capacity than it releases.

### Bulkhead pattern {#sec-inference-scale-bulkhead-pattern-1c3a}

The bulkhead pattern[^fn-bulkhead-service-isolation] [@nygard2007releaseit] physically isolates tenant workloads, preventing failures from propagating across tenants. The pattern is named after ship compartments that contain flooding to isolated sections.
...
\begin{tikzpicture}[font=\small\sffamily, scale=0.9, transform shape]
  ...
  \node[anchor=west, below=2cm of gold_title.west, xshift=-0.8cm] (std_title) {\textbf{Standard Tier} (Shared)};
  \node[lb, below=0.2cm of std_title.south west, anchor=north west, xshift=1.8cm] (std_lb) {Std LB};
  ...
  % Isolation Barrier
  \draw[dashed, ultra thick, black!50] (-0.5, 2.24) -- (5.5, 2.24);
  \node[fill=white, inner sep=2pt, font=\scriptsize\sffamily, align=center] at (8.0, 2.24) {\textbf{Complete Isolation}\\No Shared Resources};

  % Impact Annotation
  \node[align=left, font=\scriptsize\sffamily, color=FailColor!80!black, right=0.5cm of r4] {Failure constrained\\to Shared Pool};
  \node[align=left, font=\scriptsize\sffamily, color=GoldColor!50!black, right=0.5cm of r1, yshift=-0.5cm] {Gold Tier\\unaffected};
...
A serving autoscaler that reacts only to average accelerator utilization can scale in the wrong direction. Decode can saturate memory bandwidth while arithmetic units appear idle, long prompts can consume prefill capacity before request count rises, and queued work can violate an SLO while fleet-wide utilization remains moderate. The control signal must represent outstanding work relative to phase-specific service capacity.

For a conventional fixed-cost model, queueing delay and request rate per ready replica can be sufficient. Autoregressive serving needs at least separate prompt-token and output-token arrival rates, active KV-cache occupancy, and the observed service rates of the prefill and decode pools. The controller converts each signal into a required replica count, then takes the maximum requirement plus failure and forecast headroom. Scaling on SLO goodput prevents the controller from counting requests that complete after their TTFT or TPOT targets as usable capacity.

Scale-up and scale-down require asymmetric evidence. Scale-up acts on sustained queue growth, forecast demand, or declining deadline slack because cold-start delay makes late reactions expensive. Scale-down requires low demand over a longer window, enough spare capacity to survive one replica failure, and a drain plan for in-flight requests and any optional session cache. This hysteresis prevents oscillation in which replicas repeatedly load large weights, accept work, and terminate before amortizing startup cost.
...
**Scenario**: An autoscaling engine provisions a new Llama-70B serving replica on an H100 GPU instance during a sudden traffic surge.

**Diagnosis**: Under the scenario assumptions, bringing up a fresh replica takes ~7 minutes ($T_{\text{cold start}} = T_{\text{provision}} + T_{\text{load}} + T_{\text{warmup}}$), including 5 minutes to fetch 140 GB of model artifacts from object storage and 1 minute for device loading and warmup. Reactive autoscaling fails to absorb the surge before requests time out.

**Systems lesson**: Cold-start latency depends on artifact size, effective storage bandwidth, provisioning delay, compilation, and warmup. The controller must measure this distribution and begin provisioning far enough ahead that its high percentile fits inside the forecast horizon; the 5–10 minute lead is specific to this scenario.

:::

::: {#exmp-inference-chatgpt-traffic-spike .callout-example title="The ChatGPT traffic spike"}

**Scenario**: ChatGPT scaled from 0 to 100 million active users in two months, creating unprecedented traffic surges across global inference clusters.
...
**Systems lesson**: Predictive autoscaling converts diurnal time-series forecasts into scheduled pre-warming windows, as shown in the schedule breakdown (@tbl-inference-chatbot-predictive-schedule). Triggering scale-up actions 30 minutes before expected ramps ensures replicas are fully warmed before traffic arrives.
...
The cost savings demonstrated in example \ref{exmp-inference-predictive-scaling-traffic} depend on anticipating traffic before it arrives. Proactive provisioning avoids the SLO violations that reactive-only systems suffer during ramp-up periods (@fig-predictive-reactive-scaling).
...
\begin{tikzpicture}[font=\small\sffamily, scale=0.8]
  ...
  \node[RedLine, font=\tiny\sffamily, rotate=90] at (5.1, 2.7) {SLO VIOLATION};
...
: **Scaling response time components**: The four phases of a scaling event with typical durations and their primary optimization lever. Provisioning dominates the budget by an order of magnitude, which is why warm pools are a high-leverage optimization for latency-sensitive serving fleets. {#tbl-scaling-response-components}
...
Cloud providers may offer discounted GPU instances[^fn-spot-instance-inference] that can be reclaimed with short notice, so the scheduling decision is whether the workload can drain, reroute, or recompute without violating its SLO. @Tbl-gpu-instance-types lays out the three GPU instance classes and the workloads each suits best:

[^fn-spot-instance-inference]: **Spot-instance economics**\index{Spot Instance Economics}\index{Spot Instances!inference}: Provider discounts and termination notices vary by region, instance type, and time. For inference, revocation can destroy in-flight requests and KV-cache state, so any savings must cover the cost of draining, rerouting, and restoring capacity.
...
The sequence matters because the steps are not interchangeable. Stopping admission and deregistering first prevent new requests from arriving during the drain window; the handler then completes in-flight work, persists recoverable state, and shuts down before the termination deadline.

A spot-aware architecture splits traffic between on-demand and spot replicas by SLO class (@fig-spot-aware-routing). SLO-critical requests always route to on-demand capacity, while best-effort requests absorb the cost savings and interruption risk of spot instances.
...
The split architecture in @fig-spot-aware-routing achieves approximately 18--27 percent cost reduction compared to a fully on-demand fleet in the representative discount scenario, while absorbing preemption risk only for best-effort traffic and keeping SLO-critical requests on guaranteed capacity.
...
Each region runs independent inference replicas with identical models. A global load balancer routes each user to the nearest region based on measured round-trip latency (@fig-global-inference-multiregion). A shared model registry synchronizes weights across regions during rollouts.
...
#### Pattern 3: Cross-region model sharding {#sec-inference-scale-pattern-3-cross-region-model-sharding-7a2f}
...
When a region becomes unavailable, traffic must reroute to healthy regions. @Lst-active-failover shows active-active failover routing logic that falls back to the next healthy region after timeout or unavailability.
...
Cost-aware routing directs latency-tolerant workloads (batch inference, background processing) to the cheapest available region (@lst-cost-aware-routing).
...
Consider a large language-model layer where most activation values are modest but specific channels produce much larger outliers [@xiao2023smoothquant]. Symmetric INT8 quantization with range [-127, 127] has no painless scale choice: a wide range preserves outliers but collapses typical values into too few bins, while a narrow range preserves resolution for typical values but clips outliers and introduces large errors.
...
Generalized post-training quantization (GPTQ) [@frantar2023gptq] is the weight-only choice when deployment needs 4-bit compression after training and can afford calibration data. Its serving value is that the model gets smaller without a full retraining run. The risk is that rounding one weight changes the layer output in a way later weights must absorb. GPTQ addresses that risk by using calibration activations to estimate which weight errors matter most,[^fn-hessian-second-order] then compensating the remaining unquantized weights as each column is rounded.

[^fn-hessian-second-order]: **Hessian matrix in quantization**\index{Quantization!Hessian matrix}\index{Hessian!quantization}: The approximation $\mathbf{H} = \mathbf{X}^T \mathbf{X}$ ranks local output sensitivity under the calibration inputs. GPTQ uses that information to compensate quantization error instead of assuming that every weight contributes equally.

GPTQ processes the model one layer at a time, quantizing each weight matrix column by column[^fn-gptq-column-wise] and compensating the still-unquantized columns so the layer's output drifts as little as possible. The serving consequence is a two-part deploy-time cost: a calibration pass over 128--256 representative samples that must match production traffic, and a per-layer second-order update that makes quantization minutes-to-hours of offline work rather than a one-shot cast. Both are paid once before deployment, so they do not touch serving latency, but a calibration set that misrepresents the deployed traffic mix is the failure mode to test for before rollout.

[^fn-gptq-column-wise]: **GPTQ (Generalized Post-Training Quantization) Column-Wise Update**\index{GPTQ!column-wise update}: GPTQ factors the inverse Hessian $H^{-1}$ via Cholesky decomposition so the compensation becomes a stable triangular update: after rounding column $q$, the residual error is propagated to columns $q{+}1{:}$ weighted by $[H^{-1}][:,q{+}1{:}] / [H^{-1}]_{qq}$. The factorization avoids recomputing a full inverse per column, giving $\mathcal{O}(d_{\text{row}} \cdot d_{\text{col}}^2)$ work instead of the naive $\mathcal{O}(d_{\text{row}} \cdot d_{\text{col}}^3)$. Per-group scaling (group size 128) gives outlier channels a finer step than a single per-matrix scale.
...
@Tbl-quarot-perplexity compares perplexity across quantization methods and precision choices on Llama 2 70B:
...
: **Tensor core precision support and speedup**: Native precision support across NVIDIA Ampere and Hopper Tensor Cores. The 2$\times$ speedup at INT8 and FP8 reflects the doubled throughput of those native paths. A100 provides native INT4 Tensor Core operations, although realized LLM speedup still depends on framework and kernel support; H100 INT4 deployment remains software/framework-dependent, and native FP4 first appears in Blackwell. {#tbl-tensor-core-precision-support}
...
@Tbl-quantization-deployment-configurations shows that the configuration follows two things the deployment fixes in advance: the precision path the target hardware supports and whether the goal is memory residency or throughput. Memory-constrained consumer GPUs take W4A16, INT8 accelerators take W8A8 for Tensor Core throughput, and H100-class hardware takes its native FP8 path.
...
The resulting GPU-CPU hybrid architecture runs dense model computation (ranking towers) on GPUs, while sparse embedding lookups run on CPU servers with large memory and SSD storage. @Tbl-meta-dlrm-hybrid-hardware places each component on the hardware whose strength matches its bottleneck: the memory-bound sparse lookups land on SSD-backed CPU servers, the low-intensity feature processing stays on CPU, and only the compute-dense ranking pass uses the GPU.
...
An Orca-style long-context serving design uses continuous batching to maintain high GPU utilization despite variable output lengths. Chunked prefill bounds decode latency by processing long prompts in chunks that interleave with ongoing generation. @Tbl-batching-strategies-comparison contrasts the three approaches:
...
\begin{tikzpicture}[font=\small\sffamily]
  ...
  \node[anchor=west, font=\scriptsize\sffamily, text=red!80, right=0.2cm of L0] {Low Precision, High Recall};
  \node[anchor=west, font=\scriptsize\sffamily, text=green!60!black, right=0.2cm of L3] {High Precision, Low Recall};
...
Search ranking deployments can run ranking models on Tensor Processing Units (TPUs)[^fn-tpu-serving-accelerator] optimized for transformer inference. TPU pods provide:
...
[^fn-two-tower-serving]: **Two-Tower Architecture**\index{Two-Tower Architecture!serving}: Encodes user features and item features through separate neural networks ("towers") into embeddings, then scores via dot product. The serving advantage: item embeddings can be precomputed offline and cached, reducing online inference to the user tower forward pass plus a sub-millisecond vector similarity lookup. The trade-off is reduced model expressiveness from late interaction, since the towers cannot attend to each other's features during encoding.
...
: **Case study summary**: Synthesis of primary serving techniques and key architectural innovations across Meta, Google, and TikTok production workloads. {#tbl-case-studies-summary tbl-colwidths="[24,38,38]"}
...
Continuous batching dramatically improves GPU utilization for LLM serving, but it addresses only one dimension of the problem. Prefill remains a bottleneck for long contexts, as the quadratic attention computation cannot be avoided regardless of how subsequent decode iterations are batched. @Sec-inference-scale-kv-cache-management-04ed demonstrates that KV cache memory, not compute, often limits batch size. Network bandwidth between sharded model components can dominate latency for large models. Continuous batching is necessary but not sufficient for efficient LLM serving.
...
Queuing theory establishes that systems provisioned for average load violate SLOs during traffic peaks. At 80 percent average utilization, a modest 25 percent traffic spike pushes utilization above 100 percent, causing unbounded queue growth. @Sec-fleet-orchestration-predictive-autoscaling details how the cold start problem exacerbates this: by the time new capacity is available, which can take multiple minutes for GPU instances, SLO violations have already occurred. Capacity planning must account for peak load plus headroom, not average load.
...
Simple load balancing strategies like round-robin seem adequate until examined quantitatively. @Sec-inference-scale-load-balancing-request-routing-4997 shows that random assignment produces maximum queue lengths of $\mathcal{O}(\log R / \log \log R)$ across $R$ servers. Power-of-two-choices reduces this to $\mathcal{O}(\log \log R)$, an exponential improvement. For a 1,000-server cluster, this translates from ~4-5 requests maximum queue to ~2 requests. At the tail latencies that determine SLO compliance, this difference is substantial. The choice of load balancing algorithm has first-order impact on system performance.
...
While larger GPU memory enables larger batches for models that fit in memory, the bottleneck often shifts before memory is exhausted. Memory bandwidth limits throughput for bandwidth-bound operations (LLM decode). Compute limits throughput for compute-bound operations (prefill, vision inference). A bandwidth-bound decode workload on an H100 with `{python} InferenceBottleneckHardware.h100_mem_gb_str` memory eventually plateaus when HBM3 bandwidth (`{python} InferenceBottleneckHardware.h100_mem_bw_tb_per_s_str`) saturates; adding memory alone does not raise token throughput once that point is reached. Understanding whether the workload is compute-bound, memory-bandwidth-bound, or capacity-bound guides appropriate resource allocation.
...
* **Serving cost compounds forever**: Training is paid once, but inference OpEx accrues on every request (principle \ref{pri-serving-cost-dominance}); for high-traffic production systems it can dominate by 100$\times$ or more over a model's lifetime. Milliseconds, memory fragments, and utilization points are financial levers, not local optimizations.
...
* **Decode turns memory into capacity**: LLM prefill is compute-bound, while autoregressive decode rereads weights and expands private KV state token by token (principle \ref{pri-autoregressive-bottleneck}). Continuous batching, PagedAttention, prefix reuse, and KV compression matter because they convert scarce HBM bandwidth and memory into admitted requests.
...
* **Distribution always sends a bill**: Tensor parallelism, expert routing, global load balancing, multi-tenancy, and failover (principle \ref{pri-power-two-choices}) all buy capacity or isolation by adding communication and coordination. Production serving works only when that tax is explicitly budgeted inside latency, cost, and reliability envelopes.

:::

Ultimately, serving efficiency is governed by the serving cost dominance law (principle \ref{pri-serving-cost-dominance}): training CapEx is paid once, but inference OpEx accrues continuously on every request over the model's operational lifetime. Because autoregressive decode is physically constrained by HBM bandwidth ($D_{\text{vol}} / \text{BW}$) rather than raw arithmetic ($R_{\text{peak}}$), system efficiency depends on converting idle memory capacity and bandwidth into admitted concurrency. At fleet scale, recovering a single megabyte of fragmented KV cache or reclaiming ten milliseconds of synchronization tax translates directly into millions of dollars in sustained operational savings.

::: {.callout-chapter-connection title="From data center to edge"}

The architectural foundations for inference at scale developed here—continuous batching, PagedAttention, model sharding, and queue-depth load balancing—all assume data center-grade hardware: enterprise accelerators, high-speed NVLink interconnects (@sec-collective-communication), and terabytes of HBM. Building on local runtime compilation (@sec-performance-engineering), this chapter scaled serving to data center fleets. Many applications, however, require predictions closer to the user: on smartphones, IoT devices, or local gateways where power budgets are measured in milliwatts, memory in megabytes, and connectivity is intermittent. @Sec-edge-intelligence examines how serving principles change when intelligence moves to the edge and every data center assumption is inverted.
