...
::: {#fig-gpu-memory-wall fig-env="figure" fig-pos="htb" fig-cap="**Decode inherits the bandwidth deficit**: Peak compute outpaces memory bandwidth by roughly two to one across four accelerator generations, normalized to the V100. Because autoregressive decode reads the full weight set per token, it advances at the bandwidth rate rather than the compute rate. Specifications come from the MLSysIM hardware registry." fig-alt="Line chart normalized to the V100 showing peak compute reaching 18 times and memory bandwidth 8.9 times by the B200, the gap that bounds decode throughput."}
