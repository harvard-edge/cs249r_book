...
The physical and logical constraints just surveyed often appear as lost throughput or added wall-clock time. Societal constraints have a different role: security, privacy, fairness, and governance define whether a system may operate and what outcomes it must prevent, rather than merely adding execution overhead. The single-machine foundation for diagnosing performance is the **Data · Algorithm · Machine (D·A·M) Taxonomy**\index{D·A·M taxonomy!definition}, which @sec-appdx-dam-taxonomy develops as a complete diagnostic framework. Data\index{D·A·M taxonomy!data} is the information the system learns from, and performance becomes data-bound when the I/O pipeline cannot feed the processor fast enough. The algorithm\index{D·A·M taxonomy!algorithm}\index{Algorithm!D·A·M taxonomy} is the mathematical logic being executed, and performance becomes compute-bound when arithmetic units are the limiting factor. The machine\index{D·A·M taxonomy!machine} is the physical hardware substrate, and performance becomes memory-bound when memory bandwidth or capacity limits throughput.

Scaling from one machine to a fleet introduces three execution dimensions that compete for wall-clock time. Where the canonical triad of walls names where the fleet binds, these dimensions name where distributed execution spends time. The **C$^3$ taxonomy**\index{C³ taxonomy!definition} extends rather than replaces the D·A·M lens: D·A·M continues to describe the workload and machine, while C$^3$ identifies the additional execution costs created by distribution. Compute\index{C³ taxonomy!compute} ($C_1$, the math) is local execution on individual accelerators. Communication\index{C³ taxonomy!communication} ($C_2$, the wire) moves data across the network fabric. Coordination\index{C³ taxonomy!coordination} ($C_3$, the logic) manages shared progress and state across nodes through collectives, scheduling, consistency, and recovery.

These three dimensions form the **triad of distributed efficiency**\index{Triad of distributed efficiency!definition}. If the fleet spends too much time on communication or coordination, the expensive compute capacity sits idle. The central challenge is engineering the fleet to minimize the C$^3$ gap: the difference between theoretical hardware peak and actual distributed throughput. While this chapter introduces the three dimensions as an organizing framework, @sec-appdx-c3-taxonomy develops the complete taxonomy and diagnostic structure that localizes bottlenecks across the three dimensions.
...
The three dimensions of the C$^3$ taxonomy rarely operate in isolation. Just as Volume 1 expanded the single-node D·A·M triad into an intersection landscape (@sec-appdx-dam-taxonomy), distributed execution expands Compute, Communication, and Coordination into a co-design space where the dominant performance and reliability bottlenecks arise. @Fig-c3-taxonomy maps this landscape.
...
\tikzset{%
  TxtC/.style={font=\sffamily\bfseries\small,text=c1col,align=center},
  TxtIT/.style={font=\sffamily\itshape\fontsize{8}{10.5}\selectfont,text=black!70,align=center},
  TxtSM/.style={font=\fontsize{7}{9}\selectfont\sffamily,text=black!85,align=center},
  TxtJM/.style={font=\fontsize{6}{8}\selectfont\sffamily\itshape,text=black!60,align=center},
  TxtCOL/.style={font=\fontsize{8}{10.5}\selectfont\sffamily\bfseries,text=purple!80!black,align=center},
  ...
  Box/.style={align=left,
    ...
    line width=0.65pt,
    fill=brown!05,
    font=\fontsize{7}{9.5}\selectfont\sffamily,
    text width=71mm,
    minimum width=71mm,
    rounded corners=2pt,
  },
...
\node[TxtC,text=c1col](NC1) at (0, 5.25) {Compute ($C_1$)};
\node[TxtIT,below=1pt of NC1](TC1) {The Node Substrate};
\node[TxtSM,below=2pt of TC1](SC1) {Accelerators, Tensor Cores \& Matrix Engines\\HBM3e Memory Wall, SRAM \& NVMe Tiers};
\node[TxtJM,below=2pt of SC1](JC1) {Ch: Compute Infra, Storage, Performance};

% --- PURE ZONE: Communication (C2, Bottom-Left) ---
\node[TxtC,text=c2col](NC2) at (-3.40, -0.65) {Communication ($C_2$)};
\node[TxtIT,below=1pt of NC2](TC2) {The Networking Backbone};
\node[TxtSM,below=2pt of TC2](SC2) {Optical Fabrics, InfiniBand \& RoCEv2\\Rail Topologies \& Bisection BW};
\node[TxtJM,below=2pt of SC2](JC2) {Ch: Network Fabrics, Collective Comm.};

% --- PURE ZONE: Coordination (C3, Bottom-Right) ---
\node[TxtC,text=c3col](NC3) at (3.40, -0.65) {Coordination ($C_3$)};
\node[TxtIT,below=1pt of NC3](TC3) {The Orchestration Layer};
\node[TxtSM,below=2pt of TC3](SC3) {Gang Schedulers (Slurm, K8s, Ray)\\Heartbeats, Consensus \& Fast Reboot};
\node[TxtJM,below=2pt of SC3](JC3) {Ch: Fleet Orchestration, Fault Tolerance, Ops};

% --- INTERSECTION: C1 ∩ C2 (Top-Left) ---
\node[TxtC,text=black!80](PC1C2) at (-2.20, 2.25) {$C_1 \cap C_2$};
\node[TxtCOL,below=0pt of PC1C2](THNL) {Hiding Network\\Latency};
\draw[LineA](THNL.150)--++(150:4.1)coordinate(BOX1);
\node[Box,anchor=south] at (BOX1){%
  \textbf{Hiding Network Latency ($C_1 \cap C_2$)}\\[2pt]
  ...
  {\fontsize{6}{8}\selectfont\itshape\textcolor{black!60}{Ch: Distributed Training, Collective Communication}}
};

% --- INTERSECTION: C1 ∩ C3 (Top-Right) ---
\node[TxtC,text=black!80](PC1C3) at (2.20, 2.25) {$C_1 \cap C_3$};
\node[TxtCOL,below=0pt of PC1C3](TMFG) {Maximizing Fleet\\Goodput};
\draw[LineA](TMFG.30)--++(30:4.1)coordinate(BOX2);
\node[Box,anchor=south] at (BOX2){%
  \textbf{Maximizing Fleet Goodput ($C_1 \cap C_3$)}\\[2pt]
  ...
  {\fontsize{6}{8}\selectfont\itshape\textcolor{black!60}{Ch: Fault Tolerance, Fleet Orchestration}}
};

% --- INTERSECTION: C2 ∩ C3 (Bottom Overlap) ---
\node[TxtC,text=black!80](PC2C3) at (0, -1.05) {$C_2 \cap C_3$};
\node[TxtCOL,below=0pt of PC2C3](TCS) {Collective\\Synchronization};
% Leader line drops straight down to y = -4.50 (0.50 below circles)
\draw[LineA]([yshift=-2pt]TCS.south)--(0, -4.50)coordinate(BOX3);
\node[BoxBottom,anchor=north] at (BOX3){%
  \textbf{Collective Synchronization ($C_2 \cap C_3$)}\\[2pt]
  ...
  {\fontsize{6}{8}\selectfont\itshape\textcolor{black!60}{Ch: Collective Communication, Fault Tolerance, Fleet Orchestration}}
};

% --- TRIPLE INTERSECTION: C1 ∩ C2 ∩ C3 (Center) ---
\node[TxtC,text=c1col] at (0, 1.05) (CENT) {$C_1 \cap C_2 \cap C_3$};
\node[TxtCOL,below=0pt of CENT,text=c1col](CENT_TITLE) {Fleet-Scale ML\\Systems Engineering};
\draw[LineA]([yshift=-1mm]CENT_TITLE.190)--++(175:6.4)coordinate(BOX4);
\node[BoxCenter,anchor=east] at (BOX4){%
  ...
  {\fontsize{6}{8}\selectfont\itshape\textcolor{black!60}{Ch: Distributed Training, Inference, Edge}}
};

% Bottom dividing line and quote
\draw[gray!50,line width=0.5pt] (-7.5, -7.90) -- (7.5, -7.90);
...
Read the Venn diagram in @fig-c3-taxonomy from the outer regions inward. Compute ($C_1$) captures the physical node substrate: accelerator arithmetic, the high-bandwidth memory (HBM) memory wall, SRAM caching, and high-throughput local Non-Volatile Memory Express (NVMe) tiers (@sec-compute-infrastructure, @sec-data-storage, @sec-performance-engineering). Communication ($C_2$) captures the networking backbone: optical fabrics, InfiniBand and RoCEv2 interconnects, rail-optimized cluster topologies, and collective communication primitives (@sec-network-fabrics, @sec-collective-communication). Coordination ($C_3$) captures the orchestration layer: gang schedulers (Slurm, Kubernetes, Ray), failure detection heartbeats, non-blocking state persistence, and distributed consensus (@sec-fault-tolerance-reliability, @sec-fleet-orchestration, @sec-ops-scale).

The pairwise intersections define the core co-design tensions of distributed systems:
