...
::: {#fig-storage-compute-gap fig-env="figure" fig-pos="htb" fig-cap="**The storage-compute gap**: While accelerator compute (TFLOPS) and internal high-bandwidth memory (HBM) have grown exponentially, local storage bandwidth (NVMe) has scaled much slower. This growing divergence creates a fuel-line bottleneck where computation must pause to wait for data delivery." fig-alt="Log-scale three-line chart from 2010 to 2024 comparing accelerator compute, HBM bandwidth, and NVMe bandwidth. Compute rises fastest and NVMe slowest, so the vertical distance between the top and bottom lines widens over time."}
