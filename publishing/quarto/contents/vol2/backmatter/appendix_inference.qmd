...
#### Queueing notation and the M/G/1 batch surrogate {#sec-appdx-inference-scale-mgck-queue-model-gpu-serving-c4c4}

The **M/G/c/K notation**\index{M/G/c/K Queues} names four characteristics that a production GPU-serving model may need to represent:
...
::: {#fig-queuing-hockey-stick fig-env="figure" fig-pos="htb" fig-cap="**The queuing hockey stick**: For an M/M/1 queue, $\rho_{\text{serv}}/(1-\rho_{\text{serv}})$ is the expected number of requests in the system, including the request in service. Its hyperbolic rise near saturation explains why production systems retain utilization headroom; the shaded operating bands are illustrative rather than universal thresholds." fig-alt="Plot of the expected number of requests in an M/M/1 system versus serving utilization rho_serv from 0 to 1. The curve rises gradually at low utilization, bends upward through illustrative headroom and caution bands, and rises sharply as utilization approaches 1."}
