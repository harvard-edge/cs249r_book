...
::: {#fig-conclusion-memory-wall fig-env="figure" fig-pos="htb" fig-cap="**The memory wall widens with every generation**: Peak compute and memory bandwidth for NVIDIA data center accelerators, each normalized to the V100. Across 2017 to 2024 compute rose 18$\times$ while bandwidth rose 8.9$\times$, so the gap a decode step must cross roughly doubled. Specifications are drawn from the MLSysIM hardware registry, which records the vendor datasheet behind each value." fig-alt="Line chart of two series normalized to the V100. Peak compute climbs to 18 times by the B200 while memory bandwidth reaches 8.9 times, and the widening vertical gap between the two lines is the memory wall."}
