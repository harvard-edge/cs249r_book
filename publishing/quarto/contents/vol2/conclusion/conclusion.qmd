...
::: {#fig-gpu-memory-wall fig-env="figure" fig-pos="htb" fig-cap="**The GPU Memory Wall**: Normalized growth of peak compute (TFLOP/s) versus memory bandwidth (GB/s) for NVIDIA data center GPUs (2010–2024), scaled relative to 2010. Compute capacity has scaled over 3,000$\\times$, while memory bandwidth has grown less than 50$\\times$. This divergence creates the memory wall that bottlenecks autoregressive decode."}
...
data['Compute_Norm'] = data['Compute_TFLOPS'] / data['Compute_TFLOPS'].iloc[0]
data['Mem_BW_Norm'] = data['Memory_Bandwidth_GBps'] / data['Memory_Bandwidth_GBps'].iloc[0]

fig, ax = plt.subplots(figsize=(8, 5))

ax.plot(data['Year'], data['Compute_Norm'], marker='o', color='tab:red', linewidth=2, label='Peak Compute (Normalized)')
ax.plot(data['Year'], data['Mem_BW_Norm'], marker='s', color='tab:blue', linewidth=2, linestyle='--', label='Memory Bandwidth (Normalized)')

for i, row in data.iterrows():
    if row['Year'] in [2010, 2016, 2020, 2024]:
        ax.annotate(row['Architecture'], 
                    (row['Year'], data['Compute_Norm'].iloc[i]),
...
ax.set_yscale('log')
ax.set_xlabel('Year')
ax.set_ylabel('Growth Relative to 2010 (Log Scale)')
ax.set_title('The Growing Gap: Compute vs. Memory Bandwidth')
