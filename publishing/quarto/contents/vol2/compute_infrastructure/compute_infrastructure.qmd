...
::: {#fig-gpu-scaling-trends fig-env="figure" fig-pos="htb" fig-cap="**GPU scaling trends (2017–2024)**: Normalized relative to the V100, peak compute throughput scales significantly faster than memory bandwidth, driving the \"memory wall.\" Concurrently, thermal design power (TDP) outpaces efficiency gains, pushing against data center power delivery constraints."}
...
data = pd.read_csv("data/gpu_scaling_trends.csv")

# Normalize data to the first row (V100)
baseline_compute = data['Compute_TFLOPs'].iloc[0]
baseline_mem = data['Memory_Bandwidth_GBps'].iloc[0]
baseline_power = data['Power_W'].iloc[0]

data['Compute_Norm'] = data['Compute_TFLOPs'] / baseline_compute
data['Mem_Norm'] = data['Memory_Bandwidth_GBps'] / baseline_mem
data['Power_Norm'] = data['Power_W'] / baseline_power

# Create labels combining GPU name and year
x_labels = data['GPU'] + '\n(' + data['Year'].astype(str) + ')'

sns.set_theme(style="whitegrid")
fig, ax = plt.subplots(figsize=(8, 5))

ax.plot(x_labels, data['Compute_Norm'], marker='o', linewidth=2.5, color='#d62728', label='Peak Compute (TFLOPs)')
ax.plot(x_labels, data['Mem_Norm'], marker='s', linewidth=2.5, color='#1f77b4', label='Memory Bandwidth (GB/s)')
ax.plot(x_labels, data['Power_Norm'], marker='^', linewidth=2.5, color='#2ca02c', label='TDP (W)')

ax.set_title('Normalized GPU Scaling (Baseline: V100, 2017 = 1x)', fontsize=14, pad=15)
ax.set_ylabel('Relative Growth Multiplier', fontsize=12)
ax.set_xlabel('Generation', fontsize=12)
ax.grid(True, linestyle='--', alpha=0.7)
ax.legend(loc='upper left', fontsize=11)

# Annotate the final points to emphasize the divergence
for i, col in enumerate(['Compute_Norm', 'Mem_Norm', 'Power_Norm']):
    final_val = data[col].iloc[-1]
    ax.annotate(f'{final_val:.1f}x', 
                xy=(3, final_val), 
                xytext=(10, 0), 
