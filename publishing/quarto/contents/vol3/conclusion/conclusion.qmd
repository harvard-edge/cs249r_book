...
As illustrated in @fig-vol3-four-volume-arc, systems engineering provides the structural container that guarantees invariant closure across the complete machine learning systems curriculum: from single-node kernel execution to fleet-scale distributed serving, multi-step trajectory runtimes, and physical robot actuation.

::: {#fig-vol3-four-volume-arc fig-env="figure" fig-pos="htb" fig-cap="**The Machine Learning Systems Paradigm: Invariant Evolution**: Computational abstractions, governing physics, fundamental triads, and systems boundaries across the complete MLSys curriculum." fig-alt="Four-panel diagram illustrating the MLSys curriculum across single-node systems (The Model), fleet-scale systems (The Fleet), agentic systems (The Agent), and embodied physical AI."}
...
The journey across this volume operated entirely behind the glass of digital systems, where failed actions can be sandboxed, rollbacks are possible via Copy-on-Write filesystems, and latency costs only developer patience. But when autonomous intelligence is embedded into physical machines—quadrupeds, manipulators, and vehicles—computational delay translates directly into physical displacement ($\Delta x = \int v \, dt$), actions cannot be un-executed, and safety requires hard real-time guarantees. Volume IV takes this leap: turning from software trajectories to Physical AI, where machine learning systems must sense and act within the irreversible constraints of the physical world.
:::
