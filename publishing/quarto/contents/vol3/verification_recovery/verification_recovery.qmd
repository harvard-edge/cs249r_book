...
```{python}
#| label: vol3-recovery-compounding-decay
#| echo: false
# +-----------------------------------------------------------------------------
# | UNVERIFIED TRAJECTORY DECAY ACROSS HORIZONS
# +-----------------------------------------------------------------------------
# | Context: @tbl-compounding-decay and the paragraph that reads it.
# |
# | Goal:    Show how far an unverified trajectory survives as the horizon grows,
# |          for the per-step accuracies frontier models actually reach.
# | Show:    Survival probability across the accuracy and horizon sweep.
# | How:     calc_trajectory_reliability with no verifier, which is the p^N bound
# |          stated above the table.
# +-----------------------------------------------------------------------------
from mlsysim import *

def _pct(ratio):
    """Percent string carrying three significant digits at any magnitude."""
    value = ratio * 100
    if abs(value - round(value)) < 1e-9:
        ...
        precision = 8
    return fmt_percent(ratio, precision=precision, style="symbol")

class CompoundingDecay:
    # +-- 1. LOAD (Constants) --------------------
    accuracies = (0.90, 0.95, 0.98, 0.99)  # scenario: per-step accuracies from good to frontier
    horizons = (5, 10, 25, 50, 100, 200)  # scenario: trajectory depths a coding agent reaches

    # +-- 2. EXECUTE (The Compute) ---------------
    survival = {}
    for p in accuracies:
        for n in horizons:
            survival[(p, n)] = calc_trajectory_reliability(p, n)

    # +-- 3. GUARD (Invariants) ------------------
    check(
        abs(survival[(0.98, 100)] - 0.1326) < 1e-3,
        f"A 98 percent agent over 100 steps should survive about 13.3 percent of the time, got {survival[(0.98, 100)]}",
    )
    check(survival[(0.98, 50)] < 0.37, f"A 98 percent agent should not clear 37 percent over 50 steps, got {survival[(0.98, 50)]}")
    check(survival[(0.90, 200)] < 1e-8, f"A 90 percent agent over 200 steps should be effectively impossible, got {survival[(0.90, 200)]}")
    check(
        survival[(0.99, 200)] > survival[(0.98, 200)] > survival[(0.95, 200)] > survival[(0.90, 200)],
        "Survival must rise monotonically with per-step accuracy",
    )
    ...
    for n in horizons:
        cells = []
        for p in accuracies:
            cells.append(_pct(survival[(p, n)]))
        rows[n] = cells
    n5_p90_str, n5_p95_str, n5_p98_str, n5_p99_str = rows[5]
    n10_p90_str, n10_p95_str, n10_p98_str, n10_p99_str = rows[10]
    n25_p90_str, n25_p95_str, n25_p98_str, n25_p99_str = rows[25]
    n50_p90_str, n50_p95_str, n50_p98_str, n50_p99_str = rows[50]
    n100_p90_str, n100_p95_str, n100_p98_str, n100_p99_str = rows[100]
    n200_p90_str, n200_p95_str, n200_p98_str, n200_p99_str = rows[200]
    p98_n50_prose_str = fmt_percent(survival[(0.98, 50)], precision=1, style="prose")
    p98_n100_prose_str = fmt_percent(survival[(0.98, 100)], precision=1, style="prose")
```

| Horizon ($N$) | Step Accuracy $p = 0.90$                 | Step Accuracy $p = 0.95$                 | Step Accuracy $p = 0.98$                 | Step Accuracy $p = 0.99$                 |
|:--------------|:-----------------------------------------|:-----------------------------------------|:-----------------------------------------|:-----------------------------------------|
| **$N = 5$**   | `{python} CompoundingDecay.n5_p90_str`   | `{python} CompoundingDecay.n5_p95_str`   | `{python} CompoundingDecay.n5_p98_str`   | `{python} CompoundingDecay.n5_p99_str`   |
| **$N = 10$**  | `{python} CompoundingDecay.n10_p90_str`  | `{python} CompoundingDecay.n10_p95_str`  | `{python} CompoundingDecay.n10_p98_str`  | `{python} CompoundingDecay.n10_p99_str`  |
| **$N = 25$**  | `{python} CompoundingDecay.n25_p90_str`  | `{python} CompoundingDecay.n25_p95_str`  | `{python} CompoundingDecay.n25_p98_str`  | `{python} CompoundingDecay.n25_p99_str`  |
| **$N = 50$**  | `{python} CompoundingDecay.n50_p90_str`  | `{python} CompoundingDecay.n50_p95_str`  | `{python} CompoundingDecay.n50_p98_str`  | `{python} CompoundingDecay.n50_p99_str`  |
| **$N = 100$** | `{python} CompoundingDecay.n100_p90_str` | `{python} CompoundingDecay.n100_p95_str` | `{python} CompoundingDecay.n100_p98_str` | `{python} CompoundingDecay.n100_p99_str` |
| **$N = 200$** | `{python} CompoundingDecay.n200_p90_str` | `{python} CompoundingDecay.n200_p95_str` | `{python} CompoundingDecay.n200_p98_str` | `{python} CompoundingDecay.n200_p99_str` |
...
As demonstrated in @tbl-compounding-decay, an agent possessing a remarkable 98 percent single-step accuracy has a `{python} CompoundingDecay.p98_n50_prose_str` chance of completing a 50-step task, and only a `{python} CompoundingDecay.p98_n100_prose_str` chance of completing a 100-step task.
...
This mathematical reality explains the empirical plateau observed on benchmarks like SWE-bench [@jimenez2024swebench] (see @sec-vol3-telemetry-evaluation). **Scaling pretraining parameters cannot overcome the exponent $N$.** Even if a multi-trillion parameter model achieves $p = 0.99$, a 200-step trajectory yields an 86.6 percent failure rate. The only mathematical path to high-horizon reliability is **intermediate verification**: resetting the compounding exponent by validating and checkpointing state at frequent intervals.
...
Both failure modes are visible as the two arms of the same curve. @Fig-verification-tax plots them together for a heavy oracle and a fast gate, and the two minima fall in different places, which is why a single verification cadence cannot be correct for both.
...
::: {#fig-verification-tax fig-env="figure" fig-pos="htb" fig-cap="**The Verification Tax Trade-Off**: Expected wall-clock cost of a fifty-step trajectory against verification cadence, for a heavy integration suite and a fast static gate. Each curve has a distinct minimum, and the heavy oracle's optimum sits at a far larger step interval, so the right cadence is a property of the verifier rather than a policy choice." fig-alt="Two U-shaped cost curves of expected wall-clock cost against verification cadence for a heavy test oracle and a fast static gate. Each curve has a marked minimum, and the heavy oracle's optimum sits at a much larger step interval than the fast gate's."}
...
data = pd.read_csv("data/verification_tax.csv")
fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 5))

# Plot the Heavy Verifier cost curve
ax.plot(data["Cadence_K"], data["Cost_Heavy"], label="Heavy Test Oracle ($T_{ver}=10$s)", color="#d62728", linewidth=2.5)

# Plot the Fast Verifier cost curve
ax.plot(data["Cadence_K"], data["Cost_Fast"], label="Fast Static Gate ($T_{ver}=0.05$s)", color="#1f77b4", linewidth=2.5)

# Annotate optimal points
min_heavy = data.loc[data["Cost_Heavy"].idxmin()]
min_fast = data.loc[data["Cost_Fast"].idxmin()]

k_star_heavy = int(min_heavy["Cadence_K"])
ax.plot(min_heavy["Cadence_K"], min_heavy["Cost_Heavy"], marker="o", markersize=8, color="#d62728")
ax.annotate(f"Optimal $K^*={k_star_heavy}$", 
            xy=(min_heavy["Cadence_K"], min_heavy["Cost_Heavy"]),
            xytext=(min_heavy["Cadence_K"] + 3, min_heavy["Cost_Heavy"] + 50),
            arrowprops=dict(facecolor='black', arrowstyle="->"),
            fontsize=10)

k_star_fast = int(min_fast["Cadence_K"])
ax.plot(min_fast["Cadence_K"], min_fast["Cost_Fast"], marker="o", markersize=8, color="#1f77b4")
ax.annotate(f"Optimal $K^*={k_star_fast}$", 
            xy=(min_fast["Cadence_K"], min_fast["Cost_Fast"]),
            xytext=(min_fast["Cadence_K"] + 3, min_fast["Cost_Fast"] + 50),
            arrowprops=dict(facecolor='black', arrowstyle="->"),
            fontsize=10)

# Formatting
ax.set_xlabel("Verification Cadence (Steps between verifications, $K$)", fontsize=11, fontweight="bold")
ax.set_ylabel("Expected Wall-Clock Cost (Seconds)", fontsize=11, fontweight="bold")
ax.set_title("The verification tax trade-off", fontsize=12, fontweight="bold")
ax.set_xlim(1, 50)
ax.set_ylim(0, 800)
ax.legend()
...
Only three quantities fix where that minimum falls: the wall-clock cost of a step, the wall-clock cost of the verifier, and the per-step error rate.

::: {#nbk-recovery-verification-cadence .callout-notebook title="Where the verification tax curve bottoms out"}

**Problem**: An agent executes a 50-step trajectory behind a full test-suite gate. How many steps should run between verifications, and how much wall-clock time separates that cadence from the extremes of verifying every step and verifying only at the end?

**Variables**: Trajectory length $N = 50$ steps, action step cost $T_{\text{step}} = 1\text{ s}$, verifier cost $T_{\text{ver}} = 10\text{ s}$ for a full suite run, and per-step semantic correctness $p = 0.95$ from @tbl-compounding-decay. A block of steps that fails its gate is rolled back and redone in full. The step and verifier costs are the over-verification example stated above, and all four quantities are scenario assumptions.

**Math**: Verifying every $m$ steps partitions the trajectory into $N/m$ blocks. A block clears its gate only if all $m$ of its steps are correct, with probability $p^{m}$, so it takes $1/p^{m}$ attempts on average, and each attempt costs $m\,T_{\text{step}} + T_{\text{ver}}$ of wall clock:

$$T(m) = \frac{N}{m} \cdot \frac{m\,T_{\text{step}} + T_{\text{ver}}}{p^{m}}$$

Evaluating at five cadences, carrying $p^{m}$ to four figures:

$$T(1) = 50 \times \frac{11}{0.9500} = 579\text{ s}, \qquad T(5) = 10 \times \frac{15}{0.7738} = 194\text{ s}$$

$$T(10) = 5 \times \frac{20}{0.5987} = 167\text{ s}$$

$$T(25) = 2 \times \frac{35}{0.2774} = 252\text{ s}, \qquad T(50) = 1 \times \frac{60}{0.07694} = 780\text{ s}$$

Swapping the ten-second suite for the Stage 2 static gate at $T_{\text{ver}} = 0.05\text{ s}$ leaves $p$ and $N$ untouched but rescales the tax:

$$T(1) = 50 \times \frac{1.05}{0.9500} = 55.3\text{ s}, \qquad T(10) = 5 \times \frac{10.05}{0.5987} = 83.9\text{ s}$$

**Result**: Against the ten-second suite the minimum sits at $m = 10$, five gates across the trajectory, costing $167\text{ s}$ over a $50\text{ s}$ error-free floor. Verifying every step costs $579\text{ s}$, or $3.5\times$ the optimum, and verifying only at the end costs $780\text{ s}$, or $4.7\times$. The basin is broad, with every cadence from $5$ to $25$ landing within $1.5\times$ of the minimum. Against the static gate the minimum moves to $m = 1$.

**Systems insight**: The optimal cadence is a property of the verifier rather than of the agent, and it shifts by an order of magnitude when the gate changes. A gate costing $5\%$ of a step is cheapest run on every action, because the rework it prevents always outweighs what it charges, while a gate costing ten steps has to be amortized across a block. This is the quantitative case for the cascading pipeline of @fig-verification-pipeline. Stages 1 and 2 belong on every candidate action, and only the dynamic test oracle of Stage 3 needs a cadence sized for it, which need only be correct to within a factor of two because the curve is flat near its minimum and steep only at the extremes.
...
Classical distributed architectures prevent cascading resource exhaustion via the **Circuit Breaker** pattern [@nygard2007releaseit]. In agentic ML runtimes, circuit breakers must be implemented at three distinct tiers to contain stochastic runaway execution:
...
::: {#fig-reversibility-boundary fig-env="figure" fig-pos="htb" fig-cap="**The Reversibility Boundary**: Architectural bifurcation between low-latency invertible software states and high-stakes non-invertible real-world side-effects. The boundary is the only place where verification cost is unbounded, because anything left of it can be undone by a rollback while anything crossing it has to be right the first time." fig-alt="Diagram illustrating the Reversibility Boundary dividing the Reversible Internal State Domain on the left (context working sets, git worktrees, microVM copy-on-write filesystems) from the Irreversible External Actuation Domain on the right (production databases, cloud infrastructure, external emails, financial transfers), separated by an oversight gateway enforcing human review and state suspension."}
![](images/svg/reversibility_boundary.svg)
:::

What that boundary costs when it is crossed without a stop is a matter of public record (\ref{ws-recovery-knight-capital-2012}).

::: {#ws-recovery-knight-capital-2012 .callout-war-story title="Knight Capital and the forty-five minute loop (2012)"}

**Context**: On 1 August 2012 Knight Capital Americas deployed new order-routing code to its production servers ahead of a change in NYSE retail liquidity handling. The deployment reached seven of eight servers. On the eighth, dormant code from an earlier system remained in place, and the release repurposed a configuration flag that the dormant code still read [@sec2013knight].

**Mechanism**: When trading opened, the flag activated the old logic on the one stale server. It began emitting orders without the counter that had previously bounded it, so each parent order produced a stream of child orders that were never marked complete. The system was not confused about market conditions and was not malfunctioning in any way its own monitoring could name. It executed exactly what its state told it to execute, at machine speed.

**Impact**: Over roughly forty-five minutes the firm sent millions of orders into 154 stocks, accumulating multi-billion-dollar positions it had never intended to hold. Knight lost approximately \$460 million, more than its market capitalization, and was acquired within months. The SEC found the firm had violated the Market Access Rule, which requires controls that operate before an order reaches the market.

**Fix**: Pre-trade limits enforced outside the trading logic, a kill switch that any operator can pull without diagnosing the cause first, and deployment procedures that verify every target rather than assuming uniformity.

**Systems lesson**: Every executed trade was a legally final transfer, so no rollback existed at any layer. Knight is what the Reversibility Boundary looks like when it is crossed without a gate: the loop was bounded only by the time it took humans to understand what was happening, and each of those forty-five minutes wrote permanent state into an external system. An agentic runtime placing orders, sending payments, or moving physical mass inherits this exactly. The engineering question is not whether the policy is confident, but whether anything outside the policy can stop it before the action becomes a fact.
...
When an agent must cross the Reversibility Boundary to execute a high-consequence, non-invertible action, production systems enforce an **Oversight Gate** (Human-in-the-Loop approval).
...
Pairing the fastest machine loop with the slowest human gives a **$10^6\times$ disparity**, and pairing the slowest loop with the fastest human still gives $10^2\times$. The gate costs between two and six orders of magnitude depending on which pair the system actually sees, and a runtime that provisions for the optimistic end of that band will hold GPU memory for hours.
...
::: {.callout-definition title="Agent saga"}
...
To survive infrastructure crashes, host reboots, network partitioning, and multi-hour human approval pauses, autonomous agent runtimes must implement **Durable Execution**, as embodied in workflow systems like Temporal and LangGraph.
...
Drawing on principles from distributed replay systems (Friday [@geels2007friday]) and virtual machine time-travel debugging [@king2005debugging], production agent platforms incorporate an **Immutable Replay Engine** (see @sec-vol3-telemetry-evaluation).
...
The trace below is constructed rather than observed, and its timings are illustrative. An autonomous agent is tasked with migrating an internal payment reconciliation service from synchronous HTTP to asynchronous Celery workers, and the point of following it step by step is to watch which recovery mechanism catches each of the eight failures.
...
## Summary {#sec-vol3-recovery-summary}

Long-horizon autonomous execution demands a rigorous engineering discipline for failure detection and state recovery. Because non-deterministic foundation models suffer from fail-plausible errors that compound exponentially over multi-step horizons, reliable runtimes must enforce deterministic test oracles, respect the reversibility boundary, and coordinate mutating side effects through compensating distributed Sagas.

::: {.callout-takeaways title="The reversibility boundary and compensating Sagas"}
...
Deterministic recovery acknowledges that failures are an unavoidable property of non-deterministic computing. By decoupling execution into reversible internal states and compensating external Sagas, runtimes provide the resilience guarantees needed to deploy autonomous agents across extended operational horizons without manual supervision.

::: {.callout-chapter-connection title="From single-agent recovery to multi-agent coordination"}
Single-agent recovery ensures that one autonomous worker can detect errors and compensate for failed actions, but complex enterprise tasks exceed the context window and specialization of any individual agent. Concluding **Part IV (Security, Isolation, and Red Teaming)**, we enter **Part V (Scale, Evaluation, and Operations)**: beginning in @sec-vol3-multi-agent with multi-agent coordination, hierarchical supervisor trees, and distributed IPC protocols across heterogeneous agent fleets.
:::
