...
::: {#fig-verification-tax fig-env="figure" fig-pos="htb" fig-cap="**The verification tax trade-off**: Total expected wall-clock cost of a 50-step trajectory as a function of the verification cadence ($K$). A heavy integration test suite ($T_{\text{ver}}=10\text{s}$) forces an optimal cadence at $K=10$, while a fast static compiler gate ($T_{\text{ver}}=0.05\text{s}$) is optimal to run on every step ($K=1$). The curve mathematically demonstrates the tension between under-verification (massive rollback waste) and over-verification (excessive verifier tax)." fig-alt="Two U-shaped cost curves of expected wall-clock cost against verification cadence for a heavy test oracle and a fast static gate. Each curve has a marked minimum, and the heavy oracle's optimum sits at a much larger step interval than the fast gate's."}
