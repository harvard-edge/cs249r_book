...
This chapter develops the systems architecture of trajectory-aware scheduling, cache-affinity routing, multi-resource fair allocation under Dominant Resource Fairness (DRF), and preemption policies rigorously bounded by external reversibility constraints.
...
## Introduction to Trajectory Scheduling: The Post-Request Serving Dilemma {#sec-vol3-scheduling-intro}
...
To achieve high cluster efficiency, an agentic scheduler must optimize for **Trajectory Goodput** ($G$)—the rate of successfully certified task completions per unit cost. This requires co-designing cache-affinity routing (directing consecutive turns to the same GPU to maximize Radix prefix reuse), predictive admission control (forecasting trajectory horizon lengths), and stateful suspension mechanisms.
...
Let $N$ denote the total number of execution steps (or turns) required for an agent to complete a task. In empirical agent workloads, the complementary cumulative distribution function (CCDF) of $N$ follows a power-law distribution:
...
In real-world agent clusters, empirical measurements reveal tail indices in the range $1.0 < \alpha < 1.4$:
...
Because $\alpha \le 2$, the theoretical variance of the service time distribution is infinite:
...
When variance is unbounded, classical queueing models completely collapse. In an $M/G/1$ queue governed by the Pollaczek-Khinchine formula, the mean waiting time in the queue is directly proportional to the second moment of service time $\mathbb{E}[S^2]$:
...
The scalar $s_i$ is defined as tenant $i$'s **dominant share**.
...
The systems boundary separating safe, repeatable compute from permanent external mutations is defined as the **Reversibility Boundary** [@garciamolina1987sagas].
