...
::: {#fig-vol3-context-length-scaling fig-env="figure" fig-pos="htb" fig-cap="**Historical Context Length Scaling**: The explosive growth in supported context lengths from 2k tokens in 2020 to 2M tokens in 2024 fundamentally altered serving memory dynamics. As context limits scaled logarithmically faster than GPU memory capacity, oblivious request-level schedulers became mathematically guaranteed to trigger out-of-memory deadlocks."}
