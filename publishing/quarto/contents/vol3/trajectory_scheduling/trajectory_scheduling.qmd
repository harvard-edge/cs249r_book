...
When an agent is deployed as part of a cooperating swarm—where multiple parallel agent instances communicate to solve a joint task (e.g., an architect orchestrating three parallel coding workers via a shared message broker)—scheduling individual agents independently causes synchronization deadlocks.
...
: Preemption, suspension, and eviction policies across the four execution zones governed by external irreversibility. {#tbl-vol3-scheduling-policy-matrix}
...
![The Preemption Decision Matrix. Categorization of agent execution into four zones governing whether an in-flight trajectory can be safely preempted, conditionally compensated, or must remain strictly pinned to completion.](images/svg/reversibility_boundary_preemption_matrix.svg){#fig-vol3-scheduling-preemption-matrix}
