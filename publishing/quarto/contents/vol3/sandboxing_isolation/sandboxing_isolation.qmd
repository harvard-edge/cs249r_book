...
::: {#fig-boot-latency-trend fig-env="figure" fig-pos="htb" fig-cap="**The evolution of virtualization boot latency**: Historical progression of cold-boot times, spanning from legacy full VMs (minutes) down to containers, hardware microVMs, and WebAssembly (milliseconds). The exponential decay in startup latency was the primary enabler for ephemeral, per-turn agent sandboxing." fig-alt="Log-scale scatter of cold-boot latency against year of introduction for isolation primitives from full VMs in 2001 to WebAssembly runtimes in 2022, falling about five orders of magnitude. A shaded band marks the sub-hundred-millisecond agentic isolation target that only the newest primitives reach."}
...
data = pd.read_csv("data/virtualization_boot_latency.csv")
