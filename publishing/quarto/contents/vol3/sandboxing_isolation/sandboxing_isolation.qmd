...
Containing side effects within ephemeral sandboxes protects the host, but does not guarantee that the agent completes its delegated task correctly. In @sec-vol3-verification-recovery, we explore verification oracles, the reversibility boundary, and distributed compensating Sagas that enable autonomous error recovery.
