...
```
   ...
   In an agent executing a 30-turn interaction where each turn appends tool definitions, execution logs, and intermediate reasoning, $T$ readily surpasses `{python} TuningMemory.session_tokens_str` tokens. In a 70-billion-parameter model using 16-bit precision and grouped-query attention with `{python} TuningMemory.kv_heads_str` key-value heads, serving a single agent session requires `{python} TuningMemory.kv_gqa_str` of high-bandwidth memory (HBM) exclusively for KV tensors. Multi-head attention over the full model width would cost `{python} TuningMemory.group_ratio_mult_str` that. When thousands of concurrent agents run in parallel, memory bandwidth and capacity become the primary throughput bottleneck [@kaplan2020scaling].

::: {#fig-vol3-tuning-context-scaling fig-env="figure" fig-pos="htb" fig-cap="**The context window explosion**: Foundation model context lengths have scaled exponentially over the last five years. However, while maximum context sizes have reached millions of tokens, populating these contexts with repetitive tool definitions and few-shot exemplars introduces severe KV-cache memory saturation and inference latency penalties." fig-alt="Log-scale scatter of model context length by release date, rising from two thousand tokens to two million across the 2020 to 2024 window."}
