...
1. **Computational Complexity and Key-Value Cache Saturation:**
   ...
   In an agent executing a 30-turn interaction where each turn appends tool definitions, execution logs, and intermediate reasoning, $T$ readily surpasses 32,000 tokens. In a 70-billion-parameter model using 16-bit precision and grouped-query attention with eight key-value heads, serving a single agent session requires roughly 10 GB of high-bandwidth memory (HBM) exclusively for KV tensors. Multi-head attention over the full model width would cost eight times that. When thousands of concurrent agents run in parallel, memory bandwidth and capacity become the primary throughput bottleneck [@kaplan2020scaling].
...
For agent serving systems, the true power of LoRA lies in runtime multi-tenancy. Modern serving runtimes (such as S-LoRA and Punica) exploit the property that a LoRA adapter is small next to the base model it steers: at rank $r=16$ over the query and value projections, roughly 80 megabytes for a 70-billion-parameter model and under 20 megabytes for a 7-billion-parameter one.
