...
As briefly introduced in @sec-vol3-context-working-sets, to overcome the fatal fragmentation of static contiguous allocation, Kwon et al. introduced PagedAttention, an architecture inspired by classic operating system virtual memory paging [@kwon2023; @denning1970].
...
- **Prefill Phase:** Computes across all input prompt tokens simultaneously using large matrix-matrix multiplications (GEMM). Operating in the compute-bound regime of the hardware Roofline model, it achieves high Arithmetic Intensity (exceeding $100\text{ FLOPs/byte}$) and saturates Tensor Cores at peak utilization.
...
To overcome node-level isolation, Qin et al. introduced Mooncake (introduced conceptually in @sec-vol3-context-working-sets), a KVCache-centric disaggregated architecture that elevates the KV cache to a first-class distributed storage system across the entire datacenter [@qin2024mooncake].
