...
: **KV-Cache Footprint by Attention Architecture**: Per-token KV bytes and total context buffer requirements for frontier model architectures at 16-bit precision. Grouped-query and latent attention cut the per-token cost by nearly an order of magnitude relative to multi-head attention, which is what makes long multi-turn agent contexts affordable at all. {#tbl-vol3-caching-model-kv-footprint}
