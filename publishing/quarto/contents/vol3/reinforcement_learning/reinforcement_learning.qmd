...
**Variables**: $P = 70 \times 10^9$ parameters, with $1\text{ GB} = 10^9$ bytes. Mixed-precision Adam holds, per trainable parameter, $2\text{ bytes}$ of BF16 weights, $2\text{ bytes}$ of BF16 gradients, $4\text{ bytes}$ of FP32 master weights, and $4\text{ bytes} + 4\text{ bytes}$ of FP32 momentum and variance. The frozen reference model holds BF16 weights only, at $2\text{ bytes}$ per parameter. Accelerators carry $80\text{ GB}$ of HBM each, eight to a node. Two values are scenario assumptions: the Critic matches the Actor in parameter capacity, and the byte budget counts persistent state only, excluding activations and KV cache, so every total below is a floor.
