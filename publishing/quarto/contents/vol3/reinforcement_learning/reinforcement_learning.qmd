...
- **Inference Actor Cluster (GPU Workers):** Dedicated GPU nodes running high-throughput serving engines (vLLM, SGLang) execute batched autoregressive generation. These nodes manage Radix attention caches and KV memory pools, sampling candidate actions from $\pi_{\theta_{\text{old}}}$ across thousands of concurrent queries. Because GRPO samples $G$ independent responses for the exact same prompt $q$, the inference engine computes the prefix KV cache for $q$ exactly once and shares it across all $G$ trajectories, dramatically reducing VRAM consumption and Time-To-First-Token (TTFT).
