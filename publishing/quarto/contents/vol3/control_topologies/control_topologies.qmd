...
which gives $80\text{ s}$ at $M=2$, $40\text{ s}$ at $M=6$, and $30\text{ s}$ at $M=12$. @Fig-agent-amdahls-law generalizes that result across serial fractions, and the shape is the argument: each curve bends toward a ceiling fixed by the planner and aggregator, so the fan-out decision is a question of what fraction of the plan is genuinely parallel rather than how many workers are available.
...
::: {#fig-agent-amdahls-law fig-env="figure" fig-pos="htb" fig-cap="**Amdahl's Law in Agent Topologies**: Speedup against worker count for three serial fractions, each flattening toward its own ceiling. The unparallelizable segment is planner and aggregator inference, so the ceiling is set by two model calls that do not shrink when workers are added." fig-alt="Speedup curves against number of concurrent worker agents for three serial-fraction scenarios, each flattening toward its own asymptotic ceiling marked by a dashed horizontal line. Higher coordination overhead produces a lower ceiling reached at fewer agents."}
...
data = pd.read_csv("data/agent_amdahls_law.csv")
fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 5))
_groups = list(dict.fromkeys(data["Scenario"]))
_cmap = plt.get_cmap('viridis', max(len(_groups), 2))
for _i, _g in enumerate(_groups):
    _sub = data[data["Scenario"] == _g]
    ax.plot(_sub["Workers"], _sub["Speedup"], color=_cmap(_i), label=str(_g), linewidth=2.5)

# Add theoretical asymptotes (1/s)
for s, color in zip([0.05, 0.143, 0.33], [COLORS["BlueLine"], COLORS["OrangeLine"], COLORS["GreenLine"]]):
    plt.axhline(y=1/s, color=color, linestyle="--", alpha=0.6)

plt.xlabel("Number of Concurrent Worker Agents")
plt.ylabel("Effective Latency Speedup")
plt.title("Agentic fan-out scalability ceiling")
plt.xlim(1, 32)
plt.ylim(1, 22)
plt.legend(title="Execution Scenario")
plt.tight_layout()
plt.show()
plt.close()
```
...
::: {#fig-vol3-topologies-anti-swarm fig-env="figure" fig-pos="htb" fig-cap="**The Anti-Swarm Principle (Mesh Swarms vs. Hierarchical Supervisor Trees)**: Contrast between chaotic peer-to-peer mesh swarms suffering quadratic communication explosion and production supervisor trees with strict unidirectional delegation, context isolation barriers, and schema-gated verification. Count the edges rather than the agents, because the mesh grows connections quadratically while the tree grows them linearly, so a swarm's cost appears in coordination traffic before it appears in useful work." fig-alt="Two-panel comparison illustrating the Anti-Swarm Principle: left panel shows four agents connected in a peer-to-peer mesh with quadratic message chaos and failure modes; right panel shows a root supervisor agent delegating tasks down to isolated worker agents with a context isolation boundary and upward verification gatekeeper."}
...
A control topology does not make a foundation model infallible; it ensures that the failure of an individual step does not compromise the host system. By governing transitions with external invariants, circuit breakers, and state machine graphs, engineers ensure that autonomous agency remains bounded, observable, and interruptible under any operational failure mode.

::: {.callout-chapter-connection title="From control topologies to trajectory fine-tuning"}
Structural graphs provide the external scaffolding for agent execution, but navigating complex decision graphs requires neural policies trained specifically for multi-turn reasoning and tool invocation. With the structural foundations of agency in place, **Part II (Agent Training and Test-Time Compute)** examines how foundation models are adapted to navigate stateful graphs: beginning in @sec-vol3-trajectory-fine-tuning with trajectory-level fine-tuning and supervised policy alignment.
:::
