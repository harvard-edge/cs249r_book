...
::: {#fig-agent-amdahls-law fig-env="figure" fig-pos="htb" fig-cap="**Amdahl's law in agent topologies**: The theoretical maximum speedup of a fan-out topology is strictly bounded by the serial fraction $s$. As concurrent worker agents scale, the constant time of planner inference and aggregator synthesis dominates the total latency, creating a hard scalability ceiling." fig-alt="Speedup curves against number of concurrent worker agents for three serial-fraction scenarios, each flattening toward its own asymptotic ceiling marked by a dashed horizontal line. Higher coordination overhead produces a lower ceiling reached at fewer agents."}
