...
Every one of these fields has a counterpart in the POSIX process control block, but the counterparts are not equivalents. In each row of @tbl-pcb-acb-comparison, a hardware-enforced kernel mechanism gives way to a software convention with no silicon behind it, and the context switch that an MMU performs in microseconds takes the agent runtime milliseconds to seconds.
...
::: {#fig-acb-lifecycle fig-env="figure" fig-pos="htb" fig-cap="**The Complete ACB Lifecycle Finite State Machine**: State transitions between initialized, active inference phases (Prefill and Decode), suspended states (Tool-Wait and Human-Gate), memory swapping, Sagas rollback, and terminal states, governed by invariant evaluation. The two suspended states are where an agent spends most of its wall clock, holding resident memory while doing no inference, so the transitions into and out of them decide serving cost." fig-alt="State machine diagram showing the ACB lifecycle. States include Initialized, Active Prefill, Active Decode, Suspended (Tool-Wait), Suspended (Human-Gate), Memory Swapping, Sagas Rollback, Terminated (Success), and Terminated (Failed), connected by transition arrows with guard conditions."}
...
If a tool invocation is predicted to block for more than $T_{\text{breakeven}} \approx 500\,\text{ms}$ (as is the case for virtually all network APIs, compiler invocations, and test suites), the memory manager immediately initiates asynchronous DMA transfer to host DRAM. When the tool completion webhook arrives, the scheduler prefetches the blocks back to GPU HBM before the agent's turn arrives in the run queue, completely hiding transfer latency. That threshold is not a constant of the runtime; it is a property of the link. @Fig-acb-breakeven sweeps context length across three PCIe generations, and the breakeven point moves by roughly a factor of four between Gen3 and Gen5 for the same model.

::: {#fig-acb-breakeven fig-env="figure" fig-pos="htb" fig-cap="**KV-Cache Swapping Breakeven Thresholds**: Round-trip DMA transfer time against context length for three PCIe generations, on a 70B model. Every generation rises linearly with context, and the breakeven suspension duration rises with it, so the decision to swap depends on the bus as much as on the expected block time." fig-alt="Log-scale line chart of KV-cache transfer time against context length for three PCIe generations. All three rise linearly on the log axis, with Gen5 roughly four times faster than Gen3 at every context length."}
...
# Load data
data = pd.read_csv("data/breakeven_thresholds.csv")

# Filter for the 70B model to keep the plot focused
data_70b = data[data["Model"] == "70B (327 KB/token)"]

fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 5))
_groups = list(dict.fromkeys(data_70b["PCIe Gen"]))
_cmap = plt.get_cmap('viridis', max(len(_groups), 2))
for _i, _g in enumerate(_groups):
    _sub = data_70b[data_70b["PCIe Gen"] == _g]
    ax.plot(_sub["Context Length"], _sub["Breakeven Time (ms)"], color=_cmap(_i), label=str(_g), marker="o", linewidth=2)

plt.xscale("log", base=2)
plt.yscale("log")
plt.xlabel("Context Length (Tokens)")
plt.ylabel("Breakeven Transfer Time (ms)")
plt.title("KV-cache PCIe transfer latency (70B model, FP16)")
plt.xticks([8192, 16384, 32768, 65536, 131072], ["8K", "16K", "32K", "64K", "128K"])
plt.grid(True, which="both", ls="--", alpha=0.5)
plt.legend(title="Bus Bandwidth")
plt.tight_layout()
...
Failing to implement context swapping causes idle agents awaiting HTTP responses to lock up physical GPU VRAM. The size of that loss is worth computing rather than asserting, because it is what decides whether swapping is worth its own cost.

::: {#nbk-descriptors-tool-wait-concurrency .callout-notebook title="What tool waits cost a resident cluster"}

**Problem**: A node holds 100 agent sessions resident in HBM. Each step generates a short tool call, then blocks on a web request. How many of those 100 VRAM allocations are doing GPU work at any instant?

**Variables**:

* $N = 100$ resident agent sessions.
* $t_{\text{gen}} = 1.2\text{ s}$, the decode time for one tool-call turn. Scenario assumption.
* $t_{\text{wait}} = 5.0\text{ s}$, the average external tool latency. Scenario assumption.

**Math**: Each session completes one step every $W = t_{\text{gen}} + t_{\text{wait}} = 1.2 + 5.0 = 6.2\text{ s}$, so the node's step arrival rate is

$$\lambda = \frac{N}{W} = \frac{100}{6.2} = 16.1\ \text{steps/s}$$

Applying Little's Law to the decode stage alone, where the residence time is $t_{\text{gen}}$ rather than the whole step:

$$L_{\text{gpu}} = \lambda \, t_{\text{gen}} = 16.1 \times 1.2 = 19.4$$

**Result**: About 19 of the 100 resident sessions occupy the accelerator at any instant. The other 81 hold their KV cache in HBM while contributing nothing, so memory allocation reads $100\%$ while useful concurrency sits near $19\%$.

**Systems insight**: The collapse is a duty cycle, $t_{\text{gen}} / (t_{\text{gen}} + t_{\text{wait}})$, and it is set by the environment rather than by the model. Faster decoding makes it worse, because shrinking $t_{\text{gen}}$ shrinks the numerator while the tool latency stays fixed. This is why the descriptor has to be able to release memory during a suspension, and it is the term $T_{\text{breakeven}}$ is weighed against: swapping is worth its round-trip only when the wait is long enough that those idle allocations exceed the cost of moving them.
...
The Agent Control Block is what makes principle \ref{pri-vol3-trajectory-encapsulation} implementable. Treating the trajectory as the atomic unit is a claim about scope until some structure actually holds that scope, and the ACB is that structure: it is where a token allowance, a capability lease and a recovery handle outlive the request that created them.

Principle \ref{pri-vol3-acb-binding} is refined here into a field list and a state machine. The descriptor must bind principal identity, allowance and lease because each is a quantity a single request cannot bound, and the lifecycle states exist because an agent suspended on a tool call is neither running nor finished.

## Summary {#sec-vol3-descriptors-summary}

The Agent Control Block transforms the trajectory from an ephemeral prompt abstraction into an authoritative, kernel-managed computing entity. By explicitly decoupling external ground-truth state from the model's stochastic internal activations, the ACB provides the foundational systems substrate required to schedule, isolate, meter, and recover long-running agentic workloads.

::: {.callout-takeaways title="The Agent Control Block and lifecycle invariants"}
...
An execution descriptor is ultimately an assertion of authority over stochastic generation. When an autonomous runtime treats an agent as a structured operating system entity rather than an unconstrained chat stream, capability leases, memory reclamation, and crash recovery become solvable systems engineering problems rather than prompt engineering heuristics.

::: {.callout-chapter-connection title="From execution blocks to control topologies"}
An isolated execution descriptor defines how a single agent state is preserved and metered, but autonomous tasks rarely execute as isolated linear loops. @Sec-vol3-control-topologies addresses how these execution units are wired into multi-step architectures: formalizing directed acyclic graphs, supervisor-worker hierarchies, and semantic cycle detectors that prevent reasoning loops from degenerating into unbounded token burn.
:::
