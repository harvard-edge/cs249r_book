...
::: {#fig-acb-breakeven fig-env="figure" fig-pos="htb" fig-cap="**KV-cache swapping breakeven thresholds**: The round-trip DMA transfer latency dictates the breakeven suspension duration ($T_{\text{breakeven}}$). As context lengths scale to 128K tokens, swapping a 70B model's KV cache over older PCIe Gen3 fabrics incurs multi-second penalties, making asynchronous swapping viable only for long-running tool invocations." fig-alt="Log-scale line chart of KV-cache transfer time against context length for three PCIe generations. All three rise linearly on the log axis, with Gen5 roughly four times faster than Gen3 at every context length."}
