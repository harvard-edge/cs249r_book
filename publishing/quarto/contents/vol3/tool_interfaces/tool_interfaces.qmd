...
- Implement grammar-constrained logit decoding via Finite State Automata (FSAs) to guarantee 100 percent syntactic compliance at zero latency overhead.
...
: **Three Generations of Tool Invocation**: Interface mechanism, protocol contract, failure modes, and context overhead across the three generations of model-to-software interfaces. Each generation moves failure earlier in the pipeline, from parse-time text scraping, to validation-time function calling, to typed wire protocols that leave only semantic errors. {#tbl-vol3-tools-generation-evolution}
...
Generation 3 establishes formal tool ABIs. Tools are registered via typed schemas (JSON Schema Draft 2020-12). At inference time, the model runtime compiles schemas into finite state automata that physically prevent the generation of invalid token sequences. Standardized wire protocols such as the Model Context Protocol (MCP) decouple the host agent runtime from local and remote tool implementations. The three generations differ in where correctness is enforced, and @fig-tool-syntax-errors shows what moving that enforcement into the decoder buys: scraping left roughly one call in five malformed, native JSON reduced the rate without eliminating it, and schema-compiled automata drove it to zero because a malformed token can no longer be sampled.

::: {#fig-tool-syntax-errors fig-env="figure" fig-pos="htb" fig-cap="**Tool Invocation Syntax Error Rates (2022--2025)**: Malformed-call rate across three generations of agent-to-software interface, falling from roughly 21 percent under text scraping to effectively zero under constrained decoding. The decisive change is architectural: the third generation makes an invalid call unrepresentable rather than merely unlikely." fig-alt="Chart of tool invocation syntax error rate over time across three generations, dropping from about 21 percent under text scraping to near zero under constrained decoding, with vertical markers at each generational transition."}
...
df = pd.read_csv("data/syntax_error_rates.csv")
df['Date'] = pd.to_datetime(df['Date'])

fig, ax, COLORS, plt = viz.setup_plot(figsize=(7, 3.5))

generations = df['Generation'].unique()
colors = ['#d9534f', '#f0ad4e', '#5cb85c']
markers = ['o', 's', 'D']

for gen, color, marker in zip(generations, colors, markers):
    gen_data = df[df['Generation'] == gen]
    ax.plot(gen_data['Date'], gen_data['ErrorRate'], marker=marker, markersize=5,
            linewidth=2, label=gen, color=color, alpha=0.9)
...
# Annotations
ax.axvline(pd.to_datetime('2023-06-01'), color='gray', linestyle='--', alpha=0.5)
ax.text(pd.to_datetime('2023-06-15'), 18, 'Native JSON\nIntroduced', 
        fontsize=9, color='gray', va='center')

ax.axvline(pd.to_datetime('2024-06-01'), color='gray', linestyle='--', alpha=0.5)
ax.text(pd.to_datetime('2024-06-15'), 18, 'Constrained\nDecoding', 
        fontsize=9, color='gray', va='center')
...
::: {#fig-vol3-tools-mcp-architecture fig-env="figure" fig-pos="htb" fig-cap="**MCP Architecture and Capability Negotiation**: A host application, a client protocol manager, and decoupled servers exchange JSON-RPC 2.0 messages over stdio or server-sent events after a handshake that discovers tools, resources, and prompts. The layer worth locating is the host policy and approval gateway, because the protocol standardizes serialization while authorization stays on the host side of that boundary." fig-alt="A layered diagram. The top block is the MCP host application, holding an agent runtime and inference core that formats tool schemas into the system prompt and emits structured tool calls. Below it, a client protocol manager lists three numbered stages, handshake and initialize, dynamic discovery, and invocation and sampling, beside a host policy and approval gateway labeled as the true authorization boundary. A duplex transport layer underneath shows stdio for local subprocesses and server-sent events over HTTP for remote services, with a sample JSON-RPC request and its matching response displayed side by side."}
...
That warning describes the direction most deployments consider, in which arguments travel from the model through the client and into a server. Setup traffic runs the other way. The client acts on fields the server returns before any tool has been called, and a critical vulnerability disclosed in 2025 against a widely deployed MCP proxy began in one of them (\ref{ws-tools-mcp-remote-2025}).

::: {#ws-tools-mcp-remote-2025 .callout-war-story title="The mcp-remote authorization endpoint (2025)"}

**Context**: The `mcp-remote` package is a proxy that lets hosts that speak only the local stdio transport, including Claude Desktop, Cursor, and Windsurf, reach remote MCP servers over HTTP. Configuring it takes a single field, the remote server's URL. JFrog Security Research disclosed CVE-2025-6514 against the package on July 9, 2025, with a base score of 9.6 [@jfrog2025mcpremote].

**Mechanism**: When the remote server answered the first request with a 401, the proxy opened an OAuth flow and fetched the server's metadata document, whose `authorization_endpoint` field names a URL the client is expected to open in a browser. The client checked that field only by passing it to the URL constructor, which tests syntax and nothing more, then handed the result to a helper that opens a link in the desktop browser. On Windows that helper builds a PowerShell command line. A value carrying an unrecognized scheme is left unencoded by the URL constructor, so a string of the form `a:$(cmd.exe /c whoami)` passed validation intact and PowerShell evaluated the subexpression. Every component behaved as documented. The defect was that a field arriving from the far end of the protocol was treated as trusted.

**Impact**: Versions 0.0.5 through 0.1.15 were affected. On Windows, JFrog demonstrated arbitrary operating system command execution with full control of arguments, amounting to full compromise of the developer machine hosting the client. On macOS and Linux, where the helper calls `open` or `xdg-open` and no shell is invoked, the same field yields execution of arbitrary executables with limited control of arguments. Two routes reach the bug. One is connecting to a malicious or hijacked server. The other is connecting to any server over plain HTTP while an attacker on the local network redirects the traffic, a likely case because MCP clients tend to trust servers on the same network and connect to them insecurely.

**Fix**: Version 0.1.16, published on June 17, 2025 ahead of public disclosure, routes the discovered URL through a sanitizer before opening it. The sanitizer rejects any scheme other than `http` and `https`, rejects a host containing any character that percent-encoding would alter, and re-encodes the user information, path, query, and fragment. The standing mitigation is to connect only to trusted MCP servers over HTTPS.

**Systems lesson**: A tool protocol types every field and types nothing about trust. The `authorization_endpoint` value had the declared type and valid URL syntax and was still hostile, because parsing as a URL says nothing about what the program receiving it will do. Validation at a protocol boundary therefore has a direction, and the client owes a server's replies the same suspicion the host owes the model's arguments. The operational form of that rule is narrow enough to implement. Any value crossing into an interpreter (a shell, a browser launcher, a query engine) must be validated against the grammar of that interpreter, not merely against the grammar of the wire.
...
```{python}
#| label: vol3-tools-schema-tax
#| echo: false
# +-----------------------------------------------------------------------------
# | PROMPT TOKEN TAX OF A TOOL CATALOG
# +-----------------------------------------------------------------------------
# | Context: The two bullets below and @tbl-vol3-tools-deep-vs-shallow.
# |
# | Goal:    Price a shallow tool catalog against a single deep module, in the
# |          currency the agent actually spends: prefill tokens per step.
# | Show:    Static schema footprint, its cost across a trajectory, and the
# |          reduction a deep module buys.
# | How:     Multiply catalog size by per-tool schema cost, then by trajectory
# |          depth, since the whole catalog is re-read on every step. The tool
# |          counts and schema sizes are design assumptions of this example and
# |          are declared here rather than pulled from a registry; no vendor
# |          publishes a canonical bytes-per-tool-schema figure.
# +-----------------------------------------------------------------------------
from mlsysim import *

def _pct_precision(ratio):
    """Keep a fractional endpoint without inventing one on a whole endpoint."""
    value = ratio * 100
    return 0 if abs(value - round(value)) < 1e-9 else 1

class SchemaTax:
    # +-- 1. LOAD (Constants) --------------------
    shallow_tools = 100  # scenario assumption: one tool per Kubernetes operation
    tokens_per_tool = 350  # scenario assumption: typical JSON schema for one tool
    deep_tokens = 120  # scenario assumption: schema for a single kubectl primitive
    trajectory_steps = 30  # scenario assumption: steps in the worked trajectory
    catalog_min, catalog_max = 50, 200  # scenario assumption: shallow catalog range
    deep_min, deep_max = 2, 5  # scenario assumption: deep catalog range
    footprint_min, footprint_max = 20_000, 60_000  # scenario assumption: shallow prompt range
    deep_footprint_min, deep_footprint_max = 500, 1_500

    # +-- 2. EXECUTE (The Compute) ---------------
    shallow_footprint = shallow_tools * tokens_per_tool
    trajectory_tokens = shallow_footprint * trajectory_steps
    schema_reduction = 1 - deep_tokens / shallow_footprint
    count_reduction_low = 1 - deep_max / catalog_min
    count_reduction_high = 1 - deep_min / catalog_max
    footprint_reduction_low = 1 - deep_footprint_max / footprint_min
    footprint_reduction_high = 1 - deep_footprint_min / footprint_max

    # +-- 3. GUARD (Invariants) ------------------
    check(shallow_footprint == 35_000, f"The worked catalog should cost 35,000 tokens, got {shallow_footprint}")
    check(trajectory_tokens > 1_000_000, f"A 30-step trajectory should re-read more than a million tokens, got {trajectory_tokens}")
    check(0.99 < schema_reduction < 1.0, f"A deep module should cut schema overhead by more than 99 percent, got {schema_reduction}")
    check(
        count_reduction_low < count_reduction_high and footprint_reduction_low < footprint_reduction_high,
        "Each reduction range must run from its weakest to its strongest case",
    )

    # +-- 4. OUTPUT (Formatting) -----------------
    shallow_tools_str = fmt_int(shallow_tools)
    tokens_per_tool_str = fmt_int(tokens_per_tool)
    deep_tokens_str = fmt_int(deep_tokens)
    trajectory_steps_str = fmt_int(trajectory_steps)
    shallow_footprint_str = fmt_int(shallow_footprint)
    trajectory_tokens_str = fmt_int(trajectory_tokens)
    schema_reduction_str = fmt_percent(schema_reduction, precision=1, style="prose")
    count_range_str = fmt_range(catalog_min, catalog_max, kind="number", precision=0)
    deep_range_str = fmt_range(deep_min, deep_max, kind="number", precision=0)
    footprint_range_str = fmt_range(footprint_min, footprint_max, kind="number", precision=0)
    deep_footprint_range_str = fmt_range(deep_footprint_min, deep_footprint_max, kind="number", precision=0)
    count_reduction_range_str = fmt_text(
        fmt_percent(count_reduction_low, precision=_pct_precision(count_reduction_low), style="number"),
        "\u2013",
        fmt_percent(count_reduction_high, precision=_pct_precision(count_reduction_high), style="symbol"),
    )
    footprint_reduction_range_str = fmt_text(
        fmt_percent(footprint_reduction_low, precision=_pct_precision(footprint_reduction_low), style="number"),
        "\u2013",
        fmt_percent(footprint_reduction_high, precision=_pct_precision(footprint_reduction_high), style="symbol"),
    )
```

- **The Shallow Approach (One Tool per Operation):**
  The architect exposes discrete tools for every possible operation: `list_pods`, `get_pod_logs`, `delete_pod`, `scale_deployment`, `restart_statefulset`, `update_ingress`, and so on.
  Each tool requires approximately `{python} SchemaTax.tokens_per_tool_str` tokens of JSON schema documentation, so a catalog of `{python} SchemaTax.shallow_tools_str` tools consumes:
  $$\text{Schema Footprint} = N_{\text{tools}} \times \text{tokens per schema}$$
  That is `{python} SchemaTax.shallow_footprint_str` tokens of the context window spent on static API signatures before the user types a single prompt character. Across a `{python} SchemaTax.trajectory_steps_str`-step trajectory the model re-reads them on every step, spending `{python} SchemaTax.trajectory_tokens_str` tokens of prefill compute on a catalog that never changes. Furthermore, large tool catalogs induce **attention dilution**: the model's attention heads struggle to distinguish between overlapping parameter signatures, increasing parameter hallucination rates.

- **The Deep Approach (Universal Primitive):**
  The architect exposes a single deep module: `kubectl_exec(command: string)`.
  The tool schema requires `{python} SchemaTax.deep_tokens_str` tokens.
  The model uses its pretrained understanding of the `kubectl` CLI syntax to compose commands dynamically, reducing schema overhead by `{python} SchemaTax.schema_reduction_str` while supporting the entire surface area of the Kubernetes API.
...
|:----------------------------|:---------------------------------------------------:|:----------------------------------------------------:|:-----------------------------------------------------------------------------------------:|
| **Interface Surface**       | `{python} SchemaTax.count_range_str` distinct tools | `{python} SchemaTax.deep_range_str` expressive tools |     `{python} SchemaTax.count_reduction_range_str` reduction in registered tool count     |
| **System Prompt Footprint** |   `{python} SchemaTax.footprint_range_str` tokens   | `{python} SchemaTax.deep_footprint_range_str` tokens | Saves `{python} SchemaTax.footprint_reduction_range_str` of prefill compute and KV memory |
...
: **Shallow Micro-Tools versus Deep Modules**: Interface surface, system-prompt footprint, attention accuracy, compositionality, and validation burden for the two tool-catalog designs. The tool-count and prompt-footprint rows move together, which is why collapsing a catalog into a few deep modules buys back prefill compute and attention headroom at the same time. {#tbl-vol3-tools-deep-vs-shallow}
...
Even with advanced alignment, there is a nonzero probability that the model samples a token that breaks JSON syntax (e.g., an unescaped double quote inside a string, a trailing comma before a closing bracket, or a malformed numeric literal). In production pipelines, an unhandled syntax error causes an immediate exception, requiring an expensive re-prompting roundtrip that wastes tokens and latency.
...
- **Vocabulary Indexing:** Because language models generate multi-character tokens rather than single characters, the tokenizer vocabulary $V$ (typically 32,000 to 128,000 tokens) is preprocessed against the automaton. For each state $q \in Q$, the compiler precomputes the set of **allowed tokens** $A(q) \subseteq V$:
...
::: {#fig-vol3-tools-grammar-decoding fig-env="figure" fig-pos="htb" fig-cap="**Grammar-Constrained Logit Decoding**: A tool schema compiles ahead of time into a finite state automaton and a per-state token bitmask, and at each decode step a GPU kernel drives the logits of every disallowed token to $-\infty$. Applying the mask before the softmax is what makes the guarantee structural: an invalid token carries exactly zero probability, so syntactic compliance becomes a property of the sampler rather than of model behavior." fig-alt="A two-stage diagram. The upper stage shows ahead-of-time compilation, in which a typed schema class becomes a regular expression and a deterministic automaton with labeled states, which in turn yields a precomputed per-state vocabulary bitmask. The lower stage shows real-time masking, in which unconstrained logits for several candidate tokens feed a CUDA kernel that leaves the single allowed token at its original score and drives every other token to negative infinity."}

![](images/svg/grammar_constrained_decoding_fsm.svg)

:::

@Fig-vol3-tools-grammar-decoding traces the three systems benefits this architecture yields:

- **Zero Syntax Failure Rate:** It is mathematically impossible for the engine to emit an invalid JSON delimiter, unclosed brace, or illegal field name.
...
Modern serving runtimes such as Outlines, Guidance, SGLang, and vLLM provide native support for grammar-constrained decoding. Whether the per-step mask kernel earns its place depends on how often an unconstrained decoder would have emitted a malformed call in the first place.

::: {#nbk-tools-mask-breakeven .callout-notebook title="When logit masking pays for itself"}

**Problem**: Grammar-constrained decoding adds a mask kernel to every decode step but removes the repair turn that follows a malformed tool call. How often must unconstrained decoding fail before the mask is the cheaper option?

**Variables**: A typed JSON tool call of $150$ output tokens, a decode latency of $20\text{ ms}$ per output token, the mask kernel overhead of $0.5\text{ ms}$ per step stated above, a structured error diff of $250$ tokens appended on a validation failure, and prefill at $20{,}000$ tokens per second. Every figure except the mask overhead is a scenario assumption.

**Math**: Constrained decoding pays the mask on every step, so the tax per call is $150 \times 0.5 = 75\text{ ms}$. Generating the call once costs $150 \times 20 = 3000\text{ ms}$. A repair turn re-prefills the failed call plus the diff, $(150 + 250)/20{,}000 = 20\text{ ms}$, then regenerates the call, giving $20 + 3000 = 3020\text{ ms}$. Writing $p$ for the unconstrained failure rate, the two paths cost

$$3000 + 75 \quad \text{versus} \quad 3000 + p \cdot 3020$$

and they cross at $p^{*} = 75/3020 = 0.0248$.

**Result**: The crossover sits at a $2.5\%$ syntax failure rate, one malformed call in $40$. The mask tax itself is $75\text{ ms}$, or $2.5\%$ of the call's decode time. Framed over the $30$-step trajectory used earlier, masking every step of every call costs $30 \times 75 = 2250\text{ ms}$, less than the $3020\text{ ms}$ a single repair turn costs, so one bad call anywhere in the trajectory already pays for masking on all thirty steps.

**Systems insight**: The call length cancels out of the crossover, leaving $p^{*} \approx 0.5/20$, the ratio of mask time to decode time per token, so the break-even is a property of the serving kernel rather than of any individual tool schema. A runtime that clears it clears it for every tool at once, which is why grammar-constrained decoding is enabled globally at the engine instead of negotiated schema by schema.
...
Unlike human developers who can read changelogs and intuitively adapt to updated APIs, language models are notoriously brittle when interacting with evolving tool schemas. If a backend system adds a new required parameter to an existing tool, an agent relying on few-shot examples, pretrained weights, or static prompts tailored to the V1 schema will systematically fail.
...
@Fig-vol3-tools-idempotency-flow follows the tool gateway as it coordinates idempotency through a distributed state store such as Redis or DynamoDB:
...
::: {#fig-vol3-tools-idempotency-flow fig-env="figure" fig-pos="htb" fig-cap="**Idempotency Nonce Pipeline**: A mutating tool call is hashed into a nonce over session, step, tool name, and canonical arguments, and the gateway resolves that key against a distributed state store before any side effect runs. The three lookup outcomes are the whole mechanism: they separate a first execution from a concurrent retry and from a completed call whose cached result replays without touching the sandbox, which is what lets an at-least-once transport carry exactly-once actions." fig-alt="A three-stage pipeline. The first stage shows a mutating funds-transfer tool call fed into a cryptographic nonce generator that hashes session identifier, step number, tool name, and canonical arguments into an idempotency key carried as a request header. The second stage shows a gateway looking that key up in a distributed state store with three outcomes, not found, in flight, and completed, leading respectively to lock acquisition, a wait or conflict response, and a cached replay that bypasses the sandbox. The third stage shows sandboxed execution followed by a write-ahead log commit."}
...
This accessibility compilation compresses web pages by up to 90 percent, allowing agents to navigate complex multi-page workflows within tight context budgets.
...
## Summary {#sec-vol3-tools-summary}

Tool interfaces govern the crucial boundary where statistical token generators interact with deterministic system APIs. By replacing unstructured natural language string parsing with typed, versioned ABIs (such as the Model Context Protocol), grammar-constrained logit masking, and cryptographic idempotency keys, systems turn brittle text completions into enforceable system calls.

::: {.callout-takeaways title="Model Context Protocol and tool invocation contracts"}
...
* **Ousterhout’s Deep Modules:** Designing a few expressive, deep tools (such as persistent shells and atomic file editors) saves up to 95 percent of prompt token overhead compared to shallow micro-tool catalogs.
* **Grammar-constrained logit decoding:** Compiling schemas into Deterministic Finite Automata (DFAs) allows CUDA kernels to mask invalid token logits to $-\infty$, mathematically guaranteeing 100 percent syntactic compliance at sub-millisecond latency.
...
Standardizing the tool interface turns the model from a passive responder into an active client of external systems. Yet structured schemas and grammar-constrained decoding only guarantee syntactic validity: a syntactically perfect JSON payload can still issue a catastrophic command or delete a production database.

::: {.callout-chapter-connection title="From tool protocols to execution sandboxing"}
Structured tool protocols establish how an agent expresses operational intent, but syntactic validity offers zero defense against destructive actions or prompt injection attacks. @Sec-vol3-sandboxing-isolation moves beneath the wire protocol to explore execution sandboxing: enforcing hardware virtualization, ephemeral Copy-on-Write workspaces, and seccomp syscall filters to contain untrusted agent reasoning within strict fault domains.
:::
