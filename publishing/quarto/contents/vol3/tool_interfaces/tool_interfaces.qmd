...
To guarantee **at-most-once execution semantics**, every mutating tool invocation must be bound to a unique, deterministic cryptographic nonce $\eta$ synthesized from the trajectory invariants (as defined in @sec-trajectory-contract):
...
- **Offline Catalog Indexing:** Every tool is indexed in a vector store (discussed in @sec-vol3-episodic-memory). The indexing payload includes the tool name, docstring description, and sample usage queries embedded via a dense text embedding model.
