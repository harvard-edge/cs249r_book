...
- `AgentDelegate(task_prompt: str)`: Sub-agent spawning primitive for hierarchical task decomposition.
...
- **Dynamic Schema Injection and Eviction:** Only the top-$K$ schemas are injected into the active prompt. Once the sub-task completes, the specialized schemas are evicted, restoring the context working set to its baseline footprint.
