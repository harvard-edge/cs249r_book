...
Even under Grouped-Query Attention with $g = 8$, operational intensity during decode rarely exceeds $4\ \text{FLOPs/Byte}$—two orders of magnitude below the machine balance point $I^* = 295.4\ \text{FLOPs/Byte}$. Autoregressive decode utilizes less than 2 percent of the H100's raw arithmetic capacity. The time to generate a single token is entirely dominated by the memory transfer latency of reading multi-gigabyte KV caches off HBM:
...
As detailed in @tbl-vol3-context-decode-economics, expanding context from 4k to 524k tokens increases Time-To-First-Token (TTFT) by over 230$\times$ and collapses decode throughput from 84 tokens per second to 5 tokens per second. In multi-step agent loops where an agent executes 50 to 100 tool cycles, maintaining an unmanaged 500k context turns an interactive system into an economically non-viable crawl.
