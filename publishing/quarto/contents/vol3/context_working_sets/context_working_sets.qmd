...
## Introduction to Context Working Sets: Memory Hierarchies in Autonomous Agents {#sec-vol3-context-intro}
...
- **Tier 1 (L1 Context Working Set):** The active transformer attention window holding pinned system instructions, active tool schemas, and the immediate operational working set $W(t, \Delta t)$.
- **Tier 2 (L2 KV-Cache Prefix Store and Paging):** High-speed Radix tree caches residing in GPU HBM and host DRAM, enabling sub-millisecond prefix reuse across multi-turn steps and branching search trees.
...
- **The Commit Phase (Pruning and State Mutation):** The runtime extracts the executable tool call $a_t$, executes it within the sandbox, and receives environment observation $o_t$. Before appending to the persistent trajectory history, the runtime **discards the ephemeral CoT tokens**, committing only the structured tuple:
