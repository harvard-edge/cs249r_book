...
As illustrated in @fig-vol3-context-working-set-dynamics, treating context as an unmanaged, append-only repository inexorably drives the system toward the HBM capacity ceiling. On an NVIDIA H100 SXM5 GPU carrying `{python} ContextHeadroom.capacity_gb_str` of HBM3, model weights for a 70-billion-parameter model in 8-bit precision consume `{python} ContextHeadroom.weights_gb_str`, leaving `{python} ContextHeadroom.headroom_gb_str` for KV cache and scratchpad activations. How long an unmanaged agent survives inside that residual budget depends entirely on what a single token of KV cache costs under the model's attention scheme.
