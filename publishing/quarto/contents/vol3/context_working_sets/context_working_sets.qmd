...
Managing the working set inside an active context window addresses only half the memory equation. In @sec-vol3-prefix-caching-paging, we look at how the underlying serving runtime manages KV cache memory across requests and branches, examining Radix prefix trees, page-table virtualization, and cross-session reuse.
