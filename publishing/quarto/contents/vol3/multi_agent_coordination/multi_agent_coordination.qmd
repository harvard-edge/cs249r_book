...
Agentic systems introduce a fundamentally new computing primitive: the **Stochastic Compute Node**. An agent is an autonomous software entity driven by an autoregressive neural network whose outputs are probabilistic draws conditioned on contextual memory. Coordinating multiple stochastic nodes demands that systems discard conversational assumptions in favor of formal actor semantics, bounded communication graphs, and deterministic verification.
...
- **Attention KV Cache Eviction:** Shared context thrashing destroys GPU prefix caching (detailed in @sec-vol3-prefix-caching-paging). Because each agent appends non-deterministic turns in interleaved order, the common prefix is perpetually invalidated across the inference cluster.
...
```json
{
  ...
  "vector_clock": {"worker_ast_1": 14, "supervisor": 42},
...
In asynchronous multi-agent networks, relying on physical wall-clock time to order events introduces fatal race conditions due to physical clock skew. To reconstruct the true sequence of agent interactions—which is essential for state rollback, trace debugging, and distributed cycle detection—the orchestrator must implement **Logical Clocks** (such as Lamport Timestamps or Vector Clocks) [@lamport1978time].

Every message passed between mailboxes carries a logical clock vector (as seen in the `vector_clock` field above). When Agent B processes a message from Agent A and emits a subsequent tool call, it increments its local logical clock. This guarantees that if Agent A's output causally influenced Agent B's decision, the runtime can deterministically reconstruct the causal dependency chain across the cluster, completely independent of network delays.

### Queue management and backpressure

Because multi-agent topologies execute asynchronously, a fast producer (e.g., a root supervisor rapidly iterating over a codebase) can easily overwhelm a slower consumer (e.g., a static analysis worker constrained by GPU inference latency). Without intervention, the slow worker's mailbox queue grows unbounded, leading to catastrophic Out-Of-Memory (OOM) crashes and immense queueing delays.

Robust systems engineering mandates explicit **Backpressure** mechanisms. When a worker's mailbox queue depth exceeds a predefined high-water mark, the distributed runtime must propagate backpressure upstream—either blocking the supervisor's execution thread, shedding load, or dynamically auto-scaling the worker pool—ensuring the system remains stable under bursty workload conditions.

## Multi-Agent Topology Taxonomy {#sec-vol3-multi-agent-topology-taxonomy-hierarchical-peer-to-peer}

The communication graph connecting multi-agent nodes governs the runtime's scalability, failure blast radius, and verification guarantees. @Fig-multi-agent-topologies sorts production coordination topologies into four archetypes.

::: {#fig-multi-agent-topologies fig-env="figure" fig-pos="htb" fig-cap="**Multi-Agent Coordination Topologies**: Architectural trade-offs across communication complexity, failure blast-radii, and isolation boundaries. The three properties are not independent choices, because the same edge that shortens a communication path also carries corrupted state, so isolation is what a designer trades to get either one." fig-alt="Diagram comparing four multi-agent topologies: Peer-to-Peer Swarm with O(M^2) quadratic complexity, Hierarchical Supervisor Tree with O(M) bounded complexity, Sequential Assembly Pipeline with linear phase gates, and Shared Blackboard with decoupled asynchronous tuple space."}
...
```{python}
#| label: vol3-coordination-amdahl-ceiling
#| echo: false
# +-----------------------------------------------------------------------------
# | THE CEILING A SEQUENTIAL FRACTION IMPOSES
# +-----------------------------------------------------------------------------
# | Context: The Amdahl paragraph immediately below.
# |
# | Goal:    Show what a fixed sequential fraction costs a fan-out of subagents,
# |          and that the ceiling does not move with the agent count.
# | Show:    Speedup at a modest and an extreme fan-out, and the asymptote.
# | How:     calc_amdahls_speedup with the parallel fraction 1 - s. The ceiling
# |          is 1/s, which the guard checks the large fan-out approaches.
# +-----------------------------------------------------------------------------
from mlsysim import *

class CoordinationCeiling:
    # +-- 1. LOAD (Constants) --------------------
    sequential = 0.15  # scenario assumption: fraction that cannot be parallelized
    small_fanout = 10  # scenario assumption: a modest subagent fleet
    large_fanout = 10_000  # scenario assumption: a fan-out far past any practical limit

    # +-- 2. EXECUTE (The Compute) ---------------
    parallel = 1 - sequential
    speedup_small = calc_amdahls_speedup(parallel, small_fanout)
    speedup_large = calc_amdahls_speedup(parallel, large_fanout)
    ceiling = 1 / sequential

    # +-- 3. GUARD (Invariants) ------------------
    check(speedup_small < speedup_large < ceiling, "Speedup must rise with fan-out and stay under the ceiling")
    check(ceiling - speedup_large < 0.01, f"A fan-out of {large_fanout} should sit within a hundredth of the ceiling, got {ceiling - speedup_large}")
    check(abs(ceiling - 1 / sequential) < 1e-12, "The ceiling is the reciprocal of the sequential fraction and nothing else")

    # +-- 4. OUTPUT (Formatting) -----------------
    sequential_str = fmt(sequential, precision=2, commas=False)
    sequential_pct_str = fmt_percent(sequential, precision=0, style="prose")
    small_fanout_str = fmt_int(small_fanout)
    large_fanout_str = fmt_int(large_fanout)
    ceiling_mult_str = fmt_multiple(ceiling, precision=2)
```

As $M \to \infty$, the speedup is strictly bounded by $1/s$. At a sequential fraction $s$ of `{python} CoordinationCeiling.sequential_str`, meaning `{python} CoordinationCeiling.sequential_pct_str` of the task cannot be parallelized, the maximum theoretical speedup is `{python} CoordinationCeiling.ceiling_mult_str`, regardless of whether `{python} CoordinationCeiling.small_fanout_str` or `{python} CoordinationCeiling.large_fanout_str` agents are deployed.
...
- **Optimal Scaling Point ($M^*$):** Speedup reaches its mathematical global maximum:
  ...
  Solving that stationary condition for a representative parameter set is one line of arithmetic, and it is worth doing rather than quoting, because the answer is smaller than most teams expect.

::: {#nbk-multi-agent-optimal-fleet .callout-notebook title="How many agents before coordination eats the gain"}

**Problem**: Under the coordination-tax model above, at what fleet size does adding another agent start making the task slower?

**Variables**: $s = 0.12$ serial fraction, $\alpha = 0.003$ quadratic coordination coefficient, $\beta = 0.02$ linear coordination coefficient. All three are scenario assumptions, not measured constants.

**Math**: Setting the derivative to zero and clearing $M^2$ turns the stationary condition into a cubic:

$$2\alpha M^3 + \beta M^2 - (1-s) = 0 \;\Longrightarrow\; 0.006\,M^3 + 0.02\,M^2 - 0.88 = 0$$

Bracketing it by hand: at $M = 4$ the left side is $0.384 + 0.32 - 0.88 = -0.176$, and at $M = 5$ it is $0.75 + 0.5 - 0.88 = +0.37$. The root sits between them, at $M^* \approx 4.4$.

**Result**: The integer optimum is **four agents**, where total time is $T(4) = 0.468$ against $T(1) = 1.023$, a speedup of $2.19\times$. Five agents give $2.17\times$, so the curve is already flat at its peak.

**Systems insight**: The ceiling is low and it is set by the coordination coefficients, not by the model. Doubling the fleet from four to eight does not halve anything; it drops the speedup to $1.76\times$ while roughly doubling token spend. The cubic also shows why the ceiling barely moves: $M^*$ scales as $\alpha^{-1/3}$, so cutting coordination overhead by an order of magnitude buys about a doubling of useful fleet size.

:::

- **Negative Return Collapse Zone ($M > M^*$):** Beyond $M^*$, the $O(M^2)$ Coordination Tax dominates the parallel savings $(1-s)/M$. Communication latency, token serialization delays, and lock contention compound. Under the parameters above the speedup is still $1.76\times$ at $M = 8$ and does not fall below $1.0\times$ until roughly $M = 14$, at which point the multi-agent ensemble takes **longer** to complete the task than a single focused agent working alone, while consuming orders of magnitude more financial compute budget.

## Concurrency Control and State Races {#sec-vol3-multi-agent-concurrency-control-and-shared}

When multiple subagents execute concurrently in a software engineering or cloud operations environment, they inevitably compete for shared physical resources:
...
Production runtimes enforce **Time-Bounded Leases** [@gray1989leases] (detailed in @sec-vol3-descriptors-capability-leases-and-security). A lease is an exclusive lock granted to an agent for a strictly bounded duration (e.g., 30 seconds):
...
Multi-agent security is governed by the classical **Principle of Least Privilege** [@saltzer1975] enforced through **Monotonic Capability Attenuation** [@miller2006robust] (detailed in @sec-vol3-sandboxing-isolation).
...
When a high-level user request triggers a cascade of subagent interactions across multiple machines, diagnosing latency bottlenecks requires standard distributed observability (a discipline we formalize in @sec-vol3-telemetry-evaluation). Production agent runtimes inject W3C Trace Contexts (Trace IDs and Span IDs) into the metadata headers of every RPC message. This enables traditional systems monitoring tools (e.g., Jaeger, OpenTelemetry) to visualize the multi-agent execution graph, allowing systems engineers to track token consumption, tool execution latencies, and causal wait states across the entire distributed cluster.
...
The system checkpoints every agent's input/output state in an immutable Write-Ahead Log. During post-incident analysis, the runtime performs counterfactual replay (a time-travel debugging technique we explore in @sec-vol3-telemetry-evaluation): substituting Agent $i$'s decision with a corrected reference input while holding all other downstream agent decisions constant. If the downstream pipeline transitions from failure to success, causal blame is mathematically assigned to Agent $i$.
...
- **`TOOL_EXEC`:** The model emits a tool call. The execution thread enters an asynchronous wait state inside a containerized sandbox (detailed in @sec-vol3-sandboxing-isolation).
...
To survive host crashes and network partitioning [@zaharia2012resilient], multi-agent runtimes serialize the global stategraph to persistent storage at every turn boundary (detailed in @sec-vol3-recovery-trajectory-checkpointing-persistent-stategraphs). State checkpoints record:
...
### Tool idempotency and delivery semantics

In any distributed architecture, the underlying network is inherently unreliable. The platform can reliably guarantee only "at-most-once" or "at-least-once" message delivery; true "exactly-once" delivery over an unreliable network is a well-known impossibility.

When a foundation model API drops a connection or a worker micro-VM reboots, the supervisor node must retry the instruction. Consequently, all state-mutating tool calls executed by agents must be strictly **idempotent**. If an agent calls `execute_sql_migration()` or `provision_cloud_vm()` multiple times due to a network timeout, the systems infrastructure must yield the exact same physical state as if the command were executed exactly once. Production runtimes enforce this by automatically injecting cryptographic idempotency keys into every agent tool invocation, dropping duplicate execution requests at the gateway.
...
@Fig-supervisor-tree-isolation shows the Erlang-style supervisor trees and isolation firewalls production runtimes deploy to eliminate cascading collapse.
...
Circuit breakers and jitter defend a fleet against a failing dependency outside it. Neither defends a fleet whose dependency is itself, which is the failure mode that the peer-to-peer topology in @fig-multi-agent-topologies makes structurally available. A production incident at cloud scale shows how a peer mesh consumes an invisible resource and then fails all at once (\ref{ws-multi-agent-kinesis-thread-exhaustion-2020}).

::: {#ws-multi-agent-kinesis-thread-exhaustion-2020 .callout-war-story title="Amazon Kinesis front-end thread exhaustion (2020)"}

**Context**: Beginning at 2:44 AM PST on November 25, 2020, and finishing at 3:47 AM PST, Amazon Web Services added a relatively small amount of capacity to the front-end fleet of Amazon Kinesis in the US-EAST-1 region [@aws2020kinesis]. The front end handles authentication, throttling, and routing of requests to the correct stream shards on the back-end clusters. It is a peer group rather than a supervisor tree. Each server maintains a shard-map cache built in part from continuous processing of messages from every other front-end server. Nothing in this path was a learned system, which is precisely why the coordination mechanism transfers.

**Mechanism**: Each front-end server creates operating system threads for each of the other servers in the fleet, so the thread count a server must maintain is directly proportional to fleet size, and the fleet-wide total is therefore quadratic in it. Existing members take up to an hour to learn of new participants, so the added capacity raised every server's thread count gradually rather than at a single moment. When the total crossed a maximum imposed by an operating system configuration, cache construction stopped completing, and front-end servers ended up holding shard-maps that could not route requests to any back-end cluster. Every peer failed in the same way at nearly the same time, because every peer was coupled to every other peer through the same growing resource, and nothing was alarming on that resource.

**Impact**: Alarms for errors on putting and getting Kinesis records began at 5:15 AM PST. Root cause was not confirmed until 9:39 AM PST, because errors appeared in every call the fleet was making and side effects were difficult to separate from the cause. Recovery required restarting a fleet of many thousands of servers at a rate of only a few hundred per hour, since a faster restart put shard-map construction in contention with request handling, and the resulting slow servers would be deemed unhealthy and removed from the fleet, setting the recovery back. Kinesis returned fully to normal at 10:23 PM PST. The blast radius reached well past Kinesis. Cognito web servers blocked on backlogged buffers, CloudWatch alarms moved to the INSUFFICIENT_DATA state, Lambda invocations hit memory contention from locally buffered metric data, and EventBridge delays propagated into ECS and EKS cluster provisioning. The tool used to post to the public Service Health Dashboard depended on Cognito, so the channel for reporting the incident was inside the incident.

**Fix**: AWS moved to larger CPU and memory servers so that fewer servers were needed and each maintained fewer threads, added fine-grained alarming for thread consumption, tested an increase to the operating system thread limit, moved the front-end cache to a dedicated fleet, partitioned large consumers such as CloudWatch onto a separate front-end fleet, and accelerated cellularization of the front end so that its components operate within a previously tested and operated range.

**Systems lesson**: A peer topology hides its coordination cost in a resource nobody budgeted. Adding one node did not add load to one node, it added load to every node, which is the operational shape of the quadratic term derived in @sec-vol3-multi-agent-the-coordination-tax-and rather than an abstraction over it. Three properties of this failure transfer directly to an agent runtime. The exhausted resource was a per-peer allocation rather than a request rate, so request-level circuit breakers had nothing to trip on. The health check that removed slow servers during recovery was a correct isolation mechanism operating on a false premise, and it extended the outage rather than containing it. And the fix that mattered was not a better retry policy but cellularization, which is the same argument as bounding the coordination graph: keep each group small enough that its peer cost stays inside a range that has actually been tested.
...
Principle \ref{pri-vol3-stochastic-actors} is stress-tested here on the assumption that usually rescues redundancy. Replication tolerates faults when failures are independent; subagents sharing a pretraining checkpoint fail together, so adding them multiplies the coordination cost without buying the reliability that multiplication is supposed to pay for.
...
::: {.callout-takeaways title="Hierarchical delegation and IPC topologies"}

- **Stochastic Node Foundations:** Multi-agent collaboration is distributed computing over non-deterministic, fail-plausible nodes. Systems must abandon anthropomorphic conversational metaphors in favor of the formal Actor model: private state, FIFO mailboxes, and explicit message serialization.
- **Zero Shared Context & Causality:** Shared conversation buffers trigger $\mathcal{O}(M \cdot N^2)$ token explosion. Private mailboxes enforce context isolation, while logical vector clocks and explicit backpressure bound queue depths and prevent out-of-memory crashes.
- **Topological Invariants:** Peer-to-peer swarms suffer from $\mathcal{O}(M^2)$ quadratic message overhead and cyclic deadlocks. Production systems rely on Hierarchical Supervisor Trees ($\mathcal{O}(M)$ bounded fanout) and Sequential Assembly Pipelines with explicit verification gates.
...
- **Concurrency Control & Idempotency:** Concurrent mutations mandate Git worktree isolation, distributed TTL leases, and optimistic concurrency control (OCC). Furthermore, because network failures force automatic retries, all agent tool calls must be strictly idempotent.
- **Cycle Detection & Tracing:** Semantic deadlocks and conversational livelocks are mitigated by maintaining causal message DAGs and enforcing $\epsilon$-progress tripwires. Distributed tracing (W3C trace contexts) exposes cross-agent latencies and causal dependencies.
...
Multi-agent systems demonstrate that scaling agency is fundamentally a problem of distributed systems coordination. By enforcing strict message passing, bounded supervisor fanouts, and deterministic concurrency controls, engineering teams can deploy collaborating groups of agents while avoiding the quadratic context explosions and livelocks that plague unconstrained swarms.

::: {.callout-chapter-connection title="From agent coordination to production telemetry"}
Structuring distributed agent communication prevents message collapse, but evaluating whether a fleet of autonomous agents delivers reliable business value requires deep observability and standardized evaluation harnesses. @Sec-vol3-telemetry-evaluation builds the evaluation gyms, distributed telemetry span hierarchies, and Trajectory Goodput economics required to measure, benchmark, and govern autonomous agents in production.
:::
