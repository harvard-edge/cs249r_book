...
data = pd.read_csv("data/amdahls_law_multi_agent.csv")

plt.figure(figsize=(8, 5))
sns.set_theme(style="whitegrid")

plt.plot(data["Agents"], data["Classical_Speedup"], label="Classical Amdahl's Law", linestyle="--", color="gray")
plt.plot(data["Agents"], data["Multi_Agent_Speedup"], label="Multi-Agent Amdahl's Law (with Coordination Tax)", color="firebrick", linewidth=2.5)

plt.axhline(1.0, color="black", linestyle=":", label="No Speedup (1x)")
plt.axvline(4.4, color="black", linestyle="-.", alpha=0.5, label="Optimal Fleet Size ($M^* \\approx 4.4$)")

plt.title("Scaling of Multi-Agent Speedup with Coordination Tax", fontsize=14)
plt.xlabel("Number of Agents ($M$)", fontsize=12)
...
- **Zero Shared Context & Causality:** Shared conversation buffers trigger $O(M \cdot N^2)$ token explosion. Private mailboxes enforce mathematical context isolation. The system reconstructs message causality and limits OOM crashes using logical vector clocks and explicit backpressure.
...
- **Cycle Detection & Tracing:** Semantic deadlocks and conversational livelocks are mitigated by maintaining causal message DAGs and enforcing $\epsilon$-progress tripwires. Distributed tracing (W3C trace contexts) allows engineers to observe latencies across the swarm.
