...
For example, to reliably detect a modest $5\%$ capability delta (e.g. improving from 30 to 35 percent success rate) with $80\%$ power, the evaluation suite must evaluate at least **1,380 independent tasks**. Evaluating on only 50 or 100 tasks yields statistical power below $15\%$. Under such low power, observed improvements are indistinguishable from a coin toss, and production deployments will routinely ship severe capability regressions masked by evaluation noise.
