...
In production fleets, reliability demands **Deterministic Trace Reconstructibility**: capturing all non-deterministic interactions across the environment boundary into immutable event-sourced streams. Furthermore, operating at scale mandates structured distributed tracing that conforms to the OpenTelemetry GenAI Semantic Conventions, binding multi-model inferences, token consumption, tool execution latencies, and monetary costs into unified causal Directed Acyclic Graphs (DAGs). At the evaluation layer, static question-answering datasets (Massive Multitask Language Understanding (MMLU), GSM8K) have collapsed under benchmark saturation, test-set data contamination, and the total absence of interactive feedback. Agentic systems must be benchmarked inside hermetic, reproducible execution gyms (SWE-bench, WebArena) and evaluated across stochastic distributions using unbiased Pass@$k$ estimators, Trajectory Goodput ($G$), and sequential statistical release gates.
...
**Invariant**: The governing metric of autonomous serving is Trajectory Goodput ($G$), defined as verified successful outcomes completed per dollar per GPU-hour. Maximizing token throughput (tokens/second) is irrelevant if those tokens are expended on hallucinated loops or unverified search branches.

**Implication**: Systems architectures must optimize the Trajectory Goodput Pareto frontier, balancing sparse Mixture-of-Experts (MoE) routing, prefix caching, and verifier-guided search to minimize the Cost-per-Goal.

:::

We formally define **Trajectory Goodput** ($G$) in @eq-vol3-trajectory-goodput as:

$$G = \frac{N_{\text{success}}}{C_{\text{tokens}} + C_{\text{compute}}}$$ {#eq-vol3-trajectory-goodput}

where $N_{\text{success}}$ is the number of verified goal completions, $C_{\text{tokens}}$ is the direct cost of input/output token generation across all foundation model calls, and $C_{\text{compute}}$ is the ancillary infrastructure cost (tool execution, container sandboxes, vector database lookups, and network egress).

Associated with Goodput is **Cost-per-Goal** ($C_{\text{goal}}$), formulated in @eq-vol3-cost-per-goal, which measures the average operational cost required to achieve a single verified successful outcome:

$$C_{\text{goal}} = \frac{\sum_{i=1}^M \text{Cost}(\tau_i)}{N_{\text{success}}} = \frac{\overline{\text{Cost}}(\tau)}{\text{Pass}@1}$$ {#eq-vol3-cost-per-goal}

where $M$ is the total number of attempted trajectories and $\overline{\text{Cost}}(\tau)$ is the mean cost per attempt. Notice the fundamental non-linearity: if an agent variant doubles single-step accuracy such that $\text{Pass}@1$ increases from $10\%$ to $40\%$ while its average execution cost only increases by $50\%$, its Cost-per-Goal drops by more than half:
...
To determine whether an observed improvement $\Delta p = p_2 - p_1$ is statistically significant, systems engineers must formulate a formal two-proportion hypothesis test. The null hypothesis $H_0$ states that the two agent variants have identical underlying success rates ($p_1 = p_2$), while the alternative hypothesis $H_1$ asserts that variant 2 represents a genuine improvement.

As established by @card2020little, the required sample size $N$ (number of evaluated benchmark tasks) to detect an effect size $\Delta p$ with statistical significance $\alpha$ (Type I error rate, typically $0.05$) and statistical power $1 - \beta$ (where $\beta$ is Type II error rate, typically $0.20$ for $80\%$ power) is governed by:

$$N \ge \frac{\left( z_{1 - \alpha/2} \sqrt{2 \bar{p}(1 - \bar{p})} + z_{1 - \beta} \sqrt{p_1(1 - p_1) + p_2(1 - p_2)} \right)^2}{(p_2 - p_1)^2}$$ {#eq-vol3-sample-size}

where $\bar{p} = (p_1 + p_2) / 2$, $z_{1 - \alpha/2}$ is the standard normal critical value ($1.96$ for $\alpha = 0.05$), and $z_{1 - \beta}$ is the power critical value ($0.84$ for $80\%$ power).

Consider a realistic engineering scenario: an agent has a baseline success rate of $p_1 = 0.30$ on a coding benchmark, and an engineer hopes to detect an improvement to $p_2 = 0.35$ (a $+5\%$ absolute gain). Substituting these values into @eq-vol3-sample-size:

$$\bar{p} = 0.325, \quad \Delta p = 0.05$$
$$N \ge \frac{\left( 1.96 \sqrt{2(0.325)(0.675)} + 0.84 \sqrt{0.30(0.70) + 0.35(0.65)} \right)^2}{(0.05)^2} \approx 1,\!380$$

To reliably detect a $5\%$ capability delta with $80\%$ power, the evaluation suite must evaluate at least **1,380 independent tasks**. Evaluating on only $N=50$ or $N=100$ tasks yields statistical power below $15\%$. Under such low power, observed improvements are indistinguishable from a coin toss, and production deployments will routinely ship severe capability regressions masked by evaluation noise.
...
Given an empirical dataset of $N$ evaluated trajectories $\mathcal{D} = \{ \tau_1, \tau_2, \dots, \tau_N \}$:

1. Draw $B$ bootstrap resamples $\mathcal{D}_1^*, \mathcal{D}_2^*, \dots, \mathcal{D}_B^*$ of size $N$ from $\mathcal{D}$ with replacement (typically $B = 10,\!000$).
2. For each resample $\mathcal{D}_b^*$, compute the metric of interest $\hat{\theta}_b^* = f(\mathcal{D}_b^*)$ (such as Pass@$k$ or Trajectory Goodput $G$).
3. Sort the resulting bootstrap distribution $\{ \hat{\theta}_1^*, \hat{\theta}_2^*, \dots, \hat{\theta}_B^* \}$.
4. The two-sided $95\%$ bootstrap percentile confidence interval $[\theta_{\text{lower}}, \theta_{\text{upper}}]$ is computed via @eq-vol3-bootstrap-ci directly from the empirical $2.5\text{th}$ and $97.5\text{th}$ percentiles:

$$[\theta_{\text{lower}}, \theta_{\text{upper}}] = \left[ \hat{\theta}^*_{\lfloor 0.025 B \rfloor}, \; \hat{\theta}^*_{\lfloor 0.975 B \rfloor} \right]$$ {#eq-vol3-bootstrap-ci}
...
1. **Branch-Scoped Contexts:** Each search branch must instantiate an independent child context cloned from the parent, inheriting the global `trace_id` but assigning a unique `span_id` as the branch parent.
2. **Baggage Isolation:** State mutations within a branch's `baggage` (such as local tool mutation counters) must be strictly isolated to prevent pollution of sibling branches.
3. **Causal Linkage:** When the search algorithm completes and selects the winning branch $\tau^*$, the runtime creates a synthetic `search.consensus` span that records OpenTelemetry `SpanLink` pointers to all explored branches, preserving the full counterfactual search history for offline analysis.
...
**Invariant**: Robust production operations require capturing all non-deterministic interactions across the environment boundary into immutable event streams to enable bit-exact offline replay and counterfactual diffing.

**Implication**: Runtimes must implement deterministic event-sourcing, intercepting and recording model completions, tool outputs, pseudorandom seeds, and environmental clocks.
...
1. **Model Completions ($y_{\text{model}}$):** Every token, text chunk, logit bias, and finish reason returned by the model API is serialized into the append-only event log.
2. **Tool Responses ($o_{\text{tool}}, c_{\text{exit}}$):** All standard output, standard error, exit codes, and returned files from shell commands or APIs are captured verbatim.
3. **Pseudorandom Seeds ($s_{\text{seed}}$):** The initial PRNG seed governing local sampling is logged.
4. **Environment Clocks ($t_{\text{wall}}$):** All calls to `time.now()` or filesystem timestamp queries are intercepted and recorded.
...
1. **Bidirectional Stepping:** The debugger allows an engineer to step forward and backward through the execution history:
   ```bash
   ...
   ```
2. **State Rewind Mechanics:** To rewind to step $t$, the debugger:
...
1. **Alignment:** Traces are aligned along normalized execution milestones (e.g. file discovery, test execution, patch generation) using dynamic time warping.
2. **Divergence Detection:** The runtime identifies the critical divergence step $t^*$ where the state representations or action distributions between successful and failed runs first diverge beyond threshold $\epsilon$:
...
3. **Attribution Categorization:** Once $t^*$ is isolated, the fault localization engine categorizes the failure into one of three structural root causes:
...
1. **Offline Synthetic Regression Gate:** The candidate agent must achieve a statistically significant non-inferiority score on a curated benchmark suite ($N = 500$ tasks) evaluated using bootstrap confidence intervals.
2. **Shadow Traffic Mirroring:** Live production requests are mirrored to the candidate agent in real time. The candidate agent executes all planning and model inference, but its tool actuation is sandboxed in a shadow read-only environment to measure behavioral divergence without impacting production databases.
3. **Canary Rollout with SPRT:** The candidate agent is allocated $5\%$ of live production traffic. Waiting for a fixed sample size of $N=2,\!000$ users before making a promotion or rollback decision introduces unacceptable risk if the new version contains a critical flaw. Runtimes implement Abraham Wald's **Sequential Probability Ratio Test (SPRT)** (@wald1945sequential).
...
If a deployed candidate version contains a severe bug (e.g. crashing on $40\%$ of trajectories), SPRT detects the anomaly within 15 to 20 trajectories, triggering an automated instantaneous rollback before widespread customer disruption occurs.

## Supervisory Observability and Human Oversight Telemetry {#sec-vol3-telemetry-supervisory-observability-and-human}

As established in @sec-vol3-verification-recovery, high-stakes enterprise agents operate under supervisory authority structures, requiring human approval before executing irreversible actions (financial transactions, production infrastructure deployments, external email communications). Observability runtimes must monitor the human supervisory interface itself (@parasuraman2000model; @cummings2004automation).
...
Deploying autonomous agent fleets at scale exposes subtle operational pitfalls at the intersection of non-deterministic inference, systems isolation, and telemetry infrastructure. The following misconceptions frequently derail production reliability, benchmark integrity, and resource allocation.

### Fallacy 1: Evaluating an agent on a single run (Pass@1) provides a reliable capability metric.
...
### Fallacy 2: Passing unit tests in a sandbox environment guarantees real-world task success.
...
### Fallacy 3: Logging raw text strings into Elasticsearch is sufficient for production observability.
...
### Pitfall 1: Benchmark Contamination via Unrestricted Internet Access.
...
### Pitfall 2: The Tool-Wait Profiling Trap.
...
### Pitfall 3: Shipping regressions masked by statistical power failure.
