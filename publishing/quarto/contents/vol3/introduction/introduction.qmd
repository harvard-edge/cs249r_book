...
- Navigate the five-part architectural organization of Volume III and select tailored reading paths across systems infrastructure, policy modeling, and enterprise platforms.

:::

## The Agentic Systems Moment: The Post-Request Paradigm {#sec-vol3-intro-the-post-request-paradigm}

This textbook marks a fundamental inflection point across the four-volume arc of *Machine Learning Systems*. In **Volume I: The Model**, we explored the single-node regime: how machine learning algorithms, compiler frameworks, and hardware silicon co-design to train and execute models on a single workstation or accelerator, bound by the Roofline model, arithmetic intensity, memory bandwidth, and the Data-Algorithm-Machine (D·A·M) framework. In **Volume II: The Fleet**, we expanded that horizon to the datacenter scale: how tens of thousands of accelerators coordinate across high-speed interconnects (NVLink, InfiniBand) to train trillion-parameter models and serve millions of concurrent queries, governed by 3D parallelism, collective communication, and the $C^3$ taxonomy (Compute, Communication, Coordination).

**Volume III: Agentic Machine Learning Systems** confronts the next frontier: *giving these models agency*. A model at rest behind an API endpoint is merely a passive statistical calculator; an agent in motion is an active, closed-loop controller. In this volume, computation moves from stateless, single-turn inference to stateful, multi-turn trajectories where models formulate plans, dispatch tool invocations, inspect operating system environments, diagnose failures, and drive real-world mutations over extended horizons. This volume establishes the systems principles governing this transition, preparing the architectural foundation for **Volume IV: Physical AI**, where autonomous control loops extend into embodied robotics and real-time cyber-physical systems.
...
* **Systems Physics (Silicon and Hardware Execution Limits):** The immutable physical constraints of hardware execution. Foundation models do not execute in an abstract computational vacuum; they are bound by the physics of silicon: High Bandwidth Memory (HBM) capacity, memory bus streaming bandwidth, arithmetic intensity (FLOP/byte), the quadratic attention memory tax ($O(T^2)$), and interconnect latencies across PCIe Gen5 and CXL fabrics. Systems physics dictates what physical hardware permits, regardless of software ambition.
...
- **Verification & Test Oracles:** Evaluating intermediate post-conditions after every actuation step.
- **Test-Time Search:** Exploring alternative solution branches via Monte Carlo Tree Search (MCTS) or beam search [@yao2023tree].
- **Rollback & Recovery:** Detecting divergence early and reverting state mutations to clean checkpoints before errors compound into fatal failures.
...
## Book Organization: The Architecture of Volume III {#sec-vol3-intro-book-organization}

The core concepts established in this introductory chapter—the Post-Request Paradigm, the Invariant Closure Principle, the Two Inescapable Physical Walls of Agency, and the $H\text{--}S\text{--}A\text{--}C$ taxonomy—form the foundational spine for the fifteen chapters of this volume.

The architecture of Volume III is structured around a strict dependency progression: *from the fundamental execution abstractions of an individual agent to cluster-scale multi-agent coordination and operational assurance*. Just as Volume I progressed from single-node silicon limits to model optimization and deployment, and Volume II progressed from physical datacenter fabrics to distributed training and serving fleets, Volume III follows a five-part pedagogical progression designed for both university curricula and production systems engineering (@tbl-vol3-book-organization).
...
* **The Architecture of Volume III:** Structured across five parts: Foundations of Agency (Chapters 1–3), Policy & Reasoning Systems (Chapters 4–6), Serving & Memory Systems (Chapters 7–9), Security, Isolation & Verification (Chapters 10–12), and Fleet Orchestration & Production Assurance (Chapters 13–15).
