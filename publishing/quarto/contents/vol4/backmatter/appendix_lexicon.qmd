...
Physical AI sits at the intersection of four engineering traditions that developed their vocabularies in isolation from one another: modern deep learning, classical robotics and nonlinear control, real-time embedded systems and silicon architecture, and safety-critical systems assurance. Engineers drawn from different backgrounds consequently use one word for incompatible concepts and several words for a single underlying physical quantity. This appendix is the translation table between those vocabularies, so a reader can identify which tradition a term came from, restate it in the language of the discipline they are addressing, and catch the false friends that quietly survive a design review.

## How to Use This Appendix {.unnumbered}

This appendix is consulted at the moment a term crosses a disciplinary boundary rather than read from front to back.

- When a specification, ticket, or review comment reads clearly to one team and not to another, the four-tribe table gives the same physical concept in all four vocabularies.
- Words that appear to agree are the dangerous case, and the false-friends section separates the meanings that deterministic, real-time, state, latency, safe, robustness, and policy carry in each field.
- An engineer entering Physical AI from one of the four traditions can start from the translation guide written for that background, which names what the other three will assume is already known.
- The archetype quick-reference at the end places a system in its class, which settles which vocabulary will dominate its documentation and its review.
