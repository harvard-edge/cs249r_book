...
::: {#dfn-physical-ai-system .callout-definition title="Physical AI System"}
***Physical AI system***\index{Physical AI system!definition} is an integrated machine architecture in which high-capacity stochastic neural deliberation (the "brain") interacts with and controls moving mass across continuous Newtonian dynamics (the "body"), strictly mediated by a deterministic hardware enforcement layer (the "nervous system").
:::
...
<!-- covers: Reference the cover organ locator image (@fig-04-locator) to anchor the 4-tier stack. -->
<!-- covers: The reconciliation problem: bridging a deterministic physical plant that obeys mechanics (@sec-body) with an unbounded, stochastic model that generates proposals (@sec-brain). -->
...
<!-- BEGIN -->
Physical machines present a fundamental temporal conflict that no digital software system ever confronts: *the cognitive brain needs time to deliberate, but the physical body cannot wait to survive.*

In @sec-boundary, we established that physical AI begins the instant digital computation crosses the causal boundary to command moving mass. In @sec-body, we quantified the unyielding physical budgets that govern that mass: reflected rotor inertia ($N^2 J$), stopping distance dynamics ($d_{\text{stop}}$), feedback phase lag ($\Delta \phi$), and thermal dissipation limits ($I^2 R$). Mass will not wait for computation. Yet high-capacity neural foundation models require tens to hundreds of milliseconds to ingest multimodal tokens and synthesize future plans—outputs that are inherently stochastic, non-Lipschitz, and subject to variable execution latencies. As highlighted in the systems organ locator (@fig-04-locator), the **real-time nervous system** occupies the intermediate architectural tier (Layer 2) designed to resolve this contradiction: a deterministic hardware and communication bridge binding the deliberative cognitive brain (Layer 3) to the dynamical physical body (Layer 1).
...
<!-- covers: @tbl-04-cadence-hierarchy: Multi-Rate Cadence Hierarchy. -->
...
<!-- covers: @tbl-04-fieldbus-comparison: Fieldbus Comparison (SPI, CAN-FD, EtherCAT, TSN). -->
...
To see how unanalyzed interference breaks an empirical guarantee, consider an analytical example of a robotic torque computation running at $1000\text{ Hz}$ with a hard deadline $D = 1000\,\mu\text{s}$. On an isolated $2.0\text{ GHz}$ application processor with empty bus queues and hot caches, the workload exhibits an unloaded baseline execution time of $C = 120\,\mu\text{s}$. Across millions of undisturbed bench cycles, the measured tail latency never exceeds $P_{99.99} = 150\,\mu\text{s}$, leaving an apparent safety margin of $850\,\mu\text{s}$.

When the same binary runs on an integrated system-on-chip amidst normal machine activity, the operating environment introduces contention across shared silicon resources. High-resolution camera streams transfer image frames via DMA at $1.2\text{ GB/s}$, an onboard neural network accelerator reads weight matrices in large bursts from shared memory, and ambient thermal buildup causes the dynamic voltage and frequency scaling (DVFS) governor to drop core clock frequency from $2.0\text{ GHz}$ to $1.2\text{ GHz}$. The core slowdown expands the base compute time by a factor of $2.0 / 1.2 = 1.67$, raising execution to $C = 200\,\mu\text{s}$. Concurrent bus traffic invalidates cache lines and stalls load instructions at the memory crossbar, introducing $I_{\text{bus}} = 750\,\mu\text{s}$ of memory interference, while unmasked device driver interrupts and lock contention add $B = 180\,\mu\text{s}$ of blocking. Summing these terms yields a total response time of:

$$R = 200\,\mu\text{s} + 180\,\mu\text{s} + 750\,\mu\text{s} = 1130\,\mu\text{s}$$

The task crosses the $1000\,\mu\text{s}$ deadline by $130\,\mu\text{s}$, not because the algorithmic complexity grew, but because shared silicon resources lacked isolation.
...
::: {#ws-nervous-1997-mars-pathfinder-priority-inversion .callout-war-story title="The 1997 Mars Pathfinder Priority Inversion"}
...
- Data transmission time per frame: $t_{\text{data}} = \frac{550\text{ bits}}{5\times 10^6\text{ bps}} = 110\,\mu\text{s}$.
- Header/arbitration time: $t_{\text{arb}} = \frac{35\text{ bits}}{1\times 10^6\text{ bps}} = 35\,\mu\text{s}$.
- Total frame time per axis: $t_{\text{frame}} \approx 145\,\mu\text{s}$.

For 8 sequential axis transactions over a single shared bus:
$$t_{\text{total, CAN-FD}} = 8 \times 145\,\mu\text{s} = 1160\,\mu\text{s} = 1.16\text{ ms}$$
...
The duration of the watchdog heartbeat lease is not an arbitrary software tuning parameter, but a bound dictated by the physical dynamics of the uncontrolled plant. In the event of a silent brain stall, the actuators continue applying their last commanded torque until the enforcer detects the timeout and commands emergency braking. By enforcing a geometric boundary condition that limits the unbraked acceleration displacement plus the subsequent stopping distance to the remaining safe clearance margin, we can derive a strict upper bound on the watchdog timeout.[^fn-math-watchdog] Setting the lease any higher permits the physical plant to breach its dynamic safety envelope before the nervous system is permitted to intervene.
...
<!-- covers: @tbl-04-synthesis-grid: Master 5x5 Handoffs-by-Limits Synthesis Grid. -->
...
## Fallacies and Pitfalls {#sec-nervous-4-10}
...
*The dilemma.* The diagnostic write command overwrites the inverter current target register with an unvalidated test value of $35.0\text{ A}$, commanding forward torque that accelerates the vehicle to $1.8\text{ m/s}$. The primary safety enforcer, reading zero velocity on its command mailbox, detects no safety violation in the proposal stream and leaves its emergency brake relays open, moving the machine without kinematic gating or obstacle avoidance.

*Systems takeaway: Exclusive Physical Authority Invariant.* Actuation authority is the sum of every physical wire and register that can command power stages. Auxiliary diagnostic ports must be physically interlocked with hardware jumpers, blown fuses, or power disconnect relays during operational mode.
...
<!-- covers: Mapping Part II to the synthesis grid: @sec-data, @sec-training, and @sec-evaluation. -->
...
With these deterministic safety guarantees in place, @sec-brain examines the final pillar of the Part I anatomy: **the deliberative brain**. We turn from the millisecond reflex enforcers to the cognitive engines—investigating how vision-language-action (VLA) foundation models, multimodal encoders, and diffusion action decoders generate purposeful physical behavior across unstructured environments without violating the causal boundaries of the machine.
...
[^fn-math-inference-latency]: We can model inference latency $T_{\text{inf}}$ using a log-normal distribution, where the natural logarithm of latency follows $\mathcal{N}(\mu_{\ln}, \sigma_{\ln}^2)$. With a median runtime $m = 50\text{ ms}$ ($\mu_{\ln} \approx 3.912$) and dispersion $\sigma_{\ln} = 0.35$, the probability of exceeding a $T_{\text{deadline}} = 100\text{ ms}$ deadline is $P(T_{\text{inf}} > T_{\text{deadline}}) = 1 - \Phi\left(\frac{\ln T_{\text{deadline}} - \mu_{\ln}}{\sigma_{\ln}}\right)$. Evaluating the standard normal variable yields $z \approx 1.98$, giving an overrun probability of $\approx 2.38\%$.

[^fn-math-braking-dynamics]: For an articulated joint with effective rotor inertia $J = 0.050\text{ kg}\cdot\text{m}^2$ rotating at $\omega_0 = 10.0\text{ rad/s}$ ($E_k = 2.5\text{ J}$), and a maximum deceleration $\alpha_{\max} = 50.0\text{ rad/s}^2$, the braking time is $t_{\text{brake}} = \omega_0 / \alpha_{\max} = 200\text{ ms}$. The angular displacement during braking is $\Delta \theta = \omega_0^2 / (2 \alpha_{\max}) = 1.00\text{ rad}$. The average thermal dissipation rate is $P_{\text{avg}} = 2.5\text{ J} / 0.20\text{ s} = 12.5\text{ W}$.

[^fn-math-watchdog]: Consider a platform traveling at $v_0 = 1.00\text{ m/s}$ with safe clearance $d_{\text{margin}} = 0.25\text{ m}$. Assume worst-case uncommanded acceleration $a_{\text{fault}} = 2.00\text{ m/s}^2$, emergency braking deceleration $a_{\text{brake}} = 5.00\text{ m/s}^2$, and actuation delay $t_{\text{act}} = 10\text{ ms}$. The total unbraked duration is $\tau = T_{\text{wd}} + t_{\text{act}}$. The displacement during acceleration is $d_1 = v_0 \tau + \frac{1}{2} a_{\text{fault}} \tau^2$, and the stopping distance is $d_{\text{brake}} = (v_0 + a_{\text{fault}} \tau)^2 / (2 a_{\text{brake}})$. Enforcing $d_1 + d_{\text{brake}} \le d_{\text{margin}}$ yields $1.40 \tau^2 + 1.40 \tau - 0.15 \le 0$. The positive root is $\tau \le 97.6\text{ ms}$. Subtracting the $10\text{ ms}$ actuation delay gives a watchdog timeout bound of $T_{\text{wd}} \le 87.6\text{ ms}$.
