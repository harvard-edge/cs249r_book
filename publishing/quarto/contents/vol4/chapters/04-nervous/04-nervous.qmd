...
The duration of the watchdog heartbeat lease is not an arbitrary software tuning parameter, but a bound dictated by the physical dynamics of the uncontrolled plant. In the event of a silent brain stall, the actuators continue applying their last commanded torque until the enforcer detects the timeout and commands emergency braking. By enforcing a geometric boundary condition that limits the unbraked acceleration displacement plus the subsequent stopping distance to the remaining safe clearance margin, we can derive a strict upper bound on the watchdog timeout.[^fn-watchdog-math] Setting the lease any higher permits the physical plant to breach its dynamic safety envelope before the nervous system is permitted to intervene.
