...
% Reader-ready PDFs are clean by default. From the repository root, use
%     ./book/binder build pdf --vol1 --print-marks
% (or --vol2, --vol3, --vol4) only when a printer requests camera/trim marks.
% Binder enables this switch temporarily and restores this file after the build.
\CropMarksfalse
...
% CROP / BLEED MARKS
% =============================================================================
% Camera/trim marks are managed by \CropMarks above. Keep the clean default and
% use `./book/binder build pdf --volN --print-marks` when production requests them.
% Do not load a second instance of the crop package here.
...
\renewcommand{\@makecaption}[2]{%
  ...
  \def\@figtest{figure}%
  \ifx\@captype\@figtest
    % Freeze the figure number at caption time, but defer the page number until
    % shipout. Deferring both lets later floats advance \c@figure before an
    % earlier page ships, which duplicates and skips numbers in the manifest.
    \begingroup
      \edef\figmanifestwrite{\endgroup
        \noexpand\write\figlatex{Figure \thefigure\space | Page \noexpand\thepage}}%
    \figmanifestwrite
  \fi
}
