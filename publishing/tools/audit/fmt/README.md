# Fmt / notation audit (`publishing/tools/audit/fmt`)
...
PYTHONPATH=mlsysim python3 publishing/tools/audit/fmt/fix_precision.py "$CH"

# 2. Fix assignments that still render as *.0 in prose preview
PYTHONPATH=mlsysim python3 publishing/tools/audit/fmt/fix_spurious_prose.py "$CH"
...
# or: ./binder check math --scope canonical --path "$CH"
...
./publishing/tools/audit/fmt/render_html.sh vol2   # extend chapter list in script
```

Archived output: `books/_build/html-audit/<vol>/<chapter>.html`

Legacy wrappers at `publishing/tools/audit/render_vol1_audit.sh` and
...
- [`publishing/docs/LEGO_CELLS.md`](../../../docs/LEGO_CELLS.md) — inline `{python}` cell placement and review contract
