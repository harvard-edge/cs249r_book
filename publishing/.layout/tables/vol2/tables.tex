...
% --- Fleet Stack Layer Helper (margin: title only, no subtitle) -----------------
% Used in chapter Purpose sections. Subtitle text reserved for About page.
% Part color + intensity gradient: fades by relevance (10=light tint, 100=focal).
% #1: y-position (bottom), #2: label, #3: intensity (0-100), #4: base color, #5: icon
\newcommand{\fleetstacklayernostub}[5]{
    \ifnum#3=0
        \def\fillcol{black!5}
        \def\textcol{black!30}
    \else
    ...
    % Text (same positioning as \stacklayer)
    % Font note: \rmfamily for Palatino body-font consistency (see \stacklayer).
    \node[anchor=west, text=\textcol, font=\rmfamily\scriptsize\bfseries]
        at ($(box.west)+(0.12,0)$) {#2};
    % Icon (same scale/position as \stacklayer)
    \path (box.east) +(-0.50,0)
        pic[scale=0.78, draw=\textcol, fill=\textcol, text=\textcol] {#5};
}


% --- Fleet Stack Layer Helper (full-size, with subtitle for About page) ----------
% #1: y-position, #2: label, #3: subtitle, #4: intensity (0-100),
% #5: base color, #6: icon
% Part color + intensity gradient: fades by relevance (matches margin version).
\newcommand{\fleetstacklayerfull}[6]{
    ...
    \node[anchor=west, text=\textcol,
          font=\rmfamily\fontsize{5}{6}\selectfont, opacity=0.9,
...
% 4-Part Fleet Stack for chapter margin figures (title only; subtitles in About)
...
% Node styling matches \stacklayer (Vol1) for consistency
\newcommand{\mlfleetstack}[4]{
    \begin{tikzpicture}
        ...
        % Font note: \rmfamily for Palatino body-font consistency (see \stacklayer).
        \node[font=\rmfamily\fontsize{5}{6}\selectfont\bfseries,
...
\newcommand{\mlfleetstackfull}[4]{
    \begin{tikzpicture}
        ...
        % --- Part labels (left accent) — ETH Zurich Blue ---
        % Font note: \rmfamily for Palatino body-font consistency (see \stacklayer).
        \node[font=\rmfamily\fontsize{6}{7}\selectfont\bfseries,
...
\texttt{MLSysBook\ VOL2\ Table\ Layout\ Audit}

\texttt{001\ tbl-distribution-triggers}

\texttt{contents/vol2/inference/inference.qmd:146}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.4653}}
...
\caption{\textbf{Triggers for Distributed Inference}: Each constraint
type indicates different distribution strategies. Memory constraints
require model sharding; throughput constraints require replication;
latency constraints may require either depending on whether the
bottleneck is compute or memory
bandwidth.}\label{tbl-distribution-triggers}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Constraint}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\textbf{Single-Machine Limit}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Memory} & 80 GB (H100) & GPT-4 (400 GB+) & Tensor/pipeline
parallelism \\
\textbf{Throughput}\index{Distributed Inference!throughput constraint} &
\textasciitilde10K QPS (vision) & 100K QPS RecSys & Horizontal
replication \\
\textbf{Latency}\index{Distributed Inference!latency constraint} & Model
execution time & 500 ms LLM TTFT & Model sharding \\
\end{longtable}

\clearpage

\texttt{002\ tbl-training-inference-inversion}

\texttt{contents/vol2/inference/inference.qmd:176}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2907}}
  >{\centering\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3605}}
  >{\centering\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3488}}@{}}
\caption{\textbf{Training vs.~Inference System Requirements}: The
fundamental inversion from throughput to latency optimization ripples
through every aspect of system
design.}\label{tbl-training-inference-inversion}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Aspect}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Primary metric} & Throughput (samples/hour) & Latency (P99
ms) \\
\textbf{Acceptable variance} & Hours & Milliseconds \\
\textbf{State management} & Checkpoints (periodic) & Session state
(continuous) \\
\textbf{Batch formation} & Large, controlled & Request-driven,
variable \\
\textbf{Failure tolerance} & Restart from checkpoint & Redirect without
user impact \\
\textbf{Cost structure} & Fixed duration, variable rate & Variable
duration, fixed SLO \\
\end{longtable}

\clearpage

\texttt{003\ tbl-cost-ratios}

\texttt{contents/vol2/inference/inference.qmd:368}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3298}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2021}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2660}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2021}}@{}}
\caption{\textbf{Training vs.~Serving Cost Ratios}: High-QPS
applications like recommendation systems show the most extreme cost
dominance of serving over
training.}\label{tbl-cost-ratios}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Application}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Recommendation (high QPS)} & \$10K--\$100K & \$1M--\$10M &
100--1000\(\times\) \\
\textbf{Search ranking} & \$100K--\$1M & \$10M--\$100M &
100--1000\(\times\) \\
\textbf{LLM API} & \$1M--\$100M & \$10M--\$1B & 10--100\(\times\) \\
\textbf{Internal analytics} & \$1K--\$10K & \$10K--\$100K &
10--100\(\times\) \\
\end{longtable}
