...
const TERMINAL_NAME = 'Labs';
const STATE_HISTORY_KEY = 'labs.commandHistory';
...
/** Run a command in the dedicated Labs terminal */
...
export function rerunLastCommand(): void {
  ...
  if (!last) {
    vscode.window.showWarningMessage('Labs: no previous command to rerun.');
    return;
  }
  rerunCommand(last);
}

/** Show the Labs terminal */
