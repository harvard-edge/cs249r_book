...
.j404-joke { font-size: 1.6rem; font-weight: 500; line-height: 1.45; color: #1a1a1a; margin: 0 auto 1.25rem; font-style: italic; min-height: 4.5rem; }
...
@media (max-width: 540px) { .j404-wrap { margin-top: 2.5rem; } .j404-joke { font-size: 1.3rem; } }
...
<div class="j404-wrap">
  ...
  <div class="j404-illustration">
    <svg viewBox="0 0 680 280" xmlns="http://www.w3.org/2000/svg" font-family="Helvetica Neue, Helvetica, Arial, sans-serif" role="img" aria-labelledby="r404-title r404-desc">
      ...
      <!-- Title -->
      <text x="340" y="22" text-anchor="middle" font-size="11" font-weight="700" fill="#333">Roofline analysis of your request</text>
      <!-- Axes -->
      ...
      <text x="340" y="69" text-anchor="middle" font-size="9" fill="#555">ridge point</text>
      <!-- Region labels -->
      <text x="200" y="155" text-anchor="middle" font-size="9.5" font-weight="700" fill="#4a90c4">memory-bound</text>
      <text x="490" y="120" text-anchor="middle" font-size="9.5" font-weight="700" fill="#3d9e5a">compute-bound</text>
      <!-- "Achievable" italics inside the shaded region -->
      <text x="430" y="200" text-anchor="middle" font-size="8.5" font-style="italic" fill="#999">achievable region</text>
      ...
      <text x="290" y="149" text-anchor="start" font-size="11" font-weight="700" fill="#a31f34">your page (404)</text>
      <!-- Axis labels -->
      <text x="50" y="140" text-anchor="middle" font-size="9.5" fill="#555" transform="rotate(-90 50 140)">throughput (FLOP/s)</text>
      <text x="365" y="265" text-anchor="middle" font-size="9.5" fill="#555">arithmetic intensity (FLOP/byte)</text>
    </svg>
  </div>
    <p class="j404-joke" id="j404-joke">&nbsp;</p>
  <p class="j404-sub">Looks like this lab slipped past our load balancer.</p>
  <div class="j404-actions">
    <a class="j404-btn j404-btn-primary" href="/">← Labs home</a>
...
(function() {
  const jokes = [
    ...
    "Speed of light says the page is still 18 ms away. Patience is a systems virtue.",
    "We trained on this page. It was in the test set the whole time.",
    "p50 says the page exists. p99 disagrees. p99.9 timed out.",
    ...
    "SLA says 99.9 percent. The other 0.1 percent gets the interesting URLs.",
    ...
    "Pipeline bubble fraction hit 100 percent. The whole run was bubble.",
    "Weak scaling held until we asked for the page.",
    "Rendezvous timed out at 30 minutes. The page showed up at 31.",
    "Data parallel replica 17 has a different version of this page. We will not be reconciling.",
    "Tensor parallel rank 0 has the page. Tensor parallel rank 1 disagrees.",
    "Launched the run on 8,192 H100s. Your shard sits on the one in the Reno DC that just lost cooling.",
    "We trained for 47 hours. The page diverged at hour 46.",
    ...
    "Sticky session routing sent you to the replica that crashed at boot.",
    "Sharded across 8 GPUs. The GPU with your shard is running someone else's job.",
    "The load balancer routed you to a replica that exists only in YAML.",
    ...
    "The replica that had this page is in a graceful drain that started yesterday.",
    "Batch size auto-tuner picked 1. We are now serving you, alone, very slowly.",
    "Blue/green deployment is currently green/green. Neither has the page.",
    ...
    "Stale weights served this page until 14 minutes ago. Now nothing serves it.",
    "Vector index has not been rebuilt since the last incident. Neither has trust.",
    "DRAM refresh stole the cycle that would have served this URL.",
    "The thermal sensor read 95 C. The page throttled to a lower frequency, then to zero.",
    "Tensor cores idle. SMs idle. The page is also idle, somewhere on the die.",
    "The page crossed a NUMA boundary and never came home.",
    ...
    "The page was offloaded to swap. There is no swap on a microcontroller.",
    "Gang scheduling failed. 4,095 GPUs are waiting on the one that has this page.",
    "NCCL hung at 86 percent. The remaining 14 percent contained the page.",
    "Slurm allocated the nodes. Slurm did not allocate the page.",
    "Kubernetes evicted the pod that had cached this page. The replacement pulled a stale image.",
    "Elastic training rescaled the world size mid-step. The page belonged to the old world.",
    "Fault recovery loaded the last checkpoint. The page was added after.",
    ...
    "The tokenizer split this URL into bytes nobody had seen before.",
    ...
    "Request queue depth exceeded the batcher's patience. Dropped on the floor.",
    "Batch window closed before your request joined. The next window is in 40 ms, or never.",
    "We serve this URL at 73 percent acceptance rate. Today is in the 27.",
    "This route violated the throughput SLO and was demoted to best-effort.",
    "Traffic shifted 5 percent to the new model. You were in the 5 percent.",
    "Shadow deployment served this page. Production never noticed it was missing.",
    "Data drift detector fired on this URL. The route was retrained out of existence.",
    "Warm pool was sized for predicted traffic. Your traffic was unpredicted.",
    "Prefix cache hit. The prefix matched a different URL by 31 of 32 tokens.",
    "Runbook for this 404 references a wiki that was deprecated with the page.",
    "The page was quantized to INT4 for the edge. The decimal point wandered off.",
    "This page was scheduled on the systolic array. Row 7 is still propagating.",
    "The ridge point is at 76 FLOPs per byte. This page came in at 0.4.",
    "NVLink saturated between GPUs. The page was queued behind the gradients.",
    "Ring AllReduce completed in O(N) hops. The page was at hop N+1.",
    "GPU-direct RDMA bypassed the CPU. It also bypassed this page.",
    "Gradient bucketing flushed the bucket containing this page into the void.",
    "Latency-bandwidth tradeoff: we picked bandwidth. The page needed latency.",
    "Training resumed from a checkpoint that predated the page.",
    "Micro-batch 14 of 16 contained the page. Micro-batch 14 was skipped to fit the schedule.",
    "ZeRO-2 offloaded the gradient for this page to CPU. The CPU was OOM.",
    "Amdahl's Law says the serial fraction was 4 percent. This page is in the 4 percent.",
    "TLB miss. The page table is the page.",
    "Dennard scaling ended. The page kept the leakage and lost the frequency.",
    "False sharing. Two cores wrote adjacent fields. The cache line ping-ponged the page out of existence.",
    "Fat-tree topology was oversubscribed at the spine. The page took the long way and never arrived."
  ];
  function pick() {
