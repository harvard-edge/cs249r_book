...
The current [lab sequence](lab-sequence.md) gives every team a guarded one-axis visual follower, then offers active inspection and retained-object routing. It establishes a common model → MCU permission → physical action → observation path. The syllabus has moved from book front matter to the [instructor materials](../../instructors/vol4/README.md). The [kit sheet](kit-bom.md) and [feasibility plan](feasibility-plan.md) specify a candidate station and required tests. **No station, model runtime, sensor mount, simulator comparison, or SO-101 STM32-to-servo command path has yet been demonstrated by these documents.**
