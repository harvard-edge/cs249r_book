...
# ═══════════════════════════════════════════════════════════════════════════════
# ZONE B: PART A — THE IMPOSSIBILITY WALL
# ═══════════════════════════════════════════════════════════════════════════════
...
def _(mo):
    ...
    mo.hstack([partA_threshold_slider, partA_base_rate_a, partA_base_rate_b], justify="start", gap=1)
    return (partA_threshold_slider, partA_base_rate_a, partA_base_rate_b,)


@app.cell(hide_code=True)
def _(mo, go, np, apply_plotly_theme, COLORS, partA_threshold_slider, partA_base_rate_a, partA_base_rate_b):
...
# ═══════════════════════════════════════════════════════════════════════════════
# ZONE C: PART B — THE FAIRNESS TAX
# ═══════════════════════════════════════════════════════════════════════════════
...
def _(mo):
    ...
    mo.vstack([partB_gap_slider])
    return (partB_gap_slider,)


@app.cell(hide_code=True)
def _(mo, go, np, apply_plotly_theme, COLORS, BASELINE_ACCURACY,
      DP_TAX_PER_GAP, EO_TAX_PER_GAP, EQOP_TAX_PER_GAP, partB_gap_slider):
...
def _(mo, partB_pred, _dp_at_gap):
    ...
    mo.callout(mo.md(f"**{_msg}**"), kind="success" if _diff < 3 else "warn")
    return


# ═══════════════════════════════════════════════════════════════════════════════
# ZONE D: PART C — THE FEEDBACK LOOP
# ═══════════════════════════════════════════════════════════════════════════════
...
def _(mo):
    ...
    return (partC_data_audit, partC_fairness_constraint, partC_output_monitor, partC_feedback_gov,)


@app.cell(hide_code=True)
def _(mo, go, np, apply_plotly_theme, COLORS, INITIAL_BIAS, AMPLIFICATION_RATE,
      partC_data_audit, partC_fairness_constraint, partC_output_monitor, partC_feedback_gov):
...
# ═══════════════════════════════════════════════════════════════════════════════
# ZONE E: PART D — THE RESPONSIBLE AI OVERHEAD BUDGET
# ═══════════════════════════════════════════════════════════════════════════════
...
def _(mo):
    ...
    mo.hstack([partD_metric, partD_monitor, partD_explain], justify="start", gap=1)
    return (partD_metric, partD_monitor, partD_explain,)


@app.cell(hide_code=True)
def _(mo, go, apply_plotly_theme, COLORS,
      INFERENCE_LATENCY_MS, MONITORING_BASIC_MS, MONITORING_FULL_MS,
      LIME_OVERHEAD_MS, SHAP_OVERHEAD_MS, NETWORK_OVERHEAD_MS, SLA_MS,
      BASELINE_ACCURACY, DP_TAX_PER_GAP, EO_TAX_PER_GAP, EQOP_TAX_PER_GAP,
      partD_metric, partD_monitor, partD_explain):
    # Latency calculation
    ...
    return (_accuracy, _disparity, _total_latency,)
...
# ═══════════════════════════════════════════════════════════════════════════════
# ZONE F: PART E — THE FAIRNESS AUDIT PIPELINE
# ═══════════════════════════════════════════════════════════════════════════════
...
@app.cell(hide_code=True)
def _(mo, COLORS, AUDIT_COMPUTE_BASE, partE_sample, partE_test, partE_threshold, partE_ab_days):
    ...
    return (_total_remediation_hours, _total_compute, partD_metric,)
...
def _(mo, ledger, COLORS, partD_metric, _accuracy, _disparity, _total_latency):
    ...
    ledger.save(chapter=15, design={
        ...
        "accuracy_after_fairness": float(f"{_accuracy:.1f}"),
    })
