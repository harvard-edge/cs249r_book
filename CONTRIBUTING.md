...
| Add or fix a TinyTorch module / test / milestone | **TinyTorch** | [`packages/tinytorch/CONTRIBUTING.md`](packages/tinytorch/CONTRIBUTING.md) |
...
| Contribute an MLSys·im model, scenario, or scorecard | **MLSys·im** | [`packages/mlsysim/docs/contributing.qmd`](packages/mlsysim/docs/contributing.qmd) |
| Add a workload to the MLPerf EDU benchmark suite | **MLPerf EDU** | [`packages/mlperf-edu/README.md`](packages/mlperf-edu/README.md) |
| Author or fix a StaffML interview question | **StaffML** | [`staffml/CONTRIBUTING.md`](staffml/CONTRIBUTING.md) |
| Improve teaching materials, syllabi, or rubrics | **Instructors** | [`materials/instructors/README.md`](materials/instructors/README.md) |
| Update slides for a chapter | **Slides** | [`materials/slides/README.md`](materials/slides/README.md) |
...
* **TinyTorch uses the `tito` CLI for everything.** Module status, tests,
  ...
  [`packages/tinytorch/CONTRIBUTING.md`](packages/tinytorch/CONTRIBUTING.md) for the full
  command list. If `tito` isn't on your PATH after
  `pip install -e packages/tinytorch/`, re-activate your venv.
* **TinyTorch source edits need an export step.** When you change a file
  under `packages/tinytorch/src/`, the in-package version under `packages/tinytorch/tinytorch/`
  is regenerated by `tito dev export` (see
  [`packages/tinytorch/CONTRIBUTING.md`](packages/tinytorch/CONTRIBUTING.md), "Module
  Development"). `packages/tinytorch/tinytorch/*` is gitignored — the source of
  truth is `src/`.
...
* **Where each area lives** — the table above is the authoritative map.
  At a glance: `books/` for the textbook, `packages/tinytorch/` for the framework,
  `labs/` for browser labs, `kits/` for hardware recipes, `packages/mlsysim/` for
  the simulator, `materials/instructors/` for teaching materials, `materials/slides/` for
...
`packages/tinytorch/.pre-commit-config.yaml` (markdown collapse, CLI doc checks); run it
...
| TinyTorch | `pip install -r packages/tinytorch/requirements.txt && pip install -e packages/tinytorch/` |
| StaffML / vault-cli | `pip install -e staffml/vault-cli/[dev]` |
| MLSys·im | `pip install -e packages/mlsysim/[dev]` |
| MLPerf EDU | `pip install -e packages/mlperf-edu/[dev]` |
